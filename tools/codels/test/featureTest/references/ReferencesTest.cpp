...
namespace TestLspReference {
    bool LspReferenceTest(TestParam param)
    {
    ...
    class ReferencesTest : public testing::TestWithParam<struct TestParam> {
    protected:
        void SetUp()
        {
            SetUpConfig("references");
        }
    };

    INSTANTIATE_TEST_SUITE_P(Reference, ReferencesTest, testing::ValuesIn(GetTestCaseList("references")));

    TEST_P(ReferencesTest, ReferenceCase)
    {
        TestParam param = GetParam();
        ASSERT_TRUE(LspReferenceTest(param));
    }
}
