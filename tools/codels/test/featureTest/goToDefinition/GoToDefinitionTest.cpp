...
namespace TestLspGoToDefinition {
    bool LspGoToDefinitionTest(TestParam param)
    {
        ...
        bool showErr = CheckResult(expLines, result, TestType::GoToDefinition, info);
        if (!showErr) {
    ...
    class GoToDefinitionTest : public testing::TestWithParam<struct TestParam> {
    protected:
        void SetUp()
        {
            SetUpConfig("goToDefinition");
        }
    };

    INSTANTIATE_TEST_SUITE_P(GoToDefinition, GoToDefinitionTest, testing::ValuesIn(GetTestCaseList("goToDefinition")));

    TEST_P(GoToDefinitionTest, GoToDefinitionCase)
    {
        TestParam param = GetParam();
        ASSERT_TRUE(LspGoToDefinitionTest(param));
    }
}
