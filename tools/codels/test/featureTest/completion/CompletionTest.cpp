...
namespace TestCompletion {
    bool LspCompletionTest(TestParam param)
    {
        ...
        bool showErr = CheckResult(expLines, result, TestType::Completion, reason);
        if (!showErr) {
    ...
    class CompletionTest : public testing::TestWithParam<struct TestParam> {
    protected:
        void SetUp()
        {
            SetUpConfig("completion");
        }
    };

    INSTANTIATE_TEST_SUITE_P(Completion, CompletionTest, testing::ValuesIn(GetTestCaseList("completion")));

    TEST_P(CompletionTest, CompletionCase)
    {
        TestParam param = GetParam();
        ASSERT_TRUE(LspCompletionTest(param));
    }
}
