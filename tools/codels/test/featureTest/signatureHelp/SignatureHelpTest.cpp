...
namespace TestLspSignatureHelp {
    bool LspSignatureHelpTest(TestParam param)
    {
        ...
        bool showErr = CheckSignatureHelpResult(expect, actual, reason);
        if (!showErr) {
    ...
    class SignatureHelpTest : public testing::TestWithParam<struct TestParam> {
    protected:
        void SetUp()
        {
            SetUpConfig("signatureHelp");
        }
    };

    INSTANTIATE_TEST_SUITE_P(SignatureHelp, SignatureHelpTest,
            testing::ValuesIn(GetTestCaseList("signatureHelp")));

    TEST_P(SignatureHelpTest, SignatureHelpCase)
    {
        TestParam param = GetParam();
        ASSERT_TRUE(LspSignatureHelpTest(param));
    }
}
