...
std::vector<Completion *> SourceKit::CodeCompletion::extendCompletions(
    ArrayRef<CodiraResult *> languageResults, CompletionSink &sink,
    CodiraCompletionInfo &info, const NameToPopularityMap *nameToPopularity,
  ...
  if (info.completionContext)
    sink.adoptCodiraSink(info.completionContext->getResultSink());

  std::vector<Completion *> results;
  for (auto *result : languageResults) {
...
void CodeCompletionOrganizer::preSortCompletions(
    toolchain::MutableArrayRef<Completion *> completions) {
  // We do a case-sensitive sort here, then do a case-insensitive sort after any
  // name-based grouping.

  toolchain::array_pod_sort(completions.begin(), completions.end(),
...
bool FilterRules::hideCompletion(const Completion &completion) const {
  return hideCompletion(completion.getCodiraResult(), completion.getFilterName(),
                        completion.getDescription(),
                        completion.getCustomKind());
}

bool FilterRules::hideCompletion(const CodiraResult &completion,
...
static ResultBucket getResultBucket(Item &item, bool hasRequiredTypes,
  ...
  case CodeCompletionResultKind::Declaration:
    ...
    case language::ide::CodeCompletionResultTypeRelation::Convertible:
      return ResultBucket::NormalTypeMatch;
    case language::ide::CodeCompletionResultTypeRelation::NotApplicable:
    case language::ide::CodeCompletionResultTypeRelation::Unknown:
    case language::ide::CodeCompletionResultTypeRelation::Unrelated:
      return ResultBucket::Normal;
    case language::ide::CodeCompletionResultTypeRelation::Invalid:
      if (!skipMetaGroups)
        return ResultBucket::NotRecommended;
      return ResultBucket::Normal;
    }
  case CodeCompletionResultKind::BuiltinOperator:
    toolchain_unreachable("operators should be handled above");
  }
}
...
static bool isTopNonLiteralResult(Item &item, ResultBucket literalBucket) {
  ...
  case ResultBucket::LiteralTypeMatch:
    return completion->getExpectedTypeRelation() >=
           CodeCompletionResultTypeRelation::Convertible;
  default:
    toolchain_unreachable("invalid literal bucket");
  }
}
...
static void sortRecursive(const Options &options, Group *group,
  ...
  if (options.sortByName) {
    toolchain::array_pod_sort(contents.begin(), contents.end(),
...
Completion *CompletionBuilder::finish() {
  const CodiraResult *newBase = &this->base;
  toolchain::SmallString<64> nameStorage;
  if (modified) {
    ...
    newBase = base.withContextFreeResultSemanticContextAndFlair(
        *contextFreeResult, semanticContext, flair, sink.codeSink);
  }
