...
private:
  void printTypeRef(
    ...
    printXML("<Type usr=\"");
    CodiraLangSupport::printUSR(TD, OS);
    printXML("\">");
...
static StringRef getTagForDecl(const Decl *D, bool isRef) {
  auto UID = CodiraLangSupport::getUIDForDecl(D, isRef);
  static const char *prefix = "source.lang.code.";
...
static StringRef getTagForParameter(PrintStructureKind context) {
  ...
  case PrintStructureKind::FunctionParameterType:
    ...
    toolchain_unreachable("ignored structure kind");
  }
  toolchain_unreachable("unexpected structure kind");
}
...
public:
  PrintContext(const Decl *D) : value(uintptr_t(D)) {
    static_assert(toolchain::PointerLikeTypeTraits<Decl *>::NumLowBitsAvailable >=
                      tagShift,
...
private:
  ...
  void openTagWithUSRForDecl(StringRef tag, const ValueDecl *VD) {
    OS << "<" << tag << " usr=\"";
    CodiraLangSupport::printUSR(VD, OS);
    OS << "\">";
  }
...
private:
  ...
  toolchain::SmallVector<PrintContext, 3> contextStack;
...
void CodiraLangSupport::printFullyAnnotatedDeclaration(const ValueDecl *VD,
                                                      Type BaseTy,
                                                      raw_ostream &OS) {
...
void CodiraLangSupport::printFullyAnnotatedDeclaration(const ExtensionDecl *ED,
                                                      raw_ostream &OS) {
  FullyAnnotatedDeclarationPrinter Printer(OS);
  PrintOptions PO = PrintOptions::printQuickHelpDeclaration();
  ED->print(Printer, PO);
...
void CodiraLangSupport::printFullyAnnotatedSynthesizedDeclaration(
    const language::ValueDecl *VD, TypeOrExtensionDecl Target,
...
void CodiraLangSupport::printFullyAnnotatedSynthesizedDeclaration(
    const language::ExtensionDecl *ED, TypeOrExtensionDecl Target,
    toolchain::raw_ostream &OS) {
  FullyAnnotatedDeclarationPrinter Printer(OS);
  PrintOptions PO = PrintOptions::printQuickHelpDeclaration();
  PO.initForSynthesizedExtension(Target);
...
static void walkRelatedDecls(const ValueDecl *VD, const FnTy &Fn) {
  ...
  toolchain::SmallDenseMap<DeclName, unsigned, 16> NamesSeen;
...
//===----------------------------------------------------------------------===//
// CodiraLangSupport::getCursorInfo
//===----------------------------------------------------------------------===//
...
static StringRef getSourceToken(unsigned Offset,
  ...
  SourceManager SM;
  auto MemBufRef = toolchain::MemoryBuffer::getMemBuffer(MemBuf->getBuffer(),
...
static void mapLocToLatestSnapshot(
    CodiraLangSupport &Lang, LocationInfo &Location,
...
static bool passCursorInfoForModule(ModuleEntity Mod,
  ...
  Symbol.Kind = CodiraLangSupport::getUIDForModuleRef();
  ...
  if (auto MD = Mod.getAsCodiraModule()) {
    ide::collectModuleGroups(const_cast<ModuleDecl *>(MD), ModuleGroups);
    Symbol.ModuleGroupArray = toolchain::ArrayRef(ModuleGroups);
  }
...
static void addRefactorings(
  ...
  for (auto info : availableInfos) {
    auto uid = CodiraLangSupport::getUIDForRefactoringKind(info.Kind);
    bool hasRefactoringWithSameUID =
        toolchain::any_of(intoLangInfos, [&](RefactoringInfo &existing) {
...
static StringRef copyAndClearString(toolchain::BumpPtrAllocator &Allocator,
...
template <typename T>
static ArrayRef<T> copyAndClearArray(toolchain::BumpPtrAllocator &Allocator,
                                     SmallVectorImpl<T> &Array) {
  auto Ref = copyArray(Allocator, toolchain::ArrayRef(Array));
  Array.clear();
  return Ref;
}
...
#include "language/Basic/MacroRoles.def"
      ...
      toolchain_unreachable("All switch cases either explicitly continue or return");
    }
...
fillSymbolInfo(CursorSymbolInfo &Symbol, const DeclInfo &DInfo,
               SourceLoc CursorLoc, bool AddSymbolGraph, CodiraLangSupport &Lang,
               const CompilerInvocation &Invoc,
  ...
  LANGUAGE_DEFER { DInfo.VD->setInterfaceType(OriginalInterfaceType); };

  SmallString<256> Buffer;
  SmallVector<StringRef, 4> Strings;
  toolchain::raw_svector_ostream OS(Buffer);

  Symbol.DeclarationLang = CodiraLangSupport::getUIDForDeclLanguage(DInfo.VD);
  Symbol.Kind = CodiraLangSupport::getUIDForDecl(DInfo.VD, DInfo.IsRef);

  CodiraLangSupport::printDisplayName(DInfo.VD, OS);
  Symbol.Name = copyAndClearString(Allocator, Buffer);

  CodiraLangSupport::printUSR(DInfo.OriginalProperty, OS);
  if (DInfo.InSynthesizedExtension) {
    OS << LangSupport::SynthesizedUSRSeparator;
    CodiraLangSupport::printUSR(DInfo.BaseType->getAnyNominal(), OS);
  }
  ...
  CodiraLangSupport::printFullyAnnotatedDeclaration(DInfo.VD, DInfo.BaseType,
                                                   OS);
  ...
  if (AddSymbolGraph) {
    ...
    for (auto &Component : PathComponents) {
      CodiraLangSupport::printUSR(Component.VD, OS);
      Parents.emplace_back(Component.Title.str().copy(Allocator),
                           Component.Kind,
                           copyAndClearString(Allocator, Buffer));
    };
    Symbol.ParentContexts = copyArray(Allocator, toolchain::ArrayRef(Parents));
    ...
    for (auto &FI: FragmentInfos) {
      ...
      CodiraLangSupport::printUSR(FI.VD, OS);
      ReferencedDecls.emplace_back(
          copyAndClearString(Allocator, Buffer),
          CodiraLangSupport::getUIDForDeclLanguage(FI.VD),
          language::getAccessLevelSpelling(FI.VD->getFormalAccess()), Filename,
          getModuleName(FI.VD, Allocator),
          FI.VD->getModuleContext()->isNonUserModule(), FI.VD->isSPI(),
          copyArray(Allocator, toolchain::ArrayRef(FIParents)));
    }
    Symbol.ReferencedSymbols =
        copyArray(Allocator, toolchain::ArrayRef(ReferencedDecls));
  }
  ...
  walkRelatedDecls(DInfo.VD, [&](const ValueDecl *RelatedDecl,
    ...
    OS << "<RelatedName usr=\"";
    CodiraLangSupport::printUSR(RelatedDecl, OS);
    OS << "\">";
    if (isa<AbstractFunctionDecl>(RelatedDecl) && DuplicateName) {
      ...
      toolchain::raw_svector_ostream RelatedOS(RelatedBuffer);
      CodiraLangSupport::printDisplayName(RelatedDecl, RelatedOS);
      language::markup::appendWithXMLEscaping(OS, RelatedBuffer);
    }
    OS << "</RelatedName>";
  ...
  for (auto *ReceiverTy : DInfo.ReceiverTypes) {
    if (!CodiraLangSupport::printUSR(ReceiverTy, OS))
      Strings.push_back(copyAndClearString(Allocator, Buffer));
  }
...
static bool addCursorInfoForLiteral(
    CursorInfoData &Data, Expr *LitExpr, CodiraLangSupport &Lang,
  ...
  bool Success = true;
  toolchain::handleAllErrors(std::move(Err), [&](const toolchain::StringError &E) {
...
addCursorInfoForDecl(CursorInfoData &Data, ResolvedValueRefCursorInfoPtr Info,
                     ...
                     CodiraLangSupport &Lang, const CompilerInvocation &Invoc,
    ...
    for (auto D : Info->getShorthandShadowedDecls()) {
      ...
      if (auto Err =
              fillSymbolInfo(SymbolInfo, DInfo, Info->getLoc(), AddSymbolGraph,
                             Lang, Invoc, PreviousSnaps, Data.Allocator)) {
        // Ignore but make sure to remove the partially-filled symbol
        toolchain::handleAllErrors(std::move(Err),
                              [](const toolchain::StringError &E) {});
        Data.Symbols.pop_back();
      }
    }
  }
...
static DeclName getCodiraDeclName(const ValueDecl *VD,
                                 NameTranslatingInfo &Info) {
  auto &Ctx = VD->getDeclContext()->getASTContext();
  assert(CodiraLangSupport::getNameKindForUID(Info.NameKind) == NameKind::Codira);
  ...
  return DeclName(Ctx, BaseName, toolchain::ArrayRef(Args));
}
...
static bool passNameInfoForDecl(
  ...
  switch (CodiraLangSupport::getNameKindForUID(Info.NameKind)) {
  case NameKind::Codira: {
    NameTranslatingInfo Result;
    auto DeclName = getCodiraDeclName(VD, Info);
    if (!DeclName) {
      Diagnostic = "Unable to resolve Codira declaration name.";
      return false;
    }
    auto ResultPair =
        language::objc_translation::getObjCNameForCodiraDecl(VD, DeclName);
    Identifier Name = ResultPair.first;
    if (!Name.empty()) {
      Result.NameKind = CodiraLangSupport::getUIDForNameKind(NameKind::ObjC);
  ...
  case NameKind::ObjC: {
    ...
    NameTranslatingInfo Result;
    Result.NameKind = CodiraLangSupport::getUIDForNameKind(NameKind::Codira);
    Result.BaseName = Name.getBaseName().userFacingName();
    toolchain::transform(Name.getArgumentNames(),
...
class CursorRangeInfoConsumer : public CodiraASTConsumer {
protected:
...
protected:
  ArrayRef<ImmutableTextSnapshotRef> getPreviousASTSnaps() {
    return toolchain::ArrayRef(PreviousASTSnaps);
  }

public:
  CursorRangeInfoConsumer(StringRef PrimaryFilePath, StringRef InputBufferName,
                          unsigned Offset, unsigned Length,
                          CodiraLangSupport &Lang, CodiraInvocationRef ASTInvok,
...
SourceFile *SourceKit::retrieveInputFile(StringRef inputBufferName,
  ...
  if (!inputBufferID) {
    ...
    if (haveRealPath)
      return nullptr;
    std::string realPath =
        CodiraLangSupport::resolvePathSymlinks(inputBufferName);
...
static void resolveCursor(
    CodiraLangSupport &Lang, StringRef PrimaryFile, StringRef InputBufferName,
    unsigned Offset, unsigned Length, bool Actionables, bool SymbolGraph,
    CodiraInvocationRef Invok, bool TryExistingAST,
    bool CancelOnSubsequentRequest,
  ...
  public:
    ...
    void handlePrimaryAST(ASTUnitRef AstUnit) override {
      ...
      toolchain::SmallVector<RefactoringInfo, 8> Actions;
...
static void computeDiagnostics(
    CodiraLangSupport &Lang, CodiraInvocationRef Invok,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
    SourceKitCancellationToken CancellationToken,
    std::function<void(const RequestResult<DiagnosticsResult> &)> Receiver) {

  class DiagnosticsConsumer : public CodiraASTConsumer {
    std::function<void(const RequestResult<DiagnosticsResult> &)> Receiver;
...
static void resolveName(
    CodiraLangSupport &Lang, StringRef PrimaryFilePath,
    StringRef InputBufferName, unsigned Offset, CodiraInvocationRef Invok,
  ...
  public:
    ...
    void handlePrimaryAST(ASTUnitRef AstUnit) override {
      ...
      case CursorInfoKind::Invalid:
        toolchain_unreachable("bad sema token kind.");
      }
    }
  ...
  Lang.getASTManager()->processASTAsync(
      ...
      CancellationToken, toolchain::vfs::getRealFileSystem());
}

static void
resolveRange(CodiraLangSupport &Lang, StringRef PrimaryFilePath,
  ...
  public:
    RangeInfoConsumer(
        StringRef PrimaryFilePath, StringRef InputBufferName, unsigned Offset,
        unsigned Length, CodiraLangSupport &Lang, CodiraInvocationRef ASTInvok,
        bool TryExistingAST, bool CancelOnSubsequentRequest,
        SourceKitCancellationToken CancellationToken,
        std::function<void(const RequestResult<RangeInfo> &)> Receiver)
...
static void deliverCursorInfoResults(
    std::function<void(const RequestResult<CursorInfoData> &)> Receiver,
    CancellableResult<CursorInfoResults> Results, CodiraLangSupport &Lang,
...
void CodiraLangSupport::getCursorInfo(
    StringRef PrimaryFilePath, StringRef InputBufferName, unsigned Offset,
    unsigned Length, bool Actionables, bool SymbolGraph,
  ...
  if (auto IFaceGenRef = IFaceGenContexts.get(PrimaryFilePath)) {
    IFaceGenRef->accessASTAsync([this, IFaceGenRef, Offset, Actionables,
      ...
      CodiraInterfaceGenContext::ResolvedEntity Entity;
  ...
  // cursor info.
  std::shared_ptr<toolchain::MemoryBuffer> InputBuffer;
  if (InputBufferName.empty() && Length == 0) {
    std::string InputFileError;
    InputBuffer =
        std::shared_ptr<toolchain::MemoryBuffer>(getASTManager()->getMemoryBuffer(
  ...
  auto ASTBasedReceiver = [this, CancellationToken, Invok, InputBuffer,
    ...
    if (Res.isValue()) {
      ...
      bool AllResultsHaveErrorType =
          toolchain::all_of(Res.value().Symbols, [](const CursorSymbolInfo &Symbol) {
...
void CodiraLangSupport::getDiagnostics(
...
void CodiraLangSupport::getRangeInfo(
    StringRef PrimaryFilePath, StringRef InputBufferName, unsigned Offset,
...
void CodiraLangSupport::getNameInfo(
    StringRef PrimaryFilePath, StringRef InputBufferName, unsigned Offset,
...
static void resolveCursorFromUSR(
    CodiraLangSupport &Lang, StringRef InputFile, StringRef USR,
    CodiraInvocationRef Invok, bool TryExistingAST,
    bool CancelOnSubsequentRequest,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> fileSystem,
    SourceKitCancellationToken CancellationToken,
  ...
  class CursorInfoConsumer : public CodiraASTConsumer {
  ...
  public:
    CursorInfoConsumer(
        StringRef InputFile, StringRef USR, CodiraLangSupport &Lang,
        CodiraInvocationRef ASTInvok, bool TryExistingAST,
        bool CancelOnSubsequentRequest,
        SourceKitCancellationToken CancellationToken,
        std::function<void(const RequestResult<CursorInfoData> &)> Receiver)
...
void CodiraLangSupport::getCursorInfoFromUSR(
...
//===----------------------------------------------------------------------===//
// CodiraLangSupport::findUSRRange
//===----------------------------------------------------------------------===//

std::optional<std::pair<unsigned, unsigned>>
CodiraLangSupport::findUSRRange(StringRef DocumentName, StringRef USR) {
...
//===----------------------------------------------------------------------===//
// CodiraLangSupport::findRelatedIdentifiersInFile
//===----------------------------------------------------------------------===//

void CodiraLangSupport::findRelatedIdentifiersInFile(
    StringRef PrimaryFilePath, StringRef InputBufferName, unsigned Offset,
  ...
  class RelatedIdConsumer : public CodiraASTConsumer {
    std::string InputFile;
    unsigned Offset;
...
//===----------------------------------------------------------------------===//
// CodiraLangSupport::findActiveRegionsInFile
//===----------------------------------------------------------------------===//

void CodiraLangSupport::findActiveRegionsInFile(
  ...
  class IfConfigConsumer : public CodiraASTConsumer {
    std::string InputFile;
    std::function<void(const RequestResult<ActiveRegionsInfo> &)> Receiver;
    CodiraInvocationRef Invok;
...
//===----------------------------------------------------------------------===//
// CodiraLangSupport::semanticRefactoring
//===----------------------------------------------------------------------===//
...
void CodiraLangSupport::semanticRefactoring(
  ...
  class SemaRefactoringConsumer : public CodiraASTConsumer {
  ...
  public:
    ...
    void handlePrimaryAST(ASTUnitRef AstUnit) override {
      ...
      refactorCodiraModule(CompIns.getMainModule(), Opts, EditConsumer,
                          EditConsumer);
    }
...
void CodiraLangSupport::collectExpressionTypes(
    StringRef PrimaryFilePath, StringRef InputBufferName,
  ...
  assert(Invok);
  class ExpressionTypeCollector: public CodiraASTConsumer {
...
void CodiraLangSupport::collectVariableTypes(
    StringRef PrimaryFilePath, StringRef InputBufferName,
  ...
  class VariableTypeCollectorASTConsumer : public CodiraASTConsumer {
  private:
    std::function<void(const RequestResult<VariableTypesInFile> &)> Receiver;
    std::string InputFile;
  ...
  public:
    ...
    void handlePrimaryAST(ASTUnitRef AstUnit) override {
      ...
      toolchain::raw_string_ostream OS(TypeBuffer);
