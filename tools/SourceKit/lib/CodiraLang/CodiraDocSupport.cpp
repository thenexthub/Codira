...
class AnnotatingPrinter : public StreamPrinter {
  ...
  typedef toolchain::SmallDenseMap<ValueDecl*, ValueDecl*> DefaultImplementMap;
  toolchain::SmallDenseMap<ProtocolDecl*, DefaultImplementMap> AllDefaultMaps;
  ...
  void initDefaultMapToUse(const Decl *D) {
    ...
    if (auto NTD = ED->getExtendedNominal()) {
      if (auto *PD = dyn_cast<ProtocolDecl>(NTD)) {
        ...
        if (Pair.second) {
          language::collectDefaultImplementationForProtocolMembers(PD,
...
static void initDocGenericParams(const Decl *D, DocEntityInfo &Info,
  ...
  for (auto Req: Reqs) {
    ...
    std::string ReqStr;
    toolchain::raw_string_ostream OS(ReqStr);
    PrintOptions Opts;
    if (Req.getKind() != RequirementKind::Layout) {
...
static bool initDocEntityInfo(const Decl *D,
  ...
  if (!D || isa<ParamDecl>(D) ||
    ...
    Info.Kind = CodiraLangSupport::getUIDForLocalVar(IsRef);
    if (D) {
      toolchain::raw_svector_ostream OS(Info.Name);
      CodiraLangSupport::printDisplayName(cast<ValueDecl>(D), OS);
    } else {
      Info.Name = "_";
    }
  ...
  if (IsSynthesizedExtension) {
    Info.Kind =
        CodiraLangSupport::getUIDForExtensionOfDecl(SynthesizedTargetNTD);
  } else
    Info.Kind = CodiraLangSupport::getUIDForDecl(D, IsRef);

  if (Info.Kind.isInvalid())
    return true;
  if (const auto *VD = dyn_cast<ValueDecl>(D)) {
    toolchain::raw_svector_ostream NameOS(Info.Name);
    CodiraLangSupport::printDisplayName(VD, NameOS);
    {
      toolchain::raw_svector_ostream OS(Info.USR);
      CodiraLangSupport::printUSR(VD, OS, /*distinguishSynthesizedDecls*/ true);
      if (SynthesizedTarget) {
        OS << CodiraLangSupport::SynthesizedUSRSeparator;
        CodiraLangSupport::printUSR(SynthesizedTargetNTD, OS);
        {
          toolchain::raw_svector_ostream OS(Info.OriginalUSR);
  ...
  if (DefaultImplementationOf) {
    toolchain::raw_svector_ostream OS(Info.ProvideImplementationOfUSR);
    CodiraLangSupport::printUSR((const ValueDecl*)DefaultImplementationOf, OS);
  }
  ...
  if (!IsRef) {
    toolchain::raw_svector_ostream OS(Info.DocCommentAsXML);

    {
      toolchain::SmallString<128> DocBuffer;
      {
        toolchain::raw_svector_ostream OSS(DocBuffer);
    ...
    toolchain::raw_svector_ostream LocalizationKeyOS(Info.LocalizationKey);
    ide::getLocalizationKey(D, LocalizationKeyOS);

    if (auto *VD = dyn_cast<ValueDecl>(D)) {
      toolchain::raw_svector_ostream OS(Info.FullyAnnotatedDecl);
      if (SynthesizedTarget)
        CodiraLangSupport::printFullyAnnotatedSynthesizedDeclaration(
            VD, SynthesizedTarget, OS);
      else
        CodiraLangSupport::printFullyAnnotatedDeclaration(VD, Type(), OS);
      ...
      if (SynthesizedTarget)
        ...
        CodiraLangSupport::printFullyAnnotatedDeclaration(ED, OS);
    }
    ...
    if (DeclaringModForCrossImport) {
      ...
      if (MD->getRequiredBystandersIfCrossImportOverlay(
        ...
        toolchain_unreachable("DeclaringModForCrossImport not correct?");
      }
    }
  }
    ...
    case DeclContextKind::FileUnit: {
      if (auto *CD = D->getClangDecl()) {
        if (auto *M = CD->getImportedOwningModule()) {
          if (M->isSubModule()) {
            toolchain::raw_svector_ostream OS(Info.SubModuleName);
...
static void reportRelated(ASTContext &Ctx, const Decl *D,
  ...
  if (const auto *ED = dyn_cast<ExtensionDecl>(D)) {
    ...
    toolchain::SmallVector<InheritedEntry, 4> AllInheritsForPrinting;
...
static void reportAvailabilityAttributes(ASTContext &Ctx, const Decl *D,
                                         DocInfoConsumer &Consumer) {
  static UIdent AvailableAttrKind("source.lang.code.attribute.availability");
...
public:
  ...
  bool walkToNodePre(SyntaxNode Node) override {
    ...
    DocEntityInfo Info;
    Info.Kind = CodiraLangSupport::getUIDForSyntaxNodeKind(Node.Kind);
    Info.Offset = Offset;
    Info.Length = Length;
  ...
  bool walkToSubStructurePre(SyntaxStructureNode Node) override {
    if (Node.Kind == SyntaxStructureKind::Parameter) {
      ...
      // Argument
      static UIdent KindArgument("source.lang.code.syntaxtype.argument");
      passAnnotation(KindArgument, Param->getArgumentNameLoc(),
                     Param->getArgumentName());
      LastArgLoc = Param->getArgumentNameLoc();

      // Parameter
      static UIdent KindParameter("source.lang.code.syntaxtype.parameter");
...
class FuncWalker : public ASTWalker {
  SourceManager &SM;
  unsigned BufferID;
  toolchain::MutableArrayRef<TextEntity*> FuncEnts;
...
public:
  ...
  PreWalkAction walkToDeclPre(Decl *D) override {
    ...
    FuncEnts = toolchain::MutableArrayRef<TextEntity*>(Found+1, FuncEnts.end());
    return Action::SkipNode(); // skip body.
  }
};
} // end anonymous namespace
...
static void addParameterEntities(CompilerInstance &CI,
  ...
  toolchain::MutableArrayRef<TextEntity*> FuncEnts(FuncEntities.data(),
                                              FuncEntities.size());
...
public:
  ...
  ~Implementation() {
    ...
    for (unsigned I = 0, N = UIds.size(); I < N; I ++) {
      ...
      Results.push_back({UIds[I], toolchain::ArrayRef(AllEdits.data() + Pair.first,
  ...
  void accept(SourceManager &SM, RegionType RegionType,
    ...
    toolchain::transform(
        ...
        [&](const Replacement &R) -> Edit {
          ...
          toolchain::transform(
              ...
              [](language::ide::NoteRegion R) -> SourceKit::NoteRegion {
                return {CodiraLangSupport::getUIDForRefactoringRangeKind(R.Kind),
                        R.StartLine,
                        R.StartColumn,
                        R.EndLine,
                        R.EndColumn,
    ...
    UIds.push_back(CodiraLangSupport::getUIDForRegionType(RegionType));
  }
};
...
static std::vector<CategorizedRenameRanges> getCategorizedRenameRanges(
  ...
  for (SyntacticRenameRangeDetails RangeDetails : SyntacticRenameRanges) {
    ...
    CategorizedRanges.Category =
        CodiraLangSupport::getUIDForRegionType(RangeDetails.Type);
    for (const auto &R : RangeDetails.Ranges) {
      ...
      Result.Kind =
          CodiraLangSupport::getUIDForRefactoringRangeKind(R.RangeKind);
...
CodiraLangSupport::findRenameRanges(toolchain::MemoryBuffer *InputBuf,
  ...
  auto SyntacticRenameRanges =
      language::ide::findSyntacticRenameRanges(SF, RenameLocs,
                                            /*NewName=*/StringRef());
...
void CodiraLangSupport::findLocalRenameRanges(
  ...
  struct LocalRenameRangeASTConsumer : public CodiraASTConsumer {
    ...
    void handlePrimaryAST(ASTUnitRef AstUnit) override {
      ...
      language::ide::RangeConfig Range{SF.getBufferID(), Line, Column, Length};
      SourceManager &SM = SF.getASTContext().SourceMgr;
      auto SyntacticRenameRanges =
          language::ide::findLocalRenameRanges(&SF, Range);
      auto Result =
...
SourceFile *CodiraLangSupport::getSyntacticSourceFile(
...
void CodiraLangSupport::getDocInfo(toolchain::MemoryBuffer *InputBuf,
                                  StringRef ModuleName,
                                  ArrayRef<const char *> Args,
                                  DocInfoConsumer &Consumer) {
  CompilerInstance CI;
...
findModuleGroups(StringRef ModuleName, ArrayRef<const char *> Args,
  ...
  toolchain::SmallVector<StringRef, 0> Groups;
