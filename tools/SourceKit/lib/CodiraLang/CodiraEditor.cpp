...
static std::vector<unsigned> getSortedBufferIDs(
    const toolchain::DenseMap<unsigned, std::vector<DiagnosticEntryInfo>> &Map) {
  std::vector<unsigned> bufferIDs;
  bufferIDs.reserve(Map.size());
  for (auto I = Map.begin(), E = Map.end(); I != E; ++I) {
    bufferIDs.push_back(I->getFirst());
  }
  toolchain::array_pod_sort(bufferIDs.begin(), bufferIDs.end());
  return bufferIDs;
}
...
void EditorDiagConsumer::handleDiagnostic(SourceManager &SM,
  ...
  case DiagnosticKind::Remark:
    toolchain_unreachable("already covered");
  }
...
CodiraEditorDocumentRef
CodiraEditorDocumentFileMap::getByUnresolvedName(StringRef FilePath) {
...
CodiraEditorDocumentRef
CodiraEditorDocumentFileMap::findByPath(StringRef FilePath, bool IsRealpath) {
  CodiraEditorDocumentRef EditorDoc;

  std::string Scratch;
  if (!IsRealpath) {
    Scratch = CodiraLangSupport::resolvePathSymlinks(FilePath);
    FilePath = Scratch;
  }
  Queue.dispatchSync([&]{
    for (auto &Entry : Docs) {
...
bool CodiraEditorDocumentFileMap::getOrUpdate(
    StringRef FilePath, CodiraLangSupport &LangSupport,
    CodiraEditorDocumentRef &EditorDoc) {

  bool found = false;

  std::string ResolvedPath = CodiraLangSupport::resolvePathSymlinks(FilePath);
...
CodiraEditorDocumentRef CodiraEditorDocumentFileMap::remove(StringRef FilePath) {
  CodiraEditorDocumentRef Removed;
  Queue.dispatchBarrierSync([&]{
...
struct CodiraSyntaxToken {
  unsigned Offset;
  unsigned Length:24;
  SyntaxNodeKind Kind:8;

  static CodiraSyntaxToken createInvalid() {
    return {0, 0, SyntaxNodeKind::AttributeBuiltin};
  }

  CodiraSyntaxToken(unsigned Offset, unsigned Length, SyntaxNodeKind Kind)
  ...
  bool operator==(const CodiraSyntaxToken &Other) const {
...
struct CodiraEditorCharRange {
  unsigned Offset;
  unsigned EndOffset;

  CodiraEditorCharRange(unsigned Offset, unsigned EndOffset) :
    Offset(Offset), EndOffset(EndOffset) {}

  CodiraEditorCharRange(CodiraSyntaxToken Token) :
  ...
  bool intersects(const CodiraSyntaxToken &Token) const {
    return this->Offset < (Token.endOffset()) && this->EndOffset > Token.Offset;
  }
  void extendToInclude(const CodiraEditorCharRange &Range) {
...
struct CodiraSyntaxMap {
  std::vector<CodiraSyntaxToken> Tokens;

  explicit CodiraSyntaxMap(unsigned Capacity = 0) {
    if (Capacity)
      Tokens.reserve(Capacity);
  }

  void addToken(const CodiraSyntaxToken &Token) {
    assert(Tokens.empty() || Token.Offset >= Tokens.back().Offset);
    Tokens.push_back(Token);
  }

  /// Merge this nested token into the last token that was added
  void mergeToken(const CodiraSyntaxToken &Token) {
    if (Tokens.empty()) {
      Tokens.push_back(Token);
      return;
    }
  ...
  std::optional<CodiraEditorCharRange>
  adjustForReplacement(unsigned Offset, unsigned Len, unsigned NewLen) {
    ...
    CodiraEditorCharRange Affected = { /*Offset=*/ReplacedStart,
    ...
    while (Token != Tokens.end() && Token->Offset < ReplacedEnd) {
      ...
      *Token = CodiraSyntaxToken::createInvalid();
      ++Token;
    }
  ...
  /// Passes each token in this CodiraSyntaxMap to the given \p Consumer
  void forEach(EditorConsumer &Consumer) {
    for (auto &Token: Tokens) {
      auto Kind = CodiraLangSupport::getUIDForSyntaxNodeKind(Token.Kind);
      Consumer.handleSyntaxMap(Token.Offset, Token.Length, Kind);
    }
  }

  /// Finds the delta between the given CodiraSyntaxMap, \p Prev, and this one.
  ...
  ///
  /// Returns true if this CodiraSyntaxMap is different to \p Prev.
  bool forEachChanged(const CodiraSyntaxMap &Prev,
                      std::optional<CodiraEditorCharRange> &Affected,
                      EditorConsumer &Consumer) const {
    typedef std::vector<CodiraSyntaxToken>::const_iterator ForwardIt;
    typedef std::vector<CodiraSyntaxToken>::const_reverse_iterator ReverseIt;
    ...
    for (; From < To.base(); ++From) {
      auto Kind = CodiraLangSupport::getUIDForSyntaxNodeKind(From->Kind);
...
class CodiraDocumentSemanticInfo :
    public ThreadSafeRefCountedBase<CodiraDocumentSemanticInfo> {

  const std::string Filename;
  std::weak_ptr<CodiraASTManager> ASTMgr;
  std::shared_ptr<NotificationCenter> NotificationCtr;
  ThreadSafeRefCntPtr<CodiraInvocation> InvokRef;
  ...
  std::vector<CodiraSemanticToken> SemaToks;
...
public:
  CodiraDocumentSemanticInfo(
      StringRef Filename, std::weak_ptr<CodiraASTManager> ASTMgr,
      std::shared_ptr<NotificationCenter> NotificationCtr,
      toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> fileSystem)
      : Filename(Filename), ASTMgr(ASTMgr), NotificationCtr(NotificationCtr),
        fileSystem(fileSystem) {}

  CodiraInvocationRef getInvocation() const {
    return InvokRef;
  }
  ...
  void readSemanticInfo(ImmutableTextSnapshotRef NewSnapshot,
                        std::vector<CodiraSemanticToken> &Tokens,
  ...
  void updateSemanticInfo(std::vector<CodiraSemanticToken> Toks,
...
private:
  std::vector<CodiraSemanticToken> takeSemanticTokens(
...
class CodiraDocumentSyntaxInfo {
  SourceManager SM;
...
public:
  CodiraDocumentSyntaxInfo(const CompilerInvocation &CompInv,
                          ImmutableTextSnapshotRef Snapshot,
                          std::vector<std::string> &Args,
                          StringRef FilePath)
        : Args(Args), PrimaryFile(FilePath) {

    std::unique_ptr<toolchain::MemoryBuffer> BufCopy =
  ...
  void parseIfNeeded() {
    if (!IsParsed) {
      ...
      static WorkQueue BigStackQueue{
          ...
          "CodiraDocumentSyntaxInfo::parseIfNeeded.BigStackQueue"};
...
uint64_t CodiraDocumentSemanticInfo::getASTGeneration() const {
  toolchain::sys::ScopedLock L(Mtx);
  return ASTGeneration;
}

void CodiraDocumentSemanticInfo::readSemanticInfo(
    ImmutableTextSnapshotRef NewSnapshot,
    std::vector<CodiraSemanticToken> &Tokens,
    std::optional<std::vector<DiagnosticEntryInfo>> &Diags,
    ArrayRef<DiagnosticEntryInfo> ParserDiags) {
...
std::vector<CodiraSemanticToken>
CodiraDocumentSemanticInfo::takeSemanticTokens(
    ImmutableTextSnapshotRef NewSnapshot) {
  ...
  TokSnapshot->foreachReplaceUntil(NewSnapshot,
    [&](ReplaceImmutableTextUpdateRef Upd) -> bool {
      ...
      auto ReplaceBegin = std::lower_bound(SemaToks.begin(), SemaToks.end(),
          Upd->getByteOffset(),
          [&](const CodiraSemanticToken &Tok, unsigned StartOffset) -> bool {
            return Tok.ByteOffset+Tok.Length < StartOffset;
          });

      std::vector<CodiraSemanticToken>::iterator ReplaceEnd;
      if (ReplaceBegin == SemaToks.end()) {
        ...
        ReplaceEnd = std::upper_bound(ReplaceBegin, SemaToks.end(),
            ...
            [&](unsigned EndOffset, const CodiraSemanticToken &Tok) -> bool {
      ...
      if (Delta != 0) {
        for (std::vector<CodiraSemanticToken>::iterator
...
std::optional<std::vector<DiagnosticEntryInfo>>
CodiraDocumentSemanticInfo::getSemanticDiagnostics(
    ImmutableTextSnapshotRef NewSnapshot,
    ArrayRef<DiagnosticEntryInfo> ParserDiags) {
...
void CodiraDocumentSemanticInfo::updateSemanticInfo(
    std::vector<CodiraSemanticToken> Toks,
    std::vector<DiagnosticEntryInfo> Diags,
    ImmutableTextSnapshotRef Snapshot,
    uint64_t ASTGeneration) {
...
#include "language/Basic/MacroRoles.def"
  ...
  bool visitDeclReference(ValueDecl *D, SourceRange Range, TypeDecl *CtorTyRef,
    ...
    if (D == D->getASTContext().getOptionalNoneDecl() &&
      ...
      // If a 'nil' literal occurs in a language-case statement, it gets replaced
...
class AnnotAndDiagASTConsumer : public CodiraASTConsumer {
  EditableTextBufferRef EditableBuffer;
  RefPtr<CodiraDocumentSemanticInfo> SemaInfoRef;
...
void CodiraDocumentSemanticInfo::processLatestSnapshotAsync(
    EditableTextBufferRef EditableBuffer,
    SourceKitCancellationToken CancellationToken) {

  CodiraInvocationRef Invok = InvokRef;
  if (!Invok)
    return;

  RefPtr<CodiraDocumentSemanticInfo> SemaInfoRef = this;
  ...
  // CodiraDocumentSemanticInfo pointer so use that for the token.
...
struct CodiraEditorDocument::Implementation {
  std::weak_ptr<CodiraASTManager> ASTMgr;
  std::shared_ptr<NotificationCenter> NotificationCtr;
  ...
  CodiraSyntaxMap SyntaxMap;
  /// The minimal range of syntax highlighted tokens affected by the last edit
  std::optional<CodiraEditorCharRange> AffectedRange;
  /// Whether the last operation was an edit rather than a document open
  bool Edited;

  std::vector<DiagnosticEntryInfo> ParserDiagnostics;
  RefPtr<CodiraDocumentSemanticInfo> SemanticInfo;
  CodeFormatOptions FormatOptions;

  std::shared_ptr<CodiraDocumentSyntaxInfo> SyntaxInfo;

  std::shared_ptr<CodiraDocumentSyntaxInfo> getSyntaxInfo() {
    toolchain::sys::ScopedLock L(AccessMtx);
    SyntaxInfo->parseIfNeeded();
    return SyntaxInfo;
  }

  toolchain::sys::Mutex AccessMtx;

  Implementation(StringRef FilePath, CodiraLangSupport &LangSupport,
    ...
    // cursor_info is made.
    SemanticInfo = new CodiraDocumentSemanticInfo(
...
static UIdent getAccessLevelUID(AccessLevel Access) {
  static UIdent AccessOpen("source.lang.code.accessibility.open");
  static UIdent AccessPublic("source.lang.code.accessibility.public");
  static UIdent AccessPackage("source.lang.code.accessibility.package");
  static UIdent AccessInternal("source.lang.code.accessibility.internal");
  static UIdent AccessFilePrivate("source.lang.code.accessibility.fileprivate");
  static UIdent AccessPrivate("source.lang.code.accessibility.private");
...
static std::optional<AccessLevel> inferAccessSyntactically(const ValueDecl *D) {
  ...
  toolchain_unreachable("Unhandled DeclContextKind in switch.");
}
...
class CodiraDocumentStructureWalker: public ide::SyntaxModelWalker {
  SourceManager &SrcManager;
  EditorConsumer &Consumer;
  unsigned BufferID;

public:
  CodiraDocumentStructureWalker(SourceManager &SrcManager,
                               unsigned BufferID,
                               EditorConsumer &Consumer)
  ...
  bool walkToSubStructurePre(SyntaxStructureNode Node) override {
    ...
    UIdent Kind = CodiraLangSupport::getUIDForSyntaxStructureKind(Node.Kind);
    UIdent AccessLevel;
    ...
    if (auto ValueD = dyn_cast_or_null<ValueDecl>(Node.Dcl)) {
      toolchain::raw_svector_ostream OS(DisplayNameBuf);
      if (!CodiraLangSupport::printDisplayName(ValueD, OS))
        DisplayName = OS.str();
    }
    ...
    for (auto Attr : Node.Attrs) {
      if (auto AttrUID = CodiraLangSupport::getUIDForDeclAttribute(Attr)) {
    ...
    for (const auto &Elem : Node.Elements) {
      ...
      UIdent Kind = CodiraLangSupport::getUIDForSyntaxStructureElementKind(Elem.Kind);
  ...
  bool walkToNodePre(SyntaxNode Node) override {
    ...
    unsigned EndOffset = SrcManager.getLocOffsetInBuffer(Node.Range.getEnd(),
                                                         BufferID);
    UIdent Kind = CodiraLangSupport::getUIDForSyntaxNodeKind(Node.Kind);
    Consumer.beginDocumentSubStructure(StartOffset, EndOffset - StartOffset,
...
/// Walks the syntax model to populate a given CodiraSyntaxMap with the token
/// ranges to highlight and pass document structure information to the given
/// EditorConsumer.
class CodiraEditorSyntaxWalker: public ide::SyntaxModelWalker {
  /// The syntax map to populate
  CodiraSyntaxMap &SyntaxMap;
  SourceManager &SrcManager;
  unsigned BufferID;
  CodiraDocumentStructureWalker DocStructureWalker;
  /// The current token nesting level (e.g. for a field in a doc comment)
  unsigned NestingLevel = 0;
public:
  CodiraEditorSyntaxWalker(CodiraSyntaxMap &SyntaxMap,
...
public:
  ...
  bool scan(SourceFile &SF, unsigned BufID, unsigned Offset, unsigned Length,
    ...
    for (const auto &param : toolchain::ArrayRef(params).slice(firstTrailingIndex)) {
...
CodiraEditorDocument::CodiraEditorDocument(
    StringRef FilePath, CodiraLangSupport &LangSupport,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> fileSystem,
    CodeFormatOptions Options)
...
ImmutableTextSnapshotRef CodiraEditorDocument::initializeText(
    toolchain::MemoryBuffer *Buf, ArrayRef<const char *> Args,
    bool ProvideSemanticInfo,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> fileSystem) {
  toolchain::sys::ScopedLock L(Impl.AccessMtx);
  ...
  if (ProvideSemanticInfo || !Args.empty()) {
    Impl.SemanticInfo = new CodiraDocumentSemanticInfo(
...
static void updateSemaInfo(RefPtr<CodiraDocumentSemanticInfo> SemanticInfo,
                           EditableTextBufferRef EditableBuffer,
                           SourceKitCancellationToken CancellationToken) {
  if (SemanticInfo) {
...
ImmutableTextSnapshotRef CodiraEditorDocument::replaceText(
    unsigned Offset, unsigned Length, toolchain::MemoryBuffer *Buf,
  ...
  RefPtr<CodiraDocumentSemanticInfo> SemanticInfo;
  {
    ...
    Impl.Edited = true;
    toolchain::StringRef Str = Buf->getBuffer();
...
void CodiraEditorDocument::updateSemaInfo(
    SourceKitCancellationToken CancellationToken) {
  Impl.AccessMtx.lock();
...
void CodiraEditorDocument::resetSyntaxInfo(ImmutableTextSnapshotRef Snapshot,
  ...
  if (Impl.SemanticInfo->getInvocation()) {
    ...
    // Use stdin as a .code input to satisfy the driver. Note that we don't
  ...
  Impl.SyntaxInfo.reset(
    new CodiraDocumentSyntaxInfo(CompInv, Snapshot, Args, Impl.FilePath));
}

static UIdent SemaDiagStage("source.diagnostic.stage.code.sema");
static UIdent ParseDiagStage("source.diagnostic.stage.code.parse");

void CodiraEditorDocument::readSyntaxInfo(EditorConsumer &Consumer, bool ReportDiags) {
  toolchain::sys::ScopedLock L(Impl.AccessMtx);
  ...
  CodiraSyntaxMap NewMap = CodiraSyntaxMap(Impl.SyntaxMap.Tokens.size() + 16);
  ...
  if (Consumer.documentStructureEnabled() || Consumer.syntaxMapEnabled()) {
    ...
    CodiraEditorSyntaxWalker SyntaxWalker(
...
void CodiraEditorDocument::readSemanticInfo(ImmutableTextSnapshotRef Snapshot,
                                           EditorConsumer& Consumer) {
  toolchain::sys::ScopedLock L(Impl.AccessMtx);
  std::vector<CodiraSemanticToken> SemaToks;
...
void CodiraEditorDocument::removeCachedAST() {
  Impl.SemanticInfo->removeCachedAST();
}

void CodiraEditorDocument::cancelBuildsForCachedAST() {
  Impl.SemanticInfo->cancelBuildsForCachedAST();
}

void CodiraEditorDocument::applyFormatOptions(OptionsDictionary &FmtOptions) {
...
const CodeFormatOptions &CodiraEditorDocument::getFormatOptions() {
  return Impl.FormatOptions;
}

std::string CodiraEditorDocument::getFilePath() const { return Impl.FilePath; }

toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem>
CodiraEditorDocument::getFileSystem() const {
  toolchain::sys::ScopedLock L(Impl.AccessMtx);
  return Impl.SemanticInfo ? Impl.SemanticInfo->getFileSystem()
                           : toolchain::vfs::getRealFileSystem();
}

void CodiraEditorDocument::formatText(unsigned Line, unsigned Length,
                                     EditorConsumer &Consumer) {
...
printClosureBody(const PlaceholderExpansionScanner::ClosureInfo &closure,
                 toolchain::raw_ostream &OS, const SourceManager &SM) {
...
void CodiraEditorDocument::expandPlaceholder(unsigned Offset, unsigned Length,
                                            EditorConsumer &Consumer) {
  auto SyntaxInfo = Impl.getSyntaxInfo();
  SourceManager &SM = SyntaxInfo->getSourceManager();
  ...
  Scanner.scan(SF, BufID, Offset, Length,
      ...
      toolchain::SmallString<128> ExpansionStr;
      {
        toolchain::raw_svector_ostream OS(ExpansionStr);
...
ImmutableTextSnapshotRef CodiraEditorDocument::getLatestSnapshot() const {
  toolchain::sys::ScopedLock L(Impl.AccessMtx);
  return Impl.EditableBuffer->getSnapshot();
}

void CodiraEditorDocument::reportDocumentStructure(SourceFile &SrcFile,
                                                  EditorConsumer &Consumer) {
  ide::SyntaxModelContext ModelContext(SrcFile);
  CodiraDocumentStructureWalker Walker(SrcFile.getASTContext().SourceMgr,
                                      SrcFile.getBufferID(),
                                      Consumer);
...
// EditorOpen
//===----------------------------------------------------------------------===//
void CodiraLangSupport::editorOpen(StringRef Name, toolchain::MemoryBuffer *Buf,
                                  EditorConsumer &Consumer,
  ...
  if (!EditorDoc) {
    EditorDoc = new CodiraEditorDocument(Name, *this, fileSystem);
...
void CodiraLangSupport::editorClose(StringRef Name, bool CancelBuilds,
                                   bool RemoveCache) {
...
void CodiraLangSupport::editorReplaceText(StringRef Name,
...
// EditorFormatText
//===----------------------------------------------------------------------===//
void CodiraLangSupport::editorApplyFormatOptions(StringRef Name,
...
void CodiraLangSupport::editorFormatText(StringRef Name, unsigned Line,
                                        unsigned Length,
                                        EditorConsumer &Consumer) {
  auto EditorDoc = EditorDocuments->getByUnresolvedName(Name);
  if (!EditorDoc) {
...
void CodiraLangSupport::editorExtractTextFromComment(StringRef Source,
                                                    EditorConsumer &Consumer) {
  Consumer.handleSourceText(extractPlainTextFromComment(Source));
}

void CodiraLangSupport::editorConvertMarkupToXML(StringRef Source,
                                                EditorConsumer &Consumer) {
  std::string Result;
...
//===----------------------------------------------------------------------===//
void CodiraLangSupport::editorExpandPlaceholder(StringRef Name, unsigned Offset,
                                               unsigned Length,
                                               EditorConsumer &Consumer) {
  auto EditorDoc = EditorDocuments->getByUnresolvedName(Name);
  if (!EditorDoc) {
...
void CodiraLangSupport::getSemanticTokens(
  ...
  CodiraInvocationRef Invok = ASTMgr->getTypecheckInvocation(
  ...
  class SemanticTokensConsumer : public CodiraASTConsumer {
