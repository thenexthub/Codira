...
struct ASTKey {
  toolchain::FoldingSetNodeID FSID;
};
...
struct CodiraInvocation::Implementation {
...
CodiraInvocation::~CodiraInvocation() {
  delete &Impl;
}

ArrayRef<std::string> CodiraInvocation::getArgs() const {
  return ArrayRef(Impl.Opts.Args);
}

void CodiraInvocation::applyTo(language::CompilerInvocation &CompInvok) const {
  return Impl.Opts.applyTo(CompInvok);
}

void CodiraInvocation::raw(std::vector<std::string> &Args,
...
void InvocationOptions::profile(toolchain::FoldingSetNodeID &ID) const {
...
namespace SourceKit {
  struct ASTUnit::Implementation {
    ...
    WorkQueue Queue{ WorkQueue::Dequeuing::Serial, "sourcekit.code.ConsumeAST" };

    Implementation(uint64_t Generation, std::shared_ptr<CodiraStatistics> Stats)
        : Generation(Generation), Stats(Stats) {}

    void consumeAsync(CodiraASTConsumerRef ASTConsumer, ASTUnitRef ASTRef);
  };

  void ASTUnit::Implementation::consumeAsync(CodiraASTConsumerRef ConsumerRef,
                                             ASTUnitRef ASTRef) {
#if defined(_WIN32)
    ...
    Queue.dispatch([ASTRef, ConsumerRef]{
      CodiraASTConsumer &ASTConsumer = *ConsumerRef;
  ...
  ASTUnit::ASTUnit(uint64_t Generation, std::shared_ptr<CodiraStatistics> Stats)
  ...
  language::CompilerInstance &ASTUnit::getCompilerInstance() const {
...
struct FileContent {
  ...
  FileContent(ImmutableTextSnapshotRef Snapshot, std::string Filename,
              std::unique_ptr<toolchain::MemoryBuffer> Buffer, bool IsPrimary,
              BufferStamp Stamp)
...
/// a list of \c CodiraASTConsumers about the built AST.
/// It also supports cancellation with the following paradigm: If an \c
/// CodiraASTConsumer is no longer needed, it can be cancelled, which will remove
...
class ASTBuildOperation
  ...
  const CodiraInvocationRef InvokRef;
  const IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem;
  ...
  /// \c DependencyStamps).
  toolchain::sys::Mutex DependencyStampsMtx;
  ...
  CodiraASTManagerRef ASTManager;
  ...
  toolchain::sys::Mutex ConsumersAndResultMtx;
  ...
  SmallVector<CodiraASTConsumerRef, 4> Consumers = {};
  ...
  /// with an error.
  void informConsumer(CodiraASTConsumerRef Consumer);
...
public:
  ASTBuildOperation(IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
                    CodiraInvocationRef InvokRef, CodiraASTManagerRef ASTManager,
  ...
  bool isFinished() {
    toolchain::sys::ScopedLock L(ConsumersAndResultMtx);
  ...
  /// it.
  bool addConsumer(CodiraASTConsumerRef Consumer);
  ...
  bool matchesSourceState(IntrusiveRefCntPtr<toolchain::vfs::FileSystem> fileSystem);
  ...
  void requestConsumerCancellation(CodiraASTConsumerRef Consumer);
...
/// It is thus able to serve an \c CodiraASTConsumer with an AST from an older
...
class ASTProducer : public std::enable_shared_from_this<ASTProducer> {
  ...
  void cleanBuildOperations() {
    auto ReverseOperations = toolchain::reverse(BuildOperations);
    auto LastOperationWithResultIt =
        toolchain::find_if(ReverseOperations, [](ASTBuildOperationRef BuildOp) {
    ...
    toolchain::erase_if(BuildOperations, [LastOperationWithResult](
  ...
  ASTBuildOperationRef getBuildOperationForConsumer(
      CodiraASTConsumerRef Consumer,
      IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
      CodiraASTManagerRef Mgr);

public:
  explicit ASTProducer(CodiraInvocationRef InvokRef)
  ...
  void enqueueConsumer(CodiraASTConsumerRef Consumer,
                       IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
                       CodiraASTManagerRef Mgr);
...
struct CodiraASTManager::Implementation {
  explicit Implementation(
      std::shared_ptr<CodiraEditorDocumentFileMap> EditorDocs,
      std::shared_ptr<GlobalConfig> Config,
      std::shared_ptr<CodiraStatistics> Stats,
      std::shared_ptr<RequestTracker> ReqTracker,
      std::shared_ptr<PluginRegistry> Plugins, StringRef CodiraExecutablePath,
  ...
  Cache<ASTKey, ASTProducerRef> ASTCache{ "sourcekit.code.ASTCache" };
  toolchain::sys::Mutex CacheMtx;
  ...
  struct ScheduledConsumer {
    CodiraASTConsumerWeakRef Consumer;
  ...
  toolchain::sys::Mutex ScheduledConsumersMtx;
  ...
  WorkQueue ConsumerNotificationQueue{
      ...
      "CodiraASTManager::Implementation::ConsumerNotificationQueue"};
  ...
  void cleanDeletedConsumers() {
    toolchain::sys::ScopedLock L(ScheduledConsumersMtx);
    toolchain::erase_if(ScheduledConsumers, [](ScheduledConsumer Consumer) {
      return Consumer.Consumer.expired();
    });
  }

  /// Retrieve the ASTProducer for a given invocation, creating one if needed.
  ASTProducerRef getOrCreateASTProducer(CodiraInvocationRef InvokRef);

  /// Retrieve the ASTProducer for a given invocation, returning \c nullopt if
  /// not present.
  std::optional<ASTProducerRef> getASTProducer(CodiraInvocationRef Invok);

  /// Updates the cache entry to account for any changes to the ASTProducer
  /// for the given invocation.
  void updateASTProducer(CodiraInvocationRef Invok);
...
CodiraASTManager::CodiraASTManager(
    std::shared_ptr<CodiraEditorDocumentFileMap> EditorDocs,
    std::shared_ptr<GlobalConfig> Config,
    std::shared_ptr<CodiraStatistics> Stats,
    std::shared_ptr<RequestTracker> ReqTracker,
    ...
    : Impl(*new Implementation(EditorDocs, Config, Stats, ReqTracker, Plugins,
                               CodiraExecutablePath, RuntimeResourcePath)) {}

CodiraASTManager::~CodiraASTManager() {
  delete &Impl;
}

std::unique_ptr<toolchain::MemoryBuffer> CodiraASTManager::getMemoryBuffer(
    StringRef Filename,
...
bool CodiraASTManager::initCompilerInvocation(
    CompilerInvocation &Invocation, ArrayRef<const char *> OrigArgs,
    language::FrontendOptions::ActionType Action, DiagnosticEngine &Diags,
  ...
  return initCompilerInvocation(Invocation, OrigArgs, Action, Diags,
                                ...
                                toolchain::vfs::getRealFileSystem(), Error);
}
...
bool CodiraASTManager::initCompilerInvocation(
  ...
  return ide::initCompilerInvocation(
      ...
      Impl.CodiraExecutablePath, Impl.RuntimeResourcePath, Impl.SessionTimestamp,
      Error);
}

bool CodiraASTManager::initCompilerInvocation(
    CompilerInvocation &CompInvok, ArrayRef<const char *> OrigArgs,
    language::FrontendOptions::ActionType Action, StringRef PrimaryFile,
    std::string &Error) {
...
bool CodiraASTManager::initCompilerInvocationNoInputs(
    language::CompilerInvocation &Invocation, ArrayRef<const char *> OrigArgs,
  ...
  // Use stdin as a .code input to satisfy the driver.
  Args.push_back("-");
...
CodiraInvocationRef
CodiraASTManager::getTypecheckInvocation(ArrayRef<const char *> OrigArgs,
...
CodiraInvocationRef CodiraASTManager::getTypecheckInvocation(
  ...
  if (initCompilerInvocation(CompInvok, OrigArgs,
    ...
    if (TracedOp.enabled()) {
      trace::CodiraInvocation TraceInfo;
  ...
  return new CodiraInvocation(
      *new CodiraInvocation::Implementation(std::move(Opts)));
}

void CodiraASTManager::processASTAsync(
    CodiraInvocationRef InvokRef, CodiraASTConsumerRef ASTConsumer,
  ...
  {
    toolchain::sys::ScopedLock L(Impl.ScheduledConsumersMtx);
    if (OncePerASTToken) {
  ...
  auto WeakConsumer = CodiraASTConsumerWeakRef(ASTConsumer);
  auto WeakThis = std::weak_ptr<CodiraASTManager>(shared_from_this());
...
CodiraASTManager::Implementation::getASTProducer(CodiraInvocationRef Invok) {
  toolchain::sys::ScopedLock L(CacheMtx);
  return ASTCache.get(Invok->Impl.Key);
}

void CodiraASTManager::Implementation::updateASTProducer(
...
void CodiraASTManager::removeCachedAST(CodiraInvocationRef Invok) {
  toolchain::sys::ScopedLock L(Impl.CacheMtx);
  Impl.ASTCache.remove(Invok->Impl.Key);
}

void CodiraASTManager::cancelBuildsForCachedAST(CodiraInvocationRef Invok) {
...
ASTProducerRef CodiraASTManager::Implementation::getOrCreateASTProducer(
    CodiraInvocationRef InvokRef) {
  toolchain::sys::ScopedLock L(CacheMtx);
...
static FileContent getFileContentFromSnap(ImmutableTextSnapshotRef Snap,
  ...
  auto Buf = toolchain::MemoryBuffer::getMemBufferCopy(
      Snap->getBuffer()->getText(), FilePath);
  return FileContent(Snap, FilePath.str(), std::move(Buf), IsPrimary,
                     Snap->getStamp());
}

FileContent CodiraASTManager::Implementation::getFileContent(
    StringRef UnresolvedPath, bool IsPrimary,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
    std::string &Error) const {
  std::string FilePath = CodiraLangSupport::resolvePathSymlinks(UnresolvedPath);
...
BufferStamp CodiraASTManager::Implementation::getBufferStamp(
    StringRef FilePath,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
...
std::unique_ptr<toolchain::MemoryBuffer>
CodiraASTManager::Implementation::getMemoryBuffer(
    StringRef Filename,
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FileSystem,
    std::string &Error) const {
  assert(FileSystem);
  ...
  toolchain::raw_string_ostream OSErr(Error);
...
ASTBuildOperation::fileContentsForFilesInCompilerInvocation() {
  ...
  for (const auto &input :
    ...
    if (!Content.Buffer) {
      ...
      Content.Buffer = toolchain::WritableMemoryBuffer::getNewMemBuffer(0, Filename);
    }
...
bool ASTBuildOperation::matchesSourceState(
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> OtherFileSystem) {
  const InvocationOptions &Opts = InvokRef->Impl.Opts;
  ...
  toolchain::sys::ScopedLock L(DependencyStampsMtx);
...
void ASTBuildOperation::requestConsumerCancellation(
  ...
  // `schedule()`.
  auto ConsumerIndex = toolchain::find_if(
      Consumers, [&Consumer](CodiraASTConsumerRef ConsumerInQueue) {
...
static void collectModuleDependencies(ModuleDecl *TopMod,
    toolchain::SmallPtrSetImpl<ModuleDecl *> &Visited,
...
void ASTBuildOperation::informConsumer(CodiraASTConsumerRef Consumer) {
  assert(Result &&
...
ASTUnitRef ASTBuildOperation::buildASTUnit(std::string &Error) {
  ...
  toolchain::SmallPtrSet<ModuleDecl *, 16> Visited;
  ...
  if (!Consumer.hadAnyError()) {
    ...
    // don't block any other AST processing for the same CodiraInvocation.
    ...
    if (auto SF = CompIns.getPrimarySourceFile()) {
      ...
      toolchain::SaveAndRestore<std::shared_ptr<std::atomic<bool>>> DisableCancellationDuringSILGen(CompIns.getASTContext().CancellationFlag, nullptr);
...
void ASTBuildOperation::schedule(WorkQueue Queue) {
  ...
  Queue.dispatch(
      [this, SharedThis] {
        ...
        SmallVector<CodiraASTConsumerRef, 4> ConsumersToCancel;
        {
          toolchain::sys::ScopedLock L(ConsumersAndResultMtx);
          if (Consumers.empty()) {
        ...
        SmallVector<CodiraASTConsumerRef, 4> ConsumersToInform;
        {
          toolchain::sys::ScopedLock L(ConsumersAndResultMtx);
...
bool ASTBuildOperation::addConsumer(CodiraASTConsumerRef Consumer) {
  {
    toolchain::sys::ScopedLock L(ConsumersAndResultMtx);
    if (isCancelled()) {
      return false;
  ...
  Consumer->setCancellationRequestCallback(
      [WeakThis](CodiraASTConsumerRef Consumer) {
        if (auto This = WeakThis.lock()) {
...
ASTBuildOperationRef ASTProducer::getBuildOperationForConsumer(
  ...
  for (auto &BuildOp : toolchain::reverse(BuildOperations)) {
...
void ASTProducer::enqueueConsumer(
  ...
  BuildOperationsQueue.dispatch([Consumer, FileSystem, Mgr, This]() {
    ...
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FS = FileSystem;
