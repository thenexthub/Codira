...
void Codira::Format::TypePatternFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto pattern = As<ASTKind::TYPE_PATTERN>(node);
    AddTypePattern(doc, *pattern, level);
}

void TypePatternFormatter::AddTypePattern(Doc& doc, const Codira::AST::TypePattern& typePattern, int level)
{
