...
void Codira::Format::ConstPatternFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto pattern = As<ASTKind::CONST_PATTERN>(node);
    AddConstPattern(doc, *pattern, level);
}

void ConstPatternFormatter::AddConstPattern(Doc& doc, const Codira::AST::ConstPattern& constPattern, int level)
{
