...
void EnumPatternFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto pattern = As<ASTKind::ENUM_PATTERN>(node);
    AddEnumPattern(doc, *pattern, level);
}

void EnumPatternFormatter::AddEnumPattern(Doc& doc, const Codira::AST::EnumPattern& enumPattern, int level)
{
