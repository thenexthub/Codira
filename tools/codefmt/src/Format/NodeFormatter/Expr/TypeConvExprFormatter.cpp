...
void TypeConvExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto typeConvExpr = As<ASTKind::TYPE_CONV_EXPR>(node);
    AddTypeConvExpr(doc, *typeConvExpr, level);
}

void TypeConvExprFormatter::AddTypeConvExpr(Doc& doc, const Codira::AST::TypeConvExpr& typeConvExpr, int level)
{
