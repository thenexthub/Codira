...
void IncOrDecExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto incOrDecExpr = As<ASTKind::INC_OR_DEC_EXPR>(node);
    AddIncOrDecExpr(doc, *incOrDecExpr, level);
}

void IncOrDecExprFormatter::AddIncOrDecExpr(Doc& doc, const Codira::AST::IncOrDecExpr& incOrDecExpr, int level)
{
