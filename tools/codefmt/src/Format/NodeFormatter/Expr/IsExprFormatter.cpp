...
void IsExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto isExpr = As<ASTKind::IS_EXPR>(node);
    AddIsExpr(doc, *isExpr, level);
}

void IsExprFormatter::AddIsExpr(Doc& doc, const Codira::AST::IsExpr& isExpr, int level)
{
