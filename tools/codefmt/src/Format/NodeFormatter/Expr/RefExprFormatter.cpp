...
void RefExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto refExpr = As<ASTKind::REF_EXPR>(node);
    AddRefExpr(doc, *refExpr, level);
}

void RefExprFormatter::AddRefExpr(Doc& doc, const Codira::AST::RefExpr& refExpr, int level)
{
