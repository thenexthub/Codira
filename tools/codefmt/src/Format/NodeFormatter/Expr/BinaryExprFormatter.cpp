...
void BinaryExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto binaryExpr = As<ASTKind::BINARY_EXPR>(node);
    AddBinaryExpr(doc, *binaryExpr, level);
}

void BinaryExprFormatter::AddBinaryExpr(Doc& doc, const Codira::AST::BinaryExpr& binaryExpr, int level)
{
