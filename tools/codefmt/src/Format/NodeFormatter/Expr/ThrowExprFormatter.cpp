...
void ThrowExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto* throwExpr = As<ASTKind::THROW_EXPR>(node);
    AddThrowExpr(doc, *throwExpr, level);
}

void ThrowExprFormatter::AddThrowExpr(Doc& doc, const Codira::AST::ThrowExpr& throwExpr, int level)
{
