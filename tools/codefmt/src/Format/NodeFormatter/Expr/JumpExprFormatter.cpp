...
void JumpExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto jumpExpr = As<ASTKind::JUMP_EXPR>(node);
    AddJumpExpr(doc, *jumpExpr, level);
}

void JumpExprFormatter::AddJumpExpr(Doc& doc, const Codira::AST::JumpExpr& jumpExpr, int level)
{
