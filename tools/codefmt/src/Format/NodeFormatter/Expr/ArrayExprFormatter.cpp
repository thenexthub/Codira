...
void ArrayExprFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto arrayExpr = As<ASTKind::ARRAY_EXPR>(node);
    AddArrayExpr(doc, *arrayExpr, level);
}

void ArrayExprFormatter::AddArrayExpr(Doc& doc, const Codira::AST::ArrayExpr& arrayExpr, int level)
{
