...
void Codira::Format::VArrayTypeFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto type = As<ASTKind::VARRAY_TYPE>(node);
    AddVArrayType(doc, *type, level);
}

void VArrayTypeFormatter::AddVArrayType(Doc& doc, const Codira::AST::VArrayType& vArrayType, int level)
{
