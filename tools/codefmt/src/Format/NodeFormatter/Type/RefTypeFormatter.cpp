...
void Codira::Format::RefTypeFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto type = As<ASTKind::REF_TYPE>(node);
    AddRefType(doc, *type, level);
}

void RefTypeFormatter::AddRefType(Doc& doc, const Codira::AST::RefType& refType, int level)
{
