...
void ConstantTypeFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto type = As<ASTKind::CONSTANT_TYPE>(node);
    AddConstantType(doc, *type, level);
}

void ConstantTypeFormatter::AddConstantType(Doc& doc, const Codira::AST::ConstantType& constantType, int level)
{
