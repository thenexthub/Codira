...
void Codira::Format::FuncTypeFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto type = As<ASTKind::FUNC_TYPE>(node);
    AddFuncType(doc, *type, level);
}

void FuncTypeFormatter::AddFuncType(Doc& doc, const Codira::AST::FuncType& funcType, int level)
{
