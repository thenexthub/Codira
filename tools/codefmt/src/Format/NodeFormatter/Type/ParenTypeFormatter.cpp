...
void Codira::Format::ParenTypeFormatter::ASTToDoc(Doc& doc, Ptr<Codira::AST::Node> node, int level, FuncOptions&)
{
    auto type = As<ASTKind::PAREN_TYPE>(node);
    AddParenType(doc, *type, level);
}

void ParenTypeFormatter::AddParenType(Doc& doc, const Codira::AST::ParenType& parenType, int level)
{
