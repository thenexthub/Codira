...
#ifdef BUILD_LIB_CODIRA_DEMANGLE // to reuse the code to compile CodiraDemangle.cpp
#include <cstring>
#include <string>
...
{
#if defined(MRT_DEBUG) && (MRT_DEBUG == 1) && !defined(BUILD_LIB_CODIRA_DEMANGLE)
...
DemangleInfo<T> Demangler<T>::Demangle(bool isType)
{
    ...
    mangledName = DeCompression<T>(mangledName.SubStr(0)).CODEMangledDeCompression();
    if (IsGlobalInit()) {
    ...
    // C func wrapper like "cfuncInCODE$real"
...
void Demangler<T>::SkipPrivateTopLevelDeclHash()
{
    ...
    const bool isBoxedDecl = static_cast<size_t>(mangledName.Find(BOX_DECL_PREFIX, currentIndex)) == currentIndex &&
        !mangledName.SubStr(currentIndex, hashLen).EndsWith(CODE_FILE_EXT);
    if (isBoxedDecl) {
        currentIndex = oldIdx;
        return;
    }
...
DemangleInfo<T> Demangler<T>::DemangleCommonDecl(bool isClass)
{
    ...
    // The codedb currently expects global variable initialization symbol is demangled as a
...
{
    SkipString(MANGLE_CODIRA_PREFIX);
    if (!isValid) {
        return Reject();
    }
...
DemangleInfo<T> Demangler<T>::DemangleWrappedFunction() 
{
    ...
    mangledName = T(MANGLE_CODIRA_PREFIX) + mangledName.SubStr(prefixIndex, endIndex - prefixIndex);
