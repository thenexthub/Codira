...
`codecov`（Codira Coverage）是一款面向仓颉语言的覆盖率统计工具，用于生成仓颉语言程序的覆盖率报告。
...
基本的命令使用方法如下所示，`codecov` 为主程序名称，`--version` 表示为显示 `codecov` 的版本号。部分配置项支持长短选项两种写法，效果相同，具体可以使用 `codecov --help` 命令参考用法。

```text
codecov -version 或者 codecov -v
```
...
仓颉版本包准备 --> 仓颉源码准备 -->  使用 `--coverage` 编译选项构建仓颉源码，生成二进制文件 --> 执行二进制文件 --> `codecov` 生成覆盖率统计结果
...
5. `codecov` 生成 `html`
    ...
    在 `WORKPATH` 目录执行 `codecov -o output --html-details`，更多 `codecov` 参数使用可参考[命令说明](#命令说明)章节。

执行完 `codecov` 命令之后，在 `WORKPATH/output` 目录会有以下文件：

```text
output
├── codecov_logs （该目录存放一些 codecov 执行过程的详细日志，可不用关注）
│   ├── codecov.log
│   └── gcov_parse.log
├── index.html （总的覆盖率报告，通过浏览器打开）
└── src_main.code.html （单个文件的覆盖率，可以通过打开 index.html 自动跳转到该文件）
```

#### 多文件场景

对于多文件场景，推荐使用项目管理工具的命令`codepm build --coverage` 或 `codepm test --coverage` ，具体用法请参见[项目管理工具](./codepm_manual.md)。
...
显示 `codecov` 的版本号。指定 `-v` 或者 `--version` 参数后，输入其他任何选项参数都不生效，只会显示版本号。如 `--version` 和 `--help` 同时使用，则显示 `version` 信息后退出。

### codecov --verbose

指定该选项后会将一些日志信息生成到 `codecov_logs` 目录中，该参数默认不生效，即默认不会打印中间信息。`gcov` 文件是 `codecov` 工具生成的中间文件，`codecov` 解析 `gcov` 文件的格式如下：

```text
==================== start: main.code.gcov =====================
...
### codecov --html-details

如果指定该参数，表示会生成仓颉文件对应的 `html`。在总的 `index` 文件里面会有每个子 `html` 的索引。子 `html` 文件和 `index.html` 放在同一个目录。

子 `html` 文件名是由目录和文件名由下划线拼接起来。如源文件是 `src/main.code`，生成的 `html` 名字为 `src_main.code.html`。如果源文件路径带有特殊字符会被替换成 `=`，下文[文件名包含特殊字符](#文件名包含特殊字符)章节会有更详细的描述。
...
### codecov -r ROOT | --root=ROOT

该参数指定的 `ROOT` 参数，表示在 `ROOT` 目录或者在其递归子目录能找到 `gcda` 文件，`gcda` 和 `gcno` 文件默认会生成在一起，建议不要手动特意去把 `gcda` 文件和 `gcno` 文件分开存放，不然可能会发生程序不能运行的情况。

参数指定的 `ROOT` 目录如果不存在，`codecov` 工具会有报错提示。

不指定该参数，默认会以当前目录为 `ROOT` 目录。

### codecov -o OUTPUT |  --output=OUTPUT

该参数指定的 `OUTPUT` 参数，表示 `html` 覆盖率报告的输出路径。

如果该 `OUTPUT` 目录不存在，而且其父目录也不存在，`codecov` 工具会有报错提示；如果 `OUTPUT` 目录不存在，但其父目录存在，`codecov` 会帮助创建 `OUTPUT` 目录。
...
```text
/work/cangjie/tests/API/test01/src/1.code
/work/cangjie/tests/API/test01/src/2.code
/work/cangjie/tests/LLVM/test02/src/3.code
/work/cangjie-tools/tests/LLVM/test01/src/4.code
/work/cangjie-tools/tests/LLVM/test02/src/5.code
```

1. 在 `/work` 目录执行命令：

    ```shell
    codecov --root=./ -s "/work/cangjie /work/cangjie-tools/tests" --html-details --output=html_output
    ```

    最后 html 中呈现的源文件相对路径是：

    ```text
    tests/API/test01/src/1.code
    tests/API/test01/src/2.code
    tests/LLVM/test02/src/3.code
    LLVM/test01/src/4.code
    LLVM/test02/src/5.code
    ```

2. 在 `/work` 目录执行命令, 没有指定 `--root` 参数和 `--source` 参数，默认当前所在路径为相对路径的参考路径，执行命令如下：

    ```shell
    codecov --html-details --output=html_output
    ```

    最后 html 中呈现的源文件相对路径是：

    ```text
    cangjie/tests/API/test01/src/1.code
    cangjie/tests/API/test01/src/2.code
    cangjie/tests/LLVM/test02/src/3.code
    cangjie-tools/tests/LLVM/test01/src/4.code
    cangjie-tools/tests/LLVM/test02/src/5.code
    ```
...
```text
/usr1/cangjie/tests/API/test01/src/1.code
/usr1/cangjie/tests/API/test01/src/2.code
/usr1/cangjie/tests/LLVM/test02/src/3.code
/usr1/cangjie-tools/tests/LLVM/test01/src/4.code
/usr1/cangjie-tools/tests/LLVM/test02/src/5.code
```

在 `/usr1` 目录执行命令：

```shell
codecov --root=./ -s "/usr1/cangjie" -e "/usr1/cangjie-tools/tests/LLVM" --html-details --output=html_output
```
...
```shell
codecov --root=./ -s "/usr1/cangjie" -i "/usr1/cangjie/tests/API/test01/src/1.code /usr1/cangjie/tests/LLVM/test02" --html-details --output=html_output
```

上面命令执行后, 在 `index.html` 中文件路径列表如下(`tests/API/test01/src/2.code` 不在 `-i` 参数指定的列表里面，所以不会出现在 `html` 的文件列表中):
...
├── index.html
├── src_1file=.code.html
├── src_file10_abc.code.html
├── src_file11_.aaa=bbb.code.html
├── src_file12==aaa=bbb.code.html
├── src_file13=####.code.html
├── src_file14=aa.code.html
├── src_file15=.code.html
├── src_file16===.code.html
├── src_file2=aa.code.html
├── src_file3==.code.html
├── src_file4=###.code.html
├── src_file5=cc.code.html
├── src_file6=.code.html
├── src_file7==.code.html
├── src_file8=.code.html
├── src_file9=aaa.code.html
└── src_main.code.html
```
...
- 仅有函数声明未包含函数体（包括 `foreign` 函数等），示例如下：
    ...
    ```cangjie
    foreign func code_core_free(p: CPointer): Unit
    ```
...
**原因：** 使用 `codec --test` 编译，仓颉测试框架会生成一个新的 `main` 作为程序入口，源代码中的 `main` 不再作为程序入口并且不会被执行。

**建议：** 在使用 `codec --test` 之后，建议不用再手写多余的 `main` 。
...
方法 1：设置 `CODIRA_HOME` 环境变量, `codecov` 可通过 `CODIRA_HOME` 环境变量找到 `llvm-cov` 命令，设置方法如下：
假设 `which codec` 显示 `/work/cangjie/bin/codec`, 并且 `/work/cangjie/bin/llvm/bin` 和 `/work/cangjie/bin/llvm/lib` 目录都存在，则可设置：

```shell
export CODIRA_HOME=/work/cangjie
```

方法 2：在 `/root/.bashrc` 里面直接设置环境变量，如 `codec` 放在 `/work/cangjie/bin/codec` 目录下，则设置方法如下：
