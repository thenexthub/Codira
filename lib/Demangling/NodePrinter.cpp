//===--- NodePrinter.cpp - Codira Demangling Node Printer ------------------===//
//
...
bool NodePrinter::printContext(NodePointer Context) {
  ...
  if (Context->getKind() == Node::Kind::Module) {
    if (Context->getText() == language::STDLIB_NAME)
      return Options.DisplayStdlibModule;
    if (Context->getText() == language::MANGLING_MODULE_OBJC)
...
NodePrinter::SugarType NodePrinter::findSugar(NodePointer Node) {
  ...
  if (Node->getKind() == Node::Kind::BoundGenericEnum) {
    ...
    if (isIdentifier(unboundType->getChild(1), "Optional") &&
        ...
        isCodiraModule(unboundType->getChild(0))) {
      return SugarType::Optional;
    }

    // Codira.ImplicitlyUnwrappedOptional
...
NodePointer NodePrinter::print(NodePointer Node, unsigned depth,
  ...
  case Node::Kind::Extension:
    ...
    if (Node->getNumChildren() == 3) {
      ...
      // This is an open to-do in language::_buildDemanglingForContext().
...
#define INVERTIBLE_PROTOCOL(Name, Bit) \
    case Bit: Printer << "Codira." << #Name; break;
#include "language/ABI/InvertibleProtocols.def"
    default:
      Printer << "Codira.<bit " << Node->getChild(1)->getIndex() << ">";
      break;
    }
    return nullptr;
  }
...
void NodePrinter::printFunctionName(bool hasName,
                                    toolchain::StringRef &OverwriteName,
                                    toolchain::StringRef &ExtraName,
