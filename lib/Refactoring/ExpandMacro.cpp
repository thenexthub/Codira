...
static toolchain::SmallVector<unsigned, 2>
getMacroExpansionBuffers(MacroDecl *macro, const CustomAttr *attr, Decl *decl) {
  ...
  toolchain::SmallVector<unsigned, 2> allBufferIDs;
  ...
  if (roles.contains(MacroRole::Preamble)) {
    if (auto fn = dyn_cast<AbstractFunctionDecl>(decl)) {
      auto bufferIDs =
          evaluateOrDefault(ctx.evaluator, ExpandPreambleMacroRequest{fn}, {});
      allBufferIDs.append(bufferIDs.begin(), bufferIDs.end());
    }
  }
  ...
  if (roles.contains(MacroRole::Body)) {
    if (auto fn = dyn_cast<AbstractFunctionDecl>(decl)) {
      auto bufferID =
          evaluateOrDefault(ctx.evaluator, ExpandBodyMacroRequest{fn}, {});
      if (bufferID)
