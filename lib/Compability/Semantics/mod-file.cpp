...
#include "language/Compability/Common/restorer.h"
...
static void PutPassName(toolchain::raw_ostream &, const std::optional<SourceName> &);
static void PutInit(toolchain::raw_ostream &, const Symbol &, const MaybeExpr &,
    const parser::Expr *, SemanticsContext &);
static void PutInit(toolchain::raw_ostream &, const MaybeIntExpr &);
static void PutBound(toolchain::raw_ostream &, const Bound &);
static void PutShapeSpec(toolchain::raw_ostream &, const ShapeSpec &);
static void PutShape(
    toolchain::raw_ostream &, const ArraySpec &, char open, char close);

static toolchain::raw_ostream &PutAttr(toolchain::raw_ostream &, Attr);
static toolchain::raw_ostream &PutType(toolchain::raw_ostream &, const DeclTypeSpec &);
static toolchain::raw_ostream &PutLower(toolchain::raw_ostream &, std::string_view);
...
void ModFileWriter::WriteClosure(toolchain::raw_ostream &out, const Symbol &symbol,
...
std::string ModFileWriter::GetAsString(const Symbol &symbol) {
  std::string buf;
  toolchain::raw_string_ostream all{buf};
  all << needs_.str();
  needs_.str().clear();
...
void ModFileWriter::PutSymbols(
  ...
  toolchain::raw_string_ostream typeBindings{buf};
...
static toolchain::raw_ostream &PutGenericName(
    toolchain::raw_ostream &os, const Symbol &symbol) {
...
void ModFileWriter::PutSymbol(
    toolchain::raw_ostream &typeBindings, const Symbol &symbol) {
  common::visit(
      common::visitors{
...
static void PutOpenACCDeviceTypeRoutineInfo(
    toolchain::raw_ostream &os, const OpenACCRoutineDeviceTypeInfo &info) {
  if (info.isSeq()) {
    os << " seq";
  }
  if (info.isGang()) {
...
static void PutOpenACCRoutineInfo(
    toolchain::raw_ostream &os, const SubprogramDetails &details) {
...
void ModFileWriter::PutSubprogram(const Symbol &symbol) {
  ...
  std::string ssBuf;
  toolchain::raw_string_ostream ss{ssBuf};
  PutAttrs(ss, attrs);
  if (!ss.str().empty()) {
    decls_ << ss.str().substr(1) << "::" << symbol.name() << '\n';
  }
  bool isInterface{details.isInterface()};
  toolchain::raw_ostream &os{isInterface ? decls_ : contains_};
  if (isInterface) {
  ...
  toolchain::raw_string_ostream typeBindings{typeBindingsBuf};
...
void ModFileWriter::PutEntity(toolchain::raw_ostream &os, const Symbol &symbol) {
  common::visit(
      common::visitors{
...
void PutShapeSpec(toolchain::raw_ostream &os, const ShapeSpec &x) {
  if (x.lbound().isStar()) {
...
void PutShape(
    toolchain::raw_ostream &os, const ArraySpec &shape, char open, char close) {
  if (!shape.empty()) {
    os << open;
    bool first{true};
...
void ModFileWriter::PutObjectEntity(
  ...
  if (symbol.test(language::Compability::semantics::Symbol::Flag::CrayPointer)) {
...
void ModFileWriter::PutProcEntity(toolchain::raw_ostream &os, const Symbol &symbol) {
...
void PutPassName(
    toolchain::raw_ostream &os, const std::optional<SourceName> &passName) {
  if (passName) {
    os << ",pass(" << *passName << ')';
  }
}

void ModFileWriter::PutTypeParam(toolchain::raw_ostream &os, const Symbol &symbol) {
...
void PutInit(toolchain::raw_ostream &os, const Symbol &symbol, const MaybeExpr &init,
...
void PutInit(toolchain::raw_ostream &os, const MaybeIntExpr &init) {
  if (init) {
    init->AsFortran(os << '=');
  }
}

void PutBound(toolchain::raw_ostream &os, const Bound &x) {
  if (x.isStar()) {
    os << '*';
  } else if (x.isColon()) {
    os << ':';
...
toolchain::raw_ostream &ModFileWriter::PutAttrs(toolchain::raw_ostream &os, Attrs attrs,
...
toolchain::raw_ostream &PutAttr(toolchain::raw_ostream &os, Attr attr) {
  return PutLower(os, AttrToString(attr));
}

toolchain::raw_ostream &PutType(toolchain::raw_ostream &os, const DeclTypeSpec &type) {
  return PutLower(os, type.AsFortran());
}

toolchain::raw_ostream &PutLower(toolchain::raw_ostream &os, std::string_view str) {
  for (char c : str) {
...
void PutOpenACCDirective(toolchain::raw_ostream &os, const Symbol &symbol) {
...
void PutOpenMPDirective(toolchain::raw_ostream &os, const Symbol &symbol) {
  if (symbol.test(Symbol::Flag::OmpThreadprivate)) {
    os << "!$omp threadprivate(" << symbol.name() << ")\n";
  }
}

void ModFileWriter::PutDirective(toolchain::raw_ostream &os, const Symbol &symbol) {
...
struct Temp {
  ...
  ~Temp() {
    if (fd >= 0) {
      toolchain::sys::fs::file_t native{toolchain::sys::fs::convertFDToNativeFile(fd)};
      toolchain::sys::fs::closeFile(native);
      toolchain::sys::fs::remove(path.c_str());
    }
  }
  int fd;
  std::string path;
...
static toolchain::ErrorOr<Temp> MkTemp(const std::string &path) {
  ...
  int fd;
  toolchain::SmallString<16> tempPath;
  if (std::error_code err{toolchain::sys::fs::createUniqueFile(
...
static std::error_code WriteFile(const std::string &path,
  ...
  if (debug) {
    toolchain::dbgs() << "Processing module " << path << ": ";
  }
  if (FileContentsMatch(path, header, contents)) {
    if (debug) {
      toolchain::dbgs() << "module unchanged, not writing\n";
    }
    return {};
  }
  toolchain::ErrorOr<Temp> temp{MkTemp(path)};
  if (!temp) {
    return temp.getError();
  }
  toolchain::raw_fd_ostream writer(temp->fd, /*shouldClose=*/false);
  writer << header;
  writer << contents;
  writer.flush();
  if (writer.has_error()) {
  ...
  if (debug) {
    toolchain::dbgs() << "module written\n";
  }
  return toolchain::sys::fs::rename(temp->path, path);
}
...
static bool FileContentsMatch(const std::string &path,
  ...
  auto buf_or{toolchain::MemoryBuffer::getFile(path)};
  if (!buf_or) {
    return false;
  }
...
static std::optional<ModuleCheckSumType> VerifyHeader(
    toolchain::ArrayRef<char> content) {
...
static void GetModuleDependences(
    ModuleDependences &dependences, toolchain::ArrayRef<char> content) {
...
Scope *ModFileReader::Read(SourceName name, std::optional<bool> isIntrinsic,
  ...
  toolchain::raw_null_ostream NullStream;
...
void SubprogramSymbolCollector::DoSymbol(
  ...
  } else if (symbol.test(language::Compability::semantics::Symbol::Flag::CrayPointee)) {
