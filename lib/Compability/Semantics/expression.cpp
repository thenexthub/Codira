...
using MaybeExpr =
    std::optional<language::Compability::evaluate::Expr<language::Compability::evaluate::SomeType>>;
...
// and appears here in namespace language::Compability::evaluate for convenience.
...
public:
  ...
  void Dump(toolchain::raw_ostream &);
...
void ExpressionAnalyzer::Analyze(const parser::CallStmt &callStmt) {
  ...
  if (auto chevrons{AnalyzeChevrons(callStmt)};
    ...
    if (!context_.AnyFatalError()) {
      std::string buf;
      toolchain::raw_string_ostream dump{buf};
...
void ArgumentAnalyzer::Dump(toolchain::raw_ostream &os) {
