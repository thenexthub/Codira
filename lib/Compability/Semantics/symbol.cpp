...
template <typename T>
static void DumpOptional(toolchain::raw_ostream &os, const char *label, const T &x) {
  if (x) {
    os << ' ' << label << ':' << *x;
  }
}
template <typename T>
static void DumpExpr(toolchain::raw_ostream &os, const char *label,
...
static void DumpBool(toolchain::raw_ostream &os, const char *label, bool x) {
  if (x) {
    os << ' ' << label;
  }
}

static void DumpSymbolVector(toolchain::raw_ostream &os, const SymbolVector &list) {
  char sep{' '};
...
static void DumpType(toolchain::raw_ostream &os, const Symbol &symbol) {
  if (const auto *type{symbol.GetType()}) {
    os << *type << ' ';
  }
}
static void DumpType(toolchain::raw_ostream &os, const DeclTypeSpec *type) {
  if (type) {
    os << ' ' << *type;
  }
}
...
template <typename T>
static void DumpList(toolchain::raw_ostream &os, const char *label, const T &list) {
  if (!list.empty()) {
    os << ' ' << label << ':';
    char sep{' '};
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const SubprogramDetails &x) {
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const OpenACCRoutineDeviceTypeInfo &x) {
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const OpenACCRoutineInfo &x) {
  if (x.isNohost()) {
    os << " nohost";
  }
...
void GenericDetails::CopyFrom(const GenericDetails &from) {
  ...
  for (std::size_t i{0}; i < from.specificProcs_.size(); ++i) {
    if (toolchain::none_of(specificProcs_, [&](const Symbol &mySymbol) {
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const EntityDetails &x) {
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const ObjectEntityDetails &x) {
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const AssocEntityDetails &x) {
  os << *static_cast<const EntityDetails *>(&x);
  if (x.IsAssumedSize()) {
    os << " RANK(*)";
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const ProcEntityDetails &x) {
  if (x.procInterface_) {
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const DerivedTypeDetails &x) {
  DumpBool(os, "sequence", x.sequence_);
  DumpList(os, "components", x.componentNames_);
  return os;
}

toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const GenericDetails &x) {
  os << ' ' << x.kind().ToString();
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const Details &details) {
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &o, Symbol::Flag flag) {
  return o << Symbol::EnumToString(flag);
}

toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &o, const Symbol::Flags &flags) {
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const Symbol &symbol) {
  os << symbol.name();
...
void Symbol::dump() const { toolchain::errs() << *this << '\n'; }
#endif
...
static void DumpUniqueName(toolchain::raw_ostream &os, const Scope &scope) {
  if (!scope.IsTopLevel()) {
...
toolchain::raw_ostream &DumpForUnparse(
    toolchain::raw_ostream &os, const Symbol &symbol, bool isDef) {
