...
#define CHECK_SIMPLE_CLAUSE(X, Y) \
  void OmpStructureChecker::Enter(const parser::OmpClause::X &) { \
    CheckAllowedClause(toolchain::omp::Clause::Y); \
  }

#define CHECK_REQ_CONSTANT_SCALAR_INT_CLAUSE(X, Y) \
  void OmpStructureChecker::Enter(const parser::OmpClause::X &c) { \
    CheckAllowedClause(toolchain::omp::Clause::Y); \
    RequiresConstantPositiveParameter(toolchain::omp::Clause::Y, c.v); \
  }

#define CHECK_REQ_SCALAR_INT_CLAUSE(X, Y) \
  void OmpStructureChecker::Enter(const parser::OmpClause::X &c) { \
    CheckAllowedClause(toolchain::omp::Clause::Y); \
    RequiresPositiveParameter(toolchain::omp::Clause::Y, c.v); \
  }
...
#define CHECK_SIMPLE_PARSER_CLAUSE(X, Y) \
  void OmpStructureChecker::Enter(const parser::X &) { \
    CheckAllowedClause(toolchain::omp::Y); \
  }
...
bool OmpStructureChecker::CheckAllowedClause(toolchainOmpClause clause) {
  ...
  toolchain::omp::Directive dir{dirCtx.directive};

  if (!toolchain::omp::isAllowedClauseForDirective(dir, clause, version)) {
    unsigned allowedInVersion{[&] {
      for (unsigned v : toolchain::omp::getOpenMPVersions()) {
        if (v <= version) {
          continue;
        }
        if (toolchain::omp::isAllowedClauseForDirective(dir, clause, v)) {
          return v;
        }
      }
      return 0u;
  ...
  if (CurrentDirectiveIsNested()) {
    ...
    while (index != -1) {
      if (set.test(dirContext_[index].directive)) {
        return true;
      } else if (toolchain::omp::allParallelSet.test(dirContext_[index].directive)) {
        return false;
      }
      index--;
    }
...
void OmpStructureChecker::CheckDirectiveSpelling(
  ...
  // with spaces removed).
  auto removeSpaces = [](toolchain::StringRef s) {
    std::string n{s.str()};
  ...
  toolchain::StringRef ref(lowerNoWS);
  ...
  for (unsigned v : toolchain::omp::getOpenMPVersions()) {
    ...
    toolchain::StringRef name{toolchain::omp::getOpenMPDirectiveName(id, v)};
    auto [kind, versions]{toolchain::omp::getOpenMPDirectiveKindAndVersions(name)};
...
void OmpStructureChecker::CheckMultipleOccurrence(
  ...
  for (auto const &var : nameList) {
    if (toolchain::is_contained(listVars, *(var.symbol))) {
      context_.Say(item,
...
void OmpStructureChecker::CheckMultListItems() {
  ...
  // Aligned clause
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_aligned)) {
  ...
  // Nontemporal clause
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_nontemporal)) {
  ...
  // Linear clause
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_linear)) {
    ...
    toolchain::transform(symbols, std::back_inserter(nameList), [&](auto &&pair) {
...
void OmpStructureChecker::HasInvalidTeamsNesting(
  ...
  if (!toolchain::omp::nestedTeamsAllowedSet.test(dir)) {
    context_.Say(source,
...
void OmpStructureChecker::Enter(const parser::OmpClause::Hint &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_hint);
...
void OmpStructureChecker::Enter(const parser::OpenMPConstruct &x) {
  DirectiveSpellingVisitor visitor(
      [this](parser::CharBlock source, toolchain::omp::Directive id) {
...
void OmpStructureChecker::CheckTargetNest(const parser::OpenMPConstruct &c) {
  ...
  toolchain::omp::Directive ineligibleTargetDir;
  ...
  common::visit(
      common::visitors{
          [&](const parser::OpenMPBlockConstruct &c) {
            ...
            if (beginSpec.DirId() == toolchain::omp::Directive::OMPD_target_data) {
            ...
            common::visit(
                common::visitors{
                    [&](const parser::OpenMPSimpleStandaloneConstruct &c) {
                      ...
                      switch (toolchain::omp::Directive dirId{c.v.DirId()}) {
                      case toolchain::omp::Directive::OMPD_target_update:
                      case toolchain::omp::Directive::OMPD_target_enter_data:
                      case toolchain::omp::Directive::OMPD_target_exit_data:
                        eligibleTarget = false;
          ...
          [&](const parser::OpenMPLoopConstruct &c) {
            ...
            if (toolchain::omp::allTargetSet.test(beginDir.v)) {
              eligibleTarget = false;
...
void OmpStructureChecker::Enter(const parser::OpenMPBlockConstruct &x) {
  ...
  if (toolchain::omp::allTargetSet.test(GetContext().directive)) {
    EnterDirectiveNest(TargetNest);
  }

  if (CurrentDirectiveIsNested()) {
    if (toolchain::omp::bottomTeamsSet.test(GetContextParent().directive)) {
      HasInvalidTeamsNesting(beginSpec.DirId(), beginSpec.source);
    }
    if (GetContext().directive == toolchain::omp::Directive::OMPD_master) {
      CheckMasterNesting(x);
    }
    // A teams region can only be strictly nested within the implicit parallel
    // region or a target region.
    if (GetContext().directive == toolchain::omp::Directive::OMPD_teams &&
        GetContextParent().directive != toolchain::omp::Directive::OMPD_target) {
  ...
  if (toolchain::omp::allTargetSet.test(beginSpec.DirId())) {
    deviceConstructFound_ = true;
  }

  if (GetContext().directive == toolchain::omp::Directive::OMPD_single) {
    ...
    auto catchCopyPrivateNowaitClauses = [&](const auto &dirSpec, bool isEnd) {
      for (auto &clause : dirSpec.Clauses().v) {
        if (clause.Id() == toolchain::omp::Clause::OMPC_copyprivate) {
        ...
        } else if (clause.Id() == toolchain::omp::Clause::OMPC_nowait) {
          if (foundNowait) {
            context_.Say(clause.source,
  ...
  case toolchain::omp::Directive::OMPD_target:
    if (CheckTargetBlockOnlyTeams(block)) {
      EnterDirectiveNest(TargetBlockOnlyTeams);
    }
    break;
  case toolchain::omp::OMPD_workshare:
  case toolchain::omp::OMPD_parallel_workshare:
    CheckWorkshareBlockStmts(block, beginSpec.source);
    HasInvalidWorksharingNesting(
        beginSpec.source, toolchain::omp::nestedWorkshareErrSet);
    break;
  case toolchain::omp::Directive::OMPD_scope:
  case toolchain::omp::Directive::OMPD_single:
    // TODO: This check needs to be extended while implementing nesting of
    // regions checks.
    HasInvalidWorksharingNesting(
        beginSpec.source, toolchain::omp::nestedWorkshareErrSet);
    break;
  case toolchain::omp::Directive::OMPD_task:
...
void OmpStructureChecker::CheckMasterNesting(
  ...
  // supported.
  if (IsCloselyNestedRegion(toolchain::omp::nestedMasterErrSet)) {
...
void OmpStructureChecker::Enter(const parser::OpenMPAssumeConstruct &x) {
  PushContextAndClauseSets(x.source, toolchain::omp::Directive::OMPD_assume);
}
...
void OmpStructureChecker::Enter(const parser::OpenMPDeclarativeAssumes &x) {
  PushContextAndClauseSets(x.source, toolchain::omp::Directive::OMPD_assumes);
}
...
void OmpStructureChecker::ChecksOnOrderedAsBlock() {
  if (FindClause(toolchain::omp::Clause::OMPC_depend)) {
  ...
  if (CurrentDirectiveIsNested()) {
    for (int i = (int)dirContext_.size() - 2; i >= 0; i--) {
      if (toolchain::omp::nestedOrderedErrSet.test(dirContext_[i].directive)) {
        context_.Say(GetContext().directiveSource,
            "`ORDERED` region may not be closely nested inside of `CRITICAL`, "
            "`ORDERED`, explicit `TASK` or `TASKLOOP` region."_err_en_US);
        break;
      } else if (toolchain::omp::allDoSet.test(dirContext_[i].directive)) {
        isNestedInDo = true;
        isNestedInDoSIMD =
            toolchain::omp::allDoSimdSet.test(dirContext_[i].directive);
        if (const auto *clause{
                FindClause(dirContext_[i], toolchain::omp::Clause::OMPC_ordered)}) {
        ...
        break;
      } else if (toolchain::omp::allSimdSet.test(dirContext_[i].directive)) {
        isNestedInSIMD = true;
        break;
      } else if (toolchain::omp::nestedOrderedParallelErrSet.test(
  ...
  if (!isCloselyNestedRegion) {
    ...
    if (CurrentDirectiveIsNested() &&
        FindClause(toolchain::omp::Clause::OMPC_simd) &&
...
void OmpStructureChecker::Leave(const parser::OmpBeginDirective &) {
  ...
  case toolchain::omp::Directive::OMPD_ordered:
...
void OmpStructureChecker::Enter(const parser::OpenMPSectionsConstruct &x) {
  ...
  HasInvalidWorksharingNesting(
      beginDir.source, toolchain::omp::nestedWorkshareErrSet);
}
...
void OmpStructureChecker::Enter(const parser::OmpEndSectionsDirective &x) {
  ...
  case toolchain::omp::Directive::OMPD_sections:
    PushContextAndClauseSets(
        dir.source, toolchain::omp::Directive::OMPD_end_sections);
    break;
  default:
...
void OmpStructureChecker::Leave(const parser::OmpEndSectionsDirective &x) {
  if (GetContext().directive == toolchain::omp::Directive::OMPD_end_sections) {
    dirContext_.pop_back();
  }
}
              ...
              if (const auto *name{parser::Unwrap<parser::Name>(ompObject)}) {
                ...
                if (name->symbol->GetUltimate().IsSubprogram()) {
                  if (GetContext().directive ==
                      toolchain::omp::Directive::OMPD_threadprivate)
                    context_.Say(name->source,
                  ...
                  if (GetContext().directive ==
                    ...
                    context_.Say(name->source,
                      ...
                      toolchain::omp::Directive::OMPD_declare_target)
...
void OmpStructureChecker::Enter(const parser::OpenMPThreadprivate &c) {
  ...
  PushContextAndClauseSets(
      dir.source, toolchain::omp::Directive::OMPD_threadprivate);
}
...
void OmpStructureChecker::Enter(const parser::OpenMPDeclareSimdConstruct &x) {
  ...
  PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_declare_simd);
}
...
void OmpStructureChecker::Enter(const parser::OmpDeclareVariantDirective &x) {
  const auto &dir{std::get<parser::Verbatim>(x.t)};
  PushContextAndClauseSets(
      dir.source, toolchain::omp::Directive::OMPD_declare_variant);
}
...
void OmpStructureChecker::Enter(const parser::OpenMPDepobjConstruct &x) {
  ...
  PushContextAndClauseSets(dirName.source, toolchain::omp::Directive::OMPD_depobj);
  unsigned version{context_.langOptions().OpenMPVersion};
  ...
  if (clause.Id() == toolchain::omp::Clause::OMPC_destroy) {
...
void OmpStructureChecker::Enter(const parser::OpenMPRequiresConstruct &x) {
  const auto &dir{std::get<parser::Verbatim>(x.t)};
  PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_requires);
  ...
  for (const parser::OmpClause &clause : clauseList.v) {
    toolchain::omp::Clause id{clause.Id()};
    if (id == toolchain::omp::Clause::OMPC_atomic_default_mem_order) {
      parser::MessageFormattedText txt(
          "REQUIRES directive with '%s' clause found lexically after atomic operation without a memory order clause"_err_en_US,
          parser::ToUpperCaseLetters(toolchain::omp::getOpenMPClauseName(id)));
...
void OmpStructureChecker::Enter(const parser::OpenMPDeclarativeAllocate &x) {
  ...
  PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_allocate);
  const auto &clauseList{std::get<parser::OmpClauseList>(x.t)};
...
void OmpStructureChecker::Enter(const parser::OmpClause::Allocator &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_allocator);
  ...
  RequiresPositiveParameter(toolchain::omp::Clause::OMPC_allocator, x.v);
}

void OmpStructureChecker::Enter(const parser::OmpClause::Allocate &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_allocate);
  if (OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_allocate, GetContext().clauseSource, context_)) {
...
void OmpStructureChecker::Enter(const parser::OmpDeclareTargetWithClause &x) {
  SetClauseSets(toolchain::omp::Directive::OMPD_declare_target);
}

void OmpStructureChecker::Leave(const parser::OmpDeclareTargetWithClause &x) {
  if (x.v.v.size() > 0) {
    const parser::OmpClause *enterClause =
        FindClause(toolchain::omp::Clause::OMPC_enter);
    const parser::OmpClause *toClause = FindClause(toolchain::omp::Clause::OMPC_to);
    const parser::OmpClause *linkClause =
        FindClause(toolchain::omp::Clause::OMPC_link);
    const parser::OmpClause *indirectClause =
        FindClause(toolchain::omp::Clause::OMPC_indirect);
    ...
    if (indirectClause) {
      CheckAllowedClause(toolchain::omp::Clause::OMPC_indirect);
    }
  }
}
...
void OmpStructureChecker::Enter(const parser::OpenMPDeclareMapperConstruct &x) {
  const auto &dir{std::get<parser::Verbatim>(x.t)};
  PushContextAndClauseSets(
      dir.source, toolchain::omp::Directive::OMPD_declare_mapper);
...
void OmpStructureChecker::Enter(
  ...
  const auto &dir{std::get<parser::Verbatim>(x.t)};
  PushContextAndClauseSets(
      dir.source, toolchain::omp::Directive::OMPD_declare_reduction);
}
...
void OmpStructureChecker::Enter(const parser::OpenMPDeclareTargetConstruct &x) {
  const auto &dir{std::get<parser::Verbatim>(x.t)};
  PushContext(dir.source, toolchain::omp::Directive::OMPD_declare_target);
}
...
void OmpStructureChecker::Enter(const parser::OmpErrorDirective &x) {
  const auto &dir{std::get<parser::Verbatim>(x.t)};
  PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_error);
}
...
void OmpStructureChecker::Enter(const parser::OpenMPDispatchConstruct &x) {
  ...
  PushContextAndClauseSets(
      dirSpec.DirName().source, toolchain::omp::Directive::OMPD_dispatch);
...
void OmpStructureChecker::Enter(const parser::OmpClause::At &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_at);
...
void OmpStructureChecker::Enter(const parser::OpenMPAllocatorsConstruct &x) {
  ...
  PushContextAndClauseSets(
      dirSpec.DirName().source, toolchain::omp::Directive::OMPD_allocators);
...
void OmpStructureChecker::CheckScan(
  ...
  if (!CurrentDirectiveIsNested() ||
      !toolchain::omp::scanParentAllowedSet.test(GetContextParent().directive)) {
    context_.Say(x.source,
...
void OmpStructureChecker::CheckBarrierNesting(
  ...
  // TODO:  Expand the check to include `LOOP` construct as well when it is
  // supported.
  if (IsCloselyNestedRegion(toolchain::omp::nestedBarrierErrSet)) {
...
void OmpStructureChecker::ChecksOnOrderedAsStandalone() {
  if (FindClause(toolchain::omp::Clause::OMPC_threads) ||
      FindClause(toolchain::omp::Clause::OMPC_simd)) {
    context_.Say(GetContext().clauseSource,
  ...
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_depend)) {
  ...
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_doacross)) {
  ...
  if (CurrentDirectiveIsNested() &&
      toolchain::omp::nestedOrderedDoAllowedSet.test(GetContextParent().directive)) {
    if (const auto *clause{
            FindClause(GetContextParent(), toolchain::omp::Clause::OMPC_ordered)}) {
      const auto &orderedClause{
          std::get<parser::OmpClause::Ordered>(clause->u)};
      const auto orderedValue{GetIntValue(orderedClause.v)};
      if (orderedValue > 0) {
...
void OmpStructureChecker::CheckTargetUpdate() {
  const parser::OmpClause *toWrapper{FindClause(toolchain::omp::Clause::OMPC_to)};
  const parser::OmpClause *fromWrapper{
      FindClause(toolchain::omp::Clause::OMPC_from)};
...
void OmpStructureChecker::Enter(
  ...
  switch (dir.v) {
  case toolchain::omp::Directive::OMPD_barrier:
...
void OmpStructureChecker::Enter(const parser::OpenMPFlushConstruct &x) {
  const auto &dirName{std::get<parser::OmpDirectiveName>(x.v.t)};
  PushContextAndClauseSets(dirName.source, toolchain::omp::Directive::OMPD_flush);
}
...
void OmpStructureChecker::Leave(const parser::OpenMPFlushConstruct &x) {
  ...
  if (flushList) {
    ...
    if (FindClause(toolchain::omp::Clause::OMPC_acquire) ||
        FindClause(toolchain::omp::Clause::OMPC_release) ||
        FindClause(toolchain::omp::Clause::OMPC_acq_rel)) {
...
void OmpStructureChecker::Enter(const parser::OpenMPCancelConstruct &x) {
  ...
  PushContextAndClauseSets(dirName.source, toolchain::omp::Directive::OMPD_cancel);

  if (auto maybeConstruct{GetCancelType(
          toolchain::omp::Directive::OMPD_cancel, x.source, maybeClauses)}) {
    ...
    if (CurrentDirectiveIsNested()) {
      ...
      auto getParentClauses{[&]() {
        ...
        return toolchain::concat<const toolchain::omp::Clause>(
            parent.actualClauses, parent.endDirectiveClauses);
      }};

      if (toolchain::omp::nestedCancelDoAllowedSet.test(*maybeConstruct)) {
        for (toolchain::omp::Clause clause : getParentClauses()) {
          if (clause == toolchain::omp::Clause::OMPC_nowait) {
...
void OmpStructureChecker::Enter(const parser::OpenMPCriticalConstruct &x) {
  ...
  const auto &block{std::get<parser::Block>(x.t)};
  CheckNoBranching(
      block, toolchain::omp::Directive::OMPD_critical, beginSpec.DirName().source);
...
void OmpStructureChecker::Enter(
  ...
  toolchain::omp::Directive dir{GetContext().directive};
  auto &dirName{std::get<parser::OmpDirectiveName>(x.v.t)};

  if (dir != toolchain::omp::Directive::OMPD_cancel &&
      dir != toolchain::omp::Directive::OMPD_cancellation_point) {
    ...
    switch (dirName.v) {
    case toolchain::omp::Directive::OMPD_do:
    case toolchain::omp::Directive::OMPD_parallel:
    case toolchain::omp::Directive::OMPD_sections:
    case toolchain::omp::Directive::OMPD_taskgroup:
      break;
    default:
      context_.Say(dirName.source,
...
void OmpStructureChecker::Enter(
  ...
  auto &maybeClauses{std::get<std::optional<parser::OmpClauseList>>(x.v.t)};
  PushContextAndClauseSets(
      dirName.source, toolchain::omp::Directive::OMPD_cancellation_point);

  if (auto maybeConstruct{
          GetCancelType(toolchain::omp::Directive::OMPD_cancellation_point, x.source,
              maybeClauses)}) {
    CheckCancellationNest(dirName.source, *maybeConstruct);
  }
}
...
std::optional<toolchain::omp::Directive> OmpStructureChecker::GetCancelType(
  ...
  std::optional<toolchain::omp::Directive> cancelee;
  toolchain::StringRef cancelName{getDirectiveName(cancelDir)};
...
void OmpStructureChecker::CheckCancellationNest(
  ...
  toolchain::StringRef typeName{getDirectiveName(type)};
  ...
  if (CurrentDirectiveIsNested()) {
    ...
    switch (type) {
    case toolchain::omp::Directive::OMPD_taskgroup:
      if (toolchain::omp::nestedCancelTaskgroupAllowedSet.test(
        ...
        if (dirContext_.size() >= 3) {
          ...
          if (GetContextParent().directive == toolchain::omp::Directive::OMPD_task ||
              FindClauseParent(toolchain::omp::Clause::OMPC_nogroup)) {
            for (int i = dirContext_.size() - 3; i >= 0; i--) {
              if (dirContext_[i].directive ==
                  toolchain::omp::Directive::OMPD_taskgroup) {
                break;
              }
              if (toolchain::omp::nestedCancelParallelAllowedSet.test(
                      dirContext_[i].directive)) {
      ...
      return;
    case toolchain::omp::Directive::OMPD_sections:
      if (toolchain::omp::nestedCancelSectionsAllowedSet.test(
              GetContextParent().directive)) {
        eligibleCancellation = true;
      }
      break;
    case toolchain::omp::Directive::OMPD_do:
      if (toolchain::omp::nestedCancelDoAllowedSet.test(
              GetContextParent().directive)) {
        eligibleCancellation = true;
      }
      break;
...
void OmpStructureChecker::Enter(const parser::OmpEndDirective &x) {
  ...
  switch (x.DirId()) {
  case toolchain::omp::Directive::OMPD_scope:
    PushContextAndClauseSets(source, toolchain::omp::Directive::OMPD_end_scope);
    break;
  // 2.7.3 end-single-clause -> copyprivate-clause |
  //                            nowait-clause
  case toolchain::omp::Directive::OMPD_single:
    PushContextAndClauseSets(source, toolchain::omp::Directive::OMPD_end_single);
    break;
  // 2.7.4 end-workshare -> END WORKSHARE [nowait-clause]
  case toolchain::omp::Directive::OMPD_workshare:
    PushContextAndClauseSets(source, toolchain::omp::Directive::OMPD_end_workshare);
    break;
  default:
    // no clauses are allowed
    break;
...
void OmpStructureChecker::Leave(const parser::OmpEndDirective &x) {
  if ((GetContext().directive == toolchain::omp::Directive::OMPD_end_scope) ||
      (GetContext().directive == toolchain::omp::Directive::OMPD_end_single) ||
      (GetContext().directive == toolchain::omp::Directive::OMPD_end_workshare)) {
    dirContext_.pop_back();
  }
}
...
void OmpStructureChecker::Leave(const parser::OmpClauseList &) {
  ...
  if (toolchain::omp::allDoSet.test(GetContext().directive)) {
    if (auto *clause{FindClause(toolchain::omp::Clause::OMPC_schedule)}) {
      ...
      if (ordering &&
        ...
        if (FindClause(toolchain::omp::Clause::OMPC_ordered)) {
          context_.Say(clause->source,
    ...
    if (auto *clause{FindClause(toolchain::omp::Clause::OMPC_ordered)}) {
      ...
      if (orderedClause.v) {
        CheckNotAllowedIfClause(
            toolchain::omp::Clause::OMPC_ordered, {toolchain::omp::Clause::OMPC_linear});

        if (auto *clause2{FindClause(toolchain::omp::Clause::OMPC_collapse)}) {
  ...
  if (toolchain::omp::allSimdSet.test(GetContext().directive)) {
    if (auto *clause{FindClause(toolchain::omp::Clause::OMPC_simdlen)}) {
      if (auto *clause2{FindClause(toolchain::omp::Clause::OMPC_safelen)}) {
    ...
    if (auto *sl_clause{FindClause(toolchain::omp::Clause::OMPC_safelen)}) {
      if (auto *o_clause{FindClause(toolchain::omp::Clause::OMPC_order)}) {
  ...
  if (GetContext().directive == toolchain::omp::Directive::OMPD_task) {
    if (auto *detachClause{FindClause(toolchain::omp::Clause::OMPC_detach)}) {
      unsigned version{context_.langOptions().OpenMPVersion};
      if (version == 50 || version == 51) {
        // OpenMP 5.0: 2.10.1 Task construct restrictions
        CheckNotAllowedIfClause(toolchain::omp::Clause::OMPC_detach,
            {toolchain::omp::Clause::OMPC_mergeable});
      } else if (version >= 52) {
        // OpenMP 5.2: 12.5.2 Detach construct restrictions
        if (FindClause(toolchain::omp::Clause::OMPC_final)) {
          context_.Say(GetContext().clauseSource,
        ...
        if (const auto *name{parser::Unwrap<parser::Name>(detach.v.v)}) {
          ...
          if (auto *dataEnvClause{
                  FindClause(toolchain::omp::Clause::OMPC_private)}) {
            const auto &pClause{
  ...
  OmpClauseSet threadprivateAllowedSet{toolchain::omp::Clause::OMPC_copyin,
      toolchain::omp::Clause::OMPC_copyprivate, toolchain::omp::Clause::OMPC_schedule,
      toolchain::omp::Clause::OMPC_num_threads, toolchain::omp::Clause::OMPC_thread_limit,
      toolchain::omp::Clause::OMPC_if};
  for (auto it : GetContext().clauseInfo) {
    toolchainOmpClause type = it.first;
...
void OmpStructureChecker::Enter(const parser::OmpClause &x) {
  ...
  switch (x.Id()) {
  case toolchain::omp::Clause::OMPC_copyprivate:
  case toolchain::omp::Clause::OMPC_enter:
  case toolchain::omp::Clause::OMPC_lastprivate:
  case toolchain::omp::Clause::OMPC_reduction:
  case toolchain::omp::Clause::OMPC_to:
    return;
  default:
    break;
  }
...
void OmpStructureChecker::Enter(const parser::OmpClause::Destroy &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_destroy);

  toolchain::omp::Directive dir{GetContext().directive};
  unsigned version{context_.langOptions().OpenMPVersion};
  if (dir == toolchain::omp::Directive::OMPD_depobj) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::Reduction &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_reduction);
  auto &objects{std::get<parser::OmpObjectList>(x.v.t)};

  if (OmpVerifyModifiers(x.v, toolchain::omp::OMPC_reduction,
  ...
  CheckReductionObjects(objects, toolchain::omp::Clause::OMPC_reduction);
  ...
  if (toolchain::omp::nestedReduceWorkshareAllowedSet.test(GetContext().directive)) {
    CheckSharedBindingInOuterContext(objects);
  }

  if (GetContext().directive == toolchain::omp::Directive::OMPD_loop) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::InReduction &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_in_reduction);
  auto &objects{std::get<parser::OmpObjectList>(x.v.t)};

  if (OmpVerifyModifiers(x.v, toolchain::omp::OMPC_in_reduction,
          GetContext().clauseSource, context_)) {
    auto &modifiers{OmpGetModifiers(x.v)};
    const auto *ident{
        OmpGetUniqueModifier<parser::OmpReductionIdentifier>(modifiers)};
    assert(ident && "reduction-identifier is a required modifier");
    if (CheckReductionOperator(*ident, OmpGetModifierSource(modifiers, ident),
            toolchain::omp::OMPC_in_reduction)) {
      CheckReductionObjectTypes(objects, *ident);
    }
  }
  CheckReductionObjects(objects, toolchain::omp::Clause::OMPC_in_reduction);
}

void OmpStructureChecker::Enter(const parser::OmpClause::TaskReduction &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_task_reduction);
  auto &objects{std::get<parser::OmpObjectList>(x.v.t)};

  if (OmpVerifyModifiers(x.v, toolchain::omp::OMPC_task_reduction,
          GetContext().clauseSource, context_)) {
    auto &modifiers{OmpGetModifiers(x.v)};
    const auto *ident{
        OmpGetUniqueModifier<parser::OmpReductionIdentifier>(modifiers)};
    assert(ident && "reduction-identifier is a required modifier");
    if (CheckReductionOperator(*ident, OmpGetModifierSource(modifiers, ident),
            toolchain::omp::OMPC_task_reduction)) {
      CheckReductionObjectTypes(objects, *ident);
    }
  }
  CheckReductionObjects(objects, toolchain::omp::Clause::OMPC_task_reduction);
}

bool OmpStructureChecker::CheckReductionOperator(
    const parser::OmpReductionIdentifier &ident, parser::CharBlock source,
    toolchain::omp::Clause clauseId) {
  ...
  auto visitDesignator{[&](const parser::ProcedureDesignator &procD) {
    ...
    if (name && name->symbol) {
      ...
      valid =
          toolchain::is_contained({"max", "min", "iand", "ior", "ieor"}, realName);
      if (!valid) {
...
void OmpStructureChecker::CheckReductionModifier(
  ...
  if (dirCtx.directive == toolchain::omp::Directive::OMPD_loop ||
      dirCtx.directive == toolchain::omp::Directive::OMPD_taskloop) {
    // [5.2:257:33-34]
  ...
  if (modifier.v == ReductionModifier::Value::Task) {
    ...
    static toolchain::omp::Directive worksharing[]{
        toolchain::omp::Directive::OMPD_do, toolchain::omp::Directive::OMPD_scope,
        toolchain::omp::Directive::OMPD_sections,
    ...
    if (dirCtx.directive != toolchain::omp::Directive::OMPD_parallel &&
        !toolchain::is_contained(worksharing, dirCtx.directive)) {
      context_.Say(GetContext().clauseSource,
    ...
    if (!toolchain::omp::scanParentAllowedSet.test(dirCtx.directive)) {
      context_.Say(GetContext().clauseSource,
...
void OmpStructureChecker::CheckSharedBindingInOuterContext(
  ...
  if (auto *enclosingContext{GetEnclosingDirContext()}) {
    for (auto it : enclosingContext->clauseInfo) {
      toolchainOmpClause type = it.first;
      const auto *clause = it.second;
      if (toolchain::omp::privateReductionSet.test(type)) {
        if (const auto *objList{GetOmpObjectList(*clause)}) {
          for (const auto &ompObject : objList->v) {
            if (const auto *name{parser::Unwrap<parser::Name>(ompObject)}) {
              if (const auto *symbol{name->symbol}) {
                    ...
                    if (const auto *rsymbol{rname->symbol}) {
                      if (rsymbol->name() == symbol->name()) {
                        context_.Say(GetContext().clauseSource,
                            ...
                            parser::ToUpperCaseLetters(
                                getClauseName(toolchain::omp::Clause::OMPC_reduction)
                                    .str()),
                            symbol->name(),
                            parser::ToUpperCaseLetters(
...
void OmpStructureChecker::Enter(const parser::OmpClause::Ordered &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_ordered);
  // the parameter of ordered clause is optional
  if (const auto &expr{x.v}) {
    RequiresConstantPositiveParameter(toolchain::omp::Clause::OMPC_ordered, *expr);
    // 2.8.3 Loop SIMD Construct Restriction
    if (toolchain::omp::allDoSimdSet.test(GetContext().directive)) {
      context_.Say(GetContext().clauseSource,
...
void OmpStructureChecker::Enter(const parser::OmpClause::Shared &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_shared);
...
void OmpStructureChecker::Enter(const parser::OmpClause::Private &x) {
  SymbolSourceMap symbols;
  GetSymbolsInObjectList(x.v, symbols);
  CheckAllowedClause(toolchain::omp::Clause::OMPC_private);
  CheckVarIsNotPartOfAnotherVar(GetContext().clauseSource, x.v, "PRIVATE");
  CheckIntentInPointer(symbols, toolchain::omp::Clause::OMPC_private);
  CheckCrayPointee(x.v, "PRIVATE");
}

void OmpStructureChecker::Enter(const parser::OmpClause::Nowait &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_nowait);
}
              ...
              if (IsDataRefTypeParamInquiry(dataRef)) {
                context_.Say(source,
                    ...
                    ContextDirectiveAsFortran());
                             ...
                             ompObject) ||
                  parser::Unwrap<parser::ArrayElement>(ompObject)) {
                if (toolchain::omp::nonPartialVarSet.test(GetContext().directive)) {
                  context_.Say(source,
...
void OmpStructureChecker::Enter(const parser::OmpClause::Firstprivate &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_firstprivate);

  CheckVarIsNotPartOfAnotherVar(GetContext().clauseSource, x.v, "FIRSTPRIVATE");
  CheckCrayPointee(x.v, "FIRSTPRIVATE");
  CheckIsLoopIvPartOfClause(toolchainOmpClause::OMPC_firstprivate, x.v);

  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(x.v, currSymbols);
  CheckCopyingPolymorphicAllocatable(
      currSymbols, toolchain::omp::Clause::OMPC_firstprivate);

  DirectivesClauseTriple dirClauseTriple;
  // Check firstprivate variables in worksharing constructs
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_do,
      std::make_pair(
          toolchain::omp::Directive::OMPD_parallel, toolchain::omp::privateReductionSet));
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_sections,
      std::make_pair(
          toolchain::omp::Directive::OMPD_parallel, toolchain::omp::privateReductionSet));
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_single,
      std::make_pair(
          toolchain::omp::Directive::OMPD_parallel, toolchain::omp::privateReductionSet));
  // Check firstprivate variables in distribute construct
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_distribute,
      std::make_pair(
          toolchain::omp::Directive::OMPD_teams, toolchain::omp::privateReductionSet));
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_distribute,
      std::make_pair(toolchain::omp::Directive::OMPD_target_teams,
          toolchain::omp::privateReductionSet));
  // Check firstprivate variables in task and taskloop constructs
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_task,
      std::make_pair(toolchain::omp::Directive::OMPD_parallel,
          OmpClauseSet{toolchain::omp::Clause::OMPC_reduction}));
  dirClauseTriple.emplace(toolchain::omp::Directive::OMPD_taskloop,
      std::make_pair(toolchain::omp::Directive::OMPD_parallel,
          OmpClauseSet{toolchain::omp::Clause::OMPC_reduction}));

  CheckPrivateSymbolsInOuterCxt(
      currSymbols, dirClauseTriple, toolchain::omp::Clause::OMPC_firstprivate);
}
...
void OmpStructureChecker::Enter(const parser::OmpClause::Aligned &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_aligned);
  if (OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_aligned, GetContext().clauseSource, context_)) {
    auto &modifiers{OmpGetModifiers(x.v)};
...
void OmpStructureChecker::Enter(const parser::OmpClause::Defaultmap &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_defaultmap);
  unsigned version{context_.langOptions().OpenMPVersion};
  ...
  if (!OmpVerifyModifiers(x.v, toolchain::omp::OMPC_defaultmap,
          GetContext().clauseSource, context_)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::If &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_if);
  unsigned version{context_.langOptions().OpenMPVersion};
  toolchain::omp::Directive dir{GetContext().directive};

  auto isConstituent{[](toolchain::omp::Directive dir, toolchain::omp::Directive part) {
    using namespace toolchain::omp;
    toolchain::ArrayRef<Directive> dirLeafs{getLeafConstructsOrSelf(dir)};
    toolchain::ArrayRef<Directive> partLeafs{getLeafConstructsOrSelf(part)};
    ...
    if (auto first = toolchain::find(dirLeafs, partLeafs.front());
        first != dirLeafs.end()) {
      ...
      toolchain::ArrayRef<Directive> subSeq{
  ...
  if (OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_if, GetContext().clauseSource, context_)) {
    auto &modifiers{OmpGetModifiers(x.v)};
    if (auto *dnm{OmpGetUniqueModifier<parser::OmpDirectiveNameModifier>(
            modifiers)}) {
      toolchain::omp::Directive sub{dnm->v};
      std::string subName{
      ...
      if (!isConstituent(dir, sub)) {
        ...
        static toolchain::omp::Directive valid45[]{
            toolchain::omp::OMPD_cancel, //
            toolchain::omp::OMPD_parallel, //
            /* OMP 5.0+ also allows OMPD_simd */
            toolchain::omp::OMPD_target, //
            toolchain::omp::OMPD_target_data, //
            toolchain::omp::OMPD_target_enter_data, //
            toolchain::omp::OMPD_target_exit_data, //
            toolchain::omp::OMPD_target_update, //
            toolchain::omp::OMPD_task, //
            toolchain::omp::OMPD_taskloop, //
            /* OMP 5.2+ also allows OMPD_teams */
        };
        if (version < 50 && sub == toolchain::omp::OMPD_simd) {
          context_.Say(modifierSource,
              "%s is not allowed as '%s' in %s, %s"_warn_en_US, subName,
              modName, ThisVersion(version), TryVersion(50));
        } else if (version < 52 && sub == toolchain::omp::OMPD_teams) {
          context_.Say(modifierSource,
              "%s is not allowed as '%s' in %s, %s"_warn_en_US, subName,
              modName, ThisVersion(version), TryVersion(52));
        } else if (!toolchain::is_contained(valid45, sub) &&
            sub != toolchain::omp::OMPD_simd && sub != toolchain::omp::OMPD_teams) {
          context_.Say(modifierSource,
...
void OmpStructureChecker::Enter(const parser::OmpClause::Detach &x) {
  unsigned version{context_.langOptions().OpenMPVersion};
  if (version >= 52) {
    SetContextClauseInfo(toolchain::omp::Clause::OMPC_detach);
  } else {
    // OpenMP 5.0: 2.10.1 Task construct restrictions
    CheckAllowedClause(toolchain::omp::Clause::OMPC_detach);
  }
...
void OmpStructureChecker::CheckAllowedMapTypes(parser::OmpMapType::Value type,
    toolchain::ArrayRef<parser::OmpMapType::Value> allowed) {
  if (toolchain::is_contained(allowed, type)) {
    return;
  }

  toolchain::SmallVector<std::string> names;
  toolchain::transform(
  ...
  context_.Say(GetContext().clauseSource,
      ...
      toolchain::join(names, ", "), ContextDirectiveAsFortran());
}

void OmpStructureChecker::Enter(const parser::OmpClause::Map &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_map);
  if (!OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_map, GetContext().clauseSource, context_)) {
    return;
  }
  ...
  if (auto *type{OmpGetUniqueModifier<parser::OmpMapType>(modifiers)}) {
    ...
    toolchain::SmallVector<parser::OmpMapType::Value> mapEnteringTypes{[&]() {
      toolchain::SmallVector<parser::OmpMapType::Value> result;
    ...
    toolchain::SmallVector<parser::OmpMapType::Value> mapExitingTypes{[&]() {
      toolchain::SmallVector<parser::OmpMapType::Value> result;
      for (size_t i{0}; i != parser::OmpMapType::Value_enumSize; ++i) {
        auto t{static_cast<parser::OmpMapType::Value>(i)};
    ...
    toolchain::omp::Directive dir{GetContext().directive};
    toolchain::ArrayRef<toolchain::omp::Directive> leafs{
        toolchain::omp::getLeafConstructsOrSelf(dir)};

    if (toolchain::is_contained(leafs, Directive::OMPD_target) ||
        toolchain::is_contained(leafs, Directive::OMPD_target_data)) {
      if (version >= 60) {
    ...
    } else if (toolchain::is_contained(leafs, Directive::OMPD_target_enter_data)) {
      CheckAllowedMapTypes(type->v, mapEnteringTypes);
    } else if (toolchain::is_contained(leafs, Directive::OMPD_target_exit_data)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::Schedule &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_schedule);
  const parser::OmpScheduleClause &scheduleClause = x.v;
  if (!OmpVerifyModifiers(scheduleClause, toolchain::omp::OMPC_schedule,
          GetContext().clauseSource, context_)) {
    return;
  }
    ...
    if (chunk) {
      ...
      if (const auto &chunkExpr{std::get<std::optional<parser::ScalarIntExpr>>(
        ...
        RequiresPositiveParameter(
            toolchain::omp::Clause::OMPC_schedule, *chunkExpr, "chunk size");
      }
    }
...
void OmpStructureChecker::Enter(const parser::OmpClause::Device &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_device);
  const parser::OmpDeviceClause &deviceClause{x.v};
  const auto &device{std::get<parser::ScalarIntExpr>(deviceClause.t)};
  RequiresPositiveParameter(
      toolchain::omp::Clause::OMPC_device, device, "device expression");
  toolchain::omp::Directive dir{GetContext().directive};

  if (OmpVerifyModifiers(deviceClause, toolchain::omp::OMPC_device,
          GetContext().clauseSource, context_)) {
    ...
    if (auto *deviceMod{
      ...
      if (dir != toolchain::omp::OMPD_target && deviceMod->v == Value::Ancestor) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::Depend &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_depend);
  toolchain::omp::Directive dir{GetContext().directive};
  unsigned version{context_.langOptions().OpenMPVersion};
  ...
  if (dir == toolchain::omp::OMPD_depobj) {
  ...
  } else if (dir != toolchain::omp::OMPD_ordered) {
    if (doaDep) {
      context_.Say(GetContext().clauseSource,
        ...
        if (auto *dataRef{std::get_if<parser::DataRef>(&designator->u)}) {
          ...
          if (const auto *arr{
            ...
            CheckArraySection(arr->value(), GetLastName(*dataRef),
                toolchain::omp::Clause::OMPC_depend);
          }
        }
      }
    }
    if (OmpVerifyModifiers(*taskDep, toolchain::omp::OMPC_depend,
            GetContext().clauseSource, context_)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::Doacross &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_doacross);
  CheckDoacross(x.v.v);
}
...
void OmpStructureChecker::CheckDoacross(const parser::OmpDoacross &doa) {
  ...
  for (const LoopConstruct &loop : toolchain::reverse(loopStack_)) {
    if (auto *doc{std::get_if<const parser::DoConstruct *>(&loop)}) {
      ...
      toolchain::omp::Directive loopDir{GetLoopDirective(**loopc)};
      if (loopDir == toolchain::omp::OMPD_do || loopDir == toolchain::omp::OMPD_simd) {
        auto IsOrdered{[](const parser::OmpClause &c) {
          return c.Id() == toolchain::omp::OMPC_ordered;
        }};
        // If it has ORDERED clause, stop the traversal.
        if (toolchain::any_of(GetLoopClauses(**loopc), IsOrdered)) {
          break;
        }
      }
    }
...
void OmpStructureChecker::CheckCopyingPolymorphicAllocatable(
    SymbolSourceMap &symbols, const toolchain::omp::Clause clause) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::Copyprivate &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_copyprivate);
  SymbolSourceMap symbols;
  GetSymbolsInObjectList(x.v, symbols);
  CheckVariableListItem(symbols);
  CheckIntentInPointer(symbols, toolchain::omp::Clause::OMPC_copyprivate);
  CheckCopyingPolymorphicAllocatable(
      symbols, toolchain::omp::Clause::OMPC_copyprivate);
}

void OmpStructureChecker::Enter(const parser::OmpClause::Lastprivate &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_lastprivate);
  ...
  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(objectList, currSymbols);
  CheckDefinableObjects(currSymbols, toolchain::omp::Clause::OMPC_lastprivate);
  CheckCopyingPolymorphicAllocatable(
      currSymbols, toolchain::omp::Clause::OMPC_lastprivate);
  ...
  CheckPrivateSymbolsInOuterCxt(
      currSymbols, dirClauseTriple, toolchain::omp::Clause::OMPC_lastprivate);

  if (OmpVerifyModifiers(x.v, toolchain::omp::OMPC_lastprivate,
          GetContext().clauseSource, context_)) {
    auto &modifiers{OmpGetModifiers(x.v)};
...
void OmpStructureChecker::Enter(const parser::OmpClause::Copyin &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_copyin);

  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(x.v, currSymbols);
  CheckCopyingPolymorphicAllocatable(
      currSymbols, toolchain::omp::Clause::OMPC_copyin);
}
...
void OmpStructureChecker::Enter(const parser::OmpClause::Update &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_update);
  toolchain::omp::Directive dir{GetContext().directive};
  unsigned version{context_.langOptions().OpenMPVersion};
  ...
  if (!depType && !taskType) {
    assert(dir == toolchain::omp::Directive::OMPD_atomic &&
        "Unexpected alternative in update clause");
    return;
  }
...
void OmpStructureChecker::Enter(const parser::OmpClause::UseDevicePtr &x) {
  CheckStructureComponent(x.v, toolchain::omp::Clause::OMPC_use_device_ptr);
  CheckAllowedClause(toolchain::omp::Clause::OMPC_use_device_ptr);
  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(x.v, currSymbols);
  semantics::UnorderedSymbolSet listVars;
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_use_device_ptr)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::UseDeviceAddr &x) {
  CheckStructureComponent(x.v, toolchain::omp::Clause::OMPC_use_device_addr);
  CheckAllowedClause(toolchain::omp::Clause::OMPC_use_device_addr);
  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(x.v, currSymbols);
  semantics::UnorderedSymbolSet listVars;
  for (auto [_, clause] :
      FindClauses(toolchain::omp::Clause::OMPC_use_device_addr)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::IsDevicePtr &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_is_device_ptr);
  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(x.v, currSymbols);
  semantics::UnorderedSymbolSet listVars;
  for (auto [_, clause] : FindClauses(toolchain::omp::Clause::OMPC_is_device_ptr)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::HasDeviceAddr &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_has_device_addr);
  SymbolSourceMap currSymbols;
  GetSymbolsInObjectList(x.v, currSymbols);
  semantics::UnorderedSymbolSet listVars;
  for (auto [_, clause] :
      FindClauses(toolchain::omp::Clause::OMPC_has_device_addr)) {
...
void OmpStructureChecker::Enter(const parser::OmpClause::Enter &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_enter);
  if (!OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_enter, GetContext().clauseSource, context_)) {
    return;
  }
...
void OmpStructureChecker::Enter(const parser::OmpClause::From &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_from);
  if (!OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_from, GetContext().clauseSource, context_)) {
    return;
  }
...
void OmpStructureChecker::Enter(const parser::OmpClause::To &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_to);
  if (!OmpVerifyModifiers(
          x.v, toolchain::omp::OMPC_to, GetContext().clauseSource, context_)) {
    return;
  }
  ...
  // update".
  if (GetContext().directive == toolchain::omp::OMPD_declare_target) {
    return;
  }

  assert(GetContext().directive == toolchain::omp::OMPD_target_update);
  if (auto *iter{OmpGetUniqueModifier<parser::OmpIterator>(modifiers)}) {
    CheckIteratorModifier(*iter);
  }
...
void OmpStructureChecker::Enter(const parser::OmpClause::OmpxBare &x) {
  ...
  if (GetContext().directive != toolchain::omp::Directive::OMPD_target_teams) {
    context_.Say(GetContext().clauseSource,
        "%s clause is only allowed on combined TARGET TEAMS"_err_en_US,
        parser::ToUpperCaseLetters(getClauseName(toolchain::omp::OMPC_ompx_bare)));
  }
}

toolchain::StringRef OmpStructureChecker::getClauseName(toolchain::omp::Clause clause) {
  return toolchain::omp::getOpenMPClauseName(clause);
}

toolchain::StringRef OmpStructureChecker::getDirectiveName(
    toolchain::omp::Directive directive) {
  unsigned version{context_.langOptions().OpenMPVersion};
  return toolchain::omp::getOpenMPDirectiveName(directive, version);
}
...
void OmpStructureChecker::CheckArraySection(
    ...
    const toolchain::omp::Clause clause) {
...
void OmpStructureChecker::CheckIntentInPointer(
    SymbolSourceMap &symbols, toolchain::omp::Clause clauseId) {
  for (auto &[symbol, source] : symbols) {
...
void OmpStructureChecker::CheckProcedurePointer(
    SymbolSourceMap &symbols, toolchain::omp::Clause clause) {
  for (const auto &[symbol, source] : symbols) {
    if (IsProcedurePointer(*symbol)) {
      context_.Say(source,
...
void OmpStructureChecker::CheckCrayPointee(
    const parser::OmpObjectList &objectList, toolchain::StringRef clause,
...
void OmpStructureChecker::CheckPrivateSymbolsInOuterCxt(
    ...
    const toolchain::omp::Clause currClause) {
  ...
  if (!block.empty()) {
    ...
    if (const auto *ompConstruct{
      ...
      if (const auto *ompBlockConstruct{
        ...
        toolchain::omp::Directive dirId{ompBlockConstruct->BeginDir().DirId()};
        if (dirId == toolchain::omp::Directive::OMPD_teams) {
          nestedTeams = true;
        }
      }
    }
  ...
  for (auto it{block.begin()}; it != block.end(); ++it) {
    if (parser::Unwrap<parser::AssignmentStmt>(*it) ||
      ...
      if (const auto *ompAtomicConstruct{
        ...
        // 'Parallel' constructs
        auto currentDir{toolchain::omp::Directive::OMPD_unknown};
        if (const auto *ompBlockConstruct{
                std::get_if<parser::OpenMPBlockConstruct>(&ompConstruct->u)}) {
        ...
        if (!toolchain::omp::topParallelSet.test(currentDir)) {
          context_.Say(source,
...
void OmpStructureChecker::Enter(
  ...
  CheckAllowedRequiresClause(toolchain::omp::Clause::OMPC_atomic_default_mem_order);
}

void OmpStructureChecker::Enter(const parser::OmpClause::DynamicAllocators &x) {
  CheckAllowedRequiresClause(toolchain::omp::Clause::OMPC_dynamic_allocators);
}

void OmpStructureChecker::Enter(const parser::OmpClause::ReverseOffload &x) {
  CheckAllowedRequiresClause(toolchain::omp::Clause::OMPC_reverse_offload);
}

void OmpStructureChecker::Enter(const parser::OmpClause::UnifiedAddress &x) {
  CheckAllowedRequiresClause(toolchain::omp::Clause::OMPC_unified_address);
}

void OmpStructureChecker::Enter(
    const parser::OmpClause::UnifiedSharedMemory &x) {
  CheckAllowedRequiresClause(toolchain::omp::Clause::OMPC_unified_shared_memory);
}

void OmpStructureChecker::Enter(const parser::OmpClause::SelfMaps &x) {
  CheckAllowedRequiresClause(toolchain::omp::Clause::OMPC_self_maps);
}
...
void OmpStructureChecker::Enter(const parser::OpenMPInteropConstruct &x) {
  ...
  PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_interop);
  const auto &clauseList{std::get<std::optional<parser::OmpClauseList>>(x.v.t)};
  for (const auto &clause : clauseList->v) {
    common::visit(
        common::visitors{
            [&](const parser::OmpClause::Init &initClause) {
              if (OmpVerifyModifiers(initClause.v, toolchain::omp::OMPC_init,
              ...
              if (toolchain::is_contained(objectSymbolList, objectSymbol)) {
                context_.Say(GetContext().directiveSource,
...
void OmpStructureChecker::CheckAllowedRequiresClause(toolchainOmpClause clause) {
  CheckAllowedClause(clause);

  if (clause != toolchain::omp::Clause::OMPC_atomic_default_mem_order) {
