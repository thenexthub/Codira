...
const DeclTypeSpec &InstantiateHelper::InstantiateIntrinsicType(
  ...
  if (auto value{evaluate::ToInt64(copy)}) {
    ...
    toolchain::raw_string_ostream sstream(exprString);
  ...
  case (language::Compability::semantics::DerivedTypeSpec::Category::IntrinsicVector): {
    ...
    for (const auto &pair : parameters()) {
      if (pair.first == "element_category") {
        vecElemCategory =
            language::Compability::evaluate::ToInt64(pair.second.GetExplicit()).value_or(-1);
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &o, const DerivedTypeSpec &x) {
  return o << x.AsFortran();
}

Bound::Bound(common::ConstantSubscript bound) : expr_{bound} {}

toolchain::raw_ostream &operator<<(toolchain::raw_ostream &o, const Bound &x) {
  if (x.isStar()) {
    o << '*';
  } else if (x.isColon()) {
    o << ':';
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &o, const ShapeSpec &x) {
  if (x.lb_.isStar()) {
    CHECK(x.ub_.isStar());
    o << "..";
  } else {
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const ArraySpec &arraySpec) {
  char sep{'('};
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &o, const ParamValue &x) {
  return o << x.AsFortran();
}
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &o, const DeclTypeSpec &x) {
  return o << x.AsFortran();
}
