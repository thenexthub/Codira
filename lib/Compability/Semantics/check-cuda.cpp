...
static const toolchain::StringSet<> warpFunctions_ = {"match_all_syncjj",
          ...
          if (*attrs == common::CUDASubprogramAttrs::HostDevice ||
            ...
            if (warpFunctions_.contains(sym->name().ToString()) &&
                !context_.languageFeatures().IsEnabled(
                    language::Compability::common::LanguageFeature::CudaWarpMatchFunction)) {
...
struct FindHostArray
  ...
  Result operator()(const Symbol &symbol) const {
    if (const auto *details{
      ...
      if (details->IsArray() &&
          !symbol.attrs().test(language::Compability::semantics::Attr::PARAMETER) &&
        ...
        common::visitors{
            ...
            [&](const common::Indirection<parser::WriteStmt> &x) {
              if (x.value().format) { // Formatted write to '*' or '6'
                if (std::holds_alternative<language::Compability::parser::Star>(
                        x.value().format->u)) {
                  if (x.value().iounit) {
                    if (std::holds_alternative<language::Compability::parser::Star>(
                            x.value().iounit->u)) {
    ...
    if (loopControl) {
      if (const auto *concurrentControl{
        ...
        const auto &concurrentHeader =
            std::get<language::Compability::parser::ConcurrentHeader>(concurrentControl->t);
        const auto &controls =
...
static bool IsOpenACCComputeConstruct(const parser::OpenACCBlockConstruct &x) {
  const auto &beginBlockDirective =
      std::get<language::Compability::parser::AccBeginBlockDirective>(x.t);
  ...
  if (blockDirective.v == toolchain::acc::ACCD_parallel ||
      blockDirective.v == toolchain::acc::ACCD_serial ||
      blockDirective.v == toolchain::acc::ACCD_kernels) {
    return true;
  }
  return false;
}
