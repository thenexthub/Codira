...
#ifndef LANGUAGE_COMPABILITY_SEMANTICS_CHECK_DIRECTIVE_STRUCTURE_H_
#define LANGUAGE_COMPABILITY_SEMANTICS_CHECK_DIRECTIVE_STRUCTURE_H_
...
public:
  ...
  void Post(const parser::CycleStmt &cycleStmt) {
    if (const auto &cycleName{cycleStmt.v}) {
      ...
      if constexpr (std::is_same_v<D, toolchain::omp::Directive>) {
        switch ((toolchain::omp::Directive)currentDirective_) {
        ...
        case toolchain::omp::Directive::OMPD_distribute_parallel_for:
        case toolchain::omp::Directive::OMPD_distribute_simd:
        case toolchain::omp::Directive::OMPD_distribute_parallel_for_simd:
        ...
        case toolchain::omp::Directive::
            OMPD_target_teams_distribute_parallel_do_simd:
          return;
        default:
          break;
        }
      } else if constexpr (std::is_same_v<D, toolchain::acc::Directive>) {
        switch ((toolchain::acc::Directive)currentDirective_) {
        // exclude loop directives which do not need a check for unlabelled
        // CYCLES
        case toolchain::acc::Directive::ACCD_loop:
...
protected:
  ...
  toolchain::iterator_range<typename ClauseMapTy::iterator> FindClauses(C type) {
    auto it{GetContext().clauseInfo.equal_range(type)};
    return toolchain::make_range(it);
  }
  ...
  void RequiresPositiveParameter(const C &clause,
      const parser::ScalarIntExpr &i, toolchain::StringRef paramName = "parameter");

  void OptionalConstantPositiveParameter(
      const C &clause, const std::optional<parser::ScalarIntConstantExpr> &o);

  virtual toolchain::StringRef getClauseName(C clause) { return ""; };

  virtual toolchain::StringRef getDirectiveName(D directive) { return ""; };
...
void DirectiveStructureChecker<D, C, PC,
  ...
  if (!toolchain::is_contained(GetContext().actualClauses, clause)) {
...
void DirectiveStructureChecker<D, C, PC,
    ...
    const parser::ScalarIntExpr &i, toolchain::StringRef paramName) {
  if (const auto v{GetIntValue(i)}) {
    if (*v < 0) {
      context_.Say(GetContext().clauseSource,
