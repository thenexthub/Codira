...
void OmpStructureChecker::HasInvalidDistributeNesting(
  ...
  if (toolchain::omp::topDistributeSet.test(beginDir.v)) {
...
void OmpStructureChecker::HasInvalidLoopBinding(
  ...
  if (toolchain::omp::Directive::OMPD_loop == beginDir.v &&
      CurrentDirectiveIsNested() &&
      toolchain::omp::bottomTeamsSet.test(GetContextParent().directive)) {
    teamsBindingChecker(
  ...
  if (OmpDirectiveSet{
          toolchain::omp::OMPD_teams_loop, toolchain::omp::OMPD_target_teams_loop}
          .test(beginDir.v)) {
    teamsBindingChecker(
  ...
  common::visit(
      common::visitors{
          ...
          [&](const parser::OpenMPBlockConstruct &c) {
            ...
            if (beginSpec.DirId() == toolchain::omp::Directive::OMPD_ordered) {
          ...
          [&](const parser::OpenMPStandaloneConstruct &c) {
            if (auto *ssc{std::get_if<parser::OpenMPSimpleStandaloneConstruct>(
                    &c.u)}) {
              toolchain::omp::Directive dirId{ssc->v.DirId()};
              if (dirId == toolchain::omp::Directive::OMPD_ordered) {
                for (const parser::OmpClause &x : ssc->v.Clauses().v) {
                  if (x.Id() == toolchain::omp::Clause::OMPC_simd) {
                    eligibleSIMD = true;
                    break;
                  }
                }
              } else if (dirId == toolchain::omp::Directive::OMPD_scan) {
                eligibleSIMD = true;
              }
            }
          },
          ...
          [&](const parser::OpenMPLoopConstruct &c) {
            ...
            if ((beginDir.v == toolchain::omp::Directive::OMPD_simd) ||
                (beginDir.v == toolchain::omp::Directive::OMPD_do_simd) ||
                (beginDir.v == toolchain::omp::Directive::OMPD_loop)) {
              eligibleSIMD = true;
            }
          },
...
void OmpStructureChecker::Enter(const parser::OpenMPLoopConstruct &x) {
  ...
  if ((beginDir.v == toolchain::omp::Directive::OMPD_distribute_parallel_do_simd) ||
      (beginDir.v == toolchain::omp::Directive::OMPD_distribute_simd)) {
    CheckDistLinear(x);
  }
}
...
void OmpStructureChecker::Enter(const parser::OmpEndLoopDirective &x) {
  ...
  case toolchain::omp::Directive::OMPD_do:
    PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_end_do);
    break;
  case toolchain::omp::Directive::OMPD_do_simd:
    PushContextAndClauseSets(
        dir.source, toolchain::omp::Directive::OMPD_end_do_simd);
    break;
  default:
...
void OmpStructureChecker::Leave(const parser::OmpEndLoopDirective &x) {
  if ((GetContext().directive == toolchain::omp::Directive::OMPD_end_do) ||
      (GetContext().directive == toolchain::omp::Directive::OMPD_end_do_simd)) {
    dirContext_.pop_back();
  }
}

void OmpStructureChecker::Enter(const parser::OmpClause::Linear &x) {
  CheckAllowedClause(toolchain::omp::Clause::OMPC_linear);
  ...
  if (OmpVerifyModifiers(x.v, toolchain::omp::OMPC_linear, clauseSource, context_)) {
    auto &modifiers{OmpGetModifiers(x.v)};
    linearMod = OmpGetUniqueModifier<parser::OmpLinearModifier>(modifiers);
    if (linearMod) {
      // 2.7 Loop Construct Restriction
      if ((toolchain::omp::allDoSet | toolchain::omp::allSimdSet).test(dir)) {
  ...
  if (const auto *clause{
          FindClause(GetContext(), toolchain::omp::Clause::OMPC_ordered)}) {
  ...
  for (auto &[symbol, source] : symbols) {
    ...
    if (dir == toolchain::omp::Directive::OMPD_declare_simd && !IsDummy(*symbol)) {
      context_.Say(source,
