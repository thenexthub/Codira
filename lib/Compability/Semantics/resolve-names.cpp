...
#include "language/Compability/Semantics/program-tree.h"
...
private:
  ...
  friend toolchain::raw_ostream &operator<<(
      toolchain::raw_ostream &, const ImplicitRules &);
  friend void ShowImplicitRule(
      toolchain::raw_ostream &, const ImplicitRules &, char);
};
...
protected:
  ...
  Attr AccessSpecToAttr(const parser::AccessSpec &x) {
    ...
    toolchain_unreachable("Switch covers all cases"); // suppress g++ warning
  }
...
public:
  ...
  bool Pre(const parser::OmpBeginDirective &x) {
    ...
    if (x.DirName().v == toolchain::omp::Directive::OMPD_critical) {
...
bool OmpVisitor::NeedsScope(const parser::OmpClause &x) {
  ...
  // the clause.
  return toolchain::omp::canHaveIterator(x.Id());
}
...
parser::CharBlock MangleSpecialFunctions(const parser::CharBlock &name) {
  return toolchain::StringSwitch<parser::CharBlock>(name.ToString())
      .Case("max", {"op.max", 6})
      .Case("min", {"op.min", 6})
      .Case("iand", {"op.iand", 7})
      .Case("ior", {"op.ior", 6})
...
void OmpVisitor::ResolveCriticalName(const parser::OmpArgument &arg) {
  auto &globalScope{[&]() -> Scope & {
    ...
    toolchain_unreachable("Cannot find global scope");
  }()};
...
bool OmpVisitor::Pre(const parser::OmpDirectiveSpecification &x) {
  ...
  switch (x.DirId()) {
  case toolchain::omp::Directive::OMPD_declare_mapper:
    ...
    break;
  case toolchain::omp::Directive::OMPD_declare_reduction:
    if (maybeArgs && maybeClauses) {
      const parser::OmpArgument &first{maybeArgs->v.front()};
...
public:
  ...
  bool Pre(const parser::MainProgram &) {
    toolchain_unreachable("This node is handled in ProgramUnit");
  }
...
toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &o, const ImplicitRules &implicitRules) {
  o << "ImplicitRules:\n";
...
void ShowImplicitRule(
    toolchain::raw_ostream &o, const ImplicitRules &implicitRules, char ch) {
...
void DeclarationVisitor::Post(const parser::VectorTypeSpec &x) {
  ...
  toolchain::SmallVector<ParamValue> typeParams;
