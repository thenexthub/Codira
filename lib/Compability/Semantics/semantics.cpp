...
static void DoDumpSymbols(toolchain::raw_ostream &, const Scope &, int indent = 0);
static void PutIndent(toolchain::raw_ostream &, int indent);
  ...
  static Symbol *CommonBlockIsInitialized(const Symbol &common) {
    ...
    for (const language::Compability::semantics::EquivalenceSet &set :
      ...
      for (const language::Compability::semantics::EquivalenceObject &obj : set) {
        if (!obj.symbol.test(
                language::Compability::semantics::Symbol::Flag::CompilerCreated)) {
...
bool Semantics::Perform() {
  ...
  if (!program_.v.empty()) {
    ...
    if (frontModule &&
      ...
      toolchain::Triple targetTriple{toolchain::Triple(
...
void Semantics::EmitMessages(toolchain::raw_ostream &os) {
...
void SemanticsContext::DumpSymbols(toolchain::raw_ostream &os) {
...
void Semantics::DumpSymbols(toolchain::raw_ostream &os) { context_.DumpSymbols(os); }

void Semantics::DumpSymbolsSources(toolchain::raw_ostream &os) const {
...
void DoDumpSymbols(toolchain::raw_ostream &os, const Scope &scope, int indent) {
  PutIndent(os, indent);
...
static void PutIndent(toolchain::raw_ostream &os, int indent) {
