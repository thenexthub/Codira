...
#include "language/Compability/Evaluate/match.h"
#include "language/Compability/Evaluate/rewrite.h"
...
namespace operation = language::Compability::evaluate::operation;
...
static std::pair<parser::CharBlock, parser::CharBlock> SplitAssignmentSource(
  ...
  for (size_t next{source.size()}; next > 1; --next) {
    if (sv[next - 1] == '=' && !toolchain::is_contained("<>=/", sv[next - 2])) {
      std::string_view lhs(sv.data(), next - 1);
      std::string_view rhs(sv.data() + next, sv.size() - next);
      return std::make_pair(trim(lhs), trim(rhs));
    }
  }
  toolchain_unreachable("Could not find assignment operator");
}
...
void OmpStructureChecker::CheckAtomicType(
  ...
  if (const DerivedTypeSpec *derived{typeSpec->AsDerived()}) {
    if (toolchain::any_of(derived->parameters(), [](auto &&entry) {
...
void OmpStructureChecker::CheckStorageOverlap(const SomeExpr &base,
    toolchain::ArrayRef<evaluate::Expr<evaluate::SomeType>> exprs,
    parser::CharBlock source) {
...
OmpStructureChecker::CheckUpdateCapture(
  ...
  toolchain_unreachable("Unchecked condition");
}
...
void OmpStructureChecker::Enter(const parser::OpenMPAtomicConstruct &x) {
  ...
  std::array atomic{toolchain::omp::Clause::OMPC_read,
      toolchain::omp::Clause::OMPC_update, toolchain::omp::Clause::OMPC_write};
  std::array memoryOrder{toolchain::omp::Clause::OMPC_acq_rel,
      toolchain::omp::Clause::OMPC_acquire, toolchain::omp::Clause::OMPC_relaxed,
      toolchain::omp::Clause::OMPC_release, toolchain::omp::Clause::OMPC_seq_cst};

  auto checkExclusive{[&](toolchain::ArrayRef<toolchain::omp::Clause> group,
                          std::string_view name,
                          const parser::OmpClauseList &clauses) {
    const parser::OmpClause *present{nullptr};
    for (const parser::OmpClause &clause : clauses.v) {
      toolchain::omp::Clause id{clause.Id()};
      if (!toolchain::is_contained(group, id)) {
        continue;
      }
      if (present == nullptr) {
        present = &clause;
  ...
  PushContextAndClauseSets(dir.source, toolchain::omp::Directive::OMPD_atomic);
  toolchain::omp::Clause kind{x.GetKind()};
