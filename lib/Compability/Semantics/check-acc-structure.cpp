...
#define CHECK_SIMPLE_CLAUSE(X, Y) \
  void AccStructureChecker::Enter(const parser::AccClause::X &) { \
    CheckAllowed(toolchain::acc::Clause::Y); \
  }

#define CHECK_REQ_SCALAR_INT_CONSTANT_CLAUSE(X, Y) \
  void AccStructureChecker::Enter(const parser::AccClause::X &c) { \
    CheckAllowed(toolchain::acc::Clause::Y); \
    RequiresConstantPositiveParameter(toolchain::acc::Clause::Y, c.v); \
  }

using ReductionOpsSet =
    language::Compability::common::EnumSet<language::Compability::parser::ReductionOperator::Operator,
        language::Compability::parser::ReductionOperator::Operator_enumSize>;

static ReductionOpsSet reductionIntegerSet{
    language::Compability::parser::ReductionOperator::Operator::Plus,
    language::Compability::parser::ReductionOperator::Operator::Multiply,
    language::Compability::parser::ReductionOperator::Operator::Max,
    language::Compability::parser::ReductionOperator::Operator::Min,
    language::Compability::parser::ReductionOperator::Operator::Iand,
    language::Compability::parser::ReductionOperator::Operator::Ior,
    language::Compability::parser::ReductionOperator::Operator::Ieor};
...
static ReductionOpsSet reductionLogicalSet{
    language::Compability::parser::ReductionOperator::Operator::And,
    language::Compability::parser::ReductionOperator::Operator::Or,
    language::Compability::parser::ReductionOperator::Operator::Eqv,
    language::Compability::parser::ReductionOperator::Operator::Neqv};

namespace language::Compability::semantics {

static constexpr inline AccClauseSet
    computeConstructOnlyAllowedAfterDeviceTypeClauses{
        toolchain::acc::Clause::ACCC_async, toolchain::acc::Clause::ACCC_wait,
        toolchain::acc::Clause::ACCC_num_gangs, toolchain::acc::Clause::ACCC_num_workers,
        toolchain::acc::Clause::ACCC_vector_length};

static constexpr inline AccClauseSet loopOnlyAllowedAfterDeviceTypeClauses{
    toolchain::acc::Clause::ACCC_auto, toolchain::acc::Clause::ACCC_collapse,
    toolchain::acc::Clause::ACCC_independent, toolchain::acc::Clause::ACCC_gang,
    toolchain::acc::Clause::ACCC_seq, toolchain::acc::Clause::ACCC_tile,
    toolchain::acc::Clause::ACCC_vector, toolchain::acc::Clause::ACCC_worker};
...
static constexpr inline AccClauseSet routineOnlyAllowedAfterDeviceTypeClauses{
    toolchain::acc::Clause::ACCC_bind, toolchain::acc::Clause::ACCC_gang,
    toolchain::acc::Clause::ACCC_vector, toolchain::acc::Clause::ACCC_worker,
    toolchain::acc::Clause::ACCC_seq};

static constexpr inline AccClauseSet routineMutuallyExclusiveClauses{
    toolchain::acc::Clause::ACCC_gang, toolchain::acc::Clause::ACCC_worker,
    toolchain::acc::Clause::ACCC_vector, toolchain::acc::Clause::ACCC_seq};

bool AccStructureChecker::CheckAllowedModifier(toolchain::acc::Clause clause) {
  if (GetContext().directive == toolchain::acc::ACCD_enter_data ||
      GetContext().directive == toolchain::acc::ACCD_exit_data) {
...
bool AccStructureChecker::IsComputeConstruct(
    toolchain::acc::Directive directive) const {
  return directive == toolchain::acc::ACCD_parallel ||
      directive == toolchain::acc::ACCD_parallel_loop ||
      directive == toolchain::acc::ACCD_serial ||
      directive == toolchain::acc::ACCD_serial_loop ||
      directive == toolchain::acc::ACCD_kernels ||
      directive == toolchain::acc::ACCD_kernels_loop;
}

bool AccStructureChecker::IsLoopConstruct(
    toolchain::acc::Directive directive) const {
  return directive == toolchain::acc::Directive::ACCD_loop ||
      directive == toolchain::acc::ACCD_parallel_loop ||
      directive == toolchain::acc::ACCD_serial_loop ||
      directive == toolchain::acc::ACCD_kernels_loop;
}

std::optional<toolchain::acc::Directive>
...
bool AccStructureChecker::IsInsideParallelConstruct() const {
  if (auto directive = getParentComputeConstruct())
    if (*directive == toolchain::acc::ACCD_parallel ||
        *directive == toolchain::acc::ACCD_parallel_loop)
      return true;
  return false;
}
...
void AccStructureChecker::Leave(const parser::OpenACCBlockConstruct &x) {
  ...
  case toolchain::acc::Directive::ACCD_serial:
    ...
    CheckOnlyAllowedAfter(toolchain::acc::Clause::ACCC_device_type,
        computeConstructOnlyAllowedAfterDeviceTypeClauses);
...
void AccStructureChecker::Leave(
  ...
  if (containingScope.kind() == Scope::Kind::Module) {
    for (auto cl : GetContext().actualClauses) {
      if (cl != toolchain::acc::Clause::ACCC_create &&
          cl != toolchain::acc::Clause::ACCC_copyin &&
          cl != toolchain::acc::Clause::ACCC_device_resident &&
          cl != toolchain::acc::Clause::ACCC_link) {
        context_.Say(GetContext().directiveSource,
            "%s clause is not allowed on the %s directive in module "
            "declaration "
            "section"_err_en_US,
            parser::ToUpperCaseLetters(
                toolchain::acc::getOpenACCClauseName(cl).str()),
            ContextDirectiveAsFortran());
      }
    }
  }
    ...
    if (const auto *gangClause{
      ...
      if (gangClause->v) {
        ...
        for (const language::Compability::parser::AccGangArg &gangArg : x.v)
          if (const auto *dim{
                  std::get_if<language::Compability::parser::AccGangArg::Dim>(&gangArg.u)})
  ...
  for (std::size_t i = dirContext_.size() - 1; i > 0; --i) {
    ...
    if (IsLoopConstruct(parent.directive)) {
      for (auto parentClause : parent.actualClauses) {
        for (auto cl : GetContext().actualClauses) {
          bool invalid{false};
          if (parentClause == toolchain::acc::Clause::ACCC_gang &&
              cl == toolchain::acc::Clause::ACCC_gang) {
            if (IsInsideParallelConstruct()) {
              ...
              if (parentDimNum <= currentDimNum) {
                ...
                context_.Say(GetContext().clauseSource,
                    ...
                    currentDimStr,
                    parser::ToUpperCaseLetters(
                        toolchain::acc::getOpenACCClauseName(parentClause).str()),
                    parentDimStr);
                continue;
              }
            } else {
          ...
          } else if (parentClause == toolchain::acc::Clause::ACCC_worker &&
              (cl == toolchain::acc::Clause::ACCC_gang ||
                  cl == toolchain::acc::Clause::ACCC_worker)) {
            invalid = true;
          } else if (parentClause == toolchain::acc::Clause::ACCC_vector &&
              (cl == toolchain::acc::Clause::ACCC_gang ||
                  cl == toolchain::acc::Clause::ACCC_worker ||
                  cl == toolchain::acc::Clause::ACCC_vector)) {
            invalid = true;
          }
          if (invalid)
            context_.Say(GetContext().clauseSource,
...
void AccStructureChecker::Leave(const parser::OpenACCLoopConstruct &x) {
  ...
  if (loopDir.v == toolchain::acc::Directive::ACCD_loop) {
    ...
    CheckNotAllowedIfClause(toolchain::acc::Clause::ACCC_seq,
        {toolchain::acc::Clause::ACCC_gang, toolchain::acc::Clause::ACCC_vector,
            toolchain::acc::Clause::ACCC_worker});
...
void AccStructureChecker::Enter(const parser::OpenACCRoutineConstruct &x) {
  PushContextAndClauseSets(x.source, toolchain::acc::Directive::ACCD_routine);
...
void AccStructureChecker::Enter(const parser::OpenACCWaitConstruct &x) {
  const auto &verbatim{std::get<parser::Verbatim>(x.t)};
  PushContextAndClauseSets(verbatim.source, toolchain::acc::Directive::ACCD_wait);
}
...
void AccStructureChecker::Enter(const parser::OpenACCAtomicConstruct &x) {
  PushContextAndClauseSets(x.source, toolchain::acc::Directive::ACCD_atomic);
}
...
void AccStructureChecker::Enter(const parser::AccAtomicCapture &capture) {
  const language::Compability::parser::AssignmentStmt &stmt1{
      std::get<language::Compability::parser::AccAtomicCapture::Stmt1>(capture.t)
          .v.statement};
  const language::Compability::parser::AssignmentStmt &stmt2{
      std::get<language::Compability::parser::AccAtomicCapture::Stmt2>(capture.t)
          .v.statement};
...
void AccStructureChecker::Enter(const parser::OpenACCCacheConstruct &x) {
  const auto &verbatim = std::get<parser::Verbatim>(x.t);
  PushContextAndClauseSets(verbatim.source, toolchain::acc::Directive::ACCD_cache);
...
void AccStructureChecker::CheckMultipleOccurrenceInDeclare(
    const parser::AccObjectList &list, toolchain::acc::Clause clause) {
  if (GetContext().directive != toolchain::acc::Directive::ACCD_declare)
    return;
                  ...
                  if (declareSymbols[&name->symbol->GetUltimate()] == clause) {
                    ...
                    context_.Say(GetContext().clauseSource,
                        ...
                        parser::ToUpperCaseLetters(
                            toolchain::acc::getOpenACCClauseName(clause).str()),
                        parser::ToUpperCaseLetters(
                            toolchain::acc::getOpenACCClauseName(
...
void AccStructureChecker::CheckMultipleOccurrenceInDeclare(
    const parser::AccObjectListWithModifier &list, toolchain::acc::Clause clause) {
  const auto &objectList = std::get<language::Compability::parser::AccObjectList>(list.t);
  CheckMultipleOccurrenceInDeclare(objectList, clause);
}

void AccStructureChecker::Enter(const parser::AccClause::Async &c) {
  toolchain::acc::Clause crtClause = toolchain::acc::Clause::ACCC_async;
  CheckAllowed(crtClause);
  CheckAllowedOncePerGroup(crtClause, toolchain::acc::Clause::ACCC_device_type);
}

void AccStructureChecker::Enter(const parser::AccClause::Create &c) {
  CheckAllowed(toolchain::acc::Clause::ACCC_create);
  ...
  if (const auto &modifier{
    ...
    if (modifier->v != parser::AccDataModifier::Modifier::Zero) {
      context_.Say(GetContext().clauseSource,
          ...
          "on the %s directive"_err_en_US,
          parser::ToUpperCaseLetters(
              toolchain::acc::getOpenACCClauseName(toolchain::acc::Clause::ACCC_create)
                  .str()),
          ContextDirectiveAsFortran());
    }
    if (GetContext().directive == toolchain::acc::Directive::ACCD_declare) {
  ...
  CheckMultipleOccurrenceInDeclare(
      modifierClause, toolchain::acc::Clause::ACCC_create);
}

void AccStructureChecker::Enter(const parser::AccClause::Copyin &c) {
  CheckAllowed(toolchain::acc::Clause::ACCC_copyin);
  const auto &modifierClause{c.v};
  if (const auto &modifier{
          std::get<std::optional<parser::AccDataModifier>>(modifierClause.t)}) {
    if (CheckAllowedModifier(toolchain::acc::Clause::ACCC_copyin)) {
      return;
    }
    if (modifier->v != parser::AccDataModifier::Modifier::ReadOnly) {
      context_.Say(GetContext().clauseSource,
          ...
          "on the %s directive"_err_en_US,
          parser::ToUpperCaseLetters(
              toolchain::acc::getOpenACCClauseName(toolchain::acc::Clause::ACCC_copyin)
                  .str()),
          ContextDirectiveAsFortran());
    }
  }
  CheckMultipleOccurrenceInDeclare(
      modifierClause, toolchain::acc::Clause::ACCC_copyin);
}

void AccStructureChecker::Enter(const parser::AccClause::Copyout &c) {
  CheckAllowed(toolchain::acc::Clause::ACCC_copyout);
  const auto &modifierClause{c.v};
  if (const auto &modifier{
          std::get<std::optional<parser::AccDataModifier>>(modifierClause.t)}) {
    if (CheckAllowedModifier(toolchain::acc::Clause::ACCC_copyout)) {
      return;
    }
    if (modifier->v != parser::AccDataModifier::Modifier::Zero) {
      context_.Say(GetContext().clauseSource,
          ...
          "on the %s directive"_err_en_US,
          parser::ToUpperCaseLetters(
              toolchain::acc::getOpenACCClauseName(toolchain::acc::Clause::ACCC_copyout)
                  .str()),
          ContextDirectiveAsFortran());
    }
    if (GetContext().directive == toolchain::acc::Directive::ACCD_declare) {
  ...
  CheckMultipleOccurrenceInDeclare(
      modifierClause, toolchain::acc::Clause::ACCC_copyout);
}

void AccStructureChecker::Enter(const parser::AccClause::DeviceType &d) {
  CheckAllowed(toolchain::acc::Clause::ACCC_device_type);
  if (GetContext().directive == toolchain::acc::Directive::ACCD_set &&
      d.v.v.size() > 1) {
    context_.Say(GetContext().clauseSource,
        "The %s clause on the %s directive accepts only one value"_err_en_US,
        parser::ToUpperCaseLetters(
            toolchain::acc::getOpenACCClauseName(toolchain::acc::Clause::ACCC_device_type)
                .str()),
        ContextDirectiveAsFortran());
  }
  ResetCrtGroup();
...
void AccStructureChecker::Enter(const parser::AccClause::Seq &g) {
  toolchain::acc::Clause crtClause = toolchain::acc::Clause::ACCC_seq;
  if (GetContext().directive == toolchain::acc::Directive::ACCD_routine) {
    CheckMutuallyExclusivePerGroup(crtClause,
        toolchain::acc::Clause::ACCC_device_type, routineMutuallyExclusiveClauses);
  }
  CheckAllowed(crtClause);
}

void AccStructureChecker::Enter(const parser::AccClause::Vector &g) {
  toolchain::acc::Clause crtClause = toolchain::acc::Clause::ACCC_vector;
  if (GetContext().directive == toolchain::acc::Directive::ACCD_routine) {
    CheckMutuallyExclusivePerGroup(crtClause,
        toolchain::acc::Clause::ACCC_device_type, routineMutuallyExclusiveClauses);
  }
...
void AccStructureChecker::Enter(const parser::AccClause::Worker &g) {
  toolchain::acc::Clause crtClause = toolchain::acc::Clause::ACCC_worker;
  if (GetContext().directive == toolchain::acc::Directive::ACCD_routine) {
    CheckMutuallyExclusivePerGroup(crtClause,
        toolchain::acc::Clause::ACCC_device_type, routineMutuallyExclusiveClauses);
  }
...
void AccStructureChecker::Enter(const parser::AccClause::Tile &g) {
  CheckAllowed(toolchain::acc::Clause::ACCC_tile);
  CheckAllowedOncePerGroup(
      toolchain::acc::Clause::ACCC_tile, toolchain::acc::Clause::ACCC_device_type);
}

void AccStructureChecker::Enter(const parser::AccClause::Gang &g) {
  toolchain::acc::Clause crtClause = toolchain::acc::Clause::ACCC_gang;
  if (GetContext().directive == toolchain::acc::Directive::ACCD_routine) {
    CheckMutuallyExclusivePerGroup(crtClause,
        toolchain::acc::Clause::ACCC_device_type, routineMutuallyExclusiveClauses);
  }
  ...
  if (g.v) {
    ...
    const language::Compability::parser::AccGangArgList &x = *g.v;
    for (const language::Compability::parser::AccGangArg &gangArg : x.v) {
      if (std::get_if<language::Compability::parser::AccGangArg::Num>(&gangArg.u)) {
        hasNum = true;
      } else if (std::get_if<language::Compability::parser::AccGangArg::Dim>(&gangArg.u)) {
        hasDim = true;
      } else if (std::get_if<language::Compability::parser::AccGangArg::Static>(&gangArg.u)) {
        hasStatic = true;
      }
    }
    ...
    if (GetContext().directive == toolchain::acc::Directive::ACCD_routine &&
      ...
      context_.Say(GetContext().clauseSource,
          ...
          parser::ToUpperCaseLetters(
              toolchain::acc::getOpenACCClauseName(toolchain::acc::Clause::ACCC_gang)
                  .str()),
          ContextDirectiveAsFortran());
    }
...
void AccStructureChecker::Enter(const parser::AccClause::NumGangs &n) {
  CheckAllowed(toolchain::acc::Clause::ACCC_num_gangs,
      /*warnInsteadOfError=*/GetContext().directive ==
              toolchain::acc::Directive::ACCD_serial ||
          GetContext().directive == toolchain::acc::Directive::ACCD_serial_loop);
  CheckAllowedOncePerGroup(
      toolchain::acc::Clause::ACCC_num_gangs, toolchain::acc::Clause::ACCC_device_type);
...
void AccStructureChecker::Enter(const parser::AccClause::NumWorkers &n) {
  CheckAllowed(toolchain::acc::Clause::ACCC_num_workers,
      /*warnInsteadOfError=*/GetContext().directive ==
              toolchain::acc::Directive::ACCD_serial ||
          GetContext().directive == toolchain::acc::Directive::ACCD_serial_loop);
  CheckAllowedOncePerGroup(
      toolchain::acc::Clause::ACCC_num_workers, toolchain::acc::Clause::ACCC_device_type);
}

void AccStructureChecker::Enter(const parser::AccClause::VectorLength &n) {
  CheckAllowed(toolchain::acc::Clause::ACCC_vector_length,
      /*warnInsteadOfError=*/GetContext().directive ==
              toolchain::acc::Directive::ACCD_serial ||
          GetContext().directive == toolchain::acc::Directive::ACCD_serial_loop);
  CheckAllowedOncePerGroup(toolchain::acc::Clause::ACCC_vector_length,
      toolchain::acc::Clause::ACCC_device_type);
}

void AccStructureChecker::Enter(const parser::AccClause::Reduction &reduction) {
  CheckAllowed(toolchain::acc::Clause::ACCC_reduction);
...
void AccStructureChecker::Enter(const parser::AccClause::Self &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_self);
  const std::optional<parser::AccSelfClause> &accSelfClause = x.v;
  if (GetContext().directive == toolchain::acc::Directive::ACCD_update &&
      ((accSelfClause &&
    ...
    context_.Say(GetContext().clauseSource,
        ...
        ContextDirectiveAsFortran());
  } else if (GetContext().directive != toolchain::acc::Directive::ACCD_update &&
      accSelfClause &&
...
void AccStructureChecker::Enter(const parser::AccClause::Collapse &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_collapse);
  CheckAllowedOncePerGroup(
      toolchain::acc::Clause::ACCC_collapse, toolchain::acc::Clause::ACCC_device_type);
  ...
  RequiresConstantPositiveParameter(
      toolchain::acc::Clause::ACCC_collapse, collapseValue);
}

void AccStructureChecker::Enter(const parser::AccClause::Present &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_present);
  CheckMultipleOccurrenceInDeclare(x.v, toolchain::acc::Clause::ACCC_present);
}

void AccStructureChecker::Enter(const parser::AccClause::Copy &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_copy);
  CheckMultipleOccurrenceInDeclare(x.v, toolchain::acc::Clause::ACCC_copy);
}

void AccStructureChecker::Enter(const parser::AccClause::Deviceptr &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_deviceptr);
  CheckMultipleOccurrenceInDeclare(x.v, toolchain::acc::Clause::ACCC_deviceptr);
}

void AccStructureChecker::Enter(const parser::AccClause::DeviceResident &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_device_resident);
  CheckMultipleOccurrenceInDeclare(
      x.v, toolchain::acc::Clause::ACCC_device_resident);
}

void AccStructureChecker::Enter(const parser::AccClause::Link &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_link);
  CheckMultipleOccurrenceInDeclare(x.v, toolchain::acc::Clause::ACCC_link);
}

void AccStructureChecker::Enter(const parser::AccClause::Shortloop &x) {
  if (CheckAllowed(toolchain::acc::Clause::ACCC_shortloop)) {
...
void AccStructureChecker::Enter(const parser::AccClause::If &x) {
  CheckAllowed(toolchain::acc::Clause::ACCC_if);
...
toolchain::StringRef AccStructureChecker::getDirectiveName(
    toolchain::acc::Directive directive) {
  return toolchain::acc::getOpenACCDirectiveName(directive);
}

toolchain::StringRef AccStructureChecker::getClauseName(toolchain::acc::Clause clause) {
  return toolchain::acc::getOpenACCClauseName(clause);
}
