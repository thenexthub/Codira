...
static bool isReal8OrLarger(const language::Compability::semantics::DeclTypeSpec *type) {
...
void ComputeOffsetsHelper::Compute(Scope &scope) {
  ...
  for (auto &symbol : scope.GetSymbols()) {
    if (!FindCommonBlockContaining(*symbol) &&
      ...
      auto triple{toolchain::Triple(
          toolchain::Triple::normalize(toolchain::sys::getDefaultTargetTriple()))};
      if (triple.getOS() == toolchain::Triple::OSType::AIX) {
