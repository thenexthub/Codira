...
#include "toolchain/Frontend/OpenMP/OMP.h.inc"
...
template <typename T>
static language::Compability::semantics::Scope *GetScope(
    language::Compability::semantics::SemanticsContext &context, const T &x) {
  std::optional<language::Compability::parser::CharBlock> source{GetLastSource(x)};
...
class AccAttributeVisitor : DirectiveAttributeVisitor<toolchain::acc::Directive> {
public:
...
private:
  ...
  void EnsureAllocatableOrPointer(
      const toolchain::acc::Clause clause, const parser::AccObjectList &objectList);
...
class OmpAttributeVisitor : DirectiveAttributeVisitor<toolchain::omp::Directive> {
public:
  ...
  bool Pre(const parser::OmpMetadirectiveDirective &x) {
    PushContext(x.source, toolchain::omp::Directive::OMPD_metadirective);
    return true;
  }
  ...
  bool Pre(const parser::OpenMPStandaloneConstruct &x) {
    common::visit(
        [&](auto &&s) {
          using TypeS = toolchain::remove_cvref_t<decltype(s)>;
  ...
  bool Pre(const parser::OpenMPDeclareSimdConstruct &x) {
    PushContext(x.source, toolchain::omp::Directive::OMPD_declare_simd);
  ...
  bool Pre(const parser::OpenMPDepobjConstruct &x) {
    PushContext(x.source, toolchain::omp::Directive::OMPD_depobj);
  ...
  bool Pre(const parser::OpenMPFlushConstruct &x) {
    PushContext(x.source, toolchain::omp::Directive::OMPD_flush);
    for (auto &arg : x.v.Arguments().v) {
      if (auto *locator{std::get_if<parser::OmpLocator>(&arg.u)}) {
        if (auto *object{std::get_if<parser::OmpObject>(&locator->u)}) {
  ...
  bool Pre(const parser::OpenMPRequiresConstruct &x) {
    ...
    PushContext(x.source, toolchain::omp::Directive::OMPD_requires);
  ...
  bool Pre(const parser::OmpDeclareVariantDirective &x) {
    PushContext(x.source, toolchain::omp::Directive::OMPD_declare_variant);
    return true;
  }
...
private:
  ...
  bool IsNestedInDirective(toolchain::omp::Directive directive);
  ...
  void IssueNonConformanceWarning(toolchain::omp::Directive D,
...
bool AccAttributeVisitor::Pre(const parser::OpenACCBlockConstruct &x) {
  ...
  switch (blockDir.v) {
  case toolchain::acc::Directive::ACCD_data:
  case toolchain::acc::Directive::ACCD_host_data:
  case toolchain::acc::Directive::ACCD_kernels:
  case toolchain::acc::Directive::ACCD_parallel:
  case toolchain::acc::Directive::ACCD_serial:
...
bool AccAttributeVisitor::Pre(const parser::OpenACCDeclarativeConstruct &x) {
  if (const auto *declConstruct{
    ...
    PushContext(declDir.source, toolchain::acc::Directive::ACCD_declare);
  }
  ClearDataSharingAttributeObjects();
  return true;
}
...
static const parser::AccObjectList &GetAccObjectList(
  ...
  if (const auto *copyClause =
    ...
    return accObjectList;
                 ...
                 std::get_if<language::Compability::parser::AccClause::Copyout>(&clause.u)) {
    const language::Compability::parser::AccObjectListWithModifier &listWithModifier =
        copyoutClause->v;
    const language::Compability::parser::AccObjectList &accObjectList =
        std::get<language::Compability::parser::AccObjectList>(listWithModifier.t);
    return accObjectList;
                 ...
                 std::get_if<language::Compability::parser::AccClause::Present>(&clause.u)) {
    ...
    toolchain_unreachable("Clause without object list!");
  }
}
...
bool AccAttributeVisitor::Pre(const parser::OpenACCLoopConstruct &x) {
  ...
  if (loopDir.v == toolchain::acc::Directive::ACCD_loop) {
...
bool AccAttributeVisitor::Pre(const parser::OpenACCStandaloneConstruct &x) {
  ...
  case toolchain::acc::Directive::ACCD_enter_data:
  case toolchain::acc::Directive::ACCD_exit_data:
  case toolchain::acc::Directive::ACCD_init:
  case toolchain::acc::Directive::ACCD_set:
  case toolchain::acc::Directive::ACCD_shutdown:
  case toolchain::acc::Directive::ACCD_update:
...
MaybeExpr EvaluateExpr(
    language::Compability::semantics::SemanticsContext &semanticsContext, const T &expr) {
  return FoldExpr(
...
void AccAttributeVisitor::AddRoutineInfoToSymbol(
  ...
  if (symbol.has<SubprogramDetails>()) {
    language::Compability::semantics::OpenACCRoutineInfo info;
    std::vector<OpenACCRoutineDeviceTypeInfo *> currentDevices;
    currentDevices.push_back(&info);
    const auto &clauses{std::get<language::Compability::parser::AccClauseList>(x.t)};
    for (const language::Compability::parser::AccClause &clause : clauses.v) {
      if (const auto *dTypeClause{
              std::get_if<language::Compability::parser::AccClause::DeviceType>(&clause.u)}) {
        currentDevices.clear();
        for (const auto &deviceTypeExpr : dTypeClause->v.v) {
          currentDevices.push_back(&info.add_deviceTypeInfo(deviceTypeExpr.v));
        }
      } else if (std::get_if<language::Compability::parser::AccClause::Nohost>(&clause.u)) {
        info.set_isNohost();
      } else if (std::get_if<language::Compability::parser::AccClause::Seq>(&clause.u)) {
        for (auto &device : currentDevices) {
          device->set_isSeq();
        }
      } else if (std::get_if<language::Compability::parser::AccClause::Vector>(&clause.u)) {
        for (auto &device : currentDevices) {
          device->set_isVector();
        }
      } else if (std::get_if<language::Compability::parser::AccClause::Worker>(&clause.u)) {
        for (auto &device : currentDevices) {
          device->set_isWorker();
        }
      } else if (const auto *gangClause{
                     std::get_if<language::Compability::parser::AccClause::Gang>(
                         &clause.u)}) {
        for (auto &device : currentDevices) {
          device->set_isGang();
        }
        if (gangClause->v) {
          ...
          for (const language::Compability::parser::AccGangArg &gangArg : x.v) {
            ...
            if (const auto *dim{std::get_if<language::Compability::parser::AccGangArg::Dim>(
                    &gangArg.u)}) {
            ...
            numArgs++;
                     ...
                     std::get_if<language::Compability::parser::AccClause::Bind>(
                         &clause.u)}) {
        if (const auto *name{
                std::get_if<language::Compability::parser::Name>(&bindClause->v.u)}) {
          if (Symbol * sym{ResolveFctName(*name)}) {
            ...
            context_.Say((*name).source,
                ...
                (*name).source);
                       ...
                       std::get_if<language::Compability::parser::ScalarDefaultCharExpr>(
                           &bindClause->v.u)}) {
          auto *charConst{
              language::Compability::parser::Unwrap<language::Compability::parser::CharLiteralConstant>(
                  *charExpr)};
...
bool AccAttributeVisitor::Pre(const parser::OpenACCRoutineConstruct &x) {
  ...
  if (topScope_) {
    PushContext(
        verbatim.source, toolchain::acc::Directive::ACCD_routine, *topScope_);
  } else {
    PushContext(verbatim.source, toolchain::acc::Directive::ACCD_routine);
  }
...
bool AccAttributeVisitor::Pre(const parser::OpenACCCombinedConstruct &x) {
  ...
  case toolchain::acc::Directive::ACCD_kernels_loop:
  case toolchain::acc::Directive::ACCD_parallel_loop:
  case toolchain::acc::Directive::ACCD_serial_loop:
        ...
        common::visitors{
              ...
              if (!IsLastNameArray(designator)) {
                context_.Say(designator.source,
                    ...
                    parser::ToUpperCaseLetters(
                        toolchain::acc::getOpenACCDirectiveName(
            ...
            [&](const auto &name) {
              context_.Say(name.source,
                  "Only array element or subarray are allowed in %s directive"_err_en_US,
                  parser::ToUpperCaseLetters(
                      toolchain::acc::getOpenACCDirectiveName(GetContext().directive)
                          .str()));
            },
        },
        accObject.u);
...
bool AccAttributeVisitor::Pre(const parser::OpenACCCacheConstruct &x) {
  const auto &verbatim{std::get<parser::Verbatim>(x.t)};
  PushContext(verbatim.source, toolchain::acc::Directive::ACCD_cache);
  ClearDataSharingAttributeObjects();

  const auto &objectListWithModifier =
      std::get<parser::AccObjectListWithModifier>(x.t);
  const auto &objectList =
      std::get<language::Compability::parser::AccObjectList>(objectListWithModifier.t);
...
void AccAttributeVisitor::CheckAssociatedLoop(
  ...
  auto checkExprHasSymbols = [&](toolchain::SmallVector<Symbol *> &ivs,
  ...
  toolchain::SmallVector<Symbol *> ivs;
  using Bounds = parser::LoopControl::Bounds;
            ...
            [&](const parser::Designator &designator) {
              ...
              if (!IsAllocatableOrObjectPointer(lastName.symbol)) {
                context_.Say(designator.source,
                    ...
                    parser::ToUpperCaseLetters(
                        toolchain::acc::getOpenACCClauseName(clause).str()));
              }
            },
            [&](const auto &name) {
              context_.Say(name.source,
...
bool AccAttributeVisitor::Pre(const parser::AccClause::Attach &x) {
  ...
  EnsureAllocatableOrPointer(toolchain::acc::Clause::ACCC_attach, x.v);
  return true;
}

bool AccAttributeVisitor::Pre(const parser::AccClause::Detach &x) {
  // Restriction - line 1715-1717
  EnsureAllocatableOrPointer(toolchain::acc::Clause::ACCC_detach, x.v);
  return true;
}
...
void AccAttributeVisitor::Post(const parser::AccDefaultClause &x) {
  if (!dirContext_.empty()) {
    switch (x.v) {
    case toolchain::acc::DefaultValue::ACC_Default_present:
      SetContextDefaultDSA(Symbol::Flag::AccPresent);
      break;
    case toolchain::acc::DefaultValue::ACC_Default_none:
...
Symbol *AccAttributeVisitor::DeclareOrMarkOtherAccessEntity(
  ...
  if (accFlagsRequireMark.test(accFlag)) {
    if (GetContext().directive == toolchain::acc::ACCD_declare) {
...
static toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, const Symbol::Flags &flags);

namespace dbg {
static void DumpAssocSymbols(toolchain::raw_ostream &os, const Symbol &sym);
static std::string ScopeSourcePos(const language::Compability::semantics::Scope &scope);
} // namespace dbg
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPBlockConstruct &x) {
  ...
  toolchain::omp::Directive dirId{dirSpec.DirId()};
  switch (dirId) {
  case toolchain::omp::Directive::OMPD_masked:
  case toolchain::omp::Directive::OMPD_parallel_masked:
  case toolchain::omp::Directive::OMPD_master:
  case toolchain::omp::Directive::OMPD_parallel_master:
  ...
  case toolchain::omp::Directive::OMPD_parallel_workshare:
  case toolchain::omp::Directive::OMPD_target_teams:
  case toolchain::omp::Directive::OMPD_target_parallel:
  ...
  if (dirId == toolchain::omp::Directive::OMPD_master ||
      dirId == toolchain::omp::Directive::OMPD_parallel_master)
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPLoopConstruct &x) {
  ...
  case toolchain::omp::Directive::OMPD_distribute_parallel_do:
  case toolchain::omp::Directive::OMPD_distribute_parallel_do_simd:
  case toolchain::omp::Directive::OMPD_distribute_simd:
  case toolchain::omp::Directive::OMPD_do:
  case toolchain::omp::Directive::OMPD_do_simd:
  case toolchain::omp::Directive::OMPD_loop:
  case toolchain::omp::Directive::OMPD_masked_taskloop_simd:
  case toolchain::omp::Directive::OMPD_masked_taskloop:
  case toolchain::omp::Directive::OMPD_master_taskloop_simd:
  case toolchain::omp::Directive::OMPD_master_taskloop:
  case toolchain::omp::Directive::OMPD_parallel_do:
  case toolchain::omp::Directive::OMPD_parallel_do_simd:
  case toolchain::omp::Directive::OMPD_parallel_masked_taskloop_simd:
  case toolchain::omp::Directive::OMPD_parallel_masked_taskloop:
  case toolchain::omp::Directive::OMPD_parallel_master_taskloop_simd:
  case toolchain::omp::Directive::OMPD_parallel_master_taskloop:
  case toolchain::omp::Directive::OMPD_simd:
  case toolchain::omp::Directive::OMPD_target_loop:
  case toolchain::omp::Directive::OMPD_target_parallel_do:
  case toolchain::omp::Directive::OMPD_target_parallel_do_simd:
  case toolchain::omp::Directive::OMPD_target_parallel_loop:
  case toolchain::omp::Directive::OMPD_target_teams_distribute:
  case toolchain::omp::Directive::OMPD_target_teams_distribute_parallel_do:
  case toolchain::omp::Directive::OMPD_target_teams_distribute_parallel_do_simd:
  case toolchain::omp::Directive::OMPD_target_teams_distribute_simd:
  case toolchain::omp::Directive::OMPD_target_teams_loop:
  case toolchain::omp::Directive::OMPD_target_simd:
  case toolchain::omp::Directive::OMPD_taskloop:
  case toolchain::omp::Directive::OMPD_taskloop_simd:
  case toolchain::omp::Directive::OMPD_teams_distribute:
  case toolchain::omp::Directive::OMPD_teams_distribute_parallel_do:
  case toolchain::omp::Directive::OMPD_teams_distribute_parallel_do_simd:
  case toolchain::omp::Directive::OMPD_teams_distribute_simd:
  case toolchain::omp::Directive::OMPD_teams_loop:
  ...
  if (beginDir.v == toolchain::omp::OMPD_master_taskloop ||
      beginDir.v == toolchain::omp::OMPD_master_taskloop_simd ||
      beginDir.v == toolchain::omp::OMPD_parallel_master_taskloop ||
      beginDir.v == toolchain::omp::OMPD_parallel_master_taskloop_simd ||
      beginDir.v == toolchain::omp::Directive::OMPD_target_loop)
    IssueNonConformanceWarning(beginDir.v, beginDir.source, 52);
  ClearDataSharingAttributeObjects();
  SetContextAssociatedLoopLevel(GetAssociatedLoopLevelFromClauses(clauseList));

  if (beginDir.v == toolchain::omp::Directive::OMPD_do) {
...
void OmpAttributeVisitor::ResolveSeqLoopIndexInParallelOrTaskConstruct(
  ...
  for (;; ++targetIt) {
    ...
    if (toolchain::omp::allParallelSet.test(targetIt->directive) ||
        toolchain::omp::taskGeneratingSet.test(targetIt->directive)) {
      break;
    }
  }
...
bool OmpAttributeVisitor::Pre(const parser::DoConstruct &x) {
  if (!dirContext_.empty() && GetContext().withinConstruct) {
    toolchain::SmallVector<const parser::Name *> ivs;
    if (x.IsDoNormal()) {
...
void OmpAttributeVisitor::PrivatizeAssociatedLoopIndexAndCheckLoopLevel(
  ...
  bool isLoopConstruct{
      GetContext().directive == toolchain::omp::Directive::OMPD_loop};
  const parser::OmpClause *clause{GetAssociatedClause()};
  bool hasCollapseClause{
      clause ? (clause->Id() == toolchain::omp::OMPC_collapse) : false};
  ...
  if (optLoopCons.has_value()) {
    if (const auto &outer{std::get_if<parser::DoConstruct>(&*optLoopCons)}) {
      ...
      if (beginLoopDirective.v != toolchain::omp::Directive::OMPD_unroll &&
          beginLoopDirective.v != toolchain::omp::Directive::OMPD_tile) {
        context_.Say(GetContext().directiveSource,
            "Only UNROLL or TILE constructs are allowed between an OpenMP Loop Construct and a DO construct"_err_en_US,
            parser::ToUpperCaseLetters(toolchain::omp::getOpenMPDirectiveName(
      ...
      context_.Say(GetContext().directiveSource,
          ...
          parser::ToUpperCaseLetters(
              toolchain::omp::getOpenMPDirectiveName(GetContext().directive, version)
                  .str()));
    }
  }
}
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPSectionsConstruct &x) {
  ...
  switch (beginDir.v) {
  case toolchain::omp::Directive::OMPD_parallel_sections:
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPSectionConstruct &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_section);
  GetContext().withinConstruct = true;
  return true;
}
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPDeclareTargetConstruct &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_declare_target);
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPDeclareMapperConstruct &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_declare_mapper);
  return true;
}

bool OmpAttributeVisitor::Pre(
    const parser::OpenMPDeclareReductionConstruct &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_declare_reduction);
  return true;
}

bool OmpAttributeVisitor::Pre(const parser::OpenMPThreadprivate &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_threadprivate);
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPDeclarativeAllocate &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_allocate);
  const auto &list{std::get<parser::OmpObjectList>(x.t)};
...
bool OmpAttributeVisitor::Pre(const parser::OpenMPAtomicConstruct &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_atomic);
  return true;
}

bool OmpAttributeVisitor::Pre(const parser::OpenMPDispatchConstruct &x) {
  PushContext(x.source, toolchain::omp::Directive::OMPD_dispatch);
  return true;
}

bool OmpAttributeVisitor::Pre(const parser::OpenMPExecutableAllocate &x) {
  IssueNonConformanceWarning(toolchain::omp::Directive::OMPD_allocate, x.source, 52);
...
bool OmpAttributeVisitor::IsNestedInDirective(toolchain::omp::Directive directive) {
...
void OmpAttributeVisitor::Post(const parser::OpenMPExecutableAllocate &x) {
  ...
  if (IsNestedInDirective(toolchain::omp::Directive::OMPD_target) && !hasAllocator) {
  ...
  if (allocate) {
    for (const auto &clause : dirSpec.Clauses().v) {
      if (auto *alloc{std::get_if<parser::OmpClause::Allocate>(&clause.u)}) {
        ...
        if (IsNestedInDirective(toolchain::omp::Directive::OMPD_target) &&
            !hasAllocator) {
          context_.Say(x.source,
...
static bool IsSymbolStaticStorageDuration(const Symbol &symbol) {
  LLVM_DEBUG(toolchain::dbgs() << "IsSymbolStaticStorageDuration(" << symbol.name()
                          << "):\n");
...
static bool IsTargetCaptureImplicitlyFirstprivatizeable(const Symbol &symbol,
  ...
  auto checkSymbol = [&](const Symbol &checkSym) {
    ...
    if ((!checkSym.GetShape() || checkSym.GetShape()->empty()) &&
        (type->category() ==
                language::Compability::semantics::DeclTypeSpec::Category::Numeric ||
            type->category() ==
                language::Compability::semantics::DeclTypeSpec::Category::Logical ||
            type->category() ==
...
void OmpAttributeVisitor::CreateImplicitSymbols(const Symbol *symbol) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "CreateImplicitSymbols: " << *symbol << '\n');
  ...
  for (int dirDepth{0}; dirDepth < (int)dirContext_.size(); ++dirDepth) {
    ...
    auto printImplicitRule = [&](const char *id) {
      LLVM_DEBUG(toolchain::dbgs() << "\t" << id << ": dsa: " << dsa << '\n');
      LLVM_DEBUG(
          toolchain::dbgs() << "\t\tScope: " << dbg::ScopeSourcePos(scope) << '\n');
    };
...
#define PRINT_IMPLICIT_RULE(id)
    ...
    bool taskGenDir = toolchain::omp::taskGeneratingSet.test(dirContext.directive);
    bool targetDir = toolchain::omp::allTargetSet.test(dirContext.directive);
    bool parallelDir = toolchain::omp::topParallelSet.test(dirContext.directive);
    bool teamsDir = toolchain::omp::allTeamsSet.test(dirContext.directive);
...
static bool SymbolOrEquivalentIsInNamelist(const Symbol &symbol) {
  ...
  return toolchain::any_of(*eqv, [isInNamelist](const EquivalenceObject &obj) {
...
void OmpAttributeVisitor::ResolveOmpDesignator(
    const parser::Designator &designator, Symbol::Flag ompFlag) {
  unsigned version{context_.langOptions().OpenMPVersion};
  toolchain::omp::Directive directive{GetContext().directive};
  ...
  if (auto *symbol{ResolveOmp(*name, ompFlag, currScope())}) {
        ...
        [&](const Symbol *symbol1, Symbol::Flag firstOmpFlag,
          ...
          if ((symbol1->test(firstOmpFlag) && symbol2->test(secondOmpFlag)) ||
            ...
            context_.Say(designator.source,
                ...
                parser::ToUpperCaseLetters(
                    toolchain::omp::getOpenMPDirectiveName(directive, version)));
          }
        }};
    ...
    if (ompFlag == Symbol::Flag::OmpDeclarativeAllocateDirective &&
        ...
        !IsNestedInDirective(toolchain::omp::Directive::OMPD_allocate)) {
      context_.Say(designator.source,
    ...
    if (toolchain::omp::allTargetSet.test(directive)) {
      ...
      if (!toolchain::omp::allTeamsSet.test(directive) &&
          !toolchain::omp::allParallelSet.test(directive)) {
...
void OmpAttributeVisitor::ResolveOmpCommonBlock(
  ...
  if (auto *symbol{ResolveOmpCommonBlockName(&name)}) {
    ...
    for (auto [index, object] : toolchain::enumerate(details.objects())) {
    ...
    if (const common::OmpMemoryOrderType *
      ...
      if (combinedMemOrder && *combinedMemOrder != *memOrder) {
        context.Say(symbol.scope()->sourceRange(),
            ...
            "unit"_err_en_US,
            parser::ToUpperCaseLetters(toolchain::omp::getOpenMPClauseName(
                toolchain::omp::Clause::OMPC_atomic_default_mem_order)
                                           .str()));
      }
...
static bool IsSymbolPrivate(const Symbol &symbol) {
  LLVM_DEBUG(toolchain::dbgs() << "IsSymbolPrivate(" << symbol.name() << "):\n");
  LLVM_DEBUG(dbg::DumpAssocSymbols(toolchain::dbgs(), symbol));
...
void OmpAttributeVisitor::CheckDataCopyingClause(
  ...
  if (ompFlag == Symbol::Flag::OmpCopyIn) {
    ...
    if (!IsSymbolThreadprivate(symbol)) {
      ...
      GetContext().directive == toolchain::omp::Directive::OMPD_single) {
...
void OmpAttributeVisitor::CheckObjectIsPrivatizable(
  ...
  toolchain::StringRef clauseName{"PRIVATE"};
...
void OmpAttributeVisitor::IssueNonConformanceWarning(toolchain::omp::Directive D,
    parser::CharBlock source, unsigned EmitFromVersion) {
  std::string warnStr;
  toolchain::raw_string_ostream warnStrOS(warnStr);
  unsigned version{context_.langOptions().OpenMPVersion};
  ...
  warnStrOS << "OpenMP directive "
            << parser::ToUpperCaseLetters(
                   toolchain::omp::getOpenMPDirectiveName(D, version).str())
            << " has been deprecated";

  auto setAlternativeStr = [&warnStrOS](toolchain::StringRef alt) {
    warnStrOS << ", please use " << alt << " instead.";
  };
  switch (D) {
  case toolchain::omp::OMPD_master:
    setAlternativeStr("MASKED");
    break;
  case toolchain::omp::OMPD_master_taskloop:
    setAlternativeStr("MASKED TASKLOOP");
    break;
  case toolchain::omp::OMPD_master_taskloop_simd:
    setAlternativeStr("MASKED TASKLOOP SIMD");
    break;
  case toolchain::omp::OMPD_parallel_master:
    setAlternativeStr("PARALLEL MASKED");
    break;
  case toolchain::omp::OMPD_parallel_master_taskloop:
    setAlternativeStr("PARALLEL MASKED TASKLOOP");
    break;
  case toolchain::omp::OMPD_parallel_master_taskloop_simd:
    setAlternativeStr("PARALLEL_MASKED TASKLOOP SIMD");
    break;
  case toolchain::omp::OMPD_allocate:
    setAlternativeStr("ALLOCATORS");
    break;
  case toolchain::omp::OMPD_target_loop:
  default:;
  }
...
static toolchain::raw_ostream &operator<<(
    toolchain::raw_ostream &os, std::optional<parser::SourcePosition> srcPos) {
  if (srcPos) {
...
static std::optional<parser::SourcePosition> GetSourcePosition(
    ...
    const language::Compability::parser::CharBlock &src) {
