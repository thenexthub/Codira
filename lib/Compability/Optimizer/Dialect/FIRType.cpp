...
#include "language/Compability/Tools/PointerModels.h"
...
#include "language/Compability/Optimizer/Dialect/FIROpsTypes.cpp.inc"
...
bool verifySameLists(toolchain::ArrayRef<RecordType::TypePair> a1,
                     toolchain::ArrayRef<RecordType::TypePair> a2) {
  // FIXME: do we need to allow for any variance here?
  return a1 == a2;
}

static toolchain::StringRef getVolatileKeyword() { return "volatile"; }
...
RecordType verifyDerived(mlir::AsmParser &parser, RecordType derivedTy,
                         toolchain::ArrayRef<RecordType::TypePair> lenPList,
                         toolchain::ArrayRef<RecordType::TypePair> typeList) {
...
struct RecordTypeStorage : public mlir::TypeStorage {
  using KeyTy = toolchain::StringRef;

  static unsigned hashKey(const KeyTy &key) {
    return toolchain::hash_combine(key.str());
  }
  ...
  toolchain::StringRef getName() const { return name; }

  void setLenParamList(toolchain::ArrayRef<RecordType::TypePair> list) {
    lens = list;
  }
  toolchain::ArrayRef<RecordType::TypePair> getLenParamList() const { return lens; }

  void setTypeList(toolchain::ArrayRef<RecordType::TypePair> list) { types = list; }
  toolchain::ArrayRef<RecordType::TypePair> getTypeList() const { return types; }

  bool isFinalized() const { return finalized; }
  void finalize(toolchain::ArrayRef<RecordType::TypePair> lenParamList,
                toolchain::ArrayRef<RecordType::TypePair> typeList) {
    if (finalized)
      return;
    finalized = true;
...
private:
  ...
  explicit RecordTypeStorage(toolchain::StringRef name)
...
bool isa_fir_type(mlir::Type t) {
  return toolchain::isa<FIROpsDialect>(t.getDialect());
}

bool isa_std_type(mlir::Type t) {
  return toolchain::isa<mlir::BuiltinDialect>(t.getDialect());
}

bool isa_fir_or_std_type(mlir::Type t) {
  if (auto funcType = mlir::dyn_cast<mlir::FunctionType>(t))
    return toolchain::all_of(funcType.getInputs(), isa_fir_or_std_type) &&
           toolchain::all_of(funcType.getResults(), isa_fir_or_std_type);
  return isa_fir_type(t) || isa_std_type(t);
}

mlir::Type getDerivedType(mlir::Type ty) {
  return toolchain::TypeSwitch<mlir::Type, mlir::Type>(ty)
...
mlir::Type dyn_cast_ptrEleTy(mlir::Type t) {
  return toolchain::TypeSwitch<mlir::Type, mlir::Type>(t)
...
int getTypeCode(mlir::Type ty, const fir::KindMapping &kindMap) {
  if (mlir::IntegerType intTy = mlir::dyn_cast<mlir::IntegerType>(ty)) {
    if (intTy.isUnsigned()) {
      ...
      toolchain_unreachable("unsupported integer type");
    } else {
      switch (intTy.getWidth()) {
      case 8:
  ...
  if (fir::LogicalType logicalTy = mlir::dyn_cast<fir::LogicalType>(ty)) {
    ...
    toolchain_unreachable("unsupported logical type");
  }
  if (mlir::FloatType floatTy = mlir::dyn_cast<mlir::FloatType>(ty)) {
    ...
    toolchain_unreachable("unsupported real type");
  }
  if (mlir::ComplexType complexTy = mlir::dyn_cast<mlir::ComplexType>(ty)) {
    ...
    toolchain_unreachable("unsupported complex size");
  }
  if (fir::CharacterType charTy = mlir::dyn_cast<fir::CharacterType>(ty)) {
    ...
    toolchain_unreachable("unsupported character type");
  }
...
std::string getTypeAsString(mlir::Type ty, const fir::KindMapping &kindMap,
  ...
  toolchain::raw_string_ostream name{buf};
  if (!prefix.empty())
    name << "_";
    ...
    while (ty) {
      if (fir::isa_trivial(ty)) {
        if (mlir::isa<mlir::IndexType>(ty)) {
          ...
          toolchain::report_fatal_error("unsupported type");
        }
        break;
...
mlir::Type changeElementType(mlir::Type type, mlir::Type newElementType,
  ...
  return toolchain::TypeSwitch<mlir::Type, mlir::Type>(type)
      ...
      .Default([&](mlir::Type t) -> mlir::Type {
        assert((fir::isa_trivial(t) || toolchain::isa<fir::RecordType>(t) ||
                toolchain::isa<mlir::NoneType>(t)) &&
...
static toolchain::SmallPtrSet<detail::RecordTypeStorage const *, 4>
    recordTypeVisited;
...
void fir::verifyIntegralType(mlir::Type type) {
  if (isaIntegerType(type) || mlir::isa<mlir::IndexType>(type))
    return;
  toolchain::report_fatal_error("expected integral type");
}

void fir::printFirType(FIROpsDialect *, mlir::Type ty,
                       mlir::DialectAsmPrinter &p) {
  if (mlir::failed(generatedTypePrinter(ty, p)))
    toolchain::report_fatal_error("unknown type to print");
}
...
bool fir::isa_volatile_type(mlir::Type t) {
  return toolchain::TypeSwitch<mlir::Type, bool>(t)
...
BoxProcType::verify(toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
                    mlir::Type eleTy) {
...
toolchain::LogicalResult
fir::BoxType::verify(toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
...
toolchain::LogicalResult
fir::ClassType::verify(toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
                       mlir::Type eleTy, bool isVolatile) {
...
toolchain::LogicalResult
fir::HeapType::verify(toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
                      mlir::Type eleTy) {
...
toolchain::LogicalResult fir::PointerType::verify(
...
void fir::RecordType::finalize(toolchain::ArrayRef<TypePair> lenPList,
                               toolchain::ArrayRef<TypePair> typeList) {
  getImpl()->finalize(lenPList, typeList);
}

toolchain::StringRef fir::RecordType::getName() const {
...
toolchain::LogicalResult fir::RecordType::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
    toolchain::StringRef name) {
  if (name.size() == 0)
...
mlir::Type fir::RecordType::getType(toolchain::StringRef ident) {
...
unsigned fir::RecordType::getFieldIndex(toolchain::StringRef ident) {
  for (auto f : toolchain::enumerate(getTypeList()))
...
toolchain::LogicalResult fir::ReferenceType::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError, mlir::Type eleTy,
    bool isVolatile) {
...
toolchain::LogicalResult fir::SequenceType::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
    toolchain::ArrayRef<int64_t> shape, mlir::Type eleTy,
...
toolchain::LogicalResult fir::TypeDescType::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
    mlir::Type eleTy) {
  if (mlir::isa<BoxType, BoxCharType, BoxProcType, ShapeType, ShapeShiftType,
...
toolchain::LogicalResult fir::VectorType::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError, uint64_t len,
    mlir::Type eleTy) {
...
/// Convert toolchain::Type::TypeID to mlir::Type. \p kind is provided for error
/// messages only.
mlir::Type fir::fromRealTypeID(mlir::MLIRContext *context,
                               toolchain::Type::TypeID typeID, fir::KindTy kind) {
  switch (typeID) {
...
mlir::Type BaseBoxType::getEleTy() const {
  return toolchain::TypeSwitch<fir::BaseBoxType, mlir::Type>(*this)
...
changeTypeShape(mlir::Type type,
  ...
  return toolchain::TypeSwitch<mlir::Type, mlir::Type>(type)
      ...
      .Default([&](mlir::Type t) -> mlir::Type {
        assert((fir::isa_trivial(t) || toolchain::isa<fir::RecordType>(t) ||
                toolchain::isa<mlir::NoneType>(t) ||
                toolchain::isa<fir::CharacterType>(t)) &&
               "unexpected FIR leaf type");
        if (newShape)
...
fir::BaseBoxType fir::BaseBoxType::getBoxTypeWithNewAttr(
  ...
  return toolchain::TypeSwitch<fir::BaseBoxType, fir::BaseBoxType>(*this)
...
bool fir::BaseBoxType::isPointer() const {
  return toolchain::isa<fir::PointerType>(getEleTy());
}

bool fir::BaseBoxType::isPointerOrAllocatable() const {
  return toolchain::isa<fir::PointerType, fir::HeapType>(getEleTy());
}
...
fir::getTypeSizeAndAlignment(mlir::Location loc, mlir::Type ty,
  ...
  if (ty.isIntOrIndexOrFloat() ||
    ...
    toolchain::TypeSize size = dl.getTypeSize(ty);
  ...
  if (auto seqTy = mlir::dyn_cast<fir::SequenceType>(ty)) {
    ...
    std::uint64_t size =
        toolchain::alignTo(eleSize, eleAlign) * seqTy.getConstantArraySize();
  ...
  if (auto recTy = mlir::dyn_cast<fir::RecordType>(ty)) {
    ...
    for (auto component : recTy.getTypeList()) {
      ...
      size =
          toolchain::alignTo(size, compAlign) + toolchain::alignTo(compSize, compAlign);
