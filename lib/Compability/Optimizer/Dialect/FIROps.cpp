...
namespace {
#include "language/Compability/Optimizer/Dialect/CanonicalizationPatterns.inc"
} // namespace

static toolchain::cl::opt<bool> clUseStrictVolatileVerification(
    "strict-fir-volatile-verifier", toolchain::cl::init(false),
    toolchain::cl::desc(
...
static bool verifyInType(mlir::Type inType,
                         toolchain::SmallVectorImpl<toolchain::StringRef> &visited,
  ...
  if (auto st = mlir::dyn_cast<fir::SequenceType>(inType)) {
    ...
    if (toolchain::is_contained(visited, rt.getName()))
      return false;
...
static mlir::ParseResult parseAllocatableOp(FN wrapResultType,
  ...
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> operands;
  toolchain::SmallVector<mlir::Type> typeVec;
...
void fir::AllocaOp::build(mlir::OpBuilder &builder,
                          ...
                          toolchain::StringRef uniqName, mlir::ValueRange typeparams,
...
void fir::AllocaOp::build(mlir::OpBuilder &builder,
                          mlir::OperationState &result, mlir::Type inType,
                          toolchain::StringRef uniqName, bool pinned,
...
toolchain::LogicalResult fir::AllocaOp::verify() {
  toolchain::SmallVector<toolchain::StringRef> visited;
...
toolchain::LogicalResult fir::AllocMemOp::verify() {
  toolchain::SmallVector<toolchain::StringRef> visited;
  if (verifyInType(getInType(), visited, numShapeOperands()))
    return emitOpError("invalid type for allocation");
  if (verifyTypeParamCount(getInType(), numLenParams()))
...
toolchain::LogicalResult fir::ArrayCoorOp::verify() {
...
private:
  ...
  getShiftedIndices(mlir::Value v, mlir::ValueRange indices,
    ...
    auto insertAdjustments = [&](mlir::Operation *op, mlir::ValueRange lbs) {
      ...
      for (auto [lb, idx] : toolchain::zip(lbs, indices)) {
...
toolchain::LogicalResult fir::ArrayLoadOp::verify() {
  auto eleTy = fir::dyn_cast_ptrOrBoxEleTy(getMemref().getType());
  auto arrTy = mlir::dyn_cast<fir::SequenceType>(eleTy);
  if (!arrTy)
    return emitOpError("must be a reference to an array");
...
toolchain::LogicalResult fir::ArrayMergeStoreOp::verify() {
...
toolchain::LogicalResult fir::ArrayFetchOp::verify() {
...
toolchain::LogicalResult fir::ArrayAccessOp::verify() {
  auto arrTy = mlir::cast<fir::SequenceType>(getSequence().getType());
...
toolchain::LogicalResult fir::ArrayUpdateOp::verify() {
...
toolchain::LogicalResult fir::ArrayModifyOp::verify() {
  auto arrTy = mlir::cast<fir::SequenceType>(getSequence().getType());
  auto indSize = getIndices().size();
  if (indSize < arrTy.getDimension())
...
void fir::BoxAddrOp::build(mlir::OpBuilder &builder,
  ...
  mlir::Type type =
      toolchain::TypeSwitch<mlir::Type, mlir::Type>(val.getType())
...
mlir::Type fir::BoxDimsOp::getTupleType() {
  ...
  toolchain::SmallVector<mlir::Type> triple{
...
mlir::ParseResult fir::CallOp::parse(mlir::OpAsmParser &parser,
  ...
  toolchain::StringRef fmfAttrName = getFastmathAttrName(result.name);
  ...
  toolchain::SmallVector<mlir::DictionaryAttr> argAttrs;
  toolchain::SmallVector<mlir::DictionaryAttr> resultAttrs;
  ...
  if (isDirect) {
    ...
    auto funcArgs =
        toolchain::ArrayRef<mlir::OpAsmParser::UnresolvedOperand>(operands)
            .drop_front();
...
void fir::CallOp::build(mlir::OpBuilder &builder, mlir::OperationState &result,
                        ...
                        toolchain::ArrayRef<mlir::Type> results,
...
toolchain::LogicalResult fir::CharConvertOp::verify() {
...
static mlir::ParseResult parseCmpOp(mlir::OpAsmParser &parser,
                                    mlir::OperationState &result) {
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> ops;
  mlir::NamedAttrList attrs;
  ...
  toolchain::StringRef predicateName =
...
fir::CmpcOp::getPredicateByName(toolchain::StringRef name) {
...
static bool typesMatchExceptForVolatility(mlir::Type fromType,
  ...
  const bool match =
      toolchain::TypeSwitch<mlir::Type, bool>(fromType)
...
toolchain::LogicalResult fir::VolatileCastOp::verify() {
...
toolchain::LogicalResult fir::ConvertOp::verify() {
...
void fir::CoordinateOp::build(mlir::OpBuilder &builder,
  ...
  toolchain::SmallVector<int32_t> fieldIndices;
  toolchain::SmallVector<mlir::Value> dynamicIndices;
  bool anyField = false;
...
void fir::CoordinateOp::build(mlir::OpBuilder &builder,
                              mlir::OperationState &result,
                              mlir::Type resultType, mlir::Value ref,
                              toolchain::ArrayRef<fir::IntOrValue> coor) {
  toolchain::SmallVector<int32_t> fieldIndices;
  toolchain::SmallVector<mlir::Value> dynamicIndices;
  bool anyField = false;
  for (fir::IntOrValue index : coor) {
    toolchain::TypeSwitch<fir::IntOrValue>(index)
...
void fir::CoordinateOp::print(mlir::OpAsmPrinter &p) {
  ...
  if (!getFieldIndicesAttr()) {
    ...
    for (auto index : getIndices()) {
      ...
      toolchain::TypeSwitch<fir::IntOrValue>(index)
          .Case<mlir::IntegerAttr>([&](mlir::IntegerAttr intAttr) {
            if (auto recordType = toolchain::dyn_cast<fir::RecordType>(eleTy)) {
...
mlir::ParseResult fir::CoordinateOp::parse(mlir::OpAsmParser &parser,
  ...
  if (parser.parseOperand(memref) || parser.parseComma())
    return mlir::failure();
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> coorOperands;
  toolchain::SmallVector<std::pair<toolchain::StringRef, int>> fieldNames;
  toolchain::SmallVector<int32_t> fieldIndices;
  while (true) {
    toolchain::StringRef fieldName;
  ...
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> allOperands;
  ...
  if (!fieldNames.empty()) {
    ...
    for (auto [fieldName, operandPosition] : fieldNames) {
      auto recTy = toolchain::dyn_cast<fir::RecordType>(eleTy);
      if (!recTy)
...
toolchain::LogicalResult fir::CoordinateOp::verify() {
  ...
  for (auto coorOperand : toolchain::enumerate(getCoor())) {
...
toolchain::LogicalResult fir::DispatchOp::verify() {
...
toolchain::LogicalResult fir::TypeInfoOp::verify() {
...
static toolchain::LogicalResult
verifyEmboxOpVolatilityInvariants(mlir::Type memrefType,
  ...
  mlir::Type boxElementType =
      toolchain::TypeSwitch<mlir::Type, mlir::Type>(resultType)
...
toolchain::LogicalResult fir::EmboxOp::verify() {
...
toolchain::LogicalResult fir::EmboxCharOp::verify() {
  auto eleTy = fir::dyn_cast_ptrEleTy(getMemref().getType());
...
toolchain::LogicalResult fir::EmboxProcOp::verify() {
...
toolchain::LogicalResult fir::TypeDescOp::verify() {
...
void fir::GlobalOp::build(mlir::OpBuilder &builder,
                          mlir::OperationState &result, toolchain::StringRef name,
...
mlir::ParseResult fir::GlobalOp::verifyValidLinkage(toolchain::StringRef linkage) {
  // Supporting only a subset of the LLVM linkage types for now
  static const char *validNames[] = {"common", "internal", "linkonce",
                                     "linkonce_odr", "weak"};
  return mlir::success(toolchain::is_contained(validNames, linkage));
}
...
void fir::FieldIndexOp::build(mlir::OpBuilder &builder,
                              mlir::OperationState &result,
                              toolchain::StringRef fieldName, mlir::Type recTy,
                              mlir::ValueRange operands) {
...
toolchain::SmallVector<mlir::Attribute> fir::FieldIndexOp::getAttributes() {
...
parseCustomRangeSubscript(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<std::int64_t> lbounds;
  toolchain::SmallVector<std::int64_t> ubounds;
  if (parser.parseKeyword("from") ||
    ...
    return mlir::failure();
  toolchain::SmallVector<std::int64_t> zippedBounds;
  for (auto zip : toolchain::zip(lbounds, ubounds)) {
...
static void printCustomRangeSubscript(mlir::OpAsmPrinter &printer,
                                      ...
                                      mlir::DenseIntElementsAttr coord) {
  printer << "from (";
  auto enumerate = toolchain::enumerate(coord.getValues<std::int64_t>());
...
toolchain::LogicalResult fir::InsertOnRangeOp::verify() {
...
void fir::IterWhileOp::build(mlir::OpBuilder &builder,
  ...
  bodyRegion->front().addArguments(
      iterArgs.getTypes(),
      toolchain::SmallVector<mlir::Location>(iterArgs.size(), result.location));
  result.addAttributes(attributes);
}
...
mlir::ParseResult fir::IterWhileOp::parse(mlir::OpAsmParser &parser,
  ...
  // Induction variable.
  toolchain::SmallVector<mlir::OpAsmParser::Argument> regionArgs;
  regionArgs.push_back(inductionVariable);
  regionArgs.push_back(iterateVar);

  if (succeeded(parser.parseOptionalKeyword("iter_args"))) {
    toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> operands;
    toolchain::SmallVector<mlir::Type> regionTypes;
    ...
    if (regionTypes.size() == operands.size() + 2)
      prependCount = true;
    toolchain::ArrayRef<mlir::Type> resTypes = regionTypes;
    resTypes = prependCount ? resTypes.drop_front(2) : resTypes;
    // Resolve input operands.
    for (auto operandType : toolchain::zip(operands, resTypes))
    ...
    toolchain::SmallVector<mlir::Type> typeList;
...
toolchain::LogicalResult fir::IterWhileOp::verify() {
  ...
  unsigned i = 0u;
  for (auto e : toolchain::zip(iterOperands, iterArgs, opResults)) {
...
void fir::IterWhileOp::print(mlir::OpAsmPrinter &p) {
  ...
  if (regionArgs.size() > 1) {
    p << " iter_args(";
    toolchain::interleaveComma(
        toolchain::zip(regionArgs.drop_front(), operands.drop_front()), p,
        [&](auto it) { p << std::get<0>(it) << " = " << std::get<1>(it); });
    p << ") -> (";
    toolchain::interleaveComma(
        toolchain::drop_begin(getResultTypes(), getFinalValue() ? 0 : 1), p);
    p << ")";
...
toolchain::SmallVector<mlir::Region *> fir::IterWhileOp::getLoopRegions() {
  return {&getRegion()};
}

mlir::BlockArgument fir::IterWhileOp::iterArgToBlockArg(mlir::Value iterArg) {
  for (auto i : toolchain::enumerate(getInitArgs()))
    if (iterArg == i.value())
...
void fir::IterWhileOp::resultToSourceOps(
    toolchain::SmallVectorImpl<mlir::Value> &results, unsigned resultNum) {
...
std::optional<toolchain::MutableArrayRef<mlir::OpOperand>>
...
toolchain::SmallVector<mlir::Attribute> fir::LenParamIndexOp::getAttributes() {
  toolchain::SmallVector<mlir::Attribute> attrs;
  attrs.push_back(getFieldIdAttr());
  attrs.push_back(getOnTypeAttr());
  return attrs;
...
void fir::DoLoopOp::build(mlir::OpBuilder &builder,
                          ...
                          toolchain::ArrayRef<mlir::Attribute> reduceAttrs,
                          toolchain::ArrayRef<mlir::NamedAttribute> attributes) {
...
mlir::ParseResult fir::DoLoopOp::parse(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> reduceOperands;
  toolchain::SmallVector<mlir::Type> reduceArgTypes;
  if (succeeded(parser.parseOptionalKeyword("reduce"))) {
    // Parse reduction attributes and variables.
    toolchain::SmallVector<ReduceAttr> attributes;
    if (failed(parser.parseCommaSeparatedList(
      ...
      return mlir::failure();
    // Resolve input operands.
    for (auto operand_type : toolchain::zip(reduceOperands, reduceArgTypes))
      if (parser.resolveOperand(std::get<0>(operand_type),
                                std::get<1>(operand_type), result.operands))
        return mlir::failure();
    toolchain::SmallVector<mlir::Attribute> arrayAttr(attributes.begin(),
                                                 attributes.end());
    result.addAttribute(getReduceAttrsAttrName(result.name),
  ...
  toolchain::SmallVector<mlir::OpAsmParser::Argument> regionArgs;
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> iterOperands;
  toolchain::SmallVector<mlir::Type> argTypes;
  ...
  if (succeeded(parser.parseOptionalKeyword("iter_args"))) {
    ...
    if (result.types.size() == iterOperands.size() + 1)
      prependCount = true;
    // Resolve input operands.
    toolchain::ArrayRef<mlir::Type> resTypes = result.types;
    for (auto operand_type : toolchain::zip(
...
toolchain::LogicalResult fir::DoLoopOp::verify() {
  // Check that the body defines as single block argument for the induction
  // variable.
  auto *body = getBody();
  if (!body->getArgument(0).getType().isIndex())
...
void fir::DoLoopOp::print(mlir::OpAsmPrinter &p) {
  ...
  if (hasReduceOperands()) {
    ...
    toolchain::interleaveComma(toolchain::zip(attrs, operands), p, [&](auto it) {
  ...
  if (hasIterOperands()) {
    ...
    auto regionArgs = getRegionIterArgs();
    auto operands = getIterOperands();
    toolchain::interleaveComma(toolchain::zip(regionArgs, operands), p, [&](auto it) {
      p << std::get<0>(it) << " = " << std::get<1>(it);
    });
    p << ") -> (" << getResultTypes() << ')';
    printBlockTerminators = true;
...
toolchain::SmallVector<mlir::Region *> fir::DoLoopOp::getLoopRegions() {
  return {&getRegion()};
}
...
mlir::ParseResult fir::DTEntryOp::parse(mlir::OpAsmParser &parser,
                                        mlir::OperationState &result) {
  toolchain::StringRef methodName;
...
toolchain::LogicalResult fir::ReboxOp::verify() {
...
toolchain::LogicalResult fir::ReboxAssumedRankOp::verify() {
...
toolchain::LogicalResult fir::ResultOp::verify() {
  ...
  for (auto e : toolchain::zip(results, operands))
...
toolchain::LogicalResult fir::SaveResultOp::verify() {
...
//===----------------------------------------------------------------------===//
static constexpr toolchain::StringRef getCompareOffsetAttr() {
  return "compare_operand_offsets";
}

static constexpr toolchain::StringRef getTargetOffsetAttr() {
  return "target_operand_offsets";
}

template <typename OpT>
static toolchain::LogicalResult verifyIntegralSwitchTerminator(OpT op) {
...
static mlir::ParseResult parseIntegralSwitchTerminator(
    ...
    toolchain::StringRef casesAttr, toolchain::StringRef operandSegmentAttr) {
  mlir::OpAsmParser::UnresolvedOperand selector;
  mlir::Type type;
  if (fir::parseSelector(parser, result, selector, type))
    return mlir::failure();

  toolchain::SmallVector<mlir::Attribute> ivalues;
  toolchain::SmallVector<mlir::Block *> dests;
  toolchain::SmallVector<toolchain::SmallVector<mlir::Value>> destArgs;
  while (true) {
    mlir::Attribute ivalue; // Integer or Unit
    mlir::Block *dest;
    toolchain::SmallVector<mlir::Value> destArg;
  ...
  toolchain::SmallVector<int32_t> argOffs;
  int32_t sumArgs = 0;
...
toolchain::LogicalResult fir::SelectOp::verify() {
...
getMutableSuccessorOperands(unsigned pos, mlir::MutableOperandRange operands,
                            toolchain::StringRef offsetAttr) {
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectOp::getCompareOperands(toolchain::ArrayRef<mlir::Value>, unsigned) {
  return {};
}
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectOp::getSuccessorOperands(toolchain::ArrayRef<mlir::Value> operands,
                                    unsigned oper) {
  auto a =
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectCaseOp::getCompareOperands(toolchain::ArrayRef<mlir::Value> operands,
                                      unsigned cond) {
  auto a =
      (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(getCompareOffsetAttr());
  auto segments = (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectCaseOp::getSuccessorOperands(toolchain::ArrayRef<mlir::Value> operands,
                                        unsigned oper) {
  auto a =
      (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(getTargetOffsetAttr());
  auto segments = (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(
...
mlir::ParseResult fir::SelectCaseOp::parse(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::Attribute> attrs;
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> opers;
  toolchain::SmallVector<mlir::Block *> dests;
  toolchain::SmallVector<toolchain::SmallVector<mlir::Value>> destArgs;
  toolchain::SmallVector<std::int32_t> argOffs;
  ...
  if (parser.resolveOperands(opers, type, result.operands))
    return mlir::failure();
  toolchain::SmallVector<int32_t> targOffs;
  int32_t toffSize = 0;
  const auto count = dests.size();
  for (std::remove_const_t<decltype(count)> i = 0; i != count; ++i) {
    result.addSuccessors(dests[i]);
...
void fir::SelectCaseOp::build(mlir::OpBuilder &builder,
                              mlir::OperationState &result,
                              mlir::Value selector,
                              toolchain::ArrayRef<mlir::Attribute> compareAttrs,
                              toolchain::ArrayRef<mlir::ValueRange> cmpOperands,
                              toolchain::ArrayRef<mlir::Block *> destinations,
                              toolchain::ArrayRef<mlir::ValueRange> destOperands,
                              toolchain::ArrayRef<mlir::NamedAttribute> attributes) {
  result.addOperands(selector);
  result.addAttribute(getCasesAttr(), builder.getArrayAttr(compareAttrs));
  toolchain::SmallVector<int32_t> operOffs;
  int32_t operSize = 0;
...
void fir::SelectCaseOp::build(mlir::OpBuilder &builder,
                              ...
                              mlir::Value selector,
                              toolchain::ArrayRef<mlir::Attribute> compareAttrs,
                              toolchain::ArrayRef<mlir::Value> cmpOpList,
                              toolchain::ArrayRef<mlir::Block *> destinations,
                              toolchain::ArrayRef<mlir::ValueRange> destOperands,
                              toolchain::ArrayRef<mlir::NamedAttribute> attributes) {
  toolchain::SmallVector<mlir::ValueRange> cmpOpers;
...
toolchain::LogicalResult fir::SelectCaseOp::verify() {
  if (!mlir::isa<mlir::IntegerType, mlir::IndexType, fir::IntegerType,
...
toolchain::LogicalResult fir::SelectRankOp::verify() {
  return verifyIntegralSwitchTerminator(*this);
}
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectRankOp::getCompareOperands(toolchain::ArrayRef<mlir::Value>, unsigned) {
  return {};
}
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectRankOp::getSuccessorOperands(toolchain::ArrayRef<mlir::Value> operands,
                                        unsigned oper) {
  auto a =
      (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(getTargetOffsetAttr());
  auto segments = (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectTypeOp::getCompareOperands(toolchain::ArrayRef<mlir::Value>, unsigned) {
  return {};
}
...
std::optional<toolchain::ArrayRef<mlir::Value>>
fir::SelectTypeOp::getSuccessorOperands(toolchain::ArrayRef<mlir::Value> operands,
                                        unsigned oper) {
  auto a =
      (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(getTargetOffsetAttr());
  auto segments = (*this)->getAttrOfType<mlir::DenseI32ArrayAttr>(
...
toolchain::LogicalResult fir::SelectTypeOp::verify() {
...
void fir::SelectTypeOp::build(mlir::OpBuilder &builder,
                              mlir::OperationState &result,
                              mlir::Value selector,
                              toolchain::ArrayRef<mlir::Attribute> typeOperands,
                              toolchain::ArrayRef<mlir::Block *> destinations,
                              toolchain::ArrayRef<mlir::ValueRange> destOperands,
                              toolchain::ArrayRef<mlir::NamedAttribute> attributes) {
  result.addOperands(selector);
...
toolchain::LogicalResult fir::ShapeOp::verify() {
...
toolchain::LogicalResult fir::ShapeShiftOp::verify() {
...
toolchain::LogicalResult fir::ShiftOp::verify() {
...
toolchain::LogicalResult fir::SliceOp::verify() {
...
toolchain::LogicalResult fir::StoreOp::verify() {
...
toolchain::LogicalResult fir::CopyOp::verify() {
...
mkNamedIntegerAttr(mlir::OpBuilder &builder, toolchain::StringRef name, int64_t v) {
  assert(v > 0);
...
void fir::StringLitOp::build(mlir::OpBuilder &builder,
                             mlir::OperationState &result,
                             fir::CharacterType inType, toolchain::StringRef val,
...
void fir::StringLitOp::build(mlir::OpBuilder &builder,
                             mlir::OperationState &result,
                             fir::CharacterType inType,
                             toolchain::ArrayRef<char16_t> vlist,
                             std::optional<std::int64_t> len) {
  auto valAttr =
      builder.getNamedAttr(xlist(), convertToArrayAttr(builder, vlist));
  std::int64_t length = len ? *len : inType.getLen();
...
void fir::StringLitOp::build(mlir::OpBuilder &builder,
                             mlir::OperationState &result,
                             fir::CharacterType inType,
                             toolchain::ArrayRef<char32_t> vlist,
                             std::optional<std::int64_t> len) {
  auto valAttr =
      builder.getNamedAttr(xlist(), convertToArrayAttr(builder, vlist));
  std::int64_t length = len ? *len : inType.getLen();
...
mlir::ParseResult fir::StringLitOp::parse(mlir::OpAsmParser &parser,
  ...
  mlir::Attribute val;
  mlir::NamedAttrList attrs;
  toolchain::SMLoc trailingTypeLoc;
...
toolchain::LogicalResult fir::StringLitOp::verify() {
...
toolchain::LogicalResult fir::UnboxProcOp::verify() {
...
void fir::IfOp::getEntrySuccessorRegions(
    toolchain::ArrayRef<mlir::Attribute> operands,
    toolchain::SmallVectorImpl<mlir::RegionSuccessor> &regions) {
...
void fir::IfOp::getRegionInvocationBounds(
    toolchain::ArrayRef<mlir::Attribute> operands,
    toolchain::SmallVectorImpl<mlir::InvocationBounds> &invocationBounds) {
...
toolchain::LogicalResult fir::IfOp::verify() {
...
void fir::IfOp::resultToSourceOps(toolchain::SmallVectorImpl<mlir::Value> &results,
                                  unsigned resultNum) {
...
toolchain::LogicalResult fir::BoxOffsetOp::verify() {
...
unsigned fir::getCaseArgumentOffset(toolchain::ArrayRef<mlir::Attribute> cases,
                                    unsigned dest) {
  unsigned o = 0;
...
valueCheckFirAttributes(mlir::Value value,
                        ...
                        bool checkAny) {
  auto testAttributeSets = [&](toolchain::ArrayRef<mlir::NamedAttribute> setAttrs,
                               toolchain::ArrayRef<toolchain::StringRef> checkAttrs) {
    if (checkAny) {
      // Return true iff any of checkAttrs attributes is present
      // in setAttrs set.
      for (toolchain::StringRef checkAttrName : checkAttrs)
        if (toolchain::any_of(setAttrs, [&](mlir::NamedAttribute setAttr) {
    ...
    for (mlir::StringRef checkAttrName : checkAttrs)
      if (toolchain::none_of(setAttrs, [&](mlir::NamedAttribute setAttr) {
            return setAttr.getName() == checkAttrName;
          }))
        return false;
...
bool fir::valueMayHaveFirAttributes(
    mlir::Value value, toolchain::ArrayRef<toolchain::StringRef> attributeNames) {
...
bool fir::valueHasFirAttribute(mlir::Value value,
                               toolchain::StringRef attributeName) {
  std::optional<bool> mayHaveAttr =
      valueCheckFirAttributes(value, {attributeName}, /*checkAny=*/false);
  return mayHaveAttr.value_or(false);
}

bool fir::anyFuncArgsHaveAttr(mlir::func::FuncOp func, toolchain::StringRef attr) {
...
mlir::Type fir::applyPathToType(mlir::Type eleTy, mlir::ValueRange path) {
  for (auto i = path.begin(), end = path.end(); eleTy && i < end;) {
    eleTy = toolchain::TypeSwitch<mlir::Type, mlir::Type>(eleTy)
...
toolchain::LogicalResult fir::DeclareOp::verify() {
  auto fortranVar =
...
toolchain::LogicalResult fir::PackArrayOp::verify() {
...
static void printPackArrayConstraints(mlir::OpAsmPrinter &p,
  ...
  toolchain::SmallVector<mlir::NamedAttribute> attributes;
  if (maxSize)
...
toolchain::LogicalResult fir::UnpackArrayOp::verify() {
...
// https://discourse.toolchain.org/t/dialect-for-data-locality-sharing-specifiers-clauses-in-openmp-openacc-and-do-concurrent/86108.
toolchain::LogicalResult fir::LocalitySpecifierOp::verifyRegions() {
  mlir::Type argType = getArgType();
  auto verifyTerminator = [&](mlir::Operation *terminator,
                              bool yieldsValue) -> toolchain::LogicalResult {
    if (!terminator->getBlock()->getSuccessors().empty())
      return toolchain::success();

    if (!toolchain::isa<fir::YieldOp>(terminator))
      return mlir::emitError(terminator->getLoc())
             << "expected exit block terminator to be an `fir.yield` op.";

    YieldOp yieldOp = toolchain::cast<YieldOp>(terminator);
  ...
  auto verifyRegion = [&](mlir::Region &region, unsigned expectedNumArgs,
                          toolchain::StringRef regionName,
                          bool yieldsValue) -> toolchain::LogicalResult {
    assert(!region.empty());
...
toolchain::LogicalResult fir::DoConcurrentOp::verify() {
...
static mlir::ParseResult parseSpecifierList(
    mlir::OpAsmParser &parser, mlir::OperationState &result,
    toolchain::StringRef specifierKeyword, toolchain::StringRef symsAttrName,
    toolchain::SmallVectorImpl<mlir::OpAsmParser::Argument> &regionArgs,
    toolchain::SmallVectorImpl<mlir::Type> &regionArgTypes,
    int32_t &numSpecifierOperands, bool isReduce = false) {
  auto &builder = parser.getBuilder();
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand> specifierOperands;
  ...
  toolchain::SmallVector<bool> isByRefVec;
  toolchain::SmallVector<mlir::SymbolRefAttr> spceifierSymbolVec;
  toolchain::SmallVector<fir::ReduceAttr> attributes;
  ...
  for (auto operandType :
       toolchain::zip_equal(specifierOperands,
                       toolchain::drop_begin(regionArgTypes, oldArgTypesSize)))
    if (parser.resolveOperand(std::get<0>(operandType),
                              std::get<1>(operandType), result.operands))
      return mlir::failure();
  ...
  toolchain::SmallVector<mlir::Attribute> symbolAttrs(spceifierSymbolVec.begin(),
...
mlir::ParseResult fir::DoConcurrentLoopOp::parse(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::OpAsmParser::Argument, 4> regionArgs;

  if (parser.parseArgumentList(regionArgs, mlir::OpAsmParser::Delimiter::Paren))
    return mlir::failure();

  toolchain::SmallVector<mlir::Type> argTypes(regionArgs.size(),
                                         builder.getIndexType());

  // Parse loop bounds.
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 4> lower;
  ...
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 4> upper;
  ...
  // Parse step values.
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 4> steps;
  ...
  // Now parse the body.
  for (auto [arg, type] : toolchain::zip_equal(regionArgs, argTypes))
    arg.type = type;
...
void fir::DoConcurrentLoopOp::print(mlir::OpAsmPrinter &p) {
  ...
  if (!getLocalVars().empty()) {
    p << " local(";
    toolchain::interleaveComma(toolchain::zip_equal(getLocalSymsAttr(), getLocalVars(),
                                          getRegionLocalArgs()),
                          p, [&](auto it) {
                            p << std::get<0>(it) << " " << std::get<1>(it)
                              << " -> " << std::get<2>(it);
    ...
    p << " : ";
    toolchain::interleaveComma(getLocalVars(), p,
  ...
  if (!getReduceVars().empty()) {
    p << " reduce(";
    toolchain::interleaveComma(
        toolchain::zip_equal(getReduceByrefAttr().asArrayRef(), getReduceSymsAttr(),
    ...
    p << " : ";
    toolchain::interleaveComma(getReduceVars(), p,
                          [&](auto it) { p << it.getType(); });
    p << ")";
  }
...
toolchain::SmallVector<mlir::Region *> fir::DoConcurrentLoopOp::getLoopRegions() {
  return {&getRegion()};
}

toolchain::LogicalResult fir::DoConcurrentLoopOp::verify() {
...
std::optional<toolchain::SmallVector<mlir::Value>>
fir::DoConcurrentLoopOp::getLoopInductionVars() {
  return toolchain::SmallVector<mlir::Value>{
