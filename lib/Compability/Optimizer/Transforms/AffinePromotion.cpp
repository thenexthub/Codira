...
struct AffineFunctionAnalysis {
  ...
  toolchain::DenseMap<mlir::Operation *, AffineLoopAnalysis> loopAnalysisMap;
  toolchain::DenseMap<mlir::Operation *, AffineIfAnalysis> ifAnalysisMap;
};
} // namespace
...
static bool analyzeCoordinate(mlir::Value coordinate, mlir::Operation *op) {
  if (auto blockArg = mlir::dyn_cast<mlir::BlockArgument>(coordinate)) {
    if (isa<fir::DoLoopOp>(blockArg.getOwner()->getParentOp()))
      return true;
    LLVM_DEBUG(toolchain::dbgs() << "AffineLoopAnalysis: array coordinate is not a "
                               "loop induction variable (owner not loopOp)\n";
               op->dump());
    return false;
  }
  LLVM_DEBUG(
      toolchain::dbgs() << "AffineLoopAnalysis: array coordinate is not a loop "
...
private:
  ...
  bool analyzeLoop(fir::DoLoopOp loopOperation,
                   AffineFunctionAnalysis &functionAnalysis) {
    LLVM_DEBUG(toolchain::dbgs() << "AffineLoopAnalysis: \n"; loopOperation.dump(););
  ...
  bool analyzeReference(mlir::Value memref, mlir::Operation *op) {
    if (auto acoOp = memref.getDefiningOp<ArrayCoorOp>()) {
      if (mlir::isa<fir::BoxType>(acoOp.getMemref().getType())) {
        ...
        LLVM_DEBUG(toolchain::dbgs() << "AffineLoopAnalysis: cannot promote loop, "
    ...
    LLVM_DEBUG(toolchain::dbgs() << "AffineLoopAnalysis: unknown type of memory "
...
AffineFunctionAnalysis::getChildLoopAnalysis(fir::DoLoopOp op) const {
  ...
  if (it == loopAnalysisMap.end()) {
    LLVM_DEBUG(toolchain::dbgs() << "AffineFunctionAnalysis: not computed for:\n";
               op.dump(););
...
private:
  ...
  toolchain::SmallVector<mlir::Value> affineArgs;
...
public:
  ...
  matchAndRewrite(fir::DoLoopOp loop,
    ...
    LLVM_DEBUG(toolchain::dbgs() << "AffineLoopConversion: rewriting loop:\n";
               loop.dump(););
    ...
    LLVM_DEBUG(toolchain::dbgs() << "AffineLoopConversion: loop rewriten to:\n";
               affineFor.dump(););
...
public:
  ...
  matchAndRewrite(fir::IfOp op,
                  mlir::PatternRewriter &rewriter) const override {
    LLVM_DEBUG(toolchain::dbgs() << "AffineIfConversion: rewriting if:\n";
               op.dump(););
    ...
    LLVM_DEBUG(toolchain::dbgs() << "AffineIfConversion: if converted to:\n";
               affineIf.dump(););
