...
mangleExternalName(const std::pair<fir::NameUniquer::NameKind,
  ...
  if (result.first == fir::NameUniquer::NameKind::COMMON &&
    ...
    return language::Compability::common::blankCommonObjectName;
  return language::Compability::common::GetExternalAssemblyName(result.second.name,
                                                  appendUnderscore);
}
...
processSymbolRef(mlir::SymbolRefAttr symRef, mlir::Operation *nestedOp,
                 const toolchain::DenseMap<mlir::StringAttr, mlir::FlatSymbolRefAttr>
                     &remappings) {
...
void ExternalNameConversionPass::runOnOperation() {
  ...
  auto processFctOrGlobal = [&](mlir::Operation &funcOrGlobal) {
    ...
    if (fir::NameUniquer::isExternalFacingUniquedName(deconstructedName)) {
      ...
      if (toolchain::isa<mlir::func::FuncOp>(funcOrGlobal))
  ...
  op.walk([&remappings](mlir::Operation *nestedOp) {
    toolchain::SmallVector<std::pair<mlir::StringAttr, mlir::SymbolRefAttr>>
        symRefUpdates;
    toolchain::SmallVector<std::pair<mlir::StringAttr, mlir::ArrayAttr>>
        arrayUpdates;
        ...
        if (auto newSymRef = processSymbolRef(symRef, nestedOp, remappings))
          symRefUpdates.emplace_back(
              std::pair<mlir::StringAttr, mlir::SymbolRefAttr>{attr.getName(),
                     ...
                     toolchain::dyn_cast<mlir::ArrayAttr>(attr.getValue())) {
        toolchain::SmallVector<mlir::Attribute> symbolRefs;
        for (auto element : arrayAttr) {
          if (!element) {
            symbolRefs.push_back(element);
            continue;
          }
          auto symRef = toolchain::dyn_cast<mlir::SymbolRefAttr>(element);
