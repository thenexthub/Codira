...
std::uint64_t getComponentOffset(const mlir::DataLayout &dl,
  ...
  std::uint64_t fieldAlignment = dl.getTypeABIAlignment(toolchainFieldType);
  return toolchain::alignTo(offset, fieldAlignment);
}
template <>
...
DebugTypeGenerator::DebugTypeGenerator(mlir::ModuleOp m,
    ...
    : module(m), symbolTable(symbolTable_), dataLayout{&dl},
      kindMapping(getKindMapping(m)), toolchainTypeConverter(m, false, false, dl) {
  LLVM_DEBUG(toolchain::dbgs() << "DITypeAttr generator\n");
  ...
  mlir::Type toolchainDimsType = getDescFieldTypeModel<kDimsPosInBox>()(context);
  mlir::Type toolchainPtrType = getDescFieldTypeModel<kAddrPosInBox>()(context);
  mlir::Type toolchainLenType = getDescFieldTypeModel<kElemLenPosInBox>()(context);
  mlir::Type toolchainRankType = getDescFieldTypeModel<kRankPosInBox>()(context);

  dimsOffset =
      getComponentOffset<kDimsPosInBox>(*dataLayout, context, toolchainDimsType);
  dimsSize = dataLayout->getTypeSize(toolchainDimsType);
  ptrSize = dataLayout->getTypeSize(toolchainPtrType);
  rankSize = dataLayout->getTypeSize(toolchainRankType);
  lenOffset =
      getComponentOffset<kElemLenPosInBox>(*dataLayout, context, toolchainLenType);
  rankOffset =
      getComponentOffset<kRankPosInBox>(*dataLayout, context, toolchainRankType);
}
...
static mlir::LLVM::DITypeAttr genBasicType(mlir::MLIRContext *context,
  ...
  return mlir::LLVM::DIBasicTypeAttr::get(
      context, toolchain::dwarf::DW_TAG_base_type, name, bitSize, decoding);
}

static mlir::LLVM::DITypeAttr genPlaceholderType(mlir::MLIRContext *context) {
  return genBasicType(context, mlir::StringAttr::get(context, "integer"),
                      /*bitSize=*/32, toolchain::dwarf::DW_ATE_signed);
}
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertBoxedSequenceType(
  ...
  mlir::MLIRContext *context = module.getContext();
  toolchain::SmallVector<mlir::LLVM::DINodeAttr> elements;
  toolchain::SmallVector<mlir::LLVM::DIExpressionElemAttr> ops;
  auto addOp = [&](unsigned opc, toolchain::ArrayRef<uint64_t> vals) {
    ops.push_back(mlir::LLVM::DIExpressionElemAttr::get(context, opc, vals));
  };

  addOp(toolchain::dwarf::DW_OP_push_object_address, {});
  addOp(toolchain::dwarf::DW_OP_deref, {});
  ...
  if (seqTy.hasUnknownShape()) {
    addOp(toolchain::dwarf::DW_OP_push_object_address, {});
    addOp(toolchain::dwarf::DW_OP_plus_uconst, {rankOffset});
    addOp(toolchain::dwarf::DW_OP_deref_size, {rankSize});
    ...
    auto genSubrangeOp = [&](unsigned field) -> mlir::LLVM::DIExpressionAttr {
      ...
      addOp(toolchain::dwarf::DW_OP_push_object_address, {});
      addOp(toolchain::dwarf::DW_OP_over, {});
      addOp(toolchain::dwarf::DW_OP_constu, {dimsSize});
      addOp(toolchain::dwarf::DW_OP_mul, {});
      addOp(toolchain::dwarf::DW_OP_plus_uconst,
            {dimsOffset + ((dimsSize / 3) * field)});
      addOp(toolchain::dwarf::DW_OP_plus, {});
      addOp(toolchain::dwarf::DW_OP_deref, {});
    ...
    return mlir::LLVM::DICompositeTypeAttr::get(
        context, toolchain::dwarf::DW_TAG_array_type, /*name=*/nullptr,
  ...
  addOp(toolchain::dwarf::DW_OP_push_object_address, {});
  addOp(toolchain::dwarf::DW_OP_deref, {});
  addOp(toolchain::dwarf::DW_OP_lit0, {});
  addOp(toolchain::dwarf::DW_OP_ne, {});
  ...
  for ([[maybe_unused]] auto _ : seqTy.getShape()) {
    ...
    if (declOp && declOp.getShift().size() > index) {
      if (std::optional<std::int64_t> optint =
        ...
        lowerAttr = mlir::IntegerAttr::get(intTy, toolchain::APInt(64, *optint));
      else
...
DebugTypeGenerator::getFieldSizeAndAlign(mlir::Type fieldTy) {
  mlir::Type toolchainTy;
  if (auto boxTy = mlir::dyn_cast_if_present<fir::BaseBoxType>(fieldTy))
    toolchainTy = toolchainTypeConverter.convertBoxTypeAsStruct(boxTy, getBoxRank(boxTy));
  else
    toolchainTy = toolchainTypeConverter.convertType(fieldTy);

  uint64_t byteSize = dataLayout->getTypeSize(toolchainTy);
  unsigned short byteAlign = dataLayout->getTypeABIAlignment(toolchainTy);
...
void DerivedTypeCache::finalize(mlir::Type ty, mlir::LLVM::DITypeAttr attr,
  ...
  if (activeRecursionLevels.back() >= derivedTypeDepth) {
    auto last = toolchain::find_if(activeRecursionLevels, [&](std::int32_t depth) {
...
void DerivedTypeCache::insertCacheCleanUp(mlir::Type type, int32_t depth) {
  auto iter = toolchain::find_if(cacheCleanupList,
                            [&](const auto &x) { return x.second >= depth; });
  if (iter == cacheCleanupList.end()) {
    cacheCleanupList.emplace_back(
        std::pair<toolchain::SmallVector<mlir::Type>, int32_t>({type}, depth));
    return;
  }
  ...
  cacheCleanupList.insert(
      iter, std::pair<toolchain::SmallVector<mlir::Type>, int32_t>({type}, depth));
}
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertRecordType(
  ...
  auto placeHolder = mlir::LLVM::DICompositeTypeAttr::get(
      context, recId, /*isRecSelf=*/true, toolchain::dwarf::DW_TAG_structure_type,
  ...
  for (auto [fieldName, fieldTy] : Ty.getTypeList()) {
    ...
    std::optional<toolchain::ArrayRef<int64_t>> lowerBounds =
    ...
    if (lowerBounds && seqTy &&
      ...
      toolchain::SmallVector<mlir::LLVM::DINodeAttr> arrayElements;
      for (auto [bound, dim] :
           toolchain::zip_equal(*lowerBounds, seqTy.getShape())) {
        auto countAttr = mlir::IntegerAttr::get(intTy, toolchain::APInt(64, dim));
        auto lowerAttr = mlir::IntegerAttr::get(intTy, toolchain::APInt(64, bound));
    ...
    offset = toolchain::alignTo(offset, byteAlign);
    mlir::LLVM::DIDerivedTypeAttr tyAttr = mlir::LLVM::DIDerivedTypeAttr::get(
        context, toolchain::dwarf::DW_TAG_member,
    ...
    offset += toolchain::alignTo(byteSize, byteAlign);
    derivedTypeCache.postComponentVisitUpdate(nestedRecursions);
  }

  auto finalAttr = mlir::LLVM::DICompositeTypeAttr::get(
      context, recId, /*isRecSelf=*/false, toolchain::dwarf::DW_TAG_structure_type,
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertTupleType(
  ...
  for (auto fieldTy : Ty.getTypes()) {
    ...
    offset = toolchain::alignTo(offset, byteAlign);
    mlir::LLVM::DIDerivedTypeAttr tyAttr = mlir::LLVM::DIDerivedTypeAttr::get(
        context, toolchain::dwarf::DW_TAG_member, mlir::StringAttr::get(context, ""),
  ...
  auto typeAttr = mlir::LLVM::DICompositeTypeAttr::get(
      context, toolchain::dwarf::DW_TAG_structure_type,
      mlir::StringAttr::get(context, ""), fileAttr, /*line=*/0, scope,
      /*baseType=*/nullptr, mlir::LLVM::DIFlags::Zero, offset * 8,
      /*alignInBits=*/0, elements, /*dataLocation=*/nullptr, /*rank=*/nullptr,
      /*allocated=*/nullptr, /*associated=*/nullptr);
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertSequenceType(
  ...
  for (fir::SequenceType::Extent dim : seqTy.getShape()) {
    ...
    if (dim == seqTy.getUnknownExtent()) {
      ...
      countAttr = mlir::IntegerAttr::get(intTy, toolchain::APInt(64, dim));
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertVectorType(
  ...
  auto intTy = mlir::IntegerType::get(context, 64);
  auto countAttr =
      mlir::IntegerAttr::get(intTy, toolchain::APInt(64, vecTy.getLen()));
  auto subrangeTy = mlir::LLVM::DISubrangeAttr::get(
      context, countAttr, /*lowerBound=*/nullptr, /*upperBound=*/nullptr,
      /*stride=*/nullptr);
  elements.push_back(subrangeTy);
  mlir::Type toolchainTy = toolchainTypeConverter.convertType(vecTy.getEleTy());
  uint64_t sizeInBits = dataLayout->getTypeSize(toolchainTy) * vecTy.getLen() * 8;
  ...
  return mlir::LLVM::DICompositeTypeAttr::get(
      context, toolchain::dwarf::DW_TAG_array_type,
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertCharacterType(
  ...
  // 10646:2000)."
  unsigned encoding = toolchain::dwarf::DW_ATE_ASCII;
  if (charTy.getFKind() != 1)
    encoding = toolchain::dwarf::DW_ATE_UCS;
  ...
  if (hasDescriptor) {
    ...
    addOp(toolchain::dwarf::DW_OP_push_object_address, {});
    addOp(toolchain::dwarf::DW_OP_plus_uconst, {lenOffset});
  ...
  // FIXME: Currently the DIStringType in toolchain does not have the option to set
  ...
  return mlir::LLVM::DIStringTypeAttr::get(
      context, toolchain::dwarf::DW_TAG_string_type,
...
mlir::LLVM::DITypeAttr DebugTypeGenerator::convertPointerLikeType(
  ...
  return mlir::LLVM::DIDerivedTypeAttr::get(
      context, toolchain::dwarf::DW_TAG_pointer_type,
...
DebugTypeGenerator::convertType(mlir::Type Ty, mlir::LLVM::DIFileAttr fileAttr,
  ...
  if (Ty.isInteger()) {
    ...
    return genBasicType(context, mlir::StringAttr::get(context, "complex"),
                        bitWidth * 2, toolchain::dwarf::DW_ATE_complex_float);
    ...
    return genBasicType(context, mlir::StringAttr::get(context, "integer"),
                        toolchainTypeConverter.getIndexTypeBitwidth(),
