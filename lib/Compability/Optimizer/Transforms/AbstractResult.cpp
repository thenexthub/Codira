...
struct LazySymbolTable {
  ...
  template <typename T>
  T lookup(toolchain::StringRef name) {
    build();
...
static mlir::Type getResultArgumentType(mlir::Type resultType,
  ...
  return toolchain::TypeSwitch<mlir::Type, mlir::Type>(resultType)
      ...
      .Default([](mlir::Type) -> mlir::Type {
        toolchain_unreachable("bad abstract result type");
      });
}
...
static mlir::FunctionType getNewFunctionType(mlir::FunctionType funcTy,
  ...
  toolchain::SmallVector<mlir::Type> newInputTypes = {argTy};
...
static mlir::FunctionType getCPtrFunctionType(mlir::FunctionType funcTy) {
  ...
  toolchain::SmallVector<mlir::Type> outputTypes{
...
public:
  ...
  matchAndRewrite(Op op, mlir::PatternRewriter &rewriter) const override {
    ...
    toolchain::SmallVector<mlir::Type> newResultTypes;
    ...
    if constexpr (std::is_same_v<Op, fir::CallOp>) {
      if (op.getCallee()) {
        ...
        // Indirect calls.
        toolchain::SmallVector<mlir::Type> newInputTypes;
        if (!isResultBuiltinCPtr)
  ...
  void runOnFunctionLikeOperation(OpTy func, bool shouldBoxResult,
    ...
    if (hasAbstractResult(funcTy)) {
      ...
      if (!func.empty()) {
        ...
        auto resultType = funcTy.getResult(0);
        auto argTy = getResultArgumentType(resultType, shouldBoxResult);
        toolchain::LogicalResult res = func.insertArgument(0u, argTy, {}, loc);
        (void)res;
        assert(toolchain::succeeded(res) && "failed to insert function argument");
        res = func.eraseResult(0u);
        (void)res;
        assert(toolchain::succeeded(res) && "failed to erase function result");
        ...
        toolchain::SmallVector<mlir::DictionaryAttr> allArgs;
