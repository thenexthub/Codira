...
static toolchain::cl::opt<std::size_t> maxAllocsPerFunc(
    "stack-arrays-max-allocs",
    toolchain::cl::desc("The maximum number of heap allocations to consider in one "
                   "function before skipping (to save compilation time). Set "
                   "to 0 for no limit."),
    toolchain::cl::init(1000), toolchain::cl::Hidden);

static toolchain::cl::opt<bool> emitLifetimeMarkers(
    "stack-arrays-lifetime",
    toolchain::cl::desc("Add lifetime markers to generated constant size allocas"),
...
class InsertionPoint {
  toolchain::PointerUnion<mlir::Operation *, mlir::Block *> location;
  bool saveRestoreStack;

  /// Get contained pointer type or nullptr
  template <class T>
  T *tryGetPtr() const {
    // Use toolchain::dyn_cast_if_present because location may be null here.
    if (T *ptr = toolchain::dyn_cast_if_present<T *>(location))
      return ptr;
    return nullptr;
  }
...
public:
  ...
  /// Get whether the stack should be saved/restored. If yes, an toolchain.stacksave
  /// intrinsic should be added before the alloca, and an toolchain.stackrestore
...
class LatticePoint : public mlir::dataflow::AbstractDenseLattice {
  ...
  toolchain::SmallDenseMap<mlir::Value, AllocationState, 1> stateMap;
...
public:
  ...
  /// fir.alloca ops
  void appendFreedValues(toolchain::DenseSet<mlir::Value> &out) const;
...
public:
  ...
  using AllocMemMap = toolchain::DenseMap<mlir::Operation *, InsertionPoint>;
...
private:
  toolchain::DenseMap<mlir::Operation *, AllocMemMap> funcMaps;

  toolchain::LogicalResult analyseFunction(mlir::Operation *func);
};
...
public:
  ...
  findAllocaInsertionPoint(fir::AllocMemOp &oldAlloc,
                           const toolchain::SmallVector<mlir::Operation *> &freeOps);
...
private:
  ...
  /// be safe to use an toolchain.stacksave/toolchain.stackrestore inside the loop
...
static void print(toolchain::raw_ostream &os, AllocationState state) {
  switch (state) {
...
void LatticePoint::print(toolchain::raw_ostream &os) const {
...
void LatticePoint::appendFreedValues(toolchain::DenseSet<mlir::Value> &out) const {
...
static mlir::Value lookThroughDeclaresAndConverts(mlir::Value value) {
  while (mlir::Operation *op = value.getDefiningOp()) {
    if (auto declareOp = toolchain::dyn_cast<fir::DeclareOp>(op))
      value = declareOp.getMemref();
    else if (auto convertOp = toolchain::dyn_cast<fir::ConvertOp>(op))
...
mlir::LogicalResult AllocationAnalysis::visitOperation(
  ...
  LLVM_DEBUG(toolchain::dbgs() << "StackArrays: Visiting operation: " << *op
                          << "\n");
  LLVM_DEBUG(toolchain::dbgs() << "--Lattice in: " << before << "\n");
  ...
  if (auto allocmem = mlir::dyn_cast<fir::AllocMemOp>(op)) {
    ...
    if (attr && attr.getValue()) {
      LLVM_DEBUG(toolchain::dbgs() << "--Found fir.must_be_heap: skipping\n");
  ...
  LLVM_DEBUG(toolchain::dbgs() << "--Lattice out: " << *after << "\n");
...
StackArraysAnalysisWrapper::analyseFunction(mlir::Operation *func) {
  ...
  if ((maxAllocsPerFunc != 0) && (nAllocs > maxAllocsPerFunc)) {
    LLVM_DEBUG(toolchain::dbgs() << "Skipping stack arrays for function with "
  ...
  if (failed(solver.initializeAndRun(func))) {
    toolchain::errs() << "DataFlowSolver failed!";
    return mlir::failure();
  }
  ...
  toolchain::DenseSet<mlir::Value> freedValues;
  ...
  toolchain::DenseMap<mlir::Operation *, toolchain::SmallVector<mlir::Operation *>>
      allocToFreeMemMap;
  ...
  LLVM_DEBUG(for (auto [allocMemOp, _]
                  : candidateOps) {
    toolchain::dbgs() << "StackArrays: Found candidate op: " << *allocMemOp << '\n';
  });
  return mlir::success();
}
...
AllocMemConversion::matchAndRewrite(fir::AllocMemOp allocmem,
  ...
  toolchain::SmallVector<mlir::Operation *> erases;
...
InsertionPoint AllocMemConversion::findAllocaInsertionPoint(
  ...
  // outside of that
  LLVM_DEBUG(toolchain::dbgs() << "StackArrays: findAllocaInsertionPoint: "
                          << oldAlloc << "\n");
  ...
  for (mlir::Value operand : oldAlloc.getOperands()) {
    LLVM_DEBUG(toolchain::dbgs() << "--considering operand " << operand << "\n");
  ...
  LLVM_DEBUG(toolchain::dbgs() << "--Placing at the start of func entry block\n");
...
InsertionPoint AllocMemConversion::findAllocaLoopInsertionPoint(
  ...
  // toolchain.stacksave/toolchain.stackrestore
...
AllocMemConversion::insertAlloca(fir::AllocMemOp &oldAlloc,
  ...
  toolchain::StringRef uniqName = unpackName(oldAlloc.getUniqName());
  toolchain::StringRef bindcName = unpackName(oldAlloc.getBindcName());
  auto alloca =
...
void AllocMemConversion::insertLifetimeMarkers(
  ...
  if (!dl || !kindMap)
    return;
  toolchain::StringRef attrName = fir::getHasLifetimeMarkerAttrName();
...
toolchain::StringRef StackArraysPass::getDescription() const {
...
void StackArraysPass::runOnOperation() {
  ...
  toolchain::SmallVector<mlir::Operation *> opsToConvert;
