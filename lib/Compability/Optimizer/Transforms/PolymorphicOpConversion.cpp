...
using BindingTable = toolchain::DenseMap<toolchain::StringRef, unsigned>;
using BindingTables = toolchain::DenseMap<toolchain::StringRef, BindingTable>;

static std::string getTypeDescriptorTypeName() {
  toolchain::SmallVector<toolchain::StringRef, 1> modules = {
      language::Compability::semantics::typeInfoBuiltinModule};
  return fir::NameUniquer::doType(modules, /*proc=*/{}, /*blockId=*/0,
                                  language::Compability::semantics::typeDescriptorTypeName,
                                  /*kinds=*/{});
}
...
private:
  ...
  toolchain::LogicalResult genTypeLadderStep(mlir::Location loc,
                                        mlir::Value selector,
  ...
  toolchain::SmallSet<toolchain::StringRef, 4> collectAncestors(fir::TypeInfoOp dt,
...
struct DispatchOpConv : public OpConversionPattern<fir::DispatchOp> {
  ...
  matchAndRewrite(fir::DispatchOp dispatch, OpAdaptor adaptor,
    ...
    auto bindingsCompName = language::Compability::semantics::bindingDescCompName;
    ...
    auto procCompName = language::Compability::semantics::procCompName;
    ...
    auto addressFieldName = language::Compability::lower::builtin::cptrFieldName;
    ...
    // Make the call.
    toolchain::SmallVector<mlir::Value> args{funcPtr};
...
public:
  toolchain::LogicalResult initialize(mlir::MLIRContext *ctx) override {
    return mlir::success();
  }
...
toolchain::LogicalResult SelectTypeConv::matchAndRewrite(
  ...
  toolchain::SmallVector<unsigned> orderedTypeGuards;
  toolchain::SmallVector<unsigned> orderedClassIsGuards;
  ...
  for (unsigned t = 0; t < typeGuardNum; ++t) {
    ...
    if (auto a = mlir::dyn_cast<fir::SubclassAttr>(typeGuards[t])) {
      if (auto recTy = mlir::dyn_cast<fir::RecordType>(a.getType())) {
        ...
        toolchain::SmallSet<toolchain::StringRef, 4> ancestors =
...
toolchain::LogicalResult SelectTypeConv::genTypeLadderStep(
  ...
  if (auto a = mlir::dyn_cast<fir::ExactTypeAttr>(attr)) {
    ...
    toolchain::StringRef fctName = RTNAME_STRING(ClassIs);
...
toolchain::SmallSet<toolchain::StringRef, 4>
