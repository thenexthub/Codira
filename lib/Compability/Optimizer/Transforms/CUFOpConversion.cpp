...
#include "language/Compability/Optimizer/Transforms/CUFOpConversion.h"
...
#include "language/Compability/Runtime/CUDA/allocatable.h"
...
#include "language/Compability/Runtime/CUDA/pointer.h"
...
static inline unsigned getMemType(cuf::DataAttribute attr) {
  ...
  toolchain::report_fatal_error("unsupported memory type");
}
    ...
    if (!mlir::isa<fir::BaseBoxType>(srcTy) &&
      ...
      if (op.getShape()) {
        ...
        if (auto shapeOp =
          ...
          for (auto i : toolchain::enumerate(shapeShiftOp.getPairs()))
            if (i.index() & 1)
