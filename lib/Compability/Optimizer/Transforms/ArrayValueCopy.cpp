...
using OperationUseMapT = toolchain::DenseMap<mlir::Operation *, mlir::Operation *>;
...
public:
  using ConflictSetT = toolchain::SmallPtrSet<mlir::Operation *, 16>;
  using UseSetT = toolchain::SmallPtrSet<mlir::OpOperand *, 8>;
  using LoadMapSetsT = toolchain::DenseMap<mlir::Operation *, UseSetT>;
  using AmendAccessSetT = toolchain::SmallPtrSet<mlir::Operation *, 4>;
  ...
  void arrayMentions(toolchain::SmallVectorImpl<mlir::Operation *> &mentions,
                     ArrayLoadOp load);
...
public:
  ReachCollector(toolchain::SmallVectorImpl<mlir::Operation *> &reach,
  ...
  // reach them.
  void collectAccesses(toolchain::SmallVector<ArrayAccessOp> &result,
                       mlir::Block *block) {
    for (auto &op : *block) {
      if (auto access = mlir::dyn_cast<ArrayAccessOp>(op)) {
        LLVM_DEBUG(toolchain::dbgs() << "adding access: " << access << '\n');
  ...
  void collectArrayMentionFrom(mlir::Operation *op, mlir::Value val) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "popset: " << *op << '\n');
    auto popFn = [&](auto rop) {
    ...
    if (auto amend = mlir::dyn_cast<ArrayAmendOp>(op)) {
      reach.push_back(op);
      toolchain::SmallVector<ArrayAccessOp> accesses;
    ...
    if (mlir::isa<ArrayAccessOp, ArrayLoadOp, ArrayUpdateOp, ArrayModifyOp,
                  ArrayFetchOp>(op)) {
      LLVM_DEBUG(toolchain::dbgs() << "add " << *op << " to reachable set\n");
      reach.push_back(op);
    }
    ...
    if (auto arrLd = mlir::dyn_cast<ArrayLoadOp>(op))
      for (auto *user : arrLd.getResult().getUsers())
        if (mlir::isa<ArrayAccessOp>(user)) {
          LLVM_DEBUG(toolchain::dbgs() << "add " << *user << " to reachable set\n");
  ...
  /// `array_merge_store`.
  static void reachingValues(toolchain::SmallVectorImpl<mlir::Operation *> &reach,
                             mlir::Value seq) {
    reach.clear();
...
private:
  ...
  toolchain::SmallVectorImpl<mlir::Operation *> &reach;
  toolchain::SmallPtrSet<mlir::Value, 16> visited;
...
void ArrayCopyAnalysisBase::arrayMentions(
    toolchain::SmallVectorImpl<mlir::Operation *> &mentions, ArrayLoadOp load) {
  mentions.clear();
  ...
  UseSetT visited;
  toolchain::SmallVector<mlir::OpOperand *> queue; // uses of ArrayLoad[orig]
  ...
  while (!queue.empty()) {
    ...
    if (auto ro = mlir::dyn_cast<DoLoopOp>(owner)) {
      ...
      toolchain::report_fatal_error("array value reached unexpected op");
    }
  }
...
static bool mutuallyExclusiveSliceRange(ArrayLoadOp ld, ArrayMergeStoreOp st) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "detected non-overlapping slice ranges on " << ld
...
static bool conflictOnLoad(toolchain::ArrayRef<mlir::Operation *> reach,
...
static bool conflictOnMerge(toolchain::ArrayRef<mlir::Operation *> mentions) {
  if (mentions.size() < 2)
    return false;
  toolchain::SmallVector<mlir::Value> indices;
  LLVM_DEBUG(toolchain::dbgs() << "check merge conflict on with " << mentions.size()
                          << " mentions on the list\n");
  bool valSeen = false;
  bool refSeen = false;
  for (auto *op : mentions) {
    toolchain::SmallVector<mlir::Value> compareVector;
    ...
    if (compareVector.size() != indices.size() ||
        toolchain::any_of(toolchain::zip(compareVector, indices), [&](auto pair) {
          return std::get<0>(pair) != std::get<1>(pair);
        }))
      return true;
    LLVM_DEBUG(toolchain::dbgs() << "vectors compare equal\n");
  }
...
static bool conflictOnReference(toolchain::ArrayRef<mlir::Operation *> mentions) {
  LLVM_DEBUG(toolchain::dbgs() << "checking reference semantics " << mentions.size()
                          << '\n');
  if (mentions.size() < 3)
    return false;
  ...
  for (auto *op : mentions) {
    if (mlir::isa<ArrayAmendOp>(op) && ++amendCount > 1) {
      LLVM_DEBUG(toolchain::dbgs() << "conflict: multiple amends of array value\n");
      return true;
    }
...
amendingAccess(toolchain::ArrayRef<mlir::Operation *> mentions) {
  for (auto *op : mentions)
    if (auto amend = mlir::dyn_cast<ArrayAmendOp>(op))
...
static bool conflictDetected(toolchain::ArrayRef<mlir::Operation *> reach,
                             toolchain::ArrayRef<mlir::Operation *> mentions,
                             ArrayMergeStoreOp st, bool optimize) {
...
static bool
conservativeCallConflict(toolchain::ArrayRef<mlir::Operation *> reaches) {
  return toolchain::any_of(reaches, [](mlir::Operation *op) {
...
void ArrayCopyAnalysisBase::construct(mlir::Operation *topLevelOp) {
  topLevelOp->walk([&](Operation *op) {
    if (auto st = mlir::dyn_cast<fir::ArrayMergeStoreOp>(op)) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "process load: " << load
                              << ", mentions: " << mentions.size() << '\n');
      for (auto *acc : mentions) {
        LLVM_DEBUG(toolchain::dbgs() << " mention: " << *acc << '\n');
        if (mlir::isa<ArrayAccessOp, ArrayAmendOp, ArrayFetchOp, ArrayUpdateOp,
                      ArrayModifyOp>(acc)) {
          if (useMap.count(acc)) {
            mlir::emitError(
...
public:
  ...
  matchAndRewrite(ArrayLoadOp load,
    ...
    LLVM_DEBUG(toolchain::dbgs() << "replace load " << load << " with undef.\n");
...
public:
  ...
  matchAndRewrite(ArrayMergeStoreOp store,
                  mlir::PatternRewriter &rewriter) const override {
    LLVM_DEBUG(toolchain::dbgs() << "marking store " << store << " as dead.\n");
...
static bool isAssumedSize(toolchain::SmallVectorImpl<mlir::Value> &extents) {
  if (extents.empty())
    return false;
...
static mlir::Value getOrReadExtentsAndShapeOp(
    ...
    toolchain::SmallVectorImpl<mlir::Value> &result, bool &copyUsingSlice) {
  assert(result.empty());
  ...
  if (auto boxTy = mlir::dyn_cast<BoxType>(arrLoad.getMemref().getType())) {
    ...
    toolchain::SmallVector<mlir::Value> shapeShiftOperands;
    for (auto [lb, extent] : toolchain::zip(shiftOp.getOrigins(), result)) {
...
static mlir::Value genCoorOp(mlir::PatternRewriter &rewriter,
                             ...
                             bool skipOrig = false) {
  toolchain::SmallVector<mlir::Value> originated;
  if (skipOrig)
  ...
  mlir::Value result = ArrayCoorOp::create(
      ...
      toolchain::ArrayRef<mlir::Value>{originated}.take_front(dimension),
      typeparams);
  if (dimension < originated.size())
    result = fir::CoordinateOp::create(
        rewriter, loc, resTy, result,
        toolchain::ArrayRef<mlir::Value>{originated}.drop_front(dimension));
  return result;
}
...
void genArrayCopy(mlir::Location loc, mlir::PatternRewriter &rewriter,
  ...
  for (auto sh : toolchain::reverse(extents)) {
...
findNonconstantExtents(mlir::Type memrefTy,
  ...
  toolchain::SmallVector<mlir::Value> nce;
  auto arrTy = unwrapPassByRefType(memrefTy);
  auto seqTy = mlir::cast<SequenceType>(arrTy);
  for (auto [s, x] : toolchain::zip(seqTy.getShape(), extents))
...
allocateArrayTemp(mlir::Location loc, mlir::PatternRewriter &rewriter,
                  ArrayLoadOp load, toolchain::ArrayRef<mlir::Value> extents,
                  mlir::Value shape) {
  mlir::Type baseType = load.getMemref().getType();
  toolchain::SmallVector<mlir::Value> nonconstantExtents =
...
public:
  ...
  materializeAssignment(mlir::Location loc, mlir::PatternRewriter &rewriter,
    ...
    auto *loadOp = useMap.lookup(op);
    auto load = mlir::cast<ArrayLoadOp>(loadOp);
    LLVM_DEBUG(toolchain::outs() << "does " << load << " have a conflict?\n");
    if (analysis.hasPotentialConflict(loadOp)) {
      ...
      LLVM_DEBUG(toolchain::outs() << "Yes, conflict was found\n");
      rewriter.setInsertionPoint(loadOp);
      // Copy in.
      toolchain::SmallVector<mlir::Value> extents;
      bool copyUsingSlice = false;
    ...
    LLVM_DEBUG(toolchain::outs() << "No, conflict wasn't found\n");
    rewriter.setInsertionPoint(op);
