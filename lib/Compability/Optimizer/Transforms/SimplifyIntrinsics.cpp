...
#include "language/Compability/Optimizer/Transforms/Utils.h"
...
class SimplifyIntrinsicsPass
  ...
  using FunctionTypeGeneratorTy =
      toolchain::function_ref<mlir::FunctionType(fir::FirOpBuilder &)>;
  using FunctionBodyGeneratorTy =
      toolchain::function_ref<void(fir::FirOpBuilder &, mlir::func::FuncOp &)>;
  using GenReductionBodyTy = toolchain::function_ref<void(
...
Op expectOp(mlir::Value val) {
  if (Op op = mlir::dyn_cast_or_null<Op>(val.getDefiningOp()))
    return op;
  LLVM_DEBUG(toolchain::dbgs() << "Didn't find expected " << Op::getOperationName()
                          << '\n');
  return nullptr;
}
...
using BodyOpGeneratorTy = toolchain::function_ref<mlir::Value(
    fir::FirOpBuilder &, mlir::Location, const mlir::Type &, mlir::Value,
    mlir::Value)>;
using ContinueLoopGenTy = toolchain::function_ref<toolchain::SmallVector<mlir::Value>(
...
genReductionLoop(fir::FirOpBuilder &builder, mlir::func::FuncOp &funcOp,
  ...
  toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> bounds;
  ...
  // DoLoopOp loops.
  toolchain::SmallVector<mlir::Value> results = loopCond(builder, loc, reductionVal);
...
static toolchain::SmallVector<mlir::Value> nopLoopCond(fir::FirOpBuilder &builder,
                                                  mlir::Location loc,
...
static void genRuntimeMaxvalBody(fir::FirOpBuilder &builder,
  ...
  auto init = [](fir::FirOpBuilder builder, mlir::Location loc,
    ...
    if (auto ty = mlir::dyn_cast<mlir::FloatType>(elementType)) {
      ...
      return builder.createRealConstant(
          loc, elementType, toolchain::APFloat::getLargest(sem, /*Negative=*/true));
    }
    unsigned bits = elementType.getIntOrFloatBitWidth();
    int64_t minInt = toolchain::APInt::getSignedMinValue(bits).getSExtValue();
  ...
  auto genBodyOp = [](fir::FirOpBuilder builder, mlir::Location loc,
                      ...
                      mlir::Value elem2) -> mlir::Value {
    if (mlir::isa<mlir::FloatType>(elementType)) {
      // arith.maxf later converted to toolchain.intr.maxnum does not work
      // correctly for NaNs and -0.0 (see maxnum/minnum pattern matching
      // in LLVM's InstCombine pass). Moreover, toolchain.intr.maxnum
...
static void genRuntimeCountBody(fir::FirOpBuilder &builder,
  ...
  auto zero = [](fir::FirOpBuilder builder, mlir::Location loc,
                 mlir::Type elementType) {
    unsigned bits = elementType.getIntOrFloatBitWidth();
    int64_t zeroInt = toolchain::APInt::getZero(bits).getSExtValue();
...
static void genRuntimeAnyBody(fir::FirOpBuilder &builder,
  ...
  auto continueCond = [](fir::FirOpBuilder builder, mlir::Location loc,
    ...
    toolchain::SmallVector<mlir::Value> results = {eor, reductionVal};
    return results;
  };
...
static void genRuntimeAllBody(fir::FirOpBuilder &builder,
  ...
  auto continueCond = [](fir::FirOpBuilder builder, mlir::Location loc,
                         mlir::Value reductionVal) {
    toolchain::SmallVector<mlir::Value> results = {reductionVal, reductionVal};
    return results;
  };
...
static void genRuntimeMinMaxlocBody(fir::FirOpBuilder &builder,
  ...
  auto init = [isMax](fir::FirOpBuilder builder, mlir::Location loc,
    ...
    if (auto ty = mlir::dyn_cast<mlir::FloatType>(elementType)) {
      const toolchain::fltSemantics &sem = ty.getFloatSemantics();
      toolchain::APFloat limit = toolchain::APFloat::getInf(sem, /*Negative=*/isMax);
      return builder.createRealConstant(loc, elementType, limit);
    }
    unsigned bits = elementType.getIntOrFloatBitWidth();
    int64_t initValue = (isMax ? toolchain::APInt::getSignedMinValue(bits)
                               : toolchain::APInt::getSignedMaxValue(bits))
                            .getSExtValue();
  ...
  auto genBodyOp =
      [&rank, &resultArr, isMax, &mask, &maskElemType, &maskRank](
          ...
          const toolchain::SmallVectorImpl<mlir::Value> &indices) -> mlir::Value {
    // We are in the innermost loop: generate the reduction body.
    if (maskRank > 0) {
      ...
      toolchain::SmallVector<mlir::Type> resultsTy = {elementType, elementType};
      fir::IfOp ifOp =
...
mlir::func::FuncOp SimplifyIntrinsicsPass::getOrCreateFunction(
  ...
  newFunc->setAttr("toolchain.linkage", linkage);
    ...
    if (auto call = mlir::dyn_cast<fir::CallOp>(op)) {
      ...
      if (mlir::SymbolRefAttr callee = call.getCalleeAttr()) {
        ...
        if (funcName.starts_with(RTNAME_STRING(DotProduct))) {
          LLVM_DEBUG(toolchain::dbgs() << "Handling " << funcName << "\n");
          LLVM_DEBUG(toolchain::dbgs() << "Call operation:\n"; op->dump();
                     toolchain::dbgs() << "\n");
          mlir::Operation::operand_range args = call.getArgs();
          ...
          toolchain::raw_string_ostream nameOS(typedFuncName);
          // We must mangle the generated function name with FastMathFlags
          // value.
          if (!fmfString.empty())
            nameOS << '_' << fmfString;
          ...
          LLVM_DEBUG(toolchain::dbgs() << "Replaced with:\n"; newCall.dump();
                     toolchain::dbgs() << "\n");
          return;
        }
