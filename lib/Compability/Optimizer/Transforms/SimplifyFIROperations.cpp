...
public:
  ...
  matchAndRewrite(fir::DoConcurrentOp doConcurentOp,
    ...
    if (!loop.getLocalVars().empty()) {
      ...
      for (auto localInfo : toolchain::zip_equal(
        ...
        mlir::SymbolRefAttr localizerName =
            toolchain::cast<mlir::SymbolRefAttr>(localizerSym);
    ...
    for (auto [reduceVar, reduceArg] :
         toolchain::zip_equal(loop.getReduceVars(), loop.getRegionReduceArgs()))
    ...
    for (mlir::Operation &op : toolchain::drop_end(wrapperBlock))
    ...
    for (auto [lb, ub, st, iv] :
         toolchain::zip_equal(loop.getLowerBound(), loop.getUpperBound(),
