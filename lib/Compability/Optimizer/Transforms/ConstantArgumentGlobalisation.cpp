...
public:
  ...
  matchAndRewrite(fir::CallOp callOp,
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Processing call op: " << callOp << "\n");
    auto module = callOp->getParentOfType<mlir::ModuleOp>();
    bool needUpdate = false;
    fir::FirOpBuilder builder(rewriter, module);
    toolchain::SmallVector<mlir::Value> newOperands;
    toolchain::SmallVector<std::pair<mlir::Operation *, mlir::Operation *>> allocas;
    for (const mlir::Value &a : callOp.getArgs()) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << " found store " << *store << "\n");
      ...
      LLVM_DEBUG(toolchain::dbgs() << " found define " << *definingOp << "\n");
    ...
    if (needUpdate) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "global constant for " << callOp << " as "
                              << newOp << '\n');
      return mlir::success();
    }
