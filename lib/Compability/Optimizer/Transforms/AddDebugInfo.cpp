...
private:
  toolchain::StringMap<mlir::LLVM::DIModuleAttr> moduleMap;
  toolchain::StringMap<mlir::LLVM::DICommonBlockAttr> commonBlockMap;
  // List of GlobalVariableExpressionAttr that are attached to a given global
  // that represents the storage for common block.
  toolchain::DenseMap<fir::GlobalOp, toolchain::SmallVector<mlir::Attribute>>
  ...
  getOrCreateCommonBlockAttr(toolchain::StringRef name,
                             mlir::LLVM::DIFileAttr fileAttr,
...
mlir::StringAttr getTargetFunctionName(mlir::MLIRContext *context,
  ...
  toolchain::StringRef fileName = fileLoc.getFilename().getValue();
  ...
  if (auto ec = toolchain::sys::fs::getUniqueID(fileName, id)) {
    fileId = toolchain::hash_value(fileName.str());
    deviceId = 0xdeadf17e;
  } else {
    fileId = id.getFile();
  ...
  return mlir::StringAttr::get(
      context,
      std::string(toolchain::formatv("__omp_offloading_{0:x-}_{1:x-}_{2}_l{3}",
  ...
  if (auto cordOp = mlir::dyn_cast_if_present<fir::CoordinateOp>(op)) {
    ...
    if (auto addrOfOp = mlir::dyn_cast_if_present<fir::AddrOfOp>(op)) {
      ...
      if (auto global =
        ...
        toolchain::StringRef commonName(sym.getRootReference());
        ...
        if (commonName != language::Compability::common::blankCommonObjectName &&
            commonName.back() == '_')
        ...
        if (*optint != 0) {
          ...
          ops.push_back(mlir::LLVM::DIExpressionElemAttr::get(
              context, toolchain::dwarf::DW_OP_plus_uconst, *optint));
...
void AddDebugInfoPass::handleDeclareOp(fir::cg::XDeclareOp declOp,
  ...
  if (defOp && toolchain::isa<fir::AddrOfOp>(defOp)) {
    if (auto global =
  ...
  if (declOp.getDummyScope()) {
    if (auto arg = toolchain::dyn_cast<mlir::BlockArgument>(declOp.getMemref())) {
...
mlir::LLVM::DICommonBlockAttr AddDebugInfoPass::getOrCreateCommonBlockAttr(
    toolchain::StringRef name, mlir::LLVM::DIFileAttr fileAttr,
    mlir::LLVM::DIScopeAttr scope, unsigned line) {
  mlir::MLIRContext *context = &getContext();
...
void AddDebugInfoPass::handleFuncOp(mlir::func::FuncOp funcOp,
  ...
  mlir::MLIRContext *context = &getContext();
  mlir::OpBuilder builder(context);
  toolchain::StringRef fileName(fileAttr.getName());
  toolchain::StringRef filePath(fileAttr.getDirectory());
  unsigned int CC = (funcOp.getName() == fir::NameUniquer::doProgramEntry())
                        ? toolchain::dwarf::getCallingConvention("DW_CC_program")
                        : toolchain::dwarf::getCallingConvention("DW_CC_normal");

  if (auto funcLoc = mlir::dyn_cast<mlir::FileLineColLoc>(l)) {
    fileName = toolchain::sys::path::filename(funcLoc.getFilename().getValue());
    filePath = toolchain::sys::path::parent_path(funcLoc.getFilename().getValue());
  }
  ...
  toolchain::SmallVector<mlir::LLVM::DITypeAttr> types;
  ...
  auto addTargetOpDISP = [&](bool lineTableOnly,
                             toolchain::ArrayRef<mlir::LLVM::DINodeAttr> entities) {
    ...
    // the scope. In the codegen (to toolchain ir), OpenMP target op results in the
    ...
    funcOp.walk([&](mlir::omp::TargetOp targetOp) {
      ...
      CC = toolchain::dwarf::getCallingConvention("DW_CC_normal");
      ...
      // new function.
      toolchain::SmallVector<mlir::LLVM::DINodeAttr> opEntities;
      for (mlir::LLVM::DINodeAttr N : entities) {
        if (auto entity = mlir::dyn_cast<mlir::LLVM::DIImportedEntityAttr>(N)) {
          auto importedEntity = mlir::LLVM::DIImportedEntityAttr::get(
              context, toolchain::dwarf::DW_TAG_imported_module, spAttr,
  ...
  toolchain::DenseSet<mlir::LLVM::DIImportedEntityAttr> importedModules;
  funcOp.walk([&](fir::cg::XDeclareOp declOp) {
    if (&funcOp.front() == declOp->getBlock())
      if (auto global =
        ...
        if (modOpt) {
          auto importedEntity = mlir::LLVM::DIImportedEntityAttr::get(
              context, toolchain::dwarf::DW_TAG_imported_module, spAttr, *modOpt,
              fileAttr, /*line=*/1, /*name=*/nullptr, /*elements*/ {});
          importedModules.insert(importedEntity);
        }
      }
  });
  toolchain::SmallVector<mlir::LLVM::DINodeAttr> entities(importedModules.begin(),
                                                     importedModules.end());
  ...
  funcOp.walk([&](fir::cg::XDeclareOp declOp) {
    ...
    if (auto tOp = declOp->getParentOfType<mlir::omp::TargetOp>()) {
      if (auto fusedLoc = toolchain::dyn_cast<mlir::FusedLoc>(tOp.getLoc())) {
        if (auto sp = toolchain::dyn_cast<mlir::LLVM::DISubprogramAttr>(
                fusedLoc.getMetadata()))
          spTy = sp;
      }
    }
...
void AddDebugInfoPass::runOnOperation() {
  ...
  if (inputFilename.empty()) {
    if (auto fileLoc = mlir::dyn_cast<mlir::FileLineColLoc>(module.getLoc())) {
      fileName = toolchain::sys::path::filename(fileLoc.getFilename().getValue());
      filePath = toolchain::sys::path::parent_path(fileLoc.getFilename().getValue());
    } else
      fileName = "-";
  } else {
  ...
  mlir::StringAttr producer =
      mlir::StringAttr::get(context, language::Compability::common::getFlangFullVersion());
  mlir::LLVM::DICompileUnitAttr cuAttr = mlir::LLVM::DICompileUnitAttr::get(
      mlir::DistinctAttr::create(mlir::UnitAttr::get(context)),
      toolchain::dwarf::getLanguage("DW_LANG_Fortran95"), fileAttr, producer,
