...
struct ArgsUsageInLoop {
  ...
  toolchain::DenseMap<mlir::Value, ArgInfo> usageInfo;
  ...
  /// in this loop.
  toolchain::SetVector<mlir::Value> cannotTransform;
  ...
  void dump(fir::DoLoopOp loop) const {
    LLVM_DEBUG({
      ...
      toolchain::dbgs() << "Arguments usage info for loop:\n";
      loop.print(toolchain::dbgs(), printFlags);
      toolchain::dbgs() << "\nUsed args:\n";
      for (auto &use : usageInfo) {
        mlir::Value v = use.first;
        v.print(toolchain::dbgs(), printFlags);
        toolchain::dbgs() << "\n";
      }
      toolchain::dbgs() << "\nCannot transform args:\n";
      for (mlir::Value arg : cannotTransform) {
        arg.print(toolchain::dbgs(), printFlags);
        toolchain::dbgs() << "\n";
      }
      toolchain::dbgs() << "====\n";
    });
  ...
  // of given arguments.
  void eraseUsage(const toolchain::SetVector<mlir::Value> &args) {
    for (auto &arg : args)
      usageInfo.erase(arg);
  ...
  void eraseUsage(const toolchain::DenseMap<mlir::Value, ArgInfo> &args) {
    for (auto &arg : args) {
...
getRankAndElementSize(const fir::KindMapping &kindMap,
  ...
  if (auto seqTy = getAsSequenceType(v)) {
    ...
    if (rank > 0 &&
      ...
      if (fir::isa_trivial(elementType)) {
        ...
        typeSize = toolchain::alignTo(eleSize, eleAlign);
      }
      if (typeSize)
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Unsupported rank/type: " << v << '\n');
  return {0, 0};
}
...
static mlir::Value unwrapReboxOp(mlir::Value val) {
  while (fir::ReboxOp rebox = val.getDefiningOp<fir::ReboxOp>()) {
    if (!fir::reboxPreservesContinuity(rebox,
                                       ...
                                       /*checkWhole=*/false)) {
      LLVM_DEBUG(toolchain::dbgs() << "REBOX may produce non-contiguous array: "
                              << rebox << '\n');
      break;
    }
    val = rebox.getBox();
...
void LoopVersioningPass::runOnOperation() {
  ...
  for (auto &arg : args) {
    ...
    if (func.getArgAttrOfType<mlir::UnitAttr>(arg.getArgNumber(),
      ...
      LLVM_DEBUG(toolchain::dbgs() << "OPTIONAL is not supported\n");
      continue;
    }
  ...
  toolchain::DenseMap<fir::DoLoopOp, ArgsUsageInLoop> argsInLoops;
  ...
  LLVM_DEBUG({
    toolchain::dbgs() << "Initial usage info:\n";
  ...
  LLVM_DEBUG({
    toolchain::dbgs() << "Final usage info:\n";
    for (fir::DoLoopOp loop : originalLoops) {
      auto &argsInLoop = argsInLoops[loop];
      argsInLoop.dump(loop);
    }
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Func Before transformation:\n");
  LLVM_DEBUG(func->dump());

  LLVM_DEBUG(toolchain::dbgs() << "loopsOfInterest: " << loopsOfInterest.size()
                          << "\n");
  for (auto op : loopsOfInterest) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Creating cloned loop\n");
    ...
    for (auto &arg : op.argsAndDims) {
      ...
      clonedLoop->walk([&](mlir::Operation *coop) {
        ...
        if (coop->getOperand(0) == arg.arg && coop->getOperands().size() >= 2) {
          ...
          if (totalIndex) {
            assert(toolchain::isPowerOf2_32(arg.size) &&
                   "Expected power of two here");
            unsigned bits = toolchain::Log2_32(arg.size);
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Func After transform:\n");
  LLVM_DEBUG(func->dump());
