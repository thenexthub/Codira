...
hlfir::getExplicitExtentsFromShape(mlir::Value shape,
  ...
  if (auto s = mlir::dyn_cast_or_null<fir::ShapeOp>(shapeOp)) {
    ...
    toolchain::ArrayRef<int64_t> exprShape = expr.getShape();
...
getNonDefaultLowerBounds(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  if (auto varIface = entity.getIfVariableInterface()) {
    toolchain::SmallVector<mlir::Value> lbounds = getExplicitLbounds(varIface);
    if (!lbounds.empty())
      return lbounds;
  }
...
static toolchain::SmallVector<mlir::Value> toSmallVector(mlir::ValueRange range) {
  toolchain::SmallVector<mlir::Value> res;
  res.append(range.begin(), range.end());
  return res;
}

static toolchain::SmallVector<mlir::Value> getExplicitTypeParams(hlfir::Entity var) {
...
mlir::Operation *traverseConverts(mlir::Operation *op) {
  while (auto convert = toolchain::dyn_cast_or_null<fir::ConvertOp>(op))
...
bool hlfir::Entity::mayBeOptional() const {
  ...
  if (mlir::Operation *op = traverseConverts(getDefiningOp())) {
    if (auto varIface = toolchain::dyn_cast<fir::FortranVariableOpInterface>(op))
      return varIface.isOptional();
    return !toolchain::isa<fir::AllocaOp, fir::AllocMemOp, fir::ReboxOp,
...
hlfir::genDeclare(mlir::Location loc, fir::FirOpBuilder &builder,
                  const fir::ExtendedValue &exv, toolchain::StringRef name,
...
static hlfir::Entity changeBoxAttributes(mlir::Location loc,
  ...
  assert(toolchain::isa<fir::BaseBoxType>(var.getType()) && "expect box type");
...
hlfir::Entity hlfir::getElementAt(mlir::Location loc,
  ...
  if (!lbounds.empty()) {
    ...
    for (auto [oneBased, lb] : toolchain::zip(oneBasedIndices, lbounds)) {
...
toolchain::SmallVector<std::pair<mlir::Value, mlir::Value>>
hlfir::genBounds(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  mlir::Type idxTy = builder.getIndexType();
  mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
  toolchain::SmallVector<std::pair<mlir::Value, mlir::Value>> result;
...
hlfir::genBounds(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
  toolchain::SmallVector<std::pair<mlir::Value, mlir::Value>> result;
  for (auto extent : toolchain::enumerate(extents)) {
...
toolchain::SmallVector<mlir::Value> hlfir::genLowerbounds(mlir::Location loc,
                                                     fir::FirOpBuilder &builder,
                                                     mlir::Value shape,
                                                     unsigned rank) {
  toolchain::SmallVector<mlir::Value> lbounds;
  ...
  return toolchain::SmallVector<mlir::Value>(rank, one);
}
...
toolchain::SmallVector<mlir::Value> getVariableExtents(mlir::Location loc,
                                                  fir::FirOpBuilder &builder,
                                                  hlfir::Entity variable) {
...
mlir::Value hlfir::genCharLength(mlir::Location loc, fir::FirOpBuilder &builder,
                                 hlfir::Entity entity) {
  toolchain::SmallVector<mlir::Value, 1> lenParams;
  genLengthParameters(loc, builder, entity, lenParams);
...
std::pair<mlir::Value, mlir::Value> hlfir::genVariableFirBaseShapeAndParams(
    ...
    toolchain::SmallVectorImpl<mlir::Value> &typeParams) {
  auto [exv, cleanup] = translateToExtendedValue(loc, builder, entity);
...
static hlfir::ExprType getArrayExprType(mlir::Type elementType,
  ...
  if (auto shapeOp = shape.getDefiningOp<fir::ShapeOp>())
    for (auto extent : toolchain::enumerate(shapeOp.getExtents()))
    ...
    if (auto apply = mlir::dyn_cast<hlfir::ApplyOp>(op))
      if (auto appliedElemental =
        ...
        if (mustRecursivelyInline(appliedElemental)) {
          toolchain::SmallVector<mlir::Value> clonedApplyIndices;
...
hlfir::LoopNest hlfir::genLoopNest(mlir::Location loc,
  ...
  if (emitWorkshareLoop) {
    ...
    for (auto extent : toolchain::reverse(extents)) {
...
toolchain::SmallVector<mlir::Value> hlfir::genLoopNestWithReductions(
  ...
  toolchain::SmallVector<mlir::Value> oneBasedIndices;
  ...
  toolchain::SmallVector<mlir::Value> reductionValues;
  reductionValues =
...
conditionallyEvaluate(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  toolchain::SmallVector<mlir::Value> absentValues;
...
static fir::ExtendedValue translateVariableToExtendedValue(
  ...
  if (mlir::isa<fir::BaseBoxType>(base.getType())) {
    ...
    if (!contiguous || variable.isPolymorphic() ||
      ...
      toolchain::SmallVector<mlir::Value> nonDefaultLbounds;
      if (!isAssumedRank)
...
hlfir::createTempFromMold(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  toolchain::StringRef tmpName{".tmp"};
...
hlfir::genTypeAndKindConvert(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  if (preserveLowerBounds && source.mayHaveNonDefaultLowerBounds()) {
    ...
    toolchain::SmallVector<mlir::Value> lbAndExtents;
    for (unsigned dim = 0; dim < rank; ++dim) {
...
std::pair<hlfir::Entity, bool> hlfir::computeEvaluateOpInNewTemp(
  ...
  toolchain::StringRef tmpName{".tmp.expr_result"};
  toolchain::SmallVector<mlir::Value> extents =
...
toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank>
hlfir::genExtentsVector(mlir::Location loc, fir::FirOpBuilder &builder,
                        hlfir::Entity entity) {
  entity = hlfir::derefPointersAndAllocatables(loc, builder, entity);
  mlir::Value shape = hlfir::genShape(loc, builder, entity);
  toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> extents =
      hlfir::getExplicitExtentsFromShape(shape, builder);
...
bool hlfir::designatePreservesContinuity(hlfir::DesignateOp op) {
  ...
  unsigned i = 0;
  for (auto isTriplet : toolchain::enumerate(op.getIsTriplet())) {
