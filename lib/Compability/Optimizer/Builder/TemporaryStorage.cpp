...
fir::factory::HomogeneousScalarStack::HomogeneousScalarStack(
    ...
    toolchain::ArrayRef<mlir::Value> lengths, bool allocateOnHeap,
    bool stackThroughLoops, toolchain::StringRef tempName)
...
void fir::factory::AnyAddressStack::pushValue(mlir::Location loc,
  ...
  if (auto boxProcType = toolchain::dyn_cast<fir::BoxProcType>(variable.getType())) {
    cast =
...
mlir::Value fir::factory::AnyAddressStack::fetch(mlir::Location loc,
  ...
  if (auto boxProcType = toolchain::dyn_cast<fir::BoxProcType>(addressType)) {
