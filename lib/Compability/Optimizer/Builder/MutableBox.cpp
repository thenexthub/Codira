...
createNewFirBox(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  if (!extents.empty()) {
    if (lbounds.empty()) {
      ...
      toolchain::SmallVector<mlir::Value> shapeShiftBounds;
      for (auto [lb, extent] : toolchain::zip(lbounds, extents)) {
  ...
  // error in the embox).
  toolchain::SmallVector<mlir::Value> cleanedLengths;
...
public:
  ...
  readShape(toolchain::SmallVectorImpl<mlir::Value> *lbounds = nullptr) {
  ...
  mlir::Value read(toolchain::SmallVectorImpl<mlir::Value> &lbounds,
                   toolchain::SmallVectorImpl<mlir::Value> &extents,
                   toolchain::SmallVectorImpl<mlir::Value> &lengths) {
  ...
  void getLowerBounds(toolchain::SmallVectorImpl<mlir::Value> &lbounds) {
    auto rank = box.rank();
    for (decltype(rank) dim = 0; dim < rank; ++dim)
...
private:
  ...
  void updateMutableProperties(mlir::Value addr, mlir::ValueRange lbounds,
    ...
    for (auto [extent, extentVar] :
         toolchain::zip(extents, mutableProperties.extents))
    ...
    if (!mutableProperties.lbounds.empty()) {
      if (lbounds.empty()) {
        ...
        for (auto [lbound, lboundVar] :
             toolchain::zip(lbounds, mutableProperties.lbounds))
          castAndStore(lbound, lboundVar);
      }
    }
    if (box.isCharacter())
      // toolchain::zip account for the fact that the length only needs to be stored
      // when it is specified in the allocation and deferred in the
      // MutableBoxValue.
      for (auto [len, lenVar] :
           toolchain::zip(lengths, mutableProperties.deferredParams))
...
mlir::Value fir::factory::createUnallocatedBox(
  ...
  if (auto seqTy = mlir::dyn_cast<fir::SequenceType>(type)) {
    ...
    toolchain::SmallVector<mlir::Value> extents(seqTy.getDimension(), zero);
...
fir::MutableBoxValue fir::factory::createTempMutableBox(
    ...
    toolchain::StringRef name, mlir::Value typeSourceBox, bool isPolymorphic) {
  mlir::Type boxType;
...
void fir::factory::associateMutableBoxWithRemap(
  ...
  if (!lbounds.empty()) {
    ...
    for (auto [lb, ub] : toolchain::zip(lbounds, ubounds)) {
  ...
  source.match(
      ...
      [&](const fir::BoxValue &arr) {
        ...
        if (box.isDescribedByVariables()) {
          ...
          auto idxTy = builder.getIndexType();
          for (auto [lbnd, ext] : toolchain::zip(lbounds, extents)) {
...
void fir::factory::genInlinedAllocation(
    ...
    toolchain::StringRef allocName, bool mustBeHeap) {
  auto lengths = getNewLengths(builder, loc, box, lenParams);
  toolchain::SmallVector<mlir::Value> safeExtents;
  ...
  auto ifOp =
      builder
          ...
          .genThen([&]() {
            ...
            toolchain::SmallVector<mlir::Value> previousExtents = reader.readShape();
            if (!shape.empty())
              for (auto [previousExtent, requested] :
                   toolchain::zip(previousExtents, shape))
...
void fir::factory::finalizeRealloc(fir::FirOpBuilder &builder,
  ...
  builder.genIfThen(loc, realloc.wasReallocated)
      .genThen([&]() {
        auto reader = MutablePropertyReader(builder, loc, box);
        toolchain::SmallVector<mlir::Value> previousLbounds;
...
mlir::Value fir::factory::getAndEstablishBoxStorage(
    ...
    mlir::Value shape, toolchain::ArrayRef<mlir::Value> typeParams,
