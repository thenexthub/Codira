...
static fir::CharacterType recoverCharacterType(mlir::Type type) {
  ...
  if (auto charTy = mlir::dyn_cast<fir::CharacterType>(type))
    return charTy;
  toolchain::report_fatal_error("expected a character type");
}
...
fir::factory::CharacterExprHelper::materializeValue(mlir::Value str) {
  ...
  if (!charTy || charTy.getLen() == fir::CharacterType::unknownLen()) {
    LLVM_DEBUG(toolchain::dbgs() << "cannot materialize: " << str << '\n');
    toolchain_unreachable("must be a !fir.char<N> type");
  }
  auto len = builder.createIntegerConstant(
      loc, builder.getCharacterLengthType(), charTy.getLen());
  auto temp = fir::AllocaOp::create(builder, loc, charTy);
  fir::StoreOp::create(builder, loc, str, temp);
  LLVM_DEBUG(toolchain::dbgs() << "materialized as local: " << str << " -> (" << temp
                          << ", " << len << ")\n");
  return {temp, len};
}
...
fir::factory::CharacterExprHelper::toExtendedValue(mlir::Value character,
  ...
  if (auto charTy = mlir::dyn_cast<fir::CharacterType>(type)) {
    ...
    toolchain_unreachable("Cannot translate mlir::Value to character ExtendedValue");
  }
  ...
  if (!resultLen)
    toolchain::report_fatal_error("no dynamic length found for character");
  if (!extents.empty())
...
fir::factory::CharacterExprHelper::createLoadCharAt(mlir::Value buff,
                                                    mlir::Value index) {
  LLVM_DEBUG(toolchain::dbgs() << "load a char: " << buff << " type: "
...
void fir::factory::CharacterExprHelper::createStoreCharAt(mlir::Value str,
                                                          mlir::Value index,
                                                          mlir::Value c) {
  LLVM_DEBUG(toolchain::dbgs() << "store the char: " << c << " into: " << str
...
void fir::factory::CharacterExprHelper::createCopy(
  ...
  LLVM_DEBUG(toolchain::dbgs() << "create char copy from: "; src.dump();
             toolchain::dbgs() << " to: "; dest.dump();
             toolchain::dbgs() << " count: " << count << '\n');
  ...
  if (getCharacterKind(dest.getBuffer().getType()) == kind) {
    ...
    auto toolchainPointerType =
        mlir::LLVM::LLVMPointerType::get(builder.getContext());
    auto toPtr = builder.createConvert(loc, toolchainPointerType, toBuff);
    auto fromPtr = builder.createConvert(loc, toolchainPointerType, fromBuff);
...
fir::CharBoxValue fir::factory::CharacterExprHelper::createSubstring(
    const fir::CharBoxValue &box, toolchain::ArrayRef<mlir::Value> bounds) {
...
fir::factory::CharacterExprHelper::createLenTrim(const fir::CharBoxValue &str) {
  ...
  toolchain::SmallVector<mlir::Value> results = {isBlank, index};
...
fir::factory::CharacterExprHelper::createUnboxChar(mlir::Value boxChar) {
  ...
  return toExtendedValue(boxChar).match(
      ...
      [](const auto &) -> T { toolchain::report_fatal_error("not a character"); });
}
...
fir::CharBoxValue fir::factory::CharacterExprHelper::createCharExtremum(
    bool predIsMin, toolchain::ArrayRef<fir::CharBoxValue> opCBVs) {
