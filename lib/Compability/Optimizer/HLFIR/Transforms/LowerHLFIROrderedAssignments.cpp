...
// info.
static toolchain::cl::opt<bool> dbgScheduleOnly(
    "flang-dbg-order-assignment-schedule-only",
    toolchain::cl::desc("Only run ordered assignment scheduling with no codegen"),
...
struct MaskedArrayExpr {
  ...
  toolchain::SmallSet<mlir::Operation *, 4> elementalParts{};
...
private:
  ...
  bool mustSaveRegionIn(
      ...
      toolchain::SmallVectorImpl<hlfir::SaveEntity> &saveEntities) const;
  ...
  bool currentLoopNestIterationNumberCanBeComputed(
      toolchain::SmallVectorImpl<fir::DoLoopOp> &loopNest);
  ...
  /// operation.
  toolchain::SmallVector<mlir::Operation *> constructStack;
  ...
  /// toolchain::MapVector is used to guarantee deterministic order
  /// of iterating through savedEntities (e.g. for generating
  /// destruction code for the temporary storages).
  toolchain::MapVector<mlir::Region *, fir::factory::TemporaryStorage> savedEntities;
  ...
  toolchain::DenseMap<mlir::Region *, ValueAndCleanUp> savedInCurrentRunBeforeUse;
...
void OrderedAssignmentRewriter::walk(
  ...
  toolchain::SmallVector<hlfir::SaveEntity> saveEntities;
  ...
  if (mustVisit) {
    toolchain::TypeSwitch<mlir::Operation *, void>(nodeOp)
...
convertToMoldType(mlir::Location loc, fir::FirOpBuilder &builder,
                  ...
                  toolchain::SmallVectorImpl<hlfir::CleanupFunction> &cleanups) {
...
void OrderedAssignmentRewriter::pre(hlfir::RegionAssignOp regionAssignOp) {
  ...
  if (!regionAssignOp.getUserDefinedAssignment().empty()) {
    ...
    toolchain::SmallVector<hlfir::CleanupFunction, 2> argConversionCleanups;
  ...
  if (fir::isa_trivial(oldYield.getEntity().getType()) &&
    ...
    mlir::WalkResult walkResult =
        region.walk([&](mlir::Operation *op) -> mlir::WalkResult {
          if (toolchain::any_of(op->getOperands(), [](mlir::Value value) {
...
static bool isInOrderApply(hlfir::ApplyOp apply,
  ...
  for (auto [elementalIdx, applyIdx] :
       toolchain::zip(elementalIndices, apply.getIndices()))
...
gatherElementalTree(hlfir::ElementalOpInterface elemental,
                    toolchain::SmallPtrSetImpl<mlir::Operation *> &elementalOps,
                    bool isOutOfOrder) {
  if (elemental) {
...
void MaskedArrayExpr::generateNoneElementalCleanupIfAny(
  ...
  if (!isOuterMaskExpr) {
    ...
    for (mlir::Operation &op :
         toolchain::reverse(region.back().without_terminator()))
      if (auto exactlyOnce = mlir::dyn_cast<hlfir::ExactlyOnceOp>(op)) {
        mlir::Region &cleanupRegion =
...
bool OrderedAssignmentRewriter::currentLoopNestIterationNumberCanBeComputed(
  ...
  while (currentConstruct) {
    if (auto doLoop = mlir::dyn_cast<fir::DoLoopOp>(currentConstruct)) {
      if (toolchain::any_of(doLoop->getOperands(), [&](mlir::Value value) {
...
computeLoopNestIterationNumber(mlir::Location loc, fir::FirOpBuilder &builder,
                               toolchain::ArrayRef<fir::DoLoopOp> loopNest) {
...
void OrderedAssignmentRewriter::generateSaveEntity(
  ...
  toolchain::StringRef tempName = getTempName(root);
  ...
  if (constructStack.empty()) {
    ...
    toolchain::SmallVector<fir::DoLoopOp> loopNest;
...
static bool isVectorSubscripted(mlir::Region &region) {
  return toolchain::isa<hlfir::ElementalAddrOp>(region.back().back());
}
...
void OrderedAssignmentRewriter::canonicalizeExactlyOnceInsideWhere(
  ...
  auto getDefinition = [](mlir::Value v) {
    ...
    if (!effects || !effects->empty()) {
      LLVM_DEBUG(toolchain::dbgs() << "Side effects on operation with result live "
  ...
  whereOp->walk([&](hlfir::ExactlyOnceOp op) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Canonicalizing:\n" << op << "\n");
    ...
    toolchain::SmallVector<mlir::Operation *> workList(cloneSet.begin(),
                                                  cloneSet.end());
    ...
    toolchain::SmallVector<mlir::Operation *> cloneList(cloneSet.begin(),
                                                   cloneSet.end());
    toolchain::sort(cloneList, [&](mlir::Operation *L, mlir::Operation *R) {
    ...
    for (auto *toClone : cloneList) {
      LLVM_DEBUG(toolchain::dbgs() << "Cloning: " << *toClone << "\n");
    ...
    for (mlir::Operation *oldOps : liveInSet)
      for (mlir::Value oldVal : oldOps->getResults()) {
        ...
        if (!newVal) {
          LLVM_DEBUG(toolchain::dbgs() << "No clone found for: " << oldVal << "\n");
          assert(false && "missing clone");
        }
        mlir::replaceAllUsesInRegionWith(oldVal, newVal, op.getBody());
      }

    LLVM_DEBUG(toolchain::dbgs() << "Finished canonicalization\n");
    if (!liveInSet.empty())
      LLVM_DEBUG(toolchain::dbgs() << op << "\n");
  });
}
...
static toolchain::LogicalResult rewrite(hlfir::OrderedAssignmentTreeOpInterface root,
