...
class DesignateOpConversion
  ...
  static mlir::Value genSubscriptBeginAddr(
      ...
      mlir::Value shape,
      const toolchain::SmallVector<mlir::Value> &firBaseTypeParameters) {
    assert(!designate.getIndices().empty());
    toolchain::SmallVector<mlir::Value> firstElementIndices;
...
public:
  ...
  matchAndRewrite(hlfir::DesignateOp designate,
    ...
    toolchain::SmallVector<mlir::Value> firBaseTypeParameters;
    ...
    if (mlir::isa<fir::BaseBoxType>(designateResultType)) {
      ...
      toolchain::SmallVector<mlir::Value> sliceFields;
      ...
      if (fieldIndex && baseEntity.isArray()) {
        ...
        if (!subscripts.empty()) {
          toolchain::SmallVector<mlir::Value> lbounds = hlfir::genLowerbounds(
              loc, builder, designate.getComponentShape(), subscripts.size());
          for (auto [i, lb] : toolchain::zip(subscripts, lbounds)) {
      ...
      toolchain::SmallVector<mlir::Type> resultType{
...
public:
  ...
  matchAndRewrite(hlfir::GetExtentOp getExtentOp,
    ...
    if (auto s = mlir::dyn_cast_or_null<fir::ShapeOp>(shapeOp)) {
      ...
      toolchain::APInt dim = getExtentOp.getDim();
