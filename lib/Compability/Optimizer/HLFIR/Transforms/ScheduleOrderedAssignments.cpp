...
private:
  ...
  saveEvaluation(mlir::Region &yieldRegion,
                 toolchain::ArrayRef<mlir::MemoryEffects::EffectInstance> effects,
                 bool anyWrite);
  ...
  toolchain::SmallVector<mlir::MemoryEffects::EffectInstance> assignEffects;
  ...
  toolchain::SmallVector<mlir::MemoryEffects::EffectInstance> assignEvaluateEffects;
  /// Memory effects of the unsaved evaluation region that are controlling or
  /// masking the current assignments.
  toolchain::SmallVector<mlir::MemoryEffects::EffectInstance>
  ...
  toolchain::SmallSet<mlir::Region *, 16> savedRegions;
...
static void gatherMemoryEffects(
    ...
    toolchain::SmallVectorImpl<mlir::MemoryEffects::EffectInstance> &effects) {
  ...
  for (mlir::Operation &op : region.getOps()) {
    ...
    if (!interface) {
      LLVM_DEBUG(toolchain::dbgs() << "unknown effect: " << op << "\n";);
    ...
    // also ignored.
    toolchain::SmallVector<mlir::MemoryEffects::EffectInstance> opEffects;
    ...
    for (auto &effect : opEffects)
      if (!isForallIndex(effect.getValue())) {
        if (mlir::isa<mlir::MemoryEffects::Read>(effect.getEffect())) {
          LLVM_DEBUG(logIfUnkownEffectValue(toolchain::dbgs(), effect, op););
...
static void gatherAssignEffects(
    ...
    toolchain::SmallVectorImpl<mlir::MemoryEffects::EffectInstance> &assignEffects) {
...
/// effectsB?
static bool
anyRAWorWAW(toolchain::ArrayRef<mlir::MemoryEffects::EffectInstance> effectsA,
            toolchain::ArrayRef<mlir::MemoryEffects::EffectInstance> effectsB,
      ...
      for (const auto &effectA : effectsA)
        if (mlir::isa<mlir::MemoryEffects::Write, mlir::MemoryEffects::Read>(
          ...
          if (!writtenVarB || !writtenOrReadVarA) {
            LLVM_DEBUG(
                logConflict(toolchain::dbgs(), writtenOrReadVarA, writtenVarB););
...
static bool
conflict(toolchain::ArrayRef<mlir::MemoryEffects::EffectInstance> effectsA,
         toolchain::ArrayRef<mlir::MemoryEffects::EffectInstance> effectsB) {
  fir::AliasAnalysis aliasAnalysis;
  // (RAW || WAW) || (WAR || WAW).
...
static bool
anyNonLocalWrite(toolchain::ArrayRef<mlir::MemoryEffects::EffectInstance> effects,
                 mlir::Region &region) {
...
void Scheduler::saveEvaluationIfConflict(mlir::Region &yieldRegion,
  ...
  if (savedRegions.contains(&yieldRegion))
    return;
  toolchain::SmallVector<mlir::MemoryEffects::EffectInstance> effects;
...
void Scheduler::saveEvaluation(
  ...
  LLVM_DEBUG(
      logSaveEvaluation(toolchain::dbgs(), schedule.size(), yieldRegion, anyWrite););
}
...
void Scheduler::finishSchedulingAssignment(hlfir::RegionAssignOp assign) {
  ...
  LLVM_DEBUG(logAssignmentEvaluation(toolchain::dbgs(), schedule.size(), assign));
}
...
gatherAssignments(hlfir::OrderedAssignmentTreeOpInterface root,
                  toolchain::SmallVector<hlfir::RegionAssignOp> &assignments) {
  toolchain::SmallVector<mlir::Operation *> nodeStack{root.getOperation()};
  while (!nodeStack.empty()) {
    ...
    if (nodeIface)
      if (mlir::Block *block = nodeIface.getSubTreeBlock())
        for (mlir::Operation &op : toolchain::reverse(block->getOperations()))
...
static void gatherParents(
    ...
    toolchain::SmallVectorImpl<hlfir::OrderedAssignmentTreeOpInterface> &parents) {
  while (node) {
    auto parent =
...
hlfir::buildEvaluationSchedule(hlfir::OrderedAssignmentTreeOpInterface root,
  ...
  LLVM_DEBUG(logStartScheduling(toolchain::dbgs(), root););
  ...
  for (hlfir::RegionAssignOp assign : assignments) {
    ...
    toolchain::SmallVector<hlfir::OrderedAssignmentTreeOpInterface> parents;
    gatherAssignmentParents(assign, parents);
    for (hlfir::OrderedAssignmentTreeOpInterface parent :
         toolchain::reverse(parents)) {
      scheduler.startIndependentEvaluationGroup();
      toolchain::SmallVector<mlir::Region *, 4> yieldRegions;
...
static toolchain::raw_ostream &printRegionId(toolchain::raw_ostream &os,
                                        mlir::Region &yieldRegion) {
...
static toolchain::raw_ostream &
printNodeIndexInBody(toolchain::raw_ostream &os,
                     hlfir::OrderedAssignmentTreeOpInterface node,
...
static toolchain::raw_ostream &printNodePath(toolchain::raw_ostream &os,
                                        mlir::Operation *op) {
  auto node =
  ...
  for (auto parent : toolchain::reverse(parents)) {
...
static toolchain::raw_ostream &printRegionPath(toolchain::raw_ostream &os,
                                          mlir::Region &yieldRegion) {
...
static void LLVM_ATTRIBUTE_UNUSED logConflict(toolchain::raw_ostream &os,
                                              mlir::Value writtenOrReadVarA,
                                              mlir::Value writtenVarB) {
  auto printIfValue = [&](mlir::Value var) -> toolchain::raw_ostream & {
    if (!var)
      return os << "<unknown>";
    return os << var;
  };
