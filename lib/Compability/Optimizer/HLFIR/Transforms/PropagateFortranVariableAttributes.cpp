...
void Propagator::setContiguousAttr(fir::FortranVariableOpInterface op) {
  LLVM_DEBUG(toolchain::dbgs() << "Setting continuity for:\n" << op << "\n");
...
void Propagator::process(mlir::Operation *op) {
  if (!isContiguous(op))
    return;
  toolchain::SmallVector<mlir::Operation *> workList{op};
  while (!workList.empty()) {
    mlir::Operation *current = workList.pop_back_val();
    LLVM_DEBUG(toolchain::dbgs() << "Propagating continuity from operation:\n"
                            << *current << "\n");
