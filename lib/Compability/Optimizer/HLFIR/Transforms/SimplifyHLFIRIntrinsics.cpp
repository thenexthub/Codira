...
static toolchain::cl::opt<bool> forceMatmulAsElemental(
    "flang-inline-matmul-as-elemental",
    toolchain::cl::desc("Expand hlfir.matmul as elemental operation"),
...
public:
  ...
  mlir::Value genAccumulateProduct(mlir::Value acc, mlir::Value v1,
    ...
    if (mlir::isa<mlir::FloatType>(resultType)) {
      ...
      toolchain_unreachable("unsupported type");
    }
...
private:
  static mlir::Value genResultShape(mlir::Location loc,
                                    fir::FirOpBuilder &builder,
                                    hlfir::Entity array) {
    toolchain::SmallVector<mlir::Value, 2> inExtents =
...
protected:
  ...
  virtual toolchain::SmallVector<mlir::Value>
  genReductionInitValues(mlir::ValueRange oneBasedIndices,
                         const toolchain::SmallVectorImpl<mlir::Value> &extents) = 0;
  ...
  virtual toolchain::SmallVector<mlir::Value>
  reduceOneElement(const toolchain::SmallVectorImpl<mlir::Value> &currentValue,
  ...
  genFinalResult(const toolchain::SmallVectorImpl<mlir::Value> &reductionResults) {
...
static mlir::Value genMinMaxInitValue(mlir::Location loc,
  ...
  if (auto ty = mlir::dyn_cast<mlir::FloatType>(type)) {
    const toolchain::fltSemantics &sem = ty.getFloatSemantics();
    // We must not use +/-INF here. If the reduction input is empty,
    // the result of reduction must be +/-LARGEST.
    toolchain::APFloat limit = toolchain::APFloat::getLargest(sem, /*Negative=*/IS_MAX);
    return builder.createRealConstant(loc, type, limit);
  }
  unsigned bits = type.getIntOrFloatBitWidth();
  int64_t limitInt = IS_MAX
                         ? toolchain::APInt::getSignedMinValue(bits).getSExtValue()
                         : toolchain::APInt::getSignedMaxValue(bits).getSExtValue();
...
genIsNotEmptyArrayExtents(mlir::Location loc, fir::FirOpBuilder &builder,
                          const toolchain::SmallVectorImpl<mlir::Value> &extents) {
...
static toolchain::SmallVector<mlir::Value> genFirstElementIndicesForReduction(
    mlir::Location loc, fir::FirOpBuilder &builder, bool isTotalReduction,
    mlir::FailureOr<int64_t> dim, unsigned rank,
    mlir::ValueRange oneBasedIndices) {
  toolchain::SmallVector<mlir::Value> indices{oneBasedIndices};
  mlir::Value one =
...
class MinMaxlocAsElementalConverter : public ReductionAsElementalConverter {
  ...
  static constexpr unsigned maxRank = language::Compability::common::maxRank;
  ...
  // value is not used.
  static constexpr unsigned maxNumReductions = language::Compability::common::maxRank + 2;
...
private:
  ...
  virtual toolchain::SmallVector<mlir::Value> genReductionInitValues(
      mlir::ValueRange oneBasedIndices,
      const toolchain::SmallVectorImpl<mlir::Value> &extents) final;
  virtual toolchain::SmallVector<mlir::Value>
  reduceOneElement(const toolchain::SmallVectorImpl<mlir::Value> &currentValue,
                   hlfir::Entity array, mlir::ValueRange oneBasedIndices) final;
  virtual hlfir::Entity genFinalResult(
      const toolchain::SmallVectorImpl<mlir::Value> &reductionResults) final;
...
private:
  ...
  void
  checkReductions(const toolchain::SmallVectorImpl<mlir::Value> &reductions) const {
    if (!useIsFirst())
  ...
  mlir::Value
  getCurrentMinMax(const toolchain::SmallVectorImpl<mlir::Value> &reductions) const {
    checkReductions(reductions);
    return reductions[getNumCoors()];
  }

  mlir::Value
  getIsFirst(const toolchain::SmallVectorImpl<mlir::Value> &reductions) const {
    checkReductions(reductions);
...
MinMaxlocAsElementalConverter<T>::genReductionInitValues(
  ...
  if (!useIsFirst() && honorNans()) {
    ...
    toolchain::SmallVector<mlir::Value> indices = genFirstElementIndicesForReduction(
        loc, builder, isTotalReduction(), getConstDim(), getSourceRank(),
        oneBasedIndices);

    toolchain::SmallVector<mlir::Type> ifTypes(getNumCoors(),
    ...
    toolchain::SmallVector<mlir::Value> results(getNumCoors(), one);
  ...
  toolchain::SmallVector<mlir::Value> result(getNumCoors(), zeroCoor);
...
MinMaxlocAsElementalConverter<T>::reduceOneElement(
    const toolchain::SmallVectorImpl<mlir::Value> &currentValue, hlfir::Entity array,
    mlir::ValueRange oneBasedIndices) {
  ...
  toolchain::SmallVector<mlir::Value> newIndices;
  int64_t dim = 1;
  if (!isTotalReduction()) {
...
hlfir::Entity MinMaxlocAsElementalConverter<T>::genFinalResult(
    const toolchain::SmallVectorImpl<mlir::Value> &reductionResults) {
...
protected:
  ...
  void checkReductions(const toolchain::SmallVectorImpl<mlir::Value> &reductions) {
...
private:
  virtual toolchain::SmallVector<mlir::Value> genReductionInitValues(
      ...
      [[maybe_unused]] const toolchain::SmallVectorImpl<mlir::Value> &extents)
      final {
    return {
        fir::factory::createZeroValue(builder, loc, getResultElementType())};
  }
...
protected:
  ...
  virtual hlfir::Entity genFinalResult(
      const toolchain::SmallVectorImpl<mlir::Value> &reductionResults) override {
    checkReductions(reductionResults);
    return hlfir::Entity{reductionResults[0]};
  }
};
...
private:
  ...
  virtual hlfir::Entity genFinalResult(
      const toolchain::SmallVectorImpl<mlir::Value> &reductionValues) final {
...
mlir::LogicalResult ReductionAsElementalConverter::convert() {
  ...
  toolchain::SmallVector<mlir::Value> arrayExtents;
  if (isTotalReduce)
  ...
  auto genKernel = [&](mlir::Location loc, fir::FirOpBuilder &builder,
    ...
    auto genBody = [&](mlir::Location loc, fir::FirOpBuilder &builder,
      ...
      toolchain::SmallVector<mlir::Value, 1> reductionValues = reductionArgs;
      toolchain::SmallVector<mlir::Type, 1> reductionTypes;
      toolchain::transform(reductionValues, std::back_inserter(reductionTypes),
    ...
    toolchain::SmallVector<mlir::Value, 1> reductionFinalValues =
...
ReductionAsElementalConverter::genResultShapeForPartialReduction(
  ...
  toolchain::SmallVector<mlir::Value> inExtents =
      hlfir::genExtentsVector(loc, builder, array);
...
mlir::Value SumAsElementalConverter::genScalarAdd(mlir::Value value1,
  ...
  toolchain_unreachable("unsupported SUM reduction type");
}
...
private:
  ...
  static mlir::Operation *genElementalCShift(mlir::PatternRewriter &rewriter,
                                             ...
                                             int64_t dimVal) {
    using language::Compability::common::maxRank;
    ...
    toolchain::SmallVector<mlir::Value, maxRank> arrayExtents =
    ...
    auto genKernel = [&](mlir::Location loc, fir::FirOpBuilder &builder,
                         mlir::ValueRange inputIndices) -> hlfir::Entity {
      toolchain::SmallVector<mlir::Value, maxRank> indices{inputIndices};
      if (!shiftVal) {
        ...
        toolchain::SmallVector<mlir::Value, maxRank> shiftIndices{indices};
  ...
  static mlir::Operation *genInMemCShift(mlir::PatternRewriter &rewriter,
    ...
    auto genDimensionShift = [&](mlir::Location loc, fir::FirOpBuilder &builder,
      ...
      srcIndices.append(oneBasedIndices.begin() + (dimVal - 1),
                        oneBasedIndices.end());
      toolchain::SmallVector<mlir::Value, maxRank> dstIndices(srcIndices);
      ...
      if (exposeContiguity && mlir::isa<fir::BaseBoxType>(srcArray.getType())) {
        ...
        toolchain::SmallVector<mlir::Value, maxRank> arrayLbounds =
    ...
    toolchain::SmallVector<mlir::Value, maxRank> newExtents(arrayExtents);
...
private:
  ...
  genResultShape(mlir::Location loc, fir::FirOpBuilder &builder,
    ...
    toolchain::SmallVector<mlir::Value, 2> input1Extents =
        hlfir::genExtentsVector(loc, builder, input1);
    toolchain::SmallVector<mlir::Value, 2> input2Extents =
        hlfir::genExtentsVector(loc, builder, input2);

    toolchain::SmallVector<mlir::Value, 2> newExtents;
    ...
    // optimizations.
    toolchain::SmallVector<mlir::Value> innerProductExtent =
  ...
  genContiguousMatmul(mlir::Location loc, fir::FirOpBuilder &builder,
    ...
    toolchain::SmallVector<mlir::Value, 2> resultExtents =
    ...
    toolchain_unreachable("unsupported MATMUL arguments' ranks");
  }
  ...
  genElementalMatmul(mlir::Location loc, fir::FirOpBuilder &builder,
    ...
    auto genKernel = [&](mlir::Location loc, fir::FirOpBuilder &builder,
      ...
      auto genBody = [&](mlir::Location loc, fir::FirOpBuilder &builder,
                         ...
                         mlir::ValueRange reductionArgs)
          -> toolchain::SmallVector<mlir::Value, 1> {
        toolchain::SmallVector<mlir::Value, 2> lhsIndices;
        toolchain::SmallVector<mlir::Value, 2> rhsIndices;
        // MATMUL:
        //   LHS(NROWS,N) * RHS(N,NCOLS) -> RESULT(NROWS,NCOLS)
        //   LHS(NROWS,N) * RHS(N) -> RESULT(NROWS)
        //   LHS(N) * RHS(N,NCOLS) -> RESULT(NCOLS)
      ...
      toolchain::SmallVector<mlir::Value, 1> innerProductValue =
...
public:
  ...
  matchAndRewrite(hlfir::DotProductOp product,
    ...
    toolchain::SmallVector<mlir::Value, 1> result = hlfir::genLoopNestWithReductions(
        loc, builder, {extent},
...
private:
  static mlir::Value genProductExtent(mlir::Location loc,
                                      ...
                                      hlfir::Entity input1,
                                      hlfir::Entity input2) {
    toolchain::SmallVector<mlir::Value, 1> input1Extents =
        hlfir::genExtentsVector(loc, builder, input1);
    toolchain::SmallVector<mlir::Value, 1> input2Extents =
        hlfir::genExtentsVector(loc, builder, input2);

    assert(input1Extents.size() == 1 && input2Extents.size() == 1 &&
           "hlfir.dot_product arguments must be vectors");
    toolchain::SmallVector<mlir::Value, 1> extent =
...
public:
  ...
  matchAndRewrite(hlfir::ReshapeOp reshape,
    ...
    toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> arrayExtents =
        hlfir::genExtentsVector(loc, builder, array);
    ...
    toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> resultExtents;
    mlir::Type indexType = builder.getIndexType();
    ...
    auto genKernel = [&](mlir::Location loc, fir::FirOpBuilder &builder,
      ...
      if (pad) {
        ...
        toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> padExtents =
        ...
        toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> padIndices =
      ...
      toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> arrayIndices =
...
private:
  ...
  static mlir::Value computeLinearIndex(mlir::Location loc,
    ...
    std::size_t idx = 0;
    for (auto index : toolchain::reverse(indices)) {
  ...
  ///     in := linearIndex + 1
  ///   }
  static toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank>
  delinearizeIndex(mlir::Location loc, fir::FirOpBuilder &builder,
                   mlir::ValueRange extents, mlir::Value linearIndex,
                   bool wrapAround) {
    toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> indices;
    mlir::Type indexType = builder.getIndexType();
    mlir::Value one = builder.createIntegerConstant(loc, indexType, 1);
