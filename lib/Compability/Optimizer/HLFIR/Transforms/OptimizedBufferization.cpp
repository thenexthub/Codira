...
ArraySectionAnalyzer::analyze(mlir::Value ref1, mlir::Value ref2) {
  ...
  if (des1.getMemref() != des2.getMemref()) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "No identical base for:\n"
                            << des1 << "and:\n"
                            << des2 << "\n");
    return SlicesOverlapKind::Unknown;
  }
  ...
  if (des1.getComponent() != des2.getComponent() ||
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Different designator specs for:\n"
                            << des1 << "and:\n"
                            << des2 << "\n");
    return SlicesOverlapKind::Unknown;
  }
  ...
  for (auto [isTriplet1, isTriplet2] :
       toolchain::zip(des1.getIsTriplet(), des2.getIsTriplet())) {
    ...
    if (!areIdenticalSections(desc1, desc2)) {
      if (isTriplet1 || isTriplet2) {
        ...
        LLVM_DEBUG(toolchain::dbgs() << "Triplet mismatch for:\n"
                                << des1 << "and:\n"
                                << des2 << "\n");
      } else {
        identicalIndices = false;
        LLVM_DEBUG(toolchain::dbgs() << "Indices mismatch for:\n"
                                << des1 << "and:\n"
                                << des2 << "\n");
      }
    }
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Different sections for:\n"
                          << des1 << "and:\n"
                          << des2 << "\n");
  return SlicesOverlapKind::Unknown;
}
...
ElementalAssignBufferization::getDesignatorIndices(
  ...
  if (mlir::isa<fir::BaseBoxType>(memref.getType())) {
    ...
    for (auto index : toolchain::enumerate(designate.getIndices())) {
...
ElementalAssignBufferization::findMatch(hlfir::ElementalOp elemental) {
  ...
  if (std::distance(users.begin(), users.end()) != 2) {
    LLVM_DEBUG(toolchain::dbgs() << "Too many uses of the elemental\n");
    return std::nullopt;
  }
  ...
  if (hlfir::elementalOpMustProduceTemp(elemental)) {
    LLVM_DEBUG(toolchain::dbgs() << "ElementalOp must produce a temp\n");
    return std::nullopt;
  }
  ...
  if (!match.assign || !match.destroy) {
    LLVM_DEBUG(toolchain::dbgs() << "Couldn't find assign or destroy\n");
    return std::nullopt;
  }
  ...
  if (!arrayType) {
    LLVM_DEBUG(toolchain::dbgs() << "AssignOp's result is not an array\n");
    return std::nullopt;
  }
  ...
  if (!fir::isa_trivial(eleTy)) {
    LLVM_DEBUG(toolchain::dbgs() << "AssignOp's data type is not trivial\n");
    return std::nullopt;
  }
  ...
  if (match.assign.isAllocatableAssignment()) {
    LLVM_DEBUG(toolchain::dbgs() << "AssignOp may involve (re)allocation of LHS\n");
    return std::nullopt;
  }
  ...
  for (const mlir::MemoryEffects::EffectInstance &effect : *effects) {
    ...
    if (mlir::isa<mlir::MemoryEffects::Write>(effect.getEffect())) {
      LLVM_DEBUG(toolchain::dbgs() << "write inside the elemental body\n");
      return std::nullopt;
    }
    ...
    if (!res.isPartial()) {
      if (auto designate =
        ...
        if (overlap == ArraySectionAnalyzer::SlicesOverlapKind::Unknown) {
          LLVM_DEBUG(toolchain::dbgs() << "possible read conflict: " << designate
                                  << " at " << elemental.getLoc() << "\n");
          return std::nullopt;
        }
    ...
    LLVM_DEBUG(toolchain::dbgs() << "disallowed side-effect: " << effect.getValue()
...
toolchain::LogicalResult ElementalAssignBufferization::matchAndRewrite(
...
toolchain::LogicalResult BroadcastAssignBufferization::matchAndRewrite(
  ...
  if (lhs.isSimplyContiguous() && extents.size() > 1) {
    ...
    toolchain::SmallVector<mlir::Value> flatExtents = {n};
    ...
    if (mlir::isa<fir::BoxType>(lhs.getType())) {
      ...
      toolchain::ArrayRef<int64_t> fixedShape = seqTy.getShape();
...
tryUsingAssignLhsDirectly(hlfir::EvaluateInMemoryOp evalInMem,
  ...
  hlfir::DestroyOp destroy;
  hlfir::AssignOp assign;
  for (auto user : toolchain::enumerate(evalInMem->getUsers())) {
    if (user.index() > 2)
      return mlir::failure();
...
toolchain::LogicalResult EvaluateIntoMemoryAssignBufferization::matchAndRewrite(
