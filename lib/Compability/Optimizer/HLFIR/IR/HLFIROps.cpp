...
static toolchain::cl::opt<bool> useStrictIntrinsicVerifier(
    "strict-intrinsic-verifier", toolchain::cl::init(false),
    toolchain::cl::desc("use stricter verifier for HLFIR intrinsic operations"));
...
getIntrinsicEffects(mlir::Operation *self,
                    toolchain::SmallVectorImpl<mlir::SideEffects::EffectInstance<
...
static toolchain::LogicalResult areMatchingTypes(Op &op, mlir::Type type1,
                                            mlir::Type type2,
...
toolchain::LogicalResult hlfir::AssignOp::verify() {
...
updateDeclaredInputTypeWithVolatility(mlir::Type inputType, mlir::Value memref,
  ...
  toolchain::TypeSwitch<mlir::Type>(inputType)
...
void hlfir::DeclareOp::build(mlir::OpBuilder &builder,
                             ...
                             toolchain::StringRef uniq_name, mlir::Value shape,
...
toolchain::LogicalResult hlfir::DeclareOp::verify() {
...
void hlfir::DesignateOp::build(
    ...
    mlir::Type result_type, mlir::Value memref, toolchain::StringRef component,
    mlir::Value component_shape, toolchain::ArrayRef<Subscript> subscripts,
  ...
  toolchain::SmallVector<bool> isTriplet;
...
void hlfir::DesignateOp::build(mlir::OpBuilder &builder,
                               ...
                               mlir::ValueRange typeparams,
                               fir::FortranVariableFlagsAttr fortran_attrs) {
  toolchain::SmallVector<bool> isTriplet(indices.size(), false);
  auto isTripletAttr =
      mlir::DenseBoolArrayAttr::get(builder.getContext(), isTriplet);
...
static mlir::ParseResult parseDesignatorIndices(
    ...
    toolchain::SmallVectorImpl<mlir::OpAsmParser::UnresolvedOperand> &indices,
...
template <typename Op>
static toolchain::LogicalResult verifyTypeparams(Op &op, mlir::Type elementType,
...
toolchain::LogicalResult hlfir::DesignateOp::verify() {
  ...
  unsigned subscriptsRank =
      toolchain::count_if(getIsTriplet(), [](bool isTriplet) { return isTriplet; });
...
toolchain::LogicalResult hlfir::ParentComponentOp::verify() {
  mlir::Type baseType =
  ...
  if (maybeOutputSeqType && maybeInputSeqType)
    for (auto [inputDim, outputDim] :
         toolchain::zip(maybeInputSeqType.getShape(), maybeOutputSeqType.getShape()))
...
verifyLogicalReductionOp(LogicalReductionOp reductionOp) {
  ...
  toolchain::ArrayRef<int64_t> maskShape = maskTy.getShape();
  ...
  if (mlir::isa<fir::LogicalType>(resultType)) {
    ...
    if (maskShape.size() > 1 && dim != nullptr) {
      ...
      toolchain::ArrayRef<int64_t> resultShape = resultExpr.getShape();
      // Result has rank n-1
...
toolchain::LogicalResult hlfir::AllOp::verify() {
...
toolchain::LogicalResult hlfir::AnyOp::verify() {
...
toolchain::LogicalResult hlfir::CountOp::verify() {
...
toolchain::LogicalResult hlfir::ConcatOp::verify() {
...
verifyArrayAndMaskForReductionOp(NumericalReductionOp reductionOp) {
  ...
  toolchain::ArrayRef<int64_t> arrayShape = arrayTy.getShape();

  if (mask) {
    fir::SequenceType maskSeq = mlir::dyn_cast<fir::SequenceType>(
        hlfir::getFortranElementOrSequenceType(mask.getType()));
    toolchain::ArrayRef<int64_t> maskShape;
...
toolchain::LogicalResult hlfir::ProductOp::verify() {
...
toolchain::LogicalResult hlfir::MaxvalOp::verify() {
  mlir::Operation *op = getOperation();
...
toolchain::LogicalResult hlfir::MinvalOp::verify() {
  mlir::Operation *op = getOperation();
...
toolchain::LogicalResult hlfir::MinlocOp::verify() {
...
toolchain::LogicalResult hlfir::MaxlocOp::verify() {
  auto res = verifyArrayAndMaskForReductionOp(this);
  if (failed(res))
    return res;
...
toolchain::LogicalResult hlfir::SumOp::verify() {
...
toolchain::LogicalResult hlfir::DotProductOp::verify() {
  ...
  toolchain::ArrayRef<int64_t> lhsShape = lhsTy.getShape();
  toolchain::ArrayRef<int64_t> rhsShape = rhsTy.getShape();
...
toolchain::LogicalResult hlfir::MatmulOp::verify() {
  mlir::Value lhs = getLhs();
  mlir::Value rhs = getRhs();
  fir::SequenceType lhsTy = mlir::cast<fir::SequenceType>(
      hlfir::getFortranElementOrSequenceType(lhs.getType()));
  ...
  toolchain::ArrayRef<int64_t> resultShape = resultTy.getShape();
  ...
  toolchain::SmallVector<int64_t, 2> expectedResultShape;
  if (lhsRank == 2) {
    if (rhsRank == 2) {
...
hlfir::MatmulOp::canonicalize(MatmulOp matmulOp,
  ...
  if (auto transposeOp = lhs.getDefiningOp<hlfir::TransposeOp>()) {
    if (isOtherwiseUnused(transposeOp)) {
      ...
      toolchain::SmallVector<mlir::Operation *> users(
...
toolchain::LogicalResult hlfir::TransposeOp::verify() {
  mlir::Value array = getArray();
  fir::SequenceType arrayTy = mlir::cast<fir::SequenceType>(
      hlfir::getFortranElementOrSequenceType(array.getType()));
  toolchain::ArrayRef<int64_t> inShape = arrayTy.getShape();
...
toolchain::LogicalResult hlfir::MatmulTransposeOp::verify() {
  mlir::Value lhs = getLhs();
  mlir::Value rhs = getRhs();
  fir::SequenceType lhsTy = mlir::cast<fir::SequenceType>(
      hlfir::getFortranElementOrSequenceType(lhs.getType()));
...
toolchain::LogicalResult hlfir::CShiftOp::verify() {
  mlir::Value array = getArray();
  fir::SequenceType arrayTy = mlir::cast<fir::SequenceType>(
      hlfir::getFortranElementOrSequenceType(array.getType()));
  toolchain::ArrayRef<int64_t> inShape = arrayTy.getShape();
  ...
  constexpr int64_t unknownExtent = fir::SequenceType::getUnknownExtent();
  for (auto [inDim, resultDim] : toolchain::zip(inShape, resultShape))
  ...
  if (auto shiftSeqTy = mlir::dyn_cast<fir::SequenceType>(shiftTy)) {
    ...
    toolchain::ArrayRef<int64_t> shiftShape = shiftSeqTy.getShape();
    ...
    if (useStrictIntrinsicVerifier && dimVal != -1) {
      ...
      for (auto shiftDim : shiftShape) {
        ...
        if (inShape[arrayDimIdx] != unknownExtent &&
          ...
          return emitOpError("SHAPE(ARRAY)(" + toolchain::Twine(arrayDimIdx + 1) +
                             ") must be equal to SHAPE(SHIFT)(" +
                             toolchain::Twine(shiftDimIdx + 1) +
                             "): " + toolchain::Twine(inShape[arrayDimIdx]) +
                             " != " + toolchain::Twine(shiftDim));
        ++arrayDimIdx;
        ++shiftDimIdx;
      }
    }
...
toolchain::LogicalResult hlfir::ReshapeOp::verify() {
...
toolchain::LogicalResult hlfir::EndAssociateOp::verify() {
...
toolchain::LogicalResult hlfir::ElementalOp::verify() {
...
toolchain::LogicalResult hlfir::DestroyOp::verify() {
  if (mustFinalizeExpr()) {
...
toolchain::LogicalResult hlfir::ShapeOfOp::verify() {
  mlir::Value expr = getExpr();
  hlfir::ExprType exprTy = mlir::cast<hlfir::ExprType>(expr.getType());
...
hlfir::ShapeOfOp::canonicalize(ShapeOfOp shapeOf,
  ...
  if (!shape)
    ...
    return toolchain::LogicalResult::failure();

  rewriter.replaceAllUsesWith(shapeOf.getResult(), shape);
  rewriter.eraseOp(shapeOf);
  return toolchain::LogicalResult::success();
}
...
toolchain::LogicalResult hlfir::RegionAssignOp::verify() {
...
toolchain::LogicalResult hlfir::ElementalAddrOp::verify() {
...
toolchain::LogicalResult hlfir::OrderedAssignmentTreeOpInterface::verifyImpl() {
...
toolchain::LogicalResult hlfir::ForallMaskOp::verify() {
...
template <typename ConcreteOp>
static toolchain::LogicalResult verifyWhereAndElseWhereBody(ConcreteOp &concreteOp) {
...
toolchain::LogicalResult hlfir::WhereOp::verify() {
...
toolchain::LogicalResult hlfir::ElseWhereOp::verify() {
...
hlfir::ForallIndexOp::canonicalize(hlfir::ForallIndexOp indexOp,
  ...
  toolchain::SmallVector<mlir::Operation *> users(indexOp->getResult(0).getUsers());
  for (mlir::Operation *user : users)
...
toolchain::LogicalResult hlfir::CharExtremumOp::verify() {
  if (getStrings().size() < 2)
    return emitOpError("must be provided at least two string operands");
  unsigned kind = getCharacterKind(getResult().getType());
  for (auto string : getStrings())
...
toolchain::LogicalResult hlfir::EvaluateInMemoryOp::verify() {
  unsigned shapeRank = 0;
  if (mlir::Value shape = getShape())
...
#include "language/Compability/Optimizer/HLFIR/HLFIROpInterfaces.cpp.inc"
#define GET_OP_CLASSES
#include "language/Compability/Optimizer/HLFIR/HLFIREnums.cpp.inc"
