...
class MapInfoFinalizationPass
  ...
  getMemberUserList(mlir::omp::MapInfoOp op,
                    toolchain::SmallVectorImpl<ParentAndPlacement> &mapMemberUsers) {
    for (auto *user : op->getUsers())
      if (auto map = mlir::dyn_cast_if_present<mlir::omp::MapInfoOp>(user))
        for (auto [i, mapMember] : toolchain::enumerate(map.getMembers()))
          if (mapMember.getDefiningOp() == op)
            mapMemberUsers.push_back({map, i});
  }

  void getAsIntegers(toolchain::ArrayRef<mlir::Attribute> values,
                     toolchain::SmallVectorImpl<int64_t> &ints) {
    ints.reserve(values.size());
    toolchain::transform(values, std::back_inserter(ints),
  ...
  void getMemberIndicesAsVectors(
      ...
      toolchain::SmallVectorImpl<toolchain::SmallVector<int64_t>> &indices) {
    indices.reserve(mapInfo.getMembersIndexAttr().getValue().size());
    toolchain::transform(mapInfo.getMembersIndexAttr().getValue(),
                    std::back_inserter(indices), [this](mlir::Attribute value) {
                      auto memberIndex = mlir::cast<mlir::ArrayAttr>(value);
                      toolchain::SmallVector<int64_t> indexes;
  ...
  mlir::omp::MapInfoOp genBaseAddrMap(mlir::Value descriptor,
    ...
    mlir::Type underlyingVarType =
        toolchain::cast<mlir::omp::PointerLikeType>(
            fir::unwrapRefType(baseAddrAddr.getType()))
            .getElementType();
    if (auto seqType = toolchain::dyn_cast<fir::SequenceType>(underlyingVarType))
  ...
  void adjustMemberIndices(
      toolchain::SmallVectorImpl<toolchain::SmallVector<int64_t>> &memberIndices,
      size_t memberIndex) {
    toolchain::SmallVector<int64_t> baseAddrIndex = memberIndices[memberIndex];
    ...
    // for that member.
    for (toolchain::SmallVector<int64_t> &member : memberIndices)
  ...
  unsigned long getDescriptorMapType(unsigned long mapTypeFlag,
    ...
    if (toolchain::isa_and_nonnull<mlir::omp::TargetExitDataOp,
    ...
    return toolchain::to_underlying(flags);
  }
  ...
  bool isHasDeviceAddr(mlir::omp::MapInfoOp mapOp, mlir::Operation *userOp) {
    ...
    if (auto targetOp = toolchain::dyn_cast<mlir::omp::TargetOp>(userOp)) {
  ...
  mlir::omp::MapInfoOp genBoxcharMemberMap(mlir::omp::MapInfoOp op,
    ...
    toolchain::SmallVector<toolchain::SmallVector<int64_t>> memberIdx = {{0}};
    ...
    mlir::omp::MapInfoOp newMapInfoOp = mlir::omp::MapInfoOp::create(
        ...
        mlir::TypeAttr::get(
            toolchain::cast<mlir::omp::PointerLikeType>(varPtr.getType())
                .getElementType()),
        op.getMapTypeAttr(), op.getMapCaptureTypeAttr(),
        /*varPtrPtr=*/mlir::Value{},
        /*members=*/toolchain::SmallVector<mlir::Value>{memberMapInfoOp},
        /*member_index=*/newMembersAttr,
        /*bounds=*/toolchain::SmallVector<mlir::Value>{},
  ...
  mlir::omp::MapInfoOp genDescriptorMemberMaps(mlir::omp::MapInfoOp op,
                                               fir::FirOpBuilder &builder,
                                               mlir::Operation *target) {
    toolchain::SmallVector<ParentAndPlacement> mapMemberUsers;
    ...
    toolchain::SmallVector<toolchain::SmallVector<int64_t>> memberIndices;
    ...
    if (!mapMemberUsers.empty()) {
      ...
      toolchain::SmallVector<mlir::Value> newMemberOps;
    ...
    if (IsHasDeviceAddr) {
      mapType |= toolchain::to_underlying(
          toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_ALWAYS);
    }
  ...
  void addImplicitMembersToTarget(mlir::omp::MapInfoOp op,
                                  ...
                                  mlir::Operation *target) {
    auto mapClauseOwner =
        toolchain::dyn_cast_if_present<mlir::omp::MapClauseOwningOpInterface>(
            target);
    ...
    auto addOperands = [&](mlir::MutableOperandRange &mutableOpRange,
      ...
      if (!toolchain::is_contained(mutableOpRange.getAsOperandRange(),
                              op.getResult()))
        return;
      ...
      // vector.
      toolchain::SmallVector<mlir::Value> newMapOps;
      ...
      for (auto mapMember : op.getMembers()) {
        if (toolchain::is_contained(mutableOpRange.getAsOperandRange(), mapMember))
          continue;
    ...
    auto argIface =
        toolchain::dyn_cast<mlir::omp::BlockArgOpenMPOpInterface>(target);

    if (auto mapClauseOwner =
            toolchain::dyn_cast<mlir::omp::MapClauseOwningOpInterface>(target)) {
      ...
      addOperands(mapMutableOpRange,
                  toolchain::dyn_cast_if_present<mlir::omp::TargetOp>(
                      argIface.getOperation()),
                  blockArgInsertIndex);
    }

    if (auto targetDataOp = toolchain::dyn_cast<mlir::omp::TargetDataOp>(target)) {
    ...
    } else if (auto targetOp = toolchain::dyn_cast<mlir::omp::TargetOp>(target)) {
  ...
  mlir::Operation *getFirstTargetUser(mlir::omp::MapInfoOp mapOp) {
    for (auto *user : mapOp->getUsers()) {
      if (toolchain::isa<mlir::omp::TargetOp, mlir::omp::TargetDataOp,
                    mlir::omp::TargetUpdateOp, mlir::omp::TargetExitDataOp,
                    mlir::omp::TargetEnterDataOp,
                    mlir::omp::DeclareMapperInfoOp>(user))
        return user;

      if (auto mapUser = toolchain::dyn_cast<mlir::omp::MapInfoOp>(user))
  ...
  void runOnOperation() override {
    ...
    getOperation()->walk([&](mlir::Operation *func) {
      ...
      func->walk([&](mlir::omp::MapInfoOp op) {
        ...
        toolchain::SetVector<mlir::Operation *> mapVarForwardSlice;
        ...
        toolchain::SmallVector<mlir::Value> newMapOpsForFields;
        toolchain::SmallVector<int64_t> fieldIndicies;
        ...
        for (auto fieldMemTyPair : recordType.getTypeList()) {
          ...
          bool shouldMapField =
              toolchain::find_if(mapVarForwardSlice, [&](mlir::Operation *sliceOp) {
          ...
          auto fieldCoord = fir::CoordinateOp::create(
              ...
              toolchain::SmallVector<fir::IntOrValue, 1>{idxConst});
          fir::factory::AddrAndBoundsInfo info =
              fir::factory::getDataOperandBaseAddr(
        ...
        toolchain::SmallVector<toolchain::SmallVector<int64_t>> newMemberIndices;
        ...
        if (oldMembersIdxAttr)
          for (mlir::Attribute indexList : oldMembersIdxAttr) {
            toolchain::SmallVector<int64_t> listVec;
        ...
        for (int64_t newFieldIdx : fieldIndicies)
          newMemberIndices.emplace_back(
              toolchain::SmallVector<int64_t>(1, newFieldIdx));
      ...
      func->walk([&](mlir::omp::MapInfoOp op) {
        ...
        // over-mapping.
        bool hasImplicitMap =
            (toolchain::omp::OpenMPOffloadMappingFlags(op.getMapType()) &
             toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_IMPLICIT) ==
            toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_IMPLICIT;
        if (hasImplicitMap)
          return;

        assert(toolchain::hasSingleElement(op->getUsers()) &&
