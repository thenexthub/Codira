...
#include <toolchain/ADT/SmallVectorExtras.h>
...
static void cleanupBlock(Block *block) {
  for (Operation &op : toolchain::make_early_inc_range(
           toolchain::make_range(block->rbegin(), block->rend())))
...
static void parallelizeRegion(Region &sourceRegion, Region &targetRegion,
  ...
  auto moveToSingle =
    ...
    for (Operation &op : toolchain::make_range(sr.begin, sr.end)) {
      if (isSafeToParallelize(&op)) {
        singleBuilder.clone(op, singleMapping);
        if (toolchain::all_of(op.getOperands(), [&](Value opr) {
          ...
          // `op`.
          assert(toolchain::all_of(op.getResults(), [&](Value v) {
  ...
  for (Block &block : sourceRegion) {
    Block *targetBlock = rootBuilder.createBlock(
        ...
        toolchain::map_to_vector(block.getArguments(),
  ...
  auto handleOneBlock = [&](Block &block) {
    ...
    for (auto [i, opOrSingle] : toolchain::enumerate(regions)) {
      ...
      if (std::holds_alternative<SingleRegion>(opOrSingle)) {
        ...
        if (auto wslw = dyn_cast<omp::WorkshareLoopWrapperOp>(op)) {
          ...
          for (auto [region, clonedRegion] :
               toolchain::zip(op->getRegions(), cloned->getRegions()))
  ...
  if (sourceRegion.hasOneBlock()) {
    ...
    for (auto node : toolchain::breadth_first(domTree.getRootNode())) {
