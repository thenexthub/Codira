...
/// https://discourse.toolchain.org/t/rfc-representing-combined-composite-constructs-in-the-openmp-dialect/76986.
  ...
  matchAndRewrite(mlir::omp::LoopOp loopOp, OpAdaptor adaptor,
    ...
    case GenericLoopCombinedInfo::TeamsLoop:
      if (teamsLoopCanBeParallelFor(loopOp)) {
        ...
        auto teamsOp = toolchain::cast<mlir::omp::TeamsOp>(loopOp->getParentOp());
        auto teamsBlockArgIface =
            toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(*teamsOp);
        auto loopBlockArgIface =
            toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(*loopOp);
...
private:
  ...
  rewriteToSingleWrapperOp(mlir::omp::LoopOp loopOp,
    ...
    language::Compability::common::openmp::EntryBlockArgs args;
    ...
    for (auto [loopOpArg, opArg] :
         toolchain::zip_equal(loopBlock.getArguments(), opBlock->getArguments()))
  ...
  void rewriteToDistributeParallelDo(
    ...
    language::Compability::common::openmp::EntryBlockArgs parallelArgs;
    ...
    language::Compability::common::openmp::EntryBlockArgs wsloopArgs;
    ...
    auto parallelBlockInterface =
        toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(*parallelOp);
    auto wsloopBlockInterface =
        toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(*wsloopOp);

    for (auto [loopOpArg, parallelOpArg] :
         toolchain::zip_equal(loopBlockInterface.getPrivateBlockArgs(),
                         parallelBlockInterface.getPrivateBlockArgs()))
      mapper.map(loopOpArg, parallelOpArg);

    for (auto [loopOpArg, wsloopOpArg] :
         toolchain::zip_equal(loopBlockInterface.getReductionBlockArgs(),
  ...
  populateReductionClauseOps(mlir::omp::LoopOp loopOp,
    ...
    std::optional<toolchain::ArrayRef<bool>> reductionByref =
...
public:
  ...
  tryToFindNestedLoopWithReduction(mlir::omp::TeamsOp teamsOp) {
    ...
    auto loopOpIter = toolchain::find_if(teamsBlock, [](mlir::Operation &op) {
      auto nestedLoopOp = toolchain::dyn_cast<mlir::omp::LoopOp>(&op);
    ...
    return toolchain::cast<mlir::omp::LoopOp>(loopOpIter);
  }
  ...
  matchAndRewrite(mlir::omp::TeamsOp teamsOp, OpAdaptor adaptor,
    ...
    rewriter.modifyOpInPlace(teamsOp, [&]() {
      ...
      toolchain::SmallVector<mlir::Value> newLoopOpReductionOperands;

      for (auto [idx, reductionVar] :
           toolchain::enumerate(nestedLoopOp.getReductionVars())) {
