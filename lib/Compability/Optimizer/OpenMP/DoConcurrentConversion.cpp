...
struct InductionVariableInfo {
  ...
  /// iteration number.
  toolchain::SmallVector<mlir::Operation *, 2> indVarUpdateOps;
...
using InductionVariableInfos = toolchain::SmallVector<InductionVariableInfo>;
...
void collectLoopLocalValues(fir::DoConcurrentLoopOp loop,
                            toolchain::SetVector<mlir::Value> &locals) {
...
public:
  ...
  DoConcurrentConversion(
      ...
      toolchain::DenseSet<fir::DoConcurrentOp> &concurrentLoopsToSkip)
  ...
  matchAndRewrite(fir::DoConcurrentOp doLoop, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<mlir::Operation *> opsToMove;
    for (mlir::Operation &op : toolchain::drop_end(doLoop))
      opsToMove.push_back(&op);

    mlir::Block *allocBlock = builder.getAllocaBlock();

    for (mlir::Operation *op : toolchain::reverse(opsToMove)) {
...
private:
  ...
  genInductionVariableAlloc(mlir::ConversionPatternRewriter &rewriter,
    ...
    toolchain::SmallSetVector<mlir::Operation *, 2> indVarDeclareAndAlloc;
  ...
  genLoopNestClauseOps(mlir::Location loc,
    ...
    auto populateBounds = [](mlir::Value var,
                             toolchain::SmallVectorImpl<mlir::Value> &bounds) {
      bounds.push_back(var.getDefiningOp()->getResult(0));
    };

    for (auto [lb, ub, st] : toolchain::zip_equal(
  ...
  genWsLoopOp(mlir::ConversionPatternRewriter &rewriter,
    ...
    if (!loop.getLocalVars().empty())
      for (auto [op, sym, arg] : toolchain::zip_equal(
               loop.getLocalVars(),
    ...
    if (!loop.getReduceVars().empty()) {
      for (auto [op, byRef, sym, arg] : toolchain::zip_equal(
    ...
    language::Compability::common::openmp::genEntryBlock(rewriter, wsloopArgs,
                                           wsloopOp.getRegion());
    ...
    for (auto [wsloopArg, loopNestArg] :
         toolchain::zip_equal(wsloopOp.getRegion().getArguments(),
