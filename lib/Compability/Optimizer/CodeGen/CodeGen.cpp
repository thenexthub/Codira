...
#include "language/Compability/Optimizer/CodeGen/FIROpPatterns.h"
...
#include "language/Compability/Runtime/CUDA/memory.h"
...
static toolchain::SmallVector<mlir::NamedAttribute>
...
mlir::Value replaceWithAddrOfOrASCast(mlir::ConversionPatternRewriter &rewriter,
                                      ...
                                      toolchain::StringRef symName, mlir::Type type,
                                      mlir::Operation *replaceOp = nullptr) {
  if (mlir::isa<mlir::LLVM::LLVMPointerType>(type)) {
    if (globalAS != programAS) {
      auto toolchainAddrOp = mlir::LLVM::AddressOfOp::create(
...
/// Lower `fir.address_of` operation to `toolchain.address_of` operation.
...
struct AllocaOpConversion : public fir::FIROpConversion<fir::AllocaOp> {
  ...
  matchAndRewrite(fir::AllocaOp alloc, OpAdaptor adaptor,
    ...
    mlir::Type toolchainObjectType = convertObjectType(firObjType);
    if (alloc.hasLenParams()) {
      ...
      if (auto chrTy = mlir::dyn_cast<fir::CharacterType>(scalarType)) {
        ...
        toolchainObjectType = convertType(rawCharTy);
        assert(end == 1);
        ...
        size = call.getResult();
        toolchainObjectType = ::getI8Type(alloc.getContext());
      } else {
    ...
    if (mlir::isa<mlir::LLVM::ConstantOp>(size.getDefiningOp())) {
      // Set the Block in which the toolchain alloca should be inserted.
      ...
      // toolchain.constant operation, it is faster to clone it than to calculate the
    ...
    auto toolchainAlloc = mlir::LLVM::AllocaOp::create(
        rewriter, loc, ::getLlvmPtrType(alloc.getContext(), allocaAs),
        toolchainObjectType, size);
    if (alloc.getPinned())
      toolchainAlloc->setDiscardableAttr(alloc.getPinnedAttrName(),
                                    alloc.getPinnedAttr());
    if (alloc.getBindcName())
      toolchainAlloc->setDiscardableAttr(alloc.getBindcNameAttrName(),
                                    alloc.getBindcNameAttr());
    if (allocaAs == programAs) {
      rewriter.replaceOp(alloc, toolchainAlloc);
    } else {
      ...
      rewriter.replaceOpWithNewOp<mlir::LLVM::AddrSpaceCastOp>(
          alloc, ::getLlvmPtrType(alloc.getContext(), programAs), toolchainAlloc);
    }
...
/// Convert `!fir.boxchar_len` to  `!toolchain.extractvalue` for the 2nd part of the
/// boxchar.
...
struct BoxDimsOpConversion : public fir::FIROpConversion<fir::BoxDimsOp> {
  ...
  matchAndRewrite(fir::BoxDimsOp boxdims, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const override {
    toolchain::SmallVector<mlir::Type, 3> resultTypes = {
...
struct StringLitOpConversion : public fir::FIROpConversion<fir::StringLitOp> {
  ...
  matchAndRewrite(fir::StringLitOp constop, OpAdaptor adaptor,
    ...
    if (auto arr = mlir::dyn_cast<mlir::DenseElementsAttr>(attr)) {
      ...
      for (auto a : toolchain::enumerate(arr.getValue())) {
...
struct CallOpConversion : public fir::FIROpConversion<fir::CallOp> {
  ...
  matchAndRewrite(fir::CallOp call, OpAdaptor adaptor,
    ...
    mlir::arith::AttrConvertFastMathToLLVM<fir::CallOp, mlir::LLVM::CallOp>
        attrConvert(call);
    auto toolchainCall = rewriter.replaceOpWithNewOp<mlir::LLVM::CallOp>(
    ...
    if (mlir::ArrayAttr argAttrsArray = call.getArgAttrsAttr()) {
      ...
      auto convertTypeAttr = [&](const mlir::NamedAttribute &attr) {
        return mlir::TypeAttr::get(convertType(
            toolchain::cast<mlir::TypeAttr>(attr.getValue()).getValue()));
      };
      toolchain::SmallVector<mlir::Attribute> newArgAttrsArray;
      for (auto argAttrs : argAttrsArray) {
        toolchain::SmallVector<mlir::NamedAttribute> convertedAttrs;
        for (const mlir::NamedAttribute &attr :
             toolchain::cast<mlir::DictionaryAttr>(argAttrs)) {
      ...
      toolchainCall.setArgAttrsAttr(rewriter.getArrayAttr(newArgAttrsArray));
    }
    if (mlir::ArrayAttr resAttrs = call.getResAttrsAttr())
      toolchainCall.setResAttrsAttr(resAttrs);

    if (memAttr)
      toolchainCall.setMemoryEffectsAttr(
...
struct CmpcOpConversion : public fir::FIROpConversion<fir::CmpcOp> {
  ...
  matchAndRewrite(fir::CmpcOp cmp, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<mlir::Value, 2> cp = {rcp, icp};
...
struct ConvertOpConversion : public fir::FIROpConversion<fir::ConvertOp> {
  ...
  matchAndRewrite(fir::ConvertOp convert, OpAdaptor adaptor,
    ...
    if (fir::isa_complex(fromFirTy) && fir::isa_complex(toFirTy)) {
      ...
      toolchain::SmallVector<int64_t> pos{0};
    ...
    if (isFloatingPointTy(fromTy)) {
      ...
      if (mlir::isa<mlir::IntegerType>(toTy)) {
        ...
        if (toFirTy.isUnsignedInteger()) {
          auto intrinsicName =
              mlir::StringAttr::get(convert.getContext(), "toolchain.fptoui.sat");
          rewriter.replaceOpWithNewOp<mlir::LLVM::CallIntrinsicOp>(
              convert, toTy, intrinsicName, op0);
        } else {
          auto intrinsicName =
              mlir::StringAttr::get(convert.getContext(), "toolchain.fptosi.sat");
          rewriter.replaceOpWithNewOp<mlir::LLVM::CallIntrinsicOp>(
              convert, toTy, intrinsicName, op0);
        }
        return mlir::success();
...
/// instructions that generate `!toolchain.struct<(ptr<ik>, i64)>`. The 1st element
...
struct EmboxCharOpConversion : public fir::FIROpConversion<fir::EmboxCharOp> {
  ...
  matchAndRewrite(fir::EmboxCharOp emboxChar, OpAdaptor adaptor,
    ...
    mlir::Type toolchainStructTy = convertType(emboxChar.getType());
    auto toolchainStruct = mlir::LLVM::UndefOp::create(rewriter, loc, toolchainStructTy);

    mlir::Type lenTy =
        mlir::cast<mlir::LLVM::LLVMStructType>(toolchainStructTy).getBody()[1];
    mlir::Value lenAfterCast = integerCast(loc, rewriter, lenTy, charBufferLen);

    mlir::Type addrTy =
        mlir::cast<mlir::LLVM::LLVMStructType>(toolchainStructTy).getBody()[0];
    ...
    auto insertBufferOp = mlir::LLVM::InsertValueOp::create(
        rewriter, loc, toolchainStruct, charBuffer, pos);
...
/// type.
static mlir::Value
computeElementDistance(mlir::Location loc, mlir::Type toolchainObjectType,
                       mlir::Type idxTy,
                       mlir::ConversionPatternRewriter &rewriter,
                       const mlir::DataLayout &dataLayout) {
  toolchain::TypeSize size = dataLayout.getTypeSize(toolchainObjectType);
  unsigned short alignment = dataLayout.getTypeABIAlignment(toolchainObjectType);
  std::int64_t distance = toolchain::alignTo(size, alignment);
...
namespace {
/// Lower a `fir.allocmem` instruction into `toolchain.call @malloc`
struct AllocMemOpConversion : public fir::FIROpConversion<fir::AllocMemOp> {
  ...
  matchAndRewrite(fir::AllocMemOp heap, OpAdaptor adaptor,
    ...
    mlir::Type toolchainObjectTy = convertObjectType(dataTy);
    if (fir::isRecordWithTypeParameters(fir::unwrapSequenceType(dataTy)))
      TODO(loc, "fir.allocmem codegen of derived type with length parameters");
    mlir::Value size = genTypeSizeInBytes(loc, ity, rewriter, toolchainObjectTy);
...
getFreeInModule(ModuleOp mod, fir::FreeMemOp op,
  ...
  // Create toolchain declaration for free.
  mlir::OpBuilder moduleBuilder(mod.getBodyRegion());
...
namespace {
/// Lower a `fir.freemem` instruction into `toolchain.call @free`
...
convertSubcomponentIndices(mlir::Location loc, mlir::Type eleTy,
  ...
  toolchain::SmallVector<mlir::Value> arrayIndices;
...
genCUFAllocDescriptor(mlir::Location loc,
  ...
  if (!dl)
    mlir::emitError(mod.getLoc(),
                    ...
                    "to generate toolchain IR from FIR");
  ...
  mlir::LLVM::LLVMPointerType toolchainPointerType =
      mlir::LLVM::LLVMPointerType::get(ctx);
  mlir::Type toolchainInt32Type = mlir::IntegerType::get(ctx, 32);
  mlir::Type toolchainIntPtrType =
      mlir::IntegerType::get(ctx, typeConverter.getPointerBitwidth(0));
  auto fctTy = mlir::LLVM::LLVMFunctionType::get(
      toolchainPointerType, {toolchainIntPtrType, toolchainPointerType, toolchainInt32Type});

  auto toolchainFunc = mod.lookupSymbol<mlir::LLVM::LLVMFuncOp>(
      RTNAME_STRING(CUFAllocDescriptor));
  auto funcFunc =
      mod.lookupSymbol<mlir::func::FuncOp>(RTNAME_STRING(CUFAllocDescriptor));
  if (!toolchainFunc && !funcFunc) {
  ...
  mlir::Value sizeInBytes =
      genConstantIndex(loc, toolchainIntPtrType, rewriter, boxSize);
  toolchain::SmallVector args = {sizeInBytes, sourceFile, sourceLine};
...
getTypeDescriptor(ModOpTy mod, mlir::ConversionPatternRewriter &rewriter,
  ...
  mlir::Type toolchainPtrTy = ::getLlvmPtrType(mod.getContext());
  ...
  if (options.ignoreMissingTypeDescriptors ||
      fir::NameUniquer::belongsToModule(
          name, language::Compability::semantics::typeInfoBuiltinModule))
    return mlir::LLVM::ZeroOp::create(rewriter, loc, toolchainPtrTy);
  ...
  mlir::LLVM::GlobalOp global = mlir::LLVM::GlobalOp::create(
      rewriter, loc, toolchainPtrTy, /*constant=*/true,
...
struct EmboxCommonConversion : public fir::FIROpConversion<OP> {
  ...
  mlir::Value insertField(mlir::ConversionPatternRewriter &rewriter,
                          ...
                          toolchain::ArrayRef<std::int64_t> fldIndexes,
  ...
  mlir::Value populateDescriptor(mlir::Location loc, ModOpTy mod,
    ...
    auto toolchainBoxTy = this->lowerTy().convertBoxTypeAsStruct(boxTy, rank);
    ...
    mlir::Value descriptor =
        mlir::LLVM::UndefOp::create(rewriter, loc, toolchainBoxTy);
    descriptor =
    ...
    if (extraField) {
      ...
      if (hasAddendum) {
        auto maskAttr = mlir::IntegerAttr::get(
            ...
            toolchain::APInt(8, (uint64_t)_CFI_ADDENDUM_FLAG, /*isSigned=*/false));
        ...
        auto maskAttr = mlir::IntegerAttr::get(
            rewriter.getIntegerType(8, /*isSigned=*/false),
            toolchain::APInt(8, (uint64_t)~_CFI_ADDENDUM_FLAG, /*isSigned=*/true));
        mlir::LLVM::ConstantOp mask = mlir::LLVM::ConstantOp::create(
            rewriter, loc, rewriter.getI8Type(), maskAttr);
  ...
  consDescriptorPrefix(BOX box, mlir::Type inputType,
    ...
    toolchain::SmallVector<mlir::Value> typeparams = lenParams;
  ...
  genBoxOffsetGep(mlir::ConversionPatternRewriter &rewriter, mlir::Location loc,
                  mlir::Value base, mlir::Type toolchainBaseObjectType,
    ...
    toolchain::SmallVector<mlir::LLVM::GEPArg> gepArgs{outerOffset};
    mlir::Type resultTy = toolchainBaseObjectType;
    // Fortran is column major, toolchain GEP is row major: reverse the indices here.
    for (mlir::Value interiorIndex : toolchain::reverse(cstInteriorIndices)) {
    ...
    toolchain::SmallVector<mlir::Value> gepIndices =
    ...
    if (substringOffset) {
      if (auto arrayTy = mlir::dyn_cast<mlir::LLVM::LLVMArrayType>(resultTy)) {
        ...
        // degenerated to an toolchain.ptr<i[width]>, and there should not be any
        ...
        mlir::Type outterOffsetTy =
            toolchain::cast<mlir::Value>(gepArgs[0]).getType();
        mlir::Value cast =
            this->integerCast(loc, rewriter, outterOffsetTy, *substringOffset);

        gepArgs[0] = mlir::LLVM::AddOp::create(
            rewriter, loc, outterOffsetTy, toolchain::cast<mlir::Value>(gepArgs[0]),
            cast);
      }
    }
    mlir::Type toolchainPtrTy = ::getLlvmPtrType(resultTy.getContext());
    return mlir::LLVM::GEPOp::create(rewriter, loc, toolchainPtrTy,
                                     toolchainBaseObjectType, base, gepArgs);
  }
  ...
  placeInMemoryIfNotGlobalInit(mlir::ConversionPatternRewriter &rewriter,
    ...
    if (isInGlobalOp(rewriter))
      return boxValue;
    mlir::Type toolchainBoxTy = boxValue.getType();
    mlir::Value storage;
    if (needDeviceAllocation) {
      ...
      storage = this->genAllocaAndAddrCastWithType(loc, toolchainBoxTy, defaultAlign,
                                                   rewriter);
    }
    auto storeOp =
...
static bool isDeviceAllocation(mlir::Value val, mlir::Value adaptorVal) {
  ...
  if (!val.getDefiningOp() && adaptorVal) {
    if (auto blockArg = toolchain::cast<mlir::BlockArgument>(adaptorVal)) {
...
struct XEmboxOpConversion : public EmboxCommonConversion<fir::cg::XEmboxOp> {
  ...
  matchAndRewrite(fir::cg::XEmboxOp xbox, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<mlir::Value> cstInteriorIndices;
    ...
    if (fir::hasDynamicSize(seqEleTy)) {
      if (auto charTy = mlir::dyn_cast<fir::CharacterType>(seqEleTy)) {
        // The GEP pointer type decays to toolchain.ptr<i[width]>.
    ...
    if (hasSlice || hasSubcomp || hasSubstr) {
      ...
      toolchain::SmallVector<mlir::Value> fieldIndices;
      ...
      mlir::Type toolchainBaseType =
          convertType(fir::unwrapRefType(xbox.getMemref().getType()));
      base = genBoxOffsetGep(rewriter, loc, base, toolchainBaseType, ptrOffset,
...
struct XReboxOpConversion : public EmboxCommonConversion<fir::cg::XReboxOp> {
  ...
  matchAndRewrite(fir::cg::XReboxOp rebox, OpAdaptor adaptor,
    ...
    // Inside a fir.global, the input box was produced as an toolchain.struct<>
    ...
    // fir.box was translated to an toolchain.ptr<toolchain.struct<>> and the MLIR pass
    ...
    toolchain::SmallVector<mlir::Value, 2> lenParams;
    ...
    toolchain::SmallVector<mlir::Value> inputExtents;
    toolchain::SmallVector<mlir::Value> inputStrides;
    const unsigned inputRank = rebox.getRank();
    for (unsigned dim = 0; dim < inputRank; ++dim) {
      toolchain::SmallVector<mlir::Value, 3> dimInfo =
...
private:
  ...
  finalizeRebox(fir::cg::XReboxOp rebox, OpAdaptor adaptor,
    ...
    for (auto iter : toolchain::enumerate(toolchain::zip(extents, strides))) {
  ...
  sliceBox(fir::cg::XReboxOp rebox, OpAdaptor adaptor, mlir::Type destBoxTy,
    ...
    if (!rebox.getSubcomponent().empty() || !rebox.getSubstr().empty()) {
      ...
      mlir::Type inputEleTy = getInputEleTy(rebox);
      mlir::Type toolchainBaseObjectType = convertType(inputEleTy);
      toolchain::SmallVector<mlir::Value> fieldIndices;
      std::optional<mlir::Value> substringOffset;
      ...
      base = genBoxOffsetGep(rewriter, loc, base, toolchainBaseObjectType, zero,
    ...
    // and strides.
    toolchain::SmallVector<mlir::Value> slicedExtents;
    toolchain::SmallVector<mlir::Value> slicedStrides;
  ...
  reshapeBox(fir::cg::XReboxOp rebox, OpAdaptor adaptor, mlir::Type destBoxTy,
    ...
    toolchain::SmallVector<mlir::Value> newStrides;
    toolchain::SmallVector<mlir::Value> newExtents;
    mlir::Type idxTy = lowerTy().indexType();
...
struct ValueOpCommon {
  ...
  static void toRowMajor(toolchain::SmallVectorImpl<int64_t> &indices,
                         mlir::Type ty) {
    ...
    for (std::remove_const_t<decltype(end)> i = 0; i < end; ++i) {
      if (auto seq = mlir::dyn_cast<mlir::LLVM::LLVMArrayType>(ty)) {
        ...
        toolchain_unreachable("index into invalid type");
      }
    }
  }

  static toolchain::SmallVector<int64_t>
  collectIndices(mlir::ConversionPatternRewriter &rewriter,
                 mlir::ArrayAttr arrAttr) {
    toolchain::SmallVector<int64_t> indices;
...
struct InsertOnRangeOpConversion
  ...
  void incrementSubscripts(toolchain::ArrayRef<int64_t> dims,
                           toolchain::SmallVectorImpl<int64_t> &subscripts) const {
  ...
  doRewrite(fir::InsertOnRangeOp range, mlir::Type ty, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<std::int64_t> dims;
    ...
    if (range.isFullRange()) {
      toolchain::FailureOr<mlir::Attribute> cst =
          fir::tryFoldingLLVMInsertChain(adaptor.getVal(), rewriter);
      if (toolchain::succeeded(cst)) {
        mlir::Attribute dimVal = *cst;
        for (auto dim : toolchain::reverse(dims)) {
    ...
    // insert_range into an toolchain.insertvalue chain.
    ...
    auto reversedCoor = toolchain::reverse(coor.getValues<int64_t>());
...
struct XArrayCoorOpConversion
  ...
  toolchain::LogicalResult
  doRewrite(fir::cg::XArrayCoorOp coor, mlir::Type toolchainPtrTy, OpAdaptor adaptor,
            mlir::ConversionPatternRewriter &rewriter) const override {
    ...
    if (baseIsBoxed) {
      ...
      toolchain::SmallVector<mlir::LLVM::GEPArg> args{offset};
      auto addr = mlir::LLVM::GEPOp::create(rewriter, loc, toolchainPtrTy, byteTy,
                                            base, args);
      ...
      toolchain::SmallVector<mlir::Value> indices = convertSubcomponentIndices(
          loc, elementType,
          operands.slice(coor.getSubcomponentOperandIndex(),
                         coor.getSubcomponent().size()));
      args.append(indices.begin(), indices.end());
      rewriter.replaceOpWithNewOp<mlir::LLVM::GEPOp>(coor, toolchainPtrTy,
    ...
    toolchain::SmallVector<mlir::LLVM::GEPArg> args;
    if (coor.getSubcomponent().empty()) {
      // No subcomponent.
      if (!coor.getLenParams().empty()) {
    ...
    rewriter.replaceOpWithNewOp<mlir::LLVM::GEPOp>(
        coor, toolchainPtrTy, gepObjectType, adaptor.getMemref(), args);
    return mlir::success();
  }
};
} // namespace
...
struct CoordinateOpConversion
  ...
  doRewrite(fir::CoordinateOp coor, mlir::Type ty, OpAdaptor adaptor,
    ...
    mlir::Type toolchainObjectTy = convertType(objectTy);
    ...
    if (fir::isa_complex(objectTy)) {
      mlir::Value gep =
          genGEP(loc, toolchainObjectTy, rewriter, base, 0, operands[1]);
    ...
    if (mlir::isa<fir::ReferenceType, fir::PointerType, fir::HeapType>(
            baseObjectTy))
      return doRewriteRefOrPtr(coor, toolchainObjectTy, operands, loc, rewriter);
  ...
  arraysHaveKnownShape(mlir::Type type, fir::CoordinateOp coor) {
    ...
    for (auto it = begin, end = indices.end(); it != end;) {
      if (auto arrTy = mlir::dyn_cast<fir::SequenceType>(type)) {
        ...
        for (auto dimExtent : toolchain::enumerate(arrTy.getShape())) {
      ...
      if (auto strTy = mlir::dyn_cast<fir::RecordType>(type)) {
        auto intAttr = toolchain::dyn_cast<mlir::IntegerAttr>(*it);
        if (!intAttr) {
        ...
        auto value = toolchain::dyn_cast<mlir::Value>(*it);
        if (!value) {
          mlir::emitError(
              coor.getLoc(),
...
private:
  ...
  doRewriteBox(fir::CoordinateOp coor, mlir::ValueRange operands,
    ...
    mlir::Type toolchainPtrTy = ::getLlvmPtrType(coor.getContext());
    mlir::Type byteTy = ::getI8Type(coor.getContext());
    mlir::LLVM::IntegerOverflowFlags nsw =
        mlir::LLVM::IntegerOverflowFlags::nsw;
    ...
    for (auto it = indices.begin(), end = indices.end(); it != end;) {
      if (auto arrTy = mlir::dyn_cast<fir::SequenceType>(cpnTy)) {
        ...
        resultAddr = mlir::LLVM::GEPOp::create(
            rewriter, loc, toolchainPtrTy, byteTy, resultAddr,
            toolchain::ArrayRef<mlir::LLVM::GEPArg>{off});
        ...
        auto toolchainRecTy = lowerTy().convertType(recTy);
        resultAddr = mlir::LLVM::GEPOp::create(
            rewriter, loc, toolchainPtrTy, toolchainRecTy, resultAddr,
            toolchain::ArrayRef<mlir::LLVM::GEPArg>{0, fieldIndex});
      } else {
  ...
  toolchain::LogicalResult
  doRewriteRefOrPtr(fir::CoordinateOp coor, mlir::Type toolchainObjectTy,
    ...
    if (shapeAnalysis->hasKnownShape || shapeAnalysis->columnIsDeferred) {
      toolchain::SmallVector<mlir::LLVM::GEPArg> offs;
      ...
      toolchain::SmallVector<mlir::Value> arrIdx;
      int nextIndexValue = 1;
      for (auto index : coor.getIndices()) {
        if (auto intAttr = toolchain::dyn_cast<mlir::IntegerAttr>(index)) {
          // Addressing derived type component.
          auto recordType = toolchain::dyn_cast<fir::RecordType>(cpnTy);
          if (!recordType)
            return mlir::emitError(
                loc,
        ...
        if (auto tupTy = mlir::dyn_cast<mlir::TupleType>(cpnTy)) {
          ...
          if (!dims) {
            if (auto arrayType = toolchain::dyn_cast<fir::SequenceType>(cpnTy)) {
      ...
      mlir::Value base = operands[0];
      mlir::Value retval = genGEP(loc, toolchainObjectTy, rewriter, base, offs);
...
/// Lower `fir.has_value` operation to `toolchain.return` operation.
...
/// Lower `fir.global` operation to `toolchain.global` operation.
...
struct GlobalOpConversion : public fir::FIROpConversion<fir::GlobalOp> {
  ...
  matchAndRewrite(fir::GlobalOp global, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<mlir::Attribute> dbgExprs;
    ...
    // other ways.
    toolchain::SmallDenseSet<toolchain::StringRef> elidedAttrsSet(
  ...
  // enumeration.
  mlir::LLVM::Linkage
  convertLinkage(std::optional<toolchain::StringRef> optLinkage) const {
    if (optLinkage) {
...
private:
  static void addComdat(mlir::LLVM::GlobalOp &global,
    ...
    const char *comdatName = "__toolchain_comdat";
...
struct LoadOpConversion : public fir::FIROpConversion<fir::LoadOp> {
  ...
  matchAndRewrite(fir::LoadOp load, OpAdaptor adaptor,
    ...
    mlir::Type toolchainLoadTy = convertObjectType(load.getType());
    const bool isVolatile = fir::isa_volatile_type(load.getMemref().getType());
    if (auto boxTy = mlir::dyn_cast<fir::BaseBoxType>(load.getType())) {
      // fir.box is a special case because it is considered an ssa value in
      // fir, but it is lowered as a pointer to a descriptor. So
      // fir.ref<fir.box> and fir.box end up being the same toolchain types and
      ...
      if (!newBoxStorage)
        newBoxStorage = genAllocaAndAddrCastWithType(loc, toolchainLoadTy,
                                                     defaultAlign, rewriter);

      TypePair boxTypePair{boxTy, toolchainLoadTy};
      mlir::Value boxSize =
      ...
      mlir::LLVM::LoadOp loadOp =
          mlir::LLVM::LoadOp::create(rewriter, load.getLoc(), toolchainLoadTy,
...
struct SelectCaseOpConversion : public fir::FIROpConversion<fir::SelectCaseOp> {
  ...
  matchAndRewrite(fir::SelectCaseOp caseOp, OpAdaptor adaptor,
    ...
    toolchain::ArrayRef<mlir::Attribute> cases = caseOp.getCases().getValue();
...
private:
  ...
  toolchain::FailureOr<mlir::Block *>
...
protected:
  ...
  selectMatchAndRewrite(OP select, typename OP::Adaptor adaptor,
    ...
    toolchain::SmallVector<mlir::Block *> destinations;
    toolchain::SmallVector<mlir::ValueRange> destinationsOperands;
    ...
    toolchain::SmallVector<int64_t> caseValues;
    ...
    rewriter.replaceOpWithNewOp<mlir::LLVM::SwitchOp>(
        ...
        /*branchWeights=*/toolchain::ArrayRef<std::int32_t>());
    return mlir::success();
  }
};
...
struct StoreOpConversion : public fir::FIROpConversion<fir::StoreOp> {
  ...
  matchAndRewrite(fir::StoreOp store, OpAdaptor adaptor,
    ...
    mlir::Value toolchainValue = adaptor.getValue();
    mlir::Value toolchainMemref = adaptor.getMemref();
    mlir::LLVM::AliasAnalysisOpInterface newOp;
    const bool isVolatile =
        fir::isa_volatile_type(store.getMemref().getType()) ||
        fir::isa_volatile_type(store.getValue().getType());
    if (auto boxTy = mlir::dyn_cast<fir::BaseBoxType>(storeTy)) {
      mlir::Type toolchainBoxTy = lowerTy().convertBoxTypeAsStruct(boxTy);
      // Always use memcpy because LLVM is not as effective at optimizing
      // aggregate loads/stores as it is optimizing memcpy.
      TypePair boxTypePair{boxTy, toolchainBoxTy};
      mlir::Value boxSize =
          computeBoxSize(loc, boxTypePair, toolchainValue, rewriter);
      newOp = mlir::LLVM::MemcpyOp::create(rewriter, loc, toolchainMemref, toolchainValue,
                                           boxSize, isVolatile);
    } else {
      mlir::LLVM::StoreOp storeOp =
          mlir::LLVM::StoreOp::create(rewriter, loc, toolchainValue, toolchainMemref);
...
/// `fir.copy` --> `toolchain.memcpy` or `toolchain.memmove`
struct CopyOpConversion : public fir::FIROpConversion<fir::CopyOp> {
  ...
  matchAndRewrite(fir::CopyOp copy, OpAdaptor adaptor,
    ...
    mlir::Value toolchainSource = adaptor.getSource();
    mlir::Value toolchainDestination = adaptor.getDestination();
    ...
    if (copy.getNoOverlap())
      newOp = mlir::LLVM::MemcpyOp::create(rewriter, loc, toolchainDestination,
                                           toolchainSource, copySize, isVolatile);
    else
      newOp = mlir::LLVM::MemmoveOp::create(rewriter, loc, toolchainDestination,
                                            toolchainSource, copySize, isVolatile);
...
/// Convert `fir.unboxchar` into two `toolchain.extractvalue` instructions. One for
...
struct UnboxCharOpConversion : public fir::FIROpConversion<fir::UnboxCharOp> {
  ...
  matchAndRewrite(fir::UnboxCharOp unboxchar, OpAdaptor adaptor,
    ...
    rewriter.replaceOp(unboxchar,
                       toolchain::ArrayRef<mlir::Value>{ptrToBuffer, lenAfterCast});
    return mlir::success();
  }
};
...
/// `fir.unreachable` --> `toolchain.unreachable`
...
/// `fir.is_present` -->
/// ```
///  %0 = toolchain.mlir.constant(0 : i64)
///  %1 = toolchain.ptrtoint %0
///  %2 = toolchain.icmp "ne" %1, %0 : i64
/// ```
...
/// `fir.absent !fir.ref<i64>` -->  `toolchain.mlir.zero : !toolchain.ptr<i64>`
...
struct BoxOffsetOpConversion : public fir::FIROpConversion<fir::BoxOffsetOp> {
  ...
  matchAndRewrite(fir::BoxOffsetOp boxOffset, OpAdaptor adaptor,
    ...
    mlir::Type toolchainBoxTy;
    int fieldId;
    if (auto boxType = mlir::dyn_cast_or_null<fir::BaseBoxType>(boxRefType)) {
      toolchainBoxTy = lowerTy().convertBoxTypeAsStruct(
      ...
      toolchainBoxTy = lowerTy().convertType(boxCharType);
      fieldId = kAddrPosInBox;
    }
    rewriter.replaceOpWithNewOp<mlir::LLVM::GEPOp>(
        boxOffset, pty, toolchainBoxTy, adaptor.getBoxRef(),
        toolchain::ArrayRef<mlir::LLVM::GEPArg>{0, fieldId});
    return mlir::success();
  }
};
...
public:
  ...
  void runOnOperation() override final {
    ...
    if (!forcedDataLayout.empty()) {
      toolchain::DataLayout dl(forcedDataLayout);
    ...
    // operations into other dialects (toolchain, func, etc.).
    ...
    if (options.ComplexRange ==
        language::Compability::frontend::CodeGenOptions::ComplexRangeKind::CX_Basic) {
      complexToStandardOptions.complexRange =
          mlir::complex::ComplexRangeFlags::basic;
    } else if (options.ComplexRange == language::Compability::frontend::CodeGenOptions::
...
struct LLVMIRLoweringPass
  ...
  LLVMIRLoweringPass(toolchain::raw_ostream &output, fir::LLVMIRLoweringPrinter p)
  ...
  void runOnOperation() override final {
    ...
    if (auto toolchainModule = mlir::translateModuleToLLVMIR(
            getModule(), toolchainCtx, optName ? *optName : "FIRModule")) {
      printer(*toolchainModule, output);
      return;
    }
...
std::unique_ptr<mlir::Pass>
fir::createLLVMDialectToLLVMPass(toolchain::raw_ostream &output,
                                 fir::LLVMIRLoweringPrinter printer) {
