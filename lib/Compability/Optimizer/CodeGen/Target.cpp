...
toolchain::StringRef Attributes::getIntExtensionAttrName() const {
  ...
  if (isZeroExt())
    return "toolchain.zeroext";
  else if (isSignExt())
    return "toolchain.signext";
  return {};
}
} // namespace fir::details

// Reduce a REAL/float type to the floating point semantics.
static const toolchain::fltSemantics &floatToSemantics(const KindMapping &kindMap,
                                                  mlir::Type type) {
  assert(isa_real(type));
  return mlir::cast<mlir::FloatType>(type).getFloatSemantics();
}

static void typeTodo(const toolchain::fltSemantics *sem, mlir::Location loc,
...
struct TargetI386 : public GenericTarget<TargetI386> {
  ...
  complexReturnType(mlir::Location loc, mlir::Type eleTy) const override {
    ...
    if (sem == &toolchain::APFloat::IEEEsingle()) {
...
struct TargetX86_64 : public GenericTarget<TargetX86_64> {
  ...
  void classify(mlir::Location loc, mlir::Type type, std::uint64_t byteOffset,
    ...
    toolchain::TypeSwitch<mlir::Type>(type)
        ...
        .template Case<mlir::FloatType>([&](mlir::Type floatTy) {
          ...
          if (sem == &toolchain::APFloat::x87DoubleExtended()) {
            Lo = ArgClass::X87;
            Hi = ArgClass::X87Up;
          } else if (sem == &toolchain::APFloat::IEEEquad()) {
            Lo = ArgClass::SSE;
        ...
        .template Case<fir::VectorType>([&](fir::VectorType vecTy) {
          ...
          if (!(sem == &toolchain::APFloat::IEEEsingle() && vecTy.getLen() <= 2) &&
              !(sem == &toolchain::APFloat::IEEEhalf() && vecTy.getLen() <= 4))
  ...
  void classifyArray(mlir::Location loc, fir::SequenceType seqTy,
    ...
    std::uint64_t eleStorageSize = toolchain::alignTo(eleSize, eleAlign);
    for (std::uint64_t i = 0; i < arraySize; ++i) {
      byteOffset = toolchain::alignTo(byteOffset, eleAlign);
      if (byteOffset > 16) {
    ...
    if (sem == &toolchain::APFloat::IEEEsingle()) {
      ...
      marshal.emplace_back(
          ...
          AT{/*align=*/8, /*byval=*/true});
               ...
               sem == &toolchain::APFloat::x87DoubleExtended()) {
      // Use a type that will be translated into LLVM as:
...
struct TargetAArch64 : public GenericTarget<TargetAArch64> {
  ...
  complexArgumentType(mlir::Location loc, mlir::Type eleTy) const override {
    ...
    const auto *sem = &floatToSemantics(kindMap, eleTy);
    if (sem == &toolchain::APFloat::IEEEsingle() ||
        sem == &toolchain::APFloat::IEEEdouble() ||
        sem == &toolchain::APFloat::IEEEquad()) {
  ...
  flattenTypeList(const RecordType::TypeList &types) {
    ...
    for (auto [c, type] : types) {
      ...
      if (auto recTy = mlir::dyn_cast<RecordType>(type)) {
        ...
        if (!subTypeList)
          return std::nullopt;
        toolchain::copy(*subTypeList, std::back_inserter(flatTypes));
        continue;
      }
  ...
  static std::optional<int> usedRegsForHFA(fir::RecordType ty) {
    ...
    return toolchain::all_equal(*flatTypes) ? std::optional<int>{flatTypes->size()}
                                       : std::nullopt;
  }
  ...
  NRegs usedRegsForType(mlir::Location loc, mlir::Type type) const {
    return toolchain::TypeSwitch<mlir::Type, NRegs>(type)
...
struct TargetLoongArch64 : public GenericTarget<TargetLoongArch64> {
  ...
  toolchain::SmallVector<mlir::Type> flattenTypeList(mlir::Location loc,
                                                const mlir::Type type) const {
    toolchain::SmallVector<mlir::Type> flatTypes;
        ...
        .template Case<fir::SequenceType>([&](fir::SequenceType seqTy) {
          if (!seqTy.hasDynamicExtents()) {
            ...
            if (!mlir::isa<mlir::IntegerType, mlir::FloatType>(eleTy)) {
              toolchain::SmallVector<mlir::Type> subTypeList =
  ...
  bool detectFARsEligibleStruct(mlir::Location loc, fir::RecordType recTy,
    ...
    toolchain::SmallVector<mlir::Type> flatTypes = flattenTypeList(loc, recTy);
  ...
  bool hasEnoughRegisters(mlir::Location loc, int GARsLeft, int FARsLeft,
    ...
    for (auto &typeAndAttr : previousArguments) {
      ...
      toolchain::SmallVector<mlir::Type> flatTypes = flattenTypeList(loc, type);
...
fir::CodeGenSpecifics::get(mlir::MLIRContext *ctx, toolchain::Triple &&trp,
  ...
  case toolchain::Triple::ArchType::ppc64le:
    return std::make_unique<TargetPPC64le>(
        ctx, std::move(trp), std::move(kindMap), targetCPU, targetFeatures, dl);
  case toolchain::Triple::ArchType::sparc:
    return std::make_unique<TargetSparc>(
        ctx, std::move(trp), std::move(kindMap), targetCPU, targetFeatures, dl);
  case toolchain::Triple::ArchType::sparcv9:
  ...
  case toolchain::Triple::ArchType::amdgcn:
    return std::make_unique<TargetAMDGPU>(
        ctx, std::move(trp), std::move(kindMap), targetCPU, targetFeatures, dl);
  case toolchain::Triple::ArchType::nvptx64:
    return std::make_unique<TargetNVPTX>(
        ctx, std::move(trp), std::move(kindMap), targetCPU, targetFeatures, dl);
  case toolchain::Triple::ArchType::loongarch64:
...
std::unique_ptr<fir::CodeGenSpecifics> fir::CodeGenSpecifics::get(
    mlir::MLIRContext *ctx, toolchain::Triple &&trp, KindMapping &&kindMap,
