...
private:
  static constexpr toolchain::StringRef bufferName = ".repacked";
...
mlir::Value PackArrayConversion::allocateTempBuffer(
    ...
    mlir::Value origBox, toolchain::ArrayRef<mlir::Value> lbounds,
    toolchain::ArrayRef<mlir::Value> extents,
    toolchain::ArrayRef<mlir::Value> typeParams) {
...
PackArrayConversion::genRepackedBox(fir::FirOpBuilder &builder,
  ...
  toolchain::SmallVector<mlir::Value> typeParams(op.getTypeparams().begin(),
  ...
  toolchain::SmallVector<mlir::Value, language::Compability::common::maxRank> lbounds, extents;
  fir::factory::genDimInfoFromBox(builder, loc, box, &lbounds, &extents,
                                  /*strides=*/nullptr);
  // Get the type parameters from the box, if needed.
  toolchain::SmallVector<mlir::Value> assumedTypeParams;
  if (numTypeParams != 0) {
    if (auto charType =
            mlir::dyn_cast<fir::CharacterType>(boxType.unwrapInnerType()))
      if (charType.hasDynamicLen()) {
