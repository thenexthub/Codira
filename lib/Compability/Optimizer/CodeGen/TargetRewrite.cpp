...
public:
  ...
  void runOnOperation() override final {
    ...
    mod.walk([&](mlir::Operation *op) {
      if (auto call = mlir::dyn_cast<fir::CallOp>(op)) {
        ...
        toolchain::SmallVector<mlir::Type> operandsTypes;
  ...
  void passArgumentOnStackOrWithNewType(
      ...
      toolchain::SmallVectorImpl<mlir::Value> &newOpers,
      mlir::Value &savedStackPtr) {
  ...
  void passSplitArgument(mlir::Location loc,
    ...
    if (!fir::isa_complex(oldType)) {
      ...
      // via memory.
      toolchain::SmallVector<mlir::Type> partTypes;
    ...
    for (auto e : toolchain::enumerate(splitArgs)) {
      auto &tup = e.value();
  ...
  void rewriteCallOperands(
      ...
      toolchain::SmallVectorImpl<mlir::Value> &newOpers, mlir::Value &savedStackPtr,
      fir::CodeGenSpecifics::Marshalling &newInTyAndAttrs) {
    if (passArgAs.size() == 1) {
  ...
  static bool hasByValOrSRetArgs(
    ...
    return toolchain::any_of(newInTyAndAttrs, [](auto arg) {
  ...
  void convertCallOp(A callOp, mlir::FunctionType fnTy) {
    ...
    toolchain::SmallVector<mlir::Type> newResTys;
    fir::CodeGenSpecifics::Marshalling newInTyAndAttrs;
    toolchain::SmallVector<mlir::Value> newOpers;
    ...
    if (fnTy.getResults().size() == 1) {
      ...
      toolchain::TypeSwitch<mlir::Type>(ty)
    ...
    toolchain::SmallVector<mlir::Type> trailingInTys;
    toolchain::SmallVector<mlir::Value> trailingOpers;
    ...
    for (auto e : toolchain::enumerate(
             toolchain::zip(fnTy.getInputs().drop_front(dropFront), operands))) {
      ...
      toolchain::TypeSwitch<mlir::Type>(ty)
          .template Case<fir::BoxCharType>([&](fir::BoxCharType boxTy) {
            ...
            for (auto e : toolchain::enumerate(m)) {
          ...
          .template Case<mlir::TupleType>([&](mlir::TupleType tuple) {
            if (fir::isCharacterProcedureTuple(tuple)) {
              ...
              if constexpr (std::is_same_v<std::decay_t<A>, fir::CallOp>) {
                if (callOp.getCallee()) {
                  toolchain::StringRef charProcAttr =
    ...
    toolchain::SmallVector<mlir::Type> newInTypes = toTypeList(newInTyAndAttrs);
    newInTypes.insert(newInTypes.end(), trailingInTys.begin(),
                      trailingInTys.end());
    newOpers.insert(newOpers.end(), trailingOpers.begin(), trailingOpers.end());

    toolchain::SmallVector<mlir::Value, 1> newCallResults;
    ...
    if constexpr (std::is_same_v<std::decay_t<A>, mlir::gpu::LaunchFuncOp>) {
      ...
      // https://toolchain.org/docs/LangRef.html#parameter-attributes.
      if (hasByValOrSRetArgs(newInTyAndAttrs)) {
        toolchain::SmallVector<mlir::Attribute> argAttrsArray;
        for (const auto &arg :
             toolchain::ArrayRef<fir::CodeGenSpecifics::TypeAndAttr>(newInTyAndAttrs)
      ...
      LLVM_DEBUG(toolchain::dbgs() << "replacing call with " << newCall << '\n');
      if (wrap)
  ...
  toolchain::LogicalResult convertTypes(mlir::ModuleOp mod) {
  ...
  bool hasPortableSignature(mlir::Type signature, mlir::Operation *op) {
    ...
    for (auto ty : func.getResults())
      if ((mlir::isa<fir::BoxCharType>(ty) && !noCharacterConversion) ||
        ...
        LLVM_DEBUG(toolchain::dbgs() << "rewrite " << signature << " for target\n");
        return false;
      }
  ...
  void convertSignature(FuncOpTy func) {
    ...
    toolchain::SmallVector<mlir::Type> newResTys;
    fir::CodeGenSpecifics::Marshalling newInTyAndAttrs;
    toolchain::SmallVector<std::pair<unsigned, mlir::NamedAttribute>> savedAttrs;
    toolchain::SmallVector<std::pair<unsigned, mlir::NamedAttribute>> extraAttrs;
    toolchain::SmallVector<FixupTy> fixups;
    toolchain::SmallVector<std::pair<unsigned, mlir::NamedAttrList>, 1> resultAttrs;

    // Save argument attributes in case there is a shift so we can replace them
    // correctly.
    for (auto e : toolchain::enumerate(funcTy.getInputs())) {
      unsigned index = e.index();
    ...
    for (auto ty : funcTy.getResults())
      toolchain::TypeSwitch<mlir::Type>(ty)
          ...
          .template Case<mlir::IntegerType>([&](mlir::IntegerType intTy) {
            ...
            toolchain::StringRef extensionAttrName = attr.getIntExtensionAttrName();
    ...
    // Convert arguments
    toolchain::SmallVector<mlir::Type> trailingTys;
    for (auto e : toolchain::enumerate(funcTy.getInputs())) {
      auto ty = e.value();
      unsigned index = e.index();
      toolchain::TypeSwitch<mlir::Type>(ty)
      ...
      if (func.template getArgAttrOfType<mlir::UnitAttr>(
        ...
        extraAttrs.push_back(
            {newInTyAndAttrs.size() - 1,
             rewriter->getNamedAttr("toolchain.nest", rewriter->getUnitAttr())});
      }
    }
    ...
    LLVM_DEBUG(toolchain::dbgs() << "new func: " << newFuncTy << '\n');
      ...
      if (auto align = attr.getAlignment())
        fixups.emplace_back(
            FixupTy::Codes::ReturnAsStore, argNo, [=](OpTy func) {
              ...
              func.setArgAttr(argNo, "toolchain.sret",
                              mlir::TypeAttr::get(elemType));
              func.setArgAttr(argNo, "toolchain.align",
    ...
    if (auto align = attr.getAlignment())
      fixups.emplace_back(
          FixupTy::Codes::ReturnType, newResTys.size(), [=](OpTy func) {
            func.setArgAttr(
                newResTys.size(), "toolchain.align",
                rewriter->getIntegerAttr(rewriter->getIntegerType(32), align));
          });
    else
  ...
  void createFuncOpArgFixups(
    ...
    for (auto e : toolchain::enumerate(argsInTys)) {
      auto &tup = e.value();
      auto index = e.index();
      auto attr = std::get<fir::CodeGenSpecifics::Attributes>(tup);
      auto argNo = newInTyAndAttrs.size();
        ...
        if (auto align = attr.getAlignment())
          fixups.emplace_back(FixupTy::Codes::ArgumentAsLoad, argNo,
                              [=](OpTy func) {
                                auto elemType = fir::dyn_cast_ptrOrBoxEleTy(
                                    func.getFunctionType().getInput(argNo));
                                func.setArgAttr(argNo, "toolchain.byval",
                                                mlir::TypeAttr::get(elemType));
                                func.setArgAttr(
                                    argNo, "toolchain.align",
                                    rewriter->getIntegerAttr(
                                        rewriter->getIntegerType(32), align));
                              });
        else
...
private:
  ...
  // Inserts a call to toolchain.stacksave at the current insertion
  ...
  // Inserts a call to toolchain.stackrestore at the current insertion
