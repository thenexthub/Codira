...
public:
  ...
  BoxprocTypeRewriter(mlir::Location location) : loc{location} {
    ...
    addConversion([&](mlir::TupleType tupTy) {
      toolchain::SmallVector<mlir::Type> memTys;
    ...
    addConversion([&](mlir::FunctionType funcTy) {
      toolchain::SmallVector<mlir::Type> inTys;
      toolchain::SmallVector<mlir::Type> resTys;
...
private:
  ...
  // is the whole pass.
  toolchain::DenseMap<mlir::Type, bool> visitedTypes;
  bool needConversionIsVisitingRecordType = false;
  toolchain::DenseMap<mlir::Type, mlir::Type> convertedTypes;
  mlir::Location loc;
};
    ...
    if (options.useThunks) {
      ...
      getModule().walk([&](mlir::Operation *op) {
        ...
        if (auto addr = mlir::dyn_cast<BoxAddrOp>(op)) {
          ...
          if (toolchain::isa<mlir::FunctionType>(ty) ||
              toolchain::isa<fir::BoxProcType>(ty)) {
          ...
          if (typeConverter.needsConversion(ty)) {
            ...
            if (!func.empty())
              for (auto e : toolchain::enumerate(toTy.getInputs())) {
                unsigned i = e.index();
          ...
          if (typeConverter.needsConversion(ty)) {
            ...
            toolchain::StringRef uniqName =
                mem.getUniqName().value_or(toolchain::StringRef());
            toolchain::StringRef bindcName =
                mem.getBindcName().value_or(toolchain::StringRef());
          ...
          for (auto i : toolchain::enumerate(op->getResultTypes()))
          ...
          for (const mlir::NamedAttribute &attr : op->getAttrDictionary())
            if (auto tyAttr = toolchain::dyn_cast<mlir::TypeAttr>(attr.getValue()))
