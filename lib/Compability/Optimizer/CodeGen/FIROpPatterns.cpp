...
ConvertFIRToLLVMPattern::ConvertFIRToLLVMPattern(
    toolchain::StringRef rootOpName, mlir::MLIRContext *context,
...
ConvertFIRToLLVMPattern::getBoxTypePair(mlir::Type firBoxTy) const {
  ...
  return TypePair{firBoxTy, toolchainBoxTy};
}
...
mlir::Value ConvertFIRToLLVMPattern::getValueFromBox(
  ...
  if (mlir::isa<mlir::LLVM::LLVMPointerType>(box.getType())) {
    ...
    auto p = mlir::LLVM::GEPOp::create(
        rewriter, loc, pty, boxTy.toolchain, box,
        toolchain::ArrayRef<mlir::LLVM::GEPArg>{0, boxValue});
    auto fldTy = getBoxEleTy(boxTy.toolchain, {boxValue});
...
/// from a box.
toolchain::SmallVector<mlir::Value, 3> ConvertFIRToLLVMPattern::getDimsFromBox(
    mlir::Location loc, toolchain::ArrayRef<mlir::Type> retTys, TypePair boxTy,
...
mlir::Value ConvertFIRToLLVMPattern::loadDimFieldFromBox(
  ...
  assert(mlir::isa<mlir::LLVM::LLVMPointerType>(box.getType()) &&
         ...
         "in memory");
  mlir::LLVM::GEPOp p = genGEP(loc, boxTy.toolchain, rewriter, box, 0,
...
mlir::Value ConvertFIRToLLVMPattern::getDimFieldFromBox(
  ...
  return mlir::LLVM::ExtractValueOp::create(
      rewriter, loc, box,
      toolchain::ArrayRef<std::int64_t>{kDimsPosInBox, dim, off});
}
...
mlir::Value ConvertFIRToLLVMPattern::getBaseAddrFromBox(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  mlir::Type resultTy = ::getLlvmPtrType(boxTy.toolchain.getContext());
...
mlir::Value ConvertFIRToLLVMPattern::getRankFromBox(
    mlir::Location loc, TypePair boxTy, mlir::Value box,
    mlir::ConversionPatternRewriter &rewriter) const {
  mlir::Type resultTy = getBoxEleTy(boxTy.toolchain, {kRankPosInBox});
...
mlir::Value ConvertFIRToLLVMPattern::getExtraFromBox(
    mlir::Location loc, TypePair boxTy, mlir::Value box,
    mlir::ConversionPatternRewriter &rewriter) const {
  mlir::Type resultTy = getBoxEleTy(boxTy.toolchain, {kExtraPosInBox});
...
mlir::Type ConvertFIRToLLVMPattern::getBoxEleTy(
    mlir::Type type, toolchain::ArrayRef<std::int64_t> indexes) const {
...
mlir::Value ConvertFIRToLLVMPattern::computeBoxSize(
  ...
  if (!firBoxType.isAssumedRank())
    return genConstantOffset(loc, rewriter, dl.getTypeSize(boxTy.toolchain));
  fir::BaseBoxType firScalarBoxType = firBoxType.getBoxTypeWithNewShape(0);
  mlir::Type toolchainScalarBoxType =
      lowerTy().convertBoxTypeAsStruct(firScalarBoxType);
  toolchain::TypeSize scalarBoxSizeCst = dl.getTypeSize(toolchainScalarBoxType);
  ...
  mlir::Type toolchainDimsType = getBoxEleTy(boxTy.toolchain, {kDimsPosInBox, 1});
  toolchain::TypeSize sizePerDimCst = dl.getTypeSize(toolchainDimsType);
...
mlir::Block *ConvertFIRToLLVMPattern::getBlockForAllocaInsert(
  ...
  if (auto toolchainFuncOp = mlir::dyn_cast<mlir::LLVM::LLVMFuncOp>(op))
    return &toolchainFuncOp.front();
...
mlir::Value ConvertFIRToLLVMPattern::genAllocaAndAddrCastWithType(
    mlir::Location loc, mlir::Type toolchainObjectTy, unsigned alignment,
    mlir::ConversionPatternRewriter &rewriter) const {
  ...
  mlir::Value al = mlir::LLVM::AllocaOp::create(
      rewriter, loc, ::getLlvmPtrType(toolchainObjectTy.getContext(), allocaAs),
      toolchainObjectTy, size, alignment);
  ...
  if (allocaAs != programAs) {
    al = mlir::LLVM::AddrSpaceCastOp::create(
        rewriter, loc, ::getLlvmPtrType(toolchainObjectTy.getContext(), programAs),
        al);
  }
...
unsigned ConvertFIRToLLVMPattern::getAllocaAddressSpace(
  ...
  if (auto module = parentOp->getParentOfType<mlir::ModuleOp>())
    if (mlir::Attribute addrSpace =
      ...
      return toolchain::cast<mlir::IntegerAttr>(addrSpace).getUInt();
