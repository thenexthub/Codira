...
static bool isEvaluateInMemoryBlockArg(mlir::Value v) {
  if (auto evalInMem = toolchain::dyn_cast_or_null<hlfir::EvaluateInMemoryOp>(
...
static bool isPrivateArg(omp::BlockArgOpenMPOpInterface &argIface,
  ...
  for (auto [opSym, blockArg] :
       toolchain::zip_equal(*op.getPrivateSyms(), argIface.getPrivateBlockArgs())) {
...
void AliasAnalysis::Source::print(toolchain::raw_ostream &os) const {
  if (auto v = toolchain::dyn_cast<mlir::Value>(origin.u))
    os << v;
  else if (auto gbl = toolchain::dyn_cast<mlir::SymbolRefAttr>(origin.u))
    os << gbl;
...
bool AliasAnalysis::Source::isFortranUserVariable() const {
  if (!origin.instantiationPoint)
    return false;
  return toolchain::TypeSwitch<mlir::Operation *, bool>(origin.instantiationPoint)
...
AliasResult AliasAnalysis::alias(Source lhsSrc, Source rhsSrc, mlir::Value lhs,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "\nAliasAnalysis::alias\n";
             ...
             toolchain::dbgs() << "  lhsSrc: " << lhsSrc << "\n";
             toolchain::dbgs() << "  rhs: " << rhs << "\n";
             toolchain::dbgs() << "  rhsSrc: " << rhsSrc << "\n";);

  // Indirect case currently not handled. Conservatively assume
  // it aliases with everything
  if (lhsSrc.kind >= SourceKind::Indirect ||
      rhsSrc.kind >= SourceKind::Indirect) {
    LLVM_DEBUG(toolchain::dbgs() << "  aliasing because of indirect access\n");
  ...
  if (lhsSrc.kind == rhsSrc.kind) {
    ...
    if (lhsSrc.kind == SourceKind::HostAssoc) {
      LLVM_DEBUG(toolchain::dbgs() << "  aliasing because of host association\n");
      return AliasResult::MayAlias;
    }
  }
  ...
  if (src1->isTargetOrPointer() && src2->isTargetOrPointer() &&
    ...
    LLVM_DEBUG(toolchain::dbgs() << "  aliasing because of target or pointer\n");
    return AliasResult::MayAlias;
  }
...
static bool isSavedLocal(const fir::AliasAnalysis::Source &src) {
  if (auto symRef = toolchain::dyn_cast<mlir::SymbolRefAttr>(src.origin.u)) {
...
ModRefResult AliasAnalysis::getModRef(Operation *op, Value location) {
  ...
  if (!interface) {
    if (auto call = toolchain::dyn_cast<fir::CallOp>(op))
  ...
  while (defOp && !breakFromLoop) {
    ...
    toolchain::TypeSwitch<Operation *>(defOp)
        ...
        .Case<fir::LoadOp>([&](auto op) {
          ...
          bool isDeclareOp =
              toolchain::isa_and_present<fir::DeclareOp>(loadMemrefOp) ||
              toolchain::isa_and_present<hlfir::DeclareOp>(loadMemrefOp);
          if (isDeclareOp &&
              toolchain::isa<omp::TargetOp>(loadMemrefOp->getParentOp())) {
            v = op.getMemref();
            defOp = v.getDefiningOp();
            return;
          }
          ...
          if (auto boxTy = mlir::dyn_cast<fir::BaseBoxType>(ty);
            ...
            global = toolchain::dyn_cast<mlir::SymbolRefAttr>(boxSrc.origin.u);
            if (global) {
              type = SourceKind::Global;
            } else {
              auto def = toolchain::cast<mlir::Value>(boxSrc.origin.u);
        ...
        .Case<fir::AddrOfOp>([&](auto op) {
          ...
          if (isPointerReference(ty))
            attributes.set(Attribute::Pointer);
          global = toolchain::cast<fir::AddrOfOp>(op).getSymbol();
          breakFromLoop = true;
        })
        .Case<hlfir::DeclareOp, fir::DeclareOp>([&](auto op) {
          ...
          if (omp::BlockArgOpenMPOpInterface argIface =
            ...
            Value ompValArg;
            toolchain::TypeSwitch<Operation *>(op->getParentOp())
                .template Case<omp::TargetOp>([&](auto targetOp) {
                  // If declare operation is inside omp target region,
                  // continue alias analysis outside the target region
                  for (auto [opArg, blockArg] : toolchain::zip_equal(
                           targetOp.getMapVars(), argIface.getMapBlockArgs())) {
                    if (blockArg == op.getMemref()) {
                      omp::MapInfoOp mapInfo =
                          toolchain::cast<omp::MapInfoOp>(opArg.getDefiningOp());
          ...
          auto varIf = toolchain::cast<fir::FortranVariableOpInterface>(defOp);
