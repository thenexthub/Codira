...
template <typename Ty>
std::optional<toolchain::TypeSize> OpenACCMappableModel<Ty>::getSizeInBytes(
  ...
  return {toolchain::TypeSize::getFixed(sizeAndAlignment->first)};
}

template std::optional<toolchain::TypeSize>
...
generateSeqTyAccBounds(fir::SequenceType seqType, mlir::Value var,
  ...
  if (seqType.hasDynamicExtents() || seqType.hasUnknownShape()) {
    ...
    if (mlir::isa<hlfir::DeclareOp, fir::DeclareOp>(var.getDefiningOp())) {
      ...
      toolchain::SmallVector<mlir::Value> accBounds;
      if (auto shapeOp =
        ...
        mlir::Value cummulativeExtent = one;
        for (auto [idx, val] : toolchain::enumerate(shapeShiftOp.getPairs())) {
          if (idx % 2 == 0) {
            lowerbound = val;
          } else {
...
template toolchain::SmallVector<mlir::Value>
...
mlir::Value OpenACCMappableModel<Ty>::generatePrivateInit(
    ...
    mlir::TypedValue<mlir::acc::MappableType> var, toolchain::StringRef varName,
  ...
  if (fir::isa_trivial(unwrappedTy)) {
    ...
    if (fir::isa_trivial(seqTy.getEleTy())) {
      ...
      if (seqTy.hasDynamicExtents()) {
        shape = fir::ShapeOp::create(firBuilder, loc, toolchain::to_vector(extents));
      } else {
