...
static toolchain::cl::opt<bool> forceNoAlias("force-no-alias", toolchain::cl::Hidden,
...
getEmissionKind(toolchain::codegenoptions::DebugInfoKind kind) {
  switch (kind) {
  case toolchain::codegenoptions::DebugInfoKind::FullDebugInfo:
    return mlir::LLVM::DIEmissionKind::Full;
  case toolchain::codegenoptions::DebugInfoKind::DebugLineTablesOnly:
...
void addDebugInfoPass(mlir::PassManager &pm,
  ...
  options.isOptimized = optLevel != toolchain::OptimizationLevel::O0;
...
void registerDefaultInlinerPass(MLIRToLLVMPassPipelineConfig &config) {
  config.registerFIRInlinerCallback(
      [](mlir::PassManager &pm, toolchain::OptimizationLevel level) {
        toolchain::StringMap<mlir::OpPassManager> pipelines;
...
void createHLFIRToFIRPassPipeline(mlir::PassManager &pm,
  ...
  if (optLevel.isOptimizingForSpeed()) {
    ...
    if (optLevel == toolchain::OptimizationLevel::O3) {
      addNestedPassToAllTopLevelOperations<PassConstructor>(
...
void createDebugPasses(mlir::PassManager &pm,
                       ...
                       toolchain::OptimizationLevel OptLevel,
                       toolchain::StringRef inputFilename) {
  if (debugLevel != toolchain::codegenoptions::NoDebugInfo)
...
void createDefaultFIRCodeGenPassPipeline(mlir::PassManager &pm,
  ...
  fir::addCodeGenRewritePass(
      pm, (config.DebugInfo != toolchain::codegenoptions::NoDebugInfo));
  ...
  if (config.FramePointerKind == toolchain::FramePointerKind::NonLeaf)
    framePointerKind = mlir::LLVM::framePointerKind::FramePointerKind::NonLeaf;
  else if (config.FramePointerKind == toolchain::FramePointerKind::All)
    framePointerKind = mlir::LLVM::framePointerKind::FramePointerKind::All;
  else if (config.FramePointerKind == toolchain::FramePointerKind::Reserved)
