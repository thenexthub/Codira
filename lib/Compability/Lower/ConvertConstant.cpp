...
static toolchain::APFloat consAPFloat(const toolchain::fltSemantics &fsem,
                                 toolchain::StringRef s) {
  assert(!s.contains(' '));
  if (s.compare_insensitive("-inf") == 0)
    return toolchain::APFloat::getInf(fsem, /*negative=*/true);
  if (s.compare_insensitive("inf") == 0 || s.compare_insensitive("+inf") == 0)
    return toolchain::APFloat::getInf(fsem);
  // TODO: Add support for quiet and signaling NaNs.
  if (s.compare_insensitive("-nan") == 0)
    return toolchain::APFloat::getNaN(fsem, /*negative=*/true);
  if (s.compare_insensitive("nan") == 0 || s.compare_insensitive("+nan") == 0)
    return toolchain::APFloat::getNaN(fsem);
  return {fsem, s};
}

//===----------------------------------------------------------------------===//
// language::Compability::lower::tryCreatingDenseGlobal implementation
//===----------------------------------------------------------------------===//
...
static mlir::Attribute convertToAttribute(
    ...
    const language::Compability::evaluate::Scalar<language::Compability::evaluate::Type<TC, KIND>> &value,
    mlir::Type type) {
  if constexpr (TC == language::Compability::common::TypeCategory::Integer) {
    if constexpr (KIND <= 8)
      return builder.getIntegerAttr(type, value.ToInt64());
    else {
      static_assert(KIND <= 16, "integers with KIND > 16 are not supported");
      return builder.getIntegerAttr(
          type, toolchain::APInt(KIND * 8,
                            {value.ToUInt64(), value.SHIFTR(64).ToUInt64()}));
    }
  } else if constexpr (TC == language::Compability::common::TypeCategory::Logical) {
    ...
    if constexpr (TC == language::Compability::common::TypeCategory::Real) {
      return getFloatAttr(value, type);
    } else {
      static_assert(TC == language::Compability::common::TypeCategory::Complex,
                    "type values cannot be converted to attributes");
      mlir::Type eleTy = mlir::cast<mlir::ComplexType>(type).getElementType();
      toolchain::SmallVector<mlir::Attribute, 2> attrs = {
...
public:
  static fir::GlobalOp tryCreating(fir::FirOpBuilder &builder,
    ...
    language::Compability::common::visit(
        language::Compability::common::visitors{
            [&](const language::Compability::evaluate::Expr<language::Compability::evaluate::SomeLogical> &
                    x) { globalBuilder.tryConvertingToAttributes(builder, x); },
            [&](const language::Compability::evaluate::Expr<language::Compability::evaluate::SomeInteger> &
                    x) { globalBuilder.tryConvertingToAttributes(builder, x); },
            [&](const language::Compability::evaluate::Expr<language::Compability::evaluate::SomeReal> &x) {
              globalBuilder.tryConvertingToAttributes(builder, x);
            },
            [&](const language::Compability::evaluate::Expr<language::Compability::evaluate::SomeComplex> &
                    x) { globalBuilder.tryConvertingToAttributes(builder, x); },
            [](const auto &) {},
        },
        initExpr.u);
  ...
  static fir::GlobalOp tryCreating(
      ...
      toolchain::StringRef globalName, mlir::StringAttr linkage, bool isConst,
      const language::Compability::evaluate::Constant<language::Compability::evaluate::Type<TC, KIND>>
          &constant,
      cuf::DataAttributeAttr dataAttr) {
    DenseGlobalBuilder globalBuilder;
...
private:
  ...
  void tryConvertingToAttributes(
      ...
      const language::Compability::evaluate::Constant<language::Compability::evaluate::Type<TC, KIND>>
          &constant) {
    static_assert(TC != language::Compability::common::TypeCategory::Character,
                  "must be numerical or logical");
    auto attrTc = TC == language::Compability::common::TypeCategory::Logical
                      ? language::Compability::common::TypeCategory::Integer
                      : TC;
    attributeElementType =
        language::Compability::lower::getFIRType(builder.getContext(), attrTc, KIND, {});
  ...
  void tryConvertingToAttributes(fir::FirOpBuilder &builder,
                                 const language::Compability::evaluate::Expr<SomeCat> &expr) {
    language::Compability::common::visit(
        [&](const auto &x) {
          using TR = language::Compability::evaluate::ResultType<decltype(x)>;
          if (const auto *constant =
                  std::get_if<language::Compability::evaluate::Constant<TR>>(&x.u))
  ...
  fir::GlobalOp tryCreatingGlobal(fir::FirOpBuilder &builder,
    ...
    toolchain::SmallVector<int64_t> tensorShape(arrTy.getShape());
...
fir::GlobalOp language::Compability::lower::tryCreatingDenseGlobal(
    fir::FirOpBuilder &builder, mlir::Location loc, mlir::Type symTy,
    toolchain::StringRef globalName, mlir::StringAttr linkage, bool isConst,
    const language::Compability::lower::SomeExpr &initExpr, cuf::DataAttributeAttr dataAttr) {
  return DenseGlobalBuilder::tryCreating(builder, loc, symTy, globalName,
                                         linkage, isConst, initExpr, dataAttr);
}

//===----------------------------------------------------------------------===//
// language::Compability::lower::convertConstant
...
static mlir::Value genRealConstant(fir::FirOpBuilder &builder,
                                   mlir::Location loc,
                                   const toolchain::APFloat &value) {
  mlir::Type fltTy = language::Compability::lower::convertReal(builder.getContext(), KIND);
...
static mlir::Value genScalarLit(
    ...
    const language::Compability::evaluate::Scalar<language::Compability::evaluate::Type<TC, KIND>> &value) {
  if constexpr (TC == language::Compability::common::TypeCategory::Integer ||
                TC == language::Compability::common::TypeCategory::Unsigned) {
    ...
    if (KIND == 16) {
      auto bigInt = toolchain::APInt(ty.getIntOrFloatBitWidth(),
                                TC == language::Compability::common::TypeCategory::Unsigned
  ...
  } else if constexpr (TC == language::Compability::common::TypeCategory::Real) {
    std::string str = value.DumpHexadecimal();
    if constexpr (KIND == 2) {
      auto floatVal = consAPFloat(toolchain::APFloatBase::IEEEhalf(), str);
      return genRealConstant<KIND>(builder, loc, floatVal);
    } else if constexpr (KIND == 3) {
      auto floatVal = consAPFloat(toolchain::APFloatBase::BFloat(), str);
      return genRealConstant<KIND>(builder, loc, floatVal);
    } else if constexpr (KIND == 4) {
      auto floatVal = consAPFloat(toolchain::APFloatBase::IEEEsingle(), str);
      return genRealConstant<KIND>(builder, loc, floatVal);
    } else if constexpr (KIND == 10) {
      auto floatVal = consAPFloat(toolchain::APFloatBase::x87DoubleExtended(), str);
      return genRealConstant<KIND>(builder, loc, floatVal);
    } else if constexpr (KIND == 16) {
      auto floatVal = consAPFloat(toolchain::APFloatBase::IEEEquad(), str);
      return genRealConstant<KIND>(builder, loc, floatVal);
    } else {
      // convert everything else to double
      auto floatVal = consAPFloat(toolchain::APFloatBase::IEEEdouble(), str);
      return genRealConstant<KIND>(builder, loc, floatVal);
    }
  } else if constexpr (TC == language::Compability::common::TypeCategory::Complex) {
    mlir::Value real = genScalarLit<language::Compability::common::TypeCategory::Real, KIND>(
        builder, loc, value.REAL());
    mlir::Value imag = genScalarLit<language::Compability::common::TypeCategory::Real, KIND>(
        builder, loc, value.AIMAG());
    return fir::factory::Complex{builder, loc}.createComplex(real, imag);
  } else /*constexpr*/ {
    toolchain_unreachable("unhandled constant");
  }
}
...
createStringLitOp(fir::FirOpBuilder &builder, mlir::Location loc,
                  const language::Compability::evaluate::Scalar<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Character, KIND>> &value,
  ...
  if constexpr (KIND == 1) {
    ...
    mlir::ShapedType shape = mlir::RankedTensorType::get(
        toolchain::ArrayRef<std::int64_t>{size},
        mlir::IntegerType::get(builder.getContext(), sizeof(ET) * 8));
    auto denseAttr = mlir::DenseElementsAttr::get(
        shape, toolchain::ArrayRef<ET>{value.data(), value.size()});
    ...
    toolchain::SmallVector<mlir::NamedAttribute> attrs = {dataAttr, sizeAttr};
    return fir::StringLitOp::create(builder, loc,
                                    toolchain::ArrayRef<mlir::Type>{type},
...
genScalarLit(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  toolchain::StringRef strVal(reinterpret_cast<const char *>(value.c_str()), size);
...
genConstantValue(language::Compability::lower::AbstractConverter &converter,
                 mlir::Location loc,
                 const language::Compability::lower::SomeExpr &constantExpr);

static mlir::Value genStructureComponentInit(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::semantics::Symbol &sym, const language::Compability::lower::SomeExpr &expr,
    mlir::Value res) {
  ...
  if (language::Compability::semantics::IsAllocatable(sym)) {
    if (!language::Compability::evaluate::IsNullPointerOrAllocatable(&expr)) {
  ...
  if (language::Compability::semantics::IsPointer(sym)) {
    ...
    if (language::Compability::semantics::IsProcedure(sym)) {
      if (language::Compability::evaluate::UnwrapExpr<language::Compability::evaluate::NullPointer>(expr))
        initialTarget =
        ...
        auto box{getBase(language::Compability::lower::convertExprToAddress(
            loc, converter, expr, globalOpSymMap, stmtCtx))};
        initialTarget = builder.createConvert(loc, componentTy, box);
      }
    } else
      initialTarget = language::Compability::lower::genInitialDataTarget(converter, loc,
                                                           componentTy, expr);
    res =
  ...
  if (language::Compability::lower::isDerivedTypeWithLenParameters(sym))
  ...
  if (language::Compability::semantics::IsBuiltinCPtr(sym) && sym.Rank() == 0 &&
      (language::Compability::evaluate::GetLastSymbol(expr) ||
       language::Compability::evaluate::IsNullPointer(&expr))) {
    ...
    mlir::Value addr = fir::getBase(
        language::Compability::lower::genExtAddrInInitializer(converter, loc, expr));
    ...
    toolchain::StringRef addrFieldName = language::Compability::lower::builtin::cptrFieldName;
...
static mlir::Value genInlinedStructureCtorLitImpl(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::evaluate::StructureConstructor &ctor, mlir::Type type) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  auto insertParentValueIntoExtension = [&](mlir::Type typeExtension) {
    ...
    toolchain::SmallVector<mlir::Type> parentTypes = {typeExtension};
    while (true) {
    ...
    for (mlir::Type parentType : toolchain::reverse(parentTypes)) {
  ...
  const language::Compability::semantics::DerivedTypeSpec *curentType = nullptr;
  for (const auto &[sym, expr] : ctor.values()) {
...
static mlir::Value genScalarLit(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::evaluate::Scalar<language::Compability::evaluate::SomeDerived> &value,
  ...
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  auto expr = std::make_unique<language::Compability::lower::SomeExpr>(toEvExpr(
      language::Compability::evaluate::Constant<language::Compability::evaluate::SomeDerived>(value)));
  toolchain::StringRef globalName =
...
template <typename T>
static mlir::Value
genInlinedArrayLit(language::Compability::lower::AbstractConverter &converter,
                   mlir::Location loc, mlir::Type arrayTy,
                   const language::Compability::evaluate::Constant<T> &con) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::IndexType idxTy = builder.getIndexType();
  language::Compability::evaluate::ConstantSubscripts subscripts = con.lbounds();
  auto createIdx = [&]() {
    toolchain::SmallVector<mlir::Attribute> idx;
  ...
  if (language::Compability::evaluate::GetSize(con.shape()) == 0)
    return array;
  if constexpr (T::category == language::Compability::common::TypeCategory::Character) {
    do {
  ...
  } else if constexpr (T::category == language::Compability::common::TypeCategory::Derived) {
    do {
      mlir::Type eleTy =
    ...
    toolchain::SmallVector<mlir::Attribute> rangeStartIdx;
    ...
    do {
      ...
      language::Compability::evaluate::ConstantSubscripts nextSubscripts = subscripts;
      ...
      if (!rangeSize && !nextIsSame) { // single (non-range) value
        ...
        toolchain::SmallVector<mlir::Attribute> idx = createIdx();
...
template <typename T>
static mlir::Value
genOutlineArrayLit(language::Compability::lower::AbstractConverter &converter,
                   mlir::Location loc, mlir::Type arrayTy,
                   const language::Compability::evaluate::Constant<T> &constant) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Type eleTy = mlir::cast<fir::SequenceType>(arrayTy).getElementType();
  toolchain::StringRef globalName = converter.getUniqueLitName(
      loc, std::make_unique<language::Compability::lower::SomeExpr>(toEvExpr(constant)),
      eleTy);
  fir::GlobalOp global = builder.getNamedGlobal(globalName);
  if (!global) {
    ...
    // always possible.
    if constexpr (T::category == language::Compability::common::TypeCategory::Logical ||
                  T::category == language::Compability::common::TypeCategory::Integer ||
                  T::category == language::Compability::common::TypeCategory::Real ||
                  T::category == language::Compability::common::TypeCategory::Complex) {
...
template <typename T>
static fir::ExtendedValue
genArrayLit(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
            const language::Compability::evaluate::Constant<T> &con,
            bool outlineInReadOnlyMemory) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::evaluate::ConstantSubscript size =
      language::Compability::evaluate::GetSize(con.shape());
  if (size > std::numeric_limits<std::uint32_t>::max())
    // toolchain::SmallVector has limited size
    TODO(loc, "Creation of very large array constants");
  fir::SequenceType::Shape shape(con.shape().begin(), con.shape().end());
  toolchain::SmallVector<std::int64_t> typeParams;
  if constexpr (T::category == language::Compability::common::TypeCategory::Character)
    typeParams.push_back(con.LEN());
  mlir::Type eleTy;
  if constexpr (T::category == language::Compability::common::TypeCategory::Derived)
    eleTy = language::Compability::lower::translateDerivedTypeToFIRType(
        converter, con.GetType().GetDerivedTypeSpec());
  else
    eleTy = language::Compability::lower::getFIRType(builder.getContext(), T::category,
                                       T::kind, typeParams);
  ...
  if (toolchain::any_of(con.lbounds(), [](auto lb) { return lb != 1; }))
...
template <typename T>
fir::ExtendedValue language::Compability::lower::ConstantBuilder<T>::gen(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::evaluate::Constant<T> &constant,
  ...
  std::optional<language::Compability::evaluate::Scalar<T>> opt = constant.GetScalarValue();
  ...
  if constexpr (T::category == language::Compability::common::TypeCategory::Character) {
    ...
    mlir::Type eleTy = language::Compability::lower::translateDerivedTypeToFIRType(
...
genConstantValue(language::Compability::lower::AbstractConverter &converter,
                 mlir::Location loc,
                 const language::Compability::evaluate::Expr<language::Compability::evaluate::SomeDerived>
                     &constantExpr) {
  if (const auto *constant = std::get_if<
          language::Compability::evaluate::Constant<language::Compability::evaluate::SomeDerived>>(
          &constantExpr.u))
    return language::Compability::lower::convertConstant(converter, loc, *constant,
                                           /*outline=*/false);
  if (const auto *structCtor =
          std::get_if<language::Compability::evaluate::StructureConstructor>(&constantExpr.u))
    return language::Compability::lower::genInlinedStructureCtorLit(converter, loc,
                                                      *structCtor);
...
static fir::ExtendedValue genConstantValue(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::evaluate::Expr<language::Compability::evaluate::Type<TC, KIND>>
        &constantExpr) {
  using T = language::Compability::evaluate::Type<TC, KIND>;
  if (const auto *constant =
          std::get_if<language::Compability::evaluate::Constant<T>>(&constantExpr.u))
    return language::Compability::lower::convertConstant(converter, loc, *constant,
                                           /*outline=*/false);
...
fir::ExtendedValue language::Compability::lower::genInlinedStructureCtorLit(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::evaluate::StructureConstructor &ctor) {
  mlir::Type type = language::Compability::lower::translateDerivedTypeToFIRType(
...
FOR_EACH_SPECIFIC_TYPE(template class language::Compability::lower::ConstantBuilder, )
