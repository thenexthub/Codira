...
static toolchain::cl::opt<bool> unwrapFirBox(
    "openacc-unwrap-fir-box",
    toolchain::cl::desc(
        "Whether to use the address from fix.box in data clause operations."),
    toolchain::cl::init(false));

static toolchain::cl::opt<bool> generateDefaultBounds(
    "openacc-generate-default-bounds",
    toolchain::cl::desc("Whether to generate default bounds for arrays."),
    toolchain::cl::init(false));

static toolchain::cl::opt<bool> strideIncludeLowerExtent(
...
static constexpr toolchain::StringRef accRoutinePrefix = "acc_routine_";
static constexpr toolchain::StringRef accPrivateInitName = "acc.private.init";
static constexpr toolchain::StringRef accReductionInitName = "acc.reduction.init";
static constexpr toolchain::StringRef accFirDescriptorPostfix = "_desc";

static mlir::Location
genOperandLocation(language::Compability::lower::AbstractConverter &converter,
                   const language::Compability::parser::AccObject &accObject) {
  mlir::Location loc = converter.genUnknownLocation();
  language::Compability::common::visit(
      language::Compability::common::visitors{
          [&](const language::Compability::parser::Designator &designator) {
            loc = converter.genLocation(designator.source);
          },
          [&](const language::Compability::parser::Name &name) {
...
static void addOperands(toolchain::SmallVectorImpl<mlir::Value> &operands,
                        toolchain::SmallVectorImpl<int32_t> &operandSegments,
                        toolchain::ArrayRef<mlir::Value> clauseOperands) {
  operands.append(clauseOperands.begin(), clauseOperands.end());
  operandSegments.push_back(clauseOperands.size());
}

static void addOperand(toolchain::SmallVectorImpl<mlir::Value> &operands,
                       toolchain::SmallVectorImpl<int32_t> &operandSegments,
...
createDataEntryOp(fir::FirOpBuilder &builder, mlir::Location loc,
                  ...
                  mlir::Type retTy, toolchain::ArrayRef<mlir::Value> async,
                  toolchain::ArrayRef<mlir::Attribute> asyncDeviceTypes,
                  toolchain::ArrayRef<mlir::Attribute> asyncOnlyDeviceTypes,
  ...
  toolchain::SmallVector<mlir::Value, 8> operands;
  toolchain::SmallVector<int32_t, 8> operandSegments;
...
createDeclareFunc(mlir::OpBuilder &modBuilder, fir::FirOpBuilder &builder,
                  mlir::Location loc, toolchain::StringRef funcName,
                  toolchain::SmallVector<mlir::Type> argsTy = {},
...
createSimpleOp(fir::FirOpBuilder &builder, mlir::Location loc,
               const toolchain::SmallVectorImpl<mlir::Value> &operands,
               const toolchain::SmallVectorImpl<int32_t> &operandSegments) {
  toolchain::ArrayRef<mlir::Type> argTy;
...
static void createDeclareAllocFuncWithArg(mlir::OpBuilder &modBuilder,
  ...
  toolchain::SmallVector<int32_t> operandSegments{0, 0, 0, 1};
  toolchain::SmallVector<mlir::Value> operands{updateDeviceOp.getResult()};
...
static void createDeclareDeallocFuncWithArg(
    ...
    mlir::Type descTy, toolchain::StringRef funcNamePrefix,
  ...
  preDeallocFuncName << funcNamePrefix.str()
                     << language::Compability::lower::declarePreDeallocSuffix.str();
  if (!mlir::isa<fir::ReferenceType>(descTy))
    descTy = fir::ReferenceType::get(descTy);
  ...
  postDeallocFuncName << funcNamePrefix.str()
                      << language::Compability::lower::declarePostDeallocSuffix.str();
...
language::Compability::semantics::Symbol &
getSymbolFromAccObject(const language::Compability::parser::AccObject &accObject) {
  if (const auto *designator =
          std::get_if<language::Compability::parser::Designator>(&accObject.u)) {
    if (const auto *name =
            language::Compability::semantics::getDesignatorNameIfDataRef(*designator))
      return *name->symbol;
    if (const auto *arrayElement =
            language::Compability::parser::Unwrap<language::Compability::parser::ArrayElement>(
                *designator)) {
      const language::Compability::parser::Name &name =
          language::Compability::parser::GetLastName(arrayElement->base);
      return *name.symbol;
    }
    if (const auto *component =
            language::Compability::parser::Unwrap<language::Compability::parser::StructureComponent>(
                *designator)) {
      return *component->component.symbol;
    }
  } else if (const auto *name =
                 std::get_if<language::Compability::parser::Name>(&accObject.u)) {
    return *name->symbol;
  }
  toolchain::report_fatal_error("Could not find symbol");
}

/// Used to generate atomic.read operation which is created in existing
/// location set by builder.
static inline void
genAtomicCaptureStatement(language::Compability::lower::AbstractConverter &converter,
...
/// location set by builder.
static inline void
genAtomicWriteStatement(language::Compability::lower::AbstractConverter &converter,
...
static inline void genAtomicUpdateStatement(
    language::Compability::lower::AbstractConverter &converter, mlir::Value lhsAddr,
    mlir::Type varType, const language::Compability::parser::Variable &assignmentStmtVariable,
    const language::Compability::parser::Expr &assignmentStmtExpr, mlir::Location loc,
    mlir::Operation *atomicCaptureOp = nullptr,
    language::Compability::lower::StatementContext *atomicCaptureStmtCtx = nullptr) {
  ...
  auto getArgExpression =
      [](std::list<language::Compability::parser::ActualArgSpec>::const_iterator it) {
        const auto &arg{std::get<language::Compability::parser::ActualArg>((*it).t)};
        const auto *parserExpr{
            std::get_if<language::Compability::common::Indirection<language::Compability::parser::Expr>>(
                &arg.u)};
        return parserExpr;
      };
  ...
  language::Compability::lower::ExprToValueMap exprValueOverrides;
  ...
  toolchain::SmallVector<const language::Compability::lower::SomeExpr *> nonAtomicSubExprs;
  language::Compability::common::visit(
      language::Compability::common::visitors{
          [&](const language::Compability::common::Indirection<
              language::Compability::parser::FunctionReference> &funcRef) -> void {
            const auto &args{
                std::get<std::list<language::Compability::parser::ActualArgSpec>>(
                    funcRef.value().v.t)};
            std::list<language::Compability::parser::ActualArgSpec>::const_iterator beginIt =
                args.begin();
            std::list<language::Compability::parser::ActualArgSpec>::const_iterator endIt =
                args.end();
            ...
            for (it = beginIt; it != endIt; it++) {
              const language::Compability::common::Indirection<language::Compability::parser::Expr> *expr =
                  getArgExpression(it);
              if (expr)
                nonAtomicSubExprs.push_back(language::Compability::semantics::GetExpr(*expr));
            }
          },
          [&](const auto &op) -> void {
            using T = std::decay_t<decltype(op)>;
            if constexpr (std::is_base_of<
                              language::Compability::parser::Expr::IntrinsicBinary,
                              T>::value) {
              ...
              if (exprLeft.value().source == assignmentStmtVariable.GetSource())
                nonAtomicSubExprs.push_back(
                    language::Compability::semantics::GetExpr(exprRight));
              else
                nonAtomicSubExprs.push_back(
                    language::Compability::semantics::GetExpr(exprLeft));
            }
          },
      },
      assignmentStmtExpr.u);
  language::Compability::lower::StatementContext nonAtomicStmtCtx;
  language::Compability::lower::StatementContext *stmtCtxPtr = &nonAtomicStmtCtx;
  ...
  toolchain::SmallVector<mlir::Type> varTys = {varType};
  toolchain::SmallVector<mlir::Location> locs = {currentLocation};
  ...
  exprValueOverrides.try_emplace(
      language::Compability::semantics::GetExpr(assignmentStmtVariable), val);
  {
    // statement context inside the atomic block.
    converter.overrideExprValues(&exprValueOverrides);
    language::Compability::lower::StatementContext atomicStmtCtx;
    mlir::Value rhsExpr = fir::getBase(converter.genExprValue(
        *language::Compability::semantics::GetExpr(assignmentStmtExpr), atomicStmtCtx));
...
void genAtomicWrite(language::Compability::lower::AbstractConverter &converter,
                    const language::Compability::parser::AccAtomicWrite &atomicWrite,
                    mlir::Location loc) {
  const language::Compability::parser::AssignmentStmt &stmt =
      std::get<language::Compability::parser::Statement<language::Compability::parser::AssignmentStmt>>(
          atomicWrite.t)
          .statement;
  const language::Compability::evaluate::Assignment &assign = *stmt.typedAssignment->v;
...
void genAtomicRead(language::Compability::lower::AbstractConverter &converter,
                   const language::Compability::parser::AccAtomicRead &atomicRead,
                   mlir::Location loc) {
  const auto &assignmentStmtExpr = std::get<language::Compability::parser::Expr>(
      std::get<language::Compability::parser::Statement<language::Compability::parser::AssignmentStmt>>(
          atomicRead.t)
          .statement.t);
  const auto &assignmentStmtVariable = std::get<language::Compability::parser::Variable>(
      std::get<language::Compability::parser::Statement<language::Compability::parser::AssignmentStmt>>(
          atomicRead.t)
          .statement.t);

  language::Compability::lower::StatementContext stmtCtx;
  const language::Compability::semantics::SomeExpr &fromExpr =
      *language::Compability::semantics::GetExpr(assignmentStmtExpr);
  mlir::Type elementType = converter.genType(fromExpr);
  mlir::Value fromAddress =
      fir::getBase(converter.genExprAddr(fromExpr, stmtCtx));
  mlir::Value toAddress = fir::getBase(converter.genExprAddr(
      *language::Compability::semantics::GetExpr(assignmentStmtVariable), stmtCtx));
  genAtomicCaptureStatement(converter, fromAddress, toAddress, elementType,
                            loc);
}

/// Processes an atomic construct with update clause.
void genAtomicUpdate(language::Compability::lower::AbstractConverter &converter,
                     const language::Compability::parser::AccAtomicUpdate &atomicUpdate,
                     mlir::Location loc) {
  const auto &assignmentStmtExpr = std::get<language::Compability::parser::Expr>(
      std::get<language::Compability::parser::Statement<language::Compability::parser::AssignmentStmt>>(
          atomicUpdate.t)
...
void genAtomicCapture(language::Compability::lower::AbstractConverter &converter,
                      const language::Compability::parser::AccAtomicCapture &atomicCapture,
                      mlir::Location loc) {
  fir::FirOpBuilder &firOpBuilder = converter.getFirOpBuilder();

  const language::Compability::parser::AssignmentStmt &stmt1 =
      std::get<language::Compability::parser::AccAtomicCapture::Stmt1>(atomicCapture.t)
          .v.statement;
  const language::Compability::evaluate::Assignment &assign1 = *stmt1.typedAssignment->v;
  const auto &stmt1Var{std::get<language::Compability::parser::Variable>(stmt1.t)};
  const auto &stmt1Expr{std::get<language::Compability::parser::Expr>(stmt1.t)};
  const language::Compability::parser::AssignmentStmt &stmt2 =
      std::get<language::Compability::parser::AccAtomicCapture::Stmt2>(atomicCapture.t)
          .v.statement;
  const language::Compability::evaluate::Assignment &assign2 = *stmt2.typedAssignment->v;
  const auto &stmt2Var{std::get<language::Compability::parser::Variable>(stmt2.t)};
  const auto &stmt2Expr{std::get<language::Compability::parser::Expr>(stmt2.t)};
  ...
  if (language::Compability::parser::CheckForSingleVariableOnRHS(stmt1)) {
    if (language::Compability::evaluate::CheckForSymbolMatch(
            language::Compability::semantics::GetExpr(stmt2Var),
            language::Compability::semantics::GetExpr(stmt2Expr))) {
      // Atomic capture construct is of the form [capture-stmt, update-stmt]
      const language::Compability::semantics::SomeExpr &fromExpr =
          *language::Compability::semantics::GetExpr(stmt1Expr);
      mlir::Type elementType = converter.genType(fromExpr);
...
template <typename Op>
static void
genDataOperandOperations(const language::Compability::parser::AccObjectList &objectList,
                         language::Compability::lower::AbstractConverter &converter,
                         language::Compability::semantics::SemanticsContext &semanticsContext,
                         language::Compability::lower::StatementContext &stmtCtx,
                         toolchain::SmallVectorImpl<mlir::Value> &dataOperands,
                         mlir::acc::DataClause dataClause, bool structured,
                         bool implicit, toolchain::ArrayRef<mlir::Value> async,
                         toolchain::ArrayRef<mlir::Attribute> asyncDeviceTypes,
                         toolchain::ArrayRef<mlir::Attribute> asyncOnlyDeviceTypes,
  ...
  for (const auto &accObject : objectList.v) {
    ...
    language::Compability::semantics::Symbol &symbol = getSymbolFromAccObject(accObject);
    language::Compability::semantics::MaybeExpr designator = language::Compability::common::visit(
        [&](auto &&s) { return ea.Analyze(s); }, accObject.u);
    fir::factory::AddrAndBoundsInfo info =
        language::Compability::lower::gatherDataOperandAddrAndBounds<
    ...
    LLVM_DEBUG(toolchain::dbgs() << __func__ << "\n"; info.dump(toolchain::dbgs()));
...
static void genDeclareDataOperandOperations(
    const language::Compability::parser::AccObjectList &objectList,
    language::Compability::lower::AbstractConverter &converter,
    language::Compability::semantics::SemanticsContext &semanticsContext,
    language::Compability::lower::StatementContext &stmtCtx,
    toolchain::SmallVectorImpl<mlir::Value> &dataOperands,
...
static void genDeclareDataOperandOperationsWithModifier(
    const Clause *x, language::Compability::lower::AbstractConverter &converter,
    language::Compability::semantics::SemanticsContext &semanticsContext,
    language::Compability::lower::StatementContext &stmtCtx,
    language::Compability::parser::AccDataModifier::Modifier mod,
    toolchain::SmallVectorImpl<mlir::Value> &dataClauseOperands,
    const mlir::acc::DataClause clause,
    const mlir::acc::DataClause clauseWithModifier) {
  const language::Compability::parser::AccObjectListWithModifier &listWithModifier = x->v;
  const auto &accObjectList =
      std::get<language::Compability::parser::AccObjectList>(listWithModifier.t);
  const auto &modifier =
      std::get<std::optional<language::Compability::parser::AccDataModifier>>(
          listWithModifier.t);
...
genDataExitOperations(fir::FirOpBuilder &builder,
                      toolchain::SmallVector<mlir::Value> operands, bool structured,
...
static R getReductionInitValue(mlir::acc::ReductionOperator op, mlir::Type ty) {
  if (op == mlir::acc::ReductionOperator::AccMin) {
    ...
    if constexpr (std::is_same_v<R, toolchain::APInt>) {
      assert(ty.isIntOrIndex() && "expect integer or index type");
      return toolchain::APInt::getSignedMaxValue(ty.getIntOrFloatBitWidth());
    }
    if constexpr (std::is_same_v<R, toolchain::APFloat>) {
      auto floatTy = mlir::dyn_cast_or_null<mlir::FloatType>(ty);
      assert(floatTy && "expect float type");
      return toolchain::APFloat::getLargest(floatTy.getFloatSemantics(),
                                       /*negative=*/false);
    }
    ...
    if constexpr (std::is_same_v<R, toolchain::APInt>) {
      assert(ty.isIntOrIndex() && "expect integer or index type");
      return toolchain::APInt::getSignedMinValue(ty.getIntOrFloatBitWidth());
    }
    if constexpr (std::is_same_v<R, toolchain::APFloat>) {
      auto floatTy = mlir::dyn_cast_or_null<mlir::FloatType>(ty);
      assert(floatTy && "expect float type");
      return toolchain::APFloat::getSmallest(floatTy.getFloatSemantics(),
                                        /*negative=*/true);
    }
    ...
    if constexpr (std::is_same_v<R, toolchain::APInt>) {
      ...
      return toolchain::APInt::getAllOnes(bits);
    }
  } else {
    ...
    if constexpr (std::is_same_v<R, toolchain::APInt>) {
      assert(ty.isIntOrIndex() && "expect integer or index type");
      return toolchain::APInt(ty.getIntOrFloatBitWidth(), value, true);
    }

    if constexpr (std::is_same_v<R, toolchain::APFloat>) {
      assert(mlir::isa<mlir::FloatType>(ty) && "expect float type");
      auto floatTy = mlir::dyn_cast<mlir::FloatType>(ty);
      return toolchain::APFloat(floatTy.getFloatSemantics(), value);
    }

    if constexpr (std::is_same_v<R, int64_t>)
      return value;
  }
  toolchain_unreachable("OpenACC reduction unsupported type");
}
...
static mlir::Value getReductionInitValue(fir::FirOpBuilder &builder,
  ...
  if (ty.isIntOrIndex())
    return mlir::arith::ConstantOp::create(
        builder, loc, ty,
        builder.getIntegerAttr(ty, getReductionInitValue<toolchain::APInt>(op, ty)));
  if (op == mlir::acc::ReductionOperator::AccMin ||
    ...
    if (auto floatTy = mlir::dyn_cast_or_null<mlir::FloatType>(ty))
      return mlir::arith::ConstantOp::create(
          ...
          builder.getFloatAttr(ty,
                               getReductionInitValue<toolchain::APFloat>(op, ty)));
  ...
  toolchain::report_fatal_error("Unsupported OpenACC reduction type");
}

template <typename RecipeOp>
static RecipeOp genRecipeOp(
    fir::FirOpBuilder &builder, mlir::ModuleOp mod, toolchain::StringRef recipeName,
    mlir::Location loc, mlir::Type ty,
  ...
  toolchain::SmallVector<mlir::Type> argsTy{ty};
  toolchain::SmallVector<mlir::Location> argsLoc{loc};
...
language::Compability::lower::createOrGetPrivateRecipe(fir::FirOpBuilder &builder,
                                         toolchain::StringRef recipeName,
                                         mlir::Location loc, mlir::Type ty) {
  mlir::ModuleOp mod =
...
bool areAllBoundConstant(const toolchain::SmallVector<mlir::Value> &bounds) {
...
static toolchain::SmallVector<mlir::Value>
genConstantBounds(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  if (dataBound.getLowerbound() &&
    ...
    toolchain::report_fatal_error("Expect constant lb/ub or extent");
  }
  return {lb, ub, step};
}
...
static mlir::Value genShapeFromBoundsOrArgs(
    ...
    const toolchain::SmallVector<mlir::Value> &bounds, mlir::ValueRange arguments) {
  ...
  if (bounds.empty() && seqTy) {
    ...
    for (auto bound : toolchain::reverse(bounds)) {
      auto dataBound =
...
mlir::acc::FirstprivateRecipeOp language::Compability::lower::createOrGetFirstprivateRecipe(
    fir::FirOpBuilder &builder, toolchain::StringRef recipeName, mlir::Location loc,
    mlir::Type ty, toolchain::SmallVector<mlir::Value> &bounds) {
  mlir::ModuleOp mod =
      builder.getBlock()->getParent()->getParentOfType<mlir::ModuleOp>();
  if (auto recipe =
  ...
  toolchain::SmallVector<mlir::Type> argsTy{ty, ty};
  toolchain::SmallVector<mlir::Location> argsLoc{loc, loc};
  if (!allConstantBound) {
    for (mlir::Value bound : toolchain::reverse(bounds)) {
      auto dataBound =
          mlir::dyn_cast<mlir::acc::DataBoundsOp>(bound.getDefiningOp());
  ...
  if (fir::isa_trivial(ty)) {
    ...
    auto leftDeclOp = hlfir::DeclareOp::create(
        builder, loc, recipe.getCopyRegion().getArgument(0), toolchain::StringRef{},
        shape, toolchain::ArrayRef<mlir::Value>{}, /*dummy_scope=*/nullptr,
        fir::FortranVariableFlagsAttr{});
    auto rightDeclOp = hlfir::DeclareOp::create(
        builder, loc, recipe.getCopyRegion().getArgument(1), toolchain::StringRef{},
        shape, toolchain::ArrayRef<mlir::Value>{}, /*dummy_scope=*/nullptr,
        fir::FortranVariableFlagsAttr{});
    ...
    fir::FirOpBuilder firBuilder{builder, recipe.getOperation()};
    toolchain::SmallVector<mlir::Value> tripletArgs;
...
std::string getBoundsString(toolchain::SmallVector<mlir::Value> &bounds) {
...
mlir::Type getTypeFromBounds(toolchain::SmallVector<mlir::Value> &bounds,
                             mlir::Type ty) {
  auto seqTy =
      mlir::dyn_cast_or_null<fir::SequenceType>(fir::unwrapRefType(ty));
  if (!bounds.empty() && seqTy) {
    toolchain::SmallVector<int64_t> shape;
    for (auto b : bounds) {
      auto boundsOp =
...
static void genPrivatizationRecipes(
    ...
    language::Compability::lower::StatementContext &stmtCtx,
    toolchain::SmallVectorImpl<mlir::Value> &dataOperands,
    toolchain::SmallVector<mlir::Attribute> &privatizationRecipes,
    toolchain::ArrayRef<mlir::Value> async,
    toolchain::ArrayRef<mlir::Attribute> asyncDeviceTypes,
    toolchain::ArrayRef<mlir::Attribute> asyncOnlyDeviceTypes) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::evaluate::ExpressionAnalyzer ea{semanticsContext};
    ...
    if constexpr (std::is_same_v<RecipeOp, mlir::acc::PrivateRecipeOp>) {
      std::string recipeName =
          fir::getTypeAsString(retTy, converter.getKindMap(),
                               language::Compability::lower::privatizationRecipePrefix);
      recipe = language::Compability::lower::createOrGetPrivateRecipe(builder, recipeName,
      ...
      recipe = language::Compability::lower::createOrGetFirstprivateRecipe(
...
getReductionOperator(const language::Compability::parser::ReductionOperator &op) {
  switch (op.v) {
  ...
  toolchain_unreachable("unexpected reduction operator");
}
...
static void genCombiner(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  if (auto seqTy = mlir::dyn_cast<fir::SequenceType>(ty)) {
    ...
    toolchain::SmallVector<mlir::Value> ivs;
    if (seqTy.hasDynamicExtents()) {
      auto shape =
      ...
      toolchain::SmallVector<mlir::Value> lenParamsRight;
...
mlir::acc::ReductionRecipeOp language::Compability::lower::createOrGetReductionRecipe(
    fir::FirOpBuilder &builder, toolchain::StringRef recipeName, mlir::Location loc,
...
static void
genReductions(const language::Compability::parser::AccObjectListWithReduction &objectList,
              language::Compability::lower::AbstractConverter &converter,
              language::Compability::semantics::SemanticsContext &semanticsContext,
              language::Compability::lower::StatementContext &stmtCtx,
              toolchain::SmallVectorImpl<mlir::Value> &reductionOperands,
              toolchain::SmallVector<mlir::Attribute> &reductionRecipes,
              toolchain::ArrayRef<mlir::Value> async,
              toolchain::ArrayRef<mlir::Attribute> asyncDeviceTypes,
              toolchain::ArrayRef<mlir::Attribute> asyncOnlyDeviceTypes) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  const auto &objects = std::get<language::Compability::parser::AccObjectList>(objectList.t);
  const auto &op = std::get<language::Compability::parser::ReductionOperator>(objectList.t);
  ...
  for (const auto &accObject : objects.v) {
    ...
    mlir::acc::ReductionRecipeOp recipe =
        language::Compability::lower::createOrGetReductionRecipe(
...
createRegionOp(fir::FirOpBuilder &builder, mlir::Location loc,
               mlir::Location returnLoc, language::Compability::lower::pft::Evaluation &eval,
               const toolchain::SmallVectorImpl<mlir::Value> &operands,
               const toolchain::SmallVectorImpl<int32_t> &operandSegments,
  ...
  if (eval.lowerAsUnstructured() && !outerCombined)
    language::Compability::lower::createEmptyRegionBlocks<mlir::acc::TerminatorOp,
                                            mlir::acc::YieldOp>(
...
static void genAsyncClause(language::Compability::lower::AbstractConverter &converter,
                           const language::Compability::parser::AccClause::Async *asyncClause,
                           mlir::Value &async, bool &addAsyncAttr,
                           language::Compability::lower::StatementContext &stmtCtx) {
  const auto &asyncClauseValue = asyncClause->v;
  if (asyncClauseValue) { // async has a value.
    async = fir::getBase(converter.genExprValue(
        *language::Compability::semantics::GetExpr(*asyncClauseValue), stmtCtx));
  } else {
    addAsyncAttr = true;
  }
}
...
static void
genAsyncClause(language::Compability::lower::AbstractConverter &converter,
               const language::Compability::parser::AccClause::Async *asyncClause,
               toolchain::SmallVector<mlir::Value> &async,
               toolchain::SmallVector<mlir::Attribute> &asyncDeviceTypes,
               toolchain::SmallVector<mlir::Attribute> &asyncOnlyDeviceTypes,
               toolchain::SmallVector<mlir::Attribute> &deviceTypeAttrs,
               language::Compability::lower::StatementContext &stmtCtx) {
  const auto &asyncClauseValue = asyncClause->v;
  if (asyncClauseValue) { // async has a value.
...
getDeviceType(language::Compability::common::OpenACCDeviceType device) {
  switch (device) {
  case language::Compability::common::OpenACCDeviceType::Star:
    return mlir::acc::DeviceType::Star;
  case language::Compability::common::OpenACCDeviceType::Default:
    return mlir::acc::DeviceType::Default;
  case language::Compability::common::OpenACCDeviceType::Nvidia:
    return mlir::acc::DeviceType::Nvidia;
  case language::Compability::common::OpenACCDeviceType::Radeon:
    return mlir::acc::DeviceType::Radeon;
  case language::Compability::common::OpenACCDeviceType::Host:
    return mlir::acc::DeviceType::Host;
  case language::Compability::common::OpenACCDeviceType::Multicore:
    return mlir::acc::DeviceType::Multicore;
  case language::Compability::common::OpenACCDeviceType::None:
...
static void gatherDeviceTypeAttrs(
    fir::FirOpBuilder &builder,
    const language::Compability::parser::AccClause::DeviceType *deviceTypeClause,
    toolchain::SmallVector<mlir::Attribute> &deviceTypes) {
  const language::Compability::parser::AccDeviceTypeExprList &deviceTypeExprList =
      deviceTypeClause->v;
...
static void genIfClause(language::Compability::lower::AbstractConverter &converter,
                        mlir::Location clauseLocation,
                        const language::Compability::parser::AccClause::If *ifClause,
                        mlir::Value &ifCond,
                        language::Compability::lower::StatementContext &stmtCtx) {
  fir::FirOpBuilder &firOpBuilder = converter.getFirOpBuilder();
  mlir::Value cond = fir::getBase(converter.genExprValue(
      *language::Compability::semantics::GetExpr(ifClause->v), stmtCtx, &clauseLocation));
  ifCond = firOpBuilder.createConvert(clauseLocation, firOpBuilder.getI1Type(),
                                      cond);
}

static void genWaitClause(language::Compability::lower::AbstractConverter &converter,
                          const language::Compability::parser::AccClause::Wait *waitClause,
                          toolchain::SmallVectorImpl<mlir::Value> &operands,
                          mlir::Value &waitDevnum, bool &addWaitAttr,
                          language::Compability::lower::StatementContext &stmtCtx) {
  const auto &waitClauseValue = waitClause->v;
  if (waitClauseValue) { // wait has a value.
    const language::Compability::parser::AccWaitArgument &waitArg = *waitClauseValue;
    const auto &waitList =
        std::get<std::list<language::Compability::parser::ScalarIntExpr>>(waitArg.t);
    for (const language::Compability::parser::ScalarIntExpr &value : waitList) {
      mlir::Value v = fir::getBase(
          converter.genExprValue(*language::Compability::semantics::GetExpr(value), stmtCtx));
      operands.push_back(v);
    }

    const auto &waitDevnumValue =
        std::get<std::optional<language::Compability::parser::ScalarIntExpr>>(waitArg.t);
    if (waitDevnumValue)
      waitDevnum = fir::getBase(converter.genExprValue(
          *language::Compability::semantics::GetExpr(*waitDevnumValue), stmtCtx));
  } else {
    addWaitAttr = true;
  }
}
...
static void genWaitClauseWithDeviceType(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::parser::AccClause::Wait *waitClause,
    toolchain::SmallVector<mlir::Value> &waitOperands,
    toolchain::SmallVector<mlir::Attribute> &waitOperandsDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &waitOnlyDeviceTypes,
    toolchain::SmallVector<bool> &hasDevnums,
    toolchain::SmallVector<int32_t> &waitOperandsSegments,
    toolchain::SmallVector<mlir::Attribute> deviceTypeAttrs,
    language::Compability::lower::StatementContext &stmtCtx) {
  const auto &waitClauseValue = waitClause->v;
  if (waitClauseValue) { // wait has a value.
    toolchain::SmallVector<mlir::Value> waitValues;
    ...
    for (const language::Compability::parser::ScalarIntExpr &value : waitList) {
      waitValues.push_back(fir::getBase(converter.genExprValue(
          *language::Compability::semantics::GetExpr(value), stmtCtx)));
    }
...
mlir::Type getTypeFromIvTypeSize(fir::FirOpBuilder &builder,
                                 const language::Compability::semantics::Symbol &ivSym) {
  std::size_t ivTypeSize = ivSym.size();
  if (ivTypeSize == 0)
    toolchain::report_fatal_error("unexpected induction variable size");
  // ivTypeSize is in bytes and IntegerType needs to be in bits.
  return builder.getIntegerType(ivTypeSize * 8);
}

static void
privatizeIv(language::Compability::lower::AbstractConverter &converter,
            const language::Compability::semantics::Symbol &sym, mlir::Location loc,
            toolchain::SmallVector<mlir::Type> &ivTypes,
            toolchain::SmallVector<mlir::Location> &ivLocs,
            toolchain::SmallVector<mlir::Value> &privateOperands,
            toolchain::SmallVector<mlir::Value> &ivPrivate,
            toolchain::SmallVector<mlir::Attribute> &privatizationRecipes,
  ...
  if (privateOp == nullptr) {
    std::string recipeName =
        fir::getTypeAsString(ivValue.getType(), converter.getKindMap(),
                             language::Compability::lower::privatizationRecipePrefix);
    auto recipe = language::Compability::lower::createOrGetPrivateRecipe(
        builder, recipeName, loc, ivValue.getType());

    std::stringstream asFortran;
    asFortran << language::Compability::lower::mangle::demangleName(toStringRef(sym.name()));
    auto op = createDataEntryOp<mlir::acc::PrivateOp>(
...
static void determineDefaultLoopParMode(
    language::Compability::lower::AbstractConverter &converter, mlir::acc::LoopOp &loopOp,
    toolchain::SmallVector<mlir::Attribute> &seqDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &independentDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &autoDeviceTypes) {
  ...
  bool hasDefaultSeq = toolchain::any_of(seqDeviceTypes, hasDeviceNone);
  bool hasDefaultIndependent =
      toolchain::any_of(independentDeviceTypes, hasDeviceNone);
  bool hasDefaultAuto = toolchain::any_of(autoDeviceTypes, hasDeviceNone);
...
static void processDoLoopBounds(
    language::Compability::lower::AbstractConverter &converter,
    mlir::Location currentLocation, language::Compability::lower::StatementContext &stmtCtx,
    fir::FirOpBuilder &builder,
    const language::Compability::parser::DoConstruct &outerDoConstruct,
    language::Compability::lower::pft::Evaluation &eval,
    toolchain::SmallVector<mlir::Value> &lowerbounds,
    toolchain::SmallVector<mlir::Value> &upperbounds,
    ...
    toolchain::SmallVector<mlir::Type> &ivTypes,
    toolchain::SmallVector<mlir::Location> &ivLocs,
    toolchain::SmallVector<bool> &inclusiveBounds,
    toolchain::SmallVector<mlir::Location> &locs, uint64_t loopsToProcess) {
  assert(loopsToProcess > 0 && "expect at least one loop");
  locs.push_back(currentLocation); // Location of the directive
  language::Compability::lower::pft::Evaluation *crtEval = &eval.getFirstNestedEvaluation();
  ...
  if (isDoConcurrent) {
    ...
    for (const auto &control : controls) {
      ...
      if (const auto &expr =
        ...
        steps.push_back(fir::getBase(converter.genExprValue(
            *language::Compability::semantics::GetExpr(*expr), stmtCtx)));
    ...
    for (uint64_t i = 0; i < loopsToProcess; ++i) {
      ...
      if (i == 0) {
        ...
        auto *doCons = crtEval->getIf<language::Compability::parser::DoConstruct>();
        assert(doCons && "expect do construct");
        loopControl = &*doCons->GetLoopControl();
        locs.push_back(converter.genLocation(
            language::Compability::parser::FindSourceLocation(*doCons)));
      }
...
buildACCLoopOp(language::Compability::lower::AbstractConverter &converter,
               ...
               toolchain::SmallVector<mlir::Value> &privateOperands,
               toolchain::SmallVector<mlir::Attribute> &privatizationRecipes,
               toolchain::SmallVector<mlir::Value> &gangOperands,
               toolchain::SmallVector<mlir::Value> &workerNumOperands,
               toolchain::SmallVector<mlir::Value> &vectorOperands,
               toolchain::SmallVector<mlir::Value> &tileOperands,
               toolchain::SmallVector<mlir::Value> &cacheOperands,
               toolchain::SmallVector<mlir::Value> &reductionOperands,
               toolchain::SmallVector<mlir::Type> &retTy, mlir::Value yieldValue,
  ...
  toolchain::SmallVector<bool> inclusiveBounds;
  toolchain::SmallVector<mlir::Location> locs;
  toolchain::SmallVector<mlir::Value> lowerbounds, upperbounds, steps;
  ...
  toolchain::SmallVector<int32_t> operandSegments;
...
static mlir::acc::LoopOp createLoopOp(
    ...
    const language::Compability::parser::DoConstruct &outerDoConstruct,
    language::Compability::lower::pft::Evaluation &eval,
    const language::Compability::parser::AccClauseList &accClauseList,
    std::optional<mlir::acc::CombinedConstructsType> combinedConstructs =
        std::nullopt,
    bool needEarlyReturnHandling = false) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  toolchain::SmallVector<mlir::Value> tileOperands, privateOperands,
      reductionOperands, cacheOperands, vectorOperands, workerNumOperands,
      gangOperands;
  toolchain::SmallVector<mlir::Attribute> privatizationRecipes, reductionRecipes;
  toolchain::SmallVector<int32_t> tileOperandsSegments, gangOperandsSegments;
  toolchain::SmallVector<int64_t> collapseValues;

  toolchain::SmallVector<mlir::Attribute> gangArgTypes;
  toolchain::SmallVector<mlir::Attribute> seqDeviceTypes, independentDeviceTypes,
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    mlir::Location clauseLocation = converter.genLocation(clause.source);
    if (const auto *gangClause =
            std::get_if<language::Compability::parser::AccClause::Gang>(&clause.u)) {
      if (gangClause->v) {
        const language::Compability::parser::AccGangArgList &x = *gangClause->v;
        mlir::SmallVector<mlir::Value> gangValues;
        mlir::SmallVector<mlir::Attribute> gangArgs;
        for (const language::Compability::parser::AccGangArg &gangArg : x.v) {
          if (const auto *num =
                  std::get_if<language::Compability::parser::AccGangArg::Num>(&gangArg.u)) {
            gangValues.push_back(fir::getBase(converter.genExprValue(
                *language::Compability::semantics::GetExpr(num->v), stmtCtx)));
            gangArgs.push_back(mlir::acc::GangArgTypeAttr::get(
                builder.getContext(), mlir::acc::GangArgType::Num));
          } else if (const auto *staticArg =
                         std::get_if<language::Compability::parser::AccGangArg::Static>(
                             &gangArg.u)) {
            const language::Compability::parser::AccSizeExpr &sizeExpr = staticArg->v;
            if (sizeExpr.v) {
              gangValues.push_back(fir::getBase(converter.genExprValue(
                  *language::Compability::semantics::GetExpr(*sizeExpr.v), stmtCtx)));
            } else {
            ...
            gangArgs.push_back(mlir::acc::GangArgTypeAttr::get(
                builder.getContext(), mlir::acc::GangArgType::Static));
                         ...
                         std::get_if<language::Compability::parser::AccGangArg::Dim>(
                             &gangArg.u)) {
            gangValues.push_back(fir::getBase(converter.genExprValue(
                *language::Compability::semantics::GetExpr(dim->v), stmtCtx)));
            gangArgs.push_back(mlir::acc::GangArgTypeAttr::get(
                builder.getContext(), mlir::acc::GangArgType::Dim));
          }
        }
        for (auto crtDeviceTypeAttr : crtDeviceTypes) {
          for (const auto &pair : toolchain::zip(gangValues, gangArgs)) {
        ...
        for (auto crtDeviceTypeAttr : crtDeviceTypes)
          gangDeviceTypes.push_back(crtDeviceTypeAttr);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Worker>(&clause.u)) {
      if (workerClause->v) {
        mlir::Value workerNumValue = fir::getBase(converter.genExprValue(
            *language::Compability::semantics::GetExpr(*workerClause->v), stmtCtx));
        for (auto crtDeviceTypeAttr : crtDeviceTypes) {
        ...
        for (auto crtDeviceTypeAttr : crtDeviceTypes)
          workerNumDeviceTypes.push_back(crtDeviceTypeAttr);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Vector>(&clause.u)) {
      if (vectorClause->v) {
        mlir::Value vectorValue = fir::getBase(converter.genExprValue(
            *language::Compability::semantics::GetExpr(*vectorClause->v), stmtCtx));
        for (auto crtDeviceTypeAttr : crtDeviceTypes) {
        ...
        for (auto crtDeviceTypeAttr : crtDeviceTypes)
          vectorDeviceTypes.push_back(crtDeviceTypeAttr);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Tile>(&clause.u)) {
      const language::Compability::parser::AccTileExprList &accTileExprList = tileClause->v;
      toolchain::SmallVector<mlir::Value> tileValues;
      for (const auto &accTileExpr : accTileExprList.v) {
        const auto &expr =
            std::get<std::optional<language::Compability::parser::ScalarIntConstantExpr>>(
                accTileExpr.t);
        if (expr) {
      ...
      for (auto crtDeviceTypeAttr : crtDeviceTypes) {
        ...
        tileOperandsSegments.push_back(tileValues.size());
                   ...
                   std::get_if<language::Compability::parser::AccClause::Private>(
                       &clause.u)) {
      genPrivatizationRecipes<mlir::acc::PrivateRecipeOp>(
          ...
          /*asyncDeviceTypes=*/{}, /*asyncOnlyDeviceTypes=*/{});
                   ...
                   std::get_if<language::Compability::parser::AccClause::Reduction>(
                       &clause.u)) {
      genReductions(reductionClause->v, converter, semanticsContext, stmtCtx,
                    reductionOperands, reductionRecipes, /*async=*/{},
                    /*asyncDeviceTypes=*/{}, /*asyncOnlyDeviceTypes=*/{});
    } else if (std::get_if<language::Compability::parser::AccClause::Seq>(&clause.u)) {
      for (auto crtDeviceTypeAttr : crtDeviceTypes)
        seqDeviceTypes.push_back(crtDeviceTypeAttr);
    } else if (std::get_if<language::Compability::parser::AccClause::Independent>(
                   &clause.u)) {
      for (auto crtDeviceTypeAttr : crtDeviceTypes)
        independentDeviceTypes.push_back(crtDeviceTypeAttr);
    } else if (std::get_if<language::Compability::parser::AccClause::Auto>(&clause.u)) {
      for (auto crtDeviceTypeAttr : crtDeviceTypes)
        autoDeviceTypes.push_back(crtDeviceTypeAttr);
    } else if (const auto *deviceTypeClause =
                   std::get_if<language::Compability::parser::AccClause::DeviceType>(
                       &clause.u)) {
      crtDeviceTypes.clear();
      gatherDeviceTypeAttrs(builder, deviceTypeClause, crtDeviceTypes);
    } else if (const auto *collapseClause =
                   std::get_if<language::Compability::parser::AccClause::Collapse>(
                       &clause.u)) {
      const language::Compability::parser::AccCollapseArg &arg = collapseClause->v;
      const auto &force = std::get<bool>(arg.t);
      if (force)
        TODO(clauseLocation, "OpenACC collapse force modifier");

      const auto &intExpr =
          std::get<language::Compability::parser::ScalarIntConstantExpr>(arg.t);
      const auto *expr = language::Compability::semantics::GetExpr(intExpr);
      const std::optional<int64_t> collapseValue =
          language::Compability::evaluate::ToInt64(*expr);
  ...
  uint64_t loopsToProcess =
      language::Compability::lower::getLoopCountForCollapseAndTile(accClauseList);
...
static bool hasEarlyReturn(language::Compability::lower::pft::Evaluation &eval) {
  bool hasReturnStmt = false;
  for (auto &e : eval.getNestedEvaluations()) {
    e.visit(language::Compability::common::visitors{
        [&](const language::Compability::parser::ReturnStmt &) { hasReturnStmt = true; },
        [&](const auto &s) {},
    });
...
static mlir::Value
genACC(language::Compability::lower::AbstractConverter &converter,
       language::Compability::semantics::SemanticsContext &semanticsContext,
       language::Compability::lower::pft::Evaluation &eval,
       const language::Compability::parser::OpenACCLoopConstruct &loopConstruct) {

  const auto &beginLoopDirective =
      std::get<language::Compability::parser::AccBeginLoopDirective>(loopConstruct.t);
  const auto &loopDirective =
      std::get<language::Compability::parser::AccLoopDirective>(beginLoopDirective.t);
  ...
  assert(loopDirective.v == toolchain::acc::ACCD_loop &&
         "Unsupported OpenACC loop construct");
  (void)loopDirective;

  const auto &accClauseList =
      std::get<language::Compability::parser::AccClauseList>(beginLoopDirective.t);
  const auto &outerDoConstruct =
      std::get<std::optional<language::Compability::parser::DoConstruct>>(loopConstruct.t);
...
static Op createComputeOp(
    language::Compability::lower::AbstractConverter &converter,
    mlir::Location currentLocation, language::Compability::lower::pft::Evaluation &eval,
    language::Compability::semantics::SemanticsContext &semanticsContext,
    language::Compability::lower::StatementContext &stmtCtx,
    const language::Compability::parser::AccClauseList &accClauseList,
    std::optional<mlir::acc::CombinedConstructsType> combinedConstructs =
  ...
  mlir::Value ifCond;
  mlir::Value selfCond;
  toolchain::SmallVector<mlir::Value> waitOperands, attachEntryOperands,
  ...
  toolchain::SmallVector<mlir::Attribute> numGangsDeviceTypes, numWorkersDeviceTypes,
      vectorLengthDeviceTypes, asyncDeviceTypes, asyncOnlyDeviceTypes,
      waitOperandsDeviceTypes, waitOnlyDeviceTypes;
  toolchain::SmallVector<int32_t> numGangsSegments, waitOperandsSegments;
  toolchain::SmallVector<bool> hasWaitDevnums;

  toolchain::SmallVector<mlir::Value> reductionOperands, privateOperands,
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    if (const auto *asyncClause =
            std::get_if<language::Compability::parser::AccClause::Async>(&clause.u)) {
      genAsyncClause(converter, asyncClause, async, asyncDeviceTypes,
                     asyncOnlyDeviceTypes, crtDeviceTypes, stmtCtx);
    } else if (const auto *waitClause =
                   std::get_if<language::Compability::parser::AccClause::Wait>(&clause.u)) {
      genWaitClauseWithDeviceType(converter, waitClause, waitOperands,
                   ...
                   std::get_if<language::Compability::parser::AccClause::NumGangs>(
                       &clause.u)) {
      toolchain::SmallVector<mlir::Value> numGangValues;
      for (const language::Compability::parser::ScalarIntExpr &expr : numGangsClause->v)
      ...
      for (auto crtDeviceTypeAttr : crtDeviceTypes) {
        ...
        numGangsSegments.push_back(numGangValues.size());
                   ...
                   std::get_if<language::Compability::parser::AccClause::NumWorkers>(
                       &clause.u)) {
      mlir::Value numWorkerValue = fir::getBase(converter.genExprValue(
          *language::Compability::semantics::GetExpr(numWorkersClause->v), stmtCtx));
      for (auto crtDeviceTypeAttr : crtDeviceTypes) {
        ...
        numWorkersDeviceTypes.push_back(crtDeviceTypeAttr);
                   ...
                   std::get_if<language::Compability::parser::AccClause::VectorLength>(
                       &clause.u)) {
      mlir::Value vectorLengthValue = fir::getBase(converter.genExprValue(
          *language::Compability::semantics::GetExpr(vectorLengthClause->v), stmtCtx));
      for (auto crtDeviceTypeAttr : crtDeviceTypes) {
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    ...
    if (const auto *ifClause =
            std::get_if<language::Compability::parser::AccClause::If>(&clause.u)) {
      genIfClause(converter, clauseLocation, ifClause, ifCond, stmtCtx);
    } else if (const auto *selfClause =
                   std::get_if<language::Compability::parser::AccClause::Self>(&clause.u)) {
      const std::optional<language::Compability::parser::AccSelfClause> &accSelfClause =
          selfClause->v;
      if (accSelfClause) {
        if (const auto *optCondition =
                std::get_if<std::optional<language::Compability::parser::ScalarLogicalExpr>>(
                    &(*accSelfClause).u)) {
          if (*optCondition) {
            mlir::Value cond = fir::getBase(converter.genExprValue(
                *language::Compability::semantics::GetExpr(*optCondition), stmtCtx));
            selfCond = builder.createConvert(clauseLocation,
                                             builder.getI1Type(), cond);
          }
        } else if (const auto *accClauseList =
                       std::get_if<language::Compability::parser::AccObjectList>(
                           &(*accSelfClause).u)) {
          ...
          if (accClauseList->v.size() == 1) {
            ...
            if (const auto *designator =
                    std::get_if<language::Compability::parser::Designator>(&accObject.u)) {
              if (const auto *name =
                      language::Compability::semantics::getDesignatorNameIfDataRef(
                          *designator)) {
        ...
        addSelfAttr = true;
                   ...
                   std::get_if<language::Compability::parser::AccClause::Copy>(&clause.u)) {
      ...
      copyEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Copyin>(&clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
      genDataOperandOperationsWithModifier<mlir::acc::CopyinOp,
                                           language::Compability::parser::AccClause::Copyin>(
          copyinClause, converter, semanticsContext, stmtCtx,
          language::Compability::parser::AccDataModifier::Modifier::ReadOnly,
      ...
      copyinEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Copyout>(
                       &clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
      genDataOperandOperationsWithModifier<mlir::acc::CreateOp,
                                           language::Compability::parser::AccClause::Copyout>(
      ...
      copyoutEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Create>(&clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
      genDataOperandOperationsWithModifier<mlir::acc::CreateOp,
                                           language::Compability::parser::AccClause::Create>(
          createClause, converter, semanticsContext, stmtCtx,
          language::Compability::parser::AccDataModifier::Modifier::Zero, dataClauseOperands,
      ...
      createEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::NoCreate>(
                       &clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
      ...
      nocreateEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Present>(
                       &clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
      ...
      presentEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Deviceptr>(
                       &clause.u)) {
      genDataOperandOperations<mlir::acc::DevicePtrOp>(
          ...
          asyncOnlyDeviceTypes);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Attach>(&clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
      ...
      if (!combinedConstructs)
        genPrivatizationRecipes<mlir::acc::PrivateRecipeOp>(
            ...
            asyncOnlyDeviceTypes);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Firstprivate>(
                       &clause.u)) {
      ...
      if (!combinedConstructs) {
        ...
        auto crtDataStart = dataClauseOperands.size();
        genDataOperandOperations<mlir::acc::CopyinOp>(
            std::get<language::Compability::parser::AccObjectList>(reductionClause->v.t),
        ...
        copyEntryOperands.append(dataClauseOperands.begin() + crtDataStart,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Default>(
                       &clause.u)) {
      if ((defaultClause->v).v == toolchain::acc::DefaultValue::ACC_Default_none)
        hasDefaultNone = true;
      else if ((defaultClause->v).v ==
               toolchain::acc::DefaultValue::ACC_Default_present)
...
static void genACCDataOp(language::Compability::lower::AbstractConverter &converter,
                         mlir::Location currentLocation,
                         ...
                         const language::Compability::parser::AccClauseList &accClauseList) {
  mlir::Value ifCond;
  toolchain::SmallVector<mlir::Value> attachEntryOperands, createEntryOperands,
      copyEntryOperands, copyinEntryOperands, copyoutEntryOperands,
      nocreateEntryOperands, presentEntryOperands, dataClauseOperands,
      waitOperands, async;
  toolchain::SmallVector<mlir::Attribute> asyncDeviceTypes, asyncOnlyDeviceTypes,
      waitOperandsDeviceTypes, waitOnlyDeviceTypes;
  toolchain::SmallVector<int32_t> waitOperandsSegments;
  toolchain::SmallVector<bool> hasWaitDevnums;
...
static void
genACCHostDataOp(language::Compability::lower::AbstractConverter &converter,
                 mlir::Location currentLocation,
                 language::Compability::lower::pft::Evaluation &eval,
                 language::Compability::semantics::SemanticsContext &semanticsContext,
                 language::Compability::lower::StatementContext &stmtCtx,
                 const language::Compability::parser::AccClauseList &accClauseList) {
  mlir::Value ifCond;
  toolchain::SmallVector<mlir::Value> dataOperands;
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    ...
    if (const auto *ifClause =
      ...
      genIfClause(converter, clauseLocation, ifClause, ifCond, stmtCtx);
                   ...
                   std::get_if<language::Compability::parser::AccClause::UseDevice>(
                       &clause.u)) {
      genDataOperandOperations<mlir::acc::UseDeviceOp>(
          ...
          /*structured=*/true, /*implicit=*/false, /*async=*/{},
          /*asyncDeviceTypes=*/{}, /*asyncOnlyDeviceTypes=*/{});
    } else if (std::get_if<language::Compability::parser::AccClause::IfPresent>(&clause.u)) {
...
genACC(language::Compability::lower::AbstractConverter &converter,
       language::Compability::semantics::SemanticsContext &semanticsContext,
       language::Compability::lower::pft::Evaluation &eval,
       const language::Compability::parser::OpenACCBlockConstruct &blockConstruct) {
  const auto &beginBlockDirective =
      std::get<language::Compability::parser::AccBeginBlockDirective>(blockConstruct.t);
  const auto &blockDirective =
      std::get<language::Compability::parser::AccBlockDirective>(beginBlockDirective.t);
  const auto &accClauseList =
      std::get<language::Compability::parser::AccClauseList>(beginBlockDirective.t);
  const auto &endBlockDirective =
      std::get<language::Compability::parser::AccEndBlockDirective>(blockConstruct.t);
...
genACC(language::Compability::lower::AbstractConverter &converter,
       language::Compability::semantics::SemanticsContext &semanticsContext,
       language::Compability::lower::pft::Evaluation &eval,
       const language::Compability::parser::OpenACCCombinedConstruct &combinedConstruct) {
  const auto &beginCombinedDirective =
      std::get<language::Compability::parser::AccBeginCombinedDirective>(combinedConstruct.t);
  const auto &combinedDirective =
      std::get<language::Compability::parser::AccCombinedDirective>(beginCombinedDirective.t);
  const auto &accClauseList =
      std::get<language::Compability::parser::AccClauseList>(beginCombinedDirective.t);
  const auto &outerDoConstruct =
      std::get<std::optional<language::Compability::parser::DoConstruct>>(
          combinedConstruct.t);
  ...
  if (combinedDirective.v == toolchain::acc::ACCD_kernels_loop) {
  ...
  } else if (combinedDirective.v == toolchain::acc::ACCD_parallel_loop) {
  ...
  } else if (combinedDirective.v == toolchain::acc::ACCD_serial_loop) {
    ...
    toolchain::report_fatal_error("Unknown combined construct encountered");
  }
}

static void
genACCEnterDataOp(language::Compability::lower::AbstractConverter &converter,
                  mlir::Location currentLocation,
                  language::Compability::semantics::SemanticsContext &semanticsContext,
                  language::Compability::lower::StatementContext &stmtCtx,
                  const language::Compability::parser::AccClauseList &accClauseList) {
  mlir::Value ifCond, async, waitDevnum;
  toolchain::SmallVector<mlir::Value> waitOperands, dataClauseOperands;
  ...
  toolchain::SmallVector<mlir::Attribute> asyncDeviceTypes, asyncOnlyDeviceTypes;
  toolchain::SmallVector<mlir::Value> asyncValues;
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    ...
    if (const auto *ifClause =
      ...
      genWaitClause(converter, waitClause, waitOperands, waitDevnum,
                   ...
                   std::get_if<language::Compability::parser::AccClause::Copyin>(&clause.u)) {
      const language::Compability::parser::AccObjectListWithModifier &listWithModifier =
          copyinClause->v;
      const auto &accObjectList =
          std::get<language::Compability::parser::AccObjectList>(listWithModifier.t);
      genDataOperandOperations<mlir::acc::CopyinOp>(
      ...
      if (modifier &&
          (*modifier).v == language::Compability::parser::AccDataModifier::Modifier::Zero)
      ...
      genDataOperandOperations<mlir::acc::AttachOp>(
          ...
          /*implicit=*/false, asyncValues, asyncDeviceTypes,
          asyncOnlyDeviceTypes);
    } else if (!std::get_if<language::Compability::parser::AccClause::Async>(&clause.u)) {
      toolchain::report_fatal_error(
  ...
  // Prepare the operand segment size attribute and the operands value range.
  toolchain::SmallVector<mlir::Value, 16> operands;
  toolchain::SmallVector<int32_t, 8> operandSegments;
  addOperand(operands, operandSegments, ifCond);
...
static void
genACCExitDataOp(language::Compability::lower::AbstractConverter &converter,
                 mlir::Location currentLocation,
                 language::Compability::semantics::SemanticsContext &semanticsContext,
                 language::Compability::lower::StatementContext &stmtCtx,
                 const language::Compability::parser::AccClauseList &accClauseList) {
    ...
    if (const auto *ifClause =
      ...
      genDataOperandOperations<mlir::acc::GetDevicePtrOp>(
          ...
          asyncValues, asyncDeviceTypes, asyncOnlyDeviceTypes);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Delete>(&clause.u)) {
      genDataOperandOperations<mlir::acc::GetDevicePtrOp>(
          ...
          asyncValues, asyncDeviceTypes, asyncOnlyDeviceTypes);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Detach>(&clause.u)) {
      genDataOperandOperations<mlir::acc::GetDevicePtrOp>(
          detachClause->v, converter, semanticsContext, stmtCtx, detachOperands,
          mlir::acc::DataClause::acc_detach, false, /*implicit=*/false,
          asyncValues, asyncDeviceTypes, asyncOnlyDeviceTypes);
    } else if (std::get_if<language::Compability::parser::AccClause::Finalize>(&clause.u)) {
  ...
  // Prepare the operand segment size attribute and the operands value range.
  toolchain::SmallVector<mlir::Value, 14> operands;
  toolchain::SmallVector<int32_t, 7> operandSegments;
  addOperand(operands, operandSegments, ifCond);
  addOperand(operands, operandSegments, async);
  addOperand(operands, operandSegments, waitDevnum);
  addOperands(operands, operandSegments, waitOperands);
...
template <typename Op>
static void
genACCInitShutdownOp(language::Compability::lower::AbstractConverter &converter,
                     mlir::Location currentLocation,
                     const language::Compability::parser::AccClauseList &accClauseList) {
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    ...
    if (const auto *ifClause =
      ...
      genIfClause(converter, clauseLocation, ifClause, ifCond, stmtCtx);
                   ...
                   std::get_if<language::Compability::parser::AccClause::DeviceNum>(
                       &clause.u)) {
      deviceNum = fir::getBase(converter.genExprValue(
          *language::Compability::semantics::GetExpr(deviceNumClause->v), stmtCtx));
    } else if (const auto *deviceTypeClause =
                   std::get_if<language::Compability::parser::AccClause::DeviceType>(
                       &clause.u)) {
  ...
  // Prepare the operand segment size attribute and the operands value range.
  toolchain::SmallVector<mlir::Value, 6> operands;
  toolchain::SmallVector<int32_t, 2> operandSegments;
...
void genACCSetOp(language::Compability::lower::AbstractConverter &converter,
                 mlir::Location currentLocation,
                 const language::Compability::parser::AccClauseList &accClauseList) {
  mlir::Value ifCond, deviceNum, defaultAsync;
  toolchain::SmallVector<mlir::Value> deviceTypeOperands;
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    ...
    if (const auto *ifClause =
      ...
      genIfClause(converter, clauseLocation, ifClause, ifCond, stmtCtx);
                   ...
                   std::get_if<language::Compability::parser::AccClause::DefaultAsync>(
                       &clause.u)) {
      defaultAsync = fir::getBase(converter.genExprValue(
          *language::Compability::semantics::GetExpr(defaultAsyncClause->v), stmtCtx));
    } else if (const auto *deviceNumClause =
                   std::get_if<language::Compability::parser::AccClause::DeviceNum>(
                       &clause.u)) {
      deviceNum = fir::getBase(converter.genExprValue(
  ...
  // Prepare the operand segment size attribute and the operands value range.
  toolchain::SmallVector<mlir::Value> operands;
  toolchain::SmallVector<int32_t, 3> operandSegments;
...
getArrayAttr(fir::FirOpBuilder &b,
             toolchain::SmallVector<mlir::Attribute> &attributes) {
  return attributes.empty() ? nullptr : b.getArrayAttr(attributes);
}

static inline mlir::ArrayAttr
getBoolArrayAttr(fir::FirOpBuilder &b, toolchain::SmallVector<bool> &values) {
...
getDenseI32ArrayAttr(fir::FirOpBuilder &builder,
                     toolchain::SmallVector<int32_t> &values) {
  return values.empty() ? nullptr : builder.getDenseI32ArrayAttr(values);
}

static void
genACCUpdateOp(language::Compability::lower::AbstractConverter &converter,
               mlir::Location currentLocation,
               language::Compability::semantics::SemanticsContext &semanticsContext,
               language::Compability::lower::StatementContext &stmtCtx,
               const language::Compability::parser::AccClauseList &accClauseList) {
  mlir::Value ifCond;
  toolchain::SmallVector<mlir::Value> dataClauseOperands, updateHostOperands,
      waitOperands, deviceTypeOperands, asyncOperands;
  toolchain::SmallVector<mlir::Attribute> asyncOperandsDeviceTypes,
  ...
  for (const language::Compability::parser::AccClause &clause : accClauseList.v) {
    ...
    if (const auto *ifClause =
      ...
      genIfClause(converter, clauseLocation, ifClause, ifCond, stmtCtx);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Host>(&clause.u)) {
      genDataOperandOperations<mlir::acc::GetDevicePtrOp>(
          ...
          asyncOnlyDeviceTypes);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Device>(&clause.u)) {
      ...
      const std::optional<language::Compability::parser::AccSelfClause> &accSelfClause =
          selfClause->v;
      const auto *accObjectList =
          std::get_if<language::Compability::parser::AccObjectList>(&(*accSelfClause).u);
...
genACC(language::Compability::lower::AbstractConverter &converter,
       language::Compability::semantics::SemanticsContext &semanticsContext,
       const language::Compability::parser::OpenACCStandaloneConstruct &standaloneConstruct) {
  const auto &standaloneDirective =
      std::get<language::Compability::parser::AccStandaloneDirective>(standaloneConstruct.t);
  const auto &accClauseList =
      std::get<language::Compability::parser::AccClauseList>(standaloneConstruct.t);

  mlir::Location currentLocation =
      converter.genLocation(standaloneDirective.source);
  language::Compability::lower::StatementContext stmtCtx;

  if (standaloneDirective.v == toolchain::acc::Directive::ACCD_enter_data) {
    genACCEnterDataOp(converter, currentLocation, semanticsContext, stmtCtx,
                      accClauseList);
  } else if (standaloneDirective.v == toolchain::acc::Directive::ACCD_exit_data) {
    genACCExitDataOp(converter, currentLocation, semanticsContext, stmtCtx,
                     accClauseList);
  } else if (standaloneDirective.v == toolchain::acc::Directive::ACCD_init) {
    genACCInitShutdownOp<mlir::acc::InitOp>(converter, currentLocation,
                                            accClauseList);
  } else if (standaloneDirective.v == toolchain::acc::Directive::ACCD_shutdown) {
    genACCInitShutdownOp<mlir::acc::ShutdownOp>(converter, currentLocation,
                                                accClauseList);
  } else if (standaloneDirective.v == toolchain::acc::Directive::ACCD_set) {
    genACCSetOp(converter, currentLocation, accClauseList);
  } else if (standaloneDirective.v == toolchain::acc::Directive::ACCD_update) {
    genACCUpdateOp(converter, currentLocation, semanticsContext, stmtCtx,
                   accClauseList);
  }
}

static void genACC(language::Compability::lower::AbstractConverter &converter,
                   const language::Compability::parser::OpenACCWaitConstruct &waitConstruct) {

  const auto &waitArgument =
      std::get<std::optional<language::Compability::parser::AccWaitArgument>>(
          waitConstruct.t);
  const auto &accClauseList =
      std::get<language::Compability::parser::AccClauseList>(waitConstruct.t);
  ...
  if (waitArgument) { // wait has a value.
    const language::Compability::parser::AccWaitArgument &waitArg = *waitArgument;
    const auto &waitList =
        std::get<std::list<language::Compability::parser::ScalarIntExpr>>(waitArg.t);
    for (const language::Compability::parser::ScalarIntExpr &value : waitList) {
      mlir::Value v = fir::getBase(
...
static void createDeclareAllocFunc(mlir::OpBuilder &modBuilder,
  ...
  std::stringstream asFortran;
  asFortran << language::Compability::lower::mangle::demangleName(globalOp.getSymName());
  std::stringstream asFortranDesc;
  asFortranDesc << asFortran.str();
  if (unwrapFirBox)
    asFortranDesc << accFirDescriptorPostfix.str();
...
template <typename EntryOp, typename ExitOp>
static void genGlobalCtors(language::Compability::lower::AbstractConverter &converter,
                           mlir::OpBuilder &modBuilder,
                           const language::Compability::parser::AccObjectList &accObjectList,
                           mlir::acc::DataClause clause) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  auto genCtors = [&](const mlir::Location operandLocation,
    ...
    if (!globalOp) {
      if (language::Compability::semantics::FindEquivalenceSet(symbol)) {
        for (language::Compability::semantics::EquivalenceObject eqObj :
             *language::Compability::semantics::FindEquivalenceSet(symbol)) {
        ...
        if (!globalOp)
          toolchain::report_fatal_error("could not retrieve global symbol");
      } else {
        toolchain::report_fatal_error("could not retrieve global symbol");
      }
    }
        ...
        language::Compability::common::visitors{
            ...
            [&](const language::Compability::parser::Name &name) {
              if (const auto *symbol = name.symbol) {
                if (symbol
                        ->detailsIf<language::Compability::semantics::CommonBlockDetails>()) {
...
static void
genGlobalCtorsWithModifier(language::Compability::lower::AbstractConverter &converter,
...
static void
genDeclareInFunction(language::Compability::lower::AbstractConverter &converter,
                     language::Compability::semantics::SemanticsContext &semanticsContext,
                     language::Compability::lower::StatementContext &openAccCtx,
                     mlir::Location loc,
                     const language::Compability::parser::AccClauseList &accClauseList) {
  toolchain::SmallVector<mlir::Value> dataClauseOperands, copyEntryOperands,
    ...
    if (const auto *copyClause =
      ...
      genDeclareDataOperandOperations<mlir::acc::DevicePtrOp,
          ...
          /*structured=*/true, /*implicit=*/false);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Link>(&clause.u)) {
      genDeclareDataOperandOperations<mlir::acc::DeclareLinkOp,
          ...
          /*structured=*/true, /*implicit=*/false);
                   ...
                   std::get_if<language::Compability::parser::AccClause::DeviceResident>(
                       &clause.u)) {
      auto crtDataStart = dataClauseOperands.size();
...
static void
genDeclareInModule(language::Compability::lower::AbstractConverter &converter,
    ...
    if (const auto *createClause =
      ...
      genGlobalCtors<mlir::acc::CreateOp, mlir::acc::DeleteOp>(
          ...
          mlir::acc::DataClause::acc_create);
                   ...
                   std::get_if<language::Compability::parser::AccClause::Copyin>(&clause.u)) {
      genGlobalCtorsWithModifier<language::Compability::parser::AccClause::Copyin,
      ...
      toolchain::report_fatal_error("unsupported clause on DECLARE directive");
    }
  }
}
...
static void genACC(language::Compability::lower::AbstractConverter &converter,
                   language::Compability::semantics::SemanticsContext &semanticsContext,
                   language::Compability::lower::StatementContext &openAccCtx,
                   const language::Compability::parser::OpenACCStandaloneDeclarativeConstruct
                       &declareConstruct) {

  const auto &declarativeDir =
      std::get<language::Compability::parser::AccDeclarativeDirective>(declareConstruct.t);
  mlir::Location directiveLocation =
      converter.genLocation(declarativeDir.source);
  const auto &accClauseList =
      std::get<language::Compability::parser::AccClauseList>(declareConstruct.t);

  if (declarativeDir.v == toolchain::acc::Directive::ACCD_declare) {
    fir::FirOpBuilder &builder = converter.getFirOpBuilder();
    auto moduleOp =
        builder.getBlock()->getParent()->getParentOfType<mlir::ModuleOp>();
    auto funcOp =
  ...
  toolchain_unreachable("unsupported declarative directive");
}

static bool hasDeviceType(toolchain::SmallVector<mlir::Attribute> &arrayAttr,
...
getAttributeValueByDeviceType(toolchain::SmallVector<mlir::Attribute> &attributes,
                              toolchain::SmallVector<mlir::Attribute> &deviceTypes,
                              mlir::acc::DeviceType deviceType) {
  assert(attributes.size() == deviceTypes.size() &&
         "expect same number of attributes");
  for (auto it : toolchain::enumerate(deviceTypes)) {
...
static std::optional<toolchain::StringRef> getBindNameStringValue(
  ...
  return std::visit(
      [](const auto &attr) -> std::optional<toolchain::StringRef> {
...
static bool compareDeviceTypeInfo(
    ...
    toolchain::SmallVector<mlir::Attribute> &bindIdNameArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &bindStrNameArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &bindIdNameDeviceTypeArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &bindStrNameDeviceTypeArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &gangArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &gangDimArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &gangDimDeviceTypeArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &seqArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &workerArrayAttr,
    toolchain::SmallVector<mlir::Attribute> &vectorArrayAttr) {
  for (uint32_t dtypeInt = 0;
    ...
    if (bindNameValue !=
            getAttributeValueByDeviceType<toolchain::StringRef, mlir::StringAttr>(
...
static void attachRoutineInfo(mlir::func::FuncOp func,
  ...
  toolchain::SmallVector<mlir::SymbolRefAttr> routines;
...
void createOpenACCRoutineConstruct(
    ...
    bool hasNohost, toolchain::SmallVector<mlir::Attribute> &bindIdNames,
    toolchain::SmallVector<mlir::Attribute> &bindStrNames,
    toolchain::SmallVector<mlir::Attribute> &bindIdNameDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &bindStrNameDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &gangDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &gangDimValues,
    toolchain::SmallVector<mlir::Attribute> &gangDimDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &seqDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &workerDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &vectorDeviceTypes) {
...
static void interpretRoutineDeviceInfo(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::semantics::OpenACCRoutineDeviceTypeInfo &dinfo,
    toolchain::SmallVector<mlir::Attribute> &seqDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &vectorDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &workerDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &bindIdNameDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &bindStrNameDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &bindIdNames,
    toolchain::SmallVector<mlir::Attribute> &bindStrNames,
    toolchain::SmallVector<mlir::Attribute> &gangDeviceTypes,
    toolchain::SmallVector<mlir::Attribute> &gangDimValues,
    toolchain::SmallVector<mlir::Attribute> &gangDimDeviceTypes) {
  ...
  if (dinfo.bindNameOpt().has_value()) {
    ...
    if (const auto &bindSym{
            std::get_if<language::Compability::semantics::SymbolRef>(&bindName)}) {
      ...
      toolchain_unreachable("Unsupported bind name type");
    }
  }
}

void language::Compability::lower::genOpenACCRoutineConstruct(
    language::Compability::lower::AbstractConverter &converter, mlir::ModuleOp mod,
    mlir::func::FuncOp funcOp,
    const std::vector<language::Compability::semantics::OpenACCRoutineInfo> &routineInfos) {
  ...
  toolchain::SmallVector<mlir::Attribute> seqDeviceTypes, vectorDeviceTypes,
  ...
  for (const language::Compability::semantics::OpenACCRoutineInfo &info : routineInfos) {
    ...
    for (const language::Compability::semantics::OpenACCRoutineDeviceTypeInfo &dinfo :
...
genACC(language::Compability::lower::AbstractConverter &converter,
       language::Compability::lower::pft::Evaluation &eval,
       const language::Compability::parser::OpenACCAtomicConstruct &atomicConstruct) {
...
genACC(language::Compability::lower::AbstractConverter &converter,
       language::Compability::semantics::SemanticsContext &semanticsContext,
       const language::Compability::parser::OpenACCCacheConstruct &cacheConstruct) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  if (loopOp) {
    ...
    toolchain::SmallVector<mlir::Value> cacheOperands;
    const language::Compability::parser::AccObjectListWithModifier &listWithModifier =
        std::get<language::Compability::parser::AccObjectListWithModifier>(cacheConstruct.t);
    const auto &accObjectList =
        std::get<language::Compability::parser::AccObjectList>(listWithModifier.t);
    const auto &modifier =
        std::get<std::optional<language::Compability::parser::AccDataModifier>>(
    ...
    if (modifier &&
        (*modifier).v == language::Compability::parser::AccDataModifier::Modifier::ReadOnly)
...
mlir::Value language::Compability::lower::genOpenACCConstruct(
    language::Compability::lower::AbstractConverter &converter,
    language::Compability::semantics::SemanticsContext &semanticsContext,
    language::Compability::lower::pft::Evaluation &eval,
    const language::Compability::parser::OpenACCConstruct &accConstruct) {
  ...
  language::Compability::common::visit(
      common::visitors{
          ...
          [&](const language::Compability::parser::OpenACCCombinedConstruct
                  &combinedConstruct) {
          ...
          [&](const language::Compability::parser::OpenACCStandaloneConstruct
                  &standaloneConstruct) {
          ...
          [&](const language::Compability::parser::OpenACCEndConstruct &) {
            // No op
          },
      },
      accConstruct.u);
...
void language::Compability::lower::genOpenACCDeclarativeConstruct(
    language::Compability::lower::AbstractConverter &converter,
    language::Compability::semantics::SemanticsContext &semanticsContext,
    language::Compability::lower::StatementContext &openAccCtx,
    const language::Compability::parser::OpenACCDeclarativeConstruct &accDeclConstruct) {
  ...
  language::Compability::common::visit(
      common::visitors{
          ...
          [&](const language::Compability::parser::OpenACCRoutineConstruct &x) {},
      },
      accDeclConstruct.u);
}

void language::Compability::lower::attachDeclarePostAllocAction(
...
void language::Compability::lower::attachDeclarePreDeallocAction(
    AbstractConverter &converter, fir::FirOpBuilder &builder,
    mlir::Value beginOpValue, const language::Compability::semantics::Symbol &sym) {
  if (!sym.test(language::Compability::semantics::Symbol::Flag::AccCreate) &&
      !sym.test(language::Compability::semantics::Symbol::Flag::AccCopyIn) &&
      !sym.test(language::Compability::semantics::Symbol::Flag::AccCopyInReadOnly) &&
      !sym.test(language::Compability::semantics::Symbol::Flag::AccCopy) &&
      !sym.test(language::Compability::semantics::Symbol::Flag::AccCopyOut) &&
      !sym.test(language::Compability::semantics::Symbol::Flag::AccDeviceResident))
    return;
...
void language::Compability::lower::attachDeclarePostDeallocAction(
    AbstractConverter &converter, fir::FirOpBuilder &builder,
    const language::Compability::semantics::Symbol &sym) {
  if (!sym.test(language::Compability::semantics::Symbol::Flag::AccCreate) &&
      !sym.test(language::Compability::semantics::Symbol::Flag::AccCopyIn) &&
...
void language::Compability::lower::genOpenACCTerminator(fir::FirOpBuilder &builder,
                                          mlir::Operation *op,
                                          mlir::Location loc) {
...
bool language::Compability::lower::isInOpenACCLoop(fir::FirOpBuilder &builder) {
  if (builder.getBlock()->getParent()->getParentOfType<mlir::acc::LoopOp>())
    return true;
  return false;
}

bool language::Compability::lower::isInsideOpenACCComputeConstruct(
    fir::FirOpBuilder &builder) {
  return mlir::isa_and_nonnull<ACC_COMPUTE_CONSTRUCT_OPS>(
      mlir::acc::getEnclosingComputeOp(builder.getRegion()));
}

void language::Compability::lower::setInsertionPointAfterOpenACCLoopIfInside(
    fir::FirOpBuilder &builder) {
  if (auto loopOp =
...
void language::Compability::lower::genEarlyReturnInOpenACCLoop(fir::FirOpBuilder &builder,
                                                 mlir::Location loc) {
  mlir::Value yieldValue =
...
uint64_t language::Compability::lower::getLoopCountForCollapseAndTile(
    const language::Compability::parser::AccClauseList &clauseList) {
  uint64_t collapseLoopCount = 1;
  uint64_t tileLoopCount = 1;
  for (const language::Compability::parser::AccClause &clause : clauseList.v) {
    if (const auto *collapseClause =
            std::get_if<language::Compability::parser::AccClause::Collapse>(&clause.u)) {
      const parser::AccCollapseArg &arg = collapseClause->v;
      const auto &collapseValue{std::get<parser::ScalarIntConstantExpr>(arg.t)};
      collapseLoopCount = *language::Compability::semantics::GetIntValue(collapseValue);
    }
...
/// ACC loop creation
mlir::Operation *language::Compability::lower::genOpenACCLoopFromDoConstruct(
  ...
  auto scopeGuard = toolchain::make_scope_exit([&]() { localSymbols.popScope(); });

  // Prepare empty operand vectors since there are no associated `acc loop`
  // clauses with the Fortran do loops being handled here.
  toolchain::SmallVector<mlir::Value> privateOperands, gangOperands,
      workerNumOperands, vectorOperands, tileOperands, cacheOperands,
      reductionOperands;
  toolchain::SmallVector<mlir::Attribute> privatizationRecipes;
  toolchain::SmallVector<mlir::Type> retTy;
  ...
  if (parMode == mlir::acc::LoopParMode::loop_independent) {
    ...
    toolchain_unreachable("Unexpected loop par mode");
  }
