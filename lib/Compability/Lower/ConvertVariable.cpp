...
static toolchain::cl::opt<bool>
    allowAssumedRank("allow-assumed-rank",
                     toolchain::cl::desc("Enable assumed rank lowering"),
                     toolchain::cl::init(true));

#define DEBUG_TYPE "flang-lower-variable"

/// Helper to lower a scalar expression using a specific symbol mapping.
static mlir::Value genScalarValue(language::Compability::lower::AbstractConverter &converter,
                                  mlir::Location loc,
  ...
  if (converter.getLoweringOptions().getLowerToHighLevelFIR()) {
    hlfir::EntityWithAttributes loweredExpr =
        language::Compability::lower::convertExprToHLFIR(loc, converter, expr, symMap,
                                           context);
    return hlfir::loadTrivialScalar(loc, converter.getFirOpBuilder(),
                                    loweredExpr);
  }
  return fir::getBase(language::Compability::lower::createSomeExtendedExpression(
      loc, converter, expr, symMap, context));
}

/// Does this variable have a default initialization?
bool language::Compability::lower::hasDefaultInitialization(
    const language::Compability::semantics::Symbol &sym) {
  if (sym.has<language::Compability::semantics::ObjectEntityDetails>() && sym.size())
    if (!language::Compability::semantics::IsAllocatableOrPointer(sym))
      if (const language::Compability::semantics::DeclTypeSpec *declTypeSpec = sym.GetType())
        if (const language::Compability::semantics::DerivedTypeSpec *derivedTypeSpec =
          ...
          // lowering to NULL() (in language::Compability::lower::createMutableBox). However,
          ...
          const bool ignorePointer = language::Compability::semantics::IsIntentOut(sym);
...
static bool hasFinalization(const language::Compability::semantics::Symbol &sym) {
  if (sym.has<language::Compability::semantics::ObjectEntityDetails>())
    if (const language::Compability::semantics::DeclTypeSpec *declTypeSpec = sym.GetType())
      if (const language::Compability::semantics::DerivedTypeSpec *derivedTypeSpec =
              declTypeSpec->AsDerived())
        return language::Compability::semantics::IsFinalizable(*derivedTypeSpec);
  return false;
}

// Does this variable have an allocatable direct component?
static bool
hasAllocatableDirectComponent(const language::Compability::semantics::Symbol &sym) {
  if (sym.has<language::Compability::semantics::ObjectEntityDetails>())
    if (const language::Compability::semantics::DeclTypeSpec *declTypeSpec = sym.GetType())
      if (const language::Compability::semantics::DerivedTypeSpec *derivedTypeSpec =
              declTypeSpec->AsDerived())
        return language::Compability::semantics::HasAllocatableDirectComponent(
            *derivedTypeSpec);
  return false;
}
//===----------------------------------------------------------------===//
...
genInitializerExprValue(language::Compability::lower::AbstractConverter &converter,
                        mlir::Location loc,
                        const language::Compability::lower::SomeExpr &expr,
  ...
  language::Compability::lower::SymMap emptyMap;
  return language::Compability::lower::createSomeInitializerExpression(loc, converter, expr,
                                                         emptyMap, stmtCtx);
}

/// Can this symbol constant be placed in read-only memory?
static bool isConstant(const language::Compability::semantics::Symbol &sym) {
  return sym.attrs().test(language::Compability::semantics::Attr::PARAMETER) ||
         sym.test(language::Compability::semantics::Symbol::Flag::ReadOnly);
}
...
static mlir::Location genLocation(language::Compability::lower::AbstractConverter &converter,
                                  const language::Compability::semantics::Symbol &sym) {
...
static fir::GlobalOp declareGlobal(language::Compability::lower::AbstractConverter &converter,
...
static bool
hasDerivedTypeWithLengthParameters(const language::Compability::semantics::Symbol &sym) {
...
fir::ExtendedValue language::Compability::lower::genExtAddrInInitializer(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::lower::SomeExpr &addr) {
  ...
  if (const language::Compability::semantics::Symbol *sym =
          language::Compability::evaluate::GetFirstSymbol(addr)) {
    ...
    auto var = language::Compability::lower::pft::Variable(*sym, /*global=*/true);
    language::Compability::lower::instantiateVariable(converter, var, globalOpSymMap,
                                        storeMap);
  }

  if (converter.getLoweringOptions().getLowerToHighLevelFIR())
    return language::Compability::lower::convertExprToAddress(loc, converter, addr,
                                                globalOpSymMap, stmtCtx);
  return language::Compability::lower::createInitializerAddress(loc, converter, addr,
                                                  globalOpSymMap, stmtCtx);
}

/// create initial-data-target fir.box in a global initializer region.
mlir::Value language::Compability::lower::genInitialDataTarget(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Type boxType, const language::Compability::lower::SomeExpr &initialTarget,
  ...
  for (const auto &sym : language::Compability::evaluate::CollectSymbols(initialTarget)) {
    ...
    if (couldBeInEquivalence) {
      auto dependentVariableList =
          language::Compability::lower::pft::getDependentVariableList(sym);
      for (language::Compability::lower::pft::Variable var : dependentVariableList) {
  ...
  if (const language::Compability::evaluate::ProcedureRef *procRef =
          language::Compability::evaluate::UnwrapProcedureRef(initialTarget)) {
    ...
    if (intrinsic && intrinsic->name == "null") {
      ...
      assert(argExpr);
      const language::Compability::semantics::Symbol *sym =
          language::Compability::evaluate::GetFirstSymbol(*argExpr);
  ...
  if (converter.getLoweringOptions().getLowerToHighLevelFIR()) {
    auto target = language::Compability::lower::convertExprToBox(
    ...
    if (initialTarget.Rank() > 0) {
      auto target = language::Compability::lower::createSomeArrayBox(converter, initialTarget,
                                                       globalOpSymMap, stmtCtx);
      targetBox = fir::getBase(target);
      targetShift = builder.createShape(loc, target);
    } else {
      fir::ExtendedValue addr = language::Compability::lower::createInitializerAddress(
          loc, converter, initialTarget, globalOpSymMap, stmtCtx);
...
static mlir::Value genDefaultInitializerValue(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::semantics::Symbol &sym, mlir::Type symTy,
    language::Compability::lower::StatementContext &stmtCtx);
...
static mlir::Value genComponentDefaultInit(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::semantics::Symbol &component, fir::RecordType recTy,
    mlir::Value insertInto, language::Compability::lower::StatementContext &stmtCtx) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  if (const auto *object{
    ...
    if (const auto &init = object->init()) {
      ...
      if (language::Compability::semantics::IsPointer(component))
        // Initial data target.
        componentValue =
    ...
    } else if (language::Compability::semantics::IsAllocatableOrPointer(component)) {
    ...
    } else if (language::Compability::lower::hasDefaultInitialization(component)) {
      ...
      componentValue = fir::ZeroOp::create(builder, loc, componentTy);
                 ...
                 component
                     .detailsIf<language::Compability::semantics::ProcEntityDetails>()}) {
    if (proc->init().has_value()) {
      auto sym{*proc->init()};
      if (sym) // Has a procedure target.
        componentValue =
            language::Compability::lower::convertProcedureDesignatorInitialTarget(converter,
                                                                    loc, *sym);
...
static mlir::Value genDefaultInitializerValue(
  ...
  const language::Compability::semantics::DeclTypeSpec *declTy = sym.GetType();
  ...
  if (converter.getLoweringOptions().getLowerToHighLevelFIR()) {
    ...
    const language::Compability::semantics::Symbol &typeSymbol =
    ...
    language::Compability::semantics::OrderedComponentIterator components(
  ...
  if (sequenceType) {
    ...
    toolchain::SmallVector<int64_t> rangeBounds;
...
fir::GlobalOp language::Compability::lower::defineGlobal(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::pft::Variable &var, toolchain::StringRef globalName,
  ...
  if (!converter.getLoweringOptions().getLowerToHighLevelFIR() &&
      language::Compability::semantics::IsProcedurePointer(sym))
  ...
  if (mlir::isa<fir::SequenceType>(symTy) &&
      !language::Compability::semantics::IsAllocatableOrPointer(sym)) {
    ...
    if (mlir::isa<mlir::IntegerType, mlir::FloatType, mlir::ComplexType,
      ...
      if (details->init()) {
        global = language::Compability::lower::tryCreatingDenseGlobal(
  ...
  if (language::Compability::semantics::IsAllocatableOrPointer(sym) &&
      !language::Compability::semantics::IsProcedure(sym)) {
    const auto *details =
        sym.detailsIf<language::Compability::semantics::ObjectEntityDetails>();
    if (details && details->init()) {
      ...
      createGlobalInitialization(builder, global, [&](fir::FirOpBuilder &b) {
        mlir::Value box =
            language::Compability::lower::genInitialDataTarget(converter, loc, symTy, expr);
    ...
    } else if (language::Compability::lower::hasDefaultInitialization(sym)) {
      createGlobalInitialization(
          builder, global, [&](fir::FirOpBuilder &builder) {
            language::Compability::lower::StatementContext stmtCtx(
                /*cleanupProhibited=*/true);
  ...
  } else if (language::Compability::semantics::IsProcedurePointer(sym)) {
    const auto *details{sym.detailsIf<language::Compability::semantics::ProcEntityDetails>()};
    if (details && details->init()) {
      ...
      if (sym) // Has a procedure target.
        createGlobalInitialization(
            builder, global, [&](fir::FirOpBuilder &b) {
              language::Compability::lower::StatementContext stmtCtx(
                  /*cleanupProhibited=*/true);
              auto box{language::Compability::lower::convertProcedureDesignatorInitialTarget(
                  converter, loc, *sym)};
  ...
  } else if (sym.has<language::Compability::semantics::CommonBlockDetails>()) {
  ...
  if (!globalIsInitialized(global)) {
    ...
    // file.
    if (sym.attrs().test(language::Compability::semantics::Attr::BIND_C))
...
getLinkageAttribute(language::Compability::lower::AbstractConverter &converter,
                    const language::Compability::lower::pft::Variable &var) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  if (var.isRuntimeTypeInfoData() &&
      (!converter.getLoweringOptions().getSkipExternalRttiDefinition() ||
       language::Compability::semantics::IsFromBuiltinModule(var.getSymbol())))
...
static void instantiateGlobal(language::Compability::lower::AbstractConverter &converter,
                              const language::Compability::lower::pft::Variable &var,
                              language::Compability::lower::SymMap &symMap) {
  const language::Compability::semantics::Symbol &sym = var.getSymbol();
...
bool needCUDAAlloc(const language::Compability::semantics::Symbol &sym) {
  if (language::Compability::semantics::IsDummy(sym))
    return false;
  if (const auto *details{
          sym.GetUltimate()
              .detailsIf<language::Compability::semantics::ObjectEntityDetails>()}) {
    if (details->cudaDataAttr() &&
        (*details->cudaDataAttr() == language::Compability::common::CUDADataAttr::Device ||
         *details->cudaDataAttr() == language::Compability::common::CUDADataAttr::Managed ||
         *details->cudaDataAttr() == language::Compability::common::CUDADataAttr::Unified ||
         *details->cudaDataAttr() == language::Compability::common::CUDADataAttr::Shared ||
...
/// constructed.
static mlir::Value createNewLocal(language::Compability::lower::AbstractConverter &converter,
                                  mlir::Location loc,
                                  const language::Compability::lower::pft::Variable &var,
                                  mlir::Value preAlloc,
                                  toolchain::ArrayRef<mlir::Value> shape = {},
                                  toolchain::ArrayRef<mlir::Value> lenParams = {}) {
  if (preAlloc)
    return preAlloc;
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  toolchain::StringRef symNm = toStringRef(ultimateSymbol.name());
  ...
  if (ultimateSymbol.test(language::Compability::semantics::Symbol::Flag::CrayPointee))
  ...
  if (needCUDAAlloc(ultimateSymbol)) {
    ...
    toolchain::SmallVector<mlir::Value> elidedShape =
        fir::factory::elideExtentsAlreadyInType(ty, shape);
    toolchain::SmallVector<mlir::Value> elidedLenParams =
  ...
  if (!language::Compability::semantics::IsProcedurePointer(ultimateSymbol))
...
static void
initializeDeviceComponentAllocator(language::Compability::lower::AbstractConverter &converter,
  ...
  if (const auto *details{
    ...
    if (derived) {
      ...
      for (const auto &sym : components) {
        if (language::Compability::semantics::IsDeviceAllocatable(sym)) {
          toolchain::SmallVector<mlir::Value> coord;
          mlir::Type fieldTy =
              language::Compability::lower::gatherDeviceComponentCoordinatesAndType(
...
static bool
mustBeDefaultInitializedAtRuntime(const language::Compability::lower::pft::Variable &var) {
  if (!var.hasSymbol())
    return false;
  const language::Compability::semantics::Symbol &sym = var.getSymbol();
  if (var.isGlobal())
    ...
    return false;
  if (language::Compability::semantics::IsDummy(sym) && !language::Compability::semantics::IsIntentOut(sym))
    return false;
  // Polymorphic intent(out) dummy might need default initialization
  // at runtime.
  if (language::Compability::semantics::IsPolymorphic(sym) &&
      language::Compability::semantics::IsDummy(sym) &&
      language::Compability::semantics::IsIntentOut(sym) &&
      !language::Compability::semantics::IsAllocatable(sym) &&
      !language::Compability::semantics::IsPointer(sym))
    return true;
  ...
  return language::Compability::lower::hasDefaultInitialization(sym);
}

/// Call default initialization runtime routine to initialize \p var.
void language::Compability::lower::defaultInitializeAtRuntime(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::semantics::Symbol &sym, language::Compability::lower::SymMap &symMap) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Location loc = converter.getCurrentLocation();
  ...
  if (language::Compability::semantics::IsOptional(sym)) {
    ...
    const auto *details =
        sym.detailsIf<language::Compability::semantics::ObjectEntityDetails>();
    if (details && !language::Compability::semantics::IsPolymorphic(sym) &&
        declTy->category() ==
            language::Compability::semantics::DeclTypeSpec::Category::TypeDerived &&
        !mlir::isa<fir::SequenceType>(symTy) &&
        !sym.test(language::Compability::semantics::Symbol::Flag::OmpPrivate) &&
        !sym.test(language::Compability::semantics::Symbol::Flag::OmpFirstPrivate)) {
...
void language::Compability::lower::initializeCloneAtRuntime(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::semantics::Symbol &sym, language::Compability::lower::SymMap &symMap) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Location loc = converter.getCurrentLocation();
...
needDeallocationOrFinalization(const language::Compability::lower::pft::Variable &var) {
  if (!var.hasSymbol())
    return std::nullopt;
  const language::Compability::semantics::Symbol &sym = var.getSymbol();
  const language::Compability::semantics::Scope &owner = sym.owner();
  if (owner.kind() == language::Compability::semantics::Scope::Kind::MainProgram) {
  ...
  if (!language::Compability::semantics::IsPointer(sym) &&
      ...
      !language::Compability::semantics::IsSaved(sym)) {
    if (language::Compability::semantics::IsAllocatable(sym))
...
static bool
needDummyIntentoutFinalization(const language::Compability::semantics::Symbol &sym) {
  if (!language::Compability::semantics::IsDummy(sym) ||
      !language::Compability::semantics::IsIntentOut(sym) ||
      language::Compability::semantics::IsAllocatable(sym) ||
      language::Compability::semantics::IsPointer(sym))
  ...
  if (language::Compability::semantics::IsPolymorphic(sym) ||
      language::Compability::semantics::IsUnlimitedPolymorphic(sym))
    return true;
...
/// Must be nonpointer, nonallocatable, INTENT (OUT) dummy argument.
static bool
needDummyIntentoutFinalization(const language::Compability::lower::pft::Variable &var) {
  if (!var.hasSymbol())
    return false;
...
/// Call default initialization runtime routine to initialize \p var.
static void finalizeAtRuntime(language::Compability::lower::AbstractConverter &converter,
                              const language::Compability::lower::pft::Variable &var,
                              language::Compability::lower::SymMap &symMap) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Location loc = converter.getCurrentLocation();
...
static void deallocateIntentOut(language::Compability::lower::AbstractConverter &converter,
                                const language::Compability::lower::pft::Variable &var,
                                language::Compability::lower::SymMap &symMap) {
  if (!var.hasSymbol())
    return;
  ...
  if (language::Compability::semantics::IsDummy(sym) &&
    ...
    if (auto mutBox = extVal.getBoxOf<fir::MutableBoxValue>()) {
      ...
      if (language::Compability::semantics::IsOptional(sym)) {
        ...
        builder.genIfThen(loc, isPresent)
            .genThen([&]() {
              language::Compability::lower::genDeallocateIfAllocated(converter, *mutBox, loc);
            })
            .end();
      } else {
        language::Compability::lower::genDeallocateIfAllocated(converter, *mutBox, loc);
...
static bool needsRepack(language::Compability::lower::AbstractConverter &converter,
                        const language::Compability::semantics::Symbol &sym) {
  const auto &attrs = sym.attrs();
  if (!converter.getLoweringOptions().getRepackArrays() ||
      !converter.isRegisteredDummySymbol(sym) ||
      !language::Compability::semantics::IsAssumedShape(sym) ||
      language::Compability::evaluate::IsSimplyContiguous(sym,
      ...
      attrs.HasAny({language::Compability::semantics::Attr::ASYNCHRONOUS,
...
getSafeRepackAttrs(language::Compability::lower::AbstractConverter &converter) {
  toolchain::SmallVector<mlir::Attribute> attrs;
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  const auto &langFeatures = converter.getFoldingContext().languageFeatures();
  if (langFeatures.IsEnabled(language::Compability::common::LanguageFeature::OpenACC))
    attrs.push_back(
        fir::OpenACCSafeTempArrayCopyAttr::get(builder.getContext()));
  if (langFeatures.IsEnabled(language::Compability::common::LanguageFeature::OpenMP))
    attrs.push_back(
...
static void instantiateLocal(language::Compability::lower::AbstractConverter &converter,
                             const language::Compability::lower::pft::Variable &var,
                             language::Compability::lower::SymMap &symMap) {
  assert(!var.isAlias());
  language::Compability::lower::StatementContext stmtCtx;
  ...
  const bool isUnusedEntryDummy =
      var.hasSymbol() && language::Compability::semantics::IsDummy(var.getSymbol()) &&
  ...
  if (mustBeDefaultInitializedAtRuntime(var))
    language::Compability::lower::defaultInitializeAtRuntime(converter, var.getSymbol(),
                                               symMap);
  ...
  if (std::optional<VariableCleanUp> cleanup =
    ...
    case VariableCleanUp::Deallocate:
      ...
      converter.getFctCtx().attachCleanup([converterPtr, loc, exv, sym]() {
        ...
        language::Compability::lower::genDeallocateIfAllocated(*converterPtr, *mutableBox,
                                                 loc, sym);
      });
    }
    ...
    converter.getFctCtx().attachCleanup([converterPtr, loc, varDef, sym]() {
      language::Compability::lower::genUnpackArray(*converterPtr, loc, *varDef, *sym);
    });
  }
}
...
static void insertAggregateStore(language::Compability::lower::AggregateStoreMap &storeMap,
                                 const language::Compability::lower::pft::Variable &var,
                                 mlir::Value aggregateStore) {
  std::size_t off = var.getAggregateStore().getOffset();
  language::Compability::lower::AggregateStoreKey key = {var.getOwningScope(), off};
...
/// AggregateStoreMap.
static mlir::Value
getAggregateStore(language::Compability::lower::AggregateStoreMap &storeMap,
                  const language::Compability::lower::pft::Variable &alias) {
  language::Compability::lower::AggregateStoreKey key = {alias.getOwningScope(),
...
static std::string mangleGlobalAggregateStore(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::pft::Variable::AggregateStore &st) {
  return converter.mangleName(st.getNamingSymbol());
}

/// Build the type for the storage of an equivalence.
static mlir::Type
getAggregateType(language::Compability::lower::AbstractConverter &converter,
                 const language::Compability::lower::pft::Variable::AggregateStore &st) {
  if (const language::Compability::semantics::Symbol *initSym = st.getInitialValueSymbol())
...
static fir::GlobalOp defineGlobalAggregateStore(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::pft::Variable::AggregateStore &aggregate,
    toolchain::StringRef aggName, mlir::StringAttr linkage) {
  ...
  if (const language::Compability::semantics::Symbol *initSym =
          aggregate.getInitialValueSymbol())
    if (const auto *objectDetails =
            initSym->detailsIf<language::Compability::semantics::ObjectEntityDetails>())
...
static void
instantiateAggregateStore(language::Compability::lower::AbstractConverter &converter,
                          const language::Compability::lower::pft::Variable &var,
                          language::Compability::lower::AggregateStoreMap &storeMap) {
...
static void instantiateAlias(language::Compability::lower::AbstractConverter &converter,
                             const language::Compability::lower::pft::Variable &var,
                             language::Compability::lower::SymMap &symMap,
                             language::Compability::lower::AggregateStoreMap &storeMap) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
...
static bool
commonBlockHasInit(const language::Compability::semantics::MutableSymbolVector &cmnBlkMems) {
  for (const language::Compability::semantics::MutableSymbolRef &mem : cmnBlkMems) {
    if (const auto *memDet =
            mem->detailsIf<language::Compability::semantics::ObjectEntityDetails>())
      if (memDet->init())
        return true;
  }
  return false;
...
static mlir::TupleType getTypeOfCommonWithInit(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::semantics::MutableSymbolVector &cmnBlkMems,
...
/// list.
static language::Compability::semantics::MutableSymbolVector
getCommonMembersWithInitAliases(const language::Compability::semantics::Symbol &common) {
  const auto &commonDetails =
      common.get<language::Compability::semantics::CommonBlockDetails>();
  ...
  for (const language::Compability::semantics::EquivalenceSet &set :
       common.owner().equivalenceSets())
    for (const language::Compability::semantics::EquivalenceObject &obj : set) {
      if (!obj.symbol.test(language::Compability::semantics::Symbol::Flag::CompilerCreated)) {
        if (const auto &details =
                obj.symbol
                    .detailsIf<language::Compability::semantics::ObjectEntityDetails>()) {
          const language::Compability::semantics::Symbol *com =
              FindCommonBlockContaining(obj.symbol);
          if (!details->init() || com != &common)
            continue;
          // This is an alias with an init that belongs to the list
          if (!toolchain::is_contained(members, obj.symbol))
...
static fir::GlobalOp
getCommonBlockGlobal(language::Compability::lower::AbstractConverter &converter,
                     const language::Compability::semantics::Symbol &common) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
...
static std::optional<std::tuple<
    fir::GlobalOp, language::Compability::semantics::MutableSymbolVector, mlir::Location>>
declareCommonBlock(language::Compability::lower::AbstractConverter &converter,
                   const language::Compability::semantics::Symbol &common,
                   std::size_t commonSize) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  std::string commonName = converter.mangleName(common);
  ...
  if (global)
    return std::nullopt;
  language::Compability::semantics::MutableSymbolVector cmnBlkMems =
  ...
  const auto *details =
      common.detailsIf<language::Compability::semantics::CommonBlockDetails>();
  ...
  if (!commonBlockHasInit(cmnBlkMems)) {
    ...
    auto init = mlir::DenseElementsAttr::get(vecTy, toolchain::ArrayRef(zero));
...
static void finalizeCommonBlockDefinition(
  ...
  auto initFunc = [&](fir::FirOpBuilder &builder) {
    ...
    unsigned tupIdx = 0;
    std::size_t offset = 0;
    LLVM_DEBUG(toolchain::dbgs() << "block {\n");
    for (const language::Compability::semantics::MutableSymbolRef &mem : cmnBlkMems) {
      if (const auto *memDet =
              mem->detailsIf<language::Compability::semantics::ObjectEntityDetails>()) {
        ...
        if (memDet->init()) {
          ...
          fir::ExtendedValue initVal =
              language::Compability::semantics::IsPointer(*mem)
                  ? language::Compability::lower::genInitialDataTarget(
...
void language::Compability::lower::defineCommonBlocks(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::semantics::CommonBlockList &commonBlocks) {
  ...
  std::vector<std::tuple<fir::GlobalOp, language::Compability::semantics::MutableSymbolVector,
                         mlir::Location>>
...
mlir::Value language::Compability::lower::genCommonBlockMember(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::semantics::Symbol &sym, mlir::Value commonValue) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  return language::Compability::semantics::FindEquivalenceSet(sym) != nullptr
...
/// the symbol map.
static void instantiateCommon(language::Compability::lower::AbstractConverter &converter,
                              const language::Compability::semantics::Symbol &common,
                              const language::Compability::lower::pft::Variable &var,
                              language::Compability::lower::SymMap &symMap) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  const language::Compability::semantics::Symbol &varSym = var.getSymbol();
  mlir::Location loc = converter.genLocation(varSym.name());

  mlir::Value commonAddr;
  if (language::Compability::lower::SymbolBox symBox = symMap.lookupSymbol(common))
...
static bool lowerToBoxValue(const language::Compability::semantics::Symbol &sym,
                            mlir::Value dummyArg,
                            language::Compability::lower::AbstractConverter &converter) {
...
static void lowerExplicitLowerBounds(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::lower::BoxAnalyzer &box,
    toolchain::SmallVectorImpl<mlir::Value> &result, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  ...
  for (const language::Compability::semantics::ShapeSpec *spec : box.dynamicBound()) {
    if (auto low = spec->lbound().GetExplicit()) {
      auto expr = language::Compability::lower::SomeExpr{*low};
...
/// assumed-size array.
static void
lowerExplicitExtents(language::Compability::lower::AbstractConverter &converter,
                     mlir::Location loc, const language::Compability::lower::BoxAnalyzer &box,
                     toolchain::SmallVectorImpl<mlir::Value> &lowerBounds,
                     toolchain::SmallVectorImpl<mlir::Value> &result,
                     language::Compability::lower::SymMap &symMap,
                     language::Compability::lower::StatementContext &stmtCtx) {
  if (!box.isArray())
    return;
  ...
  for (const auto &spec : toolchain::enumerate(box.dynamicBound())) {
    if (auto up = spec.value()->ubound().GetExplicit()) {
      auto expr = language::Compability::lower::SomeExpr{*up};
...
/// explicit length.
static mlir::Value
lowerExplicitCharLen(language::Compability::lower::AbstractConverter &converter,
                     mlir::Location loc, const language::Compability::lower::BoxAnalyzer &box,
                     language::Compability::lower::SymMap &symMap,
                     language::Compability::lower::StatementContext &stmtCtx) {
  if (!box.isChar())
  ...
  if (std::optional<language::Compability::lower::SomeExpr> lenExpr = box.getCharLenExpr())
...
inline static toolchain::SmallVector<std::int64_t>
recoverShapeVector(toolchain::ArrayRef<std::int64_t> shapeVec, mlir::Value initVal) {
  toolchain::SmallVector<std::int64_t> result;
  if (initVal) {
    if (auto seqTy = fir::unwrapUntilSeqType(initVal.getType())) {
      for (auto [fst, snd] : toolchain::zip(shapeVec, seqTy.getShape()))
...
fir::FortranVariableFlagsAttr language::Compability::lower::translateSymbolAttributes(
  ...
  const auto &attrs = sym.attrs();
  if (attrs.test(language::Compability::semantics::Attr::ALLOCATABLE))
    flags = flags | fir::FortranVariableFlagsEnum::allocatable;
  if (attrs.test(language::Compability::semantics::Attr::ASYNCHRONOUS))
    flags = flags | fir::FortranVariableFlagsEnum::asynchronous;
  if (attrs.test(language::Compability::semantics::Attr::BIND_C))
    flags = flags | fir::FortranVariableFlagsEnum::bind_c;
  if (attrs.test(language::Compability::semantics::Attr::CONTIGUOUS))
    flags = flags | fir::FortranVariableFlagsEnum::contiguous;
  if (attrs.test(language::Compability::semantics::Attr::INTENT_IN))
    flags = flags | fir::FortranVariableFlagsEnum::intent_in;
  if (attrs.test(language::Compability::semantics::Attr::INTENT_INOUT))
    flags = flags | fir::FortranVariableFlagsEnum::intent_inout;
  if (attrs.test(language::Compability::semantics::Attr::INTENT_OUT))
    flags = flags | fir::FortranVariableFlagsEnum::intent_out;
  if (attrs.test(language::Compability::semantics::Attr::OPTIONAL))
    flags = flags | fir::FortranVariableFlagsEnum::optional;
  if (attrs.test(language::Compability::semantics::Attr::PARAMETER))
    flags = flags | fir::FortranVariableFlagsEnum::parameter;
  if (attrs.test(language::Compability::semantics::Attr::POINTER))
    flags = flags | fir::FortranVariableFlagsEnum::pointer;
  if (attrs.test(language::Compability::semantics::Attr::TARGET))
    flags = flags | fir::FortranVariableFlagsEnum::target;
  if (attrs.test(language::Compability::semantics::Attr::VALUE))
    flags = flags | fir::FortranVariableFlagsEnum::value;
  if (attrs.test(language::Compability::semantics::Attr::VOLATILE))
...
static bool
isCapturedInInternalProcedure(language::Compability::lower::AbstractConverter &converter,
                              const language::Compability::semantics::Symbol &sym) {
  const language::Compability::lower::pft::FunctionLikeUnit *funit =
  ...
  if (const auto *equivSet = language::Compability::semantics::FindEquivalenceSet(sym))
    for (const language::Compability::semantics::EquivalenceObject &eqObj : *equivSet)
...
static void genDeclareSymbol(language::Compability::lower::AbstractConverter &converter,
                             language::Compability::lower::SymMap &symMap,
                             const language::Compability::semantics::Symbol &sym,
                             mlir::Value base, mlir::Value len = {},
                             toolchain::ArrayRef<mlir::Value> shape = {},
                             toolchain::ArrayRef<mlir::Value> lbounds = {},
                             bool force = false) {
  ...
  if (converter.getLoweringOptions().getLowerToHighLevelFIR() &&
      (!language::Compability::semantics::IsProcedure(sym) ||
       language::Compability::semantics::IsPointer(sym)) &&
      !sym.detailsIf<language::Compability::semantics::CommonBlockDetails>()) {
    fir::FirOpBuilder &builder = converter.getFirOpBuilder();
    const mlir::Location loc = genLocation(converter, sym);
    ...
    if (sym.test(language::Compability::semantics::Symbol::Flag::CrayPointee)) {
      mlir::Type ptrBoxType =
          language::Compability::lower::getCrayPointeeBoxType(base.getType());
      ...
      mlir::Value nullAddr = builder.createNullConstant(
          loc, toolchain::cast<fir::BaseBoxType>(ptrBoxType).getEleTy());
...
void language::Compability::lower::genDeclareSymbol(
    language::Compability::lower::AbstractConverter &converter,
    language::Compability::lower::SymMap &symMap, const language::Compability::semantics::Symbol &sym,
    const fir::ExtendedValue &exv, fir::FortranVariableFlagsEnum extraFlags,
    bool force) {
  if (converter.getLoweringOptions().getLowerToHighLevelFIR() &&
      (!language::Compability::semantics::IsProcedure(sym) ||
       language::Compability::semantics::IsPointer(sym.GetUltimate())) &&
      !sym.detailsIf<language::Compability::semantics::CommonBlockDetails>()) {
    fir::FirOpBuilder &builder = converter.getFirOpBuilder();
    const mlir::Location loc = genLocation(converter, sym);
    if (isCapturedInInternalProcedure(converter, sym))
    ...
    fir::FortranVariableFlagsAttr attributes =
        language::Compability::lower::translateSymbolAttributes(
...
static void
genAllocatableOrPointerDeclare(language::Compability::lower::AbstractConverter &converter,
                               language::Compability::lower::SymMap &symMap,
                               const language::Compability::semantics::Symbol &sym,
...
static void genProcPointer(language::Compability::lower::AbstractConverter &converter,
                           language::Compability::lower::SymMap &symMap,
                           const language::Compability::semantics::Symbol &sym,
...
static void genBoxDeclare(language::Compability::lower::AbstractConverter &converter,
                          language::Compability::lower::SymMap &symMap,
                          const language::Compability::semantics::Symbol &sym,
                          mlir::Value box, toolchain::ArrayRef<mlir::Value> lbounds,
                          toolchain::ArrayRef<mlir::Value> explicitParams,
                          toolchain::ArrayRef<mlir::Value> explicitExtents,
                          bool replace = false) {
  if (converter.getLoweringOptions().getLowerToHighLevelFIR()) {
    fir::BoxValue boxValue{box, lbounds, explicitParams, explicitExtents};
    language::Compability::lower::genDeclareSymbol(
...
void language::Compability::lower::mapSymbolAttributes(
    AbstractConverter &converter, const language::Compability::lower::pft::Variable &var,
  ...
  const mlir::Location loc = genLocation(converter, sym);
  mlir::IndexType idxTy = builder.getIndexType();
  const bool isDeclaredDummy = language::Compability::semantics::IsDummy(sym);
  ...
  const bool isResult = language::Compability::semantics::IsFunctionResult(sym);
  ...
  if (language::Compability::semantics::IsProcedure(sym)) {
    if (isUnusedEntryDummy) {
      ...
      mlir::Type dummyProcType =
          language::Compability::lower::getDummyProcedureType(sym, converter);
      mlir::Value undefOp = fir::UndefOp::create(builder, loc, dummyProcType);

      language::Compability::lower::genDeclareSymbol(converter, symMap, sym, undefOp);
    }
    ...
    if (language::Compability::semantics::IsPointer(sym)) {
      ...
      if (!boxAlloc)
        if (language::Compability::lower::SymbolBox symbox = symMap.lookupSymbol(sym))
  ...
  const bool isAssumedRank = language::Compability::evaluate::IsAssumedRank(sym);
  ...
  if (language::Compability::semantics::IsAllocatableOrPointer(sym)) {
    ...
    } else if (const language::Compability::semantics::DeclTypeSpec *declTy = sym.GetType()) {
      if (const language::Compability::semantics::DerivedTypeSpec *derived =
              declTy->AsDerived())
    ...
    fir::MutableBoxValue box = language::Compability::lower::createMutableBox(
        converter, loc, var, boxAlloc, nonDeferredLenParams,
        /*alwaysUseBox=*/
        converter.getLoweringOptions().getLowerToHighLevelFIR(),
        language::Compability::lower::getAllocatorIdx(var.getSymbol()));
  ...
  if (isDummy) {
    ...
    if (lowerToBoxValue(sym, dummyArg, converter)) {
      ...
      toolchain::SmallVector<mlir::Value> explicitExtents;
      toolchain::SmallVector<mlir::Value> explicitParams;
  ...
  auto genUnusedEntryPointBox = [&]() {
    if (isUnusedEntryDummy) {
      assert(!language::Compability::semantics::IsAllocatableOrPointer(sym) &&
             "handled above");
  ...
  auto genValue = [&](const language::Compability::lower::SomeExpr &expr) {
  ...
  auto populateShape = [&](auto &shapes, const auto &bounds, mlir::Value box) {
    for (auto iter : toolchain::enumerate(bounds)) {
      ...
      if (auto high = spec->ubound().GetExplicit()) {
        language::Compability::lower::SomeExpr highEx{*high};
        ...
        toolchain::report_fatal_error("unknown bound category");
      }
    }
  };
  ...
  auto populateLBoundsExtents = [&](auto &lbounds, auto &extents,
    ...
    for (auto iter : toolchain::enumerate(bounds)) {
      ...
      if (spec->lbound().isColon() || spec->ubound().isColon()) {
        ...
        if (auto high = spec->ubound().GetExplicit()) {
          auto expr = language::Compability::lower::SomeExpr{*high};
  ...
  if (ba.isChar()) {
    ...
    if (std::optional<int64_t> cstLen = ba.getCharLenConst()) {
      ...
      if (genUnusedEntryPointBox())
        return;
      if (std::optional<language::Compability::lower::SomeExpr> charLenExpr =
              ba.getCharLenExpr()) {
        // Explicit length
  ...
  if (!addr) {
    if (arg) {
      ...
      const bool isCptrByVal = language::Compability::semantics::IsBuiltinCPtr(sym) &&
                               language::Compability::lower::isCPtrArgByValueType(argType);
...
void language::Compability::lower::defineModuleVariable(
    AbstractConverter &converter, const language::Compability::lower::pft::Variable &var) {
  ...
  const language::Compability::semantics::Symbol &sym = var.getSymbol();
  if (const language::Compability::semantics::Symbol *common =
          language::Compability::semantics::FindCommonBlockContaining(var.getSymbol())) {
...
void language::Compability::lower::instantiateVariable(AbstractConverter &converter,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "instantiateVariable: "; var.dump());
  if (var.isAggregateStore())
    instantiateAggregateStore(converter, var, storeMap);
  else if (const language::Compability::semantics::Symbol *common =
               language::Compability::semantics::FindCommonBlockContaining(
                   var.getSymbol().GetUltimate()))
...
static void
mapCallInterfaceSymbol(const language::Compability::semantics::Symbol &interfaceSymbol,
                       language::Compability::lower::AbstractConverter &converter,
                       const language::Compability::lower::CallerInterface &caller,
                       language::Compability::lower::SymMap &symMap) {
  language::Compability::lower::AggregateStoreMap storeMap;
  for (language::Compability::lower::pft::Variable var :
       language::Compability::lower::pft::getDependentVariableList(interfaceSymbol)) {
    if (var.isAggregateStore()) {
...
void language::Compability::lower::mapCallInterfaceSymbolsForResult(
    AbstractConverter &converter, const language::Compability::lower::CallerInterface &caller,
    SymMap &symMap) {
  const language::Compability::semantics::Symbol &result = caller.getResultSymbol();
  mapCallInterfaceSymbol(result, converter, caller, symMap);
}

void language::Compability::lower::mapCallInterfaceSymbolsForDummyArgument(
    AbstractConverter &converter, const language::Compability::lower::CallerInterface &caller,
...
void language::Compability::lower::mapSymbolAttributes(
    AbstractConverter &converter, const language::Compability::semantics::SymbolRef &symbol,
    language::Compability::lower::SymMap &symMap, language::Compability::lower::StatementContext &stmtCtx,
    mlir::Value preAlloc) {
...
void language::Compability::lower::createIntrinsicModuleGlobal(
    language::Compability::lower::AbstractConverter &converter, const pft::Variable &var) {
  defineGlobal(converter, var, converter.mangleName(var.getSymbol()),
               converter.getFirOpBuilder().createLinkOnceODRLinkage());
}

void language::Compability::lower::createRuntimeTypeInfoGlobal(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::semantics::Symbol &typeInfoSym) {
  std::string globalName = converter.mangleName(typeInfoSym);
  auto var = language::Compability::lower::pft::Variable(typeInfoSym, /*global=*/true);
  mlir::StringAttr linkage = getLinkageAttribute(converter, var);
  defineGlobal(converter, var, globalName, linkage);
}

mlir::Type language::Compability::lower::getCrayPointeeBoxType(mlir::Type fortranType) {
  ...
  if (auto seqType = mlir::dyn_cast<fir::SequenceType>(baseType)) {
    ...
    toolchain::SmallVector<int64_t> shape(seqType.getDimension(),
...
fir::ExtendedValue
language::Compability::lower::genPackArray(language::Compability::lower::AbstractConverter &converter,
                             const language::Compability::semantics::Symbol &sym,
                             fir::ExtendedValue exv) {
  ...
  exv.match(
      ...
      [](const auto &) {
        toolchain_unreachable("unexpected lowering for assumed-shape dummy");
      });
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  const mlir::Location loc = genLocation(converter, sym);
  ...
  // value of the array.
  bool noCopy = language::Compability::semantics::IsIntentOut(sym) &&
...
void language::Compability::lower::genUnpackArray(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
  ...
  bool noCopy = language::Compability::semantics::IsIntentIn(sym);
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
