...
public:
  ...
  lower(const language::Compability::lower::PreparedActualArguments &loweredActuals,
...
protected:
  ...
  mlir::Location loc;
  toolchain::SmallVector<hlfir::CleanupFunction, 3> cleanupFns;

  virtual mlir::Value
  lowerImpl(const language::Compability::lower::PreparedActualArguments &loweredActuals,
            const fir::IntrinsicArgumentLoweringRules *argLowering,
            mlir::Type stmtResultType) = 0;

  toolchain::SmallVector<mlir::Value> getOperandVector(
  ...
  mlir::Value loadBoxAddress(
      const std::optional<language::Compability::lower::PreparedActualArgument> &arg);
...
toolchain::SmallVector<mlir::Value> HlfirTransformationalIntrinsic::getOperandVector(
    const language::Compability::lower::PreparedActualArguments &loweredActuals,
    const fir::IntrinsicArgumentLoweringRules *argLowering) {
  ...
  for (size_t i = 0; i < loweredActuals.size(); ++i) {
    std::optional<language::Compability::lower::PreparedActualArgument> arg =
        loweredActuals[i];
    if (!arg) {
...
mlir::Value HlfirTransposeLowering::lowerImpl(
  ...
  toolchain::ArrayRef<int64_t> arrayShape = array.getShape();
...
std::optional<hlfir::EntityWithAttributes> language::Compability::lower::lowerHlfirIntrinsic(
