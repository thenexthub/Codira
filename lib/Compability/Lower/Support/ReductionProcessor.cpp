...
static toolchain::cl::opt<bool> forceByrefReduction(
    "force-byref-reduction",
    toolchain::cl::desc("Pass all reduction arguments by reference"),
    toolchain::cl::Hidden);
...
template bool ReductionProcessor::processReductionArguments<
    fir::DeclareReductionOp, toolchain::SmallVector<fir::ReduceOperationEnum>>(
    mlir::Location currentLocation, lower::AbstractConverter &converter,
    const toolchain::SmallVector<fir::ReduceOperationEnum> &redOperatorList,
    toolchain::SmallVectorImpl<mlir::Value> &reductionVars,
    toolchain::SmallVectorImpl<bool> &reduceVarByRef,
    toolchain::SmallVectorImpl<mlir::Attribute> &reductionDeclSymbols,
    const toolchain::SmallVectorImpl<const semantics::Symbol *> &reductionSymbols);
...
ReductionProcessor::createDeclareReduction<mlir::omp::DeclareReductionOp>(
    AbstractConverter &converter, toolchain::StringRef reductionOpName,
...
ReductionProcessor::ReductionIdentifier ReductionProcessor::getReductionType(
  ...
  auto redType = toolchain::StringSwitch<std::optional<ReductionIdentifier>>(
...
ReductionProcessor::ReductionIdentifier ReductionProcessor::getReductionType(
  ...
  default:
    toolchain_unreachable("unexpected intrinsic operator in reduction");
  }
}
...
ReductionProcessor::getReductionType(const fir::ReduceOperationEnum &redOp) {
  ...
  toolchain_unreachable("Unhandled ReductionIdentifier case");
}
...
bool ReductionProcessor::supportedIntrinsicProcReduction(
  ...
  if (!sym->GetUltimate().attrs().test(semantics::Attr::INTRINSIC))
    return false;
  auto redType = toolchain::StringSwitch<bool>(getRealName(sym).ToString())
                     .Case("max", true)
                     .Case("min", true)
                     .Case("iand", true)
                     .Case("ior", true)
...
std::string
ReductionProcessor::getReductionName(toolchain::StringRef name,
  ...
  // reference
  toolchain::StringRef byrefAddition{""};
  if (isByRef)
...
ReductionProcessor::getReductionInitValue(mlir::Location loc, mlir::Type type,
  ...
  case ReductionIdentifier::MAX: {
    if (auto ty = mlir::dyn_cast<mlir::FloatType>(type)) {
      ...
      return builder.createRealConstant(
          loc, type, toolchain::APFloat::getLargest(sem, /*Negative=*/true));
    }
  ...
  case ReductionIdentifier::MIN: {
    if (auto ty = mlir::dyn_cast<mlir::FloatType>(type)) {
      const toolchain::fltSemantics &sem = ty.getFloatSemantics();
      return builder.createRealConstant(
          loc, type, toolchain::APFloat::getLargest(sem, /*Negative=*/false));
    }
    unsigned bits = type.getIntOrFloatBitWidth();
    int64_t maxInt = toolchain::APInt::getSignedMaxValue(bits).getSExtValue();
  ...
  case ReductionIdentifier::IAND: {
    unsigned bits = type.getIntOrFloatBitWidth();
    int64_t allOnInt = toolchain::APInt::getAllOnes(bits).getSExtValue();
  ...
  toolchain_unreachable("Unhandled Reduction identifier : getReductionInitValue");
}
...
bool ReductionProcessor::processReductionArguments(
  ...
  if constexpr (std::is_same_v<RedOperatorListTy,
    ...
    const language::Compability::lower::omp::clause::ReductionOperator &redOperator =
  ...
  unsigned idx = 0;
  for (auto [symVal, isByRef] : toolchain::zip(reductionVars, reduceVarByRef)) {
