...
public:
  ...
  language::Compability::lower::VectorSubscriptBox gen(const language::Compability::lower::SomeExpr &expr) {
    elementType = genDesignator(expr);
    return language::Compability::lower::VectorSubscriptBox(
...
private:
  using LoweredVectorSubscript =
      language::Compability::lower::VectorSubscriptBox::LoweredVectorSubscript;
  using LoweredTriplet = language::Compability::lower::VectorSubscriptBox::LoweredTriplet;
  using LoweredSubscript = language::Compability::lower::VectorSubscriptBox::LoweredSubscript;
  using MaybeSubstring = language::Compability::lower::VectorSubscriptBox::MaybeSubstring;
  ...
  template <typename T>
  mlir::Type genDesignator(const language::Compability::evaluate::Expr<T> &expr) {
    using ExprVariant = decltype(language::Compability::evaluate::Expr<T>::u);
    using Designator = language::Compability::evaluate::Designator<T>;
    if constexpr (language::Compability::common::HasMember<Designator, ExprVariant>) {
  ...
  mlir::Type gen(const language::Compability::evaluate::DataRef &dataRef) {
    return language::Compability::common::visit(
        [&](const auto &ref) -> mlir::Type { return gen(ref); }, dataRef.u);
  }

  mlir::Type gen(const language::Compability::evaluate::SymbolRef &symRef) {
  ...
  mlir::Type gen(const language::Compability::evaluate::Substring &substring) {
    ...
    mlir::Type baseElementType =
        gen(std::get<language::Compability::evaluate::DataRef>(substring.parent()));
  ...
  mlir::Type gen(const language::Compability::evaluate::ComplexPart &complexPart) {
    auto complexType = gen(complexPart.complex());
    fir::FirOpBuilder &builder = converter.getFirOpBuilder();
    mlir::Type i32Ty = builder.getI32Type(); // toolchain's GEP requires i32
    mlir::Value offset = builder.createIntegerConstant(
        loc, i32Ty,
        complexPart.part() == language::Compability::evaluate::ComplexPart::Part::RE ? 0 : 1);
    componentPath.emplace_back(offset);
    return fir::factory::Complex{builder, loc}.getComplexPartType(complexType);
  }

  mlir::Type gen(const language::Compability::evaluate::Component &component) {
    auto recTy = mlir::cast<fir::RecordType>(gen(component.base()));
    const language::Compability::semantics::Symbol &componentSymbol =
        component.GetLastSymbol();
    // Parent components will not be found here, they are not part
    // of the FIR type and cannot be used in the path yet.
    if (componentSymbol.test(language::Compability::semantics::Symbol::Flag::ParentComp))
      TODO(loc, "reference to parent component");
    mlir::Type fldTy = fir::FieldType::get(&converter.getMLIRContext());
    toolchain::StringRef componentName = toStringRef(componentSymbol.name());
  ...
  mlir::Type gen(const language::Compability::evaluate::ArrayRef &arrayRef) {
    auto isTripletOrVector =
        [](const language::Compability::evaluate::Subscript &subscript) -> bool {
      return language::Compability::common::visit(
          language::Compability::common::visitors{
              [](const language::Compability::evaluate::IndirectSubscriptIntegerExpr &expr) {
                return expr.value().Rank() != 0;
              },
              [&](const language::Compability::evaluate::Triplet &) { return true; }},
          subscript.u);
    };
    if (toolchain::any_of(arrayRef.subscript(), isTripletOrVector))
    ...
    for (const language::Compability::evaluate::Subscript &subscript : arrayRef.subscript()) {
      const auto &expr =
  ...
  mlir::Type genRankedArrayRefSubscriptAndBase(
      const language::Compability::evaluate::ArrayRef &arrayRef) {
    // Lower the save the base
    language::Compability::lower::SomeExpr baseExpr = namedEntityToExpr(arrayRef.base());
    loweredBase = converter.genExprAddr(baseExpr, stmtCtx);
    // Lower and save the subscripts
    fir::FirOpBuilder &builder = converter.getFirOpBuilder();
    mlir::Type idxTy = builder.getIndexType();
    mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
    for (const auto &subscript : toolchain::enumerate(arrayRef.subscript())) {
      language::Compability::common::visit(
          language::Compability::common::visitors{
              ...
              [&](const language::Compability::evaluate::Triplet &triplet) {
                mlir::Value lb, ub;
  ...
  mlir::Type gen(const language::Compability::evaluate::CoarrayRef &) {
  ...
  namedEntityToDataRef(const language::Compability::evaluate::NamedEntity &namedEntity) {
    if (namedEntity.IsSymbol())
      return language::Compability::evaluate::DataRef{namedEntity.GetFirstSymbol()};
    return language::Compability::evaluate::DataRef{namedEntity.GetComponent()};
  }

  language::Compability::lower::SomeExpr
  namedEntityToExpr(const language::Compability::evaluate::NamedEntity &namedEntity) {
    return language::Compability::evaluate::AsGenericExpr(namedEntityToDataRef(namedEntity))
        .value();
  }
  ...
  toolchain::SmallVector<LoweredSubscript, 16> loweredSubscripts;
  toolchain::SmallVector<mlir::Value> componentPath;
  MaybeSubstring substringBounds;
  mlir::Type elementType;
};
} // namespace

language::Compability::lower::VectorSubscriptBox language::Compability::lower::genVectorSubscriptBox(
...
mlir::Value language::Compability::lower::VectorSubscriptBox::loopOverElementsBase(
  ...
  // major order.
  toolchain::SmallVector<mlir::Value> inductionVariables;
  LoopType outerLoop;
...
void language::Compability::lower::VectorSubscriptBox::loopOverElements(
    fir::FirOpBuilder &builder, mlir::Location loc,
...
mlir::Value language::Compability::lower::VectorSubscriptBox::loopOverElementsWhile(
    fir::FirOpBuilder &builder, mlir::Location loc,
...
mlir::Value
language::Compability::lower::VectorSubscriptBox::createSlice(fir::FirOpBuilder &builder,
                                                mlir::Location loc) {
  mlir::Type idxTy = builder.getIndexType();
  toolchain::SmallVector<mlir::Value> triples;
  mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
...
toolchain::SmallVector<std::tuple<mlir::Value, mlir::Value, mlir::Value>>
language::Compability::lower::VectorSubscriptBox::genLoopBounds(fir::FirOpBuilder &builder,
                                                  mlir::Location loc) {
  mlir::Type idxTy = builder.getIndexType();
  mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
  mlir::Value zero = builder.createIntegerConstant(loc, idxTy, 0);
  toolchain::SmallVector<std::tuple<mlir::Value, mlir::Value, mlir::Value>> bounds;
  size_t dimension = loweredSubscripts.size();
  for (const LoweredSubscript &subscript : toolchain::reverse(loweredSubscripts)) {
    --dimension;
...
fir::ExtendedValue language::Compability::lower::VectorSubscriptBox::getElementAt(
  ...
  mlir::Type idxTy = builder.getIndexType();
  toolchain::SmallVector<mlir::Value> indexes;
