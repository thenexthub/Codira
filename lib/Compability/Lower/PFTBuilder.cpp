...
#include "language/Compability/Lower/IntervalSet.h"
...
static toolchain::cl::opt<bool> clDisableStructuredFir(
    "no-structured-fir", toolchain::cl::desc("disable generation of structured FIR"),
    toolchain::cl::init(false), toolchain::cl::Hidden);

using namespace language::Compability;

namespace {
/// Helpers to unveil parser node inside language::Compability::parser::Statement<>,
/// language::Compability::parser::UnlabeledStatement, and language::Compability::common::Indirection<>
template <typename A>
...
public:
  ...
  void checkForFPEnvironmentCalls(const parser::CallStmt &callStmt) {
    ...
    if (!callName)
      return;
    const language::Compability::semantics::Symbol &procSym = callName->symbol->GetUltimate();
    if (!procSym.owner().IsModule())
      return;
    const language::Compability::semantics::Symbol &modSym = *procSym.owner().symbol();
    if (!modSym.attrs().test(language::Compability::semantics::Attr::INTRINSIC))
      return;
    ...
    toolchain::StringRef modName = toStringRef(modSym.name());
    if (!modName.starts_with("ieee_") && !modName.starts_with("__fortran_"))
      return;
    toolchain::StringRef procName = toStringRef(procSym.name());
  ...
  bool Pre(const parser::Statement<parser::ForallAssignmentStmt> &statement) {
    addEvaluation(language::Compability::common::visit(
        [&](const auto &x) {
...
private:
  ...
  void endFunctionBody() {
    ...
    if (evaluationList->empty() || !evaluationList->back().isEndStmt()) {
      ...
      endStmt.visit(common::visitors{
          ...
          [&](const auto &s) {
            toolchain::report_fatal_error("missing end statement or unexpected "
  ...
  void rewriteIfGotos() {
    ...
    toolchain::SmallVector<T> ifCandidateStack;
    const auto *doStmt =
    ...
    for (auto it = evaluationList.begin(), end = evaluationList.end();
      ...
      const language::Compability::lower::pft::Evaluation &targetEval = *firstStmt(&eval);
      ...
      if (targetEval.label || targetEvalIsEndDoStmt) {
        while (!ifCandidateStack.empty() && branchTargetMatch()) {
          ...
          if (successorIt != it) {
            language::Compability::lower::pft::EvaluationList &ifBodyList =
      ...
      for (const auto &spec : specList) {
        language::Compability::common::visit(
            language::Compability::common::visitors{
                [&](const language::Compability::parser::Format &format) {
  ...
  void processEntryPoints() {
    ...
    else if (initialEval->isA<language::Compability::parser::EntryStmt>())
  ...
  toolchain::SmallVector<bool> containsStmtStack{};
  ...
  toolchain::DenseMap<parser::Label, lower::pft::Evaluation *> *labelEvaluationMap{};
...
void dumpScope(const semantics::Scope *scope, int depth) {
  ...
  if (depth < 0) {
    ...
    while (!scope->IsGlobal())
      scope = &scope->parent();
    LLVM_DEBUG(toolchain::dbgs() << "Full program scope information.\n"
  ...
  if (depth >= 0) {
    LLVM_DEBUG(toolchain::dbgs() << w << "<" << scope << "> ");
    if (auto *sym{scope->symbol()}) {
      LLVM_DEBUG(toolchain::dbgs() << sym << " " << *sym << "\n");
    } else {
      if (scope->IsIntrinsicModules()) {
        LLVM_DEBUG(toolchain::dbgs() << "IntrinsicModules (no detail)\n");
        return;
      }
      if (scope->kind() == language::Compability::semantics::Scope::Kind::BlockConstruct)
        LLVM_DEBUG(toolchain::dbgs() << "[block]\n");
      else
        LLVM_DEBUG(toolchain::dbgs() << "[anonymous]\n");
    }
  }
  ...
  for (auto iter = scope->begin(); iter != scope->end(); ++iter) {
    ...
    if (auto scp = sym->scope())
      ...
      LLVM_DEBUG(toolchain::dbgs() << w + "  " << &*sym << "   " << *sym << "\n");
  }
}
#endif // NDEBUG
...
class PFTDumper {
public:
  void dumpPFT(toolchain::raw_ostream &outputStream,
  ...
  toolchain::StringRef evaluationName(const lower::pft::Evaluation &eval) {
    return eval.visit([](const auto &parseTreeNode) {
      return parser::ParseTreeDumper::GetNodeName(parseTreeNode);
    });
  }

  void dumpEvaluation(toolchain::raw_ostream &outputStream,
                      const lower::pft::Evaluation &eval,
                      const std::string &indentString, int indent = 1) {
    toolchain::StringRef name = evaluationName(eval);
    toolchain::StringRef newBlock = eval.isNewBlock ? "^" : "";
    toolchain::StringRef bang = eval.isUnstructured ? "!" : "";
  ...
  void dumpEvaluation(toolchain::raw_ostream &ostream,
                      const lower::pft::Evaluation &eval) {
    dumpEvaluation(ostream, eval, "");
  }

  void dumpEvaluationList(toolchain::raw_ostream &outputStream,
  ...
  dumpFunctionLikeUnit(toolchain::raw_ostream &outputStream,
    ...
    if (functionLikeUnit.beginStmt) {
      functionLikeUnit.beginStmt->visit(common::visitors{
          ...
          [&](const auto &) { toolchain_unreachable("not a valid begin stmt"); },
      });
    } else {
      unitKind = "Program";
      name = "<anonymous>";
  ...
  void dumpModuleLikeUnit(toolchain::raw_ostream &outputStream,
    ...
    moduleLikeUnit.beginStmt.visit(common::visitors{
        ...
        [&](const auto &) {
          toolchain_unreachable("not a valid module begin stmt");
        },
    });
  ...
  void dumpCompilerDirectiveUnit(
      toolchain::raw_ostream &outputStream,
  ...
  void
  dumpOpenACCDirectiveUnit(toolchain::raw_ostream &outputStream,
...
private:
  toolchain::DenseMap<const void *, std::size_t> nodeIndexes;
...
static const semantics::Symbol *getSymbol(A &beginStmt) {
  const auto *symbol = beginStmt.visit(common::visitors{
      ...
      [](const auto &) -> const semantics::Symbol * {
        toolchain_unreachable("unknown FunctionLike or ModuleLike beginStmt");
        return nullptr;
      }});
...
bool language::Compability::lower::pft::Evaluation::lowerAsStructured() const {
  return !lowerAsUnstructured();
}

bool language::Compability::lower::pft::Evaluation::lowerAsUnstructured() const {
  return isUnstructured || clDisableStructuredFir;
}

bool language::Compability::lower::pft::Evaluation::forceAsUnstructured() const {
  return clDisableStructuredFir;
}

lower::pft::FunctionLikeUnit *
language::Compability::lower::pft::Evaluation::getOwningProcedure() const {
...
bool language::Compability::lower::definedInCommonBlock(const semantics::Symbol &sym) {
  return semantics::FindCommonBlockContaining(sym);
}

/// Is the symbol `sym` a global?
bool language::Compability::lower::symbolIsGlobal(const semantics::Symbol &sym) {
...
struct SymbolDependenceAnalysis {
  ...
  language::Compability::lower::pft::VariableList getVariableList() {
...
private:
  ...
  void analyzeLocalEquivalenceSets(const semantics::Scope &scope) {
    ...
    std::list<std::list<semantics::SymbolRef>> aggregates =
        language::Compability::semantics::GetStorageAssociations(scope);
    for (std::list<semantics::SymbolRef> aggregate : aggregates) {
      const language::Compability::semantics::Symbol *aggregateSym = nullptr;
      bool isGlobal = false;
      ...
      const language::Compability::semantics::Symbol *namingSym = nullptr;
      ...
      if (!aggregateSym) {
        stores.emplace_back(
            language::Compability::lower::pft::Variable::Interval{start, end - start},
            *namingSym, isGlobal);
      } else {
  ...
  int analyze(const semantics::Symbol &sym) {
    ...
    if (!done.second)
      return 0;
    LLVM_DEBUG(toolchain::dbgs() << "analyze symbol " << &sym << " in <"
                            << &sym.owner() << ">: " << sym << '\n');
    ...
    if (sym.has<semantics::DerivedTypeDetails>())
      toolchain_unreachable("not yet implemented - derived type analysis");
    ...
    if (ultimate.test(language::Compability::semantics::Symbol::Flag::CrayPointee))
      depth = std::max(
          analyze(language::Compability::semantics::GetCrayPointer(ultimate)) + 1, depth);
    adjustSize(depth + 1);
  ...
  language::Compability::lower::pft::Variable::AggregateStore *
  findStoreIfAlias(const language::Compability::evaluate::Symbol &sym) {
    const semantics::Symbol &ultimate = sym.GetUltimate();
    ...
    if (aliasSyms.contains(&ultimate)) {
      LLVM_DEBUG(toolchain::dbgs() << "found aggregate containing " << &ultimate
      ...
      // clang-format off
      LLVM_DEBUG(
          toolchain::dbgs() << "looking for " << off << "\n{\n";
          for (lower::pft::Variable::AggregateStore &v : stores) {
            toolchain::dbgs() << " in scope: " << &v.getOwningScope() << "\n";
            toolchain::dbgs() << "  i = [" << std::get<0>(v.interval) << ".."
                << std::get<0>(v.interval) + std::get<1>(v.interval)
                << "]\n";
          }
          toolchain::dbgs() << "}\n");
      // clang-format on
      toolchain_unreachable("the store must be present");
    }
    return nullptr;
  }
  ...
  toolchain::SmallSet<const semantics::Symbol *, 32> seen;
  std::vector<language::Compability::lower::pft::VariableList> layeredVarList;
  toolchain::SmallSet<const semantics::Symbol *, 32> aliasSyms;
  /// Set of scopes that have been analyzed for aliases.
  toolchain::SmallSet<const semantics::Scope *, 4> analyzedScopes;
  std::vector<language::Compability::lower::pft::Variable::AggregateStore> stores;
};
} // namespace
...
language::Compability::lower::pft::FunctionLikeUnit::FunctionLikeUnit(
...
language::Compability::lower::HostAssociations &
language::Compability::lower::pft::FunctionLikeUnit::parentHostAssoc() {
  if (auto *par = parent.getIf<FunctionLikeUnit>())
    return par->hostAssociations;
  toolchain::report_fatal_error("parent is not a function");
}

bool language::Compability::lower::pft::FunctionLikeUnit::parentHasTupleHostAssoc() {
  if (auto *par = parent.getIf<FunctionLikeUnit>())
    return par->hostAssociations.hasTupleAssociations();
  return false;
}

bool language::Compability::lower::pft::FunctionLikeUnit::parentHasHostAssoc() {
  if (auto *par = parent.getIf<FunctionLikeUnit>())
...
parser::CharBlock
language::Compability::lower::pft::FunctionLikeUnit::getStartingSourceLoc() const {
  if (beginStmt)
...
language::Compability::lower::pft::ModuleLikeUnit::ModuleLikeUnit(
...
parser::CharBlock
language::Compability::lower::pft::ModuleLikeUnit::getStartingSourceLoc() const {
  return stmtSourceLoc(beginStmt);
}
const language::Compability::semantics::Scope &
language::Compability::lower::pft::ModuleLikeUnit::getScope() const {
  const language::Compability::semantics::Symbol *symbol = getSymbol(beginStmt);
...
language::Compability::lower::pft::BlockDataUnit::BlockDataUnit(
...
bool language::Compability::lower::pft::Variable::isRuntimeTypeInfoData() const {
  ...
  using Flags = language::Compability::semantics::Symbol::Flag;
...
language::Compability::lower::createPFT(const parser::Program &root,
                          const semantics::SemanticsContext &semanticsContext) {
...
void language::Compability::lower::dumpPFT(toolchain::raw_ostream &outputStream,
                             const lower::pft::Program &pft) {
  PFTDumper{}.dumpPFT(outputStream, pft);
}

void language::Compability::lower::pft::Program::dump() const {
  dumpPFT(toolchain::errs(), *this);
}

void language::Compability::lower::pft::Evaluation::dump() const {
  PFTDumper{}.dumpEvaluation(toolchain::errs(), *this);
}

void language::Compability::lower::pft::Variable::dump() const {
  if (auto *s = std::get_if<Nominal>(&var)) {
    toolchain::errs() << s->symbol << " " << *s->symbol;
    toolchain::errs() << " (depth: " << s->depth << ')';
    if (s->global)
      toolchain::errs() << ", global";
    if (s->heapAlloc)
      toolchain::errs() << ", allocatable";
    if (s->pointer)
      toolchain::errs() << ", pointer";
    if (s->target)
      toolchain::errs() << ", target";
    if (s->aliaser)
      toolchain::errs() << ", equivalence(" << s->aliasOffset << ')';
  } else if (auto *s = std::get_if<AggregateStore>(&var)) {
    toolchain::errs() << "interval[" << std::get<0>(s->interval) << ", "
                 << std::get<1>(s->interval) << "]:";
    toolchain::errs() << " name: " << toStringRef(s->getNamingSymbol().name());
    if (s->isGlobal())
      toolchain::errs() << ", global";
    if (s->initialValueSymbol)
      toolchain::errs() << ", initial value: {" << *s->initialValueSymbol << "}";
  } else {
    toolchain_unreachable("not a Variable");
  }
  toolchain::errs() << '\n';
}

void language::Compability::lower::pft::dump(language::Compability::lower::pft::VariableList &variableList,
                               std::string s) {
  toolchain::errs() << (s.empty() ? "VariableList" : s) << " " << &variableList
...
void language::Compability::lower::pft::FunctionLikeUnit::dump() const {
  PFTDumper{}.dumpFunctionLikeUnit(toolchain::errs(), *this);
}

void language::Compability::lower::pft::ModuleLikeUnit::dump() const {
  PFTDumper{}.dumpModuleLikeUnit(toolchain::errs(), *this);
}

/// The BlockDataUnit dump is just the associated symbol table.
void language::Compability::lower::pft::BlockDataUnit::dump() const {
  toolchain::errs() << "block data {\n" << symTab << "\n}\n";
}
...
struct SymbolVisitor {
  template <typename A>
  bool Pre(const A &x) {
    if constexpr (language::Compability::parser::HasTypedExpr<A>::value)
      ...
      if (const auto *expr = language::Compability::semantics::GetExpr(nullptr, x))
        visitExpr(*expr);
    return true;
  }

  bool Pre(const language::Compability::parser::Name &name) {
  ...
  template <typename T>
  void visitExpr(const language::Compability::evaluate::Expr<T> &expr) {
    for (const semantics::Symbol &symbol :
         language::Compability::evaluate::CollectSymbols(expr))
      visitSymbol(symbol);
  }

  void visitSymbol(const language::Compability::semantics::Symbol &symbol) {
    callBack(symbol);
    ...
    if (const auto *subprogramDetails =
      ...
      if (const auto &maybeExpr = subprogramDetails->stmtFunction()) {
        ...
        if (subprogramDetails->isFunction()) {
          ...
          const language::Compability::semantics::Symbol &result =
              subprogramDetails->result();
          if (const auto *objectDetails =
                  result.detailsIf<language::Compability::semantics::ObjectEntityDetails>())
    ...
    if (language::Compability::semantics::IsProcedure(symbol)) {
      if (auto dynamicType = language::Compability::evaluate::DynamicType::From(symbol)) {
        ...
        if (dynamicType->category() ==
          ...
          if (std::optional<language::Compability::evaluate::ExtentExpr> length =
            ...
            visitExpr(*length);
        } else if (const language::Compability::semantics::DerivedTypeSpec *derivedTypeSpec =
                       language::Compability::evaluate::GetDerivedTypeSpec(dynamicType)) {
          for (const auto &[_, param] : derivedTypeSpec->parameters())
            if (const language::Compability::semantics::MaybeIntExpr &expr =
                    param.GetExplicit())
    ...
    if (symbol.GetUltimate().test(
      ...
      visitSymbol(language::Compability::semantics::GetCrayPointer(symbol));
  }

  template <typename A>
  constexpr void Post(const A &) {}

  const std::function<void(const language::Compability::semantics::Symbol &)> &callBack;
};
} // namespace

void language::Compability::lower::pft::visitAllSymbols(
...
void language::Compability::lower::pft::visitAllSymbols(
    const language::Compability::lower::pft::Evaluation &eval,
    const std::function<void(const language::Compability::semantics::Symbol &)> callBack) {
  SymbolVisitor visitor{callBack};
