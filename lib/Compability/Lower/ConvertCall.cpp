...
#include "language/Compability/Lower/HlfirIntrinsics.h"
...
static toolchain::cl::opt<bool> useHlfirIntrinsicOps(
    "use-hlfir-intrinsic-ops", toolchain::cl::init(true),
    toolchain::cl::desc("Lower via HLFIR transformational intrinsic operations such "
                   "as hlfir.sum"));
...
static fir::ExtendedValue toExtendedValue(mlir::Location loc, mlir::Value base,
                                          toolchain::ArrayRef<mlir::Value> extents,
...
[[maybe_unused]] static unsigned findHostAssocTuplePos(mlir::func::FuncOp fn) {
  ...
  toolchain_unreachable("anyFuncArgsHaveAttr failed");
}

mlir::Value
language::Compability::lower::argumentHostAssocs(language::Compability::lower::AbstractConverter &converter,
                                   mlir::Value arg) {
...
static bool mustCastFuncOpToCopeWithImplicitInterfaceMismatch(
  ...
  if (converter.getLoweringOptions().getLowerToHighLevelFIR())
    for (auto [actualType, dummyType] :
         toolchain::zip(callSiteType.getInputs(), funcOpType.getInputs()))
...
static mlir::Value readDim3Value(fir::FirOpBuilder &builder, mlir::Location loc,
                                 mlir::Value dim3Addr, toolchain::StringRef comp) {
...
static mlir::Value remapActualToDummyDescriptor(
    ...
    language::Compability::lower::CallerInterface &caller, bool isBindcCall) {
  ...
  if (mapSymbols) {
    symMap.pushScope();
    const language::Compability::semantics::Symbol *sym = caller.getDummySymbol(arg);
    assert(sym && "call must have explicit interface to map interface symbols");
    language::Compability::lower::mapCallInterfaceSymbolsForDummyArgument(converter, caller,
                                                            symMap, *sym);
  }
  ...
  if (mlir::isa<fir::SequenceType>(dummyBaseType))
    caller.walkDummyArgumentExtents(
        arg, [&](const language::Compability::lower::SomeExpr &e, bool isAssumedSizeExtent) {
  ...
  if (!extents.empty()) {
    if (isBindcCall) {
      ...
      toolchain::SmallVector<mlir::Value> lowerBounds(extents.size(), zero);
  ...
  if (auto recType = toolchain::dyn_cast<fir::RecordType>(dummyElementType))
...
static void remapActualToDummyDescriptors(
  ...
  for (auto [preparedActual, arg] :
       toolchain::zip(loweredActuals, caller.getPassedArguments())) {
...
std::pair<language::Compability::lower::LoweredResult, bool>
language::Compability::lower::genCallOpAndResult(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
  ...
  if (caller.mustMapInterfaceSymbolsForResult()) {
    symMap.pushScope();
    mustPopSymMap = true;
    language::Compability::lower::mapCallInterfaceSymbolsForResult(converter, caller, symMap);
  }
  ...
  if (const language::Compability::evaluate::ProcedureDesignator *procDesignator =
    ...
    if (mlir::Value passedArg = caller.getIfPassedArg()) {
      ...
      // component.
      funcPointer = language::Compability::lower::derefPassProcPointerComponent(
          loc, converter, *procDesignator, passedArg, symMap, stmtCtx);
    } else {
      language::Compability::lower::SomeExpr expr{*procDesignator};
  ...
  const bool isExprCall =
      ...
      toolchain::isa<fir::SequenceType>(callSiteType.getResult(0));
  ...
  toolchain::SmallVector<mlir::Value> resultLengths;
  ...
  auto allocatedResult = [&]() -> std::optional<fir::ExtendedValue> {
    ...
    if (mlir::isa<fir::SequenceType>(type))
      caller.walkResultExtents(
          [&](const language::Compability::lower::SomeExpr &e, bool isAssumedSizeExtent) {
  ...
  if (!funcPointer) {
    ...
    if (caller.getFuncOp()->hasAttrOfType<mlir::UnitAttr>(
      ...
      LLVM_DEBUG(mlir::emitWarning(
          loc,
          toolchain::Twine("function name '") +
              toolchain::Twine(symbolAttr.getLeafReference()) +
              toolchain::Twine("' conflicts with a runtime function name used by "
  ...
  for (auto [fst, snd] : toolchain::zip(caller.getInputs(), funcType.getInputs())) {
    ...
    if (mlir::isa<fir::BoxProcType>(snd) &&
      ...
      if (mlir::Value host = argumentHostAssocs(converter, fst)) {
        cast = fir::EmboxProcOp::create(builder, loc, boxProcTy,
                                        toolchain::ArrayRef<mlir::Value>{fst, host});
      } else {
      ...
      if (fir::isa_builtin_cptr_type(fromTy) &&
          language::Compability::lower::isCPtrArgByValueType(snd)) {
    ...
    if (caller.getCallDescription().chevrons().empty() &&
      ...
      for (auto [oper, arg] :
        ...
        if (auto boxTy = mlir::dyn_cast<fir::BaseBoxType>(oper.getType())) {
          const language::Compability::semantics::Symbol *sym = caller.getDummySymbol(arg);
          if (sym && language::Compability::evaluate::IsCUDADeviceSymbol(*sym))
  ...
  if (!caller.getCallDescription().chevrons().empty()) {
    ...
    if (const auto &binding{
            ultimateSymbol.get<language::Compability::semantics::ProcBindingDetails>()};
    ...
    if (std::optional<unsigned> passArg = caller.getPassArgIndex()) {
      ...
      fir::ExtendedValue dataRefValue = language::Compability::lower::convertDataRefToValue(
  ...
  std::optional<language::Compability::evaluate::DynamicType> retTy =
  ...
  const bool mustFinalizeResult =
      ...
      (retTy->category() == language::Compability::common::TypeCategory::Derived ||
  ...
  if (allocatedResult) {
    ...
    if (mustFinalizeResult) {
      if (retTy->IsPolymorphic() || retTy->IsUnlimitedPolymorphic()) {
        ...
        if (language::Compability::semantics::MayRequireFinalization(typeSpec) ||
...
static hlfir::EntityWithAttributes genStmtFunctionRef(
  ...
  // the local map here.
  symMap.pushScope();
  toolchain::SmallVector<hlfir::AssociateOp> exprAssociations;
  for (auto [arg, bind] : toolchain::zip(details.dummyArgs(), procRef.arguments())) {
    ...
    hlfir::EntityWithAttributes loweredArg = language::Compability::lower::convertExprToHLFIR(
    ...
    const language::Compability::semantics::DeclTypeSpec *type = arg->GetType();
    if (type &&
      ...
      language::Compability::lower::mapSymbolAttributes(converter, *arg, symMap, stmtCtx);
    } else {
  ...
  hlfir::Entity result = language::Compability::lower::convertExprToHLFIR(
...
struct CallContext {
  CallContext(const language::Compability::evaluate::ProcedureRef &procRef,
  ...
  bool isBindcCall() const {
    if (const language::Compability::semantics::Symbol *symbol = procRef.proc().GetSymbol())
      return language::Compability::semantics::IsBindCProcedure(*symbol);
    return false;
  }
...
extendedValueToHlfirEntity(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  if (auto charTy = mlir::dyn_cast<fir::CharacterType>(firBase.getType())) {
    ...
    // this could be fixed, but for now toolchain opt -O1 is able to get
...
struct CallCleanUp {
  ...
  std::optional<CallCleanUp> genCleanUp(mlir::Location loc,
    ...
    language::Compability::common::visit(language::Compability::common::visitors{
...
struct PreparedDummyArgument {
  ...
  toolchain::SmallVector<CallCleanUp, 2> cleanups;
};
...
struct ConditionallyPreparedDummy {
  ...
  toolchain::SmallVector<mlir::Type> getIfResulTypes() const {
  ...
  void genElseResult(mlir::Location loc, fir::FirOpBuilder &builder) const {
    toolchain::SmallVector<mlir::Value> elseResultValues;
  ...
  toolchain::SmallVector<mlir::Value> thenResultValues;
};
} // namespace
...
mlir::Value static getZeroLowerBounds(mlir::Location loc,
  ...
  toolchain::SmallVector<mlir::Value> lowerBounds(entity.getRank(), zero);
  return builder.genShift(loc, lowerBounds);
}

static bool
isSimplyContiguous(const language::Compability::evaluate::ActualArgument &arg,
                   language::Compability::evaluate::FoldingContext &foldingContext) {
  if (const auto *expr = arg.UnwrapExpr())
    return language::Compability::evaluate::IsSimplyContiguous(*expr, foldingContext);
  const language::Compability::semantics::Symbol *sym = arg.GetAssumedTypeDummy();
  assert(sym &&
         "expect ActualArguments to be expression or assumed-type symbols");
  return sym->Rank() == 0 ||
         language::Compability::evaluate::IsSimplyContiguous(*sym, foldingContext);
}
...
static bool isParameterObjectOrSubObject(hlfir::Entity entity) {
  ...
  while (mlir::Operation *op = base ? base.getDefiningOp() : nullptr) {
    base =
        toolchain::TypeSwitch<mlir::Operation *, mlir::Value>(op)
...
static PreparedDummyArgument preparePresentUserCallActualArgument(
    ...
    const language::Compability::lower::PreparedActualArgument &preparedActual,
    mlir::Type dummyType,
    const language::Compability::lower::CallerInterface::PassedEntity &arg,
  ...
  const bool ignoreTKRtype = arg.testTKR(language::Compability::common::IgnoreTKR::Type);
  ...
  if (auto baseBoxDummy = mlir::dyn_cast<fir::BaseBoxType>(dummyType)) {
    if (baseBoxDummy.isAssumedRank() ||
        arg.testTKR(language::Compability::common::IgnoreTKR::Rank) ||
  ...
  if (entity.isVariable()) {
    ...
    const language::Compability::lower::SomeExpr *expr = arg.entity->UnwrapExpr();
...
static PreparedDummyArgument prepareUserCallActualArgument(
  ...
  toolchain::SmallVector<mlir::Type> ifOpResultTypes;
...
static PreparedDummyArgument prepareProcedurePointerActualArgument(
  ...
  if (language::Compability::evaluate::UnwrapExpr<language::Compability::evaluate::NullPointer>(
    ...
    auto boxTy{language::Compability::lower::getUntypedBoxProcType(builder.getContext())};
...
void prepareUserCallArguments(
    language::Compability::lower::PreparedActualArguments &loweredActuals,
    language::Compability::lower::CallerInterface &caller, mlir::FunctionType callSiteType,
    CallContext &callContext, toolchain::SmallVector<CallCleanUp> &callCleanUps) {
  using PassBy = language::Compability::lower::CallerInterface::PassEntityBy;
  ...
  for (auto [preparedActual, arg] :
    ...
    case PassBy::MutableBox: {
      ...
      if (fir::isPointerType(argTy) &&
        ...
        // argument to it.
        auto dataTy = toolchain::cast<fir::BaseBoxType>(fir::unwrapRefType(argTy));
        fir::ExtendedValue actualExv = language::Compability::lower::convertToAddress(
      ...
      if (fir::isAllocatableType(argTy) && arg.isIntentOut() &&
        ...
        language::Compability::lower::genDeallocateIfAllocated(callContext.converter,
                                                 *mutableBox, loc);
      }
      caller.placeInput(arg, actual);
    } break;
...
genUserCall(language::Compability::lower::PreparedActualArguments &loweredActuals,
            language::Compability::lower::CallerInterface &caller,
            mlir::FunctionType callSiteType, CallContext &callContext) {
  mlir::Location loc = callContext.loc;
  toolchain::SmallVector<CallCleanUp> callCleanUps;
  fir::FirOpBuilder &builder = callContext.getBuilder();
  ...
  // arguments.
  auto [loweredResult, resultIsFinalized] = language::Compability::lower::genCallOpAndResult(
  ...
  toolchain::SmallVector<CallCleanUp> associateCleanups;
...
static std::optional<hlfir::EntityWithAttributes> genCustomIntrinsicRefCore(
  ...
  assert(intrinsic &&
         language::Compability::lower::intrinsicRequiresCustomOptionalHandling(
  ...
  auto getArgument = [&](std::size_t i, bool loadArg) -> fir::ExtendedValue {
    ...
    return language::Compability::lower::translateToExtendedValue(loc, builder, actual,
                                                    callContext.stmtCtx);
  };
  ...
  fir::ExtendedValue result = language::Compability::lower::lowerCustomIntrinsic(
...
static std::optional<hlfir::EntityWithAttributes>
genIntrinsicRefCore(language::Compability::lower::PreparedActualArguments &loweredActuals,
                    const language::Compability::evaluate::SpecificIntrinsic *intrinsic,
  ...
  toolchain::SmallVector<fir::ExtendedValue> operands;
  toolchain::SmallVector<hlfir::CleanupFunction> cleanupFns;
  ...
  for (auto arg : toolchain::enumerate(loweredActuals)) {
    if (!arg.value()) {
    ...
    if (!argLowering) {
      ...
      operands.emplace_back(
          language::Compability::lower::convertToValue(loc, converter, actual, stmtCtx));
      continue;
    }
    ...
    auto getActualFortranElementType = [&]() -> mlir::Type {
      if (const language::Compability::lower::SomeExpr *expr =
    ...
    case fir::LowerIntrinsicArgAs::Value:
      ...
      continue;
    case fir::LowerIntrinsicArgAs::Addr:
      operands.emplace_back(language::Compability::lower::convertToAddress(
          loc, converter, actual, stmtCtx, getActualFortranElementType()));
      continue;
    case fir::LowerIntrinsicArgAs::Box:
      operands.emplace_back(language::Compability::lower::convertToBox(
          loc, converter, actual, stmtCtx, getActualFortranElementType()));
      continue;
    case fir::LowerIntrinsicArgAs::Inquired:
      if (const language::Compability::lower::SomeExpr *expr =
        ...
        if (language::Compability::evaluate::UnwrapExpr<language::Compability::evaluate::NullPointer>(
          ...
          hlfir::EntityWithAttributes nullBoxEntity =
              extendedValueToHlfirEntity(loc, builder, boxStorage,
                                         ".tmp.null_box");
          operands.emplace_back(language::Compability::lower::translateToExtendedValue(
...
static std::optional<hlfir::EntityWithAttributes> genHLFIRIntrinsicRefCore(
  ...
  if (useHlfirIntrinsicOps && callContext.resultType) {
    ...
    std::optional<hlfir::EntityWithAttributes> res =
        language::Compability::lower::lowerHlfirIntrinsic(builder, loc, intrinsicName,
...
public:
  ...
  genElementalCall(language::Compability::lower::PreparedActualArguments &loweredActuals,
    ...
    mlir::Value shape;
    language::Compability::lower::PreparedActualArgument *optionalWithShape;
...
public:
  ElementalUserCallBuilder(language::Compability::lower::CallerInterface &caller,
                           mlir::FunctionType callSiteType)
      : caller{caller}, callSiteType{callSiteType} {}
  std::optional<hlfir::Entity>
  genElementalKernel(language::Compability::lower::PreparedActualArguments &loweredActuals,
                     CallContext &callContext) {
  ...
  bool resultMayRequireFinalization(CallContext &callContext) const {
    ...
    if (retTy->category() == language::Compability::common::TypeCategory::Derived) {
      const language::Compability::semantics::DerivedTypeSpec &typeSpec =
          retTy->GetDerivedTypeSpec();
      return language::Compability::semantics::IsFinalizable(typeSpec);
    }
...
genCustomElementalIntrinsicRef(
  ...
  mlir::Location loc = callContext.loc;
  auto &converter = callContext.converter;
  language::Compability::lower::PreparedActualArguments operands;
  assert(intrinsic && language::Compability::lower::intrinsicRequiresCustomOptionalHandling(
                          callContext.procRef, *intrinsic, converter));

  // callback for optional arguments
  auto prepareOptionalArg = [&](const language::Compability::lower::SomeExpr &expr) {
    hlfir::EntityWithAttributes actual = language::Compability::lower::convertExprToHLFIR(
    ...
    operands.emplace_back(
        language::Compability::lower::PreparedActualArgument{actual, isPresent});
  };
  ...
  auto prepareOtherArg = [&](const language::Compability::lower::SomeExpr &expr,
    ...
    hlfir::EntityWithAttributes actual = language::Compability::lower::convertExprToHLFIR(
        loc, converter, expr, callContext.symMap, callContext.stmtCtx);
    operands.emplace_back(language::Compability::lower::PreparedActualArgument{
...
static std::optional<hlfir::EntityWithAttributes>
genCustomIntrinsicRef(const language::Compability::evaluate::SpecificIntrinsic *intrinsic,
                      CallContext &callContext) {
  assert(!callContext.isElementalProcWithArrayArgs() &&
  ...
  auto prepareOtherArg = [&](const language::Compability::lower::SomeExpr &expr,
    ...
    switch (lowerAs) {
    case fir::LowerIntrinsicArgAs::Value:
      exv = language::Compability::lower::convertToValue(loc, converter, actual, stmtCtx);
      break;
    case fir::LowerIntrinsicArgAs::Addr:
      exv = language::Compability::lower::convertToAddress(loc, converter, actual, stmtCtx,
                                             getActualFortranElementType());
      break;
    case fir::LowerIntrinsicArgAs::Box:
      exv = language::Compability::lower::convertToBox(loc, converter, actual, stmtCtx,
                                         getActualFortranElementType());
      break;
    case fir::LowerIntrinsicArgAs::Inquired:
      exv = language::Compability::lower::translateToExtendedValue(loc, builder, actual,
                                                     stmtCtx);
      break;
    }
    if (!exv)
    ...
    loweredActuals.emplace_back(language::Compability::lower::PreparedActualArgument{
        actual, /*isPresent=*/std::nullopt});
  };
...
static std::optional<hlfir::EntityWithAttributes>
genIntrinsicRef(const language::Compability::evaluate::SpecificIntrinsic *intrinsic,
                const fir::IntrinsicHandlerEntry &intrinsicEntry,
                CallContext &callContext) {
  mlir::Location loc = callContext.loc;
  language::Compability::lower::PreparedActualArguments loweredActuals;
  const fir::IntrinsicArgumentLoweringRules *argLowering =
      intrinsicEntry.getArgumentLoweringRules();
  for (const auto &arg : toolchain::enumerate(callContext.procRef.arguments())) {
    ...
    if (!expr) {
      ...
      loweredActuals.push_back(language::Compability::lower::PreparedActualArgument{
    ...
    auto loweredActual = language::Compability::lower::convertExprToHLFIR(
    ...
    loweredActuals.push_back(
        language::Compability::lower::PreparedActualArgument{loweredActual, isPresent});
  }
...
genProcedureRef(CallContext &callContext) {
  ...
  if (language::Compability::lower::isIntrinsicModuleProcRef(callContext.procRef) &&
      !callContext.isBindcCall())
  ...
  language::Compability::lower::CallerInterface caller(callContext.procRef,
  ...
  for (const language::Compability::lower::CallInterface<
    ...
    if (const auto *actual = arg.entity) {
      ...
      if (language::Compability::evaluate::UnwrapExpr<language::Compability::evaluate::NullPointer>(
              *expr)) {
        if ((arg.passBy !=
             language::Compability::lower::CallerInterface::PassEntityBy::MutableBox) &&
            (arg.passBy !=
             language::Compability::lower::CallerInterface::PassEntityBy::BoxProcRef)) {
          assert(
              arg.isOptional() &&
      ...
      if (isElemental && !arg.hasValueAttribute() &&
          language::Compability::evaluate::IsVariable(*expr) &&
        ...
        hlfir::ElementalAddrOp elementalAddr =
            language::Compability::lower::convertVectorSubscriptedExprToElementalAddr(
                ...
                callContext.stmtCtx);
        loweredActuals.emplace_back(
            language::Compability::lower::PreparedActualArgument{elementalAddr});
        continue;
      }
  ...
  if (isElemental) {
    bool isImpure = false;
    if (const language::Compability::semantics::Symbol *procSym =
            callContext.procRef.proc().GetSymbol())
      isImpure = !language::Compability::semantics::IsPureProcedure(*procSym);
...
hlfir::Entity language::Compability::lower::PreparedActualArgument::getActual(
...
bool language::Compability::lower::isIntrinsicModuleProcRef(
    const language::Compability::evaluate::ProcedureRef &procRef) {
  const language::Compability::semantics::Symbol *symbol = procRef.proc().GetSymbol();
  if (!symbol)
    return false;
  const language::Compability::semantics::Symbol *module =
      symbol->GetUltimate().owner().GetSymbol();
  return module && module->attrs().test(language::Compability::semantics::Attr::INTRINSIC);
}
...
std::optional<hlfir::EntityWithAttributes> language::Compability::lower::convertCallToHLFIR(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
  ...
  if (resultType && !procRef.IsElemental() &&
    ...
    language::Compability::lower::genCleanUpInRegionIfAny(loc, builder, yield.getCleanup(),
                                            localStmtCtx);
...
void language::Compability::lower::convertUserDefinedAssignmentToHLFIR(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
    const evaluate::ProcedureRef &procRef, hlfir::Entity lhs, hlfir::Entity rhs,
    language::Compability::lower::SymMap &symMap) {
  language::Compability::lower::StatementContext definedAssignmentContext;
