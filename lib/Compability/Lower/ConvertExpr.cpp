...
#include "language/Compability/Common/unwrap.h"
#include "language/Compability/Evaluate/fold.h"
#include "language/Compability/Evaluate/real.h"
#include "language/Compability/Evaluate/traverse.h"
...
#include "language/Compability/Optimizer/Builder/Factory.h"
...
#include "language/Compability/Semantics/dump-expr.h"
...
//===----------------------------------------------------------------------===//
// The composition and structure of language::Compability::evaluate::Expr is defined in
...
static toolchain::cl::opt<bool> generateArrayCoordinate(
    "gen-array-coor",
    toolchain::cl::desc("in lowering create ArrayCoorOp instead of CoordinateOp"),
...
static toolchain::cl::opt<bool> optimizeTranspose(
    "opt-transpose",
    toolchain::cl::desc("lower transpose without using a runtime call"),
...
// to the optimizer.
static toolchain::cl::opt<bool> inlineCopyInOutForBoxes(
...
arrayLoadExtValue(fir::FirOpBuilder &builder, mlir::Location loc,
                  fir::ArrayLoadOp load, toolchain::ArrayRef<mlir::Value> path,
  ...
  auto arrayToExtendedValue =
      [&](const toolchain::SmallVector<mlir::Value> &extents,
          const toolchain::SmallVector<mlir::Value> &origins) -> fir::ExtendedValue {
...
placeScalarValueInMemory(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  mlir::Value temp = builder.createTemporary(
      loc, storageType,
      toolchain::ArrayRef<mlir::NamedAttribute>{fir::getAdaptToByRefAttr(builder)});
...
template <typename T>
static bool isParenthesizedVariable(const language::Compability::evaluate::Expr<T> &expr) {
  using ExprVariant = decltype(language::Compability::evaluate::Expr<T>::u);
  using Parentheses = language::Compability::evaluate::Parentheses<T>;
  if constexpr (language::Compability::common::HasMember<Parentheses, ExprVariant>) {
    if (const auto *parentheses = std::get_if<Parentheses>(&expr.u))
      return language::Compability::evaluate::IsVariable(parentheses->left());
    return false;
  } else {
...
static bool
isElementalProcWithArrayArgs(const language::Compability::evaluate::ProcedureRef &procRef) {
...
template <typename T>
static bool isElementalProcWithArrayArgs(const language::Compability::evaluate::Expr<T> &) {
  return false;
}
template <>
bool isElementalProcWithArrayArgs(const language::Compability::lower::SomeExpr &x) {
  if (const auto *procRef = std::get_if<language::Compability::evaluate::ProcedureRef>(&x.u))
...
static mlir::Value
createBoxProcCharTuple(language::Compability::lower::AbstractConverter &converter,
...
// associations.
template <typename A>
const language::Compability::semantics::Symbol &getFirstSym(const A &obj) {
  const language::Compability::semantics::Symbol &sym = obj.GetFirstSymbol();
  return sym.HasLocalLocality() ? sym : sym.GetUltimate();
}

// Helper to get the ultimate last symbol.
template <typename A>
const language::Compability::semantics::Symbol &getLastSym(const A &obj) {
  const language::Compability::semantics::Symbol &sym = obj.GetLastSymbol();
  return sym.HasLocalLocality() ? sym : sym.GetUltimate();
}

// Return true if TRANSPOSE should be lowered without a runtime call.
static bool
isTransposeOptEnabled(const language::Compability::lower::AbstractConverter &converter) {
...
static bool isOptimizableTranspose(const T &,
                                   const language::Compability::lower::AbstractConverter &) {
  return false;
}

static bool
isOptimizableTranspose(const language::Compability::evaluate::ProcedureRef &procRef,
                       const language::Compability::lower::AbstractConverter &converter) {
  const language::Compability::evaluate::SpecificIntrinsic *intrin =
      procRef.proc().GetSpecificIntrinsic();
  if (isTransposeOptEnabled(converter) && intrin &&
      intrin->name == "transpose") {
    const std::optional<language::Compability::evaluate::ActualArgument> matrix =
...
template <typename T>
static bool
isOptimizableTranspose(const language::Compability::evaluate::FunctionRef<T> &funcRef,
                       const language::Compability::lower::AbstractConverter &converter) {
  return isOptimizableTranspose(
      static_cast<const language::Compability::evaluate::ProcedureRef &>(funcRef), converter);
}

template <typename T>
static bool
isOptimizableTranspose(language::Compability::evaluate::Expr<T> expr,
                       const language::Compability::lower::AbstractConverter &converter) {
...
/// Lowering of language::Compability::evaluate::Expr<T> expressions
...
public:
  ...
  ExtValue genExtAddr(const language::Compability::lower::SomeExpr &expr) {
    return gen(expr);
  }
  ...
  ExtValue genBoxArg(const language::Compability::lower::SomeExpr &expr) {
  ...
  ExtValue genExtValue(const language::Compability::lower::SomeExpr &expr) {
    return genval(expr);
  }
  ...
  /// MutableBoxValue.
  fir::MutableBoxValue
  genMutableBoxValue(const language::Compability::lower::SomeExpr &expr) {
    ...
    ExtValue exv = language::Compability::common::visit(
  ...
  ExtValue genMutableBoxValueImpl(const language::Compability::evaluate::NullPointer &) {
  ...
  template <typename T>
  ExtValue
  genMutableBoxValueImpl(const language::Compability::evaluate::FunctionRef<T> &funRef) {
    return genRawProcedureRef(funRef, converter.genType(toEvExpr(funRef)));
  }

  template <typename T>
  ExtValue
  genMutableBoxValueImpl(const language::Compability::evaluate::Designator<T> &designator) {
    return language::Compability::common::visit(
        language::Compability::common::visitors{
            [&](const language::Compability::evaluate::SymbolRef &sym) -> ExtValue {
              return converter.getSymbolExtendedValue(*sym, &symMap);
            },
            [&](const language::Compability::evaluate::Component &comp) -> ExtValue {
  ...
  template <typename T>
  ExtValue genMutableBoxValueImpl(const language::Compability::evaluate::Expr<T> &expr) {
    return language::Compability::common::visit(
        [&](const auto &x) { return genMutableBoxValueImpl(x); }, expr.u);
  }
  ...
  mlir::Value genIntegerConstant(mlir::MLIRContext *context,
                                 std::int64_t value) {
    mlir::Type type =
        converter.genType(language::Compability::common::TypeCategory::Integer, KIND);
  ...
  mlir::func::FuncOp getFunction(toolchain::StringRef name,
      ...
      if (const fir::UnboxedValue *rhs = right.getUnboxed()) {
        ...
        if (unsignedKind) {
          mlir::Type signlessType = converter.genType(
              language::Compability::common::TypeCategory::Integer, *unsignedKind);
  ...
  /// one.
  ExtValue gen(language::Compability::semantics::SymbolRef sym) {
  ...
  ExtValue genval(language::Compability::semantics::SymbolRef sym) {
    mlir::Location loc = getLoc();
    ...
    if (const fir::UnboxedValue *s = var.getUnboxed()) {
      if (fir::isa_ref_type(s->getType())) {
        ...
        if (language::Compability::semantics::IsFunctionResult(sym)) {
          ...
          if (addr.getType() != resultType)
            addr = builder.createConvert(loc, builder.getRefType(resultType),
                                         addr);
        } else if (sym->test(language::Compability::semantics::Symbol::Flag::CrayPointee)) {
          // get the corresponding Cray pointer
          language::Compability::semantics::SymbolRef ptrSym{
  ...
  ExtValue genval(const language::Compability::evaluate::BOZLiteralConstant &) {
    TODO(getLoc(), "BOZ");
  }
  ...
  ExtValue genval(const language::Compability::evaluate::ProcedureDesignator &proc) {
    return language::Compability::lower::convertProcedureDesignator(getLoc(), converter, proc,
                                                      symMap, stmtCtx);
  }
  ExtValue genval(const language::Compability::evaluate::NullPointer &) {
  ...
  ExtValue genval(const language::Compability::evaluate::StructureConstructor &ctor) {
    mlir::Location loc = getLoc();
    if (inInitializer)
      return language::Compability::lower::genInlinedStructureCtorLit(converter, loc, ctor);
    ...
    for (const auto &value : ctor.values()) {
      const language::Compability::semantics::Symbol &sym = *value.first;
      const language::Compability::lower::SomeExpr &expr = value.second.value();
      if (sym.test(language::Compability::semantics::Symbol::Flag::ParentComp)) {
      ...
      to.match(
          ...
          [&](const fir::ArrayBoxValue &) {
            language::Compability::lower::createSomeArrayAssignment(converter, to, expr,
                                                      symMap, stmtCtx);
          },
          ...
          [&](const fir::MutableBoxValue &toBox) {
            if (toBox.isPointer()) {
              language::Compability::lower::associateMutableBox(
  ...
  ExtValue genval(const language::Compability::evaluate::ImpliedDoIndex &var) {
  ...
  ExtValue genval(const language::Compability::evaluate::DescriptorInquiry &desc) {
  ...
  ExtValue genval(const language::Compability::evaluate::TypeParamInquiry &) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::ComplexComponent<KIND> &part) {
    return extractComplexPart(genunbox(part.left()), part.isImaginaryPart);
  }

  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Integer, KIND>> &op) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Unsigned, KIND>> &op) {
    auto loc = getLoc();
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Real, KIND>> &op) {
    return mlir::arith::NegFOp::create(builder, getLoc(), genunbox(op.left()));
  }
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Complex, KIND>> &op) {
...
#define GENBIN(GenBinEvOp, GenBinTyCat, GenBinFirOp)                           \
  template <int KIND>                                                          \
  ExtValue genval(const language::Compability::evaluate::GenBinEvOp<language::Compability::evaluate::Type< \
                      language::Compability::common::TypeCategory::GenBinTyCat, KIND>> &x) { \
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Divide<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Complex, KIND>> &op) {
    mlir::Type ty =
        converter.genType(language::Compability::common::TypeCategory::Complex, KIND);
  ...
  ExtValue genval(
      const language::Compability::evaluate::Power<language::Compability::evaluate::Type<TC, KIND>> &op) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::ComplexConstructor<KIND> &op) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Concat<KIND> &op) {
  ...
  template <language::Compability::common::TypeCategory TC, int KIND>
  ExtValue
  genval(const language::Compability::evaluate::Extremum<language::Compability::evaluate::Type<TC, KIND>>
             &op) {
    mlir::Value lhs = genunbox(op.left());
    mlir::Value rhs = genunbox(op.right());
    switch (op.ordering) {
    case language::Compability::evaluate::Ordering::Greater:
      return fir::genMax(builder, getLoc(),
                         toolchain::ArrayRef<mlir::Value>{lhs, rhs});
    case language::Compability::evaluate::Ordering::Less:
      return fir::genMin(builder, getLoc(),
                         toolchain::ArrayRef<mlir::Value>{lhs, rhs});
    case language::Compability::evaluate::Ordering::Equal:
      toolchain_unreachable("Equal is not a valid ordering in this context");
    }
    toolchain_unreachable("unknown ordering");
  }
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::SetLength<KIND> &x) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Relational<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Integer, KIND>> &op) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Relational<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Character, KIND>> &op) {
    return createCharCompare(op, translateSignedRelational(op.opr));
  }

  ExtValue
  genval(const language::Compability::evaluate::Relational<language::Compability::evaluate::SomeType> &op) {
    return language::Compability::common::visit([&](const auto &x) { return genval(x); },
                                  op.u);
  }
  ...
  ExtValue
  genval(const language::Compability::evaluate::Convert<language::Compability::evaluate::Type<TC1, KIND>,
                                          TC2> &convert) {
  ...
  template <typename A>
  ExtValue genval(const language::Compability::evaluate::Parentheses<A> &op) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Not<KIND> &op) {
  ...
  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::LogicalOperation<KIND> &op) {
  ...
  template <language::Compability::common::TypeCategory TC, int KIND>
  ExtValue
  genval(const language::Compability::evaluate::Constant<language::Compability::evaluate::Type<TC, KIND>>
             &con) {
    return language::Compability::lower::convertConstant(
        converter, getLoc(), con,
        /*outlineBigConstantsInReadOnlyMemory=*/!inInitializer);
  }

  fir::ExtendedValue genval(
      const language::Compability::evaluate::Constant<language::Compability::evaluate::SomeDerived> &con) {
  ...
  template <typename A>
  ExtValue genval(const language::Compability::evaluate::ArrayConstructor<A> &) {
    fir::emitFatalError(getLoc(), "array constructor: should not reach here");
  }

  ExtValue gen(const language::Compability::evaluate::ComplexPart &x) {
    mlir::Location loc = getLoc();
    ...
    mlir::Value offset = builder.createIntegerConstant(
        loc, idxTy,
        x.part() == language::Compability::evaluate::ComplexPart::Part::RE ? 0 : 1);
    mlir::Value result =
        fir::CoordinateOp::create(builder, loc, builder.getRefType(eleTy), base,
                                  mlir::ValueRange{offset});
    return {result};
  }
  ExtValue genval(const language::Compability::evaluate::ComplexPart &x) {
    return genLoad(gen(x));
  }

  /// Reference to a substring.
  ExtValue gen(const language::Compability::evaluate::Substring &s) {
    // Get base string
    auto baseString = language::Compability::common::visit(
        language::Compability::common::visitors{
            [&](const language::Compability::evaluate::DataRef &x) { return gen(x); },
            [&](const language::Compability::evaluate::StaticDataObject::Pointer &p)
                -> ExtValue {
    ...
    if (language::Compability::evaluate::MaybeExtentExpr upperBound = s.upper()) {
  ...
  ExtValue genval(const language::Compability::evaluate::Substring &ss) {
    // FIXME: why is the value of a substring being lowered the same as the
    // address of a substring?
    return gen(ss);
  }

  ExtValue genval(const language::Compability::evaluate::Subscript &subs) {
    if (auto *s = std::get_if<language::Compability::evaluate::IndirectSubscriptIntegerExpr>(
            &subs.u)) {
      if (s->value().Rank() > 0)
  ...
  ExtValue genSubscript(const language::Compability::evaluate::Subscript &subs) {
    return genval(subs);
  }

  ExtValue gen(const language::Compability::evaluate::DataRef &dref) {
    return language::Compability::common::visit([&](const auto &x) { return gen(x); },
                                  dref.u);
  }
  ExtValue genval(const language::Compability::evaluate::DataRef &dref) {
    return language::Compability::common::visit([&](const auto &x) { return genval(x); },
                                  dref.u);
  }
  ...
  static language::Compability::evaluate::DataRef const *
  reverseComponents(const language::Compability::evaluate::Component &cmpt,
                    std::list<const language::Compability::evaluate::Component *> &list) {
    if (!getLastSym(cmpt).test(language::Compability::semantics::Symbol::Flag::ParentComp))
      list.push_front(&cmpt);
    return language::Compability::common::visit(
        language::Compability::common::visitors{
            [&](const language::Compability::evaluate::Component &x) {
              if (language::Compability::semantics::IsAllocatableOrPointer(getLastSym(x)))
                return &cmpt.base();
  ...
  ExtValue genComponent(const language::Compability::evaluate::Component &cmpt) {
    std::list<const language::Compability::evaluate::Component *> list;
    const language::Compability::evaluate::DataRef *base = reverseComponents(cmpt, list);
    toolchain::SmallVector<mlir::Value> coorArgs;
    ...
    for (const language::Compability::evaluate::Component *field : list) {
      auto recTy = mlir::cast<fir::RecordType>(ty);
      const language::Compability::semantics::Symbol &sym = getLastSym(*field);
      std::string name = converter.getRecordTypeFieldName(sym);
  ...
  ExtValue gen(const language::Compability::evaluate::Component &cmpt) {
  ...
  ExtValue genval(const language::Compability::evaluate::Component &cmpt) {
  ...
  mlir::Type genSubType(mlir::Type arrTy, unsigned dims) {
    ...
    toolchain::ArrayRef<int64_t> shape = seqTy.getShape();
  ...
  ExtValue genval(const language::Compability::semantics::Bound &bound) {
    if (bound.isExplicit()) {
      language::Compability::semantics::MaybeSubscriptIntExpr sub = bound.GetExplicit();
      if (sub.has_value())
        return genval(*sub);
  ...
  static bool isSlice(const language::Compability::evaluate::ArrayRef &aref) {
    for (const language::Compability::evaluate::Subscript &sub : aref.subscript())
      if (std::holds_alternative<language::Compability::evaluate::Triplet>(sub.u))
        return true;
    return false;
  }
  ...
  ExtValue genCoordinateOp(const ExtValue &array,
                           const language::Compability::evaluate::ArrayRef &aref) {
    mlir::Location loc = getLoc();
    ...
    for (const auto &subsc : toolchain::enumerate(aref.subscript())) {
    ...
    if (baseSym.test(language::Compability::semantics::Symbol::Flag::CrayPointee)) {
      // get the corresponding Cray pointer
      language::Compability::semantics::SymbolRef ptrSym{
          language::Compability::semantics::GetCrayPointer(baseSym)};
  ...
  ExtValue genOffsetAndCoordinateOp(const ExtValue &array,
    ...
    auto genFullDim = [&](const auto &arr, mlir::Value delta) -> mlir::Value {
      ...
      unsigned dim = 0;
      for (auto [ext, sub] : toolchain::zip(arr.getExtents(), aref.subscript())) {
      ...
      auto coor = fir::CoordinateOp::create(builder, loc, refTy, base,
                                            toolchain::ArrayRef<mlir::Value>{total});
  ...
  ExtValue genArrayCoorOp(const ExtValue &exv,
    ...
    mlir::Type refTy = builder.getRefType(eleTy);
    mlir::IndexType idxTy = builder.getIndexType();
    toolchain::SmallVector<mlir::Value> arrayCoorArgs;
  ...
  ExtValue gen(const language::Compability::evaluate::ArrayRef &aref) {
  ...
  ExtValue genval(const language::Compability::evaluate::ArrayRef &aref) {
    return genLoad(gen(aref));
  }

  ExtValue gen(const language::Compability::evaluate::CoarrayRef &coref) {
    return language::Compability::lower::CoarrayExprHelper{converter, getLoc(), symMap}
        .genAddr(coref);
  }

  ExtValue genval(const language::Compability::evaluate::CoarrayRef &coref) {
    return language::Compability::lower::CoarrayExprHelper{converter, getLoc(), symMap}
        .genValue(coref);
  }

  template <typename A>
  ExtValue gen(const language::Compability::evaluate::Designator<A> &des) {
    return language::Compability::common::visit([&](const auto &x) { return gen(x); }, des.u);
  }
  template <typename A>
  ExtValue genval(const language::Compability::evaluate::Designator<A> &des) {
    return language::Compability::common::visit([&](const auto &x) { return genval(x); },
                                  des.u);
  }

  mlir::Type genType(const language::Compability::evaluate::DynamicType &dt) {
    if (dt.category() != language::Compability::common::TypeCategory::Derived)
  ...
  template <typename A>
  ExtValue genFunctionRef(const language::Compability::evaluate::FunctionRef<A> &funcRef) {
  ...
  template <typename A>
  ExtValue gen(const language::Compability::evaluate::FunctionRef<A> &funcRef) {
  ...
  ExtValue
  lowerIntrinsicArgumentAsInquired(const language::Compability::lower::SomeExpr &expr) {
    if (language::Compability::evaluate::IsAllocatableOrPointerObject(expr))
    ...
    if (language::Compability::evaluate::IsVariable(expr) && isArray(expr) &&
  ...
  ExtValue lowerIntrinsicArgumentAsBox(const language::Compability::lower::SomeExpr &expr) {
    mlir::Location loc = getLoc();
    ...
    if (language::Compability::lower::isParentComponent(expr)) {
      fir::ExtendedValue newExv =
          language::Compability::lower::updateBoxForParentComponent(converter, box, expr);
  ...
  ExtValue genIntrinsicRef(
      ...
      std::optional<const language::Compability::evaluate::SpecificIntrinsic> intrinsic =
          std::nullopt) {
    toolchain::SmallVector<ExtValue> operands;

    std::string name =
        intrinsic ? intrinsic->name
                  : procRef.proc().GetSymbol()->GetUltimate().name().ToString();
    mlir::Location loc = getLoc();
    if (intrinsic && language::Compability::lower::intrinsicRequiresCustomOptionalHandling(
                         procRef, *intrinsic, converter)) {
      using ExvAndPresence = std::pair<ExtValue, std::optional<mlir::Value>>;
      toolchain::SmallVector<ExvAndPresence, 4> operands;
      auto prepareOptionalArg = [&](const language::Compability::lower::SomeExpr &expr) {
      ...
      auto prepareOtherArg = [&](const language::Compability::lower::SomeExpr &expr,
      ...
      language::Compability::lower::prepareCustomIntrinsicArgument(
      ...
      return language::Compability::lower::lowerCustomIntrinsic(
    ...
    for (const auto &arg : toolchain::enumerate(procRef.arguments())) {
      auto *expr =
      ...
      if (!expr && arg.value() && arg.value()->GetAssumedTypeDummy()) {
        ...
        const language::Compability::evaluate::Symbol *assumedTypeSym =
      ...
      if (argRules.handleDynamicOptional &&
        ...
        toolchain_unreachable("bad switch");
      }
      switch (argRules.lowerAs) {
      case fir::LowerIntrinsicArgAs::Value:
        operands.emplace_back(genval(*expr));
    ...
    return language::Compability::lower::genIntrinsicCall(builder, getLoc(), name, resultType,
                                            operands, stmtCtx, &converter);
  }

  /// helper to detect statement functions
  static bool
  isStatementFunctionCall(const language::Compability::evaluate::ProcedureRef &procRef) {
    if (const language::Compability::semantics::Symbol *symbol = procRef.proc().GetSymbol())
      if (const auto *details =
              symbol->detailsIf<language::Compability::semantics::SubprogramDetails>())
        return details->stmtFunction().has_value();
    return false;
  }

  /// Generate Statement function calls
  ExtValue genStmtFunctionRef(const language::Compability::evaluate::ProcedureRef &procRef) {
    const language::Compability::semantics::Symbol *symbol = procRef.proc().GetSymbol();
    assert(symbol && "expected symbol in ProcedureRef of statement functions");
    const auto &details = symbol->get<language::Compability::semantics::SubprogramDetails>();
    ...
    symMap.pushScope();
    for (auto [arg, bind] :
         toolchain::zip(details.dummyArgs(), procRef.arguments())) {
      ...
      if (const language::Compability::semantics::DeclTypeSpec *type = arg->GetType())
        if (type->category() == language::Compability::semantics::DeclTypeSpec::Character)
          if (const language::Compability::semantics::MaybeIntExpr &lenExpr =
    ...
    for (const language::Compability::semantics::SymbolRef &sym :
         language::Compability::evaluate::CollectSymbols(*details.stmtFunction()))
      if (const auto *details =
    ...
    LLVM_DEBUG(toolchain::dbgs() << "stmt-function: " << result << '\n');
    symMap.popScope();
    return result;
  }
  ...
  ExtValue genArrayTempFromMold(const ExtValue &mold,
    ...
    toolchain::SmallVector<mlir::Value> allocMemTypeParams =
  ...
  ExtValue genProcedureRef(const language::Compability::evaluate::ProcedureRef &procRef,
                           std::optional<mlir::Type> resultType) {
  ...
  ExtValue genTempExtAddr(const language::Compability::lower::SomeExpr &expr) {
  ...
  using CopyOutPairs = toolchain::SmallVector<CopyOutPair, 4>;
  ...
  ExtValue genCopyIn(const ExtValue &actualArg,
                     const language::Compability::lower::CallerInterface::PassedEntity &arg,
    ...
    toolchain::StringRef tempName = byValue ? ".copy" : ".copyinout";
    mlir::Location loc = getLoc();
  ...
  prepareActualThatMayBeAbsent(const language::Compability::lower::SomeExpr &expr) {
    mlir::Location loc = getLoc();
    if (language::Compability::evaluate::IsAllocatableOrPointerObject(expr)) {
  ...
  prepareActualToBaseAddressLike(
    ...
    const bool actualArgIsVariable = language::Compability::evaluate::IsVariable(expr);
    ...
    const bool isStaticConstantByValue =
        byValue && language::Compability::evaluate::IsActuallyConstant(expr) &&
        (isCharacterType(expr));
    const bool variableNeedsCopy =
        actualArgIsVariable &&
        (byValue || (isArray && !language::Compability::evaluate::IsSimplyContiguous(
  ...
  ExtValue genRawProcedureRef(const language::Compability::evaluate::ProcedureRef &procRef,
                              std::optional<mlir::Type> resultType) {
    mlir::Location loc = getLoc();
    ...
    if (language::Compability::lower::isIntrinsicModuleProcRef(procRef) &&
        !language::Compability::semantics::IsBindCProcedure(*procRef.proc().GetSymbol()))
      return genIntrinsicRef(procRef, resultType);

    if (isStatementFunctionCall(procRef))
      return genStmtFunctionRef(procRef);

    language::Compability::lower::CallerInterface caller(procRef, converter);
    using PassBy = language::Compability::lower::CallerInterface::PassEntityBy;

    toolchain::SmallVector<fir::MutableBoxValue> mutableModifiedByCall;
    ...
    // arguments.
    for (const language::Compability::lower::CallInterface<
             language::Compability::lower::CallerInterface>::PassedEntity &arg :
      ...
      if (arg.passBy == PassBy::MutableBox) {
        ...
        if (fir::isPointerType(argTy) &&
            !language::Compability::evaluate::IsObjectPointer(*expr)) {
          ...
          fir::MutableBoxValue pointer(irBox,
                                       ...
                                       /*mutableProperties=*/{});
          language::Compability::lower::associateMutableBox(converter, loc, pointer, *expr,
                                              /*lbounds=*/{}, stmtCtx);
        ...
        if (fir::isAllocatableType(argTy) && arg.isIntentOut() &&
            language::Compability::semantics::IsBindCProcedure(*procRef.proc().GetSymbol()))
          language::Compability::lower::genDeallocateIfAllocated(converter, mutableBox, loc);
        mlir::Value irBox =
      ...
      if (arg.passBy == PassBy::BaseAddress || arg.passBy == PassBy::BoxChar ||
        ...
        if (arg.mustBeMadeContiguous() &&
          ...
          if (language::Compability::evaluate::IsAssumedRank(*expr))
          ...
          if (isPresentValue) {
            ...
            caller.placeInput(arg, builder.createBox(loc, argAddr));
                   ...
                   language::Compability::evaluate::IsAllocatableOrPointerObject(*expr)) {
          ...
          if (dynamicType->category() !=
            ...
            // actually a variable.
            box = language::Compability::evaluate::IsVariable(*expr)
            ...
            } else if (language::Compability::lower::isParentComponent(*expr)) {
              fir::ExtendedValue newExv =
                  language::Compability::lower::updateBoxForParentComponent(converter, box,
                                                              *expr);
              box = fir::getBase(newExv);
            }
          }
    ...
    auto loweredResult =
        language::Compability::lower::genCallOpAndResult(loc, converter, symMap, stmtCtx,
  ...
  template <typename A>
  ExtValue genval(const language::Compability::evaluate::FunctionRef<A> &funcRef) {
  ...
  ExtValue genval(const language::Compability::evaluate::ProcedureRef &procRef) {
  ...
  template <typename T>
  bool isTransformationalRef(const language::Compability::evaluate::FunctionRef<T> &funcRef) {
    return !funcRef.IsElemental() && funcRef.Rank();
  }
  template <typename T>
  bool isTransformationalRef(language::Compability::evaluate::Expr<T> expr) {
    return language::Compability::common::visit(
  ...
  ExtValue asArray(const A &x) {
    return language::Compability::lower::createSomeArrayTempValue(converter, toEvExpr(x),
                                                    symMap, stmtCtx);
  }
  ...
  template <typename A>
  ExtValue asArrayArg(const language::Compability::evaluate::Expr<A> &x) {
    return language::Compability::common::visit(
        [&](const auto &e) { return asArrayArg(e, x); }, x.u);
  }
  template <typename A, typename B>
  ExtValue asArrayArg(const language::Compability::evaluate::Expr<A> &x, const B &y) {
    return language::Compability::common::visit(
        [&](const auto &e) { return asArrayArg(e, y); }, x.u);
  }
  template <typename A, typename B>
  ExtValue asArrayArg(const language::Compability::evaluate::Designator<A> &, const B &x) {
    // Designator is being passed as an argument to a procedure. Lower the
    // expression to a boxed value.
    auto someExpr = toEvExpr(x);
    return language::Compability::lower::createBoxValue(getLoc(), converter, someExpr, symMap,
                                          stmtCtx);
  }
  template <typename A, typename B>
  ...
  template <typename A>
  mlir::Value getIfOverridenExpr(const language::Compability::evaluate::Expr<A> &x) {
    if (const language::Compability::lower::ExprToValueMap *map =
            converter.getExprOverrides()) {
      language::Compability::lower::SomeExpr someExpr = toEvExpr(x);
  ...
  template <typename A>
  ExtValue gen(const language::Compability::evaluate::Expr<A> &x) {
    ...
    if (isScalar(x) ||
        language::Compability::evaluate::UnwrapWholeSymbolOrComponentDataRef(x) ||
        (isTransformationalRef(x) && !isOptimizableTranspose(x, converter)))
      return language::Compability::common::visit([&](const auto &e) { return genref(e); },
                                    x.u);
    if (useBoxArg)
      return asArrayArg(x);
    return asArray(x);
  ...
  template <typename A>
  ExtValue genval(const language::Compability::evaluate::Expr<A> &x) {
    if (mlir::Value val = getIfOverridenExpr(x))
      return val;
    if (isScalar(x) || language::Compability::evaluate::UnwrapWholeSymbolDataRef(x) ||
        inInitializer)
      return language::Compability::common::visit([&](const auto &e) { return genval(e); },
                                    x.u);
    return asArray(x);
  }

  template <int KIND>
  ExtValue genval(const language::Compability::evaluate::Expr<language::Compability::evaluate::Type<
                      language::Compability::common::TypeCategory::Logical, KIND>> &exp) {
  ...
  using RefSet =
      std::tuple<language::Compability::evaluate::ComplexPart, language::Compability::evaluate::Substring,
                 language::Compability::evaluate::DataRef, language::Compability::evaluate::Component,
                 language::Compability::evaluate::ArrayRef, language::Compability::evaluate::CoarrayRef,
                 language::Compability::semantics::SymbolRef>;
  template <typename A>
  static constexpr bool inRefSet = language::Compability::common::HasMember<A, RefSet>;
  ...
  template <typename A, template <typename> typename T,
            ...
            std::enable_if_t<
                std::is_same_v<B, language::Compability::evaluate::Expr<A>> ||
                    std::is_same_v<B, language::Compability::evaluate::Designator<A>> ||
                    std::is_same_v<B, language::Compability::evaluate::FunctionRef<A>>,
                bool> = true>
...
#define PushSemantics(PushVal)                                                 \
  [[maybe_unused]] auto CONCAT(pushSemanticsLocalVariable, __LINE__) =         \
      language::Compability::common::ScopedSet(semant, PushVal);
...
createCharArrayAmend(mlir::Location loc, fir::FirOpBuilder &builder,
                     ...
                     toolchain::ArrayRef<mlir::Value> bounds) {
...
convertToArrayBoxValue(mlir::Location loc, fir::FirOpBuilder &builder,
  ...
  toolchain::SmallVector<mlir::Value> extents(seqTy.getDimension(), undef);
...
class ArrayExprLowering {
  ...
  using ImplicitSubscripts = language::Compability::lower::details::ImplicitSubscripts;
  using PathComponent = language::Compability::lower::PathComponent;

  /// Active iteration space.
  using IterationSpace = language::Compability::lower::IterationSpace;
  using IterSpace = const language::Compability::lower::IterationSpace &;

  /// Current continuation. Function that will generate IR for a single
  /// iteration of the pending iterative loop structure.
  using CC = language::Compability::lower::GenerateElementalArrayFunc;

  /// Projection continuation. Function that will project one iteration space
  /// into another.
  using PC = std::function<IterationSpace(IterSpace)>;
  using ArrayBaseTy =
      std::variant<std::monostate, const language::Compability::evaluate::ArrayRef *,
                   const language::Compability::evaluate::DataRef *>;
  using ComponentPath = language::Compability::lower::ComponentPath;
...
public:
  ...
  static void lowerArrayAssignment(language::Compability::lower::AbstractConverter &converter,
                                   language::Compability::lower::SymMap &symMap,
                                   language::Compability::lower::StatementContext &stmtCtx,
  ...
  static void lowerAnyMaskedArrayAssignment(
      ...
      const language::Compability::lower::SomeExpr &lhs, const language::Compability::lower::SomeExpr &rhs,
      language::Compability::lower::ExplicitIterSpace &explicitSpace,
      language::Compability::lower::ImplicitIterSpace &implicitSpace) {
  ...
  static void lowerArrayOfPointerAssignment(
      ...
      language::Compability::lower::ExplicitIterSpace &explicitSpace,
      language::Compability::lower::ImplicitIterSpace &implicitSpace,
      const toolchain::SmallVector<mlir::Value> &lbounds,
      std::optional<toolchain::SmallVector<mlir::Value>> ubounds) {
    ArrayExprLowering ael(converter, stmtCtx, symMap,
                          ConstituentSemantics::CopyInCopyOut, &explicitSpace,
                          &implicitSpace);
  ...
  void lowerArrayOfPointerAssignment(
    ...
    if (rhs.Rank() == 0 ||
        (language::Compability::evaluate::UnwrapWholeSymbolOrComponentDataRef(rhs) &&
         language::Compability::evaluate::IsAllocatableOrPointerObject(rhs))) {
  ...
  void lowerAllocatableArrayAssignment(const language::Compability::lower::SomeExpr &lhs,
    ...
    fir::MutableBoxValue mutableBox =
        language::Compability::lower::createMutableBox(loc, converter, lhs, symMap);
    if (rhs.Rank() > 0)
  ...
  static ExtValue
  lowerBoxedArrayExpression(language::Compability::lower::AbstractConverter &converter,
                            language::Compability::lower::SymMap &symMap,
                            language::Compability::lower::StatementContext &stmtCtx,
                            const language::Compability::lower::SomeExpr &expr) {
    ArrayExprLowering ael{converter, stmtCtx, symMap,
  ...
  ExtValue lowerBoxedArrayExpr(const language::Compability::lower::SomeExpr &exp) {
  ...
  static ExtValue
  lowerNewArrayExpression(language::Compability::lower::AbstractConverter &converter,
                          language::Compability::lower::SymMap &symMap,
                          language::Compability::lower::StatementContext &stmtCtx,
                          const language::Compability::lower::SomeExpr &expr) {
    ArrayExprLowering ael{converter, stmtCtx, symMap};
  ...
  static void lowerLazyArrayExpression(
      language::Compability::lower::AbstractConverter &converter,
      language::Compability::lower::SymMap &symMap, language::Compability::lower::StatementContext &stmtCtx,
      const language::Compability::lower::SomeExpr &expr, mlir::Value raggedHeader) {
    ArrayExprLowering ael(converter, stmtCtx, symMap);
  ...
  void lowerLazyArrayExpression(const language::Compability::lower::SomeExpr &expr,
                                mlir::Value header) {
    mlir::Location loc = getLoc();
    ...
    ccPrelude = [=](toolchain::ArrayRef<mlir::Value> shape) {
    ...
    ccLoadDest = [=](toolchain::ArrayRef<mlir::Value> shape) -> fir::ArrayLoadOp {
    ...
    ccStoreToDest = [=](IterSpace iters) {
      ...
      toolchain::SmallVector<mlir::Value> indices = fir::factory::originateIndices(
  ...
  static void
  lowerElementalUserAssignment(language::Compability::lower::AbstractConverter &converter,
                               language::Compability::lower::SymMap &symMap,
                               language::Compability::lower::StatementContext &stmtCtx,
                               language::Compability::lower::ExplicitIterSpace &explicitSpace,
                               language::Compability::lower::ImplicitIterSpace &implicitSpace,
    ...
    mlir::func::FuncOp func =
        language::Compability::lower::CallerInterface(procRef, converter).getFuncOp();
  ...
  static void
  lowerElementalSubroutine(language::Compability::lower::AbstractConverter &converter,
                           language::Compability::lower::SymMap &symMap,
                           language::Compability::lower::StatementContext &stmtCtx,
                           const language::Compability::lower::SomeExpr &call) {
    ArrayExprLowering ael(converter, stmtCtx, symMap,
                          ConstituentSemantics::RefTransparent);
    ael.lowerElementalSubroutine(call);
  }

  static const std::optional<language::Compability::evaluate::ActualArgument>
  extractPassedArgFromProcRef(const language::Compability::evaluate::ProcedureRef &procRef,
                              language::Compability::lower::AbstractConverter &converter) {
  ...
  // TODO: See the comment in genarr(const language::Compability::lower::Parentheses<T>&).
  // This is skipping generation of copy-in/copy-out code for analysis that is
  // required when arguments are in parentheses.
  void lowerElementalSubroutine(const language::Compability::lower::SomeExpr &call) {
    if (const auto *procRef =
            std::get_if<language::Compability::evaluate::ProcedureRef>(&call.u))
      setLoweredProcRef(procRef);
    auto f = genarr(call);
    toolchain::SmallVector<mlir::Value> shape = genIterationShape();
  ...
  ExtValue lowerScalarAssignment(const language::Compability::lower::SomeExpr &lhs,
                                 const language::Compability::lower::SomeExpr &rhs) {
    PushSemantics(ConstituentSemantics::RefTransparent);
    ...
    if (mlir::Operation *defOp = fir::getBase(lexv).getDefiningOp()) {
      toolchain::TypeSwitch<mlir::Operation *>(defOp)
  ...
  static ExtValue lowerScalarUserAssignment(
      ...
      const language::Compability::lower::SomeExpr &lhs,
      const language::Compability::lower::SomeExpr &rhs) {
    language::Compability::lower::ImplicitIterSpace implicit;
    ArrayExprLowering ael(converter, stmtCtx, symMap,
                          ConstituentSemantics::RefTransparent,
...
private:
  ...
  void determineShapeOfDest(const language::Compability::lower::SomeExpr &lhs) {
    if (!destShape.empty())
      return;
    ...
    mlir::Location loc = getLoc();
    if (std::optional<language::Compability::evaluate::ConstantSubscripts> constantShape =
            language::Compability::evaluate::GetConstantExtents(converter.getFoldingContext(),
                                                  lhs))
      for (language::Compability::common::ConstantSubscript extent : *constantShape)
        destShape.push_back(builder.createIntegerConstant(loc, idxTy, extent));
  }

  bool genShapeFromDataRef(const language::Compability::semantics::Symbol &x) {
    return false;
  }
  bool genShapeFromDataRef(const language::Compability::evaluate::CoarrayRef &) {
    TODO(getLoc(), "coarray: reference to a coarray in an expression");
    return false;
  }
  bool genShapeFromDataRef(const language::Compability::evaluate::Component &x) {
    return x.base().Rank() > 0 ? genShapeFromDataRef(x.base()) : false;
  }
  bool genShapeFromDataRef(const language::Compability::evaluate::ArrayRef &x) {
    if (x.Rank() == 0)
      return false;
    if (x.base().Rank() > 0)
    ...
    mlir::Location loc = getLoc();
    mlir::IndexType idxTy = builder.getIndexType();
    toolchain::SmallVector<mlir::Value> definedShape =
        fir::factory::getExtents(loc, builder, exv);
    mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
    for (auto ss : toolchain::enumerate(x.subscript())) {
      language::Compability::common::visit(
          language::Compability::common::visitors{
              [&](const language::Compability::evaluate::Triplet &trip) {
  ...
  bool genShapeFromDataRef(const language::Compability::evaluate::NamedEntity &x) {
    if (x.IsSymbol())
      return genShapeFromDataRef(getFirstSym(x));
    return genShapeFromDataRef(x.GetComponent());
  }
  bool genShapeFromDataRef(const language::Compability::evaluate::DataRef &x) {
    return language::Compability::common::visit(
  ...
  bool determineShapeWithSlice(const language::Compability::lower::SomeExpr &lhs) {
    LLVM_DEBUG(language::Compability::semantics::DumpEvaluateExpr::Dump(
        toolchain::dbgs() << "determine shape of:\n", lhs));
    // FIXME: We may not want to use ExtractDataRef here since it doesn't deal
    // with substrings, etc.
    std::optional<language::Compability::evaluate::DataRef> dref =
        language::Compability::evaluate::ExtractDataRef(lhs);
  ...
  ExtValue lowerArrayExpression(const language::Compability::lower::SomeExpr &exp) {
    mlir::Type resTy = converter.genType(exp);

    if (fir::isPolymorphicType(resTy) &&
        language::Compability::evaluate::HasVectorSubscript(exp))
      TODO(getLoc(),
  ...
  void populateBounds(toolchain::SmallVectorImpl<mlir::Value> &bounds,
                      const language::Compability::evaluate::Substring *substring) {
    if (!substring)
      return;
  ...
  CC defaultStoreToDestination(const language::Compability::evaluate::Substring *substring) {
    return [=](IterSpace iterSpace) -> ExtValue {
      ...
      if (isAdjustedArrayElementType(eleTy)) {
        ...
        if (auto charTy = mlir::dyn_cast<fir::CharacterType>(eleTy)) {
          toolchain::SmallVector<mlir::Value> substringBounds;
  ...
  toolchain::SmallVector<mlir::Value> computeSliceShape(mlir::Value slice) {
    toolchain::SmallVector<mlir::Value> slicedShape;
  ...
  toolchain::SmallVector<mlir::Value> getShape(ArrayOperand array) {
    if (array.slice)
  ...
  toolchain::SmallVector<mlir::Value> getShape(fir::ArrayLoadOp arrayLoad) {
  ...
  /// language::Compability::evaluate::Shape from the front end. If the shape is explicit,
  /// this returns any implicit shape component, if it exists.
  toolchain::SmallVector<mlir::Value> genIterationShape() {
    ...
    if (loweredProcRef && loweredProcRef->IsElemental()) {
      const std::optional<language::Compability::evaluate::ActualArgument> passArg =
  ...
  mlir::Value prepareRaggedArrays(language::Compability::lower::FrontEndExpr expr) {
    assert(explicitSpaceIsActive());
    mlir::Location loc = getLoc();
    mlir::TupleType raggedTy = fir::factory::getRaggedArrayHeaderType(builder);
    toolchain::SmallVector<toolchain::SmallVector<fir::DoLoopOp>> loopStack =
    ...
    for (std::remove_const_t<decltype(depth)> i = 0; i < depth; ++i) {
      ...
      toolchain::SmallVector<mlir::Value> offsets;
      for (auto doLoop : loopStack[i]) {
      ...
      for (const auto *e : implicitSpace->getExprs())
        if (e && !implicitSpace->isLowered(e)) {
          if (mlir::Value var = implicitSpace->lookupMaskVariable(e)) {
            ...
            language::Compability::lower::createLazyArrayTempValue(converter, *e, header,
                                                     symMap, stmtCtx);
          ...
          fir::ExtendedValue tmp = language::Compability::lower::createSomeArrayTempValue(
  ...
  genImplicitLoops(mlir::ValueRange shape, mlir::Value innerArg) {
    ...
    mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
    mlir::Value zero = builder.createIntegerConstant(loc, idxTy, 0);
    toolchain::SmallVector<mlir::Value> loopUppers;
    ...
    toolchain::SmallVector<mlir::Value> ivars;

    for (auto i : toolchain::enumerate(toolchain::reverse(loopUppers))) {
      if (i.index() > 0) {
        assert(!loops.empty());
    ...
    return {IterationSpace(innerArg, outerRes, toolchain::reverse(ivars)),
            afterLoopNest};
  }
  ...
  genIterSpace(mlir::Type resultType) {
    ...
    if (implicitSpaceHasMasks()) {
      ...
      auto genCond = [&](language::Compability::lower::FrontEndExpr e, IterSpace iters) {
      ...
      for (toolchain::SmallVector<language::Compability::lower::FrontEndExpr> maskExprs :
  ...
  createAndLoadSomeArrayTemp(mlir::Type type,
                             toolchain::ArrayRef<mlir::Value> shape) {
    mlir::Location loc = getLoc();
  ...
  static fir::ShapeOp genShapeOp(mlir::Location loc, fir::FirOpBuilder &builder,
                                 toolchain::ArrayRef<mlir::Value> shape) {
    mlir::IndexType idxTy = builder.getIndexType();
    toolchain::SmallVector<mlir::Value> idxShape;
    for (auto s : shape)
      idxShape.push_back(builder.createConvert(loc, idxTy, s));
    return fir::ShapeOp::create(builder, loc, idxShape);
  }

  fir::ShapeOp genShapeOp(toolchain::ArrayRef<mlir::Value> shape) {
  ...
  language::Compability::lower::StatementContext &getElementCtx() {
    if (!elementCtx) {
      stmtCtx.pushScope();
      elementCtx = true;
    }
  ...
  CC genElementalArgument(const A &x) {
    ...
    if ((semant == ConstituentSemantics::RefOpaque ||
      ...
      if (!language::Compability::evaluate::IsVariable(x)) {
  ...
  CC genElementalIntrinsicProcRef(
    ...
    toolchain::SmallVector<CC> operands;
    std::string name =
        intrinsic ? intrinsic->name
                  : procRef.proc().GetSymbol()->GetUltimate().name().ToString();
    const fir::IntrinsicArgumentLoweringRules *argLowering =
    ...
    if (intrinsic && language::Compability::lower::intrinsicRequiresCustomOptionalHandling(
      ...
      toolchain::SmallVector<CcPairT> operands;
      auto prepareOptionalArg = [&](const language::Compability::lower::SomeExpr &expr) {
        if (expr.Rank() == 0) {
    ...
    return [=](IterSpace iters) {
      toolchain::SmallVector<ExtValue> args;
      for (const auto &cc : operands)
        args.push_back(cc(iters));
      return language::Compability::lower::genIntrinsicCall(builder, loc, name, retTy, args,
                                              getElementCtx());
    };
  }
  ...
  CC genElementalUserDefinedProcRef(
    ...
    if (const language::Compability::semantics::Symbol *procSym = procRef.proc().GetSymbol())
      if (!language::Compability::semantics::IsPureProcedure(*procSym))
        setUnordered(false);
    ...
    for (const language::Compability::lower::CallInterface<
      ...
      LLVM_DEBUG(expr->AsFortran(toolchain::dbgs()
    ...
    return [=,
      ...
      for (const auto &[cc, argIface] :
           toolchain::zip(operands, caller.getPassedArguments())) {
        auto exv = cc(iters);
        auto arg = exv.match(
      ...
      language::Compability::lower::LoweredResult res =
          language::Compability::lower::genCallOpAndResult(loc, converter, symMap,
  ...
  CC genTransposeProcRef(const language::Compability::evaluate::ProcedureRef &procRef) {
    assert(procRef.arguments().size() == 1 &&
           "TRANSPOSE must have one argument.");
    const auto *argExpr = procRef.arguments()[0].value().UnwrapExpr();
    assert(argExpr);

    toolchain::SmallVector<mlir::Value> savedDestShape = destShape;
    ...
    PushSemantics(ConstituentSemantics::RefTransparent);
    toolchain::SmallVector<CC> operands{genElementalArgument(*argExpr)};
  ...
  CC genProcRef(const language::Compability::evaluate::ProcedureRef &procRef,
                std::optional<mlir::Type> retTy) {
    mlir::Location loc = getLoc();
    setLoweredProcRef(&procRef);
    ...
    if (procRef.IsElemental()) {
      if (const language::Compability::evaluate::SpecificIntrinsic *intrin =
              procRef.proc().GetSpecificIntrinsic()) {
  ...
  CC genarr(const language::Compability::evaluate::ProcedureDesignator &) {
    TODO(getLoc(), "procedure designator");
  }
  CC genarr(const language::Compability::evaluate::ProcedureRef &x) {
  ...
  template <typename A, typename = std::enable_if_t<language::Compability::common::HasMember<
                            A, language::Compability::evaluate::TypelessExpression>>>
  CC genarr(const A &x) {
    return genScalarAndForwardValue(x);
  }

  template <typename A>
  CC genarr(const language::Compability::evaluate::Expr<A> &x) {
    LLVM_DEBUG(language::Compability::semantics::DumpEvaluateExpr::Dump(toolchain::dbgs(), x));
    if (isArray(x) || (explicitSpaceIsActive() && isLeftHandSide()) ||
        isElementalProcWithArrayArgs(x))
      return language::Compability::common::visit([&](const auto &e) { return genarr(e); },
                                    x.u);
    if (explicitSpaceIsActive()) {
      assert(!isArray(x) && !isLeftHandSide());
      auto cc =
          language::Compability::common::visit([&](const auto &e) { return genarr(e); }, x.u);
  ...
  template <language::Compability::common::TypeCategory TC1, int KIND,
            language::Compability::common::TypeCategory TC2>
  CC genarr(const language::Compability::evaluate::Convert<language::Compability::evaluate::Type<TC1, KIND>,
                                             TC2> &x) {
    mlir::Location loc = getLoc();
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::ComplexComponent<KIND> &x) {
    mlir::Location loc = getLoc();
    auto lambda = genarr(x.left());
  ...
  template <typename T>
  CC genarr(const language::Compability::evaluate::Parentheses<T> &x) {
    mlir::Location loc = getLoc();
  ...
  template <language::Compability::common::TypeCategory CAT, int KIND>
  CC genarrIntNeg(
      const language::Compability::evaluate::Expr<language::Compability::evaluate::Type<CAT, KIND>> &left) {
    mlir::Location loc = getLoc();
    auto f = genarr(left);
    return [=](IterSpace iters) -> ExtValue {
      ...
      if constexpr (CAT == language::Compability::common::TypeCategory::Unsigned) {
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                language::Compability::common::TypeCategory::Integer, KIND>> &x) {
    return genarrIntNeg(x.left());
  }
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                language::Compability::common::TypeCategory::Unsigned, KIND>> &x) {
    return genarrIntNeg(x.left());
  }
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                language::Compability::common::TypeCategory::Real, KIND>> &x) {
    mlir::Location loc = getLoc();
    auto f = genarr(x.left());
    return [=](IterSpace iters) -> ExtValue {
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Negate<language::Compability::evaluate::Type<
                language::Compability::common::TypeCategory::Complex, KIND>> &x) {
    mlir::Location loc = getLoc();
    auto f = genarr(x.left());
    return [=](IterSpace iters) -> ExtValue {
...
#define GENBIN(GenBinEvOp, GenBinTyCat, GenBinFirOp)                           \
  template <int KIND>                                                          \
  CC genarr(const language::Compability::evaluate::GenBinEvOp<language::Compability::evaluate::Type<       \
                language::Compability::common::TypeCategory::GenBinTyCat, KIND>> &x) {       \
    return createBinaryOp<GenBinFirOp>(x);                                     \
  }
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Divide<language::Compability::evaluate::Type<
                language::Compability::common::TypeCategory::Complex, KIND>> &x) {
    mlir::Location loc = getLoc();
    mlir::Type ty =
        converter.genType(language::Compability::common::TypeCategory::Complex, KIND);
  ...
  CC genarr(
    ...
    case language::Compability::evaluate::Ordering::Greater:
      return [=](IterSpace iters) -> ExtValue {
        mlir::Value lhs = fir::getBase(lf(iters));
        mlir::Value rhs = fir::getBase(rf(iters));
        return fir::genMax(builder, loc, toolchain::ArrayRef<mlir::Value>{lhs, rhs});
      };
    case language::Compability::evaluate::Ordering::Less:
      return [=](IterSpace iters) -> ExtValue {
        mlir::Value lhs = fir::getBase(lf(iters));
        mlir::Value rhs = fir::getBase(rf(iters));
        return fir::genMin(builder, loc, toolchain::ArrayRef<mlir::Value>{lhs, rhs});
      };
    case language::Compability::evaluate::Ordering::Equal:
      toolchain_unreachable("Equal is not a valid ordering in this context");
    }
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::ComplexConstructor<KIND> &x) {
    mlir::Location loc = getLoc();
    auto lf = genarr(x.left());
    auto rf = genarr(x.right());
    return [=](IterSpace iters) -> ExtValue {
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Concat<KIND> &x) {
    mlir::Location loc = getLoc();
    auto lf = genarr(x.left());
    auto rf = genarr(x.right());
    return [=](IterSpace iters) -> ExtValue {
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::SetLength<KIND> &x) {
    auto lf = genarr(x.left());
  ...
  template <typename T>
  CC genarr(const language::Compability::evaluate::Constant<T> &x) {
    if (x.Rank() == 0)
      return genScalarAndForwardValue(x);
    return genarr(language::Compability::lower::convertConstant(
        converter, getLoc(), x,
  ...
  static language::Compability::lower::SomeExpr
  ...
  extractSubscriptSymbol(const language::Compability::evaluate::Expr<A> &x) {
    return language::Compability::common::visit(
        [&](const auto &v) { return extractSubscriptSymbol(v); }, x.u);
  }
  template <typename A>
  static const language::Compability::semantics::Symbol *
  extractSubscriptSymbol(const language::Compability::evaluate::Designator<A> &x) {
    return language::Compability::evaluate::UnwrapWholeSymbolDataRef(x);
  }
  template <typename A>
  static const language::Compability::semantics::Symbol *extractSubscriptSymbol(const A &x) {
    return nullptr;
  }
  ...
  bool hasEvArrayRef(const A &x) {
    struct HasEvArrayRefHelper
        : public language::Compability::evaluate::AnyTraverse<HasEvArrayRefHelper> {
      HasEvArrayRefHelper()
          : language::Compability::evaluate::AnyTraverse<HasEvArrayRefHelper>(*this) {}
      using language::Compability::evaluate::AnyTraverse<HasEvArrayRefHelper>::operator();
      bool operator()(const language::Compability::evaluate::ArrayRef &) const {
        return true;
      }
    } helper;
    return helper(x);
  ...
  CC genVectorSubscriptArrayFetch(const language::Compability::lower::SomeExpr &expr,
                                  std::size_t dim) {
    PushSemantics(ConstituentSemantics::RefTransparent);
    auto saved = language::Compability::common::ScopedSet(explicitSpace, nullptr);
    toolchain::SmallVector<mlir::Value> savedDestShape = destShape;
    destShape.clear();
  ...
  mlir::Value genAccessByVector(mlir::Location loc, CC genArrFetch,
    ...
    IterationSpace vecIters(iters,
                            toolchain::ArrayRef<mlir::Value>{iters.iterValue(dim)});
  ...
  void genSliceIndices(ComponentPath &cmptData, const ExtValue &arrayExv,
                       const language::Compability::evaluate::ArrayRef &x, bool atBase) {
    mlir::Location loc = getLoc();
    mlir::IndexType idxTy = builder.getIndexType();
    mlir::Value one = builder.createIntegerConstant(loc, idxTy, 1);
    toolchain::SmallVector<mlir::Value> &trips = cmptData.trips;
    LLVM_DEBUG(toolchain::dbgs() << "array: " << arrayExv << '\n');
    auto &pc = cmptData.pc;
    const bool useTripsForSlice = !explicitSpaceIsActive();
    const bool createDestShape = destShape.empty();
    bool useSlice = false;
    std::size_t shapeIndex = 0;
    for (auto sub : toolchain::enumerate(x.subscript())) {
      const std::size_t subsIndex = sub.index();
      language::Compability::common::visit(
          language::Compability::common::visitors{
              [&](const language::Compability::evaluate::Triplet &t) {
              ...
              [&](const language::Compability::evaluate::IndirectSubscriptIntegerExpr &ie) {
  ...
  toolchain::SmallVector<mlir::Value> getShape(mlir::Type ty) {
    toolchain::SmallVector<mlir::Value> result;
  ...
  CC genarr(const language::Compability::semantics::SymbolRef &sym,
  ...
  inline toolchain::SmallVector<mlir::Value>
  padSlice(toolchain::ArrayRef<mlir::Value> triples, mlir::Value shapeVal) {
    toolchain::SmallVector<mlir::Value> result;
    mlir::Location loc = getLoc();
    if (triples.size()) {
      ...
      if (!shapeVal) {
        ...
        for (auto ext : shapeOp.getExtents()) {
          ...
          result.push_back(one);
                     ...
                     shapeVal.getDefiningOp())) {
        for (auto [lb, ext] :
             toolchain::zip(shapeShift.getOrigins(), shapeShift.getExtents())) {
          result.push_back(lb);
          result.push_back(ext);
          result.push_back(one);
        }
  ...
  CC genarr(const ExtValue &extMemref, ComponentPath &components,
    ...
    toolchain::SmallVector<mlir::Value> arrLdTypeParams =
  ...
  genOptionalArrayFetch(const language::Compability::lower::SomeExpr &expr) {
  ...
  CC genarrForwardOptionalArgumentToCall(const language::Compability::lower::SomeExpr &expr) {
    mlir::Location loc = getLoc();
  ...
  CC genarr(const language::Compability::evaluate::ComplexPart &x,
            ComponentPath &components) {
  ...
  CC genarr(const language::Compability::evaluate::StaticDataObject::Pointer &,
            ComponentPath &components) {
    TODO(getLoc(), "substring of static object inside FORALL");
  }

  /// Substrings (see 9.4.1)
  CC genarr(const language::Compability::evaluate::Substring &x, ComponentPath &components) {
  ...
  template <typename T>
  CC genarr(const language::Compability::evaluate::FunctionRef<T> &funRef) {
  ...
  mlir::FunctionType memcpyType() {
    ...
    toolchain::SmallVector<mlir::Type> args = {ptrTy, ptrTy, builder.getI64Type()};
    return mlir::FunctionType::get(builder.getContext(), args, {});
  }

  /// Create a call to the LLVM memcpy intrinsic.
  void createCallMemcpy(toolchain::ArrayRef<mlir::Value> args, bool isVolatile) {
    mlir::Location loc = getLoc();
  ...
  genArrayCtorInitializer(const language::Compability::evaluate::Expr<A> &x, mlir::Type,
  ...
  template <typename A>
  std::pair<ExtValue, bool>
  genArrayCtorInitializer(const language::Compability::evaluate::ImpliedDo<A> &x,
    ...
    for (const language::Compability::evaluate::ArrayConstructorValue<A> &acv : x.values()) {
      auto [exv, copyNeeded] = language::Compability::common::visit(
          [&](const auto &v) {
  ...
  template <typename A>
  CC genarr(const language::Compability::evaluate::ArrayConstructor<A> &x) {
    mlir::Location loc = getLoc();
  ...
  CC genarr(const language::Compability::evaluate::ImpliedDoIndex &) {
    fir::emitFatalError(getLoc(), "implied do index cannot have rank > 0");
  }
  CC genarr(const language::Compability::evaluate::TypeParamInquiry &x) {
    TODO(getLoc(), "array expr type parameter inquiry");
    return [](IterSpace iters) -> ExtValue { return mlir::Value{}; };
  }
  CC genarr(const language::Compability::evaluate::DescriptorInquiry &x) {
    TODO(getLoc(), "array expr descriptor inquiry");
    return [](IterSpace iters) -> ExtValue { return mlir::Value{}; };
  }
  CC genarr(const language::Compability::evaluate::StructureConstructor &x) {
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Not<KIND> &x) {
    mlir::Location loc = getLoc();
    mlir::IntegerType i1Ty = builder.getI1Type();
    auto lambda = genarr(x.left());
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::LogicalOperation<KIND> &x) {
    ...
    case language::Compability::evaluate::LogicalOperator::Not:
      toolchain_unreachable(".NOT. handled elsewhere");
    }
  ...
  template <int KIND>
  CC genarr(const language::Compability::evaluate::Relational<language::Compability::evaluate::Type<
                language::Compability::common::TypeCategory::Integer, KIND>> &x) {
    return createCompareOp<mlir::arith::CmpIOp>(
  ...
  CC genarr(
      const language::Compability::evaluate::Relational<language::Compability::evaluate::SomeType> &r) {
    return language::Compability::common::visit([&](const auto &x) { return genarr(x); },
                                  r.u);
  }

  template <typename A>
  CC genarr(const language::Compability::evaluate::Designator<A> &des) {
  ...
  void extendComponent(language::Compability::lower::ComponentPath &component,
                       mlir::Type coorTy, mlir::ValueRange vals) {
    auto *bldr = &converter.getFirOpBuilder();
    toolchain::SmallVector<mlir::Value> offsets(vals.begin(), vals.end());
  ...
  mlir::Type lowerPath(const ExtValue &arrayExv, ComponentPath &components) {
    ...
    PushSemantics(isProjectedCopyInCopyOut()
                      ...
                      : nextPathSemantics());
    unsigned index = 0;
    for (const auto &v : toolchain::reverse(revPath)) {
      language::Compability::common::visit(
          language::Compability::common::visitors{
              ...
              [&](const language::Compability::evaluate::ComplexPart *x) {
                assert(!prefix && "complex part must be at end");
                mlir::Value offset = builder.createIntegerConstant(
                    loc, builder.getI32Type(),
                    x->part() == language::Compability::evaluate::ComplexPart::Part::RE ? 0
                                                                          : 1);
                components.suffixComponents.push_back(offset);
                ty = fir::applyPathToType(ty, mlir::ValueRange{offset});
              },
              [&](const language::Compability::evaluate::ArrayRef *x) {
                if (language::Compability::lower::isRankedArrayAccess(*x)) {
                  ...
                  unsigned ssIndex = 0u;
                  toolchain::SmallVector<mlir::Value> componentsToAdd;
                  for (const auto &ss : x->subscript()) {
                    language::Compability::common::visit(
                        language::Compability::common::visitors{
                            [&](const language::Compability::evaluate::
  ...
  toolchain::SmallVector<mlir::Value> genSubstringBounds(ComponentPath &components) {
    toolchain::SmallVector<mlir::Value> result;
    if (components.substring)
  ...
  CC genImplicitArrayAccess(const language::Compability::evaluate::NamedEntity &x,
                            ComponentPath &components) {
    if (x.IsSymbol())
  ...
  CC genImplicitArrayAccess(const language::Compability::semantics::Symbol &x,
                            ComponentPath &components) {
    mlir::Value ptrVal = nullptr;
    if (x.test(language::Compability::semantics::Symbol::Flag::CrayPointee)) {
      language::Compability::semantics::SymbolRef ptrSym{
          language::Compability::semantics::GetCrayPointer(x)};
  ...
  bool tailIsPointerInPointerAssignment(const language::Compability::semantics::Symbol &x,
                                        ComponentPath &components) {
    return isPointerAssignment() && language::Compability::semantics::IsPointer(x) &&
           !components.hasComponents();
  }
  bool tailIsPointerInPointerAssignment(const language::Compability::evaluate::Component &x,
                                        ComponentPath &components) {
    return tailIsPointerInPointerAssignment(getLastSym(x), components);
  }

  CC genarr(const language::Compability::semantics::Symbol &x, ComponentPath &components) {
    if (explicitSpaceIsActive()) {
  ...
  CC genarr(const language::Compability::evaluate::Component &x, ComponentPath &components) {
    if (explicitSpaceIsActive()) {
    ...
    if (!getLastSym(x).test(language::Compability::semantics::Symbol::Flag::ParentComp))
  ...
  CC genarr(const language::Compability::evaluate::ArrayRef &x, ComponentPath &components) {
    if (explicitSpaceIsActive()) {
      if (language::Compability::lower::isRankedArrayAccess(x))
        components.reversePath.push_back(ImplicitSubscripts{});
      if (fir::ArrayLoadOp load = explicitSpace->findBinding(&x)) {
  ...
  CC genarr(const language::Compability::evaluate::CoarrayRef &x, ComponentPath &components) {
    TODO(getLoc(), "coarray: reference to a coarray in an expression");
  }

  CC genarr(const language::Compability::evaluate::NamedEntity &x,
            ComponentPath &components) {
    return x.IsSymbol() ? genarr(getFirstSym(x), components)
                        : genarr(x.GetComponent(), components);
  }

  CC genarr(const language::Compability::evaluate::DataRef &x, ComponentPath &components) {
    return language::Compability::common::visit(
  ...
  explicit ArrayExprLowering(language::Compability::lower::AbstractConverter &converter,
                             language::Compability::lower::StatementContext &stmtCtx,
                             language::Compability::lower::SymMap &symMap)
  ...
  explicit ArrayExprLowering(language::Compability::lower::AbstractConverter &converter,
                             ...
                             language::Compability::lower::SymMap &symMap,
                             ConstituentSemantics sem,
                             language::Compability::lower::ExplicitIterSpace *expSpace,
                             language::Compability::lower::ImplicitIterSpace *impSpace)
      : converter{converter}, builder{converter.getFirOpBuilder()},
        stmtCtx{stmtCtx}, symMap{symMap},
  ...
  void setPointerAssignmentBounds(
      const toolchain::SmallVector<mlir::Value> &lbs,
      std::optional<toolchain::SmallVector<mlir::Value>> ubs) {
    lbounds = lbs;
    ubounds = ubs;
  }

  void setLoweredProcRef(const language::Compability::evaluate::ProcedureRef *procRef) {
  ...
  std::optional<std::function<void(toolchain::ArrayRef<mlir::Value>)>> ccPrelude;
  std::optional<std::function<fir::ArrayLoadOp(toolchain::ArrayRef<mlir::Value>)>>
      ccLoadDest;
  ...
  toolchain::SmallVector<mlir::Value> destShape;
  /// List of arrays in the expression that have been loaded.
  toolchain::SmallVector<ArrayOperand> arrayOperands;
  /// If there is a user-defined iteration space, explicitShape will hold the
  /// information from the front end.
  language::Compability::lower::ExplicitIterSpace *explicitSpace = nullptr;
  language::Compability::lower::ImplicitIterSpace *implicitSpace = nullptr;
  ...
  std::optional<toolchain::SmallVector<mlir::Value>> lbounds;
  std::optional<toolchain::SmallVector<mlir::Value>> ubounds;
  ...
  const language::Compability::evaluate::ProcedureRef *loweredProcRef = nullptr;
};
} // namespace

fir::ExtendedValue language::Compability::lower::createSomeExtendedExpression(
    ...
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "expr: ") << '\n');
  return ScalarExprLowering{loc, converter, symMap, stmtCtx}.genval(expr);
}

fir::ExtendedValue language::Compability::lower::createSomeInitializerExpression(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &expr, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "expr: ") << '\n');
...
fir::ExtendedValue language::Compability::lower::createSomeExtendedAddress(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &expr, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "address: ") << '\n');
  return ScalarExprLowering(loc, converter, symMap, stmtCtx).gen(expr);
}

fir::ExtendedValue language::Compability::lower::createInitializerAddress(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &expr, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "address: ") << '\n');
...
void language::Compability::lower::createSomeArrayAssignment(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &lhs, const language::Compability::lower::SomeExpr &rhs,
    language::Compability::lower::SymMap &symMap, language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(lhs.AsFortran(toolchain::dbgs() << "onto array: ") << '\n';
             rhs.AsFortran(toolchain::dbgs() << "assign expression: ") << '\n';);
  ArrayExprLowering::lowerArrayAssignment(converter, symMap, stmtCtx, lhs, rhs);
}

void language::Compability::lower::createSomeArrayAssignment(
    language::Compability::lower::AbstractConverter &converter, const fir::ExtendedValue &lhs,
    const language::Compability::lower::SomeExpr &rhs, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(toolchain::dbgs() << "onto array: " << lhs << '\n';
             rhs.AsFortran(toolchain::dbgs() << "assign expression: ") << '\n';);
  ArrayExprLowering::lowerArrayAssignment(converter, symMap, stmtCtx, lhs, rhs);
}
void language::Compability::lower::createSomeArrayAssignment(
    language::Compability::lower::AbstractConverter &converter, const fir::ExtendedValue &lhs,
    const fir::ExtendedValue &rhs, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(toolchain::dbgs() << "onto array: " << lhs << '\n';
             toolchain::dbgs() << "assign expression: " << rhs << '\n';);
  ArrayExprLowering::lowerArrayAssignment(converter, symMap, stmtCtx, lhs, rhs);
}

void language::Compability::lower::createAnyMaskedArrayAssignment(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &lhs, const language::Compability::lower::SomeExpr &rhs,
    language::Compability::lower::ExplicitIterSpace &explicitSpace,
    language::Compability::lower::ImplicitIterSpace &implicitSpace,
...
void language::Compability::lower::createAllocatableArrayAssignment(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &lhs, const language::Compability::lower::SomeExpr &rhs,
    language::Compability::lower::ExplicitIterSpace &explicitSpace,
    language::Compability::lower::ImplicitIterSpace &implicitSpace,
    language::Compability::lower::SymMap &symMap, language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(lhs.AsFortran(toolchain::dbgs() << "defining array: ") << '\n';
             rhs.AsFortran(toolchain::dbgs() << "assign expression: ")
             << " given the explicit iteration space:\n"
             << explicitSpace << "\n and implied mask conditions:\n"
             << implicitSpace << '\n';);
...
void language::Compability::lower::createArrayOfPointerAssignment(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &lhs, const language::Compability::lower::SomeExpr &rhs,
    language::Compability::lower::ExplicitIterSpace &explicitSpace,
    language::Compability::lower::ImplicitIterSpace &implicitSpace,
    ...
    std::optional<toolchain::SmallVector<mlir::Value>> ubounds,
    language::Compability::lower::SymMap &symMap, language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(lhs.AsFortran(toolchain::dbgs() << "defining pointer: ") << '\n';
             rhs.AsFortran(toolchain::dbgs() << "assign expression: ")
             << " given the explicit iteration space:\n"
             << explicitSpace << "\n and implied mask conditions:\n"
             << implicitSpace << '\n';);
...
fir::ExtendedValue language::Compability::lower::createSomeArrayTempValue(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &expr, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "array value: ") << '\n');
  return ArrayExprLowering::lowerNewArrayExpression(converter, symMap, stmtCtx,
                                                    expr);
}

void language::Compability::lower::createLazyArrayTempValue(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &expr, mlir::Value raggedHeader,
    language::Compability::lower::SymMap &symMap, language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "array value: ") << '\n');
...
fir::ExtendedValue
language::Compability::lower::createSomeArrayBox(language::Compability::lower::AbstractConverter &converter,
                                   const language::Compability::lower::SomeExpr &expr,
                                   language::Compability::lower::SymMap &symMap,
                                   language::Compability::lower::StatementContext &stmtCtx) {
  LLVM_DEBUG(expr.AsFortran(toolchain::dbgs() << "box designator: ") << '\n');
...
fir::MutableBoxValue language::Compability::lower::createMutableBox(
  ...
  // bridge.
  language::Compability::lower::StatementContext dummyStmtCtx;
  return ScalarExprLowering{loc, converter, symMap, dummyStmtCtx}
      .genMutableBoxValue(expr);
}

bool language::Compability::lower::isParentComponent(const language::Compability::lower::SomeExpr &expr) {
  if (const language::Compability::semantics::Symbol * symbol{GetLastSymbol(expr)}) {
    if (symbol->test(language::Compability::semantics::Symbol::Flag::ParentComp))
      return true;
  }
  return false;
}
...
fir::ExtendedValue language::Compability::lower::updateBoxForParentComponent(
    language::Compability::lower::AbstractConverter &converter, fir::ExtendedValue box,
    const language::Compability::lower::SomeExpr &expr) {
  mlir::Location loc = converter.getCurrentLocation();
  auto &builder = converter.getFirOpBuilder();
...
fir::ExtendedValue language::Compability::lower::createBoxValue(
    mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
    const language::Compability::lower::SomeExpr &expr, language::Compability::lower::SymMap &symMap,
    language::Compability::lower::StatementContext &stmtCtx) {
  if (expr.Rank() > 0 && language::Compability::evaluate::IsVariable(expr) &&
      !language::Compability::evaluate::HasVectorSubscript(expr)) {
    fir::ExtendedValue result =
        language::Compability::lower::createSomeArrayBox(converter, expr, symMap, stmtCtx);
    if (isParentComponent(expr))
      result = updateBoxForParentComponent(converter, result, expr);
    return result;
  }
  fir::ExtendedValue addr = language::Compability::lower::createSomeExtendedAddress(
...
mlir::Value language::Compability::lower::createSubroutineCall(
  ...
  if (isUserDefAssignment) {
    ...
    if (call.IsElemental() && lhs->Rank() > 0) {
      ...
      mlir::func::FuncOp func =
          language::Compability::lower::CallerInterface(call, converter).getFuncOp();
      ...
      language::Compability::lower::createSomeExtendedExpression(
  ...
  auto res = language::Compability::lower::createSomeExtendedExpression(
      loc, converter, toEvExpr(call), symMap, stmtCtx);
...
template <>
fir::ArrayLoadOp
genArrayLoad(mlir::Location loc, language::Compability::lower::AbstractConverter &converter,
             fir::FirOpBuilder &builder, const language::Compability::evaluate::ArrayRef *x,
             language::Compability::lower::SymMap &symMap,
             language::Compability::lower::StatementContext &stmtCtx) {
  if (x->base().IsSymbol())
...
void language::Compability::lower::createArrayLoads(
    language::Compability::lower::AbstractConverter &converter,
    language::Compability::lower::ExplicitIterSpace &esp, language::Compability::lower::SymMap &symMap) {
  std::size_t counter = esp.getCounter();
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Location loc = converter.getCurrentLocation();
  language::Compability::lower::StatementContext &stmtCtx = esp.stmtContext();
  ...
  if (esp.lhsBases[counter]) {
    ...
    auto load = language::Compability::common::visit(genLoad, base);
    esp.initialArgs.push_back(load);
    esp.resetInnerArgs();
    esp.bindLoad(base, load);
  }
  for (const auto &base : esp.rhsBases[counter])
    esp.bindLoad(base, language::Compability::common::visit(genLoad, base));
}

void language::Compability::lower::createArrayMergeStores(
    language::Compability::lower::AbstractConverter &converter,
    language::Compability::lower::ExplicitIterSpace &esp) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Location loc = converter.getCurrentLocation();
  builder.setInsertionPointAfter(esp.getOuterLoop());
  // Gen the fir.array_merge_store ops for all LHS arrays.
  for (auto i : toolchain::enumerate(esp.getOuterLoop().getResults()))
...
mlir::Value language::Compability::lower::addCrayPointerInst(mlir::Location loc,
                                               fir::FirOpBuilder &builder,
                                               mlir::Value ptrVal,
                                               mlir::Type ptrTy,
                                               mlir::Type pteTy) {
