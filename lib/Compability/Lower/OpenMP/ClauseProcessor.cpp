...
using ReductionModifier =
    language::Compability::lower::omp::clause::Reduction::ReductionModifier;
...
genAllocateClause(lower::AbstractConverter &converter,
                  ...
                  toolchain::SmallVectorImpl<mlir::Value> &allocatorOperands,
                  toolchain::SmallVectorImpl<mlir::Value> &allocateOperands) {
...
genDependKindAttr(lower::AbstractConverter &converter,
  ...
  case omp::clause::DependenceType::Source:
    toolchain_unreachable("unhandled parser task dependence type");
    break;
  }
...
static void addUseDeviceClause(
    ...
    toolchain::SmallVectorImpl<const semantics::Symbol *> &useDeviceSyms) {
...
bool ClauseProcessor::processCancelDirectiveName(
  ...
  if (auto *clause = findUniqueClause<omp::CancellationConstructType>()) {
    switch (clause->v) {
    case toolchain::omp::OMP_CANCELLATION_CONSTRUCT_Parallel:
      directive = mlir::omp::ClauseCancellationConstructType::Parallel;
      break;
    case toolchain::omp::OMP_CANCELLATION_CONSTRUCT_Loop:
      directive = mlir::omp::ClauseCancellationConstructType::Loop;
      break;
    case toolchain::omp::OMP_CANCELLATION_CONSTRUCT_Sections:
      directive = mlir::omp::ClauseCancellationConstructType::Sections;
      break;
    case toolchain::omp::OMP_CANCELLATION_CONSTRUCT_Taskgroup:
      directive = mlir::omp::ClauseCancellationConstructType::Taskgroup;
      break;
    case toolchain::omp::OMP_CANCELLATION_CONSTRUCT_None:
      toolchain_unreachable("OMP_CANCELLATION_CONSTRUCT_None");
      break;
    }
  } else {
    toolchain_unreachable("cancel construct missing cancellation construct type");
  }
...
//===----------------------------------------------------------------------===//
static toolchain::StringMap<bool> getTargetFeatures(mlir::ModuleOp module) {
  toolchain::StringMap<bool> featuresMap;
  toolchain::SmallVector<toolchain::StringRef> targetFeaturesVec;
  if (mlir::LLVM::TargetFeaturesAttr features =
          fir::getTargetFeatures(module)) {
    toolchain::ArrayRef<mlir::StringAttr> featureAttrs = features.getFeatures();
    for (auto &featureAttr : featureAttrs) {
      toolchain::StringRef featureKeyString = featureAttr.strref();
...
addAlignedClause(lower::AbstractConverter &converter,
                 ...
                 toolchain::SmallVectorImpl<mlir::Value> &alignedVars,
                 toolchain::SmallVectorImpl<mlir::Attribute> &alignments) {
  ...
  if (auto &alignmentValueParserExpr =
    ...
    toolchain::StringMap<bool> featuresMap = getTargetFeatures(builder.getModule());
    toolchain::Triple triple = fir::getTargetTriple(builder.getModule());
    alignment =
        toolchain::OpenMPIRBuilder::getOpenMPDefaultSimdAlign(triple, featuresMap);
  }
...
public:
  ...
  toolchain::ArrayRef<int64_t> getShape() const { return shape; }
...
void TypeInfo::typeScan(mlir::Type ty) {
  if (auto sty = mlir::dyn_cast<fir::SequenceType>(ty)) {
    ...
    shape = toolchain::SmallVector<int64_t>(sty.getShape());
...
createCopyFunc(mlir::Location loc, lower::AbstractConverter &converter,
  ...
  toolchain::SmallVector<mlir::Type> argsTy = {varType, varType};
...
bool ClauseProcessor::processHasDeviceAddr(
  ...
  bool clauseFound = findRepeatableClause<omp::clause::HasDeviceAddr>(
      [&](const omp::clause::HasDeviceAddr &clause,
        ...
        toolchain::transform(clause.v, std::back_inserter(baseObjects),
...
bool ClauseProcessor::processInReduction(
  ...
  return findRepeatableClause<omp::clause::InReduction>(
      [&](const omp::clause::InReduction &clause, const parser::CharBlock &) {
        toolchain::SmallVector<mlir::Value> inReductionVars;
        toolchain::SmallVector<bool> inReduceVarByRef;
        toolchain::SmallVector<mlir::Attribute> inReductionDeclSymbols;
        ...
        toolchain::copy(inReductionVars, std::back_inserter(result.inReductionVars));
        toolchain::copy(inReduceVarByRef,
                   std::back_inserter(result.inReductionByref));
        toolchain::copy(inReductionDeclSymbols,
                   std::back_inserter(result.inReductionSyms));
        toolchain::copy(inReductionSyms, std::back_inserter(outReductionSyms));
      });
}
...
bool ClauseProcessor::processLink(
    toolchain::SmallVectorImpl<DeclareTargetCaptureInfo> &result) const {
...
bool ClauseProcessor::processMap(
    ...
    mlir::omp::MapClauseOps &result, toolchain::omp::Directive directive,
    toolchain::SmallVectorImpl<const semantics::Symbol *> *mapSyms) const {
  ...
  toolchain::SmallVector<const semantics::Symbol *> localMapSyms;
  toolchain::SmallVectorImpl<const semantics::Symbol *> *ptrMapSyms =
  ...
  auto process = [&](const omp::clause::Map &clause,
    ...
    // default value
    Map::MapType type;
    if (directive == toolchain::omp::Directive::OMPD_target_enter_data &&
        semaCtx.langOptions().OpenMPVersion >= 52)
      type = mapType.value_or(Map::MapType::To);
    else if (directive == toolchain::omp::Directive::OMPD_target_exit_data &&
             semaCtx.langOptions().OpenMPVersion >= 52)
    ...
    switch (type) {
    case Map::MapType::To:
      mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TO;
      break;
    case Map::MapType::From:
      mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_FROM;
      break;
    ...
    if (typeMods) {
      ...
      if (toolchain::is_contained(*typeMods, Map::MapTypeModifier::Always))
        mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_ALWAYS;
      if (toolchain::is_contained(*typeMods, Map::MapTypeModifier::Present))
        mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_PRESENT;
      if (toolchain::is_contained(*typeMods, Map::MapTypeModifier::Close))
        mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_CLOSE;
      if (toolchain::is_contained(*typeMods, Map::MapTypeModifier::Delete))
        mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_DELETE;
      if (toolchain::is_contained(*typeMods, Map::MapTypeModifier::OmpxHold))
        mapTypeBits |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_OMPX_HOLD;
    }
...
bool ClauseProcessor::processMotionClauses(lower::StatementContext &stmtCtx,
  ...
  std::map<Object, OmpMapParentAndMemberData> parentMemberIndices;
  toolchain::SmallVector<const semantics::Symbol *> mapSymbols;
  ...
  auto callbackFn = [&](const auto &clause, const parser::CharBlock &source) {
    ...
    toolchain::omp::OpenMPOffloadMappingFlags mapTypeBits =
        std::is_same_v<toolchain::remove_cvref_t<decltype(clause)>, omp::clause::To>
            ? toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TO
            : toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_FROM;
...
bool ClauseProcessor::processReduction(
  ...
  return findRepeatableClause<omp::clause::Reduction>(
      [&](const omp::clause::Reduction &clause, const parser::CharBlock &) {
        toolchain::SmallVector<mlir::Value> reductionVars;
        ...
        if (!rp.processReductionArguments<mlir::omp::DeclareReductionOp>(
          ...
          TODO(currentLocation, "Lowering unrecognised reduction type");
        // Copy local lists into the output.
        toolchain::copy(reductionVars, std::back_inserter(result.reductionVars));
        toolchain::copy(reduceVarByRef, std::back_inserter(result.reductionByref));
        toolchain::copy(reductionDeclSymbols,
                   std::back_inserter(result.reductionSyms));
        toolchain::copy(reductionSyms, std::back_inserter(outReductionSyms));
      });
}
...
bool ClauseProcessor::processTaskReduction(
  ...
  return findRepeatableClause<omp::clause::TaskReduction>(
      [&](const omp::clause::TaskReduction &clause, const parser::CharBlock &) {
        toolchain::SmallVector<mlir::Value> taskReductionVars;
        toolchain::SmallVector<bool> taskReduceVarByRef;
        toolchain::SmallVector<mlir::Attribute> taskReductionDeclSymbols;
        ...
        if (!rp.processReductionArguments<mlir::omp::DeclareReductionOp>(
          ...
          TODO(currentLocation, "Lowering unrecognised reduction type");
        // Copy local lists into the output.
        toolchain::copy(taskReductionVars,
                   std::back_inserter(result.taskReductionVars));
        toolchain::copy(taskReduceVarByRef,
                   std::back_inserter(result.taskReductionByref));
        toolchain::copy(taskReductionDeclSymbols,
                   std::back_inserter(result.taskReductionSyms));
        toolchain::copy(taskReductionSyms, std::back_inserter(outReductionSyms));
      });
}
