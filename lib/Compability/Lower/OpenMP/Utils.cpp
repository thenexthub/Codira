...
toolchain::cl::opt<bool> treatIndexAsSection(
    "openmp-treat-index-as-section",
    toolchain::cl::desc("In the OpenMP data clauses treat `a(N)` as `a(N:N)`."),
...
int64_t getCollapseValue(const List<Clause> &clauses) {
  auto iter = toolchain::find_if(clauses, [](const Clause &clause) {
    return clause.id == toolchain::omp::Clause::OMPC_collapse;
  });
  if (iter != clauses.end()) {
...
createMapInfoOp(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  if (auto boxTy = toolchain::dyn_cast<fir::BaseBoxType>(baseAddr.getType())) {
    baseAddr = fir::BoxAddrOp::create(builder, loc, baseAddr);
    retTy = baseAddr.getType();
  }

  mlir::TypeAttr varType = mlir::TypeAttr::get(
      toolchain::cast<mlir::omp::PointerLikeType>(retTy).getElementType());

  // For types with unknown extents such as <2x?xi32> we discard the incomplete
  // type info and only retain the base type. The correct dimensions are later
  // recovered through the bounds info.
  if (auto seqType = toolchain::dyn_cast<fir::SequenceType>(varType.getValue()))
...
omp::ObjectList gatherObjectsOf(omp::Object derivedTypeMember,
  ...
  return omp::ObjectList{toolchain::reverse(objList)};
}
...
static void generateArrayIndices(lower::AbstractConverter &converter,
                                 ...
                                 toolchain::SmallVectorImpl<mlir::Value> &indices,
                                 omp::Object object) {
  ...
  for (auto v : arr->subscript()) {
    ...
    auto expr = std::get<language::Compability::evaluate::IndirectSubscriptIntegerExpr>(v.u);
  ...
  for (size_t i = 0; i < objectList.size(); ++i) {
    ...
    if (fir::SequenceType arrType = mlir::dyn_cast<fir::SequenceType>(
      ...
      if (isArrayExprWithSubscript(objectList[i])) {
        toolchain::SmallVector<mlir::Value> subscriptIndices;
    ...
    if (fir::RecordType recordType = mlir::dyn_cast<fir::RecordType>(
      ...
      curValue = fir::CoordinateOp::create(
          ...
          curValue, toolchain::SmallVector<fir::IntOrValue, 1>{idxConst});
      ...
      toolchain::SmallVector<int64_t> interimIndices(
      ...
      if (!parentMemberIndices.isDuplicateMemberMapInfo(interimIndices)) {
        ...
        toolchain::SmallVector<mlir::Value> interimBounds;
        ...
        toolchain::omp::OpenMPOffloadMappingFlags interimMapType = mapTypeBits;
        interimMapType &= ~toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TO;
        interimMapType &= ~toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_FROM;
        interimMapType &=
...
getComponentPlacementInParent(const semantics::Symbol *componentSym) {
  ...
  for (auto [placement, name] : toolchain::enumerate(derived->componentNames()))
...
void generateMemberPlacementIndices(const Object &object,
  ...
  indices = toolchain::SmallVector<int64_t>{toolchain::reverse(indices)};
}
...
void insertChildMapInfoIntoParent(
  ...
  for (auto indices : parentMemberIndices) {
    auto *parentIter =
        toolchain::find_if(mapSyms, [&indices](const semantics::Symbol *v) {
          return v == indices.first.sym();
        });
    if (parentIter != mapSyms.end()) {
      auto mapOp = toolchain::cast<mlir::omp::MapInfoOp>(
      ...
      toolchain::SmallVector<mlir::Value> members;
