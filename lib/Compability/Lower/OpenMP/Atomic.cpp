...
static toolchain::cl::opt<bool> DumpAtomicAnalysis("fdebug-dump-atomic-analysis");

using namespace language::Compability;

// Don't import the entire language::Compability::lower.
namespace omp {
...
dumpAtomicAnalysis(const parser::OpenMPAtomicConstruct::Analysis &analysis) {
  ...
  toolchain::errs() << "Analysis {\n";
  toolchain::errs() << "  atom: " << atom.AsFortran() << "\n";
  toolchain::errs() << "  cond: " << exprStr(analysis.cond) << "\n";
  toolchain::errs() << "  op0 {\n";
  toolchain::errs() << "    what: " << whatStr(analysis.op0.what) << "\n";
  toolchain::errs() << "    assign: " << assignStr(analysis.op0.assign) << "\n";
  toolchain::errs() << "  }\n";
  toolchain::errs() << "  op1 {\n";
  toolchain::errs() << "    what: " << whatStr(analysis.op1.what) << "\n";
  toolchain::errs() << "    assign: " << assignStr(analysis.op1.assign) << "\n";
  toolchain::errs() << "  }\n";
  toolchain::errs() << "}\n";
}
...
static mlir::IntegerAttr getAtomicHint(lower::AbstractConverter &converter,
  ...
  for (const omp::Clause &clause : clauses) {
    if (clause.id != toolchain::omp::Clause::OMPC_hint)
      continue;
...
getMemoryOrderKind(toolchain::omp::Clause clauseId) {
  switch (clauseId) {
...
void language::Compability::lower::omp::lowerAtomic(
