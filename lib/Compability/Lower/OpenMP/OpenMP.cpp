...
using namespace language::Compability::common::openmp;
...
public:
  ...
  /// bindOperands().
  void collectValues(toolchain::SmallVectorImpl<mlir::Value> &vars) const {
  ...
  void bindOperands(toolchain::ArrayRef<mlir::BlockArgument> args) {
    assert(args.size() ==
  ...
  bool apply(mlir::omp::LoopNestOperands &clauseOps,
             toolchain::SmallVectorImpl<const semantics::Symbol *> &ivOut) {
...
static void bindEntryBlockArgs(lower::AbstractConverter &converter,
  ...
  auto bindMapLike =
      [&bindSingleMapLike](toolchain::ArrayRef<const semantics::Symbol *> syms,
                           toolchain::ArrayRef<mlir::BlockArgument> args) {
        // Structure component symbols don't have bindings, and can only be
        // explicitly mapped individually. If a member is captured implicitly
        // we map the entirety of the derived type when we find its symbol.
        toolchain::SmallVector<const semantics::Symbol *> processedSyms;
        toolchain::copy_if(syms, std::back_inserter(processedSyms),
                      [](auto *sym) { return !sym->owner().IsDerivedType(); });

        for (auto [sym, arg] : toolchain::zip_equal(processedSyms, args))
          bindSingleMapLike(*sym, arg);
      };

  auto bindPrivateLike = [&converter, &firOpBuilder](
                             toolchain::ArrayRef<const semantics::Symbol *> syms,
    ...
    for (auto *sym : syms) {
      if (const auto *commonDet =
        ...
        toolchain::transform(commonDet->objects(), std::back_inserter(processedSyms),
    ...
    for (auto [sym, var, arg] : toolchain::zip_equal(processedSyms, vars, args))
      converter.bindSymbol(
          *sym,
...
static void
extractMappedBaseValues(toolchain::ArrayRef<mlir::Value> vars,
                        toolchain::SmallVectorImpl<mlir::Value> &baseOps) {
  toolchain::transform(vars, std::back_inserter(baseOps), [](mlir::Value map) {
...
static void processHostEvalClauses(lower::AbstractConverter &converter,
  ...
  auto extractOnlyOmpNestedDir = [](lower::pft::Evaluation &parent)
      -> std::optional<toolchain::omp::Directive> {
    if (!parent.hasNestedEvaluations())
      return std::nullopt;

    toolchain::omp::Directive dir;
  ...
  processEval = [&](lower::pft::Evaluation &eval, const List<Clause> &clauses) {
    ...
    auto processSingleNestedIf = [&](toolchain::function_ref<bool(Directive)> test) {
  ...
  const auto *ompEval = eval.getIf<parser::OpenMPConstruct>();
  assert(ompEval &&
         toolchain::omp::allTargetSet.test(
...
static fir::GlobalOp globalInitialization(lower::AbstractConverter &converter,
  ...
  return language::Compability::lower::defineGlobal(converter, var, globalName, linkage);
}
...
static void threadPrivatizeVars(lower::AbstractConverter &converter,
  ...
  toolchain::SetVector<const semantics::Symbol *> threadprivateSyms;
  ...
  toolchain::SetVector<const semantics::Symbol *> commonSyms;
...
static void promoteNonCPtrUseDevicePtrArgsToUseDeviceAddr(
    toolchain::SmallVectorImpl<mlir::Value> &useDeviceAddrVars,
    toolchain::SmallVectorImpl<const semantics::Symbol *> &useDeviceAddrSyms,
    toolchain::SmallVectorImpl<mlir::Value> &useDevicePtrVars,
    toolchain::SmallVectorImpl<const semantics::Symbol *> &useDevicePtrSyms) {
...
static void collectDeferredDeclareTargets(
    ...
    lower::pft::Evaluation &eval,
    const parser::OpenMPDeclareTargetConstruct &declareTargetConstruct,
    toolchain::SmallVectorImpl<lower::OMPDeferredDeclareTargetInfo>
        &deferredDeclareTarget) {
  mlir::omp::DeclareTargetOperands clauseOps;
  toolchain::SmallVector<DeclareTargetCaptureInfo> symbolAndClause;
...
static void genLoopVars(
    ...
    mlir::Location &loc, toolchain::ArrayRef<const semantics::Symbol *> args,
    toolchain::ArrayRef<
  ...
  toolchain::SmallVector<mlir::Type> tiv(args.size(), loopVarType);
  toolchain::SmallVector<mlir::Location> locs(args.size(), loc);
  ...
  for (auto [argGeneratingOp, blockArgs] : wrapperArgs) {
    ...
    for (const auto [arg, var] : toolchain::zip_equal(
  ...
  for (auto [argIndex, argSymbol] : toolchain::enumerate(args)) {
...
getDefaultmapIfPresent(const DefaultMapsTy &defaultMaps, mlir::Type varType) {
  ...
  if (toolchain::is_contained(defaultMaps, DefMap::VariableCategory::All))
  ...
  if ((fir::isa_trivial(varType) || fir::isa_char(varType) ||
      ...
      toolchain::is_contained(defaultMaps, DefMap::VariableCategory::Scalar))
    return defaultMaps.at(DefMap::VariableCategory::Scalar);

  if (fir::isPointerType(varType) &&
      toolchain::is_contained(defaultMaps, DefMap::VariableCategory::Pointer))
    return defaultMaps.at(DefMap::VariableCategory::Pointer);

  if (fir::isAllocatableType(varType) &&
      toolchain::is_contained(defaultMaps, DefMap::VariableCategory::Allocatable))
    return defaultMaps.at(DefMap::VariableCategory::Allocatable);

  if (fir::isa_aggregate(varType) &&
      toolchain::is_contained(defaultMaps, DefMap::VariableCategory::Aggregate))
    return defaultMaps.at(DefMap::VariableCategory::Aggregate);

  return DefMap::ImplicitBehavior::Default;
}

static std::pair<toolchain::omp::OpenMPOffloadMappingFlags,
...
getImplicitMapTypeAndKind(fir::FirOpBuilder &firOpBuilder,
  ...
  toolchain::omp::OpenMPOffloadMappingFlags mapFlag =
      toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_IMPLICIT;
  ...
  if (implicitBehaviour == DefMap::ImplicitBehavior::Default) {
    ...
    auto declareTargetOp =
        toolchain::dyn_cast_if_present<mlir::omp::DeclareTargetInterface>(op);
    if (declareTargetOp && declareTargetOp.isDeclareTarget()) {
      if (declareTargetOp.getDeclareTargetCaptureClause() ==
              mlir::omp::DeclareTargetCaptureClause::link &&
          declareTargetOp.getDeclareTargetDeviceType() !=
              mlir::omp::DeclareTargetDeviceType::nohost) {
        mapFlag |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TO;
        mapFlag |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_FROM;
      }
  ...
  case DefMap::ImplicitBehavior::Alloc:
    return std::make_pair(toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_NONE,
                          mlir::omp::VariableCaptureKind::ByRef);
    break;
  ...
  case DefMap::ImplicitBehavior::From:
    return std::make_pair(mapFlag |=
                          toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_FROM,
                          mlir::omp::VariableCaptureKind::ByRef);
    break;
  case DefMap::ImplicitBehavior::Present:
    return std::make_pair(mapFlag |=
                          toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_PRESENT,
                          mlir::omp::VariableCaptureKind::ByRef);
    break;
...
markDeclareTarget(mlir::Operation *op, lower::AbstractConverter &converter,
  ...
  auto declareTargetOp = toolchain::dyn_cast<mlir::omp::DeclareTargetInterface>(op);
  if (!declareTargetOp)
    fir::emitFatalError(
...
struct OpWithBodyGenInfo {
  ...
  using GenOMPRegionEntryCBFn =
      std::function<toolchain::SmallVector<const semantics::Symbol *>(
          mlir::Operation *)>;

  OpWithBodyGenInfo(lower::AbstractConverter &converter,
                    lower::SymMap &symTable,
                    semantics::SemanticsContext &semaCtx, mlir::Location loc,
                    lower::pft::Evaluation &eval, toolchain::omp::Directive dir)
...
static void createBodyOfOp(mlir::Operation &op, const OpWithBodyGenInfo &info,
  ...
  auto regionArgs = [&]() -> toolchain::SmallVector<const semantics::Symbol *> {
    ...
    if (info.blockArgs) {
      ...
      bindEntryBlockArgs(info.converter,
                         toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(op),
                         *info.blockArgs);
      return toolchain::to_vector(info.blockArgs->getSyms());
    }
  ...
  bool isLoop = toolchain::omp::getDirectiveAssociation(info.dir) ==
                toolchain::omp::Association::Loop;
  ...
  if (privatize && !info.dsp) {
    tempDsp.emplace(info.converter, info.semaCtx, *info.clauses, info.eval,
                    language::Compability::lower::omp::isLastItemInQueue(item, queue),
                    /*useDelayedPrivatization=*/false, info.symTable);
    tempDsp->processStep1();
  }

  if (info.dir == toolchain::omp::Directive::OMPD_parallel) {
  ...
  auto getUniqueExit = [&](mlir::Region &region) -> mlir::Block * {
    ...
    toolchain::SmallVector<mlir::Block *> exits;
  ...
  if (auto *exitBlock = getUniqueExit(op.getRegion(0))) {
    ...
    if (privatize) {
      ...
      if (auto loopNest = toolchain::dyn_cast<mlir::omp::LoopNestOp>(op)) {
        toolchain::SmallVector<mlir::omp::LoopWrapperInterface> wrappers;
...
static void genIntermediateCommonBlockAccessors(
    ...
    toolchain::ArrayRef<mlir::BlockArgument> mapBlockArgs,
    toolchain::ArrayRef<const language::Compability::semantics::Symbol *> mapSyms) {
  ...
  for (auto [mapSym, mapArg] : toolchain::zip_first(mapSyms, mapBlockArgs)) {
    auto *details = mapSym->detailsIf<language::Compability::semantics::CommonBlockDetails>();
    if (!details)
      continue;

    for (auto obj : details->objects()) {
      auto targetCBMemberBind = language::Compability::lower::genCommonBlockMember(
...
static void genBodyOfTargetOp(
  ...
  fir::FirOpBuilder &firOpBuilder = converter.getFirOpBuilder();
  auto argIface = toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(*targetOp);
  ...
  toolchain::SetVector<mlir::Value> valuesDefinedAbove;
    ...
    for (mlir::Value val : valuesDefinedAbove) {
      ...
      if (valOp && mlir::isMemoryEffectFree(valOp) &&
        ...
        mlir::Value mapOp = createMapInfoOp(
            ...
            /*members=*/toolchain::SmallVector<mlir::Value>{},
...
static void genCancelClauses(lower::AbstractConverter &converter,
  ...
  cp.processIf(toolchain::omp::Directive::OMPD_cancel, clauseOps);
}
...
static void genCriticalDeclareClauses(
    lower::AbstractConverter &converter, semantics::SemanticsContext &semaCtx,
    const List<Clause> &clauses, mlir::Location loc,
    mlir::omp::CriticalDeclareOperands &clauseOps, toolchain::StringRef name) {
  ClauseProcessor cp(converter, semaCtx, clauses);
...
static void genLoopClauses(
    ...
    toolchain::SmallVectorImpl<const semantics::Symbol *> &reductionSyms) {
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processBind(clauseOps);
  cp.processOrder(clauseOps);
  cp.processReduction(loc, clauseOps, reductionSyms);
  cp.processTODO<clause::Lastprivate>(loc, toolchain::omp::Directive::OMPD_loop);
}
...
genOrderedRegionClauses(lower::AbstractConverter &converter,
  ...
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processTODO<clause::Simd>(loc, toolchain::omp::Directive::OMPD_ordered);
}
...
static void genParallelClauses(
  ...
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processAllocate(clauseOps);
  cp.processIf(toolchain::omp::Directive::OMPD_parallel, clauseOps);
...
static void genSimdClauses(
  ...
  cp.processIf(toolchain::omp::Directive::OMPD_simd, clauseOps);
  ...
  cp.processTODO<clause::Linear>(loc, toolchain::omp::Directive::OMPD_simd);
}
...
static void genTargetClauses(
    ...
    toolchain::SmallVectorImpl<const semantics::Symbol *> &hasDeviceAddrSyms,
    toolchain::SmallVectorImpl<const semantics::Symbol *> &isDevicePtrSyms,
  ...
  cp.processIf(toolchain::omp::Directive::OMPD_target, clauseOps);
  cp.processIsDevicePtr(clauseOps, isDevicePtrSyms);
  cp.processMap(loc, stmtCtx, clauseOps, toolchain::omp::Directive::OMPD_unknown,
                &mapSyms);
  cp.processNowait(clauseOps);
  cp.processThreadLimit(stmtCtx, clauseOps);

  cp.processTODO<clause::Allocate, clause::InReduction, clause::UsesAllocators>(
      loc, toolchain::omp::Directive::OMPD_target);
...
static void genTargetDataClauses(
  ...
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processDevice(stmtCtx, clauseOps);
  cp.processIf(toolchain::omp::Directive::OMPD_target_data, clauseOps);
...
static void genTargetEnterExitUpdateDataClauses(
  ...
  if (directive == toolchain::omp::Directive::OMPD_target_update)
...
static void genTaskClauses(
    ...
    toolchain::SmallVectorImpl<const semantics::Symbol *> &inReductionSyms) {
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processAllocate(clauseOps);
  cp.processDepend(symTable, stmtCtx, clauseOps);
  cp.processFinal(stmtCtx, clauseOps);
  cp.processIf(toolchain::omp::Directive::OMPD_task, clauseOps);
  ...
  cp.processTODO<clause::Affinity>(loc, toolchain::omp::Directive::OMPD_task);
}
...
static void genTaskgroupClauses(
    ...
    toolchain::SmallVectorImpl<const semantics::Symbol *> &taskReductionSyms) {
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processAllocate(clauseOps);
...
static void genTaskloopClauses(lower::AbstractConverter &converter,
  ...
  cp.processTODO<clause::Allocate, clause::Collapse, clause::Default,
                 ...
                 clause::Untied>(loc, toolchain::omp::Directive::OMPD_taskloop);
}
...
static void genTaskwaitClauses(lower::AbstractConverter &converter,
  ...
  cp.processTODO<clause::Depend, clause::Nowait>(
      loc, toolchain::omp::Directive::OMPD_taskwait);
}
...
static void genTeamsClauses(
  ...
  ClauseProcessor cp(converter, semaCtx, clauses);
  cp.processAllocate(clauseOps);
  cp.processIf(toolchain::omp::Directive::OMPD_teams, clauseOps);
...
static void genWsloopClauses(
  ...
  cp.processTODO<clause::Allocate, clause::Linear>(
      loc, toolchain::omp::Directive::OMPD_do);
}
...
genCriticalOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  return genOpWithBody<mlir::omp::CriticalOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_critical),
      queue, item, nameAttr);
}
...
genFlushOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
           ...
           const ConstructQueue &queue, ConstructQueue::const_iterator item) {
  toolchain::SmallVector<mlir::Value> operandRange;
...
static mlir::omp::LoopNestOp genLoopNestOp(
    ...
    toolchain::ArrayRef<const semantics::Symbol *> iv,
    toolchain::ArrayRef<
        std::pair<mlir::omp::BlockArgOpenMPOpInterface, const EntryBlockArgs &>>
        wrapperArgs,
    toolchain::omp::Directive directive, DataSharingProcessor &dsp) {
  auto ivCallback = [&](mlir::Operation *op) {
    genLoopVars(op, converter, loc, iv, wrapperArgs);
    return toolchain::SmallVector<const semantics::Symbol *>(iv);
  };
...
genLoopOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  toolchain::SmallVector<const semantics::Symbol *> loopReductionSyms;
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, item,
                ...
                toolchain::omp::Directive::OMPD_loop, dsp);
  return loopOp;
}
...
genCanonicalLoopOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
                   ...
                   const ConstructQueue &queue,
                   ConstructQueue::const_iterator item,
                   toolchain::ArrayRef<const semantics::Symbol *> ivs,
                   toolchain::omp::Directive directive) {
  fir::FirOpBuilder &firOpBuilder = converter.getFirOpBuilder();
  ...
  auto ivCallback = [&](mlir::Operation *op)
      -> toolchain::SmallVector<const language::Compability::semantics::Symbol *> {
...
static void genUnrollOp(language::Compability::lower::AbstractConverter &converter,
                        language::Compability::lower::SymMap &symTable,
                        lower::StatementContext &stmtCtx,
  ...
  cp.processTODO<clause::Partial, clause::Full>(
      loc, toolchain::omp::Directive::OMPD_unroll);

  // Emit the associated loop
  auto canonLoop =
      genCanonicalLoopOp(converter, symTable, semaCtx, eval, loc, queue, item,
                         iv, toolchain::omp::Directive::OMPD_unroll);
...
genMaskedOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  return genOpWithBody<mlir::omp::MaskedOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_masked),
      queue, item, clauseOps);
}
...
genMasterOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  return genOpWithBody<mlir::omp::MasterOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_master),
      queue, item);
}
...
genOrderedRegionOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  return genOpWithBody<mlir::omp::OrderedRegionOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_ordered),
      queue, item, clauseOps);
}
...
genParallelOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  OpWithBodyGenInfo genInfo =
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_parallel)
          .setClauses(&item->clauses)
...
genSectionsOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  mlir::omp::SectionsOperands clauseOps;
  toolchain::SmallVector<const semantics::Symbol *> reductionSyms;
  ...
  for (const Clause &clause : item->clauses) {
    if (clause.id == toolchain::omp::Clause::OMPC_lastprivate) {
      ...
      switch (clause.id) {
      case toolchain::omp::Clause::OMPC_firstprivate:
      case toolchain::omp::Clause::OMPC_private:
      case toolchain::omp::Clause::OMPC_shared:
        break;
      default:
  ...
  for (auto [construct, nestedEval] :
       toolchain::zip(sectionBlocks, eval.getNestedEvaluations())) {
    ...
    ConstructQueue sectionQueue{buildConstructQueue(
        ...
        sectionConstruct->source, toolchain::omp::Directive::OMPD_section, {})};

    builder.setInsertionPoint(terminator);
    genOpWithBody<mlir::omp::SectionOp>(
        OpWithBodyGenInfo(converter, symTable, semaCtx, loc, nestedEval,
                          toolchain::omp::Directive::OMPD_section)
  ...
  if (!lastprivates.empty()) {
    ...
    auto lastSectionOp = toolchain::find_if(
        toolchain::reverse(body.getOperations()), [](const mlir::Operation &op) {
          return toolchain::isa<mlir::omp::SectionOp>(op);
        });
...
genSingleOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  return genOpWithBody<mlir::omp::SingleOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_single)
          .setClauses(&item->clauses),
      queue, item, clauseOps);
}
...
static bool isDuplicateMappedSymbol(
    const semantics::Symbol &sym,
    const toolchain::SetVector<const semantics::Symbol *> &privatizedSyms,
    const toolchain::SmallVectorImpl<const semantics::Symbol *> &hasDevSyms,
    const toolchain::SmallVectorImpl<const semantics::Symbol *> &mappedSyms) {
  toolchain::SmallVector<const semantics::Symbol *> concatSyms;
...
genTargetOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  fir::FirOpBuilder &firOpBuilder = converter.getFirOpBuilder();
  bool isTargetDevice =
      toolchain::cast<mlir::omp::OffloadModuleInterface>(*converter.getModuleOp())
          .getIsTargetDevice();
  ...
  DefaultMapsTy defaultMaps;
  toolchain::SmallVector<const semantics::Symbol *> mapSyms, isDevicePtrSyms,
      hasDeviceAddrSyms;
  ...
  auto captureImplicitMap = [&](const semantics::Symbol &sym) {
    ...
    if (const language::Compability::semantics::Symbol *common =
            language::Compability::semantics::FindCommonBlockContaining(sym.GetUltimate()))
      if (toolchain::is_contained(mapSyms, common))
        return;
    ...
    if (!isDuplicateMappedSymbol(sym, dsp.getAllSymbolsToPrivatize(),
      ...
      if (sym.GetType()->category() == semantics::DeclTypeSpec::TypeDerived) {
        ...
        std::string mapperIdName =
            typeSpec.name().ToString() + toolchain::omp::OmpDefaultMapperName;
      ...
      fir::factory::AddrAndBoundsInfo info =
          language::Compability::lower::getDataOperandBaseAddr(
      ...
      std::pair<toolchain::omp::OpenMPOffloadMappingFlags,
                mlir::omp::VariableCaptureKind>
  ...
  toolchain::SmallVector<mlir::Value> hasDeviceAddrBaseValues, mapBaseValues;
...
static mlir::omp::TargetDataOp genTargetDataOp(
  ...
  toolchain::SmallVector<const semantics::Symbol *> useDeviceAddrSyms,
      useDevicePtrSyms;
  ...
  toolchain::SmallVector<mlir::Value> useDeviceAddrBaseValues,
...
static OpTy genTargetEnterExitUpdateDataOp(
  ...
  [[maybe_unused]] toolchain::omp::Directive directive;
  if constexpr (std::is_same_v<OpTy, mlir::omp::TargetEnterDataOp>) {
    directive = toolchain::omp::Directive::OMPD_target_enter_data;
  } else if constexpr (std::is_same_v<OpTy, mlir::omp::TargetExitDataOp>) {
    directive = toolchain::omp::Directive::OMPD_target_exit_data;
  } else if constexpr (std::is_same_v<OpTy, mlir::omp::TargetUpdateOp>) {
    directive = toolchain::omp::Directive::OMPD_target_update;
  } else {
    toolchain_unreachable("Unexpected TARGET DATA construct");
  }
...
genTaskOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
          ...
          ConstructQueue::const_iterator item) {
  mlir::omp::TaskOperands clauseOps;
  toolchain::SmallVector<const semantics::Symbol *> inReductionSyms;
  ...
  if (!enableDelayedPrivatization)
    return genOpWithBody<mlir::omp::TaskOp>(
        OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                          toolchain::omp::Directive::OMPD_task)
            .setClauses(&item->clauses),
        queue, item, clauseOps);
...
genTaskgroupOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
               ...
               ConstructQueue::const_iterator item) {
  mlir::omp::TaskgroupOperands clauseOps;
  toolchain::SmallVector<const semantics::Symbol *> taskReductionSyms;
...
static mlir::omp::WorkshareOp genWorkshareOp(
  ...
  return genOpWithBody<mlir::omp::WorkshareOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_workshare)
          .setClauses(&item->clauses),
      queue, item, clauseOps);
}
...
genTeamsOp(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  return genOpWithBody<mlir::omp::TeamsOp>(
      OpWithBodyGenInfo(converter, symTable, semaCtx, loc, eval,
                        toolchain::omp::Directive::OMPD_teams)
          .setClauses(&item->clauses)
          .setEntryBlockArgs(&args),
      queue, item, clauseOps);
}
...
static mlir::omp::DistributeOp genStandaloneDistribute(
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, item,
                ...
                toolchain::omp::Directive::OMPD_distribute, dsp);
  return distributeOp;
}
...
static mlir::omp::WsloopOp genStandaloneDo(
  ...
  toolchain::SmallVector<const semantics::Symbol *> wsloopReductionSyms;
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, item,
                ...
                toolchain::omp::Directive::OMPD_do, dsp);
  return wsloopOp;
}
...
static mlir::omp::ParallelOp genStandaloneParallel(
  ...
  toolchain::SmallVector<const semantics::Symbol *> parallelReductionSyms;
...
genStandaloneSimd(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  toolchain::SmallVector<const semantics::Symbol *> simdReductionSyms;
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, item,
                ...
                toolchain::omp::Directive::OMPD_simd, dsp);
  return simdOp;
}
...
static mlir::omp::TaskloopOp genStandaloneTaskloop(
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, item,
                ...
                toolchain::omp::Directive::OMPD_taskloop, dsp);
  return taskLoopOp;
}
...
static mlir::omp::DistributeOp genCompositeDistributeParallelDo(
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, doItem,
                ...
                toolchain::omp::Directive::OMPD_distribute_parallel_do, dsp);
  return distributeOp;
}
...
static mlir::omp::DistributeOp genCompositeDistributeParallelDoSimd(
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, simdItem,
                ...
                {{distributeOp, distributeArgs},
                 {wsloopOp, wsloopArgs},
                 {simdOp, simdArgs}},
                toolchain::omp::Directive::OMPD_distribute_parallel_do_simd,
                simdItemDSP);
  return distributeOp;
}
...
static mlir::omp::DistributeOp genCompositeDistributeSimd(
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, simdItem,
                ...
                toolchain::omp::Directive::OMPD_distribute_simd, simdItemDSP);
  return distributeOp;
}
...
static mlir::omp::WsloopOp genCompositeDoSimd(
  ...
  genLoopNestOp(converter, symTable, semaCtx, eval, loc, queue, simdItem,
                ...
                toolchain::omp::Directive::OMPD_do_simd, simdItemDSP);
  return wsloopOp;
}
...
static void genOMPDispatch(lower::AbstractConverter &converter,
  ...
  bool loopLeaf = toolchain::omp::getDirectiveAssociation(item->id) ==
                  toolchain::omp::Association::Loop;
  if (loopLeaf) {
    symTable.pushScope();
  ...
  case toolchain::omp::Directive::OMPD_target:
    newOp = genTargetOp(converter, symTable, stmtCtx, semaCtx, eval, loc, queue,
                        item);
    break;
  case toolchain::omp::Directive::OMPD_target_data:
  ...
  case toolchain::omp::Directive::OMPD_taskgroup:
    ...
    break;
  case toolchain::omp::Directive::OMPD_taskloop:
    newOp = genStandaloneTaskloop(converter, symTable, stmtCtx, semaCtx, eval,
                                  loc, queue, item);
    break;
  case toolchain::omp::Directive::OMPD_taskwait:
    newOp = genTaskwaitOp(converter, symTable, semaCtx, eval, loc, queue, item);
    break;
  case toolchain::omp::Directive::OMPD_taskyield:
    newOp =
        genTaskyieldOp(converter, symTable, semaCtx, eval, loc, queue, item);
    break;
  case toolchain::omp::Directive::OMPD_teams:
    newOp = genTeamsOp(converter, symTable, stmtCtx, semaCtx, eval, loc, queue,
                       item);
    break;
  case toolchain::omp::Directive::OMPD_tile: {
    unsigned version = semaCtx.langOptions().OpenMPVersion;
    if (!semaCtx.langOptions().OpenMPSimd)
      TODO(loc, "Unhandled loop directive (" +
                    toolchain::omp::getOpenMPDirectiveName(dir, version) + ")");
    break;
  }
  case toolchain::omp::Directive::OMPD_unroll:
    genUnrollOp(converter, symTable, stmtCtx, semaCtx, eval, loc, queue, item);
    break;
  // case toolchain::omp::Directive::OMPD_workdistribute:
  ...
  default:
    ...
    assert(!toolchain::omp::isLeafConstruct(dir) &&
...
static void genOMP(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  if (directive.v == toolchain::omp::Directive::OMPD_ordered) {
...
static void genOMP(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  ConstructQueue queue{buildConstructQueue(
      ...
      toolchain::omp::Directive::OMPD_flush, clauses)};
...
static void genOMP(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  ConstructQueue queue{buildConstructQueue(
      ...
      cancelConstruct.source, toolchain::omp::Directive::OMPD_cancel, clauses)};
...
static void genOMP(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  ConstructQueue queue{
      buildConstructQueue(converter.getFirOpBuilder().getModule(), semaCtx,
                          ...
                          toolchain::omp::Directive::OMPD_cancel, clauses)};
...
static void genOMP(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  toolchain::omp::Directive directive = beginSpec.DirId();
  assert(toolchain::omp::blockConstructSet.test(directive) &&
  ...
  for (const Clause &clause : clauses) {
    ...
    if (!std::holds_alternative<clause::Affinity>(clause.u) &&
      ...
      std::string name =
          parser::ToUpperCaseLetters(toolchain::omp::getOpenMPClauseName(clause.id));
      if (!semaCtx.langOptions().OpenMPSimd)
...
static void genOMP(lower::AbstractConverter &converter, lower::SymMap &symTable,
  ...
  ConstructQueue queue{buildConstructQueue(
      ...
      toolchain::omp::Directive::OMPD_critical, clauses)};
  ...
  if (optLoopCons.has_value()) {
    if (auto *ompNestedLoopCons{
            std::get_if<common::Indirection<parser::OpenMPLoopConstruct>>(
                &*optLoopCons)}) {
      toolchain::omp::Directive nestedDirective =
      ...
      default: {
        ...
        TODO(currentLocation,
             "Applying a loop-associated on the loop generated by the " +
                 toolchain::omp::getOpenMPDirectiveName(nestedDirective, version) +
                 " construct");
      }
      }
    }
...
mlir::Operation *language::Compability::lower::genOpenMPTerminator(fir::FirOpBuilder &builder,
                                                     mlir::Operation *op,
                                                     mlir::Location loc) {
...
void language::Compability::lower::genOpenMPConstruct(lower::AbstractConverter &converter,
                                        lower::SymMap &symTable,
                                        semantics::SemanticsContext &semaCtx,
                                        lower::pft::Evaluation &eval,
...
void language::Compability::lower::genOpenMPDeclarativeConstruct(
    lower::AbstractConverter &converter, lower::SymMap &symTable,
    semantics::SemanticsContext &semaCtx, lower::pft::Evaluation &eval,
...
void language::Compability::lower::genOpenMPSymbolProperties(
...
int64_t
language::Compability::lower::getCollapseValue(const parser::OmpClauseList &clauseList) {
...
void language::Compability::lower::genThreadprivateOp(lower::AbstractConverter &converter,
                                        const lower::pft::Variable &var) {
  fir::FirOpBuilder &firOpBuilder = converter.getFirOpBuilder();
  mlir::Location currentLocation = converter.getCurrentLocation();
...
// generation.
void language::Compability::lower::genDeclareTargetIntGlobal(
    lower::AbstractConverter &converter, const lower::pft::Variable &var) {
  if (!var.isGlobal()) {
...
bool language::Compability::lower::isOpenMPTargetConstruct(
    const parser::OpenMPConstruct &omp) {
  toolchain::omp::Directive dir = toolchain::omp::Directive::OMPD_unknown;
  ...
  return toolchain::omp::allTargetSet.test(dir);
}

void language::Compability::lower::gatherOpenMPDeferredDeclareTargets(
    lower::AbstractConverter &converter, semantics::SemanticsContext &semaCtx,
    lower::pft::Evaluation &eval,
...
bool language::Compability::lower::isOpenMPDeviceDeclareTarget(
    lower::AbstractConverter &converter, semantics::SemanticsContext &semaCtx,
    lower::pft::Evaluation &eval,
    const parser::OpenMPDeclarativeConstruct &ompDecl) {
...
// on the module.
bool language::Compability::lower::markOpenMPDeferredDeclareTargetFunctions(
    mlir::Operation *mod,
    toolchain::SmallVectorImpl<OMPDeferredDeclareTargetInfo> &deferredDeclareTargets,
    AbstractConverter &converter) {
  bool deviceCodeFound = false;
  auto modOp = toolchain::cast<mlir::ModuleOp>(mod);
...
void language::Compability::lower::genOpenMPRequires(mlir::Operation *mod,
                                       const semantics::Symbol *symbol) {
  using MlirRequires = mlir::omp::ClauseRequires;
  using SemaRequires = semantics::WithOmpDeclarative::RequiresFlag;

  if (auto offloadMod =
          toolchain::dyn_cast<mlir::omp::OffloadModuleInterface>(mod)) {
