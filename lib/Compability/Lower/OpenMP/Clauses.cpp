...
struct SymbolAndDesignatorExtractor {
  ...
  static SymbolWithDesignator visit(T &&) {
    ...
    // toolchain::errs() << "NULL: " << __PRETTY_FUNCTION__ << '\n';
  ...
  static SymbolWithDesignator visit(const evaluate::Expr<T> &e) {
    return language::Compability::common::visit([](auto &&s) { return visit(s); }, e.u);
  }
  ...
  static void verify(const SymbolWithDesignator &sd) {
    ...
    if (maybeRef) {
      ...
      toolchain_unreachable("Expecting designator for given symbol");
    } else {
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
      maybeDsg->dump();
#endif
      toolchain_unreachable("Expecting DataRef designator");
    }
  }
};
    ...
    return common::visit(
        common::visitors{
            [&](const parser::OmpLocator &locator) -> Object {
              ...
              toolchain_unreachable("Expecting object");
            },
            [](auto &&s) -> Object { //
              toolchain_unreachable("Expecting object");
            },
...
std::optional<Object> getBaseObject(const Object &object,
  ...
  if (std::get_if<evaluate::SymbolRef>(&ref.u)) {
    ...
    toolchain_unreachable("Coarray reference not supported at the moment");
  }
  return std::nullopt;
}
...
Iterator makeIterator(const parser::OmpIterator &inp,
  ...
  Iterator iterator;
  for (auto &&spec : inp.v)
    toolchain::append_range(iterator, makeIteratorSpecifiers(spec, semaCtx));
  return iterator;
}
...
makeProcedureDesignator(const parser::ProcedureDesignator &inp,
                        semantics::SemanticsContext &semaCtx) {
  return ProcedureDesignator{language::Compability::common::visit(
      common::visitors{
...
clause::DependenceType makeDepType(const parser::OmpDependenceType &inp) {
  ...
  toolchain_unreachable("Unexpected dependence type");
}
...
clause::DependenceType makeDepType(const parser::OmpTaskDependenceType &inp) {
  ...
  toolchain_unreachable("Unexpected task dependence type");
}
...
makePrescriptiveness(parser::OmpPrescriptiveness::Value v) {
  ...
  toolchain_unreachable("Unexpected prescriptiveness");
}
...
Absent make(const parser::OmpClause::Absent &inp,
            semantics::SemanticsContext &semaCtx) {
  toolchain_unreachable("Unimplemented: absent");
}
...
Align make(const parser::OmpClause::Align &inp,
           semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: align");
}
...
At make(const parser::OmpClause::At &inp,
        semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: at");
}
...
make(const parser::OmpClause::CancellationConstructType &inp,
  ...
  CLAUSET_ENUM_CONVERT(
      convert, toolchain::omp::Directive, toolchain::omp::CancellationConstructType,
      // clang-format off
...
Contains make(const parser::OmpClause::Contains &inp,
              semantics::SemanticsContext &semaCtx) {
  toolchain_unreachable("Unimplemented: contains");
}
...
Holds make(const parser::OmpClause::Holds &inp,
           semantics::SemanticsContext &semaCtx) {
  toolchain_unreachable("Unimplemented: holds");
}
...
Init make(const parser::OmpClause::Init &inp,
          semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: init");
}

Initializer make(const parser::OmpClause::Initializer &inp,
                 semantics::SemanticsContext &semaCtx) {
  toolchain_unreachable("Empty: initializer");
}
...
Map make(const parser::OmpClause::Map &inp,
  ...
  toolchain::DenseSet<Map::MapTypeModifier> modSet;
...
Message make(const parser::OmpClause::Message &inp,
             semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: message");
}
...
Severity make(const parser::OmpClause::Severity &inp,
              semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: severity");
}
...
Use make(const parser::OmpClause::Use &inp,
         semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: use");
}
...
UsesAllocators make(const parser::OmpClause::UsesAllocators &inp,
                    semantics::SemanticsContext &semaCtx) {
  // inp -> empty
  toolchain_unreachable("Empty: uses_allocators");
}
      ...
      common::visitors{
          [&](const parser::OmpClause::Default &s) {
            ...
            if (std::holds_alternative<DSA>(s.v.u)) {
              return makeClause(toolchain::omp::Clause::OMPC_default,
                                clause::makeDefault(s, semaCtx), cls.source);
            } else {
              return makeClause(toolchain::omp::Clause::OMPC_otherwise,
...
bool transferLocations(const List<Clause> &from, List<Clause> &to) {
  ...
  for (Clause &clause : to) {
    if (!clause.source.empty())
      continue;
    auto found =
        toolchain::find_if(from, [&](const Clause &c) { return c.id == clause.id; });
