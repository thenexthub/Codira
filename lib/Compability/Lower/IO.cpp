...
#include "language/Compability/Lower/VectorSubscripts.h"
...
struct ConditionSpecInfo {
  const language::Compability::lower::SomeExpr *ioStatExpr{};
...
template <typename D>
static void genIoLoop(language::Compability::lower::AbstractConverter &converter,
...
static constexpr const char *getName() {
  return std::get<A>(language::Compability::lower::newIOTable).name;
}
...
static constexpr fir::runtime::FuncTypeBuilderFunc getTypeModel() {
  return std::get<A>(language::Compability::lower::newIOTable).getTypeModel();
}
...
static mlir::Value genEndIO(language::Compability::lower::AbstractConverter &converter,
...
using DefinedIoProcMap = std::multimap<const language::Compability::semantics::Symbol *,
                                       language::Compability::semantics::NonTbpDefinedIo>;

/// Get the current scope's non-type-bound defined IO procedures.
static DefinedIoProcMap
getDefinedIoProcMap(language::Compability::lower::AbstractConverter &converter) {
  const language::Compability::semantics::Scope *scope = &converter.getCurrentScope();
  for (; !scope->IsGlobal(); scope = &scope->parent())
    if (scope->kind() == language::Compability::semantics::Scope::Kind::MainProgram ||
        scope->kind() == language::Compability::semantics::Scope::Kind::Subprogram ||
        scope->kind() == language::Compability::semantics::Scope::Kind::BlockConstruct)
      break;
  return language::Compability::semantics::CollectNonTbpDefinedIoGenericInterfaces(*scope,
                                                                     false);
}
...
static bool hasLocalDefinedIoProc(DefinedIoProcMap &definedIoProcMap) {
  for (auto &iface : definedIoProcMap) {
    const language::Compability::semantics::Symbol *procSym = iface.second.subroutine;
    if (!procSym)
      continue;
    procSym = &procSym->GetUltimate();
    if (language::Compability::semantics::IsProcedurePointer(*procSym) ||
        language::Compability::semantics::IsDummy(*procSym))
      return true;
  }
  return false;
}
...
static mlir::Value
getNonTbpDefinedIoTableAddr(language::Compability::lower::AbstractConverter &converter,
  ...
  auto listFunc = [&](fir::FirOpBuilder &builder) {
    ...
    for (auto &iface : definedIoProcMap) {
      ...
      const language::Compability::semantics::Symbol &dtSym = iface.first->GetUltimate();
      ...
      if (procSym) {
        ...
        } else if (language::Compability::semantics::IsDummy(*procSym)) {
          language::Compability::lower::StatementContext stmtCtx;
          insert(fir::BoxAddrOp::create(
              builder, loc, refTy,
              fir::getBase(converter.genExprAddr(
                  loc,
                  language::Compability::lower::SomeExpr{
                      language::Compability::evaluate::ProcedureDesignator{*procSym}},
                  stmtCtx))));
        } else {
          mlir::func::FuncOp procDef = language::Compability::lower::getOrDeclareFunction(
              language::Compability::evaluate::ProcedureDesignator{*procSym}, converter);
...
static mlir::Value
getNamelistGroup(language::Compability::lower::AbstractConverter &converter,
  ...
  const auto &details =
      symbol.GetUltimate().get<language::Compability::semantics::NamelistDetails>();
  mlir::MLIRContext *context = builder.getContext();
  mlir::StringAttr linkOnce = builder.createLinkOnceLinkage();
  mlir::Type idxTy = builder.getIndexType();
  mlir::Type sizeTy =
  ...
  auto stringAddress = [&](const language::Compability::semantics::Symbol &symbol) {
  ...
  for (const language::Compability::semantics::Symbol &s : details.objects()) {
    stringAddress(s);
    ...
    if (!IsAllocatableOrObjectPointer(&s)) {
      std::string mangleName =
          language::Compability::lower::mangle::globalNamelistDescriptorName(s);
      if (builder.getNamedGlobal(mangleName))
        continue;
      const auto expr = language::Compability::evaluate::AsGenericExpr(s);
      fir::BoxType boxTy =
          fir::BoxType::get(fir::PointerType::get(converter.genType(s)));
      auto descFunc = [&](fir::FirOpBuilder &b) {
        bool couldBeInEquivalence =
            language::Compability::semantics::FindEquivalenceSet(s) != nullptr;
        auto box = language::Compability::lower::genInitialDataTarget(
    ...
    for (const language::Compability::semantics::Symbol &s : details.objects()) {
      ...
      if (auto desc = builder.getNamedGlobal(
        ...
        descAddr = fir::AddrOfOp::create(builder, loc, desc.resultType(),
                                         desc.getSymbol());
      } else if (language::Compability::semantics::FindCommonBlockContaining(s) &&
                 IsAllocatableOrPointer(s)) {
        mlir::Type symType = converter.genType(s);
        const language::Compability::semantics::Symbol *commonBlockSym =
            language::Compability::semantics::FindCommonBlockContaining(s);
...
static void genNamelistIO(language::Compability::lower::AbstractConverter &converter,
                          mlir::Value cookie, mlir::func::FuncOp funcOp,
                          language::Compability::semantics::Symbol &symbol, bool checkResult,
                          mlir::Value &ok,
                          language::Compability::lower::StatementContext &stmtCtx) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::Location loc = converter.getCurrentLocation();
  ...
  toolchain::SmallVector<mlir::Value> args = {cookie, groupAddr};
...
static mlir::func::FuncOp getOutputFunc(mlir::Location loc,
  ...
  if (auto ty = mlir::dyn_cast<mlir::IntegerType>(type)) {
    if (!ty.isUnsigned()) {
      ...
      toolchain_unreachable("unknown OutputInteger kind");
    }
  }
...
static void genOutputItemList(
    language::Compability::lower::AbstractConverter &converter, mlir::Value cookie,
    const std::list<language::Compability::parser::OutputItem> &items, bool isFormatted,
    bool checkResult, mlir::Value &ok, bool inLoop) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  for (const language::Compability::parser::OutputItem &item : items) {
    if (const auto &impliedDo = std::get_if<1>(&item.u)) {
      genIoLoop(converter, cookie, impliedDo->value(), isFormatted, checkResult,
                ok, inLoop);
      continue;
    }
    auto &pExpr = std::get<language::Compability::parser::Expr>(item.u);
    mlir::Location loc = converter.genLocation(pExpr.source);
    makeNextConditionalOn(builder, loc, checkResult, ok, inLoop);
    language::Compability::lower::StatementContext stmtCtx;

    const auto *expr = language::Compability::semantics::GetExpr(pExpr);
    if (!expr)
    ...
    toolchain::SmallVector<mlir::Value> outputFuncArgs = {cookie};
...
static mlir::Value
createIoRuntimeCallForItem(language::Compability::lower::AbstractConverter &converter,
  ...
  toolchain::SmallVector<mlir::Value> inputFuncArgs = {cookie};
...
static void genInputItemList(language::Compability::lower::AbstractConverter &converter,
                             mlir::Value cookie,
                             const std::list<language::Compability::parser::InputItem> &items,
                             bool isFormatted, bool checkResult,
                             mlir::Value &ok, bool inLoop) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  for (const language::Compability::parser::InputItem &item : items) {
    if (const auto &impliedDo = std::get_if<1>(&item.u)) {
      genIoLoop(converter, cookie, impliedDo->value(), isFormatted, checkResult,
                ok, inLoop);
      continue;
    }
    auto &pVar = std::get<language::Compability::parser::Variable>(item.u);
    mlir::Location loc = converter.genLocation(pVar.GetSource());
    makeNextConditionalOn(builder, loc, checkResult, ok, inLoop);
    language::Compability::lower::StatementContext stmtCtx;
    const auto *expr = language::Compability::semantics::GetExpr(pVar);
    if (!expr)
      fir::emitFatalError(loc, "internal error: could not get evaluate::Expr");
    if (language::Compability::evaluate::HasVectorSubscript(*expr)) {
      auto vectorSubscriptBox =
          language::Compability::lower::genVectorSubscriptBox(loc, converter, stmtCtx, *expr);
...
static void genIoLoop(language::Compability::lower::AbstractConverter &converter,
  ...
  mlir::Value loopVar = fir::getBase(converter.genExprAddr(
      language::Compability::evaluate::AsGenericExpr(loopSym).value(), stmtCtx));
  auto genControlValue = [&](const language::Compability::parser::ScalarIntExpr &expr) {
    mlir::Value v = fir::getBase(
        converter.genExprValue(*language::Compability::semantics::GetExpr(expr), stmtCtx));
  ...
  auto genItemList = [&](const D &ioImpliedDo) {
    if constexpr (std::is_same_v<D, language::Compability::parser::InputImpliedDo>)
  ...
  toolchain::SmallVector<mlir::Value> results = {inductionResult, iterateResult};
...
static mlir::Value locToFilename(language::Compability::lower::AbstractConverter &converter,
...
static mlir::Value locToLineNo(language::Compability::lower::AbstractConverter &converter,
                               mlir::Location loc, mlir::Type toType) {
...
genBuffer(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
          const language::Compability::lower::SomeExpr &expr, mlir::Type strTy,
          mlir::Type lenTy, language::Compability::lower::StatementContext &stmtCtx) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
...
lowerStringLit(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
               language::Compability::lower::StatementContext &stmtCtx, const A &syntax,
               mlir::Type strTy, mlir::Type lenTy, mlir::Type ty2 = {}) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  auto *expr = language::Compability::semantics::GetExpr(syntax);
  if (!expr)
...
static std::tuple<mlir::Value, mlir::Value, mlir::Value>
lowerSourceTextAsStringLit(language::Compability::lower::AbstractConverter &converter,
                           mlir::Location loc, toolchain::StringRef text,
...
mlir::Value genIntIOOption(language::Compability::lower::AbstractConverter &converter,
                           mlir::Location loc, mlir::Value cookie,
                           const B &spec) {
  language::Compability::lower::StatementContext localStatementCtx;
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  mlir::Value expr = fir::getBase(converter.genExprValue(
      loc, language::Compability::semantics::GetExpr(spec.v), localStatementCtx));
  mlir::Value val = builder.createConvert(loc, ioFuncTy.getInput(1), expr);
  toolchain::SmallVector<mlir::Value> ioArgs = {cookie, val};
...
template <typename A, typename B>
mlir::Value genCharIOOption(language::Compability::lower::AbstractConverter &converter,
                            mlir::Location loc, mlir::Value cookie,
                            const B &spec) {
  language::Compability::lower::StatementContext localStatementCtx;
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  toolchain::SmallVector<mlir::Value> ioArgs = {cookie, std::get<0>(tup),
                                           std::get<1>(tup)};
  return fir::CallOp::create(builder, loc, ioFunc, ioArgs).getResult(0);
}

template <typename A>
mlir::Value genIOOption(language::Compability::lower::AbstractConverter &converter,
  ...
  using PreprocessedSpecs =
      std::tuple<language::Compability::parser::EndLabel, language::Compability::parser::EorLabel,
                 language::Compability::parser::ErrLabel, language::Compability::parser::FileUnitNumber,
                 language::Compability::parser::Format, language::Compability::parser::IoUnit,
                 language::Compability::parser::MsgVariable, language::Compability::parser::Name,
                 language::Compability::parser::StatVariable>;
  static_assert(language::Compability::common::HasMember<A, PreprocessedSpecs>,
                "missing genIOOPtion specialization");
  return {};
}

template <>
mlir::Value genIOOption<language::Compability::parser::FileNameExpr>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::FileNameExpr &spec) {
  language::Compability::lower::StatementContext localStatementCtx;
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
...
template <>
mlir::Value genIOOption<language::Compability::parser::ConnectSpec::CharExpr>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::ConnectSpec::CharExpr &spec) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::func::FuncOp ioFunc;
  switch (std::get<language::Compability::parser::ConnectSpec::CharExpr::Kind>(spec.t)) {
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Access:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetAccess)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Action:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetAction)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Asynchronous:
    ioFunc =
        fir::runtime::getIORuntimeFunc<mkIOKey(SetAsynchronous)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Blank:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetBlank)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Decimal:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetDecimal)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Delim:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetDelim)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Encoding:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetEncoding)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Form:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetForm)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Pad:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetPad)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Position:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetPosition)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Round:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetRound)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Sign:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetSign)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Carriagecontrol:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetCarriagecontrol)>(
        loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Convert:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetConvert)>(loc, builder);
    break;
  case language::Compability::parser::ConnectSpec::CharExpr::Kind::Dispose:
  ...
  std::tuple<mlir::Value, mlir::Value, mlir::Value> tup =
      lowerStringLit(converter, loc, localStatementCtx,
                     std::get<language::Compability::parser::ScalarDefaultCharExpr>(spec.t),
                     ioFuncTy.getInput(1), ioFuncTy.getInput(2));
  toolchain::SmallVector<mlir::Value> ioArgs = {cookie, std::get<0>(tup),
                                           std::get<1>(tup)};
  return fir::CallOp::create(builder, loc, ioFunc, ioArgs).getResult(0);
...
template <>
mlir::Value genIOOption<language::Compability::parser::ConnectSpec::Recl>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::ConnectSpec::Recl &spec) {
  return genIntIOOption<mkIOKey(SetRecl)>(converter, loc, cookie, spec);
}

template <>
mlir::Value genIOOption<language::Compability::parser::StatusExpr>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::StatusExpr &spec) {
  return genCharIOOption<mkIOKey(SetStatus)>(converter, loc, cookie, spec.v);
}

template <>
mlir::Value genIOOption<language::Compability::parser::IoControlSpec::CharExpr>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::IoControlSpec::CharExpr &spec) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::func::FuncOp ioFunc;
  switch (std::get<language::Compability::parser::IoControlSpec::CharExpr::Kind>(spec.t)) {
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Advance:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetAdvance)>(loc, builder);
    break;
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Blank:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetBlank)>(loc, builder);
    break;
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Decimal:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetDecimal)>(loc, builder);
    break;
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Delim:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetDelim)>(loc, builder);
    break;
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Pad:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetPad)>(loc, builder);
    break;
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Round:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetRound)>(loc, builder);
    break;
  case language::Compability::parser::IoControlSpec::CharExpr::Kind::Sign:
    ioFunc = fir::runtime::getIORuntimeFunc<mkIOKey(SetSign)>(loc, builder);
    break;
  }
  language::Compability::lower::StatementContext localStatementCtx;
...
template <>
mlir::Value genIOOption<language::Compability::parser::IoControlSpec::Asynchronous>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie,
    const language::Compability::parser::IoControlSpec::Asynchronous &spec) {
  return genCharIOOption<mkIOKey(SetAsynchronous)>(converter, loc, cookie,
                                                   spec.v);
}

template <>
mlir::Value genIOOption<language::Compability::parser::IoControlSpec::Pos>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::IoControlSpec::Pos &spec) {
  return genIntIOOption<mkIOKey(SetPos)>(converter, loc, cookie, spec);
}

template <>
mlir::Value genIOOption<language::Compability::parser::IoControlSpec::Rec>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, const language::Compability::parser::IoControlSpec::Rec &spec) {
...
static void genIOGetVar(language::Compability::lower::AbstractConverter &converter,
                        mlir::Location loc, mlir::Value cookie,
                        const VAR &parserVar) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  mlir::func::FuncOp ioFunc =
  ...
  fir::ExtendedValue var = converter.genExprAddr(
      loc, language::Compability::semantics::GetExpr(parserVar.v), localStatementCtx);
...
template <typename SEEK, typename A>
static const language::Compability::lower::SomeExpr *getExpr(const A &stmt) {
  for (const auto &spec : stmt.v)
    if (auto *f = std::get_if<SEEK>(&spec.u))
      return language::Compability::semantics::GetExpr(f->v);
  toolchain::report_fatal_error("must have a file unit");
}

/// For each specifier, build the appropriate call, threading the cookie.
template <typename A>
static void threadSpecs(language::Compability::lower::AbstractConverter &converter,
                        mlir::Location loc, mlir::Value cookie,
                        const A &specList, bool checkResult, mlir::Value &ok) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  for (const auto &spec : specList) {
    makeNextConditionalOn(builder, loc, checkResult, ok);
    ok = language::Compability::common::visit(
        language::Compability::common::visitors{
            [&](const language::Compability::parser::IoControlSpec::Size &x) -> mlir::Value {
              // Size must be queried after the related READ runtime calls, not
              // before.
              return ok;
            },
            [&](const language::Compability::parser::ConnectSpec::Newunit &x) -> mlir::Value {
              // Newunit must be queried after OPEN specifier runtime calls
              // that may fail to avoid modifying the newunit variable if
              // there is an error.
              return ok;
            },
            [&](const language::Compability::parser::IdVariable &) -> mlir::Value {
...
template <typename A>
ConditionSpecInfo lowerErrorSpec(language::Compability::lower::AbstractConverter &converter,
                                 mlir::Location loc, const A &specList) {
  ConditionSpecInfo csi;
  const language::Compability::lower::SomeExpr *ioMsgExpr = nullptr;
  for (const auto &spec : specList) {
    language::Compability::common::visit(
        language::Compability::common::visitors{
            [&](const language::Compability::parser::StatVariable &var) {
              csi.ioStatExpr = language::Compability::semantics::GetExpr(var);
            },
            [&](const language::Compability::parser::InquireSpec::IntVar &var) {
              if (std::get<language::Compability::parser::InquireSpec::IntVar::Kind>(var.t) ==
                  language::Compability::parser::InquireSpec::IntVar::Kind::Iostat)
                csi.ioStatExpr = language::Compability::semantics::GetExpr(
                    std::get<language::Compability::parser::ScalarIntVariable>(var.t));
            },
            [&](const language::Compability::parser::MsgVariable &var) {
              ioMsgExpr = language::Compability::semantics::GetExpr(var);
            },
            [&](const language::Compability::parser::InquireSpec::CharVar &var) {
              if (std::get<language::Compability::parser::InquireSpec::CharVar::Kind>(
                      var.t) ==
                  language::Compability::parser::InquireSpec::CharVar::Kind::Iomsg)
                ioMsgExpr = language::Compability::semantics::GetExpr(
                    std::get<language::Compability::parser::ScalarDefaultCharVariable>(
                        var.t));
            },
            [&](const language::Compability::parser::EndLabel &) { csi.hasEnd = true; },
            [&](const language::Compability::parser::EorLabel &) { csi.hasEor = true; },
            [&](const language::Compability::parser::ErrLabel &) { csi.hasErr = true; },
            [](const auto &) {}},
        spec.u);
  }
  if (ioMsgExpr) {
...
template <typename A>
static void
genConditionHandlerCall(language::Compability::lower::AbstractConverter &converter,
                        mlir::Location loc, mlir::Value cookie,
  ...
  toolchain::SmallVector<mlir::Value> ioArgs = {cookie,
...
static bool formatIsActuallyNamelist(const A &format) {
  if (auto *e = std::get_if<language::Compability::parser::Expr>(&format.u)) {
    auto *expr = language::Compability::semantics::GetExpr(*e);
    if (const language::Compability::semantics::Symbol *y =
            language::Compability::evaluate::UnwrapWholeSymbolDataRef(*expr))
      return y->has<language::Compability::semantics::NamelistDetails>();
  }
  return false;
}
...
static bool isDataTransferFormatted(const A &stmt) {
  ...
  return hasIOControl<language::Compability::parser::Format>(stmt);
}
template <>
constexpr bool isDataTransferFormatted<language::Compability::parser::PrintStmt>(
    const language::Compability::parser::PrintStmt &) {
...
static bool isDataTransferList(const A &stmt) {
  if (stmt.format)
    return std::holds_alternative<language::Compability::parser::Star>(stmt.format->u);
  if (auto *mem = getIOControl<language::Compability::parser::Format>(stmt))
    return std::holds_alternative<language::Compability::parser::Star>(mem->u);
  return false;
}
template <>
bool isDataTransferList<language::Compability::parser::PrintStmt>(
    const language::Compability::parser::PrintStmt &stmt) {
  return std::holds_alternative<language::Compability::parser::Star>(
      std::get<language::Compability::parser::Format>(stmt.t).u);
}

template <typename A>
static bool isDataTransferInternal(const A &stmt) {
  if (stmt.iounit.has_value())
    return std::holds_alternative<language::Compability::parser::Variable>(stmt.iounit->u);
  if (auto *unit = getIOControl<language::Compability::parser::IoUnit>(stmt))
    return std::holds_alternative<language::Compability::parser::Variable>(unit->u);
  return false;
}
template <>
constexpr bool isDataTransferInternal<language::Compability::parser::PrintStmt>(
    const language::Compability::parser::PrintStmt &) {
  return false;
}
...
static std::optional<fir::ExtendedValue> getVariableBufferRequiredDescriptor(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::parser::Variable &var,
    language::Compability::lower::StatementContext &stmtCtx) {
...
maybeGetInternalIODescriptor(language::Compability::lower::AbstractConverter &converter,
                             mlir::Location loc, const A &stmt,
                             language::Compability::lower::StatementContext &stmtCtx) {
  if (stmt.iounit.has_value())
    if (auto *var = std::get_if<language::Compability::parser::Variable>(&stmt.iounit->u))
      return getVariableBufferRequiredDescriptor(converter, loc, *var, stmtCtx);
  if (auto *unit = getIOControl<language::Compability::parser::IoUnit>(stmt))
    if (auto *var = std::get_if<language::Compability::parser::Variable>(&unit->u))
      return getVariableBufferRequiredDescriptor(converter, loc, *var, stmtCtx);
  return std::nullopt;
}
template <>
inline std::optional<fir::ExtendedValue>
maybeGetInternalIODescriptor<language::Compability::parser::PrintStmt>(
    language::Compability::lower::AbstractConverter &, mlir::Location loc,
    const language::Compability::parser::PrintStmt &, language::Compability::lower::StatementContext &) {
  return std::nullopt;
}
...
static bool isDataTransferNamelist(const A &stmt) {
  if (stmt.format)
    return formatIsActuallyNamelist(*stmt.format);
  return hasIOControl<language::Compability::parser::Name>(stmt);
}
template <>
constexpr bool isDataTransferNamelist<language::Compability::parser::PrintStmt>(
    const language::Compability::parser::PrintStmt &) {
  return false;
}
...
static std::tuple<mlir::Value, mlir::Value, mlir::Value>
lowerReferenceAsStringSelect(language::Compability::lower::AbstractConverter &converter,
                             mlir::Location loc,
  ...
  language::Compability::lower::pft::LabelSet labels;
  ...
  for (auto label : labels) {
    ...
    toolchain::StringRef text = toStringRef(eval->position);
    mlir::Value stringRef;
    mlir::Value stringLen;
    if (eval->isA<language::Compability::parser::FormatStmt>()) {
    ...
    // statement.
    toolchain::SmallVector<mlir::Value> args = {stringRef, stringLen};
...
static std::tuple<mlir::Value, mlir::Value, mlir::Value>
genFormat(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
          const language::Compability::parser::Format &format, mlir::Type strTy,
          mlir::Type lenTy, language::Compability::lower::StatementContext &stmtCtx) {
  if (const auto *label = std::get_if<language::Compability::parser::Label>(&format.u)) {
  ...
  const auto *pExpr = std::get_if<language::Compability::parser::Expr>(&format.u);
  assert(pExpr && "missing format expression");
  auto e = language::Compability::semantics::GetExpr(*pExpr);
  if (language::Compability::semantics::ExprHasTypeCategory(
          *e, language::Compability::common::TypeCategory::Character)) {
  ...
  if (language::Compability::semantics::ExprHasTypeCategory(
          *e, language::Compability::common::TypeCategory::Integer) &&
      e->Rank() == 0 && language::Compability::evaluate::UnwrapWholeSymbolDataRef(*e)) {
...
getFormat(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
  ...
  return genFormat(converter, loc, *getIOControl<language::Compability::parser::Format>(stmt),
                   strTy, lenTy, stmtCtx);
}
template <>
std::tuple<mlir::Value, mlir::Value, mlir::Value>
getFormat<language::Compability::parser::PrintStmt>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    const language::Compability::parser::PrintStmt &stmt, mlir::Type strTy, mlir::Type lenTy,
    language::Compability::lower::StatementContext &stmtCtx) {
  return genFormat(converter, loc, std::get<language::Compability::parser::Format>(stmt.t),
                   strTy, lenTy, stmtCtx);
}
...
getBuffer(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
          const A &stmt, mlir::Type strTy, mlir::Type lenTy,
          language::Compability::lower::StatementContext &stmtCtx) {
  const language::Compability::parser::IoUnit *iounit =
      stmt.iounit ? &*stmt.iounit : getIOControl<language::Compability::parser::IoUnit>(stmt);
  if (iounit)
    if (auto *var = std::get_if<language::Compability::parser::Variable>(&iounit->u))
      if (auto *expr = language::Compability::semantics::GetExpr(*var))
        return genBuffer(converter, loc, *expr, strTy, lenTy, stmtCtx);
  toolchain::report_fatal_error("failed to get IoUnit expr");
}

static mlir::Value genIOUnitNumber(language::Compability::lower::AbstractConverter &converter,
                                   mlir::Location loc,
                                   const language::Compability::lower::SomeExpr *iounit,
...
static mlir::Value genIOUnit(language::Compability::lower::AbstractConverter &converter,
                             mlir::Location loc,
                             const language::Compability::parser::IoUnit *iounit,
                             mlir::Type ty, ConditionSpecInfo &csi,
                             language::Compability::lower::StatementContext &stmtCtx,
  ...
  if (iounit)
    if (auto *e =
            std::get_if<language::Compability::common::Indirection<language::Compability::parser::Expr>>(
                &iounit->u))
      return genIOUnitNumber(converter, loc, language::Compability::semantics::GetExpr(*e),
                             ty, csi, stmtCtx);
  return mlir::arith::ConstantOp::create(
...
template <typename A>
static mlir::Value
getIOUnit(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
          const A &stmt, mlir::Type ty, ConditionSpecInfo &csi,
          language::Compability::lower::StatementContext &stmtCtx, int defaultUnitNumber) {
  const language::Compability::parser::IoUnit *iounit =
      stmt.iounit ? &*stmt.iounit : getIOControl<language::Compability::parser::IoUnit>(stmt);
...
static mlir::Value genBasicIOStmt(language::Compability::lower::AbstractConverter &converter,
                                  const S &stmt) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::lower::StatementContext stmtCtx;
  mlir::Location loc = converter.getCurrentLocation();
  ...
  mlir::Value unit = genIOUnitNumber(
      converter, loc, getExpr<language::Compability::parser::FileUnitNumber>(stmt),
...
mlir::Value language::Compability::lower::genBackspaceStatement(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::parser::BackspaceStmt &stmt) {
  return genBasicIOStmt<mkIOKey(BeginBackspace)>(converter, stmt);
}

mlir::Value language::Compability::lower::genEndfileStatement(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::parser::EndfileStmt &stmt) {
  return genBasicIOStmt<mkIOKey(BeginEndfile)>(converter, stmt);
}

mlir::Value
language::Compability::lower::genFlushStatement(language::Compability::lower::AbstractConverter &converter,
                                  const language::Compability::parser::FlushStmt &stmt) {
  return genBasicIOStmt<mkIOKey(BeginFlush)>(converter, stmt);
}

mlir::Value
language::Compability::lower::genRewindStatement(language::Compability::lower::AbstractConverter &converter,
                                   const language::Compability::parser::RewindStmt &stmt) {
  return genBasicIOStmt<mkIOKey(BeginRewind)>(converter, stmt);
}

static mlir::Value
genNewunitSpec(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
               mlir::Value cookie,
               const std::list<language::Compability::parser::ConnectSpec> &specList) {
  for (const auto &spec : specList)
    if (auto *newunit =
            std::get_if<language::Compability::parser::ConnectSpec::Newunit>(&spec.u)) {
      language::Compability::lower::StatementContext stmtCtx;
      fir::FirOpBuilder &builder = converter.getFirOpBuilder();
      mlir::func::FuncOp ioFunc =
          fir::runtime::getIORuntimeFunc<mkIOKey(GetNewUnit)>(loc, builder);
      mlir::FunctionType ioFuncTy = ioFunc.getFunctionType();
      const auto *var = language::Compability::semantics::GetExpr(newunit->v);
      ...
      toolchain::SmallVector<mlir::Value> ioArgs = {cookie, addr, kind};
      return fir::CallOp::create(builder, loc, ioFunc, ioArgs).getResult(0);
    }
  toolchain_unreachable("missing Newunit spec");
}

mlir::Value
language::Compability::lower::genOpenStatement(language::Compability::lower::AbstractConverter &converter,
                                 const language::Compability::parser::OpenStmt &stmt) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::lower::StatementContext stmtCtx;
  mlir::func::FuncOp beginFunc;
  toolchain::SmallVector<mlir::Value> beginArgs;
  mlir::Location loc = converter.getCurrentLocation();
  ConditionSpecInfo csi = lowerErrorSpec(converter, loc, stmt.v);
  bool hasNewunitSpec = false;
  if (hasSpec<language::Compability::parser::FileUnitNumber>(stmt)) {
    beginFunc =
    ...
    hasNewunitSpec = hasSpec<language::Compability::parser::ConnectSpec::Newunit>(stmt);
...
mlir::Value
language::Compability::lower::genCloseStatement(language::Compability::lower::AbstractConverter &converter,
                                  const language::Compability::parser::CloseStmt &stmt) {
  return genBasicIOStmt<mkIOKey(BeginClose)>(converter, stmt);
}

mlir::Value
language::Compability::lower::genWaitStatement(language::Compability::lower::AbstractConverter &converter,
                                 const language::Compability::parser::WaitStmt &stmt) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::lower::StatementContext stmtCtx;
  mlir::Location loc = converter.getCurrentLocation();
  ConditionSpecInfo csi = lowerErrorSpec(converter, loc, stmt.v);
  bool hasId = hasSpec<language::Compability::parser::IdExpr>(stmt);
  mlir::func::FuncOp beginFunc =
      hasId
  ...
  mlir::Value unit = genIOUnitNumber(
      converter, loc, getExpr<language::Compability::parser::FileUnitNumber>(stmt),
      beginFuncTy.getInput(0), csi, stmtCtx);
  toolchain::SmallVector<mlir::Value> args{unit};
  if (hasId) {
    mlir::Value id = fir::getBase(converter.genExprValue(
        loc, getExpr<language::Compability::parser::IdExpr>(stmt), stmtCtx));
...
void genBeginDataTransferCallArgs(
    toolchain::SmallVectorImpl<mlir::Value> &ioArgs,
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
...
static mlir::Value
genDataTransferStmt(language::Compability::lower::AbstractConverter &converter,
                    const A &stmt) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::lower::StatementContext stmtCtx;
  mlir::Location loc = converter.getCurrentLocation();
  ...
  mlir::func::FuncOp ioFunc = getBeginDataTransferFunc<isInput>(
      ...
      isInternalWithDesc);
  toolchain::SmallVector<mlir::Value> ioArgs;
  genBeginDataTransferCallArgs<
      hasIOCtrl, isInput ? language::Compability::runtime::io::DefaultInputUnit
                         : language::Compability::runtime::io::DefaultOutputUnit>(
  ...
  if constexpr (isInput) { // READ
    if (isNml)
      genNamelistIO(
          ...
          *getIOControl<language::Compability::parser::Name>(stmt)->symbol,
  ...
  } else if constexpr (std::is_same_v<A, language::Compability::parser::WriteStmt>) {
    if (isNml)
      genNamelistIO(
          converter, cookie,
  ...
  if constexpr (hasIOCtrl) {
    for (const auto &spec : stmt.controls)
      if (const auto *size =
              std::get_if<language::Compability::parser::IoControlSpec::Size>(&spec.u)) {
        ...
        genIOGetVar<mkIOKey(GetSize)>(converter, loc, cookie, *size);
                     ...
                     std::get_if<language::Compability::parser::IdVariable>(&spec.u)) {
...
void language::Compability::lower::genPrintStatement(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::parser::PrintStmt &stmt) {
...
mlir::Value
language::Compability::lower::genWriteStatement(language::Compability::lower::AbstractConverter &converter,
                                  const language::Compability::parser::WriteStmt &stmt) {
  return genDataTransferStmt</*isInput=*/false>(converter, stmt);
}

mlir::Value
language::Compability::lower::genReadStatement(language::Compability::lower::AbstractConverter &converter,
                                 const language::Compability::parser::ReadStmt &stmt) {
...
static std::pair<const language::Compability::lower::SomeExpr *, bool>
getInquireFileExpr(const std::list<language::Compability::parser::InquireSpec> *stmt) {
  if (!stmt)
    return {nullptr, /*filename?=*/false};
  for (const language::Compability::parser::InquireSpec &spec : *stmt) {
    if (auto *f = std::get_if<language::Compability::parser::FileUnitNumber>(&spec.u))
      return {language::Compability::semantics::GetExpr(*f), /*filename?=*/false};
    if (auto *f = std::get_if<language::Compability::parser::FileNameExpr>(&spec.u))
      return {language::Compability::semantics::GetExpr(*f), /*filename?=*/true};
  }
  // semantics should have already caught this condition
  toolchain::report_fatal_error("inquire spec must have a file");
}
...
template <typename A>
static mlir::Value genInquireSpec(language::Compability::lower::AbstractConverter &converter,
                                  mlir::Location loc, mlir::Value cookie,
...
template <>
mlir::Value genInquireSpec<language::Compability::parser::InquireSpec::CharVar>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, mlir::Value idExpr,
    const language::Compability::parser::InquireSpec::CharVar &var,
    language::Compability::lower::StatementContext &stmtCtx) {
  // IOMSG is handled with exception conditions
  if (std::get<language::Compability::parser::InquireSpec::CharVar::Kind>(var.t) ==
      language::Compability::parser::InquireSpec::CharVar::Kind::Iomsg)
    return {};
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  ...
  const auto *varExpr = language::Compability::semantics::GetExpr(
      std::get<language::Compability::parser::ScalarDefaultCharVariable>(var.t));
  ...
  toolchain::SmallVector<mlir::Value> args = {
      ...
      builder.createIntegerConstant(
          ...
          language::Compability::runtime::io::HashInquiryKeyword(std::string{
              language::Compability::parser::InquireSpec::CharVar::EnumToString(
                  std::get<language::Compability::parser::InquireSpec::CharVar::Kind>(var.t))}
                                                       .c_str())),
...
template <>
mlir::Value genInquireSpec<language::Compability::parser::InquireSpec::IntVar>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, mlir::Value idExpr,
    const language::Compability::parser::InquireSpec::IntVar &var,
    language::Compability::lower::StatementContext &stmtCtx) {
  ...
  toolchain::SmallVector<mlir::Value> args = {
      ...
      builder.createIntegerConstant(
          loc, specFuncTy.getInput(1),
          language::Compability::runtime::io::HashInquiryKeyword(std::string{
              language::Compability::parser::InquireSpec::IntVar::EnumToString(
                  std::get<language::Compability::parser::InquireSpec::IntVar::Kind>(var.t))}
                                                       .c_str())),
...
template <>
mlir::Value genInquireSpec<language::Compability::parser::InquireSpec::LogVar>(
    language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
    mlir::Value cookie, mlir::Value idExpr,
    const language::Compability::parser::InquireSpec::LogVar &var,
    language::Compability::lower::StatementContext &stmtCtx) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  auto logVarKind = std::get<language::Compability::parser::InquireSpec::LogVar::Kind>(var.t);
  bool pendId =
      idExpr &&
      logVarKind == language::Compability::parser::InquireSpec::LogVar::Kind::Pending;
  mlir::func::FuncOp specFunc =
  ...
  mlir::Value addr = fir::getBase(converter.genExprAddr(
      ...
      language::Compability::semantics::GetExpr(
          std::get<language::Compability::parser::Scalar<
              language::Compability::parser::Logical<language::Compability::parser::Variable>>>(var.t)),
      stmtCtx));
  toolchain::SmallVector<mlir::Value> args = {
      builder.createConvert(loc, specFuncTy.getInput(0), cookie)};
  if (pendId)
    ...
    args.push_back(builder.createIntegerConstant(
        loc, specFuncTy.getInput(1),
        language::Compability::runtime::io::HashInquiryKeyword(std::string{
            language::Compability::parser::InquireSpec::LogVar::EnumToString(logVarKind)}
                                                     .c_str())));
...
static mlir::Value
lowerIdExpr(language::Compability::lower::AbstractConverter &converter, mlir::Location loc,
            const std::list<language::Compability::parser::InquireSpec> &ispecs,
            language::Compability::lower::StatementContext &stmtCtx) {
  for (const language::Compability::parser::InquireSpec &spec : ispecs)
    if (mlir::Value v = language::Compability::common::visit(
            language::Compability::common::visitors{
                [&](const language::Compability::parser::IdExpr &idExpr) {
                  return fir::getBase(converter.genExprValue(
                      loc, language::Compability::semantics::GetExpr(idExpr), stmtCtx));
                },
...
static void threadInquire(language::Compability::lower::AbstractConverter &converter,
                          mlir::Location loc, mlir::Value cookie,
                          const std::list<language::Compability::parser::InquireSpec> &ispecs,
  ...
  for (const language::Compability::parser::InquireSpec &spec : ispecs) {
    makeNextConditionalOn(builder, loc, checkResult, ok);
    ok = language::Compability::common::visit(language::Compability::common::visitors{[&](const auto &x) {
...
mlir::Value language::Compability::lower::genInquireStatement(
    language::Compability::lower::AbstractConverter &converter,
    const language::Compability::parser::InquireStmt &stmt) {
  fir::FirOpBuilder &builder = converter.getFirOpBuilder();
  language::Compability::lower::StatementContext stmtCtx;
  mlir::Location loc = converter.getCurrentLocation();
  mlir::func::FuncOp beginFunc;
  ...
  if (inquireFileUnit()) {
    ...
    const auto *ioLength =
        std::get_if<language::Compability::parser::InquireStmt::Iolength>(&stmt.u);
    ...
    genOutputItemList(
        converter, cookie,
        std::get<std::list<language::Compability::parser::OutputItem>>(ioLength->t),
        /*isFormatted=*/false, /*checkResult=*/false, ok, /*inLoop=*/false);
    auto *ioLengthVar = language::Compability::semantics::GetExpr(
        std::get<language::Compability::parser::ScalarIntVariable>(ioLength->t));
    mlir::Value ioLengthVarAddr =
        fir::getBase(converter.genExprAddr(loc, ioLengthVar, stmtCtx));
    toolchain::SmallVector<mlir::Value> args = {cookie};
    mlir::Value length =
        fir::CallOp::create(
            builder, loc,
