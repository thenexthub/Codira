...
#include "language/Compability/Testing/testing.h"
...
int Complete() {
  if (failures == 0) {
    if (passes == 1) {
      toolchain::outs() << "single test PASSES\n";
    } else {
      toolchain::outs() << "all " << passes << " tests PASS\n";
    }
    passes = 0;
    return EXIT_SUCCESS;
  } else {
    if (passes == 1) {
      toolchain::errs() << "1 test passes, ";
    } else {
      toolchain::errs() << passes << " tests pass, ";
    }
    if (failures == 1) {
      toolchain::errs() << "1 test FAILS\n";
    } else {
      toolchain::errs() << failures << " tests FAIL\n";
    }
    passes = failures = 0;
    return EXIT_FAILURE;
  }
