...
toolchain::raw_ostream &TypeAndShape::Dump(toolchain::raw_ostream &o) const {
...
toolchain::raw_ostream &DummyDataObject::Dump(toolchain::raw_ostream &o) const {
...
toolchain::raw_ostream &DummyProcedure::Dump(toolchain::raw_ostream &o) const {
  attrs.Dump(o, EnumToString);
  if (intent != common::Intent::Default) {
    o << "INTENT(" << common::EnumToString(intent) << ')';
  }
...
toolchain::raw_ostream &AlternateReturn::Dump(toolchain::raw_ostream &o) const {
  return o << '*';
}
...
toolchain::raw_ostream &DummyArgument::Dump(toolchain::raw_ostream &o) const {
  if (!name.empty()) {
    o << name << '=';
  }
  if (pass) {
...
toolchain::raw_ostream &FunctionResult::Dump(toolchain::raw_ostream &o) const {
  attrs.Dump(o, EnumToString);
  common::visit(common::visitors{
...
toolchain::raw_ostream &Procedure::Dump(toolchain::raw_ostream &o) const {
  attrs.Dump(o, EnumToString);
  if (functionResult) {
...
int DistinguishUtils::CountCompatibleWith(
  ...
  return toolchain::count_if(args, [&](const DummyArgument &y) {
...
template class language::Compability::common::Indirection<
    language::Compability::evaluate::characteristics::Procedure, true>;
