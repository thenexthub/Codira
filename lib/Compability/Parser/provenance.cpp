...
const SourceFile *AllSources::OpenPath(
    std::string path, toolchain::raw_ostream &error) {
...
const SourceFile *AllSources::Open(std::string path, toolchain::raw_ostream &error,
...
const SourceFile *AllSources::ReadStandardInput(toolchain::raw_ostream &error) {
  std::unique_ptr<SourceFile> source{std::make_unique<SourceFile>(encoding_)};
...
static void EmitPrefix(toolchain::raw_ostream &o, toolchain::raw_ostream::Colors color,
...
void AllSources::EmitMessage(toolchain::raw_ostream &o,
    const std::optional<ProvenanceRange> &range, const std::string &message,
    const std::string &prefix, toolchain::raw_ostream::Colors color,
...
static void DumpRange(toolchain::raw_ostream &o, const ProvenanceRange &r) {
  o << "[" << r.start().offset() << ".." << r.Last().offset() << "] ("
    << r.size() << " bytes)";
}

toolchain::raw_ostream &ProvenanceRangeToOffsetMappings::Dump(
...
toolchain::raw_ostream &OffsetToProvenanceMappings::Dump(
    toolchain::raw_ostream &o) const {
...
toolchain::raw_ostream &AllSources::Dump(toolchain::raw_ostream &o) const {
  o << "AllSources range_ ";
  DumpRange(o, range_);
  o << '\n';
...
toolchain::raw_ostream &CookedSource::Dump(toolchain::raw_ostream &o) const {
...
void AllCookedSources::Dump(toolchain::raw_ostream &o) const {
  o << "AllSources:\n";
  allSources_.Dump(o);
  for (const auto &c : cooked_) {
    c.Dump(o);
