...
void Definition::Print(toolchain::raw_ostream &out, const char *macroName) const {
  if (!isFunctionLike_) {
    ...
    if (!def->isFunctionLike()) {
      if (def->isPredefined() && !def->replacement().empty()) {
        ...
        if (name == "__FILE__") {
          ...
          repl = "??? ??? ?? ??:??:?? ????";
          if (!toolchain::sys::fs::status(path, status)) {
            auto modTime{toolchain::sys::toTimeT(status.getLastModificationTime())};
...
void Preprocessor::Directive(const TokenSequence &dir, Prescanner &prescanner) {
  ...
  if (dirName == "line") {
    ...
    std::string buf;
    toolchain::raw_string_ostream error{buf};
    if (const SourceFile *
...
void Preprocessor::PrintMacros(toolchain::raw_ostream &out) const {
