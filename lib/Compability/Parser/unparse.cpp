...
public:
  UnparseVisitor(toolchain::raw_ostream &out, const common::LangOptions &langOpts,
...
#undef FMT
  ...
  void Unparse(const toolchain::acc::Directive &x) {
    Word(toolchain::acc::getOpenACCDirectiveName(x).str());
  }
...
#include "toolchain/Frontend/OpenACC/ACC.inc"
  ...
  void Unparse(const OmpMapperSpecifier &x) {
    ...
    if (mapperName.find(toolchain::omp::OmpDefaultMapperName) == std::string::npos) {
      Walk(mapperName);
      Put(":");
    }
  ...
  void Unparse(const toolchain::omp::Directive &x) {
    unsigned ompVersion{langOpts_.OpenMPVersion};
    Word(toolchain::omp::getOpenMPDirectiveName(x, ompVersion).str());
  }
  void Unparse(const OmpDirectiveSpecification &x) {
    ...
    if (flags == OmpDirectiveSpecification::Flags::DeprecatedSyntax) {
      if (x.DirId() == toolchain::omp::Directive::OMPD_flush) {
  ...
  void Unparse(const OmpDirectiveNameModifier &x) {
    unsigned ompVersion{langOpts_.OpenMPVersion};
    Word(toolchain::omp::getOpenMPDirectiveName(x.v, ompVersion));
  }
...
private:
  ...
  template <typename T> void Walk(const T &x) {
    language::Compability::parser::Walk(x, *this);
  }
...
template void Unparse<Program>(toolchain::raw_ostream &, const Program &,
    const common::LangOptions &, Encoding, bool, bool, preStatementType *,
    AnalyzedObjectsAsFortran *);
template void Unparse<Expr>(toolchain::raw_ostream &, const Expr &,
    const common::LangOptions &, Encoding, bool, bool, preStatementType *,
    AnalyzedObjectsAsFortran *);

template void Unparse<parser::OpenMPDeclareReductionConstruct>(
    toolchain::raw_ostream &, const parser::OpenMPDeclareReductionConstruct &,
    const common::LangOptions &, Encoding, bool, bool, preStatementType *,
    AnalyzedObjectsAsFortran *);
template void Unparse<parser::OmpMetadirectiveDirective>(toolchain::raw_ostream &,
