...
TYPE_PARSER(sourced(construct<AccDeviceTypeExpr>(
    first("*" >> pure(language::Compability::common::OpenACCDeviceType::Star),
        "DEFAULT" >> pure(language::Compability::common::OpenACCDeviceType::Default),
        "NVIDIA" >> pure(language::Compability::common::OpenACCDeviceType::Nvidia),
        "ACC_DEVICE_NVIDIA" >> pure(language::Compability::common::OpenACCDeviceType::Nvidia),
        "RADEON" >> pure(language::Compability::common::OpenACCDeviceType::Radeon),
        "HOST" >> pure(language::Compability::common::OpenACCDeviceType::Host),
        "MULTICORE" >> pure(language::Compability::common::OpenACCDeviceType::Multicore)))))
...
TYPE_PARSER(construct<AccDefaultClause>(
    first("NONE" >> pure(toolchain::acc::DefaultValue::ACC_Default_none),
        "PRESENT" >> pure(toolchain::acc::DefaultValue::ACC_Default_present))))
...
TYPE_PARSER(sourced(construct<AccCombinedDirective>(
    first("KERNELS LOOP" >> pure(toolchain::acc::Directive::ACCD_kernels_loop),
        "PARALLEL LOOP" >> pure(toolchain::acc::Directive::ACCD_parallel_loop),
        "SERIAL LOOP" >> pure(toolchain::acc::Directive::ACCD_serial_loop)))))

// Block directives
TYPE_PARSER(sourced(construct<AccBlockDirective>(
    first("DATA" >> pure(toolchain::acc::Directive::ACCD_data),
        "HOST_DATA" >> pure(toolchain::acc::Directive::ACCD_host_data),
        "KERNELS" >> pure(toolchain::acc::Directive::ACCD_kernels),
        "PARALLEL" >> pure(toolchain::acc::Directive::ACCD_parallel),
        "SERIAL" >> pure(toolchain::acc::Directive::ACCD_serial)))))

// Standalone directives
TYPE_PARSER(sourced(construct<AccStandaloneDirective>(
    first("ENTER DATA" >> pure(toolchain::acc::Directive::ACCD_enter_data),
        "EXIT DATA" >> pure(toolchain::acc::Directive::ACCD_exit_data),
        "INIT" >> pure(toolchain::acc::Directive::ACCD_init),
        "SHUTDOWN" >> pure(toolchain::acc::Directive::ACCD_shutdown),
        "SET" >> pure(toolchain::acc::Directive::ACCD_set),
        "UPDATE" >> pure(toolchain::acc::Directive::ACCD_update)))))

// Loop directives
TYPE_PARSER(sourced(construct<AccLoopDirective>(
    first("LOOP" >> pure(toolchain::acc::Directive::ACCD_loop)))))
...
TYPE_PARSER(sourced(construct<AccDeclarativeDirective>(
    first("DECLARE" >> pure(toolchain::acc::Directive::ACCD_declare)))))
...
TYPE_PARSER(startAccLine >> sourced(construct<AccEndBlockDirective>("END"_tok >>
                                recovery(sourced(Parser<AccBlockDirective>{}),
                                    construct<AccBlockDirective>(pure(
                                        toolchain::acc::Directive::ACCD_data))))))
...
TYPE_PARSER(construct<OpenACCBlockConstruct>(
    ...
    recovery(withMessage("expected OpenACC end block directive"_err_en_US,
        ...
        construct<AccEndBlockDirective>(construct<AccBlockDirective>(
            pure(toolchain::acc::Directive::ACCD_data))))))
...
TYPE_PARSER(sourced(construct<OpenACCEndConstruct>(
    "END"_tok >> "LOOP"_tok >> pure(toolchain::acc::Directive::ACCD_loop))))
...
TYPE_PARSER(startAccLine >>
    sourced(construct<AccEndCombinedDirective>(sourced("END"_tok >>
        construct<AccCombinedDirective>("KERNELS"_tok >> maybe("LOOP"_tok) >>
                pure(toolchain::acc::Directive::ACCD_kernels_loop) ||
            "PARALLEL"_tok >> maybe("LOOP"_tok) >>
                pure(toolchain::acc::Directive::ACCD_parallel_loop) ||
            "SERIAL"_tok >> maybe("LOOP"_tok) >>
                pure(toolchain::acc::Directive::ACCD_serial_loop))))))
