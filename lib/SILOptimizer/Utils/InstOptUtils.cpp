...
static toolchain::cl::opt<bool> KeepWillThrowCall(
    "keep-will-throw-call", toolchain::cl::init(false),
    toolchain::cl::desc(
      "Keep calls to language_willThrow, even if the throw is optimized away"));

std::optional<SILBasicBlock::iterator>
language::getInsertAfterPoint(SILValue val) {
...
language::createIncrementBefore(SILValue ptr, SILInstruction *insertPt) {
...
/// reference-semantics.
NullablePtr<SILInstruction>
language::createDecrementBefore(SILValue ptr, SILInstruction *insertPt) {
...
bool language::canTriviallyDeleteOSSAEndScopeInst(SILInstruction *i) {
...
bool language::isInstructionTriviallyDead(SILInstruction *inst) {
...
bool language::isIntermediateRelease(SILInstruction *inst,
...
bool language::hasOnlyEndOfScopeOrEndOfLifetimeUses(SILInstruction *inst) {
...
unsigned language::getNumInOutArguments(FullApplySite applySite) {
  assert(applySite);
...
void language::collectUsesOfValue(SILValue v,
...
void language::eraseUsesOfValue(SILValue v) {
  toolchain::SmallPtrSet<SILInstruction *, 4> insts;
  // Collect the uses.
...
bool language::hasValueDeinit(SILType type) {
...
bool language::mayBindDynamicSelf(SILFunction *F) {
...
static SILValue skipAddrProjections(SILValue v) {
  ...
  toolchain_unreachable("there is no escape from an infinite loop");
}

/// Check whether the \p addr is an address of a tail-allocated array element.
bool language::isAddressOfArrayElement(SILValue addr) {
...
void language::placeFuncRef(ApplyInst *ai, DominanceInfo *domInfo) {
...
/// instruction.
TermInst *language::addArgumentsToBranch(ArrayRef<SILValue> vals,
...
SILLinkage language::getSpecializedLinkage(SILFunction *f, SILLinkage linkage) {
...
language::castValueToABICompatibleType(SILBuilder *builder, SILPassManager *pm,
                                    SILLocation loc,
    ...
    if (destTy.is<SILFunctionType>()) {
      assert(srcTy.getAs<SILFunctionType>()->isNoEscape()
             ...
             || srcTy.getAs<SILFunctionType>()->getRepresentation()
                    ...
                    && "Codira thick functions that differ in escapeness are "
                       "not ABI "
                       "compatible");
  ...
  if (srcNominal && srcNominal == destNominal &&
    ...
    //   public struct X2<ID> {
    //     let p: any P2<ID>
    //     public fn testit(i: ID, x: ID) -> S2<ID> {
    //       return p.get(x: x)
    //     }
    //   }
    // ```
  ...
  toolchain::errs() << "Source type: " << srcTy << "\n";
  toolchain::errs() << "Destination type: " << destTy << "\n";
  toolchain_unreachable("Unknown combination of types for casting");
}
...
bool language::layoutIsTypeDependent(NominalTypeDecl *decl) {
...
ProjectBoxInst *language::getOrCreateProjectBox(AllocBoxInst *abi,
                                             unsigned index) {
...
language::findInitAddressForTrivialEnum(UncheckedTakeEnumDataAddrInst *utedai) {
...
void language::emitDestroyOperation(SILBuilder &builder, SILLocation loc,
...
void language::releasePartialApplyCapturedArg(SILBuilder &builder, SILLocation loc,
                                           SILValue arg,
                                           SILParameterInfo paramInfo,
...
void language::getConsumedPartialApplyArgs(PartialApplyInst *pai,
...
bool language::collectDestroys(SingleValueInstruction *inst,
                            SmallVectorImpl<Operand *> &destroys) {
...
bool language::tryDeleteDeadClosure(SingleValueInstruction *closure,
                                 InstModCallbacks callbacks,
...
bool language::simplifyUsers(SingleValueInstruction *inst) {
  bool changed = false;
  InstModCallbacks callbacks;
...
protected:
  ...
  bool recursivelyAnalyzeInstruction(SILInstruction *inst) {
    if (auto *si = dyn_cast<StructInst>(inst)) {
      ...
      return toolchain::all_of(si->getAllOperands(), [&](Operand &operand) -> bool {
    ...
    if (auto *ti = dyn_cast<TupleInst>(inst)) {
      ...
      return toolchain::all_of(ti->getAllOperands(), [&](Operand &operand) -> bool {
        return recursivelyAnalyzeOperand(operand.get());
      });
    }
    if (auto *bi = dyn_cast<BuiltinInst>(inst)) {
...
bool language::analyzeStaticInitializer(
...
bool language::canReplaceLoadSequence(SILInstruction *inst) {
...
/// tuple_element_addr?
void language::replaceLoadSequence(SILInstruction *inst, SILValue value) {
  ...
  toolchain_unreachable("Unknown instruction sequence for reading from a global");
}

std::optional<FindLocalApplySitesResult>
language::findLocalApplySites(FunctionRefBaseInst *fri) {
  ...
  while (!worklist.empty()) {
    ...
    if (auto *pai = dyn_cast<PartialApplyInst>(user)) {
      if (pai->getCallee() == op->get()) {
        ...
        // as well.
        toolchain::copy(pai->getUses(), std::back_inserter(worklist));
        continue;
      }
    }
    ...
    // escapes:
    case SILInstructionKind::CopyValueInst:
      toolchain::copy(cast<CopyValueInst>(user)->getUses(),
                 std::back_inserter(worklist));
      continue;
    case SILInstructionKind::StrongRetainInst:
    case SILInstructionKind::StrongReleaseInst:
...
void language::insertDestroyOfCapturedArguments(
...
void language::insertDeallocOfCapturedArguments(
...
AbstractFunctionDecl *language::getBaseMethod(AbstractFunctionDecl *FD) {
...
FullApplySite
language::cloneFullApplySiteReplacingCallee(FullApplySite applySite,
                                         SILValue newCallee,
                                         SILBuilderContext &builderCtx) {
  SmallVector<SILValue, 16> arguments;
  toolchain::copy(applySite.getArguments(), std::back_inserter(arguments));
  ...
  case FullApplySiteKind::BeginApplyInst: {
    toolchain_unreachable("begin_apply support not implemented?!");
  }
  }
...
// deleting them.
SILBasicBlock::iterator language::replaceAllUses(SILValue oldValue,
                                              SILValue newValue,
...
language::replaceAllUsesAndErase(SingleValueInstruction *svi, SILValue newValue,
                              InstModCallbacks &callbacks) {
...
SILBasicBlock::iterator
language::replaceAllUsesAndErase(SILValue oldValue, SILValue newValue,
                              InstModCallbacks &callbacks) {
  ...
  toolchain_unreachable("Untested");
...
SILBasicBlock::iterator language::replaceSingleUse(Operand *use, SILValue newValue,
                                                InstModCallbacks &callbacks) {
...
SILValue language::makeCopiedValueAvailable(SILValue value, SILBasicBlock *inBlock) {
...
SILValue language::makeValueAvailable(SILValue value, SILBasicBlock *inBlock) {
  if (isa<SILUndef>(value))
    return value;
...
bool language::tryEliminateOnlyOwnershipUsedForwardingInst(
...
void language::endLifetimeAtLeakingBlocks(SILValue value,
  ...
  findJointPostDominatingSet(
      ...
      [&](SILBasicBlock *postDomBlock) {
        ...
        auto front = postDomBlock->begin();
        SILBuilderWithScope newBuilder(front);
        language::IsDeadEnd_t isDeadEnd = language::IsntDeadEnd;
        if (deadEndBlocks) {
...
void language::salvageStoreDebugInfo(SILInstruction *SI,
...
// the same variable.
void language::salvageDebugInfo(SILInstruction *I) {
  if (!I)
    return;
  ...
  if (auto *STI = dyn_cast<StructInst>(I)) {
    ...
    toolchain::ArrayRef<VarDecl *> FieldDecls =
...
void language::salvageLoadDebugInfo(LoadOperation load) {
...
// TODO: this currently fails to notify the pass with notifyNewInstruction.
void language::createDebugFragments(SILValue oldValue, Projection proj,
                                 SILValue newValue) {
...
IntegerLiteralInst *language::optimizeBuiltinCanBeObjCClass(BuiltinInst *bi,
                                                         SILBuilder &builder) {
  ...
  toolchain_unreachable("Unhandled TypeTraitResult in switch.");
}

SILValue language::createEmptyAndUndefValue(SILType ty,
  ...
  if (auto *decl = ty.getStructOrBoundGenericStruct()) {
    ...
    if (decl->isResilient(tec.getContext()->getParentModule(),
      ...
      toolchain::errs() << "Attempting to create value for illegal empty type:\n";
      ty.print(toolchain::errs());
      toolchain::report_fatal_error("illegal empty type: resilient struct");
    }
    SmallVector<SILValue, 4> elements;
    for (auto *field : decl->getStoredProperties()) {
...
bool language::findUnreferenceableStorage(StructDecl *decl, SILType structType,
                                       SILFunction *fn) {
  ...
  for (auto *field : decl->getStoredProperties()) {
    TypeExpansionContext tec = *fn;
    auto fieldTy = structType.getFieldType(field, fn->getModule(), tec);
    if (auto *fieldStructDecl = fieldTy.getStructOrBoundGenericStruct()) {
      if (findUnreferenceableStorage(fieldStructDecl, fieldTy, fn)) {
        return true;
      }
    }
  }
...
SILValue language::getInitOfTemporaryAllocStack(AllocStackInst *asi) {
...
bool language::isDestructorSideEffectFree(SILInstruction *mayRelease,
                                       DestructorAnalysis *DA) {
