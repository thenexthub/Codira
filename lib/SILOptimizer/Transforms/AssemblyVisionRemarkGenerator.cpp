...
static toolchain::cl::opt<bool> ForceVisitImplicitAutogeneratedFunctions(
    "assemblyvisionremarkgen-visit-implicit-autogen-funcs", toolchain::cl::Hidden,
    toolchain::cl::desc(
        "Emit opt remarks even on implicit and autogenerated functions"),
    toolchain::cl::init(false));

static toolchain::cl::opt<bool> DecllessDebugValueUseSILDebugInfo(
...
struct ValueToDeclInferrer {
  ...
  ///      debug_value %2 : $Array, ...
  ///    }
  void printNote(toolchain::raw_string_ostream &stream, StringRef name,
  ...
  void printNote(toolchain::raw_string_ostream &stream, const ValueDecl *decl,
                 bool shouldPrintAccessPath = true) {
    printNote(stream, decl->getBaseName().userFacingName(),
              shouldPrintAccessPath);
  }
  ...
  /// stream.
  void printAccessPath(toolchain::raw_string_ostream &stream);
};

} // anonymous namespace

void ValueToDeclInferrer::printAccessPath(toolchain::raw_string_ostream &stream) {
...
void ValueToDeclInferrer::printNote(toolchain::raw_string_ostream &stream,
                                    StringRef name,
...
struct ValueUseToDeclInferrer {
  ...
  SmallPtrSet<language::SILInstruction *, 8> visitedDebugValueInsts;
...
bool ValueUseToDeclInferrer::findDecls(Operand *use, SILValue value) {
  ...
  TOOLCHAIN_DEBUG(toolchain::dbgs() << "Found DebugInst: " << **debugInst);
  ...
  if (auto *decl = debugInst.getDecl()) {
    std::string msg;
    {
      toolchain::raw_string_ostream stream(msg);
...
bool ValueToDeclInferrer::infer(
  ...
  LANGUAGE_DEFER { accessPath.clear(); };
  ...
  // next step.
  TOOLCHAIN_DEBUG(toolchain::dbgs() << "Searching for decls!\n");
  while (true) {
    TOOLCHAIN_DEBUG(toolchain::dbgs() << "Visiting: " << *value);
    ...
    //   %1 = address_to_pointer %0 : $*_CodiraEmptyArrayStorage to
    ...
    if (auto *asi = dyn_cast<AllocStackInst>(value)) {
      ...
      TOOLCHAIN_DEBUG(toolchain::dbgs() << "Checking for single init use!\n");
      if (auto *initUse = getSingleInitAllocStackUse(asi)) {
        TOOLCHAIN_DEBUG(toolchain::dbgs() << "Found one: " << *initUse->getUser());
...
class AssemblyVisionRemarkGenerator : public SILFunctionTransform {
  ...
  bool isOptRemarksEnabled() {
    ...
    if (fn->hasSelfParam()) {
      if (auto *nomType = fn->getSelfArgument()
        ...
        TOOLCHAIN_DEBUG(toolchain::dbgs() << "Checking for remark on: "
                                << nomType->getName().get() << "\n");
        if (nomType->shouldEmitAssemblyVisionRemarksOnMethods()) {
          TOOLCHAIN_DEBUG(toolchain::dbgs() << "Success! Will emit remarks!!\n");
          return true;
        }
        TOOLCHAIN_DEBUG(toolchain::dbgs() << "Fail! No remarks will be emitted!!\n");
      }
    }
  ...
  void run() override {
    ...
    if (!ForceVisitImplicitAutogeneratedFunctions) {
      ...
      if (auto *ctx = fn->getDeclContext()) {
        if (auto *decl = ctx->getAsDecl()) {
          if (decl->isImplicit()) {
            TOOLCHAIN_DEBUG(toolchain::dbgs() << "Skipping implicit decl function: "
                                    << fn->getName() << "\n");
            return;
          }
        }
      ...
      if (auto loc = fn->getDebugScope()->getLoc()) {
        if (loc.isAutoGenerated()) {
          TOOLCHAIN_DEBUG(toolchain::dbgs() << "Skipping autogenerated function: "
                                  << fn->getName() << "\n");
          return;
        }
      }
    ...
    TOOLCHAIN_DEBUG(toolchain::dbgs() << "Visiting: " << fn->getName() << "\n");
...
SILTransform *language::createAssemblyVisionRemarkGenerator() {
