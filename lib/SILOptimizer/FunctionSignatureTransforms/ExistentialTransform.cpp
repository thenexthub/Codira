...
void ExistentialSpecializerCloner::cloneAndPopulateFunction() {
  ...
  for (auto *exitBB : exitingBlocks) {
    ...
    for (auto *ASI : toolchain::reverse(AllocStackInsts))
...
void ExistentialSpecializerCloner::cloneArguments(
  ...
  for (auto &ArgDesc : ArgumentDescList) {
    ...
    default: {
      toolchain_unreachable("Unhandled existential type in ExistentialTransform!");
      break;
    }
    };
  }
...
void ExistentialTransform::populateThunkBody() {
  ...
  for (auto &ArgDesc : ArgumentDescList) {
    ...
    if (iter != ArgToGenericTypeMap.end() &&
      ...
      auto CodiraType = ArgDesc.Arg->getType().getASTType();
      auto OpenedType = ExistentialArchetypeType::getAny(CodiraType)
              ->getCanonicalType();
  ...
  for (auto &Temp : toolchain::reverse(Temps)) {
...
void ExistentialTransform::createExistentialSpecializedFunction() {
  ...
  TOOLCHAIN_DEBUG(toolchain::dbgs() << "After ExistentialSpecializer Pass\n"; F->dump();
             NewF->dump(););
}
