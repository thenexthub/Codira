...
static std::string QualTypeToString(ASTContext &Ctx, QualType QT) {
  ...
  if (const auto *TTy = toolchain::dyn_cast<TagType>(NonRefTy))
  ...
  if (const auto *TDTy = dyn_cast<TypedefType>(NonRefTy)) {
    ...
    if (toolchain::isa<SubstTemplateTypeParmType>(SSDesugar))
      return GetFullTypeName(Ctx, Canon);
    else if (toolchain::isa<TemplateSpecializationType>(SSDesugar))
...
static std::string EnumToString(const Value &V) {
  ...
  bool IsFirst = true;
  toolchain::APSInt AP = Ctx.MakeIntValue(Data, DesugaredTy);
  ...
  toolchain::SmallString<64> APStr;
...
static std::string FunctionToString(const Value &V, const void *Ptr) {
  ...
  if (auto *InterfaceCall = toolchain::dyn_cast<CallExpr>(TLSD->getStmt())) {
    const auto *Arg = InterfaceCall->getArg(/*Val*/ 3);
    // Get rid of cast nodes.
    while (const CastExpr *CastE = toolchain::dyn_cast<CastExpr>(Arg))
      Arg = CastE->getSubExpr();
    if (const DeclRefExpr *DeclRefExp = toolchain::dyn_cast<DeclRefExpr>(Arg))
      FD = toolchain::dyn_cast<FunctionDecl>(DeclRefExp->getDecl());

    if (FD) {
      SS << '\n';
      const language::Core::FunctionDecl *FDef;
      if (FD->hasBody(FDef))
        FDef->print(SS);
    }
  }
  ...
  if (auto *BT = DesugaredTy.getCanonicalType()->getAs<BuiltinType>()) {
    ...
    auto formatFloating = [](auto Val, char Suffix = '\0') -> std::string {
      ...
      if (std::isnan(Val) || std::isinf(Val)) {
        SS << toolchain::format("%g", Val);
        return SS.str();
      }
      if (Val == static_cast<decltype(Val)>(static_cast<int64_t>(Val)))
        SS << toolchain::format("%.1f", Val);
      else if (std::abs(Val) < 1e-4 || std::abs(Val) > 1e6 || Suffix == 'f')
        SS << toolchain::format("%#.6g", Val);
      else if (Suffix == 'L')
        SS << toolchain::format("%#.12Lg", Val);
      else
        SS << toolchain::format("%#.8g", Val);
...
Interpreter::CompileDtorCall(CXXRecordDecl *CXXRD) const {
  ...
  if (CXXRD->hasIrrelevantDestructor())
    return toolchain::orc::ExecutorAddr{};
...
class InterfaceKindVisitor
  ...
  Sema &S;
  Expr *E;
  toolchain::SmallVectorImpl<Expr *> &Args;

public:
  InterfaceKindVisitor(Sema &S, Expr *E, toolchain::SmallVectorImpl<Expr *> &Args)
      : S(S), E(E), Args(Args) {}
...
static constexpr toolchain::StringRef VPName[] = {
...
//   xQualType)) (x);
toolchain::Expected<Expr *> Interpreter::convertExprToValue(Expr *E) {
  Sema &S = getCompilerInstance()->getSema();
  ASTContext &Ctx = S.getASTContext();

  // Find the value printing builtins.
  if (!ValuePrintingInfo[0]) {
    assert(toolchain::all_of(ValuePrintingInfo, [](Expr *E) { return !E; }));

    auto LookupInterface = [&](Expr *&Interface,
                               toolchain::StringRef Name) -> toolchain::Error {
      ...
      if (R.empty())
        return toolchain::make_error<toolchain::StringError>(
            Name + " not found!", toolchain::inconvertibleErrorCode());
  ...
  toolchain::SmallVector<Expr *, 4> AdjustedArgs;
  ...
  if (auto *EWC = toolchain::dyn_cast_if_present<ExprWithCleanups>(E))
    E = EWC->getSubExpr();
  ...
  default:
    toolchain_unreachable("Unhandled InterfaceKind");
  }
