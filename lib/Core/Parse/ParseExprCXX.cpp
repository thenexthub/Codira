...
static int SelectDigraphErrorMessage(tok::TokenKind Kind) {
  switch (Kind) {
    ...
    default:
      toolchain_unreachable("Unknown type for digraph error message.");
  }
}
...
bool Parser::ParseLambdaIntroducer(LambdaIntroducer &Intro,
  ...
  auto Invalid = [&](toolchain::function_ref<void()> Action) {
    if (Tentative) {
  ...
  auto NonTentativeAction = [&](toolchain::function_ref<void()> Action) {
    if (Tentative)
  ...
  while (Tok.isNot(tok::r_square)) {
    ...
    if (toolchain::any_of(EllipsisLocs,
...
ExprResult Parser::ParseCXXCasts() {
  ...
  switch (Kind) {
  default: toolchain_unreachable("Unknown C++ cast!");
...
void Parser::ParseCXXSimpleTypeSpecifier(DeclSpec &DS) {
  ...
  const language::Core::PrintingPolicy &Policy =
      Actions.getASTContext().getPrintingPolicy();

  switch (Tok.getKind()) {
  case tok::identifier:   // foo::bar
  case tok::coloncolon:   // ::foo::bar
    toolchain_unreachable("Annotation token should already be formed!");
  default:
    toolchain_unreachable("Not a simple-type-specifier token!");
...
ExprResult Parser::ParseRequiresExpression() {
  ...
  toolchain::SmallVector<ParmVarDecl *, 2> LocalParameterDecls;
  BalancedDelimiterTracker Parens(*this, tok::l_paren);
  if (Tok.is(tok::l_paren)) {
    ...
    if (!Tok.is(tok::r_paren)) {
      ...
      SourceLocation EllipsisLoc;
      toolchain::SmallVector<DeclaratorChunk::ParamInfo, 2> LocalParameters;
  ...
  toolchain::SmallVector<concepts::Requirement *, 2> Requirements;
...
static TypeTrait TypeTraitFromTokKind(tok::TokenKind kind) {
  switch (kind) {
  default: toolchain_unreachable("Not a known type trait");
...
static ArrayTypeTrait ArrayTypeTraitFromTokKind(tok::TokenKind kind) {
  switch (kind) {
  default:
    toolchain_unreachable("Not a known array type trait");
...
static ExpressionTrait ExpressionTraitFromTokKind(tok::TokenKind kind) {
  switch (kind) {
  default:
    toolchain_unreachable("Not a known unary expression trait.");
...
ExprResult Parser::ParseArrayTypeTrait() {
  ...
  toolchain_unreachable("Invalid ArrayTypeTrait!");
}
