    ...
    if (!LHS.isInvalid()) {
      ...
      if (TernaryMiddle.isInvalid()) {
        ...
        ExprResult CondOp = Actions.ActOnConditionalOp(
            ...
            RHS.get());
        if (CondOp.isInvalid()) {
          std::vector<language::Core::Expr *> Args;
...
#undef RTT_JOIN
  ...
  toolchain::SmallDenseMap<IdentifierInfo *, tok::TokenKind>::iterator Known =
    ...
    case tok::l_square: {  // postfix-expression: p-e '[' expression ']'
      ...
      if (ArgExprs.size() <= 1 && AllowOpenACCArraySections) {
        ...
        if (getLangOpts().OpenMP >= 50 &&
            (OMPClauseKind == toolchain::omp::Clause::OMPC_to ||
             OMPClauseKind == toolchain::omp::Clause::OMPC_from) &&
            Tok.is(tok::colon)) {
          // Consume ':'
...
ExprResult Parser::ParseBuiltinPrimaryExpression() {
  ...
  switch (T) {
  default: toolchain_unreachable("Not a builtin primary expression!");
  case tok::kw___builtin_va_arg: {
  ...
  case tok::kw___builtin_source_location: {
    ...
    SourceLocIdentKind Kind = [&] {
      ...
      default:
        toolchain_unreachable("invalid keyword");
      }
    }();
...
static bool CheckAvailabilitySpecList(Parser &P,
  ...
  toolchain::SmallSet<StringRef, 4> Platforms;
