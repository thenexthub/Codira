...
#include "toolchain/Frontend/OpenMP/DirectiveNameParser.h"
...
public:
  ...
  toolchain::ArrayRef<Expr *> getIdentifiers() const { return Identifiers; }
};
} // namespace
...
bool Parser::ParseOpenMPDeclareBeginVariantDirective(SourceLocation Loc) {
  ...
  TargetOMPContext OMPCtx(
      ...
      /* ConstructTraits */ ArrayRef<toolchain::omp::TraitProperty>(),
...
static bool checkForDuplicates(Parser &P, StringRef Name,
                               ...
                               toolchain::StringMap<SourceLocation> &Seen,
                               OMPContextLvl Lvl) {
...
void Parser::parseOMPTraitPropertyKind(OMPTraitProperty &TIProperty,
                                       toolchain::omp::TraitSet Set,
                                       toolchain::omp::TraitSelector Selector,
                                       toolchain::StringMap<SourceLocation> &Seen) {
  TIProperty.Kind = TraitProperty::invalid;

  SourceLocation NameLoc = Tok.getLocation();
  StringRef Name;
  if (Selector == toolchain::omp::TraitSelector::target_device_device_num) {
    Name = "number";
...
static bool checkExtensionProperty(Parser &P, SourceLocation Loc,
  ...
  assert(TISelector.Kind ==
             toolchain::omp::TraitSelector::implementation_extension &&
  ...
  auto IsMatchExtension = [](OMPTraitProperty &TP) {
    return (TP.Kind ==
                toolchain::omp::TraitProperty::implementation_extension_match_all ||
            TP.Kind ==
                toolchain::omp::TraitProperty::implementation_extension_match_any ||
            TP.Kind ==
                toolchain::omp::TraitProperty::implementation_extension_match_none);
  };
  ...
  if (IsMatchExtension(TIProperty)) {
    for (OMPTraitProperty &SeenProp : TISelector.Properties)
      if (IsMatchExtension(SeenProp)) {
        ...
        StringRef SeenName = toolchain::omp::getOpenMPContextTraitPropertyName(
  ...
  toolchain_unreachable("Unknown extension property!");
}
...
void Parser::parseOMPContextProperty(OMPTraitSelector &TISelector,
  ...
  if (TISelector.Kind == toolchain::omp::TraitSelector::implementation_extension)
...
void Parser::parseOMPContextSelector(
    OMPTraitSelector &TISelector, toolchain::omp::TraitSet Set,
    toolchain::StringMap<SourceLocation> &SeenSelectors) {
  ...
  toolchain::StringMap<SourceLocation> SeenProperties;
  do {
...
void Parser::parseOMPContextSelectorSet(
    OMPTraitSet &TISet, toolchain::StringMap<SourceLocation> &SeenSets) {
...
bool Parser::parseOMPContextSelectors(SourceLocation Loc, OMPTraitInfo &TI) {
  toolchain::StringMap<SourceLocation> SeenSets;
  do {
    OMPTraitSet TISet;
  ...
  while (Tok.isNot(tok::annot_pragma_openmp_end)) {
    ...
    if (!IsError) {
      ...
      case OMPC_adjust_args: {
        ...
        if (!IsError) {
          ...
          case OMPC_ADJUST_ARGS_nothing:
            toolchain::append_range(AdjustNothing, Vars);
            break;
          case OMPC_ADJUST_ARGS_need_device_ptr:
            toolchain::append_range(AdjustNeedDevicePtr, Vars);
            break;
          case OMPC_ADJUST_ARGS_need_device_addr:
            toolchain::append_range(AdjustNeedDeviceAddr, Vars);
            break;
          default:
            toolchain_unreachable("Unexpected 'adjust_args' clause modifier.");
          }
        }
        break;
      }
      case OMPC_append_args:
        ...
        break;
      default:
        toolchain_unreachable("Unexpected clause for declare variant.");
      }
    }
    if (IsError) {
      while (!SkipUntil(tok::annot_pragma_openmp_end, StopBeforeMatch))
        ...
        for (OMPTraitSelector &Selector : Set.Selectors) {
          ...
          for (const OMPTraitProperty &ParentProperty :
            ...
            for (OMPTraitProperty &Property : Selector.Properties) {
              ...
              if (Selector.Kind == toolchain::omp::TraitSelector::user_condition) {
...
void Parser::ParseOpenMPClauses(OpenMPDirectiveKind DKind,
                                ...
                                SourceLocation Loc) {
  std::bitset<toolchain::omp::Clause_enumSize + 1> SeenClauses;
  while (Tok.isNot(tok::annot_pragma_openmp_end)) {
    OpenMPClauseKind CKind = Tok.isAnnotation()
                                 ? OMPC_unknown
                                 : getOpenMPClauseKind(PP.getSpelling(Tok));
...
void Parser::ParseOpenMPAssumesDirective(OpenMPDirectiveKind DKind,
  ...
  auto SkipBraces = [&](toolchain::StringRef Spelling, bool IssueNote) {
    BalancedDelimiterTracker T(*this, tok::l_paren,
                               tok::annot_pragma_openmp_end);
  ...
  auto MatchACMClause = [&](StringRef RawString) {
    toolchain::StringSwitch<int> SS(RawString);
    unsigned ACMIdx = 0;
  ...
  while (Tok.isNot(tok::annot_pragma_openmp_end)) {
    ...
    if (Idx == -1) {
      ...
      Diag(StartLoc, diag::warn_omp_unknown_assumption_clause_missing_id)
          << toolchain::omp::getOpenMPDirectiveName(DKind, OMPVersion)
          << toolchain::omp::getAllAssumeClauseOptions() << NextIsLPar;
      if (NextIsLPar)
      ...
      if (IsDeviceTypeClause) {
        ...
        if (DevTypeData) {
          ...
          case OMPC_DEVICE_TYPE_unknown:
            toolchain_unreachable("Unexpected device_type");
          }
...
Parser::DeclGroupPtrTy Parser::ParseOpenMPDeclarativeDirectiveWithExtDecl(
  ...
  case OMPD_requires: {
    ...
    toolchain::SmallBitVector SeenClauses(toolchain::omp::Clause_enumSize + 1);
    if (Tok.is(tok::annot_pragma_openmp_end)) {
      Diag(Tok, diag::err_omp_expected_clause)
  ...
  case OMPD_declare_target: {
    ...
    toolchain::SmallVector<Decl *, 4> Decls;
...
StmtResult Parser::ParseOpenMPDeclarativeOrExecutableDirective(
  ...
  case OMPD_metadirective: {
    ...
    TargetOMPContext OMPCtx(ASTContext, std::move(DiagUnknownTrait),
                            /* CurrentFunctionDecl */ nullptr,
                            ArrayRef<toolchain::omp::TraitProperty>(),
                            Actions.OpenMP().getOpenMPDeviceNum());
...
bool Parser::ParseOpenMPSimpleVarList(
    OpenMPDirectiveKind Kind,
    const toolchain::function_ref<void(CXXScopeSpec &, DeclarationNameInfo)>
        &Callback,
...
OMPClause *Parser::ParseOpenMPClause(OpenMPDirectiveKind DKind,
  ...
  case OMPC_ompx_attribute:
    ...
    break;
  case OMPC_ompx_bare:
    if (DKind == toolchain::omp::Directive::OMPD_target) {
...
OMPClause *Parser::ParseOpenMPInteropClause(OpenMPClauseKind Kind,
  ...
  toolchain_unreachable("Unexpected interop variable clause.");
}
...
parseOpenMPAllocateClauseModifiers(Parser &P, OpenMPClauseKind Kind,
  ...
  do {
    ...
    if (Tok.is(tok::l_paren)) {
      ...
      default:
        toolchain_unreachable("Unexpected allocate modifier");
      }
    } else {
