...
#include "language/Core/Parse/ParseHLSLRootSignature.h"

#include "language/Core/Lex/LiteralSupport.h"

using namespace toolchain::hlsl::rootsig;
...
RootSignatureParser::RootSignatureParser(
    toolchain::dxbc::RootSignatureVersion Version, StringLiteral *Signature,
    Preprocessor &PP)
...
static FlagType maybeOrFlag(std::optional<FlagType> Flags, FlagType Flag) {
  ...
  return static_cast<FlagType>(toolchain::to_underlying(Flags.value()) |
                               toolchain::to_underlying(Flag));
}

std::optional<toolchain::dxbc::RootFlags> RootSignatureParser::parseRootFlags() {
  ...
  std::optional<toolchain::dxbc::RootFlags> Flags = toolchain::dxbc::RootFlags::None;
...
#define ROOT_FLAG_ENUM(NAME, LIT)                                              \
  case TokenKind::en_##NAME:                                                   \
    Flags = maybeOrFlag<toolchain::dxbc::RootFlags>(Flags,                          \
                                               toolchain::dxbc::RootFlags::NAME);   \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
        default:
          toolchain_unreachable("Switch for consumed enum token was not provided");
        }
      } else {
...
std::optional<RootDescriptor> RootSignatureParser::parseRootDescriptor() {
  ...
  default:
    toolchain_unreachable("Switch for consumed token was not provided");
...
#define SHADER_VISIBILITY_ENUM(NAME, LIT)                                      \
  case TokenKind::en_##NAME:                                                   \
    return toolchain::dxbc::ShaderVisibility::NAME;                                 \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
  default:
    toolchain_unreachable("Switch for consumed enum token was not provided");
...
#define FILTER_ENUM(NAME, LIT)                                                 \
  case TokenKind::en_##NAME:                                                   \
    return toolchain::dxbc::SamplerFilter::NAME;                                    \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
  default:
    toolchain_unreachable("Switch for consumed enum token was not provided");
...
#define TEXTURE_ADDRESS_MODE_ENUM(NAME, LIT)                                   \
  case TokenKind::en_##NAME:                                                   \
    return toolchain::dxbc::TextureAddressMode::NAME;                               \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
  default:
    toolchain_unreachable("Switch for consumed enum token was not provided");
...
#define COMPARISON_FUNC_ENUM(NAME, LIT)                                        \
  case TokenKind::en_##NAME:                                                   \
    return toolchain::dxbc::ComparisonFunc::NAME;                                   \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
  default:
    toolchain_unreachable("Switch for consumed enum token was not provided");
...
#define STATIC_BORDER_COLOR_ENUM(NAME, LIT)                                    \
  case TokenKind::en_##NAME:                                                   \
    return toolchain::dxbc::StaticBorderColor::NAME;                                \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
  default:
    toolchain_unreachable("Switch for consumed enum token was not provided");
...
RootSignatureParser::parseRootDescriptorFlags(TokenKind Context) {
  ...
  if (tryConsumeExpectedToken(TokenKind::int_literal)) {
    ...
    return toolchain::dxbc::RootDescriptorFlags::None;
  }
...
#define ROOT_DESCRIPTOR_FLAG_ENUM(NAME, LIT)                                   \
  case TokenKind::en_##NAME:                                                   \
    Flags = maybeOrFlag<toolchain::dxbc::RootDescriptorFlags>(                      \
        Flags, toolchain::dxbc::RootDescriptorFlags::NAME);                         \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
      default:
        toolchain_unreachable("Switch for consumed enum token was not provided");
...
RootSignatureParser::parseDescriptorRangeFlags(TokenKind Context) {
  ...
  if (tryConsumeExpectedToken(TokenKind::int_literal)) {
    ...
    return toolchain::dxbc::DescriptorRangeFlags::None;
  }
...
#define DESCRIPTOR_RANGE_FLAG_ENUM(NAME, LIT, ON)                              \
  case TokenKind::en_##NAME:                                                   \
    Flags = maybeOrFlag<toolchain::dxbc::DescriptorRangeFlags>(                     \
        Flags, toolchain::dxbc::DescriptorRangeFlags::NAME);                        \
    break;
#include "language/Core/Lex/HLSLRootSignatureTokenKinds.def"
      default:
        toolchain_unreachable("Switch for consumed enum token was not provided");
...
std::optional<uint32_t> RootSignatureParser::handleUIntLiteral() {
  ...
  language::Core::NumericLiteralParser Literal(
  ...
  toolchain::APSInt Val(32, /*IsUnsigned=*/true);
...
std::optional<float> RootSignatureParser::handleFloatLiteral(bool Negated) {
  ...
  // to:
  auto DXCSemantics = toolchain::APFloat::Semantics::S_IEEEdouble;
  auto DXCRoundingMode = toolchain::RoundingMode::NearestTiesToEven;

  toolchain::APFloat Val(toolchain::APFloat::EnumToSemantics(DXCSemantics));
  toolchain::APFloat::opStatus Status(Literal.GetFloatValue(Val, DXCRoundingMode));
  ...
  assert(!(Status & toolchain::APFloat::opStatus::opInvalidOp) &&
         "NumSpelling consists only of [0-9.ef+-]. Any malformed NumSpelling "
         "will be caught and reported by NumericLiteralParser.");

  assert(!(Status & toolchain::APFloat::opStatus::opDivByZero) &&
         "It is not possible for a division to be performed when "
         "constructing an APFloat from a string");

  if (Status & toolchain::APFloat::opStatus::opUnderflow) {
    // Report that the value has underflowed
    reportDiag(diag::err_hlsl_number_literal_underflow);
    return std::nullopt;
  }

  if (Status & toolchain::APFloat::opStatus::opOverflow) {
    // Report that the value has overflowed
...
bool RootSignatureParser::peekExpectedToken(ArrayRef<TokenKind> AnyExpected) {
  ...
  return toolchain::is_contained(AnyExpected, Result.TokKind);
}
