...
void Parser::ParseObjCTypeQualifierList(ObjCDeclSpec &DS,
  ...
  while (true) {
    ...
    const IdentifierInfo *II = Tok.getIdentifierInfo();
    for (unsigned i = 0; i != toolchain::to_underlying(ObjCTypeQual::NumQuals);
         ++i) {
      ObjCTypeQual TQ = static_cast<ObjCTypeQual>(i);
      if (II != ObjCTypeQuals[toolchain::to_underlying(TQ)] ||
      ...
      NullabilityKind Nullability;
      switch (TQ) {
      default: toolchain_unreachable("Unknown decl qualifier");
...
static void takeDeclAttributes(ParsedAttributesView &attrs,
  ...
  for (auto &AL : toolchain::reverse(from)) {
...
ExprResult Parser::ParseObjCAtExpression(SourceLocation AtLoc) {
  ...
  case tok::plus: {
    ...
    if (!Tok.is(tok::numeric_constant)) {
      ...
      default: toolchain_unreachable("missing unary operator case");
      }
