...
#include "language/Core/Parse/LoopHint.h"
#include "language/Core/Parse/Parser.h"
#include "language/Core/Parse/RAIIObjectsForParser.h"
...
void markAsReinjectedForRelexing(toolchain::MutableArrayRef<language::Core::Token> Toks) {
  for (auto &T : Toks)
    T.setFlag(language::Core::Token::IsReinjected);
}
}  // end namespace
...
void Parser::HandlePragmaFEnvRound() {
  ...
  auto RM = static_cast<toolchain::RoundingMode>(
      reinterpret_cast<uintptr_t>(Tok.getAnnotationValue()));
...
void Parser::HandlePragmaMSPragma() {
  ...
  PragmaHandler Handler =
      toolchain::StringSwitch<PragmaHandler>(PragmaName)
...
bool Parser::HandlePragmaMSSection(StringRef PragmaName,
  ...
  while (Tok.is(tok::comma)) {
    ...
    ASTContext::PragmaSectionFlag Flag =
      toolchain::StringSwitch<ASTContext::PragmaSectionFlag>(
...
bool Parser::HandlePragmaMSInitSeg(StringRef PragmaName,
                                   SourceLocation PragmaLocation) {
  if (getTargetInfo().getTriple().getEnvironment() != toolchain::Triple::MSVC) {
  ...
  if (Tok.isAnyIdentifier()) {
    ...
    StringRef Section = toolchain::StringSwitch<StringRef>(II->getName())
...
static std::string PragmaLoopHintString(Token PragmaName, Token Option) {
  ...
  return std::string(toolchain::StringSwitch<StringRef>(Str)
...
bool Parser::HandlePragmaLoopHint(LoopHint &Hint) {
  ...
  toolchain::ArrayRef<Token> Toks = Info->Toks;

  // Return a valid hint if pragma unroll or nounroll were specified
  // without an argument.
  auto IsLoopHint = toolchain::StringSwitch<bool>(PragmaNameInfo->getName())
  ...
  if (OptionInfo) { // Pragma Unroll does not specify an option.
    ...
    StateOption = toolchain::StringSwitch<bool>(OptionInfo->getName())
                      .Case("vectorize", true)
  ...
  if (StateOption) {
    ...
    bool Valid = StateInfo &&
                 toolchain::StringSwitch<bool>(StateInfo->getName())
                     .Case("disable", true)
...
#include "language/Core/Parse/AttrSubMatchRulesParserStringSwitches.inc"
...
#include "language/Core/Basic/AttrSubMatchRulesList.inc"
  ...
  toolchain_unreachable("Invalid attribute subject match rule");
  return false;
}
...
DiagnosticBuilder createExpectedAttributeSubjectRulesTokenDiagnostic(
  ...
  if (EndPoint == MissingAttributeSubjectRulesRecoveryPoint::None) {
    ...
    toolchain::BitVector IsMatchRuleAvailable(attr::SubjectMatchRule_Last + 1, true);
    ...
    for (const ParsedAttr &Attribute : Attrs) {
      ...
      toolchain::BitVector IsSupported(attr::SubjectMatchRule_Last + 1);
...
void PragmaFloatControlHandler::HandlePragma(Preprocessor &PP,
  ...
  PragmaFloatControlKind Kind =
      toolchain::StringSwitch<PragmaFloatControlKind>(II->getName())
...
void PragmaCommentHandler::HandlePragma(Preprocessor &PP,
  ...
  PragmaMSCommentKind Kind =
    toolchain::StringSwitch<PragmaMSCommentKind>(II->getName())
...
void PragmaFPHandler::HandlePragma(Preprocessor &PP,
  ...
  while (Tok.is(tok::identifier)) {
    ...
    auto FlagKind =
        toolchain::StringSwitch<std::optional<PragmaFPKind>>(OptionInfo->getName())
    ...
    if (FlagKind == PFK_Contract) {
      AnnotValue->ContractValue =
          toolchain::StringSwitch<std::optional<LangOptions::FPModeKind>>(
              II->getName())
      ...
      Value = toolchain::StringSwitch<std::optional<LangOptions::FPModeKind>>(
                  II->getName())
                  .Case("on", LangOptions::FPModeKind::FPM_On)
                  .Case("off", LangOptions::FPModeKind::FPM_Off)
                  .Default(std::nullopt);
      ...
      AnnotValue->ExceptionsValue =
          toolchain::StringSwitch<std::optional<LangOptions::FPExceptionModeKind>>(
              II->getName())
      ...
      AnnotValue->EvalMethodValue =
          toolchain::StringSwitch<std::optional<LangOptions::FPEvalMethodKind>>(
              II->getName())
...
void PragmaSTDC_FENV_ROUNDHandler::HandlePragma(Preprocessor &PP,
  ...
  auto RM =
      toolchain::StringSwitch<toolchain::RoundingMode>(II->getName())
          .Case("FE_TOWARDZERO", toolchain::RoundingMode::TowardZero)
          .Case("FE_TONEAREST", toolchain::RoundingMode::NearestTiesToEven)
          .Case("FE_UPWARD", toolchain::RoundingMode::TowardPositive)
          .Case("FE_DOWNWARD", toolchain::RoundingMode::TowardNegative)
          .Case("FE_TONEARESTFROMZERO", toolchain::RoundingMode::NearestTiesToAway)
          .Case("FE_DYNAMIC", toolchain::RoundingMode::Dynamic)
          .Default(toolchain::RoundingMode::Invalid);
  if (RM == toolchain::RoundingMode::Invalid) {
...
static bool ParseLoopHintValue(Preprocessor &PP, Token &Tok, Token PragmaName,
  ...
  Info.Toks = toolchain::ArrayRef(ValueList).copy(PP.getPreprocessorAllocator());
...
///    constant-expression
///
/// Specifying vectorize(enable) or vectorize_width(_value_) instructs toolchain to
/// try vectorizing the instructions of the loop it precedes. Specifying
/// interleave(enable) or interleave_count(_value_) instructs toolchain to try
...
/// unroller. Specifying unroll(enable) instructs toolchain to unroll the loop
...
/// loop. Specifying unroll_count(_value_) instructs toolchain to try to unroll the
...
void PragmaLoopHintHandler::HandlePragma(Preprocessor &PP,
  ...
  while (Tok.is(tok::identifier)) {
    ...
    bool OptionValid = toolchain::StringSwitch<bool>(OptionInfo->getName())
                           .Case("vectorize", true)
                           .Case("interleave", true)
                           .Case("unroll", true)
...
/// contained in parentheses. With no argument the directive instructs toolchain to
...
bool Parser::HandlePragmaMSFunction(StringRef PragmaName,
  ...
  toolchain::SmallVector<StringRef> NoBuiltins;
  while (Tok.is(tok::identifier)) {
    IdentifierInfo *II = Tok.getIdentifierInfo();
    if (!II->getBuiltinID())
...
void PragmaAttributeHandler::HandlePragma(Preprocessor &PP,
  ...
  if ((Info->Action == PragmaAttributeInfo::Push && Tok.isNot(tok::eod)) ||
    ...
    Info->Tokens =
        toolchain::ArrayRef(AttributeTokens).copy(PP.getPreprocessorAllocator());
  }
