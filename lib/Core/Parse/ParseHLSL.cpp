...
void Parser::ParseHLSLAnnotations(ParsedAttributes &Attrs,
  ...
  case ParsedAttr::AT_HLSLPackOffset: {
    ...
    ArgExprs.push_back(IntegerLiteral::Create(
        Ctx, toolchain::APInt(SizeTySize, SubComponent), SizeTy, SubComponentLoc));
    ArgExprs.push_back(IntegerLiteral::Create(
        Ctx, toolchain::APInt(SizeTySize, Component), SizeTy, ComponentLoc));
    if (ExpectAndConsume(tok::r_paren, diag::err_expected)) {
      SkipUntil(tok::r_paren, StopAtSemi); // skip through )
      return;
    }
  ...
  case ParsedAttr::AT_HLSLSV_Position:
    break;
  default:
    toolchain_unreachable("invalid HLSL Annotation");
    break;
  }
