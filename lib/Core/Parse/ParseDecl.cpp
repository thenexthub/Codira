...
TypeResult Parser::ParseTypeName(SourceRange *Range, DeclaratorContext Context,
  ...
  if (Attrs) {
    toolchain::SmallVector<ParsedAttr *, 1> ToBeMoved;
...
#define CLANG_ATTR_LATE_PARSED_EXPERIMENTAL_EXT_LIST
  return toolchain::StringSwitch<bool>(normalizeAttrName(II.getName()))
#include "language/Core/Parse/AttrParserStringSwitches.inc"
      .Default(false);
...
static bool attributeHasIdentifierArg(const toolchain::Triple &T,
...
attributeStringLiteralListArg(const toolchain::Triple &T, const IdentifierInfo &II,
                              ParsedAttr::Syntax Syntax,
                              IdentifierInfo *ScopeName) {
#define CLANG_ATTR_STRING_LITERAL_ARG_LIST
  return toolchain::StringSwitch<uint32_t>(normalizeAttrName(II.getName()))
#include "language/Core/Parse/AttrParserStringSwitches.inc"
      .Default(0);
...
unsigned Parser::ParseAttributeArgsCommon(
  ...
  if (!ArgExprs.empty() ? Tok.is(tok::comma) : Tok.isNot(tok::r_paren)) {
    ...
    if (AttributeIsTypeArgAttr) {
      ...
      toolchain::append_range(ArgExprs, ParsedExprs);
    }
  }
...
bool Parser::DiagnoseProhibitedCXX11Attribute() {
  ...
  toolchain_unreachable("All cases handled above.");
}
...
Parser::getDeclSpecContextFromDeclaratorContext(DeclaratorContext Context) {
  ...
  toolchain_unreachable("Missing DeclaratorContext case");
}
...
void Parser::ParseBoundsAttribute(IdentifierInfo &AttrName,
  ...
  ArgExprs.push_back(IntegerLiteral::Create(
      Ctx, toolchain::APInt(Ctx.getTypeSize(Ctx.getSizeType()), 0),
  ...
  if (Tok.isOneOf(tok::kw_typename, tok::annot_typename)) {
    ...
    if (Next.isOneOf(tok::star, tok::amp, tok::ampamp, tok::identifier,
      ...
      case NameClassificationKind::Keyword:
        toolchain_unreachable("typo correction is not possible here");
...
void Parser::ParseDeclarationSpecifiers(
  ...
  while (true) {
    ...
    if (getLangOpts().MSVCCompat && Tok.is(tok::kw__Atomic) &&
        DS.getStorageClassSpec() == language::Core::DeclSpec::SCS_typedef &&
...
void Parser::ParseTypeQualifierListOpt(
    ...
    bool IdentifierRequired, toolchain::function_ref<void()> CodeCompletionHandler) {
...
void Parser::ParseFunctionDeclarator(Declarator &D,
  ...
  if (getCurScope()->isFunctionDeclarationScope() && !getLangOpts().CPlusPlus) {
    ...
    toolchain::sort(DeclsInPrototype, [](Decl *D1, Decl *D2) {
...
void Parser::ParseFunctionDeclaratorIdentifierList(
  ...
  toolchain::SmallSet<const IdentifierInfo*, 16> ParamsSoFar;
...
void Parser::ParseParameterDeclarationClause(
  ...
  // See https://bugs.toolchain.org/show_bug.cgi?id=19607
...
TypeResult Parser::ParseTypeFromString(StringRef TypeStr, StringRef Context,
  ...
  SmallVector<Token, 4> Tokens;
  {
    ...
    FileID FID = SourceMgr.createFileID(
        toolchain::MemoryBuffer::getMemBufferCopy(TypeStr, Context), SrcMgr::C_User,
        0, 0, IncludeLoc);
