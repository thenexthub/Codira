...
#include "language/Core/Basic/TransformTypeTraits.def"
  default:
    toolchain_unreachable("passed in an unhandled type transformation built-in");
  }
}
...
#include "language/Core/Basic/TransformTypeTraits.def"
  ...
  if (TemplateId) {
    ...
    TagOrTempResult = Actions.ActOnTag(
        ...
        SourceLocation(), false, language::Core::TypeResult(),
  ...
  if (Tok.is(tok::kw_concept)) {
    Diag(
        ...
        DS.isFriendSpecified() || NextToken().is(tok::kw_friend)
            ? toolchain::to_underlying(diag::err_friend_concept)
            : toolchain::to_underlying(
                  diag::
...
void Parser::ParseCXXMemberSpecification(SourceLocation RecordLoc,
  ...
  toolchain::TimeTraceScope TimeScope("ParseClass", [&]() {
