...
void Parser::Initialize() {
  ...
  if (getLangOpts().ObjC) {
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::in)] =
        &PP.getIdentifierTable().get("in");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::out)] =
        &PP.getIdentifierTable().get("out");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::inout)] =
        &PP.getIdentifierTable().get("inout");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::oneway)] =
        &PP.getIdentifierTable().get("oneway");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::bycopy)] =
        &PP.getIdentifierTable().get("bycopy");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::byref)] =
        &PP.getIdentifierTable().get("byref");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::nonnull)] =
        &PP.getIdentifierTable().get("nonnull");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::nullable)] =
        &PP.getIdentifierTable().get("nullable");
    ObjCTypeQuals[toolchain::to_underlying(ObjCTypeQual::null_unspecified)] =
...
Parser::ParseExternalDeclaration(ParsedAttributes &Attrs,
  ...
  case tok::kw_import: {
    ...
    if (getLangOpts().CPlusPlusModules) {
      toolchain_unreachable("not expecting a c++20 import here");
      ProhibitAttributes(Attrs);
    }
  ...
  if (Tok.is(tok::semi)) {
    auto LengthOfTSTToken = [](DeclSpec::TST TKind) {
      ...
      case DeclSpec::TST_interface:
        return 9;
      default:
        toolchain_unreachable("we only expect to get the length of the class/struct/union/enum");
      }
...
Parser::DeclGroupPtrTy Parser::ParseDeclarationOrFunctionDefinition(
  ...
  // "EvaluateAsConstExpr".
  toolchain::TimeTraceScope TimeScope("ParseDeclarationOrFunctionDefinition", [&]() {
...
Decl *Parser::ParseFunctionDefinition(ParsingDeclarator &D,
  ...
  toolchain::TimeTraceScope TimeScope("ParseFunctionDefinition", [&]() {
  ...
  if (TryConsumeToken(tok::equal)) {
    ...
    if (TryConsumeToken(tok::kw_delete, KWLoc)) {
      ...
      toolchain_unreachable("function definition after = not 'delete' or 'default'");
    }
...
void Parser::CodeCompleteIncludedFile(toolchain::StringRef Dir, bool IsAngled) {
...
void Parser::ParseMicrosoftIfExistsExternalDeclaration() {
  ...
  case IfExistsBehavior::Dependent:
    toolchain_unreachable("Cannot have a dependent external declaration");
...
Decl *Parser::ParseModuleImport(SourceLocation AtLoc,
  ...
  if (IsObjCAtImport && AtLoc.isValid()) {
    ...
    if (FE && toolchain::sys::path::parent_path(FE->getDir().getName())
                  .ends_with(".framework"))
