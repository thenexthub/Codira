...
#include "toolchain/MC/MCAsmInfo.h"
#include "toolchain/MC/MCContext.h"
#include "toolchain/MC/MCInstPrinter.h"
#include "toolchain/MC/MCInstrInfo.h"
#include "toolchain/MC/MCObjectFileInfo.h"
#include "toolchain/MC/MCParser/MCAsmParser.h"
#include "toolchain/MC/MCParser/MCTargetAsmParser.h"
#include "toolchain/MC/MCRegisterInfo.h"
#include "toolchain/MC/MCStreamer.h"
...
namespace {
class ClangAsmParserCallback : public toolchain::MCAsmParserSemaCallback {
  Parser &TheParser;
...
public:
  ...
  StringRef LookupInlineAsmLabel(StringRef Identifier, toolchain::SourceMgr &LSM,
                                 toolchain::SMLoc Location,
  ...
  static void DiagHandlerCallback(const toolchain::SMDiagnostic &D, void *Context) {
...
private:
  ...
  SourceLocation translateLocation(const toolchain::SourceMgr &LSM,
                                   toolchain::SMLoc SMLoc);

  void handleDiagnostic(const toolchain::SMDiagnostic &D);
};
}

void ClangAsmParserCallback::LookupInlineAsmIdentifier(
    StringRef &LineBuf, toolchain::InlineAsmIdentifierInfo &Info,
...
void ClangAsmParserCallback::findTokensForString(
  ...
  const unsigned *FirstTokOffset =
      toolchain::lower_bound(AsmTokOffsets, FirstCharOffset);
...
SourceLocation
ClangAsmParserCallback::translateLocation(const toolchain::SourceMgr &LSM,
                                          toolchain::SMLoc SMLoc) {
  // Compute an offset into the inline asm buffer.
  // FIXME: This isn't right if .macro is involved (but hopefully, no
  // real-world code does that).
  const toolchain::MemoryBuffer *LBuf =
      LSM.getMemoryBuffer(LSM.FindBufferContainingLoc(SMLoc));
  unsigned Offset = SMLoc.getPointer() - LBuf->getBufferStart();

  // Figure out which token that offset points into.
  const unsigned *TokOffsetPtr = toolchain::lower_bound(AsmTokOffsets, Offset);
...
void ClangAsmParserCallback::handleDiagnostic(const toolchain::SMDiagnostic &D) {
...
ExprResult Parser::ParseMSAsmIdentifier(toolchain::SmallVectorImpl<Token> &LineToks,
...
StmtResult Parser::ParseMicrosoftAsmStatement(SourceLocation AsmLoc) {
  ...
  const toolchain::Triple &TheTriple = Actions.Context.getTargetInfo().getTriple();
  const std::string &TT = TheTriple.getTriple();
  const toolchain::Target *TheTarget = nullptr;
  if (!TheTriple.isX86()) {
    Diag(AsmLoc, diag::err_msasm_unsupported_arch) << TheTriple.getArchName();
  } else {
    std::string Error;
    TheTarget = toolchain::TargetRegistry::lookupTarget(TT, Error);
    if (!TheTarget)
  ...
  std::string FeaturesStr =
      toolchain::join(TO.Features.begin(), TO.Features.end(), ",");

  std::unique_ptr<toolchain::MCRegisterInfo> MRI(TheTarget->createMCRegInfo(TT));
  if (!MRI) {
  ...
  toolchain::MCTargetOptions MCOptions;
  std::unique_ptr<toolchain::MCAsmInfo> MAI(
      TheTarget->createMCAsmInfo(*MRI, TT, MCOptions));
  // Get the instruction descriptor.
  std::unique_ptr<toolchain::MCInstrInfo> MII(TheTarget->createMCInstrInfo());
  ...
  toolchain::SourceMgr TempSrcMgr;
  toolchain::MCContext Ctx(TheTriple, MAI.get(), MRI.get(), STI.get(), &TempSrcMgr);
  std::unique_ptr<toolchain::MCObjectFileInfo> MOFI(
      TheTarget->createMCObjectFileInfo(Ctx, /*PIC=*/false));
  Ctx.setObjectFileInfo(MOFI.get());

  std::unique_ptr<toolchain::MemoryBuffer> Buffer =
      toolchain::MemoryBuffer::getMemBuffer(AsmString, "<MS inline asm>");

  // Tell SrcMgr about this buffer, which is what the parser will pick up.
  TempSrcMgr.AddNewSourceBuffer(std::move(Buffer), toolchain::SMLoc());

  std::unique_ptr<toolchain::MCStreamer> Str(createNullStreamer(Ctx));
  std::unique_ptr<toolchain::MCAsmParser> Parser(
      createMCAsmParser(TempSrcMgr, Ctx, *Str, *MAI));

  std::unique_ptr<toolchain::MCTargetAsmParser> TargetParser(
  ...
  std::unique_ptr<toolchain::MCInstPrinter> IP(
      TheTarget->createMCInstPrinter(toolchain::Triple(TT), 1, *MAI, *MII, *MRI));
  ...
  toolchain::erase_if(Clobbers, [](const std::string &C) {
    return C == "fpsr" || C == "mxcsr";
  });

  // Build the vector of clobber StringRefs.
  toolchain::append_range(ClobberRefs, Clobbers);
...
const char *Parser::GNUAsmQualifiers::getQualifierName(AQ Qualifier) {
  ...
  toolchain_unreachable("Unknown GNUAsmQualifier");
}
