...
OpenACCDirectiveKindEx getOpenACCDirectiveKind(Token Tok) {
  ...
  OpenACCDirectiveKind DirKind =
      toolchain::StringSwitch<OpenACCDirectiveKind>(
  ...
  return toolchain::StringSwitch<OpenACCDirectiveKindEx>(
             Tok.getIdentifierInfo()->getName())
...
OpenACCClauseKind getOpenACCClauseKind(Token Tok) {
  ...
  return toolchain::StringSwitch<OpenACCClauseKind>(
             Tok.getIdentifierInfo()->getName())
...
OpenACCAtomicKind getOpenACCAtomicKind(Token Tok) {
  ...
  return toolchain::StringSwitch<OpenACCAtomicKind>(
             Tok.getIdentifierInfo()->getName())
...
OpenACCDefaultClauseKind getOpenACCDefaultClauseKind(Token Tok) {
  ...
  return toolchain::StringSwitch<OpenACCDefaultClauseKind>(
             Tok.getIdentifierInfo()->getName())
...
bool isOpenACCSpecialToken(OpenACCSpecialTokenKind Kind, Token Tok) {
  ...
  toolchain_unreachable("Unknown 'Kind' Passed");
}
...
OpenACCReductionOperator ParseReductionOperator(Parser &P) {
  ...
  toolchain_unreachable("Reduction op token kind not caught by 'default'?");
}
...
ClauseParensKind getClauseParensKind(OpenACCDirectiveKind DirKind,
  ...
  toolchain_unreachable("Unhandled clause kind");
}
...
bool doesDirectiveHaveAssociatedStmt(OpenACCDirectiveKind DirKind) {
  ...
  toolchain_unreachable("Unhandled directive->assoc stmt");
}
...
unsigned getOpenACCScopeFlags(OpenACCDirectiveKind DirKind) {
  ...
  case OpenACCDirectiveKind::Update:
    return 0;
  case OpenACCDirectiveKind::Invalid:
    toolchain_unreachable("Shouldn't be creating a scope for an invalid construct");
  }
  toolchain_unreachable("Shouldn't be creating a scope for an invalid construct");
}
...
bool Parser::ParseOpenACCDeviceTypeList(
    toolchain::SmallVector<IdentifierLoc> &Archs) {
...
bool Parser::ParseOpenACCSizeExprList(
    OpenACCClauseKind CK, toolchain::SmallVectorImpl<Expr *> &SizeExprs) {
...
bool Parser::ParseOpenACCGangArgList(
    SourceLocation GangLoc, toolchain::SmallVectorImpl<OpenACCGangKind> &GKs,
    toolchain::SmallVectorImpl<Expr *> &IntExprs) {
...
Parser::OpenACCClauseParseResult Parser::ParseOpenACCClauseParams(
  ...
  if (ClauseHasRequiredParens(DirKind, ClauseKind)) {
    ...
    default:
      toolchain_unreachable("Not a required parens type?");
    }
    ...
    if (!Parens.consumeOpen()) {
      ...
      case OpenACCClauseKind::Gang: {
        toolchain::SmallVector<OpenACCGangKind> GKs;
        toolchain::SmallVector<Expr *> IntExprs;
      ...
      default:
        toolchain_unreachable("Not an optional parens type?");
      }
      ParsedClause.setEndLoc(getCurToken().getLocation());
      if (Parens.consumeClose())
        return OpenACCCannotContinue();
...
std::variant<std::monostate, language::Core::StringLiteral *, IdentifierInfo *>
...
toolchain::SmallVector<Expr *> Parser::ParseOpenACCVarList(OpenACCDirectiveKind DK,
                                                      OpenACCClauseKind CK) {
  toolchain::SmallVector<Expr *> Vars;
...
Parser::ParseOpenACCDirective() {
  ...
  if (DirKind == OpenACCDirectiveKind::Wait)
    ...
    ParseInfo.Exprs = toolchain::SmallVector<Expr *>(1, RoutineName.get());
