...
Retry:
  ...
  default: {
    ...
    bool AllAttrsAreStmtAttrs = toolchain::all_of(CXX11Attrs, IsStmtAttr) &&
                                toolchain::all_of(GNUAttrs, IsStmtAttr);
  ...
  if (IsConsteval) {
    auto IsCompoundStatement = [](const Stmt *S) {
      if (const auto *Outer = dyn_cast_if_present<AttributedStmt>(S))
        S = Outer->getSubStmt();
      return isa_and_nonnull<language::Core::CompoundStmt>(S);
    };
...
bool Parser::trySkippingFunctionBody() {
  ...
  if (toolchain::any_of(Toks, [](const Token &Tok) {
...
void Parser::ParseMicrosoftIfExistsStatement(StmtVector &Stmts) {
  ...
  case IfExistsBehavior::Dependent:
    toolchain_unreachable("Dependent case handled above");
