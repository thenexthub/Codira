...
/// Default \c PathComparator using \c toolchain::sys::fs::equivalent().
struct DefaultPathComparator : public PathComparator {
  bool equivalent(StringRef FileA, StringRef FileB) const override {
    return FileA == FileB || toolchain::sys::fs::equivalent(FileA, FileB);
  }
};
...
public:
  ...
  void insert(StringRef NewPath, unsigned ConsumedLength = 0) {
    ...
    if (toolchain::sys::path::is_relative(NewPath))
      return;
    if (Path.empty()) {
    ...
    if (Children.empty()) {
      ...
      StringRef Element(toolchain::sys::path::filename(
  ...
  StringRef findEquivalent(const PathComparator& Comparator,
    ...
    if (Children.empty()) {
      ...
      if (toolchain::sys::path::filename(Path) ==
              toolchain::sys::path::filename(FileName) &&
          Comparator.equivalent(StringRef(Path), FileName))
        return StringRef(Path);
      return {};
    }
    StringRef Element(toolchain::sys::path::filename(FileName.drop_back(
        ConsumedLength)));
    toolchain::StringMap<FileMatchTrieNode>::const_iterator MatchingChild =
...
private:
  ...
  void getAll(std::vector<StringRef> &Results,
              toolchain::StringMap<FileMatchTrieNode>::const_iterator Except) const {
    if (Path.empty())
      return;
    if (Children.empty()) {
    ...
    for (toolchain::StringMap<FileMatchTrieNode>::const_iterator
  ...
  toolchain::StringMap<FileMatchTrieNode> Children;
};
