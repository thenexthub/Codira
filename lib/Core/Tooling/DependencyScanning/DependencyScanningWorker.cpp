...
public:
  ...
  void finishedMainFile(DiagnosticsEngine &Diags) override {
    ...
    toolchain::SmallString<256> CanonPath;
    for (const auto &File : getDependencies()) {
      CanonPath = File;
      toolchain::sys::path::remove_dots(CanonPath, /*remove_dot_dot=*/true);
      toolchain::sys::fs::make_absolute(WorkingDirectory, CanonPath);
      ...
      if (Diags) {
        ...
        auto VFSNote = [&](int Type, ArrayRef<std::string> VFSOverlays) {
          if (VFSOverlays.empty()) {
            ...
            std::string Files = toolchain::join(VFSOverlays, "\n");
...
public:
  PrebuiltModuleListener(PrebuiltModuleFilesT &PrebuiltModuleFiles,
                         toolchain::SmallVector<std::string> &NewModuleFiles,
  ...
  bool ReadHeaderSearchPaths(const HeaderSearchOptions &HSOpts,
    ...
    PrebuiltModule.setVFS(
        toolchain::StringSet<>(toolchain::from_range, HSOpts.VFSOverlayFiles));
...
static bool visitPrebuiltModule(StringRef PrebuiltModuleFilename,
  ...
  toolchain::SmallVector<std::string> Worklist;
...
static std::string makeObjFileName(StringRef FileName) {
  ...
  toolchain::sys::path::replace_extension(ObjFileName, "o");
...
static void sanitizeDiagOpts(DiagnosticOptions &DiagOpts) {
  ...
  toolchain::erase_if(DiagOpts.Warnings, [](StringRef Warning) {
    return toolchain::StringSwitch<bool>(Warning)
...
static void canonicalizeDefines(PreprocessorOptions &PPOpts) {
  ...
  toolchain::stable_sort(SimpleNames, toolchain::less_first());
...
public:
  ScanningDependencyDirectivesGetter(FileManager &FileMgr) : DepFS(nullptr) {
    FileMgr.getVirtualFileSystem().visit([&](toolchain::vfs::FileSystem &FS) {
      auto *DFS = toolchain::dyn_cast<DependencyScanningWorkerFilesystem>(&FS);
      if (DFS) {
...
public:
  DependencyScanningAction(
      ...
      toolchain::IntrusiveRefCntPtr<DependencyScanningWorkerFilesystem> DepFS,
  ...
  bool runInvocation(std::shared_ptr<CompilerInvocation> Invocation,
    ...
    // those directories.
    toolchain::SmallVector<StringRef> StableDirs;
    const StringRef Sysroot = ScanInstance.getHeaderSearchOpts().Sysroot;
    if (!Sysroot.empty() &&
        (toolchain::sys::path::root_directory(Sysroot) != Sysroot))
...
DependencyScanningWorker::DependencyScanningWorker(
  ...
  if (Service.shouldTraceVFS())
    FS = toolchain::makeIntrusiveRefCnt<toolchain::vfs::TracingFileSystem>(std::move(FS));

  switch (Service.getMode()) {
  case ScanningMode::DependencyDirectivesScan:
    DepFS = toolchain::makeIntrusiveRefCnt<DependencyScanningWorkerFilesystem>(
...
toolchain::Error DependencyScanningWorker::computeDependencies(
  ...
  toolchain::raw_string_ostream DiagnosticsOS(DiagnosticOutput);
  ...
  return toolchain::make_error<toolchain::StringError>(DiagnosticsOS.str(),
...
static bool forEachDriverJob(
    ...
    IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FS,
    toolchain::function_ref<bool(const driver::Command &Cmd)> Callback) {
  ...
  std::unique_ptr<driver::Driver> Driver = std::make_unique<driver::Driver>(
      Argv[0], toolchain::sys::getDefaultTargetTriple(), Diags,
...
static bool createAndRunToolInvocation(
    ...
    IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FS,
    std::shared_ptr<language::Core::PCHContainerOperations> &PCHContainerOps,
...
bool DependencyScanningWorker::scanDependencies(
    StringRef WorkingDirectory, const std::vector<std::string> &CommandLine,
    DependencyConsumer &Consumer, DependencyActionController &Controller,
    DiagnosticConsumer &DC, toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FS,
    std::optional<StringRef> ModuleName) {
  std::vector<const char *> CCommandLine(CommandLine.size(), nullptr);
  toolchain::transform(CommandLine, CCommandLine.begin(),
  ...
  if (CommandLine[1] == "-cc1") {
    ...
    Success = forEachDriverJob(
        CommandLine, *Diags, FS, [&](const driver::Command &Cmd) {
          ...
          toolchain::append_range(Argv, Cmd.getArguments());
...
bool DependencyScanningWorker::computeDependencies(
    StringRef WorkingDirectory, const std::vector<std::string> &CommandLine,
    DependencyConsumer &Consumer, DependencyActionController &Controller,
    DiagnosticConsumer &DC, std::optional<toolchain::MemoryBufferRef> TUBuffer) {
  // Reset what might have been modified in the previous worker invocation.
  BaseFS->setCurrentWorkingDirectory(WorkingDirectory);

  std::optional<std::vector<std::string>> ModifiedCommandLine;
  toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> ModifiedFS;
  ...
  if (TUBuffer) {
    ...
    InMemoryFS->addFile(
        InputPath, 0,
        toolchain::MemoryBuffer::getMemBufferCopy(TUBuffer->getBuffer()));
    toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> InMemoryOverlay =
        InMemoryFS;
...
bool DependencyScanningWorker::computeDependencies(
  ...
  auto OverlayFS =
      toolchain::makeIntrusiveRefCnt<toolchain::vfs::OverlayFileSystem>(BaseFS);
  auto InMemoryFS = toolchain::makeIntrusiveRefCnt<toolchain::vfs::InMemoryFileSystem>();
  InMemoryFS->setCurrentWorkingDirectory(WorkingDirectory);
  SmallString<128> FakeInputPath;
  // TODO: We should retry the creation if the path already exists.
  toolchain::sys::fs::createUniquePath(ModuleName + "-%%%%%%%%.input", FakeInputPath,
                                  /*MakeAbsolute=*/false);
  InMemoryFS->addFile(FakeInputPath, 0, toolchain::MemoryBuffer::getMemBuffer(""));
  toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem> InMemoryOverlay = InMemoryFS;
