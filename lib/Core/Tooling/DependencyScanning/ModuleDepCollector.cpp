...
void ModuleDeps::forEachFileDep(toolchain::function_ref<void(StringRef)> Cb) const {
...
optimizeHeaderSearchOpts(HeaderSearchOptions &Opts, ASTReader &Reader,
  ...
  if (any(OptimizeArgs & ScanningOptimizations::HeaderSearch)) {
    ...
    toolchain::BitVector SearchPathUsage(Entries.size());
    toolchain::DenseSet<const serialization::ModuleFile *> Visited;
  ...
  if (any(OptimizeArgs & ScanningOptimizations::VFS)) {
    ...
    toolchain::BitVector VFSUsage(VFSOverlayFiles.size());
    toolchain::DenseSet<const serialization::ModuleFile *> Visited;
    std::function<void(const serialization::ModuleFile *)> VisitMF =
        [&](const serialization::ModuleFile *MF) {
          Visited.insert(MF);
...
ModuleDepCollector::getInvocationAdjustedForModuleBuildWithoutOutputs(
    ...
    toolchain::function_ref<void(CowCompilerInvocation &)> Optimize) const {
  ...
  if (!CI.getDiagnosticOpts().SystemHeaderWarningsModules.empty()) {
    ...
    if (toolchain::is_contained(CI.getDiagnosticOpts().SystemHeaderWarningsModules,
                           Deps.ID.ModuleName))
...
toolchain::DenseSet<const FileEntry *> ModuleDepCollector::collectModuleMapFiles(
    ArrayRef<ModuleID> ClangModuleDeps) const {
  toolchain::DenseSet<const FileEntry *> ModuleMapFiles;
...
void ModuleDepCollector::applyDiscoveredDependencies(CompilerInvocation &CI) {
  ...
  if (toolchain::any_of(CI.getFrontendOpts().Inputs, needsModules)) {
...
#define IF_RELATIVE_RETURN_FALSE(PATH)                                         \
  do {                                                                         \
    if (!PATH.empty() && !toolchain::sys::path::is_absolute(PATH))                  \
      return false;                                                            \
  } while (0)

#define IF_ANY_RELATIVE_RETURN_FALSE(PATHS)                                    \
  do {                                                                         \
    if (toolchain::any_of(PATHS, [](const auto &P) {                                \
          return !P.empty() && !toolchain::sys::path::is_absolute(P);               \
        }))                                                                    \
      return false;                                                            \
  } while (0)
...
static std::string getModuleContextHash(const ModuleDeps &MD,
  ...
  toolchain::HashBuilder<toolchain::TruncatedBLAKE3<16>, toolchain::endianness::native>
      HashBuilder;
  ...
  toolchain::ErrorOr<std::string> CWD = VFS.getCurrentWorkingDirectory();
  if (CWD && !IgnoreCWD)
    HashBuilder.add(*CWD);
  ...
  toolchain::BLAKE3Result<16> Hash = HashBuilder.final();
  std::array<uint64_t, 2> Words;
  static_assert(sizeof(Hash) == sizeof(Words), "Hash must match Words");
  std::memcpy(Words.data(), Hash.data(), sizeof(Hash));
  return toString(toolchain::APInt(sizeof(Words) * 8, Words), 36, /*Signed=*/false);
}
...
void ModuleDepCollectorPP::LexedFileChanged(FileID FID,
  ...
  if (std::optional<StringRef> Filename = SM.getNonBuiltinFilenameForID(FID))
    MDC.addFileDep(toolchain::sys::path::remove_leading_dotslash(*Filename));
}
...
ModuleDepCollectorPP::handleTopLevelModule(const Module *M) {
  ...
  toolchain::DenseSet<const Module *> SeenDeps;
...
static void forEachSubmoduleSorted(const Module *M,
                                   toolchain::function_ref<void(const Module *)> F) {
  ...
  toolchain::stable_sort(Submodules, [](const Module *A, const Module *B) {
...
void ModuleDepCollectorPP::addAllSubmodulePrebuiltDeps(
    ...
    toolchain::DenseSet<const Module *> &SeenSubmodules) {
...
void ModuleDepCollectorPP::addAllSubmoduleDeps(
    const Module *M, ModuleDeps &MD,
    toolchain::DenseSet<const Module *> &AddedModules) {
...
void ModuleDepCollector::addVisibleModules() {
  toolchain::DenseSet<const Module *> ImportedModules;
...
static StringRef makeAbsoluteAndPreferred(CompilerInstance &CI, StringRef Path,
  ...
  if (toolchain::sys::path::is_absolute(Path) &&
      !toolchain::sys::path::is_style_windows(toolchain::sys::path::Style::native))
    return Path;
  Storage.assign(Path.begin(), Path.end());
  CI.getFileManager().makeAbsolutePath(Storage);
  toolchain::sys::path::make_preferred(Storage);
...
void ModuleDepCollector::addFileDep(StringRef Path) {
  ...
  toolchain::SmallString<256> Storage;
