...
CompilationDatabase::autoDetectFromSource(StringRef SourceFile,
  ...
  StringRef Directory = toolchain::sys::path::parent_path(AbsolutePath);
...
std::string GetClangToolCommand() {
  ...
  std::string ClangExecutable =
      toolchain::sys::fs::getMainExecutable("clang", (void *)&Dummy);
  SmallString<128> ClangToolPath;
  ClangToolPath = toolchain::sys::path::parent_path(ClangExecutable);
  toolchain::sys::path::append(ClangToolPath, "clang-tool");
...
/// language::Core::driver::Driver although what the interface should look like is not
/// clear.
///
...
static bool stripPositionalArgs(std::vector<const char *> Args,
  ...
  toolchain::raw_string_ostream Output(ErrorMsg);
  ...
  std::unique_ptr<driver::Driver> NewDriver(new driver::Driver(
      /* ClangExecutable= */ "", toolchain::sys::getDefaultTargetTriple(),
      Diagnostics));
  ...
  toolchain::erase_if(Args, FilterUnusedFlags());
  ...
  std::vector<const char *>::iterator End =
      toolchain::remove_if(Args, [&](StringRef S) {
        return toolchain::is_contained(CompileAnalyzer.Inputs, S) ||
               toolchain::is_contained(DiagClient.UnusedInputs, S);
      });
...
FixedCompilationDatabase::loadFromFile(StringRef Path, std::string &ErrorMsg) {
  ...
  return loadFromBuffer(toolchain::sys::path::parent_path(Path),
...
class FixedCompilationDatabasePlugin : public CompilationDatabasePlugin {
  ...
  loadFromDirectory(StringRef Directory, std::string &ErrorMessage) override {
    ...
    toolchain::sys::path::append(DatabasePath, "compile_flags.txt");
