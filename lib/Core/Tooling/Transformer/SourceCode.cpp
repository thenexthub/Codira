...
StringRef language::Core::tooling::getText(CharSourceRange Range,
                                  const ASTContext &Context) {
  return Lexer::getSourceText(Range, Context.getSourceManager(),
                              Context.getLangOpts());
}

CharSourceRange language::Core::tooling::maybeExtendRange(CharSourceRange Range,
                                                 tok::TokenKind Next,
                                                 ASTContext &Context) {
...
toolchain::Error language::Core::tooling::validateRange(const CharSourceRange &Range,
...
toolchain::Error language::Core::tooling::validateEditRange(const CharSourceRange &Range,
                                              const SourceManager &SM) {
...
std::optional<CharSourceRange> language::Core::tooling::getFileRangeForEdit(
  ...
  bool IsInvalid = toolchain::errorToBool(validateEditRange(Range, SM));
  if (IsInvalid)
    return std::nullopt;
  return Range;
}

std::optional<CharSourceRange> language::Core::tooling::getFileRange(
    const CharSourceRange &EditRange, const SourceManager &SM,
    const LangOptions &LangOpts, bool IncludeMacroExpansion) {
  CharSourceRange Range =
      getRange(EditRange, SM, LangOpts, IncludeMacroExpansion);
  bool IsInvalid =
      toolchain::errorToBool(validateRange(Range, SM, /*AllowSystemHeaders=*/true));
  if (IsInvalid)
    return std::nullopt;
  return Range;
}
...
getEntityEndLoc(const SourceManager &SM, SourceLocation EntityLast,
  ...
  std::unique_ptr<Lexer> Lexer = [&]() {
    ...
    toolchain::StringRef File = SM.getBufferData(FileOffset.first, &Invalid);
    assert(!Invalid && "Cannot get file/offset");
    return std::make_unique<language::Core::Lexer>(
...
static std::set<tok::TokenKind> getTerminators(const Decl &D) {
  if (toolchain::isa<RecordDecl>(D) || toolchain::isa<UsingDecl>(D))
    return {tok::semi};

  if (toolchain::isa<FunctionDecl>(D) || toolchain::isa<LinkageSpecDecl>(D))
    return {tok::r_brace, tok::semi};

  if (toolchain::isa<VarDecl>(D) || toolchain::isa<FieldDecl>(D))
...
CharSourceRange tooling::getAssociatedRange(const Decl &Decl,
  ...
  if (const auto *Record = toolchain::dyn_cast<CXXRecordDecl>(&Decl)) {
  ...
  } else if (const auto *F = toolchain::dyn_cast<FunctionDecl>(&Decl)) {
  ...
  for (auto *Attr : Decl.attrs()) {
    ...
    if (Invalid)
      continue;
    toolchain::StringRef BeforeAttr =
        Source.substr(0, SM.getFileOffset(Range.getBegin()));
    toolchain::StringRef BeforeAttrStripped = BeforeAttr.rtrim();

    for (toolchain::StringRef Prefix : {"[[", "__attribute__(("}) {
