...
transformer::flattenVector(SmallVector<EditGenerator, 2> Generators) {
  ...
  return
      [Gs = std::move(Generators)](
          const MatchResult &Result) -> toolchain::Expected<SmallVector<Edit, 1>> {
        SmallVector<Edit, 1> AllEdits;
        for (const auto &G : Gs) {
          toolchain::Expected<SmallVector<Edit, 1>> Edits = G(Result);
          if (!Edits)
...
public:
  ...
  toolchain::Error eval(const ast_matchers::MatchFinder::MatchResult &,
                   std::string *Result) const override {
    Result->append(S);
    return toolchain::Error::success();
  }
  std::string toString() const override {
    return (toolchain::Twine("text(\"") + S + "\")").str();
  }
};
} // namespace
...
static std::string formatHeaderPath(StringRef Header, IncludeFormat Format) {
  ...
  toolchain_unreachable("Unknown transformer::IncludeFormat enum");
}
...
EditGenerator
transformer::detail::makeEditGenerator(toolchain::SmallVector<ASTEdit, 1> Edits) {
...
template <typename T>
toolchain::Expected<SmallVector<language::Core::transformer::Edit, 1>>
...
size_t transformer::detail::findSelectedCase(const MatchResult &Result,
  ...
  toolchain_unreachable("No tag found for this rule.");
}
