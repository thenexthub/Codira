...
static std::string getReplacementErrString(replacement_error Err) {
  ...
  toolchain_unreachable("A value of replacement_error has no message.");
}
...
Replacements::mergeIfOrderIndependent(const Replacement &R) const {
  ...
  return toolchain::make_error<ReplacementError>(replacement_error::overlap_conflict,
                                            R, *Replaces.begin());
}

toolchain::Error Replacements::add(const Replacement &R) {
  // Check the file path.
  if (!Replaces.empty() && R.getFilePath() != Replaces.begin()->getFilePath())
    return toolchain::make_error<ReplacementError>(
  ...
  if (!Overlap(R, *I)) {
    ...
    toolchain::Expected<Replacements> Merged =
...
static std::vector<Range> combineAndSortRanges(std::vector<Range> Ranges) {
  toolchain::sort(Ranges, [](const Range &LHS, const Range &RHS) {
    if (LHS.getOffset() != RHS.getOffset())
      return LHS.getOffset() < RHS.getOffset();
    return LHS.getLength() < RHS.getLength();
  });
...
toolchain::Expected<std::string> applyAllReplacements(StringRef Code,
  ...
  InMemoryFileSystem->addFile(
      "<stdin>", 0, toolchain::MemoryBuffer::getMemBuffer(Code, "<stdin>"));
...
std::map<std::string, Replacements> groupReplacementsByFile(
  ...
  toolchain::SmallPtrSet<const FileEntry *, 16> ProcessedFileEntries;
  for (const auto &Entry : FileToReplaces) {
    auto FE = FileMgr.getOptionalFileRef(Entry.first);
    if (!FE)
      toolchain::errs() << "File path " << Entry.first << " is invalid.\n";
