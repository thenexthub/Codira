...
// match, maybe other path segments too. (include/toolchain/IR/Use.h, lib/IR/Use.cc).
...
namespace types = language::Core::driver::types;
...
struct TransferableCommand {
  ...
  TransferableCommand(CompileCommand C)
    ...
    toolchain::opt::InputArgList ArgList;
    {
      ...
      ClangCLMode = !TmpArgv.empty() &&
                    driver::IsClangCL(driver::getDriverMode(
                        TmpArgv.front(), toolchain::ArrayRef(TmpArgv).slice(1)));
    ...
    auto &OptTable = language::Core::driver::getDriverOptTable();
    if (!OldArgs.empty())
    ...
    for (unsigned Pos = 1; Pos < OldArgs.size();) {
      ...
      std::unique_ptr<toolchain::opt::Arg> Arg(OptTable.ParseOneArg(
          ArgList, Pos,
          toolchain::opt::Visibility(ClangCLMode ? CLOption : ClangOption)));
  ...
  CompileCommand transferTo(StringRef Filename) && {
    ...
    if (Std != LangStandard::lang_unspecified && foldType(TargetType) == Type) {
      Result.CommandLine.emplace_back((
          toolchain::Twine(ClangCLMode ? "/std:" : "-std=") +
...
private:
  ...
  std::optional<types::ID> tryParseTypeArg(const toolchain::opt::Arg &Arg) {
    const toolchain::opt::Option &Opt = Arg.getOption();
    using namespace driver::options;
    if (ClangCLMode) {
  ...
  std::optional<LangStandard::Kind> tryParseStdArg(const toolchain::opt::Arg &Arg) {
    using namespace driver::options;
...
public:
  FileIndex(std::vector<std::string> Files)
    ...
    toolchain::sort(Paths);
    toolchain::sort(Stems);
    toolchain::sort(Components);
  }
  ...
  StringRef chooseProxy(StringRef OriginalFilename,
    ...
    DEBUG_WITH_TYPE(
        "interpolate",
        toolchain::dbgs() << "interpolate: chose " << OriginalPaths[Best.first]
...
private:
  ...
  DenseMap<size_t, int> scoreCandidates(StringRef Filename) const {
    ...
    toolchain::SmallVector<StringRef, DirectorySegmentsQueried> Dirs;
  ...
  SubstringAndIndex longestMatch(StringRef Key,
    ...
    auto It = toolchain::lower_bound(Idx, SubstringAndIndex{Key, 0});
    if (It == Idx.begin())
      return *It;
    if (It == Idx.end())
      return *--It;
