...
unsigned getOffsetAfterTokenSequence(
    ...
    toolchain::function_ref<unsigned(const SourceManager &, Lexer &, Token &)>
...
StringRef matchingStem(toolchain::StringRef Path) {
  StringRef Name = toolchain::sys::path::filename(Path);
...
IncludeCategoryManager::IncludeCategoryManager(const IncludeStyle &Style,
  ...
  for (const auto &Category : Style.IncludeCategories) {
    CategoryRegexs.emplace_back(Category.Regex, Category.RegexIsCaseSensitive
                                                    ? toolchain::Regex::NoFlags
                                                    : toolchain::Regex::IgnoreCase);
  }
  ...
  if (!Style.IncludeIsMainSourceRegex.empty()) {
    toolchain::Regex MainFileRegex(Style.IncludeIsMainSourceRegex);
...
bool IncludeCategoryManager::isMainHeader(StringRef IncludeName) const {
  ...
  // headers may not.
  StringRef HeaderStem = toolchain::sys::path::stem(IncludeName);
  StringRef FileStem = toolchain::sys::path::stem(FileName); // foo.cu for foo.cu.cc
  ...
  if (!Matching.empty()) {
    toolchain::Regex MainIncludeRegex(HeaderStem.str() + Style.IncludeIsMainRegex,
                                 toolchain::Regex::IgnoreCase);
...
const toolchain::Regex HeaderIncludes::IncludeRegex(IncludeRegexPattern);
...
HeaderIncludes::insert(toolchain::StringRef IncludeName, bool IsAngled,
  ...
  std::string Quoted =
      std::string(toolchain::formatv(IsAngled ? "<{0}>" : "\"{0}\"", IncludeName));
...
tooling::Replacements HeaderIncludes::remove(toolchain::StringRef IncludeName,
                                             bool IsAngled) const {
  assert(IncludeName == trimInclude(IncludeName));
  ...
  for (const auto &Inc : Iter->second) {
    if ((IsAngled && StringRef(Inc.Name).starts_with("\"")) ||
        (!IsAngled && StringRef(Inc.Name).starts_with("<")))
      continue;
    toolchain::Error Err = Result.add(tooling::Replacement(
    ...
    if (Err) {
      ...
      toolchain_unreachable(ErrMsg.c_str());
    }
  }
  return Result;
}
