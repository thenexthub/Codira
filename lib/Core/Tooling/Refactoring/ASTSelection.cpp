...
#include "language/Core/AST/LexicallyOrderedRecursiveASTVisitor.h"
...
public:
  ...
  bool TraversePseudoObjectExpr(PseudoObjectExpr *E) {
    ...
    toolchain::SaveAndRestore LookThrough(LookThroughOpaqueValueExprs, true);
  ...
  bool TraverseOpaqueValueExpr(OpaqueValueExpr *E) {
    if (!LookThroughOpaqueValueExprs)
      return true;
    toolchain::SaveAndRestore LookThrough(LookThroughOpaqueValueExprs, false);
...
language::Core::tooling::findSelectedASTNodes(const ASTContext &Context,
...
static const char *selectionKindToString(SourceSelectionKind Kind) {
  ...
  toolchain_unreachable("invalid selection kind");
}

static void dump(const SelectedASTNode &Node, toolchain::raw_ostream &OS,
                 unsigned Indent = 0) {
  OS.indent(Indent * 2);
...
void SelectedASTNode::dump(toolchain::raw_ostream &OS) const { ::dump(*this, OS); }
...
static void findDeepestWithKind(
    ...
    toolchain::SmallVectorImpl<SelectedNodeWithParents> &MatchingNodes,
    SourceSelectionKind Kind,
    toolchain::SmallVectorImpl<SelectedASTNode::ReferenceType> &ParentStack) {
...
static void findDeepestWithKind(
    ...
    toolchain::SmallVectorImpl<SelectedNodeWithParents> &MatchingNodes,
    SourceSelectionKind Kind) {
  toolchain::SmallVector<SelectedASTNode::ReferenceType, 16> ParentStack;
...
CodeRangeASTSelection::create(SourceRange SelectionRange,
  ...
  if (SelectionRange.getBegin() == SelectionRange.getEnd())
    return std::nullopt;
  toolchain::SmallVector<SelectedNodeWithParents, 4> ContainSelection;
...
bool CodeRangeASTSelection::isInFunctionLikeBodyOfCode() const {
  ...
  // declaration.
  for (const auto &Parent : toolchain::reverse(Parents)) {
