...
#include "language/Core/Tooling/Refactoring/RecursiveSymbolVisitor.h"
...
bool IsValidEditLoc(const language::Core::SourceManager& SM, language::Core::SourceLocation Loc) {
  if (Loc.isInvalid())
    return false;
  const language::Core::FullSourceLoc FullLoc(Loc, SM);
...
SourceLocation StartLocationForType(TypeLoc TL) {
  ...
  case TypeLoc::DeducedTemplateSpecialization: {
    auto DTL = TL.castAs<language::Core::DeducedTemplateSpecializationTypeLoc>();
  ...
  default:
    toolchain_unreachable("unhandled TypeLoc class");
  }
}
...
public:
  RenameLocFinder(toolchain::ArrayRef<std::string> USRs, ASTContext &Context)
  ...
  bool VisitNamedDecl(const NamedDecl *Decl) {
    ...
    if (toolchain::isa<UsingDecl>(Decl))
      return true;

    // DestructorDecl has been handled in Typeloc.
    if (toolchain::isa<CXXDestructorDecl>(Decl))
      return true;
  ...
  bool VisitDeclRefExpr(const DeclRefExpr *Expr) {
    ...
    // using declaration.
    if (auto *UsingShadow = toolchain::dyn_cast<UsingShadowDecl>(Decl)) {
    ...
    if (const auto *MD = toolchain::dyn_cast<CXXMethodDecl>(Decl)) {
      if (isInUSRSet(MD)) {
    ...
    // TypeLoc.
    if (const auto *T = toolchain::dyn_cast<EnumConstantDecl>(Decl)) {
      ...
      if (const auto *ED =
              toolchain::dyn_cast_or_null<EnumDecl>(getClosestAncestorDecl(*T))) {
        if (ED->isScoped())
          return true;
        Decl = ED;
      }
...
private:
  ...
  const NamedDecl *getSupportedDeclFromTypeLoc(TypeLoc Loc) {
    if (const auto* TT = Loc.getType()->getAs<language::Core::TypedefType>())
      return TT->getDecl();
  ...
  bool isInUSRSet(const Decl *Decl) const {
    ...
    if (USR.empty())
      return false;
    return toolchain::is_contained(USRSet, USR);
  }
...
createRenameAtomicChanges(toolchain::ArrayRef<std::string> USRs,
  ...
  auto Replace = [&](SourceLocation Start, SourceLocation End,
    ...
    toolchain::Error Err = ReplaceChange.replace(
        SM, CharSourceRange::getTokenRange(Start, End), Text);
    if (Err) {
      toolchain::errs() << "Failed to add replacement to AtomicChange: "
  ...
  for (const auto &RenameInfo : Finder.getRenameInfos()) {
    ...
    if (RenameInfo.IgnorePrefixQualifiers) {
      ...
      if (RenameInfo.FromDecl && RenameInfo.Context) {
        if (!toolchain::isa<language::Core::TranslationUnitDecl>(
          ...
          // name.
          toolchain::StringRef ActualName = Lexer::getSourceText(
