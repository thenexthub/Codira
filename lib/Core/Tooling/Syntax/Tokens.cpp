...
using namespace language::Core::syntax;

namespace {
// Finds the smallest consecutive subsuquence of Toks that covers R.
toolchain::ArrayRef<syntax::Token>
getTokensCovering(toolchain::ArrayRef<syntax::Token> Toks, SourceRange R,
                  const SourceManager &SM) {
  if (R.isInvalid())
    return {};
  const syntax::Token *Begin =
      toolchain::partition_point(Toks, [&](const syntax::Token &T) {
...
syntax::Token::Token(const language::Core::Token &T)
    : Token(T.getLocation(), T.getLength(), T.getKind()) {
  assert(!T.isAnnotation());
}

toolchain::StringRef syntax::Token::text(const SourceManager &SM) const {
  bool Invalid = false;
  const char *Start = SM.getCharacterData(location(), &Invalid);
  assert(!Invalid);
  return toolchain::StringRef(Start, length());
}
...
toolchain::raw_ostream &syntax::operator<<(toolchain::raw_ostream &OS, const Token &T) {
  return OS << T.str();
}
...
toolchain::raw_ostream &syntax::operator<<(toolchain::raw_ostream &OS,
                                      const FileRange &R) {
  return OS << toolchain::formatv("FileRange(file = {0}, offsets = {1}-{2})",
                             R.file().getHashValue(), R.beginOffset(),
                             R.endOffset());
}

toolchain::StringRef FileRange::text(const SourceManager &SM) const {
  bool Invalid = false;
...
toolchain::ArrayRef<syntax::Token> TokenBuffer::expandedTokens(SourceRange R) const {
  if (R.isInvalid())
    return {};
  if (!ExpandedTokIndex.empty()) {
...
TokenBuffer::spelledForExpandedToken(const syntax::Token *Expanded) const {
  ...
  auto It = toolchain::partition_point(File.Mappings, [&](const Mapping &M) {
...
TokenBuffer::mappingStartingBeforeSpelled(const MarkedFile &F,
  ...
  auto It = toolchain::partition_point(F.Mappings, [SpelledI](const Mapping &M) {
...
toolchain::SmallVector<toolchain::ArrayRef<syntax::Token>, 1>
TokenBuffer::expandedForSpelled(toolchain::ArrayRef<syntax::Token> Spelled) const {
  if (Spelled.empty())
    return {};
  ...
  if (ExpandedBegin == ExpandedEnd)
    return {};
  return {toolchain::ArrayRef(ExpandedTokens.data() + ExpandedBegin,
                         ExpandedTokens.data() + ExpandedEnd)};
}

toolchain::ArrayRef<syntax::Token> TokenBuffer::spelledTokens(FileID FID) const {
...
TokenBuffer::spelledTokenContaining(SourceLocation Loc) const {
  assert(Loc.isFileID());
  const auto *Tok = toolchain::partition_point(
...
std::string TokenBuffer::Mapping::str() const {
  return std::string(
      toolchain::formatv("spelled tokens: [{0},{1}), expanded tokens: [{2},{3})",
                    BeginSpelled, EndSpelled, BeginExpanded, EndExpanded));
}

std::optional<toolchain::ArrayRef<syntax::Token>>
TokenBuffer::spelledForExpanded(toolchain::ArrayRef<syntax::Token> Expanded) const {
...
TokenBuffer::Expansion TokenBuffer::makeExpansion(const MarkedFile &F,
  ...
  Expansion E;
  E.Spelled = toolchain::ArrayRef(F.SpelledTokens.data() + M.BeginSpelled,
                             F.SpelledTokens.data() + M.EndSpelled);
  E.Expanded = toolchain::ArrayRef(ExpandedTokens.data() + M.BeginExpanded,
                              ExpandedTokens.data() + M.EndExpanded);
  return E;
}

const TokenBuffer::MarkedFile &
TokenBuffer::fileForSpelled(toolchain::ArrayRef<syntax::Token> Spelled) const {
...
TokenBuffer::expansionStartingAt(const syntax::Token *Spelled) const {
  ...
  auto M = toolchain::partition_point(File.Mappings, [&](const Mapping &M) {
...
std::vector<TokenBuffer::Expansion> TokenBuffer::expansionsOverlapping(
    toolchain::ArrayRef<syntax::Token> Spelled) const {
  if (Spelled.empty())
    return {};
  const auto &File = fileForSpelled(Spelled);
...
syntax::spelledTokensTouching(SourceLocation Loc,
                              toolchain::ArrayRef<syntax::Token> Tokens) {
  assert(Loc.isFileID());

  auto *Right = toolchain::partition_point(
  ...
  return toolchain::ArrayRef(Right - (AcceptLeft ? 1 : 0),
...
std::vector<syntax::Token> syntax::tokenize(const FileRange &FR,
  ...
  auto AddToken = [&](language::Core::Token T) {
    ...
    if (T.getKind() == tok::raw_identifier && !T.needsCleaning() &&
      ...
      language::Core::IdentifierInfo &II = Identifiers.get(T.getRawIdentifier());
...
public:
  ...
  void MacroExpands(const language::Core::Token &MacroNameTok, const MacroDefinition &MD,
...
TokenCollector::TokenCollector(Preprocessor &PP) : PP(PP) {
  ...
  PP.setTokenWatcher([this](const language::Core::Token &T) {
    if (T.isAnnotation())
      return;
    DEBUG_WITH_TYPE("collect-tokens", toolchain::dbgs()
                                          << "Token: "
...
public:
  ...
  TokenBuffer build() && {
    ...
    for (auto &pair : Result.Files) {
      ...
      assert(toolchain::is_sorted(mappings, [](const TokenBuffer::Mapping &M1,
...
private:
  ...
  void diagnoseAdvanceFailure() {
    ...
    for (unsigned I = (NextExpanded < 10) ? 0 : NextExpanded - 10;
      ...
      toolchain::errs() << L << Result.ExpandedTokens[I].dumpForTests(SM) << "\n";
    }
#endif
    toolchain_unreachable("Couldn't map expanded token to spelled tokens!");
  }
  ...
  toolchain::DenseMap<FileID, unsigned> NextSpelled; // cursor in SpelledTokens
...
std::string syntax::Token::str() const {
  return std::string(toolchain::formatv("Token({0}, length = {1})",
                                   tok::getTokenName(kind()), length()));
}

std::string syntax::Token::dumpForTests(const SourceManager &SM) const {
  return std::string(toolchain::formatv("Token(`{0}`, {1}, length = {2})", text(SM),
                                   tok::getTokenName(kind()), length()));
}
...
std::string TokenBuffer::dumpForTests() const {
  ...
  auto DumpTokens = [this, &PrintToken](toolchain::raw_ostream &OS,
                                        toolchain::ArrayRef<syntax::Token> Tokens) {
    if (Tokens.empty()) {
      OS << "<empty>";
      return;
  ...
  std::string Dump;
  toolchain::raw_string_ostream OS(Dump);

  OS << "expanded tokens:\n"
     << "  ";
  // (!) we do not show '<eof>'.
  DumpTokens(OS, toolchain::ArrayRef(ExpandedTokens).drop_back());
  OS << "\n";
  ...
  for (FileID ID : Keys) {
    ...
    std::string Path = toolchain::sys::path::convert_to_slash(Entry->getName());
    OS << toolchain::formatv("file '{0}'\n", Path) << "  spelled tokens:\n"
       << "    ";
