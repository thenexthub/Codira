...
static void traverse(const syntax::Node *N,
                     toolchain::function_ref<void(const syntax::Node *)> Visit) {
...
static void traverse(syntax::Node *N,
                     toolchain::function_ref<void(syntax::Node *)> Visit) {
...
static void dumpNode(raw_ostream &OS, const syntax::Node *N,
                     const syntax::TokenManager &TM, toolchain::BitVector IndentMask) {
...
std::string syntax::Node::dumpTokens(const TokenManager &TM) const {
  std::string Storage;
  toolchain::raw_string_ostream OS(Storage);
...
std::vector<syntax::Node *> syntax::List::getElementsAsNodes() {
  ...
  for (Node &C : getChildren()) {
    ...
    default:
      toolchain_unreachable("A list has only elements or delimiters.");
    }
  }
...
language::Core::tok::TokenKind syntax::List::getDelimiterTokenKind() const {
  switch (this->getKind()) {
  case NodeKind::NestedNameSpecifier:
    return language::Core::tok::coloncolon;
  case NodeKind::CallArguments:
  case NodeKind::ParameterDeclarationList:
  case NodeKind::DeclaratorList:
    return language::Core::tok::comma;
  default:
    toolchain_unreachable("This is not a subclass of List, thus "
...
bool syntax::List::canBeEmpty() const {
  ...
  case NodeKind::DeclaratorList:
    return true;
  default:
    toolchain_unreachable("This is not a subclass of List, thus canBeEmpty() "
                     "cannot be called");
  }
}
