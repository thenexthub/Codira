...
static CallExpr::arg_range dropDefaultArgs(CallExpr::arg_range Args) {
  auto FirstDefaultArg =
      toolchain::find_if(Args, [](auto It) { return isa<CXXDefaultArgExpr>(It); });
  return toolchain::make_range(Args.begin(), FirstDefaultArg);
}
...
static syntax::NodeKind getOperatorNodeKind(const CXXOperatorCallExpr &E) {
  ...
  case OO_MinusMinus:
    ...
    default:
      toolchain_unreachable("Invalid number of arguments for operator");
    }
  ...
  case OO_None:
    toolchain_unreachable("Not an overloadable operator");
  }
  toolchain_unreachable("Unknown OverloadedOperatorKind enum");
}
...
using ASTPtr = toolchain::PointerUnion<Stmt *, Decl *>;
...
private:
  toolchain::DenseMap<ASTPtr, syntax::Tree *> Nodes;
  toolchain::DenseMap<NestedNameSpecifierLoc, syntax::Tree *> NNSNodes;
};
} // namespace
...
public:
  ...
  toolchain::BumpPtrAllocator &allocator() { return Arena.getAllocator(); }
  ...
  void foldNode(toolchain::ArrayRef<syntax::Token> Range, syntax::Tree *New,
  ...
  /// When called on expressions (language::Core::Expr is derived from language::Core::Stmt),
  ...
  ArrayRef<syntax::Token> getRange(SourceLocation First,
    ...
    return toolchain::ArrayRef(findToken(First), std::next(findToken(Last)));
  }
...
private:
  ...
  withTrailingSemicolon(ArrayRef<syntax::Token> Tokens) const {
    ...
    if (Tokens.back().kind() != tok::semi && Tokens.end()->kind() == tok::semi)
      return toolchain::ArrayRef(Tokens.begin(), Tokens.end() + 1);
    return Tokens;
  }
  ...
  toolchain::DenseMap<SourceLocation, const syntax::Token *> LocationToToken;
  Forest Pending;
  toolchain::DenseSet<Decl *> DeclsWithoutSemicolons;
...
public:
  ...
  syntax::Declaration *handleFreeStandingTagDecl(TagDecl *C) {
    ...
    auto ConsumeTemplateParameters = [&](const TemplateParameterList &L) {
      ...
      auto R = toolchain::ArrayRef(TemplateKW, DeclarationRange.end());
      Result =
  ...
  buildUserDefinedLiteral(UserDefinedLiteral *S) {
    ...
    toolchain_unreachable("Unknown literal operator kind.");
  }
  ...
  buildNameSpecifier(const NestedNameSpecifierLoc &NNSLoc) {
    ...
    default:
      ...
      toolchain::report_fatal_error("We don't yet support the __super specifier",
                               true);
    }
  }
  ...
  buildCallArguments(CallExpr::arg_range ArgsAndDefaultArgs) {
    ...
    for (auto *Arg : Args) {
      ...
      if (DelimiterToken->kind() == language::Core::tok::TokenKind::comma)
  ...
  bool WalkUpFromCallExpr(CallExpr *S) {
    ...
    if (LParenToken->kind() == language::Core::tok::l_paren)
  ...
  bool WalkUpFromCXXOperatorCallExpr(CXXOperatorCallExpr *S) {
    ...
    default:
      toolchain_unreachable("getOperatorNodeKind() does not return this value");
    }
  }
  ...
  // Declarator chunks, they are produced by type locs and some language::Core::Decls.
...
private:
  ...
  syntax::TrailingReturnType *buildTrailingReturn(FunctionProtoTypeLoc L) {
    ...
    auto Tokens = toolchain::ArrayRef(Arrow, Return.end());
  ...
  toolchain::BumpPtrAllocator &allocator() { return Builder.allocator(); }
