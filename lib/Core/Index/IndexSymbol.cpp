...
bool index::isFunctionLocalSymbol(const Decl *D) {
  ...
  if (const NamedDecl *ND = dyn_cast<NamedDecl>(D)) {
    ...
    case Linkage::UniqueExternal:
      toolchain_unreachable("Not a sema linkage");
    case Linkage::Module:
...
SymbolInfo index::getSymbolInfo(const Decl *D) {
  ...
  if (const TagDecl *TD = dyn_cast<TagDecl>(D)) {
    ...
    case Decl::Enum:
    case Decl::Record:
      toolchain_unreachable("records handled before");
      break;
    ...
    case Decl::VarTemplate:
      toolchain_unreachable("variables handled before");
      break;
...
bool index::applyForEachSymbolRoleInterruptible(SymbolRoleSet Roles,
                                   toolchain::function_ref<bool(SymbolRole)> Fn) {
...
StringRef index::getSymbolSubKindString(SymbolSubKind K) {
  ...
  toolchain_unreachable("invalid symbol subkind");
}
...
StringRef index::getSymbolLanguageString(SymbolLanguage K) {
  ...
  toolchain_unreachable("invalid symbol language kind");
}

void index::applyForEachSymbolProperty(SymbolPropertySet Props,
                                  toolchain::function_ref<void(SymbolProperty)> Fn) {
