...
struct FullCommentParts {
  ...
  toolchain::TinyPtrVector<const BlockCommandComment *> Exceptions;
...
FullCommentParts::FullCommentParts(const FullComment *C,
  ...
  for (Comment::child_iterator I = C->child_begin(), E = C->child_end();
    ...
    case CommentKind::FullComment:
      toolchain_unreachable("AST node of this kind can't be a child of "
                       "a FullComment");
    }
  }
  ...
  toolchain::stable_sort(Params, ParamCommandCommentCompareIndex());
  toolchain::stable_sort(TParams, TParamCommandCommentComparePosition());
}

void printHTMLStartTagComment(const HTMLStartTagComment *C,
                              toolchain::raw_svector_ostream &Result) {
  Result << "<" << C->getTagName();
...
void CommentASTToHTMLConverter::visitVerbatimBlockLineComment(
  ...
  toolchain_unreachable("should not see this AST node");
}
...
void CommentASTToXMLConverter::visitHTMLStartTagComment(
  ...
  Result << ">";
  {
    SmallString<32> Tag;
    {
      toolchain::raw_svector_ostream TagOS(Tag);
  ...
  if (DI) {
    ...
    case DeclInfo::FunctionKind:
      ...
      case DeclInfo::TemplatePartialSpecialization:
        toolchain_unreachable("partial specializations of functions "
