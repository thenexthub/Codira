...
static bool printLoc(toolchain::raw_ostream &OS, SourceLocation Loc,
  ...
  if (FE) {
    OS << toolchain::sys::path::filename(FE->getName());
  } else {
...
class USRGenerator : public ConstDeclVisitor<USRGenerator> {
  ...
  toolchain::DenseMap<const Type *, unsigned> TypeSubstitutions;
...
void USRGenerator::VisitObjCContainerDecl(const ObjCContainerDecl *D,
  ...
  switch (D->getKind()) {
    default:
      toolchain_unreachable("Invalid ObjC container.");
  ...
  if (const CXXRecordDecl *CXXRecord = dyn_cast<CXXRecordDecl>(D)) {
    if (ClassTemplateDecl *ClassTmpl = CXXRecord->getDescribedClassTemplate()) {
      ...
      case TagTypeKind::Enum:
        toolchain_unreachable("enum template");
      }
      ...
      case TagTypeKind::Union:
        ...
        break;
      case TagTypeKind::Enum:
        toolchain_unreachable("enum partial specialization");
      }
      VisitTemplateParameterList(PartialSpec->getTemplateParameters());
    }
  }
...
static void printQualifier(toolchain::raw_ostream &Out, const LangOptions &LangOpts,
...
void language::Core::index::generateUSRForObjCClass(StringRef Cls, raw_ostream &OS,
...
void language::Core::index::generateUSRForObjCCategory(StringRef Cls, StringRef Cat,
                                              raw_ostream &OS,
...
void language::Core::index::generateUSRForObjCIvar(StringRef Ivar, raw_ostream &OS) {
  OS << '@' << Ivar;
}

void language::Core::index::generateUSRForObjCMethod(StringRef Sel,
                                            bool IsInstanceMethod,
                                            raw_ostream &OS) {
  OS << (IsInstanceMethod ? "(im)" : "(cm)") << Sel;
}

void language::Core::index::generateUSRForObjCProperty(StringRef Prop, bool isClassProp,
                                              raw_ostream &OS) {
  OS << (isClassProp ? "(cpy)" : "(py)") << Prop;
}

void language::Core::index::generateUSRForObjCProtocol(StringRef Prot, raw_ostream &OS,
                                              StringRef ExtSymDefinedIn) {
...
void language::Core::index::generateUSRForGlobalEnum(StringRef EnumName, raw_ostream &OS,
                                            StringRef ExtSymDefinedIn) {
  if (!ExtSymDefinedIn.empty())
    OS << "@M@" << ExtSymDefinedIn;
  OS << "@E@" << EnumName;
...
void language::Core::index::generateUSRForEnumConstant(StringRef EnumConstantName,
                                              raw_ostream &OS) {
  OS << '@' << EnumConstantName;
}

bool language::Core::index::generateUSRForDecl(const Decl *D,
                                      SmallVectorImpl<char> &Buf) {
  if (!D)
    return true;
...
bool language::Core::index::generateUSRForDecl(const Decl *D, SmallVectorImpl<char> &Buf,
                                      const LangOptions &LangOpts) {
  if (!D)
    return true;
...
bool language::Core::index::generateUSRForMacro(const MacroDefinitionRecord *MD,
...
bool language::Core::index::generateUSRForMacro(StringRef MacroName, SourceLocation Loc,
                                       const SourceManager &SM,
                                       SmallVectorImpl<char> &Buf) {
  if (MacroName.empty())
    return true;
...
bool language::Core::index::generateUSRForType(QualType T, ASTContext &Ctx,
                                      SmallVectorImpl<char> &Buf) {
...
bool language::Core::index::generateFullUSRForModule(const Module *Mod,
                                            raw_ostream &OS) {
  if (!Mod->Parent)
...
bool language::Core::index::generateFullUSRForTopLevelModuleName(StringRef ModName,
                                                        raw_ostream &OS) {
  OS << getUSRSpacePrefix();
  return generateUSRFragmentForModuleName(ModName, OS);
}

bool language::Core::index::generateUSRFragmentForModule(const Module *Mod,
                                                raw_ostream &OS) {
  return generateUSRFragmentForModuleName(Mod->Name, OS);
}

bool language::Core::index::generateUSRFragmentForModuleName(StringRef ModName,
                                                    raw_ostream &OS) {
  OS << "@M@" << ModName;
  return false;
}
