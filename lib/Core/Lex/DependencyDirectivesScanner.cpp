...
using namespace language::Core::dependency_directives_scan;
...
///
/// \code
///   #if __has_cpp_attribute(language::Core::fallthrough)
/// \endcode
///
...
private:
  ...
  /// any processing needed to handle \c language::Core::Token::NeedsCleaning.
...
void Scanner::skipDirective(StringRef Name, const char *&First,
                            const char *const End) {
  if (toolchain::StringSwitch<bool>(Name)
          .Case("warning", true)
          .Case("error", true)
          .Default(false))
...
bool Scanner::lexModuleDirectiveBody(DirectiveKind Kind, const char *&First,
  ...
  for (;;) {
    ...
    if ((Tok.is(tok::hash) || Tok.is(tok::at)) &&
        (Tok.Flags & language::Core::Token::StartOfLine)) {
      CurDirToks.pop_back();
      First = Previous;
      return false;
    }
...
Scanner::cleanStringIfNeeded(const dependency_directives_scan::Token &Tok) {
  bool NeedsCleaning = Tok.Flags & language::Core::Token::NeedsCleaning;
...
bool Scanner::lexPragma(const char *&First, const char *const End) {
  ...
  auto Kind = toolchain::StringSwitch<DirectiveKind>(Id)
...
bool language::Core::scanSourceForDependencyDirectives(
...
void language::Core::printDependencyDirectivesAsSource(
    StringRef Source,
