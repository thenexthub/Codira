...
static bool EvaluateDirectiveSubExpr(PPValue &LHS, unsigned MinPrec,
  ...
  while (true) {
    ...
    toolchain::APSInt Res(LHS.getBitWidth());
    switch (Operator) {
    ...
    bool Overflow = false;
    switch (Operator) {
    default: toolchain_unreachable("Unknown operator token!");
    case tok::percent:
      if (RHS.Val != 0)
        Res = LHS.Val % RHS.Val;
      else if (ValueLive) {
    ...
    case tok::slash:
      if (RHS.Val != 0) {
        if (LHS.Val.isSigned())
          Res = toolchain::APSInt(LHS.Val.sdiv_ov(RHS.Val, Overflow), false);
        else
          Res = LHS.Val / RHS.Val;
      } else if (ValueLive) {
    ...
    case tok::star:
      if (Res.isSigned())
        Res = toolchain::APSInt(LHS.Val.smul_ov(RHS.Val, Overflow), false);
      else
        Res = LHS.Val * RHS.Val;
      break;
    case tok::lessless: {
      ...
      if (LHS.isUnsigned())
        ...
        Res = toolchain::APSInt(LHS.Val.sshl_ov(RHS.Val, Overflow), false);
      break;
    }
    ...
    case tok::plus:
      if (LHS.isUnsigned())
        ...
        Res = toolchain::APSInt(LHS.Val.sadd_ov(RHS.Val, Overflow), false);
      break;
    case tok::minus:
      if (LHS.isUnsigned())
        Res = LHS.Val - RHS.Val;
      else
        Res = toolchain::APSInt(LHS.Val.ssub_ov(RHS.Val, Overflow), false);
      break;
    case tok::lessequal:
      Res = LHS.Val <= RHS.Val;
...
getCXXStandardLibraryVersion(Preprocessor &PP, StringRef MacroName,
  ...
  bool Invalid = false;
  toolchain::SmallVector<char, 10> Buffer;
  toolchain::StringRef RevisionDate =
