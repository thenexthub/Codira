...
#include "toolchain/Support/NativeFormatting.h"
#include "toolchain/Support/Unicode.h"
#include "toolchain/Support/UnicodeCharRanges.h"
#include <algorithm>
#include <cassert>
#include <cstddef>
#include <cstdint>
...
void Lexer::InitLexer(const char *BufStart, const char *BufPtr,
  ...
  if (BufferStart == BufferPtr) {
    ...
    size_t BOMLength = toolchain::StringSwitch<size_t>(Buf)
...
Lexer::Lexer(FileID FID, const toolchain::MemoryBufferRef &InputFile,
...
/// suitable for calls to 'LexFromRawLexer'.  This lexer assumes that the text
/// range will outlive it, so it doesn't take ownership of it.
Lexer::Lexer(FileID FID, const toolchain::MemoryBufferRef &FromFile,
...
Lexer *Lexer::Create_PragmaLexer(SourceLocation SpellingLoc,
  ...
  toolchain::MemoryBufferRef InputFile = SM.getBufferOrFake(SpellingFID);
    ...
    if (TheTok.isAtStartOfLine() && TheTok.getKind() == tok::hash) {
      ...
      if (TheTok.getKind() == tok::raw_identifier && !TheTok.needsCleaning()) {
        ...
        PreambleDirectiveKind PDK
          = toolchain::StringSwitch<PreambleDirectiveKind>(Keyword)
...
static bool isUnicodeWhitespace(uint32_t Codepoint) {
  static const toolchain::sys::UnicodeCharSet UnicodeWhitespaceChars(
      UnicodeWhitespaceCharRanges);
  return UnicodeWhitespaceChars.contains(Codepoint);
}

static toolchain::SmallString<5> codepointAsHexString(uint32_t C) {
  toolchain::SmallString<5> CharBuf;
  toolchain::raw_svector_ostream CharOS(CharBuf);
  toolchain::write_hex(CharOS, C, toolchain::HexPrintStyle::Upper, 4);
  return CharBuf;
}

// To mitigate https://github.com/toolchain/toolchain-project/issues/54732,
...
static bool isMathematicalExtensionID(uint32_t C, const LangOptions &LangOpts,
  ...
  static const toolchain::sys::UnicodeCharSet MathStartChars(
      MathematicalNotationProfileIDStartRanges);
  static const toolchain::sys::UnicodeCharSet MathContinueChars(
...
static bool isAllowedIDChar(uint32_t C, const LangOptions &LangOpts,
  ...
  if (LangOpts.AsmPreprocessor) {
    ...
    static const toolchain::sys::UnicodeCharSet XIDStartChars(XIDStartRanges);
    static const toolchain::sys::UnicodeCharSet XIDContinueChars(XIDContinueRanges);
    if (C == '_' || XIDStartChars.contains(C) || XIDContinueChars.contains(C))
      return true;
    return isMathematicalExtensionID(C, LangOpts, /*IsStart=*/false,
                                     IsExtension);
  } else if (LangOpts.C11) {
    static const toolchain::sys::UnicodeCharSet C11AllowedIDChars(
        C11AllowedIDCharRanges);
    return C11AllowedIDChars.contains(C);
  } else {
    static const toolchain::sys::UnicodeCharSet C99AllowedIDChars(
...
static bool isAllowedInitiallyIDChar(uint32_t C, const LangOptions &LangOpts,
  ...
  if (!isAllowedIDChar(C, LangOpts, IsExtension))
    return false;
  if (LangOpts.C11) {
    static const toolchain::sys::UnicodeCharSet C11DisallowedInitialIDChars(
        C11DisallowedInitialIDCharRanges);
    return !C11DisallowedInitialIDChars.contains(C);
  }
  static const toolchain::sys::UnicodeCharSet C99DisallowedInitialIDChars(
...
bool Lexer::tryConsumeIdentifierUTF8Char(const char *&CurPtr, Token &Result) {
  ...
  toolchain::ConversionResult ConvResult = toolchain::convertUTF8Sequence(
      (const toolchain::UTF8 **)&UnicodePtr, (const toolchain::UTF8 *)BufferEnd,
      &CodePoint, toolchain::strictConversion);
  if (ConvResult != toolchain::conversionOK)
    return false;
...
bool Lexer::LexRawStringLiteral(Token &Result, const char *CurPtr,
  ...
  while (PrefixLen != 16 && isRawStringDelimBody(CurPtr[PrefixLen])) {
    if (!isLexingRawMode() &&
        toolchain::is_contained({'$', '@', '`'}, CurPtr[PrefixLen])) {
...
void Lexer::codeCompleteIncludedFile(const char *PathStart,
  ...
  toolchain::StringRef SlashChars = LangOpts.MSVCCompat ? "/\\" : "/";
...
bool Lexer::SkipLineComment(Token &Result, const char *CurPtr,
  ...
  while (true) {
    ...
    if (!isASCII(C)) {
      unsigned Length = toolchain::getUTF8SequenceSize(
          (const toolchain::UTF8 *)CurPtr, (const toolchain::UTF8 *)BufferEnd);
      if (Length == 0) {
...
bool Lexer::SkipBlockComment(Token &Result, const char *CurPtr,
  ...
  while (true) {
    ...
    if (CurPtr + 24 < BufferEnd &&
      ...
      while (CurPtr + 16 < BufferEnd) {
        ...
        if (cmp != 0) {
          ...
          // the outer loop.
          CurPtr += toolchain::countr_zero<unsigned>(cmp) + 1;
          goto FoundSlash;
        }
        CurPtr += 16;
      }
    ...
    MultiByteUTF8:
      ...
      unsigned Length = toolchain::getUTF8SequenceSize(
          (const toolchain::UTF8 *)CurPtr - 1, (const toolchain::UTF8 *)BufferEnd);
      if (Length == 0) {
        if (!UnicodeDecodingAlreadyDiagnosed && !isLexingRawMode())
...
std::optional<uint32_t> Lexer::tryReadNumericUCN(const char *&StartPtr,
  ...
  while (Count != NumHexDigits || Delimited) {
    ...
    unsigned Value = toolchain::hexDigitValue(C);
...
std::optional<uint32_t> Lexer::tryReadNamedUCN(const char *&StartPtr,
  ...
  bool FoundEndDelimiter = false;
  toolchain::SmallVector<char, 30> Buffer;
  while (C) {
    C = getCharAndSize(CurPtr, CharSize);
    CurPtr += CharSize;
    if (C == '}') {
  ...
  std::optional<char32_t> Match =
      toolchain::sys::unicode::nameToCodepointStrict(Name);
  std::optional<toolchain::sys::unicode::LooseMatchingResult> LooseMatch;
  if (!Match) {
    LooseMatch = toolchain::sys::unicode::nameToCodepointLooseMatching(Name);
    if (Diagnose) {
...
LexStart:
  ...
  default: {
    ...
    // an escaped newline.
    --CurPtr;
    toolchain::ConversionResult Status =
        toolchain::convertUTF8Sequence((const toolchain::UTF8 **)&CurPtr,
                                  (const toolchain::UTF8 *)BufferEnd,
                                  &CodePoint,
                                  toolchain::strictConversion);
    if (Status == toolchain::conversionOK) {
      if (CheckUnicodeWhitespace(Result, CodePoint, CurPtr)) {
        if (SkipWhitespace(Result, CurPtr, TokAtPhysicalStartOfLine))
          return true; // KeepWhitespaceMode
...
bool Lexer::LexDependencyDirectiveTokenWhileSkipping(Token &Result) {
  ...
  do {
    ...
    case pp_none:
      toolchain_unreachable("unexpected 'pp_none'");
    case pp_include:
