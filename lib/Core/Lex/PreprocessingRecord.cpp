...
PreprocessingRecord::getPreprocessedEntitiesInRange(SourceRange Range) {
  if (Range.isInvalid())
    return toolchain::make_range(iterator(), iterator());

  if (CachedRangeQuery.Range == Range) {
    return toolchain::make_range(iterator(this, CachedRangeQuery.Result.first),
  ...
  return toolchain::make_range(iterator(this, Res.first),
...
PreprocessingRecord::findEndLocalPreprocessedEntity(SourceLocation Loc) const {
  ...
  auto I = toolchain::upper_bound(PreprocessedEntities, Loc,
...
PreprocessingRecord::addPreprocessedEntity(PreprocessedEntity *Entity) {
  ...
  pp_iter I =
      toolchain::upper_bound(PreprocessedEntities, BeginLoc,
                        PPEntityComp<&SourceRange::getBegin>(SourceMgr));
...
void PreprocessingRecord::InclusionDirective(
  ...
  language::Core::InclusionDirective *ID = new (*this) language::Core::InclusionDirective(
...
size_t PreprocessingRecord::getTotalMemory() const {
  return BumpAlloc.getTotalMemory()
    + toolchain::capacity_in_bytes(MacroDefinitions)
    + toolchain::capacity_in_bytes(PreprocessedEntities)
    + toolchain::capacity_in_bytes(LoadedPreprocessedEntities)
    + toolchain::capacity_in_bytes(SkippedRanges);
}
