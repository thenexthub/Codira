...
void Preprocessor::DumpToken(const Token &Tok, bool DumpFlags) const {
  toolchain::errs() << tok::getTokenName(Tok.getKind());

  if (!Tok.isAnnotation())
    toolchain::errs() << " '" << getSpelling(Tok) << "'";

  if (!DumpFlags) return;

  toolchain::errs() << "\t";
  if (Tok.isAtStartOfLine())
    toolchain::errs() << " [StartOfLine]";
  if (Tok.hasLeadingSpace())
    toolchain::errs() << " [LeadingSpace]";
  if (Tok.isExpandDisabled())
    toolchain::errs() << " [ExpandDisabled]";
  if (Tok.isFirstPPToken())
    toolchain::errs() << " [First pp-token]";
  if (Tok.needsCleaning()) {
    const char *Start = SourceMgr.getCharacterData(Tok.getLocation());
    toolchain::errs() << " [UnClean='" << StringRef(Start, Tok.getLength())
                 << "']";
  }
...
void Preprocessor::DumpLocation(SourceLocation Loc) const {
  Loc.print(toolchain::errs(), SourceMgr);
}
...
void Preprocessor::PrintStats() {
  toolchain::errs() << "\n*** Preprocessor Stats:\n";
  toolchain::errs() << NumDirectives << " directives found:\n";
  toolchain::errs() << "  " << NumDefined << " #define.\n";
  toolchain::errs() << "  " << NumUndefined << " #undef.\n";
  toolchain::errs() << "  #include/#include_next/#import:\n";
  toolchain::errs() << "    " << NumEnteredSourceFiles << " source files entered.\n";
  toolchain::errs() << "    " << MaxIncludeStackDepth << " max include stack depth\n";
  toolchain::errs() << "  " << NumIf << " #if/#ifndef/#ifdef.\n";
  toolchain::errs() << "  " << NumElse << " #else/#elif/#elifdef/#elifndef.\n";
  toolchain::errs() << "  " << NumEndif << " #endif.\n";
  toolchain::errs() << "  " << NumPragma << " #pragma.\n";
  toolchain::errs() << NumSkipped << " #if/#ifndef#ifdef regions skipped\n";

  toolchain::errs() << NumMacroExpanded << "/" << NumFnMacroExpanded << "/"
             << NumBuiltinMacroExpanded << " obj/fn/builtin macros expanded, "
             << NumFastMacroExpanded << " on the fast path.\n";
  toolchain::errs() << (NumFastTokenPaste+NumTokenPaste)
  ...
  toolchain::errs() << "\n  Macro Expanded Tokens: "
               << toolchain::capacity_in_bytes(MacroExpandedTokens);
  toolchain::errs() << "\n  Predefines Buffer: " << Predefines.capacity();
  // FIXME: List information for all submodules.
  toolchain::errs() << "\n  Macros: "
               << toolchain::capacity_in_bytes(CurSubmoduleState->Macros);
  toolchain::errs() << "\n  #pragma push_macro Info: "
               << toolchain::capacity_in_bytes(PragmaPushMacroInfo);
  toolchain::errs() << "\n  Poison Reasons: "
               << toolchain::capacity_in_bytes(PoisonReasons);
  toolchain::errs() << "\n  Comment Handlers: "
               << toolchain::capacity_in_bytes(CommentHandlers) << "\n";
}
...
void Preprocessor::CodeCompleteIncludedFile(toolchain::StringRef Dir,
                                            bool IsAngled) {
...
void Preprocessor::EnterMainSourceFile() {
  ...
  std::unique_ptr<toolchain::MemoryBuffer> SB =
    toolchain::MemoryBuffer::getMemBufferCopy(Predefines, "<built-in>");
...
void Preprocessor::HandlePoisonedIdentifier(Token & Identifier) {
  ...
  toolchain::DenseMap<IdentifierInfo*,unsigned>::const_iterator it =
...
void Preprocessor::addCommentHandler(CommentHandler *Handler) {
  ...
  assert(!toolchain::is_contained(CommentHandlers, Handler) &&
...
void Preprocessor::removeCommentHandler(CommentHandler *Handler) {
  std::vector<CommentHandler *>::iterator Pos =
      toolchain::find(CommentHandlers, Handler);
...
bool Preprocessor::isSafeBufferOptOut(const SourceManager &SourceMgr,
  ...
  auto TestInMap = [&SourceMgr](const SafeBufferOptOutRegionsTy &Map,
    ...
    auto FirstRegionEndingAfterLoc = toolchain::partition_point(
        Map, [&SourceMgr,
