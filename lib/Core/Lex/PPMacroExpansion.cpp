...
#include "language/Core/Lex/MacroArgs.h"
...
void Preprocessor::updateModuleMacroInfo(const IdentifierInfo *II,
  ...
  toolchain::DenseMap<ModuleMacro *, int> NumHiddenOverrides;
  for (auto *O : Info.OverriddenMacros)
    NumHiddenOverrides[O] = -1;

  // Collect all macros that are not overridden by a visible macro.
  toolchain::SmallVector<ModuleMacro *, 16> Worklist;
...
void Preprocessor::dumpMacroInfo(const IdentifierInfo *II) {
  ...
  toolchain::errs() << "MacroState " << State << " " << II->getNameStart();
  if (State && State->isAmbiguous(*this, II))
    toolchain::errs() << " ambiguous";
  if (State && !State->getOverriddenMacros().empty()) {
    toolchain::errs() << " overrides";
    for (auto *O : State->getOverriddenMacros())
      toolchain::errs() << " " << O->getOwningModule()->getFullModuleName();
  }
  toolchain::errs() << "\n";
  ...
  // Dump module macros.
  toolchain::DenseSet<ModuleMacro*> Active;
  for (auto *MM : State ? State->getActiveModuleMacros(*this, II)
                        : ArrayRef<ModuleMacro *>())
    Active.insert(MM);
  toolchain::DenseSet<ModuleMacro*> Visited;
  toolchain::SmallVector<ModuleMacro *, 16> Worklist(Leaf);
  while (!Worklist.empty()) {
    auto *MM = Worklist.pop_back_val();
    toolchain::errs() << " ModuleMacro " << MM << " "
                 << MM->getOwningModule()->getFullModuleName();
    if (!MM->getMacroInfo())
      toolchain::errs() << " undef";

    if (Active.count(MM))
      toolchain::errs() << " active";
    else if (!CurSubmoduleState->VisibleModules.isVisible(
                 MM->getOwningModule()))
      toolchain::errs() << " hidden";
    else if (MM->getMacroInfo())
      toolchain::errs() << " overridden";
...
static bool isTrivialSingleTokenExpansion(const MacroInfo *MI,
  ...
  return !toolchain::is_contained(MI->params(), II);
}
...
static void ComputeDATE_TIME(SourceLocation &DATELoc, SourceLocation &TIMELoc,
...

  {
    ...
    toolchain::raw_svector_ostream TmpStream(TmpBuffer);
    if (TM)
      TmpStream << toolchain::format("\"%s %2d %4d\"", Months[TM->tm_mon],
...

  {
    ...
    toolchain::raw_svector_ostream TmpStream(TmpBuffer);
    if (TM)
      TmpStream << toolchain::format("\"%02d:%02d:%02d\"", TM->tm_hour, TM->tm_min,
                                TM->tm_sec);
    else
      TmpStream << "??:??:??";
    Token TmpTok;
...
#include "language/Core/Basic/Features.def"
      .Default(false);
#undef FEATURE
}
...
#define EXTENSION(Name, Predicate) .Case(#Name, Predicate)
  return toolchain::StringSwitch<bool>(Extension)
#include "language/Core/Basic/Features.def"
      .Default(false);
#undef EXTENSION
}
...
/// integer values.
static void EvaluateFeatureLikeBuiltinMacro(toolchain::raw_svector_ostream& OS,
...
static bool isTargetArch(const TargetInfo &TI, const IdentifierInfo *II) {
  toolchain::Triple Arch(II->getName().lower() + "--");
  const toolchain::Triple &TT = TI.getTriple();
  if (TT.isThumb()) {
    // arm matches thumb or thumbv7. armv7 matches thumbv7.
    if ((Arch.getSubArch() == toolchain::Triple::NoSubArch ||
         Arch.getSubArch() == TT.getSubArch()) &&
        ((TT.getArch() == toolchain::Triple::thumb &&
          Arch.getArch() == toolchain::Triple::arm) ||
         (TT.getArch() == toolchain::Triple::thumbeb &&
          Arch.getArch() == toolchain::Triple::armeb)))
      return true;
  }
  ...
  return (Arch.getSubArch() == toolchain::Triple::NoSubArch ||
          Arch.getSubArch() == TT.getSubArch()) &&
...
static bool isTargetOS(const TargetInfo &TI, const IdentifierInfo *II) {
  toolchain::Triple OS(toolchain::Twine("unknown-unknown-") + II->getName().lower());
  if (OS.getOS() == toolchain::Triple::Darwin) {
...
static bool isTargetEnvironment(const TargetInfo &TI,
                                const IdentifierInfo *II) {
  toolchain::Triple Env(toolchain::Twine("---") + II->getName().lower());
  // The unknown environment is matched only if
  // '__is_target_environment(unknown)' is used.
  if (Env.getEnvironment() == toolchain::Triple::UnknownEnvironment &&
...
static bool isTargetVariantOS(const TargetInfo &TI, const IdentifierInfo *II) {
  if (TI.getTriple().isOSDarwin()) {
    const toolchain::Triple *VariantTriple = TI.getDarwinTargetVariantTriple();
    if (!VariantTriple)
      return false;
...
void Preprocessor::ExpandBuiltinMacro(Token &Tok) {
  ...
  toolchain::raw_svector_ostream OS(TmpBuffer);
    ...
    EvaluateFeatureLikeBuiltinMacro(
        ...
        [this](Token &Tok, bool &HasLexedNextToken) -> int {
          ...
          if (BuiltinID != 0) {
            ...
            return toolchain::StringSwitch<bool>(II->getName())
...
#include "language/Core/Basic/BuiltinTemplates.inc"
    ...
    toolchain_unreachable("Unknown identifier!");
  }
...
void Preprocessor::processPathForFileMacro(SmallVectorImpl<char> &Path,
  ...
  if (LangOpts.UseTargetPathSeparator) {
    if (TI.getTriple().isOSWindows())
      toolchain::sys::path::remove_dots(Path, false,
                                   toolchain::sys::path::Style::windows_backslash);
    else
      toolchain::sys::path::remove_dots(Path, false, toolchain::sys::path::Style::posix);
  }
}
...
void Preprocessor::processPathToFileName(SmallVectorImpl<char> &FileName,
  ...
  // presumed location.
  StringRef PLFileName = toolchain::sys::path::filename(PLoc.getFilename());
  if (PLFileName.empty())
