...
static unsigned getCharWidth(tok::TokenKind kind, const TargetInfo &Target) {
  switch (kind) {
  default: toolchain_unreachable("Unknown token type!");
...
static unsigned getEncodingPrefixLen(tok::TokenKind kind) {
  switch (kind) {
  default:
    toolchain_unreachable("Unknown token type!");
  case tok::char_constant:
  case tok::string_literal:
    return 0;
  case tok::utf8_char_constant:
...
static unsigned ProcessCharEscape(const char *ThisTokBegin,
  ...
  case 'x': { // Hex escape.
    ...
    for (; ThisTokBuf != ThisTokEnd; ++ThisTokBuf) {
      ...
      int CharVal = toolchain::hexDigitValue(*ThisTokBuf);
      if (CharVal == -1) {
  ...
  default:
    ...
    if (isPrintable(ResultChar))
      ...
      Diag(Diags, Features, Loc, ThisTokBegin, EscapeBegin, ThisTokBuf,
           diag::ext_unknown_escape)
        << "x" + toolchain::utohexstr(ResultChar);
    break;
  }
...
static void appendCodePoint(unsigned Codepoint,
                            toolchain::SmallVectorImpl<char> &Str) {
  char ResultBuf[4];
  char *ResultPtr = ResultBuf;
  if (toolchain::ConvertCodePointToUTF8(Codepoint, ResultPtr))
    Str.append(ResultBuf, ResultPtr);
}

void language::Core::expandUCNs(SmallVectorImpl<char> &Buf, StringRef Input) {
  for (StringRef::iterator I = Input.begin(), E = Input.end(); I != E; ++I) {
    ...
    if (Kind == 'u' && *I == '{') {
      for (++I; *I != '}'; ++I) {
        unsigned Value = toolchain::hexDigitValue(*I);
        assert(Value != -1U);
        CodePoint <<= 4;
        CodePoint += Value;
      }
    ...
    if (Kind == 'N') {
      ...
      std::optional<toolchain::sys::unicode::LooseMatchingResult> Res =
          toolchain::sys::unicode::nameToCodepointLooseMatching(Name);
...
bool language::Core::isFunctionLocalStringLiteralMacro(tok::TokenKind K,
...
bool language::Core::tokenIsLikeStringLiteral(const Token &Tok, const LangOptions &LO) {
...
static void DiagnoseInvalidUnicodeCharacterName(
  ...
  namespace u = toolchain::sys::unicode;
  ...
  for (const auto &Match : Matches) {
    ...
    std::string Str;
    toolchain::UTF32 V = Match.Value;
    bool Converted =
        toolchain::convertUTF32ToUTF8String(toolchain::ArrayRef<toolchain::UTF32>(&V, 1), Str);
    (void)Converted;
    assert(Converted && "Found a match wich is not a unicode character");

    Diag(Diags, Features, Loc, TokBegin, TokRangeBegin, TokRangeEnd,
         diag::note_invalid_ucn_name_candidate)
        << Match.Name << toolchain::utohexstr(Match.Value)
...
static bool ProcessNamedUCNEscape(const char *ThisTokBegin,
  ...
  std::optional<char32_t> Res = toolchain::sys::unicode::nameToCodepointStrict(Name);
  if (!Res) {
    if (Diags)
...
static void EncodeUCNEscape(const char *ThisTokBegin, const char *&ThisTokBuf,
  ...
  if (CharByteWidth == 4) {
    ...
    toolchain::UTF32 *ResultPtr = reinterpret_cast<toolchain::UTF32*>(ResultBuf);
    *ResultPtr = UcnVal;
    ResultBuf += 4;
    return;
  }
  ...
  if (CharByteWidth == 2) {
    // FIXME: Make the type of the result buffer correct instead of
    // using reinterpret_cast.
    toolchain::UTF16 *ResultPtr = reinterpret_cast<toolchain::UTF16*>(ResultBuf);
...
bool NumericLiteralParser::isValidUDSuffix(const LangOptions &LangOpts,
  ...
  return toolchain::StringSwitch<bool>(Suffix)
      .Cases("h", "min", "s", true)
      .Cases("ms", "us", "ns", true)
      .Cases("il", "i", "if", true)
...
void NumericLiteralParser::ParseNumberStartingWithZero(SourceLocation TokLoc) {
  ...
  auto _ = toolchain::make_scope_exit([&] {
...
static bool alwaysFitsInto64Bits(unsigned Radix, unsigned NumDigits) {
  ...
  default:
    toolchain_unreachable("impossible Radix");
  }
}
...
bool NumericLiteralParser::GetIntegerValue(toolchain::APInt &Val) {
  ...
  if (alwaysFitsInto64Bits(radix, NumDigits)) {
    ...
    for (const char *Ptr = DigitsBegin; Ptr != SuffixBegin; ++Ptr)
      if (!isDigitSeparator(*Ptr))
        N = N * radix + toolchain::hexDigitValue(*Ptr);
  ...
  toolchain::APInt RadixVal(Val.getBitWidth(), radix);
  toolchain::APInt CharVal(Val.getBitWidth(), 0);
  toolchain::APInt OldVal = Val;
  ...
  while (Ptr < SuffixBegin) {
    ...
    unsigned C = toolchain::hexDigitValue(*Ptr++);
...
toolchain::APFloat::opStatus
NumericLiteralParser::GetFloatValue(toolchain::APFloat &Result,
...
bool NumericLiteralParser::GetFixedPointValue(toolchain::APInt &StoreVal, unsigned Scale) {
  ...
  if (saw_exponent) {
    ...
    if (alwaysFitsInto64Bits(radix, NumExpDigits)) {
      toolchain::StringRef ExpStr(Ptr, NumExpDigits);
      toolchain::APInt ExpInt(/*numBits=*/64, ExpStr, /*radix=*/10);
  ...
  if (NumBitsNeeded > std::numeric_limits<unsigned>::max())
    ExpOverflowOccurred = true;
  toolchain::APInt Val(static_cast<unsigned>(NumBitsNeeded), 0, /*isSigned=*/false);
  ...
  auto MaxVal = toolchain::APInt::getMaxValue(StoreVal.getBitWidth());
...
CharLiteralParser::CharLiteralParser(const char *begin, const char *end,
  ...
  while (begin != end) {
    ...
    if (begin[0] != '\\') {
      ...
      toolchain::ConversionResult res =
          toolchain::ConvertUTF8toUTF32(reinterpret_cast<toolchain::UTF8 const **>(&start),
                             reinterpret_cast<toolchain::UTF8 const *>(begin),
                             &buffer_begin, buffer_end, toolchain::strictConversion);
      if (res != toolchain::conversionOK) {
  ...
  toolchain::APInt LitVal(PP.getTargetInfo().getIntWidth(), 0);
...
void StringLiteralParser::init(ArrayRef<Token> StringToks){
    ...
    if (ThisTokBuf[0] == 'R') {
      ...
      while (ThisTokBuf != ThisTokEnd) {
        ...
        if (CharByteWidth == 4) {
          // FIXME: Make the type of the result buffer correct instead of
          // using reinterpret_cast.
          toolchain::UTF32 *ResultWidePtr = reinterpret_cast<toolchain::UTF32*>(ResultPtr);
          *ResultWidePtr = ResultChar;
          ResultPtr += 4;
        } else if (CharByteWidth == 2) {
          // FIXME: Make the type of the result buffer correct instead of
          // using reinterpret_cast.
          toolchain::UTF16 *ResultWidePtr = reinterpret_cast<toolchain::UTF16*>(ResultPtr);
  ...
  if (Pascal) {
    if (CharByteWidth == 4) {
      // FIXME: Make the type of the result buffer correct instead of
      // using reinterpret_cast.
      toolchain::UTF32 *ResultWidePtr = reinterpret_cast<toolchain::UTF32*>(ResultBuf.data());
      ResultWidePtr[0] = GetNumStringChars() - 1;
    } else if (CharByteWidth == 2) {
      // FIXME: Make the type of the result buffer correct instead of
      // using reinterpret_cast.
      toolchain::UTF16 *ResultWidePtr = reinterpret_cast<toolchain::UTF16*>(ResultBuf.data());
      ResultWidePtr[0] = GetNumStringChars() - 1;
    } else {
      assert(CharByteWidth == 1 && "Unexpected char width");
...
static const char *resyncUTF8(const char *Err, const char *End) {
  if (Err == End)
    return End;
  End = Err + std::min<unsigned>(toolchain::getNumBytesForUTF8(*Err), End-Err);
...
bool StringLiteralParser::CopyStringFragment(const Token &Tok,
                                             const char *TokBegin,
                                             StringRef Fragment) {
  const toolchain::UTF8 *ErrorPtrTmp;
