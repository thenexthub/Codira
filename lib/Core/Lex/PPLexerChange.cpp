...
bool Preprocessor::isInPrimaryFile() const {
  ...
  return toolchain::none_of(
      toolchain::drop_begin(IncludeMacroStack),
...
PreprocessorLexer *Preprocessor::getCurrentFileLexer() const {
  ...
  for (const IncludeStackInfo &ISI : toolchain::reverse(IncludeMacroStack)) {
    if (IsFileLexer(ISI))
...
bool Preprocessor::EnterSourceFile(FileID FID, ConstSearchDirIterator CurDir,
  ...
  std::optional<toolchain::MemoryBufferRef> InputFile =
  ...
  while (!Path.empty()) {
    if (auto CurDir = FM.getOptionalDirectoryRef(Path)) {
      if (*CurDir == Dir) {
        ...
        toolchain::sys::path::append(Result,
                                toolchain::sys::path::filename(File.getName()));
        return;
      }
    }

    Path = toolchain::sys::path::parent_path(Path);
  }
...
void Preprocessor::diagnoseMissingHeaderInUmbrellaDir(const Module &Mod) {
  ...
  std::error_code EC;
  for (toolchain::vfs::recursive_directory_iterator Entry(FS, Dir->getName(), EC),
       End;
       Entry != End && !EC; Entry.increment(EC)) {
    using toolchain::StringSwitch;

    // Check whether this entry has an extension typically associated with
    // headers.
    if (!StringSwitch<bool>(toolchain::sys::path::extension(Entry->path()))
...
bool Preprocessor::HandleEndOfFile(Token &Result, bool isEndOfMacro) {
  ...
  if (Module *Mod = getCurrentModule()) {
    toolchain::SmallVector<const Module *, 4> AllMods;
...
Module *Preprocessor::LeaveSubmodule(bool ForPragma) {
  ...
  toolchain::SmallPtrSet<const IdentifierInfo*, 8> VisitedMacros;
