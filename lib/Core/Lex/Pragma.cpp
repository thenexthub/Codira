...
void language::Core::prepare_PragmaString(SmallVectorImpl<char> &StrVal) {
...
void Preprocessor::HandlePragmaPopMacro(Token &PopMacroTok) {
  ...
  toolchain::DenseMap<IdentifierInfo *, std::vector<MacroInfo *>>::iterator iter =
...
static bool LexModuleName(Preprocessor &PP, Token &Tok,
                          toolchain::SmallVectorImpl<IdentifierLoc> &ModuleName) {
  while (true) {
  ...
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
    ...
    if (II->isStr("assert")) {
      if (!PP.getPreprocessorOpts().DisablePragmaDebugCrash)
        toolchain_unreachable("This is an assertion!");
    } else if (II->isStr("crash")) {
      toolchain::Timer T("crash", "pragma crash");
      toolchain::TimeRegion R(&T);
      if (!PP.getPreprocessorOpts().DisablePragmaDebugCrash)
        LLVM_BUILTIN_TRAP;
    ...
    } else if (II->isStr("toolchain_fatal_error")) {
      if (!PP.getPreprocessorOpts().DisablePragmaDebugCrash)
        toolchain::report_fatal_error("#pragma clang __debug toolchain_fatal_error");
    } else if (II->isStr("toolchain_unreachable")) {
      if (!PP.getPreprocessorOpts().DisablePragmaDebugCrash)
        toolchain_unreachable("#pragma clang __debug toolchain_unreachable");
      ...
      toolchain::SmallVector<IdentifierLoc, 8> ModuleName;
      ...
      struct ModuleVisitor {
        ...
        void visit(Module *M, bool VisibleOnly) {
          ...
          if (!VisibleOnly || ImportLoc.isValid()) {
            toolchain::errs() << M->getFullModuleName() << " ";
            if (ImportLoc.isValid()) {
              toolchain::errs() << M << " visible ";
              ImportLoc.print(toolchain::errs(), PP.getSourceManager());
            }
            toolchain::errs() << "\n";
          }
      ...
      if (!DumpII) {
        ...
        for (auto &Building : PP.getBuildingSubmodules()) {
          toolchain::errs() << "in " << Building.M->getFullModuleName();
          if (Building.ImportLoc.isValid()) {
            toolchain::errs() << " imported ";
            if (Building.IsPragma)
              toolchain::errs() << "via pragma ";
            toolchain::errs() << "at ";
            Building.ImportLoc.print(toolchain::errs(), PP.getSourceManager());
            toolchain::errs() << "\n";
          }
        }
      } else {
...
public:
  ...
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
    ...
    diag::Severity SV = toolchain::StringSwitch<diag::Severity>(II->getName())
  ...
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
    ...
    if (II && II->isStr("push")) {
      ...
      while (true) {
        ...
        if (II) {
          int SpecifierInt = toolchain::StringSwitch<int>(II->getName())
...
private:
  ...
  static const char* PragmaKind(PPCallbacks::PragmaMessageKind Kind,
    ...
    toolchain_unreachable("Unknown PragmaMessageKind!");
  }
