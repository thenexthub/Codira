...
#include "language/Core/Lex/ModuleMapFile.h"
...
Module::HeaderKind ModuleMap::headerRoleToKind(ModuleHeaderRole Role) {
  ...
  toolchain_unreachable("unknown header role");
}
...
ModuleMap::headerKindToRole(Module::HeaderKind Kind) {
  ...
  toolchain_unreachable("unknown header kind");
}
...
static void appendSubframeworkPaths(Module *Mod,
  ...
  for (StringRef Framework : toolchain::drop_begin(toolchain::reverse(Paths)))
    toolchain::sys::path::append(Path, "Frameworks", Framework + ".framework");
}
...
OptionalFileEntryRef ModuleMap::findHeader(
  ...
  auto GetFrameworkFile = [&]() -> OptionalFileEntryRef {
    ...
    toolchain::sys::path::append(RelativePathName, "Headers", Header.FileName);
    toolchain::sys::path::append(FullPathName, RelativePathName);
    ...
    toolchain::sys::path::append(RelativePathName, "PrivateHeaders",
                            Header.FileName);
    toolchain::sys::path::append(FullPathName, RelativePathName);
    return GetFile(FullPathName);
  };

  if (toolchain::sys::path::is_absolute(Header.FileName)) {
    RelativePathName.clear();
  ...
  toolchain::sys::path::append(RelativePathName, Header.FileName);
  toolchain::sys::path::append(FullPathName, RelativePathName);
...
static bool isBuiltinHeaderName(StringRef FileName) {
  return toolchain::StringSwitch<bool>(FileName)
           .Case("float.h", true)
           .Case("iso646.h", true)
           .Case("limits.h", true)
...
static bool isBuiltInModuleName(StringRef ModuleName) {
  return toolchain::StringSwitch<bool>(ModuleName)
...
bool ModuleMap::resolveAsBuiltinHeader(
  ...
  if (Header.Kind == Module::HK_Excluded ||
      toolchain::sys::path::is_absolute(Header.FileName) ||
  ...
  toolchain::sys::path::append(Path, BuiltinIncludeDir->getName(), Header.FileName);
...
static StringRef sanitizeFilenameAsIdentifier(StringRef Name,
  ...
  while (toolchain::StringSwitch<bool>(Name)
...
bool ModuleMap::isBuiltinHeader(FileEntryRef File) {
  return File.getDir() == BuiltinIncludeDir && LangOpts.BuiltinHeadersInSystemModules &&
         isBuiltinHeaderName(toolchain::sys::path::filename(File.getName()));
}
...
ModuleMap::findOrCreateModuleForHeaderInUmbrellaDir(FileEntryRef File) {
  ...
  if (H) {
    ...
    if (UmbrellaModule->InferSubmodules) {
      ...
      for (DirectoryEntryRef SkippedDir : toolchain::reverse(SkippedDirs)) {
        // Find or create the module that corresponds to this directory name.
        SmallString<32> NameBuf;
        StringRef Name = sanitizeFilenameAsIdentifier(
            toolchain::sys::path::stem(SkippedDir.getName()), NameBuf);
      ...
      SmallString<32> NameBuf;
      StringRef Name = sanitizeFilenameAsIdentifier(
                         toolchain::sys::path::stem(File.getName()), NameBuf);
      Result = findOrCreateModuleFirst(Name, Result, /*IsFramework=*/false,
                                       Explicit);
      setInferredModuleAllowedBy(Result, UmbrellaModuleMap);
  ...
  do {
    ...
    if (KnownDir != UmbrellaDirs.end()) {
      ...
      if (UmbrellaModule->InferSubmodules) {
        ...
        SmallString<32> NameBuf;
        StringRef Name = sanitizeFilenameAsIdentifier(
                           toolchain::sys::path::stem(Header.getName()),
                           NameBuf);
        Found = lookupModuleQualified(Name, Found);
        if (!Found)
          return false;
...
Module *ModuleMap::findModule(StringRef Name) const {
  toolchain::StringMap<Module *>::const_iterator Known = Modules.find(Name);
...
Module *ModuleMap::inferFrameworkModule(DirectoryEntryRef FrameworkDir,
  ...
  StringRef ModuleName = sanitizeFilenameAsIdentifier(
      toolchain::sys::path::stem(FrameworkDirName), ModuleNameStorage);
  ...
  if (!Parent) {
    ...
    bool canInfer = false;
    if (toolchain::sys::path::has_parent_path(FrameworkDirName)) {
      // Figure out the parent path.
      StringRef Parent = toolchain::sys::path::parent_path(FrameworkDirName);
      if (auto ParentDir = FileMgr.getOptionalDirectoryRef(Parent)) {
        // Check whether we have already looked into the parent directory
        // for a module map.
        toolchain::DenseMap<const DirectoryEntry *, InferredDirectory>::const_iterator
        ...
        if (inferred->second.InferModules) {
          ...
          StringRef Name = toolchain::sys::path::stem(FrameworkDirName);
          canInfer =
              !toolchain::is_contained(inferred->second.ExcludedModules, Name);
  ...
  toolchain::sys::path::append(UmbrellaName, "Headers", ModuleName + ".h");
  ...
  RelativePath = toolchain::sys::path::relative_path(RelativePath);
  ...
  toolchain::sys::path::append(SubframeworksDirName, "Frameworks");
  toolchain::sys::path::native(SubframeworksDirName);
  ...
  for (toolchain::vfs::directory_iterator
    ...
    if (auto SubframeworkDir = FileMgr.getOptionalDirectoryRef(Dir->path())) {
      ...
      do {
        ...
        SubframeworkDirName
          = toolchain::sys::path::parent_path(SubframeworkDirName);
...
void ModuleMap::addHeader(Module *Mod, Module::Header Header,
  ...
  if (toolchain::is_contained(HeaderList, KH))
    return;
...
bool ModuleMap::parseModuleMapFile(FileEntryRef File, bool IsSystem,
  ...
  toolchain::DenseMap<const FileEntry *, const modulemap::ModuleMapFile *>::iterator
  ...
  std::optional<toolchain::MemoryBufferRef> Buffer = SourceMgr.getBufferOrNone(ID);
  if (!Buffer) {
  ...
  for (const auto &Decl : MMF.Decls) {
    std::visit(toolchain::makeVisitor(
  ...
  for (const modulemap::ExternModuleDecl *EMD : PendingExternalModuleMaps) {
    ...
    if (toolchain::sys::path::is_relative(FileNameRef)) {
      ModuleMapFileName += Dir.getName();
      toolchain::sys::path::append(ModuleMapFileName, EMD->Path);
...
ModuleMap::canonicalizeModuleMapPath(SmallVectorImpl<char> &Path) {
  StringRef Dir = toolchain::sys::path::parent_path({Path.data(), Path.size()});

  // Do not canonicalize within the framework; the module map loader expects
  // Modules/ not Versions/A/Modules.
  if (toolchain::sys::path::filename(Dir) == "Modules") {
    StringRef Parent = toolchain::sys::path::parent_path(Dir);
  ...
  if (!DirEntry)
    return toolchain::errorToErrorCode(DirEntry.takeError());

  // Canonicalize the directory.
  StringRef CanonicalDir = FM.getCanonicalName(*DirEntry);
  if (CanonicalDir != Dir)
    toolchain::sys::path::replace_path_prefix(Path, Dir, CanonicalDir);
...
LLVM_DUMP_METHOD void ModuleMap::dump() {
  toolchain::errs() << "Modules:";
  for (toolchain::StringMap<Module *>::iterator M = Modules.begin(),
                                        MEnd = Modules.end();
       M != MEnd; ++M)
    M->getValue()->print(toolchain::errs(), 2);

  toolchain::errs() << "Headers:";
  for (HeadersMap::iterator H = Headers.begin(), HEnd = Headers.end();
       H != HEnd; ++H) {
    toolchain::errs() << "  \"" << H->first.getName() << "\" -> ";
    for (SmallVectorImpl<KnownHeader>::const_iterator I = H->second.begin(),
                                                      E = H->second.end();
         I != E; ++I) {
      if (I != H->second.begin())
        toolchain::errs() << ",";
      toolchain::errs() << I->getModule()->getFullModuleName();
    }
    toolchain::errs() << "\n";
  }
}
...
class ModuleMapLoader {
  ...
  toolchain::SmallPtrSet<Module *, 2> UsesRequiresExcludedHack;
...
void ModuleMapLoader::handleUmbrellaDirDecl(
  ...
  if (toolchain::sys::path::is_absolute(DirName)) {
    Dir = SourceMgr.getFileManager().getOptionalDirectoryRef(DirName);
  } else {
    SmallString<128> PathName;
    PathName = Directory.getName();
    toolchain::sys::path::append(PathName, DirName);
  ...
  if (UsesRequiresExcludedHack.count(ActiveModule)) {
    ...
    for (toolchain::vfs::recursive_directory_iterator I(FS, Dir->getName(), EC), E;
    ...
    toolchain::stable_sort(Headers, compareModuleHeaders);
...
bool ModuleMap::parseAndLoadModuleMapFile(FileEntryRef File, bool IsSystem,
  ...
  toolchain::DenseMap<const FileEntry *, bool>::iterator Known =
