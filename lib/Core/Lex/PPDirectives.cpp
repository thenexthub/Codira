...
#include "language/Core/Lex/CodeCompletionHandler.h"
...
static bool isFeatureTestMacro(StringRef MacroName) {
  ...
  return toolchain::binary_search(ReservedMacro, MacroName);
}
...
static bool warnByDefaultOnWrongCase(StringRef Include) {
  ...
  if (::toolchain::sys::path::begin(Include)->equals_insensitive("boost"))
    return true;
  ...
  for (char &Ch : LowerInclude) {
    ...
    else if (::toolchain::sys::path::is_separator(Ch))
      Ch = '/';
  }

  // The standard C/C++ and Posix headers
  return toolchain::StringSwitch<bool>(LowerInclude)
    // C library headers
...
void Preprocessor::SkipExcludedConditionalBlock(SourceLocation HashTokenLoc,
  ...
  toolchain::SaveAndRestore SARSkipping(SkippingExcludedConditionalBlock, true);
...
OptionalFileEntryRef Preprocessor::LookupFile(
  ...
  if (!FromDir && !FromFile) {
    ...
    if (LangOpts.MSVCCompat && !isAngled) {
      for (IncludeStackInfo &ISEntry : toolchain::reverse(IncludeMacroStack)) {
...
Preprocessor::LookupEmbedFile(StringRef Filename, bool isAngled, bool OpenFile,
  ...
  if (toolchain::sys::path::is_absolute(Filename)) {
    ...
    toolchain::Expected<FileEntryRef> ShouldBeEntry = FM.getFileRef(
        Filename, OpenFile, /*CacheFailure=*/true, /*IsText=*/false);
    return toolchain::expectedToOptional(std::move(ShouldBeEntry));
  }
  ...
  auto SeparateComponents = [](SmallVectorImpl<char> &LookupPath,
    ...
    toolchain::sys::path::native(StartingFrom, LookupPath);
    if (RemoveInitialFileComponentFromLookupPath)
      toolchain::sys::path::remove_filename(LookupPath);
    if (!LookupPath.empty() &&
        !toolchain::sys::path::is_separator(LookupPath.back())) {
      LookupPath.push_back(toolchain::sys::path::get_separator().front());
    }
  ...
  if (!isAngled) {
    if (LookupFromFile) {
      ...
      if (!FullFileDir.empty()) {
        ...
        if (ShouldBeEntry)
          return toolchain::expectedToOptional(std::move(ShouldBeEntry));
        toolchain::consumeError(ShouldBeEntry.takeError());
      }
    }
...
static void diagnoseAutoModuleImport(Preprocessor &PP, SourceLocation HashLoc,
  ...
  default:
    toolchain_unreachable("unknown include directive kind");
  }
...
static bool trySimplifyPath(SmallVectorImpl<StringRef> &Components,
                            ...
                            toolchain::sys::path::Style Separator) {
  auto RealPathComponentIter = toolchain::sys::path::rbegin(RealPathName);
  auto RealPathComponentEnd = toolchain::sys::path::rend(RealPathName);
  int Cnt = 0;
  bool SuggestReplacement = false;

  auto IsSep = [Separator](StringRef Component) {
    return Component.size() == 1 &&
           toolchain::sys::path::is_separator(Component[0], Separator);
  };

  // Below is a best-effort to handle ".." in paths. It is admittedly
  // not 100% correct in the presence of symlinks.
  for (auto &Component : toolchain::reverse(Components)) {
    if ("." == Component) {
    } else if (".." == Component) {
      ++Cnt;
    } else if (Cnt) {
...
OptionalFileEntryRef Preprocessor::LookupHeaderIncludeOrImport(
  ...
  if (LangOpts.SpellChecking) {
    ...
    auto CorrectTypoFilename = [](toolchain::StringRef Filename) {
...
Preprocessor::ImportAction Preprocessor::HandleHeaderIncludeOrImport(
  ...
  toolchain::sys::path::Style BackslashStyle = toolchain::sys::path::Style::native;
  if (is_style_posix(BackslashStyle) && LangOpts.MicrosoftExt) {
    NormalizedPath = Filename.str();
    toolchain::sys::path::native(NormalizedPath);
    LookupFilename = NormalizedPath;
    BackslashStyle = toolchain::sys::path::Style::windows;
  }
  ...
  if (CheckIncludePathPortability) {
    ...
    SmallVector<StringRef, 16> Components(toolchain::sys::path::begin(Name),
                                          toolchain::sys::path::end(Name));
#if defined(_WIN32)
    ...
    if (toolchain::sys::path::is_absolute(Name) &&
        toolchain::sys::path::is_absolute(RealPathName) &&
    ...
    if (trySimplifyPath(Components, RealPathName, BackslashStyle)) {
      ...
      const auto IsSep = [BackslashStyle](char c) {
        return toolchain::sys::path::is_separator(c, BackslashStyle);
      };
...
bool Preprocessor::ReadMacroParameterList(MacroInfo *MI, Token &Tok) {
  ...
  while (true) {
    ...
    default:
      ...
      // #define X(A,A.
      if (toolchain::is_contained(Parameters, II)) { // C99 6.10.3p6
...
Preprocessor::LexEmbedParameters(Token &CurTok, bool ForHasEmbed) {
  ...
  auto LexPPParameterName = [&]() -> std::optional<std::string> {
    ...
    if (CurTok.is(tok::coloncolon)) {
      ...
      LexNonComment(CurTok);
      return (toolchain::Twine(Prefix->getName()) + "::" + Suffix->getName()).str();
    }
  ...
  auto LexParenthesizedIntegerExpr = [&]() -> std::optional<size_t> {
    ...
    if (LimitEvalResult.Value) {
      const toolchain::APSInt &Result = *LimitEvalResult.Value;
  ...
  auto LexParenthesizedBalancedTokenSoup =
      [&](toolchain::SmallVectorImpl<Token> &Tokens) {
  ...
  while (!CurTok.isOneOf(EndTokenKind, tok::eod)) {
    ...
    } else if (Parameter == "language::Core::offset") {
...
void Preprocessor::HandleEmbedDirectiveImpl(
  ...
  if (BinaryContents.empty()) {
    ...
    if (Params.MaybeIfEmptyParam) {
      ...
      toolchain::copy(Toks, NewToks.get());
  ...
  if (Params.MaybePrefixParam) {
    toolchain::copy(Params.MaybePrefixParam->Tokens, &Toks[CurIdx]);
  ...
  if (Params.MaybeSuffixParam) {
    toolchain::copy(Params.MaybeSuffixParam->Tokens, &Toks[CurIdx]);
...
void Preprocessor::HandleEmbedDirective(SourceLocation HashLoc, Token &EmbedTok,
  ...
  std::optional<toolchain::MemoryBufferRef> MaybeFile =
  ...
  if (Params->MaybeLimitParam) {
    // FIXME: just like with the language::Core::offset() and if_empty() parameters,
