...
#include "language/Core/Lex/DirectoryLookup.h"
...
void HeaderSearch::PrintStats() {
  ...
  toolchain::errs() << "  " << NumOnceOnlyFiles << " #import/#pragma once files.\n";

  toolchain::errs() << "  " << NumIncluded << " #include/#include_next/#import.\n"
               << "    " << NumMultiIncludeFileOptzn
               << " #includes skipped due to the multi-include optimization.\n";

  toolchain::errs() << NumFrameworkLookups << " framework lookups.\n"
...
void HeaderSearch::SetSearchPaths(
    ...
    toolchain::DenseMap<unsigned int, unsigned int> searchDirToHSEntry) {
...
std::vector<bool> HeaderSearch::collectVFSUsageAndClear() const {
  ...
  toolchain::vfs::FileSystem &RootFS = FileMgr.getVirtualFileSystem();
  ...
  //       OverlayFileDir.
  RootFS.visit([&](toolchain::vfs::FileSystem &FS) {
    if (auto *RFS = dyn_cast<toolchain::vfs::RedirectingFileSystem>(&FS)) {
...
std::string HeaderSearch::getPrebuiltModuleFileName(StringRef ModuleName,
  ...
  for (const std::string &Dir : HSOpts.PrebuiltModulePaths) {
    ...
    if (ModuleName.contains(':'))
      ...
      toolchain::sys::path::append(Result, ModuleName.split(':').first + "-" +
                                          ModuleName.split(':').second +
                                          ".pcm");
    else
      toolchain::sys::path::append(Result, ModuleName + ".pcm");
...
std::string HeaderSearch::getPrebuiltImplicitModuleFileName(Module *Module) {
  ...
  for (const std::string &Dir : HSOpts.PrebuiltModulePaths) {
    ...
    toolchain::sys::fs::make_absolute(CachePath);
    toolchain::sys::path::append(CachePath, ModuleCacheHash);
    std::string FileName =
...
std::string HeaderSearch::getCachedModuleFileNameImpl(StringRef ModuleName,
  ...
  if (HSOpts.DisableModuleHash) {
    ...
    auto Hash = toolchain::xxh3_64bits(CanonicalPath.str().lower());

    SmallString<128> HashStr;
    toolchain::APInt(64, Hash).toStringUnsigned(HashStr, /*Radix*/36);
    toolchain::sys::path::append(Result, ModuleName + "-" + HashStr + ".pcm");
  }
...
Module *HeaderSearch::lookupModule(StringRef ModuleName, StringRef SearchName,
  ...
  for (DirectoryLookup &Dir : search_dir_range()) {
    if (Dir.isFramework()) {
      ...
      toolchain::sys::path::append(FrameworkDirName, SearchName + ".framework");
    ...
    toolchain::sys::path::append(NestedModuleMapDirName, ModuleName);
...
void HeaderSearch::indexInitialHeaderMaps() {
  toolchain::StringMap<unsigned, toolchain::BumpPtrAllocator> Index(SearchDirs.size());
...
OptionalFileEntryRef HeaderSearch::getFileAndSuggestModule(
  ...
  if (!File) {
    ...
    // message.
    std::error_code EC = toolchain::errorToErrorCode(File.takeError());
    if (EC != toolchain::errc::no_such_file_or_directory &&
        EC != toolchain::errc::invalid_argument &&
        EC != toolchain::errc::is_a_directory && EC != toolchain::errc::not_a_directory) {
...
OptionalFileEntryRef DirectoryLookup::LookupFile(
  ...
  if (isNormalDir()) {
    ...
    toolchain::sys::path::append(TmpDir, Filename);
    if (SearchPath) {
  ...
  // framework include.
  if (toolchain::sys::path::is_relative(Dest)) {
...
getTopFrameworkDir(FileManager &FileMgr, StringRef DirName,
  ...
  assert(toolchain::sys::path::extension(DirName) == ".framework" &&
  ...
  do {
    ...
    DirName = toolchain::sys::path::parent_path(DirName);
    if (DirName.empty())
      break;
    ...
    // framework.
    if (toolchain::sys::path::extension(DirName) == ".framework") {
      SubmodulePath.push_back(std::string(toolchain::sys::path::stem(DirName)));
      TopFrameworkDir = *Dir;
    }
  } while (true);
...
OptionalFileEntryRef DirectoryLookup::DoFrameworkLookup(
  ...
  if (!CacheEntry.Directory) {
    ...
    if (getDirCharacteristic() == SrcMgr::C_User) {
      ...
      if (toolchain::sys::fs::exists(SystemFrameworkMarker)) {
  ...
  if (File && needModuleLookup(RequestingModule, SuggestedModule)) {
    ...
    do {
      ...
      // If this is a framework directory, then we're a subframework of this
      // framework.
      if (toolchain::sys::path::extension(FrameworkPath) == ".framework") {
        FoundFramework = true;
        break;
      }

      // Get the parent directory name.
      FrameworkPath = toolchain::sys::path::parent_path(FrameworkPath);
...
static const char *copyString(StringRef Str, toolchain::BumpPtrAllocator &Alloc) {
  assert(!Str.empty());
...
size_t HeaderSearch::getTotalMemory() const {
  return SearchDirs.capacity()
    + toolchain::capacity_in_bytes(FileInfo)
    + toolchain::capacity_in_bytes(HeaderMaps)
...
bool HeaderSearch::hasModuleMap(StringRef FileName,
  ...
  do {
    ...
    switch (parseAndLoadModuleMapFile(
        *Dir, IsSystem,
        toolchain::sys::path::extension(Dir->getName()) == ".framework")) {
...
bool HeaderSearch::findUsableModuleForFrameworkHeader(
  ...
  if (needModuleLookup(RequestingModule, SuggestedModule)) {
    ...
    StringRef ModuleName = toolchain::sys::path::stem(TopFrameworkDir->getName());
...
static OptionalFileEntryRef getPrivateModuleMap(FileEntryRef File,
  ...
  if (Filename == "module.map")
    toolchain::sys::path::append(PrivateFilename, "module_private.map");
  else if (Filename == "module.modulemap")
    toolchain::sys::path::append(PrivateFilename, "module.private.modulemap");
  else
    return std::nullopt;
...
bool HeaderSearch::parseAndLoadModuleMapFile(FileEntryRef File, bool IsSystem,
  ...
  if (getHeaderSearchOpts().ModuleMapFileHomeIsCwd) {
    ...
    if (!OriginalModuleMapFile.empty()) {
      ...
      Dir = FileMgr.getOptionalDirectoryRef(
          toolchain::sys::path::parent_path(OriginalModuleMapFile));
      if (!Dir) {
    ...
    if (toolchain::sys::path::filename(DirName) == "Modules") {
      DirName = toolchain::sys::path::parent_path(DirName);
  ...
  toolchain_unreachable("Unknown load module map result");
}
...
HeaderSearch::lookupModuleMapFile(DirectoryEntryRef Dir, bool IsFramework) {
  ...
  if (IsFramework)
    toolchain::sys::path::append(ModuleMapFileName, "Modules");
  toolchain::sys::path::append(ModuleMapFileName, "module.modulemap");
  if (auto F = FileMgr.getOptionalFileRef(ModuleMapFileName))
    return *F;

  // Continue to allow module.map, but warn it's deprecated.
  ModuleMapFileName = Dir.getName();
  toolchain::sys::path::append(ModuleMapFileName, "module.map");
  if (auto F = FileMgr.getOptionalFileRef(ModuleMapFileName)) {
    Diags.Report(diag::warn_deprecated_module_dot_map)
...
void HeaderSearch::collectAllModules(SmallVectorImpl<Module *> &Modules) {
  ...
  if (HSOpts.ImplicitModuleMaps) {
    ...
    for (DirectoryLookup &DL : search_dir_range()) {
      ...
      if (DL.isFramework()) {
        ...
        toolchain::sys::path::native(DL.getFrameworkDirRef()->getName(), DirNative);

        // Search each of the ".framework" directories to load them as modules.
        toolchain::vfs::FileSystem &FS = FileMgr.getVirtualFileSystem();
        for (toolchain::vfs::directory_iterator Dir = FS.dir_begin(DirNative, EC),
                                           DirEnd;
             Dir != DirEnd && !EC; Dir.increment(EC)) {
          if (toolchain::sys::path::extension(Dir->path()) != ".framework")
            continue;
          ...
          loadFrameworkModule(toolchain::sys::path::stem(Dir->path()), *FrameworkDir,
                              IsSystem);
        }
        continue;
      }
  ...
  toolchain::append_range(Modules, toolchain::make_second_range(ModMap.modules()));
}
...
void HeaderSearch::loadSubdirectoryModuleMaps(DirectoryLookup &SearchDir) {
  ...
  SmallString<128> DirNative;
  toolchain::sys::path::native(Dir, DirNative);
  toolchain::vfs::FileSystem &FS = FileMgr.getVirtualFileSystem();
  for (toolchain::vfs::directory_iterator Dir = FS.dir_begin(DirNative, EC), DirEnd;
       Dir != DirEnd && !EC; Dir.increment(EC)) {
    if (Dir->type() == toolchain::sys::fs::file_type::regular_file)
      continue;
    bool IsFramework = toolchain::sys::path::extension(Dir->path()) == ".framework";
...
std::string HeaderSearch::suggestPathToFileForDiagnostics(
    FileEntryRef File, toolchain::StringRef MainFile, bool *IsAngled) const {
  return suggestPathToFileForDiagnostics(File.getName(), /*WorkingDir=*/"",
                                         MainFile, IsAngled);
}

std::string HeaderSearch::suggestPathToFileForDiagnostics(
    toolchain::StringRef File, toolchain::StringRef WorkingDir, toolchain::StringRef MainFile,
    bool *IsAngled) const {
  using namespace toolchain::sys;

  toolchain::SmallString<32> FilePath = File;
  ...
  auto CheckDir = [&](toolchain::SmallString<32> Dir) -> bool {
