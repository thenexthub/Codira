...
struct ModuleMapFileParser {
  ...
  std::optional<ExcludeDecl> parseExcludeDecl(language::Core::SourceLocation LeadingLoc);
...
modulemap::parseModuleMap(FileID ID, language::Core::DirectoryEntryRef Dir,
                          SourceManager &SM, DiagnosticsEngine &Diags,
                          bool IsSystem, unsigned *Offset) {
  std::optional<toolchain::MemoryBufferRef> Buffer = SM.getBufferOrNone(ID);
  LangOptions LOpts;
  LOpts.LangStd = language::Core::LangStandard::lang_c99;
...
ModuleMapFileParser::parseHeaderDecl(MMToken::TokenKind LeadingToken,
                                     language::Core::SourceLocation LeadingLoc) {
  HeaderDecl HD;
  HD.Private = false;
  HD.Excluded = false;
  HD.Textual = false;
  ...
  if (Tok.is(MMToken::LBrace)) {
    ...
    while (!Tok.is(MMToken::RBrace) && !Tok.is(MMToken::EndOfFile)) {
      ...
      switch (toolchain::StringSwitch<Attribute>(Str)
                  .Case("size", Size)
                  .Case("mtime", ModTime)
                  .Default(Unknown)) {
      case Size:
...
ModuleMapFileParser::parseExcludeDecl(language::Core::SourceLocation LeadingLoc) {
  // FIXME: Support string-literal module names here.
  if (!Tok.is(MMToken::Identifier)) {
...
std::optional<UmbrellaDirDecl>
ModuleMapFileParser::parseUmbrellaDirDecl(language::Core::SourceLocation UmbrellaLoc) {
  UmbrellaDirDecl UDD;
...
retry:
  ...
  case tok::raw_identifier: {
    ...
    Tok.Kind = toolchain::StringSwitch<MMToken::TokenKind>(RI)
...
bool ModuleMapFileParser::parseOptionalAttributes(ModuleAttributes &Attrs) {
  ...
  while (Tok.is(MMToken::LSquare)) {
    ...
    AttributeKind Attribute =
        toolchain::StringSwitch<AttributeKind>(Tok.getString())
...
static void dumpModule(const ModuleDecl &MD, toolchain::raw_ostream &out, int depth);

static void dumpExternModule(const ExternModuleDecl &EMD,
                             toolchain::raw_ostream &out, int depth) {
  out.indent(depth * 2);
  out << "extern module " << formatModuleId(EMD.Id) << " \"" << EMD.Path
      << "\"\n";
}

static void dumpDecls(ArrayRef<Decl> Decls, toolchain::raw_ostream &out, int depth) {
...
static void dumpModule(const ModuleDecl &MD, toolchain::raw_ostream &out,
                       int depth) {
  out.indent(depth * 2);
...
void ModuleMapFile::dump(toolchain::raw_ostream &out) const {
  for (const auto &Decl : Decls) {
    std::visit(
        toolchain::makeVisitor([&](const ModuleDecl &MD) { dumpModule(MD, out, 0); },
                          [&](const ExternModuleDecl &EMD) {
