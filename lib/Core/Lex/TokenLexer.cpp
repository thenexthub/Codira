...
void TokenLexer::stringifyVAOPTContents(
  ...
  for (unsigned int CurTokenIdx = 0; CurTokenIdx != NumVAOptTokens;
    ...
    if (VAOPTTokens[CurTokenIdx].is(tok::hashhash)) {
      ...
      pasteTokens(LHS, toolchain::ArrayRef(VAOPTTokens, NumVAOptTokens),
                  CurTokenIdx);
  ...
  for (unsigned I = 0, E = NumTokens; I != E; ++I) {
    ...
    if (!PasteBefore && !PasteAfter) {
      ...
      if (ResultArgToks->isNot(tok::eof)) {
        ...
        // to avoid pasting.
        for (Token &Tok : toolchain::drop_begin(ResultToks, FirstResult))
    ...
    if (NumToks) {  // Not an empty argument?
      ...
      // If the '##' came from expanding an argument, turn it into 'unknown'
      // to avoid pasting.
      for (Token &Tok : toolchain::make_range(ResultToks.end() - NumToks,
                                         ResultToks.end())) {
        if (Tok.is(tok::hashhash))
          Tok.setKind(tok::unknown);
      }
...
bool TokenLexer::pasteTokens(Token &Tok) {
  return pasteTokens(Tok, toolchain::ArrayRef(Tokens, NumTokens), CurTokenIdx);
}
...
static void updateConsecutiveMacroArgTokens(SourceManager &SM,
  ...
  toolchain::MutableArrayRef<Token> All(begin_tokens, end_tokens);
  toolchain::MutableArrayRef<Token> Partition;
  ...
  if (BeginLoc.isFileID()) {
    ...
    Partition = All.take_while([&](const Token &T) {
      ...
      //
      // See https://github.com/toolchain/toolchain-project/issues/60722.
  ...
  assert(toolchain::all_of(Partition.drop_front(),
