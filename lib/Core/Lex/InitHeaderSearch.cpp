...
#include "language/Core/Config/config.h" // C_INCLUDE_DIRS
...
using namespace language::Core::frontend;
...
public:
  ...
  void AddDefaultCIncludePaths(const toolchain::Triple &triple,
  ...
  bool ShouldAddDefaultIncludePaths(const toolchain::Triple &triple);
...
#if defined(_WIN32)
  return !Path.empty() && toolchain::sys::path::is_separator(Path[0]);
#else
  return toolchain::sys::path::is_absolute(Path);
#endif
}
...
bool InitHeaderSearch::AddUnmappedPath(const Twine &Path, IncludeDirGroup Group,
  ...
  if (Verbose)
    toolchain::errs() << "ignoring nonexistent directory \""
                 << MappedPathStr << "\"\n";
  return false;
}

void InitHeaderSearch::AddDefaultCIncludePaths(const toolchain::Triple &triple,
                                            const HeaderSearchOptions &HSOpts) {
  if (!ShouldAddDefaultIncludePaths(triple))
    toolchain_unreachable("Include management is handled in the driver.");
...
void InitHeaderSearch::AddDefaultIncludePaths(
    const LangOptions &Lang, const toolchain::Triple &triple,
    const HeaderSearchOptions &HSOpts) {
...
static unsigned RemoveDuplicates(std::vector<DirectoryLookupInfo> &SearchList,
  ...
  toolchain::SmallPtrSet<const DirectoryEntry *, 8> SeenDirs;
  toolchain::SmallPtrSet<const DirectoryEntry *, 8> SeenFrameworkDirs;
  toolchain::SmallPtrSet<const HeaderMap *, 8> SeenHeaderMaps;
  ...
  for (unsigned i = First; i != SearchList.size(); ++i) {
    ...
    if (Verbose) {
      toolchain::errs() << "ignoring duplicate directory \""
                   << CurEntry.getName() << "\"\n";
      if (DirToRemove != i)
        toolchain::errs() << "  as it is a non-system directory that duplicates "
                     << "a system directory\n";
    }
    if (DirToRemove != i)
      ++NonSystemRemoved;
...
void InitHeaderSearch::Realize(const LangOptions &Lang) {
  ...
  if (Verbose) {
    toolchain::errs() << "#include \"...\" search starts here:\n";
    for (unsigned i = 0, e = SearchList.size(); i != e; ++i) {
      ...
      toolchain::errs() << " " << Name << Suffix << "\n";
    }
    toolchain::errs() << "End of search list.\n";
  }
}

void language::Core::ApplyHeaderSearchOptions(HeaderSearch &HS,
                                     const HeaderSearchOptions &HSOpts,
