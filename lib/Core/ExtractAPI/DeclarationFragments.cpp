...
void findTypeLocForBlockDecl(const language::Core::TypeSourceInfo *TSInfo,
                             language::Core::FunctionTypeLoc &Block,
                             language::Core::FunctionProtoTypeLoc &BlockProto) {
  if (!TSInfo)
    return;

  language::Core::TypeLoc TL = TSInfo->getTypeLoc().getUnqualifiedLoc();
  while (true) {
    // Look through qualified types
    if (auto QualifiedTL = TL.getAs<language::Core::QualifiedTypeLoc>()) {
      TL = QualifiedTL.getUnqualifiedLoc();
      continue;
    }

    if (auto AttrTL = TL.getAs<language::Core::AttributedTypeLoc>()) {
    ...
    // then we're done.
    if (auto BlockPtr = TL.getAs<language::Core::BlockPointerTypeLoc>()) {
      TL = BlockPtr.getPointeeLoc().IgnoreParens();
      Block = TL.getAs<language::Core::FunctionTypeLoc>();
      BlockProto = TL.getAs<language::Core::FunctionProtoTypeLoc>();
    }
    break;
  }
}
...
DeclarationFragments DeclarationFragments::getExceptionSpecificationString(
  ...
  toolchain_unreachable("Unhandled exception specification");
}
...
DeclarationFragments DeclarationFragmentsBuilder::getFragmentsForType(
  ...
  if (const TagType *TagTy = dyn_cast<TagType>(T)) {
    ...
    language::Core::index::generateUSRForDecl(Decl, TagUSR);
    return Fragments.append(Decl->getName(),
                            DeclarationFragments::FragmentKind::TypeIdentifier,
                            TagUSR, Decl);
  }
  ...
  SmallString<128> USR;
  language::Core::index::generateUSRForType(Base, Context, USR);
...
DeclarationFragmentsBuilder::getFragmentsForFunction(const FunctionDecl *Func) {
  ...
  case SC_Auto:
  case SC_Register:
    toolchain_unreachable("invalid for functions");
  }
