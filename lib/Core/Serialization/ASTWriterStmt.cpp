...
void ASTStmtWriter::VisitPredefinedExpr(PredefinedExpr *E) {
  ...
  Record.push_back(
      toolchain::to_underlying(E->getIdentKind())); // FIXME: stable encoding
...
void ASTStmtWriter::VisitStringLiteral(StringLiteral *E) {
  ...
  Record.push_back(toolchain::to_underlying(E->getKind()));
...
void ASTStmtWriter::VisitSourceLocExpr(SourceLocExpr *E) {
  ...
  Record.AddSourceLocation(E->getBeginLoc());
  Record.AddSourceLocation(E->getEndLoc());
  Record.push_back(toolchain::to_underlying(E->getIdentKind()));
...
void ASTStmtWriter::VisitCXXConstructExpr(CXXConstructExpr *E) {
  ...
  Record.push_back(
      toolchain::to_underlying(E->getConstructionKind())); // FIXME: stable encoding
...
void ASTStmtWriter::VisitUnresolvedLookupExpr(UnresolvedLookupExpr *E) {
  ...
  if (Writer.isWritingStdCXXNamedModules() && Writer.getChain()) {
    ...
    toolchain::SmallVector<NamespaceDecl *> ExternalNSs;
...
void ASTWriter::WriteSubStmt(ASTContext &Context, Stmt *S) {
  ...
  toolchain::DenseMap<Stmt *, uint64_t>::iterator I = SubStmtEntries.find(S);
  ...
  struct ParentStmtInserterRAII {
    ...
    ParentStmtInserterRAII(Stmt *S, toolchain::DenseSet<Stmt *> &ParentStmts)
