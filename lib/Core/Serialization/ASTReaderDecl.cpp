...
public:
  ...
  void ReadSpecializations(ModuleFile &M, Decl *D,
                           toolchain::BitstreamCursor &DeclsCursor, bool IsPartial);
...
template <typename DeclT>
static toolchain::iterator_range<MergedRedeclIterator<DeclT>>
merged_redecls(DeclT *D) {
  return toolchain::make_range(MergedRedeclIterator<DeclT>(D),
...
void ASTDeclReader::VisitTranslationUnitDecl(TranslationUnitDecl *TU) {
  toolchain_unreachable("Translation units are not serialized");
}
...
RedeclarableResult ASTDeclReader::VisitTagDecl(TagDecl *TD) {
  ...
  case 2: // TypedefNameForAnonDecl
    ...
    break;
  default:
    toolchain_unreachable("unexpected tag info kind");
  }
...
void ASTDeclReader::VisitImplicitConceptSpecializationDecl(
  ...
  VisitDecl(D);
  toolchain::SmallVector<TemplateArgument, 4> Args;
...
void ASTDeclReader::ReadSpecializations(ModuleFile &M, Decl *D,
                                        toolchain::BitstreamCursor &DeclsCursor,
                                        bool IsPartial) {
...
void ASTDeclReader::VisitBuiltinTemplateDecl(BuiltinTemplateDecl *D) {
  toolchain_unreachable("BuiltinTemplates are not serialized");
}
...
template<typename T> static T assert_cast(...) {
  toolchain_unreachable("bad assert_cast");
}
...
void ASTDeclMerger::mergeTemplatePattern(RedeclarableTemplateDecl *D,
  ...
  if (auto *DAlias = dyn_cast<TypeAliasDecl>(DPattern))
    return mergeRedeclarable(DAlias, cast<TypedefNameDecl>(ExistingPattern),
                             Result);
  toolchain_unreachable("merged an unknown kind of redeclarable template");
}
...
#include "language/Core/Serialization/AttrPCHRead.inc"
...
Decl *ASTDeclReader::getMostRecentDeclImpl(...) {
  toolchain_unreachable("getMostRecentDecl on non-redeclarable declaration");
}
...
void ASTDeclReader::attachPreviousDeclImpl(ASTReader &Reader, ...) {
  toolchain_unreachable("attachPreviousDecl on non-redeclarable declaration");
}
...
void ASTDeclReader::attachLatestDeclImpl(...) {
  toolchain_unreachable("attachLatestDecl on non-redeclarable declaration");
}
...
void ASTDeclReader::markIncompleteDeclChainImpl(...) {
  toolchain_unreachable("markIncompleteDeclChain on non-redeclarable declaration");
}
...
Decl *ASTReader::ReadDeclRecord(GlobalDeclID ID) {
  ...
  toolchain::BitstreamCursor &DeclsCursor = Loc.F->DeclsCursor;
  ...
  auto Fail = [](const char *what, toolchain::Error &&Err) {
    toolchain::report_fatal_error(Twine("ASTReader::readDeclRecord failed ") + what +
                             ": " + toString(std::move(Err)));
  };

  if (toolchain::Error JumpFailed = DeclsCursor.JumpToBit(Loc.Offset))
  ...
  case DECL_PARTIAL_SPECIALIZATIONS:
    toolchain_unreachable("Record cannot be de-serialized with readDeclRecord");
  case DECL_TYPEDEF:
...
void ASTReader::loadDeclUpdateRecords(PendingUpdateRecord &Record) {
  ...
  if (UpdI != DeclUpdateOffsets.end()) {
    ...
    for (auto &FileAndOffset : UpdateOffsets) {
      ...
      toolchain::BitstreamCursor &Cursor = F->DeclsCursor;
      SavedStreamPosition SavedPosition(Cursor);
      if (toolchain::Error JumpFailed = Cursor.JumpToBit(Offset))
...
void ASTReader::loadPendingDeclChain(Decl *FirstLocal, uint64_t LocalOffset) {
  ...
  toolchain::BitstreamCursor &Cursor = M->DeclsCursor;
  SavedStreamPosition SavedPosition(Cursor);
  if (toolchain::Error JumpFailed = Cursor.JumpToBit(LocalOffset))
    toolchain::report_fatal_error(
...
namespace {
  ...
  class ObjCCategoriesVisitor {
    ...
    toolchain::SmallPtrSetImpl<ObjCCategoryDecl *> &Deserialized;
    ObjCCategoryDecl *Tail = nullptr;
    toolchain::DenseMap<DeclarationName, ObjCCategoryDecl *> NameCategoryMap;
