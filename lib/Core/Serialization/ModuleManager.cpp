...
ModuleManager::addModule(StringRef FileName, ModuleKind Type,
  ...
  if (std::unique_ptr<toolchain::MemoryBuffer> Buffer = lookupBuffer(FileName)) {
    ...
    Entry->closeFile();
  } else if (toolchain::MemoryBuffer *Buffer =
...
void ModuleManager::removeModules(ModuleIterator First) {
  ...
  toolchain::SmallPtrSet<ModuleFile *, 4> victimSet(
      (toolchain::pointer_iterator<ModuleIterator>(First)),
      (toolchain::pointer_iterator<ModuleIterator>(Last)));
  ...
  toolchain::erase_if(Roots, IsVictim);

  // Remove the modules from the PCH chain.
  for (auto I = First; I != Last; ++I) {
    if (!I->isModule()) {
      PCHChain.erase(toolchain::find(PCHChain, &*I), PCHChain.end());
      break;
    }
  }
...
void ModuleManager::visit(toolchain::function_ref<bool(ModuleFile &M)> Visitor,
                          toolchain::SmallPtrSetImpl<ModuleFile *> *ModuleFilesHit) {
  ...
  if (VisitOrder.size() != Chain.size()) {
    ...
    Queue.reserve(N);
    toolchain::SmallVector<unsigned, 4> UnusedIncomingEdges;
    UnusedIncomingEdges.resize(size());
    for (ModuleFile &M : toolchain::reverse(*this)) {
    ...
    while (!Queue.empty()) {
      ...
      for (ModuleFile *M : toolchain::reverse(CurrentModule->Imports)) {
  ...
  for (unsigned I = 0, N = VisitOrder.size(); I != N; ++I) {
    ...
    do {
      // For any module that this module depends on, push it on the
      // stack (if it hasn't already been marked as visited).
      for (toolchain::SetVector<ModuleFile *>::iterator
...
namespace toolchain {
  ...
  struct GraphTraits<ModuleManager> {
    ...
    using ChildIteratorType = toolchain::SetVector<ModuleFile *>::const_iterator;
...
void ModuleManager::viewGraph() {
  toolchain::ViewGraph(*this, "Modules");
}
#endif
