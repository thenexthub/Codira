...
#include "language/Core/AST/ASTMutationListener.h"
#include "language/Core/AST/ASTStructuralEquivalence.h"
#include "language/Core/AST/ASTUnresolvedSet.h"
...
#include "language/Core/AST/ODRDiagsEmitter.h"
#include "language/Core/AST/OpenACCClause.h"
#include "language/Core/AST/OpenMPClause.h"
#include "language/Core/AST/RawCommentList.h"
...
#include "language/Core/Basic/PragmaKinds.h"
#include "language/Core/Basic/Sanitizers.h"
#include "language/Core/Basic/SourceLocation.h"
#include "language/Core/Basic/SourceManager.h"
#include "language/Core/Basic/SourceManagerInternals.h"
...
#include "language/Core/Sema/ObjCMethodList.h"
#include "language/Core/Sema/Scope.h"
#include "language/Core/Sema/Sema.h"
#include "language/Core/Sema/SemaCUDA.h"
#include "language/Core/Sema/SemaObjC.h"
...
#include "language/Core/Serialization/ASTRecordReader.h"
...
#include "language/Core/Serialization/SerializationDiagnostic.h"
...
#include "toolchain/Support/LEB128.h"
...
using namespace language::Core::serialization::reader;
using toolchain::BitstreamCursor;
...
#undef CHECK_TARGET_OPT
  ...
  toolchain::sort(ExistingFeatures);
  toolchain::sort(ReadFeatures);
...
using MacroDefinitionsMap =
    toolchain::StringMap<std::pair<StringRef, bool /*IsUndef*/>>;
using DeclsMap = toolchain::DenseMap<DeclarationName, SmallVector<NamedDecl *, 8>>;
...
bool PCHValidator::ReadDiagnosticOptions(DiagnosticOptions &DiagOpts,
  ...
  auto Diags = toolchain::makeIntrusiveRefCnt<DiagnosticsEngine>(DiagIDs, DiagOpts);
  ...
  bool SystemHeaderWarningsInModule =
      Importer && toolchain::is_contained(ExistingOpts.SystemHeaderWarningsModules,
                                     Importer->Name);
...
static bool checkPreprocessorOptions(
  ...
  if (ReadMacros) {
    ...
    for (unsigned I = 0, N = ExistingMacroNames.size(); I != N; ++I) {
      ...
      toolchain::StringMap<std::pair<StringRef, bool /*IsUndef*/>>::iterator Known =
  ...
  for (unsigned I = 0, N = ExistingPPOpts.Includes.size(); I != N; ++I) {
    ...
    if (toolchain::is_contained(PPOpts.Includes, File))
      continue;
  ...
  for (unsigned I = 0, N = ExistingPPOpts.MacroIncludes.size(); I != N; ++I) {
    ...
    if (toolchain::is_contained(PPOpts.MacroIncludes, File))
      continue;
...
static bool checkModuleCachePath(toolchain::vfs::FileSystem &VFS,
...
static uint64_t readULEB(const unsigned char *&P) {
  ...
  uint64_t Val = toolchain::decodeULEB128(P, &Length, nullptr, &Error);
  if (Error)
    toolchain::report_fatal_error(Error);
  P += Length;
  return Val;
}
...
readULEBKeyDataLength(const unsigned char *&P) {
  ...
  if ((unsigned)KeyLen != KeyLen)
    toolchain::report_fatal_error("key too large");

  unsigned DataLen = readULEB(P);
  if ((unsigned)DataLen != DataLen)
    toolchain::report_fatal_error("data too large");
...
ASTSelectorLookupTrait::ReadKey(const unsigned char* d, unsigned) {
  ...
  unsigned N = endian::readNext<uint16_t, toolchain::endianness::little>(d);
  const IdentifierInfo *FirstII = Reader.getLocalIdentifier(
      F, endian::readNext<IdentifierID, toolchain::endianness::little>(d));
  if (N == 0)
...
ASTSelectorLookupTrait::ReadData(Selector, const unsigned char* d,
  ...
  Result.ID = Reader.getGlobalSelectorID(
      F, endian::readNext<uint32_t, toolchain::endianness::little>(d));
  unsigned FullInstanceBits =
      endian::readNext<uint16_t, toolchain::endianness::little>(d);
  ...
  for (unsigned I = 0; I != NumInstanceMethods; ++I) {
    if (ObjCMethodDecl *Method = Reader.GetLocalDeclAs<ObjCMethodDecl>(
            F, LocalDeclID::get(
                   Reader, F,
                   endian::readNext<DeclID, toolchain::endianness::little>(d))))
...
IdentifierInfo *ASTIdentifierLookupTrait::ReadData(const internal_key_type& k,
  ...
  unsigned ObjCOrBuiltinID =
      endian::readNext<uint16_t, toolchain::endianness::little>(d);
  unsigned Bits = endian::readNext<uint16_t, toolchain::endianness::little>(d);
  ...
  if (HasMacroDefinition) {
    uint32_t MacroDirectivesOffset =
        endian::readNext<uint32_t, toolchain::endianness::little>(d);
    DataLen -= 4;
...
void ASTDeclContextNameLookupTraitBase::ReadDataIntoImpl(
  ...
  for (unsigned NumDecls = DataLen / sizeof(DeclID); NumDecls; --NumDecls) {
    LocalDeclID ID = LocalDeclID::get(
        Reader, F, endian::readNext<DeclID, toolchain::endianness::little>(d));
...
ModuleLocalNameLookupTrait::ReadKey(const unsigned char *d, unsigned) {
  ...
  unsigned PrimaryModuleHash =
      toolchain::support::endian::readNext<uint32_t, toolchain::endianness::little>(d);
...
LazySpecializationInfoLookupTrait::ReadFileRef(const unsigned char *&d) {
  ...
  uint32_t ModuleFileID =
      endian::readNext<uint32_t, toolchain::endianness::little, unaligned>(d);
  return Reader.getLocalModuleFile(F, ModuleFileID);
}
...
LazySpecializationInfoLookupTrait::ReadKey(const unsigned char *d, unsigned) {
  using namespace toolchain::support;
  return endian::readNext<uint32_t, toolchain::endianness::little, unaligned>(d);
}
...
void LazySpecializationInfoLookupTrait::ReadDataInto(internal_key_type,
  ...
  for (unsigned NumDecls =
    ...
    LocalDeclID LocalID = LocalDeclID::get(
        Reader, F,
        endian::readNext<DeclID, toolchain::endianness::little, unaligned>(d));
...
struct AlreadyReportedDiagnosticError
    : toolchain::ErrorInfo<AlreadyReportedDiagnosticError> {
  static char ID;

  void log(raw_ostream &OS) const override {
    toolchain_unreachable("reporting an already-reported diagnostic error");
  }
...
void ASTReader::Error(toolchain::Error &&Err) const {
  handleAllErrors(
      std::move(Err), [](AlreadyReportedDiagnosticError &) {},
      [&](toolchain::ErrorInfoBase &E) { return Error(E.message()); });
}
...
toolchain::Error ASTReader::ReadSourceManagerBlock(ModuleFile &F) {
  ...
  if (toolchain::Error Err = F.Stream.SkipBlock())
    return Err;

  // Enter the source manager block.
  if (toolchain::Error Err = SLocEntryCursor.EnterSubBlock(SOURCE_MANAGER_BLOCK_ID))
    return Err;
  F.SourceManagerBlockStartOffset = SLocEntryCursor.GetCurrentBitNo();

  RecordData Record;
  while (true) {
    Expected<toolchain::BitstreamEntry> MaybeE =
        SLocEntryCursor.advanceSkippingSubblocks();
    if (!MaybeE)
      return MaybeE.takeError();
    toolchain::BitstreamEntry E = MaybeE.get();

    switch (E.Kind) {
    case toolchain::BitstreamEntry::SubBlock: // Handled for us already.
...
toolchain::Expected<SourceLocation::UIntTy>
ASTReader::readSLocOffset(ModuleFile *F, unsigned Index) {
  BitstreamCursor &Cursor = F->SLocEntryCursor;
  SavedStreamPosition SavedPosition(Cursor);
  if (toolchain::Error Err = Cursor.JumpToBit(F->SLocEntryOffsetsBase +
...
int ASTReader::getSLocEntryID(SourceLocation::UIntTy SLocOffset) {
  ...
  auto It = toolchain::upper_bound(
      toolchain::index_range(0, F->LocalNumSLocEntries), SLocOffset,
...
bool ASTReader::ReadSLocEntry(int ID) {
  ...
  auto ReadBuffer = [this](
      ...
      StringRef Name) -> std::unique_ptr<toolchain::MemoryBuffer> {
    RecordData Record;
    StringRef Blob;
    ...
    if (RecCode == SM_SLOC_BUFFER_BLOB_COMPRESSED) {
      ...
      const toolchain::compression::Format F =
          Blob.size() > 0 && Blob.data()[0] == 0x78
              ? toolchain::compression::Format::Zlib
              : toolchain::compression::Format::Zstd;
      if (const char *Reason = toolchain::compression::getReasonIfUnsupported(F)) {
        Error(Reason);
        return nullptr;
      }
      SmallVector<uint8_t, 0> Decompressed;
      if (toolchain::Error E = toolchain::compression::decompress(
              F, toolchain::arrayRefFromStringRef(Blob), Decompressed, Record[0])) {
        Error("could not decompress embedded file contents: " +
              toolchain::toString(std::move(E)));
        return nullptr;
      }
      return toolchain::MemoryBuffer::getMemBufferCopy(
          toolchain::toStringRef(Decompressed), Name);
    } else if (RecCode == SM_SLOC_BUFFER_BLOB) {
      return toolchain::MemoryBuffer::getMemBuffer(Blob.drop_back(1), Name, true);
    } else {
  ...
  if (toolchain::Error Err = F->SLocEntryCursor.JumpToBit(
  ...
  ++NumSLocEntriesRead;
  Expected<toolchain::BitstreamEntry> MaybeEntry = SLocEntryCursor.advance();
  if (!MaybeEntry) {
  ...
  case SM_SLOC_FILE_ENTRY: {
    ...
    if (NumFileDecls && ContextObj) {
      ...
      FileDeclIDs[FID] =
          FileDeclsInfo(F, toolchain::ArrayRef(FirstDecl, NumFileDecls));
    }
...
toolchain::Error ASTReader::ReadBlockAbbrevs(BitstreamCursor &Cursor,
                                        unsigned BlockID,
                                        uint64_t *StartOfBlockOffset) {
  if (toolchain::Error Err = Cursor.EnterSubBlock(BlockID))
    return Err;
  ...
  while (true) {
    ...
    if (Code != toolchain::bitc::DEFINE_ABBREV) {
      if (toolchain::Error Err = Cursor.JumpToBit(Offset))
        return Err;
      return toolchain::Error::success();
    }
    if (toolchain::Error Err = Cursor.ReadAbbrevRecord())
      return Err;
  }
}
...
Token ASTReader::ReadToken(ModuleFile &M, const RecordDataImpl &Record,
  ...
  if (Tok.isAnnotation()) {
    ...
    case tok::annot_pragma_loop_hint: {
      ...
      Info->Toks = toolchain::ArrayRef(Toks).copy(PP.getPreprocessorAllocator());
    ...
    case tok::annot_pragma_pack: {
      ...
      Info->SlotLabel =
          toolchain::StringRef(SlotLabel).copy(PP.getPreprocessorAllocator());
    ...
    case tok::annot_repl_input_end:
      break;
    default:
      toolchain_unreachable("missing deserialization code for annotation token");
    }
  } else {
...
MacroInfo *ASTReader::ReadMacroRecord(ModuleFile &F, uint64_t Offset) {
  ...
  if (toolchain::Error Err = Stream.JumpToBit(Offset)) {
  ...
  toolchain::MutableArrayRef<Token> MacroTokens;
...
unsigned HeaderFileInfoTrait::ComputeHash(internal_key_ref ikey) {
  ...
  return toolchain::xxh3_64bits(buf);
}
...
bool HeaderFileInfoTrait::EqualKey(internal_key_ref a, internal_key_ref b) {
  ...
  if (toolchain::sys::path::is_absolute(a.Filename) && a.Filename == b.Filename)
    return true;
...
HeaderFileInfoTrait::ReadKey(const unsigned char *d, unsigned) {
  ...
  ikey.Size = off_t(endian::readNext<uint64_t, toolchain::endianness::little>(d));
  ikey.ModTime =
      time_t(endian::readNext<uint64_t, toolchain::endianness::little>(d));
...
HeaderFileInfoTrait::ReadData(internal_key_ref key, const unsigned char *d,
  ...
  HFI.LazyControllingMacro = Reader.getGlobalIdentifierID(
      M, endian::readNext<IdentifierID, toolchain::endianness::little>(d));
...
void ASTReader::ReadDefinedMacros() {
  ...
  for (ModuleFile &I : toolchain::reverse(ModuleMgr)) {
    ...
    if (toolchain::Error Err = Cursor.JumpToBit(I.MacroStartOffset)) {
      Error(std::move(Err));
      return;
    }
    ...
    RecordData Record;
    while (true) {
      Expected<toolchain::BitstreamEntry> MaybeE = Cursor.advanceSkippingSubblocks();
      if (!MaybeE) {
...
void ASTReader::resolvePendingMacro(IdentifierInfo *II,
  ...
  toolchain::SmallVector<ModuleMacroRecord, 8> ModuleMacros;
  ...
  // Module macros are listed in reverse dependency order.
  {
    ...
    toolchain::SmallVector<ModuleMacro*, 8> Overrides;
...
InputFileInfo ASTReader::getInputFileInfo(ModuleFile &F, unsigned ID) {
  ...
  SavedStreamPosition SavedPosition(Cursor);
  if (toolchain::Error Err = Cursor.JumpToBit(F.InputFilesOffsetBase +
  ...
  if (!Overridden && FileChange.Kind != Change::None) {
    if (Complain) {
      ...
      Diag(diag::err_fe_ast_file_modified)
          ...
          << toolchain::itostr(FileChange.Old.value_or(0))
          << toolchain::itostr(FileChange.New.value_or(0));
...
ASTReader::ResolveImportedPath(SmallString<0> &Buf, StringRef Path,
  ...
  if (Prefix.empty() || Path.empty() || toolchain::sys::path::is_absolute(Path) ||
      Path == "<built-in>" || Path == "<command line>")
    return {Path, Buf};

  Buf.clear();
  toolchain::sys::path::append(Buf, Prefix, Path);
...
static bool isDiagnosedResult(ASTReader::ASTReadResult ARR, unsigned Caps) {
  ...
  toolchain_unreachable("unknown ASTReadResult");
}
...
ASTReader::ASTReadResult ASTReader::ReadOptionsBlock(
  ...
  if (toolchain::Error Err = Stream.EnterSubBlock(OPTIONS_BLOCK_ID)) {
    // FIXME this drops errors on the floor.
    consumeError(std::move(Err));
    return Failure;
  }
  ...
  while (true) {
    Expected<toolchain::BitstreamEntry> MaybeEntry = Stream.advance();
    if (!MaybeEntry) {
      // FIXME this drops errors on the floor.
      consumeError(MaybeEntry.takeError());
      return Failure;
...
ASTReader::ReadControlBlock(ModuleFile &F,
  ...
  if (toolchain::Error Err = Stream.EnterSubBlock(CONTROL_BLOCK_ID)) {
    Error(std::move(Err));
    return Failure;
  }
  ...
  while (true) {
    ...
    case INPUT_FILE_OFFSETS:
      ...
      F.InputFileOffsets =
          (const toolchain::support::unaligned_uint64_t *)Blob.data();
...
toolchain::Error ASTReader::ReadASTBlock(ModuleFile &F,
                                    unsigned ClientLoadCapabilities) {
  BitstreamCursor &Stream = F.Stream;

  if (toolchain::Error Err = Stream.EnterSubBlock(AST_BLOCK_ID))
    return Err;
  ...
  while (true) {
    ...
    case toolchain::BitstreamEntry::SubBlock:
      ...
      case DECLTYPES_BLOCK_ID:
        ...
        if (toolchain::Error Err = Stream.SkipBlock())
          return Err;
        if (toolchain::Error Err = ReadBlockAbbrevs(
      ...
      case PREPROCESSOR_DETAIL_BLOCK_ID:
        ...
        if (toolchain::Error Err = ReadBlockAbbrevs(F.PreprocessorDetailCursor,
      ...
      case SOURCE_MANAGER_BLOCK_ID:
        if (toolchain::Error Err = ReadSourceManagerBlock(F))
          return Err;
        break;

      case SUBMODULE_BLOCK_ID:
        if (toolchain::Error Err = ReadSubmoduleBlock(F, ClientLoadCapabilities))
          return Err;
        break;
      ...
      case COMMENTS_BLOCK_ID: {
        ...
        if (toolchain::Error Err = Stream.SkipBlock())
          return Err;
        if (toolchain::Error Err = ReadBlockAbbrevs(C, COMMENTS_BLOCK_ID))
          return Err;
...
void ASTReader::ReadModuleOffsetMap(ModuleFile &F) const {
  ...
  while (Data < DataEnd) {
    ...
    ModuleKind Kind = static_cast<ModuleKind>(
        endian::readNext<uint8_t, toolchain::endianness::little>(Data));
    uint16_t Len = endian::readNext<uint16_t, toolchain::endianness::little>(Data);
    ...
    uint32_t MacroIDOffset =
        endian::readNext<uint32_t, toolchain::endianness::little>(Data);
        ...
        if (auto ASTFE = M ? M->getASTFile() : std::nullopt) {
          ...
          if (ImportedBy && ImportedBy->Kind == MK_PCH)
            Diag(diag::note_imported_by_pch_module_not_found)
                << toolchain::sys::path::parent_path(F.ModuleMapPath);
        }
      }
      return OutOfDate;
    }
...
void ASTReader::makeModuleVisible(Module *Mod,
  ...
  toolchain::SmallPtrSet<Module *, 4> Visited;
...
bool ASTReader::loadGlobalIndex() {
  ...
  std::pair<GlobalModuleIndex *, toolchain::Error> Result =
      GlobalModuleIndex::readIndex(ModuleCachePath);
  if (toolchain::Error Err = std::move(Result.second)) {
    assert(!Result.first);
...
static bool SkipCursorToBlock(BitstreamCursor &Cursor, unsigned BlockID) {
  while (true) {
    ...
    case toolchain::BitstreamEntry::SubBlock:
      ...
      if (toolchain::Error Err = Cursor.SkipBlock()) {
        // FIXME this drops the error on the floor.
        consumeError(std::move(Err));
        return true;
      }
...
ASTReader::ASTReadResult ASTReader::ReadAST(StringRef FileName, ModuleKind Type,
  ...
  toolchain::TimeTraceScope scope("ReadAST", FileName);

  toolchain::SaveAndRestore SetCurImportLocRAII(CurrentImportLoc, ImportLoc);
  toolchain::SaveAndRestore<std::optional<ModuleKind>> SetCurModuleKindRAII(
  ...
  for (ImportedModule &M : Loaded) {
    ModuleFile &F = *M.Mod;
    toolchain::TimeTraceScope Scope2("Read Loaded AST", F.ModuleName);

    // Read the AST block.
    if (toolchain::Error Err = ReadASTBlock(F, ClientLoadCapabilities)) {
      Error(std::move(Err));
      return Failure;
    }
    ...
    while (!SkipCursorToBlock(F.Stream, EXTENSION_BLOCK_ID)) {
      if (toolchain::Error Err = ReadExtensionBlock(F)) {
        Error(std::move(Err));
        return Failure;
      }
    }
...
static toolchain::Error doesntStartWithASTFileMagic(BitstreamCursor &Stream) {
...
static unsigned moduleKindForDiagnostic(ModuleKind Kind) {
  ...
  toolchain_unreachable("unknown module kind");
}
...
ASTReader::ReadASTCore(StringRef FileName,
  ...
  auto FinalizeOrDropPCM = toolchain::make_scope_exit([&]() {
  ...
  if (toolchain::Error Err = doesntStartWithASTFileMagic(Stream)) {
  ...
  toolchain_unreachable("unexpected break; expected return");
}
...
ASTReader::ASTReadResult ASTReader::readUnhashedControlBlockImpl(
    ModuleFile *F, toolchain::StringRef StreamData, StringRef Filename,
    unsigned ClientLoadCapabilities, bool AllowCompatibleConfigurationMismatch,
...
toolchain::Error ASTReader::ReadExtensionBlock(ModuleFile &F) {
  BitstreamCursor &Stream = F.Stream;
  ...
  toolchain_unreachable("ReadExtensionBlock should return from while loop");
}
...
bool ASTReader::readASTFileControlBlock(
    ...
    unsigned ClientLoadCapabilities) {
  // Open the AST file.
  std::unique_ptr<toolchain::MemoryBuffer> OwnedBuffer;
  ...
  while (!DoneWithControlBlock) {
    ...
    case INPUT_FILE_OFFSETS: {
      ...
      const toolchain::support::unaligned_uint64_t *InputFileOffs =
          (const toolchain::support::unaligned_uint64_t *)Blob.data();
...
toolchain::Error ASTReader::ReadSubmoduleBlock(ModuleFile &F,
                                          unsigned ClientLoadCapabilities) {
  // Enter the submodule block.
  if (toolchain::Error Err = F.Stream.EnterSubBlock(SUBMODULE_BLOCK_ID))
    return Err;
      ...
      if (!ParentModule) {
        if (OptionalFileEntryRef CurFile = CurrentModule->getASTFile()) {
          ...
          if (!bool(PP.getPreprocessorOpts().DisablePCHOrModuleValidation &
            ...
            return toolchain::make_error<AlreadyReportedDiagnosticError>();
          }
        }
...
#include "language/Core/Basic/Sanitizers.def"
  ...
  for (unsigned N = Record[Idx++]; N; --N) {
    LangOpts.OMPTargetTriples.push_back(toolchain::Triple(ReadString(Record, Idx)));
  }
...
#define ENUM_CODEGENOPT(Name, Type, Bits, Default, Compatibility)              \
  if constexpr (CK::Compatibility != CK::Benign)                               \
    CGOpts.set##Name(static_cast<language::Core::CodeGenOptions::Type>(Record[Idx++]));
#define DEBUGOPT(Name, Bits, Default, Compatibility)
#define VALUE_DEBUGOPT(Name, Bits, Default, Compatibility)
#define ENUM_DEBUGOPT(Name, Type, Bits, Default, Compatibility)
#include "language/Core/Basic/CodeGenOptions.def"
...
#include "language/Core/Basic/DiagnosticOptions.def"
...
toolchain::iterator_range<PreprocessingRecord::iterator>
ASTReader::getModulePreprocessedEntities(ModuleFile &Mod) const {
  if (PreprocessingRecord *PPRec = PP.getPreprocessingRecord())
    return PPRec->getIteratorsForLoadedRange(Mod.BasePreprocessedEntityID,
                                             Mod.NumPreprocessedEntities);

  return toolchain::make_range(PreprocessingRecord::iterator(),
...
toolchain::iterator_range<ASTReader::ModuleDeclIterator>
ASTReader::getModuleFileLevelDecls(ModuleFile &Mod) {
  return toolchain::make_range(
...
PreprocessedEntity *ASTReader::ReadPreprocessedEntity(unsigned Index) {
  ...
  if (toolchain::Error Err = M.PreprocessorDetailCursor.JumpToBit(
  ...
  toolchain_unreachable("Invalid PreprocessorDetailRecordTypes");
}
...
#include "language/Core/Serialization/TypeBitCodes.def"
  default:
    return std::nullopt;
  }
}
...
QualType ASTReader::readTypeRecord(TypeID ID) {
  ...
  if (toolchain::Error Err = DeclsCursor.JumpToBit(Loc.Offset)) {
    Error(std::move(Err));
    return QualType();
  }
...
void TypeLocReader::VisitBitIntTypeLoc(language::Core::BitIntTypeLoc TL) {
  TL.setNameLoc(readSourceLocation());
}

void TypeLocReader::VisitDependentBitIntTypeLoc(
    language::Core::DependentBitIntTypeLoc TL) {
  TL.setNameLoc(readSourceLocation());
}
...
static unsigned getIndexForTypeID(serialization::TypeID ID) {
  return (ID & toolchain::maskTrailingOnes<TypeID>(32)) >> Qualifiers::FastWidth;
}
...
QualType ASTReader::GetType(TypeID ID) {
  ...
  if (isPredefinedType(ID)) {
    ...
    case PREDEF_TYPE_LAST_ID:
      ...
      toolchain_unreachable("Invalid predefined type");
      break;
...
serialization::TypeID ASTReader::getGlobalTypeID(ModuleFile &F,
  ...
  LocalID &= toolchain::maskTrailingOnes<TypeID>(32);
...
ASTRecordReader::readTemplateArgumentLocInfo(TemplateArgument::ArgKind Kind) {
  ...
  toolchain_unreachable("unexpected template argument loc");
}
...
ASTReader::GetExternalCXXCtorInitializers(uint64_t Offset) {
  ...
  SavedStreamPosition SavedPosition(Cursor);
  if (toolchain::Error Err = Cursor.JumpToBit(Loc.Offset)) {
    Error(std::move(Err));
    return nullptr;
  }
...
#define BuiltinTemplate(BTName)                                                \
  case PREDEF_DECL##BTName##_ID:                                               \
    ...
    break;
#include "language/Core/Basic/BuiltinTemplates.inc"

  case NUM_PREDEF_DECL_IDS:
    toolchain_unreachable("Invalid decl ID");
    break;
  }
...
Stmt *ASTReader::GetExternalDeclStmt(uint64_t Offset) {
  ...
  RecordLocation Loc = getLocalBitOffset(Offset);
  if (toolchain::Error Err = Loc.F->DeclsCursor.JumpToBit(Loc.Offset)) {
    Error(std::move(Err));
    return nullptr;
  }
...
bool ASTReader::LoadExternalSpecializationsImpl(SpecLookupTableTy &SpecLookups,
  ...
  toolchain::SmallVector<serialization::reader::LazySpecializationInfo, 8> Infos =
...
bool ASTReader::LoadExternalSpecializationsImpl(
  ...
  toolchain::TimeTraceScope TimeScope("Load External Specializations for ", [&] {
    std::string Name;
...
void ASTReader::FindFileRegionDecls(FileID File,
  ...
  toolchain::DenseMap<FileID, FileDeclsInfo>::iterator I = FileDeclIDs.find(File);
  ...
  ArrayRef<unaligned_decl_id_t>::iterator BeginIt =
      toolchain::lower_bound(DInfo.Decls, BeginLoc, DIDComp);
  ...
  ArrayRef<unaligned_decl_id_t>::iterator EndIt =
      toolchain::upper_bound(DInfo.Decls, EndLoc, DIDComp);
...
bool ASTReader::FindExternalVisibleDeclsByName(const DeclContext *DC,
  ...
  toolchain::SmallPtrSet<NamedDecl *, 8> Found;
...
void ASTReader::PrintStats() {
  ...
  unsigned NumTypesLoaded =
      TypesLoaded.size() - toolchain::count(TypesLoaded.materialized(), QualType());
  unsigned NumDeclsLoaded =
      DeclsLoaded.size() -
      toolchain::count(DeclsLoaded.materialized(), (Decl *)nullptr);
  unsigned NumIdentifiersLoaded =
      IdentifiersLoaded.size() -
      toolchain::count(IdentifiersLoaded, (IdentifierInfo *)nullptr);
  unsigned NumMacrosLoaded =
      MacrosLoaded.size() - toolchain::count(MacrosLoaded, (MacroInfo *)nullptr);
  unsigned NumSelectorsLoaded =
      SelectorsLoaded.size() - toolchain::count(SelectorsLoaded, Selector());
...
dumpModuleIDMap(StringRef Name,
  ...
  for (typename MapType::const_iterator I = Map.begin(), IEnd = Map.end();
       I != IEnd; ++I)
    toolchain::errs() << "  " << (DeclID)I->first << " -> " << I->second->FileName
                 << "\n";
}

LLVM_DUMP_METHOD void ASTReader::dump() {
  toolchain::errs() << "*** PCH/ModuleFile Remappings:\n";
...
void ASTReader::getMemoryBufferSizes(MemoryBufferSizes &sizes) const {
  for (ModuleFile &I : ModuleMgr) {
    if (toolchain::MemoryBuffer *buf = I.Buffer) {
      size_t bytes = buf->getBufferSize();
      switch (buf->getBufferKind()) {
        case toolchain::MemoryBuffer::MemoryBuffer_Malloc:
          sizes.malloc_bytes += bytes;
          break;
        case toolchain::MemoryBuffer::MemoryBuffer_MMap:
...
void ASTReader::InitializeSema(Sema &S) {
  ...
  for (GlobalDeclID ID : DeclsWithEffectsToVerify) {
    ...
    if (auto *FD = dyn_cast<FunctionDecl>(D))
      ...
      toolchain_unreachable("unexpected Decl type in DeclsWithEffectsToVerify");
  }
...
void ASTReader::UpdateSema() {
  ...
  if (PragmaAlignPackCurrentValue) {
    ...
    for (const auto &Entry :
         toolchain::ArrayRef(PragmaAlignPackStack).drop_front(DropFirst ? 1 : 0)) {
      SemaObj->AlignPackStack.Stack.emplace_back(
  ...
  if (FpPragmaCurrentValue) {
    ...
    for (const auto &Entry :
         toolchain::ArrayRef(FpPragmaStack).drop_front(DropFirst ? 1 : 0))
      SemaObj->FpPragmaStack.Stack.emplace_back(
          Entry.SlotLabel, Entry.Value, Entry.Location, Entry.PushLocation);
...
static void addMethodsToPool(Sema &S, ArrayRef<ObjCMethodDecl *> Methods,
                             ObjCMethodList &List) {
  for (ObjCMethodDecl *M : toolchain::reverse(Methods))
...
void ASTReader::ReadMismatchingDeleteExpressions(toolchain::MapVector<
...
ASTReader::translateIdentifierIDToIndex(IdentifierID ID) const {
  ...
  unsigned LocalID = ID & toolchain::maskTrailingOnes<IdentifierID>(32);
...
IdentifierID ASTReader::getGlobalIdentifierID(ModuleFile &M, uint64_t LocalID) {
  ...
  LocalID &= toolchain::maskTrailingOnes<IdentifierID>(32);
  ModuleFile *MF =
...
unsigned ASTReader::getModuleFileID(ModuleFile *M) {
  ...
  auto I = toolchain::find(PCHModules, M);
...
std::optional<ASTSourceDescriptor> ASTReader::getSourceDescriptor(unsigned ID) {
  ...
  if (std::distance(std::begin(PCHChain), std::end(PCHChain))) {
    ...
    StringRef ModuleName = toolchain::sys::path::filename(MF.OriginalSourceFileName);
    StringRef FileName = toolchain::sys::path::filename(MF.FileName);
    return ASTSourceDescriptor(ModuleName,
                               toolchain::sys::path::parent_path(MF.FileName),
...
toolchain::BitVector ASTReader::ReadBitVector(const RecordData &Record,
                                         const StringRef Blob) {
  unsigned Count = Record[0];
  const char *Byte = Blob.data();
  toolchain::BitVector Ret = toolchain::BitVector(Count, false);
...
toolchain::APFloat ASTRecordReader::readAPFloat(const toolchain::fltSemantics &Sem) {
  return toolchain::APFloat(Sem, readAPInt());
}
...
void ASTReader::visitInputFileInfos(
    ...
    toolchain::function_ref<void(const serialization::InputFileInfo &IFI,
                            bool IsSystem)>
        Visitor) {
...
void ASTReader::visitInputFiles(serialization::ModuleFile &MF,
                    ...
                    toolchain::function_ref<void(const serialization::InputFile &IF,
...
void ASTReader::visitTopLevelModuleMaps(
    ...
    toolchain::function_ref<void(FileEntryRef FE)> Visitor) {
  unsigned NumInputs = MF.InputFilesLoaded.size();
...
void ASTReader::finishPendingActions() {
  while (!PendingIdentifierInfos.empty() ||
    ...
    using TopLevelDeclsMap =
        toolchain::DenseMap<IdentifierInfo *, SmallVector<Decl *, 2>>;
...
void ASTReader::diagnoseOdrViolations() {
  ...
  while (!PendingOdrMergeChecks.empty()) {
    ...
    toolchain::SmallVector<const NamedDecl*, 4> Candidates;
...
void ASTReader::pushExternalDeclIntoScope(NamedDecl *D, DeclarationName Name) {
  ...
  if (SemaObj->IdResolver.tryAddTopLevelDecl(D, Name) && SemaObj->TUScope) {
    ...
    if (toolchain::is_contained(SemaObj->IdResolver.decls(Name), D))
      SemaObj->TUScope->AddDecl(D);
  }
}
...
ASTReader::ASTReader(Preprocessor &PP, ModuleCache &ModCache,
                     ...
                     std::unique_ptr<toolchain::Timer> ReadTimer)
...
Expected<unsigned> ASTRecordReader::readRecord(toolchain::BitstreamCursor &Cursor,
                                               unsigned AbbrevID) {
  Idx = 0;
  Record.clear();
...
// This has to be in namespace language::Core because it's friended by all
// of the OMP clauses.
namespace language::Core {
...
OMPClause *OMPClauseReader::readClause() {
  OMPClause *C = nullptr;
  switch (toolchain::omp::Clause(Record.readInt())) {
  case toolchain::omp::OMPC_if:
    C = new (Context) OMPIfClause();
    break;
  case toolchain::omp::OMPC_final:
    C = new (Context) OMPFinalClause();
    break;
  case toolchain::omp::OMPC_num_threads:
    C = new (Context) OMPNumThreadsClause();
    break;
  case toolchain::omp::OMPC_safelen:
    C = new (Context) OMPSafelenClause();
    break;
  case toolchain::omp::OMPC_simdlen:
    C = new (Context) OMPSimdlenClause();
    break;
  case toolchain::omp::OMPC_sizes: {
    unsigned NumSizes = Record.readInt();
    C = OMPSizesClause::CreateEmpty(Context, NumSizes);
    break;
  }
  case toolchain::omp::OMPC_permutation: {
    unsigned NumLoops = Record.readInt();
    C = OMPPermutationClause::CreateEmpty(Context, NumLoops);
    break;
  }
  case toolchain::omp::OMPC_full:
    C = OMPFullClause::CreateEmpty(Context);
    break;
  case toolchain::omp::OMPC_partial:
    C = OMPPartialClause::CreateEmpty(Context);
    break;
  case toolchain::omp::OMPC_allocator:
    C = new (Context) OMPAllocatorClause();
    break;
  case toolchain::omp::OMPC_collapse:
    C = new (Context) OMPCollapseClause();
    break;
  case toolchain::omp::OMPC_default:
    C = new (Context) OMPDefaultClause();
    break;
  case toolchain::omp::OMPC_proc_bind:
    C = new (Context) OMPProcBindClause();
    break;
  case toolchain::omp::OMPC_schedule:
    C = new (Context) OMPScheduleClause();
    break;
  case toolchain::omp::OMPC_ordered:
    C = OMPOrderedClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_nowait:
    C = new (Context) OMPNowaitClause();
    break;
  case toolchain::omp::OMPC_untied:
    C = new (Context) OMPUntiedClause();
    break;
  case toolchain::omp::OMPC_mergeable:
    C = new (Context) OMPMergeableClause();
    break;
  case toolchain::omp::OMPC_read:
    C = new (Context) OMPReadClause();
    break;
  case toolchain::omp::OMPC_write:
    C = new (Context) OMPWriteClause();
    break;
  case toolchain::omp::OMPC_update:
    C = OMPUpdateClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_capture:
    C = new (Context) OMPCaptureClause();
    break;
  case toolchain::omp::OMPC_compare:
    C = new (Context) OMPCompareClause();
    break;
  case toolchain::omp::OMPC_fail:
    C = new (Context) OMPFailClause();
    break;
  case toolchain::omp::OMPC_seq_cst:
    C = new (Context) OMPSeqCstClause();
    break;
  case toolchain::omp::OMPC_acq_rel:
    C = new (Context) OMPAcqRelClause();
    break;
  case toolchain::omp::OMPC_absent: {
    unsigned NumKinds = Record.readInt();
    C = OMPAbsentClause::CreateEmpty(Context, NumKinds);
    break;
  }
  case toolchain::omp::OMPC_holds:
    C = new (Context) OMPHoldsClause();
    break;
  case toolchain::omp::OMPC_contains: {
    unsigned NumKinds = Record.readInt();
    C = OMPContainsClause::CreateEmpty(Context, NumKinds);
    break;
  }
  case toolchain::omp::OMPC_no_openmp:
    C = new (Context) OMPNoOpenMPClause();
    break;
  case toolchain::omp::OMPC_no_openmp_routines:
    C = new (Context) OMPNoOpenMPRoutinesClause();
    break;
  case toolchain::omp::OMPC_no_openmp_constructs:
    C = new (Context) OMPNoOpenMPConstructsClause();
    break;
  case toolchain::omp::OMPC_no_parallelism:
    C = new (Context) OMPNoParallelismClause();
    break;
  case toolchain::omp::OMPC_acquire:
    C = new (Context) OMPAcquireClause();
    break;
  case toolchain::omp::OMPC_release:
    C = new (Context) OMPReleaseClause();
    break;
  case toolchain::omp::OMPC_relaxed:
    C = new (Context) OMPRelaxedClause();
    break;
  case toolchain::omp::OMPC_weak:
    C = new (Context) OMPWeakClause();
    break;
  case toolchain::omp::OMPC_threads:
    C = new (Context) OMPThreadsClause();
    break;
  case toolchain::omp::OMPC_simd:
    C = new (Context) OMPSIMDClause();
    break;
  case toolchain::omp::OMPC_nogroup:
    C = new (Context) OMPNogroupClause();
    break;
  case toolchain::omp::OMPC_unified_address:
    C = new (Context) OMPUnifiedAddressClause();
    break;
  case toolchain::omp::OMPC_unified_shared_memory:
    C = new (Context) OMPUnifiedSharedMemoryClause();
    break;
  case toolchain::omp::OMPC_reverse_offload:
    C = new (Context) OMPReverseOffloadClause();
    break;
  case toolchain::omp::OMPC_dynamic_allocators:
    C = new (Context) OMPDynamicAllocatorsClause();
    break;
  case toolchain::omp::OMPC_atomic_default_mem_order:
    C = new (Context) OMPAtomicDefaultMemOrderClause();
    break;
  case toolchain::omp::OMPC_self_maps:
    C = new (Context) OMPSelfMapsClause();
    break;
  case toolchain::omp::OMPC_at:
    C = new (Context) OMPAtClause();
    break;
  case toolchain::omp::OMPC_severity:
    C = new (Context) OMPSeverityClause();
    break;
  case toolchain::omp::OMPC_message:
    C = new (Context) OMPMessageClause();
    break;
  case toolchain::omp::OMPC_private:
    C = OMPPrivateClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_firstprivate:
    C = OMPFirstprivateClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_lastprivate:
    C = OMPLastprivateClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_shared:
    C = OMPSharedClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_reduction: {
  ...
  case toolchain::omp::OMPC_num_teams:
    C = OMPNumTeamsClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_thread_limit:
    C = OMPThreadLimitClause::CreateEmpty(Context, Record.readInt());
    break;
  case toolchain::omp::OMPC_priority:
    C = new (Context) OMPPriorityClause();
    break;
  case toolchain::omp::OMPC_grainsize:
    C = new (Context) OMPGrainsizeClause();
    break;
  case toolchain::omp::OMPC_num_tasks:
    C = new (Context) OMPNumTasksClause();
    break;
  case toolchain::omp::OMPC_hint:
    C = new (Context) OMPHintClause();
    break;
  case toolchain::omp::OMPC_dist_schedule:
    C = new (Context) OMPDistScheduleClause();
    break;
  case toolchain::omp::OMPC_defaultmap:
    C = new (Context) OMPDefaultmapClause();
    break;
  case toolchain::omp::OMPC_to: {
    OMPMappableExprListSizeTy Sizes;
    Sizes.NumVars = Record.readInt();
    Sizes.NumUniqueDeclarations = Record.readInt();
    Sizes.NumComponentLists = Record.readInt();
  ...
  case toolchain::omp::OMPC_use_device_ptr: {
    OMPMappableExprListSizeTy Sizes;
    Sizes.NumVars = Record.readInt();
    Sizes.NumUniqueDeclarations = Record.readInt();
    Sizes.NumComponentLists = Record.readInt();
  ...
  case toolchain::omp::OMPC_use_device_addr: {
    OMPMappableExprListSizeTy Sizes;
    Sizes.NumVars = Record.readInt();
    Sizes.NumUniqueDeclarations = Record.readInt();
    Sizes.NumComponentLists = Record.readInt();
  ...
  case toolchain::omp::OMPC_is_device_ptr: {
    OMPMappableExprListSizeTy Sizes;
    Sizes.NumVars = Record.readInt();
    Sizes.NumUniqueDeclarations = Record.readInt();
    Sizes.NumComponentLists = Record.readInt();
  ...
  case toolchain::omp::OMPC_has_device_addr: {
    OMPMappableExprListSizeTy Sizes;
    Sizes.NumVars = Record.readInt();
    Sizes.NumUniqueDeclarations = Record.readInt();
    Sizes.NumComponentLists = Record.readInt();
...
void OMPClauseReader::VisitOMPDefaultClause(OMPDefaultClause *C) {
  C->setDefaultKind(static_cast<toolchain::omp::DefaultKind>(Record.readInt()));
  C->setLParenLoc(Record.readSourceLocation());
  C->setDefaultKindKwLoc(Record.readSourceLocation());
}

void OMPClauseReader::VisitOMPProcBindClause(OMPProcBindClause *C) {
  C->setProcBindKind(static_cast<toolchain::omp::ProcBindKind>(Record.readInt()));
  C->setLParenLoc(Record.readSourceLocation());
...
void OMPClauseReader::VisitOMPAbsentClause(OMPAbsentClause *C) {
  ...
  C->setLParenLoc(Record.readSourceLocation());
  toolchain::SmallVector<OpenMPDirectiveKind, 4> DKVec;
  DKVec.reserve(Count);
...
void OMPClauseReader::VisitOMPReductionClause(OMPReductionClause *C) {
  ...
  for ([[maybe_unused]] unsigned I : toolchain::seq<unsigned>(NumFlags))
...
OMPTraitInfo *ASTRecordReader::readOMPTraitInfo() {
  ...
  for (auto &Set : TI.Sets) {
    Set.Kind = readEnum<toolchain::omp::TraitSet>();
    Set.Selectors.resize(readUInt32());
    for (auto &Selector : Set.Selectors) {
      Selector.Kind = readEnum<toolchain::omp::TraitSelector>();
      ...
      for (auto &Property : Selector.Properties)
        Property.Kind = readEnum<toolchain::omp::TraitProperty>();
    }
  }
  return &TI;
}
...
SmallVector<Expr *> ASTRecordReader::readOpenACCVarList() {
  ...
  toolchain::SmallVector<Expr *> VarList;
  for (unsigned I = 0; I < NumVars; ++I)
...
SmallVector<Expr *> ASTRecordReader::readOpenACCIntExprList() {
  ...
  toolchain::SmallVector<Expr *> ExprList;
...
OpenACCClause *ASTRecordReader::readOpenACCClause() {
  ...
  case OpenACCClauseKind::NumGangs: {
    ...
    toolchain::SmallVector<Expr *> IntExprs;
  ...
  case OpenACCClauseKind::Private: {
    SourceLocation LParenLoc = readSourceLocation();
    toolchain::SmallVector<Expr *> VarList = readOpenACCVarList();

    toolchain::SmallVector<VarDecl *> RecipeList;
  ...
  case OpenACCClauseKind::FirstPrivate: {
    SourceLocation LParenLoc = readSourceLocation();
    toolchain::SmallVector<Expr *> VarList = readOpenACCVarList();
    toolchain::SmallVector<OpenACCFirstPrivateRecipe> RecipeList;
    for (unsigned I = 0; I < VarList.size(); ++I) {
  ...
  case OpenACCClauseKind::Wait: {
    ...
    toolchain::SmallVector<Expr *> QueueIdExprs = readOpenACCIntExprList();
  ...
  case OpenACCClauseKind::DType: {
    SourceLocation LParenLoc = readSourceLocation();
    toolchain::SmallVector<DeviceTypeArgument> Archs;
  ...
  case OpenACCClauseKind::Tile: {
    SourceLocation LParenLoc = readSourceLocation();
    unsigned NumClauses = readInt();
    toolchain::SmallVector<Expr *> SizeExprs;
    for (unsigned I = 0; I < NumClauses; ++I)
  ...
  case OpenACCClauseKind::Gang: {
    SourceLocation LParenLoc = readSourceLocation();
    unsigned NumExprs = readInt();
    toolchain::SmallVector<OpenACCGangKind> GangKinds;
    toolchain::SmallVector<Expr *> Exprs;
    for (unsigned I = 0; I < NumExprs; ++I) {
  ...
  case OpenACCClauseKind::Invalid:
    toolchain_unreachable("Clause serialization not yet implemented");
  }
...
UnsignedOrNone language::Core::getPrimaryModuleHash(const Module *M) {
  if (!M)
    return std::nullopt;
