...
namespace language::Core {
  ...
  public:
    ASTStmtReader(ASTRecordReader &Record, toolchain::BitstreamCursor &Cursor)
...
void ASTStmtReader::VisitFloatingLiteral(FloatingLiteral *E) {
  VisitExpr(E);
  E->setRawSemantics(
      static_cast<toolchain::APFloatBase::Semantics>(Record.readInt()));
...
void ASTStmtReader::VisitRequiresExpr(RequiresExpr *E) {
  ...
  toolchain::SmallVector<ParmVarDecl *, 4> LocalParameters;
  ...
  toolchain::SmallVector<concepts::Requirement *, 4> Requirements;
      ...
      case concepts::Requirement::RK_Compound: {
        auto Status =
            static_cast<concepts::ExprRequirement::SatisfactionStatus>(
                Record.readInt());
        toolchain::PointerUnion<concepts::Requirement::SubstitutionDiagnostic *,
                           Expr *> E;
...
Stmt *ASTReader::ReadStmt(ModuleFile &F) {
  switch (ReadingKind) {
  case Read_None:
    toolchain_unreachable("should not call this when not reading anything");
  case Read_Decl:
  case Read_Type:
  ...
  toolchain_unreachable("ReadingKind not set ?");
}
...
Stmt *ASTReader::ReadStmtFromStream(ModuleFile &F) {
  ...
  toolchain::DenseMap<uint64_t, Stmt *> StmtEntries;
  ...
  while (true) {
    ...
    case EXPR_OBJC_KVC_REF_EXPR:
      toolchain_unreachable("mismatching AST file");
