...
#include "language/Core/Serialization/ASTRecordWriter.h"
...
#include "language/Core/Serialization/TypeBitCodes.def"
  case Type::Builtin:
    toolchain_unreachable("shouldn't be serializing a builtin type this way");
  }
...
struct AffectingModuleMaps {
  toolchain::DenseSet<FileID> DefinitionFileIDs;
  toolchain::DenseSet<const FileEntry *> DefinitionFiles;
};
...
GetAffectingModuleMaps(const Preprocessor &PP, Module *RootModule) {
  ...
  toolchain::DenseMap<FileID, AffectedReason> ModuleMaps;
  toolchain::DenseMap<const Module *, AffectedReason> ProcessedModules;
  ...
  toolchain::DenseSet<const FileEntry *> ModuleFileEntries;
  toolchain::DenseSet<FileID> ModuleFileIDs;
...
void TypeLocWriter::VisitBitIntTypeLoc(language::Core::BitIntTypeLoc TL) {
  addSourceLocation(TL.getNameLoc());
}
...
void TypeLocWriter::VisitPredefinedSugarTypeLoc(
    language::Core::PredefinedSugarTypeLoc TL) {
  // Nothing to do.
}
...
static void AddStmtsExprs(toolchain::BitstreamWriter &Stream,
                          ASTWriter::RecordDataImpl &Record) {
...
static bool cleanPathForOutput(FileManager &FileMgr,
  ...
  return Changed | toolchain::sys::path::remove_dots(Path);
}
...
adjustFilenameForRelocatableAST(const char *Filename, StringRef BaseDir) {
  ...
  if (!toolchain::sys::path::is_separator(Filename[Pos])) {
    if (!toolchain::sys::path::is_separator(BaseDir.back()))
      return Filename;
  } else {
...
static void BackpatchSignatureAt(toolchain::BitstreamWriter &Stream,
...
void ASTWriter::WriteControlBlock(Preprocessor &PP, StringRef isysroot) {
  ...
  if (WritingModule && WritingModule->Kind == Module::ModuleMapModule) {
    ...
    if (auto *AdditionalModMaps =
      ...
      toolchain::sort(ModMaps, [](FileEntryRef A, FileEntryRef B) {
  ...
  if (Chain) {
    ...
    for (ModuleFile &M : Chain->getModuleManager()) {
      ...
      if (M.StandardCXXModule) {
        ...
        toolchain::append_range(Blob, M.Signature);
...
struct InputFileEntry {
  ...
  void trySetContentHash(
      Preprocessor &PP,
      toolchain::function_ref<std::optional<toolchain::MemoryBufferRef>()> GetMemBuff) {
    ContentHash[0] = 0;
    ContentHash[1] = 0;
...
void ASTWriter::WriteInputFiles(SourceManager &SourceMgr) {
  ...
  if (!Sysroot.empty()) {
    ...
    toolchain::sys::path::append(SDKSettingsJSON, "SDKSettings.json");
  ...
  auto SortedFiles = toolchain::concat<InputFileEntry>(std::move(UserFiles),
...
/// file.
static unsigned CreateSLocFileAbbrev(toolchain::BitstreamWriter &Stream) {
  using namespace toolchain;
...
/// Create an abbreviation for the SLocEntry that refers to a
/// buffer.
static unsigned CreateSLocBufferAbbrev(toolchain::BitstreamWriter &Stream) {
  using namespace toolchain;
...
/// Create an abbreviation for the SLocEntry that refers to a
/// buffer's blob.
static unsigned CreateSLocBufferBlobAbbrev(toolchain::BitstreamWriter &Stream,
                                           bool Compressed) {
  using namespace toolchain;
...
/// expansion.
static unsigned CreateSLocExpansionAbbrev(toolchain::BitstreamWriter &Stream) {
  using namespace toolchain;
...
emitULEBKeyDataLength(unsigned KeyLen, unsigned DataLen, raw_ostream &Out) {
  toolchain::encodeULEB128(KeyLen, Out);
  toolchain::encodeULEB128(DataLen, Out);
...
namespace {
  ...
  public:
    ...
    using UnresolvedModule =
        toolchain::PointerIntPair<Module *, 2, ModuleMap::ModuleHeaderRole>;
...
void ASTWriter::WriteHeaderSearch(const HeaderSearch &HS) {
  ...
  toolchain::OnDiskChainedHashTableGenerator<HeaderFileInfoTrait> Generator;
  ...
  if (WritingModule) {
    toolchain::SmallVector<Module *, 16> Worklist(1, WritingModule);
    while (!Worklist.empty()) {
      ...
      for (const auto &U : M->MissingHeaders) {
        ...
        toolchain::sys::path::append(Filename, U.FileName);
  ...
  uint32_t BucketOffset;
  {
    ...
    toolchain::raw_svector_ostream Out(TableData);
...
static void emitBlob(toolchain::BitstreamWriter &Stream, StringRef Blob,
  ...
  if (toolchain::compression::zstd::isAvailable()) {
    toolchain::compression::zstd::compress(
        toolchain::arrayRefFromStringRef(Blob.drop_back(1)), CompressedBuffer, 9);
    RecordDataType Record[] = {SM_SLOC_BUFFER_BLOB_COMPRESSED, Blob.size() - 1};
    Stream.EmitRecordWithBlob(SLocBufferBlobCompressedAbbrv, Record,
                              toolchain::toStringRef(CompressedBuffer));
    return;
  }
  if (toolchain::compression::zlib::isAvailable()) {
    toolchain::compression::zlib::compress(
        toolchain::arrayRefFromStringRef(Blob.drop_back(1)), CompressedBuffer);
    RecordDataType Record[] = {SM_SLOC_BUFFER_BLOB_COMPRESSED, Blob.size() - 1};
    Stream.EmitRecordWithBlob(SLocBufferBlobCompressedAbbrv, Record,
                              toolchain::toStringRef(CompressedBuffer));
    return;
...
void ASTWriter::WriteSourceManagerBlock(SourceManager &SourceMgr) {
  ...
  for (unsigned I = 1, N = SourceMgr.local_sloc_entry_size();
    ...
    if (SLoc->isFile()) {
      ...
      if (Content->OrigEntry) {
        ...
        // that is required by toolchain::MemoryBuffer::getMemBuffer (on
        // the reader side).
        std::optional<toolchain::MemoryBufferRef> Buffer = Content->getBufferOrNone(
      ...
      if (EmitBlob) {
        ...
        std::optional<toolchain::MemoryBufferRef> Buffer = Content->getBufferOrNone(
            SourceMgr.getDiagnostics(), SourceMgr.getFileManager());
        if (!Buffer)
          Buffer = toolchain::MemoryBufferRef("<<<INVALID BUFFER>>>", "");
  ...
  if (SourceMgr.hasLineTable()) {
    ...
    toolchain::DenseMap<int, int> FilenameMap;
...
void ASTWriter::WritePreprocessor(const Preprocessor &PP, bool IsModule) {
  ...
  toolchain::sort(MacroIdentifiers, toolchain::deref<std::less<>>());
  ...
  for (const IdentifierInfo *Name : MacroIdentifiers) {
    ...
    if (IsModule && WritingModule->isHeaderUnit()) {
      ...
      toolchain::DenseMap<ModuleMacro *, unsigned> Visits;
      while (!Worklist.empty()) {
...
void ASTWriter::WritePreprocessorDetail(PreprocessingRecord &PPRec,
  ...
  for (PreprocessingRecord::iterator E = PPRec.local_begin(),
    ...
    toolchain_unreachable("Unhandled PreprocessedEntity in ASTWriter");
  }
  Stream.ExitBlock();
...
void ASTWriter::WritePragmaDiagnosticMappings(const DiagnosticsEngine &Diag,
                                              bool isModule) {
  toolchain::SmallDenseMap<const DiagnosticsEngine::DiagState *, unsigned, 64>
      DiagStateIDMap;
  unsigned CurrID = 0;
  RecordData Record;
  ...
  auto AddDiagState = [&](const DiagnosticsEngine::DiagState *State,
    ...
    if (DiagStateID == 0) {
      ...
      toolchain::sort(Mappings, toolchain::less_first());
...
void ASTWriter::WriteType(ASTContext &Context, QualType T) {
  ...
  if (TypeOffsets.size() == Index)
    ...
    toolchain_unreachable("Types emitted in wrong order");
  }
}
...
void ASTWriter::WriteFileDeclIDsMap() {
  ...
  toolchain::sort(SortedFileDeclIDs, toolchain::less_first());
  ...
  for (auto &FileDeclEntry : SortedFileDeclIDs) {
    ...
    toolchain::stable_sort(Info.DeclIDs);
...
void ASTWriter::WriteComments(ASTContext &Context) {
  ...
  auto _ = toolchain::make_scope_exit([this] { Stream.ExitBlock(); });
...
void ASTWriter::WriteSelectors(Sema &SemaRef) {
  ...
  {
    toolchain::OnDiskChainedHashTableGenerator<ASTMethodPoolTrait> Generator;
    ...
    uint32_t BucketOffset;
    {
      ...
      ASTMethodPoolTrait Trait(*this);
      toolchain::raw_svector_ostream Out(MethodPool);
      // Make sure that no bucket is at offset 0
      endian::write<uint32_t>(Out, 0, toolchain::endianness::little);
...
public:
  ...
  static hash_value_type ComputeHash(const IdentifierInfo* II) {
    return toolchain::djbHash(II->getName());
  }
  ...
  void EmitData(raw_ostream &Out, const IdentifierInfo *II, IdentifierID ID,
    ...
    if (NeedDecls && IdResolver) {
      ...
      for (NamedDecl *D : toolchain::reverse(Decls))
...
void ASTWriter::WriteIdentifierTable(Preprocessor &PP,
  ...
  // strings.
  {
    toolchain::OnDiskChainedHashTableGenerator<ASTIdentifierTableTrait> Generator;
...
protected:
  ASTWriter &Writer;
  using DeclIDsTy = toolchain::SmallVector<LocalDeclID, 64>;
  DeclIDsTy DeclIDs;
...
public:
  ...
  void EmitKeyBase(raw_ostream &Out, DeclarationNameKey Name) {
    ...
    toolchain_unreachable("Invalid name kind?");
  }
...
public:
  using TULocalDeclsMapTy = toolchain::DenseMap<key_type, DeclIDsTy>;

  using ModuleLevelDeclsMapTy =
      toolchain::DenseMap<ModuleLevelNameLookupTrait::key_type, DeclIDsTy>;

private:
  enum class LookupVisibility {
    GenerallyVisibile,
    // The decls can only be found by other TU in the same module.
    // Note a language::Core::Module models a module unit instead of logical module
    // in C++20.
    ModuleLocalVisible,
...
class LazySpecializationInfoLookupTrait {
  ASTWriter &Writer;
  toolchain::SmallVector<serialization::reader::LazySpecializationInfo, 64> Specs;
...
unsigned CalculateODRHashForSpecs(const Decl *Spec) {
  ...
  if (auto *CTSD = dyn_cast<ClassTemplateSpecializationDecl>(Spec))
    ...
    toolchain_unreachable("New Specialization Kind?");
...
void ASTWriter::GenerateSpecializationInfoLookupTable(
    const NamedDecl *D, toolchain::SmallVectorImpl<const Decl *> &Specializations,
    toolchain::SmallVectorImpl<char> &LookupTable, bool IsPartial) {
  ...
  toolchain::MapVector<unsigned, toolchain::SmallVector<const NamedDecl *, 4>>
      SpecializationMaps;
    ...
    if (Iter == SpecializationMaps.end())
      Iter = SpecializationMaps
                 .try_emplace(HashedValue,
                              toolchain::SmallVector<const NamedDecl *, 4>())
                 .first;
  ...
  for (auto &[HashValue, Specs] : SpecializationMaps) {
    ...
    // language::Core::serialization::MultiOnDiskHashTableGenerator since that generator
    // assumes the we'll get the same value with the same key.
    // And also underlying toolchain::OnDiskChainedHashTableGenerator assumes that we
...
uint64_t ASTWriter::WriteSpecializationInfoLookupTable(
  ...
  toolchain::SmallString<4096> LookupTable;
...
void ASTWriter::GenerateNameLookupTable(
    ...
    toolchain::SmallVectorImpl<char> &LookupTable,
    toolchain::SmallVectorImpl<char> &ModuleLocalLookupTable,
    toolchain::SmallVectorImpl<char> &TULookupTable) {
  ...
  if (IncludeConstructorNames || IncludeConversionNames) {
    ...
    toolchain::SmallPtrSet<DeclarationName, 8> AddedNames;
...
void ASTWriter::WriteDeclContextVisibleBlock(
  ...
  if (isa<NamespaceDecl>(DC) && Chain &&
    ...
    toolchain::sort(LookupResults, toolchain::less_first());
...
void ASTWriter::WriteObjCCategories() {
  ...
  toolchain::array_pod_sort(CategoriesMap.begin(), CategoriesMap.end());
...
void ASTWriter::WriteModuleFileExtension(Sema &SemaRef,
  ...
  Abv->Add(toolchain::BitCodeAbbrevOp(EXTENSION_METADATA));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Blob));
...
void ASTRecordWriter::AddAttr(const Attr *A) {
  ...
  // https://github.com/toolchain/toolchain-project/issues/56490 for example.
  if (!A ||
...
#include "language/Core/Serialization/AttrPCHWrite.inc"
}
...
void ASTWriter::AddToken(const Token &Tok, RecordDataImpl &Record) {
  ...
  if (Tok.isAnnotation()) {
    ...
    case tok::annot_repl_input_end:
      break;
    default:
      toolchain_unreachable("missing serialization code for annotation token");
    }
  } else {
...
ASTWriter::ASTWriter(toolchain::BitstreamWriter &Stream,
...
ASTFileSignature
ASTWriter::WriteAST(toolchain::PointerUnion<Sema *, Preprocessor *> Subject,
                    StringRef OutputFile, Module *WritingModule,
                    StringRef isysroot, bool ShouldCacheASTInMemory) {
  toolchain::TimeTraceScope scope("WriteAST", OutputFile);
  WritingAST = true;
  ...
  if (ShouldCacheASTInMemory) {
    ...
    ModCache.getInMemoryModuleCache().addBuiltPCM(
        OutputFile, toolchain::MemoryBuffer::getMemBufferCopy(
...
#include "language/Core/Basic/BuiltinTemplates.inc"
  ...
  if (!WritingModule || !getLangOpts().CPlusPlus) {
    toolchain::SmallVector<const IdentifierInfo*, 256> IIs;
    ...
    toolchain::sort(IIs, toolchain::deref<std::less<>>());
...
ASTFileSignature ASTWriter::WriteASTCore(Sema *SemaPtr, StringRef isysroot,
  ...
  if (SemaPtr) {
    toolchain::SmallVector<Selector, 256> AllSelectors;
    for (auto &SelectorAndID : SelectorIDs)
  ...
  if (!WritingModule) {
    ...
    toolchain::SmallVector<ModuleInfo, 64> Imports;
    if (SemaPtr) {
    ...
    if (!Imports.empty()) {
      ...
      toolchain::sort(Imports, Cmp);
      Imports.erase(toolchain::unique(Imports, Eq), Imports.end());
...
void ASTWriter::WriteDeclAndTypes(ASTContext &Context) {
  ...
  if (!RelatedDeclsMap.empty()) {
    ...
    auto Abv = std::make_shared<toolchain::BitCodeAbbrev>();
    Abv->Add(toolchain::BitCodeAbbrevOp(RELATED_DECLS_MAP));
    Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Array));
    Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  ...
  auto Abv = std::make_shared<toolchain::BitCodeAbbrev>();
  Abv->Add(toolchain::BitCodeAbbrevOp(TU_UPDATE_LEXICAL));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Blob));
  ...
  Abv = std::make_shared<toolchain::BitCodeAbbrev>();
  Abv->Add(toolchain::BitCodeAbbrevOp(UPDATE_VISIBLE));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Blob));
  UpdateVisibleAbbrev = Stream.EmitAbbrev(std::move(Abv));

  Abv = std::make_shared<toolchain::BitCodeAbbrev>();
  Abv->Add(toolchain::BitCodeAbbrevOp(UPDATE_MODULE_LOCAL_VISIBLE));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Blob));
  ModuleLocalUpdateVisibleAbbrev = Stream.EmitAbbrev(std::move(Abv));

  Abv = std::make_shared<toolchain::BitCodeAbbrev>();
  Abv->Add(toolchain::BitCodeAbbrevOp(UPDATE_TU_LOCAL_VISIBLE));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Blob));
...
void ASTWriter::WriteSpecializationsUpdates(bool IsPartial) {
  ...
  auto Abv = std::make_shared<toolchain::BitCodeAbbrev>();
  Abv->Add(toolchain::BitCodeAbbrevOp(RecordType));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::VBR, 6));
  Abv->Add(toolchain::BitCodeAbbrevOp(toolchain::BitCodeAbbrevOp::Blob));
  ...
  for (auto &DeclUpdate : LocalUpdates) {
    ...
    for (auto &Update : DeclUpdate.second) {
      ...
      if (Kind == DeclUpdateKind::CXXAddedFunctionDefinition)
        ...
        Record.push_back(toolchain::to_underlying(Kind));
      ...
      case DeclUpdateKind::CXXInstantiatedClassDefinition: {
        ...
        Record.push_back(toolchain::to_underlying(RD->getArgPassingRestrictions()));
        ...
        Record.push_back(toolchain::to_underlying(RD->getTagKind()));
      ...
      case DeclUpdateKind::AddedAttrToRecord:
        Record.AddAttributes(toolchain::ArrayRef(Update.getAttr()));
        break;
      }
    }
    ...
    if (!GeneratingReducedBMI || !CanElideDeclDef(D)) {
      if (HasUpdatedBody) {
        ...
        Record.push_back(
            toolchain::to_underlying(DeclUpdateKind::CXXAddedFunctionDefinition));
        ...
        Record.push_back(
            toolchain::to_underlying(DeclUpdateKind::CXXAddedVarDefinition));
...
FileID ASTWriter::getAdjustedFileID(FileID FID) const {
  if (FID.isInvalid() || PP->getSourceManager().isLoadedFileID(FID) ||
    ...
    return FID;
  auto It = toolchain::lower_bound(NonAffectingFileIDs, FID);
...
ASTWriter::getAdjustment(SourceLocation::UIntTy Offset) const {
  ...
  auto It = toolchain::lower_bound(NonAffectingRanges, Offset, Contains);
...
void ASTRecordWriter::AddAPFloat(const toolchain::APFloat &Value) {
...
#include "language/Core/AST/CXXRecordDeclDefinitionBits.def"
#undef FIELD
...
void ASTWriter::AddedVisibleDecl(const DeclContext *DC, const Decl *D) {
  ...
  if (UpdatedDeclContexts.insert(DC) && !cast<Decl>(DC)->isFromASTFile()) {
    ...
    toolchain::append_range(DeclsToEmitEvenIfUnreferenced, DC->decls());
  }
...
void OMPClauseWriter::VisitOMPReductionClause(OMPReductionClause *C) {
  ...
  for (auto *E : C->reduction_ops())
    Record.AddStmt(E);
  if (C->getModifier() == language::Core::OMPC_REDUCTION_inscan) {
