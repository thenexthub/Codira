...
public:
  TypeVisitorHelper(toolchain::FoldingSetNodeID &ID, TemplateArgumentHasher &Hash)
      : ID(ID), Hash(Hash) {}
  ...
  void VisitArrayType(const ArrayType *T) {
    ...
    Hash.AddInteger(toolchain::to_underlying(T->getSizeModifier()));
  ...
  void VisitVectorType(const VectorType *T) {
    ...
    Hash.AddInteger(toolchain::to_underlying(T->getVectorKind()));
  }
...
unsigned language::Core::serialization::StableHashForTemplateArguments(
    toolchain::ArrayRef<TemplateArgument> Args) {
  toolchain::TimeTraceScope TimeScope("Stable Hash for Template Arguments");
