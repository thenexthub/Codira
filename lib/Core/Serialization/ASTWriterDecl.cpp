...
namespace language::Core {
  ...
  public:
    ...
    uint64_t Emit(Decl *D) {
      if (!Code)
        toolchain::report_fatal_error(StringRef("unexpected declaration kind '") +
            D->getDeclKindName() + "'");
    ...
    void CollectFirstDeclFromEachModule(
        ...
        toolchain::MapVector<ModuleFile *, const Decl *> &Firsts) {
    ...
    void AddFirstSpecializationDeclFromEachModule(
        const Decl *D, toolchain::SmallVectorImpl<const Decl *> &SpecsInMap,
        toolchain::SmallVectorImpl<const Decl *> &PartialSpecsInMap) {
    ...
    void AddTemplateSpecializations(DeclTy *D) {
      ...
      toolchain::SmallVector<const Decl *, 16> AllSpecs;
      for (auto &Entry : Common->Specializations)
        AllSpecs.push_back(getSpecializationDecl(Entry));
      for (auto &Entry : getPartialSpecializations(Common))
        AllSpecs.push_back(getSpecializationDecl(Entry));

      toolchain::SmallVector<const Decl *, 16> Specs;
      toolchain::SmallVector<const Decl *, 16> PartialSpecs;
      for (auto *D : AllSpecs) {
...
bool language::Core::CanElideDeclDef(const Decl *D) {
...
void ASTDeclWriter::VisitTranslationUnitDecl(TranslationUnitDecl *D) {
  toolchain_unreachable("Translation units aren't directly serialized");
}
...
void ASTDeclWriter::VisitTagDecl(TagDecl *D) {
  ...
  TagDeclBits.addBits(toolchain::to_underlying(D->getTagKind()), /*BitWidth=*/3);
...
void ASTDeclWriter::VisitRecordDecl(RecordDecl *D) {
  ...
  RecordDeclBits.addBits(toolchain::to_underlying(D->getArgPassingRestrictions()), 2);
...
void ASTDeclWriter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  FunctionDeclBits.addBits(toolchain::to_underlying(D->getLinkageInternal()), 3);
...
void ASTDeclWriter::VisitObjCMethodDecl(ObjCMethodDecl *D) {
  ...
  Record.push_back(toolchain::to_underlying(D->getImplementationControl()));
...
void ASTDeclWriter::VisitObjCImplementationDecl(ObjCImplementationDecl *D) {
  ...
  if (D->NumIvarInitializers)
    Record.AddCXXCtorInitializers(
        toolchain::ArrayRef(D->init_begin(), D->init_end()));
...
void ASTDeclWriter::VisitVarDecl(VarDecl *D) {
  ...
  VarDeclBits.addBits(toolchain::to_underlying(D->getLinkageInternal()),
                      /*BitWidth=*/3);
  ...
  if (!isa<ParmVarDecl>(D)) {
    ...
    if (const auto *IPD = dyn_cast<ImplicitParamDecl>(D))
      VarDeclBits.addBits(toolchain::to_underlying(IPD->getParameterKind()),
                          /*Width=*/3);
    else
...
void ASTDeclWriter::VisitLinkageSpecDecl(LinkageSpecDecl *D) {
  ...
  VisitDecl(D);
  Record.push_back(toolchain::to_underlying(D->getLanguage()));
...
void ASTDeclWriter::VisitCXXRecordDecl(CXXRecordDecl *D) {
  ...
  if (ClassTemplateDecl *TemplD = D->getDescribedClassTemplate()) {
    ...
    if (auto *FD = toolchain::dyn_cast_or_null<FunctionDecl>(D->getDeclContext());
...
void ASTDeclWriter::VisitOMPDeclareReductionDecl(OMPDeclareReductionDecl *D) {
  ...
  Record.push_back(toolchain::to_underlying(D->getInitializerKind()));
...
std::shared_ptr<toolchain::BitCodeAbbrev>
getFunctionDeclAbbrev(serialization::DeclCode Code) {
  ...
  if constexpr (Kind == FunctionDecl::TK_NonTemplate) {
    ...
    toolchain_unreachable("Unknown templated kind?");
  }
  // Decl
  Abv->Add(BitCodeAbbrevOp(BitCodeAbbrevOp::Fixed,
...
template <FunctionDecl::TemplatedKind Kind>
std::shared_ptr<toolchain::BitCodeAbbrev> getCXXMethodAbbrev() {
...
void ASTWriter::WriteDecl(ASTContext &Context, Decl *D) {
  ...
  if (DeclOffsets.size() == Index)
    ...
    toolchain_unreachable("declarations should be emitted in ID order");
  }
...
void ASTRecordWriter::AddFunctionDefinition(const FunctionDecl *FD) {
  ...
  if (auto *CD = dyn_cast<CXXConstructorDecl>(FD)) {
    ...
    if (CD->getNumCtorInitializers())
      AddCXXCtorInitializers(toolchain::ArrayRef(CD->init_begin(), CD->init_end()));
  }
  AddStmt(FD->getBody());
}
