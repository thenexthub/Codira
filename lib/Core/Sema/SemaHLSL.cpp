...
#include "language/Core/AST/Attrs.inc"
...
#include "toolchain/Frontend/HLSL/HLSLBinding.h"
#include "toolchain/Frontend/HLSL/RootSignatureValidations.h"
#include "toolchain/Support/Casting.h"
#include "toolchain/Support/DXILABI.h"
...
static RegisterType getRegisterType(ResourceClass RC) {
  ...
  toolchain_unreachable("unexpected ResourceClass value");
}
...
static ResourceClass getResourceClass(RegisterType RT) {
  ...
  toolchain_unreachable("unexpected RegisterType value");
}
...
Decl *SemaHLSL::ActOnStartBuffer(Scope *BufferScope, bool CBuffer,
  ...
  auto RC = CBuffer ? toolchain::hlsl::ResourceClass::CBuffer
                    : toolchain::hlsl::ResourceClass::SRV;
...
static unsigned calculateLegacyCbufferSize(const ASTContext &Context,
  ...
  if (const RecordType *RT = T->getAs<RecordType>()) {
    ...
    for (const FieldDecl *Field : RD->fields()) {
      ...
      // next row
      unsigned AlignSize = toolchain::alignTo(Size, FieldAlign);
      if ((AlignSize % CBufferAlign) + FieldSize > CBufferAlign) {
        FieldAlign = CBufferAlign;
      }

      Size = toolchain::alignTo(Size, FieldAlign);
      Size += FieldSize;
    }
    return Size;
  }
  ...
  if (const ConstantArrayType *AT = Context.getAsConstantArrayType(T)) {
    ...
    unsigned AlignedElementSize = toolchain::alignTo(ElementSize, CBufferAlign);
...
static void validatePackoffset(Sema &S, HLSLBufferDecl *BufDecl) {
  toolchain::SmallVector<std::pair<VarDecl *, HLSLPackOffsetAttr *>> PackOffsetVec;
...
static IdentifierInfo *getHostLayoutStructName(Sema &S, NamedDecl *BaseDecl,
  ...
  toolchain::SmallString<64> Name("__cblayout_");
  if (NameBaseII) {
  ...
  while (true) {
    if (suffix != 0) {
      Name.append("_");
      Name.append(toolchain::Twine(suffix).str());
...
SemaHLSL::mergeVkConstantIdAttr(Decl *D, const AttributeCommonInfo &AL,
  ...
  if (TargetInfo.getTriple().getArch() != toolchain::Triple::spirv) {
...
void SemaHLSL::ActOnTopLevelFunction(FunctionDecl *FD) {
  ...
  toolchain::Triple::EnvironmentType Env = TargetInfo.getTriple().getEnvironment();
  if (HLSLShaderAttr::isValidShaderType(Env) && Env != toolchain::Triple::Library) {
    ...
    case toolchain::Triple::Library:
      break;
    default:
      toolchain_unreachable("Unhandled environment in triple");
    }
  }
}
...
void SemaHLSL::CheckEntryPoint(FunctionDecl *FD) {
  ...
  toolchain::Triple::EnvironmentType ST = ShaderAttr->getType();
  auto &TargetInfo = getASTContext().getTargetInfo();
  VersionTuple Ver = TargetInfo.getTriple().getOSVersion();
  switch (ST) {
  case toolchain::Triple::Pixel:
  case toolchain::Triple::Vertex:
  case toolchain::Triple::Geometry:
  case toolchain::Triple::Hull:
  case toolchain::Triple::Domain:
  case toolchain::Triple::RayGeneration:
  case toolchain::Triple::Intersection:
  case toolchain::Triple::AnyHit:
  case toolchain::Triple::ClosestHit:
  case toolchain::Triple::Miss:
  case toolchain::Triple::Callable:
    if (const auto *NT = FD->getAttr<HLSLNumThreadsAttr>()) {
      DiagnoseAttrStageMismatch(NT, ST,
                                {toolchain::Triple::Compute,
                                 toolchain::Triple::Amplification,
                                 toolchain::Triple::Mesh});
      FD->setInvalidDecl();
    }
  ...
  case toolchain::Triple::Compute:
  case toolchain::Triple::Amplification:
  case toolchain::Triple::Mesh:
    if (!FD->hasAttr<HLSLNumThreadsAttr>()) {
      Diag(FD->getLocation(), diag::err_hlsl_missing_numthreads)
          << toolchain::Triple::getEnvironmentTypeName(ST);
      FD->setInvalidDecl();
    }
    if (const auto *WS = FD->getAttr<HLSLWaveSizeAttr>()) {
  ...
  for (ParmVarDecl *Param : FD->parameters()) {
    if (const auto *AnnotationAttr = Param->getAttr<HLSLAnnotationAttr>()) {
      ...
      // See: https://github.com/toolchain/toolchain-project/issues/57875
...
void SemaHLSL::CheckSemanticAnnotation(
  ...
  case attr::HLSLSV_GroupID:
    if (ST == toolchain::Triple::Compute)
      return;
    DiagnoseAttrStageMismatch(AnnotationAttr, ST, {toolchain::Triple::Compute});
    break;
  case attr::HLSLSV_Position:
    // TODO(#143523): allow use on other shader types & output once the overall
    // semantic logic is implemented.
    if (ST == toolchain::Triple::Pixel)
      return;
    DiagnoseAttrStageMismatch(AnnotationAttr, ST, {toolchain::Triple::Pixel});
    break;
  default:
    toolchain_unreachable("Unknown HLSLAnnotationAttr");
  }
}
...
void SemaHLSL::DiagnoseAttrStageMismatch(
  ...
  toolchain::transform(AllowedStages, std::back_inserter(StageStrings),
                  [](toolchain::Triple::EnvironmentType ST) {
                    return StringRef(
                        HLSLShaderAttr::ConvertEnvironmentTypeToStr(ST));
                  });
  Diag(A->getLoc(), diag::err_hlsl_attr_unsupported_in_stage)
      << A->getAttrName() << toolchain::Triple::getEnvironmentTypeName(Stage)
...
static QualType handleFloatVectorBinOpConversion(
  ...
  assert(RHSFloat);
  if (IsCompAssign)
    return castElement<language::Core::CK_FloatingToIntegral>(SemaRef, RHS, LHSType);
...
void SemaHLSL::emitLogicalOperatorFixIt(Expr *LHS, Expr *RHS,
  ...
  toolchain::SmallVector<char, 256> Buff;
  toolchain::raw_svector_ostream OS(Buff);
...
SemaHLSL::ActOnStartRootSignatureDecl(StringRef Signature) {
  toolchain::hash_code Hash = toolchain::hash_value(Signature);
...
void SemaHLSL::ActOnFinishRootSignatureDecl(
  ...
  SmallVector<toolchain::hlsl::rootsig::RootElement> Elements;
...
struct PerVisibilityBindingChecker {
  SemaHLSL *S;
  // We need one builder per `toolchain::dxbc::ShaderVisibility` value.
  std::array<toolchain::hlsl::BindingInfoBuilder, 8> Builders;

  struct ElemInfo {
    const hlsl::RootSignatureElement *Elem;
    toolchain::dxbc::ShaderVisibility Vis;
    bool Diagnosed;
  };
  toolchain::SmallVector<ElemInfo> ElemInfoMap;

  PerVisibilityBindingChecker(SemaHLSL *S) : S(S) {}

  void trackBinding(toolchain::dxbc::ShaderVisibility Visibility,
                    toolchain::dxil::ResourceClass RC, uint32_t Space,
                    uint32_t LowerBound, uint32_t UpperBound,
                    const hlsl::RootSignatureElement *Elem) {
    uint32_t BuilderIndex = toolchain::to_underlying(Visibility);
    ...
    static_assert(toolchain::to_underlying(toolchain::dxbc::ShaderVisibility::All) == 0,
                  "'All' visibility must come first");
    if (Visibility == toolchain::dxbc::ShaderVisibility::All)
  ...
  bool checkOverlap() {
    toolchain::sort(ElemInfoMap, [](const auto &LHS, const auto &RHS) {
      return LHS.Elem < RHS.Elem;
    });

    bool HadOverlap = false;

    using toolchain::hlsl::BindingInfoBuilder;
    auto ReportOverlap = [this, &HadOverlap](
      ...
      toolchain::dxbc::ShaderVisibility CommonVis =
          Info.Vis == toolchain::dxbc::ShaderVisibility::All ? PrevInfo.Vis
                                                        : Info.Vis;

      this->S->Diag(Elem->getLocation(), diag::err_hlsl_resource_range_overlap)
          << toolchain::to_underlying(Reported.RC) << Reported.LowerBound
          << Reported.isUnbounded() << Reported.UpperBound
          << toolchain::to_underlying(Previous.RC) << Previous.LowerBound
...
bool SemaHLSL::handleRootSignatureElements(
  ...
  auto ReportFloatError = [this, &HadError](SourceLocation Loc,
    ...
    HadError = true;
    this->Diag(Loc, diag::err_hlsl_invalid_rootsig_value)
        << toolchain::formatv("{0:f}", LowerBound).sstr<6>()
        << toolchain::formatv("{0:f}", UpperBound).sstr<6>();
  };

  auto VerifyRegister = [ReportError](SourceLocation Loc, uint32_t Register) {
    if (!toolchain::hlsl::rootsig::verifyRegisterValue(Register))
      ReportError(Loc, 0, 0xfffffffe);
  };

  auto VerifySpace = [ReportError](SourceLocation Loc, uint32_t Space) {
    if (!toolchain::hlsl::rootsig::verifyRegisterSpace(Space))
      ReportError(Loc, 0, 0xffffffef);
  };

  const uint32_t Version =
      toolchain::to_underlying(SemaRef.getLangOpts().HLSLRootSigVer);
  ...
  for (const hlsl::RootSignatureElement &RootSigElem : Elements) {
    ...
    const toolchain::hlsl::rootsig::RootElement &Elem = RootSigElem.getElement();
    if (const auto *Descriptor =
            std::get_if<toolchain::hlsl::rootsig::RootDescriptor>(&Elem)) {
      VerifyRegister(Loc, Descriptor->Reg.Number);
      VerifySpace(Loc, Descriptor->Space);

      if (!toolchain::hlsl::rootsig::verifyRootDescriptorFlag(
              Version, toolchain::to_underlying(Descriptor->Flags)))
        ReportFlagError(Loc);
    } else if (const auto *Constants =
                   std::get_if<toolchain::hlsl::rootsig::RootConstants>(&Elem)) {
      VerifyRegister(Loc, Constants->Reg.Number);
      VerifySpace(Loc, Constants->Space);
    } else if (const auto *Sampler =
                   std::get_if<toolchain::hlsl::rootsig::StaticSampler>(&Elem)) {
      ...
      if (!toolchain::hlsl::rootsig::verifyMaxAnisotropy(Sampler->MaxAnisotropy))
        ReportError(Loc, 0, 16);
      if (!toolchain::hlsl::rootsig::verifyMipLODBias(Sampler->MipLODBias))
        ReportFloatError(Loc, -16.f, 15.99f);
    } else if (const auto *Clause =
                   std::get_if<toolchain::hlsl::rootsig::DescriptorTableClause>(
                       &Elem)) {
      VerifyRegister(Loc, Clause->Reg.Number);
      VerifySpace(Loc, Clause->Space);

      if (!toolchain::hlsl::rootsig::verifyNumDescriptors(Clause->NumDescriptors)) {
      ...
      if (!toolchain::hlsl::rootsig::verifyDescriptorRangeFlag(
              Version, toolchain::to_underlying(Clause->Type),
              toolchain::to_underlying(Clause->Flags)))
        ReportFlagError(Loc);
    }
  }

  PerVisibilityBindingChecker BindingChecker(this);
  SmallVector<std::pair<const toolchain::hlsl::rootsig::DescriptorTableClause *,
  ...
  for (const hlsl::RootSignatureElement &RootSigElem : Elements) {
    ...
    if (const auto *Descriptor =
      ...
      BindingChecker.trackBinding(
          ...
          static_cast<toolchain::dxil::ResourceClass>(Descriptor->Type),
      ...
      BindingChecker.trackBinding(
          Constants->Visibility, toolchain::dxil::ResourceClass::CBuffer,
      ...
      BindingChecker.trackBinding(
          Sampler->Visibility, toolchain::dxil::ResourceClass::Sampler,
      ...
      UnboundClauses.emplace_back(Clause, &RootSigElem);
                   ...
                   std::get_if<toolchain::hlsl::rootsig::DescriptorTable>(&Elem)) {
      ...
      for (const auto &[Clause, ClauseElem] : UnboundClauses) {
        ...
        BindingChecker.trackBinding(
            Table->Visibility,
            static_cast<toolchain::dxil::ResourceClass>(Clause->Type), Clause->Space,
...
static bool isValidWaveSizeValue(unsigned Value) {
  return toolchain::isPowerOf2_32(Value) && Value >= 4 && Value <= 128;
}
  ...
  if (SpelledArgsCount > 2) {
    if (!isValidWaveSizeValue(Preferred)) {
      Diag(AL.getArgAsExpr(2)->getExprLoc(),
          ...
          << AL << toolchain::dxil::MinWaveSize << toolchain::dxil::MaxWaveSize
          << Preferred;
      return;
    }
    ...
    if (!isValidWaveSizeValue(Max)) {
      Diag(AL.getArgAsExpr(1)->getExprLoc(),
           diag::err_attribute_power_of_two_in_range)
          << AL << toolchain::dxil::MinWaveSize << toolchain::dxil::MaxWaveSize << Max;
      return;
    }
    if (Max < Min) {
    ...
    if (!isValidWaveSizeValue(Min)) {
      Diag(AL.getArgAsExpr(0)->getExprLoc(),
           diag::err_attribute_power_of_two_in_range)
          << AL << toolchain::dxil::MinWaveSize << toolchain::dxil::MaxWaveSize << Min;
      return;
    }
  }
...
bool language::Core::CreateHLSLAttributedResourceType(
  ...
  for (const Attr *A : AttrList) {
    ...
    default:
      toolchain_unreachable("unhandled resource attribute type");
    }
  }
...
bool SemaHLSL::handleResourceTypeAttr(QualType T, const ParsedAttr &AL) {
  ...
  default:
    toolchain_unreachable("unhandled HLSL attribute");
  }
...
static bool DiagnoseLocalRegisterBinding(Sema &S, SourceLocation &ArgLoc,
  ...
  const language::Core::Type *Ty = VD->getType().getTypePtr();
...
class DiagnoseHLSLAvailability : public DynamicRecursiveASTVisitor {
  ...
  toolchain::SmallVector<const FunctionDecl *, 8> DeclsToScan;
  ...
  // The toolchain::Triple::EnvironmentType enum values for shader stages guaranteed
  // to be numbered from toolchain::Triple::Pixel to toolchain::Triple::Amplification
  ...
  // by subtracting the value of toolchain::Triple::Pixel first.
  //
  // The N'th bit in the set will be set if the function has been scanned
  // in shader environment whose toolchain::Triple::EnvironmentType integer value
  // equals (toolchain::Triple::Pixel + N).
  //
  // For example, if a function has been scanned in compute and pixel stage
  // environment, the value will be 0x21 (100001 binary) because:
  //
  //   (int)(toolchain::Triple::Pixel - toolchain::Triple::Pixel) == 0
  //   (int)(toolchain::Triple::Compute - toolchain::Triple::Pixel) == 5
  //
  // A FunctionDecl is mapped to 0 (or not included in the map) if it has not
  // been scanned in any environment.
  toolchain::DenseMap<const FunctionDecl *, unsigned> ScannedDecls;

  // Do not access these directly, use the get/set methods below to make
  // sure the values are in sync
  toolchain::Triple::EnvironmentType CurrentShaderEnvironment;
  ...
  void SetShaderStageContext(toolchain::Triple::EnvironmentType ShaderType) {
    static_assert(sizeof(unsigned) >= 4);
    assert(HLSLShaderAttr::isValidShaderType(ShaderType));
    assert((unsigned)(ShaderType - toolchain::Triple::Pixel) < 31 &&
           "ShaderType is too big for this bitmap"); // 31 is reserved for
                                                     // "unknown"

    unsigned bitmapIndex = ShaderType - toolchain::Triple::Pixel;
    CurrentShaderEnvironment = ShaderType;
    CurrentShaderStageBit = (1 << bitmapIndex);
  }

  void SetUnknownShaderStageContext() {
    CurrentShaderEnvironment = toolchain::Triple::UnknownEnvironment;
    CurrentShaderStageBit = (1 << 31);
  }

  toolchain::Triple::EnvironmentType GetCurrentShaderEnvironment() const {
    return CurrentShaderEnvironment;
  }

  bool InUnknownShaderStageContext() const {
    return CurrentShaderEnvironment == toolchain::Triple::UnknownEnvironment;
  }
...
public:
  ...
  bool VisitDeclRefExpr(DeclRefExpr *DRE) override {
    FunctionDecl *FD = toolchain::dyn_cast<FunctionDecl>(DRE->getDecl());
    if (FD)
  ...
  bool VisitMemberExpr(MemberExpr *ME) override {
    FunctionDecl *FD = toolchain::dyn_cast<FunctionDecl>(ME->getMemberDecl());
    if (FD)
...
void DiagnoseHLSLAvailability::RunOnTranslationUnit(
  ...
  toolchain::SmallVector<const DeclContext *, 8> DeclContextsToScan;
  ...
  while (!DeclContextsToScan.empty()) {
    ...
    for (auto &D : DC->decls()) {
      ...
      // scanned later
      if (toolchain::dyn_cast<NamespaceDecl>(D) || toolchain::dyn_cast<ExportDecl>(D)) {
        DeclContextsToScan.push_back(toolchain::dyn_cast<DeclContext>(D));
        continue;
      }

      // skip over other decls or function decls without body
      const FunctionDecl *FD = toolchain::dyn_cast<FunctionDecl>(D);
...
bool DiagnoseHLSLAvailability::HasMatchingEnvironmentOrNone(
  ...
  toolchain::Triple::EnvironmentType CurrentEnv = GetCurrentShaderEnvironment();
  if (CurrentEnv == toolchain::Triple::UnknownEnvironment)
    return false;

  toolchain::Triple::EnvironmentType AttrEnv =
...
void DiagnoseHLSLAvailability::CheckDeclAvailability(NamedDecl *D,
  ...
  toolchain::StringRef PlatformName(
      AvailabilityAttr::getPrettyPlatformName(TI.getPlatformName()));

  toolchain::StringRef CurrentEnvStr =
      toolchain::Triple::getEnvironmentTypeName(GetCurrentShaderEnvironment());

  toolchain::StringRef AttrEnvStr =
...
void SemaHLSL::diagnoseAvailabilityViolations(TranslationUnitDecl *TU) {
  ...
  const TargetInfo &TI = SemaRef.getASTContext().getTargetInfo();
  if (SemaRef.getLangOpts().HLSLStrictAvailability &&
      TI.getTriple().getEnvironment() != toolchain::Triple::EnvironmentType::Library)
    return;
...
static bool CheckAllArgTypesAreCorrect(
    ...
    toolchain::function_ref<bool(Sema *S, SourceLocation Loc, int ArgOrdinal,
                            language::Core::QualType PassedType)>
        Check) {
...
static bool CheckFloatOrHalfRepresentation(Sema *S, SourceLocation Loc,
                                           int ArgOrdinal,
                                           language::Core::QualType PassedType) {
  language::Core::QualType BaseType =
      PassedType->isVectorType()
          ? PassedType->castAs<language::Core::VectorType>()->getElementType()
          : PassedType;
...
static bool CheckResourceHandle(
    ...
    toolchain::function_ref<bool(const HLSLAttributedResourceType *ResType)> Check =
        nullptr) {
  assert(TheCall->getNumArgs() >= ArgIndex);
  QualType ArgType = TheCall->getArg(ArgIndex)->getType();
...
bool SemaHLSL::CheckBuiltinFunctionCall(unsigned BuiltinID, CallExpr *TheCall) {
  ...
  // Note these are toolchain builtins that we want to catch invalid intrinsic
  ...
  case Builtin::BI__builtin_hlsl_buffer_update_counter: {
    ...
    std::optional<toolchain::APSInt> Offset =
...
static void BuildFlattenedTypeList(QualType BaseTy,
                                   toolchain::SmallVectorImpl<QualType> &List) {
  toolchain::SmallVector<QualType, 16> WorkList;
  ...
  while (!WorkList.empty()) {
    ...
    if (const auto *AT = dyn_cast<ConstantArrayType>(T)) {
      toolchain::SmallVector<QualType, 16> ElementFields;
      ...
      for (uint64_t Ct = 0; Ct < AT->getZExtSize(); ++Ct)
        toolchain::append_range(List, ElementFields);
      continue;
    }
    ...
    if (const auto *RT = dyn_cast<RecordType>(T)) {
      ...
      toolchain::append_range(WorkList, FieldTypes);
...
bool SemaHLSL::IsTypedResourceElementCompatible(language::Core::QualType QT) {
...
bool SemaHLSL::IsScalarizedLayoutCompatible(QualType T1, QualType T2) const {
  ...
  toolchain::SmallVector<QualType, 16> T1Types;
  BuildFlattenedTypeList(T1, T1Types);
  toolchain::SmallVector<QualType, 16> T2Types;
  BuildFlattenedTypeList(T2, T2Types);

  // Check the flattened type list
  return toolchain::equal(T1Types, T2Types,
...
bool SemaHLSL::CanPerformScalarCast(QualType SrcTy, QualType DestTy) {
  ...
  case Type::STK_Integral:
    ...
    case Type::STK_MemberPointer:
      toolchain_unreachable("HLSL doesn't support pointers.");
    case Type::STK_IntegralComplex:
    case Type::STK_FloatingComplex:
      toolchain_unreachable("HLSL doesn't support complex types.");
    case Type::STK_FixedPoint:
      toolchain_unreachable("HLSL doesn't support fixed point types.");
    }
    toolchain_unreachable("Should have returned before this");
  ...
  toolchain_unreachable("Unhandled scalar cast");
}
...
bool SemaHLSL::CanPerformAggregateSplatCast(Expr *Src, QualType DestTy) {
  ...
  toolchain::SmallVector<QualType> DestTypes;
...
bool SemaHLSL::CanPerformElementwiseCast(Expr *Src, QualType DestTy) {
  ...
  toolchain::SmallVector<QualType> DestTypes;
  BuildFlattenedTypeList(DestTy, DestTypes);
  toolchain::SmallVector<QualType> SrcTypes;
...
bool SemaHLSL::initGlobalResourceDecl(VarDecl *VD) {
  ...
  IntegerLiteral *RangeSize = IntegerLiteral::Create(
      AST, toolchain::APInt(IntTySize, 1), AST.IntTy, SourceLocation());
  IntegerLiteral *Index = IntegerLiteral::Create(
      AST, toolchain::APInt(UIntTySize, 0), AST.UnsignedIntTy, SourceLocation());
  IntegerLiteral *Space =
      IntegerLiteral::Create(AST, toolchain::APInt(UIntTySize, SpaceNo),
  ...
  if (RegisterSlot.has_value()) {
    IntegerLiteral *RegSlot = IntegerLiteral::Create(
        AST, toolchain::APInt(UIntTySize, RegisterSlot.value()), AST.UnsignedIntTy,
        SourceLocation());
  ...
  IntegerLiteral *OrderId = IntegerLiteral::Create(
      AST, toolchain::APInt(UIntTySize, getNextImplicitBindingOrderID()),
      AST.UnsignedIntTy, SourceLocation());
...
bool SemaHLSL::ActOnUninitializedVarDecl(VarDecl *VD) {
  ...
  // (toolchain/toolchain-project#133835, toolchain/toolchain-project#133837)
...
class InitListTransformer {
  ...
  bool buildInitializerListImpl(Expr *E) {
    ...
    if (auto *VecTy = Ty->getAs<VectorType>()) {
      ...
      for (uint64_t I = 0; I < Size; ++I) {
        auto *Idx = IntegerLiteral::Create(Ctx, toolchain::APInt(SizeTySize, I),
    ...
    if (auto *RTy = Ty->getAs<RecordType>()) {
      toolchain::SmallVector<const RecordType *> RecordTypes;
  ...
  Expr *generateInitListsImpl(QualType Ty) {
    ...
    toolchain::SmallVector<Expr *> Inits;
...
public:
  toolchain::SmallVector<QualType, 16> DestTypes;
  toolchain::SmallVector<Expr *, 16> ArgExprs;
  ...
  Expr *generateInitLists() {
    ...
    auto *NewInit = new (Ctx) InitListExpr(Ctx, Inits.front()->getBeginLoc(),
                                           Inits, Inits.back()->getEndLoc());
    toolchain::APInt ArySize(64, Inits.size());
...
bool SemaHLSL::handleInitialization(VarDecl *VDecl, Expr *&Init) {
  ...
  toolchain::APInt IDVal(Context.getIntWidth(Context.IntTy), ConstantID);
