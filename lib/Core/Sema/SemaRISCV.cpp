...
using namespace language::Core::RISCV;
...
#include "language/Core/Basic/riscv_vector_builtin_sema.inc"
#undef DECL_SIGNATURE_TABLE
};

static const PrototypeDescriptor RVSiFiveVectorSignatureTable[] = {
#define DECL_SIGNATURE_TABLE
#include "language/Core/Basic/riscv_sifive_vector_builtin_sema.inc"
#undef DECL_SIGNATURE_TABLE
};

static const PrototypeDescriptor RVAndesVectorSignatureTable[] = {
#define DECL_SIGNATURE_TABLE
#include "language/Core/Basic/riscv_andes_vector_builtin_sema.inc"
#undef DECL_SIGNATURE_TABLE
};
...
ProtoSeq2ArrayRef(IntrinsicKind K, uint16_t Index, uint8_t Length) {
  ...
  toolchain_unreachable("Unhandled IntrinsicKind");
}
...
static QualType RVVType2Qual(ASTContext &Context, const RVVType *Type) {
  ...
  case ScalarTypeKind::Float:
    ...
    case 16:
      ...
      break;
    default:
      toolchain_unreachable("Unsupported floating point width.");
    }
    break;
  case Invalid:
  case Undefined:
...
public:
  RISCVIntrinsicManagerImpl(language::Core::Sema &S) : S(S) {
...
void RISCVIntrinsicManagerImpl::ConstructRVVIntrinsics(
  ...
  for (auto &Record : Recs) {
    ...
    toolchain::SmallVector<PrototypeDescriptor> ProtoSeq =
        RVVIntrinsic::computeBuiltinTypes(
            BasicProtoSeq, /*IsMasked=*/false,
            /*HasMaskedOffOperand=*/false, Record.HasVL, Record.NF,
            UnMaskedPolicyScheme, DefaultPolicy, Record.IsTuple);

    toolchain::SmallVector<PrototypeDescriptor> ProtoMaskSeq;
    if (Record.HasMasked)
      ...
      for (int Log2LMUL = -3; Log2LMUL <= 3; Log2LMUL++) {
        ...
        if (Record.UnMaskedPolicyScheme != PolicyScheme::SchemeNone) {
          for (auto P : SupportedUnMaskedPolicies) {
            toolchain::SmallVector<PrototypeDescriptor> PolicyPrototype =
                RVVIntrinsic::computeBuiltinTypes(
                    BasicProtoSeq, /*IsMasked=*/false,
                    /*HasMaskedOffOperand=*/false, Record.HasVL, Record.NF,
...
std::unique_ptr<language::Core::sema::RISCVIntrinsicManager>
...
static bool CheckInvalidVLENandLMUL(const TargetInfo &TI,
                                    toolchain::StringMap<bool> &FunctionFeatureMap,
  ...
  unsigned MinRequiredVLEN = VScaleFactor * toolchain::RISCV::RVVBitsPerBlock;
...
bool SemaRISCV::CheckBuiltinFunctionCall(const TargetInfo &TI,
  ...
  if (const auto *A = TheCall->getCalleeDecl()->getAttr<TargetAttr>()) {
    ...
    toolchain::SmallVector<StringRef> RequiredFeatures;
  ...
  auto CheckVSetVL = [&](unsigned SEWOffset, unsigned LMULOffset) -> bool {
    ...
    toolchain::StringMap<bool> FunctionFeatureMap;
    Context.getFunctionFeatureMap(FunctionFeatureMap, FD);
    toolchain::APSInt SEWResult;
    toolchain::APSInt LMULResult;
...
bool SemaRISCV::checkTargetVersionAttr(const StringRef Param,
  ...
  toolchain::SmallVector<StringRef, 8> AttrStrs;
  ...
  for (StringRef AttrStr : AttrStrs) {
    ...
    if (AttrStr.starts_with("arch=+")) {
      ...
      if (TargetAttr.Features.empty() ||
          toolchain::any_of(TargetAttr.Features, [&](const StringRef Ext) {
...
bool SemaRISCV::checkTargetClonesAttr(
  ...
  for (unsigned I = 0, E = Params.size(); I < E; ++I) {
    ...
    if (toolchain::is_contained(NewParams, Param))
