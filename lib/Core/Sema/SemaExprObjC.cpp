...
#include "language/Core/Edit/Rewriters.h"
...
ExprResult SemaObjC::ParseObjCStringLiteral(SourceLocation *AtLocs,
  ...
  if (Strings.size() != 1) {
    ...
    QualType StrTy = Context.getConstantArrayType(
        CAT->getElementType(), toolchain::APInt(32, StrBuf.size() + 1), nullptr,
...
ClassKindFromLiteralKind(SemaObjC::ObjCLiteralKind LiteralKind) {
  ...
  toolchain_unreachable("LiteralKind can't be converted into a ClassKind");
}
      ...
      if (auto *CE = dyn_cast<ImplicitCastExpr>(ValueExpr))
        if (CE->getCastKind() == CK_ArrayToPointerDecay)
          if (auto *SL =
            ...
            const toolchain::UTF8 *StrBegin = Str.bytes_begin();
            const toolchain::UTF8 *StrEnd = Str.bytes_end();
            // Check that this is a valid UTF-8 string.
            if (toolchain::isLegalUTF8String(&StrBegin, StrEnd)) {
...
CheckObjCDictionaryLiteralDuplicateKeys(Sema &S,
  ...
  struct APSIntCompare {
    bool operator()(const toolchain::APSInt &LHS, const toolchain::APSInt &RHS) const {
      return toolchain::APSInt::compareValues(LHS, RHS) < 0;
    }
  };

  toolchain::DenseMap<StringRef, SourceLocation> StringKeys;
  std::map<toolchain::APSInt, SourceLocation, APSIntCompare> IntegralKeys;
        ...
        if (QIDNSCopying.isNull()) {
          ...
          if (ObjCProtocolDecl *NSCopyingPDecl =
            ...
            QIDNSCopying = Context.getObjCObjectType(
                ...
                toolchain::ArrayRef((ObjCProtocolDecl **)PQ, 1), false);
...
void SemaObjC::diagnoseARCUnbridgedCast(Expr *e) {
  ...
  if (CStyleCastExpr *cast = dyn_cast<CStyleCastExpr>(realCast)) {
    ...
    toolchain_unreachable("Unexpected ImplicitCastExpr");
  }
...
ExprResult SemaObjC::ActOnObjCAvailabilityCheckExpr(
    toolchain::ArrayRef<AvailabilitySpec> AvailSpecs, SourceLocation AtLoc,
    SourceLocation RParen) {
  ASTContext &Context = getASTContext();
  auto FindSpecVersion =
      [&](StringRef Platform,
          const toolchain::Triple::OSType &OS) -> std::optional<VersionTuple> {
    auto Spec = toolchain::find_if(AvailSpecs, [&](const AvailabilitySpec &Spec) {
    ...
    if (Spec == AvailSpecs.end() && Platform == "maccatalyst") {
      Spec = toolchain::find_if(AvailSpecs, [&](const AvailabilitySpec &Spec) {
    ...
    return toolchain::Triple::getCanonicalVersionForOS(
        OS, Spec->getVersion(),
        toolchain::Triple::isValidVersionForOS(OS, Spec->getVersion()));
  };
