...
struct TemplateParameterListBuilder {
  ...
  toolchain::SmallVector<NamedDecl *> Params;
...
private:
  ...
  bool IsConst;
  bool IsCtor;
  toolchain::SmallVector<Param> Params;
  toolchain::SmallVector<Stmt *> StmtsList;
...
BuiltinTypeDeclBuilder &BuiltinTypeDeclBuilder::addArraySubscriptOperators() {
  ...
  if (getResourceAttrs().ResourceClass == toolchain::dxil::ResourceClass::UAV)
...
Expr *BuiltinTypeDeclBuilder::getConstantIntExpr(int value) {
  ...
  return IntegerLiteral::Create(
      AST, toolchain::APInt(AST.getTypeSize(AST.IntTy), value, true), AST.IntTy,
      SourceLocation());
}
