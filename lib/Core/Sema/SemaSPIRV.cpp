...
static bool CheckAllArgTypesAreCorrect(
    ...
    toolchain::ArrayRef<
        toolchain::function_ref<bool(Sema *, SourceLocation, int, QualType)>>
        Checks) {
...
bool SemaSPIRV::CheckSPIRVBuiltinFunctionCall(const TargetInfo &TI,
  ...
  if (BuiltinID >= SPIRV::FirstVKBuiltin && BuiltinID <= SPIRV::LastVKBuiltin &&
      TI.getTriple().getArch() != toolchain::Triple::spirv) {
  ...
  if (BuiltinID >= SPIRV::FirstCLBuiltin && BuiltinID <= SPIRV::LastTSBuiltin &&
      TI.getTriple().getArch() != toolchain::Triple::spirv32 &&
      TI.getTriple().getArch() != toolchain::Triple::spirv64) {
  ...
  case SPIRV::BI__builtin_spirv_refract: {
    ...
    language::Core::QualType BaseType =
        Arg0Type->isVectorType()
            ? Arg0Type->castAs<language::Core::VectorType>()->getElementType()
            : Arg0Type;
    if (!SemaRef.getASTContext().hasSameUnqualifiedType(
