...
#include "language/Core/Lex/LiteralSupport.h"
...
#include "language/Core/Sema/ParsedTemplate.h"
...
#include "language/Core/Sema/SemaOpenMP.h"
#include "language/Core/Sema/SemaPseudoObject.h"
...
ExprResult Sema::UsualUnaryFPConversions(Expr *E) {
  ...
  if (EvalMethod != LangOptions::FEM_Source && Ty->isFloatingType() &&
    ...
    switch (EvalMethod) {
    default:
      toolchain_unreachable("Unrecognized float evaluation method");
      break;
    case LangOptions::FEM_UnsetOnCommandLine:
      toolchain_unreachable("Float evaluation method should be set by now");
      break;
...
static bool unsupportedTypeConversion(const Sema &S, QualType LHSType,
  ...
  const toolchain::fltSemantics &LHSSem = S.Context.getFloatTypeSemantics(LHSElem);
  const toolchain::fltSemantics &RHSSem = S.Context.getFloatTypeSemantics(RHSElem);

  if ((&LHSSem != &toolchain::APFloat::PPCDoubleDouble() ||
       &RHSSem != &toolchain::APFloat::IEEEquad()) &&
      (&LHSSem != &toolchain::APFloat::IEEEquad() ||
       &RHSSem != &toolchain::APFloat::PPCDoubleDouble()))
    return false;
...
static unsigned GetFixedPointRank(QualType Ty) {
  ...
  default:
    if (BTy->isInteger())
      return 0;
    toolchain_unreachable("Unexpected fixed point or integer type");
  }
}
...
static void CheckUnicodeArithmeticConversions(Sema &SemaRef, Expr *LHS,
  ...
  if (ACK == ArithConvKind::Comparison) {
    ...
    auto IsSingleCodeUnitCP = [](const QualType &T, const toolchain::APSInt &Value) {
      if (T->isChar8Type())
    ...
    toolchain::APSInt LHSValue(32);
    LHSValue = LHSRes.Val.getInt();
    toolchain::APSInt RHSValue(32);
...
ExprResult Sema::ActOnGenericSelectionExpr(
  ...
  ExprResult ER = CreateGenericSelectionExpr(
      ...
      toolchain::ArrayRef(Types, NumAssocs), ArgExprs);
  delete [] Types;
  return ER;
}
...
static PredefinedIdentKind getPredefinedExprKind(tok::TokenKind Kind) {
  switch (Kind) {
  default:
    toolchain_unreachable("unexpected TokenKind");
...
static ExprResult BuildCookedLiteralOperatorCall(Sema &S, Scope *Scope,
  ...
  if (S.LookupLiteralOperator(Scope, R, toolchain::ArrayRef(ArgTy, Args.size()),
...
Sema::ActOnStringLiteral(ArrayRef<Token> StringToks, Scope *UDLScope) {
  ...
  case LOLR_Cooked: {
    toolchain::APInt Len(Context.getIntWidth(SizeType), Literal.GetNumStringChars());
  ...
  case LOLR_StringTemplatePack: {
    ...
    toolchain::APSInt Value(CharBits, CharIsUnsigned);
  ...
  case LOLR_ErrorNoDiagnostic:
    toolchain_unreachable("unexpected literal operator lookup result");
  case LOLR_Error:
    return ExprError();
  }
  toolchain_unreachable("unexpected literal operator lookup result");
...
#include "language/Core/AST/DeclNodes.inc"
    toolchain_unreachable("invalid value decl kind");

  // These shouldn't make it here.
  case Decl::ObjCAtDefsField:
    toolchain_unreachable("forming non-member reference to ivar?");
  ...
  case Decl::CXXDeductionGuide:
    toolchain_unreachable("building reference to deduction guide");
...
static void ConvertUTF8ToWideString(unsigned CharByteWidth, StringRef Source,
  ...
  const toolchain::UTF8 *ErrorPtr;
  bool success =
      toolchain::ConvertUTF8toWide(CharByteWidth, Source, ResultPtr, ErrorPtr);
  (void)success;
  assert(success);
...
ExprResult Sema::BuildPredefinedExpr(SourceLocation Loc,
  ...
  if (cast<DeclContext>(currentDecl)->isDependentContext())
    ...
    toolchain::APInt LengthI(32, Length + 1);
...
ExprResult Sema::ActOnIntegerConstant(SourceLocation Loc, int64_t Val) {
  ...
  return IntegerLiteral::Create(Context,
                                toolchain::APInt(IntSize, Val, /*isSigned=*/true),
                                Context.IntTy, Loc);
}

static Expr *BuildFloatingLiteral(Sema &S, NumericLiteralParser &Literal,
                                  QualType Ty, SourceLocation Loc) {
  const toolchain::fltSemantics &Format = S.Context.getFloatTypeSemantics(Ty);

  using toolchain::APFloat;
  APFloat Val(Format);

  toolchain::RoundingMode RM = S.CurFPFeatures.getRoundingMode();
...
ExprResult Sema::ActOnNumericConstant(const Token &Tok, Scope *UDLScope) {
  ...
  if (Literal.hasUDSuffix()) {
    ...
    case LOLR_Cooked: {
      ...
      if (Literal.isFloatingLiteral()) {
        ...
        toolchain::APInt ResultVal(Context.getTargetInfo().getLongLongWidth(), 0);
    ...
    case LOLR_Raw: {
      ...
      QualType StrTy = Context.getConstantArrayType(
          ...
          toolchain::APInt(32, Length + 1), nullptr, ArraySizeModifier::Normal, 0);
      Expr *Lit =
  ...
  if (Literal.isFixedPointLiteral()) {
    ...
    toolchain::APInt Val(bit_width, 0, isSigned);
    ...
    // toolchain::IntegerType::MAX_INT_BITS is a big performance hit.
    unsigned BitsNeeded = Context.getTargetInfo().getIntMaxTWidth();
    if (Literal.isBitInt)
      BitsNeeded = toolchain::APInt::getSufficientBitsNeeded(
    ...
    toolchain::APInt ResultVal(BitsNeeded, 0);
...
Sema::ActOnPostfixUnaryOp(Scope *S, SourceLocation OpLoc,
  ...
  switch (Kind) {
  default: toolchain_unreachable("Unknown unary op!");
...
// FIXME: is this important to avoid? See https://reviews.toolchain.org/D107275
...
ExprResult Sema::CreateBuiltinMatrixSubscriptExpr(Expr *Base, Expr *RowIdx,
  ...
  auto IsIndexValid = [&](Expr *IndexExpr, unsigned Dim,
    ...
    if (std::optional<toolchain::APSInt> Idx =
...
Sema::CreateBuiltinArraySubscriptExpr(Expr *Base, SourceLocation LLoc,
  ...
  if ((IndexExpr->getType()->isSpecificBuiltinType(BuiltinType::Char_S) ||
      ...
      !IndexExpr->isTypeDependent()) {
    std::optional<toolchain::APSInt> IntegerContantExpr =
...
static FieldDecl *FindFieldDeclInstantiationPattern(const ASTContext &Ctx,
  ...
  auto Rng = toolchain::make_filter_range(
...
static bool isParenthetizedAndQualifiedAddressOfExpr(Expr *Fn) {
  ...
  if (!UO || UO->getOpcode() != language::Core::UO_AddrOf)
    return false;
...
#include "language/Core/AST/BuiltinTypes.def"
  ...
  toolchain_unreachable("bad builtin type kind");
}
...
ExprResult Sema::BuildCallExpr(Scope *Scope, Expr *Fn, SourceLocation LParenLoc,
  ...
  if (Context.isDependenceAllowed() &&
    ...
    assert((Fn->containsErrors() ||
            toolchain::any_of(ArgExprs,
                         [](language::Core::Expr *E) { return E->containsErrors(); })) &&
...
CastKind Sema::PrepareScalarCast(ExprResult &Src, QualType DestTy) {
  ...
  case Type::STK_MemberPointer:
    toolchain_unreachable("member pointer type in C");
  ...
  case Type::STK_ObjCObjectPointer:
    ...
    case Type::STK_FixedPoint:
      toolchain_unreachable("illegal cast from pointer");
    }
  ...
  case Type::STK_FixedPoint:
    ...
    case Type::STK_BlockPointer:
    case Type::STK_MemberPointer:
      toolchain_unreachable("illegal cast to pointer type");
    }
    toolchain_unreachable("Should have returned before this");
  ...
  case Type::STK_Floating:
    ...
    case Type::STK_ObjCObjectPointer:
    case Type::STK_BlockPointer:
      toolchain_unreachable("valid float->pointer cast?");
    case Type::STK_MemberPointer:
      toolchain_unreachable("member pointer type in C");
    case Type::STK_FixedPoint:
  ...
  case Type::STK_FloatingComplex:
    ...
    case Type::STK_ObjCObjectPointer:
    case Type::STK_BlockPointer:
      toolchain_unreachable("valid complex float->pointer cast?");
    case Type::STK_MemberPointer:
      toolchain_unreachable("member pointer type in C");
    case Type::STK_FixedPoint:
      Diag(Src.get()->getExprLoc(),
  ...
  case Type::STK_IntegralComplex:
    ...
    case Type::STK_ObjCObjectPointer:
    case Type::STK_BlockPointer:
      toolchain_unreachable("valid complex int->pointer cast?");
    case Type::STK_MemberPointer:
      toolchain_unreachable("member pointer type in C");
    case Type::STK_FixedPoint:
      Diag(Src.get()->getExprLoc(),
...
static bool canConvertIntToOtherIntTy(Sema &S, ExprResult *Int,
  ...
  if (CstInt) {
    ...
    toolchain::APSInt Result = EVResult.Val.getInt();
...
static bool canConvertIntTyToFloatTy(Sema &S, ExprResult *Int,
  ...
  if (CstInt) {
    ...
    toolchain::APSInt Result = EVResult.Val.getInt();
    toolchain::APFloat Float(S.Context.getFloatTypeSemantics(FloatTy));
    Float.convertFromAPInt(Result, IntTy->hasSignedIntegerRepresentation(),
                           toolchain::APFloat::rmTowardZero);
    toolchain::APSInt ConvertBack(S.Context.getIntWidth(IntTy),
                             !IntTy->hasSignedIntegerRepresentation());
    bool Ignored = false;
    Float.convertToInteger(ConvertBack, toolchain::APFloat::rmNearestTiesToEven,
                           &Ignored);
    ...
    unsigned FloatPrec = toolchain::APFloat::semanticsPrecision(
...
static bool tryGCCVectorConvertAndSplat(Sema &S, ExprResult *Scalar,
  ...
  if (const auto *VT = VectorTy->getAs<VectorType>()) {
    ...
    toolchain_unreachable("Only Fixed-Length and SVE Vector types are handled here");
  }
    ...
    if (ScalarTy->isRealFloatingType()) {
      ...
      if (CstScalar) {
        ...
        Result.convert(S.Context.getFloatTypeSemantics(VectorEltTy),
                       toolchain::APFloat::rmNearestTiesToEven, &Truncated);
        if (Truncated)
          return true;
      }
...
static void DetectPrecisionLossInComplexDivision(Sema &S, QualType DivisorTy,
  ...
  const toolchain::fltSemantics &ElementTypeSemantics =
      Ctx.getFloatTypeSemantics(ElementType);
  const toolchain::fltSemantics &HigherElementTypeSemantics =
      Ctx.getFloatTypeSemantics(HigherElementType);

  if ((toolchain::APFloat::semanticsMaxExponent(ElementTypeSemantics) * 2 + 1 >
       toolchain::APFloat::semanticsMaxExponent(HigherElementTypeSemantics)) ||
...
static void diagnoseStringPlusChar(Sema &Self, SourceLocation OpLoc,
  ...
  if (!CharType->isAnyCharacterType() &&
      ...
      toolchain::isUIntN(Ctx.getCharWidth(), CharExpr->getValue())) {
...
static void DiagnoseBadShiftValues(Sema& S, ExprResult &LHS, ExprResult &RHS,
  ...
  if (RHS.get()->isValueDependent() ||
    ...
    return;
  toolchain::APSInt Right = RHSResult.Val.getInt();
  ...
  if (LHS.get()->isValueDependent() ||
    ...
    return;
  toolchain::APSInt Left = LHSResult.Val.getInt();
  ...
  toolchain::APInt ResultBits =
      static_cast<toolchain::APInt &>(Right) + Left.getSignificantBits();
  if (ResultBits.ule(LeftSize))
    return;
  toolchain::APSInt Result = Left.extend(ResultBits.getLimitedValue());
...
static QualType checkVectorShift(Sema &S, ExprResult &LHS, ExprResult &RHS,
  ...
  if (!LHSVecTy) {
    ...
    if (!S.LangOpts.OpenCL && !S.LangOpts.ZVector) {
      const BuiltinType *LHSBT = LHSEleType->getAs<language::Core::BuiltinType>();
      const BuiltinType *RHSBT = RHSEleType->getAs<language::Core::BuiltinType>();
      if (LHSBT != RHSBT &&
...
static QualType checkSizelessVectorShift(Sema &S, ExprResult &LHS,
  ...
  if (!LHSType->isSveVLSBuiltinType()) {
    ...
    if (IsCompAssign)
      return RHSType;
    if (LHSEleType != RHSEleType) {
      LHS = S.ImpCastExprToType(LHS.get(), RHSEleType, language::Core::CK_IntegralCast);
      LHSEleType = RHSEleType;
    }
    const toolchain::ElementCount VecSize =
        S.Context.getBuiltinVectorTypeInfo(RHSBuiltinTy).EC;
    QualType VecTy =
        S.Context.getScalableVectorType(LHSEleType, VecSize.getKnownMinValue());
    LHS = S.ImpCastExprToType(LHS.get(), VecTy, language::Core::CK_VectorSplat);
    LHSType = VecTy;
    ...
    const toolchain::ElementCount VecSize =
        S.Context.getBuiltinVectorTypeInfo(LHSBuiltinTy).EC;
    if (LHSEleType != RHSEleType) {
      RHS = S.ImpCastExprToType(RHS.get(), LHSEleType, language::Core::CK_IntegralCast);
...
static void diagnoseObjCLiteralComparison(Sema &S, SourceLocation Loc,
  ...
  if (LiteralKind == SemaObjC::LK_None) {
    toolchain_unreachable("Unknown Objective-C object literal kind");
  }
...
static ImplicitConversionKind castKindToImplicitConversionKind(CastKind CK) {
  ...
  default: {
#ifndef NDEBUG
    toolchain::errs() << "unhandled cast kind: " << CastExpr::getCastKindName(CK)
                 << "\n";
#endif
    toolchain_unreachable("unhandled cast kind");
  }
    ...
    if (IsRelational &&
      ...
      if (auto *CTSD = dyn_cast<ClassTemplateSpecializationDecl>(DC)) {
        if (CTSD->isInStdNamespace() &&
            toolchain::StringSwitch<bool>(CTSD->getName())
...
QualType Sema::GetSignedSizelessVectorType(QualType V) {
  ...
  const toolchain::ElementCount VecSize = Context.getBuiltinVectorTypeInfo(VTy).EC;
...
static void diagnoseXorMisusedAsPow(Sema &S, const ExprResult &XorLHS,
  ...
  const toolchain::APInt &LeftSideValue = LHSInt->getValue();
  toolchain::APInt RightSideValue = RHSInt->getValue();
  ...
  const toolchain::APInt XorValue = LeftSideValue ^ RightSideValue;
  ...
  if (LeftSideValue == 2 && RightSideIntValue >= 0) {
    ...
    bool Overflow = false;
    toolchain::APInt One = (LeftSideValue - 1);
    toolchain::APInt PowValue = One.sshl_ov(RightSideValue, Overflow);
    if (Overflow) {
  ...
  while (RecordTypeList.size() > NextToCheckIndex) {
    ...
    for (const FieldDecl *Field : RecordTypeList[NextToCheckIndex]
      ...
      if (const auto *FieldRecTy = FieldTy->getAs<RecordType>()) {
        if (!toolchain::is_contained(RecordTypeList, FieldRecTy))
...
static bool CheckForModifiableLvalue(Expr *E, SourceLocation Loc, Sema &S) {
  ...
  case Expr::MLV_LValueCast:
    ...
    break;
  case Expr::MLV_Valid:
    toolchain_unreachable("did not take early return for MLV_Valid");
  ...
  case Expr::MLV_NoSetterProperty:
    toolchain_unreachable("readonly properties should be processed differently");
...
QualType Sema::CheckAddressOfOperand(ExprResult &OrigOp, SourceLocation OpLoc) {
  ...
  if (lval == Expr::LV_ClassTemporary || lval == Expr::LV_ArrayTemporary) {
    ...
    if (const VarDecl *vd = dyn_cast<VarDecl>(dcl)) {
      ...
      toolchain_unreachable("Unknown/unexpected decl type");
  }
...
BinaryOperatorKind Sema::ConvertTokenKindToBinaryOpcode(tok::TokenKind Kind) {
  BinaryOperatorKind Opc;
  switch (Kind) {
  default: toolchain_unreachable("Unknown binop!");
...
Sema::getSelfAssignmentClassMemberCandidate(const ValueDecl *SelfAssigned) {
  ...
  auto Field =
      toolchain::find_if(Parent->fields(),
...
static bool needsConversionOfHalfVec(bool OpRequiresConversion, ASTContext &Ctx,
  ...
  auto HasVectorOfHalfType = [&Ctx](Expr *E) {
    ...
    // discussion here: https://reviews.toolchain.org/rG825235c140e7
  ...
  if (const BuiltinType *pty = LHSExpr->getType()->getAsPlaceholderType()) {
    ...
    if (Opc == BO_LT && inTemplateInstantiation() &&
      ...
      if (OE && !OE->hasTemplateKeyword() && !OE->hasExplicitTemplateArgs() &&
          toolchain::any_of(OE->decls(), [](NamedDecl *ND) {
...
ExprResult Sema::ActOnGNUNullExpr(SourceLocation TokenLoc) {
  ...
  if (pw == Context.getTargetInfo().getIntWidth())
    ...
    toolchain_unreachable("I don't know size of pointer!");
  }
...
bool Sema::DiagnoseAssignmentResult(AssignConvertType ConvTy,
  ...
  case AssignConvertType::IncompatiblePointerDiscardsQualifiers: {
    ...
    toolchain_unreachable("unknown error case for discarding qualifiers!");
    // fallthrough
  }
  ...
  if (DiagKind == diag::ext_typecheck_convert_incompatible_pointer_sign ||
      DiagKind == diag::err_typecheck_convert_incompatible_pointer_sign) {
    auto isPlainChar = [](const language::Core::Type *Type) {
...
ExprResult
Sema::VerifyIntegerConstantExpression(Expr *E, toolchain::APSInt *Result,
  ...
  if (getLangOpts().CPlusPlus11) {
    ...
    public:
      ...
      SemaDiagnosticBuilder diagnoseConversion(
          Sema &S, SourceLocation Loc, QualType T, QualType ConvTy) override {
        toolchain_unreachable("conversion functions are permitted");
      }
...
void Sema::CheckUnusedVolatileAssignment(Expr *E) {
  ...
  if (auto *BO = dyn_cast<BinaryOperator>(E->IgnoreParenImpCasts())) {
    if (BO->getOpcode() == BO_Assign) {
      ...
      toolchain::erase(LHSs, BO->getLHS());
    }
  }
}
...
ExprResult Sema::CheckForImmediateInvocation(ExprResult E, FunctionDecl *Decl) {
  ...
  auto CheckConstantExpressionAndKeepResult = [&]() {
    toolchain::SmallVector<PartialDiagnosticAt, 8> Notes;
...
HandleImmediateInvocations(Sema &SemaRef,
  ...
  if (Rec.ImmediateInvocationCandidates.size() > 1 ||
    ...
    toolchain::SaveAndRestore DisableIITracking(
    ...
    struct SimpleRemove : DynamicRecursiveASTVisitor {
      toolchain::SmallPtrSetImpl<DeclRefExpr *> &DRSet;
      SimpleRemove(toolchain::SmallPtrSetImpl<DeclRefExpr *> &S) : DRSet(S) {}
  ...
  if (!Rec.Lambdas.empty()) {
    ...
    if (!getLangOpts().CPlusPlus20 &&
      ...
      if (Rec.isUnevaluated()) {
        ...
        toolchain_unreachable("Couldn't infer lambda error message.");
...
static bool isPotentiallyConstantEvaluatedContext(Sema &SemaRef) {
  ...
  toolchain_unreachable("Invalid context");
}
  ...
  public:
    ...
    void diagnose(Sema &S, SourceLocation Loc, QualType T) override {
      ...
      case CC_X86VectorCall:
        ...
        break;
      default:
        toolchain_unreachable("CC does not need mangling");
      }
        ...
        if (FirstInstantiation || TSK != TSK_ImplicitInstantiation ||
          ...
          if (isa<CXXRecordDecl>(Func->getDeclContext()) &&
            ...
            if (toolchain::isTimeTraceVerbose()) {
              toolchain::timeTraceAddInstantEvent("DeferInstantiation", [&] {
                std::string Name;
...
static void buildLambdaCaptureFixit(Sema &Sema, LambdaScopeInfo *LSI,
  ...
  // [&A, &B]: Don't offer.
  // [A, B]: Don't offer.
  if (toolchain::any_of(LSI->Captures, [](Capture &C) {
  ...
  if (ShouldOfferCopyFix) {
    ...
    // capture by copy.
    if (CanDefaultCopyCapture && toolchain::none_of(LSI->Captures, [](Capture &C) {
  ...
  if (toolchain::none_of(LSI->Captures, [](Capture &C) {
...
#if __has_cpp_attribute(language::Core::lifetimebound)
...
static ExprResult rebuildPotentialResultsAsNonOdrUsed(Sema &S, Expr *E,
  ...
  auto IsPotentialResultOdrUsed = [&](NamedDecl *D) {
    ...
    case NOUR_Unevaluated:
      toolchain_unreachable("unexpected non-odr-use-reason");
...
void Sema::CleanupVarDeclMarking() {
  ...
  for (Expr *E : LocalMaybeODRUseExprs) {
    if (auto *DRE = dyn_cast<DeclRefExpr>(E)) {
      ...
      toolchain_unreachable("Unexpected expression");
    }
  }
...
static void FixDependencyOfIdExpressionsInLambdaWithDependentObjectParameter(
  ...
  auto IsDependent = [&]() {
    for (auto *Scope : toolchain::reverse(SemaRef.FunctionScopes)) {
...
public:
  ...
  void Visit(Expr *E) {
    if (toolchain::is_contained(StopAt, E))
      return;
    Inherited::Visit(E);
  }
...
bool Sema::DiagRuntimeBehavior(SourceLocation Loc, const Stmt *Statement,
  ...
  return DiagRuntimeBehavior(
      Loc, Statement ? toolchain::ArrayRef(Statement) : toolchain::ArrayRef<Stmt *>(),
      PD);
}
...
namespace {
  ...
  struct RebuildUnknownAnyFunction
    ...
    ExprResult VisitStmt(Stmt *S) {
      toolchain_unreachable("unexpected statement!");
    }
...
ExprResult RebuildUnknownAnyExpr::VisitImplicitCastExpr(ImplicitCastExpr *E) {
  ...
  if (E->getCastKind() == CK_FunctionToPointerDecay) {
    ...
    toolchain_unreachable("Unhandled cast type!");
  }
}
...
#include "language/Core/AST/BuiltinTypes.def"
  ...
  toolchain_unreachable("invalid placeholder type!");
}
