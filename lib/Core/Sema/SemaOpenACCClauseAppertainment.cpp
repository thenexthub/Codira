...
public:
  ...
  unsigned popcount() const { return toolchain::popcount<uint64_t>(Data); }
};
...
// This introduces these in a toolchain::acc namespace, so make sure this stays in
...
const LLVMClauseLists &getListsForDirective(OpenACCDirectiveKind DK) {
  ...
  auto Res = toolchain::find_if(Relations,
...
bool SemaOpenACC::DiagnoseAllowedOnceClauses(
  ...
  auto Res = toolchain::find_if(Clauses, [=](const OpenACCClause *C) {
