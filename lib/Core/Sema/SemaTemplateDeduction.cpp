...
checkDeducedTemplateArguments(ASTContext &Context,
  ...
  case TemplateArgument::Null:
    toolchain_unreachable("Non-deduced template arguments handled above");
  ...
  case TemplateArgument::Integral:
    ...
    if (Y.getKind() == TemplateArgument::Expression ||
        ...
        (Y.getKind() == TemplateArgument::Integral &&
         toolchain::APSInt::isSameValue(X.getAsIntegral(), Y.getAsIntegral())))
      return X.wasDeducedFromArrayBound() ? Y : X;
  ...
  case TemplateArgument::Pack: {
    ...
    toolchain::SmallVector<TemplateArgument, 8> NewPack;
  ...
  toolchain_unreachable("Invalid TemplateArgument Kind!");
}
...
static TemplateDeductionResult DeduceNonTypeTemplateArgument(
    ...
    NonTypeOrVarTemplateParmDecl NTTP, const toolchain::APSInt &Value,
...
struct language::Core::DeducedPack {
...
private:
  ...
  unsigned addPacks(TemplateArgument Pattern) {
    ...
    toolchain::SmallBitVector SawIndices(TemplateParams->size());
    toolchain::SmallVector<TemplateArgument, 4> ExtraDeductions;
  ...
  void finishConstruction(unsigned NumNamedPacks) {
    ...
    // https://github.com/toolchain/toolchain-project/issues/100095
    ...
    for (auto &Pack : Packs) {
      ...
      if (Pack.New.empty()) {
        ...
        NewPack = DeducedTemplateArgument(
            TemplateArgument(toolchain::ArrayRef(ArgumentPack, Pack.New.size())),
...
static TemplateDeductionResult DeduceTemplateArguments(
    ...
    toolchain::SmallBitVector *HasDeducedParam) {
...
DeduceTemplateBases(Sema &S, const CXXRecordDecl *RD,
  ...
  // disqualified.
  toolchain::SmallPtrSet<const CXXRecordDecl *, 8> Visited;
  ...
  toolchain::MapVector<const CXXRecordDecl *, MatchValue> Matches;
...
#define NON_CANONICAL_TYPE(Class, Base) \
  case Type::Class: toolchain_unreachable("deducing non-canonical type: " #Class);
#define TYPE(Class, Base)
#include "language/Core/AST/TypeNodes.inc"

    case Type::TemplateTypeParm:
    case Type::SubstTemplateTypeParmPack:
      toolchain_unreachable("Type nodes handled above");
    ...
    case Type::DependentSizedArray: {
      ...
      if (const auto *CAA = dyn_cast<ConstantArrayType>(AA)) {
        toolchain::APSInt Size(CAA->getSize());
        return DeduceNonTypeTemplateArgument(
    ...
    case Type::DependentVector: {
      ...
      if (const auto *VA = A->getAs<VectorType>()) {
        ...
        toolchain::APSInt ArgSize(S.Context.getTypeSize(S.Context.IntTy), false);
      ...
      auto DeduceMatrixArg =
          [&S, &Info, &Deduced, &TemplateParams, &HasDeducedAnyParam, POK](
            ...
            if (!ParamExpr->isValueDependent()) {
              std::optional<toolchain::APSInt> ParamConst =
              ...
              if (std::optional<toolchain::APSInt> ArgConst =
    ...
    case Type::DependentAddressSpace: {
      ...
      if (isTargetAddressSpace(A.getAddressSpace())) {
        toolchain::APSInt ArgAddressSpace(S.Context.getTypeSize(S.Context.IntTy),
                                     false);
...
DeduceTemplateArguments(Sema &S, TemplateParameterList *TemplateParams,
  ...
  switch (P.getKind()) {
  case TemplateArgument::Null:
    toolchain_unreachable("Null template argument in parameter list");
  ...
  case TemplateArgument::TemplateExpansion:
    toolchain_unreachable("caller should handle pack expansions");
  ...
  case TemplateArgument::Integral:
    if (A.getKind() == TemplateArgument::Integral) {
      if (toolchain::APSInt::isSameValue(P.getAsIntegral(), A.getAsIntegral()))
        return TemplateDeductionResult::Success;
    }
    Info.FirstArg = P;
    Info.SecondArg = A;
    ...
    if (NonTypeOrVarTemplateParmDecl NTTP =
      ...
      case TemplateArgument::Expression: {
        ...
        if (const auto *ICE = dyn_cast<ImplicitCastExpr>(E);
            ICE && ICE->getCastKind() == language::Core::CK_Dependent) {
          E = ICE->getSubExpr();
          if (auto Result = DeduceTemplateArgumentsByTypeMatch(
      ...
      toolchain_unreachable("Unknown template argument kind");
    }
    // Can't deduce anything, but that's okay.
    return TemplateDeductionResult::Success;
  case TemplateArgument::Pack:
    toolchain_unreachable("Argument packs should be expanded by the caller!");
  }
...
Sema::getTrivialTemplateArgumentLoc(const TemplateArgument &Arg,
  ...
  switch (Arg.getKind()) {
  case TemplateArgument::Null:
    toolchain_unreachable("Can't get a NULL template argument here");
...
CheckDeducedArgumentConstraints(Sema &S, NamedDecl *Template,
                                ...
                                TemplateDeductionInfo &Info) {
  toolchain::SmallVector<AssociatedConstraint, 3> AssociatedConstraints;
...
getPackIndexForParam(Sema &S, FunctionTemplateDecl *FunctionTemplate,
  ...
  toolchain_unreachable("parameter index would not be produced from template");
}
...
TemplateDeductionResult Sema::FinishTemplateArgumentDeduction(
  ...
  if (OriginalCallArgs) {
    ...
    toolchain::SmallDenseMap<std::pair<unsigned, QualType>, QualType> DeducedATypes;
...
static TemplateDeductionResult DeduceFromInitializerList(
  ...
  if (auto *DependentArrTy = dyn_cast_or_null<DependentSizedArrayType>(ArrTy)) {
    ...
    if (NonTypeOrVarTemplateParmDecl NTTP = getDeducedNTTParameterFromExpr(
      ...
      toolchain::APInt Size(S.Context.getIntWidth(T),
                       ILE->getNumInitsWithEmbedExpanded());
      if (auto Result = DeduceNonTypeTemplateArgument(
              S, TemplateParams, NTTP, toolchain::APSInt(Size), T,
...
static bool isAtLeastAsSpecializedAs(
  ...
  toolchain::SmallBitVector UsedParameters(TemplateParams->size());
  switch (TPOC) {
  case TPOC_Call:
...
FunctionTemplateDecl *Sema::getMoreSpecializedTemplate(
  ...
  toolchain::SmallVector<AssociatedConstraint, 3> AC1, AC2;
...
FunctionDecl *Sema::getMoreConstrainedFunction(FunctionDecl *FD1,
  ...
  toolchain::SmallVector<AssociatedConstraint, 1> AC1, AC2;
...
bool Sema::isTemplateTemplateParameterAtLeastAsSpecializedAs(
  ...
  switch (::DeduceTemplateArguments(
      *this, A, AArgs, PArgs, Info, Deduced,
      ...
      /*HasDeducedAnyParam=*/nullptr)) {
  case language::Core::TemplateDeductionResult::Success:
  ...
  toolchain_unreachable("Unexpected TDK");
}
...
struct MarkUsedTemplateParameterVisitor : DynamicRecursiveASTVisitor {
  ...
  MarkUsedTemplateParameterVisitor(toolchain::SmallBitVector &Used,
                                   unsigned Depth)
  ...
  bool TraverseTemplateName(TemplateName Template) override {
    if (auto *TTP = toolchain::dyn_cast_or_null<TemplateTemplateParmDecl>(
...
bool hasDeducibleTemplateParameters(Sema &S,
  ...
  TemplateParameterList *TemplateParams
    = FunctionTemplate->getTemplateParameters();
  toolchain::SmallBitVector Deduced(TemplateParams->size());
