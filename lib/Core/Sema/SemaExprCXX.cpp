...
#include "language/Core/Sema/SemaLambda.h"
...
ParsedType Sema::getDestructorName(const IdentifierInfo &II,
  ...
  toolchain::SmallVector<NamedDecl*, 8> FoundDecls;
  toolchain::SmallPtrSet<CanonicalDeclPtr<Decl>, 8> FoundDeclSet;
  ...
  toolchain::stable_sort(FoundDecls, [](NamedDecl *A, NamedDecl *B) {
...
bool Sema::checkLiteralOperatorId(const CXXScopeSpec &SS,
  ...
  toolchain_unreachable("unknown nested name specifier kind");
}
...
getUuidAttrOfType(Sema &SemaRef, QualType QT,
                  toolchain::SmallSetVector<const UuidAttr *, 1> &UuidAttrs) {
...
ExprResult Sema::BuildCXXThrow(SourceLocation OpLoc, Expr *Ex,
  ...
  const toolchain::Triple &T = Context.getTargetInfo().getTriple();
...
collectPublicBases(CXXRecordDecl *RD,
                   toolchain::DenseMap<CXXRecordDecl *, unsigned> &SubobjectsSeen,
                   toolchain::SmallPtrSetImpl<CXXRecordDecl *> &VBases,
                   toolchain::SetVector<CXXRecordDecl *> &PublicSubobjectsSeen,
                   bool ParentIsPublic) {
...
static void getUnambiguousPublicSubobjects(
    CXXRecordDecl *RD, toolchain::SmallVectorImpl<CXXRecordDecl *> &Objects) {
  toolchain::DenseMap<CXXRecordDecl *, unsigned> SubobjectsSeen;
  toolchain::SmallSet<CXXRecordDecl *, 2> VBases;
  toolchain::SetVector<CXXRecordDecl *> PublicSubobjectsSeen;
  SubobjectsSeen[RD] = 1;
...
bool Sema::CheckCXXThrowOperand(SourceLocation ThrowLoc,
  ...
  if (Context.getTargetInfo().getCXXABI().isMicrosoft()) {
    ...
    toolchain::SmallVector<CXXRecordDecl *, 2> UnambiguousPublicSubobjects;
...
void Sema::MarkThisReferenced(CXXThisExpr *This) {
  ...
  auto IsDependent = [&]() {
    for (auto *Scope : toolchain::reverse(FunctionScopes)) {
...
bool Sema::isUsualDeallocationFunction(const CXXMethodDecl *Method) {
  ...
  return toolchain::none_of(PreventedBy, [&](const FunctionDecl *FD) {
...
static UsualDeallocFnInfo resolveDeallocationOverload(
    ...
    SourceLocation Loc,
    toolchain::SmallVectorImpl<UsualDeallocFnInfo> *BestFns = nullptr) {
...
ExprResult Sema::BuildCXXNew(SourceRange Range, bool UseGlobal,
  ...
  InitializationKind Kind = [&] {
    ...
    toolchain_unreachable("Unknown initialization kind");
  }();
  ...
  if (OperatorNew) {
    ...
    if (!ArraySize && !AllocType->isDependentType()) {
      ...
      bool Overflow;
      AllocationSize = toolchain::APInt(SizeTyWidth, *KnownArraySize)
    ...
    IntegerLiteral AllocationSizeLiteral(
        Context, AllocationSize.value_or(toolchain::APInt::getZero(SizeTyWidth)),
        SizeTy, StartLoc);
    ...
    IntegerLiteral AlignmentLiteral(
        Context,
        toolchain::APInt(Context.getTypeSize(SizeTy),
    ...
    toolchain::SmallVector<Expr *, 8> CallArgs;
    ...
    toolchain::append_range(CallArgs, PlacementArgs);
  ...
  if (!AllocType->isDependentType() &&
    ...
    if (KnownArraySize)
      InitType = Context.getConstantArrayType(
          AllocType,
          toolchain::APInt(Context.getTypeSize(Context.getSizeType()),
                      *KnownArraySize),
...
static bool resolveAllocationOverloadInterior(
  ...
  case OR_No_Viable_Function:
    ...
    if (Diagnose) {
      ...
      toolchain::SmallVector<Expr*, 4> AlignedArgs;
  ...
  toolchain_unreachable("Unreachable, bad result from BestViableFunction");
}
...
bool Sema::FindAllocationFunctions(
  ...
  QualType SizeTy = Context.getSizeType();
  unsigned SizeTyWidth = Context.getTypeSize(SizeTy);
  IntegerLiteral Size(Context, toolchain::APInt::getZero(SizeTyWidth), SizeTy,
                      SourceLocation());
  ...
  toolchain::append_range(AllocArgs, PlaceArgs);
  ...
  if (isPlacementNew) {
    ...
    toolchain::SmallVector<UsualDeallocFnInfo, 4> BestDeallocFns;
    ImplicitDeallocationParameters IDP = {
...
void Sema::DeclareGlobalNewDelete() {
  ...
  auto DeclareGlobalAllocationFunctions = [&](OverloadedOperatorKind Kind,
    ...
    toolchain::SmallVector<QualType, 3> Params;
...
void Sema::DeclareGlobalAllocationFunction(DeclarationName Name,
  ...
  if (HasBadAllocExceptionSpec) {
    if (!getLangOpts().CPlusPlus11) {
      ...
      EPI.ExceptionSpec.Exceptions = toolchain::ArrayRef(BadAllocType);
    }
  ...
  auto CreateAllocationFunctionDecl = [&](Attr *ExtraAttr) {
    ...
    toolchain::SmallVector<ParmVarDecl *, 3> ParamDecls;
...
bool Sema::FindDeallocationFunction(SourceLocation StartLoc, CXXRecordDecl *RD,
  ...
  //   If the deallocation functions have class scope, the one without a
  //   parameter of type std::size_t is selected.
  toolchain::SmallVector<UsualDeallocFnInfo, 4> Matches;
...
public:
  ...
  toolchain::SmallVector<const CXXNewExpr *, 4> NewExprs;
...
void Sema::AnalyzeDeleteExprMismatch(FieldDecl *Field, SourceLocation DeleteLoc,
  ...
  case MismatchingNewDeleteDetector::VarInitMismatches:
    toolchain_unreachable("This analysis should have been done for class members.");
  case MismatchingNewDeleteDetector::AnalyzeLater:
    toolchain_unreachable("Analysis cannot be postponed any point beyond end of "
                     "translation unit.");
  case MismatchingNewDeleteDetector::MemberInitMismatches:
...
ExprResult Sema::CheckConditionVariable(VarDecl *ConditionVar,
  ...
  toolchain_unreachable("unexpected condition kind");
}
...
static ExprResult BuildCXXCastArgument(Sema &S,
                                       ...
                                       Expr *From) {
  switch (Kind) {
  default: toolchain_unreachable("Unhandled cast kind!");
...
Sema::PerformImplicitConversion(Expr *From, QualType ToType,
  ...
  case ImplicitConversionSequence::StaticObjectArgumentConversion:
    toolchain_unreachable("bad conversion");
...
Sema::PerformImplicitConversion(Expr *From, QualType ToType,
  ...
  default:
    toolchain_unreachable("Improper first standard conversion");
  }
  ...
  case ICK_Pointer_Member: {
    ...
    case MemberPointerConversionResult::DifferentPointee:
      toolchain_unreachable("unexpected result");
    case MemberPointerConversionResult::NotDerived:
      toolchain_unreachable("Should not have been called if derivation isn't OK.");
  ...
  case ICK_HLSL_Vector_Splat:
    toolchain_unreachable("Improper second standard conversion");
  }
  ...
  if (SCS.Dimension != ICK_Identity) {
    ...
    case ICK_Identity:
    default:
      toolchain_unreachable("Improper element standard conversion");
    }
  }
  ...
  default:
    toolchain_unreachable("Improper third standard conversion");
  }
...
static bool FindConditionalOverload(Sema &Self, ExprResult &LHS, ExprResult &RHS,
  ...
  switch (CandidateSet.BestViableFunction(Self, QuestionLoc, Best)) {
    ...
    case OR_Deleted:
      toolchain_unreachable("Conditional operator has only built-in overloads");
  }
  return true;
}
...
QualType Sema::CheckSizelessVectorConditionalTypes(ExprResult &Cond,
  ...
  toolchain::ElementCount CondElementCount =
  ...
  toolchain::ElementCount ResultElementCount =
...
QualType Sema::FindCompositePointerType(SourceLocation Loc,
  ...
  struct Step {
    ...
    QualType rebuild(ASTContext &Ctx, QualType T) const {
      ...
      toolchain_unreachable("unknown step kind");
    }
  };
  ...
  for (auto &S : toolchain::reverse(Steps))
...
Expr *Sema::MaybeCreateExprWithCleanups(Expr *SubExpr) {
  ...
  auto Cleanups = toolchain::ArrayRef(ExprCleanupObjects.begin() + FirstCleanup,
                                 ExprCleanupObjects.size() - FirstCleanup);
...
ExprResult Sema::ActOnStartCXXMemberReference(Scope *S, Expr *Base,
  ...
  if (OpKind == tok::arrow) {
    ...
    toolchain::SmallPtrSet<CanQualType,8> CTypes;
...
ExprResult Sema::ActOnPseudoDestructorExpr(Scope *S, Expr *Base,
  ...
  default:
    toolchain_unreachable("Unsupported type in pseudo destructor");
  }
...
static void MaybeDecrementCount(
    Expr *E, toolchain::DenseMap<const VarDecl *, int> &RefsMinusAssignments) {
...
Sema::CheckMicrosoftIfExistsSymbol(Scope *S, CXXScopeSpec &SS,
  ...
  toolchain_unreachable("Invalid LookupResult Kind!");
}
...
Sema::BuildExprRequirement(
  ...
  if (E->isInstantiationDependent() || E->getType()->isPlaceholderType() ||
    ...
    toolchain::SmallVector<TemplateArgument, 1> Args;
