...
#include "language/Core/AST/LocInfoType.h"
...
namespace {
  ...
  class TypeProcessingState {
    ...
    toolchain::DenseMap<const MacroQualifiedType *, SourceLocation> LocsForMacros;
  ...
  public:
    ...
    void saveDeclSpecAttrs() {
      ...
      toolchain::append_range(savedAttrs,
                         toolchain::make_pointer_range(spec.getAttributes()));
    }
    ...
    const Attr *takeAttrForAttributedType(const AttributedType *AT) {
      if (!AttrsForTypesSorted) {
        toolchain::stable_sort(AttrsForTypes, toolchain::less_first());
        AttrsForTypesSorted = true;
      }

      // FIXME: This is quadratic if we have lots of reuses of the same
      // attributed type.
      for (auto It = toolchain::partition_point(
               AttrsForTypes,
      ...
      toolchain_unreachable("no Attr* for AttributedType*");
    }
  ...
  for (; i != 0; --i) {
    ...
    case DeclaratorChunk::Function:
      for (--i; i != 0; --i) {
        ...
        toolchain_unreachable("bad declarator chunk kind");
      }
...
#include "language/Core/Basic/TransformTypeTraits.def"
  default:
    toolchain_unreachable("attempted to parse a non-unary transform builtin");
  }
}
...
static QualType ConvertDeclSpecToType(TypeProcessingState &state) {
  ...
  case DeclSpec::TST_accum: {
    ...
    case TypeSpecifierWidth::Long:
      ...
      break;
    case TypeSpecifierWidth::LongLong:
      toolchain_unreachable("Unable to specify long long as _Accum width");
    }
  ...
  case DeclSpec::TST_fract: {
    ...
    case TypeSpecifierWidth::Long:
      ...
      break;
    case TypeSpecifierWidth::LongLong:
      toolchain_unreachable("Unable to specify long long as _Fract width");
    }
...
#include "language/Core/Basic/TransformTypeTraits.def"
  ...
  case DeclSpec::TST_decltype_auto: {
    ...
    toolchain::SmallVector<TemplateArgument, 8> TemplateArgs;
...
#define GENERIC_IMAGE_TYPE(ImgType, Id)                                        \
  case DeclSpec::TST_##ImgType##_t:                                            \
    ...
    case OpenCLAccessAttr::SpellingNotCalculated:                              \
      toolchain_unreachable("Spelling not yet calculated");                         \
    }                                                                          \
    break;
...
static ExprResult checkArraySize(Sema &S, Expr *&ArraySize,
                                 toolchain::APSInt &SizeVal, unsigned VLADiag,
                                 bool VLAIsError) {
  if (S.getLangOpts().CPlusPlus14 &&
      (VLAIsError ||
...
QualType Sema::BuildArrayType(QualType T, ArraySizeModifier ASM,
  ...
  auto IsStaticAssertLike = [](const Expr *ArraySize, ASTContext &Context) {
    ...
    if (const auto *CondExpr = dyn_cast_if_present<ConditionalOperator>(
      ...
      std::optional<toolchain::APSInt> LHS =
          CondExpr->getLHS()->getIntegerConstantExpr(Context);
      std::optional<toolchain::APSInt> RHS =
  ...
  toolchain::APSInt ConstVal(Context.getTypeSize(Context.getSizeType()));
  if (!ArraySize) {
  ...
  if (T->isVariableArrayType()) {
    if (!Context.getTargetInfo().isVLASupported()) {
      ...
      targetDiag(Loc,
          ...
          << (IsCUDADevice ? toolchain::to_underlying(CUDA().CurrentTarget()) : 0);
...
QualType Sema::BuildVectorType(QualType CurType, Expr *SizeExpr,
  ...
  std::optional<toolchain::APSInt> VecSize =
...
QualType Sema::BuildMatrixType(QualType ElementTy, Expr *NumRows, Expr *NumCols,
  ...
  std::optional<toolchain::APSInt> ValueRows =
      NumRows->getIntegerConstantExpr(Context);
  std::optional<toolchain::APSInt> ValueColumns =
...
static void checkExtParameterInfos(Sema &S, ArrayRef<QualType> paramTypes,
                    ...
                    toolchain::function_ref<SourceLocation(unsigned)> getParamLoc) {
...
static void diagnoseRedundantReturnTypeQualifiers(Sema &S, QualType RetTy,
  ...
  for (unsigned OuterChunkIndex = FunctionChunkIndex + 1,
    ...
    toolchain_unreachable("unknown declarator chunk kind");
  }
...
static QualType GetDeclSpecTypeForDeclarator(TypeProcessingState &state,
    ...
    case DeclaratorContext::Member: {
      ...
      if (isa<ObjCContainerDecl>(SemaRef.CurContext)) {
        ...
        case TagTypeKind::Enum:
          toolchain_unreachable("unhandled tag kind");
  ...
  if (SemaRef.getLangOpts().CPlusPlus &&
    ...
    case DeclaratorContext::TrailingReturnVar:
      ...
      toolchain_unreachable("parser should not have allowed this");
      break;
    case DeclaratorContext::File:
    case DeclaratorContext::Member:
    case DeclaratorContext::Block:
...
static CallingConv getCCForDeclaratorChunk(
  ...
  if (S.getLangOpts().CUDA) {
    ...
    toolchain::Triple Triple = S.Context.getTargetInfo().getTriple();
    if (Triple.isSPIRV() && Triple.getVendor() != toolchain::Triple::AMD) {
...
static Attr *createNullabilityAttr(ASTContext &Ctx, ParsedAttr &Attr,
  ...
  toolchain_unreachable("unknown NullabilityKind");
}
...
static void transferARCOwnershipToDeclaratorChunk(TypeProcessingState &state,
  ...
  switch (ownership) {
  case Qualifiers::OCL_None: toolchain_unreachable("no ownership!");
...
static void fillMatrixTypeLoc(MatrixTypeLoc MTL,
  ...
  toolchain_unreachable("no matrix_type attribute found at the expected location!");
}
...
static void fillAtomicQualLoc(AtomicTypeLoc ATL, const DeclaratorChunk &Chunk) {
  ...
  case DeclaratorChunk::Paren:
  case DeclaratorChunk::Pipe:
    toolchain_unreachable("cannot be _Atomic qualified");
...
namespace {
  ...
  public:
    ...
    void VisitQualifiedTypeLoc(QualifiedTypeLoc TL) {
      toolchain_unreachable("qualified type locs not expected here!");
    }
    void VisitDecayedTypeLoc(DecayedTypeLoc TL) {
      toolchain_unreachable("decayed type locs not expected here!");
    }
    void VisitArrayParameterTypeLoc(ArrayParameterTypeLoc TL) {
      toolchain_unreachable("array parameter type locs not expected here!");
    }
    ...
    void VisitTypeLoc(TypeLoc TL) {
      toolchain_unreachable("unsupported TypeLoc kind in declarator!");
    }
  };
} // end anonymous namespace
...
void LocInfoType::getAsStringInternal(std::string &Str,
  ...
  toolchain_unreachable("LocInfoType leaked into the type system; an opaque TypeTy*"
...
static bool BuildAddressSpaceIndex(Sema &S, LangAS &ASIdx,
  ...
  if (!AddrSpace->isValueDependent()) {
    std::optional<toolchain::APSInt> OptAddrSpace =
    ...
    toolchain::APSInt &addrSpace = *OptAddrSpace;
    ...
    toolchain::APSInt max(addrSpace.getBitWidth());
    max =
...
static void HandleAddressSpaceTypeAttribute(QualType &Type,
  ...
  if (Attr.getKind() == ParsedAttr::AT_AddressSpace) {
    ...
    if (ASIdx == LangAS::Default)
      toolchain_unreachable("Invalid address space");
...
namespace {
  ...
  private:
    ...
    QualType wrap(ASTContext &C, const Type *Old, unsigned I) {
      ...
      toolchain_unreachable("unknown wrapping kind");
    }
  };
} // end anonymous namespace
...
static bool handleMSPointerTypeQualifierAttr(TypeProcessingState &State,
  ...
  default: toolchain_unreachable("Unknown attribute kind");
  case ParsedAttr::AT_Ptr32:
...
static NullabilityKind mapNullabilityAttrKind(ParsedAttr::Kind kind) {
  ...
  default:
    toolchain_unreachable("not a nullability attribute kind");
  }
}
...
static Attr *getCCTypeAttr(ASTContext &Ctx, ParsedAttr &Attr) {
  ...
  switch (Attr.getKind()) {
  default:
    toolchain_unreachable("not a calling convention attribute");
  case ParsedAttr::AT_CDecl:
  ...
  case ParsedAttr::AT_Pcs: {
    ...
    if (!PcsAttr::ConvertStrToPCSType(Str, Type))
      toolchain_unreachable("already validated the attribute");
  ...
  case ParsedAttr::AT_RISCVVLSCC: {
    ...
    if (Attr.getNumArgs()) {
      std::optional<toolchain::APSInt> MaybeABIVLen =
          Attr.getArgAsExpr(0)->getIntegerConstantExpr(Ctx);
      if (!MaybeABIVLen)
        toolchain_unreachable("Invalid RISC-V ABI VLEN");
  ...
  toolchain_unreachable("unexpected attribute kind!");
}
...
Sema::ActOnEffectExpression(Expr *CondExpr, StringRef AttributeName) {
  ...
  std::optional<toolchain::APSInt> ConditionValue =
...
static bool checkMutualExclusion(TypeProcessingState &state,
  ...
  auto OtherAttr = toolchain::find_if(
...
static bool handleFunctionTypeAttr(TypeProcessingState &state, ParsedAttr &attr,
  ...
  if (attr.getKind() == ParsedAttr::AT_ArmStreaming ||
    ...
    case ParsedAttr::AT_ArmAgnostic:
      ...
      break;
    default:
      toolchain_unreachable("Unsupported attribute");
    }
  ...
  if (attr.getKind() == ParsedAttr::AT_NoThrow) {
    ...
    if (Proto->hasExceptionSpec()) {
      ...
      case EST_None:
        toolchain_unreachable("This doesn't have an exception spec!");
...
static bool isPermittedNeonBaseType(QualType &Ty, VectorKind VecKind, Sema &S) {
  ...
  // now.
  bool IsPolyUnsigned = Triple.getArch() == toolchain::Triple::aarch64 ||
  ...
  if ((Triple.isArch64Bit() || Triple.getArch() == toolchain::Triple::aarch64_32) &&
...
static bool verifyValidIntegerConstantExpr(Sema &S, const ParsedAttr &Attr,
  ...
  if (!AttrExpr->isTypeDependent()) {
    if (std::optional<toolchain::APSInt> Res =
...
static void HandleNeonVectorTypeAttr(QualType &CurType, const ParsedAttr &Attr,
  ...
  toolchain::APSInt numEltsInt(32);
...
static void HandleArmSveVectorBitsTypeAttr(QualType &CurType, ParsedAttr &Attr,
  ...
  toolchain::APSInt SveVectorSizeInBits(32);
...
static void HandleRISCVRVVVectorBitsTypeAttr(QualType &CurType,
  ...
  // The vector size must be an integer constant expression.
  toolchain::APSInt RVVVectorSizeInBits(32);
  ...
  if (const TypedefType* TypedefTy = CurType->getAs<TypedefType>()) {
    ...
    if (BaseTy->isPipeType()) {
      ...
      case BuiltinType::Id:                                          \
        ...
        #include "language/Core/Basic/OpenCLImageTypes.def"
      default:
        toolchain_unreachable("Unable to find corresponding image type.");
      }
    } else {
...
static void HandleAnnotateTypeAttr(TypeProcessingState &State,
  ...
  toolchain::SmallVector<Expr *, 4> Args;
...
static void processTypeAttrs(TypeProcessingState &state, QualType &type,
  ...
  for (ParsedAttr &attr : AttrsCopy) {
    ...
    case ParsedAttr::AT_NoDeref: {
      ...
      // See https://github.com/toolchain/toolchain-project/issues/55790 for details.
  ...
  if (const auto CastE = dyn_cast<ExplicitCastExpr>(E)) {
    ...
    if (const auto *IAT = Context.getAsIncompleteArrayType(DestType)) {
      ...
      QualType ResultType = Context.getConstantArrayType(
          IAT->getElementType(),
          toolchain::APInt(Context.getTypeSize(Context.getSizeType()), 1),
...
static unsigned getLiteralDiagFromTagKind(TagTypeKind Tag) {
  ...
  case TagTypeKind::Class:
    return 2;
  default: toolchain_unreachable("Invalid tag kind for literal type diagnostic!");
  }
}
...
BuildTypeCoupledDecls(Expr *E,
                      toolchain::SmallVectorImpl<TypeCoupledDeclRefInfo> &Decls) {
...
QualType Sema::BuildCountAttributedArrayOrPointerType(QualType WrappedTy,
  ...
  toolchain::SmallVector<TypeCoupledDeclRefInfo, 1> Decls;
...
QualType Sema::BuildPackIndexingType(QualType Pattern, Expr *IndexExpr,
  ...
  if (FullySubstituted && !IndexExpr->isValueDependent() &&
    ...
    toolchain::APSInt Value(Context.getIntWidth(Context.getSizeType()));
...
static QualType ChangeIntegralSignedness(Sema &S, QualType BaseType,
  ...
  auto *Result =
      toolchain::find_if(*Consider, [&S, BaseSize](const CanQual<Type> *T) {
