  ...
  if (isa<ObjCMethodDecl>(D) || isa<FunctionDecl>(D)) {
    ...
    if (SwiftParamCount == ParamCount) {
      ...
      unsigned MaybeOutParamCount =
          toolchain::count_if(Params, [](const ParmVarDecl *Param) -> bool {
...
void SemaSwift::AddParameterABIAttr(Decl *D, const AttributeCommonInfo &CI,
  ...
  case ParameterABI::HLSLInOut:
    toolchain_unreachable("explicit attribute for non-swift parameter ABI?");
  case ParameterABI::Ordinary:
    toolchain_unreachable("explicit attribute for ordinary parameter ABI?");
  ...
  toolchain_unreachable("bad parameter ABI attribute");
}
