...
class AtomicOperandChecker {
  ...
  IDACInfo CheckAssignmentWithBinOpOnRHS(BinaryOpInfo AssignInf,
    ...
    toolchain::FoldingSetNodeID LHS_ID, InnerLHS_ID, InnerRHS_ID;
  ...
  IDACInfo CheckIncDecAssignCompoundAssign(const Expr *E,
    ...
    toolchain_unreachable("all binary operator kinds should be checked above");
  }
  ...
  bool CheckVarRefsSame(IDACInfo::ExprKindTy FirstKind, const Expr *FirstX,
    ...
    toolchain::FoldingSetNodeID First_ID, Second_ID;
  ...
  StmtResult CheckCapture() {
    if (const auto *CmpdStmt = dyn_cast<CompoundStmt>(AssocStmt.get())) {
      ...
      case IDACInfo::ExprAssign:
        toolchain_unreachable("Should have error'ed out by now");
      case IDACInfo::Unary:
...
public:
  ...
  StmtResult Check() {
    ...
    toolchain_unreachable("Unhandled atomic kind?");
  }
};
} // namespace
