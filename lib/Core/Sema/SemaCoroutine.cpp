...
VarDecl *Sema::buildCoroutinePromise(SourceLocation Loc) {
  ...
  toolchain::SmallVector<Expr *, 4> CtorArgExprs;
...
static void checkNoThrow(Sema &S, const Stmt *E,
                         toolchain::SmallPtrSetImpl<const Decl *> &ThrowingDecls) {
...
bool Sema::checkFinalSuspendNoThrow(const Stmt *FinalSuspend) {
  toolchain::SmallPtrSet<const Decl *, 4> ThrowingDecls;
  ...
  auto SortedDecls = toolchain::SmallVector<const Decl *, 4>{ThrowingDecls.begin(),
                                                        ThrowingDecls.end()};
...
static void applySafeElideContext(Expr *Operand) {
  ...
  // Check parameter
  auto *Fn = toolchain::dyn_cast_if_present<FunctionDecl>(Call->getCalleeDecl());
  if (!Fn)
    return;
...
void Sema::CheckCompletedCoroutineBody(FunctionDecl *FD, Stmt *&Body) {
  ...
  // problems (see https://toolchain.org/PR53413). Tell the user about it.
...
bool CoroutineStmtBuilder::makeNewAndDeleteExpr() {
  ...
  if (OperatorNew->getNumParams() > NewArgs.size())
    toolchain::append_range(NewArgs, PlacementArgs);
...
bool CoroutineStmtBuilder::makeGroDeclAndReturnStmt() {
  ...
  StmtResult ReturnStmt;
  language::Core::VarDecl *GroDecl = nullptr;
  if (GroMatchesRetType) {
  ...
  if (!GroMatchesRetType &&
      cast<language::Core::ReturnStmt>(ReturnStmt.get())->getNRVOCandidate() == GroDecl)
