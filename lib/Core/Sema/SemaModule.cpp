...
makeTransitiveImportsVisible(ASTContext &Ctx, VisibleModuleSet &VisibleModules,
  ...
  toolchain::SmallVector<Module *, 4> Worklist;
  toolchain::SmallSet<Module *, 16> Visited;
...
static bool DiagReservedModuleName(Sema &S, const IdentifierInfo *II,
  ...
  toolchain_unreachable("fell off a fully covered switch");
}
...
Sema::ActOnModuleDecl(SourceLocation StartLoc, SourceLocation ModuleLoc,
  ...
  if (IsPartition)
    ...
    case ModuleDeclKind::Interface:
      ...
      break;
    default:
      toolchain_unreachable("how did we get a partition type set?");
    }
  ...
  if (!getSourceManager().isInSystemHeader(Path[0].getLoc()) &&
      (FirstComponentName == "std" ||
       (FirstComponentName.starts_with("std") &&
        toolchain::all_of(FirstComponentName.drop_front(3), &toolchain::isDigit))))
...
private:
  toolchain::DenseSet<const NamedDecl *> ExposureSet;
  toolchain::DenseSet<const NamedDecl *> KnownNonExposureSet;
};
