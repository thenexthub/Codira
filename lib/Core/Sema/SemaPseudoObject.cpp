...
namespace {
  ...
  struct Rebuilder {
    ...
    typedef toolchain::function_ref<Expr *(Expr *, unsigned)> SpecificRebuilderRefTy;
    ...
    Expr *rebuild(Expr *e) {
      ...
      toolchain_unreachable("bad expression to rebuild!");
    }
  };
...
PseudoOpBuilder::buildIncDecOperation(Scope *Sc, SourceLocation opcLoc,
  ...
  toolchain::APInt oneV(S.Context.getTypeSize(S.Context.IntTy), 1);
  ...
  syntacticBase =
      Rebuilder(S, [=](Expr *, unsigned Idx) -> Expr * {
        ...
        case 1:
          return InstanceKey;
        default:
          toolchain_unreachable("Unexpected index for ObjCSubscriptExpr");
        }
      }).rebuild(syntacticBase);
...
ExprResult SemaPseudoObject::checkRValue(Expr *E) {
  ...
  if (ObjCPropertyRefExpr *refExpr
    ...
    toolchain_unreachable("unknown pseudo-object kind!");
  }
}
