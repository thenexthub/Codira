...
#include "language/Core/Sema/SemaConcept.h"
...
struct SatisfactionStackRAII {
  ...
  SatisfactionStackRAII(Sema &SemaRef, const NamedDecl *ND,
                        const toolchain::FoldingSetNodeID &FSNID)
      : SemaRef(SemaRef) {
      if (ND) {
...
static bool
DiagRecursiveConstraintEval(Sema &S, toolchain::FoldingSetNodeID &ID,
...
static ExprResult calculateConstraintSatisfaction(
  ...
  if (Op == language::Core::OO_PipePipe && IsLHSSatisfied)
    // [temp.constr.op] p3
  ...
  if (Op == language::Core::OO_AmpAmp && !IsLHSSatisfied)
    // [temp.constr.op] p2
  ...
  if (Op == language::Core::OO_PipePipe && IsRHSSatisfied) {
...
static bool CheckConstraintSatisfaction(
    ...
    toolchain::SmallVectorImpl<Expr *> &Converted,
...
bool Sema::CheckConstraintSatisfaction(
  ...
  // here.
  toolchain::SmallVector<TemplateArgument, 4> FlattenedArgs;
  for (auto List : TemplateArgsLists)
    toolchain::append_range(FlattenedArgs, List.Args);
...
static const Expr *SubstituteConstraintExpressionWithoutSatisfaction(
  ...
  ExprResult SubstConstr = S.SubstConstraintExprWithoutSatisfaction(
      const_cast<language::Core::Expr *>(ConstrExpr), MLTAL);
...
static void diagnoseUnsatisfiedRequirement(Sema &S,
  ...
  switch (Req->getSatisfactionStatus()) {
    case concepts::ExprRequirement::SS_Dependent:
      toolchain_unreachable("Diagnosing a dependent requirement");
      break;
    ...
    case concepts::ExprRequirement::SS_Satisfied:
      toolchain_unreachable("We checked this above");
  }
}
...
static void diagnoseUnsatisfiedRequirement(Sema &S,
  ...
  default:
    toolchain_unreachable("Unknown satisfaction status");
    return;
  }
}
...
static void diagnoseWellFormedUnsatisfiedConstraintExpr(Sema &S,
  ...
  if (BinaryOperator *BO = dyn_cast<BinaryOperator>(SubstExpr)) {
    ...
    return;
             ...
             TTE && TTE->getTrait() == language::Core::TypeTrait::BTT_IsDeducible) {
...
static void diagnoseUnsatisfiedConstraintExpr(
    Sema &S, const toolchain::PointerUnion<Expr *, SubstitutionDiagnostic *> &Record,
    bool First = true) {
...
const NormalizedConstraint *language::Core::getNormalizedAssociatedConstraints(
...
substituteParameterMappings(Sema &S, NormalizedConstraint &N,
  ...
  if (!Atomic.ParameterMapping) {
    toolchain::SmallBitVector OccurringIndices(TemplateParams->size());
...
bool FoldExpandedConstraint::AreCompatibleForSubsumption(
  ...
  toolchain::SmallVector<UnexpandedParameterPack> APacks, BPacks;
  ...
  for (const UnexpandedParameterPack &APack : APacks) {
    ...
    auto it = toolchain::find_if(BPacks, [&](const UnexpandedParameterPack &BPack) {
...
auto SubsumptionChecker::find(FoldExpandedConstraint *Ori) -> Literal {
  ...
  auto It = toolchain::find_if(Elems, [&K](const FoldExpendedConstraintKey &Other) {
...
FormulaType SubsumptionChecker::Normalize(const NormalizedConstraint &NC) {
  ...
  auto Add = [&, this](Clause C) {
    ...
    toolchain::sort(C);
    C.erase(toolchain::unique(C), C.end());
  ...
  for (const auto &LTransform : Left) {
    for (const auto &RTransform : Right) {
      ...
      toolchain::append_range(Combined, LTransform);
      toolchain::append_range(Combined, RTransform);
...
bool SubsumptionChecker::DNFSubsumes(const Clause &P, const Clause &Q) {
  ...
  return toolchain::any_of(P, [&](Literal LP) {
    return toolchain::any_of(Q, [this, LP](Literal LQ) { return Subsumes(LP, LQ); });
  });
}
...
bool SubsumptionChecker::Subsumes(Literal A, Literal B) {
  ...
  toolchain_unreachable("unknown literal kind");
}
