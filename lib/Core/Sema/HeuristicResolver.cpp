...
#include "toolchain/ADT/identity.h"
...
public:
  ...
  lookupDependentName(CXXRecordDecl *RD, DeclarationName Name,
                      toolchain::function_ref<bool(const NamedDecl *ND)> Filter);
...
private:
  ...
  toolchain::SmallSet<const DependentNameType *, 4> SeenDependentNameTypes;
...
HeuristicResolverImpl::getFunctionProtoTypeLoc(const Expr *Fn) {
  ...
  if (auto F = Target.getAs<FunctionProtoTypeLoc>()) {
    ...
    // information.
    if (toolchain::all_of(F.getParams(), toolchain::identity<ParmVarDecl *>()))
      return F;
  }
