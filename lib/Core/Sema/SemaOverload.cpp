...
static bool functionHasPassObjectSizeParams(const FunctionDecl *FD) {
  return toolchain::any_of(FD->parameters(), [](const ParmVarDecl *P) {
...
ImplicitConversionRank language::Core::GetConversionRank(ImplicitConversionKind Kind) {
...
language::Core::GetDimensionConversionRank(ImplicitConversionRank Base,
...
NarrowingKind StandardConversionSequence::getNarrowingKind(
  ...
  FloatingIntegralConversion:
    if (FromType->isRealFloatingType() && ToType->isIntegralType(Ctx)) {
      ...
      if (std::optional<toolchain::APSInt> IntConstantValue =
              Initializer->getIntegerConstantExpr(Ctx)) {
        // Convert the integer to the floating type.
        toolchain::APFloat Result(Ctx.getFloatTypeSemantics(ToType));
        Result.convertFromAPInt(*IntConstantValue, IntConstantValue->isSigned(),
                                toolchain::APFloat::rmNearestTiesToEven);
        // And back.
        toolchain::APSInt ConvertedValue = *IntConstantValue;
        bool ignored;
        Result.convertToInteger(ConvertedValue,
                                toolchain::APFloat::rmTowardZero, &ignored);
  ...
  case ICK_Floating_Conversion:
    if (FromType->isRealFloatingType() && ToType->isRealFloatingType() &&
      ...
      if ((Ctx.getLangOpts().C23 && Initializer->EvaluateAsRValue(R, Ctx)) ||
        ...
        toolchain::APFloat FloatVal = ConstantValue.getFloat();
        // Convert the source value into the target type.
        bool ignored;
        toolchain::APFloat Converted = FloatVal;
        toolchain::APFloat::opStatus ConvertStatus =
            Converted.convert(Ctx.getFloatTypeSemantics(ToType),
                              toolchain::APFloat::rmNearestTiesToEven, &ignored);
        ...
        if (Ctx.getLangOpts().C23) {
          ...
          if (ConvertStatus & toolchain::APFloat::opOverflow) {
            ConstantType = Initializer->getType();
            return NK_Constant_Narrowing;
          }
        }
  ...
  IntegralConversion: {
    ...
    std::optional<toolchain::APSInt> OptInitializerValue =
        Initializer->getIntegerConstantExpr(Ctx);
    ...
    toolchain::APSInt &InitializerValue = *OptInitializerValue;
    ...
    if (FromWidth < ToWidth) {
      ...
      toolchain::APSInt ConvertedValue = InitializerValue;
...
language::Core::MakeDeductionFailureInfo(ASTContext &Context,
  ...
  case TemplateDeductionResult::AlreadyDiagnosed:
    toolchain_unreachable("not a deduction failure");
  }
...
static bool IsOverloadOrOverrideImpl(Sema &SemaRef, FunctionDecl *New,
  ...
  for (specific_attr_iterator<EnableIfAttr>
    ...
    if (NewI == NewE || OldI == OldE)
      return true;
    toolchain::FoldingSetNodeID NewID, OldID;
...
static bool IsFloatingPointConversion(Sema &S, QualType FromType,
  ...
  // permitted.
  const toolchain::fltSemantics &FromSem = S.Context.getFloatTypeSemantics(FromType);
  const toolchain::fltSemantics &ToSem = S.Context.getFloatTypeSemantics(ToType);
  if ((&FromSem == &toolchain::APFloat::PPCDoubleDouble() &&
       &ToSem == &toolchain::APFloat::IEEEquad()) ||
      (&FromSem == &toolchain::APFloat::IEEEquad() &&
       &ToSem == &toolchain::APFloat::PPCDoubleDouble()))
    return false;
  return true;
}
...
bool Sema::IsIntegralPromotion(Expr *From, QualType FromType, QualType ToType) {
  ...
  if (From) {
    if (FieldDecl *MemberDecl = From->getSourceBitField()) {
      std::optional<toolchain::APSInt> BitWidth;
      if (FromType->isIntegralType(Context) &&
          (BitWidth =
               MemberDecl->getBitWidth()->getIntegerConstantExpr(Context))) {
        toolchain::APSInt ToSize(BitWidth->getBitWidth(), BitWidth->isUnsigned());
...
bool Sema::FunctionParamTypesAreEqual(ArrayRef<QualType> Old,
                                      ...
                                      bool Reversed) {
  assert(toolchain::size(Old) == toolchain::size(New) &&
         "Can't compare parameters of functions with different number of "
         "parameters!");

  for (auto &&[Idx, Type] : toolchain::enumerate(Old)) {
    // Reverse iterate over the parameters of `OldType` if `Reversed` is true.
    size_t J = Reversed ? (toolchain::size(New) - Idx - 1) : Idx;
...
IsInitializerListConstructorConversion(Sema &S, Expr *From, QualType ToType,
  ...
  toolchain_unreachable("Invalid OverloadResult!");
}
...
IsUserDefinedConversion(Sema &S, Expr *From, QualType ToType,
        ...
        if (Usable) {
          ...
          if (Info.ConstructorTmpl)
            S.AddTemplateOverloadCandidate(
                ...
                /*ExplicitArgs*/ nullptr, toolchain::ArrayRef(Args, NumArgs),
                CandidateSet, SuppressUserConversions,
                /*PartialOverloading*/ false,
                AllowExplicit == AllowedExplicit::All);
          else
            ...
            S.AddOverloadCandidate(Info.Constructor, Info.FoundDecl,
                                   toolchain::ArrayRef(Args, NumArgs), CandidateSet,
                                   SuppressUserConversions,
                                   /*PartialOverloading*/ false,
                                   AllowExplicit == AllowedExplicit::All);
        }
  ...
  case OR_Deleted:
    ...
    toolchain_unreachable("Not a constructor or conversion function?");
  ...
  if ((AT || S.isStdInitializerList(ToType, &InitTy)) && !IsDesignatedInit) {
    ...
    if (AT) {
      ...
      if (ConstantArrayType const *CT = dyn_cast<ConstantArrayType>(AT)) {
        ...
        toolchain::APInt Size(S.Context.getTypeSize(S.Context.getSizeType()), e);
...
static ImplicitConversionSequence TryObjectArgumentInitialization(
  ...
  auto ValueKindFromClassification = [](Expr::Classification C) {
    if (C.isPRValue())
      return language::Core::VK_PRValue;
    if (C.isXValue())
      return VK_XValue;
    return language::Core::VK_LValue;
  };
...
ExprResult Sema::PerformImplicitObjectArgumentInitialization(
  ...
  if (ICS.isBad()) {
    ...
    case BadConversionSequence::too_many_initializers:
      toolchain_unreachable("Lists are not objects");
    }
...
static bool CheckConvertedConstantConversions(Sema &S,
  ...
  case ICK_HLSL_Array_RValue:
    toolchain_unreachable("found a first conversion kind in Second");

  case ICK_Function_Conversion:
  case ICK_Qualification:
    toolchain_unreachable("found a third conversion kind in Second");

  case ICK_Num_Conversion_Kinds:
    break;
  }

  toolchain_unreachable("unknown conversion kind");
}
...
static ExprResult BuildConvertedConstantExpression(Sema &S, Expr *From,
  ...
  case ImplicitConversionSequence::StaticObjectArgumentConversion:
    toolchain_unreachable("bad conversion in converted constant expression");
  }
...
void Sema::AddOverloadCandidate(
  ...
  Candidate.IsADLCandidate = toolchain::to_underlying(IsADLCandidate);
  ...
  for (auto *EIA : EnableIfAttrs) {
    ...
    if (EIA->getCond()->isValueDependent() ||
        !EIA->getCond()->EvaluateWithSubstitution(
            Result, Context, Function, toolchain::ArrayRef(ConvertedArgs)))
      return EIA;
...
static bool diagnoseDiagnoseIfAttrsWith(Sema &S, const NamedDecl *ND,
  ...
  auto ErrAttr = toolchain::find_if(toolchain::make_range(Attrs.begin(), WarningBegin),
                               IsSuccessful);
  ...
  for (const auto *DIA : toolchain::make_range(WarningBegin, Attrs.end()))
    if (IsSuccessful(DIA)) {
...
static void AddMethodTemplateCandidateImmediately(
  ...
  if (TemplateDeductionResult Result = S.DeduceTemplateArguments(
          ...
          CandidateSet.getKind() ==
              language::Core::OverloadCandidateSet::CSK_AddressOfOverloadSet,
...
bool Sema::CheckNonDependentConversions(
  ...
  auto MaybeInvolveUserDefinedConversion = [&](QualType ParamType,
    ...
    if (auto *RD = ParamType->getAsCXXRecordDecl();
        RD && RD->hasDefinition() &&
        toolchain::any_of(LookupConstructors(RD), [](NamedDecl *ND) {
...
class BuiltinCandidateTypeSet  {
  ...
  typedef toolchain::SmallSetVector<QualType, 8> TypeSet;
...
public:
  ...
  toolchain::iterator_range<iterator> pointer_types() { return PointerTypes; }
  toolchain::iterator_range<iterator> member_pointer_types() {
    return MemberPointerTypes;
  }
  toolchain::iterator_range<iterator> enumeration_types() {
    return EnumerationTypes;
  }
  toolchain::iterator_range<iterator> vector_types() { return VectorTypes; }
  toolchain::iterator_range<iterator> matrix_types() { return MatrixTypes; }
  toolchain::iterator_range<iterator> bitint_types() { return BitIntTypes; }
...
static void forAllQualifierCombinationsImpl(
    ...
    toolchain::function_ref<void(QualifiersAndAtomic)> Callback) {
  // _Atomic
...
class BuiltinOperatorOverloadBuilder {
  ...
  void InitArithmeticTypes() {
    ...
    /// of arithmetic types.
    toolchain::SmallSetVector<CanQualType, 2> BitIntCandidates;
    ...
    toolchain::move(BitIntCandidates, std::back_inserter(ArithmeticTypes));
...
public:
  ...
  void addEqualEqualOrNotEqualMemberPointerOrNullptrOverloads() {
    ...
    toolchain::SmallPtrSet<QualType, 8> AddedTypes;
  ...
  void addGenericBinaryPointerOrEnumeralOverloads(bool IsSpaceship) {
    ...
    toolchain::DenseSet<std::pair<CanQualType, CanQualType> >
...
void Sema::AddBuiltinOperatorCandidates(OverloadedOperatorKind Op,
  ...
  case NUM_OVERLOADED_OPERATORS:
    toolchain_unreachable("Expected an overloaded operator");
...
isBetterMultiversionCandidate(const OverloadCandidate &Cand1,
  ...
  toolchain_unreachable("No way to get here unless both had cpu_dispatch");
}
...
bool language::Core::isBetterOverloadCandidate(
...
bool Sema::isEquivalentInternalLinkageDeclaration(const NamedDecl *A,
  ...
  if (auto *EA = dyn_cast<EnumConstantDecl>(VA)) {
    if (auto *EB = dyn_cast<EnumConstantDecl>(VB)) {
      ...
      return toolchain::APSInt::isSameValue(EA->getInitVal(), EB->getInitVal());
    }
  }
...
void OverloadCandidateSet::CudaExcludeWrongSideCandidates(
  ...
  bool ContainsSameSideCandidate =
      toolchain::any_of(Candidates, [&](const OverloadCandidate *Cand) {
  ...
  toolchain::erase_if(Candidates, IsWrongSideCandidate);
}
...
OverloadingResult OverloadCandidateSet::BestViableFunctionImpl(
  ...
  toolchain::SmallVector<OverloadCandidate *, 16> Candidates;
  ...
  toolchain::SmallVector<OverloadCandidate *, 4> PendingBest;
  toolchain::SmallVector<const NamedDecl *, 4> EquivalentCands;
...
static bool checkAddressOfFunctionIsAvailable(Sema &S, const FunctionDecl *FD,
  ...
  auto I = toolchain::find_if(FD->parameters(), [](const ParmVarDecl *P) {
    return P->hasAttr<PassObjectSizeAttr>();
  });
  if (I == FD->param_end())
    return true;
...
static void DiagnoseBadConversion(Sema &S, OverloadCandidate *Cand,
  ...
  bool HasParamPack =
      toolchain::any_of(Fn->parameters().take_front(I), [](const ParmVarDecl *Parm) {
...
static TemplateDecl *getDescribedTemplate(Decl *Templated) {
  if (TemplateDecl *TD = Templated->getDescribedTemplate())
    return TD;
  toolchain_unreachable("Unsupported: Getting the described template declaration"
...
static void DiagnoseBadDeduction(Sema &S, NamedDecl *Found, Decl *Templated,
  ...
  case TemplateDeductionResult::NonDependentConversionFailure:
    toolchain_unreachable("TemplateDeductionResult::NonDependentConversionFailure "
...
static void DiagnoseFailedExplicitSpec(Sema &S, OverloadCandidate *Cand) {
  ...
  case Decl::Kind::CXXDeductionGuide:
    ...
    break;
  default:
    toolchain_unreachable("invalid Decl");
  }
...
static void NoteImplicitDeductionGuide(Sema &S, FunctionDecl *Fn) {
  ...
  toolchain::raw_string_ostream OS(FunctionProto);
  ...
  if (!Template) {
    ...
    if (!Pattern) {
      ...
      // aliases.
      // FIXME: Add a test once https://github.com/toolchain/toolchain-project/pull/96686
      // gets merged.
...
static void NoteFunctionCandidate(Sema &S, OverloadCandidate *Cand,
  ...
  // noisy.
  auto _ = toolchain::make_scope_exit([&] { NoteImplicitDeductionGuide(S, Fn); });
...
static unsigned RankDeductionFailure(const DeductionFailureInfo &DFI) {
  ...
  case TemplateDeductionResult::NonDependentConversionFailure:
  case TemplateDeductionResult::AlreadyDiagnosed:
    toolchain_unreachable("non-deduction failure while diagnosing bad deduction");
  ...
  toolchain_unreachable("Unhandled deduction result");
}
...
SmallVector<OverloadCandidate *, 32> OverloadCandidateSet::CompleteCandidates(
    ...
    SourceLocation OpLoc,
    toolchain::function_ref<bool(OverloadCandidate &)> Filter) {
...
void TemplateSpecCandidateSet::NoteCandidates(Sema &S, SourceLocation Loc) {
  ...
  toolchain::sort(Cands, CompareTemplateSpecCandidatesForDisplay(S));
...
private:
  ...
  bool eliminiateSuboptimalOverloadCandidates() {
    ...
    if (!toolchain::all_of(Matches, IsBestOrInferiorToBest))
      return false;
    Matches[0] = *Best;
    Matches.resize(1);
    return true;
...
bool Sema::buildOverloadedCallSet(Scope *S, Expr *Fn,
  ...
  if (ULE->requiresADL()) {
    ...
    if (ULE->decls_begin() != ULE->decls_end() &&
      ...
      toolchain_unreachable("performing ADL for builtin");
...
Sema::CreateOverloadedUnaryOp(SourceLocation OpLoc, UnaryOperatorKind Opc,
  ...
  if (Opc == UO_PostInc || Opc == UO_PostDec) {
    toolchain::APSInt Zero(Context.getTypeSize(Context.IntTy), false);
    ...
    case OR_Success: {
      ...
      if (FnDecl) {
        ...
        if (AllowRewrittenCandidates && !IsReversed &&
          ...
          toolchain::SmallVector<FunctionDecl*, 4> AmbiguousWith;
...
ExprResult Sema::BuildCallToMemberFunction(Scope *S, Expr *MemExprE,
  ...
  auto BuildRecoveryExpr = [&](QualType Type) {
    ...
    toolchain::append_range(SubExprs, Args);
  ...
  toolchain::SmallVector<Expr *, 8> NewArgs;
  if (Method->isExplicitObjectMemberFunction()) {
...
ExprResult Sema::BuildLiteralOperatorCall(LookupResult &R,
  ...
  UserDefinedLiteral *UDL = UserDefinedLiteral::Create(
      Context, Fn.get(), toolchain::ArrayRef(ConvArgs, Args.size()), ResultTy, VK,
...
ExprResult Sema::FixOverloadedFunctionReference(Expr *E, DeclAccessPair Found,
  ...
  toolchain_unreachable("Invalid reference to overloaded function");
}
...
bool language::Core::shouldEnforceArgLimit(bool PartialOverloading,
