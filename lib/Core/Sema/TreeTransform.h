...
#ifndef LANGUAGE_CORE_LIB_SEMA_TREETRANSFORM_H
#define LANGUAGE_CORE_LIB_SEMA_TREETRANSFORM_H
...
protected:
  ...
  toolchain::DenseMap<Decl *, Decl *> TransformedLocalDecls;
...
#include "language/Core/Basic/AttrList.inc"
  ...
  Decl *TransformDecl(SourceLocation Loc, Decl *D) {
    toolchain::DenseMap<Decl *, Decl *>::iterator Known
  ...
  bool TransformRequiresExprRequirements(
      ...
      toolchain::SmallVectorImpl<concepts::Requirement *> &Transformed);
...
#include "toolchain/Frontend/OpenMP/OMP.inc"
  ...
  QualType RebuildArrayType(QualType ElementType, ArraySizeModifier SizeMod,
                            const toolchain::APInt *Size, Expr *SizeExpr,
  ...
  QualType RebuildDependentNameType(ElaboratedTypeKeyword Keyword,
    ...
    case LookupResultKind::FoundUnresolvedValue:
      toolchain_unreachable("Tag lookup cannot find non-tags");
  ...
  ExprResult RebuildCXXNamedCastExpr(SourceLocation OpLoc,
    ...
    default:
      toolchain_unreachable("Invalid C++ named cast");
    }
  }
  ...
  TemplateArgumentLoc RebuildPackExpansion(TemplateArgumentLoc Pattern,
    ...
    case TemplateArgument::NullPtr:
      toolchain_unreachable("Pack expansion pattern has no parameter packs");
  ...
  StmtResult RebuildOpenACCWaitConstruct(
    ...
    toolchain::append_range(Exprs, QueueIdExprs);
    return getSema().OpenACC().ActOnEndStmtDirective(
...
private:
  ...
  toolchain::SmallVector<OpenACCClause *>
...
TemplateName TreeTransform<Derived>::TransformTemplateName(
  ...
  toolchain_unreachable("overloaded function decl survived to here");
}
...
bool TreeTransform<Derived>::TransformTemplateArgument(
  ...
  case TemplateArgument::Null:
  case TemplateArgument::Pack:
    toolchain_unreachable("Unexpected TemplateArgument");
  ...
  case TemplateArgument::StructuralValue: {
    ...
    if (NewT == T && D == NewD)
      ...
      toolchain_unreachable("unexpected template argument kind");
  ...
  case TemplateArgument::TemplateExpansion:
    toolchain_unreachable("Caller should expand pack expansions");
...
bool TreeTransform<Derived>::TransformTemplateArguments(
    ...
    bool Uneval) {
  for (TemplateArgumentLoc In : toolchain::make_range(First, Last)) {
...
#include "language/Core/AST/TypeLocNodes.def"
  ...
  toolchain_unreachable("unhandled type loc!");
}
...
QualType TreeTransform<Derived>::TransformFunctionProtoType(
  ...
  if (auto NewExtParamInfos =
    ...
    if (!EPI.ExtParameterInfos ||
        toolchain::ArrayRef(EPI.ExtParameterInfos, TL.getNumParams()) !=
            toolchain::ArrayRef(NewExtParamInfos, ParamTypes.size())) {
      EPIChanged = true;
    }
  ...
  if (getDerived().AlwaysRebuild() || ResultType != T->getReturnType() ||
      T->getParamTypes() != toolchain::ArrayRef(ParamTypes) || EPIChanged) {
...
TreeTransform<Derived>::TransformPackIndexingType(TypeLocBuilder &TLB,
  ...
  toolchain::ArrayRef<QualType> Types = PIT->getExpansions();

  bool NotYetExpanded = Types.empty();
  bool FullySubstituted = true;

  if (Types.empty() && !PIT->expandsToEmptyPack())
    Types = toolchain::ArrayRef<QualType>(&Pattern, 1);
...
QualType TreeTransform<Derived>::TransformPredefinedSugarType(
  ...
  toolchain_unreachable("This type does not need to be transformed.");
}
...
QualType TreeTransform<Derived>::TransformAutoType(TypeLocBuilder &TLB,
  ...
  if (getDerived().AlwaysRebuild() || NewDeduced != OldDeduced ||
    ...
    toolchain::SmallVector<TemplateArgument, 4> NewArgList;
...
QualType TreeTransform<Derived>::TransformBTFTagAttributedType(
  ...
  toolchain_unreachable("Unexpected TreeTransform for BTFTagAttributedType");
}
...
QualType TreeTransform<Derived>::TransformDependentTemplateSpecializationType(
  ...
  TemplateArgumentListInfo NewTemplateArgs(TL.getLAngleLoc(),
                                           TL.getRAngleLoc());
  auto ArgsRange = toolchain::make_range<TemplateArgumentLocContainerIterator<
  ...
  if (getDerived().TransformTemplateArguments(ArgsRange.begin(),
    ...
    return QualType();
  bool TemplateArgumentsChanged = !toolchain::equal(
...
TreeTransform<Derived>::TransformObjCTypeParamType(TypeLocBuilder &TLB,
  ...
  if (getDerived().AlwaysRebuild() ||
    ...
    Result = getDerived().RebuildObjCTypeParamType(
        ...
        toolchain::ArrayRef(TL.getTypePtr()->qual_begin(), TL.getNumProtocols()),
...
TreeTransform<Derived>::TransformMSAsmStmt(MSAsmStmt *S) {
  ArrayRef<Token> AsmToks = toolchain::ArrayRef(S->getAsmToks(), S->getNumAsmToks());
...
StmtResult TreeTransform<Derived>::TransformOMPExecutableDirective(
  ...
  toolchain::SmallVector<OMPClause *, 16> TClauses;
...
OMPClause *TreeTransform<Derived>::TransformOMPInitClause(OMPInitClause *C) {
  ...
  for (Expr *E : toolchain::drop_begin(C->varlist())) {
...
OMPClause *TreeTransform<Derived>::TransformOMPUnifiedAddressClause(
  ...
  toolchain_unreachable("unified_address clause cannot appear in dependent context");
}
...
OMPClause *TreeTransform<Derived>::TransformOMPReverseOffloadClause(
  ...
  toolchain_unreachable("reverse_offload clause cannot appear in dependent context");
}
...
TreeTransform<Derived>::TransformOMPSelfMapsClause(OMPSelfMapsClause *C) {
  toolchain_unreachable("self_maps clause cannot appear in dependent context");
}
...
TreeTransform<Derived>::TransformOMPPrivateClause(OMPPrivateClause *C) {
  toolchain::SmallVector<Expr *, 16> Vars;
...
TreeTransform<Derived>::TransformOMPReductionClause(OMPReductionClause *C) {
  ...
  toolchain::SmallVector<Expr *, 16> UnresolvedReductions;
...
bool transformOMPMappableExprListClause(
    ...
    toolchain::SmallVectorImpl<Expr *> &Vars, CXXScopeSpec &MapperIdScopeSpec,
    DeclarationNameInfo &MapperIdInfo,
    toolchain::SmallVectorImpl<Expr *> &UnresolvedMappers) {
...
OMPClause *TreeTransform<Derived>::TransformOMPMapClause(OMPMapClause *C) {
  ...
  CXXScopeSpec MapperIdScopeSpec;
  DeclarationNameInfo MapperIdInfo;
  toolchain::SmallVector<Expr *, 16> UnresolvedMappers;
...
TreeTransform<Derived>::TransformOMPNumTeamsClause(OMPNumTeamsClause *C) {
  toolchain::SmallVector<Expr *, 3> Vars;
  Vars.reserve(C->varlist_size());
  for (auto *VE : C->varlist()) {
    ExprResult EVar = getDerived().TransformExpr(cast<Expr>(VE));
    if (EVar.isInvalid())
...
class OpenACCClauseTransform final
  ...
  toolchain::SmallVector<Expr *> VisitVarList(ArrayRef<Expr *> VarList) {
    toolchain::SmallVector<Expr *> InstantiatedVarList;
...
void OpenACCClauseTransform<Derived>::VisitNumGangsClause(
  ...
  toolchain::SmallVector<Expr *> InstantiatedIntExprs;
...
void OpenACCClauseTransform<Derived>::VisitPrivateClause(
  ...
  toolchain::SmallVector<Expr *> InstantiatedVarList;
  toolchain::SmallVector<VarDecl *> InitRecipes;

  for (const auto [RefExpr, InitRecipe] :
       toolchain::zip(C.getVarList(), C.getInitRecipes())) {
...
void OpenACCClauseTransform<Derived>::VisitFirstPrivateClause(
  ...
  toolchain::SmallVector<Expr *> InstantiatedVarList;
  toolchain::SmallVector<OpenACCFirstPrivateRecipe> InitRecipes;
...
void OpenACCClauseTransform<Derived>::VisitLinkClause(
  ...
  toolchain_unreachable("link clause not valid unless a decl transform");
}

template <typename Derived>
void OpenACCClauseTransform<Derived>::VisitDeviceResidentClause(
    const OpenACCDeviceResidentClause &C) {
  toolchain_unreachable("device_resident clause not valid unless a decl transform");
}
template <typename Derived>
void OpenACCClauseTransform<Derived>::VisitNoHostClause(
    const OpenACCNoHostClause &C) {
  toolchain_unreachable("nohost  clause not valid unless a decl transform");
}
template <typename Derived>
void OpenACCClauseTransform<Derived>::VisitBindClause(
    const OpenACCBindClause &C) {
  toolchain_unreachable("bind clause not valid unless a decl transform");
}
...
void OpenACCClauseTransform<Derived>::VisitAttachClause(
  ...
  toolchain::SmallVector<Expr *> VarList = VisitVarList(C.getVarList());

  // Ensure each var is a pointer type.
  toolchain::erase_if(VarList, [&](Expr *E) {
...
void OpenACCClauseTransform<Derived>::VisitWaitClause(
  ...
  if (C.hasExprs()) {
    ...
    toolchain::SmallVector<Expr *> InstantiatedQueueIdExprs;
...
void OpenACCClauseTransform<Derived>::VisitTileClause(
  ...
  toolchain::SmallVector<Expr *> TransformedExprs;
...
void OpenACCClauseTransform<Derived>::VisitGangClause(
  ...
  toolchain::SmallVector<OpenACCGangKind> TransformedGangKinds;
  toolchain::SmallVector<Expr *> TransformedIntExprs;
...
TreeTransform<Derived>::TransformOpenACCClauseList(
  ...
  toolchain::SmallVector<OpenACCClause *> TransformedClauses;
...
TreeTransform<Derived>::TransformOpenACCWaitConstruct(OpenACCWaitConstruct *C) {
  ...
  toolchain::SmallVector<Expr *> QueueIdExprs;
...
StmtResult TreeTransform<Derived>::TransformOpenACCCacheConstruct(
  ...
  toolchain::SmallVector<Expr *> TransformedVarList;
...
ExprResult TreeTransform<Derived>::TransformRecoveryExpr(RecoveryExpr *E) {
  toolchain::SmallVector<Expr *, 8> Children;
  bool Changed = false;
...
TreeTransform<Derived>::TransformDesignatedInitUpdateExpr(
  ...
  toolchain_unreachable("Unexpected DesignatedInitUpdateExpr in syntactic form of "
                   "initializer");
  return ExprError();
}
...
TreeTransform<Derived>::TransformNoInitExpr(
    NoInitExpr *E) {
  toolchain_unreachable("Unexpected NoInitExpr in syntactic form of initializer");
  return ExprError();
}
...
TreeTransform<Derived>::TransformArrayInitLoopExpr(ArrayInitLoopExpr *E) {
  toolchain_unreachable("Unexpected ArrayInitLoopExpr outside of initializer");
  return ExprError();
}
...
TreeTransform<Derived>::TransformArrayInitIndexExpr(ArrayInitIndexExpr *E) {
  toolchain_unreachable("Unexpected ArrayInitIndexExpr outside of initializer");
  return ExprError();
}
...
TreeTransform<Derived>::TransformCXXOperatorCallExpr(CXXOperatorCallExpr *E) {
  ...
  case OO_Array_Delete:
    toolchain_unreachable("new and delete operators cannot use CXXOperatorCallExpr");
...
#include "language/Core/Basic/OperatorKinds.def"
  ...
  case OO_Conditional:
    toolchain_unreachable("conditional operator is not actually overloadable");
...
TreeTransform<Derived>::TransformExprRequirement(concepts::ExprRequirement *Req) {
  toolchain::PointerUnion<Expr *, concepts::Requirement::SubstitutionDiagnostic *> TransExpr;
...
TreeTransform<Derived>::TransformLambdaExpr(LambdaExpr *E) {
  ...
  for (LambdaExpr::capture_iterator C = E->capture_begin(),
    ...
    if (E->isInitCapture(C)) {
      ...
      auto *OldVD = cast<VarDecl>(C->getCapturedVar());
      toolchain::SmallVector<Decl*, 4> NewVDs;
  ...
  C.Kind = language::Core::Sema::CodeSynthesisContext::LambdaExpressionSubstitution;
...
TreeTransform<Derived>::TransformCXXFoldExpr(CXXFoldExpr *E) {
  ...
  if (NumExpansions && SemaRef.getLangOpts().BracketDepth < *NumExpansions) {
    SemaRef.Diag(E->getEllipsisLoc(),
                 language::Core::diag::err_fold_expression_limit_exceeded)
...
QualType TreeTransform<Derived>::RebuildArrayType(
    QualType ElementType, ArraySizeModifier SizeMod, const toolchain::APInt *Size,
...
QualType TreeTransform<Derived>::RebuildExtVectorType(QualType ElementType,
                                                      unsigned NumElements,
                                                 SourceLocation AttributeLoc) {
  toolchain::APInt numElements(SemaRef.Context.getIntWidth(SemaRef.Context.IntTy),
                          NumElements, true);
...
QualType TreeTransform<Derived>::RebuildBitIntType(bool IsUnsigned,
                                                   unsigned NumBits,
                                                   SourceLocation Loc) {
  toolchain::APInt NumBitsAP(SemaRef.Context.getIntWidth(SemaRef.Context.IntTy),
                        NumBits, true);
...
TreeTransform<Derived>::TransformSYCLKernelCallStmt(SYCLKernelCallStmt *S) {
  ...
  toolchain_unreachable("SYCL kernel call statement cannot appear in dependent "
                   "context");
}
...
#endif // LANGUAGE_CORE_LIB_SEMA_TREETRANSFORM_H
