...
TypeResult SemaObjC::actOnObjCProtocolQualifierType(
  ...
  QualType Result = Context.getObjCObjectType(
      ...
      toolchain::ArrayRef((ObjCProtocolDecl *const *)protocols.data(),
                     protocols.size()),
      false);
...
bool SemaObjC::CheckObjCString(Expr *Arg) {
  ...
  if (Literal->containsNonAsciiOrNull()) {
    ...
    SmallVector<toolchain::UTF16, 128> ToBuf(NumBytes);
    ...
    toolchain::ConversionResult Result =
        toolchain::ConvertUTF8toUTF16(&FromPtr, FromPtr + NumBytes, &ToPtr,
...
void SemaObjC::handleXReturnsXRetainedAttr(Decl *D, const ParsedAttr &AL) {
  ...
  if (const auto *MD = dyn_cast<ObjCMethodDecl>(D)) {
    ...
    switch (AL.getKind()) {
    default:
      toolchain_unreachable("invalid ownership attribute");
    case ParsedAttr::AT_NSReturnsRetained:
    case ParsedAttr::AT_NSReturnsAutoreleased:
    case ParsedAttr::AT_NSReturnsNotRetained:
