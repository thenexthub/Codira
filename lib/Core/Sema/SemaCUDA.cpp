...
static bool hasAttr(const Decl *D, bool IgnoreImplicitAttr) {
  return D->hasAttrs() && toolchain::any_of(D->getAttrs(), [&](Attr *Attribute) {
...
SemaCUDA::IdentifyPreference(const FunctionDecl *Caller,
  ...
  toolchain_unreachable("All cases should've been handled by now.");
}
...
void SemaCUDA::EraseUnwantedMatches(
  ...
  CUDAFunctionPreference BestCFP =
      GetCFP(*toolchain::max_element(Matches, [&](const Pair &M1, const Pair &M2) {
        return GetCFP(M1) < GetCFP(M2);
      }));

  // Erase all functions with lower priority.
  toolchain::erase_if(Matches,
...
bool SemaCUDA::inferTargetForImplicitSpecialMember(CXXRecordDecl *ClassDecl,
  ...
  toolchain::SmallVector<const CXXBaseSpecifier *, 16> Bases;
  ...
  if (!ClassDecl->isAbstract()) {
    toolchain::append_range(Bases, toolchain::make_pointer_range(ClassDecl->vbases()));
  }
...
bool SemaCUDA::isEmptyConstructor(SourceLocation Loc, CXXConstructorDecl *CD) {
  ...
  if (!toolchain::all_of(CD->inits(), [&](const CXXCtorInitializer *CI) {
...
bool SemaCUDA::isEmptyDestructor(SourceLocation Loc, CXXDestructorDecl *DD) {
  ...
  if (!toolchain::all_of(ClassDecl->bases(), [&](const CXXBaseSpecifier &BS) {
  ...
  // ... and member fields.
  if (!toolchain::all_of(ClassDecl->fields(), [&](const FieldDecl *Field) {
...
void SemaCUDA::maybeAddHostDeviceAttrs(FunctionDecl *NewD,
  ...
  auto It = toolchain::find_if(Previous, IsMatchingDeviceFn);
  if (It != Previous.end()) {
...
std::string SemaCUDA::getConfigureFuncName() const {
  if (getLangOpts().OffloadViaLLVM)
    return "__toolchainPushCallConfiguration";
