...
struct EffectiveContext {
  ...
  bool includesClass(const CXXRecordDecl *R) const {
    ...
    return toolchain::is_contained(Records, R);
  }
...
static AccessResult HasAccess(Sema &S,
  ...
  if (Access == AS_protected && Target.isInstanceMember()) {
    ...
    toolchain_unreachable("impossible friendship kind");
  }
...
static void DiagnoseAccessPath(Sema &S,
  ...
  if (entity.isMemberAccess()) {
    ...
    case AR_dependent:
      toolchain_unreachable("cannot diagnose dependent access");
    }
  }
...
static AccessResult CheckEffectiveAccess(Sema &S,
  ...
  toolchain_unreachable("invalid access result");
}
...
bool Sema::isMemberAccessibleForDeletion(CXXRecordDecl *NamingClass,
  ...
  case AR_dependent: toolchain_unreachable("dependent for =delete computation");
  case AR_delayed: toolchain_unreachable("cannot delay =delete computation");
  }
  toolchain_unreachable("bad access result");
}
...
Sema::AccessResult Sema::CheckBaseClassAccess(
    ...
    toolchain::function_ref<void(PartialDiagnostic &)> SetupPDiag, bool ForceCheck,
  ...
  if (ForceUnprivileged) {
    ...
    toolchain_unreachable("unexpected result from CheckEffectiveAccess");
  }
