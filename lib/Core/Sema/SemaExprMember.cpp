...
typedef toolchain::SmallPtrSet<const CXXRecordDecl*, 4> BaseSet;
...
ExprResult Sema::BuildPossibleImplicitMemberExpr(
  ...
  toolchain_unreachable("unexpected instance member access kind");
}
...
Sema::ActOnDependentMemberExpr(Expr *BaseExpr, QualType BaseType,
  ...
  assert(BaseType->isDependentType() || NameInfo.getName().isDependentName() ||
         ...
         (TemplateArgs && toolchain::any_of(TemplateArgs->arguments(),
