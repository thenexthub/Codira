...
SourceRange
language::Core::getTemplateParamsRange(TemplateParameterList const * const *Ps,
                              unsigned N) {
...
bool Sema::LookupTemplateName(LookupResult &Found, Scope *S, CXXScopeSpec &SS,
  ...
  if (ATK && SS.isEmpty() && ObjectType.isNull() &&
    ...
    bool AllFunctions =
        getLangOpts().CPlusPlus20 && toolchain::all_of(Found, [](NamedDecl *ND) {
...
void Sema::diagnoseExprIntendedAsTemplateName(Scope *S, ExprResult TemplateName,
  ...
  if (auto *DRE = dyn_cast<DeclRefExpr>(TemplateName.get())) {
    ...
    toolchain_unreachable("unexpected kind of potential template name");
  }
...
static TemplateArgumentLoc translateTemplateArgument(Sema &SemaRef,
  ...
  toolchain_unreachable("Unhandled parsed template argument");
}
...
public:
  ...
  Decl *TransformDecl(SourceLocation Loc, Decl *D) {
    ...
    if (auto *TD = dyn_cast<TypedefNameDecl>(D))
      ...
      toolchain_unreachable("Don't know how to handle this declaration type yet");
    return D;
  }
};
} // namespace
...
Sema::ActOnTemplateParameterList(unsigned Depth,
  ...
  return TemplateParameterList::Create(Context, TemplateLoc, LAngleLoc,
                                       toolchain::ArrayRef(Params), RAngleLoc,
                                       RequiresClause);
}
...
DeclResult Sema::CheckClassTemplate(
  ...
  if (NumOuterTemplateParamLists > 0)
    NewClass->setTemplateParameterListsInfo(
        Context,
        toolchain::ArrayRef(OuterTemplateParamLists, NumOuterTemplateParamLists));
...
static bool DiagnoseDefaultTemplateArgument(Sema &S,
  ...
  toolchain_unreachable("Invalid TemplateParamListContext!");
}
...
static QualType builtinCommonTypeImpl(Sema &S, ElaboratedTypeKeyword Keyword,
  ...
  default: {
    ...
    for (auto T : toolchain::drop_begin(Ts)) {
...
static SpirvOperand checkHLSLSpirvTypeOperand(Sema &SemaRef,
  ...
  if (auto *RT = OperandArg->getAs<RecordType>()) {
    ...
    if (RT && isInVkNamespace(RT) &&
      ...
      toolchain::APInt Value = ConstantArgs[1].getAsIntegral();
...
static QualType checkBuiltinTemplateIdType(
  ...
  case BTK__make_integer_seq: {
    ...
    if (toolchain::APSInt NumArgs = NumArgsArg.getAsIntegral(); NumArgs >= 0) {
      // Expand N into 0 ... N-1.
      for (toolchain::APSInt I(NumArgs.getBitWidth(), NumArgs.isUnsigned());
           I < NumArgs; ++I) {
  ...
  case BTK__type_pack_element: {
    ...
    toolchain::APSInt Index = IndexArg.getAsIntegral();
  ...
  case BTK__builtin_common_type: {
    ...
    if (toolchain::any_of(Converted, [](auto &C) { return C.isDependent(); }))
      return QualType();
  ...
  case BTK__hlsl_spirv_type: {
    ...
    toolchain::SmallVector<SpirvOperand> Operands;
  ...
  toolchain_unreachable("unexpected BuiltinTemplateDecl!");
}
...
Sema::findFailedBooleanCondition(Expr *Cond) {
  ...
  {
    toolchain::raw_string_ostream Out(Description);
...
QualType Sema::CheckTemplateIdType(ElaboratedTypeKeyword Keyword,
  ...
  if (isa<TemplateTemplateParmDecl>(Template)) {
    ...
    toolchain_unreachable("Unhandled template kind");
  }
...
static bool isTemplateArgumentTemplateParameter(const TemplateArgument &Arg,
  ...
  toolchain_unreachable("unexpected kind of template argument");
}
...
noteNonDeducibleParameters(Sema &S, TemplateParameterList *TemplateParams,
                           const toolchain::SmallBitVector &DeducibleParams) {
...
static void checkTemplatePartialSpecialization(Sema &S,
  ...
  toolchain::SmallBitVector DeducibleParams(TemplateParams->size());
  ...
  if (Template->getDeclContext()->isDependentContext() ||
    ...
    for (VarTemplatePartialSpecializationDecl *Partial : PartialSpecs)
      if (ParsingInitForAutoVars.count(Partial) &&
          toolchain::equal(CTAI.CanonicalConverted,
...
bool Sema::CheckTemplateArgument(NamedDecl *Param, TemplateArgumentLoc &ArgLoc,
  ...
  if (NonTypeTemplateParmDecl *NTTP =dyn_cast<NonTypeTemplateParmDecl>(Param)) {
    ...
    switch (Arg.getKind()) {
    case TemplateArgument::Null:
      toolchain_unreachable("Should never see a NULL template argument here");
    ...
    case TemplateArgument::Pack:
      toolchain_unreachable("Caller must expand template argument packs");
    }
  ...
  case TemplateArgument::StructuralValue:
  case TemplateArgument::NullPtr:
    toolchain_unreachable("non-type argument with template template parameter");
...
static bool diagnoseMissingArgument(Sema &S, SourceLocation Loc,
  ...
  toolchain::SmallVector<Module*, 8> Modules;
      ...
      if (ArgIsExpansion && CTAI.MatchingTTP) {
        ...
        for (TemplateParameterList::iterator First = Param; Param != ParamEnd;
          ...
          CTAI.CanonicalConverted.back().setIsDefaulted(
              language::Core::isSubstitutedDefaultArgument(Context, Arg, *Param,
                                                  CTAI.CanonicalConverted,
                                                  Params->getDepth()));
        }
        ArgLoc =
        ...
        if (CheckTemplateArgument(*Param, ArgLoc, Template, TemplateLoc,
          ...
          return true;
        CTAI.CanonicalConverted.back().setIsDefaulted(
            language::Core::isSubstitutedDefaultArgument(Context, ArgLoc.getArgument(),
...
bool UnnamedLocalNoLinkageFinder::VisitNestedNameSpecifier(
  ...
  toolchain_unreachable("Invalid NestedNameSpecifier::Kind!");
}
...
ExprResult Sema::CheckTemplateArgument(NamedDecl *Param, QualType ParamType,
  ...
  if (ParamType->isIntegralOrEnumerationType()) {
    ...
    if (ParamType->isBooleanType()) {
      ...
      toolchain::APSInt OldValue = Value;
...
bool Sema::CheckDeclCompatibleWithTemplateTemplate(
  ...
  if (auto *TTP = toolchain::dyn_cast<TemplateTemplateParmDecl>(Template)) {
...
static Expr *BuildExpressionFromIntegralTemplateArgumentValue(
    Sema &S, QualType OrigT, const toolchain::APSInt &Int, SourceLocation Loc) {
...
static Expr *BuildExpressionFromNonTypeTemplateArgumentValue(
  ...
  case APValue::Union:
    ...
    toolchain_unreachable("unexpected template argument value");
  ...
  case APValue::Vector: {
    ...
    toolchain::SmallVector<Expr *, 8> Elts;
  ...
  case APValue::Indeterminate:
    toolchain_unreachable("Unexpected APValue kind.");
  case APValue::LValue:
...
Sema::BuildExpressionFromNonTypeTemplateArgument(const TemplateArgument &Arg,
  ...
  case TemplateArgument::TemplateExpansion:
  case TemplateArgument::Pack:
    toolchain_unreachable("not a non-type template argument");
  ...
  toolchain_unreachable("Unhandled TemplateArgument::ArgKind enum");
}
...
static bool MatchTemplateParameterKind(
  ...
  if (Kind != Sema::TPL_TemplateParamsEquivalent &&
    ...
    if (isa<TemplateTypeParmDecl>(New)) {
      ...
      toolchain_unreachable("unexpected template parameter type");
...
bool Sema::CheckConceptUseInDefinition(NamedDecl *Concept, SourceLocation Loc) {
  if (auto *CE = toolchain::dyn_cast<ConceptDecl>(Concept);
...
Sema::CheckSpecializationInstantiationRedecl(SourceLocation NewLoc,
  ...
  case TSK_ExplicitSpecialization:
    ...
    toolchain_unreachable("The switch over PrevTSK must be exhaustive.");
  ...
  case TSK_ExplicitInstantiationDeclaration:
    ...
    toolchain_unreachable("Unexpected TemplateSpecializationKind!");
  ...
  toolchain_unreachable("Missing specialization/instantiation case?");
}
...
bool Sema::CheckFunctionTemplateSpecialization(
  ...
  toolchain::SmallDenseMap<FunctionDecl *, TemplateArgumentListInfo, 8>
...
Sema::CheckMemberSpecialization(NamedDecl *Member, LookupResult &Previous) {
  ...
  if (auto *MemberFunction = dyn_cast<FunctionDecl>(Member)) {
    ...
    toolchain_unreachable("unknown member specialization kind");
  }
