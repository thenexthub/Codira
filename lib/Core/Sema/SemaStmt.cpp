...
void DiagnoseUnused(Sema &S, const Expr *E, std::optional<unsigned> DiagID) {
  ...
  S.DiagIfReachable(Loc, toolchain::ArrayRef<const Stmt *>(E),
...
namespace {
  struct CaseCompareFunctor {
    bool operator()(const std::pair<toolchain::APSInt, CaseStmt*> &LHS,
                    const toolchain::APSInt &RHS) {
      return LHS.first < RHS;
    }
    bool operator()(const std::pair<toolchain::APSInt, CaseStmt*> &LHS,
                    const std::pair<toolchain::APSInt, CaseStmt*> &RHS) {
      return LHS.first < RHS.first;
    }
    bool operator()(const toolchain::APSInt &LHS,
                    const std::pair<toolchain::APSInt, CaseStmt*> &RHS) {
...
///
static bool CmpCaseVals(const std::pair<toolchain::APSInt, CaseStmt*>& lhs,
                        const std::pair<toolchain::APSInt, CaseStmt*>& rhs) {
...
///
static bool CmpEnumVals(const std::pair<toolchain::APSInt, EnumConstantDecl*>& lhs,
                        const std::pair<toolchain::APSInt, EnumConstantDecl*>& rhs)
{
  return lhs.first < rhs.first;
}
...
///
static bool EqEnumVals(const std::pair<toolchain::APSInt, EnumConstantDecl*>& lhs,
                       const std::pair<toolchain::APSInt, EnumConstantDecl*>& rhs)
{
  return lhs.first == rhs.first;
}
...
static void AdjustAPSInt(toolchain::APSInt &Val, unsigned BitWidth, bool IsSigned) {
...
/// type.
static void checkCaseValue(Sema &S, SourceLocation Loc, const toolchain::APSInt &Val,
...
typedef SmallVector<std::pair<toolchain::APSInt, EnumConstantDecl*>, 64> EnumValsTy;
...
Sema::ActOnFinishSwitchStmt(SourceLocation SwitchLoc, Stmt *Switch,
  ...
  typedef SmallVector<std::pair<toolchain::APSInt, CaseStmt*>, 64> CaseValsTy;
  CaseValsTy CaseVals;

  // Keep track of any GNU case ranges we see.  The APSInt is the low value.
  typedef std::vector<std::pair<toolchain::APSInt, CaseStmt*> > CaseRangesTy;
  ...
  for (SwitchCase *SC = SS->getSwitchCaseList(); SC && !HasDependentValue;
    ...
    if (DefaultStmt *DS = dyn_cast<DefaultStmt>(SC)) {
      ...
      toolchain::APSInt LoVal = LoBeforePromotion->EvaluateKnownConstInt(Context);
  ...
  if (!HasDependentValue) {
    ...
    toolchain::stable_sort(CaseVals, CmpCaseVals);
    ...
    if (!CaseRanges.empty()) {
      ...
      toolchain::stable_sort(CaseRanges);

      // Scan the ranges, computing the high values and removing empty ranges.
      std::vector<toolchain::APSInt> HiVals;
      for (unsigned i = 0, e = CaseRanges.size(); i != e; ++i) {
        toolchain::APSInt &LoVal = CaseRanges[i].first;
        CaseStmt *CR = CaseRanges[i].second;
        Expr *Hi = CR->getRHS();

        const Expr *HiBeforePromotion = Hi;
        GetTypeBeforeIntegralPromotion(HiBeforePromotion);
        toolchain::APSInt HiVal = HiBeforePromotion->EvaluateKnownConstInt(Context);
      ...
      for (unsigned i = 0, e = CaseRanges.size(); i != e; ++i) {
        toolchain::APSInt &CRLo = CaseRanges[i].first;
        toolchain::APSInt &CRHi = HiVals[i];
        CaseStmt *CR = CaseRanges[i].second;

        // Check to see whether the case range overlaps with any
        // singleton cases.
        CaseStmt *OverlapStmt = nullptr;
        toolchain::APSInt OverlapVal(32);

        // Find the smallest value >= the lower bound.  If I is in the
        // case range, then we have overlap.
        CaseValsTy::iterator I =
            toolchain::lower_bound(CaseVals, CRLo, CaseCompareFunctor());
    ...
    if (!CaseListIsErroneous && !CaseListIsIncomplete && !HasConstantCond &&
      ...
      for (auto *EDI : ED->enumerators()) {
        toolchain::APSInt Val = EDI->getInitVal();
        AdjustAPSInt(Val, CondWidth, CondIsSigned);
        EnumVals.push_back(std::make_pair(Val, EDI));
      }
      toolchain::stable_sort(EnumVals, CmpEnumVals);
      auto EI = EnumVals.begin(), EIEnd = toolchain::unique(EnumVals, EqEnumVals);
...
Sema::DiagnoseAssignmentEnum(QualType DstType, QualType SrcType,
  ...
  std::optional<toolchain::APSInt> RHSVal = SrcExpr->getIntegerConstantExpr(Context);
  if (!RHSVal)
    return;
  ...
  typedef SmallVector<std::pair<toolchain::APSInt, EnumConstantDecl *>, 64>
      EnumValsTy;
  EnumValsTy EnumVals;
  ...
  if (EnumVals.empty())
    return;
  toolchain::stable_sort(EnumVals, CmpEnumVals);
  EnumValsTy::iterator EIend = toolchain::unique(EnumVals, EqEnumVals);
...
namespace {
  ...
  using DeclSetVector = toolchain::SmallSetVector<VarDecl *, 8>;
  ...
  if (CXXRecordDecl *D = RangeType->getAsCXXRecordDecl()) {
    ...
    if (BeginMemberLookup.empty() != EndMemberLookup.empty()) {
      ...
      auto BuildNonmember = [&](
          ...
          toolchain::function_ref<Sema::ForRangeStatus()> BuildFound,
          toolchain::function_ref<Sema::ForRangeStatus()> BuildNotFound) {
        ...
        toolchain_unreachable("unexpected ForRangeStatus");
      };
  ...
  if (RangeVarType->isDependentType()) {
    ...
    if (const ArrayType *UnqAT = RangeType->getAsArrayTypeUnsafe()) {
      ...
      if (const ConstantArrayType *CAT = dyn_cast<ConstantArrayType>(UnqAT))
        ...
        // codegen that then knows to simply use that initial toolchain::Value (that
        ...
        toolchain_unreachable("Unexpected array type in for-range");
      }
...
/// language::Core::trivial_abi attribute.
...
VerifyInitializationSequenceCXX98(const Sema &S,
  ...
  const auto *Step = toolchain::find_if(Seq.steps(), [](const auto &Step) {
...
class CatchHandlerType {
  ...
  friend struct toolchain::DenseMapInfo<CatchHandlerType>;
...
class CatchTypePublicBases {
  const toolchain::DenseMap<QualType, CXXCatchStmt *> &TypesToCheck;
...
public:
  CatchTypePublicBases(const toolchain::DenseMap<QualType, CXXCatchStmt *> &T,
                       QualType QT)
...
StmtResult Sema::ActOnCXXTryBlock(SourceLocation TryLoc, Stmt *TryBlock,
  ...
  toolchain::DenseMap<QualType, CXXCatchStmt *> HandledBaseTypes;
  toolchain::DenseMap<CatchHandlerType, CXXCatchStmt *> HandledTypes;
