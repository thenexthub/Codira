...
bool Sema::isLibstdcxxEagerExceptionSpecHack(const Declarator &D) {
  ...
  return toolchain::StringSwitch<bool>(RD->getIdentifier()->getName())
      .Case("array", true)
      .Case("pair", IsInStd)
...
Sema::ResolveExceptionSpec(SourceLocation Loc, const FunctionProtoType *FPT) {
  ...
  const FunctionProtoType *Proto =
    SourceDecl->getType()->castAs<FunctionProtoType>();
  if (Proto->getExceptionSpecType() == language::Core::EST_Unparsed) {
    Diag(Loc, diag::err_exception_spec_not_parsed);
    Proto = nullptr;
  }
  return Proto;
...
bool Sema::CheckEquivalentExceptionSpec(FunctionDecl *Old, FunctionDecl *New) {
  ...
  toolchain::raw_svector_ostream OS(ExceptionSpecString);
  ...
  case EST_Unparsed:
    toolchain_unreachable("This spec type is compatible with none.");
  }
...
static bool CheckEquivalentExceptionSpecImpl(
  ...
  if (OldEST == EST_DependentNoexcept && NewEST == EST_DependentNoexcept) {
    toolchain::FoldingSetNodeID OldFSN, NewFSN;
  ...
  if (OldEST == EST_Dynamic && NewEST == EST_Dynamic) {
    ...
    // to the second.
    toolchain::SmallPtrSet<CanQualType, 8> OldTypes, NewTypes;
...
bool Sema::handlerCanCatch(QualType HandlerType, QualType ExceptionType) {
  ...
  case AR_dependent:
    toolchain_unreachable("access check dependent for unprivileged context");
  case AR_delayed:
    toolchain_unreachable("access check delayed in non-declaration");
  }
  toolchain_unreachable("unexpected access check result");
}
...
static CanThrowResult canDynamicCastThrow(const CXXDynamicCastExpr *DC) {
  ...
  return DC->getCastKind() == language::Core::CK_Dynamic? CT_Can : CT_Cannot;
}
...
CanThrowResult Sema::canThrow(const Stmt *S) {
  ...
  case Expr::MSPropertySubscriptExprClass:
    toolchain_unreachable("Invalid class for expression");
  ...
  case Stmt::NoStmtClass:
    toolchain_unreachable("Invalid class for statement");
  }
  toolchain_unreachable("Bogus StmtClass");
}
