...
static inline UnsignedOrNone getStackIndexOfNearestEnclosingCaptureReadyLambda(
    ArrayRef<const language::Core::sema::FunctionScopeInfo *> FunctionScopes,
  ...
  while (CurScopeIndex > 0 && isa<language::Core::sema::CapturedRegionScopeInfo>(
                                  FunctionScopes[CurScopeIndex]))
    --CurScopeIndex;
  assert(
      isa<language::Core::sema::LambdaScopeInfo>(FunctionScopes[CurScopeIndex]) &&
  ...
  do {
    const language::Core::sema::LambdaScopeInfo *LSI =
...
UnsignedOrNone language::Core::getStackIndexOfNearestEnclosingCaptureCapableLambda(
...
static QualType
buildTypeForLambdaCallOperator(Sema &S, language::Core::CXXRecordDecl *Class,
...
void Sema::ActOnLambdaExpressionAfterIntroducer(LambdaIntroducer &Intro,
  ...
  if (CurScope->getTemplateParamParent() != nullptr) {
    ...
    if (P->isFunctionDeclarationScope() &&
        toolchain::any_of(P->decls(), [](Decl *D) {
  ...
  toolchain::DenseMap<IdentifierInfo *, ValueDecl *> CaptureNames;
  ...
  for (auto C = Intro.Captures.begin(), E = Intro.Captures.end(); C != E;
    ...
    if (C->Init.isUsable()) {
      ...
      case LambdaCaptureInitKind::NoInit:
        toolchain_unreachable("not an init-capture?");
...
static void repeatForLambdaConversionFunctionCallingConvs(
  ...
  if (S.getLangOpts().MSVCCompat) {
    ...
    toolchain::sort(Convs);
    toolchain::iterator_range<CallingConv *> Range(std::begin(Convs),
                                              toolchain::unique(Convs));
...
static void addFunctionPointerConversion(Sema &S, SourceRange IntroducerRange,
  ...
  if (toolchain::any_of(CallOperator->parameters(), HasPassObjectSizeAttr))
    return;
...
mapImplicitCaptureStyle(CapturingScopeInfo::ImplicitCaptureStyle ICS) {
  ...
  case CapturingScopeInfo::ImpCap_Block:
    toolchain_unreachable("block capture in lambda");
  }
  toolchain_unreachable("Unknown implicit capture style");
}
...
bool Sema::addInstantiatedCapturesToScope(
  ...
  if (LambdaPattern->capture_size() && !LambdaClass->capture_size()) {
    for (FunctionScopeInfo *Scope : toolchain::reverse(FunctionScopes)) {
...
Sema::LambdaScopeForCallOperatorInstantiationRAII::
  ...
  toolchain::SmallVector<std::pair<FunctionDecl *, FunctionDecl *>, 4>
  ...
  for (auto [FDPattern, FD] : toolchain::reverse(InstantiationAndPatterns)) {
