...
bool SemaX86::CheckBuiltinFunctionCall(const TargetInfo &TI, unsigned BuiltinID,
  ...
  if (TT.getArch() != toolchain::Triple::x86 && isX86_32Builtin(BuiltinID))
...
void SemaX86::handleAnyInterruptAttr(Decl *D, const ParsedAttr &AL) {
  ...
  unsigned TypeSize =
      Context.getTargetInfo().getTriple().getArch() == toolchain::Triple::x86_64
          ? 64
          : 32;
  if (NumParams == 2 &&
...
bool SemaX86::checkTargetClonesAttr(
  ...
  for (unsigned I = 0, E = Params.size(); I < E; ++I) {
    ...
    do {
      ...
      if (toolchain::is_contained(NewParams, LHS))
