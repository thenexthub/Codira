...
#include "toolchain/MC/MCSectionMachO.h"
...
static bool checkPositiveIntArgument(Sema &S, const AttrInfo &AI, const Expr *Expr,
  ...
  if (UVal > (uint32_t)std::numeric_limits<int>::max()) {
    toolchain::APSInt I(32); // for toString
    I = UVal;
  ...
  for (unsigned Idx = Sidx; Idx < AL.getNumArgs(); ++Idx) {
    ...
    if(!RT && ParamIdxOk) {
      ...
      if(FD && IL) {
        ...
        toolchain::APInt ArgValue = IL->getValue();
...
#endif
  toolchain::SmallPtrSet<const ParmVarDecl *, 16> Parms;
  bool Result;
...
static void handleNoBuiltinAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  toolchain::SmallVector<StringRef, 16> Names;
  ...
  Names.erase(toolchain::unique(Names), Names.end());
...
static void handleNonNullAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  toolchain::array_pod_sort(Start, Start + Size);
...
void Sema::AddAssumeAlignedAttr(Decl *D, const AttributeCommonInfo &CI, Expr *E,
  ...
  if (!E->isValueDependent()) {
    std::optional<toolchain::APSInt> I = toolchain::APSInt(64);
...
static void handleOwnershipAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned i = 1; i < AL.getNumArgs(); ++i) {
    ...
    for (const auto *I : D->specific_attrs<OwnershipAttr>()) {
      ...
      // index.
      if (I->getOwnKind() != K && toolchain::is_contained(I->args(), Idx)) {
        ...
        // a different index.
        if (!toolchain::is_contained(I->args(), Idx)) {
...
// https://github.com/toolchain/toolchain-project/issues/88825). This should handle the
// majority of use cases while leaving namespace scope names unmarked.
static void markUsedForAliasOrIfunc(Sema &S, Decl *D, const ParsedAttr &AL,
                                    StringRef Str) {
  std::unique_ptr<char, toolchain::FreeDeleter> Demangled;
  if (S.getASTContext().getCXXABIKind() != TargetCXXABI::Microsoft)
    Demangled.reset(toolchain::itaniumDemangle(Str, /*ParseParams=*/false));
...
static void handleCPUSpecificAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned ArgNo = 0; ArgNo < getNumAttributeArgs(AL); ++ArgNo) {
    ...
    if (toolchain::any_of(CPUs, [CPUName, &Target](const IdentifierInfo *Cur) {
...
static void handleNakedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (AL.isDeclspecAttribute()) {
    ...
    if (Arch != toolchain::Triple::x86 &&
        (Arch != toolchain::Triple::arm && Arch != toolchain::Triple::thumb)) {
...
void language::Core::inferNoReturnAttr(Sema &S, const Decl *D) {
...
static void handleDependencyAttr(Sema &S, Scope *Scope, Decl *D,
  ...
  if (isa<ParmVarDecl>(D)) {
    ...
    if (!(Scope->getFlags() & language::Core::Scope::FunctionDeclarationScope)) {
      S.Diag(AL.getLoc(),
...
static void handleAvailabilityAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  const toolchain::Triple::OSType PlatformOS = AvailabilityAttr::getOSType(
  ...
  auto reportAndUpdateIfInvalidOS = [&](auto &InputVersion) -> void {
    const bool IsInValidRange =
        toolchain::Triple::isValidVersionForOS(PlatformOS, InputVersion);
    // Canonicalize availability versions.
    auto CanonicalVersion = toolchain::Triple::getCanonicalVersionForOS(
  ...
  if (PlatformOS != toolchain::Triple::OSType::UnknownOS) {
  ...
  if (EnvironmentLoc) {
    if (S.getLangOpts().HLSL) {
      ...
      if (AvailabilityAttr::getEnvironmentType(
          ...
          toolchain::Triple::EnvironmentType::UnknownEnvironment)
...
static void handleSentinelAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (AL.getNumArgs() > 0) {
    Expr *E = AL.getArgAsExpr(0);
    std::optional<toolchain::APSInt> Idx = toolchain::APSInt(32);
...
static void handleWarnUnusedResult(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (AL.isStandardAttributeSyntax()) {
    // If this is spelled [[language::Core::warn_unused_result]] we look for an optional
...
static void handleWorkGroupSize(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  auto IsZero = [&](Expr *E) {
    if (E->isValueDependent())
      return false;
    std::optional<toolchain::APSInt> I = E->getIntegerConstantExpr(S.Context);
    assert(I && "Non-integer constant expr");
    return I->isZero();
  };

  if (!toolchain::all_of(WGSize, IsZero)) {
    for (unsigned i = 0; i < 3; ++i) {
  ...
  auto Equal = [&](Expr *LHS, Expr *RHS) {
    if (LHS->isValueDependent() || RHS->isValueDependent())
      return true;
    std::optional<toolchain::APSInt> L = LHS->getIntegerConstantExpr(S.Context);
    assert(L && "Non-integer constant expr");
    std::optional<toolchain::APSInt> R = RHS->getIntegerConstantExpr(S.Context);
    assert(L && "Non-integer constant expr");
    return L == R;
  };
  ...
  if (Existing &&
      !toolchain::equal(std::initializer_list<Expr *>{Existing->getXDim(),
                                                 Existing->getYDim(),
                                                 Existing->getZDim()},
                   WGSize, Equal))
...
toolchain::Error Sema::isValidSectionSpecifier(StringRef SecName) {
  ...
  bool HasTAA;
  return toolchain::MCSectionMachO::ParseSectionSpecifier(SecName, Segment, Section,
                                                     TAA, HasTAA, StubSize);
}

bool Sema::checkSectionName(SourceLocation LiteralLoc, StringRef SecName) {
  if (toolchain::Error E = isValidSectionSpecifier(SecName)) {
...
static bool isValidCodeModelAttr(toolchain::Triple &Triple, StringRef Str) {
  if (Triple.isLoongArch()) {
    return Str == "normal" || Str == "medium" || Str == "extreme";
  } else {
    assert(Triple.getArch() == toolchain::Triple::x86_64 &&
...
static void handleCodeModelAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  StringRef Str;
  SourceLocation LiteralLoc;
  auto IsTripleSupported = [](toolchain::Triple &Triple) {
    return Triple.getArch() == toolchain::Triple::ArchType::x86_64 ||
           Triple.isLoongArch();
  };
  ...
  SmallVector<toolchain::Triple, 2> Triples = {
  ...
  auto SupportedTripleIt = toolchain::find_if(Triples, IsTripleSupported);
  if (SupportedTripleIt == Triples.end()) {
    S.Diag(LiteralLoc, diag::warn_unknown_attribute_ignored) << AL;
    return;
  }

  toolchain::CodeModel::Model CM;
...
static bool checkCodeSegName(Sema &S, SourceLocation LiteralLoc,
  ...
  if (toolchain::Error E = S.isValidSectionSpecifier(CodeSegName)) {
...
static FormatAttrKind getFormatAttrKind(StringRef Format) {
  return toolchain::StringSwitch<FormatAttrKind>(Format)
...
static void handleCallbackAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  toolchain::StringMap<int> NameIdxMapping;
  NameIdxMapping["__"] = -1;
  ...
  for (unsigned I = 0, E = AL.getNumArgs(); I < E; ++I) {
    ...
    if (AL.isArgIdent(I)) {
      ...
      toolchain_unreachable("Unexpected ParsedAttr argument type!");
    }
...
void Sema::AddAlignValueAttr(Decl *D, const AttributeCommonInfo &CI, Expr *E) {
  ...
  if (const auto *TD = dyn_cast<TypedefNameDecl>(D))
    ...
    toolchain_unreachable("Unknown decl type for align_value");
...
void Sema::AddAlignedAttr(Decl *D, const AttributeCommonInfo &CI, Expr *E,
  ...
  if (!(TmpAttr.isAlignas() && !Alignment)) {
    if (!toolchain::isPowerOf2_64(AlignVal)) {
      Diag(AttrLoc, diag::err_alignment_not_power_of_two)
        << E->getSourceRange();
      return;
    }
...
void Sema::AddModeAttr(Decl *D, const AttributeCommonInfo &CI,
  ...
  toolchain::APInt VectorSize(64, 0);
...
static void handleCallConvAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  case ParsedAttr::AT_Pcs: {
    ...
    case CC_AAPCS_VFP:
      ...
      break;
    default:
      toolchain_unreachable("unexpected calling convention in pcs attribute");
    }
  ...
  case ParsedAttr::AT_RISCVVLSCC: {
    ...
    if (!toolchain::isPowerOf2_64(VectorLength)) {
...
bool Sema::CheckCallingConvAttr(const ParsedAttr &Attrs, CallingConv &CC,
  ...
  default: toolchain_unreachable("unexpected attribute kind");
  }
  ...
  if (LangOpts.CUDA) {
    ...
    case CUDAFunctionTarget::InvalidTarget:
      toolchain_unreachable("unexpected cuda target");
    }
...
static OffloadArch getOffloadArch(const TargetInfo &TI) {
  if (!TI.getTriple().isNVPTX())
    toolchain_unreachable("getOffloadArch is only valid for NVPTX triple");
...
static void handlePatchableFunctionEntryAttr(Sema &S, Decl *D,
  ...
  if (AL.getNumArgs() == 3) {
    ...
    if (!S.checkStringLiteralArgumentAttr(AL, 2, Section, &LiteralLoc))
      return;
    if (toolchain::Error E = S.isValidSectionSpecifier(Section)) {
      S.Diag(LiteralLoc,
...
static void handleAbiTagAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  toolchain::sort(Tags);
  Tags.erase(toolchain::unique(Tags), Tags.end());
...
static void handleNoSanitizeSpecificAttr(Sema &S, Decl *D,
  ...
  StringRef SanitizerName = toolchain::StringSwitch<StringRef>(AttrName)
...
static void handleCountedByAttrField(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  case ParsedAttr::AT_SizedByOrNull:
    ...
    break;
  default:
    toolchain_unreachable("unexpected counted_by family attribute");
  }
...
static const AttrTy *findEnforceTCBAttrByName(Decl *D, StringRef Name) {
  ...
  auto I = toolchain::find_if(Attrs,
                         [Name](const AttrTy *A) {
