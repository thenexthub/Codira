...
static bool tryDiagnoseOverloadedCast(Sema &S, CastType CT,
  ...
  case OR_Success: toolchain_unreachable("successful failed overload");
...
static TryCastResult getCastAwayConstnessCastKind(CastAwayConstnessKind CACK,
  ...
  case CastAwayConstnessKind::CACK_None:
    toolchain_unreachable("did not cast away constness");
  ...
  toolchain_unreachable("unexpected cast away constness kind");
}
...
TryCastResult TryStaticDowncast(Sema &Self, CanQualType SrcType,
  ...
  if (Paths.isAmbiguous(SrcType.getUnqualifiedType())) {
    ...
    for (language::Core::CXXBasePath &Path : Paths) {
      if (DisplayedPaths.insert(Path.back().SubobjectNumber).second) {
        // We haven't displayed a path to this particular base
        // class subobject yet.
        PathDisplayStr += "\n    ";
        for (CXXBasePathElement &PE : toolchain::reverse(Path))
...
static void DiagnoseCallingConvCast(Sema &Self, const ExprResult &SrcExpr,
  ...
  toolchain::raw_svector_ostream OS(CCAttrText);
  ...
  if (!DestType->isScalarType() && !DestType->isVectorType() &&
    ...
    if (Self.getLangOpts().OpenCL && DestType->isEventT()) {
      ...
      if (SrcExpr.get()->EvaluateAsInt(Result, Self.Context)) {
        toolchain::APSInt CastInt = Result.Val.getInt();
        if (0 == CastInt) {
