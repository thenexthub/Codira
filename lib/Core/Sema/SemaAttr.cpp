...
void Sema::AddAlignmentAttributesForRecord(RecordDecl *RD) {
  ...
  // included file.
  for (auto &AlignPackedInclude : toolchain::reverse(AlignPackIncludeStack)) {
...
void Sema::inferGslPointerAttribute(NamedDecl *ND,
  ...
  static const toolchain::StringSet<> Containers{
      "array",
      "basic_string",
      "deque",
      "forward_list",
  ...
  static const toolchain::StringSet<> Iterators{"iterator", "const_iterator",
                                           "reverse_iterator",
...
void Sema::inferGslOwnerPointerAttribute(CXXRecordDecl *Record) {
  static const toolchain::StringSet<> StdOwners{
      "any",
      "array",
      "basic_regex",
      "basic_string",
  ...
  static const toolchain::StringSet<> StdPointers{
      "basic_string_view",
      "reference_wrapper",
      "regex_iterator",
      "span",
...
void Sema::inferLifetimeCaptureByAttribute(FunctionDecl *FD) {
  ...
  if (!MD->getIdentifier()) {
    static const toolchain::StringSet<> MapLikeContainer{
        "map",
        "multimap",
        "unordered_map",
        "unordered_multimap",
  ...
  static const toolchain::StringSet<> CapturingMethods{
...
void Sema::inferNullableClassAttribute(CXXRecordDecl *CRD) {
  static const toolchain::StringSet<> Nullable{
...
void Sema::ActOnPragmaClangSection(SourceLocation PragmaLoc,
  ...
  switch (SecKind) {
    ...
    case PragmaClangSectionKind::Text:
      ...
      break;
    default:
      toolchain_unreachable("invalid clang section kind");
  }
...
void Sema::ActOnPragmaPack(SourceLocation PragmaLoc, PragmaMsStackAction Action,
  ...
  if (Alignment) {
    std::optional<toolchain::APSInt> Val;
  ...
  for (unsigned Idx = 0; Idx < Args.size(); Idx++) {
    ...
    if (E->getType()->isArrayType())
      E = ImpCastExprToType(E, Context.getPointerType(E->getType()),
                            language::Core::CK_ArrayToPointerDecay)
              .get();
    if (E->getType()->isFunctionType())
      E = ImplicitCastExpr::Create(Context,
                                   Context.getPointerType(E->getType()),
                                   language::Core::CK_FunctionToPointerDecay, E, nullptr,
                                   VK_PRValue, FPOptionsOverride());
    if (E->isLValue())
      E = ImplicitCastExpr::Create(Context, E->getType().getNonReferenceType(),
                                   language::Core::CK_LValueToRValue, E, nullptr,
                                   VK_PRValue, FPOptionsOverride());
...
void Sema::DiagnoseUnterminatedPragmaAlignPack() {
  ...
  // information, diagnostics below might not be accurate if we have mixed
  // pragmas.
  for (const auto &StackSlot : toolchain::reverse(AlignPackStack.Stack)) {
...
void Sema::ActOnPragmaFPEvalMethod(SourceLocation Loc,
  ...
  switch (Value) {
  default:
    toolchain_unreachable("invalid pragma eval_method kind");
...
void Sema::ActOnPragmaFloatControl(SourceLocation Loc,
  ...
  switch (Value) {
  default:
    toolchain_unreachable("invalid pragma float_control kind");
  case PFC_Precise:
...
void Sema::ActOnPragmaMSSeg(SourceLocation PragmaLocation,
                            ...
                            toolchain::StringRef PragmaName) {
  PragmaStack<StringLiteral *> *Stack =
    toolchain::StringSwitch<PragmaStack<StringLiteral *> *>(PragmaName)
...
attrMatcherRuleListToString(ArrayRef<attr::SubjectMatchRule> Rules) {
  ...
  for (const auto &I : toolchain::enumerate(Rules)) {
    if (I.index())
...
void Sema::ActOnPragmaAttributeAttribute(
  ...
  if (StrictSubjectMatchRuleSet.empty()) {
    ...
    toolchain::SmallDenseMap<int, std::pair<int, SourceRange>, 2>
    ...
    for (const auto &Rule : RulesToCheck) {
      ...
      if (auto ParentRule = getParentAttrMatcherRule(MatchRule)) {
        if (toolchain::any_of(StrictSubjectMatchRuleSet,
...
void Sema::ActOnPragmaMSFunction(
    SourceLocation Loc, const toolchain::SmallVectorImpl<StringRef> &NoBuiltins) {
  if (!CurContext->getRedeclContext()->isFileContext()) {
...
void Sema::ActOnPragmaFPValueChangingOption(SourceLocation Loc,
  ...
  case PFK_Reciprocal:
    ...
    break;
  default:
    toolchain_unreachable("unhandled value changing pragma fp");
  }
...
void Sema::ActOnPragmaFEnvRound(SourceLocation Loc, toolchain::RoundingMode FPR) {
  FPOptionsOverride NewFPFeatures = CurFPFeatureOverrides();
