...
private:
  toolchain::PointerIntPair<CXXDefaultArgExpr *, 2, Kind> Impl;
...
struct CallableInfo {
  ...
  bool isVerifiable() const {
    ...
    toolchain_unreachable("undefined CallableType");
  }
...
class EffectToViolationMap {
  ...
  using ImplVec = toolchain::SmallVector<Violation, 1>;
...
public:
  ...
  ArrayRef<Violation> getSortedViolationsForExplicitEffects(SourceManager &SM) {
    if (!ViolationsForExplicitEffects.empty())
      toolchain::sort(ViolationsForExplicitEffects,
  ...
  void dump(Sema &SemaRef, toolchain::raw_ostream &OS) const {
    OS << "Pending: Declared ";
...
class Analyzer {
  ...
  using FuncAnalysisPtr =
      toolchain::PointerUnion<PendingFunctionAnalysis *, CompleteFunctionAnalysis *>;
  ...
  class AnalysisMap : toolchain::DenseMap<const Decl *, FuncAnalysisPtr> {
    using Base = toolchain::DenseMap<const Decl *, FuncAnalysisPtr>;
  ...
  public:
    ...
    void dump(Sema &SemaRef, toolchain::raw_ostream &OS) {
      OS << "\nAnalysisMap:\n";
...
public:
  ...
  void run(const TranslationUnitDecl &TU) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "AllInferrableEffectsToVerify: ";
               AllInferrableEffectsToVerify.dump(toolchain::dbgs());
               toolchain::dbgs() << "\n";);
...
private:
  ...
  PendingFunctionAnalysis *verifyDecl(const Decl *D) {
    ...
    LLVM_DEBUG(toolchain::dbgs()
               ...
               FAnalysis.dump(S, toolchain::dbgs()););
    ...
    LLVM_DEBUG(toolchain::dbgs() << "inserted pending " << PendingPtr << "\n";
               DeclAnalysis.dump(S, toolchain::dbgs()););
    return PendingPtr;
  }
  ...
  void completeAnalysis(const CallableInfo &CInfo,
    ...
    LLVM_DEBUG(toolchain::dbgs() << "inserted complete " << CompletePtr << "\n";
               DeclAnalysis.dump(S, toolchain::dbgs()););
  }
  ...
  void finishPendingAnalysis(const Decl *D, PendingFunctionAnalysis *Pending) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "finishPendingAnalysis for "
                            << Caller.getNameForDiagnostic(S) << " : ";
               Pending->dump(S, toolchain::dbgs()); toolchain::dbgs() << "\n";);
  ...
  void followCall(const CallableInfo &Caller, PendingFunctionAnalysis &PFA,
    ...
    LLVM_DEBUG(toolchain::dbgs()
               ...
               CalleeEffects.dump(toolchain::dbgs()); toolchain::dbgs() << "\n";
               toolchain::dbgs() << "  callee " << Callee.CDecl << " canonical "
  ...
  void emitDiagnostics(ArrayRef<Violation> Viols, const CallableInfo &CInfo) {
    ...
    for (const Violation &Viol1 : Viols) {
      ...
      case ViolationID::DeclDisallowsInference: // Shouldn't happen
                                                // here.
        toolchain_unreachable("Unexpected violation kind");
        break;
  ...
  struct FunctionBodyASTVisitor : DynamicRecursiveASTVisitor {
    ...
    bool VisitCXXConstructExpr(CXXConstructExpr *Construct) override {
      LLVM_DEBUG(toolchain::dbgs() << "VisitCXXConstructExpr : "
