...
bool CodeCompletionContext::wantConstructorResults() const {
  ...
  toolchain_unreachable("Invalid CodeCompletionContext::Kind!");
}

StringRef language::Core::getCompletionKindString(CodeCompletionContext::Kind Kind) {
...
CodeCompletionString::Chunk::Chunk(ChunkKind Kind, const char *Text)
  ...
  case CK_Optional:
    toolchain_unreachable("Optional strings cannot be created from text");
...
StringRef CodeCompletionTUInfo::getParentName(const DeclContext *DC) {
...

  {
    ...
    bool First = true;
    for (const DeclContext *CurDC : toolchain::reverse(Contexts)) {
      if (First)
        First = false;
      else {
        OS << "::";
...
CodeCompleteConsumer::OverloadCandidate::getFunctionType() const {
  ...
  toolchain_unreachable("Invalid CandidateKind!");
}
...
bool PrintingCodeCompleteConsumer::isResultFilteredOut(
  ...
  toolchain_unreachable("Unknown code completion result Kind.");
}
...
void CodeCompletionResult::computeCursorKindAndAvailability(bool Accessible) {
  ...
  case RK_Macro:
  case RK_Keyword:
    toolchain_unreachable("Macro and keyword kinds are handled by the constructors");
  }
...
bool language::Core::operator<(const CodeCompletionResult &X,
