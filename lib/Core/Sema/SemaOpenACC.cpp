...
void CollectActiveReductionClauses(
    toolchain::SmallVector<OpenACCReductionClause *> &ActiveClauses,
...
bool PreserveLoopRAIIDepthInAssociatedStmtRAII(OpenACCDirectiveKind DK) {
  ...
  case OpenACCDirectiveKind::Update:
    toolchain_unreachable("Doesn't have an associated stmt");
  case OpenACCDirectiveKind::Invalid:
    toolchain_unreachable("Unhandled directive kind?");
  }
  toolchain_unreachable("Unhandled directive kind?");
}
...
SemaOpenACC::AssociatedStmtRAII::AssociatedStmtRAII(
  ...
  if (DirKind == OpenACCDirectiveKind::Parallel ||
    ...
    SemaRef.LoopWithoutSeqInfo = {};
    if (Clauses.end() ==
        toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCSeqClause>))
    ...
    if (DirKind == OpenACCDirectiveKind::KernelsLoop && UnInstClauses.empty()) {
      ...
      auto *Itr = toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCGangClause>);
      if (Itr != Clauses.end())
        SemaRef.LoopGangClauseOnKernel = {(*Itr)->getBeginLoc(), DirKind};
    }

    if (UnInstClauses.empty()) {
      auto *Itr = toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCWorkerClause>);
      if (Itr != Clauses.end())
        SemaRef.LoopWorkerClauseLoc = (*Itr)->getBeginLoc();

      auto *Itr2 = toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCVectorClause>);
      if (Itr2 != Clauses.end())
...
void SemaOpenACC::AssociatedStmtRAII::SetCollapseInfoBeforeAssociatedStmt(
  ...
  auto *UnInstClauseItr =
      toolchain::find_if(UnInstClauses, toolchain::IsaPred<OpenACCCollapseClause>);
  auto *ClauseItr =
      toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCCollapseClause>);
    ...
    UnInstClauseItr =
        UnInstClauseItr == UnInstClauses.end()
            ...
            : std::find_if(std::next(UnInstClauseItr), UnInstClauses.end(),
                           toolchain::IsaPred<OpenACCCollapseClause>);
...
void SemaOpenACC::AssociatedStmtRAII::SetTileInfoBeforeAssociatedStmt(
  ...
  if (UnInstClauses.size() > 0)
    return;
  auto *TileClauseItr =
      toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCTileClause>);
  ...
  while (Clauses.end() !=
         (TileClauseItr = std::find_if(std::next(TileClauseItr), Clauses.end(),
                                       toolchain::IsaPred<OpenACCTileClause>))) {
...
ExprResult CheckVarType(SemaOpenACC &S, OpenACCClauseKind CK, Expr *VarExpr,
  ...
  if (auto *ArrTy = InnerTy->getAsArrayTypeUnsafe()) {
    ...
    if (!ArrTy->isConstantArrayType()) {
      S.Diag(InnerLoc, language::Core::diag::warn_acc_var_referenced_non_const_array)
          << InnerTy << CK;
      return VarExpr;
    }
  ...
  if (CK == OpenACCClauseKind::Private) {
    bool HasNonDeletedDefaultCtor =
        toolchain::find_if(RD->ctors(), [](const CXXConstructorDecl *CD) {
          return CD->isDefaultConstructor() && !CD->isDeleted();
        }) != RD->ctors().end();
    if (!HasNonDeletedDefaultCtor && !RD->needsImplicitDefaultConstructor()) {
      S.Diag(InnerLoc, language::Core::diag::warn_acc_var_referenced_lacks_op)
          << InnerTy << CK << language::Core::diag::AccVarReferencedReason::DefCtor;
      return ExprError();
    }
    ...
    if (!RD->hasSimpleCopyConstructor()) {
      ...
      if (SMOR.getKind() != Sema::SpecialMemberOverloadResult::Success ||
        ...
        S.Diag(InnerLoc, language::Core::diag::warn_acc_var_referenced_lacks_op)
            << InnerTy << CK << language::Core::diag::AccVarReferencedReason::CopyCtor;
        return ExprError();
      }
    }
  ...
  if (DestructorDeleted && !RD->needsImplicitDestructor()) {
    S.Diag(InnerLoc, language::Core::diag::warn_acc_var_referenced_lacks_op)
        << InnerTy << CK << language::Core::diag::AccVarReferencedReason::Dtor;
    return ExprError();
  }
  return VarExpr;
}
...
ExprResult SemaOpenACC::ActOnArraySectionExpr(Expr *Base, SourceLocation LBLoc,
  ...
  std::optional<toolchain::APSInt> BaseSize;
  ...
  auto GetBoundValue = [&](Expr *E) -> std::optional<toolchain::APSInt> {
  ...
  std::optional<toolchain::APSInt> LowerBoundValue = GetBoundValue(LowerBound);
  std::optional<toolchain::APSInt> LengthValue = GetBoundValue(Length);
  ...
  if (LowerBoundValue.has_value()) {
    if (LowerBoundValue->isNegative()) {
      ...
      LowerBound = GetRecovery(LowerBound, LowerBound->getType());
               ...
               toolchain::APSInt::compareValues(*LowerBoundValue, *BaseSize) >= 0) {
  ...
  if (LengthValue.has_value()) {
    if (LengthValue->isNegative()) {
      ...
      Length = GetRecovery(Length, Length->getType());
               ...
               toolchain::APSInt::compareValues(*LengthValue, *BaseSize) > 0) {
  ...
  auto AddAPSInt = [](toolchain::APSInt LHS, toolchain::APSInt RHS) -> toolchain::APSInt {
    if (LHS.isSigned() == RHS.isSigned())
      return LHS + RHS;

    unsigned Width = std::max(LHS.getBitWidth(), RHS.getBitWidth()) + 1;
    return toolchain::APSInt(LHS.sext(Width) + RHS.sext(Width), /*Signed=*/true);
  };
  ...
  if (BaseSize.has_value() && LowerBoundValue.has_value() &&
      ...
      toolchain::APSInt::compareValues(AddAPSInt(*LowerBoundValue, *LengthValue),
                                  *BaseSize) > 0) {
...
bool isValidLoopVariableType(QualType LoopVarTy) {
  ...
  if (const auto *RD = LoopVarTy->getAsCXXRecordDecl()) {
    ...
    for (const auto *TD :
         toolchain::make_filter_range(RD->decls(), toolchain::IsaPred<TypedefNameDecl>)) {
...
StmtResult SemaOpenACC::ActOnEndStmtDirective(
  ...
  case OpenACCDirectiveKind::Routine:
    toolchain_unreachable("routine shouldn't handled here");
  case OpenACCDirectiveKind::Declare: {
  ...
  toolchain_unreachable("Unhandled case in directive handling?");
}
...
StmtResult SemaOpenACC::ActOnAssociatedStmt(
  ...
  switch (K) {
  default:
    toolchain_unreachable("Unimplemented associated statement application");
  case OpenACCDirectiveKind::EnterData:
  case OpenACCDirectiveKind::ExitData:
  case OpenACCDirectiveKind::Wait:
  case OpenACCDirectiveKind::Init:
  ...
  toolchain_unreachable("Invalid associated statement application");
}
...
bool CheckValidRoutineGangWorkerVectorSeqClauses(
    ...
    ArrayRef<const OpenACCClause *> Clauses) {
  auto RequiredPred = toolchain::IsaPred<OpenACCGangClause, OpenACCWorkerClause,
  ...
  auto *FirstDeviceType =
      toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCDeviceTypeClause>);
...
DeclGroupRef SemaOpenACC::ActOnEndDeclDirective(
  ...
  case OpenACCDirectiveKind::Routine:
    toolchain_unreachable("routine shouldn't be handled here");
  }
  toolchain_unreachable("unhandled case in directive handling?");
}
...
void SemaOpenACC::CheckRoutineDecl(SourceLocation DirLoc,
  ...
  auto BindItr = toolchain::find_if(Clauses, toolchain::IsaPred<OpenACCBindClause>);
  for (auto *A : NextParsedFDecl->attrs()) {
    ...
    if (auto *RA = dyn_cast<OpenACCRoutineDeclAttr>(A)) {
      auto OtherBindItr =
          toolchain::find_if(RA->Clauses, toolchain::IsaPred<OpenACCBindClause>);
...
SemaOpenACC::CreateInitRecipe(OpenACCClauseKind CK, const Expr *VarExpr) {
  ...
  if (CK == OpenACCClauseKind::Private) {
    ...
    toolchain_unreachable("Unknown clause kind in CreateInitRecipe");
  }
