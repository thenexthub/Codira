...
StmtResult Sema::ActOnGCCAsmStmt(SourceLocation AsmLoc, bool IsSimple,
  ...
  if (asmString->getDependence() != ExprDependence::None ||
      ...
      toolchain::any_of(clobbers, [](Expr *E) {
  ...
  toolchain::stable_sort(NamedOperandList, toolchain::less_first());
...
void Sema::FillInlineAsmIdentifierInfo(Expr *Res,
  ...
  if (Res->isPRValue()) {
    bool IsEnum = isa<language::Core::EnumType>(T);
    if (DeclRefExpr *DRE = dyn_cast<language::Core::DeclRefExpr>(Res))
...
LabelDecl *Sema::GetOrCreateMSAsmLabel(StringRef ExternalLabelName,
  ...
  if (Label->isMSAsmLabel()) {
    ...
    toolchain::raw_string_ostream OS(InternalName);
