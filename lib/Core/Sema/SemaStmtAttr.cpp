...
static Attr *handleLoopHintAttr(Sema &S, Stmt *St, const ParsedAttr &A,
  ...
  StringRef PragmaName =
      toolchain::StringSwitch<StringRef>(
  ...
  if (PragmaName == "nounroll") {
    ...
    Option = toolchain::StringSwitch<LoopHintAttr::OptionType>(
    ...
    if (Option == LoopHintAttr::VectorizeWidth) {
      ...
      if (StateLoc->getIdentifierInfo()->isStr("disable"))
        ...
        toolchain_unreachable("bad loop hint argument");
    } else
      toolchain_unreachable("bad loop hint");
  }
...
static bool CheckStmtInlineAttr(Sema &SemaRef, const Stmt *OrigSt,
  ...
  // instantiation.
  // ie: [[language::Core::always_inline]] non_dependent(), (other_call<Pack>()...)
  ...
  for (const auto &Tup :
       toolchain::zip_longest(OrigCEF.getCallExprs(), CEF.getCallExprs())) {
...
static void CheckForDuplicateLoopAttrs(Sema &S, ArrayRef<const Attr *> Attrs) {
  ...
  const auto *FirstItr = toolchain::find_if(Attrs, FindFunc);

  if (FirstItr == Attrs.end()) // no attributes found
    return;

  const auto *LastFoundItr = FirstItr;
  std::optional<toolchain::APSInt> FirstValue;
  ...
  while (Attrs.end() != (LastFoundItr = std::find_if(LastFoundItr + 1,
    ...
    toolchain::APSInt SecondValue = CASA->getResultAsAPSInt();
    if (!FirstValue)
...
static Attr *handleOpenCLUnrollHint(Sema &S, Stmt *St, const ParsedAttr &A,
  ...
  if (A.getNumArgs() == 1) {
    Expr *E = A.getArgAsExpr(0);
    std::optional<toolchain::APSInt> ArgVal;
...
static Attr *handleHLSLLoopHintAttr(Sema &S, Stmt *St, const ParsedAttr &A,
  ...
  if (A.getNumArgs() == 1) {
    ...
    std::optional<toolchain::APSInt> ArgVal = E->getIntegerConstantExpr(S.Context);
