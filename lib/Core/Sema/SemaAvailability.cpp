...
static bool hasMatchingEnvironmentOrNone(const ASTContext &Context,
  ...
  if (!IIEnvironment || Environment == toolchain::Triple::UnknownEnvironment)
    return true;

  toolchain::Triple::EnvironmentType ET =
    ...
    if (!S.getLangOpts().HLSLStrictAvailability ||
        (DeclEnv != nullptr &&
         S.getASTContext().getTargetInfo().getTriple().getEnvironment() ==
             toolchain::Triple::EnvironmentType::Library))
      return false;
  }
...
static void DoEmitAvailabilityWarning(Sema &S, AvailabilityResult K,
  ...
  case AR_NotYetIntroduced: {
    ...
    toolchain::StringRef TargetEnvironment(
        toolchain::Triple::getEnvironmentTypeName(TI.getTriple().getEnvironment()));
    toolchain::StringRef AttrEnvironment =
    ...
    if (const auto *Enclosing = findEnclosingDeclToAnnotate(Ctx)) {
      ...
      toolchain::Twine MacroPrefix = "__API_AVAILABLE_PLATFORM_";
      auto AvailablePlatform =
          toolchain::find_if(EquivalentPlatforms, [&](StringRef EquivalentPlatform) {
      ...
      FixitNoteDiag << FixItHint::CreateInsertion(
          Insertion->Loc,
          (toolchain::Twine(Insertion->Prefix) + "API_AVAILABLE(" +
  ...
  case AR_Available:
    toolchain_unreachable("Warning for availability of available declaration?");
  }
...
public:
  ...
  static const Stmt *findLastStmtThatUsesDecl(const Decl *D,
    ...
    Visitor.D = D;
    for (const Stmt *S : toolchain::reverse(Scope->body())) {
...
void DiagnoseUnguardedAvailability::DiagnoseDeclAvailability(
  ...
  if (Result != AR_Available) {
    ...
    std::string PlatformName(
        AvailabilityAttr::getPrettyPlatformName(TI.getPlatformName()));
    toolchain::StringRef TargetEnvironment(TI.getTriple().getEnvironmentName());
    toolchain::StringRef AttrEnvironment =
        AA->getEnvironment() ? AA->getEnvironment()->getName() : "";
    bool UseEnvironment =
        (!AttrEnvironment.empty() && !TargetEnvironment.empty());
    ...
    for (const Stmt *S : toolchain::reverse(StmtStack)) {
    ...
    toolchain::raw_string_ostream FixItOS(FixItString);
