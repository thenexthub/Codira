...
/// the target is unknown. Return statements with \c [[language::Core::musttail]] cannot
...
class JumpScopeChecker {
  ...
  toolchain::DenseMap<Stmt*, unsigned> LabelAndGotoScopes;
  ...
  if (destructKind != QualType::DK_none) {
    ...
    switch (destructKind) {
      ...
      case QualType::DK_nontrivial_c_struct:
        ...
        break;
      case QualType::DK_none:
        toolchain_unreachable("non-lifetime captured variable");
    }
...
void JumpScopeChecker::VerifyIndirectJumps() {
  ...
  {
    toolchain::DenseMap<unsigned, Stmt*> JumpScopesMap;
  ...
  toolchain::DenseMap<unsigned, LabelDecl*> TargetScopes;
  ...
  toolchain::BitVector Reachable(Scopes.size(), false);
...
const Attr *JumpScopeChecker::GetMustTailAttr(AttributedStmt *AS) {
  ...
  const auto *Iter =
      toolchain::find_if(Attrs, [](const Attr *A) { return isa<MustTailAttr>(A); });
