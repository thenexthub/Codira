...
#include "language/Core/Sema/RISCVIntrinsicManager.h"
...
#include "toolchain/ADT/edit_distance.h"
...
namespace {
  ...
  class UnqualUsingDirectiveSet {
    ...
    ListTy list;
    toolchain::SmallPtrSet<DeclContext*, 8> visited;
  ...
  public:
    ...
    void done() { toolchain::sort(list, UnqualUsingEntry::Comparator()); }
    ...
    getNamespacesFor(const DeclContext *DC) const {
      return toolchain::make_range(std::equal_range(begin(), end(),
...
void LookupResult::resolveKind() {
  ...
  toolchain::SmallDenseMap<const NamedDecl *, unsigned, 16> Unique;
  toolchain::SmallDenseMap<QualType, unsigned, 16> UniqueTypes;
  ...
  toolchain::SmallVector<const NamedDecl *, 4> EquivalentNonFunctions;
  toolchain::BitVector RemovedDecls(N);
...
LLVM_DUMP_METHOD void LookupResult::dump() {
  toolchain::errs() << "lookup results for " << getLookupName().getAsString()
               << ":\n";
...
static QualType diagOpenCLBuiltinTypeError(Sema &S, toolchain::StringRef TypeClass,
...
static QualType getOpenCLEnumType(Sema &S, toolchain::StringRef Name) {
...
static QualType getOpenCLTypedefType(Sema &S, toolchain::StringRef Name) {
  LookupResult Result(S, &S.Context.Idents.get(Name), SourceLocation(),
...
toolchain::DenseSet<Module*> &Sema::getLookupModules() {
...
bool Sema::isUsableModule(const Module *M) {
  ...
  if (toolchain::count_if(ModuleScopes,
...
hasAcceptableDefaultArgument(Sema &S, const ParmDecl *D,
  ...
  toolchain::SmallPtrSet<const ParmDecl *, 4> Visited;
...
bool Sema::hasAcceptableDefaultArgument(
    const NamedDecl *D, toolchain::SmallVectorImpl<Module *> *Modules,
    Sema::AcceptableKind Kind) {
...
hasAcceptableDeclarationImpl(Sema &S, const NamedDecl *D,
                             toolchain::SmallVectorImpl<Module *> *Modules, Filter F,
                             Sema::AcceptableKind Kind) {
      ...
      [](const NamedDecl *D) {
        ...
        if (auto *VD = dyn_cast<VarDecl>(D))
          return VD->getTemplateSpecializationKind() ==
                 TSK_ExplicitSpecialization;
        toolchain_unreachable("unknown explicit specialization kind");
      },
      Kind);
}
...
bool Sema::isModuleVisible(const Module *M, bool ModulePrivate) {
  ...
  return toolchain::any_of(LookupModules, [&](const Module *LookupM) {
...
bool LookupResult::isAvailableForLookup(Sema &SemaRef, NamedDecl *ND) {
  ...
  // (See https://github.com/toolchain/toolchain-project/issues/59601)
  //
  ...
  // https://github.com/toolchain/toolchain-project/issues/131058 But it is much complext
...
bool Sema::LookupQualifiedName(LookupResult &R, DeclContext *LookupCtx,
  ...
  if (const auto *TD = dyn_cast<TagDecl>(LookupCtx);
    ...
    toolchain_unreachable("Declaration context must already be complete!");
#endif
  ...
  auto HasSameDeclarations = [&](DeclContext::lookup_iterator A,
    ...
    while (true) {
      ...
      if (AResult != BResult) {
        ...
        toolchain::SmallDenseMap<Result, bool, 32> AResults;
...
void Sema::DiagnoseAmbiguousLookup(LookupResult &Result) {
  ...
  case LookupAmbiguityKind::AmbiguousTagHiding: {
    ...
    toolchain::SmallPtrSet<NamedDecl*, 8> TagDecls;
...
Sema::LookupSpecialMember(CXXRecordDecl *RD, CXXSpecialMemberKind SM,
  ...
  ID.AddPointer(RD);
  ID.AddInteger(toolchain::to_underlying(SM));
    ...
    if (CXXMethodDecl *M = dyn_cast<CXXMethodDecl>(Cand->getUnderlyingDecl())) {
      if (SM == CXXSpecialMemberKind::CopyAssignment ||
        ...
        AddMethodCandidate(M, Cand, RD, ThisTy, Classification,
                           toolchain::ArrayRef(&Arg, NumArgs), OCS, true);
      else if (CtorInfo)
        AddOverloadCandidate(CtorInfo.Constructor, CtorInfo.FoundDecl,
                             toolchain::ArrayRef(&Arg, NumArgs), OCS,
                             /*SuppressUserConversions*/ true);
      else
        AddOverloadCandidate(M, Cand, toolchain::ArrayRef(&Arg, NumArgs), OCS,
                             /*SuppressUserConversions*/ true);
      ...
      for (D = D->getMostRecentDecl(); D;
        ...
        if (D->getIdentifierNamespace() & Decl::IDNS_Ordinary) {
          ...
          if (D->isInExportDeclContext()) {
            ...
            if (D->isInAnotherModuleUnit() &&
                toolchain::any_of(AssociatedClasses, [&](auto *E) {
...
public:
  ...
  /// of declarations.
  typedef toolchain::TinyPtrVector<NamedDecl*> ShadowMapEntry;
...
private:
  ...
  toolchain::SmallPtrSet<DeclContext *, 8> VisitedContexts;
...
private:
  void lookupInDeclContext(DeclContext *Ctx, LookupResult &Result,
    ...
    toolchain::SmallVector<NamedDecl *, 4> DeclsToVisit;
...
static void checkCorrectionVisibility(Sema &SemaRef, TypoCorrection &TC) {
  ...
  toolchain::SmallVector<NamedDecl*, 4> NewDecls(TC.begin(), DI);
...
void TypoCorrectionConsumer::addCorrection(TypoCorrection Correction) {
  ...
  if (NamedDecl *NewND = Correction.getCorrectionDecl()) {
    auto RI = toolchain::find_if(CList, [NewND](const TypoCorrection &TypoCorr) {
    ...
    if (RI != CList.end()) {
      ...
      auto IsDeprecated = [](Decl *D) {
        while (D) {
          if (D->isDeprecated())
            return true;
          D = toolchain::dyn_cast_or_null<NamespaceDecl>(D->getDeclContext());
        }
        return false;
      };
...
void TypoCorrectionConsumer::addNamespaces(
    const toolchain::MapVector<NamespaceDecl *, bool> &KnownNamespaces) {
    ...
    for (const auto &NSI : Namespaces) {
      ...
      case LookupResultKind::FoundOverloaded: {
        if (SS && SS->isValid()) {
          ...
          toolchain::raw_string_ostream OldOStream(OldQualified);
...
TypoCorrectionConsumer::NamespaceSpecifierSet::NamespaceSpecifierSet(
  ...
  if (NestedNameSpecifier NNS =
    ...
    toolchain::raw_string_ostream SpecifierOStream(CurNameSpecifier);
  ...
  // context.
  for (DeclContext *C : toolchain::reverse(CurContextChain)) {
...
TypoCorrectionConsumer::NamespaceSpecifierSet::buildNestedNameSpecifier(
  ...
  for (DeclContext *C : toolchain::reverse(DeclChain)) {
    if (auto *ND = dyn_cast_or_null<NamespaceDecl>(C)) {
...
void TypoCorrectionConsumer::NamespaceSpecifierSet::addNameSpecifier(
  ...
  if (NamespaceDeclChain.empty()) {
    ...
    if (toolchain::is_contained(CurNameSpecifierIdentifiers, Name)) {
      std::string NewNameSpecifier;
      toolchain::raw_string_ostream SpecifierOStream(NewNameSpecifier);
      SmallVector<const IdentifierInfo *, 4> NewNameSpecifierIdentifiers;
      getNestedNameSpecifierIdentifiers(NNS, NewNameSpecifierIdentifiers);
      NNS.print(SpecifierOStream, Context.getPrintingPolicy());
      SameNameSpecifier = NewNameSpecifier == CurNameSpecifier;
    }
    if (SameNameSpecifier || toolchain::is_contained(CurContextIdentifiers, Name)) {
      // Rebuild the NestedNameSpecifier as a globally-qualified specifier.
      NNS = NestedNameSpecifier::getGlobal();
      NumSpecifiers =
          buildNestedNameSpecifier(FullNamespaceDeclChain, NNS);
  ...
  if (NNS && !CurNameSpecifierIdentifiers.empty()) {
    ...
    getNestedNameSpecifierIdentifiers(NNS, NewNameSpecifierIdentifiers);
    NumSpecifiers =
        toolchain::ComputeEditDistance(toolchain::ArrayRef(CurNameSpecifierIdentifiers),
                                  toolchain::ArrayRef(NewNameSpecifierIdentifiers));
  }
...
std::string TypoCorrection::getAsString(const LangOptions &LO) const {
  if (CorrectionNameSpec) {
    ...
    toolchain::raw_string_ostream PrefixOStream(tmpBuffer);
...
void Sema::diagnoseMissingImport(SourceLocation Loc, const NamedDecl *Decl,
  ...
  toolchain::SmallVector<Module*, 8> OwningModules;
  OwningModules.push_back(Owner);
  auto Merged = Context.getModulesWithMergedDefinition(Def);
  toolchain::append_range(OwningModules, Merged);
...
static std::string getHeaderNameForHeader(Preprocessor &PP, FileEntryRef E,
                                          toolchain::StringRef IncludingFile) {
  bool IsAngled = false;
...
void Sema::diagnoseMissingImport(SourceLocation UseLoc, const NamedDecl *Decl,
  ...
  // See https://github.com/toolchain/toolchain-project/issues/73893. It is generally
  ...
  toolchain::SmallVector<Module*, 8> UniqueModules;
  toolchain::SmallDenseSet<Module*, 8> UniqueModuleSet;
  for (auto *M : Modules) {
