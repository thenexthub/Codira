...
Sema::DiagnoseUnexpandedParameterPacks(SourceLocation Loc,
  ...
  if (sema::CapturingScopeInfo *CSI = getEnclosingLambdaOrBlock()) {
    for (auto &Pack : Unexpanded) {
      ...
      if (toolchain::any_of(CSI->LocalPacks, DeclaresThisPack))
  ...
  toolchain::SmallPtrSet<IdentifierInfo *, 4> NamesKnown;
...
bool Sema::DiagnoseUnexpandedParameterPackInRequiresExpr(RequiresExpr *RE) {
  ...
  toolchain::SmallPtrSet<NamedDecl *, 8> ParmSet(toolchain::from_range, Parms);
  ...
  for (UnexpandedParameterPack ParmPack : Unexpanded) {
    ...
    if (const TemplateTypeParmType *TTP =
      ...
      if (isa<VarDecl>(ND))
        ...
        toolchain::PointerUnion<Decl *, DeclArgumentPack *> *Instantiation =
    ...
    if (IsVarDeclPack) {
      ...
      PendingPackExpansionSize =
          toolchain::count_if(Pack, [](const TemplateArgument &TA) {
