...
#include "language/Core/Sema/SemaCodeCompletion.h"
...
private:
  ...
  toolchain::SmallPtrSet<const Decl *, 16> AllDeclsFound;
  ...
  class ShadowMapEntry {
    ...
    toolchain::PointerUnion<const NamedDecl *, DeclIndexPairVector *> DeclOrVector;
  ...
  /// results.
  typedef toolchain::DenseMap<DeclarationName, ShadowMapEntry> ShadowMap;
  ...
  toolchain::DenseMap<std::pair<DeclContext *, /*Name*/uintptr_t>, ShadowMapEntry>
      OverloadMap;
...
void PreferredTypeBuilder::enterVariableInit(SourceLocation Tok, Decl *D) {
  if (!Enabled)
    return;
  auto *VD = toolchain::dyn_cast_or_null<ValueDecl>(D);
  ComputeType = nullptr;
...
void PreferredTypeBuilder::enterFunctionArgument(
    SourceLocation Tok, toolchain::function_ref<QualType()> ComputeType) {
  if (!Enabled)
    return;
...
class ResultBuilder::ShadowMapEntry::iterator {
  toolchain::PointerUnion<const NamedDecl *, const DeclIndexPair *> DeclOrIterator;
...
SimplifiedTypeClass language::Core::getSimplifiedTypeClass(CanQualType T) {
...
QualType language::Core::getDeclUsageType(ASTContext &C, NestedNameSpecifier Qualifier,
                                 const NamedDecl *ND) {
  ND = ND->getUnderlyingDecl();
...
static OverloadCompare compareOverloads(const CXXMethodDecl &Candidate,
  ...
  if (CandidateRef != IncumbentRef) {
    ...
    if (ObjectKind == language::Core::VK_XValue)
...
bool ResultBuilder::canCxxMethodBeCalled(const CXXMethodDecl *Method,
  ...
  const auto *CurrentClassScope = [&]() -> const CXXRecordDecl * {
    for (DeclContext *Ctx = CurContext; Ctx; Ctx = Ctx->getParent()) {
      const auto *CtxMethod = toolchain::dyn_cast<CXXMethodDecl>(Ctx);
...
bool ResultBuilder::canFunctionBeCalled(const NamedDecl *ND,
  ...
  //   calls.
  if (CompletionContext.getKind() == language::Core::CodeCompletionContext::CCC_Symbol) {
...
public:
  CodeCompletionDeclConsumer(
    ...
    NamingClass = toolchain::dyn_cast<CXXRecordDecl>(InitialLookupCtx);
...
private:
  bool IsAccessible(NamedDecl *ND, DeclContext *Ctx) {
    ...
    if (auto *Cls = toolchain::dyn_cast_or_null<CXXRecordDecl>(Ctx)) {
      if (!NamingClass)
        NamingClass = Cls;
...
static bool WantTypesInContext(SemaCodeCompletion::ParserCompletionContext CCC,
  ...
  toolchain_unreachable("Invalid ParserCompletionContext!");
}
...
static void AddOverrideResults(ResultBuilder &Results,
  ...
  const auto *CR = toolchain::dyn_cast<CXXRecordDecl>(S.CurContext);
  // If not inside a class/struct/union return empty.
  if (!CR)
    return;
  // First store overrides within current class.
  // These are stored by name to make querying fast in the later step.
  toolchain::StringMap<std::vector<FunctionDecl *>> Overrides;
...
AddOrdinaryNameResults(SemaCodeCompletion::ParserCompletionContext CCC,
  ...
  case SemaCodeCompletion::PCC_Namespace:
    if (SemaRef.getLangOpts().CPlusPlus) {
      ...
      if (SemaRef.getLangOpts().CPlusPlus20 &&
        ...
        language::Core::Module *CurrentModule = SemaRef.getCurrentModule();
  ...
  case SemaCodeCompletion::PCC_Statement: {
    ...
    if (ReturnType.isNull() || ReturnType->isVoidType()) {
      ...
      // "return expression ;"
      Builder.AddTypedTextChunk("return");
      Builder.AddChunk(language::Core::CodeCompletionString::CK_HorizontalSpace);
      Builder.AddPlaceholderChunk("expression");
      Builder.AddChunk(CodeCompletionString::CK_SemiColon);
      Results.AddResult(Result(Builder.TakeString()));
  ...
  if (ObjCQuals & Decl::OBJC_TQ_CSNullability) {
    if (auto nullability = AttributedType::stripOuterNullability(Type)) {
      ...
      case NullabilityKind::NullableResult:
        toolchain_unreachable("Not supported as a context-sensitive keyword!");
        break;
      }
    }
  }
  ...
  for (TemplateParameterList::iterator P = Params->begin() + Start; P != PEnd;
    ...
    if (TemplateTypeParmDecl *TTP = dyn_cast<TemplateTypeParmDecl>(*P)) {
      if (TTP->wasDeclaredWithTypename())
        ...
        toolchain::raw_string_ostream OS(PlaceholderStr);
        TC->print(OS, Policy);
      } else
...
static void AddQualifierToCompletionString(CodeCompletionBuilder &Result,
  ...
  {
    toolchain::raw_string_ostream OS(PrintedNNS);
...
CodeCompletionString *CodeCompletionResult::createCodeCompletionStringForDecl(
  ...
  if (const FunctionTemplateDecl *FunTmpl =
    ...
    // cases.
    toolchain::SmallBitVector Deduced(FunTmpl->getTemplateParameters()->size());
...
const RawComment *language::Core::getCompletionComment(const ASTContext &Ctx,
                                              const NamedDecl *ND) {
  if (!ND)
    return nullptr;
...
const RawComment *language::Core::getPatternCompletionComment(const ASTContext &Ctx,
                                                     const NamedDecl *ND) {
...
const RawComment *language::Core::getParameterComment(
    const ASTContext &Ctx,
...
static void AddOverloadAggregateChunks(const RecordDecl *RD,
  ...
  auto AddChunk = [&](toolchain::StringRef Placeholder) {
    if (ChunkIndex > 0)
      Result.AddChunk(CodeCompletionString::CK_Comma);
  ...
  if (auto *CRD = toolchain::dyn_cast<CXXRecordDecl>(RD)) {
...
static CodeCompletionString *createTemplateSignatureString(
    ...
    const PrintingPolicy &Policy) {
  toolchain::ArrayRef<NamedDecl *> Params = TD->getTemplateParameters()->asArray();
...
unsigned language::Core::getMacroUsagePriority(StringRef MacroName,
                                      const LangOptions &LangOpts,
...
CXCursorKind language::Core::getCursorKindForDecl(const Decl *D) {
  if (!D)
...
static const char *underscoreAttrScope(toolchain::StringRef Scope) {
  if (Scope == "clang")
    return "_Clang";
  if (Scope == "gnu")
    return "__gnu__";
...
static const char *noUnderscoreAttrScope(toolchain::StringRef Scope) {
  if (Scope == "_Clang")
    return "clang";
  if (Scope == "__gnu__")
    return "gnu";
...
void SemaCodeCompletion::CodeCompleteAttribute(
  ...
  // language::Core::foo but you can also write _Clang::__foo__.
  //
  // (Clang supports a mix like language::Core::__foo__ but we won't suggest it: either
  ...
  toolchain::StringRef InScopeName;
  ...
  toolchain::DenseSet<toolchain::StringRef> FoundScopes;
  auto AddCompletions = [&](const ParsedAttrInfo &A) {
    ...
    for (const auto &S : A.Spellings) {
      if (S.Syntax != Syntax)
        continue;
      toolchain::StringRef Name = S.NormalizedFullName;
      ...
      auto Add = [&](toolchain::StringRef Scope, toolchain::StringRef Name,
                     bool Underscores) {
        CodeCompletionBuilder Builder(Results.getAllocator(),
                                      Results.getCodeCompletionTUInfo());
        toolchain::SmallString<32> Text;
        if (!Scope.empty()) {
          Text.append(Scope);
          Text.append("::");
        }
...
struct CoveredEnumerators {
  toolchain::SmallPtrSet<EnumConstantDecl *, 8> Seen;
...
static void AddLambdaCompletion(ResultBuilder &Results,
                                toolchain::ArrayRef<QualType> Parameters,
                                const LangOptions &LangOpts) {
  if (!Results.includeCodePatterns())
    return;
  ...
  if (!Parameters.empty()) {
    ...
    for (auto Parameter : Parameters) {
      ...
      constexpr toolchain::StringLiteral NamePlaceholder = "!#!NAME_GOES_HERE!#!";
      std::string Type = std::string(NamePlaceholder);
      Parameter.getAsStringInternal(Type, PrintingPolicy(LangOpts));
      toolchain::StringRef Prefix, Suffix;
      std::tie(Prefix, Suffix) = toolchain::StringRef(Type).split(NamePlaceholder);
      Prefix = Prefix.rtrim();
      Suffix = Suffix.ltrim();
  ...
  Completion.AddChunk(language::Core::CodeCompletionString::CK_HorizontalSpace);
...
/// property name.
typedef toolchain::SmallPtrSet<const IdentifierInfo *, 16> AddedPropertiesSet;
...
public:
  ...
  struct Member {
    ...
    CodeCompletionString *render(Sema &S, CodeCompletionAllocator &Alloc,
      ...
      if (ResultType) {
        std::string AsString;
        {
          toolchain::raw_string_ostream OS(AsString);
      ...
      if (ArgTypes) {
        B.AddChunk(language::Core::CodeCompletionString::CK_LeftParen);
        bool First = true;
        for (QualType Arg : *ArgTypes) {
          if (First)
            First = false;
          else {
            B.AddChunk(language::Core::CodeCompletionString::CK_Comma);
            B.AddChunk(language::Core::CodeCompletionString::CK_HorizontalSpace);
          }
          B.AddPlaceholderChunk(Alloc.CopyString(
              Arg.getAsString(getCompletionPrintingPolicy(S))));
        }
        B.AddChunk(language::Core::CodeCompletionString::CK_RightParen);
      }
      return B.TakeString();
    }
  };
  ...
  std::vector<Member> members() {
    ...
    toolchain::sort(Results, [](const Member &L, const Member &R) {
...
private:
  ...
  toolchain::DenseMap<const IdentifierInfo *, Member> Results;
};
...
QualType getApproximateType(const Expr *E, HeuristicResolver &Resolver) {
  ...
  if (const CallExpr *CE = toolchain::dyn_cast<CallExpr>(E)) {
    ...
    if (const auto *OE = toolchain::dyn_cast<OverloadExpr>(CE->getCallee())) {
      ...
      for (const auto *D : OE->decls()) {
        QualType ReturnType;
        if (const auto *FD = toolchain::dyn_cast<FunctionDecl>(D))
          ReturnType = FD->getReturnType();
        else if (const auto *FTD = toolchain::dyn_cast<FunctionTemplateDecl>(D))
  ...
  if (const auto *CDSME = toolchain::dyn_cast<CXXDependentScopeMemberExpr>(E)) {
  ...
  if (const auto *DRE = toolchain::dyn_cast<DeclRefExpr>(E)) {
    if (const auto *VD = toolchain::dyn_cast<VarDecl>(DRE->getDecl())) {
      if (VD->hasInit())
        return getApproximateType(VD->getInit(), Resolver);
    }
  }
  if (const auto *UO = toolchain::dyn_cast<UnaryOperator>(E)) {
...
Expr *unwrapParenList(Expr *Base) {
  if (auto *PLE = toolchain::dyn_cast_or_null<ParenListExpr>(Base)) {
...
void SemaCodeCompletion::CodeCompleteTag(Scope *S, unsigned TagSpec) {
  ...
  default:
    toolchain_unreachable("Unknown type specifier kind in CodeCompleteTag");
  }
...
static void mergeCandidatesWithResults(
  ...
  toolchain::stable_sort(CandidateSet, [&](const OverloadCandidate &X,
...
static bool argMatchesTemplateParams(const ParsedTemplateArgument &Arg,
  ...
  case ParsedTemplateArgument::Type:
    return toolchain::isa<TemplateTypeParmDecl>(Param); // constraints not checked
  case ParsedTemplateArgument::NonType:
    return toolchain::isa<NonTypeTemplateParmDecl>(Param); // type not checked
  case ParsedTemplateArgument::Template:
    return toolchain::isa<TemplateTemplateParmDecl>(Param); // signature not checked
  }
...
QualType SemaCodeCompletion::ProduceTemplateArgumentSignatureHelp(
  ...
  if (const auto *TD = Template.getAsTemplateDecl()) {
    ...
    for (const NamedDecl *ND : *OTS)
      if (const auto *TD = toolchain::dyn_cast<TemplateDecl>(ND))
        Consider(TD);
  }
    ...
    if (D.isArrayDesignator() || D.isArrayRangeDesignator()) {
      ...
      if (RD && RD->isCompleteDefinition()) {
        for (const auto *Member : RD->lookup(D.getFieldDecl()))
          if (const FieldDecl *FD = toolchain::dyn_cast<FieldDecl>(Member)) {
            NextType = FD->getType();
            break;
          }
      }
...
void SemaCodeCompletion::CodeCompleteDesignator(
    QualType BaseType, toolchain::ArrayRef<Expr *> InitExprs, const Designation &D) {
...
void SemaCodeCompletion::CodeCompleteConstructorInitializer(
  ...
  toolchain::SmallPtrSet<FieldDecl *, 4> InitializedFields;
  toolchain::SmallPtrSet<CanQualType, 4> InitializedBases;
...
void SemaCodeCompletion::CodeCompleteLambdaIntroducer(Scope *S,
  ...
  toolchain::SmallPtrSet<IdentifierInfo *, 4> Known;
...
typedef toolchain::SmallPtrSet<Selector, 16> VisitedSelectorSet;
...
static ObjCInterfaceDecl *GetAssumedMessageSendExprType(Expr *E) {
  ...
  if (Method->isInstanceMethod())
    return toolchain::StringSwitch<ObjCInterfaceDecl *>(Id->getName())
        .Case("retain", IFace)
        .Case("strong", IFace)
...
void SemaCodeCompletion::CodeCompleteObjCInterfaceCategory(
  ...
  // interface.
  toolchain::SmallPtrSet<IdentifierInfo *, 16> CategoryNames;
  NamedDecl *CurClass = SemaRef.LookupSingleName(
      SemaRef.TUScope, ClassName, ClassNameLoc, Sema::LookupOrdinaryName);
...
typedef toolchain::DenseMap<Selector,
                       toolchain::PointerIntPair<ObjCMethodDecl *, 1, bool>>
    KnownMethodsMap;
...
void SemaCodeCompletion::CodeCompleteObjCMethodDecl(
  ...
  if (Context.getLangOpts().ObjC) {
    ...
    if (IFace)
      toolchain::append_range(Containers, IFace->visible_categories());
...
void SemaCodeCompletion::CodeCompleteIncludedFile(toolchain::StringRef Dir,
                                                  bool Angled) {
  // RelDir should use /, but unescaped \ is possible on windows!
  // Our completions will normalize to / for simplicity, this case is rare.
  std::string RelDir = toolchain::sys::path::convert_to_slash(Dir);
  ...
  toolchain::DenseSet<StringRef> SeenResults; // To deduplicate results.
  ...
  auto AddFilesFromIncludeDir = [&](StringRef IncludeDir,
    ...
    if (!NativeRelDir.empty()) {
      if (LookupType == DirectoryLookup::LT_Framework) {
        ...
        auto Begin = toolchain::sys::path::begin(NativeRelDir);
        auto End = toolchain::sys::path::end(NativeRelDir);

        toolchain::sys::path::append(Dir, *Begin + ".framework", "Headers");
        toolchain::sys::path::append(Dir, ++Begin, End);
      } else {
        toolchain::sys::path::append(Dir, NativeRelDir);
      }
    }

    const StringRef &Dirname = toolchain::sys::path::filename(Dir);
    ...
    for (auto It = FS.dir_begin(Dir, EC);
         !EC && It != toolchain::vfs::directory_iterator(); It.increment(EC)) {
      ...
      // symlinks.
      toolchain::sys::fs::file_type Type = It->type();
      if (Type == toolchain::sys::fs::file_type::symlink_file) {
...
void SemaCodeCompletion::CodeCompleteAvailabilityPlatformName() {
  ...
  for (const char *Platform : toolchain::ArrayRef(Platforms)) {
