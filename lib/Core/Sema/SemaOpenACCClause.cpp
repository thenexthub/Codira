...
class SemaOpenACCClauseVisitor {
  ...
  DiagGangWorkerVectorSeqConflict(SemaOpenACC::OpenACCParsedClause &Clause) {
    ...
    const auto *Itr =
        toolchain::find_if(ExistingClauses, toolchain::IsaPred<OpenACCSeqClause>);
  ...
  CheckModifierList(SemaOpenACC::OpenACCParsedClause &Clause,
    ...
    default:
      toolchain_unreachable("Only for copy, copyin, copyout, create");
    ...
    toolchain_unreachable("didn't return from switch above?");
  }
  ...
  CheckValidRoutineNewClauseHelper(Pred HasPredicate,
    ...
    auto *FirstDeviceType =
        toolchain::find_if(ExistingClauses, toolchain::IsaPred<OpenACCDeviceTypeClause>);
    ...
    if (FirstDeviceType == ExistingClauses.end()) {
      ...
      auto *ExistingProhibitedClause =
          toolchain::find_if(ExistingClauses, HasPredicate);
    ...
    auto LastDeviceTypeItr =
        std::find_if(ExistingClauses.rbegin(), ExistingClauses.rend(),
                     toolchain::IsaPred<OpenACCDeviceTypeClause>);
  ...
  bool CheckValidRoutineGangWorkerVectorSeqNewClause(
    ...
    if (Clause.getClauseKind() != OpenACCClauseKind::Gang &&
      ...
      return false;
    auto ProhibitedPred = toolchain::IsaPred<OpenACCGangClause, OpenACCWorkerClause,
  ...
  CheckValidRoutineBindNewClause(SemaOpenACC::OpenACCParsedClause &Clause) {
    ...
    auto HasBindPred = toolchain::IsaPred<OpenACCBindClause>;
...
public:
  ...
  OpenACCClause *Visit(SemaOpenACC::OpenACCParsedClause &Clause) {
    ...
    case OpenACCClauseKind::Shortloop:
      toolchain_unreachable("Shortloop shouldn't be generated in clang");
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitTileClause(
  ...
  toolchain::SmallVector<Expr *> NewSizeExprs;
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitIfClause(
  ...
  if (Clause.getDirectiveKind() != OpenACCDirectiveKind::Update) {
    const auto *Itr =
        toolchain::find_if(ExistingClauses, toolchain::IsaPred<OpenACCSelfClause>);
    if (Itr != ExistingClauses.end()) {
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitSelfClause(
  ...
  const auto *Itr =
      toolchain::find_if(ExistingClauses, toolchain::IsaPred<OpenACCIfClause>);
  if (Itr != ExistingClauses.end()) {
    SemaRef.Diag(Clause.getBeginLoc(), diag::warn_acc_if_self_conflict);
    SemaRef.Diag((*Itr)->getBeginLoc(), diag::note_acc_previous_clause_here)
        << (*Itr)->getClauseKind();
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitNumGangsClause(
  ...
  if (Clause.getIntExprs().size() > 1 &&
      isOpenACCCombinedDirectiveKind(Clause.getDirectiveKind())) {
    auto *GangClauseItr =
        toolchain::find_if(ExistingClauses, toolchain::IsaPred<OpenACCGangClause>);
    auto *ReductionClauseItr =
        toolchain::find_if(ExistingClauses, toolchain::IsaPred<OpenACCReductionClause>);
  ...
  if (Clause.getDirectiveKind() == OpenACCDirectiveKind::KernelsLoop) {
    auto GangClauses = toolchain::make_filter_range(
        ExistingClauses, toolchain::IsaPred<OpenACCGangClause>);
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitNumWorkersClause(
  ...
  // kernels construct.
  if (Clause.getDirectiveKind() == OpenACCDirectiveKind::KernelsLoop) {
    auto WorkerClauses = toolchain::make_filter_range(
        ExistingClauses, toolchain::IsaPred<OpenACCWorkerClause>);
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitVectorLengthClause(
  ...
  // 'kernels' construct.
  if (Clause.getDirectiveKind() == OpenACCDirectiveKind::KernelsLoop) {
    auto VectorClauses = toolchain::make_filter_range(
        ExistingClauses, toolchain::IsaPred<OpenACCVectorClause>);
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitAttachClause(
  ...
  toolchain::SmallVector<Expr *> VarList{Clause.getVarList()};
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitDeviceTypeClause(
  ...
  const std::array<toolchain::StringLiteral, 6> ValidValues{
  ...
  toolchain::SmallVector<DeviceTypeArgument> Architectures{
  ...
  auto FilterPred = [&](const DeviceTypeArgument &Arch) {
    ...
    if (!Arch.getIdentifierInfo())
      return false;
    return toolchain::find_if(ValidValues, [&](StringRef RHS) {
  ...
  // anything.
  toolchain::for_each(toolchain::make_filter_range(Architectures, FilterPred), Diagnose);
  if (Diagnosed)
    toolchain::erase_if(Architectures, FilterPred);
...
ExprResult CheckGangParallelExpr(SemaOpenACC &S, OpenACCDirectiveKind DK,
  ...
  toolchain_unreachable("Unknown gang kind in gang parallel check");
}
...
ExprResult CheckGangKernelsExpr(SemaOpenACC &S,
  ...
  case OpenACCGangKind::Num: {
    ...
    const auto *Itr =
        toolchain::find_if(Collection, toolchain::IsaPred<OpenACCNumGangsClause>);
  ...
  toolchain_unreachable("Unknown gang kind in gang kernels check");
}
...
ExprResult CheckGangSerialExpr(SemaOpenACC &S, OpenACCDirectiveKind DK,
  ...
  toolchain_unreachable("Unknown gang kind in gang serial check");
}
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitVectorClause(
  ...
  if (IntExpr) {
    ...
    default:
      toolchain_unreachable("Invalid directive kind for this clause");
    case OpenACCDirectiveKind::Loop:
      ...
      case OpenACCDirectiveKind::KernelsLoop: {
        const auto *Itr =
            toolchain::find_if(SemaRef.getActiveComputeConstructInfo().Clauses,
                          toolchain::IsaPred<OpenACCVectorLengthClause>);
      ...
      default:
        toolchain_unreachable("Non compute construct in active compute construct");
      }
      break;
    case OpenACCDirectiveKind::KernelsLoop: {
      const auto *Itr = toolchain::find_if(ExistingClauses,
                                      toolchain::IsaPred<OpenACCVectorLengthClause>);
      if (Itr != ExistingClauses.end()) {
        SemaRef.Diag(IntExpr->getBeginLoc(), diag::err_acc_num_arg_conflict)
            << "length" << OpenACCClauseKind::Vector
  ...
  if (IntExpr) {
    ...
    case OpenACCDirectiveKind::Loop:
      ...
      case OpenACCDirectiveKind::Kernels: {
        const auto *Itr =
            toolchain::find_if(SemaRef.getActiveComputeConstructInfo().Clauses,
                          toolchain::IsaPred<OpenACCNumWorkersClause>);
        if (Itr != SemaRef.getActiveComputeConstructInfo().Clauses.end()) {
          SemaRef.Diag(IntExpr->getBeginLoc(), diag::err_acc_num_arg_conflict)
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitGangClause(
  ...
  if ((Clause.getDirectiveKind() == OpenACCDirectiveKind::Loop &&
    ...
    auto *NumGangsClauseItr = toolchain::find_if(
        ActiveComputeConstructContainer, toolchain::IsaPred<OpenACCNumGangsClause>);
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitSeqClause(
  ...
  if (Clause.getDirectiveKind() == OpenACCDirectiveKind::Loop ||
      isOpenACCCombinedDirectiveKind(Clause.getDirectiveKind())) {
    const auto *Itr = toolchain::find_if(
        ExistingClauses, toolchain::IsaPred<OpenACCGangClause, OpenACCVectorClause,
                                       OpenACCWorkerClause>);
    if (Itr != ExistingClauses.end()) {
      SemaRef.Diag(Clause.getBeginLoc(), diag::err_acc_clause_cannot_combine)
          << Clause.getClauseKind() << (*Itr)->getClauseKind()
...
OpenACCClause *SemaOpenACCClauseVisitor::VisitReductionClause(
  ...
  if (Clause.getDirectiveKind() == OpenACCDirectiveKind::Parallel ||
      Clause.getDirectiveKind() == OpenACCDirectiveKind::ParallelLoop) {
    auto NumGangsClauses = toolchain::make_filter_range(
        ExistingClauses, toolchain::IsaPred<OpenACCNumGangsClause>);
...
bool areVarsEqual(Expr *VarExpr1, Expr *VarExpr2) {
  ...
  toolchain_unreachable("Unknown variable type encountered");
}
} // namespace
...
SemaOpenACC::ActOnClause(ArrayRef<const OpenACCClause *> ExistingClauses,
  ...
  if (const auto *DevTypeClause = toolchain::find_if(
          ExistingClauses, toolchain::IsaPred<OpenACCDeviceTypeClause>);
...
SemaOpenACC::CheckGangExpr(ArrayRef<const OpenACCClause *> ExistingClauses,
  ...
  case OpenACCDirectiveKind::Invalid:
    ...
    return ExprError();
  default:
    toolchain_unreachable("Invalid directive kind for a Gang clause");
  }
  toolchain_unreachable("Compute construct directive not handled?");
}
...
SemaOpenACC::CheckGangClause(OpenACCDirectiveKind DirKind,
  ...
  if (DirKind != OpenACCDirectiveKind::Routine) {
    ...
    if (ReductionItr != ExistingClauses.end()) {
      const auto GangZip = toolchain::zip_equal(GangKinds, IntExprs);
      const auto GangItr = toolchain::find_if(GangZip, [](const auto &Tuple) {
...
OpenACCClause *SemaOpenACC::CheckReductionClause(
  ...
  if (DirectiveKind == OpenACCDirectiveKind::Loop ||
    ...
    // that has a gang clause with a dim: argument whose value is greater
    // than 1.
    const auto GangClauses = toolchain::make_filter_range(
        ExistingClauses, toolchain::IsaPred<OpenACCGangClause>);
...
SemaOpenACC::CheckLinkClauseVarList(ArrayRef<Expr *> VarExprs) {
  ...
  if (isa<NamespaceDecl, TranslationUnitDecl>(DC))
    return toolchain::SmallVector<Expr *>(VarExprs);

  toolchain::SmallVector<Expr *> NewVarList;
...
bool SemaOpenACC::CheckDeclareClause(SemaOpenACC::OpenACCParsedClause &Clause,
  ...
  toolchain::SmallVector<Expr *> FilteredVarList;
