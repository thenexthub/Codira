...
#include "language/Core/Lex/HeaderSearch.h" // TODO: Sema shouldn't depend on Lex
#include "language/Core/Lex/Lexer.h" // TODO: Extract static functions to fix layering.
#include "language/Core/Lex/ModuleLoader.h" // TODO: Sema shouldn't depend on Lex
#include "language/Core/Lex/Preprocessor.h" // Included for isCodeCompletionEnabled()
...
#include "language/Core/Sema/SemaSwift.h"
...
synthesizeCurrentNestedNameSpecifier(ASTContext &Context, DeclContext *DC) {
  ...
  toolchain_unreachable("something isn't in TU scope?");
}
  ...
  if (TypoCorrection Corrected =
    ...
    if (Corrected.isKeyword()) {
      ...
      if (!SS || !SS->isSet()) {
        ...
        toolchain_unreachable("could not have corrected a typo here");
      }
...
static bool AllowOverloadingOfFunction(const LookupResult &Previous,
  ...
  if (Previous.getResultKind() == LookupResultKind::FoundOverloaded) {
    return toolchain::any_of(Previous, [](const NamedDecl *ND) {
...
bool Sema::CheckRedeclarationExported(NamedDecl *New, NamedDecl *Old) {
  ...
  // https://github.com/toolchain/toolchain-project/issues/98583 for details.
...
static StringRef getHeaderName(Builtin::Context &BuiltinInfo, unsigned ID,
  ...
  toolchain_unreachable("unhandled error kind");
}
...
#include "language/Core/Sema/AttrParsedAttrImpl.inc"
...
static void diagnoseMissingConstinit(Sema &S, const VarDecl *InitDecl,
  ...
  if (SuitableSpelling.empty() && S.getLangOpts().CPlusPlus11)
    SuitableSpelling = "[[language::Core::require_constant_initialization]]";
  if (SuitableSpelling.empty())
  ...
  if (const auto *NewAbiTagAttr = New->getAttr<AbiTagAttr>()) {
    if (const auto *OldAbiTagAttr = Old->getAttr<AbiTagAttr>()) {
      for (const auto &NewTag : NewAbiTagAttr->tags()) {
        if (!toolchain::is_contained(OldAbiTagAttr->tags(), NewTag)) {
...
bool Sema::MergeFunctionDecl(FunctionDecl *New, NamedDecl *&OldD, Scope *S,
  ...
  if (!getLangOpts().CPlusPlus) {
    ...
    if (OldOvl != New->hasAttr<OverloadableAttr>() && !Old->isImplicit()) {
      ...
      if (OldOvl) {
        auto OldIter = toolchain::find_if(Old->redecls(), [](const Decl *D) {
...
void Sema::setTagNameForLinkagePurposes(TagDecl *TagFromDeclSpec,
  ...
  if (NonCLike || ChangesLinkage) {
    ...
    toolchain::SmallString<40> TextToInsert;
    TextToInsert += ' ';
...
static unsigned GetDiagnosticTypeSpecifierID(const DeclSpec &DS) {
  ...
  case DeclSpec::TST_enum:
    ...
    return 4;
  default:
    toolchain_unreachable("unexpected type specifier");
  }
}
...
Decl *Sema::ParsedFreeStandingDeclSpec(Scope *S, AccessSpecifier AS,
  ...
  if (!DS.getAttributes().empty() || !DeclAttrs.empty()) {
    ...
    if (TypeSpecType == DeclSpec::TST_class ||
      ...
      toolchain::for_each(DS.getAttributes(), EmitAttributeDiagnostic);
      toolchain::for_each(DeclAttrs, EmitAttributeDiagnostic);
    }
  }
...
StorageClassSpecToVarDeclStorageClass(const DeclSpec &DS) {
  ...
  toolchain_unreachable("unknown storage class specifier");
}
...
static SourceLocation findDefaultInitializer(const CXXRecordDecl *Record) {
  ...
  toolchain_unreachable("couldn't find in-class initializer");
}
...
static QualType TryToFixInvalidVariablyModifiedType(QualType T,
  ...
  toolchain::APSInt Res = Result.Val.getInt();
...
static bool shouldConsiderLinkage(const VarDecl *VD) {
  ...
  if (isa<RequiresExprBodyDecl>(DC))
    return false;
  toolchain_unreachable("Unexpected context");
}
...
static bool isDeclExternC(const Decl *D) {
  ...
  toolchain_unreachable("Unknown type of decl!");
}
...
NamedDecl *Sema::ActOnVariableDeclarator(
  ...
  if (!getLangOpts().CPlusPlus) {
    ...
    if (CurContext->isRecord()) {
      ...
      case SC_PrivateExtern:
        toolchain_unreachable("C storage class in c++!");
      }
    }
  ...
  if (Expr *E = D.getAsmLabel()) {
    ...
    toolchain::DenseMap<IdentifierInfo*,AsmLabelAttr*>::iterator I =
...
static bool CheckC23ConstexprVarType(Sema &SemaRef, SourceLocation VarLoc,
  ...
  if (CanonT->isRecordType()) {
    ...
    if (!RD->isInvalidDecl() &&
        toolchain::any_of(RD->fields(), [&SemaRef, VarLoc](const FieldDecl *F) {
...
bool Sema::AddOverriddenMethods(CXXRecordDecl *DC, CXXMethodDecl *MD) {
  toolchain::SmallPtrSet<const CXXMethodDecl*, 4> Overridden;
...
static StorageClass getFunctionStorageClass(Sema &SemaRef, Declarator &D) {
  ...
  default: toolchain_unreachable("Unknown storage class!");
...
static bool isOpenCLSizeDependentType(ASTContext &C, QualType Ty) {
  ...
  do {
    ...
    auto Match = toolchain::find(Names, DesugaredTy.getUnqualifiedType().getAsString());
    if (Names.end() != Match)
      return true;
...
static void checkIsValidOpenCLKernelParameter(
  ...
  Declarator &D,
  ParmVarDecl *Param,
  toolchain::SmallPtrSetImpl<const Type *> &ValidTypes) {
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  toolchain::append_range(TemplateParamLists, TemplateParamListsRef);
  ...
  if (getLangOpts().OpenCL && NewFD->hasAttr<DeviceKernelAttr>()) {
    ...
    toolchain::SmallPtrSet<const Type *, 16> ValidTypes;
...
/// https://reviews.toolchain.org/D22931, the Microsoft feedback page is no longer
...
static bool CheckDeclarationCausesMultiVersioning(Sema &S, FunctionDecl *OldFD,
  ...
  if (NewTA) {
    ...
    toolchain::sort(OldParsed.Features);
    ParsedTargetAttr NewParsed =
        S.getASTContext().getTargetInfo().parseTargetAttr(
            NewTA->getFeaturesStr());
    // Sort order doesn't matter, it just needs to be consistent.
    toolchain::sort(NewParsed.Features);
...
static bool CheckMultiVersionAdditionalDecl(
  ...
  toolchain::SmallVector<StringRef, 8> NewFeats;
  if (NewTVA) {
    NewTVA->getFeatures(NewFeats);
    toolchain::sort(NewFeats);
  }
  ...
  for (NamedDecl *ND : Previous) {
    ...
    case MultiVersionKind::Target: {
      ...
      toolchain::sort(CurParsed.Features);
    ...
    case MultiVersionKind::TargetVersion: {
      if (const auto *CurTVA = CurFD->getAttr<TargetVersionAttr>()) {
        ...
        toolchain::SmallVector<StringRef, 8> CurFeats;
        CurTVA->getFeatures(CurFeats);
        toolchain::sort(CurFeats);
...
bool Sema::CheckFunctionDeclaration(Scope *S, FunctionDecl *NewFD,
  ...
  if (Redeclaration) {
    ...
    assert((Previous.empty() ||
            toolchain::any_of(Previous,
                         [](const NamedDecl *ND) {
                           return ND->hasAttr<OverloadableAttr>();
                         })) &&
           "Non-redecls shouldn't happen without overloadable present");

    auto OtherUnmarkedIter = toolchain::find_if(Previous, [](const NamedDecl *ND) {
...
static bool isDefaultStdCall(FunctionDecl *FD, Sema &S) {
  ...
  const toolchain::Triple &T = S.Context.getTargetInfo().getTriple();
  if (T.isOSCygMing())
    return false;

  // Default calling convention for WinMain, wWinMain and DllMain
  // is __stdcall on 32 bit Windows
  if (T.isOSWindows() && T.getArch() == toolchain::Triple::x86)
    return true;
...
namespace {
  ...
  public:
    ...
    bool CheckInitListMemberExpr(MemberExpr *E, bool CheckReference) {
      ...
      for (const FieldDecl *I : toolchain::reverse(Fields))
    ...
    void VisitCXXOperatorCallExpr(CXXOperatorCallExpr *E) {
      toolchain::SaveAndRestore CxxOpCallScope(isInCXXOperatorCall, true);
...
bool Sema::GloballyUniqueObjectMightBeAccidentallyDuplicated(
  ...
  language::Core::LinkageInfo Lnk = Target->getLinkageAndVisibility();
...
void Sema::FinalizeDeclaration(Decl *ThisDecl) {
  ...
  for (const auto *I : ThisDecl->specific_attrs<TypeTagForDatatypeAttr>()) {
    ...
    std::optional<toolchain::APSInt> MagicValueInt;
    ...
    for (int i = FTI.NumParams; i != 0; /* decrement in loop */) {
      ...
      if (FTI.Params[i].Param == nullptr) {
        if (getLangOpts().C99) {
          ...
          toolchain::raw_svector_ostream(Code)
...
static void diagnoseImplicitlyRetainedSelf(Sema &S) {
  toolchain::DenseMap<const BlockDecl *, bool> EscapeInfo;
...
void Sema::AddKnownFunctionAttributes(FunctionDecl *FD) {
  ...
  if (unsigned BuiltinID = FD->getBuiltinID()) {
    ...
    // C standard.
    const toolchain::Triple &Trip = Context.getTargetInfo().getTriple();
...
static unsigned getRedeclDiagFromTagKind(TagTypeKind Tag) {
  ...
  case TagTypeKind::Class:
    return 2;
  default: toolchain_unreachable("Invalid tag kind for redecl diagnostic!");
  }
}
...
NonTagKind Sema::getNonTagTypeDeclKind(const Decl *PrevDecl, TagTypeKind TTK) {
  ...
  toolchain_unreachable("invalid TTK");
}
...
static FixItHint createFriendTagNNSFixIt(Sema &SemaRef, NamedDecl *ND, Scope *S,
  ...
  toolchain::raw_svector_ostream OS(Insertion);
  if (DC->isTranslationUnit())
    OS << "::";
...
Sema::ActOnTag(Scope *S, unsigned TagSpec, TagUseKind TUK, SourceLocation KWLoc,
  ...
  // redeclaration.
  toolchain::PointerUnion<const Type*, TypeSourceInfo*> EnumUnderlying;
...
void Sema::ActOnLastBitfield(SourceLocation DeclLoc,
  ...
  toolchain::APInt Zero(Context.getTypeSize(Context.IntTy), 0);
  Expr * BW = IntegerLiteral::Create(Context, Zero, Context.IntTy, DeclLoc);
  Expr *BitWidth =
      ConstantExpr::Create(Context, BW, APValue(toolchain::APSInt(Zero)));
...
static bool AreSpecialMemberFunctionsSameKind(ASTContext &Context,
  ...
  // https://github.com/toolchain/toolchain-project/issues/59206
...
bool Sema::EntirelyFunctionPointers(const RecordDecl *Record) {
  ...
  return toolchain::all_of(Record->decls(), IsFunctionPointerOrForwardDecl);
}
...
EnumConstantDecl *Sema::CheckEnumConstant(EnumDecl *Enum,
  ...
  toolchain::APSInt EnumVal(IntWidth);
  QualType EltTy;
...
static void CheckForDuplicateEnumValues(Sema &S, ArrayRef<Decl *> Elements,
  ...
  typedef toolchain::PointerUnion<EnumConstantDecl*, ECDVector*> DeclOrVector;
  ...
  auto EnumConstantToKey = [](const EnumConstantDecl *D) {
    toolchain::APSInt Val = D->getInitVal();
  ...
  for (const auto &Vec : DupVector) {
    ...
    // the same value.
    for (auto *ECD : toolchain::drop_begin(*Vec))
...
bool Sema::IsValueInFlagEnum(const EnumDecl *ED, const toolchain::APInt &Val,
  ...
  toolchain::APInt FlagMask = ~FlagBits.zextOrTrunc(Val.getBitWidth());
