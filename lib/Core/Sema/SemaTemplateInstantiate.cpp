...
getEnclosingTypeAliasTemplateDecl(Sema &SemaRef) {
  for (auto &CSC : toolchain::reverse(SemaRef.CodeSynthesisContexts)) {
...
Response HandleFunction(Sema &SemaRef, const FunctionDecl *Function,
  ...
  if (!RelativeToPrimary &&
    ...
    if (ForConstraintInstantiation) {
      for (auto &Inst : toolchain::reverse(SemaRef.CodeSynthesisContexts)) {
...
bool Sema::CodeSynthesisContext::isInstantiationRecord() const {
  ...
  toolchain_unreachable("Invalid SynthesisKind!");
}
...
static std::string convertCallArgsToString(Sema &S,
                                           toolchain::ArrayRef<const Expr *> Args) {
  std::string Result;
  toolchain::raw_string_ostream OS(Result);
  toolchain::ListSeparator Comma;
...
void Sema::PrintInstantiationStack(InstantiationContextDiagFuncRef DiagFunc) {
  ...
  for (SmallVectorImpl<CodeSynthesisContext>::reverse_iterator
    ...
    case CodeSynthesisContext::DefaultTemplateArgumentInstantiation: {
      ...
      toolchain::raw_svector_ostream OS(TemplateArgsStr);
    ...
    case CodeSynthesisContext::DeducedTemplateArgumentSubstitution: {
      if (FunctionTemplateDecl *FnTmpl =
        ...
        if (auto *D = dyn_cast<TemplateDecl>(Active->Entity)) {
          ...
          toolchain_unreachable("unexpected template kind");
        }
      ...
      DiagFunc(Active->PointOfInstantiation,
               PDiag(diag::note_building_builtin_dump_struct_call)
                   << convertCallArgsToString(
                          *this, toolchain::ArrayRef(Active->CallArgs,
                                                Active->NumCallArgs)));
      break;
...
namespace {
  ...
  public:
    ...
    bool TransformTemplateArgument(const TemplateArgumentLoc &Input,
      ...
      case TemplateArgument::Pack:
        ...
        Output = SemaRef.getTrivialTemplateArgumentLoc(
            TemplateArgument(toolchain::ArrayRef(TArgs).copy(SemaRef.Context)),
    ...
    StmtResult TransformLambdaBody(LambdaExpr *E, Stmt *Body) {
      ...
      // p0588r1.
      toolchain::SaveAndRestore _(EvaluateConstraints, true);
...
TemplateInstantiator::TransformOpenACCRoutineDeclAttr(
  ...
  toolchain_unreachable("RoutineDecl should only be a declaration attribute, as it "
...
TemplateInstantiator::TransformFunctionParmPackRefExpr(DeclRefExpr *E,
  ...
  toolchain::PointerUnion<Decl *, DeclArgumentPack *> *Found
...
createSubstDiag(Sema &S, TemplateDeductionInfo &Info,
  ...
  toolchain::raw_svector_ostream OS(Entity);
  Printer(OS);
...
ExprResult TemplateInstantiator::TransformRequiresTypeParams(
  ...
  if (getDerived().TransformFunctionTypeParams(
    ...
    TransReqs.push_back(RebuildTypeRequirement(createSubstDiag(
        SemaRef, Info, [&](toolchain::raw_ostream &OS) { OS << *FailedDecl; })));
...
TemplateInstantiator::TransformExprRequirement(concepts::ExprRequirement *Req) {
  ...
  toolchain::PointerUnion<Expr *, concepts::Requirement::SubstitutionDiagnostic *>
      TransExpr;
  if (Req->isExprSubstitutionFailure())
    ...
    if (TransExprRes.isInvalid() || Trap.hasErrorOccurred())
      TransExpr = createSubstDiag(SemaRef, Info, [&](toolchain::raw_ostream &OS) {
...
TemplateInstantiator::TransformNestedRequirement(
  ...
  TemplateDeductionInfo Info(Req->getConstraintExpr()->getBeginLoc());
  {
    ...
    if (ConstrInst.isInvalid())
      return nullptr;
    toolchain::SmallVector<Expr *> Result;
...
Sema::InstantiateClass(SourceLocation PointOfInstantiation,
  ...
  toolchain::TimeTraceScope TimeScope("InstantiateClass", [&]() {
...
toolchain::PointerUnion<Decl *, LocalInstantiationScope::DeclArgumentPack *> *
...
void LocalInstantiationScope::InstantiatedLocal(const Decl *D, Decl *Inst) {
  D = getCanonicalParmVarDecl(D);
  toolchain::PointerUnion<Decl *, DeclArgumentPack *> &Stored = LocalDecls[D];
  if (Stored.isNull()) {
#ifndef NDEBUG
...
bool LocalInstantiationScope::isLocalPackExpansion(const Decl *D) {
  for (DeclArgumentPack *Pack : ArgumentPacks)
    if (toolchain::is_contained(*Pack, D))
      return true;
  return false;
}
