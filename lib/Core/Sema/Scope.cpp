...
LLVM_DUMP_METHOD void Scope::dump() const { dumpImpl(toolchain::errs()); }
...
void Scope::dumpImpl(raw_ostream &OS) const {
  ...
  if (const Scope *Parent = getParent())
    OS << "Parent: (language::Core::Scope*)" << Parent << '\n';

  OS << "Depth: " << Depth << '\n';
  OS << "MSLastManglingNumber: " << getMSLastManglingNumber() << '\n';
  OS << "MSCurManglingNumber: " << getMSCurManglingNumber() << '\n';
  if (const DeclContext *DC = getEntity())
    OS << "Entity : (language::Core::DeclContext*)" << DC << '\n';

  if (!NRVO)
    OS << "there is no NRVO candidate\n";
  else if (*NRVO)
    OS << "NRVO candidate : (language::Core::VarDecl*)" << *NRVO << '\n';
  else
