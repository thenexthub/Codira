...
class ExtractTypeForDeductionGuide
  ...
  toolchain::SmallVectorImpl<TypedefNameDecl *> &MaterializedTypedefs;
...
buildDeductionGuide(Sema &SemaRef, TemplateDecl *OriginalTemplate,
                    ...
                    toolchain::ArrayRef<TypedefNameDecl *> MaterializedTypedefs = {},
...
NamedDecl *transformTemplateParameter(Sema &SemaRef, DeclContext *DC,
  ...
  toolchain_unreachable("Unhandled template parameter types");
}
...
SmallVector<unsigned> TemplateParamsReferencedInTemplateArgumentList(
  ...
  toolchain::SmallBitVector ReferencedTemplateParams(TemplateParamsList->size());
...
toolchain::DenseSet<const NamedDecl *> getSourceDeductionGuides(DeclarationName Name,
                                                           DeclContext *DC) {
  assert(Name.getNameKind() ==
             DeclarationName::NameKind::CXXDeductionGuideName &&
         "name must be a deduction guide name");
  toolchain::DenseSet<const NamedDecl *> Result;
...
buildAssociatedConstraints(Sema &SemaRef, FunctionTemplateDecl *F,
  ...
  ArgsForBuildingRC.setKind(language::Core::TemplateSubstitutionKind::Rewrite);
  ...
  if (F->getLexicalDeclContext()->getDeclKind() ==
      language::Core::Decl::ClassTemplateSpecialization) {
...
std::pair<TemplateDecl *, toolchain::ArrayRef<TemplateArgument>>
getRHSTemplateDeclAndArgs(Sema &SemaRef, TypeAliasTemplateDecl *AliasTemplate) {
  ...
  toolchain::ArrayRef<TemplateArgument> AliasRhsTemplateArgs;
...
bool IsNonDeducedArgument(const TemplateArgument &TA) {
  ...
  return TA.isNull() ||
         (TA.getKind() == TemplateArgument::Pack &&
          toolchain::any_of(TA.pack_elements(), IsNonDeducedArgument));
}
...
void DeclareImplicitDeductionGuidesForTypeAlias(
  ...
  LookupResult Guides(SemaRef, NameInfo, language::Core::Sema::LookupOrdinaryName);
...
FunctionTemplateDecl *DeclareAggregateDeductionGuideForTypeAlias(
  ...
  toolchain::SmallVector<TypedefNameDecl *> TypedefDecls;
  toolchain::SmallVector<QualType> NewParamTypes;
...
FunctionTemplateDecl *Sema::DeclareAggregateDeductionGuideFromInitList(
  ...
  if (auto *AliasTemplate = toolchain::dyn_cast<TypeAliasTemplateDecl>(Template)) {
...
void Sema::DeclareImplicitDeductionGuides(TemplateDecl *Template,
  ...
  ClassTemplateDecl *Pattern =
      Transform.NestedPattern ? Transform.NestedPattern : Transform.Template;
  ContextRAII SavedContext(*this, Pattern->getTemplatedDecl());
  toolchain::SmallPtrSet<NamedDecl *, 8> ProcessedCtors;
  bool AddedAny = false;
  for (NamedDecl *D : LookupConstructors(Pattern->getTemplatedDecl())) {
    ...
    if (toolchain::any_of(CD->parameters(), [](ParmVarDecl *P) {
