...
bool SemaARM::BuiltinARMSpecialReg(unsigned BuiltinID, CallExpr *TheCall,
  ...
  if (Fields.size() > 1) {
    ...
    auto MaxLimit = toolchain::StringSwitch<std::optional<unsigned>>(Reg)
                        .CaseLower("spsel", 15)
...
bool SemaARM::CheckImmediateArg(CallExpr *TheCall, unsigned CheckTy,
  ...
  auto CheckImmediateInSet = [&](std::initializer_list<int64_t> Set,
    ...
    toolchain::APSInt Imm;
    if (SemaRef.BuiltinConstantArg(TheCall, ArgIdx, Imm))
      return true;

    if (!toolchain::is_contained(Set, Imm.getSExtValue()))
...
static bool checkArmStreamingBuiltin(Sema &S, CallExpr *TheCall,
  ...
  if (BuiltinType == SemaARM::VerifyRuntimeMode) {
    toolchain::StringMap<bool> CallerFeatures;
...
#include "language/Core/Basic/arm_sme_builtins_za_state.inc"
...
#include "language/Core/Basic/arm_sme_streaming_attrs.inc"
...
#include "language/Core/Basic/arm_sme_sema_rangechecks.inc"
...
#include "language/Core/Basic/arm_sve_streaming_attrs.inc"
...
#include "language/Core/Basic/arm_sve_sema_rangechecks.inc"
...
#undef GET_NEON_OVERLOAD_CHECK
  ...
  if (PtrArgNum >= 0) {
    ...
    toolchain::Triple::ArchType Arch = TI.getTriple().getArch();
    bool IsPolyUnsigned = Arch == toolchain::Triple::aarch64 ||
                          Arch == toolchain::Triple::aarch64_32 ||
                          Arch == toolchain::Triple::aarch64_be;
...
bool SemaARM::CheckMVEBuiltinFunctionCall(unsigned BuiltinID,
  ...
  default:
    return false;
#include "language/Core/Basic/arm_mve_builtin_sema.inc"
  }
}
...
bool SemaARM::CheckCDEBuiltinFunctionCall(const TargetInfo &TI,
  ...
  default:
    return false;
#include "language/Core/Basic/arm_cde_builtin_sema.inc"
  }
...
bool SemaARM::CheckARMCoprocessorImmediate(const TargetInfo &TI,
  ...
  toolchain::APSInt CoprocNoAP = *CoprocArg->getIntegerConstantExpr(Context);
...
bool SemaARM::CheckARMBuiltinExclusiveCall(const TargetInfo &TI,
  ...
  if (!TI.getTriple().isAArch64()) {
    ...
    bool Supported =
        (toolchain::isPowerOf2_64(Bits)) && Bits >= 8 && (Mask & (Bits / 8));
...
static bool BuiltinAliasValid(unsigned BuiltinID, StringRef AliasName,
  ...
  const IntrinToName *It =
      toolchain::lower_bound(Map, BuiltinID, [](const IntrinToName &L, unsigned Id) {
        return L.Id < Id;
      });
...
#include "language/Core/Basic/arm_mve_builtin_aliases.inc"
...
#include "language/Core/Basic/arm_cde_builtin_aliases.inc"
  return BuiltinAliasValid(BuiltinID, AliasName, Map, IntrinNames);
}
...
void SemaARM::handleNewAttr(Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned I = 0, E = AL.getNumArgs(); I != E; ++I) {
    ...
    if (!toolchain::is_contained(NewState, StateName)) // Avoid adding duplicates.
  ...
  if (FD->hasAttr<ArmLocallyStreamingAttr>()) {
    if (FD->getReturnType()->isSizelessVectorType())
      Diag(FD->getLocation(),
          ...
          << /*IsArg=*/false;
    if (toolchain::any_of(FD->parameters(), [](ParmVarDecl *P) {
...
bool SemaARM::checkTargetClonesAttr(
  ...
  for (unsigned I = 0, E = Params.size(); I < E; ++I) {
    ...
    toolchain::SmallVector<StringRef, 8> Features;
    toolchain::SmallVector<StringRef, 8> ValidFeatures;
    Param.split(Features, '+');
    for (StringRef Feat : Features) {
      Feat = Feat.trim();
      if (!getASTContext().getTargetInfo().validateCpuSupports(Feat)) {
    ...
    toolchain::sort(ValidFeatures);
    SmallString<64> NewParam(toolchain::join(ValidFeatures, "+"));
    if (toolchain::is_contained(NewParams, NewParam)) {
      Diag(Loc, diag::warn_target_clone_duplicate_options);
      continue;
    }
