...
using namespace toolchain::hlsl;

using language::Core::hlsl::BuiltinTypeDeclBuilder;
...
void HLSLExternalSemaSource::defineHLSLVectorAlias() {
  ...
  toolchain::SmallVector<NamedDecl *> TemplateParams;
  ...
  toolchain::APInt Val(AST.getIntWidth(AST.IntTy), 4);
  TemplateArgument Default(AST, toolchain::APSInt(std::move(Val)), AST.IntTy,
                           /*IsDefaulted=*/true);
...
static Expr *constructStructuredBufferConstraintExpr(Sema &S,
  ...
  IntegerLiteral *rhs = IntegerLiteral::Create(
      Context, toolchain::APInt(Context.getTypeSize(Context.getSizeType()), 1, true),
