...
#include "language/Core/Sema/CXXFieldCollector.h"
...
Sema::ImplicitExceptionSpecification::CalledDecl(SourceLocation CallLoc,
  ...
  case EST_Unevaluated:
    toolchain_unreachable("should not see unresolved exception specs here");
...
static bool functionDeclHasDefaultArgument(const FunctionDecl *FD) {
  return toolchain::any_of(FD->parameters(), [](ParmVarDecl *P) {
  ...
  auto &DS = D.getDeclSpec();
  {
    ...
    if (!BadSpecifiers.empty()) {
      ...
      Err << (int)BadSpecifiers.size()
          << toolchain::join(BadSpecifiers.begin(), BadSpecifiers.end(), " ");
      ...
      Warn << (int)CPlusPlus20Specifiers.size()
           << toolchain::join(CPlusPlus20Specifiers.begin(),
...
static bool CheckBindingsCount(Sema &S, DecompositionDecl *DD,
  ...
  auto BindingWithPackItr = toolchain::find_if(
...
static bool checkSimpleDecomposition(
    ...
    QualType DecompType, const toolchain::APSInt &NumElemsAPS, QualType ElemType,
    toolchain::function_ref<ExprResult(SourceLocation, Expr *, unsigned)> GetInit) {
...
static bool checkArrayLikeDecomposition(Sema &S,
                                        ...
                                        const toolchain::APSInt &NumElems,
                                        QualType ElemType) {
...
static bool checkVectorDecomposition(Sema &S, ArrayRef<BindingDecl*> Bindings,
  ...
  return checkArrayLikeDecomposition(
      S, Bindings, Src, DecompType, toolchain::APSInt::get(VT->getNumElements()),
...
static bool checkComplexDecomposition(Sema &S,
  ...
  return checkSimpleDecomposition(
      S, Bindings, Src, DecompType, toolchain::APSInt::get(2),
...
static bool checkTupleLikeDecomposition(Sema &S,
                                        ...
                                        const toolchain::APSInt &TupleSize) {
  auto *DD = cast<DecompositionDecl>(Src);
...
static bool checkMemberDecomposition(Sema &S, ArrayRef<BindingDecl*> Bindings,
  ...
  auto *DD = cast<DecompositionDecl>(Src);
  unsigned NumFields = toolchain::count_if(
  ...
  assert(toolchain::range_size(FlatBindings) == NumFields);
...
UnsignedOrNone Sema::GetDecompositionElementCount(QualType T,
  ...
  toolchain::APSInt TupleSize(Ctx.getTypeSize(Ctx.getSizeType()));
...
static bool CheckLiteralType(Sema &SemaRef, Sema::CheckConstexprKind Kind,
  ...
  toolchain_unreachable("unknown CheckConstexprKind");
}
...
static unsigned getRecordDiagFromTagKind(TagTypeKind Tag) {
  ...
  case TagTypeKind::Class:
    return 2;
  default: toolchain_unreachable("Invalid tag kind for record diagnostic!");
  }
}
...
static bool CheckConstexprCtorInitializer(Sema &SemaRef,
                                          ...
                                          FieldDecl *Field,
                                          toolchain::SmallSet<Decl*, 16> &Inits,
                                          bool &Diagnosed,
                                          Sema::CheckConstexprKind Kind) {
...
void Sema::DiagnoseImmediateEscalatingReason(FunctionDecl *FD) {
  ...
  struct ImmediateEscalatingExpressionsVisitor : DynamicRecursiveASTVisitor {
    ...
    bool TraverseConstructorInitializer(CXXCtorInitializer *Init) override {
      toolchain::SaveAndRestore RAII(CurrentInit, Init);
    ...
    bool TraverseCXXConstructorDecl(CXXConstructorDecl *Ctr) override {
      toolchain::SaveAndRestore RAII(CurrentConstructor, Ctr);
  ...
  if (BaseDecl) {
    ...
    if (BaseType.hasQualifiers()) {
      ...
      Diag(BaseLoc, diag::warn_qual_base_type)
          << Quals << toolchain::count(Quals, ' ') + 1 << BaseType;
...
typedef toolchain::SmallPtrSet<QualType, 4> IndirectBaseSet;
...
namespace {
  class UninitializedFieldVisitor
    ...
    toolchain::SmallVector<unsigned, 4> InitFieldIndex;
  ...
  public:
    ...
    bool IsInitListMemberExprInitialized(MemberExpr *ME,
      ...
      toolchain::SmallVector<FieldDecl*, 4> Fields;
      ...
      toolchain::SmallVector<unsigned, 4> UsedFieldIndex;
      // Discard the first field since it is the field decl that is being
      // initialized.
      for (const FieldDecl *FD : toolchain::drop_begin(toolchain::reverse(Fields)))
  ...
  static void DiagnoseUninitializedFields(
    ...
    toolchain::SmallPtrSet<ValueDecl*, 4> UninitializedFields;
    ...
    toolchain::SmallPtrSet<QualType, 4> UninitializedBaseClasses;
...
bool Sema::DiagRedefinedPlaceholderFieldDecl(SourceLocation Loc,
  ...
  DeclContextLookupResult::iterator Found =
      toolchain::find_if(Result, [this](const NamedDecl *Elem) {
...
struct BaseAndFieldInfo {
  ...
  toolchain::DenseMap<const void *, CXXCtorInitializer*> AllBaseFields;
  SmallVector<CXXCtorInitializer*, 8> AllToInit;
  toolchain::DenseMap<TagDecl*, FieldDecl*> ActiveUnionMember;
  ...
  bool isImplicitCopyOrMove() const {
    ...
    toolchain_unreachable("Invalid ImplicitInitializerKind!");
  }
...
static void MarkBaseDestructorsReferenced(Sema &S, SourceLocation Location,
  ...
  toolchain::SmallPtrSet<const CXXRecordDecl *, 8> DirectVirtualBases;
...
bool Sema::SetCtorInitializers(CXXConstructorDecl *Constructor, bool AnyErrors,
  ...
  toolchain::SmallPtrSet<CXXBaseSpecifier *, 16> DirectVBases;
...
static void DiagnoseBaseOrMemInitializerOrder(
  ...
  toolchain::sort(CorrelatedInitOrder, toolchain::less_first());
  ...
  for (unsigned WarnIndex : WarnIndexes) {
    const language::Core::CXXCtorInitializer *PrevInit = Inits[WarnIndex - 1];
...
typedef toolchain::DenseMap<RecordDecl*, UnionEntry> RedundantUnionMap;
...
void Sema::ActOnMemInitializers(Decl *ConstructorDecl,
  ...
  toolchain::DenseMap<const void *, CXXCtorInitializer *> Members;
...
void Sema::DiagnoseAbstractType(const CXXRecordDecl *RD) {
  ...
  // more than once.
  toolchain::SmallPtrSet<const CXXMethodDecl *, 8> SeenPureMethods;
...
static void DefineDefaultedFunction(Sema &S, FunctionDecl *FD,
  ...
  case CXXSpecialMemberKind::Invalid:
    toolchain_unreachable("Invalid special member.");
  }
}
...
ReportOverrides(Sema &S, unsigned DiagID, const CXXMethodDecl *MD,
                toolchain::function_ref<bool(const CXXMethodDecl *)> Report) {
...
void Sema::CheckCompletedCXXClass(Scope *S, CXXRecordDecl *Record) {
  ...
  toolchain::SmallVector<FunctionDecl*, 5> DefaultedSecondaryComparisons;
  ...
  auto CompleteMemberFunction = [&](CXXMethodDecl *M) {
    ...
    // See https://github.com/toolchain/toolchain-project/issues/102293 for more info.
    if (isa<CXXDestructorDecl>(M)) {
      toolchain::SmallDenseSet<QualType> Visited;
  ...
  toolchain::SmallDenseMap<OverloadedOperatorKind,
                      toolchain::SmallVector<const FunctionDecl *, 2>, 4>
...
class Sema::InheritedConstructorInfo {
  ...
  toolchain::DenseMap<CXXRecordDecl *, ConstructorUsingShadowDecl *>
      InheritedFromBases;
...
public:
  ...
  ResultList visit() {
    ...
    case DefaultedComparisonKind::None:
      toolchain_unreachable("not a defaulted comparison");
...
protected:
  ...
  Result visitSubobjectArray(QualType Type, const toolchain::APInt &Size,
                             Subobject Subobj) {
...
struct StmtListResult {
  bool IsInvalid = false;
  toolchain::SmallVector<Stmt*, 16> Stmts;
...
public:
  ...
  StmtResult build() {
    ...
    case DefaultedComparisonKind::Equal: {
      ...
      for (Stmt *EAsStmt : toolchain::reverse(OldStmts)) {
...
private:
  ...
  StmtResult visitSubobjectArray(QualType Type, toolchain::APInt Size,
                                 ExprPair Subobj) {
    ...
    toolchain::APInt Zero(S.Context.getTypeSize(SizeType), 0);
  ...
  StmtResult visitExpandedSubobject(QualType Type, ExprPair Obj) {
    ...
    case DefaultedComparisonKind::ThreeWay: {
      ...
      if (VDRef.isInvalid())
        return StmtError();
      toolchain::APInt ZeroVal(S.Context.getIntWidth(S.Context.IntTy), 0);
      Expr *Zero =
...
bool Sema::CheckExplicitlyDefaultedComparison(Scope *S, FunctionDecl *FD,
  ...
  if (IsMethod) {
    ...
    if (toolchain::none_of(RD->friends(), [&](const FriendDecl *F) {
...
struct SpecialMemberVisitor {
  ...
  SpecialMemberVisitor(Sema &S, CXXMethodDecl *MD, CXXSpecialMemberKind CSM,
    ...
    case CXXSpecialMemberKind::Destructor:
      break;
    case CXXSpecialMemberKind::Invalid:
      toolchain_unreachable("invalid special member kind");
    }
  ...
  typedef toolchain::PointerUnion<CXXBaseSpecifier*, FieldDecl*> Subobject;
...
static bool findTrivialSpecialMember(Sema &S, CXXRecordDecl *RD,
  ...
  switch (CSM) {
  case CXXSpecialMemberKind::Invalid:
    toolchain_unreachable("not a special member");
  ...
  toolchain_unreachable("unknown special method kind");
}
...
bool Sema::SpecialMemberIsTrivial(CXXMethodDecl *MD, CXXSpecialMemberKind CSM,
  ...
  if (CSM != CXXSpecialMemberKind::Destructor &&
    ...
    toolchain_unreachable("dynamic class with no vbases and no virtual functions");
  }
...
struct FindHiddenVirtualMethod {
  ...
  toolchain::SmallPtrSet<const CXXMethodDecl *, 8> OverridenAndUsingBaseMethods;
  SmallVector<CXXMethodDecl *, 8> OverloadedMethods;
...
private:
  ...
  static bool CheckMostOverridenMethods(
      ...
      const toolchain::SmallPtrSetImpl<const CXXMethodDecl *> &Methods) {
    if (MD->size_overridden_methods() == 0)
...
static void AddMostOverridenMethods(const CXXMethodDecl *MD,
                        toolchain::SmallPtrSetImpl<const CXXMethodDecl *>& Methods) {
  if (MD->size_overridden_methods() == 0)
...
static void findImplicitlyDeclaredEqualityComparisons(
    ...
    toolchain::SmallVectorImpl<FunctionDecl *> &Spaceships) {
...
void Sema::AddImplicitlyDeclaredMembersToClass(CXXRecordDecl *ClassDecl) {
  ...
  if (getLangOpts().CPlusPlus20 && !inTemplateInstantiation()) {
    toolchain::SmallVector<FunctionDecl *, 4> DefaultedSpaceships;
...
void Sema::CheckExplicitObjectMemberFunction(Declarator &D,
  ...
  if (D.getDeclSpec().getStorageClassSpec() == DeclSpec::SCS_static ||
      (D.getContext() == language::Core::DeclaratorContext::Member &&
       D.isStaticMember())) {
...
bool Sema::CheckDeductionGuideDeclarator(Declarator &D, QualType &R,
  ...
  for (const DeclaratorChunk &Chunk : toolchain::reverse(D.type_objects())) {
    if (Chunk.Kind == DeclaratorChunk::Paren)
      continue;
...
static bool TryNamespaceTypoCorrection(Sema &S, LookupResult &R, Scope *Sc,
  ...
  if (TypoCorrection Corrected =
    ...
    // invisible namespace.
    // See https://github.com/toolchain/toolchain-project/issues/73893.
    //
...
Decl *Sema::ActOnUsingDeclaration(Scope *S, AccessSpecifier AS,
  ...
  case UnqualifiedIdKind::IK_DeductionGuideName:
    toolchain_unreachable("cannot parse qualified deduction guide name");
  }
...
static bool isVirtualDirectBase(CXXRecordDecl *Derived, CXXRecordDecl *Base) {
  ...
  for (auto &B : Derived->bases())
    if (B.getType()->getAsCXXRecordDecl() == Base)
      return B.isVirtual();
  toolchain_unreachable("not a direct base class");
}
...
buildMemcpyForAssignmentOp(Sema &S, SourceLocation Loc, QualType T,
  ...
  QualType SizeType = S.Context.getSizeType();
  toolchain::APInt Size(S.Context.getTypeSize(SizeType),
...
static void checkMoveAssignmentForRepeatedMove(Sema &S, CXXRecordDecl *Class,
  ...
  toolchain::SmallVector<CXXBaseSpecifier *, 16> Worklist;
  typedef toolchain::DenseMap<CXXRecordDecl*, CXXBaseSpecifier*> VBaseMap;
  VBaseMap VBases;
  ...
  for (auto &BI : Class->bases()) {
    ...
    while (!Worklist.empty()) {
      ...
      if (BaseSpec->isVirtual()) {
        ...
        toolchain::append_range(Worklist, toolchain::make_pointer_range(Base->bases()));
      }
    }
  }
}
...
bool Sema::CompleteConstructorCall(CXXConstructorDecl *Constructor,
  ...
  bool Invalid = GatherArgumentsForCall(
      Loc, Constructor, Proto, 0, toolchain::ArrayRef(Args, NumArgs), AllArgs,
      CallType, AllowExplicit, IsListInitialization);
  ConvertedArgs.append(AllArgs.begin(), AllArgs.end());

  DiagnoseSentinelCalls(Constructor, Loc, AllArgs);

  CheckConstructorCall(Constructor, DeclInitType, toolchain::ArrayRef(AllArgs),
                       Proto, Loc);
...
Sema::BuildTypeAwareUsualDelete(FunctionTemplateDecl *FnTemplateDecl,
  ...
  // the first parameter.
  if (toolchain::any_of(FnDecl->parameters().drop_front(),
...
static void WriteCharTypePrefix(BuiltinType::Kind BTK, toolchain::raw_ostream &OS) {
  switch (BTK) {
  ...
  case BuiltinType::WChar_U:
    ...
    break;
  default:
    toolchain_unreachable("Non-character type");
  }
}
...
static void WriteCharValueForDiagnostic(uint32_t Value, const BuiltinType *BTy,
  ...
  case BuiltinType::WChar_S:
  case BuiltinType::WChar_U: {
    if (toolchain::ConvertCodePointToUTF8(Value, Ptr))
      EscapeStringForDiagnostic(StringRef(Arr, Ptr - Arr), Str);
    else
      OS << "\\x"
         << toolchain::format_hex_no_prefix(Value, TyWidth / 4, /*Upper=*/true);
    break;
  }
  default:
    toolchain_unreachable("Non-character type is passed");
  }
}
    ...
    if (T->isBooleanType()) {
      ...
      if (BTy) {
        ...
        case BuiltinType::WChar_U: {
          ...
          OS << "' (0x"
             << toolchain::format_hex_no_prefix(CodeUnit, /*Width=*/2,
                                           /*Upper=*/true)
             << ", " << V.getInt() << ')';
          return true;
        }
...
static bool EvaluateAsStringImpl(Sema &SemaRef, Expr *Message,
  ...
  if (const auto *SL = dyn_cast<StringLiteral>(Message)) {
    ...
    if constexpr (std::is_same_v<APValue, ResultType>) {
      ...
      toolchain::APSInt Value(SemaRef.getASTContext().getTypeSize(CharType),
...
Decl *Sema::BuildStaticAssertDeclaration(SourceLocation StaticAssertLoc,
  ...
  if (!AssertExpr->isTypeDependent() && !AssertExpr->isValueDependent() &&
    ...
    if (!Failed && !Cond && !InTemplateDefinition) {
      ...
      toolchain::raw_svector_ostream Msg(MsgBuffer);
...
void Sema::LoadExternalVTableUses() {
  ...
  for (unsigned I = 0, N = VTables.size(); I != N; ++I) {
    toolchain::DenseMap<CXXRecordDecl *, bool>::iterator Pos
...
void Sema::MarkVTableUsed(SourceLocation Loc, CXXRecordDecl *Class,
  ...
  std::pair<toolchain::DenseMap<CXXRecordDecl *, bool>::iterator, bool>
...
void DelegatingCycleHelper(CXXConstructorDecl* Ctor,
                           toolchain::SmallPtrSet<CXXConstructorDecl*, 4> &Valid,
                           toolchain::SmallPtrSet<CXXConstructorDecl*, 4> &Invalid,
                           toolchain::SmallPtrSet<CXXConstructorDecl*, 4> &Current,
                           Sema &S) {
...
void Sema::CheckDelegatingCtorCycles() {
  toolchain::SmallPtrSet<CXXConstructorDecl*, 4> Valid, Invalid, Current;
...
bool Sema::checkThisInStaticMemberFunctionAttributes(CXXMethodDecl *Method) {
  ...
  for (const auto *A : Method->attrs()) {
    ...
    if (const auto *G = dyn_cast<GuardedByAttr>(A))
      ...
      Args = toolchain::ArrayRef(AA->args_begin(), AA->args_size());
    else if (const auto *AB = dyn_cast<AcquiredBeforeAttr>(A))
      Args = toolchain::ArrayRef(AB->args_begin(), AB->args_size());
    else if (const auto *LR = dyn_cast<LockReturnedAttr>(A))
      Arg = LR->getArg();
    else if (const auto *LE = dyn_cast<LocksExcludedAttr>(A))
      Args = toolchain::ArrayRef(LE->args_begin(), LE->args_size());
    else if (const auto *RC = dyn_cast<RequiresCapabilityAttr>(A))
      Args = toolchain::ArrayRef(RC->args_begin(), RC->args_size());
    else if (const auto *AC = dyn_cast<AcquireCapabilityAttr>(A))
      Args = toolchain::ArrayRef(AC->args_begin(), AC->args_size());
...
void Sema::actOnDelayedExceptionSpecification(
  ...
  // Check the exception specification.
  toolchain::SmallVector<QualType, 4> Exceptions;
  FunctionProtoType::ExceptionSpecInfo ESI;
...
void Sema::ActOnStartFunctionDeclarationDeclarator(
  ...
  if (ExplicitParams && !ExplicitParams->empty()) {
    ...
    toolchain::append_range(Info.TemplateParams, *ExplicitParams);
