...
using LifetimeResult =
    toolchain::PointerIntPair<const InitializedEntity *, 3, LifetimeKind>;
} // namespace
...
getEntityLifetime(const InitializedEntity *Entity,
  ...
  toolchain_unreachable("unknown entity kind");
}
...
using IndirectLocalPath = toolchain::SmallVectorImpl<IndirectLocalPathEntry>;
...
using LocalVisitor = toolchain::function_ref<bool(IndirectLocalPath &Path, Local L,
                                             ReferenceKind RK)>;
} // namespace
...
static bool pathContainsInit(const IndirectLocalPath &Path) {
  return toolchain::any_of(Path, [=](IndirectLocalPathEntry E) {
...
static bool shouldTrackImplicitObjectArg(const CXXMethodDecl *Callee) {
  ...
  if (isPointerLikeType(Callee->getReturnType())) {
    if (!Callee->getIdentifier())
      return false;
    return toolchain::StringSwitch<bool>(Callee->getName())
...
static bool shouldTrackFirstArgument(const FunctionDecl *FD) {
  ...
  if (FD->getReturnType()->isPointerType() ||
    ...
    return toolchain::StringSwitch<bool>(FD->getName())
        .Cases("begin", "rbegin", "cbegin", "crbegin", true)
        .Cases("end", "rend", "cend", "crend", true)
        .Case("data", true)
        .Default(false);
...
static void visitFunctionCallArguments(IndirectLocalPath &Path, Expr *Call,
  ...
  if (auto *CE = dyn_cast<CallExpr>(Call)) {
    ...
    Args = toolchain::ArrayRef(CE->getArgs(), CE->getNumArgs());
  } else {
    auto *CCE = cast<CXXConstructExpr>(Call);
    Callee = CCE->getConstructor();
    Args = toolchain::ArrayRef(CCE->getArgs(), CCE->getNumArgs());
  }
  if (!Callee)
    return;
  ...
  auto VisitGSLPointerArg = [&](const FunctionDecl *Callee, Expr *Arg) {
    ...
    if (ReturnType->isReferenceType() &&
      ...
      for (const IndirectLocalPathEntry &PE : toolchain::reverse(Path)) {
  ...
  for (unsigned I = 0, N = std::min<unsigned>(NP, Args.size()); I != N; ++I) {
    ...
    if (CheckCoroCall ||
      ...
      VisitLifetimeBoundArg(CanonCallee->getParamDecl(I), Arg);
             ...
             toolchain::any_of(CaptureAttr->params(), [](int ArgIdx) {
      ...
      // struct Foo {
      //   const int& a;
      //   // Equivalent to Foo(const int& t [[language::Core::lifetimebound]])
      //   Foo(const int& t [[language::Core::lifetime_capture_by(this)]]) : a(t) {}
      // };
      //
...
static bool pathOnlyHandlesGslPointer(const IndirectLocalPath &Path) {
  for (const auto &It : toolchain::reverse(Path)) {
    switch (It.Kind) {
    case IndirectLocalPathEntry::VarInit:
    case IndirectLocalPathEntry::AddressOf:
    case IndirectLocalPathEntry::LifetimeBoundCall:
...
static AnalysisResult analyzePathForGSLPointer(const IndirectLocalPath &Path,
  ...
  for (const auto &E : Path) {
    if (E.Kind == IndirectLocalPathEntry::MemberExpr) {
      ...
      if (const auto *FD = toolchain::dyn_cast_or_null<FieldDecl>(E.D);
  ...
  // For example:
  //    const std::string& Ref(const std::string& a [[language::Core::lifetimebound]]);
  ...
  if (Path.back().Kind == IndirectLocalPathEntry::LifetimeBoundCall) {
    ...
    const FunctionDecl *FD =
        toolchain::dyn_cast_or_null<FunctionDecl>(Path.back().D);
    // The lifetimebound applies to a function parameter.
    if (const auto *PD = toolchain::dyn_cast<ParmVarDecl>(Path.back().D))
      FD = toolchain::dyn_cast<FunctionDecl>(PD->getDeclContext());

    if (isa_and_present<CXXConstructorDecl>(FD)) {
      // Constructor case: the parameter is annotated with lifetimebound
      //   e.g., GSLPointer(const S& s [[language::Core::lifetimebound]])
      // We still respect this case even the type S is not an owner.
      return Report;
    }
    // Check the return type, e.g.
    //   const GSLOwner& func(const Foo& foo [[language::Core::lifetimebound]])
    //   GSLOwner* func(cosnt Foo& foo [[language::Core::lifetimebound]])
    //   GSLPointer func(const Foo& foo [[language::Core::lifetimebound]])
    if (FD &&
...
checkExprLifetimeImpl(Sema &SemaRef, const InitializedEntity *InitEntity,
  ...
  auto TemporaryVisitor = [&](const IndirectLocalPath &Path, Local L,
    ...
    case LK_FullExpression:
      toolchain_unreachable("already handled this");
  ...
  toolchain::SmallVector<IndirectLocalPathEntry, 8> Path;
  switch (LK) {
  case LK_Assignment: {
