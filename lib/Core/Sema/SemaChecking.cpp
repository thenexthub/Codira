...
#include "language/Core/AST/IgnoreExpr.h"
#include "language/Core/AST/NSAPI.h"
#include "language/Core/AST/NonTrivialTypeVisitor.h"
...
#include "language/Core/Basic/SyncScope.h"
#include "language/Core/Basic/TargetInfo.h"
#include "language/Core/Basic/TypeTraits.h"
#include "language/Core/Lex/Lexer.h" // TODO: Extract static functions to fix layering.
...
#include "language/Core/Sema/ScopeInfo.h"
#include "language/Core/Sema/Sema.h"
#include "language/Core/Sema/SemaAMDGPU.h"
#include "language/Core/Sema/SemaARM.h"
#include "language/Core/Sema/SemaBPF.h"
#include "language/Core/Sema/SemaDirectX.h"
#include "language/Core/Sema/SemaHLSL.h"
#include "language/Core/Sema/SemaHexagon.h"
#include "language/Core/Sema/SemaLoongArch.h"
#include "language/Core/Sema/SemaMIPS.h"
#include "language/Core/Sema/SemaNVPTX.h"
#include "language/Core/Sema/SemaObjC.h"
#include "language/Core/Sema/SemaOpenCL.h"
#include "language/Core/Sema/SemaPPC.h"
#include "language/Core/Sema/SemaRISCV.h"
#include "language/Core/Sema/SemaSPIRV.h"
#include "language/Core/Sema/SemaSystemZ.h"
#include "language/Core/Sema/SemaWasm.h"
#include "language/Core/Sema/SemaX86.h"
...
static bool BuiltinAlignment(Sema &S, CallExpr *TheCall, unsigned ID) {
  ...
  language::Core::Expr *Source = TheCall->getArg(0);
  ...
  language::Core::Expr *AlignOp = TheCall->getArg(1);
  ...
  if (!AlignOp->isValueDependent() &&
    ...
    toolchain::APSInt AlignValue = AlignResult.Val.getInt();
    toolchain::APSInt MaxValue(
        toolchain::APInt::getOneBitSet(MaxAlignmentBits + 1, MaxAlignmentBits));
    if (AlignValue < 1) {
      S.Diag(AlignOp->getExprLoc(), diag::err_alignment_too_small) << 1;
      return true;
    }
    if (toolchain::APSInt::compareValues(AlignValue, MaxValue) > 0) {
...
static bool BuiltinOverflow(Sema &S, CallExpr *TheCall, unsigned BuiltinID) {
  ...
  bool CkdOperation = toolchain::any_of(Builtins, [&](const std::pair<unsigned,
    const char *> &P) {
...
struct BuiltinDumpStructGenerator {
  ...
  Expr *getStringLiteral(toolchain::StringRef Str) {
  ...
  bool callPrintFunction(toolchain::StringRef Format,
                         toolchain::ArrayRef<Expr *> Exprs = {}) {
    ...
    toolchain::append_range(Args, Exprs);
  ...
  Expr *getIndentString(unsigned Depth) {
    ...
    toolchain::SmallString<32> Indent;
  ...
  bool appendFormatSpecifier(QualType T, toolchain::SmallVectorImpl<char> &Str) {
  ...
  bool dumpRecordValue(const RecordDecl *RD, Expr *E, Expr *RecordIndent,
    ...
    for (auto *D : RD->decls()) {
      ...
      toolchain::SmallString<20> Format = toolchain::StringRef("%s%s %s ");
      toolchain::SmallVector<Expr *, 5> Args = {FieldIndentArg,
      ...
      if (FD->isBitField()) {
        ...
        toolchain::APInt BitWidth(S.Context.getIntWidth(SizeT),
                             FD->getBitWidthValue());
...
class ScanfDiagnosticFormatHandler
  ...
  using ComputeSizeFunction =
      toolchain::function_ref<std::optional<toolchain::APSInt>(unsigned)>;
  ...
  using DiagnoseFunction =
      toolchain::function_ref<void(unsigned, unsigned, unsigned)>;
...
public:
  ...
  bool HandleScanfSpecifier(const analyze_scanf::ScanfSpecifier &FS,
    ...
    std::optional<toolchain::APSInt> DestSizeAPS =
...
void Sema::checkFortifiedBuiltinMemoryFunction(FunctionDecl *FD,
  ...
  auto ComputeExplicitObjectSizeArgument =
      [&](unsigned Index) -> std::optional<toolchain::APSInt> {
    ...
    if (!SizeArg->EvaluateAsInt(Result, getASTContext()))
      return std::nullopt;
    toolchain::APSInt Integer = Result.Val.getInt();
  ...
  auto ComputeSizeArgument =
    ...
    return toolchain::APSInt::getUnsigned(Result).extOrTrunc(SizeTypeWidth);
  };
  ...
  auto ComputeStrLenArgument =
    ...
    if (!ObjArg->tryEvaluateStrLen(Result, getASTContext()))
      return std::nullopt;
    // Add 1 for null byte.
    return toolchain::APSInt::getUnsigned(Result + 1).extOrTrunc(SizeTypeWidth);
  };

  std::optional<toolchain::APSInt> SourceSize;
  std::optional<toolchain::APSInt> DestinationSize;
  unsigned DiagID = 0;
  ...
  auto GetFunctionName = [&]() {
    ...
    toolchain::StringRef FunctionName = FunctionNameStr;
  ...
  case Builtin::BI__builtin___sprintf_chk: {
    ...
    if (ProcessFormatStringLiteral(FormatExpr, FormatStrRef, StrLen, Context)) {
      ...
      if (!analyze_format_string::ParsePrintfString(
        ...
        SourceSize = toolchain::APSInt::getUnsigned(H.getSizeLowerBound())
  ...
  case Builtin::BI__builtin_vsnprintf: {
    ...
    if (SourceSize &&
      ...
      if (!analyze_format_string::ParsePrintfString(
        ...
        toolchain::APSInt FormatSize =
            toolchain::APSInt::getUnsigned(H.getSizeLowerBound())
                .extOrTrunc(SizeTypeWidth);
  ...
  if (!SourceSize || !DestinationSize ||
      toolchain::APSInt::compareValues(*SourceSize, *DestinationSize) <= 0)
    return;
...
bool Sema::checkConstantPointerAuthKey(Expr *Arg, unsigned &Result) {
  ...
  std::optional<toolchain::APSInt> KeyValue = Arg->getIntegerConstantExpr(Context);
  if (!KeyValue) {
  ...
  if (!Context.getTargetInfo().validatePointerAuthKey(*KeyValue)) {
    toolchain::SmallString<32> Value;
    {
      toolchain::raw_svector_ostream Str(Value);
      Str << *KeyValue;
    }
...
bool Sema::checkPointerAuthDiscriminatorArg(Expr *Arg,
  ...
  std::optional<toolchain::APSInt> Result = Arg->getIntegerConstantExpr(Context);
  if (!Result) {
...
static bool CheckBuiltinTargetNotInUnsupported(
    ...
    ArrayRef<toolchain::Triple::ObjectFormatType> UnsupportedObjectFormatTypes) {
  toolchain::Triple::ObjectFormatType CurObjFormat =
      S.getASTContext().getTargetInfo().getTriple().getObjectFormat();
  if (toolchain::is_contained(UnsupportedObjectFormatTypes, CurObjFormat)) {
...
CheckBuiltinTargetInSupported(Sema &S, CallExpr *TheCall,
                              ArrayRef<toolchain::Triple::ArchType> SupportedArchs) {
  toolchain::Triple::ArchType CurArch =
      S.getASTContext().getTargetInfo().getTriple().getArch();
  if (toolchain::is_contained(SupportedArchs, CurArch))
    return false;
  S.Diag(TheCall->getBeginLoc(), diag::err_builtin_target_unsupported)
      << TheCall->getSourceRange();
  return true;
...
bool Sema::CheckTSBuiltinFunctionCall(const TargetInfo &TI, unsigned BuiltinID,
  ...
  case toolchain::Triple::spirv64:
    if (TI.getTriple().getOS() != toolchain::Triple::OSType::AMDHSA)
...
static ExprResult BuiltinInvoke(Sema &S, CallExpr *TheCall) {
  ...
  assert(toolchain::none_of(Args, [](Expr *Arg) { return Arg->isTypeDependent(); }));
...
Sema::CheckBuiltinFunctionCall(FunctionDecl *FDecl, unsigned BuiltinID,
  ...
  case Builtin::BI__builtin___CFStringMakeConstantString:
    ...
    if (CheckBuiltinTargetNotInUnsupported(
            *this, BuiltinID, TheCall,
            {toolchain::Triple::GOFF, toolchain::Triple::XCOFF}))
      return ExprError();
  ...
  case Builtin::BI_interlockedbittestandreset_nf:
    if (CheckBuiltinTargetInSupported(
            *this, TheCall,
            {toolchain::Triple::arm, toolchain::Triple::thumb, toolchain::Triple::aarch64}))
      return ExprError();
    break;
  ...
  case Builtin::BI_interlockedbittestandset64:
    if (CheckBuiltinTargetInSupported(
            *this, TheCall,
            {toolchain::Triple::x86_64, toolchain::Triple::arm, toolchain::Triple::thumb,
             toolchain::Triple::aarch64, toolchain::Triple::amdgcn}))
      return ExprError();
    break;
  ...
  case Builtin::BI_interlockedbittestandset64_nf:
    if (CheckBuiltinTargetInSupported(*this, TheCall, {toolchain::Triple::aarch64}))
      return ExprError();
    break;
  ...
  case Builtin::BI__builtin_set_flt_rounds:
    if (CheckBuiltinTargetInSupported(
            *this, TheCall,
            {toolchain::Triple::x86, toolchain::Triple::x86_64, toolchain::Triple::arm,
             toolchain::Triple::thumb, toolchain::Triple::aarch64, toolchain::Triple::amdgcn,
             toolchain::Triple::ppc, toolchain::Triple::ppc64, toolchain::Triple::ppcle,
             toolchain::Triple::ppc64le}))
      return ExprError();
    break;
  ...
  case Builtin::BI__builtin_memcpy_inline: {
    language::Core::Expr *SizeOp = TheCall->getArg(2);
...
#include "language/Core/Basic/Builtins.inc"
  ...
  case Builtin::BI__builtin_expect_with_probability: {
    ...
    toolchain::APFloat Probability = Eval.Val.getFloat();
    bool LoseInfo = false;
    Probability.convert(toolchain::APFloat::IEEEdouble(),
                        toolchain::RoundingMode::Dynamic, &LoseInfo);
    if (!(Probability >= toolchain::APFloat(0.0) &&
          Probability <= toolchain::APFloat(1.0))) {
...
static void CheckNonNullArguments(Sema &S,
  ...
  toolchain::SmallBitVector NonNullArgs;
  if (FDecl) {
...
void Sema::checkCall(NamedDecl *FDecl, const FunctionProtoType *Proto,
  ...
  toolchain::SmallBitVector CheckedVarArgs;
  if (FDecl) {
  ...
  if (FDecl && FDecl->hasAttr<AllocAlignAttr>()) {
    ...
    if (!Arg->isValueDependent()) {
      ...
      if (Arg->EvaluateAsInt(Align, Context)) {
        const toolchain::APSInt &I = Align.Val.getInt();
        if (!I.isPowerOf2())
...
bool Sema::CheckFunctionCall(FunctionDecl *FDecl, CallExpr *TheCall,
  ...
  checkCall(FDecl, Proto, ImplicitThis, toolchain::ArrayRef(Args, NumArgs),
...
bool Sema::CheckPointerCall(NamedDecl *NDecl, CallExpr *TheCall,
  ...
  checkCall(NDecl, Proto, /*ThisArg=*/nullptr,
            toolchain::ArrayRef(TheCall->getArgs(), TheCall->getNumArgs()),
...
static bool isValidOrderingForOp(int64_t Ordering, AtomicExpr::AtomicOp Op) {
  if (!toolchain::isValidAtomicOrderingCABI(Ordering))
    return false;

  auto OrderingCABI = (toolchain::AtomicOrderingCABI)Ordering;
  switch (Op) {
  case AtomicExpr::AO__c11_atomic_init:
  case AtomicExpr::AO__opencl_atomic_init:
    toolchain_unreachable("There is no ordering argument for an init");
  ...
  case AtomicExpr::AO__scoped_atomic_load:
    return OrderingCABI != toolchain::AtomicOrderingCABI::release &&
           OrderingCABI != toolchain::AtomicOrderingCABI::acq_rel;
  ...
  case AtomicExpr::AO__atomic_clear:
    return OrderingCABI != toolchain::AtomicOrderingCABI::consume &&
           OrderingCABI != toolchain::AtomicOrderingCABI::acquire &&
           OrderingCABI != toolchain::AtomicOrderingCABI::acq_rel;
...
ExprResult Sema::BuildAtomicExpr(SourceRange CallRange, SourceRange ExprRange,
    ...
    auto IsAllowedValueType = [&](QualType ValType,
      ...
      if (ValType->isSpecificBuiltinType(BuiltinType::LongDouble) &&
          &Context.getTargetInfo().getLongDoubleFormat() ==
              &toolchain::APFloat::x87DoubleExtended())
        return false;
      return true;
    };
  ...
  if (SubExprs.size() >= 2 && Form != Init) {
    std::optional<toolchain::APSInt> Success =
    ...
    if (SubExprs.size() >= 5) {
      if (std::optional<toolchain::APSInt> Failure =
              SubExprs[3]->getIntegerConstantExpr(Context)) {
        if (!toolchain::is_contained(
                {toolchain::AtomicOrderingCABI::relaxed,
                 toolchain::AtomicOrderingCABI::consume,
                 toolchain::AtomicOrderingCABI::acquire,
                 toolchain::AtomicOrderingCABI::seq_cst},
                (toolchain::AtomicOrderingCABI)Failure->getSExtValue())) {
...
#undef BUILTIN_ROW
  ...
  switch (BuiltinID) {
  default: toolchain_unreachable("Unknown overloaded atomic builtin!");
  case Builtin::BI__sync_fetch_and_add:
  case Builtin::BI__sync_fetch_and_add_1:
  case Builtin::BI__sync_fetch_and_add_2:
  case Builtin::BI__sync_fetch_and_add_4:
  ...
  if (BitIntValType && !toolchain::isPowerOf2_64(BitIntValType->getNumBits())) {
...
static bool checkVAStartABI(Sema &S, unsigned BuiltinID, Expr *Fn) {
  const toolchain::Triple &TT = S.Context.getTargetInfo().getTriple();
  bool IsX64 = TT.getArch() == toolchain::Triple::x86_64;
  bool IsAArch64 = (TT.getArch() == toolchain::Triple::aarch64 ||
                    TT.getArch() == toolchain::Triple::aarch64_32);
...
bool Sema::BuiltinVAStart(unsigned BuiltinID, CallExpr *TheCall) {
  ...
  if (std::optional<toolchain::APSInt> Val =
...
bool Sema::BuiltinFPClassification(CallExpr *TheCall, unsigned NumArgs,
  ...
  if (IsFPClass)
    if (BuiltinConstantArgRange(TheCall, 1, 0, toolchain::fcAllFlags))
      return true;
...
bool Sema::BuiltinAllocaWithAlign(CallExpr *TheCall) {
  ...
  if (!Arg->isTypeDependent() && !Arg->isValueDependent()) {
    ...
    toolchain::APSInt Result = Arg->EvaluateKnownConstInt(Context);
...
bool Sema::BuiltinOSLogFormat(CallExpr *TheCall) {
  ...
  if (!IsSizeCall) {
    toolchain::SmallBitVector CheckedVarArgs(NumArgs, false);
...
bool Sema::BuiltinConstantArg(CallExpr *TheCall, int ArgNum,
  ...
  std::optional<toolchain::APSInt> R;
...
static bool IsShiftedByte(toolchain::APSInt Value) {
  if (Value.isNegative())
    return false;
...
static void sumOffsets(toolchain::APSInt &Offset, toolchain::APSInt Addend,
  ...
  bool Ov = false;
  toolchain::APSInt ResOffset = Offset;
  if (BinOpKind == BO_Add)
...
static void CheckFormatString(
    ...
    toolchain::SmallBitVector &CheckedVarArgs, UncoveredArgHandler &UncoveredArg,
...
static StringLiteralCheckType checkFormatStringExpr(
    ...
    toolchain::SmallBitVector &CheckedVarArgs, UncoveredArgHandler &UncoveredArg,
    toolchain::APSInt Offset, bool IgnoreStringsWithoutSpecifiers = false) {
  if (S.isConstantEvaluatedContext())
...
tryAgain:
  ...
  case Stmt::InitListExprClass:
    ...
    if (const language::Core::Expr *SLE = maybeConstEvalStringLiteral(S.Context, E)) {
...
FormatStringType Sema::GetFormatStringType(StringRef Flavor) {
  return toolchain::StringSwitch<FormatStringType>(Flavor)
...
bool Sema::CheckFormatArguments(ArrayRef<const Expr *> Args,
  ...
  StringLiteralCheckType CT = checkFormatStringExpr(
      *this, ReferenceFormatString, OrigFormatExpr, Args, APK, format_idx,
      ...
      /*no string offset*/ toolchain::APSInt(64, false) = 0);
...
protected:
  ...
  ArrayRef<const Expr *> Args;
  unsigned FormatIdx;
  toolchain::SmallBitVector CoveredArgs;
...
CheckFormatHandler::HandleInvalidConversionSpecifier(unsigned argIndex,
  ...
  if (!toolchain::sys::locale::isPrint(*csStart)) {
    toolchain::UTF32 CodePoint;
    const toolchain::UTF8 **B = reinterpret_cast<const toolchain::UTF8 **>(&csStart);
    const toolchain::UTF8 *E =
        reinterpret_cast<const toolchain::UTF8 *>(csStart + csLen);
    toolchain::ConversionResult Result =
        toolchain::convertUTF8Sequence(B, E, &CodePoint, toolchain::strictConversion);

    if (Result != toolchain::conversionOK) {
      unsigned char FirstChar = *csStart;
      CodePoint = (toolchain::UTF32)FirstChar;
    }

    toolchain::raw_string_ostream OS(CodePointStr);
    if (CodePoint < 256)
      OS << "\\x" << toolchain::format("%02x", CodePoint);
    else if (CodePoint <= 0xFFFF)
      OS << "\\u" << toolchain::format("%04x", CodePoint);
    else
      OS << "\\U" << toolchain::format("%08x", CodePoint);
...
public:
  ...
  std::string buildFormatSpecifier() const {
    std::string result;
    toolchain::raw_string_ostream(result)
...
class DecomposePrintfHandler : public CheckPrintfHandler {
  toolchain::SmallVectorImpl<EquatableFormatArgument> &Specs;
  bool HadError;
...
public:
  ...
  GetSpecifiers(Sema &S, const FormatStringLiteral *FSL, const Expr *FmtExpr,
                ...
                toolchain::SmallVectorImpl<EquatableFormatArgument> &Args);
...
bool EquatableFormatArgument::VerifyCompatible(
  ...
  case MK::MatchPromotion:
    // Per consensus reached at https://discourse.toolchain.org/t/-/83076/12,
...
bool DecomposePrintfHandler::GetSpecifiers(
  ...
  toolchain::stable_sort(Args, [](const EquatableFormatArgument &A,
...
static toolchain::SmallPtrSet<MemberKind*, 1>
CXXRecordMembersNamed(StringRef Name, Sema &S, QualType Ty) {
  const RecordType *RT = Ty->getAs<RecordType>();
  toolchain::SmallPtrSet<MemberKind*, 1> Results;
...
bool Sema::hasCStrMethod(const Expr *E) {
  using MethodSet = toolchain::SmallPtrSet<CXXMethodDecl *, 1>;
...
shouldNotPrintDirectly(const ASTContext &Context,
  ...
  while (const TypedefType *UserTy = TyTy->getAs<TypedefType>()) {
    ...
    QualType CastTy = toolchain::StringSwitch<QualType>(Name)
...
CheckPrintfHandler::checkFormatExpr(const analyze_printf::PrintfSpecifier &FS,
  ...
  if (FS.getConversionSpecifier().getKind() == ConversionSpecifier::cArg &&
    ...
    toolchain::raw_svector_ostream os(FSString);
    FS.toString(os);
  ...
  if (const ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(E)) {
    ...
    if (ExprTy == S.Context.IntTy &&
      ...
      if (toolchain::isUIntN(S.Context.getCharWidth(), CL->getValue())) {
  ...
  if (isObjCContext() &&
    ...
    if (ExprTy->isIntegralOrUnscopedEnumerationType() &&
      ...
      if (const IntegerLiteral *IL = dyn_cast<IntegerLiteral>(E)) {
        const toolchain::APInt &V = IL->getValue();
  ...
  if (Success) {
    ...
    if (IntendedTy == ExprTy && !ShouldNotPrintDirectly && !IsScopedEnum) {
      ...
      case ArgType::NoMatchPromotionTypeConfusion:
        toolchain_unreachable("expected non-matching");
      ...
      toolchain::raw_svector_ostream CastFix(CastBuf);
...
bool Sema::CheckFormatStringsCompatible(
  ...
  bool IsObjC =
      Type == FormatStringType::NSString || Type == FormatStringType::OSTrace;
  toolchain::SmallVector<EquatableFormatArgument, 9> RefArgs, FmtArgs;
...
bool Sema::ValidateFormatString(FormatStringType Type,
  ...
  toolchain::SmallVector<EquatableFormatArgument, 9> Args;
  bool IsObjC =
      Type == FormatStringType::NSString || Type == FormatStringType::OSTrace;
...
static AbsoluteValueKind getAbsoluteValueKind(QualType T) {
  ...
  toolchain_unreachable("Type not integer, floating, or complex");
}
...
static unsigned changeAbsFunction(unsigned AbsKind,
  ...
  toolchain_unreachable("Unable to convert function");
}
...
static unsigned getAbsoluteValueFunctionKind(const FunctionDecl *FDecl) {
  ...
  toolchain_unreachable("Unknown Builtin type");
}
...
static void emitReplacement(Sema &S, SourceLocation Loc, SourceRange Range,
  ...
  if (S.getLangOpts().CPlusPlus && !ArgType->isAnyComplexType()) {
    ...
    if (ArgType->isIntegralOrEnumerationType()) {
      ...
      toolchain_unreachable("Invalid Type");
    }
...
static bool IsInfOrNanFunction(StringRef calleeName, MathCheck Check) {
  auto MatchesAny = [&](std::initializer_list<toolchain::StringRef> names) {
    return toolchain::is_contained(names, calleeName);
  };
  ...
  toolchain_unreachable("unknown MathCheck");
}
...
void Sema::CheckMemaccessArguments(const CallExpr *Call,
  ...
  toolchain::FoldingSetNodeID SizeOfArgID;
  ...
  for (unsigned ArgIdx = 0; ArgIdx != LastArg; ++ArgIdx) {
    ...
    if (const PointerType *DestPtrTy = DestTy->getAs<PointerType>()) {
      ...
      if (SizeOfArg &&
        ...
        if (SizeOfArgID == toolchain::FoldingSetNodeID())
          SizeOfArg->Profile(SizeOfArgID, Context, true);
        toolchain::FoldingSetNodeID DestID;
...
void Sema::CheckStrlcpycatArguments(const CallExpr *Call,
  ...
  toolchain::raw_svector_ostream OS(sizeString);
  OS << "sizeof(";
...
void CheckFreeArgumentsCast(Sema &S, const std::string &CalleeName,
  ...
  language::Core::CastKind Kind = Cast->getCastKind();
  if (Kind == language::Core::CK_BitCast &&
      !Cast->getSubExpr()->getType()->isFunctionPointerType())
    return;
  if (Kind == language::Core::CK_IntegralToPointer &&
      !isa<IntegerLiteral>(
          Cast->getSubExpr()->IgnoreParenImpCasts()->IgnoreParens()))
    return;

  switch (Cast->getCastKind()) {
  case language::Core::CK_BitCast:
  case language::Core::CK_IntegralToPointer:
  case language::Core::CK_FunctionToPointerDecay:
    OS << '\'';
...
void Sema::CheckFloatComparison(SourceLocation Loc, const Expr *LHS,
  ...
  if (getCastAndLiteral(LHS, RHS) || getCastAndLiteral(RHS, LHS)) {
    ...
    if (SourceTy && TargetTy && SourceTy->isFloatingPoint() &&
      ...
      bool Lossy;
      toolchain::APFloat TargetC = FPLiteral->getValue();
      TargetC.convert(Context.getFloatTypeSemantics(QualType(SourceTy, 0)),
                      toolchain::APFloat::rmNearestTiesToEven, &Lossy);
      if (Lossy) {
...
static IntRange GetValueRange(toolchain::APSInt &value, unsigned MaxWidth) {
...
static std::optional<IntRange> TryGetExprRange(ASTContext &C, const Expr *E,
  ...
  if (const auto *BO = dyn_cast<BinaryOperator>(E)) {
    ...
    switch (BO->getOpcode()) {
    case BO_Cmp:
      toolchain_unreachable("builtin <=> should have class type");
    ...
    case BO_ShrAssign: {
      ...
      // that much.
      if (std::optional<toolchain::APSInt> shift =
    ...
    case BO_Div: {
      ...
      if (std::optional<toolchain::APSInt> divisor =
              BO->getRHS()->getIntegerConstantExpr(C)) {
...
/// target semantics.
static bool IsSameFloatAfterCast(const toolchain::APFloat &value,
                                 const toolchain::fltSemantics &Src,
                                 const toolchain::fltSemantics &Tgt) {
  toolchain::APFloat truncated = value;

  bool ignored;
  truncated.convert(Src, toolchain::APFloat::rmNearestTiesToEven, &ignored);
  truncated.convert(Tgt, toolchain::APFloat::rmNearestTiesToEven, &ignored);
...
struct PromotedRange {
  ...
  toolchain::APSInt PromotedMin;
  // Max, or HoleMin if there is a hole.
  toolchain::APSInt PromotedMax;

  PromotedRange(IntRange R, unsigned BitWidth, bool Unsigned) {
    if (R.Width == 0)
      PromotedMin = PromotedMax = toolchain::APSInt(BitWidth, Unsigned);
      ...
      PromotedMin = toolchain::APSInt::getMinValue(BitWidth, Unsigned);
      PromotedMax = toolchain::APSInt::getMaxValue(BitWidth, Unsigned);
    } else {
      PromotedMin = toolchain::APSInt::getMinValue(R.Width, R.NonNegative)
                        .extOrTrunc(BitWidth);
      PromotedMin.setIsUnsigned(Unsigned);

      PromotedMax = toolchain::APSInt::getMaxValue(R.Width, R.NonNegative)
                        .extOrTrunc(BitWidth);
  ...
  ComparisonResult compare(const toolchain::APSInt &Value) const {
    ...
    switch (toolchain::APSInt::compareValues(Value, PromotedMin)) {
    case -1: return Less;
    case 0: return PromotedMin == PromotedMax ? OnlyValue : Min;
    case 1:
      switch (toolchain::APSInt::compareValues(Value, PromotedMax)) {
    ...
    toolchain_unreachable("impossible compare result");
  }
...
static bool CheckTautologicalComparison(Sema &S, BinaryOperator *E,
  ...
  toolchain::raw_svector_ostream OS(PrettySourceValue);
  if (ED) {
    OS << '\'' << *ED << "' (" << Value << ")";
...
static void AnalyzeComparison(Sema &S, BinaryOperator *E) {
  ...
  if (T->isIntegralType(S.Context)) {
    std::optional<toolchain::APSInt> RHSValue =
        RHS->getIntegerConstantExpr(S.Context);
    std::optional<toolchain::APSInt> LHSValue =
    ...
    if ((bool)RHSValue ^ (bool)LHSValue) {
      ...
      const toolchain::APSInt &Value = RhsConstant ? *RHSValue : *LHSValue;
...
static bool AnalyzeBitFieldAssignment(Sema &S, FieldDecl *Bitfield, Expr *Init,
  ...
  toolchain::APSInt TruncatedValue = Value.trunc(FieldWidth);
  TruncatedValue.setIsSigned(BitfieldType->isSignedIntegerType());

  // Check whether the stored value is equal to the original value.
  TruncatedValue = TruncatedValue.extend(OriginalWidth);
  if (toolchain::APSInt::isSameValue(Value, TruncatedValue))
    return false;
...
static void DiagnoseFloatingImpCast(Sema &S, const Expr *E, QualType T,
  ...
  toolchain::APSInt IntegerValue(S.Context.getIntWidth(T),
                            T->hasUnsignedIntegerRepresentation());
  toolchain::APFloat::opStatus Result = Value.convertToInteger(
      IntegerValue, toolchain::APFloat::rmTowardZero, &isExact);
  ...
  unsigned precision = toolchain::APFloat::semanticsPrecision(Value.getSemantics());
  ...
  if (Result == toolchain::APFloat::opOK && isExact) {
    if (IsLiteral) return;
  ...
  if (!IsBool && Result == toolchain::APFloat::opInvalidOp)
...
static std::string PrettyPrintInRange(const toolchain::APSInt &Value,
                                      IntRange Range) {
  if (!Range.Width) return "0";

  toolchain::APSInt ValueInRange = Value;
  ...
  if (E->EvaluateAsInt(Result, S.getASTContext(), Expr::SE_AllowSideEffects,
    ...
    if (!ConversionPreservesSemantics) {
      auto IsSingleCodeUnitCP = [](const QualType &T,
                                   const toolchain::APSInt &Value) {
        if (T->isChar8Type())
          return toolchain::IsSingleCodeUnitUTF8Codepoint(Value.getExtValue());
        if (T->isChar16Type())
          return toolchain::IsSingleCodeUnitUTF16Codepoint(Value.getExtValue());
        assert(T->isChar32Type());
        return toolchain::IsSingleCodeUnitUTF32Codepoint(Value.getExtValue());
      };
...
void Sema::CheckImplicitConversion(Expr *E, QualType T, SourceLocation CC,
  ...
  if (Source->isFixedPointType()) {
    if (Target->isUnsaturatedFixedPointType()) {
      ...
      if (E->EvaluateAsFixedPoint(Result, Context, Expr::SE_AllowSideEffects,
                                  isConstantEvaluatedContext())) {
        toolchain::APFixedPoint Value = Result.Val.getFixedPoint();
        toolchain::APFixedPoint MaxVal = Context.getFixedPointMax(T);
        toolchain::APFixedPoint MinVal = Context.getFixedPointMin(T);
        if (Value > MaxVal || Value < MinVal) {
          DiagRuntimeBehavior(E->getExprLoc(), E,
                              PDiag(diag::warn_impcast_fixed_point_range)
                                  << Value.toString() << T
                                  << E->getSourceRange()
                                  << language::Core::SourceRange(CC));
          return;
        }
      }
      ...
      if (!isConstantEvaluatedContext() &&
        ...
        toolchain::APFixedPoint FXResult = Result.Val.getFixedPoint();

        bool Overflowed;
        toolchain::APSInt IntResult = FXResult.convertToInt(
    ...
    if (Source->isIntegerType()) {
      ...
      if (!isConstantEvaluatedContext() &&
        ...
        bool Overflowed;
        toolchain::APFixedPoint IntResult = toolchain::APFixedPoint::getFromIntValue(
  ...
  if (SourceBT && TargetBT && SourceBT->isIntegerType() &&
    ...
    unsigned int TargetPrecision = toolchain::APFloatBase::semanticsPrecision(
        Context.getFloatTypeSemantics(QualType(TargetBT, 0)));

    if (SourcePrecision > 0 && TargetPrecision > 0 &&
        SourcePrecision > TargetPrecision) {

      if (std::optional<toolchain::APSInt> SourceInt =
        ...
        toolchain::APFloat TargetFloatValue(
            Context.getFloatTypeSemantics(QualType(TargetBT, 0)));
        toolchain::APFloat::opStatus ConversionStatus =
            TargetFloatValue.convertFromAPInt(
                *SourceInt, SourceBT->isSignedInteger(),
                toolchain::APFloat::rmNearestTiesToEven);

        if (ConversionStatus != toolchain::APFloat::opOK) {
          ...
          DiagRuntimeBehavior(
              ...
              PDiag(diag::warn_impcast_integer_float_precision_constant)
                  ...
                  << E->getSourceRange() << language::Core::SourceRange(CC));
        }
      } else {
...
static void CheckCommaOperand(
    ...
    toolchain::SmallVectorImpl<AnalyzeImplicitConversionsWorkItem> &WorkList) {
  E = E->IgnoreParenImpCasts();
...
static void AnalyzeImplicitConversions(
  ...
  if (auto *BO = dyn_cast<BinaryOperator>(SourceExpr)) {
    if ((BO->getOpcode() == BO_And || BO->getOpcode() == BO_Or) &&
      ...
      StringRef SR = language::Core::Lexer::getSourceText(
          language::Core::CharSourceRange::getTokenRange(BLoc, ELoc), SM, LO);
...
static void AnalyzeImplicitConversions(Sema &S, Expr *OrigE, SourceLocation CC,
                                       bool IsListInit/*= false*/) {
  toolchain::SmallVector<AnalyzeImplicitConversionsWorkItem, 16> WorkList;
...
void Sema::DiagnoseAlwaysNonNullPointer(Expr *E,
  ...
  auto ComplainAboutNonnullParamOrCall = [&](const Attr *NonnullAttr) {
    ...
    std::string Str;
    toolchain::raw_string_ostream S(Str);
    ...
    if (getCurFunction() &&
      ...
      if (const auto *FD = dyn_cast<FunctionDecl>(PV->getDeclContext())) {
        ...
        if (FD->getTemplatedKind() == FunctionDecl::TK_FunctionTemplate)
          return;
        auto ParamIter = toolchain::find(FD->parameters(), PV);
  ...
  if (IsAddressOf)
    ...
    toolchain_unreachable("Could not determine diagnostic.");
...
class SequenceChecker : public ConstEvaluatedExprVisitor<SequenceChecker> {
  ...
  using UsageInfoMap = toolchain::SmallDenseMap<Object, UsageInfo, 16>;
  ...
  struct SequencedSubexpression {
    ...
    ~SequencedSubexpression() {
      for (const std::pair<Object, Usage> &M : toolchain::reverse(ModAsSideEffect)) {
...
public:
  ...
  void VisitCXXOperatorCallExpr(const CXXOperatorCallExpr *CXXOCE) {
    ...
    SemaRef.runWithSufficientStackSpace(CXXOCE->getExprLoc(), [&] {
      ...
      if (SequencingKind == LHSBeforeRest) {
        ...
        toolchain::ArrayRef<const Expr *> Args(CXXOCE->getArgs() + 1,
                                          CXXOCE->getNumArgs() - 1);
  ...
  void VisitCXXConstructExpr(const CXXConstructExpr *CCE) {
    ...
    SequenceExpressionsInOrder(
        toolchain::ArrayRef(CCE->getArgs(), CCE->getNumArgs()));
  }
...
void Sema::CheckCompletedExpr(Expr *E, SourceLocation CheckLoc,
                              bool IsConstexpr) {
  toolchain::SaveAndRestore ConstantContext(isConstantEvaluatedOverride,
...
getAlignmentAndOffsetFromBinAddOrSub(const Expr *PtrE, const Expr *IntE,
  ...
  if (std::optional<toolchain::APSInt> IdxRes = IntE->getIntegerConstantExpr(Ctx)) {
...
void Sema::CheckArrayAccess(const Expr *BaseExpr, const Expr *IndexExpr,
  ...
  toolchain::APSInt index = Result.Val.getInt();
  if (IndexNegated) {
  ...
  if (IsUnboundedArray) {
    ...
    if (index.isUnsigned() || !index.isNegative()) {
      ...
      if (!ElemCharUnits || ElemCharUnits->isZero())
        return;
      toolchain::APInt ElemBytes(index.getBitWidth(), ElemCharUnits->getQuantity());
      ...
      toolchain::APInt MaxElems = toolchain::APInt::getMaxValue(AddrBits);
  ...
  if (index.isUnsigned() || !index.isNegative()) {
    ...
    toolchain::APInt size = ArrayTy->getSize();
    ...
    if (BaseType != EffectiveType) {
      ...
      if (ptrarith_typesize != array_typesize) {
        ...
        if (ptrarith_typesize * ratio == array_typesize)
          size *= toolchain::APInt(size.getBitWidth(), ratio);
      }
    }
...
static bool isLayoutCompatible(const ASTContext &C, const FieldDecl *Field1,
  ...
  if (Field1->hasAttr<language::Core::NoUniqueAddressAttr>() ||
      Field2->hasAttr<language::Core::NoUniqueAddressAttr>())
    return false;
...
static bool isLayoutCompatibleStruct(const ASTContext &C, const RecordDecl *RD1,
  ...
  // Check the fields.
  return toolchain::equal(RD1->fields(), RD2->fields(),
...
static bool isLayoutCompatibleUnion(const ASTContext &C, const RecordDecl *RD1,
                                    const RecordDecl *RD2) {
  toolchain::SmallPtrSet<const FieldDecl *, 8> UnmatchedFields(toolchain::from_range,
                                                          RD2->fields());
...
static bool FindTypeTagExpr(const Expr *TypeExpr, const ASTContext &Ctx,
  ...
  while(true) {
    ...
    case Stmt::IntegerLiteralClass: {
      ...
      toolchain::APInt MagicValueAPInt = IL->getValue();
...
static bool GetMatchingCType(
    ...
    const toolchain::DenseMap<Sema::TypeTagMagicValue, Sema::TypeTagData>
        *MagicValues,
  ...
  toolchain::DenseMap<Sema::TypeTagMagicValue,
...
void Sema::RegisterTypeTagForDatatype(const IdentifierInfo *ArgumentKind,
  ...
  if (!TypeTagForDatatypeMagicValues)
    TypeTagForDatatypeMagicValues.reset(
        new toolchain::DenseMap<TypeTagMagicValue, TypeTagData>);
...
void Sema::DiscardMisalignedMemberAddress(const Type *T, Expr *E) {
  ...
  if (isa<UnaryOperator>(E) &&
    ...
    if (isa<MemberExpr>(Op)) {
      ...
      auto *MA = toolchain::find(MisalignedMembersForExpr, MisalignedMember(Op));
...
void Sema::RefersToMemberWithReducedAlignment(
    Expr *E,
    toolchain::function_ref<void(Expr *, RecordDecl *, FieldDecl *, CharUnits)>
        Action) {
  ...
  CharUnits Offset;
  for (const FieldDecl *FD : toolchain::reverse(ReverseMemberChain))
...
getAndVerifyMatrixDimension(Expr *Expr, StringRef Name, Sema &S) {
  std::optional<toolchain::APSInt> Value = Expr->getIntegerConstantExpr(S.Context);
  if (!Value) {
...
ExprResult Sema::BuiltinMatrixColumnMajorLoad(CallExpr *TheCall,
  ...
  if (MaybeRows) {
    if (std::optional<toolchain::APSInt> Value =
...
void Sema::CheckTCBEnforcement(const SourceLocation CallExprLoc,
  ...
  toolchain::StringSet<> CalleeTCBs;
