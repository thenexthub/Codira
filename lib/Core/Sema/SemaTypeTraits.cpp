...
static CXXMethodDecl *LookupSpecialMemberFromXValue(Sema &SemaRef,
  ...
  ExprValueKind VK = language::Core::VK_XValue;
  ...
  for (NamedDecl *CandDecl : Candidates) {
    ...
    DeclAccessPair Cand = DeclAccessPair::make(CandDecl, language::Core::AS_none);
    ...
    if (CXXMethodDecl *M = dyn_cast<CXXMethodDecl>(Cand->getUnderlyingDecl())) {
      ...
      if (Assign)
        SemaRef.AddMethodTemplateCandidate(
            ...
            Classification, toolchain::ArrayRef(&Arg, NumArgs), OCS, true);
      else {
        assert(CtorInfo);
...
static bool DiagnoseVLAInCXXTypeTrait(Sema &S, const TypeSourceInfo *T,
                                      language::Core::tok::TokenKind TypeTraitID) {
...
static bool CheckUnaryTypeTraitTypeCompleteness(Sema &S, TypeTrait UTT,
  ...
  switch (UTT) {
  default:
    toolchain_unreachable("not a UTT");
...
static bool HasNonDeletedDefaultedEqualityComparison(Sema &S,
  ...
  return toolchain::all_of(Decl->bases(),
         ...
         toolchain::all_of(Decl->fields(), [&](const FieldDecl *FD) {
...
static ExprResult CheckConvertibilityForTypeTraits(
    ...
    SourceLocation KeyLoc, toolchain::BumpPtrAllocator &OpaqueExprAllocator) {
...
static APValue EvaluateSizeTTypeTrait(Sema &S, TypeTrait Kind,
  ...
  default:
    toolchain_unreachable("Not a SizeT type trait");
  }
}
...
static bool EvaluateBooleanTypeTrait(Sema &S, TypeTrait Kind,
  ...
  switch (Kind) {
  case language::Core::BTT_ReferenceBindsToTemporary:
  case language::Core::BTT_ReferenceConstructsFromTemporary:
  case language::Core::BTT_ReferenceConvertsFromTemporary:
  case language::Core::TT_IsConstructible:
  case language::Core::TT_IsNothrowConstructible:
  case language::Core::TT_IsTriviallyConstructible: {
    ...
    bool UseRawObjectType =
        Kind == language::Core::BTT_ReferenceBindsToTemporary ||
        Kind == language::Core::BTT_ReferenceConstructsFromTemporary ||
        Kind == language::Core::BTT_ReferenceConvertsFromTemporary;

    toolchain::BumpPtrAllocator OpaqueExprAllocator;
    ...
    if (Kind == language::Core::TT_IsConstructible)
      return true;

    if (Kind == language::Core::BTT_ReferenceBindsToTemporary ||
        Kind == language::Core::BTT_ReferenceConstructsFromTemporary ||
        Kind == language::Core::BTT_ReferenceConvertsFromTemporary) {
      if (!T->isReferenceType())
        return false;
    ...
    if (Kind == language::Core::TT_IsNothrowConstructible)
      return S.canThrow(Result.get()) == CT_Cannot;

    if (Kind == language::Core::TT_IsTriviallyConstructible) {
    ...
    toolchain_unreachable("unhandled type trait");
    return false;
  }
  default:
    toolchain_unreachable("not a TT");
  }
...
void DiagnoseBuiltinDeprecation(Sema &S, TypeTrait Kind, SourceLocation KWLoc) {
  ...
  case UTT_HasTrivialDestructor:
    ...
    break;
  case UTT_IsTriviallyRelocatable:
    Replacement = language::Core::UTT_IsCppTriviallyRelocatable;
    break;
  case BTT_ReferenceBindsToTemporary:
    Replacement = language::Core::BTT_ReferenceConstructsFromTemporary;
    break;
  default:
    return;
  }
...
ExprResult Sema::BuildTypeTrait(TypeTrait Kind, SourceLocation KWLoc,
  ...
  toolchain_unreachable("unhandled type trait return type");
}
...
static bool EvaluateBinaryTypeTrait(Sema &Self, TypeTrait BTT,
  ...
  default:
    toolchain_unreachable("not a BTT");
  }
  toolchain_unreachable("Unknown type trait or not implemented");
}
...
static bool EvaluateExpressionTrait(ExpressionTrait ET, Expr *E) {
  ...
  toolchain_unreachable("Expression trait not covered by switch");
}
...
static std::optional<TypeTrait> StdNameToTypeTrait(StringRef Name) {
  return toolchain::StringSwitch<std::optional<TypeTrait>>(Name)
...
using ExtractedTypeTraitInfo =
    std::optional<std::pair<TypeTrait, toolchain::SmallVector<QualType, 1>>>;
...
static ExtractedTypeTraitInfo ExtractTypeTraitFromExpression(const Expr *E) {
  toolchain::SmallVector<QualType, 1> Args;
...
static void DiagnoseNonConstructibleReason(
    ...
    const toolchain::SmallVector<language::Core::QualType, 1> &Ts) {
  if (Ts.empty()) {
    return;
  }
