...
void AttributePool::takePool(AttributePool &pool) {
  toolchain::append_range(Attrs, pool.Attrs);
  pool.Attrs.clear();
}
...
void AttributePool::takeFrom(ParsedAttributesView &List, AttributePool &Pool) {
  ...
  for (ParsedAttr *A : List.AttrList)
    Pool.remove(A);
  toolchain::append_range(Attrs, List.AttrList);
}
...
ArrayRef<const ParsedAttrInfo *> ParsedAttrInfo::getAllBuiltin() {
  return toolchain::ArrayRef(AttrInfoMap);
}
...
void language::Core::takeAndConcatenateAttrs(ParsedAttributes &First,
