...
static StringInitFailureKind IsStringInit(Expr *Init, const ArrayType *AT,
  ...
  toolchain_unreachable("missed a StringLiteral kind?");
}
...
static void CheckStringInit(Expr *Str, QualType &DeclT, const ArrayType *AT,
  ...
  if (const IncompleteArrayType *IAT = dyn_cast<IncompleteArrayType>(AT)) {
    ...
    toolchain::APInt ConstVal(32, StrLength);
...
class InitListChecker {
  ...
  bool CheckDesignatedInitializer(const InitializedEntity &Entity,
                                  ...
                                  toolchain::APSInt *NextElementIndex,
                                  unsigned &Index,
                                  InitListExpr *StructuredList,
                                  unsigned &StructuredIndex,
...
ExprResult InitListChecker::PerformEmptyInit(SourceLocation Loc,
  ...
  if (!InitSeq && EmptyInitList &&
    ...
    if (CtorDecl->getMinRequiredArguments() == 0 &&
      ...
      if (IsInStd && toolchain::StringSwitch<bool>(R->getName())
...
void InitListChecker::CheckImplicitInitList(const InitializedEntity &Entity,
  ...
  if (T->isArrayType())
    ...
    toolchain_unreachable("CheckImplicitInitList(): Illegal type");
...
static void warnBracedScalarInit(Sema &S, const InitializedEntity &Entity,
  ...
  case InitializedEntity::EK_StmtExprResult:
    toolchain_unreachable("unexpected braced scalar init");
  }
  ...
  if (!SemaRef.getLangOpts().OpenCL && !SemaRef.getLangOpts().HLSL ) {
    ...
    if (isBigEndian && (T->getVectorKind() == VectorKind::Neon ||
      ...
      if (elementType->isFloatingType())
        ...
        toolchain_unreachable("Invalid element type!");
...
void InitListChecker::CheckArrayType(const InitializedEntity &Entity,
  ...
  toolchain::APSInt maxElements(elementIndex.getBitWidth(),
  ...
  if (!hadError && DeclType->isIncompleteArrayType() && !VerifyOnly) {
    ...
    // be calculated here.
    toolchain::APSInt Zero(maxElements.getBitWidth(), maxElements.isUnsigned());
...
void InitListChecker::CheckStructUnionTypes(
  ...
  size_t NumRecordDecls = toolchain::count_if(RD->decls(), [&](const Decl *D) {
...
InitListChecker::CheckDesignatedInitializer(const InitializedEntity &Entity,
  ...
  toolchain::APSInt DesignatedStartIndex, DesignatedEndIndex;
  ...
  if (isa<ConstantArrayType>(AT)) {
    toolchain::APSInt MaxElements(cast<ConstantArrayType>(AT)->getSize(), false);
    DesignatedStartIndex
  ...
  if (IsStringLiteralInitUpdate && VerifyOnly) {
    ...
    if (StringLiteral *SL = dyn_cast<StringLiteral>(SubExpr)) {
      ...
      for (unsigned i = 0, e = StrLen; i != e; ++i) {
        toolchain::APInt CodeUnit(PromotedCharTyWidth, SL->getCodeUnit(i));
      ...
      // the init list.
      for (unsigned i = 0, e = StrLen; i != e; ++i) {
        toolchain::APInt CodeUnit(PromotedCharTyWidth, Str[i]);
        Expr *Init = new (Context) IntegerLiteral(
            Context, CodeUnit, PromotedCharTy, SubExpr->getExprLoc());
        if (CharTy != PromotedCharTy)
          Init = ImplicitCastExpr::Create(Context, CharTy, CK_IntegralCast,
...
static ExprResult
CheckArrayDesignatorExpr(Sema &S, Expr *Index, toolchain::APSInt &Value) {
...
ExprResult Sema::ActOnDesignatedInitializer(Designation &Desig,
  ...
  for (unsigned Idx = 0; Idx < Desig.getNumDesignators(); ++Idx) {
    ...
    if (D.isFieldDesignator()) {
      ...
      toolchain::APSInt StartValue;
      toolchain::APSInt EndValue;
...
DeclarationName InitializedEntity::getName() const {
  ...
  toolchain_unreachable("Invalid EntityKind!");
}
...
bool InitializationSequence::isDirectReferenceBinding() const {
  ...
  for (const Step &S : toolchain::reverse(Steps)) {
...
static bool TryInitializerListConstruction(Sema &S,
  ...
  QualType ArrayType = S.Context.getConstantArrayType(
      E.withConst(),
      toolchain::APInt(S.Context.getTypeSize(S.Context.getSizeType()),
                  List->getNumInitsWithEmbedExpanded()),
      nullptr, language::Core::ArraySizeModifier::Normal, 0);
  ...
  if (Sequence) {
    if (DestType->isRValueReferenceType() ||
      ...
      if (S.getLangOpts().CPlusPlus20 &&
        ...
        Sequence.AddQualificationConversionStep(cv1T1, language::Core::VK_PRValue);
      }
  ...
  if (S.getLangOpts().CPlusPlus11 && InitList->getNumInits() == 1 &&
    ...
    if (const ArrayType *DestAT = S.Context.getAsArrayType(DestType)) {
      ...
      if (isa<ConstantArrayType>(DestAT) &&
        ...
        assert(Kind.getKind() == language::Core::InitializationKind::IK_DirectList &&
    ...
    if (S.Context.getCanonicalType(T2) == S.Context.OverloadTy)
      ...
      case Sema::Ref_Compatible:
        if (Initializer->refersToBitField())
          ...
          toolchain_unreachable("unexpected kind of compatible initializer");
        break;
  ...
  if (const ArrayType *AT =
    ...
    if (ResultType.isNull()) {
      ResultType = S.Context.getConstantArrayType(
          AT->getElementType(), toolchain::APInt(/*numBits=*/32, ArrayLength),
    ...
    if (RD->isInvalidDecl()) {
      ...
      Sequence.SetFailed(
          language::Core::InitializationSequence::FK_ParenthesizedListInitFailed);
      return;
    }
...
static bool IsZeroInitializer(const Expr *Init, ASTContext &Ctx) {
  std::optional<toolchain::APSInt> Value = Init->getIntegerConstantExpr(Ctx);
...
void InitializationSequence::InitializeFrom(Sema &S,
  ...
  if (S.getLangOpts().HLSL && Args.size() > 1 && DestType->isExtVectorType() &&
    ...
    toolchain::SmallVector<Expr *> InitArgs;
    for (auto *Arg : Args) {
        ...
        for (unsigned Idx = 0; Idx < Elm; ++Idx) {
          InitArgs.emplace_back(new (Context) ArraySubscriptExpr(
              ...
              IntegerLiteral::Create(
                  Context, toolchain::APInt(Context.getIntWidth(Context.IntTy), Idx),
...
static bool shouldBindAsTemporary(const InitializedEntity &Entity) {
  ...
  toolchain_unreachable("missed an InitializedEntity kind?");
}
  ...
  case FK_UserConversionOverloadFailed:
    ...
    case OR_Deleted: {
      ...
      if (Ovl == OR_Deleted) {
        ...
        toolchain_unreachable("Inconsistent overload resolution?");
      }
      break;
    }

    case OR_Success:
      toolchain_unreachable("Conversion did not fail!");
    }
    break;
...
static void DiagnoseNarrowingInInitList(Sema &S,
  ...
  toolchain::raw_svector_ostream OS(StaticCast);
  OS << "static_cast<";
...
static void CheckC23ConstexprInitStringLiteral(const StringLiteral *SE,
  ...
  toolchain::APSInt Value(BitWidth, isUnsigned);
...
Sema::PerformCopyInitialization(const InitializedEntity &Entity,
  ...
  if (ShouldTrackCopy) {
    if (toolchain::is_contained(CurrentParameterCopyTypes, Entity.getType())) {
      ...
      auto Candidate =
          toolchain::find_if(Seq.getFailedCandidateSet(),
...
QualType Sema::DeduceTemplateSpecializationFromInitializer(
  ...
  if (!Template) {
    if (auto *AliasTemplate = dyn_cast_or_null<TypeAliasTemplateDecl>(
      ...
      if (const auto *TST =
        ...
        if (const auto *CTSD = toolchain::dyn_cast<ClassTemplateSpecializationDecl>(
  ...
  auto addDeductionCandidate = [&](FunctionTemplateDecl *TD,
    ...
    if (TD) {
      ...
      for (auto [I, E] : toolchain::enumerate(Inits)) {
