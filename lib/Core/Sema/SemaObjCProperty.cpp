...
CheckPropertyAgainstProtocol(Sema &S, ObjCPropertyDecl *Prop,
                             ...
                             toolchain::SmallPtrSetImpl<ObjCProtocolDecl *> &Known) {
...
static unsigned deducePropertyOwnershipFromType(Sema &S, QualType T) {
  ...
  if (S.getLangOpts().getGC() != LangOptions::NonGC) {
    ...
    toolchain_unreachable("bad qualifier");
  }
...
Decl *SemaObjC::ActOnProperty(Scope *S, SourceLocation AtLoc,
  ...
  toolchain::SmallPtrSet<ObjCProtocolDecl *, 16> KnownProtos;
...
static void checkARCPropertyImpl(Sema &S, SourceLocation propertyImplLoc,
  ...
  case Qualifiers::OCL_Autoreleasing:
    toolchain_unreachable("properties cannot be autoreleasing");
...
SelectPropertyForSynthesisFromProtocols(Sema &S, SourceLocation AtLoc,
  ...
  for (const auto &Prop : toolchain::enumerate(Properties)) {
...
static void DiagnoseUnimplementedAccessor(
    ...
    ObjCPropertyDecl *Prop,
    toolchain::SmallPtrSet<const ObjCMethodDecl *, 8> &SMap) {
  // Check to see if we have a corresponding selector in SMap and with the
  // right method type.
  auto I = toolchain::find_if(SMap, [&](const ObjCMethodDecl *x) {
...
void SemaObjC::DiagnoseUnimplementedProperties(Scope *S, ObjCImplDecl *IMPDecl,
  ...
  toolchain::DenseSet<ObjCPropertyDecl *> PropImplMap;
  for (const auto *I : IMPDecl->property_impls())
    PropImplMap.insert(I->getPropertyDecl());

  // Collect property accessors implemented in current implementation.
  toolchain::SmallPtrSet<const ObjCMethodDecl *, 8> InsMap(toolchain::from_range,
                                                      IMPDecl->methods());
