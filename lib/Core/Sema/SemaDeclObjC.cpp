...
SemaObjC::actOnObjCTypeParamList(Scope *S, SourceLocation lAngleLoc,
  ...
  toolchain::SmallDenseMap<IdentifierInfo *, ObjCTypeParamDecl *> knownParams;
...
void SemaObjC::actOnObjCTypeArgsOrProtocolQualifiers(
  ...
  auto resolvedAsProtocols = [&] {
    ...
    if (allAreTypeNames && firstClassNameLoc.isValid()) {
      toolchain::SmallPtrSet<ObjCProtocolDecl*, 8> knownProtocols;
  ...
  // \c*.
  typedef toolchain::PointerUnion<TypeDecl *, ObjCInterfaceDecl *> TypeOrClassDecl;
...
void SemaObjC::DiagnoseClassExtensionDupMethods(ObjCCategoryDecl *CAT,
  ...
  toolchain::DenseMap<Selector, const ObjCMethodDecl*> MethodMap;
...
static void WarnUndefinedMethod(Sema &S, ObjCImplDecl *Impl,
  ...
  // matches what we do with protocols.
  {
    ...
    std::string FixItStr;
    toolchain::raw_string_ostream Out(FixItStr);
...
static bool checkMethodFamilyMismatch(Sema &S, ObjCMethodDecl *impl,
  ...
  switch (family) {
  case OMF_None: toolchain_unreachable("logic error, no method convention");
  case OMF_retain:
  case OMF_release:
  case OMF_autorelease:
  case OMF_dealloc:
...
typedef toolchain::DenseSet<IdentifierInfo*> ProtocolNameSet;
...
void SemaObjC::ImplMethodsVsClassMethods(Scope *S, ObjCImplDecl *IMPDecl,
  ...
  if (ObjCInterfaceDecl *I = dyn_cast<ObjCInterfaceDecl> (CDecl)) {
    ...
    toolchain_unreachable("invalid ObjCContainerDecl type.");
}
...
static bool FilterMethodsByTypeBound(ObjCMethodDecl *Method,
  ...
  toolchain_unreachable("unknown method context");
}
...
static void DiagnoseVariableSizedIvars(Sema &S, ObjCContainerDecl *OCD) {
  ...
  ObjCInterfaceDecl::ivar_range Ivars = toolchain::make_range(
...
static void DiagnoseCategoryDirectMembersProtocolConformance(
    ...
    const toolchain::iterator_range<ObjCProtocolList::iterator> &Protocols) {
...
static void DiagnoseCategoryDirectMembersProtocolConformance(
  ...
  toolchain::SmallVector<const Decl *, 4> DirectMembers;
...
Decl *SemaObjC::ActOnAtEnd(Scope *S, SourceRange AtEnd,
  ...
  toolchain::DenseMap<Selector, const ObjCMethodDecl*> InsMap;
  toolchain::DenseMap<Selector, const ObjCMethodDecl*> ClsMap;
...
public:
  ...
  toolchain::SmallSetVector<ObjCMethodDecl*, 4> Overridden;
  bool Recursive;
...
#include "language/Core/AST/DeclNodes.inc"
      toolchain_unreachable("not an ObjC container!");
    }
  }
...
TranslateIvarVisibility(tok::ObjCKeywordKind ivarVisibility) {
  ...
  default:
    toolchain_unreachable("Unknown visitibility kind");
