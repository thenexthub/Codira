...
DarwinSDKInfo *Sema::getDarwinSDKInfoForAvailabilityChecking() {
  ...
  if (!SDKInfo)
    toolchain::consumeError(SDKInfo.takeError());
...
IdentifierInfo *Sema::InventAbbreviatedTemplateParameterTypeName(
  ...
  toolchain::raw_string_ostream OS(InventedName);
...
class SemaPPCallbacks : public PPCallbacks {
  Sema *S = nullptr;
  toolchain::SmallVector<SourceLocation, 8> IncludeStack;
  toolchain::SmallVector<toolchain::TimeTraceProfilerEntry *, 8> ProfilerStack;
  ...
  void FileChanged(SourceLocation Loc, FileChangeReason Reason,
    ...
    case EnterFile: {
      ...
      if (IncludeLoc.isValid()) {
        if (toolchain::timeTraceProfilerEnabled()) {
          ...
          ProfilerStack.push_back(toolchain::timeTraceAsyncProfilerBegin(
    ...
    case ExitFile:
      if (!IncludeStack.empty()) {
        if (toolchain::timeTraceProfilerEnabled())
          toolchain::timeTraceProfilerEnd(ProfilerStack.pop_back_val());
...
void Sema::addExternalSource(IntrusiveRefCntPtr<ExternalSemaSource> E) {
  ...
  if (auto *Ex = dyn_cast<MultiplexExternalSemaSource>(ExternalSource.get()))
    ...
    ExternalSource = toolchain::makeIntrusiveRefCnt<MultiplexExternalSemaSource>(
        ExternalSource, std::move(E));
}

void Sema::PrintStats() const {
  toolchain::errs() << "\n*** Semantic Analysis Stats:\n";
  toolchain::errs() << NumSFINAEErrors << " SFINAE diagnostics trapped.\n";
...
CastKind Sema::ScalarTypeToBooleanCastKind(QualType ScalarTy) {
  ...
  toolchain_unreachable("unknown scalar type kind");
}
...
typedef toolchain::DenseMap<const CXXRecordDecl*, bool> RecordCompleteMap;
...
void Sema::ActOnEndOfTranslationUnitFragment(TUFragmentKind Kind) {
  if (Kind == TUFragmentKind::Global) {
    ...
    toolchain::TimeTraceScope TimeScope("PerformPendingInstantiations");
...
void Sema::ActOnEndOfTranslationUnit() {
  ...
  if (Module *CurrentModule = getCurrentModule();
    ...
    CurrentModule->NamedModuleHasInit =
        ...
        toolchain::any_of(CurrentModule->submodules(), DoesModNeedInit);
  }
  ...
  toolchain::SmallSet<VarDecl *, 32> Seen;
  for (TentativeDefinitionsType::iterator
    ...
    if (const IncompleteArrayType *ArrayT
      ...
      toolchain::APInt One(Context.getTypeSize(Context.getSizeType()), true);
...
void Sema::EmitDiagnostic(unsigned DiagID, const DiagnosticBuilder &DB) {
  ...
  // TODO: Stop doing that.  See: https://reviews.toolchain.org/D45093#1090292
...
public:
  ...
  toolchain::SmallPtrSet<CanonicalDeclPtr<Decl>, 4> InUsePath;

  // The current use-path.
  toolchain::SmallVector<CanonicalDeclPtr<FunctionDecl>, 4> UsePath;
  ...
  toolchain::SmallPtrSet<CanonicalDeclPtr<Decl>, 4> DoneMap[2];
...
void Sema::checkTypeSupport(QualType Ty, SourceLocation Loc, ValueDecl *D) {
  ...
  auto CheckDeviceType = [&](QualType Ty) {
    ...
    if (Ty->isRealFloatingType() && Context.getTypeSize(Ty) == 128) {
      const toolchain::fltSemantics &Sem = Context.getFloatTypeSemantics(Ty);
      if ((&Sem != &toolchain::APFloat::PPCDoubleDouble() &&
           !Context.getTargetInfo().hasFloat128Type()) ||
          (&Sem == &toolchain::APFloat::PPCDoubleDouble() &&
...
void Sema::ActOnComment(SourceRange Comment) {
  ...
  if (RC.isAlmostTrailingComment() || RC.hasUnsupportedSplice(SourceMgr)) {
    ...
    case RawComment::RCK_Invalid:
      ...
      return;
    default:
      toolchain_unreachable("if this is an almost Doxygen comment, "
...
void ExternalSemaSource::ReadMismatchingDeleteExpressions(toolchain::MapVector<
...
const toolchain::MapVector<FieldDecl *, Sema::DeleteLocs> &
...
bool Sema::isDeclaratorFunctionLike(Declarator &D) {
  ...
  bool Result = toolchain::all_of(LR, [](Decl *Dcl) {
