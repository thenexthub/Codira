...
#include "language/Core/Analysis/Analyses/Consumed.h"
#include "language/Core/Analysis/Analyses/LifetimeSafety.h"
#include "language/Core/Analysis/Analyses/ReachableCode.h"
#include "language/Core/Analysis/Analyses/ThreadSafety.h"
#include "language/Core/Analysis/Analyses/UninitializedValues.h"
...
static bool checkForRecursiveFunctionCall(const FunctionDecl *FD, CFG *cfg) {
  toolchain::SmallPtrSet<CFGBlock *, 16> Visited;
  toolchain::SmallVector<CFGBlock *, 16> WorkList;
...
static bool throwEscapes(Sema &S, const CXXThrowExpr *E, CFGBlock &ThrowBlock,
  ...
  toolchain::BitVector Queued(Body->getNumBlockIDs());
...
static void visitReachableThrows(
    CFG *BodyCFG,
    toolchain::function_ref<void(const CXXThrowExpr *, CFGBlock &)> Visit) {
  toolchain::BitVector Reachable(BodyCFG->getNumBlockIDs());
  language::Core::reachable_code::ScanReachableFromBlock(&BodyCFG->getEntry(), Reachable);
...
static bool areAllValuesNoReturn(const VarDecl *VD, const CFGBlock &VarBlk,
  ...
  using MapTy = toolchain::DenseMap<const CFGBlock *, std::optional<bool>>;
  ...
  toolchain::DenseSet<const CFGBlock *> Visited;
  ...
  while (const CFGBlock *B = Worklist.dequeue()) {
    ...
    if (toolchain::all_of(BlocksToCheck, BlockSatisfiesCondition))
      return true;
...
static ControlFlowKind CheckFallThrough(AnalysisDeclContext &AC) {
  ...
  toolchain::BitVector live(cfg->getNumBlockIDs());
  ...
  for (CFGBlock::filtered_pred_iterator I =
    ...
    if (!toolchain::is_contained(B.succs(), &cfg->getExit())) {
      HasAbnormalEdge = true;
      continue;
    }
...
public:
  ...
  typedef toolchain::SmallPtrSet<const AttributedStmt *, 8> AttrStmts;
  ...
  bool checkFallThroughIntoBlock(const CFGBlock &B, int &AnnotatedCnt,
    ...
    while (!BlockQueue.empty()) {
      ...
      if (!ReachableBlocks.count(P)) {
        for (const CFGElement &Elem : toolchain::reverse(*P)) {
  ...
  private:
    ...
    static const Stmt *getLastStmt(const CFGBlock &B) {
      if (const Stmt *Term = B.getTerminatorStmt())
        return Term;
      for (const CFGElement &Elem : toolchain::reverse(B))
        if (std::optional<CFGStmt> CS = Elem.getAs<CFGStmt>())
          return CS->getStmt();
    ...
    Sema &S;
    toolchain::SmallPtrSet<const CFGBlock *, 16> ReachableBlocks;
};
} // anonymous namespace
...
static StringRef getFallthroughAttrSpelling(Preprocessor &PP,
  ...
  if (MacroName.empty()) {
    if (!PreferClangAttr)
      ...
      MacroName = "[[language::Core::fallthrough]]";
    else
...
static void DiagnoseSwitchLabelsFallthrough(Sema &S, AnalysisDeclContext &AC,
  ...
  for (const CFGBlock *B : toolchain::reverse(*Cfg)) {
...
static void diagnoseRepeatedUseOfWeak(Sema &S,
  ...
  for (const auto &P : UsesByStmt) {
    ...
    if (isa<VarDecl>(KeyProp))
      ...
      toolchain_unreachable("Unexpected weak object kind!");
...
class UninitValsDiagReporter : public UninitVariablesHandler {
  ...
  typedef toolchain::PointerIntPair<UsesVec *, 1, bool> MappedType;
  // Prefer using MapVector to DenseMap, so that iteration order will be
  // the same as insertion order. This is needed to obtain a deterministic
  // order of diagnostics when calling flushDiagnostics().
  typedef toolchain::MapVector<const VarDecl *, MappedType> UsesMap;
  UsesMap uses;
...
private:
  static bool hasAlwaysUninitializedUse(const UsesVec* vec) {
    return toolchain::any_of(*vec, [](const UninitUse &U) {
  ...
  void diagnoseUnitializedVar(const VarDecl *vd, bool hasSelfInit,
    ...
    // a stable ordering.
    toolchain::sort(*vec, [](const UninitUse &a, const UninitUse &b) {
...
private:
  ...
  toolchain::DenseMap<const BlockDecl *, DelayedDiagnostics> DelayedBlockWarnings;
};
...
bool shouldAnalyzeCalledOnceImpl(toolchain::ArrayRef<unsigned> DiagIDs,
                                 const DiagnosticsEngine &Diags,
                                 SourceLocation At) {
  return toolchain::any_of(DiagIDs, [&Diags, At](unsigned DiagID) {
...
namespace {
class ThreadSafetyReporter : public language::Core::threadSafety::ThreadSafetyHandler {
  Sema &S;
  DiagList Warnings;
 ...
 public:
  ...
  void handleNoMutexHeld(const NamedDecl *D, ProtectedOperationKind POK,
    ...
    case POK_FunctionCall:
      toolchain_unreachable("Only works for variables");
      break;
    }
...
public:
  ...
  void handleUnsafeVariableGroup(const VarDecl *Variable,
    ...
    if (!Fixes.empty()) {
      ...
      case language::Core::FixitStrategy::Kind::Span:
        FixItStrategy = 0;
        break;
      case language::Core::FixitStrategy::Kind::Array:
        FixItStrategy = 1;
        break;
      default:
  ...
  // Returns the text representation of language::Core::unsafe_buffer_usage attribute.
  ...
  getUnsafeBufferUsageAttributeTextAt(SourceLocation Loc,
    ...
    if (MacroName.empty())
      MacroName = "[[language::Core::unsafe_buffer_usage]]";
...
private:
  toolchain::function_ref<void(const Decl *)> Callback;
  const Module *const TUModule;

public:
  CallableVisitor(toolchain::function_ref<void(const Decl *)> Callback,
                  const Module *const TUModule)
...
void language::Core::sema::AnalysisBasedWarnings::IssueWarnings(
  ...
  auto CallAnalyzers = [&](const Decl *Node) -> void {
    ...
    if (!Diags.isIgnored(diag::warn_unsafe_buffer_operation,
        ...
        !Diags.isIgnored(diag::warn_unsafe_buffer_libc_call,
                         Node->getBeginLoc())) {
      language::Core::checkUnsafeBufferUsage(Node, R,
...
void language::Core::sema::AnalysisBasedWarnings::PrintStats() const {
  toolchain::errs() << "\n*** Analysis Based Warnings Stats:\n";

  unsigned NumCFGsBuilt = NumFunctionsAnalyzed - NumFunctionsWithBadCFGs;
  unsigned AvgCFGBlocksPerFunction =
      !NumCFGsBuilt ? 0 : NumCFGBlocks/NumCFGsBuilt;
  toolchain::errs() << NumFunctionsAnalyzed << " functions analyzed ("
  ...
  toolchain::errs() << NumUninitAnalysisFunctions
