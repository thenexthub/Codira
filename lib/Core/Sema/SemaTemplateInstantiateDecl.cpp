...
#include "language/Core/AST/DependentDiagnostic.h"
#include "language/Core/AST/Expr.h"
#include "language/Core/AST/ExprCXX.h"
#include "language/Core/AST/PrettyDeclStackTrace.h"
...
#include "language/Core/Sema/AttrTemplateInstantiate.inc"
...
static void instantiateDependentAllocAlignAttr(
  ...
  Expr *Param = IntegerLiteral::Create(
      S.getASTContext(),
      toolchain::APInt(64, Align->getParamIndex().getSourceIndex()),
...
static void instantiateDependentAnnotationAttr(
  ...
  if (HasDelayedArgs) {
    ...
    toolchain::SmallVector<Expr *, 4> ActualArgs;
...
attrToRetainOwnershipKind(const Attr *A) {
  switch (A->getKind()) {
  case language::Core::attr::CFConsumed:
    return Sema::RetainOwnershipKind::CF;
  case language::Core::attr::OSConsumed:
    return Sema::RetainOwnershipKind::OS;
  case language::Core::attr::NSConsumed:
    return Sema::RetainOwnershipKind::NS;
  default:
    toolchain_unreachable("Wrong argument supplied");
  }
}
...
TemplateDeclInstantiator::VisitTranslationUnitDecl(TranslationUnitDecl *D) {
  toolchain_unreachable("Translation units cannot be instantiated");
}

Decl *TemplateDeclInstantiator::VisitHLSLBufferDecl(HLSLBufferDecl *Decl) {
  toolchain_unreachable("HLSL buffer declarations cannot be instantiated");
}

Decl *TemplateDeclInstantiator::VisitHLSLRootSignatureDecl(
    HLSLRootSignatureDecl *Decl) {
  toolchain_unreachable("HLSL root signature declarations cannot be instantiated");
}

Decl *
TemplateDeclInstantiator::VisitPragmaCommentDecl(PragmaCommentDecl *D) {
  toolchain_unreachable("pragma comment cannot be instantiated");
}
...
TemplateDeclInstantiator::VisitExternCContextDecl(ExternCContextDecl *D) {
  toolchain_unreachable("extern \"C\" context cannot be instantiated");
}

Decl *TemplateDeclInstantiator::VisitMSGuidDecl(MSGuidDecl *D) {
  toolchain_unreachable("GUID declaration cannot be instantiated");
}

Decl *TemplateDeclInstantiator::VisitUnnamedGlobalConstantDecl(
    UnnamedGlobalConstantDecl *D) {
  toolchain_unreachable("UnnamedGlobalConstantDecl cannot be instantiated");
}

Decl *TemplateDeclInstantiator::VisitTemplateParamObjectDecl(
    TemplateParamObjectDecl *D) {
  toolchain_unreachable("template parameter objects cannot be instantiated");
}
...
TemplateDeclInstantiator::VisitNamespaceDecl(NamespaceDecl *D) {
  toolchain_unreachable("Namespaces cannot be instantiated");
}
...
#define CLAUSE_NOT_ON_DECLS(CLAUSE_NAME)                                       \
  void OpenACCDeclClauseInstantiator::Visit##CLAUSE_NAME##Clause(              \
    ...
    toolchain_unreachable("Clause type invalid on declaration construct, or "       \
...
toolchain::SmallVector<OpenACCClause *> InstantiateOpenACCClauseList(
...
Decl *TemplateDeclInstantiator::VisitVarDecl(VarDecl *D,
  ...
  if (D->isNRVOVariable() && !Var->isInvalidDecl()) {
    ...
    if (auto *F = dyn_cast<FunctionDecl>(DC))
      ...
      toolchain_unreachable("Unknown context type");
...
Decl *TemplateDeclInstantiator::VisitEnumConstantDecl(EnumConstantDecl *D) {
  toolchain_unreachable("EnumConstantDecls can only occur within EnumDecls.");
}

Decl *
TemplateDeclInstantiator::VisitBuiltinTemplateDecl(BuiltinTemplateDecl *D) {
  toolchain_unreachable("BuiltinTemplateDecls cannot be instantiated.");
}
  ...
  if (isFriend) {
    if (NumTempParamLists)
      Method->setTemplateParameterListsInfo(
          SemaRef.Context,
          toolchain::ArrayRef(TempParamLists.data(), NumTempParamLists));
...
Decl *TemplateDeclInstantiator::VisitUnresolvedUsingIfExistsDecl(
  ...
  toolchain_unreachable("referring to unresolved decl out of UsingShadowDecl");
}
...
TemplateDeclInstantiator::VisitOMPDeclareMapperDecl(OMPDeclareMapperDecl *D) {
  ...
  SemaRef.OpenMP().StartOpenMPDSABlock(toolchain::omp::OMPD_declare_mapper, DirName,
                                       /*S=*/nullptr,
...
Decl *TemplateDeclInstantiator::VisitOMPCapturedExprDecl(
  ...
  toolchain_unreachable("Should not be met in templates");
}
...
Decl *TemplateDeclInstantiator::VisitRecordDecl(RecordDecl *D) {
  toolchain_unreachable("There are only CXXRecordDecls in C++");
}
...
Decl *TemplateDeclInstantiator::VisitObjCAtDefsFieldDecl(ObjCAtDefsFieldDecl *D) {
  toolchain_unreachable("@defs is not supported in Objective-C++");
}
...
Decl *TemplateDeclInstantiator::VisitConceptDecl(ConceptDecl *D) {
  toolchain_unreachable("Concept definitions cannot reside inside a template");
}

Decl *TemplateDeclInstantiator::VisitImplicitConceptSpecializationDecl(
    ImplicitConceptSpecializationDecl *D) {
  toolchain_unreachable("Concept specializations cannot reside inside a template");
}
...
Decl *TemplateDeclInstantiator::VisitDecl(Decl *D) {
  toolchain_unreachable("Unexpected decl");
}
...
void Sema::addInstantiatedLocalVarsToScope(FunctionDecl *Function,
  ...
  for (auto *decl : PatternDecl->decls()) {
    ...
    auto it = toolchain::find_if(Function->decls(), [&](Decl *inst) {
...
void Sema::InstantiateFunctionDefinition(SourceLocation PointOfInstantiation,
  ...
  toolchain::TimeTraceScope TimeScope("InstantiateFunction", [&]() {
    toolchain::TimeTraceMetadata M;
    toolchain::raw_string_ostream OS(M.Detail);
    Function->getNameForDiagnostic(OS, getPrintingPolicy(),
                                   /*Qualified=*/true);
    if (toolchain::isTimeTraceVerbose()) {
  ...
  if (PatternDecl->isDefaulted()) {
    ...
    if (auto *Primary = Function->getPrimaryTemplate();
      ...
      auto It = toolchain::find_if(Primary->redecls(),
  ...
  bool ShouldSkipCG = [&] {
    ...
    return toolchain::any_of(ExprEvalContexts, [](auto &Context) {
...
void Sema::InstantiateVariableDefinition(SourceLocation PointOfInstantiation,
  ...
  if (Def->isStaticDataMember() && !Def->isOutOfLine()) {
    ...
    if (Var) {
      toolchain::PointerUnion<VarTemplateDecl *,
  ...
  if (ParentDC != D->getDeclContext()) {
    ...
    if (!Result) {
      if (isa<UsingShadowDecl>(D)) {
        ...
        toolchain_unreachable("Unable to find instantiation of declaration!");
      }
    }
...
void Sema::PerformPendingInstantiations(bool LocalOnly, bool AtEndOfTU) {
  ...
  while (!PendingLocalImplicitInstantiations.empty() ||
    ...
    case TSK_Undeclared:
      toolchain_unreachable("Cannot instantitiate an undeclared specialization.");
