//===-- CGValue.h - LLVM CodeGen wrappers for toolchain::Value* ------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// These classes implement wrappers around toolchain::Value in order to
// fully represent the range of values for C L- and R- values.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_LIB_CODEGEN_CGVALUE_H
#define LANGUAGE_CORE_LIB_CODEGEN_CGVALUE_H
...
class RValue {
  ...
  union {
    ...
    struct {
      toolchain::Value *first;
      toolchain::Value *second;
    } Vals;
...
public:
  ...
  toolchain::Value *getScalarVal() const {
  ...
  ///
  std::pair<toolchain::Value *, toolchain::Value *> getComplexVal() const {
  ...
  toolchain::Value *getAggregatePointer(QualType PointeeType,
  ...
  static RValue get(toolchain::Value *V) {
    RValue ER;
    ER.Vals.first = V;
    ER.Flavor = Scalar;
    ER.IsVolatile = false;
  ...
  static RValue getComplex(toolchain::Value *V1, toolchain::Value *V2) {
    RValue ER;
    ER.Vals = {V1, V2};
    ER.Flavor = Complex;
    ER.IsVolatile = false;
  ...
  static RValue getComplex(const std::pair<toolchain::Value *, toolchain::Value *> &C) {
...
class LValue {
  ...
  union {
    ...
    toolchain::Value *VectorIdx;

    // ExtVector element subset: V.xyx
    toolchain::Constant *VectorElts;
...
public:
  ...
  // simple lvalue
  toolchain::Value *getPointer(CodeGenFunction &CGF) const;
  toolchain::Value *emitResignedPointer(QualType PointeeTy,
                                   CodeGenFunction &CGF) const;
  ...
  toolchain::Value *getRawVectorPointer(CodeGenFunction &CGF) const {
    assert(isVectorElt());
    return Addr.emitRawPointer(CGF);
  }
  toolchain::Value *getVectorPointer() const {
    assert(isVectorElt());
    return Addr.getBasePointer();
  }
  toolchain::Value *getVectorIdx() const {
    assert(isVectorElt());
    return VectorIdx;
  }
  ...
  toolchain::Value *getMatrixPointer() const {
    assert(isMatrixElt());
    return Addr.getBasePointer();
  }
  toolchain::Value *getMatrixIdx() const {
    assert(isMatrixElt());
    return VectorIdx;
  }
  ...
  toolchain::Value *getRawExtVectorPointer(CodeGenFunction &CGF) const {
    assert(isExtVectorElt());
    return Addr.emitRawPointer(CGF);
  }
  toolchain::Constant *getExtVectorElts() const {
    assert(isExtVectorElt());
    return VectorElts;
  }
  ...
  toolchain::Value *getRawBitFieldPointer(CodeGenFunction &CGF) const {
    assert(isBitField());
    return Addr.emitRawPointer(CGF);
  }
  ...
  toolchain::Value *getGlobalReg() const { assert(isGlobalReg()); return V; }
  ...
  static LValue MakeVectorElt(Address vecAddress, toolchain::Value *Idx,
  ...
  static LValue MakeExtVectorElt(Address Addr, toolchain::Constant *Elts,
                                 QualType type, LValueBaseInfo BaseInfo,
                                 TBAAAccessInfo TBAAInfo) {
    LValue R;
  ...
  static LValue MakeGlobalReg(toolchain::Value *V, CharUnits alignment,
                              QualType type) {
    LValue R;
    R.LVType = GlobalReg;
  ...
  static LValue MakeMatrixElt(Address matAddress, toolchain::Value *Idx,
                              QualType type, LValueBaseInfo BaseInfo,
                              TBAAAccessInfo TBAAInfo) {
    LValue R;
    R.LVType = MatrixElt;
...
public:
  ...
  toolchain::Value *getPointer(QualType PointeeTy, CodeGenFunction &CGF) const;
