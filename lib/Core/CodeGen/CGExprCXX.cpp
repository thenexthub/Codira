...
commonEmitCXXMemberOrOperatorCall(CodeGenFunction &CGF, GlobalDecl GD,
                                  toolchain::Value *This, toolchain::Value *ImplicitParam,
...
RValue CodeGenFunction::EmitCXXMemberOrOperatorCall(
    ...
    ReturnValueSlot ReturnValue, toolchain::Value *This, toolchain::Value *ImplicitParam,
...
RValue CodeGenFunction::EmitCXXDestructorCall(
    GlobalDecl Dtor, const CGCallee &Callee, toolchain::Value *This, QualType ThisTy,
    toolchain::Value *ImplicitParam, QualType ImplicitParamTy, const CallExpr *CE,
    toolchain::CallBase **CallOrInvoke) {
  ...
  if (SrcAS != DstAS) {
    ...
    toolchain::Type *NewType = CGM.getTypes().ConvertType(DstTy);
...
RValue CodeGenFunction::EmitCXXMemberOrOperatorMemberCallExpr(
  ...
  toolchain::FunctionType *Ty = CGM.getTypes().GetFunctionType(*FInfo);
...
CodeGenFunction::EmitCXXMemberPointerCallExpr(const CXXMemberCallExpr *E,
  ...
  toolchain::Value *MemFnPtr = EmitScalarExpr(MemFnExpr);

  // Ask the ABI to load the callee.  Note that This is modified.
  toolchain::Value *ThisPtrForCall = nullptr;
  CGCallee Callee =
...
RValue CodeGenFunction::EmitCXXOperatorMemberCallExpr(
    ...
    ReturnValueSlot ReturnValue, toolchain::CallBase **CallOrInvoke) {
  assert(MD->isImplicitObjectMemberFunction() &&
...
static void EmitNullBaseClassInitialization(CodeGenFunction &CGF,
  ...
  toolchain::Constant *NullConstantForBase = CGF.CGM.EmitNullConstantForBase(Base);
  if (!NullConstantForBase->isNullValue()) {
    toolchain::GlobalVariable *NullVariable = new toolchain::GlobalVariable(
        CGF.CGM.getModule(), NullConstantForBase->getType(),
        /*isConstant=*/true, toolchain::GlobalVariable::PrivateLinkage,
    ...
    for (std::pair<CharUnits, CharUnits> Store : Stores) {
      ...
      toolchain::Value *StoreSizeVal = CGF.CGM.getSize(StoreSize);
...
static toolchain::Value *EmitCXXNewAllocSize(CodeGenFunction &CGF,
                                        const CXXNewExpr *e,
                                        unsigned minElements,
                                        toolchain::Value *&numElements,
                                        toolchain::Value *&sizeWithoutCookie) {
  QualType type = e->getAllocatedType();

  if (!e->isArray()) {
    CharUnits typeSize = CGF.getContext().getTypeSizeInChars(type);
    sizeWithoutCookie
      = toolchain::ConstantInt::get(CGF.SizeTy, typeSize.getQuantity());
  ...
  toolchain::APInt cookieSize(sizeWidth,
  ...
  assert(isa<toolchain::IntegerType>(numElements->getType()));
  ...
  toolchain::IntegerType *numElementsType
    = cast<toolchain::IntegerType>(numElements->getType());
  unsigned numElementsWidth = numElementsType->getBitWidth();

  // Compute the constant factor.
  toolchain::APInt arraySizeMultiplier(sizeWidth, 1);
  ...
  CharUnits typeSize = CGF.getContext().getTypeSizeInChars(type);
  toolchain::APInt typeSizeMultiplier(sizeWidth, typeSize.getQuantity());
  ...
  if (toolchain::ConstantInt *numElementsC =
        dyn_cast<toolchain::ConstantInt>(numElements)) {
    const toolchain::APInt &count = numElementsC->getValue();
    ...
    toolchain::APInt adjustedCount = count.zextOrTrunc(sizeWidth);
    ...
    numElements = toolchain::ConstantInt::get(CGF.SizeTy,
                                         adjustedCount * arraySizeMultiplier);

    // Compute the size before cookie, and track whether it overflowed.
    bool overflow;
    toolchain::APInt allocationSize
    ...
    if (cookieSize != 0) {
      ...
      sizeWithoutCookie = toolchain::ConstantInt::get(CGF.SizeTy, allocationSize);
    ...
    if (hasAnyOverflow) {
      size = toolchain::Constant::getAllOnesValue(CGF.SizeTy);
    } else {
      size = toolchain::ConstantInt::get(CGF.SizeTy, allocationSize);
    }
  ...
  // Otherwise, we might need to use the overflow intrinsics.
    ...
    toolchain::Value *hasOverflow = nullptr;

    // If numElementsWidth > sizeWidth, then one way or another, we're
    // going to have to do a comparison for (2), and this happens to
    // take care of (1), too.
    if (numElementsWidth > sizeWidth) {
      toolchain::APInt threshold =
          toolchain::APInt::getOneBitSet(numElementsWidth, sizeWidth);

      toolchain::Value *thresholdV
        = toolchain::ConstantInt::get(numElementsType, threshold);
    ...
    // Otherwise, if we're signed, we want to sext up to size_t.
      ...
      if (typeSizeMultiplier == 1)
        hasOverflow = CGF.Builder.CreateICmpSLT(numElements,
                              toolchain::ConstantInt::get(CGF.SizeTy, minElements));
    ...
    // allocation fails.
    if (typeSizeMultiplier != 1) {
      toolchain::Function *umul_with_overflow
        = CGF.CGM.getIntrinsic(toolchain::Intrinsic::umul_with_overflow, CGF.SizeTy);

      toolchain::Value *tsmV =
        toolchain::ConstantInt::get(CGF.SizeTy, typeSizeMultiplier);
      toolchain::Value *result =
          CGF.Builder.CreateCall(umul_with_overflow, {size, tsmV});

      toolchain::Value *overflowed = CGF.Builder.CreateExtractValue(result, 1);
      if (hasOverflow)
      ...
      if (arraySizeMultiplier != 1) {
        ...
        // Otherwise we need a separate multiply.
          ...
          toolchain::Value *asmV =
            toolchain::ConstantInt::get(CGF.SizeTy, arraySizeMultiplier);
    ...
    if (cookieSize != 0) {
      ...
      toolchain::Function *uadd_with_overflow
        = CGF.CGM.getIntrinsic(toolchain::Intrinsic::uadd_with_overflow, CGF.SizeTy);

      toolchain::Value *cookieSizeV = toolchain::ConstantInt::get(CGF.SizeTy, cookieSize);
      toolchain::Value *result =
    ...
    if (hasOverflow)
      size = CGF.Builder.CreateSelect(hasOverflow,
                                 toolchain::Constant::getAllOnesValue(CGF.SizeTy),
                                      size);
  }
...
void CodeGenFunction::EmitNewArrayInitializer(
    const CXXNewExpr *E, QualType ElementType, toolchain::Type *ElementTy,
    Address BeginPtr, toolchain::Value *NumElements,
  ...
  auto TryMemsetInitialization = [&]() -> bool {
    ...
    if (InitListElements) {
      ...
      auto *InitializedSize = toolchain::ConstantInt::get(
  ...
  if (ILE || CPLIE || SL || OCEE) {
    ...
    if ((ILE && ILE->isStringLiteralInit()) || SL || OCEE) {
      ...
      toolchain::ConstantInt *ConstNum = dyn_cast<toolchain::ConstantInt>(NumElements);
    ...
    if (DtorKind) {
      ...
      toolchain::Instruction *DominatingIP =
          Builder.CreateFlagLoad(toolchain::ConstantInt::getNullValue(Int8PtrTy));
  ...
  if (const CXXConstructExpr *CCE = dyn_cast<CXXConstructExpr>(Init)) {
    ...
    if (InitListElements)
      NumElements = Builder.CreateSub(
          NumElements,
          toolchain::ConstantInt::get(NumElements->getType(), InitListElements));
  ...
  toolchain::BasicBlock *LoopBB = createBasicBlock("new.loop");
  toolchain::BasicBlock *ContBB = createBasicBlock("new.loop.end");

  // Find the end of the array, hoisted out of the loop.
  toolchain::Value *EndPtr = Builder.CreateInBoundsGEP(
  ...
  if (!ConstNum) {
    toolchain::Value *IsEmpty = Builder.CreateICmpEQ(CurPtr.emitRawPointer(*this),
  ...
  toolchain::Value *NextPtr = Builder.CreateConstInBoundsGEP1_32(
      ElementTy, CurPtr.emitRawPointer(*this), 1, "array.next");

  // Check whether we've gotten to the end of the array and, if so,
  // exit the loop.
  toolchain::Value *IsEnd = Builder.CreateICmpEQ(NextPtr, EndPtr, "array.atend");
...
static void EmitNewInitializer(CodeGenFunction &CGF, const CXXNewExpr *E,
                               QualType ElementType, toolchain::Type *ElementTy,
                               Address NewPtr, toolchain::Value *NumElements,
                               toolchain::Value *AllocSizeWithoutCookie) {
...
static RValue EmitNewDeleteCall(CodeGenFunction &CGF,
  ...
  toolchain::CallBase *CallOrInvoke;
  toolchain::Constant *CalleePtr = CGF.CGM.GetAddrOfFunction(CalleeDecl);
  ...
  toolchain::Function *Fn = dyn_cast<toolchain::Function>(CalleePtr);
  if (CalleeDecl->isReplaceableGlobalAllocationFunction() &&
      Fn && Fn->hasFnAttribute(toolchain::Attribute::NoBuiltin)) {
    CallOrInvoke->addFnAttr(toolchain::Attribute::Builtin);
  }
...
RValue CodeGenFunction::EmitBuiltinNewDeleteCall(const FunctionProtoType *Type,
  ...
  toolchain_unreachable("predeclared global operator new/delete is missing");
}
...
namespace {
  ...
  class CallDeleteDuringNew final : public EHScopeStack::Cleanup {
    /// Type used to hold toolchain::Value*s.
  ...
  public:
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      if (isAlignedAllocation(Params.Alignment))
        DeleteArgs.add(RValue::get(toolchain::ConstantInt::get(
...
static void EnterNewDeleteCleanup(CodeGenFunction &CGF, const CXXNewExpr *E,
                                  ...
                                  toolchain::Value *AllocSize, CharUnits AllocAlign,
  ...
  if (!CGF.isInConditionalBranch()) {
    struct DirectCleanupTraits {
      typedef toolchain::Value *ValueTy;
...
toolchain::Value *CodeGenFunction::EmitCXXNewExpr(const CXXNewExpr *E) {
  ...
  toolchain::Value *allocSizeWithoutCookie = nullptr;
  toolchain::Value *allocSize =
  ...
  if (allocator->isReservedGlobalPlacementOperator()) {
    ...
    if (isAlignedAllocation(IAP.PassAlignment)) {
      ...
      allocatorArgs.add(
          RValue::get(toolchain::ConstantInt::get(SizeTy, allocAlign.getQuantity())),
          AlignValT);
    }
    ...
    if (getDebugInfo())
      if (auto *newCall = dyn_cast<toolchain::CallBase>(RV.getScalarVal()))
    ...
    if (!E->passAlignment() &&
      ...
      unsigned AllocatorAlign = toolchain::bit_floor(std::min<uint64_t>(
  ...
  toolchain::BasicBlock *nullCheckBB = nullptr;
  ...
  if (nullCheck) {
    ...
    toolchain::BasicBlock *notNullBB = createBasicBlock("new.notnull");
    contBB = createBasicBlock("new.cont");

    toolchain::Value *isNull = Builder.CreateIsNull(allocation, "new.isnull");
  ...
  toolchain::Instruction *cleanupDominator = nullptr;
  if (E->getOperatorDelete() &&
      !E->getOperatorDelete()->isReservedGlobalPlacementOperator()) {
  ...
  toolchain::Type *elementTy = ConvertTypeForMem(allocType);
  ...
  EmitNewInitializer(*this, E, allocType, elementTy, result, numElements,
                     allocSizeWithoutCookie);
  toolchain::Value *resultPtr = result.emitRawPointer(*this);
  ...
  if (nullCheck) {
    ...
    toolchain::PHINode *PHI = Builder.CreatePHI(resultPtr->getType(), 2);
    PHI->addIncoming(resultPtr, notNullBB);
    PHI->addIncoming(toolchain::Constant::getNullValue(resultPtr->getType()),
                     nullCheckBB);
...
void CodeGenFunction::EmitDeleteCall(const FunctionDecl *DeleteFD,
                                     toolchain::Value *DeletePtr, QualType DeleteTy,
                                     toolchain::Value *NumElements,
  ...
  std::optional<toolchain::AllocaInst *> TagAlloca;
  auto EmitTag = [&](QualType TagType, const char *TagName) {
    assert(!TagAlloca);
    toolchain::Type *Ty = getTypes().ConvertType(TagType);
    CharUnits Align = CGM.getNaturalTypeAlignment(TagType);
    toolchain::AllocaInst *TagAllocation = CreateTempAlloca(Ty, TagName);
  ...
  if (Params.Size) {
    ...
    toolchain::Value *Size = toolchain::ConstantInt::get(ConvertType(SizeType),
    ...
    if (!CookieSize.isZero())
      Size = Builder.CreateAdd(
          Size, toolchain::ConstantInt::get(SizeTy, CookieSize.getQuantity()));
  ...
  if (isAlignedAllocation(Params.Alignment)) {
    ...
    toolchain::Value *Align = toolchain::ConstantInt::get(ConvertType(AlignValType),
...
namespace {
  ...
  struct CallObjectDelete final : EHScopeStack::Cleanup {
    ...
    CallObjectDelete(toolchain::Value *Ptr,
                     const FunctionDecl *OperatorDelete,
                     QualType ElementType)
...
static bool EmitObjectDelete(CodeGenFunction &CGF,
                             ...
                             Address Ptr,
                             QualType ElementType,
                             toolchain::BasicBlock *UnconditionalDeleteBlock) {
  // C++11 [expr.delete]p3:
...
namespace {
  ...
  struct CallArrayDelete final : EHScopeStack::Cleanup {
    ...
    CallArrayDelete(toolchain::Value *Ptr,
                    const FunctionDecl *OperatorDelete,
                    toolchain::Value *NumElements,
                    QualType ElementType,
                    CharUnits CookieSize)
...
static void EmitArrayDelete(CodeGenFunction &CGF,
                            ...
                            QualType elementType) {
  toolchain::Value *numElements = nullptr;
  toolchain::Value *allocatedPtr = nullptr;
  CharUnits cookieSize;
  ...
  if (QualType::DestructionKind dtorKind = elementType.isDestructedType()) {
    ...
    toolchain::Value *arrayBegin = deletedPtr.emitRawPointer(CGF);
    toolchain::Value *arrayEnd = CGF.Builder.CreateInBoundsGEP(
...
void CodeGenFunction::EmitCXXDeleteExpr(const CXXDeleteExpr *E) {
  ...
  toolchain::BasicBlock *DeleteNotNull = createBasicBlock("delete.notnull");
  toolchain::BasicBlock *DeleteEnd = createBasicBlock("delete.end");

  toolchain::Value *IsNull = Builder.CreateIsNull(Ptr, "isnull");
...
static toolchain::Value *EmitTypeidFromVTable(CodeGenFunction &CGF, const Expr *E,
  ...
  if (HasNullCheck &&
    ...
    toolchain::BasicBlock *BadTypeidBlock =
        CGF.createBasicBlock("typeid.bad_typeid");
    toolchain::BasicBlock *EndBlock = CGF.createBasicBlock("typeid.end");

    toolchain::Value *IsNull = CGF.Builder.CreateIsNull(ThisPtr);
...
toolchain::Value *CodeGenFunction::EmitCXXTypeidExpr(const CXXTypeidExpr *E) {
  ...
  // https://reviews.toolchain.org/D157452 has more context, and a possible solution.
  toolchain::Type *PtrTy = Int8PtrTy;
...
static toolchain::Value *EmitDynamicCastToNull(CodeGenFunction &CGF,
                                          QualType DestTy) {
  toolchain::Type *DestLTy = CGF.ConvertType(DestTy);
  if (DestTy->isPointerType())
    return toolchain::Constant::getNullValue(DestLTy);
  ...
  return toolchain::PoisonValue::get(DestLTy);
}

toolchain::Value *CodeGenFunction::EmitDynamicCast(Address ThisAddr,
  ...
  if (DCE->isAlwaysNull()) {
    if (toolchain::Value *T = EmitDynamicCastToNull(*this, DestTy)) {
  ...
  if (IsExact) {
    ExactCastInfo = CGM.getCXXABI().getExactDynamicCastInfo(SrcRecordTy, DestTy,
                                                            DestRecordTy);
    if (!ExactCastInfo) {
      toolchain::Value *NullValue = EmitDynamicCastToNull(*this, DestTy);
      if (!Builder.GetInsertBlock())
        EmitBlock(createBasicBlock("dynamic_cast.unreachable"));
      return NullValue;
    }
  ...
  toolchain::BasicBlock *CastEnd = createBasicBlock("dynamic_cast.end");

  if (ShouldNullCheckSrcValue) {
    CastNull = createBasicBlock("dynamic_cast.null");
    CastNotNull = createBasicBlock("dynamic_cast.notnull");

    toolchain::Value *IsNull = Builder.CreateIsNull(ThisAddr);
  ...
  toolchain::Value *NullValue = nullptr;
  if (ShouldNullCheckSrcValue) {
    EmitBranch(CastEnd);
