...
/// toolchain::Type.  Some of the lowering is straightforward, some is not.  Here we
/// detail some of the complexities and weirdnesses here.
/// * LLVM does not have unions - Unions can, in theory be represented by any
///   toolchain::Type with correct size.  We choose a field via a specific heuristic
...
///   when lowered to toolchain types. For example unsigned x : 24 gets lowered to
...
struct CGRecordLowering {
  ...
  struct MemberInfo {
    ...
    MemberInfo(CharUnits Offset, InfoKind Kind, toolchain::Type *Data,
  ...
  /// Constructs a MemberInfo instance from an offset and toolchain::Type *.
  static MemberInfo StorageInfo(CharUnits Offset, toolchain::Type *Data) {
  ...
  /// Wraps toolchain::Type::getIntNTy with some implicit arguments.
  toolchain::Type *getIntNType(uint64_t NumBits) const {
    unsigned AlignedBits = toolchain::alignTo(NumBits, Context.getCharWidth());
    return toolchain::Type::getIntNTy(Types.getLLVMContext(), AlignedBits);
  }
  /// Get the LLVM type sized as one character unit.
  toolchain::Type *getCharType() const {
    return toolchain::Type::getIntNTy(Types.getLLVMContext(),
                                 Context.getCharWidth());
  }
  /// Gets an toolchain type of size NumChars and alignment 1.
  toolchain::Type *getByteArrayType(CharUnits NumChars) const {
    assert(!NumChars.isZero() && "Empty byte arrays aren't allowed.");
    toolchain::Type *Type = getCharType();
    return NumChars == CharUnits::One() ? Type :
        (toolchain::Type *)toolchain::ArrayType::get(Type, NumChars.getQuantity());
  }
  /// Gets the storage type for a field decl and handles storage
  /// for itanium bitfields that are smaller than their declared type.
  toolchain::Type *getStorageType(const FieldDecl *FD) const {
    toolchain::Type *Type = Types.ConvertTypeForMem(FD->getType());
    if (!FD->isBitField()) return Type;
    if (isDiscreteBitFieldABI()) return Type;
    return getIntNType(std::min(FD->getBitWidthValue(),
                                (unsigned)Context.toBits(getSize(Type))));
  }
  /// Gets the toolchain Basesubobject type from a CXXRecordDecl.
  toolchain::Type *getStorageType(const CXXRecordDecl *RD) const {
  ...
  CharUnits getSize(toolchain::Type *Type) const {
    return CharUnits::fromQuantity(DataLayout.getTypeAllocSize(Type));
  }
  CharUnits getAlignment(toolchain::Type *Type) const {
  ...
  /// Lowers an ASTRecordLayout to a toolchain type.
  ...
  /// Determines if we need a packed toolchain struct.
  ...
  SmallVector<toolchain::Type *, 16> FieldTypes;
  toolchain::DenseMap<const FieldDecl *, unsigned> Fields;
  toolchain::DenseMap<const FieldDecl *, CGBitFieldInfo> BitFields;
  toolchain::DenseMap<const CXXRecordDecl *, unsigned> NonVirtualBases;
  toolchain::DenseMap<const CXXRecordDecl *, unsigned> VirtualBases;
...
void CGRecordLowering::setBitFieldInfo(
    const FieldDecl *FD, CharUnits StartOffset, toolchain::Type *StorageType) {
...
void CGRecordLowering::lower(bool NVBaseType) {
  ...
  // 4) Determine if the toolchain-struct requires packing.  It's important that this
  //    phase occur after clipping, because clipping changes the toolchain type.
...
void CGRecordLowering::lowerUnion(bool isNonVirtualBaseType) {
  ...
  toolchain::Type *StorageType = nullptr;
  ...
  for (const auto *Field : D->fields()) {
    if (Field->isBitField()) {
      if (Field->isZeroLengthBitField())
        continue;
      toolchain::Type *FieldType = getStorageType(Field);
...
CGRecordLowering::accumulateBitFields(bool isNonVirtualBaseType,
  ...
  if (isDiscreteBitFieldABI()) {
    ...
    for (; Field != FieldEnd && Field->isBitField(); ++Field) {
      ...
      toolchain::Type *Type = Types.ConvertTypeForMem(Field->getType());
  ...
  for (;;) {
    ...
    if (AtAlignedBoundary) {
      ...
      if (!InstallBest) {
        ...
        // access unit or not.
        toolchain::Type *Type = getIntNType(Context.toBits(AccessSize));
...
void CGRecordLowering::computeVolatileBitfields() {
  ...
  for (auto &I : BitFields) {
    ...
    toolchain::Type *ResLTy = Types.ConvertTypeForMem(Field->getType());
  ...
  if (Layout.hasOwnVFPtr())
    Members.push_back(
        MemberInfo(CharUnits::Zero(), MemberInfo::VFPtr,
                   toolchain::PointerType::getUnqual(Types.getLLVMContext())));
...
CGBitFieldInfo CGBitFieldInfo::MakeInfo(CodeGenTypes &Types,
  ...
  toolchain::Type *Ty = Types.ConvertTypeForMem(FD->getType());
...
CodeGenTypes::ComputeRecordLayout(const RecordDecl *D, toolchain::StructType *Ty) {
  CGRecordLowering Builder(*this, D, /*Packed=*/false);

  Builder.lower(/*NonVirtualBaseType=*/false);

  // If we're in C++, compute the base subobject type.
  toolchain::StructType *BaseTy = nullptr;
  if (isa<CXXRecordDecl>(D)) {
    ...
    if (Builder.Layout.getNonVirtualSize() != Builder.Layout.getSize()) {
      ...
      BaseTy = toolchain::StructType::create(
  ...
  if (getContext().getLangOpts().DumpRecordLayouts) {
    ...
    toolchain::outs() << "Record: ";
    D->dump(toolchain::outs());
    toolchain::outs() << "\nLayout: ";
    RL->print(toolchain::outs());
  }
  ...
  toolchain::StructType *ST = RL->getLLVMType();
  const toolchain::StructLayout *SL = getDataLayout().getStructLayout(ST);
  ...
  for (unsigned i = 0, e = AST_RL.getFieldCount(); i != e; ++i, ++it) {
    ...
    toolchain::Type *ElementTy = ST->getTypeAtIndex(RL->getLLVMFieldNo(FD));
...
void CGRecordLayout::print(raw_ostream &OS) const {
  ...
  toolchain::array_pod_sort(BFIs.begin(), BFIs.end());
  for (auto &BFI : BFIs) {
    OS.indent(4);
    BFI.second->print(OS);
    OS << "\n";
