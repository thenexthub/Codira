...
#include "toolchain/Frontend/OpenMP/OMPDeviceConstants.h"
#include "toolchain/Frontend/OpenMP/OMPGridValues.h"
...
class NVPTXActionTy final : public PrePostActionTy {
  toolchain::FunctionCallee EnterCallee = nullptr;
  ArrayRef<toolchain::Value *> EnterArgs;
  toolchain::FunctionCallee ExitCallee = nullptr;
  ArrayRef<toolchain::Value *> ExitArgs;
  bool Conditional = false;
  toolchain::BasicBlock *ContBlock = nullptr;

public:
  NVPTXActionTy(toolchain::FunctionCallee EnterCallee,
                ArrayRef<toolchain::Value *> EnterArgs,
...
static RecordDecl *buildRecordForGlobalizedVars(
    ...
    toolchain::SmallDenseMap<const ValueDecl *, const FieldDecl *>
        &MappedDeclsFields,
    int BufSize) {
  ...
  toolchain::SmallPtrSet<const ValueDecl *, 16> SingleEscaped(toolchain::from_range,
  ...
  for (const auto &Pair : GlobalizedVars) {
    ...
    if (SingleEscaped.count(VD)) {
      ...
      if (BufSize > 1) {
        toolchain::APInt ArraySize(32, BufSize);
      ...
      Field = FieldDecl::Create(
          ...
          /*InitStyle=*/ICIS_NoInit);
      Field->setAccess(AS_public);
      toolchain::APInt Align(32, Pair.first.getQuantity());
...
class CheckVarsEscapingDeclContext final
  ...
  CodeGenFunction &CGF;
  toolchain::SetVector<const ValueDecl *> EscapedDecls;
  toolchain::SetVector<const ValueDecl *> EscapedVariableLengthDecls;
  toolchain::SetVector<const ValueDecl *> DelayedVariableLengthDecls;
  toolchain::SmallPtrSet<const Decl *, 4> EscapedParameters;
  RecordDecl *GlobalizedRD = nullptr;
  toolchain::SmallDenseMap<const ValueDecl *, const FieldDecl *> MappedDeclsFields;
        ...
        if (IsCombinedParallelRegion) {
          ...
          for (const OMPClause *C : Clauses) {
            ...
            if (const auto *PC = dyn_cast<OMPFirstprivateClause>(C))
              ...
              toolchain_unreachable("Unexpected clause.");
...
public:
  CheckVarsEscapingDeclContext(CodeGenFunction &CGF,
      ...
      : CGF(CGF), EscapedDecls(toolchain::from_range, TeamsReductions) {}
  ...
  /// value.
  const toolchain::SmallPtrSetImpl<const Decl *> &getEscapedParameters() const {
...
void CGOpenMPRuntimeGPU::emitNonSPMDKernel(const OMPExecutableDirective &D,
                                             ...
                                             toolchain::Constant *&OutlinedFnID,
                                             bool IsOffloadEntry,
...
void CGOpenMPRuntimeGPU::emitKernelInit(const OMPExecutableDirective &D,
  ...
  Attrs.ExecFlags =
      IsSPMD ? toolchain::omp::OMPTgtExecModeFlags::OMP_TGT_EXEC_MODE_SPMD
             : toolchain::omp::OMPTgtExecModeFlags::OMP_TGT_EXEC_MODE_GENERIC;
...
void CGOpenMPRuntimeGPU::emitKernelDeinit(CodeGenFunction &CGF,
  ...
  toolchain::Type *LLVMReductionsBufferTy =
...
CGOpenMPRuntimeGPU::CGOpenMPRuntimeGPU(CodeGenModule &CGM)
  ...
  if (!CGM.getLangOpts().OpenMPIsTargetDevice)
    toolchain_unreachable("OpenMP can only handle device code.");

  if (CGM.getLangOpts().OpenMPCUDAMode)
    CurrentDataSharingMode = CGOpenMPRuntimeGPU::DS_CUDA;

  toolchain::OpenMPIRBuilder &OMPBuilder = getOMPBuilder();
...
toolchain::Function *CGOpenMPRuntimeGPU::emitParallelOutlinedFunction(
  ...
  IsInTTDRegion = false;
  auto *OutlinedFun =
      cast<toolchain::Function>(CGOpenMPRuntime::emitParallelOutlinedFunction(
          CGF, D, ThreadIDVar, InnermostKind, CodeGen));
  IsInTTDRegion = PrevIsInTTDRegion;
  if (getExecutionMode() != CGOpenMPRuntimeGPU::EM_SPMD) {
    toolchain::Function *WrapperFun =
...
getDistributeLastprivateVars(ASTContext &Ctx, const OMPExecutableDirective &D,
                             toolchain::SmallVectorImpl<const ValueDecl *> &Vars) {
...
toolchain::Function *CGOpenMPRuntimeGPU::emitTeamsOutlinedFunction(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    const VarDecl *ThreadIDVar, OpenMPDirectiveKind InnermostKind,
    const RegionCodeGenTy &CodeGen) {
  ...
  toolchain::SmallVector<const ValueDecl *, 4> LastPrivatesReductions;
  toolchain::SmallDenseMap<const ValueDecl *, const FieldDecl *> MappedDeclsFields;
  ...
  CodeGen.setAction(Action);
  toolchain::Function *OutlinedFun = CGOpenMPRuntime::emitTeamsOutlinedFunction(
      CGF, D, ThreadIDVar, InnermostKind, CodeGen);
...
void CGOpenMPRuntimeGPU::emitGenericVarsProlog(CodeGenFunction &CGF,
  ...
  for (auto &Rec : I->getSecond().LocalVarData) {
    ...
    toolchain::Value *AllocArgs[] = {CGF.getTypeSize(VD->getType())};
    toolchain::CallBase *VoidPtr =
        CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
                                CGM.getModule(), OMPRTL___kmpc_alloc_shared),
                            AllocArgs, VD->getName());
    // FIXME: We should use the variables actual alignment as an argument.
    VoidPtr->addRetAttr(toolchain::Attribute::get(
        CGM.getLLVMContext(), toolchain::Attribute::Alignment,
        CGM.getContext().getTargetInfo().getNewAlign() / 8));

    // Cast the void pointer and get the address of the globalized variable.
    toolchain::Value *CastedVoidPtr = Bld.CreatePointerBitCastOrAddrSpaceCast(
...
bool CGOpenMPRuntimeGPU::isDelayedVariableLengthDecl(CodeGenFunction &CGF,
  ...
  return toolchain::is_contained(I->getSecond().DelayedVariableLengthDecls, VD);
}
...
CGOpenMPRuntimeGPU::getKmpcAllocShared(CodeGenFunction &CGF,
  ...
  toolchain::Value *Size = CGF.getTypeSize(VD->getType());
  CharUnits Align = CGM.getContext().getDeclAlign(VD);
  Size = Bld.CreateNUWAdd(
      Size, toolchain::ConstantInt::get(CGF.SizeTy, Align.getQuantity() - 1));
  toolchain::Value *AlignVal =
      toolchain::ConstantInt::get(CGF.SizeTy, Align.getQuantity());
  Size = Bld.CreateUDiv(Size, AlignVal);
  Size = Bld.CreateNUWMul(Size, AlignVal);

  // Allocate space for this VLA object to be globalized.
  toolchain::Value *AllocArgs[] = {Size};
  toolchain::CallBase *VoidPtr =
      CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
                              CGM.getModule(), OMPRTL___kmpc_alloc_shared),
                          AllocArgs, VD->getName());
  VoidPtr->addRetAttr(toolchain::Attribute::get(
      CGM.getLLVMContext(), toolchain::Attribute::Alignment, Align.getQuantity()));
...
void CGOpenMPRuntimeGPU::getKmpcFreeShared(
    CodeGenFunction &CGF,
    const std::pair<toolchain::Value *, toolchain::Value *> &AddrSizePair) {
...
void CGOpenMPRuntimeGPU::emitGenericVarsEpilog(CodeGenFunction &CGF) {
  ...
  if (I != FunctionGlobalizedDecls.end()) {
    ...
    for (const auto &AddrSizePair :
         toolchain::reverse(I->getSecond().EscapedVariableLengthDeclsAddrs)) {
      CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
                              CGM.getModule(), OMPRTL___kmpc_free_shared),
                          {AddrSizePair.first, AddrSizePair.second});
    }
    // Deallocate the memory for each globalized value
    for (auto &Rec : toolchain::reverse(I->getSecond().LocalVarData)) {
      const auto *VD = cast<VarDecl>(Rec.first);
      I->getSecond().MappedParams->restore(CGF);

      toolchain::Value *FreeArgs[] = {Rec.second.GlobalizedVal,
...
void CGOpenMPRuntimeGPU::emitTeamsCall(CodeGenFunction &CGF,
  ...
  if (IsBareKernel)
    OutlinedFnArgs.push_back(toolchain::ConstantPointerNull::get(CGM.VoidPtrTy));
  else
...
void CGOpenMPRuntimeGPU::emitParallelCall(CodeGenFunction &CGF,
  ...
  auto &&ParallelGen = [this, Loc, OutlinedFn, CapturedVars, IfCond,
    ...
    CGBuilderTy &Bld = CGF.Builder;
    toolchain::Value *NumThreadsVal = NumThreads;
    toolchain::Function *WFn = WrapperFunctionsMap[OutlinedFn];
    toolchain::Value *ID = toolchain::ConstantPointerNull::get(CGM.Int8PtrTy);
    if (WFn)
      ID = Bld.CreateBitOrPointerCast(WFn, CGM.Int8PtrTy);
    toolchain::Value *FnPtr = Bld.CreateBitOrPointerCast(OutlinedFn, CGM.Int8PtrTy);
    ...
    Address CapturedVarsAddrs = CGF.CreateDefaultAlignTempAlloca(
        toolchain::ArrayType::get(CGM.VoidPtrTy, CapturedVars.size()),
        "captured_vars_addrs");
    ...
    if (IfCond)
      ...
      IfCondVal = toolchain::ConstantInt::get(CGF.Int32Ty, 1);

    if (!NumThreadsVal)
      NumThreadsVal = toolchain::ConstantInt::get(CGF.Int32Ty, -1);
    else
    ...
    toolchain::Value *Args[] = {
        ...
        IfCondVal,
        NumThreadsVal,
        toolchain::ConstantInt::get(CGF.Int32Ty, -1),
        FnPtr,
        ID,
        Bld.CreateBitOrPointerCast(CapturedVarsAddrs.emitRawPointer(CGF),
                                   CGF.VoidPtrPtrTy),
        toolchain::ConstantInt::get(CGM.SizeTy, CapturedVars.size())};
    CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntimeGPU::syncCTAThreads(CodeGenFunction &CGF) {
  ...
  toolchain::Value *Args[] = {
      toolchain::ConstantPointerNull::get(
          cast<toolchain::PointerType>(getIdentTyPointerTy())),
      toolchain::ConstantInt::get(CGF.Int32Ty, /*V=*/0, /*isSigned=*/true)};
  CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntimeGPU::emitCriticalRegion(
    ...
    const Expr *Hint) {
  toolchain::BasicBlock *LoopBB = CGF.createBasicBlock("omp.critical.loop");
  toolchain::BasicBlock *TestBB = CGF.createBasicBlock("omp.critical.test");
  toolchain::BasicBlock *SyncBB = CGF.createBasicBlock("omp.critical.sync");
  toolchain::BasicBlock *BodyBB = CGF.createBasicBlock("omp.critical.body");
  toolchain::BasicBlock *ExitBB = CGF.createBasicBlock("omp.critical.exit");

  auto &RT = static_cast<CGOpenMPRuntimeGPU &>(CGF.CGM.getOpenMPRuntime());

  // Get the mask of active threads in the warp.
  toolchain::Value *Mask = CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
      CGM.getModule(), OMPRTL___kmpc_warp_active_thread_mask));
  // Fetch team-local id of the thread.
  toolchain::Value *ThreadID = RT.getGPUThreadID(CGF);

  // Get the width of the team.
  toolchain::Value *TeamWidth = RT.getGPUNumThreads(CGF);
  ...
  CGF.EmitStoreOfScalar(toolchain::Constant::getNullValue(CGM.Int32Ty), CounterLVal,
                        /*isInit=*/true);

  // Block checks if loop counter exceeds upper bound.
  CGF.EmitBlock(LoopBB);
  toolchain::Value *CounterVal = CGF.EmitLoadOfScalar(CounterLVal, Loc);
  toolchain::Value *CmpLoopBound = CGF.Builder.CreateICmpSLT(CounterVal, TeamWidth);
  ...
  toolchain::Value *CmpThreadToCounter =
...
static toolchain::Value *castValueToType(CodeGenFunction &CGF, toolchain::Value *Val,
  ...
  assert(!CGF.getContext().getTypeSizeInChars(ValTy).isZero() &&
         "Val type must sized.");
  toolchain::Type *LLVMCastTy = CGF.ConvertTypeForMem(CastTy);
  if (ValTy == CastTy)
    return Val;
...
void CGOpenMPRuntimeGPU::emitReduction(
  ...
  toolchain::SmallDenseMap<const ValueDecl *, const FieldDecl *> VarFieldMap;
  toolchain::SmallVector<const ValueDecl *, 4> PrivatesReductions(Privates.size());
  int Cnt = 0;
  ...
  toolchain::OpenMPIRBuilder::LocationDescription OmpLoc(
      CodeGenIP, CGF.SourceLocToDebugLoc(Loc));
  toolchain::SmallVector<toolchain::OpenMPIRBuilder::ReductionInfo> ReductionInfos;
  ...
  for (const Expr *Private : Privates) {
    ...
    toolchain::Value *PrivateVariable;
    toolchain::OpenMPIRBuilder::ReductionGenAtomicCBTy AtomicReductionGen = nullptr;
    ...
    toolchain::OpenMPIRBuilder::EvalKind EvalKind;
    switch (CGF.getEvaluationKind(Private->getType())) {
    case TEK_Scalar:
      EvalKind = toolchain::OpenMPIRBuilder::EvalKind::Scalar;
      break;
    case TEK_Complex:
      EvalKind = toolchain::OpenMPIRBuilder::EvalKind::Complex;
      break;
    case TEK_Aggregate:
      EvalKind = toolchain::OpenMPIRBuilder::EvalKind::Aggregate;
      break;
    }
    auto ReductionGen = [&](InsertPointTy CodeGenIP, unsigned I,
                            toolchain::Value **LHSPtr, toolchain::Value **RHSPtr,
                            toolchain::Function *NewFunc) {
    ...
    ReductionInfos.emplace_back(toolchain::OpenMPIRBuilder::ReductionInfo(
  ...
  toolchain::OpenMPIRBuilder::InsertPointTy AfterIP =
      cantFail(OMPBuilder.createReductionsGPU(
          ...
          toolchain::OpenMPIRBuilder::ReductionGenCBKind::Clang,
...
CGOpenMPRuntimeGPU::getParameterAddress(CodeGenFunction &CGF,
  ...
  toolchain::Value *TargetAddr = CGF.EmitLoadOfScalar(LocalAddr, /*Volatile=*/false,
                                                 TargetTy, SourceLocation());
  // Cast to native address space.
  TargetAddr = CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
      TargetAddr,
      toolchain::PointerType::get(CGF.getLLVMContext(), NativePointeeAddrSpace));
...
void CGOpenMPRuntimeGPU::emitOutlinedFunctionCall(
  ...
  SmallVector<toolchain::Value *, 4> TargetArgs;
  ...
  for (unsigned I = 0, E = Args.size(); I < E; ++I) {
    ...
    toolchain::Type *TargetType = FnType->getParamType(I);
    toolchain::Value *NativeArg = Args[I];
...
toolchain::Function *CGOpenMPRuntimeGPU::createParallelDataSharingWrapper(
    toolchain::Function *OutlinedParallelFn, const OMPExecutableDirective &D) {
  ...
  auto *Fn = toolchain::Function::Create(
      CGM.getTypes().GetFunctionType(CGFI), toolchain::GlobalValue::InternalLinkage,
  ...
  Fn->setLinkage(toolchain::GlobalValue::InternalLinkage);
  ...
  toolchain::Value *GlobalArgsPtr = GlobalArgs.getPointer();
  toolchain::Value *DataSharingArgs[] = {GlobalArgsPtr};
  CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
  ...
  if (isOpenMPLoopBoundSharingDirective(D.getDirectiveKind())) {
    ...
    toolchain::Value *LB = CGF.EmitLoadOfScalar(
        TypedAddress,
        /*Volatile=*/false,
    ...
    TypedAddress = Bld.CreatePointerBitCastOrAddrSpaceCast(Src, Bld.getPtrTy(0),
                                                           CGF.SizeTy);
    toolchain::Value *UB = CGF.EmitLoadOfScalar(
        TypedAddress,
        /*Volatile=*/false,
        CGF.getContext().getPointerType(CGF.getContext().getSizeType()),
  ...
  if (CS.capture_size() > 0) {
    ...
    for (unsigned I = 0, E = CS.capture_size(); I < E; ++I, ++CI, ++CurField) {
      ...
      toolchain::Value *Arg = CGF.EmitLoadOfScalar(TypedAddress,
                                              /*Volatile=*/false,
...
Address CGOpenMPRuntimeGPU::getAddressOfLocalVariable(CodeGenFunction &CGF,
  ...
  if (VD && VD->hasAttr<OMPAllocateDeclAttr>()) {
    ...
    toolchain::Type *VarTy = CGF.ConvertTypeForMem(VD->getType());
    auto *GV = new toolchain::GlobalVariable(
        CGM.getModule(), VarTy, /*isConstant=*/false,
        toolchain::GlobalValue::InternalLinkage, toolchain::PoisonValue::get(VarTy),
        VD->getName(),
        /*InsertBefore=*/nullptr, toolchain::GlobalValue::NotThreadLocal,
...
void CGOpenMPRuntimeGPU::adjustTargetSpecificDataForLambdas(
  ...
  for (const CapturedStmt::Capture &C : CS->captures()) {
    ...
    if (ThisCapture && CGF.CapturedStmtInfo->isCXXThisExprCaptured()) {
      ...
      toolchain::Value *CXXThis = CGF.LoadCXXThis();
  ...
  for (const OMPClause *Clause : D->clauselists()) {
    if (Clause->getClauseKind() == OMPC_unified_shared_memory) {
      ...
      case OffloadArch::LAST:
        toolchain_unreachable("Unexpected GPU arch.");
      }
    }
  }
...
toolchain::Value *CGOpenMPRuntimeGPU::getGPUNumThreads(CodeGenFunction &CGF) {
  CGBuilderTy &Bld = CGF.Builder;
  toolchain::Module *M = &CGF.CGM.getModule();
  const char *LocSize = "__kmpc_get_hardware_num_threads_in_block";
  toolchain::Function *F = M->getFunction(LocSize);
  if (!F) {
    F = toolchain::Function::Create(toolchain::FunctionType::get(CGF.Int32Ty, {}, false),
                               toolchain::GlobalVariable::ExternalLinkage, LocSize,
                               &CGF.CGM.getModule());
  }
  return Bld.CreateCall(F, {}, "nvptx_num_threads");
}

toolchain::Value *CGOpenMPRuntimeGPU::getGPUThreadID(CodeGenFunction &CGF) {
