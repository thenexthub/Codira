...
class PCHContainerGenerator : public ASTConsumer {
  ...
  std::unique_ptr<toolchain::LLVMContext> VMContext;
  ...
  struct DebugTypeVisitor : public RecursiveASTVisitor<DebugTypeVisitor> {
    language::Core::CodeGen::CGDebugInfo &DI;
    ASTContext &Ctx;
    DebugTypeVisitor(language::Core::CodeGen::CGDebugInfo &DI, ASTContext &Ctx)
        : DI(DI), Ctx(Ctx) {}
...
public:
  PCHContainerGenerator(CompilerInstance &CI, const std::string &MainFileName,
    ...
    CodeGenOpts.setDebugInfo(toolchain::codegenoptions::FullDebugInfo);
  ...
  void Initialize(ASTContext &Context) override {
    ...
    Ctx = &Context;
    VMContext.reset(new toolchain::LLVMContext());
    M.reset(new toolchain::Module(MainFileName, *VMContext));
    ...
    StringRef ModuleName = toolchain::sys::path::filename(MainFileName);
    DI->setPCHDescriptor(
  ...
  void HandleTranslationUnit(ASTContext &Ctx) override {
    ...
    std::unique_ptr<toolchain::LLVMContext> VMContext = std::move(this->VMContext);
    std::unique_ptr<toolchain::Module> M = std::move(this->M);
    ...
    if (!toolchain::TargetRegistry::lookupTarget(Triple, Error))
    ...
    if (Triple.isOSBinFormatWasm()) {
      ...
      // represented as their own sections (in the toolchain sense).
      // TODO: https://github.com/WebAssembly/tool-conventions/issues/138
      toolchain::NamedMDNode *MD =
          M->getOrInsertNamedMetadata("wasm.custom_sections");
      toolchain::Metadata *Ops[2] = {
          toolchain::MDString::get(*VMContext, "__clangast"),
          toolchain::MDString::get(*VMContext,
                              StringRef(SerializedAST.data(), Size))};
      auto *NameAndContent = toolchain::MDTuple::get(*VMContext, Ops);
      MD->addOperand(NameAndContent);
    } else {
      auto Int8Ty = toolchain::Type::getInt8Ty(*VMContext);
      auto *Ty = toolchain::ArrayType::get(Int8Ty, Size);
      auto *Data = toolchain::ConstantDataArray::getString(
          *VMContext, StringRef(SerializedAST.data(), Size),
          /*AddNull=*/false);
      auto *ASTSym = new toolchain::GlobalVariable(
          *M, Ty, /*constant*/ true, toolchain::GlobalVariable::InternalLinkage,
          Data, "__clang_ast");
      // The on-disk hashtable needs to be aligned.
      ASTSym->setAlignment(toolchain::Align(8));
    ...
    LLVM_DEBUG({
      ...
      language::Core::emitBackendOutput(
          CI, CodeGenOpts, Ctx.getTargetInfo().getDataLayoutString(), M.get(),
          BackendAction::Backend_EmitLL, FS,
          std::make_unique<toolchain::raw_svector_ostream>(Buffer));
      toolchain::dbgs() << Buffer;
    });

    // Use the LLVM backend to emit the pch container.
    language::Core::emitBackendOutput(CI, CodeGenOpts,
