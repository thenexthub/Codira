...
toolchain::LLVMContext &ABIInfo::getVMContext() const {
  return CGT.getLLVMContext();
}

const toolchain::DataLayout &ABIInfo::getDataLayout() const {
...
void ABIInfo::appendAttributeMangling(StringRef AttrStr,
  ...
  toolchain::sort(Info.Features, [&TI](StringRef LHS, StringRef RHS) {
...
ABIInfo::getOptimalVectorMemoryType(toolchain::FixedVectorType *T,
                                    const LangOptions &Opt) const {
  if (T->getNumElements() == 3 && !Opt.PreserveVec3Type)
    return toolchain::FixedVectorType::get(T->getElementType(), 4);
  return T;
}
...
/// return registers.
bool SwiftABIInfo::occupiesMoreThan(ArrayRef<toolchain::Type *> scalarTypes,
                                    unsigned maxAllRegisters) const {
  unsigned intCount = 0, fpCount = 0;
  for (toolchain::Type *type : scalarTypes) {
    if (type->isPointerTy()) {
      intCount++;
    } else if (auto intTy = dyn_cast<toolchain::IntegerType>(type)) {
...
bool SwiftABIInfo::shouldPassIndirectly(ArrayRef<toolchain::Type *> ComponentTys,
                                        bool AsReturnValue) const {
  return occupiesMoreThan(ComponentTys, /*total=*/4);
}

bool SwiftABIInfo::isLegalVectorType(CharUnits VectorSize, toolchain::Type *EltTy,
