...
static Value *handleAsDoubleBuiltin(CodeGenFunction &CGF, const CallExpr *E) {
  ...
  toolchain::Type *ResultType = CGF.DoubleTy;
  int N = 1;
  if (auto *VTy = E->getArg(0)->getType()->getAs<language::Core::VectorType>()) {
    N = VTy->getNumElements();
    ResultType = toolchain::FixedVectorType::get(CGF.DoubleTy, N);
  }
  ...
  toolchain::SmallVector<int> Mask;
  for (int i = 0; i < N; i++) {
    Mask.push_back(i);
    Mask.push_back(i + N);
  }
...
static Value *handleHlslClip(const CallExpr *E, CodeGenFunction *CGF) {
  ...
  if (const auto *VecTy = E->getArg(0)->getType()->getAs<language::Core::VectorType>()) {
  ...
  if (CGF->CGM.getTarget().getTriple().isDXIL()) {
    ...
    toolchain_unreachable("Backend Codegen not supported.");
  }
...
static Value *handleHlslSplitdouble(const CallExpr *E, CodeGenFunction *CGF) {
  ...
  if (CGF->CGM.getTarget().getTriple().isDXIL()) {
    toolchain::Type *RetElementTy = CGF->Int32Ty;
    if (auto *Op0VecTy = E->getArg(0)->getType()->getAs<language::Core::VectorType>())
      RetElementTy = toolchain::VectorType::get(
          CGF->Int32Ty, ElementCount::getFixed(Op0VecTy->getNumElements()));
    auto *RetTy = toolchain::StructType::get(RetElementTy, RetElementTy);
    ...
    if (!Op0->getType()->isVectorTy()) {
      ...
      int NumElements = 1;
      if (const auto *VecTy =
              E->getArg(0)->getType()->getAs<language::Core::VectorType>())
...
static Intrinsic::ID getWaveActiveSumIntrinsic(toolchain::Triple::ArchType Arch,
  ...
  default:
    toolchain_unreachable("Intrinsic WaveActiveSum"
                     " not supported by target architecture");
  }
}

// Return wave active sum that corresponds to the QT scalar type
static Intrinsic::ID getWaveActiveMaxIntrinsic(toolchain::Triple::ArchType Arch,
                                               CGHLSLRuntime &RT, QualType QT) {
  switch (Arch) {
  case toolchain::Triple::spirv:
    if (QT->isUnsignedIntegerType())
  ...
  default:
    toolchain_unreachable("Intrinsic WaveActiveMax"
                     " not supported by target architecture");
  }
}
...
static std::string getSpecConstantFunctionName(language::Core::QualType SpecConstantType,
                                               ASTContext &Context) {
  ...
  std::string Name;
  toolchain::raw_string_ostream MangledNameStream(Name);
...
Value *CodeGenFunction::EmitHLSLBuiltinExpr(unsigned BuiltinID,
  ...
  case Builtin::BI__builtin_hlsl_adduint64: {
    ...
    toolchain::Value *LowA;
    toolchain::Value *HighA;
    toolchain::Value *LowB;
    toolchain::Value *HighB;
    ...
    toolchain::Value *LowSum = EmitOverflowIntrinsic(
        *this, Intrinsic::uadd_with_overflow, LowA, LowB, Carry);
    toolchain::Value *ZExtCarry =
        Builder.CreateZExt(Carry, HighA->getType(), "CarryZExt");

    // Sum the high words and the carry
    toolchain::Value *HighSum = Builder.CreateAdd(HighA, HighB, "HighSum");
    toolchain::Value *HighSumPlusCarry =
    ...
    toolchain::Value *Result = PoisonValue::get(OpA->getType());
  ...
  case Builtin::BI__builtin_hlsl_resource_uninitializedhandle: {
    toolchain::Type *HandleTy = CGM.getTypes().ConvertType(E->getType());
    return toolchain::PoisonValue::get(HandleTy);
  }
  case Builtin::BI__builtin_hlsl_resource_handlefrombinding: {
    ...
    // (toolchain/toolchain-project#135452)
    Value *NonUniform =
        toolchain::ConstantInt::get(toolchain::Type::getInt1Ty(getLLVMContext()), false);
  ...
  case Builtin::BI__builtin_hlsl_all: {
    ...
    return Builder.CreateIntrinsic(
        /*ReturnType=*/toolchain::Type::getInt1Ty(getLLVMContext()),
  ...
  case Builtin::BI__builtin_hlsl_dot: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    Value *Op1 = EmitScalarExpr(E->getArg(1));
    toolchain::Type *T0 = Op0->getType();
    toolchain::Type *T1 = Op1->getType();
  ...
  case Builtin::BI__builtin_hlsl_lerp: {
    ...
    if (!E->getArg(0)->getType()->hasFloatingRepresentation())
      toolchain_unreachable("lerp operand must have a float representation");
    return Builder.CreateIntrinsic(
  ...
  case Builtin::BI__builtin_hlsl_elementwise_frac: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    if (!E->getArg(0)->getType()->hasFloatingRepresentation())
      toolchain_unreachable("frac operand must have a float representation");
    return Builder.CreateIntrinsic(
  ...
  case Builtin::BI__builtin_hlsl_elementwise_isinf: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    toolchain::Type *Xty = Op0->getType();
    toolchain::Type *retType = toolchain::Type::getInt1Ty(this->getLLVMContext());
    if (Xty->isVectorTy()) {
      auto *XVecTy = E->getArg(0)->getType()->castAs<VectorType>();
      retType = toolchain::VectorType::get(
          retType, ElementCount::getFixed(XVecTy->getNumElements()));
    }
    if (!E->getArg(0)->getType()->hasFloatingRepresentation())
      toolchain_unreachable("isinf operand must have a float representation");
  ...
  case Builtin::BI__builtin_hlsl_mad: {
    ...
    if (E->getArg(0)->getType()->hasSignedIntegerRepresentation()) {
      if (CGM.getTarget().getTriple().getArch() == toolchain::Triple::dxil)
        return Builder.CreateIntrinsic(
  ...
  case Builtin::BI__builtin_hlsl_elementwise_rcp: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    if (!E->getArg(0)->getType()->hasFloatingRepresentation())
      toolchain_unreachable("rcp operand must have a float representation");
    toolchain::Type *Ty = Op0->getType();
    toolchain::Type *EltTy = Ty->getScalarType();
  ...
  case Builtin::BI__builtin_hlsl_elementwise_rsqrt: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    if (!E->getArg(0)->getType()->hasFloatingRepresentation())
      toolchain_unreachable("rsqrt operand must have a float representation");
    return Builder.CreateIntrinsic(
  ...
  case Builtin::BI__builtin_hlsl_wave_get_lane_index: {
    ...
    case toolchain::Triple::spirv:
      return EmitRuntimeCall(CGM.CreateRuntimeFunction(
          toolchain::FunctionType::get(IntTy, {}, false),
  ...
  case Builtin::BI__builtin_hlsl_elementwise_sign: {
    ...
    toolchain::Type *Xty = Op0->getType();
    toolchain::Type *retType = toolchain::Type::getInt32Ty(this->getLLVMContext());
    if (Xty->isVectorTy()) {
  ...
  case Builtin::BI__builtin_get_spirv_spec_constant_double: {
    toolchain::Function *SpecConstantFn = getSpecConstantFunction(E->getType());
    toolchain::Value *SpecId = EmitScalarExpr(E->getArg(0));
    toolchain::Value *DefaultVal = EmitScalarExpr(E->getArg(1));
    toolchain::Value *Args[] = {SpecId, DefaultVal};
...
toolchain::Function *language::Core::CodeGen::CodeGenFunction::getSpecConstantFunction(
    const language::Core::QualType &SpecConstantType) {
  ...
  toolchain::Function *SpecConstantFn = M->getFunction(MangledName);

  if (!SpecConstantFn) {
    toolchain::Type *IntType = ConvertType(getContext().IntTy);
    toolchain::Type *RetTy = ConvertType(SpecConstantType);
    toolchain::Type *ArgTypes[] = {IntType, RetTy};
    toolchain::FunctionType *FnTy = toolchain::FunctionType::get(RetTy, ArgTypes, false);
    SpecConstantFn = toolchain::Function::Create(
        FnTy, toolchain::GlobalValue::ExternalLinkage, MangledName, M);
  }
