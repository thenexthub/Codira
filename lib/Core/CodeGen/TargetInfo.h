...
#ifndef LANGUAGE_CORE_LIB_CODEGEN_TARGETINFO_H
#define LANGUAGE_CORE_LIB_CODEGEN_TARGETINFO_H
...
public:
  ...
  virtual void setTargetAttributes(const Decl *D, toolchain::GlobalValue *GV,
  ...
  virtual void emitTargetMetadata(
      ...
      const toolchain::MapVector<GlobalDecl, StringRef> &MangledDeclNames) const {}
  ...
  /// https://discourse.toolchain.org/t/rfc-avoid-inlining-alwaysinline-functions-when-they-cannot-be-inlined/79528
  virtual bool
  ...
  virtual toolchain::Value *decodeReturnAddress(CodeGen::CodeGenFunction &CGF,
                                           toolchain::Value *Address) const {
    return Address;
  }
  ...
  virtual toolchain::Value *encodeReturnAddress(CodeGen::CodeGenFunction &CGF,
                                           toolchain::Value *Address) const {
    return Address;
  }
  ...
  testFPKind(toolchain::Value *V, unsigned BuiltinID, CGBuilderTy &Builder,
  ...
  virtual toolchain::Type *adjustInlineAsmType(CodeGen::CodeGenFunction &CGF,
                                          StringRef Constraint,
                                          toolchain::Type *Ty) const {
    return Ty;
  }
  ...
  virtual void addReturnRegisterOutputs(
      ...
      std::string &Constraints, std::vector<toolchain::Type *> &ResultRegTypes,
  ...
  getUBSanFunctionSignature(CodeGen::CodeGenModule &CGM) const {
    return toolchain::ConstantInt::get(CGM.Int32Ty, 0xc105cafe);
  }
  ...
  /// platform.
  virtual void getDependentLibraryOption(toolchain::StringRef Lib,
                                         toolchain::SmallString<24> &Opt) const;

  /// Gets the linker options necessary to detect object file mismatches on
  /// this platform.
  virtual void getDetectMismatchOption(toolchain::StringRef Name,
                                       toolchain::StringRef Value,
                                       toolchain::SmallString<32> &Opt) const {}
  ...
  virtual toolchain::Constant *getNullPointer(const CodeGen::CodeGenModule &CGM,
  ...
  Address performAddrSpaceCast(CodeGen::CodeGenFunction &CGF, Address Addr,
                               LangAS SrcAddr, toolchain::Type *DestTy,
  ...
  virtual toolchain::Value *performAddrSpaceCast(CodeGen::CodeGenFunction &CGF,
                                            toolchain::Value *V, LangAS SrcAddr,
                                            toolchain::Type *DestTy,
                                            bool IsNonNull = false) const;
  ...
  /// \param DestAddr is the targeted language address space.
  /// \param DestTy is the destination LLVM pointer type.
  virtual toolchain::Constant *performAddrSpaceCast(CodeGenModule &CGM,
                                               toolchain::Constant *V,
                                               LangAS SrcAddr,
  ...
  virtual toolchain::SyncScope::ID getLLVMSyncScopeID(const LangOptions &LangOpts,
                                                 SyncScope Scope,
  ...
  public:
    typedef std::pair<toolchain::Value *, StringRef> ValueTy;
    TargetOpenCLBlockHelper() {}
    virtual ~TargetOpenCLBlockHelper() {}
    /// Get the custom field types for OpenCL blocks.
    virtual toolchain::SmallVector<toolchain::Type *, 1> getCustomFieldTypes() = 0;
    /// Get the custom field values for OpenCL blocks.
    virtual toolchain::SmallVector<ValueTy, 1>
    ...
    /// constants.
    virtual toolchain::SmallVector<toolchain::Constant *, 1>
  ...
  virtual toolchain::Type *getCUDADeviceBuiltinSurfaceDeviceType() const {
    // By default, no change from the original one.
    return nullptr;
  }
  /// Return the device-side type for the CUDA device builtin texture type.
  virtual toolchain::Type *getCUDADeviceBuiltinTextureDeviceType() const {
    // By default, no change from the original one.
    return nullptr;
  }

  /// Return the WebAssembly externref reference type.
  virtual toolchain::Type *getWasmExternrefReferenceType() const { return nullptr; }

  /// Return the WebAssembly funcref reference type.
  virtual toolchain::Type *getWasmFuncrefReferenceType() const { return nullptr; }
  ...
  virtual toolchain::Type *getOpenCLType(CodeGenModule &CGM, const Type *T) const {
    return nullptr;
  }
...
protected:
  ...
  void addStackProbeTargetAttributes(const Decl *D, toolchain::GlobalValue *GV,
...
#endif // LANGUAGE_CORE_LIB_CODEGEN_TARGETINFO_H
