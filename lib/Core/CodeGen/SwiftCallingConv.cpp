...
static toolchain::Type *getCommonType(toolchain::Type *first, toolchain::Type *second) {
  assert(first != second);
  ...
  } else if (auto firstVecTy = dyn_cast<toolchain::VectorType>(first)) {
    if (auto secondVecTy = dyn_cast<toolchain::VectorType>(second)) {
...
static CharUnits getTypeStoreSize(CodeGenModule &CGM, toolchain::Type *type) {
  return CharUnits::fromQuantity(CGM.getDataLayout().getTypeStoreSize(type));
}

static CharUnits getTypeAllocSize(CodeGenModule &CGM, toolchain::Type *type) {
...
void SwiftAggLowering::addTypedData(QualType type, CharUnits begin) {
  ...
  // Member pointer types.
    ...
    auto *toolchainType = CGM.getTypes().ConvertType(type);
    addTypedData(toolchainType, begin);
  }
}
...
void SwiftAggLowering::addTypedData(toolchain::Type *type, CharUnits begin) {
  assert(type && "didn't provide type for typed data");
  addTypedData(type, begin, begin + getTypeStoreSize(CGM, type));
}

void SwiftAggLowering::addTypedData(toolchain::Type *type,
                                    CharUnits begin, CharUnits end) {
  assert(type && "didn't provide type for typed data");
  assert(getTypeStoreSize(CGM, type) == end - begin);

  // Legalize vector types.
  if (auto vecTy = dyn_cast<toolchain::VectorType>(type)) {
    SmallVector<toolchain::Type*, 4> componentTys;
    ...
    for (size_t i = 0, e = componentTys.size(); i != e - 1; ++i) {
      toolchain::Type *componentTy = componentTys[i];
  ...
  if (auto intTy = dyn_cast<toolchain::IntegerType>(type)) {
...
void SwiftAggLowering::addLegalTypedData(toolchain::Type *type,
                                         CharUnits begin, CharUnits end) {
...
void SwiftAggLowering::addEntry(toolchain::Type *type,
                                CharUnits begin, CharUnits end) {
  assert((!type ||
          (!isa<toolchain::StructType>(type) && !isa<toolchain::ArrayType>(type))) &&
...
restartAfterSplit:
  ...
  if (auto vecTy = dyn_cast_or_null<toolchain::VectorType>(type)) {
    auto eltTy = vecTy->getElementType();
    CharUnits eltSize =
        (end - begin) / cast<toolchain::FixedVectorType>(vecTy)->getNumElements();
    assert(eltSize == getTypeStoreSize(CGM, eltTy));
    for (unsigned i = 0,
                  e = cast<toolchain::FixedVectorType>(vecTy)->getNumElements();
         i != e; ++i) {
...
void SwiftAggLowering::splitVectorEntry(unsigned index) {
  auto vecTy = cast<toolchain::VectorType>(Entries[index].Type);
...
static bool isMergeableEntryType(toolchain::Type *type) {
...
void SwiftAggLowering::finish() {
  ...
  for (size_t i = 0, e = orig.size(); i != e; ++i) {
    ...
    do {
      ...
      auto entryTy =
        toolchain::IntegerType::get(CGM.getLLVMContext(),
...
std::pair<toolchain::StructType*, toolchain::Type*>
SwiftAggLowering::getCoerceAndExpandTypes() const {
  ...
  toolchain::Type *unpaddedType = coercionType;
  if (hasPadding) {
    elts.clear();
    for (auto &entry : Entries) {
      elts.push_back(entry.Type);
    ...
    if (elts.size() == 1) {
      ...
      unpaddedType = toolchain::StructType::get(ctx, elts, /*packed*/ false);
    }
...
bool SwiftAggLowering::shouldPassIndirectly(bool asReturnValue) const {
  ...
  SmallVector<toolchain::Type*, 8> componentTys;
...
bool swiftcall::shouldPassIndirectly(CodeGenModule &CGM,
                                     ArrayRef<toolchain::Type*> componentTys,
                                     bool asReturnValue) {
  return getSwiftABIInfo(CGM).shouldPassIndirectly(componentTys, asReturnValue);
}
...
CharUnits swiftcall::getNaturalAlignment(CodeGenModule &CGM, toolchain::Type *type) {
...
swiftcall::splitLegalVectorType(CodeGenModule &CGM, CharUnits vectorSize,
                                toolchain::VectorType *vectorTy) {
  auto numElts = cast<toolchain::FixedVectorType>(vectorTy)->getNumElements();
  ...
  if (numElts >= 4 && isPowerOf2(numElts)) {
    if (isLegalVectorType(CGM, vectorSize / 2, eltTy, numElts / 2))
      return {toolchain::FixedVectorType::get(eltTy, numElts / 2), 2};
  }
...
void swiftcall::legalizeVectorType(CodeGenModule &CGM, CharUnits origVectorSize,
                                   toolchain::VectorType *origVectorTy,
                             toolchain::SmallVectorImpl<toolchain::Type*> &components) {
  ...
  auto numElts = cast<toolchain::FixedVectorType>(origVectorTy)->getNumElements();
  auto eltTy = origVectorTy->getElementType();
  assert(numElts != 1);

  // The largest size that we're still considering making subvectors of.
  // Always a power of 2.
  unsigned logCandidateNumElts = toolchain::Log2_32(numElts);
  ...
  while (logCandidateNumElts > 0) {
    ...
    components.append(numVecs,
                      toolchain::FixedVectorType::get(eltTy, candidateNumElts));
    ...
    if (numElts > 2 && !isPowerOf2(numElts) &&
      ...
      components.push_back(toolchain::FixedVectorType::get(eltTy, numElts));
      return;
    }
