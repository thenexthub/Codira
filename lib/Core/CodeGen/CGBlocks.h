...
//===----------------------------------------------------------------------===//
//
// This is the internal state used for toolchain translation for block literals.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_LIB_CODEGEN_CGBLOCKS_H
#define LANGUAGE_CORE_LIB_CODEGEN_CGBLOCKS_H
...
public:
  ...
  public:
    ...
    toolchain::Value *getConstant() const {
      assert(isConstant());
      return reinterpret_cast<toolchain::Value*>(Data);
    }
    ...
    static Capture makeConstant(toolchain::Value *value,
                                const BlockDecl::Capture *Cap) {
      Capture v;
