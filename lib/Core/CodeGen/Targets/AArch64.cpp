...
private:
  ...
  toolchain::Type *convertFixedToScalableVectorType(const VectorType *VT) const;
  ...
  ABIArgInfo coerceAndExpandPureScalableAggregate(
      ...
      const SmallVectorImpl<toolchain::Type *> &UnpaddedCoerceToSeq, unsigned &NSRN,
      unsigned &NPRN) const;
  ...
  bool passAsPureScalableType(QualType Ty, unsigned &NV, unsigned &NP,
                              SmallVectorImpl<toolchain::Type *> &CoerceToSeq) const;

  void flattenType(toolchain::Type *Ty,
                   SmallVectorImpl<toolchain::Type *> &Flattened) const;
  ...
  RValue EmitVAArg(CodeGenFunction &CGF, Address VAListAddr, QualType Ty,
    ...
    toolchain::Type *BaseTy = CGF.ConvertType(Ty);
    if (isa<toolchain::ScalableVectorType>(BaseTy))
      toolchain::report_fatal_error("Passing SVE types to variadic functions is "
...
public:
  ...
  bool isLegalVectorType(CharUnits VectorSize, toolchain::Type *EltTy,
...
public:
  ...
  void setTargetAttributes(const Decl *D, toolchain::GlobalValue *GV,
    ...
    auto *Fn = dyn_cast<toolchain::Function>(GV);
    if (!Fn)
      return;
  ...
  bool isScalarizableAsmOperand(CodeGen::CodeGenFunction &CGF,
    ...
    if (CGF.getTarget().hasFeature("ls64")) {
      auto *ST = dyn_cast<toolchain::StructType>(Ty);
      if (ST && ST->getNumElements() == 1) {
        auto *AT = dyn_cast<toolchain::ArrayType>(ST->getElementType(0));
...
AArch64ABIInfo::convertFixedToScalableVectorType(const VectorType *VT) const {
  ...
  if (VT->getVectorKind() == VectorKind::SveFixedLengthPredicate) {
    ...
    return toolchain::ScalableVectorType::get(toolchain::Type::getInt1Ty(getVMContext()),
                                         16);
  }
  ...
  if (VT->getVectorKind() == VectorKind::SveFixedLengthData) {
    ...
    switch (BT->getKind()) {
    default:
      toolchain_unreachable("unexpected builtin type for SVE vector!");

    case BuiltinType::SChar:
    case BuiltinType::UChar:
    case BuiltinType::MFloat8:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getInt8Ty(getVMContext()), 16);

    case BuiltinType::Short:
    case BuiltinType::UShort:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getInt16Ty(getVMContext()), 8);

    case BuiltinType::Int:
    case BuiltinType::UInt:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getInt32Ty(getVMContext()), 4);
    ...
    case BuiltinType::Half:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getHalfTy(getVMContext()), 8);

    case BuiltinType::Float:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getFloatTy(getVMContext()), 4);

    case BuiltinType::Double:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getDoubleTy(getVMContext()), 2);

    case BuiltinType::BFloat16:
      return toolchain::ScalableVectorType::get(
          toolchain::Type::getBFloatTy(getVMContext()), 8);
    }
  }

  toolchain_unreachable("expected fixed-length SVE vector");
}
...
ABIArgInfo AArch64ABIInfo::coerceIllegalVector(QualType Ty, unsigned &NSRN,
  ...
  if (VT->getVectorKind() == VectorKind::SveFixedLengthPredicate) {
    ...
    return ABIArgInfo::getDirect(toolchain::ScalableVectorType::get(
        toolchain::Type::getInt1Ty(getVMContext()), 16));
  }
  ...
  if ((isAndroid() || isOHOSFamily()) && (Size <= 16)) {
    toolchain::Type *ResType = toolchain::Type::getInt16Ty(getVMContext());
    return ABIArgInfo::getDirect(ResType);
  }
  if (Size <= 32) {
    toolchain::Type *ResType = toolchain::Type::getInt32Ty(getVMContext());
    return ABIArgInfo::getDirect(ResType);
  }
  if (Size == 64) {
    NSRN = std::min(NSRN + 1, 8u);
    auto *ResType =
        toolchain::FixedVectorType::get(toolchain::Type::getInt32Ty(getVMContext()), 2);
    return ABIArgInfo::getDirect(ResType);
  }
  if (Size == 128) {
    NSRN = std::min(NSRN + 1, 8u);
    auto *ResType =
        toolchain::FixedVectorType::get(toolchain::Type::getInt32Ty(getVMContext()), 4);
    return ABIArgInfo::getDirect(ResType);
  }
...
ABIArgInfo AArch64ABIInfo::coerceAndExpandPureScalableAggregate(
  ...
  toolchain::Type *UnpaddedCoerceToType =
      UnpaddedCoerceToSeq.size() == 1
          ? UnpaddedCoerceToSeq[0]
          : toolchain::StructType::get(CGT.getLLVMContext(), UnpaddedCoerceToSeq,
                                  true);

  SmallVector<toolchain::Type *> CoerceToSeq;
  flattenType(CGT.ConvertType(Ty), CoerceToSeq);
  auto *CoerceToType =
      toolchain::StructType::get(CGT.getLLVMContext(), CoerceToSeq, false);
...
ABIArgInfo AArch64ABIInfo::classifyArgumentType(QualType Ty, bool IsVariadicFn,
  ...
  if (!Ty->isSVESizelessBuiltinType() && (IsEmpty || Size == 0)) {
    ...
    return ABIArgInfo::getDirect(toolchain::Type::getInt8Ty(getVMContext()));
  }
  ...
  bool IsWin64 = Kind == AArch64ABIKind::Win64 ||
                 CallingConvention == toolchain::CallingConv::Win64;
  ...
  if (!IsWinVariadic && isHomogeneousAggregate(Ty, Base, Members)) {
    ...
    if (Kind != AArch64ABIKind::AAPCS)
      return ABIArgInfo::getDirect(
          toolchain::ArrayType::get(CGT.ConvertType(QualType(Base, 0)), Members));
    ...
    Align = (Align >= 16) ? 16 : 8;
    return ABIArgInfo::getDirect(
        toolchain::ArrayType::get(CGT.ConvertType(QualType(Base, 0)), Members), 0,
        nullptr, true, Align);
  }
  ...
  if (Kind == AArch64ABIKind::AAPCS) {
    ...
    SmallVector<toolchain::Type *> UnpaddedCoerceToSeq;
  ...
  if (Size <= 128) {
    ...
    Size = toolchain::alignTo(Size, Alignment);
    ...
    toolchain::Type *BaseTy = toolchain::Type::getIntNTy(getVMContext(), Alignment);
    if ((Size == 64 || Size == 128) && Alignment == 64 &&
        ContainsOnlyPointers(ContainsOnlyPointers, Ty))
      BaseTy = toolchain::PointerType::getUnqual(getVMContext());
    return ABIArgInfo::getDirect(
        Size == Alignment ? BaseTy
                          : toolchain::ArrayType::get(BaseTy, Size / Alignment));
  }
...
ABIArgInfo AArch64ABIInfo::classifyReturnType(QualType RetTy,
  ...
  if (isHomogeneousAggregate(RetTy, Base, Members) &&
      !(getTarget().getTriple().getArch() == toolchain::Triple::aarch64_32 &&
        IsVariadicFn))
  ...
  if (Size <= 128) {
    if (Size <= 64 && getDataLayout().isLittleEndian()) {
      ...
      // types.
      return ABIArgInfo::getDirect(
          toolchain::IntegerType::get(getVMContext(), Size));
    }

    unsigned Alignment = getContext().getTypeAlign(RetTy);
    Size = toolchain::alignTo(Size, 64); // round up to multiple of 8 bytes

    // We use a pair of i64 for 16-byte aggregate with 8-byte alignment.
    // For aggregates with 16-byte alignment, we use i128.
    if (Alignment < 128 && Size == 128) {
      toolchain::Type *BaseTy = toolchain::Type::getInt64Ty(getVMContext());
      return ABIArgInfo::getDirect(toolchain::ArrayType::get(BaseTy, Size / 64));
    }
...
bool AArch64ABIInfo::isIllegalVectorType(QualType Ty) const {
  if (const VectorType *VT = Ty->getAs<VectorType>()) {
    ...
    if (!toolchain::isPowerOf2_32(NumElements))
      return true;

    // arm64_32 has to be compatible with the ARM logic here, which allows huge
    // vectors for some reason.
    toolchain::Triple Triple = getTarget().getTriple();
    if (Triple.getArch() == toolchain::Triple::aarch64_32 &&
...
bool AArch64SwiftABIInfo::isLegalVectorType(CharUnits VectorSize,
  ...
  if (!toolchain::isPowerOf2_32(NumElts))
    return false;
...
bool AArch64ABIInfo::passAsPureScalableType(
  ...
  if (const ConstantArrayType *AT = getContext().getAsConstantArrayType(Ty)) {
    ...
    unsigned NV = 0, NP = 0;
    SmallVector<toolchain::Type *> EltCoerceToSeq;
    ...
    for (uint64_t I = 0; I < NElt; ++I)
      toolchain::append_range(CoerceToSeq, EltCoerceToSeq);
...
#include "language/Core/Basic/AArch64ACLETypes.def"
  ...
  toolchain::Type *EltTy = Info.ElementType->isMFloat8Type()
                          ? toolchain::Type::getInt8Ty(getVMContext())
                          : CGT.ConvertType(Info.ElementType);
  auto *VTy = toolchain::ScalableVectorType::get(EltTy, Info.EC.getKnownMinValue());
...
void AArch64ABIInfo::flattenType(
    toolchain::Type *Ty, SmallVectorImpl<toolchain::Type *> &Flattened) const {
  ...
  if (const auto *AT = dyn_cast<toolchain::ArrayType>(Ty)) {
    uint64_t NElt = AT->getNumElements();
    if (NElt == 0)
      return;

    SmallVector<toolchain::Type *> EltFlattened;
    flattenType(AT->getElementType(), EltFlattened);

    for (uint64_t I = 0; I < NElt; ++I)
      toolchain::append_range(Flattened, EltFlattened);
    return;
  }

  if (const auto *ST = dyn_cast<toolchain::StructType>(Ty)) {
...
RValue AArch64ABIInfo::EmitAAPCSVAArg(Address VAListAddr, QualType Ty,
  ...
  toolchain::Type *BaseTy = CGF.ConvertType(Ty);
  if (IsIndirect)
    BaseTy = toolchain::PointerType::getUnqual(BaseTy->getContext());
  else if (AI.getCoerceToType())
    BaseTy = AI.getCoerceToType();

  unsigned NumRegs = 1;
  if (toolchain::ArrayType *ArrTy = dyn_cast<toolchain::ArrayType>(BaseTy)) {
  ...
  toolchain::Value *reg_offs = nullptr;
  int reg_top_index;
  ...
  if (!IsFPR) {
    ...
    RegSize = toolchain::alignTo(RegSize, 8);
  } else {
  ...
  UsingStack = CGF.Builder.CreateICmpSGE(
      reg_offs, toolchain::ConstantInt::get(CGF.Int32Ty, 0));
  ...
  if (!IsFPR && !IsIndirect && TyAlign.getQuantity() > 8) {
    ...
    reg_offs = CGF.Builder.CreateAdd(
        reg_offs, toolchain::ConstantInt::get(CGF.Int32Ty, Align - 1),
        "align_regoffs");
    reg_offs = CGF.Builder.CreateAnd(
        reg_offs, toolchain::ConstantInt::get(CGF.Int32Ty, -Align),
        "aligned_regoffs");
  }
  ...
  toolchain::Value *NewOffset = nullptr;
  NewOffset = CGF.Builder.CreateAdd(
      reg_offs, toolchain::ConstantInt::get(CGF.Int32Ty, RegSize), "new_reg_offs");
  ...
  InRegs = CGF.Builder.CreateICmpSLE(
      NewOffset, toolchain::ConstantInt::get(CGF.Int32Ty, 0), "inreg");
  ...
  toolchain::Value *reg_top = nullptr;
  Address reg_top_p =
  ...
  toolchain::Type *MemTy = CGF.ConvertTypeForMem(Ty), *ElementTy = MemTy;

  if (IsIndirect) {
    // If it's been passed indirectly (actually a struct), whatever we find from
    // stored registers or on the stack will actually be a struct **.
    MemTy = toolchain::PointerType::getUnqual(MemTy->getContext());
  }
  ...
  if (IsHFA && NumMembers > 1) {
    ...
    toolchain::Type *BaseTy = CGF.ConvertType(QualType(Base, 0));
    toolchain::Type *HFATy = toolchain::ArrayType::get(BaseTy, NumMembers);
    ...
    for (unsigned i = 0; i < NumMembers; ++i) {
      ...
      toolchain::Value *Elem = CGF.Builder.CreateLoad(LoadAddr);
  ...
  toolchain::Value *OnStackPtr = CGF.Builder.CreateLoad(stack_p, "stack");
  ...
  toolchain::Value *StackSizeC = CGF.Builder.getSize(StackSize);
  toolchain::Value *NewStack = CGF.Builder.CreateInBoundsGEP(
...
void AArch64ABIInfo::appendAttributeMangling(StringRef AttrStr,
  ...
  toolchain::sort(Features, [](const StringRef LHS, const StringRef RHS) {
    return LHS.compare(RHS) < 0;
  });

  toolchain::SmallDenseSet<StringRef, 8> UniqueFeats;
  for (auto &Feat : Features)
    if (auto Ext = toolchain::AArch64::parseFMVExtension(Feat))
