...
#include "toolchain/IR/IntrinsicsS390.h"
...
public:
  ...
  toolchain::Type *getFPArgumentType(QualType Ty, uint64_t Size) const;
...
public:
  ...
  void handleExternallyVisibleObjABI(const Type *Ty, CodeGen::CodeGenModule &M,
    ...
    if (!HasVisibleVecABIFlag && isVectorTypeBased(Ty, IsParam)) {
      M.getModule().addModuleFlag(toolchain::Module::Warning,
  ...
  toolchain::Value *testFPKind(toolchain::Value *V, unsigned BuiltinID,
                          CGBuilderTy &Builder,
    ...
    if (Ty->isHalfTy() || Ty->isFloatTy() || Ty->isDoubleTy() ||
      ...
      toolchain::Function *TDCFunc = toolchain::Intrinsic::getOrInsertDeclaration(
          &M, toolchain::Intrinsic::s390_tdc, Ty);
      unsigned TDCBits = 0;
      switch (BuiltinID) {
      ...
      if (TDCBits)
        return Builder.CreateCall(
            TDCFunc,
            {V, toolchain::ConstantInt::get(toolchain::Type::getInt64Ty(Ctx), TDCBits)});
    }
    return nullptr;
  }
};
...
toolchain::Type *SystemZABIInfo::getFPArgumentType(QualType Ty,
                                              uint64_t Size) const {
  if (IsSoftFloatABI)
    return nullptr;
  ...
  if (const BuiltinType *BT = Ty->getAs<BuiltinType>())
    ...
    case BuiltinType::Float16:
      if (Size == 16)
        return toolchain::Type::getHalfTy(getVMContext());
      LLVM_FALLTHROUGH;
    case BuiltinType::Float:
      if (Size == 32)
        return toolchain::Type::getFloatTy(getVMContext());
      LLVM_FALLTHROUGH;
...
RValue SystemZABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  toolchain::Type *ArgTy = CGF.ConvertTypeForMem(Ty);
  toolchain::Type *DirectTy = ArgTy;
  ...
  SZCGI.handleExternallyVisibleObjABI(Ty.getTypePtr(), CGT.getCGM(),
                                      /*IsParam*/true);
  if (IsIndirect) {
    DirectTy = toolchain::PointerType::getUnqual(DirectTy->getContext());
  ...
  toolchain::Type *IndexTy = CGF.Int64Ty;
  toolchain::Value *PaddedSizeV =
    toolchain::ConstantInt::get(IndexTy, PaddedSize.getQuantity());
  ...
  if (IsVector) {
    ...
    toolchain::Value *NewOverflowArgArea = CGF.Builder.CreateGEP(
  ...
  toolchain::Value *RegCount = CGF.Builder.CreateLoad(RegCountPtr, "reg_count");
  toolchain::Value *MaxRegsV = toolchain::ConstantInt::get(IndexTy, MaxRegs);
  toolchain::Value *InRegs = CGF.Builder.CreateICmpULT(RegCount, MaxRegsV,
                                                 "fits_in_regs");
  ...
  toolchain::Value *ScaledRegCount =
    CGF.Builder.CreateMul(RegCount, PaddedSizeV, "scaled_reg_count");
  toolchain::Value *RegBase =
    toolchain::ConstantInt::get(IndexTy, RegSaveIndex * PaddedSize.getQuantity()
                                      + RegPadding.getQuantity());
  toolchain::Value *RegOffset =
    CGF.Builder.CreateAdd(ScaledRegCount, RegBase, "reg_offset");
  Address RegSaveAreaPtr =
      CGF.Builder.CreateStructGEP(VAListAddr, 3, "reg_save_area_ptr");
  toolchain::Value *RegSaveArea =
  ...
  toolchain::Value *One = toolchain::ConstantInt::get(IndexTy, 1);
  toolchain::Value *NewRegCount =
...
ABIArgInfo SystemZABIInfo::classifyArgumentType(QualType Ty) const {
  ...
  if (const RecordType *RT = Ty->getAs<RecordType>()) {
    ...
    // or a double.
    if (toolchain::Type *FPArgTy = getFPArgumentType(SingleElementTy, Size)) {
      assert(Size == 16 || Size == 32 || Size == 64);
      return ABIArgInfo::getDirect(FPArgTy);
    } else {
      toolchain::IntegerType *PassTy = toolchain::IntegerType::get(getVMContext(), Size);
