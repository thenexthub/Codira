...
public:
  ...
  bool detectFARsEligibleStruct(QualType Ty, toolchain::Type *&Field1Ty,
                                CharUnits &Field1Off, toolchain::Type *&Field2Ty,
                                CharUnits &Field2Off, int &NeededArgGPRs,
                                int &NeededArgFPRs) const;
  ABIArgInfo coerceAndExpandFARsEligibleStruct(toolchain::Type *Field1Ty,
                                               CharUnits Field1Off,
                                               toolchain::Type *Field2Ty,
                                               CharUnits Field2Off) const;
};
...
bool LoongArchABIInfo::detectFARsEligibleStructHelper(
    QualType Ty, CharUnits CurOff, toolchain::Type *&Field1Ty, CharUnits &Field1Off,
    toolchain::Type *&Field2Ty, CharUnits &Field2Off) const {
...
bool LoongArchABIInfo::detectFARsEligibleStruct(
    QualType Ty, toolchain::Type *&Field1Ty, CharUnits &Field1Off,
    toolchain::Type *&Field2Ty, CharUnits &Field2Off, int &NeededGARs,
...
ABIArgInfo LoongArchABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
  ...
  if (Size <= 2 * GRLen) {
    ...
    if (Size <= GRLen) {
      return ABIArgInfo::getDirect(
          toolchain::IntegerType::get(getVMContext(), GRLen));
    }
    if (getContext().getTypeAlign(Ty) == 2 * GRLen) {
      return ABIArgInfo::getDirect(
          toolchain::IntegerType::get(getVMContext(), 2 * GRLen));
    }
    return ABIArgInfo::getDirect(
        toolchain::ArrayType::get(toolchain::IntegerType::get(getVMContext(), GRLen), 2));
  }
  return getNaturalAlignIndirect(
      Ty, /*AddrSpace=*/getDataLayout().getAllocaAddrSpace(),
      /*ByVal=*/false);
