...
typedef toolchain::SmallString<128> SmallStringEnc;
...
class XCoreTargetCodeGenInfo : public TargetCodeGenInfo {
  ...
  void emitTargetMD(const Decl *D, toolchain::GlobalValue *GV,
...
RValue XCoreABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  toolchain::Type *ArgPtrTy = toolchain::PointerType::getUnqual(ArgTy->getContext());
  ...
  case ABIArgInfo::Ignore:
    Val = Address(toolchain::UndefValue::get(ArgPtrTy), ArgTy, TypeAlign);
...
void XCoreTargetCodeGenInfo::emitTargetMD(
    const Decl *D, toolchain::GlobalValue *GV,
    const CodeGen::CodeGenModule &CGM) const {
  SmallStringEnc Enc;
  if (getTypeString(Enc, D, CGM, TSC)) {
    toolchain::LLVMContext &Ctx = CGM.getModule().getContext();
    toolchain::Metadata *MDVals[] = {toolchain::ConstantAsMetadata::get(GV),
                                toolchain::MDString::get(Ctx, Enc.str())};
...
void XCoreTargetCodeGenInfo::emitTargetMetadata(
  ...
  for (unsigned I = 0; I != MangledDeclNames.size(); ++I) {
    ...
    toolchain::GlobalValue *GV = CGM.GetGlobalValue(Val.second);
    if (GV) {
...
static bool extractFieldType(SmallVectorImpl<FieldEncoding> &FE,
  ...
  for (const auto *Field : RD->fields()) {
    ...
    if (Field->isBitField()) {
      Enc += "b(";
      toolchain::raw_svector_ostream OS(Enc);
