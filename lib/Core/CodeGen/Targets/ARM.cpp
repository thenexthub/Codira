...
private:
  ...
  toolchain::CallingConv::ID getLLVMDefaultCC() const;
  toolchain::CallingConv::ID getABIDefaultCC() const;
  void setCCs();
};
...
public:
  ...
  void setTargetAttributes(const Decl *D, toolchain::GlobalValue *GV,
    ...
    toolchain::AttrBuilder B(Fn->getContext());
...
void ARMABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  toolchain::CallingConv::ID cc = getRuntimeCC();
  if (cc != toolchain::CallingConv::C)
    FI.setEffectiveCallingConvention(cc);
}

/// Return the default calling convention that LLVM will use.
toolchain::CallingConv::ID ARMABIInfo::getLLVMDefaultCC() const {
  ...
  if (isEABIHF() || getTarget().getTriple().isWatchABI())
    ...
    return toolchain::CallingConv::ARM_APCS;
}

/// Return the calling convention that our ABI would like us to use
/// as the C calling convention.
toolchain::CallingConv::ID ARMABIInfo::getABIDefaultCC() const {
  switch (getABIKind()) {
...
void ARMABIInfo::setCCs() {
  ...
  toolchain::CallingConv::ID abiCC = getABIDefaultCC();
...
ABIArgInfo ARMABIInfo::coerceIllegalVector(QualType Ty) const {
  ...
  if (Size <= 32) {
    toolchain::Type *ResType =
        toolchain::Type::getInt32Ty(getVMContext());
    return ABIArgInfo::getDirect(ResType);
  }
  if (Size == 64 || Size == 128) {
    auto *ResType = toolchain::FixedVectorType::get(
        toolchain::Type::getInt32Ty(getVMContext()), Size / 32);
    return ABIArgInfo::getDirect(ResType);
  }
...
ABIArgInfo ARMABIInfo::classifyHomogeneousAggregate(QualType Ty,
  ...
  if (const VectorType *VT = Base->getAs<VectorType>()) {
    ...
    if (!getTarget().hasLegalHalfType() && containsAnyFP16Vectors(Ty)) {
      ...
      auto *NewVecTy = toolchain::FixedVectorType::get(
          toolchain::Type::getInt32Ty(getVMContext()), Size / 32);
      toolchain::Type *Ty = toolchain::ArrayType::get(NewVecTy, Members);
...
ABIArgInfo ARMABIInfo::classifyArgumentType(QualType Ty, bool isVariadic,
  ...
  // we can.
  if (TyAlign <= 4) {
    ElemTy = toolchain::Type::getInt32Ty(getVMContext());
    SizeRegs = (getContext().getTypeSize(Ty) + 31) / 32;
  } else {
    ElemTy = toolchain::Type::getInt64Ty(getVMContext());
    SizeRegs = (getContext().getTypeSize(Ty) + 63) / 64;
  }

  return ABIArgInfo::getDirect(toolchain::ArrayType::get(ElemTy, SizeRegs));
}
...
ABIArgInfo ARMABIInfo::classifyReturnType(QualType RetTy, bool isVariadic,
  ...
  if (getABIKind() == ARMABIKind::APCS) {
    ...
    if (RetTy->isAnyComplexType())
      return ABIArgInfo::getDirect(toolchain::IntegerType::get(
  ...
  if (Size <= 32) {
    ...
    toolchain::Type *Int32Ty = toolchain::Type::getInt32Ty(getVMContext());
    toolchain::Type *CoerceTy =
        toolchain::ArrayType::get(Int32Ty, toolchain::alignTo(Size, 32) / 32);
...
bool ARMABIInfo::isIllegalVectorType(QualType Ty) const {
  if (const VectorType *VT = Ty->getAs<VectorType> ()) {
    ...
    if (isAndroid()) {
      ...
      if (!toolchain::isPowerOf2_32(NumElements) && NumElements != 3)
        return true;
    } else {
      // Check whether VT is legal.
      unsigned NumElements = VT->getNumElements();
...
bool ARMABIInfo::containsAnyFP16Vectors(QualType Ty) const {
  if (const ConstantArrayType *AT = getContext().getAsConstantArrayType(Ty)) {
    ...
    if (const CXXRecordDecl *CXXRD = dyn_cast<CXXRecordDecl>(RD))
      if (toolchain::any_of(CXXRD->bases(), [this](const CXXBaseSpecifier &B) {
            return containsAnyFP16Vectors(B.getType());
          }))
        return true;

    if (toolchain::any_of(RD->fields(), [this](FieldDecl *FD) {
...
bool ARMSwiftABIInfo::isLegalVectorType(CharUnits VectorSize, toolchain::Type *EltTy,
...
bool ARMABIInfo::isEffectivelyAAPCS_VFP(unsigned callConvention,
  ...
  if (callConvention != toolchain::CallingConv::C)
    return (callConvention == toolchain::CallingConv::ARM_AAPCS_VFP);
  else
