...
private:
  ...
  bool detectFPCCEligibleStructHelper(QualType Ty, CharUnits CurOff,
                                      toolchain::Type *&Field1Ty,
                                      CharUnits &Field1Off,
                                      toolchain::Type *&Field2Ty,
                                      CharUnits &Field2Off) const;

  bool detectVLSCCEligibleStruct(QualType Ty, unsigned ABIVLen,
                                 toolchain::Type *&VLSType) const;
...
public:
  ...
  ABIArgInfo extendType(QualType Ty, toolchain::Type *CoerceTy = nullptr) const;

  bool detectFPCCEligibleStruct(QualType Ty, toolchain::Type *&Field1Ty,
                                CharUnits &Field1Off, toolchain::Type *&Field2Ty,
                                CharUnits &Field2Off, int &NeededArgGPRs,
                                int &NeededArgFPRs) const;
  ABIArgInfo coerceAndExpandFPCCEligibleStruct(toolchain::Type *Field1Ty,
                                               CharUnits Field1Off,
                                               toolchain::Type *Field2Ty,
                                               CharUnits Field2Off) const;
...
void RISCVABIInfo::appendAttributeMangling(StringRef AttrStr,
  ...
  toolchain::stable_sort(Features);
...
bool RISCVABIInfo::detectFPCCEligibleStruct(QualType Ty, toolchain::Type *&Field1Ty,
                                            CharUnits &Field1Off,
                                            toolchain::Type *&Field2Ty,
                                            CharUnits &Field2Off,
                                            int &NeededArgGPRs,
...
ABIArgInfo RISCVABIInfo::coerceAndExpandFPCCEligibleStruct(
    toolchain::Type *Field1Ty, CharUnits Field1Off, toolchain::Type *Field2Ty,
    CharUnits Field2Off) const {
  SmallVector<toolchain::Type *, 3> CoerceElts;
  SmallVector<toolchain::Type *, 2> UnpaddedCoerceElts;
  if (!Field1Off.isZero())
    CoerceElts.push_back(toolchain::ArrayType::get(
        toolchain::Type::getInt8Ty(getVMContext()), Field1Off.getQuantity()));
  ...
  if (!Field2Ty) {
    return ABIArgInfo::getCoerceAndExpand(
        toolchain::StructType::get(getVMContext(), CoerceElts, !Field1Off.isZero()),
  ...
  if (!Padding.isZero())
    CoerceElts.push_back(toolchain::ArrayType::get(
        toolchain::Type::getInt8Ty(getVMContext()), Padding.getQuantity()));

  CoerceElts.push_back(Field2Ty);
  UnpaddedCoerceElts.push_back(Field2Ty);

  auto CoerceToType =
      toolchain::StructType::get(getVMContext(), CoerceElts, IsPacked);
  auto UnpaddedCoerceToType =
      toolchain::StructType::get(getVMContext(), UnpaddedCoerceElts, IsPacked);
...
bool RISCVABIInfo::detectVLSCCEligibleStruct(QualType Ty, unsigned ABIVLen,
  ...
  toolchain::StructType *STy = dyn_cast<toolchain::StructType>(CGT.ConvertType(Ty));
  if (!STy)
    return false;
  ...
  if (auto *ArrayTy = dyn_cast<toolchain::ArrayType>(FirstEltTy)) {
  ...
  auto *FixedVecTy = dyn_cast<toolchain::FixedVectorType>(FirstEltTy);
  if (!FixedVecTy)
    return false;

  // Check registers needed <= 8.
  if (NumElts * toolchain::divideCeil(
  ...
  if (NumElts == 1) {
    ...
    VLSType = toolchain::ScalableVectorType::get(
        FixedVecTy->getElementType(),
        toolchain::divideCeil(FixedVecTy->getNumElements() *
                             toolchain::RISCV::RVVBitsPerBlock,
                         ABIVLen));
    return true;
  }
  ...
  unsigned I8EltCount =
      toolchain::divideCeil(FixedVecTy->getNumElements() *
                           ...
                           toolchain::RISCV::RVVBitsPerBlock,
                       ABIVLen * 8);
  VLSType = toolchain::TargetExtType::get(
      getVMContext(), "riscv.vector.tuple",
      toolchain::ScalableVectorType::get(toolchain::Type::getInt8Ty(getVMContext()),
                                    I8EltCount),
      NumElts);
  return true;
}
...
ABIArgInfo RISCVABIInfo::coerceVLSVector(QualType Ty, unsigned ABIVLen) const {
  ...
  toolchain::Type *EltType = toolchain::Type::getInt1Ty(getVMContext());
  ...
  toolchain::ScalableVectorType *ResType;
  ...
  if (ABIVLen == 0) {
    ...
    ResType = toolchain::ScalableVectorType::get(EltType, NumElts / VScale->first);
  } else {
    // Check registers needed <= 8.
    ...
    ResType = toolchain::ScalableVectorType::get(
        EltType,
        toolchain::divideCeil(NumElts * toolchain::RISCV::RVVBitsPerBlock, ABIVLen));
    ...
    if ((EltType->isHalfTy() && !TI.hasFeature("zvfhmin")) ||
      ...
      ResType = toolchain::ScalableVectorType::get(
          ...
          toolchain::divideCeil(EltType->getScalarSizeInBits() * NumElts *
                               toolchain::RISCV::RVVBitsPerBlock,
                           8 * ABIVLen));
    }
  }
...
ABIArgInfo RISCVABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
  ...
  if (IsFixed && FLen && Ty->isStructureOrClassType()) {
    toolchain::Type *Field1Ty = nullptr;
    toolchain::Type *Field2Ty = nullptr;
  ...
  if (IsFixed && Ty->isStructureOrClassType()) {
    toolchain::Type *VLSType = nullptr;
  ...
  if (Size <= 2 * XLen) {
    ...
    if (Size <= XLen) {
      ...
      return ABIArgInfo::getDirect(
          toolchain::IntegerType::get(getVMContext(), 2 * XLen));
    } else {
      return ABIArgInfo::getDirect(toolchain::ArrayType::get(
          toolchain::IntegerType::get(getVMContext(), XLen), 2));
    }
  }
  return getNaturalAlignIndirect(
      Ty, /*AddrSpace=*/getDataLayout().getAllocaAddrSpace(),
...
ABIArgInfo RISCVABIInfo::extendType(QualType Ty, toolchain::Type *CoerceTy) const {
