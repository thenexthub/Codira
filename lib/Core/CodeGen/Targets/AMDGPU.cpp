...
private:
  ...
  toolchain::Type *coerceKernelArgumentType(toolchain::Type *Ty, unsigned FromAS,
                                       unsigned ToAS) const {
    // Single value types.
    auto *PtrTy = toolchain::dyn_cast<toolchain::PointerType>(Ty);
    if (PtrTy && PtrTy->getAddressSpace() == FromAS)
      return toolchain::PointerType::get(Ty->getContext(), ToAS);
    return Ty;
  }
...
void AMDGPUABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  for (auto &Arg : FI.arguments()) {
    if (CC == toolchain::CallingConv::AMDGPU_KERNEL) {
...
ABIArgInfo AMDGPUABIInfo::classifyReturnType(QualType RetTy) const {
  if (isAggregateTypeForABI(RetTy)) {
    ...
    if (!getRecordArgABI(RetTy, getCXXABI())) {
      ...
      if (Size <= 16)
        return ABIArgInfo::getDirect(toolchain::Type::getInt16Ty(getVMContext()));

      if (Size <= 32)
        return ABIArgInfo::getDirect(toolchain::Type::getInt32Ty(getVMContext()));

      if (Size <= 64) {
        toolchain::Type *I32Ty = toolchain::Type::getInt32Ty(getVMContext());
        return ABIArgInfo::getDirect(toolchain::ArrayType::get(I32Ty, 2));
      }
...
ABIArgInfo AMDGPUABIInfo::classifyKernelArgumentType(QualType Ty) const {
  ...
  toolchain::Type *OrigLTy = CGT.ConvertType(Ty);
  toolchain::Type *LTy = OrigLTy;
...
public:
  ...
  void setFunctionDeclAttributes(const FunctionDecl *FD, toolchain::Function *F,
  ...
  toolchain::Constant *getNullPointer(const CodeGen::CodeGenModule &CGM,
      toolchain::PointerType *T, QualType QT) const override;
  ...
  void setTargetAtomicMetadata(CodeGenFunction &CGF,
                               toolchain::Instruction &AtomicInst,
                               const AtomicExpr *Expr = nullptr) const override;
  toolchain::Value *createEnqueuedBlockKernel(CodeGenFunction &CGF,
                                         toolchain::Function *BlockInvokeFunc,
                                         toolchain::Type *BlockTy) const override;
...
static bool requiresAMDGPUProtectedVisibility(const Decl *D,
  ...
  if (GV->getVisibility() != toolchain::GlobalValue::HiddenVisibility)
    return false;
...
void AMDGPUTargetCodeGenInfo::setFunctionDeclAttributes(
    const FunctionDecl *FD, toolchain::Function *F, CodeGenModule &M) const {
  const auto *ReqdWGS =
  ...
  if (ReqdWGS || FlatWGS) {
    ...
    std::string AttrVal =
        std::string("1,") + toolchain::utostr(DefaultMaxWorkGroupSize);
  ...
  if (const auto *Attr = FD->getAttr<AMDGPUNumSGPRAttr>()) {
    ...
    if (NumSGPR != 0)
      F->addFnAttr("amdgpu-num-sgpr", toolchain::utostr(NumSGPR));
  }
  ...
  if (const auto *Attr = FD->getAttr<AMDGPUNumVGPRAttr>()) {
    ...
    if (NumVGPR != 0)
      F->addFnAttr("amdgpu-num-vgpr", toolchain::utostr(NumVGPR));
  }
  ...
  if (const auto *Attr = FD->getAttr<AMDGPUMaxNumWorkGroupsAttr>()) {
    ...
    toolchain::SmallString<32> AttrVal;
    toolchain::raw_svector_ostream OS(AttrVal);
    OS << X << ',' << Y << ',' << Z;
...
void AMDGPUTargetCodeGenInfo::setTargetAttributes(
  ...
  toolchain::Function *F = dyn_cast<toolchain::Function>(GV);
  if (!F)
    return;
...
unsigned AMDGPUTargetCodeGenInfo::getDeviceKernelCallingConv() const {
  return toolchain::CallingConv::AMDGPU_KERNEL;
}
...
toolchain::Constant *AMDGPUTargetCodeGenInfo::getNullPointer(
    const CodeGen::CodeGenModule &CGM, toolchain::PointerType *PT,
    QualType QT) const {
...
void AMDGPUTargetCodeGenInfo::setTargetAtomicMetadata(
    CodeGenFunction &CGF, toolchain::Instruction &AtomicInst,
    const AtomicExpr *AE) const {
  auto *RMW = dyn_cast<toolchain::AtomicRMWInst>(&AtomicInst);
  auto *CmpX = dyn_cast<toolchain::AtomicCmpXchgInst>(&AtomicInst);
  ...
  if (((RMW && RMW->getPointerAddressSpace() == toolchain::AMDGPUAS::FLAT_ADDRESS) ||
       (CmpX &&
        CmpX->getPointerAddressSpace() == toolchain::AMDGPUAS::FLAT_ADDRESS)) &&
      AE && AE->threadPrivateMemoryAtomicsAreUndefined()) {
    toolchain::MDBuilder MDHelper(CGF.getLLVMContext());
    toolchain::MDNode *ASRange = MDHelper.createRange(
        toolchain::APInt(32, toolchain::AMDGPUAS::PRIVATE_ADDRESS),
        toolchain::APInt(32, toolchain::AMDGPUAS::PRIVATE_ADDRESS + 1));
    AtomicInst.setMetadata(toolchain::LLVMContext::MD_noalias_addrspace, ASRange);
  }
  ...
  toolchain::MDNode *Empty = toolchain::MDNode::get(CGF.getLLVMContext(), {});
  if (!AO.getOption(language::Core::AtomicOptionKind::FineGrainedMemory))
    RMW->setMetadata("amdgpu.no.fine.grained.memory", Empty);
  if (!AO.getOption(language::Core::AtomicOptionKind::RemoteMemory))
    RMW->setMetadata("amdgpu.no.remote.memory", Empty);
  if (AO.getOption(language::Core::AtomicOptionKind::IgnoreDenormalMode) &&
      RMW->getOperation() == toolchain::AtomicRMWInst::FAdd &&
...
getAMDGPURuntimeHandleType(toolchain::LLVMContext &C,
                           toolchain::Type *KernelDescriptorPtrTy) {
  toolchain::Type *Int32 = toolchain::Type::getInt32Ty(C);
  return toolchain::StructType::create(C, {KernelDescriptorPtrTy, Int32, Int32},
...
toolchain::Value *AMDGPUTargetCodeGenInfo::createEnqueuedBlockKernel(
    CodeGenFunction &CGF, toolchain::Function *Invoke, toolchain::Type *BlockTy) const {
  auto &Builder = CGF.Builder;
  auto &C = CGF.getLLVMContext();

  auto *InvokeFT = Invoke->getFunctionType();
  toolchain::SmallVector<toolchain::Type *, 2> ArgTys;
  toolchain::SmallVector<toolchain::Metadata *, 8> AddressQuals;
  toolchain::SmallVector<toolchain::Metadata *, 8> AccessQuals;
  toolchain::SmallVector<toolchain::Metadata *, 8> ArgTypeNames;
  toolchain::SmallVector<toolchain::Metadata *, 8> ArgBaseTypeNames;
  toolchain::SmallVector<toolchain::Metadata *, 8> ArgTypeQuals;
  toolchain::SmallVector<toolchain::Metadata *, 8> ArgNames;

  ArgTys.push_back(BlockTy);
  ArgTypeNames.push_back(toolchain::MDString::get(C, "__block_literal"));
  AddressQuals.push_back(toolchain::ConstantAsMetadata::get(Builder.getInt32(0)));
  ArgBaseTypeNames.push_back(toolchain::MDString::get(C, "__block_literal"));
  ArgTypeQuals.push_back(toolchain::MDString::get(C, ""));
  AccessQuals.push_back(toolchain::MDString::get(C, "none"));
  ArgNames.push_back(toolchain::MDString::get(C, "block_literal"));
  for (unsigned I = 1, E = InvokeFT->getNumParams(); I < E; ++I) {
    ArgTys.push_back(InvokeFT->getParamType(I));
    ArgTypeNames.push_back(toolchain::MDString::get(C, "void*"));
    AddressQuals.push_back(toolchain::ConstantAsMetadata::get(Builder.getInt32(3)));
    AccessQuals.push_back(toolchain::MDString::get(C, "none"));
    ArgBaseTypeNames.push_back(toolchain::MDString::get(C, "void*"));
    ArgTypeQuals.push_back(toolchain::MDString::get(C, ""));
    ArgNames.push_back(
        toolchain::MDString::get(C, (Twine("local_arg") + Twine(I)).str()));
  }

  toolchain::Module &Mod = CGF.CGM.getModule();
  const toolchain::DataLayout &DL = Mod.getDataLayout();

  toolchain::Twine Name = Invoke->getName() + "_kernel";
  auto *FT = toolchain::FunctionType::get(toolchain::Type::getVoidTy(C), ArgTys, false);

  // The kernel itself can be internal, the runtime does not directly access the
  // kernel address (only the kernel descriptor).
  auto *F = toolchain::Function::Create(FT, toolchain::GlobalValue::InternalLinkage, Name,
                                   &Mod);
  F->setCallingConv(toolchain::CallingConv::AMDGPU_KERNEL);
  ...
  toolchain::SmallVector<toolchain::Value *, 2> Args;
  Args.push_back(Cast);
  for (toolchain::Argument &A : toolchain::drop_begin(F->args()))
    Args.push_back(&A);
  ...
  F->setMetadata("kernel_arg_addr_space", toolchain::MDNode::get(C, AddressQuals));
  F->setMetadata("kernel_arg_access_qual", toolchain::MDNode::get(C, AccessQuals));
  F->setMetadata("kernel_arg_type", toolchain::MDNode::get(C, ArgTypeNames));
  F->setMetadata("kernel_arg_base_type",
                 toolchain::MDNode::get(C, ArgBaseTypeNames));
  F->setMetadata("kernel_arg_type_qual", toolchain::MDNode::get(C, ArgTypeQuals));
  if (CGF.CGM.getCodeGenOpts().EmitOpenCLArgMetadata)
    F->setMetadata("kernel_arg_name", toolchain::MDNode::get(C, ArgNames));

  toolchain::StructType *HandleTy = getAMDGPURuntimeHandleType(
      C, toolchain::PointerType::get(C, DL.getDefaultGlobalsAddressSpace()));
  toolchain::Constant *RuntimeHandleInitializer =
      toolchain::ConstantAggregateZero::get(HandleTy);

  toolchain::Twine RuntimeHandleName = F->getName() + ".runtime.handle";
  ...
  auto *RuntimeHandle = new toolchain::GlobalVariable(
      Mod, HandleTy,
  ...
  toolchain::MDNode *HandleAsMD =
      toolchain::MDNode::get(C, toolchain::ValueAsMetadata::get(RuntimeHandle));
  F->setMetadata(toolchain::LLVMContext::MD_associated, HandleAsMD);
...
void CodeGenModule::handleAMDGPUFlatWorkGroupSizeAttr(
    toolchain::Function *F, const AMDGPUFlatWorkGroupSizeAttr *FlatWGS,
  ...
  if (Min != 0) {
    ...
    if (MaxThreadsVal)
      *MaxThreadsVal = Max;
    std::string AttrVal = toolchain::utostr(Min) + "," + toolchain::utostr(Max);
    if (F)
      F->addFnAttr("amdgpu-flat-work-group-size", AttrVal);
  } else
...
void CodeGenModule::handleAMDGPUWavesPerEUAttr(
    toolchain::Function *F, const AMDGPUWavesPerEUAttr *Attr) {
  unsigned Min =
  ...
  if (Min != 0) {
    ...
    std::string AttrVal = toolchain::utostr(Min);
    if (Max != 0)
      AttrVal = AttrVal + "," + toolchain::utostr(Max);
