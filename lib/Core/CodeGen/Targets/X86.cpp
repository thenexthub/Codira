...
bool IsX86_MMXType(toolchain::Type *IRType) {
  // Return true if the type is an MMX type <2 x i32>, <4 x i16>, or <8 x i8>.
  return IRType->isVectorTy() && IRType->getPrimitiveSizeInBits() == 64 &&
    cast<toolchain::VectorType>(IRType)->getElementType()->isIntegerTy() &&
    IRType->getScalarSizeInBits() != 64;
}

static toolchain::Type *X86AdjustInlineAsmType(CodeGen::CodeGenFunction &CGF,
                                          StringRef Constraint,
                                          toolchain::Type *Ty) {
  if (Constraint == "k") {
    toolchain::Type *Int1Ty = toolchain::Type::getInt1Ty(CGF.getLLVMContext());
    return toolchain::FixedVectorType::get(Int1Ty, Ty->getScalarSizeInBits());
  }
...
static ABIArgInfo getDirectX86Hva(toolchain::Type* T = nullptr) {
...
/// Similar to toolchain::CCState, but for Clang.
struct CCState {
  ...
  toolchain::SmallBitVector IsPreassigned;
...
class X86_32ABIInfo : public ABIInfo {
  ...
  void addFieldToArgStruct(SmallVector<toolchain::Type *, 6> &FrameFields,
...
public:
  ...
  bool shouldPassIndirectly(ArrayRef<toolchain::Type *> ComponentTys,
...
public:
  ...
  static bool isStructReturnInRegABI(
      const toolchain::Triple &Triple, const CodeGenOptions &Opts);
  ...
  toolchain::Type* adjustInlineAsmType(CodeGen::CodeGenFunction &CGF,
                                  StringRef Constraint,
                                  toolchain::Type* Ty) const override {
...
void X86_32TargetCodeGenInfo::addReturnRegisterOutputs(
  ...
  toolchain::Type *CoerceTy = toolchain::IntegerType::get(CGF.getLLVMContext(), RetWidth);
...
ABIArgInfo X86_32ABIInfo::getIndirectReturnResult(QualType RetTy, CCState &State) const {
  ...
  // integer register.
  if (State.CC != toolchain::CallingConv::X86_FastCall &&
      State.CC != toolchain::CallingConv::X86_VectorCall && State.FreeRegs) {
    --State.FreeRegs;
    if (!IsMCUABI)
...
ABIArgInfo X86_32ABIInfo::classifyReturnType(QualType RetTy,
  ...
  uint64_t NumElts = 0;
  if ((State.CC == toolchain::CallingConv::X86_VectorCall ||
       State.CC == toolchain::CallingConv::X86_RegCall) &&
  ...
  if (const VectorType *VT = RetTy->getAs<VectorType>()) {
    ...
    if (IsDarwinVectorABI) {
      ...
      // backend will like.
      if (Size == 128)
        return ABIArgInfo::getDirect(toolchain::FixedVectorType::get(
            toolchain::Type::getInt64Ty(getVMContext()), 2));

      // Always return in register if it fits in a general purpose
      // register, or if it is 64 bits and has a single element.
      if ((Size == 8 || Size == 16 || Size == 32) ||
          (Size == 64 && VT->getNumElements() == 1))
        return ABIArgInfo::getDirect(toolchain::IntegerType::get(getVMContext(),
                                                            Size));
  ...
  if (isAggregateTypeForABI(RetTy)) {
    ...
    if (const ComplexType *CT = RetTy->getAs<ComplexType>()) {
      ...
      if (ET->isFloat16Type())
        return ABIArgInfo::getDirect(toolchain::FixedVectorType::get(
            toolchain::Type::getHalfTy(getVMContext()), 2));
    }
    ...
    if (shouldReturnTypeInRegister(RetTy, getContext())) {
      ...
      // padding.
      return ABIArgInfo::getDirect(toolchain::IntegerType::get(getVMContext(),Size));
    }
...
bool X86_32ABIInfo::shouldAggregateUseDirect(QualType Ty, CCState &State,
  ...
  if (State.CC == toolchain::CallingConv::X86_FastCall ||
      State.CC == toolchain::CallingConv::X86_VectorCall ||
      State.CC == toolchain::CallingConv::X86_RegCall) {
...
bool X86_32ABIInfo::shouldPrimitiveUseInReg(QualType Ty, CCState &State) const {
  ...
  if (!IsPtrOrInt && (State.CC == toolchain::CallingConv::X86_FastCall ||
                      State.CC == toolchain::CallingConv::X86_VectorCall))
    return false;
  ...
  if (!IsPtrOrInt && State.CC == toolchain::CallingConv::X86_RegCall)
    return false;
...
ABIArgInfo X86_32ABIInfo::classifyArgumentType(QualType Ty, CCState &State,
  ...
  bool IsFastCall = State.CC == toolchain::CallingConv::X86_FastCall;
  bool IsRegCall = State.CC == toolchain::CallingConv::X86_RegCall;
  bool IsVectorCall = State.CC == toolchain::CallingConv::X86_VectorCall;
  ...
  if (isAggregateTypeForABI(Ty)) {
    ...
    toolchain::IntegerType *PaddingType = NeedsPadding ? Int32 : nullptr;
  ...
  if (const VectorType *VT = Ty->getAs<VectorType>()) {
    ...
    if (IsDarwinVectorABI) {
      if ((TI.Width == 8 || TI.Width == 16 || TI.Width == 32) ||
        ...
        return ABIArgInfo::getDirect(
            toolchain::IntegerType::get(getVMContext(), TI.Width));
    }

    if (IsX86_MMXType(CGT.ConvertType(Ty)))
      return ABIArgInfo::getDirect(toolchain::IntegerType::get(getVMContext(), 64));
...
void X86_32ABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  if (IsMCUABI)
    State.FreeRegs = 3;
  else if (State.CC == toolchain::CallingConv::X86_FastCall) {
    State.FreeRegs = 2;
    State.FreeSSERegs = 3;
  } else if (State.CC == toolchain::CallingConv::X86_VectorCall) {
    State.FreeRegs = 2;
    State.FreeSSERegs = 6;
  } else if (FI.getHasRegParm())
    State.FreeRegs = FI.getRegParm();
  else if (State.CC == toolchain::CallingConv::X86_RegCall) {
    State.FreeRegs = 5;
    State.FreeSSERegs = 8;
...
void
X86_32ABIInfo::addFieldToArgStruct(SmallVector<toolchain::Type *, 6> &FrameFields,
                                   CharUnits &StackOffset, ABIArgInfo &Info,
                                   QualType Type) const {
  ...
  toolchain::Type *LLTy = CGT.ConvertTypeForMem(Type);
  if (IsIndirect)
    LLTy = toolchain::PointerType::getUnqual(getVMContext());
  ...
  if (StackOffset != FieldEnd) {
    ...
    toolchain::Type *Ty = toolchain::Type::getInt8Ty(getVMContext());
    Ty = toolchain::ArrayType::get(Ty, NumBytes.getQuantity());
...
void X86_32ABIInfo::rewriteWithInAlloca(CGFunctionInfo &FI) const {
  ...
  SmallVector<toolchain::Type *, 6> FrameFields;
  ...
  bool IsThisCall =
      FI.getCallingConvention() == toolchain::CallingConv::X86_ThisCall;
  ...
  FI.setArgStruct(toolchain::StructType::get(getVMContext(), FrameFields,
                                        /*isPacked=*/true),
                  StackAlign);
}
...
bool X86_32TargetCodeGenInfo::isStructReturnInRegABI(
    const toolchain::Triple &Triple, const CodeGenOptions &Opts) {
  assert(Triple.getArch() == toolchain::Triple::x86);
...
static void addX86InterruptAttrs(const FunctionDecl *FD, toolchain::GlobalValue *GV,
                                 CodeGen::CodeGenModule &CGM) {
  if (!FD->hasAttr<AnyX86InterruptAttr>())
    return;

  toolchain::Function *Fn = cast<toolchain::Function>(GV);
  Fn->setCallingConv(toolchain::CallingConv::X86_INTR);
  if (FD->getNumParams() == 0)
    return;

  auto PtrTy = cast<PointerType>(FD->getParamDecl(0)->getType());
  toolchain::Type *ByValTy = CGM.getTypes().ConvertType(PtrTy->getPointeeType());
  toolchain::Attribute NewAttr = toolchain::Attribute::getWithByValType(
...
bool X86_32TargetCodeGenInfo::initDwarfEHRegSizeTable(
  ...
  if (CGF.CGM.getTarget().getTriple().isOSDarwin()) {
    ...
    toolchain::Value *Sixteen8 = toolchain::ConstantInt::get(CGF.Int8Ty, 16);
    ...
    toolchain::Value *Twelve8 = toolchain::ConstantInt::get(CGF.Int8Ty, 12);
...
static unsigned getNativeVectorSizeForAVXABI(X86AVXABILevel AVXLevel) {
  ...
  toolchain_unreachable("Unknown AVXLevel");
}
...
class X86_64ABIInfo : public ABIInfo {
  ...
  toolchain::Type *GetByteVectorType(QualType Ty) const;
  toolchain::Type *GetSSETypeAtOffset(toolchain::Type *IRType,
                                 unsigned IROffset, QualType SourceTy,
                                 unsigned SourceOffset) const;
  toolchain::Type *GetINTEGERTypeAtOffset(toolchain::Type *IRType,
                                     unsigned IROffset, QualType SourceTy,
                                     unsigned SourceOffset) const;
  ...
  bool classifyIntegerMMXAsSSE() const {
    ...
    const toolchain::Triple &Triple = getTarget().getTriple();
  ...
  bool passInt128VectorsInMem() const {
    ...
    const toolchain::Triple &T = getTarget().getTriple();
...
public:
  ...
  bool isPassedUsingAVXType(QualType type) const {
    ...
    ABIArgInfo info = classifyArgumentType(type, 0, neededInt, neededSSE,
                                           /*isNamedArg*/true);
    if (info.isDirect()) {
      toolchain::Type *ty = info.getCoerceToType();
      if (toolchain::VectorType *vectorTy = dyn_cast_or_null<toolchain::VectorType>(ty))
...
public:
  ...
  bool initDwarfEHRegSizeTable(CodeGen::CodeGenFunction &CGF,
                               toolchain::Value *Address) const override {
    toolchain::Value *Eight8 = toolchain::ConstantInt::get(CGF.Int8Ty, 8);
...
static void initFeatureMaps(const ASTContext &Ctx,
                            toolchain::StringMap<bool> &CallerMap,
                            const FunctionDecl *Caller,
                            toolchain::StringMap<bool> &CalleeMap,
...
static bool checkAVXParamFeature(DiagnosticsEngine &Diag,
                                 ...
                                 const toolchain::StringMap<bool> &CallerMap,
                                 const toolchain::StringMap<bool> &CalleeMap,
...
static bool checkAVXParam(DiagnosticsEngine &Diag, ASTContext &Ctx,
                          SourceLocation CallLoc,
                          const toolchain::StringMap<bool> &CallerMap,
                          const toolchain::StringMap<bool> &CalleeMap, QualType Ty,
                          bool IsArgument) {
...
public:
  ...
  void getDetectMismatchOption(toolchain::StringRef Name,
...
void X86_64ABIInfo::classify(QualType Ty, uint64_t OffsetBase, Class &Lo,
  ...
  if (const BuiltinType *BT = Ty->getAs<BuiltinType>()) {
    ...
    if (k == BuiltinType::Void) {
      ...
      const toolchain::fltSemantics *LDF = &getTarget().getLongDoubleFormat();
      if (LDF == &toolchain::APFloat::IEEEquad()) {
        Lo = SSE;
        Hi = SSEUp;
      } else if (LDF == &toolchain::APFloat::x87DoubleExtended()) {
        Lo = X87;
        Hi = X87Up;
      } else if (LDF == &toolchain::APFloat::IEEEdouble()) {
        Current = SSE;
      } else
        toolchain_unreachable("unexpected long double representation!");
    }
...
toolchain::Type *X86_64ABIInfo::GetByteVectorType(QualType Ty) const {
  // Wrapper structs/arrays that only contain vectors are passed just like
  // vectors; strip them off if present.
  if (const Type *InnerTy = isSingleElementStruct(Ty, getContext()))
    Ty = QualType(InnerTy, 0);

  toolchain::Type *IRType = CGT.ConvertType(Ty);
  if (isa<toolchain::VectorType>(IRType)) {
    // Don't pass vXi128 vectors in their native type, the backend can't
    // legalize them.
    if (passInt128VectorsInMem() &&
        cast<toolchain::VectorType>(IRType)->getElementType()->isIntegerTy(128)) {
      // Use a vXi64 vector.
      uint64_t Size = getContext().getTypeSize(Ty);
      return toolchain::FixedVectorType::get(toolchain::Type::getInt64Ty(getVMContext()),
                                        Size / 64);
    }
  ...
  if (IRType->getTypeID() == toolchain::Type::FP128TyID)
    return IRType;
  ...
  return toolchain::FixedVectorType::get(toolchain::Type::getDoubleTy(getVMContext()),
                                    Size / 64);
}
...
static toolchain::Type *getFPTypeAtOffset(toolchain::Type *IRType, unsigned IROffset,
                                     const toolchain::DataLayout &TD) {
  if (IROffset == 0 && IRType->isFloatingPointTy())
    return IRType;

  // If this is a struct, recurse into the field at the specified offset.
  if (toolchain::StructType *STy = dyn_cast<toolchain::StructType>(IRType)) {
    if (!STy->getNumContainedTypes())
      return nullptr;

    const toolchain::StructLayout *SL = TD.getStructLayout(STy);
  ...
  if (toolchain::ArrayType *ATy = dyn_cast<toolchain::ArrayType>(IRType)) {
    toolchain::Type *EltTy = ATy->getElementType();
...
toolchain::Type *X86_64ABIInfo::
GetSSETypeAtOffset(toolchain::Type *IRType, unsigned IROffset,
                   QualType SourceTy, unsigned SourceOffset) const {
  const toolchain::DataLayout &TD = getDataLayout();
  unsigned SourceSize =
      (unsigned)getContext().getTypeSize(SourceTy) / 8 - SourceOffset;
  toolchain::Type *T0 = getFPTypeAtOffset(IRType, IROffset, TD);
  if (!T0 || T0->isDoubleTy())
    return toolchain::Type::getDoubleTy(getVMContext());

  // Get the adjacent FP type.
  toolchain::Type *T1 = nullptr;
  ...
  if (T0->isFloatTy() && T1->isFloatTy())
    return toolchain::FixedVectorType::get(T0, 2);

  if (T0->is16bitFPTy() && T1->is16bitFPTy()) {
    toolchain::Type *T2 = nullptr;
    if (SourceSize > 4)
      T2 = getFPTypeAtOffset(IRType, IROffset + 4, TD);
    if (T2 == nullptr)
      return toolchain::FixedVectorType::get(T0, 2);
    return toolchain::FixedVectorType::get(T0, 4);
  }

  if (T0->is16bitFPTy() || T1->is16bitFPTy())
    return toolchain::FixedVectorType::get(toolchain::Type::getHalfTy(getVMContext()), 4);
...
///
toolchain::Type *X86_64ABIInfo::
GetINTEGERTypeAtOffset(toolchain::Type *IRType, unsigned IROffset,
                       QualType SourceTy, unsigned SourceOffset) const {
  // If we're dealing with an un-offset LLVM IR type, then it means that we're
  // returning an 8-byte unit starting with it.  See if we can safely use it.
  if (IROffset == 0) {
    // Pointers and int64's always fill the 8-byte unit.
    if ((isa<toolchain::PointerType>(IRType) && Has64BitPointers) ||
    ...
    if (IRType->isIntegerTy(8) || IRType->isIntegerTy(16) ||
        ...
        (isa<toolchain::PointerType>(IRType) && !Has64BitPointers)) {
      unsigned BitWidth = isa<toolchain::PointerType>(IRType) ? 32 :
          cast<toolchain::IntegerType>(IRType)->getBitWidth();
  ...
  if (toolchain::StructType *STy = dyn_cast<toolchain::StructType>(IRType)) {
    // If this is a struct, recurse into the field at the specified offset.
    const toolchain::StructLayout *SL = getDataLayout().getStructLayout(STy);
  ...
  return toolchain::IntegerType::get(getVMContext(),
...
/// return {i32*, float}.
static toolchain::Type *
GetX86_64ByValArgumentPair(toolchain::Type *Lo, toolchain::Type *Hi,
                           const toolchain::DataLayout &TD) {
  ...
  toolchain::Align HiAlign = TD.getABITypeAlign(Hi);
  unsigned HiStart = toolchain::alignTo(LoSize, HiAlign);
  ...
  if (HiStart != 8) {
    ...
    if (Lo->isHalfTy() || Lo->isFloatTy())
      Lo = toolchain::Type::getDoubleTy(Lo->getContext());
    else {
      assert((Lo->isIntegerTy() || Lo->isPointerTy())
             && "Invalid/unknown lo type");
      Lo = toolchain::Type::getInt64Ty(Lo->getContext());
    }
  }

  toolchain::StructType *Result = toolchain::StructType::get(Lo, Hi);
...
ABIArgInfo X86_64ABIInfo::classifyReturnType(QualType RetTy) const {
  ...
  toolchain::Type *ResType = nullptr;
  switch (Lo) {
  case NoClass:
    if (Hi == NoClass)
      return ABIArgInfo::getIgnore();
  ...
  case SSEUp:
  case X87Up:
    toolchain_unreachable("Invalid classification for lo word.");
  ...
  case Integer:
    ...
    if (Hi == NoClass && isa<toolchain::IntegerType>(ResType)) {
      // Treat an enum type as its underlying type.
      if (const EnumType *EnumTy = RetTy->getAs<EnumType>())
        RetTy =
  ...
  case X87:
    ResType = toolchain::Type::getX86_FP80Ty(getVMContext());
    break;
  ...
  case ComplexX87:
    ...
    ResType = toolchain::StructType::get(toolchain::Type::getX86_FP80Ty(getVMContext()),
                                    toolchain::Type::getX86_FP80Ty(getVMContext()));
    break;
  }

  toolchain::Type *HighPart = nullptr;
  switch (Hi) {
    // Memory was handled previously and X87 should
    // never occur as a hi class.
  case Memory:
  case X87:
    toolchain_unreachable("Invalid classification for hi word.");
...
void X86_64ABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  if (CallingConv == toolchain::CallingConv::Win64) {
    WinX86_64ABIInfo Win64ABIInfo(CGT, AVXLevel);
    Win64ABIInfo.computeInfo(FI);
    return;
  }

  bool IsRegCall = CallingConv == toolchain::CallingConv::X86_RegCall;
...
static Address EmitX86_64VAArgFromMemory(CodeGenFunction &CGF,
  ...
  toolchain::Value *overflow_arg_area =
  ...
  toolchain::Type *LTy = CGF.ConvertTypeForMem(Ty);
  toolchain::Value *Res = overflow_arg_area;
  ...
  toolchain::Value *Offset =
      toolchain::ConstantInt::get(CGF.Int32Ty, (SizeInBytes + 7)  & ~7);
...
RValue X86_64ABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  toolchain::Value *InRegs = nullptr;
  Address gp_offset_p = Address::invalid(), fp_offset_p = Address::invalid();
  toolchain::Value *gp_offset = nullptr, *fp_offset = nullptr;
  if (neededInt) {
    gp_offset_p = CGF.Builder.CreateStructGEP(VAListAddr, 0, "gp_offset_p");
    gp_offset = CGF.Builder.CreateLoad(gp_offset_p, "gp_offset");
    InRegs = toolchain::ConstantInt::get(CGF.Int32Ty, 48 - neededInt * 8);
  ...
  if (neededSSE) {
    ...
    toolchain::Value *FitsInFP =
      toolchain::ConstantInt::get(CGF.Int32Ty, 176 - neededSSE * 16);
    FitsInFP = CGF.Builder.CreateICmpULE(fp_offset, FitsInFP, "fits_in_fp");
    InRegs = InRegs ? CGF.Builder.CreateAnd(InRegs, FitsInFP) : FitsInFP;
  }

  toolchain::BasicBlock *InRegBlock = CGF.createBasicBlock("vaarg.in_reg");
  toolchain::BasicBlock *InMemBlock = CGF.createBasicBlock("vaarg.in_mem");
  ...
  toolchain::Type *LTy = CGF.ConvertTypeForMem(Ty);
  toolchain::Value *RegSaveArea = CGF.Builder.CreateLoad(
  ...
  if (neededInt && neededSSE) {
    ...
    toolchain::StructType *ST = cast<toolchain::StructType>(AI.getCoerceToType());
    Address Tmp = CGF.CreateMemTemp(Ty);
    Tmp = Tmp.withElementType(ST);
    assert(ST->getNumElements() == 2 && "Unexpected ABI info for mixed regs");
    toolchain::Type *TyLo = ST->getElementType(0);
    toolchain::Type *TyHi = ST->getElementType(1);
    assert((TyLo->isFPOrFPVectorTy() ^ TyHi->isFPOrFPVectorTy()) &&
           "Unexpected ABI info for mixed regs");
    toolchain::Value *GPAddr =
        CGF.Builder.CreateGEP(CGF.Int8Ty, RegSaveArea, gp_offset);
    toolchain::Value *FPAddr =
        CGF.Builder.CreateGEP(CGF.Int8Ty, RegSaveArea, fp_offset);
    toolchain::Value *RegLoAddr = TyLo->isFPOrFPVectorTy() ? FPAddr : GPAddr;
    toolchain::Value *RegHiAddr = TyLo->isFPOrFPVectorTy() ? GPAddr : FPAddr;

    // Copy the first element.
    // FIXME: Our choice of alignment here and below is probably pessimistic.
    toolchain::Value *V = CGF.Builder.CreateAlignedLoad(
        TyLo, RegLoAddr,
    ...
    toolchain::Type *CoTy = nullptr;
    if (AI.isDirect())
      CoTy = AI.getCoerceToType();

    toolchain::Value *GpOrFpOffset = neededInt ? gp_offset : fp_offset;
    ...
    if (CoTy && (AI.getDirectOffset() == 8 || RegSize < TySize)) {
      ...
      toolchain::Value *Src = CGF.Builder.CreateAlignedLoad(CoTy, Addr, TyAlign);
      toolchain::Value *PtrOffset =
          toolchain::ConstantInt::get(CGF.Int32Ty, AI.getDirectOffset());
      Address Dst = Address(
    ...
    toolchain::Type *ST = AI.canHaveCoerceToType()
                         ? AI.getCoerceToType()
                         : toolchain::StructType::get(CGF.DoubleTy, CGF.DoubleTy);
    toolchain::Value *V;
    Address Tmp = CGF.CreateMemTemp(Ty);
    Tmp = Tmp.withElementType(ST);
  ...
  if (neededInt) {
    toolchain::Value *Offset = toolchain::ConstantInt::get(CGF.Int32Ty, neededInt * 8);
    CGF.Builder.CreateStore(CGF.Builder.CreateAdd(gp_offset, Offset),
                            gp_offset_p);
  }
  if (neededSSE) {
    toolchain::Value *Offset = toolchain::ConstantInt::get(CGF.Int32Ty, neededSSE * 16);
...
RValue X86_64ABIInfo::EmitMSVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  bool IsIndirect = Width > 64 || !toolchain::isPowerOf2_64(Width);
...
ABIArgInfo WinX86_64ABIInfo::classify(QualType Ty, unsigned &FreeSSERegs,
  ...
  if (Ty->isMemberPointerType()) {
    ...
    // directly.
    toolchain::Type *LLTy = CGT.ConvertType(Ty);
  ...
  if (RT || Ty->isAnyComplexType() || Ty->isMemberPointerType()) {
    // MS x64 ABI requirement: "Any argument that doesn't fit in 8 bytes, or is
    // not 1, 2, 4, or 8 bytes, must be passed by reference."
    if (Width > 64 || !toolchain::isPowerOf2_64(Width))
      return getNaturalAlignIndirect(Ty, getDataLayout().getAllocaAddrSpace(),
                                     /*ByVal=*/false);

    // Otherwise, coerce it to a small integer.
    return ABIArgInfo::getDirect(toolchain::IntegerType::get(getVMContext(), Width));
  }
  ...
  if (const BuiltinType *BT = Ty->getAs<BuiltinType>()) {
    ...
    case BuiltinType::LongDouble:
      ...
      if (IsMingw64) {
        const toolchain::fltSemantics *LDF = &getTarget().getLongDoubleFormat();
        if (LDF == &toolchain::APFloat::x87DoubleExtended())
          return ABIArgInfo::getIndirect(
              Align, /*AddrSpace=*/getDataLayout().getAllocaAddrSpace(),
              /*ByVal=*/false);
      }
...
void WinX86_64ABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  bool IsVectorCall = CC == toolchain::CallingConv::X86_VectorCall;
  bool IsRegCall = CC == toolchain::CallingConv::X86_RegCall;

  // If __attribute__((sysv_abi)) is in use, use the SysV argument
  // classification rules.
  if (CC == toolchain::CallingConv::X86_64_SysV) {
