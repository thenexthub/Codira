...
static RValue complexTempStructure(CodeGenFunction &CGF, Address VAListAddr,
  ...
  toolchain::Type *EltTy = CGF.ConvertTypeForMem(CTy->getElementType());
  RealAddr = RealAddr.withElementType(EltTy);
  ImagAddr = ImagAddr.withElementType(EltTy);
  toolchain::Value *Real = CGF.Builder.CreateLoad(RealAddr, ".vareal");
  toolchain::Value *Imag = CGF.Builder.CreateLoad(ImagAddr, ".vaimag");

  return RValue::getComplex(Real, Imag);
}

static bool PPC_initDwarfEHRegSizeTable(CodeGen::CodeGenFunction &CGF,
                                        toolchain::Value *Address, bool Is64Bit,
                                        bool IsAIX) {
  ...
  toolchain::Value *Sixteen8 = toolchain::ConstantInt::get(i8, 16);
...
bool AIXTargetCodeGenInfo::initDwarfEHRegSizeTable(
    CodeGen::CodeGenFunction &CGF, toolchain::Value *Address) const {
...
void AIXTargetCodeGenInfo::setTargetAttributes(
  ...
  if (!isa<toolchain::GlobalVariable>(GV))
    return;

  auto *GVar = cast<toolchain::GlobalVariable>(GV);
  auto GVId = GV->getName();

  // Is this a global variable specified by the user as toc-data?
  bool UserSpecifiedTOC =
      toolchain::binary_search(M.getCodeGenOpts().TocDataVarsUserSpecified, GVId);
  // Assumes the same variable cannot be in both TocVarsUserSpecified and
  // NoTocVars.
  if (UserSpecifiedTOC ||
      ((M.getCodeGenOpts().AllTocData) &&
       !toolchain::binary_search(M.getCodeGenOpts().NoTocDataVars, GVId))) {
...
public:
  ...
  static bool isStructReturnInRegABI(const toolchain::Triple &Triple,
...
ABIArgInfo PPC32_SVR4_ABIInfo::classifyReturnType(QualType RetTy) const {
  ...
  if (isAggregateTypeForABI(RetTy) && IsRetSmallStructInRegABI &&
    ...
    if (Size == 0)
      ...
      toolchain::Type *CoerceTy = toolchain::Type::getIntNTy(getVMContext(), Size);
...
RValue PPC32_SVR4_ABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAList,
  ...
  toolchain::Value *NumRegs = Builder.CreateLoad(NumRegsAddr, "numUsedRegs");
  ...
  toolchain::BasicBlock *UsingRegs = CGF.createBasicBlock("using_regs");
  toolchain::BasicBlock *UsingOverflow = CGF.createBasicBlock("using_overflow");
  toolchain::BasicBlock *Cont = CGF.createBasicBlock("cont");

  Builder.CreateCondBr(CC, UsingRegs, UsingOverflow);

  toolchain::Type *DirectTy = CGF.ConvertType(Ty), *ElementTy = DirectTy;
  if (isIndirect)
  ...
  Address MemAddr = Address::invalid();
  {
    ...
    if (Align > OverflowAreaAlign) {
      toolchain::Value *Ptr = OverflowArea.emitRawPointer(CGF);
...
public:
  ...
  void emitTargetMetadata(CodeGen::CodeGenModule &CGM,
                          const toolchain::MapVector<GlobalDecl, StringRef>
...
PPC64_SVR4_ABIInfo::classifyArgumentType(QualType Ty) const {
  ...
  if (Ty->isVectorType()) {
    ...
    if (Size > 128)
      ...
      toolchain::Type *CoerceTy = toolchain::IntegerType::get(getVMContext(), Size);
      return ABIArgInfo::getDirect(CoerceTy);
    }
  }
  ...
  if (isAggregateTypeForABI(Ty)) {
    ...
    if (Kind == PPC64_SVR4_ABIKind::ELFv2 &&
      ...
      toolchain::Type *BaseTy = CGT.ConvertType(QualType(Base, 0));
      toolchain::Type *CoerceTy = toolchain::ArrayType::get(BaseTy, Members);
      return ABIArgInfo::getDirect(CoerceTy);
    }
    ...
    if (Bits > 0 && Bits <= 8 * GPRBits) {
      ...
      // according to the required alignment in the save area.
        ...
        uint64_t NumRegs = toolchain::alignTo(Bits, RegBits) / RegBits;
        toolchain::Type *RegTy = toolchain::IntegerType::get(getVMContext(), RegBits);
        CoerceTy = toolchain::ArrayType::get(RegTy, NumRegs);
      }
...
PPC64_SVR4_ABIInfo::classifyReturnType(QualType RetTy) const {
  ...
  if (isAggregateTypeForABI(RetTy)) {
    ...
    if (Kind == PPC64_SVR4_ABIKind::ELFv2 && Bits <= 2 * GPRBits) {
      ...
      toolchain::Type *CoerceTy;
      if (Bits > GPRBits) {
        CoerceTy = toolchain::IntegerType::get(getVMContext(), GPRBits);
        CoerceTy = toolchain::StructType::get(CoerceTy, CoerceTy);
      } else
        CoerceTy =
            toolchain::IntegerType::get(getVMContext(), toolchain::alignTo(Bits, 8));
      return ABIArgInfo::getDirect(CoerceTy);
...
void PPC64_SVR4_TargetCodeGenInfo::emitTargetMetadata(
  ...
  if (CGM.getTypes().isLongDoubleReferenced()) {
    ...
    if (flt == &toolchain::APFloat::PPCDoubleDouble())
      CGM.getModule().addModuleFlag(toolchain::Module::Error, "float-abi",
                                    toolchain::MDString::get(Ctx, "doubledouble"));
    else if (flt == &toolchain::APFloat::IEEEquad())
      CGM.getModule().addModuleFlag(toolchain::Module::Error, "float-abi",
                                    toolchain::MDString::get(Ctx, "ieeequad"));
    else if (flt == &toolchain::APFloat::IEEEdouble())
      CGM.getModule().addModuleFlag(toolchain::Module::Error, "float-abi",
                                    toolchain::MDString::get(Ctx, "ieeedouble"));
  }
}
