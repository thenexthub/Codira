...
#include "language/Core/AST/OSLog.h"
...
static Value *EmitTargetArchBuiltinExpr(CodeGenFunction *CGF,
  ...
  case toolchain::Triple::spirv64:
    if (CGF->getTarget().getTriple().getOS() == toolchain::Triple::OSType::AMDHSA)
      return CGF->EmitAMDGPUBuiltinExpr(BuiltinID, E);
    [[fallthrough]];
...
static void initializeAlloca(CodeGenFunction &CGF, AllocaInst *AI, Value *Size,
  ...
  case LangOptions::TrivialAutoVarInitKind::Pattern: {
    toolchain::Type *Int8 = toolchain::IntegerType::getInt8Ty(CGF.CGM.getLLVMContext());
    Byte = toolchain::dyn_cast<toolchain::ConstantInt>(
...
toolchain::Constant *CodeGenModule::getBuiltinLibFunction(const FunctionDecl *FD,
                                                     unsigned BuiltinID) {
  ...
  if (FD->hasAttr<AsmLabelAttr>())
    ...
    if (getTriple().isPPC64() &&
        &getTarget().getLongDoubleFormat() == &toolchain::APFloat::IEEEquad() &&
        F128Builtins.contains(BuiltinID))
      Name = F128Builtins[BuiltinID];
    else if (getTriple().isOSAIX() &&
             &getTarget().getLongDoubleFormat() ==
                 &toolchain::APFloat::IEEEdouble() &&
  ...
  toolchain::FunctionType *Ty =
    cast<toolchain::FunctionType>(getTypes().ConvertType(FD->getType()));
...
Value *EmitToInt(CodeGenFunction &CGF, toolchain::Value *V,
                        QualType T, toolchain::IntegerType *IntType) {
...
Value *EmitFromInt(CodeGenFunction &CGF, toolchain::Value *V,
                          QualType T, toolchain::Type *ResultType) {
...
Value *MakeBinaryAtomicValue(
    CodeGenFunction &CGF, toolchain::AtomicRMWInst::BinOp Kind, const CallExpr *E,
  ...
  toolchain::IntegerType *IntType = toolchain::IntegerType::get(
      CGF.getLLVMContext(), CGF.getContext().getTypeSize(T));

  toolchain::Value *Val = CGF.EmitScalarExpr(E->getArg(1));
  toolchain::Type *ValueType = Val->getType();
...
static RValue EmitBinaryAtomicPost(CodeGenFunction &CGF,
  ...
  toolchain::Value *Result = CGF.Builder.CreateAtomicRMW(
      Kind, DestAddr, Val, toolchain::AtomicOrdering::SequentiallyConsistent);
  Result = CGF.Builder.CreateBinOp(Op, Result, Val);
  if (Invert)
    Result =
        CGF.Builder.CreateBinOp(toolchain::Instruction::Xor, Result,
                                toolchain::ConstantInt::getAllOnesValue(IntType));
...
///                               T Comparand);
///
/// Whereas the toolchain 'cmpxchg' instruction has the following syntax:
...
static Value *EmitAtomicCmpXchg128ForMSIntrin(CodeGenFunction &CGF,
  ...
  toolchain::Value *DestPtr = CGF.EmitScalarExpr(E->getArg(0));
  toolchain::Value *ExchangeHigh = CGF.EmitScalarExpr(E->getArg(1));
  toolchain::Value *ExchangeLow = CGF.EmitScalarExpr(E->getArg(2));
  ...
  // destination pointer.
  toolchain::Type *Int128Ty = toolchain::IntegerType::get(CGF.getLLVMContext(), 128);
  ...
  ExchangeHigh =
      CGF.Builder.CreateShl(ExchangeHigh, toolchain::ConstantInt::get(Int128Ty, 64));
  toolchain::Value *Exchange = CGF.Builder.CreateOr(ExchangeHigh, ExchangeLow);

  // Load the comparand for the instruction.
  toolchain::Value *Comparand = CGF.Builder.CreateLoad(ComparandAddr);
...
static Value *EmitISOVolatileLoad(CodeGenFunction &CGF, const CallExpr *E) {
  ...
  toolchain::Type *ITy =
      toolchain::IntegerType::get(CGF.getLLVMContext(), LoadSize.getQuantity() * 8);
  toolchain::LoadInst *Load = CGF.Builder.CreateAlignedLoad(ITy, Ptr, LoadSize);
...
Value *emitUnaryMaybeConstrainedFPBuiltin(CodeGenFunction &CGF,
  ...
  toolchain::Value *Src0 = CGF.EmitScalarExpr(E->getArg(0));
...
static Value *emitBinaryMaybeConstrainedFPBuiltin(CodeGenFunction &CGF,
                                ...
                                unsigned ConstrainedIntrinsicID) {
  toolchain::Value *Src0 = CGF.EmitScalarExpr(E->getArg(0));
  toolchain::Value *Src1 = CGF.EmitScalarExpr(E->getArg(1));
...
static Value *emitTernaryMaybeConstrainedFPBuiltin(CodeGenFunction &CGF,
  ...
  toolchain::Value *Src0 = CGF.EmitScalarExpr(E->getArg(0));
  toolchain::Value *Src1 = CGF.EmitScalarExpr(E->getArg(1));
  toolchain::Value *Src2 = CGF.EmitScalarExpr(E->getArg(2));
...
static Value *emitFrexpBuiltin(CodeGenFunction &CGF, const CallExpr *E,
  ...
  toolchain::Type *IntTy = CGF.ConvertType(IntPtrTy);
  toolchain::Function *F =
      CGF.CGM.getIntrinsic(IntrinsicID, {Src0->getType(), IntTy});
  toolchain::Value *Call = CGF.Builder.CreateCall(F, Src0);

  toolchain::Value *Exp = CGF.Builder.CreateExtractValue(Call, 1);
...
static void emitSincosBuiltin(CodeGenFunction &CGF, const CallExpr *E,
                              Intrinsic::ID IntrinsicID) {
  toolchain::Value *Val = CGF.EmitScalarExpr(E->getArg(0));
  toolchain::Value *Dest0 = CGF.EmitScalarExpr(E->getArg(1));
  toolchain::Value *Dest1 = CGF.EmitScalarExpr(E->getArg(2));

  toolchain::Function *F = CGF.CGM.getIntrinsic(IntrinsicID, {Val->getType()});
  toolchain::Value *Call = CGF.Builder.CreateCall(F, Val);

  toolchain::Value *SinResult = CGF.Builder.CreateExtractValue(Call, 0);
  toolchain::Value *CosResult = CGF.Builder.CreateExtractValue(Call, 1);
  ...
  toolchain::StoreInst *StoreSin =
      CGF.Builder.CreateStore(SinResult, SinLV.getAddress());
  toolchain::StoreInst *StoreCos =
  ...
  toolchain::MDBuilder MDHelper(CGF.getLLVMContext());
...
static toolchain::Value *emitModfBuiltin(CodeGenFunction &CGF, const CallExpr *E,
                                    Intrinsic::ID IntrinsicID) {
  toolchain::Value *Val = CGF.EmitScalarExpr(E->getArg(0));
  toolchain::Value *IntPartDest = CGF.EmitScalarExpr(E->getArg(1));

  toolchain::Value *Call =
      CGF.Builder.CreateIntrinsic(IntrinsicID, {Val->getType()}, Val);

  toolchain::Value *FractionalResult = CGF.Builder.CreateExtractValue(Call, 0);
  toolchain::Value *IntegralResult = CGF.Builder.CreateExtractValue(Call, 1);
...
/// EmitFAbs - Emit a call to @toolchain.fabs().
static Value *EmitFAbs(CodeGenFunction &CGF, Value *V) {
  Function *F = CGF.CGM.getIntrinsic(Intrinsic::fabs, V->getType());
  toolchain::CallInst *Call = CGF.Builder.CreateCall(F, V);
...
static Value *EmitSignBit(CodeGenFunction &CGF, Value *V) {
  ...
  toolchain::Type *Ty = V->getType();
  int Width = Ty->getPrimitiveSizeInBits();
  toolchain::Type *IntTy = toolchain::IntegerType::get(C, Width);
  ...
  if (Ty->isPPC_FP128Ty()) {
    ...
    if (CGF.getTarget().isBigEndian()) {
      Value *ShiftCst = toolchain::ConstantInt::get(IntTy, Width);
      V = CGF.Builder.CreateLShr(V, ShiftCst);
    }
    // We are truncating value in order to extract the higher-order
    // double, which we will be using to extract the sign from.
    IntTy = toolchain::IntegerType::get(C, Width);
    V = CGF.Builder.CreateTrunc(V, IntTy);
  }
  Value *Zero = toolchain::Constant::getNullValue(IntTy);
...
static bool HasNoIndirectArgumentsOrResults(CGFunctionInfo const &FnInfo) {
  ...
  return !IsIndirect(FnInfo.getReturnInfo()) &&
         toolchain::none_of(FnInfo.arguments(),
...
static RValue emitLibraryCall(CodeGenFunction &CGF, const FunctionDecl *FD,
                              const CallExpr *E, toolchain::Constant *calleeValue) {
  CodeGenFunction::CGFPOptionsRAII FPOptsRAII(CGF, E);
  ...
  if (unsigned BuiltinID = FD->getBuiltinID()) {
    ...
    if (ConstWithoutErrnoAndExceptions && CGF.CGM.getLangOpts().MathErrno &&
      ...
      language::Core::QualType IntTy = Context.IntTy;
...
/// Emit a call to toolchain.{sadd,uadd,ssub,usub,smul,umul}.with.overflow.*
/// depending on IntrinsicID.
///
/// \arg CGF The current codegen function.
/// \arg IntrinsicID The ID for the Intrinsic we wish to generate.
/// \arg X The first argument to the toolchain.*.with.overflow.*.
/// \arg Y The second argument to the toolchain.*.with.overflow.*.
/// \arg Carry The carry returned by the toolchain.*.with.overflow.*.
/// \returns The result (i.e. sum/product) returned by the intrinsic.
toolchain::Value *EmitOverflowIntrinsic(CodeGenFunction &CGF,
                                   const Intrinsic::ID IntrinsicID,
                                   toolchain::Value *X, toolchain::Value *Y,
  ...
  toolchain::Value *Tmp = CGF.Builder.CreateCall(Callee, {X, Y});
...
getIntegerWidthAndSignedness(const language::Core::ASTContext &context,
...
getDefaultBuiltinObjectSizeResult(unsigned Type, toolchain::IntegerType *ResType) {
...
public:
  ...
  const Expr *VisitUnaryAddrOf(const language::Core::UnaryOperator *E) {
    AddrOfSeen = true;
    return Visit(E->getSubExpr());
  }
  const Expr *VisitUnaryDeref(const language::Core::UnaryOperator *E) {
    AddrOfSeen = false;
    return Visit(E->getSubExpr());
  }
  const Expr *VisitBinaryOperator(const language::Core::BinaryOperator *Op) {
...
toolchain::Value *CodeGenFunction::emitCountedBySize(const Expr *E,
                                                toolchain::Value *EmittedE,
                                                unsigned Type,
                                                toolchain::IntegerType *ResType) {
...
static toolchain::Value *EmitPositiveResultOrZero(CodeGenFunction &CGF,
                                             toolchain::Value *Res,
...
GetCountFieldAndIndex(CodeGenFunction &CGF, const MemberExpr *ME,
                      ...
                      const Expr *Idx, toolchain::IntegerType *ResType,
                      bool IsSigned) {
  //  count = ptr->count;
...
toolchain::Value *CodeGenFunction::emitCountedByPointerSize(
    const ImplicitCastExpr *E, const Expr *Idx, toolchain::Value *EmittedE,
    QualType CastedArrayElementTy, unsigned Type, toolchain::IntegerType *ResType) {
  ...
  //  array_element_size = sizeof (*ptr->array)
  auto *ArrayElementSize = toolchain::ConstantInt::get(
      ResType, ArrayElementBaseSize->getQuantity(), IsSigned);

  //  casted_array_element_size = sizeof (*((cast) ptr->array));
  auto *CastedArrayElementSize = toolchain::ConstantInt::get(
...
toolchain::Value *CodeGenFunction::emitCountedByMemberSize(
    const MemberExpr *ME, const Expr *Idx, toolchain::Value *EmittedE,
    QualType CastedArrayElementTy, unsigned Type, toolchain::IntegerType *ResType) {
  ...
  if (FlexibleArrayMemberFD != FD) {
    ...
    if (!Offset)
      return nullptr;
    FieldOffset =
        toolchain::ConstantInt::get(ResType, *Offset / CharWidth, IsSigned);
  }
  ...
  auto *FlexibleArrayMemberElementSize =
      toolchain::ConstantInt::get(ResType, BaseSize.getQuantity(), IsSigned);
  ...
  if (FlexibleArrayMemberFD == FD) {
    if (Idx) { // Option (2) '&((cast) ptr->array)[idx]'
      //  casted_flexible_array_member_element_size =
      //      sizeof (*((cast) ptr->array));
      toolchain::ConstantInt *CastedFlexibleArrayMemberElementSize =
    ...
    //  sizeof_struct = sizeof (struct s);
    toolchain::StructType *StructTy = getTypes().getCGRecordLayout(RD).getLLVMType();
    const toolchain::DataLayout &Layout = CGM.getDataLayout();
    TypeSize Size = Layout.getTypeSizeInBits(StructTy);
    Value *SizeofStruct =
        toolchain::ConstantInt::get(ResType, Size.getKnownMinValue() / CharWidth);
    ...
    if (Idx) { // Option (4) '&((cast) ptr->field_array)[idx]'
      ...
      toolchain::ConstantInt *CastedFieldElementSize =
          toolchain::ConstantInt::get(ResType, BaseSize.getQuantity(), IsSigned);
...
///   - A toolchain::Argument (if E is a param with the pass_object_size attribute on
///     it)
///   - A call to the @toolchain.objectsize intrinsic
///
/// EmittedE is the result of emitting `E` as a scalar expr. If it's non-null
/// and we wouldn't otherwise try to reference a pass_object_size parameter,
/// we'll call @toolchain.objectsize on EmittedE, rather than emitting E.
toolchain::Value *
...
BitTest BitTest::decodeBitTestBuiltin(unsigned BuiltinID) {
  ...
  toolchain_unreachable("expected only bittest intrinsics");
}
...
static toolchain::Value *EmitX86BitTestIntrinsic(CodeGenFunction &CGF,
                                            BitTest BT,
  ...
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGF.Int8Ty, {CGF.UnqualPtrTy, IntType}, false);

  toolchain::InlineAsm *IA =
      toolchain::InlineAsm::get(FTy, Asm, Constraints, /*hasSideEffects=*/true);
...
getBitTestAtomicOrdering(BitTest::InterlockingKind I) {
  switch (I) {
  case BitTest::Unlocked:   return toolchain::AtomicOrdering::NotAtomic;
  case BitTest::Sequential: return toolchain::AtomicOrdering::SequentiallyConsistent;
  case BitTest::Acquire:    return toolchain::AtomicOrdering::Acquire;
  case BitTest::Release:    return toolchain::AtomicOrdering::Release;
  case BitTest::NoFence:    return toolchain::AtomicOrdering::Monotonic;
  }
  toolchain_unreachable("invalid interlocking");
}
...
static toolchain::Value *EmitBitTestIntrinsic(CodeGenFunction &CGF,
                                         unsigned BuiltinID,
                                         const CallExpr *E) {
  ...
  Value *ByteIndex = CGF.Builder.CreateAShr(
      BitPos, toolchain::ConstantInt::get(BitPos->getType(), 3), "bittest.byteidx");
  ...
  Value *PosLow =
      CGF.Builder.CreateAnd(CGF.Builder.CreateTrunc(BitPos, CGF.Int8Ty),
                            toolchain::ConstantInt::get(CGF.Int8Ty, 0x7));

  // The updating instructions will need a mask.
  Value *Mask = nullptr;
  if (BT.Action != BitTest::TestOnly) {
    Mask = CGF.Builder.CreateShl(toolchain::ConstantInt::get(CGF.Int8Ty, 1), PosLow,
                                 "bittest.mask");
  }

  // Check the action and ordering of the interlocked intrinsics.
  toolchain::AtomicOrdering Ordering = getBitTestAtomicOrdering(BT.Interlocking);

  Value *OldByte = nullptr;
  if (Ordering != toolchain::AtomicOrdering::NotAtomic) {
    // Emit a combined atomicrmw load/store operation for the interlocked
    // intrinsics.
    toolchain::AtomicRMWInst::BinOp RMWOp = toolchain::AtomicRMWInst::Or;
    if (BT.Action == BitTest::Reset) {
      Mask = CGF.Builder.CreateNot(Mask);
      RMWOp = toolchain::AtomicRMWInst::And;
    }
  ...
  return CGF.Builder.CreateAnd(
      ShiftedByte, toolchain::ConstantInt::get(CGF.Int8Ty, 1), "bittest.res");
}
...
static RValue EmitMSVCRTSetJmp(CodeGenFunction &CGF, MSVCSetJmpKind SJKind,
                               const CallExpr *E) {
  toolchain::Value *Arg1 = nullptr;
  toolchain::Type *Arg1Ty = nullptr;
  StringRef Name;
  bool IsVarArg = false;
  if (SJKind == MSVCSetJmpKind::_setjmp3) {
    Name = "_setjmp3";
    Arg1Ty = CGF.Int32Ty;
    Arg1 = toolchain::ConstantInt::get(CGF.IntTy, 0);
    IsVarArg = true;
  } else {
    Name = SJKind == MSVCSetJmpKind::_setjmp ? "_setjmp" : "_setjmpex";
    Arg1Ty = CGF.Int8PtrTy;
    if (CGF.getTarget().getTriple().getArch() == toolchain::Triple::aarch64) {
      ...
      Arg1 = CGF.Builder.CreateCall(
          ...
          toolchain::ConstantInt::get(CGF.Int32Ty, 0));
  }

  // Mark the call site and declaration with ReturnsTwice.
  toolchain::Type *ArgTypes[2] = {CGF.Int8PtrTy, Arg1Ty};
  toolchain::AttributeList ReturnsTwiceAttr = toolchain::AttributeList::get(
      CGF.getLLVMContext(), toolchain::AttributeList::FunctionIndex,
      toolchain::Attribute::ReturnsTwice);
  toolchain::FunctionCallee SetJmpFn = CGF.CGM.CreateRuntimeFunction(
      toolchain::FunctionType::get(CGF.IntTy, ArgTypes, IsVarArg), Name,
      ReturnsTwiceAttr, /*Local=*/true);

  toolchain::Value *Buf = CGF.Builder.CreateBitOrPointerCast(
      CGF.EmitScalarExpr(E->getArg(0)), CGF.Int8PtrTy);
  toolchain::Value *Args[] = {Buf, Arg1};
  toolchain::CallBase *CB = CGF.EmitRuntimeCallOrInvoke(SetJmpFn, Args);
...
Value *CodeGenFunction::EmitMSVCBuiltinExpr(MSVCIntrin BuiltinID,
  ...
  case MSVCIntrin::_BitScanReverse: {
    ...
    toolchain::Type *IndexType = IndexAddress.getElementType();
    toolchain::Type *ResultType = ConvertType(E->getType());

    Value *ArgZero = toolchain::Constant::getNullValue(ArgType);
    Value *ResZero = toolchain::Constant::getNullValue(ResultType);
    Value *ResOne = toolchain::ConstantInt::get(ResultType, 1);
    ...
    if (BuiltinID == MSVCIntrin::_BitScanForward) {
      ...
      unsigned ArgWidth = cast<toolchain::IntegerType>(ArgType)->getBitWidth();
      Value *ArgTypeLastIndex = toolchain::ConstantInt::get(IndexType, ArgWidth - 1);
  ...
  case MSVCIntrin::__fastfail: {
    ...
    toolchain::Triple::ArchType ISA = getTarget().getTriple().getArch();
    ...
    toolchain::FunctionType *FTy = toolchain::FunctionType::get(VoidTy, {Int32Ty}, false);
    toolchain::InlineAsm *IA =
        toolchain::InlineAsm::get(FTy, Asm, Constraints, /*hasSideEffects=*/true);
    toolchain::AttributeList NoReturnAttr = toolchain::AttributeList::get(
        getLLVMContext(), toolchain::AttributeList::FunctionIndex,
        toolchain::Attribute::NoReturn);
    toolchain::CallInst *CI = Builder.CreateCall(IA, EmitScalarExpr(E->getArg(0)));
    CI->setAttributes(NoReturnAttr);
    return CI;
  }
  }
  toolchain_unreachable("Incorrect MSVC intrinsic!");
}

namespace {
// ARC cleanup for __builtin_os_log_format
struct CallObjCArcUse final : EHScopeStack::Cleanup {
  CallObjCArcUse(toolchain::Value *object) : object(object) {}
...
Value *CodeGenFunction::EmitCheckedArgForBuiltin(const Expr *E,
  ...
  Value *Cond = Builder.CreateICmpNE(
      ArgValue, toolchain::Constant::getNullValue(ArgValue->getType()));
  EmitCheck(std::make_pair(Cond, CheckOrdinal), CheckHandler,
            {EmitCheckSourceLocation(E->getExprLoc()),
             toolchain::ConstantInt::get(Builder.getInt8Ty(), Kind)},
            {});
  return ArgValue;
}
...
Value *CodeGenFunction::EmitCheckedArgForAssume(const Expr *E) {
  ...
  EmitCheck(
      ...
      {EmitCheckSourceLocation(E->getExprLoc()),
       toolchain::ConstantInt::get(Builder.getInt8Ty(), BCK_AssumePassedFalse)},
      {});
  return ArgValue;
}
...
static Value *EmitOverflowCheckedAbs(CodeGenFunction &CGF, const CallExpr *E,
  ...
  if (const auto *VCI = dyn_cast<toolchain::ConstantInt>(ArgValue)) {
...
toolchain::Function *CodeGenFunction::generateBuiltinOSLogHelperFunction(
  ...
  if (toolchain::Function *F = CGM.getModule().getFunction(Name))
    return F;

  toolchain::SmallVector<QualType, 4> ArgTys;
  ...
  for (unsigned int I = 0, E = Layout.Items.size(); I < E; ++I) {
    ...
    Args.push_back(ImplicitParamDecl::Create(
        ...
        &Ctx.Idents.get(std::string("arg") + toolchain::to_string(I)), ArgTy,
        ImplicitParamKind::Other));
  ...
  toolchain::FunctionType *FuncTy = CGM.getTypes().GetFunctionType(FI);
  toolchain::Function *Fn = toolchain::Function::Create(
      FuncTy, toolchain::GlobalValue::LinkOnceODRLinkage, Name, &CGM.getModule());
  Fn->setVisibility(toolchain::GlobalValue::HiddenVisibility);
...
RValue CodeGenFunction::emitBuiltinOSLogFormat(const CallExpr &E) {
  ...
  for (const auto &Item : Layout.Items) {
    ...
    if (Item.getKind() == analyze_os_log::OSLogBufferItem::MaskKind) {
      ...
      ArgVal = toolchain::Constant::getIntegerValue(Int64Ty, toolchain::APInt(64, Val));
    ...
    toolchain::IntegerType *IntTy = toolchain::Type::getIntNTy(getLLVMContext(),
                                                     ArgValSize);
  ...
  toolchain::Function *F = CodeGenFunction(CGM).generateBuiltinOSLogHelperFunction(
...
static RValue EmitCheckedUnsignedMultiplySignedResult(
    CodeGenFunction &CGF, const language::Core::Expr *Op1, WidthAndSignedness Op1Info,
    const language::Core::Expr *Op2, WidthAndSignedness Op2Info,
    const language::Core::Expr *ResultArg, QualType ResultQTy,
    WidthAndSignedness ResultInfo) {
  ...
  toolchain::Value *V1 = CGF.EmitScalarExpr(Op1);
  toolchain::Value *V2 = CGF.EmitScalarExpr(Op2);

  toolchain::Value *HasOverflow;
  toolchain::Value *Result = EmitOverflowIntrinsic(
  ...
  auto IntMax = toolchain::APInt::getSignedMaxValue(ResultInfo.Width);
  toolchain::Value *IntMaxValue = toolchain::ConstantInt::get(Result->getType(), IntMax);

  toolchain::Value *IntMaxOverflow = CGF.Builder.CreateICmpUGT(Result, IntMaxValue);
...
static RValue
EmitCheckedMixedSignMultiply(CodeGenFunction &CGF, const language::Core::Expr *Op1,
                             WidthAndSignedness Op1Info, const language::Core::Expr *Op2,
                             WidthAndSignedness Op2Info,
                             const language::Core::Expr *ResultArg, QualType ResultQTy,
                             WidthAndSignedness ResultInfo) {
  ...
  const language::Core::Expr *SignedOp = Op1Info.Signed ? Op1 : Op2;
  const language::Core::Expr *UnsignedOp = Op1Info.Signed ? Op2 : Op1;
  toolchain::Value *Signed = CGF.EmitScalarExpr(SignedOp);
  toolchain::Value *Unsigned = CGF.EmitScalarExpr(UnsignedOp);
  ...
  toolchain::Type *OpTy = Signed->getType();
  toolchain::Value *Zero = toolchain::Constant::getNullValue(OpTy);
  Address ResultPtr = CGF.EmitPointerWithAlignment(ResultArg);
  toolchain::Type *ResTy = CGF.getTypes().ConvertType(ResultQTy);
  unsigned OpWidth = std::max(Op1Info.Width, Op2Info.Width);

  // Take the absolute value of the signed operand.
  toolchain::Value *IsNegative = CGF.Builder.CreateICmpSLT(Signed, Zero);
  toolchain::Value *AbsOfNegative = CGF.Builder.CreateSub(Zero, Signed);
  toolchain::Value *AbsSigned =
      CGF.Builder.CreateSelect(IsNegative, AbsOfNegative, Signed);

  // Perform a checked unsigned multiplication.
  toolchain::Value *UnsignedOverflow;
  toolchain::Value *UnsignedResult =
      EmitOverflowIntrinsic(CGF, Intrinsic::umul_with_overflow, AbsSigned,
                            Unsigned, UnsignedOverflow);

  toolchain::Value *Overflow, *Result;
  if (ResultInfo.Signed) {
    // Signed overflow occurs if the result is greater than INT_MAX or lesser
    // than INT_MIN, i.e when |Result| > (INT_MAX + IsNegative).
    auto IntMax =
        toolchain::APInt::getSignedMaxValue(ResultInfo.Width).zext(OpWidth);
    toolchain::Value *MaxResult =
        CGF.Builder.CreateAdd(toolchain::ConstantInt::get(OpTy, IntMax),
                              CGF.Builder.CreateZExt(IsNegative, OpTy));
    toolchain::Value *SignedOverflow =
    ...
    toolchain::Value *NegativeResult = CGF.Builder.CreateNeg(UnsignedResult);
    toolchain::Value *SignedResult =
    ...
    toolchain::Value *Underflow = CGF.Builder.CreateAnd(
        IsNegative, CGF.Builder.CreateIsNotNull(UnsignedResult));
    Overflow = CGF.Builder.CreateOr(UnsignedOverflow, Underflow);
    if (ResultInfo.Width < OpWidth) {
      auto IntMax =
          toolchain::APInt::getMaxValue(ResultInfo.Width).zext(OpWidth);
      toolchain::Value *TruncOverflow = CGF.Builder.CreateICmpUGT(
          UnsignedResult, toolchain::ConstantInt::get(OpTy, IntMax));
...
TypeRequiresBuiltinLaunderImp(const ASTContext &Ctx, QualType Ty,
                              toolchain::SmallPtrSetImpl<const Decl *> &Seen) {
...
static bool TypeRequiresBuiltinLaunder(CodeGenModule &CGM, QualType Ty) {
  if (!CGM.getCodeGenOpts().StrictVTablePointers)
    return false;
  toolchain::SmallPtrSet<const Decl *, 16> Seen;
...
RValue CodeGenFunction::emitRotate(const CallExpr *E, bool IsRotateRight) {
  toolchain::Value *Src = EmitScalarExpr(E->getArg(0));
  toolchain::Value *ShiftAmt = EmitScalarExpr(E->getArg(1));

  // The builtin's shift arg may have a different type than the source arg and
  // result, but the LLVM intrinsic uses the same type for all values.
  toolchain::Type *Ty = Src->getType();
...
static RValue EmitHipStdParUnsupportedBuiltin(CodeGenFunction *CGF,
  ...
  for (auto &&FormalTy : FnTy->params())
    Args.push_back(toolchain::PoisonValue::get(FormalTy));
...
RValue CodeGenFunction::EmitBuiltinExpr(const GlobalDecl GD, unsigned BuiltinID,
  ...
  if (E->isPRValue() && E->EvaluateAsRValue(Result, CGM.getContext()) &&
    ...
    if (Result.Val.isInt())
      return RValue::get(toolchain::ConstantInt::get(getLLVMContext(),
                                                Result.Val.getInt()));
    if (Result.Val.isFloat())
      return RValue::get(toolchain::ConstantFP::get(getLLVMContext(),
                                               Result.Val.getFloat()));
  }
  ...
  if (GenerateIntrinsics) {
    ...
    case Builtin::BI__builtin_elementwise_sqrt: {
      toolchain::Value *Call = emitUnaryMaybeConstrainedFPBuiltin(
  ...
  case Builtin::BI__builtin_clrsbll: {
    ...
    toolchain::Type *ResultType = ConvertType(E->getType());
    Value *Zero = toolchain::Constant::getNullValue(ArgType);
    ...
    Value *Result = Builder.CreateSub(Ctlz, toolchain::ConstantInt::get(ArgType, 1));
  ...
  case Builtin::BI__builtin_ffsll: {
    ...
    Value *Tmp =
        Builder.CreateAdd(Builder.CreateCall(F, {ArgValue, Builder.getTrue()}),
                          toolchain::ConstantInt::get(ArgType, 1));
    Value *Zero = toolchain::Constant::getNullValue(ArgType);
    Value *IsZero = Builder.CreateICmpEQ(ArgValue, Zero, "iszero");
  ...
  case Builtin::BI__builtin_parityll: {
    ...
    Value *Result = Builder.CreateAnd(Tmp, toolchain::ConstantInt::get(ArgType, 1));
    if (Result->getType() != ResultType)
      Result = Builder.CreateIntCast(Result, ResultType, /*isSigned*/true,
                                     "cast");
    return RValue::get(Result);
  ...
  case Builtin::BI__builtin_expect: {
    ...
    // Don't generate toolchain.expect on -O0 as the backend won't use it for
    // anything.
  ...
  case Builtin::BI__builtin_expect_with_probability: {
    ...
    Value *ExpectedValue = EmitScalarExpr(E->getArg(1));
    toolchain::APFloat Probability(0.0);
    ...
    (void)EvalSucceed;
    bool LoseInfo = false;
    Probability.convert(toolchain::APFloat::IEEEdouble(),
                        toolchain::RoundingMode::Dynamic, &LoseInfo);
    toolchain::Type *Ty = ConvertType(ProbArg->getType());
    Constant *Confidence = ConstantFP::get(Ty, Probability);
    // Don't generate toolchain.expect.with.probability on -O0 as the backend
  ...
  case Builtin::BI__builtin_assume_aligned: {
    ...
    if (AlignmentCI->getValue().ugt(toolchain::Value::MaximumAlignment))
      AlignmentCI = ConstantInt::get(AlignmentCI->getIntegerType(),
                                     toolchain::Value::MaximumAlignment);
  ...
  case Builtin::BI__builtin_allow_runtime_check: {
    ...
    toolchain::Value *Allow = Builder.CreateCall(
        ...
        toolchain::MetadataAsValue::get(Ctx, toolchain::MDString::get(Ctx, Kind)));
  ...
  case Builtin::BI__arithmetic_fence: {
    ...
    CodeGenFunction::CGFPOptionsRAII FPOptsRAII(*this, E);
    toolchain::FastMathFlags FMF = Builder.getFastMathFlags();
  ...
  case Builtin::BI__builtin_object_size: {
    ...
    auto *ResType = cast<toolchain::IntegerType>(ConvertType(E->getType()));
  ...
  case Builtin::BI__builtin_counted_by_ref: {
    ...
    toolchain::Value *Result = toolchain::ConstantPointerNull::get(
    ...
    if (const MemberExpr *ME = dyn_cast_if_present<MemberExpr>(Arg)) {
      if (auto *CATy =
        ...
        if (const FieldDecl *CountFD = FAMDecl->findCountedByField())
          ...
          toolchain::report_fatal_error("Cannot find the counted_by 'count' field");
      }
    }
  ...
  case Builtin::BI__builtin_prefetch: {
    ...
    Locality = (E->getNumArgs() > 2) ? EmitScalarExpr(E->getArg(2)) :
      toolchain::ConstantInt::get(Int32Ty, 3);
  ...
  case Builtin::BI__builtin_powil: {
    toolchain::Value *Src0 = EmitScalarExpr(E->getArg(0));
    toolchain::Value *Src1 = EmitScalarExpr(E->getArg(1));

    if (Builder.getIsFPConstrained()) {
      // FIXME: toolchain.powi has 2 mangling types,
      // toolchain.experimental.constrained.powi has one.
      CodeGenFunction::CGFPOptionsRAII FPOptsRAII(*this, E);
  ...
  case Builtin::BI__builtin_frexpl: {
    ...
    if (&getTarget().getLongDoubleFormat() == &toolchain::APFloat::PPCDoubleDouble())
      break;
    [[fallthrough]];
  }
  ...
  case Builtin::BI__builtin_isunordered: {
    ...
    switch (BuiltinID) {
    default: toolchain_unreachable("Unknown ordered comparison");
    case Builtin::BI__builtin_isgreater:
  ...
  case Builtin::BI__builtin_nondeterministic_value: {
    toolchain::Type *Ty = ConvertType(E->getArg(0)->getType());
  ...
  case Builtin::BI__builtin_elementwise_maxnum: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    Value *Op1 = EmitScalarExpr(E->getArg(1));
    Value *Result = Builder.CreateBinaryIntrinsic(toolchain::Intrinsic::maxnum, Op0,
  ...
  case Builtin::BI__builtin_elementwise_minnum: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    Value *Op1 = EmitScalarExpr(E->getArg(1));
    Value *Result = Builder.CreateBinaryIntrinsic(toolchain::Intrinsic::minnum, Op0,
  ...
  case Builtin::BI__builtin_fpclassify: {
    ...
    toolchain::Type *Ty = ConvertType(E->getArg(5)->getType());
  ...
  case Builtin::BI__builtin_alloca: {
    ...
    if (AAS != EAS) {
      toolchain::Type *Ty = CGM.getTypes().ConvertType(E->getType());
      return RValue::get(
  ...
  case Builtin::BI__builtin___memcpy_chk: {
    ...
    if (!E->getArg(2)->EvaluateAsInt(SizeResult, CGM.getContext()) ||
      ...
      break;
    toolchain::APSInt Size = SizeResult.Val.getInt();
    toolchain::APSInt DstSize = DstSizeResult.Val.getInt();
    if (Size.ugt(DstSize))
      break;
    Address Dest = EmitPointerWithAlignment(E->getArg(0));
    Address Src = EmitPointerWithAlignment(E->getArg(1));
    Value *SizeVal = toolchain::ConstantInt::get(Builder.getContext(), Size);
    auto *I = Builder.CreateMemCpy(Dest, Src, SizeVal, false);
    addInstToNewSourceAtom(I, nullptr);
    return RValue::get(Dest, *this);
  }
  ...
  case Builtin::BI__builtin_wmemchr: {
    ...
    toolchain::Type *WCharTy = ConvertType(getContext().WCharTy);
    ...
    Ret->addIncoming(toolchain::Constant::getNullValue(Str->getType()), Entry);
    Ret->addIncoming(toolchain::Constant::getNullValue(Str->getType()), Next);
  ...
  case Builtin::BI__builtin_dwarf_cfa: {
    ...
    // lowering toolchain.eh.dwarf.cfa()?
    //
    ...
    return RValue::get(Builder.CreateCall(F,
                                      toolchain::ConstantInt::get(Int32Ty, Offset)));
  }
  ...
  case Builtin::BI__builtin_dwarf_sp_column: {
    toolchain::IntegerType *Ty
      = cast<toolchain::IntegerType>(ConvertType(E->getType()));
    int Column = getTargetHooks().getDwarfEHStackPointer(CGM);
    if (Column == -1) {
      CGM.ErrorUnsupported(E, "__builtin_dwarf_sp_column");
      return RValue::get(toolchain::UndefValue::get(Ty));
    }
    return RValue::get(toolchain::ConstantInt::get(Ty, Column, true));
  }
  case Builtin::BI__builtin_init_dwarf_reg_size_table: {
    Value *Address = EmitScalarExpr(E->getArg(0));
    if (getTargetHooks().initDwarfEHRegSizeTable(*this, Address))
      CGM.ErrorUnsupported(E, "__builtin_init_dwarf_reg_size_table");
    return RValue::get(toolchain::UndefValue::get(ConvertType(E->getType())));
  }
  case Builtin::BI__builtin_eh_return: {
    Value *Int = EmitScalarExpr(E->getArg(0));
    Value *Ptr = EmitScalarExpr(E->getArg(1));

    toolchain::IntegerType *IntTy = cast<toolchain::IntegerType>(Int->getType());
  ...
  case Builtin::BI__builtin_setjmp: {
    ...
    if (getTarget().getTriple().getArch() == toolchain::Triple::systemz) {
  ...
  case Builtin::BI__sync_swap:
    toolchain_unreachable("Shouldn't make it through sema");
  case Builtin::BI__sync_fetch_and_add_1:
  case Builtin::BI__sync_fetch_and_add_2:
  case Builtin::BI__sync_fetch_and_add_4:
  case Builtin::BI__sync_fetch_and_add_8:
  case Builtin::BI__sync_fetch_and_add_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Add, E);
  case Builtin::BI__sync_fetch_and_sub_1:
  case Builtin::BI__sync_fetch_and_sub_2:
  case Builtin::BI__sync_fetch_and_sub_4:
  case Builtin::BI__sync_fetch_and_sub_8:
  case Builtin::BI__sync_fetch_and_sub_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Sub, E);
  case Builtin::BI__sync_fetch_and_or_1:
  case Builtin::BI__sync_fetch_and_or_2:
  case Builtin::BI__sync_fetch_and_or_4:
  case Builtin::BI__sync_fetch_and_or_8:
  case Builtin::BI__sync_fetch_and_or_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Or, E);
  case Builtin::BI__sync_fetch_and_and_1:
  case Builtin::BI__sync_fetch_and_and_2:
  case Builtin::BI__sync_fetch_and_and_4:
  case Builtin::BI__sync_fetch_and_and_8:
  case Builtin::BI__sync_fetch_and_and_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::And, E);
  case Builtin::BI__sync_fetch_and_xor_1:
  case Builtin::BI__sync_fetch_and_xor_2:
  case Builtin::BI__sync_fetch_and_xor_4:
  case Builtin::BI__sync_fetch_and_xor_8:
  case Builtin::BI__sync_fetch_and_xor_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Xor, E);
  case Builtin::BI__sync_fetch_and_nand_1:
  case Builtin::BI__sync_fetch_and_nand_2:
  case Builtin::BI__sync_fetch_and_nand_4:
  case Builtin::BI__sync_fetch_and_nand_8:
  case Builtin::BI__sync_fetch_and_nand_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Nand, E);

  // Clang extensions: not overloaded yet.
  case Builtin::BI__sync_fetch_and_min:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Min, E);
  case Builtin::BI__sync_fetch_and_max:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Max, E);
  case Builtin::BI__sync_fetch_and_umin:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::UMin, E);
  case Builtin::BI__sync_fetch_and_umax:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::UMax, E);
  ...
  case Builtin::BI__sync_add_and_fetch_16:
    return EmitBinaryAtomicPost(*this, toolchain::AtomicRMWInst::Add, E,
                                toolchain::Instruction::Add);
  case Builtin::BI__sync_sub_and_fetch_1:
  case Builtin::BI__sync_sub_and_fetch_2:
  case Builtin::BI__sync_sub_and_fetch_4:
  case Builtin::BI__sync_sub_and_fetch_8:
  case Builtin::BI__sync_sub_and_fetch_16:
    return EmitBinaryAtomicPost(*this, toolchain::AtomicRMWInst::Sub, E,
  ...
  case Builtin::BI__sync_and_and_fetch_16:
    return EmitBinaryAtomicPost(*this, toolchain::AtomicRMWInst::And, E,
                                toolchain::Instruction::And);
  case Builtin::BI__sync_or_and_fetch_1:
  case Builtin::BI__sync_or_and_fetch_2:
  case Builtin::BI__sync_or_and_fetch_4:
  case Builtin::BI__sync_or_and_fetch_8:
  case Builtin::BI__sync_or_and_fetch_16:
    return EmitBinaryAtomicPost(*this, toolchain::AtomicRMWInst::Or, E,
                                toolchain::Instruction::Or);
  case Builtin::BI__sync_xor_and_fetch_1:
  case Builtin::BI__sync_xor_and_fetch_2:
  case Builtin::BI__sync_xor_and_fetch_4:
  case Builtin::BI__sync_xor_and_fetch_8:
  case Builtin::BI__sync_xor_and_fetch_16:
    return EmitBinaryAtomicPost(*this, toolchain::AtomicRMWInst::Xor, E,
                                toolchain::Instruction::Xor);
  case Builtin::BI__sync_nand_and_fetch_1:
  case Builtin::BI__sync_nand_and_fetch_2:
  case Builtin::BI__sync_nand_and_fetch_4:
  case Builtin::BI__sync_nand_and_fetch_8:
  case Builtin::BI__sync_nand_and_fetch_16:
    return EmitBinaryAtomicPost(*this, toolchain::AtomicRMWInst::Nand, E,
                                toolchain::Instruction::And, true);
  ...
  case Builtin::BI__sync_swap_16:
    return EmitBinaryAtomic(*this, toolchain::AtomicRMWInst::Xchg, E);
  ...
  case Builtin::BI__sync_lock_release_16: {
    ...
    toolchain::Type *ITy = toolchain::IntegerType::get(getLLVMContext(),
                                             getContext().getTypeSize(ElTy));
    toolchain::StoreInst *Store =
        Builder.CreateStore(toolchain::Constant::getNullValue(ITy), Ptr);
    Store->setAtomic(toolchain::AtomicOrdering::Release);
    return RValue::get(nullptr);
  }
  ...
  case Builtin::BI__sync_synchronize: {
    ...
    // semantics.
    Builder.CreateFence(toolchain::AtomicOrdering::SequentiallyConsistent);
    return RValue::get(nullptr);
  }
  ...
  case Builtin::BI__atomic_is_lock_free: {
    ...
    if (BuiltinID == Builtin::BI__atomic_is_lock_free)
      ...
      Args.add(RValue::get(toolchain::Constant::getNullValue(VoidPtrTy)),
               getContext().VoidPtrTy);
    const CGFunctionInfo &FuncInfo =
        CGM.getTypes().arrangeBuiltinFunctionCall(E->getType(), Args);
    toolchain::FunctionType *FTy = CGM.getTypes().GetFunctionType(FuncInfo);
    toolchain::FunctionCallee Func = CGM.CreateRuntimeFunction(FTy, LibCallName);
  ...
  case Builtin::BI__c11_atomic_signal_fence: {
    ...
    if (BuiltinID == Builtin::BI__atomic_signal_fence ||
      ...
      SSID = toolchain::SyncScope::SingleThread;
    else
      SSID = toolchain::SyncScope::System;
    Value *Order = EmitScalarExpr(E->getArg(0));
    if (isa<toolchain::ConstantInt>(Order)) {
      int ord = cast<toolchain::ConstantInt>(Order)->getZExtValue();
      switch (ord) {
      ...
      case 2:  // memory_order_acquire
        Builder.CreateFence(toolchain::AtomicOrdering::Acquire, SSID);
        break;
      case 3:  // memory_order_release
        Builder.CreateFence(toolchain::AtomicOrdering::Release, SSID);
        break;
      case 4:  // memory_order_acq_rel
        Builder.CreateFence(toolchain::AtomicOrdering::AcquireRelease, SSID);
        break;
      case 5:  // memory_order_seq_cst
        Builder.CreateFence(toolchain::AtomicOrdering::SequentiallyConsistent, SSID);
        break;
      }
      return RValue::get(nullptr);
    }

    toolchain::BasicBlock *AcquireBB, *ReleaseBB, *AcqRelBB, *SeqCstBB;
    AcquireBB = createBasicBlock("acquire", CurFn);
    ReleaseBB = createBasicBlock("release", CurFn);
    AcqRelBB = createBasicBlock("acqrel", CurFn);
    SeqCstBB = createBasicBlock("seqcst", CurFn);
    toolchain::BasicBlock *ContBB = createBasicBlock("atomic.continue", CurFn);

    Order = Builder.CreateIntCast(Order, Builder.getInt32Ty(), false);
    toolchain::SwitchInst *SI = Builder.CreateSwitch(Order, ContBB);
  ...
  case Builtin::BI__scoped_atomic_thread_fence: {
    ...
    auto Ord = dyn_cast<toolchain::ConstantInt>(Order);
    auto Scp = dyn_cast<toolchain::ConstantInt>(Scope);
    if (Ord && Scp) {
      ...
      case 2: // memory_order_acquire
        Builder.CreateFence(
            toolchain::AtomicOrdering::Acquire,
      ...
      case 3: // memory_order_release
        Builder.CreateFence(
            toolchain::AtomicOrdering::Release,
            getTargetHooks().getLLVMSyncScopeID(getLangOpts(), SS,
                                                toolchain::AtomicOrdering::Release,
                                                getLLVMContext()));
        break;
      case 4: // memory_order_acq_rel
        Builder.CreateFence(toolchain::AtomicOrdering::AcquireRelease,
                            getTargetHooks().getLLVMSyncScopeID(
                                getLangOpts(), SS,
                                toolchain::AtomicOrdering::AcquireRelease,
                                getLLVMContext()));
        break;
      case 5: // memory_order_seq_cst
        Builder.CreateFence(toolchain::AtomicOrdering::SequentiallyConsistent,
    ...
    toolchain::BasicBlock *ContBB = createBasicBlock("atomic.scope.continue", CurFn);

    toolchain::SmallVector<std::pair<toolchain::BasicBlock *, toolchain::AtomicOrdering>>
        OrderBBs;
    if (Ord) {
      switch (Ord->getZExtValue()) {
      case 0:  // memory_order_relaxed
      ...
      toolchain::BasicBlock *AcquireBB = createBasicBlock("acquire", CurFn);
      toolchain::BasicBlock *ReleaseBB = createBasicBlock("release", CurFn);
      toolchain::BasicBlock *AcqRelBB = createBasicBlock("acqrel", CurFn);
      toolchain::BasicBlock *SeqCstBB = createBasicBlock("seqcst", CurFn);
      ...
      OrderBBs.emplace_back(AcquireBB, toolchain::AtomicOrdering::Acquire);
      OrderBBs.emplace_back(ReleaseBB, toolchain::AtomicOrdering::Release);
      OrderBBs.emplace_back(AcqRelBB, toolchain::AtomicOrdering::AcquireRelease);
  ...
  case Builtin::BI__annotation: {
    ...
    for (const Expr *Arg : E->arguments()) {
      ...
      Strings.push_back(toolchain::MDString::get(getLLVMContext(), StrUtf8));
    }

    // Build and MDTuple of MDStrings and emit the intrinsic call.
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::codeview_annotation, {});
  ...
  case Builtin::BI__builtin_annotation: {
    toolchain::Value *AnnVal = EmitScalarExpr(E->getArg(0));
  ...
  case Builtin::BI__builtin_subcll: {
    ...
    //
    //   %tmp1 = call {i32, i1} @toolchain.uadd.with.overflow.i32(i32 %x, i32 %y)
    //   %tmpsum1 = extractvalue {i32, i1} %tmp1, 0
    //   %carry1 = extractvalue {i32, i1} %tmp1, 1
    //   %tmp2 = call {i32, i1} @toolchain.uadd.with.overflow.i32(i32 %tmpsum1,
    //                                                       i32 %carryin)
    ...
    toolchain::Value *X = EmitScalarExpr(E->getArg(0));
    toolchain::Value *Y = EmitScalarExpr(E->getArg(1));
    toolchain::Value *Carryin = EmitScalarExpr(E->getArg(2));
    Address CarryOutPtr = EmitPointerWithAlignment(E->getArg(3));

    // Decide if we are lowering to a uadd.with.overflow or usub.with.overflow.
    Intrinsic::ID IntrinsicId;
    switch (BuiltinID) {
    default: toolchain_unreachable("Unknown multiprecision builtin id.");
    case Builtin::BI__builtin_addcb:
    case Builtin::BI__builtin_addcs:
    case Builtin::BI__builtin_addc:
    case Builtin::BI__builtin_addcl:
    ...
    toolchain::Value *Carry1;
    toolchain::Value *Sum1 = EmitOverflowIntrinsic(*this, IntrinsicId,
                                              X, Y, Carry1);
    toolchain::Value *Carry2;
    toolchain::Value *Sum2 = EmitOverflowIntrinsic(*this, IntrinsicId,
                                              Sum1, Carryin, Carry2);
    toolchain::Value *CarryOut = Builder.CreateZExt(Builder.CreateOr(Carry1, Carry2),
                                               X->getType());
  ...
  case Builtin::BI__builtin_mul_overflow: {
    const language::Core::Expr *LeftArg = E->getArg(0);
    const language::Core::Expr *RightArg = E->getArg(1);
    const language::Core::Expr *ResultArg = E->getArg(2);

    language::Core::QualType ResultQTy =
    ...
    toolchain::Type *EncompassingLLVMTy =
        toolchain::IntegerType::get(CGM.getLLVMContext(), EncompassingInfo.Width);

    toolchain::Type *ResultLLVMTy = CGM.getTypes().ConvertType(ResultQTy);

    Intrinsic::ID IntrinsicId;
    switch (BuiltinID) {
    default:
      toolchain_unreachable("Unknown overflow builtin id.");
    case Builtin::BI__builtin_add_overflow:
    ...
    toolchain::Value *Left = EmitScalarExpr(LeftArg);
    toolchain::Value *Right = EmitScalarExpr(RightArg);
  ...
  case Builtin::BI__builtin_smulll_overflow: {
    ...
    // We translate all of these builtins directly to the relevant toolchain IR node.
    ...
    Intrinsic::ID IntrinsicId;
    switch (BuiltinID) {
    default: toolchain_unreachable("Unknown overflow builtin id.");
    case Builtin::BI__builtin_uadd_overflow:
    case Builtin::BI__builtin_uaddl_overflow:
    case Builtin::BI__builtin_uaddll_overflow:
      IntrinsicId = Intrinsic::uadd_with_overflow;
    ...
    toolchain::Value *Carry;
    toolchain::Value *Sum = EmitOverflowIntrinsic(*this, IntrinsicId, X, Y, Carry);
  ...
  case Builtin::BI__builtin_ptrauth_strip: {
    // Emit the arguments.
    SmallVector<toolchain::Value *, 5> Args;
    for (auto argExpr : E->arguments())
      Args.push_back(EmitScalarExpr(argExpr));

    // Cast the value to intptr_t, saving its original type.
    toolchain::Type *OrigValueType = Args[0]->getType();
    ...
    auto IntrinsicID = [&]() -> unsigned {
      ...
      toolchain_unreachable("bad ptrauth intrinsic");
    }();
    auto Intrinsic = CGM.getIntrinsic(IntrinsicID);
    toolchain::Value *Result = EmitRuntimeCall(Intrinsic, Args);
  ...
  case Builtin::BI_setjmp:
    if (getTarget().getTriple().isOSMSVCRT() && E->getNumArgs() == 1 &&
        E->getArg(0)->getType()->isPointerType()) {
      if (getTarget().getTriple().getArch() == toolchain::Triple::x86)
        return EmitMSVCRTSetJmp(*this, MSVCSetJmpKind::_setjmp3, E);
      else if (getTarget().getTriple().getArch() == toolchain::Triple::aarch64)
        return EmitMSVCRTSetJmp(*this, MSVCSetJmpKind::_setjmpex, E);
  ...
  case Builtin::BI__GetExceptionInfo: {
    if (toolchain::GlobalVariable *GV =
  ...
  case Builtin::BIwrite_pipe: {
    ...
    toolchain::Type *I8PTy = toolchain::PointerType::get(getLLVMContext(), GenericAS);
    ...
    if (2U == E->getNumArgs()) {
      ...
      // user defined type.
      toolchain::Type *ArgTys[] = {Arg0->getType(), I8PTy, Int32Ty, Int32Ty};
      toolchain::FunctionType *FTy = toolchain::FunctionType::get(Int32Ty, ArgTys, false);
      ...
      toolchain::Type *ArgTys[] = {Arg0->getType(), Arg1->getType(), Int32Ty, I8PTy,
                              Int32Ty, Int32Ty};
  ...
  case Builtin::BIsub_group_reserve_write_pipe: {
    ...
    Value *Arg0 = EmitScalarExpr(E->getArg(0)),
          *Arg1 = EmitScalarExpr(E->getArg(1));
    toolchain::Type *ReservedIDTy = ConvertType(getContext().OCLReserveIDTy);
    CGOpenCLRuntime OpenCLRT(CGM);
    Value *PacketSize = OpenCLRT.getPipeElemSize(E->getArg(0));
    Value *PacketAlign = OpenCLRT.getPipeElemAlign(E->getArg(0));

    // Building the generic function prototype.
    toolchain::Type *ArgTys[] = {Arg0->getType(), Int32Ty, Int32Ty, Int32Ty};
    toolchain::FunctionType *FTy =
        toolchain::FunctionType::get(ReservedIDTy, ArgTys, false);
  ...
  case Builtin::BIsub_group_commit_write_pipe: {
    ...
    // Building the generic function prototype.
    toolchain::Type *ArgTys[] = {Arg0->getType(), Arg1->getType(), Int32Ty, Int32Ty};
    toolchain::FunctionType *FTy = toolchain::FunctionType::get(
        toolchain::Type::getVoidTy(getLLVMContext()), ArgTys, false);
  ...
  case Builtin::BIget_pipe_max_packets: {
    ...
    Value *PacketSize = OpenCLRT.getPipeElemSize(E->getArg(0));
    Value *PacketAlign = OpenCLRT.getPipeElemAlign(E->getArg(0));
    toolchain::Type *ArgTys[] = {Arg0->getType(), Int32Ty, Int32Ty};
    toolchain::FunctionType *FTy = toolchain::FunctionType::get(Int32Ty, ArgTys, false);
  ...
  case Builtin::BIto_private: {
    ...
    auto NewArgT = toolchain::PointerType::get(
        getLLVMContext(),
        CGM.getContext().getTargetAddressSpace(LangAS::opencl_generic));
    auto NewRetT = toolchain::PointerType::get(
        getLLVMContext(),
        CGM.getContext().getTargetAddressSpace(
            E->getType()->getPointeeType().getAddressSpace()));
    auto FTy = toolchain::FunctionType::get(NewRetT, {NewArgT}, false);
    toolchain::Value *NewArg;
  ...
  case Builtin::BIenqueue_kernel: {
    ...
    toolchain::Type *QueueTy = ConvertType(getContext().OCLQueueTy);
    toolchain::Type *GenericVoidPtrTy = Builder.getPtrTy(
        getContext().getTargetAddressSpace(LangAS::opencl_generic));

    toolchain::Value *Queue = EmitScalarExpr(E->getArg(0));
    toolchain::Value *Flags = EmitScalarExpr(E->getArg(1));
    LValue NDRangeL = EmitAggExprToLValue(E->getArg(2));
    toolchain::Value *Range = NDRangeL.getAddress().emitRawPointer(*this);
    ...
    toolchain::Type *RangePtrTy = Range->getType();

    if (NumArgs == 4) {
      // The most basic form of the call with parameters:
      // queue_t, kernel_enqueue_flags_t, ndrange_t, block(void)
      Name = "__enqueue_kernel_basic";
      toolchain::Type *ArgTys[] = {QueueTy, Int32Ty, RangePtrTy, GenericVoidPtrTy,
                              GenericVoidPtrTy};
      toolchain::FunctionType *FTy = toolchain::FunctionType::get(Int32Ty, ArgTys, false);
    ...
    auto CreateArrayForSizeVar =
        [=](unsigned First) -> std::pair<toolchain::Value *, toolchain::Value *> {
      toolchain::APInt ArraySize(32, NumArgs - First);
      QualType SizeArrayTy = getContext().getConstantArrayType(
          getContext().getSizeType(), ArraySize, nullptr,
          ArraySizeModifier::Normal,
          /*IndexTypeQuals=*/0);
      auto Tmp = CreateMemTemp(SizeArrayTy, "block_sizes");
      toolchain::Value *TmpPtr = Tmp.getPointer();
      ...
      // stripPointerCasts()
      toolchain::Value *Alloca = TmpPtr->stripPointerCasts();
      toolchain::Value *ElemPtr;
      EmitLifetimeStart(Alloca);
      // Each of the following arguments specifies the size of the corresponding
      // argument passed to the enqueued block.
      auto *Zero = toolchain::ConstantInt::get(IntTy, 0);
      for (unsigned I = First; I < NumArgs; ++I) {
        auto *Index = toolchain::ConstantInt::get(IntTy, I - First);
        auto *GEP =
    ...
    if (E->getArg(3)->getType()->isBlockPointerType()) {
      ...
      toolchain::Value *const Args[] = {Queue,  Flags,
                                   Range,  Kernel,
                                   Block,  ConstantInt::get(IntTy, NumArgs - 4),
                                   ElemPtr};
      toolchain::Type *const ArgTys[] = {
    ...
    if (NumArgs >= 7) {
      ...
      toolchain::Value *EventWaitList = nullptr;
      if (E->getArg(4)->isNullPointerConstant(
              getContext(), Expr::NPC_ValueDependentIsNotNull)) {
        EventWaitList = toolchain::ConstantPointerNull::get(PtrTy);
      } else {
        EventWaitList =
      ...
      toolchain::Value *EventRet = nullptr;
      if (E->getArg(5)->isNullPointerConstant(
              getContext(), Expr::NPC_ValueDependentIsNotNull)) {
        EventRet = toolchain::ConstantPointerNull::get(PtrTy);
      } else {
        EventRet =
      ...
      std::vector<toolchain::Type *> ArgTys = {
          QueueTy, Int32Ty, RangePtrTy,       Int32Ty,
          PtrTy,   PtrTy,   GenericVoidPtrTy, GenericVoidPtrTy};

      std::vector<toolchain::Value *> Args = {Queue,     Flags,         Range,
      ...
      if (NumArgs == 7) {
        ...
        toolchain::FunctionType *FTy =
            toolchain::FunctionType::get(Int32Ty, ArgTys, false);
        return RValue::get(
            EmitRuntimeCall(CGM.CreateRuntimeFunction(FTy, Name), Args));
      }
    ...
    toolchain_unreachable("Unexpected enqueue_kernel signature");
  }
  ...
  case Builtin::BIget_kernel_work_group_size: {
    ...
    return RValue::get(EmitRuntimeCall(
        CGM.CreateRuntimeFunction(
            toolchain::FunctionType::get(IntTy, {GenericVoidPtrTy, GenericVoidPtrTy},
                                    false),
  ...
  case Builtin::BIget_kernel_sub_group_count_for_ndrange: {
    ...
    toolchain::Value *NDRange = NDRangeL.getAddress().emitRawPointer(*this);
    auto Info =
  ...
  case Builtin::BI__builtin_thread_pointer: {
    ...
    return RValue::get(Builder.CreateIntrinsic(toolchain::Intrinsic::thread_pointer,
                                               {GlobalsInt8PtrTy}, {}));
  }
  ...
  StringRef Prefix =
      toolchain::Triple::getArchTypePrefix(getTarget().getTriple().getArch());
  if (!Prefix.empty()) {
    IntrinsicID = Intrinsic::getIntrinsicForClangBuiltin(Prefix.data(), Name);
    if (IntrinsicID == Intrinsic::not_intrinsic && Prefix == "spv" &&
        getTarget().getTriple().getOS() == toolchain::Triple::OSType::AMDHSA)
  ...
  if (IntrinsicID != Intrinsic::not_intrinsic) {
    ...
    for (unsigned i = 0, e = E->getNumArgs(); i != e; ++i) {
      ...
      toolchain::Type *PTy = FTy->getParamType(i);
      if (PTy != ArgValue->getType()) {
        // XXX - vector of pointers?
        if (auto *PtrTy = dyn_cast<toolchain::PointerType>(PTy)) {
          if (PtrTy->getAddressSpace() !=
              ArgValue->getType()->getPointerAddressSpace()) {
            ArgValue = Builder.CreateAddrSpaceCast(
                ArgValue, toolchain::PointerType::get(getLLVMContext(),
    ...
    toolchain::Type *RetTy = VoidTy;
    if (!BuiltinRetType->isVoidType())
      RetTy = ConvertType(BuiltinRetType);

    if (RetTy != V->getType()) {
      // XXX - vector of pointers?
      if (auto *PtrTy = dyn_cast<toolchain::PointerType>(RetTy)) {
        if (PtrTy->getAddressSpace() != V->getType()->getPointerAddressSpace()) {
          V = Builder.CreateAddrSpaceCast(
              V, toolchain::PointerType::get(getLLVMContext(),
                                        PtrTy->getAddressSpace()));
        }
      }
  ...
  if (Value *V = EmitTargetBuiltinExpr(BuiltinID, E, ReturnValue)) {
    ...
    case TEK_Complex:
      toolchain_unreachable("No current target builtin returns complex");
    }
    toolchain_unreachable("Bad evaluation kind in EmitBuiltinExpr");
  }
  ...
  if (Value *V = EmitHLSLBuiltinExpr(BuiltinID, E, ReturnValue)) {
    ...
    case TEK_Aggregate:
      return RValue::getAggregate(ReturnValue.getAddress(),
                                  ReturnValue.isVolatile());
    case TEK_Complex:
      toolchain_unreachable("No current hlsl builtin returns complex");
    }
    toolchain_unreachable("Bad evaluation kind in EmitBuiltinExpr");
  }
...
struct BuiltinAlignArgs {
  toolchain::Value *Src = nullptr;
  toolchain::Type *SrcType = nullptr;
  toolchain::Value *Alignment = nullptr;
  toolchain::Value *Mask = nullptr;
  toolchain::IntegerType *IntType = nullptr;
...
RValue CodeGenFunction::EmitBuiltinIsAligned(const CallExpr *E) {
  ...
  toolchain::Value *SrcAddress = Args.Src;
  ...
  return RValue::get(Builder.CreateICmpEQ(
      ...
      toolchain::Constant::getNullValue(Args.IntType), "is_aligned"));
}
...
/// toolchain.ptrmask intrinsic (with a GEP before in the align_up case).
RValue CodeGenFunction::EmitBuiltinAlignTo(const CallExpr *E, bool AlignUp) {
  BuiltinAlignArgs Args(E, *this);
  toolchain::Value *SrcForMask = Args.Src;
  if (AlignUp) {
  ...
  toolchain::Value *InvertedMask = Builder.CreateNot(Args.Mask, "inverted_mask");
