...
Address CodeGenFunction::EmitCXXMemberDataPointerAddress(
  ...
  toolchain::Value *ptr = CGM.getCXXABI().EmitMemberDataPointerAddress(
...
CodeGenModule::GetNonVirtualBaseClassOffset(const CXXRecordDecl *ClassDecl,
  ...
  return toolchain::ConstantInt::get(PtrDiffTy, Offset.getQuantity());
}
...
ApplyNonVirtualAndVirtualOffset(CodeGenFunction &CGF, Address addr,
  ...
  toolchain::Value *baseOffset;
  if (!nonVirtualOffset.isZero()) {
    toolchain::Type *OffsetType =
        (CGF.CGM.getTarget().getCXXABI().isItaniumFamily() &&
         CGF.CGM.getItaniumVTableContext().isRelativeLayout())
            ? CGF.Int32Ty
            : CGF.PtrDiffTy;
    baseOffset =
        toolchain::ConstantInt::get(OffsetType, nonVirtualOffset.getQuantity());
    if (virtualOffset) {
  ...
  toolchain::Value *ptr = addr.emitRawPointer(CGF);
...
Address CodeGenFunction::GetAddressOfBaseClass(
  ...
  toolchain::Type *BaseValueTy = ConvertType((PathEnd[-1])->getType());
  toolchain::Type *PtrTy = toolchain::PointerType::get(
  ...
  toolchain::BasicBlock *origBB = nullptr;
  toolchain::BasicBlock *endBB = nullptr;

  // Skip over the offset (and the vtable load) if we're supposed to
  // null-check the pointer.
  if (NullCheckValue) {
    origBB = Builder.GetInsertBlock();
    toolchain::BasicBlock *notNullBB = createBasicBlock("cast.notnull");
    endBB = createBasicBlock("cast.end");

    toolchain::Value *isNull = Builder.CreateIsNull(Value);
  ...
  toolchain::Value *VirtualOffset = nullptr;
  if (VBase) {
    VirtualOffset =
  ...
  if (NullCheckValue) {
    toolchain::BasicBlock *notNullBB = Builder.GetInsertBlock();
    Builder.CreateBr(endBB);
    EmitBlock(endBB);

    toolchain::PHINode *PHI = Builder.CreatePHI(PtrTy, 2, "cast.result");
    PHI->addIncoming(Value.emitRawPointer(*this), notNullBB);
    PHI->addIncoming(toolchain::Constant::getNullValue(PtrTy), origBB);
...
CodeGenFunction::GetAddressOfDerivedClass(Address BaseAddr,
  ...
  CanQualType DerivedTy = getContext().getCanonicalTagType(Derived);
  toolchain::Type *DerivedValueTy = ConvertType(DerivedTy);

  toolchain::Value *NonVirtualOffset =
  ...
  toolchain::BasicBlock *CastNull = nullptr;
  toolchain::BasicBlock *CastNotNull = nullptr;
  toolchain::BasicBlock *CastEnd = nullptr;
  ...
  if (NullCheckValue) {
    ...
    toolchain::Value *IsNull = Builder.CreateIsNull(BaseAddr);
  ...
  if (NullCheckValue) {
    ...
    toolchain::Value *Value = Addr.emitRawPointer(*this);
    toolchain::PHINode *PHI = Builder.CreatePHI(Value->getType(), 2);
    PHI->addIncoming(Value, CastNotNull);
    PHI->addIncoming(toolchain::Constant::getNullValue(Value->getType()), CastNull);
...
toolchain::Value *CodeGenFunction::GetVTTParameter(GlobalDecl GD,
                                              bool ForVirtualBase,
                                              bool Delegating) {
  ...
  if (CGM.getCXXABI().NeedsVTTParameter(CurGD)) {
    ...
    toolchain::Value *VTT = LoadCXXVTT();
    return Builder.CreateConstInBoundsGEP1_64(VoidPtrTy, VTT, SubVTTIndex);
  } else {
    // We're the complete constructor, so get the VTT by name.
    toolchain::GlobalValue *VTT = CGM.getVTables().GetAddrOfVTT(RD);
...
static void EmitMemberInitializer(CodeGenFunction &CGF,
  ...
  toolchain::Value *ThisPtr = CGF.LoadCXXThis();
...
namespace {
  ...
  public:
    ...
    void emitMemcpy() {
      ...
      toolchain::Value *SrcPtr = CGF.Builder.CreateLoad(CGF.GetAddrOfLocalVar(SrcRec));
...
void CodeGenFunction::EmitCtorPrologue(const CXXConstructorDecl *CD,
  ...
  toolchain::BasicBlock *BaseCtorContinueBB = nullptr;
  if (ConstructVBases &&
      !CGM.getTarget().getCXXABI().hasConstructorVariants()) {
    BaseCtorContinueBB =
...
void CodeGenFunction::EmitDestructorBody(FunctionArgList &Args) {
  ...
  switch (DtorType) {
  case Dtor_Comdat: toolchain_unreachable("not expecting a COMDAT");
  case Dtor_Deleting: toolchain_unreachable("already handled deleting case");
  ...
  case Dtor_Base:
    ...
    if (!CanSkipVTablePointerInitialization(*this, Dtor)) {
      // Insert the toolchain.launder.invariant.group intrinsic before initializing
    ...
    if (getLangOpts().AppleKext)
      CurFn->addFnAttr(toolchain::Attribute::AlwaysInline);
...
namespace {
  toolchain::Value *LoadThisForDtorDelete(CodeGenFunction &CGF,
  ...
  void EmitConditionalDtorDeleteCall(CodeGenFunction &CGF,
                                     toolchain::Value *ShouldDeleteCondition,
                                     bool ReturnAfterDelete) {
    toolchain::BasicBlock *callDeleteBB = CGF.createBasicBlock("dtor.call_delete");
    toolchain::BasicBlock *continueBB = CGF.createBasicBlock("dtor.continue");
    toolchain::Value *ShouldCallDelete
  ...
  public:
    CallDtorDeleteConditional(toolchain::Value *ShouldDeleteCondition)
  ...
  static void EmitSanitizerDtorCallback(
      CodeGenFunction &CGF, StringRef Name, toolchain::Value *Ptr,
      std::optional<CharUnits::QuantityType> PoisonSize = {}) {
    CodeGenFunction::SanitizerScope SanScope(&CGF);
    // Pass in void pointer and size of region as arguments to runtime
    // function
    SmallVector<toolchain::Value *, 2> Args = {Ptr};
    SmallVector<toolchain::Type *, 2> ArgTypes = {CGF.VoidPtrTy};

    if (PoisonSize.has_value()) {
      Args.emplace_back(toolchain::ConstantInt::get(CGF.SizeTy, *PoisonSize));
      ArgTypes.emplace_back(CGF.SizeTy);
    }

    toolchain::FunctionType *FnType =
        toolchain::FunctionType::get(CGF.VoidTy, ArgTypes, false);
    toolchain::FunctionCallee Fn = CGF.CGM.CreateRuntimeFunction(FnType, Name);

    CGF.EmitNounwindRuntimeCall(Fn, Args);
  }

  static void
  EmitSanitizerDtorFieldsCallback(CodeGenFunction &CGF, toolchain::Value *Ptr,
  ...
  public:
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      toolchain::ConstantInt *OffsetSizePtr =
          toolchain::ConstantInt::get(CGF.SizeTy, PoisonStart.getQuantity());
 ...
 class SanitizeDtorVTable final : public EHScopeStack::Cleanup {
  ...
  public:
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      toolchain::Value *VTablePtr = CGF.LoadCXXThis();
...
void CodeGenFunction::EmitCXXAggrConstructorCall(const CXXConstructorDecl *ctor,
  ...
  toolchain::BranchInst *zeroCheckBranch = nullptr;

  // Optimize for a constant count.
  toolchain::ConstantInt *constantCount
    = dyn_cast<toolchain::ConstantInt>(numElements);
  if (constantCount) {
  ...
  // Otherwise, emit the check.
    ...
    toolchain::BasicBlock *loopBB = createBasicBlock("new.ctorloop");
    toolchain::Value *iszero = Builder.CreateIsNull(numElements, "isempty");
    zeroCheckBranch = Builder.CreateCondBr(iszero, loopBB, loopBB);
    EmitBlock(loopBB);
  }

  // Find the end of the array.
  toolchain::Type *elementType = arrayBase.getElementType();
  toolchain::Value *arrayBegin = arrayBase.emitRawPointer(*this);
  toolchain::Value *arrayEnd = Builder.CreateInBoundsGEP(
      elementType, arrayBegin, numElements, "arrayctor.end");

  // Enter the loop, setting up a phi for the current location to initialize.
  toolchain::BasicBlock *entryBB = Builder.GetInsertBlock();
  toolchain::BasicBlock *loopBB = createBasicBlock("arrayctor.loop");
  EmitBlock(loopBB);
  toolchain::PHINode *cur = Builder.CreatePHI(arrayBegin->getType(), 2,
                                         "arrayctor.cur");
  ...
  toolchain::Value *next = Builder.CreateInBoundsGEP(
      elementType, cur, toolchain::ConstantInt::get(SizeTy, 1), "arrayctor.next");
  cur->addIncoming(next, Builder.GetInsertBlock());

  // Check whether that's the end of the loop.
  toolchain::Value *done = Builder.CreateICmpEQ(next, arrayEnd, "arrayctor.done");
  toolchain::BasicBlock *contBB = createBasicBlock("arrayctor.cont");
...
void CodeGenFunction::EmitCXXConstructorCall(const CXXConstructorDecl *D,
  ...
  toolchain::Value *ThisPtr =
      getAsNaturalPointerTo(This, D->getThisType()->getPointeeType());

  if (SlotAS != ThisAS) {
    unsigned TargetThisAS = getContext().getTargetAddressSpace(ThisAS);
    toolchain::Type *NewType =
        toolchain::PointerType::get(getLLVMContext(), TargetThisAS);
    ThisPtr =
...
void CodeGenFunction::EmitCXXConstructorCall(
    ...
    bool NewPointerIsChecked, toolchain::CallBase **CallOrInvoke) {
  ...
  // Emit the call.
  toolchain::Constant *CalleePtr = CGM.getAddrOfCXXStructor(GlobalDecl(D, Type));
...
CodeGenFunction::EmitSynthesizedCXXCopyCtorCall(const CXXConstructorDecl *D,
  ...
  toolchain::Type *t = CGM.getTypes().ConvertType(QT);
  toolchain::Value *Val = getAsNaturalPointerTo(Src, D->getThisType());
  toolchain::Value *SrcVal = Builder.CreateBitCast(Val, t);
...
void CodeGenFunction::InitializeVTablePointer(const VPtr &Vptr) {
  ...
  toolchain::Value *VTableAddressPoint =
  ...
  toolchain::Type *PtrTy = toolchain::PointerType::get(CGM.getLLVMContext(), GlobalsAS);
  ...
  toolchain::StoreInst *Store = Builder.CreateStore(VTableAddressPoint, VTableField);
...
toolchain::Value *CodeGenFunction::GetVTablePtr(Address This,
                                           toolchain::Type *VTableTy,
                                           const CXXRecordDecl *RD,
                                           VTableAuthMode AuthMode) {
  Address VTablePtrSrc = This.withElementType(VTableTy);
  toolchain::Instruction *VTable = Builder.CreateLoad(VTablePtrSrc, "vtable");
  ...
  if (auto AuthenticationInfo =
    ...
    if (AuthMode != VTableAuthMode::UnsafeUbsanStrip) {
      VTable = cast<toolchain::Instruction>(
      ...
      if (AuthMode == VTableAuthMode::MustTrap) {
        ...
        VTable =
            cast<toolchain::Instruction>(Builder.CreateBitCast(VTable, Int8PtrTy));
        Builder.CreateLoad(RawAddress(VTable, Int8Ty, CGM.getPointerAlign()),
                           /* IsVolatile */ true);
      }
    } else {
      VTable = cast<toolchain::Instruction>(EmitPointerAuthAuth(
...
// functions on class instances with specific properties (e.g. toolchain::Operator)
...
void CodeGenFunction::EmitTypeMetadataCodeForVCall(const CXXRecordDecl *RD,
  ...
  if (SanOpts.has(SanitizerKind::CFIVCall))
    ...
    toolchain::Metadata *MD = CGM.CreateMetadataIdentifierForType(Ty);
    toolchain::Value *TypeId =
        toolchain::MetadataAsValue::get(CGM.getLLVMContext(), MD);

    // If we already know that the call has hidden LTO visibility, emit
    // @toolchain.type.test(). Otherwise emit @toolchain.public.type.test(), which WPD
    // will convert to @toolchain.type.test() if we assert at link time that we have
...
SanitizerInfoFromCFICheckKind(CodeGenFunction::CFITypeCheckKind TCK) {
  ...
  toolchain_unreachable("Unknown CFITypeCheckKind enum");
}
...
void CodeGenFunction::EmitVTablePtrCheckForCast(QualType T, Address Derived,
  ...
  if (MayBeNull) {
    toolchain::Value *DerivedNotNull =
        Builder.CreateIsNotNull(Derived.emitRawPointer(*this), "cast.nonnull");

    toolchain::BasicBlock *CheckBlock = createBasicBlock("cast.check");
...
void CodeGenFunction::EmitVTablePtrCheck(const CXXRecordDecl *RD,
  ...
  toolchain::Metadata *MD = CGM.CreateMetadataIdentifierForType(T);
  toolchain::Value *TypeId = toolchain::MetadataAsValue::get(getLLVMContext(), MD);

  toolchain::Value *TypeTest = Builder.CreateCall(
      CGM.getIntrinsic(toolchain::Intrinsic::type_test), {VTable, TypeId});
...
toolchain::Value *CodeGenFunction::EmitVTableTypeCheckedLoad(
    const CXXRecordDecl *RD, toolchain::Value *VTable, toolchain::Type *VTableTy,
    uint64_t VTableByteOffset) {
  auto CheckOrdinal = SanitizerKind::SO_CFIVCall;
  auto CheckHandler = SanitizerHandler::CFICheckFail;
  SanitizerDebugLocation SanScope(this, {CheckOrdinal}, CheckHandler);

  EmitSanitizerStatReport(toolchain::SanStat_CFI_VCall);

  CanQualType T = CGM.getContext().getCanonicalTagType(RD);
  toolchain::Metadata *MD = CGM.CreateMetadataIdentifierForType(T);
  toolchain::Value *TypeId = toolchain::MetadataAsValue::get(CGM.getLLVMContext(), MD);

  auto CheckedLoadIntrinsic = CGM.getVTables().useRelativeLayout()
                                  ? toolchain::Intrinsic::type_checked_load_relative
                                  : toolchain::Intrinsic::type_checked_load;
  toolchain::Value *CheckedLoad = Builder.CreateCall(
      CGM.getIntrinsic(CheckedLoadIntrinsic),
      {VTable, toolchain::ConstantInt::get(Int32Ty, VTableByteOffset), TypeId});

  toolchain::Value *CheckResult = Builder.CreateExtractValue(CheckedLoad, 1);
...
void CodeGenFunction::EmitForwardingCallToLambda(
    ...
    const CGFunctionInfo *calleeFnInfo, toolchain::Constant *calleePtr) {
...
void CodeGenFunction::EmitLambdaDelegatingInvokeBody(const CXXMethodDecl *MD,
  ...
  if (hasInAllocaArg(MD)) {
    ...
    toolchain::Function *ImplFn = nullptr;
...
void CodeGenFunction::EmitLambdaInAllocaCallOpBody(const CXXMethodDecl *MD) {
  ...
  CanQualType ThisType = getContext().getPointerType(LambdaType);
  toolchain::Value *ThisArg = CurFn->getArg(0);
...
void CodeGenFunction::EmitLambdaInAllocaImplFn(
  ...
  toolchain::Function *CallOpFn =
      cast<toolchain::Function>(CGM.GetAddrOfFunction(GlobalDecl(CallOp)));
  ...
  *ImplFnInfo = &CGM.getTypes().arrangeLLVMFunctionInfo(
  ...
  toolchain::Function *Fn = CallOpFn->getParent()->getFunction(ImplName);
  if (!Fn) {
    Fn = toolchain::Function::Create(CGM.getTypes().GetFunctionType(**ImplFnInfo),
                                toolchain::GlobalValue::InternalLinkage, ImplName,
                                CGM.getModule());
