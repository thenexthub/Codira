...
#include "language/Core/Basic/OpenMPKinds.h"
...
#include "toolchain/Support/AtomicOrdering.h"
...
public:
  class UntiedTaskActionTy final : public PrePostActionTy {
    ...
    toolchain::SwitchInst *UntiedSwitch = nullptr;
  ...
  public:
    ...
    void Enter(CodeGenFunction &CGF) override {
      if (Untied) {
        ...
        toolchain::BasicBlock *DoneBB = CGF.createBasicBlock(".untied.done.");
...
public:
  ...
  toolchain::Value *getContextValue() const override {
    if (OuterRegionInfo)
      return OuterRegionInfo->getContextValue();
    toolchain_unreachable("No context value for inlined OpenMP region");
  }

  void setContextValue(toolchain::Value *V) override {
    if (OuterRegionInfo) {
  ...
  LValue getThreadIDVariableLValue(CodeGenFunction &CGF) override {
    ...
    toolchain_unreachable("No LValue for inlined OpenMP construct");
  }
  ...
  StringRef getHelperName() const override {
    ...
    toolchain_unreachable("No helper name for inlined OpenMP construct");
  }
...
static void EmptyCodeGen(CodeGenFunction &, PrePostActionTy &) {
  toolchain_unreachable("No codegen for expressions");
}
...
public:
  ...
  /// Emit the captured statement body.
  void EmitBody(CodeGenFunction &CGF, const Stmt *S) override {
    toolchain_unreachable("No body for expressions");
  }

  /// Get a variable or parameter for storing global thread id
  /// inside OpenMP construct.
  const VarDecl *getThreadIDVariable() const override {
    toolchain_unreachable("No thread id for expressions");
  }

  /// Get the name of the capture helper.
  StringRef getHelperName() const override {
    toolchain_unreachable("No helper name for expressions");
  }
...
class InlinedOpenMPRegionRAII {
  CodeGenFunction &CGF;
  toolchain::DenseMap<const ValueDecl *, FieldDecl *> LambdaCaptureFields;
...
/// from https://github.com/toolchain/toolchain-project/blob/main/openmp/runtime/src/kmp.h
...
/// https://github.com/toolchain/toolchain-project/blob/main/openmp/runtime/src/kmp.h
/// Original structure:
...
static void emitInitWithReductionInitializer(CodeGenFunction &CGF,
  ...
  if (DRD->getInitializer()) {
    std::pair<toolchain::Function *, toolchain::Function *> Reduction =
...
static void EmitOMPAggregateInit(CodeGenFunction &CGF, Address DestAddr,
  ...
  toolchain::Value *NumElements = CGF.emitArrayLength(ArrayTy, ElementTy, DestAddr);
  if (DRD)
    SrcAddr = SrcAddr.withElementType(DestAddr.getElementType());

  toolchain::Value *SrcBegin = nullptr;
  if (DRD)
    SrcBegin = SrcAddr.emitRawPointer(CGF);
  toolchain::Value *DestBegin = DestAddr.emitRawPointer(CGF);
  // Cast from pointer to array type to pointer to single element.
  toolchain::Value *DestEnd =
      CGF.Builder.CreateGEP(DestAddr.getElementType(), DestBegin, NumElements);
  // The basic structure here is a while-do loop.
  toolchain::BasicBlock *BodyBB = CGF.createBasicBlock("omp.arrayinit.body");
  toolchain::BasicBlock *DoneBB = CGF.createBasicBlock("omp.arrayinit.done");
  toolchain::Value *IsEmpty =
      CGF.Builder.CreateICmpEQ(DestBegin, DestEnd, "omp.arrayinit.isempty");
  CGF.Builder.CreateCondBr(IsEmpty, DoneBB, BodyBB);

  // Enter the loop body, making that address the current address.
  toolchain::BasicBlock *EntryBB = CGF.Builder.GetInsertBlock();
  CGF.EmitBlock(BodyBB);

  CharUnits ElementSize = CGF.getContext().getTypeSizeInChars(ElementTy);

  toolchain::PHINode *SrcElementPHI = nullptr;
  ...
  toolchain::PHINode *DestElementPHI = CGF.Builder.CreatePHI(
  ...
  if (DRD) {
    ...
    toolchain::Value *SrcElementNext = CGF.Builder.CreateConstGEP1_32(
  ...
  // Shift the address forward by one element.
  toolchain::Value *DestElementNext = CGF.Builder.CreateConstGEP1_32(
      DestAddr.getElementType(), DestElementPHI, /*Idx0=*/1,
      "omp.arraycpy.dest.element");
  // Check whether we've reached the end.
  toolchain::Value *Done =
...
void ReductionCodeGen::emitInitialization(
    CodeGenFunction &CGF, unsigned N, Address PrivateAddr, Address SharedAddr,
    toolchain::function_ref<bool(CodeGenFunction &)> DefaultInit) {
...
static Address castToBase(CodeGenFunction &CGF, QualType BaseTy, QualType ElTy,
                          Address OriginalBaseAddress, toolchain::Value *Addr) {
...
CGOpenMPRuntime::CGOpenMPRuntime(CodeGenModule &CGM)
  ...
  KmpCriticalNameTy = toolchain::ArrayType::get(CGM.Int32Ty, /*NumElements*/ 8);
...
void CGOpenMPRuntime::clear() {
  ...
  for (const auto &Data : EmittedNonTargetVariables) {
    if (!Data.getValue().pointsToAliveValue())
      continue;
    auto *GV = dyn_cast<toolchain::GlobalVariable>(Data.getValue());
    if (!GV)
      continue;
...
emitCombinerOrInitializer(CodeGenModule &CGM, QualType Ty,
  ...
  toolchain::FunctionType *FnTy = CGM.getTypes().GetFunctionType(FnInfo);
  std::string Name = CGM.getOpenMPRuntime().getName(
      {IsCombiner ? "omp_combiner" : "omp_initializer", ""});
  auto *Fn = toolchain::Function::Create(FnTy, toolchain::GlobalValue::InternalLinkage,
                                    Name, &CGM.getModule());
  CGM.SetInternalFunctionAttributes(GlobalDecl(), Fn, FnInfo);
  if (CGM.getCodeGenOpts().OptimizationLevel != 0) {
    Fn->removeFnAttr(toolchain::Attribute::NoInline);
    Fn->removeFnAttr(toolchain::Attribute::OptimizeNone);
...
void CGOpenMPRuntime::emitUserDefinedReduction(
  ...
  if (UDRMap.count(D) > 0)
    return;
  toolchain::Function *Combiner = emitCombinerOrInitializer(
      ...
      /*IsCombiner=*/true);
  toolchain::Function *Initializer = nullptr;
...
std::pair<toolchain::Function *, toolchain::Function *>
...
struct PushAndPopStackRAII {
  PushAndPopStackRAII(toolchain::OpenMPIRBuilder *OMPBuilder, CodeGenFunction &CGF,
                      bool HasCancel, toolchain::omp::Directive Kind)
    ...
    auto FiniCB = [&CGF](toolchain::OpenMPIRBuilder::InsertPointTy IP) {
    ...
    toolchain::OpenMPIRBuilder::FinalizationInfo FI({FiniCB, Kind, HasCancel});
...
static toolchain::Function *emitParallelOrTeamsOutlinedFunction(
  ...
  toolchain::OpenMPIRBuilder &OMPBuilder = CGM.getOpenMPRuntime().getOMPBuilder();
...
toolchain::Function *CGOpenMPRuntime::emitParallelOutlinedFunction(
...
toolchain::Function *CGOpenMPRuntime::emitTeamsOutlinedFunction(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    const VarDecl *ThreadIDVar, OpenMPDirectiveKind InnermostKind,
    const RegionCodeGenTy &CodeGen) {
...
toolchain::Function *CGOpenMPRuntime::emitTaskOutlinedFunction(
  ...
  auto &&UntiedCodeGen = [this, &D, TaskTVar](CodeGenFunction &CGF,
                                              PrePostActionTy &) {
    toolchain::Value *ThreadID = getThreadID(CGF, D.getBeginLoc());
    toolchain::Value *UpLoc = emitUpdateLocation(CGF, D.getBeginLoc());
    toolchain::Value *TaskArgs[] = {
        UpLoc, ThreadID,
  ...
  CodeGenFunction::CGCapturedStmtRAII CapInfoRAII(CGF, &CGInfo);
  toolchain::Function *Res = CGF.GenerateCapturedStmtFunction(*CS);
  if (!Tied)
...
void CGOpenMPRuntime::setLocThreadIdInsertPt(CodeGenFunction &CGF,
  ...
  toolchain::Value *Undef = toolchain::UndefValue::get(CGF.Int32Ty);
  if (AtCurrentPoint) {
    Elem.ServiceInsertPt = new toolchain::BitCastInst(Undef, CGF.Int32Ty, "svcpt",
                                                 CGF.Builder.GetInsertBlock());
  } else {
    Elem.ServiceInsertPt = new toolchain::BitCastInst(Undef, CGF.Int32Ty, "svcpt");
...
void CGOpenMPRuntime::clearLocThreadIdInsertPt(CodeGenFunction &CGF) {
  ...
  if (Elem.ServiceInsertPt) {
    toolchain::Instruction *Ptr = Elem.ServiceInsertPt;
...
toolchain::Value *CGOpenMPRuntime::emitUpdateLocation(CodeGenFunction &CGF,
                                                 SourceLocation Loc,
                                                 unsigned Flags, bool EmitLoc) {
  uint32_t SrcLocStrSize;
  toolchain::Constant *SrcLocStr;
  ...
  return OMPBuilder.getOrCreateIdent(
      SrcLocStr, SrcLocStrSize, toolchain::omp::IdentFlag(Flags), Reserved2Flags);
}

toolchain::Value *CGOpenMPRuntime::getThreadID(CodeGenFunction &CGF,
                                          SourceLocation Loc) {
  ...
  toolchain::Value *ThreadID = nullptr;
  ...
  if (auto *OMPRegionInfo =
    ...
    if (OMPRegionInfo->getThreadIDVariable()) {
      ...
      toolchain::BasicBlock *TopBlock = CGF.AllocaInsertPt->getParent();
      if (!CGF.EHStack.requiresLandingPad() || !CGF.getLangOpts().Exceptions ||
          !CGF.getLangOpts().CXXExceptions ||
          CGF.Builder.GetInsertBlock() == TopBlock ||
          !isa<toolchain::Instruction>(LVal.getPointer(CGF)) ||
          cast<toolchain::Instruction>(LVal.getPointer(CGF))->getParent() ==
              TopBlock ||
          cast<toolchain::Instruction>(LVal.getPointer(CGF))->getParent() ==
              CGF.Builder.GetInsertBlock()) {
  ...
  toolchain::CallInst *Call = CGF.Builder.CreateCall(
...
toolchain::Type *CGOpenMPRuntime::getIdentTyPointerTy() {
...
convertDeviceClause(const VarDecl *VD) {
  ...
  if (!DevTy)
    return toolchain::OffloadEntriesInfoManager::OMPTargetDeviceClauseNone;
...
static toolchain::TargetRegionEntryInfo getEntryInfoFromPresumedLoc(
    CodeGenModule &CGM, toolchain::OpenMPIRBuilder &OMPBuilder,
    SourceLocation BeginLoc, toolchain::StringRef ParentName = "") {
...
ConstantAddress CGOpenMPRuntime::getAddrOfDeclareTargetVar(const VarDecl *VD) {
  ...
  toolchain::Type *LlvmPtrTy = CGM.getTypes().ConvertTypeForMem(
      CGM.getContext().getPointerType(VD->getType()));
  toolchain::Constant *addr = OMPBuilder.getAddrOfDeclareTargetVar(
...
Address CGOpenMPRuntime::getAddrOfThreadPrivate(CodeGenFunction &CGF,
  ...
  toolchain::Type *VarTy = VDAddr.getElementType();
...
void CGOpenMPRuntime::emitThreadPrivateVarInit(
    CodeGenFunction &CGF, Address VDAddr, toolchain::Value *Ctor,
    toolchain::Value *CopyCtor, toolchain::Value *Dtor, SourceLocation Loc) {
  // Call kmp_int32 __kmpc_global_thread_num(&loc) to init OpenMP runtime
  // library.
  toolchain::Value *OMPLoc = emitUpdateLocation(CGF, Loc);
  CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
toolchain::Function *CGOpenMPRuntime::emitThreadPrivateVarDefinition(
  ...
  if (VD && ThreadPrivateWithDefinition.insert(CGM.getMangledName(VD)).second) {
    ...
    toolchain::Value *Ctor = nullptr, *CopyCtor = nullptr, *Dtor = nullptr;
    ...
    if (CGM.getLangOpts().CPlusPlus && PerformInit) {
      ...
      toolchain::FunctionType *FTy = CGM.getTypes().GetFunctionType(FI);
      ...
      CtorCGF.StartFunction(GlobalDecl(), CGM.getContext().VoidPtrTy, Fn, FI,
                            Args, Loc, Loc);
      toolchain::Value *ArgVal = CtorCGF.EmitLoadOfScalar(
    ...
    if (VD->getType().isDestructedType() != QualType::DK_none) {
      ...
      toolchain::Value *ArgVal = DtorCGF.EmitLoadOfScalar(
    ...
    CopyCtor = toolchain::Constant::getNullValue(CGM.UnqualPtrTy);
    if (Ctor == nullptr) {
      Ctor = toolchain::Constant::getNullValue(CGM.UnqualPtrTy);
    }
    if (Dtor == nullptr) {
      Dtor = toolchain::Constant::getNullValue(CGM.UnqualPtrTy);
    }
    if (!CGF) {
      auto *InitFunctionTy =
          toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg*/ false);
      std::string Name = getName({"__omp_threadprivate_init_", ""});
      toolchain::Function *InitFunction = CGM.CreateGlobalInitOrCleanUpFunction(
...
void CGOpenMPRuntime::emitDeclareTargetFunction(const FunctionDecl *FD,
  ...
  toolchain::TargetRegionEntryInfo EntryInfo = getEntryInfoFromPresumedLoc(
  ...
  // access its value.
  toolchain::GlobalValue *Addr = GV;
  if (CGM.getLangOpts().OpenMPIsTargetDevice) {
    Addr = new toolchain::GlobalVariable(
        CGM.getModule(), CGM.VoidPtrTy,
        /*isConstant=*/true, toolchain::GlobalValue::ExternalLinkage, GV, Name,
        nullptr, toolchain::GlobalValue::NotThreadLocal,
        CGM.getModule().getDataLayout().getDefaultGlobalsAddressSpace());
    Addr->setVisibility(toolchain::GlobalValue::ProtectedVisibility);
  }
  ...
  OMPBuilder.OffloadInfoManager.registerDeviceGlobalVarEntryInfo(
      ...
      toolchain::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryIndirect,
...
Address CGOpenMPRuntime::getAddrOfArtificialThreadPrivate(CodeGenFunction &CGF,
  ...
  toolchain::Type *VarLVType = CGF.ConvertTypeForMem(VarType);
  toolchain::GlobalVariable *GAddr = OMPBuilder.getOrCreateInternalVariable(
...
void CGOpenMPRuntime::emitIfClause(CodeGenFunction &CGF, const Expr *Cond,
  ...
  toolchain::BasicBlock *ThenBlock = CGF.createBasicBlock("omp_if.then");
  toolchain::BasicBlock *ElseBlock = CGF.createBasicBlock("omp_if.else");
  toolchain::BasicBlock *ContBlock = CGF.createBasicBlock("omp_if.end");
...
void CGOpenMPRuntime::emitParallelCall(CodeGenFunction &CGF, SourceLocation Loc,
                                       toolchain::Function *OutlinedFn,
                                       ArrayRef<toolchain::Value *> CapturedVars,
                                       const Expr *IfCond,
                                       toolchain::Value *NumThreads) {
  if (!CGF.HaveInsertPoint())
    return;
  toolchain::Value *RTLoc = emitUpdateLocation(CGF, Loc);
  ...
  auto &&ThenGen = [&M, OutlinedFn, CapturedVars, RTLoc,
    ...
    toolchain::Value *Args[] = {
        ...
        OutlinedFn};
    toolchain::SmallVector<toolchain::Value *, 16> RealArgs;
    RealArgs.append(std::begin(Args), std::end(Args));
    RealArgs.append(CapturedVars.begin(), CapturedVars.end());

    toolchain::FunctionCallee RTLFn =
  ...
  auto &&ElseGen = [&M, OutlinedFn, CapturedVars, RTLoc, Loc,
    ...
    toolchain::Value *ThreadID = RT.getThreadID(CGF, Loc);
    // Build calls:
    // __kmpc_serialized_parallel(&Loc, GTid);
    toolchain::Value *Args[] = {RTLoc, ThreadID};
    CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
    ...
    toolchain::SmallVector<toolchain::Value *, 16> OutlinedFnArgs;
    ...
    OutlinedFn->removeFnAttr(toolchain::Attribute::AlwaysInline);
    OutlinedFn->addFnAttr(toolchain::Attribute::NoInline);
    RT.emitOutlinedFunctionCall(CGF, Loc, OutlinedFn, OutlinedFnArgs);

    // __kmpc_end_serialized_parallel(&Loc, GTid);
    toolchain::Value *EndArgs[] = {RT.emitUpdateLocation(CGF, Loc), ThreadID};
    CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
Address CGOpenMPRuntime::emitThreadIDAddress(CodeGenFunction &CGF,
  ...
  toolchain::Value *ThreadID = getThreadID(CGF, Loc);
  QualType Int32Ty =
...
toolchain::Value *CGOpenMPRuntime::getCriticalRegionLock(StringRef CriticalName) {
...
class CommonActionTy final : public PrePostActionTy {
  toolchain::FunctionCallee EnterCallee;
  ArrayRef<toolchain::Value *> EnterArgs;
  toolchain::FunctionCallee ExitCallee;
  ArrayRef<toolchain::Value *> ExitArgs;
  bool Conditional;
  toolchain::BasicBlock *ContBlock = nullptr;

public:
  CommonActionTy(toolchain::FunctionCallee EnterCallee,
                 ArrayRef<toolchain::Value *> EnterArgs,
                 toolchain::FunctionCallee ExitCallee,
                 ArrayRef<toolchain::Value *> ExitArgs, bool Conditional = false)
      : EnterCallee(EnterCallee), EnterArgs(EnterArgs), ExitCallee(ExitCallee),
        ExitArgs(ExitArgs), Conditional(Conditional) {}
  void Enter(CodeGenFunction &CGF) override {
    toolchain::Value *EnterRes = CGF.EmitRuntimeCall(EnterCallee, EnterArgs);
    if (Conditional) {
      toolchain::Value *CallBool = CGF.Builder.CreateIsNotNull(EnterRes);
...
void CGOpenMPRuntime::emitCriticalRegion(CodeGenFunction &CGF,
  ...
  if (!CGF.HaveInsertPoint())
    return;
  toolchain::Value *Args[] = {emitUpdateLocation(CGF, Loc), getThreadID(CGF, Loc),
                         getCriticalRegionLock(CriticalName)};
  toolchain::SmallVector<toolchain::Value *, 4> EnterArgs(std::begin(Args),
                                                std::end(Args));
  if (Hint) {
...
void CGOpenMPRuntime::emitMaskedRegion(CodeGenFunction &CGF,
  ...
  toolchain::Value *FilterVal = Filter
                               ? CGF.EmitScalarExpr(Filter, CGF.Int32Ty)
                               : toolchain::ConstantInt::get(CGM.Int32Ty, /*V=*/0);
  toolchain::Value *Args[] = {emitUpdateLocation(CGF, Loc), getThreadID(CGF, Loc),
                         FilterVal};
  toolchain::Value *ArgsEnd[] = {emitUpdateLocation(CGF, Loc),
                            getThreadID(CGF, Loc)};
  CommonActionTy Action(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntime::emitTaskyieldCall(CodeGenFunction &CGF,
  ...
  if (CGF.CGM.getLangOpts().OpenMPIRBuilder) {
    ...
    toolchain::Value *Args[] = {
        emitUpdateLocation(CGF, Loc), getThreadID(CGF, Loc),
        toolchain::ConstantInt::get(CGM.IntTy, /*V=*/0, /*isSigned=*/true)};
    CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
static Address emitAddrOfVarFromArray(CodeGenFunction &CGF, Address Array,
  ...
  toolchain::Value *Ptr = CGF.Builder.CreateLoad(PtrAddr);

  toolchain::Type *ElemTy = CGF.ConvertTypeForMem(Var->getType());
  return Address(Ptr, ElemTy, CGF.getContext().getDeclAlign(Var));
}

static toolchain::Value *emitCopyprivateCopyFunction(
    CodeGenModule &CGM, toolchain::Type *ArgsElemType,
  ...
  auto *Fn = toolchain::Function::Create(CGM.getTypes().GetFunctionType(CGFI),
                                    toolchain::GlobalValue::InternalLinkage, Name,
                                    &CGM.getModule());
...
void CGOpenMPRuntime::emitSingleRegion(CodeGenFunction &CGF,
  ...
  // <copy_func>, did_it);
  if (DidIt.isValid()) {
    toolchain::APInt ArraySize(/*unsigned int numBits=*/32, CopyprivateVars.size());
...
void CGOpenMPRuntime::getDefaultScheduleAndChunk(
  ...
  if (toolchain::any_of(
    ...
    toolchain::APInt ChunkSize(32, 1);
...
void CGOpenMPRuntime::emitBarrierCall(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  // thread_id);
  toolchain::Value *Args[] = {emitUpdateLocation(CGF, Loc, Flags),
                         getThreadID(CGF, Loc)};
  if (OMPRegionInfo) {
    if (!ForceSimpleCall && OMPRegionInfo->hasCancel()) {
      toolchain::Value *Result = CGF.EmitRuntimeCall(
          OMPBuilder.getOrCreateRuntimeFunction(CGM.getModule(),
      ...
      if (EmitChecks) {
        ...
        //   exit from construct;
        // }
        toolchain::BasicBlock *ExitBB = CGF.createBasicBlock(".cancel.exit");
        toolchain::BasicBlock *ContBB = CGF.createBasicBlock(".cancel.continue");
        toolchain::Value *Cmp = CGF.Builder.CreateIsNotNull(Result);
...
static OpenMPSchedType getRuntimeSchedule(OpenMPScheduleClauseKind ScheduleKind,
  ...
  toolchain_unreachable("Unexpected runtime schedule");
}
...
void CGOpenMPRuntime::emitForDispatchInit(
  ...
  toolchain::Value *Chunk = DispatchValues.Chunk ? DispatchValues.Chunk
...
static void emitForStaticInitCall(
    CodeGenFunction &CGF, toolchain::Value *UpdateLocation, toolchain::Value *ThreadId,
    toolchain::FunctionCallee ForStaticInitFunction, OpenMPSchedType Schedule,
  ...
  toolchain::Value *Chunk = Values.Chunk;
  if (Chunk == nullptr) {
...
void CGOpenMPRuntime::emitForStaticInit(CodeGenFunction &CGF,
  ...
  toolchain::Value *UpdatedLocation = emitUpdateLocation(CGF, Loc,
                                             isOpenMPLoopDirective(DKind)
                                                 ? OMP_IDENT_WORK_LOOP
                                                 : OMP_IDENT_WORK_SECTIONS);
  toolchain::Value *ThreadId = getThreadID(CGF, Loc);
  toolchain::FunctionCallee StaticInitFunction =
...
void CGOpenMPRuntime::emitDistributeStaticInit(
  ...
  toolchain::Value *UpdatedLocation =
...
toolchain::Value *CGOpenMPRuntime::emitForNext(CodeGenFunction &CGF,
  ...
  toolchain::Value *Call = CGF.EmitRuntimeCall(
...
void CGOpenMPRuntime::emitProcBindClause(CodeGenFunction &CGF,
  ...
  toolchain::Value *Args[] = {
      emitUpdateLocation(CGF, Loc), getThreadID(CGF, Loc),
      toolchain::ConstantInt::get(CGM.IntTy, unsigned(ProcBind), /*isSigned=*/true)};
  CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntime::emitFlush(CodeGenFunction &CGF, ArrayRef<const Expr *>,
                                SourceLocation Loc, toolchain::AtomicOrdering AO) {
  if (CGF.CGM.getLangOpts().OpenMPIRBuilder) {
...
void CGOpenMPRuntime::createOffloadEntriesAndInfoMetadata() {
  ...
  toolchain::OpenMPIRBuilder::EmitMetadataErrorReportFunctionTy &&ErrorReportFn =
      [this](toolchain::OpenMPIRBuilder::EmitMetadataErrorKind Kind,
             const toolchain::TargetRegionEntryInfo &EntryInfo) -> void {
    SourceLocation Loc;
    if (Kind != toolchain::OpenMPIRBuilder::EMIT_MD_GLOBAL_VAR_LINK_ERROR) {
    ...
    switch (Kind) {
    case toolchain::OpenMPIRBuilder::EMIT_MD_TARGET_REGION_ERROR: {
    ...
    case toolchain::OpenMPIRBuilder::EMIT_MD_DECLARE_TARGET_ERROR: {
      unsigned DiagID = CGM.getDiags().getCustomDiagID(
    ...
    case toolchain::OpenMPIRBuilder::EMIT_MD_GLOBAL_VAR_LINK_ERROR: {
      unsigned DiagID = CGM.getDiags().getCustomDiagID(
...
emitProxyTaskFunction(CodeGenModule &CGM, SourceLocation Loc,
                      ...
                      QualType SharedsPtrTy, toolchain::Function *TaskFunction,
                      toolchain::Value *TaskPrivatesMap) {
  ASTContext &C = CGM.getContext();
  FunctionArgList Args;
  ...
  toolchain::FunctionType *TaskEntryTy =
      CGM.getTypes().GetFunctionType(TaskEntryFnInfo);
  std::string Name = CGM.getOpenMPRuntime().getName({"omp_task_entry", ""});
  auto *TaskEntry = toolchain::Function::Create(
      TaskEntryTy, toolchain::GlobalValue::InternalLinkage, Name, &CGM.getModule());
  ...
  toolchain::Value *GtidParam = CGF.EmitLoadOfScalar(
  ...
  toolchain::Value *PartidParam = PartIdLVal.getPointer(CGF);

  auto SharedsFI = std::next(KmpTaskTQTyRD->field_begin(), KmpTaskTShareds);
  LValue SharedsLVal = CGF.EmitLValueForField(Base, *SharedsFI);
  toolchain::Value *SharedsParam = CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
      CGF.EmitLoadOfScalar(SharedsLVal, Loc),
      CGF.ConvertTypeForMem(SharedsPtrTy));

  auto PrivatesFI = std::next(KmpTaskTWithPrivatesQTyRD->field_begin(), 1);
  toolchain::Value *PrivatesParam;
  if (PrivatesFI != KmpTaskTWithPrivatesQTyRD->field_end()) {
    ...
    PrivatesParam = toolchain::ConstantPointerNull::get(CGF.VoidPtrTy);
  }

  toolchain::Value *CommonArgs[] = {
...
static toolchain::Value *emitDestructorsFunction(CodeGenModule &CGM,
                                            SourceLocation Loc,
                                            QualType KmpInt32Ty,
                                            QualType KmpTaskTWithPrivatesPtrQTy,
  ...
  const auto &DestructorFnInfo =
      CGM.getTypes().arrangeBuiltinFunctionDeclaration(KmpInt32Ty, Args);
  toolchain::FunctionType *DestructorFnTy =
      CGM.getTypes().GetFunctionType(DestructorFnInfo);
  std::string Name =
      CGM.getOpenMPRuntime().getName({"omp_task_destructor", ""});
  auto *DestructorFn =
      toolchain::Function::Create(DestructorFnTy, toolchain::GlobalValue::InternalLinkage,
                             Name, &CGM.getModule());
...
emitTaskPrivateMappingFunction(CodeGenModule &CGM, SourceLocation Loc,
  ...
  toolchain::DenseMap<CanonicalDeclPtr<const VarDecl>, unsigned> PrivateVarsPos;
  unsigned Counter = 1;
  ...
  const auto &TaskPrivatesMapFnInfo =
      CGM.getTypes().arrangeBuiltinFunctionDeclaration(C.VoidTy, Args);
  toolchain::FunctionType *TaskPrivatesMapTy =
      CGM.getTypes().GetFunctionType(TaskPrivatesMapFnInfo);
  std::string Name =
      CGM.getOpenMPRuntime().getName({"omp_task_privates_map", ""});
  auto *TaskPrivatesMap = toolchain::Function::Create(
      TaskPrivatesMapTy, toolchain::GlobalValue::InternalLinkage, Name,
      &CGM.getModule());
  CGM.SetInternalFunctionAttributes(GlobalDecl(), TaskPrivatesMap,
                                    TaskPrivatesMapFnInfo);
  if (CGM.getCodeGenOpts().OptimizationLevel != 0) {
    TaskPrivatesMap->removeFnAttr(toolchain::Attribute::NoInline);
    TaskPrivatesMap->removeFnAttr(toolchain::Attribute::OptimizeNone);
    TaskPrivatesMap->addFnAttr(toolchain::Attribute::AlwaysInline);
  }
  CodeGenFunction CGF(CGM);
...
emitTaskDupFunction(CodeGenModule &CGM, SourceLocation Loc,
  ...
  const auto &TaskDupFnInfo =
      CGM.getTypes().arrangeBuiltinFunctionDeclaration(C.VoidTy, Args);
  toolchain::FunctionType *TaskDupTy = CGM.getTypes().GetFunctionType(TaskDupFnInfo);
  std::string Name = CGM.getOpenMPRuntime().getName({"omp_task_dup", ""});
  auto *TaskDup = toolchain::Function::Create(
      TaskDupTy, toolchain::GlobalValue::InternalLinkage, Name, &CGM.getModule());
  ...
  if (WithLastIter) {
    ...
    LValue LILVal = CGF.EmitLValueForField(Base, *LIFI);
    toolchain::Value *Lastpriv = CGF.EmitLoadOfScalar(
...
public:
  OMPIteratorGeneratorScope(CodeGenFunction &CGF, const OMPIteratorExpr *E)
    ...
    if (!E)
      return;
    SmallVector<toolchain::Value *, 4> Uppers;
    ...
    for (unsigned I = 0, End = E->numOfIterators(); I < End; ++I) {
      ...
      CGF.EmitStoreOfScalar(
          toolchain::ConstantInt::get(CLVal.getAddress().getElementType(), 0),
          CLVal);
      ...
      toolchain::BasicBlock *BodyBB = CGF.createBasicBlock("iter.body");
...
static std::pair<toolchain::Value *, toolchain::Value *>
getPointerAndSize(CodeGenFunction &CGF, const Expr *E) {
  ...
  if (OASE) {
    ...
    toolchain::Value *UpAddr = CGF.Builder.CreateConstGEP1_32(
        UpAddrAddress.getElementType(), UpAddrAddress.emitRawPointer(CGF),
        /*Idx0=*/1);
    toolchain::Value *LowIntPtr = CGF.Builder.CreatePtrToInt(Addr, CGF.SizeTy);
    toolchain::Value *UpIntPtr = CGF.Builder.CreatePtrToInt(UpAddr, CGF.SizeTy);
...
CGOpenMPRuntime::emitTaskInit(CodeGenFunction &CGF, SourceLocation Loc,
                              const OMPExecutableDirective &D,
                              toolchain::Function *TaskFunction, QualType SharedsTy,
                              Address Shareds, const OMPTaskDataTy &Data) {
  ASTContext &C = CGM.getContext();
  toolchain::SmallVector<PrivateDataTy, 4> Privates;
  ...
  toolchain::stable_sort(Privates,
  ...
  toolchain::Type *KmpTaskTWithPrivatesPtrTy = CGF.Builder.getPtrTy(0);
  toolchain::Value *KmpTaskTWithPrivatesTySize =
      CGF.getTypeSize(KmpTaskTWithPrivatesQTy);
  QualType SharedsPtrTy = C.getPointerType(SharedsTy);

  // Emit initial values for private copies (if any).
  toolchain::Value *TaskPrivatesMap = nullptr;
  toolchain::Type *TaskPrivatesMapTy =
  ...
  if (!Privates.empty()) {
    ...
    TaskPrivatesMap = toolchain::ConstantPointerNull::get(
        cast<toolchain::PointerType>(TaskPrivatesMapTy));
  }
  // Build a proxy function kmp_int32 .omp_task_entry.(kmp_int32 gtid,
  // kmp_task_t *tt);
  toolchain::Function *TaskEntry = emitProxyTaskFunction(
  ...
  toolchain::Value *SharedsSize = CGM.getSize(C.getTypeSizeInChars(SharedsTy));
  SmallVector<toolchain::Value *, 8> AllocArgs = {emitUpdateLocation(CGF, Loc),
  ...
  if (const auto *DC = D.getSingleClause<OMPDetachClause>()) {
    ...
    toolchain::Value *Loc = emitUpdateLocation(CGF, DC->getBeginLoc());
    toolchain::Value *Tid = getThreadID(CGF, DC->getBeginLoc());
    Tid = CGF.Builder.CreateIntCast(Tid, CGF.IntTy, /*isSigned=*/false);
    toolchain::Value *EvtVal = CGF.EmitRuntimeCall(
        OMPBuilder.getOrCreateRuntimeFunction(
  ...
  if (D.hasClausesOfKind<OMPAffinityClause>()) {
    ...
    toolchain::Value *NumOfElements = nullptr;
    ...
    for (const auto *C : D.getClausesOfKind<OMPAffinityClause>()) {
      if (const Expr *Modifier = C->getModifier()) {
        ...
        for (unsigned I = 0, E = IE->numOfIterators(); I < E; ++I) {
          toolchain::Value *Sz = CGF.EmitScalarExpr(IE->getHelper(I).Upper);
    ...
    if (NumOfElements) {
      NumOfElements = CGF.Builder.CreateNUWAdd(
          toolchain::ConstantInt::get(CGF.SizeTy, NumAffinities), NumOfElements);
      ...
      KmpTaskAffinityInfoArrayTy = C.getConstantArrayType(
          ...
          toolchain::APInt(C.getTypeSize(C.getSizeType()), NumAffinities), nullptr,
      ...
      NumOfElements = toolchain::ConstantInt::get(CGM.Int32Ty, NumAffinities,
                                             /*isSigned=*/false);
    }
    ...
    if (HasIterator) {
      PosLVal = CGF.MakeAddrLValue(
          ...
          C.getSizeType());
      CGF.EmitStoreOfScalar(toolchain::ConstantInt::get(CGF.SizeTy, Pos), PosLVal);
    }
    ...
    for (const auto *C : D.getClausesOfKind<OMPAffinityClause>()) {
      ...
      for (const Expr *E : C->varlist()) {
        ...
        toolchain::Value *Size;
        std::tie(Addr, Size) = getPointerAndSize(CGF, E);
        toolchain::Value *Idx = CGF.EmitLoadOfScalar(PosLVal, E->getExprLoc());
        LValue Base =
        ...
        Idx = CGF.Builder.CreateNUWAdd(
            Idx, toolchain::ConstantInt::get(Idx->getType(), 1));
    ...
    toolchain::Value *LocRef = emitUpdateLocation(CGF, Loc);
    toolchain::Value *GTid = getThreadID(CGF, Loc);
    toolchain::Value *AffinListPtr = CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
  ...
  toolchain::Value *NewTaskNewTaskTTy =
      CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
  ...
  const RecordDecl *KmpCmplrdataUD = (*FI)
                                         ...
                                         ->getDefinitionOrSelf();
  if (NeedsCleanup) {
    toolchain::Value *DestructorFn = emitDestructorsFunction(
...
static RTLDependenceKindTy translateDependencyKind(OpenMPDependClauseKind K) {
  ...
  case OMPC_DEPEND_unknown:
    toolchain_unreachable("Unknown task dependence type");
  }
  return DepKind;
}
...
std::pair<toolchain::Value *, LValue>
CGOpenMPRuntime::getDepobjElements(CodeGenFunction &CGF, LValue DepobjLVal,
  ...
  Address DepObjAddr = CGF.Builder.CreateGEP(
      CGF, Base.getAddress(),
      toolchain::ConstantInt::get(CGF.IntPtrTy, -1, /*isSigned=*/true));
...
static void emitDependData(CodeGenFunction &CGF, QualType &KmpDependInfoTy,
  ...
  RecordDecl *KmpDependInfoRD =
      cast<RecordDecl>(KmpDependInfoTy->getAsTagDecl());
  toolchain::Type *LLVMFlagsTy = CGF.ConvertTypeForMem(FlagsTy);
  ...
  for (const Expr *E : Data.DepExprs) {
    ...
    if (E) {
      ...
      Addr = toolchain::ConstantInt::get(CGF.IntPtrTy, 0);
      Size = toolchain::ConstantInt::get(CGF.SizeTy, 0);
    }
    LValue Base;
    ...
    CGF.EmitStoreOfScalar(
        toolchain::ConstantInt::get(LLVMFlagsTy, static_cast<unsigned int>(DepKind)),
        FlagsLVal);
    if (unsigned *P = dyn_cast<unsigned *>(Pos)) {
      ++(*P);
    } else {
      ...
      Idx = CGF.Builder.CreateNUWAdd(Idx,
                                     toolchain::ConstantInt::get(Idx->getType(), 1));
      CGF.EmitStoreOfScalar(Idx, PosLVal);
    }
  }
}

SmallVector<toolchain::Value *, 4> CGOpenMPRuntime::emitDepobjElementsSizes(
    CodeGenFunction &CGF, QualType &KmpDependInfoTy,
    const OMPTaskDataTy::DependData &Data) {
  assert(Data.DepKind == OMPC_DEPEND_depobj &&
         "Expected depobj dependency kind.");
  SmallVector<toolchain::Value *, 4> Sizes;
  ...
  ASTContext &C = CGF.getContext();
  {
    ...
    for (const Expr *E : Data.DepExprs) {
      ...
      LValue NumLVal = CGF.MakeAddrLValue(
          ...
          C.getUIntPtrType());
      CGF.Builder.CreateStore(toolchain::ConstantInt::get(CGF.IntPtrTy, 0),
                              NumLVal.getAddress());
      toolchain::Value *PrevVal = CGF.EmitLoadOfScalar(NumLVal, E->getExprLoc());
      toolchain::Value *Add = CGF.Builder.CreateNUWAdd(PrevVal, NumDeps);
...
void CGOpenMPRuntime::emitDepobjElements(CodeGenFunction &CGF,
  ...
  assert(Data.DepKind == OMPC_DEPEND_depobj &&
         "Expected depobj dependency kind.");
  toolchain::Value *ElSize = CGF.getTypeSize(KmpDependInfoTy);
  {
    OMPIteratorGeneratorScope IteratorScope(
        CGF, cast_or_null<OMPIteratorExpr>(
                 Data.IteratorExpr ? Data.IteratorExpr->IgnoreParenImpCasts()
    ...
    for (const Expr *E : Data.DepExprs) {
      ...
      toolchain::Value *Size = CGF.Builder.CreateNUWMul(
          ElSize,
          CGF.Builder.CreateIntCast(NumDeps, CGF.SizeTy, /*isSigned=*/false));
      toolchain::Value *Pos = CGF.EmitLoadOfScalar(PosLVal, E->getExprLoc());
      Address DepAddr = CGF.Builder.CreateGEP(CGF, DependenciesArray, Pos);
      CGF.Builder.CreateMemCpy(DepAddr, Base.getAddress(), Size);

      // Increase pos.
      // pos += size;
      toolchain::Value *Add = CGF.Builder.CreateNUWAdd(Pos, NumDeps);
      CGF.EmitStoreOfScalar(Add, PosLVal);
    }
  }
}

std::pair<toolchain::Value *, Address> CGOpenMPRuntime::emitDependClause(
    CodeGenFunction &CGF, ArrayRef<OMPTaskDataTy::DependData> Dependencies,
    SourceLocation Loc) {
  if (toolchain::all_of(Dependencies, [](const OMPTaskDataTy::DependData &D) {
  ...
  toolchain::Value *NumOfDepobjElements = toolchain::ConstantInt::get(CGF.IntPtrTy, 0);
  toolchain::Value *NumOfRegularWithIterators =
      toolchain::ConstantInt::get(CGF.IntPtrTy, 0);
  ...
  for (const OMPTaskDataTy::DependData &D : Dependencies) {
    if (D.DepKind == OMPC_DEPEND_depobj) {
      ...
      for (toolchain::Value *Size : Sizes) {
        NumOfDepobjElements =
    ...
    if (const auto *IE = cast_or_null<OMPIteratorExpr>(D.IteratorExpr)) {
      toolchain::Value *ClauseIteratorSpace =
          toolchain::ConstantInt::get(CGF.IntPtrTy, 1);
      for (unsigned I = 0, E = IE->numOfIterators(); I < E; ++I) {
        toolchain::Value *Sz = CGF.EmitScalarExpr(IE->getHelper(I).Upper);
      ...
      toolchain::Value *NumClauseDeps = CGF.Builder.CreateNUWMul(
          ClauseIteratorSpace,
          toolchain::ConstantInt::get(CGF.IntPtrTy, D.DepExprs.size()));
  ...
  if (HasDepobjDeps || HasRegularWithIterators) {
    NumOfElements = toolchain::ConstantInt::get(CGM.IntPtrTy, NumDependencies,
                                           /*isSigned=*/false);
    if (HasDepobjDeps) {
      NumOfElements =
    ...
    KmpDependInfoArrayTy = C.getConstantArrayType(
        KmpDependInfoTy, toolchain::APInt(/*numBits=*/64, NumDependencies), nullptr,
        ArraySizeModifier::Normal, /*IndexTypeQuals=*/0);
    DependenciesArray =
        CGF.CreateMemTemp(KmpDependInfoArrayTy, ".dep.arr.addr");
    DependenciesArray = CGF.Builder.CreateConstArrayGEP(DependenciesArray, 0);
    NumOfElements = toolchain::ConstantInt::get(CGM.Int32Ty, NumDependencies,
                                           /*isSigned=*/false);
  }
  unsigned Pos = 0;
...
Address CGOpenMPRuntime::emitDepobjDependClause(
  ...
  if (Dependencies.IteratorExpr) {
    PosLVal = CGF.MakeAddrLValue(
        ...
        C.getSizeType());
    CGF.EmitStoreOfScalar(toolchain::ConstantInt::get(CGF.SizeTy, Idx), PosLVal,
                          /*IsInit=*/true);
    Pos = &PosLVal;
  } else {
    Pos = &Idx;
...
void CGOpenMPRuntime::emitDestroyClause(CodeGenFunction &CGF, LValue DepobjLVal,
  ...
  toolchain::Value *DepObjAddr = CGF.Builder.CreateGEP(
  ...
  // Use default allocator.
  toolchain::Value *Allocator = toolchain::ConstantPointerNull::get(CGF.VoidPtrTy);
  toolchain::Value *Args[] = {ThreadID, DepObjAddr, Allocator};
...
void CGOpenMPRuntime::emitUpdateClause(CodeGenFunction &CGF, LValue DepobjLVal,
  ...
  // Cast from pointer to array type to pointer to single element.
  toolchain::Value *End = CGF.Builder.CreateGEP(Begin.getElementType(),
                                           Begin.emitRawPointer(CGF), NumDeps);
  // The basic structure here is a while-do loop.
  toolchain::BasicBlock *BodyBB = CGF.createBasicBlock("omp.body");
  toolchain::BasicBlock *DoneBB = CGF.createBasicBlock("omp.done");
  toolchain::BasicBlock *EntryBB = CGF.Builder.GetInsertBlock();
  CGF.EmitBlock(BodyBB);
  toolchain::PHINode *ElementPHI =
...
void CGOpenMPRuntime::emitTaskCall(CodeGenFunction &CGF, SourceLocation Loc,
                                   const OMPExecutableDirective &D,
                                   toolchain::Function *TaskFunction,
  ...
  toolchain::Value *NewTask = Result.NewTask;
  toolchain::Function *TaskEntry = Result.TaskEntry;
  toolchain::Value *NewTaskNewTaskTTy = Result.NewTaskNewTaskTTy;
  LValue TDBase = Result.TDBase;
  const RecordDecl *KmpTaskTQTyRD = Result.KmpTaskTQTyRD;
  // Process list of dependences.
  Address DependenciesArray = Address::invalid();
  toolchain::Value *NumOfElements;
  ...
  // list is not empty
  toolchain::Value *ThreadID = getThreadID(CGF, Loc);
  toolchain::Value *UpLoc = emitUpdateLocation(CGF, Loc);
  toolchain::Value *TaskArgs[] = { UpLoc, ThreadID, NewTask };
  toolchain::Value *DepTaskArgs[7];
  if (!Data.Dependences.empty()) {
    ...
    DepTaskArgs[6] = toolchain::ConstantPointerNull::get(CGF.VoidPtrTy);
  }
  ...
  toolchain::Value *DepWaitTaskArgs[7];
  if (!Data.Dependences.empty()) {
    ...
    DepWaitTaskArgs[5] = toolchain::ConstantPointerNull::get(CGF.VoidPtrTy);
    DepWaitTaskArgs[6] =
        toolchain::ConstantInt::get(CGF.Int32Ty, Data.HasNowaitClause);
  }
  auto &M = CGM.getModule();
  auto &&ElseCodeGen = [this, &M, &TaskArgs, ThreadID, NewTaskNewTaskTTy,
    ...
    auto &&CodeGen = [TaskEntry, ThreadID, NewTaskNewTaskTTy,
      ...
      Action.Enter(CGF);
      toolchain::Value *OutlinedFnArgs[] = {ThreadID, NewTaskNewTaskTTy};
...
void CGOpenMPRuntime::emitTaskLoopCall(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  toolchain::Value *ThreadID = getThreadID(CGF, Loc);
  toolchain::Value *UpLoc = emitUpdateLocation(CGF, Loc);
  toolchain::Value *IfVal;
  if (IfCond) {
    IfVal = CGF.Builder.CreateIntCast(CGF.EvaluateExprAsBool(IfCond), CGF.IntTy,
                                      /*isSigned=*/true);
  } else {
    IfVal = toolchain::ConstantInt::getSigned(CGF.IntTy, /*V=*/1);
  }
  ...
  toolchain::SmallVector<toolchain::Value *, 12> TaskArgs{
      UpLoc,
      ThreadID,
      Result.NewTask,
      IfVal,
      ...
      toolchain::ConstantInt::getSigned(
      ...
      Data.Schedule.getPointer()
          ? CGF.Builder.CreateIntCast(Data.Schedule.getPointer(), CGF.Int64Ty,
                                      /*isSigned=*/false)
          : toolchain::ConstantInt::get(CGF.Int64Ty, /*V=*/0)};
  if (Data.HasModifier)
    TaskArgs.push_back(toolchain::ConstantInt::get(CGF.Int32Ty, 1));
...
static void EmitOMPAggregateReduction(
    ...
    const toolchain::function_ref<void(CodeGenFunction &CGF, const Expr *,
  ...
  // Drill down to the base element type on both arrays.
  const ArrayType *ArrayTy = Type->getAsArrayTypeUnsafe();
  toolchain::Value *NumElements = CGF.emitArrayLength(ArrayTy, ElementTy, LHSAddr);

  toolchain::Value *RHSBegin = RHSAddr.emitRawPointer(CGF);
  toolchain::Value *LHSBegin = LHSAddr.emitRawPointer(CGF);
  // Cast from pointer to array type to pointer to single element.
  toolchain::Value *LHSEnd =
      CGF.Builder.CreateGEP(LHSAddr.getElementType(), LHSBegin, NumElements);
  // The basic structure here is a while-do loop.
  toolchain::BasicBlock *BodyBB = CGF.createBasicBlock("omp.arraycpy.body");
  toolchain::BasicBlock *DoneBB = CGF.createBasicBlock("omp.arraycpy.done");
  toolchain::Value *IsEmpty =
  ...
  toolchain::PHINode *RHSElementPHI = CGF.Builder.CreatePHI(
  ...
  toolchain::PHINode *LHSElementPHI = CGF.Builder.CreatePHI(
  ...
  // Shift the address forward by one element.
  toolchain::Value *LHSElementNext = CGF.Builder.CreateConstGEP1_32(
      LHSAddr.getElementType(), LHSElementPHI, /*Idx0=*/1,
      "omp.arraycpy.dest.element");
  toolchain::Value *RHSElementNext = CGF.Builder.CreateConstGEP1_32(
...
toolchain::Function *CGOpenMPRuntime::emitReductionFunction(
    StringRef ReducerName, SourceLocation Loc, toolchain::Type *ArgsElemType,
  ...
  for (unsigned I = 0, E = ReductionOps.size(); I < E; ++I, ++IPriv, ++Idx) {
    ...
    if (PrivTy->isVariablyModifiedType()) {
      ...
      toolchain::Value *Ptr = CGF.Builder.CreateLoad(Elem);
...
static std::string generateUniqueName(CodeGenModule &CGM,
                                      toolchain::StringRef Prefix, const Expr *Ref);
...
void CGOpenMPRuntime::emitPrivateReduction(
  ...
  toolchain::Type *LLVMType = CGF.ConvertTypeForMem(PrivateType);
  ...
  toolchain::GlobalVariable *SharedVar = OMPBuilder.getOrCreateInternalVariable(
      LLVMType, ".omp.reduction." + SharedName);

  SharedVar->setAlignment(
      toolchain::MaybeAlign(CGF.getContext().getTypeAlign(PrivateType) / 8));

  Address SharedResult =
      CGF.MakeNaturalAlignRawAddrLValue(SharedVar, PrivateType).getAddress();

  toolchain::Value *ThreadId = getThreadID(CGF, Loc);
  toolchain::Value *BarrierLoc = emitUpdateLocation(CGF, Loc, OMP_ATOMIC_REDUCE);
  toolchain::Value *BarrierArgs[] = {BarrierLoc, ThreadId};

  toolchain::BasicBlock *InitBB = CGF.createBasicBlock("init");
  toolchain::BasicBlock *InitEndBB = CGF.createBasicBlock("init.end");

  toolchain::Value *IsWorker = CGF.Builder.CreateICmpEQ(
      ThreadId, toolchain::ConstantInt::get(ThreadId->getType(), 0));
  ...
  auto EmitSharedInit = [&]() {
    if (UDR) { // Check if it's a User-Defined Reduction
      if (const Expr *UDRInitExpr = UDR->getInitializer()) {
        std::pair<toolchain::Function *, toolchain::Function *> FnPair =
            getUserDefinedReduction(UDR);
        toolchain::Function *InitializerFn = FnPair.second;
        if (InitializerFn) {
          if (const auto *CE =
  ...
  toolchain::Value *FinalResultVal = nullptr;
...
void CGOpenMPRuntime::emitReduction(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  for (unsigned I : toolchain::seq<unsigned>(
  ...
  toolchain::APInt ArraySize(/*unsigned int numBits=*/32, Size);
  ...
  for (unsigned I = 0, E = RHSExprs.size(); I < E; ++I, ++IPriv, ++Idx) {
    ...
    if ((*IPriv)->getType()->isVariablyModifiedType()) {
      ...
      toolchain::Value *Size = CGF.Builder.CreateIntCast(
          CGF.getVLASize(
  ...
  // 2. Emit reduce_func().
  toolchain::Function *ReductionFn = emitReductionFunction(
      CGF.CurFn->getName(), Loc, CGF.ConvertTypeForMem(ReductionArrayTy),
      Privates, LHSExprs, RHSExprs, ReductionOps);

  // 3. Create static kmp_critical_name lock = { 0 };
  std::string Name = getName({"reduction"});
  toolchain::Value *Lock = getCriticalRegionLock(Name);

  // 4. Build res = __kmpc_reduce{_nowait}(<loc>, <gtid>, <n>, sizeof(RedList),
  // RedList, reduce_func, &<lock>);
  toolchain::Value *IdentTLoc = emitUpdateLocation(CGF, Loc, OMP_ATOMIC_REDUCE);
  toolchain::Value *ThreadId = getThreadID(CGF, Loc);
  toolchain::Value *ReductionArrayTySize = CGF.getTypeSize(ReductionArrayTy);
  toolchain::Value *RL = CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
  ...
  toolchain::Value *Res = CGF.EmitRuntimeCall(
      OMPBuilder.getOrCreateRuntimeFunction(
          CGM.getModule(),
  ...
  // 5. Build switch(res)
  toolchain::BasicBlock *DefaultBB = CGF.createBasicBlock(".omp.reduction.default");
  toolchain::SwitchInst *SwInst =
  ...
  // __kmpc_end_reduce{_nowait}(<loc>, <gtid>, &<lock>);
  // break;
  toolchain::BasicBlock *Case1BB = CGF.createBasicBlock(".omp.reduction.case1");
  SwInst->addCase(CGF.Builder.getInt32(1), Case1BB);
  CGF.EmitBlock(Case1BB);

  // Add emission of __kmpc_end_reduce{_nowait}(<loc>, <gtid>, &<lock>);
  toolchain::Value *EndArgs[] = {
      IdentTLoc, // ident_t *<loc>
      ThreadId,  // i32 <gtid>
      Lock       // kmp_critical_name *&<lock>
  };
  ...
  //  ...
  // break;
  toolchain::BasicBlock *Case2BB = CGF.createBasicBlock(".omp.reduction.case2");
      ...
      if (XExpr) {
        ...
        auto &&AtomicRedGen = [BO, VD,
          ...
          CGF.EmitOMPAtomicSimpleUpdateExpr(
              ...
              toolchain::AtomicOrdering::Monotonic, Loc,
...
static std::string generateUniqueName(CodeGenModule &CGM, StringRef Prefix,
  ...
  const language::Core::DeclRefExpr *DE;
...
/// }
/// \endcode
static toolchain::Value *emitReduceInitFunction(CodeGenModule &CGM,
                                           SourceLocation Loc,
...
/// }
/// \endcode
static toolchain::Value *emitReduceCombFunction(CodeGenModule &CGM,
                                           SourceLocation Loc,
                                           ReductionCodeGen &RCG, unsigned N,
                                           const Expr *ReductionOp,
...
/// }
/// \endcode
static toolchain::Value *emitReduceFiniFunction(CodeGenModule &CGM,
                                           SourceLocation Loc,
                                           ReductionCodeGen &RCG, unsigned N) {
...
toolchain::Value *CGOpenMPRuntime::emitTaskReductionInit(
  ...
  toolchain::APInt ArraySize(/*numBits=*/64, Size);
  ...
  for (unsigned Cnt = 0; Cnt < Size; ++Cnt) {
    ...
    toolchain::Value *Idxs[] = {toolchain::ConstantInt::get(CGM.SizeTy, /*V=*/0),
                           toolchain::ConstantInt::get(CGM.SizeTy, Cnt)};
    toolchain::Value *GEP = CGF.EmitCheckedInBoundsGEP(
    ...
    toolchain::Value *Shared = RCG.getSharedLValue(Cnt).getPointer(CGF);
    ...
    toolchain::Value *Orig = RCG.getOrigLValue(Cnt).getPointer(CGF);
    CGF.EmitStoreOfScalar(Orig, OrigLVal);
    RCG.emitAggregateType(CGF, Cnt);
    toolchain::Value *SizeValInChars;
    toolchain::Value *SizeVal;
    ...
    toolchain::Value *InitAddr = emitReduceInitFunction(CGM, Loc, RCG, Cnt);
    CGF.EmitStoreOfScalar(InitAddr, InitLVal);
    // ElemLVal.reduce_fini = fini;
    LValue FiniLVal = CGF.EmitLValueForField(ElemLVal, FiniFD);
    toolchain::Value *Fini = emitReduceFiniFunction(CGM, Loc, RCG, Cnt);
    toolchain::Value *FiniAddr =
        Fini ? Fini : toolchain::ConstantPointerNull::get(CGM.VoidPtrTy);
    CGF.EmitStoreOfScalar(FiniAddr, FiniLVal);
    // ElemLVal.reduce_comb = comb;
    LValue CombLVal = CGF.EmitLValueForField(ElemLVal, CombFD);
    toolchain::Value *CombAddr = emitReduceCombFunction(
    ...
    if (DelayedCreation) {
      CGF.EmitStoreOfScalar(
          toolchain::ConstantInt::get(CGM.Int32Ty, /*V=*/1, /*isSigned=*/true),
          FlagsLVal);
    } else
  ...
  if (Data.IsReductionWithTaskMod) {
    ...
    toolchain::Value *IdentTLoc = emitUpdateLocation(CGF, Loc);
    toolchain::Value *GTid = CGF.Builder.CreateIntCast(getThreadID(CGF, Loc),
                                                  CGM.IntTy, /*isSigned=*/true);
    toolchain::Value *Args[] = {
        IdentTLoc, GTid,
        toolchain::ConstantInt::get(CGM.IntTy, Data.IsWorksharingReduction ? 1 : 0,
                               /*isSigned=*/true),
        toolchain::ConstantInt::get(CGM.IntTy, Size, /*isSigned=*/true),
        CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
...
void CGOpenMPRuntime::emitTaskReductionFini(CodeGenFunction &CGF,
  ...
  toolchain::Value *GTid = CGF.Builder.CreateIntCast(getThreadID(CGF, Loc),
                                                CGM.IntTy, /*isSigned=*/true);
  toolchain::Value *Args[] = {IdentTLoc, GTid,
                         toolchain::ConstantInt::get(CGM.IntTy,
...
void CGOpenMPRuntime::emitTaskReductionFixups(CodeGenFunction &CGF,
  ...
  if (Sizes.second) {
    toolchain::Value *SizeVal = CGF.Builder.CreateIntCast(Sizes.second, CGM.SizeTy,
                                                     /*isSigned=*/false);
...
Address CGOpenMPRuntime::getTaskReductionItem(CodeGenFunction &CGF,
                                              SourceLocation Loc,
                                              toolchain::Value *ReductionsPtr,
                                              LValue SharedLVal) {
  // Build call void *__kmpc_task_reduction_get_th_data(int gtid, void *tg, void
  // *d);
  toolchain::Value *Args[] = {CGF.Builder.CreateIntCast(getThreadID(CGF, Loc),
                                                   CGM.IntTy,
                                                   /*isSigned=*/true),
                         ReductionsPtr,
                         CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
...
void CGOpenMPRuntime::emitTaskwaitCall(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  if (CGF.CGM.getLangOpts().OpenMPIRBuilder && Data.Dependences.empty()) {
    ...
    if (!Data.Dependences.empty()) {
      ...
      // global_tid);
      toolchain::Value *Args[] = {UpLoc, ThreadID};
...
void CGOpenMPRuntime::emitTargetOutlinedFunction(
    ...
    toolchain::Function *&OutlinedFn, toolchain::Constant *&OutlinedFnID,
...
void CGOpenMPRuntime::emitUsesAllocatorsInit(CodeGenFunction &CGF,
  ...
  toolchain::Value *ThreadId = getThreadID(CGF, Allocator->getExprLoc());
  ThreadId = CGF.Builder.CreateIntCast(ThreadId, CGF.IntTy, /*isSigned=*/true);
  // Use default memspace handle.
  toolchain::Value *MemSpaceHandle = toolchain::ConstantPointerNull::get(CGF.VoidPtrTy);
  toolchain::Value *NumTraits = toolchain::ConstantInt::get(
  ...
  toolchain::Value *Traits = Addr.emitRawPointer(CGF);

  toolchain::Value *AllocatorVal =
      CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntime::emitTargetOutlinedFunctionHelper(
  ...
  CodeGenFunction CGF(CGM, true);
  toolchain::OpenMPIRBuilder::FunctionGenCallback &&GenerateOutlinedFunction =
  ...
  while (const auto *C = dyn_cast_or_null<CompoundStmt>(Child)) {
    ...
    for (const Stmt *S : C->body()) {
      ...
      if (const auto *DS = dyn_cast<DeclStmt>(S)) {
        if (toolchain::all_of(DS->decls(), [](const Decl *D) {
...
const Expr *CGOpenMPRuntime::getNumTeamsExprForTargetDirective(
  ...
  toolchain_unreachable("Unexpected directive kind.");
}

toolchain::Value *CGOpenMPRuntime::emitNumTeamsForTargetDirective(
    CodeGenFunction &CGF, const OMPExecutableDirective &D) {
  ...
  if (NumTeams != nullptr) {
    ...
    case OMPD_target: {
      ...
      CodeGenFunction::CGCapturedStmtRAII CapInfoRAII(CGF, &CGInfo);
      toolchain::Value *NumTeamsVal = CGF.EmitScalarExpr(NumTeams,
  ...
  return toolchain::ConstantInt::get(CGF.Int32Ty, MinNT);
}
...
static void getNumThreads(CodeGenFunction &CGF, const CapturedStmt *CS,
                          ...
                          bool UpperBoundOnly, toolchain::Value **CondVal) {
...
const Expr *CGOpenMPRuntime::getNumThreadsExprForTargetDirective(
    ...
    bool UpperBoundOnly, toolchain::Value **CondVal, const Expr **ThreadLimitExpr) {
  ...
  toolchain_unreachable("Unsupported directive kind.");
}

toolchain::Value *CGOpenMPRuntime::emitNumThreadsForTargetDirective(
    CodeGenFunction &CGF, const OMPExecutableDirective &D) {
  toolchain::Value *NumThreadsVal = nullptr;
  toolchain::Value *CondVal = nullptr;
  toolchain::Value *ThreadLimitVal = nullptr;
...
public:
  ...
  using DeviceInfoTy = toolchain::OpenMPIRBuilder::DeviceInfoTy;
  using MapBaseValuesArrayTy = toolchain::OpenMPIRBuilder::MapValuesArrayTy;
  using MapValuesArrayTy = toolchain::OpenMPIRBuilder::MapValuesArrayTy;
  using MapFlagsArrayTy = toolchain::OpenMPIRBuilder::MapFlagsArrayTy;
  using MapDimArrayTy = toolchain::OpenMPIRBuilder::MapDimArrayTy;
  using MapNonContiguousArrayTy =
      toolchain::OpenMPIRBuilder::MapNonContiguousArrayTy;
  ...
  struct MapCombinedInfoTy : toolchain::OpenMPIRBuilder::MapInfosTy {
...
private:
  ...
  toolchain::PointerUnion<const OMPExecutableDirective *,
  ...
  toolchain::DenseMap<CanonicalDeclPtr<const VarDecl>, bool> FirstPrivateDecls;
  ...
  toolchain::DenseMap<const ValueDecl *, const OMPMapClause *> LambdasMap;

  toolchain::Value *getExprTypeSize(const Expr *E) const {
    ...
    if (const auto *OAE = dyn_cast<ArraySectionExpr>(E)) {
      ...
      if (const Expr *LenExpr = OAE->getLength()) {
        toolchain::Value *LengthVal = CGF.EmitScalarExpr(LenExpr);
      ...
      toolchain::Value *LengthVal = CGF.getTypeSize(BaseTy);
      toolchain::Value *LBVal = CGF.EmitScalarExpr(OAE->getLowerBound());
      ...
      toolchain::Value *Cmp = CGF.Builder.CreateICmpUGT(LengthVal, LBVal);
      toolchain::Value *TrueVal = CGF.Builder.CreateNUWSub(LengthVal, LBVal);
      LengthVal = CGF.Builder.CreateSelect(
          Cmp, TrueVal, toolchain::ConstantInt::get(CGF.SizeTy, 0));
      return LengthVal;
    }
  ...
  OpenMPOffloadMappingFlags getMapTypeBits(
    ...
    case OMPC_MAP_unknown:
      toolchain_unreachable("Unexpected map type!");
    }
    if (AddPtrFlag)
      Bits |= OpenMPOffloadMappingFlags::OMP_MAP_PTR_AND_OBJ;
    if (AddIsTargetParamFlag)
      Bits |= OpenMPOffloadMappingFlags::OMP_MAP_TARGET_PARAM;
    if (toolchain::is_contained(MapModifiers, OMPC_MAP_MODIFIER_always))
      Bits |= OpenMPOffloadMappingFlags::OMP_MAP_ALWAYS;
    if (toolchain::is_contained(MapModifiers, OMPC_MAP_MODIFIER_close))
      Bits |= OpenMPOffloadMappingFlags::OMP_MAP_CLOSE;
    if (toolchain::is_contained(MapModifiers, OMPC_MAP_MODIFIER_present) ||
        toolchain::is_contained(MotionModifiers, OMPC_MOTION_MODIFIER_present))
      Bits |= OpenMPOffloadMappingFlags::OMP_MAP_PRESENT;
    if (toolchain::is_contained(MapModifiers, OMPC_MAP_MODIFIER_ompx_hold))
  ...
  bool isFinalArraySectionExpression(const Expr *E) const {
    ...
    toolchain::APSInt ConstLength = Result.Val.getInt();
  ...
  public:
    ...
    void processField(
        ...
        const FieldDecl *FD,
        toolchain::function_ref<LValue(CodeGenFunction &, const MemberExpr *)>
            EmitMemberExprBase) {
    ...
    void copyUntilField(const FieldDecl *FD, Address ComponentLB) {
      toolchain::Value *ComponentLBPtr = ComponentLB.emitRawPointer(CGF);
      toolchain::Value *LBPtr = LB.emitRawPointer(CGF);
      toolchain::Value *Size =
    ...
    void copyUntilEnd(Address HB) {
      ...
      toolchain::Value *LBPtr = LB.emitRawPointer(CGF);
      toolchain::Value *Size = CGF.Builder.CreatePtrDiff(
          CGF.Int8Ty, CGF.Builder.CreateConstGEP(HB, 1).emitRawPointer(CGF),
          LBPtr);
      copySizedChunk(LBPtr, Size);
    }

    void copySizedChunk(toolchain::Value *Base, toolchain::Value *Size) {
  ...
  void generateInfoForComponentList(
    ...
    for (; I != CE; ++I) {
      ...
      if (Next == CE || IsMemberReference || IsNonDerefPointer ||
        ...
        toolchain::Value *Size = getExprTypeSize(I->getAssociatedExpression());
    ...
    MapValuesArrayTy CurOffsets = {toolchain::ConstantInt::get(CGF.CGM.Int64Ty, 0)};
    MapValuesArrayTy CurCounts = {toolchain::ConstantInt::get(CGF.CGM.Int64Ty, 1)};
    MapValuesArrayTy CurStrides;
    MapValuesArrayTy DimSizes{toolchain::ConstantInt::get(CGF.CGM.Int64Ty, 1)};
    ...
    for (const OMPClauseMappableExprCommon::MappableComponent &Component :
      ...
      if (CurStrides.empty()) {
        ...
        if (ElementType) {
          ...
          CurStrides.push_back(
              toolchain::ConstantInt::get(CGF.Int64Ty, ElementTypeSize));
        }
      }
      ...
      if (DimSizes.size() < Components.size() - 1) {
        if (CAT)
          DimSizes.push_back(
              toolchain::ConstantInt::get(CGF.Int64Ty, CAT->getZExtSize()));
        else if (VAT)
    ...
    toolchain::Value *DimProd =
        toolchain::ConstantInt::get(CGF.CGM.Int64Ty, ElementTypeSize);
    ...
    for (const OMPClauseMappableExprCommon::MappableComponent &Component :
      ...
      if (const auto *AE = dyn_cast<ArraySubscriptExpr>(AssocExpr)) {
        toolchain::Value *Offset = CGF.Builder.CreateIntCast(
            CGF.EmitScalarExpr(AE->getIdx()), CGF.Int64Ty,
            /*isSigned=*/false);
        CurOffsets.push_back(Offset);
        CurCounts.push_back(toolchain::ConstantInt::get(CGF.Int64Ty, /*V=*/1));
      ...
      if (!OffsetExpr) {
        ...
        Offset = toolchain::ConstantInt::get(CGF.Int64Ty, 0);
      } else {
      ...
      if (!CountExpr) {
        ...
        if (!OASE->getColonLocFirst().isValid() &&
          ...
          Count = toolchain::ConstantInt::get(CGF.Int64Ty, 1);
        } else {
  ...
  void getPlainLayout(const CXXRecordDecl *RD,
    ...
    for (const toolchain::PointerUnion<const CXXRecordDecl *, const FieldDecl *>
             &Data : RecordLayout) {
      if (Data.isNull())
        continue;
  ...
  void generateAllInfoForClauses(
      ...
      toolchain::OpenMPIRBuilder &OMPBuilder,
      const toolchain::DenseSet<CanonicalDeclPtr<const Decl>> &SkipVarSet =
          toolchain::DenseSet<CanonicalDeclPtr<const Decl>>()) const {
    // We have to process the component lists that relate with the same
    // declaration in a single chunk so that we can generate the map flags
    // correctly. Therefore, we organize all lists in a map.
    enum MapKind { Present, Allocs, Other, Total };
    toolchain::MapVector<CanonicalDeclPtr<const Decl>,
    ...
    for (const auto *Cl : Clauses) {
      ...
      if (!C)
        continue;
      MapKind Kind = Other;
      if (toolchain::is_contained(C->getMapTypeModifiers(),
    ...
    for (const auto *Cl : Clauses) {
      ...
      if (!C)
        continue;
      MapKind Kind = Other;
      if (toolchain::is_contained(C->getMotionModifiers(),
    ...
    auto &&UseDeviceDataCombinedInfoGen =
        [&UseDeviceDataCombinedInfo](const ValueDecl *VD, toolchain::Value *Ptr,
          ...
          UseDeviceDataCombinedInfo.Sizes.push_back(
              toolchain::Constant::getNullValue(CGF.Int64Ty));
    ...
    auto &&IsMapInfoExist = [&Info](CodeGenFunction &CGF, const ValueDecl *VD,
      ...
      if (It != Info.end()) {
        ...
        for (auto &Data : It->second) {
          auto *CI = toolchain::find_if(Data, [VD](const MapInfo &MI) {
    ...
    toolchain::SmallDenseSet<CanonicalDeclPtr<const Decl>, 4> Processed;
    for (const auto *Cl : Clauses) {
      ...
      if (CI != DeferredInfo.end()) {
        for (const DeferredDevicePtrEntryTy &L : CI->second) {
          ...
          CurInfo.Sizes.push_back(
              toolchain::Constant::getNullValue(this->CGF.Int64Ty));
...
public:
  ...
  void emitCombinedEntry(MapCombinedInfoTy &CombinedInfo,
    ...
    toolchain::Value *LB = LBAddr.emitRawPointer(CGF);
    ...
    if (HasBaseClass) {
      ...
      toolchain::Value *HB = HBAddr.emitRawPointer(CGF);
      toolchain::Value *HAddr = CGF.Builder.CreateConstGEP1_32(
          HBAddr.getElementType(), HB, /*Idx0=*/1);
      toolchain::Value *CLAddr = CGF.Builder.CreatePointerCast(LB, CGF.VoidPtrTy);
      toolchain::Value *CHAddr = CGF.Builder.CreatePointerCast(HAddr, CGF.VoidPtrTy);
      toolchain::Value *Diff = CGF.Builder.CreatePtrDiff(CGF.Int8Ty, CHAddr, CLAddr);
      toolchain::Value *Size = CGF.Builder.CreateIntCast(Diff, CGF.Int64Ty,
                                                    /*isSigned=*/false);
      CombinedInfo.Sizes.push_back(Size);
    }
    CombinedInfo.Mappers.push_back(nullptr);
    ...
    if (CurTypes.end() !=
        toolchain::find_if(CurTypes, [](OpenMPOffloadMappingFlags Type) {
  ...
  void generateAllInfo(
      MapCombinedInfoTy &CombinedInfo, toolchain::OpenMPIRBuilder &OMPBuilder,
      const toolchain::DenseSet<CanonicalDeclPtr<const Decl>> &SkipVarSet =
          toolchain::DenseSet<CanonicalDeclPtr<const Decl>>()) const {
  ...
  void generateAllInfoForMapper(MapCombinedInfoTy &CombinedInfo,
                                toolchain::OpenMPIRBuilder &OMPBuilder) const {
  ...
  void generateInfoForLambdaCaptures(
      const ValueDecl *VD, toolchain::Value *Arg, MapCombinedInfoTy &CombinedInfo,
      toolchain::DenseMap<toolchain::Value *, toolchain::Value *> &LambdaPointers) const {
    ...
    toolchain::DenseMap<const ValueDecl *, FieldDecl *> Captures;
    ...
    for (const LambdaCapture &LC : RD->captures()) {
      ...
      if (LC.getCaptureKind() == LCK_ByRef) {
        ...
        CombinedInfo.Sizes.push_back(toolchain::ConstantInt::get(CGF.Int64Ty, 0));
      }
      CombinedInfo.Types.push_back(
          OpenMPOffloadMappingFlags::OMP_MAP_PTR_AND_OBJ |
          OpenMPOffloadMappingFlags::OMP_MAP_LITERAL |
  ...
  void adjustMemberOfForLambdaCaptures(
      toolchain::OpenMPIRBuilder &OMPBuilder,
      const toolchain::DenseMap<toolchain::Value *, toolchain::Value *> &LambdaPointers,
    ...
    for (unsigned I = 0, E = Types.size(); I < E; ++I) {
      ...
      if (Types[I] != (OpenMPOffloadMappingFlags::OMP_MAP_PTR_AND_OBJ |
                       ...
                       OpenMPOffloadMappingFlags::OMP_MAP_IMPLICIT))
        continue;
      toolchain::Value *BasePtr = LambdaPointers.lookup(BasePointers[I]);
  ...
  void generateInfoForCaptureFromClauseInfo(
      const CapturedStmt::Capture *Cap, toolchain::Value *Arg,
      MapCombinedInfoTy &CurCaptureVarInfo, toolchain::OpenMPIRBuilder &OMPBuilder,
    ...
    toolchain::stable_sort(DeclComponentLists, [](const MapData &LHS,
                                             const MapData &RHS) {
      ArrayRef<OpenMPMapModifierKind> MapModifiers = std::get<2>(LHS);
      OpenMPMapClauseKind MapType = std::get<1>(RHS);
      bool HasPresent =
          toolchain::is_contained(MapModifiers, language::Core::OMPC_MAP_MODIFIER_present);
  ...
  void generateInfoForCaptureFromComponentLists(
    ...
    toolchain::SmallVector<const FieldDecl *, 4> Layout;
    ...
    for (auto &Pair : OverlappedData) {
      toolchain::stable_sort(
          ...
          [&Layout](
            ...
            const auto *It =
                toolchain::find_if(Layout, [FD1, FD2](const FieldDecl *FD) {
  ...
  void generateDefaultMapInfo(const CapturedStmt::Capture &CI,
                              const FieldDecl &RI, toolchain::Value *CV,
    ...
    if (CI.capturesThis()) {
      ...
      if (!RI.getType()->isAnyPointerType()) {
        ...
        CombinedInfo.Sizes.push_back(toolchain::Constant::getNullValue(CGF.Int64Ty));
      }
...
emitMappingInformation(CodeGenFunction &CGF, toolchain::OpenMPIRBuilder &OMPBuilder,
  ...
  if (MapExprs.getMapExpr()) {
    ...
    toolchain::raw_string_ostream OS(ExprName);
...
static void emitOffloadingArraysAndArgs(
    ...
    CGOpenMPRuntime::TargetDataInfo &Info, toolchain::OpenMPIRBuilder &OMPBuilder,
  ...
  auto DeviceAddrCB = [&](unsigned int I, toolchain::Value *NewDecl) {
  ...
  auto CustomMapperCB = [&](unsigned int I) {
    toolchain::Function *MFunc = nullptr;
...
getNestedDistributeDirective(ASTContext &Ctx, const OMPExecutableDirective &D) {
  ...
  if (const auto *NestedDir =
    ...
    case OMPD_unknown:
    default:
      toolchain_unreachable("Unexpected directive.");
    }
  }
...
void CGOpenMPRuntime::emitUserDefinedMapper(const OMPDeclareMapperDecl *D,
  ...
  toolchain::Type *ElemTy = CGM.getTypes().ConvertTypeForMem(Ty);

  CodeGenFunction MapperCGF(CGM);
  MappableExprsHandler::MapCombinedInfoTy CombinedInfo;
  auto PrivatizeAndGenMapInfoCB =
      [&](toolchain::OpenMPIRBuilder::InsertPointTy CodeGenIP, toolchain::Value *PtrPHI,
          toolchain::Value *BeginArg) -> toolchain::OpenMPIRBuilder::MapInfosTy & {
    ...
    if (CGM.getCodeGenOpts().getDebugInfo() !=
      ...
      toolchain::transform(CombinedInfo.Exprs, CombinedInfo.Names.begin(),
                      FillInfoMap);
    }
  ...
  auto CustomMapperCB = [&](unsigned I) {
    toolchain::Function *MapperFunc = nullptr;
    if (CombinedInfo.Mappers[I]) {
  ...
  toolchain::raw_svector_ostream Out(TyStr);
  CGM.getCXXABI().getMangleContext().mangleCanonicalTypeName(Ty, Out);
  std::string Name = getName({"omp_mapper", TyStr, D->getName()});

  toolchain::Function *NewFn = cantFail(OMPBuilder.emitUserDefinedMapper(
...
toolchain::Function *CGOpenMPRuntime::getOrCreateUserDefinedMapperFunc(
...
toolchain::Value *CGOpenMPRuntime::emitTargetNumIterationsCall(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    toolchain::function_ref<toolchain::Value *(CodeGenFunction &CGF,
                                     const OMPLoopDirective &D)>
        SizeEmitter) {
  ...
  if (!TD)
    return toolchain::ConstantInt::get(CGF.Int64Ty, 0);

  const auto *LD = cast<OMPLoopDirective>(TD);
  if (toolchain::Value *NumIterations = SizeEmitter(CGF, *LD))
    return NumIterations;
  return toolchain::ConstantInt::get(CGF.Int64Ty, 0);
}

static void
emitTargetCallFallback(CGOpenMPRuntime *OMPRuntime, toolchain::Function *OutlinedFn,
                       const OMPExecutableDirective &D,
                       toolchain::SmallVectorImpl<toolchain::Value *> &CapturedVars,
...
static toolchain::Value *emitDeviceID(
    toolchain::PointerIntPair<const Expr *, 2, OpenMPDeviceClauseModifier> Device,
    CodeGenFunction &CGF) {
  ...
  if (Device.getPointer()) {
    ...
    toolchain::Value *DevVal = CGF.EmitScalarExpr(Device.getPointer());
    DeviceID =
...
static toolchain::Value *emitDynCGGroupMem(const OMPExecutableDirective &D,
                                      CodeGenFunction &CGF) {
  toolchain::Value *DynCGroupMem = CGF.Builder.getInt32(0);

  if (auto *DynMemClause = D.getSingleClause<OMPXDynCGroupMemClause>()) {
    CodeGenFunction::RunCleanupsScope DynCGroupMemScope(CGF);
    toolchain::Value *DynCGroupMemVal = CGF.EmitScalarExpr(
...
static void genMapInfoForCaptures(
    ...
    const CapturedStmt &CS, toolchain::SmallVectorImpl<toolchain::Value *> &CapturedVars,
    toolchain::OpenMPIRBuilder &OMPBuilder,
    toolchain::DenseSet<CanonicalDeclPtr<const Decl>> &MappedVarSet,
    MappableExprsHandler::MapCombinedInfoTy &CombinedInfo) {

  toolchain::DenseMap<toolchain::Value *, toolchain::Value *> LambdaPointers;
...
genMapInfo(MappableExprsHandler &MEHandler, CodeGenFunction &CGF,
           MappableExprsHandler::MapCombinedInfoTy &CombinedInfo,
           toolchain::OpenMPIRBuilder &OMPBuilder,
           const toolchain::DenseSet<CanonicalDeclPtr<const Decl>> &SkippedVarSet =
               toolchain::DenseSet<CanonicalDeclPtr<const Decl>>()) {
...
emitClauseForBareTargetDirective(CodeGenFunction &CGF,
                                 const OMPExecutableDirective &D,
                                 toolchain::SmallVectorImpl<toolchain::Value *> &Values) {
  ...
  for (auto *E : C->varlist()) {
    toolchain::Value *V = CGF.EmitScalarExpr(E);
    Values.push_back(
...
static void emitTargetCallKernelLaunch(
    CGOpenMPRuntime *OMPRuntime, toolchain::Function *OutlinedFn,
    const OMPExecutableDirective &D,
    toolchain::SmallVectorImpl<toolchain::Value *> &CapturedVars, bool RequiresOuterTask,
    const CapturedStmt &CS, bool OffloadingMandatory,
    toolchain::PointerIntPair<const Expr *, 2, OpenMPDeviceClauseModifier> Device,
    toolchain::Value *OutlinedFnID, CodeGenFunction::OMPTargetDataInfo &InputInfo,
    toolchain::Value *&MapTypesArray, toolchain::Value *&MapNamesArray,
    toolchain::function_ref<toolchain::Value *(CodeGenFunction &CGF,
                                     const OMPLoopDirective &D)>
        SizeEmitter,
    CodeGenFunction &CGF, CodeGenModule &CGM) {
  toolchain::OpenMPIRBuilder &OMPBuilder = OMPRuntime->getOMPBuilder();
  ...
  auto &&ThenGen = [&OMPRuntime, OutlinedFn, &D, &CapturedVars,
    ...
    toolchain::Value *BasePointersArray =
        InputInfo.BasePointersArray.emitRawPointer(CGF);
    toolchain::Value *PointersArray = InputInfo.PointersArray.emitRawPointer(CGF);
    toolchain::Value *SizesArray = InputInfo.SizesArray.emitRawPointer(CGF);
    toolchain::Value *MappersArray = InputInfo.MappersArray.emitRawPointer(CGF);

    auto &&EmitTargetCallFallbackCB =
        [&OMPRuntime, OutlinedFn, &D, &CapturedVars, RequiresOuterTask, &CS,
         OffloadingMandatory, &CGF](toolchain::OpenMPIRBuilder::InsertPointTy IP)
        -> toolchain::OpenMPIRBuilder::InsertPointTy {
      CGF.Builder.restoreIP(IP);
      emitTargetCallFallback(OMPRuntime, OutlinedFn, D, CapturedVars,
                             RequiresOuterTask, CS, OffloadingMandatory, CGF);
    ...
    SmallVector<toolchain::Value *, 3> NumTeams;
    SmallVector<toolchain::Value *, 3> NumThreads;
    if (IsBare) {
    ...
    toolchain::Value *DeviceID = emitDeviceID(Device, CGF);
    toolchain::Value *RTLoc = OMPRuntime->emitUpdateLocation(CGF, D.getBeginLoc());
    toolchain::Value *NumIterations =
        OMPRuntime->emitTargetNumIterationsCall(CGF, D, SizeEmitter);
    toolchain::Value *DynCGGroupMem = emitDynCGGroupMem(D, CGF);
    toolchain::OpenMPIRBuilder::InsertPointTy AllocaIP(
        CGF.AllocaInsertPt->getParent(), CGF.AllocaInsertPt->getIterator());

    toolchain::OpenMPIRBuilder::TargetDataRTArgs RTArgs(
        BasePointersArray, PointersArray, SizesArray, MapTypesArray,
        nullptr /* MapTypesArrayEnd */, MappersArray, MapNamesArray);

    toolchain::OpenMPIRBuilder::TargetKernelArgs Args(
...
static void
emitTargetCallElse(CGOpenMPRuntime *OMPRuntime, toolchain::Function *OutlinedFn,
                   const OMPExecutableDirective &D,
                   toolchain::SmallVectorImpl<toolchain::Value *> &CapturedVars,
                   bool RequiresOuterTask, const CapturedStmt &CS,
                   bool OffloadingMandatory, CodeGenFunction &CGF) {
...
void CGOpenMPRuntime::emitTargetCall(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    toolchain::Function *OutlinedFn, toolchain::Value *OutlinedFnID, const Expr *IfCond,
    toolchain::PointerIntPair<const Expr *, 2, OpenMPDeviceClauseModifier> Device,
    toolchain::function_ref<toolchain::Value *(CodeGenFunction &CGF,
                                     const OMPLoopDirective &D)>
        SizeEmitter) {
  ...
  toolchain::SmallVector<toolchain::Value *, 16> CapturedVars;
  const CapturedStmt &CS = *D.getCapturedStmt(OMPD_target);
  ...
  CodeGenFunction::OMPTargetDataInfo InputInfo;
  toolchain::Value *MapTypesArray = nullptr;
  toolchain::Value *MapNamesArray = nullptr;
...
void CGOpenMPRuntime::scanForTargetRegionsFunctions(const Stmt *S,
  ...
  if (RequiresDeviceCodegen) {
    ...
    case OMPD_unknown:
    default:
      toolchain_unreachable("Unknown target directive for OpenMP device codegen.");
    }
    return;
  }
  ...
  for (const OMPClause *Clause : D->clauselists()) {
    if (Clause->getClauseKind() == OMPC_unified_shared_memory) {
      ...
      case OMPC_ATOMIC_DEFAULT_MEM_ORDER_acq_rel:
        RequiresAtomicOrdering = toolchain::AtomicOrdering::AcquireRelease;
        break;
      case OMPC_ATOMIC_DEFAULT_MEM_ORDER_seq_cst:
        RequiresAtomicOrdering = toolchain::AtomicOrdering::SequentiallyConsistent;
        break;
      case OMPC_ATOMIC_DEFAULT_MEM_ORDER_relaxed:
        RequiresAtomicOrdering = toolchain::AtomicOrdering::Monotonic;
        break;
...
toolchain::AtomicOrdering CGOpenMPRuntime::getDefaultMemoryOrdering() const {
...
bool CGOpenMPRuntime::hasAllocateAttributeForGlobalVar(const VarDecl *VD,
  ...
  case OMPAllocateDeclAttr::OMPUserDefinedMemAlloc:
    toolchain_unreachable("Expected predefined allocator for the variables with the "
                     "static storage.");
  }
  return false;
}
...
bool CGOpenMPRuntime::markAsGlobalTarget(GlobalDecl GD) {
  ...
  if (OMPDeclareTargetDeclAttr::isDeclareTargetDeclaration(D)) {
    if (D->hasBody() && AlreadyEmittedTargetDecls.count(D) == 0) {
      if (auto *F = dyn_cast_or_null<toolchain::Function>(
...
void CGOpenMPRuntime::emitTeamsCall(CodeGenFunction &CGF,
  ...
  toolchain::FunctionCallee RTLFn = OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntime::emitNumTeamsClause(CodeGenFunction &CGF,
  ...
  toolchain::Value *PushNumTeamsArgs[] = {RTLoc, getThreadID(CGF, Loc), NumTeamsVal,
                                     ThreadLimitVal};
  CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntime::emitThreadLimitClause(CodeGenFunction &CGF,
  ...
  toolchain::Value *ThreadLimitArgs[] = {RTLoc, getThreadID(CGF, Loc),
                                    ThreadLimitVal};
  CGF.EmitRuntimeCall(OMPBuilder.getOrCreateRuntimeFunction(
...
void CGOpenMPRuntime::emitTargetDataCalls(
  ...
  toolchain::Value *IfCondVal = nullptr;
  if (IfCond)
    IfCondVal = CGF.EvaluateExprAsBool(IfCond);

  // Emit device ID if any.
  toolchain::Value *DeviceID = nullptr;
  if (Device) {
    DeviceID = CGF.Builder.CreateIntCast(CGF.EmitScalarExpr(Device),
                                         CGF.Int64Ty, /*isSigned=*/true);
  } else {
  ...
  MappableExprsHandler::MapCombinedInfoTy CombinedInfo;
  auto GenMapInfoCB =
      [&](InsertPointTy CodeGenIP) -> toolchain::OpenMPIRBuilder::MapInfosTy & {
  ...
  toolchain::Value *RTLoc = emitUpdateLocation(CGF, D.getBeginLoc());

  InsertPointTy AllocaIP(CGF.AllocaInsertPt->getParent(),
                         CGF.AllocaInsertPt->getIterator());
  InsertPointTy CodeGenIP(CGF.Builder.GetInsertBlock(),
                          CGF.Builder.GetInsertPoint());
  toolchain::OpenMPIRBuilder::LocationDescription OmpLoc(CodeGenIP);
  toolchain::OpenMPIRBuilder::InsertPointTy AfterIP =
...
void CGOpenMPRuntime::emitTargetDataStandAloneCall(
  ...
  auto &&ThenGen = [this, &D, Device, &InputInfo, &MapTypesArray,
    ...
    toolchain::Constant *PointerNum =
        CGF.Builder.getInt32(InputInfo.NumberOfTargetItems);

    // Source location for the ident struct
    toolchain::Value *RTLoc = emitUpdateLocation(CGF, D.getBeginLoc());

    SmallVector<toolchain::Value *, 13> OffloadingArgs(
    ...
    case OMPD_unknown:
    default:
      toolchain_unreachable("Unexpected standalone target data directive.");
      break;
    }
    if (HasNowait) {
      OffloadingArgs.push_back(toolchain::Constant::getNullValue(CGF.Int32Ty));
      OffloadingArgs.push_back(toolchain::Constant::getNullValue(CGF.VoidPtrTy));
      OffloadingArgs.push_back(toolchain::Constant::getNullValue(CGF.Int32Ty));
      OffloadingArgs.push_back(toolchain::Constant::getNullValue(CGF.VoidPtrTy));
    }
    CGF.EmitRuntimeCall(
...
struct ParamAttrTy {
  ...
  toolchain::APSInt StrideOrArg;
  toolchain::APSInt Alignment;
...
static void
emitX86DeclareSimdFunction(const FunctionDecl *FD, toolchain::Function *Fn,
                           const toolchain::APSInt &VLENVal,
                           ArrayRef<ParamAttrTy> ParamAttrs,
  ...
  toolchain::SmallVector<char, 2> Masked;
  switch (State) {
  ...
  for (char Mask : Masked) {
    for (const ISADataTy &Data : ISAData) {
      ...
      if (!VLENVal) {
        ...
        Out << toolchain::APSInt::getUnsigned(Data.VecRegSize / NumElts);
      } else {
        Out << VLENVal;
      }
...
getNDSWDS(const FunctionDecl *FD, ArrayRef<ParamAttrTy> ParamAttrs) {
  ...
  toolchain::SmallVector<unsigned, 8> Sizes;
  ...
  return std::make_tuple(*toolchain::min_element(Sizes), *toolchain::max_element(Sizes),
                         OutputBecomesInput);
}
...
static void addAArch64AdvSIMDNDSNames(unsigned NDS, StringRef Mask,
  ...
  case 128:
    ...
    break;
  default:
    toolchain_unreachable("Scalar type is too wide.");
  }
}
...
static void emitAArch64DeclareSimdFunction(
    ...
    char ISA, unsigned VecRegSize, toolchain::Function *Fn, SourceLocation SLoc) {
  ...
  if (ISA == 'n' && UserVLEN && !toolchain::isPowerOf2_32(UserVLEN)) {
    unsigned DiagID = CGM.getDiags().getCustomDiagID(
...
void CGOpenMPRuntime::emitDeclareSimdFunction(const FunctionDecl *FD,
  ...
  while (FD) {
    ...
    toolchain::DenseMap<const Decl *, unsigned> ParamPositions;
    ...
    for (const auto *Attr : FD->specific_attrs<OMPDeclareSimdDeclAttr>()) {
      toolchain::SmallVector<ParamAttrTy, 8> ParamAttrs(ParamPositions.size());
      ...
      for (const Expr *E : Attr->linears()) {
        ...
        ParamAttr.StrideOrArg = toolchain::APSInt::getUnsigned(1);
        if (*SI) {
          Expr::EvalResult Result;
          if (!(*SI)->EvaluateAsInt(Result, C, Expr::SE_AllowSideEffects)) {
            if (const auto *DRE =
              ...
              if (const auto *StridePVD =
                ...
                ParamAttr.StrideOrArg = toolchain::APSInt::getUnsigned(It->second);
              }
            }
          } else {
      ...
      } else if (CGM.getTriple().getArch() == toolchain::Triple::aarch64) {
...
private:
  toolchain::FunctionCallee RTLFn;
  toolchain::Value *Args[DoacrossFinArgs];

public:
  DoacrossCleanupTy(toolchain::FunctionCallee RTLFn,
                    ArrayRef<toolchain::Value *> CallArgs)
      : RTLFn(RTLFn) {
...
void CGOpenMPRuntime::emitDoacrossInit(CodeGenFunction &CGF,
  ...
  toolchain::APInt Size(/*numBits=*/32, NumIterations.size());
  ...
  for (unsigned I = 0, E = NumIterations.size(); I < E; ++I) {
    ...
    toolchain::Value *NumIterVal = CGF.EmitScalarConversion(
    ...
    CGF.EmitStoreOfScalar(toolchain::ConstantInt::getSigned(CGM.Int64Ty, /*V=*/1),
                          StrideLVal);
  }
  ...
  toolchain::Value *Args[] = {
      ...
      toolchain::ConstantInt::getSigned(CGM.Int32Ty, NumIterations.size()),
      CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
  ...
  CGF.EmitRuntimeCall(RTLFn, Args);
  toolchain::Value *FiniArgs[DoacrossCleanupTy::DoacrossFinArgs] = {
      emitUpdateLocation(CGF, D.getEndLoc()), getThreadID(CGF, D.getEndLoc())};
  toolchain::FunctionCallee FiniRTLFn = OMPBuilder.getOrCreateRuntimeFunction(
      CGM.getModule(), OMPRTL___kmpc_doacross_fini);
  CGF.EHStack.pushCleanup<DoacrossCleanupTy>(NormalAndEHCleanup, FiniRTLFn,
                                             toolchain::ArrayRef(FiniArgs));
}

template <typename T>
static void EmitDoacrossOrdered(CodeGenFunction &CGF, CodeGenModule &CGM,
                                const T *C, toolchain::Value *ULoc,
                                toolchain::Value *ThreadID) {
  QualType Int64Ty =
      CGM.getContext().getIntTypeForBitwidth(/*DestWidth=*/64, /*Signed=*/1);
  toolchain::APInt Size(/*numBits=*/32, C->getNumLoops());
  ...
  for (unsigned I = 0, E = C->getNumLoops(); I < E; ++I) {
    ...
    assert(CounterVal);
    toolchain::Value *CntVal = CGF.EmitScalarConversion(
...
void CGOpenMPRuntime::emitCall(CodeGenFunction &CGF, SourceLocation Loc,
                               toolchain::FunctionCallee Callee,
                               ArrayRef<toolchain::Value *> Args) const {
  assert(Loc.isValid() && "Outlined function call location must be valid.");
  auto DL = ApplyDebugLocation::CreateDefaultArtificial(CGF, Loc);

  if (auto *Fn = dyn_cast<toolchain::Function>(Callee.getCallee())) {
    if (Fn->doesNotThrow()) {
...
void CGOpenMPRuntime::emitOutlinedFunctionCall(
    CodeGenFunction &CGF, SourceLocation Loc, toolchain::FunctionCallee OutlinedFn,
    ArrayRef<toolchain::Value *> Args) const {
...
static toolchain::Value *getAllocatorVal(CodeGenFunction &CGF,
                                    const Expr *Allocator) {
  toolchain::Value *AllocVal;
  if (Allocator) {
    ...
    AllocVal = toolchain::Constant::getNullValue(
...
static toolchain::Value *getAlignmentValue(CodeGenModule &CGM, const VarDecl *VD) {
  std::optional<CharUnits> AllocateAlignment = CGM.getOMPAllocateAlignment(VD);

  if (!AllocateAlignment)
    return nullptr;

  return toolchain::ConstantInt::get(CGM.SizeTy, AllocateAlignment->getQuantity());
}
...
Address CGOpenMPRuntime::getAddressOfLocalVariable(CodeGenFunction &CGF,
  ...
  if (CVD->hasAttr<OMPAllocateDeclAttr>()) {
    ...
    toolchain::Value *ThreadID = getThreadID(CGF, CVD->getBeginLoc());
    const auto *AA = CVD->getAttr<OMPAllocateDeclAttr>();
    const Expr *Allocator = AA->getAllocator();
    toolchain::Value *AllocVal = getAllocatorVal(CGF, Allocator);
    toolchain::Value *Alignment = getAlignmentValue(CGM, CVD);
    ...
    toolchain::omp::RuntimeFunction FnID =
        Alignment ? OMPRTL___kmpc_aligned_alloc : OMPRTL___kmpc_alloc;
    toolchain::Value *Addr = CGF.EmitRuntimeCall(
    ...
    public:
      OMPAllocateCleanupTy(toolchain::FunctionCallee RTLFn,
      ...
      void Emit(CodeGenFunction &CGF, Flags /*flags*/) override {
        ...
        toolchain::Value *AllocVal = getAllocatorVal(CGF, AllocExpr);
        Args[2] = AllocVal;
        CGF.EmitRuntimeCall(RTLFn, Args);
      }
    };
...
CGOpenMPRuntime::UntiedTaskLocalDeclsRAII::UntiedTaskLocalDeclsRAII(
    CodeGenFunction &CGF,
    const toolchain::MapVector<CanonicalDeclPtr<const VarDecl>,
...
void CGOpenMPRuntime::LastprivateConditionalRAII::tryToDisableInnerAnalysis(
    ...
    toolchain::DenseSet<CanonicalDeclPtr<const Decl>> &NeedToAddForLPCsAsDisabled)
    const {
  toolchain::DenseSet<CanonicalDeclPtr<const Decl>> NeedToCheckForLPCs;
  ...
  for (const Decl *VD : NeedToCheckForLPCs) {
    for (const LastprivateConditionalData &Data :
         toolchain::reverse(CGM.getOpenMPRuntime().LastprivateConditionalStack)) {
...
CGOpenMPRuntime::LastprivateConditionalRAII::LastprivateConditionalRAII(
    ...
    : CGM(CGF.CGM),
      Action((CGM.getLangOpts().OpenMP >= 50 &&
              toolchain::any_of(S.getClausesOfKind<OMPLastprivateClause>(),
...
Address CGOpenMPRuntime::emitLastprivateConditionalInit(CodeGenFunction &CGF,
  ...
  CGF.EmitStoreOfScalar(
      toolchain::ConstantInt::getNullValue(CGF.ConvertTypeForMem(C.CharTy)),
      FiredLVal);
...
public:
  ...
  explicit LastprivateConditionalRefChecker(
      ArrayRef<CGOpenMPRuntime::LastprivateConditionalData> LPM)
      : LPM(LPM) {}
  std::tuple<const Expr *, const Decl *, StringRef, LValue, toolchain::Function *>
  getFoundData() const {
...
void CGOpenMPRuntime::emitLastprivateConditionalUpdate(CodeGenFunction &CGF,
  ...
  // int<xx> last_iv = 0;
  toolchain::Type *LLIVTy = CGF.ConvertTypeForMem(IVLVal.getType());
  toolchain::Constant *LastIV = OMPBuilder.getOrCreateInternalVariable(
      LLIVTy, getName({UniqueDeclName, "iv"}));
  cast<toolchain::GlobalVariable>(LastIV)->setAlignment(
  ...
  toolchain::GlobalVariable *Last = OMPBuilder.getOrCreateInternalVariable(
      CGF.ConvertTypeForMem(LVal.getType()), UniqueDeclName);
  cast<toolchain::GlobalVariable>(Last)->setAlignment(
  ...
  // iv
  toolchain::Value *IVVal = CGF.EmitLoadOfScalar(IVLVal, Loc);
  ...
  auto &&CodeGen = [&LastIVLVal, &IVLVal, IVVal, &LVal, &LastLVal,
                    Loc](CodeGenFunction &CGF, PrePostActionTy &Action) {
    Action.Enter(CGF);
    toolchain::Value *LastIVVal = CGF.EmitLoadOfScalar(LastIVLVal, Loc);
    // (last_iv <= iv) ? Check if the variable is updated and store new
    // value in global var.
    toolchain::Value *CmpRes;
    ...
    toolchain::BasicBlock *ThenBB = CGF.createBasicBlock("lp_cond_then");
    toolchain::BasicBlock *ExitBB = CGF.createBasicBlock("lp_cond_exit");
    ...
    case TEK_Scalar: {
      toolchain::Value *PrivVal = CGF.EmitLoadOfScalar(LVal, Loc);
...
void CGOpenMPRuntime::checkAndEmitLastprivateConditional(CodeGenFunction &CGF,
  ...
  if (FoundFn != CGF.CurFn) {
    ...
    CGF.EmitAtomicStore(RValue::get(toolchain::ConstantInt::get(
                            CGF.ConvertTypeForMem(FiredDecl->getType()), 1)),
                        FiredLVal, toolchain::AtomicOrdering::Unordered,
...
void CGOpenMPRuntime::checkAndEmitSharedLastprivateConditional(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    const toolchain::DenseSet<CanonicalDeclPtr<const VarDecl>> &IgnoredDecls) {
  if (CGF.getLangOpts().OpenMP < 50 || LastprivateConditionalStack.empty())
    return;
  auto Range = toolchain::reverse(LastprivateConditionalStack);
  ...
  for (const auto &Pair : It->DeclToUniqueName) {
    ...
    toolchain::Value *Res = CGF.EmitLoadOfScalar(FiredLVal, D.getBeginLoc());
    toolchain::Value *Cmp = CGF.Builder.CreateIsNotNull(Res);
    toolchain::BasicBlock *ThenBB = CGF.createBasicBlock("lpc.then");
    toolchain::BasicBlock *DoneBB = CGF.createBasicBlock("lpc.done");
    // if (Cmp) {
...
void CGOpenMPRuntime::emitLastprivateConditionalFinalUpdate(
  ...
  toolchain::GlobalVariable *GV = CGM.getModule().getNamedGlobal(UniqueName);
  ...
  toolchain::Value *Res = CGF.EmitLoadOfScalar(LPLVal, Loc);
  CGF.EmitStoreOfScalar(Res, PrivLVal);
}

toolchain::Function *CGOpenMPSIMDRuntime::emitParallelOutlinedFunction(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    const VarDecl *ThreadIDVar, OpenMPDirectiveKind InnermostKind,
    const RegionCodeGenTy &CodeGen) {
  toolchain_unreachable("Not supported in SIMD-only mode");
}

toolchain::Function *CGOpenMPSIMDRuntime::emitTeamsOutlinedFunction(
    CodeGenFunction &CGF, const OMPExecutableDirective &D,
    const VarDecl *ThreadIDVar, OpenMPDirectiveKind InnermostKind,
    const RegionCodeGenTy &CodeGen) {
  toolchain_unreachable("Not supported in SIMD-only mode");
...
toolchain::Function *CGOpenMPSIMDRuntime::emitTaskOutlinedFunction(
    const OMPExecutableDirective &D, const VarDecl *ThreadIDVar,
    const VarDecl *PartIDVar, const VarDecl *TaskTVar,
    OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen,
    bool Tied, unsigned &NumberOfParts) {
...
toolchain::Value *CGOpenMPSIMDRuntime::emitForNext(CodeGenFunction &CGF,
                                              SourceLocation Loc,
...
toolchain::Function *CGOpenMPSIMDRuntime::emitThreadPrivateVarDefinition(
...
void CGOpenMPSIMDRuntime::emitTaskLoopCall(
    ...
    toolchain::Function *TaskFunction, QualType SharedsTy, Address Shareds,
...
toolchain::Value *CGOpenMPSIMDRuntime::emitTaskReductionInit(
    CodeGenFunction &CGF, SourceLocation Loc, ArrayRef<const Expr *> LHSExprs,
    ArrayRef<const Expr *> RHSExprs, const OMPTaskDataTy &Data) {
  toolchain_unreachable("Not supported in SIMD-only mode");
}
