...
class LazyRuntimeFunction {
  ...
  toolchain::FunctionType *FTy = nullptr;
  const char *FunctionName = nullptr;
  toolchain::FunctionCallee Function = nullptr;
...
public:
  ...
  void init(CodeGenModule *Mod, const char *name, toolchain::Type *RetTy,
            Tys *... Types) {
    CGM = Mod;
    FunctionName = name;
    Function = nullptr;
    if(sizeof...(Tys)) {
      SmallVector<toolchain::Type *, 8> ArgTys({Types...});
      FTy = toolchain::FunctionType::get(RetTy, ArgTys, false);
    }
    else {
      FTy = toolchain::FunctionType::get(RetTy, {}, false);
    }
  }

  toolchain::FunctionType *getType() { return FTy; }

  /// Overloaded cast operator, allows the class to be implicitly cast to an
  /// LLVM constant.
  operator toolchain::FunctionCallee() {
    if (!Function) {
      if (!FunctionName)
        return nullptr;
...
protected:
  ...
  toolchain::StructType *ObjCSuperTy;
  /// struct objc_super*.  The type of the argument to the superclass message
  /// lookup functions.
  toolchain::PointerType *PtrToObjCSuperTy;
  /// LLVM type for selectors.  Opaque pointer (i8*) unless a header declaring
  /// SEL is included in a header somewhere, in which case it will be whatever
  /// type is declared in that header, most likely {i8*, i8*}.
  toolchain::PointerType *SelectorTy;
  /// Element type of SelectorTy.
  toolchain::Type *SelectorElemTy;
  /// LLVM i8 type.  Cached here to avoid repeatedly getting it in all of the
  /// places where it's used
  toolchain::IntegerType *Int8Ty;
  /// Pointer to i8 - LLVM type of char*, for all of the places where the
  /// runtime needs to deal with C strings.
  toolchain::PointerType *PtrToInt8Ty;
  ...
  toolchain::PointerType *IdTy;
  /// Element type of IdTy.
  toolchain::Type *IdElemTy;
  /// Pointer to a pointer to an Objective-C object.  Used in the new ABI
  /// message lookup function and some GC-related functions.
  toolchain::PointerType *PtrToIdTy;
  ...
  toolchain::Constant *MakeConstantString(StringRef Str, const char *Name = "") {
  ...
  /// few other things.
  toolchain::Constant *ExportUniqueString(const std::string &Str,
                                     const std::string &prefix,
                                     bool Private=false) {
    std::string name = prefix + Str;
    auto *ConstStr = TheModule.getGlobalVariable(name);
    if (!ConstStr) {
      toolchain::Constant *value = toolchain::ConstantDataArray::getString(VMContext,Str);
      auto *GV = new toolchain::GlobalVariable(TheModule, value->getType(), true,
              toolchain::GlobalValue::LinkOnceODRLinkage, value, name);
  ...
  toolchain::Constant *MakePropertyEncodingString(const ObjCPropertyDecl *PD,
  ...
  virtual toolchain::Constant *GenerateCategoryProtocolList(const
  ...
  virtual ConstantArrayBuilder PushPropertyListHeader(ConstantStructBuilder &Fields,
      ...
      if (isRuntime(ObjCRuntime::GNUstep, 2)) {
        const toolchain::DataLayout &DL = TheModule.getDataLayout();
  ...
  virtual void PushProperty(ConstantArrayBuilder &PropertiesArray,
    ...
    auto addPropertyMethod = [&](const ObjCMethodDecl *accessor) {
      if (accessor) {
        ...
        toolchain::Constant *TypeEncoding = MakeConstantString(TypeStr);
  ...
  /// redundant.
  toolchain::Value *EnforceType(CGBuilderTy &B, toolchain::Value *V, toolchain::Type *Ty) {
    if (V->getType() == Ty)
      return V;
  ...
  toolchain::Constant *Zeros[2];
  /// Null pointer value.  Mainly used as a terminator in various arrays.
  toolchain::Constant *NULLPtr;
  /// LLVM context.
...
protected:
  ...
  /// module.
  toolchain::GlobalAlias *ClassPtrAlias;
  ...
  toolchain::StringMap<toolchain::Constant*> ObjCStrings;
  /// All of the protocols that have been declared.
  toolchain::StringMap<toolchain::Constant*> ExistingProtocols;
  ...
  /// value later.
  typedef std::pair<std::string, toolchain::GlobalAlias*> TypedSelector;
  /// Type of the selector map.  This is roughly equivalent to the structure
  /// used in the GNUstep runtime, which maintains a list of all of the valid
  /// types for a selector in a table.
  typedef toolchain::DenseMap<Selector, SmallVector<TypedSelector, 2> >
    SelectorMap;
...
protected:
  ...
  virtual toolchain::Constant *GenerateIvarList(ArrayRef<toolchain::Constant *> IvarNames,
                             ArrayRef<toolchain::Constant *> IvarTypes,
                             ArrayRef<toolchain::Constant *> IvarOffsets,
                             ArrayRef<toolchain::Constant *> IvarAlign,
  ...
  toolchain::Constant *GenerateMethodList(StringRef ClassName,
  ...
  /// real protocol.
  virtual toolchain::Constant *GenerateEmptyProtocol(StringRef ProtocolName);

  /// Generates a list of property metadata structures.  This follows the same
  /// pattern as method and instance variable metadata lists.
  toolchain::Constant *GeneratePropertyList(const Decl *Container,
  ...
  toolchain::Constant *GenerateProtocolList(ArrayRef<std::string> Protocols);
  ...
  toolchain::Constant *GenerateClassStructure(
      toolchain::Constant *MetaClass,
      toolchain::Constant *SuperClass,
      unsigned info,
      const char *Name,
      toolchain::Constant *Version,
      toolchain::Constant *InstanceSize,
      toolchain::Constant *IVars,
      toolchain::Constant *Methods,
      toolchain::Constant *Protocols,
      toolchain::Constant *IvarOffsets,
      toolchain::Constant *Properties,
      toolchain::Constant *StrongIvarBitmap,
      toolchain::Constant *WeakIvarBitmap,
      bool isMeta=false);

  /// Generates a method list.  This is used by protocols to define the required
  /// and optional methods.
  virtual toolchain::Constant *GenerateProtocolMethodList(
      ArrayRef<const ObjCMethodDecl*> Methods);
  /// Emits optional and required method lists.
  template<class T>
  void EmitProtocolMethodList(T &&Methods, toolchain::Constant *&Required,
      toolchain::Constant *&Optional) {
  ...
  virtual toolchain::Value *GetTypedSelector(CodeGenFunction &CGF, Selector Sel,
  ...
  virtual toolchain::Value *GetClassNamed(CodeGenFunction &CGF,
                                     const std::string &Name, bool isWeak);

  /// Looks up the method for sending a message to the specified object.  This
  /// mechanism differs between the GCC and GNU runtimes, so this method must be
  /// overridden in subclasses.
  virtual toolchain::Value *LookupIMP(CodeGenFunction &CGF,
  ...
  virtual toolchain::Value *LookupIMPSuper(CodeGenFunction &CGF,
                                      Address ObjCSuper,
                                      toolchain::Value *cmd,
                                      MessageSendInfo &MSI) = 0;
  ...
  toolchain::Constant *MakeBitField(ArrayRef<bool> bits);
...
public:
  ...
  GenerateMessageSend(CodeGenFunction &CGF, ReturnValueSlot Return,
                      ...
                      toolchain::Value *Receiver, const CallArgList &CallArgs,
  ...
  GenerateMessageSendSuper(CodeGenFunction &CGF, ReturnValueSlot Return,
                           QualType ResultType, Selector Sel,
                           const ObjCInterfaceDecl *Class,
                           bool isCategoryImpl, toolchain::Value *Receiver,
  ...
  toolchain::Value *GetSelector(CodeGenFunction &CGF,
                           const ObjCMethodDecl *Method) override;
  virtual toolchain::Constant *GetConstantSelector(Selector Sel,
                                              const std::string &TypeEncoding) {
    toolchain_unreachable("Runtime unable to generate constant selector");
  }
  toolchain::Constant *GetConstantSelector(const ObjCMethodDecl *M) {
  ...
  toolchain::Function *GenerateMethod(const ObjCMethodDecl *OMD,
  ...
  virtual toolchain::Constant *GenerateProtocolRef(const ObjCProtocolDecl *PD);
...
protected:
  toolchain::Value *LookupIMP(CodeGenFunction &CGF, toolchain::Value *&Receiver,
                         toolchain::Value *cmd, toolchain::MDNode *node,
                         MessageSendInfo &MSI) override {
    CGBuilderTy &Builder = CGF.Builder;
    toolchain::Value *args[] = {
            EnforceType(Builder, Receiver, IdTy),
            EnforceType(Builder, cmd, SelectorTy) };
    toolchain::CallBase *imp = CGF.EmitRuntimeCallOrInvoke(MsgLookupFn, args);
    imp->setMetadata(msgSendMDKind, node);
    return imp;
  }

  toolchain::Value *LookupIMPSuper(CodeGenFunction &CGF, Address ObjCSuper,
                              toolchain::Value *cmd, MessageSendInfo &MSI) override {
    CGBuilderTy &Builder = CGF.Builder;
    toolchain::Value *lookupArgs[] = {
...
class CGObjCGNUstep : public CGObjCGNU {
    ...
    /// lookup functions.
    toolchain::Type *SlotTy;
    /// Type of a slot structure.
    toolchain::Type *SlotStructTy;
  ...
  protected:
    toolchain::Value *LookupIMP(CodeGenFunction &CGF, toolchain::Value *&Receiver,
                           ...
                           MessageSendInfo &MSI) override {
      CGBuilderTy &Builder = CGF.Builder;
      toolchain::FunctionCallee LookupFn = SlotLookupFn;
      ...
      if (isa<ObjCMethodDecl>(CGF.CurCodeDecl)) {
        ...
        self = toolchain::ConstantPointerNull::get(IdTy);
      }

      // The lookup function is guaranteed not to capture the receiver pointer.
      if (auto *LookupFn2 = dyn_cast<toolchain::Function>(LookupFn.getCallee()))
        LookupFn2->addParamAttr(
            0, toolchain::Attribute::getWithCaptureInfo(CGF.getLLVMContext(),
                                                   toolchain::CaptureInfo::none()));

      toolchain::Value *args[] = {
          EnforceType(Builder, ReceiverPtr.getPointer(), PtrToIdTy),
          EnforceType(Builder, cmd, SelectorTy),
          EnforceType(Builder, self, IdTy)};
      toolchain::CallBase *slot = CGF.EmitRuntimeCallOrInvoke(LookupFn, args);
      slot->setOnlyReadsMemory();
      slot->setMetadata(msgSendMDKind, node);

      // Load the imp from the slot
      toolchain::Value *imp = Builder.CreateAlignedLoad(
    ...
    toolchain::Value *LookupIMPSuper(CodeGenFunction &CGF, Address ObjCSuper,
                                ...
                                MessageSendInfo &MSI) override {
      CGBuilderTy &Builder = CGF.Builder;
      toolchain::Value *lookupArgs[] = {ObjCSuper.emitRawPointer(CGF), cmd};

      toolchain::CallInst *slot =
  ...
  public:
    ...
    CGObjCGNUstep(CodeGenModule &Mod, unsigned ABI, unsigned ProtocolABI,
      ...
      SlotStructTy = toolchain::StructType::get(PtrTy, PtrTy, PtrTy, IntTy, IMPTy);
      SlotTy = PtrTy;
      ...
      toolchain::Type *VoidTy = toolchain::Type::getVoidTy(VMContext);
...
class CGObjCGNUstep2 : public CGObjCGNUstep {
  ...
  typedef std::pair<std::string, std::pair<toolchain::GlobalVariable*, int>>
      EarlyInitPair;
  ...
  void CallRuntimeFunction(CGBuilderTy &B, StringRef FunctionName,
    ...
    toolchain::FunctionType *FT = toolchain::FunctionType::get(B.getVoidTy(), Types,
        false);
    toolchain::FunctionCallee Fn = CGM.CreateRuntimeFunction(FT, FunctionName);
  ...
  ConstantAddress GenerateConstantString(const StringLiteral *SL) override {
    ...
    toolchain::StringMap<toolchain::Constant*>::iterator old = ObjCStrings.find(Str);
    ...
    if ((CGM.getTarget().getPointerWidth(LangAS::Default) == 64) &&
      ...
      // Set the tag
      str |= 4;
      auto *ObjCStr = toolchain::ConstantExpr::getIntToPtr(
          toolchain::ConstantInt::get(Int64Ty, str), IdTy);
    ...
    toolchain::Constant *isa = TheModule.getNamedGlobal(Sym);

    if (!isa) {
      isa = new toolchain::GlobalVariable(TheModule, IdTy, /* isConstant */false,
              toolchain::GlobalValue::ExternalLinkage, nullptr, Sym);
      if (CGM.getTriple().isOSBinFormatCOFF()) {
        cast<toolchain::GlobalValue>(isa)->setDLLStorageClass(toolchain::GlobalValue::DLLImportStorageClass);
      }
    }
    ...
    if (isNonASCII) {
      ...
      SmallVector<toolchain::UTF16, 128> ToBuf(NumU8CodeUnits + 1);
      const toolchain::UTF8 *FromPtr = (const toolchain::UTF8 *)Str.data();
      toolchain::UTF16 *ToPtr = &ToBuf[0];
      (void)toolchain::ConvertUTF8toUTF16(&FromPtr, FromPtr + NumU8CodeUnits,
          &ToPtr, ToPtr + NumU8CodeUnits, toolchain::strictConversion);
      ...
      *ToPtr = 0;
      ...
      auto Arr = toolchain::ArrayRef(&ToBuf[0], ToPtr + 1);
      auto *C = toolchain::ConstantDataArray::get(VMContext, Arr);
      auto *Buffer = new toolchain::GlobalVariable(TheModule, C->getType(),
          /*isConstant=*/true, toolchain::GlobalValue::PrivateLinkage, C, ".str");
      Buffer->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
      Fields.add(Buffer);
    } else {
    ...
    toolchain::GlobalVariable *ObjCStrGV =
      Fields.finishAndCreateGlobal(
          isNamed ? StringRef(StringName) : ".objc_string",
          Align, false, isNamed ? toolchain::GlobalValue::LinkOnceODRLinkage
                                : toolchain::GlobalValue::PrivateLinkage);
    ObjCStrGV->setSection(sectionName<ConstantStringSection>());
    if (isNamed) {
      ObjCStrGV->setComdat(TheModule.getOrInsertComdat(StringName));
      ObjCStrGV->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    }
    if (CGM.getTriple().isOSBinFormatCOFF()) {
      std::pair<toolchain::GlobalVariable*, int> v{ObjCStrGV, 0};
  ...
  GenerateProtocolMethodList(ArrayRef<const ObjCMethodDecl*> Methods) override {
    ...
    //   const char *types;
    // };
    toolchain::StructType *ObjCMethodDescTy =
      toolchain::StructType::get(CGM.getLLVMContext(),
  ...
  toolchain::Constant *GenerateCategoryProtocolList(const ObjCCategoryDecl *OCD)
    override {
    ...
    SmallVector<toolchain::Constant *, 16> Protocols;
  ...
  toolchain::GlobalVariable *GetClassVar(StringRef Name, bool isWeak=false) {
    std::string SymbolName = SymbolForClassRef(Name, isWeak);
    auto *ClassSymbol = TheModule.getNamedGlobal(SymbolName);
    if (ClassSymbol)
      return ClassSymbol;
    ClassSymbol = new toolchain::GlobalVariable(TheModule,
        IdTy, false, toolchain::GlobalValue::ExternalLinkage,
        nullptr, SymbolName);
    ...
    if (isWeak)
      ClassSymbol->setInitializer(new toolchain::GlobalVariable(TheModule,
          Int8Ty, false, toolchain::GlobalValue::ExternalWeakLinkage,
      ...
      if (CGM.getTriple().isOSBinFormatCOFF()) {
        ...
        auto Storage = toolchain::GlobalValue::DefaultStorageClass;
        if (OID->hasAttr<DLLImportAttr>())
          Storage = toolchain::GlobalValue::DLLImportStorageClass;
        else if (OID->hasAttr<DLLExportAttr>())
          Storage = toolchain::GlobalValue::DLLExportStorageClass;

        cast<toolchain::GlobalValue>(ClassSymbol)->setDLLStorageClass(Storage);
      }
    }
    assert(ClassSymbol->getName() == SymbolName);
    return ClassSymbol;
  }
  toolchain::Value *GetClassNamed(CodeGenFunction &CGF,
  ...
  toolchain::Constant *GenerateIvarList(ArrayRef<toolchain::Constant *> IvarNames,
                   ArrayRef<toolchain::Constant *> IvarTypes,
                   ArrayRef<toolchain::Constant *> IvarOffsets,
                   ArrayRef<toolchain::Constant *> IvarAlign,
                   ArrayRef<Qualifiers::ObjCLifetime> IvarOwnership) override {
    toolchain_unreachable("Method should not be called!");
  }

  toolchain::Constant *GenerateEmptyProtocol(StringRef ProtocolName) override {
    std::string Name = SymbolForProtocol(ProtocolName);
    auto *GV = TheModule.getGlobalVariable(Name);
    if (!GV) {
      // Emit a placeholder symbol.
      GV = new toolchain::GlobalVariable(TheModule, ProtocolTy, false,
          toolchain::GlobalValue::ExternalLinkage, nullptr, Name);
  ...
  toolchain::StringMap<toolchain::Constant*> ExistingProtocolRefs;
  ...
  toolchain::Value *GenerateProtocolRef(CodeGenFunction &CGF,
    ...
    if (!Ref) {
      ...
      auto GV = new toolchain::GlobalVariable(TheModule, ProtocolPtrTy, false,
  ...
  toolchain::Constant *GenerateProtocolList(ArrayRef<toolchain::Constant*> Protocols) {
    toolchain::ArrayType *ProtocolArrayTy = toolchain::ArrayType::get(ProtocolPtrTy,
        Protocols.size());
    toolchain::Constant * ProtocolArray = toolchain::ConstantArray::get(ProtocolArrayTy,
        Protocols);
    ConstantInitBuilder builder(CGM);
    ...
    return ProtocolBuilder.finishAndCreateGlobal(".objc_protocol_list",
        CGM.getPointerAlign(), false, toolchain::GlobalValue::InternalLinkage);
  }

  void GenerateProtocol(const ObjCProtocolDecl *PD) override {
    // Do nothing - we only emit referenced protocols.
  }
  toolchain::Constant *GenerateProtocolRef(const ObjCProtocolDecl *PD) override {
    ...
    if (const ObjCProtocolDecl *Def = PD->getDefinition())
      ...
      // don't).
      assert(!OldGV);
      Protocol = new toolchain::GlobalVariable(TheModule, ProtocolTy,
        /*isConstant*/false,
        toolchain::GlobalValue::ExternalLinkage, nullptr, SymName);
      return Protocol;
    }
    ...
    for (const auto *PI : RuntimeProtocols)
      Protocols.push_back(GenerateProtocolRef(PI));
    toolchain::Constant *ProtocolList = GenerateProtocolList(Protocols);

    // Collect information about methods
    toolchain::Constant *InstanceMethodList, *OptionalInstanceMethodList;
    toolchain::Constant *ClassMethodList, *OptionalClassMethodList;
    ...
    ConstantInitBuilder builder(CGM);
    auto ProtocolBuilder = builder.beginStruct();
    ProtocolBuilder.add(toolchain::ConstantExpr::getIntToPtr(
          toolchain::ConstantInt::get(Int32Ty, ProtocolVersion), IdTy));
    ...
    auto *GV = ProtocolBuilder.finishAndCreateGlobal(SymName,
        CGM.getPointerAlign(), false, toolchain::GlobalValue::ExternalLinkage);
  ...
  toolchain::Value *GetTypedSelector(CodeGenFunction &CGF, Selector Sel,
  ...
  std::string GetSymbolNameForTypeEncoding(const std::string &TypeEncoding) {
    ...
    if (CGM.getTriple().isOSBinFormatELF())
      toolchain::replace(MangledTypes, '@', '\1');
    // = in dll exported names causes lld to fail when linking on Windows.
    if (CGM.getTriple().isOSWindows())
      toolchain::replace(MangledTypes, '=', '\2');
    return MangledTypes;
  }
  toolchain::Constant  *GetTypeString(toolchain::StringRef TypeEncoding) {
    ...
    if (!TypesGlobal) {
      toolchain::Constant *Init = toolchain::ConstantDataArray::getString(VMContext,
          TypeEncoding);
      auto *GV = new toolchain::GlobalVariable(TheModule, Init->getType(),
          true, toolchain::GlobalValue::LinkOnceODRLinkage, Init, TypesVarName);
  ...
  toolchain::Constant *GetConstantSelector(Selector Sel,
                                      const std::string &TypeEncoding) override {
    ...
    auto *GV = SelBuilder.finishAndCreateGlobal(SelVarName,
        CGM.getPointerAlign(), false, toolchain::GlobalValue::LinkOnceODRLinkage);
    GV->setComdat(TheModule.getOrInsertComdat(SelVarName));
    GV->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    GV->setSection(sectionName<SelectorSection>());
    return GV;
  }
  toolchain::StructType *emptyStruct = nullptr;
  ...
  std::pair<toolchain::Constant*,toolchain::Constant*>
  GetSectionBounds(StringRef Section) {
    if (CGM.getTriple().isOSBinFormatCOFF()) {
      if (emptyStruct == nullptr) {
        emptyStruct = toolchain::StructType::create(
            VMContext, {}, ".objc_section_sentinel", /*isPacked=*/true);
      }
      auto ZeroInit = toolchain::Constant::getNullValue(emptyStruct);
      auto Sym = [&](StringRef Prefix, StringRef SecSuffix) {
        auto *Sym = new toolchain::GlobalVariable(TheModule, emptyStruct,
            /*isConstant*/false,
            toolchain::GlobalValue::LinkOnceODRLinkage, ZeroInit, Prefix +
            Section);
        Sym->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    ...
    auto *Start = new toolchain::GlobalVariable(TheModule, PtrTy,
        /*isConstant*/false,
        toolchain::GlobalValue::ExternalLinkage, nullptr, StringRef("__start_") +
        Section);
    Start->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    auto *Stop = new toolchain::GlobalVariable(TheModule, PtrTy,
        /*isConstant*/false,
        toolchain::GlobalValue::ExternalLinkage, nullptr, StringRef("__stop_") +
        Section);
    Stop->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    return { Start, Stop };
  }
  ...
  toolchain::Function *ModuleInitFunction() override {
    toolchain::Function *LoadFunction = toolchain::Function::Create(
      toolchain::FunctionType::get(toolchain::Type::getVoidTy(VMContext), false),
      toolchain::GlobalValue::LinkOnceODRLinkage, ".objcv2_load_function",
      &TheModule);
    LoadFunction->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    LoadFunction->setComdat(TheModule.getOrInsertComdat(".objcv2_load_function"));

    toolchain::BasicBlock *EntryBB =
        toolchain::BasicBlock::Create(VMContext, "entry", LoadFunction);
    ...
    auto *InitStruct = InitStructBuilder.finishAndCreateGlobal(".objc_init",
        CGM.getPointerAlign(), false, toolchain::GlobalValue::LinkOnceODRLinkage);
    InitStruct->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    ...
    // We have to do this by hand, rather than with @toolchain.ctors, so that the
    // linker can remove the duplicate invocations.
    auto *InitVar = new toolchain::GlobalVariable(TheModule, LoadFunction->getType(),
        /*isConstant*/false, toolchain::GlobalValue::LinkOnceAnyLinkage,
    ...
    InitVar->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    InitVar->setComdat(TheModule.getOrInsertComdat(".objc_ctor"));
    CGM.addUsedGlobal(InitVar);
    for (auto *C : Categories) {
      auto *Cat = cast<toolchain::GlobalVariable>(C->stripPointerCasts());
      Cat->setSection(sectionName<CategorySection>());
      CGM.addUsedGlobal(Cat);
    }
    auto createNullGlobal = [&](StringRef Name, ArrayRef<toolchain::Constant*> Init,
        StringRef Section) {
      auto nullBuilder = builder.beginStruct();
      for (auto *F : Init)
        nullBuilder.add(F);
      auto GV = nullBuilder.finishAndCreateGlobal(Name, CGM.getPointerAlign(),
          false, toolchain::GlobalValue::LinkOnceODRLinkage);
    ...
    if (!CGM.getTriple().isOSBinFormatCOFF()) {
      ...
      if (ConstantStrings.empty()) {
        auto i32Zero = toolchain::ConstantInt::get(Int32Ty, 0);
    ...
    if (EarlyInitList.size() > 0) {
      auto *Init = toolchain::Function::Create(toolchain::FunctionType::get(CGM.VoidTy,
            {}), toolchain::GlobalValue::InternalLinkage, ".objc_early_init",
          &CGM.getModule());
      toolchain::IRBuilder<> b(toolchain::BasicBlock::Create(CGM.getLLVMContext(), "entry",
            Init));
      for (const auto &lateInit : EarlyInitList) {
        auto *global = TheModule.getGlobalVariable(lateInit.first);
        if (global) {
          toolchain::GlobalVariable *GV = lateInit.second.first;
      ...
      auto *InitVar = new toolchain::GlobalVariable(CGM.getModule(), Init->getType(),
          /*isConstant*/true, toolchain::GlobalValue::InternalLinkage,
  ...
  toolchain::Value *EmitIvarOffset(CodeGenFunction &CGF,
    ...
    toolchain::GlobalVariable *IvarOffsetPointer = TheModule.getNamedGlobal(Name);
    if (!IvarOffsetPointer) {
      IvarOffsetPointer = new toolchain::GlobalVariable(TheModule, IntTy, false,
              toolchain::GlobalValue::ExternalLinkage, nullptr, Name);
  ...
  void GenerateClass(const ObjCImplementationDecl *OID) override {
    ...
    toolchain::Constant *SuperClass = nullptr;
    if (SuperClassDecl) {
      auto SuperClassName = SymbolForClass(SuperClassDecl->getNameAsString());
      SuperClass = TheModule.getNamedGlobal(SuperClassName);
      if (!SuperClass)
      {
        SuperClass = new toolchain::GlobalVariable(TheModule, PtrTy, false,
            toolchain::GlobalValue::ExternalLinkage, nullptr, SuperClassName);
        if (IsCOFF) {
          auto Storage = toolchain::GlobalValue::DefaultStorageClass;
          ...
          cast<toolchain::GlobalValue>(SuperClass)->setDLLStorageClass(Storage);
        }
      }
      if (!IsCOFF)
    ...
    if (classDecl->all_declared_ivar_begin() == nullptr)
      ...
      // size_t size;
      toolchain::StructType *ObjCIvarTy = toolchain::StructType::get(
        PtrToInt8Ty,
        PtrToInt8Ty,
        PtrToInt8Ty,
        Int32Ty,
      ...
      for (const ObjCIvarDecl *IVD = classDecl->all_declared_ivar_begin(); IVD;
        ...
        toolchain::Constant *OffsetValue = toolchain::ConstantInt::get(IntTy, Offset);
        std::string OffsetName = GetIVarOffsetVariableName(classDecl, IVD);
        toolchain::GlobalVariable *OffsetVar = TheModule.getGlobalVariable(OffsetName);
        if (OffsetVar)
          OffsetVar->setInitializer(OffsetValue);
        else
          OffsetVar = new toolchain::GlobalVariable(TheModule, IntTy,
        ...
        if (ivarVisibility != toolchain::GlobalValue::HiddenVisibility)
        ...
        unsigned align =
            toolchain::Log2_32(Context.getTypeAlignInChars(ivarTy).getQuantity());
    ...
    toolchain::GlobalVariable *classStruct =
    ...
    if (IsCOFF) {
      ...
      if (OID->getClassInterface()->hasAttr<DLLExportAttr>()) {
        classStruct->setDLLStorageClass(toolchain::GlobalValue::DLLExportStorageClass);
        cast<toolchain::GlobalValue>(classRefSymbol)->setDLLStorageClass(toolchain::GlobalValue::DLLExportStorageClass);
      }

      if (SuperClass) {
        std::pair<toolchain::GlobalVariable*, int> v{classStruct, 1};
    ...
    auto classInitRef = new toolchain::GlobalVariable(TheModule,
        classStruct->getType(), false, toolchain::GlobalValue::ExternalLinkage,
  ...
  public:
    CGObjCGNUstep2(CodeGenModule &Mod) : CGObjCGNUstep(Mod, 10, 4, 2) {
      ...
      SentInitializeFn.init(&CGM, "objc_send_initialize",
                            toolchain::Type::getVoidTy(VMContext), IdTy);
      // struct objc_property
      // {
      //   const char *name;
      //   const char *attributes;
    ...
    void GenerateDirectMethodPrologue(CodeGenFunction &CGF, toolchain::Function *Fn,
      ...
      if (OMD->isClassMethod()) {
        ...
        auto *classStart =
            toolchain::StructType::get(PtrTy, PtrTy, PtrTy, LongTy, LongTy);
        ...
        // https://github.com/toolchain/toolchain-project/pull/102681
        toolchain::Value *Val = Builder.CreateStructGEP(classStart, selfValue, 4);
        ...
        toolchain::BasicBlock *notInitializedBlock =
            CGF.createBasicBlock("objc_direct_method.class_uninitialized");
        toolchain::BasicBlock *initializedBlock =
...
protected:
  ...
  toolchain::Value *GetClassNamed(CodeGenFunction &CGF, const std::string &Name,
                             bool isWeak) override {
    if (isWeak)
    ...
    toolchain::GlobalVariable *ClassSymbol = TheModule.getGlobalVariable(SymbolName);
    if (!ClassSymbol)
      ClassSymbol = new toolchain::GlobalVariable(TheModule, LongTy, false,
...
void CGObjCGNU::EmitClassRef(const std::string &className) {
  ...
  new toolchain::GlobalVariable(TheModule, ClassSymbol->getType(), true,
    toolchain::GlobalValue::WeakAnyLinkage, ClassSymbol, symbolRef);
}
...
CGObjCGNU::CGObjCGNU(CodeGenModule &cgm, unsigned runtimeABIVersion,
  ...
  IntTy = cast<toolchain::IntegerType>(
      Types.ConvertType(CGM.getContext().IntTy));
  LongTy = cast<toolchain::IntegerType>(
      Types.ConvertType(CGM.getContext().LongTy));
  SizeTy = cast<toolchain::IntegerType>(
      Types.ConvertType(CGM.getContext().getSizeType()));
  PtrDiffTy = cast<toolchain::IntegerType>(
      Types.ConvertType(CGM.getContext().getPointerDiffType()));
  BoolTy = CGM.getTypes().ConvertType(CGM.getContext().BoolTy);

  Int8Ty = toolchain::Type::getInt8Ty(VMContext);

  PtrTy = toolchain::PointerType::getUnqual(cgm.getLLVMContext());
  PtrToIntTy = PtrTy;
  // C string type.  Used in lots of places.
  PtrToInt8Ty = PtrTy;
  ProtocolPtrTy = PtrTy;

  Zeros[0] = toolchain::ConstantInt::get(LongTy, 0);
  Zeros[1] = Zeros[0];
  NULLPtr = toolchain::ConstantPointerNull::get(PtrToInt8Ty);
  ...
  if (QualType() == selTy) {
    ...
    SelectorTy = cast<toolchain::PointerType>(CGM.getTypes().ConvertType(selTy));
    SelectorElemTy = CGM.getTypes().ConvertTypeForMem(selTy->getPointeeType());
  }

  Int32Ty = toolchain::Type::getInt32Ty(VMContext);
  Int64Ty = toolchain::Type::getInt64Ty(VMContext);
  ...
  if (UnqualIdTy != QualType()) {
    ...
    IdTy = cast<toolchain::PointerType>(CGM.getTypes().ConvertType(ASTIdTy));
  ...
  PtrToIdTy = PtrTy;
  ProtocolTy = toolchain::StructType::get(IdTy,
      PtrToInt8Ty, // name
  ...
  // }
  PropertyMetadataTy = toolchain::StructType::get(CGM.getLLVMContext(), {
      PtrToInt8Ty, Int8Ty, Int8Ty, Int8Ty, Int8Ty, PtrToInt8Ty, PtrToInt8Ty,
      PtrToInt8Ty, PtrToInt8Ty });

  ObjCSuperTy = toolchain::StructType::get(IdTy, IdTy);
...
toolchain::Value *CGObjCGNU::GetClassNamed(CodeGenFunction &CGF,
                                      const std::string &Name, bool isWeak) {
  toolchain::Constant *ClassName = MakeConstantString(Name);
  ...
  toolchain::FunctionCallee ClassLookupFn = CGM.CreateRuntimeFunction(
      toolchain::FunctionType::get(IdTy, PtrToInt8Ty, true), "objc_lookup_class");
  return CGF.EmitNounwindRuntimeCall(ClassLookupFn, ClassName);
}

// This has to perform the lookup every time, since posing and related
// techniques can modify the name -> class mapping.
toolchain::Value *CGObjCGNU::GetClass(CodeGenFunction &CGF,
                                 const ObjCInterfaceDecl *OID) {
  auto *Value =
      GetClassNamed(CGF, OID->getNameAsString(), OID->isWeakImported());
  if (auto *ClassSymbol = dyn_cast<toolchain::GlobalVariable>(Value))
    CGM.setGVProperties(ClassSymbol, OID);
  return Value;
}

toolchain::Value *CGObjCGNU::EmitNSAutoreleasePoolClassRef(CodeGenFunction &CGF) {
...
toolchain::Value *CGObjCGNU::GetTypedSelector(CodeGenFunction &CGF, Selector Sel,
                                         const std::string &TypeEncoding) {
  SmallVectorImpl<TypedSelector> &Types = SelectorTable[Sel];
  toolchain::GlobalAlias *SelValue = nullptr;
  ...
  if (!SelValue) {
    SelValue = toolchain::GlobalAlias::create(SelectorElemTy, 0,
                                         toolchain::GlobalValue::PrivateLinkage,
...
Address CGObjCGNU::GetAddrOfSelector(CodeGenFunction &CGF, Selector Sel) {
  toolchain::Value *SelValue = GetSelector(CGF, Sel);
...
toolchain::Value *CGObjCGNU::GetSelector(CodeGenFunction &CGF, Selector Sel) {
  return GetTypedSelector(CGF, Sel, std::string());
}

toolchain::Value *CGObjCGNU::GetSelector(CodeGenFunction &CGF,
...
toolchain::Constant *CGObjCGNU::GetEHType(QualType T) {
...
toolchain::Constant *CGObjCGNUstep::GetEHType(QualType T) {
  ...
  if (T->isObjCIdType() ||
    ...
    toolchain::Constant *IDEHType =
      CGM.getModule().getGlobalVariable("__objc_id_type_info");
    if (!IDEHType)
      IDEHType =
        new toolchain::GlobalVariable(CGM.getModule(), PtrToInt8Ty,
                                 false,
                                 toolchain::GlobalValue::ExternalLinkage,
  ...
  if (toolchain::Constant *typeinfo = TheModule.getGlobalVariable(typeinfoName))
    return typeinfo;
  ...
  if (!Vtable) {
    Vtable = new toolchain::GlobalVariable(TheModule, PtrToInt8Ty, true,
                                      toolchain::GlobalValue::ExternalLinkage,
                                      nullptr, vtableName);
  }
  toolchain::Constant *Two = toolchain::ConstantInt::get(IntTy, 2);
  auto *BVtable =
      toolchain::ConstantExpr::getGetElementPtr(Vtable->getValueType(), Vtable, Two);

  toolchain::Constant *typeName =
...
ConstantAddress CGObjCGNU::GenerateConstantString(const StringLiteral *SL) {
  ...
  toolchain::Constant *ObjCStr = Fields.finishAndCreateGlobal(".objc_str", Align);
  ObjCStrings[Str] = ObjCStr;
...
CGObjCGNU::GenerateMessageSendSuper(CodeGenFunction &CGF,
  ...
  toolchain::Value *cmd = GetSelector(CGF, Sel);
  ...
  toolchain::Value *ReceiverClass = nullptr;
  ...
  if (isV2ABI) {
    ...
    if (isCategoryImpl) {
      toolchain::FunctionCallee classLookupFunction = nullptr;
      if (IsClassMessage)  {
        classLookupFunction = CGM.CreateRuntimeFunction(toolchain::FunctionType::get(
      ...
      if (IsClassMessage)  {
        if (!MetaClassPtrAlias) {
          MetaClassPtrAlias = toolchain::GlobalAlias::create(
              IdElemTy, 0, toolchain::GlobalValue::InternalLinkage,
              ".objc_metaclass_ref" + Class->getNameAsString(), &TheModule);
        }
        ReceiverClass = MetaClassPtrAlias;
      } else {
        if (!ClassPtrAlias) {
          ClassPtrAlias = toolchain::GlobalAlias::create(
              IdElemTy, 0, toolchain::GlobalValue::InternalLinkage,
    ...
    toolchain::Type *CastTy = toolchain::StructType::get(IdTy, IdTy);
  ...
  toolchain::StructType *ObjCSuperTy =
      toolchain::StructType::get(Receiver->getType(), IdTy);
  ...
  // Get the IMP
  toolchain::Value *imp = LookupIMPSuper(CGF, ObjCSuper, cmd, MSI);
  imp = EnforceType(Builder, imp, MSI.MessengerType);

  toolchain::Metadata *impMD[] = {
      toolchain::MDString::get(VMContext, Sel.getAsString()),
      toolchain::MDString::get(VMContext, Class->getSuperClass()->getNameAsString()),
      toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::get(
          toolchain::Type::getInt1Ty(VMContext), IsClassMessage))};
  toolchain::MDNode *node = toolchain::MDNode::get(VMContext, impMD);
...
CGObjCGNU::GenerateMessageSend(CodeGenFunction &CGF,
  ...
  toolchain::Metadata *impMD[] = {
      toolchain::MDString::get(VMContext, Sel.getAsString()),
      toolchain::MDString::get(VMContext, Class ? Class->getNameAsString() : ""),
      toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::get(
          toolchain::Type::getInt1Ty(VMContext), Class != nullptr))};
  toolchain::MDNode *node = toolchain::MDNode::get(VMContext, impMD);
  ...
  if (requiresNilReceiverCheck) {
    ...
    toolchain::Value *isNil = Builder.CreateICmpEQ(Receiver,
            toolchain::Constant::getNullValue(Receiver->getType()));
  ...
  if (isDirect)
    ...
    case CodeGenOptions::NonLegacy:
      ...
      // function anyway
      imp = CGM.CreateRuntimeFunction(toolchain::FunctionType::get(IdTy, IdTy, true),
                                      name)
                .getCallee();
    }
  ...
  if (requiresNilReceiverCheck) {
    toolchain::BasicBlock *nonNilPathBB = CGF.Builder.GetInsertBlock();
    ...
    if (msgRet.isScalar()) {
      ...
      if (toolchain::Value *v = msgRet.getScalarVal()) {
        toolchain::PHINode *phi = Builder.CreatePHI(v->getType(), 2);
      ...
      std::pair<toolchain::Value*,toolchain::Value*> v = msgRet.getComplexVal();
      toolchain::PHINode *phi = Builder.CreatePHI(v.first->getType(), 2);
      phi->addIncoming(v.first, nonNilPathBB);
      phi->addIncoming(toolchain::Constant::getNullValue(v.first->getType()),
                       nilPathBB);
      toolchain::PHINode *phi2 = Builder.CreatePHI(v.second->getType(), 2);
      phi2->addIncoming(v.second, nonNilPathBB);
      phi2->addIncoming(toolchain::Constant::getNullValue(v.second->getType()),
                        nilPathBB);
...
GenerateMethodList(StringRef ClassName,
  ...
  toolchain::StructType *ObjCMethodTy =
    toolchain::StructType::get(CGM.getLLVMContext(), {
...
toolchain::Constant *CGObjCGNU::
GenerateIvarList(ArrayRef<toolchain::Constant *> IvarNames,
                 ArrayRef<toolchain::Constant *> IvarTypes,
                 ArrayRef<toolchain::Constant *> IvarOffsets,
                 ArrayRef<toolchain::Constant *> IvarAlign,
                 ArrayRef<Qualifiers::ObjCLifetime> IvarOwnership) {
  ...
  toolchain::StructType *ObjCIvarTy =
      toolchain::StructType::get(PtrToInt8Ty, PtrToInt8Ty, IntTy);
...
toolchain::Constant *CGObjCGNU::GenerateClassStructure(
    toolchain::Constant *MetaClass,
    toolchain::Constant *SuperClass,
    unsigned info,
    const char *Name,
  ...
  // be ignored.
  toolchain::StructType *ClassTy = toolchain::StructType::get(
      PtrToInt8Ty,        // isa
  ...
  std::string ClassSym((isMeta ? "_OBJC_METACLASS_": "_OBJC_CLASS_") +
          std::string(Name));
  toolchain::GlobalVariable *ClassRef = TheModule.getNamedGlobal(ClassSym);
  toolchain::Constant *Class =
...
GenerateProtocolMethodList(ArrayRef<const ObjCMethodDecl*> Methods) {
  // Get the method structure type.
  toolchain::StructType *ObjCMethodDescTy =
    toolchain::StructType::get(CGM.getLLVMContext(), { PtrToInt8Ty, PtrToInt8Ty });
  ASTContext &Context = CGM.getContext();
  ConstantInitBuilder Builder(CGM);
  auto MethodList = Builder.beginStruct();
  MethodList.addInt(IntTy, Methods.size());
...
CGObjCGNU::GenerateProtocolList(ArrayRef<std::string> Protocols) {
  ...
  for (const std::string &Protocol : Protocols) {
    toolchain::Constant *protocol = nullptr;
    toolchain::StringMap<toolchain::Constant *>::iterator value =
...
toolchain::Value *CGObjCGNU::GenerateProtocolRef(CodeGenFunction &CGF,
                                            const ObjCProtocolDecl *PD) {
  return GenerateProtocolRef(PD);
}

toolchain::Constant *CGObjCGNU::GenerateProtocolRef(const ObjCProtocolDecl *PD) {
  toolchain::Constant *&protocol = ExistingProtocols[PD->getNameAsString()];
  if (!protocol)
    GenerateProtocol(PD);
...
CGObjCGNU::GenerateEmptyProtocol(StringRef ProtocolName) {
  toolchain::Constant *ProtocolList = GenerateProtocolList({});
  toolchain::Constant *MethodList = GenerateProtocolMethodList({});
  ...
  // The isa pointer must be set to a magic number so the runtime knows it's
  // the correct layout.
  Elements.add(toolchain::ConstantExpr::getIntToPtr(
          toolchain::ConstantInt::get(Int32Ty, ProtocolVersion), IdTy));
...
void CGObjCGNU::GenerateProtocol(const ObjCProtocolDecl *PD) {
  ...
  toolchain::Constant *ProtocolList = GenerateProtocolList(Protocols);
  toolchain::Constant *InstanceMethodList =
    GenerateProtocolMethodList(InstanceMethods);
  toolchain::Constant *ClassMethodList =
    GenerateProtocolMethodList(ClassMethods);
  toolchain::Constant *OptionalInstanceMethodList =
    GenerateProtocolMethodList(OptionalInstanceMethods);
  toolchain::Constant *OptionalClassMethodList =
  ...
  toolchain::Constant *OptionalPropertyList =
...
toolchain::Constant *CGObjCGNU::MakeBitField(ArrayRef<bool> bits) {
  ...
  if (bitCount < ptrBits) {
    ...
    return toolchain::ConstantInt::get(IntPtrTy, val);
  }
  SmallVector<toolchain::Constant *, 8> values;
  int v=0;
  while (v < bitCount) {
    int32_t word = 0;
    ...
    values.push_back(toolchain::ConstantInt::get(Int32Ty, word));
  }
  ...
  toolchain::Constant *GS =
    fields.finishAndCreateGlobal("", CharUnits::fromQuantity(4));
  toolchain::Constant *ptr = toolchain::ConstantExpr::getPtrToInt(GS, IntPtrTy);
  return ptr;
}

toolchain::Constant *CGObjCGNU::GenerateCategoryProtocolList(const
    ObjCCategoryDecl *OCD) {
...
toolchain::Constant *CGObjCGNU::GeneratePropertyList(const Decl *Container,
    const ObjCContainerDecl *OCD,
...
void CGObjCGNU::GenerateClass(const ObjCImplementationDecl *OID) {
  ...
  if (auto *symbol = TheModule.getGlobalVariable(classSymbolName)) {
    symbol->setInitializer(toolchain::ConstantInt::get(LongTy, 0));
  } else {
    new toolchain::GlobalVariable(TheModule, LongTy, false,
                             toolchain::GlobalValue::ExternalLinkage,
                             toolchain::ConstantInt::get(LongTy, 0),
                             classSymbolName);
  }
  ...
  SmallVector<toolchain::Constant*, 16> IvarNames;
  SmallVector<toolchain::Constant*, 16> IvarTypes;
  SmallVector<toolchain::Constant*, 16> IvarOffsets;
  SmallVector<toolchain::Constant*, 16> IvarAligns;
  ...
  for (const ObjCIvarDecl *IVD = ClassDecl->all_declared_ivar_begin(); IVD;
      ...
      IvarAligns.push_back(toolchain::ConstantInt::get(IntTy,
      ...
      if (OffsetVar) {
        ...
        // copy.
        OffsetVar->setLinkage(toolchain::GlobalValue::ExternalLinkage);
      } else
        OffsetVar = new toolchain::GlobalVariable(TheModule, Int32Ty,
          false, toolchain::GlobalValue::ExternalLinkage,
          OffsetValue, OffsetName);
  ...
  toolchain::Constant *StrongIvarBitmap = MakeBitField(StrongIvars);
  toolchain::Constant *WeakIvarBitmap = MakeBitField(WeakIvars);
  toolchain::GlobalVariable *IvarOffsetArray =
  ...
  toolchain::Constant *Properties = GeneratePropertyList(OID, ClassDecl);
  ...
  if (!SuperClassName.empty()) {
    ...
    SuperClass = toolchain::ConstantPointerNull::get(PtrToInt8Ty);
  }
  // Generate the method and instance variable lists
  toolchain::Constant *MethodList = GenerateMethodList(ClassName, "",
      InstanceMethods, false);
  toolchain::Constant *ClassMethodList = GenerateMethodList(ClassName, "",
      ClassMethods, true);
  toolchain::Constant *IvarList = GenerateIvarList(IvarNames, IvarTypes,
  ...
  toolchain::Type *IndexTy = Int32Ty;
  toolchain::Constant *offsetPointerIndexes[] = {Zeros[0],
      toolchain::ConstantInt::get(IndexTy, ClassABIVersion > 1 ? 2 : 1), nullptr,
      toolchain::ConstantInt::get(IndexTy, ClassABIVersion > 1 ? 3 : 2) };

  unsigned ivarIndex = 0;
  for (const ObjCIvarDecl *IVD = ClassDecl->all_declared_ivar_begin(); IVD;
       IVD = IVD->getNextIvar()) {
      const std::string Name = GetIVarOffsetVariableName(ClassDecl, IVD);
      offsetPointerIndexes[2] = toolchain::ConstantInt::get(IndexTy, ivarIndex);
      // Get the correct ivar field
      toolchain::Constant *offsetValue = toolchain::ConstantExpr::getGetElementPtr(
          cast<toolchain::GlobalVariable>(IvarList)->getValueType(), IvarList,
          offsetPointerIndexes);
      // Get the existing variable, if one exists.
      toolchain::GlobalVariable *offset = TheModule.getNamedGlobal(Name);
      if (offset) {
        offset->setInitializer(offsetValue);
        // If this is the real definition, change its linkage type so that
        // different modules will use this one, rather than their private
        // copy.
        offset->setLinkage(toolchain::GlobalValue::ExternalLinkage);
      } else
        // Add a new alias if there isn't one already.
        new toolchain::GlobalVariable(TheModule, offsetValue->getType(),
                false, toolchain::GlobalValue::ExternalLinkage, offsetValue, Name);
      ++ivarIndex;
  }
  toolchain::Constant *ZeroPtr = toolchain::ConstantInt::get(IntPtrTy, 0);

  //Generate metaclass for class methods
  toolchain::Constant *MetaClassStruct = GenerateClassStructure(
  ...
  CGM.setGVProperties(cast<toolchain::GlobalValue>(MetaClassStruct),
                      OID->getClassInterface());

  // Generate the class structure
  toolchain::Constant *ClassStruct = GenerateClassStructure(
      MetaClassStruct, SuperClass, 0x11L, ClassName.c_str(), nullptr,
      toolchain::ConstantInt::get(LongTy, instanceSize), IvarList, MethodList,
      GenerateProtocolList(Protocols), IvarOffsetArray, Properties,
      StrongIvarBitmap, WeakIvarBitmap);
  CGM.setGVProperties(cast<toolchain::GlobalValue>(ClassStruct),
                      OID->getClassInterface());
...
toolchain::Function *CGObjCGNU::ModuleInitFunction() {
  ...
  toolchain::StructType *selStructTy = dyn_cast<toolchain::StructType>(SelectorElemTy);
  if (!selStructTy) {
    selStructTy = toolchain::StructType::get(CGM.getLLVMContext(),
                                        { PtrToInt8Ty, PtrToInt8Ty });
  }

  // Generate statics list:
  toolchain::Constant *statics = NULLPtr;
  if (!ConstantStrings.empty()) {
    toolchain::GlobalVariable *fileStatics = [&] {
      ConstantInitBuilder builder(CGM);
  ...
  SmallVector<toolchain::GlobalAlias*, 16> selectorAliases;
  unsigned selectorCount;

  // Pointer to an array of selectors used in this module.
  toolchain::GlobalVariable *selectorList = [&] {
    ConstantInitBuilder builder(CGM);
    ...
    toolchain::sort(allSelectors);

    for (auto &untypedSel : allSelectors) {
      std::string selNameStr = untypedSel.getAsString();
      toolchain::Constant *selName = ExportUniqueString(selNameStr, ".objc_sel_name");

      for (TypedSelector &sel : table[untypedSel]) {
        toolchain::Constant *selectorTypeEncoding = NULLPtr;
        if (!sel.first.empty())
  ...
  for (unsigned i = 0; i < selectorCount; ++i) {
    toolchain::Constant *idxs[] = {
      Zeros[0],
      toolchain::ConstantInt::get(Int32Ty, i)
    };
    // FIXME: We're generating redundant loads and stores here!
    toolchain::Constant *selPtr = toolchain::ConstantExpr::getGetElementPtr(
        selectorList->getValueType(), selectorList, idxs);
    selectorAliases[i]->replaceAllUsesWith(selPtr);
    selectorAliases[i]->eraseFromParent();
  }

  toolchain::GlobalVariable *symtab = [&] {
    ConstantInitBuilder builder(CGM);
  ...
  // constants
  toolchain::Constant *module = [&] {
    toolchain::Type *moduleEltTys[] = {
      LongTy, LongTy, PtrToInt8Ty, symtab->getType(), IntTy
    };
    toolchain::StructType *moduleTy = toolchain::StructType::get(
        CGM.getLLVMContext(),
  ...
  toolchain::Function * LoadFunction = toolchain::Function::Create(
      toolchain::FunctionType::get(toolchain::Type::getVoidTy(VMContext), false),
      toolchain::GlobalValue::InternalLinkage, ".objc_load_function",
      &TheModule);
  toolchain::BasicBlock *EntryBB =
      toolchain::BasicBlock::Create(VMContext, "entry", LoadFunction);
  ...
  toolchain::FunctionType *FT =
    toolchain::FunctionType::get(Builder.getVoidTy(), module->getType(), true);
  toolchain::FunctionCallee Register =
      CGM.CreateRuntimeFunction(FT, "__objc_exec_class");
  Builder.CreateCall(Register, module);

  if (!ClassAliases.empty()) {
    toolchain::Type *ArgTypes[2] = {PtrTy, PtrToInt8Ty};
    toolchain::FunctionType *RegisterAliasTy =
      toolchain::FunctionType::get(Builder.getVoidTy(),
                              ArgTypes, false);
    toolchain::Function *RegisterAlias = toolchain::Function::Create(
      RegisterAliasTy,
      toolchain::GlobalValue::ExternalWeakLinkage, "class_registerAlias_np",
      &TheModule);
    toolchain::BasicBlock *AliasBB =
      toolchain::BasicBlock::Create(VMContext, "alias", LoadFunction);
    toolchain::BasicBlock *NoAliasBB =
      toolchain::BasicBlock::Create(VMContext, "no_alias", LoadFunction);

    // Branch based on whether the runtime provided class_registerAlias_np()
    toolchain::Value *HasRegisterAlias = Builder.CreateICmpNE(RegisterAlias,
            toolchain::Constant::getNullValue(RegisterAlias->getType()));
...
toolchain::Function *CGObjCGNU::GenerateMethod(const ObjCMethodDecl *OMD,
  ...
  if (!isDirect)
    return toolchain::Function::Create(MethodTy,
  ...
  if (I == DirectMethodDefinitions.end()) {
    auto *F =
        toolchain::Function::Create(MethodTy, toolchain::GlobalVariable::ExternalLinkage,
                               FunctionName, &TheModule);
...
toolchain::FunctionCallee CGObjCGNU::GetPropertyGetFunction() {
  return GetPropertyFn;
}

toolchain::FunctionCallee CGObjCGNU::GetPropertySetFunction() {
  return SetPropertyFn;
}

toolchain::FunctionCallee CGObjCGNU::GetOptimizedPropertySetFunction(bool atomic,
                                                                bool copy) {
  return nullptr;
}

toolchain::FunctionCallee CGObjCGNU::GetGetStructFunction() {
  return GetStructPropertyFn;
}

toolchain::FunctionCallee CGObjCGNU::GetSetStructFunction() {
  return SetStructPropertyFn;
}

toolchain::FunctionCallee CGObjCGNU::GetCppAtomicObjectGetFunction() {
  return nullptr;
}

toolchain::FunctionCallee CGObjCGNU::GetCppAtomicObjectSetFunction() {
  return nullptr;
}

toolchain::FunctionCallee CGObjCGNU::EnumerationMutationFunction() {
...
void CGObjCGNU::EmitThrowStmt(CodeGenFunction &CGF,
  ...
  if (isRethrow && (usesSEHExceptions || usesCxxExceptions)) {
    ...
    // argument.
    toolchain::CallBase *Throw = CGF.EmitRuntimeCallOrInvoke(ExceptionReThrowFn);
    Throw->setDoesNotReturn();
  } else {
    ExceptionAsObject = CGF.Builder.CreateBitCast(ExceptionAsObject, IdTy);
    toolchain::CallBase *Throw =
...
toolchain::Value * CGObjCGNU::EmitObjCWeakRead(CodeGenFunction &CGF,
                                          Address AddrWeakObj) {
...
void CGObjCGNU::EmitObjCWeakAssign(CodeGenFunction &CGF,
  ...
  toolchain::Value *dstVal = EnforceType(B, dst.emitRawPointer(CGF), PtrToIdTy);
...
void CGObjCGNU::EmitObjCIvarAssign(CodeGenFunction &CGF,
  ...
  CGBuilderTy &B = CGF.Builder;
  src = EnforceType(B, src, IdTy);
  toolchain::Value *dstVal = EnforceType(B, dst.emitRawPointer(CGF), IdTy);
...
void CGObjCGNU::EmitGCMemmoveCollectable(CodeGenFunction &CGF,
  ...
  CGBuilderTy &B = CGF.Builder;
  toolchain::Value *DestPtrVal = EnforceType(B, DestPtr.emitRawPointer(CGF), PtrTy);
  toolchain::Value *SrcPtrVal = EnforceType(B, SrcPtr.emitRawPointer(CGF), PtrTy);

  B.CreateCall(MemMoveFn, {DestPtrVal, SrcPtrVal, Size});
}

toolchain::GlobalVariable *CGObjCGNU::ObjCIvarOffsetVariable(
  ...
  toolchain::GlobalVariable *IvarOffsetPointer = TheModule.getNamedGlobal(Name);
  if (!IvarOffsetPointer)
    IvarOffsetPointer = new toolchain::GlobalVariable(
        TheModule, toolchain::PointerType::getUnqual(VMContext), false,
        toolchain::GlobalValue::ExternalLinkage, nullptr, Name);
...
toolchain::Value *CGObjCGNU::EmitIvarOffset(CodeGenFunction &CGF,
                         const ObjCInterfaceDecl *Interface,
                         const ObjCIvarDecl *Ivar) {
  if (CGM.getLangOpts().ObjCRuntime.isNonFragile()) {
    ...
    CharUnits Align = CGM.getIntAlign();
    toolchain::Value *Offset = TheModule.getGlobalVariable(name);
    if (!Offset) {
      auto GV = new toolchain::GlobalVariable(TheModule, IntTy,
          false, toolchain::GlobalValue::LinkOnceAnyLinkage,
          toolchain::Constant::getNullValue(IntTy), name);
  ...
  return toolchain::ConstantInt::get(PtrDiffTy, Offset, /*isSigned*/true);
}

CGObjCRuntime *
language::Core::CodeGen::CreateGNUObjCRuntime(CodeGenModule &CGM) {
  ...
  case ObjCRuntime::WatchOS:
    toolchain_unreachable("these runtimes are not GNU runtimes");
  }
