...
namespace {
  ...
  public:
    AtomicInfo(CodeGenFunction &CGF, LValue &lvalue)
      ...
      if (lvalue.isSimple()) {
        ...
        toolchain::Value *BitFieldPtr = lvalue.getRawBitFieldPointer(CGF);
        auto OffsetInChars =
            (C.toCharUnitsFromBits(OrigBFI.Offset) / lvalue.getAlignment()) *
            lvalue.getAlignment();
        toolchain::Value *StoragePtr = CGF.Builder.CreateConstGEP1_64(
        ...
        toolchain::Type *StorageTy = CGF.Builder.getIntNTy(AtomicSizeInBits);
        ...
        AtomicTy = ValueTy = CGF.getContext().getExtVectorType(
            lvalue.getType(), cast<toolchain::FixedVectorType>(
    ...
    toolchain::Value *getAtomicPointer() const {
      if (LVal.isSimple())
    ...
    toolchain::Value *getAtomicSizeValue() const {
    ...
    toolchain::Value *getScalarRValValueOrNull(RValue RVal) const;

    /// Converts an rvalue to integer value if needed.
    toolchain::Value *convertRValueToInt(RValue RVal, bool CmpXchg = false) const;

    RValue ConvertToValueOrAtomic(toolchain::Value *IntVal, AggValueSlot ResultSlot,
    ...
    RValue EmitAtomicLoad(AggValueSlot ResultSlot, SourceLocation Loc,
                          bool AsValue, toolchain::AtomicOrdering AO,
                          bool IsVolatile);
    ...
    std::pair<RValue, toolchain::Value *>
    ...
    void EmitAtomicUpdate(toolchain::AtomicOrdering AO,
                          const toolchain::function_ref<RValue(RValue)> &UpdateOp,
                          bool IsVolatile);
    /// Emits atomic update.
    /// \param AO Atomic ordering.
    void EmitAtomicUpdate(toolchain::AtomicOrdering AO, RValue UpdateRVal,
                          bool IsVolatile);
  ...
  private:
    bool requiresMemSetZero(toolchain::Type *type) const;


    /// Emits atomic load as a libcall.
    void EmitAtomicLoadLibcall(toolchain::Value *AddForLoaded,
                               toolchain::AtomicOrdering AO, bool IsVolatile);
    /// Emits atomic load as LLVM instruction.
    toolchain::Value *EmitAtomicLoadOp(toolchain::AtomicOrdering AO, bool IsVolatile,
                                  bool CmpXchg = false);
    /// Emits atomic compare-and-exchange op as a libcall.
    toolchain::Value *EmitAtomicCompareExchangeLibcall(
        toolchain::Value *ExpectedAddr, toolchain::Value *DesiredAddr,
        toolchain::AtomicOrdering Success =
            toolchain::AtomicOrdering::SequentiallyConsistent,
        toolchain::AtomicOrdering Failure =
            toolchain::AtomicOrdering::SequentiallyConsistent);
    /// Emits atomic compare-and-exchange op as LLVM instruction.
    std::pair<toolchain::Value *, toolchain::Value *> EmitAtomicCompareExchangeOp(
        toolchain::Value *ExpectedVal, toolchain::Value *DesiredVal,
        toolchain::AtomicOrdering Success =
            toolchain::AtomicOrdering::SequentiallyConsistent,
        toolchain::AtomicOrdering Failure =
            toolchain::AtomicOrdering::SequentiallyConsistent,
    ...
    void
    EmitAtomicUpdateLibcall(toolchain::AtomicOrdering AO,
                            const toolchain::function_ref<RValue(RValue)> &UpdateOp,
                            bool IsVolatile);
    /// Emit atomic update as LLVM instructions.
    void EmitAtomicUpdateOp(toolchain::AtomicOrdering AO,
                            const toolchain::function_ref<RValue(RValue)> &UpdateOp,
                            bool IsVolatile);
    /// Emit atomic update as libcalls.
    void EmitAtomicUpdateLibcall(toolchain::AtomicOrdering AO, RValue UpdateRVal,
                                 bool IsVolatile);
    /// Emit atomic update as LLVM instructions.
    void EmitAtomicUpdateOp(toolchain::AtomicOrdering AO, RValue UpdateRal,
                            bool IsVolatile);
  };
}
...
static RValue emitAtomicLibcall(CodeGenFunction &CGF,
  ...
  toolchain::FunctionType *fnTy = CGF.CGM.getTypes().GetFunctionType(fnInfo);
  toolchain::AttrBuilder fnAttrB(CGF.getLLVMContext());
  fnAttrB.addAttribute(toolchain::Attribute::NoUnwind);
  fnAttrB.addAttribute(toolchain::Attribute::WillReturn);
  toolchain::AttributeList fnAttrs = toolchain::AttributeList::get(
      CGF.getLLVMContext(), toolchain::AttributeList::FunctionIndex, fnAttrB);
...
static bool isFullSizeType(CodeGenModule &CGM, toolchain::Type *type,
...
bool AtomicInfo::requiresMemSetZero(toolchain::Type *type) const {
...
bool AtomicInfo::emitMemSetZeroIfNecessary() const {
  ...
  CGF.Builder.CreateMemSet(
      addr.emitRawPointer(CGF), toolchain::ConstantInt::get(CGF.Int8Ty, 0),
...
static void emitAtomicCmpXchg(CodeGenFunction &CGF, AtomicExpr *E, bool IsWeak,
                              ...
                              uint64_t Size,
                              toolchain::AtomicOrdering SuccessOrder,
                              toolchain::AtomicOrdering FailureOrder,
                              toolchain::SyncScope::ID Scope) {
  // Note that cmpxchg doesn't support weak cmpxchg, at least at the moment.
  toolchain::Value *Expected = CGF.Builder.CreateLoad(Val1);
  toolchain::Value *Desired = CGF.Builder.CreateLoad(Val2);

  toolchain::AtomicCmpXchgInst *Pair = CGF.Builder.CreateAtomicCmpXchg(
  ...
  // false on failure.
  toolchain::Value *Old = CGF.Builder.CreateExtractValue(Pair, 0);
  toolchain::Value *Cmp = CGF.Builder.CreateExtractValue(Pair, 1);

  // This basic block is used to hold the store instruction if the operation
  // failed.
  toolchain::BasicBlock *StoreExpectedBB =
...
static void emitAtomicCmpXchgFailureSet(CodeGenFunction &CGF, AtomicExpr *E,
                                        ...
                                        Address Val1, Address Val2,
                                        toolchain::Value *FailureOrderVal,
                                        uint64_t Size,
                                        toolchain::AtomicOrdering SuccessOrder,
                                        toolchain::SyncScope::ID Scope) {
  toolchain::AtomicOrdering FailureOrder;
  if (toolchain::ConstantInt *FO = dyn_cast<toolchain::ConstantInt>(FailureOrderVal)) {
    auto FOS = FO->getSExtValue();
    if (!toolchain::isValidAtomicOrderingCABI(FOS))
      FailureOrder = toolchain::AtomicOrdering::Monotonic;
    else
      switch ((toolchain::AtomicOrderingCABI)FOS) {
      case toolchain::AtomicOrderingCABI::relaxed:
      // 31.7.2.18: "The failure argument shall not be memory_order_release
      // nor memory_order_acq_rel". Fallback to monotonic.
      case toolchain::AtomicOrderingCABI::release:
      case toolchain::AtomicOrderingCABI::acq_rel:
        FailureOrder = toolchain::AtomicOrdering::Monotonic;
        break;
      case toolchain::AtomicOrderingCABI::consume:
      case toolchain::AtomicOrderingCABI::acquire:
        FailureOrder = toolchain::AtomicOrdering::Acquire;
        break;
      case toolchain::AtomicOrderingCABI::seq_cst:
        FailureOrder = toolchain::AtomicOrdering::SequentiallyConsistent;
        break;
      }
  ...
  toolchain::SwitchInst *SI = CGF.Builder.CreateSwitch(FailureOrderVal, MonotonicBB);
  // Implemented as acquire, since it's the closest in LLVM.
  SI->addCase(CGF.Builder.getInt32((int)toolchain::AtomicOrderingCABI::consume),
              AcquireBB);
  SI->addCase(CGF.Builder.getInt32((int)toolchain::AtomicOrderingCABI::acquire),
              AcquireBB);
  SI->addCase(CGF.Builder.getInt32((int)toolchain::AtomicOrderingCABI::seq_cst),
              SeqCstBB);

  // Emit all the different atomics
  CGF.Builder.SetInsertPoint(MonotonicBB);
  emitAtomicCmpXchg(CGF, E, IsWeak, Dest, Ptr, Val1, Val2,
                    Size, SuccessOrder, toolchain::AtomicOrdering::Monotonic, Scope);
  CGF.Builder.CreateBr(ContBB);

  CGF.Builder.SetInsertPoint(AcquireBB);
  emitAtomicCmpXchg(CGF, E, IsWeak, Dest, Ptr, Val1, Val2, Size, SuccessOrder,
                    toolchain::AtomicOrdering::Acquire, Scope);
  CGF.Builder.CreateBr(ContBB);

  CGF.Builder.SetInsertPoint(SeqCstBB);
  emitAtomicCmpXchg(CGF, E, IsWeak, Dest, Ptr, Val1, Val2, Size, SuccessOrder,
                    toolchain::AtomicOrdering::SequentiallyConsistent, Scope);
  CGF.Builder.CreateBr(ContBB);
...
static toolchain::Value *EmitPostAtomicMinMax(CGBuilderTy &Builder,
                                         AtomicExpr::AtomicOp Op,
                                         bool IsSigned,
                                         toolchain::Value *OldVal,
                                         toolchain::Value *RHS) {
  toolchain::CmpInst::Predicate Pred;
  switch (Op) {
  default:
    toolchain_unreachable("Unexpected min/max operation");
  case AtomicExpr::AO__atomic_max_fetch:
  case AtomicExpr::AO__scoped_atomic_max_fetch:
    Pred = IsSigned ? toolchain::CmpInst::ICMP_SGT : toolchain::CmpInst::ICMP_UGT;
    break;
  case AtomicExpr::AO__atomic_min_fetch:
  case AtomicExpr::AO__scoped_atomic_min_fetch:
    Pred = IsSigned ? toolchain::CmpInst::ICMP_SLT : toolchain::CmpInst::ICMP_ULT;
    break;
  }
  toolchain::Value *Cmp = Builder.CreateICmp(Pred, OldVal, RHS, "tst");
  return Builder.CreateSelect(Cmp, OldVal, RHS, "newval");
}

static void EmitAtomicOp(CodeGenFunction &CGF, AtomicExpr *E, Address Dest,
                         Address Ptr, Address Val1, Address Val2,
                         toolchain::Value *IsWeak, toolchain::Value *FailureOrder,
                         uint64_t Size, toolchain::AtomicOrdering Order,
                         toolchain::SyncScope::ID Scope) {
  toolchain::AtomicRMWInst::BinOp Op = toolchain::AtomicRMWInst::Add;
  ...
  case AtomicExpr::AO__scoped_atomic_compare_exchange_n: {
    if (toolchain::ConstantInt *IsWeakC = dyn_cast<toolchain::ConstantInt>(IsWeak)) {
      emitAtomicCmpXchgFailureSet(CGF, E, IsWeakC->getZExtValue(), Dest, Ptr,
                                  Val1, Val2, FailureOrder, Size, Order, Scope);
    } else {
      // Create all the relevant BB's
      toolchain::BasicBlock *StrongBB =
          CGF.createBasicBlock("cmpxchg.strong", CGF.CurFn);
      toolchain::BasicBlock *WeakBB = CGF.createBasicBlock("cmxchg.weak", CGF.CurFn);
      toolchain::BasicBlock *ContBB =
          CGF.createBasicBlock("cmpxchg.continue", CGF.CurFn);

      toolchain::SwitchInst *SI = CGF.Builder.CreateSwitch(IsWeak, WeakBB);
  ...
  case AtomicExpr::AO__scoped_atomic_load: {
    toolchain::LoadInst *Load = CGF.Builder.CreateLoad(Ptr);
  ...
  case AtomicExpr::AO__scoped_atomic_store_n: {
    toolchain::Value *LoadVal1 = CGF.Builder.CreateLoad(Val1);
    toolchain::StoreInst *Store = CGF.Builder.CreateStore(LoadVal1, Ptr);
  ...
  case AtomicExpr::AO__scoped_atomic_exchange:
    Op = toolchain::AtomicRMWInst::Xchg;
    break;

  case AtomicExpr::AO__atomic_add_fetch:
  case AtomicExpr::AO__scoped_atomic_add_fetch:
    PostOp = E->getValueType()->isFloatingType() ? toolchain::Instruction::FAdd
  ...
  case AtomicExpr::AO__scoped_atomic_fetch_add:
    Op = E->getValueType()->isFloatingType() ? toolchain::AtomicRMWInst::FAdd
                                             : toolchain::AtomicRMWInst::Add;
    break;

  case AtomicExpr::AO__atomic_sub_fetch:
  case AtomicExpr::AO__scoped_atomic_sub_fetch:
    PostOp = E->getValueType()->isFloatingType() ? toolchain::Instruction::FSub
  ...
  case AtomicExpr::AO__scoped_atomic_fetch_sub:
    Op = E->getValueType()->isFloatingType() ? toolchain::AtomicRMWInst::FSub
  ...
  case AtomicExpr::AO__scoped_atomic_and_fetch:
    PostOp = toolchain::Instruction::And;
    [[fallthrough]];
  ...
  case AtomicExpr::AO__scoped_atomic_fetch_and:
    Op = toolchain::AtomicRMWInst::And;
    break;

  case AtomicExpr::AO__atomic_or_fetch:
  case AtomicExpr::AO__scoped_atomic_or_fetch:
    PostOp = toolchain::Instruction::Or;
    [[fallthrough]];
  ...
  case AtomicExpr::AO__scoped_atomic_fetch_or:
    Op = toolchain::AtomicRMWInst::Or;
    break;

  case AtomicExpr::AO__atomic_xor_fetch:
  case AtomicExpr::AO__scoped_atomic_xor_fetch:
    PostOp = toolchain::Instruction::Xor;
    [[fallthrough]];
  ...
  case AtomicExpr::AO__scoped_atomic_fetch_xor:
    Op = toolchain::AtomicRMWInst::Xor;
    break;

  case AtomicExpr::AO__atomic_nand_fetch:
  case AtomicExpr::AO__scoped_atomic_nand_fetch:
    PostOp = toolchain::Instruction::And; // the NOT is special cased below
    [[fallthrough]];
  case AtomicExpr::AO__c11_atomic_fetch_nand:
  case AtomicExpr::AO__atomic_fetch_nand:
  case AtomicExpr::AO__scoped_atomic_fetch_nand:
    Op = toolchain::AtomicRMWInst::Nand;
    break;

  case AtomicExpr::AO__atomic_test_and_set: {
    toolchain::AtomicRMWInst *RMWI =
        CGF.emitAtomicRMWInst(toolchain::AtomicRMWInst::Xchg, Ptr,
                              CGF.Builder.getInt8(1), Order, Scope, E);
    RMWI->setVolatile(E->isVolatile());
    toolchain::Value *Result = CGF.Builder.CreateIsNotNull(RMWI, "tobool");
  ...
  toolchain::Value *Result = RMWI;
  if (PostOpMinMax)
    Result = EmitPostAtomicMinMax(CGF.Builder, E->getOp(),
                                  E->getValueType()->isSignedIntegerType(),
                                  RMWI, LoadVal1);
  else if (PostOp)
    Result = CGF.Builder.CreateBinOp((toolchain::Instruction::BinaryOps)PostOp, RMWI,
                                     LoadVal1);
...
static void EmitAtomicOp(CodeGenFunction &CGF, AtomicExpr *Expr, Address Dest,
  ...
  if (auto SC = dyn_cast<toolchain::ConstantInt>(Scope)) {
  ...
  toolchain::DenseMap<unsigned, toolchain::BasicBlock *> BB;
  for (auto S : Scopes)
  ...
  toolchain::SwitchInst *SI = Builder.CreateSwitch(SC, BB[FallBack]);
  for (auto S : Scopes) {
    auto *B = BB[S];
    if (S != FallBack)
...
RValue CodeGenFunction::EmitAtomicExpr(AtomicExpr *E) {
  ...
  toolchain::Value *IsWeak = nullptr, *OrderFail = nullptr;
  ...
  toolchain::Value *Order = EmitScalarExpr(E->getOrder());
  toolchain::Value *Scope =
  ...
  case AtomicExpr::AO__c11_atomic_init:
  case AtomicExpr::AO__opencl_atomic_init:
    toolchain_unreachable("Already handled above with EmitAtomicInit!");
  ...
  case AtomicExpr::AO__opencl_atomic_fetch_sub:
    if (MemTy->isPointerType()) {
      ...
      toolchain::Value *Val1Scalar = EmitScalarExpr(E->getVal1());
  ...
  //
  // See: https://toolchain.org/docs/Atomics.html#libcalls-atomic
  if (UseLibcall) {
    CallArgList Args;
    // For non-optimized library calls, the size is the first parameter.
    Args.add(RValue::get(toolchain::ConstantInt::get(SizeTy, Size)),
             getContext().getSizeType());

    // The atomic address is the second parameter.
    // The OpenCL atomic library functions only accept pointer arguments to
    // generic address space.
    auto CastToGenericAddrSpace = [&](toolchain::Value *V, QualType PT) {
      if (!E->isOpenCL())
        return V;
      ...
      auto *DestType = toolchain::PointerType::get(getLLVMContext(), DestAS);
    ...
    case AtomicExpr::AO__atomic_test_and_set:
    case AtomicExpr::AO__atomic_clear:
      toolchain_unreachable("Integral atomic operations always become atomicrmw!");
    }
  ...
  if (isa<toolchain::ConstantInt>(Order)) {
    auto ord = cast<toolchain::ConstantInt>(Order)->getZExtValue();
    // We should not ever get to a case where the ordering isn't a valid C ABI
    // value, but it's hard to enforce that in general.
    if (toolchain::isValidAtomicOrderingCABI(ord))
      switch ((toolchain::AtomicOrderingCABI)ord) {
      case toolchain::AtomicOrderingCABI::relaxed:
        EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                     toolchain::AtomicOrdering::Monotonic, Scope);
        break;
      case toolchain::AtomicOrderingCABI::consume:
      case toolchain::AtomicOrderingCABI::acquire:
        if (IsStore)
      ...
      case toolchain::AtomicOrderingCABI::release:
        if (IsLoad)
          break; // Avoid crashing on code with undefined behavior
        EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                     toolchain::AtomicOrdering::Release, Scope);
        break;
      case toolchain::AtomicOrderingCABI::acq_rel:
        if (IsLoad || IsStore)
          break; // Avoid crashing on code with undefined behavior
        EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                     toolchain::AtomicOrdering::AcquireRelease, Scope);
        break;
      case toolchain::AtomicOrderingCABI::seq_cst:
        EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                     toolchain::AtomicOrdering::SequentiallyConsistent, Scope);
  ...
  // Create all the relevant BB's
  toolchain::BasicBlock *MonotonicBB = nullptr, *AcquireBB = nullptr,
  ...
  toolchain::SwitchInst *SI = Builder.CreateSwitch(Order, MonotonicBB);
  ...
  if (!IsStore) {
    ...
    EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                 toolchain::AtomicOrdering::Acquire, Scope);
    Builder.CreateBr(ContBB);
    SI->addCase(Builder.getInt32((int)toolchain::AtomicOrderingCABI::consume),
                AcquireBB);
    SI->addCase(Builder.getInt32((int)toolchain::AtomicOrderingCABI::acquire),
                AcquireBB);
  }
  if (!IsLoad) {
    ...
    EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                 toolchain::AtomicOrdering::Release, Scope);
    Builder.CreateBr(ContBB);
    SI->addCase(Builder.getInt32((int)toolchain::AtomicOrderingCABI::release),
                ReleaseBB);
  }
  if (!IsLoad && !IsStore) {
    ...
    EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
                 toolchain::AtomicOrdering::AcquireRelease, Scope);
    Builder.CreateBr(ContBB);
    SI->addCase(Builder.getInt32((int)toolchain::AtomicOrderingCABI::acq_rel),
                AcqRelBB);
  }
  ...
  EmitAtomicOp(*this, E, Dest, Ptr, Val1, Val2, IsWeak, OrderFail, Size,
               toolchain::AtomicOrdering::SequentiallyConsistent, Scope);
  Builder.CreateBr(ContBB);
  SI->addCase(Builder.getInt32((int)toolchain::AtomicOrderingCABI::seq_cst),
              SeqCstBB);
...
Address AtomicInfo::castToAtomicIntPointer(Address addr) const {
  toolchain::IntegerType *ty =
    toolchain::IntegerType::get(CGF.getLLVMContext(), AtomicSizeInBits);
  return addr.withElementType(ty);
}

Address AtomicInfo::convertToAtomicIntPointer(Address Addr) const {
  toolchain::Type *Ty = Addr.getElementType();
...
static bool shouldCastToInt(toolchain::Type *ValTy, bool CmpXchg) {
  if (ValTy->isFloatingPointTy())
    return ValTy->isX86_FP80Ty() || CmpXchg;
  return !ValTy->isIntegerTy() && !ValTy->isPointerTy();
}

RValue AtomicInfo::ConvertToValueOrAtomic(toolchain::Value *Val,
                                          AggValueSlot ResultSlot,
                                          SourceLocation Loc, bool AsValue,
                                          bool CmpXchg) const {
  ...
  if (getEvaluationKind() == TEK_Scalar &&
    ...
    if (toolchain::CastInst::isBitCastable(Val->getType(), ValTy))
...
void AtomicInfo::EmitAtomicLoadLibcall(toolchain::Value *AddForLoaded,
                                       toolchain::AtomicOrdering AO, bool) {
  // void __atomic_load(size_t size, void *mem, void *return, int order);
  CallArgList Args;
  ...
  Args.add(
      RValue::get(toolchain::ConstantInt::get(CGF.IntTy, (int)toolchain::toCABI(AO))),
      CGF.getContext().IntTy);
  emitAtomicLibcall(CGF, "__atomic_load", CGF.getContext().VoidTy, Args);
}

toolchain::Value *AtomicInfo::EmitAtomicLoadOp(toolchain::AtomicOrdering AO,
                                          bool IsVolatile, bool CmpXchg) {
  // Okay, we're doing this natively.
  Address Addr = getAtomicAddress();
  if (shouldCastToInt(Addr.getElementType(), CmpXchg))
    Addr = castToAtomicIntPointer(Addr);
  toolchain::LoadInst *Load = CGF.Builder.CreateLoad(Addr, "atomic-load");
  Load->setAtomic(AO);
...
toolchain::Value *AtomicInfo::getScalarRValValueOrNull(RValue RVal) const {
  if (RVal.isScalar() && (!hasPadding() || !LVal.isSimple()))
    return RVal.getScalarVal();
  return nullptr;
}

toolchain::Value *AtomicInfo::convertRValueToInt(RValue RVal, bool CmpXchg) const {
  // If we've got a scalar value of the right size, try to avoid going
  // through memory. Floats get casted if needed by AtomicExpandPass.
  if (toolchain::Value *Value = getScalarRValValueOrNull(RVal)) {
    if (!shouldCastToInt(Value->getType(), CmpXchg))
      return CGF.EmitToMemory(Value, ValueTy);
    else {
      toolchain::IntegerType *InputIntTy = toolchain::IntegerType::get(
          CGF.getLLVMContext(),
          LVal.isSimple() ? getValueSizeInBits() : getAtomicSizeInBits());
      if (toolchain::BitCastInst::isBitCastable(Value->getType(), InputIntTy))
...
std::pair<toolchain::Value *, toolchain::Value *> AtomicInfo::EmitAtomicCompareExchangeOp(
    toolchain::Value *ExpectedVal, toolchain::Value *DesiredVal,
    toolchain::AtomicOrdering Success, toolchain::AtomicOrdering Failure, bool IsWeak) {
  // Do the atomic store.
...
toolchain::Value *
AtomicInfo::EmitAtomicCompareExchangeLibcall(toolchain::Value *ExpectedAddr,
                                             toolchain::Value *DesiredAddr,
                                             toolchain::AtomicOrdering Success,
                                             toolchain::AtomicOrdering Failure) {
  ...
  Args.add(RValue::get(
               toolchain::ConstantInt::get(CGF.IntTy, (int)toolchain::toCABI(Success))),
           CGF.getContext().IntTy);
  Args.add(RValue::get(
               toolchain::ConstantInt::get(CGF.IntTy, (int)toolchain::toCABI(Failure))),
           CGF.getContext().IntTy);
...
std::pair<RValue, toolchain::Value *> AtomicInfo::EmitAtomicCompareExchange(
    RValue Expected, RValue Desired, toolchain::AtomicOrdering Success,
    toolchain::AtomicOrdering Failure, bool IsWeak) {
  // Check whether we should use a library call.
  if (shouldUseLibcall()) {
    // Produce a source address.
    Address ExpectedAddr = materializeRValue(Expected);
    toolchain::Value *ExpectedPtr = ExpectedAddr.emitRawPointer(CGF);
    toolchain::Value *DesiredPtr = materializeRValue(Desired).emitRawPointer(CGF);
...
void AtomicInfo::EmitAtomicUpdateLibcall(
    toolchain::AtomicOrdering AO, const toolchain::function_ref<RValue(RValue)> &UpdateOp,
    bool IsVolatile) {
  auto Failure = toolchain::AtomicCmpXchgInst::getStrongestFailureOrdering(AO);
  ...
  toolchain::Value *ExpectedPtr = ExpectedAddr.emitRawPointer(CGF);
  toolchain::Value *DesiredPtr = DesiredAddr.emitRawPointer(CGF);
  auto *Res =
...
void AtomicInfo::EmitAtomicUpdateOp(
  ...
  CGF.EmitBlock(ContBB);
  toolchain::PHINode *PHI = CGF.Builder.CreatePHI(OldVal->getType(),
                                             /*NumReservedValues=*/2);
...
void AtomicInfo::EmitAtomicUpdateLibcall(toolchain::AtomicOrdering AO,
...
void AtomicInfo::EmitAtomicUpdateOp(toolchain::AtomicOrdering AO, RValue UpdateRVal,
                                    bool IsVolatile) {
  auto Failure = toolchain::AtomicCmpXchgInst::getStrongestFailureOrdering(AO);
...
void AtomicInfo::EmitAtomicUpdate(toolchain::AtomicOrdering AO, RValue UpdateRVal,
                                  bool IsVolatile) {
  if (shouldUseLibcall()) {
  ...
  if (LVal.isSimple()) {
    ...
    if (atomics.shouldUseLibcall()) {
      ...
      args.add(RValue::get(srcAddr.emitRawPointer(*this)),
               getContext().VoidPtrTy);
      args.add(
          RValue::get(toolchain::ConstantInt::get(IntTy, (int)toolchain::toCABI(AO))),
          getContext().IntTy);
    ...
    // Okay, we're doing this natively.
    toolchain::Value *ValToStore = atomics.convertRValueToInt(rvalue);

    // Do the atomic store.
    Address Addr = atomics.getAtomicAddress();
    if (toolchain::Value *Value = atomics.getScalarRValValueOrNull(rvalue))
      if (shouldCastToInt(Value->getType(), /*CmpXchg=*/false)) {
        Addr = atomics.castToAtomicIntPointer(Addr);
        ValToStore = Builder.CreateIntCast(ValToStore, Addr.getElementType(),
                                           /*isSigned=*/false);
      }
    toolchain::StoreInst *store = Builder.CreateStore(ValToStore, Addr);

    if (AO == toolchain::AtomicOrdering::Acquire)
      AO = toolchain::AtomicOrdering::Monotonic;
    else if (AO == toolchain::AtomicOrdering::AcquireRelease)
      AO = toolchain::AtomicOrdering::Release;
...
///
std::pair<RValue, toolchain::Value *> CodeGenFunction::EmitAtomicCompareExchange(
...
CodeGenFunction::emitAtomicRMWInst(toolchain::AtomicRMWInst::BinOp Op, Address Addr,
                                   toolchain::Value *Val, toolchain::AtomicOrdering Order,
                                   toolchain::SyncScope::ID SSID,
                                   const AtomicExpr *AE) {
  toolchain::AtomicRMWInst *RMW =
...
void CodeGenFunction::EmitAtomicUpdate(
    LValue LVal, toolchain::AtomicOrdering AO,
    const toolchain::function_ref<RValue(RValue)> &UpdateOp, bool IsVolatile) {
...
void CodeGenFunction::EmitAtomicInit(Expr *init, LValue dest) {
  ...
  case TEK_Scalar: {
    toolchain::Value *value = EmitScalarExpr(init);
