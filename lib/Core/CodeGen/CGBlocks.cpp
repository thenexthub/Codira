...
static toolchain::Constant *buildGlobalBlock(CodeGenModule &CGM,
                                        const CGBlockInfo &blockInfo,
                                        toolchain::Constant *blockFn);

/// Build the helper function to copy a block.
static toolchain::Constant *buildCopyHelper(CodeGenModule &CGM,
                                       const CGBlockInfo &blockInfo) {
  return CodeGenFunction(CGM).GenerateCopyHelperFunction(blockInfo);
}

/// Build the helper function to dispose of a block.
static toolchain::Constant *buildDisposeHelper(CodeGenModule &CGM,
                                          const CGBlockInfo &blockInfo) {
...
static std::string getBlockDescriptorName(const CGBlockInfo &BlockInfo,
  ...
  Name += toolchain::to_string(BlockInfo.BlockSize.getQuantity()) + "_";
  ...
  if (BlockInfo.NeedsCopyDispose) {
    ...
    if (CGM.getCodeGenOpts().ObjCAutoRefCountExceptions)
      Name += "a";
    Name += toolchain::to_string(BlockInfo.BlockAlign.getQuantity()) + "_";

    for (auto &Cap : BlockInfo.SortedCaptures) {
      if (Cap.isConstantOrTrivial())
        continue;

      Name += toolchain::to_string(Cap.getOffset().getQuantity());
  ...
  if (!CGM.getCodeGenOpts().DisableBlockSignatureString) {
    ...
    toolchain::replace(TypeAtEncoding, '@', '\1');
  }
  Name += "e" + toolchain::to_string(TypeAtEncoding.size()) + "_" + TypeAtEncoding;
...
/// };
/// \endcode
static toolchain::Constant *buildBlockDescriptor(CodeGenModule &CGM,
                                            const CGBlockInfo &blockInfo) {
  ASTContext &C = CGM.getContext();

  toolchain::IntegerType *ulong =
    cast<toolchain::IntegerType>(CGM.getTypes().ConvertType(C.UnsignedLongTy));
  toolchain::PointerType *i8p = nullptr;
  if (CGM.getLangOpts().OpenCL)
    i8p = toolchain::PointerType::get(
  ...
  if (C.getLangOpts().ObjC &&
    ...
    if (toolchain::GlobalValue *desc = CGM.getModule().getNamedValue(descName))
      return desc;
  }
  ...
  if (blockInfo.NeedsCopyDispose) {
    ...
    toolchain::Constant *copyHelper = buildCopyHelper(CGM, blockInfo);
    elements.addSignedPointer(copyHelper, Schema, GlobalDecl(), QualType());

    // destroy_func_decl
    toolchain::Constant *disposeHelper = buildDisposeHelper(CGM, blockInfo);
    elements.addSignedPointer(disposeHelper, Schema, GlobalDecl(), QualType());

    if (cast<toolchain::Function>(copyHelper->stripPointerCasts())
            ->hasInternalLinkage() ||
        cast<toolchain::Function>(disposeHelper->stripPointerCasts())
            ->hasInternalLinkage())
  ...
  if (descName.empty()) {
    ...
    linkage = toolchain::GlobalValue::LinkOnceODRLinkage;
  }

  toolchain::GlobalVariable *global =
  ...
  if (linkage == toolchain::GlobalValue::LinkOnceODRLinkage) {
    ...
    global->setVisibility(toolchain::GlobalValue::HiddenVisibility);
...
namespace {
  ...
  struct BlockLayoutChunk {
    ...
    BlockLayoutChunk(CharUnits align, CharUnits size,
                     const BlockDecl::Capture *capture, toolchain::Type *type,
...
/// captured variables.
static toolchain::Constant *tryCaptureAsConstant(CodeGenModule &CGM,
                                            CodeGenFunction *CGF,
                                            const VarDecl *var) {
...
static void initializeForBlockHeader(CodeGenModule &CGM, CGBlockInfo &info,
                             SmallVectorImpl<toolchain::Type*> &elementTypes) {
...
static void computeBlockInfo(CodeGenModule &CGM, CodeGenFunction *CGF,
  ...
  SmallVector<toolchain::Type*, 8> elementTypes;
  ...
  if (!block->hasCaptures() && !hasNonConstantCustomFields) {
    info.StructureType =
      toolchain::StructType::get(CGM.getLLVMContext(), elementTypes, true);
  ...
  if (block->capturesCXXThis()) {
    ...
    toolchain::Type *toolchainType = CGM.getTypes().ConvertType(thisType);
    auto TInfo = CGM.getContext().getTypeInfoInChars(thisType);
    maxFieldAlign = std::max(maxFieldAlign, TInfo.Align);

    addBlockLayout(TInfo.Align, TInfo.Width, nullptr, toolchainType, thisType,
                   layout, info, CGM);
  }
  ...
  for (const auto &CI : block->captures()) {
    ...
    // the declaration.
    if (toolchain::Constant *constant = tryCaptureAsConstant(CGM, CGF, variable)) {
    ...
    addBlockLayout(align, size, &CI, toolchainType, VT, layout, info, CGM);
  }
  ...
  // toolchain::array_pod_stable_sort.
  toolchain::stable_sort(layout);
  ...
  if (endAlign < maxFieldAlign) {
    ...
    elementTypes.push_back(toolchain::ArrayType::get(CGM.Int8Ty,
                                                padding.getQuantity()));
...
toolchain::Value *CodeGenFunction::EmitBlockLiteral(const BlockExpr *blockExpr) {
  ...
  if (!blockExpr->getBlockDecl()->hasCaptures())
    ...
    if (toolchain::Constant *Block = CGM.getAddrOfGlobalBlockIfEmitted(blockExpr))
      return Block;
...
toolchain::Value *CodeGenFunction::EmitBlockLiteral(const CGBlockInfo &blockInfo) {
  bool IsOpenCL = CGM.getContext().getLangOpts().OpenCL;
  toolchain::PointerType *GenVoidPtrTy =
  ...
  auto *blockFn = toolchain::ConstantExpr::getPointerCast(InvokeFn, GenVoidPtrTy);
  ...
  toolchain::Constant *isa;
  toolchain::Constant *descriptor;
  BlockFlags flags;
  if (!IsOpenCL) {
    ...
    toolchain::Constant *blockISA = blockInfo.NoEscape
  ...
  auto storeField = [&](toolchain::Value *value, unsigned index, const Twine &name) {
  ...
  // Initialize the block header.
  {
    ...
    unsigned index = 0;
    CharUnits offset;
    auto addHeaderField = [&](toolchain::Value *value, CharUnits size,
                              const Twine &name) {
    ...
    auto addSignedHeaderField =
        [&](toolchain::Value *Value, const PointerAuthSchema &Schema,
    ...
    if (!IsOpenCL) {
      ...
      addHeaderField(toolchain::ConstantInt::get(IntTy, flags.getBitMask()),
                     getIntSize(), "block.flags");
      addHeaderField(toolchain::ConstantInt::get(IntTy, 0), getIntSize(),
                     "block.reserved");
    } else {
      addHeaderField(
          toolchain::ConstantInt::get(IntTy, blockInfo.BlockSize.getQuantity()),
          getIntSize(), "block.size");
      addHeaderField(
          toolchain::ConstantInt::get(IntTy, blockInfo.BlockAlign.getQuantity()),
          getIntSize(), "block.align");
    }

    if (!IsOpenCL) {
      toolchain::Value *blockFnPtr =
          toolchain::ConstantExpr::getBitCast(InvokeFn, VoidPtrTy);
  ...
  for (const auto &CI : blockDecl->captures()) {
    ...
    // If it's a reference variable, copy the reference into the block field.
      ...
      toolchain::Value *value = Builder.CreateLoad(src, "captured");
    ...
    // contained within the stack block's.
      ...
      toolchain::Value *value = Builder.CreateLoad(src, "block.captured_block");
    ...
    auto IsBlockDeclInRetExpr = [&]() {
      auto *EWC = toolchain::dyn_cast_or_null<ExprWithCleanups>(RetExpr);
      if (EWC)
  ...
  toolchain::Value *result = Builder.CreatePointerCast(
...
toolchain::Type *CodeGenModule::getBlockDescriptorType() {
  ...
  BlockDescriptorType = toolchain::PointerType::get(getLLVMContext(), AddrSpace);
  return BlockDescriptorType;
}

toolchain::Type *CodeGenModule::getGenericBlockLiteralType() {
  if (GenericBlockLiteralType)
    return GenericBlockLiteralType;

  toolchain::Type *BlockDescPtrTy = getBlockDescriptorType();
  ...
  if (getLangOpts().OpenCL) {
    ...
    if (auto *Helper = getTargetCodeGenInfo().getTargetOpenCLBlockHelper()) {
      toolchain::append_range(StructFields, Helper->getCustomFieldTypes());
    }
    GenericBlockLiteralType = toolchain::StructType::create(
    ...
    GenericBlockLiteralType =
        toolchain::StructType::create("struct.__block_literal_generic", VoidPtrTy,
...
RValue CodeGenFunction::EmitBlockCallExpr(const CallExpr *E,
  ...
  toolchain::Value *BlockPtr = EmitScalarExpr(E->getCallee());
  toolchain::Type *GenBlockTy = CGM.getGenericBlockLiteralType();
  toolchain::Value *Func = nullptr;
  QualType FnType = BPT->getPointeeType();
  ASTContext &Ctx = getContext();
  CallArgList Args;

  toolchain::Value *FuncPtr = nullptr;
  ...
  if (getLangOpts().OpenCL) {
    ...
    toolchain::Type *GenericVoidPtrTy =
        CGM.getOpenCLRuntime().getGenericVoidPointerType();
    toolchain::Value *BlockDescriptor = Builder.CreatePointerCast(
...
CodeGenModule::GetAddrOfGlobalBlock(const BlockExpr *BE,
                                    StringRef Name) {
  if (toolchain::Constant *Block = getAddrOfGlobalBlockIfEmitted(BE))
    return Block;
...
static toolchain::Constant *buildGlobalBlock(CodeGenModule &CGM,
  ...
  if (!IsOpenCL) {
    // Descriptor
    toolchain::Constant *Descriptor = buildBlockDescriptor(CGM, blockInfo);
  ...
  toolchain::GlobalVariable *literal = fields.finishAndCreateGlobal(
      "__block_literal_global", blockInfo.BlockAlign,
      /*constant*/ !IsWindows, toolchain::GlobalVariable::InternalLinkage, AddrSpace);
  ...
  if (IsWindows) {
    auto *Init = toolchain::Function::Create(toolchain::FunctionType::get(CGM.VoidTy,
          {}), toolchain::GlobalValue::InternalLinkage, ".block_isa_init",
        &CGM.getModule());
  ...
  toolchain::Type *RequiredType =
    CGM.getTypes().ConvertType(blockInfo.getBlockExpr()->getType());
  toolchain::Constant *Result =
      toolchain::ConstantExpr::getPointerCast(literal, RequiredType);
  CGM.setAddrOfGlobalBlock(blockInfo.BlockExpression, Result);
  if (CGM.getContext().getLangOpts().OpenCL)
    CGM.getOpenCLRuntime().recordBlockInfo(
        blockInfo.BlockExpression,
        cast<toolchain::Function>(blockFn->stripPointerCasts()), Result,
    ...
    if (CGM.getCodeGenOpts().hasReducedDebugInfo()) {
      ...
      DI->EmitDeclareOfBlockLiteralArgVariable(
          *BlockInfo, D->getName(), argNum,
          cast<toolchain::AllocaInst>(alloc.getPointer()->stripPointerCasts()),
          Builder);
    }
  }
...
toolchain::Function *CodeGenFunction::GenerateBlockFunction(
  ...
  toolchain::FunctionType *fnLLVMType = CGM.getTypes().GetFunctionType(fnInfo);

  StringRef name = CGM.getBlockMangledName(GD, blockDecl);
  toolchain::Function *fn = toolchain::Function::Create(
      fnLLVMType, toolchain::GlobalValue::InternalLinkage, name, &CGM.getModule());
  ...
  if (BuildGlobalBlock) {
    ...
    buildGlobalBlock(CGM, blockInfo,
                     toolchain::ConstantExpr::getPointerCast(fn, GenVoidPtrTy));
  }
  ...
  toolchain::Value *BlockPointerDbgLoc = BlockPointer;
  if (CGM.getCodeGenOpts().OptimizationLevel == 0) {
    ...
    // frame setup instruction by toolchain::DwarfDebug::beginFunction().
  ...
  toolchain::BasicBlock *entry = Builder.GetInsertBlock();
  toolchain::BasicBlock::iterator entry_ptr = Builder.GetInsertPoint();
  --entry_ptr;
  ...
  toolchain::BasicBlock *resume = Builder.GetInsertBlock();
...
computeCopyInfoForBlockCapture(const BlockDecl::Capture &CI, QualType T,
  ...
  toolchain_unreachable("after exhaustive PrimitiveCopyKind switch");
}
...
static std::string getBlockCaptureStr(const CGBlockInfo::Capture &Cap,
  ...
  case BlockCaptureEntityKind::CXXRecord: {
    ...
    Str += toolchain::to_string(TyStr.size()) + TyStr.c_str();
    break;
  }
  case BlockCaptureEntityKind::ARCWeak:
    Str += "w";
  ...
  case BlockCaptureEntityKind::AddressDiscriminatedPointerAuth: {
    ...
    Str += "p" + toolchain::to_string(PtrAuth.getKey()) + "d" +
           toolchain::to_string(PtrAuth.getExtraDiscriminator());
    break;
  }
  case BlockCaptureEntityKind::BlockObject: {
  ...
  case BlockCaptureEntityKind::NonTrivialCStruct: {
    ...
    Str += toolchain::to_string(FuncStr.size()) + "_" + FuncStr;
    break;
  }
...
static std::string getCopyDestroyHelperFuncName(
  ...
  if (CGM.getCodeGenOpts().ObjCAutoRefCountExceptions)
    Name += "a";
  Name += toolchain::to_string(BlockAlignment.getQuantity()) + "_";
...
static void setBlockHelperAttributesVisibility(bool CapturesNonExternalType,
  ...
  if (CapturesNonExternalType) {
    ...
    Fn->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
...
CodeGenFunction::GenerateCopyHelperFunction(const CGBlockInfo &blockInfo) {
  ...
  if (toolchain::GlobalValue *Func = CGM.getModule().getNamedValue(FuncName))
    return Func;
  ...
  toolchain::Function *Fn =
    toolchain::Function::Create(LTy, toolchain::GlobalValue::LinkOnceODRLinkage,
  ...
  for (auto &capture : blockInfo.SortedCaptures) {
    ...
    case BlockCaptureEntityKind::ARCStrong: {
      toolchain::Value *srcValue = Builder.CreateLoad(srcField, "blockcopy.src");
      // At -O0, store null into the destination field (so that the
      // storeStrong doesn't over-release) and then call storeStrong.
      // This is a workaround to not having an initStrong call.
      if (CGM.getCodeGenOpts().OptimizationLevel == 0) {
        auto *ty = cast<toolchain::PointerType>(srcValue->getType());
        toolchain::Value *null = toolchain::ConstantPointerNull::get(ty);
      ...
      // just emit a retain of the src field.
        ...
        if (!needsEHCleanup(captureType.isDestructedType()))
          if (auto *I = cast_or_null<toolchain::Instruction>(
    ...
    case BlockCaptureEntityKind::BlockObject: {
      toolchain::Value *srcValue = Builder.CreateLoad(srcField, "blockcopy.src");
      toolchain::Value *dstAddr = dstField.emitRawPointer(*this);
      toolchain::Value *args[] = {
        dstAddr, srcValue, toolchain::ConstantInt::get(Int32Ty, flags.getBitMask())
      };
...
computeDestroyInfoForBlockCapture(const BlockDecl::Capture &CI, QualType T,
  ...
  toolchain_unreachable("after exhaustive DestructionKind switch");
}
...
public:
  ...
  void emitCopy(CodeGenFunction &CGF, Address destField,
    ...
    toolchain::Value *srcValue = CGF.Builder.CreateLoad(srcField);

    unsigned flags = (Flags | BLOCK_BYREF_CALLER).getBitMask();

    toolchain::Value *flagsVal = toolchain::ConstantInt::get(CGF.Int32Ty, flags);
    toolchain::FunctionCallee fn = CGF.CGM.getBlockObjectAssign();

    toolchain::Value *args[] = {destField.emitRawPointer(CGF), srcValue, flagsVal};
    CGF.EmitNounwindRuntimeCall(fn, args);
  }

  void emitDispose(CodeGenFunction &CGF, Address field) override {
    field = field.withElementType(CGF.Int8PtrTy);
    toolchain::Value *value = CGF.Builder.CreateLoad(field);

    CGF.BuildBlockRelease(value, Flags | BLOCK_BYREF_CALLER, false);
  }

  void profileImpl(toolchain::FoldingSetNodeID &id) const override {
...
public:
  ...
  void emitCopy(CodeGenFunction &CGF, Address destField,
    ...
    toolchain::Value *value = CGF.Builder.CreateLoad(srcField);

    toolchain::Value *null =
      toolchain::ConstantPointerNull::get(cast<toolchain::PointerType>(value->getType()));
...
public:
  ...
  void emitCopy(CodeGenFunction &CGF, Address destField,
    ...
    toolchain::Value *oldValue = CGF.Builder.CreateLoad(srcField);
    toolchain::Value *copy = CGF.EmitARCRetainBlock(oldValue, /*mandatory*/ true);
...
public:
  ...
  void emitDispose(CodeGenFunction &CGF, Address Field) override {
    toolchain_unreachable("should never be called");
  }
...
generateByrefCopyHelper(CodeGenFunction &CGF, const BlockByrefInfo &byrefInfo,
  ...
  toolchain::FunctionType *LTy = CGF.CGM.getTypes().GetFunctionType(FI);
...
static toolchain::Constant *buildByrefCopyHelper(CodeGenModule &CGM,
...
static toolchain::Constant *buildByrefDisposeHelper(CodeGenModule &CGM,
                                               const BlockByrefInfo &byrefInfo,
                                               BlockByrefHelpers &generator) {
  CodeGenFunction CGF(CGM);
...
BlockByrefHelpers *
CodeGenFunction::buildByrefHelpers(toolchain::StructType &byrefType,
...
const BlockByrefInfo &CodeGenFunction::getBlockByrefInfo(const VarDecl *D) {
  ...
  // T x;
  toolchain::Type *varTy = ConvertTypeForMem(Ty);
  ...
  if (varOffset != size) {
    toolchain::Type *paddingTy =
      toolchain::ArrayType::get(Int8Ty, (varOffset - size).getQuantity());
  ...
  toolchain::StructType *byrefType = toolchain::StructType::create(
...
void CodeGenFunction::emitByrefStructureInit(const AutoVarEmission &emission) {
  ...
  toolchain::StructType *byrefType = cast<toolchain::StructType>(addr.getElementType());

  unsigned nextHeaderIndex = 0;
  CharUnits nextHeaderOffset;
  auto storeHeaderField = [&](toolchain::Value *value, CharUnits fieldSize,
  ...
  storeHeaderField(toolchain::ConstantInt::get(IntTy, flags.getBitMask()),
                   getIntSize(), "byref.flags");

  CharUnits byrefSize = CGM.GetTargetTypeStoreSize(byrefType);
  V = toolchain::ConstantInt::get(IntTy, byrefSize.getQuantity());
...
void CodeGenFunction::BuildBlockRelease(toolchain::Value *V, BlockFieldFlags flags,
                                        bool CanThrow) {
  toolchain::FunctionCallee F = CGM.getBlockObjectDispose();
  toolchain::Value *args[] = {V,
                         toolchain::ConstantInt::get(Int32Ty, flags.getBitMask())};
...
static void configureBlocksRuntimeObject(CodeGenModule &CGM,
  ...
  auto *GV = cast<toolchain::GlobalValue>(C->stripPointerCasts());
  ...
  if (!CGM.getCodeGenOpts().StaticClosure &&
    ...
    assert((isa<toolchain::Function>(C->stripPointerCasts()) ||
            isa<toolchain::GlobalVariable>(C->stripPointerCasts())) &&
...
toolchain::FunctionCallee CodeGenModule::getBlockObjectDispose() {
  ...
  configureBlocksRuntimeObject(
      *this, cast<toolchain::Constant>(BlockObjectDispose.getCallee()));
  return BlockObjectDispose;
}

toolchain::FunctionCallee CodeGenModule::getBlockObjectAssign() {
  ...
  configureBlocksRuntimeObject(
      *this, cast<toolchain::Constant>(BlockObjectAssign.getCallee()));
  return BlockObjectAssign;
}

toolchain::Constant *CodeGenModule::getNSConcreteGlobalBlock() {
...
toolchain::Constant *CodeGenModule::getNSConcreteStackBlock() {
