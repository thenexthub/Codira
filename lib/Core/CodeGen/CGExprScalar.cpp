...
bool mayHaveIntegerOverflow(toolchain::ConstantInt *LHS, toolchain::ConstantInt *RHS,
                             BinaryOperator::Opcode Opcode, bool Signed,
                             toolchain::APInt &Result) {
...
struct BinOpInfo {
  ...
  bool mayHaveIntegerOverflow() const {
    ...
    auto *LHSCI = dyn_cast<toolchain::ConstantInt>(LHS);
    auto *RHSCI = dyn_cast<toolchain::ConstantInt>(RHS);
    if (!LHSCI || !RHSCI)
      return true;
  ...
  bool mayHaveIntegerDivisionByZero() const {
    if (isDivremOp())
      if (auto *CI = dyn_cast<toolchain::ConstantInt>(RHS))
        return CI->isZero();
    return true;
  }
  ...
  bool mayHaveFloatDivisionByZero() const {
    if (isDivremOp())
      if (auto *CFP = dyn_cast<toolchain::ConstantFP>(RHS))
        return CFP->isZero();
    return true;
  }
...
public:
  ...
  toolchain::Type *ConvertType(QualType T) { return CGF.ConvertType(T); }
  ...
  void EmitLValueAlignmentAssumption(const Expr *E, Value *V) {
    ...
    toolchain::ConstantInt *AlignmentCI = cast<toolchain::ConstantInt>(AlignmentValue);
  ...
  void EmitFloatConversionCheck(Value *OrigSrc, QualType OrigSrcType,
                                ...
                                toolchain::Type *DstTy, SourceLocation Loc);
  ...
  struct ScalarConversionOpts {
    ...
    ScalarConversionOpts(language::Core::SanitizerSet SanOpts)
        : TreatBooleanAsSigned(false),
  ...
  Value *EmitScalarCast(Value *Src, QualType SrcType, QualType DstType,
                        toolchain::Type *SrcTy, toolchain::Type *DstTy,
  ...
  Value *EmitFloatToBoolConversion(Value *V) {
    ...
    toolchain::Value *Zero = toolchain::Constant::getNullValue(V->getType());
    return Builder.CreateFCmpUNE(V, Zero, "tobool");
  }

  /// EmitPointerToBoolConversion - Perform a pointer to boolean conversion.
  Value *EmitPointerToBoolConversion(Value *V, QualType QT) {
    Value *Zero = CGF.CGM.getNullPointer(cast<toolchain::PointerType>(V->getType()), QT);
  ...
  Value *EmitIntToBoolConversion(Value *V) {
    ...
    if (toolchain::ZExtInst *ZI = dyn_cast<toolchain::ZExtInst>(V)) {
  ...
  Value *VisitStmt(Stmt *S) {
    S->dump(toolchain::errs(), CGF.getContext());
    toolchain_unreachable("Stmt can't have complex result type!");
  }
  ...
  Value *VisitFloatingLiteral(const FloatingLiteral *E) {
    return toolchain::ConstantFP::get(VMContext, E->getValue());
  }
  Value *VisitCharacterLiteral(const CharacterLiteral *E) {
    return toolchain::ConstantInt::get(ConvertType(E->getType()), E->getValue());
  }
  ...
  Value *VisitAddrLabelExpr(const AddrLabelExpr *E) {
    toolchain::Value *V = CGF.GetAddrOfLabel(E->getLabel());
    return Builder.CreateBitCast(V, ConvertType(E->getType()));
  }

  Value *VisitSizeOfPackExpr(SizeOfPackExpr *E) {
    return toolchain::ConstantInt::get(ConvertType(E->getType()),E->getPackLength());
  }
  ...
  Value *VisitOpenACCAsteriskSizeExpr(OpenACCAsteriskSizeExpr *E) {
    toolchain_unreachable("Codegen for this isn't defined/implemented");
  }
  ...
  Value *VisitObjCAvailabilityCheckExpr(ObjCAvailabilityCheckExpr *E) {
    ...
    if (Version <= CGF.CGM.getTarget().getPlatformMinVersion())
      return toolchain::ConstantInt::get(Builder.getInt1Ty(), 1);
  ...
  Value *VisitCompoundLiteralExpr(CompoundLiteralExpr *E) {
    ...
    // See the discussion here: https://reviews.toolchain.org/D64464
    return EmitLoadOfLValue(E);
  }
  ...
  toolchain::Value *EmitIncDecConsiderOverflowBehavior(const UnaryOperator *E,
                                                  toolchain::Value *InVal,
                                                  bool IsInc);
  ...
  Value *VisitTypeTraitExpr(const TypeTraitExpr *E) {
    if (E->isStoredAsBoolean())
      return toolchain::ConstantInt::get(ConvertType(E->getType()),
                                    E->getBoolValue());
  ...
  Value *VisitExpressionTraitExpr(const ExpressionTraitExpr *E) {
    return toolchain::ConstantInt::get(Builder.getInt1Ty(), E->getValue());
  }
  ...
  void EmitUndefinedBehaviorIntegerDivAndRemCheck(const BinOpInfo &Ops,
                                                  toolchain::Value *Zero,bool isDiv);
...
#undef HANDLEBINOP
  ...
  // Comparisons.
  Value *EmitCompare(const BinaryOperator *E, toolchain::CmpInst::Predicate UICmpOpc,
                     toolchain::CmpInst::Predicate SICmpOpc,
                     toolchain::CmpInst::Predicate FCmpOpc, bool IsSignaling);
#define VISITCOMP(CODE, UI, SI, FP, SIG) \
    Value *VisitBin##CODE(const BinaryOperator *E) { \
      return EmitCompare(E, toolchain::ICmpInst::UI, toolchain::ICmpInst::SI, \
                         toolchain::FCmpInst::FP, SIG); }
...
Value *ScalarExprEmitter::EmitConversionToBool(Value *Src, QualType SrcType) {
  ...
  assert((SrcType->isIntegerType() || isa<toolchain::PointerType>(Src->getType())) &&
         "Unknown scalar type to convert");

  if (isa<toolchain::IntegerType>(Src->getType()))
    return EmitIntToBoolConversion(Src);

  assert(isa<toolchain::PointerType>(Src->getType()));
  return EmitPointerToBoolConversion(Src, SrcType);
}

void ScalarExprEmitter::EmitFloatConversionCheck(
    Value *OrigSrc, QualType OrigSrcType, Value *Src, QualType SrcType,
    QualType DstType, toolchain::Type *DstTy, SourceLocation Loc) {
  assert(SrcType->isFloatingType() && "not a conversion from floating point");
  if (!isa<toolchain::IntegerType>(DstTy))
    return;
  ...
  using toolchain::APSInt;

  toolchain::Value *Check = nullptr;
  const toolchain::fltSemantics &SrcSema =
  ...
  if (OrigSrcType->isHalfType()) {
    const toolchain::fltSemantics &Sema =
  ...
  toolchain::Value *GE =
    Builder.CreateFCmpOGT(Src, toolchain::ConstantFP::get(VMContext, MinSrc));
  toolchain::Value *LE =
    Builder.CreateFCmpOLT(Src, toolchain::ConstantFP::get(VMContext, MaxSrc));
  Check = Builder.CreateAnd(GE, LE);

  toolchain::Constant *StaticArgs[] = {CGF.EmitCheckSourceLocation(Loc),
...
void ScalarExprEmitter::EmitIntegerTruncationCheck(Value *Src, QualType SrcType,
  ...
  toolchain::Constant *StaticArgs[] = {
      CGF.EmitCheckSourceLocation(Loc), CGF.EmitCheckTypeDescriptor(SrcType),
      CGF.EmitCheckTypeDescriptor(DstType),
      toolchain::ConstantInt::get(Builder.getInt8Ty(), Check.first),
      toolchain::ConstantInt::get(Builder.getInt32Ty(), 0)};

  CGF.EmitCheck(Check.second, CheckHandler, StaticArgs, {Src, Dst});
}

static toolchain::Value *EmitIsNegativeTestHelper(Value *V, QualType VType,
                                             const char *Name,
                                             CGBuilderTy &Builder) {
  bool VSigned = VType->isSignedIntegerOrEnumerationType();
  toolchain::Type *VTy = V->getType();
  if (!VSigned) {
    // If the value is unsigned, then it is never negative.
    return toolchain::ConstantInt::getFalse(VTy->getContext());
  }
  toolchain::Constant *Zero = toolchain::ConstantInt::get(VTy, 0);
  return Builder.CreateICmp(toolchain::ICmpInst::ICMP_SLT, V, Zero,
                            toolchain::Twine(Name) + "." + V->getName() +
                                ".negativitycheck");
}
...
EmitIntegerSignChangeCheckHelper(Value *Src, QualType SrcType, Value *Dst,
  ...
  toolchain::Value *SrcIsNegative =
      EmitIsNegativeTestHelper(Src, SrcType, "src", Builder);
  // 2. Is the new Value negative?
  toolchain::Value *DstIsNegative =
...
void ScalarExprEmitter::EmitIntegerSignChangeCheck(Value *Src, QualType SrcType,
  ...
  toolchain::SmallVector<std::pair<toolchain::Value *, SanitizerKind::SanitizerOrdinal>,
                    2>
      Checks;
  ...
  toolchain::Constant *StaticArgs[] = {
      CGF.EmitCheckSourceLocation(Loc), CGF.EmitCheckTypeDescriptor(SrcType),
      CGF.EmitCheckTypeDescriptor(DstType),
      toolchain::ConstantInt::get(Builder.getInt8Ty(), CheckKind),
      toolchain::ConstantInt::get(Builder.getInt32Ty(), 0)};
...
void CodeGenFunction::EmitBitfieldConversionCheck(Value *Src, QualType SrcType,
  ...
  // This should be truncation of integral types.
  assert(isa<toolchain::IntegerType>(Src->getType()) &&
         isa<toolchain::IntegerType>(Dst->getType()) && "non-integer toolchain type");
  ...
  toolchain::Constant *StaticArgs[] = {
      ...
      toolchain::ConstantInt::get(Builder.getInt8Ty(), CheckKind),
      toolchain::ConstantInt::get(Builder.getInt32Ty(), Info.Size)};

  EmitCheck(Check.second, CheckHandler, StaticArgs, {Src, Dst});
}

Value *ScalarExprEmitter::EmitScalarCast(Value *Src, QualType SrcType,
                                         QualType DstType, toolchain::Type *SrcTy,
                                         toolchain::Type *DstTy,
                                         ScalarConversionOpts Opts) {
  // The Element types determine the type of cast to perform.
  toolchain::Type *SrcElementTy;
  toolchain::Type *DstElementTy;
  QualType SrcElementType;
  QualType DstElementType;
  if (SrcType->isMatrixType() && DstType->isMatrixType()) {
    SrcElementTy = cast<toolchain::VectorType>(SrcTy)->getElementType();
    DstElementTy = cast<toolchain::VectorType>(DstTy)->getElementType();
  ...
  if (isa<toolchain::IntegerType>(SrcElementTy)) {
    ...
    if (isa<toolchain::IntegerType>(DstElementTy))
  ...
  if (isa<toolchain::IntegerType>(DstElementTy)) {
    ...
    if (!CGF.CGM.getCodeGenOpts().StrictFloatCastOverflow) {
      toolchain::Intrinsic::ID IID =
          IsSigned ? toolchain::Intrinsic::fptosi_sat : toolchain::Intrinsic::fptoui_sat;
...
Value *ScalarExprEmitter::EmitScalarConversion(Value *Src, QualType SrcType,
  ...
  toolchain::Type *DstTy = ConvertType(DstType);
    ...
    if (DstTy->isFloatingPointTy()) {
      if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics())
        return Builder.CreateCall(
            CGF.CGM.getIntrinsic(toolchain::Intrinsic::convert_from_fp16, DstTy),
            Src);
    } else {
      ...
      if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics()) {
        Src = Builder.CreateCall(
            CGF.CGM.getIntrinsic(toolchain::Intrinsic::convert_from_fp16,
                                 CGF.CGM.FloatTy),
            Src);
      } else {
  ...
  if (auto DstPT = dyn_cast<toolchain::PointerType>(DstTy)) {
    ...
    // extension.
    toolchain::Type *MiddleTy = CGF.CGM.getDataLayout().getIntPtrType(DstPT);
    bool InputSigned = SrcType->isSignedIntegerOrEnumerationType();
    toolchain::Value* IntResult =
  ...
  if (isa<toolchain::PointerType>(SrcTy)) {
    ...
    assert(isa<toolchain::IntegerType>(DstTy) && "not ptr->int?");
  ...
  if (DstType->isExtVectorType() && !SrcType->isVectorType()) {
    ...
    unsigned NumElements = cast<toolchain::FixedVectorType>(DstTy)->getNumElements();
  ...
  if (isa<toolchain::VectorType>(SrcTy) || isa<toolchain::VectorType>(DstTy)) {
    // Allow bitcast from vector to integer/fp of the same size.
    toolchain::TypeSize SrcSize = SrcTy->getPrimitiveSizeInBits();
    toolchain::TypeSize DstSize = DstTy->getPrimitiveSizeInBits();
    if (SrcSize == DstSize)
    ...
    toolchain::Type *SrcElementTy = cast<toolchain::VectorType>(SrcTy)->getElementType();
    toolchain::Type *DstElementTy = cast<toolchain::VectorType>(DstTy)->getElementType();
    (void)DstElementTy;
  ...
  if (DstType->isHalfType() && !CGF.getContext().getLangOpts().NativeHalfType) {
    ...
    if (SrcTy->isFloatingPointTy()) {
      ...
      if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics())
        return Builder.CreateCall(
            CGF.CGM.getIntrinsic(toolchain::Intrinsic::convert_to_fp16, SrcTy), Src);
  ...
  if (DstTy != ResTy) {
    if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics()) {
      ...
      Res = Builder.CreateCall(
        CGF.CGM.getIntrinsic(toolchain::Intrinsic::convert_to_fp16, CGF.CGM.FloatTy),
        Res);
    } else {
...
Value *ScalarExprEmitter::EmitFixedPointConversion(Value *Src, QualType SrcTy,
                                                   QualType DstTy,
                                                   SourceLocation Loc) {
  toolchain::FixedPointBuilder<CGBuilderTy> FPBuilder(Builder);
...
void ScalarExprEmitter::EmitBinOpCheck(
  ...
  SmallVector<toolchain::Constant *, 4> StaticData;
  SmallVector<toolchain::Value *, 2> DynamicData;
  ...
  if (UO && UO->getOpcode() == UO_Minus) {
    ...
    if (BinaryOperator::isShiftOp(Opcode)) {
      ...
      default: toolchain_unreachable("unexpected opcode for bin op check");
      }
      StaticData.push_back(CGF.EmitCheckTypeDescriptor(Info.Ty));
    }
...
Value *ScalarExprEmitter::VisitExpr(Expr *E) {
  ...
  if (E->getType()->isVoidType())
    return nullptr;
  return toolchain::PoisonValue::get(CGF.ConvertType(E->getType()));
}
...
ScalarExprEmitter::VisitSYCLUniqueStableNameExpr(SYCLUniqueStableNameExpr *E) {
  ...
  toolchain::Constant *GlobalConstStr = Builder.CreateGlobalString(
      E->ComputeName(Context), "__usn_str", AddrSpace);

  toolchain::Type *ExprTy = ConvertType(E->getType());
...
Value *ScalarExprEmitter::VisitShuffleVectorExpr(ShuffleVectorExpr *E) {
  ...
  if (E->getNumSubExprs() == 2) {
    ...
    auto *LTy = cast<toolchain::FixedVectorType>(LHS->getType());
    unsigned LHSElts = LTy->getNumElements();

    Mask = RHS;

    auto *MTy = cast<toolchain::FixedVectorType>(Mask->getType());

    // Mask off the high bits of each shuffle index.
    Value *MaskBits =
        toolchain::ConstantInt::get(MTy, toolchain::NextPowerOf2(LHSElts - 1) - 1);
    ...
    auto *RTy = toolchain::FixedVectorType::get(LTy->getElementType(),
                                           MTy->getNumElements());
    Value* NewV = toolchain::PoisonValue::get(RTy);
    for (unsigned i = 0, e = MTy->getNumElements(); i != e; ++i) {
      Value *IIndx = toolchain::ConstantInt::get(CGF.SizeTy, i);
  ...
  for (unsigned i = 2; i < E->getNumSubExprs(); ++i) {
    toolchain::APSInt Idx = E->getShuffleMaskIdx(i - 2);
...
Value *ScalarExprEmitter::VisitConvertVectorExpr(ConvertVectorExpr *E) {
  ...
  toolchain::Type *SrcEltTy = cast<toolchain::VectorType>(SrcTy)->getElementType(),
             *DstEltTy = cast<toolchain::VectorType>(DstTy)->getElementType();

  if (DstEltType->isBooleanType()) {
    assert((SrcEltTy->isFloatingPointTy() ||
            isa<toolchain::IntegerType>(SrcEltTy)) && "Unknown boolean conversion");

    toolchain::Value *Zero = toolchain::Constant::getNullValue(SrcTy);
  ...
  if (isa<toolchain::IntegerType>(SrcEltTy)) {
    bool InputSigned = SrcEltType->isSignedIntegerOrEnumerationType();
    if (isa<toolchain::IntegerType>(DstEltTy))
  ...
  } else if (isa<toolchain::IntegerType>(DstEltTy)) {
...
Value *ScalarExprEmitter::VisitMemberExpr(MemberExpr *E) {
  ...
  toolchain::Value *Result = EmitLoadOfLValue(E);
  ...
  if (CGF.CGM.getCodeGenOpts().DebugInfoForProfiling && CGF.getDebugInfo()) {
    if (toolchain::LoadInst *Load = dyn_cast<toolchain::LoadInst>(Result)) {
      if (toolchain::GetElementPtrInst *GEP =
              dyn_cast<toolchain::GetElementPtrInst>(Load->getPointerOperand())) {
        if (toolchain::Instruction *Pointer =
                dyn_cast<toolchain::Instruction>(GEP->getPointerOperand())) {
...
static int getMaskElt(toolchain::ShuffleVectorInst *SVI, unsigned Idx,
                      unsigned Off) {
...
static int getAsInt32(toolchain::ConstantInt *C, toolchain::Type *I32Ty) {
  assert(toolchain::ConstantInt::isValueValidForType(I32Ty, C->getZExtValue()) &&
...
Value *ScalarExprEmitter::VisitInitListExpr(InitListExpr *E) {
  ...
  toolchain::VectorType *VType =
    dyn_cast<toolchain::VectorType>(ConvertType(E->getType()));
  ...
  if (isa<toolchain::ScalableVectorType>(VType)) {
    if (NumInitElements == 0) {
    ...
    toolchain_unreachable("Unexpected initialization of a scalable vector!");
  }

  unsigned ResElts = cast<toolchain::FixedVectorType>(VType)->getNumElements();
  ...
  toolchain::Value *V = toolchain::PoisonValue::get(VType);
  for (unsigned i = 0; i != NumInitElements; ++i) {
    Expr *IE = E->getInit(i);
    Value *Init = Visit(IE);
    SmallVector<int, 16> Args;

    toolchain::VectorType *VVT = dyn_cast<toolchain::VectorType>(Init->getType());
    ...
    if (!VVT) {
      if (isa<ExtVectorElementExpr>(IE)) {
        toolchain::ExtractElementInst *EI = cast<toolchain::ExtractElementInst>(Init);

        if (cast<toolchain::FixedVectorType>(EI->getVectorOperandType())
                ->getNumElements() == ResElts) {
          toolchain::ConstantInt *C = cast<toolchain::ConstantInt>(EI->getIndexOperand());
          ...
          if (CurIdx == 0) {
            ...
            toolchain::ShuffleVectorInst *SVV = cast<toolchain::ShuffleVectorInst>(V);
            for (unsigned j = 0; j != CurIdx; ++j)
              Args.push_back(getMaskElt(SVV, j, 0));
            Args.push_back(ResElts + C->getZExtValue());
            Args.resize(ResElts, -1);

            LHS = cast<toolchain::ShuffleVectorInst>(V)->getOperand(0);
    ...
    unsigned InitElts = cast<toolchain::FixedVectorType>(VVT)->getNumElements();
    ...
    if (isa<ExtVectorElementExpr>(IE)) {
      toolchain::ShuffleVectorInst *SVI = cast<toolchain::ShuffleVectorInst>(Init);
      Value *SVOp = SVI->getOperand(0);
      auto *OpTy = cast<toolchain::FixedVectorType>(SVOp->getType());
      ...
      if (OpTy->getNumElements() == ResElts) {
        for (unsigned j = 0; j != CurIdx; ++j) {
          ...
          if (VIsPoisonShuffle) {
            Args.push_back(getMaskElt(cast<toolchain::ShuffleVectorInst>(V), j, 0));
          } else {
            Args.push_back(j);
          }
        }
        ...
        if (VIsPoisonShuffle)
          V = cast<toolchain::ShuffleVectorInst>(V)->getOperand(0);
    ...
    VIsPoisonShuffle = isa<toolchain::PoisonValue>(Init);
    CurIdx += InitElts;
  }

  // FIXME: evaluate codegen vs. shuffling against constant null vector.
  // Emit remaining default initializers.
  toolchain::Type *EltTy = VType->getElementType();

  // Emit remaining default initializers
  for (/* Do not initialize i*/; CurIdx < ResElts; ++CurIdx) {
    Value *Idx = Builder.getInt32(CurIdx);
    toolchain::Value *Init = toolchain::Constant::getNullValue(EltTy);
...
static Value *EmitHLSLElementwiseCast(CodeGenFunction &CGF, Address RHSVal,
                                      ...
                                      SourceLocation Loc) {
  SmallVector<std::pair<Address, toolchain::Value *>, 16> LoadGEPList;
  ...
  if (auto *VecTy = LHSTy->getAs<VectorType>()) {
    ...
    for (unsigned I = 0, E = VecTy->getNumElements(); I < E; I++) {
      toolchain::Value *Load = CGF.Builder.CreateLoad(LoadGEPList[I].first, "load");
      toolchain::Value *Idx = LoadGEPList[I].second;
      Load = Idx ? CGF.Builder.CreateExtractElement(Load, Idx, "vec.extract")
                 : Load;
      toolchain::Value *Cast = CGF.EmitScalarConversion(
  ...
  toolchain::Value *Load = CGF.Builder.CreateLoad(LoadGEPList[0].first, "load");
  toolchain::Value *Idx = LoadGEPList[0].second;
  Load =
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  switch (Kind) {
  case CK_Dependent: toolchain_unreachable("dependent cast kind in IR gen!");
  ...
  case CK_BitCast: {
    Value *Src = Visit(E);
    toolchain::Type *SrcTy = Src->getType();
    toolchain::Type *DstTy = ConvertType(DestTy);
    ...
    if (auto A = dyn_cast<toolchain::Argument>(Src); A && A->hasStructRetAttr())
    ...
    if (auto *CI = dyn_cast<toolchain::CallBase>(Src)) {
    ...
    // same element type, use the toolchain.vector.insert intrinsic to perform the
    // bitcast.
    if (auto *FixedSrcTy = dyn_cast<toolchain::FixedVectorType>(SrcTy)) {
      if (auto *ScalableDstTy = dyn_cast<toolchain::ScalableVectorType>(DstTy)) {
        ...
        if (FixedSrcTy->getElementType() == ScalableDstTy->getElementType()) {
          toolchain::Value *PoisonVec = toolchain::PoisonValue::get(ScalableDstTy);
          toolchain::Value *Result = Builder.CreateInsertVector(
              ScalableDstTy, PoisonVec, Src, uint64_t(0), "cast.scalable");
          ScalableDstTy = cast<toolchain::ScalableVectorType>(
              toolchain::VectorType::getWithSizeAndScalar(ScalableDstTy, DstTy));
    ...
    // same element type, use the toolchain.vector.extract intrinsic to perform the
    // bitcast.
    if (auto *ScalableSrcTy = dyn_cast<toolchain::ScalableVectorType>(SrcTy)) {
      if (auto *FixedDstTy = dyn_cast<toolchain::FixedVectorType>(DstTy)) {
        ...
        if (ScalableSrcTy->getElementType()->isIntegerTy(1) &&
          ...
          if (!ScalableSrcTy->getElementCount().isKnownMultipleOf(8)) {
            ScalableSrcTy = toolchain::ScalableVectorType::get(
                ScalableSrcTy->getElementType(),
                toolchain::alignTo<8>(
                    ScalableSrcTy->getElementCount().getKnownMinValue()));
            toolchain::Value *ZeroVec = toolchain::Constant::getNullValue(ScalableSrcTy);
    ...
    // TODO: since the toolchain.vector.{insert,extract} intrinsics
    //       require the element types of the vectors to be the same, we
    //       need to keep this around for bitcasts between VLAT <-> VLST where
    //       the element types of the vectors are not the same, until we figure
    //       out a better way of doing these casts.
    if ((isa<toolchain::FixedVectorType>(SrcTy) &&
         isa<toolchain::ScalableVectorType>(DstTy)) ||
        (isa<toolchain::ScalableVectorType>(SrcTy) &&
         isa<toolchain::FixedVectorType>(DstTy))) {
    ...
    toolchain::Value *Result = Builder.CreateBitCast(Src, DstTy);
  ...
  case CK_AddressSpaceConversion: {
    ...
    if (E->EvaluateAsRValue(Result, CGF.getContext()) &&
      ...
      if (Result.HasSideEffects)
        Visit(E);
      return CGF.CGM.getNullPointer(cast<toolchain::PointerType>(
  ...
  case CK_NullToPointer:
    ...
    return CGF.CGM.getNullPointer(cast<toolchain::PointerType>(ConvertType(DestTy)),
                              DestTy);
  ...
  case CK_HLSLArrayRValue:
    toolchain_unreachable("scalar cast to non-scalar value");
  ...
  case CK_IntegralToPointer: {
    ...
    toolchain::Type *MiddleTy = CGF.CGM.getDataLayout().getIntPtrType(DestLLVMTy);
  ...
  case CK_VectorSplat: {
    ...
    Value *Elt = Visit(E);
    // Splat the element across to all elements
    toolchain::ElementCount NumElements =
        cast<toolchain::VectorType>(DstTy)->getElementCount();
  ...
  case CK_MemberPointerToBoolean: {
    toolchain::Value *MemPtr = Visit(E);
  ...
  case CK_ZeroToOCLOpaqueType: {
    ...
    return toolchain::Constant::getNullValue(ConvertType(DestTy));
  }
  ...
  case CK_HLSLVectorTruncation: {
    ...
    toolchain::Value *Zero = toolchain::Constant::getNullValue(CGF.SizeTy);
  ...
  toolchain_unreachable("unknown scalar cast");
}
...
static BinOpInfo createBinOpInfoFromIncDec(const UnaryOperator *E,
                                           toolchain::Value *InVal, bool IsInc,
                                           FPOptions FPFeatures) {
  BinOpInfo BinOp;
  BinOp.LHS = InVal;
  BinOp.RHS = toolchain::ConstantInt::get(InVal->getType(), 1, false);
  BinOp.Ty = E->getType();
...
toolchain::Value *ScalarExprEmitter::EmitIncDecConsiderOverflowBehavior(
    const UnaryOperator *E, toolchain::Value *InVal, bool IsInc) {
  toolchain::Value *Amount =
      toolchain::ConstantInt::get(InVal->getType(), IsInc ? 1 : -1, true);
  ...
  toolchain_unreachable("Unknown SignedOverflowBehaviorTy");
}
...
ScalarExprEmitter::EmitScalarPrePostIncDec(const UnaryOperator *E, LValue LV,
  ...
  toolchain::PHINode *atomicPHI = nullptr;
  toolchain::Value *value;
  toolchain::Value *input;
  ...
  if (const AtomicType *atomicTy = type->getAs<AtomicType>()) {
    ...
    if (isInc && type->isBooleanType()) {
      toolchain::Value *True = CGF.EmitToMemory(Builder.getTrue(), type);
      if (isPre) {
        Builder.CreateStore(True, LV.getAddress(), LV.isVolatileQualified())
            ->setAtomic(toolchain::AtomicOrdering::SequentiallyConsistent);
        return Builder.getTrue();
      }
      // For atomic bool increment, we just store true and return it for
      // preincrement, do an atomic swap with true for postincrement
      return Builder.CreateAtomicRMW(
          toolchain::AtomicRMWInst::Xchg, LV.getAddress(), True,
    ...
    if (!type->isBooleanType() && type->isIntegerType() &&
      ...
      toolchain::AtomicRMWInst::BinOp aop = isInc ? toolchain::AtomicRMWInst::Add :
        toolchain::AtomicRMWInst::Sub;
      toolchain::Instruction::BinaryOps op = isInc ? toolchain::Instruction::Add :
        toolchain::Instruction::Sub;
      toolchain::Value *amt = CGF.EmitToMemory(
          toolchain::ConstantInt::get(ConvertType(type), 1, true), type);
      toolchain::Value *old =
    ...
    if (type->isFloatingType()) {
      toolchain::Type *Ty = ConvertType(type);
      if (toolchain::has_single_bit(Ty->getScalarSizeInBits())) {
        toolchain::AtomicRMWInst::BinOp aop =
            isInc ? toolchain::AtomicRMWInst::FAdd : toolchain::AtomicRMWInst::FSub;
        toolchain::Instruction::BinaryOps op =
            isInc ? toolchain::Instruction::FAdd : toolchain::Instruction::FSub;
        toolchain::Value *amt = toolchain::ConstantFP::get(Ty, 1.0);
        toolchain::AtomicRMWInst *old =
    ...
    toolchain::BasicBlock *opBB = CGF.createBasicBlock("atomic_op", CGF.CurFn);
  ...
  // Most common case by far: integer increment.
    ...
    if (CGF.SanOpts.hasOneOf(
      ...
      Value *amt = toolchain::ConstantInt::get(value->getType(), amount, true);
      ...
      toolchain::Value *amt = toolchain::ConstantInt::get(value->getType(), amount, true);
      value = Builder.CreateAdd(value, amt, isInc ? "inc" : "dec");
    }
  ...
  // Next most common: pointer increment.
    ...
    if (const VariableArrayType *vla
      ...
      toolchain::Value *numElts = CGF.getVLASize(vla).NumElts;
      if (!isInc) numElts = Builder.CreateNSWNeg(numElts, "vla.negsize");
      toolchain::Type *elemTy = CGF.ConvertTypeForMem(vla->getElementType());
    ...
    // Arithmetic on function pointers (!) is just +-1.
      ...
      toolchain::Value *amt = Builder.getInt32(amount);
    ...
    // For everything else, we can just do a simple increment.
      ...
      toolchain::Value *amt = Builder.getInt32(amount);
      toolchain::Type *elemTy = CGF.ConvertTypeForMem(type);
      if (CGF.getLangOpts().PointerOverflowDefined)
  ...
  // Vector increment/decrement.
    ...
    if (type->hasIntegerRepresentation()) {
      toolchain::Value *amt = toolchain::ConstantInt::get(value->getType(), amount);

      value = Builder.CreateAdd(value, amt, isInc ? "inc" : "dec");
    } else {
      value = Builder.CreateFAdd(
                  value,
                  toolchain::ConstantFP::get(value->getType(), amount),
                  isInc ? "inc" : "dec");
    }
  ...
  // Floating point.
    ...
    if (value->getType()->isFloatTy())
      amt = toolchain::ConstantFP::get(VMContext,
                                  toolchain::APFloat(static_cast<float>(amount)));
    else if (value->getType()->isDoubleTy())
      amt = toolchain::ConstantFP::get(VMContext,
                                  toolchain::APFloat(static_cast<double>(amount)));
    else {
      // Remaining types are Half, Bfloat16, LongDouble, __ibm128 or __float128.
      // Convert from float.
      toolchain::APFloat F(static_cast<float>(amount));
      bool ignored;
      const toolchain::fltSemantics *FS;
      ...
      F.convert(*FS, toolchain::APFloat::rmTowardZero, &ignored);
      amt = toolchain::ConstantFP::get(VMContext, F);
    }
    ...
    if (type->isHalfType() && !CGF.getContext().getLangOpts().NativeHalfType) {
      if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics()) {
        value = Builder.CreateCall(
            CGF.CGM.getIntrinsic(toolchain::Intrinsic::convert_to_fp16,
                                 CGF.CGM.FloatTy),
            value, "incdec.conv");
      } else {
  ...
  // Fixed-point types.
    ...
    Info.LHS = value;
    Info.RHS = toolchain::ConstantInt::get(value->getType(), 1, false);
  ...
  // Objective-C pointer types.
    ...
    toolchain::Value *sizeValue =
      toolchain::ConstantInt::get(CGF.SizeTy, size.getQuantity());
  ...
  if (atomicPHI) {
    toolchain::BasicBlock *curBlock = Builder.GetInsertBlock();
    toolchain::BasicBlock *contBB = CGF.createBasicBlock("atomic_cont", CGF.CurFn);
    auto Pair = CGF.EmitAtomicCompareExchange(
        LV, RValue::get(atomicPHI), RValue::get(value), E->getExprLoc());
    toolchain::Value *old = CGF.EmitToMemory(Pair.first.getScalarVal(), type);
    toolchain::Value *success = Pair.second;
...
Value *ScalarExprEmitter::VisitMinus(const UnaryOperator *E,
  ...
  BinOpInfo BinOp;
  BinOp.RHS = Op;
  BinOp.LHS = toolchain::Constant::getNullValue(BinOp.RHS->getType());
  BinOp.Ty = E->getType();
  BinOp.Opcode = BO_Sub;
...
Value *ScalarExprEmitter::VisitUnaryLNot(const UnaryOperator *E) {
  ...
  if (E->getType()->isVectorType() &&
    ...
    Value *Zero = toolchain::Constant::getNullValue(Oper->getType());
    Value *Result;
    if (Oper->getType()->isFPOrFPVectorTy()) {
      CodeGenFunction::CGFPOptionsRAII FPOptsRAII(
          CGF, E->getFPFeaturesInEffect(CGF.getLangOpts()));
      Result = Builder.CreateFCmp(toolchain::CmpInst::FCMP_OEQ, Oper, Zero, "cmp");
    } else
      Result = Builder.CreateICmp(toolchain::CmpInst::ICMP_EQ, Oper, Zero, "cmp");
...
Value *ScalarExprEmitter::VisitOffsetOfExpr(OffsetOfExpr *E) {
  ...
  if (E->EvaluateAsInt(EVResult, CGF.getContext())) {
    toolchain::APSInt Value = EVResult.Val.getInt();
    return Builder.getInt(Value);
  }
  ...
  toolchain::Value* Result = toolchain::Constant::getNullValue(ResultType);
  ...
  for (unsigned i = 0; i != n; ++i) {
    ...
    case OffsetOfNode::Array: {
      ...
      toolchain::Value* Idx = CGF.EmitScalarExpr(IdxExpr);
      ...
      toolchain::Value* ElemSize = toolchain::ConstantInt::get(ResultType,
    ...
    case OffsetOfNode::Field: {
      ...
      Offset = toolchain::ConstantInt::get(ResultType, OffsetInt);
    ...
    case OffsetOfNode::Identifier:
      toolchain_unreachable("dependent __builtin_offsetof");
    ...
    case OffsetOfNode::Base: {
      ...
      Offset = toolchain::ConstantInt::get(ResultType, OffsetInt.getQuantity());
      break;
    }
    }
  ...
  if (auto Kind = E->getKind();
    ...
    auto Alignment =
        CGF.getContext()
            ...
            .getQuantity();
    return toolchain::ConstantInt::get(CGF.SizeTy, Alignment);
  } else if (E->getKind() == UETT_VectorElements) {
    auto *VecTy = cast<toolchain::VectorType>(ConvertType(E->getTypeOfArgument()));
...
Value *ScalarExprEmitter::VisitImag(const UnaryOperator *E,
  ...
  if (!PromotionType.isNull())
    return toolchain::Constant::getNullValue(ConvertType(PromotionType));
  return toolchain::Constant::getNullValue(ConvertType(E->getType()));
}
...
LValue ScalarExprEmitter::EmitCompoundAssignLValue(
  ...
  if (const AtomicType *atomicTy = LHSTy->getAs<AtomicType>()) {
    ...
    if (!type->isBooleanType() && type->isIntegerType() &&
        ...
        CGF.getLangOpts().getSignedOverflowBehavior() !=
            LangOptions::SOB_Trapping) {
      toolchain::AtomicRMWInst::BinOp AtomicOp = toolchain::AtomicRMWInst::BAD_BINOP;
      toolchain::Instruction::BinaryOps Op;
      switch (OpInfo.Opcode) {
        ...
        case BO_ShrAssign:
          break;
        case BO_AddAssign:
          AtomicOp = toolchain::AtomicRMWInst::Add;
          Op = toolchain::Instruction::Add;
          break;
        case BO_SubAssign:
          AtomicOp = toolchain::AtomicRMWInst::Sub;
          Op = toolchain::Instruction::Sub;
          break;
        case BO_AndAssign:
          AtomicOp = toolchain::AtomicRMWInst::And;
          Op = toolchain::Instruction::And;
          break;
        case BO_XorAssign:
          AtomicOp = toolchain::AtomicRMWInst::Xor;
          Op = toolchain::Instruction::Xor;
          break;
        case BO_OrAssign:
          AtomicOp = toolchain::AtomicRMWInst::Or;
          Op = toolchain::Instruction::Or;
          break;
        default:
          toolchain_unreachable("Invalid compound assignment type");
      }
      if (AtomicOp != toolchain::AtomicRMWInst::BAD_BINOP) {
        toolchain::Value *Amt = CGF.EmitToMemory(
            EmitScalarConversion(OpInfo.RHS, E->getRHS()->getType(), LHSTy,
                                 E->getExprLoc()),
            LHSTy);

        toolchain::AtomicRMWInst *OldVal =
  ...
  if (atomicPHI) {
    ...
    toolchain::Value *old = CGF.EmitToMemory(Pair.first.getScalarVal(), LHSTy);
    toolchain::Value *success = Pair.second;
    atomicPHI->addIncoming(old, curBlock);
    Builder.CreateCondBr(success, contBB, atomicPHI->getParent());
    Builder.SetInsertPoint(contBB);
...
void ScalarExprEmitter::EmitUndefinedBehaviorIntegerDivAndRemCheck(
    const BinOpInfo &Ops, toolchain::Value *Zero, bool isDiv) {
  SmallVector<std::pair<toolchain::Value *, SanitizerKind::SanitizerOrdinal>, 2>
      Checks;
  ...
  if (CGF.SanOpts.has(SanitizerKind::SignedIntegerOverflow) &&
    ...
    toolchain::IntegerType *Ty = cast<toolchain::IntegerType>(Zero->getType());

    toolchain::Value *IntMin =
      Builder.getInt(toolchain::APInt::getSignedMinValue(Ty->getBitWidth()));
    toolchain::Value *NegOne = toolchain::Constant::getAllOnesValue(Ty);

    toolchain::Value *LHSCmp = Builder.CreateICmpNE(Ops.LHS, IntMin);
    toolchain::Value *RHSCmp = Builder.CreateICmpNE(Ops.RHS, NegOne);
    toolchain::Value *NotOverflow = Builder.CreateOr(LHSCmp, RHSCmp, "or");
    Checks.push_back(
...
Value *ScalarExprEmitter::EmitDiv(const BinOpInfo &Ops) {
  {
    ...
    if ((CGF.SanOpts.has(SanitizerKind::IntegerDivideByZero) ||
      ...
      toolchain::Value *Zero = toolchain::Constant::getNullValue(ConvertType(Ops.Ty));
      EmitUndefinedBehaviorIntegerDivAndRemCheck(Ops, Zero, true);
    } else if (CGF.SanOpts.has(SanitizerKind::FloatDivideByZero) &&
               Ops.Ty->isRealFloatingType() &&
               Ops.mayHaveFloatDivisionByZero()) {
      toolchain::Value *Zero = toolchain::Constant::getNullValue(ConvertType(Ops.Ty));
      toolchain::Value *NonZero = Builder.CreateFCmpUNE(Ops.RHS, Zero);
      EmitBinOpCheck(
...
Value *ScalarExprEmitter::EmitOverflowCheckedBinOp(const BinOpInfo &Ops) {
  ...
  case BO_AddAssign:
    OpID = 1;
    IID = isSigned ? toolchain::Intrinsic::sadd_with_overflow :
                     toolchain::Intrinsic::uadd_with_overflow;
    OverflowKind = SanitizerHandler::AddOverflow;
    break;
  case BO_Sub:
  case BO_SubAssign:
    OpID = 2;
    IID = isSigned ? toolchain::Intrinsic::ssub_with_overflow :
                     toolchain::Intrinsic::usub_with_overflow;
    OverflowKind = SanitizerHandler::SubOverflow;
    break;
  case BO_Mul:
  case BO_MulAssign:
    OpID = 3;
    IID = isSigned ? toolchain::Intrinsic::smul_with_overflow :
                     toolchain::Intrinsic::umul_with_overflow;
    OverflowKind = SanitizerHandler::MulOverflow;
    break;
  default:
    toolchain_unreachable("Unsupported operation for overflow detection");
  }
  OpID <<= 1;
  if (isSigned)
    OpID |= 1;
  ...
  SanitizerDebugLocation SanScope(&CGF,
                                  ...
                                  OverflowKind);
  toolchain::Type *opTy = CGF.CGM.getTypes().ConvertType(Ops.Ty);

  toolchain::Function *intrinsic = CGF.CGM.getIntrinsic(IID, opTy);

  Value *resultAndOverflow = Builder.CreateCall(intrinsic, {Ops.LHS, Ops.RHS});
  Value *result = Builder.CreateExtractValue(resultAndOverflow, 0);
  Value *overflow = Builder.CreateExtractValue(resultAndOverflow, 1);

  // Handle overflow with toolchain.trap if no custom handler has been specified.
  ...
  if (handlerName->empty()) {
    ...
    if (!isSigned || CGF.SanOpts.has(SanitizerKind::SignedIntegerOverflow)) {
      toolchain::Value *NotOverflow = Builder.CreateNot(overflow);
      SanitizerKind::SanitizerOrdinal Ordinal =
  ...
  toolchain::BasicBlock *initialBB = Builder.GetInsertBlock();
  toolchain::BasicBlock *continueBB =
      CGF.createBasicBlock("nooverflow", CGF.CurFn, initialBB->getNextNode());
  toolchain::BasicBlock *overflowBB = CGF.createBasicBlock("overflow", CGF.CurFn);
  ...
  toolchain::Type *Int8Ty = CGF.Int8Ty;
  toolchain::Type *argTypes[] = { CGF.Int64Ty, CGF.Int64Ty, Int8Ty, Int8Ty };
  toolchain::FunctionType *handlerTy =
      toolchain::FunctionType::get(CGF.Int64Ty, argTypes, true);
  toolchain::FunctionCallee handler =
      CGF.CGM.CreateRuntimeFunction(handlerTy, *handlerName);

  // Sign extend the args to 64-bit, so that we can use the same handler for
  // all types of overflow.
  toolchain::Value *lhs = Builder.CreateSExt(Ops.LHS, CGF.Int64Ty);
  toolchain::Value *rhs = Builder.CreateSExt(Ops.RHS, CGF.Int64Ty);

  // Call the handler with the two arguments, the operation, and the size of
  // the result.
  toolchain::Value *handlerArgs[] = {
    lhs,
    rhs,
    Builder.getInt8(OpID),
    Builder.getInt8(cast<toolchain::IntegerType>(opTy)->getBitWidth())
  };
  toolchain::Value *handlerResult =
  ...
  toolchain::PHINode *phi = Builder.CreatePHI(opTy, 2);
...
toolchain::Value *CodeGenFunction::EmitPointerArithmetic(
    const BinaryOperator *BO, Expr *pointerOperand, toolchain::Value *pointer,
    Expr *indexOperand, toolchain::Value *index, bool isSubtraction) {
  bool isSigned = indexOperand->getType()->isSignedIntegerOrEnumerationType();

  unsigned width = cast<toolchain::IntegerType>(index->getType())->getBitWidth();
  auto &DL = CGM.getDataLayout();
  auto *PtrTy = cast<toolchain::PointerType>(pointer->getType());
  ...
  if (BinaryOperator::isNullPointerArithmeticExtension(
    ...
    toolchain::Value *Ptr = Builder.CreateIntToPtr(index, pointer->getType());
    ...
    toolchain::Value *IsZeroIndex = Builder.CreateIsNull(index);
    toolchain::Constant *StaticArgs[] = {EmitCheckSourceLocation(BO->getExprLoc())};
    toolchain::Type *IntPtrTy = DL.getIntPtrType(PtrTy);
    toolchain::Value *IntPtr = toolchain::Constant::getNullValue(IntPtrTy);
    toolchain::Value *ComputedGEP = Builder.CreateZExtOrTrunc(index, IntPtrTy);
    toolchain::Value *DynamicArgs[] = {IntPtr, ComputedGEP};
  ...
  if (!pointerType) {
    QualType objectType = pointerOperand->getType()
                              ...
                              ->getPointeeType();
    toolchain::Value *objectSize =
        CGM.getSize(getContext().getTypeSizeInChars(objectType));

    index = Builder.CreateMul(index, objectSize);

    toolchain::Value *result = Builder.CreateGEP(Int8Ty, pointer, index, "add.ptr");
  ...
  if (const VariableArrayType *vla =
    ...
    toolchain::Type *elemTy = ConvertTypeForMem(vla->getElementType());
    if (getLangOpts().PointerOverflowDefined) {
...
static Value* buildFMulAdd(toolchain::Instruction *MulOp, Value *Addend,
  ...
  if (Builder.getIsFPConstrained()) {
    assert(isa<toolchain::ConstrainedFPIntrinsic>(MulOp) &&
           "Only constrained operation should be created when Builder is in FP "
           "constrained mode");
    FMulAdd = Builder.CreateConstrainedFPCall(
        CGF.CGM.getIntrinsic(toolchain::Intrinsic::experimental_constrained_fmuladd,
                             Addend->getType()),
        {MulOp0, MulOp1, Addend});
  } else {
    FMulAdd = Builder.CreateCall(
        CGF.CGM.getIntrinsic(toolchain::Intrinsic::fmuladd, Addend->getType()),
        {MulOp0, MulOp1, Addend});
  }
...
static Value* tryEmitFMulAdd(const BinOpInfo &op,
  ...
  bool NegLHS = false;
  if (auto *LHSUnOp = dyn_cast<toolchain::UnaryOperator>(LHS)) {
    if (LHSUnOp->getOpcode() == toolchain::Instruction::FNeg &&
  ...
  bool NegRHS = false;
  if (auto *RHSUnOp = dyn_cast<toolchain::UnaryOperator>(RHS)) {
    if (RHSUnOp->getOpcode() == toolchain::Instruction::FNeg &&
  ...
  if (auto *LHSBinOp = dyn_cast<toolchain::BinaryOperator>(LHS)) {
    if (LHSBinOp->getOpcode() == toolchain::Instruction::FMul &&
        (LHSBinOp->use_empty() || NegLHS)) {
      // If we looked through fneg, erase it.
      if (NegLHS)
        cast<toolchain::Instruction>(op.LHS)->eraseFromParent();
      return buildFMulAdd(LHSBinOp, op.RHS, CGF, Builder, NegLHS, isSub);
    }
  }
  if (auto *RHSBinOp = dyn_cast<toolchain::BinaryOperator>(RHS)) {
    if (RHSBinOp->getOpcode() == toolchain::Instruction::FMul &&
        (RHSBinOp->use_empty() || NegRHS)) {
      // If we looked through fneg, erase it.
      if (NegRHS)
        cast<toolchain::Instruction>(op.RHS)->eraseFromParent();
      return buildFMulAdd(RHSBinOp, op.LHS, CGF, Builder, isSub ^ NegRHS, false);
    }
  }

  if (auto *LHSBinOp = dyn_cast<toolchain::CallBase>(LHS)) {
    if (LHSBinOp->getIntrinsicID() ==
            toolchain::Intrinsic::experimental_constrained_fmul &&
        (LHSBinOp->use_empty() || NegLHS)) {
      // If we looked through fneg, erase it.
      if (NegLHS)
        cast<toolchain::Instruction>(op.LHS)->eraseFromParent();
  ...
  if (auto *RHSBinOp = dyn_cast<toolchain::CallBase>(RHS)) {
...
Value *ScalarExprEmitter::EmitFixedPointBinOp(const BinOpInfo &op) {
  ...
  case BO_LAnd:
  case BO_LOr:
    toolchain_unreachable("Found unimplemented fixed point binary operation");
  case BO_PtrMemD:
  case BO_PtrMemI:
  case BO_Rem:
  case BO_Xor:
  ...
  case BO_OrAssign:
  case BO_Comma:
    toolchain_unreachable("Found unsupported binary operation for fixed point types.");
  }
...
Value *ScalarExprEmitter::EmitSub(const BinOpInfo &op) {
  ...
  toolchain::Value *divisor = nullptr;
...
Value *ScalarExprEmitter::GetMaximumShiftAmount(Value *LHS, Value *RHS,
                                                bool RHSIsSigned) {
  toolchain::IntegerType *Ty;
  if (toolchain::VectorType *VT = dyn_cast<toolchain::VectorType>(LHS->getType()))
    Ty = cast<toolchain::IntegerType>(VT->getElementType());
  else
    Ty = cast<toolchain::IntegerType>(LHS->getType());
  ...
  toolchain::Type *RHSTy = RHS->getType();
  toolchain::APInt RHSMax =
      RHSIsSigned ? toolchain::APInt::getSignedMaxValue(RHSTy->getScalarSizeInBits())
                  : toolchain::APInt::getMaxValue(RHSTy->getScalarSizeInBits());
  if (RHSMax.ult(Ty->getBitWidth()))
    return toolchain::ConstantInt::get(RHSTy, RHSMax);
  return toolchain::ConstantInt::get(RHSTy, Ty->getBitWidth() - 1);
}

Value *ScalarExprEmitter::ConstrainShiftValue(Value *LHS, Value *RHS,
                                              const Twine &Name) {
  toolchain::IntegerType *Ty;
  if (auto *VT = dyn_cast<toolchain::VectorType>(LHS->getType()))
    Ty = cast<toolchain::IntegerType>(VT->getElementType());
  else
    Ty = cast<toolchain::IntegerType>(LHS->getType());

  if (toolchain::isPowerOf2_64(Ty->getBitWidth()))
    return Builder.CreateAnd(RHS, GetMaximumShiftAmount(LHS, RHS, false), Name);

  return Builder.CreateURem(
      RHS, toolchain::ConstantInt::get(RHS->getType(), Ty->getBitWidth()), Name);
}
...
Value *ScalarExprEmitter::EmitShl(const BinOpInfo &Ops) {
  ...
  if (CGF.getLangOpts().OpenCL || CGF.getLangOpts().HLSL)
    RHS = ConstrainShiftValue(Ops.LHS, RHS, "shl.mask");
           ...
           isa<toolchain::IntegerType>(Ops.LHS->getType())) {
    ...
    toolchain::Value *WidthMinusOne =
        GetMaximumShiftAmount(Ops.LHS, Ops.RHS, RHSIsSigned);
    toolchain::Value *ValidExponent = Builder.CreateICmpULE(Ops.RHS, WidthMinusOne);
    ...
    if (SanitizeBase) {
      ...
      toolchain::BasicBlock *Orig = Builder.GetInsertBlock();
      toolchain::BasicBlock *Cont = CGF.createBasicBlock("cont");
      toolchain::BasicBlock *CheckShiftBase = CGF.createBasicBlock("check");
      Builder.CreateCondBr(ValidExponent, CheckShiftBase, Cont);
      toolchain::Value *PromotedWidthMinusOne =
          (RHS == Ops.RHS) ? WidthMinusOne
                           : GetMaximumShiftAmount(Ops.LHS, RHS, RHSIsSigned);
      CGF.EmitBlock(CheckShiftBase);
      toolchain::Value *BitsShiftedOff = Builder.CreateLShr(
      ...
      if (SanitizeUnsignedBase || CGF.getLangOpts().CPlusPlus) {
        ...
        toolchain::Value *One = toolchain::ConstantInt::get(BitsShiftedOff->getType(), 1);
        BitsShiftedOff = Builder.CreateLShr(BitsShiftedOff, One);
      }
      toolchain::Value *Zero = toolchain::ConstantInt::get(BitsShiftedOff->getType(), 0);
      toolchain::Value *ValidBase = Builder.CreateICmpEQ(BitsShiftedOff, Zero);
      CGF.EmitBlock(Cont);
      toolchain::PHINode *BaseCheck = Builder.CreatePHI(ValidBase->getType(), 2);
...
Value *ScalarExprEmitter::EmitShr(const BinOpInfo &Ops) {
  ...
  if (CGF.getLangOpts().OpenCL || CGF.getLangOpts().HLSL)
    ...
    SanitizerDebugLocation SanScope(&CGF, {SanitizerKind::SO_ShiftExponent},
                                    SanitizerHandler::ShiftOutOfBounds);
    bool RHSIsSigned = Ops.rhsHasSignedIntegerRepresentation();
    toolchain::Value *Valid = Builder.CreateICmpULE(
...
static toolchain::Intrinsic::ID GetIntrinsic(IntrinsicType IT,
                                        BuiltinType::Kind ElemKind) {
  switch (ElemKind) {
  default: toolchain_unreachable("unexpected element type");
  case BuiltinType::Char_U:
  case BuiltinType::UChar:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequb_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtub_p;
  case BuiltinType::Char_S:
  case BuiltinType::SChar:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequb_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtsb_p;
  case BuiltinType::UShort:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequh_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtuh_p;
  case BuiltinType::Short:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequh_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtsh_p;
  case BuiltinType::UInt:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequw_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtuw_p;
  case BuiltinType::Int:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequw_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtsw_p;
  case BuiltinType::ULong:
  case BuiltinType::ULongLong:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequd_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtud_p;
  case BuiltinType::Long:
  case BuiltinType::LongLong:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequd_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtsd_p;
  case BuiltinType::Float:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpeqfp_p :
                            toolchain::Intrinsic::ppc_altivec_vcmpgtfp_p;
  case BuiltinType::Double:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_vsx_xvcmpeqdp_p :
                            toolchain::Intrinsic::ppc_vsx_xvcmpgtdp_p;
  case BuiltinType::UInt128:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequq_p
                          : toolchain::Intrinsic::ppc_altivec_vcmpgtuq_p;
  case BuiltinType::Int128:
    return (IT == VCMPEQ) ? toolchain::Intrinsic::ppc_altivec_vcmpequq_p
                          : toolchain::Intrinsic::ppc_altivec_vcmpgtsq_p;
  }
}

Value *ScalarExprEmitter::EmitCompare(const BinaryOperator *E,
                                      toolchain::CmpInst::Predicate UICmpOpc,
                                      toolchain::CmpInst::Predicate SICmpOpc,
                                      toolchain::CmpInst::Predicate FCmpOpc,
                                      bool IsSignaling) {
  TestAndClearIgnoreResultAssign();
  Value *Result;
  QualType LHSTy = E->getLHS()->getType();
  ...
  if (const MemberPointerType *MPT = LHSTy->getAs<MemberPointerType>()) {
    ...
    if (LHSTy->isVectorType() && !E->getType()->isVectorType()) {
      ...
      toolchain::Intrinsic::ID ID = toolchain::Intrinsic::not_intrinsic;
      ...
      switch(E->getOpcode()) {
      default: toolchain_unreachable("is not a comparison operation");
      case BO_EQ:
        CR6 = CR6_LT;
      ...
      case BO_LE:
        if (ElementKind == BuiltinType::Float) {
          CR6 = CR6_LT;
          ID = toolchain::Intrinsic::ppc_altivec_vcmpgefp_p;
          std::swap(FirstVecArg, SecondVecArg);
        }
        else {
          CR6 = CR6_EQ;
      ...
      toolchain::Function *F = CGF.CGM.getIntrinsic(ID);
      ...
      // crash later.
      toolchain::IntegerType *ResultTy = cast<toolchain::IntegerType>(Result->getType());
    ...
    if (BOInfo.isFixedPointOp()) {
      ...
      if (CGF.CGM.getCodeGenOpts().StrictVTablePointers &&
          !isa<toolchain::ConstantPointerNull>(LHS) &&
          !isa<toolchain::ConstantPointerNull>(RHS)) {
    ...
    if (auto *CTy = LHSTy->getAs<ComplexType>()) {
      ...
      LHS.second = toolchain::Constant::getNullValue(LHS.first->getType());
      CETy = LHSTy;
    }
    if (auto *CTy = RHSTy->getAs<ComplexType>()) {
      ...
      RHS.second = toolchain::Constant::getNullValue(RHS.first->getType());
...
toolchain::Value *CodeGenFunction::EmitWithOriginalRHSBitfieldAssignment(
...
Value *ScalarExprEmitter::VisitBinLAnd(const BinaryOperator *E) {
  ...
  if (E->getType()->isVectorType()) {
    ...
    Value *Zero = toolchain::ConstantAggregateZero::get(LHS->getType());
    if (LHS->getType()->isFPOrFPVectorTy()) {
      CodeGenFunction::CGFPOptionsRAII FPOptsRAII(
          CGF, E->getFPFeaturesInEffect(CGF.getLangOpts()));
      LHS = Builder.CreateFCmp(toolchain::CmpInst::FCMP_UNE, LHS, Zero, "cmp");
      RHS = Builder.CreateFCmp(toolchain::CmpInst::FCMP_UNE, RHS, Zero, "cmp");
    } else {
      LHS = Builder.CreateICmp(toolchain::CmpInst::ICMP_NE, LHS, Zero, "cmp");
      RHS = Builder.CreateICmp(toolchain::CmpInst::ICMP_NE, RHS, Zero, "cmp");
    }
  ...
  if (CGF.ConstantFoldsToSimpleInteger(E->getLHS(), LHSCondVal)) {
    if (LHSCondVal) { // If we have 1 && X, just emit X.
      ...
      if (InstrumentRegions &&
        ...
        toolchain::BasicBlock *FBlock = CGF.createBasicBlock("land.end");
        toolchain::BasicBlock *RHSBlockCnt = CGF.createBasicBlock("land.rhscnt");
    ...
    if (!CGF.ContainsLabel(E->getRHS())) {
      CGF.markStmtMaybeUsed(E->getRHS());
      return toolchain::Constant::getNullValue(ResTy);
    }
  }
  ...
  toolchain::BasicBlock *ContBlock = CGF.createBasicBlock("land.end");
  toolchain::BasicBlock *RHSBlock  = CGF.createBasicBlock("land.rhs");
  ...
  toolchain::PHINode *PN = toolchain::PHINode::Create(toolchain::Type::getInt1Ty(VMContext), 2,
                                            "", ContBlock);
  for (toolchain::pred_iterator PI = pred_begin(ContBlock), PE = pred_end(ContBlock);
       PI != PE; ++PI)
    PN->addIncoming(toolchain::ConstantInt::getFalse(VMContext), *PI);
...
Value *ScalarExprEmitter::VisitBinLOr(const BinaryOperator *E) {
  ...
  if (CGF.ConstantFoldsToSimpleInteger(E->getLHS(), LHSCondVal)) {
    if (!LHSCondVal) { // If we have 0 || X, just emit X.
      ...
      if (InstrumentRegions &&
          CodeGenFunction::isInstrumentedCondition(E->getRHS())) {
        CGF.maybeUpdateMCDCCondBitmap(E->getRHS(), RHSCond);
        toolchain::BasicBlock *FBlock = CGF.createBasicBlock("lor.end");
        toolchain::BasicBlock *RHSBlockCnt = CGF.createBasicBlock("lor.rhscnt");
    ...
    if (!CGF.ContainsLabel(E->getRHS())) {
      CGF.markStmtMaybeUsed(E->getRHS());
      return toolchain::ConstantInt::get(ResTy, 1);
    }
  }
  ...
  toolchain::BasicBlock *ContBlock = CGF.createBasicBlock("lor.end");
  toolchain::BasicBlock *RHSBlock = CGF.createBasicBlock("lor.rhs");
  ...
  for (toolchain::pred_iterator PI = pred_begin(ContBlock), PE = pred_end(ContBlock);
       PI != PE; ++PI)
    PN->addIncoming(toolchain::ConstantInt::getTrue(VMContext), *PI);
...
VisitAbstractConditionalOperator(const AbstractConditionalOperator *E) {
  ...
  if (CGF.ConstantFoldsToSimpleInteger(condExpr, CondExprBool)) {
    ...
    if (!CGF.ContainsLabel(dead)) {
      if (CondExprBool) {
        if (toolchain::EnableSingleByteCoverage) {
      ...
      if (!Result && !E->getType()->isVoidType())
        Result = toolchain::UndefValue::get(CGF.ConvertType(E->getType()));
  ...
  if ((CGF.getLangOpts().OpenCL && condExpr->getType()->isVectorType()) ||
    ...
    toolchain::Value *CondV = CGF.EmitScalarExpr(condExpr);
    toolchain::Value *LHS = Visit(lhsExpr);
    toolchain::Value *RHS = Visit(rhsExpr);

    toolchain::Type *condType = ConvertType(condExpr->getType());
    auto *vecTy = cast<toolchain::FixedVectorType>(condType);

    unsigned numElem = vecTy->getNumElements();
    toolchain::Type *elemType = vecTy->getElementType();

    toolchain::Value *zeroVec = toolchain::Constant::getNullValue(vecTy);
    toolchain::Value *TestMSB = Builder.CreateICmpSLT(CondV, zeroVec);
    toolchain::Value *tmp = Builder.CreateSExt(
        TestMSB, toolchain::FixedVectorType::get(elemType, numElem), "sext");
    toolchain::Value *tmp2 = Builder.CreateNot(tmp);

    // Cast float to int to perform ANDs if necessary.
    toolchain::Value *RHSTmp = RHS;
    toolchain::Value *LHSTmp = LHS;
    bool wasCast = false;
    toolchain::VectorType *rhsVTy = cast<toolchain::VectorType>(RHS->getType());
    ...
    toolchain::Value *tmp3 = Builder.CreateAnd(RHSTmp, tmp2);
    toolchain::Value *tmp4 = Builder.CreateAnd(LHSTmp, tmp);
    toolchain::Value *tmp5 = Builder.CreateOr(tmp3, tmp4, "cond");
    if (wasCast)
  ...
  if (condExpr->getType()->isVectorType() ||
    ...
    toolchain::Type *CondType = ConvertType(condExpr->getType());
    auto *VecTy = cast<toolchain::VectorType>(CondType);
    toolchain::Value *ZeroVec = toolchain::Constant::getNullValue(VecTy);
  ...
  if (isCheapEnoughToEvaluateUnconditionally(lhsExpr, CGF) &&
    ...
    toolchain::Value *CondV = CGF.EvaluateExprAsBool(condExpr);
    toolchain::Value *StepV = Builder.CreateZExtOrBitCast(CondV, CGF.Int64Ty);
  ...
  toolchain::BasicBlock *LHSBlock = CGF.createBasicBlock("cond.true");
  toolchain::BasicBlock *RHSBlock = CGF.createBasicBlock("cond.false");
  toolchain::BasicBlock *ContBlock = CGF.createBasicBlock("cond.end");
  ...
  toolchain::PHINode *PN = Builder.CreatePHI(LHS->getType(), 2, "cond");
...
static Value *ConvertVec3AndVec4(CGBuilderTy &Builder, CodeGenFunction &CGF,
  ...
  return Builder.CreateShuffleVector(Src, toolchain::ArrayRef(Mask, NumElementsDst));
}
...
static Value *createCastsForTypeOfSameSize(CGBuilderTy &Builder,
                                           ...
                                           Value *Src, toolchain::Type *DstTy,
                                           StringRef Name = "") {
...
Value *ScalarExprEmitter::VisitAsTypeExpr(AsTypeExpr *E) {
  Value *Src  = CGF.EmitScalarExpr(E->getSrcExpr());
  toolchain::Type *DstTy = ConvertType(E->getType());

  toolchain::Type *SrcTy = Src->getType();
  unsigned NumElementsSrc =
      isa<toolchain::VectorType>(SrcTy)
          ? cast<toolchain::FixedVectorType>(SrcTy)->getNumElements()
          : 0;
  unsigned NumElementsDst =
      isa<toolchain::VectorType>(DstTy)
          ? cast<toolchain::FixedVectorType>(DstTy)->getNumElements()
          : 0;
  ...
  // get a vec3.
  if (NumElementsSrc != 3 && NumElementsDst == 3) {
    auto *Vec4Ty = toolchain::FixedVectorType::get(
        cast<toolchain::VectorType>(DstTy)->getElementType(), 4);
    Src = createCastsForTypeOfSameSize(Builder, CGF.CGM.getDataLayout(), Src,
                                       Vec4Ty);
...
#undef COMPOUND_OP
  ...
  case BO_Assign:
  case BO_Comma:
    toolchain_unreachable("Not valid compound assignment operators");
  }

  toolchain_unreachable("Unhandled compound assignment operator");
}

struct GEPOffsetAndOverflow {
  // The total (signed) byte offset for the GEP.
  toolchain::Value *TotalOffset;
  // The offset overflow flag - true if the total offset overflows.
  toolchain::Value *OffsetOverflows;
};
...
static GEPOffsetAndOverflow EmitGEPOffsetInBytes(Value *BasePtr, Value *GEPVal,
  ...
  // The total (signed) byte offset for the GEP.
  toolchain::Value *TotalOffset = nullptr;

  // Was the GEP already reduced to a constant?
  if (isa<toolchain::Constant>(GEPVal)) {
  ...
  auto *GEP = cast<toolchain::GEPOperator>(GEPVal);
  ...
  auto *Zero = toolchain::ConstantInt::getNullValue(IntPtrTy);
  auto *SAddIntrinsic =
      CGM.getIntrinsic(toolchain::Intrinsic::sadd_with_overflow, IntPtrTy);
  auto *SMulIntrinsic =
      CGM.getIntrinsic(toolchain::Intrinsic::smul_with_overflow, IntPtrTy);

  // The offset overflow flag - true if the total offset overflows.
  toolchain::Value *OffsetOverflows = Builder.getFalse();

  /// Return the result of the given binary operation.
  auto eval = [&](BinaryOperator::Opcode Opcode, toolchain::Value *LHS,
                  toolchain::Value *RHS) -> toolchain::Value * {
    assert((Opcode == BO_Add || Opcode == BO_Mul) && "Can't eval binop");

    // If the operands are constants, return a constant result.
    if (auto *LHSCI = dyn_cast<toolchain::ConstantInt>(LHS)) {
      if (auto *RHSCI = dyn_cast<toolchain::ConstantInt>(RHS)) {
        toolchain::APInt N;
        bool HasOverflow = mayHaveIntegerOverflow(LHSCI, RHSCI, Opcode,
                                                  /*Signed=*/true, N);
        if (HasOverflow)
          OffsetOverflows = Builder.getTrue();
        return toolchain::ConstantInt::get(VMContext, N);
      }
    }
  ...
  for (auto GTI = toolchain::gep_type_begin(GEP), GTE = toolchain::gep_type_end(GEP);
       GTI != GTE; ++GTI) {
    toolchain::Value *LocalOffset;
    ...
    if (auto *STy = GTI.getStructTypeOrNull()) {
      ...
      // specified field.
      unsigned FieldNo = cast<toolchain::ConstantInt>(Index)->getZExtValue();
      LocalOffset = toolchain::ConstantInt::get(
          IntPtrTy, DL.getStructLayout(STy)->getElementOffset(FieldNo));
    } else {
      // Otherwise this is array-like indexing. The local offset is the index
      // multiplied by the element size.
      auto *ElementSize =
          toolchain::ConstantInt::get(IntPtrTy, GTI.getSequentialElementStride(DL));
...
Value *
CodeGenFunction::EmitCheckedInBoundsGEP(toolchain::Type *ElemTy, Value *Ptr,
                                        ArrayRef<Value *> IdxList,
                                        bool SignedIndices, bool IsSubtraction,
                                        SourceLocation Loc, const Twine &Name) {
  toolchain::Type *PtrTy = Ptr->getType();

  toolchain::GEPNoWrapFlags NWFlags = toolchain::GEPNoWrapFlags::inBounds();
  if (!SignedIndices && !IsSubtraction)
    NWFlags |= toolchain::GEPNoWrapFlags::noUnsignedWrap();
  ...
  bool PerformOverflowCheck =
      !isa<toolchain::Constant>(GEPVal) && PtrTy->getPointerAddressSpace() == 0;
  ...
  assert((!isa<toolchain::Constant>(EvaluatedGEP.TotalOffset) ||
  ...
  if (PerformOverflowCheck) {
    ...
    if (SignedIndices) {
      ...
      toolchain::Value *NegValid = Builder.CreateICmpULT(ComputedGEP, IntPtr);
      ValidGEP =
      ...
      // Equivalent to `@toolchain.uadd.with.overflow(%base, %offset)`.
      ValidGEP = Builder.CreateICmpUGE(ComputedGEP, IntPtr);
    } else {
      // GEP is computed as `unsigned base - unsigned offset`, therefore the
      // computed pointer can not be [unsigned] greater than base pointer,
      // unless there was an overflow.
      // Equivalent to `@toolchain.usub.with.overflow(%base, sub(0, %offset))`.
...
Address CodeGenFunction::EmitCheckedInBoundsGEP(
    Address Addr, ArrayRef<Value *> IdxList, toolchain::Type *elementType,
