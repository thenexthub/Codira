...
#include "toolchain/Analysis/ObjCARCUtil.h"
...
typedef toolchain::PointerIntPair<toolchain::Value*,1,bool> TryEmitResult;
...
static toolchain::Constant *getNullForVariable(Address addr) {
  toolchain::Type *type = addr.getElementType();
  return toolchain::ConstantPointerNull::get(cast<toolchain::PointerType>(type));
}

/// Emits an instance of NSConstantString representing the object.
toolchain::Value *CodeGenFunction::EmitObjCStringLiteral(const ObjCStringLiteral *E)
{
...
CodeGenFunction::EmitObjCBoxedExpr(const ObjCBoxedExpr *E) {
  ...
  toolchain::Value *Receiver = Runtime.GetClass(*this, ClassDecl);
  ...
  if (ValueType->isObjCBoxableRecordType()) {
    ...
    toolchain::Value *BitCast = Builder.CreateBitCast(
    ...
    toolchain::Constant *GV = CGM.GetAddrOfConstantCString(Str).getPointer();
    ...
    toolchain::Value *Cast = Builder.CreateBitCast(GV, ConvertType(EncodingQT));
...
toolchain::Value *CodeGenFunction::EmitObjCCollectionLiteral(const Expr *E,
  ...
  if (NumElements == 0 && CGM.getLangOpts().ObjCRuntime.hasEmptyCollections()) {
    ...
    toolchain::Value *Ptr = EmitLoadOfScalar(LV, E->getBeginLoc());
    cast<toolchain::LoadInst>(Ptr)->setMetadata(
  ...
  toolchain::APInt APNumElements(Context.getTypeSize(Context.getSizeType()),
                            NumElements);
  ...
  SmallVector<toolchain::Value *, 16> NeededObjects;
  ...
  for (uint64_t i = 0; i < NumElements; i++) {
    if (ALE) {
      ...
      toolchain::Value *value = EmitScalarExpr(Rhs);
      ...
      LValue KeyLV = MakeAddrLValue(Builder.CreateConstArrayGEP(Keys, i),
                                    ElementType, AlignmentSource::Decl);
      toolchain::Value *keyValue = EmitScalarExpr(Key);
      ...
      LValue ValueLV = MakeAddrLValue(Builder.CreateConstArrayGEP(Objects, i),
                                      ElementType, AlignmentSource::Decl);
      toolchain::Value *valueValue = EmitScalarExpr(Value);
  ...
  toolchain::Value *Count =
    toolchain::ConstantInt::get(CGM.getTypes().ConvertType(ArgQT), NumElements);
  ...
  CGObjCRuntime &Runtime = CGM.getObjCRuntime();
  toolchain::Value *Receiver = Runtime.GetClass(*this, Class);
...
toolchain::Value *CodeGenFunction::EmitObjCArrayLiteral(const ObjCArrayLiteral *E) {
  return EmitObjCCollectionLiteral(E, E->getArrayWithObjectsMethod());
}

toolchain::Value *CodeGenFunction::EmitObjCDictionaryLiteral(
                                            const ObjCDictionaryLiteral *E) {
  return EmitObjCCollectionLiteral(E, E->getDictWithObjectsMethod());
}

/// Emit a selector.
toolchain::Value *CodeGenFunction::EmitObjCSelectorExpr(const ObjCSelectorExpr *E) {
  // Untyped selector.
...
toolchain::Value *CodeGenFunction::EmitObjCProtocolExpr(const ObjCProtocolExpr *E) {
...
static RValue AdjustObjCObjectType(CodeGenFunction &CGF, QualType ExpT,
  ...
  toolchain::Type *ExpLLVMTy = CGF.ConvertType(ExpT);
...
shouldExtendReceiverForInnerPointerMessage(const ObjCMessageExpr *message) {
  ...
  toolchain_unreachable("invalid receiver kind");
}
...
static std::optional<toolchain::Value *> tryGenerateSpecializedMessageSend(
    CodeGenFunction &CGF, QualType ResultType, toolchain::Value *Receiver,
  ...
  case OMF_alloc:
    if (isClassMessage &&
        ...
        if (Sel.isKeywordSelector() && Sel.getNumArgs() == 1 &&
          ...
          const toolchain::Value* arg = Args.front().getKnownRValue().getScalarVal();
          if (isa<toolchain::ConstantPointerNull>(arg))
...
CodeGen::RValue CGObjCRuntime::GeneratePossiblySpecializedMessageSend(
    ...
    bool isClassMessage) {
  if (std::optional<toolchain::Value *> SpecializedResult =
...
static void AppendFirstImpliedRuntimeProtocols(
    ...
    toolchain::UniqueVector<const ObjCProtocolDecl *> &PDs) {
...
CGObjCRuntime::GetRuntimeProtocolList(ObjCProtocolDecl::protocol_iterator begin,
  ...
  toolchain::DenseSet<const ObjCProtocolDecl *> NonRuntimePDs;
  ...
  toolchain::UniqueVector<const ObjCProtocolDecl *> FirstImpliedProtos;
  ...
  toolchain::DenseSet<const ObjCProtocolDecl *> AllImpliedProtocols;
...
static std::optional<toolchain::Value *>
tryEmitSpecializedAllocInit(CodeGenFunction &CGF, const ObjCMessageExpr *OME) {
  ...
  toolchain::Value *Receiver = nullptr;
...
RValue CodeGenFunction::EmitObjCMessageExpr(const ObjCMessageExpr *E,
  ...
  if (method && E->getReceiverKind() == ObjCMessageExpr::Instance &&
    ...
    if (auto lvalueExpr = findWeakLValue(E->getInstanceReceiver())) {
      ...
      toolchain::Value *result = EmitARCLoadWeakRetained(lvalue.getAddress());
      return AdjustObjCObjectType(*this, E->getType(), RValue::get(result));
    }
  }

  if (std::optional<toolchain::Value *> Val = tryEmitSpecializedAllocInit(*this, E))
  ...
  if (isDelegateInit) {
    Address selfAddr =
      GetAddrOfLocalVar(cast<ObjCMethodDecl>(CurCodeDecl)->getSelfDecl());
    toolchain::Value *newSelf = result.getScalarVal();

    // The delegate return type isn't necessarily a matching type; in
    // fact, it's quite likely to be 'id'.
    toolchain::Type *selfTy = selfAddr.getElementType();
...
struct FinishARCDealloc final : EHScopeStack::Cleanup {
  void Emit(CodeGenFunction &CGF, Flags flags) override {
    ...
    toolchain::Value *self = CGF.LoadObjCSelf();
...
void CodeGenFunction::StartObjCMethod(const ObjCMethodDecl *OMD,
  ...
  toolchain::Function *Fn = CGM.getObjCRuntime().GenerateMethod(OMD, CD);
...
static toolchain::Value *emitARCRetainLoadOfScalar(CodeGenFunction &CGF,
...
static void emitStructGetterCall(CodeGenFunction &CGF, ObjCIvarDecl *ivar,
  ...
  toolchain::Value *dest = CGF.ReturnValue.emitRawPointer(CGF);
  ...
  toolchain::FunctionCallee fn = CGF.CGM.getObjCRuntime().GetGetStructFunction();
...
/// call and a mutex.
static bool hasUnalignedAtomics(toolchain::Triple::ArchType arch) {
...
static void emitCPPObjectAtomicGetterCall(CodeGenFunction &CGF,
  ...
  toolchain::FunctionCallee copyCppAtomicObjectFn =
...
// `_cmd` storage.
static toolchain::Value *emitCmdValueForGetterSetterBody(CodeGenFunction &CGF,
                                                   ObjCMethodDecl *MD) {
  if (MD->isDirectMethod()) {
    ...
    toolchain::Type *selType = CGF.ConvertType(CGF.getContext().getObjCSelType());
    return toolchain::PoisonValue::get(selType);
  }
...
CodeGenFunction::generateObjCGetterBody(const ObjCImplementationDecl *classImpl,
  ...
  case PropertyImplStrategy::Native: {
    ...
    toolchain::Type *bitcastType = toolchain::Type::getIntNTy(getLLVMContext(), ivarSize);

    // Perform an atomic load.  This does not impose ordering constraints.
    Address ivarAddr = LV.getAddress();
    ivarAddr = ivarAddr.withElementType(bitcastType);
    toolchain::LoadInst *load = Builder.CreateLoad(ivarAddr, "load");
    load->setAtomic(toolchain::AtomicOrdering::Unordered);
    toolchain::Value *ivarVal = load;
    ...
    toolchain::Type *retTy = ConvertType(getterMethod->getReturnType());
    uint64_t retTySize = CGM.getDataLayout().getTypeSizeInBits(retTy);
    if (ivarSize > retTySize) {
      bitcastType = toolchain::Type::getIntNTy(getLLVMContext(), retTySize);
  ...
  case PropertyImplStrategy::GetSetProperty: {
    toolchain::FunctionCallee getPropertyFn =
    ...
    toolchain::Value *cmd = emitCmdValueForGetterSetterBody(*this, getterMethod);
    toolchain::Value *self = Builder.CreateBitCast(LoadObjCSelf(), VoidPtrTy);
    ...
    toolchain::CallBase *CallInstruction;
    ...
    if (toolchain::CallInst *call = dyn_cast<toolchain::CallInst>(CallInstruction))
      call->setTailCall();
    ...
    case TEK_Scalar: {
      ...
      if (propType->isReferenceType()) {
        ...
        // final autorelease.
          ...
          if (PointerAuthQualifier PAQ = ivar->getType().getPointerAuth()) {
            Address ivarAddr = LV.getAddress();
            toolchain::LoadInst *LoadInst = Builder.CreateLoad(ivarAddr, "load");
            toolchain::Value *Load = LoadInst;
  ...
  toolchain_unreachable("bad @property implementation strategy!");
}
...
static void emitStructSetterCall(CodeGenFunction &CGF, ObjCMethodDecl *OMD,
  ...
  DeclRefExpr argRef(CGF.getContext(), argVar, false,
                     ...
                     SourceLocation());
  toolchain::Value *argAddr = CGF.EmitLValue(&argRef).getPointer(CGF);
  ...
  toolchain::FunctionCallee fn = CGF.CGM.getObjCRuntime().GetSetStructFunction();
  CGCallee callee = CGCallee::forDirect(fn);
  CGF.EmitCall(
      CGF.getTypes().arrangeBuiltinFunctionCall(CGF.getContext().VoidTy, args),
               callee, ReturnValueSlot(), args);
...
CodeGenFunction::generateObjCSetterBody(const ObjCImplementationDecl *classImpl,
  ...
  case PropertyImplStrategy::Native: {
    ...
    // Currently, all atomic accesses have to be through integer
    // types, so there's no point in trying to pick a prettier type.
    toolchain::Type *castType = toolchain::Type::getIntNTy(
    ...
    toolchain::Value *load = Builder.CreateLoad(argAddr);
    ...
    toolchain::StoreInst *store = Builder.CreateStore(load, ivarAddr);
    store->setAtomic(toolchain::AtomicOrdering::Unordered);
    return;
  }
  ...
  case PropertyImplStrategy::SetPropertyAndExpressionGet: {
    ...
    toolchain::FunctionCallee setOptimizedPropertyFn = nullptr;
    toolchain::FunctionCallee setPropertyFn = nullptr;
    ...
    //                       <is-atomic>, <is-copy>).
    toolchain::Value *cmd = emitCmdValueForGetterSetterBody(*this, setterMethod);
    toolchain::Value *self =
      Builder.CreateBitCast(LoadObjCSelf(), VoidPtrTy);
    toolchain::Value *ivarOffset =
        EmitIvarOffsetAsPointerDiff(classImpl->getClassInterface(), ivar);
    Address argAddr = GetAddrOfLocalVar(*setterMethod->param_begin());
    toolchain::Value *arg = Builder.CreateLoad(argAddr, "arg");
...
namespace {
  ...
  public:
    DestroyIvar(toolchain::Value *addr, const ObjCIvarDecl *ivar,
                CodeGenFunction::Destroyer *destroyer,
                bool useEHCleanupForArray)
...
static void destroyARCStrongWithStore(CodeGenFunction &CGF,
                                      Address addr,
                                      QualType type) {
  toolchain::Value *null = getNullForVariable(addr);
...
toolchain::Value *CodeGenFunction::LoadObjCSelf() {
...
void CodeGenFunction::EmitObjCForCollectionStmt(const ObjCForCollectionStmt &S){
  toolchain::FunctionCallee EnumerationMutationFnPtr =
  ...
  QualType ItemsTy = getContext().getConstantArrayType(
      getContext().getObjCIdType(), toolchain::APInt(32, NumItems), nullptr,
      ArraySizeModifier::Normal, 0);
  Address ItemsPtr = CreateMemTemp(ItemsTy, "items.ptr");

  // Emit the collection pointer.  In ARC, we do a retain.
  toolchain::Value *Collection;
  if (getLangOpts().ObjCAutoRefCount) {
  ...
  toolchain::Type *NSUIntegerTy = ConvertType(getContext().getNSUIntegerType());
  toolchain::Constant *Count = toolchain::ConstantInt::get(NSUIntegerTy, NumItems);
  ...
  toolchain::Value *initialBufferLimit = CountRV.getScalarVal();

  toolchain::BasicBlock *EmptyBB = createBasicBlock("forcoll.empty");
  toolchain::BasicBlock *LoopInitBB = createBasicBlock("forcoll.loopinit");

  toolchain::Value *zero = toolchain::Constant::getNullValue(NSUIntegerTy);
  ...
  toolchain::Value *StateMutationsPtr
    = Builder.CreateLoad(StateMutationsPtrPtr, "mutationsptr");

  toolchain::Type *UnsignedLongTy = ConvertType(getContext().UnsignedLongTy);
  toolchain::Value *initialMutations =
    Builder.CreateAlignedLoad(UnsignedLongTy, StateMutationsPtr,
                              getPointerAlign(), "forcoll.initial-mutations");

  // Start looping.  This is the point we return to whenever we have a
  // fresh, non-empty batch of objects.
  toolchain::BasicBlock *LoopBodyBB = createBasicBlock("forcoll.loopbody");
  EmitBlock(LoopBodyBB);

  // The current index into the buffer.
  toolchain::PHINode *index = Builder.CreatePHI(NSUIntegerTy, 3, "forcoll.index");
  index->addIncoming(zero, LoopInitBB);

  // The current buffer size.
  toolchain::PHINode *count = Builder.CreatePHI(NSUIntegerTy, 3, "forcoll.count");
  ...
  toolchain::Value *currentMutations
    = Builder.CreateAlignedLoad(UnsignedLongTy, StateMutationsPtr,
                                getPointerAlign(), "statemutations");

  toolchain::BasicBlock *WasMutatedBB = createBasicBlock("forcoll.mutated");
  toolchain::BasicBlock *WasNotMutatedBB = createBasicBlock("forcoll.notmutated");
  ...
  toolchain::Type *ObjCIdType = ConvertType(getContext().getObjCIdType());
  toolchain::Value *V =
  ...
  toolchain::Type *convertedElementType = ConvertType(elementType);
  ...
  toolchain::Value *EnumStateItems =
    Builder.CreateLoad(StateItemsPtr, "stateitems");

  // Fetch the value at the current index from the buffer.
  toolchain::Value *CurrentItemPtr = Builder.CreateInBoundsGEP(
      ObjCIdType, EnumStateItems, index, "currentitem.ptr");
  toolchain::Value *CurrentItem =
  ...
  if (SanOpts.has(SanitizerKind::ObjCCast)) {
    ...
    if (InterfaceTy) {
      ...
      EmitCheck({{IsClass, CheckOrdinal}}, CheckHandler,
                ArrayRef<toolchain::Constant *>(StaticData), CurrentItem);
    }
  }
  ...
  toolchain::BasicBlock *FetchMoreBB = createBasicBlock("forcoll.refetch");

  // First we check in the local buffer.
  toolchain::Value *indexPlusOne =
      Builder.CreateNUWAdd(index, toolchain::ConstantInt::get(NSUIntegerTy, 1));
  ...
  toolchain::Value *refetchCount = CountRV.getScalarVal();
  ...
  if (!elementIsVariable) {
    ...
    toolchain::Value *null = toolchain::Constant::getNullValue(convertedElementType);
    elementLValue = EmitLValue(cast<Expr>(S.getElement()));
...
namespace {
  struct CallObjCRelease final : EHScopeStack::Cleanup {
    CallObjCRelease(toolchain::Value *object) : object(object) {}
...
toolchain::Value *CodeGenFunction::EmitObjCConsumeObject(QualType type,
                                                    toolchain::Value *object) {
...
toolchain::Value *CodeGenFunction::EmitObjCExtendObjectLifetime(QualType type,
                                                           toolchain::Value *value) {
...
void CodeGenFunction::EmitARCIntrinsicUse(ArrayRef<toolchain::Value*> values) {
  toolchain::Function *&fn = CGM.getObjCEntrypoints().clang_arc_use;
  if (!fn)
    fn = CGM.getIntrinsic(toolchain::Intrinsic::objc_clang_arc_use);
...
void CodeGenFunction::EmitARCNoopIntrinsicUse(ArrayRef<toolchain::Value *> values) {
  toolchain::Function *&fn = CGM.getObjCEntrypoints().clang_arc_noop_use;
  if (!fn)
    fn = CGM.getIntrinsic(toolchain::Intrinsic::objc_clang_arc_noop_use);
  EmitNounwindRuntimeCall(fn, values);
}

static void setARCRuntimeFunctionLinkage(CodeGenModule &CGM, toolchain::Value *RTF) {
  if (auto *F = dyn_cast<toolchain::Function>(RTF)) {
...
static void setARCRuntimeFunctionLinkage(CodeGenModule &CGM,
                                         toolchain::FunctionCallee RTF) {
  setARCRuntimeFunctionLinkage(CGM, RTF.getCallee());
}

static toolchain::Function *getARCIntrinsic(toolchain::Intrinsic::ID IntID,
                                       CodeGenModule &CGM) {
  toolchain::Function *fn = CGM.getIntrinsic(IntID);
...
static toolchain::Value *emitARCValueOperation(
    CodeGenFunction &CGF, toolchain::Value *value, toolchain::Type *returnType,
    toolchain::Function *&fn, toolchain::Intrinsic::ID IntID,
    toolchain::CallInst::TailCallKind tailKind = toolchain::CallInst::TCK_None) {
  if (isa<toolchain::ConstantPointerNull>(value))
    return value;
  ...
  toolchain::Type *origType = returnType ? returnType : value->getType();
  value = CGF.Builder.CreateBitCast(value, CGF.Int8PtrTy);

  // Call the function.
  toolchain::CallInst *call = CGF.EmitNounwindRuntimeCall(fn, value);
...
///   i8* (i8**)
static toolchain::Value *emitARCLoadOperation(CodeGenFunction &CGF, Address addr,
...
/// Perform an operation having the following signature:
///   i8* (i8**, i8*)
static toolchain::Value *emitARCStoreOperation(CodeGenFunction &CGF, Address addr,
                                          toolchain::Value *value,
                                          toolchain::Function *&fn,
                                          toolchain::Intrinsic::ID IntID,
                                          bool ignored) {
  ...
  toolchain::Type *origType = value->getType();
  ...
  toolchain::CallInst *result = CGF.EmitNounwindRuntimeCall(fn, args);
...
///   i8* (i8*)
/// where a null input causes a no-op and returns null.
static toolchain::Value *emitObjCValueOperation(CodeGenFunction &CGF,
                                           toolchain::Value *value,
  ...
  if (!fn) {
    toolchain::FunctionType *fnType =
      toolchain::FunctionType::get(CGF.Int8PtrTy, CGF.Int8PtrTy, false);
    fn = CGF.CGM.CreateRuntimeFunction(fnType, fnName);

    // We have Native ARC, so set nonlazybind attribute for performance
    if (toolchain::Function *f = dyn_cast<toolchain::Function>(fn.getCallee()))
      if (fnName == "objc_retain")
        f->addFnAttr(toolchain::Attribute::NonLazyBind);
  }
  ...
  // Call the function.
  toolchain::CallBase *Inst = CGF.EmitCallOrInvoke(fn, value);
  ...
  if (fnName == "objc_autorelease")
    if (auto *Call = dyn_cast<toolchain::CallInst>(Inst))
      Call->setTailCall();
...
toolchain::Value *CodeGenFunction::EmitARCRetain(QualType type, toolchain::Value *value) {
...
///   call i8* \@objc_retain(i8* %value)
toolchain::Value *CodeGenFunction::EmitARCRetainNonBlock(toolchain::Value *value) {
  return emitARCValueOperation(*this, value, nullptr,
                               CGM.getObjCEntrypoints().objc_retain,
                               toolchain::Intrinsic::objc_retain);
}
...
toolchain::Value *CodeGenFunction::EmitARCRetainBlock(toolchain::Value *value,
                                                 bool mandatory) {
  toolchain::Value *result
    = emitARCValueOperation(*this, value, nullptr,
                            CGM.getObjCEntrypoints().objc_retainBlock,
                            toolchain::Intrinsic::objc_retainBlock);
  ...
  // count as escaping.
  if (!mandatory && isa<toolchain::Instruction>(result)) {
    toolchain::CallInst *call
      = cast<toolchain::CallInst>(result->stripPointerCasts());
  ...
  if (!marker) {
    ...
    // in a moment.
      ...
      toolchain::FunctionType *type =
        toolchain::FunctionType::get(CGF.VoidTy, /*variadic*/false);

      marker = toolchain::InlineAsm::get(type, assembly, "", /*sideeffects*/ true);

    // If we're at -O1 and above, we don't want to litter the code
    // with this marker yet, so leave a breadcrumb for the ARC
    // optimizer to pick up.
    } else {
      const char *retainRVMarkerKey = toolchain::objcarc::getRVMarkerModuleFlagStr();
      if (!CGF.CGM.getModule().getModuleFlag(retainRVMarkerKey)) {
        auto *str = toolchain::MDString::get(CGF.getLLVMContext(), assembly);
        CGF.CGM.getModule().addModuleFlag(toolchain::Module::Error,
...
static toolchain::Value *emitOptimizedARCReturnCall(toolchain::Value *value,
                                               bool IsRetainRV,
                                               CodeGenFunction &CGF) {
  ...
  toolchain::Function *&EP = IsRetainRV
                            ? EPs.objc_retainAutoreleasedReturnValue
                            : EPs.objc_unsafeClaimAutoreleasedReturnValue;
  toolchain::Intrinsic::ID IID =
      IsRetainRV ? toolchain::Intrinsic::objc_retainAutoreleasedReturnValue
                 : toolchain::Intrinsic::objc_unsafeClaimAutoreleasedReturnValue;
  EP = getARCIntrinsic(IID, CGF.CGM);

  toolchain::Triple::ArchType Arch = CGF.CGM.getTriple().getArch();
  ...
  if (CGF.CGM.getCodeGenOpts().OptimizationLevel > 0 &&
      (Arch == toolchain::Triple::aarch64 || Arch == toolchain::Triple::aarch64_32 ||
       Arch == toolchain::Triple::x86_64)) {
    toolchain::Value *bundleArgs[] = {EP};
    toolchain::OperandBundleDef OB("clang.arc.attachedcall", bundleArgs);
    auto *oldCall = cast<toolchain::CallBase>(value);
    toolchain::CallBase *newCall = toolchain::CallBase::addOperandBundle(
        oldCall, toolchain::LLVMContext::OB_clang_arc_attachedcall, OB,
        oldCall->getIterator());
  ...
  toolchain::CallInst::TailCallKind tailKind =
      isNoTail ? toolchain::CallInst::TCK_NoTail : toolchain::CallInst::TCK_None;
...
toolchain::Value *
CodeGenFunction::EmitARCRetainAutoreleasedReturnValue(toolchain::Value *value) {
...
toolchain::Value *
CodeGenFunction::EmitARCUnsafeClaimAutoreleasedReturnValue(toolchain::Value *value) {
  return emitOptimizedARCReturnCall(value, false, *this);
}

/// Release the given object.
///   call void \@objc_release(i8* %value)
void CodeGenFunction::EmitARCRelease(toolchain::Value *value,
                                     ARCPreciseLifetime_t precise) {
  if (isa<toolchain::ConstantPointerNull>(value)) return;

  toolchain::Function *&fn = CGM.getObjCEntrypoints().objc_release;
  if (!fn)
    fn = getARCIntrinsic(toolchain::Intrinsic::objc_release, CGM);

  // Cast the argument to 'id'.
  value = Builder.CreateBitCast(value, Int8PtrTy);

  // Call objc_release.
  toolchain::CallInst *call = EmitNounwindRuntimeCall(fn, value);
...
toolchain::Value *CodeGenFunction::EmitARCStoreStrongCall(Address addr,
                                                     toolchain::Value *value,
                                                     bool ignored) {
  assert(addr.getElementType() == value->getType());

  toolchain::Function *&fn = CGM.getObjCEntrypoints().objc_storeStrong;
  if (!fn)
    fn = getARCIntrinsic(toolchain::Intrinsic::objc_storeStrong, CGM);
...
toolchain::Value *CodeGenFunction::EmitARCStoreStrong(LValue dst,
                                                 toolchain::Value *newValue,
                                                 bool ignored) {
  ...
  // Read the old value.
  toolchain::Value *oldValue = EmitLoadOfScalar(dst, SourceLocation());
...
toolchain::Value *CodeGenFunction::EmitARCAutorelease(toolchain::Value *value) {
  return emitARCValueOperation(*this, value, nullptr,
                               CGM.getObjCEntrypoints().objc_autorelease,
                               toolchain::Intrinsic::objc_autorelease);
}

/// Autorelease the given object.
///   call i8* \@objc_autoreleaseReturnValue(i8* %value)
toolchain::Value *
CodeGenFunction::EmitARCAutoreleaseReturnValue(toolchain::Value *value) {
  return emitARCValueOperation(*this, value, nullptr,
                            CGM.getObjCEntrypoints().objc_autoreleaseReturnValue,
                               toolchain::Intrinsic::objc_autoreleaseReturnValue,
                               toolchain::CallInst::TCK_Tail);
}

/// Do a fused retain/autorelease of the given object.
///   call i8* \@objc_retainAutoreleaseReturnValue(i8* %value)
toolchain::Value *
CodeGenFunction::EmitARCRetainAutoreleaseReturnValue(toolchain::Value *value) {
  return emitARCValueOperation(*this, value, nullptr,
                     CGM.getObjCEntrypoints().objc_retainAutoreleaseReturnValue,
                             toolchain::Intrinsic::objc_retainAutoreleaseReturnValue,
                               toolchain::CallInst::TCK_Tail);
}
...
toolchain::Value *CodeGenFunction::EmitARCRetainAutorelease(QualType type,
                                                       toolchain::Value *value) {
  if (!type->isBlockPointerType())
    return EmitARCRetainAutoreleaseNonBlock(value);

  if (isa<toolchain::ConstantPointerNull>(value)) return value;
...
///   call i8* \@objc_retainAutorelease(i8* %value)
toolchain::Value *
CodeGenFunction::EmitARCRetainAutoreleaseNonBlock(toolchain::Value *value) {
  return emitARCValueOperation(*this, value, nullptr,
                               CGM.getObjCEntrypoints().objc_retainAutorelease,
                               toolchain::Intrinsic::objc_retainAutorelease);
}

/// i8* \@objc_loadWeak(i8** %addr)
/// Essentially objc_autorelease(objc_loadWeakRetained(addr)).
toolchain::Value *CodeGenFunction::EmitARCLoadWeak(Address addr) {
  return emitARCLoadOperation(*this, addr,
                              CGM.getObjCEntrypoints().objc_loadWeak,
                              toolchain::Intrinsic::objc_loadWeak);
}

/// i8* \@objc_loadWeakRetained(i8** %addr)
toolchain::Value *CodeGenFunction::EmitARCLoadWeakRetained(Address addr) {
  return emitARCLoadOperation(*this, addr,
                              CGM.getObjCEntrypoints().objc_loadWeakRetained,
                              toolchain::Intrinsic::objc_loadWeakRetained);
}

/// i8* \@objc_storeWeak(i8** %addr, i8* %value)
/// Returns %value.
toolchain::Value *CodeGenFunction::EmitARCStoreWeak(Address addr,
                                               toolchain::Value *value,
                                               bool ignored) {
  return emitARCStoreOperation(*this, addr, value,
                               CGM.getObjCEntrypoints().objc_storeWeak,
                               toolchain::Intrinsic::objc_storeWeak, ignored);
}
...
void CodeGenFunction::EmitARCInitWeak(Address addr, toolchain::Value *value) {
  ...
  emitARCStoreOperation(*this, addr, value,
                        ...
                        toolchain::Intrinsic::objc_initWeak, /*ignored*/ true);
}

/// void \@objc_destroyWeak(i8** %addr)
/// Essentially objc_storeWeak(addr, nil).
void CodeGenFunction::EmitARCDestroyWeak(Address addr) {
  toolchain::Function *&fn = CGM.getObjCEntrypoints().objc_destroyWeak;
  if (!fn)
    fn = getARCIntrinsic(toolchain::Intrinsic::objc_destroyWeak, CGM);
...
void CodeGenFunction::EmitARCMoveWeak(Address dst, Address src) {
  emitARCCopyOperation(*this, dst, src,
                       ...
                       toolchain::Intrinsic::objc_moveWeak);
}
...
void CodeGenFunction::EmitARCCopyWeak(Address dst, Address src) {
  emitARCCopyOperation(*this, dst, src,
                       ...
                       toolchain::Intrinsic::objc_copyWeak);
}

void CodeGenFunction::emitARCCopyAssignWeak(QualType Ty, Address DstAddr,
                                            Address SrcAddr) {
  toolchain::Value *Object = EmitARCLoadWeakRetained(SrcAddr);
...
toolchain::Value *CodeGenFunction::EmitObjCAutoreleasePoolPush() {
  toolchain::Function *&fn = CGM.getObjCEntrypoints().objc_autoreleasePoolPush;
  if (!fn)
    fn = getARCIntrinsic(toolchain::Intrinsic::objc_autoreleasePoolPush, CGM);
...
void CodeGenFunction::EmitObjCAutoreleasePoolPop(toolchain::Value *value) {
  ...
  if (getInvokeDest()) {
    ...
    if (!fn) {
      toolchain::FunctionType *fnType =
        toolchain::FunctionType::get(Builder.getVoidTy(), Int8PtrTy, false);
    ...
    toolchain::FunctionCallee &fn = CGM.getObjCEntrypoints().objc_autoreleasePoolPop;
    if (!fn)
      fn = getARCIntrinsic(toolchain::Intrinsic::objc_autoreleasePoolPop, CGM);
...
///
toolchain::Value *CodeGenFunction::EmitObjCMRRAutoreleasePoolPush() {
  CGObjCRuntime &Runtime = CGM.getObjCRuntime();
  toolchain::Value *Receiver = Runtime.EmitNSAutoreleasePoolClassRef(*this);
...
toolchain::Value *CodeGenFunction::EmitObjCAlloc(toolchain::Value *value,
...
toolchain::Value *CodeGenFunction::EmitObjCAllocWithZone(toolchain::Value *value,
                                                    toolchain::Type *resultType) {
  return emitObjCValueOperation(*this, value, resultType,
...
toolchain::Value *CodeGenFunction::EmitObjCAllocInit(toolchain::Value *value,
                                                toolchain::Type *resultType) {
  return emitObjCValueOperation(*this, value, resultType,
...
/// Produce the code to do a primitive release.
/// [tmp drain];
void CodeGenFunction::EmitObjCMRRAutoreleasePoolPop(toolchain::Value *Arg) {
...
void CodeGenFunction::emitARCIntrinsicUse(CodeGenFunction &CGF, Address addr,
                                          QualType type) {
  toolchain::Value *value = CGF.Builder.CreateLoad(addr);
  CGF.EmitARCIntrinsicUse(value);
}

/// Autorelease the given object.
///   call i8* \@objc_autorelease(i8* %value)
toolchain::Value *CodeGenFunction::EmitObjCAutorelease(toolchain::Value *value,
                                                  toolchain::Type *returnType) {
...
/// Retain the given object, with normal retain semantics.
///   call i8* \@objc_retain(i8* %value)
toolchain::Value *CodeGenFunction::EmitObjCRetainNonBlock(toolchain::Value *value,
                                                     toolchain::Type *returnType) {
  return emitObjCValueOperation(
      *this, value, returnType,
...
/// Release the given object.
///   call void \@objc_release(i8* %value)
void CodeGenFunction::EmitObjCRelease(toolchain::Value *value,
                                      ARCPreciseLifetime_t precise) {
  if (isa<toolchain::ConstantPointerNull>(value)) return;
  ...
  // Call objc_release.
  toolchain::CallBase *call = EmitCallOrInvoke(fn, value);
...
namespace {
  struct CallObjCAutoreleasePoolObject final : EHScopeStack::Cleanup {
    toolchain::Value *Token;

    CallObjCAutoreleasePoolObject(toolchain::Value *token) : Token(token) {}
  ...
  struct CallObjCMRRAutoreleasePoolObject final : EHScopeStack::Cleanup {
    ...
    CallObjCMRRAutoreleasePoolObject(toolchain::Value *token) : Token(token) {}
...
void CodeGenFunction::EmitObjCAutoreleasePoolCleanup(toolchain::Value *Ptr) {
  if (CGM.getLangOpts().ObjCAutoRefCount)
...
static bool shouldRetainObjCLifetime(Qualifiers::ObjCLifetime lifetime) {
  ...
  toolchain_unreachable("impossible lifetime!");
}
...
static TryEmitResult tryEmitARCRetainLoadOfScalar(CodeGenFunction &CGF,
  ...
  if (e->isXValue() &&
    ...
    toolchain::Value *result = CGF.EmitLoadOfLValue(lv,
...
typedef toolchain::function_ref<toolchain::Value *(CodeGenFunction &CGF,
                                         toolchain::Value *value)>
  ValueTransform;
...
static toolchain::Value *emitARCOperationAfterCall(CodeGenFunction &CGF,
                                              toolchain::Value *value,
                                              ValueTransform doAfterCall,
                                              ValueTransform doFallback) {
  CGBuilderTy::InsertPoint ip = CGF.Builder.saveIP();
  auto *callBase = dyn_cast<toolchain::CallBase>(value);

  if (callBase && toolchain::objcarc::hasAttachedCallOpBundle(callBase)) {
    // Fall back if the call base has operand bundle "clang.arc.attachedcall".
    value = doFallback(CGF, value);
  } else if (toolchain::CallInst *call = dyn_cast<toolchain::CallInst>(value)) {
    // Place the retain immediately following the call.
    CGF.Builder.SetInsertPoint(call->getParent(),
                               ++toolchain::BasicBlock::iterator(call));
    value = doAfterCall(CGF, value);
  } else if (toolchain::InvokeInst *invoke = dyn_cast<toolchain::InvokeInst>(value)) {
    // Place the retain at the beginning of the normal destination block.
    toolchain::BasicBlock *BB = invoke->getNormalDest();
    CGF.Builder.SetInsertPoint(BB, BB->begin());
    value = doAfterCall(CGF, value);

  // Bitcasts can arise because of related-result returns.  Rewrite
  // the operand.
  } else if (toolchain::BitCastInst *bitcast = dyn_cast<toolchain::BitCastInst>(value)) {
    ...
    toolchain::Value *operand = bitcast->getOperand(0);
    operand = emitARCOperationAfterCall(CGF, operand, doAfterCall, doFallback);
    bitcast->setOperand(0, operand);
    value = bitcast;
  } else {
    auto *phi = dyn_cast<toolchain::PHINode>(value);
    if (phi && phi->getNumIncomingValues() == 2 &&
        isa<toolchain::ConstantPointerNull>(phi->getIncomingValue(1)) &&
        isa<toolchain::CallBase>(phi->getIncomingValue(0))) {
      // Handle phi instructions that are generated when it's necessary to check
      // whether the receiver of a message is null.
      toolchain::Value *inVal = phi->getIncomingValue(0);
...
static toolchain::Value *emitARCRetainCallResult(CodeGenFunction &CGF,
                                            const Expr *e) {
  toolchain::Value *value = CGF.EmitScalarExpr(e);
  return emitARCOperationAfterCall(CGF, value,
           [](CodeGenFunction &CGF, toolchain::Value *value) {
...
static toolchain::Value *emitARCUnsafeClaimCallResult(CodeGenFunction &CGF,
                                                 const Expr *e) {
  toolchain::Value *value = CGF.EmitScalarExpr(e);
  return emitARCOperationAfterCall(CGF, value,
           [](CodeGenFunction &CGF, toolchain::Value *value) {
...
toolchain::Value *CodeGenFunction::EmitARCReclaimReturnedObject(const Expr *E,
                                                      bool allowUnsafeClaim) {
  if (allowUnsafeClaim &&
      CGM.getLangOpts().ObjCRuntime.hasARCUnsafeClaimAutoreleasedReturnValue()) {
    return emitARCUnsafeClaimCallResult(*this, E);
  } else {
    toolchain::Value *value = emitARCRetainCallResult(*this, E);
...
public:
  ...
  //
  //   Result emitBitCast(Result result, toolchain::Type *resultType)
  //   toolchain::Value *getValueOfResult(Result result)
};
}
...
Result ARCExprEmitter<Impl,Result>::visitBinAssign(const BinaryOperator *e) {
  ...
  toolchain_unreachable("bad ObjC ownership qualifier");
}
...
struct ARCRetainExprEmitter :
  ...
  toolchain::Value *getValueOfResult(TryEmitResult result) {
    return result.getPointer();
  }

  TryEmitResult emitBitCast(TryEmitResult result, toolchain::Type *resultType) {
    toolchain::Value *value = result.getPointer();
  ...
  TryEmitResult visitConsumeObject(const Expr *e) {
    toolchain::Value *result = CGF.EmitScalarExpr(e);
    return TryEmitResult(result, true);
  }
  ...
  TryEmitResult visitExtendBlockObject(const Expr *e) {
    toolchain::Value *result; // will be a +0 value
  ...
  TryEmitResult visitReclaimReturnedObject(const Expr *e) {
    toolchain::Value *result = emitARCRetainCallResult(CGF, e);
    return TryEmitResult(result, true);
  }
...
/// retained objects.
toolchain::Value *CodeGenFunction::EmitARCRetainScalarExpr(const Expr *e) {
...
toolchain::Value *CodeGenFunction::EmitARCExtendBlockObject(const Expr *e) {
  toolchain::Value *result;
  bool doRetain;
...
toolchain::Value *CodeGenFunction::EmitObjCThrowOperand(const Expr *expr) {
...
struct ARCUnsafeUnretainedExprEmitter :
  public ARCExprEmitter<ARCUnsafeUnretainedExprEmitter, toolchain::Value*> {

  ARCUnsafeUnretainedExprEmitter(CodeGenFunction &CGF) : ARCExprEmitter(CGF) {}

  toolchain::Value *getValueOfResult(toolchain::Value *value) {
    return value;
  }

  toolchain::Value *emitBitCast(toolchain::Value *value, toolchain::Type *resultType) {
    return CGF.Builder.CreateBitCast(value, resultType);
  }

  toolchain::Value *visitLValueToRValue(const Expr *e) {
    return CGF.EmitScalarExpr(e);
  }

  /// For consumptions, just emit the subexpression and perform the
  /// consumption like normal.
  toolchain::Value *visitConsumeObject(const Expr *e) {
    toolchain::Value *value = CGF.EmitScalarExpr(e);
  ...
  toolchain::Value *visitExtendBlockObject(const Expr *e) {
    return CGF.EmitARCExtendBlockObject(e);
  }

  /// For reclaims, perform an unsafeClaim if that's enabled.
  toolchain::Value *visitReclaimReturnedObject(const Expr *e) {
    return CGF.EmitARCReclaimReturnedObject(e, /*unsafe*/ true);
  }

  /// When we have an undecorated call, just emit it without adding
  /// the unsafeClaim.
  toolchain::Value *visitCall(const Expr *e) {
    return CGF.EmitScalarExpr(e);
  }

  /// Just do normal scalar emission in the default case.
  toolchain::Value *visitExpr(const Expr *e) {
    return CGF.EmitScalarExpr(e);
  }
};
}

static toolchain::Value *emitARCUnsafeUnretainedScalarExpr(CodeGenFunction &CGF,
                                                      const Expr *e) {
...
toolchain::Value *CodeGenFunction::EmitARCUnsafeUnretainedScalarExpr(const Expr *e) {
...
CodeGenFunction::EmitARCStoreUnsafeUnretained(const BinaryOperator *e,
  ...
  return std::pair<LValue,toolchain::Value*>(std::move(lvalue), value);
}
...
CodeGenFunction::EmitARCStoreStrong(const BinaryOperator *e,
  ...
  if (hasImmediateRetain) {
    toolchain::Value *oldValue = EmitLoadOfScalar(lvalue, SourceLocation());
    EmitStoreOfScalar(value, lvalue);
  ...
  return std::pair<LValue,toolchain::Value*>(lvalue, value);
}

std::pair<LValue,toolchain::Value*>
CodeGenFunction::EmitARCStoreAutoreleasing(const BinaryOperator *e) {
  toolchain::Value *value = EmitARCRetainAutoreleaseScalarExpr(e->getRHS());
  LValue lvalue = EmitLValue(e->getLHS());
...
void CodeGenFunction::EmitObjCAutoreleasePoolStmt(
  ...
  RunCleanupsScope Scope(*this);
  if (CGM.getLangOpts().ObjCRuntime.hasNativeARC()) {
    toolchain::Value *token = EmitObjCAutoreleasePoolPush();
    EHStack.pushCleanup<CallObjCAutoreleasePoolObject>(NormalCleanup, token);
  } else {
    toolchain::Value *token = EmitObjCMRRAutoreleasePoolPush();
...
void CodeGenFunction::EmitExtendGCLifetime(toolchain::Value *object) {
  // We just use an inline assembly.
  toolchain::FunctionType *extenderType
    = toolchain::FunctionType::get(VoidTy, VoidPtrTy, RequiredArgs::All);
  toolchain::InlineAsm *extender = toolchain::InlineAsm::get(extenderType,
                                                   /* assembly */ "",
                                                   /* constraints */ "r",
                                                   /* side effects */ true);
...
CodeGenFunction::GenerateObjCAtomicSetterCopyHelperFunction(
  ...
  if (Ty.isNonTrivialToPrimitiveCopy() == QualType::PCK_Struct) {
    ...
    toolchain::Constant *Fn = getNonTrivialCStructMoveAssignmentOperator(
  ...
  if (!Ty->isRecordType())
    return nullptr;
  toolchain::Constant *HelperFn = nullptr;
  ...
  toolchain::FunctionType *LTy = CGM.getTypes().GetFunctionType(FI);

  toolchain::Function *Fn =
    toolchain::Function::Create(LTy, toolchain::GlobalValue::InternalLinkage,
...
toolchain::Constant *CodeGenFunction::GenerateObjCAtomicGetterCopyHelperFunction(
    const ObjCPropertyImplDecl *PID) {
  const ObjCPropertyDecl *PD = PID->getPropertyDecl();
  if ((!(PD->getPropertyAttributes() & ObjCPropertyAttribute::kind_atomic)))
    return nullptr;
  ...
  if (Ty.isNonTrivialToPrimitiveCopy() == QualType::PCK_Struct) {
    CharUnits Alignment = C.getTypeAlignInChars(Ty);
    toolchain::Constant *Fn = getNonTrivialCStructCopyConstructor(
        CGM, Alignment, Alignment, Ty.isVolatileQualified(), Ty);
    return Fn;
  }
  ...
  toolchain::Function *Fn = toolchain::Function::Create(
      LTy, toolchain::GlobalValue::InternalLinkage, "__copy_helper_atomic_property_",
      &CGM.getModule());
...
toolchain::Value *
CodeGenFunction::EmitBlockCopyAndAutorelease(toolchain::Value *Block, QualType Ty) {
...
static unsigned getBaseMachOPlatformID(const toolchain::Triple &TT) {
  ...
  case toolchain::Triple::DriverKit:
    return toolchain::MachO::PLATFORM_DRIVERKIT;
  default:
    return toolchain::MachO::PLATFORM_UNKNOWN;
  }
}

static toolchain::Value *emitIsPlatformVersionAtLeast(CodeGenFunction &CGF,
  ...
  toolchain::SmallVector<toolchain::Value *, 8> Args;

  auto EmitArgs = [&](const VersionTuple &Version, const toolchain::Triple &TT) {
    std::optional<unsigned> Min = Version.getMinor(),
                            SMin = Version.getSubminor();
    Args.push_back(
        toolchain::ConstantInt::get(CGM.Int32Ty, getBaseMachOPlatformID(TT)));
    Args.push_back(toolchain::ConstantInt::get(CGM.Int32Ty, Version.getMajor()));
    Args.push_back(toolchain::ConstantInt::get(CGM.Int32Ty, Min.value_or(0)));
    Args.push_back(toolchain::ConstantInt::get(CGM.Int32Ty, SMin.value_or(0)));
  };
  ...
  return CGF.Builder.CreateICmpNE(Check,
                                  toolchain::Constant::getNullValue(CGM.Int32Ty));
}
...
CodeGenFunction::EmitBuiltinAvailable(const VersionTuple &Version) {
  ...
  if (!CGM.IsOSVersionAtLeastFn) {
    toolchain::FunctionType *FTy =
        toolchain::FunctionType::get(Int32Ty, {Int32Ty, Int32Ty, Int32Ty}, false);
  ...
  std::optional<unsigned> Min = Version.getMinor(),
                          SMin = Version.getSubminor();
  toolchain::Value *Args[] = {
      toolchain::ConstantInt::get(CGM.Int32Ty, Version.getMajor()),
      toolchain::ConstantInt::get(CGM.Int32Ty, Min.value_or(0)),
      toolchain::ConstantInt::get(CGM.Int32Ty, SMin.value_or(0))};

  toolchain::Value *CallRes =
      EmitNounwindRuntimeCall(CGM.IsOSVersionAtLeastFn, Args);

  return Builder.CreateICmpNE(CallRes, toolchain::Constant::getNullValue(Int32Ty));
}

static bool isFoundationNeededForDarwinAvailabilityCheck(
    const toolchain::Triple &TT, const VersionTuple &TargetVersion) {
...
void CodeGenModule::emitAtAvailableLinkGuard() {
  ...
  toolchain::Metadata *Args[2] = {toolchain::MDString::get(Context, "-framework"),
                             toolchain::MDString::get(Context, "CoreFoundation")};
  LinkerOptionsMetadata.push_back(toolchain::MDNode::get(Context, Args));
  // Emit a reference to a symbol from CoreFoundation to ensure that
  // CoreFoundation is linked into the final binary.
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(Int32Ty, {VoidPtrTy}, false);
  toolchain::FunctionCallee CFFunc =
      CreateRuntimeFunction(FTy, "CFBundleGetVersionNumber");

  toolchain::FunctionType *CheckFTy = toolchain::FunctionType::get(VoidTy, {}, false);
  toolchain::FunctionCallee CFLinkCheckFuncRef = CreateRuntimeFunction(
      CheckFTy, "__clang_at_available_requires_core_foundation_framework",
      toolchain::AttributeList(), /*Local=*/true);
  toolchain::Function *CFLinkCheckFunc =
      cast<toolchain::Function>(CFLinkCheckFuncRef.getCallee()->stripPointerCasts());
  if (CFLinkCheckFunc->empty()) {
    CFLinkCheckFunc->setLinkage(toolchain::GlobalValue::LinkOnceAnyLinkage);
    CFLinkCheckFunc->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    CodeGenFunction CGF(*this);
    CGF.Builder.SetInsertPoint(CGF.createBasicBlock("", CFLinkCheckFunc));
    CGF.EmitNounwindRuntimeCall(CFFunc,
                                toolchain::Constant::getNullValue(VoidPtrTy));
