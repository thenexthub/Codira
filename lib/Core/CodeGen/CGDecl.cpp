...
static_assert(language::Core::Sema::MaximumAlignment <= toolchain::Value::MaximumAlignment,
...
void CodeGenFunction::EmitDecl(const Decl &D, bool EvaluateConditionDecl) {
  ...
  case Decl::HLSLRootSignature:
    toolchain_unreachable("Declaration should not be in declstmts!");
...
static std::string getStaticDeclName(CodeGenModule &CGM, const VarDecl &D) {
  ...
  if (const auto *FD = dyn_cast<FunctionDecl>(DC))
    ...
    toolchain_unreachable("Unknown context for static var decl");

  ContextName += "." + D.getNameAsString();
  return ContextName;
}

toolchain::Constant *CodeGenModule::getOrCreateStaticVarDecl(
    const VarDecl &D, toolchain::GlobalValue::LinkageTypes Linkage) {
  // In general, we don't always emit static var decls once before we reference
  // them. It is possible to reference them before emitting the function that
  // contains them, and it is possible to emit the containing function multiple
  // times.
  if (toolchain::Constant *ExistingGV = StaticLocalDeclMap[&D])
    return ExistingGV;
  ...
  toolchain::Type *LTy = getTypes().ConvertTypeForMem(Ty);
  LangAS AS = GetGlobalVarAddressSpace(&D);
  unsigned TargetAS = getContext().getTargetAddressSpace(AS);

  // OpenCL variables in local address space and CUDA shared
  // variables cannot have an initializer.
  toolchain::Constant *Init = nullptr;
  if (Ty.getAddressSpace() == LangAS::opencl_local ||
      D.hasAttr<CUDASharedAttr>() || D.hasAttr<LoaderUninitializedAttr>())
    Init = toolchain::UndefValue::get(LTy);
  else
    Init = EmitNullConstant(Ty);

  toolchain::GlobalVariable *GV = new toolchain::GlobalVariable(
      getModule(), LTy, Ty.isConstant(getContext()), Linkage, Init, Name,
      nullptr, toolchain::GlobalVariable::NotThreadLocal, TargetAS);
...
CodeGenFunction::AddInitializerToStaticVarDecl(const VarDecl &D,
  ...
  toolchain::Constant *Init = emitter.tryEmitForInitializer(D);
...
void CodeGenFunction::EmitStaticVarDecl(const VarDecl &D,
  ...
  toolchain::Constant *addr = CGM.getOrCreateStaticVarDecl(D, Linkage);
  CharUnits alignment = getContext().getDeclAlign(&D);

  // Store into LocalDeclMap before generating initializer to handle
  // circular references.
  toolchain::Type *elemTy = ConvertTypeForMem(D.getType());
  ...
  toolchain::Type *expectedType = addr->getType();

  toolchain::GlobalVariable *var =
    cast<toolchain::GlobalVariable>(addr->stripPointerCasts());
  ...
  toolchain::Constant *castedAddr =
    toolchain::ConstantExpr::getPointerBitCastOrAddrSpaceCast(var, expectedType);
...
namespace {
  ...
  struct DestroyNRVOVariable : EHScopeStack::Cleanup {
    DestroyNRVOVariable(Address addr, QualType type, toolchain::Value *NRVOFlag)
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      toolchain::BasicBlock *SkipDtorBB = nullptr;
      if (NRVO) {
        // If we exited via NRVO, we skip the destructor call.
        toolchain::BasicBlock *RunDtorBB = CGF.createBasicBlock("nrvo.unused");
        SkipDtorBB = CGF.createBasicBlock("nrvo.skipdtor");
        toolchain::Value *DidNRVO =
  ...
  struct DestroyNRVOVariableCXX final
    ...
    DestroyNRVOVariableCXX(Address addr, QualType type,
                           const CXXDestructorDecl *Dtor, toolchain::Value *NRVOFlag)
  ...
  struct DestroyNRVOVariableC final
    ...
    DestroyNRVOVariableC(Address addr, toolchain::Value *NRVOFlag, QualType Ty)
  ...
  struct CallStackRestore final : EHScopeStack::Cleanup {
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      toolchain::Value *V = CGF.Builder.CreateLoad(Stack);
  ...
  struct KmpcAllocFree final : EHScopeStack::Cleanup {
    ...
    KmpcAllocFree(const std::pair<toolchain::Value *, toolchain::Value *> &AddrSizePair)
  ...
  struct ExtendGCLifetime final : EHScopeStack::Cleanup {
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      toolchain::Value *value = CGF.EmitLoadOfScalar(CGF.EmitDeclRefLValue(&DRE),
                                                SourceLocation());
  ...
  struct CallCleanupFunction final : EHScopeStack::Cleanup {
    toolchain::Constant *CleanupFn;
    const CGFunctionInfo &FnInfo;
    const VarDecl &Var;
    const CleanupAttr *Attribute;

    CallCleanupFunction(toolchain::Constant *CleanupFn, const CGFunctionInfo *Info,
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      // or something.
      toolchain::Value *Addr = CGF.EmitDeclRefLValue(&DRE).getPointer(CGF);
...
void CodeGenFunction::EmitNullabilityCheck(LValue LHS, toolchain::Value *RHS,
                                           SourceLocation Loc) {
  ...
  toolchain::Value *IsNotNull = Builder.CreateIsNotNull(RHS);
  toolchain::Constant *StaticData[] = {
      EmitCheckSourceLocation(Loc), EmitCheckTypeDescriptor(LHS.getType()),
      toolchain::ConstantInt::get(Int8Ty, 0), // The LogAlignment info is unused.
      toolchain::ConstantInt::get(Int8Ty, TCK_NonnullAssign)};
...
void CodeGenFunction::EmitScalarInit(const Expr *init, const ValueDecl *D,
  ...
  if (accessedByInit) {
    ...
    auto ty = cast<toolchain::PointerType>(tempLV.getAddress().getElementType());
    toolchain::Value *zero = CGM.getNullPointer(ty, tempLV.getType());
  ...
  if (accessedByInit && lifetime == Qualifiers::OCL_Strong) {
    toolchain::Value *oldValue = EmitLoadOfScalar(lvalue, init->getExprLoc());
...
static bool canEmitInitWithFewStoresAfterBZero(toolchain::Constant *Init,
                                               unsigned &NumStores) {
  // Zero and Undef never requires any extra stores.
  if (isa<toolchain::ConstantAggregateZero>(Init) ||
      isa<toolchain::ConstantPointerNull>(Init) ||
      isa<toolchain::UndefValue>(Init))
    return true;
  if (isa<toolchain::ConstantInt>(Init) || isa<toolchain::ConstantFP>(Init) ||
      isa<toolchain::ConstantVector>(Init) || isa<toolchain::BlockAddress>(Init) ||
      isa<toolchain::ConstantExpr>(Init))
    return Init->isNullValue() || NumStores--;

  // See if we can emit each element.
  if (isa<toolchain::ConstantArray>(Init) || isa<toolchain::ConstantStruct>(Init)) {
    for (unsigned i = 0, e = Init->getNumOperands(); i != e; ++i) {
      toolchain::Constant *Elt = cast<toolchain::Constant>(Init->getOperand(i));
  ...
  if (toolchain::ConstantDataSequential *CDS =
        dyn_cast<toolchain::ConstantDataSequential>(Init)) {
    for (unsigned i = 0, e = CDS->getNumElements(); i != e; ++i) {
      toolchain::Constant *Elt = CDS->getElementAsConstant(i);
      if (!canEmitInitWithFewStoresAfterBZero(Elt, NumStores))
        return false;
    }
    return true;
...
void CodeGenFunction::emitStoresForInitAfterBZero(toolchain::Constant *Init,
                                                  Address Loc, bool isVolatile,
                                                  bool IsAutoInit) {
  assert(!Init->isNullValue() && !isa<toolchain::UndefValue>(Init) &&
  ...
  if (toolchain::ConstantDataSequential *CDS =
    ...
    for (unsigned i = 0, e = CDS->getNumElements(); i != e; ++i) {
      ...
      if (!Elt->isNullValue() && !isa<toolchain::UndefValue>(Elt))
  ...
  assert((isa<toolchain::ConstantStruct>(Init) || isa<toolchain::ConstantArray>(Init)) &&
         "Unknown value type!");
...
static bool shouldUseBZeroPlusStoresToInitialize(toolchain::Constant *Init,
                                                 uint64_t GlobalSize) {
  // If a global is all zeros, always use a bzero.
  if (isa<toolchain::ConstantAggregateZero>(Init)) return true;
...
static toolchain::Value *shouldUseMemSetToInitialize(toolchain::Constant *Init,
                                                uint64_t GlobalSize,
                                                const toolchain::DataLayout &DL) {
  uint64_t SizeLimit = 32;
  if (GlobalSize <= SizeLimit)
    return nullptr;
  return toolchain::isBytewiseValue(Init, DL);
}
...
static toolchain::Constant *patternOrZeroFor(CodeGenModule &CGM, IsPattern isPattern,
                                        toolchain::Type *Ty) {
...
static toolchain::Constant *constWithPadding(CodeGenModule &CGM, IsPattern isPattern,
                                        toolchain::Constant *constant);

/// Helper function for constWithPadding() to deal with padding in structures.
static toolchain::Constant *constStructWithPadding(CodeGenModule &CGM,
                                              IsPattern isPattern,
                                              toolchain::StructType *STy,
                                              toolchain::Constant *constant) {
  const toolchain::DataLayout &DL = CGM.getDataLayout();
  const toolchain::StructLayout *Layout = DL.getStructLayout(STy);
  toolchain::Type *Int8Ty = toolchain::IntegerType::getInt8Ty(CGM.getLLVMContext());
  ...
  for (unsigned i = 0, e = STy->getNumElements(); i != e; i++) {
    ...
    if (SizeSoFar < CurOff) {
      assert(!STy->isPacked());
      auto *PadTy = toolchain::ArrayType::get(Int8Ty, CurOff - SizeSoFar);
      Values.push_back(patternOrZeroFor(CGM, isPattern, PadTy));
    }
    toolchain::Constant *CurOp;
    if (constant->isZeroValue())
      CurOp = toolchain::Constant::getNullValue(STy->getElementType(i));
    else
      CurOp = cast<toolchain::Constant>(constant->getAggregateElement(i));
  ...
  if (SizeSoFar < TotalSize) {
    auto *PadTy = toolchain::ArrayType::get(Int8Ty, TotalSize - SizeSoFar);
    Values.push_back(patternOrZeroFor(CGM, isPattern, PadTy));
  }
  if (NestedIntact && Values.size() == STy->getNumElements())
    return constant;
  return toolchain::ConstantStruct::getAnon(Values, STy->isPacked());
}
...
static toolchain::Constant *constWithPadding(CodeGenModule &CGM, IsPattern isPattern,
                                        toolchain::Constant *constant) {
  toolchain::Type *OrigTy = constant->getType();
  if (const auto STy = dyn_cast<toolchain::StructType>(OrigTy))
    return constStructWithPadding(CGM, isPattern, STy, constant);
  if (auto *ArrayTy = dyn_cast<toolchain::ArrayType>(OrigTy)) {
    toolchain::SmallVector<toolchain::Constant *, 8> Values;
    uint64_t Size = ArrayTy->getNumElements();
    if (!Size)
      return constant;
    toolchain::Type *ElemTy = ArrayTy->getElementType();
    bool ZeroInitializer = constant->isNullValue();
    toolchain::Constant *OpValue, *PaddedOp;
    if (ZeroInitializer) {
      OpValue = toolchain::Constant::getNullValue(ElemTy);
    ...
    if (NewElemTy == ElemTy)
      return constant;
    auto *NewArrayTy = toolchain::ArrayType::get(NewElemTy, Size);
    return toolchain::ConstantArray::get(NewArrayTy, Values);
  }
...
Address CodeGenModule::createUnnamedGlobalFrom(const VarDecl &D,
  ...
  auto FunctionName = [&](const DeclContext *DC) -> std::string {
    if (const auto *FD = dyn_cast<FunctionDecl>(DC)) {
      ...
      toolchain_unreachable("expected a function or method");
    }
  };

  // Form a simple per-variable cache of these values in case we find we
  // want to reuse them.
  toolchain::GlobalVariable *&CacheEntry = InitializerConstants[&D];
  if (!CacheEntry || CacheEntry->getInitializer() != Constant) {
    auto *Ty = Constant->getType();
    bool isConstant = true;
    toolchain::GlobalVariable *InsertBefore = nullptr;
    unsigned AS =
    ...
    if (D.hasGlobalStorage())
      ...
      toolchain_unreachable("local variable has no parent function or method");
    toolchain::GlobalVariable *GV = new toolchain::GlobalVariable(
        getModule(), Ty, isConstant, toolchain::GlobalValue::PrivateLinkage,
        Constant, Name, InsertBefore, toolchain::GlobalValue::NotThreadLocal, AS);
...
void CodeGenFunction::emitStoresForConstant(const VarDecl &D, Address Loc,
  ...
  auto *SizeVal = toolchain::ConstantInt::get(CGM.IntPtrTy, ConstantSize);

  // If the initializer is all or mostly the same, codegen with bzero / memset
  // then do a few stores afterward.
  if (shouldUseBZeroPlusStoresToInitialize(constant, ConstantSize)) {
    auto *I = Builder.CreateMemSet(Loc, toolchain::ConstantInt::get(CGM.Int8Ty, 0),
                                   SizeVal, isVolatile);
    addInstToCurrentSourceAtom(I, nullptr);
    ...
    bool valueAlreadyCorrect =
        constant->isNullValue() || isa<toolchain::UndefValue>(constant);
  ...
  toolchain::Value *Pattern =
      shouldUseMemSetToInitialize(constant, ConstantSize, CGM.getDataLayout());
  if (Pattern) {
    uint64_t Value = 0x00;
    if (!isa<toolchain::UndefValue>(Pattern)) {
      const toolchain::APInt &AP = cast<toolchain::ConstantInt>(Pattern)->getValue();
      assert(AP.getBitWidth() <= 8);
      Value = AP.getLimitedValue();
    }
    auto *I = Builder.CreateMemSet(
        Loc, toolchain::ConstantInt::get(CGM.Int8Ty, Value), SizeVal, isVolatile);
    addInstToCurrentSourceAtom(I, nullptr);
    if (IsAutoInit)
      I->addAnnotationMetadata("auto-init");
    return;
  ...
  if (shouldSplitConstantStore(CGM, ConstantSize)) {
    if (auto *STy = dyn_cast<toolchain::StructType>(Ty)) {
    ...
    } else if (auto *ATy = dyn_cast<toolchain::ArrayType>(Ty)) {
...
void CodeGenFunction::emitStoresForZeroInit(const VarDecl &D, Address Loc,
                                            bool isVolatile) {
  toolchain::Type *ElTy = Loc.getElementType();
  toolchain::Constant *constant =
      constWithPadding(CGM, IsPattern::No, toolchain::Constant::getNullValue(ElTy));
...
void CodeGenFunction::emitStoresForPatternInit(const VarDecl &D, Address Loc,
                                               bool isVolatile) {
  toolchain::Type *ElTy = Loc.getElementType();
  toolchain::Constant *constant = constWithPadding(
      CGM, IsPattern::Yes, initializationPatternFor(CGM, ElTy));
  assert(!isa<toolchain::UndefValue>(constant));
  emitStoresForConstant(D, Loc, isVolatile, constant,
                        /*IsAutoInit=*/true);
}

static bool containsUndef(toolchain::Constant *constant) {
  auto *Ty = constant->getType();
  if (isa<toolchain::UndefValue>(constant))
    return true;
  if (Ty->isStructTy() || Ty->isArrayTy() || Ty->isVectorTy())
    for (toolchain::Use &Op : constant->operands())
      if (containsUndef(cast<toolchain::Constant>(Op)))
        return true;
  return false;
}

static toolchain::Constant *replaceUndef(CodeGenModule &CGM, IsPattern isPattern,
                                    toolchain::Constant *constant) {
  auto *Ty = constant->getType();
  if (isa<toolchain::UndefValue>(constant))
  ...
  if (!containsUndef(constant))
    return constant;
  toolchain::SmallVector<toolchain::Constant *, 8> Values(constant->getNumOperands());
  for (unsigned Op = 0, NumOp = constant->getNumOperands(); Op != NumOp; ++Op) {
    auto *OpValue = cast<toolchain::Constant>(constant->getOperand(Op));
    Values[Op] = replaceUndef(CGM, isPattern, OpValue);
  }
  if (Ty->isStructTy())
    return toolchain::ConstantStruct::get(cast<toolchain::StructType>(Ty), Values);
  if (Ty->isArrayTy())
    return toolchain::ConstantArray::get(cast<toolchain::ArrayType>(Ty), Values);
  assert(Ty->isVectorTy());
  return toolchain::ConstantVector::get(Values);
}
...
bool CodeGenFunction::EmitLifetimeStart(toolchain::Value *Addr) {
  ...
  toolchain::CallInst *C = Builder.CreateCall(CGM.getLLVMLifetimeStartFn(), {Addr});
  C->setDoesNotThrow();
  return true;
}

void CodeGenFunction::EmitLifetimeEnd(toolchain::Value *Addr) {
  if (!ShouldEmitLifetimeMarkers)
    return;
  ...
  assert(Addr->getType()->getPointerAddressSpace() ==
             CGM.getDataLayout().getAllocaAddrSpace() &&
         "Pointer should be in alloca address space");
  toolchain::CallInst *C = Builder.CreateCall(CGM.getLLVMLifetimeEndFn(), {Addr});
  C->setDoesNotThrow();
}

void CodeGenFunction::EmitFakeUse(Address Addr) {
  auto NL = ApplyDebugLocation::CreateEmpty(*this);
  toolchain::Value *V = Builder.CreateLoad(Addr, "fake.use");
  toolchain::CallInst *C = Builder.CreateCall(CGM.getLLVMFakeUseFn(), {V});
  C->setDoesNotThrow();
  C->setTailCallKind(toolchain::CallInst::TCK_NoTail);
}
...
void CodeGenFunction::EmitAndRegisterVariableArrayDimensions(
  ...
  while (getContext().getAsVariableArrayType(Type1D)) {
    ...
    if (auto *C = dyn_cast<toolchain::ConstantInt>(VlaSize.NumElts))
  ...
  for (auto &VlaSize : Dimensions) {
    ...
    if (auto *C = dyn_cast<toolchain::ConstantInt>(VlaSize.NumElts))
      MD = toolchain::ConstantAsMetadata::get(C);
    else {
...
CodeGenFunction::EmitAutoVarAlloca(const VarDecl &D) {
  ...
  if (getLangOpts().OpenMP && OpenMPLocalAddr.isValid()) {
    ...
    if (D.getInit() && (Ty->isArrayType() || Ty->isRecordType()) &&
      ...
      if ((!getLangOpts().OpenCL ||
        ...
        EmitStaticVarDecl(D, toolchain::GlobalValue::InternalLinkage);
    ...
    if (NRVO) {
      ...
      if (const RecordType *RecordTy = Ty->getAs<RecordType>()) {
        ...
        if ((CXXRD && !CXXRD->hasTrivialDestructor()) ||
          ...
          // applied.
          toolchain::Value *Zero = Builder.getFalse();
          RawAddress NRVOFlag =
    ...
    if (!VarAllocated) {
      if (!DidCallStackSave) {
        ...
        toolchain::Value *V = Builder.CreateStackSave();
      ...
      toolchain::Type *toolchainTy = ConvertTypeForMem(VlaSize.Type);

      // Allocate memory for the array.
      address = CreateTempAlloca(toolchainTy, alignment, "vla", VlaSize.NumElts,
                                 &AllocaAddr);
    }
  ...
  // Make sure we call @toolchain.lifetime.end.
...
void CodeGenFunction::emitZeroOrPatternForAutoVarInit(QualType type,
  ...
  if (!Size.isZero()) {
    ...
    case LangOptions::TrivialAutoVarInitKind::Uninitialized:
      toolchain_unreachable("Uninitialized handled by caller");
  ...
  case LangOptions::TrivialAutoVarInitKind::Zero: {
    ...
    auto *I = Builder.CreateMemSet(Loc, toolchain::ConstantInt::get(Int8Ty, 0),
                                   SizeVal, isVolatile);
    I->addAnnotationMetadata("auto-init");
    break;
  }
  ...
  case LangOptions::TrivialAutoVarInitKind::Pattern: {
    ...
    toolchain::BasicBlock *SetupBB = createBasicBlock("vla-setup.loop");
    toolchain::BasicBlock *LoopBB = createBasicBlock("vla-init.loop");
    toolchain::BasicBlock *ContBB = createBasicBlock("vla-init.cont");
    toolchain::Value *IsZeroSizedVLA = Builder.CreateICmpEQ(
        SizeVal, toolchain::ConstantInt::get(SizeVal->getType(), 0),
        "vla.iszerosized");
    ...
    toolchain::Value *BaseSizeInChars =
        toolchain::ConstantInt::get(IntPtrTy, EltSize.getQuantity());
    ...
    toolchain::Value *End = Builder.CreateInBoundsGEP(Begin.getElementType(),
                                                 ...
                                                 SizeVal, "vla.end");
    toolchain::BasicBlock *OriginBB = Builder.GetInsertBlock();
    EmitBlock(LoopBB);
...
void CodeGenFunction::EmitAutoVarInit(const AutoVarEmission &emission) {
  ...
  toolchain::Constant *constant = nullptr;
  if (emission.IsConstantAggregate ||
    ...
    if (constant && type->isBitIntType() &&
      ...
      constant = toolchain::ConstantFoldLoadFromConst(
          constant, LoadType, toolchain::APInt::getZero(32), CGM.getDataLayout());
    }
  }
...
void CodeGenFunction::emitAutoVarTypeCleanup(
  ...
  case QualType::DK_none:
    toolchain_unreachable("no cleanup for trivially-destructible variable");
...
void CodeGenFunction::EmitAutoVarCleanups(const AutoVarEmission &emission) {
  ...
  if (const CleanupAttr *CA = D.getAttr<CleanupAttr>()) {
    ...
    toolchain::Constant *F = CGM.GetAddrOfFunction(FD);
...
CodeGenFunction::getDestroyer(QualType::DestructionKind kind) {
  switch (kind) {
  case QualType::DK_none: toolchain_unreachable("no destroyer for trivial dtor");
  case QualType::DK_cxx_destructor:
  ...
  toolchain_unreachable("Unknown DestructionKind");
}
...
void CodeGenFunction::pushKmpcAllocFree(
    CleanupKind Kind, std::pair<toolchain::Value *, toolchain::Value *> AddrSizePair) {
...
void CodeGenFunction::emitDestroy(Address addr, QualType type,
  ...
  toolchain::Value *length = emitArrayLength(arrayType, type, addr);
  ...
  if (toolchain::ConstantInt *constLength = dyn_cast<toolchain::ConstantInt>(length)) {
    // ...and if it's constant zero, we can just skip the entire thing.
    if (constLength->isZero()) return;
    checkZeroLength = false;
  }

  toolchain::Value *begin = addr.emitRawPointer(*this);
  toolchain::Value *end =
...
///   element throws
void CodeGenFunction::emitArrayDestroy(toolchain::Value *begin,
                                       toolchain::Value *end,
  ...
  toolchain::BasicBlock *bodyBB = createBasicBlock("arraydestroy.body");
  toolchain::BasicBlock *doneBB = createBasicBlock("arraydestroy.done");

  if (checkZeroLength) {
    toolchain::Value *isEmpty = Builder.CreateICmpEQ(begin, end,
  ...
  EmitBlock(bodyBB);
  toolchain::PHINode *elementPast =
    Builder.CreatePHI(begin->getType(), 2, "arraydestroy.elementPast");
  elementPast->addIncoming(end, entryBB);

  // Shift the address back by one element.
  toolchain::Value *negativeOne = toolchain::ConstantInt::get(SizeTy, -1, true);
  toolchain::Type *toolchainElementType = ConvertTypeForMem(elementType);
  toolchain::Value *element = Builder.CreateInBoundsGEP(
      toolchainElementType, elementPast, negativeOne, "arraydestroy.element");
  ...
  destroyer(*this, Address(element, toolchainElementType, elementAlign),
            elementType);
  ...
  toolchain::Value *done = Builder.CreateICmpEQ(element, begin, "arraydestroy.done");
...
static void emitPartialArrayDestroy(CodeGenFunction &CGF,
                                    toolchain::Value *begin, toolchain::Value *end,
  ...
  if (arrayDepth) {
    toolchain::Value *zero = toolchain::ConstantInt::get(CGF.SizeTy, 0);

    SmallVector<toolchain::Value*,4> gepIndices(arrayDepth+1, zero);
...
namespace {
  ...
  class RegularPartialArrayDestroy final : public EHScopeStack::Cleanup {
    toolchain::Value *ArrayBegin;
    toolchain::Value *ArrayEnd;
    QualType ElementType;
    CodeGenFunction::Destroyer *Destroyer;
    CharUnits ElementAlign;
  public:
    RegularPartialArrayDestroy(toolchain::Value *arrayBegin, toolchain::Value *arrayEnd,
  ...
  class IrregularPartialArrayDestroy final : public EHScopeStack::Cleanup {
    ...
    CharUnits ElementAlign;
  public:
    IrregularPartialArrayDestroy(toolchain::Value *arrayBegin,
                                 Address arrayEndPointer,
                                 QualType elementType,
                                 CharUnits elementAlign,
                                 CodeGenFunction::Destroyer *destroyer)
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      toolchain::Value *arrayEnd = CGF.Builder.CreateLoad(ArrayEndPointer);
      emitPartialArrayDestroy(CGF, ArrayBegin, arrayEnd,
                              ElementType, ElementAlign, Destroyer);
    }
  };
...
void CodeGenFunction::pushIrregularPartialArrayCleanup(toolchain::Value *arrayBegin,
                                                       Address arrayEndPointer,
                                                       QualType elementType,
                                                       CharUnits elementAlign,
                                                       Destroyer *destroyer) {
...
/// \param elementType - the immediate element type of the array;
///   possibly still an array type
void CodeGenFunction::pushRegularPartialArrayCleanup(toolchain::Value *arrayBegin,
                                                     toolchain::Value *arrayEnd,
                                                     QualType elementType,
                                                     CharUnits elementAlign,
                                                     Destroyer *destroyer) {
...
/// Lazily declare the @toolchain.lifetime.start intrinsic.
toolchain::Function *CodeGenModule::getLLVMLifetimeStartFn() {
  if (LifetimeStartFn)
    return LifetimeStartFn;
  LifetimeStartFn = toolchain::Intrinsic::getOrInsertDeclaration(
      &getModule(), toolchain::Intrinsic::lifetime_start, AllocaInt8PtrTy);
  return LifetimeStartFn;
}

/// Lazily declare the @toolchain.lifetime.end intrinsic.
toolchain::Function *CodeGenModule::getLLVMLifetimeEndFn() {
  if (LifetimeEndFn)
    return LifetimeEndFn;
  LifetimeEndFn = toolchain::Intrinsic::getOrInsertDeclaration(
      &getModule(), toolchain::Intrinsic::lifetime_end, AllocaInt8PtrTy);
  return LifetimeEndFn;
}

/// Lazily declare the @toolchain.fake.use intrinsic.
toolchain::Function *CodeGenModule::getLLVMFakeUseFn() {
  if (FakeUseFn)
    return FakeUseFn;
  FakeUseFn = toolchain::Intrinsic::getOrInsertDeclaration(
      &getModule(), toolchain::Intrinsic::fake_use);
  return FakeUseFn;
}
...
namespace {
  ...
  struct ConsumeARCParameter final : EHScopeStack::Cleanup {
    ConsumeARCParameter(toolchain::Value *param,
                        ARCPreciseLifetime_t precise)
      : Param(param), Precise(precise) {}

    toolchain::Value *Param;
    ARCPreciseLifetime_t Precise;
...
void CodeGenFunction::EmitParmDecl(const VarDecl &D, ParamValue Arg,
  ...
  if (!isa<toolchain::GlobalValue>(Arg.getAnyValue()))
  ...
  if (auto IPD = dyn_cast<ImplicitParamDecl>(&D)) {
    ...
    if (BlockInfo) {
      toolchain::Value *V = Arg.isIndirect()
  ...
  if (Arg.isIndirect()) {
    ...
    if (SrcLangAS != DestLangAS) {
      ...
      auto *T = toolchain::PointerType::get(getLLVMContext(), DestAS);
  ...
  toolchain::Value *ArgVal = (DoStore ? Arg.getDirectValue() : nullptr);
      ...
      if (lt == Qualifiers::OCL_Strong) {
        if (!isConsumed) {
          if (CGM.getCodeGenOpts().OptimizationLevel == 0) {
            ...
            toolchain::Value *Null = CGM.EmitNullConstant(D.getType());
  ...
  if (CGDebugInfo *DI = getDebugInfo()) {
    if (CGM.getCodeGenOpts().hasReducedDebugInfo() && !CurFuncIsThunk &&
        !NoDebugInfo) {
      toolchain::DILocalVariable *DILocalVar = DI->EmitDeclareOfArgVariable(
...
void CodeGenModule::EmitOMPAllocateDecl(const OMPAllocateDecl *D) {
  for (const Expr *E : D->varlist()) {
    ...
    language::Core::LangAS GVAS = GetGlobalVarAddressSpace(VD);
    auto TargetAS = getContext().getTargetAddressSpace(GVAS);
    if (Entry->getType()->getAddressSpace() == TargetAS)
      continue;

    toolchain::PointerType *PTy = toolchain::PointerType::get(getLLVMContext(), TargetAS);
    ...
    toolchain::GlobalVariable *DummyGV = new toolchain::GlobalVariable(
        getModule(), Entry->getValueType(), false,
        toolchain::GlobalValue::CommonLinkage, nullptr, "dummy", nullptr,
        toolchain::GlobalVariable::NotThreadLocal, Entry->getAddressSpace());
    Entry->replaceAllUsesWith(DummyGV);

    Entry->mutateType(PTy);
    toolchain::Constant *NewPtrForOldDecl =
        toolchain::ConstantExpr::getAddrSpaceCast(Entry, DummyGV->getType());
