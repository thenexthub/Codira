...
toolchain::Constant *CodeGenModule::GetAddrOfThunk(StringRef Name, toolchain::Type *FnTy,
                                              GlobalDecl GD) {
...
static void setThunkProperties(CodeGenModule &CGM, const ThunkInfo &Thunk,
                               toolchain::Function *ThunkFn, bool ForVTable,
                               GlobalDecl GD) {
  ...
  if (!CGM.getCXXABI().exportThunk()) {
    ThunkFn->setDLLStorageClass(toolchain::GlobalValue::DefaultStorageClass);
...
static RValue PerformReturnAdjustment(CodeGenFunction &CGF,
  ...
  toolchain::BasicBlock *AdjustNull = nullptr;
  toolchain::BasicBlock *AdjustNotNull = nullptr;
  toolchain::BasicBlock *AdjustEnd = nullptr;

  toolchain::Value *ReturnValue = RV.getScalarVal();
  ...
  if (NullCheckValue) {
    ...
    toolchain::Value *IsNull = CGF.Builder.CreateIsNull(ReturnValue);
  ...
  if (NullCheckValue) {
    ...
    toolchain::PHINode *PHI = CGF.Builder.CreatePHI(ReturnValue->getType(), 2);
    PHI->addIncoming(ReturnValue, AdjustNotNull);
    PHI->addIncoming(toolchain::Constant::getNullValue(ReturnValue->getType()),
                     AdjustNull);
    ReturnValue = PHI;
  }
...
static void resolveTopLevelMetadata(toolchain::Function *Fn,
                                    toolchain::ValueToValueMapTy &VMap) {
  // Clone the DISubprogram node and put it into the Value map.
  auto *DIS = Fn->getSubprogram();
  if (!DIS)
    return;
  auto *NewDIS = toolchain::MDNode::replaceWithDistinct(DIS->clone());
  VMap.MD()[DIS].reset(NewDIS);

  // Find all toolchain.dbg.declare intrinsics and resolve the DILocalVariable nodes
  // they are referencing.
  for (auto &BB : *Fn) {
    for (auto &I : BB) {
      for (toolchain::DbgVariableRecord &DVR :
           toolchain::filterDbgVars(I.getDbgRecordRange())) {
        auto *DILocal = DVR.getVariable();
        if (!DILocal->isResolved())
          DILocal->resolve();
      }
      if (auto *DII = dyn_cast<toolchain::DbgVariableIntrinsic>(&I)) {
...
toolchain::Function *
CodeGenFunction::GenerateVarArgsThunk(toolchain::Function *Fn,
  ...
  toolchain::Type *Ty = CGM.getTypes().GetFunctionType(FnInfo);
  toolchain::Value *Callee = CGM.GetAddrOfFunction(GD, Ty, /*ForVTable=*/true);
  toolchain::Function *BaseFn = cast<toolchain::Function>(Callee);
  ...
  toolchain::Function *NewFn = toolchain::CloneFunction(BaseFn, VMap);
  ...
  // Get the "this" value
  toolchain::Function::arg_iterator AI = Fn->arg_begin();
  ...
  toolchain::BasicBlock *EntryBB = &Fn->front();
  toolchain::BasicBlock::iterator ThisStore =
      toolchain::find_if(*EntryBB, [&](toolchain::Instruction &I) {
        return isa<toolchain::StoreInst>(I) && I.getOperand(0) == &*AI;
      });
  ...
  toolchain::Value *AdjustedThisPtr = CGM.getCXXABI().performThisAdjustment(
  ...
  if (!Thunk.Return.isEmpty()) {
    ...
    for (toolchain::BasicBlock &BB : *Fn) {
      toolchain::Instruction *T = BB.getTerminator();
      if (isa<toolchain::ReturnInst>(T)) {
...
void CodeGenFunction::StartThunk(toolchain::Function *Fn, GlobalDecl GD,
                                 const CGFunctionInfo &FnInfo,
                                 bool IsUnprototyped) {
...
void CodeGenFunction::EmitCallAndReturnForThunk(toolchain::FunctionCallee Callee,
                                                const ThunkInfo *Thunk,
                                                bool IsUnprototyped) {
  ...
  toolchain::Value *AdjustedThisPtr =
  ...
  if (CurFnInfo->usesInAlloca() || CurFnInfo->isVariadic() || IsUnprototyped) {
    if (Thunk && !Thunk->Return.isEmpty()) {
      if (IsUnprototyped)
        ...
        toolchain_unreachable("shouldn't try to emit musttail return-adjusting "
  ...
  if (Thunk && !Thunk->Return.isEmpty())
    RV = PerformReturnAdjustment(*this, ResultType, RV, *Thunk);
  else if (toolchain::CallInst* Call = dyn_cast<toolchain::CallInst>(CallOrInvoke))
    Call->setTailCallKind(toolchain::CallInst::TCK_Tail);
...
void CodeGenFunction::EmitMustTailThunk(GlobalDecl GD,
  ...
  SmallVector<toolchain::Value *, 8> Args(toolchain::make_pointer_range(CurFn->args()));
  ...
  if (ThisAI.isDirect()) {
    ...
    toolchain::Type *ThisType = Args[ThisArgNo]->getType();
    ...
    toolchain::Type *ThisType = ThisAddr.getElementType();
    if (ThisType != AdjustedThisPtr->getType())
      AdjustedThisPtr = Builder.CreateBitCast(AdjustedThisPtr, ThisType);
  ...
  toolchain::CallInst *Call = Builder.CreateCall(Callee, Args);
  ...
  Call->setCallingConv(static_cast<toolchain::CallingConv::ID>(CallingConv));
...
void CodeGenFunction::generateThunk(toolchain::Function *Fn,
  ...
  toolchain::Type *Ty;
  if (IsUnprototyped)
    Ty = toolchain::StructType::get(getLLVMContext());
  else
    Ty = CGM.getTypes().GetFunctionType(FnInfo);

  toolchain::Constant *Callee = CGM.GetAddrOfFunction(GD, Ty, /*ForVTable=*/true);

  // Make the call and return the result.
  EmitCallAndReturnForThunk(toolchain::FunctionCallee(Fn->getFunctionType(), Callee),
...
toolchain::Constant *CodeGenVTables::maybeEmitThunk(GlobalDecl GD,
                                               const ThunkInfo &TI,
                                               bool ForVTable) {
  const CXXMethodDecl *MD = cast<CXXMethodDecl>(GD.getDecl());
  ...
  toolchain::Type *ThunkVTableTy = CGM.getTypes().GetFunctionTypeForVTable(GD);
  toolchain::Constant *Thunk = CGM.GetAddrOfThunk(Name, ThunkVTableTy, GD);
  ...
  toolchain::FunctionType *ThunkFnTy = CGM.getTypes().GetFunctionType(FnInfo);

  // If the type of the underlying GlobalValue is wrong, we'll have to replace
  // it. It should be a declaration.
  toolchain::Function *ThunkFn = cast<toolchain::Function>(Thunk->stripPointerCasts());
  if (ThunkFn->getFunctionType() != ThunkFnTy) {
    toolchain::GlobalValue *OldThunkFn = ThunkFn;

    assert(OldThunkFn->isDeclaration() && "Shouldn't replace non-declaration");

    // Remove the name from the old thunk function and get a new thunk.
    OldThunkFn->setName(StringRef());
    ThunkFn = toolchain::Function::Create(ThunkFnTy, toolchain::Function::ExternalLinkage,
...
void CodeGenVTables::addRelativeComponent(ConstantArrayBuilder &builder,
  ...
  if (component->isNullValue())
    return builder.add(toolchain::ConstantInt::get(CGM.Int32Ty, 0));

  auto *globalVal =
      cast<toolchain::GlobalValue>(component->stripPointerCastsAndAliases());
  toolchain::Module &module = CGM.getModule();
  ...
  if (auto *func = dyn_cast<toolchain::Function>(globalVal)) {
    target = toolchain::DSOLocalEquivalent::get(func);
  } else {
    toolchain::SmallString<16> rttiProxyName(globalVal->getName());
    ...
    toolchain::GlobalVariable *proxy = module.getNamedGlobal(rttiProxyName);
    if (!proxy) {
      proxy = new toolchain::GlobalVariable(module, globalVal->getType(),
                                       /*isConstant=*/true, stubLinkage,
                                       globalVal, rttiProxyName);
      proxy->setDSOLocal(true);
      proxy->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
      if (!proxy->hasLocalLinkage()) {
        proxy->setVisibility(toolchain::GlobalValue::HiddenVisibility);
...
toolchain::Type *CodeGenModule::getVTableComponentType() const {
  if (UseRelativeLayout(*this))
    return Int32Ty;
  return GlobalsInt8PtrTy;
}

toolchain::Type *CodeGenVTables::getVTableComponentType() const {
...
static void AddPointerLayoutOffset(const CodeGenModule &CGM,
                                   ...
                                   CharUnits offset) {
  builder.add(toolchain::ConstantExpr::getIntToPtr(
      toolchain::ConstantInt::get(CGM.PtrDiffTy, offset.getQuantity()),
...
static void AddRelativeLayoutOffset(const CodeGenModule &CGM,
                                    ConstantArrayBuilder &builder,
                                    CharUnits offset) {
  builder.add(toolchain::ConstantInt::get(CGM.Int32Ty, offset.getQuantity()));
}
...
void CodeGenVTables::addVTableComponent(ConstantArrayBuilder &builder,
  ...
  case VTableComponent::CK_DeletingDtorPointer: {
    ...
    if (CGM.getLangOpts().CUDA) {
      ...
      if (!CanEmitMethod) {
        ...
        return builder.add(
            toolchain::ConstantExpr::getNullValue(CGM.GlobalsInt8PtrTy));
      }
      // Method is acceptable, continue processing as usual.
    }

    auto getSpecialVirtualFn = [&](StringRef name) -> toolchain::Constant * {
      ...
      if (useRelativeLayout())
        return toolchain::ConstantPointerNull::get(CGM.GlobalsInt8PtrTy);
      ...
      toolchain::Constant *fn = cast<toolchain::Constant>(
          CGM.CreateRuntimeFunction(fnTy, name).getCallee());
      if (auto f = dyn_cast<toolchain::Function>(fn))
    ...
    // Otherwise we can use the method definition directly.
      ...
      toolchain::Type *fnTy = CGM.getTypes().GetFunctionTypeForVTable(GD);
    ...
    if (useRelativeLayout()) {
      ...
      if (FnAS != GVAS)
        fnPtr =
            toolchain::ConstantExpr::getAddrSpaceCast(fnPtr, CGM.GlobalsInt8PtrTy);
  ...
  case VTableComponent::CK_UnusedFunctionPointer:
    if (useRelativeLayout())
      return builder.add(toolchain::ConstantExpr::getNullValue(CGM.Int32Ty));
    else
      return builder.addNullPointer(CGM.GlobalsInt8PtrTy);
  }

  toolchain_unreachable("Unexpected vtable component kind");
}

toolchain::Type *CodeGenVTables::getVTableType(const VTableLayout &layout) {
  SmallVector<toolchain::Type *, 4> tys;
  toolchain::Type *componentType = getVTableComponentType();
  for (unsigned i = 0, e = layout.getNumVTables(); i != e; ++i)
    tys.push_back(toolchain::ArrayType::get(componentType, layout.getVTableSize(i)));

  return toolchain::StructType::get(CGM.getLLVMContext(), tys);
}
...
toolchain::GlobalVariable *CodeGenVTables::GenerateConstructionVTable(
  ...
  toolchain::Type *VTType = getVTableType(*VTLayout);
  ...
  if (Linkage == toolchain::GlobalVariable::AvailableExternallyLinkage)
    Linkage = toolchain::GlobalVariable::InternalLinkage;

  toolchain::Align Align = CGM.getDataLayout().getABITypeAlign(VTType);
  ...
  toolchain::Constant *RTTI = CGM.GetAddrOfRTTIDescriptor(
...
// value in hwasan.
void CodeGenVTables::RemoveHwasanMetadata(toolchain::GlobalValue *GV) const {
  if (CGM.getLangOpts().Sanitize.has(SanitizerKind::HWAddress)) {
    toolchain::GlobalValue::SanitizerMetadata Meta;
...
void CodeGenVTables::GenerateRelativeVTableAlias(toolchain::GlobalVariable *VTable,
  ...
  toolchain::SmallString<256> AliasName(AliasNameRef);
  VTable->setName(AliasName + ".local");

  auto Linkage = VTable->getLinkage();
  assert(toolchain::GlobalAlias::isValidLinkage(Linkage) &&
         "Invalid vtable alias linkage");

  toolchain::GlobalAlias *VTableAlias = CGM.getModule().getNamedAlias(AliasName);
  if (!VTableAlias) {
    VTableAlias = toolchain::GlobalAlias::create(VTable->getValueType(),
  ...
  if (!VTable->hasComdat()) {
    VTable->setLinkage(toolchain::GlobalValue::InternalLinkage);
  } else {
    ...
    VTable->setVisibility(toolchain::GlobalValue::HiddenVisibility);
  }
...
toolchain::GlobalVariable::LinkageTypes
CodeGenModule::getVTableLinkage(const CXXRecordDecl *RD) {
  if (!RD->isExternallyVisible())
    return toolchain::GlobalVariable::InternalLinkage;
    ...
    case TSK_ExplicitSpecialization:
      assert(
          (IsInNamedModule || def || CodeGenOpts.OptimizationLevel > 0 ||
           CodeGenOpts.getDebugInfo() != toolchain::codegenoptions::NoDebugInfo) &&
      ...
      if (IsExternalDefinition && CodeGenOpts.OptimizationLevel > 0)
        return toolchain::GlobalVariable::AvailableExternallyLinkage;

      if (keyFunction && keyFunction->isInlined())
        return !Context.getLangOpts().AppleKext
                   ? toolchain::GlobalVariable::LinkOnceODRLinkage
      ...
      case TSK_ExplicitInstantiationDefinition:
        return !Context.getLangOpts().AppleKext ?
                 toolchain::GlobalVariable::WeakODRLinkage :
                 toolchain::Function::InternalLinkage;

      case TSK_ExplicitInstantiationDeclaration:
        return IsExternalDefinition
                   ? toolchain::GlobalVariable::AvailableExternallyLinkage
  ...
  toolchain::GlobalVariable::LinkageTypes DiscardableODRLinkage =
      toolchain::GlobalValue::LinkOnceODRLinkage;
  toolchain::GlobalVariable::LinkageTypes NonDiscardableODRLinkage =
  ...
  if (RD->hasAttr<DLLExportAttr>()) {
    ...
    DiscardableODRLinkage = toolchain::GlobalVariable::AvailableExternallyLinkage;
    NonDiscardableODRLinkage = toolchain::GlobalVariable::AvailableExternallyLinkage;
  }
  ...
  toolchain_unreachable("Invalid TemplateSpecializationKind!");
}
...
toolchain::GlobalObject::VCallVisibility CodeGenModule::GetVCallVisibilityLevel(
    const CXXRecordDecl *RD, toolchain::DenseSet<const CXXRecordDecl *> &Visited) {
  ...
  if (!Visited.insert(RD).second)
    return toolchain::GlobalObject::VCallVisibilityTranslationUnit;

  LinkageInfo LV = RD->getLinkageAndVisibility();
  toolchain::GlobalObject::VCallVisibility TypeVis;
  if (!isExternallyVisible(LV.getLinkage()))
    TypeVis = toolchain::GlobalObject::VCallVisibilityTranslationUnit;
  else if (HasHiddenLTOVisibility(RD))
    TypeVis = toolchain::GlobalObject::VCallVisibilityLinkageUnit;
  else
    TypeVis = toolchain::GlobalObject::VCallVisibilityPublic;
...
void CodeGenModule::EmitVTableTypeMetadata(const CXXRecordDecl *RD,
  ...
  for (auto &&AP : VTLayout.getAddressPoints()) {
    AddressPoint N{AP.first.getBase(),
                   ...
                   {}};
    toolchain::raw_string_ostream Stream(N.TypeName);
  ...
  for (auto AP : AddressPoints) {
    ...
    for (unsigned I = 0; I != Comps.size(); ++I) {
      if (Comps[I].getKind() != VTableComponent::CK_FunctionPointer)
        continue;
      toolchain::Metadata *MD = CreateMetadataIdentifierForVirtualMemPtrType(
