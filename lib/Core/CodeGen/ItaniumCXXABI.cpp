...
class ItaniumCXXABI : public CodeGen::CGCXXABI {
  ...
  toolchain::DenseMap<const CXXRecordDecl *, toolchain::GlobalVariable *> VTables;

  /// All the thread wrapper functions that have been used.
  toolchain::SmallVector<std::pair<const VarDecl *, toolchain::Function *>, 8>
      ThreadWrappers;
...
public:
  ...
  bool isThisCompleteObject(GlobalDecl GD) const override {
    ...
    if (isa<CXXDestructorDecl>(GD.getDecl())) {
      ...
      case Dtor_Comdat:
        toolchain_unreachable("emitting dtor comdat as function?");
      }
    ...
    if (isa<CXXConstructorDecl>(GD.getDecl())) {
      ...
      case Ctor_DefaultClosure:
        toolchain_unreachable("closure ctors in Itanium ABI?");

      case Ctor_Comdat:
        toolchain_unreachable("emitting ctor comdat as function?");
      }
      toolchain_unreachable("bad dtor kind");
    }
  ...
  toolchain::Constant *BuildMemberPointer(const CXXMethodDecl *MD,
                                     CharUnits ThisAdjustment);

  toolchain::Value *EmitMemberPointerComparison(CodeGenFunction &CGF,
                                           toolchain::Value *L, toolchain::Value *R,
                                           const MemberPointerType *MPT,
  ...
  bool hasUniqueVTablePointer(QualType RecordTy) {
    ...
    // unique address.
    if (!toolchain::GlobalValue::isWeakForLinker(CGM.getVTableLinkage(RD)))
      return true;
  ...
  toolchain::Value *emitExactDynamicCast(CodeGenFunction &CGF, Address ThisAddr,
                                    QualType SrcRecordTy, QualType DestTy,
                                    QualType DestRecordTy,
  ...
  toolchain::Value *getVTableAddressPointInStructorWithVTT(
      CodeGenFunction &CGF, const CXXRecordDecl *VTableClass,
  ...
  void setThunkLinkage(toolchain::Function *Thunk, bool ForVTable, GlobalDecl GD,
                       bool ReturnAdjustment) override {
    // Allow inlining of thunks by emitting them with available_externally
    // linkage together with vtables when needed.
    if (ForVTable && !Thunk->hasLocalLinkage())
      Thunk->setLinkage(toolchain::GlobalValue::AvailableExternallyLinkage);
  ...
  toolchain::Function *getOrCreateThreadLocalWrapper(const VarDecl *VD,
                                                toolchain::Value *Val);
...
public:
  ...
  classifyRTTIUniqueness(QualType CanTy,
                         toolchain::GlobalValue::LinkageTypes Linkage) const;
...
public:
  ...
  Address InitializeArrayCookie(CodeGenFunction &CGF,
                                ...
                                const CXXNewExpr *expr,
                                QualType ElementType) override;
  toolchain::Value *readArrayCookieImpl(CodeGenFunction &CGF, Address allocPtr,
                                   CharUnits cookieSize) override;
};
...
private:
  void emitCXXStermFinalizer(const VarDecl &D, toolchain::Function *dtorStub,
...
CodeGen::CGCXXABI *CodeGen::CreateItaniumCXXABI(CodeGenModule &CGM) {
  ...
  case TargetCXXABI::Microsoft:
    toolchain_unreachable("Microsoft ABI is not Itanium-based");
  }
  toolchain_unreachable("bad ABI kind");
}
...
ItaniumCXXABI::ConvertMemberPointerType(const MemberPointerType *MPT) {
  ...
  return toolchain::StructType::get(CGM.PtrDiffTy, CGM.PtrDiffTy);
}
...
CGCallee ItaniumCXXABI::EmitLoadOfMemberFunctionPointer(
    ...
    toolchain::Value *&ThisPtrForCall,
    toolchain::Value *MemFnPtr, const MemberPointerType *MPT) {
  ...
  toolchain::Constant *ptrdiff_1 = toolchain::ConstantInt::get(CGM.PtrDiffTy, 1);

  toolchain::BasicBlock *FnVirtual = CGF.createBasicBlock("memptr.virtual");
  toolchain::BasicBlock *FnNonVirtual = CGF.createBasicBlock("memptr.nonvirtual");
  toolchain::BasicBlock *FnEnd = CGF.createBasicBlock("memptr.end");

  // Extract memptr.adj, which is in the second field.
  toolchain::Value *RawAdj = Builder.CreateExtractValue(MemFnPtr, 1, "memptr.adj");

  // Compute the true adjustment.
  toolchain::Value *Adj = RawAdj;
  if (UseARMMethodPtrABI)
    Adj = Builder.CreateAShr(Adj, ptrdiff_1, "memptr.adj.shifted");

  // Apply the adjustment and cast back to the original struct type
  // for consistency.
  toolchain::Value *This = ThisAddr.emitRawPointer(CGF);
  This = Builder.CreateInBoundsGEP(Builder.getInt8Ty(), This, Adj);
  ThisPtrForCall = This;

  // Load the function pointer.
  toolchain::Value *FnAsInt = Builder.CreateExtractValue(MemFnPtr, 0, "memptr.ptr");
  ...
  toolchain::Type *VTableTy = CGF.CGM.GlobalsInt8PtrTy;
  CharUnits VTablePtrAlign =
    CGF.CGM.getDynamicOffsetAlignment(ThisAddr.getAlignment(), RD,
                                      CGF.getPointerAlign());
  toolchain::Value *VTable = CGF.GetVTablePtr(
  ...
  toolchain::Value *VTableOffset = FnAsInt;
  if (!UseARMMethodPtrABI)
  ...
  // pointers is enabled.
  toolchain::Constant *CheckSourceLocation;
  toolchain::Constant *CheckTypeDesc;
  ...
  toolchain::Value *VirtualFn = nullptr;

  {
    ...
    toolchain::Value *TypeId = nullptr;
    toolchain::Value *CheckResult = nullptr;

    if (ShouldEmitCFICheck || ShouldEmitVFEInfo || ShouldEmitWPDInfo) {
      // If doing CFI, VFE or WPD, we will need the metadata node to check
      // against.
      toolchain::Metadata *MD =
          CGM.CreateMetadataIdentifierForVirtualMemPtrType(QualType(MPT, 0));
      TypeId = toolchain::MetadataAsValue::get(CGF.getLLVMContext(), MD);
    }

    if (ShouldEmitVFEInfo) {
      toolchain::Value *VFPAddr =
      ...
      toolchain::Value *CheckedLoad = Builder.CreateCall(
          CGM.getIntrinsic(toolchain::Intrinsic::type_checked_load),
          {VFPAddr, toolchain::ConstantInt::get(CGM.Int32Ty, 0), TypeId});
      ...
      if (ShouldEmitCFICheck || ShouldEmitWPDInfo) {
        toolchain::Value *VFPAddr =
            Builder.CreateGEP(CGF.Int8Ty, VTable, VTableOffset);
        toolchain::Intrinsic::ID IID = CGM.HasHiddenLTOVisibility(RD)
                                      ? toolchain::Intrinsic::type_test
                                      : toolchain::Intrinsic::public_type_test;
      ...
      if (CGM.getItaniumVTableContext().isRelativeLayout()) {
        VirtualFn = CGF.Builder.CreateCall(
            CGM.getIntrinsic(toolchain::Intrinsic::load_relative,
    ...
    if (ShouldEmitCFICheck) {
      ...
      toolchain::Constant *StaticData[] = {
          toolchain::ConstantInt::get(CGF.Int8Ty, CodeGenFunction::CFITCK_VMFCall),
          CheckSourceLocation,
          CheckTypeDesc,
      };
      ...
      if (CGM.getCodeGenOpts().SanitizeTrap.has(SanitizerKind::CFIMFCall)) {
        ...
        toolchain::Value *AllVtables = toolchain::MetadataAsValue::get(
            CGM.getLLVMContext(),
            toolchain::MDString::get(CGM.getLLVMContext(), "all-vtables"));
        toolchain::Value *ValidVtable = Builder.CreateCall(
            CGM.getIntrinsic(toolchain::Intrinsic::type_test), {VTable, AllVtables});
  ...
  if (ShouldEmitCFICheck) {
    ...
    if (RD->hasDefinition()) {
      ...
      toolchain::Constant *StaticData[] = {
          toolchain::ConstantInt::get(CGF.Int8Ty, CodeGenFunction::CFITCK_NVMFCall),
          CheckSourceLocation,
          CheckTypeDesc,
      };

      toolchain::Value *Bit = Builder.getFalse();
      for (const CXXRecordDecl *Base : CGM.getMostBaseClasses(RD)) {
        toolchain::Metadata *MD = CGM.CreateMetadataIdentifierForType(
            getContext().getMemberPointerType(MPT->getPointeeType(),
                                              /*Qualifier=*/std::nullopt,
                                              Base->getCanonicalDecl()));
        toolchain::Value *TypeId =
            toolchain::MetadataAsValue::get(CGF.getLLVMContext(), MD);

        toolchain::Value *TypeTest =
            Builder.CreateCall(CGM.getIntrinsic(toolchain::Intrinsic::type_test),
                               {NonVirtualFn, TypeId});
        Bit = Builder.CreateOr(Bit, TypeTest);
      }

      CGF.EmitCheck(std::make_pair(Bit, CheckOrdinal), CheckHandler, StaticData,
                    {NonVirtualFn, toolchain::UndefValue::get(CGF.IntPtrTy)});
  ...
  // We're done.
  CGF.EmitBlock(FnEnd);
  toolchain::PHINode *CalleePtr = Builder.CreatePHI(CGF.UnqualPtrTy, 2);
  ...
  if (const auto &Schema =
    ...
    toolchain::PHINode *DiscriminatorPHI = Builder.CreatePHI(CGF.IntPtrTy, 2);
    DiscriminatorPHI->addIncoming(toolchain::ConstantInt::get(CGF.IntPtrTy, 0),
                                  FnVirtual);
    const auto &AuthInfo =
...
/// base object.
toolchain::Value *ItaniumCXXABI::EmitMemberDataPointerAddress(
  ...
  // Apply the offset.
  toolchain::Value *BaseAddr = Base.emitRawPointer(CGF);
...
static toolchain::Constant *pointerAuthResignConstant(
    toolchain::Value *Ptr, const CGPointerAuthInfo &CurAuthInfo,
    const CGPointerAuthInfo &NewAuthInfo, CodeGenModule &CGM) {
  const auto *CPA = dyn_cast<toolchain::ConstantPtrAuth>(Ptr);
  ...
  return CGM.getConstantSignedPointer(
      ...
      cast<toolchain::ConstantInt>(NewAuthInfo.getDiscriminator()));
}
...
ItaniumCXXABI::EmitMemberPointerConversion(CodeGenFunction &CGF,
  ...
  if (DstType->isMemberFunctionPointerType()) {
    if (const auto &NewAuthInfo =
      ...
      toolchain::Value *MemFnPtr = Builder.CreateExtractValue(src, 0, "memptr.ptr");
      toolchain::Type *OrigTy = MemFnPtr->getType();

      toolchain::BasicBlock *StartBB = Builder.GetInsertBlock();
      toolchain::BasicBlock *ResignBB = CGF.createBasicBlock("resign");
      toolchain::BasicBlock *MergeBB = CGF.createBasicBlock("merge");

      // Check whether we have a virtual offset or a pointer to a function.
      assert(UseARMMethodPtrABI && "ARM ABI expected");
      toolchain::Value *Adj = Builder.CreateExtractValue(src, 1, "memptr.adj");
      toolchain::Constant *Ptrdiff_1 = toolchain::ConstantInt::get(CGM.PtrDiffTy, 1);
      toolchain::Value *AndVal = Builder.CreateAnd(Adj, Ptrdiff_1);
      toolchain::Value *IsVirtualOffset =
      ...
      toolchain::Type *PtrTy = toolchain::PointerType::getUnqual(CGM.getLLVMContext());
      MemFnPtr = Builder.CreateIntToPtr(MemFnPtr, PtrTy);
      MemFnPtr =
          CGF.emitPointerAuthResign(MemFnPtr, SrcType, CurAuthInfo, NewAuthInfo,
                                    isa<toolchain::Constant>(src));
      MemFnPtr = Builder.CreatePtrToInt(MemFnPtr, OrigTy);
      toolchain::Value *ResignedVal = Builder.CreateInsertValue(src, MemFnPtr, 0);
      ResignBB = Builder.GetInsertBlock();

      CGF.EmitBlock(MergeBB);
      toolchain::PHINode *NewSrc = Builder.CreatePHI(src->getType(), 2);
  ...
  toolchain::Constant *adj = getMemberPointerAdjustment(E);
  if (!adj) return src;
  ...
  if (destTy->isMemberDataPointer()) {
    ...
    // Null check.
    toolchain::Value *null = toolchain::Constant::getAllOnesValue(src->getType());
    toolchain::Value *isNull = Builder.CreateICmpEQ(src, null, "memptr.isnull");
    return Builder.CreateSelect(isNull, src, dst);
  }

  // The this-adjustment is left-shifted by 1 on ARM.
  if (UseARMMethodPtrABI) {
    uint64_t offset = cast<toolchain::ConstantInt>(adj)->getZExtValue();
    offset <<= 1;
    adj = toolchain::ConstantInt::get(adj->getType(), offset);
  }

  toolchain::Value *srcAdj = Builder.CreateExtractValue(src, 1, "src.adj");
  toolchain::Value *dstAdj;
  if (isDerivedToBase)
...
pointerAuthResignMemberFunctionPointer(toolchain::Constant *Src, QualType DestType,
  ...
  toolchain::Constant *MemFnPtr = Src->getAggregateElement(0u);
  if (MemFnPtr->getNumOperands() == 0) {
    // src must be a pair of null pointers.
    assert(isa<toolchain::ConstantInt>(MemFnPtr) && "constant int expected");
    return Src;
  }

  toolchain::Constant *ConstPtr = pointerAuthResignConstant(
      cast<toolchain::User>(MemFnPtr)->getOperand(0), CurAuthInfo, NewAuthInfo, CGM);
  ConstPtr = toolchain::ConstantExpr::getPtrToInt(ConstPtr, MemFnPtr->getType());
...
ItaniumCXXABI::EmitMemberPointerConversion(const CastExpr *E,
  ...
  if (destTy->isMemberDataPointer()) {
    ...
    if (isDerivedToBase)
      return toolchain::ConstantExpr::getNSWSub(src, adj);
    else
      return toolchain::ConstantExpr::getNSWAdd(src, adj);
  }
  ...
  toolchain::Constant *srcAdj = src->getAggregateElement(1);
  toolchain::Constant *dstAdj;
  if (isDerivedToBase)
    dstAdj = toolchain::ConstantExpr::getNSWSub(srcAdj, adj);
  else
    dstAdj = toolchain::ConstantExpr::getNSWAdd(srcAdj, adj);

  toolchain::Constant *res = ConstantFoldInsertValueInstruction(src, dstAdj, 1);
...
ItaniumCXXABI::EmitNullMemberPointer(const MemberPointerType *MPT) {
  ...
  if (MPT->isMemberDataPointer())
    return toolchain::ConstantInt::get(CGM.PtrDiffTy, -1ULL, /*isSigned=*/true);

  toolchain::Constant *Zero = toolchain::ConstantInt::get(CGM.PtrDiffTy, 0);
  toolchain::Constant *Values[2] = { Zero, Zero };
  return toolchain::ConstantStruct::getAnon(Values);
}
...
ItaniumCXXABI::EmitMemberDataPointer(const MemberPointerType *MPT,
  ...
  return toolchain::ConstantInt::get(CGM.PtrDiffTy, offset.getQuantity());
}
...
toolchain::Constant *ItaniumCXXABI::BuildMemberPointer(const CXXMethodDecl *MD,
                                                  CharUnits ThisAdjustment) {
  ...
  if (MD->isVirtual()) {
    ...
    if (UseARMMethodPtrABI) {
      ...
      MemPtr[0] = toolchain::ConstantInt::get(CGM.PtrDiffTy, VTableOffset + 1);
      MemPtr[1] = toolchain::ConstantInt::get(CGM.PtrDiffTy,
    ...
    toolchain::Constant *addr = CGM.getMemberFunctionPointer(MD, Ty);

    MemPtr[0] = toolchain::ConstantExpr::getPtrToInt(addr, CGM.PtrDiffTy);
    MemPtr[1] = toolchain::ConstantInt::get(CGM.PtrDiffTy,
                                       (UseARMMethodPtrABI ? 2 : 1) *
                                       ThisAdjustment.getQuantity());
  }

  return toolchain::ConstantStruct::getAnon(MemPtr);
}

toolchain::Constant *ItaniumCXXABI::EmitMemberPointer(const APValue &MP,
                                                 QualType MPType) {
  ...
  if (const CXXMethodDecl *MD = dyn_cast<CXXMethodDecl>(MPD)) {
    toolchain::Constant *Src = BuildMemberPointer(MD, ThisAdjustment);
...
ItaniumCXXABI::EmitMemberPointerComparison(CodeGenFunction &CGF,
  ...
  toolchain::Value *LPtr = Builder.CreateExtractValue(L, 0, "lhs.memptr.ptr");
  toolchain::Value *RPtr = Builder.CreateExtractValue(R, 0, "rhs.memptr.ptr");

  // This condition tests whether L.ptr == R.ptr.  This must always be
  // true for equality to hold.
  toolchain::Value *PtrEq = Builder.CreateICmp(Eq, LPtr, RPtr, "cmp.ptr");

  // This condition, together with the assumption that L.ptr == R.ptr,
  // tests whether the pointers are both null.  ARM imposes an extra
  // condition.
  toolchain::Value *Zero = toolchain::Constant::getNullValue(LPtr->getType());
  toolchain::Value *EqZero = Builder.CreateICmp(Eq, LPtr, Zero, "cmp.ptr.null");

  // This condition tests whether L.adj == R.adj.  If this isn't
  // true, the pointers are unequal unless they're both null.
  toolchain::Value *LAdj = Builder.CreateExtractValue(L, 1, "lhs.memptr.adj");
  toolchain::Value *RAdj = Builder.CreateExtractValue(R, 1, "rhs.memptr.adj");
  toolchain::Value *AdjEq = Builder.CreateICmp(Eq, LAdj, RAdj, "cmp.adj");

  // Null member function pointers on ARM clear the low bit of Adj,
  // so the zero condition has to check that neither low bit is set.
  if (UseARMMethodPtrABI) {
    toolchain::Value *One = toolchain::ConstantInt::get(LPtr->getType(), 1);

    // Compute (l.adj | r.adj) & 1 and test it against zero.
    toolchain::Value *OrAdj = Builder.CreateOr(LAdj, RAdj, "or.adj");
    toolchain::Value *OrAdjAnd1 = Builder.CreateAnd(OrAdj, One);
    toolchain::Value *OrAdjAnd1EqZero = Builder.CreateICmp(Eq, OrAdjAnd1, Zero,
                                                      "cmp.or.adj");
    EqZero = Builder.CreateBinOp(And, EqZero, OrAdjAnd1EqZero);
  }

  // Tie together all our conditions.
  toolchain::Value *Result = Builder.CreateBinOp(Or, EqZero, AdjEq);
...
ItaniumCXXABI::EmitMemberPointerIsNotNull(CodeGenFunction &CGF,
  ...
  if (MPT->isMemberDataPointer()) {
    assert(MemPtr->getType() == CGM.PtrDiffTy);
    toolchain::Value *NegativeOne =
      toolchain::Constant::getAllOnesValue(MemPtr->getType());
    return Builder.CreateICmpNE(MemPtr, NegativeOne, "memptr.tobool");
  }

  // In Itanium, a member function pointer is not null if 'ptr' is not null.
  toolchain::Value *Ptr = Builder.CreateExtractValue(MemPtr, 0, "memptr.ptr");

  toolchain::Constant *Zero = toolchain::ConstantInt::get(Ptr->getType(), 0);
  toolchain::Value *Result = Builder.CreateICmpNE(Ptr, Zero, "memptr.tobool");

  // On ARM, a member function pointer is also non-null if the low bit of 'adj'
  // (the virtual bit) is set.
  if (UseARMMethodPtrABI) {
    toolchain::Constant *One = toolchain::ConstantInt::get(Ptr->getType(), 1);
    toolchain::Value *Adj = Builder.CreateExtractValue(MemPtr, 1, "memptr.adj");
    toolchain::Value *VirtualBit = Builder.CreateAnd(Adj, One, "memptr.virtualbit");
    toolchain::Value *IsVirtual = Builder.CreateICmpNE(VirtualBit, Zero,
                                                  "memptr.isvirtual");
...
void ItaniumCXXABI::emitVirtualObjectDelete(CodeGenFunction &CGF,
  ...
  if (UseGlobalDelete) {
    ...
    auto *ClassDecl = cast<CXXRecordDecl>(
                          ...
                          ->getDefinitionOrSelf();
    toolchain::Value *VTable = CGF.GetVTablePtr(Ptr, CGF.UnqualPtrTy, ClassDecl);

    // Track back to entry -2 and pull out the offset there.
    toolchain::Value *OffsetPtr = CGF.Builder.CreateConstInBoundsGEP1_64(
        CGF.IntPtrTy, VTable, -2, "complete-offset.ptr");
    toolchain::Value *Offset = CGF.Builder.CreateAlignedLoad(CGF.IntPtrTy, OffsetPtr,
                                                        CGF.getPointerAlign());

    // Apply the offset.
    toolchain::Value *CompletePtr = Ptr.emitRawPointer(CGF);
    CompletePtr =
...
void ItaniumCXXABI::emitRethrow(CodeGenFunction &CGF, bool isNoReturn) {
  ...
  toolchain::FunctionCallee Fn = CGM.CreateRuntimeFunction(FTy, "__cxa_rethrow");
...
static toolchain::FunctionCallee getAllocateExceptionFn(CodeGenModule &CGM) {
  // void *__cxa_allocate_exception(size_t thrown_size);

  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(CGM.Int8PtrTy, CGM.SizeTy, /*isVarArg=*/false);

  return CGM.CreateRuntimeFunction(FTy, "__cxa_allocate_exception");
}

static toolchain::FunctionCallee getThrowFn(CodeGenModule &CGM) {
  // void __cxa_throw(void *thrown_exception, std::type_info *tinfo,
  //                  void (*dest) (void *));

  toolchain::Type *Args[3] = { CGM.Int8PtrTy, CGM.GlobalsInt8PtrTy, CGM.Int8PtrTy };
  toolchain::FunctionType *FTy =
...
void ItaniumCXXABI::emitThrow(CodeGenFunction &CGF, const CXXThrowExpr *E) {
  ...
  toolchain::Type *SizeTy = CGF.ConvertType(getContext().getSizeType());
  uint64_t TypeSize = getContext().getTypeSizeInChars(ThrowType).getQuantity();

  toolchain::FunctionCallee AllocExceptionFn = getAllocateExceptionFn(CGM);
  toolchain::CallInst *ExceptionPtr = CGF.EmitNounwindRuntimeCall(
      AllocExceptionFn, toolchain::ConstantInt::get(SizeTy, TypeSize), "exception");
  ...
  toolchain::Constant *TypeInfo = CGM.GetAddrOfRTTIDescriptor(ThrowType,
                                                         /*ForEH=*/true);

  // The address of the destructor.  If the exception type has a
  // trivial destructor (or isn't a record), we just pass null.
  toolchain::Constant *Dtor = nullptr;
  ...
  if (!Dtor) Dtor = toolchain::Constant::getNullValue(CGM.Int8PtrTy);

  toolchain::Value *args[] = { ExceptionPtr, TypeInfo, Dtor };
  CGF.EmitNoreturnRuntimeCallOrInvoke(getThrowFn(CGM), args);
}

static toolchain::FunctionCallee getItaniumDynamicCastFn(CodeGenFunction &CGF) {
  // void *__dynamic_cast(const void *sub,
  //                      global_as const abi::__class_type_info *src,
  //                      global_as const abi::__class_type_info *dst,
  //                      std::ptrdiff_t src2dst_offset);

  toolchain::Type *Int8PtrTy = CGF.Int8PtrTy;
  toolchain::Type *GlobInt8PtrTy = CGF.GlobalsInt8PtrTy;
  toolchain::Type *PtrDiffTy =
    CGF.ConvertType(CGF.getContext().getPointerDiffType());

  toolchain::Type *Args[4] = { Int8PtrTy, GlobInt8PtrTy, GlobInt8PtrTy, PtrDiffTy };
  ...
  toolchain::AttrBuilder FuncAttrs(CGF.getLLVMContext());
  FuncAttrs.addAttribute(toolchain::Attribute::NoUnwind);
  FuncAttrs.addAttribute(toolchain::Attribute::WillReturn);
  FuncAttrs.addMemoryAttr(toolchain::MemoryEffects::readOnly());
  toolchain::AttributeList Attrs = toolchain::AttributeList::get(
      CGF.getLLVMContext(), toolchain::AttributeList::FunctionIndex, FuncAttrs);

  return CGF.CGM.CreateRuntimeFunction(FTy, "__dynamic_cast", Attrs);
}

static toolchain::FunctionCallee getBadCastFn(CodeGenFunction &CGF) {
  // void __cxa_bad_cast();
  toolchain::FunctionType *FTy = toolchain::FunctionType::get(CGF.VoidTy, false);
...
static toolchain::FunctionCallee getBadTypeidFn(CodeGenFunction &CGF) {
...
void ItaniumCXXABI::EmitBadTypeidCall(CodeGenFunction &CGF) {
  toolchain::FunctionCallee Fn = getBadTypeidFn(CGF);
  toolchain::CallBase *Call = CGF.EmitRuntimeCallOrInvoke(Fn);
  Call->setDoesNotReturn();
  CGF.Builder.CreateUnreachable();
}

toolchain::Value *ItaniumCXXABI::EmitTypeid(CodeGenFunction &CGF,
  ...
  auto *ClassDecl =
      cast<CXXRecordDecl>(SrcRecordTy->castAs<RecordType>()->getOriginalDecl())
          ->getDefinitionOrSelf();
  toolchain::Value *Value = CGF.GetVTablePtr(ThisPtr, CGM.GlobalsInt8PtrTy,
                                        ClassDecl);

  if (CGM.getItaniumVTableContext().isRelativeLayout()) {
    // Load the type info.
    Value = CGF.Builder.CreateCall(
        CGM.getIntrinsic(toolchain::Intrinsic::load_relative, {CGM.Int32Ty}),
        {Value, toolchain::ConstantInt::get(CGM.Int32Ty, -4)});
  } else {
    // Load the type info.
    Value =
...
toolchain::Value *ItaniumCXXABI::emitDynamicCastCall(
    CodeGenFunction &CGF, Address ThisAddr, QualType SrcRecordTy,
    QualType DestTy, QualType DestRecordTy, toolchain::BasicBlock *CastEnd) {
  toolchain::Type *PtrDiffLTy =
      CGF.ConvertType(CGF.getContext().getPointerDiffType());
  ...
  toolchain::Value *OffsetHint = toolchain::ConstantInt::get(
      PtrDiffLTy,
      computeOffsetHint(CGF.getContext(), SrcDecl, DestDecl).getQuantity());

  // Emit the call to __dynamic_cast.
  toolchain::Value *Value = ThisAddr.emitRawPointer(CGF);
  ...
  toolchain::Value *args[] = {Value, SrcRTTI, DestRTTI, OffsetHint};
  Value = CGF.EmitNounwindRuntimeCall(getItaniumDynamicCastFn(CGF), args);

  /// C++ [expr.dynamic.cast]p9:
  ///   A failed cast to reference type throws std::bad_cast
  if (DestTy->isReferenceType()) {
    toolchain::BasicBlock *BadCastBlock =
        CGF.createBasicBlock("dynamic_cast.bad_cast");

    toolchain::Value *IsNull = CGF.Builder.CreateIsNull(Value);
...
toolchain::Value *ItaniumCXXABI::emitExactDynamicCast(
    CodeGenFunction &CGF, Address ThisAddr, QualType SrcRecordTy,
    QualType DestTy, QualType DestRecordTy,
    const ExactDynamicCastInfo &ExactCastInfo, toolchain::BasicBlock *CastSuccess,
    toolchain::BasicBlock *CastFail) {
  const CXXRecordDecl *SrcDecl = SrcRecordTy->getAsCXXRecordDecl();
  const CXXRecordDecl *DestDecl = DestRecordTy->getAsCXXRecordDecl();

  toolchain::Value *VTable = nullptr;
  ...
  // this offset.
  toolchain::Constant *ExpectedVTable = getVTableAddressPoint(
      BaseSubobject(SrcDecl, ExactCastInfo.Offset), DestDecl);
  toolchain::Value *Success = CGF.Builder.CreateICmpEQ(VTable, ExpectedVTable);
  toolchain::Value *AdjustedThisPtr = ThisAddr.emitRawPointer(CGF);

  if (!ExactCastInfo.Offset.isZero()) {
    CharUnits::QuantityType Offset = ExactCastInfo.Offset.getQuantity();
    toolchain::Constant *OffsetConstant =
        toolchain::ConstantInt::get(CGF.PtrDiffTy, -Offset);
...
toolchain::Value *ItaniumCXXABI::emitDynamicCastToVoid(CodeGenFunction &CGF,
                                                  Address ThisAddr,
                                                  QualType SrcRecordTy) {
  auto *ClassDecl =
      cast<CXXRecordDecl>(SrcRecordTy->castAs<RecordType>()->getOriginalDecl())
          ->getDefinitionOrSelf();
  toolchain::Value *OffsetToTop;
  if (CGM.getItaniumVTableContext().isRelativeLayout()) {
...
bool ItaniumCXXABI::EmitBadCastCall(CodeGenFunction &CGF) {
  toolchain::FunctionCallee Fn = getBadCastFn(CGF);
  toolchain::CallBase *Call = CGF.EmitRuntimeCallOrInvoke(Fn);
  Call->setDoesNotReturn();
  CGF.Builder.CreateUnreachable();
  return true;
...
ItaniumCXXABI::GetVirtualBaseClassOffset(CodeGenFunction &CGF,
  ...
  toolchain::Value *VTablePtr = CGF.GetVTablePtr(This, CGM.Int8PtrTy, ClassDecl);
  CharUnits VBaseOffsetOffset =
      CGM.getItaniumVTableContext().getVirtualBaseOffsetOffset(ClassDecl,
                                                               BaseClassDecl);
  toolchain::Value *VBaseOffsetPtr =
...
toolchain::Value *ItaniumCXXABI::getCXXDestructorImplicitParam(
...
static void setVTableSelectiveDLLImportExport(CodeGenModule &CGM,
  ...
  if (VTable->getDLLStorageClass() !=
          toolchain::GlobalVariable::DefaultStorageClass ||
      RD->hasAttr<DLLImportAttr>() || RD->hasAttr<DLLExportAttr>())
    return;

  if (CGM.getVTables().isVTableExternal(RD)) {
    if (CXXRecordNonInlineHasAttr<DLLImportAttr>(RD))
      VTable->setDLLStorageClass(toolchain::GlobalValue::DLLImportStorageClass);
  } else if (CXXRecordNonInlineHasAttr<DLLExportAttr>(RD))
    VTable->setDLLStorageClass(toolchain::GlobalValue::DLLExportStorageClass);
}

void ItaniumCXXABI::emitVTableDefinitions(CodeGenVTables &CGVT,
                                          const CXXRecordDecl *RD) {
  toolchain::GlobalVariable *VTable = getAddrOfVTable(RD, CharUnits());
  ...
  CGVT.createVTableInitializer(components, VTLayout, RTTI,
                               toolchain::GlobalValue::isLocalLinkage(Linkage));
  ...
  if (!VTable->isDeclarationForLinker() ||
    ...
    // @toolchain.compiler.used so that it isn't deleted before whole program
    // analysis.
    if (VTable->isDeclarationForLinker()) {
...
toolchain::Value *ItaniumCXXABI::getVTableAddressPointInStructor(
...
ItaniumCXXABI::getVTableAddressPoint(BaseSubobject Base,
  ...
  toolchain::GlobalValue *VTable = getAddrOfVTable(VTableClass, CharUnits());
  ...
  return toolchain::ConstantExpr::getGetElementPtr(
      VTable->getValueType(), VTable, Indices, /*InBounds=*/true, InRange);
}

toolchain::Value *ItaniumCXXABI::getVTableAddressPointInStructorWithVTT(
    CodeGenFunction &CGF, const CXXRecordDecl *VTableClass, BaseSubobject Base,
    const CXXRecordDecl *NearestVBase) {
  ...
  /// Load the VTT.
  toolchain::Value *VTT = CGF.LoadCXXVTT();
...
toolchain::GlobalVariable *ItaniumCXXABI::getAddrOfVTable(const CXXRecordDecl *RD,
                                                     CharUnits VPtrOffset) {
  assert(VPtrOffset.isZero() && "Itanium ABI only supports zero vptr offsets");

  toolchain::GlobalVariable *&VTable = VTables[RD];
  if (VTable)
    return VTable;
  ...
  VTable = CGM.CreateOrReplaceCXXRuntimeVariable(
      Name, VTableType, toolchain::GlobalValue::ExternalLinkage,
...
CGCallee ItaniumCXXABI::getVirtualFunctionPointer(CodeGenFunction &CGF,
                                                  ...
                                                  toolchain::Type *Ty,
                                                  SourceLocation Loc) {
  toolchain::Type *PtrTy = CGM.GlobalsInt8PtrTy;
  auto *MethodDecl = cast<CXXMethodDecl>(GD.getDecl());
  toolchain::Value *VTable = CGF.GetVTablePtr(This, PtrTy, MethodDecl->getParent());

  uint64_t VTableIndex = CGM.getItaniumVTableContext().getMethodVTableIndex(GD);
  toolchain::Value *VFunc, *VTableSlotPtr = nullptr;
  auto &Schema = CGM.getCodeGenOpts().PointerAuth.CXXVirtualFunctionPointers;

  toolchain::Type *ComponentTy = CGM.getVTables().getVTableComponentType();
  uint64_t ByteOffset =
  ...
  if (!Schema && CGF.ShouldEmitVTableTypeCheckedLoad(MethodDecl->getParent())) {
    ...
    toolchain::Value *VFuncLoad;
    if (CGM.getItaniumVTableContext().isRelativeLayout()) {
      VFuncLoad = CGF.Builder.CreateCall(
          CGM.getIntrinsic(toolchain::Intrinsic::load_relative, {CGM.Int32Ty}),
          {VTable, toolchain::ConstantInt::get(CGM.Int32Ty, ByteOffset)});
    } else {
    ...
    if (CGM.getCodeGenOpts().OptimizationLevel > 0 &&
      ...
      if (auto *VFuncLoadInstr = dyn_cast<toolchain::Instruction>(VFuncLoad)) {
        VFuncLoadInstr->setMetadata(
            toolchain::LLVMContext::MD_invariant_load,
            toolchain::MDNode::get(CGM.getLLVMContext(),
...
toolchain::Value *ItaniumCXXABI::EmitVirtualDestructorCall(
...
void ItaniumCXXABI::emitVirtualInheritanceTables(const CXXRecordDecl *RD) {
  ...
  toolchain::GlobalVariable *VTT = VTables.GetAddrOfVTT(RD);
...
static toolchain::Value *performTypeAdjustment(CodeGenFunction &CGF,
                                          Address InitialPtr,
  ...
  toolchain::Value *ResultPtr;
  if (VirtualAdjustment) {
    toolchain::Value *VTablePtr =
...
void ARMCXXABI::EmitReturnFromThunk(CodeGenFunction &CGF,
  ...
  toolchain::Type *T = CGF.ReturnValue.getElementType();
  RValue Undef = RValue::get(toolchain::UndefValue::get(T));
...
Address ItaniumCXXABI::InitializeArrayCookie(CodeGenFunction &CGF,
  ...
  toolchain::Instruction *SI = CGF.Builder.CreateStore(NumElements, NumElementsPtr);
  ...
  if (CGM.getLangOpts().Sanitize.has(SanitizerKind::Address) && AS == 0 &&
    ...
    toolchain::FunctionType *FTy =
        toolchain::FunctionType::get(CGM.VoidTy, NumElementsPtr.getType(), false);
    toolchain::FunctionCallee F =
...
toolchain::Value *ItaniumCXXABI::readArrayCookieImpl(CodeGenFunction &CGF,
                                                Address allocPtr,
                                                CharUnits cookieSize) {
  ...
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGF.SizeTy, CGF.UnqualPtrTy, false);
  toolchain::FunctionCallee F =
...
Address ARMCXXABI::InitializeArrayCookie(CodeGenFunction &CGF,
  ...
  toolchain::Value *elementSize = toolchain::ConstantInt::get(CGF.SizeTy,
...
toolchain::Value *ARMCXXABI::readArrayCookieImpl(CodeGenFunction &CGF,
                                            Address allocPtr,
                                            CharUnits cookieSize) {
...
static toolchain::FunctionCallee getGuardAcquireFn(CodeGenModule &CGM,
                                              toolchain::PointerType *GuardPtrTy) {
  // int __cxa_guard_acquire(__guard *guard_object);
  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(CGM.getTypes().ConvertType(CGM.getContext().IntTy),
...
static toolchain::FunctionCallee getGuardReleaseFn(CodeGenModule &CGM,
                                              toolchain::PointerType *GuardPtrTy) {
  // void __cxa_guard_release(__guard *guard_object);
  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(CGM.VoidTy, GuardPtrTy, /*isVarArg=*/false);
  return CGM.CreateRuntimeFunction(
...
static toolchain::FunctionCallee getGuardAbortFn(CodeGenModule &CGM,
                                            toolchain::PointerType *GuardPtrTy) {
...
namespace {
  struct CallGuardAbort final : EHScopeStack::Cleanup {
    toolchain::GlobalVariable *Guard;
    CallGuardAbort(toolchain::GlobalVariable *Guard) : Guard(Guard) {}
...
void ItaniumCXXABI::EmitGuardedInit(CodeGenFunction &CGF,
  ...
  toolchain::PointerType *guardPtrTy = toolchain::PointerType::get(
      CGF.CGM.getLLVMContext(),
      CGF.CGM.getDataLayout().getDefaultGlobalsAddressSpace());

  // Create the guard variable if we don't already have it (as we
  // might if we're double-emitting this function body).
  toolchain::GlobalVariable *guard = CGM.getStaticLocalDeclGuardAddress(&D);
  if (!guard) {
    ...
    // variable.
    guard = new toolchain::GlobalVariable(CGM.getModule(), guardTy,
                                     false, var->getLinkage(),
                                     toolchain::ConstantInt::get(guardTy, 0),
                                     guardName.str());
    ...
    toolchain::Comdat *C = var->getComdat();
  ...
  if (!threadsafe || MaxInlineWidthInBits) {
    ...
    toolchain::Value *V =
        (UseARMGuardVarABI && !useInt8GuardVariable)
            ? Builder.CreateAnd(LI, toolchain::ConstantInt::get(CGM.Int8Ty, 1))
            : LI;
    toolchain::Value *NeedsInit = Builder.CreateIsNull(V, "guard.uninitialized");

    toolchain::BasicBlock *InitCheckBlock = CGF.createBasicBlock("init.check");
  ...
  if (threadsafe) {
    ...
    Builder.CreateStore(toolchain::ConstantInt::get(CGM.Int8Ty, 1),
...
static void emitGlobalDtorWithCXAAtExit(CodeGenFunction &CGF,
                                        toolchain::FunctionCallee dtor,
                                        toolchain::Constant *addr, bool TLS) {
  ...
  if (TLS) {
    const toolchain::Triple &T = CGF.getTarget().getTriple();
    Name = T.isOSDarwin() ?  "_tlv_atexit" : "__cxa_thread_atexit";
  }

  // We're assuming that the destructor function is something we can
  // reasonably call with the default CC.
  toolchain::Type *dtorTy = CGF.UnqualPtrTy;

  // Preserve address space of addr.
  auto AddrAS = addr ? addr->getType()->getPointerAddressSpace() : 0;
  auto AddrPtrTy = AddrAS ? toolchain::PointerType::get(CGF.getLLVMContext(), AddrAS)
                          : CGF.Int8PtrTy;

  // Create a variable that binds the atexit to this shared object.
  toolchain::Constant *handle =
      CGF.CGM.CreateRuntimeVariable(CGF.Int8Ty, "__dso_handle");
  auto *GV = cast<toolchain::GlobalValue>(handle->stripPointerCasts());
  GV->setVisibility(toolchain::GlobalValue::HiddenVisibility);

  // extern "C" int __cxa_atexit(void (*f)(void *), void *p, void *d);
  toolchain::Type *paramTys[] = {dtorTy, AddrPtrTy, handle->getType()};
  toolchain::FunctionType *atexitTy =
    toolchain::FunctionType::get(CGF.IntTy, paramTys, false);

  // Fetch the actual function.
  toolchain::FunctionCallee atexit = CGF.CGM.CreateRuntimeFunction(atexitTy, Name);
  if (toolchain::Function *fn = dyn_cast<toolchain::Function>(atexit.getCallee()))
    fn->setDoesNotThrow();
  ...
  toolchain::Constant *dtorCallee = cast<toolchain::Constant>(dtor.getCallee());
  ...
  if (!addr)
    ...
    // function.
    addr = toolchain::Constant::getNullValue(CGF.Int8PtrTy);

  toolchain::Value *args[] = {dtorCallee, addr, handle};
  CGF.EmitNounwindRuntimeCall(atexit, args);
}

static toolchain::Function *createGlobalInitOrCleanupFn(CodeGen::CodeGenModule &CGM,
                                                   StringRef FnName) {
  ...
  toolchain::Function *GlobalInitOrCleanupFn = CGM.CreateGlobalInitOrCleanUpFunction(
...
void CodeGenModule::unregisterGlobalDtorsWithUnAtExit() {
  for (const auto &I : DtorsUsingAtExit) {
    ...
    std::string GlobalCleanupFnName =
        std::string("__GLOBAL_cleanup_") + toolchain::to_string(Priority);

    toolchain::Function *GlobalCleanupFn =
    ...
    toolchain::FunctionType *dtorFuncTy = toolchain::FunctionType::get(CGF.VoidTy, false);

    // Destructor functions are run/unregistered in non-ascending
    // order of their priorities.
    const toolchain::TinyPtrVector<toolchain::Function *> &Dtors = I.second;
    auto itv = Dtors.rbegin();
    while (itv != Dtors.rend()) {
      toolchain::Function *Dtor = *itv;

      // We're assuming that the destructor function is something we can
      // reasonably call with the correct CC.
      toolchain::Value *V = CGF.unregisterGlobalDtorWithUnAtExit(Dtor);
      toolchain::Value *NeedsDestruct =
          CGF.Builder.CreateIsNull(V, "needs_destruct");

      toolchain::BasicBlock *DestructCallBlock =
          CGF.createBasicBlock("destruct.call");
      toolchain::BasicBlock *EndBlock = CGF.createBasicBlock(
      ...
      toolchain::CallInst *CI = CGF.Builder.CreateCall(dtorFuncTy, Dtor);
...
void CodeGenModule::registerGlobalDtorsWithAtExit() {
  for (const auto &I : DtorsUsingAtExit) {
    ...
    std::string GlobalInitFnName =
        std::string("__GLOBAL_init_") + toolchain::to_string(Priority);
    toolchain::Function *GlobalInitFn =
...
getThreadLocalWrapperLinkage(const VarDecl *VD, CodeGen::CodeGenModule &CGM) {
  toolchain::GlobalValue::LinkageTypes VarLinkage =
      CGM.getLLVMLinkageVarDefinition(VD);

  // For internal linkage variables, we don't need an external or weak wrapper.
  if (toolchain::GlobalValue::isLocalLinkage(VarLinkage))
    return VarLinkage;

  // If the thread wrapper is replaceable, give it appropriate linkage.
  if (isThreadWrapperReplaceable(VD, CGM))
    if (!toolchain::GlobalVariable::isLinkOnceLinkage(VarLinkage) &&
        !toolchain::GlobalVariable::isWeakODRLinkage(VarLinkage))
      return VarLinkage;
...
ItaniumCXXABI::getOrCreateThreadLocalWrapper(const VarDecl *VD,
  ...
  {
    toolchain::raw_svector_ostream Out(WrapperName);
    getMangleContext().mangleItaniumThreadLocalWrapper(VD, Out);
  }

  // FIXME: If VD is a definition, we should regenerate the function attributes
  // before returning.
  if (toolchain::Value *V = CGM.getModule().getNamedValue(WrapperName))
    return cast<toolchain::Function>(V);
  ...
  toolchain::FunctionType *FnTy = CGM.getTypes().GetFunctionType(FI);
  toolchain::Function *Wrapper =
      toolchain::Function::Create(FnTy, getThreadLocalWrapperLinkage(VD, CGM),
  ...
  if (!Wrapper->hasLocalLinkage())
    if (!isThreadWrapperReplaceable(VD, CGM) ||
        toolchain::GlobalVariable::isLinkOnceLinkage(Wrapper->getLinkage()) ||
        toolchain::GlobalVariable::isWeakODRLinkage(Wrapper->getLinkage()) ||
        VD->getVisibility() == HiddenVisibility)
      Wrapper->setVisibility(toolchain::GlobalValue::HiddenVisibility);

  if (isThreadWrapperReplaceable(VD, CGM)) {
    Wrapper->setCallingConv(toolchain::CallingConv::CXX_FAST_TLS);
    Wrapper->addFnAttr(toolchain::Attribute::NoUnwind);
  }
...
void ItaniumCXXABI::EmitThreadLocalInitFuncs(
  ...
  toolchain::Function *InitFunc = nullptr;

  // Separate initializers into those with ordered (or partially-ordered)
  // initialization and those with unordered initialization.
  toolchain::SmallVector<toolchain::Function *, 8> OrderedInits;
  toolchain::SmallDenseMap<const VarDecl *, toolchain::Function *> UnorderedInits;
  ...
  if (!OrderedInits.empty()) {
    ...
    InitFunc = CGM.CreateGlobalInitOrCleanUpFunction(FTy, "__tls_init", FI,
                                                     SourceLocation(),
                                                     /*TLS=*/true);
    toolchain::GlobalVariable *Guard = new toolchain::GlobalVariable(
        CGM.getModule(), CGM.Int8Ty, /*isConstant=*/false,
        toolchain::GlobalVariable::InternalLinkage,
        toolchain::ConstantInt::get(CGM.Int8Ty, 0), "__tls_guard");
    ...
    if (CGM.getTarget().getTriple().isOSDarwin()) {
      InitFunc->setCallingConv(toolchain::CallingConv::CXX_FAST_TLS);
      InitFunc->addFnAttr(toolchain::Attribute::NoUnwind);
    }
  }
  ...
  for (const VarDecl *VD : CXXThreadLocals) {
    if (VD->hasDefinition() &&
      ...
      toolchain::GlobalValue *GV = CGM.GetGlobalValue(CGM.getMangledName(VD));
  ...
  for (auto VDAndWrapper : ThreadWrappers) {
    ...
    toolchain::GlobalVariable *Var =
        cast<toolchain::GlobalVariable>(CGM.GetGlobalValue(CGM.getMangledName(VD)));
    toolchain::Function *Wrapper = VDAndWrapper.second;
    ...
    if (!VD->hasDefinition()) {
      if (isThreadWrapperReplaceable(VD, CGM)) {
        Wrapper->setLinkage(toolchain::Function::ExternalLinkage);
        continue;
      }

      // If this isn't a TU in which this variable is defined, the thread
      // wrapper is discardable.
      if (Wrapper->getLinkage() == toolchain::Function::WeakODRLinkage)
        Wrapper->setLinkage(toolchain::Function::LinkOnceODRLinkage);
    }
    ...
    {
      toolchain::raw_svector_ostream Out(InitFnName);
      getMangleContext().mangleItaniumThreadLocalInit(VD, Out);
    }

    toolchain::FunctionType *InitFnTy = toolchain::FunctionType::get(CGM.VoidTy, false);

    // If we have a definition for the variable, emit the initialization
    // function as an alias to the global Init function (if any). Otherwise,
    // produce a declaration of the initialization function.
    toolchain::GlobalValue *Init = nullptr;
    ...
    if (!usesThreadWrapperFunction(VD)) {
      ...
      InitIsInitFunc = true;
      toolchain::Function *InitFuncToUse = InitFunc;
      if (isTemplateInstantiation(VD->getTemplateSpecializationKind()))
        InitFuncToUse = UnorderedInits.lookup(VD->getCanonicalDecl());
      if (InitFuncToUse)
        Init = toolchain::GlobalAlias::create(Var->getLinkage(), InitFnName.str(),
                                         InitFuncToUse);
    } else {
      ...
      Init = toolchain::Function::Create(InitFnTy,
                                    toolchain::GlobalVariable::ExternalWeakLinkage,
                                    InitFnName.str(), &CGM.getModule());
      const CGFunctionInfo &FI = CGM.getTypes().arrangeNullaryFunction();
      CGM.SetLLVMFunctionAttributes(
          GlobalDecl(), FI, cast<toolchain::Function>(Init), /*IsThunk=*/false);
    }
    ...
    toolchain::LLVMContext &Context = CGM.getModule().getContext();
    ...
    if (CGM.getTriple().isOSAIX() && VD->hasDefinition() &&
      ...
      toolchain::Function *Func = toolchain::Function::Create(
          InitFnTy, Var->getLinkage(), InitFnName.str(), &CGM.getModule());
      const CGFunctionInfo &FI = CGM.getTypes().arrangeNullaryFunction();
      CGM.SetLLVMFunctionAttributes(GlobalDecl(), FI,
                                    cast<toolchain::Function>(Func),
                                    /*IsThunk=*/false);
      // Create a function body that just returns
      toolchain::BasicBlock *Entry = toolchain::BasicBlock::Create(Context, "", Func);
      CGBuilderTy Builder(CGM, Entry);
      Builder.CreateRetVoid();
    }

    toolchain::BasicBlock *Entry = toolchain::BasicBlock::Create(Context, "", Wrapper);
    CGBuilderTy Builder(CGM, Entry);
    if (HasConstantInitialization) {
      // No dynamic initialization to invoke.
    } else if (InitIsInitFunc) {
      if (Init) {
        toolchain::CallInst *CallVal = Builder.CreateCall(InitFnTy, Init);
        if (isThreadWrapperReplaceable(VD, CGM)) {
          CallVal->setCallingConv(toolchain::CallingConv::CXX_FAST_TLS);
          toolchain::Function *Fn =
              cast<toolchain::Function>(cast<toolchain::GlobalAlias>(Init)->getAliasee());
          Fn->setCallingConv(toolchain::CallingConv::CXX_FAST_TLS);
        }
      }
      ...
      toolchain::Value *Have = Builder.CreateIsNotNull(Init);
      toolchain::BasicBlock *InitBB = toolchain::BasicBlock::Create(Context, "", Wrapper);
      toolchain::BasicBlock *ExitBB = toolchain::BasicBlock::Create(Context, "", Wrapper);
    ...
    toolchain::Value *Val = Builder.CreateThreadLocalAddress(Var);
...
LValue ItaniumCXXABI::EmitThreadLocalVarDeclLValue(CodeGenFunction &CGF,
                                                   const VarDecl *VD,
                                                   QualType LValType) {
  toolchain::Value *Val = CGF.CGM.GetAddrOfGlobalVar(VD);
  toolchain::Function *Wrapper = getOrCreateThreadLocalWrapper(VD, Val);

  toolchain::CallInst *CallVal = CGF.Builder.CreateCall(Wrapper);
...
ItaniumCXXABI::getOrCreateVirtualFunctionPointerThunk(const CXXMethodDecl *MD) {
  ...
  toolchain::GlobalValue::LinkageTypes Linkage =
      MD->isExternallyVisible() ? toolchain::GlobalValue::LinkOnceODRLinkage
                                : toolchain::GlobalValue::InternalLinkage;
  ThunkFn =
      toolchain::Function::Create(ThunkTy, Linkage, ThunkName, &CGM.getModule());
  if (Linkage == toolchain::GlobalValue::LinkOnceODRLinkage)
    ThunkFn->setVisibility(toolchain::GlobalValue::HiddenVisibility);
  ...
  ThunkFn->removeFnAttr(toolchain::Attribute::StackProtect);
  ThunkFn->removeFnAttr(toolchain::Attribute::StackProtectStrong);
  ThunkFn->removeFnAttr(toolchain::Attribute::StackProtectReq);
  ...
  toolchain::Value *ThisVal = loadIncomingCXXThis(CGF);
  ...
  auto *Call = cast<toolchain::CallInst>(CallOrInvoke);
  Call->setTailCallKind(toolchain::CallInst::TCK_MustTail);
...
class ItaniumRTTIBuilder {
  ...
  SmallVector<toolchain::Constant *, 16> Fields;

  /// GetAddrOfTypeName - Returns the mangled type name of the given type.
  toolchain::GlobalVariable *
  GetAddrOfTypeName(QualType Ty, toolchain::GlobalVariable::LinkageTypes Linkage);

  /// GetAddrOfExternalRTTIDescriptor - Returns the constant for the RTTI
  /// descriptor of the given type.
  toolchain::Constant *GetAddrOfExternalRTTIDescriptor(QualType Ty);

  /// BuildVTablePointer - Build the vtable pointer for the given type.
  void BuildVTablePointer(const Type *Ty, toolchain::Constant *StorageAddress);
...
public:
  ...
  toolchain::Constant *BuildTypeInfo(QualType Ty);

  /// BuildTypeInfo - Build the RTTI type info struct for the given type.
  toolchain::Constant *BuildTypeInfo(
      QualType Ty,
      toolchain::GlobalVariable::LinkageTypes Linkage,
      toolchain::GlobalValue::VisibilityTypes Visibility,
      toolchain::GlobalValue::DLLStorageClassTypes DLLStorageClass);
};
}

toolchain::GlobalVariable *ItaniumRTTIBuilder::GetAddrOfTypeName(
    QualType Ty, toolchain::GlobalVariable::LinkageTypes Linkage) {
  SmallString<256> Name;
  toolchain::raw_svector_ostream Out(Name);
  ...
  toolchain::Constant *Init = toolchain::ConstantDataArray::getString(VMContext,
                                                            Name.substr(4));
...
ItaniumRTTIBuilder::GetAddrOfExternalRTTIDescriptor(QualType Ty) {
  ...
  toolchain::GlobalVariable *GV = CGM.getModule().getNamedGlobal(Name);
  ...
  if (!GV) {
    ...
    GV = new toolchain::GlobalVariable(
        CGM.getModule(), CGM.GlobalsInt8PtrTy,
        /*isConstant=*/true, toolchain::GlobalValue::ExternalLinkage, nullptr, Name);
...
#include "language/Core/AST/BuiltinTypes.def"
      toolchain_unreachable("asking for RRTI for a placeholder type!");

    case BuiltinType::ObjCId:
    case BuiltinType::ObjCClass:
    case BuiltinType::ObjCSel:
      toolchain_unreachable("FIXME: Objective-C types are unsupported!");
  }

  toolchain_unreachable("Invalid BuiltinType Kind!");
}
...
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("Non-canonical and dependent types shouldn't get here");

  case Type::LValueReference:
  case Type::RValueReference:
    toolchain_unreachable("References shouldn't get here");

  case Type::Auto:
  case Type::DeducedTemplateSpecialization:
    toolchain_unreachable("Undeduced type shouldn't get here");

  case Type::Pipe:
    toolchain_unreachable("Pipe types shouldn't get here");

  case Type::ArrayParameter:
    toolchain_unreachable("Array Parameter types should not get here.");
  ...
  case Type::HLSLInlineSpirv:
    toolchain_unreachable("HLSL doesn't support virtual functions");
  }

  toolchain::Constant *VTable = nullptr;

  // Check if the alias exists. If it doesn't, then get or create the global.
  if (CGM.getItaniumVTableContext().isRelativeLayout())
    VTable = CGM.getModule().getNamedAlias(VTableName);
  if (!VTable) {
    toolchain::Type *Ty = toolchain::ArrayType::get(CGM.GlobalsInt8PtrTy, 0);
    VTable = CGM.getModule().getOrInsertGlobal(VTableName, Ty);
  }

  CGM.setDSOLocal(cast<toolchain::GlobalValue>(VTable->stripPointerCasts()));
  ...
  if (CGM.getItaniumVTableContext().isRelativeLayout()) {
    ...
    toolchain::Constant *Eight = toolchain::ConstantInt::get(CGM.Int32Ty, 8);
    VTable =
        toolchain::ConstantExpr::getInBoundsGetElementPtr(CGM.Int8Ty, VTable, Eight);
  } else {
    toolchain::Constant *Two = toolchain::ConstantInt::get(PtrDiffTy, 2);
    VTable = toolchain::ConstantExpr::getInBoundsGetElementPtr(CGM.GlobalsInt8PtrTy,
                                                          VTable, Two);
  }
...
static toolchain::GlobalVariable::LinkageTypes getTypeInfoLinkage(CodeGenModule &CGM,
                                                             QualType Ty) {
...
toolchain::Constant *ItaniumRTTIBuilder::BuildTypeInfo(QualType Ty) {
  ...
  toolchain::GlobalVariable *OldGV = CGM.getModule().getNamedGlobal(Name);
  ...
  toolchain::GlobalVariable::LinkageTypes Linkage = getTypeInfoLinkage(CGM, Ty);

  // Give the type_info object and name the formal visibility of the
  // type itself.
  toolchain::GlobalValue::VisibilityTypes toolchainVisibility;
  if (toolchain::GlobalValue::isLocalLinkage(Linkage))
    // If the linkage is local, only default visibility makes sense.
    toolchainVisibility = toolchain::GlobalValue::DefaultVisibility;
  else if (CXXABI.classifyRTTIUniqueness(Ty, Linkage) ==
           ItaniumCXXABI::RUK_NonUniqueHidden)
    toolchainVisibility = toolchain::GlobalValue::HiddenVisibility;
  else
    toolchainVisibility = CodeGenModule::GetLLVMVisibility(Ty->getVisibility());
  ...
  if (auto RD = Ty->getAsCXXRecordDecl()) {
    if ((CGM.getTriple().isWindowsItaniumEnvironment() &&
        ...
        (CGM.shouldMapVisibilityToDLLExport(RD) &&
         !toolchain::GlobalValue::isLocalLinkage(Linkage) &&
         toolchainVisibility == toolchain::GlobalValue::DefaultVisibility))
      DLLStorageClass = toolchain::GlobalValue::DLLExportStorageClass;
  }
  return BuildTypeInfo(Ty, Linkage, toolchainVisibility, DLLStorageClass);
}

toolchain::Constant *ItaniumRTTIBuilder::BuildTypeInfo(
      QualType Ty,
      toolchain::GlobalVariable::LinkageTypes Linkage,
      toolchain::GlobalValue::VisibilityTypes Visibility,
      toolchain::GlobalValue::DLLStorageClassTypes DLLStorageClass) {
  ...
  toolchain::GlobalVariable *OldGV = M.getNamedGlobal(Name);
  // int8 is an arbitrary type to be replaced later with replaceInitializer.
  toolchain::GlobalVariable *GV =
      new toolchain::GlobalVariable(M, CGM.Int8Ty, /*isConstant=*/true, Linkage,
  ...
  // And the name.
  toolchain::GlobalVariable *TypeName = GetAddrOfTypeName(Ty, Linkage);
  toolchain::Constant *TypeNameField;
  ...
  if (RTTIUniqueness != ItaniumCXXABI::RUK_Unique) {
    ...
    TypeNameField = toolchain::ConstantExpr::getPtrToInt(TypeName, CGM.Int64Ty);
    toolchain::Constant *flag =
        toolchain::ConstantInt::get(CGM.Int64Ty, ((uint64_t)1) << 63);
    TypeNameField = toolchain::ConstantExpr::getAdd(TypeNameField, flag);
    TypeNameField =
        toolchain::ConstantExpr::getIntToPtr(TypeNameField, CGM.GlobalsInt8PtrTy);
  } else {
...
#include "language/Core/AST/TypeNodes.inc"
  ...
  case Type::HLSLAttributedResource:
  case Type::HLSLInlineSpirv:
    toolchain_unreachable("HLSL doesn't support RTTI");
  }

  GV->replaceInitializer(toolchain::ConstantStruct::getAnon(Fields));

  // Export the typeinfo in the same circumstances as the vtable is exported.
  auto GVDLLStorageClass = DLLStorageClass;
  if (CGM.getTarget().hasPS4DLLImportExport() &&
      GVDLLStorageClass != toolchain::GlobalVariable::DLLExportStorageClass) {
    if (const RecordType *RecordTy = dyn_cast<RecordType>(Ty)) {
...
void ItaniumRTTIBuilder::BuildObjCObjectTypeInfo(const ObjCObjectType *OT) {
  ...
  toolchain::Constant *BaseTypeInfo =
...
namespace {
  ...
  /// a class hierarchy.
  struct SeenBases {
    toolchain::SmallPtrSet<const CXXRecordDecl *, 16> NonVirtualBases;
    toolchain::SmallPtrSet<const CXXRecordDecl *, 16> VirtualBases;
  };
}
...
void ItaniumRTTIBuilder::BuildVMIClassTypeInfo(const CXXRecordDecl *RD) {
  ...
  Fields.push_back(toolchain::ConstantInt::get(UnsignedIntLTy, Flags));

  // Itanium C++ ABI 2.9.5p6c:
  //   __base_count is a word with the number of direct proper base class
  //   descriptions that follow.
  Fields.push_back(toolchain::ConstantInt::get(UnsignedIntLTy, RD->getNumBases()));
  ...
  for (const auto &Base : RD->bases()) {
    ...
    Fields.push_back(toolchain::ConstantInt::get(OffsetFlagsLTy, OffsetFlags));
  }
}
...
void ItaniumRTTIBuilder::BuildPointerTypeInfo(QualType PointeeTy) {
  ...
  toolchain::Constant *PointeeTypeInfo =
...
toolchain::Constant *ItaniumCXXABI::getAddrOfRTTIDescriptor(QualType Ty) {
  ...
  for (const QualType &FundamentalType : FundamentalTypes) {
    ...
    for (QualType Type : {FundamentalType, PointerType, PointerTypeConst})
      ItaniumRTTIBuilder(*this).BuildTypeInfo(
          Type, toolchain::GlobalValue::ExternalLinkage,
...
ItaniumCXXABI::RTTIUniquenessKind ItaniumCXXABI::classifyRTTIUniqueness(
    QualType CanTy, toolchain::GlobalValue::LinkageTypes Linkage) const {
  ...
  assert(Linkage == toolchain::GlobalValue::WeakODRLinkage);
...
static StructorCodegen getCodegenToUse(CodeGenModule &CGM,
  ...
  toolchain::GlobalValue::LinkageTypes Linkage = CGM.getFunctionLinkage(AliasDecl);
...
static void emitConstructorDestructorAlias(CodeGenModule &CGM,
  ...
  toolchain::GlobalValue *Entry = CGM.GetGlobalValue(MangledName);
  if (Entry && !Entry->isDeclaration())
    return;

  auto *Aliasee = cast<toolchain::GlobalValue>(CGM.GetAddrOfGlobal(TargetDecl));

  // Create the alias with no name.
  auto *Alias = toolchain::GlobalAlias::create(Linkage, "", Aliasee);
...
void ItaniumCXXABI::emitCXXStructor(GlobalDecl GD) {
  ...
  if (CGType == StructorCodegen::COMDAT) {
    ...
    toolchain::Comdat *C = CGM.getModule().getOrInsertComdat(Out.str());
    Fn->setComdat(C);
  } else {
...
static toolchain::FunctionCallee getBeginCatchFn(CodeGenModule &CGM) {
  // void *__cxa_begin_catch(void*);
  toolchain::FunctionType *FTy = toolchain::FunctionType::get(
      CGM.Int8PtrTy, CGM.Int8PtrTy, /*isVarArg=*/false);

  return CGM.CreateRuntimeFunction(FTy, "__cxa_begin_catch");
}

static toolchain::FunctionCallee getEndCatchFn(CodeGenModule &CGM) {
...
static toolchain::FunctionCallee getGetExceptionPtrFn(CodeGenModule &CGM) {
...
static toolchain::Value *CallBeginCatch(CodeGenFunction &CGF,
                                   toolchain::Value *Exn,
                                   bool EndMightThrow) {
...
static void InitCatchParam(CodeGenFunction &CGF,
  ...
  toolchain::Value *Exn = CGF.getExceptionFromSlot();

  CanQualType CatchType =
    CGF.CGM.getContext().getCanonicalType(CatchParam.getType());
  toolchain::Type *LLVMCatchTy = CGF.ConvertTypeForMem(CatchType);
  ...
  if (isa<ReferenceType>(CatchType)) {
    ...
    toolchain::Value *AdjustedExn = CallBeginCatch(CGF, Exn, EndCatchMightThrow);
    ...
    if (const PointerType *PT = dyn_cast<PointerType>(CaughtType)) {
      ...
      // pad.  The best solution is to fix the personality function.
        ...
        toolchain::Type *PtrTy = CGF.ConvertTypeForMem(CaughtType);

        // Create the temporary and write the adjusted pointer into it.
        Address ExnPtrTmp =
          CGF.CreateTempAlloca(PtrTy, CGF.getPointerAlign(), "exn.byref.tmp");
        toolchain::Value *Casted = CGF.Builder.CreateBitCast(AdjustedExn, PtrTy);
  ...
  if (TEK != TEK_Aggregate) {
    toolchain::Value *AdjustedExn = CallBeginCatch(CGF, Exn, false);

    // If the catch type is a pointer type, __cxa_begin_catch returns
    // the pointer by value.
    if (CatchType->hasPointerRepresentation()) {
      toolchain::Value *CastExn =
      ...
      toolchain_unreachable("bad ownership qualifier!");
    }
    ...
    case TEK_Complex:
      ...
      return;
    case TEK_Scalar: {
      toolchain::Value *ExnLoad = CGF.EmitLoadOfScalar(srcLV, Loc);
      CGF.EmitStoreOfScalar(ExnLoad, destLV, /*init*/ true);
      return;
    }
    case TEK_Aggregate:
      toolchain_unreachable("evaluation kind filtered out!");
    }
    toolchain_unreachable("bad evaluation kind");
  }
  ...
  toolchain::Type *PtrTy = CGF.UnqualPtrTy; // addrspace 0 ok

  // Check for a copy expression.  If we don't have a copy expression,
  // that means a trivial copy is okay.
  const Expr *copyExpr = CatchParam.getInit();
  if (!copyExpr) {
    toolchain::Value *rawAdjustedExn = CallBeginCatch(CGF, Exn, true);
  ...
  toolchain::CallInst *rawAdjustedExn =
...
static toolchain::FunctionCallee getClangCallTerminateFn(CodeGenModule &CGM) {
  ASTContext &C = CGM.getContext();
  const CGFunctionInfo &FI = CGM.getTypes().arrangeBuiltinFunctionDeclaration(
      C.VoidTy, {C.getPointerType(C.CharTy)});
  toolchain::FunctionType *fnTy = CGM.getTypes().GetFunctionType(FI);
  toolchain::FunctionCallee fnRef = CGM.CreateRuntimeFunction(
      fnTy, "__clang_call_terminate", toolchain::AttributeList(), /*Local=*/true);
  toolchain::Function *fn =
      cast<toolchain::Function>(fnRef.getCallee()->stripPointerCasts());
  if (fn->empty()) {
    ...
    fn->setLinkage(toolchain::Function::LinkOnceODRLinkage);
    fn->setVisibility(toolchain::Function::HiddenVisibility);
    if (CGM.supportsCOMDAT())
      fn->setComdat(CGM.getModule().getOrInsertComdat(fn->getName()));

    // Set up the function.
    toolchain::BasicBlock *entry =
        toolchain::BasicBlock::Create(CGM.getLLVMContext(), "", fn);
    CGBuilderTy builder(CGM, entry);

    // Pull the exception pointer out of the parameter list.
    toolchain::Value *exn = &*fn->arg_begin();

    // Call __cxa_begin_catch(exn).
    toolchain::CallInst *catchCall = builder.CreateCall(getBeginCatchFn(CGM), exn);
    catchCall->setDoesNotThrow();
    catchCall->setCallingConv(CGM.getRuntimeCC());

    // Call std::terminate().
    toolchain::CallInst *termCall = builder.CreateCall(CGM.getTerminateFn());
...
ItaniumCXXABI::getSignedVirtualMemberFunctionPointer(const CXXMethodDecl *MD) {
  const CXXMethodDecl *origMD =
      cast<CXXMethodDecl>(CGM.getItaniumVTableContext()
                              ...
                              .getDecl());
  toolchain::Constant *thunk = getOrCreateVirtualFunctionPointerThunk(origMD);
...
void WebAssemblyCXXABI::emitBeginCatch(CodeGenFunction &CGF,
  ...
  if (CGF.getTarget().hasFeature("exception-handling"))
    CGF.EHStack.pushCleanup<CatchRetScope>(
        NormalCleanup, cast<toolchain::CatchPadInst>(CGF.CurrentFuncletPad));
...
void XLCXXABI::registerGlobalDtor(CodeGenFunction &CGF, const VarDecl &D,
  ...
  if (D.getTLSKind() != VarDecl::TLS_None) {
    toolchain::PointerType *PtrTy = CGF.UnqualPtrTy;

    // extern "C" int __pt_atexit_np(int flags, int(*)(int,...), ...);
    toolchain::FunctionType *AtExitTy =
        toolchain::FunctionType::get(CGM.IntTy, {CGM.IntTy, PtrTy}, true);
    ...
    toolchain::Function *DtorStub = CGF.createTLSAtExitStub(D, Dtor, Addr, AtExit);

    // Register above __dtor with atexit().
    // First param is flags and must be 0, second param is function ptr
    toolchain::Value *NV = toolchain::Constant::getNullValue(CGM.IntTy);
  ...
  // Create __dtor function for the var decl.
  toolchain::Function *DtorStub =
      cast<toolchain::Function>(CGF.createAtExitStub(D, Dtor, Addr));
...
void XLCXXABI::emitCXXStermFinalizer(const VarDecl &D, toolchain::Function *dtorStub,
                                     toolchain::Constant *addr) {
  toolchain::FunctionType *FTy = toolchain::FunctionType::get(CGM.VoidTy, false);
  SmallString<256> FnName;
  {
    toolchain::raw_svector_ostream Out(FnName);
    getMangleContext().mangleDynamicStermFinalizer(&D, Out);
  }

  // Create the finalization action associated with a variable.
  const CGFunctionInfo &FI = CGM.getTypes().arrangeNullaryFunction();
  toolchain::Function *StermFinalizer = CGM.CreateGlobalInitOrCleanUpFunction(
  ...
  toolchain::Value *V = CGF.unregisterGlobalDtorWithUnAtExit(dtorStub);

  toolchain::Value *NeedsDestruct = CGF.Builder.CreateIsNull(V, "needs_destruct");

  toolchain::BasicBlock *DestructCallBlock = CGF.createBasicBlock("destruct.call");
  toolchain::BasicBlock *EndBlock = CGF.createBasicBlock("destruct.end");
  ...
  toolchain::CallInst *CI = CGF.Builder.CreateCall(dtorStub);
  ...
  if (auto *IPA = D.getAttr<InitPriorityAttr>()) {
    ...
    // their own toolchain.global_dtors entry.
