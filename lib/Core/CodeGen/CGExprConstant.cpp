...
toolchain::Constant *getPadding(const CodeGenModule &CGM, CharUnits PadSize) {
  toolchain::Type *Ty = CGM.CharTy;
  if (PadSize > CharUnits::One())
    Ty = toolchain::ArrayType::get(Ty, PadSize.getQuantity());
  if (CGM.shouldZeroInitPadding()) {
    return toolchain::Constant::getNullValue(Ty);
  }
  return toolchain::UndefValue::get(Ty);
}
...
struct ConstantAggregateBuilderUtils {
  ...
  CharUnits getAlignment(const toolchain::Constant *C) const {
    return CharUnits::fromQuantity(
        CGM.getDataLayout().getABITypeAlign(C->getType()));
  }

  CharUnits getSize(toolchain::Type *Ty) const {
    return CharUnits::fromQuantity(CGM.getDataLayout().getTypeAllocSize(Ty));
  }

  CharUnits getSize(const toolchain::Constant *C) const {
    return getSize(C->getType());
  }

  toolchain::Constant *getPadding(CharUnits PadSize) const {
    return ::getPadding(CGM, PadSize);
  }

  toolchain::Constant *getZeroes(CharUnits ZeroSize) const {
    toolchain::Type *Ty = toolchain::ArrayType::get(CGM.CharTy, ZeroSize.getQuantity());
    return toolchain::ConstantAggregateZero::get(Ty);
  }
};

/// Incremental builder for an toolchain::Constant* holding a struct or array
/// constant.
class ConstantAggregateBuilder : private ConstantAggregateBuilderUtils {
  ...
  toolchain::SmallVector<toolchain::Constant*, 32> Elems;
  toolchain::SmallVector<CharUnits, 32> Offsets;
  ...
  static toolchain::Constant *buildFrom(CodeGenModule &CGM,
                                   ArrayRef<toolchain::Constant *> Elems,
                                   ArrayRef<CharUnits> Offsets,
                                   CharUnits StartOffset, CharUnits Size,
                                   bool NaturalLayout, toolchain::Type *DesiredTy,
                                   bool AllowOversized);
...
public:
  ...
  ///        detect bugs.
  bool add(toolchain::Constant *C, CharUnits Offset, bool AllowOverwrite);

  /// Update or overwrite the bits starting at \p OffsetInBits with \p Bits.
  bool addBits(toolchain::APInt Bits, uint64_t OffsetInBits, bool AllowOverwrite);

  /// Attempt to condense the value starting at \p Offset to a constant of type
  /// \p DesiredTy.
  void condense(CharUnits Offset, toolchain::Type *DesiredTy);
  ...
  toolchain::Constant *build(toolchain::Type *DesiredTy, bool AllowOversized) const {
...
static void replace(Container &C, size_t BeginOff, size_t EndOff, Range Vals) {
  ...
  toolchain::replace(C, C.begin() + BeginOff, C.begin() + EndOff, Vals);
}

bool ConstantAggregateBuilder::add(toolchain::Constant *C, CharUnits Offset,
                          bool AllowOverwrite) {
...
bool ConstantAggregateBuilder::addBits(toolchain::APInt Bits, uint64_t OffsetInBits,
                              bool AllowOverwrite) {
  ...
  for (CharUnits OffsetInChars =
    ...
    toolchain::APInt BitsThisChar = Bits;
    ...
    if (WantedBits == CharWidth) {
      ...
      add(toolchain::ConstantInt::get(CGM.getLLVMContext(), BitsThisChar),
      ...
      toolchain::APInt UpdateMask(CharWidth, 0);
      if (CGM.getDataLayout().isBigEndian())
      ...
      if (*FirstElemToUpdate == *LastElemToUpdate ||
        ...
        ToUpdate = toolchain::ConstantInt::get(CGM.getLLVMContext(), BitsThisChar);
      }
    }
...
std::optional<size_t> ConstantAggregateBuilder::splitAt(CharUnits Pos) {
  ...
  while (true) {
    auto FirstAfterPos = toolchain::upper_bound(Offsets, Pos);
...
bool ConstantAggregateBuilder::split(size_t Index, CharUnits Hint) {
  NaturalLayout = false;
  toolchain::Constant *C = Elems[Index];
  CharUnits Offset = Offsets[Index];

  if (auto *CA = dyn_cast<toolchain::ConstantAggregate>(C)) {
    // Expand the sequence into its contained elements.
    // FIXME: This assumes vector elements are byte-sized.
    replace(Elems, Index, Index + 1,
            toolchain::map_range(toolchain::seq(0u, CA->getNumOperands()),
                            [&](unsigned Op) { return CA->getOperand(Op); }));
    if (isa<toolchain::ArrayType>(CA->getType()) ||
        isa<toolchain::VectorType>(CA->getType())) {
      // Array or vector.
      toolchain::Type *ElemTy =
          toolchain::GetElementPtrInst::getTypeAtIndex(CA->getType(), (uint64_t)0);
      ...
      // Must be a struct.
      auto *ST = cast<toolchain::StructType>(CA->getType());
      const toolchain::StructLayout *Layout =
          CGM.getDataLayout().getStructLayout(ST);
      replace(Offsets, Index, Index + 1,
              toolchain::map_range(
                  toolchain::seq(0u, CA->getNumOperands()), [&](unsigned Op) {
  ...
  if (auto *CDS = dyn_cast<toolchain::ConstantDataSequential>(C)) {
    // Expand the sequence into its contained elements.
    // FIXME: This assumes vector elements are byte-sized.
    // FIXME: If possible, split into two ConstantDataSequentials at Hint.
    CharUnits ElemSize = getSize(CDS->getElementType());
    replace(Elems, Index, Index + 1,
            toolchain::map_range(toolchain::seq(uint64_t(0u), CDS->getNumElements()),
                            [&](uint64_t Elem) {
                              return CDS->getElementAsConstant(Elem);
                            }));
    replace(Offsets, Index, Index + 1,
            toolchain::map_range(
                toolchain::seq(uint64_t(0u), CDS->getNumElements()),
                [&](uint64_t Elem) { return Offset + Elem * ElemSize; }));
    return true;
  }

  if (isa<toolchain::ConstantAggregateZero>(C)) {
...
EmitArrayConstant(CodeGenModule &CGM, toolchain::ArrayType *DesiredType,
                  toolchain::Type *CommonElementType, uint64_t ArrayBound,
                  SmallVectorImpl<toolchain::Constant *> &Elements,
                  toolchain::Constant *Filler);

toolchain::Constant *ConstantAggregateBuilder::buildFrom(
    CodeGenModule &CGM, ArrayRef<toolchain::Constant *> Elems,
    ArrayRef<CharUnits> Offsets, CharUnits StartOffset, CharUnits Size,
    bool NaturalLayout, toolchain::Type *DesiredTy, bool AllowOversized) {
  ConstantAggregateBuilderUtils Utils(CGM);

  if (Elems.empty())
    return toolchain::UndefValue::get(DesiredTy);

  auto Offset = [&](size_t I) { return Offsets[I] - StartOffset; };

  // If we want an array type, see if all the elements are the same type and
  // appropriately spaced.
  if (toolchain::ArrayType *ATy = dyn_cast<toolchain::ArrayType>(DesiredTy)) {
    assert(!AllowOversized && "oversized array emission not supported");

    bool CanEmitArray = true;
    toolchain::Type *CommonType = Elems[0]->getType();
    toolchain::Constant *Filler = toolchain::Constant::getNullValue(CommonType);
    CharUnits ElemSize = Utils.getSize(ATy->getElementType());
    SmallVector<toolchain::Constant*, 32> ArrayElements;
  ...
  for (toolchain::Constant *C : Elems)
  ...
  bool Packed = false;
  ArrayRef<toolchain::Constant*> UnpackedElems = Elems;
  toolchain::SmallVector<toolchain::Constant*, 32> UnpackedElemStorage;
  ...
  toolchain::SmallVector<toolchain::Constant*, 32> PackedElems;
  if (!NaturalLayout) {
  ...
  toolchain::StructType *STy = toolchain::ConstantStruct::getTypeForElements(
  ...
  if (toolchain::StructType *DesiredSTy = dyn_cast<toolchain::StructType>(DesiredTy)) {
    if (DesiredSTy->isLayoutIdentical(STy))
      STy = DesiredSTy;
  }

  return toolchain::ConstantStruct::get(STy, Packed ? PackedElems : UnpackedElems);
}

void ConstantAggregateBuilder::condense(CharUnits Offset,
                                        toolchain::Type *DesiredTy) {
  ...
  if (Length == 1 && Offsets[First] == Offset &&
    ...
    auto *STy = dyn_cast<toolchain::StructType>(DesiredTy);
    if (STy && STy->getNumElements() == 1 &&
        STy->getElementType(0) == Elems[First]->getType())
      Elems[First] = toolchain::ConstantStruct::get(STy, Elems[First]);
    return;
  }

  toolchain::Constant *Replacement = buildFrom(
...
public:
  static toolchain::Constant *BuildStruct(ConstantEmitter &Emitter,
...
private:
  ...
  bool AppendField(const FieldDecl *Field, uint64_t FieldOffset,
                   toolchain::Constant *InitExpr, bool AllowOverwrite = false);

  bool AppendBytes(CharUnits FieldOffsetInChars, toolchain::Constant *InitCst,
  ...
  bool DoZeroInitPadding(const ASTRecordLayout &Layout, bool AllowOverwrite,
                         CharUnits SizeSoFar);
  toolchain::Constant *Finalize(QualType Ty);
};

bool ConstStructBuilder::AppendField(
    const FieldDecl *Field, uint64_t FieldOffset, toolchain::Constant *InitCst,
    bool AllowOverwrite) {
  const ASTContext &Context = CGM.getContext();
...
bool ConstStructBuilder::AppendBytes(CharUnits FieldOffsetInChars,
                                     toolchain::Constant *InitCst,
                                     bool AllowOverwrite) {
  return Builder.add(InitCst, StartOffset + FieldOffsetInChars, AllowOverwrite);
}

bool ConstStructBuilder::AppendBitField(const FieldDecl *Field,
                                        uint64_t FieldOffset, toolchain::Constant *C,
                                        bool AllowOverwrite) {

  toolchain::ConstantInt *CI = dyn_cast<toolchain::ConstantInt>(C);
  if (!CI) {
    ...
    toolchain::Type *LoadType =
        CGM.getTypes().convertTypeForLoadStore(Field->getType(), C->getType());
    toolchain::Constant *FoldedConstant = toolchain::ConstantFoldLoadFromConst(
        C, LoadType, toolchain::APInt::getZero(32), CGM.getDataLayout());
    CI = dyn_cast_if_present<toolchain::ConstantInt>(FoldedConstant);
    if (!CI)
      return false;
  }
  ...
  toolchain::APInt FieldValue = CI->getValue();
...
static bool EmitDesignatedInitUpdater(ConstantEmitter &Emitter,
  ...
  toolchain::Type *ElemTy = Emitter.CGM.getTypes().ConvertTypeForMem(ElemType);

  toolchain::Constant *FillC = nullptr;
  ...
  for (unsigned I = 0; I != NumElementsToUpdate; ++I, Offset += ElemSize) {
    ...
    if (!Init && FillC) {
      ...
      toolchain::Constant *Val = Emitter.tryEmitPrivateForMemory(Init, ElemType);
...
bool ConstStructBuilder::Build(const APValue &Val, const RecordDecl *RD,
  ...
  if (const CXXRecordDecl *CD = dyn_cast<CXXRecordDecl>(RD)) {
    ...
    if (Layout.hasOwnVFPtr()) {
      toolchain::Constant *VTableAddressPoint =
...
toolchain::Constant *ConstStructBuilder::Finalize(QualType Type) {
  Type = Type.getNonReferenceType();
  RecordDecl *RD =
      Type->castAs<RecordType>()->getOriginalDecl()->getDefinitionOrSelf();
  toolchain::Type *ValTy = CGM.getTypes().ConvertType(Type);
  return Builder.build(ValTy, RD->hasFlexibleArrayMember());
}

toolchain::Constant *ConstStructBuilder::BuildStruct(ConstantEmitter &Emitter,
                                                const InitListExpr *ILE,
                                                QualType ValTy) {
...
tryEmitGlobalCompoundLiteral(ConstantEmitter &emitter,
  ...
  if (toolchain::GlobalVariable *Addr =
          CGM.getAddrOfConstantCompoundLiteralIfEmitted(E))
    return ConstantAddress(Addr, Addr->getValueType(), Align);

  LangAS addressSpace = E->getType().getAddressSpace();
  toolchain::Constant *C = emitter.tryEmitForInitializer(E->getInitializer(),
  ...
  auto GV = new toolchain::GlobalVariable(
      ...
      toolchain::GlobalValue::InternalLinkage, C, ".compoundliteral", nullptr,
...
EmitArrayConstant(CodeGenModule &CGM, toolchain::ArrayType *DesiredType,
  ...
  if (NonzeroLength == 0)
    return toolchain::ConstantAggregateZero::get(DesiredType);
  ...
  if (TrailingZeroes >= 8) {
    ...
    if (CommonElementType && NonzeroLength >= 8) {
      toolchain::Constant *Initial = toolchain::ConstantArray::get(
          toolchain::ArrayType::get(CommonElementType, NonzeroLength),
    ...
    FillerType = toolchain::ArrayType::get(FillerType, TrailingZeroes);
    Elements.back() = toolchain::ConstantAggregateZero::get(FillerType);
  ...
  if (CommonElementType)
    return toolchain::ConstantArray::get(
        toolchain::ArrayType::get(CommonElementType, ArrayBound), Elements);

  // We have mixed types. Use a packed struct.
  toolchain::SmallVector<toolchain::Type *, 16> Types;
  Types.reserve(Elements.size());
  for (toolchain::Constant *Elt : Elements)
    Types.push_back(Elt->getType());
  toolchain::StructType *SType =
      toolchain::StructType::get(CGM.getLLVMContext(), Types, true);
  return toolchain::ConstantStruct::get(SType, Elements);
}
...
class ConstExprEmitter
    : public ConstStmtVisitor<ConstExprEmitter, toolchain::Constant *, QualType> {
  CodeGenModule &CGM;
  ConstantEmitter &Emitter;
...
public:
  ...
  toolchain::Constant *VisitStmt(const Stmt *S, QualType T) { return nullptr; }

  toolchain::Constant *VisitConstantExpr(const ConstantExpr *CE, QualType T) {
    if (toolchain::Constant *Result = Emitter.tryEmitConstantExpr(CE))
      return Result;
    return Visit(CE->getSubExpr(), T);
  }

  toolchain::Constant *VisitParenExpr(const ParenExpr *PE, QualType T) {
  ...
  toolchain::Constant *VisitGenericSelectionExpr(const GenericSelectionExpr *GE,
                                            QualType T) {
    return Visit(GE->getResultExpr(), T);
  }

  toolchain::Constant *VisitChooseExpr(const ChooseExpr *CE, QualType T) {
    return Visit(CE->getChosenSubExpr(), T);
  }

  toolchain::Constant *VisitCompoundLiteralExpr(const CompoundLiteralExpr *E,
                                           QualType T) {
    return Visit(E->getInitializer(), T);
  }

  toolchain::Constant *ProduceIntToIntCast(const Expr *E, QualType DestType) {
    QualType FromType = E->getType();
    // See also HandleIntToIntCast in ExprConstant.cpp
    if (FromType->isIntegerType())
      if (toolchain::Constant *C = Visit(E, FromType))
        if (auto *CI = dyn_cast<toolchain::ConstantInt>(C)) {
          unsigned SrcWidth = CGM.getContext().getIntWidth(FromType);
          unsigned DstWidth = CGM.getContext().getIntWidth(DestType);
          if (DstWidth == SrcWidth)
            return CI;
          toolchain::APInt A = FromType->isSignedIntegerType()
                              ? CI->getValue().sextOrTrunc(DstWidth)
                              : CI->getValue().zextOrTrunc(DstWidth);
          return toolchain::ConstantInt::get(CGM.getLLVMContext(), A);
        }
    return nullptr;
  }

  toolchain::Constant *VisitCastExpr(const CastExpr *E, QualType destType) {
    ...
    case CK_ToUnion: {
      ...
      SmallVector<toolchain::Constant*, 2> Elts;
      SmallVector<toolchain::Type*, 2> Types;
      Elts.push_back(C);
      ...
      if (unsigned NumPadBytes = TotalSize - CurSize) {
        toolchain::Constant *Padding =
            getPadding(CGM, CharUnits::fromQuantity(NumPadBytes));
        Elts.push_back(Padding);
        Types.push_back(Padding->getType());
      }

      toolchain::StructType *STy = toolchain::StructType::get(VMContext, Types, false);
      return toolchain::ConstantStruct::get(STy, Elts);
    }
    ...
    case CK_AddressSpaceConversion: {
      ...
      toolchain::Type *destTy = ConvertType(E->getType());
    ...
    case CK_IntToOCLSampler:
      toolchain_unreachable("global sampler variables are not generated");

    case CK_IntegralCast:
      return ProduceIntToIntCast(subExpr, destType);

    case CK_Dependent: toolchain_unreachable("saw dependent cast!");
    ...
    toolchain_unreachable("Invalid CastKind");
  }

  toolchain::Constant *VisitCXXDefaultInitExpr(const CXXDefaultInitExpr *DIE,
                                          QualType T) {
  ...
  toolchain::Constant *VisitExprWithCleanups(const ExprWithCleanups *E, QualType T) {
    return Visit(E->getSubExpr(), T);
  }

  toolchain::Constant *VisitIntegerLiteral(const IntegerLiteral *I, QualType T) {
    return toolchain::ConstantInt::get(CGM.getLLVMContext(), I->getValue());
  }

  static APValue withDestType(ASTContext &Ctx, const Expr *E, QualType SrcType,
                              QualType DestType, const toolchain::APSInt &Value) {
    if (!Ctx.hasSameType(SrcType, DestType)) {
      if (DestType->isFloatingType()) {
        toolchain::APFloat Result =
            toolchain::APFloat(Ctx.getFloatTypeSemantics(DestType), 1);
        toolchain::RoundingMode RM =
            E->getFPFeaturesInEffect(Ctx.getLangOpts()).getRoundingMode();
        if (RM == toolchain::RoundingMode::Dynamic)
          RM = toolchain::RoundingMode::NearestTiesToEven;
  ...
  toolchain::Constant *EmitArrayInitialization(const InitListExpr *ILE, QualType T) {
    ...
    SmallVector<toolchain::Constant *, 16> Elts;
    if (fillC && fillC->isNullValue())
      Elts.reserve(NumInitableElts + 1);
    else
      Elts.reserve(NumElements);

    toolchain::Type *CommonElementType = nullptr;
    ...
    for (unsigned i = 0; i < ILE->getNumInits(); ++i) {
      ...
      if (auto *EmbedS = dyn_cast<EmbedExpr>(Init->IgnoreParenImpCasts())) {
        ...
        toolchain::APSInt Value(CGM.getContext().getTypeSize(DestTy),
        ...
        for (unsigned I = EmbedS->getStartingElementPos(),
          ...
          if (DestTy->isIntegerType()) {
            C = toolchain::ConstantInt::get(CGM.getLLVMContext(), Value);
          } else {
    ...
    toolchain::ArrayType *Desired =
        cast<toolchain::ArrayType>(CGM.getTypes().ConvertType(ILE->getType()));
    return EmitArrayConstant(CGM, Desired, CommonElementType, NumElements, Elts,
                             fillC);
  }

  toolchain::Constant *EmitRecordInitialization(const InitListExpr *ILE,
                                           QualType T) {
    return ConstStructBuilder::BuildStruct(Emitter, ILE, T);
  }

  toolchain::Constant *VisitImplicitValueInitExpr(const ImplicitValueInitExpr *E,
                                             QualType T) {
    return CGM.EmitNullConstant(T);
  }

  toolchain::Constant *VisitInitListExpr(const InitListExpr *ILE, QualType T) {
  ...
  VisitDesignatedInitUpdateExpr(const DesignatedInitUpdateExpr *E,
    ...
    toolchain::Type *ValTy = CGM.getTypes().ConvertType(destType);
  ...
  toolchain::Constant *VisitCXXConstructExpr(const CXXConstructExpr *E,
                                        QualType Ty) {
  ...
  toolchain::Constant *VisitStringLiteral(const StringLiteral *E, QualType T) {
    // This is a string literal initializing an array in an initializer.
    return CGM.GetConstantArrayFromStringLiteral(E);
  }

  toolchain::Constant *VisitObjCEncodeExpr(const ObjCEncodeExpr *E, QualType T) {
  ...
  toolchain::Constant *VisitUnaryExtension(const UnaryOperator *E, QualType T) {
    return Visit(E->getSubExpr(), T);
  }

  toolchain::Constant *VisitUnaryMinus(const UnaryOperator *U, QualType T) {
    if (toolchain::Constant *C = Visit(U->getSubExpr(), T))
      if (auto *CI = dyn_cast<toolchain::ConstantInt>(C))
        return toolchain::ConstantInt::get(CGM.getLLVMContext(), -CI->getValue());
    return nullptr;
  }

  toolchain::Constant *VisitPackIndexingExpr(const PackIndexingExpr *E, QualType T) {
...
toolchain::Constant *ConstantEmitter::validateAndPopAbstract(toolchain::Constant *C,
                                                        AbstractState saved) {
...
toolchain::Constant *ConstantEmitter::tryEmitConstantExpr(const ConstantExpr *CE) {
...
toolchain::Constant *ConstantEmitter::tryEmitForInitializer(const VarDecl &D) {
  initializeNonAbstract(D.getType().getAddressSpace());
  return markIfFailed(tryEmitPrivateForVarInit(D));
}

toolchain::Constant *ConstantEmitter::tryEmitForInitializer(const Expr *E,
...
toolchain::Constant *ConstantEmitter::emitForInitializer(const APValue &value,
                                                    LangAS destAddrSpace,
                                                    QualType destType) {
  initializeNonAbstract(destAddrSpace);
...
toolchain::GlobalValue *ConstantEmitter::getCurrentAddrPrivate() {
  ...
  // if it survives.
  auto global = new toolchain::GlobalVariable(CGM.getModule(), CGM.Int8Ty, true,
...
void ConstantEmitter::registerCurrentAddrPrivate(toolchain::Constant *signal,
...
namespace {
  struct ReplacePlaceholders {
    ...
    toolchain::Type *BaseValueTy = nullptr;

    /// The placeholder addresses that were registered during emission.
    toolchain::DenseMap<toolchain::Constant*, toolchain::GlobalVariable*> PlaceholderAddresses;

    /// The locations of the placeholder signals.
    toolchain::DenseMap<toolchain::GlobalVariable*, toolchain::Constant*> Locations;
    ...
    toolchain::SmallVector<unsigned, 8> Indices;
    toolchain::SmallVector<toolchain::Constant*, 8> IndexValues;

    ReplacePlaceholders(CodeGenModule &CGM, toolchain::Constant *base,
                        ArrayRef<std::pair<toolchain::Constant*,
                                           toolchain::GlobalVariable*>> addresses)
        : CGM(CGM), Base(base),
          PlaceholderAddresses(addresses.begin(), addresses.end()) {
    }

    void replaceInInitializer(toolchain::Constant *init) {
      ...
      // messing around with toolchain::Constant structures, which never itself
  ...
  private:
    void findLocations(toolchain::Constant *init) {
      // Recurse into aggregates.
      if (auto agg = dyn_cast<toolchain::ConstantAggregate>(init)) {
      ...
      while (true) {
        ...
        if (auto expr = dyn_cast<toolchain::ConstantExpr>(init)) {
    ...
    void setLocation(toolchain::GlobalVariable *placeholder) {
      ...
      for (size_t i = Indices.size() - 1; i != size_t(-1); --i) {
        if (IndexValues[i]) {
          ...
          for (size_t j = 0; j != i + 1; ++j) {
            assert(IndexValues[j] &&
                   isa<toolchain::ConstantInt>(IndexValues[j]) &&
                   cast<toolchain::ConstantInt>(IndexValues[j])->getZExtValue()
                     == Indices[j]);
          }
#endif
          break;
        ...
        IndexValues[i] = toolchain::ConstantInt::get(CGM.Int32Ty, Indices[i]);
      }

      toolchain::Constant *location = toolchain::ConstantExpr::getInBoundsGetElementPtr(
...
void ConstantEmitter::finalize(toolchain::GlobalVariable *global) {
...
toolchain::Constant *ConstantEmitter::tryEmitPrivateForVarInit(const VarDecl &D) {
  ...
  if (!destType->isReferenceType()) {
    ...
    if (toolchain::Constant *C = ConstExprEmitter(*this).Visit(E, nonMemoryDestType))
...
toolchain::Constant *ConstantEmitter::tryEmitPrivateForMemory(const Expr *E,
                                                         QualType destType) {
  auto nonMemoryDestType = getNonMemoryType(CGM, destType);
  toolchain::Constant *C = tryEmitPrivate(E, nonMemoryDestType);
  return (C ? emitForMemory(C, destType) : nullptr);
}

toolchain::Constant *ConstantEmitter::tryEmitPrivateForMemory(const APValue &value,
                                                         QualType destType) {
  auto nonMemoryDestType = getNonMemoryType(CGM, destType);
...
toolchain::Constant *
ConstantEmitter::tryEmitConstantSignedPointer(toolchain::Constant *UnsignedPointer,
  ...
  toolchain::GlobalValue *StorageAddress = nullptr;
  ...
  toolchain::ConstantInt *Discriminator =
      toolchain::ConstantInt::get(CGM.IntPtrTy, Schema.getExtraDiscriminator());

  toolchain::Constant *SignedPointer = CGM.getConstantSignedPointer(
...
toolchain::Constant *ConstantEmitter::emitForMemory(CodeGenModule &CGM,
                                               toolchain::Constant *C,
                                               QualType destType) {
  ...
  if (auto AT = destType->getAs<AtomicType>()) {
    ...
    toolchain::Constant *elts[] = {
      C,
      toolchain::ConstantAggregateZero::get(
          toolchain::ArrayType::get(CGM.Int8Ty, (outerSize - innerSize) / 8))
    };
    return toolchain::ConstantStruct::getAnon(elts);
  }
  ...
  if ((C->getType()->isIntegerTy(1) && !destType->isBitIntType()) ||
    ...
    toolchain::Type *boolTy = CGM.getTypes().ConvertTypeForMem(destType);
    toolchain::Constant *Res = toolchain::ConstantFoldCastOperand(
        toolchain::Instruction::ZExt, C, boolTy, CGM.getDataLayout());
  ...
  if (destType->isBitIntType()) {
    ...
    toolchain::Type *LoadStoreTy = CGM.getTypes().convertTypeForLoadStore(destType);
    // ptrtoint/inttoptr should not involve _BitInt in constant expressions, so
    // casting to ConstantInt is safe here.
    auto *CI = cast<toolchain::ConstantInt>(C);
    toolchain::Constant *Res = toolchain::ConstantFoldCastOperand(
        destType->isSignedIntegerOrEnumerationType() ? toolchain::Instruction::SExt
                                                     : toolchain::Instruction::ZExt,
    ...
    if (CGM.getTypes().typeRequiresSplitIntoByteArray(destType, C->getType())) {
      ...
      toolchain::Type *DesiredTy = CGM.getTypes().ConvertTypeForMem(destType);
      toolchain::APInt Value = cast<toolchain::ConstantInt>(Res)->getValue();
...
toolchain::Constant *ConstantEmitter::tryEmitPrivate(const Expr *E,
                                                QualType destType) {
  assert(!destType->isVoidType() && "can't emit a void constant");

  if (!destType->isReferenceType())
    if (toolchain::Constant *C = ConstExprEmitter(*this).Visit(E, destType))
      return C;
...
toolchain::Constant *CodeGenModule::getNullPointer(toolchain::PointerType *T, QualType QT) {
...
struct ConstantLValue {
  ...
  /*implicit*/ ConstantLValue(toolchain::Constant *value,
...
public:
  ...
  toolchain::Constant *tryEmit();

private:
  toolchain::Constant *tryEmitAbsolute(toolchain::Type *destTy);
  ...
  toolchain::Constant *emitPointerAuthPointer(const Expr *E);
  unsigned emitPointerAuthKey(const Expr *E);
  std::pair<toolchain::Constant *, toolchain::ConstantInt *>
  ...
  toolchain::Constant *getOffset() {
    return toolchain::ConstantInt::get(CGM.Int64Ty,
                                  Value.getLValueOffset().getQuantity());
  }

  /// Apply the value offset to the given constant.
  toolchain::Constant *applyOffset(toolchain::Constant *C) {
    if (!hasNonZeroOffset())
      return C;

    return toolchain::ConstantExpr::getGetElementPtr(CGM.Int8Ty, C, getOffset());
  }
};

}

toolchain::Constant *ConstantLValueEmitter::tryEmit() {
  ...
  assert(isa<toolchain::IntegerType>(destTy) || isa<toolchain::PointerType>(destTy));
  ...
  toolchain::Constant *value = result.Value;
  ...
  if (isa<toolchain::PointerType>(destTy))
    return toolchain::ConstantExpr::getPointerCast(value, destTy);

  return toolchain::ConstantExpr::getPtrToInt(value, destTy);
}

/// Try to emit an absolute l-value, such as a null pointer or an integer
/// bitcast to pointer type.
toolchain::Constant *
ConstantLValueEmitter::tryEmitAbsolute(toolchain::Type *destTy) {
  // If we're producing a pointer, this is easy.
  auto destPtrTy = cast<toolchain::PointerType>(destTy);
  ...
  toolchain::Constant *C;
  C = toolchain::ConstantFoldIntegerCast(getOffset(), intptrTy, /*isSigned*/ false,
                                    CGM.getDataLayout());
  assert(C && "Must have folded, as Offset is a ConstantInt");
  C = toolchain::ConstantExpr::getIntToPtr(C, destPtrTy);
  return C;
}
...
ConstantLValueEmitter::tryEmitBase(const APValue::LValueBase &base) {
  ...
  if (const ValueDecl *D = base.dyn_cast<const ValueDecl*>()) {
    ...
    auto PtrAuthSign = [&](toolchain::Constant *C) {
      if (PointerAuthQualifier PointerAuth = DestType.getPointerAuth()) {
        C = applyOffset(C);
      ...
      if (AuthInfo) {
        ...
        C = CGM.getConstantSignedPointer(
            ...
            cast_or_null<toolchain::ConstantInt>(AuthInfo.getDiscriminator()));
        return ConstantLValue(C, /*applied offset*/ true, /*signed*/ true);
      }
    ...
    if (const auto *FD = dyn_cast<FunctionDecl>(D)) {
      toolchain::Constant *C = CGM.getRawFunctionPointer(FD);
      if (FD->getType()->isCFIUncheckedCalleeFunctionType())
        C = toolchain::NoCFIValue::get(cast<toolchain::GlobalValue>(C));
...
ConstantLValueEmitter::VisitConstantExpr(const ConstantExpr *E) {
  if (toolchain::Constant *Result = Emitter.tryEmitConstantExpr(E))
    return Result;
...
ConstantLValueEmitter::VisitAddrLabelExpr(const AddrLabelExpr *E) {
  ...
  toolchain::Constant *Ptr = Emitter.CGF->GetAddrOfLabel(E->getLabel());
  return Ptr;
}
...
ConstantLValueEmitter::emitPointerAuthSignConstant(const CallExpr *E) {
  toolchain::Constant *UnsignedPointer = emitPointerAuthPointer(E->getArg(0));
...
toolchain::Constant *ConstantLValueEmitter::emitPointerAuthPointer(const Expr *E) {
  Expr::EvalResult Result;
...
ConstantLValueEmitter::emitPointerAuthDiscriminator(const Expr *E) {
  ...
  if (const auto *Call = dyn_cast<CallExpr>(E)) {
    if (Call->getBuiltinCallee() ==
      ...
      toolchain::Constant *Pointer = ConstantEmitter(CGM).emitAbstract(
          Call->getArg(0), Call->getArg(0)->getType());
      auto *Extra = cast<toolchain::ConstantInt>(ConstantEmitter(CGM).emitAbstract(
          Call->getArg(1), Call->getArg(1)->getType()));
      return {Pointer, Extra};
    }
  }

  toolchain::Constant *Result = ConstantEmitter(CGM).emitAbstract(E, E->getType());
  if (Result->getType()->isPointerTy())
    return {Result, nullptr};
  return {nullptr, cast<toolchain::ConstantInt>(Result)};
}
...
ConstantEmitter::tryEmitPrivate(const APValue &Value, QualType DestType,
  ...
  case APValue::Indeterminate:
    ...
    return toolchain::UndefValue::get(CGM.getTypes().ConvertType(DestType));
  case APValue::LValue:
  ...
  case APValue::Int:
    if (PointerAuthQualifier PointerAuth = DestType.getPointerAuth();
      ...
      return nullptr;
    return toolchain::ConstantInt::get(CGM.getLLVMContext(), Value.getInt());
  case APValue::FixedPoint:
    return toolchain::ConstantInt::get(CGM.getLLVMContext(),
                                  Value.getFixedPoint().getValue());
  case APValue::ComplexInt: {
    toolchain::Constant *Complex[2];

    Complex[0] = toolchain::ConstantInt::get(CGM.getLLVMContext(),
                                        Value.getComplexIntReal());
    Complex[1] = toolchain::ConstantInt::get(CGM.getLLVMContext(),
                                        Value.getComplexIntImag());

    // FIXME: the target may want to specify that this is packed.
    toolchain::StructType *STy =
        toolchain::StructType::get(Complex[0]->getType(), Complex[1]->getType());
    return toolchain::ConstantStruct::get(STy, Complex);
  }
  case APValue::Float: {
    const toolchain::APFloat &Init = Value.getFloat();
    if (&Init.getSemantics() == &toolchain::APFloat::IEEEhalf() &&
        !CGM.getContext().getLangOpts().NativeHalfType &&
        CGM.getContext().getTargetInfo().useFP16ConversionIntrinsics())
      return toolchain::ConstantInt::get(CGM.getLLVMContext(),
                                    Init.bitcastToAPInt());
    else
      return toolchain::ConstantFP::get(CGM.getLLVMContext(), Init);
  }
  case APValue::ComplexFloat: {
    toolchain::Constant *Complex[2];

    Complex[0] = toolchain::ConstantFP::get(CGM.getLLVMContext(),
                                       Value.getComplexFloatReal());
    Complex[1] = toolchain::ConstantFP::get(CGM.getLLVMContext(),
  ...
  case APValue::Vector: {
    ...
    SmallVector<toolchain::Constant *, 4> Inits(NumElts);

    for (unsigned I = 0; I != NumElts; ++I) {
      const APValue &Elt = Value.getVectorElt(I);
      if (Elt.isInt())
        Inits[I] = toolchain::ConstantInt::get(CGM.getLLVMContext(), Elt.getInt());
      else if (Elt.isFloat())
        Inits[I] = toolchain::ConstantFP::get(CGM.getLLVMContext(), Elt.getFloat());
      else if (Elt.isIndeterminate())
        Inits[I] = toolchain::UndefValue::get(CGM.getTypes().ConvertType(
            DestType->castAs<VectorType>()->getElementType()));
      else
        toolchain_unreachable("unsupported vector element type");
    }
    return toolchain::ConstantVector::get(Inits);
  }
  case APValue::AddrLabelDiff: {
    const AddrLabelExpr *LHSExpr = Value.getAddrLabelDiffLHS();
    const AddrLabelExpr *RHSExpr = Value.getAddrLabelDiffRHS();
    toolchain::Constant *LHS = tryEmitPrivate(LHSExpr, LHSExpr->getType());
    toolchain::Constant *RHS = tryEmitPrivate(RHSExpr, RHSExpr->getType());
    if (!LHS || !RHS) return nullptr;

    // Compute difference
    toolchain::Type *ResultType = CGM.getTypes().ConvertType(DestType);
    LHS = toolchain::ConstantExpr::getPtrToInt(LHS, CGM.IntPtrTy);
    RHS = toolchain::ConstantExpr::getPtrToInt(RHS, CGM.IntPtrTy);
    toolchain::Constant *AddrLabelDiff = toolchain::ConstantExpr::getSub(LHS, RHS);

    // LLVM is a bit sensitive about the exact format of the
    // address-of-label difference; make sure to truncate after
    // the subtraction.
    return toolchain::ConstantExpr::getTruncOrBitCast(AddrLabelDiff, ResultType);
  }
  case APValue::Struct:
  case APValue::Union:
  ...
  case APValue::Array: {
    ...
    toolchain::Constant *Filler = nullptr;
    ...
    for (unsigned I = 0; I < NumInitElts; ++I) {
      toolchain::Constant *C = tryEmitPrivateForMemory(
    ...
    toolchain::ArrayType *Desired =
        cast<toolchain::ArrayType>(CGM.getTypes().ConvertType(DestType));

    // Fix the type of incomplete arrays if the initializer isn't empty.
    if (DestType->isIncompleteArrayType() && !Elts.empty())
      Desired = toolchain::ArrayType::get(Desired->getElementType(), Elts.size());
  ...
  toolchain_unreachable("Unknown APValue kind");
}

toolchain::GlobalVariable *CodeGenModule::getAddrOfConstantCompoundLiteralIfEmitted(
    const CompoundLiteralExpr *E) {
  return EmittedCompoundLiterals.lookup(E);
}

void CodeGenModule::setAddrOfConstantCompoundLiteral(
    const CompoundLiteralExpr *CLE, toolchain::GlobalVariable *GV) {
...
static toolchain::Constant *EmitNullConstantForBase(CodeGenModule &CGM,
                                               toolchain::Type *baseType,
                                               const CXXRecordDecl *base);

static toolchain::Constant *EmitNullConstant(CodeGenModule &CGM,
                                        const RecordDecl *record,
                                        bool asCompleteObject) {
  const CGRecordLayout &layout = CGM.getTypes().getCGRecordLayout(record);
  toolchain::StructType *structure =
    (asCompleteObject ? layout.getLLVMType()
                      : layout.getBaseSubobjectLLVMType());

  unsigned numElements = structure->getNumElements();
  std::vector<toolchain::Constant *> elements(numElements);
  ...
  if (CXXR) {
    for (const auto &I : CXXR->bases()) {
      ...
      toolchain::Type *baseType = structure->getElementType(fieldIndex);
  ...
  for (unsigned i = 0; i != numElements; ++i) {
    if (!elements[i])
      elements[i] = toolchain::Constant::getNullValue(structure->getElementType(i));
  }

  return toolchain::ConstantStruct::get(structure, elements);
}
...
static toolchain::Constant *EmitNullConstantForBase(CodeGenModule &CGM,
  ...
  if (baseLayout.isZeroInitializableAsBase())
    return toolchain::Constant::getNullValue(baseType);

  // Otherwise, we can just use its null constant.
  return EmitNullConstant(CGM, base, /*asCompleteObject=*/false);
}

toolchain::Constant *ConstantEmitter::emitNullForMemory(CodeGenModule &CGM,
                                                   QualType T) {
  return emitForMemory(CGM, CGM.EmitNullConstant(T), T);
}

toolchain::Constant *CodeGenModule::EmitNullConstant(QualType T) {
  if (T->getAs<PointerType>())
    return getNullPointer(
        cast<toolchain::PointerType>(getTypes().ConvertTypeForMem(T)), T);

  if (getTypes().isZeroInitializable(T))
    return toolchain::Constant::getNullValue(getTypes().ConvertTypeForMem(T));

  if (const ConstantArrayType *CAT = Context.getAsConstantArrayType(T)) {
    toolchain::ArrayType *ATy =
      cast<toolchain::ArrayType>(getTypes().ConvertTypeForMem(T));
    ...
    SmallVector<toolchain::Constant *, 8> Array(NumElements, Element);
    return toolchain::ConstantArray::get(ATy, Array);
  }
