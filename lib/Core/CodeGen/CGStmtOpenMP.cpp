...
#include "toolchain/BinaryFormat/Dwarf.h"
...
class OMPLoopScope : public CodeGenFunction::RunCleanupsScope {
  void emitPreInitStmt(CodeGenFunction &CGF, const OMPLoopBasedDirective &S) {
    ...
    if (auto *LD = dyn_cast<OMPLoopDirective>(&S)) {
      toolchain::DenseSet<const VarDecl *> EmittedAsPrivate;
      ...
      for (const auto *C : LD->getClausesOfKind<OMPPrivateClause>()) {
        for (const Expr *IRef : C->varlist()) {
          ...
          if (EmittedAsPrivate.insert(OrigVD->getCanonicalDecl()).second) {
            ...
            (void)PreCondVars.setVarAddr(
                CGF, OrigVD,
                Address(toolchain::UndefValue::get(CGF.ConvertTypeForMem(
      ...
      toolchain_unreachable("Unknown loop-based directive kind.");
    }
    if (PreInits) {
      ...
      if (auto *PreInitCompound = dyn_cast<CompoundStmt>(PreInits))
        toolchain::append_range(PreInitStmts, PreInitCompound->body());
      else
...
public:
  OMPSimdLexicalScope(CodeGenFunction &CGF, const OMPExecutableDirective &S)
    ...
    toolchain::DenseSet<CanonicalDeclPtr<const Decl>> CopyArrayTemps;
...
toolchain::Value *CodeGenFunction::getTypeSize(QualType Ty) {
  ...
  if (SizeInChars.isZero()) {
    ...
    if (SizeInChars.isZero())
      return toolchain::ConstantInt::get(SizeTy, /*V=*/0);
...
void CodeGenFunction::GenerateOpenMPCapturedVars(
    const CapturedStmt &S, SmallVectorImpl<toolchain::Value *> &CapturedVars) {
  ...
  for (CapturedStmt::const_capture_init_iterator I = S.capture_init_begin(),
    ...
    if (CurField->hasCapturedVLAType()) {
      ...
      toolchain::Value *Val = VLASizeMap[VAT->getSizeExpr()];
      CapturedVars.push_back(Val);
    } else if (CurCap->capturesThis()) {
      CapturedVars.push_back(CXXThisValue);
    } else if (CurCap->capturesVariableByCopy()) {
      toolchain::Value *CV = EmitLoadOfScalar(EmitLValue(*I), CurCap->getLocation());
      ...
      if (!CurField->getType()->isAnyPointerType()) {
        ...
        toolchain::Value *SrcAddrVal = EmitScalarConversion(
...
static Address castValueFromUintptr(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  toolchain::Value *CastedPtr = CGF.EmitScalarConversion(
...
static toolchain::Function *emitOutlinedFunctionPrologue(
    CodeGenFunction &CGF, FunctionArgList &Args,
    toolchain::MapVector<const Decl *, std::pair<const VarDecl *, Address>>
        &LocalAddrs,
    toolchain::DenseMap<const Decl *, std::pair<const Expr *, toolchain::Value *>>
        &VLASizes,
    toolchain::Value *&CXXThisValue, const FunctionOptions &FO) {
  ...
  for (const FieldDecl *FD : RD->fields()) {
    ...
    VarDecl *Arg;
    if (CapVar && (CapVar->getTLSKind() != language::Core::VarDecl::TLS_None)) {
  ...
  toolchain::FunctionType *FuncLLVMTy = CGM.getTypes().GetFunctionType(FuncInfo);

  auto *F =
      toolchain::Function::Create(FuncLLVMTy, toolchain::GlobalValue::InternalLinkage,
  ...
  if (CGM.getCodeGenOpts().OptimizationLevel != 0) {
    F->removeFnAttr(toolchain::Attribute::NoInline);
    F->addFnAttr(toolchain::Attribute::AlwaysInline);
  }
  ...
  for (const FieldDecl *FD : RD->fields()) {
    ...
    if (FD->hasCapturedVLAType()) {
      ...
      toolchain::Value *ExprArg = CGF.EmitLoadOfScalar(ArgLVal, I->getLocation());
...
CodeGenFunction::GenerateOpenMPCapturedStmtFunction(const CapturedStmt &S,
  ...
  toolchain::MapVector<const Decl *, std::pair<const VarDecl *, Address>> LocalAddrs,
      WrapperLocalAddrs;
  toolchain::DenseMap<const Decl *, std::pair<const Expr *, toolchain::Value *>> VLASizes,
      WrapperVLASizes;
  ...
  toolchain::Function *WrapperF = nullptr;
  ...
  FunctionOptions FO(&S, !NeedWrapperFunction, /*RegisterCastedArgsOnly=*/false,
                     Out.str(), Loc);
  toolchain::Function *F = emitOutlinedFunctionPrologue(
  ...
  toolchain::SmallVector<toolchain::Value *, 4> CallArgs;
  auto *PI = F->arg_begin();
  for (const auto *Arg : Args) {
    toolchain::Value *CallArg;
...
void CodeGenFunction::EmitOMPAggregateAssign(
  ...
  toolchain::Value *NumElements = emitArrayLength(ArrayTy, ElementTy, DestAddr);
  SrcAddr = SrcAddr.withElementType(DestAddr.getElementType());

  toolchain::Value *SrcBegin = SrcAddr.emitRawPointer(*this);
  toolchain::Value *DestBegin = DestAddr.emitRawPointer(*this);
  // Cast from pointer to array type to pointer to single element.
  toolchain::Value *DestEnd = Builder.CreateInBoundsGEP(DestAddr.getElementType(),
                                                   DestBegin, NumElements);

  // The basic structure here is a while-do loop.
  toolchain::BasicBlock *BodyBB = createBasicBlock("omp.arraycpy.body");
  toolchain::BasicBlock *DoneBB = createBasicBlock("omp.arraycpy.done");
  toolchain::Value *IsEmpty =
      Builder.CreateICmpEQ(DestBegin, DestEnd, "omp.arraycpy.isempty");
  Builder.CreateCondBr(IsEmpty, DoneBB, BodyBB);

  // Enter the loop body, making that address the current address.
  toolchain::BasicBlock *EntryBB = Builder.GetInsertBlock();
  EmitBlock(BodyBB);

  CharUnits ElementSize = getContext().getTypeSizeInChars(ElementTy);

  toolchain::PHINode *SrcElementPHI =
  ...
  toolchain::PHINode *DestElementPHI = Builder.CreatePHI(
...
bool CodeGenFunction::EmitOMPFirstprivateClause(const OMPExecutableDirective &D,
  ...
  toolchain::DenseMap<const VarDecl *, OpenMPLastprivateModifier> Lastprivates;
  ...
  toolchain::DenseSet<const VarDecl *> EmittedAsFirstprivate;
  toolchain::SmallVector<OpenMPDirectiveKind, 4> CaptureRegions;
...
bool CodeGenFunction::EmitOMPCopyinClause(const OMPExecutableDirective &D) {
  ...
  toolchain::DenseSet<const VarDecl *> CopiedVars;
  toolchain::BasicBlock *CopyBegin = nullptr, *CopyEnd = nullptr;
...
bool CodeGenFunction::EmitOMPLastprivateClauseInit(
  ...
  OpenMPDirectiveKind EKind = getEffectiveDirectiveKind(D);
  toolchain::DenseSet<const VarDecl *> SIMDLCVs;
  ...
  toolchain::DenseSet<const VarDecl *> AlreadyEmittedVars;
  for (const auto *C : D.getClausesOfKind<OMPLastprivateClause>()) {
...
void CodeGenFunction::EmitOMPLastprivateClauseFinal(
    ...
    toolchain::Value *IsLastIterCond) {
  if (!HaveInsertPoint())
    return;
  // Emit following code:
  // if (<IsLastIterCond>) {
  ...
  // }
  toolchain::BasicBlock *ThenBB = nullptr;
  ...
  if (IsLastIterCond) {
    ...
    if (!getLangOpts().OpenMPSimd &&
        toolchain::any_of(D.getClausesOfKind<OMPLastprivateClause>(),
  ...
  toolchain::DenseSet<const VarDecl *> AlreadyEmittedVars;
  toolchain::DenseMap<const VarDecl *, const Expr *> LoopCountersAndUpdates;
...
void CodeGenFunction::EmitOMPReductionClauseInit(
  ...
  if (!Data.ReductionVars.empty()) {
    ...
    toolchain::Value *ReductionDesc = CGM.getOpenMPRuntime().emitTaskReductionInit(
    ...
    case OMPD_unknown:
    default:
      toolchain_unreachable("Unexpected directive with task reductions.");
    }
...
void CodeGenFunction::EmitOMPReductionClauseFinal(
  ...
  if (!HaveInsertPoint())
    return;
  toolchain::SmallVector<const Expr *, 8> Privates;
  toolchain::SmallVector<const Expr *, 8> LHSExprs;
  toolchain::SmallVector<const Expr *, 8> RHSExprs;
  toolchain::SmallVector<const Expr *, 8> ReductionOps;
  toolchain::SmallVector<bool, 8> IsPrivateVarReduction;
...
static void emitPostUpdateForReductionClause(
  ...
  for (const auto *C : D.getClausesOfKind<OMPReductionClause>()) {
    if (const Expr *PostUpdate = C->getPostUpdateExpr()) {
      if (!DoneBB) {
        if (toolchain::Value *Cond = CondGen(CGF)) {
          // If the first post-update expression is found, emit conditional
          // block if it was requested.
          toolchain::BasicBlock *ThenBB = CGF.createBasicBlock(".omp.reduction.pu");
...
typedef toolchain::function_ref<void(CodeGenFunction &,
                                const OMPExecutableDirective &,
                                toolchain::SmallVectorImpl<toolchain::Value *> &)>
...
checkForLastprivateConditionalUpdate(CodeGenFunction &CGF,
  ...
  if (CGF.getLangOpts().OpenMP < 50)
    return;
  toolchain::DenseSet<CanonicalDeclPtr<const VarDecl>> PrivateDecls;
  for (const auto *C : S.getClausesOfKind<OMPReductionClause>()) {
...
Address CodeGenFunction::OMPBuilderCBHelpers::getAddressOfLocalVariable(
  ...
  toolchain::Value *Allocator = CGF.EmitScalarExpr(AA->getAllocator());
  ...
  toolchain::Value *Addr = OMPBuilder.createOMPAlloc(
      CGF.Builder, Size, Allocator,
      getNameWithSeparators({CVD->getName(), ".void.addr"}, ".", "."));
  toolchain::CallInst *FreeCI =
...
Address CodeGenFunction::OMPBuilderCBHelpers::getAddrOfThreadPrivate(
  ...
  toolchain::ConstantInt *Size = CGM.getSize(CGM.GetTargetTypeStoreSize(VarTy));
  std::string Suffix = getNameWithSeparators({"cache", ""});
  toolchain::Twine CacheName = Twine(CGM.getMangledName(VD)).concat(Suffix);

  toolchain::CallInst *ThreadPrivateCacheCall =
...
void CodeGenFunction::OMPBuilderCBHelpers::EmitOMPInlinedRegionBody(
  ...
  toolchain::BasicBlock *FiniBB = splitBBWithSuffix(Builder, /*CreateBranch=*/false,
                                               "." + RegionName + ".after");
...
void CodeGenFunction::EmitOMPParallelDirective(const OMPParallelDirective &S) {
  if (CGM.getLangOpts().OpenMPIRBuilder) {
    ...
    toolchain::OpenMPIRBuilder::InsertPointTy AfterIP = cantFail(
...
void CodeGenFunction::EmitOMPLoopBody(const OMPLoopDirective &D,
  ...
  for (const Expr *E : D.finals_conditions()) {
    ...
    // space.
    toolchain::BasicBlock *NextBB = createBasicBlock("omp.body.next");
...
using EmittedClosureTy = std::pair<toolchain::Function *, toolchain::Value *>;
...
static EmittedClosureTy emitCapturedStmtFunc(CodeGenFunction &ParentCGF,
  ...
  toolchain::Function *F = CGF.GenerateCapturedStmtFunction(*S);

  return {F, CapStruct.getPointer(ParentCGF)};
}

/// Emit a call to a previously captured closure.
static toolchain::CallInst *
...
CodeGenFunction::EmitOMPCollapsedCanonicalLoopNest(const Stmt *S, int Depth) {
  ...
  toolchain::CanonicalLoopInfo *Result = OMPLoopNestStack.back();
...
void CodeGenFunction::EmitOMPCanonicalLoop(const OMPCanonicalLoop *S) {
  ...
  if (const auto *For = dyn_cast<ForStmt>(SyntacticalLoop)) {
    ...
    toolchain_unreachable("Expected for-stmt or range-based for-stmt");
  ...
  toolchain::Value *DistVal = Builder.CreateLoad(CountAddr, ".count");

  // Emit the loop structure.
  toolchain::OpenMPIRBuilder &OMPBuilder = CGM.getOpenMPRuntime().getOMPBuilder();
  auto BodyGen = [&, this](toolchain::OpenMPIRBuilder::InsertPointTy CodeGenIP,
                           toolchain::Value *IndVar) {
    Builder.restoreIP(CodeGenIP);
  ...
  toolchain::CanonicalLoopInfo *CL =
...
void CodeGenFunction::EmitOMPInnerLoop(
  ...
  toolchain::BasicBlock *ExitBlock = LoopExit.getBlock();
  if (RequiresCleanup)
    ExitBlock = createBasicBlock("omp.inner.for.cond.cleanup");

  toolchain::BasicBlock *LoopBody = createBasicBlock("omp.inner.for.body");
...
void CodeGenFunction::EmitOMPLinearClauseFinal(
  ...
  for (const auto *C : D.getClausesOfKind<OMPLinearClause>()) {
    ...
    for (const Expr *F : C->finals()) {
      if (!DoneBB) {
        if (toolchain::Value *Cond = CondGen(*this)) {
          // If the first post-update expression is found, emit conditional
          // block if it was requested.
          toolchain::BasicBlock *ThenBB = createBasicBlock(".omp.linear.pu");
...
static void emitAlignedClause(CodeGenFunction &CGF,
  ...
  for (const auto *Clause : D.getClausesOfKind<OMPAlignedClause>()) {
    toolchain::APInt ClauseAlignment(64, 0);
    if (const Expr *AlignmentExpr = Clause->getAlignment()) {
      auto *AlignmentCI =
          cast<toolchain::ConstantInt>(CGF.EmitScalarExpr(AlignmentExpr));
      ClauseAlignment = AlignmentCI->getValue();
    }
    for (const Expr *E : Clause->varlist()) {
      toolchain::APInt Alignment(ClauseAlignment);
      if (Alignment == 0) {
      ...
      if (Alignment != 0) {
        toolchain::Value *PtrValue = CGF.EmitScalarExpr(E);
        CGF.emitAlignmentAssumption(
            PtrValue, E, /*No second loc needed*/ SourceLocation(),
            toolchain::ConstantInt::get(CGF.getLLVMContext(), Alignment));
      }
    }
  }
}
...
static void emitPreCond(CodeGenFunction &CGF, const OMPLoopDirective &S,
                        const Expr *Cond, toolchain::BasicBlock *TrueBlock,
...
static void emitSimdlenSafelenClause(CodeGenFunction &CGF,
  ...
  if (const auto *C = D.getSingleClause<OMPSimdlenClause>()) {
    RValue Len = CGF.EmitAnyExpr(C->getSimdlen(), AggValueSlot::ignored(),
                                 /*ignoreResult=*/true);
    auto *Val = cast<toolchain::ConstantInt>(Len.getScalarVal());
...
void CodeGenFunction::EmitOMPSimdInit(const OMPLoopDirective &D) {
  ...
  if ((EKind == OMPD_simd ||
      ...
      toolchain::any_of(D.getClausesOfKind<OMPReductionClause>(),
    ...
    if (LocalDeclMap.count(OrigVD) || CapturedStmtInfo->lookup(OrigVD) ||
      ...
      if (!DoneBB) {
        if (toolchain::Value *Cond = CondGen(*this)) {
          // If the first post-update expression is found, emit conditional
          // block if it was requested.
          toolchain::BasicBlock *ThenBB = createBasicBlock(".omp.final.then");
...
static void emitOMPSimdRegion(CodeGenFunction &CGF, const OMPLoopDirective &S,
  ...
  if (CGF.ConstantFoldsToSimpleInteger(S.getPreCond(), CondConstant)) {
    ...
    toolchain::BasicBlock *ThenBlock = CGF.createBasicBlock("simd.if.then");
...
static toolchain::MapVector<toolchain::Value *, toolchain::Value *>
  ...
  if (UseOMPIRBuilder) {
    auto &&CodeGenIRBuilder = [&S, &CGM, UseOMPIRBuilder](CodeGenFunction &CGF,
      ...
      if (UseOMPIRBuilder) {
        ...
        toolchain::CanonicalLoopInfo *CLI =
        ...
        toolchain::omp::OrderKind Order = toolchain::omp::OrderKind::OMP_ORDER_unknown;
        if (const auto *C = S.getSingleClause<OMPOrderClause>()) {
          if (C->getKind() == OpenMPOrderClauseKind::OMPC_ORDER_concurrent) {
            Order = toolchain::omp::OrderKind::OMP_ORDER_concurrent;
          }
        }
...
void CodeGenFunction::EmitOMPUnrollDirective(const OMPUnrollDirective &S) {
  ...
  if (UseOMPIRBuilder) {
    ...
    toolchain::CanonicalLoopInfo *CLI = EmitOMPCollapsedCanonicalLoopNest(Inner, 1);
    OMPLoopNestStack.clear();

    toolchain::OpenMPIRBuilder &OMPBuilder = CGM.getOpenMPRuntime().getOMPBuilder();

    bool NeedsUnrolledCLI = ExpectedOMPLoopDepth >= 1;
    toolchain::CanonicalLoopInfo *UnrolledCLI = nullptr;
...
void CodeGenFunction::EmitOMPOuterLoop(
  ...
  // Start the loop with a block that tests the condition.
  toolchain::BasicBlock *CondBlock = createBasicBlock("omp.dispatch.cond");
  EmitBlock(CondBlock);
  const SourceRange R = S.getSourceRange();
  OMPLoopNestStack.clear();
  LoopStack.push(CondBlock, SourceLocToDebugLoc(R.getBegin()),
  ...
  toolchain::Value *BoolCondVal = nullptr;
  if (!DynamicOrOrdered) {
  ...
  toolchain::BasicBlock *LoopBody = createBasicBlock("omp.dispatch.body");
  ...
  emitCommonSimdLoop(
      *this, S,
      [&S, IsMonotonic, EKind](CodeGenFunction &CGF, PrePostActionTy &) {
        // Generate !toolchain.loop.parallel metadata for loads and stores for loops
...
void CodeGenFunction::EmitOMPForOuterLoop(
  ...
  if (DynamicOrOrdered) {
    const std::pair<toolchain::Value *, toolchain::Value *> DispatchBounds =
        CGDispatchBounds(*this, S, LoopArgs.LB, LoopArgs.UB);
    toolchain::Value *LBVal = DispatchBounds.first;
    toolchain::Value *UBVal = DispatchBounds.second;
...
emitDistributeParallelForInnerBounds(CodeGenFunction &CGF,
  ...
  toolchain::Value *PrevLBVal = CGF.EmitLoadOfScalar(
  ...
  toolchain::Value *PrevUBVal = CGF.EmitLoadOfScalar(
...
static void emitDistributeParallelForDistributeInnerBoundParams(
  ...
  toolchain::Value *LBCast = CGF.Builder.CreateIntCast(
      CGF.Builder.CreateLoad(LB.getAddress()), CGF.SizeTy, /*isSigned=*/false);
  CapturedVars.push_back(LBCast);
  LValue UB =
      CGF.EmitLValue(cast<DeclRefExpr>(Dir.getCombinedUpperBoundVariable()));

  toolchain::Value *UBCast = CGF.Builder.CreateIntCast(
...
bool CodeGenFunction::EmitOMPWorksharingLoop(
  ...
  // Check pre-condition.
  {
    ...
    if (ConstantFoldsToSimpleInteger(S.getPreCond(), CondConstant)) {
      ...
      toolchain::BasicBlock *ThenBlock = createBasicBlock("omp.precond.then");
    ...
    // Emit 'then' code.
    {
      ...
      if (ChunkExpr) {
        ...
        if (ChunkExpr->EvaluateAsInt(Result, getContext())) {
          toolchain::APSInt EvaluatedChunk = Result.Val.getInt();
...
emitDispatchForLoopBounds(CodeGenFunction &CGF, const OMPExecutableDirective &S,
  ...
  toolchain::Value *LBVal = CGF.Builder.getIntN(IVSize, 0);
  toolchain::Value *UBVal = CGF.EmitScalarExpr(LS.getLastIteration());
  return {LBVal, UBVal};
}
...
static void emitScanBasedDirectiveDecls(
    ...
    toolchain::function_ref<toolchain::Value *(CodeGenFunction &)> NumIteratorsGen) {
  toolchain::Value *OMPScanNumIterations = CGF.Builder.CreateIntCast(
...
static void emitScanBasedDirectiveFinals(
  ...
  // LHS = TMP[LastIter];
  toolchain::Value *OMPLast = CGF.Builder.CreateNSWSub(
      OMPScanNumIterations,
      toolchain::ConstantInt::get(CGF.SizeTy, 1, /*isSigned=*/false));
...
static void emitScanBasedDirective(
    CodeGenFunction &CGF, const OMPLoopDirective &S,
    toolchain::function_ref<toolchain::Value *(CodeGenFunction &)> NumIteratorsGen,
    toolchain::function_ref<void(CodeGenFunction &)> FirstGen,
    toolchain::function_ref<void(CodeGenFunction &)> SecondGen) {
  toolchain::Value *OMPScanNumIterations = CGF.Builder.CreateIntCast(
      NumIteratorsGen(CGF), CGF.SizeTy, /*isSigned=*/false);
  SmallVector<const Expr *, 4> Privates;
  SmallVector<const Expr *, 4> ReductionOps;
  ...
  auto &&CodeGen = [&S, OMPScanNumIterations, &LHSs, &RHSs, &CopyArrayElems,
    ...
    toolchain::BasicBlock *InputBB = CGF.Builder.GetInsertBlock();
    toolchain::BasicBlock *LoopBB = CGF.createBasicBlock("omp.outer.log.scan.body");
    toolchain::BasicBlock *ExitBB = CGF.createBasicBlock("omp.outer.log.scan.exit");
    toolchain::Function *F =
        CGF.CGM.getIntrinsic(toolchain::Intrinsic::log2, CGF.DoubleTy);
    toolchain::Value *Arg =
        CGF.Builder.CreateUIToFP(OMPScanNumIterations, CGF.DoubleTy);
    toolchain::Value *LogVal = CGF.EmitNounwindRuntimeCall(F, Arg);
    F = CGF.CGM.getIntrinsic(toolchain::Intrinsic::ceil, CGF.DoubleTy);
    LogVal = CGF.EmitNounwindRuntimeCall(F, LogVal);
    LogVal = CGF.Builder.CreateFPToUI(LogVal, CGF.IntTy);
    toolchain::Value *NMin1 = CGF.Builder.CreateNUWSub(
        OMPScanNumIterations, toolchain::ConstantInt::get(CGF.SizeTy, 1));
    ...
    Counter->addIncoming(toolchain::ConstantInt::get(CGF.IntTy, 0), InputBB);
    Pow2K->addIncoming(toolchain::ConstantInt::get(CGF.SizeTy, 1), InputBB);
    // for (size i = n - 1; i >= 2 ^ k; --i)
    //   tmp[i] op= tmp[i-pow2k];
    toolchain::BasicBlock *InnerLoopBB =
        CGF.createBasicBlock("omp.inner.log.scan.body");
    toolchain::BasicBlock *InnerExitBB =
        CGF.createBasicBlock("omp.inner.log.scan.exit");
    toolchain::Value *CmpI = CGF.Builder.CreateICmpUGE(NMin1, Pow2K);
    ...
    IVal->addIncoming(NMin1, LoopBB);
    {
      ...
      for (const Expr *CopyArrayElem : CopyArrayElems) {
        ...
        {
          toolchain::Value *OffsetIVal = CGF.Builder.CreateNUWSub(IVal, Pow2K);
          CodeGenFunction::OpaqueValueMapping IdxMapping(
              CGF,
              cast<OpaqueValueExpr>(
                  cast<ArraySubscriptExpr>(CopyArrayElem)->getIdx()),
    ...
    toolchain::Value *NextIVal =
        CGF.Builder.CreateNUWSub(IVal, toolchain::ConstantInt::get(CGF.SizeTy, 1));
    ...
    toolchain::Value *Next =
        CGF.Builder.CreateNUWAdd(Counter, toolchain::ConstantInt::get(CGF.IntTy, 1));
    Counter->addIncoming(Next, CGF.Builder.GetInsertBlock());
    // pow2k <<= 1;
    toolchain::Value *NextPow2K =
        CGF.Builder.CreateShl(Pow2K, 1, "", /*HasNUW=*/true);
    Pow2K->addIncoming(NextPow2K, CGF.Builder.GetInsertBlock());
    toolchain::Value *Cmp = CGF.Builder.CreateICmpNE(Next, LogVal);
...
static bool emitWorksharingDirective(CodeGenFunction &CGF,
  ...
  OpenMPDirectiveKind EKind = getEffectiveDirectiveKind(S);
  if (toolchain::any_of(S.getClausesOfKind<OMPReductionClause>(),
                   [](const OMPReductionClause *C) {
                     return C->getModifier() == OMPC_REDUCTION_inscan;
                   })) {
...
convertClauseKindToSchedKind(OpenMPScheduleClauseKind ScheduleClauseKind) {
  ...
  toolchain_unreachable("Unhandled schedule kind");
}
...
static void emitOMPForDirective(const OMPLoopDirective &S, CodeGenFunction &CGF,
  ...
  auto &&CodeGen = [&S, &CGM, HasCancel, &HasLastprivates,
    ...
    if (UseOMPIRBuilder) {
      ...
      toolchain::omp::ScheduleKind SchedKind = toolchain::omp::OMP_SCHEDULE_Default;
      toolchain::Value *ChunkSize = nullptr;
...
void CodeGenFunction::EmitSections(const OMPExecutableDirective &S) {
  ...
  auto &&CodeGen = [&S, CapturedStmt, CS, EKind,
    ...
    toolchain::ConstantInt *GlobalUBVal = CS != nullptr
    ...
    auto &&BodyGen = [CapturedStmt, CS, &S, &IV](CodeGenFunction &CGF) {
      ...
      // }
      // .omp.sections.exit:
      toolchain::BasicBlock *ExitBB = CGF.createBasicBlock(".omp.sections.exit");
      toolchain::SwitchInst *SwitchStmt =
      ...
      if (CS) {
        ...
        toolchain::BasicBlock *CaseBB = CGF.createBasicBlock(".omp.sections.case");
        CGF.EmitBlock(CaseBB);
    ...
    // UB = min(UB, GlobalUB);
    toolchain::Value *UBVal = CGF.EmitLoadOfScalar(UB, S.getBeginLoc());
    toolchain::Value *MinUBGlobalUB = CGF.Builder.CreateSelect(
...
void CodeGenFunction::EmitOMPSectionsDirective(const OMPSectionsDirective &S) {
  if (CGM.getLangOpts().OpenMPIRBuilder) {
    toolchain::OpenMPIRBuilder &OMPBuilder = CGM.getOpenMPRuntime().getOMPBuilder();
    using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
    using BodyGenCallbackTy = toolchain::OpenMPIRBuilder::StorableBodyGenCallbackTy;
    ...
    const Stmt *CapturedStmt = S.getInnermostCapturedStmt()->getCapturedStmt();
    const auto *CS = dyn_cast<CompoundStmt>(CapturedStmt);
    toolchain::SmallVector<BodyGenCallbackTy, 4> SectionCBVector;
    if (CS) {
      for (const Stmt *SubStmt : CS->children()) {
...
void CodeGenFunction::EmitOMPSingleDirective(const OMPSingleDirective &S) {
  toolchain::SmallVector<const Expr *, 8> CopyprivateVars;
  toolchain::SmallVector<const Expr *, 8> DestExprs;
  toolchain::SmallVector<const Expr *, 8> SrcExprs;
  toolchain::SmallVector<const Expr *, 8> AssignmentOps;
...
void CodeGenFunction::EmitOMPCriticalDirective(const OMPCriticalDirective &S) {
  if (CGM.getLangOpts().OpenMPIRBuilder) {
    ...
    toolchain::Value *HintInst = nullptr;
    if (Hint)
      HintInst =
...
void CodeGenFunction::EmitOMPParallelForDirective(
  ...
  };
  {
    ...
    bool IsInscan = toolchain::any_of(S.getClausesOfKind<OMPReductionClause>(),
                     [](const OMPReductionClause *C) {
                       return C->getModifier() == OMPC_REDUCTION_inscan;
                     });
    if (IsInscan)
...
class CheckVarsEscapingUntiedTaskDeclContext final
  ...
  toolchain::SmallVector<const VarDecl *, 4> PrivateDecls;
...
void CodeGenFunction::EmitOMPTaskBasedDirective(
  ...
  toolchain::MapVector<const VarDecl *, const DeclRefExpr *> LastprivateDstsOrigs;
  for (const auto *C : S.getClausesOfKind<OMPLastprivateClause>()) {
    auto IRef = C->varlist_begin();
  ...
  auto &&CodeGen = [&Data, &S, CS, &BodyGen, &LastprivateDstsOrigs,
                    CapturedRegion](CodeGenFunction &CGF,
                                    PrePostActionTy &Action) {
    toolchain::MapVector<CanonicalDeclPtr<const VarDecl>,
    ...
    if (auto *DI = CGF.getDebugInfo()) {
      ...
      toolchain::Value *ContextValue = CGF.CapturedStmtInfo->getContextValue();
      if (CaptureFields.size() && ContextValue) {
        ...
        // shared variable, debug intrinsic toolchain.dbg.declare is generated with
        // appropriate expressions (DIExpression).
        // Ex:
        //  %12 = load %struct.anon*, %struct.anon** %__context.addr.i
        //  call void @toolchain.dbg.declare(metadata %struct.anon* %12,
        //            metadata !svar1,
        ...
        for (auto It = CaptureFields.begin(); It != CaptureFields.end(); ++It) {
          ...
          auto UpdateExpr = [](toolchain::LLVMContext &Ctx, auto *Declare,
                               unsigned Offset) {
            SmallVector<uint64_t, 8> Ops;
            // Add offset to the base address if non zero.
            if (Offset) {
              Ops.push_back(toolchain::dwarf::DW_OP_plus_uconst);
              Ops.push_back(Offset);
            }
            Ops.push_back(toolchain::dwarf::DW_OP_deref);
            Declare->setExpression(toolchain::DIExpression::get(Ctx, Ops));
          };
          toolchain::Instruction &Last = CGF.Builder.GetInsertBlock()->back();
          if (auto DDI = dyn_cast<toolchain::DbgVariableIntrinsic>(&Last))
          ...
          if (auto *Marker =
            ...
            for (toolchain::DbgVariableRecord &DVR : toolchain::reverse(
                     toolchain::filterDbgVars(Marker->getDbgRecordRange()))) {
    ...
    toolchain::SmallVector<std::pair<const VarDecl *, Address>, 16> FirstprivatePtrs;
    if (!Data.PrivateVars.empty() || !Data.FirstprivateVars.empty() ||
        !Data.LastprivateVars.empty() || !Data.PrivateLocals.empty()) {
      enum { PrivatesParam = 2, CopyFnParam = 3 };
      toolchain::Value *CopyFn = CGF.Builder.CreateLoad(
          CGF.GetAddrOfLocalVar(CS->getCapturedDecl()->getParam(CopyFnParam)));
      toolchain::Value *PrivatesPtr = CGF.Builder.CreateLoad(CGF.GetAddrOfLocalVar(
          CS->getCapturedDecl()->getParam(PrivatesParam)));
      // Map privates.
      toolchain::SmallVector<std::pair<const VarDecl *, Address>, 16> PrivatePtrs;
      toolchain::SmallVector<toolchain::Value *, 16> CallArgs;
      toolchain::SmallVector<toolchain::Type *, 4> ParamTypes;
      ...
      auto *CopyFnTy = toolchain::FunctionType::get(CGF.Builder.getVoidTy(),
      ...
      for (auto &Pair : UntiedLocalVars) {
        ...
        if (isAllocatableDecl(Pair.first)) {
          toolchain::Value *Ptr = CGF.Builder.CreateLoad(Pair.second.first);
          Address Replacement(
              Ptr,
    ...
    if (Data.Reductions) {
      ...
      toolchain::Value *ReductionsPtr = CGF.Builder.CreateLoad(
    ...
    if (!InRedVars.empty()) {
      ...
      for (unsigned Cnt = 0, E = InRedVars.size(); Cnt < E; ++Cnt) {
        ...
        if (const Expr *TRExpr = TaskgroupDescriptors[Cnt]) {
          ...
          ReductionsPtr = toolchain::ConstantPointerNull::get(CGF.VoidPtrTy);
        }
        Address Replacement = CGF.CGM.getOpenMPRuntime().getTaskReductionItem(
            CGF, S.getBeginLoc(), ReductionsPtr, RedCG.getSharedLValue(Cnt));
        Replacement = Address(
  ...
  OpenMPDirectiveKind EKind = getEffectiveDirectiveKind(S);
  toolchain::Function *OutlinedFn = CGM.getOpenMPRuntime().emitTaskOutlinedFunction(
...
void CodeGenFunction::EmitOMPTargetTaskBasedDirective(
  ...
  if (InputInfo.NumberOfTargetItems > 0) {
    ...
    toolchain::APInt ArrSize(/*numBits=*/32, InputInfo.NumberOfTargetItems);
    ...
    if (!isa_and_nonnull<toolchain::ConstantPointerNull>(
  ...
  toolchain::Function *OutlinedFn = CGM.getOpenMPRuntime().emitTaskOutlinedFunction(
      ...
      Data.NumberOfParts);
  toolchain::APInt TrueOrFalse(32, S.hasClausesOfKind<OMPNowaitClause>() ? 1 : 0);
...
void CodeGenFunction::EmitOMPTaskDirective(const OMPTaskDirective &S) {
  ...
  auto &&TaskGen = [&S, SharedsTy, CapturedStruct,
                    IfCond](CodeGenFunction &CGF, toolchain::Function *OutlinedFn,
...
void CodeGenFunction::EmitOMPFlushDirective(const OMPFlushDirective &S) {
  toolchain::AtomicOrdering AO = S.getSingleClause<OMPFlushClause>()
                                ? toolchain::AtomicOrdering::NotAtomic
                                : toolchain::AtomicOrdering::AcquireRelease;
  CGM.getOpenMPRuntime().emitFlush(
      *this,
      [&S]() -> ArrayRef<const Expr *> {
        if (const auto *FlushClause = S.getSingleClause<OMPFlushClause>())
          return toolchain::ArrayRef(FlushClause->varlist_begin(),
...
void CodeGenFunction::EmitOMPScanDirective(const OMPScanDirective &S) {
  ...
  if (ParentDir.getDirectiveKind() == OMPD_simd ||
    ...
    //   <first part>
    // }
    toolchain::BasicBlock *OMPScanReduce = createBasicBlock("omp.inscan.reduce");
  ...
  if (OMPFirstScanLoop) {
    ...
    toolchain::Value *IdxVal = EmitLoadOfScalar(IdxLVal, IVExpr->getExprLoc());
  ...
  if (!OMPFirstScanLoop) {
    ...
    toolchain::Value *IdxVal = EmitLoadOfScalar(IdxLVal, IVExpr->getExprLoc());
    IdxVal = Builder.CreateIntCast(IdxVal, SizeTy, /*isSigned=*/false);
    toolchain::BasicBlock *ExclusiveExitBB = nullptr;
    if (!IsInclusive) {
      toolchain::BasicBlock *ContBB = createBasicBlock("omp.exclusive.dec");
      ExclusiveExitBB = createBasicBlock("omp.exclusive.copy.exit");
      toolchain::Value *Cmp = Builder.CreateIsNull(IdxVal);
      Builder.CreateCondBr(Cmp, ExclusiveExitBB, ContBB);
      EmitBlock(ContBB);
      // Use idx - 1 iteration for exclusive scan.
      IdxVal = Builder.CreateNUWSub(IdxVal, toolchain::ConstantInt::get(SizeTy, 1));
    }
    for (unsigned I = 0, E = CopyArrayElems.size(); I < E; ++I) {
      const Expr *PrivateExpr = Privates[I];
      const Expr *OrigExpr = Shareds[I];
...
static toolchain::Function *emitOutlinedOrderedFunction(CodeGenModule &CGM,
                                                   const CapturedStmt *S,
                                                   SourceLocation Loc) {
  CodeGenFunction CGF(CGM, /*suppressNewContext=*/true);
  CodeGenFunction::CGCapturedStmtInfo CapStmtInfo;
  CGF.CapturedStmtInfo = &CapStmtInfo;
  toolchain::Function *Fn = CGF.GenerateOpenMPCapturedStmtFunction(*S, Loc);
...
static void emitRestoreIP(CodeGenFunction &CGF, const T *C,
  ...
  toolchain::SmallVector<toolchain::Value *> StoreValues;
  for (unsigned I = 0; I < NumLoops; I++) {
    const Expr *CounterVal = C->getLoopData(I);
    assert(CounterVal);
    toolchain::Value *StoreValue = CGF.EmitScalarConversion(
  ...
  if (CGM.getLangOpts().OpenMPIRBuilder) {
    ...
    if (S.hasClausesOfKind<OMPDependClause>() ||
      ...
      auto BodyGenCB = [&S, C, this](InsertPointTy AllocaIP,
        ...
        if (C) {
          toolchain::BasicBlock *FiniBB = splitBBWithSuffix(
...
static toolchain::Value *convertToScalarValue(CodeGenFunction &CGF, RValue Val,
...
convertToComplexValue(CodeGenFunction &CGF, RValue Val, QualType SrcType,
  ...
  if (Val.isScalar()) {
    ...
    toolchain::Value *ScalarVal = CGF.EmitScalarConversion(
        Val.getScalarVal(), SrcType, DestElementType, Loc);
    ComplexVal = CodeGenFunction::ComplexPairTy(
        ScalarVal, toolchain::Constant::getNullValue(ScalarVal->getType()));
  } else {
...
static void emitSimpleAtomicStore(CodeGenFunction &CGF, toolchain::AtomicOrdering AO,
...
static RValue emitSimpleAtomicLoad(CodeGenFunction &CGF,
                                   toolchain::AtomicOrdering AO, LValue LVal,
                                   SourceLocation Loc) {
  if (LVal.isGlobalReg())
    return CGF.EmitLoadOfLValue(LVal, Loc);
  return CGF.EmitAtomicLoad(
      LVal, Loc, toolchain::AtomicCmpXchgInst::getStrongestFailureOrdering(AO),
      LVal.isVolatile());
}
...
void CodeGenFunction::emitOMPSimpleStore(LValue LVal, RValue RVal,
  ...
  case TEK_Complex:
    ...
    break;
  case TEK_Aggregate:
    toolchain_unreachable("Must be a scalar or complex.");
  }
}

static void emitOMPAtomicReadExpr(CodeGenFunction &CGF, toolchain::AtomicOrdering AO,
  ...
  switch (AO) {
  case toolchain::AtomicOrdering::Acquire:
  case toolchain::AtomicOrdering::AcquireRelease:
  case toolchain::AtomicOrdering::SequentiallyConsistent:
    CGF.CGM.getOpenMPRuntime().emitFlush(CGF, {}, Loc,
                                         toolchain::AtomicOrdering::Acquire);
    break;
  case toolchain::AtomicOrdering::Monotonic:
  case toolchain::AtomicOrdering::Release:
    break;
  case toolchain::AtomicOrdering::NotAtomic:
  case toolchain::AtomicOrdering::Unordered:
    toolchain_unreachable("Unexpected ordering.");
  }
...
static void emitOMPAtomicWriteExpr(CodeGenFunction &CGF,
                                   toolchain::AtomicOrdering AO, const Expr *X,
...
static std::pair<bool, RValue> emitOMPAtomicRMW(CodeGenFunction &CGF, LValue X,
  ...
  if (BO == BO_Comma || !Update.isScalar() || !X.isSimple() ||
      (!isa<toolchain::ConstantInt>(Update.getScalarVal()) &&
       (Update.getScalarVal()->getType() != X.getAddress().getElementType())) ||
      !Context.getTargetInfo().hasBuiltinAtomic(
          Context.getTypeSize(X.getType()), Context.toBits(X.getAlignment())))
    return std::make_pair(false, RValue::get(nullptr));

  auto &&CheckAtomicSupport = [&CGF](toolchain::Type *T, BinaryOperatorKind BO) {
    if (T->isIntegerTy())
      return true;

    if (T->isFloatingPointTy() && (BO == BO_Add || BO == BO_Sub))
      return toolchain::isPowerOf2_64(CGF.CGM.getDataLayout().getTypeStoreSize(T));
  ...
  toolchain::AtomicRMWInst::BinOp RMWOp;
  switch (BO) {
  case BO_Add:
    RMWOp = IsInteger ? toolchain::AtomicRMWInst::Add : toolchain::AtomicRMWInst::FAdd;
    break;
  case BO_Sub:
    if (!IsXLHSInRHSPart)
      return std::make_pair(false, RValue::get(nullptr));
    RMWOp = IsInteger ? toolchain::AtomicRMWInst::Sub : toolchain::AtomicRMWInst::FSub;
    break;
  case BO_And:
    RMWOp = toolchain::AtomicRMWInst::And;
    break;
  case BO_Or:
    RMWOp = toolchain::AtomicRMWInst::Or;
    break;
  case BO_Xor:
    RMWOp = toolchain::AtomicRMWInst::Xor;
    break;
  case BO_LT:
    if (IsInteger)
      RMWOp = X.getType()->hasSignedIntegerRepresentation()
                  ? (IsXLHSInRHSPart ? toolchain::AtomicRMWInst::Min
                                     : toolchain::AtomicRMWInst::Max)
                  : (IsXLHSInRHSPart ? toolchain::AtomicRMWInst::UMin
                                     : toolchain::AtomicRMWInst::UMax);
    else
      RMWOp = IsXLHSInRHSPart ? toolchain::AtomicRMWInst::FMin
                              : toolchain::AtomicRMWInst::FMax;
    break;
  case BO_GT:
    if (IsInteger)
      RMWOp = X.getType()->hasSignedIntegerRepresentation()
                  ? (IsXLHSInRHSPart ? toolchain::AtomicRMWInst::Max
                                     : toolchain::AtomicRMWInst::Min)
                  : (IsXLHSInRHSPart ? toolchain::AtomicRMWInst::UMax
                                     : toolchain::AtomicRMWInst::UMin);
    else
      RMWOp = IsXLHSInRHSPart ? toolchain::AtomicRMWInst::FMax
                              : toolchain::AtomicRMWInst::FMin;
    break;
  case BO_Assign:
    RMWOp = toolchain::AtomicRMWInst::Xchg;
    break;
  case BO_Mul:
  case BO_Div:
  case BO_Rem:
  ...
  case BO_ShrAssign:
  case BO_Comma:
    toolchain_unreachable("Unsupported atomic update operation");
  }
  toolchain::Value *UpdateVal = Update.getScalarVal();
  if (auto *IC = dyn_cast<toolchain::ConstantInt>(UpdateVal)) {
    if (IsInteger)
      UpdateVal = CGF.Builder.CreateIntCast(
          IC, X.getAddress().getElementType(),
          X.getType()->hasSignedIntegerRepresentation());
    else
      UpdateVal = CGF.Builder.CreateCast(toolchain::Instruction::CastOps::UIToFP, IC,
                                         X.getAddress().getElementType());
  }
  toolchain::AtomicRMWInst *Res =
  ...
  if (CGF.CGM.getLangOpts().OpenMP < 51) {
    ...
    case toolchain::AtomicOrdering::SequentiallyConsistent:
      CGF.CGM.getOpenMPRuntime().emitFlush(
          CGF, {}, Loc, toolchain::AtomicOrdering::AcquireRelease);
      break;
    case toolchain::AtomicOrdering::Monotonic:
      break;
    case toolchain::AtomicOrdering::NotAtomic:
...
static void emitOMPAtomicCompareExpr(
    CodeGenFunction &CGF, toolchain::AtomicOrdering AO, toolchain::AtomicOrdering FailAO,
  ...
  case BO_GT:
    ...
    break;
  default:
    toolchain_unreachable("unsupported atomic compare binary operator");
  }
  ...
  auto EmitRValueWithCastIfNeeded = [&CGF, Loc](const Expr *X, const Expr *E) {
    ...
    toolchain::Value *V = CGF.EmitScalarExpr(NewE);
    if (NewE->getType() == X->getType())
      return V;
    return CGF.EmitScalarConversion(V, NewE->getType(), X->getType(), Loc);
  };

  toolchain::Value *EVal = EmitRValueWithCastIfNeeded(X, E);
  toolchain::Value *DVal = D ? EmitRValueWithCastIfNeeded(X, D) : nullptr;
  if (auto *CI = dyn_cast<toolchain::ConstantInt>(EVal))
    EVal = CGF.Builder.CreateIntCast(
        CI, XLVal.getAddress().getElementType(),
        E->getType()->hasSignedIntegerRepresentation());
  if (DVal)
    if (auto *CI = dyn_cast<toolchain::ConstantInt>(DVal))
      DVal = CGF.Builder.CreateIntCast(
          CI, XLVal.getAddress().getElementType(),
          D->getType()->hasSignedIntegerRepresentation());

  toolchain::OpenMPIRBuilder::AtomicOpValue XOpVal{
      XAddr.emitRawPointer(CGF), XAddr.getElementType(),
      X->getType()->hasSignedIntegerRepresentation(),
      X->getType().isVolatileQualified()};
  toolchain::OpenMPIRBuilder::AtomicOpValue VOpVal, ROpVal;
  if (V) {
  ...
  if (FailAO == toolchain::AtomicOrdering::NotAtomic) {
...
static void emitOMPAtomicExpr(CodeGenFunction &CGF, OpenMPClauseKind Kind,
                              toolchain::AtomicOrdering AO,
                              toolchain::AtomicOrdering FailAO, bool IsPostfixUpdate,
  ...
  default:
    toolchain_unreachable("Clause is not allowed in 'omp atomic'.");
  }
}

void CodeGenFunction::EmitOMPAtomicDirective(const OMPAtomicDirective &S) {
  toolchain::AtomicOrdering AO = CGM.getOpenMPRuntime().getDefaultMemoryOrdering();
  // Fail Memory Clause Ordering.
  toolchain::AtomicOrdering FailAO = toolchain::AtomicOrdering::NotAtomic;
  bool MemOrderingSpecified = false;
  if (S.getSingleClause<OMPSeqCstClause>()) {
    AO = toolchain::AtomicOrdering::SequentiallyConsistent;
    MemOrderingSpecified = true;
  } else if (S.getSingleClause<OMPAcqRelClause>()) {
    AO = toolchain::AtomicOrdering::AcquireRelease;
    MemOrderingSpecified = true;
  } else if (S.getSingleClause<OMPAcquireClause>()) {
    AO = toolchain::AtomicOrdering::Acquire;
    MemOrderingSpecified = true;
  } else if (S.getSingleClause<OMPReleaseClause>()) {
    AO = toolchain::AtomicOrdering::Release;
    MemOrderingSpecified = true;
  } else if (S.getSingleClause<OMPRelaxedClause>()) {
    AO = toolchain::AtomicOrdering::Monotonic;
    MemOrderingSpecified = true;
  }
  toolchain::SmallSet<OpenMPClauseKind, 2> KindsEncountered;
  ...
  if (!MemOrderingSpecified) {
    toolchain::AtomicOrdering DefaultOrder =
        CGM.getOpenMPRuntime().getDefaultMemoryOrdering();
    if (DefaultOrder == toolchain::AtomicOrdering::Monotonic ||
        DefaultOrder == toolchain::AtomicOrdering::SequentiallyConsistent ||
        (DefaultOrder == toolchain::AtomicOrdering::AcquireRelease &&
         Kind == OMPC_capture)) {
      AO = DefaultOrder;
    } else if (DefaultOrder == toolchain::AtomicOrdering::AcquireRelease) {
  ...
  if (KindsEncountered.contains(OMPC_compare) &&
    ...
    if (FailClause) {
      ...
      if (FailParameter == toolchain::omp::OMPC_relaxed)
        FailAO = toolchain::AtomicOrdering::Monotonic;
      else if (FailParameter == toolchain::omp::OMPC_acquire)
        FailAO = toolchain::AtomicOrdering::Acquire;
      else if (FailParameter == toolchain::omp::OMPC_seq_cst)
        FailAO = toolchain::AtomicOrdering::SequentiallyConsistent;
    }
  }
...
static void emitCommonOMPTargetDirective(CodeGenFunction &CGF,
  ...
  toolchain::Constant *FnID = nullptr;
  ...
  auto &&SizeEmitter =
      [IsOffloadEntry](CodeGenFunction &CGF,
                       const OMPLoopDirective &D) -> toolchain::Value * {
    if (IsOffloadEntry) {
      OMPLoopScope(CGF, D);
      // Emit calculation of the iterations count.
      toolchain::Value *NumIterations = CGF.EmitScalarExpr(D.getNumIterations());
...
void CodeGenFunction::EmitOMPInteropDirective(const OMPInteropDirective &S) {
  toolchain::OpenMPIRBuilder &OMPBuilder = CGM.getOpenMPRuntime().getOMPBuilder();
  toolchain::Value *Device = nullptr;
  toolchain::Value *NumDependences = nullptr;
  toolchain::Value *DependenceList = nullptr;
  ...
  if (!ItOMPInitClause.empty()) {
    ...
    for (const OMPInitClause *C : ItOMPInitClause) {
      toolchain::Value *InteropvarPtr =
          EmitLValue(C->getInteropVar()).getPointer(*this);
      toolchain::omp::OMPInteropType InteropType =
          toolchain::omp::OMPInteropType::Unknown;
      if (C->getIsTarget()) {
        InteropType = toolchain::omp::OMPInteropType::Target;
      } else {
        assert(C->getIsTargetSync() &&
               "Expected interop-type target/targetsync");
        InteropType = toolchain::omp::OMPInteropType::TargetSync;
      }
...
void CodeGenFunction::EmitOMPCancelDirective(const OMPCancelDirective &S) {
  ...
  if (CGM.getLangOpts().OpenMPIRBuilder) {
    ...
    if (S.getCancelRegion() == OMPD_parallel ||
      ...
      toolchain::Value *IfCondition = nullptr;
      if (IfCond)
...
void CodeGenFunction::EmitOMPUseDevicePtrClause(
  ...
  for (const Expr *OrigVarIt : C.varlist()) {
    ...
    toolchain::Type *Ty = ConvertTypeForMem(OrigVD->getType().getNonReferenceType());
...
void CodeGenFunction::EmitOMPTaskLoopBasedDirective(const OMPLoopDirective &S) {
  ...
  auto &&BodyGen = [CS, &S](CodeGenFunction &CGF, PrePostActionTy &) {
    ...
    if (CGF.ConstantFoldsToSimpleInteger(S.getPreCond(), CondConstant)) {
      ...
      toolchain::BasicBlock *ThenBlock = CGF.createBasicBlock("taskloop.if.then");
...
static void emitTargetTeamsLoopCodegenStatus(CodeGenFunction &CGF,
  ...
  toolchain::dbgs() << StatusMsg << ": " << FileName << ": " << LineNo << "\n";
}
#endif
