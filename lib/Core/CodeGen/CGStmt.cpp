...
void CodeGenFunction::EmitStmt(const Stmt *S, ArrayRef<const Attr *> Attrs) {
  ...
  case Stmt::MSDependentExistsStmtClass:
    toolchain_unreachable("invalid statement class to emit generically");
  ...
  case Stmt::SYCLKernelCallStmtClass:
    toolchain_unreachable("should have emitted these statements as simple");
...
#include "language/Core/AST/StmtNodes.inc"
  {
    ...
    toolchain::BasicBlock *incoming = Builder.GetInsertBlock();
    assert(incoming && "expression emission must have an insertion point");

    EmitIgnoredExpr(cast<Expr>(S));

    toolchain::BasicBlock *outgoing = Builder.GetInsertBlock();
    ...
    if (GetLast && ExprResult == CurStmt) {
      ...
      while (!isa<Expr>(ExprResult)) {
        if (const auto *LS = dyn_cast<LabelStmt>(ExprResult)) {
          ...
          toolchain_unreachable("unknown value statement");
        }
      }
...
void CodeGenFunction::SimplifyForwardingBlocks(toolchain::BasicBlock *BB) {
  toolchain::BranchInst *BI = dyn_cast<toolchain::BranchInst>(BB->getTerminator());
...
void CodeGenFunction::EmitBlock(toolchain::BasicBlock *BB, bool IsFinished) {
...
void CodeGenFunction::EmitBranch(toolchain::BasicBlock *Target) {
...
void CodeGenFunction::EmitBlockAfterUses(toolchain::BasicBlock *block) {
  bool inserted = false;
  for (toolchain::User *u : block->users()) {
    if (toolchain::Instruction *insn = dyn_cast<toolchain::Instruction>(u)) {
  ...
  for (const auto *A : S.getAttrs()) {
    ...
    case attr::CXXAssume: {
      ...
      if (getLangOpts().CXXAssumptions && Builder.GetInsertBlock() &&
        ...
        toolchain::Value *AssumptionVal = EmitCheckedArgForAssume(Assumption);
...
void CodeGenFunction::EmitIndirectGotoStmt(const IndirectGotoStmt &S) {
  ...
  toolchain::Value *V = Builder.CreateBitCast(EmitScalarExpr(S.getTarget()),
                                         Int8PtrTy, "addr");
  toolchain::BasicBlock *CurBB = Builder.GetInsertBlock();

  // Get the basic block for the indirect goto.
  toolchain::BasicBlock *IndGotoBB = GetIndirectGotoBlock();

  // The first instruction in the block has to be the PHI for the switch dest,
  // add an entry for this branch.
  cast<toolchain::PHINode>(IndGotoBB->begin())->addIncoming(V, CurBB);
...
void CodeGenFunction::EmitIfStmt(const IfStmt &S) {
  ...
  toolchain::BasicBlock *ThenBlock = createBasicBlock("if.then");
  toolchain::BasicBlock *ContBlock = createBasicBlock("if.end");
  toolchain::BasicBlock *ElseBlock = ContBlock;
  if (Else)
  ...
  if (!CGM.getCodeGenOpts().MCDCCoverage) {
    ...
    toolchain::Value *BoolCondVal = EvaluateExprAsBool(S.getCond());
    MaybeEmitDeferredVarDeclInit(S.getConditionVariable());
...
bool CodeGenFunction::checkIfLoopMustProgress(const Expr *ControllingExpression,
  ...
  if (CGM.getCodeGenOpts().getFiniteLoops() ==
    ...
    if (HasEmptyBody && CondIsTrue) {
      CurFn->removeFnAttr(toolchain::Attribute::MustProgress);
      return false;
    }
    return true;
  }
...
void CodeGenFunction::EmitWhileStmt(const WhileStmt &S,
  ...
  toolchain::ConstantInt *C = dyn_cast<toolchain::ConstantInt>(BoolCondVal);
  ...
  toolchain::BasicBlock *LoopBody = createBasicBlock("while.body");
  if (EmitBoolCondBranch) {
    toolchain::BasicBlock *ExitBlock = LoopExit.getBlock();
    if (ConditionScope.requiresCleanups())
      ExitBlock = createBasicBlock("while.exit");
    toolchain::MDNode *Weights =
    ...
    if (auto *CondI = dyn_cast<toolchain::Instruction>(BoolCondVal))
...
void CodeGenFunction::EmitDoStmt(const DoStmt &S,
  ...
  toolchain::BasicBlock *LoopBody = createBasicBlock("do.body");
...
void CodeGenFunction::EmitForStmt(const ForStmt &S,
  ...
  toolchain::BasicBlock *CondBlock = CondDest.getBlock();
  EmitBlock(CondBlock);
  ...
  if (S.getCond()) {
    ...
    // As long as the condition is true, iterate the loop.
    toolchain::BasicBlock *ForBody = createBasicBlock("for.body");
...
CodeGenFunction::EmitCXXForRangeStmt(const CXXForRangeStmt &S,
  ...
  // If there's an increment, the continue scope will be overwritten
  // later.
  toolchain::BasicBlock *CondBlock = createBasicBlock("for.cond");
  EmitBlock(CondBlock);
...
void CodeGenFunction::EmitReturnStmt(const ReturnStmt &S) {
  ...
  if (requiresReturnValueCheck()) {
    toolchain::Constant *SLoc = EmitCheckSourceLocation(S.getBeginLoc());
    auto *SLocPtr =
        new toolchain::GlobalVariable(CGM.getModule(), SLoc->getType(), false,
                                 toolchain::GlobalVariable::PrivateLinkage, SLoc);
    SLocPtr->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
  ...
  std::optional<toolchain::SaveAndRestore<const CallExpr *>> SaveMustTail;
  if (RV && CurFnInfo &&
  ...
  if (getLangOpts().ElideConstructors && S.getNRVOCandidate() &&
    ...
    if (toolchain::Value *NRVOFlag = NRVOFlags[S.getNRVOCandidate()])
    ...
    case TEK_Scalar: {
      toolchain::Value *Ret = EmitScalarExpr(RV);
...
void CodeGenFunction::EmitCaseStmtRange(const CaseStmt &S,
  ...
  toolchain::APSInt LHS = S.getLHS()->EvaluateKnownConstInt(getContext());
  toolchain::APSInt RHS = S.getRHS()->EvaluateKnownConstInt(getContext());

  // Emit the code for this case. We do this first to make sure it is
  // properly chained from our predecessor before generating the
  // switch machinery to enter this block.
  toolchain::BasicBlock *CaseDest = createBasicBlock("sw.bb");
  ...
  toolchain::APInt Range = RHS - LHS;
  // FIXME: parameters such as this should not be hardcoded.
  if (Range.ult(toolchain::APInt(Range.getBitWidth(), 64))) {
  ...
  toolchain::BasicBlock *RestoreBB = Builder.GetInsertBlock();

  // Push this test onto the chain of range checks (which terminates
  // in the default basic block). The switch's default will be changed
  // to the top of this chain after switch emission is complete.
  toolchain::BasicBlock *FalseDest = CaseRangeBlock;
      ...
      if (CGDebugInfo *Dbg = getDebugInfo())
        if (CGM.getCodeGenOpts().hasReducedDebugInfo())
          Dbg->EmitGlobalVariable(DE->getDecl(),
              APValue(toolchain::APSInt(CaseVal->getValue())));
  }
...
void CodeGenFunction::EmitDefaultStmt(const DefaultStmt &S,
  ...
  toolchain::BasicBlock *DefaultBlock = SwitchInsn->getDefaultDest();
...
static bool FindCaseStatementsForValue(const SwitchStmt &S,
                                       const toolchain::APSInt &ConstantCondValue,
                                SmallVectorImpl<const Stmt*> &ResultStmts,
                                       ASTContext &C,
...
void CodeGenFunction::EmitSwitchStmt(const SwitchStmt &S) {
  ...
  toolchain::SwitchInst *SavedSwitchInsn = SwitchInsn;
  SmallVector<uint64_t, 16> *SavedSwitchWeights = SwitchWeights;
  SmallVector<Stmt::Likelihood, 16> *SavedSwitchLikelihood = SwitchLikelihood;
  toolchain::BasicBlock *SavedCRBlock = CaseRangeBlock;

  // See if we can constant fold the condition of the switch and therefore only
  // emit the live case statement (if any) of the switch.
  toolchain::APSInt ConstantCondValue;
  ...
  if (S.getConditionVariable())
    EmitDecl(*S.getConditionVariable());
  toolchain::Value *CondV = EmitScalarExpr(S.getCond());
  MaybeEmitDeferredVarDeclInit(S.getConditionVariable());
  ...
  // failure.
  toolchain::BasicBlock *DefaultBlock = createBasicBlock("sw.default");
  ...
  if (HLSLControlFlowAttr != HLSLControlFlowHintAttr::SpellingNotCalculated) {
    ...
    toolchain::Metadata *Vals[] = {MDHelper.createString("hlsl.controlflow.hint"),
  ...
  if (Call && CGM.getCodeGenOpts().OptimizationLevel != 0) {
    ...
    if (FD && FD->getBuiltinID() == Builtin::BI__builtin_unpredictable) {
      ...
      SwitchInsn->setMetadata(toolchain::LLVMContext::MD_unpredictable,
  ...
  if (SwitchWeights) {
    ...
    if (SwitchWeights->size() > 1)
      SwitchInsn->setMetadata(toolchain::LLVMContext::MD_prof,
...
std::pair<toolchain::Value*, toolchain::Type *> CodeGenFunction::EmitAsmInputLValue(
  ...
  if (Info.allowsRegister() || !Info.allowsMemory()) {
    ...
    toolchain::Type *Ty = ConvertType(InputType);
    uint64_t Size = CGM.getDataLayout().getTypeSizeInBits(Ty);
    if ((Size <= 64 && toolchain::isPowerOf2_64(Size)) ||
        getTargetHooks().isScalarizableAsmOperand(*this, Ty)) {
      Ty = toolchain::IntegerType::get(getLLVMContext(), Size);
...
CodeGenFunction::EmitAsmInput(const TargetInfo::ConstraintInfo &Info,
  ...
  if (!Info.allowsRegister() && !Info.allowsMemory()) {
    if (Info.requiresImmediateConstant()) {
      ...
      toolchain::APSInt IntResult;
      if (EVResult.Val.toIntegralConstant(IntResult, InputExpr->getType(),
                                          getContext()))
        return {toolchain::ConstantInt::get(getLLVMContext(), IntResult), nullptr};
    }

    Expr::EvalResult Result;
    if (InputExpr->EvaluateAsInt(Result, getContext()))
      return {toolchain::ConstantInt::get(getLLVMContext(), Result.Val.getInt()),
              nullptr};
  }
...
/// asm.
static toolchain::MDNode *getAsmSrcLocInfo(const StringLiteral *Str,
                                      CodeGenFunction &CGF) {
  SmallVector<toolchain::Metadata *, 8> Locs;
  // Add the location of the first line to the MDNode.
  Locs.push_back(toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::get(
  ...
  if (!StrVal.empty()) {
    ...
    for (unsigned i = 0, e = StrVal.size() - 1; i != e; ++i) {
      ...
      Locs.push_back(toolchain::ConstantAsMetadata::get(
          toolchain::ConstantInt::get(CGF.Int64Ty, LineLoc.getRawEncoding())));
    }
  }

  return toolchain::MDNode::get(CGF.getLLVMContext(), Locs);
}

static void UpdateAsmCallInst(toolchain::CallBase &Result, bool HasSideEffect,
                              bool HasUnwindClobber, bool ReadOnly,
                              bool ReadNone, bool NoMerge, bool NoConvergent,
                              const AsmStmt &S,
                              const std::vector<toolchain::Type *> &ResultRegTypes,
                              const std::vector<toolchain::Type *> &ArgElemTypes,
                              CodeGenFunction &CGF,
                              std::vector<toolchain::Value *> &RegResults) {
  if (!HasUnwindClobber)
    Result.addFnAttr(toolchain::Attribute::NoUnwind);

  if (NoMerge)
    Result.addFnAttr(toolchain::Attribute::NoMerge);
  ...
  for (auto Pair : toolchain::enumerate(ArgElemTypes)) {
    if (Pair.value()) {
      auto Attr = toolchain::Attribute::get(
          CGF.getLLVMContext(), toolchain::Attribute::ElementType, Pair.value());
  ...
  if (const auto *gccAsmStmt = dyn_cast<GCCAsmStmt>(&S);
    ...
    // strings.
    toolchain::Constant *Loc =
        toolchain::ConstantInt::get(CGF.Int64Ty, S.getAsmLoc().getRawEncoding());
    Result.setMetadata("srcloc",
                       toolchain::MDNode::get(CGF.getLLVMContext(),
                                         toolchain::ConstantAsMetadata::get(Loc)));
  }
  ...
  if (!NoConvergent && CGF.getLangOpts().assumeFunctionsAreConvergent())
    ...
    Result.addFnAttr(toolchain::Attribute::Convergent);
  // Extract all of the register value results from the asm.
  if (ResultRegTypes.size() == 1) {
    RegResults.push_back(&Result);
  } else {
    for (unsigned i = 0, e = ResultRegTypes.size(); i != e; ++i) {
      toolchain::Value *Tmp = CGF.Builder.CreateExtractValue(&Result, i, "asmresult");
...
EmitAsmStores(CodeGenFunction &CGF, const AsmStmt &S,
              const toolchain::ArrayRef<toolchain::Value *> RegResults,
              const toolchain::ArrayRef<toolchain::Type *> ResultRegTypes,
              const toolchain::ArrayRef<toolchain::Type *> ResultTruncRegTypes,
              const toolchain::ArrayRef<LValue> ResultRegDests,
              const toolchain::ArrayRef<QualType> ResultRegQualTys,
              const toolchain::BitVector &ResultTypeRequiresCast,
              const toolchain::BitVector &ResultRegIsFlagReg) {
  CGBuilderTy &Builder = CGF.Builder;
  CodeGenModule &CGM = CGF.CGM;
  toolchain::LLVMContext &CTX = CGF.getLLVMContext();
  ...
  for (unsigned i = 0, e = RegResults.size(); i != e; ++i) {
    toolchain::Value *Tmp = RegResults[i];
    toolchain::Type *TruncTy = ResultTruncRegTypes[i];

    if ((i < ResultRegIsFlagReg.size()) && ResultRegIsFlagReg[i]) {
      // Target must guarantee the Value `Tmp` here is lowered to a boolean
      // value.
      toolchain::Constant *Two = toolchain::ConstantInt::get(Tmp->getType(), 2);
      toolchain::Value *IsBooleanValue =
          Builder.CreateCmp(toolchain::CmpInst::ICMP_ULT, Tmp, Two);
      toolchain::Function *FnAssume = CGM.getIntrinsic(toolchain::Intrinsic::assume);
    ...
    if (ResultRegTypes[i] != TruncTy) {
      ...
      if (TruncTy->isFloatingPointTy())
        ...
        Tmp = Builder.CreateTrunc(
            Tmp, toolchain::IntegerType::get(CTX, (unsigned)ResSize));
        Tmp = Builder.CreateIntToPtr(Tmp, TruncTy);
      } else if (Tmp->getType()->isPointerTy() && TruncTy->isIntegerTy()) {
        uint64_t TmpSize =
            CGM.getDataLayout().getTypeSizeInBits(Tmp->getType());
        Tmp = Builder.CreatePtrToInt(
            Tmp, toolchain::IntegerType::get(CTX, (unsigned)TmpSize));
    ...
    if ((i < ResultTypeRequiresCast.size()) && ResultTypeRequiresCast[i]) {
      ...
      if (CGF.getTargetHooks().isScalarizableAsmOperand(CGF, TruncTy)) {
        toolchain::StoreInst *S = Builder.CreateStore(Tmp, A);
...
static void EmitHipStdParUnsupportedAsm(CodeGenFunction *CGF,
  ...
  auto StrTy = toolchain::ConstantDataArray::getString(Ctx, Asm);
  auto FnTy = toolchain::FunctionType::get(toolchain::Type::getVoidTy(Ctx),
                                      {StrTy->getType()}, false);
...
void CodeGenFunction::EmitAsmStmt(const AsmStmt &S) {
  ...
  std::vector<toolchain::Type *> ResultRegTypes;
  std::vector<toolchain::Type *> ResultTruncRegTypes;
  std::vector<toolchain::Type *> ArgTypes;
  std::vector<toolchain::Type *> ArgElemTypes;
  std::vector<toolchain::Value*> Args;
  toolchain::BitVector ResultTypeRequiresCast;
  toolchain::BitVector ResultRegIsFlagReg;

  // Keep track of inout constraints.
  std::string InOutConstraints;
  std::vector<toolchain::Value*> InOutArgs;
  std::vector<toolchain::Type*> InOutArgTypes;
  std::vector<toolchain::Type*> InOutArgElemTypes;

  // Keep track of out constraints for tied input operand.
  std::vector<std::string> OutputConstraints;

  // Keep track of defined physregs.
  toolchain::SmallSet<std::string, 8> PhysRegOutputs;
  ...
  for (unsigned i = 0, e = S.getNumOutputs(); i != e; i++) {
    ...
    if (!Info.allowsMemory() && IsScalarOrAggregate) {
      ...
      bool IsFlagReg = toolchain::StringRef(OutputConstraint).starts_with("{@cc");
      ResultRegIsFlagReg.push_back(IsFlagReg);

      toolchain::Type *Ty = ConvertTypeForMem(QTy);
      ...
      if (auto *VT = dyn_cast<toolchain::VectorType>(ResultRegTypes.back()))
        LargestVectorWidth =
    ...
    if (Info.isReadWrite()) {
      ...
      // Update largest vector width for any vector types.
      if (auto *VT = dyn_cast<toolchain::VectorType>(Arg->getType()))
        LargestVectorWidth =
            std::max((uint64_t)LargestVectorWidth,
                     VT->getPrimitiveSizeInBits().getKnownMinValue());
      // Only tie earlyclobber physregs.
      if (Info.allowsRegister() && (GCCReg.empty() || Info.earlyClobber()))
        InOutConstraints += toolchain::utostr(i);
      else
  ...
  for (unsigned i = 0, e = S.getNumInputs(); i != e; i++) {
    ...
    if (Info.hasTiedOperand()) {
      ...
      if (getContext().getTypeSize(OutputType) >
        ...
        if (isa<toolchain::PointerType>(Arg->getType()))
          Arg = Builder.CreatePtrToInt(Arg, IntPtrTy);
        toolchain::Type *OutputTy = ConvertType(OutputType);
        if (isa<toolchain::IntegerType>(OutputTy))
          Arg = Builder.CreateZExt(Arg, OutputTy);
        else if (isa<toolchain::PointerType>(OutputTy))
  ...
  // Labels
  SmallVector<toolchain::BasicBlock *, 16> Transfer;
  toolchain::BasicBlock *Fallthrough = nullptr;
  ...
  if (ResultRegTypes.empty())
    ...
    ResultType = toolchain::StructType::get(getLLVMContext(), ResultRegTypes);

  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(ResultType, ArgTypes, false);

  bool HasSideEffect = S.isVolatile() || S.getNumOutputs() == 0;

  toolchain::InlineAsm::AsmDialect GnuAsmDialect =
      CGM.getCodeGenOpts().getInlineAsmDialect() == CodeGenOptions::IAD_ATT
          ? toolchain::InlineAsm::AD_ATT
          : toolchain::InlineAsm::AD_Intel;
  toolchain::InlineAsm::AsmDialect AsmDialect = isa<MSAsmStmt>(&S) ?
    toolchain::InlineAsm::AD_Intel : GnuAsmDialect;

  toolchain::InlineAsm *IA = toolchain::InlineAsm::get(
      FTy, AsmString, Constraints, HasSideEffect,
      /* IsAlignStack */ false, AsmDialect, HasUnwindClobber);
  std::vector<toolchain::Value*> RegResults;
  toolchain::CallBrInst *CBR;
  toolchain::DenseMap<toolchain::BasicBlock *, SmallVector<toolchain::Value *, 4>>
      CBRRegResults;
  if (IsGCCAsmGoto) {
    ...
    if (!RegResults.empty()) {
      unsigned i = 0;
      for (toolchain::BasicBlock *Dest : CBR->getIndirectDests()) {
        toolchain::Twine SynthName = Dest->getName() + ".split";
        toolchain::BasicBlock *SynthBB = createBasicBlock(SynthName);
        ...
        if (ResultRegTypes.size() == 1) {
          ...
          for (unsigned j = 0, e = ResultRegTypes.size(); j != e; ++j) {
            toolchain::Value *Tmp = Builder.CreateExtractValue(CBR, j, "asmresult");
  ...
  if (IsGCCAsmGoto && !CBRRegResults.empty()) {
    for (toolchain::BasicBlock *Succ : CBR->getIndirectDests()) {
      toolchain::IRBuilderBase::InsertPointGuard IPG(Builder);
...
CodeGenFunction::GenerateCapturedStmtFunction(const CapturedStmt &S) {
  ...
  if (CD->isNothrow())
    F->addFnAttr(toolchain::Attribute::NoUnwind);
...
static toolchain::ConvergenceControlInst *getConvergenceToken(toolchain::BasicBlock *BB) {
  for (auto &I : *BB) {
    if (auto *CI = dyn_cast<toolchain::ConvergenceControlInst>(&I))
      return CI;
  }
  return nullptr;
}
...
toolchain::CallBase *
CodeGenFunction::addConvergenceControlToken(toolchain::CallBase *Input) {
  toolchain::ConvergenceControlInst *ParentToken = ConvergenceTokenStack.back();
  assert(ParentToken);

  toolchain::Value *bundleArgs[] = {ParentToken};
  toolchain::OperandBundleDef OB("convergencectrl", bundleArgs);
  auto *Output = toolchain::CallBase::addOperandBundle(
      Input, toolchain::LLVMContext::OB_convergencectrl, OB, Input->getIterator());
...
CodeGenFunction::emitConvergenceLoopToken(toolchain::BasicBlock *BB) {
  toolchain::ConvergenceControlInst *ParentToken = ConvergenceTokenStack.back();
  assert(ParentToken);
  return toolchain::ConvergenceControlInst::CreateLoop(*BB, ParentToken);
}

toolchain::ConvergenceControlInst *
CodeGenFunction::getOrEmitConvergenceEntryToken(toolchain::Function *F) {
  toolchain::BasicBlock *BB = &F->getEntryBlock();
  toolchain::ConvergenceControlInst *Token = getConvergenceToken(BB);
  if (Token)
    return Token;
  ...
  // convergent.
  F->setConvergent();
  return toolchain::ConvergenceControlInst::CreateEntry(*BB);
}
