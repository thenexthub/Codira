...
createArrayWithNewElementType(CodeGenModule &CGM,
                              ...
                              toolchain::Type *NewElemType) {
  const language::Core::Type *ArrayElemType = ArrayType->getArrayElementTypeNoTypeQual();
  ...
  return toolchain::ArrayType::get(NewElemType, ArrayType->getSExtSize());
}

// Returns the size of a scalar or vector in bytes
static unsigned getScalarOrVectorSizeInBytes(toolchain::Type *Ty) {
  assert(Ty->isVectorTy() || Ty->isIntegerTy() || Ty->isFloatingPointTy());
  if (Ty->isVectorTy()) {
    toolchain::FixedVectorType *FVT = cast<toolchain::FixedVectorType>(Ty);
...
toolchain::TargetExtType *HLSLBufferLayoutBuilder::createLayoutType(
    const RecordType *RT, const toolchain::SmallVector<int32_t> *PackOffsets) {

  // check if we already have the layout type for this struct
  if (toolchain::TargetExtType *Ty =
          CGM.getHLSLRuntime().getHLSLBufferLayoutType(RT))
    return Ty;

  SmallVector<unsigned> Layout;
  SmallVector<toolchain::Type *> LayoutElements;
  ...
  toolchain::StructType *StructTy =
      toolchain::StructType::create(LayoutElements, Name, true);

  // create target layout type
  toolchain::TargetExtType *NewLayoutTy = toolchain::TargetExtType::get(
...
bool HLSLBufferLayoutBuilder::layoutField(const FieldDecl *FD,
  ...
  unsigned NextRowOffset = toolchain::alignTo(EndOffset, CBufferRowSizeInBytes);

  toolchain::Type *ElemLayoutTy = nullptr;
  ...
  if (FieldTy->isConstantArrayType()) {
    ...
    if (Ty->isStructureOrClassType()) {
      toolchain::Type *NewTy =
          cast<toolchain::TargetExtType>(createLayoutType(Ty->getAs<RecordType>()));
      if (!NewTy)
        return false;
      assert(isa<toolchain::TargetExtType>(NewTy) && "expected target type");
      ElemSize = cast<toolchain::TargetExtType>(NewTy)->getIntParameter(0);
    ...
    ArrayStride = toolchain::alignTo(ElemSize, CBufferRowSizeInBytes);
    ...
    if (!ElemLayoutTy)
      return false;
    assert(isa<toolchain::TargetExtType>(ElemLayoutTy) && "expected target type");
    ElemSize = cast<toolchain::TargetExtType>(ElemLayoutTy)->getIntParameter(0);
    ElemOffset = (Packoffset != -1) ? Packoffset : NextRowOffset;
    ...
    if (ElemLayoutTy->isVectorTy()) {
      ...
      const toolchain::FixedVectorType *FVT =
          cast<toolchain::FixedVectorType>(ElemLayoutTy);
    ...
    if (Packoffset != -1) {
      ...
      ElemOffset = toolchain::alignTo(EndOffset, Align);
