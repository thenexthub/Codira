...
#include "language/Core/CodeGen/SwiftCallingConv.h"
#include "toolchain/ADT/StringExtras.h"
#include "toolchain/Analysis/ValueTracking.h"
#include "toolchain/IR/Assumptions.h"
...
#include "toolchain/Transforms/Utils/Local.h"
#include <optional>
...
unsigned CodeGenTypes::ClangCallConvToLLVMCallConv(CallingConv CC) {
  switch (CC) {
  default:
    return toolchain::CallingConv::C;
  case CC_X86StdCall:
    return toolchain::CallingConv::X86_StdCall;
  case CC_X86FastCall:
    return toolchain::CallingConv::X86_FastCall;
  case CC_X86RegCall:
    return toolchain::CallingConv::X86_RegCall;
  case CC_X86ThisCall:
    return toolchain::CallingConv::X86_ThisCall;
  case CC_Win64:
    return toolchain::CallingConv::Win64;
  case CC_X86_64SysV:
    return toolchain::CallingConv::X86_64_SysV;
  case CC_AAPCS:
    return toolchain::CallingConv::ARM_AAPCS;
  case CC_AAPCS_VFP:
    return toolchain::CallingConv::ARM_AAPCS_VFP;
  case CC_IntelOclBicc:
    return toolchain::CallingConv::Intel_OCL_BI;
  // TODO: Add support for __pascal to LLVM.
  case CC_X86Pascal:
    return toolchain::CallingConv::C;
  // TODO: Add support for __vectorcall to LLVM.
  case CC_X86VectorCall:
    return toolchain::CallingConv::X86_VectorCall;
  case CC_AArch64VectorCall:
    return toolchain::CallingConv::AArch64_VectorCall;
  case CC_AArch64SVEPCS:
    return toolchain::CallingConv::AArch64_SVE_VectorCall;
  case CC_SpirFunction:
    return toolchain::CallingConv::SPIR_FUNC;
  case CC_DeviceKernel:
    return CGM.getTargetCodeGenInfo().getDeviceKernelCallingConv();
  case CC_PreserveMost:
    return toolchain::CallingConv::PreserveMost;
  case CC_PreserveAll:
    return toolchain::CallingConv::PreserveAll;
  case CC_Swift:
    return toolchain::CallingConv::Swift;
  case CC_SwiftAsync:
    return toolchain::CallingConv::SwiftTail;
  case CC_M68kRTD:
    return toolchain::CallingConv::M68k_RTD;
  case CC_PreserveNone:
    return toolchain::CallingConv::PreserveNone;
    // clang-format off
  case CC_RISCVVectorCall: return toolchain::CallingConv::RISCV_VectorCall;
    // clang-format on
#define CC_VLS_CASE(ABI_VLEN)                                                  \
  case CC_RISCVVLSCall_##ABI_VLEN:                                             \
    return toolchain::CallingConv::RISCV_VLSCall_##ABI_VLEN;
    CC_VLS_CASE(32)
    CC_VLS_CASE(64)
    CC_VLS_CASE(128)
    CC_VLS_CASE(256)
...
static void addExtParameterInfosForCall(
    toolchain::SmallVectorImpl<FunctionProtoType::ExtParameterInfo> &paramInfos,
...
static CallingConv getCallingConventionForDecl(const ObjCMethodDecl *D,
  ...
  if (RISCVVLSCCAttr *PCS = D->getAttr<RISCVVLSCCAttr>()) {
    ...
    default:
      toolchain_unreachable("Invalid RISC-V VLS ABI VLEN");
#define CC_VLS_CASE(ABI_VLEN)                                                  \
  case ABI_VLEN:                                                               \
...
const CGFunctionInfo &CodeGenTypes::arrangeLLVMFunctionInfo(
    ...
    RequiredArgs required) {
  assert(toolchain::all_of(argTypes,
...
CGFunctionInfo *CGFunctionInfo::create(unsigned toolchainCC, bool instanceMethod,
  ...
  FI->CallingConvention = toolchainCC;
  FI->EffectiveCallingConvention = toolchainCC;
  ...
  if (const RecordType *RT = Ty->getAs<RecordType>()) {
    ...
    if (RD->isUnion()) {
      ...
      if (const auto *CXXRD = dyn_cast<CXXRecordDecl>(RD)) {
        ...
        toolchain::append_range(Bases, toolchain::make_pointer_range(CXXRD->bases()));
      }
...
void CodeGenTypes::getExpandedTypes(
    QualType Ty, SmallVectorImpl<toolchain::Type *>::iterator &TI) {
  auto Exp = getTypeExpansion(Ty, Context);
  if (auto CAExp = dyn_cast<ConstantArrayExpansion>(Exp.get())) {
    ...
    toolchain::Type *EltTy = ConvertType(CExp->EltTy);
    *TI++ = EltTy;
    *TI++ = EltTy;
  } else {
    assert(isa<NoExpansion>(Exp.get()));
...
static void forConstantArrayExpansion(CodeGenFunction &CGF,
                                      ...
                                      Address BaseAddr,
                                      toolchain::function_ref<void(Address)> Fn) {
...
void CodeGenFunction::ExpandTypeToArgs(
    QualType Ty, CallArg Arg, toolchain::FunctionType *IRFuncTy,
    SmallVectorImpl<toolchain::Value *> &IRCallArgs, unsigned &IRCallArgPos) {
  auto Exp = getTypeExpansion(Ty, getContext());
  if (auto CAExp = dyn_cast<ConstantArrayExpansion>(Exp.get())) {
...
static Address EnterStructPointerForCoercedAccess(Address SrcPtr,
  ...
  toolchain::Type *FirstElt = SrcSTy->getElementType(0);
  ...
  toolchain::Type *SrcTy = SrcPtr.getElementType();
  if (toolchain::StructType *SrcSTy = dyn_cast<toolchain::StructType>(SrcTy))
...
static toolchain::Value *CoerceIntOrPtrToIntOrPtr(toolchain::Value *Val, toolchain::Type *Ty,
                                             CodeGenFunction &CGF) {
  if (Val->getType() == Ty)
    return Val;

  if (isa<toolchain::PointerType>(Val->getType())) {
    // If this is Pointer->Pointer avoid conversion to and from int.
    if (isa<toolchain::PointerType>(Ty))
  ...
  toolchain::Type *DestIntTy = Ty;
  if (isa<toolchain::PointerType>(DestIntTy))
...
static toolchain::Value *CreateCoercedLoad(Address Src, toolchain::Type *Ty,
                                      CodeGenFunction &CGF) {
  toolchain::Type *SrcTy = Src.getElementType();

  // If SrcTy and Ty are the same, just do a load.
  if (SrcTy == Ty)
    return CGF.Builder.CreateLoad(Src);

  toolchain::TypeSize DstSize = CGF.CGM.getDataLayout().getTypeAllocSize(Ty);
  ...
  toolchain::TypeSize SrcSize = CGF.CGM.getDataLayout().getTypeAllocSize(SrcTy);

  // If the source and destination are integer or pointer types, just do an
  // extension or truncation to the desired type.
  if ((isa<toolchain::IntegerType>(Ty) || isa<toolchain::PointerType>(Ty)) &&
      (isa<toolchain::IntegerType>(SrcTy) || isa<toolchain::PointerType>(SrcTy))) {
    toolchain::Value *Load = CGF.Builder.CreateLoad(Src);
  ...
  // the types match, use the toolchain.vector.insert intrinsic to perform the
  // conversion.
  if (auto *ScalableDstTy = dyn_cast<toolchain::ScalableVectorType>(Ty)) {
    if (auto *FixedSrcTy = dyn_cast<toolchain::FixedVectorType>(SrcTy)) {
      // If we are casting a fixed i8 vector to a scalable i1 predicate
      // vector, use a vector insert and bitcast the result.
      if (ScalableDstTy->getElementType()->isIntegerTy(1) &&
          FixedSrcTy->getElementType()->isIntegerTy(8)) {
        ScalableDstTy = toolchain::ScalableVectorType::get(
            FixedSrcTy->getElementType(),
            toolchain::divideCeil(
                ScalableDstTy->getElementCount().getKnownMinValue(), 8));
      }
      if (ScalableDstTy->getElementType() == FixedSrcTy->getElementType()) {
        auto *Load = CGF.Builder.CreateLoad(Src);
        auto *PoisonVec = toolchain::PoisonValue::get(ScalableDstTy);
        toolchain::Value *Result = CGF.Builder.CreateInsertVector(
            ScalableDstTy, PoisonVec, Load, uint64_t(0), "cast.scalable");
        ScalableDstTy = cast<toolchain::ScalableVectorType>(
            toolchain::VectorType::getWithSizeAndScalar(ScalableDstTy, Ty));
  ...
  CGF.Builder.CreateMemCpy(
      ...
      toolchain::ConstantInt::get(CGF.IntPtrTy, SrcSize.getKnownMinValue()));
  return CGF.Builder.CreateLoad(Tmp);
}

void CodeGenFunction::CreateCoercedStore(toolchain::Value *Src, Address Dst,
                                         toolchain::TypeSize DstSize,
                                         bool DstIsVolatile) {
  if (!DstSize)
    return;

  toolchain::Type *SrcTy = Src->getType();
  toolchain::TypeSize SrcSize = CGM.getDataLayout().getTypeAllocSize(SrcTy);
  ...
  if (SrcTy != Dst.getElementType()) {
    if (toolchain::StructType *DstSTy =
            dyn_cast<toolchain::StructType>(Dst.getElementType())) {
  ...
  if (SrcSize.isScalable() || SrcSize <= DstSize) {
    ...
    } else if (toolchain::StructType *STy =
                   dyn_cast<toolchain::StructType>(Src->getType())) {
      // Prefer scalar stores to first-class aggregate stores.
      Dst = Dst.withElementType(SrcTy);
      for (unsigned i = 0, e = STy->getNumElements(); i != e; ++i) {
        Address EltPtr = Builder.CreateStructGEP(Dst, i);
        toolchain::Value *Elt = Builder.CreateExtractValue(Src, i);
    ...
    toolchain::Type *DstIntTy = Builder.getIntNTy(DstSize.getFixedValue() * 8);
...
static std::pair<toolchain::Value *, bool>
CoerceScalableToFixed(CodeGenFunction &CGF, toolchain::FixedVectorType *ToTy,
                      toolchain::ScalableVectorType *FromTy, toolchain::Value *V,
                      StringRef Name = "") {
  ...
  if (FromTy->getElementType()->isIntegerTy(1) &&
    ...
    if (!FromTy->getElementCount().isKnownMultipleOf(8)) {
      FromTy = toolchain::ScalableVectorType::get(
          FromTy->getElementType(),
          toolchain::alignTo<8>(FromTy->getElementCount().getKnownMinValue()));
      toolchain::Value *ZeroVec = toolchain::Constant::getNullValue(FromTy);
...
void ClangToLLVMArgMapping::construct(const ASTContext &Context,
  ...
  for (CGFunctionInfo::const_arg_iterator I = FI.arg_begin(); ArgNo < NumArgs;
    ...
    case ABIArgInfo::Direct: {
      ...
      toolchain::StructType *STy = dyn_cast<toolchain::StructType>(AI.getCoerceToType());
...
toolchain::FunctionType *CodeGenTypes::GetFunctionType(GlobalDecl GD) {
  const CGFunctionInfo &FI = arrangeGlobalDeclaration(GD);
  return GetFunctionType(FI);
}

toolchain::FunctionType *CodeGenTypes::GetFunctionType(const CGFunctionInfo &FI) {

  bool Inserted = FunctionsBeingProcessed.insert(&FI).second;
  (void)Inserted;
  assert(Inserted && "Recursively being processed?");

  toolchain::Type *resultType = nullptr;
  const ABIArgInfo &retAI = FI.getReturnInfo();
  switch (retAI.getKind()) {
  case ABIArgInfo::Expand:
  case ABIArgInfo::IndirectAliased:
    toolchain_unreachable("Invalid ABI kind for return argument");
  ...
  case ABIArgInfo::InAlloca:
    if (retAI.getInAllocaSRet()) {
      ...
      resultType = toolchain::PointerType::get(getLLVMContext(), addressSpace);
    } else {
      resultType = toolchain::Type::getVoidTy(getLLVMContext());
    }
    break;
  ...
  SmallVector<toolchain::Type *, 8> ArgTypes(IRFunctionArgs.totalIRArgs());

  // Add type for sret argument.
  if (IRFunctionArgs.hasSRetArg()) {
    ArgTypes[IRFunctionArgs.getSRetArgNo()] = toolchain::PointerType::get(
  ...
  if (IRFunctionArgs.hasInallocaArg())
    ArgTypes[IRFunctionArgs.getInallocaArgNo()] =
        toolchain::PointerType::getUnqual(getLLVMContext());
  ...
  for (; it != ie; ++it, ++ArgNo) {
    ...
    case ABIArgInfo::Indirect:
      ...
      ArgTypes[FirstIRArg] = toolchain::PointerType::get(
    ...
    case ABIArgInfo::Direct: {
      ...
      toolchain::Type *argType = ArgInfo.getCoerceToType();
      toolchain::StructType *st = dyn_cast<toolchain::StructType>(argType);
  ...
  return toolchain::FunctionType::get(resultType, ArgTypes, FI.isVariadic());
}

toolchain::Type *CodeGenTypes::GetFunctionTypeForVTable(GlobalDecl GD) {
  const CXXMethodDecl *MD = cast<CXXMethodDecl>(GD.getDecl());
  const FunctionProtoType *FPT = MD->getType()->castAs<FunctionProtoType>();

  if (!isFuncTypeConvertible(FPT))
    return toolchain::StructType::get(getLLVMContext());
...
static void AddAttributesFromOMPAssumes(toolchain::AttrBuilder &FuncAttrs,
                                        const Decl *Callee) {
  if (!Callee)
    return;
  ...
  if (!Attrs.empty())
    FuncAttrs.addAttribute(toolchain::AssumptionAttrKey,
                           toolchain::join(Attrs.begin(), Attrs.end(), ","));
}
...
/// -f32 case.
static void addDenormalModeAttrs(toolchain::DenormalMode FPDenormalMode,
                                 toolchain::DenormalMode FP32DenormalMode,
                                 toolchain::AttrBuilder &FuncAttrs) {
  if (FPDenormalMode != toolchain::DenormalMode::getDefault())
...
static void getTrivialDefaultFunctionAttributes(
  ...
  if (!HasOptnone) {
    if (CodeGenOpts.OptimizeSize)
      FuncAttrs.addAttribute(toolchain::Attribute::OptimizeForSize);
    if (CodeGenOpts.OptimizeSize == 2)
      FuncAttrs.addAttribute(toolchain::Attribute::MinSize);
  }

  if (CodeGenOpts.DisableRedZone)
    FuncAttrs.addAttribute(toolchain::Attribute::NoRedZone);
  if (CodeGenOpts.IndirectTlsSegRefs)
    FuncAttrs.addAttribute("indirect-tls-seg-refs");
  if (CodeGenOpts.NoImplicitFloat)
    FuncAttrs.addAttribute(toolchain::Attribute::NoImplicitFloat);
  ...
  if (AttrOnCallSite) {
    ...
    if (!CodeGenOpts.SimplifyLibCalls || LangOpts.isNoBuiltinFunc(Name))
      FuncAttrs.addAttribute(toolchain::Attribute::NoBuiltin);
    ...
    if (CodeGenOpts.NullPointerIsValid)
      FuncAttrs.addAttribute(toolchain::Attribute::NullPointerIsValid);
    ...
    FuncAttrs.addAttribute("stack-protector-buffer-size",
                           toolchain::utostr(CodeGenOpts.SSPBufferSize));
    ...
    if (!Recips.empty())
      FuncAttrs.addAttribute("reciprocal-estimates", toolchain::join(Recips, ","));
    ...
    if (CodeGenOpts.SpeculativeLoadHardening)
      FuncAttrs.addAttribute(toolchain::Attribute::SpeculativeLoadHardening);

    // Add zero-call-used-regs attribute.
    switch (CodeGenOpts.getZeroCallUsedRegs()) {
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::Skip:
      FuncAttrs.removeAttribute("zero-call-used-regs");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::UsedGPRArg:
      FuncAttrs.addAttribute("zero-call-used-regs", "used-gpr-arg");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::UsedGPR:
      FuncAttrs.addAttribute("zero-call-used-regs", "used-gpr");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::UsedArg:
      FuncAttrs.addAttribute("zero-call-used-regs", "used-arg");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::Used:
      FuncAttrs.addAttribute("zero-call-used-regs", "used");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::AllGPRArg:
      FuncAttrs.addAttribute("zero-call-used-regs", "all-gpr-arg");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::AllGPR:
      FuncAttrs.addAttribute("zero-call-used-regs", "all-gpr");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::AllArg:
      FuncAttrs.addAttribute("zero-call-used-regs", "all-arg");
      break;
    case toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind::All:
  ...
  if (LangOpts.assumeFunctionsAreConvergent()) {
    ...
    // can.
    FuncAttrs.addAttribute(toolchain::Attribute::Convergent);
  }
...
/// from \F
static void
overrideFunctionFeaturesWithTargetFeatures(toolchain::AttrBuilder &FuncAttr,
                                           const toolchain::Function &F,
                                           const TargetOptions &TargetOpts) {
  auto FFeatures = F.getFnAttribute("target-features");

  toolchain::StringSet<> MergedNames;
  ...
  if (FFeatures.isValid())
    AddUnmergedFeatures(toolchain::split(FFeatures.getValueAsString(), ','));
  AddUnmergedFeatures(TargetOpts.Features);

  if (!MergedFeatures.empty()) {
    toolchain::sort(MergedFeatures);
    FuncAttr.addAttribute("target-features", toolchain::join(MergedFeatures, ","));
  }
}

void CodeGen::mergeDefaultFunctionDefinitionAttributes(
    toolchain::Function &F, const CodeGenOptions &CodeGenOpts,
    const LangOptions &LangOpts, const TargetOptions &TargetOpts,
    bool WillInternalize) {

  toolchain::AttrBuilder FuncAttrs(F.getContext());
  ...
  toolchain::AttributeMask AttrsToRemove;

  toolchain::DenormalMode DenormModeToMerge = F.getDenormalModeRaw();
  toolchain::DenormalMode DenormModeToMergeF32 = F.getDenormalModeF32Raw();
  toolchain::DenormalMode Merged =
      CodeGenOpts.FPDenormalMode.mergeCalleeMode(DenormModeToMerge);
  toolchain::DenormalMode MergedF32 = CodeGenOpts.FP32DenormalMode;
  ...
  if (Merged == toolchain::DenormalMode::getDefault()) {
  ...
  if (MergedF32 == toolchain::DenormalMode::getDefault()) {
...
static void addNoBuiltinAttributes(toolchain::AttrBuilder &FuncAttrs,
  ...
  toolchain::for_each(LangOpts.NoBuiltinFuncs, AddNoBuiltinAttr);
  ...
  if (toolchain::is_contained(NBA->builtinNames(), "*")) {
    FuncAttrs.addAttribute("no-builtins");
    return;
  }

  // And last, add the rest of the builtin names.
  toolchain::for_each(NBA->builtinNames(), AddNoBuiltinAttr);
}

static bool DetermineNoUndef(QualType QTy, CodeGenTypes &Types,
                             const toolchain::DataLayout &DL, const ABIArgInfo &AI,
                             bool CheckCoerce = true) {
  toolchain::Type *Ty = Types.ConvertTypeForMem(QTy);
  ...
  if (CheckCoerce && AI.canHaveCoerceToType()) {
    toolchain::Type *CoerceTy = AI.getCoerceToType();
    if (toolchain::TypeSize::isKnownGT(DL.getTypeSizeInBits(CoerceTy),
...
static bool canApplyNoFPClass(const ABIArgInfo &AI, QualType ParamType,
  ...
  toolchain::Type *IRTy = AI.getCoerceToType();
  if (toolchain::AttributeFuncs::isNoFPClassCompatibleType(IRTy))
    return true;

  if (toolchain::StructType *ST = dyn_cast<toolchain::StructType>(IRTy)) {
    return !IsReturn && AI.getCanBeFlattened() &&
           toolchain::all_of(ST->elements(),
                        toolchain::AttributeFuncs::isNoFPClassCompatibleType);
  }
...
static toolchain::FPClassTest getNoFPClassTestMask(const LangOptions &LangOpts) {
  toolchain::FPClassTest Mask = toolchain::fcNone;
  if (LangOpts.NoHonorInfs)
    Mask |= toolchain::fcInf;
  if (LangOpts.NoHonorNaNs)
    Mask |= toolchain::fcNan;
  return Mask;
}
...
void CodeGenModule::AdjustMemoryAttribute(StringRef Name,
  ...
  if (Attrs.getMemoryEffects().getModRef() == toolchain::ModRefInfo::NoModRef) {
    Attrs = Attrs.removeFnAttribute(getLLVMContext(), toolchain::Attribute::Memory);
    toolchain::Attribute MemoryAttr = toolchain::Attribute::getWithMemoryEffects(
        getLLVMContext(), toolchain::MemoryEffects::writeOnly());
...
void CodeGenModule::ConstructAttributeList(StringRef Name,
  ...
  toolchain::AttrBuilder FuncAttrs(getLLVMContext());
  toolchain::AttrBuilder RetAttrs(getLLVMContext());

  // Collect function IR attributes from the CC lowering.
  // We'll collect the paramete and result attributes later.
  CallingConv = FI.getEffectiveCallingConvention();
  if (FI.isNoReturn())
    FuncAttrs.addAttribute(toolchain::Attribute::NoReturn);
  if (FI.isCmseNSCall())
  ...
  auto AddPotentialArgAccess = [&]() {
    toolchain::Attribute A = FuncAttrs.getAttribute(toolchain::Attribute::Memory);
    if (A.isValid())
      FuncAttrs.addMemoryAttr(A.getMemoryEffects() |
                              toolchain::MemoryEffects::argMemOnly());
  };
  ...
  if (TargetDecl) {
    if (TargetDecl->hasAttr<ReturnsTwiceAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::ReturnsTwice);
    if (TargetDecl->hasAttr<NoThrowAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::NoUnwind);
    if (TargetDecl->hasAttr<NoReturnAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::NoReturn);
    if (TargetDecl->hasAttr<ColdAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::Cold);
    if (TargetDecl->hasAttr<HotAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::Hot);
    if (TargetDecl->hasAttr<NoDuplicateAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::NoDuplicate);
    ...
    if (const FunctionDecl *Fn = dyn_cast<FunctionDecl>(TargetDecl)) {
      ...
      if (AttrOnCallSite && Fn->isReplaceableGlobalAllocationFunction()) {
        ...
        if (getCodeGenOpts().AssumeSaneOperatorNew &&
          ...
          RetAttrs.addAttribute(toolchain::Attribute::NoAlias);
      }
    ...
    if (isa<FunctionDecl>(TargetDecl) || isa<VarDecl>(TargetDecl)) {
      ...
      if (AttrOnCallSite && TargetDecl->hasAttr<NoMergeAttr>())
        FuncAttrs.addAttribute(toolchain::Attribute::NoMerge);
    }

    // 'const', 'pure' and 'noalias' attributed functions are also nounwind.
    if (TargetDecl->hasAttr<ConstAttr>()) {
      FuncAttrs.addMemoryAttr(toolchain::MemoryEffects::none());
      FuncAttrs.addAttribute(toolchain::Attribute::NoUnwind);
      ...
      FuncAttrs.addMemoryAttr(toolchain::MemoryEffects::inaccessibleOrArgMemOnly());
      FuncAttrs.addAttribute(toolchain::Attribute::NoUnwind);
    }
    ...
    if (TargetDecl->hasAttr<ReturnsNonNullAttr>() &&
      ...
      RetAttrs.addAttribute(toolchain::Attribute::NonNull);
    if (TargetDecl->hasAttr<AnyX86NoCallerSavedRegistersAttr>())
      FuncAttrs.addAttribute("no_caller_saved_registers");
    if (TargetDecl->hasAttr<AnyX86NoCfCheckAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::NoCfCheck);
    if (TargetDecl->hasAttr<LeafAttr>())
      FuncAttrs.addAttribute(toolchain::Attribute::NoCallback);
    ...
    if (DeviceKernelAttr::isOpenCLSpelling(
      ...
      if (getLangOpts().OpenCLVersion <= 120) {
        ...
        FuncAttrs.addAttribute(
            ...
            toolchain::toStringRef(getLangOpts().OffloadUniformBlock));
      }
    }
  ...
  if (TargetDecl) {
    if (TargetDecl->hasAttr<NoSpeculativeLoadHardeningAttr>())
      FuncAttrs.removeAttribute(toolchain::Attribute::SpeculativeLoadHardening);
    ...
    if (CodeGenOpts.NoPLT) {
      if (auto *Fn = dyn_cast<FunctionDecl>(TargetDecl)) {
        if (!Fn->isDefined() && !AttrOnCallSite) {
          FuncAttrs.addAttribute(toolchain::Attribute::NonLazyBind);
        }
      }
    }
    // Remove 'convergent' if requested.
    if (TargetDecl->hasAttr<NoConvergentAttr>())
      FuncAttrs.removeAttribute(toolchain::Attribute::Convergent);
  }
  ...
  if (!AttrOnCallSite) {
    ...
    static const toolchain::StringSet<> ReturnsTwiceFn{
    ...
    if (!MSHotPatchFunctions.empty()) {
      bool IsHotPatched = toolchain::binary_search(MSHotPatchFunctions, Name);
      if (IsHotPatched)
  ...
  const ABIArgInfo &RetAI = FI.getReturnInfo();
  const toolchain::DataLayout &DL = getDataLayout();
  ...
  if (CodeGenOpts.EnableNoundefAttrs &&
    ...
    if (!RetTy->isVoidType() && RetAI.getKind() != ABIArgInfo::Indirect &&
      ...
      RetAttrs.addAttribute(toolchain::Attribute::NoUndef);
  }

  switch (RetAI.getKind()) {
  case ABIArgInfo::Extend:
    if (RetAI.isSignExt())
      RetAttrs.addAttribute(toolchain::Attribute::SExt);
    else if (RetAI.isZeroExt())
      RetAttrs.addAttribute(toolchain::Attribute::ZExt);
    else
      RetAttrs.addAttribute(toolchain::Attribute::NoExt);
    [[fallthrough]];
  case ABIArgInfo::Direct:
    if (RetAI.getInReg())
      RetAttrs.addAttribute(toolchain::Attribute::InReg);
  ...
  if (!IsThunk) {
    // FIXME: fix this properly, https://reviews.toolchain.org/D100388
  ...
  SmallVector<toolchain::AttributeSet, 4> ArgAttrs(IRFunctionArgs.totalIRArgs());

  // Attach attributes to sret.
  if (IRFunctionArgs.hasSRetArg()) {
    toolchain::AttrBuilder SRETAttrs(getLLVMContext());
    SRETAttrs.addStructRetAttr(getTypes().ConvertTypeForMem(RetTy));
    SRETAttrs.addAttribute(toolchain::Attribute::Writable);
    SRETAttrs.addAttribute(toolchain::Attribute::DeadOnUnwind);
    hasUsedSRet = true;
    if (RetAI.getInReg())
      SRETAttrs.addAttribute(toolchain::Attribute::InReg);
    SRETAttrs.addAlignmentAttr(RetAI.getIndirectAlign().getQuantity());
    ArgAttrs[IRFunctionArgs.getSRetArgNo()] =
        toolchain::AttributeSet::get(getLLVMContext(), SRETAttrs);
  }

  // Attach attributes to inalloca argument.
  if (IRFunctionArgs.hasInallocaArg()) {
    toolchain::AttrBuilder Attrs(getLLVMContext());
    Attrs.addInAllocaAttr(FI.getArgStruct());
    ArgAttrs[IRFunctionArgs.getInallocaArgNo()] =
        toolchain::AttributeSet::get(getLLVMContext(), Attrs);
  }
  ...
  if (FI.isInstanceMethod() && !IRFunctionArgs.hasInallocaArg() &&
    ...
    if (!CodeGenOpts.NullPointerIsValid &&
      ...
      Attrs.addAttribute(toolchain::Attribute::NonNull);
      ...
      // See https://reviews.toolchain.org/D66618 for discussions.
    ...
    ArgAttrs[IRArgs.first] = toolchain::AttributeSet::get(getLLVMContext(), Attrs);
  }
  ...
  for (CGFunctionInfo::const_arg_iterator I = FI.arg_begin(), E = FI.arg_end();
    ...
    if (IRFunctionArgs.hasPaddingArg(ArgNo)) {
      if (AI.getPaddingInReg()) {
        ArgAttrs[IRFunctionArgs.getPaddingArgNo(ArgNo)] =
            toolchain::AttributeSet::get(getLLVMContext(),
                                    toolchain::AttrBuilder(getLLVMContext())
                                        .addAttribute(toolchain::Attribute::InReg));
      }
    }
    ...
    if (CodeGenOpts.EnableNoundefAttrs &&
      ...
      Attrs.addAttribute(toolchain::Attribute::NoUndef);
    }
    ...
    case ABIArgInfo::Extend:
      if (AI.isSignExt())
        Attrs.addAttribute(toolchain::Attribute::SExt);
      else if (AI.isZeroExt())
        Attrs.addAttribute(toolchain::Attribute::ZExt);
      else
        Attrs.addAttribute(toolchain::Attribute::NoExt);
      [[fallthrough]];
    case ABIArgInfo::Direct:
      if (ArgNo == 0 && FI.isChainCall())
        Attrs.addAttribute(toolchain::Attribute::Nest);
      else if (AI.getInReg())
        Attrs.addAttribute(toolchain::Attribute::InReg);
      Attrs.addStackAlignmentAttr(toolchain::MaybeAlign(AI.getDirectAlign()));
    ...
    case ABIArgInfo::Indirect: {
      ...
      if (auto ParamABI = FI.getExtParameterInfo(ArgNo).getABI();
        ...
        if (AI.getIndirectByVal()) {
          ...
          if (!ParamType.isDestructedType() || !ParamType->isRecordType() ||
            ...
            Attrs.addAttribute(toolchain::Attribute::DeadOnReturn);
        }
      }
      ...
      if (CodeGenOpts.PassByValueIsNoAlias && Decl &&
        ...
        Attrs.addAttribute(toolchain::Attribute::NoAlias);
    ...
    case ParameterABI::SwiftErrorResult:
      Attrs.addAttribute(toolchain::Attribute::SwiftError);
      break;

    case ParameterABI::SwiftContext:
      Attrs.addAttribute(toolchain::Attribute::SwiftSelf);
      break;

    case ParameterABI::SwiftAsyncContext:
      Attrs.addAttribute(toolchain::Attribute::SwiftAsync);
      break;
    }

    if (FI.getExtParameterInfo(ArgNo).isNoEscape())
      Attrs.addCapturesAttr(toolchain::CaptureInfo::none());
    ...
    if (Attrs.hasAttributes()) {
      ...
      for (unsigned i = 0; i < NumIRArgs; i++)
        ArgAttrs[FirstIRArg + i] = ArgAttrs[FirstIRArg + i].addAttributes(
            getLLVMContext(), toolchain::AttributeSet::get(getLLVMContext(), Attrs));
    }
  }
  assert(ArgNo == FI.arg_size());

  AttrList = toolchain::AttributeList::get(
      getLLVMContext(), toolchain::AttributeSet::get(getLLVMContext(), FuncAttrs),
      toolchain::AttributeSet::get(getLLVMContext(), RetAttrs), ArgAttrs);
}

/// An argument came in as a promoted argument; demote it back to its
/// declared type.
static toolchain::Value *emitArgumentDemotion(CodeGenFunction &CGF,
                                         const VarDecl *var,
                                         toolchain::Value *value) {
  toolchain::Type *varType = CGF.ConvertType(var->getType());
  ...
  if (isa<toolchain::IntegerType>(varType))
...
struct CopyBackSwiftError final : EHScopeStack::Cleanup {
  ...
  void Emit(CodeGenFunction &CGF, Flags flags) override {
    toolchain::Value *errorValue = CGF.Builder.CreateLoad(Temp);
...
void CodeGenFunction::EmitFunctionProlog(const CGFunctionInfo &FI,
  ...
  if (const FunctionDecl *FD = dyn_cast_or_null<FunctionDecl>(CurCodeDecl)) {
    if (FD->hasImplicitReturnZero()) {
      ...
      toolchain::Type *LLVMTy = CGM.getTypes().ConvertType(RetTy);
      toolchain::Constant *Zero = toolchain::Constant::getNullValue(LLVMTy);
  ...
  if (IRFunctionArgs.hasSRetArg()) {
    ...
    AI->addAttr(toolchain::Attribute::NoAlias);
  }
  ...
  for (FunctionArgList::const_iterator i = Args.begin(), e = Args.end(); i != e;
      ...
      if (!hasScalarEvaluationKind(Ty)) {
        ...
        if (ArgI.getIndirectRealign() || ArgI.isIndirectAliased()) {
          ...
          Builder.CreateMemCpy(
              ...
              toolchain::ConstantInt::get(IntPtrTy, Size.getQuantity()));
    ...
    case ABIArgInfo::Direct: {
      ...
      toolchain::Type *LTy = ConvertType(Arg->getType());

      // Prepare parameter attributes. So far, only attributes for pointer
      // parameters are prepared. See
      // http://toolchain.org/docs/LangRef.html#paramattrs.
      if (ArgI.getDirectOffset() == 0 && LTy->isPointerTy() &&
        ...
        if (const ParmVarDecl *PVD = dyn_cast<ParmVarDecl>(Arg)) {
          ...
          if (getNonNullAttr(CurCodeDecl, PVD, PVD->getType(),
            ...
            AI->addAttr(toolchain::Attribute::NonNull);
          ...
          if (const auto *ArrTy = getContext().getAsConstantArrayType(OTy)) {
            ...
            if (ArrTy->getSizeModifier() == ArraySizeModifier::Static) {
              ...
              AI->addAttrs(toolchain::AttrBuilder(getLLVMContext())
          ...
          if (AVAttr && !SanOpts.has(SanitizerKind::Alignment)) {
            ...
            toolchain::ConstantInt *AlignmentCI =
                cast<toolchain::ConstantInt>(EmitScalarExpr(AVAttr->getAlignment()));
            uint64_t AlignmentInt =
                AlignmentCI->getLimitedValue(toolchain::Value::MaximumAlignment);
            if (AI->getParamAlign().valueOrOne() < AlignmentInt) {
              AI->removeAttr(toolchain::Attribute::AttrKind::Alignment);
              AI->addAttrs(toolchain::AttrBuilder(getLLVMContext())
                               .addAlignmentAttr(toolchain::Align(AlignmentInt)));
            }
          }
        }
      ...
      if (!isa<toolchain::StructType>(ArgI.getCoerceToType()) &&
        ...
        if (FI.getExtParameterInfo(ArgNo).getABI() ==
          ...
          toolchain::Value *incomingErrorValue = Builder.CreateLoad(arg);
      ...
      // toolchain.vector.extract to convert back to the original VLST.
      if (auto *VecTyTo = dyn_cast<toolchain::FixedVectorType>(ConvertType(Ty))) {
        toolchain::Value *ArgVal = Fn->getArg(FirstIRArg);
        if (auto *VecTyFrom =
                dyn_cast<toolchain::ScalableVectorType>(ArgVal->getType())) {
      ...
      if (ArgI.isDirect() && isa<toolchain::StructType>(ConvertType(Ty)) &&
          (isa<toolchain::TargetExtType>(ArgI.getCoerceToType()) ||
           isa<toolchain::ScalableVectorType>(ArgI.getCoerceToType()))) {
        ArgVals.push_back(ParamValue::forDirect(AI));
        break;
      }

      toolchain::StructType *STy =
          dyn_cast<toolchain::StructType>(ArgI.getCoerceToType());
      Address Alloca =
      ...
      if (ArgI.isDirect() && ArgI.getCanBeFlattened() && STy &&
        ...
        toolchain::TypeSize StructSize = CGM.getDataLayout().getTypeAllocSize(STy);
        toolchain::TypeSize PtrElementSize =
        ...
        if (StructSize.isScalable()) {
          ...
          toolchain::Value *LoadedStructValue = toolchain::PoisonValue::get(STy);
          for (unsigned i = 0, e = STy->getNumElements(); i != e; ++i) {
    ...
    case ABIArgInfo::CoerceAndExpand: {
      ...
      auto *unpaddedStruct = dyn_cast<toolchain::StructType>(unpaddedCoercionType);
      ...
      for (unsigned i = 0, e = coercionType->getNumElements(); i != e; ++i) {
        toolchain::Type *eltType = coercionType->getElementType(i);
        if (ABIArgInfo::isPaddingForCoerceAndExpand(eltType))
          continue;

        auto eltAddr = Builder.CreateStructGEP(alloca, i);
        toolchain::Value *elt = Fn->getArg(argIndex++);

        auto paramType = unpaddedStruct
                             ? unpaddedStruct->getElementType(unpaddedIndex++)
                             : unpaddedCoercionType;

        if (auto *VecTyTo = dyn_cast<toolchain::FixedVectorType>(eltType)) {
          if (auto *VecTyFrom = dyn_cast<toolchain::ScalableVectorType>(paramType)) {
            bool Extracted;
    ...
    case ABIArgInfo::Ignore:
      ...
      if (!hasScalarEvaluationKind(Ty)) {
        ...
        toolchain::Value *U = toolchain::UndefValue::get(ConvertType(Arg->getType()));
...
static void eraseUnusedBitCasts(toolchain::Instruction *insn) {
  while (insn->use_empty()) {
    toolchain::BitCastInst *bitcast = dyn_cast<toolchain::BitCastInst>(insn);
    if (!bitcast)
      return;

    // This is "safe" because we would have used a ConstantExpr otherwise.
    insn = cast<toolchain::Instruction>(bitcast->getOperand(0));
    bitcast->eraseFromParent();
  }
}

/// Try to emit a fused autorelease of a return result.
static toolchain::Value *tryEmitFusedAutoreleaseOfResult(CodeGenFunction &CGF,
                                                    toolchain::Value *result) {
  // We must be immediately followed the cast.
  toolchain::BasicBlock *BB = CGF.Builder.GetInsertBlock();
  if (BB->empty())
    return nullptr;
  if (&BB->back() != result)
    return nullptr;

  toolchain::Type *resultType = result->getType();

  // result is in a BasicBlock and is therefore an Instruction.
  toolchain::Instruction *generator = cast<toolchain::Instruction>(result);

  SmallVector<toolchain::Instruction *, 4> InstsToKill;

  // Look for:
  //  %generator = bitcast %type1* %generator2 to %type2*
  while (toolchain::BitCastInst *bitcast = dyn_cast<toolchain::BitCastInst>(generator)) {
    // We would have emitted this as a constant if the operand weren't
    // an Instruction.
    generator = cast<toolchain::Instruction>(bitcast->getOperand(0));
  ...
  toolchain::CallInst *call = dyn_cast<toolchain::CallInst>(generator);
  if (!call)
    return nullptr;
  ...
  if (call->getCalledOperand() == CGF.CGM.getObjCEntrypoints().objc_retain) {
    ...
    if (CGF.CGM.getObjCEntrypoints().retainAutoreleasedReturnValueMarker) {
      toolchain::Instruction *prev = call->getPrevNode();
      assert(prev);
      if (isa<toolchain::BitCastInst>(prev)) {
        prev = prev->getPrevNode();
        assert(prev);
      }
      assert(isa<toolchain::CallInst>(prev));
      assert(cast<toolchain::CallInst>(prev)->getCalledOperand() ==
  ...
  while (toolchain::BitCastInst *bitcast = dyn_cast<toolchain::BitCastInst>(result)) {
...
static toolchain::Value *tryRemoveRetainOfSelf(CodeGenFunction &CGF,
                                          toolchain::Value *result) {
  ...
  toolchain::CallInst *retainCall = dyn_cast<toolchain::CallInst>(result);
  if (!retainCall || retainCall->getCalledOperand() !=
                         CGF.CGM.getObjCEntrypoints().objc_retain)
    return nullptr;

  // Look for an ordinary load of 'self'.
  toolchain::Value *retainedValue = retainCall->getArgOperand(0);
  toolchain::LoadInst *load =
      dyn_cast<toolchain::LoadInst>(retainedValue->stripPointerCasts());
  ...
  toolchain::Type *resultType = result->getType();
  eraseUnusedBitCasts(cast<toolchain::Instruction>(result));
  assert(retainCall->use_empty());
  retainCall->eraseFromParent();
  eraseUnusedBitCasts(cast<toolchain::Instruction>(retainedValue));
...
static toolchain::Value *emitAutoreleaseOfResult(CodeGenFunction &CGF,
                                            toolchain::Value *result) {
  // If we're returning 'self', kill the initial retain.  This is a
  // heuristic attempt to "encourage correctness" in the really unfortunate
  // case where we have a return of self during a dealloc and we desperately
  // need to avoid the possible autorelease.
  if (toolchain::Value *self = tryRemoveRetainOfSelf(CGF, result))
    return self;

  // At -O0, try to emit a fused retain/autorelease.
  if (CGF.shouldUseFusedARCCalls())
    if (toolchain::Value *fused = tryEmitFusedAutoreleaseOfResult(CGF, result))
      return fused;
...
static toolchain::StoreInst *findDominatingStoreToReturnValue(CodeGenFunction &CGF) {
  toolchain::Value *ReturnValuePtr = CGF.ReturnValue.getBasePointer();

  // Check if a User is a store which pointerOperand is the ReturnValue.
  // We are looking for stores to the ReturnValue, not for stores of the
  // ReturnValue to some other location.
  auto GetStoreIfValid = [&CGF,
                          ReturnValuePtr](toolchain::User *U) -> toolchain::StoreInst * {
    auto *SI = dyn_cast<toolchain::StoreInst>(U);
  ...
  if (!ReturnValuePtr->hasOneUse()) {
    toolchain::BasicBlock *IP = CGF.Builder.GetInsertBlock();
    if (IP->empty())
      return nullptr;

    // Look at directly preceding instruction, skipping bitcasts, lifetime
    // markers, and fake uses and their operands.
    const toolchain::Instruction *LoadIntoFakeUse = nullptr;
    for (toolchain::Instruction &I : toolchain::reverse(*IP)) {
      ...
      if (LoadIntoFakeUse == &I)
        continue;
      if (isa<toolchain::BitCastInst>(&I))
        continue;
      if (auto *II = dyn_cast<toolchain::IntrinsicInst>(&I)) {
        if (II->getIntrinsicID() == toolchain::Intrinsic::lifetime_end)
          continue;

        if (II->getIntrinsicID() == toolchain::Intrinsic::fake_use) {
          LoadIntoFakeUse = dyn_cast<toolchain::Instruction>(II->getArgOperand(0));
          continue;
        }
      }
  ...
  toolchain::StoreInst *store = GetStoreIfValid(ReturnValuePtr->user_back());
  if (!store)
    return nullptr;

  // Now do a first-and-dirty dominance check: just walk up the
  // single-predecessors chain from the current insertion point.
  toolchain::BasicBlock *StoreBB = store->getParent();
  toolchain::BasicBlock *IP = CGF.Builder.GetInsertBlock();
  toolchain::SmallPtrSet<toolchain::BasicBlock *, 4> SeenBBs;
  while (IP != StoreBB) {
...
toolchain::Value *CodeGenFunction::EmitCMSEClearRecord(toolchain::Value *Src,
                                                  toolchain::IntegerType *ITy,
                                                  QualType QTy) {
  assert(Src->getType() == ITy);
  assert(ITy->getScalarSizeInBits() <= 64);

  const toolchain::DataLayout &DataLayout = CGM.getDataLayout();
...
toolchain::Value *CodeGenFunction::EmitCMSEClearRecord(toolchain::Value *Src,
  ...
  int MaskIndex = 0;
  toolchain::Value *R = toolchain::PoisonValue::get(ATy);
  for (int I = 0, N = ATy->getArrayNumElements(); I != N; ++I) {
    ...
    toolchain::Value *T0 = Builder.CreateExtractValue(Src, I);
    toolchain::Value *T1 = Builder.CreateAnd(T0, Mask, "cmse.clear");
...
void CodeGenFunction::EmitFunctionEpilog(
  ...
  toolchain::DebugLoc RetDbgLoc;
  toolchain::Value *RV = nullptr;
  QualType RetTy = FI.getReturnType();
  const ABIArgInfo &RetAI = FI.getReturnInfo();
  ...
  case ABIArgInfo::InAlloca:
    ...
    if (RetAI.getInAllocaSRet()) {
      toolchain::Function::arg_iterator EI = CurFn->arg_end();
      --EI;
      toolchain::Value *ArgStruct = &*EI;
      toolchain::Value *SRet = Builder.CreateStructGEP(
          FI.getArgStruct(), ArgStruct, RetAI.getInAllocaFieldIndex());
      toolchain::Type *Ty =
          cast<toolchain::GetElementPtrInst>(SRet)->getResultElementType();
  ...
  case ABIArgInfo::Direct:
    if (RetAI.getCoerceToType() == ConvertType(RetTy) &&
      ...
      if (toolchain::StoreInst *SI = findDominatingStoreToReturnValue(*this)) {
    ...
    if (AutoreleaseResult) {
      ...
      if (auto *FD = dyn_cast<FunctionDecl>(CurCodeDecl))
        ...
        toolchain_unreachable("Unexpected function/method type");
  ...
  case ABIArgInfo::CoerceAndExpand: {
    ...
    toolchain::SmallVector<toolchain::Value *, 4> results;
    ...
    for (unsigned i = 0, e = coercionType->getNumElements(); i != e; ++i) {
      ...
      toolchain::Value *elt = CreateCoercedLoad(
          eltAddr,
    ...
    // Otherwise, we need to make a first-class aggregate.
      ...
      toolchain::Type *returnType = RetAI.getUnpaddedCoerceAndExpandType();

      RV = toolchain::PoisonValue::get(returnType);
  ...
  if (RV) {
    if (CurFuncDecl && CurFuncDecl->hasAttr<CmseNSEntryAttr>()) {
      ...
      auto *ITy = dyn_cast<toolchain::IntegerType>(RV->getType());
  ...
  toolchain::Value *Backup = RV ? Ret->getOperand(0) : nullptr;
  if (RetKeyInstructionsSourceAtom)
...
void CodeGenFunction::EmitReturnValueCheck(toolchain::Value *RV) {
  ...
  toolchain::BasicBlock *Check = createBasicBlock("nullcheck");
  toolchain::BasicBlock *NoCheck = createBasicBlock("no.nullcheck");
  toolchain::Value *SLocPtr = Builder.CreateLoad(ReturnLocation, "return.sloc.load");
  toolchain::Value *CanNullCheck = Builder.CreateIsNotNull(SLocPtr);
  ...
  toolchain::Value *Cond = Builder.CreateIsNotNull(RV);
  toolchain::Constant *StaticData[] = {EmitCheckSourceLocation(AttrLoc)};
  toolchain::Value *DynamicData[] = {SLocPtr};
...
static AggValueSlot createPlaceholderSlot(CodeGenFunction &CGF, QualType Ty) {
  ...
  // placeholders.
  toolchain::Type *IRTy = CGF.ConvertTypeForMem(Ty);
  toolchain::Type *IRPtrTy = toolchain::PointerType::getUnqual(CGF.getLLVMContext());
  toolchain::Value *Placeholder = toolchain::PoisonValue::get(IRPtrTy);
...
void CodeGenFunction::EmitDelegateCallArg(CallArgList &args,
  ...
  // performed exactly once for a set of arguments, but that should be safe.
    ...
    toolchain::Value *ptr = Builder.CreateLoad(local);
    auto null =
        toolchain::ConstantPointerNull::get(cast<toolchain::PointerType>(ptr->getType()));
  ...
  if (type->isRecordType() && !CurFuncIsThunk &&
    ...
    toolchain::Instruction *isActive = Builder.CreateUnreachable();
    args.addArgCleanupDeactivation(cleanup, isActive);
  }
}

static bool isProvablyNull(toolchain::Value *addr) {
  return toolchain::isa_and_nonnull<toolchain::ConstantPointerNull>(addr);
}

static bool isProvablyNonNull(Address Addr, CodeGenFunction &CGF) {
  return toolchain::isKnownNonZero(Addr.getBasePointer(), CGF.CGM.getDataLayout());
}
...
static void emitWriteback(CodeGenFunction &CGF,
  ...
  if (!provablyNonNull) {
    toolchain::BasicBlock *writebackBB = CGF.createBasicBlock("icr.writeback");
    contBB = CGF.createBasicBlock("icr.done");

    toolchain::Value *isNull = CGF.Builder.CreateIsNull(srcAddr, "icr.isnull");
    CGF.Builder.CreateCondBr(isNull, contBB, writebackBB);
    CGF.EmitBlock(writebackBB);
  }

  // Load the value to writeback.
  toolchain::Value *value = CGF.Builder.CreateLoad(writeback.Temporary);
  ...
  if (writeback.ToUse) {
    ...
    toolchain::Value *oldValue = CGF.EmitLoadOfScalar(srcLV, SourceLocation());
...
static void deactivateArgCleanupsBeforeCall(CodeGenFunction &CGF,
  ...
  for (const auto &I : toolchain::reverse(Cleanups)) {
...
static void emitWritebackArg(CodeGenFunction &CGF, CallArgList &args,
  ...
  toolchain::PointerType *destType =
      cast<toolchain::PointerType>(CGF.ConvertType(CRE->getType()));
  toolchain::Type *destElemType =
      CGF.ConvertTypeForMem(CRE->getType()->getPointeeType());

  // If the address is a constant null, just pass the appropriate null.
  if (isProvablyNull(srcAddr.getBasePointer())) {
    args.add(RValue::get(toolchain::ConstantPointerNull::get(destType)),
             CRE->getType());
    return;
  }
  ...
  if (!shouldCopy) {
    toolchain::Value *null =
        toolchain::ConstantPointerNull::get(cast<toolchain::PointerType>(destElemType));
    CGF.Builder.CreateStore(null, temp);
  }

  toolchain::BasicBlock *contBB = nullptr;
  toolchain::BasicBlock *originBB = nullptr;

  // If the address is *not* known to be non-null, we need to switch.
  toolchain::Value *finalArgument;
  ...
  if (provablyNonNull) {
    ...
    finalArgument = CGF.Builder.CreateSelect(
        isNull, toolchain::ConstantPointerNull::get(destType),
    ...
    if (shouldCopy) {
      ...
      toolchain::BasicBlock *copyBB = CGF.createBasicBlock("icr.copy");
  ...
  toolchain::Value *valueToUse = nullptr;
  ...
  if (shouldCopy) {
    ...
    toolchain::Value *src = srcRV.getScalarVal();
  ...
  if (shouldCopy && !provablyNonNull) {
    toolchain::BasicBlock *copyBB = CGF.Builder.GetInsertBlock();
    CGF.EmitBlock(contBB);

    // Make a phi for the value to intrinsically use.
    if (valueToUse) {
      toolchain::PHINode *phiToUse =
          CGF.Builder.CreatePHI(valueToUse->getType(), 2, "icr.to-use");
      phiToUse->addIncoming(valueToUse, copyBB);
      phiToUse->addIncoming(toolchain::PoisonValue::get(valueToUse->getType()),
                            originBB);
      valueToUse = phiToUse;
    }
...
void CodeGenFunction::EmitNonNullArgCheck(RValue RV, QualType ArgType,
  ...
  SanitizerDebugLocation SanScope(this, {CheckKind}, Handler);
  toolchain::Value *Cond = EmitNonNullRValueCheck(RV, ArgType);
  toolchain::Constant *StaticData[] = {
      EmitCheckSourceLocation(ArgLoc),
      EmitCheckSourceLocation(AttrLoc),
      toolchain::ConstantInt::get(Int32Ty, ArgNo + 1),
  };
...
static bool hasInAllocaArgs(CodeGenModule &CGM, CallingConv ExplicitCC,
  ...
  if (!CGM.getTarget().getCXXABI().isMicrosoft())
    return false;
  return toolchain::any_of(ArgTypes, [&](QualType Ty) {
...
void CodeGenFunction::EmitCallArgs(
  ...
  for (auto *A : toolchain::drop_begin(ArgRange, ArgTypes.size()))
  ...
  auto MaybeEmitImplicitObjectSize = [&](unsigned I, const Expr *Arg,
    ...
    toolchain::Value *V = evaluateOrEmitBuiltinObjectSize(
  ...
  if (hasInAllocaArgs(CGM, ExplicitCC, ArgTypes)) {
    assert(getTarget().getTriple().getArch() == toolchain::Triple::x86 &&
  ...
  for (unsigned I = 0, E = ArgTypes.size(); I != E; ++I) {
    ...
    if (!Args.back().hasLValue()) {
      ...
      EmitNonNullArgCheck(RVArg, ArgTypes[Idx], (*Arg)->getExprLoc(), AC,
                          ParamsToSkip + Idx);
      // @toolchain.objectsize should never have side-effects and shouldn't need
...
void CodeGenFunction::EmitCallArg(CallArgList &args, const Expr *E,
  ...
  if (type->isRecordType() && type->castAs<RecordType>()
    ...
    if (DestroyedInCallee && NeedsCleanup) {
      ...
      // This unreachable is a temporary marker which will be removed later.
      toolchain::Instruction *IsActive =
...
void CodeGenFunction::AddObjCARCExceptionMetadata(toolchain::Instruction *Inst) {
...
toolchain::CallInst *
CodeGenFunction::EmitNounwindRuntimeCall(toolchain::FunctionCallee callee,
                                         const toolchain::Twine &name) {
  return EmitNounwindRuntimeCall(callee, ArrayRef<toolchain::Value *>(), name);
}
...
CodeGenFunction::EmitNounwindRuntimeCall(toolchain::FunctionCallee callee,
                                         ...
                                         const toolchain::Twine &name) {
  SmallVector<toolchain::Value *, 3> values;
  for (auto arg : args)
...
CodeGenFunction::EmitNounwindRuntimeCall(toolchain::FunctionCallee callee,
                                         ...
                                         const toolchain::Twine &name) {
  toolchain::CallInst *call = EmitRuntimeCall(callee, args, name);
...
/// runtime function.
toolchain::CallInst *CodeGenFunction::EmitRuntimeCall(toolchain::FunctionCallee callee,
                                                 const toolchain::Twine &name) {
...
CodeGenFunction::getBundlesForFunclet(toolchain::Value *Callee) {
  // There is no need for a funclet operand bundle if we aren't inside a
  // funclet.
  if (!CurrentFuncletPad)
    return (SmallVector<toolchain::OperandBundleDef, 1>());

  // Skip intrinsics which cannot throw (as long as they don't lower into
  // regular function calls in the course of IR transformations).
  if (auto *CalleeFn = dyn_cast<toolchain::Function>(Callee->stripPointerCasts())) {
    if (CalleeFn->isIntrinsic() && CalleeFn->doesNotThrow()) {
      auto IID = CalleeFn->getIntrinsicID();
      if (!toolchain::IntrinsicInst::mayLowerToFunctionCall(IID))
        return (SmallVector<toolchain::OperandBundleDef, 1>());
    }
  }

  SmallVector<toolchain::OperandBundleDef, 1> BundleList;
...
toolchain::CallInst *CodeGenFunction::EmitRuntimeCall(toolchain::FunctionCallee callee,
  ...
  if (CGM.shouldEmitConvergenceTokens() && call->isConvergent())
    return cast<toolchain::CallInst>(addConvergenceControlToken(call));
  return call;
}

/// Emits a call or invoke to the given noreturn runtime function.
void CodeGenFunction::EmitNoreturnRuntimeCallOrInvoke(
    toolchain::FunctionCallee callee, ArrayRef<toolchain::Value *> args) {
  SmallVector<toolchain::OperandBundleDef, 1> BundleList =
  ...
  if (getInvokeDest()) {
    ...
    toolchain::CallInst *call = Builder.CreateCall(callee, args, BundleList);
...
toolchain::CallBase *
CodeGenFunction::EmitRuntimeCallOrInvoke(toolchain::FunctionCallee callee,
                                         const Twine &name) {
...
CodeGenFunction::EmitRuntimeCallOrInvoke(toolchain::FunctionCallee callee,
                                         ArrayRef<toolchain::Value *> args,
                                         const Twine &name) {
  toolchain::CallBase *call = EmitCallOrInvoke(callee, args, name);
  call->setCallingConv(getRuntimeCC());
  return call;
}
...
toolchain::CallBase *CodeGenFunction::EmitCallOrInvoke(toolchain::FunctionCallee Callee,
                                                  ArrayRef<toolchain::Value *> Args,
                                                  const Twine &Name) {
  toolchain::BasicBlock *InvokeDest = getInvokeDest();
  SmallVector<toolchain::OperandBundleDef, 1> BundleList =
      getBundlesForFunclet(Callee.getCallee());

  toolchain::CallBase *Inst;
  if (!InvokeDest)
    Inst = Builder.CreateCall(Callee, Args, BundleList, Name);
  else {
    toolchain::BasicBlock *ContBB = createBasicBlock("invoke.cont");
...
void CodeGenFunction::deferPlaceholderReplacement(toolchain::Instruction *Old,
                                                  toolchain::Value *New) {
  DeferredReplacements.push_back(
      std::make_pair(toolchain::WeakTrackingVH(Old), New));
}
...
[[nodiscard]] toolchain::AttributeList
maybeRaiseRetAlignmentAttribute(toolchain::LLVMContext &Ctx,
                                const toolchain::AttributeList &Attrs,
                                toolchain::Align NewAlign) {
  toolchain::Align CurAlign = Attrs.getRetAlignment().valueOrOne();
  if (CurAlign >= NewAlign)
    return Attrs;
  toolchain::Attribute AlignAttr = toolchain::Attribute::getWithAlignment(Ctx, NewAlign);
  return Attrs.removeRetAttribute(Ctx, toolchain::Attribute::AttrKind::Alignment)
...
protected:
  ...
  toolchain::Value *Alignment = nullptr;      // May or may not be a constant.
  toolchain::ConstantInt *OffsetCI = nullptr; // Constant, hopefully zero.
...
public:
  ...
  [[nodiscard]] toolchain::AttributeList
  TryEmitAsCallSiteAttribute(const toolchain::AttributeList &Attrs) {
    if (!AA || OffsetCI || CGF.SanOpts.has(SanitizerKind::Alignment))
      return Attrs;
    const auto *AlignmentCI = dyn_cast<toolchain::ConstantInt>(Alignment);
    if (!AlignmentCI)
      return Attrs;
    // We may legitimately have non-power-of-2 alignment here.
    // If so, this is UB land, emit it via `@toolchain.assume` instead.
    if (!AlignmentCI->getValue().isPowerOf2())
      return Attrs;
    toolchain::AttributeList NewAttrs = maybeRaiseRetAlignmentAttribute(
...
public:
  AssumeAlignedAttrEmitter(CodeGenFunction &CGF_, const Decl *FuncDecl)
    ...
    Alignment = cast<toolchain::ConstantInt>(CGF.EmitScalarExpr(AA->getAlignment()));
    if (Expr *Offset = AA->getOffset()) {
      OffsetCI = cast<toolchain::ConstantInt>(CGF.EmitScalarExpr(Offset));
...
static unsigned getMaxVectorWidth(const toolchain::Type *Ty) {
  if (auto *VT = dyn_cast<toolchain::VectorType>(Ty))
    return VT->getPrimitiveSizeInBits().getKnownMinValue();
  if (auto *AT = dyn_cast<toolchain::ArrayType>(Ty))
    return getMaxVectorWidth(AT->getElementType());

  unsigned MaxVectorWidth = 0;
  if (auto *ST = dyn_cast<toolchain::StructType>(Ty))
...
RValue CodeGenFunction::EmitCall(const CGFunctionInfo &CallInfo,
  ...
  toolchain::FunctionType *IRFuncTy = getTypes().GetFunctionType(CallInfo);
  ...
  if (toolchain::StructType *ArgStruct = CallInfo.getArgStruct()) {
    const toolchain::DataLayout &DL = CGM.getDataLayout();
    toolchain::Instruction *IP = CallArgs.getStackBase();
    toolchain::AllocaInst *AI;
    if (IP) {
      IP = IP->getNextNode();
      AI = new toolchain::AllocaInst(ArgStruct, DL.getAllocaAddrSpace(), "argmem",
                                IP->getIterator());
    } else {
  ...
  SmallVector<toolchain::Value *, 16> IRCallArgs(IRFunctionArgs.totalIRArgs());
  ...
  if (RetAI.isIndirect() || RetAI.isInAlloca() || RetAI.isCoerceAndExpand()) {
    ...
    if (IRFunctionArgs.hasSRetArg()) {
      ...
      if (SRetPtr.getAddressSpace() != RetAI.getIndirectAddrSpace()) {
        toolchain::Value *V = SRetPtr.getBasePointer();
        LangAS SAS = getLangASFromTargetAS(SRetPtr.getAddressSpace());
        toolchain::Type *Ty = toolchain::PointerType::get(getLLVMContext(),
  ...
  for (CallArgList::const_iterator I = CallArgs.begin(), E = CallArgs.end();
    ...
    if (IRFunctionArgs.hasPaddingArg(ArgNo))
      IRCallArgs[IRFunctionArgs.getPaddingArgNo(ArgNo)] =
          toolchain::UndefValue::get(ArgInfo.getPaddingType());
    ...
    case ABIArgInfo::InAlloca: {
      ...
      if (I->isAggregate()) {
        ...
        toolchain::Instruction *Placeholder =
            cast<toolchain::Instruction>(Addr.getPointer());
    ...
    case ABIArgInfo::IndirectAliased: {
      ...
      if (I->isAggregate()) {
        ...
        const toolchain::DataLayout *TD = &CGM.getDataLayout();
        ...
        if (Addr.getAlignment() < Align &&
            toolchain::getOrEnforceKnownAlignment(Addr.emitRawPointer(*this),
                                             Align.getAsAlign(),
                                             *TD) < Align.getAsAlign()) {
          NeedCopy = true;
        } else if (I->hasLValue()) {
        ...
        if (!NeedCopy) {
          ...
          toolchain::Value *V = getAsNaturalPointerTo(Addr, I->Ty);
          auto *T = toolchain::PointerType::get(CGM.getLLVMContext(),
                                           ArgInfo.getIndirectAddrSpace());
          ...
          toolchain::Value *Val = getTargetHooks().performAddrSpaceCast(
      ...
      toolchain::Value *Val = getAsNaturalPointerTo(AI, I->Ty);
      if (ArgHasMaybeUndefAttr)
        Val = Builder.CreateFreeze(Val);
      IRCallArgs[FirstIRArg] = Val;
    ...
    case ABIArgInfo::Extend:
    case ABIArgInfo::Direct: {
      if (!isa<toolchain::StructType>(ArgInfo.getCoerceToType()) &&
        ...
        if (CallInfo.getExtParameterInfo(ArgNo).getABI() ==
          ...
          cast<toolchain::AllocaInst>(V)->setSwiftError(true);

          toolchain::Value *errorValue = Builder.CreateLoad(swiftErrorArg);
      ...
      toolchain::StructType *STy =
          dyn_cast<toolchain::StructType>(ArgInfo.getCoerceToType());
      ...
      if (STy && ArgInfo.isDirect() && ArgInfo.getCanBeFlattened()) {
        toolchain::Type *SrcTy = Src.getElementType();
        toolchain::TypeSize SrcTypeSize =
            CGM.getDataLayout().getTypeAllocSize(SrcTy);
        toolchain::TypeSize DstTypeSize = CGM.getDataLayout().getTypeAllocSize(STy);
        if (SrcTypeSize.isScalable()) {
          ...
          toolchain::Value *StoredStructValue =
              Builder.CreateLoad(Src, Src.getName() + ".tuple");
          for (unsigned i = 0, e = STy->getNumElements(); i != e; ++i) {
            toolchain::Value *Extract = Builder.CreateExtractValue(
          ...
          for (unsigned i = 0, e = STy->getNumElements(); i != e; ++i) {
            ...
            toolchain::Value *LI = Builder.CreateLoad(EltPtr);
            if (ArgHasMaybeUndefAttr)
        ...
        if (CallInfo.isCmseNSCall()) {
          ...
          auto *ATy = dyn_cast<toolchain::ArrayType>(Load->getType());
    ...
    case ABIArgInfo::CoerceAndExpand: {
      ...
      if (I->isAggregate()) {
        ...
        toolchain::Type *scalarType = RV.getScalarVal()->getType();
  ...
  toolchain::Value *CalleePtr = ConcreteCallee.getFunctionPointer();

  // If we're using inalloca, set up that argument.
  if (ArgMemory.isValid()) {
    toolchain::Value *Arg = ArgMemory.getPointer();
  ...
  auto simplifyVariadicCallee = [](toolchain::FunctionType *CalleeFT,
                                   toolchain::Value *Ptr) -> toolchain::Function * {
    if (!CalleeFT->isVarArg())
      return nullptr;

    // Get underlying value if it's a bitcast
    if (toolchain::ConstantExpr *CE = dyn_cast<toolchain::ConstantExpr>(Ptr)) {
      if (CE->getOpcode() == toolchain::Instruction::BitCast)
        Ptr = CE->getOperand(0);
    }

    toolchain::Function *OrigFn = dyn_cast<toolchain::Function>(Ptr);
    if (!OrigFn)
      return nullptr;

    toolchain::FunctionType *OrigFT = OrigFn->getFunctionType();
  ...
  if (toolchain::Function *OrigFn = simplifyVariadicCallee(IRFuncTy, CalleePtr)) {
    CalleePtr = OrigFn;
  ...
  if (const FunctionDecl *FD = dyn_cast_or_null<FunctionDecl>(CurFuncDecl)) {
    if (FD->hasAttr<StrictFPAttr>())
      ...
      Attrs = Attrs.addFnAttribute(getLLVMContext(), toolchain::Attribute::StrictFP);
  ...
  if (InNoMergeAttributedStmt)
    Attrs = Attrs.addFnAttribute(getLLVMContext(), toolchain::Attribute::NoMerge);

  // Add call-site noinline attribute if exists.
  if (InNoInlineAttributedStmt)
    Attrs = Attrs.addFnAttribute(getLLVMContext(), toolchain::Attribute::NoInline);

  // Add call-site always_inline attribute if exists.
  // Note: This corresponds to the [[language::Core::always_inline]] statement attribute.
  if (InAlwaysInlineAttributedStmt &&
      !CGM.getTargetCodeGenInfo().wouldInliningViolateFunctionCallABI(
          CallerDecl, CalleeDecl))
    Attrs =
        Attrs.addFnAttribute(getLLVMContext(), toolchain::Attribute::AlwaysInline);

  // Remove call-site convergent attribute if requested.
  if (InNoConvergentAttributedStmt)
    Attrs =
        Attrs.removeFnAttribute(getLLVMContext(), toolchain::Attribute::Convergent);
  ...
  if (currentFunctionUsesSEHTry()) {
    ...
    CannotThrow = Attrs.hasFnAttr(toolchain::Attribute::NoUnwind);

    if (auto *FPtr = dyn_cast<toolchain::Function>(CalleePtr))
      if (FPtr->hasFnAttribute(toolchain::Attribute::NoUnwind))
        CannotThrow = true;
  }
  ...
  toolchain::BasicBlock *InvokeDest = CannotThrow ? nullptr : getInvokeDest();
  ...
  toolchain::CallBase *CI;
  if (!InvokeDest) {
    CI = Builder.CreateCall(IRFuncTy, CalleePtr, IRCallArgs, BundleList);
  } else {
    toolchain::BasicBlock *Cont = createBasicBlock("invoke.cont");
  ...
  CI->setCallingConv(static_cast<toolchain::CallingConv::ID>(CallingConv));
  ...
  if (!CI->getCalledFunction())
    PGO->valueProfile(Builder, toolchain::IPVK_IndirectCallTarget, CI, CalleePtr);
  ...
  if (toolchain::CallInst *Call = dyn_cast<toolchain::CallInst>(CI)) {
        ...
        if (getTarget().getTriple().isOSAIX())
          ...
          if (getTarget().hasFeature("longcall"))
            ...
            if (!cast<FunctionDecl>(TargetDecl)->isDefined())
              ...
              toolchain::GlobalValue::LinkageTypes Linkage = CGM.getFunctionLinkage(
                  GlobalDecl(cast<FunctionDecl>(TargetDecl)));
              if (toolchain::GlobalValue::isWeakForLinker(Linkage) ||
                  toolchain::GlobalValue::isDiscardableIfUnused(Linkage))
  ...
  if (TargetDecl && TargetDecl->hasAttr<ErrorAttr>()) {
    toolchain::ConstantInt *Line =
        toolchain::ConstantInt::get(Int64Ty, Loc.getRawEncoding());
    toolchain::ConstantAsMetadata *MD = toolchain::ConstantAsMetadata::get(Line);
    toolchain::MDTuple *MDT = toolchain::MDNode::get(getLLVMContext(), {MD});
  ...
  if (CI->doesNotReturn()) {
    ...
    if (SanOpts.has(SanitizerKind::Unreachable)) {
      ...
      if (auto *F = CI->getCalledFunction())
        F->removeFnAttr(toolchain::Attribute::NoReturn);
      CI->removeFnAttr(toolchain::Attribute::NoReturn);
      ...
      if (SanOpts.hasOneOf(SanitizerKind::Address |
        ...
        toolchain::IRBuilder<>::InsertPointGuard IPGuard(Builder);
        Builder.SetInsertPoint(CI);
        auto *FnType = toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg=*/false);
        toolchain::FunctionCallee Fn =
  ...
  if (swiftErrorTemp.isValid()) {
    toolchain::Value *errorResult = Builder.CreateLoad(swiftErrorTemp);
  ...
  if (IsVirtualFunctionPointerThunk) {
    ...
    Ret = [&] {
      ...
      case ABIArgInfo::CoerceAndExpand: {
        ...
        bool requiresExtract = isa<toolchain::StructType>(CI->getType());
      ...
      case ABIArgInfo::Extend:
      case ABIArgInfo::Direct: {
        toolchain::Type *RetIRTy = ConvertType(RetTy);
        if (RetAI.getCoerceToType() == RetIRTy &&
            RetAI.getDirectOffset() == 0) {
          switch (getEvaluationKind(RetTy)) {
          case TEK_Complex: {
            toolchain::Value *Real = Builder.CreateExtractValue(CI, 0);
            toolchain::Value *Imag = Builder.CreateExtractValue(CI, 1);
        ...
        // compatibility, and the types match, use the toolchain.vector.extract
        // intrinsic to perform the conversion.
        if (auto *FixedDstTy = dyn_cast<toolchain::FixedVectorType>(RetIRTy)) {
          toolchain::Value *V = CI;
          if (auto *ScalableSrcTy =
                  dyn_cast<toolchain::ScalableVectorType>(V->getType())) {
        ...
        if (!isEmptyRecord(getContext(), RetTy, true)) {
          ...
          CreateCoercedStore(
              CI, StorePtr,
              toolchain::TypeSize::getFixed(DestSize - RetAI.getDirectOffset()),
              DestIsVolatile);
        }
      ...
      toolchain_unreachable("Unhandled ABIArgInfo::Kind");
    }();
  }
