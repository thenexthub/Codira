...
static constexpr toolchain::StringLiteral AwaitKindStr[] = {"init", "await", "yield",
                                                       "final"};
}

struct language::Core::CodeGen::CGCoroData {
  ...
  toolchain::BasicBlock *SuspendBB = nullptr;
  ...
  toolchain::Value *ResumeEHVar = nullptr;
  ...
  // Stores the toolchain.coro.id emitted in the function so that we can supply it
  // as the first argument to coro.begin, coro.alloc and coro.free intrinsics.
  // Note: toolchain.coro.id returns a token that cannot be directly expressed in a
  // builtin.
  toolchain::CallInst *CoroId = nullptr;

  // Stores the toolchain.coro.begin emitted in the function so that we can replace
  ...
  toolchain::CallInst *CoroBegin = nullptr;
  ...
  toolchain::CallInst *LastCoroFree = nullptr;
...
static void createCoroData(CodeGenFunction &CGF,
  ...
  if (CurCoro.Data) {
    if (CurCoro.Data->CoroIdExpr)
      ...
      toolchain_unreachable("EmitCoroutineBodyStatement called twice?");
...
//   if (!x.await_ready()) {
//      toolchain_coro_save();
//      toolchain_coro_await_suspend(&x, frame, wrapper) (*) (**)
//      toolchain_coro_suspend(); (***)
//   }
//   x.await_resume();
//
// where the result of the entire expression is the result of x.await_resume()
//
//   (*) toolchain_coro_await_suspend_{void, bool, handle} is lowered to
...
//        toolchain_coro_suspend();
//
//  (***) toolchain_coro_suspend() encodes three possible continuations as
//       a switch instruction:
//
//  %where-to = call i8 @toolchain.coro.suspend(...)
//  switch i8 %where-to, label %coro.ret [ ; jump to epilogue to suspend
//    i8 0, label %yield.ready   ; go here when resumed
//    i8 1, label %yield.cleanup ; go here when destroyed
//  ]
//
//  See toolchain's docs/Coroutines.rst for more details.
//
namespace {
...
static LValueOrRValue emitSuspendExpression(CodeGenFunction &CGF, CGCoroData &Coro,
  ...
  auto UnbindCommonOnExit =
      toolchain::make_scope_exit([&] { CommonBinder.unbind(CGF); });
  ...
  toolchain::Function *CoroSave = CGF.CGM.getIntrinsic(toolchain::Intrinsic::coro_save);
  auto *NullPtr = toolchain::ConstantPointerNull::get(CGF.CGM.Int8PtrTy);
  ...
  SmallVector<toolchain::Value *, 3> SuspendIntrinsicCallArgs;
  ...
  toolchain::Intrinsic::ID AwaitSuspendIID;

  switch (SuspendReturnType) {
  case CoroutineSuspendExpr::SuspendReturnType::SuspendVoid:
    AwaitSuspendIID = toolchain::Intrinsic::coro_await_suspend_void;
    break;
  case CoroutineSuspendExpr::SuspendReturnType::SuspendBool:
    AwaitSuspendIID = toolchain::Intrinsic::coro_await_suspend_bool;
    break;
  case CoroutineSuspendExpr::SuspendReturnType::SuspendHandle:
    AwaitSuspendIID = toolchain::Intrinsic::coro_await_suspend_handle;
    break;
  }

  toolchain::Function *AwaitSuspendIntrinsic = CGF.CGM.getIntrinsic(AwaitSuspendIID);
  ...
  toolchain::CallBase *SuspendRet = nullptr;
  ...
  toolchain::Function *CoroSuspend =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::coro_suspend);
...
CodeGenFunction::generateAwaitSuspendWrapper(Twine const &CoroName,
  ...
  toolchain::Function *Fn = toolchain::Function::Create(
      LTy, toolchain::GlobalValue::InternalLinkage, FuncName, &CGM.getModule());

  Fn->addParamAttr(0, toolchain::Attribute::AttrKind::NonNull);
  Fn->addParamAttr(0, toolchain::Attribute::AttrKind::NoUndef);

  Fn->addParamAttr(1, toolchain::Attribute::AttrKind::NoUndef);

  Fn->setMustProgress();
  Fn->addFnAttr(toolchain::Attribute::AttrKind::AlwaysInline);

  StartFunction(GlobalDecl(), ReturnTy, Fn, FI, args);

  // FIXME: add TBAA metadata to the loads
  toolchain::Value *AwaiterPtr = Builder.CreateLoad(GetAddrOfLocalVar(&AwaiterDecl));
  auto AwaiterLValue =
  ...
  auto UnbindCommonOnExit =
      toolchain::make_scope_exit([&] { AwaiterBinder.unbind(*this); });
  if (SuspendRet != nullptr) {
    Fn->addRetAttr(toolchain::Attribute::AttrKind::NoUndef);
...
static SmallVector<toolchain::OperandBundleDef, 1>
getBundlesForCoroEnd(CodeGenFunction &CGF) {
  SmallVector<toolchain::OperandBundleDef, 1> BundleList;

  if (toolchain::Instruction *EHPad = CGF.CurrentFuncletPad)
...
// See toolchain/docs/Coroutines.rst for more details about coro.end.
struct CallCoroEnd final : public EHScopeStack::Cleanup {
  void Emit(CodeGenFunction &CGF, Flags flags) override {
    auto &CGM = CGF.CGM;
    auto *NullPtr = toolchain::ConstantPointerNull::get(CGF.Int8PtrTy);
    toolchain::Function *CoroEndFn = CGM.getIntrinsic(toolchain::Intrinsic::coro_end);
    // See if we have a funclet bundle to associate coro.end with. (WinEH)
    auto Bundles = getBundlesForCoroEnd(CGF);
    auto *CoroEnd =
      CGF.Builder.CreateCall(CoroEndFn,
                             {NullPtr, CGF.Builder.getTrue(),
                              toolchain::ConstantTokenNone::get(CoroEndFn->getContext())},
                             Bundles);
    if (Bundles.empty()) {
...
struct GetReturnObjectManager {
  ...
  void EmitGroAlloca() {
    ...
    if (!GroVarDecl->isNRVOVariable()) {
      ...
      auto *GroAlloca = dyn_cast_or_null<toolchain::AllocaInst>(
          GroEmission.getOriginalAllocatedAddress().getPointer());
      assert(GroAlloca && "expected alloca to be emitted");
      GroAlloca->setMetadata(toolchain::LLVMContext::MD_coro_outside_frame,
                             toolchain::MDNode::get(CGF.CGM.getLLVMContext(), {}));
    }
...
void CodeGenFunction::EmitCoroutineBody(const CoroutineBodyStmt &S) {
  auto *NullPtr = toolchain::ConstantPointerNull::get(Builder.getPtrTy());
  ...
  auto *CoroId = Builder.CreateCall(
      CGM.getIntrinsic(toolchain::Intrinsic::coro_id),
  ...
  auto *CoroAlloc = Builder.CreateCall(
      CGM.getIntrinsic(toolchain::Intrinsic::coro_alloc), {CoroId});
  ...
  if (auto *RetOnAllocFailure = S.getReturnStmtOnAllocFailure()) {
    ...
    auto *NullPtr = toolchain::ConstantPointerNull::get(Int8PtrTy);
  ...
  auto *CoroBegin = Builder.CreateCall(
      CGM.getIntrinsic(toolchain::Intrinsic::coro_begin), {CoroId, Phi});
  ...
  CurCoro.Data->CleanupJD = getJumpDestInCurrentScope(RetBB);
  {
    ...
    toolchain::ArrayRef<const Stmt *> ParamMoves = S.getParamMoves();
    assert(
    ...
    if (ParamMoves.size() == FnArgs.size() && DI)
      for (const auto Pair : toolchain::zip(FnArgs, ParamMoves))
    ...
    for (const ParmVarDecl *Parm : FnArgs) {
      ...
      if (auto *ParmAlloca =
              dyn_cast<toolchain::AllocaInst>(ParmAddr.getBasePointer())) {
        ParmAlloca->setMetadata(toolchain::LLVMContext::MD_coro_outside_frame,
    ...
    for (auto *PM : S.getParamMoves()) {
      ...
      // for the copy, so that toolchain can elide it if the copy is
      // not needed.
    }
    ...
    auto *PromiseAddrVoidPtr =
        new toolchain::BitCastInst(PromiseAddr.emitRawPointer(*this), VoidPtrTy, "",
                              CoroId->getIterator());
  ...
  toolchain::Function *CoroEnd = CGM.getIntrinsic(toolchain::Intrinsic::coro_end);
  Builder.CreateCall(CoroEnd,
                     {NullPtr, Builder.getFalse(),
                      toolchain::ConstantTokenNone::get(CoroEnd->getContext())});
...
RValue CodeGenFunction::EmitCoroutineIntrinsic(const CallExpr *E,
  ...
  // intrinsic.
  case toolchain::Intrinsic::coro_frame: {
  ...
  case toolchain::Intrinsic::coro_size: {
    auto &Context = getContext();
    toolchain::IntegerType *T =
        Builder.getIntNTy(Context.getTypeSize(Context.getSizeType()));
    toolchain::Function *F = CGM.getIntrinsic(toolchain::Intrinsic::coro_size, T);
    return RValue::get(Builder.CreateCall(F));
  }
  case toolchain::Intrinsic::coro_align: {
    auto &Context = getContext();
    toolchain::IntegerType *T =
        Builder.getIntNTy(Context.getTypeSize(Context.getSizeType()));
    toolchain::Function *F = CGM.getIntrinsic(toolchain::Intrinsic::coro_align, T);
    return RValue::get(Builder.CreateCall(F));
  }
  // The following three intrinsics take a token parameter referring to a token
  // returned by earlier call to @toolchain.coro.id. Since we cannot represent it in
  // builtins, we patch it up here.
  case toolchain::Intrinsic::coro_alloc:
  case toolchain::Intrinsic::coro_begin:
  case toolchain::Intrinsic::coro_free: {
  ...
  // @toolchain.coro.suspend takes a token parameter. Add token 'none' as the first
  // argument.
  case toolchain::Intrinsic::coro_suspend:
    Args.push_back(toolchain::ConstantTokenNone::get(getLLVMContext()));
    break;
  }
  for (const Expr *Arg : E->arguments())
    Args.push_back(EmitScalarExpr(Arg));
  // @toolchain.coro.end takes a token parameter. Add token 'none' as the last
  // argument.
  if (IID == toolchain::Intrinsic::coro_end)
    Args.push_back(toolchain::ConstantTokenNone::get(getLLVMContext()));

  toolchain::Function *F = CGM.getIntrinsic(IID);
  toolchain::CallInst *Call = Builder.CreateCall(F, Args);

  // Note: The following code is to enable to emit coro.id and coro.begin by
  // hand to experiment with coroutines in C.
  // If we see @toolchain.coro.id remember it in the CoroData. We will update
  // coro.alloc, coro.begin and coro.free intrinsics to refer to it.
  if (IID == toolchain::Intrinsic::coro_id) {
    createCoroData(*this, CurCoro, Call, E);
  }
  else if (IID == toolchain::Intrinsic::coro_begin) {
    if (CurCoro.Data)
      CurCoro.Data->CoroBegin = Call;
  }
  else if (IID == toolchain::Intrinsic::coro_free) {
