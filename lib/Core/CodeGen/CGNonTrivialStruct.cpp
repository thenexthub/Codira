...
template <class Derived> struct StructVisitor {
  ...
  template <class... Ts> void visitCXXDestructor(Ts... Args) {
    toolchain_unreachable("field of a C++ struct type is not expected");
  }
...
struct CopyStructVisitor : StructVisitor<Derived>,
  ...
  void visitTrivial(QualType FT, const FieldDecl *FD, CharUnits CurStructOffset,
    ...
    uint64_t RoundedFEnd = toolchain::alignTo(FEndInBits, Ctx.getCharWidth());
...
template <class Derived> struct GenFuncNameBase {
  std::string getVolatileOffsetStr(bool IsVolatile, CharUnits Offset) {
    ...
    if (IsVolatile)
      S = "v";
    S += toolchain::to_string(Offset.getQuantity());
    return S;
  }
  ...
  void visitArray(FieldKind FK, const ArrayType *AT, bool IsVolatile,
    ...
    appendStr("_AB" + toolchain::to_string(FieldOffset.getQuantity()) + "s" +
              toolchain::to_string(EltSize.getQuantity()) + "n" +
              toolchain::to_string(NumElts));
...
struct GenUnaryFuncName : StructVisitor<Derived>, GenFuncNameBase<Derived> {
  GenUnaryFuncName(StringRef Prefix, CharUnits DstAlignment, ASTContext &Ctx)
    ...
    this->appendStr(toolchain::to_string(DstAlignment.getQuantity()));
  }
};
...
static toolchain::Constant *getNullForVariable(Address Addr) {
  ...
  return toolchain::ConstantPointerNull::get(cast<toolchain::PointerType>(Ty));
}
...
struct GenBinaryFuncName : CopyStructVisitor<GenBinaryFuncName<IsMove>, IsMove>,
  ...
  GenBinaryFuncName(StringRef Prefix, CharUnits DstAlignment,
    ...
    this->appendStr(Prefix);
    this->appendStr(toolchain::to_string(DstAlignment.getQuantity()));
    this->appendStr("_" + toolchain::to_string(SrcAlignment.getQuantity()));
  }
  ...
  void flushTrivialFields() {
    ...
    this->appendStr("_t" + toolchain::to_string(this->Start.getQuantity()) + "w" +
                    toolchain::to_string((this->End - this->Start).getQuantity()));
  ...
  void visitVolatileTrivial(QualType FT, const FieldDecl *FD,
    ...
    this->appendStr("_tv" + toolchain::to_string(OffsetInBits) + "w" +
                    toolchain::to_string(getFieldSize(FD, FT, this->Ctx)));
  }
  ...
  void visitPtrAuth(QualType FT, const FieldDecl *FD,
    ...
    this->appendStr(toolchain::to_string(PtrAuth.getKey()) + "_");
    this->appendStr(toolchain::to_string(PtrAuth.getExtraDiscriminator()) + "_");
    if (PtrAuth.authenticatesNullValues())
      this->appendStr("anv_");
    CharUnits FieldOffset = CurStructOffset + this->getFieldOffset(FD);
    this->appendStr(toolchain::to_string(FieldOffset.getQuantity()));
  }
};
...
static const CGFunctionInfo &getFunctionInfo(CodeGenModule &CGM,
  ...
  toolchain::SmallVector<ImplicitParamDecl *, N> Params;
  ...
  toolchain::append_range(Args, Params);
...
template <class Derived> struct GenFuncBase {
  ...
  void visitArray(FieldKind FK, const ArrayType *AT, bool IsVolatile,
    ...
    toolchain::Value *NumElts = CGF.emitArrayLength(AT, BaseEltQT, DstAddr);
    unsigned BaseEltSize = Ctx.getTypeSizeInChars(BaseEltQT).getQuantity();
    toolchain::Value *BaseEltSizeVal =
        toolchain::ConstantInt::get(NumElts->getType(), BaseEltSize);
    toolchain::Value *SizeInBytes =
        CGF.Builder.CreateNUWMul(BaseEltSizeVal, NumElts);
    toolchain::Value *DstArrayEnd = CGF.Builder.CreateInBoundsGEP(
        CGF.Int8Ty, DstAddr.emitRawPointer(CGF), SizeInBytes);
    toolchain::BasicBlock *PreheaderBB = CGF.Builder.GetInsertBlock();

    // Create the header block and insert the phi instructions.
    toolchain::BasicBlock *HeaderBB = CGF.createBasicBlock("loop.header");
    CGF.EmitBlock(HeaderBB);
    toolchain::PHINode *PHIs[N];
    ...
    toolchain::BasicBlock *ExitBB = CGF.createBasicBlock("loop.exit");
    toolchain::BasicBlock *LoopBB = CGF.createBasicBlock("loop.body");
  ...
  template <size_t N>
  toolchain::Function *getFunction(StringRef FuncName, QualType QT,
                              std::array<CharUnits, N> Alignments,
                              CodeGenModule &CGM) {
    // If the special function already exists in the module, return it.
    if (toolchain::Function *F = CGM.getModule().getFunction(FuncName)) {
      bool WrongType = false;
      if (!F->getReturnType()->isVoidTy())
        WrongType = true;
      else {
        for (const toolchain::Argument &Arg : F->args())
    ...
    toolchain::Function *F =
        toolchain::Function::Create(FuncTy, toolchain::GlobalValue::LinkOnceODRLinkage,
...
struct GenBinaryFunc : CopyStructVisitor<Derived, IsMove>,
  ...
  void flushTrivialFields(std::array<Address, 2> Addrs) {
    ...
    if (Size.getQuantity() >= 16 ||
        !toolchain::has_single_bit<uint32_t>(Size.getQuantity())) {
      toolchain::Value *SizeVal =
          toolchain::ConstantInt::get(this->CGF->SizeTy, Size.getQuantity());
      ...
      toolchain::Type *Ty = toolchain::Type::getIntNTy(
      ...
      toolchain::Value *SrcVal = this->CGF->Builder.CreateLoad(SrcAddr, false);
  ...
  void visitVolatileTrivial(QualType FT, const FieldDecl *FD, CharUnits Offset,
    ...
    if (FD) {
      ...
      toolchain::Type *Ty = this->CGF->ConvertType(RT);
      ...
      toolchain::Type *Ty = this->CGF->ConvertTypeForMem(FT);
...
struct GenCopyConstructor : GenBinaryFunc<GenCopyConstructor, false> {
  ...
  void visitARCStrong(QualType QT, const FieldDecl *FD,
    ...
    toolchain::Value *SrcVal = CGF->EmitLoadOfScalar(
        Addrs[SrcIdx], QT.isVolatileQualified(), QT, SourceLocation());
    toolchain::Value *Val = CGF->EmitARCRetain(QT, SrcVal);
...
toolchain::Function *language::Core::CodeGen::getNonTrivialCStructDefaultConstructor(
...
toolchain::Function *language::Core::CodeGen::getNonTrivialCStructCopyConstructor(
...
toolchain::Function *language::Core::CodeGen::getNonTrivialCStructMoveConstructor(
    CodeGenModule &CGM, CharUnits DstAlignment, CharUnits SrcAlignment,
    bool IsVolatile, QualType QT) {
  ASTContext &Ctx = CGM.getContext();
...
toolchain::Function *language::Core::CodeGen::getNonTrivialCStructCopyAssignmentOperator(
    CodeGenModule &CGM, CharUnits DstAlignment, CharUnits SrcAlignment,
    bool IsVolatile, QualType QT) {
  ASTContext &Ctx = CGM.getContext();
...
toolchain::Function *language::Core::CodeGen::getNonTrivialCStructMoveAssignmentOperator(
    CodeGenModule &CGM, CharUnits DstAlignment, CharUnits SrcAlignment,
    bool IsVolatile, QualType QT) {
  ASTContext &Ctx = CGM.getContext();
...
toolchain::Function *language::Core::CodeGen::getNonTrivialCStructDestructor(
    CodeGenModule &CGM, CharUnits DstAlignment, bool IsVolatile, QualType QT) {
  ASTContext &Ctx = CGM.getContext();
