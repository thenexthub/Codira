...
static toolchain::cl::opt<bool>
    EnableValueProfiling("enable-value-profiling",
                         toolchain::cl::desc("Enable value profiling"),
...
void CodeGenPGO::setFuncName(StringRef Name,
  ...
  toolchain::IndexedInstrProfReader *PGOReader = CGM.getPGOReader();
  FuncName = toolchain::getPGOFuncName(
      Name, Linkage, CGM.getCodeGenOpts().MainFileName,
      PGOReader ? PGOReader->getVersion() : toolchain::IndexedInstrProf::Version);

  // If we're generating a profile, create a variable for the name.
  if (CGM.getCodeGenOpts().hasProfileClangInstr())
    FuncNameVar = toolchain::createPGOFuncNameVar(CGM.getModule(), Linkage, FuncName);
}

void CodeGenPGO::setFuncName(toolchain::Function *Fn) {
  setFuncName(Fn->getName(), Fn->getLinkage());
  // Create PGOFuncName meta data.
  toolchain::createPGOFuncNameMetadata(*Fn, FuncName);
}
...
static PGOHashVersion getPGOHashVersion(toolchain::IndexedInstrProfReader *PGOReader,
                                        CodeGenModule &CGM) {
...
struct MapRegionCounters : public RecursiveASTVisitor<MapRegionCounters> {
  ...
  bool VisitBinaryOperator(BinaryOperator *S) {
    if (S->isLogicalOp()) {
      ...
      if (CodeGenFunction::isInstrumentedCondition(S->getRHS())) {
        if (ProfileVersion >= toolchain::IndexedInstrProf::Version7)
  ...
  bool VisitConditionalOperator(ConditionalOperator *S) {
    if (toolchain::EnableSingleByteCoverage && S->getTrueExpr())
      CounterMap[S->getTrueExpr()] = NextCounter++;
    if (toolchain::EnableSingleByteCoverage && S->getFalseExpr())
  ...
  bool TraverseIfStmt(IfStmt *If) {
    ...
    // else.
    bool NoSingleByteCoverage = !toolchain::EnableSingleByteCoverage;
...
struct ComputeRegionCounts : public ConstStmtVisitor<ComputeRegionCounts> {
  ...
  toolchain::DenseMap<const Stmt *, uint64_t> &CountMap;
  ...
  ComputeRegionCounts(toolchain::DenseMap<const Stmt *, uint64_t> &CountMap,
                      CodeGenPGO &PGO)
...
void PGOHash::combine(HashType Type) {
  ...
  if (Count && Count % NumTypesPerWord == 0) {
    ...
    uint64_t Swapped =
        endian::byte_swap<uint64_t, toolchain::endianness::little>(Working);
    MD5.update(toolchain::ArrayRef((uint8_t *)&Swapped, sizeof(Swapped)));
    Working = 0;
  }
...
void CodeGenPGO::assignRegionCounters(GlobalDecl GD, toolchain::Function *Fn) {
  ...
  if (Fn->hasFnAttribute(toolchain::Attribute::NoProfile))
    return;
  if (Fn->hasFnAttribute(toolchain::Attribute::SkipProfile))
    return;
...
void CodeGenPGO::mapRegionCounters(const Decl *D) {
  ...
  uint64_t ProfileVersion = toolchain::IndexedInstrProf::Version;
  ...
  RegionCounterMap.reset(new toolchain::DenseMap<const Stmt *, CounterPair>);
...
bool CodeGenPGO::skipRegionMappingForDecl(const Decl *D) {
  ...
  return !toolchain::coverage::SystemHeadersCoverage && SM.isInSystemHeader(Loc);
}
...
void CodeGenPGO::computeRegionCounts(const Decl *D) {
  StmtCountMap.reset(new toolchain::DenseMap<const Stmt *, uint64_t>);
...
void
CodeGenPGO::applyFunctionAttributes(toolchain::IndexedInstrProfReader *PGOReader,
...
void CodeGenPGO::emitCounterSetOrIncrement(CGBuilderTy &Builder, const Stmt *S,
  ...
  auto *NormalizedFuncNameVarPtr =
      toolchain::ConstantExpr::getPointerBitCastOrAddrSpaceCast(
          FuncNameVar, toolchain::PointerType::get(CGM.getLLVMContext(), 0));
  ...
  if (toolchain::EnableSingleByteCoverage)
    Builder.CreateCall(CGM.getIntrinsic(toolchain::Intrinsic::instrprof_cover),
                       ArrayRef(Args, 4));
  else if (!StepV)
    Builder.CreateCall(CGM.getIntrinsic(toolchain::Intrinsic::instrprof_increment),
                       ArrayRef(Args, 4));
  else
    Builder.CreateCall(
        CGM.getIntrinsic(toolchain::Intrinsic::instrprof_increment_step), Args);
}
...
void CodeGenPGO::emitMCDCParameters(CGBuilderTy &Builder) {
  ...
  auto *I8PtrTy = toolchain::PointerType::getUnqual(CGM.getLLVMContext());
  ...
  // anything.
  toolchain::Value *Args[3] = {toolchain::ConstantExpr::getBitCast(FuncNameVar, I8PtrTy),
                          Builder.getInt64(FunctionHash),
                          Builder.getInt32(RegionMCDCState->BitmapBits)};
  Builder.CreateCall(
      CGM.getIntrinsic(toolchain::Intrinsic::instrprof_mcdc_parameters), Args);
}
...
void CodeGenPGO::emitMCDCTestVectorBitmapUpdate(CGBuilderTy &Builder,
  ...
  toolchain::Value *Args[4] = {toolchain::ConstantExpr::getBitCast(FuncNameVar, I8PtrTy),
                          Builder.getInt64(FunctionHash),
                          Builder.getInt32(MCDCTestVectorBitmapOffset),
                          MCDCCondBitmapAddr.emitRawPointer(CGF)};
  Builder.CreateCall(
      CGM.getIntrinsic(toolchain::Intrinsic::instrprof_mcdc_tvbitmap_update), Args);
}
...
void CodeGenPGO::setValueProfilingFlag(toolchain::Module &M) {
  if (CGM.getCodeGenOpts().hasProfileClangInstr())
    M.addModuleFlag(toolchain::Module::Warning, "EnableValueProfiling",
                    uint32_t(EnableValueProfiling));
}

void CodeGenPGO::setProfileVersion(toolchain::Module &M) {
  if (CGM.getCodeGenOpts().hasProfileClangInstr() &&
      toolchain::EnableSingleByteCoverage) {
    const StringRef VarName(INSTR_PROF_QUOTE(INSTR_PROF_RAW_VERSION_VAR));
    toolchain::Type *IntTy64 = toolchain::Type::getInt64Ty(M.getContext());
    uint64_t ProfileVersion =
        (INSTR_PROF_RAW_VERSION | VARIANT_MASK_BYTE_COVERAGE);

    auto IRLevelVersionVariable = new toolchain::GlobalVariable(
        M, IntTy64, true, toolchain::GlobalValue::WeakAnyLinkage,
        toolchain::Constant::getIntegerValue(IntTy64,
                                        toolchain::APInt(64, ProfileVersion)),
        VarName);

    IRLevelVersionVariable->setVisibility(toolchain::GlobalValue::HiddenVisibility);
    toolchain::Triple TT(M.getTargetTriple());
    if (TT.isGPU())
      IRLevelVersionVariable->setVisibility(
          toolchain::GlobalValue::ProtectedVisibility);
    if (TT.supportsCOMDAT()) {
      IRLevelVersionVariable->setLinkage(toolchain::GlobalValue::ExternalLinkage);
...
void CodeGenPGO::valueProfile(CGBuilderTy &Builder, uint32_t ValueKind,
  ...
  if (isa<toolchain::Constant>(ValuePtr))
    return;
  ...
  if (InstrumentValueSites && RegionCounterMap) {
    ...
    Builder.CreateCall(
        CGM.getIntrinsic(toolchain::Intrinsic::instrprof_value_profile), Args);
  ...
  if (PGOReader && haveRegionCounts()) {
    ...
    toolchain::annotateValueSite(CGM.getModule(), *ValueSite, *ProfRecord,
                            (toolchain::InstrProfValueKind)ValueKind,
...
void CodeGenPGO::loadRegionCounts(toolchain::IndexedInstrProfReader *PGOReader,
                                  bool IsInMainFile) {
  CGM.getPGOStats().addVisited(IsInMainFile);
  RegionCounts.clear();
  auto RecordExpected = PGOReader->getInstrProfRecord(FuncName, FunctionHash);
  if (auto E = RecordExpected.takeError()) {
    auto IPE = std::get<0>(toolchain::InstrProfError::take(std::move(E)));
    if (IPE == toolchain::instrprof_error::unknown_function)
      CGM.getPGOStats().addMissing(IsInMainFile);
    else if (IPE == toolchain::instrprof_error::hash_mismatch)
      CGM.getPGOStats().addMismatched(IsInMainFile);
    else if (IPE == toolchain::instrprof_error::malformed)
      // TODO: Consider a more specific warning for this case.
      CGM.getPGOStats().addMismatched(IsInMainFile);
    return;
  }
  ProfRecord =
      std::make_unique<toolchain::InstrProfRecord>(std::move(RecordExpected.get()));
...
toolchain::MDNode *CodeGenFunction::createProfileWeights(uint64_t TrueCount,
...
CodeGenFunction::createProfileWeights(ArrayRef<uint64_t> Weights) const {
  ...
  // Check for empty weights.
  uint64_t MaxWeight = *toolchain::max_element(Weights);
  if (MaxWeight == 0)
    return nullptr;
...
void CodeGenFunction::incrementProfileCounter(const Stmt *S,
  ...
  if (CGM.getCodeGenOpts().hasProfileClangInstr() &&
      ...
      !CurFn->hasFnAttribute(toolchain::Attribute::SkipProfile)) {
