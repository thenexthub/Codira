...
#ifndef LANGUAGE_CORE_LIB_CODEGEN_CGOPENMPRUNTIMEGPU_H
#define LANGUAGE_CORE_LIB_CODEGEN_CGOPENMPRUNTIMEGPU_H
...
private:
  ...
  toolchain::SmallVector<toolchain::Function *, 16> Work;
  ...
  void emitNonSPMDKernel(const OMPExecutableDirective &D, StringRef ParentName,
                         ...
                         toolchain::Constant *&OutlinedFnID, bool IsOffloadEntry,
...
public:
  ...
  void getKmpcFreeShared(
      CodeGenFunction &CGF,
      const std::pair<toolchain::Value *, toolchain::Value *> &AddrSizePair) override;

  /// Get the id of the current thread on the GPU.
  toolchain::Value *getGPUThreadID(CodeGenFunction &CGF);

  /// Get the maximum number of threads in a block of the GPU.
  toolchain::Value *getGPUNumThreads(CodeGenFunction &CGF);
  ...
  emitOutlinedFunctionCall(CodeGenFunction &CGF, SourceLocation Loc,
                           ...
                           ArrayRef<toolchain::Value *> Args = {}) const override;
  ...
  void getDefaultDistScheduleAndChunk(CodeGenFunction &CGF,
      ...
      toolchain::Value *&Chunk) const override;
...
private:
  ...
  toolchain::DenseMap<toolchain::Function *, toolchain::Function *> WrapperFunctionsMap;

  /// Emit function which wraps the outline parallel region
  /// and controls the parameters which are passed to this function.
  /// The wrapper ensures that the outlined function is called
  /// with the correct arguments when data is shared.
  toolchain::Function *createParallelDataSharingWrapper(
  ...
  struct MappedVarData {
    ...
    toolchain::Value *GlobalizedVal = nullptr;
    /// Corresponding address.
    Address PrivateAddr = Address::invalid();
  };
  /// The map of local variables to their addresses in the global memory.
  using DeclToAddrMapTy = toolchain::MapVector<const Decl *, MappedVarData>;
  /// Set of the parameters passed by value escaping OpenMP context.
  using EscapedParamsTy = toolchain::SmallPtrSet<const Decl *, 4>;
  struct FunctionData {
    DeclToAddrMapTy LocalVarData;
    EscapedParamsTy EscapedParameters;
    toolchain::SmallVector<const ValueDecl*, 4> EscapedVariableLengthDecls;
    toolchain::SmallVector<const ValueDecl *, 4> DelayedVariableLengthDecls;
    toolchain::SmallVector<std::pair<toolchain::Value *, toolchain::Value *>, 4>
        EscapedVariableLengthDeclsAddrs;
    std::unique_ptr<CodeGenFunction::OMPMapVars> MappedParams;
  };
  /// Maps the function to the list of the globalized variables with their
  /// addresses.
  toolchain::SmallDenseMap<toolchain::Function *, FunctionData> FunctionGlobalizedDecls;
  ...
  toolchain::SmallVector<const RecordDecl *, 4> TeamsReductions;
  /// Pair of the Non-SPMD team and all reductions variables in this team
  /// region.
  std::pair<const Decl *, toolchain::SmallVector<const ValueDecl *, 4>>
      TeamAndReductions;
};
...
#endif // LANGUAGE_CORE_LIB_CODEGEN_CGOPENMPRUNTIMEGPU_H
