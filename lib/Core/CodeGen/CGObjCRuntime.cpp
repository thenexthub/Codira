...
LValue CGObjCRuntime::EmitValueForIvarAtOffset(CodeGen::CodeGenFunction &CGF,
  ...
  CharUnits StorageSize = CGF.CGM.getContext().toCharUnitsFromBits(
      toolchain::alignTo(BitOffset + BitFieldSize, AlignmentBits));
  ...
  Address Addr =
      Address(V, toolchain::Type::getIntNTy(CGF.getLLVMContext(), Info->StorageSize),
              Alignment);
...
namespace {
  ...
  struct CallObjCEndCatch final : EHScopeStack::Cleanup {
    CallObjCEndCatch(bool MightThrow, toolchain::FunctionCallee Fn)
...
void CGObjCRuntime::EmitTryCatchStmt(CodeGenFunction &CGF,
  ...
  if (useFunclets)
    if (const ObjCAtFinallyStmt *Finally = S.getFinallyStmt()) {
        ...
        toolchain::Function *FinallyFunc = HelperCGF.CurFn;
  ...
  for (CatchHandler &Handler : Handlers) {
    ...
    if (useFunclets) {
      toolchain::BasicBlock::iterator CPICandidate =
          Handler.Block->getFirstNonPHIIt();
      if (CPICandidate != Handler.Block->end()) {
        if (auto *CPI = dyn_cast_or_null<toolchain::CatchPadInst>(CPICandidate)) {
    ...
    toolchain::Value *RawExn = CGF.getExceptionFromSlot();

    // Enter the catch.
    toolchain::Value *Exn = RawExn;
    if (beginCatchFn)
    ...
    if (const VarDecl *CatchParam = Handler.Variable) {
      toolchain::Type *CatchType = CGF.ConvertType(CatchParam->getType());
      toolchain::Value *CastExn = CGF.Builder.CreateBitCast(Exn, CatchType);
...
void CGObjCRuntime::EmitInitOfCatchParam(CodeGenFunction &CGF,
  ...
  toolchain_unreachable("invalid ownership qualifier");
}
...
namespace {
  struct CallSyncExit final : EHScopeStack::Cleanup {
    ...
    CallSyncExit(toolchain::FunctionCallee SyncExitFn, toolchain::Value *SyncArg)
...
CGObjCRuntime::getMessageSendInfo(const ObjCMethodDecl *method,
  ...
  toolchain::PointerType *signatureType =
      toolchain::PointerType::get(CGM.getLLVMContext(), ProgramAS);
...
bool CGObjCRuntime::canMessageReceiverBeNull(CodeGenFunction &CGF,
  ...
  if (auto curMethod =
    ...
    if (self->getType().isConstQualified()) {
      if (auto LI = dyn_cast<toolchain::LoadInst>(receiver->stripPointerCasts())) {
        toolchain::Value *selfAddr = CGF.GetAddrOfLocalVar(self).emitRawPointer(CGF);
...
toolchain::Constant *
language::Core::CodeGen::emitObjCProtocolObject(CodeGenModule &CGM,
