...
#include "language/Core/AST/NSAPI.h"
...
#include "toolchain/IR/MatrixBuilder.h"
...
toolchain::cl::opt<bool> ClSanitizeGuardChecks(
    "ubsan-guard-checks", toolchain::cl::Optional,
    toolchain::cl::desc("Guard UBSAN checks with `toolchain.allow.ubsan.check()`."));
...
static toolchain::StringRef GetUBSanTrapForHandler(SanitizerHandler ID) {
  switch (ID) {
...
#undef SANITIZER_CHECK
  ...
  toolchain_unreachable("unhandled switch case");
}

/// CreateTempAlloca - This creates a alloca and inserts it into the entry
/// block.
RawAddress
CodeGenFunction::CreateTempAllocaWithoutCast(toolchain::Type *Ty, CharUnits Align,
                                             const Twine &Name,
...
RawAddress CodeGenFunction::CreateTempAlloca(toolchain::Type *Ty, LangAS DestLangAS,
  ...
  if (AllocaAddr)
    *AllocaAddr = Alloca;
  toolchain::Value *V = Alloca.getPointer();
  ...
  if (DestAddrSpace != Alloca.getAddressSpace()) {
    toolchain::IRBuilderBase::InsertPointGuard IPG(Builder);
...
toolchain::AllocaInst *CodeGenFunction::CreateTempAlloca(toolchain::Type *Ty,
                                                    const Twine &Name,
                                                    toolchain::Value *ArraySize) {
  ...
  if (ArraySize)
    ...
    Alloca =
        new toolchain::AllocaInst(Ty, CGM.getDataLayout().getAllocaAddrSpace(),
...
RawAddress CodeGenFunction::CreateDefaultAlignTempAlloca(toolchain::Type *Ty,
                                                         const Twine &Name) {
  CharUnits Align =
...
RawAddress CodeGenFunction::CreateMemTemp(QualType Ty, CharUnits Align,
  ...
  if (Ty->isConstantMatrixType()) {
    auto *ArrayTy = cast<toolchain::ArrayType>(Result.getElementType());
    auto *VectorTy = toolchain::FixedVectorType::get(ArrayTy->getElementType(),
...
toolchain::Value *CodeGenFunction::EvaluateExprAsBool(const Expr *E) {
  PGO->setCurrentStmt(E);
  if (const MemberPointerType *MPT = E->getType()->getAs<MemberPointerType>()) {
    toolchain::Value *MemPtr = EmitScalarExpr(E);
...
pushTemporaryCleanup(CodeGenFunction &CGF, const MaterializeTemporaryExpr *M,
  ...
  if (auto Lifetime = M->getType().getObjCLifetime()) {
    ...
    case Qualifiers::OCL_Weak:
      ...
      case SD_Dynamic:
        toolchain_unreachable("temporary cannot have dynamic storage duration");
      }
      toolchain_unreachable("unknown storage duration");
    }
  }
  ...
  if (DK != QualType::DK_none) {
    ...
    case SD_Thread: {
      ...
      toolchain::FunctionCallee CleanupFn;
      toolchain::Constant *CleanupArg;
      if (E->getType()->isArrayType()) {
        CleanupFn = CodeGenFunction(CGF.CGM).generateDestroyHelper(
            ReferenceTemporary, E->getType(), CodeGenFunction::destroyCXXObject,
            CGF.getLangOpts().Exceptions,
            dyn_cast_or_null<VarDecl>(M->getExtendingDecl()));
        CleanupArg = toolchain::Constant::getNullValue(CGF.Int8PtrTy);
      } else {
        CleanupFn = CGF.CGM.getAddrAndTypeOfCXXStructor(
            GlobalDecl(ReferenceTemporaryDtor, Dtor_Complete));
        CleanupArg =
            cast<toolchain::Constant>(ReferenceTemporary.emitRawPointer(CGF));
      }
...
static RawAddress createReferenceTemporary(CodeGenFunction &CGF,
    ...
    if (CGF.CGM.getCodeGenOpts().MergeAllConstants &&
      ...
      if (auto Init = ConstantEmitter(CGF).tryEmitAbstract(Inner, Ty)) {
        ...
        auto *GV = new toolchain::GlobalVariable(
            ...
            toolchain::GlobalValue::PrivateLinkage, Init, ".ref.tmp", nullptr,
            toolchain::GlobalValue::NotThreadLocal,
            CGF.getContext().getTargetAddressSpace(AS));
        CharUnits alignment = CGF.getContext().getTypeAlignInChars(Ty);
        GV->setAlignment(alignment.getAsAlign());
        toolchain::Constant *C = GV;
  ...
  case SD_Dynamic:
    toolchain_unreachable("temporary can't have dynamic storage duration");
  }
  toolchain_unreachable("unknown storage duration");
}
...
EmitMaterializeTemporaryExpr(const MaterializeTemporaryExpr *M) {
  ...
  if (ownership != Qualifiers::OCL_None &&
    ...
    if (auto *Var = dyn_cast<toolchain::GlobalVariable>(Object.getPointer())) {
      toolchain::Type *Ty = ConvertTypeForMem(E->getType());
    ...
    switch (getEvaluationKind(E->getType())) {
    default: toolchain_unreachable("expected scalar or aggregate expression");
    case TEK_Scalar:
  ...
  if (auto *Var = dyn_cast<toolchain::GlobalVariable>(
          Object.getPointer()->stripPointerCasts())) {
    toolchain::Type *TemporaryType = ConvertTypeForMem(E->getType());
    ...
    case SD_FullExpression: {
      ...
      // Avoid creating a conditional cleanup just to hold an toolchain.lifetime.end
      ...
      if (isInConditionalBranch() && !E->getType().isDestructedType() &&
        ...
        toolchain::BasicBlock *Block = OldConditional->getStartingBlock();
        Builder.restoreIP(CGBuilderTy::InsertPoint(
            Block, toolchain::BasicBlock::iterator(Block->back())));
      }
  ...
  for (SubobjectAdjustment &Adjustment : toolchain::reverse(Adjustments)) {
    ...
    case SubobjectAdjustment::MemberPointerAdjustment: {
      toolchain::Value *Ptr = EmitScalarExpr(Adjustment.Ptr.RHS);
...
CodeGenFunction::EmitReferenceBindingToExpr(const Expr *E) {
  ...
  assert(LV.isSimple());
  toolchain::Value *Value = LV.getPointer(*this);
...
unsigned CodeGenFunction::getAccessedFieldNo(unsigned Idx,
                                             const toolchain::Constant *Elts) {
  return cast<toolchain::ConstantInt>(Elts->getAggregateElement(Idx))
      ->getZExtValue();
}

static toolchain::Value *emitHashMix(CGBuilderTy &Builder, toolchain::Value *Acc,
                                toolchain::Value *Ptr) {
  toolchain::Value *A0 =
...
void CodeGenFunction::EmitTypeCheck(TypeCheckKind TCK, SourceLocation Loc,
                                    toolchain::Value *Ptr, QualType Ty,
                                    CharUnits Alignment,
  ...
  auto PtrToAlloca = dyn_cast<toolchain::AllocaInst>(Ptr->stripPointerCasts());

  toolchain::Value *IsNonNull = nullptr;
  bool IsGuaranteedNonNull =
      SkippedChecks.has(SanitizerKind::Null) || PtrToAlloca;

  toolchain::BasicBlock *Done = nullptr;
...

  {
    ...
    SmallVector<std::pair<toolchain::Value *, SanitizerKind::SanitizerOrdinal>, 3>
        Checks;

    toolchain::Value *True = toolchain::ConstantInt::getTrue(getLLVMContext());
    ...
    if ((SanOpts.has(SanitizerKind::Null) || AllowNullPointers) &&
      ...
      if (!IsGuaranteedNonNull) {
        if (AllowNullPointers) {
          ...
          toolchain::BasicBlock *Rest = createBasicBlock("not.null");
    ...
    if (SanOpts.has(SanitizerKind::ObjectSize) &&
      ...
      toolchain::Value *Size = toolchain::ConstantInt::get(IntPtrTy, TySize);
      if (ArraySize)
        Size = Builder.CreateMul(Size, ArraySize);

      // Degenerate case: new X[0] does not need an objectsize check.
      toolchain::Constant *ConstantSize = dyn_cast<toolchain::Constant>(Size);
      if (!ConstantSize || !ConstantSize->isNullValue()) {
        ...
        toolchain::Type *Tys[2] = {IntPtrTy, Int8PtrTy};
        toolchain::Function *F = CGM.getIntrinsic(toolchain::Intrinsic::objectsize, Tys);
        toolchain::Value *Min = Builder.getFalse();
        toolchain::Value *NullIsUnknown = Builder.getFalse();
        toolchain::Value *Dynamic = Builder.getFalse();
        toolchain::Value *LargeEnough = Builder.CreateICmpUGE(
    ...
    toolchain::MaybeAlign AlignVal;
    toolchain::Value *PtrAsInt = nullptr;
    ...
    if (SanOpts.has(SanitizerKind::Alignment) &&
      ...
      if (AlignVal && *AlignVal > toolchain::Align(1) &&
          (!PtrToAlloca || PtrToAlloca->getAlign() < *AlignVal)) {
        PtrAsInt = Builder.CreatePtrToInt(Ptr, IntPtrTy);
        toolchain::Value *Align = Builder.CreateAnd(
            PtrAsInt, toolchain::ConstantInt::get(IntPtrTy, AlignVal->value() - 1));
        toolchain::Value *Aligned =
            Builder.CreateICmpEQ(Align, toolchain::ConstantInt::get(IntPtrTy, 0));
        if (Aligned != True)
          Checks.push_back(
    ...
    if (Checks.size() > 0) {
      toolchain::Constant *StaticData[] = {
          ...
          toolchain::ConstantInt::get(Int8Ty, AlignVal ? toolchain::Log2(*AlignVal) : 1),
          toolchain::ConstantInt::get(Int8Ty, TCK)};
  ...
  if (SanOpts.has(SanitizerKind::Vptr) &&
    ...
    if (!IsGuaranteedNonNull) {
      ...
      toolchain::BasicBlock *VptrNotNull = createBasicBlock("vptr.not.null");
    ...
    if (!CGM.getContext().getNoSanitizeList().containsType(SanitizerKind::Vptr,
      ...
      toolchain::Value *TypeHash =
          toolchain::ConstantInt::get(Int64Ty, xxh3_64bits(Out.str()));

      toolchain::Type *VPtrTy = toolchain::PointerType::get(getLLVMContext(), 0);
      Address VPtrAddr(Ptr, IntPtrTy, getPointerAlign());
      toolchain::Value *VPtrVal = GetVTablePtr(VPtrAddr, VPtrTy,
                                          Ty->getAsCXXRecordDecl(),
                                          VTableAuthMode::UnsafeUbsanStrip);
      VPtrVal = Builder.CreateBitOrPointerCast(VPtrVal, IntPtrTy);

      toolchain::Value *Hash =
          emitHashMix(Builder, TypeHash, Builder.CreateZExt(VPtrVal, Int64Ty));
      Hash = Builder.CreateTrunc(Hash, IntPtrTy);

      // Look the hash up in our cache.
      const int CacheSize = 128;
      toolchain::Type *HashTable = toolchain::ArrayType::get(IntPtrTy, CacheSize);
      toolchain::Value *Cache = CGM.CreateRuntimeVariable(HashTable,
                                                     "__ubsan_vptr_type_cache");
      toolchain::Value *Slot = Builder.CreateAnd(Hash,
                                            toolchain::ConstantInt::get(IntPtrTy,
                                                                   CacheSize-1));
      toolchain::Value *Indices[] = { Builder.getInt32(0), Slot };
      toolchain::Value *CacheVal = Builder.CreateAlignedLoad(
      ...
      // diagnostic.
      toolchain::Value *EqualHash = Builder.CreateICmpEQ(CacheVal, Hash);
      toolchain::Constant *StaticData[] = {
      ...
      toolchain::Value *DynamicData[] = { Ptr, Hash };
...
toolchain::Value *CodeGenFunction::LoadPassedObjectSize(const Expr *E,
                                                   QualType EltTy) {
  ...
  toolchain::Value *SizeInBytes = EmitLoadOfScalar(AddrOfSize, /*Volatile=*/false,
                                              C.getSizeType(), E->getExprLoc());
  toolchain::Value *SizeOfElement =
      toolchain::ConstantInt::get(SizeInBytes->getType(), EltSize);
  return Builder.CreateUDiv(SizeInBytes, SizeOfElement);
}

/// If Base is known to point to the start of an array, return the length of
/// that array. Return 0 if the length cannot be determined.
static toolchain::Value *getArrayIndexingBound(CodeGenFunction &CGF,
                                          const Expr *Base,
  ...
  if (toolchain::Value *POS = CGF.LoadPassedObjectSize(Base, EltTy)) {
    IndexedType = Base->getType();
    return POS;
  }
...
using RecIndicesTy = SmallVector<toolchain::Value *, 8>;
...
toolchain::Value *CodeGenFunction::GetCountedByFieldExprGEP(
  ...
  return Builder.CreateInBoundsGEP(ConvertType(T), Res,
                                   RecIndicesTy(toolchain::reverse(Indices)),
                                   "counted_by.gep");
}
...
toolchain::Value *CodeGenFunction::EmitLoadOfCountedByField(
    const Expr *Base, const FieldDecl *FAMDecl, const FieldDecl *CountDecl) {
  if (toolchain::Value *GEP = GetCountedByFieldExprGEP(Base, FAMDecl, CountDecl))
...
void CodeGenFunction::EmitBoundsCheckImpl(const Expr *E, toolchain::Value *Bound,
  ...
  toolchain::Value *IndexVal = Builder.CreateIntCast(Index, SizeTy, IndexSigned);
  toolchain::Value *BoundVal = Builder.CreateIntCast(Bound, SizeTy, false);
  ...
  toolchain::Value *Check = Accessed ? Builder.CreateICmpULT(IndexVal, BoundVal)
...
EmitComplexPrePostIncDec(const UnaryOperator *E, LValue LV,
  ...
  toolchain::Value *NextVal;
  if (isa<toolchain::IntegerType>(InVal.first->getType())) {
    uint64_t AmountVal = isInc ? 1 : -1;
    NextVal = toolchain::ConstantInt::get(InVal.first->getType(), AmountVal, true);

    // Add the inc/dec to the real part.
    NextVal = Builder.CreateAdd(InVal.first, NextVal, isInc ? "inc" : "dec");
  } else {
    QualType ElemTy = E->getType()->castAs<ComplexType>()->getElementType();
    toolchain::APFloat FVal(getContext().getFloatTypeSemantics(ElemTy), 1);
    if (!isInc)
      FVal.changeSign();
    NextVal = toolchain::ConstantFP::get(getLLVMContext(), FVal);
...
toolchain::Value *CodeGenFunction::EmitNonNullRValueCheck(RValue RV, QualType T) {
  toolchain::Value *V = RV.getScalarVal();
  if (auto MPT = T->getAs<MemberPointerType>())
    return CGM.getCXXABI().EmitMemberPointerIsNotNull(*this, V, MPT);
  return Builder.CreateICmpNE(V, toolchain::Constant::getNullValue(V->getType()));
}
...
RValue CodeGenFunction::GetUndefRValue(QualType Ty) {
  ...
  case TEK_Complex: {
    ...
    toolchain::Value *U = toolchain::UndefValue::get(EltTy);
  ...
  case TEK_Scalar:
    return RValue::get(toolchain::UndefValue::get(ConvertType(Ty)));
  }
  toolchain_unreachable("bad evaluation kind");
}
...
LValue CodeGenFunction::EmitUnsupportedLValue(const Expr *E,
                                              const char *Name) {
  ErrorUnsupported(E, Name);
  toolchain::Type *ElTy = ConvertType(E->getType());
  toolchain::Type *Ty = UnqualPtrTy;
  return MakeAddrLValue(
      Address(toolchain::UndefValue::get(Ty), ElTy, CharUnits::One()), E->getType());
}
...
LValue CodeGenFunction::EmitLValueHelper(const Expr *E,
  ...
  case Expr::ObjCPropertyRefExprClass:
    toolchain_unreachable("cannot emit a property reference directly");
  ...
  case Expr::ConstantExprClass: {
    ...
    if (toolchain::Value *Result = ConstantEmitter(*this).tryEmitConstantExpr(CE)) {
  ...
  case Expr::ExprWithCleanupsClass: {
    ...
    if (LV.isSimple()) {
      ...
      // cleanups.
      Address Addr = LV.getAddress();
      toolchain::Value *V = Addr.getBasePointer();
  ...
  case Expr::HLSLOutArgExprClass:
    toolchain_unreachable("cannot emit a HLSL out argument directly");
  }
}
...
CodeGenFunction::tryEmitAsConstant(const DeclRefExpr *RefExpr) {
  ...
  // Emit as a constant.
  toolchain::Constant *C = ConstantEmitter(*this).emitAbstract(
...
toolchain::Value *CodeGenFunction::emitScalarConstant(
...
toolchain::Value *CodeGenFunction::EmitLoadOfScalar(LValue lvalue,
                                               SourceLocation Loc) {
...
static bool getRangeForType(CodeGenFunction &CGF, QualType Ty,
                            toolchain::APInt &Min, toolchain::APInt &End,
  ...
  if (IsBool) {
    Min = toolchain::APInt(CGF.getContext().getTypeSize(Ty), 0);
    End = toolchain::APInt(CGF.getContext().getTypeSize(Ty), 2);
  } else {
...
toolchain::MDNode *CodeGenFunction::getRangeForLoadFromType(QualType Ty) {
  toolchain::APInt Min, End;
...
void CodeGenFunction::maybeAttachRangeForLoad(toolchain::LoadInst *Load, QualType Ty,
                                              SourceLocation Loc) {
  if (EmitScalarRangeCheck(Load, Ty, Loc)) {
    // In order to prevent the optimizer from throwing away the check, don't
    // attach range metadata to the load.
  } else if (CGM.getCodeGenOpts().OptimizationLevel > 0) {
    if (toolchain::MDNode *RangeInfo = getRangeForLoadFromType(Ty)) {
      Load->setMetadata(toolchain::LLVMContext::MD_range, RangeInfo);
      Load->setMetadata(toolchain::LLVMContext::MD_noundef,
                        toolchain::MDNode::get(CGM.getLLVMContext(), {}));
    }
  }
}

bool CodeGenFunction::EmitScalarRangeCheck(toolchain::Value *Value, QualType Ty,
                                           SourceLocation Loc) {
  ...
  if (IsBool &&
      cast<toolchain::IntegerType>(Value->getType())->getBitWidth() == 1)
    return false;
  ...
  toolchain::Value *Check;
  --End;
  if (!Min) {
    Check = Builder.CreateICmpULE(Value, toolchain::ConstantInt::get(Ctx, End));
  } else {
    toolchain::Value *Upper =
        Builder.CreateICmpSLE(Value, toolchain::ConstantInt::get(Ctx, End));
    toolchain::Value *Lower =
        Builder.CreateICmpSGE(Value, toolchain::ConstantInt::get(Ctx, Min));
    Check = Builder.CreateAnd(Upper, Lower);
  }
  toolchain::Constant *StaticArgs[] = {EmitCheckSourceLocation(Loc),
                                  EmitCheckTypeDescriptor(Ty)};
  EmitCheck(std::make_pair(Check, Kind), CheckHandler, StaticArgs, Value);
  return true;
}

toolchain::Value *CodeGenFunction::EmitLoadOfScalar(Address Addr, bool Volatile,
                                               QualType Ty,
                                               SourceLocation Loc,
                                               LValueBaseInfo BaseInfo,
                                               TBAAAccessInfo TBAAInfo,
                                               bool isNontemporal) {
  if (auto *GV = dyn_cast<toolchain::GlobalValue>(Addr.getBasePointer()))
    if (GV->isThreadLocal())
  ...
  if (const auto *ClangVecTy = Ty->getAs<VectorType>()) {
    ...
    if (ClangVecTy->isPackedVectorBoolType(getContext())) {
      toolchain::Type *ValTy = ConvertType(Ty);
      unsigned ValNumElems =
          cast<toolchain::FixedVectorType>(ValTy)->getNumElements();
      ...
      // Bitcast iP --> <P x i1>.
      auto *PaddedVecTy = toolchain::FixedVectorType::get(
          Builder.getInt1Ty(), RawIntTy->getPrimitiveSizeInBits());
      toolchain::Value *V = Builder.CreateBitCast(RawIntV, PaddedVecTy);
    ...
    auto *VTy = cast<toolchain::FixedVectorType>(Addr.getElementType());
    auto *NewVecTy =
        CGM.getABIInfo().getOptimalVectorMemoryType(VTy, getLangOpts());

    if (VTy != NewVecTy) {
      Address Cast = Addr.withElementType(NewVecTy);
      toolchain::Value *V = Builder.CreateLoad(Cast, Volatile, "loadVecN");
  ...
  toolchain::LoadInst *Load = Builder.CreateLoad(Addr, Volatile);
  if (isNontemporal) {
    toolchain::MDNode *Node = toolchain::MDNode::get(
        Load->getContext(), toolchain::ConstantAsMetadata::get(Builder.getInt32(1)));
    Load->setMetadata(toolchain::LLVMContext::MD_nontemporal, Node);
  }
...
toolchain::Value *CodeGenFunction::EmitToMemory(toolchain::Value *Value, QualType Ty) {
  if (auto *AtomicTy = Ty->getAs<AtomicType>())
    Ty = AtomicTy->getValueType();

  if (Ty->isExtVectorBoolType()) {
    toolchain::Type *StoreTy = convertTypeForLoadStore(Ty, Value->getType());
...
/// by ConvertType).
toolchain::Value *CodeGenFunction::EmitFromMemory(toolchain::Value *Value, QualType Ty) {
  if (auto *AtomicTy = Ty->getAs<AtomicType>())
    Ty = AtomicTy->getValueType();
  ...
  if (Ty->isPackedVectorBoolType(getContext())) {
    ...
    // Shuffle <P x i1> --> <N x i1> (N is the actual bit size).
    toolchain::Type *ValTy = ConvertType(Ty);
    unsigned ValNumElems = cast<toolchain::FixedVectorType>(ValTy)->getNumElements();
    return emitBoolVecConversion(V, ValNumElems, "extractvec");
  }

  toolchain::Type *ResTy = ConvertType(Ty);
  if (Ty->hasBooleanRepresentation() || Ty->isBitIntType() ||
...
static RawAddress MaybeConvertMatrixAddress(RawAddress Addr,
                                            CodeGenFunction &CGF,
                                            bool IsVector = true) {
  auto *ArrayTy = dyn_cast<toolchain::ArrayType>(Addr.getElementType());
  if (ArrayTy && IsVector) {
    auto *VectorTy = toolchain::FixedVectorType::get(ArrayTy->getElementType(),
                                                ArrayTy->getNumElements());
  ...
  auto *VectorTy = dyn_cast<toolchain::VectorType>(Addr.getElementType());
  if (VectorTy && !IsVector) {
    auto *ArrayTy = toolchain::ArrayType::get(
        VectorTy->getElementType(),
        cast<toolchain::FixedVectorType>(VectorTy)->getNumElements());
...
// (VectorType).
static void EmitStoreOfMatrixScalar(toolchain::Value *value, LValue lvalue,
...
void CodeGenFunction::EmitStoreOfScalar(toolchain::Value *Value, Address Addr,
  ...
  // Handles vectors of sizes that are likely to be expanded to a larger size
  // to optimize performance.
  toolchain::Type *SrcTy = Value->getType();
  if (const auto *ClangVecTy = Ty->getAs<VectorType>()) {
    if (auto *VecTy = dyn_cast<toolchain::FixedVectorType>(SrcTy)) {
      auto *NewVecTy =
  ...
  toolchain::StoreInst *Store = Builder.CreateStore(Value, Addr, Volatile);
  addInstToCurrentSourceAtom(Store, Value);

  if (isNontemporal) {
    toolchain::MDNode *Node =
        toolchain::MDNode::get(Store->getContext(),
                          toolchain::ConstantAsMetadata::get(Builder.getInt32(1)));
    Store->setMetadata(toolchain::LLVMContext::MD_nontemporal, Node);
  }

  CGM.DecorateInstructionWithTBAA(Store, TBAAInfo);
}

void CodeGenFunction::EmitStoreOfScalar(toolchain::Value *value, LValue lvalue,
                                        bool isInit) {
...
RValue CodeGenFunction::EmitLoadOfLValue(LValue LV, SourceLocation Loc) {
  ...
  if (PointerAuthQualifier PtrAuth = LV.getQuals().getPointerAuth()) {
    ...
    toolchain::Value *Value = EmitLoadOfLValue(LV, Loc).getScalarVal();
  ...
  if (LV.getQuals().getObjCLifetime() == Qualifiers::OCL_Weak) {
    ...
    toolchain::Value *Object = EmitARCLoadWeakRetained(LV.getAddress());
  ...
  if (LV.isVectorElt()) {
    toolchain::LoadInst *Load = Builder.CreateLoad(LV.getVectorAddress(),
  ...
  if (LV.isMatrixElt()) {
    toolchain::Value *Idx = LV.getMatrixIdx();
    if (CGM.getCodeGenOpts().OptimizationLevel > 0) {
      const auto *const MatTy = LV.getType()->castAs<ConstantMatrixType>();
      toolchain::MatrixBuilder MB(Builder);
      MB.CreateIndexAssumption(Idx, MatTy->getNumElementsFlattened());
    }
    toolchain::LoadInst *Load =
...
RValue CodeGenFunction::EmitLoadOfBitfieldLValue(LValue LV,
  ...
  // Get the output type.
  toolchain::Type *ResLTy = ConvertType(LV.getType());
  ...
  if (Info.IsSigned) {
    ...
    if (static_cast<unsigned>(Offset) + Info.Size < StorageSize)
      Val = Builder.CreateAnd(
          Val, toolchain::APInt::getLowBitsSet(StorageSize, Info.Size), "bf.clear");
  }
...
RValue CodeGenFunction::EmitLoadOfExtVectorElementLValue(LValue LV) {
  toolchain::Value *Vec = Builder.CreateLoad(LV.getExtVectorAddress(),
                                        LV.isVolatileQualified());
  ...
  if (getLangOpts().HLSL && !Vec->getType()->isVectorTy()) {
    toolchain::Type *DstTy = toolchain::FixedVectorType::get(Vec->getType(), 1);
    toolchain::Value *Zero = toolchain::Constant::getNullValue(CGM.Int64Ty);
    Vec = Builder.CreateInsertElement(DstTy, Vec, Zero, "cast.splat");
  }

  const toolchain::Constant *Elts = LV.getExtVectorElts();
  ...
  if (!ExprVT) {
    ...
    toolchain::Value *Elt = toolchain::ConstantInt::get(SizeTy, InIdx);

    toolchain::Value *Element = Builder.CreateExtractElement(Vec, Elt);

    toolchain::Type *LVTy = ConvertType(LV.getType());
...
Address CodeGenFunction::EmitExtVectorElementLValue(LValue LV) {
  ...
  toolchain::Type *VectorElementTy = CGM.getTypes().ConvertType(EQT);
...
RValue CodeGenFunction::EmitLoadOfGlobalRegLValue(LValue LV) {
  ...
  toolchain::MDNode *RegName = cast<toolchain::MDNode>(
      cast<toolchain::MetadataAsValue>(LV.getGlobalReg())->getMetadata());

  // We accept integer and pointer types only
  toolchain::Type *OrigTy = CGM.getTypes().ConvertType(LV.getType());
  toolchain::Type *Ty = OrigTy;
  if (OrigTy->isPointerTy())
    Ty = CGM.getTypes().getDataLayout().getIntPtrType(OrigTy);
  toolchain::Type *Types[] = { Ty };

  toolchain::Function *F = CGM.getIntrinsic(toolchain::Intrinsic::read_register, Types);
  toolchain::Value *Call = Builder.CreateCall(
      F, toolchain::MetadataAsValue::get(Ty->getContext(), RegName));
  if (OrigTy->isPointerTy())
...
void CodeGenFunction::EmitStoreThroughLValue(RValue Src, LValue Dst,
  ...
  if (!Dst.isSimple()) {
    if (Dst.isVectorElt()) {
      ...
      toolchain::Value *Vec = Builder.CreateLoad(Dst.getVectorAddress(),
                                            Dst.isVolatileQualified());
      toolchain::Type *VecTy = Vec->getType();
      toolchain::Value *SrcVal = Src.getScalarVal();

      if (SrcVal->getType()->getPrimitiveSizeInBits() <
          VecTy->getScalarSizeInBits())
        SrcVal = Builder.CreateZExt(SrcVal, VecTy->getScalarType());

      auto *IRStoreTy = dyn_cast<toolchain::IntegerType>(Vec->getType());
      if (IRStoreTy) {
        auto *IRVecTy = toolchain::FixedVectorType::get(
      ...
      // an ABI).
      if (auto *EltTy = dyn_cast<toolchain::FixedVectorType>(SrcVal->getType());
    ...
    if (Dst.isMatrixElt()) {
      toolchain::Value *Idx = Dst.getMatrixIdx();
      if (CGM.getCodeGenOpts().OptimizationLevel > 0) {
        const auto *const MatTy = Dst.getType()->castAs<ConstantMatrixType>();
        toolchain::MatrixBuilder MB(Builder);
        MB.CreateIndexAssumption(Idx, MatTy->getNumElementsFlattened());
      }
      toolchain::Instruction *Load = Builder.CreateLoad(Dst.getMatrixAddress());
      toolchain::Value *Vec =
  ...
  if (Qualifiers::ObjCLifetime Lifetime = Dst.getQuals().getObjCLifetime()) {
    switch (Lifetime) {
    case Qualifiers::OCL_None:
      toolchain_unreachable("present but none");
  ...
  if (Dst.isObjCWeak() && !Dst.isNonGC()) {
    ...
    toolchain::Value *src = Src.getScalarVal();
  ...
  if (Dst.isObjCStrong() && !Dst.isNonGC()) {
    ...
    if (Dst.isObjCIvar()) {
      ...
      toolchain::Type *ResultType = IntPtrTy;
      Address dst = EmitPointerWithAlignment(Dst.getBaseIvarExp());
      toolchain::Value *RHS = dst.emitRawPointer(*this);
      RHS = Builder.CreatePtrToInt(RHS, ResultType, "sub.ptr.rhs.cast");
      toolchain::Value *LHS = Builder.CreatePtrToInt(LvalueDst.emitRawPointer(*this),
                                                ResultType, "sub.ptr.lhs.cast");
      toolchain::Value *BytesBetween = Builder.CreateSub(LHS, RHS, "ivar.offset");
...
void CodeGenFunction::EmitStoreThroughBitfieldLValue(RValue Src, LValue Dst,
  ...
  toolchain::Type *ResLTy = convertTypeForLoadStore(Dst.getType());
  ...
  SrcVal = Builder.CreateIntCast(SrcVal, Ptr.getElementType(),
                                 /*isSigned=*/false);
  toolchain::Value *MaskedVal = SrcVal;
  ...
  if (StorageSize != Info.Size) {
    ...
    if (!Dst.getType()->hasBooleanRepresentation())
      SrcVal = Builder.CreateAnd(
          SrcVal, toolchain::APInt::getLowBitsSet(StorageSize, Info.Size),
          "bf.value");
    MaskedVal = SrcVal;
    if (Offset)
    ...
    Val = Builder.CreateAnd(
        Val, ~toolchain::APInt::getBitsSet(StorageSize, Offset, Offset + Info.Size),
        "bf.clear");
  ...
  if (Result) {
    toolchain::Value *ResultVal = MaskedVal;
...
void CodeGenFunction::EmitStoreThroughExtVectorComponentLValue(RValue Src,
  ...
  // value now.
  toolchain::Value *Vec = Builder.CreateLoad(DstAddr, Dst.isVolatileQualified());
  toolchain::Type *VecTy = Vec->getType();
  const toolchain::Constant *Elts = Dst.getExtVectorElts();

  if (const VectorType *VTy = Dst.getType()->getAs<VectorType>()) {
    unsigned NumSrcElts = VTy->getNumElements();
    unsigned NumDstElts = cast<toolchain::FixedVectorType>(VecTy)->getNumElements();
    if (NumDstElts == NumSrcElts) {
      ...
      toolchain::Value *ExtSrcVal = Builder.CreateShuffleVector(SrcVal, ExtMask);
      // build identity
      SmallVector<int, 4> Mask;
      ...
      toolchain_unreachable("unexpected shorten vector length");
    }
  } else {
...
void CodeGenFunction::EmitStoreThroughGlobalRegLValue(RValue Src, LValue Dst) {
  assert((Dst.getType()->isIntegerType() || Dst.getType()->isPointerType()) &&
         "Bad type for register variable");
  toolchain::MDNode *RegName = cast<toolchain::MDNode>(
      cast<toolchain::MetadataAsValue>(Dst.getGlobalReg())->getMetadata());
  assert(RegName && "Register LValue is not metadata");

  // We accept integer and pointer types only
  toolchain::Type *OrigTy = CGM.getTypes().ConvertType(Dst.getType());
  toolchain::Type *Ty = OrigTy;
  if (OrigTy->isPointerTy())
    Ty = CGM.getTypes().getDataLayout().getIntPtrType(OrigTy);
  toolchain::Type *Types[] = { Ty };

  toolchain::Function *F = CGM.getIntrinsic(toolchain::Intrinsic::write_register, Types);
  toolchain::Value *Value = Src.getScalarVal();
  if (OrigTy->isPointerTy())
    Value = Builder.CreatePtrToInt(Value, Ty);
  Builder.CreateCall(
      F, {toolchain::MetadataAsValue::get(Ty->getContext(), RegName), Value});
}
...
static LValue EmitThreadPrivateVarDeclLValue(
    ...
    toolchain::Type *RealVarTy, SourceLocation Loc) {
...
CodeGenFunction::EmitLoadOfReference(LValue RefLVal,
  ...
  if (!PTy->isIncompleteType()) {
    ...
    if (CGM.getTypes().getTargetAddressSpace(PTy) == 0 &&
      ...
      Load->setMetadata(toolchain::LLVMContext::MD_nonnull,
                        toolchain::MDNode::get(Ctx, {}));
    // Emit !align metadata
    if (PTy->isObjectType()) {
      ...
      if (AlignVal > 1) {
        Load->setMetadata(
            ...
            toolchain::MDNode::get(Ctx, MDB.createConstant(toolchain::ConstantInt::get(
...
Address CodeGenFunction::EmitLoadOfPointer(Address Ptr,
                                           ...
                                           LValueBaseInfo *BaseInfo,
                                           TBAAAccessInfo *TBAAInfo) {
  toolchain::Value *Addr = Builder.CreateLoad(Ptr);
...
toolchain::Constant *CodeGenModule::getRawFunctionPointer(GlobalDecl GD,
                                                     toolchain::Type *Ty) {
  ...
  toolchain::Constant *V = GetAddrOfFunction(GD, Ty);
  return V;
}
...
static LValue EmitFunctionDeclLValue(CodeGenFunction &CGF, const Expr *E,
                                     GlobalDecl GD) {
  const FunctionDecl *FD = cast<FunctionDecl>(GD.getDecl());
  toolchain::Constant *V = CGF.CGM.getFunctionPointer(GD);
  QualType ETy = E->getType();
  if (ETy->isCFIUncheckedCalleeFunctionType()) {
    if (auto *GV = dyn_cast<toolchain::GlobalValue>(V))
      V = toolchain::NoCFIValue::get(GV);
  }
...
/// @toolchain.read/write_register.
/// So far, only the name is being passed down, but other options such as
/// register type, allocation type or even optimization options could be
/// passed down via the metadata node.
static LValue EmitGlobalNamedRegister(const VarDecl *VD, CodeGenModule &CGM) {
  SmallString<64> Name("toolchain.named.register.");
  AsmLabelAttr *Asm = VD->getAttr<AsmLabelAttr>();
  assert(Asm->getLabel().size() < 64-Name.size() &&
      "Register name too big");
  Name.append(Asm->getLabel());
  toolchain::NamedMDNode *M =
    CGM.getModule().getOrInsertNamedMetadata(Name);
  if (M->getNumOperands() == 0) {
    toolchain::MDString *Str = toolchain::MDString::get(CGM.getLLVMContext(),
                                              Asm->getLabel());
    toolchain::Metadata *Ops[] = {Str};
    M->addOperand(toolchain::MDNode::get(CGM.getLLVMContext(), Ops));
  }

  CharUnits Alignment = CGM.getContext().getDeclAlign(VD);

  toolchain::Value *Ptr =
    toolchain::MetadataAsValue::get(CGM.getLLVMContext(), M->getOperand(0));
...
static bool canEmitSpuriousReferenceToVariable(CodeGenFunction &CGF,
  ...
  case toolchain::GlobalValue::ExternalLinkage:
  case toolchain::GlobalValue::LinkOnceODRLinkage:
  case toolchain::GlobalValue::WeakODRLinkage:
  case toolchain::GlobalValue::InternalLinkage:
  case toolchain::GlobalValue::PrivateLinkage:
    return true;
  default:
    return false;
  }
...
LValue CodeGenFunction::EmitDeclRefLValue(const DeclRefExpr *E) {
  ...
  if (const auto *VD = dyn_cast<VarDecl>(ND)) {
    ...
    if (E->isNonOdrUse() == NOUR_Constant &&
      ...
      toolchain::Constant *Val = ConstantEmitter(*this).emitAbstract(
      ...
      if (!VD->getType()->isReferenceType()) {
        ...
        toolchain::Type *VarTy = getTypes().ConvertTypeForMem(VD->getType());
        auto *PTy = toolchain::PointerType::get(
  ...
  if (const auto *VD = dyn_cast<VarDecl>(ND)) {
    ...
    // some reason; most likely, because it's in an outer function.
      ...
      toolchain::Constant *var = CGM.getOrCreateStaticVarDecl(
    ...
    // No other cases for now.
      ...
      toolchain_unreachable("DeclRefExpr for Decl not entered in LocalDeclMap?");
    }
  ...
  if (const auto *TPO = dyn_cast<TemplateParamObjectDecl>(ND)) {
    ...
    if (AS != T.getAddressSpace()) {
      ...
      auto PtrTy = toolchain::PointerType::get(CGM.getLLVMContext(), TargetAS);
  ...
  toolchain_unreachable("Unhandled DeclRefExpr");
}
...
LValue CodeGenFunction::EmitUnaryOpLValue(const UnaryOperator *E) {
  ...
  switch (E->getOpcode()) {
  default: toolchain_unreachable("Unknown unary operator lvalue!");
  case UO_Deref: {
...
LValue CodeGenFunction::EmitPredefinedLValue(const PredefinedExpr *E) {
  ...
  std::string GVName = toolchain::join(NameItems, NameItems + 2, ".");
...
/// anything else.
toolchain::Constant *CodeGenFunction::EmitCheckTypeDescriptor(QualType T) {
  // Only emit each type's descriptor once.
  if (toolchain::Constant *C = CGM.getTypeDescriptorFromMap(T))
    return C;
  ...
  if (T->isIntegerType()) {
    TypeKind = TK_Integer;
    TypeInfo = (toolchain::Log2_32(getContext().getTypeSize(T)) << 1) |
  ...
  if (IsBitInt) {
    ...
    toolchain::support::endian::write32(S + 1, Bits,
  ...
  toolchain::Constant *Components[] = {
    Builder.getInt16(TypeKind), Builder.getInt16(TypeInfo),
    toolchain::ConstantDataArray::getString(getLLVMContext(), Buffer)
  };
  toolchain::Constant *Descriptor = toolchain::ConstantStruct::getAnon(Components);

  auto *GV = new toolchain::GlobalVariable(
      CGM.getModule(), Descriptor->getType(),
      /*isConstant=*/true, toolchain::GlobalVariable::PrivateLinkage, Descriptor);
...
toolchain::Value *CodeGenFunction::EmitCheckValue(toolchain::Value *V) {
  toolchain::Type *TargetTy = IntPtrTy;
  ...
  if (V->getType()->isFloatingPointTy()) {
    ...
    if (Bits <= TargetTy->getIntegerBitWidth())
      V = Builder.CreateBitCast(V, toolchain::Type::getIntNTy(getLLVMContext(),
                                                         Bits));
  }
...
toolchain::Constant *CodeGenFunction::EmitCheckSourceLocation(SourceLocation Loc) {
  toolchain::Constant *Filename;
  int Line, Column;
  ...
  if (PLoc.isValid()) {
    ...
    if (PathComponentsToStrip < 0) {
      ...
      auto I = toolchain::sys::path::rbegin(FilenameString);
      auto E = toolchain::sys::path::rend(FilenameString);
      while (I != E && --PathComponentsToKeep)
        ++I;

      FilenameString = FilenameString.substr(I - E);
    } else if (PathComponentsToStrip > 0) {
      auto I = toolchain::sys::path::begin(FilenameString);
      auto E = toolchain::sys::path::end(FilenameString);
      while (I != E && PathComponentsToStrip--)
        ++I;

      if (I != E)
        FilenameString =
            FilenameString.substr(I - toolchain::sys::path::begin(FilenameString));
      else
        FilenameString = toolchain::sys::path::filename(FilenameString);
    }
    ...
    Filename = toolchain::Constant::getNullValue(Int8PtrTy);
    Line = Column = 0;
  }

  toolchain::Constant *Data[] = {Filename, Builder.getInt32(Line),
                            Builder.getInt32(Column)};

  return toolchain::ConstantStruct::getAnon(Data);
}
...
static void emitCheckHandlerCall(CodeGenFunction &CGF,
                                 ...
                                 ArrayRef<toolchain::Value *> FnArgs,
                                 SanitizerHandler CheckHandler,
                                 CheckRecoverableKind RecoverKind, bool IsFatal,
                                 toolchain::BasicBlock *ContBB, bool NoMerge) {
  ...
  if (CheckInfo.Version && !MinimalRuntime)
    FnName += "_v" + toolchain::utostr(CheckInfo.Version);
  if (MinimalRuntime)
    FnName += "_minimal";
  if (NeedsAbortSuffix)
    FnName += "_abort";
  ...
  toolchain::AttrBuilder B(CGF.getLLVMContext());
  if (!MayReturn) {
    B.addAttribute(toolchain::Attribute::NoReturn)
        .addAttribute(toolchain::Attribute::NoUnwind);
  }
  B.addUWTableAttr(toolchain::UWTableKind::Default);

  toolchain::FunctionCallee Fn = CGF.CGM.CreateRuntimeFunction(
      FnType, FnName,
      toolchain::AttributeList::get(CGF.getLLVMContext(),
                               toolchain::AttributeList::FunctionIndex, B),
      /*Local=*/true);
  toolchain::CallInst *HandlerCall = CGF.EmitNounwindRuntimeCall(Fn, FnArgs);
  NoMerge = NoMerge || !CGF.CGM.getCodeGenOpts().OptimizationLevel ||
            (CGF.CurCodeDecl && CGF.CurCodeDecl->hasAttr<OptimizeNoneAttr>());
  if (NoMerge)
    HandlerCall->addFnAttr(toolchain::Attribute::NoMerge);
  if (!MayReturn) {
...
void CodeGenFunction::EmitCheck(
    ArrayRef<std::pair<toolchain::Value *, SanitizerKind::SanitizerOrdinal>> Checked,
    SanitizerHandler CheckHandler, ArrayRef<toolchain::Constant *> StaticArgs,
  ...
  toolchain::Value *FatalCond = nullptr;
  toolchain::Value *RecoverableCond = nullptr;
  toolchain::Value *TrapCond = nullptr;
  bool NoMerge = false;
  // Expand checks into:
  ...
  for (auto &[Check, Ord] : Checked) {
    toolchain::Value *GuardedCheck = Check;
    if (ClSanitizeGuardChecks ||
        (CGM.getCodeGenOpts().SanitizeSkipHotCutoffs[Ord] > 0)) {
      toolchain::Value *Allow = Builder.CreateCall(
          CGM.getIntrinsic(toolchain::Intrinsic::allow_ubsan_check),
          toolchain::ConstantInt::get(CGM.Int8Ty, Ord));
      GuardedCheck = Builder.CreateOr(Check, Builder.CreateNot(Allow));
    }

    // -fsanitize-trap= overrides -fsanitize-recover=.
    toolchain::Value *&Cond = CGM.getCodeGenOpts().SanitizeTrap.has(Ord) ? TrapCond
  ...
  toolchain::BasicBlock *Cont = createBasicBlock("cont");
  toolchain::BasicBlock *Handlers = createBasicBlock("handler." + CheckName);
  toolchain::Instruction *Branch = Builder.CreateCondBr(JointCond, Cont, Handlers);
  // Give hint that we very much don't expect to execute the handler
  toolchain::MDBuilder MDHelper(getLLVMContext());
  toolchain::MDNode *Node = MDHelper.createLikelyBranchWeights();
  Branch->setMetadata(toolchain::LLVMContext::MD_prof, Node);
  EmitBlock(Handlers);
  ...
  SmallVector<toolchain::Type *, 4> ArgTypes;
  ...
  if (!StaticArgs.empty()) {
    toolchain::Constant *Info = toolchain::ConstantStruct::getAnon(StaticArgs);
    auto *InfoPtr = new toolchain::GlobalVariable(
        ...
        /*isConstant=*/false, toolchain::GlobalVariable::PrivateLinkage, Info, "",
        nullptr, toolchain::GlobalVariable::NotThreadLocal,
        CGM.getDataLayout().getDefaultGlobalsAddressSpace());
    InfoPtr->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
    CGM.getSanitizerMetadata()->disableSanitizerForGlobal(InfoPtr);
    Args.push_back(InfoPtr);
    ArgTypes.push_back(Args.back()->getType());
  }

  for (toolchain::Value *DynamicArg : DynamicArgs) {
    Args.push_back(EmitCheckValue(DynamicArg));
    ArgTypes.push_back(IntPtrTy);
  }

  toolchain::FunctionType *FnType =
    toolchain::FunctionType::get(CGM.VoidTy, ArgTypes, false);
  ...
  if (!FatalCond || !RecoverableCond) {
    ...
    toolchain::BasicBlock *NonFatalHandlerBB =
        createBasicBlock("non_fatal." + CheckName);
    toolchain::BasicBlock *FatalHandlerBB = createBasicBlock("fatal." + CheckName);
...
void CodeGenFunction::EmitCfiSlowPathCheck(
    SanitizerKind::SanitizerOrdinal Ordinal, toolchain::Value *Cond,
    toolchain::ConstantInt *TypeId, toolchain::Value *Ptr,
    ArrayRef<toolchain::Constant *> StaticArgs) {
  toolchain::BasicBlock *Cont = createBasicBlock("cfi.cont");

  toolchain::BasicBlock *CheckBB = createBasicBlock("cfi.slowpath");
  toolchain::BranchInst *BI = Builder.CreateCondBr(Cond, Cont, CheckBB);

  toolchain::MDBuilder MDHelper(getLLVMContext());
  toolchain::MDNode *Node = MDHelper.createLikelyBranchWeights();
  BI->setMetadata(toolchain::LLVMContext::MD_prof, Node);

  EmitBlock(CheckBB);

  bool WithDiag = !CGM.getCodeGenOpts().SanitizeTrap.has(Ordinal);

  toolchain::CallInst *CheckCall;
  toolchain::FunctionCallee SlowPathFn;
  if (WithDiag) {
    toolchain::Constant *Info = toolchain::ConstantStruct::getAnon(StaticArgs);
    auto *InfoPtr =
        new toolchain::GlobalVariable(CGM.getModule(), Info->getType(), false,
                                 toolchain::GlobalVariable::PrivateLinkage, Info);
    InfoPtr->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
    CGM.getSanitizerMetadata()->disableSanitizerForGlobal(InfoPtr);

    SlowPathFn = CGM.getModule().getOrInsertFunction(
        "__cfi_slowpath_diag",
        toolchain::FunctionType::get(VoidTy, {Int64Ty, Int8PtrTy, Int8PtrTy},
                                false));
    CheckCall = Builder.CreateCall(SlowPathFn, {TypeId, Ptr, InfoPtr});
  } else {
    SlowPathFn = CGM.getModule().getOrInsertFunction(
        "__cfi_slowpath",
        toolchain::FunctionType::get(VoidTy, {Int64Ty, Int8PtrTy}, false));
    CheckCall = Builder.CreateCall(SlowPathFn, {TypeId, Ptr});
  }

  CGM.setDSOLocal(
      cast<toolchain::GlobalValue>(SlowPathFn.getCallee()->stripPointerCasts()));
...
void CodeGenFunction::EmitCfiCheckStub() {
  ...
  toolchain::Function *F = toolchain::Function::Create(
      toolchain::FunctionType::get(VoidTy, {Int64Ty, VoidPtrTy, VoidPtrTy}, false),
      toolchain::GlobalValue::WeakAnyLinkage, "__cfi_check", M);
  CGM.SetLLVMFunctionAttributes(GlobalDecl(), FI, F, /*IsThunk=*/false);
  CGM.SetLLVMFunctionAttributesForDefinition(nullptr, F);
  F->setAlignment(toolchain::Align(4096));
  CGM.setDSOLocal(F);

  toolchain::LLVMContext &Ctx = M->getContext();
  toolchain::BasicBlock *BB = toolchain::BasicBlock::Create(Ctx, "entry", F);
  // CrossDSOCFI pass is not executed if there is no executable code.
  SmallVector<toolchain::Value*> Args{F->getArg(2), F->getArg(1)};
  toolchain::CallInst::Create(M->getFunction("__cfi_check_fail"), Args, "", BB);
  toolchain::ReturnInst::Create(Ctx, nullptr, BB);
}

// This function is basically a switch over the CFI failure kind, which is
// extracted from CFICheckFailData (1st function argument). Each case is either
// toolchain.trap or a call to one of the two runtime handlers, based on
...
void CodeGenFunction::EmitCfiCheckFail() {
  ...
  toolchain::Function *F = toolchain::Function::Create(
      toolchain::FunctionType::get(VoidTy, {VoidPtrTy, VoidPtrTy}, false),
      toolchain::GlobalValue::WeakODRLinkage, "__cfi_check_fail", &CGM.getModule());

  CGM.SetLLVMFunctionAttributes(GlobalDecl(), FI, F, /*IsThunk=*/false);
  CGM.SetLLVMFunctionAttributesForDefinition(nullptr, F);
  F->setVisibility(toolchain::GlobalValue::HiddenVisibility);
  ...
  toolchain::Value *DataIsNotNullPtr =
      Builder.CreateICmpNE(Data, toolchain::ConstantPointerNull::get(Int8PtrTy));
  ...
  toolchain::StructType *SourceLocationTy =
      toolchain::StructType::get(VoidPtrTy, Int32Ty, Int32Ty);
  toolchain::StructType *CfiCheckFailDataTy =
      toolchain::StructType::get(Int8Ty, SourceLocationTy, VoidPtrTy);

  toolchain::Value *V = Builder.CreateConstGEP2_32(
      CfiCheckFailDataTy, Builder.CreatePointerCast(Data, UnqualPtrTy), 0, 0);

  Address CheckKindAddr(V, Int8Ty, getIntAlign());
  toolchain::Value *CheckKind = Builder.CreateLoad(CheckKindAddr);

  toolchain::Value *AllVtables = toolchain::MetadataAsValue::get(
      CGM.getLLVMContext(),
      toolchain::MDString::get(CGM.getLLVMContext(), "all-vtables"));
  toolchain::Value *ValidVtable = Builder.CreateZExt(
  ...
  for (auto CheckKindOrdinalPair : CheckKinds) {
    ...
    toolchain::Value *Cond =
        Builder.CreateICmpNE(CheckKind, toolchain::ConstantInt::get(Int8Ty, Kind));
...
void CodeGenFunction::EmitUnreachable(SourceLocation Loc) {
  if (SanOpts.has(SanitizerKind::Unreachable)) {
    ...
    EmitCheck(std::make_pair(static_cast<toolchain::Value *>(Builder.getFalse()),
                             CheckOrdinal),
...
void CodeGenFunction::EmitTrapCheck(toolchain::Value *Checked,
  ...
  toolchain::BasicBlock *&TrapBB = TrapBBs[CheckHandlerID];

  toolchain::DILocation *TrapLocation = Builder.getCurrentDebugLocation();
  toolchain::StringRef TrapMessage = GetUBSanTrapForHandler(CheckHandlerID);
  ...
  if (TrapBB && !NoMerge) {
    ...
    assert(isa<toolchain::CallInst>(Call) && "Expected call in trap BB");
    ...
    toolchain::CallInst *TrapCall =
        Builder.CreateCall(CGM.getIntrinsic(toolchain::Intrinsic::ubsantrap),
                           toolchain::ConstantInt::get(CGM.Int8Ty, CheckHandlerID));

    if (!CGM.getCodeGenOpts().TrapFuncName.empty()) {
      auto A = toolchain::Attribute::get(getLLVMContext(), "trap-func-name",
...
toolchain::CallInst *CodeGenFunction::EmitTrapCall(toolchain::Intrinsic::ID IntrID) {
  toolchain::CallInst *TrapCall =
...
Address CodeGenFunction::EmitArrayToPointerDecay(const Expr *E,
  ...
  toolchain::Type *NewTy = ConvertType(E->getType());
  Addr = Addr.withElementType(NewTy);

  // Note that VLA pointers are always decayed, so we don't need to do
  // anything here.
  if (!E->getType()->isVariableArrayType()) {
    assert(isa<toolchain::ArrayType>(Addr.getElementType()) &&
...
static toolchain::Value *emitArraySubscriptGEP(CodeGenFunction &CGF,
                                          toolchain::Type *elemType,
                                          toolchain::Value *ptr,
                                          ArrayRef<toolchain::Value*> indices,
                                          bool inbounds,
                                          bool signedIndices,
                                          SourceLocation loc,
                                    const toolchain::Twine &name = "arrayidx") {
  if (inbounds) {
...
static Address emitArraySubscriptGEP(CodeGenFunction &CGF, Address addr,
                                     ArrayRef<toolchain::Value *> indices,
                                     toolchain::Type *elementType, bool inbounds,
...
static CharUnits getArrayElementAlign(CharUnits arrayAlign,
  ...
  if (auto constantIdx = dyn_cast<toolchain::ConstantInt>(idx)) {
...
// Wraps Addr with a call to toolchain.preserve.static.offset intrinsic.
static Address wrapWithBPFPreserveStaticOffset(CodeGenFunction &CGF,
  ...
  toolchain::Function *Fn =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::preserve_static_offset);
  toolchain::CallInst *Call = CGF.Builder.CreateCall(Fn, {Addr.emitRawPointer(CGF)});
...
static Address emitArraySubscriptGEP(CodeGenFunction &CGF, Address addr,
  ...
  for (auto *idx : indices.drop_back())
    assert(isa<toolchain::ConstantInt>(idx) &&
           cast<toolchain::ConstantInt>(idx)->isZero());
#endif
  ...
  toolchain::Value *eltPtr;
  auto LastIndex = dyn_cast<toolchain::ConstantInt>(indices.back());
  if (!LastIndex ||
    ...
    toolchain::DIType *DbgInfo = nullptr;
    if (arrayType)
...
void CodeGenFunction::EmitCountedByBoundsChecking(
    const Expr *E, toolchain::Value *Idx, Address Addr, QualType IdxTy,
  ...
  if (std::optional<int64_t> Diff =
    ...
    toolchain::Type *CountTy = ConvertType(CountFD->getType());
    toolchain::Value *Res =
...
LValue CodeGenFunction::EmitArraySubscriptExpr(const ArraySubscriptExpr *E,
  ...
  toolchain::Value *IdxPre =
      (E->getLHS() == E->getIdx()) ? EmitScalarExpr(E->getIdx()) : nullptr;
  bool SignedIndices = false;
  auto EmitIdxAfterBase = [&, IdxPre](bool Promote) -> toolchain::Value * {
    auto *Idx = IdxPre;
  ...
  if (const VariableArrayType *vla =
    ...
    toolchain::Value *numElements = getVLASize(vla).NumElts;
    ...
    toolchain::Value *InterfaceSizeVal =
        toolchain::ConstantInt::get(Idx->getType(), InterfaceSize.getQuantity());

    toolchain::Value *ScaledIdx = Builder.CreateMul(Idx, InterfaceSizeVal);

    // We don't necessarily build correct LLVM struct types for ObjC
    // interfaces, so we can't rely on GEP to do this scaling
    // correctly, so we need to cast to i8*.  FIXME: is this actually
    // true?  A lot of other things in the fragile ABI would break...
    toolchain::Type *OrigBaseElemTy = Addr.getElementType();
...
toolchain::Value *CodeGenFunction::EmitMatrixIndexExpr(const Expr *E) {
  toolchain::Value *Idx = EmitScalarExpr(E);
...
LValue CodeGenFunction::EmitMatrixSubscriptExpr(const MatrixSubscriptExpr *E) {
  ...
  toolchain::Value *RowIdx = EmitMatrixIndexExpr(E->getRowIdx());
  toolchain::Value *ColIdx = EmitMatrixIndexExpr(E->getColumnIdx());

  toolchain::Value *NumRows = Builder.getIntN(
      RowIdx->getType()->getScalarSizeInBits(),
      E->getBase()->getType()->castAs<ConstantMatrixType>()->getNumRows());
  toolchain::Value *FinalIdx =
...
static Address emitOMPArraySectionBase(CodeGenFunction &CGF, const Expr *Base,
  ...
  if (auto *ASE = dyn_cast<ArraySectionExpr>(Base->IgnoreParenImpCasts())) {
    ...
    if (BaseTy->isArrayType()) {
      ...
      // If the array type was an incomplete type, we need to make sure
      // the decay ends up being the right type.
      toolchain::Type *NewTy = CGF.ConvertType(BaseTy);
      Addr = Addr.withElementType(NewTy);
...
LValue CodeGenFunction::EmitArraySectionExpr(const ArraySectionExpr *E,
  ...
  if (IsLowerBound || E->getColonLocFirst().isInvalid()) {
    ...
    if (auto *LowerBound = E->getLowerBound()) {
      ...
      Idx = toolchain::ConstantInt::getNullValue(IntPtrTy);
  } else {
    ...
    toolchain::APSInt ConstLength;
    if (Length) {
      // Idx = LowerBound + Length - 1;
      if (std::optional<toolchain::APSInt> CL = Length->getIntegerConstantExpr(C)) {
        ConstLength = CL->zextOrTrunc(PointerWidthInBits);
        Length = nullptr;
      }
      auto *LowerBound = E->getLowerBound();
      toolchain::APSInt ConstLowerBound(PointerWidthInBits, /*isUnsigned=*/false);
      if (LowerBound) {
        if (std::optional<toolchain::APSInt> LB =
        ...
        auto *LowerBoundVal =
            LowerBound
                ? Builder.CreateIntCast(
                      EmitScalarExpr(LowerBound), IntPtrTy,
                      LowerBound->getType()->hasSignedIntegerRepresentation())
                : toolchain::ConstantInt::get(IntPtrTy, ConstLowerBound);
        auto *LengthVal =
            Length
                ? Builder.CreateIntCast(
                      EmitScalarExpr(Length), IntPtrTy,
                      Length->getType()->hasSignedIntegerRepresentation())
                : toolchain::ConstantInt::get(IntPtrTy, ConstLength);
        Idx = Builder.CreateAdd(LowerBoundVal, LengthVal, "lb_add_len",
                                /*HasNUW=*/false,
                                !getLangOpts().PointerOverflowDefined);
        if (Length && LowerBound) {
          Idx = Builder.CreateSub(
              Idx, toolchain::ConstantInt::get(IntPtrTy, /*V=*/1), "idx_sub_1",
              /*HasNUW=*/false, !getLangOpts().PointerOverflowDefined);
        }
      } else
        Idx = toolchain::ConstantInt::get(IntPtrTy, ConstLength + ConstLowerBound);
    } else {
      // Idx = ArraySize - 1;
      ...
      if (auto *VAT = C.getAsVariableArrayType(ArrayTy)) {
        ...
        if (std::optional<toolchain::APSInt> L = Length->getIntegerConstantExpr(C)) {
          ConstLength = *L;
          Length = nullptr;
        }
      } else {
      ...
      if (Length) {
        auto *LengthVal = Builder.CreateIntCast(
            ...
            Length->getType()->hasSignedIntegerRepresentation());
        Idx = Builder.CreateSub(
            LengthVal, toolchain::ConstantInt::get(IntPtrTy, /*V=*/1), "len_sub_1",
            /*HasNUW=*/false, !getLangOpts().PointerOverflowDefined);
      } else {
        ConstLength = ConstLength.zextOrTrunc(PointerWidthInBits);
        --ConstLength;
        Idx = toolchain::ConstantInt::get(IntPtrTy, ConstLength);
      }
    }
  }
  assert(Idx);
...
EmitExtVectorElementExpr(const ExtVectorElementExpr *E) {
  ...
  if (Base.isSimple()) {
    toolchain::Constant *CV =
        toolchain::ConstantDataVector::get(getLLVMContext(), Indices);
    return LValue::MakeExtVectorElt(Base.getAddress(), CV, type,
                                    Base.getBaseInfo(), TBAAAccessInfo());
  }
  assert(Base.isExtVectorElt() && "Can only subscript lvalue vec elts here!");

  toolchain::Constant *BaseElts = Base.getExtVectorElts();
  SmallVector<toolchain::Constant *, 4> CElts;

  for (unsigned Index : Indices)
    CElts.push_back(BaseElts->getAggregateElement(Index));
  toolchain::Constant *CV = toolchain::ConstantVector::get(CElts);
...
LValue CodeGenFunction::EmitMemberExpr(const MemberExpr *E) {
  ...
  toolchain_unreachable("Unhandled member declaration!");
}
...
static Address emitPreserveStructAccess(CodeGenFunction &CGF, LValue base,
  ...
  const RecordDecl *rec = field->getParent();
  toolchain::DIType *DbgInfo = CGF.getDebugInfo()->getOrCreateStandaloneType(
...
LValue CodeGenFunction::EmitLValueForField(LValue base, const FieldDecl *field,
  ...
  if (field->isBitField()) {
    ...
    if (!UseVolatile) {
      if (!IsInPreservedAIRegion &&
        ...
        toolchain::DIType *DbgInfo = getDebugInfo()->getOrCreateRecordType(
    ...
    const unsigned SS =
        UseVolatile ? Info.VolatileStorageSize : Info.StorageSize;
    // Get the access type.
    toolchain::Type *FieldIntTy = toolchain::Type::getIntNTy(getLLVMContext(), SS);
  ...
  if (rec->isUnion()) {
    ...
    if (IsInPreservedAIRegion ||
      ...
      toolchain::DIType *DbgInfo = getDebugInfo()->getOrCreateStandaloneType(base.getType(),
          rec->getLocation());
      addr =
...
CodeGenFunction::EmitLValueForFieldInitialization(LValue Base,
  ...
  toolchain::Type *toolchainType = ConvertTypeForMem(FieldType);
  V = V.withElementType(toolchainType);
  ...
  if (CGF.ConstantFoldsToSimpleInteger(condExpr, CondExprBool)) {
    ...
    if (!CGF.ContainsLabel(Dead)) {
      ...
      if (auto *ThrowExpr = dyn_cast<CXXThrowExpr>(Live->IgnoreParens())) {
        ...
        toolchain::Type *ElemTy = CGF.ConvertType(Dead->getType());
        toolchain::Type *Ty = CGF.UnqualPtrTy;
        return CGF.MakeAddrLValue(
            Address(toolchain::UndefValue::get(Ty), ElemTy, CharUnits::One()),
            Dead->getType());
      }
...
struct ConditionalInfo {
  toolchain::BasicBlock *lhsBlock, *rhsBlock;
...
ConditionalInfo EmitConditionalBlocks(CodeGenFunction &CGF,
  ...
  ConditionalInfo Info{CGF.createBasicBlock("cond.true"),
                       ...
                       std::nullopt};
  toolchain::BasicBlock *endBlock = CGF.createBasicBlock("cond.end");
...
LValue CodeGenFunction::EmitCastLValue(const CastExpr *E) {
  ...
  case CK_Dependent:
    toolchain_unreachable("dependent cast kind in IR gen!");

  case CK_BuiltinFnToFnPtr:
    toolchain_unreachable("builtin functions are handled elsewhere");
  ...
  case CK_NoOp: {
    ...
    if (LV.isSimple()) {
      Address V = LV.getAddress();
      if (V.isValid()) {
        toolchain::Type *T = ConvertTypeForMem(E->getType());
  ...
  case CK_AddressSpaceConversion: {
    ...
    toolchain::Value *V = getTargetHooks().performAddrSpaceCast(
  ...
  case CK_ZeroToOCLOpaqueType:
    toolchain_unreachable("NULL to OpenCL opaque type lvalue cast is not valid");
  ...
  toolchain_unreachable("Unhandled lvalue cast kind?");
}
...
LValue CodeGenFunction::EmitHLSLOutArgExpr(const HLSLOutArgExpr *E,
  ...
  toolchain::Value *Addr = TempLV.getAddress().getBasePointer();
  toolchain::Type *ElTy = ConvertTypeForMem(TempLV.getType());
...
CodeGenFunction::getOrCreateOpaqueLValueMapping(const OpaqueValueExpr *e) {
  ...
  toolchain::DenseMap<const OpaqueValueExpr*,LValue>::iterator
...
CodeGenFunction::getOrCreateOpaqueRValueMapping(const OpaqueValueExpr *e) {
  ...
  toolchain::DenseMap<const OpaqueValueExpr*,RValue>::iterator
...
RValue CodeGenFunction::EmitCallExpr(const CallExpr *E,
  ...
  toolchain::CallBase *CallOrInvokeStorage;
  if (!CallOrInvoke) {
    CallOrInvoke = &CallOrInvokeStorage;
  }

  auto AddCoroElideSafeOnExit = toolchain::make_scope_exit([&] {
    if (E->isCoroElideSafe()) {
      auto *I = *CallOrInvoke;
      if (I)
        I->addFnAttr(toolchain::Attribute::CoroElideSafe);
    }
  });
...
static CGCallee EmitDirectCallee(CodeGenFunction &CGF, GlobalDecl GD) {
  ...
  if (auto builtinID = FD->getBuiltinID()) {
    ...
    if (CGF.CurFn->getName() != FDInlineName &&
      ...
      toolchain::Constant *CalleePtr = CGF.CGM.getRawFunctionPointer(GD);
      toolchain::Function *Fn = toolchain::cast<toolchain::Function>(CalleePtr);
      toolchain::Module *M = Fn->getParent();
      toolchain::Function *Clone = M->getFunction(FDInlineName);
      if (!Clone) {
        Clone = toolchain::Function::Create(Fn->getFunctionType(),
                                       toolchain::GlobalValue::InternalLinkage,
                                       Fn->getAddressSpace(), FDInlineName, M);
        Clone->addFnAttr(toolchain::Attribute::AlwaysInline);
      }
  ...
  if (CGF.CGM.getLangOpts().CUDA && !CGF.CGM.getLangOpts().CUDAIsDevice &&
    ...
    CalleePtr = CGF.CGM.getCUDARuntime().getKernelStub(
        cast<toolchain::GlobalValue>(CalleePtr->stripPointerCasts()));
  ...
  if (auto ICE = dyn_cast<ImplicitCastExpr>(E)) {
    ...
    if (ICE->getCastKind() == CK_LValueToRValue) {
      ...
      if (const auto *PtrType = SubExpr->getType()->getAs<PointerType>()) {
        std::pair<toolchain::Value *, CGPointerAuthInfo> Result =
...
LValue CodeGenFunction::EmitBinaryOperatorLValue(const BinaryOperator *E) {
  ...
  case TEK_Scalar: {
    ...
    RValue RV;
    toolchain::Value *Previous = nullptr;
    ...
    if (LV.isBitField()) {
      ...
      toolchain::Value *Src = Previous ? Previous : RV.getScalarVal();
...
toolchain::Value *CodeGenFunction::EmitIvarOffset(const ObjCInterfaceDecl *Interface,
...
CodeGenFunction::EmitIvarOffsetAsPointerDiff(const ObjCInterfaceDecl *Interface,
                                             const ObjCIvarDecl *Ivar) {
  toolchain::Value *OffsetValue = EmitIvarOffset(Interface, Ivar);
...
LValue CodeGenFunction::EmitObjCIvarRefLValue(const ObjCIvarRefExpr *E) {
  ...
  toolchain::Value *BaseValue = nullptr;
...
RValue CodeGenFunction::EmitCall(QualType CalleeType,
  ...
  if (SanOpts.has(SanitizerKind::Function) &&
    ...
    if (toolchain::Constant *PrefixSig =
      ...
      toolchain::Type *PrefixSigType = PrefixSig->getType();
      toolchain::StructType *PrefixStructTy = toolchain::StructType::get(
          CGM.getLLVMContext(), {PrefixSigType, Int32Ty}, /*isPacked=*/true);

      toolchain::Value *CalleePtr = Callee.getFunctionPointer();
      ...
      toolchain::Value *AlignedCalleePtr;
      if (CGM.getTriple().isARM() || CGM.getTriple().isThumb()) {
        toolchain::Value *CalleeAddress =
            Builder.CreatePtrToInt(CalleePtr, IntPtrTy);
        toolchain::Value *Mask = toolchain::ConstantInt::get(IntPtrTy, ~1);
        toolchain::Value *AlignedCalleeAddress =
      ...
      toolchain::Value *CalleePrefixStruct = AlignedCalleePtr;
      toolchain::Value *CalleeSigPtr =
          Builder.CreateConstGEP2_32(PrefixStructTy, CalleePrefixStruct, -1, 0);
      toolchain::Value *CalleeSig =
          Builder.CreateAlignedLoad(PrefixSigType, CalleeSigPtr, getIntAlign());
      toolchain::Value *CalleeSigMatch = Builder.CreateICmpEQ(CalleeSig, PrefixSig);

      toolchain::BasicBlock *Cont = createBasicBlock("cont");
      toolchain::BasicBlock *TypeCheck = createBasicBlock("typecheck");
      Builder.CreateCondBr(CalleeSigMatch, TypeCheck, Cont);

      EmitBlock(TypeCheck);
      toolchain::Value *CalleeTypeHash = Builder.CreateAlignedLoad(
          Int32Ty,
          Builder.CreateConstGEP2_32(PrefixStructTy, CalleePrefixStruct, -1, 1),
          getPointerAlign());
      toolchain::Value *CalleeTypeHashMatch =
          Builder.CreateICmpEQ(CalleeTypeHash, TypeHash);
      toolchain::Constant *StaticData[] = {EmitCheckSourceLocation(E->getBeginLoc()),
  ...
  if (SanOpts.has(SanitizerKind::CFIICall) &&
    ...
    auto CheckHandler = SanitizerHandler::CFICheckFail;
    SanitizerDebugLocation SanScope(this, {CheckOrdinal}, CheckHandler);
    EmitSanitizerStatReport(toolchain::SanStat_CFI_ICall);
    ...
    toolchain::Value *TypeId = toolchain::MetadataAsValue::get(getLLVMContext(), MD);

    toolchain::Value *CalleePtr = Callee.getFunctionPointer();
    toolchain::Value *TypeTest = Builder.CreateCall(
        CGM.getIntrinsic(toolchain::Intrinsic::type_test), {CalleePtr, TypeId});

    auto CrossDsoTypeId = CGM.CreateCrossDsoCfiTypeId(MD);
    toolchain::Constant *StaticData[] = {
        toolchain::ConstantInt::get(Int8Ty, CFITCK_ICall),
    ...
    if (CGM.getCodeGenOpts().SanitizeCfiCrossDso && CrossDsoTypeId) {
      ...
      EmitCheck(std::make_pair(TypeTest, CheckOrdinal), CheckHandler,
                StaticData, {CalleePtr, toolchain::UndefValue::get(IntPtrTy)});
    }
  }
  ...
  if (CGM.getLangOpts().HIP && !CGM.getLangOpts().CUDAIsDevice &&
      ...
      (!TargetDecl || !isa<FunctionDecl>(TargetDecl))) {
    toolchain::Value *Handle = Callee.getFunctionPointer();
    auto *Stub = Builder.CreateLoad(
        Address(Handle, Handle->getType(), CGM.getPointerAlign()));
    Callee.setFunctionPointer(Stub);
  }
  toolchain::CallBase *LocalCallOrInvoke = nullptr;
...
EmitPointerToDataMemberBinaryExpr(const BinaryOperator *E) {
  ...
  toolchain::Value *OffsetV = EmitScalarExpr(E->getRHS());
...
void CodeGenFunction::SetFPAccuracy(toolchain::Value *Val, float Accuracy) {
  assert(Val->getType()->isFPOrFPVectorTy());
  if (Accuracy == 0.0 || !isa<toolchain::Instruction>(Val))
    return;

  toolchain::MDBuilder MDHelper(getLLVMContext());
  toolchain::MDNode *Node = MDHelper.createFPMath(Accuracy);

  cast<toolchain::Instruction>(Val)->setMetadata(toolchain::LLVMContext::MD_fpmath, Node);
}

void CodeGenFunction::SetSqrtFPAccuracy(toolchain::Value *Val) {
  toolchain::Type *EltTy = Val->getType()->getScalarType();
  if (!EltTy->isFloatTy())
    return;
...
void CodeGenFunction::SetDivFPAccuracy(toolchain::Value *Val) {
  toolchain::Type *EltTy = Val->getType()->getScalarType();
  if (!EltTy->isFloatTy())
    return;
...
void CodeGenFunction::FlattenAccessAndType(
  ...
  toolchain::SmallVector<std::pair<QualType, toolchain::SmallVector<toolchain::Value *, 4>>,
                    16>
      WorkList;
  toolchain::IntegerType *IdxTy = toolchain::IntegerType::get(getLLVMContext(), 32);
  // Addr should be a pointer so we need to 'dereference' it
  WorkList.push_back({AddrType, {toolchain::ConstantInt::get(IdxTy, 0)}});
  ...
  while (!WorkList.empty()) {
    ...
    if (const auto *CAT = dyn_cast<ConstantArrayType>(T)) {
      ...
      for (int64_t I = Size - 1; I > -1; I--) {
        toolchain::SmallVector<toolchain::Value *, 4> IdxListCopy = IdxList;
        IdxListCopy.push_back(toolchain::ConstantInt::get(IdxTy, I));
      ...
      toolchain::SmallVector<QualType, 16> FieldTypes;
      ...
      toolchain::Type *LLVMT = ConvertTypeForMem(T);
      CharUnits Align = getContext().getTypeAlignInChars(T);
      Address GEP =
          Builder.CreateInBoundsGEP(Addr, IdxList, LLVMT, Align, "vector.gep");
      for (unsigned I = 0, E = VT->getNumElements(); I < E; I++) {
        toolchain::Value *Idx = toolchain::ConstantInt::get(IdxTy, I);
