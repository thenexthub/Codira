...
#include "toolchain/IR/IntrinsicsWebAssembly.h"
#include "toolchain/Support/SaveAndRestore.h"

using namespace language::Core;
using namespace CodeGen;

static toolchain::FunctionCallee getFreeExceptionFn(CodeGenModule &CGM) {
  // void __cxa_free_exception(void *thrown_exception);

  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(CGM.VoidTy, CGM.Int8PtrTy, /*isVarArg=*/false);

  return CGM.CreateRuntimeFunction(FTy, "__cxa_free_exception");
}

static toolchain::FunctionCallee getSehTryBeginFn(CodeGenModule &CGM) {
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg=*/false);
  return CGM.CreateRuntimeFunction(FTy, "toolchain.seh.try.begin");
}

static toolchain::FunctionCallee getSehTryEndFn(CodeGenModule &CGM) {
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg=*/false);
  return CGM.CreateRuntimeFunction(FTy, "toolchain.seh.try.end");
}

static toolchain::FunctionCallee getUnexpectedFn(CodeGenModule &CGM) {
...
toolchain::FunctionCallee CodeGenModule::getTerminateFn() {
  // void __terminate();

  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(VoidTy, /*isVarArg=*/false);
...
static toolchain::FunctionCallee getCatchallRethrowFn(CodeGenModule &CGM,
                                                 StringRef Name) {
  toolchain::FunctionType *FTy =
    toolchain::FunctionType::get(CGM.VoidTy, CGM.Int8PtrTy, /*isVarArg=*/false);
...
static const EHPersonality &getCPersonality(const TargetInfo &Target,
  ...
  const toolchain::Triple &T = Target.getTriple();
...
static const EHPersonality &getSEHPersonalityMSVC(const toolchain::Triple &T) {
  if (T.getArch() == toolchain::Triple::x86)
...
const EHPersonality &EHPersonality::get(CodeGenModule &CGM,
  ...
  const toolchain::Triple &T = CGM.getTarget().getTriple();
...
static toolchain::FunctionCallee getPersonalityFn(CodeGenModule &CGM,
                                             const EHPersonality &Personality) {
  return CGM.CreateRuntimeFunction(toolchain::FunctionType::get(CGM.Int32Ty, true),
                                   Personality.PersonalityFn,
                                   toolchain::AttributeList(), /*Local=*/true);
}

static toolchain::Constant *getOpaquePersonalityFn(CodeGenModule &CGM,
                                        const EHPersonality &Personality) {
  toolchain::FunctionCallee Fn = getPersonalityFn(CGM, Personality);
  return cast<toolchain::Constant>(Fn.getCallee());
}

/// Check whether a landingpad instruction only uses C++ features.
static bool LandingPadHasOnlyCXXUses(toolchain::LandingPadInst *LPI) {
  for (unsigned I = 0, E = LPI->getNumClauses(); I != E; ++I) {
    // Look for something that would've been returned by the ObjC
    // runtime's GetEHType() method.
    toolchain::Value *Val = LPI->getClause(I)->stripPointerCasts();
    if (LPI->isCatch(I)) {
      // Check if the catch value has the ObjC prefix.
      if (toolchain::GlobalVariable *GV = dyn_cast<toolchain::GlobalVariable>(Val))
      ...
      toolchain::Constant *CVal = cast<toolchain::Constant>(Val);
      for (toolchain::User::op_iterator
              II = CVal->op_begin(), IE = CVal->op_end(); II != IE; ++II) {
        if (toolchain::GlobalVariable *GV =
            cast<toolchain::GlobalVariable>((*II)->stripPointerCasts()))
          // ObjC EH selector entries are always global variables with
          // names starting like this.
          if (GV->getName().starts_with("OBJC_EHTYPE"))
            return false;
...
static bool PersonalityHasOnlyCXXUses(toolchain::Constant *Fn) {
  for (toolchain::User *U : Fn->users()) {
    // Conditionally white-list bitcasts.
    if (toolchain::ConstantExpr *CE = dyn_cast<toolchain::ConstantExpr>(U)) {
      if (CE->getOpcode() != toolchain::Instruction::BitCast) return false;
    ...
    toolchain::Function *F = dyn_cast<toolchain::Function>(U);
    if (!F) return false;
...
void CodeGenModule::SimplifyPersonality() {
  ...
  toolchain::Function *Fn = getModule().getFunction(ObjCXX.PersonalityFn);
  ...
  // function.
  toolchain::FunctionCallee CXXFn = getPersonalityFn(*this, CXX);
...
static toolchain::Constant *getCatchAllValue(CodeGenFunction &CGF) {
  // Possibly we should use @toolchain.eh.catch.all.value here.
  return toolchain::ConstantPointerNull::get(CGF.Int8PtrTy);
}
...
namespace {
  ...
  struct FreeException final : EHScopeStack::Cleanup {
    toolchain::Value *exn;
    FreeException(toolchain::Value *exn) : exn(exn) {}
...
void CodeGenFunction::EmitAnyExprToExn(const Expr *e, Address addr) {
  ...
  DeactivateCleanupBlock(
      cleanup, cast<toolchain::Instruction>(typedAddr.emitRawPointer(*this)));
}
...
toolchain::Value *CodeGenFunction::getExceptionFromSlot() {
  return Builder.CreateLoad(getExceptionSlot(), "exn");
}

toolchain::Value *CodeGenFunction::getSelectorFromSlot() {
...
void CodeGenFunction::EmitStartEHSpec(const Decl *D) {
  ...
  if (EST == EST_Dynamic ||
    ...
    for (unsigned I = 0; I != NumExceptions; ++I) {
      ...
      toolchain::Value *EHType = CGM.GetAddrOfRTTIDescriptor(ExceptType,
                                                        /*ForEH=*/true);
...
static void emitFilterDispatchBlock(CodeGenFunction &CGF,
  ...
  toolchain::BasicBlock *dispatchBlock = filterScope.getCachedEHDispatchBlock();
  ...
  if (filterScope.getNumFilters()) {
    ...
    toolchain::Value *selector = CGF.getSelectorFromSlot();
    toolchain::BasicBlock *unexpectedBB = CGF.createBasicBlock("ehspec.unexpected");

    toolchain::Value *zero = CGF.Builder.getInt32(0);
    toolchain::Value *failsFilter =
...
void CodeGenFunction::EnterCXXTryStmt(const CXXTryStmt &S, bool IsFnTryBlock) {
  ...
  for (unsigned I = 0; I != NumHandlers; ++I) {
    ...
    toolchain::BasicBlock *Handler = createBasicBlock("catch");
...
CodeGenFunction::getEHDispatchBlock(EHScopeStack::stable_iterator si) {
  ...
  toolchain::BasicBlock *dispatchBlock = scope.getCachedEHDispatchBlock();
  if (!dispatchBlock) {
...
CodeGenFunction::getFuncletEHDispatchBlock(EHScopeStack::stable_iterator SI) {
  ...
  toolchain::BasicBlock *DispatchBlock = EHS.getCachedEHDispatchBlock();
  if (DispatchBlock)
    return DispatchBlock;
  ...
  case EHScope::Filter:
    toolchain_unreachable("exception specifications not handled yet!");
...
static bool isNonEHScope(const EHScope &S) {
  ...
  toolchain_unreachable("Invalid EHScope Kind!");
}

toolchain::BasicBlock *CodeGenFunction::getInvokeDestImpl() {
  ...
  toolchain::BasicBlock *LP = EHStack.begin()->getCachedLandingPad();
  if (LP) return LP;
...
toolchain::BasicBlock *CodeGenFunction::EmitLandingPad() {
  assert(EHStack.requiresLandingPad());
  ...
  case EHScope::Cleanup:
  case EHScope::Filter:
    if (toolchain::BasicBlock *lpad = innermostEHScope.getCachedLandingPad())
      return lpad;
  }
  ...
  toolchain::BasicBlock *lpad = createBasicBlock("lpad");
  EmitBlock(lpad);

  toolchain::LandingPadInst *LPadInst =
      Builder.CreateLandingPad(toolchain::StructType::get(Int8PtrTy, Int32Ty), 0);

  toolchain::Value *LPadExn = Builder.CreateExtractValue(LPadInst, 0);
  Builder.CreateStore(LPadExn, getExceptionSlot());
  toolchain::Value *LPadSel = Builder.CreateExtractValue(LPadInst, 1);
  ...
  SmallVector<toolchain::Value*, 4> filterTypes;
  toolchain::SmallPtrSet<toolchain::Value*, 4> catchTypes;
  ...
  // right place in the landingpad, which is to say, at the end.
    ...
    // doesn't match.
    SmallVector<toolchain::Constant*, 8> Filters;
    toolchain::ArrayType *AType =
      toolchain::ArrayType::get(!filterTypes.empty() ?
                             filterTypes[0]->getType() : Int8PtrTy,
                           filterTypes.size());

    for (toolchain::Value *filterType : filterTypes)
      Filters.push_back(cast<toolchain::Constant>(filterType));
    toolchain::Constant *FilterArray = toolchain::ConstantArray::get(AType, Filters);
...
static void emitCatchPadBlock(CodeGenFunction &CGF, EHCatchScope &CatchScope) {
  toolchain::BasicBlock *DispatchBlock = CatchScope.getCachedEHDispatchBlock();
  assert(DispatchBlock);

  CGBuilderTy::InsertPoint SavedIP = CGF.Builder.saveIP();
  CGF.EmitBlockAfterUses(DispatchBlock);

  toolchain::Value *ParentPad = CGF.CurrentFuncletPad;
  if (!ParentPad)
    ParentPad = toolchain::ConstantTokenNone::get(CGF.getLLVMContext());
  toolchain::BasicBlock *UnwindBB =
      CGF.getEHDispatchBlock(CatchScope.getEnclosingEHScope());

  unsigned NumHandlers = CatchScope.getNumHandlers();
  toolchain::CatchSwitchInst *CatchSwitch =
  ...
  for (unsigned I = 0; I < NumHandlers; ++I) {
    ...
    if (!TypeInfo.RTTI)
      TypeInfo.RTTI = toolchain::Constant::getNullValue(CGF.VoidPtrTy);
    ...
    if (EHPersonality::get(CGF).isMSVCXXPersonality()) {
      CGF.Builder.CreateCatchPad(
          CatchSwitch, {TypeInfo.RTTI, CGF.Builder.getInt32(TypeInfo.Flags),
                        toolchain::Constant::getNullValue(CGF.VoidPtrTy)});
    } else {
...
static void emitWasmCatchPadBlock(CodeGenFunction &CGF,
  ...
  toolchain::BasicBlock *WasmCatchStartBlock = CGF.createBasicBlock("catch.start");
  CatchSwitch->addHandler(WasmCatchStartBlock);
  CGF.EmitBlockAfterUses(WasmCatchStartBlock);

  // Create a catchpad instruction.
  SmallVector<toolchain::Value *, 4> CatchTypes;
  ...
  toolchain::Function *GetExnFn =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::wasm_get_exception);
  toolchain::Function *GetSelectorFn =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::wasm_get_ehselector);
  toolchain::CallInst *Exn = CGF.Builder.CreateCall(GetExnFn, CPI);
  CGF.Builder.CreateStore(Exn, CGF.getExceptionSlot());
  toolchain::CallInst *Selector = CGF.Builder.CreateCall(GetSelectorFn, CPI);

  toolchain::Function *TypeIDFn =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::eh_typeid_for, {CGF.VoidPtrTy});
  ...
  for (unsigned I = 0, E = NumHandlers;; ++I) {
    ...
    toolchain::BasicBlock *NextBlock;
    ...
    toolchain::CallInst *TypeIndex = CGF.Builder.CreateCall(TypeIDFn, TypeInfo.RTTI);
    TypeIndex->setDoesNotThrow();

    toolchain::Value *MatchesTypeIndex =
...
static void emitCatchDispatchBlock(CodeGenFunction &CGF,
  ...
  toolchain::Function *toolchain_eh_typeid_for =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::eh_typeid_for, {CGF.VoidPtrTy});
  toolchain::Type *argTy = toolchain_eh_typeid_for->getArg(0)->getType();
  ...
  for (unsigned i = 0, e = catchScope.getNumHandlers(); ; ++i) {
    ...
    toolchain::Value *typeValue = handler.Type.RTTI;
    assert(handler.Type.Flags == 0 &&
           "landingpads do not support catch handler flags");
    ...
    // Figure out the catch type's index in the LSDA's type table.
    toolchain::CallInst *typeIndex =
      CGF.Builder.CreateCall(toolchain_eh_typeid_for, typeValue);
    typeIndex->setDoesNotThrow();
...
void CodeGenFunction::ExitCXXTryStmt(const CXXTryStmt &S, bool IsFnTryBlock) {
  ...
  toolchain::BasicBlock *ContBB = createBasicBlock("try.cont");
  ...
  toolchain::BasicBlock *WasmCatchStartBlock = nullptr;
  if (EHPersonality::get(*this).isWasmPersonality()) {
    auto *CatchSwitch =
        cast<toolchain::CatchSwitchInst>(DispatchBlock->getFirstNonPHIIt());
    WasmCatchStartBlock = CatchSwitch->hasUnwindDest()
                              ? CatchSwitch->getSuccessor(1)
                              : CatchSwitch->getSuccessor(0);
    auto *CPI =
        cast<toolchain::CatchPadInst>(WasmCatchStartBlock->getFirstNonPHIIt());
  ...
  for (unsigned I = NumHandlers; I != 0; --I) {
    ...
    toolchain::BasicBlock *CatchBlock = Handlers[I-1].Block;
  ...
  if (EHPersonality::get(*this).isWasmPersonality() && !HasCatchAll) {
    ...
    // the rethrow block.
    toolchain::BasicBlock *RethrowBlock = WasmCatchStartBlock;
    while (toolchain::Instruction *TI = RethrowBlock->getTerminator()) {
      auto *BI = cast<toolchain::BranchInst>(TI);
      assert(BI->isConditional());
      RethrowBlock = BI->getSuccessor(1);
    }
    assert(RethrowBlock != WasmCatchStartBlock && RethrowBlock->empty());
    Builder.SetInsertPoint(RethrowBlock);
    toolchain::Function *RethrowInCatchFn =
        CGM.getIntrinsic(toolchain::Intrinsic::wasm_rethrow);
...
namespace {
  struct CallEndCatchForFinally final : EHScopeStack::Cleanup {
    toolchain::Value *ForEHVar;
    toolchain::FunctionCallee EndCatchFn;
    CallEndCatchForFinally(toolchain::Value *ForEHVar,
                           toolchain::FunctionCallee EndCatchFn)
        : ForEHVar(ForEHVar), EndCatchFn(EndCatchFn) {}

    void Emit(CodeGenFunction &CGF, Flags flags) override {
      toolchain::BasicBlock *EndCatchBB = CGF.createBasicBlock("finally.endcatch");
      toolchain::BasicBlock *CleanupContBB =
        CGF.createBasicBlock("finally.cleanup.cont");

      toolchain::Value *ShouldEndCatch =
  ...
  struct PerformFinally final : EHScopeStack::Cleanup {
    ...
    toolchain::Value *ForEHVar;
    toolchain::FunctionCallee EndCatchFn;
    toolchain::FunctionCallee RethrowFn;
    toolchain::Value *SavedExnVar;

    PerformFinally(const Stmt *Body, toolchain::Value *ForEHVar,
                   toolchain::FunctionCallee EndCatchFn,
                   toolchain::FunctionCallee RethrowFn, toolchain::Value *SavedExnVar)
    ...
    void Emit(CodeGenFunction &CGF, Flags flags) override {
      ...
      toolchain::Value *SavedCleanupDest =
      ...
      if (CGF.HaveInsertPoint()) {
        toolchain::BasicBlock *RethrowBB = CGF.createBasicBlock("finally.rethrow");
        toolchain::BasicBlock *ContBB = CGF.createBasicBlock("finally.cont");

        toolchain::Value *ShouldRethrow =
...
void CodeGenFunction::FinallyInfo::enter(CodeGenFunction &CGF, const Stmt *body,
  ...
  toolchain::FunctionType *rethrowFnTy = rethrowFn.getFunctionType();
  SavedExnVar = nullptr;
  ...
  toolchain::BasicBlock *catchBB = CGF.createBasicBlock("finally.catchall");
...
void CodeGenFunction::FinallyInfo::exit(CodeGenFunction &CGF) {
  ...
  toolchain::BasicBlock *catchBB = catchScope.getHandler(0).Block;
...
toolchain::BasicBlock *CodeGenFunction::getTerminateLandingPad() {
  ...
  toolchain::CallInst *terminateCall =
...
toolchain::BasicBlock *CodeGenFunction::getTerminateHandler() {
  if (TerminateHandler)
...
toolchain::BasicBlock *CodeGenFunction::getTerminateFunclet() {
  assert(EHPersonality::get(*this).usesFuncletPads() &&
         "use getTerminateLandingPad for non-funclet EH");

  toolchain::BasicBlock *&TerminateFunclet = TerminateFunclets[CurrentFuncletPad];
  if (TerminateFunclet)
  ...
  SaveAndRestore RestoreCurrentFuncletPad(CurrentFuncletPad);
  toolchain::Value *ParentPad = CurrentFuncletPad;
  if (!ParentPad)
    ParentPad = toolchain::ConstantTokenNone::get(CGM.getLLVMContext());
...
toolchain::BasicBlock *CodeGenFunction::getEHResumeBlock(bool isCleanup) {
  ...
  toolchain::Value *Exn = getExceptionFromSlot();
  toolchain::Value *Sel = getSelectorFromSlot();

  toolchain::Type *LPadType = toolchain::StructType::get(Exn->getType(), Sel->getType());
  toolchain::Value *LPadVal = toolchain::PoisonValue::get(LPadType);
...
void CodeGenFunction::EmitSEHTryStmt(const SEHTryStmt &S) {
  EnterSEHTryStmt(S);
  {
    ...
    toolchain::BasicBlock *TryBB = nullptr;
    ...
    if (TryBB) {
      toolchain::SmallPtrSet<toolchain::BasicBlock *, 10> Visited;
...
void CodeGenFunction::VolatilizeTryBlocks(
    toolchain::BasicBlock *BB, toolchain::SmallPtrSet<toolchain::BasicBlock *, 10> &V) {
  ...
  if (!BB->isEHPad()) {
    for (toolchain::BasicBlock::iterator J = BB->begin(), JE = BB->end(); J != JE;
         ++J) {
      if (auto LI = dyn_cast<toolchain::LoadInst>(J)) {
        LI->setVolatile(true);
      } else if (auto SI = dyn_cast<toolchain::StoreInst>(J)) {
        SI->setVolatile(true);
      } else if (auto* MCI = dyn_cast<toolchain::MemIntrinsic>(J)) {
        MCI->setVolatile(toolchain::ConstantInt::get(Builder.getInt1Ty(), 1));
      }
    }
  }
  const toolchain::Instruction *TI = BB->getTerminator();
  if (TI) {
...
struct PerformSEHFinally final : EHScopeStack::Cleanup {
  toolchain::Function *OutlinedFinally;
  PerformSEHFinally(toolchain::Function *OutlinedFinally)
  ...
  void Emit(CodeGenFunction &CGF, Flags F) override {
    ...
    toolchain::Value *FP = nullptr;
    // If CFG.IsOutlinedSEHHelper is true, then we are within a finally block.
    if (CGF.IsOutlinedSEHHelper) {
      FP = &CGF.CurFn->arg_begin()[1];
    } else {
      toolchain::Function *LocalAddrFn =
          CGM.getIntrinsic(toolchain::Intrinsic::localaddress);
      FP = CGF.Builder.CreateCall(LocalAddrFn);
    }

    toolchain::Value *IsForEH =
        toolchain::ConstantInt::get(CGF.ConvertType(ArgTys[0]), F.isForEHCleanup());
    ...
    if (!F.isForEHCleanup() && F.hasExitSwitch()) {
      ...
      toolchain::Value *Load = CGF.Builder.CreateLoad(Addr, "cleanup.dest");
      toolchain::Value *Zero = toolchain::Constant::getNullValue(CGM.Int32Ty);
...
struct CaptureFinder : ConstStmtVisitor<CaptureFinder> {
  ...
  toolchain::SmallSetVector<const VarDecl *, 4> Captures;
  ...
  void VisitCallExpr(const CallExpr *E) {
    ...
    if (ParentCGF.getTarget().getTriple().getArch() != toolchain::Triple::x86)
      return;
...
Address CodeGenFunction::recoverAddrOfEscapedLocal(CodeGenFunction &ParentCGF,
                                                   Address ParentVar,
                                                   toolchain::Value *ParentFP) {
  toolchain::CallInst *RecoverCall = nullptr;
  CGBuilderTy Builder(*this, AllocaInsertPt);
  if (auto *ParentAlloca =
          dyn_cast_or_null<toolchain::AllocaInst>(ParentVar.getBasePointer())) {
    // Mark the variable escaped if nobody else referenced it and compute the
    // localescape index.
    auto InsertPair = ParentCGF.EscapedLocals.insert(
        std::make_pair(ParentAlloca, ParentCGF.EscapedLocals.size()));
    int FrameEscapeIdx = InsertPair.first->second;
    // call ptr @toolchain.localrecover(ptr @parentFn, ptr %fp, i32 N)
    toolchain::Function *FrameRecoverFn = toolchain::Intrinsic::getOrInsertDeclaration(
        &CGM.getModule(), toolchain::Intrinsic::localrecover);
    RecoverCall = Builder.CreateCall(
        FrameRecoverFn, {ParentCGF.CurFn, ParentFP,
                         toolchain::ConstantInt::get(Int32Ty, FrameEscapeIdx)});

  } else {
    // If the parent didn't have an alloca, we're doing some nested outlining.
    // Just clone the existing localrecover call, but tweak the FP argument to
    // use our FP value. All other arguments are constants.
    auto *ParentRecover = cast<toolchain::IntrinsicInst>(
        ParentVar.emitRawPointer(*this)->stripPointerCasts());
    assert(ParentRecover->getIntrinsicID() == toolchain::Intrinsic::localrecover &&
           "expected alloca or localrecover in parent LocalDeclMap");
    RecoverCall = cast<toolchain::CallInst>(ParentRecover->clone());
    RecoverCall->setArgOperand(1, ParentFP);
    RecoverCall->insertBefore(AllocaInsertPt->getIterator());
  }

  // Bitcast the variable, rename it, and insert it in the local decl map.
  toolchain::Value *ChildVar =
...
void CodeGenFunction::EmitCapturedLocals(CodeGenFunction &ParentCGF,
  ...
  if (!Finder.foundCaptures() &&
      CGM.getTarget().getTriple().getArch() != toolchain::Triple::x86) {
    if (IsFilter)
      EmitSEHExceptionCodeSave(ParentCGF, nullptr, nullptr);
    return;
  }

  toolchain::Value *EntryFP = nullptr;
  CGBuilderTy Builder(CGM, AllocaInsertPt);
  if (IsFilter && CGM.getTarget().getTriple().getArch() == toolchain::Triple::x86) {
    // 32-bit SEH filters need to be careful about FP recovery.  The end of the
    // EH registration is passed in as the EBP physical register.  We can
    // recover that with toolchain.frameaddress(1).
    EntryFP = Builder.CreateCall(
        CGM.getIntrinsic(toolchain::Intrinsic::frameaddress, AllocaInt8PtrTy),
        {Builder.getInt32(1)});
  } else {
  ...
  toolchain::Value *ParentFP = EntryFP;
  if (IsFilter) {
    ...
    toolchain::Function *RecoverFPIntrin =
        CGM.getIntrinsic(toolchain::Intrinsic::eh_recoverfp);
    ...
    if (ParentCGF.ParentCGF != nullptr) {
      ...
      toolchain::AllocaInst *FramePtrAddrAlloca = nullptr;
      for (auto &I : ParentCGF.LocalDeclMap) {
        ...
        if (isa<ImplicitParamDecl>(D) &&
          ...
          FramePtrAddrAlloca =
              cast<toolchain::AllocaInst>(I.second.getBasePointer());
          break;
        }
      }
      ...
      // %0 = call ptr @toolchain.eh.recoverfp(@"?fin$0@0@main@@",..)
      // %1 = call ptr @toolchain.localrecover(@"?fin$0@0@main@@",..)
  ...
  // Create toolchain.localrecover calls for all captures.
...
void CodeGenFunction::startOutlinedSEHHelper(CodeGenFunction &ParentCGF,
  ...
  if (CGM.getTarget().getTriple().getArch() != toolchain::Triple::x86 || !IsFilter) {
  ...
  toolchain::Function *Fn = toolchain::Function::Create(
      FnTy, toolchain::GlobalValue::InternalLinkage, Name.str(), &CGM.getModule());
...
CodeGenFunction::GenerateSEHFilterFunction(CodeGenFunction &ParentCGF,
  ...
  toolchain::Value *R = EmitScalarExpr(FilterExpr);
...
void CodeGenFunction::EmitSEHExceptionCodeSave(CodeGenFunction &ParentCGF,
  ...
  toolchain::Type *RecordTy = toolchain::PointerType::getUnqual(getLLVMContext());
  toolchain::Type *PtrsTy = toolchain::StructType::get(RecordTy, CGM.VoidPtrTy);
  toolchain::Value *Rec = Builder.CreateStructGEP(PtrsTy, SEHInfo, 0);
  Rec = Builder.CreateAlignedLoad(RecordTy, Rec, getPointerAlign());
  toolchain::Value *Code = Builder.CreateAlignedLoad(Int32Ty, Rec, getIntAlign());
  assert(!SEHCodeSlotStack.empty() && "emitting EH code outside of __except");
  Builder.CreateStore(Code, SEHCodeSlotStack.back());
}

toolchain::Value *CodeGenFunction::EmitSEHExceptionInfo() {
  // Sema should diagnose calling this builtin outside of a filter context, but
  // don't crash if we screw up.
  if (!SEHInfo)
    return toolchain::PoisonValue::get(Int8PtrTy);
  assert(SEHInfo->getType() == Int8PtrTy);
  return SEHInfo;
}

toolchain::Value *CodeGenFunction::EmitSEHExceptionCode() {
  assert(!SEHCodeSlotStack.empty() && "emitting EH code outside of __except");
  return Builder.CreateLoad(SEHCodeSlotStack.back());
}

toolchain::Value *CodeGenFunction::EmitSEHAbnormalTermination() {
...
void CodeGenFunction::pushSEHCleanup(CleanupKind Kind,
                                     toolchain::Function *FinallyFunc) {
...
void CodeGenFunction::EnterSEHTryStmt(const SEHTryStmt &S) {
  ...
  toolchain::Constant *C =
    ConstantEmitter(*this).tryEmitAbstract(Except->getFilterExpr(),
                                           getContext().IntTy);
  if (CGM.getTarget().getTriple().getArch() != toolchain::Triple::x86 && C &&
      C->isOneValue()) {
...
void CodeGenFunction::ExitSEHTryStmt(const SEHTryStmt &S) {
  ...
  if (getLangOpts().EHAsynch && Builder.GetInsertBlock()) {
    toolchain::FunctionCallee SehTryEnd = getSehTryEndFn(CGM);
  ...
  // The fall-through block.
  toolchain::BasicBlock *ContBB = createBasicBlock("__try.cont");
  ...
  toolchain::BasicBlock *CatchPadBB = CatchScope.getHandler(0).Block;
  EHStack.popCatch();
  ...
  toolchain::BasicBlock *ExceptBB = createBasicBlock("__except");
  Builder.CreateCatchRet(CPI, ExceptBB);
  EmitBlock(ExceptBB);

  // On Win64, the exception code is returned in EAX. Copy it into the slot.
  if (CGM.getTarget().getTriple().getArch() != toolchain::Triple::x86) {
    toolchain::Function *SEHCodeIntrin =
        CGM.getIntrinsic(toolchain::Intrinsic::eh_exceptioncode);
    toolchain::Value *Code = Builder.CreateCall(SEHCodeIntrin, {CPI});
    Builder.CreateStore(Code, SEHCodeSlotStack.back());
  }
