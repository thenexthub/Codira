...
#ifndef LANGUAGE_CORE_LIB_CODEGEN_CGCXXABI_H
#define LANGUAGE_CORE_LIB_CODEGEN_CGCXXABI_H
...
protected:
  ...
  toolchain::Value *getThisValue(CodeGenFunction &CGF) {
  ...
  toolchain::Constant *GetBogusMemberPointer(QualType T);

  ImplicitParamDecl *&getStructorImplicitParamDecl(CodeGenFunction &CGF) {
    return CGF.CXXStructorImplicitParamDecl;
  }
  toolchain::Value *&getStructorImplicitParamValue(CodeGenFunction &CGF) {
    return CGF.CXXStructorImplicitParamValue;
  }

  /// Loads the incoming C++ this pointer as it was passed by the caller.
  toolchain::Value *loadIncomingCXXThis(CodeGenFunction &CGF);

  void setCXXABIThisValue(CodeGenFunction &CGF, toolchain::Value *ThisPtr);
...
public:
  ...
  /// pointer conversion.
  virtual toolchain::Value *EmitMemberPointerConversion(CodeGenFunction &CGF,
                                                   const CastExpr *E,
                                                   toolchain::Value *Src);

  /// Perform a derived-to-base, base-to-derived, or bitcast member
  /// pointer conversion on a constant value.
  virtual toolchain::Constant *EmitMemberPointerConversion(const CastExpr *E,
  ...
  virtual toolchain::Constant *EmitNullMemberPointer(const MemberPointerType *MPT);

  /// Create a member pointer for the given method.
  virtual toolchain::Constant *EmitMemberFunctionPointer(const CXXMethodDecl *MD);

  /// Create a member pointer for the given field.
  virtual toolchain::Constant *EmitMemberDataPointer(const MemberPointerType *MPT,
                                                CharUnits offset);

  /// Create a member pointer for the given member pointer constant.
  virtual toolchain::Constant *EmitMemberPointer(const APValue &MP, QualType MPT);
...
protected:
  ...
  /// is required.
  toolchain::Constant *getMemberPointerAdjustment(const CastExpr *E);
...
public:
  ...
  virtual toolchain::GlobalVariable *getThrowInfo(QualType T) { return nullptr; }
  ...
  virtual toolchain::CallInst *
  emitTerminateForUnexpectedException(CodeGenFunction &CGF,
                                      toolchain::Value *Exn);

  virtual toolchain::Constant *getAddrOfRTTIDescriptor(QualType Ty) = 0;
  ...
  virtual toolchain::Value *EmitTypeid(CodeGenFunction &CGF, QualType SrcRecordTy,
                                  Address ThisPtr,
                                  toolchain::Type *StdTypeInfoPtrTy) = 0;

  virtual bool shouldDynamicCastCallBeNullChecked(bool SrcIsPtr,
                                                  QualType SrcRecordTy) = 0;
  virtual bool shouldEmitExactDynamicCast(QualType DestRecordTy) = 0;

  virtual toolchain::Value *emitDynamicCastCall(CodeGenFunction &CGF, Address Value,
                                           QualType SrcRecordTy,
                                           QualType DestTy,
                                           QualType DestRecordTy,
                                           toolchain::BasicBlock *CastEnd) = 0;

  virtual toolchain::Value *emitDynamicCastToVoid(CodeGenFunction &CGF,
                                             Address Value,
                                             QualType SrcRecordTy) = 0;
  ...
  virtual toolchain::Value *emitExactDynamicCast(
      CodeGenFunction &CGF, Address Value, QualType SrcRecordTy,
      QualType DestTy, QualType DestRecordTy,
      const ExactDynamicCastInfo &CastInfo, toolchain::BasicBlock *CastSuccess,
      toolchain::BasicBlock *CastFail) = 0;

  virtual bool EmitBadCastCall(CodeGenFunction &CGF) = 0;

  virtual toolchain::Value *GetVirtualBaseClassOffset(CodeGenFunction &CGF,
                                                 Address This,
                                                 const CXXRecordDecl *ClassDecl,
                                        const CXXRecordDecl *BaseClassDecl) = 0;

  virtual toolchain::BasicBlock *EmitCtorCompleteObjectHandler(CodeGenFunction &CGF,
  ...
  virtual void setCXXDestructorDLLStorage(toolchain::GlobalValue *GV,
                                          const CXXDestructorDecl *Dtor,
                                          CXXDtorType DT) const;

  virtual toolchain::GlobalValue::LinkageTypes
  ...
  virtual toolchain::GlobalVariable *getAddrOfVTable(const CXXRecordDecl *RD,
  ...
  using DeleteOrMemberCallExpr =
      toolchain::PointerUnion<const CXXDeleteExpr *, const CXXMemberCallExpr *>;
  ...
  EmitVirtualDestructorCall(CodeGenFunction &CGF, const CXXDestructorDecl *Dtor,
                            ...
                            toolchain::CallBase **CallOrInvoke) = 0;
  ...
  virtual void setThunkLinkage(toolchain::Function *Thunk, bool ForVTable,
  ...
  virtual void ReadArrayCookie(CodeGenFunction &CGF, Address Ptr,
                               ...
                               QualType ElementType, toolchain::Value *&NumElements,
...
protected:
  ...
  ///
  /// \return a size_t
  virtual toolchain::Value *readArrayCookieImpl(CodeGenFunction &IGF, Address ptr,
...
public:
  ...
  virtual std::pair<toolchain::Value *, const CXXRecordDecl *>
...
struct CatchRetScope final : EHScopeStack::Cleanup {
  ...
  CatchRetScope(toolchain::CatchPadInst *CPI) : CPI(CPI) {}

  void Emit(CodeGenFunction &CGF, Flags flags) override {
    toolchain::BasicBlock *BB = CGF.createBasicBlock("catchret.dest");
