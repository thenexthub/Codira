...
public:
  ...
  ComplexPairTy EmitScalarToComplexCast(toolchain::Value *Val, QualType SrcType,
                                        QualType DestType, SourceLocation Loc);
  ...
  ComplexPairTy VisitConstantExpr(ConstantExpr *E) {
    if (toolchain::Constant *Result = ConstantEmitter(CGF).tryEmitConstantExpr(E))
  ...
  ComplexPairTy emitConstant(const CodeGenFunction::ConstantEmission &Constant,
    ...
    toolchain::Constant *pair = Constant.getValue();
  ...
  ComplexPairTy VisitCXXScalarValueInitExpr(CXXScalarValueInitExpr *E) {
    ...
    toolchain::Constant *Null = toolchain::Constant::getNullValue(CGF.ConvertType(Elem));
  ...
  ComplexPairTy VisitImplicitValueInitExpr(ImplicitValueInitExpr *E) {
    ...
    toolchain::Constant *Null =
                       toolchain::Constant::getNullValue(CGF.ConvertType(Elem));
    return ComplexPairTy(Null, Null);
  }
  ...
  ComplexPairTy EmitAlgebraicDiv(toolchain::Value *A, toolchain::Value *B, toolchain::Value *C,
                                 toolchain::Value *D);
  ComplexPairTy EmitRangeReductionDiv(toolchain::Value *A, toolchain::Value *B,
                                      toolchain::Value *C, toolchain::Value *D);
...
ComplexPairTy ComplexExprEmitter::EmitLoadOfLValue(LValue lvalue,
  ...
  toolchain::Value *Real = nullptr, *Imag = nullptr;
...
ComplexPairTy ComplexExprEmitter::VisitExpr(Expr *E) {
  ...
  toolchain::Value *U = toolchain::PoisonValue::get(EltTy);
  return ComplexPairTy(U, U);
}

ComplexPairTy ComplexExprEmitter::
VisitImaginaryLiteral(const ImaginaryLiteral *IL) {
  toolchain::Value *Imag = CGF.EmitScalarExpr(IL->getSubExpr());
  return ComplexPairTy(toolchain::Constant::getNullValue(Imag->getType()), Imag);
}
...
ComplexPairTy ComplexExprEmitter::EmitScalarToComplexCast(toolchain::Value *Val,
                                                          QualType SrcType,
                                                          QualType DestType,
                                                          SourceLocation Loc) {
  ...
  // Return (realval, 0).
  return ComplexPairTy(Val, toolchain::Constant::getNullValue(Val->getType()));
}
...
ComplexPairTy ComplexExprEmitter::EmitComplexBinOpLibCall(StringRef LibCallName,
  ...
  toolchain::FunctionType *FTy = CGF.CGM.getTypes().GetFunctionType(FuncInfo);
  toolchain::FunctionCallee Func = CGF.CGM.CreateRuntimeFunction(
      FTy, LibCallName, toolchain::AttributeList(), true);
...
/// multiply.
static StringRef getComplexMultiplyLibCallName(toolchain::Type *Ty) {
  switch (Ty->getTypeID()) {
  default:
    toolchain_unreachable("Unsupported floating point type!");
...
ComplexPairTy ComplexExprEmitter::EmitBinMul(const BinOpInfo &Op) {
  ...
  if (Op.LHS.first->getType()->isFloatingPointTy()) {
    ...
    if (Op.LHS.second && Op.RHS.second) {
      ...
      toolchain::BasicBlock *ContBB = CGF.createBasicBlock("complex_mul_cont");
      toolchain::BasicBlock *INaNBB = CGF.createBasicBlock("complex_mul_imag_nan");
      toolchain::Instruction *Branch = Builder.CreateCondBr(IsRNaN, INaNBB, ContBB);
      toolchain::BasicBlock *OrigBB = Branch->getParent();

      // Give hint that we very much don't expect to see NaNs.
      toolchain::MDNode *BrWeight = MDHelper.createUnlikelyBranchWeights();
      Branch->setMetadata(toolchain::LLVMContext::MD_prof, BrWeight);
      ...
      toolchain::BasicBlock *LibCallBB = CGF.createBasicBlock("complex_mul_libcall");
      ...
      toolchain::PHINode *RealPHI = Builder.CreatePHI(ResR->getType(), 3, "real_mul_phi");
      RealPHI->addIncoming(ResR, OrigBB);
      RealPHI->addIncoming(ResR, INaNBB);
      RealPHI->addIncoming(LibCallR, LibCallBB);
      toolchain::PHINode *ImagPHI = Builder.CreatePHI(ResI->getType(), 3, "imag_mul_phi");
...
ComplexPairTy ComplexExprEmitter::EmitAlgebraicDiv(toolchain::Value *LHSr,
                                                   toolchain::Value *LHSi,
                                                   toolchain::Value *RHSr,
                                                   toolchain::Value *RHSi) {
  // (a+ib) / (c+id) = ((ac+bd)/(cc+dd)) + i((bc-ad)/(cc+dd))
  toolchain::Value *DSTr, *DSTi;

  toolchain::Value *AC = Builder.CreateFMul(LHSr, RHSr); // a*c
  toolchain::Value *BD = Builder.CreateFMul(LHSi, RHSi); // b*d
  toolchain::Value *ACpBD = Builder.CreateFAdd(AC, BD);  // ac+bd

  toolchain::Value *CC = Builder.CreateFMul(RHSr, RHSr); // c*c
  toolchain::Value *DD = Builder.CreateFMul(RHSi, RHSi); // d*d
  toolchain::Value *CCpDD = Builder.CreateFAdd(CC, DD);  // cc+dd

  toolchain::Value *BC = Builder.CreateFMul(LHSi, RHSr); // b*c
  toolchain::Value *AD = Builder.CreateFMul(LHSr, RHSi); // a*d
  toolchain::Value *BCmAD = Builder.CreateFSub(BC, AD);  // bc-ad
...
static toolchain::Value *EmittoolchainFAbs(CodeGenFunction &CGF, toolchain::Value *Value) {
  toolchain::Function *Func =
      CGF.CGM.getIntrinsic(toolchain::Intrinsic::fabs, Value->getType());
  toolchain::Value *Call = CGF.Builder.CreateCall(Func, Value);
  return Call;
}

// EmitRangeReductionDiv - Implements Smith's algorithm for complex division.
// SMITH, R. L. Algorithm 116: Complex division. Commun. ACM 5, 8 (1962).
ComplexPairTy ComplexExprEmitter::EmitRangeReductionDiv(toolchain::Value *LHSr,
                                                        toolchain::Value *LHSi,
                                                        toolchain::Value *RHSr,
                                                        toolchain::Value *RHSi) {
  ...
  // (a + ib) / (c + id) = (e + if)
  toolchain::Value *FAbsRHSr = EmittoolchainFAbs(CGF, RHSr); // |c|
  toolchain::Value *FAbsRHSi = EmittoolchainFAbs(CGF, RHSi); // |d|
  // |c| >= |d|
  toolchain::Value *IsR = Builder.CreateFCmpUGT(FAbsRHSr, FAbsRHSi, "abs_cmp");

  toolchain::BasicBlock *TrueBB =
      CGF.createBasicBlock("abs_rhsr_greater_or_equal_abs_rhsi");
  toolchain::BasicBlock *FalseBB =
      CGF.createBasicBlock("abs_rhsr_less_than_abs_rhsi");
  toolchain::BasicBlock *ContBB = CGF.createBasicBlock("complex_div");
  ...
  // e = (a + br)/tmp
  // f = (b - ar)/tmp
  toolchain::Value *DdC = Builder.CreateFDiv(RHSi, RHSr); // r=d/c

  toolchain::Value *RD = Builder.CreateFMul(DdC, RHSi);  // rd
  toolchain::Value *CpRD = Builder.CreateFAdd(RHSr, RD); // tmp=c+rd

  toolchain::Value *T3 = Builder.CreateFMul(LHSi, DdC);   // br
  toolchain::Value *T4 = Builder.CreateFAdd(LHSr, T3);    // a+br
  toolchain::Value *DSTTr = Builder.CreateFDiv(T4, CpRD); // (a+br)/tmp

  toolchain::Value *T5 = Builder.CreateFMul(LHSr, DdC);   // ar
  toolchain::Value *T6 = Builder.CreateFSub(LHSi, T5);    // b-ar
  toolchain::Value *DSTTi = Builder.CreateFDiv(T6, CpRD); // (b-ar)/tmp
  Builder.CreateBr(ContBB);
  ...
  // e = (ar + b)/tmp
  // f = (br - a)/tmp
  toolchain::Value *CdD = Builder.CreateFDiv(RHSr, RHSi); // r=c/d

  toolchain::Value *RC = Builder.CreateFMul(CdD, RHSr);  // rc
  toolchain::Value *DpRC = Builder.CreateFAdd(RHSi, RC); // tmp=d+rc

  toolchain::Value *T7 = Builder.CreateFMul(LHSr, CdD);   // ar
  toolchain::Value *T8 = Builder.CreateFAdd(T7, LHSi);    // ar+b
  toolchain::Value *DSTFr = Builder.CreateFDiv(T8, DpRC); // (ar+b)/tmp

  toolchain::Value *T9 = Builder.CreateFMul(LHSi, CdD);    // br
  toolchain::Value *T10 = Builder.CreateFSub(T9, LHSr);    // br-a
  toolchain::Value *DSTFi = Builder.CreateFDiv(T10, DpRC); // (br-a)/tmp
  Builder.CreateBr(ContBB);

  // Phi together the computation paths.
  CGF.EmitBlock(ContBB);
  toolchain::PHINode *VALr = Builder.CreatePHI(DSTTr->getType(), 2);
  VALr->addIncoming(DSTTr, TrueBB);
  VALr->addIncoming(DSTFr, FalseBB);
  toolchain::PHINode *VALi = Builder.CreatePHI(DSTTi->getType(), 2);
...
ComplexPairTy ComplexExprEmitter::EmitBinDiv(const BinOpInfo &Op) {
  toolchain::Value *LHSr = Op.LHS.first, *LHSi = Op.LHS.second;
  toolchain::Value *RHSr = Op.RHS.first, *RHSi = Op.RHS.second;
  toolchain::Value *DSTr, *DSTi;
  if (LHSr->getType()->isFloatingPointTy()) {
    ...
    toolchain::Value *OrigLHSi = LHSi;
    if (!LHSi)
      LHSi = toolchain::Constant::getNullValue(RHSi->getType());
    ...
    // '-fno-cx-limited-range' or '-fcomplex-arithmetic=full'.
      ...
      if (!LHSi)
        LibCallOp.LHS.second = toolchain::Constant::getNullValue(LHSr->getType());
    ...
    assert(Op.LHS.second && Op.RHS.second &&
           "Both operands of integer complex operators must be complex!");
    // (a+ib) / (c+id) = ((ac+bd)/(cc+dd)) + i((bc-ad)/(cc+dd))
    toolchain::Value *Tmp1 = Builder.CreateMul(LHSr, RHSr); // a*c
    toolchain::Value *Tmp2 = Builder.CreateMul(LHSi, RHSi); // b*d
    toolchain::Value *Tmp3 = Builder.CreateAdd(Tmp1, Tmp2); // ac+bd

    toolchain::Value *Tmp4 = Builder.CreateMul(RHSr, RHSr); // c*c
    toolchain::Value *Tmp5 = Builder.CreateMul(RHSi, RHSi); // d*d
    toolchain::Value *Tmp6 = Builder.CreateAdd(Tmp4, Tmp5); // cc+dd

    toolchain::Value *Tmp7 = Builder.CreateMul(LHSi, RHSr); // b*c
    toolchain::Value *Tmp8 = Builder.CreateMul(LHSr, RHSi); // a*d
    toolchain::Value *Tmp9 = Builder.CreateSub(Tmp7, Tmp8); // bc-ad
...
ComplexPairTy CodeGenFunction::EmitUnPromotedValue(ComplexPairTy result,
  ...
  toolchain::Type *ComplexElementTy =
...
EmitCompoundAssignLValue(const CompoundAssignOperator *E,
  ...
  if (LHSTy->isAnyComplexType()) {
    ...
    toolchain::Value *LHSVal = CGF.EmitLoadOfScalar(LHS, Loc);
...
VisitAbstractConditionalOperator(const AbstractConditionalOperator *E) {
  ...
  toolchain::PHINode *RealPN = Builder.CreatePHI(LHS.first->getType(), 2, "cond.r");
  RealPN->addIncoming(LHS.first, LHSBlock);
  RealPN->addIncoming(RHS.first, RHSBlock);

  // Create a PHI node for the imaginary part.
  toolchain::PHINode *ImagPN = Builder.CreatePHI(LHS.first->getType(), 2, "cond.i");
...
ComplexPairTy ComplexExprEmitter::VisitInitListExpr(InitListExpr *E) {
  ...
  if (E->getNumInits() == 2) {
    toolchain::Value *Real = CGF.EmitScalarExpr(E->getInit(0));
    toolchain::Value *Imag = CGF.EmitScalarExpr(E->getInit(1));
    return ComplexPairTy(Real, Imag);
  ...
  toolchain::Type* LTy = CGF.ConvertType(Ty);
  toolchain::Value* zeroConstant = toolchain::Constant::getNullValue(LTy);
...
static CompoundFunc getComplexOp(BinaryOperatorKind Op) {
  ...
  default:
    toolchain_unreachable("unexpected complex compound assignment");
  }
}
