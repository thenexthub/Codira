...
#include "toolchain/IR/Dominators.h"
...
#include "toolchain/Transforms/Scalar/LowerExpectIntrinsic.h"
#include "toolchain/Transforms/Utils/PromoteMemToReg.h"
#include <optional>
...
CodeGenFunction::CodeGenFunction(CodeGenModule &cgm, bool suppressNewContext)
    : CodeGenTypeCache(cgm), CGM(cgm), Target(cgm.getTarget()),
      Builder(cgm, cgm.getModule().getContext(), toolchain::ConstantFolder(),
...
language::Core::ToConstrainedExceptMD(LangOptions::FPExceptionModeKind Kind) {

  switch (Kind) {
  case LangOptions::FPE_Ignore:  return toolchain::fp::ebIgnore;
  case LangOptions::FPE_MayTrap: return toolchain::fp::ebMayTrap;
  case LangOptions::FPE_Strict:  return toolchain::fp::ebStrict;
  default:
    toolchain_unreachable("Unsupported FP Exception Behavior");
  }
}

void CodeGenFunction::SetFastMathFlags(FPOptions FPFeatures) {
  toolchain::FastMathFlags FMF;
...
void CodeGenFunction::CGFPOptionsRAII::ConstructorHelper(FPOptions FPFeatures) {
  ...
  toolchain::RoundingMode NewRoundingBehavior = FPFeatures.getRoundingMode();
  ...
  assert((CGF.CurFuncDecl == nullptr || CGF.Builder.getIsFPConstrained() ||
          ...
          (NewExceptionBehavior == toolchain::fp::ebIgnore &&
           NewRoundingBehavior == toolchain::RoundingMode::NearestTiesToEven)) &&
  ...
  auto mergeFnAttrValue = [&](StringRef Name, bool Value) {
    ...
    if (OldValue != NewValue)
      CGF.CurFn->addFnAttr(Name, toolchain::toStringRef(NewValue));
  };
...
static LValue
makeNaturalAlignAddrLValue(toolchain::Value *V, QualType T, bool ForPointeeType,
...
LValue
CodeGenFunction::MakeNaturalAlignAddrLValue(toolchain::Value *V, QualType T,
...
LValue
CodeGenFunction::MakeNaturalAlignPointeeAddrLValue(toolchain::Value *V, QualType T) {
  return ::makeNaturalAlignAddrLValue(V, T, /*ForPointeeType*/ true,
                                      /*MightBeSigned*/ true, *this);
}

LValue CodeGenFunction::MakeNaturalAlignRawAddrLValue(toolchain::Value *V,
                                                      QualType T) {
  return ::makeNaturalAlignAddrLValue(V, T, /*ForPointeeType*/ false,
                                      /*MightBeSigned*/ false, *this);
}

LValue CodeGenFunction::MakeNaturalAlignPointeeRawAddrLValue(toolchain::Value *V,
                                                             QualType T) {
  return ::makeNaturalAlignAddrLValue(V, T, /*ForPointeeType*/ true,
                                      /*MightBeSigned*/ false, *this);
}

toolchain::Type *CodeGenFunction::ConvertTypeForMem(QualType T) {
  return CGM.getTypes().ConvertTypeForMem(T);
}

toolchain::Type *CodeGenFunction::ConvertType(QualType T) {
  return CGM.getTypes().ConvertType(T);
}

toolchain::Type *CodeGenFunction::convertTypeForLoadStore(QualType ASTTy,
                                                     toolchain::Type *LLVMTy) {
...
#include "language/Core/AST/TypeNodes.inc"
      toolchain_unreachable("non-canonical or dependent type in IR-generation");

    case Type::Auto:
    case Type::DeducedTemplateSpecialization:
      toolchain_unreachable("undeduced type in IR-generation");
    ...
    toolchain_unreachable("unknown type kind!");
  }
}

toolchain::DebugLoc CodeGenFunction::EmitReturnBlock() {
  ...
  if (ReturnBlock.getBlock()->hasOneUse()) {
    toolchain::BranchInst *BI =
      dyn_cast<toolchain::BranchInst>(*ReturnBlock.getBlock()->user_begin());
    if (BI && BI->isUnconditional() &&
        BI->getSuccessor(0) == ReturnBlock.getBlock()) {
      // Record/return the DebugLoc of the simple 'return' expression to be used
      // later by the actual 'ret' instruction.
      toolchain::DebugLoc Loc = BI->getDebugLoc();
...
static void EmitIfUsed(CodeGenFunction &CGF, toolchain::BasicBlock *BB) {
  if (!BB) return;
  if (!BB->use_empty()) {
...
void CodeGenFunction::FinishFunction(SourceLocation EndLoc) {
  ...
  toolchain::DebugLoc Loc = EmitReturnBlock();
  ...
  // If some of our locals escaped, insert a call to toolchain.localescape in the
  // entry block.
  if (!EscapedLocals.empty()) {
    // Invert the map from local to index into a simple vector. There should be
    // no holes.
    SmallVector<toolchain::Value *, 4> EscapeArgs;
    EscapeArgs.resize(EscapedLocals.size());
    for (auto &Pair : EscapedLocals)
      EscapeArgs[Pair.second] = Pair.first;
    toolchain::Function *FrameEscapeFn = toolchain::Intrinsic::getOrInsertDeclaration(
        &CGM.getModule(), toolchain::Intrinsic::localescape);
    CGBuilderTy(*this, AllocaInsertPt).CreateCall(FrameEscapeFn, EscapeArgs);
  }

  // Remove the AllocaInsertPt instruction, which is just a convenience for us.
  toolchain::Instruction *Ptr = AllocaInsertPt;
  ...
  if (PostAllocaInsertPt) {
    toolchain::Instruction *PostPtr = PostAllocaInsertPt;
  ...
  if (IndirectBranch) {
    toolchain::PHINode *PN = cast<toolchain::PHINode>(IndirectBranch->getAddress());
    if (PN->getNumIncomingValues() == 0) {
      PN->replaceAllUsesWith(toolchain::PoisonValue::get(PN->getType()));
      PN->eraseFromParent();
    }
  }
  ...
  for (const auto &R : DeferredReplacements) {
    if (toolchain::Value *Old = R.first) {
      Old->replaceAllUsesWith(R.second);
      cast<toolchain::Instruction>(Old)->eraseFromParent();
    }
  }
  ...
  if (NormalCleanupDest.isValid() && isCoroutine()) {
    toolchain::DominatorTree DT(*CurFn);
    toolchain::PromoteMemToReg(
        cast<toolchain::AllocaInst>(NormalCleanupDest.getPointer()), DT);
    NormalCleanupDest = Address::invalid();
  }

  // Scan function arguments for vector width.
  for (toolchain::Argument &A : CurFn->args())
    if (auto *VT = dyn_cast<toolchain::VectorType>(A.getType()))
      LargestVectorWidth =
          std::max((uint64_t)LargestVectorWidth,
                   VT->getPrimitiveSizeInBits().getKnownMinValue());

  // Update vector width based on return type.
  if (auto *VT = dyn_cast<toolchain::VectorType>(CurFn->getReturnType()))
    LargestVectorWidth =
        std::max((uint64_t)LargestVectorWidth,
                 VT->getPrimitiveSizeInBits().getKnownMinValue());
  ...
  if (getContext().getTargetInfo().getTriple().isX86())
    CurFn->addFnAttr("min-legal-vector-width",
                     toolchain::utostr(LargestVectorWidth));
  ...
  if (ReturnValue.isValid()) {
    auto *RetAlloca =
        dyn_cast<toolchain::AllocaInst>(ReturnValue.emitRawPointer(*this));
...
CodeGenFunction::getUBSanFunctionTypeHash(QualType Ty) const {
  ...
  std::string Mangled;
  toolchain::raw_string_ostream Out(Mangled);
  CGM.getCXXABI().getMangleContext().mangleCanonicalTypeName(Ty, Out, false);
  return toolchain::ConstantInt::get(
      CGM.Int32Ty, static_cast<uint32_t>(toolchain::xxh3_64bits(Mangled)));
}
...
void CodeGenFunction::EmitKernelMetadata(const FunctionDecl *FD,
  ...
  if (const VecTypeHintAttr *A = FD->getAttr<VecTypeHintAttr>()) {
    ...
    toolchain::Metadata *AttrMDArgs[] = {
        toolchain::ConstantAsMetadata::get(toolchain::PoisonValue::get(
            CGM.getTypes().ConvertType(A->getTypeHint()))),
        toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::get(
            toolchain::IntegerType::get(Context, 32),
            toolchain::APInt(32, (uint64_t)(IsSignedInteger ? 1 : 0))))};
    Fn->setMetadata("vec_type_hint", toolchain::MDNode::get(Context, AttrMDArgs));
  }
  ...
  if (const WorkGroupSizeHintAttr *A = FD->getAttr<WorkGroupSizeHintAttr>()) {
    ...
    toolchain::Metadata *AttrMDArgs[] = {
        toolchain::ConstantAsMetadata::get(Builder.getInt32(Eval(A->getXDim()))),
        toolchain::ConstantAsMetadata::get(Builder.getInt32(Eval(A->getYDim()))),
        toolchain::ConstantAsMetadata::get(Builder.getInt32(Eval(A->getZDim())))};
    Fn->setMetadata("work_group_size_hint", toolchain::MDNode::get(Context, AttrMDArgs));
  }
  ...
  if (const ReqdWorkGroupSizeAttr *A = FD->getAttr<ReqdWorkGroupSizeAttr>()) {
    ...
    toolchain::Metadata *AttrMDArgs[] = {
        ...
        toolchain::ConstantAsMetadata::get(Builder.getInt32(Eval(A->getYDim()))),
        toolchain::ConstantAsMetadata::get(Builder.getInt32(Eval(A->getZDim())))};
    Fn->setMetadata("reqd_work_group_size", toolchain::MDNode::get(Context, AttrMDArgs));
  }

  if (const OpenCLIntelReqdSubGroupSizeAttr *A =
          FD->getAttr<OpenCLIntelReqdSubGroupSizeAttr>()) {
    toolchain::Metadata *AttrMDArgs[] = {
        toolchain::ConstantAsMetadata::get(Builder.getInt32(A->getSubGroupSize()))};
    Fn->setMetadata("intel_reqd_sub_group_size",
                    toolchain::MDNode::get(Context, AttrMDArgs));
  }
}
...
void CodeGenFunction::markAsIgnoreThreadCheckingAtRuntime(toolchain::Function *Fn) {
  if (SanOpts.has(SanitizerKind::Thread)) {
    Fn->addFnAttr("sanitize_thread_no_checking_at_run_time");
    Fn->removeFnAttr(toolchain::Attribute::SanitizeThread);
  }
}
...
bool CodeGenFunction::hasInAllocaArg(const CXXMethodDecl *MD) {
  return getTarget().getTriple().getArch() == toolchain::Triple::x86 &&
         getTarget().getCXXABI().isMicrosoft() &&
         toolchain::any_of(MD->parameters(), [&](ParmVarDecl *P) {
           return isInAllocaArgument(CGM.getCXXABI(), P->getType());
         });
}

/// Return the UBSan prologue signature for \p FD if one is available.
static toolchain::Constant *getPrologueSignature(CodeGenModule &CGM,
...
#undef SANITIZER
  ...
  if (D) {
    ...
    if (SanitizeBounds && !SanOpts.hasOneOf(SanitizerKind::Bounds))
      Fn->addFnAttr(toolchain::Attribute::NoSanitizeBounds);

    if (NoSanitizeCoverage && CGM.getCodeGenOpts().hasSanitizeCoverage())
      Fn->addFnAttr(toolchain::Attribute::NoSanitizeCoverage);
  ...
  if (ShouldSkipSanitizerInstrumentation()) {
    CurFn->addFnAttr(toolchain::Attribute::DisableSanitizerInstrumentation);
  } else {
    // Apply sanitizer attributes to the function.
    if (SanOpts.hasOneOf(SanitizerKind::Address | SanitizerKind::KernelAddress))
      Fn->addFnAttr(toolchain::Attribute::SanitizeAddress);
    if (SanOpts.hasOneOf(SanitizerKind::HWAddress |
                         SanitizerKind::KernelHWAddress))
      Fn->addFnAttr(toolchain::Attribute::SanitizeHWAddress);
    if (SanOpts.has(SanitizerKind::MemtagStack))
      Fn->addFnAttr(toolchain::Attribute::SanitizeMemTag);
    if (SanOpts.has(SanitizerKind::Thread))
      Fn->addFnAttr(toolchain::Attribute::SanitizeThread);
    if (SanOpts.has(SanitizerKind::Type))
      Fn->addFnAttr(toolchain::Attribute::SanitizeType);
    if (SanOpts.has(SanitizerKind::NumericalStability))
      Fn->addFnAttr(toolchain::Attribute::SanitizeNumericalStability);
    if (SanOpts.hasOneOf(SanitizerKind::Memory | SanitizerKind::KernelMemory))
      Fn->addFnAttr(toolchain::Attribute::SanitizeMemory);
  }
  if (SanOpts.has(SanitizerKind::SafeStack))
    Fn->addFnAttr(toolchain::Attribute::SafeStack);
  if (SanOpts.has(SanitizerKind::ShadowCallStack))
    Fn->addFnAttr(toolchain::Attribute::ShadowCallStack);
    ...
    if (FD && FD->getASTContext().hasAnyFunctionEffects())
      for (const FunctionEffectWithCondition &Fe : FD->getFunctionEffects()) {
        if (Fe.Effect.kind() == FunctionEffect::Kind::NonBlocking)
          Fn->addFnAttr(toolchain::Attribute::SanitizeRealtime);
        else if (Fe.Effect.kind() == FunctionEffect::Kind::Blocking)
          Fn->addFnAttr(toolchain::Attribute::SanitizeRealtimeBlocking);
      }

  // Apply fuzzing attribute to the function.
  if (SanOpts.hasOneOf(SanitizerKind::Fuzzer | SanitizerKind::FuzzerNoLink))
    Fn->addFnAttr(toolchain::Attribute::OptForFuzzing);
      ...
      if (const auto *LogArgs = D->getAttr<XRayLogArgsAttr>())
        if (ShouldXRayInstrumentFunction())
          Fn->addFnAttr("xray-log-args",
                        toolchain::utostr(LogArgs->getArgumentCount()));
    }
  } else {
    if (ShouldXRayInstrumentFunction() && !CGM.imbueXRayAttrs(Fn, Loc))
      Fn->addFnAttr(
          "xray-instruction-threshold",
          toolchain::itostr(CGM.getCodeGenOpts().XRayInstructionThreshold));
  }
  ...
  if (ShouldXRayInstrumentFunction()) {
    ...
    if (FuncGroups > 1) {
      auto FuncName = toolchain::ArrayRef<uint8_t>(CurFn->getName().bytes_begin(),
  ...
  if (CGM.getCodeGenOpts().getProfileInstr() !=
    ...
    case ProfileList::Skip:
      Fn->addFnAttr(toolchain::Attribute::SkipProfile);
      break;
    case ProfileList::Forbid:
      Fn->addFnAttr(toolchain::Attribute::NoProfile);
      break;
  ...
  if (D && D->hasAttr<HybridPatchableAttr>())
    Fn->addFnAttr(toolchain::Attribute::HybridPatchable);
  ...
  if (D) {
    ...
    if (auto *A = D->getAttr<FunctionReturnThunksAttr>()) {
      ...
      case FunctionReturnThunksAttr::Kind::Extern:
        Fn->addFnAttr(toolchain::Attribute::FnRetThunkExtern);
        break;
      }
  ...
  if (FD && SanOpts.has(SanitizerKind::Function)) {
    if (toolchain::Constant *PrologueSig = getPrologueSignature(CGM, FD)) {
      toolchain::LLVMContext &Ctx = Fn->getContext();
      toolchain::MDBuilder MDB(Ctx);
      Fn->setMetadata(
          toolchain::LLVMContext::MD_func_sanitize,
    ...
    if (Nullability && *Nullability == NullabilityKind::NonNull &&
      ...
      if (!(SanOpts.has(SanitizerKind::ReturnsNonnullAttribute) &&
        ...
        RetValNullabilityPrecondition =
            toolchain::ConstantInt::getTrue(getLLVMContext());
    }
  }
  ...
  if (FD &&
    ...
    Fn->addFnAttr(toolchain::Attribute::NoRecurse);

  toolchain::RoundingMode RM = getLangOpts().getDefaultRoundingMode();
  toolchain::fp::ExceptionBehavior FPExceptionBehavior =
      ToConstrainedExceptMD(getLangOpts().getDefaultExceptionMode());
  Builder.setDefaultConstrainedRounding(RM);
  Builder.setDefaultConstrainedExcept(FPExceptionBehavior);
  if ((FD && (FD->UsesFPIntrin() || FD->hasAttr<StrictFPAttr>())) ||
      (!FD && (FPExceptionBehavior != toolchain::fp::ebIgnore ||
               RM != toolchain::RoundingMode::NearestTiesToEven))) {
    Builder.setIsFPConstrained(true);
    Fn->addFnAttr(toolchain::Attribute::StrictFP);
  }
  ...
  std::optional<std::pair<unsigned, unsigned>> VScaleRange =
      getContext().getTargetInfo().getVScaleRange(getLangOpts(), IsArmStreaming,
                                                  &FeatureMap);
  if (VScaleRange) {
    CurFn->addFnAttr(toolchain::Attribute::getWithVScaleRangeArgs(
        getLLVMContext(), VScaleRange->first, VScaleRange->second));
  }

  toolchain::BasicBlock *EntryBB = createBasicBlock("entry", CurFn);

  // Create a marker to make it easy to insert allocas into the entryblock
  // later.  Don't create this with the builder, because we don't want it
  // folded.
  toolchain::Value *Poison = toolchain::PoisonValue::get(Int32Ty);
  AllocaInsertPt = new toolchain::BitCastInst(Poison, Int32Ty, "allocapt", EntryBB);
  ...
  if (requiresReturnValueCheck()) {
    ...
    Builder.CreateStore(toolchain::ConstantPointerNull::get(Int8PtrTy),
                        ReturnLocation);
  }
  ...
  if (RetTy->isVoidType()) {
    ...
    --EI;
    toolchain::Value *Addr = Builder.CreateStructGEP(
        CurFnInfo->getArgStruct(), &*EI, Idx);
    toolchain::Type *Ty =
        cast<toolchain::GetElementPtrInst>(Addr)->getResultElementType();
...
void CodeGenFunction::EmitBlockWithFallThrough(toolchain::BasicBlock *BB,
                                               const Stmt *S) {
  toolchain::BasicBlock *SkipCountBB = nullptr;
  // Do not skip over the instrumentation when single byte coverage mode is
  // enabled.
  if (HaveInsertPoint() && CGM.getCodeGenOpts().hasProfileClangInstr() &&
      !toolchain::EnableSingleByteCoverage) {
...
static void TryMarkNoThrow(toolchain::Function *F) {
  // LLVM treats 'nounwind' on a function as part of the type, so we
  // can't do this on functions that can be overwritten.
  if (F->isInterposable()) return;

  for (toolchain::BasicBlock &BB : *F)
    for (toolchain::Instruction &I : BB)
      if (I.mayThrow())
        return;
...
void CodeGenFunction::GenerateCode(GlobalDecl GD, toolchain::Function *Fn,
                                   const CGFunctionInfo &FnInfo) {
  ...
  if (FD->isInlineBuiltinDeclaration()) {
    ...
    Fn->setLinkage(toolchain::GlobalValue::ExternalLinkage);
    Fn = Clone;
  } else {
    ...
    for (const FunctionDecl *PD = FD->getPreviousDecl(); PD;
      ...
      if (LLVM_UNLIKELY(PD->isInlineBuiltinDeclaration())) {
        std::string FDInlineName = (Fn->getName() + ".inline").str();
        toolchain::Module *M = Fn->getParent();
        if (toolchain::Function *Clone = M->getFunction(FDInlineName)) {
  ...
  auto Cleanup = toolchain::make_scope_exit([this] {
    if (CGDebugInfo *DI = getDebugInfo())
      DI->completeFunction();
  });
  ...
  if (Body && isa_and_nonnull<CoroutineBodyStmt>(Body))
    toolchain::append_range(FnArgs, FD->parameters());
  ...
  if (checkIfFunctionMustProgress())
    CurFn->addFnAttr(toolchain::Attribute::MustProgress);
  ...
  if (isa<CXXDestructorDecl>(FD))
    ...
    toolchain::FunctionType *FTy = CGM.getTypes().GetFunctionType(FnInfo);
    toolchain::Constant *GDStubFunctionPointer =
    ...
    toolchain_unreachable("no definition for emitted function");
  ...
  if (getLangOpts().CPlusPlus && !FD->hasImplicitReturnZero() && !SawAsmBlock &&
    ...
    if (SanOpts.has(SanitizerKind::Return)) {
      ...
      toolchain::Value *IsFalse = Builder.getFalse();
      EmitCheck(std::make_pair(IsFalse, CheckOrdinal), CheckHandler,
                EmitCheckSourceLocation(FD->getLocation()), {});
    } else if (ShouldEmitUnreachable) {
      if (CGM.getCodeGenOpts().OptimizationLevel == 0)
        EmitTrapCall(toolchain::Intrinsic::trap);
    }
...
bool CodeGenFunction::ConstantFoldsToSimpleInteger(const Expr *Cond,
  ...
  toolchain::APSInt Int = Result.Val.getInt();
...
void CodeGenFunction::EmitBranchToCounterBlock(
    const Expr *Cond, BinaryOperator::Opcode LOp, toolchain::BasicBlock *TrueBlock,
    toolchain::BasicBlock *FalseBlock, uint64_t TrueCount /* = 0 */,
  ...
  toolchain::BasicBlock *ThenBlock = nullptr;
  toolchain::BasicBlock *ElseBlock = nullptr;
  toolchain::BasicBlock *NextBlock = nullptr;

  // Create the block we'll use to increment the appropriate counter.
  toolchain::BasicBlock *CounterIncrBlock = createBasicBlock("lop.rhscnt");
  ...
  //   goto FalseBlock;
    ...
    toolchain_unreachable("Expected Opcode must be that of a Logical Operator");
  }
...
void CodeGenFunction::EmitBranchOnBoolExpr(
    const Expr *Cond, toolchain::BasicBlock *TrueBlock, toolchain::BasicBlock *FalseBlock,
  ...
  if (const BinaryOperator *CondBOp = dyn_cast<BinaryOperator>(Cond)) {
    ...
    if (CondBOp->getOpcode() == BO_LAnd) {
      ...
      toolchain::BasicBlock *LHSTrue = createBasicBlock("land.lhs.true");
    ...
    if (CondBOp->getOpcode() == BO_LOr) {
      ...
      toolchain::BasicBlock *LHSFalse = createBasicBlock("lor.lhs.false");
  ...
  if (const ConditionalOperator *CondOp = dyn_cast<ConditionalOperator>(Cond)) {
    ...
    toolchain::BasicBlock *LHSBlock = createBasicBlock("cond.true");
    toolchain::BasicBlock *RHSBlock = createBasicBlock("cond.false");
  ...
  toolchain::MDNode *Unpredictable = nullptr;
  ...
  toolchain::Value *NewCondV = emitCondLikelihoodViaExpectIntrinsic(CondV, LH);
  if (CondV != NewCondV)
    CondV = NewCondV;
  else {
  ...
  toolchain::Instruction *BrInst = Builder.CreateCondBr(CondV, TrueBlock, FalseBlock,
...
static void emitNonZeroVLAInit(CodeGenFunction &CGF, QualType baseType,
  ...
  toolchain::Value *baseSizeInChars
    = toolchain::ConstantInt::get(CGF.IntPtrTy, baseSize.getQuantity());

  Address begin = dest.withElementType(CGF.Int8Ty);
  toolchain::Value *end = Builder.CreateInBoundsGEP(begin.getElementType(),
                                               begin.emitRawPointer(CGF),
                                               sizeInChars, "vla.end");

  toolchain::BasicBlock *originBB = CGF.Builder.GetInsertBlock();
  toolchain::BasicBlock *loopBB = CGF.createBasicBlock("vla-init.loop");
  toolchain::BasicBlock *contBB = CGF.createBasicBlock("vla-init.cont");

  // Make a loop over the VLA.  C99 guarantees that the VLA element
  // count must be nonzero.
  CGF.EmitBlock(loopBB);

  toolchain::PHINode *cur = Builder.CreatePHI(begin.getType(), 2, "vla.cur");
  ...
  toolchain::Value *done = Builder.CreateICmpEQ(next, end, "vla-init.isdone");
...
CodeGenFunction::EmitNullInitialization(Address DestPtr, QualType Ty) {
  ...
  if (!CGM.getTypes().isZeroInitializable(Ty)) {
    ...
    toolchain::Constant *NullConstant = CGM.EmitNullConstant(Ty);

    toolchain::GlobalVariable *NullVariable =
      new toolchain::GlobalVariable(CGM.getModule(), NullConstant->getType(),
                               /*isConstant=*/true,
    ...
    // Get and call the appropriate toolchain.memcpy overload.
...
toolchain::BlockAddress *CodeGenFunction::GetAddrOfLabel(const LabelDecl *L) {
  // Make sure that there is a block for the indirect goto.
  if (!IndirectBranch)
    GetIndirectGotoBlock();

  toolchain::BasicBlock *BB = getJumpDestForLabel(L).getBlock();

  // Make sure the indirect branch includes all of the address-taken blocks.
  IndirectBranch->addDestination(BB);
  return toolchain::BlockAddress::get(CurFn->getType(), BB);
}

toolchain::BasicBlock *CodeGenFunction::GetIndirectGotoBlock() {
  ...
  toolchain::Value *DestVal = TmpBuilder.CreatePHI(Int8PtrTy, 0,
                                              "indirect.goto.dest");
...
toolchain::Value *CodeGenFunction::emitArrayLength(const ArrayType *origArrayType,
                                              QualType &baseType,
                                              Address &addr) {
  ...
  toolchain::Value *numVLAElements = nullptr;
  ...
  SmallVector<toolchain::Value*, 8> gepIndices;

  // GEP down to the array type.
  toolchain::ConstantInt *zero = Builder.getInt32(0);
  gepIndices.push_back(zero);

  uint64_t countFromCLAs = 1;
  QualType eltType;

  toolchain::ArrayType *toolchainArrayType =
    dyn_cast<toolchain::ArrayType>(addr.getElementType());
  while (toolchainArrayType) {
    assert(isa<ConstantArrayType>(arrayType));
    assert(cast<ConstantArrayType>(arrayType)->getZExtSize() ==
           toolchainArrayType->getNumElements());

    gepIndices.push_back(zero);
    countFromCLAs *= toolchainArrayType->getNumElements();
    eltType = arrayType->getElementType();

    toolchainArrayType =
      dyn_cast<toolchain::ArrayType>(toolchainArrayType->getElementType());
    arrayType = getContext().getAsArrayType(arrayType->getElementType());
    assert((!toolchainArrayType || arrayType) &&
  ...
  if (arrayType) {
    ...
    toolchain::Type *baseType = ConvertType(eltType);
  ...
  toolchain::Value *numElements
    = toolchain::ConstantInt::get(SizeTy, countFromCLAs);
...
CodeGenFunction::getVLASize(const VariableArrayType *type) {
  ...
  toolchain::Value *numElements = nullptr;

  QualType elementType;
  do {
    elementType = type->getElementType();
    toolchain::Value *vlaSize = VLASizeMap[type->getSizeExpr()];
...
CodeGenFunction::getVLAElements1D(const VariableArrayType *Vla) {
  toolchain::Value *VlaSize = VLASizeMap[Vla->getSizeExpr()];
  assert(VlaSize && "no size for VLA!");
  assert(VlaSize->getType() == SizeTy);
...
#include "language/Core/AST/TypeNodes.inc"
      toolchain_unreachable("unexpected dependent type!");
    ...
    case Type::PredefinedSugar:
      toolchain_unreachable("type class is never variably-modified!");
    ...
    case Type::VariableArray: {
      ...
      if (const Expr *sizeExpr = vat->getSizeExpr()) {
        ...
        toolchain::Value *&entry = VLASizeMap[sizeExpr];
        if (!entry) {
          toolchain::Value *size = EmitScalarExpr(sizeExpr);
          ...
          if (SanOpts.has(SanitizerKind::VLABound)) {
            ...
            SanitizerDebugLocation SanScope(this, {CheckOrdinal}, CheckHandler);
            toolchain::Value *Zero = toolchain::Constant::getNullValue(size->getType());
            language::Core::QualType SEType = sizeExpr->getType();
            toolchain::Value *CheckCondition =
...
CodeGenFunction::protectFromPeepholes(RValue rvalue) {
  ...
  toolchain::Value *value = rvalue.getScalarVal();
  if (!isa<toolchain::ZExtInst>(value)) return PeepholeProtection();

  // Just make an extra bitcast.
  assert(HaveInsertPoint());
  toolchain::Instruction *inst = new toolchain::BitCastInst(value, value->getType(), "",
...
void CodeGenFunction::emitAlignmentAssumption(toolchain::Value *PtrValue,
  ...
  toolchain::Value *TheCheck = nullptr;
  if (SanOpts.has(SanitizerKind::Alignment)) {
    toolchain::Value *PtrIntValue =
        Builder.CreatePtrToInt(PtrValue, IntPtrTy, "ptrint");

    if (OffsetValue) {
      bool IsOffsetZero = false;
      if (const auto *CI = dyn_cast<toolchain::ConstantInt>(OffsetValue))
    ...
    toolchain::Value *Zero = toolchain::ConstantInt::get(IntPtrTy, 0);
    toolchain::Value *Mask =
        Builder.CreateSub(Alignment, toolchain::ConstantInt::get(IntPtrTy, 1));
    toolchain::Value *MaskedPtr = Builder.CreateAnd(PtrIntValue, Mask, "maskedptr");
    TheCheck = Builder.CreateICmpEQ(MaskedPtr, Zero, "maskcond");
  }
  toolchain::Instruction *Assumption = Builder.CreateAlignmentAssumption(
...
toolchain::Value *CodeGenFunction::EmitAnnotationCall(toolchain::Function *AnnotationFn,
...
void CodeGenFunction::EmitVarAnnotations(const VarDecl *D, toolchain::Value *V) {
  assert(D->hasAttr<AnnotateAttr>() && "no annotate attribute");
  for (const auto *I : D->specific_attrs<AnnotateAttr>())
    EmitAnnotationCall(CGM.getIntrinsic(toolchain::Intrinsic::var_annotation,
...
Address CodeGenFunction::EmitFieldAnnotations(const FieldDecl *D,
                                              Address Addr) {
  assert(D->hasAttr<AnnotateAttr>() && "no annotate attribute");
  toolchain::Value *V = Addr.emitRawPointer(*this);
  toolchain::Type *VTy = V->getType();
  auto *PTy = dyn_cast<toolchain::PointerType>(VTy);
  unsigned AS = PTy ? PTy->getAddressSpace() : 0;
  toolchain::PointerType *IntrinTy =
      toolchain::PointerType::get(CGM.getLLVMContext(), AS);
  toolchain::Function *F = CGM.getIntrinsic(toolchain::Intrinsic::ptr_annotation,
...
void CodeGenFunction::InsertHelper(toolchain::Instruction *I,
...
void CGBuilderInserter::InsertHelper(
    toolchain::Instruction *I, const toolchain::Twine &Name,
    toolchain::BasicBlock::iterator InsertPt) const {
  toolchain::IRBuilderDefaultInserter::InsertHelper(I, Name, InsertPt);
  if (CGF)
...
void CodeGenFunction::checkTargetFeatures(const CallExpr *E,
  ...
  if (CGM.getContext().getTargetInfo().getTriple().isX86()) {
    ...
    if (BuiltinID == X86::BI__builtin_ia32_cmpps ||
      ...
      toolchain::StringMap<bool> TargetFetureMap;
      CGM.getContext().getFunctionFeatureMap(TargetFetureMap, FD);
      toolchain::APSInt Result =
...
void CodeGenFunction::checkTargetFeatures(SourceLocation Loc,
  ...
  toolchain::StringMap<bool> CallerFeatureMap;
  ...
  if (BuiltinID) {
    ...
    toolchain::StringMap<bool> CalleeFeatureMap;
    ...
    if (!toolchain::all_of(ReqFeatures, [&](StringRef Feature) {
...
void CodeGenFunction::EmitSanitizerStatReport(toolchain::SanitizerStatKind SSK) {
...
void CodeGenFunction::EmitKCFIOperandBundle(
    const CGCallee &Callee, SmallVectorImpl<toolchain::OperandBundleDef> &Bundles) {
...
CodeGenFunction::FormX86ResolverCondition(const FMVResolverOption &RO) {
  ...
  if (!RO.Features.empty()) {
    toolchain::Value *FeatureCond = EmitX86CpuSupports(RO.Features);
    Condition =
...
static void CreateMultiVersionResolverReturn(CodeGenModule &CGM,
                                             toolchain::Function *Resolver,
                                             CGBuilderTy &Builder,
                                             toolchain::Function *FuncToReturn,
                                             bool SupportsIFunc) {
  if (SupportsIFunc) {
  ...
  toolchain::SmallVector<toolchain::Value *, 10> Args(
      toolchain::make_pointer_range(Resolver->args()));

  toolchain::CallInst *Result = Builder.CreateCall(FuncToReturn, Args);
  Result->setTailCallKind(toolchain::CallInst::TCK_MustTail);
...
void CodeGenFunction::EmitMultiVersionResolver(
    toolchain::Function *Resolver, ArrayRef<FMVResolverOption> Options) {

  toolchain::Triple::ArchType ArchType =
...
void CodeGenFunction::EmitRISCVMultiVersionResolver(
  ...
  if (getContext().getTargetInfo().getTriple().getOS() !=
      toolchain::Triple::OSType::Linux) {
    CGM.getDiags().Report(diag::err_os_unsupport_riscv_fmv);
    return;
  }

  toolchain::BasicBlock *CurBlock = createBasicBlock("resolver_entry", Resolver);
  ...
  for (unsigned Index = 0; Index < Options.size(); Index++) {
    ...
    toolchain::SmallVector<StringRef, 8> CurrTargetAttrFeats;
    toolchain::SmallVector<std::string, 8> TargetAttrFeats;
    ...
    Builder.SetInsertPoint(CurBlock);
    toolchain::Value *FeatsCondition = EmitRISCVCpuSupports(CurrTargetAttrFeats);

    toolchain::BasicBlock *RetBlock = createBasicBlock("resolver_return", Resolver);
    CGBuilderTy RetBuilder(*this, RetBlock);
    CreateMultiVersionResolverReturn(CGM, Resolver, RetBuilder,
                                     Options[Index].Function, SupportsIFunc);
    toolchain::BasicBlock *ElseBlock = createBasicBlock("resolver_else", Resolver);
  ...
  Builder.SetInsertPoint(CurBlock);
  toolchain::CallInst *TrapCall = EmitTrapCall(toolchain::Intrinsic::trap);
...
void CodeGenFunction::EmitAArch64MultiVersionResolver(
  ...
  for (const FMVResolverOption &RO : Options) {
    Builder.SetInsertPoint(CurBlock);
    toolchain::Value *Condition = FormAArch64ResolverCondition(RO);
...
void CodeGenFunction::EmitX86MultiVersionResolver(
  ...
  for (const FMVResolverOption &RO : Options) {
    Builder.SetInsertPoint(CurBlock);
    toolchain::Value *Condition = FormX86ResolverCondition(RO);
...
void CodeGenFunction::emitAlignmentAssumptionCheck(
    toolchain::Value *Ptr, QualType Ty, SourceLocation Loc,
    SourceLocation SecondaryLoc, toolchain::Value *Alignment,
    toolchain::Value *OffsetValue, toolchain::Value *TheCheck,
    toolchain::Instruction *Assumption) {
  assert(isa_and_nonnull<toolchain::CallInst>(Assumption) &&
         cast<toolchain::CallInst>(Assumption)->getCalledOperand() ==
             toolchain::Intrinsic::getOrInsertDeclaration(
                 Builder.GetInsertBlock()->getParent()->getParent(),
                 toolchain::Intrinsic::assume) &&
         "Assumption should be a call to toolchain.assume().");
...

  {
    ...
    toolchain::Constant *StaticData[] = {EmitCheckSourceLocation(Loc),
                                    EmitCheckSourceLocation(SecondaryLoc),
                                    EmitCheckTypeDescriptor(Ty)};
    toolchain::Value *DynamicData[] = {Ptr, Alignment, OffsetValue};
...
toolchain::DebugLoc CodeGenFunction::SourceLocToDebugLoc(SourceLocation Location) {
  if (CGDebugInfo *DI = getDebugInfo())
...
toolchain::Value *
CodeGenFunction::emitCondLikelihoodViaExpectIntrinsic(toolchain::Value *Cond,
                                                      Stmt::Likelihood LH) {
  switch (LH) {
  case Stmt::LH_None:
    return Cond;
  ...
  case Stmt::LH_Unlikely:
    ...
    if (CGM.getCodeGenOpts().OptimizationLevel == 0)
      return Cond;
    toolchain::Type *CondTy = Cond->getType();
    assert(CondTy->isIntegerTy(1) && "expecting condition to be a boolean");
    toolchain::Function *FnExpect =
        CGM.getIntrinsic(toolchain::Intrinsic::expect, CondTy);
    toolchain::Value *ExpectedValueOfCond =
        toolchain::ConstantInt::getBool(CondTy, LH == Stmt::LH_Likely);
    return Builder.CreateCall(FnExpect, {Cond, ExpectedValueOfCond},
                              Cond->getName() + ".expval");
  }
  toolchain_unreachable("Unknown Likelihood");
}

toolchain::Value *CodeGenFunction::emitBoolVecConversion(toolchain::Value *SrcVec,
                                                    unsigned NumElementsDst,
                                                    const toolchain::Twine &Name) {
  auto *SrcTy = cast<toolchain::FixedVectorType>(SrcVec->getType());
...
void CodeGenFunction::EmitPointerAuthOperandBundle(
  ...
  toolchain::Value *Discriminator = PointerAuth.getDiscriminator();
  if (!Discriminator)
    Discriminator = Builder.getSize(0);

  toolchain::Value *Args[] = {Key, Discriminator};
  Bundles.emplace_back("ptrauth", Args);
}

static toolchain::Value *EmitPointerAuthCommon(CodeGenFunction &CGF,
                                          const CGPointerAuthInfo &PointerAuth,
  ...
  // call i64 @toolchain.ptrauth.sign.i64(i64 %pointer, i32 %key, i64 %discriminator)
...
CodeGenFunction::EmitPointerAuthSign(const CGPointerAuthInfo &PointerAuth,
  ...
  return EmitPointerAuthCommon(*this, PointerAuth, Pointer,
                               toolchain::Intrinsic::ptrauth_sign);
}

static toolchain::Value *EmitStrip(CodeGenFunction &CGF,
                              const CGPointerAuthInfo &PointerAuth,
                              toolchain::Value *Pointer) {
  auto StripIntrinsic = CGF.CGM.getIntrinsic(toolchain::Intrinsic::ptrauth_strip);
...
CodeGenFunction::EmitPointerAuthAuth(const CGPointerAuthInfo &PointerAuth,
  ...
  return EmitPointerAuthCommon(*this, PointerAuth, Pointer,
                               toolchain::Intrinsic::ptrauth_auth);
}

void CodeGenFunction::addInstToCurrentSourceAtom(
    toolchain::Instruction *KeyInstruction, toolchain::Value *Backup) {
  if (CGDebugInfo *DI = getDebugInfo())
    DI->addInstToCurrentSourceAtom(KeyInstruction, Backup);
}

void CodeGenFunction::addInstToSpecificSourceAtom(
    toolchain::Instruction *KeyInstruction, toolchain::Value *Backup, uint64_t Atom) {
  if (CGDebugInfo *DI = getDebugInfo())
    DI->addInstToSpecificSourceAtom(KeyInstruction, Backup, Atom);
}

void CodeGenFunction::addInstToNewSourceAtom(toolchain::Instruction *KeyInstruction,
