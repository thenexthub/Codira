...
#ifndef LANGUAGE_CORE_LIB_CODEGEN_CGCLEANUP_H
#define LANGUAGE_CORE_LIB_CODEGEN_CGCLEANUP_H
...
private:
  toolchain::BasicBlock *CachedLandingPad;
  toolchain::BasicBlock *CachedEHDispatchBlock;
...
public:
  ...
  toolchain::BasicBlock *getCachedLandingPad() const {
    return CachedLandingPad;
  }

  void setCachedLandingPad(toolchain::BasicBlock *block) {
    CachedLandingPad = block;
  }

  toolchain::BasicBlock *getCachedEHDispatchBlock() const {
    return CachedEHDispatchBlock;
  }

  void setCachedEHDispatchBlock(toolchain::BasicBlock *block) {
    CachedEHDispatchBlock = block;
  }

  bool hasEHBranches() const {
    if (toolchain::BasicBlock *block = getCachedEHDispatchBlock())
...
public:
  ...
  void setCatchAllHandler(unsigned I, toolchain::BasicBlock *Block) {
    setHandler(I, CatchTypeInfo{nullptr, 0}, Block);
  }

  void setHandler(unsigned I, toolchain::Constant *Type, toolchain::BasicBlock *Block) {
    assert(I < getNumHandlers());
    getHandlers()[I].Type = CatchTypeInfo{Type, 0};
    getHandlers()[I].Block = Block;
  }

  void setHandler(unsigned I, CatchTypeInfo Type, toolchain::BasicBlock *Block) {
    assert(I < getNumHandlers());
...
class alignas(8) EHCleanupScope : public EHScope {
  ...
  toolchain::BasicBlock *NormalBlock;
  ...
  struct ExtInfo {
    ...
    toolchain::SmallPtrSet<toolchain::BasicBlock*, 4> Branches;

    /// Normal branch-afters.
    SmallVector<std::pair<toolchain::BasicBlock*,toolchain::ConstantInt*>, 4>
      BranchAfters;
  };
  ...
  struct AuxillaryAllocas {
    SmallVector<toolchain::Instruction *, 1> AuxAllocas;
    bool used = false;

    // Records a potentially unused instruction to be erased later.
    void Add(toolchain::AllocaInst *Alloca) { AuxAllocas.push_back(Alloca); }
    ...
    ~AuxillaryAllocas() {
      if (used)
        return;
      toolchain::SetVector<toolchain::Instruction *> Uses;
      for (auto *Inst : toolchain::reverse(AuxAllocas))
        CollectUses(Inst, Uses);
      // Delete uses in the reverse order of insertion.
      for (auto *I : toolchain::reverse(Uses))
        I->eraseFromParent();
    }

  private:
    void CollectUses(toolchain::Instruction *I,
                     toolchain::SetVector<toolchain::Instruction *> &Uses) {
      if (!I || !Uses.insert(I))
        return;
      for (auto *User : I->users())
        CollectUses(cast<toolchain::Instruction>(User), Uses);
    }
  };
...
public:
  ...
  void AddAuxAllocas(toolchain::SmallVector<toolchain::AllocaInst *> Allocas) {
  ...
  toolchain::BasicBlock *getNormalBlock() const { return NormalBlock; }
  void setNormalBlock(toolchain::BasicBlock *BB) { NormalBlock = BB; }
  ...
  /// cleanup.
  void addBranchAfter(toolchain::ConstantInt *Index,
                      toolchain::BasicBlock *Block) {
  ...
  toolchain::BasicBlock *getBranchAfterBlock(unsigned I) const {
    assert(I < getNumBranchAfters());
    return ExtInfo->BranchAfters[I].first;
  }

  toolchain::ConstantInt *getBranchAfterIndex(unsigned I) const {
    assert(I < getNumBranchAfters());
  ...
  bool addBranchThrough(toolchain::BasicBlock *Block) {
...
class EHFilterScope : public EHScope {
  ...
  // toolchain::Value *FilterTypes[0];

  toolchain::Value **getFilters() {
    return reinterpret_cast<toolchain::Value**>(this+1);
  }

  toolchain::Value * const *getFilters() const {
    return reinterpret_cast<toolchain::Value* const *>(this+1);
  }
...
public:
  ...
  static size_t getSizeForNumFilters(unsigned numFilters) {
    return sizeof(EHFilterScope) + numFilters * sizeof(toolchain::Value*);
  }

  unsigned getNumFilters() const { return FilterBits.NumFilters; }

  void setFilter(unsigned i, toolchain::Value *filterValue) {
    assert(i < getNumFilters());
    getFilters()[i] = filterValue;
  }

  toolchain::Value *getFilter(unsigned i) const {
    assert(i < getNumFilters());
    return getFilters()[i];
  }
...
public:
  ...
  iterator &operator++() {
    ...
    Ptr += toolchain::alignTo(Size, ScopeStackAlignment);
    return *this;
  }
