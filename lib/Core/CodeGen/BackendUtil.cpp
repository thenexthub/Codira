...
#include "toolchain/Analysis/GlobalsModRef.h"
...
#include "toolchain/Transforms/HipStdPar/HipStdPar.h"
#include "toolchain/Transforms/IPO/EmbedBitcodePass.h"
#include "toolchain/Transforms/IPO/LowerTypeTests.h"
...
#include "toolchain/Transforms/Instrumentation/BoundsChecking.h"
#include "toolchain/Transforms/Instrumentation/DataFlowSanitizer.h"
#include "toolchain/Transforms/Instrumentation/GCOVProfiler.h"
#include "toolchain/Transforms/Instrumentation/HWAddressSanitizer.h"
#include "toolchain/Transforms/Instrumentation/InstrProfiling.h"
#include "toolchain/Transforms/Instrumentation/KCFI.h"
#include "toolchain/Transforms/Instrumentation/LowerAllowCheckPass.h"
#include "toolchain/Transforms/Instrumentation/MemProfInstrumentation.h"
#include "toolchain/Transforms/Instrumentation/MemProfUse.h"
#include "toolchain/Transforms/Instrumentation/MemorySanitizer.h"
#include "toolchain/Transforms/Instrumentation/NumericalStabilitySanitizer.h"
#include "toolchain/Transforms/Instrumentation/PGOInstrumentation.h"
#include "toolchain/Transforms/Instrumentation/RealtimeSanitizer.h"
#include "toolchain/Transforms/Instrumentation/SanitizerBinaryMetadata.h"
...
#include "toolchain/Transforms/Instrumentation/TypeSanitizer.h"
...
#include "toolchain/Transforms/Scalar/JumpThreading.h"
#include "toolchain/Transforms/Utils/Debugify.h"
...
extern toolchain::cl::opt<bool> ClSanitizeGuardChecks;
}
...
class EmitAssemblyHelper {
  ...
  const language::Core::TargetOptions &TargetOpts;
  ...
  std::unique_ptr<toolchain::ToolOutputFile> openOutputFile(StringRef Path) {
    std::error_code EC;
    auto F = std::make_unique<toolchain::ToolOutputFile>(Path, EC,
  ...
  void RunOptimizationPipeline(
      ...
      std::unique_ptr<toolchain::ToolOutputFile> &ThinLinkOS, BackendConsumer *BC);
  void RunCodegenPipeline(BackendAction Action,
                          std::unique_ptr<raw_pwrite_stream> &OS,
                          std::unique_ptr<toolchain::ToolOutputFile> &DwoOS);
  ...
  bool shouldEmitRegularLTOSummary() const {
    return CodeGenOpts.PrepareForLTO && !CodeGenOpts.DisableLLVMPasses &&
           TargetTriple.getVendor() != toolchain::Triple::Apple;
  }
...
static bool asanUseGlobalsGC(const Triple &T, const CodeGenOptions &CGOpts) {
  ...
  case Triple::GOFF:
    toolchain::report_fatal_error("ASan not implemented for GOFF");
  case Triple::XCOFF:
    toolchain::report_fatal_error("ASan not implemented for XCOFF.");
  case Triple::Wasm:
...
static std::optional<toolchain::CodeModel::Model>
getCodeModel(const CodeGenOptions &CodeGenOpts) {
  unsigned CodeModel = toolchain::StringSwitch<unsigned>(CodeGenOpts.CodeModel)
  ...
  if (CodeModel == ~1u)
    return std::nullopt;
  return static_cast<toolchain::CodeModel::Model>(CodeModel);
}
...
static std::string flattenClangCommandLine(ArrayRef<std::string> Args,
  ...
  if (!StringRef(Args[0]).contains("-cc1")) {
    toolchain::sys::printArg(OS, "-cc1", /*Quote=*/true);
    PrintedOneArg = true;
  }
...
static bool initTargetOptions(const CompilerInstance &CI,
  ...
  case LangOptions::ThreadModelKind::POSIX:
    Options.ThreadModel = toolchain::ThreadModel::POSIX;
    break;
  case LangOptions::ThreadModelKind::Single:
    Options.ThreadModel = toolchain::ThreadModel::Single;
    break;
  }
  ...
  Options.FloatABIType =
      toolchain::StringSwitch<toolchain::FloatABI::ABIType>(CodeGenOpts.FloatABI)
          .Case("soft", toolchain::FloatABI::Soft)
          .Case("softfp", toolchain::FloatABI::Soft)
          .Case("hard", toolchain::FloatABI::Hard)
          .Default(toolchain::FloatABI::Default);
  ...
  case LangOptions::FPM_Off:
    ...
    Options.AllowFPOpFusion = toolchain::FPOpFusion::Standard;
    break;
  ...
  case LangOptions::FPM_Fast:
    Options.AllowFPOpFusion = toolchain::FPOpFusion::Fast;
    break;
  }

  Options.BinutilsVersion =
      toolchain::TargetMachine::parseBinutilsVersion(CodeGenOpts.BinutilsVersion);
  ...
  if (CodeGenOpts.hasSjLjExceptions())
    Options.ExceptionModel = toolchain::ExceptionHandling::SjLj;
  if (CodeGenOpts.hasSEHExceptions())
    Options.ExceptionModel = toolchain::ExceptionHandling::WinEH;
  if (CodeGenOpts.hasDWARFExceptions())
    Options.ExceptionModel = toolchain::ExceptionHandling::DwarfCFI;
  if (CodeGenOpts.hasWasmExceptions())
    Options.ExceptionModel = toolchain::ExceptionHandling::Wasm;
  ...
  Options.BBSections =
      toolchain::StringSwitch<toolchain::BasicBlockSection>(CodeGenOpts.BBSections)
          .Case("all", toolchain::BasicBlockSection::All)
          .StartsWith("list=", toolchain::BasicBlockSection::List)
          .Case("none", toolchain::BasicBlockSection::None)
          .Default(toolchain::BasicBlockSection::None);

  if (Options.BBSections == toolchain::BasicBlockSection::List) {
  ...
  Options.MCOptions.MCUseDwarfDirectory =
      CodeGenOpts.NoDwarfDirectoryAsm
          ? toolchain::MCTargetOptions::DisableDwarfDirectory
          : toolchain::MCTargetOptions::EnableDwarfDirectory;
...
getGCOVOptions(const CodeGenOptions &CodeGenOpts, const LangOptions &LangOpts) {
  ...
  toolchain::copy(CodeGenOpts.CoverageVersion, std::begin(Options.Version));
...
static void setCommandLineOpts(const CodeGenOptions &CodeGenOpts) {
  ...
  toolchain::cl::ParseCommandLineOptions(BackendArgs.size() - 1,
                                    BackendArgs.data());
}
...
void EmitAssemblyHelper::CreateTargetMachine(bool MustCreateTM) {
  ...
  std::string Error;
  const toolchain::Triple &Triple = TheModule->getTargetTriple();
  ...
  std::optional<toolchain::CodeModel::Model> CM = getCodeModel(CodeGenOpts);
  std::string FeaturesStr =
      toolchain::join(TargetOpts.Features.begin(), TargetOpts.Features.end(), ",");
  toolchain::Reloc::Model RM = CodeGenOpts.RelocationModel;
...
bool EmitAssemblyHelper::AddEmitPasses(legacy::PassManager &CodeGenPasses,
  ...
  std::unique_ptr<TargetLibraryInfoImpl> TLII(
      toolchain::driver::createTLII(TargetTriple, CodeGenOpts.getVecLib()));
...
static OptimizationLevel mapToLevel(const CodeGenOptions &Opts) {
  ...
  case 2:
    ...
    default:
      toolchain_unreachable("Invalid optimization level for size!");
...
static void addKCFIPass(const Triple &TargetTriple, const LangOptions &LangOpts,
  ...
  if (TargetTriple.getArch() == toolchain::Triple::x86_64 ||
  ...
  auto SanitizersCallback = [&](ModulePassManager &MPM, OptimizationLevel Level,
    ...
    auto MSanPass = [&](SanitizerMask Mask, bool CompileKernel) {
      if (LangOpts.Sanitize.has(Mask)) {
        ...
        if (Level != OptimizationLevel::O0) {
          ...
          // passes.
          MPM.addPass(RequireAnalysisPass<GlobalsAA, toolchain::Module>());
    ...
    auto ASanPass = [&](SanitizerMask Mask, bool CompileKernel) {
      if (LangOpts.Sanitize.has(Mask)) {
        ...
        toolchain::AsanDtorKind DestructorKind =
    ...
    PB.registerOptimizerEarlyEPCallback(
        [SanitizersCallback](ModulePassManager &MPM, OptimizationLevel Level,
          ...
          if (!NewMPM.isEmpty()) {
            ...
            NewMPM.addPass(RequireAnalysisPass<GlobalsAA, toolchain::Module>());
...
void EmitAssemblyHelper::RunOptimizationPipeline(
  ...
  case CodeGenOptions::AssignmentTrackingOpts::Enabled:
    ...
    // cost is too high. Disable for LLDB tuning due to toolchain.org/PR43126.
    if (!CodeGenOpts.PrepareForThinLTO && !CodeGenOpts.PrepareForLTO &&
        CodeGenOpts.getDebuggerTuning() != toolchain::DebuggerKind::LLDB) {
      PB.registerPipelineStartEPCallback(
          [&](ModulePassManager &MPM, OptimizationLevel Level) {
...
#include "toolchain/Support/Extension.def"
      ...
      PB.registerScalarOptimizerLateEPCallback([this](FunctionPassManager &FPM,
        ...
        if (CodeGenOpts.SanitizeSkipHotCutoffs[SanitizerKind::SO_LocalBounds] ||
          ...
          static_assert(SanitizerKind::SO_LocalBounds <=
                        ...
                        "Update type of toolchain.allow.ubsan.check to represent "
  ...
  if (Action == Backend_EmitBC || Action == Backend_EmitLL ||
    ...
    if (CodeGenOpts.PrepareForThinLTO && !CodeGenOpts.DisableLLVMPasses) {
      if (!TheModule->getModuleFlag("EnableSplitLTOUnit"))
        TheModule->addModuleFlag(toolchain::Module::Error, "EnableSplitLTOUnit",
      ...
      if (EmitLTOSummary) {
        if (!TheModule->getModuleFlag("ThinLTO") && !CodeGenOpts.UnifiedLTO)
          TheModule->addModuleFlag(toolchain::Module::Error, "ThinLTO", uint32_t(0));
        if (!TheModule->getModuleFlag("EnableSplitLTOUnit"))
          TheModule->addModuleFlag(toolchain::Module::Error, "EnableSplitLTOUnit",
                                   uint32_t(1));
      }
    ...
    if (shouldEmitUnifiedLTOModueFlag())
      TheModule->addModuleFlag(toolchain::Module::Error, "UnifiedLTO", uint32_t(1));
  }
  ...
  // Now that we have all of the passes ready, run them.
  {
    ...
    toolchain::TimeTraceScope TimeScope("Optimizer");
    Timer timer;
...
void EmitAssemblyHelper::RunCodegenPipeline(
...

  {
    ...
    toolchain::TimeTraceScope TimeScope("CodeGenPasses");
    Timer timer;
    if (CI.getCodeGenOpts().TimePasses) {
...
void EmitAssemblyHelper::emitAssembly(BackendAction Action,
  ...
  std::unique_ptr<toolchain::ToolOutputFile> ThinLinkOS, DwoOS;
...
runThinLTOBackend(CompilerInstance &CI, ModuleSummaryIndex *CombinedIndex,
                  toolchain::Module *M, std::unique_ptr<raw_pwrite_stream> OS,
  ...
  case Backend_EmitNothing:
    Conf.PreCodeGenModuleHook = [](size_t Task, const toolchain::Module &Mod) {
      return false;
    };
    break;
  case Backend_EmitLL:
...
void language::Core::emitBackendOutput(CompilerInstance &CI, CodeGenOptions &CGOpts,
                              StringRef TDesc, toolchain::Module *M,
                              BackendAction Action,
                              IntrusiveRefCntPtr<toolchain::vfs::FileSystem> VFS,
                              std::unique_ptr<raw_pwrite_stream> OS,
                              BackendConsumer *BC) {
  toolchain::TimeTraceScope TimeScope("Backend");
  DiagnosticsEngine &Diags = CI.getDiagnostics();

  std::unique_ptr<toolchain::Module> EmptyModule;
  if (!CGOpts.ThinLTOIndexFile.empty()) {
    // If we are performing a ThinLTO importing compile, load the function index
    // into memory and pass it into runThinLTOBackend, which will run the
    // function importer and invoke LTO passes.
    std::unique_ptr<ModuleSummaryIndex> CombinedIndex;
    if (Error E = toolchain::getModuleSummaryIndexForFile(
    ...
    if (CombinedIndex) {
      ...
      // information.
      EmptyModule = std::make_unique<toolchain::Module>("empty", M->getContext());
...
// With -fembed-bitcode, save a copy of the toolchain IR as data in the
// __LLVM,__bitcode section.
void language::Core::EmbedBitcode(toolchain::Module *M, const CodeGenOptions &CGOpts,
                         toolchain::MemoryBufferRef Buf) {
  if (CGOpts.getEmbedBitcode() == CodeGenOptions::Embed_Off)
    return;
  toolchain::embedBitcodeInModule(
      *M, Buf, CGOpts.getEmbedBitcode() != CodeGenOptions::Embed_Marker,
      CGOpts.getEmbedBitcode() != CodeGenOptions::Embed_Bitcode,
      CGOpts.CmdArgs);
}

void language::Core::EmbedObject(toolchain::Module *M, const CodeGenOptions &CGOpts,
                        DiagnosticsEngine &Diags) {
  ...
  for (StringRef OffloadObject : CGOpts.OffloadObjects) {
    ...
    toolchain::embedBufferInModule(*M, **ObjectOrErr, ".toolchain.offloading",
