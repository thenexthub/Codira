...
RValue DominatingValue<RValue>::saved_type::restore(CodeGenFunction &CGF) {
  ...
  case ComplexAddress: {
    toolchain::Value *real = DominatingLLVMValue::restore(CGF, Vals.first);
    toolchain::Value *imag = DominatingLLVMValue::restore(CGF, Vals.second);
    return RValue::getComplex(real, imag);
  }
  }

  toolchain_unreachable("bad saved r-value kind");
}

/// Push an entry of the given size onto this protected-scope stack.
char *EHScopeStack::allocate(size_t Size) {
  Size = toolchain::alignTo(Size, ScopeStackAlignment);
  if (!StartOfBuffer) {
...
void EHScopeStack::deallocate(size_t Size) {
  StartOfData += toolchain::alignTo(Size, ScopeStackAlignment);
}
...
void *EHScopeStack::pushCleanup(CleanupKind Kind, size_t Size) {
  ...
  // With Windows -EHa, Invoke toolchain.seh.scope.begin() for EHCleanup
  // If exceptions are disabled/ignored and SEH is not in use, then there is no
  // invoke destination. SEH "works" even if exceptions are off. In practice,
  // this means that C++ destructors and other EH cleanups don't run, which is
  // consistent with MSVC's behavior, except in the presence of -EHa.
  // Check getInvokeDest() to generate toolchain.seh.scope.begin() as needed.
...
static void createStoreInstBefore(toolchain::Value *value, Address addr,
                                  toolchain::BasicBlock::iterator beforeInst,
                                  CodeGenFunction &CGF) {
  auto store = new toolchain::StoreInst(value, addr.emitRawPointer(CGF), beforeInst);
...
createLoadInstBefore(Address addr, const Twine &name,
                     toolchain::BasicBlock::iterator beforeInst,
                     CodeGenFunction &CGF) {
  return new toolchain::LoadInst(addr.getElementType(), addr.emitRawPointer(CGF),
                            name, false, addr.getAlignment().getAsAlign(),
                            beforeInst);
}

static toolchain::LoadInst *createLoadInstBefore(Address addr, const Twine &name,
                                            CodeGenFunction &CGF) {
  return new toolchain::LoadInst(addr.getElementType(), addr.emitRawPointer(CGF),
                            name, false, addr.getAlignment().getAsAlign());
}
...
static void ResolveAllBranchFixups(CodeGenFunction &CGF,
                                   toolchain::SwitchInst *Switch,
                                   toolchain::BasicBlock *CleanupEntry) {
  toolchain::SmallPtrSet<toolchain::BasicBlock*, 4> CasesAdded;
...
/// be a cleanup switch.
static toolchain::SwitchInst *TransitionToCleanupSwitch(CodeGenFunction &CGF,
                                                   toolchain::BasicBlock *Block) {
  // If it's a branch, turn it into a switch whose default
  // destination is its original target.
  toolchain::Instruction *Term = Block->getTerminator();
  assert(Term && "can't transition block without terminator");

  if (toolchain::BranchInst *Br = dyn_cast<toolchain::BranchInst>(Term)) {
    assert(Br->isUnconditional());
    auto Load = createLoadInstBefore(CGF.getNormalCleanupDestSlot(),
                                     "cleanup.dest", Term->getIterator(), CGF);
    toolchain::SwitchInst *Switch =
      toolchain::SwitchInst::Create(Load, Br->getSuccessor(0), 4, Block);
    Br->eraseFromParent();
    return Switch;
  } else {
    return cast<toolchain::SwitchInst>(Term);
  }
}

void CodeGenFunction::ResolveBranchFixups(toolchain::BasicBlock *Block) {
  ...
  toolchain::SmallPtrSet<toolchain::BasicBlock*, 4> ModifiedOptimisticBlocks;
  ...
  for (unsigned I = 0, E = EHStack.getNumBranchFixups(); I != E; ++I) {
    ...
    toolchain::BasicBlock *BranchBB = Fixup.OptimisticBranchBlock;
    if (!BranchBB)
      continue;
    ...
    toolchain::SwitchInst *Switch = TransitionToCleanupSwitch(*this, BranchBB);
...
void CodeGenFunction::PopCleanupBlocks(
  ...
  // insertion point.
  for (toolchain::Value **ReloadedValue : ValuesToReload) {
    auto *Inst = dyn_cast_or_null<toolchain::Instruction>(*ReloadedValue);
    if (!Inst)
      continue;
    ...
    auto *AI = dyn_cast<toolchain::AllocaInst>(Inst);
    ...
    toolchain::BasicBlock::iterator InsertBefore;
    if (auto *Invoke = dyn_cast<toolchain::InvokeInst>(Inst))
...
static toolchain::BasicBlock *CreateNormalEntry(CodeGenFunction &CGF,
                                           EHCleanupScope &Scope) {
  assert(Scope.isNormalCleanup());
  toolchain::BasicBlock *Entry = Scope.getNormalBlock();
  if (!Entry) {
...
/// is basically toolchain::MergeBlockIntoPredecessor, except
/// simplified/optimized for the tighter constraints on cleanup blocks.
///
/// Returns the new block, whatever it is.
static toolchain::BasicBlock *SimplifyCleanupEntry(CodeGenFunction &CGF,
                                              toolchain::BasicBlock *Entry) {
  toolchain::BasicBlock *Pred = Entry->getSinglePredecessor();
  if (!Pred) return Entry;

  toolchain::BranchInst *Br = dyn_cast<toolchain::BranchInst>(Pred->getTerminator());
...
static void EmitCleanup(CodeGenFunction &CGF,
  ...
  if (ActiveFlag.isValid()) {
    ...
    toolchain::BasicBlock *CleanupBB = CGF.createBasicBlock("cleanup.action");
    toolchain::Value *IsActive
...
static void ForwardPrebranchedFallthrough(toolchain::BasicBlock *Exit,
                                          toolchain::BasicBlock *From,
                                          toolchain::BasicBlock *To) {
  // Exit is the exit block of a cleanup, so it always terminates in
  // an unconditional branch or a switch.
  toolchain::Instruction *Term = Exit->getTerminator();

  if (toolchain::BranchInst *Br = dyn_cast<toolchain::BranchInst>(Term)) {
    assert(Br->isUnconditional() && Br->getSuccessor(0) == From);
    Br->setSuccessor(0, To);
  } else {
    toolchain::SwitchInst *Switch = cast<toolchain::SwitchInst>(Term);
...
static void destroyOptimisticNormalEntry(CodeGenFunction &CGF,
  ...
  toolchain::BasicBlock *unreachableBB = CGF.getUnreachableBlock();
  for (toolchain::BasicBlock::use_iterator
    ...
    toolchain::SwitchInst *si = cast<toolchain::SwitchInst>(use.getUser());
    if (si->getNumCases() == 1 && si->getDefaultDest() == unreachableBB) {
      // Replace the switch with a branch.
      toolchain::BranchInst::Create(si->case_begin()->getCaseSuccessor(),
                               si->getIterator());

      // The switch operand is a load from the cleanup-dest alloca.
      toolchain::LoadInst *condition = cast<toolchain::LoadInst>(si->getCondition());
...
void CodeGenFunction::PopCleanupBlock(bool FallthroughIsBranchThrough,
  ...
  toolchain::BasicBlock *EHEntry = Scope.getCachedEHDispatchBlock();
  ...
  toolchain::BasicBlock *FallthroughSource = Builder.GetInsertBlock();
  bool HasFallthrough =
  ...
  if (Scope.isNormalCleanup() && HasPrebranchedFallthrough &&
    ...
    assert(!IsActive);
    toolchain::BasicBlock *prebranchDest;
    ...
    toolchain::BasicBlock *normalEntry = Scope.getNormalBlock();
  ...
  if (!RequiresNormalCleanup) {
    ...
    // the cleanup block and then try to clean up after ourselves.
      ...
      toolchain::BasicBlock *NormalEntry = CreateNormalEntry(*this, Scope);
      ...
      toolchain::BasicBlock *BranchThroughDest = nullptr;
      ...
      toolchain::BasicBlock *FallthroughDest = nullptr;
      SmallVector<toolchain::Instruction*, 2> InstsToAppend;
      ...
      if (!Scope.hasBranchThroughs() && !HasFixups && !HasFallthrough &&
        ...
        toolchain::Instruction *NormalCleanupDestSlot =
            cast<toolchain::Instruction>(getNormalCleanupDestSlot().getPointer());
        ...
        toolchain::BasicBlock *BranchAfter = Scope.getBranchAfterBlock(0);
        InstsToAppend.push_back(toolchain::BranchInst::Create(BranchAfter));
      ...
      //     so must be immediately resolved
        ...
        toolchain::LoadInst *Load = createLoadInstBefore(getNormalCleanupDestSlot(),
                                                    "cleanup.dest", *this);
        toolchain::SwitchInst *Switch =
          toolchain::SwitchInst::Create(Load, Default, SwitchCapacity);
        ...
        InstsToAppend.push_back(toolchain::BranchInst::Create(BranchThroughDest));
      }
      ...
      toolchain::BasicBlock *NormalExit = Builder.GetInsertBlock();
      for (toolchain::Instruction *Inst : InstsToAppend)
      ...
      toolchain::BasicBlock *NewNormalEntry =
  ...
  if (RequiresEHCleanup) {
    ...
    toolchain::BasicBlock *NextAction = getEHDispatchBlock(EHParent);
    ...
    toolchain::CleanupPadInst *CPI = nullptr;
...
void CodeGenFunction::EmitBranchThroughCleanup(JumpDest Dest) {
  ...
  // Create the branch.
  toolchain::BranchInst *BI = Builder.CreateBr(Dest.getBlock());
  ...
  toolchain::ConstantInt *Index = Builder.getInt32(Dest.getDestIndex());
...
static void SetupCleanupBlockActivation(CodeGenFunction &CGF,
  ...
  if (!var.isValid()) {
    ...
    toolchain::Constant *value = CGF.Builder.getInt1(kind == ForDeactivation);
...
static void EmitSehScope(CodeGenFunction &CGF,
                         toolchain::FunctionCallee &SehCppScope) {
  toolchain::BasicBlock *InvokeDest = CGF.getInvokeDest();
  assert(CGF.Builder.GetInsertBlock() && InvokeDest);
  toolchain::BasicBlock *Cont = CGF.createBasicBlock("invoke.cont");
...
// Invoke a toolchain.seh.scope.begin at the beginning of a CPP scope for -EHa
void CodeGenFunction::EmitSehCppScopeBegin() {
  ...
  toolchain::FunctionCallee SehCppScope =
      CGM.CreateRuntimeFunction(FTy, "toolchain.seh.scope.begin");
  EmitSehScope(*this, SehCppScope);
}

// Invoke a toolchain.seh.scope.end at the end of a CPP scope for -EHa
//   toolchain.seh.scope.end is emitted before popCleanup, so it's "invoked"
void CodeGenFunction::EmitSehCppScopeEnd() {
  assert(getLangOpts().EHAsynch);
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg=*/false);
  toolchain::FunctionCallee SehCppScope =
      CGM.CreateRuntimeFunction(FTy, "toolchain.seh.scope.end");
  EmitSehScope(*this, SehCppScope);
}

// Invoke a toolchain.seh.try.begin at the beginning of a SEH scope for -EHa
void CodeGenFunction::EmitSehTryScopeBegin() {
  assert(getLangOpts().EHAsynch);
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg=*/false);
  toolchain::FunctionCallee SehCppScope =
      CGM.CreateRuntimeFunction(FTy, "toolchain.seh.try.begin");
  EmitSehScope(*this, SehCppScope);
}

// Invoke a toolchain.seh.try.end at the end of a SEH scope for -EHa
void CodeGenFunction::EmitSehTryScopeEnd() {
  assert(getLangOpts().EHAsynch);
  toolchain::FunctionType *FTy =
      toolchain::FunctionType::get(CGM.VoidTy, /*isVarArg=*/false);
  toolchain::FunctionCallee SehCppScope =
      CGM.CreateRuntimeFunction(FTy, "toolchain.seh.try.end");
  EmitSehScope(*this, SehCppScope);
}
