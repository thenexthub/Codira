...
CodeGenTBAA::CodeGenTBAA(ASTContext &Ctx, CodeGenTypes &CGTypes,
                         toolchain::Module &M, const CodeGenOptions &CGO,
...
toolchain::MDNode *CodeGenTBAA::getRoot() {
...
toolchain::MDNode *CodeGenTBAA::createScalarTypeNode(StringRef Name,
                                                toolchain::MDNode *Parent,
                                                uint64_t Size) {
  if (CodeGenOpts.NewStructPathTBAA) {
    toolchain::Metadata *Id = MDHelper.createString(Name);
    return MDHelper.createTBAATypeNode(Parent, Size, Id);
  }
  return MDHelper.createTBAAScalarTypeNode(Name, Parent);
}

toolchain::MDNode *CodeGenTBAA::getChar() {
...
toolchain::MDNode *CodeGenTBAA::getAnyPtr(unsigned PtrDepth) {
  ...
  if (AnyPtrs.size() < PtrDepth) {
    ...
    for (size_t Idx = AnyPtrs.size(); Idx < PtrDepth; ++Idx) {
      auto Name = ("any p" + toolchain::Twine(Idx + 1) + " pointer").str();
...
toolchain::MDNode *CodeGenTBAA::getTypeInfoHelper(const Type *Ty) {
  ...
  if (Ty->isPointerType() || Ty->isReferenceType()) {
    ...
    if (isa<BuiltinType>(Ty)) {
      toolchain::MDNode *ScalarMD = getTypeInfoHelper(Ty);
      StringRef Name =
          cast<toolchain::MDString>(
      ...
      toolchain::raw_svector_ostream TyOut(TyName);
...
toolchain::MDNode *CodeGenTBAA::getTypeInfo(QualType QTy) {
  ...
  if (toolchain::MDNode *N = MetadataCache[Ty])
    return N;

  // Note that the following helper call is allowed to add new nodes to the
  // cache, which invalidates all its previously obtained iterators. So we
  // first generate the node for the type and then add that node to the cache.
  toolchain::MDNode *TypeNode = getTypeInfoHelper(Ty);
...
TBAAAccessInfo CodeGenTBAA::getVTablePtrAccessInfo(toolchain::Type *VTablePtrType) {
  const toolchain::DataLayout &DL = Module.getDataLayout();
...
CodeGenTBAA::CollectFields(uint64_t BaseOffset,
                           QualType QTy,
                           SmallVectorImpl<toolchain::MDBuilder::TBAAStructField> &
                             Fields,
                           bool MayAlias) {
  ...
  if (const RecordType *TTy = QTy->getAs<RecordType>()) {
    if (TTy->isUnionType()) {
      ...
      toolchain::MDNode *TBAAType = getChar();
      toolchain::MDNode *TBAATag = getAccessTagInfo(TBAAAccessInfo(TBAAType, Size));
      Fields.push_back(
          toolchain::MDBuilder::TBAAStructField(BaseOffset, Size, TBAATag));
      return true;
    }
    const RecordDecl *RD = TTy->getOriginalDecl()->getDefinition();
    if (RD->hasFlexibleArrayMember())
    ...
    for (RecordDecl::field_iterator i = RD->field_begin(), e = RD->field_end();
      ...
      if ((*i)->isBitField()) {
        ...
        uint64_t Size =
            toolchain::divideCeil(CurrentBitFieldSize, Context.getCharWidth());
        toolchain::MDNode *TBAAType = getChar();
        toolchain::MDNode *TBAATag =
            getAccessTagInfo(TBAAAccessInfo(TBAAType, Size));
        Fields.push_back(
            toolchain::MDBuilder::TBAAStructField(Offset, Size, TBAATag));
        continue;
      }
  ...
  uint64_t Size = Context.getTypeSizeInChars(QTy).getQuantity();
  toolchain::MDNode *TBAAType = MayAlias ? getChar() : getTypeInfo(QTy);
  toolchain::MDNode *TBAATag = getAccessTagInfo(TBAAAccessInfo(TBAAType, Size));
  Fields.push_back(toolchain::MDBuilder::TBAAStructField(Offset, Size, TBAATag));
  return true;
}
...
CodeGenTBAA::getTBAAStructInfo(QualType QTy) {
  ...
  if (toolchain::MDNode *N = StructMetadataCache[Ty])
    return N;

  SmallVector<toolchain::MDBuilder::TBAAStructField, 4> Fields;
...
toolchain::MDNode *CodeGenTBAA::getBaseTypeInfoHelper(const Type *Ty) {
  if (auto *TTy = dyn_cast<RecordType>(Ty)) {
    const RecordDecl *RD = TTy->getOriginalDecl()->getDefinition();
    const ASTRecordLayout &Layout = Context.getASTRecordLayout(RD);
    using TBAAStructField = toolchain::MDBuilder::TBAAStructField;
    ...
    if (const CXXRecordDecl *CXXRD = dyn_cast<CXXRecordDecl>(RD)) {
      ...
      for (const CXXBaseSpecifier &B : CXXRD->bases()) {
        ...
        if (BaseRD->isEmpty())
          continue;
        toolchain::MDNode *TypeNode = isValidBaseType(BaseQTy)
        ...
        Fields.push_back(
            toolchain::MDBuilder::TBAAStructField(Offset, Size, TypeNode));
      }
    ...
    for (FieldDecl *Field : RD->fields()) {
      ...
      toolchain::MDNode *TypeNode = isValidBaseType(FieldQTy)
      ...
      Fields.push_back(toolchain::MDBuilder::TBAAStructField(Offset, Size,
                                                        TypeNode));
    }
    ...
    if (CodeGenOpts.NewStructPathTBAA) {
      toolchain::MDNode *Parent = getChar();
      uint64_t Size = Context.getTypeSizeInChars(Ty).getQuantity();
      toolchain::Metadata *Id = MDHelper.createString(OutName);
      return MDHelper.createTBAATypeNode(Parent, Size, Id, Fields);
    }

    // Create the struct type node with a vector of pairs (offset, type).
    SmallVector<std::pair<toolchain::MDNode*, uint64_t>, 4> OffsetsAndTypes;
...
toolchain::MDNode *CodeGenTBAA::getValidBaseTypeInfo(QualType QTy) {
  ...
  toolchain::MDNode *TypeNode = getBaseTypeInfoHelper(Ty);
...
toolchain::MDNode *CodeGenTBAA::getBaseTypeInfo(QualType QTy) {
  return isValidBaseType(QTy) ? getValidBaseTypeInfo(QTy) : nullptr;
}

toolchain::MDNode *CodeGenTBAA::getAccessTagInfo(TBAAAccessInfo Info) {
  ...
  toolchain::MDNode *&N = AccessTagMetadataCache[Info];
  if (N)
    return N;
