...
#include "language/Core/Lex/ModuleMap.h"
...
#include "toolchain/Support/SHA1.h"
#include "toolchain/Support/SHA256.h"
...
static bool IsDecomposedVarDecl(VarDecl const *VD) {
  ...
  auto const *RefExpr =
      toolchain::dyn_cast_or_null<DeclRefExpr>(Init->IgnoreUnlessSpelledInSource());
  if (!RefExpr)
    return false;

  return toolchain::dyn_cast_or_null<DecompositionDecl>(RefExpr->getDecl());
}
...
void CGDebugInfo::addInstSourceAtomMetadata(toolchain::Instruction *I,
  ...
  const toolchain::DebugLoc &DL = I->getDebugLoc();
  ...
  toolchain::DILocation *NewDL = toolchain::DILocation::get(
      I->getContext(), DL.getLine(), DL.getCol(), DL.getScope(),
      DL.getInlinedAt(), DL.isImplicitCode(), Group, Rank);
  I->setDebugLoc(NewDL);
}

void CGDebugInfo::addInstToCurrentSourceAtom(toolchain::Instruction *KeyInstruction,
                                             toolchain::Value *Backup) {
  addInstToSpecificSourceAtom(KeyInstruction, Backup,
                              KeyInstructionsInfo.CurrentAtom);
}

void CGDebugInfo::addInstToSpecificSourceAtom(toolchain::Instruction *KeyInstruction,
                                              toolchain::Value *Backup,
                                              uint64_t Group) {
  if (!Group || !CGM.getCodeGenOpts().DebugKeyInstructions)
    return;

  toolchain::DISubprogram *SP = KeyInstruction->getFunction()->getSubprogram();
  if (!SP || !SP->getKeyInstructionsEnabled())
    return;

  addInstSourceAtomMetadata(KeyInstruction, Group, /*Rank=*/1);

  toolchain::Instruction *BackupI =
      toolchain::dyn_cast_or_null<toolchain::Instruction>(Backup);
  if (!BackupI)
    return;
  ...
  uint8_t Rank = 3;
  while (auto *Cast = dyn_cast<toolchain::CastInst>(BackupI)) {
    BackupI = dyn_cast<toolchain::Instruction>(Cast->getOperand(0));
    if (!BackupI)
      break;
...
void ApplyDebugLocation::init(SourceLocation TemporaryLocation,
  ...
  if (DefaultToEmpty) {
    CGF->Builder.SetCurrentDebugLocation(toolchain::DebugLoc());
    return;
  }
  ...
  CGF->Builder.SetCurrentDebugLocation(
      toolchain::DILocation::get(DI->LexicalBlockStack.back()->getContext(), 0, 0,
...
ApplyDebugLocation::ApplyDebugLocation(CodeGenFunction &CGF, toolchain::DebugLoc Loc)
    : CGF(&CGF) {
  if (!CGF.getDebugInfo()) {
    this->CGF = nullptr;
    return;
  ...
  if (Loc) {
    ...
    if (Loc->getAtomGroup())
      Loc = toolchain::DILocation::get(Loc->getContext(), Loc.getLine(),
...
void CGDebugInfo::setLocation(SourceLocation Loc) {
  ...
  auto *Scope = cast<toolchain::DIScope>(LexicalBlockStack.back());
  PresumedLoc PCLoc = SM.getPresumedLoc(CurLoc);
  if (PCLoc.isInvalid() || Scope->getFile() == getOrCreateFile(CurLoc))
    return;

  if (auto *LBF = dyn_cast<toolchain::DILexicalBlockFile>(Scope)) {
    LexicalBlockStack.pop_back();
    LexicalBlockStack.emplace_back(DBuilder.createLexicalBlockFile(
        LBF->getScope(), getOrCreateFile(CurLoc)));
  } else if (isa<toolchain::DILexicalBlock>(Scope) ||
             isa<toolchain::DISubprogram>(Scope)) {
    LexicalBlockStack.pop_back();
...
toolchain::DIScope *CGDebugInfo::getDeclContextDescriptor(const Decl *D) {
  toolchain::DIScope *Mod = getParentModuleOrNull(D);
  return getContextDescriptor(cast<Decl>(D->getDeclContext()),
                              Mod ? Mod : TheCU);
}

toolchain::DIScope *CGDebugInfo::getContextDescriptor(const Decl *Context,
  ...
  if (I != RegionMap.end()) {
    toolchain::Metadata *V = I->second;
    return dyn_cast_or_null<toolchain::DIScope>(V);
  }
...
StringRef CGDebugInfo::getObjCMethodName(const ObjCMethodDecl *OMD) {
  ...
  toolchain::raw_svector_ostream OS(MethodName);
...
CGDebugInfo::computeChecksum(FileID FID, SmallString<64> &Checksum) const {
  ...
  SourceManager &SM = CGM.getContext().getSourceManager();
  std::optional<toolchain::MemoryBufferRef> MemBuffer = SM.getBufferOrNone(FID);
  if (!MemBuffer)
    return std::nullopt;

  auto Data = toolchain::arrayRefFromStringRef(MemBuffer->getBuffer());
  switch (CGM.getCodeGenOpts().getDebugSrcHash()) {
  case language::Core::CodeGenOptions::DSH_MD5:
    toolchain::toHex(toolchain::MD5::hash(Data), /*LowerCase=*/true, Checksum);
    return toolchain::DIFile::CSK_MD5;
  case language::Core::CodeGenOptions::DSH_SHA1:
    toolchain::toHex(toolchain::SHA1::hash(Data), /*LowerCase=*/true, Checksum);
    return toolchain::DIFile::CSK_SHA1;
  case language::Core::CodeGenOptions::DSH_SHA256:
    toolchain::toHex(toolchain::SHA256::hash(Data), /*LowerCase=*/true, Checksum);
    return toolchain::DIFile::CSK_SHA256;
  case language::Core::CodeGenOptions::DSH_NONE:
    return std::nullopt;
  }
  toolchain_unreachable("Unhandled DebugSrcHashKind enum");
}
...
toolchain::DIFile *CGDebugInfo::getOrCreateFile(SourceLocation Loc) {
  SourceManager &SM = CGM.getContext().getSourceManager();
  StringRef FileName;
  FileID FID;
  ...
  if (It != DIFileCache.end()) {
    ...
    if (toolchain::Metadata *V = It->second)
      return cast<toolchain::DIFile>(V);
  }

  // Put Checksum at a scope where it will persist past the createFile call.
  SmallString<64> Checksum;
  if (!CSInfo) {
    std::optional<toolchain::DIFile::ChecksumKind> CSKind =
...
toolchain::DIFile *CGDebugInfo::createFile(
    StringRef FileName,
    std::optional<toolchain::DIFile::ChecksumInfo<StringRef>> CSInfo,
  ...
  if (toolchain::sys::path::is_absolute(RemappedFile)) {
    ...
    auto FileIt = toolchain::sys::path::begin(RemappedFile);
    auto FileE = toolchain::sys::path::end(RemappedFile);
    auto CurDirIt = toolchain::sys::path::begin(CurDir);
    auto CurDirE = toolchain::sys::path::end(CurDir);
    for (; CurDirIt != CurDirE && *CurDirIt == *FileIt; ++CurDirIt, ++FileIt)
      toolchain::sys::path::append(DirBuf, *CurDirIt);
    if (toolchain::sys::path::root_path(DirBuf) == DirBuf) {
  ...
  toolchain::DIFile *F = DBuilder.createFile(File, Dir, CSInfo, Source);
...
std::string CGDebugInfo::remapDIPath(StringRef Path) const {
  ...
  for (auto &[From, To] : toolchain::reverse(CGM.getCodeGenOpts().DebugPrefixMap))
    if (toolchain::sys::path::replace_path_prefix(P, From, To))
      break;
  return P.str().str();
}
...
void CGDebugInfo::CreateCompileUnit() {
  ...
  if (OptionalFileEntryRef MainFile =
    ...
    if (!toolchain::sys::path::is_absolute(MainFileName)) {
      toolchain::SmallString<1024> MainFileDirSS(MainFileDir);
      toolchain::sys::path::Style Style =
          LO.UseTargetPathSeparator
              ? (CGM.getTarget().getTriple().isOSWindows()
                     ? toolchain::sys::path::Style::windows_backslash
                     : toolchain::sys::path::Style::posix)
              : toolchain::sys::path::Style::native;
      toolchain::sys::path::append(MainFileDirSS, Style, MainFileName);
      MainFileName = std::string(
          toolchain::sys::path::remove_leading_dotslash(MainFileDirSS, Style));
    }
  ...
  toolchain::dwarf::SourceLanguage LangTag;
  if (LO.CPlusPlus) {
    if (LO.ObjC)
      LangTag = toolchain::dwarf::DW_LANG_ObjC_plus_plus;
    else if (CGO.DebugStrictDwarf && CGO.DwarfVersion < 5)
      LangTag = toolchain::dwarf::DW_LANG_C_plus_plus;
    else if (LO.CPlusPlus14)
      LangTag = toolchain::dwarf::DW_LANG_C_plus_plus_14;
    else if (LO.CPlusPlus11)
      LangTag = toolchain::dwarf::DW_LANG_C_plus_plus_11;
    else
      LangTag = toolchain::dwarf::DW_LANG_C_plus_plus;
  } else if (LO.ObjC) {
    LangTag = toolchain::dwarf::DW_LANG_ObjC;
  } else if (LO.OpenCL && (!CGM.getCodeGenOpts().DebugStrictDwarf ||
                           CGM.getCodeGenOpts().DwarfVersion >= 5)) {
    LangTag = toolchain::dwarf::DW_LANG_OpenCL;
  } else if (LO.C11 && !(CGO.DebugStrictDwarf && CGO.DwarfVersion < 5)) {
      LangTag = toolchain::dwarf::DW_LANG_C11;
  } else if (LO.C99) {
    LangTag = toolchain::dwarf::DW_LANG_C99;
  } else {
    LangTag = toolchain::dwarf::DW_LANG_C89;
  }
  ...
  toolchain::DICompileUnit::DebugEmissionKind EmissionKind;
  switch (DebugKind) {
  case toolchain::codegenoptions::NoDebugInfo:
  case toolchain::codegenoptions::LocTrackingOnly:
    EmissionKind = toolchain::DICompileUnit::NoDebug;
    break;
  case toolchain::codegenoptions::DebugLineTablesOnly:
    EmissionKind = toolchain::DICompileUnit::LineTablesOnly;
    break;
  case toolchain::codegenoptions::DebugDirectivesOnly:
    EmissionKind = toolchain::DICompileUnit::DebugDirectivesOnly;
    break;
  case toolchain::codegenoptions::DebugInfoConstructor:
  case toolchain::codegenoptions::LimitedDebugInfo:
  case toolchain::codegenoptions::FullDebugInfo:
  case toolchain::codegenoptions::UnusedTypeInfo:
    EmissionKind = toolchain::DICompileUnit::FullDebug;
    break;
  }
  ...
  if (CSKind)
    CSInfo.emplace(*CSKind, Checksum);
  toolchain::DIFile *CUFile = DBuilder.createFile(
      remapDIPath(MainFileName), remapDIPath(getCurrentDirname()), CSInfo,
      getSource(SM, SM.getMainFileID()));

  StringRef Sysroot, SDK;
  if (CGM.getCodeGenOpts().getDebuggerTuning() == toolchain::DebuggerKind::LLDB) {
    Sysroot = CGM.getHeaderSearchOpts().Sysroot;
    auto B = toolchain::sys::path::rbegin(Sysroot);
    auto E = toolchain::sys::path::rend(Sysroot);
    auto It =
  ...
  toolchain::DICompileUnit::DebugNameTableKind NameTableKind =
      static_cast<toolchain::DICompileUnit::DebugNameTableKind>(
          CGOpts.DebugNameTable);
  if (CGM.getTarget().getTriple().isNVPTX())
    NameTableKind = toolchain::DICompileUnit::DebugNameTableKind::None;
  else if (CGM.getTarget().getTriple().getVendor() == toolchain::Triple::Apple)
    NameTableKind = toolchain::DICompileUnit::DebugNameTableKind::Apple;
...
toolchain::DIType *CGDebugInfo::CreateType(const BuiltinType *BT) {
  toolchain::dwarf::TypeKind Encoding;
  StringRef BTName;
  switch (BT->getKind()) {
...
#include "language/Core/AST/BuiltinTypes.def"
  case BuiltinType::Dependent:
    toolchain_unreachable("Unexpected builtin type");
  ...
  case BuiltinType::ObjCClass:
    if (!ClassTy)
      ClassTy =
          DBuilder.createForwardDecl(toolchain::dwarf::DW_TAG_structure_type,
  ...
  case BuiltinType::ObjCId: {
    ...
    ObjTy = DBuilder.createStructType(TheCU, "objc_object", TheCU->getFile(), 0,
                                      (uint64_t)0, 0, toolchain::DINode::FlagZero,
                                      nullptr, toolchain::DINodeArray());

    DBuilder.replaceArrays(
        ObjTy, DBuilder.getOrCreateArray(&*DBuilder.createMemberType(
                   ObjTy, "isa", TheCU->getFile(), 0, Size, 0, 0,
                   toolchain::DINode::FlagZero, ISATy)));
    return ObjTy;
  }
  case BuiltinType::ObjCSel: {
    if (!SelTy)
      SelTy = DBuilder.createForwardDecl(toolchain::dwarf::DW_TAG_structure_type,
                                         "objc_selector", TheCU,
                                         TheCU->getFile(), 0);
    return SelTy;
  }
...
#include "language/Core/Basic/AArch64ACLETypes.def"
    {
      if (BT->getKind() == BuiltinType::MFloat8) {
        Encoding = toolchain::dwarf::DW_ATE_unsigned_char;
      ...
      ASTContext::BuiltinVectorTypeInfo Info =
          ...
          BT->getKind() == BuiltinType::SveCount
              ? ASTContext::BuiltinVectorTypeInfo(
                    CGM.getContext().BoolTy, toolchain::ElementCount::getFixed(16),
                    1)
      ...
      toolchain::Metadata *LowerBound, *UpperBound;
      LowerBound = toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
          toolchain::Type::getInt64Ty(CGM.getLLVMContext()), 0));
      if (Info.EC.isScalable()) {
        unsigned NumElemsPerVG = NumElems / 2;
        SmallVector<uint64_t, 9> Expr(
            {toolchain::dwarf::DW_OP_constu, NumElemsPerVG, toolchain::dwarf::DW_OP_bregx,
             /* AArch64::VG */ 46, 0, toolchain::dwarf::DW_OP_mul,
             toolchain::dwarf::DW_OP_constu, 1, toolchain::dwarf::DW_OP_minus});
        UpperBound = DBuilder.createExpression(Expr);
      } else
        UpperBound = toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
            toolchain::Type::getInt64Ty(CGM.getLLVMContext()), NumElems - 1));

      toolchain::Metadata *Subscript = DBuilder.getOrCreateSubrange(
          /*count*/ nullptr, LowerBound, UpperBound, /*stride*/ nullptr);
      toolchain::DINodeArray SubscriptArray = DBuilder.getOrCreateArray(Subscript);
      toolchain::DIType *ElemTy =
...
#include "language/Core/Basic/RISCVVTypes.def"
    {
      ...
      SmallVector<uint64_t, 12> Expr(
          ...
          // offset.
          {toolchain::dwarf::DW_OP_bregx, // Read the contents of a register.
           4096 + 0xC22,             // RISC-V VLENB CSR register.
           0, // Offset for DW_OP_bregx. It is dummy here.
           toolchain::dwarf::DW_OP_constu,
           SEW / 8, // SEW is in bits.
           toolchain::dwarf::DW_OP_div, toolchain::dwarf::DW_OP_constu, LMUL});
      if (Fractional)
        Expr.push_back(toolchain::dwarf::DW_OP_div);
      else
        Expr.push_back(toolchain::dwarf::DW_OP_mul);
      // NFIELDS multiplier
      if (NFIELDS > 1)
        Expr.append({toolchain::dwarf::DW_OP_constu, NFIELDS, toolchain::dwarf::DW_OP_mul});
      // Element max index = count - 1
      Expr.append({toolchain::dwarf::DW_OP_constu, 1, toolchain::dwarf::DW_OP_minus});

      auto *LowerBound =
          toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
              toolchain::Type::getInt64Ty(CGM.getLLVMContext()), 0));
...
#define AMDGPU_OPAQUE_PTR_TYPE(Name, Id, SingletonId, Width, Align, AS)        \
  case BuiltinType::Id: {                                                      \
    if (!SingletonId)                                                          \
      SingletonId =                                                            \
          DBuilder.createForwardDecl(toolchain::dwarf::DW_TAG_structure_type, Name, \
...
#define AMDGPU_NAMED_BARRIER_TYPE(Name, Id, SingletonId, Width, Align, Scope)  \
  case BuiltinType::Id: {                                                      \
    if (!SingletonId)                                                          \
      SingletonId =                                                            \
          DBuilder.createBasicType(Name, Width, toolchain::dwarf::DW_ATE_unsigned); \
    return SingletonId;                                                        \
  }
#include "language/Core/Basic/AMDGPUTypes.def"
  ...
  case BuiltinType::SChar:
    Encoding = toolchain::dwarf::DW_ATE_signed_char;
    break;
  case BuiltinType::Char8:
  case BuiltinType::Char16:
  case BuiltinType::Char32:
    Encoding = toolchain::dwarf::DW_ATE_UTF;
    break;
  ...
  case BuiltinType::ULongLong:
    Encoding = toolchain::dwarf::DW_ATE_unsigned;
    break;
  ...
  case BuiltinType::LongLong:
    Encoding = toolchain::dwarf::DW_ATE_signed;
    break;
  case BuiltinType::Bool:
    Encoding = toolchain::dwarf::DW_ATE_boolean;
    break;
  ...
  case BuiltinType::Ibm128:
    ...
    Encoding = toolchain::dwarf::DW_ATE_float;
    break;
  ...
  case BuiltinType::SatLongAccum:
    Encoding = toolchain::dwarf::DW_ATE_signed_fixed;
    break;
  ...
  case BuiltinType::SatULongFract:
    Encoding = toolchain::dwarf::DW_ATE_unsigned_fixed;
    break;
  }
...
toolchain::DIType *CGDebugInfo::CreateType(const BitIntType *Ty) {

  StringRef Name = Ty->isUnsigned() ? "unsigned _BitInt" : "_BitInt";
  toolchain::dwarf::TypeKind Encoding = Ty->isUnsigned()
                                       ? toolchain::dwarf::DW_ATE_unsigned
                                       : toolchain::dwarf::DW_ATE_signed;

  return DBuilder.createBasicType(Name, CGM.getContext().getTypeSize(Ty),
                                  Encoding);
}

toolchain::DIType *CGDebugInfo::CreateType(const ComplexType *Ty) {
  // Bit size and offset of the type.
  toolchain::dwarf::TypeKind Encoding = toolchain::dwarf::DW_ATE_complex_float;
  if (Ty->isComplexIntegerType())
    Encoding = toolchain::dwarf::DW_ATE_lo_user;
...
static toolchain::dwarf::Tag getNextQualifier(Qualifiers &Q) {
  if (Q.hasConst()) {
    Q.removeConst();
    return toolchain::dwarf::DW_TAG_const_type;
  }
  if (Q.hasVolatile()) {
    Q.removeVolatile();
    return toolchain::dwarf::DW_TAG_volatile_type;
  }
  if (Q.hasRestrict()) {
    Q.removeRestrict();
    return toolchain::dwarf::DW_TAG_restrict_type;
  }
  return (toolchain::dwarf::Tag)0;
}

toolchain::DIType *CGDebugInfo::CreateQualifiedType(QualType Ty,
  ...
  // additional ones.
  toolchain::dwarf::Tag Tag = getNextQualifier(Qc);
  if (!Tag) {
    if (Qc.getPointerAuth()) {
      ...
      toolchain::DIType *FromTy = getOrCreateType(QualType(T, 0), Unit);
...
toolchain::DIType *CGDebugInfo::CreateQualifiedType(const FunctionProtoType *F,
                                               toolchain::DIFile *Unit) {
  ...
  // We will create one Derived type for one qualifier and recurse to handle any
  // additional ones.
  toolchain::dwarf::Tag Tag = getNextQualifier(Q);
  if (!Tag) {
...
toolchain::DIType *CGDebugInfo::CreateType(const ObjCObjectPointerType *Ty,
                                      toolchain::DIFile *Unit) {
  ...
  return CreatePointerLikeType(toolchain::dwarf::DW_TAG_pointer_type, Ty,
                               Ty->getPointeeType(), Unit);
}

toolchain::DIType *CGDebugInfo::CreateType(const PointerType *Ty,
                                      toolchain::DIFile *Unit) {
  return CreatePointerLikeType(toolchain::dwarf::DW_TAG_pointer_type, Ty,
                               Ty->getPointeeType(), Unit);
}
...
static bool hasCXXMangling(const TagDecl *TD, toolchain::DICompileUnit *TheCU) {
  switch (TheCU->getSourceLanguage()) {
  case toolchain::dwarf::DW_LANG_C_plus_plus:
  case toolchain::dwarf::DW_LANG_C_plus_plus_11:
  case toolchain::dwarf::DW_LANG_C_plus_plus_14:
    return true;
  case toolchain::dwarf::DW_LANG_ObjC_plus_plus:
  ...
  if (const auto *RD = dyn_cast<CXXRecordDecl>(TD))
    if (RD->getDefinition())
      if (RD->isDynamicClass() &&
          CGM.getVTableLinkage(RD) == toolchain::GlobalValue::ExternalLinkage)
        return Identifier;

  // TODO: This is using the RTTI name. Is there a better way to get
  // a unique string for a type?
  toolchain::raw_svector_ostream Out(Identifier);
  CGM.getCXXABI().getMangleContext().mangleCXXRTTIName(QualType(Ty, 0), Out);
  return Identifier;
}

/// \return the appropriate DWARF tag for a composite type.
static toolchain::dwarf::Tag getTagForRecord(const RecordDecl *RD) {
  toolchain::dwarf::Tag Tag;
  if (RD->isStruct() || RD->isInterface())
    Tag = toolchain::dwarf::DW_TAG_structure_type;
  else if (RD->isUnion())
    Tag = toolchain::dwarf::DW_TAG_union_type;
  else {
    // FIXME: This could be a struct type giving a default visibility different
    // than C++ class type, but needs toolchain metadata changes first.
    assert(RD->isClass());
    Tag = toolchain::dwarf::DW_TAG_class_type;
  }
  return Tag;
}
...
CGDebugInfo::getOrCreateRecordFwdDecl(const RecordType *Ty,
                                      toolchain::DIScope *Ctx) {
  const RecordDecl *RD = Ty->getOriginalDecl()->getDefinitionOrSelf();
  if (toolchain::DIType *T = getTypeOrNull(QualType(Ty, 0)))
    return cast<toolchain::DICompositeType>(T);
  toolchain::DIFile *DefUnit = getOrCreateFile(RD->getLocation());
  const unsigned Line =
  ...
  toolchain::DINode::DIFlags Flags = toolchain::DINode::FlagFwdDecl;
  ...
  if (const CXXRecordDecl *CXXRD = dyn_cast<CXXRecordDecl>(RD))
    if (!CXXRD->hasDefinition() ||
      ...
      Flags |= toolchain::DINode::FlagNonTrivial;
  ...
  toolchain::DICompositeType *RetTy = DBuilder.createReplaceableCompositeType(
      getTagForRecord(RD), RDName, Ctx, DefUnit, Line, 0, Size, Align, Flags,
      Identifier);
  if (CGM.getCodeGenOpts().DebugFwdTemplateParams)
    if (auto *TSpecial = dyn_cast<ClassTemplateSpecializationDecl>(RD))
      DBuilder.replaceArrays(RetTy, toolchain::DINodeArray(),
                             CollectCXXTemplateParams(TSpecial, DefUnit));
  ReplaceMap.emplace_back(
      std::piecewise_construct, std::make_tuple(Ty),
      std::make_tuple(static_cast<toolchain::Metadata *>(RetTy)));
  return RetTy;
}

toolchain::DIType *CGDebugInfo::CreatePointerLikeType(toolchain::dwarf::Tag Tag,
                                                 const Type *Ty,
                                                 QualType PointeeTy,
                                                 toolchain::DIFile *Unit) {
  ...
  SmallVector<toolchain::Metadata *, 4> Annots;
  while (BTFAttrTy) {
    StringRef Tag = BTFAttrTy->getAttr()->getBTFTypeTag();
    if (!Tag.empty()) {
      toolchain::Metadata *Ops[2] = {
          toolchain::MDString::get(CGM.getLLVMContext(), StringRef("btf_type_tag")),
          toolchain::MDString::get(CGM.getLLVMContext(), Tag)};
      Annots.insert(Annots.begin(),
                    toolchain::MDNode::get(CGM.getLLVMContext(), Ops));
    }
  ...
  if (Tag == toolchain::dwarf::DW_TAG_reference_type ||
      Tag == toolchain::dwarf::DW_TAG_rvalue_reference_type)
...
toolchain::DIType *CGDebugInfo::getOrCreateStructPtrType(StringRef Name,
                                                    toolchain::DIType *&Cache) {
  if (Cache)
    return Cache;
  Cache = DBuilder.createForwardDecl(toolchain::dwarf::DW_TAG_structure_type, Name,
                                     TheCU, TheCU->getFile(), 0);
  unsigned Size = CGM.getContext().getTypeSize(CGM.getContext().VoidPtrTy);
...
uint64_t CGDebugInfo::collectDefaultElementTypesForBlockPointer(
    const BlockPointerType *Ty, toolchain::DIFile *Unit, toolchain::DIDerivedType *DescTy,
    unsigned LineNo, SmallVectorImpl<toolchain::Metadata *> &EltTys) {
  QualType FType;
  ...
  if (CGM.getLangOpts().OpenCL) {
    ...
    EltTys.push_back(DBuilder.createMemberType(
        ...
        FieldOffset, toolchain::DINode::FlagZero, DescTy));
...
toolchain::DIType *CGDebugInfo::CreateType(const BlockPointerType *Ty,
                                      toolchain::DIFile *Unit) {
  SmallVector<toolchain::Metadata *, 8> EltTys;
  QualType FType;
  uint64_t FieldOffset;
  toolchain::DINodeArray Elements;
  ...
  toolchain::DINode::DIFlags Flags = toolchain::DINode::FlagAppleBlock;
...
static toolchain::SmallVector<TemplateArgument>
...
toolchain::DIType *CGDebugInfo::CreateType(const TemplateSpecializationType *Ty,
                                      toolchain::DIFile *Unit) {
  assert(Ty->isTypeAlias());
  toolchain::DIType *Src = getOrCreateType(Ty->getAliasedType(), Unit);
  ...
  SmallString<128> NS;
  toolchain::raw_svector_ostream OS(NS);
  ...
  if (CGM.getCodeGenOpts().DebugTemplateAlias) {
    ...
    if (CGM.getCodeGenOpts().getDebugSimpleTemplateNames() !=
            toolchain::codegenoptions::DebugTemplateNamesKind::Simple ||
        !HasReconstitutableArgs(Args.Args))
      printTemplateArgumentList(OS, Args.Args, PP);

    toolchain::DIDerivedType *AliasTy = DBuilder.createTemplateAlias(
...
static toolchain::DINode::DIFlags getAccessFlag(AccessSpecifier Access,
                                           const RecordDecl *RD) {
  AccessSpecifier Default = language::Core::AS_none;
  if (RD && RD->isClass())
    Default = language::Core::AS_private;
  else if (RD && (RD->isStruct() || RD->isUnion()))
    Default = language::Core::AS_public;

  if (Access == Default)
    return toolchain::DINode::FlagZero;

  switch (Access) {
  case language::Core::AS_private:
    return toolchain::DINode::FlagPrivate;
  case language::Core::AS_protected:
    return toolchain::DINode::FlagProtected;
  case language::Core::AS_public:
    return toolchain::DINode::FlagPublic;
  case language::Core::AS_none:
    return toolchain::DINode::FlagZero;
  }
  toolchain_unreachable("unexpected access enumerator");
}

toolchain::DIType *CGDebugInfo::CreateType(const TypedefType *Ty,
                                      toolchain::DIFile *Unit) {
  toolchain::DIType *Underlying =
  ...
  toolchain::DINodeArray Annotations = CollectBTFDeclTagAnnotations(Ty->getDecl());

  toolchain::DINode::DIFlags Flags = toolchain::DINode::FlagZero;
...
static unsigned getDwarfCC(CallingConv CC) {
  ...
  case CC_X86StdCall:
    return toolchain::dwarf::DW_CC_BORLAND_stdcall;
  case CC_X86FastCall:
    return toolchain::dwarf::DW_CC_BORLAND_msfastcall;
  case CC_X86ThisCall:
    return toolchain::dwarf::DW_CC_BORLAND_thiscall;
  case CC_X86VectorCall:
    return toolchain::dwarf::DW_CC_LLVM_vectorcall;
  case CC_X86Pascal:
    return toolchain::dwarf::DW_CC_BORLAND_pascal;
  case CC_Win64:
    return toolchain::dwarf::DW_CC_LLVM_Win64;
  case CC_X86_64SysV:
    return toolchain::dwarf::DW_CC_LLVM_X86_64SysV;
  case CC_AAPCS:
  case CC_AArch64VectorCall:
  case CC_AArch64SVEPCS:
    return toolchain::dwarf::DW_CC_LLVM_AAPCS;
  case CC_AAPCS_VFP:
    return toolchain::dwarf::DW_CC_LLVM_AAPCS_VFP;
  case CC_IntelOclBicc:
    return toolchain::dwarf::DW_CC_LLVM_IntelOclBicc;
  case CC_SpirFunction:
    return toolchain::dwarf::DW_CC_LLVM_SpirFunction;
  case CC_DeviceKernel:
    return toolchain::dwarf::DW_CC_LLVM_DeviceKernel;
  case CC_Swift:
    return toolchain::dwarf::DW_CC_LLVM_Swift;
  case CC_SwiftAsync:
    return toolchain::dwarf::DW_CC_LLVM_SwiftTail;
  case CC_PreserveMost:
    return toolchain::dwarf::DW_CC_LLVM_PreserveMost;
  case CC_PreserveAll:
    return toolchain::dwarf::DW_CC_LLVM_PreserveAll;
  case CC_X86RegCall:
    return toolchain::dwarf::DW_CC_LLVM_X86RegCall;
  case CC_M68kRTD:
    return toolchain::dwarf::DW_CC_LLVM_M68kRTD;
  case CC_PreserveNone:
    return toolchain::dwarf::DW_CC_LLVM_PreserveNone;
  case CC_RISCVVectorCall:
    return toolchain::dwarf::DW_CC_LLVM_RISCVVectorCall;
#define CC_VLS_CASE(ABI_VLEN) case CC_RISCVVLSCall_##ABI_VLEN:
    ...
    CC_VLS_CASE(65536)
#undef CC_VLS_CASE
    return toolchain::dwarf::DW_CC_LLVM_RISCVVLSCall;
  }
  return 0;
}

static toolchain::DINode::DIFlags getRefFlags(const FunctionProtoType *Func) {
  toolchain::DINode::DIFlags Flags = toolchain::DINode::FlagZero;
  if (Func->getExtProtoInfo().RefQualifier == RQ_LValue)
    Flags |= toolchain::DINode::FlagLValueReference;
  if (Func->getExtProtoInfo().RefQualifier == RQ_RValue)
    Flags |= toolchain::DINode::FlagRValueReference;
  return Flags;
}

toolchain::DIType *CGDebugInfo::CreateType(const FunctionType *Ty,
                                      toolchain::DIFile *Unit) {
  const auto *FPT = dyn_cast<FunctionProtoType>(Ty);
  if (FPT) {
    if (toolchain::DIType *QTy = CreateQualifiedType(FPT, Unit))
      return QTy;
  }

  // Create the type without any qualifiers

  SmallVector<toolchain::Metadata *, 16> EltTys;
  ...
  toolchain::DITypeRefArray EltTypeArray = DBuilder.getOrCreateTypeArray(EltTys);
  toolchain::DIType *F = DBuilder.createSubroutineType(
...
CGDebugInfo::createBitFieldType(const FieldDecl *BitFieldDecl,
                                toolchain::DIScope *RecordTy, const RecordDecl *RD) {
  StringRef Name = BitFieldDecl->getName();
  QualType Ty = BitFieldDecl->getType();
  if (BitFieldDecl->hasAttr<PreferredTypeAttr>())
    Ty = BitFieldDecl->getAttr<PreferredTypeAttr>()->getType();
  SourceLocation Loc = BitFieldDecl->getLocation();
  toolchain::DIFile *VUnit = getOrCreateFile(Loc);
  toolchain::DIType *DebugType = getOrCreateType(Ty, VUnit);

  // Get the location for the field.
  toolchain::DIFile *File = getOrCreateFile(Loc);
  ...
  toolchain::DINode::DIFlags Flags = getAccessFlag(BitFieldDecl->getAccess(), RD);
  toolchain::DINodeArray Annotations = CollectBTFDeclTagAnnotations(BitFieldDecl);
  return DBuilder.createBitFieldMemberType(
      RecordTy, Name, File, Line, SizeInBits, OffsetInBits, StorageOffsetInBits,
      Flags, DebugType, Annotations);
}

toolchain::DIDerivedType *CGDebugInfo::createBitFieldSeparatorIfNeeded(
  ...
  */
  ...
  auto *PreviousMDField =
      dyn_cast_or_null<toolchain::DIDerivedType>(PreviousMDEntry);
  ...
  toolchain::DIFile *VUnit = getOrCreateFile(Loc);
  toolchain::DIType *DebugType = getOrCreateType(Ty, VUnit);
  toolchain::DIScope *RecordTy = BitFieldDI->getScope();

  toolchain::DIFile *File = getOrCreateFile(Loc);
  unsigned Line = getLineNumber(Loc);

  uint64_t StorageOffsetInBits =
      cast<toolchain::ConstantInt>(BitFieldDI->getStorageOffsetInBits())
          ->getZExtValue();

  toolchain::DINode::DIFlags Flags =
      getAccessFlag(PreviousBitfield->getAccess(), RD);
  toolchain::DINodeArray Annotations =
...
toolchain::DIType *CGDebugInfo::createFieldType(
    StringRef name, QualType type, SourceLocation loc, AccessSpecifier AS,
    uint64_t offsetInBits, uint32_t AlignInBits, toolchain::DIFile *tunit,
    toolchain::DIScope *scope, const RecordDecl *RD, toolchain::DINodeArray Annotations) {
  toolchain::DIType *debugType = getOrCreateType(type, tunit);
  ...
  toolchain::DINode::DIFlags flags = getAccessFlag(AS, RD);
...
CGDebugInfo::createInlinedSubprogram(StringRef FuncName,
  ...
  toolchain::DISubprogram *&SP = InlinedSubprogramMap[FuncName];

  if (!SP) {
    toolchain::DISubroutineType *DIFnTy = DBuilder.createSubroutineType(nullptr);
    SP = DBuilder.createFunction(
        ...
        /*SPFlags=*/toolchain::DISubprogram::SPFlagDefinition,
...
void CGDebugInfo::CollectRecordLambdaFields(
    const CXXRecordDecl *CXXDecl, SmallVectorImpl<toolchain::Metadata *> &elements,
  ...
  for (CXXRecordDecl::capture_const_iterator I = CXXDecl->captures_begin(),
    ...
    if (C.capturesVariable()) {
      ...
      toolchain::DIType *FieldType = createFieldType(
      ...
      FieldDecl *f = *Field;
      toolchain::DIFile *VUnit = getOrCreateFile(f->getLocation());
...
toolchain::DIDerivedType *
CGDebugInfo::CreateRecordStaticField(const VarDecl *Var, toolchain::DIType *RecordTy,
                                     const RecordDecl *RD) {
  // Create the descriptor for the static variable, with or without
  // constant initializers.
  Var = Var->getCanonicalDecl();
  toolchain::DIFile *VUnit = getOrCreateFile(Var->getLocation());
  toolchain::DIType *VTy = getOrCreateType(Var->getType(), VUnit);
  ...
  if (Var->getInit()) {
    ...
    if (Value) {
      if (Value->isInt())
        C = toolchain::ConstantInt::get(CGM.getLLVMContext(), Value->getInt());
      if (Value->isFloat())
        C = toolchain::ConstantFP::get(CGM.getLLVMContext(), Value->getFloat());
    }
  }

  toolchain::DINode::DIFlags Flags = getAccessFlag(Var->getAccess(), RD);
  auto Tag = CGM.getCodeGenOpts().DwarfVersion >= 5
                 ? toolchain::dwarf::DW_TAG_variable
                 : toolchain::dwarf::DW_TAG_member;
  auto Align = getDeclAlignIfRequired(Var, CGM.getContext());
  toolchain::DIDerivedType *GV = DBuilder.createStaticMemberType(
...
void CGDebugInfo::CollectRecordNormalField(
    const FieldDecl *field, uint64_t OffsetInBits, toolchain::DIFile *tunit,
    SmallVectorImpl<toolchain::Metadata *> &elements, toolchain::DIType *RecordTy,
    const RecordDecl *RD) {
  ...
  toolchain::DIType *FieldType;
  if (field->isBitField()) {
    toolchain::DIDerivedType *BitFieldType;
    FieldType = BitFieldType = createBitFieldType(field, RecordTy, RD);
    if (toolchain::DIType *Separator =
            createBitFieldSeparatorIfNeeded(field, BitFieldType, elements, RD))
      elements.push_back(Separator);
  } else {
    auto Align = getDeclAlignIfRequired(field, CGM.getContext());
    toolchain::DINodeArray Annotations = CollectBTFDeclTagAnnotations(field);
    FieldType =
...
void CGDebugInfo::CollectRecordNestedType(
    const TypeDecl *TD, SmallVectorImpl<toolchain::Metadata *> &elements) {
  ...
  if (toolchain::DIType *nestedType = getOrCreateType(Ty, getOrCreateFile(Loc)))
    elements.push_back(nestedType);
}

void CGDebugInfo::CollectRecordFields(
    const RecordDecl *record, toolchain::DIFile *tunit,
    SmallVectorImpl<toolchain::Metadata *> &elements,
...
CGDebugInfo::getOrCreateMethodType(const CXXMethodDecl *Method,
  ...
  if (Method->isStatic())
    return cast_or_null<toolchain::DISubroutineType>(
...
toolchain::DISubroutineType *CGDebugInfo::getOrCreateMethodTypeForDestructor(
    const CXXMethodDecl *Method, toolchain::DIFile *Unit, QualType FNType) {
...
CGDebugInfo::getOrCreateInstanceMethodType(QualType ThisPtr,
                                           ...
                                           toolchain::DIFile *Unit, bool SkipFirst) {
  ...
  // Add "this" pointer.
  const auto *OriginalFunc = cast<toolchain::DISubroutineType>(
      getOrCreateType(CGM.getContext().getFunctionType(
                          Func->getReturnType(), Func->getParamTypes(), EPI),
                      Unit));
  toolchain::DITypeRefArray Args = OriginalFunc->getTypeArray();
  assert(Args.size() && "Invalid number of arguments!");

  SmallVector<toolchain::Metadata *, 16> Elts;
  ...
  if (!HasExplicitObjectParameter) {
    toolchain::DIType *ThisPtrType = getOrCreateType(ThisPtr, Unit);
  ...
  toolchain::DITypeRefArray EltTypeArray = DBuilder.getOrCreateTypeArray(Elts);
...
toolchain::DISubprogram *CGDebugInfo::CreateCXXMemberFunction(
    const CXXMethodDecl *Method, toolchain::DIFile *Unit, toolchain::DIType *RecordTy) {
  bool IsCtorOrDtor =
      isa<CXXConstructorDecl>(Method) || isa<CXXDestructorDecl>(Method);

  StringRef MethodName = getFunctionName(Method);
  toolchain::DISubroutineType *MethodTy = getOrCreateMethodType(Method, Unit);
  ...
  toolchain::DIFile *MethodDefUnit = nullptr;
  ...
  toolchain::DIType *ContainingType = nullptr;
  unsigned VIndex = 0;
  toolchain::DINode::DIFlags Flags = toolchain::DINode::FlagZero;
  toolchain::DISubprogram::DISPFlags SPFlags = toolchain::DISubprogram::SPFlagZero;
  int ThisAdjustment = 0;

  if (VTableContextBase::hasVtableSlot(Method)) {
    if (Method->isPureVirtual())
      SPFlags |= toolchain::DISubprogram::SPFlagPureVirtual;
    else
      SPFlags |= toolchain::DISubprogram::SPFlagVirtual;
    ...
    if (CGM.getTarget().getCXXABI().isItaniumFamily()) {
      ...
      if (Method->size_overridden_methods() == 0)
        Flags |= toolchain::DINode::FlagIntroducedVirtual;
  ...
  if (Method->getCanonicalDecl()->isDeleted())
    SPFlags |= toolchain::DISubprogram::SPFlagDeleted;

  if (Method->isNoReturn())
    Flags |= toolchain::DINode::FlagNoReturn;

  if (Method->isStatic())
    Flags |= toolchain::DINode::FlagStaticMember;
  if (Method->isImplicit())
    Flags |= toolchain::DINode::FlagArtificial;
  Flags |= getAccessFlag(Method->getAccess(), Method->getParent());
  if (const auto *CXXC = dyn_cast<CXXConstructorDecl>(Method)) {
    if (CXXC->isExplicit())
      Flags |= toolchain::DINode::FlagExplicit;
  } else if (const auto *CXXC = dyn_cast<CXXConversionDecl>(Method)) {
    if (CXXC->isExplicit())
      Flags |= toolchain::DINode::FlagExplicit;
  }
  if (Method->hasPrototype())
    Flags |= toolchain::DINode::FlagPrototyped;
  if (Method->getRefQualifier() == RQ_LValue)
    Flags |= toolchain::DINode::FlagLValueReference;
  if (Method->getRefQualifier() == RQ_RValue)
    Flags |= toolchain::DINode::FlagRValueReference;
  if (!Method->isExternallyVisible())
    SPFlags |= toolchain::DISubprogram::SPFlagLocalToUnit;
  if (CGM.getCodeGenOpts().OptimizationLevel != 0)
    SPFlags |= toolchain::DISubprogram::SPFlagOptimized;

  // In this debug mode, emit type info for a class when its constructor type
  // info is emitted.
  if (DebugKind == toolchain::codegenoptions::DebugInfoConstructor)
    if (const CXXConstructorDecl *CD = dyn_cast<CXXConstructorDecl>(Method))
      completeUnusedClass(*CD->getParent());

  toolchain::DINodeArray TParamsArray = CollectFunctionTemplateParams(Method, Unit);
  toolchain::DISubprogram *SP = DBuilder.createMethod(
...
void CGDebugInfo::CollectCXXMemberFunctions(
  ...
  for (const auto *I : RD->decls()) {
    ...
    EltTys.push_back(MI == SPCache.end()
                         ...
                         : static_cast<toolchain::Metadata *>(MI->second));
  }
}

void CGDebugInfo::CollectCXXBases(const CXXRecordDecl *RD, toolchain::DIFile *Unit,
  ...
  if (CGM.getCodeGenOpts().EmitCodeView) {
    CollectCXXBasesAux(RD, Unit, EltTys, RecordTy, RD->vbases(), SeenTypes,
                       toolchain::DINode::FlagIndirectVirtualBase);
  }
}
...
void CGDebugInfo::CollectCXXBasesAux(
  ...
  for (const auto &BI : Bases) {
    ...
    toolchain::DINode::DIFlags BFlags = StartingFlags;
    uint64_t BaseOffset;
    ...
    if (BI.isVirtual()) {
      ...
      BFlags |= toolchain::DINode::FlagVirtual;
    } else
    ...
    toolchain::DIType *DTy = DBuilder.createInheritance(RecordTy, BaseTy, BaseOffset,
                                                   VBPtrOffset, BFlags);
    EltTys.push_back(DTy);
  }
}

toolchain::DINodeArray
CGDebugInfo::CollectTemplateParams(std::optional<TemplateArgs> OArgs,
                                   toolchain::DIFile *Unit) {
  if (!OArgs)
    return toolchain::DINodeArray();
  TemplateArgs &Args = *OArgs;
  SmallVector<toolchain::Metadata *, 16> TemplateParams;
  for (unsigned i = 0, e = Args.Args.size(); i != e; ++i) {
    ...
    case TemplateArgument::Type: {
      toolchain::DIType *TTy = getOrCreateType(TA.getAsType(), Unit);
      TemplateParams.push_back(DBuilder.createTemplateTypeParameter(
          TheCU, Name, TTy, defaultParameter));

    } break;
    case TemplateArgument::Integral: {
      toolchain::DIType *TTy = getOrCreateType(TA.getIntegralType(), Unit);
      TemplateParams.push_back(DBuilder.createTemplateValueParameter(
          TheCU, Name, TTy, defaultParameter,
          toolchain::ConstantInt::get(CGM.getLLVMContext(), TA.getAsIntegral())));
    } break;
    case TemplateArgument::Declaration: {
      const ValueDecl *D = TA.getAsDecl();
      QualType T = TA.getParamTypeForDecl().getDesugaredType(CGM.getContext());
      toolchain::DIType *TTy = getOrCreateType(T, Unit);
      toolchain::Constant *V = nullptr;
      ...
      TemplateParams.push_back(DBuilder.createTemplateValueParameter(
          TheCU, Name, TTy, defaultParameter, cast_or_null<toolchain::Constant>(V)));
    } break;
    case TemplateArgument::NullPtr: {
      ...
      if (!V)
        V = toolchain::ConstantInt::get(CGM.Int8Ty, 0);
      TemplateParams.push_back(DBuilder.createTemplateValueParameter(
    ...
    case TemplateArgument::StructuralValue: {
      ...
      toolchain::DIType *TTy = getOrCreateType(T, Unit);
      toolchain::Constant *V = ConstantEmitter(CGM).emitAbstract(
    ...
    case TemplateArgument::Expression: {
      ...
      toolchain::Constant *V = ConstantEmitter(CGM).emitAbstract(E, T);
...
toolchain::DINodeArray CGDebugInfo::CollectVarTemplateParams(const VarDecl *VL,
                                                        toolchain::DIFile *Unit) {
  return CollectTemplateParams(GetTemplateArgs(VL), Unit);
}

toolchain::DINodeArray CGDebugInfo::CollectCXXTemplateParams(const RecordDecl *RD,
                                                        toolchain::DIFile *Unit) {
  return CollectTemplateParams(GetTemplateArgs(RD), Unit);
}

toolchain::DINodeArray CGDebugInfo::CollectBTFDeclTagAnnotations(const Decl *D) {
  if (!D->hasAttr<BTFDeclTagAttr>())
    return nullptr;

  SmallVector<toolchain::Metadata *, 4> Annotations;
  for (const auto *I : D->specific_attrs<BTFDeclTagAttr>()) {
    toolchain::Metadata *Ops[2] = {
        toolchain::MDString::get(CGM.getLLVMContext(), StringRef("btf_decl_tag")),
        toolchain::MDString::get(CGM.getLLVMContext(), I->getBTFDeclTag())};
    Annotations.push_back(toolchain::MDNode::get(CGM.getLLVMContext(), Ops));
  }
  return DBuilder.getOrCreateArray(Annotations);
}

toolchain::DIType *CGDebugInfo::getOrCreateVTablePtrType(toolchain::DIFile *Unit) {
  if (VTablePtrType)
  ...
  /* Function type */
  toolchain::Metadata *STy = getOrCreateType(Context.IntTy, Unit);
  toolchain::DITypeRefArray SElements = DBuilder.getOrCreateTypeArray(STy);
  toolchain::DIType *SubTy = DBuilder.createSubroutineType(SElements);
  unsigned Size = Context.getTypeSize(Context.VoidPtrTy);
  unsigned VtblPtrAddressSpace = CGM.getTarget().getVtblPtrAddressSpace();
  std::optional<unsigned> DWARFAddressSpace =
      CGM.getTarget().getDWARFAddressSpace(VtblPtrAddressSpace);

  toolchain::DIType *vtbl_ptr_type = DBuilder.createPointerType(
...
void CGDebugInfo::emitVTableSymbol(toolchain::GlobalVariable *VTable,
                                   const CXXRecordDecl *RD) {
  if (!CGM.getTarget().getCXXABI().isItaniumFamily() ||
      CGM.getTarget().getTriple().isOSBinFormatCOFF())
    return;
  if (DebugKind <= toolchain::codegenoptions::DebugLineTablesOnly)
    return;
  ...
  toolchain::DIScope *DContext = getContextDescriptor(RD, TheCU);
  auto *Ctxt = cast<toolchain::DICompositeType>(DContext);
  toolchain::DIFile *Unit = getOrCreateFile(Loc);
  toolchain::DIType *VTy = getOrCreateType(VoidPtr, Unit);
  toolchain::DINode::DIFlags Flags = getAccessFlag(AccessSpecifier::AS_private, RD) |
                                toolchain::DINode::FlagArtificial;
  auto Tag = CGM.getCodeGenOpts().DwarfVersion >= 5
                 ? toolchain::dwarf::DW_TAG_variable
                 : toolchain::dwarf::DW_TAG_member;
  toolchain::DIDerivedType *DT = DBuilder.createStaticMemberType(
  ...
  toolchain::DIGlobalVariableExpression *GVE =
...
StringRef CGDebugInfo::getDynamicInitializerName(const VarDecl *VD,
  ...
  StringRef GVName;
  {
    toolchain::raw_svector_ostream OS(QualifiedGV);
  ...
  toolchain::raw_svector_ostream OS(InitName);
  if (!Quals.empty())
    OS << Quals << "::";
  ...
  case DynamicInitKind::GlobalArrayDestructor:
    toolchain_unreachable("not an initializer");
...
void CGDebugInfo::CollectVTableInfo(const CXXRecordDecl *RD, toolchain::DIFile *Unit,
                                    SmallVectorImpl<toolchain::Metadata *> &EltTys) {
  ...
  toolchain::DIType *VPtrTy = nullptr;
  ...
  if (NeedVTableShape) {
    ...
    toolchain::DIType *VTableType = DBuilder.createPointerType(
  ...
  unsigned Size = CGM.getContext().getTypeSize(CGM.getContext().VoidPtrTy);
  toolchain::DIType *VPtrMember =
      DBuilder.createMemberType(Unit, getVTableName(RD), Unit, 0, Size, 0, 0,
                                toolchain::DINode::FlagArtificial, VPtrTy);
  EltTys.push_back(VPtrMember);
}

toolchain::DIType *CGDebugInfo::getOrCreateRecordType(QualType RTy,
                                                 SourceLocation Loc) {
  assert(CGM.getCodeGenOpts().hasReducedDebugInfo());
  toolchain::DIType *T = getOrCreateType(RTy, getOrCreateFile(Loc));
  return T;
}

toolchain::DIType *CGDebugInfo::getOrCreateInterfaceType(QualType D,
                                                    SourceLocation Loc) {
  return getOrCreateStandaloneType(D, Loc);
}

toolchain::DIType *CGDebugInfo::getOrCreateStandaloneType(QualType D,
                                                     SourceLocation Loc) {
  assert(CGM.getCodeGenOpts().hasReducedDebugInfo());
  assert(!D.isNull() && "null type");
  toolchain::DIType *T = getOrCreateType(D, getOrCreateFile(Loc));
...
void CGDebugInfo::addHeapAllocSiteMetadata(toolchain::CallBase *CI,
                                           QualType AllocatedTy,
                                           SourceLocation Loc) {
  if (CGM.getCodeGenOpts().getDebugInfo() <=
      toolchain::codegenoptions::DebugLineTablesOnly)
    return;
  toolchain::MDNode *node;
  if (AllocatedTy->isVoidType())
    node = toolchain::MDNode::get(CGM.getLLVMContext(), {});
  else
...
void CGDebugInfo::completeType(const EnumDecl *ED) {
  ...
  if (I == TypeCache.end() || !cast<toolchain::DIType>(I->second)->isForwardDecl())
    return;
  toolchain::DIType *Res = CreateTypeDefinition(dyn_cast<EnumType>(Ty));
  assert(!Res->isForwardDecl());
  TypeCache[TyPtr].reset(Res);
}

void CGDebugInfo::completeType(const RecordDecl *RD) {
  if (DebugKind > toolchain::codegenoptions::LimitedDebugInfo ||
  ...
  if (auto *CXXRD = dyn_cast<CXXRecordDecl>(RD))
    if (CXXRD->isDynamicClass() &&
        CGM.getVTableLinkage(CXXRD) ==
            toolchain::GlobalValue::AvailableExternallyLinkage &&
...
static bool hasExplicitMemberDefinition(CXXRecordDecl::method_iterator I,
  ...
  for (CXXMethodDecl *MD : toolchain::make_range(I, End))
...
static bool shouldOmitDefinition(toolchain::codegenoptions::DebugInfoKind DebugKind,
...
void CGDebugInfo::completeRequiredType(const RecordDecl *RD) {
  ...
  CanQualType Ty = CGM.getContext().getCanonicalTagType(RD);
  toolchain::DIType *T = getTypeOrNull(Ty);
  if (T && T->isForwardDecl())
    completeClassData(RD);
}

toolchain::DIType *CGDebugInfo::CreateType(const RecordType *Ty) {
  RecordDecl *RD = Ty->getOriginalDecl()->getDefinitionOrSelf();
  toolchain::DIType *T = cast_or_null<toolchain::DIType>(getTypeOrNull(QualType(Ty, 0)));
...
toolchain::DIType *CGDebugInfo::GetPreferredNameType(const CXXRecordDecl *RD,
                                                toolchain::DIFile *Unit) {
  if (!RD)
    return nullptr;
...
CGDebugInfo::CreateTypeDefinition(const RecordType *Ty) {
  ...
  toolchain::DICompositeType *FwdDecl = getOrCreateLimitedType(Ty);
  ...
  toolchain::DINodeArray Elements = DBuilder.getOrCreateArray(EltTys);
  DBuilder.replaceArrays(FwdDecl, Elements);

  if (FwdDecl->isTemporary())
    FwdDecl =
        toolchain::MDNode::replaceWithPermanent(toolchain::TempDICompositeType(FwdDecl));
...
toolchain::DIType *CGDebugInfo::CreateType(const ObjCObjectType *Ty,
                                      toolchain::DIFile *Unit) {
  // Ignore protocols.
  return getOrCreateType(Ty->getBaseType(), Unit);
}

toolchain::DIType *CGDebugInfo::CreateType(const ObjCTypeParamType *Ty,
                                      toolchain::DIFile *Unit) {
  // Ignore protocols.
  SourceLocation Loc = Ty->getDecl()->getLocation();
...
toolchain::DIType *CGDebugInfo::CreateType(const ObjCInterfaceType *Ty,
                                      toolchain::DIFile *Unit) {
  ...
  auto RuntimeLang =
      static_cast<toolchain::dwarf::SourceLanguage>(TheCU->getSourceLanguage());
  ...
  if (DebugTypeExtRefs && ID->isFromASTFile() && ID->getDefinition() &&
    ...
    return DBuilder.createForwardDecl(
        toolchain::dwarf::DW_TAG_structure_type, ID->getName(),
        getDeclContextDescriptor(ID), Unit, 0, RuntimeLang);

  // Get overall information about the record type for the debug info.
  toolchain::DIFile *DefUnit = getOrCreateFile(ID->getLocation());
  ...
  if (!Def || !Def->getImplementation()) {
    toolchain::DIScope *Mod = getParentModuleOrNull(ID);
    toolchain::DIType *FwdDecl = DBuilder.createReplaceableCompositeType(
        toolchain::dwarf::DW_TAG_structure_type, ID->getName(), Mod ? Mod : TheCU,
...
toolchain::DIModule *CGDebugInfo::getOrCreateModuleRef(ASTSourceDescriptor Mod,
  ...
  if (ModRef != ModuleCache.end())
    return cast<toolchain::DIModule>(ModRef->second);

  // Macro definitions that were defined with "-D" on the command line.
  SmallString<128> ConfigMacros;
  {
    toolchain::raw_svector_ostream OS(ConfigMacros);
  ...
  // language::Core::Module object, but it won't actually be built or imported; it will
  // be textual.
  ...
  auto RemapPath = [this](StringRef Path) -> std::string {
    ...
    if (Relative.consume_front(CompDir))
      Relative.consume_front(toolchain::sys::path::get_separator());
...
toolchain::DIType *CGDebugInfo::CreateTypeDefinition(const ObjCInterfaceType *Ty,
                                                toolchain::DIFile *Unit) {
  ObjCInterfaceDecl *ID = Ty->getDecl();
  toolchain::DIFile *DefUnit = getOrCreateFile(ID->getLocation());
  unsigned Line = getLineNumber(ID->getLocation());
  ...
  if (ID->getImplementation())
    Flags |= toolchain::DINode::FlagObjcClassComplete;

  toolchain::DIScope *Mod = getParentModuleOrNull(ID);
  toolchain::DICompositeType *RealDecl = DBuilder.createStructType(
      Mod ? Mod : Unit, ID->getName(), DefUnit, Line, Size, Align, Flags,
      nullptr, toolchain::DINodeArray(), RuntimeLang);
  ...
  if (SClass) {
    toolchain::DIType *SClassTy =
        getOrCreateType(CGM.getContext().getObjCInterfaceType(SClass), Unit);
    if (!SClassTy)
      return nullptr;

    toolchain::DIType *InhTag = DBuilder.createInheritance(RealDecl, SClassTy, 0, 0,
                                                      toolchain::DINode::FlagZero);
  ...
  auto AddProperty = [&](const ObjCPropertyDecl *PD) {
    ...
    toolchain::DIFile *PUnit = getOrCreateFile(Loc);
    unsigned PLine = getLineNumber(Loc);
    ObjCMethodDecl *Getter = PD->getGetterMethodDecl();
    ObjCMethodDecl *Setter = PD->getSetterMethodDecl();
    toolchain::MDNode *PropertyNode = DBuilder.createObjCProperty(
  ...
  };
  {
    ...
    toolchain::DenseSet<IsClassAndIdent> PropertySet;
  ...
  for (ObjCIvarDecl *Field = ID->all_declared_ivar_begin(); Field;
    ...
    toolchain::DIType *FieldTy = getOrCreateType(Field->getType(), Unit);
    if (!FieldTy)
      return nullptr;
    ...
    // Get the location for the field.
    toolchain::DIFile *FieldDefUnit = getOrCreateFile(Field->getLocation());
    ...
    if (Field->getAccessControl() == ObjCIvarDecl::Protected)
      Flags = toolchain::DINode::FlagProtected;
    else if (Field->getAccessControl() == ObjCIvarDecl::Private)
      Flags = toolchain::DINode::FlagPrivate;
    else if (Field->getAccessControl() == ObjCIvarDecl::Public)
      Flags = toolchain::DINode::FlagPublic;

    if (Field->isBitField())
      Flags |= toolchain::DINode::FlagBitField;

    toolchain::MDNode *PropertyNode = nullptr;
...
toolchain::DIType *CGDebugInfo::CreateType(const VectorType *Ty,
                                      toolchain::DIFile *Unit) {
  ...
  toolchain::DIType *ElementTy = getOrCreateType(Ty->getElementType(), Unit);
  int64_t Count = Ty->getNumElements();

  toolchain::Metadata *Subscript;
  QualType QTy(Ty, 0);
  ...
  if (SizeExpr != SizeExprCache.end())
    ...
    auto *CountNode =
        toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
            toolchain::Type::getInt64Ty(CGM.getLLVMContext()), Count ? Count : -1));
    Subscript = DBuilder.getOrCreateSubrange(
...
toolchain::DIType *CGDebugInfo::CreateType(const ConstantMatrixType *Ty,
                                      toolchain::DIFile *Unit) {
  ...
  toolchain::SmallVector<toolchain::Metadata *, 2> Subscripts;
  auto *ColumnCountNode =
      toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
          toolchain::Type::getInt64Ty(CGM.getLLVMContext()), Ty->getNumColumns()));
  auto *RowCountNode =
      toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
          toolchain::Type::getInt64Ty(CGM.getLLVMContext()), Ty->getNumRows()));
  ...
  Subscripts.push_back(DBuilder.getOrCreateSubrange(
      ...
      nullptr /*stride*/));
  toolchain::DINodeArray SubscriptArray = DBuilder.getOrCreateArray(Subscripts);
  return DBuilder.createArrayType(Size, Align, ElementTy, SubscriptArray);
}

toolchain::DIType *CGDebugInfo::CreateType(const ArrayType *Ty, toolchain::DIFile *Unit) {
  uint64_t Size;
  uint32_t Align;
  ...
  SmallVector<toolchain::Metadata *, 8> Subscripts;
  QualType EltTy(Ty, 0);
  while ((Ty = dyn_cast<ArrayType>(EltTy))) {
    ...
    if (SizeNode != SizeExprCache.end())
      ...
      auto *CountNode =
          toolchain::ConstantAsMetadata::get(toolchain::ConstantInt::getSigned(
              toolchain::Type::getInt64Ty(CGM.getLLVMContext()), Count));
      Subscripts.push_back(DBuilder.getOrCreateSubrange(
          CountNode /*count*/, nullptr /*lowerBound*/, nullptr /*upperBound*/,
          nullptr /*stride*/));
    }
...
toolchain::DIType *CGDebugInfo::CreateType(const LValueReferenceType *Ty,
                                      toolchain::DIFile *Unit) {
  return CreatePointerLikeType(toolchain::dwarf::DW_TAG_reference_type, Ty,
                               Ty->getPointeeType(), Unit);
}

toolchain::DIType *CGDebugInfo::CreateType(const RValueReferenceType *Ty,
                                      toolchain::DIFile *Unit) {
  toolchain::dwarf::Tag Tag = toolchain::dwarf::DW_TAG_rvalue_reference_type;
  // DW_TAG_rvalue_reference_type was introduced in DWARF 4.
  if (CGM.getCodeGenOpts().DebugStrictDwarf &&
      CGM.getCodeGenOpts().DwarfVersion < 4)
    Tag = toolchain::dwarf::DW_TAG_reference_type;

  return CreatePointerLikeType(Tag, Ty, Ty->getPointeeType(), Unit);
}

toolchain::DIType *CGDebugInfo::CreateType(const MemberPointerType *Ty,
                                      toolchain::DIFile *U) {
  toolchain::DINode::DIFlags Flags = toolchain::DINode::FlagZero;
  uint64_t Size = 0;
  ...
  if (!Ty->isIncompleteType()) {
    ...
    if (CGM.getTarget().getCXXABI().isMicrosoft()) {
      ...
      case MSInheritanceModel::Single:
        Flags |= toolchain::DINode::FlagSingleInheritance;
        break;
      case MSInheritanceModel::Multiple:
        Flags |= toolchain::DINode::FlagMultipleInheritance;
        break;
      case MSInheritanceModel::Virtual:
        Flags |= toolchain::DINode::FlagVirtualInheritance;
        break;
  ...
  toolchain::DIType *ClassType = getOrCreateType(T, U);
...
toolchain::DIType *CGDebugInfo::CreateType(const AtomicType *Ty, toolchain::DIFile *U) {
  auto *FromTy = getOrCreateType(Ty->getValueType(), U);
  return DBuilder.createQualifiedType(toolchain::dwarf::DW_TAG_atomic_type, FromTy);
}

toolchain::DIType *CGDebugInfo::CreateType(const PipeType *Ty, toolchain::DIFile *U) {
  return getOrCreateType(Ty->getElementType(), U);
}

toolchain::DIType *CGDebugInfo::CreateType(const HLSLAttributedResourceType *Ty,
                                      toolchain::DIFile *U) {
  return getOrCreateType(Ty->getWrappedType(), U);
}

toolchain::DIType *CGDebugInfo::CreateType(const HLSLInlineSpirvType *Ty,
                                      toolchain::DIFile *U) {
  // Debug information unneeded.
  return nullptr;
}

static auto getEnumInfo(CodeGenModule &CGM, toolchain::DICompileUnit *TheCU,
                        const EnumType *Ty) {
...
toolchain::DIType *CGDebugInfo::CreateEnumType(const EnumType *Ty) {
  ...
  if (isImportedFromModule || !ED->getDefinition()) {
    ...
    // complete type.
    toolchain::DIScope *EDContext = getDeclContextDescriptor(ED);
    toolchain::DIFile *DefUnit = getOrCreateFile(ED->getLocation());
    toolchain::TempDIScope TmpContext(DBuilder.createReplaceableCompositeType(
        toolchain::dwarf::DW_TAG_enumeration_type, "", TheCU, DefUnit, 0));

    unsigned Line = getLineNumber(ED->getLocation());
    StringRef EDName = ED->getName();
    toolchain::DIType *RetTy = DBuilder.createReplaceableCompositeType(
        toolchain::dwarf::DW_TAG_enumeration_type, EDName, EDContext, DefUnit, Line,
        0, Size, Align, toolchain::DINode::FlagFwdDecl, Identifier);
...
toolchain::DIType *CGDebugInfo::CreateTypeDefinition(const EnumType *Ty) {
  auto [ED, Size, Align, Identifier] = getEnumInfo(CGM, TheCU, Ty);

  SmallVector<toolchain::Metadata *, 16> Enumerators;
  ED = ED->getDefinition();
  ...
  toolchain::DINodeArray EltArray = DBuilder.getOrCreateArray(Enumerators);

  toolchain::DIFile *DefUnit = getOrCreateFile(ED->getLocation());
  unsigned Line = getLineNumber(ED->getLocation());
  toolchain::DIScope *EnumContext = getDeclContextDescriptor(ED);
  toolchain::DIType *ClassTy = getOrCreateType(ED->getIntegerType(), DefUnit);
  return DBuilder.createEnumerationType(
      EnumContext, ED->getName(), DefUnit, Line, Size, Align, EltArray, ClassTy,
      /*RunTimeLang=*/0, Identifier, ED->isScoped(), EnumKind);
}

toolchain::DIMacro *CGDebugInfo::CreateMacro(toolchain::DIMacroFile *Parent,
...
toolchain::DIMacroFile *CGDebugInfo::CreateTempMacroFile(toolchain::DIMacroFile *Parent,
                                                    SourceLocation LineLoc,
                                                    SourceLocation FileLoc) {
  toolchain::DIFile *FName = getOrCreateFile(FileLoc);
  unsigned Line = LineLoc.isInvalid() ? 0 : getLineNumber(LineLoc);
  return DBuilder.createTempMacroFile(Parent, Line, FName);
}

toolchain::DILocation *CGDebugInfo::CreateSyntheticInlineAt(toolchain::DebugLoc Location,
                                                       StringRef FuncName) {
  toolchain::DISubprogram *SP =
      createInlinedSubprogram(FuncName, Location->getFile());
  return toolchain::DILocation::get(CGM.getLLVMContext(), /*Line=*/0, /*Column=*/0,
                               /*Scope=*/SP, /*InlinedAt=*/Location);
}

toolchain::DILocation *CGDebugInfo::CreateTrapFailureMessageFor(
    toolchain::DebugLoc TrapLocation, StringRef Category, StringRef FailureMsg) {
...
toolchain::DIType *CGDebugInfo::getTypeOrNull(QualType Ty) {
  assert(Ty == UnwrapTypeForDebugInfo(Ty, CGM.getContext()));
  auto It = TypeCache.find(Ty.getAsOpaquePtr());
  if (It != TypeCache.end()) {
    // Verify that the debug info still exists.
    if (toolchain::Metadata *V = It->second)
      return cast<toolchain::DIType>(V);
  }
...
toolchain::DIType *CGDebugInfo::getOrCreateType(QualType Ty, toolchain::DIFile *Unit) {
  if (Ty.isNull())
    return nullptr;

  toolchain::TimeTraceScope TimeScope("DebugType", [&]() {
    std::string Name;
    toolchain::raw_string_ostream OS(Name);
  ...
  toolchain::DIType *Res = CreateTypeNode(Ty, Unit);
  void *TyPtr = Ty.getAsOpaquePtr();
...
toolchain::DIModule *CGDebugInfo::getParentModuleOrNull(const Decl *D) {
...
toolchain::DIType *CGDebugInfo::CreateTypeNode(QualType Ty, toolchain::DIFile *Unit) {
...
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("Dependent types cannot show up in debug information");
  ...
  toolchain_unreachable("type should have been unwrapped!");
}
...
CGDebugInfo::getOrCreateLimitedType(const RecordType *Ty) {
  ...
  auto *T = cast_or_null<toolchain::DICompositeType>(getTypeOrNull(QTy));
  ...
  toolchain::DICompositeType *Res = CreateLimitedType(Ty);

  // Propagate members from the declaration to the definition
  // CreateType(const RecordType*) will overwrite this with the members in the
  // correct order if the full type is needed.
  DBuilder.replaceArrays(Res, T ? T->getElements() : toolchain::DINodeArray());
...
toolchain::DICompositeType *CGDebugInfo::CreateLimitedType(const RecordType *Ty) {
  RecordDecl *RD = Ty->getOriginalDecl()->getDefinitionOrSelf();

  // Get overall information about the record type for the debug info.
  StringRef RDName = getClassName(RD);
  const SourceLocation Loc = RD->getLocation();
  toolchain::DIFile *DefUnit = nullptr;
  unsigned Line = 0;
  if (Loc.isValid()) {
  ...
  toolchain::DIScope *RDContext = getDeclContextDescriptor(RD);

  // If we ended up creating the type during the context chain construction,
  // just return that.
  auto *T = cast_or_null<toolchain::DICompositeType>(
  ...
  // records.
  auto Flags = toolchain::DINode::FlagZero;
  if (auto CXXRD = dyn_cast<CXXRecordDecl>(RD)) {
    if (CGM.getCXXABI().getRecordArgABI(CXXRD) == CGCXXABI::RAA_Indirect)
      Flags |= toolchain::DINode::FlagTypePassByReference;
    else
      Flags |= toolchain::DINode::FlagTypePassByValue;
    ...
    if (CXXRD->isAnonymousStructOrUnion())
        Flags |= toolchain::DINode::FlagExportSymbols;

    Flags |= getAccessFlag(CXXRD->getAccess(),
                           dyn_cast<CXXRecordDecl>(CXXRD->getDeclContext()));
  }

  toolchain::DINodeArray Annotations = CollectBTFDeclTagAnnotations(D);
  toolchain::DICompositeType *RealDecl = DBuilder.createReplaceableCompositeType(
  ...
  default:
    toolchain_unreachable("invalid composite type tag");

  case toolchain::dwarf::DW_TAG_array_type:
  ...
  case toolchain::dwarf::DW_TAG_class_type:
    ...
    RealDecl =
        toolchain::MDNode::replaceWithDistinct(toolchain::TempDICompositeType(RealDecl));
    break;
  }
  ...
  if (const auto *TSpecial = dyn_cast<ClassTemplateSpecializationDecl>(RD))
    DBuilder.replaceArrays(RealDecl, toolchain::DINodeArray(),
                           CollectCXXTemplateParams(TSpecial, DefUnit));
  return RealDecl;
}

void CGDebugInfo::CollectContainingType(const CXXRecordDecl *RD,
                                        toolchain::DICompositeType *RealDecl) {
...
toolchain::DIType *CGDebugInfo::CreateMemberType(toolchain::DIFile *Unit, QualType FType,
                                            StringRef Name, uint64_t *Offset) {
  toolchain::DIType *FieldTy = CGDebugInfo::getOrCreateType(FType, Unit);
  ...
  toolchain::DIType *Ty =
      DBuilder.createMemberType(Unit, Name, Unit, 0, FieldSize, FieldAlign,
                                *Offset, toolchain::DINode::FlagZero, FieldTy);
  *Offset += FieldSize;
  return Ty;
}

void CGDebugInfo::collectFunctionDeclProps(GlobalDecl GD, toolchain::DIFile *Unit,
                                           StringRef &Name,
  ...
  if (LinkageName == Name ||
      (CGM.getCodeGenOpts().CoverageNotesFile.empty() &&
       ...
       DebugKind <= toolchain::codegenoptions::DebugLineTablesOnly))
  ...
  if (CGM.getCodeGenOpts().hasReducedDebugInfo() ||
    ...
    if (const NamespaceDecl *NSDecl =
      ...
      toolchain::DIScope *Mod = getParentModuleOrNull(RDecl);
...
void CGDebugInfo::collectVarDeclProps(const VarDecl *VD, toolchain::DIFile *&Unit,
  ...
  if (isa<VarTemplateSpecializationDecl>(VD)) {
    toolchain::DINodeArray parameterNodes = CollectVarTemplateParams(VD, &*Unit);
  ...
  toolchain::DIScope *Mod = getParentModuleOrNull(VD);
  VDContext = getContextDescriptor(cast<Decl>(DC), Mod ? Mod : TheCU);
}

toolchain::DISubprogram *CGDebugInfo::getFunctionFwdDeclOrStub(GlobalDecl GD,
                                                          bool Stub) {
  toolchain::DINodeArray TParamsArray;
  ...
  toolchain::DIFile *Unit = getOrCreateFile(Loc);
  toolchain::DIScope *DContext = Unit;
  unsigned Line = getLineNumber(Loc);
  ...
  toolchain::DISubprogram *SP = DBuilder.createTempFunctionFwdDecl(
      DContext, Name, LinkageName, Unit, Line,
      getOrCreateFunctionType(GD.getDecl(), FnType, Unit), 0, Flags, SPFlags,
...
toolchain::DISubprogram *CGDebugInfo::getFunctionForwardDeclaration(GlobalDecl GD) {
  return getFunctionFwdDeclOrStub(GD, /* Stub = */ false);
}

toolchain::DISubprogram *CGDebugInfo::getFunctionStub(GlobalDecl GD) {
...
CGDebugInfo::getGlobalVariableForwardDeclaration(const VarDecl *VD) {
  ...
  toolchain::DIScope *DContext = Unit;
  unsigned Line = getLineNumber(Loc);
  toolchain::MDTuple *TemplateParameters = nullptr;
  ...
  FwdDeclReplaceMap.emplace_back(
      ...
      std::make_tuple(static_cast<toolchain::Metadata *>(GV)));
  return GV;
}

toolchain::DINode *CGDebugInfo::getDeclarationOrDefinition(const Decl *D) {
  ...
  if (I != DeclCache.end()) {
    auto N = I->second;
    if (auto *GVE = dyn_cast_or_null<toolchain::DIGlobalVariableExpression>(N))
      return GVE->getVariable();
    return cast<toolchain::DINode>(N);
  }
  ...
  if (IE != ImportedDeclCache.end()) {
    auto N = IE->second;
    if (auto *GVE = dyn_cast_or_null<toolchain::DIImportedEntity>(N))
      return cast<toolchain::DINode>(GVE);
    return dyn_cast_or_null<toolchain::DINode>(N);
  }
...
toolchain::DISubprogram *CGDebugInfo::getFunctionDeclaration(const Decl *D) {
  if (!D || DebugKind <= toolchain::codegenoptions::DebugLineTablesOnly)
    return nullptr;
  ...
  if (MI == SPCache.end()) {
    if (const auto *MD = dyn_cast<CXXMethodDecl>(FD->getCanonicalDecl())) {
      return CreateCXXMemberFunction(MD, getOrCreateFile(MD->getLocation()),
                                     cast<toolchain::DICompositeType>(S));
    }
  }
  if (MI != SPCache.end()) {
    auto *SP = dyn_cast_or_null<toolchain::DISubprogram>(MI->second);
...
toolchain::DISubprogram *CGDebugInfo::getObjCMethodDeclaration(
    const Decl *D, toolchain::DISubroutineType *FnType, unsigned LineNo,
    toolchain::DINode::DIFlags Flags, toolchain::DISubprogram::DISPFlags SPFlags) {
  if (!D || DebugKind <= toolchain::codegenoptions::DebugLineTablesOnly)
    return nullptr;
  ...
  if (OMD->isDirectMethod())
    SPFlags |= toolchain::DISubprogram::SPFlagObjCDirect;
  ...
  if (It == TypeCache.end())
    return nullptr;
  auto *InterfaceType = cast<toolchain::DICompositeType>(It->second);
  toolchain::DISubprogram *FD = DBuilder.createFunction(
...
toolchain::DISubroutineType *CGDebugInfo::getOrCreateFunctionType(const Decl *D,
                                                             QualType FnType,
                                                             toolchain::DIFile *F) {
  ...
  if (const auto *OMethod = dyn_cast<ObjCMethodDecl>(D)) {
    ...
    toolchain::DITypeRefArray EltTypeArray = DBuilder.getOrCreateTypeArray(Elts);
    return DBuilder.createSubroutineType(EltTypeArray, toolchain::DINode::FlagZero,
                                         getDwarfCC(CC));
  }
  ...
  return cast<toolchain::DISubroutineType>(getOrCreateType(FnType, F));
}
...
void CGDebugInfo::emitFunctionStart(GlobalDecl GD, SourceLocation Loc,
                                    ...
                                    toolchain::Function *Fn, bool CurFuncIsThunk) {
  StringRef Name;
  StringRef LinkageName;
  ...
  toolchain::DISubprogram::DISPFlags SPFlags = toolchain::DISubprogram::SPFlagZero;
  toolchain::DIFile *Unit = getOrCreateFile(Loc);
  toolchain::DIScope *FDContext = Unit;
  toolchain::DINodeArray TParamsArray;
  bool KeyInstructions = CGM.getCodeGenOpts().DebugKeyInstructions;
  if (!HasDecl) {
    // Use toolchain function name.
    LinkageName = Fn->getName();
  } else if (const auto *FD = dyn_cast<FunctionDecl>(D)) {
    // If there is a subprogram for this function available then use it.
    auto FI = SPCache.find(FD->getCanonicalDecl());
    if (FI != SPCache.end()) {
      auto *SP = dyn_cast_or_null<toolchain::DISubprogram>(FI->second);
      if (SP && SP->isDefinition()) {
  ...
  if (CurFuncIsThunk)
    Flags |= toolchain::DINode::FlagThunk;
  ...
  toolchain::DINode::DIFlags FlagsForDef = Flags | getCallSiteRelatedAttrs();
  toolchain::DISubprogram::DISPFlags SPFlagsForDef =
      SPFlags | toolchain::DISubprogram::SPFlagDefinition;

  const unsigned LineNo = getLineNumber(Loc.isValid() ? Loc : CurLoc);
  unsigned ScopeLine = getLineNumber(ScopeLoc);
  toolchain::DISubroutineType *DIFnType = getOrCreateFunctionType(D, FnType, Unit);
  toolchain::DISubprogram *Decl = nullptr;
  toolchain::DINodeArray Annotations = nullptr;
  if (D) {
  ...
  toolchain::DISubprogram *SP = DBuilder.createFunction(
...
void CGDebugInfo::EmitFunctionDecl(GlobalDecl GD, SourceLocation Loc,
                                   QualType FnType, toolchain::Function *Fn) {
  StringRef Name;
  StringRef LinkageName;
  ...
  toolchain::TimeTraceScope TimeScope("DebugFunction", [&]() {
  ...
  if (isa<FunctionDecl>(D)) {
    ...
    toolchain_unreachable("not a function or ObjC method");
  }
  Name.consume_front("\01");
  ...
  toolchain::DINodeArray Annotations = CollectBTFDeclTagAnnotations(D);
  toolchain::DISubroutineType *STy = getOrCreateFunctionType(D, FnType, Unit);
  // Key Instructions: Don't set flag on declarations.
  assert(~SPFlags & toolchain::DISubprogram::SPFlagDefinition);
  toolchain::DISubprogram *SP = DBuilder.createFunction(
  ...
  if (IsDeclForCallSite && CGM.getTarget().getTriple().isBPF()) {
    if (auto *FD = dyn_cast<FunctionDecl>(D)) {
      toolchain::DITypeRefArray ParamTypes = STy->getTypeArray();
      unsigned ArgNo = 1;
      for (ParmVarDecl *PD : FD->parameters()) {
        toolchain::DINodeArray ParamAnnotations = CollectBTFDeclTagAnnotations(PD);
        DBuilder.createParameterVariable(
            SP, PD->getName(), ArgNo, Unit, LineNo, ParamTypes[ArgNo], true,
            toolchain::DINode::FlagZero, ParamAnnotations);
        ++ArgNo;
      }
    }
  }
...
void CGDebugInfo::EmitFuncDeclForCallSite(toolchain::CallBase *CallOrInvoke,
                                          QualType CalleeType,
                                          const FunctionDecl *CalleeDecl) {
  if (!CallOrInvoke)
    return;
  auto *Func = dyn_cast<toolchain::Function>(CallOrInvoke->getCalledOperand());
  if (!Func)
    return;
  ...
  if (CalleeDecl->hasAttr<NoDebugAttr>() ||
      getCallSiteRelatedAttrs() == toolchain::DINode::FlagZero)
    return;
...
void CGDebugInfo::EmitInlineFunctionStart(CGBuilderTy &Builder, GlobalDecl GD) {
  ...
  // If there is a subprogram for this function available then use it.
  auto FI = SPCache.find(FD->getCanonicalDecl());
  toolchain::DISubprogram *SP = nullptr;
  if (FI != SPCache.end())
    SP = dyn_cast_or_null<toolchain::DISubprogram>(FI->second);
  if (!SP || !SP->isDefinition())
...
void CGDebugInfo::EmitInlineFunctionEnd(CGBuilderTy &Builder) {
  ...
  setInlinedAt(toolchain::DebugLoc(CurInlinedAt).getInlinedAt());
}
...
void CGDebugInfo::EmitLocation(CGBuilderTy &Builder, SourceLocation Loc) {
  ...
  toolchain::MDNode *Scope = LexicalBlockStack.back();
  Builder.SetCurrentDebugLocation(
      toolchain::DILocation::get(CGM.getLLVMContext(), getLineNumber(CurLoc),
                            getColumnNumber(CurLoc), Scope, CurInlinedAt));
}

void CGDebugInfo::CreateLexicalBlock(SourceLocation Loc) {
  toolchain::MDNode *Back = nullptr;
  if (!LexicalBlockStack.empty())
    Back = LexicalBlockStack.back().get();
  LexicalBlockStack.emplace_back(DBuilder.createLexicalBlock(
      cast<toolchain::DIScope>(Back), getOrCreateFile(CurLoc), getLineNumber(CurLoc),
...
void CGDebugInfo::AppendAddressSpaceXDeref(
  ...
  Expr.push_back(toolchain::dwarf::DW_OP_constu);
  Expr.push_back(*DWARFAddressSpace);
  Expr.push_back(toolchain::dwarf::DW_OP_swap);
  Expr.push_back(toolchain::dwarf::DW_OP_xderef);
}
...
void CGDebugInfo::EmitFunctionEnd(CGBuilderTy &Builder, toolchain::Function *Fn) {
  assert(!LexicalBlockStack.empty() && "Region stack mismatch, stack empty!");
...
CGDebugInfo::EmitTypeForVarWithBlocksAttr(const VarDecl *VD,
                                          uint64_t *XOffset) {
  SmallVector<toolchain::Metadata *, 5> EltTys;
  QualType FType;
  uint64_t FieldSize, FieldOffset;
  uint32_t FieldAlign;

  toolchain::DIFile *Unit = getOrCreateFile(VD->getLocation());
  ...
  if (Align > CGM.getContext().toCharUnitsFromBits(
    ...
    if (NumPaddingBytes.isPositive()) {
      toolchain::APInt pad(32, NumPaddingBytes.getQuantity());
  ...
  FType = Type;
  toolchain::DIType *WrappedTy = getOrCreateType(FType, Unit);
  FieldSize = CGM.getContext().getTypeSize(FType);
  FieldAlign = CGM.getContext().toBits(Align);

  *XOffset = FieldOffset;
  toolchain::DIType *FieldTy = DBuilder.createMemberType(
      Unit, VD->getName(), Unit, 0, FieldSize, FieldAlign, FieldOffset,
      toolchain::DINode::FlagZero, WrappedTy);
  EltTys.push_back(FieldTy);
  FieldOffset += FieldSize;

  toolchain::DINodeArray Elements = DBuilder.getOrCreateArray(EltTys);
  return {DBuilder.createStructType(Unit, "", Unit, 0, FieldOffset, 0,
                                    toolchain::DINode::FlagZero, nullptr, Elements),
          WrappedTy};
}

toolchain::DILocalVariable *CGDebugInfo::EmitDeclare(const VarDecl *VD,
                                                toolchain::Value *Storage,
  ...
  if (const auto *IPD = dyn_cast<ImplicitParamDecl>(VD)) {
    if (IPD->getParameterKind() == ImplicitParamKind::CXXThis ||
      ...
      Flags |= toolchain::DINode::FlagObjectPointer;
  ...
  if (!Name.empty()) {
    ...
    if (VD->isEscapingByref()) {
      ...
      Expr.push_back(toolchain::dwarf::DW_OP_plus_uconst);
      // offset of __forwarding field
      offset = CGM.getContext().toCharUnitsFromBits(
          CGM.getTarget().getPointerWidth(LangAS::Default));
      Expr.push_back(offset.getQuantity());
      Expr.push_back(toolchain::dwarf::DW_OP_deref);
      Expr.push_back(toolchain::dwarf::DW_OP_plus_uconst);
      // offset of x field
    ...
    if (RD->isUnion() && RD->isAnonymousStructOrUnion()) {
      ...
      for (const auto *Field : RD->fields()) {
        ...
        auto *D = DBuilder.createAutoVariable(
            ...
            Flags | toolchain::DINode::FlagArtificial, FieldAlign);

        // Insert an toolchain.dbg.declare into the current block.
        DBuilder.insertDeclare(Storage, D, DBuilder.createExpression(Expr),
                               toolchain::DILocation::get(CGM.getLLVMContext(), Line,
                                                     Column, Scope,
                                                     CurInlinedAt),
  ...
  if (UsePointerValue) {
    assert(!toolchain::is_contained(Expr, toolchain::dwarf::DW_OP_deref) &&
           "Debug info already contains DW_OP_deref.");
    Expr.push_back(toolchain::dwarf::DW_OP_deref);
  }

  // Create the descriptor for the variable.
  toolchain::DILocalVariable *D = nullptr;
  if (ArgNo) {
    toolchain::DINodeArray Annotations = CollectBTFDeclTagAnnotations(VD);
    ...
    auto RemapCoroArgToLocalVar = [&]() -> toolchain::DILocalVariable * {
      // The scope of parameter and move-parameter should be distinct
      // DISubprogram.
      if (!isa<toolchain::DISubprogram>(Scope) || !Scope->isDistinct())
        return nullptr;

      auto Iter = toolchain::find_if(CoroutineParameterMappings, [&](auto &Pair) {
      ...
      if (Iter != CoroutineParameterMappings.end()) {
        ...
        auto Iter2 = toolchain::find_if(ParamDbgMappings, [&](auto &DbgPair) {
          return DbgPair.first == PD && DbgPair.second->getScope() == Scope;
        });
        if (Iter2 != ParamDbgMappings.end())
          return const_cast<toolchain::DILocalVariable *>(Iter2->second);
      }
      return nullptr;
    };
...
toolchain::DILocalVariable *CGDebugInfo::EmitDeclare(const BindingDecl *BD,
                                                toolchain::Value *Storage,
                                                std::optional<unsigned> ArgNo,
                                                CGBuilderTy &Builder,
                                                const bool UsePointerValue) {
  ...
  toolchain::DIFile *Unit = getOrCreateFile(BD->getLocation());
  toolchain::DIType *Ty = getOrCreateType(BD->getType(), Unit);
    ...
    if (const FieldDecl *FD = dyn_cast<FieldDecl>(ME->getMemberDecl())) {
      ...
      if (FD->isBitField()) {
        ...
        Expr.push_back(Info.IsSigned
                           ? toolchain::dwarf::DW_OP_LLVM_extract_bits_sext
                           : toolchain::dwarf::DW_OP_LLVM_extract_bits_zext);
...
CGDebugInfo::EmitDeclareOfAutoVariable(const VarDecl *VD, toolchain::Value *Storage,
                                       CGBuilderTy &Builder,
                                       const bool UsePointerValue) {
  assert(CGM.getCodeGenOpts().hasReducedDebugInfo());
  ...
  if (auto *DD = dyn_cast<DecompositionDecl>(VD)) {
    ...
    // Don't emit an toolchain.dbg.declare for the composite storage as it doesn't
...
void CGDebugInfo::EmitLabel(const LabelDecl *D, CGBuilderTy &Builder) {
  ...
  auto *Scope = cast<toolchain::DIScope>(LexicalBlockStack.back());
  toolchain::DIFile *Unit = getOrCreateFile(D->getLocation());
  ...
  // Insert an toolchain.dbg.label into the current block.
  DBuilder.insertLabel(L,
                       toolchain::DILocation::get(CGM.getLLVMContext(), Line, Column,
                                             Scope, CurInlinedAt),
                       Builder.GetInsertBlock()->end());
}

toolchain::DIType *CGDebugInfo::CreateSelfType(const QualType &QualTy,
                                          toolchain::DIType *Ty) {
  toolchain::DIType *CachedTy = getTypeOrNull(QualTy);
  if (CachedTy)
    Ty = CachedTy;
...
void CGDebugInfo::EmitDeclareOfBlockDeclRefVariable(
    const VarDecl *VD, toolchain::Value *Storage, CGBuilderTy &Builder,
    const CGBlockInfo &blockInfo, toolchain::Instruction *InsertPoint) {
  assert(CGM.getCodeGenOpts().hasReducedDebugInfo());
  assert(!LexicalBlockStack.empty() && "Region stack mismatch, stack empty!");
  ...
  const toolchain::DataLayout &target = CGM.getDataLayout();
  ...
  addr.push_back(toolchain::dwarf::DW_OP_deref);
  addr.push_back(toolchain::dwarf::DW_OP_plus_uconst);
  ...
  auto Align = getDeclAlignIfRequired(VD, CGM.getContext());
  auto *D = DBuilder.createAutoVariable(
      cast<toolchain::DILocalScope>(LexicalBlockStack.back()), VD->getName(), Unit,
      Line, Ty, false, toolchain::DINode::FlagZero, Align);

  // Insert an toolchain.dbg.declare into the current block.
  auto DL = toolchain::DILocation::get(CGM.getLLVMContext(), Line, Column,
                                  LexicalBlockStack.back(), CurInlinedAt);
...
toolchain::DILocalVariable *
CGDebugInfo::EmitDeclareOfArgVariable(const VarDecl *VD, toolchain::Value *AI,
...
void CGDebugInfo::EmitDeclareOfBlockLiteralArgVariable(const CGBlockInfo &block,
  ...
  toolchain::DIFile *tunit = getOrCreateFile(loc);
  unsigned line = getLineNumber(loc);
  ...
  const toolchain::StructLayout *blockLayout =
      CGM.getDataLayout().getStructLayout(block.StructureType);

  SmallVector<toolchain::Metadata *, 16> fields;
  ...
  // Sort by offset.
  toolchain::array_pod_sort(chunks.begin(), chunks.end());
  ...
  for (const BlockLayoutChunk &Chunk : chunks) {
    ...
    if (!capture) {
      ...
      if (auto *Method =
        ...
        toolchain_unreachable("unexpected block declcontext");
    ...
    if (capture->isByRef()) {
      ...
      fieldType = DBuilder.createMemberType(tunit, name, tunit, line,
                                            ...
                                            toolchain::DINode::FlagZero, fieldType);
    } else {
  ...
  toolchain::DINodeArray fieldsArray = DBuilder.getOrCreateArray(fields);

  toolchain::DIType *type =
      DBuilder.createStructType(tunit, typeName.str(), tunit, line,
                                CGM.getContext().toBits(block.BlockSize), 0,
                                toolchain::DINode::FlagZero, nullptr, fieldsArray);
  type = DBuilder.createPointerType(type, CGM.PointerWidthInBits);

  // Get overall information about the block.
  toolchain::DINode::DIFlags flags = toolchain::DINode::FlagArtificial;
  auto *scope = cast<toolchain::DILocalScope>(LexicalBlockStack.back());
...
CGDebugInfo::getOrCreateStaticDataMemberDeclarationOrNull(const VarDecl *D) {
  ...
  auto *Ctxt = cast<toolchain::DICompositeType>(getDeclContextDescriptor(D));
  return CreateRecordStaticField(D, Ctxt, cast<RecordDecl>(DC));
}

toolchain::DIGlobalVariableExpression *CGDebugInfo::CollectAnonRecordDecls(
    const RecordDecl *RD, toolchain::DIFile *Unit, unsigned LineNo,
    StringRef LinkageName, toolchain::GlobalVariable *Var, toolchain::DIScope *DContext) {
  toolchain::DIGlobalVariableExpression *GVE = nullptr;
...
static bool ReferencesAnonymousEntity(ArrayRef<TemplateArgument> Args) {
  return toolchain::any_of(Args, [&](const TemplateArgument &TA) {
    switch (TA.getKind()) {
    case TemplateArgument::Pack:
...
bool CGDebugInfo::HasReconstitutableArgs(
  ...
  return toolchain::all_of(Args, [&](const TemplateArgument &TA) {
    switch (TA.getKind()) {
    case TemplateArgument::Template:
    ...
    default:
      toolchain_unreachable("Other, unresolved, template arguments should "
                       "not be seen here");
    }
  });
}
...
std::string CGDebugInfo::GetName(const Decl *D, bool Qualified) const {
  ...
  if (!ND)
    return Name;
  toolchain::codegenoptions::DebugTemplateNamesKind TemplateNamesKind =
      CGM.getCodeGenOpts().getDebugSimpleTemplateNames();

  if (!CGM.getCodeGenOpts().hasReducedDebugInfo())
    TemplateNamesKind = toolchain::codegenoptions::DebugTemplateNamesKind::Full;
  ...
  if (TemplateNamesKind == toolchain::codegenoptions::DebugTemplateNamesKind::Full ||
      !Reconstitutable) {
    ND->getNameForDiagnostic(OS, PP, Qualified);
  } else {
    bool Mangled = TemplateNamesKind ==
                   toolchain::codegenoptions::DebugTemplateNamesKind::Mangled;
    ...
    toolchain::raw_string_ostream EncodedOriginalNameOS(EncodedOriginalName);
    ...
    if (Mangled) {
      ...
      toolchain::raw_string_ostream OriginalOS(CanonicalOriginalName);
...
void CGDebugInfo::EmitGlobalVariable(toolchain::GlobalVariable *Var,
                                     const VarDecl *D) {
  assert(CGM.getCodeGenOpts().hasReducedDebugInfo());
  if (D->hasAttr<NoDebugAttr>())
    return;

  toolchain::TimeTraceScope TimeScope("DebugGlobalVariable", [&]() {
    return GetName(D, true);
  });

  // If we already created a DIGlobalVariable for this declaration, just attach
  // it to the toolchain::GlobalVariable.
  auto Cached = DeclCache.find(D->getCanonicalDecl());
  if (Cached != DeclCache.end())
    return Var->addDebugInfo(
        cast<toolchain::DIGlobalVariableExpression>(Cached->second));

  // Create global variable debug descriptor.
  toolchain::DIFile *Unit = nullptr;
  toolchain::DIScope *DContext = nullptr;
  unsigned LineNo;
...
void CGDebugInfo::EmitGlobalVariable(const ValueDecl *VD, const APValue &Init) {
  ...
  if (VD->hasAttr<NoDebugAttr>())
    return;
  toolchain::TimeTraceScope TimeScope("DebugConstGlobalVariable", [&]() {
  ...
  toolchain::DIFile *Unit = getOrCreateFile(VD->getLocation());
  StringRef Name = VD->getName();
  toolchain::DIType *Ty = getOrCreateType(VD->getType(), Unit);
  ...
  if (const auto *ECD = dyn_cast<EnumConstantDecl>(VD)) {
    ...
    if (CGM.getCodeGenOpts().EmitCodeView) {
      ...
      [[maybe_unused]] toolchain::DIType *EDTy = getOrCreateType(T, Unit);
      assert(EDTy->getTag() == toolchain::dwarf::DW_TAG_enumeration_type);
      return;
    }
  }
  ...
  toolchain::DIScope *DContext = getDeclContextDescriptor(VD);

  auto &GV = DeclCache[VD];
  if (GV)
    return;

  toolchain::DIExpression *InitExpr = createConstantValueExpression(VD, Init);
  toolchain::MDTuple *TemplateParameters = nullptr;

  if (isa<VarTemplateSpecializationDecl>(VD))
    if (VarD) {
      toolchain::DINodeArray parameterNodes = CollectVarTemplateParams(VarD, &*Unit);
      TemplateParameters = parameterNodes.get();
    }
...
void CGDebugInfo::EmitExternalVariable(toolchain::GlobalVariable *Var,
                                       const VarDecl *D) {
  assert(CGM.getCodeGenOpts().hasReducedDebugInfo());
  if (D->hasAttr<NoDebugAttr>())
    return;
  ...
  toolchain::DIFile *Unit = getOrCreateFile(D->getLocation());
  StringRef Name = D->getName();
  toolchain::DIType *Ty = getOrCreateType(D->getType(), Unit);

  toolchain::DIScope *DContext = getDeclContextDescriptor(D);
  toolchain::DIGlobalVariableExpression *GVE =
      DBuilder.createGlobalVariableExpression(
...
void CGDebugInfo::EmitPseudoVariable(CGBuilderTy &Builder,
                                     toolchain::Instruction *Value, QualType Ty) {
  ...
  toolchain::DILocation *DIL = Value->getDebugLoc().get();
  if (!DIL)
    return;

  toolchain::DIFile *Unit = DIL->getFile();
  toolchain::DIType *Type = getOrCreateType(Ty, Unit);
  ...
  if (toolchain::LoadInst *Load = dyn_cast<toolchain::LoadInst>(Value)) {
    toolchain::Value *Var = Load->getPointerOperand();
    ...
    auto DeclareTypeMatches = [&](toolchain::DbgVariableRecord *DbgDeclare) {
      return DbgDeclare->getVariable()->getType() == Type;
    };
    if (any_of(toolchain::findDVRDeclares(Var), DeclareTypeMatches))
      return;
  }

  toolchain::DILocalVariable *D =
      DBuilder.createAutoVariable(LexicalBlockStack.back(), "", nullptr, 0,
                                  Type, false, toolchain::DINode::FlagArtificial);
...
void CGDebugInfo::EmitGlobalAlias(const toolchain::GlobalValue *GV,
                                  const GlobalDecl GD) {
  ...
  toolchain::DIImportedEntity *ImportDI = DBuilder.createImportedDeclaration(
...
void CGDebugInfo::AddStringLiteralDebugInfo(toolchain::GlobalVariable *GV,
  ...
  toolchain::DIFile *File = getOrCreateFile(Loc);
  toolchain::DIGlobalVariableExpression *Debug =
      DBuilder.createGlobalVariableExpression(
...
toolchain::DIScope *CGDebugInfo::getCurrentContextDescriptor(const Decl *D) {
  if (!LexicalBlockStack.empty())
...
CGDebugInfo::EmitNamespaceAlias(const NamespaceAliasDecl &NA) {
  ...
  if (VH)
    return cast<toolchain::DIImportedEntity>(VH);
  toolchain::DIImportedEntity *R;
...
CGDebugInfo::getOrCreateNamespace(const NamespaceDecl *NSDecl) {
  ...
  if (I != NamespaceCache.end())
    return cast<toolchain::DINamespace>(I->second);

  toolchain::DIScope *Context = getDeclContextDescriptor(NSDecl);
  // Don't trust the context if it is a DIModule (see comment above).
  toolchain::DINamespace *NS =
...
void CGDebugInfo::finalize() {
  ...
  for (size_t i = 0; i != ObjCInterfaceCache.size(); ++i) {
    ...
    toolchain::DIType *Ty = E.Type->getDecl()->getDefinition()
                           ? CreateTypeDefinition(E.Type, E.Unit)
                           : E.Decl;
    DBuilder.replaceTemporary(toolchain::TempDIType(E.Decl), Ty);
  }
  ...
  for (const auto &P : ObjCMethodCache) {
    ...
    toolchain::DICompositeType *InterfaceDecl =
        cast<toolchain::DICompositeType>(It->second);

    auto CurElts = InterfaceDecl->getElements();
    SmallVector<toolchain::Metadata *, 16> EltTys(CurElts.begin(), CurElts.end());
  ...
  for (const auto &P : ReplaceMap) {
    assert(P.second);
    auto *Ty = cast<toolchain::DIType>(P.second);
    ...
    DBuilder.replaceTemporary(toolchain::TempDIType(Ty),
                              cast<toolchain::DIType>(It->second));
  }

  for (const auto &P : FwdDeclReplaceMap) {
    assert(P.second);
    toolchain::TempMDNode FwdDecl(cast<toolchain::MDNode>(P.second));
    toolchain::Metadata *Repl;
    ...
    if (auto *GVE = dyn_cast_or_null<toolchain::DIGlobalVariableExpression>(Repl))
      Repl = GVE->getVariable();
    DBuilder.replaceTemporary(std::move(FwdDecl), cast<toolchain::MDNode>(Repl));
  }
  ...
  for (auto &RT : RetainedTypes)
    if (auto MD = TypeCache[RT])
      DBuilder.retainType(cast<toolchain::DIType>(MD));
...
toolchain::DebugLoc CGDebugInfo::SourceLocToDebugLoc(SourceLocation Loc) {
  if (LexicalBlockStack.empty())
    return toolchain::DebugLoc();

  toolchain::MDNode *Scope = LexicalBlockStack.back();
  return toolchain::DILocation::get(CGM.getLLVMContext(), getLineNumber(Loc),
                               getColumnNumber(Loc), Scope);
}

toolchain::DINode::DIFlags CGDebugInfo::getCallSiteRelatedAttrs() const {
  // Call site-related attributes are only useful in optimized programs, and
  // when there's a possibility of debugging backtraces.
  if (CGM.getCodeGenOpts().OptimizationLevel == 0 ||
      DebugKind == toolchain::codegenoptions::NoDebugInfo ||
      DebugKind == toolchain::codegenoptions::LocTrackingOnly)
    return toolchain::DINode::FlagZero;
  ...
  bool SupportsDWARFv4Ext =
      CGM.getCodeGenOpts().DwarfVersion == 4 &&
      (CGM.getCodeGenOpts().getDebuggerTuning() == toolchain::DebuggerKind::LLDB ||
       CGM.getCodeGenOpts().getDebuggerTuning() == toolchain::DebuggerKind::GDB);

  if (!SupportsDWARFv4Ext && CGM.getCodeGenOpts().DwarfVersion < 5)
    return toolchain::DINode::FlagZero;

  return toolchain::DINode::FlagAllCallsDescribed;
}

toolchain::DIExpression *
CGDebugInfo::createConstantValueExpression(const language::Core::ValueDecl *VD,
                                           const APValue &Val) {
  ...
  toolchain::APSInt const &ValInt = Val.getInt();
...
#include "language/Core/Basic/Sanitizers.def"
  default:
    toolchain_unreachable("unexpected sanitizer kind");
  }
...
toolchain::DILocation *CodeGenFunction::SanitizerAnnotateDebugInfo(
    ArrayRef<SanitizerKind::SanitizerOrdinal> Ordinals,
    SanitizerHandler Handler) {
  toolchain::DILocation *CheckDebugLoc = Builder.getCurrentDebugLocation();
