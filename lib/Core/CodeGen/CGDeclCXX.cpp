...
static void EmitDeclDestroy(CodeGenFunction &CGF, const VarDecl &D,
  ...
  if (Record && (CanRegisterDestructor || UsingExternalHelper)) {
    ...
    if (CGF.getContext().getLangOpts().OpenCL) {
      ...
      auto DestTy = toolchain::PointerType::get(
      ...
      if (DestAS == SrcAS)
        ...
        Argument = toolchain::ConstantPointerNull::get(DestTy);
    } else {
      Argument = Addr.getPointer();
    }
  // Otherwise, the standard logic requires a helper function.
    ...
    Argument = toolchain::Constant::getNullValue(CGF.Int8PtrTy);
  }
...
void CodeGenFunction::EmitInvariantStart(toolchain::Constant *Addr, CharUnits Size) {
  // Do not emit the intrinsic if we're not optimizing.
  if (!CGM.getCodeGenOpts().OptimizationLevel)
    return;

  // Grab the toolchain.invariant.start intrinsic.
  toolchain::Intrinsic::ID InvStartID = toolchain::Intrinsic::invariant_start;
  // Overloaded address space type.
  assert(Addr->getType()->isPointerTy() && "Address must be a pointer");
  toolchain::Type *ObjectPtr[1] = {Addr->getType()};
  toolchain::Function *InvariantStart = CGM.getIntrinsic(InvStartID, ObjectPtr);

  // Emit a call with the size in bytes of the object.
  uint64_t Width = Size.getQuantity();
  toolchain::Value *Args[2] = {toolchain::ConstantInt::getSigned(Int64Ty, Width), Addr};
...
void CodeGenFunction::EmitCXXGlobalVarDeclInit(const VarDecl &D,
  ...
  toolchain::Constant *DeclPtr = GV;
  if (ActualAddrSpace != ExpectedAddrSpace) {
    toolchain::PointerType *PTy =
        toolchain::PointerType::get(getLLVMContext(), ExpectedAddrSpace);
    DeclPtr = toolchain::ConstantExpr::getAddrSpaceCast(DeclPtr, PTy);
  }
...
toolchain::Constant *CodeGenFunction::createAtExitStub(const VarDecl &VD,
                                                  toolchain::FunctionCallee dtor,
                                                  toolchain::Constant *addr) {
  // Get the destructor function type, void(*)(void).
  toolchain::FunctionType *ty = toolchain::FunctionType::get(CGM.VoidTy, false);
  ...
  toolchain::Function *fn = CGM.CreateGlobalInitOrCleanUpFunction(
  ...
  toolchain::CallInst *call = CGF.Builder.CreateCall(dtor, addr);

  // Make sure the call and the callee agree on calling convention.
  if (auto *dtorFn = dyn_cast<toolchain::Function>(
...
/// which passes the given address to the given destructor function.
toolchain::Function *CodeGenFunction::createTLSAtExitStub(
    const VarDecl &D, toolchain::FunctionCallee Dtor, toolchain::Constant *Addr,
  ...
  toolchain::FunctionType *StubTy =
      toolchain::FunctionType::get(CGM.IntTy, {CGM.IntTy}, true);

  toolchain::Function *DtorStub = CGM.CreateGlobalInitOrCleanUpFunction(
  ...
  CGF.Builder.CreateStore(toolchain::Constant::getNullValue(CGM.IntTy),
                          CGF.ReturnValue);
...
void CodeGenFunction::registerGlobalDtorWithAtExit(const VarDecl &VD,
  ...
  toolchain::Constant *dtorStub = createAtExitStub(VD, dtor, addr);
  registerGlobalDtorWithAtExit(dtorStub);
}

/// Register a global destructor using the LLVM 'toolchain.global_dtors' global.
void CodeGenFunction::registerGlobalDtorWithLLVM(const VarDecl &VD,
                                                 toolchain::FunctionCallee Dtor,
                                                 toolchain::Constant *Addr) {
  // Create a function which calls the destructor.
  toolchain::Function *dtorStub =
      cast<toolchain::Function>(createAtExitStub(VD, Dtor, Addr));
  CGM.AddGlobalDtor(dtorStub);
}

void CodeGenFunction::registerGlobalDtorWithAtExit(toolchain::Constant *dtorStub) {
  // extern "C" int atexit(void (*f)(void));
  assert(dtorStub->getType()->isPointerTy() &&
         "Argument to atexit has a wrong type.");

  toolchain::FunctionType *atexitTy =
      toolchain::FunctionType::get(IntTy, dtorStub->getType(), false);

  toolchain::FunctionCallee atexit =
      CGM.CreateRuntimeFunction(atexitTy, "atexit", toolchain::AttributeList(),
                                /*Local=*/true);
  if (toolchain::Function *atexitFn = dyn_cast<toolchain::Function>(atexit.getCallee()))
...
toolchain::Value *
CodeGenFunction::unregisterGlobalDtorWithUnAtExit(toolchain::Constant *dtorStub) {
  ...
  toolchain::FunctionType *unatexitTy =
      toolchain::FunctionType::get(IntTy, {dtorStub->getType()}, /*isVarArg=*/false);

  toolchain::FunctionCallee unatexit =
      CGM.CreateRuntimeFunction(unatexitTy, "unatexit", toolchain::AttributeList());

  cast<toolchain::Function>(unatexit.getCallee())->setDoesNotThrow();
...
void CodeGenFunction::EmitCXXGuardedInitBranch(toolchain::Value *NeedsInit,
                                               toolchain::BasicBlock *InitBlock,
                                               toolchain::BasicBlock *NoInitBlock,
                                               GuardKind Kind,
                                               const VarDecl *D) {
...
toolchain::Function *CodeGenModule::CreateGlobalInitOrCleanUpFunction(
    toolchain::FunctionType *FTy, const Twine &Name, const CGFunctionInfo &FI,
    SourceLocation Loc, bool TLS, toolchain::GlobalVariable::LinkageTypes Linkage) {
  toolchain::Function *Fn = toolchain::Function::Create(FTy, Linkage, Name, &getModule());
  ...
  if (Linkage == toolchain::GlobalVariable::InternalLinkage)
...
void CodeGenModule::EmitPointerToInitFunc(const VarDecl *D,
                                          toolchain::GlobalVariable *GV,
                                          toolchain::Function *InitFunc,
                                          InitSegAttr *ISA) {
  toolchain::GlobalVariable *PtrArray = new toolchain::GlobalVariable(
      TheModule, InitFunc->getType(), /*isConstant=*/true,
      toolchain::GlobalValue::PrivateLinkage, InitFunc, "__cxx_init_fn_ptr");
...
CodeGenModule::EmitCXXGlobalVarDeclInitFunc(const VarDecl *D,
  ...
  toolchain::Function *Fn = CreateGlobalInitOrCleanUpFunction(
  ...
  toolchain::GlobalVariable *COMDATKey =
  ...
  if (D->getTLSKind()) {
    ...
    // toolchain.global_ctors does not guarantee initialization order, so in
    ...
    // VarDecls, their order of insertion into `toolchain.global_ctors` is the same
    ...
    if (COMDATKey && (getTriple().isOSBinFormatELF() ||
      ...
      // toolchain.used to prevent linker GC.
    ...
    toolchain::Comdat *C = Addr->getComdat();
    if (COMDATKey && C &&
...
void CodeGenModule::EmitCXXModuleInitFunc(Module *Primary) {
  ...
  toolchain::SmallSetVector<Module *, 8> AllImports;
  // Ones that we export
  ...
  SmallVector<toolchain::Function *, 8> ModuleInits;
  for (Module *M : AllImports) {
    ...
    toolchain::Function *Fn = toolchain::Function::Create(
        FTy, toolchain::Function::ExternalLinkage, FnName.str(), &getModule());
  ...
  if (!PrioritizedCXXGlobalInits.empty()) {
    toolchain::array_pod_sort(PrioritizedCXXGlobalInits.begin(),
  ...
  toolchain::Function *Fn;
  {
    ...
    Fn = CreateGlobalInitOrCleanUpFunction(
        FTy, toolchain::Twine(InitFnName), FI, SourceLocation(), false,
    ...
    if (!ModuleInits.empty()) {
      ...
      toolchain::GlobalVariable *Guard = new toolchain::GlobalVariable(
          ...
          toolchain::ConstantInt::get(Int8Ty, 0), InitFnName.str() + "__in_chrg");
  ...
  if (getLangOpts().OpenCL) {
    ...
    Fn->setCallingConv(toolchain::CallingConv::SPIR_KERNEL);
  }
  ...
  if (getLangOpts().HIP && getLangOpts().CUDAIsDevice) {
    if (getTriple().isSPIRV())
      ...
      Fn->setCallingConv(toolchain::CallingConv::AMDGPU_KERNEL);
...
static SmallString<128> getTransformedFileName(toolchain::Module &M) {
  SmallString<128> FileName = toolchain::sys::path::filename(M.getName());
...
static std::string getPrioritySuffix(unsigned int Priority) {
  ...
  std::string PrioritySuffix = toolchain::utostr(Priority);
...
CodeGenModule::EmitCXXGlobalInitFunc() {
  ...
  if (!PrioritizedCXXGlobalInits.empty()) {
    SmallVector<toolchain::Function *, 8> LocalCXXGlobalInits;
    toolchain::array_pod_sort(PrioritizedCXXGlobalInits.begin(),
                         PrioritizedCXXGlobalInits.end());
  ...
  if (CXX20ModuleInits && getContext().getCurrentNamedModule() &&
    ...
    Fn = CreateGlobalInitOrCleanUpFunction(
        FTy,
        toolchain::Twine("_GLOBAL__sub_I_", getTransformedFileName(getModule())),
        FI);
...
void CodeGenModule::EmitCXXGlobalCleanUpFunc() {
  ...
  if (!PrioritizedCXXStermFinalizers.empty()) {
    ...
    toolchain::array_pod_sort(PrioritizedCXXStermFinalizers.begin(),
    ...
    for (SmallVectorImpl<StermFinalizerData>::iterator
      ...
      for (; I < PrioE; ++I) {
        toolchain::FunctionCallee DtorFn = I->second;
...
void CodeGenFunction::GenerateCXXGlobalVarDeclInitFunc(toolchain::Function *Fn,
                                                       const VarDecl *D,
                                                 toolchain::GlobalVariable *Addr,
                                                       bool PerformInit) {
...
void
CodeGenFunction::GenerateCXXGlobalInitFunc(toolchain::Function *Fn,
                                           ArrayRef<toolchain::Function *> Decls,
                                           ConstantAddress Guard) {
  {
    ...
    toolchain::BasicBlock *ExitBlock = nullptr;
    if (Guard.isValid()) {
      // If we have a guard variable, check whether we've already performed
      // these initializations. This happens for TLS initialization functions.
      toolchain::Value *GuardVal = Builder.CreateLoad(Guard);
      toolchain::Value *Uninit = Builder.CreateIsNull(GuardVal,
                                                 "guard.uninitialized");
      toolchain::BasicBlock *InitBlock = createBasicBlock("init");
      ...
      Builder.CreateStore(toolchain::ConstantInt::get(GuardVal->getType(),1), Guard);
    ...
    for (toolchain::Function *Decl : Decls)
      if (Decl)
...
void CodeGenFunction::GenerateCXXGlobalCleanUpFunc(
        ...
        DtorsOrStermFinalizers) {
  {
    ...
    for (unsigned i = 0, e = DtorsOrStermFinalizers.size(); i != e; ++i) {
      toolchain::FunctionType *CalleeTy;
      toolchain::Value *Callee;
      toolchain::Constant *Arg;
      std::tie(CalleeTy, Callee, Arg) = DtorsOrStermFinalizers[e - i - 1];

      toolchain::CallBase *CI = nullptr;
      if (Arg == nullptr) {
        assert(
        ...
        if (ActualAddrSpace != ExpectedAddrSpace) {
          toolchain::PointerType *PTy =
              toolchain::PointerType::get(getLLVMContext(), ExpectedAddrSpace);
          Arg = toolchain::ConstantExpr::getAddrSpaceCast(Arg, PTy);
        }
        CI = Builder.CreateCall(CalleeTy, Callee, Arg);
      }

      // Make sure the call and the callee agree on calling convention.
      if (toolchain::Function *F = dyn_cast<toolchain::Function>(Callee))
...
toolchain::Function *CodeGenFunction::generateDestroyHelper(
