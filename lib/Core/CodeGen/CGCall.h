...
#ifndef LANGUAGE_CORE_LIB_CODEGEN_CGCALL_H
#define LANGUAGE_CORE_LIB_CODEGEN_CGCALL_H
...
public:
  ...
  /// isn't a direct call.
  CGCallee(const CGCalleeInfo &abstractInfo, toolchain::Value *functionPtr,
  ...
  static CGCallee forDirect(toolchain::Constant *functionPtr,
                            const CGCalleeInfo &abstractInfo = CGCalleeInfo()) {
    return CGCallee(abstractInfo, functionPtr);
  }

  static CGCallee forDirect(toolchain::FunctionCallee functionPtr,
                            const CGCalleeInfo &abstractInfo = CGCalleeInfo()) {
  ...
  toolchain::Value *getFunctionPointer() const {
    assert(isOrdinary());
    return reinterpret_cast<toolchain::Value *>(uintptr_t(KindOrFunctionPointer));
  }
  void setFunctionPointer(toolchain::Value *functionPtr) {
    assert(isOrdinary());
    KindOrFunctionPointer =
        SpecialKind(reinterpret_cast<uintptr_t>(functionPtr));
  }
  ...
  toolchain::FunctionType *getVirtualFunctionType() const {
    assert(isVirtual());
...
public:
  ...
  struct CallArgCleanup {
    ...
    toolchain::Instruction *IsActiveIP;
  };
  ...
  void addFrom(const CallArgList &other) {
    toolchain::append_range(*this, other);
    toolchain::append_range(Writebacks, other.Writebacks);
    toolchain::append_range(CleanupsToDeactivate, other.CleanupsToDeactivate);
    assert(!(StackBase && other.StackBase) && "can't merge stackbases");
    if (!StackBase)
      StackBase = other.StackBase;
  }

  void addWriteback(LValue srcLV, Address temporary, toolchain::Value *toUse,
  ...
  typedef toolchain::iterator_range<SmallVectorImpl<Writeback>::const_iterator>
  ...
  toolchain::Instruction *getStackBase() const { return StackBase; }
...
private:
  ...
  toolchain::CallInst *StackBase = nullptr;
};
...
void mergeDefaultFunctionDefinitionAttributes(toolchain::Function &F,
...
inline FnInfoOpts operator|(FnInfoOpts A, FnInfoOpts B) {
  return static_cast<FnInfoOpts>(toolchain::to_underlying(A) |
