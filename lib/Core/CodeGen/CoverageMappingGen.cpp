...
#include "toolchain/ProfileData/Coverage/CoverageMappingReader.h"
...
cl::opt<bool>
    EnableSingleByteCoverage("enable-single-byte-coverage",
                             ...
                             toolchain::cl::desc("Enable single byte coverage"),
                             toolchain::cl::Hidden, toolchain::cl::init(false));
} // namespace toolchain

static toolchain::cl::opt<bool> EmptyLineCommentCoverage(
    "emptyline-comment-coverage",
    toolchain::cl::desc("Emit emptylines and comment lines as skipped regions (only "
                   "disable it on test)"),
...
using namespace toolchain::coverage;
...
CoverageMappingModuleGen::setUpCoverageCallbacks(Preprocessor &PP) {
  ...
  if (EmptyLineCommentCoverage) {
    ...
    PP.setTokenWatcher([CoverageInfo](language::Core::Token Tok) {
      // Update previous token location.
      CoverageInfo->PrevTokLoc = Tok.getLocation();
      if (Tok.getKind() != language::Core::tok::eod)
...
private:
  ...
  toolchain::SmallDenseMap<FileID, std::pair<unsigned, SourceLocation>, 8>
      FileIDMapping;

public:
  /// The coverage mapping regions for this function
  toolchain::SmallVector<CounterMappingRegion, 32> MappingRegions;
  ...
  typedef toolchain::SmallSet<std::pair<SourceLocation, SourceLocation>, 8>
      SourceRegionFilter;
  ...
  void gatherFileIDs(SmallVectorImpl<unsigned> &Mapping) {
    ...
    toolchain::SmallSet<FileID, 8> Visited;
    ...
    toolchain::stable_sort(FileLocs, toolchain::less_second());
  ...
  void gatherSkippedRegions() {
    ...
    toolchain::SmallVector<std::pair<unsigned, unsigned>, 8> FileLineRanges;
...
struct EmptyCoverageMappingBuilder : public CoverageMappingBuilder {
  ...
  void write(toolchain::raw_ostream &OS) {
...
private:
  ...
  toolchain::SmallVector<mcdc::ConditionIDs> DecisionStack;
...
struct CounterCoverageMappingBuilder
  ...
  toolchain::SmallVector<SourceMappingRegion> RegionStack;

  /// Set if the Expr should be handled as a leaf even if it is kind of binary
  /// logical ops (&&, ||).
  toolchain::DenseSet<const Stmt *> LeafExprSet;
  ...
  Counter subtractCounters(Counter LHS, Counter RHS, bool Simplify = true) {
    assert(!toolchain::EnableSingleByteCoverage &&
  ...
  bool isRegionAlreadyAdded(SourceLocation StartLoc, SourceLocation EndLoc,
    ...
    return toolchain::any_of(
        toolchain::reverse(SourceRegions), [&](const SourceMappingRegion &Region) {
  ...
  void handleFileExit(SourceLocation NewLoc) {
    ...
    toolchain::SmallSet<SourceLocation, 8> StartLocs;
    std::optional<Counter> ParentCounter;
    for (SourceMappingRegion &I : toolchain::reverse(RegionStack)) {
      if (!I.hasStartLoc())
        continue;
  ...
  /// Write the mapping data to the output stream
  void write(toolchain::raw_ostream &OS) {
    toolchain::SmallVector<unsigned, 8> VirtualFileMapping;
  ...
  void VisitDecl(const Decl *D) {
    ...
    // these children nodes (toolchain.org/PR39822), and moreover users will not be
  ...
  void VisitWhileStmt(const WhileStmt *S) {
    ...
    Counter BodyCount = toolchain::EnableSingleByteCoverage
  ...
  void VisitDoStmt(const DoStmt *S) {
    ...
    Counter CondCount = toolchain::EnableSingleByteCoverage
                            ? getRegionCounter(S->getCond())
  ...
  void VisitSwitchStmt(const SwitchStmt *S) {
    ...
    if (!BreakContinueStack.empty() && !toolchain::EnableSingleByteCoverage)
      BreakContinueStack.back().ContinueCount = addCounters(
  ...
  void VisitSwitchCase(const SwitchCase *S) {
    ...
    Counter Count = toolchain::EnableSingleByteCoverage
                        ? getRegionCounter(S)
  ...
  void coverIfConsteval(const IfStmt *S) {
    ...
    // It's better for toolchain-cov to create a new region with same counter
  ...
  void coverIfConstexpr(const IfStmt *S) {
    ...
    // to properly calculate line coverage in toolchain-cov utility
    const Counter ParentCount = getRegion().getCounter();
  ...
  void VisitIfStmt(const IfStmt *S) {
    ...
    if (const Stmt *Else = S->getElse()) {
      ...
      if (ThenHasTerminateStmt)
        HasTerminateStmt = true;
    } else if (!toolchain::EnableSingleByteCoverage)
  ...
  void createOrCancelDecision(const BinaryOperator *E, unsigned Since) {
    ...
    toolchain::SmallVector<mcdc::ConditionIDs> CondIDs(NumConds);
...
static void dump(toolchain::raw_ostream &OS, StringRef FunctionName,
...
std::string CoverageMappingModuleGen::normalizeFilename(StringRef Filename) {
  toolchain::SmallString<256> Path(Filename);
  ...
  for (const auto &[From, To] :
       toolchain::reverse(CGM.getCodeGenOpts().CoveragePrefixMap)) {
    if (toolchain::sys::path::replace_path_prefix(Path, From, To))
      break;
  }
  return Path.str().str();
}
...
static std::string getInstrProfSection(const CodeGenModule &CGM,
  ...
  return toolchain::getInstrProfSectionName(
...
void CoverageMappingModuleGen::emitFunctionMappingRecord(
  ...
  std::string FuncRecordName = "__covrec_" + toolchain::utohexstr(Info.NameHash);
...
#include "toolchain/ProfileData/InstrProfData.inc"
  ...
  auto *FunctionRecordTy =
      toolchain::StructType::get(Ctx, ArrayRef(FunctionRecordTypes),
                            /*isPacked=*/true);
...
#define COVMAP_FUNC_RECORD(Type, LLVMType, Name, Init) Init,
  ...
  auto *FuncRecordConstant =
      toolchain::ConstantStruct::get(FunctionRecordTy, ArrayRef(FunctionRecordVals));
  ...
  FuncRecord->setSection(getInstrProfSection(CGM, toolchain::IPSK_covfun));
...
void CoverageMappingModuleGen::addFunctionMappingRecord(
    toolchain::GlobalVariable *NamePtr, StringRef NameValue, uint64_t FuncHash,
  ...
  if (CGM.getCodeGenOpts().DumpCoverageMapping) {
    ...
    // expressions.
    toolchain::SmallVector<std::string, 16> FilenameStrs;
    ...
    ArrayRef<std::string> FilenameRefs = toolchain::ArrayRef(FilenameStrs);
    RawCoverageMappingReader Reader(CoverageMapping, FilenameRefs, Filenames,
                                    Expressions, Regions);
    if (Reader.read())
      return;
    dump(toolchain::outs(), NameValue, Expressions, Regions);
  }
}
...
#include "toolchain/ProfileData/InstrProfData.inc"
  ...
  auto CovDataHeaderTy =
      toolchain::StructType::get(Ctx, ArrayRef(CovDataHeaderTypes));
  toolchain::Constant *CovDataHeaderVals[] = {
#define COVMAP_HEADER(Type, LLVMType, Name, Init) Init,
#include "toolchain/ProfileData/InstrProfData.inc"
  };
  auto CovDataHeaderVal =
      toolchain::ConstantStruct::get(CovDataHeaderTy, ArrayRef(CovDataHeaderVals));

  // Create the coverage data record
  toolchain::Type *CovDataTypes[] = {CovDataHeaderTy, FilenamesVal->getType()};
  auto CovDataTy = toolchain::StructType::get(Ctx, ArrayRef(CovDataTypes));
  toolchain::Constant *TUDataVals[] = {CovDataHeaderVal, FilenamesVal};
  auto CovDataVal = toolchain::ConstantStruct::get(CovDataTy, ArrayRef(TUDataVals));
  auto CovData = new toolchain::GlobalVariable(
      CGM.getModule(), CovDataTy, true, toolchain::GlobalValue::PrivateLinkage,
      CovDataVal, toolchain::getCoverageMappingVarName());

  CovData->setSection(getInstrProfSection(CGM, toolchain::IPSK_covmap));
  ...
  if (!FunctionNames.empty()) {
    ...
    auto NamesArrTy = toolchain::ArrayType::get(
        toolchain::PointerType::get(Ctx, AddrSpace), FunctionNames.size());
    auto NamesArrVal = toolchain::ConstantArray::get(NamesArrTy, FunctionNames);
    // This variable will *NOT* be emitted to the object file. It is used
    // to pass the list of names referenced to codegen.
    new toolchain::GlobalVariable(CGM.getModule(), NamesArrTy, true,
                             toolchain::GlobalValue::InternalLinkage, NamesArrVal,
