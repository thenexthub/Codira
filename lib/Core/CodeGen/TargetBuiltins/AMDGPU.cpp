...
#include "toolchain/IR/IntrinsicsR600.h"
#include "toolchain/IR/MemoryModelRelaxationAnnotations.h"
...
Value *EmitAMDGPUDispatchPtr(CodeGenFunction &CGF,
  ...
  auto *RetTy = cast<toolchain::PointerType>(CGF.ConvertType(BuiltinRetType));
...
Value *EmitAMDGPUWorkGroupSize(CodeGenFunction &CGF, unsigned Index) {
  ...
  if (Cov == CodeObjectVersionKind::COV_None) {
    ...
    if (!ABIVersionC)
      ABIVersionC = new toolchain::GlobalVariable(
          CGF.CGM.getModule(), CGF.Int32Ty, false,
          toolchain::GlobalValue::ExternalLinkage, nullptr, Name, nullptr,
    ...
    Value *IsCOV5 = CGF.Builder.CreateICmpSGE(
        ABIVersion,
        toolchain::ConstantInt::get(CGF.Int32Ty, CodeObjectVersionKind::COV_5));
  ...
  toolchain::MDNode *RNode = MDHelper.createRange(APInt(16, 1),
      APInt(16, CGF.getTarget().getMaxOpenCLWorkGroupSize() + 1));
  LD->setMetadata(toolchain::LLVMContext::MD_range, RNode);
  LD->setMetadata(toolchain::LLVMContext::MD_noundef,
                  toolchain::MDNode::get(CGF.getLLVMContext(), {}));
  LD->setMetadata(toolchain::LLVMContext::MD_invariant_load,
                  toolchain::MDNode::get(CGF.getLLVMContext(), {}));
  return LD;
}
...
Value *EmitAMDGPUGridSize(CodeGenFunction &CGF, unsigned Index) {
  ...
  // Indexing the HSA kernel_dispatch_packet struct.
  auto *Offset = toolchain::ConstantInt::get(CGF.Int32Ty, XOffset + Index * 4);
  auto *GEP = CGF.Builder.CreateGEP(CGF.Int8Ty, DP, Offset);
  auto *LD = CGF.Builder.CreateLoad(
      Address(GEP, CGF.Int32Ty, CharUnits::fromQuantity(4)));

  toolchain::MDBuilder MDB(CGF.getLLVMContext());

  // Known non-zero.
  LD->setMetadata(toolchain::LLVMContext::MD_range,
...
static Value *EmitAMDGCNBallotForExec(CodeGenFunction &CGF, const CallExpr *E,
                                      ...
                                      toolchain::Type *ValueType, bool isExecHi) {
  CodeGen::CGBuilderTy &Builder = CGF.Builder;
  CodeGen::CodeGenModule &CGM = CGF.CGM;

  Function *F = CGM.getIntrinsic(Intrinsic::amdgcn_ballot, {RegisterType});
  toolchain::Value *Call = Builder.CreateCall(F, {Builder.getInt1(true)});
...
void CodeGenFunction::ProcessOrderScopeAMDGCN(Value *Order, Value *Scope,
  ...
  assert(toolchain::isValidAtomicOrderingCABI(ord));
  switch (static_cast<toolchain::AtomicOrderingCABI>(ord)) {
  ...
  StringRef scp;
  if (toolchain::getConstantStringInfo(Scope, scp)) {
    SSID = getLLVMContext().getOrInsertSyncScopeID(scp);
    return;
  }

  // Older builtins had an enum argument for the memory scope.
  int scope = cast<toolchain::ConstantInt>(Scope)->getZExtValue();
  switch (scope) {
...
toolchain::Value *CodeGenFunction::EmitScalarOrConstFoldImmArg(unsigned ICEArguments,
                                                          unsigned Idx,
                                                          const CallExpr *E) {
  toolchain::Value *Arg = nullptr;
  if ((ICEArguments & (1 << Idx)) == 0) {
    ...
    Arg = toolchain::ConstantInt::get(getLLVMContext(), *Result);
  }
  return Arg;
}

void CodeGenFunction::AddAMDGPUFenceAddressSpaceMMRA(toolchain::Instruction *Inst,
                                                     const CallExpr *E) {
  ...
  for (unsigned K = 2; K < E->getNumArgs(); ++K) {
    toolchain::Value *V = EmitScalarExpr(E->getArg(K));
    StringRef AS;
    if (toolchain::getConstantStringInfo(V, AS)) {
  ...
  toolchain::sort(MMRAs);
  MMRAs.erase(toolchain::unique(MMRAs), MMRAs.end());
  Inst->setMetadata(LLVMContext::MD_mmra, MMRAMetadata::getMD(Ctx, MMRAs));
}

Value *CodeGenFunction::EmitAMDGPUBuiltinExpr(unsigned BuiltinID,
                                              const CallExpr *E) {
  toolchain::AtomicOrdering AO = toolchain::AtomicOrdering::SequentiallyConsistent;
  toolchain::SyncScope::ID SSID;
  switch (BuiltinID) {
  ...
  case AMDGPU::BI__builtin_amdgcn_div_scalef: {
    ...
    toolchain::Value *Z = EmitScalarExpr(E->getArg(2));

    toolchain::Function *Callee = CGM.getIntrinsic(Intrinsic::amdgcn_div_scale,
                                           X->getType());

    toolchain::Value *Tmp = Builder.CreateCall(Callee, {X, Y, Z});

    toolchain::Value *Result = Builder.CreateExtractValue(Tmp, 0);
    toolchain::Value *Flag = Builder.CreateExtractValue(Tmp, 1);

    toolchain::Type *RealFlagType = FlagOutPtr.getElementType();

    toolchain::Value *FlagExt = Builder.CreateZExt(Flag, RealFlagType);
  ...
  case AMDGPU::BI__builtin_amdgcn_div_fmasf: {
    ...
    toolchain::Value *Src2 = EmitScalarExpr(E->getArg(2));
    toolchain::Value *Src3 = EmitScalarExpr(E->getArg(3));

    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::amdgcn_div_fmas,
                                      Src0->getType());
    toolchain::Value *Src3ToBool = Builder.CreateIsNotNull(Src3);
  ...
  case AMDGPU::BI__builtin_amdgcn_update_dpp: {
    toolchain::SmallVector<toolchain::Value *, 6> Args;
    ...
    toolchain::Type *DataTy = ConvertType(E->getArg(0)->getType());
    unsigned Size = DataTy->getPrimitiveSizeInBits();
    toolchain::Type *IntTy =
        toolchain::IntegerType::get(Builder.getContext(), std::max(Size, 32u));
    Function *F =
    ...
    if (InsertOld)
      Args.push_back(toolchain::PoisonValue::get(IntTy));
    for (unsigned I = 0; I != E->getNumArgs(); ++I) {
      toolchain::Value *V = EmitScalarOrConstFoldImmArg(ICEArguments, I, E);
      if (I < (BuiltinID == AMDGPU::BI__builtin_amdgcn_update_dpp ? 2u : 1u) &&
          Size < 32) {
        if (!DataTy->isIntegerTy())
          V = Builder.CreateBitCast(
              V, toolchain::IntegerType::get(Builder.getContext(), Size));
        V = Builder.CreateZExtOrBitCast(V, IntTy);
      }
      toolchain::Type *ExpTy =
  ...
  case AMDGPU::BI__builtin_amdgcn_ds_read_tr16_b64_v4i16: {
    ...
    toolchain::Type *LoadTy = ConvertType(E->getType());
    toolchain::Value *Addr = EmitScalarExpr(E->getArg(0));
    toolchain::Function *F = CGM.getIntrinsic(IID, {LoadTy});
  ...
  case AMDGPU::BI__builtin_amdgcn_flat_load_monitor_b128: {
    ...
    toolchain::Type *LoadTy = ConvertType(E->getType());
    toolchain::Value *Addr = EmitScalarExpr(E->getArg(0));
    toolchain::Value *Val = EmitScalarExpr(E->getArg(1));
    toolchain::Function *F = CGM.getIntrinsic(IID, {LoadTy});
  ...
  case AMDGPU::BI__builtin_amdgcn_get_fpenv: {
    Function *F = CGM.getIntrinsic(Intrinsic::get_fpenv,
                                   {toolchain::Type::getInt64Ty(getLLVMContext())});
    return Builder.CreateCall(F);
  }
  case AMDGPU::BI__builtin_amdgcn_set_fpenv: {
    Function *F = CGM.getIntrinsic(Intrinsic::set_fpenv,
                                   {toolchain::Type::getInt64Ty(getLLVMContext())});
    toolchain::Value *Env = EmitScalarExpr(E->getArg(0));
  ...
  case AMDGPU::BI__builtin_amdgcn_image_bvh_intersect_ray_lh: {
    toolchain::Value *NodePtr = EmitScalarExpr(E->getArg(0));
    toolchain::Value *RayExtent = EmitScalarExpr(E->getArg(1));
    toolchain::Value *RayOrigin = EmitScalarExpr(E->getArg(2));
    toolchain::Value *RayDir = EmitScalarExpr(E->getArg(3));
    toolchain::Value *RayInverseDir = EmitScalarExpr(E->getArg(4));
    toolchain::Value *TextureDescr = EmitScalarExpr(E->getArg(5));
  ...
  case AMDGPU::BI__builtin_amdgcn_image_bvh_dual_intersect_ray: {
    ...
    toolchain::Value *NodePtr = EmitScalarExpr(E->getArg(0));
    toolchain::Value *RayExtent = EmitScalarExpr(E->getArg(1));
    toolchain::Value *InstanceMask = EmitScalarExpr(E->getArg(2));
    toolchain::Value *RayOrigin = EmitScalarExpr(E->getArg(3));
    toolchain::Value *RayDir = EmitScalarExpr(E->getArg(4));
    toolchain::Value *Offset = EmitScalarExpr(E->getArg(5));
    toolchain::Value *TextureDescr = EmitScalarExpr(E->getArg(6));

    Address RetRayOriginPtr = EmitPointerWithAlignment(E->getArg(7));
    Address RetRayDirPtr = EmitPointerWithAlignment(E->getArg(8));

    toolchain::Function *IntrinsicFunc = CGM.getIntrinsic(IID);

    toolchain::CallInst *CI = Builder.CreateCall(
        IntrinsicFunc, {NodePtr, RayExtent, InstanceMask, RayOrigin, RayDir,
                        Offset, TextureDescr});

    toolchain::Value *RetVData = Builder.CreateExtractValue(CI, 0);
    toolchain::Value *RetRayOrigin = Builder.CreateExtractValue(CI, 1);
    toolchain::Value *RetRayDir = Builder.CreateExtractValue(CI, 2);
  ...
  case AMDGPU::BI__builtin_amdgcn_mfma_scale_f32_32x32x64_f8f6f4: {
    toolchain::FixedVectorType *VT = FixedVectorType::get(Builder.getInt32Ty(), 8);
  ...
  case AMDGPU::BI__builtin_amdgcn_swmmac_i32_16x16x128_iu8: {
    ...
    SmallVector<toolchain::Type *, 6> ArgTypes;
    if (NeedReturnType)
  ...
  case AMDGPU::BI__builtin_amdgcn_flat_atomic_fmax_f64: {
    ...
    case AMDGPU::BI__builtin_amdgcn_atomic_inc32:
    case AMDGPU::BI__builtin_amdgcn_atomic_inc64:
      BinOp = toolchain::AtomicRMWInst::UIncWrap;
      break;
    case AMDGPU::BI__builtin_amdgcn_atomic_dec32:
    case AMDGPU::BI__builtin_amdgcn_atomic_dec64:
      BinOp = toolchain::AtomicRMWInst::UDecWrap;
      break;
    case AMDGPU::BI__builtin_amdgcn_ds_faddf:
    case AMDGPU::BI__builtin_amdgcn_ds_atomic_fadd_f64:
    case AMDGPU::BI__builtin_amdgcn_ds_atomic_fadd_f32:
    ...
    case AMDGPU::BI__builtin_amdgcn_global_atomic_fadd_v2bf16:
    case AMDGPU::BI__builtin_amdgcn_flat_atomic_fadd_v2bf16:
      BinOp = toolchain::AtomicRMWInst::FAdd;
      break;
    case AMDGPU::BI__builtin_amdgcn_ds_fminf:
    case AMDGPU::BI__builtin_amdgcn_global_atomic_fmin_f64:
    case AMDGPU::BI__builtin_amdgcn_flat_atomic_fmin_f64:
      BinOp = toolchain::AtomicRMWInst::FMin;
      break;
    case AMDGPU::BI__builtin_amdgcn_global_atomic_fmax_f64:
    case AMDGPU::BI__builtin_amdgcn_flat_atomic_fmax_f64:
    case AMDGPU::BI__builtin_amdgcn_ds_fmaxf:
      BinOp = toolchain::AtomicRMWInst::FMax;
      break;
    }
    ...
    if (E->getNumArgs() >= 4) {
      ...
      if (BuiltinID == AMDGPU::BI__builtin_amdgcn_ds_atomic_fadd_v2bf16 ||
        ...
        toolchain::Type *V2BF16Ty = FixedVectorType::get(
            toolchain::Type::getBFloatTy(Builder.getContext()), 2);
    ...
    if (AddrSpace != toolchain::AMDGPUAS::LOCAL_ADDRESS) {
      ...
      if (BinOp == toolchain::AtomicRMWInst::FAdd && Val->getType()->isFloatTy())
  ...
  case AMDGPU::BI__builtin_amdgcn_permlane32_swap: {
    ...
    toolchain::Value *Elt0 = Builder.CreateExtractValue(Call, 0);
    toolchain::Value *Elt1 = Builder.CreateExtractValue(Call, 1);

    toolchain::Type *ResultType = ConvertType(E->getType());

    toolchain::Value *Insert0 = Builder.CreateInsertElement(
        toolchain::PoisonValue::get(ResultType), Elt0, UINT64_C(0));
    toolchain::Value *AsVector =
  ...
  case AMDGPU::BI__builtin_amdgcn_make_buffer_rsrc: {
    ...
    for (unsigned I = 0; I < 4; ++I)
      Args.push_back(EmitScalarExpr(E->getArg(I)));
    toolchain::PointerType *RetTy = toolchain::PointerType::get(
        Builder.getContext(), toolchain::AMDGPUAS::BUFFER_RESOURCE);
  ...
  case AMDGPU::BI__builtin_amdgcn_raw_buffer_load_b128: {
    ...
    case AMDGPU::BI__builtin_amdgcn_raw_buffer_load_b32:
      ...
      break;
    case AMDGPU::BI__builtin_amdgcn_raw_buffer_load_b64:
      RetTy = toolchain::FixedVectorType::get(Int32Ty, /*NumElements=*/2);
      break;
    case AMDGPU::BI__builtin_amdgcn_raw_buffer_load_b96:
      RetTy = toolchain::FixedVectorType::get(Int32Ty, /*NumElements=*/3);
      break;
    case AMDGPU::BI__builtin_amdgcn_raw_buffer_load_b128:
      RetTy = toolchain::FixedVectorType::get(Int32Ty, /*NumElements=*/4);
      break;
    }
    Function *F =
