...
static NVPTXMmaLdstInfo getNVPTXMmaLdstInfo(unsigned BuiltinID) {
  ...
  default:
    toolchain_unreachable("Unknown MMA builtin");
  }
}
#undef MMA_LDST
#undef MMA_INTR
...
#define MMA_VARIANTS_B1_AND(geom, type) \
  ...
  default:
    toolchain_unreachable("Unexpected builtin ID.");
  }
#undef MMA_VARIANTS
...
static Value *MakeLdu(unsigned IntrinsicID, CodeGenFunction &CGF,
  ...
  language::Core::CharUnits Align = CGF.CGM.getNaturalPointeeTypeAlignment(ArgType);
  toolchain::Type *ElemTy = CGF.ConvertTypeForMem(ArgType->getPointeeType());
...
static Value *MakeLdg(CodeGenFunction &CGF, const CallExpr *E) {
  Value *Ptr = CGF.EmitScalarExpr(E->getArg(0));
  QualType ArgType = E->getArg(0)->getType();
  language::Core::CharUnits AlignV = CGF.CGM.getNaturalPointeeTypeAlignment(ArgType);
  toolchain::Type *ElemTy = CGF.ConvertTypeForMem(ArgType->getPointeeType());
...
Value *CodeGenFunction::EmitNVPTXBuiltinExpr(unsigned BuiltinID,
  ...
  case NVPTX::BI__nvvm_atom_add_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Add, E);

  case NVPTX::BI__nvvm_atom_sub_gen_i:
  case NVPTX::BI__nvvm_atom_sub_gen_l:
  case NVPTX::BI__nvvm_atom_sub_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Sub, E);

  case NVPTX::BI__nvvm_atom_and_gen_i:
  case NVPTX::BI__nvvm_atom_and_gen_l:
  case NVPTX::BI__nvvm_atom_and_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::And, E);

  case NVPTX::BI__nvvm_atom_or_gen_i:
  case NVPTX::BI__nvvm_atom_or_gen_l:
  case NVPTX::BI__nvvm_atom_or_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Or, E);

  case NVPTX::BI__nvvm_atom_xor_gen_i:
  case NVPTX::BI__nvvm_atom_xor_gen_l:
  case NVPTX::BI__nvvm_atom_xor_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Xor, E);

  case NVPTX::BI__nvvm_atom_xchg_gen_i:
  case NVPTX::BI__nvvm_atom_xchg_gen_l:
  case NVPTX::BI__nvvm_atom_xchg_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Xchg, E);

  case NVPTX::BI__nvvm_atom_max_gen_i:
  case NVPTX::BI__nvvm_atom_max_gen_l:
  case NVPTX::BI__nvvm_atom_max_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Max, E);

  case NVPTX::BI__nvvm_atom_max_gen_ui:
  case NVPTX::BI__nvvm_atom_max_gen_ul:
  case NVPTX::BI__nvvm_atom_max_gen_ull:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::UMax, E);

  case NVPTX::BI__nvvm_atom_min_gen_i:
  case NVPTX::BI__nvvm_atom_min_gen_l:
  case NVPTX::BI__nvvm_atom_min_gen_ll:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::Min, E);

  case NVPTX::BI__nvvm_atom_min_gen_ui:
  case NVPTX::BI__nvvm_atom_min_gen_ul:
  case NVPTX::BI__nvvm_atom_min_gen_ull:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::UMin, E);
  ...
  case NVPTX::BI__nvvm_atom_add_gen_d: {
    ...
    return Builder.CreateAtomicRMW(toolchain::AtomicRMWInst::FAdd, DestAddr, Val,
                                   AtomicOrdering::SequentiallyConsistent);
  }

  case NVPTX::BI__nvvm_atom_inc_gen_ui:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::UIncWrap, E);

  case NVPTX::BI__nvvm_atom_dec_gen_ui:
    return MakeBinaryAtomicValue(*this, toolchain::AtomicRMWInst::UDecWrap, E);
  ...
  case NVPTX::BI__mma_tf32_m16n16k8_ld_c: {
    ...
    std::optional<toolchain::APSInt> isColMajorArg =
      ...
      for (unsigned i = 0; i < II.NumResults; ++i) {
        Builder.CreateAlignedStore(
            ...
            Builder.CreateGEP(Dst.getElementType(), Dst.emitRawPointer(*this),
                              toolchain::ConstantInt::get(IntTy, i)),
            CharUnits::fromQuantity(4));
      }
    }
    return Result;
  ...
  case NVPTX::BI__mma_m16n16k8_st_c_f32: {
    ...
    toolchain::Type *ParamType = Intrinsic->getFunctionType()->getParamType(1);
  ...
  case NVPTX::BI__mma_tf32_m16n16k8_mma_f32: {
    ...
    std::optional<toolchain::APSInt> LayoutArg =
    ...
    else if (std::optional<toolchain::APSInt> OptSatfArg =
    ...
    toolchain::Type *AType = Intrinsic->getFunctionType()->getParamType(0);
    // Load A
    ...
    // Load B
    toolchain::Type *BType = Intrinsic->getFunctionType()->getParamType(MI.NumEltsA);
    ...
    Value *Result = Builder.CreateCall(Intrinsic, Values);
    toolchain::Type *DType = Dst.getElementType();
