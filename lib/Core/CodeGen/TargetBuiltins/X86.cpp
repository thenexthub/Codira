...
translateX86ToMsvcIntrin(unsigned BuiltinID) {
  ...
  default:
    return std::nullopt;
  case language::Core::X86::BI_BitScanForward:
  case language::Core::X86::BI_BitScanForward64:
    return MSVCIntrin::_BitScanForward;
  case language::Core::X86::BI_BitScanReverse:
  case language::Core::X86::BI_BitScanReverse64:
    return MSVCIntrin::_BitScanReverse;
  case language::Core::X86::BI_InterlockedAnd64:
    return MSVCIntrin::_InterlockedAnd;
  case language::Core::X86::BI_InterlockedCompareExchange128:
    return MSVCIntrin::_InterlockedCompareExchange128;
  case language::Core::X86::BI_InterlockedExchange64:
    return MSVCIntrin::_InterlockedExchange;
  case language::Core::X86::BI_InterlockedExchangeAdd64:
    return MSVCIntrin::_InterlockedExchangeAdd;
  case language::Core::X86::BI_InterlockedExchangeSub64:
    return MSVCIntrin::_InterlockedExchangeSub;
  case language::Core::X86::BI_InterlockedOr64:
    return MSVCIntrin::_InterlockedOr;
  case language::Core::X86::BI_InterlockedXor64:
    return MSVCIntrin::_InterlockedXor;
  case language::Core::X86::BI_InterlockedDecrement64:
    return MSVCIntrin::_InterlockedDecrement;
  case language::Core::X86::BI_InterlockedIncrement64:
...
static Value *getMaskVecValue(CodeGenFunction &CGF, Value *Mask,
  ...
  auto *MaskTy = toolchain::FixedVectorType::get(
...
static Value *EmitX86MaskedStore(CodeGenFunction &CGF, ArrayRef<Value *> Ops,
  ...
  Value *MaskVec = getMaskVecValue(
      CGF, Ops[2],
      cast<toolchain::FixedVectorType>(Ops[1]->getType())->getNumElements());
...
static Value *EmitX86MaskedLoad(CodeGenFunction &CGF, ArrayRef<Value *> Ops,
                                Align Alignment) {
  toolchain::Type *Ty = Ops[1]->getType();
  Value *Ptr = Ops[0];

  Value *MaskVec = getMaskVecValue(
      CGF, Ops[2], cast<toolchain::FixedVectorType>(Ty)->getNumElements());
...
static Value *EmitX86ExpandLoad(CodeGenFunction &CGF,
                                ArrayRef<Value *> Ops) {
  auto *ResultTy = cast<toolchain::VectorType>(Ops[1]->getType());
  Value *Ptr = Ops[0];

  Value *MaskVec = getMaskVecValue(
      CGF, Ops[2], cast<FixedVectorType>(ResultTy)->getNumElements());

  toolchain::Function *F = CGF.CGM.getIntrinsic(Intrinsic::masked_expandload,
                                           ResultTy);
...
static Value *EmitX86CompressExpand(CodeGenFunction &CGF,
                                    ArrayRef<Value *> Ops,
                                    bool IsCompress) {
  auto *ResultTy = cast<toolchain::FixedVectorType>(Ops[1]->getType());

  Value *MaskVec = getMaskVecValue(CGF, Ops[2], ResultTy->getNumElements());

  Intrinsic::ID IID = IsCompress ? Intrinsic::x86_avx512_mask_compress
                                 : Intrinsic::x86_avx512_mask_expand;
  toolchain::Function *F = CGF.CGM.getIntrinsic(IID, ResultTy);
...
static Value *EmitX86CompressStore(CodeGenFunction &CGF,
  ...
  toolchain::Function *F = CGF.CGM.getIntrinsic(Intrinsic::masked_compressstore,
                                           ResultTy);
...
static Value *EmitX86FunnelShift(CodeGenFunction &CGF, Value *Op0, Value *Op1,
  ...
  toolchain::Type *Ty = Op0->getType();

  // Amount may be scalar immediate, in which case create a splat vector.
  // Funnel shifts amounts are treated as modulo and types are all power-of-2 so
  // we only care about the lowest log2 bits anyway.
  if (Amt->getType() != Ty) {
    unsigned NumElts = cast<toolchain::FixedVectorType>(Ty)->getNumElements();
...
static Value *EmitX86vpcom(CodeGenFunction &CGF, ArrayRef<Value *> Ops,
  ...
  Value *Op1 = Ops[1];
  toolchain::Type *Ty = Op0->getType();
  uint64_t Imm = cast<toolchain::ConstantInt>(Ops[2])->getZExtValue() & 0x7;
  ...
  case 0x5:
    ...
    break;
  case 0x6:
    return toolchain::Constant::getNullValue(Ty); // FALSE
  case 0x7:
    return toolchain::Constant::getAllOnesValue(Ty); // TRUE
  default:
    toolchain_unreachable("Unexpected XOP vpcom/vpcomu predicate");
  }
...
static Value *EmitX86Select(CodeGenFunction &CGF,
  ...
  Mask = getMaskVecValue(
      CGF, Mask, cast<toolchain::FixedVectorType>(Op0->getType())->getNumElements());
...
static Value *EmitX86MaskedCompareResult(CodeGenFunction &CGF, Value *Cmp,
  ...
  if (NumElts < 8) {
    ...
    Cmp = CGF.Builder.CreateShuffleVector(
        Cmp, toolchain::Constant::getNullValue(Cmp->getType()), Indices);
  }
...
static Value *EmitX86MaskedCompare(CodeGenFunction &CGF, unsigned CC,
  ...
  unsigned NumElts =
      cast<toolchain::FixedVectorType>(Ops[0]->getType())->getNumElements();
  Value *Cmp;

  if (CC == 3) {
    Cmp = Constant::getNullValue(
        toolchain::FixedVectorType::get(CGF.Builder.getInt1Ty(), NumElts));
  } else if (CC == 7) {
    ...
    switch (CC) {
    default: toolchain_unreachable("Unknown condition code");
...
static Value *EmitX86ConvertIntToFp(CodeGenFunction &CGF, const CallExpr *E,
  ...
  unsigned Rnd = cast<toolchain::ConstantInt>(Ops[3])->getZExtValue();
  toolchain::Type *Ty = Ops[1]->getType();
...
static Value *EmitX86FMAExpr(CodeGenFunction &CGF, const CallExpr *E,
  ...
  switch (BuiltinID) {
  default: break;
  case language::Core::X86::BI__builtin_ia32_vfmsubph512_mask3:
    Subtract = true;
    [[fallthrough]];
  case language::Core::X86::BI__builtin_ia32_vfmaddph512_mask:
  case language::Core::X86::BI__builtin_ia32_vfmaddph512_maskz:
  case language::Core::X86::BI__builtin_ia32_vfmaddph512_mask3:
    IID = Intrinsic::x86_avx512fp16_vfmadd_ph_512;
    break;
  case language::Core::X86::BI__builtin_ia32_vfmsubaddph512_mask3:
    Subtract = true;
    [[fallthrough]];
  case language::Core::X86::BI__builtin_ia32_vfmaddsubph512_mask:
  case language::Core::X86::BI__builtin_ia32_vfmaddsubph512_maskz:
  case language::Core::X86::BI__builtin_ia32_vfmaddsubph512_mask3:
    IID = Intrinsic::x86_avx512fp16_vfmaddsub_ph_512;
    break;
  case language::Core::X86::BI__builtin_ia32_vfmsubps512_mask3:
    Subtract = true;
    [[fallthrough]];
  case language::Core::X86::BI__builtin_ia32_vfmaddps512_mask:
  case language::Core::X86::BI__builtin_ia32_vfmaddps512_maskz:
  case language::Core::X86::BI__builtin_ia32_vfmaddps512_mask3:
    IID = Intrinsic::x86_avx512_vfmadd_ps_512; break;
  case language::Core::X86::BI__builtin_ia32_vfmsubpd512_mask3:
    Subtract = true;
    [[fallthrough]];
  case language::Core::X86::BI__builtin_ia32_vfmaddpd512_mask:
  case language::Core::X86::BI__builtin_ia32_vfmaddpd512_maskz:
  case language::Core::X86::BI__builtin_ia32_vfmaddpd512_mask3:
    IID = Intrinsic::x86_avx512_vfmadd_pd_512; break;
  case language::Core::X86::BI__builtin_ia32_vfmsubaddps512_mask3:
    Subtract = true;
    [[fallthrough]];
  case language::Core::X86::BI__builtin_ia32_vfmaddsubps512_mask:
  case language::Core::X86::BI__builtin_ia32_vfmaddsubps512_maskz:
  case language::Core::X86::BI__builtin_ia32_vfmaddsubps512_mask3:
    IID = Intrinsic::x86_avx512_vfmaddsub_ps_512;
    break;
  case language::Core::X86::BI__builtin_ia32_vfmsubaddpd512_mask3:
    Subtract = true;
    [[fallthrough]];
  case language::Core::X86::BI__builtin_ia32_vfmaddsubpd512_mask:
  case language::Core::X86::BI__builtin_ia32_vfmaddsubpd512_maskz:
  case language::Core::X86::BI__builtin_ia32_vfmaddsubpd512_mask3:
  ...
  if (IID != Intrinsic::not_intrinsic &&
      (cast<toolchain::ConstantInt>(Ops.back())->getZExtValue() != (uint64_t)4 ||
       IsAddSub)) {
    Function *Intr = CGF.CGM.getIntrinsic(IID);
    Res = CGF.Builder.CreateCall(Intr, {A, B, C, Ops.back() });
  } else {
    toolchain::Type *Ty = A->getType();
    Function *FMA;
...
static Value *EmitScalarFMAExpr(CodeGenFunction &CGF, const CallExpr *E,
  ...
  unsigned Rnd = 4;
  if (Ops.size() > 4)
    Rnd = cast<toolchain::ConstantInt>(Ops[4])->getZExtValue();
  ...
  if (Rnd != 4) {
    ...
    case 64:
      ...
      break;
    default:
      toolchain_unreachable("Unexpected size");
    }
...
static Value *EmitX86Muldq(CodeGenFunction &CGF, bool IsSigned,
                           ArrayRef<Value *> Ops) {
  toolchain::Type *Ty = Ops[0]->getType();
  // Arguments have a vXi32 type so cast to vXi64.
  Ty = toolchain::FixedVectorType::get(CGF.Int64Ty,
...
static Value *EmitX86Ternlog(CodeGenFunction &CGF, bool ZeroMask,
  ...
  if (VecWidth == 128 && EltWidth == 32)
    ...
    toolchain_unreachable("Unexpected intrinsic");
...
static Value *EmitX86CvtF16ToFloatExpr(CodeGenFunction &CGF,
  ...
  if (Ops.size() == 4 && cast<toolchain::ConstantInt>(Ops[3])->getZExtValue() != 4) {
    Function *F =
        CGF.CGM.getIntrinsic(Intrinsic::x86_avx512_mask_vcvtph2ps_512);
    return CGF.Builder.CreateCall(F, {Ops[0], Ops[1], Ops[2], Ops[3]});
  }

  unsigned NumDstElts = cast<toolchain::FixedVectorType>(DstTy)->getNumElements();
  Value *Src = Ops[0];

  // Extract the subvector.
  if (NumDstElts !=
      cast<toolchain::FixedVectorType>(Src->getType())->getNumElements()) {
    assert(NumDstElts == 4 && "Unexpected vector size");
    Src = CGF.Builder.CreateShuffleVector(Src, {0, 1, 2, 3});
  }

  // Bitcast from vXi16 to vXf16.
  auto *HalfTy = toolchain::FixedVectorType::get(
      toolchain::Type::getHalfTy(CGF.getLLVMContext()), NumDstElts);
...
Value *CodeGenFunction::EmitX86CpuIs(StringRef CPUStr) {
  ...
  toolchain::Type *STy = toolchain::StructType::get(Int32Ty, Int32Ty, Int32Ty,
                                          toolchain::ArrayType::get(Int32Ty, 1));

  // Grab the global __cpu_model.
  toolchain::Constant *CpuModel = CGM.CreateRuntimeVariable(STy, "__cpu_model");
  cast<toolchain::GlobalValue>(CpuModel)->setDSOLocal(true);

  // Calculate the index needed to access the correct field based on the
  // range. Also adjust the expected value.
  auto [Index, Value] = StringSwitch<std::pair<unsigned, unsigned>>(CPUStr)
#define X86_VENDOR(ENUM, STRING)                                               \
  .Case(STRING, {0u, static_cast<unsigned>(toolchain::X86::ENUM)})
#define X86_CPU_TYPE_ALIAS(ENUM, ALIAS)                                        \
  .Case(ALIAS, {1u, static_cast<unsigned>(toolchain::X86::ENUM)})
#define X86_CPU_TYPE(ENUM, STR)                                                \
  .Case(STR, {1u, static_cast<unsigned>(toolchain::X86::ENUM)})
#define X86_CPU_SUBTYPE_ALIAS(ENUM, ALIAS)                                     \
  .Case(ALIAS, {2u, static_cast<unsigned>(toolchain::X86::ENUM)})
#define X86_CPU_SUBTYPE(ENUM, STR)                                             \
  .Case(STR, {2u, static_cast<unsigned>(toolchain::X86::ENUM)})
#include "toolchain/TargetParser/X86TargetParser.def"
                               .Default({0, 0});
  assert(Value != 0 && "Invalid CPUStr passed to CpuIs");

  // Grab the appropriate field from __cpu_model.
  toolchain::Value *Idxs[] = {ConstantInt::get(Int32Ty, 0),
                         ConstantInt::get(Int32Ty, Index)};
  toolchain::Value *CpuValue = Builder.CreateInBoundsGEP(STy, CpuModel, Idxs);
...
Value *CodeGenFunction::EmitX86CpuSupports(ArrayRef<StringRef> FeatureStrs) {
  return EmitX86CpuSupports(toolchain::X86::getCpuSupportsMask(FeatureStrs));
}
...
CodeGenFunction::EmitX86CpuSupports(std::array<uint32_t, 4> FeatureMask) {
  ...
  toolchain::Type *ATy = toolchain::ArrayType::get(Int32Ty, 3);
  toolchain::Constant *CpuFeatures2 =
      CGM.CreateRuntimeVariable(ATy, "__cpu_features2");
  cast<toolchain::GlobalValue>(CpuFeatures2)->setDSOLocal(true);
  for (int i = 1; i != 4; ++i) {
...
Value *CodeGenFunction::EmitX86CpuInit() {
  toolchain::FunctionType *FTy = toolchain::FunctionType::get(VoidTy,
                                                    /*Variadic*/ false);
...
Value *CodeGenFunction::EmitX86BuiltinExpr(unsigned BuiltinID,
  ...
  auto getCmpIntrinsicCall = [this, &Ops](Intrinsic::ID ID, unsigned Imm) {
    Ops.push_back(toolchain::ConstantInt::get(Int8Ty, Imm));
    toolchain::Function *F = CGM.getIntrinsic(ID);
  ...
  auto getVectorFCmpIR = [this, &Ops, E](CmpInst::Predicate Pred,
    ...
    toolchain::VectorType *FPVecTy = cast<toolchain::VectorType>(Ops[0]->getType());
    toolchain::VectorType *IntVecTy = toolchain::VectorType::getInteger(FPVecTy);
...
#define INTRINSIC_X86_XSAVE_ID(NAME) \
    case X86::BI__builtin_ia32_##NAME: \
      ...
      break
    switch (BuiltinID) {
    default: toolchain_unreachable("Unsupported intrinsic!");
...
#undef INTRINSIC_X86_XSAVE_ID
  ...
  case X86::BI__builtin_ia32_gatherdiv16si: {
    ...
    unsigned MinElts = std::min(
        cast<toolchain::FixedVectorType>(Ops[0]->getType())->getNumElements(),
        cast<toolchain::FixedVectorType>(Ops[2]->getType())->getNumElements());
  ...
  case X86::BI__builtin_ia32_scattersiv8si: {
    ...
    unsigned MinElts = std::min(
        cast<toolchain::FixedVectorType>(Ops[2]->getType())->getNumElements(),
        cast<toolchain::FixedVectorType>(Ops[3]->getType())->getNumElements());
  ...
  case X86::BI__builtin_ia32_extracti64x2_512_mask: {
    auto *DstTy = cast<toolchain::FixedVectorType>(ConvertType(E->getType()));
    unsigned NumElts = DstTy->getNumElements();
    unsigned SrcNumElts =
        cast<toolchain::FixedVectorType>(Ops[0]->getType())->getNumElements();
    unsigned SubVectors = SrcNumElts / NumElts;
    unsigned Index = cast<ConstantInt>(Ops[1])->getZExtValue();
    assert(toolchain::isPowerOf2_32(SubVectors) && "Expected power of 2 subvectors");
  ...
  case X86::BI__builtin_ia32_pblendd256: {
    unsigned NumElts =
        cast<toolchain::FixedVectorType>(Ops[0]->getType())->getNumElements();
    unsigned Imm = cast<toolchain::ConstantInt>(Ops[2])->getZExtValue();
  ...
  case X86::BI__builtin_ia32_pshuflw512: {
    uint32_t Imm = cast<toolchain::ConstantInt>(Ops[1])->getZExtValue();
    auto *Ty = cast<toolchain::FixedVectorType>(Ops[0]->getType());
  ...
  case X86::BI__builtin_ia32_shufps512: {
    uint32_t Imm = cast<toolchain::ConstantInt>(Ops[2])->getZExtValue();
    auto *Ty = cast<toolchain::FixedVectorType>(Ops[0]->getType());
    unsigned NumElts = Ty->getNumElements();
    unsigned NumLanes = Ty->getPrimitiveSizeInBits() / 128;
    unsigned NumLaneElts = NumElts / NumLanes;
  ...
  case X86::BI__builtin_ia32_permdf512: {
    unsigned Imm = cast<toolchain::ConstantInt>(Ops[1])->getZExtValue();
    auto *Ty = cast<toolchain::FixedVectorType>(Ops[0]->getType());
    unsigned NumElts = Ty->getNumElements();
  ...
  case X86::BI__builtin_ia32_palignr512: {
    unsigned ShiftVal = cast<toolchain::ConstantInt>(Ops[2])->getZExtValue() & 0xff;
    ...
    if (ShiftVal > 16) {
      ShiftVal -= 16;
      Ops[1] = Ops[0];
      Ops[0] = toolchain::Constant::getNullValue(Ops[0]->getType());
    }
  ...
  case X86::BI__builtin_ia32_permti256: {
    ...
    for (unsigned l = 0; l != 2; ++l) {
      ...
      if (Imm & (1 << ((l * 4) + 3)))
        OutOps[l] = toolchain::ConstantAggregateZero::get(Ops[0]->getType());
      else if (Imm & (1 << ((l * 4) + 1)))
        OutOps[l] = Ops[1];
      else
        OutOps[l] = Ops[0];
  ...
  case X86::BI__builtin_ia32_pslldqi512_byteshift: {
    unsigned ShiftVal = cast<toolchain::ConstantInt>(Ops[1])->getZExtValue() & 0xff;
    auto *ResultType = cast<toolchain::FixedVectorType>(Ops[0]->getType());
    // Builtin type is vXi64 so multiply by 8 to get bytes.
    unsigned NumElts = ResultType->getNumElements() * 8;

    // If pslldq is shifting the vector more than 15 bytes, emit zero.
    if (ShiftVal >= 16)
      return toolchain::Constant::getNullValue(ResultType);
    ...
    auto *VecTy = toolchain::FixedVectorType::get(Int8Ty, NumElts);
    Value *Cast = Builder.CreateBitCast(Ops[0], VecTy, "cast");
    Value *Zero = toolchain::Constant::getNullValue(VecTy);
  ...
  case X86::BI__builtin_ia32_kshiftlidi: {
    ...
    if (ShiftVal >= NumElts)
      return toolchain::Constant::getNullValue(Ops[0]->getType());
    ...
    Value *Zero = toolchain::Constant::getNullValue(In->getType());
    Value *SV = Builder.CreateShuffleVector(
  ...
  case X86::BI__builtin_ia32_movntss: {
    ...
    SI->setMetadata(toolchain::LLVMContext::MD_nontemporal, Node);
    SI->setAlignment(toolchain::Align(1));
    return SI;
  }
  ...
  case X86::BI__builtin_ia32_cmpq512_mask: {
    unsigned CC = cast<toolchain::ConstantInt>(Ops[2])->getZExtValue() & 0x7;
  ...
  case X86::BI__builtin_ia32_kortestcdi: {
    ...
    Value *C = toolchain::Constant::getAllOnesValue(Ops[0]->getType());
  ...
  case X86::BI__builtin_ia32_kortestzdi: {
    Value *Or = EmitX86MaskLogic(*this, Instruction::Or, Ops);
    Value *C = toolchain::Constant::getNullValue(Ops[0]->getType());
    Value *Cmp = Builder.CreateICmpEQ(Or, C);
    return Builder.CreateZExt(Cmp, ConvertType(E->getType()));
  }
  ...
  case X86::BI__builtin_ia32_sqrtss_round_mask: {
    unsigned CC = cast<toolchain::ConstantInt>(Ops[4])->getZExtValue();
    ...
    if (CC != 4) {
      ...
      switch (BuiltinID) {
      default:
        toolchain_unreachable("Unsupported intrinsic!");
      case X86::BI__builtin_ia32_sqrtsh_round_mask:
  ...
  case X86::BI__builtin_ia32_sqrtpd512: {
    if (Ops.size() == 2) {
      unsigned CC = cast<toolchain::ConstantInt>(Ops[1])->getZExtValue();
      // Support only if the rounding mode is 4 (AKA CUR_DIRECTION),
      // otherwise keep the intrinsic.
      if (CC != 4) {
        Intrinsic::ID IID;
  ...
  case X86::BI__builtin_ia32_cmppd256: {
    ...
    // range [0, 31]
    unsigned CC = cast<toolchain::ConstantInt>(Ops[2])->getZExtValue() & 0x1f;
    ...
    default: toolchain_unreachable("Unhandled CC");
    }
  ...
  case X86::BI__cpuidex: {
    ...
    toolchain::StructType *CpuidRetTy =
        toolchain::StructType::get(Int32Ty, Int32Ty, Int32Ty, Int32Ty);
    toolchain::FunctionType *FTy =
        toolchain::FunctionType::get(CpuidRetTy, {Int32Ty, Int32Ty}, false);
    ...
    toolchain::InlineAsm *IA = toolchain::InlineAsm::get(FTy, Asm, Constraints,
  ...
  case X86::BI__emul:
  case X86::BI__emulu: {
    toolchain::Type *Int64Ty = toolchain::IntegerType::get(getLLVMContext(), 64);
  ...
  case X86::BI__builtin_ia32_t2rpntlvwz1rst1_internal: {
    ...
    toolchain::Type *TyPtee = ConvertType(PtreeTy);
  ...
  case X86::BI__int2c: {
    ...
    toolchain::CallInst *CI = Builder.CreateCall(IA);
  ...
  case X86::BI__readfsqword: {
    ...
    Value *Ptr = Builder.CreateIntToPtr(
        Ops[0], toolchain::PointerType::get(getLLVMContext(), 257));
  ...
  case X86::BI__readgsqword: {
    toolchain::Type *IntTy = ConvertType(E->getType());
    Value *Ptr = Builder.CreateIntToPtr(
        Ops[0], toolchain::PointerType::get(getLLVMContext(), 256));
    LoadInst *Load = Builder.CreateAlignedLoad(
        IntTy, Ptr, getContext().getTypeAlignInChars(E->getType()));
    Load->setVolatile(true);
    return Load;
  ...
  case X86::BI__builtin_ia32_aesdec256kl_u8: {
    ...
    switch (BuiltinID) {
    default:
      toolchain_unreachable("Unexpected builtin");
    case X86::BI__builtin_ia32_aesenc128kl_u8:
    ...
    Constant *Zero = toolchain::Constant::getNullValue(Out->getType());
  ...
  case X86::BI__builtin_ia32_aesdecwide256kl_u8: {
    ...
    toolchain::Type *Ty = FixedVectorType::get(Builder.getInt64Ty(), 2);
    Value *InOps[9];
    InOps[0] = Ops[2];
    for (int i = 0; i != 8; ++i) {
  ...
  case X86::BI__builtin_ia32_vfmaddcsh_round_mask: {
    ...
    Value *Call = Builder.CreateCall(CGM.getIntrinsic(IID), Ops);
    Value *And = Builder.CreateAnd(Ops[3], toolchain::ConstantInt::get(Int8Ty, 1));
  ...
  case X86::BI__builtin_ia32_prefetchi:
    return Builder.CreateCall(
        ...
        {Ops[0], toolchain::ConstantInt::get(Int32Ty, 0), Ops[1],
         toolchain::ConstantInt::get(Int32Ty, 0)});
  }
}
