...
#include "toolchain/IR/IntrinsicsHexagon.h"
...
#include "language/Core/Basic/BuiltinsHexagonMapCustomDep.def"
#undef CUSTOM_BUILTIN_MAPPING
  };

  auto CmpInfo = [] (Info A, Info B) { return A.BuiltinID < B.BuiltinID; };
  static const bool SortOnce = (toolchain::sort(Infos, CmpInfo), true);
  (void)SortOnce;

  const Info *F = toolchain::lower_bound(Infos, Info{BuiltinID, 0, 0}, CmpInfo);
...
Value *CodeGenFunction::EmitHexagonBuiltinExpr(unsigned BuiltinID,
  ...
  auto MakeCircOp = [this, E](unsigned IntID, bool IsLoad) {
    ...
    toolchain::Value *Base = Builder.CreateLoad(BP);
    ...
    SmallVector<toolchain::Value*,5> Ops = { Base };
    for (unsigned i = 1, e = E->getNumArgs(); i != e; ++i)
      Ops.push_back(EmitScalarExpr(E->getArg(i)));

    toolchain::Value *Result = Builder.CreateCall(CGM.getIntrinsic(IntID), Ops);
    ...
    toolchain::Value *NewBase = IsLoad ? Builder.CreateExtractValue(Result, 1)
                                  : Result;
    toolchain::Value *LV = EmitScalarExpr(E->getArg(0));
    Address Dest = EmitPointerWithAlignment(E->getArg(0));
    toolchain::Value *RetVal =
  ...
  auto MakeBrevLd = [this, E](unsigned IntID, toolchain::Type *DestTy) {
    ...
    toolchain::Value *BaseAddress = EmitScalarExpr(E->getArg(0));
    ...
    toolchain::Value *DestAddress = DestAddr.emitRawPointer(*this);

    // Operands are Base, Dest, Modifier.
    // The intrinsic format in LLVM IR is defined as
    // { ValueType, i8* } (i8*, i32).
    toolchain::Value *Result = Builder.CreateCall(
    ...
    toolchain::Value *DestVal = Builder.CreateExtractValue(Result, 0);
  ...
  auto V2Q = [this, VecLen] (toolchain::Value *Vec) {
  ...
  auto Q2V = [this, VecLen] (toolchain::Value *Pred) {
  ...
  case Hexagon::BI__builtin_HEXAGON_V6_vsubcarry_128B: {
    ...
    toolchain::Type *VecType = ConvertType(E->getArg(0)->getType());
    Address PredAddr =
        EmitPointerWithAlignment(E->getArg(2)).withElementType(VecType);
    toolchain::Value *PredIn = V2Q(Builder.CreateLoad(PredAddr));
    toolchain::Value *Result = Builder.CreateCall(CGM.getIntrinsic(ID),
        {EmitScalarExpr(E->getArg(0)), EmitScalarExpr(E->getArg(1)), PredIn});

    toolchain::Value *PredOut = Builder.CreateExtractValue(Result, 1);
  ...
  case Hexagon::BI__builtin_HEXAGON_V6_vmaskedstorentnq_128B: {
    SmallVector<toolchain::Value*,4> Ops;
