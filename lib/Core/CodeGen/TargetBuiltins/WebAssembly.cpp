...
Value *CodeGenFunction::EmitWebAssemblyBuiltinExpr(unsigned BuiltinID,
  ...
  case WebAssembly::BI__builtin_wasm_trunc_s_i64_f64: {
    ...
    toolchain::Type *ResT = ConvertType(E->getType());
    Function *Callee =
  ...
  case WebAssembly::BI__builtin_wasm_test_function_pointer_signature: {
    ...
    // In the toolchain IR, we won't have access any more to the type of the function
    // pointer so we need to insert this type information somehow. The
    // @toolchain.wasm.ref.test.func takes varargs arguments whose values are unused
    // to indicate the type of the function to test for. See the test here:
    // toolchain/test/CodeGen/WebAssembly/ref-test-func.ll
    //
    ...
    toolchain::FunctionType *LLVMFuncTy =
        cast<toolchain::FunctionType>(ConvertType(QualType(FuncTy, 0)));
    ...
    toolchain::Type *RetType = LLVMFuncTy->getReturnType();
    if (!RetType->isVoidTy()) {
      Args.push_back(PoisonValue::get(RetType));
    }
    // The token type indicates the boundary between return types and param
    // types.
    Args.push_back(PoisonValue::get(toolchain::Type::getTokenTy(getLLVMContext())));
  ...
  case WebAssembly::BI__builtin_wasm_abs_i64x2: {
    ...
    Constant *Zero = toolchain::Constant::getNullValue(Vec->getType());
  ...
  case WebAssembly::BI__builtin_wasm_trunc_sat_u_zero_f64x2_i32x4: {
    ...
    toolchain::Type *SrcT = Vec->getType();
    toolchain::Type *TruncT = SrcT->getWithNewType(Builder.getInt32Ty());
