...
Value *CodeGenFunction::EmitSystemZBuiltinExpr(unsigned BuiltinID,
  ...
  case SystemZ::BI__builtin_tbegin: {
    ...
    Value *Control = toolchain::ConstantInt::get(Int32Ty, 0xff0c);
  ...
  case SystemZ::BI__builtin_tbeginc: {
    Value *TDB = toolchain::ConstantPointerNull::get(Int8PtrTy);
    Value *Control = toolchain::ConstantInt::get(Int32Ty, 0xff08);
  ...
  case SystemZ::BI__builtin_s390_verllg: {
    ...
    toolchain::Value *Amt = EmitScalarExpr(E->getArg(1));
    // Splat scalar rotate amount to vector type.
    unsigned NumElts = cast<toolchain::FixedVectorType>(ResultType)->getNumElements();
  ...
  case SystemZ::BI__builtin_s390_vfidb: {
    ...
    toolchain::APSInt M4 = *E->getArg(1)->getIntegerConstantExpr(getContext());
    toolchain::APSInt M5 = *E->getArg(2)->getIntegerConstantExpr(getContext());
    ...
    switch (BuiltinID) { // FIXME: constrained version?
      ...
      default: toolchain_unreachable("Unknown BuiltinID");
    }
    Function *F = CGM.getIntrinsic(ID);
    Value *M4Value = toolchain::ConstantInt::get(getLLVMContext(), M4);
    Value *M5Value = toolchain::ConstantInt::get(getLLVMContext(), M5);
  ...
  case SystemZ::BI__builtin_s390_vfmaxdb: {
    ...
    toolchain::APSInt M4 = *E->getArg(2)->getIntegerConstantExpr(getContext());
    // Check whether this instance can be represented via a LLVM standard
