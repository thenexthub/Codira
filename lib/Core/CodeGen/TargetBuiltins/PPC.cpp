...
static toolchain::Value *emitPPCLoadReserveIntrinsic(CodeGenFunction &CGF,
                                                unsigned BuiltinID,
                                                const CallExpr *E) {
  ...
  toolchain::IntegerType *RetType = CGF.Int32Ty;

  switch (BuiltinID) {
  case language::Core::PPC::BI__builtin_ppc_ldarx:
    AsmOS << "ldarx ";
    RetType = CGF.Int64Ty;
    break;
  case language::Core::PPC::BI__builtin_ppc_lwarx:
    AsmOS << "lwarx ";
    RetType = CGF.Int32Ty;
    break;
  case language::Core::PPC::BI__builtin_ppc_lharx:
    AsmOS << "lharx ";
    RetType = CGF.Int16Ty;
    break;
  case language::Core::PPC::BI__builtin_ppc_lbarx:
    AsmOS << "lbarx ";
    RetType = CGF.Int8Ty;
    break;
  default:
    toolchain_unreachable("Expected only PowerPC load reserve intrinsics");
  }
  ...
  toolchain::Type *PtrType = CGF.UnqualPtrTy;
  toolchain::FunctionType *FTy = toolchain::FunctionType::get(RetType, {PtrType}, false);

  toolchain::InlineAsm *IA =
      toolchain::InlineAsm::get(FTy, Asm, Constraints, /*hasSideEffects=*/true);
  toolchain::CallInst *CI = CGF.Builder.CreateCall(IA, {Addr});
  CI->addParamAttr(
...
#include "toolchain/TargetParser/PPCTargetParser.def"
  auto GenAIXPPCBuiltinCpuExpr = [&](unsigned SupportMethod, unsigned FieldIdx,
                                     unsigned Mask, CmpInst::Predicate CompOp,
                                     unsigned OpValue) -> Value * {
    if (SupportMethod == BUILTIN_PPC_FALSE)
      return toolchain::ConstantInt::getFalse(ConvertType(E->getType()));

    if (SupportMethod == BUILTIN_PPC_TRUE)
      return toolchain::ConstantInt::getTrue(ConvertType(E->getType()));

    assert(SupportMethod <= SYS_CALL && "Invalid value for SupportMethod.");

    toolchain::Value *FieldValue = nullptr;
    if (SupportMethod == USE_SYS_CONF) {
      toolchain::Type *STy = toolchain::StructType::get(PPC_SYSTEMCONFIG_TYPE);
      toolchain::Constant *SysConf =
      ...
      toolchain::FunctionType *FTy =
          toolchain::FunctionType::get(Int64Ty, Int32Ty, false);
    ...
    toolchain::Type *ValueType = FieldValue->getType();
...
#include "toolchain/TargetParser/PPCTargetParser.def"
    ...
    Value *Op0 = toolchain::ConstantInt::get(Int32Ty, PPC_FAWORD_CPUID);
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::ppc_fixed_addr_ld);
    Value *TheCall = Builder.CreateCall(F, {Op0}, "cpu_is");
    return Builder.CreateICmpEQ(TheCall,
                                toolchain::ConstantInt::get(Int32Ty, LinuxIDValue));
  }
...
#include "toolchain/TargetParser/PPCTargetParser.def"
    ...
    Value *Op0 = toolchain::ConstantInt::get(Int32Ty, FeatureWord);
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::ppc_fixed_addr_ld);
    Value *TheCall = Builder.CreateCall(F, {Op0}, "cpu_supports");
    Value *Mask =
        Builder.CreateAnd(TheCall, toolchain::ConstantInt::get(Int32Ty, BitMask));
    return Builder.CreateICmpNE(Mask, toolchain::Constant::getNullValue(Int32Ty));
...
#undef PPC_FAWORD_CPUID
  ...
  case PPC::BI__builtin_vsx_lxvll:
  {
    ...
    switch (BuiltinID) {
    default: toolchain_unreachable("Unsupported ld/lvsl/lvsr intrinsic!");
    case PPC::BI__builtin_altivec_lvx:
  ...
  case PPC::BI__builtin_vsx_stxvll:
  {
    ...
    switch (BuiltinID) {
    default: toolchain_unreachable("Unsupported st intrinsic!");
    case PPC::BI__builtin_altivec_stvx:
  ...
  case PPC::BI__builtin_vsx_ldrmb: {
    ...
    toolchain::Function *Lvx = CGM.getIntrinsic(Intrinsic::ppc_altivec_lvx);
    toolchain::Function *Lvs = CGM.getIntrinsic(IsLE ? Intrinsic::ppc_altivec_lvsr
                                                : Intrinsic::ppc_altivec_lvsl);
    toolchain::Function *Vperm = CGM.getIntrinsic(Intrinsic::ppc_altivec_vperm);
    ...
    Constant *Zero = toolchain::Constant::getNullValue(IsLE ? ResTy : AllElts->getType());
  ...
  case PPC::BI__builtin_vsx_strmb: {
    ...
    auto StoreSubVec = [&](unsigned Width, unsigned Offset, unsigned EltNo) {
      ...
      switch (Width) {
      default:
        toolchain_unreachable("width for stores must be a power of 2");
      case 8:
        ConvTy = Int64Ty;
        NumElts = 2;
        break;
      ...
      Value *Vec = Builder.CreateBitCast(
          Op2, toolchain::FixedVectorType::get(ConvTy, NumElts));
      Value *Ptr =
  ...
  case PPC::BI__builtin_vsx_xvsqrtdp: {
    ...
    if (Builder.getIsFPConstrained()) {
      ...
      toolchain::Function *F = CGM.getIntrinsic(Intrinsic::sqrt, ResultType);
  ...
  case PPC::BI__builtin_altivec_vinsw_elt: {
    ...
    RangeErrMsg += " number " + toolchain::to_string(ConstArg);
    RangeErrMsg += " is outside of the valid range [0, ";
    RangeErrMsg += toolchain::to_string(ValidMaxValue) + "]";
    ...
    Op0 =
        Is32bit
            ? Builder.CreateBitCast(Op0, toolchain::FixedVectorType::get(Int32Ty, 4))
            : Builder.CreateBitCast(Op0,
                                    toolchain::FixedVectorType::get(Int64Ty, 2));
    return Builder.CreateBitCast(
  ...
  case PPC::BI__builtin_altivec_vsubuqm: {
    ...
    Op0 = Builder.CreateBitCast(Op0, toolchain::FixedVectorType::get(Int128Ty, 1));
    Op1 = Builder.CreateBitCast(Op1, toolchain::FixedVectorType::get(Int128Ty, 1));
  ...
  case PPC::BI__builtin_altivec_vsubcuq_c: {
    ...
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    Value *Op1 = EmitScalarExpr(E->getArg(1));
    toolchain::Type *V1I128Ty = toolchain::FixedVectorType::get(
        toolchain::IntegerType::get(getLLVMContext(), 128), 1);
  ...
  case PPC::BI__builtin_ppc_poppar8: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    toolchain::Type *ArgType = Op0->getType();
  ...
  case PPC::BI__builtin_ppc_cmpb: {
    ...
    // %0 = tail call i32 @toolchain.ppc.cmpb32(i32 %conv, i32 %conv1)
    // %conv5 = zext i32 %0 to i64
    // %1 = tail call i32 @toolchain.ppc.cmpb32(i32 %conv2, i32 %conv4)
  ...
  case PPC::BI__builtin_vsx_xvcpsgndp: {
    ...
    toolchain::Function *F = CGM.getIntrinsic(ID, ResultType);
  ...
  case PPC::BI__builtin_vsx_xvabssp: {
    toolchain::Type *ResultType = ConvertType(E->getType());
    Value *X = EmitScalarExpr(E->getArg(0));
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::fabs, ResultType);
    return Builder.CreateCall(F, X);
  }
  ...
  case PPC::BI__builtin_vsx_xvnmsubasp: {
    ...
    toolchain_unreachable("Unknown FMA operation");
  ...
  case PPC::BI__builtin_vsx_insertword: {
    ...
    Value *Op1 = EmitScalarExpr(E->getArg(1));
    Value *Op2 = EmitScalarExpr(E->getArg(2));
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::ppc_vsx_xxinsertw);
    ...
    // vector of long long.
    Op1 = Builder.CreateBitCast(Op1, toolchain::FixedVectorType::get(Int64Ty, 2));

    if (getTarget().isLittleEndian()) {
      // Reverse the double words in the vector we will extract from.
      Op0 = Builder.CreateBitCast(Op0, toolchain::FixedVectorType::get(Int64Ty, 2));
    ...
    Op0 = Builder.CreateBitCast(Op0, toolchain::FixedVectorType::get(Int32Ty, 4));
  ...
  case PPC::BI__builtin_vsx_extractuword: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    Value *Op1 = EmitScalarExpr(E->getArg(1));
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::ppc_vsx_xxextractuw);
  ...
  case PPC::BI__builtin_vsx_xxsldwi: {
    ...
    Op0 = Builder.CreateBitCast(Op0, toolchain::FixedVectorType::get(Int32Ty, 4));
    Op1 = Builder.CreateBitCast(Op1, toolchain::FixedVectorType::get(Int32Ty, 4));
  ...
  case PPC::BI__builtin_pack_vector_int128: {
    ...
    Value *PoisonValue =
        toolchain::PoisonValue::get(toolchain::FixedVectorType::get(Op0->getType(), 2));
  ...
  case PPC::BI__builtin_unpack_vector_int128: {
    ...
    Value *Unpacked = Builder.CreateBitCast(
        Op0, toolchain::FixedVectorType::get(ConvertType(E->getType()), 2));
  ...
  case PPC::BI__builtin_ppc_sthcx: {
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::ppc_sthcx);
    Value *Op0 = EmitScalarExpr(E->getArg(0));
...
#include "language/Core/Basic/BuiltinsPPC.def"
  {
    ...
    // result. However, the toolchain intrinsics return their result in multiple
    ...
    if (BuiltinID == PPC::BI__builtin_mma_disassemble_acc ||
      ...
      toolchain::Function *F = CGM.getIntrinsic(Intrinsic);
      Address Addr = EmitPointerWithAlignment(E->getArg(1));
      Value *Vec = Builder.CreateLoad(Addr);
      Value *Call = Builder.CreateCall(F, {Vec});
      toolchain::Type *VTy = toolchain::FixedVectorType::get(Int8Ty, 16);
      Value *Ptr = Ops[0];
      for (unsigned i=0; i<NumVecs; i++) {
        Value *Vec = Builder.CreateExtractValue(Call, i);
        toolchain::ConstantInt* Index = toolchain::ConstantInt::get(IntTy, i);
  ...
  case PPC::BI__builtin_ppc_compare_and_swaplp: {
    ...
    auto Pair = EmitAtomicCompareExchange(
        ...
        toolchain::AtomicOrdering::Monotonic, toolchain::AtomicOrdering::Monotonic, true);
  ...
  case PPC::BI__builtin_ppc_fetch_and_addlp: {
    return MakeBinaryAtomicValue(*this, AtomicRMWInst::Add, E,
                                 toolchain::AtomicOrdering::Monotonic);
  }
  ...
  case PPC::BI__builtin_ppc_mfspr: {
    Value *Op0 = EmitScalarExpr(E->getArg(0));
    toolchain::Type *RetType = CGM.getDataLayout().getTypeSizeInBits(VoidPtrTy) == 32
                              ? Int32Ty
                              : Int64Ty;
  ...
  case PPC::BI__builtin_ppc_mtfsf: {
    ...
    toolchain::Function *F = CGM.getIntrinsic(Intrinsic::ppc_mtfsf);
