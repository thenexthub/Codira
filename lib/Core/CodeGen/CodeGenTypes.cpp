...
CodeGenTypes::~CodeGenTypes() {
  for (toolchain::FoldingSet<CGFunctionInfo>::iterator
...
/// layout will break.
toolchain::Type *CodeGenTypes::ConvertTypeForMem(QualType T) {
  if (T->isConstantMatrixType()) {
    const Type *Ty = Context.getCanonicalType(T).getTypePtr();
    const ConstantMatrixType *MT = cast<ConstantMatrixType>(Ty);
    return toolchain::ArrayType::get(ConvertType(MT->getElementType()),
                                MT->getNumRows() * MT->getNumColumns());
  }

  toolchain::Type *R = ConvertType(T);

  // Check for the boolean vector case.
  if (T->isExtVectorBoolType()) {
    auto *FixedVT = cast<toolchain::FixedVectorType>(R);

    if (Context.getLangOpts().HLSL) {
      toolchain::Type *IRElemTy = ConvertTypeForMem(Context.BoolTy);
      return toolchain::FixedVectorType::get(IRElemTy, FixedVT->getNumElements());
    }

    // Pad to at least one byte.
    uint64_t BytePadded = std::max<uint64_t>(FixedVT->getNumElements(), 8);
    return toolchain::IntegerType::get(FixedVT->getContext(), BytePadded);
  }
  ...
  if (T->isBitIntType()) {
    if (typeRequiresSplitIntoByteArray(T, R))
      return toolchain::ArrayType::get(CGM.Int8Ty,
                                  Context.getTypeSizeInChars(T).getQuantity());
    return toolchain::IntegerType::get(getLLVMContext(),
...
toolchain::Type *CodeGenTypes::convertTypeForLoadStore(QualType T,
                                                  toolchain::Type *LLVMTy) {
  if (!LLVMTy)
    LLVMTy = ConvertType(T);

  if (T->isBitIntType())
    return toolchain::Type::getIntNTy(
...
bool CodeGenTypes::isRecordLayoutComplete(const Type *Ty) const {
  toolchain::DenseMap<const Type*, toolchain::StructType *>::const_iterator I =
...
static toolchain::Type *getTypeForFormat(toolchain::LLVMContext &VMContext,
                                    const toolchain::fltSemantics &format,
                                    bool UseNativeHalf = false) {
  if (&format == &toolchain::APFloat::IEEEhalf()) {
    if (UseNativeHalf)
      return toolchain::Type::getHalfTy(VMContext);
    else
      return toolchain::Type::getInt16Ty(VMContext);
  }
  if (&format == &toolchain::APFloat::BFloat())
    return toolchain::Type::getBFloatTy(VMContext);
  if (&format == &toolchain::APFloat::IEEEsingle())
    return toolchain::Type::getFloatTy(VMContext);
  if (&format == &toolchain::APFloat::IEEEdouble())
    return toolchain::Type::getDoubleTy(VMContext);
  if (&format == &toolchain::APFloat::IEEEquad())
    return toolchain::Type::getFP128Ty(VMContext);
  if (&format == &toolchain::APFloat::PPCDoubleDouble())
    return toolchain::Type::getPPC_FP128Ty(VMContext);
  if (&format == &toolchain::APFloat::x87DoubleExtended())
    return toolchain::Type::getX86_FP80Ty(VMContext);
  toolchain_unreachable("Unknown float format!");
}

toolchain::Type *CodeGenTypes::ConvertFunctionTypeInternal(QualType QFT) {
  ...
  if (FunctionsBeingProcessed.count(FI)) {
    ...
    ResultType = toolchain::StructType::get(getLLVMContext());
    SkippedLayout = true;
  } else {
...
toolchain::Type *CodeGenTypes::ConvertType(QualType T) {
  T = Context.getCanonicalType(T);
  ...
  toolchain::Type *CachedType = nullptr;
...
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("Non-canonical or dependent types aren't possible.");
  ...
  case Type::Builtin: {
    ...
    case BuiltinType::ObjCSel:
      ...
      ResultType = toolchain::Type::getInt8Ty(getLLVMContext());
      break;

    case BuiltinType::Bool:
      // Note that we always return bool as i1 for use as a scalar type.
      ResultType = toolchain::Type::getInt1Ty(getLLVMContext());
      break;
    ...
    case BuiltinType::SatULongFract:
      ResultType = toolchain::IntegerType::get(getLLVMContext(),
    ...
    case BuiltinType::NullPtr:
      ...
      ResultType = toolchain::PointerType::getUnqual(getLLVMContext());
      break;

    case BuiltinType::UInt128:
    case BuiltinType::Int128:
      ResultType = toolchain::IntegerType::get(getLLVMContext(), 128);
      break;
...
#include "language/Core/Basic/AArch64ACLETypes.def"
      {
        ...
        auto *EltTy = Info.ElementType->isMFloat8Type()
                          ? toolchain::Type::getInt8Ty(getLLVMContext())
                          : ConvertType(Info.ElementType);
        auto *VTy = toolchain::VectorType::get(EltTy, Info.EC);
        switch (Info.NumVectors) {
        default:
          toolchain_unreachable("Expected 1, 2, 3 or 4 vectors!");
        case 1:
          return VTy;
        case 2:
          return toolchain::StructType::get(VTy, VTy);
        case 3:
          return toolchain::StructType::get(VTy, VTy, VTy);
        case 4:
          return toolchain::StructType::get(VTy, VTy, VTy, VTy);
        }
      }
    case BuiltinType::SveCount:
      return toolchain::TargetExtType::get(getLLVMContext(), "aarch64.svcount");
    case BuiltinType::MFloat8:
      return toolchain::VectorType::get(toolchain::Type::getInt8Ty(getLLVMContext()), 1,
                                   false);
#define PPC_VECTOR_TYPE(Name, Id, Size) \
    case BuiltinType::Id: \
      ResultType = \
        toolchain::FixedVectorType::get(ConvertType(Context.BoolTy), Size); \
      break;
...
#include "language/Core/Basic/RISCVVTypes.def"
      {
        ...
        if (Info.NumVectors != 1) {
          ...
          return toolchain::TargetExtType::get(
              getLLVMContext(), "riscv.vector.tuple",
              toolchain::ScalableVectorType::get(
                  toolchain::Type::getInt8Ty(getLLVMContext()), I8EltCount),
              Info.NumVectors);
        }
        return toolchain::ScalableVectorType::get(ConvertType(Info.ElementType),
                                             Info.EC.getKnownMinValue());
      }
#define WASM_REF_TYPE(Name, MangledName, Id, SingletonId, AS)                  \
  case BuiltinType::Id: {                                                      \
    if (BuiltinType::Id == BuiltinType::WasmExternRef)                         \
      ...
      toolchain_unreachable("Unexpected wasm reference builtin type!");             \
  } break;
#include "language/Core/Basic/WebAssemblyReferenceTypes.def"
#define AMDGPU_OPAQUE_PTR_TYPE(Name, Id, SingletonId, Width, Align, AS)        \
  case BuiltinType::Id:                                                        \
    return toolchain::PointerType::get(getLLVMContext(), AS);
#define AMDGPU_NAMED_BARRIER_TYPE(Name, Id, SingletonId, Width, Align, Scope)  \
  case BuiltinType::Id:                                                        \
    return toolchain::TargetExtType::get(getLLVMContext(), "amdgcn.named.barrier",  \
                                    {}, {Scope});
...
#include "language/Core/AST/BuiltinTypes.def"
      toolchain_unreachable("Unexpected placeholder builtin type!");
    }
    break;
  }
  case Type::Auto:
  case Type::DeducedTemplateSpecialization:
    toolchain_unreachable("Unexpected undeduced type!");
  case Type::Complex: {
    toolchain::Type *EltTy = ConvertType(cast<ComplexType>(Ty)->getElementType());
    ResultType = toolchain::StructType::get(EltTy, EltTy);
    break;
  }
  ...
  case Type::RValueReference: {
    ...
    ResultType = toolchain::PointerType::get(getLLVMContext(), AS);
    break;
  }
  case Type::Pointer: {
  ...
  case Type::IncompleteArray: {
    ...
    ResultType = toolchain::ArrayType::get(ResultType, 0);
    break;
  }
  case Type::ArrayParameter:
  case Type::ConstantArray: {
    const ConstantArrayType *A = cast<ConstantArrayType>(Ty);
    toolchain::Type *EltTy = ConvertTypeForMem(A->getElementType());

    // Lower arrays of undefined struct type to arrays of i8 just to have a
    // concrete type.
    if (!EltTy->isSized()) {
      SkippedLayout = true;
      EltTy = toolchain::Type::getInt8Ty(getLLVMContext());
    }

    ResultType = toolchain::ArrayType::get(EltTy, A->getZExtSize());
    break;
  }
  case Type::ExtVector:
  case Type::Vector: {
    ...
    toolchain::Type *IRElemTy = VT->isPackedVectorBoolType(Context)
                               ? toolchain::Type::getInt1Ty(getLLVMContext())
                           : VT->getElementType()->isMFloat8Type()
                               ? toolchain::Type::getInt8Ty(getLLVMContext())
                               : ConvertType(VT->getElementType());
    ResultType = toolchain::FixedVectorType::get(IRElemTy, VT->getNumElements());
    break;
  }
  case Type::ConstantMatrix: {
    const ConstantMatrixType *MT = cast<ConstantMatrixType>(Ty);
    ResultType =
        toolchain::FixedVectorType::get(ConvertType(MT->getElementType()),
                                   MT->getNumRows() * MT->getNumColumns());
    break;
  }
  ...
  case Type::ObjCInterface: {
    ...
    // these.
    toolchain::Type *&T = InterfaceTypes[cast<ObjCInterfaceType>(Ty)];
    if (!T)
      T = toolchain::StructType::create(getLLVMContext());
    ResultType = T;
    break;
  }
  ...
  case Type::Enum: {
    ...
    // "right" answer.
    ResultType = toolchain::Type::getInt32Ty(getLLVMContext());
    break;
  }
  ...
  case Type::MemberPointer: {
    ...
    if (!getCXXABI().isMemberPointerConvertible(MPTy)) {
      ...
      if (Insertion.second)
        Insertion.first->second = toolchain::StructType::create(getLLVMContext());
  ...
  case Type::Atomic: {
    ...
    if (valueSize != atomicSize) {
      ...
      toolchain::Type *elts[] = {
        ResultType,
        toolchain::ArrayType::get(CGM.Int8Ty, (atomicSize - valueSize) / 8)
      };
      ResultType =
          toolchain::StructType::get(getLLVMContext(), toolchain::ArrayRef(elts));
    }
    break;
  }
  case Type::Pipe: {
  ...
  case Type::BitInt: {
    ...
    ResultType = toolchain::Type::getIntNTy(getLLVMContext(), EIT->getNumBits());
    break;
  }
...
toolchain::StructType *CodeGenTypes::ConvertRecordDeclType(const RecordDecl *RD) {
  // TagDecl's are not necessarily unique, instead use the (clang)
  // type connected to the decl.
  const Type *Key = Context.getCanonicalTagType(RD).getTypePtr();

  toolchain::StructType *&Entry = RecordDeclTypes[Key];

  // If we don't have a StructType at all yet, create the forward declaration.
  if (!Entry) {
    Entry = toolchain::StructType::create(getLLVMContext());
    addRecordTypeName(RD, Entry, "");
  }
  toolchain::StructType *Ty = Entry;
