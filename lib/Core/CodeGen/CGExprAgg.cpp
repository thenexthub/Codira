...
class AggExprEmitter : public StmtVisitor<AggExprEmitter> {
  ...
  void withReturnValueSlot(const Expr *E,
                           toolchain::function_ref<RValue(ReturnValueSlot)> Fn);
...
public:
  ...
  void EmitArrayInit(Address DestPtr, toolchain::ArrayType *AType, QualType ArrayQTy,
  ...
  void VisitConstantExpr(ConstantExpr *E) {
    ...
    if (toolchain::Value *Result = ConstantEmitter(CGF).tryEmitConstantExpr(E)) {
  ...
  void VisitArrayInitLoopExpr(const ArrayInitLoopExpr *E,
                              toolchain::Value *outerBegin = nullptr);
...
void AggExprEmitter::withReturnValueSlot(
    const Expr *E, toolchain::function_ref<RValue(ReturnValueSlot)> EmitCall) {
  ...
  toolchain::IntrinsicInst *LifetimeStartInst = nullptr;
  if (!UseTemp) {
    ...
    if (CGF.EmitLifetimeStart(RetAddr.getBasePointer())) {
      LifetimeStartInst =
          cast<toolchain::IntrinsicInst>(std::prev(Builder.GetInsertPoint()));
      assert(LifetimeStartInst->getIntrinsicID() ==
                 toolchain::Intrinsic::lifetime_start &&
...
void AggExprEmitter::EmitCopy(QualType type, const AggValueSlot &dest,
  ...
  if (dest.requiresGCollection()) {
    ...
    toolchain::Value *size = toolchain::ConstantInt::get(CGF.SizeTy, sz.getQuantity());
...
AggExprEmitter::VisitCXXStdInitializerListExpr(CXXStdInitializerListExpr *E) {
  ...
  toolchain::Value *ArrayStart = ArrayPtr.emitRawPointer(CGF);
  CGF.EmitStoreThroughLValue(RValue::get(ArrayStart), Start);
  ++Field;
  assert(Field != Record->field_end() &&
         "Expected std::initializer_list to have two fields");

  toolchain::Value *Size = Builder.getInt(ArrayType->getSize());
  ...
  if (Ctx.hasSameType(Field->getType(), Ctx.getSizeType())) {
    ...
    toolchain::Value *Zero = toolchain::ConstantInt::get(CGF.PtrDiffTy, 0);
    toolchain::Value *IdxEnd[] = { Zero, Size };
...
static void EmitHLSLAggregateSplatCast(CodeGenFunction &CGF, Address DestVal,
                                       QualType DestTy, toolchain::Value *SrcVal,
                                       QualType SrcTy, SourceLocation Loc) {
  // Flatten our destination
  SmallVector<QualType> DestTypes; // Flattened type
  SmallVector<std::pair<Address, toolchain::Value *>, 16> StoreGEPList;
  ...
  for (unsigned I = 0, Size = StoreGEPList.size(); I < Size; ++I) {
    toolchain::Value *Cast =
        CGF.EmitScalarConversion(SrcVal, SrcTy, DestTypes[I], Loc);

    // store back
    toolchain::Value *Idx = StoreGEPList[I].second;
    if (Idx) {
      toolchain::Value *V =
...
static void EmitHLSLScalarFlatCast(CodeGenFunction &CGF, Address DestVal,
  ...
  assert(StoreGEPList.size() <= VT->getNumElements() &&
         ...
         object.");
  for (unsigned I = 0, Size = StoreGEPList.size(); I < Size; I++) {
    toolchain::Value *Load = CGF.Builder.CreateExtractElement(SrcVal, I, "vec.load");
    toolchain::Value *Cast =
...
void AggExprEmitter::EmitArrayInit(Address DestPtr, toolchain::ArrayType *AType,
  ...
  toolchain::Type *toolchainElementType = CGF.ConvertTypeForMem(elementType);
  ...
  if (NumInitElements * elementSize.getQuantity() > 16 &&
    ...
    if (toolchain::Constant *C =
            Emitter.tryEmitForInitializer(ExprToVisit, AS, GVArrayQTy)) {
      auto GV = new toolchain::GlobalVariable(
          CGM.getModule(), C->getType(),
          /* isConstant= */ true, toolchain::GlobalValue::PrivateLinkage, C,
          "constinit",
          /* InsertBefore= */ nullptr, toolchain::GlobalVariable::NotThreadLocal,
  ...
  toolchain::Value *begin = DestPtr.emitRawPointer(CGF);
  if (dtorKind) {
    ...
    toolchain::Instruction *dominatingIP =
        Builder.CreateFlagLoad(toolchain::ConstantInt::getNullValue(CGF.Int8PtrTy));
  ...
  toolchain::Value *one = toolchain::ConstantInt::get(CGF.SizeTy, 1);

  auto Emit = [&](Expr *Init, uint64_t ArrayIndex) {
    toolchain::Value *element = begin;
    if (ArrayIndex > 0) {
      element = Builder.CreateInBoundsGEP(
          toolchainElementType, begin,
          toolchain::ConstantInt::get(CGF.SizeTy, ArrayIndex), "arrayinit.element");
    ...
    LValue elementLV = CGF.MakeAddrLValue(
        Address(element, toolchainElementType, elementAlign), elementType);
  ...
  if (NumInitElements != NumArrayElements &&
    ...
    if (NumInitElements) {
      element = Builder.CreateInBoundsGEP(
          toolchainElementType, element,
          toolchain::ConstantInt::get(CGF.SizeTy, NumInitElements),
          "arrayinit.start");
      if (endOfInit.isValid()) Builder.CreateStore(element, endOfInit);
    }

    // Compute the end of the array.
    toolchain::Value *end = Builder.CreateInBoundsGEP(
        toolchainElementType, begin,
        toolchain::ConstantInt::get(CGF.SizeTy, NumArrayElements), "arrayinit.end");

    toolchain::BasicBlock *entryBB = Builder.GetInsertBlock();
    toolchain::BasicBlock *bodyBB = CGF.createBasicBlock("arrayinit.body");

    // Jump into the body.
    CGF.EmitBlock(bodyBB);
    toolchain::PHINode *currentElement =
    ...
    // Emit the actual filler expression.
    {
      ...
      LValue elementLV = CGF.MakeAddrLValue(
          Address(currentElement, toolchainElementType, elementAlign), elementType);
      if (ArrayFiller)
    ...
    toolchain::Value *nextElement = Builder.CreateInBoundsGEP(
        toolchainElementType, currentElement, one, "arrayinit.next");

    // Tell the EH cleanup that we finished with the last element.
    if (endOfInit.isValid()) Builder.CreateStore(nextElement, endOfInit);

    // Leave the loop if we're done.
    toolchain::Value *done = Builder.CreateICmpEQ(nextElement, end,
                                             "arrayinit.done");
    toolchain::BasicBlock *endBB = CGF.createBasicBlock("arrayinit.end");
...
void AggExprEmitter::VisitCastExpr(CastExpr *E) {
  ...
  case CK_LValueToRValueBitCast: {
    ...
    toolchain::Value *SizeVal = toolchain::ConstantInt::get(
        CGF.SizeTy,
  ...
  case CK_UncheckedDerivedToBase: {
    toolchain_unreachable("cannot perform hierarchy conversion in EmitAggExpr: "
  ...
  case CK_HLSLAggregateSplatCast: {
    ...
    toolchain::Value *SrcVal = RV.getScalarVal();
  ...
  case CK_LValueBitCast:
    toolchain_unreachable("should not be emitting lvalue bitcast as rvalue");
  ...
  case CK_IntegralToFixedPoint:
    toolchain_unreachable("cast kind invalid for aggregate types");
  }
}
...
static toolchain::Value *EmitCompare(CGBuilderTy &Builder, CodeGenFunction &CGF,
                                const BinaryOperator *E, toolchain::Value *LHS,
                                toolchain::Value *RHS, CompareKind Kind,
  ...
  struct CmpInstInfo {
    const char *Name;
    toolchain::CmpInst::Predicate FCmp;
    toolchain::CmpInst::Predicate SCmp;
    toolchain::CmpInst::Predicate UCmp;
  };
  CmpInstInfo InstInfo = [&]() -> CmpInstInfo {
    using FI = toolchain::FCmpInst;
    using II = toolchain::ICmpInst;
    switch (Kind) {
    case CK_Less:
    ...
    toolchain_unreachable("Unrecognised CompareKind enum");
  }();

  if (ArgTy->hasFloatingRepresentation())
    return Builder.CreateFCmp(InstInfo.FCmp, LHS, RHS,
                              toolchain::Twine(InstInfo.Name) + NameSuffix);
  ...
  toolchain_unreachable("unsupported aggregate binary expression should have "
                   "already been handled");
}

void AggExprEmitter::VisitBinCmp(const BinaryOperator *E) {
  using toolchain::BasicBlock;
  using toolchain::PHINode;
...
static bool castPreservesZero(const CastExpr *CE) {
  ...
  toolchain_unreachable("Unhandled language::Core::CastKind enum");
}
...
void AggExprEmitter::EmitNullInitializationToLValue(LValue lv) {
  ...
  if (CGF.hasScalarEvaluationKind(type)) {
    ...
    toolchain::Value *null = CGF.CGM.EmitNullConstant(type);
...
void AggExprEmitter::VisitCXXParenListOrInitListExpr(
  ...
  if (toolchain::Constant *C =
    ...
    new toolchain::GlobalVariable(CGF.CGM.getModule(), C->getType(), true,
                             toolchain::GlobalValue::InternalLinkage, C, "");
  ...
  if (ExprToVisit->getType()->isConstantArrayType()) {
    auto AType = cast<toolchain::ArrayType>(Dest.getAddress().getElementType());
...
void AggExprEmitter::DoZeroInitPadding(uint64_t &PaddingStart,
  ...
  auto InitBytes = [&](uint64_t StartBit, uint64_t EndBit) {
    ...
    toolchain::Constant *SizeVal = Builder.getInt64((End - Start).getQuantity());
  ...
  if (NextField != nullptr && NextField->isBitField()) {
    ...
    if (StorageStart + Info.StorageSize > PaddingStart) {
      ...
      Addr = Addr.withElementType(
          toolchain::Type::getIntNTy(CGF.getLLVMContext(), Info.StorageSize));
...
void AggExprEmitter::VisitArrayInitLoopExpr(const ArrayInitLoopExpr *E,
  ...
  toolchain::Value *indices[] = {zero, zero};
  toolchain::Value *begin = Builder.CreateInBoundsGEP(destPtr.getElementType(),
  ...
  // Jump into the body.
  CGF.EmitBlock(bodyBB);
  toolchain::PHINode *index =
      Builder.CreatePHI(zero->getType(), 2, "arrayinit.index");
  index->addIncoming(zero, entryBB);
  toolchain::Value *element =
      Builder.CreateInBoundsGEP(toolchainElementType, begin, index);
  ...
  // Move on to the next element.
  toolchain::Value *nextIndex = Builder.CreateNUWAdd(
      index, toolchain::ConstantInt::get(CGF.SizeTy, 1), "arrayinit.next");
  index->addIncoming(nextIndex, Builder.GetInsertBlock());

  // Leave the loop if we're done.
  toolchain::Value *done = Builder.CreateICmpEQ(
      nextIndex, toolchain::ConstantInt::get(CGF.SizeTy, numElements),
      "arrayinit.done");
  toolchain::BasicBlock *endBB = CGF.createBasicBlock("arrayinit.end");
  Builder.CreateCondBr(done, endBB, bodyBB);
...
static void CheckAggExprForMemSetUse(AggValueSlot &Slot, const Expr *E,
  ...
  toolchain::Constant *SizeVal = CGF.Builder.getInt64(Size.getQuantity());
...
void CodeGenFunction::EmitAggregateCopy(LValue Dest, LValue Src, QualType Ty,
  ...
  // Aggregate assignment turns into toolchain.memcpy.  This is almost valid per
  ...
  toolchain::Value *SizeVal = nullptr;
  if (TypeInfo.Width.isZero()) {
    ...
    if (auto *VAT = dyn_cast_or_null<VariableArrayType>(
      ...
      SizeVal = Builder.CreateNUWMul(
          SizeVal,
          toolchain::ConstantInt::get(SizeTy, TypeInfo.Width.getQuantity()));
    }
  }
  if (!SizeVal) {
    SizeVal = toolchain::ConstantInt::get(SizeTy, TypeInfo.Width.getQuantity());
  }
  ...
  if (toolchain::MDNode *TBAAStructTag = CGM.getTBAAStructInfo(Ty))
    Inst->setMetadata(toolchain::LLVMContext::MD_tbaa_struct, TBAAStructTag);
