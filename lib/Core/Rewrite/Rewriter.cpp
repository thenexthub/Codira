...
#include "toolchain/ADT/RewriteRope.h"
...
bool Rewriter::overwriteChangedFiles() {
  ...
  for (buffer_iterator I = buffer_begin(), E = buffer_end(); I != E; ++I) {
    ...
    toolchain::SmallString<128> Path(Entry->getName());
    getSourceMgr().getFileManager().makeAbsolutePath(Path);
    if (auto Error = toolchain::writeToOutput(Path, [&](toolchain::raw_ostream &OS) {
          I->second.write(OS);
          return toolchain::Error::success();
        })) {
      Diag.Report(OverwriteFailure)
          << Entry->getName() << toolchain::toString(std::move(Error));
      AllWritten = false;
    }
  }
  return !AllWritten;
