...
#include "toolchain/Support/DynamicLibrary.h"
...
#include "language/Core/CIR/FrontendAction/CIRGenAction.h"
#endif
...
CreateFrontendBaseAction(CompilerInstance &CI) {
  ...
  if (!UseCIR && EmitsCIR)
    toolchain::report_fatal_error("-emit-cir and only valid when using -fclangir");
...
#else
    toolchain_unreachable("CIR suppport not built into clang");
#endif
...
bool ExecuteCompilerInvocation(CompilerInstance *Clang) {
  ...
  if (Clang->getFrontendOpts().ShowHelp) {
    driver::getDriverOptTable().printHelp(
        toolchain::outs(), "clang -cc1 [options] file...",
        "LLVM 'Clang' Compiler: http://clang.toolchain.org",
        /*ShowHidden=*/false, /*ShowAllAliases=*/false,
        toolchain::opt::Visibility(driver::options::CC1Option));
    return true;
  }
  ...
  if (AnOpts.ShowCheckerHelp || AnOpts.ShowCheckerHelpAlpha ||
    ...
    ento::printCheckerHelp(toolchain::outs(), *Clang);
    return true;
  }
  ...
  if (AnOpts.ShowCheckerOptionList || AnOpts.ShowCheckerOptionAlphaList ||
    ...
    ento::printCheckerConfigList(toolchain::outs(), *Clang);
    return true;
  }

  // Honor -analyzer-list-enabled-checkers.
  if (AnOpts.ShowEnabledCheckerList) {
    ento::printEnabledCheckerList(toolchain::outs(), *Clang);
    return true;
  }

  // Honor -analyzer-config-help.
  if (AnOpts.ShowConfigOptionsList) {
    ento::printAnalyzerConfigList(toolchain::outs());
    return true;
  }
#endif
  ...
  if (Clang->getFrontendOpts().DisableFree)
    toolchain::BuryPointer(std::move(Act));
  return Success;
}
