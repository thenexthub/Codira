...
#include "language/Core/APINotes/APINotesWriter.h"
...
class APINotesWriter::Implementation {
  ...
  using VersionedSmallVector =
      toolchain::SmallVector<std::pair<toolchain::VersionTuple, T>, 1>;

  std::string ModuleName;
  const FileEntry *SourceFile;

  /// Scratch space for bitstream writing.
  toolchain::SmallVector<uint64_t, 64> Scratch;

  /// Mapping from strings to identifier IDs.
  toolchain::StringMap<IdentifierID> IdentifierIDs;
  ...
  toolchain::DenseMap<ContextTableKey,
  ...
  toolchain::DenseMap<uint32_t, uint32_t> ParentContexts;

  /// Mapping from context IDs to the identifier ID holding the name.
  toolchain::DenseMap<unsigned, unsigned> ContextNames;
  ...
  toolchain::DenseMap<
      ...
      toolchain::SmallVector<std::pair<VersionTuple, ObjCPropertyInfo>, 1>>
      ObjCProperties;

  /// Information about C record fields.
  ///
  /// Indexed by the context ID and name ID.
  toolchain::DenseMap<SingleDeclTableKey,
                 toolchain::SmallVector<std::pair<VersionTuple, FieldInfo>, 1>>
      Fields;
  ...
  toolchain::DenseMap<std::tuple<unsigned, unsigned, char>,
                 toolchain::SmallVector<std::pair<VersionTuple, ObjCMethodInfo>, 1>>
      ObjCMethods;
  ...
  /// Indexed by the context ID and name ID.
  toolchain::DenseMap<SingleDeclTableKey,
                 toolchain::SmallVector<std::pair<VersionTuple, CXXMethodInfo>, 1>>
      CXXMethods;

  /// Mapping from selectors to selector ID.
  toolchain::DenseMap<StoredObjCSelector, SelectorID> SelectorIDs;
  ...
  toolchain::DenseMap<
      SingleDeclTableKey,
      toolchain::SmallVector<std::pair<VersionTuple, GlobalVariableInfo>, 1>>
      GlobalVariables;
  ...
  toolchain::DenseMap<
      SingleDeclTableKey,
      toolchain::SmallVector<std::pair<VersionTuple, GlobalFunctionInfo>, 1>>
      GlobalFunctions;
  ...
  toolchain::DenseMap<
      unsigned, toolchain::SmallVector<std::pair<VersionTuple, EnumConstantInfo>, 1>>
      EnumConstants;
  ...
  /// Indexed by the context ID, identifier ID.
  toolchain::DenseMap<SingleDeclTableKey,
                 toolchain::SmallVector<std::pair<VersionTuple, TagInfo>, 1>>
      Tags;
  ...
  /// Indexed by the context ID, identifier ID.
  toolchain::DenseMap<SingleDeclTableKey,
                 toolchain::SmallVector<std::pair<VersionTuple, TypedefInfo>, 1>>
      Typedefs;
...
private:
  void writeBlockInfoBlock(toolchain::BitstreamWriter &Stream);
  void writeControlBlock(toolchain::BitstreamWriter &Stream);
  void writeIdentifierBlock(toolchain::BitstreamWriter &Stream);
  void writeContextBlock(toolchain::BitstreamWriter &Stream);
  void writeObjCPropertyBlock(toolchain::BitstreamWriter &Stream);
  void writeObjCMethodBlock(toolchain::BitstreamWriter &Stream);
  void writeCXXMethodBlock(toolchain::BitstreamWriter &Stream);
  void writeFieldBlock(toolchain::BitstreamWriter &Stream);
  void writeObjCSelectorBlock(toolchain::BitstreamWriter &Stream);
  void writeGlobalVariableBlock(toolchain::BitstreamWriter &Stream);
  void writeGlobalFunctionBlock(toolchain::BitstreamWriter &Stream);
  void writeEnumConstantBlock(toolchain::BitstreamWriter &Stream);
  void writeTagBlock(toolchain::BitstreamWriter &Stream);
  void writeTypedefBlock(toolchain::BitstreamWriter &Stream);

public:
  Implementation(toolchain::StringRef ModuleName, const FileEntry *SF)
      : ModuleName(std::string(ModuleName)), SourceFile(SF) {}

  void writeToStream(toolchain::raw_ostream &OS);
};

void APINotesWriter::Implementation::writeToStream(toolchain::raw_ostream &OS) {
  toolchain::SmallVector<char, 0> Buffer;

  {
    toolchain::BitstreamWriter Stream(Buffer);
...
void emitBlockID(toolchain::BitstreamWriter &Stream, unsigned ID,
                 toolchain::StringRef Name) {
  Stream.EmitRecord(toolchain::bitc::BLOCKINFO_CODE_SETBID,
                    toolchain::ArrayRef<unsigned>{ID});

  // Emit the block name if present.
  if (Name.empty())
    return;
  Stream.EmitRecord(
      toolchain::bitc::BLOCKINFO_CODE_BLOCKNAME,
      toolchain::ArrayRef<unsigned char>(
...
void emitRecordID(toolchain::BitstreamWriter &Stream, unsigned ID,
                  toolchain::StringRef Name) {
  assert(ID < 256 && "can't fit record ID in next to name");

  toolchain::SmallVector<unsigned char, 64> Buffer;
  Buffer.resize(Name.size() + 1);
  Buffer[0] = ID;
  memcpy(Buffer.data() + 1, Name.data(), Name.size());

  Stream.EmitRecord(toolchain::bitc::BLOCKINFO_CODE_SETRECORDNAME, Buffer);
}
} // namespace

void APINotesWriter::Implementation::writeBlockInfoBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, toolchain::bitc::BLOCKINFO_BLOCK_ID, 2);
...
void APINotesWriter::Implementation::writeControlBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, CONTROL_BLOCK_ID, 3);
...
public:
  ...
  hash_value_type ComputeHash(key_type_ref Key) { return toolchain::djbHash(Key); }
  ...
  EmitKeyDataLength(raw_ostream &OS, key_type_ref Key, data_type_ref) {
    ...
    toolchain::support::endian::Writer writer(OS, toolchain::endianness::little);
...
void APINotesWriter::Implementation::writeIdentifierBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII restoreBlock(Stream, IDENTIFIER_BLOCK_ID, 3);
  ...
  uint32_t Offset;
  {
    toolchain::OnDiskChainedHashTableGenerator<IdentifierTableInfo> Generator;
    ...
    toolchain::support::endian::write<uint32_t>(BlobStream, 0,
...
unsigned getVersionedInfoSize(
    const toolchain::SmallVectorImpl<std::pair<toolchain::VersionTuple, T>> &VI,
    toolchain::function_ref<unsigned(const typename MakeDependent<T>::Type &)>
        getInfoSize) {
...
void emitVersionedInfo(
    raw_ostream &OS, toolchain::SmallVectorImpl<std::pair<VersionTuple, T>> &VI,
    toolchain::function_ref<void(raw_ostream &,
...
public:
  ...
  using key_type_ref = key_type;
  using data_type =
      toolchain::SmallVector<std::pair<toolchain::VersionTuple, UnversionedDataType>, 1>;
  ...
  void EmitData(raw_ostream &OS, key_type_ref, data_type_ref Data, unsigned) {
    emitVersionedInfo(
        OS, Data, [this](toolchain::raw_ostream &OS, const UnversionedDataType &UI) {
...
public:
  ...
  hash_value_type ComputeHash(key_type_ref Key) {
    return static_cast<size_t>(toolchain::hash_value(Key));
  }
...
void APINotesWriter::Implementation::writeContextBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII restoreBlock(Stream, OBJC_CONTEXT_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<ContextIDTableInfo> Generator;
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<ContextInfoTableInfo> Generator;
      for (auto &OC : Contexts)
...
void APINotesWriter::Implementation::writeObjCPropertyBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, OBJC_PROPERTY_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<ObjCPropertyTableInfo> Generator;
...
void emitFunctionInfo(toolchain::raw_ostream &, const FunctionInfo &);
...
void APINotesWriter::Implementation::writeObjCMethodBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, OBJC_METHOD_BLOCK_ID, 3);
...
void APINotesWriter::Implementation::writeCXXMethodBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, CXX_METHOD_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<CXXMethodTableInfo> Generator;
...
void APINotesWriter::Implementation::writeFieldBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, FIELD_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<FieldTableInfo> Generator;
      for (auto &FD : Fields)
...
public:
  ...
  hash_value_type ComputeHash(key_type_ref Key) {
    return toolchain::DenseMapInfo<StoredObjCSelector>::getHashValue(Key);
  }
...
void APINotesWriter::Implementation::writeObjCSelectorBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, OBJC_SELECTOR_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<ObjCSelectorTableInfo> Generator;
...
void APINotesWriter::Implementation::writeGlobalVariableBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, GLOBAL_VARIABLE_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<GlobalVariableTableInfo> Generator;
...
void APINotesWriter::Implementation::writeGlobalFunctionBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, GLOBAL_FUNCTION_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<GlobalFunctionTableInfo> Generator;
...
void APINotesWriter::Implementation::writeEnumConstantBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, ENUM_CONSTANT_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<EnumConstantTableInfo> Generator;
...
void APINotesWriter::Implementation::writeTagBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, TAG_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<TagTableInfo> Generator;
      for (auto &T : Tags)
...
void APINotesWriter::Implementation::writeTypedefBlock(
    toolchain::BitstreamWriter &Stream) {
  toolchain::BCBlockRAII Scope(Stream, TYPEDEF_BLOCK_ID, 3);
...

  {
    ...
    uint32_t Offset;
    {
      toolchain::OnDiskChainedHashTableGenerator<TypedefTableInfo> Generator;
      for (auto &T : Typedefs)
        Generator.insert(T.first, T.second);
...
APINotesWriter::APINotesWriter(toolchain::StringRef ModuleName, const FileEntry *SF)
    : Implementation(new class Implementation(ModuleName, SF)) {}

APINotesWriter::~APINotesWriter() = default;

void APINotesWriter::writeToStream(toolchain::raw_ostream &OS) {
...
void APINotesWriter::addCXXMethod(ContextID CtxID, toolchain::StringRef Name,
...
void APINotesWriter::addField(ContextID CtxID, toolchain::StringRef Name,
...
void APINotesWriter::addEnumConstant(toolchain::StringRef Name,
...
void APINotesWriter::addTag(std::optional<Context> Ctx, toolchain::StringRef Name,
...
void APINotesWriter::addTypedef(std::optional<Context> Ctx,
                                toolchain::StringRef Name, const TypedefInfo &Info,
                                VersionTuple SwiftVersion) {
