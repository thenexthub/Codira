...
toolchain::VersionTuple ReadVersionTuple(const uint8_t *&Data) {
  uint8_t NumVersions = (*Data++) & 0x03;

  unsigned Major = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
  if (NumVersions == 0)
    return toolchain::VersionTuple(Major);

  unsigned Minor = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
  if (NumVersions == 1)
  ...
  unsigned Build = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
...
public:
  ...
  static std::pair<unsigned, unsigned> ReadKeyDataLength(const uint8_t *&Data) {
    unsigned KeyLength =
        endian::readNext<uint16_t, toolchain::endianness::little>(Data);
    unsigned DataLength =
        endian::readNext<uint16_t, toolchain::endianness::little>(Data);
...
public:
  using internal_key_type = toolchain::StringRef;
  using external_key_type = toolchain::StringRef;
  ...
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    return toolchain::StringRef(reinterpret_cast<const char *>(Data), Length);
  }

  static data_type ReadData(internal_key_type key, const uint8_t *Data,
                            unsigned Length) {
    return endian::readNext<uint32_t, toolchain::endianness::little>(Data);
  }
};
...
public:
  ...
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    ...
    auto NameID = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
...
void ReadVariableInfo(const uint8_t *&Data, VariableInfo &Info) {
  ...
  auto TypeLen = endian::readNext<uint16_t, toolchain::endianness::little>(Data);
...
public:
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    auto ClassID = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
    auto NameID = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
    char IsInstance = endian::readNext<uint8_t, toolchain::endianness::little>(Data);
...
public:
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    auto CtxID = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
    auto NameID = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
    return {CtxID, NameID};
  }
...
void ReadParamInfo(const uint8_t *&Data, ParamInfo &Info) {
  ...
  uint8_t Payload = endian::readNext<uint8_t, toolchain::endianness::little>(Data);
...
void ReadFunctionInfo(const uint8_t *&Data, FunctionInfo &Info) {
  ...
  Info.NullabilityPayload =
      endian::readNext<uint64_t, toolchain::endianness::little>(Data);
...
public:
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    ...
    auto SelectorID =
        endian::readNext<uint32_t, toolchain::endianness::little>(Data);
    auto IsInstance = endian::readNext<uint8_t, toolchain::endianness::little>(Data);
...
public:
  ...
  static bool EqualKey(internal_key_type LHS, internal_key_type RHS) {
    return toolchain::DenseMapInfo<StoredObjCSelector>::isEqual(LHS, RHS);
  }
  ...
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    ...
    Key.NumArgs = endian::readNext<uint16_t, toolchain::endianness::little>(Data);
...
public:
  static internal_key_type ReadKey(const uint8_t *Data, unsigned Length) {
    auto CtxID = endian::readNext<uint32_t, toolchain::endianness::little>(Data);
    auto NameID =
        endian::readNext<IdentifierID, toolchain::endianness::little>(Data);
    return {CtxID, NameID};
  }
...
public:
  ...
  toolchain::MemoryBuffer *InputBuffer;
  ...
  using SerializedCXXMethodTable =
      toolchain::OnDiskIterableChainedHashTable<CXXMethodTableInfo>;

  /// The C++ method table.
  std::unique_ptr<SerializedCXXMethodTable> CXXMethodTable;

  using SerializedObjCSelectorTable =
      toolchain::OnDiskIterableChainedHashTable<ObjCSelectorTableInfo>;

  /// The Objective-C selector table.
  std::unique_ptr<SerializedObjCSelectorTable> ObjCSelectorTable;

  using SerializedGlobalVariableTable =
      toolchain::OnDiskIterableChainedHashTable<GlobalVariableTableInfo>;

  /// The global variable table.
  std::unique_ptr<SerializedGlobalVariableTable> GlobalVariableTable;

  using SerializedGlobalFunctionTable =
      toolchain::OnDiskIterableChainedHashTable<GlobalFunctionTableInfo>;

  /// The global function table.
  std::unique_ptr<SerializedGlobalFunctionTable> GlobalFunctionTable;

  using SerializedEnumConstantTable =
      toolchain::OnDiskIterableChainedHashTable<EnumConstantTableInfo>;

  /// The enumerator table.
  std::unique_ptr<SerializedEnumConstantTable> EnumConstantTable;

  using SerializedTagTable = toolchain::OnDiskIterableChainedHashTable<TagTableInfo>;

  /// The tag table.
  std::unique_ptr<SerializedTagTable> TagTable;

  using SerializedTypedefTable =
      toolchain::OnDiskIterableChainedHashTable<TypedefTableInfo>;
  ...
  std::optional<IdentifierID> getIdentifier(toolchain::StringRef Str);

  /// Retrieve the selector ID for the given selector, or an empty
  /// optional if the string is unknown.
  std::optional<SelectorID> getSelector(ObjCSelectorRef Selector);

  bool readControlBlock(toolchain::BitstreamCursor &Cursor,
                        toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readIdentifierBlock(toolchain::BitstreamCursor &Cursor,
                           toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readContextBlock(toolchain::BitstreamCursor &Cursor,
                        toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readObjCPropertyBlock(toolchain::BitstreamCursor &Cursor,
                             toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readObjCMethodBlock(toolchain::BitstreamCursor &Cursor,
                           toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readCXXMethodBlock(toolchain::BitstreamCursor &Cursor,
                          toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readFieldBlock(toolchain::BitstreamCursor &Cursor,
                      toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readObjCSelectorBlock(toolchain::BitstreamCursor &Cursor,
                             toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readGlobalVariableBlock(toolchain::BitstreamCursor &Cursor,
                               toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readGlobalFunctionBlock(toolchain::BitstreamCursor &Cursor,
                               toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readEnumConstantBlock(toolchain::BitstreamCursor &Cursor,
                             toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readTagBlock(toolchain::BitstreamCursor &Cursor,
                    toolchain::SmallVectorImpl<uint64_t> &Scratch);
  bool readTypedefBlock(toolchain::BitstreamCursor &Cursor,
                        toolchain::SmallVectorImpl<uint64_t> &Scratch);
};

std::optional<IdentifierID>
APINotesReader::Implementation::getIdentifier(toolchain::StringRef Str) {
  if (!IdentifierTable)
    return std::nullopt;
...
bool APINotesReader::Implementation::readControlBlock(
    toolchain::BitstreamCursor &Cursor, toolchain::SmallVectorImpl<uint64_t> &Scratch) {
...
APINotesReader::APINotesReader(toolchain::MemoryBuffer *InputBuffer,
  ...
  toolchain::BitstreamCursor Cursor(*Implementation->InputBuffer);
  ...
  while (!Cursor.AtEndOfStream()) {
    toolchain::Expected<toolchain::BitstreamEntry> MaybeTopLevelEntry = Cursor.advance();
    ...
    toolchain::BitstreamEntry TopLevelEntry = MaybeTopLevelEntry.get();
...
APINotesReader::Create(std::unique_ptr<toolchain::MemoryBuffer> InputBuffer,
                       toolchain::VersionTuple SwiftVersion) {
  bool Failed = false;
...
APINotesReader::VersionedInfo<T>::VersionedInfo(
    ...
    toolchain::SmallVector<std::pair<toolchain::VersionTuple, T>, 1> R)
    : Results(std::move(R)) {

  assert(!Results.empty());
  assert(toolchain::is_sorted(
      Results,
      [](const std::pair<toolchain::VersionTuple, T> &left,
         const std::pair<toolchain::VersionTuple, T> &right) -> bool {
...
auto APINotesReader::lookupObjCClassID(toolchain::StringRef Name)
...
auto APINotesReader::lookupObjCClassInfo(toolchain::StringRef Name)
...
auto APINotesReader::lookupObjCProtocolID(toolchain::StringRef Name)
    -> std::optional<ContextID> {
  if (!Implementation->ContextIDTable)
    return std::nullopt;
...
auto APINotesReader::lookupObjCProtocolInfo(toolchain::StringRef Name)
    -> VersionedInfo<ContextInfo> {
  if (!Implementation->ContextInfoTable)
    return std::nullopt;
...
auto APINotesReader::lookupObjCProperty(ContextID CtxID, toolchain::StringRef Name,
                                        bool IsInstance)
...
auto APINotesReader::lookupField(ContextID CtxID, toolchain::StringRef Name)
...
auto APINotesReader::lookupCXXMethod(ContextID CtxID, toolchain::StringRef Name)
...
auto APINotesReader::lookupGlobalVariable(toolchain::StringRef Name,
...
auto APINotesReader::lookupGlobalFunction(toolchain::StringRef Name,
                                          std::optional<Context> Ctx)
...
auto APINotesReader::lookupEnumConstant(toolchain::StringRef Name)
...
auto APINotesReader::lookupTagID(toolchain::StringRef Name,
...
auto APINotesReader::lookupTag(toolchain::StringRef Name, std::optional<Context> Ctx)
...
auto APINotesReader::lookupTypedef(toolchain::StringRef Name,
                                   std::optional<Context> Ctx)
...
auto APINotesReader::lookupNamespaceID(
    toolchain::StringRef Name, std::optional<ContextID> ParentNamespaceID)
    -> std::optional<ContextID> {
  if (!Implementation->ContextIDTable)
    return std::nullopt;
