...
#include "language/Core/InstallAPI/InstallAPIDiagnostic.h"
#include "toolchain/Demangle/Demangle.h"
#include "toolchain/TextAPI/DylibReader.h"
...
ArchitectureSet &LibAttrs::getArchSet(StringRef Attr) {
  auto *It = toolchain::find_if(LibraryAttributes, [&Attr](const auto &Input) {
...
static std::string demangle(StringRef Name) {
  ...
  if (!isCppMangled(Name))
    return Name.str();
  char *Result = toolchain::itaniumDemangle(Name);
  if (!Result)
    return Name.str();
...
std::string DylibVerifier::getAnnotatedName(const Record *R,
  ...
  toolchain_unreachable("unexpected case for EncodeKind");
}
...
Record *findRecordFromSlice(const RecordsSlice *Slice, StringRef Name,
  ...
  toolchain_unreachable("unexpected end when finding record");
}
...
DylibVerifier::Result DylibVerifier::compareAvailability(const Record *R,
  ...
  case VerificationMode::Invalid:
    toolchain_unreachable("Unexpected verification mode symbol verification");
  }
  toolchain_unreachable("Unexpected verification mode symbol verification");
}
...
void DylibVerifier::VerifierContext::emitDiag(toolchain::function_ref<void()> Report,
                                              RecordLoc *Loc) {
  ...
  if (Loc && Loc->isValid())
    toolchain::errs() << Loc->File << ":" << Loc->Line << ":" << 0 << ": ";
  ...
  auto CompareLibraries = [&](const LibAttrs &Provided, const LibAttrs &Dylib,
    ...
    for (const LibAttrs::Entry &DEntry : Dylib.get()) {
      ...
      if (PAttrEntry->second != DArchSet) {
        if (Fatal)
          toolchain_unreachable("this case was already covered above.");
      }
    }
    return true;
  };
