...
#include "language/Core/InstallAPI/DirectoryScanner.h"
...
HeaderSeq DirectoryScanner::getHeaders(ArrayRef<Library> Libraries) {
  ...
  for (const Library &Lib : Libraries)
    toolchain::append_range(Headers, Lib.Headers);
  return Headers;
}

toolchain::Error DirectoryScanner::scan(StringRef Directory) {
...
toolchain::Error DirectoryScanner::scanForUnwrappedLibraries(StringRef Directory) {
...
static bool isFramework(StringRef Path) {
  ...
  return toolchain::StringSwitch<bool>(toolchain::sys::path::extension(Path))
      .Case(".framework", true)
      .Default(false);
}
...
toolchain::Error DirectoryScanner::scanFrameworkDirectory(StringRef Path,
...
toolchain::Error DirectoryScanner::scanForFrameworks(StringRef Directory) {
  RootPath = "";
