...
#include "language/Core/Support/RISCVVIntrinsicUtils.h"
...
void RVVType::initBuiltinStr() {
  ...
  case ScalarTypeKind::UnsignedInteger:
    ...
    case 64:
      ...
      break;
    default:
      toolchain_unreachable("Unhandled ElementBitwidth!");
    }
    if (isSignedInteger())
  ...
  case ScalarTypeKind::BFloat:
    ...
    break;
  default:
    toolchain_unreachable("ScalarType is invalid!");
  }
  if (IsImmediate)
    BuiltinStr = "I" + BuiltinStr;
  if (isScalar()) {
...
void RVVType::initClangBuiltinStr() {
  ...
  case ScalarTypeKind::UnsignedInteger:
    ...
    break;
  default:
    toolchain_unreachable("ScalarTypeKind is invalid");
  }
  ...
  case ScalarTypeKind::Float:
    if (isScalar()) {
      if (ElementBitwidth == 64)
        ...
        toolchain_unreachable("Unhandled floating type.");
    } else
...
void RVVType::applyBasicType() {
  ...
  case BasicType::BFloat16:
    ...
    break;
  default:
    toolchain_unreachable("Unhandled type code!");
  }
...
PrototypeDescriptor::parsePrototypeDescriptor(
    toolchain::StringRef PrototypeDescriptorStr) {
  ...
  case 'f':
    ...
    break;
  default:
    toolchain_unreachable("Illegal primitive type transformers!");
  }
  ...
  if (PrototypeDescriptorStr.starts_with("(")) {
    ...
    if (ComplexTT.first == "Log2EEW") {
      ...
      if (ComplexTT.second.getAsInteger(10, Log2EEW)) {
        toolchain_unreachable("Invalid Log2EEW value!");
        return std::nullopt;
      }
      switch (Log2EEW) {
      case 3:
      ...
      case 6:
        ...
        break;
      default:
        toolchain_unreachable("Invalid Log2EEW value, should be [3-6]");
        return std::nullopt;
      }
    } else if (ComplexTT.first == "FixedSEW") {
      uint32_t NewSEW;
      if (ComplexTT.second.getAsInteger(10, NewSEW)) {
        toolchain_unreachable("Invalid FixedSEW value!");
        return std::nullopt;
      }
      switch (NewSEW) {
      case 8:
      ...
      case 64:
        ...
        break;
      default:
        toolchain_unreachable("Invalid FixedSEW value, should be 8, 16, 32 or 64");
        return std::nullopt;
      }
    } else if (ComplexTT.first == "LFixedLog2LMUL") {
      int32_t Log2LMUL;
      if (ComplexTT.second.getAsInteger(10, Log2LMUL)) {
        toolchain_unreachable("Invalid LFixedLog2LMUL value!");
        return std::nullopt;
      }
      switch (Log2LMUL) {
      case -3:
      ...
      case 3:
        ...
        break;
      default:
        toolchain_unreachable("Invalid LFixedLog2LMUL value, should be [-3, 3]");
        return std::nullopt;
      }
    } else if (ComplexTT.first == "SFixedLog2LMUL") {
      int32_t Log2LMUL;
      if (ComplexTT.second.getAsInteger(10, Log2LMUL)) {
        toolchain_unreachable("Invalid SFixedLog2LMUL value!");
        return std::nullopt;
      }
      switch (Log2LMUL) {
      case -3:
      ...
      int32_t Log2LMUL;
      if (ComplexTT.second.getAsInteger(10, Log2LMUL)) {
        toolchain_unreachable("Invalid SEFixedLog2LMUL value!");
        return std::nullopt;
      }
      switch (Log2LMUL) {
      case -3:
      ...
      if (ComplexTT.second.getAsInteger(10, NF)) {
        toolchain_unreachable("Invalid NF value!");
        return std::nullopt;
      }
      VTM = getTupleVTM(NF);
    } else {
      toolchain_unreachable("Illegal complex type transformers!");
    }
  }
  ...
  for (char I : PrototypeDescriptorStr) {
    ...
    case 'P':
      if ((TM & TypeModifier::Const) == TypeModifier::Const)
        toolchain_unreachable("'P' transformer cannot be used after 'C'");
      if ((TM & TypeModifier::Pointer) == TypeModifier::Pointer)
        toolchain_unreachable("'P' transformer cannot be used twice");
    ...
    case 'S':
      ...
      break;
    default:
      toolchain_unreachable("Illegal non-primitive type transformer!");
    }
  }
...
void RVVType::applyModifier(const PrototypeDescriptor &Transformer) {
  ...
  for (unsigned TypeModifierMaskShift = 0;
    ...
    case TypeModifier::LMUL1:
      ...
      break;
    default:
      toolchain_unreachable("Unknown type modifier mask!");
    }
  }
}
...
std::string RVVIntrinsic::getSuffixStr(
    ...
    toolchain::ArrayRef<PrototypeDescriptor> PrototypeDescriptors) {
...
toolchain::SmallVector<PrototypeDescriptor> RVVIntrinsic::computeBuiltinTypes(
    toolchain::ArrayRef<PrototypeDescriptor> Prototype, bool IsMasked,
...
toolchain::SmallVector<Policy> RVVIntrinsic::getSupportedUnMaskedPolicies() {
  return {Policy(Policy::PolicyType::Undisturbed)}; // TU
}

toolchain::SmallVector<Policy>
RVVIntrinsic::getSupportedMaskedPolicies(bool HasTailPolicy,
  ...
  if (!HasTailPolicy && HasMaskPolicy)
    return {Policy(Policy::PolicyType::Agnostic,
                   Policy::PolicyType::Undisturbed)}; // MU
  toolchain_unreachable("An RVV instruction should not be without both tail policy "
                   "and mask policy");
}
...
void RVVIntrinsic::updateNamesAndPolicy(
  ...
  if (IsMasked) {
    if (PolicyAttrs.isTUMUPolicy())
      ...
      toolchain_unreachable("Unhandled policy condition");
  } else {
...
SmallVector<PrototypeDescriptor> parsePrototypes(StringRef Prototypes) {
  ...
  while (!Prototypes.empty()) {
    ...
    if (!PD)
      toolchain_unreachable("Error during parsing prototype.");
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, enum PolicyScheme PS) {
  switch (PS) {
