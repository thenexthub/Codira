...
using DirectoryWatcherCallback =
    std::function<void(toolchain::ArrayRef<language::Core::DirectoryWatcher::Event>, bool)>;

using namespace toolchain;
using namespace language::Core;

class DirectoryWatcherWindows : public language::Core::DirectoryWatcher {
...
DirectoryWatcherWindows::DirectoryWatcherWindows(
  ...
  // handle to the watcher and performing synchronous operations.
  {
    ...
    toolchain::sys::windows::UTF16ToUTF8(Data, Size, Path);
  }
...
auto error(DWORD ErrorCode) {
  ...
  if (!FormatMessageA(Flags, NULL, ErrorCode,
                      ...
                      0, NULL)) {
    return make_error<toolchain::StringError>("error " + utostr(ErrorCode),
                                         inconvertibleErrorCode());
  }
  std::string Message{Buffer};
  LocalFree(Buffer);
  return make_error<toolchain::StringError>(Message, inconvertibleErrorCode());
}

} // namespace

toolchain::Expected<std::unique_ptr<DirectoryWatcher>>
language::Core::DirectoryWatcher::create(StringRef Path,
                                DirectoryWatcherCallback Receiver,
  ...
  if (sys::windows::UTF8ToUTF16(Path, WidePath))
    return toolchain::make_error<toolchain::StringError>(
        "unable to convert path to UTF-16", toolchain::inconvertibleErrorCode());
