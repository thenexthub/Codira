...
static SanitizerMask parseArgValues(const Driver &D, const toolchain::opt::Arg *A,
                                    bool DiagnoseErrors);
...
static void parseArgCutoffs(const Driver &D, const toolchain::opt::Arg *A,
                            bool DiagnoseErrors, SanitizerMaskCutoffs &Cutoffs);

/// Parse -f(no-)?sanitize-coverage= flag values, diagnosing any invalid
/// components. Returns OR of members of \c CoverageFeature enumeration.
static int parseCoverageFeatures(const Driver &D, const toolchain::opt::Arg *A,
                                 bool DiagnoseErrors);
...
static int parseBinaryMetadataFeatures(const Driver &D, const toolchain::opt::Arg *A,
                                       bool DiagnoseErrors);
...
/// "-fsanitize=alignment".
static std::string describeSanitizeArg(const toolchain::opt::Arg *A,
                                       SanitizerMask Mask);

/// Produce a string containing comma-separated names of sanitizers in \p
/// Sanitizers set.
static std::string toString(const language::Core::SanitizerSet &Sanitizers);

/// Produce a string containing comma-separated names of sanitizers and
/// sanitizer groups in \p Sanitizers set.
static std::string toStringWithGroups(const language::Core::SanitizerSet &Sanitizers);

/// Return true if an execute-only target disallows data access to code
/// sections.
static bool isExecuteOnlyTarget(const toolchain::Triple &Triple,
                                const toolchain::opt::ArgList &Args) {
  if (Triple.isPS5())
    return true;
...
static void validateSpecialCaseListFormat(const Driver &D,
  ...
  std::string BLError;
  std::unique_ptr<toolchain::SpecialCaseList> SCL(
      toolchain::SpecialCaseList::create(SCLFiles, D.getVFS(), BLError));
...
static void addDefaultIgnorelists(const Driver &D, SanitizerMask Kinds,
  ...
  for (auto BL : Ignorelists) {
    ...
    language::Core::SmallString<64> Path(D.ResourceDir);
    toolchain::sys::path::append(Path, "share", BL.File);
    if (D.getVFS().exists(Path))
      ...
      // should fail.
      D.Diag(language::Core::diag::err_drv_missing_sanitizer_ignorelist) << Path;
  }
  validateSpecialCaseListFormat(
      D, IgnorelistFiles, language::Core::diag::err_drv_malformed_sanitizer_ignorelist,
      DiagnoseErrors);
}
...
static void parseSpecialCaseListArg(const Driver &D,
                                    const toolchain::opt::ArgList &Args,
                                    std::vector<std::string> &SCLFiles,
                                    toolchain::opt::OptSpecifier SCLOptionID,
                                    toolchain::opt::OptSpecifier NoSCLOptionID,
                                    unsigned MalformedSCLErrorDiagID,
                                    bool DiagnoseErrors) {
  for (const auto *Arg : Args) {
    ...
    if (Arg->getOption().matches(SCLOptionID)) {
      ...
      if (D.getVFS().exists(SCLPath)) {
        ...
        D.Diag(language::Core::diag::err_drv_no_such_file) << SCLPath;
      }
...
parseSanitizeArgs(const Driver &D, const toolchain::opt::ArgList &Args,
...
parseSanitizeSkipHotCutoffArgs(const Driver &D, const toolchain::opt::ArgList &Args,
                               bool DiagnoseErrors) {
...
SanitizerArgs::SanitizerArgs(const ToolChain &TC,
  ...
  for (const toolchain::opt::Arg *Arg : toolchain::reverse(Args)) {
    if (Arg->getOption().matches(options::OPT_fsanitize_EQ)) {
      ...
      if (toolchain::opt::Arg *A = Args.getLastArg(options::OPT_mcmodel_EQ)) {
      ...
      if (isExecuteOnlyTarget(Triple, Args)) {
        if (SanitizerMask KindsToDiagnose =
          ...
          if (DiagnoseErrors) {
            std::string Desc = describeSanitizeArg(Arg, KindsToDiagnose);
            toolchain::opt::Arg *A = Args.getLastArgNoClaim(
      ...
      //   address to toolchain.type.test and a pointer to the address point to the
      ...
      if ((Add & SanitizerKind::Vptr) && (RTTIMode == ToolChain::RM_Disabled)) {
        if (const toolchain::opt::Arg *NoRTTIArg = TC.getRTTIArg()) {
  ...
  if ((Kinds & SanitizerKind::ShadowCallStack) && TC.getTriple().isAArch64() &&
      !toolchain::AArch64::isX18ReservedByDefault(TC.getTriple()) &&
  ...
  parseSpecialCaseListArg(
      ...
      language::Core::diag::err_drv_malformed_sanitizer_ignorelist, DiagnoseErrors);
  ...
  if (Arg *A = Args.getLastArg(
    ...
    if (S.getAsInteger(0, CoverageStackDepthCallbackMin) ||
      ...
      if (DiagnoseErrors)
        D.Diag(language::Core::diag::err_drv_invalid_value) << A->getAsString(Args) << S;
    }
  }
  ...
  if (AllAddedKinds & SanitizerKind::MemTag) {
    ...
    if (S == "async" || S == "sync") {
      ...
      D.Diag(language::Core::diag::err_drv_invalid_value_with_suggestion)
  ...
  if (DiagnoseErrors) {
    ...
    if (CoverageFeatures & CoverageTraceBB)
      D.Diag(language::Core::diag::warn_drv_deprecated_arg)
  ...
  if (CoverageFeatures) {
    parseSpecialCaseListArg(
        ...
        language::Core::diag::err_drv_malformed_sanitizer_coverage_allowlist,
        DiagnoseErrors);
    parseSpecialCaseListArg(
        D, Args, CoverageIgnorelistFiles,
        options::OPT_fsanitize_coverage_ignorelist, OptSpecifier(),
        language::Core::diag::err_drv_malformed_sanitizer_coverage_ignorelist,
        DiagnoseErrors);
  }
  ...
  if (BinaryMetadataFeatures) {
    parseSpecialCaseListArg(
        ...
        language::Core::diag::err_drv_malformed_sanitizer_metadata_ignorelist,
        DiagnoseErrors);
  }
  ...
  if (!SharedRuntime && TC.getTriple().isOSWindows()) {
    ...
    D.Diag(language::Core::diag::err_drv_unsupported_opt_for_target)
  ...
  if (AllAddedKinds & SanitizerKind::Address) {
    ...
    if (Arg *WindowsDebugRTArg =
      ...
      case options::OPT__SLASH_LDd:
        if (DiagnoseErrors) {
          ...
          D.Diag(language::Core::diag::note_drv_address_sanitizer_debug_runtime);
        }
      }
    }
    ...
    if (TC.getTriple().isOSDarwin() &&
      ...
      AsanDtorKind = toolchain::AsanDtorKind::None;
    }

    if (const auto *Arg =
            Args.getLastArg(options::OPT_sanitize_address_destructor_EQ)) {
      auto parsedAsanDtorKind = AsanDtorKindFromString(Arg->getValue());
      if (parsedAsanDtorKind == toolchain::AsanDtorKind::Invalid && DiagnoseErrors) {
        TC.getDriver().Diag(language::Core::diag::err_drv_unsupported_option_argument)
    ...
    if ((AllAddedKinds & DetectInvalidPointerPairs & ~AllRemove) &&
        DiagnoseErrors) {
      TC.getDriver().Diag(language::Core::diag::err_drv_argument_only_allowed_with)
  ...
  if (AllAddedKinds & SanitizerKind::HWAddress) {
    ...
    if (TC.getTriple().getArch() == toolchain::Triple::x86_64)
...
static std::string toString(const language::Core::SanitizerMaskCutoffs &Cutoffs) {
  toolchain::SmallVector<std::string, 4> Res;
  serializeSanitizerMaskCutoffs(Cutoffs, Res);
  return toolchain::join(Res, ",");
}
...
static void addSpecialCaseListOpt(const toolchain::opt::ArgList &Args,
...
static void addIncludeLinkerOption(const ToolChain &TC,
  ...
  if (TC.getTriple().getArch() == toolchain::Triple::x86) {
...
static bool hasTargetFeatureMTE(const toolchain::opt::ArgStringList &CmdArgs) {
...
void SanitizerArgs::addArgs(const ToolChain &TC, const toolchain::opt::ArgList &Args,
                            toolchain::opt::ArgStringList &CmdArgs,
  ...
  // FIXME: Pass these parameters as function attributes, not as -toolchain flags.
  if (!TsanMemoryAccess) {
  ...
  if (KcfiArity) {
    if (!TC.getTriple().isOSLinux() || !TC.getTriple().isArch64Bit()) {
      TC.getDriver().Diag(language::Core::diag::err_drv_kcfi_arity_unsupported_target)
          << TC.getTriple().str();
    }
  ...
  if (AsanDtorKind != toolchain::AsanDtorKind::Invalid) {
    CmdArgs.push_back(Args.MakeArgString("-fsanitize-address-destructor=" +
                                         AsanDtorKindToString(AsanDtorKind)));
  }

  if (AsanUseAfterReturn != toolchain::AsanDetectStackUseAfterReturnMode::Invalid) {
    CmdArgs.push_back(Args.MakeArgString(
  ...
  // pass. (see toolchain::maybeMarkSanitizerLibraryCallNoBuiltin)
...
SanitizerMask parseArgValues(const Driver &D, const toolchain::opt::Arg *A,
                             bool DiagnoseErrors) {
  assert(
...
void parseArgCutoffs(const Driver &D, const toolchain::opt::Arg *A,
                     bool DiagnoseErrors, SanitizerMaskCutoffs &Cutoffs) {
...
static int parseOverflowPatternExclusionValues(const Driver &D,
  ...
  for (int i = 0, n = A->getNumValues(); i != n; ++i) {
    const char *Value = A->getValue(i);
    int E =
        toolchain::StringSwitch<int>(Value)
...
int parseCoverageFeatures(const Driver &D, const toolchain::opt::Arg *A,
                          bool DiagnoseErrors) {
  ...
  for (int i = 0, n = A->getNumValues(); i != n; ++i) {
    const char *Value = A->getValue(i);
    int F = toolchain::StringSwitch<int>(Value)
...
int parseBinaryMetadataFeatures(const Driver &D, const toolchain::opt::Arg *A,
                                bool DiagnoseErrors) {
  assert(
...
std::string lastArgumentForMask(const Driver &D, const toolchain::opt::ArgList &Args,
                                SanitizerMask Mask) {
  for (toolchain::opt::ArgList::const_reverse_iterator I = Args.rbegin(),
                                                  E = Args.rend();
       I != E; ++I) {
    const auto *Arg = *I;
    if (Arg->getOption().matches(options::OPT_fsanitize_EQ)) {
  ...
  toolchain_unreachable("arg list didn't provide expected value");
}

std::string describeSanitizeArg(const toolchain::opt::Arg *A, SanitizerMask Mask) {
