...
bool Multilib::operator==(const Multilib &Other) const {
  ...
  toolchain::StringSet<> MyFlags(toolchain::from_range, Flags);
...
raw_ostream &language::Core::driver::operator<<(raw_ostream &OS, const Multilib &M) {
  M.print(OS);
  return OS;
}

MultilibSet &MultilibSet::FilterOut(FilterCallback F) {
  toolchain::erase_if(Multilibs, F);
  return *this;
}
...
static void DiagnoseUnclaimedMultilibCustomFlags(
  ...
  for (StringRef Unclaimed : UnclaimedCustomFlagValues) {
    ...
    if (!BestCandidate)
      D.Diag(language::Core::diag::err_drv_unsupported_opt)
          << (custom_flag::Prefix + Unclaimed).str();
    else
      D.Diag(language::Core::diag::err_drv_unsupported_opt_with_suggestion)
          << (custom_flag::Prefix + Unclaimed).str()
          << (custom_flag::Prefix + BestCandidate->FlagValue).str();
  }
}

namespace language::Core::driver::custom_flag {
...
MultilibSet::processCustomFlags(const Driver &D,
  ...
  // is selected).
  toolchain::SmallPtrSet<custom_flag::Declaration *, 32> TriggeredCustomFlagDecls;

  // Detect multiple values for the same flag declaration. Last one wins.
  for (auto *CustomFlagValue : toolchain::reverse(ClaimedCustomFlagValues)) {
...
bool MultilibSet::select(
    ...
    toolchain::SmallVectorImpl<Multilib> &Selected,
    toolchain::SmallVector<StringRef> *CustomFlagMacroDefines) const {
  auto [FlagsWithCustom, CFMacroDefines] = processCustomFlags(D, Flags);
  toolchain::StringSet<> FlagSet(expandFlags(FlagsWithCustom));
  Selected.clear();
  ...
  toolchain::DenseSet<StringRef> ExclusiveGroupsSelected;
  for (const Multilib &M : toolchain::reverse(Multilibs)) {
    // If this multilib doesn't match all our flags, don't select it.
    if (!toolchain::all_of(M.flags(), [&FlagSet](const std::string &F) {
...
MultilibSet::expandFlags(const Multilib::flags_list &InFlags) const {
  toolchain::StringSet<> Result(toolchain::from_range, InFlags);
  for (const FlagMatcher &M : FlagMatchers) {
    ...
    const toolchain::Regex Regex(RegexString);
    assert(Regex.isValid());
    if (toolchain::any_of(InFlags,
...
struct MultilibSetSerialization {
  toolchain::VersionTuple MultilibVersion;
...
template <> struct toolchain::yaml::MappingTraits<MultilibSerialization> {
  static void mapping(toolchain::yaml::IO &io, MultilibSerialization &V) {
...
template <> struct toolchain::yaml::ScalarEnumerationTraits<MultilibGroupType> {
  static void enumeration(IO &io, MultilibGroupType &Val) {
    io.enumCase(Val, "Exclusive", MultilibGroupType::Exclusive);
  }
};

template <> struct toolchain::yaml::MappingTraits<MultilibGroupSerialization> {
  static void mapping(toolchain::yaml::IO &io, MultilibGroupSerialization &V) {
    io.mapRequired("Name", V.Name);
    io.mapRequired("Type", V.Type);
  }
};

template <> struct toolchain::yaml::MappingTraits<MultilibSet::FlagMatcher> {
  static void mapping(toolchain::yaml::IO &io, MultilibSet::FlagMatcher &M) {
    io.mapRequired("Match", M.Match);
    io.mapRequired("Flags", M.Flags);
  }
  static std::string validate(IO &io, MultilibSet::FlagMatcher &M) {
    toolchain::Regex Regex(M.Match);
...
template <>
struct toolchain::yaml::MappingContextTraits<custom_flag::ValueDetail,
                                        toolchain::SmallSet<std::string, 32>> {
  static void mapping(toolchain::yaml::IO &io, custom_flag::ValueDetail &V,
                      toolchain::SmallSet<std::string, 32> &) {
    io.mapRequired("Name", V.Name);
    io.mapOptional("MacroDefines", V.MacroDefines);
  }
  static std::string validate(IO &io, custom_flag::ValueDetail &V,
                              toolchain::SmallSet<std::string, 32> &NameSet) {
    if (V.Name.empty())
...
template <>
struct toolchain::yaml::MappingContextTraits<custom_flag::Declaration,
                                        toolchain::SmallSet<std::string, 32>> {
  static void mapping(toolchain::yaml::IO &io, custom_flag::Declaration &V,
                      toolchain::SmallSet<std::string, 32> &NameSet) {
    io.mapRequired("Name", V.Name);
    ...
    for (auto [Idx, Value] : toolchain::enumerate(V.ValueList)) {
      Value.Decl = &V;
...
template <> struct toolchain::yaml::MappingTraits<MultilibSetSerialization> {
  static void mapping(toolchain::yaml::IO &io, MultilibSetSerialization &M) {
...
toolchain::ErrorOr<MultilibSet>
MultilibSet::parseYaml(toolchain::MemoryBufferRef Input,
                       toolchain::SourceMgr::DiagHandlerTy DiagHandler,
                       void *DiagHandlerCtxt) {
  MultilibSetSerialization MS;
  toolchain::yaml::Input YamlInput(Input, nullptr, DiagHandler, DiagHandlerCtxt);
  YamlInput >> MS;
  if (YamlInput.error())
...
raw_ostream &language::Core::driver::operator<<(raw_ostream &OS, const MultilibSet &MS) {
  MS.print(OS);
  return OS;
}
