...
#include "language/Core/Driver/Phases.h"
...
#include "toolchain/Support/ExitCodes.h"
...
template <typename F> static bool usesInput(const ArgList &Args, F &&Fn) {
  return toolchain::any_of(Args, [&](Arg *A) {
...
std::string Driver::GetResourcesPath(StringRef BinaryPath) {
  ...
  StringRef Dir = toolchain::sys::path::parent_path(BinaryPath);
  SmallString<128> P(Dir);

  StringRef ConfiguredResourceDir(CLANG_RESOURCE_DIR);
  if (!ConfiguredResourceDir.empty()) {
    // FIXME: We should fix the behavior of toolchain::sys::path::append so we don't
    // need to check for absolute paths here.
    if (toolchain::sys::path::is_absolute(ConfiguredResourceDir))
      P = ConfiguredResourceDir;
    else
      toolchain::sys::path::append(P, ConfiguredResourceDir);
  } else {
    ...
    P = toolchain::sys::path::parent_path(Dir);
    // This search path is also created in the COFF driver of lld, so any
    // changes here also needs to happen in lld/COFF/Driver.cpp
    toolchain::sys::path::append(P, CLANG_INSTALL_LIBDIR_BASENAME, "clang",
...
CUIDOptions::CUIDOptions(toolchain::opt::DerivedArgList &Args, const Driver &D)
    : UseCUID(Kind::Hash) {
  if (Arg *A = Args.getLastArg(options::OPT_fuse_cuid_EQ)) {
    StringRef UseCUIDStr = A->getValue();
    UseCUID = toolchain::StringSwitch<Kind>(UseCUIDStr)
                  .Case("hash", Kind::Hash)
                  .Case("random", Kind::Random)
                  .Case("none", Kind::None)
                  .Default(Kind::Invalid);
    if (UseCUID == Kind::Invalid)
      D.Diag(language::Core::diag::err_drv_invalid_value)
...
std::string CUIDOptions::getCUID(StringRef InputFile,
                                 toolchain::opt::DerivedArgList &Args) const {
  std::string CUID = FixedCUID.str();
  if (CUID.empty()) {
    if (UseCUID == Kind::Random)
      CUID = toolchain::utohexstr(toolchain::sys::Process::GetRandomNumber(),
                             /*LowerCase=*/true);
      ...
      Hasher.final(Hash);
      CUID = toolchain::utohexstr(Hash.low(), /*LowerCase=*/true);
    }
  }
  return CUID;
}
Driver::Driver(StringRef ClangExecutable, StringRef TargetTriple,
  ...
  if (!this->VFS)
    this->VFS = toolchain::vfs::getRealFileSystem();

  Name = std::string(toolchain::sys::path::filename(ClangExecutable));
  Dir = std::string(toolchain::sys::path::parent_path(ClangExecutable));

  if ((!SysRoot.empty()) && toolchain::sys::path::is_relative(SysRoot)) {
    // Prepend InstalledDir if SysRoot is relative
    SmallString<128> P(Dir);
    toolchain::sys::path::append(P, SysRoot);
    SysRoot = std::string(P);
  }

#if defined(CLANG_CONFIG_FILE_SYSTEM_DIR)
  if (toolchain::sys::path::is_absolute(CLANG_CONFIG_FILE_SYSTEM_DIR)) {
    SystemConfigDir = CLANG_CONFIG_FILE_SYSTEM_DIR;
  } else {
    SmallString<128> configFileDir(Dir);
    toolchain::sys::path::append(configFileDir, CLANG_CONFIG_FILE_SYSTEM_DIR);
    toolchain::sys::path::remove_dots(configFileDir, true);
  ...
  {
    SmallString<128> P;
    toolchain::sys::fs::expand_tilde(CLANG_CONFIG_FILE_USER_DIR, P);
...
void Driver::setDriverMode(StringRef Value) {
  ...
  if (auto M = toolchain::StringSwitch<std::optional<DriverMode>>(Value)
                   .Case("gcc", GCCMode)
                   .Case("g++", GXXMode)
                   .Case("cpp", CPPMode)
                   .Case("cl", CLMode)
...
InputArgList Driver::ParseArgStrings(ArrayRef<const char *> ArgStrings,
  ...
  toolchain::PrettyStackTraceString CrashInfo("Command line argument parsing");
  ContainsError = false;

  toolchain::opt::Visibility VisibilityMask = getOptionVisibilityMask(UseDriverMode);
    ...
    if (getOpts().findNearest(ArgString, Nearest, VisibilityMask) > 1) {
      if (!IsCLMode() &&
          getOpts().findExact(ArgString, Nearest,
                              toolchain::opt::Visibility(options::CC1Option))) {
...
toolchain::Expected<std::unique_ptr<toolchain::MemoryBuffer>>
Driver::executeProgram(toolchain::ArrayRef<toolchain::StringRef> Args) const {
  toolchain::SmallString<64> OutputFile;
  toolchain::sys::fs::createTemporaryFile("driver-program", "txt", OutputFile,
                                     toolchain::sys::fs::OF_Text);
  toolchain::FileRemover OutputRemover(OutputFile.c_str());
  ...
  if (std::optional<std::string> Str =
          toolchain::sys::Process::GetEnv("CLANG_TOOLCHAIN_PROGRAM_TIMEOUT")) {
    if (!toolchain::to_integer(*Str, SecondsToWait))
      return toolchain::createStringError(std::error_code(),
  ...
  if (toolchain::sys::ExecuteAndWait(Executable, Args, {}, Redirects, SecondsToWait,
                                /*MemoryLimit=*/0, &ErrorMessage))
    return toolchain::createStringError(std::error_code(),
                                   Executable + ": " + ErrorMessage);

  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> OutputBuf =
      toolchain::MemoryBuffer::getFile(OutputFile.c_str());
  if (!OutputBuf)
    return toolchain::createStringError(OutputBuf.getError(),
...
DerivedArgList *Driver::TranslateInputArgs(const InputArgList &Args) const {
  const toolchain::opt::OptTable &Opts = getOpts();
...
static void setZosTargetVersion(const Driver &D, toolchain::Triple &Target,
                                StringRef ArgTarget) {
  ...
  if (ArgTarget.equals_insensitive("CURRENT")) {
    ...
    bool IsOk = true;
    toolchain::Regex ZOsvRegex("[zZ][oO][sS][vV]([0-9])[rR]([0-9])");
    toolchain::Regex HexRegex(
        "0x4"                      /* product      */
        "([0-9a-fA-F])"            /* version     */
    ...
    if (IsOk) {
      toolchain::VersionTuple V(Version, Release, Modification);
      toolchain::VersionTuple TV = Target.getOSVersion();
      // The goal is to pick the minimally supported version of
      // the OS.  Pick the lesser as the target.
      if (TV.empty() || V < TV) {
        SmallString<16> Str;
        Str = toolchain::Triple::getOSTypeName(Target.getOS());
        Str += V.getAsString();
...
static toolchain::Triple computeTargetTriple(const Driver &D,
  ...
  toolchain::Triple Target(toolchain::Triple::normalize(TargetTriple));
  ...
  if (Arg *A = Args.getLastArgNoClaim(options::OPT_mlittle_endian,
    ...
    toolchain::Triple T = A->getOption().matches(options::OPT_mlittle_endian)
                         ? Target.getLittleEndianArchVariant()
                         : Target.getBigEndianArchVariant();
    if (T.getArch() != toolchain::Triple::UnknownArch) {
      Target = std::move(T);
  ...
  if (Target.getArch() == toolchain::Triple::tce)
    return Target;

  // On AIX, the env OBJECT_MODE may affect the resulting arch variant.
  if (Target.isOSAIX()) {
    if (std::optional<std::string> ObjectModeValue =
            toolchain::sys::Process::GetEnv("OBJECT_MODE")) {
      StringRef ObjectMode = *ObjectModeValue;
      toolchain::Triple::ArchType AT = toolchain::Triple::UnknownArch;
      ...
      if (AT != toolchain::Triple::UnknownArch && AT != Target.getArch())
        Target.setArch(AT);
    }
  }

  // Currently the only architecture supported by *-uefi triples are x86_64.
  if (Target.isUEFI() && Target.getArch() != toolchain::Triple::x86_64)
  ...
  if (A) {
    ...
    if (A->getOption().matches(options::OPT_m64) ||
      ...
      AT = Target.get64BitArchVariant().getArch();
      if (Target.getEnvironment() == toolchain::Triple::GNUX32 ||
          Target.getEnvironment() == toolchain::Triple::GNUT64)
        Target.setEnvironment(toolchain::Triple::GNU);
      else if (Target.getEnvironment() == toolchain::Triple::MuslX32)
        Target.setEnvironment(toolchain::Triple::Musl);
    } else if (A->getOption().matches(options::OPT_mx32) &&
               Target.get64BitArchVariant().getArch() == toolchain::Triple::x86_64) {
      AT = toolchain::Triple::x86_64;
      if (Target.getEnvironment() == toolchain::Triple::Musl)
        Target.setEnvironment(toolchain::Triple::MuslX32);
      else
        Target.setEnvironment(toolchain::Triple::GNUX32);
      ...
      if (D.IsFlangMode() && !Target.isOSAIX()) {
        ...
        if (Target.getEnvironment() == toolchain::Triple::GNUX32)
          ...
          Target.setEnvironment(toolchain::Triple::Musl);
               ...
               Target.get32BitArchVariant().getArch() == toolchain::Triple::x86) {
      AT = toolchain::Triple::x86;
      Target.setEnvironment(toolchain::Triple::CODE16);
    }
  ...
  // Handle -miamcu flag.
  if (Args.hasFlag(options::OPT_miamcu, options::OPT_mno_iamcu, false)) {
    if (Target.get32BitArchVariant().getArch() != toolchain::Triple::x86)
    ...
    Target.setArch(toolchain::Triple::x86);
    Target.setArchName("i586");
    Target.setEnvironment(toolchain::Triple::UnknownEnvironment);
    Target.setEnvironmentName("");
    Target.setOS(toolchain::Triple::ELFIAMCU);
    Target.setVendor(toolchain::Triple::UnknownVendor);
  ...
  if (Target.isMIPS()) {
    if ((A = Args.getLastArg(options::OPT_mabi_EQ))) {
      ...
      if (ABIName == "32") {
        ...
        if (Target.getEnvironment() == toolchain::Triple::GNUABI64 ||
            Target.getEnvironment() == toolchain::Triple::GNUABIN32)
          Target.setEnvironment(toolchain::Triple::GNU);
      } else if (ABIName == "n32") {
        Target = Target.get64BitArchVariant();
        if (Target.getEnvironment() == toolchain::Triple::GNU ||
            Target.getEnvironment() == toolchain::Triple::GNUT64 ||
            Target.getEnvironment() == toolchain::Triple::GNUABI64)
          Target.setEnvironment(toolchain::Triple::GNUABIN32);
        else if (Target.getEnvironment() == toolchain::Triple::Musl ||
                 Target.getEnvironment() == toolchain::Triple::MuslABI64)
          Target.setEnvironment(toolchain::Triple::MuslABIN32);
      } else if (ABIName == "64") {
        Target = Target.get64BitArchVariant();
        if (Target.getEnvironment() == toolchain::Triple::GNU ||
            Target.getEnvironment() == toolchain::Triple::GNUT64 ||
            Target.getEnvironment() == toolchain::Triple::GNUABIN32)
          Target.setEnvironment(toolchain::Triple::GNUABI64);
        else if (Target.getEnvironment() == toolchain::Triple::Musl ||
                 Target.getEnvironment() == toolchain::Triple::MuslABIN32)
          Target.setEnvironment(toolchain::Triple::MuslABI64);
      }
    }
  }
  ...
  if (Target.isRISCV()) {
    if (Args.hasArg(options::OPT_march_EQ) ||
      ...
      auto ISAInfo = toolchain::RISCVISAInfo::parseArchString(
          ArchName, /*EnableExperimentalExtensions=*/true);
      if (!toolchain::errorToBool(ISAInfo.takeError())) {
        unsigned XLen = (*ISAInfo)->getXLen();
        if (XLen == 32)
          Target.setArch(toolchain::Triple::riscv32);
        else if (XLen == 64)
          Target.setArch(toolchain::Triple::riscv64);
      }
    }
  }
...
// option occurs last.
static driver::LTOKind parseLTOMode(Driver &D, const toolchain::opt::ArgList &Args,
  ...
  driver::LTOKind LTOMode = toolchain::StringSwitch<LTOKind>(LTOName)
                                .Case("full", LTOK_Full)
                                .Case("thin", LTOK_Thin)
...
void Driver::setLTOMode(const toolchain::opt::ArgList &Args) {
  LTOMode =
...
Driver::OpenMPRuntimeKind Driver::getOpenMPRuntime(const ArgList &Args) const {
  ...
  auto RT = toolchain::StringSwitch<OpenMPRuntimeKind>(RuntimeName)
...
static toolchain::SmallVector<std::string>
getSystemOffloadArchs(Compilation &C, Action::OffloadKind Kind) {
  ...
  if (toolchain::ErrorOr<std::string> Executable =
          toolchain::sys::findProgramByName(Program, {C.getDriver().Dir})) {
    toolchain::SmallVector<StringRef> Args{*Executable};
    ...
    for (StringRef Arch : toolchain::split((*StdoutOrErr)->getBuffer(), "\n"))
      if (!Arch.empty())
...
static toolchain::DenseSet<toolchain::StringRef>
inferOffloadToolchains(Compilation &C, Action::OffloadKind Kind) {
  ...
  toolchain::DenseSet<toolchain::StringRef> Triples;
  for (toolchain::StringRef Arch : Archs) {
    OffloadArch ID = StringToOffloadArch(Arch);
    if (ID == OffloadArch::UNKNOWN)
      ID = StringToOffloadArch(
          getProcessorFromTargetID(toolchain::Triple("amdgcn-amd-amdhsa"), Arch));

    if (Kind == Action::OFK_HIP && !IsAMDOffloadArch(ID)) {
      C.getDriver().Diag(language::Core::diag::err_drv_offload_bad_gpu_arch)
          << "HIP" << Arch;
      return toolchain::DenseSet<toolchain::StringRef>();
    }
    ...
    if (Kind == Action::OFK_OpenMP &&
      ...
      C.getDriver().Diag(language::Core::diag::err_drv_failed_to_deduce_target_from_arch)
          << Arch;
      return toolchain::DenseSet<toolchain::StringRef>();
    }
...
void Driver::CreateOffloadingDeviceToolChains(Compilation &C,
  ...
  bool UseLLVMOffload = C.getInputArgs().hasArg(
      options::OPT_foffload_via_toolchain, options::OPT_fno_offload_via_toolchain, false);
  bool IsCuda =
      toolchain::any_of(Inputs,
                   [](std::pair<types::ID, const toolchain::opt::Arg *> &I) {
  ...
  toolchain::SmallSet<Action::OffloadKind, 4> Kinds;
  ...
  if (Kinds.size() > 1) {
    Diag(language::Core::diag::err_drv_mix_offload)
  ...
  std::multiset<toolchain::StringRef> Triples;
  ...
  for (StringRef Target : Triples) {
    ...
    if (Kinds.contains(Action::OFK_OpenMP)) {
      ...
      if (RuntimeKind != OMPRT_OMP && RuntimeKind != OMPRT_IOMP5) {
        Diag(language::Core::diag::err_drv_expecting_fopenmp_with_fopenmp_targets);
        return;
      }
    }
    ...
    if (Kinds.contains(Action::OFK_SYCL)) {
      for (auto ID :
        ...
        if (Arg *IncompatArg = C.getInputArgs().getLastArg(ID))
          Diag(language::Core::diag::err_drv_argument_not_allowed_with)
              << IncompatArg->getSpelling() << "-fsycl";
    }

    // Create a device toolchain for every specified kind and triple.
    for (Action::OffloadKind Kind : Kinds) {
      toolchain::Triple TT = Kind == Action::OFK_OpenMP
                            ? ToolChain::getOpenMPTriple(Target)
                            : toolchain::Triple(Target);
      if (TT.getArch() == toolchain::Triple::ArchType::UnknownArch) {
      ...
      if (!Inserted) {
        Diag(language::Core::diag::warn_drv_omp_offload_target_duplicate)
...
bool Driver::loadZOSCustomizationFile(toolchain::cl::ExpansionContext &ExpCtx) {
  ...
  // config file.
  if (!PathLIBEnv.empty()) {
    toolchain::sys::path::append(CustomizationFile, PathLIBEnv);
    if (toolchain::sys::fs::is_directory(PathLIBEnv))
      toolchain::sys::path::append(CustomizationFile, "/clang.cfg");
    if (toolchain::sys::fs::is_regular_file(CustomizationFile))
      return readConfigFile(CustomizationFile, ExpCtx);
    Diag(diag::err_drv_config_file_not_found) << CustomizationFile;
    return true;
  }

  SmallString<128> BaseDir(toolchain::sys::path::parent_path(Dir));
  toolchain::sys::path::append(CustomizationFile, BaseDir + "/etc/clang.cfg");
  if (toolchain::sys::fs::is_regular_file(CustomizationFile))
    return readConfigFile(CustomizationFile, ExpCtx);
...
bool Driver::readConfigFile(StringRef FileName,
                            toolchain::cl::ExpansionContext &ExpCtx) {
  ...
  if (toolchain::Error Err = ExpCtx.readConfigFile(FileName, NewCfgFileArgs)) {
  ...
  toolchain::SmallString<128> CfgFileName(FileName);
  toolchain::sys::path::native(CfgFileName);
...
bool Driver::loadConfigFiles() {
  toolchain::cl::ExpansionContext ExpCtx(Saver.getAllocator(),
                                    toolchain::cl::tokenizeConfigFile);
  ...
  if (CLOptions) {
    for (auto CfgFileName : CLOptions->getAllArgValues(options::OPT_config)) {
      ...
      // configuration file.
      if (toolchain::sys::path::has_parent_path(CfgFileName)) {
        CfgFilePath.assign(CfgFileName);
        if (toolchain::sys::path::is_relative(CfgFilePath)) {
...
static bool findTripleConfigFile(toolchain::cl::ExpansionContext &ExpCtx,
                                 SmallString<128> &ConfigFilePath,
                                 toolchain::Triple Triple, std::string Suffix) {
  ...
  if (OSVersion.getMajor() != 0) {
    Triple.setOSName(BaseOSName + toolchain::utostr(OSVersion.getMajor()));
    if (ExpCtx.findConfigFile(Triple.str() + Suffix, ConfigFilePath))
      return true;
  }
...
bool Driver::loadDefaultConfigFiles(toolchain::cl::ExpansionContext &ExpCtx) {
  ...
  if (!ClangNameParts.TargetPrefix.empty() &&
      computeTargetTriple(*this, "/invalid/", *CLOptions).str() ==
          "/invalid/") {
    toolchain::Triple PrefixTriple{ClangNameParts.TargetPrefix};
    if (PrefixTriple.getArch() == toolchain::Triple::UnknownArch ||
        PrefixTriple.isOSUnknown())
      Triple = PrefixTriple;
  }

  // Otherwise, use the real triple as used by the driver.
  toolchain::Triple RealTriple =
...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  toolchain::PrettyStackTraceString CrashInfo("Compilation construction");
  ...
  if (IsCLMode()) {
    ...
    toolchain::Triple T(TargetTriple);
    T.setOS(toolchain::Triple::Win32);
    T.setVendor(toolchain::Triple::PC);
    T.setEnvironment(toolchain::Triple::MSVC);
    T.setObjectFormat(toolchain::Triple::COFF);
    if (Args.hasArg(options::OPT__SLASH_arm64EC))
      T.setArch(toolchain::Triple::aarch64, toolchain::Triple::AArch64SubArch_arm64ec);
    TargetTriple = T.str();
  } else if (IsDXCMode()) {
    ...
    if (const Arg *A = Args.getLastArg(options::OPT_target_profile)) {
      ...
      if (Args.hasArg(options::OPT_spirv)) {
        const toolchain::StringMap<toolchain::Triple::SubArchType> ValidTargets = {
            {"vulkan1.2", toolchain::Triple::SPIRVSubArch_v15},
            {"vulkan1.3", toolchain::Triple::SPIRVSubArch_v16}};
        toolchain::Triple T(TargetTriple);
        ...
        if (TargetInfo != ValidTargets.end()) {
          ...
          T.setArch(toolchain::Triple::spirv, TargetInfo->getValue());
          TargetTriple = T.str();
        }
      }
    } else {
  ...
  if (std::optional<std::string> CompilerPathValue =
          toolchain::sys::Process::GetEnv("COMPILER_PATH")) {
    StringRef CompilerPath = *CompilerPathValue;
    while (!CompilerPath.empty()) {
      std::pair<StringRef, StringRef> Split =
          CompilerPath.split(toolchain::sys::EnvPathSeparator);
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_save_temps_EQ)) {
    SaveTemps = toolchain::StringSwitch<SaveTempsMode>(A->getValue())
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fembed_bitcode_EQ)) {
    ...
    unsigned Model = toolchain::StringSwitch<unsigned>(Name)
        .Case("off", EmbedNone)
  ...
  if (Arg *A = Args.getLastArg(options::OPT_MJ))
    toolchain::sys::fs::remove(A->getValue());
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fmodule_header_EQ,
    ...
    if (A->getOption().matches(options::OPT_fmodule_header))
      ...
      unsigned Kind = toolchain::StringSwitch<unsigned>(ArgName)
  ...
  std::unique_ptr<toolchain::opt::InputArgList> UArgs =
...

  {
    ...
    SmallVector<const char *> MLMacroDefinesChar(
        toolchain::map_range(MultilibMacroDefinesStr, [&UArgs](const auto &S) {
  ...
  toolchain::Triple Triple = TC.getTriple();
  if (!Triple.isWasm()) {
  ...
  if ((TC.getTriple().getArch() != toolchain::Triple::aarch64 ||
       TC.getTriple().getSubArch() != toolchain::Triple::AArch64SubArch_arm64ec) &&
      UArgs->hasArg(options::OPT__SLASH_arm64EC)) {
    getDiags().Report(language::Core::diag::warn_target_override_arm64ec)
        << TC.getTriple().str();
  }
  ...
  if (TC.getTriple().getOS() == toolchain::Triple::UnknownOS &&
      TC.getTriple().getVendor() == toolchain::Triple::UnknownVendor) {
...
static void printArgList(raw_ostream &OS, const toolchain::opt::ArgList &Args) {
  toolchain::opt::ArgStringList ASL;
  ...
  for (auto I = ASL.begin(), E = ASL.end(); I != E; ++I) {
    if (I != ASL.begin())
      OS << ' ';
    toolchain::sys::printArg(OS, *I, true);
  }
  OS << '\n';
}
...
bool Driver::getCrashDiagnosticFile(StringRef ReproCrashFilename,
  ...
  assert(toolchain::Triple(toolchain::sys::getProcessTriple()).isOSDarwin() &&
  ...
  for (fs::directory_iterator File(CrashDiagDir, EC), FileEnd;
    ...
    if (fs::status(File->path(), FileStatus))
      continue;
    toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> CrashFile =
        toolchain::MemoryBuffer::getFile(File->path());
    if (!CrashFile)
      continue;
...
void Driver::generateCompilationDiagnostics(
  ...
  if (Arg *A = C.getArgs().getLastArg(options::OPT_fcrash_diagnostics_EQ)) {
    Level = toolchain::StringSwitch<unsigned>(A->getValue())
                .Case("off", 0)
                .Case("compiler", 1)
                .Case("all", 2)
                .Default(1);
  ...
  if (IsLLD) {
    ...
    toolchain::opt::ArgStringList ArgList = NewLLDInvocation.getArguments();
    ...
    Diag(language::Core::diag::note_drv_command_failed_diag_msg) << BugReporMsg;
    Diag(language::Core::diag::note_drv_command_failed_diag_msg) << TmpName;
    Diag(language::Core::diag::note_drv_command_failed_diag_msg)
        << "\n\n********************";
    if (Report)
  ...
  for (const char *TempFile : TempFiles) {
    Diag(language::Core::diag::note_drv_command_failed_diag_msg) << TempFile;
    if (Report)
      Report->TemporaryFiles.push_back(TempFile);
    if (ReproCrashFilename.empty()) {
      ReproCrashFilename = TempFile;
      toolchain::sys::path::replace_extension(ReproCrashFilename, ".crash");
    }
    if (StringRef(TempFile).ends_with(".cache")) {
      // In some cases (modules) we'll dump extra data to help with reproducing
      // the crash into a directory next to the output.
      VFS = toolchain::sys::path::filename(TempFile);
      toolchain::sys::path::append(VFS, "vfs", "vfs.yaml");
    }
  }
  ...
  toolchain::SmallString<128> Script(CrashInfo.Filename);
  toolchain::sys::path::replace_extension(Script, "sh");
  std::error_code EC;
  toolchain::raw_fd_ostream ScriptOS(Script, EC, toolchain::sys::fs::CD_CreateNew,
  ...
  if (EC) {
    ...
    Diag(language::Core::diag::note_drv_command_failed_diag_msg) << Script;
  }

  // On darwin, provide information about the .crash diagnostic report.
  if (toolchain::Triple(toolchain::sys::getProcessTriple()).isOSDarwin()) {
    SmallString<128> CrashDiagDir;
    if (getCrashDiagnosticFile(ReproCrashFilename, CrashDiagDir)) {
      Diag(language::Core::diag::note_drv_command_failed_diag_msg)
          << ReproCrashFilename.str();
    } else { // Suggest a directory for the user to look for .crash files.
      toolchain::sys::path::append(CrashDiagDir, Name);
...
void Driver::setUpResponseFiles(Compilation &C, Command &Cmd) {
  ...
  if (Cmd.getResponseFileSupport().ResponseKind ==
      ...
      toolchain::sys::commandLineFitsWithinSystemLimits(Cmd.getExecutable(),
                                                   Cmd.getArguments()))
    return;
...
int Driver::ExecuteCompilation(
  ...
  for (const auto &CmdPair : FailingCommands) {
    ...
    // toolchain/lib/Support/*/Signals.inc will exit with a special return code
    ...
    if (!FailingCommand->getCreator().hasGoodDiagnostics() || CommandRes != 1) {
      ...
      if (CommandRes < 0)
        Diag(language::Core::diag::err_drv_command_signalled)
            << FailingTool.getShortName();
      else
        Diag(language::Core::diag::err_drv_command_failed)
...
void Driver::PrintHelp(bool ShowHidden) const {
  toolchain::opt::Visibility VisibilityMask = getOptionVisibilityMask();

  std::string Usage = toolchain::formatv("{0} [options] file...", Name).str();
  getOpts().printHelp(toolchain::outs(), Usage.c_str(), DriverTitle.c_str(),
...
void Driver::PrintVersion(const Compilation &C, raw_ostream &OS) const {
  ...
  if (!toolchain::cl::getCompilerBuildConfig().empty())
    toolchain::cl::printBuildConfig(OS);
...
void Driver::HandleAutocompletions(StringRef PassedFlags) const {
  ...
  toolchain::opt::Visibility VisibilityMask(options::ClangOption);

  // Make sure that Flang-only options don't pollute the Clang output
  // TODO: Make sure that Clang-only options don't pollute Flang output
  if (IsFlangMode())
    VisibilityMask = toolchain::opt::Visibility(options::FlangOption);
  ...
  // -Xclang.
  if (toolchain::is_contained(Flags, "-Xclang") || toolchain::is_contained(Flags, "-cc1"))
    VisibilityMask = toolchain::opt::Visibility(options::CC1Option);
  ...
  toolchain::sort(SuggestedCompletions, [](StringRef A, StringRef B) {
    if (int X = A.compare_insensitive(B))
      return X < 0;
    return A.compare(B) > 0;
  });

  toolchain::outs() << toolchain::join(SuggestedCompletions, "\n") << '\n';
}
...
bool Driver::HandleImmediateArgs(Compilation &C) {
  ...
  if (C.getArgs().hasArg(options::OPT_dumpmachine)) {
    toolchain::outs() << C.getDefaultToolChain().getTripleString() << '\n';
    return false;
  }
  ...
  if (C.getArgs().hasArg(options::OPT_dumpversion)) {
    ...
    toolchain::outs() << CLANG_VERSION_STRING << "\n";
    return false;
  }

  if (C.getArgs().hasArg(options::OPT__print_diagnostic_categories)) {
    PrintDiagnosticCategories(toolchain::outs());
    return false;
  }
  ...
  if (C.getArgs().hasArg(options::OPT_v)) {
    if (!SystemConfigDir.empty())
      toolchain::errs() << "System configuration file directory: "
                   << SystemConfigDir << "\n";
    if (!UserConfigDir.empty())
      toolchain::errs() << "User configuration file directory: "
                   << UserConfigDir << "\n";
  }
  ...
  if (C.getArgs().hasArg(options::OPT_v))
    TC.printVerboseInfo(toolchain::errs());

  if (C.getArgs().hasArg(options::OPT_print_resource_dir)) {
    toolchain::outs() << ResourceDir << '\n';
    return false;
  }

  if (C.getArgs().hasArg(options::OPT_print_search_dirs)) {
    toolchain::outs() << "programs: =";
    bool separator = false;
    // Print -B and COMPILER_PATH.
    for (const std::string &Path : PrefixDirs) {
      if (separator)
        toolchain::outs() << toolchain::sys::EnvPathSeparator;
      toolchain::outs() << Path;
      separator = true;
    }
    ...
    toolchain::outs() << "\n";
    toolchain::outs() << "libraries: =" << ResourceDir;
    ...
    for (const std::string &Path : TC.getFilePaths()) {
      ...
      if (Path[0] == '=')
        toolchain::outs() << sysroot << Path.substr(1);
      else
        toolchain::outs() << Path;
    }
    toolchain::outs() << "\n";
  ...
  if (C.getArgs().hasArg(options::OPT_print_std_module_manifest_path)) {
    toolchain::outs() << GetStdModuleManifestPath(C, C.getDefaultToolChain())
                 << '\n';
    return false;
  }
  ...
  if (C.getArgs().hasArg(options::OPT_print_runtime_dir)) {
    if (std::optional<std::string> RuntimePath = TC.getRuntimePath())
      toolchain::outs() << *RuntimePath << '\n';
    else
      toolchain::outs() << TC.getCompilerRTPath() << '\n';
    return false;
  }
  ...
  if (Arg *A = C.getArgs().getLastArg(options::OPT_print_file_name_EQ)) {
    toolchain::outs() << GetFilePath(A->getValue(), TC) << "\n";
    return false;
  }
  ...
  if (Arg *A = C.getArgs().getLastArg(options::OPT_print_prog_name_EQ)) {
    ...
    if (! ProgName.empty())
      toolchain::outs() << GetProgramPath(ProgName, TC);
  ...
  if (C.getArgs().hasArg(options::OPT_print_libgcc_file_name)) {
    ...
    const toolchain::Triple Triple(TC.ComputeEffectiveClangTriple(C.getArgs()));
    ...
    case ToolChain::RLT_CompilerRT:
      toolchain::outs() << TC.getCompilerRT(C.getArgs(), "builtins") << "\n";
      break;
    case ToolChain::RLT_Libgcc:
      toolchain::outs() << GetFilePath("libgcc.a", TC) << "\n";
      break;
    }
    return false;
  }
  ...
  if (C.getArgs().hasArg(options::OPT_print_multi_lib)) {
    for (const Multilib &Multilib : TC.getMultilibs())
      if (!Multilib.isError())
        toolchain::outs() << Multilib << "\n";
    return false;
  }

  if (C.getArgs().hasArg(options::OPT_print_multi_flags)) {
    Multilib::flags_list ArgFlags = TC.getMultilibFlags(C.getArgs());
    toolchain::StringSet<> ExpandedFlags = TC.getMultilibs().expandFlags(ArgFlags);
    std::set<toolchain::StringRef> SortedFlags;
  ...
  if (C.getArgs().hasArg(options::OPT_print_multi_directory)) {
    for (const Multilib &Multilib : TC.getSelectedMultilibs()) {
      if (Multilib.gccSuffix().empty())
        ...
        assert(Suffix.front() == '/');
        toolchain::outs() << Suffix.substr(1) << "\n";
      }
    }
    return false;
  }
  ...
  if (C.getArgs().hasArg(options::OPT_print_target_triple)) {
    toolchain::outs() << TC.getTripleString() << "\n";
    return false;
  }

  if (C.getArgs().hasArg(options::OPT_print_effective_triple)) {
    const toolchain::Triple Triple(TC.ComputeEffectiveClangTriple(C.getArgs()));
    toolchain::outs() << Triple.getTriple() << "\n";
    return false;
  }

  if (C.getArgs().hasArg(options::OPT_print_targets)) {
    toolchain::TargetRegistry::printRegisteredTargetsForVersion(toolchain::outs());
    return false;
  }
...
static unsigned PrintActions1(const Compilation &C, Action *A,
  ...
  toolchain::raw_string_ostream offload_os(offload_str);
  ...
  Ids[A] = Id;
  toolchain::errs() << Indent + getSelfIndent(Kind) << Id << ": " << os.str() << ", "
...
static bool ContainsCompileOrAssembleAction(const Action *A) {
  ...
  return toolchain::any_of(A->inputs(), ContainsCompileOrAssembleAction);
}
...
void Driver::BuildUniversalActions(Compilation &C, const ToolChain &TC,
  ...
  toolchain::PrettyStackTraceString CrashInfo("Building universal build actions");
  ...
  for (Arg *A : Args) {
    if (A->getOption().matches(options::OPT_arch)) {
      ...
      if (Arch == toolchain::Triple::UnknownArch) {
        Diag(language::Core::diag::err_drv_invalid_arch_name) << A->getAsString(Args);
        continue;
      }
  ...
  for (Action* Act : SingleActions) {
    ...
    if (Archs.size() > 1 && !types::canLipoType(Act->getType()))
      Diag(language::Core::diag::err_drv_invalid_output_with_multiple_archs)
...
bool Driver::DiagnoseInputExistence(const DerivedArgList &Args, StringRef Value,
  ...
  if (TypoCorrect) {
    ...
    if (getOpts().findNearest(Value, Nearest, getOptionVisibilityMask()) <= 1) {
      Diag(language::Core::diag::err_drv_no_such_file_with_suggestion)
          << Value << Nearest;
      return false;
    }
  }
  ...
  Diag(language::Core::diag::err_drv_no_such_file) << Value;
  return false;
}
...
static types::ID CXXHeaderUnitType(ModuleHeaderMode HM) {
  ...
  case HeaderMode_None:
    toolchain_unreachable("should not be called in this case");
  }
...
void Driver::BuildInputs(const ToolChain &TC, DerivedArgList &Args,
  ...
  if (Arg *TCTP = Args.getLastArgNoClaim(options::OPT__SLASH_TC,
    ...
    for (Arg *A :
      ...
      if (Previous) {
        Diag(language::Core::diag::warn_drv_overriding_option)
    ...
    if (ShowNote)
      Diag(language::Core::diag::note_drv_t_option_is_global);
  }
  ...
  // Warn -x after last input file has no effect
  {
    ...
    if (LastXArg && LastInputArg &&
      ...
      Diag(language::Core::diag::warn_drv_unused_x) << LastXArg->getValue();
  }
  ...
  for (Arg *A : Args) {
    if (A->getOption().getKind() == Option::InputClass) {
        ...
        if (memcmp(Value, "-", 2) == 0) {
          if (IsFlangMode()) {
            ...
            if (!Args.hasArgNoClaim(options::OPT_E) && !CCCIsCPP())
              Diag(IsCLMode() ? language::Core::diag::err_drv_unknown_stdin_type_clang_cl
                              : language::Core::diag::err_drv_unknown_stdin_type);
            Ty = types::TY_C;
          }
        } else {
          ...
          if (CCCIsCXX()) {
            ...
            if (Ty != OldTy && !(OldTy == types::TY_CHeader && hasHeaderMode()))
              Diag(language::Core::diag::warn_drv_treating_input_as_cxx)
      ...
      if (!InputType) {
        Diag(language::Core::diag::err_drv_unknown_language) << A->getValue();
...
class OffloadingActionBuilder final {
  ...
  public:
    typedef const toolchain::SmallVectorImpl<phases::ID> PhasesTy;
  ...
  public:
    ...
    ActionBuilderReturnCode addDeviceDependences(Action *HostAction) override {
      ...
      if (auto *UA = dyn_cast<OffloadUnbundlingJobAction>(HostAction)) {
        ...
        if (IA->getType() == types::TY_Object &&
            (!toolchain::sys::path::has_extension(FileName) ||
             types::lookupTypeForExtension(
                 toolchain::sys::path::extension(FileName).drop_front()) !=
                 types::TY_Object ||
             toolchain::sys::path::extension(FileName) == LibFileExt))
          return ABRT_Inactive;
    ...
    virtual std::optional<std::pair<toolchain::StringRef, toolchain::StringRef>>
    ...
    bool initialize() override {
      ...
      for (Action::OffloadKind Kind : {Action::OFK_Cuda, Action::OFK_HIP}) {
        for (auto &I : toolchain::make_range(C.getOffloadToolChains(Kind))) {
      ...
      EmitLLVM = Args.getLastArg(options::OPT_emit_toolchain);
  ...
  public:
    ...
    void appendLinkDeviceActions(ActionList &AL) override {
      ...
      for (auto &LI : DeviceLinkerInputs) {
        ...
        types::ID Output = Args.hasArg(options::OPT_emit_toolchain)
                                   ? types::TY_LLVM_BC
                                   : types::TY_Image;
      ...
      if (Args.hasArg(options::OPT_emit_toolchain)) {
          AL.append(Actions);
          return;
      }
...
public:
  ...
  bool addHostDependenceToDeviceActions(Action *&HostAction,
    ...
    if (CanUseBundler && isa<InputAction>(HostAction) &&
        InputArg->getOption().getKind() == toolchain::opt::Option::InputClass &&
...
void Driver::handleArguments(Compilation &C, DerivedArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT__SLASH_Fo)) {
    ...
    if (Inputs.size() > 1 && !V.empty() &&
        !toolchain::sys::path::is_separator(V.back())) {
      // Check whether /Fo tries to name an output file for multiple inputs.
      Diag(language::Core::diag::err_drv_out_file_argument_with_multiple_sources)
          << A->getSpelling() << V;
  ...
  if (Arg *A = Args.getLastArg(options::OPT__SLASH_o)) {
    if (A->getValue()[0] == '\0') {
      ...
      Diag(language::Core::diag::err_drv_missing_argument) << A->getSpelling() << 1;
  ...
  if (YcArg && YuArg && strcmp(YcArg->getValue(), YuArg->getValue()) != 0) {
    Diag(language::Core::diag::warn_drv_ycyu_different_arg_clang_cl);
    Args.eraseArg(options::OPT__SLASH_Yc);
    Args.eraseArg(options::OPT__SLASH_Yu);
    YcArg = YuArg = nullptr;
  }
  if (YcArg && Inputs.size() > 1) {
    Diag(language::Core::diag::warn_drv_yc_multiple_inputs_clang_cl);
    Args.eraseArg(options::OPT__SLASH_Yc);
    YcArg = nullptr;
  }
  ...
  if (FinalPhase == phases::Link) {
    ...
    if (Args.hasArg(options::OPT_emit_toolchain) && !Args.hasArg(options::OPT_hip_link))
      Diag(language::Core::diag::err_drv_emit_toolchain_link);
    if (C.getDefaultToolChain().getTriple().isWindowsMSVCEnvironment() &&
        LTOMode != LTOK_None &&
        !Args.getLastArgValue(options::OPT_fuse_ld_EQ)
             .starts_with_insensitive("lld"))
      Diag(language::Core::diag::err_drv_lto_without_lld);
      ...
      Arg *Arg = Args.MakeSeparateArg(
          ...
          Args.MakeArgString(
              (FinalOutput ? FinalOutput->getValue()
                           : toolchain::sys::path::stem(getDefaultImageName())) +
              "-"));
      Arg->claim();
      Args.append(Arg);
    }
  ...
  for (auto &I : Inputs) {
    ...
    if (InitialPhase > FinalPhase) {
      ...
      if (CCCIsCPP())
        Diag(language::Core::diag::warn_drv_input_file_unused_by_cpp)
      ...
      // more sense.
        ...
        Diag(language::Core::diag::warn_drv_preprocessed_input_file_unused)
            << InputArg->getAsString(Args) << !!FinalPhaseArg
            << (FinalPhaseArg ? FinalPhaseArg->getOption().getName() : "");
      else
        Diag(language::Core::diag::warn_drv_input_file_unused)
            << InputArg->getAsString(Args) << getPhaseName(InitialPhase)
            << !!FinalPhaseArg
            << (FinalPhaseArg ? FinalPhaseArg->getOption().getName() : "");
      continue;
...
void Driver::BuildActions(Compilation &C, DerivedArgList &Args,
  ...
  toolchain::PrettyStackTraceString CrashInfo("Building compilation actions");

  if (!SuppressMissingInputWarning && Inputs.empty()) {
    Diag(language::Core::diag::err_drv_no_input_files);
    return;
  }
  ...
  bool UseNewOffloadingDriver =
      ...
      Args.hasFlag(options::OPT_foffload_via_toolchain,
                   options::OPT_fno_offload_via_toolchain, false) ||
    ...
    for (phases::ID Phase : PL) {
      ...
      if (Phase == phases::Link) {
        ...
        if (!(C.getInputArgs().hasArg(options::OPT_hip_link) &&
              (C.getInputArgs().hasArg(options::OPT_emit_toolchain))) &&
            !offloadDeviceOnly())
  ...
  if (Args.hasArg(options::OPT_emit_interface_stubs)) {
    ...
    for (auto &I : Inputs) {
      ...
      // Currently clang and the toolchain assembler do not support generating symbol
      ...
      for (auto Phase : PhaseList) {
        ...
        case phases::Compile: {
          // Only IfsMerge (toolchain-ifs) can handle .o files by looking for ifs
...
static StringRef getCanonicalArchString(Compilation &C,
  ...
  if (IsAMDOffloadArch(Arch)) {
    ...
    if (!Arch) {
      C.getDriver().Diag(language::Core::diag::err_drv_bad_target_id) << ArchStr;
      return StringRef();
    }
...
static std::optional<std::pair<toolchain::StringRef, toolchain::StringRef>>
getConflictOffloadArchCombination(const toolchain::DenseSet<StringRef> &Archs,
                                  toolchain::Triple Triple) {
  if (!Triple.isAMDGPU())
    return std::nullopt;

  std::set<StringRef> ArchSet;
  toolchain::copy(Archs, std::inserter(ArchSet, ArchSet.begin()));
  return getConflictTargetIDCombination(ArchSet);
}

toolchain::SmallVector<StringRef>
Driver::getOffloadArchs(Compilation &C, const toolchain::opt::DerivedArgList &Args,
  ...
  toolchain::DenseSet<StringRef> Archs;
        ...
        if (Arch == "native" || Arch.empty()) {
          ...
          if (!GPUsOrErr) {
            TC.getDriver().Diag(diag::err_drv_undetermined_gpu_arch)
                << toolchain::Triple::getArchTypeName(TC.getArch())
                << toolchain::toString(GPUsOrErr.takeError()) << "--offload-arch";
            continue;
          }
          ...
          for (auto ArchStr : *GPUsOrErr) {
            ...
            if (!CanonicalStr.empty())
              ...
              return toolchain::SmallVector<StringRef>();
          }
        } else {
  ...
  if (auto ConflictingArchs =
    ...
    C.getDriver().Diag(language::Core::diag::err_drv_bad_offload_arch_combo)
    ...
    if (Kind == Action::OFK_Cuda) {
      ...
      if (auto *Arg = C.getArgsForToolChain(&TC, /*BoundArch=*/"", Kind)
        ...
        if (!ArchsOrErr) {
          TC.getDriver().Diag(diag::err_drv_undetermined_gpu_arch)
              << toolchain::Triple::getArchTypeName(TC.getArch())
              << toolchain::toString(ArchsOrErr.takeError()) << "--offload-arch";
...
Action *Driver::BuildOffloadingActions(Compilation &C,
                                       toolchain::opt::DerivedArgList &Args,
    ...
    for (Action *&A : DeviceActions) {
      ...
      bool IsAMDGCNSPIRV = A->getOffloadingToolChain() &&
                           A->getOffloadingToolChain()->getTriple().getOS() ==
                               toolchain::Triple::OSType::AMDHSA &&
  ...
  bool ShouldBundleHIP =
      ...
      (HIPNoRDC && offloadDeviceOnly() &&
       toolchain::none_of(OffloadActions, [](Action *A) {
  ...
  bool SingleDeviceOutput = !toolchain::any_of(OffloadActions, [](Action *A) {
...
Action *Driver::ConstructPhaseAction(
  ...
  toolchain::PrettyStackTraceString CrashInfo("Constructing phase actions");

  // Some types skip the assembler phase (e.g., toolchain-bc), but we can't
  ...
  switch (Phase) {
  case phases::Link:
    toolchain_unreachable("link action invalid here.");
  case phases::IfsMerge:
    toolchain_unreachable("ifsmerge action invalid here.");
  ...
  case phases::Backend: {
    if (isUsingLTO() && TargetDeviceOffloadKind == Action::OFK_None) {
      ...
      if (Args.hasArg(options::OPT_ffat_lto_objects) &&
          !Args.hasArg(options::OPT_emit_toolchain))
  ...
  toolchain_unreachable("invalid phase in ConstructPhaseAction");
}

void Driver::BuildJobs(Compilation &C) const {
  toolchain::PrettyStackTraceString CrashInfo("Building compilation jobs");
  ...
  if (FinalOutput) {
    ...
    for (const Action *A : C.getActions()) {
      ...
      if (A->getType() == types::TY_DX_CONTAINER &&
          (A->getKind() == language::Core::driver::Action::BinaryAnalyzeJobClass ||
           A->getKind() == language::Core::driver::Action::BinaryTranslatorJobClass))
        continue;

      if (A->getType() != types::TY_Nothing &&
          !(A->getKind() == Action::IfsMergeJobClass ||
            (A->getType() == language::Core::driver::types::TY_IFS_CPP &&
             A->getKind() == language::Core::driver::Action::CompileJobClass &&
             0 == NumIfsOutputs++) ||
    ...
    if (NumOutputs > 1) {
      Diag(language::Core::diag::err_drv_output_argument_with_multiple_files);
      FinalOutput = nullptr;
    }
  }

  const toolchain::Triple &RawTriple = C.getDefaultToolChain().getTriple();
  ...
  if (CCPrintProcessStats) {
    C.setPostCallback([=](const Command &Cmd, int Res) {
      std::optional<toolchain::sys::ProcessStatistics> ProcStat =
      ...
      if (CCPrintStatReportFilename.empty()) {
        ...
        // CSV format.
        std::string Buffer;
        toolchain::raw_string_ostream Out(Buffer);
        toolchain::sys::printArg(Out, toolchain::sys::path::filename(Cmd.getExecutable()),
                            /*Quote*/ true);
        Out << ',';
        toolchain::sys::printArg(Out, LinkingOutput, true);
        Out << ',' << ProcStat->TotalTime.count() << ','
            << ProcStat->UserTime.count() << ',' << ProcStat->PeakMemory
            << '\n';
        Out.flush();
        std::error_code EC;
        toolchain::raw_fd_ostream OS(CCPrintStatReportFilename, EC,
                                toolchain::sys::fs::OF_Append |
                                    toolchain::sys::fs::OF_Text);
        if (EC)
          return;
        auto L = OS.lock();
        if (!L) {
          toolchain::errs() << "ERROR: Cannot lock file "
  ...
  bool HasAssembleJob = toolchain::any_of(C.getJobs(), [](auto &J) {
    ...
    if (!A->isClaimed()) {
      ...
      if (!IsCLMode() || !A->getOption().matches(options::OPT_UNKNOWN)) {
        if (A->getOption().hasFlag(options::TargetSpecific) &&
          ...
          Diag(language::Core::diag::warn_drv_unused_argument)
...
static void handleTimeTrace(Compilation &C, const ArgList &Args,
  ...
  if (A->getOption().matches(options::OPT_ftime_trace_EQ)) {
    ...
    if (toolchain::sys::fs::is_directory(Path)) {
      ...
      toolchain::sys::path::replace_extension(Tmp, "json");
      toolchain::sys::path::append(Path, toolchain::sys::path::filename(Tmp));
    }
  } else {
    if (Arg *DumpDir = Args.getLastArgNoClaim(options::OPT_dumpdir)) {
      ...
      Path += toolchain::sys::path::filename(BaseInput);
    } else {
      Path = Result.getFilename();
    }
    toolchain::sys::path::replace_extension(Path, "json");
  }
...
InputInfoList Driver::BuildJobsForActionNoCache(
  ...
  toolchain::Triple EffectiveTriple;
  const ToolChain &ToolTC = T->getToolChain();
  const ArgList &Args =
      C.getArgsForToolChain(TC, BoundArch, A->getOffloadingDeviceKind());
  if (InputInfos.size() != 1) {
    EffectiveTriple = toolchain::Triple(ToolTC.ComputeEffectiveClangTriple(Args));
  } else {
    // Pass along the input type if it can be unambiguously determined.
    EffectiveTriple = toolchain::Triple(
  ...
  if (CCCPrintBindings && !CCGenDiagnostics) {
    toolchain::errs() << "# \"" << T->getToolChain().getTripleString() << '"'
                 << " - \"" << T->getName() << "\", inputs: [";
    for (unsigned i = 0, e = InputInfos.size(); i != e; ++i) {
      toolchain::errs() << InputInfos[i].getAsString();
      if (i + 1 != e)
        toolchain::errs() << ", ";
    }
    if (UnbundlingResults.empty())
      toolchain::errs() << "], output: " << Result.getAsString() << "\n";
    else {
      toolchain::errs() << "], outputs: [";
      for (unsigned i = 0, e = UnbundlingResults.size(); i != e; ++i) {
        toolchain::errs() << UnbundlingResults[i].getAsString();
        if (i + 1 != e)
          toolchain::errs() << ", ";
      }
      toolchain::errs() << "] \n";
...
static const char *MakeCLOutputFilename(const ArgList &Args, StringRef ArgValue,
  ...
  if (ArgValue.empty()) {
    ...
    Filename = BaseName;
  } else if (toolchain::sys::path::is_separator(Filename.back())) {
    // If the argument is a directory, output to BaseName in that dir.
    toolchain::sys::path::append(Filename, BaseName);
  }

  if (!toolchain::sys::path::has_extension(ArgValue)) {
    ...
    toolchain::sys::path::replace_extension(Filename, Extension);
  }
...
const char *Driver::CreateTempFile(Compilation &C, StringRef Prefix,
  ...
  std::optional<std::string> CrashDirectory =
      CCGenDiagnostics && A
          ...
          : toolchain::sys::Process::GetEnv("CLANG_CRASH_DIAGNOSTICS_DIR");
  if (CrashDirectory) {
    if (!getVFS().exists(*CrashDirectory))
      toolchain::sys::fs::create_directories(*CrashDirectory);
    SmallString<128> Path(*CrashDirectory);
    toolchain::sys::path::append(Path, Prefix);
    const char *Middle = !Suffix.empty() ? "-%%%%%%." : "-%%%%%%";
    if (std::error_code EC =
            toolchain::sys::fs::createUniqueFile(Path + Middle + Suffix, TmpName)) {
      Diag(language::Core::diag::err_unable_to_make_temp) << EC.message();
      return "";
    }
  } else {
    if (MultipleArchs && !BoundArch.empty()) {
      if (NeedUniqueDirectory) {
        ...
        toolchain::sys::path::append(TmpName,
...
const char *Driver::GetNamedOutputPath(Compilation &C, const JobAction &JA,
  ...
  if (is_style_windows(toolchain::sys::path::Style::native)) {
    ...
    toolchain::replace(BoundArch, ':', '@');
  }

  toolchain::PrettyStackTraceString CrashInfo("Computing output path");
  ...
  if (C.getArgs().hasArg(options::OPT__SLASH_P)) {
    ...
    StringRef BaseName = toolchain::sys::path::filename(BaseInput);
    StringRef NameArg;
  ...
  if ((JA.getType() == types::TY_Object &&
    ...
    if (C.getDefaultToolChain().getTriple().isDXIL()) {
      ...
      StringRef Name = toolchain::sys::path::filename(BaseInput);
  ...
  if (JA.getType() == types::TY_API_INFO &&
    ...
    Diag(language::Core::diag::err_drv_unexpected_symbol_graph_output)
  ...
  if (MultipleArchs && SpecifiedModuleOutput)
    Diag(language::Core::diag::err_drv_module_output_with_multiple_arch);
  ...
  if (isa<DsymutilJobAction>(JA) && C.getArgs().hasArg(options::OPT_dsym_dir)) {
    ...
    // fails.
    toolchain::sys::path::append(ExternalPath, toolchain::sys::path::Style::posix,
                            toolchain::sys::path::filename(BasePath));
    BaseName = ExternalPath;
  } else if (isa<DsymutilJobAction>(JA) || isa<VerifyJobAction>(JA))
    BaseName = BasePath;
  else
    BaseName = toolchain::sys::path::filename(BasePath);
  ...
  if ((JA.getType() == types::TY_Object || JA.getType() == types::TY_LTO_BC) &&
    ...
    if (IsCLMode()) {
      ...
      if (UseOutExtension) {
        Output = BaseName;
        toolchain::sys::path::replace_extension(Output, "");
      }
    ...
    // When using both -save-temps and -emit-toolchain, use a ".tmp.bc" suffix for
    // the unoptimized bitcode so that it does not get overwritten by the ".bc"
    // optimized bitcode output.
    auto IsAMDRDCInCompilePhase = [](const JobAction &JA,
                                     const toolchain::opt::DerivedArgList &Args) {
      // The relocatable compilation in HIP and OpenMP implies -emit-toolchain.
    ...
    if (!AtTopLevel && JA.getType() == types::TY_LLVM_BC &&
        (C.getArgs().hasArg(options::OPT_emit_toolchain) ||
  ...
  if (!AtTopLevel && isSaveTempsObj() && C.getArgs().hasArg(options::OPT_o) &&
    ...
    toolchain::sys::path::remove_filename(TempPath);
    StringRef OutputFileName = toolchain::sys::path::filename(NamedOutput);
    toolchain::sys::path::append(TempPath, OutputFileName);
  ...
  if (!AtTopLevel && isSaveTempsEnabled() && NamedOutput == BaseName) {
    ...
    toolchain::sys::fs::current_path(Result);
    toolchain::sys::path::append(Result, BaseName);
    toolchain::sys::fs::equivalent(BaseInput, Result.c_str(), SameFile);
  ...
  if (JA.getType() == types::TY_PCH && !IsCLMode()) {
    toolchain::sys::path::remove_filename(BasePath);
    if (BasePath.empty())
      BasePath = NamedOutput;
    else
      toolchain::sys::path::append(BasePath, NamedOutput);
...
std::string Driver::GetFilePath(StringRef Name, const ToolChain &TC) const {
  ...
  auto SearchPaths = [&](const toolchain::SmallVectorImpl<std::string> &P)
    ...
    for (const auto &Dir : P) {
      ...
      toolchain::sys::path::append(P, Name);
      if (toolchain::sys::fs::exists(Twine(P)))
        return std::string(P);
    }
    return std::nullopt;
  };
  ...
  toolchain::sys::path::append(R, Name);
  if (toolchain::sys::fs::exists(Twine(R)))
    return std::string(R);
  ...
  toolchain::sys::path::append(D, "..", Name);
  if (toolchain::sys::fs::exists(Twine(D)))
    return std::string(D);
  ...
  toolchain::sys::path::append(R2, "..", "..", Name);
  if (toolchain::sys::fs::exists(Twine(R2)))
...
static bool ScanDirForExecutable(SmallString<128> &Dir, StringRef Name) {
  toolchain::sys::path::append(Dir, Name);
  if (toolchain::sys::fs::can_execute(Twine(Dir)))
    return true;
...
std::string Driver::GetProgramPath(StringRef Name, const ToolChain &TC) const {
  ...
  for (const auto &PrefixDir : PrefixDirs) {
    if (toolchain::sys::fs::is_directory(PrefixDir)) {
      SmallString<128> P(PrefixDir);
      if (ScanDirForExecutable(P, Name))
        return std::string(P);
    } else {
      SmallString<128> P((PrefixDir + Name).str());
      if (toolchain::sys::fs::can_execute(Twine(P)))
        return std::string(P);
    }
  }
  ...
  for (const auto &TargetSpecificExecutable : TargetSpecificExecutables) {
    ...
    if (toolchain::ErrorOr<std::string> P =
            toolchain::sys::findProgramByName(TargetSpecificExecutable))
      return *P;
  }
...
std::string Driver::GetStdModuleManifestPath(const Compilation &C,
  ...
  case ToolChain::CST_Libcxx: {
    auto evaluate = [&](const char *library) -> std::optional<std::string> {
      ...
      toolchain::sys::path::append(path, "libc++.modules.json");
  ...
  case ToolChain::CST_Libstdcxx: {
    auto evaluate = [&](const char *library) -> std::optional<std::string> {
      ...
      SmallString<128> path(lib.begin(), lib.end());
      toolchain::sys::path::remove_filename(path);
      toolchain::sys::path::append(path, "libstdc++.modules.json");
      if (TC.getVFS().exists(path))
        return static_cast<std::string>(path);
...
std::string Driver::GetTemporaryPath(StringRef Prefix, StringRef Suffix) const {
  SmallString<128> Path;
  std::error_code EC = toolchain::sys::fs::createTemporaryFile(Prefix, Suffix, Path);
  if (EC) {
    Diag(language::Core::diag::err_unable_to_make_temp) << EC.message();
    return "";
  }
...
std::string Driver::GetTemporaryDirectory(StringRef Prefix) const {
  SmallString<128> Path;
  std::error_code EC = toolchain::sys::fs::createUniqueDirectory(Prefix, Path);
  if (EC) {
    Diag(language::Core::diag::err_unable_to_make_temp) << EC.message();
    return "";
  }
...
std::string Driver::GetClPchPath(Compilation &C, StringRef BaseName) const {
  ...
  if (Arg *FpArg = C.getArgs().getLastArg(options::OPT__SLASH_Fp)) {
    ...
    if (!toolchain::sys::path::has_extension(Output))
      Output += ".pch";
  } else {
    ...
    if (Output.empty())
      Output = BaseName;
    toolchain::sys::path::replace_extension(Output, ".pch");
  }
  return std::string(Output);
}

const ToolChain &Driver::getOffloadToolChain(
    const toolchain::opt::ArgList &Args, const Action::OffloadKind Kind,
    const toolchain::Triple &Target, const toolchain::Triple &AuxTarget) const {
...
const ToolChain &Driver::getToolChain(const ArgList &Args,
                                      const toolchain::Triple &Target) const {
  ...
  if (!TC) {
    ...
    case toolchain::Triple::ELFIAMCU:
      if (Target.getArch() == toolchain::Triple::hexagon)
        TC = std::make_unique<toolchains::HexagonToolChain>(*this, Target,
                                                             Args);
      else if ((Target.getVendor() == toolchain::Triple::MipsTechnologies) &&
        ...
        TC = std::make_unique<toolchains::PPCLinuxToolChain>(*this, Target,
                                                              Args);
      else if (Target.getArch() == toolchain::Triple::ve)
    ...
    case toolchain::Triple::CUDA:
      ...
      break;
    case toolchain::Triple::AMDHSA: {
      if (Target.getArch() == toolchain::Triple::spirv64) {
...
toolchain::opt::Visibility
Driver::getOptionVisibilityMask(bool UseDriverMode) const {
  if (!UseDriverMode)
    return toolchain::opt::Visibility(options::ClangOption);
  if (IsCLMode())
    return toolchain::opt::Visibility(options::CLOption);
  if (IsDXCMode())
    return toolchain::opt::Visibility(options::DXCOption);
  if (IsFlangMode())  {
    return toolchain::opt::Visibility(options::FlangOption);
  }
  return toolchain::opt::Visibility(options::ClangOption);
}
...
const char *Driver::getExecutableForDriverMode(DriverMode Mode) {
  ...
  toolchain_unreachable("Unhandled Mode");
}

bool language::Core::driver::isOptimizationLevelFast(const ArgList &Args) {
  return Args.hasFlag(options::OPT_Ofast, options::OPT_O_Group, false);
}

bool language::Core::driver::willEmitRemarks(const ArgList &Args) {
...
toolchain::StringRef language::Core::driver::getDriverMode(StringRef ProgName,
...
toolchain::Error driver::expandResponseFiles(SmallVectorImpl<const char *> &Args,
                                        bool ClangCLMode,
  ...
  if (RSPQuoting == Windows || (RSPQuoting == Default && ClangCLMode))
    Tokenizer = &toolchain::cl::TokenizeWindowsCommandLine;
  else
    Tokenizer = &toolchain::cl::TokenizeGNUCommandLine;
  ...
  if (toolchain::Error Err = ECtx.expandResponseFiles(Args))
    return Err;

  // If -cc1 came from a response file, remove the EOL sentinels.
  auto FirstArg = toolchain::find_if(toolchain::drop_begin(Args),
...
static const char *GetStableCStr(toolchain::StringSet<> &SavedStrings, StringRef S) {
...
static void applyOneOverrideOption(raw_ostream &OS,
                                   ...
                                   StringRef Edit,
                                   toolchain::StringSet<> &SavedStrings) {
  ...
  if (Edit[0] == '^') {
    ...
    for (unsigned i = 1, e = Args.size(); i != e; ++i) {
      ...
      if (Args[i] == nullptr)
        continue;
      std::string Repl = toolchain::Regex(MatchPattern).sub(ReplPattern, Args[i]);
