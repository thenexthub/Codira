...
static toolchain::opt::Arg *GetRTTIArgument(const ArgList &Args) {
...
ToolChain::ToolChain(const Driver &D, const toolchain::Triple &T,
                     const ArgList &Args)
...
void ToolChain::setTripleEnvironment(toolchain::Triple::EnvironmentType Env) {
  Triple.setEnvironment(Env);
  if (EffectiveTriple != toolchain::Triple())
    EffectiveTriple.setEnvironment(Env);
}

ToolChain::~ToolChain() = default;

toolchain::vfs::FileSystem &ToolChain::getVFS() const {
...
bool ToolChain::useIntegratedBackend() const {
  ...
  if ((IBackend && !IsIntegratedBackendSupported()) ||
    ...
    DiagID = language::Core::diag::err_drv_unsupported_opt_for_target;
  else
    DiagID = language::Core::diag::warn_drv_unsupported_opt_for_target;
...
static void getAArch64MultilibFlags(const Driver &D,
  ...
  toolchain::DenseSet<StringRef> FeatureSet(UnifiedFeatures.begin(),
                                       UnifiedFeatures.end());
  ...
  if (!ArchName.empty()) {
    ...
    Result.push_back(toolchain::join(MArch, "+"));
  }
...
static void getARMMultilibFlags(const Driver &D, const toolchain::Triple &Triple,
                                toolchain::Reloc::Model RelocationModel,
                                const toolchain::opt::ArgList &Args,
                                Multilib::flags_list &Result) {
  std::vector<StringRef> Features;
  toolchain::ARM::FPUKind FPUKind = tools::arm::getARMTargetFeatures(
...
#define ARM_FPU(NAME, KIND, VERSION, NEON_SUPPORT, RESTRICTION)                \
  case toolchain::ARM::KIND:                                                        \
    Result.push_back("-mfpu=" NAME);                                           \
    break;
#include "toolchain/TargetParser/ARMTargetParser.def"
  default:
    toolchain_unreachable("Invalid FPUKind");
  }
  ...
  case arm::FloatABI::Invalid:
    toolchain_unreachable("Invalid float ABI");
  }
...
static void getRISCVMultilibFlags(const Driver &D, const toolchain::Triple &Triple,
                                  const toolchain::opt::ArgList &Args,
                                  Multilib::flags_list &Result) {
...
Multilib::flags_list
ToolChain::getMultilibFlags(const toolchain::opt::ArgList &Args) const {
  using namespace language::Core::driver::options;

  std::vector<std::string> Result;
  const toolchain::Triple Triple(ComputeEffectiveClangTriple(Args));
  ...
  if (RelocationModel == toolchain::Reloc::PIC_)
  ...
  Result.erase(toolchain::unique(Result), Result.end());
  return Result;
}

SanitizerArgs
ToolChain::getSanitizerArgs(const toolchain::opt::ArgList &JobArgs) const {
  SanitizerArgs SanArgs(*this, JobArgs, !SanitizerArgsChecked);
  SanitizerArgsChecked = true;
  return SanArgs;
}

const XRayArgs ToolChain::getXRayArgs(const toolchain::opt::ArgList &JobArgs) const {
...
static std::string normalizeProgramName(toolchain::StringRef Argv0) {
  std::string ProgName = std::string(toolchain::sys::path::filename(Argv0));
...
ToolChain::getTargetAndModeFromProgramName(StringRef PN) {
  ...
  bool IsRegistered = toolchain::TargetRegistry::lookupTarget(Prefix, IgnoredError);
...
Tool *ToolChain::buildLinker() const {
  toolchain_unreachable("Linking is not supported by this toolchain");
}

Tool *ToolChain::buildStaticLibTool() const {
  toolchain_unreachable("Creating static lib is not supported by this toolchain");
}
...
Tool *ToolChain::getTool(Action::ActionClass AC) const {
  ...
  case Action::BinaryTranslatorJobClass:
    toolchain_unreachable("Invalid tool kind.");
...
static StringRef getArchNameForCompilerRTLib(const ToolChain &TC,
  ...
  if (TC.getArch() == toolchain::Triple::arm || TC.getArch() == toolchain::Triple::armeb)
    return (arm::getARMFloatABI(TC, Args) == arm::FloatABI::Hard && !IsWindows)
               ? "armhf"
               : "arm";

  // For historic reasons, Android library is using i686 instead of i386.
  if (TC.getArch() == toolchain::Triple::x86 && Triple.isAndroid())
    return "i686";

  if (TC.getArch() == toolchain::Triple::x86_64 && Triple.isX32())
    return "x32";

  return toolchain::Triple::getArchTypeName(TC.getArch());
}
...
std::string ToolChain::getCompilerRTPath() const {
  ...
  if (isBareMetal()) {
    toolchain::sys::path::append(Path, "lib", getOSLibName());
...
std::string ToolChain::getCompilerRTBasename(const ArgList &Args,
  ...
  return toolchain::sys::path::filename(CRTAbsolutePath).str();
}

std::string ToolChain::buildCompilerRTBasename(const toolchain::opt::ArgList &Args,
                                               StringRef Component,
                                               FileType Type, bool AddArch,
                                               bool IsFortran) const {
  const toolchain::Triple &TT = getTriple();
...
std::string ToolChain::getCompilerRT(const ArgList &Args, StringRef Component,
  ...
  for (const auto &LibPath : getLibraryPaths()) {
    ...
    toolchain::sys::path::append(P, CRTBasename);
    if (getVFS().exists(P))
      return std::string(P);
    if (Path.empty())
      Path = P;
  ...
  toolchain::sys::path::append(OldPath, CRTBasename);
...
const char *ToolChain::getCompilerRTArgString(const toolchain::opt::ArgList &Args,
                                              StringRef Component,
                                              FileType Type,
                                              bool isFortran) const {
...
void ToolChain::addFortranRuntimeLibraryPath(const toolchain::opt::ArgList &Args,
  ...
  SmallString<256> DefaultLibPath =
      toolchain::sys::path::parent_path(getDriver().Dir);
  toolchain::sys::path::append(DefaultLibPath, "lib");
...
ToolChain::getFallbackAndroidTargetPath(StringRef BaseDir) const {
  toolchain::Triple TripleWithoutLevel(getTriple());
  ...
  std::error_code EC;
  for (toolchain::vfs::directory_iterator LI = getVFS().dir_begin(BaseDir, EC), LE;
       !EC && LI != LE; LI = LI.increment(EC)) {
    StringRef DirName = toolchain::sys::path::filename(LI->path());
  ...
  toolchain::sys::path::append(P, TripleDir);
  if (UsingUnversionedDir)
    D.Diag(diag::warn_android_unversioned_fallback) << P << getTripleString();
  return std::string(P);
}

toolchain::Triple ToolChain::getTripleWithoutOSVersion() const {
  return (Triple.hasEnvironment()
              ? toolchain::Triple(Triple.getArchName(), Triple.getVendorName(),
                             toolchain::Triple::getOSTypeName(Triple.getOS()),
                             toolchain::Triple::getEnvironmentTypeName(
...
ToolChain::getTargetSubDirPath(StringRef BaseDir) const {
  auto getPathForTriple =
      [&](const toolchain::Triple &Triple) -> std::optional<std::string> {
    SmallString<128> P(BaseDir);
    toolchain::sys::path::append(P, Triple.str());
    if (getVFS().exists(P))
      return std::string(P);
    return {};
  };

  const toolchain::Triple &T = getTriple();
  if (auto Path = getPathForTriple(T))
    return *Path;

  if (T.isOSAIX()) {
    toolchain::Triple AIXTriple;
    if (T.getEnvironment() == Triple::UnknownEnvironment) {
      // Strip unknown environment and the OS version from the triple.
      AIXTriple = toolchain::Triple(T.getArchName(), T.getVendorName(),
                               toolchain::Triple::getOSTypeName(T.getOS()));
    } else {
  ...
  if (T.isOSzOS() &&
    ...
    const toolchain::Triple &TripleWithoutVersion = getTripleWithoutOSVersion();
...
std::optional<std::string> ToolChain::getRuntimePath() const {
  ...
  toolchain::sys::path::append(P, "lib");
...
ToolChain::path_list ToolChain::getArchSpecificLibPaths() const {
  ...
  auto AddPath = [&](const ArrayRef<StringRef> &SS) {
    ...
    toolchain::sys::path::append(Path, "lib");
    for (auto &S : SS)
      toolchain::sys::path::append(Path, S);
    Paths.push_back(std::string(Path));
  };

  AddPath({getTriple().str()});
  AddPath({getOSLibName(), toolchain::Triple::getArchTypeName(getArch())});
  return Paths;
}
...
bool ToolChain::needsGCovInstrumentation(const toolchain::opt::ArgList &Args) {
...
std::string ToolChain::GetLinkerPath(bool *LinkerIsLLD) const {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_ld_path_EQ)) {
    ...
    if (!Path.empty()) {
      if (toolchain::sys::path::parent_path(Path).empty())
  ...
  if (UseLinker.empty() || UseLinker == "ld") {
    ...
    if (toolchain::sys::path::is_absolute(DefaultLinker))
  ...
  if (toolchain::sys::path::is_absolute(UseLinker)) {
    // If we're passed what looks like an absolute path, don't attempt to
    // second-guess that.
    if (toolchain::sys::fs::can_execute(UseLinker))
      return std::string(UseLinker);
  } else {
    toolchain::SmallString<8> LinkerName;
    if (Triple.isOSDarwin())
    ...
    if (toolchain::sys::fs::can_execute(LinkerPath)) {
      if (LinkerIsLLD)
        *LinkerIsLLD = UseLinker == "lld";
      return LinkerPath;
    }
...
std::string ToolChain::GetStaticLibToolPath() const {
  ...
  return GetProgramPath("toolchain-ar");
}
...
bool ToolChain::isCrossCompiling() const {
  toolchain::Triple HostTriple(LLVM_HOST_TRIPLE);
  ...
  case toolchain::Triple::thumb:
  case toolchain::Triple::thumbeb:
    return getArch() != toolchain::Triple::arm && getArch() != toolchain::Triple::thumb &&
           getArch() != toolchain::Triple::armeb && getArch() != toolchain::Triple::thumbeb;
  default:
...
ToolChain::GetExceptionModel(const toolchain::opt::ArgList &Args) const {
...
bool ToolChain::isThreadModelSupported(const StringRef Model) const {
  if (Model == "single") {
    ...
    return Triple.getArch() == toolchain::Triple::arm ||
           Triple.getArch() == toolchain::Triple::armeb ||
           Triple.getArch() == toolchain::Triple::thumb ||
           Triple.getArch() == toolchain::Triple::thumbeb || Triple.isWasm();
  } else if (Model == "posix")
    return true;
...
std::string ToolChain::ComputeLLVMTriple(const ArgList &Args,
  ...
  case toolchain::Triple::amdgcn: {
    ...
    if (Args.getLastArgValue(options::OPT_mcpu_EQ) == "amdgcnspirv")
      Triple.setArch(toolchain::Triple::ArchType::spirv64);
    return Triple.getTriple();
  }
  case toolchain::Triple::arm:
  case toolchain::Triple::armeb:
...
void ToolChain::addProfileRTLibs(const toolchain::opt::ArgList &Args,
                                 toolchain::opt::ArgStringList &CmdArgs) const {
...
void ToolChain::addSystemFrameworkInclude(const toolchain::opt::ArgList &DriverArgs,
...
std::string ToolChain::concat(StringRef Path, const Twine &A, const Twine &B,
  ...
  toolchain::sys::path::append(Result, toolchain::sys::path::Style::posix, A, B, C, D);
...
std::string ToolChain::detectLibcxxVersion(StringRef IncludePath) const {
  ...
  toolchain::sys::path::append(Path, "c++");
  for (toolchain::vfs::directory_iterator LI = getVFS().dir_begin(Path, EC), LE;
       !EC && LI != LE; LI = LI.increment(EC)) {
...
bool ToolChain::ShouldLinkCXXStdlib(const toolchain::opt::ArgList &Args) const {
...
bool ToolChain::isFastMathRuntimeAvailable(const ArgList &Args,
  ...
  // https://github.com/toolchain/toolchain-project/issues/57589 for more information.
...
ToolChain::getSystemGPUArchs(const toolchain::opt::ArgList &Args) const {
...
SanitizerMask ToolChain::getSupportedSanitizers() const {
  ...
  if (getTriple().getArch() == toolchain::Triple::x86 ||
      getTriple().getArch() == toolchain::Triple::x86_64 ||
      getTriple().getArch() == toolchain::Triple::arm ||
      getTriple().getArch() == toolchain::Triple::thumb || getTriple().isWasm() ||
      getTriple().isAArch64() || getTriple().isRISCV() ||
      getTriple().isLoongArch64())
    Res |= SanitizerKind::CFIICall;
  if (getTriple().getArch() == toolchain::Triple::x86_64 ||
...
toolchain::opt::DerivedArgList *ToolChain::TranslateOpenMPTargetArgs(
  ...
  for (auto *A : Args) {
    ...
    if (A->getOption().matches(options::OPT_Xopenmp_target_EQ)) {
      toolchain::Triple TT(getOpenMPTriple(A->getValue(0)));
...
void ToolChain::TranslateXarchArgs(
  ...
  unsigned Prev = Index;
  std::unique_ptr<toolchain::opt::Arg> XarchArg(Opts.ParseOneArg(
      Args, Index, toolchain::opt::Visibility(language::Core::driver::options::ClangOption)));
...
toolchain::opt::DerivedArgList *ToolChain::TranslateXarchArgs(
