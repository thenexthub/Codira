...
#include "language/Core/Driver/OffloadBundler.h"
...
#include "toolchain/Object/ArchiveWriter.h"
...
static toolchain::ManagedStatic<toolchain::TimerGroup,
...
OffloadTargetInfo::OffloadTargetInfo(const StringRef Target,
  ...
  if (!TargetId.empty() &&
      language::Core::StringToOffloadArch(TargetId) != language::Core::OffloadArch::UNKNOWN)
  ...
  toolchain::Triple T = toolchain::Triple(toolchain::join(TripleSlice, "-"));
  this->Triple = toolchain::Triple(T.getArchName(), T.getVendorName(), T.getOSName(),
...
static uint64_t Read8byteIntegerFromBuffer(StringRef Buffer, size_t pos) {
  return toolchain::support::endian::read64le(Buffer.data() + pos);
}

/// Write 8-byte integers to a buffer in little-endian format.
static void Write8byteIntegerToBuffer(raw_ostream &OS, uint64_t Val) {
  toolchain::support::endian::write(OS, Val, toolchain::endianness::little);
}
...
public:
  ...
  Error ReadHeader(MemoryBuffer &Input) final {
    ...
    if (toolchain::identify_magic(FC) != toolchain::file_magic::offload_bundle)
      return Error::success();
...
class ObjectFileHandler final : public FileHandler {
  ...
  IsOffloadSection(SectionRef CurSection) {
    ...
    if (toolchain::identify_magic(*NameOrErr) != toolchain::file_magic::offload_bundle)
      return std::nullopt;
...
public:
  ...
  Error finalizeOutputFile() final {
    ...
    // We will use toolchain-objcopy to add target objects sections to the output
    // fat object. These sections should have 'exclude' flag set which tells
    // link editor to remove them from linker inputs when linking executable or
    // shared library.

    assert(BundlerConfig.ObjcopyPath != "" &&
           "toolchain-objcopy path not specified");

    // Temporary files that need to be removed.
    TempFileHandlerRAII TempFiles;

    // Compose toolchain-objcopy command line for add target objects' sections with
    // appropriate flags.
    BumpPtrAllocator Alloc;
    StringSaver SS{Alloc};
    SmallVector<StringRef, 8u> ObjcopyArgs{"toolchain-objcopy"};
...
private:
  ...
  Expected<std::string> getHostBundle(StringRef Input) {
    ...
    BumpPtrAllocator Alloc;
    StringSaver SS{Alloc};
    SmallVector<StringRef, 16> ObjcopyArgs{"toolchain-objcopy"};
    ...
    // to be saved to a temporary file before passed to toolchain-objcopy. Otherwise,
...
OffloadBundlerConfig::OffloadBundlerConfig()
  ...
  if (toolchain::compression::zstd::isAvailable()) {
    CompressionFormat = toolchain::compression::Format::Zstd;
    // Compression level 3 is usually sufficient for zstd since long distance
    // matching is enabled.
    CompressionLevel = 3;
  } else if (toolchain::compression::zlib::isAvailable()) {
    CompressionFormat = toolchain::compression::Format::Zlib;
    // Use default level for zlib since higher level does not have significant
    // improvement.
    CompressionLevel = toolchain::compression::zlib::DefaultCompression;
  }
  auto IgnoreEnvVarOpt =
      toolchain::sys::Process::GetEnv("OFFLOAD_BUNDLER_IGNORE_ENV_VAR");
  if (IgnoreEnvVarOpt.has_value() && IgnoreEnvVarOpt.value() == "1")
    return;
  auto VerboseEnvVarOpt = toolchain::sys::Process::GetEnv("OFFLOAD_BUNDLER_VERBOSE");
  if (VerboseEnvVarOpt.has_value())
    Verbose = VerboseEnvVarOpt.value() == "1";
  auto CompressEnvVarOpt =
      toolchain::sys::Process::GetEnv("OFFLOAD_BUNDLER_COMPRESS");
  if (CompressEnvVarOpt.has_value())
    Compress = CompressEnvVarOpt.value() == "1";
  auto CompressionLevelEnvVarOpt =
      toolchain::sys::Process::GetEnv("OFFLOAD_BUNDLER_COMPRESSION_LEVEL");
  if (CompressionLevelEnvVarOpt.has_value()) {
    toolchain::StringRef CompressionLevelStr = CompressionLevelEnvVarOpt.value();
    int Level;
  ...
  auto CompressedBundleFormatVersionOpt =
      toolchain::sys::Process::GetEnv("COMPRESSED_BUNDLE_FORMAT_VERSION");
  if (CompressedBundleFormatVersionOpt.has_value()) {
    toolchain::StringRef VersionStr = CompressedBundleFormatVersionOpt.value();
    uint16_t Version;
...
CompressedOffloadBundle::compress(toolchain::compression::Params P,
  ...
  toolchain::Timer HashTimer("Hash Calculation Timer", "Hash calculation time",
                        *ClangOffloadBundlerTimerGroup);
  if (Verbose)
  ...
  auto BufferUint8 = toolchain::ArrayRef<uint8_t>(
      reinterpret_cast<const uint8_t *>(Input.getBuffer().data()),
      Input.getBuffer().size());
  toolchain::Timer CompressTimer("Compression Timer", "Compression time",
                            *ClangOffloadBundlerTimerGroup);
  if (Verbose)
    CompressTimer.startTimer();
  toolchain::compression::compress(P, BufferUint8, CompressedBuffer);
  if (Verbose)
  ...
  toolchain::raw_svector_ostream OS(FinalBuffer);
  OS << MagicNumber;
  ...
  if (Verbose) {
    auto MethodUsed =
        P.format == toolchain::compression::Format::Zstd ? "zstd" : "zlib";
    ...
    toolchain::errs() << "Compressed bundle format version: " << Version << "\n"
                 ...
                 << "Compression rate: "
                 << toolchain::format("%.2lf", CompressionRate) << "\n"
                 << "Compression ratio: "
                 << toolchain::format("%.2lf%%", 100.0 / CompressionRate) << "\n"
                 << "Compression speed: "
                 << toolchain::format("%.2lf MB/s", CompressionSpeedMBs) << "\n"
                 << "Truncated MD5 hash: "
                 << toolchain::format_hex(TruncatedHash, 16) << "\n";
  }

  return toolchain::MemoryBuffer::getMemBufferCopy(
      toolchain::StringRef(FinalBuffer.data(), FinalBuffer.size()));
}
...
static size_t getHeaderSize(uint16_t Version) {
  ...
  default:
    toolchain_unreachable("Unsupported version");
  }
}
...
CompressedOffloadBundle::CompressedBundleHeader::tryParse(StringRef Blob) {
  ...
  assert(toolchain::identify_magic(Blob) ==
         toolchain::file_magic::offload_bundle_compressed);
...
toolchain::Expected<std::unique_ptr<toolchain::MemoryBuffer>>
CompressedOffloadBundle::decompress(const toolchain::MemoryBuffer &Input,
                                    bool Verbose) {
  StringRef Blob = Input.getBuffer();

  // Check minimum header size (using V1 as it's the smallest)
  if (Blob.size() < sizeof(RawCompressedBundleHeader::CommonFields))
    return toolchain::MemoryBuffer::getMemBufferCopy(Blob);

  if (toolchain::identify_magic(Blob) !=
      toolchain::file_magic::offload_bundle_compressed) {
    if (Verbose)
      toolchain::errs() << "Uncompressed bundle.\n";
    return toolchain::MemoryBuffer::getMemBufferCopy(Blob);
  }
  ...
  toolchain::compression::Format CompressionFormat = Normalized.CompressionFormat;
  ...
  toolchain::Timer DecompressTimer("Decompression Timer", "Decompression time",
                              *ClangOffloadBundlerTimerGroup);
  if (Verbose)
  ...
  if (toolchain::Error DecompressionError = toolchain::compression::decompress(
          CompressionFormat, toolchain::arrayRefFromStringRef(CompressedData),
          DecompressedData, UncompressedSize))
    return createStringError(inconvertibleErrorCode(),
                             "Could not decompress embedded file contents: " +
                                 toolchain::toString(std::move(DecompressionError)));
  ...
  if (Verbose) {
    ...
    toolchain::Timer HashRecalcTimer("Hash Recalculation Timer",
                                "Hash recalculation time",
                                *ClangOffloadBundlerTimerGroup);
    HashRecalcTimer.startTimer();
    toolchain::MD5 Hash;
    toolchain::MD5::MD5Result Result;
    Hash.update(toolchain::ArrayRef<uint8_t>(DecompressedData));
    Hash.final(Result);
    ...
    toolchain::errs() << "Compressed bundle format version: " << ThisVersion << "\n";
    if (ThisVersion >= 2)
      toolchain::errs() << "Total file size (from header): "
                   << formatWithCommas(TotalFileSize) << " bytes\n";
    toolchain::errs() << "Decompression method: "
                 << (CompressionFormat == toolchain::compression::Format::Zlib
                         ? "zlib"
                         : "zstd")
                 << "\n"
                 ...
                 << toolchain::format("%.2lf%%", 100.0 / CompressionRate) << "\n"
                 << "Decompression speed: "
                 << toolchain::format("%.2lf MB/s", DecompressionSpeedMBs) << "\n"
                 << "Stored hash: " << toolchain::format_hex(StoredHash, 16) << "\n"
                 << "Recalculated hash: "
                 << toolchain::format_hex(RecalculatedHash, 16) << "\n"
                 << "Hashes match: " << (HashMatch ? "Yes" : "No") << "\n";
  }

  return toolchain::MemoryBuffer::getMemBufferCopy(
      toolchain::toStringRef(DecompressedData));
}
  ...
  if (!DecompressedBufferOrErr)
    return createStringError(
        ...
        "Failed to decompress input: " +
            toolchain::toString(DecompressedBufferOrErr.takeError()));
...
/// target \p TargetInfo.
/// @link https://clang.toolchain.org/docs/ClangOffloadBundler.html#bundle-entry-id
bool isCodeObjectCompatible(const OffloadTargetInfo &CodeObjectInfo,
  ...
  toolchain::StringMap<bool> CodeObjectFeatureMap, TargetFeatureMap;
  std::optional<StringRef> CodeObjectProc = language::Core::parseTargetID(
      CodeObjectInfo.Triple, CodeObjectInfo.TargetID, &CodeObjectFeatureMap);
  std::optional<StringRef> TargetProc = language::Core::parseTargetID(
...
Error OffloadBundler::BundleFiles() {
  ...
  toolchain::raw_svector_ostream BufferStream(Buffer);
  ...
  if (BundlerConfig.Compress) {
    std::unique_ptr<toolchain::MemoryBuffer> BufferMemory =
        toolchain::MemoryBuffer::getMemBufferCopy(
            toolchain::StringRef(Buffer.data(), Buffer.size()));
...
CheckHeterogeneousArchive(StringRef ArchiveName,
  ...
  Expected<std::unique_ptr<toolchain::object::Archive>> LibOrErr =
  ...
  for (auto ArchiveIter = Archive->child_begin(ArchiveErr);
    ...
    auto &&ConflictingArchs = language::Core::getConflictTargetIDCombination(BundleIds);
    if (ConflictingArchs) {
      std::string ErrMsg =
...
Error OffloadBundler::UnbundleArchive() {
      ...
      if (getCompatibleOffloadTargets(CodeObjectInfo, CompatibleTargets,
        ...
        for (auto &CompatibleTarget : CompatibleTargets) {
          ...
          auto OutputBundleName =
              Twine(toolchain::sys::path::stem(BundledObjectFileName) + "-" +
                    CodeObject +
          ...
          toolchain::replace(OutputBundleName, ':', '_');
          ...
          toolchain::MemoryBufferRef MemBufRef =
  ...
  for (auto &Target : BundlerConfig.TargetNames) {
    ...
    StringMapIterator<std::vector<toolchain::NewArchiveMember>> CurArchiveMembers =
    ...
    if (CurArchiveMembers != OutputArchivesMap.end()) {
      ...
      std::vector<toolchain::NewArchiveMember> EmptyArchive;
      EmptyArchive.clear();
...
bool language::Core::checkOffloadBundleID(const toolchain::StringRef Str) {
  // <kind>-<triple>[-<target id>[:target features]]
  // <triple> := <arch>-<vendor>-<os>-<env>
  SmallVector<StringRef, 6> Components;
