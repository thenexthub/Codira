...
Command::Command(const Action &Source, const Tool &Creator,
                 ...
                 const toolchain::opt::ArgStringList &Arguments,
...
static bool skipArgs(const char *Flag, bool HaveCrashVFS, int &SkipNum,
  ...
  bool ShouldSkip = toolchain::StringSwitch<bool>(Flag)
  ...
  IsInclude =
      toolchain::StringSwitch<bool>(Flag)
  ...
  ShouldSkip = toolchain::StringSwitch<bool>(Flag)
...
void Command::buildArgvForResponseFile(
    toolchain::SmallVectorImpl<const char *> &Out) const {
  ...
  toolchain::StringSet<> Inputs(toolchain::from_range, InputFileList);
  Out.push_back(Executable);
...
static void
rewriteIncludes(const toolchain::ArrayRef<const char *> &Args, size_t Idx,
                size_t NumArgs,
                toolchain::SmallVectorImpl<toolchain::SmallString<128>> &IncFlags) {
...
void Command::Print(raw_ostream &OS, const char *Terminator, bool Quote,
  ...
  OS << ' ';
  toolchain::sys::printArg(OS, Executable, /*Quote=*/true);
  ...
  if (ResponseFile != nullptr) {
    ...
    OS << ' ';
    toolchain::sys::printArg(OS, PrependArg, /*Quote=*/true);
  }
  ...
  for (size_t i = 0, e = Args.size(); i < e; ++i) {
    ...
    if (CrashInfo) {
      ...
      if (HaveCrashVFS && IsInclude) {
        ...
        if (!NewIncFlags.empty()) {
          for (auto &F : NewIncFlags) {
            OS << ' ';
            toolchain::sys::printArg(OS, F.c_str(), Quote);
          }
          i += NumArgs - 1;
          continue;
        }
      ...
      auto Found = toolchain::find_if(InputInfoList, [&Arg](const InputInfo &II) {
      ...
      if (Found != InputInfoList.end() &&
        ...
        OS << ' ';
        StringRef ShortName = toolchain::sys::path::filename(CrashInfo->Filename);
        toolchain::sys::printArg(OS, ShortName.str(), Quote);
        continue;
      }
    }

    OS << ' ';
    toolchain::sys::printArg(OS, Arg, Quote);
  }

  if (CrashInfo && HaveCrashVFS) {
    OS << ' ';
    toolchain::sys::printArg(OS, "-ivfsoverlay", Quote);
    OS << ' ';
    toolchain::sys::printArg(OS, CrashInfo->VFSPath.str(), Quote);
    ...
    SmallString<128> RelModCacheDir = toolchain::sys::path::parent_path(
        toolchain::sys::path::parent_path(CrashInfo->VFSPath));
    toolchain::sys::path::append(RelModCacheDir, "repro-modules");

    std::string ModCachePath = "-fmodules-cache-path=";
    ModCachePath.append(RelModCacheDir.c_str());

    OS << ' ';
    toolchain::sys::printArg(OS, ModCachePath, Quote);
  }
...
void Command::setEnvironment(toolchain::ArrayRef<const char *> NewEnvironment) {
...
void Command::PrintFileNames() const {
  if (PrintInputFilenames) {
    for (const auto &Arg : InputInfoList)
      toolchain::outs() << toolchain::sys::path::filename(Arg.getFilename()) << "\n";
...
int Command::Execute(ArrayRef<std::optional<StringRef>> Redirects,
  ...
  if (ResponseFile == nullptr) {
    ...
    toolchain::raw_string_ostream SS(RespContents);
    ...
    if (std::error_code EC = writeFileWithEncoding(
      ...
      // Return -1 by convention (see toolchain/include/toolchain/Support/Program.h) to
  ...
  if (!Environment.empty()) {
    ...
    ArgvVectorStorage = toolchain::toStringRefArray(Environment.data());
    Env = ArrayRef(ArgvVectorStorage);
  }

  auto Args = toolchain::toStringRefArray(Argv.data());
  ...
  if (!RedirectFiles.empty()) {
    ...
    return toolchain::sys::ExecuteAndWait(Executable, Args, Env,
                                     ArrayRef(RedirectFilesOptional),
                                     /*secondsToWait=*/0, /*memoryLimit=*/0,
                                     ErrMsg, ExecutionFailed, &ProcStat);
  }

  return toolchain::sys::ExecuteAndWait(Executable, Args, Env, Redirects,
                                   /*secondsToWait*/ 0, /*memoryLimit*/ 0,
                                   ErrMsg, ExecutionFailed, &ProcStat);
}
...
int CC1Command::Execute(ArrayRef<std::optional<StringRef>> Redirects,
  ...
  // out-of-process execution. See discussion in https://reviews.toolchain.org/D74447
  if (!InProcess)
  ...
  const void *PrettyState = toolchain::SavePrettyStackState();
  const Driver &D = getCreator().getToolChain().getDriver();

  int R = 0;
  // Enter ExecuteCC1Tool() instead of starting up a new process
  if (!CRC.RunSafely([&]() { R = D.CC1Main(Argv); })) {
    toolchain::RestorePrettyStackState(PrettyState);
    return CRC.RetCode;
  }
  return R;
}

void CC1Command::setEnvironment(toolchain::ArrayRef<const char *> NewEnvironment) {
