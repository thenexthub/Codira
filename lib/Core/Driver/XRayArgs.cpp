...
XRayArgs::XRayArgs(const ToolChain &TC, const ArgList &Args) {
  ...
  if (Bundles.empty())
    ...
    for (const auto &B : Bundles) {
      ...
      toolchain::SplitString(B, BundleParts, ",");
      for (const auto &P : BundleParts) {
        // TODO: Automate the generation of the string case table.
        auto Valid = toolchain::StringSwitch<bool>(P)
  ...
  if (SpecifiedModes.empty())
    toolchain::append_range(Modes, XRaySupportedModes);
  else
    for (const auto &Arg : SpecifiedModes) {
      // Parse CSV values for -fxray-modes=...
      toolchain::SmallVector<StringRef, 2> ModeParts;
      toolchain::SplitString(Arg, ModeParts, ",");
  ...
  toolchain::sort(Modes);
  Modes.erase(toolchain::unique(Modes), Modes.end());
}
