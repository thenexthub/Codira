...
static bool isSupportedMCU(const StringRef MCU) {
  return toolchain::StringSwitch<bool>(MCU)
#define MSP430_MCU(NAME) .Case(NAME, true)
#include "language/Core/Basic/MSP430Target.def"
      .Default(false);
}
...
static StringRef getSupportedHWMult(const Arg *MCU) {
  ...
  return toolchain::StringSwitch<StringRef>(MCU->getValue())
...
static StringRef getHWMultLib(const ArgList &Args) {
  ...
  return toolchain::StringSwitch<StringRef>(HWMult)
      .Case("16bit", "-lmul_16")
      .Case("32bit", "-lmul_32")
...
void msp430::getMSP430TargetFeatures(const Driver &D, const ArgList &Args,
  ...
  if (HWMult == "auto") {
    ...
    if (!MCU)
      D.Diag(language::Core::diag::warn_drv_msp430_hwmult_no_device);
  ...
  if (MCU && SupportedHWMult == "none")
    D.Diag(language::Core::diag::warn_drv_msp430_hwmult_unsupported) << HWMult;
  if (MCU && HWMult != SupportedHWMult)
    D.Diag(language::Core::diag::warn_drv_msp430_hwmult_mismatch)
...
/// MSP430 Toolchain
MSP430ToolChain::MSP430ToolChain(const Driver &D, const toolchain::Triple &Triple,
                                 const ArgList &Args)
  ...
  toolchain::sys::path::append(SysRootDir, "msp430-elf", "lib", MultilibSuf);
...
std::string MSP430ToolChain::computeSysRoot() const {
  ...
  SmallString<128> Dir;
  if (GCCInstallation.isValid())
    toolchain::sys::path::append(Dir, GCCInstallation.getParentLibPath(), "..");
  else
    toolchain::sys::path::append(Dir, getDriver().Dir, "..");
...
void MSP430ToolChain::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  toolchain::sys::path::append(Dir, "msp430-elf", "include");
...
void msp430::Linker::AddDefaultLibs(const toolchain::opt::ArgList &Args,
...
static void AddImplicitLinkerScript(const std::string SysRoot,
  ...
  toolchain::sys::path::append(MCULinkerScriptPath, "include");
