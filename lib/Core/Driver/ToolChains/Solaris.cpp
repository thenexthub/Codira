...
void solaris::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_nodefaultlibs,
    ...
    if (Arch == toolchain::Triple::sparc) {
    ...
    if (NeedsSanitizerDeps) {
      ...
      // in Illumos.
      if (Arch == toolchain::Triple::x86_64 &&
...
static StringRef getSolarisLibSuffix(const toolchain::Triple &Triple) {
...
Solaris::Solaris(const Driver &D, const toolchain::Triple &Triple,
                 const ArgList &Args)
...
SanitizerMask Solaris::getSupportedSanitizers() const {
  const bool IsSparc = getTriple().getArch() == toolchain::Triple::sparc;
...
const char *Solaris::getDefaultLinker() const {
  ...
  return toolchain::StringSwitch<const char *>(getDriver().getPreferredLinker())
