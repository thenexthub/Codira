...
static std::string findPassPlugin(const Driver &D,
  ...
  if (!hipPath.empty()) {
    ...
    toolchain::sys::path::append(PluginPath, "lib", "libLLVMHipSpvPasses.so");
    if (toolchain::sys::fs::exists(PluginPath))
      return PluginPath.str().str();
    PluginPath.assign(hipPath);
    toolchain::sys::path::append(PluginPath, "lib", "toolchain",
...
void HIPSPV::Linker::constructLinkAndEmitSpirvCommand(
  ...
  toolchain::opt::ArgStringList TrArgs{"--spirv-max-version=1.1",
                                  "--spirv-ext=+all"};
...
HIPSPVToolChain::HIPSPVToolChain(const Driver &D, const toolchain::Triple &Triple,
...
void HIPSPVToolChain::addClangTargetOptions(
  ...
  CC1Args.append(
      {"-fcuda-is-device", "-fcuda-allow-variadic-functions",
       // A crude workaround for toolchain-spirv which does not handle the
       ...
       "-mtoolchain", "-vectorize-loops=false", "-mtoolchain", "-vectorize-slp=false"});
...
HIPSPVToolChain::getDeviceLibs(
  ...
  auto HipDeviceLibPathArgs = DriverArgs.getAllArgValues(
      ...
      language::Core::driver::options::OPT_rocm_device_lib_path_EQ);
  ...
  if (!HipPath.empty()) {
    ...
    toolchain::sys::path::append(Path, "lib", "hip-device-lib");
    LibraryPaths.push_back(DriverArgs.MakeArgString(Path));
  }
