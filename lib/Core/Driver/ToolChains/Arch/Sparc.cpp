...
const char *sparc::getSparcAsmModeForCPU(StringRef Name,
  ...
  if (Triple.getArch() == toolchain::Triple::sparcv9) {
    const char *DefV9CPU;
...
sparc::FloatABI sparc::getSparcFloatABI(const Driver &D,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_msoft_float, options::OPT_mno_fpu,
    ...
    if (A->getOption().matches(options::OPT_msoft_float) ||
      ...
      ABI = toolchain::StringSwitch<sparc::FloatABI>(A->getValue())
...
std::string sparc::getSparcTargetCPU(const Driver &D, const ArgList &Args,
  ...
  if (Triple.getArch() == toolchain::Triple::sparc &&
      (Triple.isOSSolaris() || Triple.isOSLinux()))
    return "v9";
  return "";
}

void sparc::getSparcTargetFeatures(const Driver &D, const toolchain::Triple &Triple,
                                   const ArgList &Args,
  ...
  bool IsSparcV9ATarget =
      (Triple.getArch() == toolchain::Triple::sparcv9) &&
      (Triple.isOSLinux() || Triple.isOSFreeBSD() || Triple.isOSOpenBSD());
  bool IsSparcV9BTarget = Triple.isOSSolaris();
  bool IsSparcV8PlusTarget =
      Triple.getArch() == toolchain::Triple::sparc && Triple.isOSSolaris();
