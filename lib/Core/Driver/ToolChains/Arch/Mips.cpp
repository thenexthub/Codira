...
void mips::getMipsCPUAndABI(const ArgList &Args, const toolchain::Triple &Triple,
  ...
  if (Triple.getVendor() == toolchain::Triple::ImaginationTechnologies &&
      Triple.isGNUEnvironment()) {
    DefMips32CPU = "mips32r6";
    DefMips64CPU = "mips64r6";
  }

  if (Triple.getSubArch() == toolchain::Triple::MipsSubArch_r6) {
    DefMips32CPU = "mips32r6";
    DefMips64CPU = "mips64r6";
  }
  ...
  if (Arg *A = Args.getLastArg(language::Core::driver::options::OPT_march_EQ,
                               options::OPT_mcpu_EQ))
    CPUName = A->getValue();
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mabi_EQ)) {
    ...
    ABIName = toolchain::StringSwitch<toolchain::StringRef>(ABIName)
                  .Case("32", "o32")
                  .Case("64", "n64")
                  .Default(ABIName);
  }
  ...
  if (CPUName.empty() && ABIName.empty()) {
    ...
    default:
      toolchain_unreachable("Unexpected triple arch name");
  ...
  if (ABIName.empty() &&
      (Triple.getVendor() == toolchain::Triple::MipsTechnologies ||
       Triple.getVendor() == toolchain::Triple::ImaginationTechnologies)) {
    ABIName = toolchain::StringSwitch<const char *>(CPUName)
                  .Case("mips1", "o32")
                  .Case("mips2", "o32")
                  .Case("mips3", "n64")
                  .Case("mips4", "n64")
  ...
  if (CPUName.empty()) {
    ...
    CPUName = toolchain::StringSwitch<const char *>(ABIName)
...
std::string mips::getMipsABILibSuffix(const ArgList &Args,
  ...
  return toolchain::StringSwitch<std::string>(ABIName)
      .Case("o32", "")
      .Case("n32", "32")
      .Case("n64", "64");
}
...
StringRef mips::getGnuCompatibleMipsABIName(StringRef ABI) {
  return toolchain::StringSwitch<toolchain::StringRef>(ABI)
      .Case("o32", "32")
      .Case("n64", "64")
      .Default(ABI);
}
...
mips::FloatABI mips::getMipsFloatABI(const Driver &D, const ArgList &Args,
  ...
  if (Arg *A =
    ...
    if (A->getOption().matches(options::OPT_msoft_float))
      ...
      ABI = toolchain::StringSwitch<mips::FloatABI>(A->getValue())
...
void mips::getMIPSTargetFeatures(const Driver &D, const toolchain::Triple &Triple,
                                 const ArgList &Args,
...
mips::IEEE754Standard mips::getIEEE754Standard(StringRef &CPU) {
  ...
  return (IEEE754Standard)toolchain::StringSwitch<int>(CPU)
      .Case("mips1", Legacy)
      .Case("mips2", Legacy)
      .Case("mips3", Legacy)
      .Case("mips4", Legacy)
...
bool mips::hasCompactBranches(StringRef &CPU) {
  ...
  return toolchain::StringSwitch<bool>(CPU)
      .Case("mips32r6", true)
      .Case("mips64r6", true)
      .Default(false);
}
...
bool mips::isNaN2008(const Driver &D, const ArgList &Args,
  ...
  if (Arg *NaNArg = Args.getLastArg(options::OPT_mnan_EQ))
    return toolchain::StringSwitch<bool>(NaNArg->getValue())
        .Case("2008", true)
        .Case("legacy", false)
        .Default(false);

  // NaN2008 is the default for MIPS32r6/MIPS64r6.
  return toolchain::StringSwitch<bool>(getCPUName(D, Args, Triple))
      .Cases("mips32r6", "mips64r6", true)
      .Default(false);
}

bool mips::isFPXXDefault(const toolchain::Triple &Triple, StringRef CPUName,
  ...
  return toolchain::StringSwitch<bool>(CPUName)
...
bool mips::shouldUseFPXX(const ArgList &Args, const toolchain::Triple &Triple,
                         StringRef CPUName, StringRef ABIName,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mmsa))
    if (A->getOption().matches(options::OPT_mmsa))
      UseFPXX = toolchain::StringSwitch<bool>(CPUName)
