...
static bool getArchFeatures(const Driver &D, StringRef Arch,
  ...
  auto ISAInfo =
      toolchain::RISCVISAInfo::parseArchString(Arch, EnableExperimentalExtensions);
  if (!ISAInfo) {
    handleAllErrors(ISAInfo.takeError(), [&](toolchain::StringError &ErrMsg) {
...
static void getRISCFeaturesFromMcpu(const Driver &D, const Arg *A,
  ...
  if (!toolchain::RISCV::parseCPU(Mcpu, Is64Bit)) {
    // Try inverting Is64Bit in case the CPU is valid, but for the wrong target.
    if (toolchain::RISCV::parseCPU(Mcpu, !Is64Bit))
      D.Diag(language::Core::diag::err_drv_invalid_riscv_cpu_name_for_target)
          << Mcpu << Is64Bit;
    else
...
void riscv::getRISCVTargetFeatures(const Driver &D, const toolchain::Triple &Triple,
                                   const ArgList &Args,
                                   std::vector<StringRef> &Features) {
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    ...
    if (toolchain::RISCV::hasFastScalarUnalignedAccess(CPU))
      CPUFastScalarUnaligned = true;
    if (toolchain::RISCV::hasFastVectorUnalignedAccess(CPU))
...
#undef RESERVE_REG
  ...
  if (Args.hasFlag(options::OPT_mrelax, options::OPT_mno_relax, true)) {
    ...
    if (getDebugFissionKind(D, Args, A) != DwarfFissionKind::None)
      D.Diag(language::Core::diag::err_drv_riscv_unsupported_with_linker_relaxation)
          << A->getAsString(Args);
  } else {
    Features.push_back("-relax");
  }
...
StringRef riscv::getRISCVABI(const ArgList &Args, const toolchain::Triple &Triple) {
  ...
  auto ParseResult = toolchain::RISCVISAInfo::parseArchString(
...
std::string riscv::getRISCVArch(const toolchain::opt::ArgList &Args,
                                const toolchain::Triple &Triple) {
  assert(Triple.isRISCV() && "Unexpected triple");
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    ...
    if (CPU == "native") {
      ...
      if (CPU.starts_with("generic")) {
        auto FeatureMap = toolchain::sys::getHostCPUFeatures();
    ...
    StringRef MArch = toolchain::RISCV::getMArchFromMcpu(CPU);
...
std::string riscv::getRISCVTargetCPU(const toolchain::opt::ArgList &Args,
                                     const toolchain::Triple &Triple) {
  std::string CPU;
