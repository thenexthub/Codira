...
ZOS::ZOS(const Driver &D, const toolchain::Triple &Triple, const ArgList &Args)
...
void zos::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (IsSharedLib) {
    ...
    toolchain::sys::path::replace_extension(SideDeckName, "x");
    CmdArgs.push_back("-x");
...
void ZOS::AddCXXStdlibLibArgs(const toolchain::opt::ArgList &Args,
                              toolchain::opt::ArgStringList &CmdArgs) const {
  switch (GetCXXStdlibType(Args)) {
  case ToolChain::CST_Libstdcxx:
    toolchain::report_fatal_error("linking libstdc++ is unimplemented on z/OS");
    break;
...
void ZOS::TryAddIncludeFromPath(toolchain::SmallString<128> Path,
...
void ZOS::AddClangCXXStdlibIncludeArgs(
  ...
  case ToolChain::CST_Libcxx: {
    ...
    toolchain::SmallString<128> InstallBin(getDriver().Dir);
