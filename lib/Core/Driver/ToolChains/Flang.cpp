...
static bool shouldLoopVersion(const ArgList &Args) {
  ...
  toolchain_unreachable("We should not end up here");
  return false;
}
  ...
  if (Arg *A = Args.getLastArg(options::OPT_msve_vector_bits_EQ)) {
    ...
    if (Val == "128" || Val == "256" || Val == "512" || Val == "1024" ||
      ...
      if (!Val.consume_back("+")) {
        ...
        CmdArgs.push_back(
            Args.MakeArgString("-mvscale-max=" + toolchain::Twine(Bits / 128)));
      }

      [[maybe_unused]] bool Invalid = Val.getAsInteger(10, Bits);
      assert(!Invalid && "Failed to parse value");
      CmdArgs.push_back(
          Args.MakeArgString("-mvscale-min=" + toolchain::Twine(Bits / 128)));
...
void Flang::AddX86_64TargetArgs(const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_masm_EQ)) {
    ...
    if (Value == "intel" || Value == "att") {
      CmdArgs.push_back(Args.MakeArgString("-mtoolchain"));
...
static void processVSRuntimeLibrary(const ToolChain &TC, const ArgList &Args,
  ...
  if (auto *rtl = Args.getLastArg(options::OPT_fms_runtime_lib_EQ)) {
    RTOptionID = toolchain::StringSwitch<unsigned>(rtl->getValue())
  ...
  for (size_t i = 1; i < Inputs.size(); ++i) {
    if (Inputs[i].getType() == types::TY_Nothing) {
      ...
      if (Inputs[i].getFilename()) {
        ...
        toolchain_unreachable("missing openmp host-ir file for device offloading");
      }
    } else {
      toolchain_unreachable(
...
void Flang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  bool isAtemporaryPreprocessedFile =
      Input.isFilename() &&
      toolchain::sys::path::extension(Input.getFilename())
  ...
  // Forward -mtoolchain options to the LLVM option parser. In practice, this means
