...
RocmInstallationDetector::CommonBitcodeLibsPreferences::
  ...
  const auto Kind = toolchain::AMDGPU::parseArchAMDGCN(GPUArch);
  const unsigned ArchAttr = toolchain::AMDGPU::getArchAttrAMDGCN(Kind);

  IsOpenMP = DeviceOffloadingKind == Action::OFK_OpenMP;

  const bool HasWave32 = (ArchAttr & toolchain::AMDGPU::FEATURE_WAVE32);
  Wave64 =
  ...
  const bool DefaultDAZ =
      (Kind == toolchain::AMDGPU::GK_NONE)
          ? false
          : !((ArchAttr & toolchain::AMDGPU::FEATURE_FAST_FMA_F32) &&
              (ArchAttr & toolchain::AMDGPU::FEATURE_FAST_DENORMAL_F32));
...
void RocmInstallationDetector::scanLibDevicePath(toolchain::StringRef Path) {
  assert(!Path.empty());
  ...
  std::error_code EC;
  for (toolchain::vfs::directory_iterator LI = D.getVFS().dir_begin(Path, EC), LE;
       !EC && LI != LE; LI = LI.increment(EC)) {
    StringRef FilePath = LI->path();
    StringRef FileName = toolchain::sys::path::filename(FilePath);
    ...
    if (BaseName == "ocml") {
      ...
      toolchain::Twine GfxName = Twine("gfx") + IsaVersionNumber;
      SmallString<8> Tmp;
      LibDeviceMap.insert(
...
// the parsing fails.
bool RocmInstallationDetector::parseHIPVersionFile(toolchain::StringRef V) {
  ...
  if (Major == ~0U || Minor == ~0U)
    return true;
  VersionMajorMinor = toolchain::VersionTuple(Major, Minor);
  DetectedVersion =
...
RocmInstallationDetector::getInstallationPathCandidates() {
  ...
  auto DoPrintROCmSearchDirs = [&]() {
    if (PrintROCmSearchDirs)
      for (auto Cand : ROCmSearchDirs) {
        toolchain::errs() << "ROCm installation search path: " << Cand.Path << '\n';
      }
  };
  ...
  if (!RocmPathArg.empty()) {
    ...
    return ROCmSearchDirs;
                 ...
                 toolchain::sys::Process::GetEnv("ROCM_PATH")) {
  ...
  auto DeduceROCmPath = [](StringRef ClangPath) {
    ...
    StringRef ParentDir = toolchain::sys::path::parent_path(ClangPath);
    StringRef ParentName = toolchain::sys::path::filename(ParentDir);

    // Some builds use bin/{host arch}, so go up again.
    if (ParentName == "bin") {
      ParentDir = toolchain::sys::path::parent_path(ParentDir);
      ParentName = toolchain::sys::path::filename(ParentDir);
    }

    // Some versions of the rocm toolchain package install to /opt/rocm/toolchain/bin
    // Some versions of the aomp package install to /opt/rocm/aomp/bin
    if (ParentName == "toolchain" || ParentName.starts_with("aomp"))
      ParentDir = toolchain::sys::path::parent_path(ParentDir);
  ...
  // link of clang itself.
  toolchain::SmallString<256> RealClangPath;
  toolchain::sys::fs::real_path(D.getClangProgramPath(), RealClangPath);
  auto ParentPath = toolchain::sys::path::parent_path(RealClangPath);
  ...
  auto ClangRoot = toolchain::sys::path::parent_path(InstallDir);
  auto RealClangRoot = toolchain::sys::path::parent_path(ParentPath);
  ...
  for (toolchain::vfs::directory_iterator
    ...
    toolchain::StringRef FileName = toolchain::sys::path::filename(File->path());
...
RocmInstallationDetector::RocmInstallationDetector(
    ...
    const toolchain::opt::ArgList &Args, bool DetectHIPRuntime, bool DetectDeviceLib)
    : D(D) {
  Verbose = Args.hasArg(options::OPT_v);
  RocmPathArg = Args.getLastArgValue(language::Core::driver::options::OPT_rocm_path_EQ);
  PrintROCmSearchDirs =
      Args.hasArg(language::Core::driver::options::OPT_print_rocm_search_dirs);
  RocmDeviceLibPathArg =
      Args.getAllArgValues(language::Core::driver::options::OPT_rocm_device_lib_path_EQ);
  HIPPathArg = Args.getLastArgValue(language::Core::driver::options::OPT_hip_path_EQ);
  HIPStdParPathArg =
    Args.getLastArgValue(language::Core::driver::options::OPT_hipstdpar_path_EQ);
  HasHIPStdParLibrary =
    !HIPStdParPathArg.empty() && D.getVFS().exists(HIPStdParPathArg +
                                                   "/hipstdpar_lib.hpp");
  HIPRocThrustPathArg =
    Args.getLastArgValue(language::Core::driver::options::OPT_hipstdpar_thrust_path_EQ);
  HasRocThrustLibrary = !HIPRocThrustPathArg.empty() &&
                        D.getVFS().exists(HIPRocThrustPathArg + "/thrust");
  HIPRocPrimPathArg =
    Args.getLastArgValue(language::Core::driver::options::OPT_hipstdpar_prim_path_EQ);
  HasRocPrimLibrary = !HIPRocPrimPathArg.empty() &&
                      D.getVFS().exists(HIPRocPrimPathArg + "/rocprim");

  if (auto *A = Args.getLastArg(language::Core::driver::options::OPT_hip_version_EQ)) {
    ...
    VersionMajorMinor =
        toolchain::VersionTuple(DefaultVersionMajor, DefaultVersionMinor);
...
void RocmInstallationDetector::detectDeviceLibrary() {
  ...
  if (!RocmDeviceLibPathArg.empty())
    LibDevicePath = RocmDeviceLibPathArg[RocmDeviceLibPathArg.size() - 1];
               ...
               toolchain::sys::Process::GetEnv("HIP_DEVICE_LIB_PATH"))
  ...
  toolchain::sys::path::append(LibDevicePath, CLANG_INSTALL_LIBDIR_BASENAME,
                          "amdgcn", "bitcode");
  ...
  for (const auto &Candidate : ROCmDirs) {
    ...
    toolchain::sys::path::append(LibDevicePath, "amdgcn", "bitcode");
...
void RocmInstallationDetector::detectHIPRuntime() {
  ...
  if (!HIPPathArg.empty())
    HIPSearchDirs.emplace_back(HIPPathArg.str());
               ...
               toolchain::sys::Process::GetEnv("HIP_PATH")) {
    if (!HIPPathEnv->empty())
  ...
  for (const auto &Candidate : HIPSearchDirs) {
    ...
    BinPath = InstallPath;
    toolchain::sys::path::append(BinPath, "bin");
    IncludePath = InstallPath;
    toolchain::sys::path::append(IncludePath, "include");
    LibPath = InstallPath;
    toolchain::sys::path::append(LibPath, "lib");
    SharePath = InstallPath;
    toolchain::sys::path::append(SharePath, "share");

    // Get parent of InstallPath and append "share"
    SmallString<0> ParentSharePath = toolchain::sys::path::parent_path(InstallPath);
    toolchain::sys::path::append(ParentSharePath, "share");

    auto Append = [](SmallString<0> &path, const Twine &a, const Twine &b = "",
                     const Twine &c = "", const Twine &d = "") {
      SmallString<0> newpath = path;
      toolchain::sys::path::append(newpath, a, b, c, d);
      return newpath;
    };
    ...
    for (const auto &VersionFilePath : VersionFilePaths) {
      if (VersionFilePath.empty())
        continue;
      toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> VersionFile =
...
void RocmInstallationDetector::AddHIPIncludeArgs(const ArgList &DriverArgs,
  ...
  bool UsesRuntimeWrapper = VersionMajorMinor > toolchain::VersionTuple(3, 5) &&
  ...
  if (!DriverArgs.hasArg(options::OPT_nobuiltininc)) {
    ...
    if (UsesRuntimeWrapper)
      toolchain::sys::path::append(P, "include", "cuda_wrappers");
...
void amdgpu::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (!Features.empty()) {
    CmdArgs.push_back(
        Args.MakeArgString("-plugin-opt=-mattr=" + toolchain::join(Features, ",")));
  }
  ...
  if (Args.hasArg(options::OPT_startfiles)) {
    ...
    toolchain::sys::path::append(P, "crt1.o");
...
/// AMDGPU Toolchain
AMDGPUToolChain::AMDGPUToolChain(const Driver &D, const toolchain::Triple &Triple,
                                 const ArgList &Args)
...
AMDGPUToolChain::TranslateArgs(const DerivedArgList &Args, StringRef BoundArch,
  ...
  if (LastMCPUArg && StringRef(LastMCPUArg->getValue()) == "native") {
    ...
    if (!GPUsOrErr) {
      getDriver().Diag(diag::err_drv_undetermined_gpu_arch)
          << toolchain::Triple::getArchTypeName(getArch())
          << toolchain::toString(GPUsOrErr.takeError()) << "-mcpu";
    } else {
      auto &GPUs = *GPUsOrErr;
      if (GPUs.size() > 1) {
        getDriver().Diag(diag::warn_drv_multi_gpu_arch)
            << toolchain::Triple::getArchTypeName(getArch())
            << toolchain::join(GPUs, ", ") << "-mcpu";
      }
  ...
  // Phase 1 (.cl -> .bc)
  if (Args.hasArg(options::OPT_c) && Args.hasArg(options::OPT_emit_toolchain)) {
...
bool AMDGPUToolChain::getDefaultDenormsAreZeroForTarget(
  ...
  if (Kind == toolchain::AMDGPU::GK_NONE)
    return false;
...
toolchain::DenormalMode AMDGPUToolChain::getDefaultDenormalModeForType(
    const toolchain::opt::ArgList &DriverArgs, const JobAction &JA,
    const toolchain::fltSemantics *FPType) const {
  // Denormals should always be enabled for f16 and f64.
  if (!FPType || FPType != &toolchain::APFloat::IEEEsingle())
  ...
  if (JA.getOffloadingDeviceKind() == Action::OFK_HIP ||
    ...
    auto Kind = toolchain::AMDGPU::parseArchAMDGCN(Arch);
    if (FPType && FPType == &toolchain::APFloat::IEEEsingle() &&
        DriverArgs.hasFlag(options::OPT_fgpu_flush_denormals_to_zero,
                           options::OPT_fno_gpu_flush_denormals_to_zero,
  ...
  auto Kind = toolchain::AMDGPU::parseArchAMDGCN(GpuArch);
  ...
  return DAZ ? toolchain::DenormalMode::getPreserveSign() :
               toolchain::DenormalMode::getIEEE();
}

bool AMDGPUToolChain::isWave64(const toolchain::opt::ArgList &DriverArgs,
                               toolchain::AMDGPU::GPUKind Kind) {
  const unsigned ArchAttr = toolchain::AMDGPU::getArchAttrAMDGCN(Kind);
  bool HasWave32 = (ArchAttr & toolchain::AMDGPU::FEATURE_WAVE32);
...
/// ROCM Toolchain
ROCMToolChain::ROCMToolChain(const Driver &D, const toolchain::Triple &Triple,
                             const ArgList &Args)
...
StringRef
AMDGPUToolChain::getGPUArch(const toolchain::opt::ArgList &DriverArgs) const {
  return getProcessorFromTargetID(
      getTriple(), DriverArgs.getLastArgValue(options::OPT_mcpu_EQ));
}

AMDGPUToolChain::ParsedTargetIDType
AMDGPUToolChain::getParsedTargetID(const toolchain::opt::ArgList &DriverArgs) const {
...
void ROCMToolChain::addClangTargetOptions(
  ...
  const StringRef GpuArch = getGPUArch(DriverArgs);
  auto Kind = toolchain::AMDGPU::parseArchAMDGCN(GpuArch);
  const StringRef CanonArch = toolchain::AMDGPU::getArchNameAMDGCN(Kind);
...
RocmInstallationDetector::getCommonBitcodeLibs(
    const toolchain::opt::ArgList &DriverArgs, StringRef LibDeviceFile,
    StringRef GPUArch, const Action::OffloadKind DeviceOffloadingKind,
    const bool NeedsASanRT) const {
  toolchain::SmallVector<ToolChain::BitCodeLibraryInfo, 12> BCLibs;
...
ROCMToolChain::getCommonDeviceLibNames(
    const toolchain::opt::ArgList &DriverArgs, const std::string &GPUArch,
    Action::OffloadKind DeviceOffloadingKind) const {
  auto Kind = toolchain::AMDGPU::parseArchAMDGCN(GPUArch);
  const StringRef CanonArch = toolchain::AMDGPU::getArchNameAMDGCN(Kind);
...
bool AMDGPUToolChain::shouldSkipSanitizeOption(
    const ToolChain &TC, const toolchain::opt::ArgList &DriverArgs,
    StringRef TargetID, const toolchain::opt::Arg *A) const {
  ...
  if (Loc == FeatureMap.end() || !Loc->second) {
    Diags.Report(
        language::Core::diag::warn_drv_unsupported_option_for_offload_arch_req_feature)
