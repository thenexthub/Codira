...
void netbsd::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  case toolchain::Triple::mips64el:
    if (mips::hasMipsAbiArg(Args, "32")) {
      CmdArgs.push_back("-m");
      if (Arch == toolchain::Triple::mips64)
...
NetBSD::NetBSD(const Driver &D, const toolchain::Triple &Triple, const ArgList &Args)
...
void NetBSD::addLibCxxIncludePaths(const toolchain::opt::ArgList &DriverArgs,
                                   toolchain::opt::ArgStringList &CC1Args) const {
...
void NetBSD::addLibStdCxxIncludePaths(const toolchain::opt::ArgList &DriverArgs,
                                      toolchain::opt::ArgStringList &CC1Args) const {
  addLibStdCXXIncludePaths(concat(getDriver().SysRoot, "/usr/include/g++"), "", "",
                           DriverArgs, CC1Args);
}

toolchain::ExceptionHandling NetBSD::GetExceptionModel(const ArgList &Args) const {
  // NetBSD uses Dwarf exceptions on ARM.
  toolchain::Triple::ArchType TArch = getTriple().getArch();
  if (TArch == toolchain::Triple::arm || TArch == toolchain::Triple::armeb ||
      TArch == toolchain::Triple::thumb || TArch == toolchain::Triple::thumbeb)
