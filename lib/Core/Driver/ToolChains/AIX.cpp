...
using AIX = language::Core::driver::toolchains::AIX;
...
void aix::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (!IsArch32Bit && !IsArch64Bit)
    toolchain_unreachable("Unsupported bit width value.");
...
static bool hasExportListLinkerOpts(const ArgStringList &CmdArgs) {
  for (size_t i = 0, Size = CmdArgs.size(); i < Size; ++i) {
    toolchain::StringRef ArgString(CmdArgs[i]);
    ...
    if (ArgString == "-b" && i + 1 < Size) {
      ++i;
      toolchain::StringRef ArgNextString(CmdArgs[i]);
...
void aix::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Arg *A =
          Args.getLastArg(language::Core::driver::options::OPT_mxcoff_build_id_EQ)) {
    StringRef BuildId = A->getValue();
    if (BuildId[0] != '0' || BuildId[1] != 'x' ||
        BuildId.find_if_not(toolchain::isHexDigit, 2) != StringRef::npos)
  ...
  if (Args.hasArg(options::OPT_shared) && !hasExportListLinkerOpts(CmdArgs)) {
    ...
    CmdArgs.push_back(Args.MakeArgString(toolchain::Twine("-bE:") + ExportList));
  }
  ...
  if (!Args.hasArg(options::OPT_r)) {
    ...
    if (!Args.hasArg(options::OPT_nostdlib, options::OPT_nodefaultlibs)) {
      ...
      if (Args.hasArgNoClaim(options::OPT_p, options::OPT_pg)) {
        CmdArgs.push_back(Args.MakeArgString((toolchain::Twine("-L") + D.SysRoot) +
                                             "/lib/profiled"));
        CmdArgs.push_back(Args.MakeArgString((toolchain::Twine("-L") + D.SysRoot) +
                                             "/usr/lib/profiled"));
      }
...
AIX::AIX(const Driver &D, const toolchain::Triple &Triple, const ArgList &Args)
...
toolchain::StringRef
AIX::GetHeaderSysroot(const toolchain::opt::ArgList &DriverArgs) const {
...
void AIX::AddOpenMPIncludeArgs(const ArgList &DriverArgs,
  ...
  if (DriverArgs.hasFlag(options::OPT_fopenmp, options::OPT_fopenmp_EQ,
    ...
    case Driver::OMPRT_OMP:
      ...
      toolchain::sys::path::append(PathOpenMP, "opt/IBM/openxlCSDK", "include",
                              "openmp");
...
void AIX::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  toolchain::StringRef Sysroot = GetHeaderSysroot(DriverArgs);
...
void AIX::AddClangCXXStdlibIncludeArgs(
  ...
  case ToolChain::CST_Libcxx: {
    ...
    toolchain::sys::path::append(PathCPP, "opt/IBM/openxlCSDK", "include", "c++",
                            "v1");
  ...
  toolchain_unreachable("Unexpected C++ library type; only libc++ is supported.");
}

void AIX::AddCXXStdlibLibArgs(const toolchain::opt::ArgList &Args,
                              toolchain::opt::ArgStringList &CmdArgs) const {
  switch (GetCXXStdlibType(Args)) {
  case ToolChain::CST_Libstdcxx:
    toolchain::report_fatal_error("linking libstdc++ unimplemented on AIX");
  case ToolChain::CST_Libcxx:
    CmdArgs.push_back("-lc++");
...
static void addTocDataOptions(const toolchain::opt::ArgList &Args,
  ...
  std::set<toolchain::StringRef> ExplicitlySpecifiedGlobals;
  for (const auto Arg :
  ...
  auto buildExceptionList = [](const std::set<toolchain::StringRef> &ExplicitValues,
  ...
  if (!ExplicitlySpecifiedGlobals.empty())
    CC1Args.push_back(Args.MakeArgString(toolchain::Twine(
...
void AIX::addProfileRTLibs(const toolchain::opt::ArgList &Args,
                           toolchain::opt::ArgStringList &CmdArgs) const {
