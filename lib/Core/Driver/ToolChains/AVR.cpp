...
/// AVR Toolchain
AVRToolChain::AVRToolChain(const Driver &D, const toolchain::Triple &Triple,
                           const ArgList &Args)
...
void AVRToolChain::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  if (toolchain::sys::fs::is_directory(AVRInc))
...
std::string AVRToolChain::getCompilerRT(const toolchain::opt::ArgList &Args,
                                        StringRef Component,
  ...
  toolchain::sys::path::append(Path, "avr");
  toolchain::sys::path::append(Path, File.str());
...
void AVR::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (LinkStdlib) {
    ...
    if (RtLib == ToolChain::RLT_CompilerRT) {
      ...
      if (toolchain::sys::fs::exists(RtLib))
