...
void openbsd::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Arch == toolchain::Triple::mips64)
    CmdArgs.push_back("-EB");
  else if (Arch == toolchain::Triple::mips64el)
    CmdArgs.push_back("-EL");
...
OpenBSD::OpenBSD(const Driver &D, const toolchain::Triple &Triple,
                 const ArgList &Args)
...
void OpenBSD::addLibCxxIncludePaths(const toolchain::opt::ArgList &DriverArgs,
                                    toolchain::opt::ArgStringList &CC1Args) const {
...
std::string OpenBSD::getCompilerRT(const ArgList &Args, StringRef Component,
  ...
  if (Component == "builtins") {
    ...
    toolchain::sys::path::append(Path, "/usr/lib/libcompiler_rt.a");
  ...
  toolchain::sys::path::append(P, "lib", CRTBasename);
