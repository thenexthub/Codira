...
#include "language/Core/Config/config.h" // for GCC_INSTALL_PREFIX
...
void tools::gcc::Common::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (const auto &II : Inputs) {
    ...
    if (types::isLLVMIR(II.getType()))
      D.Diag(language::Core::diag::err_drv_no_linker_toolchain_support)
...
void tools::gnutools::StaticLibTool::ConstructJob(
  ...
  // ar tool command "toolchain-ar <options> <output_file> <input_files>".
  ArgStringList CmdArgs;
...
void tools::gnutools::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const toolchain::Triple::ArchType Arch = ToolChain.getArch();
  ...
  const bool HasCRTBeginEndFiles =
      ...
      (ToolChain.getTriple().getVendor() != toolchain::Triple::MipsTechnologies);
  ...
  if (Triple.isARM() || Triple.isThumb()) {
    ...
    CmdArgs.push_back(Arch == toolchain::Triple::aarch64_be ? "-EB" : "-EL");
  }

  // Most Android ARM64 targets should enable the linker fix for erratum
  // 843419. Only non-Cortex-A53 devices are allowed to skip this flag.
  if (Arch == toolchain::Triple::aarch64 && (isAndroid || isOHOSFamily) &&
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_r)) {
    if (!Args.hasArg(options::OPT_nodefaultlibs)) {
      ...
      if (getToolChain().getTriple().getArch() == toolchain::Triple::sparc) {
...
void tools::gnutools::Assembler::ConstructJob(Compilation &C,
  ...
  case toolchain::Triple::thumbeb: {
    const toolchain::Triple &Triple2 = getToolChain().getTriple();
    CmdArgs.push_back(arm::isARMBigEndian(Triple2, Args) ? "-EB" : "-EL");
    switch (Triple2.getSubArch()) {
    case toolchain::Triple::ARMSubArch_v7:
      CmdArgs.push_back("-mfpu=neon");
      break;
    case toolchain::Triple::ARMSubArch_v8:
    ...
    case arm::FloatABI::Invalid: toolchain_unreachable("must have an ABI!");
  ...
  case toolchain::Triple::aarch64_be: {
    CmdArgs.push_back(
        getToolChain().getArch() == toolchain::Triple::aarch64_be ? "-EB" : "-EL");
    Args.AddLastArg(CmdArgs, options::OPT_march_EQ);
    normalizeCPUNamesForAssembler(Args, CmdArgs);
...
public:
  FilterNonExistent(StringRef Base, StringRef File, toolchain::vfs::FileSystem &VFS)
...
static bool isArmOrThumbArch(toolchain::Triple::ArchType Arch) {
  return Arch == toolchain::Triple::arm || Arch == toolchain::Triple::thumb;
}

static bool isMipsEL(toolchain::Triple::ArchType Arch) {
  return Arch == toolchain::Triple::mipsel || Arch == toolchain::Triple::mips64el;
}
...
static bool isMSP430(toolchain::Triple::ArchType Arch) {
  return Arch == toolchain::Triple::msp430;
}
...
bool language::Core::driver::findMIPSMultilibs(const Driver &D,
  ...
  toolchain::Triple::ArchType TargetArch = TargetTriple.getArch();
  ...
  if (TargetTriple.getVendor() == toolchain::Triple::MipsTechnologies &&
      TargetTriple.getOS() == toolchain::Triple::Linux &&
      TargetTriple.getEnvironment() == toolchain::Triple::UnknownEnvironment)
  ...
  if (TargetTriple.getVendor() == toolchain::Triple::ImaginationTechnologies &&
      TargetTriple.getOS() == toolchain::Triple::Linux &&
      TargetTriple.isGNUEnvironment())
...
static void findAndroidArmMultilibs(const Driver &D,
  ...
  Multilib::flags_list Flags;
  toolchain::StringRef Arch = Args.getLastArgValue(options::OPT_march_EQ);
  bool IsArmArch = TargetTriple.getArch() == toolchain::Triple::arm;
  bool IsThumbArch = TargetTriple.getArch() == toolchain::Triple::thumb;
  bool IsV7SubArch = TargetTriple.getSubArch() == toolchain::Triple::ARMSubArch_v7;
  bool IsThumbMode = IsThumbArch ||
      Args.hasFlag(options::OPT_mthumb, options::OPT_mno_thumb, false) ||
      (IsArmArch && toolchain::ARM::parseArchISA(Arch) == toolchain::ARM::ISAKind::THUMB);
  bool IsArmV7Mode = (IsArmArch || IsThumbArch) &&
      (toolchain::ARM::parseArchVersion(Arch) == 7 ||
...
static void findCSKYMultilibs(const Driver &D, const toolchain::Triple &TargetTriple,
                              StringRef Path, const ArgList &Args,
                              DetectedMultilibs &Result) {
  FilterNonExistent NonExistent(Path, "/crtbegin.o", D.getVFS());

  tools::csky::FloatABI TheFloatABI = tools::csky::getCSKYFloatABI(D, Args);
  std::optional<toolchain::StringRef> Res =
...
selectRISCVMultilib(const Driver &D, const MultilibSet &RISCVMultilibSet,
                    ...
                    toolchain::SmallVectorImpl<Multilib> &SelectedMultilibs) {
  ...
  toolchain::Expected<std::unique_ptr<toolchain::RISCVISAInfo>> ParseResult =
      toolchain::RISCVISAInfo::parseArchString(
          Arch, /*EnableExperimentalExtension=*/true,
          /*ExperimentalExtensionVersionCheck=*/false);
  // Ignore any error here, we assume it will be handled in another place.
  if (toolchain::errorToBool(ParseResult.takeError()))
    return false;
  ...
  toolchain::StringSet<> AllArchExts;
  ...
  for (const auto &M : RISCVMultilibSet) {
    ...
    for (StringRef Flag : M.flags()) {
      ...
      toolchain::Expected<std::unique_ptr<toolchain::RISCVISAInfo>> MLConfigParseResult =
          toolchain::RISCVISAInfo::parseArchString(
              Flag, /*EnableExperimentalExtension=*/true,
              /*ExperimentalExtensionVersionCheck=*/false);
      // Ignore any error here, we assume it will handled in another place.
      if (toolchain::errorToBool(MLConfigParseResult.takeError())) {
...
static void findRISCVBareMetalMultilibs(const Driver &D,
  ...
  Multilib::flags_list Flags;
  toolchain::StringSet<> Added_ABIs;
...
static void findRISCVMultilibs(const Driver &D,
                               const toolchain::Triple &TargetTriple, StringRef Path,
                               const ArgList &Args, DetectedMultilibs &Result) {
  if (TargetTriple.getOS() == toolchain::Triple::UnknownOS)
  ...
  Multilib::flags_list Flags;
  bool IsRV64 = TargetTriple.getArch() == toolchain::Triple::riscv64;
  StringRef ABIName = tools::riscv::getRISCVABI(Args, TargetTriple);
...
static toolchain::StringRef getGCCToolchainDir(const ArgList &Args,
                                          toolchain::StringRef SysRoot) {
  const Arg *A = Args.getLastArg(language::Core::driver::options::OPT_gcc_toolchain);
  if (A)
    return A->getValue();
...
void Generic_GCC::GCCInstallationDetector::init(
    const toolchain::Triple &TargetTriple, const ArgList &Args) {
  toolchain::Triple BiarchVariantTriple = TargetTriple.isArch32Bit()
  ...
  if (TargetTriple.getVendor() == toolchain::Triple::UnknownVendor) {
    StringRef OSEnv = TargetTriple.getOSAndEnvironmentName();
    if (TargetTriple.getEnvironment() == toolchain::Triple::GNUX32)
      OSEnv = "linux-gnu";
    TripleNoVendor = (TargetTriple.getArchName().str() + '-' + OSEnv).str();
    CandidateTripleAliases.push_back(TripleNoVendor);
    if (BiarchVariantTriple.getArch() != toolchain::Triple::UnknownArch) {
  ...
  if (const Arg *A =
          Args.getLastArg(language::Core::driver::options::OPT_gcc_install_dir_EQ);
      A && A->getValue()[0]) {
    ...
    if (!ScanGCCForMultilibs(TargetTriple, Args, InstallDir, false)) {
      ...
      StringRef VersionText = toolchain::sys::path::filename(InstallDir);
      StringRef TripleText =
          toolchain::sys::path::filename(toolchain::sys::path::parent_path(InstallDir));
  ...
  if (const Arg *A =
          Args.getLastArg(language::Core::driver::options::OPT_gcc_triple_EQ)) {
...
void Generic_GCC::GCCInstallationDetector::AddDefaultGCCPrefixes(
    const toolchain::Triple &TargetTriple, SmallVectorImpl<std::string> &Prefixes,
    StringRef SysRoot) {
  ...
  if (TargetTriple.isOSSolaris()) {
    ...
    std::error_code EC;
    for (toolchain::vfs::directory_iterator LI = D.getVFS().dir_begin(PrefixDir, EC),
                                       LE;
  ...
  // and gcc-toolsets.
  if (SysRoot.empty() && TargetTriple.getOS() == toolchain::Triple::Linux &&
...
/*static*/ void Generic_GCC::GCCInstallationDetector::CollectLibDirsAndTriples(
    const toolchain::Triple &TargetTriple, const toolchain::Triple &BiarchTriple,
  ...
  case toolchain::Triple::thumb:
    LibDirs.append(begin(ARMLibDirs), end(ARMLibDirs));
    if (TargetTriple.getEnvironment() == toolchain::Triple::GNUEABIHF ||
        TargetTriple.getEnvironment() == toolchain::Triple::GNUEABIHFT64 ||
        TargetTriple.getEnvironment() == toolchain::Triple::MuslEABIHF ||
        TargetTriple.getEnvironment() == toolchain::Triple::EABIHF) {
...
bool Generic_GCC::GCCInstallationDetector::ScanGCCForMultilibs(
  ...
  } else if (TargetArch == toolchain::Triple::avr) {
...
void Generic_GCC::GCCInstallationDetector::ScanLibDirForGCCTriple(
  ...
  struct GCCLibSuffix {
    ...
    bool Active;
      ...
      {CandidateTriple.str(), "..",
       TargetTriple.getVendor() == toolchain::Triple::Freescale ||
           TargetTriple.getVendor() == toolchain::Triple::OpenEmbedded}};
  ...
  for (auto &Suffix : Suffixes) {
    ...
    std::error_code EC;
    for (toolchain::vfs::directory_iterator
...
bool Generic_GCC::GCCInstallationDetector::ScanGentooGccConfig(
  ...
  if (File) {
    ...
    for (StringRef Line : Lines) {
      ...
      toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> ConfigFile =
...
Generic_GCC::Generic_GCC(const Driver &D, const toolchain::Triple &Triple,
                         const ArgList &Args)
...
Tool *Generic_GCC::getTool(Action::ActionClass AC) const {
  ...
  case Action::PreprocessJobClass:
    if (!Preprocess)
      Preprocess.reset(new language::Core::driver::tools::gcc::Preprocessor(*this));
...
bool Generic_GCC::isPIEDefault(const toolchain::opt::ArgList &Args) const {
  return false;
}

bool Generic_GCC::isPICDefaultForced() const {
  return getArch() == toolchain::Triple::x86_64 && getTriple().isOSWindows();
}
...
void Generic_GCC::AddMultilibPaths(const Driver &D,
  ...
  if (GCCInstallation.isValid()) {
    ...
    const toolchain::Triple &GCCTriple = GCCInstallation.getTriple();
...
void
Generic_GCC::addLibCxxIncludePaths(const toolchain::opt::ArgList &DriverArgs,
                                   toolchain::opt::ArgStringList &CC1Args) const {
  const Driver &D = getDriver();
  std::string SysRoot = computeSysRoot();
  if (SysRoot.empty())
    SysRoot = toolchain::sys::path::get_separator();
  ...
  auto AddIncludePath = [&](StringRef Path, bool TargetDirRequired = false) {
    ...
    if (TargetIncludeDir) {
      ...
      toolchain::sys::path::append(TargetDir, "c++", Version);
    ...
    toolchain::sys::path::append(GenericDir, "c++", Version);
  ...
  toolchain::sys::path::append(DriverIncludeDir, "..", "include");
  ...
  toolchain::sys::path::append(UsrLocalIncludeDir, "usr", "local", "include");
  if (AddIncludePath(UsrLocalIncludeDir))
    return;
  SmallString<128> UsrIncludeDir(SysRoot);
  toolchain::sys::path::append(UsrIncludeDir, "usr", "include");
...
bool Generic_GCC::addLibStdCXXIncludePaths(Twine IncludeDir, StringRef Triple,
  ...
  StringRef Include =
      toolchain::sys::path::parent_path(toolchain::sys::path::parent_path(Dir));
  std::string Path =
...
void
Generic_GCC::addLibStdCxxIncludePaths(const toolchain::opt::ArgList &DriverArgs,
                                      toolchain::opt::ArgStringList &CC1Args) const {
  if (GCCInstallation.isValid()) {
...
Generic_GCC::TranslateArgs(const toolchain::opt::DerivedArgList &Args,
                           StringRef BoundArch,
  ...
  const toolchain::DenseSet<unsigned> OpenMPFiltered{
