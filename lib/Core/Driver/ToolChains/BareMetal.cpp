...
static bool isRISCVBareMetal(const toolchain::Triple &Triple) {
  if (!Triple.isRISCV())
    return false;
...
static bool isPPCBareMetal(const toolchain::Triple &Triple) {
  return Triple.isPPC() && Triple.getOS() == toolchain::Triple::UnknownOS &&
         Triple.getEnvironment() == toolchain::Triple::EABI;
}
...
static std::string computeClangRuntimesSysRoot(const Driver &D,
  ...
  toolchain::sys::path::append(SysRootDir, "..", "lib", "clang-runtimes");

  if (IncludeTriple)
    toolchain::sys::path::append(SysRootDir, D.getTargetTriple());
...
bool BareMetal::initGCCInstallation(const toolchain::Triple &Triple,
...
static bool detectGCCToolchainAdjacent(const Driver &D) {
  ...
  toolchain::sys::path::append(GCCDir, D.Dir, "..", D.getTargetTriple(),
                          "lib/crt0.o");
  return toolchain::sys::fs::exists(GCCDir);
}
...
std::string BareMetal::computeSysRoot() const {
  ...
  if (IsGCCInstallationValid) {
    toolchain::sys::path::append(inferredSysRoot, GCCInstallation.getParentLibPath(),
                            "..", GCCInstallation.getTriple().str());
  } else if (detectGCCToolchainAdjacent(D)) {
    // Use the triple as provided to the driver. Unlike the parsed triple
    // this has not been normalized to always contain every field.
    toolchain::sys::path::append(inferredSysRoot, D.Dir, "..", D.getTargetTriple());
  }
  // If a valid sysroot was inferred and exists, use it
  if (!inferredSysRoot.empty() && toolchain::sys::fs::exists(inferredSysRoot))
...
// in GCCInstallation.
BareMetal::BareMetal(const Driver &D, const toolchain::Triple &Triple,
                     const ArgList &Args)
  ...
  if (IsGCCInstallationValid) {
    if (!isRISCVBareMetal(Triple))
      D.Diag(language::Core::diag::warn_drv_multilib_not_available_for_target);
    ...
    if (!SysRootDir.empty()) {
      for (const Multilib &M : getOrderedMultilibs()) {
        ...
        toolchain::sys::path::append(Dir, M.osSuffix(), "lib");
...
findMultilibsFromYAML(const ToolChain &TC, const Driver &D,
  ...
  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> MB =
      D.getVFS().getBufferForFile(MultilibPath);
  if (!MB)
    return;
  Multilib::flags_list Flags = TC.getMultilibFlags(Args);
  toolchain::ErrorOr<MultilibSet> ErrorOrMultilibSet =
  ...
  if (Result.Multilibs.select(D, Flags, Result.SelectedMultilibs,
    ...
    return;
  D.Diag(language::Core::diag::warn_drv_missing_multilib) << toolchain::join(Flags, " ");
  std::stringstream ss;
  ...
  for (const Multilib &Multilib : Result.Multilibs)
    if (!Multilib.isError())
      ss << "\n" << toolchain::join(Multilib.flags(), " ");
  D.Diag(language::Core::diag::note_drv_available_multilibs) << ss.str();
  ...
  for (const Multilib &Multilib : Result.SelectedMultilibs)
    if (Multilib.isError())
      D.Diag(language::Core::diag::err_drv_multilib_custom_error)
...
static constexpr toolchain::StringLiteral MultilibFilename = "multilib.yaml";

static std::optional<toolchain::SmallString<128>>
getMultilibConfigPath(const Driver &D, const toolchain::Triple &Triple,
                      const ArgList &Args) {
  toolchain::SmallString<128> MultilibPath;
  if (Arg *ConfigFileArg = Args.getLastArg(options::OPT_multi_lib_config)) {
    MultilibPath = ConfigFileArg->getValue();
    if (!D.getVFS().exists(MultilibPath)) {
      D.Diag(language::Core::diag::err_drv_no_such_file) << MultilibPath.str();
      return {};
    }
  } else {
    MultilibPath = computeClangRuntimesSysRoot(D, /*IncludeTriple=*/false);
    toolchain::sys::path::append(MultilibPath, MultilibFilename);
  }
  return MultilibPath;
}

void BareMetal::findMultilibs(const Driver &D, const toolchain::Triple &Triple,
                              const ArgList &Args) {
  DetectedMultilibs Result;
  // Look for a multilib.yaml before trying target-specific hardwired logic.
  // If it exists, always do what it specifies.
  std::optional<toolchain::SmallString<128>> MultilibPath =
...
bool BareMetal::handlesTarget(const toolchain::Triple &Triple) {
...
BareMetal::OrderedMultilibs BareMetal::getOrderedMultilibs() const {
  ...
  if (!SelectedMultilibs.empty())
    return toolchain::reverse(SelectedMultilibs);

  // No multilibs selected so return a single default multilib.
  static const toolchain::SmallVector<Multilib> Default = {Multilib()};
  return toolchain::reverse(Default);
}
...
BareMetal::GetUnwindLibType(const toolchain::opt::ArgList &Args) const {
...
void BareMetal::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  if (!SysRootDir.empty()) {
    for (const Multilib &M : getOrderedMultilibs()) {
      SmallString<128> Dir(SysRootDir);
      toolchain::sys::path::append(Dir, M.includeSuffix());
      toolchain::sys::path::append(Dir, "include");
      addSystemInclude(DriverArgs, CC1Args, Dir.str());
    }
  }
...
void BareMetal::AddClangCXXStdlibIncludeArgs(const ArgList &DriverArgs,
  ...
  auto AddCXXIncludePath = [&](StringRef Path) {
...

    {
      ...
      toolchain::sys::path::append(TargetDir, Target, "c++", Version);
...

    {
      ...
      toolchain::sys::path::append(Dir, "c++", Version);
  ...
  for (const Multilib &M : getOrderedMultilibs()) {
    SmallString<128> Dir(SysRootDir);
    toolchain::sys::path::append(Dir, M.gccSuffix());
    switch (GetCXXStdlibType(DriverArgs)) {
    case ToolChain::CST_Libcxx: {
      // First check sysroot/usr/include/c++/v1 if it exists.
      SmallString<128> TargetDir(Dir);
      toolchain::sys::path::append(TargetDir, "usr", "include", "c++", "v1");
      if (D.getVFS().exists(TargetDir)) {
        addSystemInclude(DriverArgs, CC1Args, TargetDir.str());
        break;
      }
      // Add generic path if nothing else succeeded so far.
      toolchain::sys::path::append(Dir, "include", "c++", "v1");
      addSystemInclude(DriverArgs, CC1Args, Dir.str());
      break;
    }
    case ToolChain::CST_Libstdcxx: {
      toolchain::sys::path::append(Dir, "include", "c++");
      std::error_code EC;
      ...
      if (Version.Major != -1) {
        toolchain::sys::path::append(Dir, Version.Text);
        addSystemInclude(DriverArgs, CC1Args, Dir.str());
      }
      break;
    }
...
void baremetal::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (const auto &LibPath : TC.getLibraryPaths())
    CmdArgs.push_back(Args.MakeArgString(toolchain::Twine("-L", LibPath)));
...
BareMetal::getMultilibMacroDefinesStr(toolchain::opt::ArgList &Args) const {
