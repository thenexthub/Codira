...
toolchains::PS4PS5Base::PS4PS5Base(const Driver &D, const toolchain::Triple &Triple,
  ...
  auto OverrideRoot = [&](const options::ID &Opt, std::string &Root,
    ...
    if (const Arg *A = Args.getLastArg(Opt)) {
      Root = A->getValue();
      if (!toolchain::sys::fs::exists(Root))
        D.Diag(language::Core::diag::warn_missing_sysroot) << Root;
      return true;
    }
    Root = Default.str();
    return false;
  ...
  auto CheckSDKPartExists = [&](StringRef Dir, StringRef Desc) {
    if (toolchain::sys::fs::exists(Dir))
      return true;
    D.Diag(language::Core::diag::warn_drv_unable_to_find_directory_expected)
  ...
  if (Linking) {
    ...
    toolchain::sys::path::append(Dir, "target/lib");
  ...
  if (!CustomSysroot && !CustomISysroot &&
    ...
    toolchain::sys::path::append(Dir, "target/include");
...
void toolchains::PS4PS5Base::addClangTargetOptions(
  ...
  if (DriverArgs.hasArg(options::OPT_fuse_init_array)) {
    ...
    getDriver().Diag(language::Core::diag::err_drv_unsupported_opt_for_target)
...
// PS4 toolchain.
toolchains::PS4CPU::PS4CPU(const Driver &D, const toolchain::Triple &Triple,
                           const toolchain::opt::ArgList &Args)
    : PS4PS5Base(D, Triple, Args, "PS4", "SCE_ORBIS_SDK_DIR") {}

// PS5 toolchain.
toolchains::PS5CPU::PS5CPU(const Driver &D, const toolchain::Triple &Triple,
                           const toolchain::opt::ArgList &Args)
