...
#include "Darwin.h"
#include "Arch/ARM.h"
#include "language/Core/Basic/AlignedAllocation.h"
...
#include "toolchain/ProfileData/MemProf.h"
...
toolchain::Triple::ArchType darwin::getArchTypeForMachOArchName(StringRef Str) {
  // See arch(3) and toolchain-gcc's driver-driver.c. We don't implement support for
  ...
  return toolchain::StringSwitch<toolchain::Triple::ArchType>(Str)
      .Cases("i386", "i486", "i486SX", "i586", "i686", toolchain::Triple::x86)
      .Cases("pentium", "pentpro", "pentIIm3", "pentIIm5", "pentium4",
             toolchain::Triple::x86)
      .Cases("x86_64", "x86_64h", toolchain::Triple::x86_64)
      // This is derived from the driver.
      .Cases("arm", "armv4t", "armv5", "armv6", "armv6m", toolchain::Triple::arm)
      .Cases("armv7", "armv7em", "armv7k", "armv7m", toolchain::Triple::arm)
      .Cases("armv7s", "xscale", toolchain::Triple::arm)
      .Cases("arm64", "arm64e", toolchain::Triple::aarch64)
      .Case("arm64_32", toolchain::Triple::aarch64_32)
      .Case("r600", toolchain::Triple::r600)
      .Case("amdgcn", toolchain::Triple::amdgcn)
      .Case("nvptx", toolchain::Triple::nvptx)
      .Case("nvptx64", toolchain::Triple::nvptx64)
      .Case("amdil", toolchain::Triple::amdil)
      .Case("spir", toolchain::Triple::spir)
      .Default(toolchain::Triple::UnknownArch);
}

void darwin::setTripleTypeForMachOArchName(toolchain::Triple &T, StringRef Str,
                                           const ArgList &Args) {
  const toolchain::Triple::ArchType Arch = getArchTypeForMachOArchName(Str);
  toolchain::ARM::ArchKind ArchKind = toolchain::ARM::parseArch(Str);
  T.setArch(Arch);
  if (Arch != toolchain::Triple::UnknownArch)
    T.setArchName(Str);

  if (ArchKind == toolchain::ARM::ArchKind::ARMV6M ||
      ArchKind == toolchain::ARM::ArchKind::ARMV7M ||
      ArchKind == toolchain::ARM::ArchKind::ARMV7EM) {
    // Don't reject these -version-min= if we have the appropriate triple.
    if (T.getOS() == toolchain::Triple::IOS)
      for (Arg *A : Args.filtered(options::OPT_mios_version_min_EQ))
        A->ignoreTargetSpecific();
    if (T.getOS() == toolchain::Triple::WatchOS)
      for (Arg *A : Args.filtered(options::OPT_mwatchos_version_min_EQ))
        A->ignoreTargetSpecific();
    if (T.getOS() == toolchain::Triple::TvOS)
      for (Arg *A : Args.filtered(options::OPT_mtvos_version_min_EQ))
        A->ignoreTargetSpecific();

    T.setOS(toolchain::Triple::UnknownOS);
    T.setObjectFormat(toolchain::Triple::MachO);
  }
}
...
void darwin::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (getToolChain().getArch() != toolchain::Triple::x86_64 &&
...
void darwin::Linker::AddLinkArgs(Compilation &C, const ArgList &Args,
  ...
  if (Version >= VersionTuple(133) && !LinkerIsLLD) {
    ...
    StringRef P = toolchain::sys::path::parent_path(D.Dir);
...
static void AppendPlatformPrefix(SmallString<128> &Path, const toolchain::Triple &T);
...
void darwin::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  toolchain::opt::ArgStringList InputFileList;
  ...
  if (!Parallelism.empty()) {
    CmdArgs.push_back("-mtoolchain");
    unsigned NumThreads =
        toolchain::get_threadpool_strategy(Parallelism)->compute_thread_count();
    ...
    if (NonStandardSearchPath) {
      if (auto *Sysroot = Args.getLastArg(options::OPT_isysroot)) {
        auto AddSearchPath = [&](StringRef Flag, StringRef SearchPath) {
          ...
          toolchain::sys::path::append(P, SearchPath);
  ...
  if (Version >= VersionTuple(705) || LinkerIsLLD) {
    ...
    ResponseSupport = {ResponseFileSupport::RF_FileList, toolchain::sys::WEM_UTF8,
                       "-filelist"};
  }
...
void darwin::StaticLibTool::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Output.isFilename() && toolchain::sys::fs::exists(OutputFileName)) {
    if (std::error_code EC = toolchain::sys::fs::remove(OutputFileName)) {
...
MachO::MachO(const Driver &D, const toolchain::Triple &Triple, const ArgList &Args)
    : ToolChain(D, Triple, Args) {
  // We expect 'as', 'ld', etc. to be adjacent to our install dir.
  getProgramPaths().push_back(getDriver().Dir);
}

AppleMachO::AppleMachO(const Driver &D, const toolchain::Triple &Triple,
                       const ArgList &Args)
    : MachO(D, Triple, Args), CudaInstallation(D, Triple, Args),
      RocmInstallation(D, Triple, Args), SYCLInstallation(D, Triple, Args) {}

/// Darwin - Darwin tool chain for i386 and x86_64.
Darwin::Darwin(const Driver &D, const toolchain::Triple &Triple, const ArgList &Args)
...
ObjCRuntime Darwin::getDefaultObjCRuntime(bool isNonFragile) const {
  ...
  if (isTargetXROS()) {
    ...
    auto T = toolchain::Triple(Twine("arm64-apple-") +
                          toolchain::Triple::getOSTypeName(toolchain::Triple::XROS) +
...
static const char *ArmMachOArchName(StringRef Arch) {
  return toolchain::StringSwitch<const char *>(Arch)
      .Case("armv6k", "armv6")
      .Case("armv6m", "armv6m")
      .Case("armv5tej", "armv5")
      .Case("xscale", "xscale")
...
static const char *ArmMachOArchNameCPU(StringRef CPU) {
  ...
  if (ArchKind == toolchain::ARM::ArchKind::INVALID)
    return nullptr;
  StringRef Arch = toolchain::ARM::getArchName(ArchKind);
...
VersionTuple MachO::getLinkerVersion(const toolchain::opt::ArgList &Args) const {
  if (LinkerVersion) {
#ifndef NDEBUG
...
std::string Darwin::ComputeEffectiveClangTriple(const ArgList &Args,
  ...
  toolchain::Triple Triple(ComputeLLVMTriple(Args, InputType));
  ...
  if (isTargetWatchOSBased())
    ...
    Str += toolchain::Triple::getOSTypeName(toolchain::Triple::XROS);
  else
    Str += "macosx";
...
DarwinClang::DarwinClang(const Driver &D, const toolchain::Triple &Triple,
                         const ArgList &Args)
...
static StringRef getXcodeDeveloperPath(StringRef PathIntoXcode) {
  static constexpr toolchain::StringLiteral XcodeAppSuffix(
...
void DarwinClang::AddLinkARCArgs(const ArgList &Args,
  ...
  if (isTargetMacOSBased() && getArch() == toolchain::Triple::x86)
    return;
  ...
  toolchain::sys::path::remove_filename(P); // 'clang'
  toolchain::sys::path::remove_filename(P); // 'bin'
  toolchain::sys::path::append(P, "lib", "arc");
  ...
  if (!getVFS().exists(P)) {
    auto updatePath = [&](const Arg *A) {
      ...
      P = XcodePathForSDK;
      toolchain::sys::path::append(P, "Toolchains/XcodeDefault.xctoolchain/usr",
                              "lib", "arc");
  ...
  toolchain::sys::path::append(P, "libarclite_");
  ...
  if (!getVFS().exists(P))
    getDriver().Diag(language::Core::diag::err_drv_darwin_sdk_missing_arclite) << P;
...
unsigned DarwinClang::GetDefaultDwarfVersion() const {
  ...
  if ((isTargetMacOSBased() && isMacosxVersionLT(15)) ||
      ...
      (isTargetWatchOSBased() && TargetVersion < toolchain::VersionTuple(11)) ||
      (isTargetXROS() && TargetVersion < toolchain::VersionTuple(2)) ||
      (isTargetDriverKit() && TargetVersion < toolchain::VersionTuple(24)) ||
      (isTargetMacOSBased() &&
...
void MachO::AddLinkRuntimeLib(const ArgList &Args, ArgStringList &CmdArgs,
  ...
  if (Opts & RLO_AddRPath) {
    ...
    CmdArgs.push_back("-rpath");
    CmdArgs.push_back(Args.MakeArgString(toolchain::sys::path::parent_path(P)));
  }
}
...
std::string MachO::getCompilerRT(const ArgList &, StringRef Component,
  ...
  toolchain::sys::path::append(FullPath, "lib", "darwin", "macho_embedded",
                          MachOLibName);
...
std::string Darwin::getCompilerRT(const ArgList &, StringRef Component,
  ...
  SmallString<128> FullPath(getDriver().ResourceDir);
  toolchain::sys::path::append(FullPath, "lib", "darwin", DarwinLibName);
  return std::string(FullPath);
}
...
StringRef Darwin::getPlatformFamily() const {
  ...
  toolchain_unreachable("Unsupported platform");
}

StringRef Darwin::getSDKName(StringRef isysroot) {
  // Assume SDK has path: SOME_PATH/SDKs/PlatformXX.YY.sdk
  auto BeginSDK = toolchain::sys::path::rbegin(isysroot);
  auto EndSDK = toolchain::sys::path::rend(isysroot);
...
void Darwin::addProfileRTLibs(const ArgList &Args,
  ...
  // Align __toolchain_prf_{cnts,bits,data} sections to the maximum expected page
  // alignment. This allows profile counters to be mmap()'d to disk. Note that
  // it's not enough to just page-align __toolchain_prf_cnts: the following section
  ...
  // enabled.
  if (!ForGCOV) {
    for (auto IPSK : {toolchain::IPSK_cnts, toolchain::IPSK_bitmap, toolchain::IPSK_data}) {
      addSectalignToPage(
          Args, CmdArgs, "__DATA",
          toolchain::getInstrProfSectionName(IPSK, toolchain::Triple::MachO,
...
ToolChain::RuntimeLibType DarwinClang::GetRuntimeLibType(
  ...
  if (Arg* A = Args.getLastArg(options::OPT_rtlib_EQ)) {
    ...
    if (Value != "compiler-rt" && Value != "platform")
      getDriver().Diag(language::Core::diag::err_drv_unsupported_rtlib_for_platform)
          << Value << "darwin";
  }
...
void DarwinClang::AddLinkRuntimeLibArgs(const ArgList &Args,
  ...
  if (Sanitize.needsMemProfRt())
    if (hasExportSymbolDirective(Args))
      addExportedSymbol(
          CmdArgs,
          toolchain::memprof::getMemprofOptionsSymbolDarwinLinkageName().data());
  ...
  if (getTriple().getArch() != toolchain::Triple::aarch64 &&
...
static std::string getSystemOrSDKMacOSVersion(StringRef MacOSSDKVersion) {
  toolchain::Triple SystemTriple(toolchain::sys::getProcessTriple());
...
struct DarwinPlatform {
  ...
  bool isValidOSVersion() const {
    return toolchain::Triple::isValidVersionForOS(getOSFromPlatform(Platform),
                                             getOSVersion());
  }

  VersionTuple getCanonicalOSVersion() const {
    return toolchain::Triple::getCanonicalVersionForOS(
  ...
  const std::optional<toolchain::Triple> &getTargetVariantTriple() const {
  ...
  std::string getAsString(DerivedArgList &Args, const OptTable &Opts) {
    ...
    case DeploymentTargetEnv:
      return (toolchain::Twine(EnvVarName) + "=" + OSVersionStr).str();
    case InferredFromSDK:
    case InferredFromArch:
      toolchain_unreachable("Cannot print arguments for inferred OS version");
    }
    toolchain_unreachable("Unsupported Darwin Source Kind");
  }
  ...
  void setEnvironment(toolchain::Triple::EnvironmentType EnvType,
                      const VersionTuple &OSVersion,
                      const std::optional<DarwinSDKInfo> &SDKInfo) {
    switch (EnvType) {
    case toolchain::Triple::Simulator:
      Environment = DarwinEnvironmentKind::Simulator;
      break;
    case toolchain::Triple::MacABI: {
  ...
  createFromTarget(const toolchain::Triple &TT, Arg *A,
                   std::optional<toolchain::Triple> TargetVariantTriple,
                   const std::optional<DarwinSDKInfo> &SDKInfo) {
  ...
  static DarwinPlatform
  createFromMTargetOS(toolchain::Triple::OSType OS, VersionTuple OSVersion,
                      toolchain::Triple::EnvironmentType Environment, Arg *A,
                      const std::optional<DarwinSDKInfo> &SDKInfo) {
  ...
  static DarwinPlatform createFromArch(StringRef Arch, toolchain::Triple::OSType OS,
...
private:
  ...
  static DarwinPlatformKind getPlatformFromOS(toolchain::Triple::OSType OS) {
    switch (OS) {
    ...
    default:
      toolchain_unreachable("Unable to infer Darwin variant");
    }
  }

  static toolchain::Triple::OSType getOSFromPlatform(DarwinPlatformKind Platform) {
    switch (Platform) {
    case DarwinPlatformKind::MacOS:
    ...
    case DarwinPlatformKind::DriverKit:
      return toolchain::Triple::DriverKit;
    case DarwinPlatformKind::XROS:
      return toolchain::Triple::XROS;
    }
    toolchain_unreachable("Unknown DarwinPlatformKind enum");
  }
...
getDeploymentTargetFromEnvironmentVariables(const Driver &TheDriver,
  ...
  static_assert(std::size(EnvVars) == Darwin::LastDarwinPlatform + 1,
                "Missing platform");
  for (const auto &I : toolchain::enumerate(toolchain::ArrayRef(EnvVars))) {
  ...
  if (!Targets[Darwin::MacOS].empty() &&
    ...
    if (Triple.getArch() == toolchain::Triple::arm ||
  ...
  for (const auto &Target : toolchain::enumerate(toolchain::ArrayRef(Targets))) {
...
VersionTuple getInferredOSVersion(toolchain::Triple::OSType OS,
                                  const toolchain::Triple &Triple,
                                  const Driver &TheDriver) {
  ...
  case toolchain::Triple::DriverKit:
    ...
    break;
  default:
    toolchain_unreachable("Unexpected OS type");
    break;
  }
  return OsVersion;
}
...
inferDeploymentTargetFromArch(DerivedArgList &Args, const Darwin &Toolchain,
                              const toolchain::Triple &Triple,
                              const Driver &TheDriver) {
  toolchain::Triple::OSType OSTy = toolchain::Triple::UnknownOS;

  StringRef MachOArchName = Toolchain.getMachOArchName(Args);
  if (MachOArchName == "arm64" || MachOArchName == "arm64e")
    OSTy = toolchain::Triple::MacOSX;
  else if (MachOArchName == "armv7" || MachOArchName == "armv7s" ||
           MachOArchName == "armv6")
    OSTy = toolchain::Triple::IOS;
  else if (MachOArchName == "armv7k" || MachOArchName == "arm64_32")
    OSTy = toolchain::Triple::WatchOS;
  else if (MachOArchName != "armv6m" && MachOArchName != "armv7m" &&
           MachOArchName != "armv7em")
    OSTy = toolchain::Triple::MacOSX;
  if (OSTy == toolchain::Triple::UnknownOS)
    return std::nullopt;
...
std::optional<DarwinPlatform> getDeploymentTargetFromTargetArg(
    DerivedArgList &Args, const toolchain::Triple &Triple, const Driver &TheDriver,
    const std::optional<DarwinSDKInfo> &SDKInfo) {
  if (!Args.hasArg(options::OPT_target))
    return std::nullopt;
  if (Triple.getOS() == toolchain::Triple::Darwin ||
      Triple.getOS() == toolchain::Triple::UnknownOS)
    return std::nullopt;
  std::optional<toolchain::Triple> TargetVariantTriple;
  for (const Arg *A : Args.filtered(options::OPT_darwin_target_variant)) {
    toolchain::Triple TVT(A->getValue());
    ...
    if ((Triple.getArch() == toolchain::Triple::aarch64 ||
      ...
      if ((Triple.isMacOSX() && TVT.getOS() == toolchain::Triple::IOS &&
           TVT.isMacCatalystEnvironment()) ||
          (TVT.isMacOSX() && Triple.getOS() == toolchain::Triple::IOS &&
...
std::optional<DarwinPlatform> getDeploymentTargetFromMTargetOSArg(
  ...
  if (!A)
    return std::nullopt;
  toolchain::Triple TT(toolchain::Twine("unknown-apple-") + A->getValue());
  switch (TT.getOS()) {
  case toolchain::Triple::MacOSX:
  case toolchain::Triple::IOS:
  case toolchain::Triple::TvOS:
...
std::optional<DarwinSDKInfo> parseSDKSettings(toolchain::vfs::FileSystem &VFS,
                                              const ArgList &Args,
                                              const Driver &TheDriver) {
  const Arg *A = Args.getLastArg(options::OPT_isysroot);
  if (!A)
...
void Darwin::AddDeploymentTarget(DerivedArgList &Args) const {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
    ...
    if (!getVFS().exists(A->getValue()))
      getDriver().Diag(language::Core::diag::warn_missing_sysroot) << A->getValue();
  } else {
    if (char *env = ::getenv("SDKROOT")) {
      // We only use this value as the default if it is an absolute path,
      // exists, and it is not the root path.
      if (toolchain::sys::path::is_absolute(env) && getVFS().exists(env) &&
          StringRef(env) != "/") {
    ...
    if (PlatformAndVersionFromOSVersionArg) {
      ...
      if (PlatformAndVersion->getPlatform() !=
        ...
        if (PlatformAndVersion->getPlatform() ==
          ...
          getDriver().Diag(language::Core::diag::warn_drv_overriding_option)
  ...
  if (Platform == MacOS) {
    ...
    toolchain_unreachable("unknown kind of Darwin platform");
  ...
  if (TargetVariantTriple &&
      !toolchain::Triple::isValidVersionForOS(TargetVariantTriple->getOS(),
...
static void AppendPlatformPrefix(SmallString<128> &Path,
  ...
  if (T.isDriverKit()) {
    toolchain::sys::path::append(Path, "System", "DriverKit");
  }
}
...
AppleMachO::GetEffectiveSysroot(const toolchain::opt::ArgList &DriverArgs) const {
...
void AppleMachO::AddClangSystemIncludeArgs(
  ...
  toolchain::SmallString<128> Sysroot = GetEffectiveSysroot(DriverArgs);
  ...
  if (!NoStdInc && !NoStdlibInc) {
      ...
      toolchain::sys::path::append(P, "usr", "local", "include");
  ...
  toolchain::StringRef CIncludeDirs(C_INCLUDE_DIRS);
  if (!CIncludeDirs.empty()) {
    toolchain::SmallVector<toolchain::StringRef, 5> dirs;
    CIncludeDirs.split(dirs, ":");
    for (toolchain::StringRef dir : dirs) {
      toolchain::StringRef Prefix =
          toolchain::sys::path::is_absolute(dir) ? "" : toolchain::StringRef(Sysroot);
      addExternCSystemInclude(DriverArgs, CC1Args, Prefix + dir);
    }
  } else {
    // Otherwise, add <sysroot>/usr/include.
    SmallString<128> P(Sysroot);
    toolchain::sys::path::append(P, "usr", "include");
...
void DarwinClang::AddClangSystemIncludeArgs(
  ...
  toolchain::sys::path::append(P1, "System", "Library", "Frameworks");
  toolchain::sys::path::append(P2, "System", "Library", "SubFrameworks");
  toolchain::sys::path::append(P3, "Library", "Frameworks");
  addSystemFrameworkIncludes(DriverArgs, CC1Args, {P1, P2, P3});
}

bool DarwinClang::AddGnuCPlusPlusIncludePaths(const toolchain::opt::ArgList &DriverArgs,
                                              toolchain::opt::ArgStringList &CC1Args,
                                              toolchain::SmallString<128> Base,
                                              toolchain::StringRef Version,
                                              toolchain::StringRef ArchDir,
                                              toolchain::StringRef BitDir) const {
  toolchain::sys::path::append(Base, Version);
  ...
  {
    toolchain::SmallString<128> P = Base;
    if (!ArchDir.empty())
      toolchain::sys::path::append(P, ArchDir);
    if (!BitDir.empty())
      toolchain::sys::path::append(P, BitDir);
    addSystemInclude(DriverArgs, CC1Args, P);
  }
  ...
  {
    toolchain::SmallString<128> P = Base;
    toolchain::sys::path::append(P, "backward");
    addSystemInclude(DriverArgs, CC1Args, P);
  }
...
void AppleMachO::AddClangCXXStdlibIncludeArgs(
  ...
  case ToolChain::CST_Libcxx: {
    ...
    // parent_path.
    toolchain::SmallString<128> InstallBin(getDriver().Dir); // <install>/bin
    toolchain::sys::path::append(InstallBin, "..", "include", "c++", "v1");
    if (getVFS().exists(InstallBin)) {
      addSystemInclude(DriverArgs, CC1Args, InstallBin);
      return;
    } else if (DriverArgs.hasArg(options::OPT_v)) {
      toolchain::errs() << "ignoring nonexistent directory \"" << InstallBin
                   << "\"\n";
    }

    // Otherwise, check for (2)
    toolchain::SmallString<128> SysrootUsr = Sysroot;
    toolchain::sys::path::append(SysrootUsr, "usr", "include", "c++", "v1");
    if (getVFS().exists(SysrootUsr)) {
      addSystemInclude(DriverArgs, CC1Args, SysrootUsr);
      return;
    } else if (DriverArgs.hasArg(options::OPT_v)) {
      toolchain::errs() << "ignoring nonexistent directory \"" << SysrootUsr
                   << "\"\n";
    }
...
void DarwinClang::AddGnuCPlusPlusIncludePaths(
    const toolchain::opt::ArgList &DriverArgs,
    toolchain::opt::ArgStringList &CC1Args) const {
  toolchain::SmallString<128> UsrIncludeCxx = GetEffectiveSysroot(DriverArgs);
  toolchain::sys::path::append(UsrIncludeCxx, "usr", "include", "c++");

  toolchain::Triple::ArchType arch = getTriple().getArch();
  ...
  case toolchain::Triple::x86_64:
    IsBaseFound = AddGnuCPlusPlusIncludePaths(
        ...
        arch == toolchain::Triple::x86_64 ? "x86_64" : "");
    IsBaseFound |= AddGnuCPlusPlusIncludePaths(
...
void AppleMachO::AddCXXStdlibLibArgs(const ArgList &Args,
  ...
  case ToolChain::CST_Libstdcxx:
    ...
    if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
      ...
      toolchain::sys::path::append(P, "usr", "lib", "libstdc++.dylib");

      if (!getVFS().exists(P)) {
        toolchain::sys::path::remove_filename(P);
        toolchain::sys::path::append(P, "libstdc++.6.dylib");
        if (getVFS().exists(P)) {
          CmdArgs.push_back(Args.MakeArgString(P));
          return;
        }
...
void DarwinClang::AddCCKextLibArgs(const ArgList &Args,
  ...
  toolchain::sys::path::append(P, "lib", "darwin");

  // Use the newer cc_kext for iOS ARM after 6.0.
  if (isTargetWatchOS()) {
    toolchain::sys::path::append(P, "libclang_rt.cc_kext_watchos.a");
  } else if (isTargetTvOS()) {
    toolchain::sys::path::append(P, "libclang_rt.cc_kext_tvos.a");
  } else if (isTargetIPhoneOS()) {
    toolchain::sys::path::append(P, "libclang_rt.cc_kext_ios.a");
  } else if (isTargetDriverKit()) {
    // DriverKit doesn't want extra runtime support.
  } else if (isTargetXROSDevice()) {
    toolchain::sys::path::append(
        P, toolchain::Twine("libclang_rt.cc_kext_") +
               toolchain::Triple::getOSTypeName(toolchain::Triple::XROS) + ".a");
  } else {
    toolchain::sys::path::append(P, "libclang_rt.cc_kext.a");
  }
...
DerivedArgList *MachO::TranslateArgs(const DerivedArgList &Args,
  ...
  if (!BoundArch.empty()) {
    ...
    const Option MArch = Opts.getOption(language::Core::driver::options::OPT_march_EQ);
...
void MachO::AddLinkRuntimeLibArgs(const ArgList &Args,
  ...
  toolchain::SmallString<32> CompilerRT = StringRef("");
  CompilerRT +=
...
bool Darwin::isAlignedAllocationUnavailable() const {
  toolchain::Triple::OSType OS;

  if (isTargetMacCatalyst())
    return TargetVersion < alignedAllocMinVersion(toolchain::Triple::MacOSX);
  switch (TargetPlatform) {
  case MacOS: // Earlier than 10.13.
    OS = toolchain::Triple::MacOSX;
    break;
  case IPhoneOS:
    OS = toolchain::Triple::IOS;
    break;
  case TvOS: // Earlier than 11.0.
    OS = toolchain::Triple::TvOS;
    break;
  case WatchOS: // Earlier than 4.0.
    OS = toolchain::Triple::WatchOS;
    break;
...
static inline toolchain::VersionTuple
sizedDeallocMinVersion(toolchain::Triple::OSType OS) {
  switch (OS) {
  default:
    break;
  case toolchain::Triple::Darwin:
  case toolchain::Triple::MacOSX: // Earliest supporting version is 10.12.
    return toolchain::VersionTuple(10U, 12U);
  case toolchain::Triple::IOS:
  case toolchain::Triple::TvOS: // Earliest supporting version is 10.0.0.
    return toolchain::VersionTuple(10U);
  case toolchain::Triple::WatchOS: // Earliest supporting version is 3.0.0.
    return toolchain::VersionTuple(3U);
  }
...
bool Darwin::isSizedDeallocationUnavailable() const {
  ...
  if (isTargetMacCatalyst())
    return TargetVersion < sizedDeallocMinVersion(toolchain::Triple::MacOSX);
  switch (TargetPlatform) {
...
void MachO::addClangTargetOptions(const toolchain::opt::ArgList &DriverArgs,
                                  toolchain::opt::ArgStringList &CC1Args,
                                  Action::OffloadKind DeviceOffloadKind) const {
...
void Darwin::addClangCC1ASTargetOptions(
    const toolchain::opt::ArgList &Args, toolchain::opt::ArgStringList &CC1ASArgs) const {
  if (TargetVariantTriple) {
  ...
  if (SDKInfo) {
    ...
    auto EmitTargetSDKVersionArg = [&](const VersionTuple &V) {
      std::string Arg;
      toolchain::raw_string_ostream OS(Arg);
...
Darwin::TranslateArgs(const DerivedArgList &Args, StringRef BoundArch,
  ...
  if ((Arch == toolchain::Triple::arm || Arch == toolchain::Triple::thumb)) {
    if (Args.hasFlag(options::OPT_fomit_frame_pointer,
      ...
      getDriver().Diag(language::Core::diag::warn_drv_unsupported_opt_for_target)
...
ToolChain::UnwindTableLevel MachO::getDefaultUnwindTableLevel(const ArgList &Args) const {
  ...
  // targeting x86_64).
  if (getArch() == toolchain::Triple::x86_64 ||
      (GetExceptionModel(Args) != toolchain::ExceptionHandling::SjLj &&
       Args.hasFlag(options::OPT_fexceptions, options::OPT_fno_exceptions,
                    true)))
    return (getArch() == toolchain::Triple::aarch64 ||
            getArch() == toolchain::Triple::aarch64_32)
...
toolchain::ExceptionHandling Darwin::GetExceptionModel(const ArgList &Args) const {
  // Darwin uses SjLj exceptions on ARM.
  if (getTriple().getArch() != toolchain::Triple::arm &&
      getTriple().getArch() != toolchain::Triple::thumb)
    return toolchain::ExceptionHandling::None;

  // Only watchOS uses the new DWARF/Compact unwinding method.
  toolchain::Triple Triple(ComputeLLVMTriple(Args));
...
bool MachO::isPIEDefault(const toolchain::opt::ArgList &Args) const { return false; }
...
static const char *getPlatformName(Darwin::DarwinPlatformKind Platform,
  ...
  toolchain_unreachable("invalid platform");
}

void Darwin::addPlatformVersionArgs(const toolchain::opt::ArgList &Args,
...
static void addDefaultCRTLinkArgs(const Darwin &D, const ArgList &Args,
  ...
  if (D.isTargetIPhoneOS()) {
    if (D.getArch() == toolchain::Triple::aarch64)
