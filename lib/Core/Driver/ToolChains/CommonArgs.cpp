...
static bool useFramePointerForTargetByDefault(const toolchain::opt::ArgList &Args,
                                              const toolchain::Triple &Triple) {
  if (Args.hasArg(language::Core::driver::options::OPT_pg) &&
      !Args.hasArg(language::Core::driver::options::OPT_mfentry))
    return true;
  ...
  case toolchain::Triple::m68k:
    return !language::Core::driver::tools::areOptimizationsEnabled(Args);
  default:
    break;
  }
...
static bool useLeafFramePointerForTargetByDefault(const toolchain::Triple &Triple) {
...
static bool mustUseNonLeafFramePointerForTarget(const toolchain::Triple &Triple) {
  switch (Triple.getArch()) {
  default:
    return false;
  case toolchain::Triple::arm:
...
static bool mustMaintainValidFrameChain(const toolchain::opt::ArgList &Args,
                                        const toolchain::Triple &Triple) {
  switch (Triple.getArch()) {
  default:
    return false;
...
static bool framePointerImpliesLeafFramePointer(const toolchain::opt::ArgList &Args,
                                                const toolchain::Triple &Triple) {
...
getFramePointerKind(const toolchain::opt::ArgList &Args,
                    const toolchain::Triple &Triple) {
  ...
  bool EnableFP =
      ...
      Args.hasFlag(language::Core::driver::options::OPT_fno_omit_frame_pointer,
                   language::Core::driver::options::OPT_fomit_frame_pointer, DefaultFP);
  ...
  bool EnableLeafFP = Args.hasFlag(
      language::Core::driver::options::OPT_mno_omit_leaf_frame_pointer,
      language::Core::driver::options::OPT_momit_leaf_frame_pointer, DefaultLeafFP);

  bool FPRegReserved = EnableFP || mustMaintainValidFrameChain(Args, Triple);

  if (EnableFP) {
    if (EnableLeafFP)
      return language::Core::CodeGenOptions::FramePointerKind::All;
    return language::Core::CodeGenOptions::FramePointerKind::NonLeaf;
  }
  if (FPRegReserved)
    return language::Core::CodeGenOptions::FramePointerKind::Reserved;
  return language::Core::CodeGenOptions::FramePointerKind::None;
}
...
static bool shouldIgnoreUnsupportedTargetFeature(const Arg &TargetFeatureArg,
  ...
  if (!T.isAMDGPU())
    return false;
  auto GPUKind = T.isAMDGCN() ? toolchain::AMDGPU::parseArchAMDGCN(Processor)
                              : toolchain::AMDGPU::parseArchR600(Processor);
  auto GPUFeatures = T.isAMDGCN() ? toolchain::AMDGPU::getArchAttrAMDGCN(GPUKind)
                                  : toolchain::AMDGPU::getArchAttrR600(GPUKind);
  if (GPUFeatures & toolchain::AMDGPU::FEATURE_WGP)
    return false;
  ...
  for (const Arg *A : Args.filtered(Group)) {
    ...
    if (shouldIgnoreUnsupportedTargetFeature(*A, Triple, Proc)) {
      if (Warned.count(Name) == 0) {
        D.getDiags().Report(
            language::Core::diag::warn_drv_unsupported_option_for_processor)
...
tools::unifyTargetFeatures(ArrayRef<StringRef> Features) {
  ...
  toolchain::DenseSet<StringRef> UsedFeatures;
  for (StringRef Feature : toolchain::reverse(Features)) {
...
void tools::addDirectoryList(const ArgList &Args, ArgStringList &CmdArgs,
  ...
  while ((Delim = Dirs.find(toolchain::sys::EnvPathSeparator)) != StringRef::npos) {
...
void tools::AddLinkerInputs(const ToolChain &TC, const InputInfoList &Inputs,
  ...
  for (const auto &II : Inputs) {
    ...
    if (!TC.HasNativeLLVMSupport() && types::isLLVMIR(II.getType()))
      ...
      D.Diag(diag::err_drv_no_linker_toolchain_support) << TC.getTripleString();
...
const char *tools::getLDMOption(const toolchain::Triple &T, const ArgList &Args) {
  switch (T.getArch()) {
  case toolchain::Triple::x86:
    if (T.isOSIAMCU())
      return "elf_iamcu";
...
static std::string getAMDGPUTargetGPU(const toolchain::Triple &T,
                                      const ArgList &Args) {
  Arg *MArch = Args.getLastArg(options::OPT_march_EQ);
  if (Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    auto GPUName = getProcessorFromTargetID(T, A->getValue());
    return toolchain::StringSwitch<std::string>(GPUName)
...
static StringRef getWebAssemblyTargetCPU(const ArgList &Args) {
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    ...
    if (CPU == "native")
      return toolchain::sys::getHostCPUName();
#endif
...
std::string tools::getCPUName(const Driver &D, const ArgList &Args,
                              const toolchain::Triple &T, bool FromAs) {
  Arg *A;
  ...
  case toolchain::Triple::ppc64le:
    if (Arg *A = Args.getLastArg(language::Core::driver::options::OPT_mcpu_EQ))
      return std::string(
          toolchain::PPC::getNormalizedPPCTargetCPU(T, A->getValue()));
    return std::string(toolchain::PPC::getNormalizedPPCTargetCPU(T));
...
void tools::getTargetFeatures(const Driver &D, const toolchain::Triple &Triple,
                              const ArgList &Args, ArgStringList &CmdArgs,
...
toolchain::StringRef tools::getLTOParallelism(const ArgList &Args, const Driver &D) {
  Arg *LtoJobsArg = Args.getLastArg(options::OPT_flto_jobs_EQ);
  if (!LtoJobsArg)
    return {};
  if (!toolchain::get_threadpool_strategy(LtoJobsArg->getValue()))
...
bool tools::isUseSeparateSections(const toolchain::Triple &Triple) {
  return Triple.isPS();
}
...
void tools::addLTOOptions(const ToolChain &ToolChain, const ArgList &Args,
  ...
  const toolchain::Triple &Triple = ToolChain.getTriple();
  ...
  if (Linker != "lld" && Linker != "lld-link" &&
      toolchain::sys::path::filename(LinkerPath) != "ld.lld" &&
      toolchain::sys::path::stem(LinkerPath) != "ld.lld" && !Triple.isOSOpenBSD()) {
    ...
    toolchain::sys::path::native(Twine(D.Dir) +
                                "/../" CLANG_INSTALL_LIBDIR_BASENAME +
                                PluginName + Suffix,
                            Plugin);
    CmdArgs.push_back(Args.MakeArgString(Twine(PluginPrefix) + Plugin));
  } else {
    // Tell LLD to find and use .toolchain.lto section in regular relocatable object
    // files
    if (IsFatLTO)
  ...
  if (ArgVecLib && ArgVecLib->getNumValues() == 1) {
    ...
    std::optional<StringRef> OptVal =
        toolchain::StringSwitch<std::optional<StringRef>>(ArgVecLib->getValue())
...
void tools::addOpenMPRuntimeLibraryPath(const ToolChain &TC,
  ...
  SmallString<256> DefaultLibPath =
      toolchain::sys::path::parent_path(TC.getDriver().Dir);
  toolchain::sys::path::append(DefaultLibPath, CLANG_INSTALL_LIBDIR_BASENAME);
...
void tools::addOpenMPHostOffloadingArgs(const Compilation &C,
  ...
  constexpr toolchain::StringLiteral Targets("--offload-targets=");
  ...
  CmdArgs.push_back(
      Args.MakeArgString(Twine(Targets) + toolchain::join(Triples, ",")));
}
...
static bool addSanitizerDynamicList(const ToolChain &TC, const ArgList &Args,
  ...
  if (toolchain::sys::fs::exists(SanRT + ".syms")) {
...
void tools::linkSanitizerRuntimeDeps(const ToolChain &TC,
  ...
  if (TC.getTriple().getOS() != toolchain::Triple::RTEMS &&
  ...
  if (!TC.getTriple().isOSFreeBSD() && !TC.getTriple().isOSNetBSD() &&
      ...
      TC.getTriple().getOS() != toolchain::Triple::RTEMS)
  ...
  if (SanArgs.needsFuzzer() && SanArgs.linkRuntimes() &&
    ...
    if (SanArgs.needsFuzzerInterceptors())
      addSanitizerRuntime(TC, Args, CmdArgs, "fuzzer_interceptors", false,
                          true);
    if (!Args.hasArg(language::Core::driver::options::OPT_nostdlibxx)) {
...
const char *tools::SplitDebugName(const JobAction &JA, const ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_dumpdir)) {
    ...
    if (FinalOutput && Args.hasArg(options::OPT_c)) {
      ...
      toolchain::sys::path::remove_filename(T);
      toolchain::sys::path::append(T,
                              toolchain::sys::path::stem(FinalOutput->getValue()));
      AddPostfix(T);
      return Args.MakeArgString(T);
    }
  }

  T += toolchain::sys::path::stem(Input.getBaseInput());
  AddPostfix(T);
  return Args.MakeArgString(T);
}
...
const char *tools::RelocationModelName(toolchain::Reloc::Model Model) {
  switch (Model) {
  case toolchain::Reloc::Static:
    return "static";
  case toolchain::Reloc::PIC_:
    return "pic";
  case toolchain::Reloc::DynamicNoPIC:
    return "dynamic-no-pic";
  case toolchain::Reloc::ROPI:
    return "ropi";
  case toolchain::Reloc::RWPI:
    return "rwpi";
  case toolchain::Reloc::ROPI_RWPI:
    return "ropi-rwpi";
  }
  toolchain_unreachable("Unknown Reloc::Model kind");
}
...
tools::ParsePICArgs(const ToolChain &ToolChain, const ArgList &Args) {
  const toolchain::Triple &EffectiveTriple = ToolChain.getEffectiveTriple();
  const toolchain::Triple &Triple = ToolChain.getTriple();
  ...
  if (Triple.isOHOSFamily() && Triple.getArch() == toolchain::Triple::aarch64)
    PIC = true;
  ...
  if (Triple.isOSWindows() && !Triple.isOSCygMing() && LastPICArg &&
    ...
    if (Triple.getArch() == toolchain::Triple::x86_64)
      return std::make_tuple(toolchain::Reloc::PIC_, 2U, false);
    return std::make_tuple(toolchain::Reloc::Static, 0U, false);
  }
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mdynamic_no_pic)) {
    ...
    // match that of toolchain-gcc and Apple GCC before that.
    PIC = ToolChain.isPICDefault() && ToolChain.isPICDefaultForced();

    return std::make_tuple(toolchain::Reloc::DynamicNoPIC, PIC ? 2U : 0U, false);
  }
  ...
  if (PIC)
    return std::make_tuple(toolchain::Reloc::PIC_, IsPICLevelTwo ? 2U : 1U, PIE);

  toolchain::Reloc::Model RelocM = toolchain::Reloc::Static;
  if (ROPI && RWPI)
    RelocM = toolchain::Reloc::ROPI_RWPI;
  else if (ROPI)
    RelocM = toolchain::Reloc::ROPI;
  else if (RWPI)
    RelocM = toolchain::Reloc::RWPI;
...
bool tools::getStaticPIE(const ArgList &Args, const ToolChain &TC) {
  ...
  if (HasStaticPIE && Args.hasArg(options::OPT_no_pie)) {
    const Driver &D = TC.getDriver();
    const toolchain::opt::OptTable &Opts = D.getOpts();
...
unsigned tools::ParseFunctionAlignment(const ToolChain &TC,
  ...
  return Value ? toolchain::Log2_32_Ceil(std::min(Value, 65536u)) : Value;
}

void tools::addDebugInfoKind(
    ArgStringList &CmdArgs, toolchain::codegenoptions::DebugInfoKind DebugInfoKind) {
...
toolchain::codegenoptions::DebugInfoKind tools::debugLevelToInfoKind(const Arg &A) {
  assert(A.getOption().matches(options::OPT_gN_Group) &&
         "Not a -g option that specifies a debug-info level");
  if (A.getOption().matches(options::OPT_g0) ||
      A.getOption().matches(options::OPT_ggdb0))
    return toolchain::codegenoptions::NoDebugInfo;
  if (A.getOption().matches(options::OPT_gline_tables_only) ||
      A.getOption().matches(options::OPT_ggdb1))
    return toolchain::codegenoptions::DebugLineTablesOnly;
  if (A.getOption().matches(options::OPT_gline_directives_only))
    return toolchain::codegenoptions::DebugDirectivesOnly;
  return toolchain::codegenoptions::DebugInfoConstructor;
}
...
unsigned tools::DwarfVersionNum(StringRef ArgValue) {
  return toolchain::StringSwitch<unsigned>(ArgValue)
      .Case("-gdwarf-2", 2)
      .Case("-gdwarf-3", 3)
      .Case("-gdwarf-4", 4)
      .Case("-gdwarf-5", 5)
...
void tools::AddAssemblerKPIC(const ToolChain &ToolChain, const ArgList &Args,
  ...
  if (RelocationModel != toolchain::Reloc::Static)
...
static void AddLibgcc(const ToolChain &TC, const Driver &D,
  ...
  if (D.IsFlangMode() && TC.getArch() == toolchain::Triple::aarch64) {
...
SmallString<128> tools::getStatsFileName(const toolchain::opt::ArgList &Args,
                                         const InputInfo &Output,
                                         const InputInfo &Input,
                                         const Driver &D) {
  ...
  if (A) {
    ...
    if (SaveStats == "obj" && Output.isFilename()) {
      ...
      toolchain::sys::path::remove_filename(StatsFile);
    } else if (SaveStats != "cwd") {
      D.Diag(diag::err_drv_invalid_value) << A->getAsString(Args) << SaveStats;
      return {};
    }

    StringRef BaseName = toolchain::sys::path::filename(Input.getBaseInput());
    toolchain::sys::path::append(StatsFile, BaseName);
    toolchain::sys::path::replace_extension(StatsFile, "stats");
  } else {
...
void tools::addX86AlignBranchArgs(const Driver &D, const ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_malign_branch_boundary_EQ)) {
    ...
    if (Value.getAsInteger(10, Boundary) || Boundary < 16 ||
        !toolchain::isPowerOf2_64(Boundary)) {
...
///
static bool SDLSearch(const Driver &D, const toolchain::opt::ArgList &DriverArgs,
  ...
  // The CUDA toolchain does not use a global device toolchain-link before the LLVM
...
/// of this SDL is passed to the toolchain-link tool.
static void GetSDLFromOffloadArchive(
    Compilation &C, const Driver &D, const Tool &T, const JobAction &JA,
    const InputInfoList &Inputs, const toolchain::opt::ArgList &DriverArgs,
    toolchain::opt::ArgStringList &CC1Args,
  ...
  toolchain::Triple Triple(D.getTargetTriple());
  bool IsMSVC = Triple.isWindowsMSVCEnvironment();
  auto Ext = IsMSVC ? ".lib" : ".a";
  if (!Lib.starts_with(":") && !Lib.starts_with("-l")) {
    if (toolchain::sys::fs::exists(Lib)) {
    ...
    for (auto LPath : LibraryPaths) {
      ...
      for (auto Prefix : {"/libdevice/", "/"}) {
        ...
        if (toolchain::sys::fs::exists(AOB)) {
  ...
  toolchain::file_magic Magic;
  auto EC = toolchain::identify_magic(ArchiveOfBundles, Magic);
  if (EC || Magic != toolchain::file_magic::archive)
    return;

  StringRef Prefix = isBitCodeSDL ? "libbc-" : "lib";
  std::string OutputLib =
      D.GetTemporaryPath(Twine(Prefix + toolchain::sys::path::filename(Lib) + "-" +
                               Arch + "-" + Target)
                             .str(),
                         "a");
...
void tools::AddStaticDeviceLibs(Compilation *C, const Tool *T,
  ...
  std::optional<std::string> LibPath =
      toolchain::sys::Process::GetEnv("LIBRARY_PATH");
  if (LibPath) {
    SmallVector<StringRef, 8> Frags;
    const char EnvPathSeparatorStr[] = {toolchain::sys::EnvPathSeparator, '\0'};
    toolchain::SplitString(*LibPath, Frags, EnvPathSeparatorStr);
  ...
  SmallString<256> DefaultLibPath = toolchain::sys::path::parent_path(D.Dir);
  toolchain::sys::path::append(DefaultLibPath, CLANG_INSTALL_LIBDIR_BASENAME);
  LibraryPaths.emplace_back(DefaultLibPath.c_str());

  // Build list of Static Device Libraries SDLs specified by -l option
  toolchain::SmallSet<std::string, 16> SDLNames;
  static const StringRef HostOnlyArchives[] = {
      "omp", "cudart", "m", "gcc", "gcc_s", "pthread", "hip_hcc"};
  for (auto SDLName : DriverArgs.getAllArgValues(options::OPT_l)) {
    if (!toolchain::is_contained(HostOnlyArchives, SDLName)) {
  ...
  for (auto Input : DriverArgs.getAllArgValues(options::OPT_INPUT)) {
    ...
    if (!toolchain::sys::path::has_extension(FileName) ||
  ...
  // the full filename of the SDL to the toolchain-link command. If no SDL is found
...
static toolchain::opt::Arg *
getAMDGPUCodeObjectArgument(const Driver &D, const toolchain::opt::ArgList &Args) {
...
void tools::addMachineOutlinerArgs(const Driver &D,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_moutline,
    ...
    if (A->getOption().matches(options::OPT_moutline)) {
      ...
      // Otherwise, add the proper mtoolchain flags.
...
void tools::addOpenMPDeviceRTL(const Driver &D,
  ...
  toolchain::sys::path::append(P, "..", "lib", Triple.getTriple());
  ...
  if (const Arg *A = DriverArgs.getLastArg(LibomptargetBCPathOpt)) {
    ...
    if (toolchain::sys::fs::exists(LibOmpTargetFile) &&
        toolchain::sys::fs::is_directory(LibOmpTargetFile)) {
      toolchain::sys::path::append(LibOmpTargetFile, LibOmpTargetName);
    }
...
void tools::addOpenCLBuiltinsLib(const Driver &D,
  ...
  toolchain::sys::path::append(LibclcPath, "lib", "libclc");
  ...
  if (FilenameSearch && toolchain::sys::fs::exists(LibclcTargetFile)) {
    CC1Args.push_back("-mlink-builtin-bitcode");
    ...
    toolchain::sys::path::append(LibclcPath, LibclcTargetFile);
    if (toolchain::sys::fs::exists(LibclcPath)) {
      CC1Args.push_back("-mlink-builtin-bitcode");
...
void tools::addOffloadCompressArgs(const toolchain::opt::ArgList &TCArgs,
                                   toolchain::opt::ArgStringList &CmdArgs) {
...
void tools::addMCModel(const Driver &D, const toolchain::opt::ArgList &Args,
                       const toolchain::Triple &Triple,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mcmodel_EQ)) {
    ...
    if (Triple.isAArch64(64)) {
      ...
      if (CM == "large" && !Triple.isOSBinFormatMachO() &&
          RelocationModel != toolchain::Reloc::Static)
      ...
      if (Ok)
        CM = toolchain::StringSwitch<StringRef>(CM)
                 .Case("normal", "small")
                 .Case("extreme", "large")
                 .Default(CM);
      ...
      if (CM == "large" && RelocationModel != toolchain::Reloc::Static)
        D.Diag(diag::err_drv_argument_not_allowed_with)
    ...
    } else if (Triple.getArch() == toolchain::Triple::x86_64) {
      Ok = toolchain::is_contained({"small", "kernel", "medium", "large"}, CM);
...
const char *tools::renderEscapedCommandLine(const ToolChain &TC,
  ...
  toolchain::opt::ArgStringList OriginalArgs;
  for (const auto &Arg : Args)
    Arg->render(Args, OriginalArgs);

  toolchain::SmallString<256> Flags;
  escapeSpacesAndBackslashes(Exec, Flags);
  for (const char *OriginalArg : OriginalArgs) {
    toolchain::SmallString<128> EscapedArg;
...
StringRef tools::parseMPreferVectorWidthOption(language::Core::DiagnosticsEngine &Diags,
                                               const toolchain::opt::ArgList &Args) {
  Arg *A = Args.getLastArg(language::Core::driver::options::OPT_mprefer_vector_width_EQ);
  if (!A)
    return "";
...
static bool getRefinementStep(StringRef In, language::Core::DiagnosticsEngine &Diags,
...
// Otherwise, return an empty string and issue a diagnosic message if needed.
StringRef tools::parseMRecipOption(language::Core::DiagnosticsEngine &Diags,
                                   const ArgList &Args) {
  ...
  toolchain::StringMap<bool> OptionStrings;
  ...
  for (unsigned i = 0; i != NumOptions; ++i) {
    ...
    toolchain::StringMap<bool>::iterator OptionIter = OptionStrings.find(ValBase);
