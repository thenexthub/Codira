...
#include "toolchain/Config/toolchain-config.h" // for LLVM_VERSION_STRING
  ...
  if (const Arg* A = Args.getLastArg(options::OPT_fuse_ld_EQ)) {
    ...
    if (!UseLinker.empty()) {
      if (toolchain::sys::path::is_absolute(UseLinker) &&
          toolchain::sys::fs::can_execute(UseLinker))
...
static bool TargetBuildsComponents(const toolchain::Triple &TargetTriple) {
...
static bool WantsPthread(const toolchain::Triple &Triple, const ArgList &Args) {
  bool WantsPthread =
...
void wasm::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // where it is.
  if (toolchain::sys::path::stem(Linker).ends_with_insensitive(
          "wasm-component-ld")) {
  ...
  if (Arg *A = Args.getLastArg(options::OPT_O_Group)) {
    if (!WasmOptPath.empty()) {
      ...
      if (OOpt != "0") {
        ...
        OptArgs.push_back(Args.MakeArgString(toolchain::Twine("-O") + OOpt));
        OptArgs.push_back("-o");
        OptArgs.push_back(Output.getFilename());
...
static std::string AppendLTOLibDir(const std::string &Dir) {
    ...
    return Dir + "/toolchain-lto/" LLVM_VERSION_STRING;
}

WebAssembly::WebAssembly(const Driver &D, const toolchain::Triple &Triple,
...
bool WebAssembly::isPIEDefault(const toolchain::opt::ArgList &Args) const {
  return false;
}
...
void WebAssembly::addClangTargetOptions(const ArgList &DriverArgs,
  ...
  if (!DriverArgs.hasFlag(language::Core::driver::options::OPT_fuse_init_array,
  ...
  auto BanIncompatibleOptionsForWasmEHSjLj = [&](StringRef CurOption) {
    ...
    for (const Arg *A : DriverArgs.filtered(options::OPT_mtoolchain)) {
  ...
  for (const Arg *A : DriverArgs.filtered(options::OPT_mtoolchain)) {
    ...
    if (Opt.starts_with("-emscripten-cxx-exceptions-allowed")) {
      // '-mtoolchain -emscripten-cxx-exceptions-allowed' should be used with
      // '-mtoolchain -enable-emscripten-cxx-exceptions'
      ...
      if (!EmEHArgExists)
        getDriver().Diag(diag::err_drv_argument_only_allowed_with)
            << "-mtoolchain -emscripten-cxx-exceptions-allowed"
            << "-mtoolchain -enable-emscripten-cxx-exceptions";
...
void WebAssembly::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  if (CIncludeDirs != "") {
    ...
    for (StringRef dir : dirs) {
      StringRef Prefix =
          toolchain::sys::path::is_absolute(dir) ? "" : StringRef(D.SysRoot);
...
void WebAssembly::AddCXXStdlibLibArgs(const toolchain::opt::ArgList &Args,
...
void WebAssembly::addLibCxxIncludePaths(
  ...
  bool IsKnownOs = (getTriple().getOS() != toolchain::Triple::UnknownOS);
