...
static StringRef getDefaultHvxLength(StringRef HvxVer) {
  return toolchain::StringSwitch<StringRef>(HvxVer)
      .Case("v60", "64b")
      .Case("v62", "64b")
      .Case("v65", "64b")
      .Default("128b");
...
void hexagon::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const char *AsName = "toolchain-mc";
  ...
  for (const auto &II : Inputs) {
    ...
    if (types::isLLVMIR(II.getType()))
      ...
      D.Diag(language::Core::diag::err_drv_no_ast_support)
          << HTC.getTripleString();
...
constructHexagonLinkArgs(Compilation &C, const JobAction &JA,
  ...
  UseLLD = UseLLD || toolchain::sys::path::filename(Exec).ends_with("ld.lld") ||
           toolchain::sys::path::stem(Exec).ends_with("ld.lld");
...
std::string HexagonToolChain::getCompilerRTPath() const {
  ...
  toolchain::sys::path::append(Dir, "usr", "lib");
...
void HexagonToolChain::getHexagonLibraryPaths(const ArgList &Args,
  ...
  for (Arg *A : Args.filtered(options::OPT_L))
    toolchain::append_range(LibPaths, A->getValues());
  ...
  if (!toolchain::is_contained(RootDirs, TargetDir))
...
HexagonToolChain::HexagonToolChain(const Driver &D, const toolchain::Triple &Triple,
...
bool HexagonToolChain::isAutoHVXEnabled(const toolchain::opt::ArgList &Args) {
