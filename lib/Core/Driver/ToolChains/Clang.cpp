...
#include "language/Core/Driver/XRayArgs.h"
...
#include "toolchain/TargetParser/ARMTargetParserCommon.h"
#include "toolchain/TargetParser/Host.h"
#include "toolchain/TargetParser/LoongArchTargetParser.h"
#include "toolchain/TargetParser/PPCTargetParser.h"
...
static void CheckPreprocessingOptions(const Driver &D, const ArgList &Args) {
  if (Arg *A = Args.getLastArg(language::Core::driver::options::OPT_C, options::OPT_CC,
    ...
    if (!Args.hasArg(options::OPT_E) && !Args.hasArg(options::OPT__SLASH_P) &&
      ...
      D.Diag(language::Core::diag::err_drv_argument_only_allowed_with)
...
forAllAssociatedToolChains(Compilation &C, const JobAction &JA,
                           ...
                           toolchain::function_ref<void(const ToolChain &)> Work) {
...
static bool addExceptionArgs(const ArgList &Args, types::ID InputType,
  ...
  const toolchain::Triple &Triple = TC.getTriple();
  ...
  if (types::isCXX(InputType)) {
    ...
    bool CXXExceptionsEnabled = Triple.getArch() != toolchain::Triple::xcore &&
...
static const char *addDebugCompDirArg(const ArgList &Args,
                                      ArgStringList &CmdArgs,
                                      const toolchain::vfs::FileSystem &VFS) {
  ...
  if (DebugCompDir.empty()) {
    if (toolchain::ErrorOr<std::string> CWD = VFS.getCurrentWorkingDirectory())
...
static void addDebugObjectName(const ArgList &Args, ArgStringList &CmdArgs,
  ...
  if (ObjFileNameForDebug != "-" &&
      !toolchain::sys::path::is_absolute(ObjFileNameForDebug) &&
      (!DebugCompilationDir ||
       toolchain::sys::path::is_absolute(DebugCompilationDir))) {
    // Make the path absolute in the debug infos like MSVC does.
    toolchain::sys::fs::make_absolute(ObjFileNameForDebug);
  }
  ...
  toolchain::sys::path::remove_dots(ObjFileNameForDebug, /*remove_dot_dot=*/true,
                               Style);
  CmdArgs.push_back(
...
static void addPGOAndCoverageFlags(const ToolChain &TC, Compilation &C,
  ...
  if (auto *ColdFuncCoverageArg = Args.getLastArg(
    ...
    SmallString<128> Path(
        ColdFuncCoverageArg->getOption().matches(
            ...
            : "");
    toolchain::sys::path::append(Path, "default_%m.profraw");
  ...
  if (PGOGenerateArg) {
    ...
    CmdArgs.push_back("-fprofile-instrument=toolchain");
  }
  if (CSPGOGenerateArg) {
    assert(!PGOGenerateArg);
    PGOGenArg = CSPGOGenerateArg;
    CmdArgs.push_back("-fprofile-instrument=cstoolchain");
  }
  if (PGOGenArg) {
    if (TC.getTriple().isWindowsMSVCEnvironment() &&
        Args.hasFlag(options::OPT_frtlib_defaultlib,
  ...
  if (ProfileUseArg) {
    if (ProfileUseArg->getOption().matches(options::OPT_fprofile_instr_use_EQ))
      ...
      if (Path.empty() || toolchain::sys::fs::is_directory(Path))
        toolchain::sys::path::append(Path, "default.profdata");
      CmdArgs.push_back(
  ...
  if (const auto *A = Args.getLastArg(options::OPT_fprofile_continuous)) {
    if (!PGOGenerateArg && !CSPGOGenerateArg && !ProfileGenerateArg)
      ...
      // (__toolchain_profile_filename). Instead, the runtime uses the name
  ...
  if (EmitCovNotes || EmitCovData) {
    ...
    if (Arg *DumpDir = Args.getLastArgNoClaim(options::OPT_dumpdir)) {
      ...
      CoverageFilename += toolchain::sys::path::filename(Output.getBaseInput());
    ...
    if (toolchain::sys::path::is_relative(CoverageFilename))
      (void)D.getVFS().makeAbsolute(CoverageFilename);
    toolchain::sys::path::replace_extension(CoverageFilename, "gcno");
    if (EmitCovNotes) {
      CmdArgs.push_back(
    ...
    if (EmitCovData) {
      if (FProfileDir) {
        ...
        toolchain::sys::path::append(CoverageFilename, Gcno);
      }
      toolchain::sys::path::replace_extension(CoverageFilename, "gcda");
      CmdArgs.push_back(
...
RenderDebugEnablingArgs(const ArgList &Args, ArgStringList &CmdArgs,
                        ...
                        toolchain::DebuggerKind DebuggerTuning) {
  addDebugInfoKind(CmdArgs, DebugInfoKind);
  if (DwarfVersion > 0)
    CmdArgs.push_back(
        Args.MakeArgString("-dwarf-version=" + Twine(DwarfVersion)));
  switch (DebuggerTuning) {
  case toolchain::DebuggerKind::GDB:
    CmdArgs.push_back("-debugger-tuning=gdb");
    break;
  case toolchain::DebuggerKind::LLDB:
    CmdArgs.push_back("-debugger-tuning=lldb");
    break;
  case toolchain::DebuggerKind::SCE:
    CmdArgs.push_back("-debugger-tuning=sce");
    break;
  case toolchain::DebuggerKind::DBX:
...
static void RenderDebugInfoCompressionArgs(const ArgList &Args,
  ...
  if (checkDebugInfoOption(A, Args, D, TC)) {
    ...
    if (Value == "none") {
      ...
      if (toolchain::compression::zlib::isAvailable()) {
        CmdArgs.push_back(
...
static void handleAMDGPUCodeObjectVersionOptions(const Driver &D,
  ...
  // requiring the corresponding toolchain to have the AMDGPU target enabled,
  ...
  if (haveAMDGPUCodeObjectVersionArgument(D, Args)) {
    ...
    CmdArgs.insert(CmdArgs.begin() + 1,
                   Args.MakeArgString(Twine("--amdhsa-code-object-version=") +
                                      Twine(CodeObjVer)));
    CmdArgs.insert(CmdArgs.begin() + 1, "-mtoolchain");
...
static bool maybeHasClangPchSignature(const Driver &D, StringRef Path) {
  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> MemBuf =
      D.getVFS().getBufferForFile(Path);
  if (!MemBuf)
    return false;
  toolchain::file_magic Magic = toolchain::identify_magic((*MemBuf)->getBuffer());
  if (Magic == toolchain::file_magic::unknown)
    return false;
  // Return true for both raw Clang AST files and object files which may
  // contain a __clangast section.
  if (Magic == toolchain::file_magic::clang_ast)
    return true;
  Expected<std::unique_ptr<toolchain::object::ObjectFile>> Obj =
      toolchain::object::ObjectFile::createObjectFile(**MemBuf, Magic);
  return !Obj.takeError();
}

static bool gchProbe(const Driver &D, StringRef Path) {
  toolchain::ErrorOr<toolchain::vfs::Status> Status = D.getVFS().status(Path);
  if (!Status)
    return false;

  if (Status->isDirectory()) {
    std::error_code EC;
    for (toolchain::vfs::directory_iterator DI = D.getVFS().dir_begin(Path, EC), DE;
...
void Clang::AddPreprocessingOptions(Compilation &C, const JobAction &JA,
  ...
  if (ArgM) {
    ...
    if (!HasTarget) {
      ...
      if (OutputOpt && Output.getType() != types::TY_Dependencies) {
        ...
        toolchain::sys::path::replace_extension(P, "o");
        DepTarget = Args.MakeArgString(toolchain::sys::path::filename(P));
      }
  ...
  if (JA.isDeviceOffloading(Action::OFK_OpenMP) &&
    ...
    if (!Args.hasArg(options::OPT_nobuiltininc)) {
      ...
      toolchain::sys::path::append(P, "openmp_wrappers");
  ...
  if (Args.hasArg(options::OPT_foffload_via_toolchain)) {
    // Add toolchain_wrappers/* to our system include path.  This lets us wrap
    // standard library headers and other headers.
    SmallString<128> P(D.ResourceDir);
    toolchain::sys::path::append(P, "include", "toolchain_offload_wrappers");
    CmdArgs.append({"-internal-isystem", Args.MakeArgString(P), "-include"});
    if (JA.isDeviceOffloading(Action::OFK_OpenMP))
      CmdArgs.push_back("__toolchain_offload_device.h");
    else
      CmdArgs.push_back("__toolchain_offload_host.h");
  }
      ...
      if (!isa<PrecompileJobAction>(JA)) {
        ...
        CmdArgs.push_back(Args.MakeArgString(D.GetClPchPath(
            C, !ThroughHeader.empty()
                   ? ThroughHeader
                   : toolchain::sys::path::filename(Inputs[0].getBaseInput()))));
      }
  ...
  for (const Arg *A : Args.filtered(options::OPT_clang_i_Group)) {
    if (A->getOption().matches(options::OPT_include) &&
      ...
      P += ".dummy";
      toolchain::sys::path::replace_extension(P, "pch");
      if (D.getVFS().exists(P))
        FoundPCH = true;

      if (!FoundPCH) {
        // For GCC compat, probe for a file or directory ending in .gch instead.
        toolchain::sys::path::replace_extension(P, "gch");
  ...
  if (!Args.hasArg(options::OPT_nostdinc) &&
    ...
    // the resource directory at clang/lib/Headers/toolchain_libc_wrappers.
    if (getToolChain().getTriple().isGPU() &&
        C.getActiveOffloadKinds() == Action::OFK_None) {
      SmallString<128> P(toolchain::sys::path::parent_path(D.Dir));
      toolchain::sys::path::append(P, "include");
      toolchain::sys::path::append(P, getToolChain().getTripleString());
      CmdArgs.push_back("-internal-isystem");
      CmdArgs.push_back(Args.MakeArgString(P));
      ...
      SmallString<128> P(D.ResourceDir);
      toolchain::sys::path::append(P, "include");
      toolchain::sys::path::append(P, "toolchain_libc_wrappers");
      CmdArgs.push_back("-internal-isystem");
      CmdArgs.push_back(Args.MakeArgString(P));
    }
  }
...
static bool isSignedCharDefault(const toolchain::Triple &Triple) {
...
static bool hasMultipleInvocations(const toolchain::Triple &Triple,
                                   const ArgList &Args) {
...
static void renderRemarksOptions(const ArgList &Args, ArgStringList &CmdArgs,
  ...
  if (A) {
    ...
    if (F.empty()) {
      ...
      F = toolchain::sys::path::stem(Input.getBaseInput());
      ...
      if (!JA.isDeviceOffloading(Action::OFK_None) &&
        ...
        toolchain::sys::path::replace_extension(F, "");
    ...
    if (hasMultipleArchs) {
      ...
      SmallString<64> OldExtension = toolchain::sys::path::extension(F);
      // then, remove it.
      toolchain::sys::path::replace_extension(F, "");
      // attach -<arch> to it.
      F += "-";
      ...
      toolchain::sys::path::replace_extension(F, OldExtension);
    }
    ...
    toolchain::sys::path::replace_extension(F, Extension);
...
namespace {
void RenderARMABI(const Driver &D, const toolchain::Triple &Triple,
                  const ArgList &Args, ArgStringList &CmdArgs) {
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mabi_EQ))
    ...
    ABIName = toolchain::ARM::computeDefaultTargetABI(Triple).data();
...
void AddUnalignedAccessWarning(ArgStringList &CmdArgs) {
  auto StrictAlignIter =
      toolchain::find_if(toolchain::reverse(CmdArgs), [](StringRef Arg) {
...
static void CollectARMPACBTIOptions(const ToolChain &TC, const ArgList &Args,
  ...
  const toolchain::Triple &Triple = TC.getEffectiveTriple();
  ...
  if (A->getOption().matches(options::OPT_msign_return_address_EQ)) {
    ...
    StringRef DiagMsg;
    toolchain::ARM::ParsedBranchProtection PBP;
    ...
    if (isAArch64) {
      auto isPAuthLR = [](const char *member) {
        toolchain::AArch64::ExtensionInfo pauthlr_extension =
            toolchain::AArch64::getExtensionByID(toolchain::AArch64::AEK_PAUTHLR);
        return pauthlr_extension.PosTargetFeature == member;
      };

      if (toolchain::any_of(CmdArgs, isPAuthLR))
        EnablePAuthLR = true;
    }
    if (!toolchain::ARM::parseBranchProtection(A->getValue(), PBP, DiagMsg,
                                          EnablePAuthLR))
      D.Diag(diag::err_drv_unsupported_option_argument)
  ...
  bool HasPtrauthReturns = toolchain::any_of(CmdArgs, [](const char *Arg) {
  ...
  if (HasPtrauthReturns &&
    ...
    if (Triple.getEnvironment() == toolchain::Triple::PAuthTest)
      D.Diag(diag::err_drv_unsupported_opt_for_target)
...
void Clang::AddARMTargetArgs(const toolchain::Triple &Triple, const ArgList &Args,
...
void Clang::RenderTargetOptions(const toolchain::Triple &EffectiveTriple,
...
namespace {
void RenderAArch64ABI(const toolchain::Triple &Triple, const ArgList &Args,
                      ArgStringList &CmdArgs) {
  const char *ABIName = nullptr;
  if (Arg *A = Args.getLastArg(options::OPT_mabi_EQ))
    ABIName = A->getValue();
    ...
    ABIName = "darwinpcs";
  else if (Triple.getEnvironment() == toolchain::Triple::PAuthTest)
    ABIName = "pauthtest";
  else
    ABIName = "aapcs";
...
void Clang::AddAArch64TargetArgs(const ArgList &Args,
                                 ArgStringList &CmdArgs) const {
  const toolchain::Triple &Triple = getToolChain().getEffectiveTriple();
  ...
  auto HandleVectorBits = [&](Arg *A, StringRef VScaleMin,
    ...
    if (Val == "128" || Val == "256" || Val == "512" || Val == "1024" ||
      ...
      if (!Val.consume_back("+")) {
        ...
        CmdArgs.push_back(
            Args.MakeArgString(VScaleMax + toolchain::Twine(Bits / 128)));
      }
      ...
      CmdArgs.push_back(
          Args.MakeArgString(VScaleMin + toolchain::Twine(Bits / 128)));
  ...
  if (const Arg *A = Args.getLastArg(language::Core::driver::options::OPT_mtune_EQ)) {
    CmdArgs.push_back("-tune-cpu");
    if (strcmp(A->getValue(), "native") == 0)
      CmdArgs.push_back(Args.MakeArgString(toolchain::sys::getHostCPUName()));
    else
      CmdArgs.push_back(A->getValue());
  }
...
void Clang::AddLoongArchTargetArgs(const ArgList &Args,
                                   ArgStringList &CmdArgs) const {
  const toolchain::Triple &Triple = getToolChain().getTriple();
...
void Clang::AddMIPSTargetArgs(const ArgList &Args,
  ...
  // the only case where -mtoolchain -mgpopt is passed.
  ...
  toolchain::Reloc::Model RelocationModel;
  unsigned PICLevel;
  bool IsPIE;
  ...
  NoABICalls = NoABICalls ||
               (RelocationModel == toolchain::Reloc::Static && ABIName == "n64");
...
void Clang::AddPPCTargetArgs(const ArgList &Args,
                             ArgStringList &CmdArgs) const {
  const Driver &D = getToolChain().getDriver();
  const toolchain::Triple &T = getToolChain().getTriple();
  if (Arg *A = Args.getLastArg(options::OPT_mtune_EQ)) {
    CmdArgs.push_back("-tune-cpu");
    StringRef CPU = toolchain::PPC::getNormalizedPPCTuneCPU(T, A->getValue());
...
void Clang::AddRISCVTargetArgs(const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mrvv_vector_bits_EQ)) {
    ...
    unsigned Bits = 0;
    if (Val == "zvl" && MinVLen >= toolchain::RISCV::RVVBitsPerBlock) {
      Bits = MinVLen;
    } else if (!Val.getAsInteger(10, Bits)) {
      // Only accept power of 2 values beteen RVVBitsPerBlock and 65536 that
      // at least MinVLen.
      if (Bits < MinVLen || Bits < toolchain::RISCV::RVVBitsPerBlock ||
          Bits > 65536 || !toolchain::isPowerOf2_32(Bits))
        Bits = 0;
    }

    // If we got a valid value try to use it.
    if (Bits != 0) {
      unsigned VScaleMin = Bits / toolchain::RISCV::RVVBitsPerBlock;
      CmdArgs.push_back(
          Args.MakeArgString("-mvscale-max=" + toolchain::Twine(VScaleMin)));
      CmdArgs.push_back(
          Args.MakeArgString("-mvscale-min=" + toolchain::Twine(VScaleMin)));
    } else if (Val != "scalable") {
...
void Clang::AddSparcTargetArgs(const ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(language::Core::driver::options::OPT_mtune_EQ)) {
    ...
    std::string TuneCPU;
    if (Name == "native")
      TuneCPU = std::string(toolchain::sys::getHostCPUName());
    else
...
void Clang::AddX86TargetArgs(const ArgList &Args,
  ...
  std::string TuneCPU;
  if (!Args.hasArg(language::Core::driver::options::OPT_march_EQ) &&
  ...
  if (const Arg *A = Args.getLastArg(language::Core::driver::options::OPT_mtune_EQ)) {
    ...
    if (Name == "native") {
      Name = toolchain::sys::getHostCPUName();
      if (!Name.empty())
        TuneCPU = std::string(Name);
    } else
      TuneCPU = std::string(Name);
...
void Clang::DumpCompilationDatabase(Compilation &C, StringRef Filename,
  ...
  using toolchain::yaml::escape;
  const Driver &D = getToolChain().getDriver();

  if (!CompilationDatabase) {
    std::error_code EC;
    auto File = std::make_unique<toolchain::raw_fd_ostream>(
        Filename, EC,
        toolchain::sys::fs::OF_TextWithCRLF | toolchain::sys::fs::OF_Append);
    if (EC) {
      D.Diag(language::Core::diag::err_drv_compilationdatabase) << Filename
                                                       << EC.message();
      return;
    }
...
void Clang::DumpCompilationDatabaseFragmentToDir(
  ...
  auto Err = toolchain::sys::fs::create_directory(Path, /*IgnoreExisting=*/true);
  if (Err) {
  ...
  toolchain::sys::path::append(
      Path,
      Twine(toolchain::sys::path::filename(Input.getFilename())) + ".%%%%.json");
  int FD;
  SmallString<256> TempPath;
  Err = toolchain::sys::fs::createUniqueFile(Path, FD, TempPath,
  ...
  CompilationDatabase =
      std::make_unique<toolchain::raw_fd_ostream>(FD, /*shouldClose=*/true);
  DumpCompilationDatabase(C, "", Target, Output, Input, Args);
}
...
static void CollectArgsForIntegratedAssembler(Compilation &C,
  ...
  const toolchain::Triple &Triple = C.getDefaultToolChain().getTriple();
  ...
  toolchain::SmallVector<const char *> SparcTargetFeatures;
  ...
  for (const Arg *A :
    ...
    for (StringRef Value : A->getValues()) {
      ...
      auto checkArg = [&](bool ValidTarget,
        ...
        if (!ValidTarget) {
          D.Diag(diag::err_drv_unsupported_opt_for_target)
              ...
              << Triple.getTriple();
        } else if (!toolchain::is_contained(Set, Equal.second)) {
          D.Diag(diag::err_drv_unsupported_option_argument)
      ...
      case toolchain::Triple::mips64el:
        ...
        MipsTargetFeature = toolchain::StringSwitch<const char *>(Value)
                                .Case("-mips1", "+mips1")
                                .Case("-mips2", "+mips2")
                                .Case("-mips3", "+mips3")
                                .Case("-mips4", "+mips4")
      ...
      case toolchain::Triple::sparcv9:
        ...
        SparcTargetFeatures =
            toolchain::StringSwitch<toolchain::SmallVector<const char *>>(Value)
                .Case("-Av8", {"-v8plus"})
      ...
      if (Value == "-force_cpusubtype_ALL") {
        ...
        if (DwarfVersion == 0) { // Send it onward, and let cc1as complain.
          ...
          RenderDebugEnablingArgs(Args, CmdArgs,
                                  toolchain::codegenoptions::DebugInfoConstructor,
                                  DwarfVersion, toolchain::DebuggerKind::Default);
        }
        ...
        D.PrintVersion(C, toolchain::outs());
      } else {
        D.Diag(diag::err_drv_unsupported_option_argument)
            << A->getSpelling() << Value;
      }
...
static void EmitComplexRangeDiag(const Driver &D, std::string str1,
  ...
  if (str1 != str2 && !str2.empty() && !str1.empty()) {
    D.Diag(language::Core::diag::warn_drv_overriding_option) << str1 << str2;
  }
}
...
static void RenderFloatingPointOptions(const ToolChain &TC, const Driver &D,
  ...
  constexpr std::array VecLibImpliesNoMathErrno{toolchain::StringLiteral("ArmPL"),
                                                toolchain::StringLiteral("SLEEF")};
  ...
  toolchain::DenormalMode DenormalFPMath =
      TC.getDefaultDenormalModeForType(Args, JA);
  toolchain::DenormalMode DenormalFP32Math =
      TC.getDefaultDenormalModeForType(Args, JA, &toolchain::APFloat::IEEEsingle());
  ...
  for (const Arg *A : Args) {
    auto CheckMathErrnoForVecLib =
        toolchain::make_scope_exit([&, MathErrnoBeforeArg = MathErrno] {
    ...
    case options::OPT_ffp_model_EQ: {
      ...
      if (OFastEnabled && Val != "aggressive") {
        ...
        D.Diag(language::Core::diag::warn_drv_overriding_option)
    ...
    case options::OPT_fveclib:
      ...
      NoMathErrnoWasImpliedByVecLib =
          toolchain::is_contained(VecLibImpliesNoMathErrno, A->getValue());
    ...
    case options::OPT_fdenormal_fp_math_EQ:
      DenormalFPMath = toolchain::parseDenormalFPAttribute(A->getValue());
    ...
    case options::OPT_fdenormal_fp_math_f32_EQ:
      DenormalFP32Math = toolchain::parseDenormalFPAttribute(A->getValue());
    ...
    case options::OPT_fexcess_precision_EQ: {
      StringRef Val = A->getValue();
      const toolchain::Triple::ArchType Arch = TC.getArch();
      if (Arch == toolchain::Triple::x86 || Arch == toolchain::Triple::x86_64) {
  ...
  if (MathErrno) {
    ...
    if (NoMathErrnoWasImpliedByVecLib)
      D.Diag(language::Core::diag::warn_drv_math_errno_enabled_after_veclib)
 ...
 if (AssociativeMath && ReciprocalMath && !SignedZeros && ApproxFunc &&
  ...
  // The default is IEEE.
  if (DenormalFPMath != toolchain::DenormalMode::getIEEE()) {
    toolchain::SmallString<64> DenormFlag;
    toolchain::raw_svector_ostream ArgStr(DenormFlag);
...
static bool isValidSymbolName(StringRef S) {
  ...
  return toolchain::all_of(S, [](char C) { return std::isalnum(C) || C == '_'; });
}

static void RenderSSPOptions(const Driver &D, const ToolChain &TC,
                             const ArgList &Args, ArgStringList &CmdArgs,
                             bool KernelOrKext) {
  const toolchain::Triple &EffectiveTriple = TC.getEffectiveTriple();
...
static void RenderHLSLOptions(const ArgList &Args, ArgStringList &CmdArgs,
  ...
  const unsigned ForwardedArguments[] = {
      ...
      options::OPT_I,
      options::OPT_O,
      options::OPT_emit_toolchain,
      options::OPT_emit_obj,
      options::OPT_disable_toolchain_passes,
...
static void RenderBuiltinOptions(const ToolChain &TC, const toolchain::Triple &T,
                                 const ArgList &Args, ArgStringList &CmdArgs) {
...
bool Driver::getDefaultModuleCachePath(SmallVectorImpl<char> &Result) {
  ...
  if (toolchain::sys::path::cache_directory(Result)) {
    toolchain::sys::path::append(Result, "clang");
    toolchain::sys::path::append(Result, "ModuleCache");
    return true;
  }
  return false;
}
...
language::Core::driver::tools::getCXX20NamedModuleOutputPath(const ArgList &Args,
  ...
  toolchain::sys::path::replace_extension(OutputPath, Extension);
  return OutputPath;
}
...
static bool RenderModulesOptions(Compilation &C, const Driver &D,
  ...
  if (!Args.hasFlag(options::OPT_fimplicit_modules,
    ...
    if (C.isForDiagnostics()) {
      ...
      toolchain::sys::path::replace_extension(Path, ".cache");
      toolchain::sys::path::append(Path, "modules");
    } else if (Path.empty()) {
  ...
  if (Args.hasArg(options::OPT_fbuiltin_module_map)) {
    ...
    toolchain::sys::path::append(BuiltinModuleMap, "include");
    toolchain::sys::path::append(BuiltinModuleMap, "module.modulemap");
    if (toolchain::sys::fs::exists(BuiltinModuleMap))
      CmdArgs.push_back(
  ...
  if (HaveModules || Input.getType() == language::Core::driver::types::TY_ModuleFile) {
  ...
  if (HaveClangModules && C.isForDiagnostics()) {
    ...
    toolchain::sys::path::replace_extension(VFSDir, ".cache");
    // Add the cache directory as a temp so the crash diagnostics pick it up.
    C.addTempFile(Args.MakeArgString(VFSDir));

    toolchain::sys::path::append(VFSDir, "vfs");
  ...
  if (HaveClangModules) {
    ...
    if (Arg *A = Args.getLastArg(options::OPT_fbuild_session_file)) {
      ...
      if (toolchain::sys::fs::status(A->getValue(), Status))
...
static void RenderCharacterOptions(const ArgList &Args, const toolchain::Triple &T,
                                   ArgStringList &CmdArgs) {
...
static void RenderObjCOptions(const ToolChain &TC, const Driver &D,
                              const toolchain::Triple &T, const ArgList &Args,
  ...
  if (Arch == toolchain::Triple::x86 && T.isMacOSX() &&
...
static void renderDwarfFormat(const Driver &D, const toolchain::Triple &T,
                              const ArgList &Args, ArgStringList &CmdArgs,
                              unsigned DwarfVersion) {
  auto *DwarfFormatArg =
...
static void
renderDebugOptions(const ToolChain &TC, const Driver &D, const toolchain::Triple &T,
  ...
  toolchain::DebuggerKind DebuggerTuning = TC.getDefaultDebuggerTuning();
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_g_Group)) {
    DebugInfoKind = toolchain::codegenoptions::DebugInfoConstructor;
    ...
    if (checkDebugInfoOption(A, Args, D, TC) &&
      ...
      if (DebugInfoKind == toolchain::codegenoptions::NoDebugInfo ||
          DebugInfoKind == toolchain::codegenoptions::DebugDirectivesOnly ||
          (DebugInfoKind == toolchain::codegenoptions::DebugLineTablesOnly &&
           SplitDWARFInlining))
        DwarfFission = DwarfFissionKind::None;
    }
  }
  ...
  if (const Arg *A =
    ...
    if (checkDebugInfoOption(A, Args, D, TC)) {
      if (A->getOption().matches(options::OPT_glldb))
        DebuggerTuning = toolchain::DebuggerKind::LLDB;
      else if (A->getOption().matches(options::OPT_gsce))
        DebuggerTuning = toolchain::DebuggerKind::SCE;
      else if (A->getOption().matches(options::OPT_gdbx))
        DebuggerTuning = toolchain::DebuggerKind::DBX;
      else
        DebuggerTuning = toolchain::DebuggerKind::GDB;
    }
  }
  ...
  if (!EmitCodeView && !EmitDwarf &&
      DebugInfoKind != toolchain::codegenoptions::NoDebugInfo) {
    switch (TC.getDefaultDebugFormat()) {
    case toolchain::codegenoptions::DIF_CodeView:
      EmitCodeView = true;
      break;
    case toolchain::codegenoptions::DIF_DWARF:
      EmitDwarf = true;
      break;
    }
  }
  ...
  if (Args.hasFlag(options::OPT_gmodules, options::OPT_gno_modules, false)) {
    ...
    if (checkDebugInfoOption(Args.getLastArg(options::OPT_gmodules), Args, D,
                             TC)) {
      if (DebugInfoKind != toolchain::codegenoptions::DebugLineTablesOnly &&
          DebugInfoKind != toolchain::codegenoptions::DebugDirectivesOnly) {
        DebugInfoKind = toolchain::codegenoptions::DebugInfoConstructor;
  ...
  if (DebugInfoKind == toolchain::codegenoptions::LimitedDebugInfo ||
      DebugInfoKind == toolchain::codegenoptions::DebugInfoConstructor) {
    if (Args.hasFlag(options::OPT_fno_eliminate_unused_debug_types,
                     options::OPT_feliminate_unused_debug_types, false))
      DebugInfoKind = toolchain::codegenoptions::UnusedTypeInfo;
    else if (NeedFullDebug)
      DebugInfoKind = toolchain::codegenoptions::FullDebugInfo;
  }
  ...
  RenderDebugEnablingArgs(Args, CmdArgs, DebugInfoKind, EffectiveDWARFVersion,
                          T.isOSAIX() && !HasDebuggerTuning
                              ? toolchain::DebuggerKind::Default
                              : DebuggerTuning);
  ...
  if (DwarfFission != DwarfFissionKind::None ||
    ...
    if ((DebuggerTuning != toolchain::DebuggerKind::LLDB || OptionSet) &&
        (!PubnamesArg ||
  ...
  bool ForwardTemplateParams = DebuggerTuning == toolchain::DebuggerKind::SCE;
  ...
  bool UseDebugTemplateAlias =
      DebuggerTuning == toolchain::DebuggerKind::SCE && RequestedDWARFVersion >= 4;
  ...
  if (Args.hasFlag(options::OPT_gomit_unreferenced_methods,
      ...
      (DebugInfoKind == toolchain::codegenoptions::DebugInfoConstructor ||
       DebugInfoKind == toolchain::codegenoptions::LimitedDebugInfo) &&
      !EnableTypeUnits) {
  ...
  // scope?
  if (DebuggerTuning == toolchain::DebuggerKind::SCE)
  ...
  if (Args.hasFlag(options::OPT_fjmc, options::OPT_fno_jmc, false)) {
    if (TC.getTriple().isOSBinFormatELF() ||
      ...
      if (DebugInfoKind >= toolchain::codegenoptions::DebugInfoConstructor)
        CmdArgs.push_back("-fjmc");
      else if (D.IsCLMode())
        D.Diag(language::Core::diag::warn_drv_jmc_requires_debuginfo) << "/JMC"
                                                             << "'/Zi', '/Z7'";
      else
        D.Diag(language::Core::diag::warn_drv_jmc_requires_debuginfo) << "-fjmc"
                                                             << "-g";
    } else {
      D.Diag(language::Core::diag::warn_drv_fjmc_for_elf_only);
    }
  }
...
static void ProcessVSRuntimeLibrary(const ToolChain &TC, const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fms_runtime_lib_EQ)) {
    RTOptionID = toolchain::StringSwitch<unsigned>(A->getValue())
  ...
  case options::OPT__SLASH_MTd:
    ...
    break;
  default:
    toolchain_unreachable("Unexpected option ID.");
  }
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const toolchain::Triple &RawTriple = TC.getTriple();
  const toolchain::Triple &Triple = TC.getEffectiveTriple();
  ...
  for (const InputInfo &I : Inputs) {
    if (&I == &Input || I.getType() == types::TY_Nothing) {
      ...
      toolchain_unreachable("unexpectedly given multiple inputs");
    }
  }

  const toolchain::Triple *AuxTriple =
  ...
  if (IsCuda || IsHIP) {
    ...
    if (JA.isDeviceOffloading(Action::OFK_Cuda) ||
      ...
      if (IsCuda) {
        ...
        // https://github.com/toolchain/toolchain-project/issues/58410
    ...
    if (JA.isDeviceOffloading(Action::OFK_HIP) &&
        (getToolChain().getTriple().isAMDGPU() ||
         (getToolChain().getTriple().isSPIRV() &&
          getToolChain().getTriple().getVendor() == toolchain::Triple::AMD))) {
  ...
  if (IsSYCL) {
    if (IsSYCLDevice) {
      ...
      toolchain::Triple AuxT = C.getDefaultToolChain().getTriple();
  ...
  if (Triple.isOSWindows() && (Triple.getArch() == toolchain::Triple::arm ||
                               Triple.getArch() == toolchain::Triple::thumb)) {
    unsigned Offset = Triple.getArch() == toolchain::Triple::arm ? 4 : 6;
    unsigned Version = 0;
    bool Failure =
  ...
  if (isa<AnalyzeJobAction>(JA)) {
    ...
    if (JA.getType() == types::TY_Nothing) {
      ...
      CmdArgs.push_back("-emit-toolchain");
    } else if (JA.getType() == types::TY_LLVM_BC ||
               JA.getType() == types::TY_LTO_BC) {
      // Emit textual toolchain IR for AMDGPU offloading for -emit-toolchain -S
      if (Triple.isAMDGCN() && IsOpenMPDevice && Args.hasArg(options::OPT_S) &&
          Args.hasArg(options::OPT_emit_toolchain)) {
        CmdArgs.push_back("-emit-toolchain");
      } else {
        CmdArgs.push_back("-emit-toolchain-bc");
      }
    ...
    if (JA.getType() == types::TY_LLVM_BC)
      CmdArgs.push_back("-emit-toolchain-uselists");
  ...
  if (C.getDriver().embedBitcodeInObject() && !IsUsingLTO &&
    ...
    Args.AddLastArg(CmdArgs, options::OPT_fembed_bitcode_EQ);
    // Disable all toolchain IR level optimizations.
    CmdArgs.push_back("-disable-toolchain-passes");
    ...
    static const constexpr unsigned kBitcodeOptionIgnorelist[] = {
        ...
        options::OPT_mtoolchain,
        options::OPT_mmlir,
    };
    for (const auto &A : Args)
      if (toolchain::is_contained(kBitcodeOptionIgnorelist, A->getOption().getID()))
  ...
  if (Args.hasFlag(options::OPT_fno_verify_intermediate_code,
    ...
    CmdArgs.push_back("-disable-toolchain-verifier");
  }
  ...
  if (Args.hasFlag(options::OPT_fdiscard_value_names,
    ...
    if (Args.hasArg(options::OPT_fdiscard_value_names) &&
        toolchain::any_of(Inputs, [](const language::Core::driver::InputInfo &II) {
  ...
  if (Triple.isOSzOS()) {
    ...
    auto findMacroDefinition = [&](const std::string &Macro) {
      ...
      return toolchain::any_of(MacroDefs, [&](const std::string &M) {
  ...
  bool IsROPI = RelocationModel == toolchain::Reloc::ROPI ||
                RelocationModel == toolchain::Reloc::ROPI_RWPI;
  bool IsRWPI = RelocationModel == toolchain::Reloc::RWPI ||
                RelocationModel == toolchain::Reloc::ROPI_RWPI;
  ...
  if (RelocationModel == toolchain::Reloc::ROPI ||
      RelocationModel == toolchain::Reloc::ROPI_RWPI)
    CmdArgs.push_back("-fropi");
  if (RelocationModel == toolchain::Reloc::RWPI ||
      RelocationModel == toolchain::Reloc::ROPI_RWPI)
  ...
  if (Triple.isOSBinFormatELF()) {
    ...
    if (RelocationModel != toolchain::Reloc::Static && !IsPIE) {
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fveclib)) {
    ...
    if (Name == "SVML") {
      ...
      if (Triple.getArch() != toolchain::Triple::aarch64 &&
          Triple.getArch() != toolchain::Triple::aarch64_be &&
          Triple.getArch() != toolchain::Triple::riscv64)
        D.Diag(diag::err_drv_unsupported_opt_for_target)
            << Name << Triple.getArchName();
    }
    A->render(Args, CmdArgs);
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mdouble_EQ)) {
    if (TC.getArch() == toolchain::Triple::avr)
      A->render(Args, CmdArgs);
    else
      D.Diag(diag::err_drv_unsupported_opt_for_target)
          << A->getAsString(Args) << TripleStr;
  ...
  // This is a coarse approximation of what toolchain-gcc actually does, both
  ...
  if (!Args.hasArg(options::OPT_fallow_unsupported)) {
    ...
    if (types::isCXX(InputType) && RawTriple.isOSDarwin() &&
        TC.getArch() == toolchain::Triple::x86) {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_fcf_runtime_abi_EQ)) {
    ...
    if (!toolchain::is_contained(kCFABIs, StringRef(A->getValue())))
  ...
  if (Args.hasArg(options::OPT_fstack_usage)) {
    ...
    if (Arg *OutputOpt = Args.getLastArg(options::OPT_o)) {
      ...
      toolchain::sys::path::replace_extension(OutputFilename, "su");
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fmessage_length_EQ)) {
    ...
    MessageLength = toolchain::sys::Process::StandardErrColumns();
  }
  if (MessageLength != 0)
    CmdArgs.push_back(
  ...
  if (Args.hasFlag(options::OPT_foffload_via_toolchain,
                   options::OPT_fno_offload_via_toolchain, false)) {
    CmdArgs.append({"--offload-new-driver", "-foffload-via-toolchain"});
  ...
  for (const auto &Filename :
    ...
    if (D.getVFS().exists(Filename))
      ...
      D.Diag(language::Core::diag::err_drv_no_such_file) << Filename;
  }
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fpatchable_function_entry_EQ)) {
    ...
    if (!Triple.isAArch64() && !Triple.isLoongArch() && !Triple.isRISCV() &&
        !Triple.isX86() &&
        !(!Triple.isOSAIX() && (Triple.getArch() == toolchain::Triple::ppc ||
                                Triple.getArch() == toolchain::Triple::ppc64)))
      D.Diag(diag::err_drv_unsupported_opt_for_target)
          << A->getAsString(Args) << TripleStr;
  ...
  if (TC.SupportsProfiling()) {
    ...
    toolchain::Triple::ArchType Arch = TC.getArch();
    if (Arg *A = Args.getLastArg(options::OPT_mfentry)) {
      if (Arch == toolchain::Triple::systemz || TC.getTriple().isX86())
        A->render(Args, CmdArgs);
      else
        D.Diag(diag::err_drv_unsupported_opt_for_target)
            << A->getAsString(Args) << TripleStr;
    ...
    if (Arg *A = Args.getLastArg(options::OPT_mnop_mcount)) {
      if (Arch == toolchain::Triple::systemz)
        A->render(Args, CmdArgs);
      else
        D.Diag(diag::err_drv_unsupported_opt_for_target)
            << A->getAsString(Args) << TripleStr;
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fcf_protection_EQ)) {
    ...
    if (Arg *SA = Args.getLastArg(options::OPT_mcf_branch_label_scheme_EQ))
      CmdArgs.push_back(Args.MakeArgString(Twine("-mcf-branch-label-scheme=") +
                                           SA->getValue()));
  } else if (Triple.isOSOpenBSD() && Triple.getArch() == toolchain::Triple::x86_64) {
  ...
  if (!(IsCudaDevice || IsHIPDevice)) {
    if (Arg *A = getLastProfileSampleUseArg(Args)) {
      ...
      if (!toolchain::sys::fs::exists(fname))
  ...
  if (Args.hasFlag(options::OPT_fshort_enums, options::OPT_fno_short_enums,
                   TC.getArch() == toolchain::Triple::hexagon || Triple.isOSzOS()))
    CmdArgs.push_back("-fshort-enums");
  ...
  if (!Args.hasFlag(
          ...
          !RawTriple.isOSAIX() &&
              ...
              ((RawTriple.getVendor() != toolchain::Triple::MipsTechnologies) ||
  ...
  if (ImplyVCPPCVer) {
    ...
    if (const Arg *StdArg = Args.getLastArg(options::OPT__SLASH_std)) {
      Std = StdArg;
      LanguageStandard = toolchain::StringSwitch<StringRef>(StdArg->getValue())
                             .Case("c11", "-std=c11")
                             .Case("c17", "-std=c17")
  ...
  if (Args.hasFlag(options::OPT_fdelayed_template_parsing,
    ...
    if (HaveCxx20)
      D.Diag(language::Core::diag::warn_drv_delayed_template_parsing_after_cxx20);
  ...
  if (A) {
    ...
    default:
      break;
    case toolchain::ExceptionHandling::DwarfCFI:
      CmdArgs.push_back("-exception-model=dwarf");
      break;
    case toolchain::ExceptionHandling::SjLj:
      CmdArgs.push_back("-exception-model=sjlj");
      break;
    case toolchain::ExceptionHandling::WinEH:
      CmdArgs.push_back("-exception-model=seh");
      break;
    }
  }
  ...
  // Forward -Xclang arguments to -cc1, and -mtoolchain arguments to the LLVM option
  // parser.
  for (auto Arg : Args.filtered(options::OPT_Xclang)) {
    Arg->claim();
  ...
  for (const Arg *A : Args.filtered(options::OPT_mtoolchain)) {
    ...
    // it and developers have been trained to spell it with -mtoolchain. Both
    // spellings are now deprecated and should be removed.
    if (StringRef(A->getValue(0)) == "-disable-toolchain-optzns") {
      CmdArgs.push_back("-disable-toolchain-optzns");
    } else {
      A->render(Args, CmdArgs);
    }
  }
  ...
  // CompileJobAction, use -disable-toolchain-passes to get pristine IR generated
  // by the frontend.
  ...
  if (IsCudaDevice || IsHIPDevice) {
    ...
    if (!InlineThresh.empty()) {
      ...
      CmdArgs.append({"-mtoolchain", Args.MakeArgStringRef(ArgStr)});
    }
  }
  ...
  if (IsOpenMPDevice) {
    ...
    // If we are offloading cuda/hip via toolchain, it's also "cuda device code".
    if (Args.hasArg(options::OPT_foffload_via_toolchain))
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fglobal_isel,
    ...
    if (A->getOption().matches(options::OPT_fglobal_isel)) {
      ...
      // some kind.
      bool IsArchSupported = Triple.getArch() == toolchain::Triple::aarch64;
  ...
  if (Output.getType() == types::TY_Dependencies) {
    ...
    if (Output.getType() == language::Core::driver::types::TY_IFS_CPP ||
        Output.getType() == language::Core::driver::types::TY_IFS) {
      SmallString<128> OutputFilename(Output.getFilename());
      toolchain::sys::path::replace_extension(OutputFilename, "ifs");
      CmdArgs.push_back("-o");
      CmdArgs.push_back(Args.MakeArgString(OutputFilename));
    } else {
      CmdArgs.push_back("-o");
  ...
  // Disable warnings for clang -E -emit-toolchain foo.c
...
// Determine the non-fragile ABI version to use.
  ...
  if (toolchain::any_of(inputs, [](const InputInfo &input) {
    ...
    for (size_t I = 0, E = EHVal.size(); I != E; ++I) {
      ...
      case 'a':
        ...
        if (EH.Asynch) {
          ...
          if (!isWindowsMSVC) {
            EH.Asynch = false;
            D.Diag(language::Core::diag::warn_drv_unused_argument) << "/EHa" << EHVal;
            continue;
          }
          EH.Synch = false;
        }
      ...
      D.Diag(language::Core::diag::err_drv_invalid_value) << "/EH" << EHVal;
      break;
    }
  }
...
void Clang::AddClangCLArgs(const ArgList &Args, types::ID InputType,
 ...
 if (Args.hasArg(options::OPT__SLASH_kernel)) {
   toolchain::Triple::ArchType Arch = getToolChain().getArch();
   std::vector<std::string> Values =
       Args.getAllArgValues(options::OPT__SLASH_arch);
   if (!Values.empty()) {
     toolchain::SmallSet<std::string, 4> SupportedArches;
     if (Arch == toolchain::Triple::x86)
  ...
  if (MostGeneralArg && BestCaseArg)
    D.Diag(language::Core::diag::err_drv_argument_not_allowed_with)
  ...
  if (Arg *CCArg =
    ...
    case options::OPT__SLASH_Gr:
      ArchSupported = Arch == toolchain::Triple::x86;
    ...
    case options::OPT__SLASH_Gv:
      ArchSupported = Arch == toolchain::Triple::x86 || Arch == toolchain::Triple::x86_64;
...
const char *Clang::getBaseInputName(const ArgList &Args,
                                    const InputInfo &Input) {
  return Args.MakeArgString(toolchain::sys::path::filename(Input.getBaseInput()));
}
...
const char *Clang::getDependencyFileName(const ArgList &Args,
  ...
  if (Arg *OutputOpt = Args.getLastArg(options::OPT_o)) {
    SmallString<128> OutputFilename(OutputOpt->getValue());
    toolchain::sys::path::replace_extension(OutputFilename, toolchain::Twine('d'));
...
void ClangAs::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // and "clang -emit-toolchain -c foo.s"
  Args.ClaimAllArgs(options::OPT_emit_toolchain);
  ...
  if (SourceAction->getType() == types::TY_Asm ||
    ...
    DebugInfoKind = (WantDebug ? toolchain::codegenoptions::DebugInfoConstructor
                               : toolchain::codegenoptions::NoDebugInfo);
  ...
  Args.AddAllArgs(CmdArgs, options::OPT_mtoolchain);

  if (DebugInfoKind > toolchain::codegenoptions::NoDebugInfo && Output.isFilename())
    addDebugObjectName(Args, CmdArgs, DebugCompilationDir,
                       Output.getFilename());
...
void OffloadBundler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (unsigned I = 0; I < Inputs.size(); ++I) {
    ...
    Triples += '-';
    Triples +=
        CurTC->getTriple().normalize(toolchain::Triple::CanonicalForm::FOUR_IDENT);
...
void OffloadBundler::ConstructJobMultipleOutputs(
    ...
    const InputInfoList &Inputs, const toolchain::opt::ArgList &TCArgs,
    const char *LinkingOutput) const {
  ...
  for (unsigned I = 0; I < DepInfo.size(); ++I) {
    ...
    Triples += Dep.DependentToolChain->getTriple().normalize(
        toolchain::Triple::CanonicalForm::FOUR_IDENT);
...
void OffloadPackager::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (const InputInfo &Input : Inputs) {
    ...
    getTargetFeatures(TC->getDriver(), TC->getTriple(), TCArgs, Features,
                      false);
    toolchain::copy_if(Features, std::back_inserter(FeatureArgs),
    ...
    CmdArgs.push_back(Args.MakeArgString("--image=" + toolchain::join(Parts, ",")));
  }
...
void LinkerWrapper::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // compilation job.
  const toolchain::DenseSet<unsigned> CompilerOptions{
      OPT_v,
      OPT_cuda_path_EQ,
      OPT_rocm_path_EQ,
      OPT_O_Group,
      ...
      OPT_flto_EQ};
  const toolchain::DenseSet<unsigned> LinkerOptions{OPT_mtoolchain, OPT_Zlinker_input};
  auto ShouldForwardForToolChain = [&](Arg *A, const ToolChain &TC) {
    // Don't forward -mtoolchain to toolchains that don't support LLVM.
    return TC.HasNativeLLVMSupport() || A->getOption().getID() != OPT_mtoolchain;
  };
  auto ShouldForward = [&](const toolchain::DenseSet<unsigned> &Set, Arg *A,
                           const ToolChain &TC) {
  ...
  for (Action::OffloadKind Kind : {Action::OFK_Cuda, Action::OFK_OpenMP,
    ...
    for (auto &I : toolchain::make_range(TCRange)) {
