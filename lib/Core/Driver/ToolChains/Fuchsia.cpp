...
void fuchsia::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const toolchain::Triple &Triple = ToolChain.getEffectiveTriple();
  ...
  if (toolchain::sys::path::filename(Exec).equals_insensitive("ld.lld") ||
      toolchain::sys::path::stem(Exec).equals_insensitive("ld.lld")) {
    CmdArgs.push_back("-z");
  ...
  if (ToolChain.getArch() == toolchain::Triple::aarch64) {
...
Fuchsia::Fuchsia(const Driver &D, const toolchain::Triple &Triple,
                 const ArgList &Args)
  ...
  auto FilePaths = [&](const Multilib &M) -> std::vector<std::string> {
    ...
    if (std::optional<std::string> Path = getStdlibPath()) {
      ...
      toolchain::sys::path::append(P, M.gccSuffix());
  ...
  Multilibs.FilterOut([&](const Multilib &M) {
    ...
    return toolchain::all_of(RD, [&](std::string P) { return !getVFS().exists(P); });
  });
...
void Fuchsia::AddClangCXXStdlibIncludeArgs(const ArgList &DriverArgs,
  ...
  auto AddCXXIncludePath = [&](StringRef Path) {
    ...
    if (!SelectedMultilibs.empty() && !SelectedMultilibs.back().isDefault()) {
      ...
      toolchain::sys::path::append(TargetDir, Target, M.gccSuffix(), "c++", Version);
