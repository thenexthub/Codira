...
CudaVersion parseCudaHFile(toolchain::StringRef Input) {
...
CudaInstallationDetector::CudaInstallationDetector(
  ...
  if (Args.hasArg(language::Core::driver::options::OPT_cuda_path_EQ)) {
    Candidates.emplace_back(
        Args.getLastArgValue(language::Core::driver::options::OPT_cuda_path_EQ).str());
    ...
    if (!Args.hasArg(language::Core::driver::options::OPT_cuda_path_ignore_env)) {
      ...
      if (toolchain::ErrorOr<std::string> ptxas =
              toolchain::sys::findProgramByName("ptxas")) {
        SmallString<256> ptxasAbsolutePath;
        toolchain::sys::fs::real_path(*ptxas, ptxasAbsolutePath);

        StringRef ptxasDir = toolchain::sys::path::parent_path(ptxasAbsolutePath);
        if (toolchain::sys::path::filename(ptxasDir) == "bin")
          Candidates.emplace_back(
              std::string(toolchain::sys::path::parent_path(ptxasDir)),
              /*StrictChecking=*/true);
      }
    }
    ...
    Distro Dist(FS, toolchain::Triple(toolchain::sys::getProcessTriple()));
  ...
  for (const auto &Candidate : Candidates) {
    ...
    if (Version >= CudaVersion::CUDA_90) {
      ...
      std::error_code EC;
      for (toolchain::vfs::directory_iterator LI = FS.dir_begin(LibDevicePath, EC),
                                         LE;
...
void CudaInstallationDetector::AddCudaIncludeArgs(
  ...
  if (!DriverArgs.hasArg(options::OPT_nobuiltininc)) {
    ...
    toolchain::sys::path::append(P, "cuda_wrappers");
...
void NVPTX::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (DIKind == EmitSameDebugInfoAsHost) {
    ...
    if (A->getOption().matches(options::OPT_O4) ||
      ...
      OOpt = toolchain::StringSwitch<const char *>(A->getValue())
                 .Case("1", "1")
                 .Case("2", "2")
                 .Case("3", "3")
                 .Case("s", "2")
    ...
    CmdArgs.push_back(Args.MakeArgString(toolchain::Twine("-O") + OOpt));
  } else {
  ...
  C.addCommand(std::make_unique<Command>(
      JA, *this,
      ResponseFileSupport{ResponseFileSupport::RF_Full, toolchain::sys::WEM_UTF8,
                          "--options-file"},
...
void NVPTX::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Args.hasArg(options::OPT_cuda_path_EQ) || TC.CudaInstallation.isValid()) {
    StringRef CudaPath = Args.getLastArgValue(
        ...
        toolchain::sys::path::parent_path(TC.CudaInstallation.getBinPath()));
...
void NVPTX::getNVPTXTargetFeatures(const Driver &D, const toolchain::Triple &Triple,
                                   const toolchain::opt::ArgList &Args,
                                   std::vector<StringRef> &Features) {
...
#undef CASE_CUDA_VERSION
  ...
  case language::Core::CudaVersion::NEW:
    PtxFeature = "+ptx86";
    break;
  default:
    PtxFeature = "+ptx42";
...
/// toolchain.
NVPTXToolChain::NVPTXToolChain(const Driver &D, const toolchain::Triple &Triple,
...
NVPTXToolChain::NVPTXToolChain(const Driver &D, const toolchain::Triple &Triple,
                               const ArgList &Args)
    : NVPTXToolChain(D, Triple, toolchain::Triple(LLVM_HOST_TRIPLE), Args) {}

toolchain::opt::DerivedArgList *
NVPTXToolChain::TranslateArgs(const toolchain::opt::DerivedArgList &Args,
                              StringRef BoundArch,
  ...
  for (Arg *A : Args)
    if (!toolchain::is_contained(*DAL, A))
      DAL->append(A);
  ...
  if (!DAL->hasArg(options::OPT_march_EQ) && OffloadKind != Action::OFK_None) {
    ...
    if (!GPUsOrErr) {
      getDriver().Diag(diag::err_drv_undetermined_gpu_arch)
          << getArchName() << toolchain::toString(GPUsOrErr.takeError()) << "-march";
    } else {
      if (GPUsOrErr->size() > 1)
        getDriver().Diag(diag::warn_drv_multi_gpu_arch)
            << getArchName() << toolchain::join(*GPUsOrErr, ", ") << "-march";
      DAL->AddJoinedArg(nullptr, Opts.getOption(options::OPT_march_EQ),
...
bool NVPTXToolChain::supportsDebugInfoOption(const toolchain::opt::Arg *A) const {
...
CudaToolChain::CudaToolChain(const Driver &D, const toolchain::Triple &Triple,
...
void CudaToolChain::addClangTargetOptions(
  ...
  CC1Args.append({"-fcuda-is-device", "-mtoolchain",
  ...
  // headers rely on that. https://github.com/toolchain/toolchain-project/issues/58410
  if (CudaInstallation.version() >= CudaVersion::CUDA_90)
    CC1Args.push_back("-fcuda-allow-variadic-functions");

  if (DriverArgs.hasFlag(options::OPT_fcuda_short_ptr,
                         options::OPT_fno_cuda_short_ptr, false))
    CC1Args.append({"-mtoolchain", "--nvptx-short-ptr"});
  ...
  if (DriverArgs.hasFlag(options::OPT_foffload_via_toolchain,
                         options::OPT_fno_offload_via_toolchain, false))
    return;

  language::Core::CudaVersion CudaInstallationVersion = CudaInstallation.version();
...
toolchain::DenormalMode CudaToolChain::getDefaultDenormalModeForType(
...
toolchain::opt::DerivedArgList *
CudaToolChain::TranslateArgs(const toolchain::opt::DerivedArgList &Args,
                             StringRef BoundArch,
