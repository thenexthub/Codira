...
static bool canExecute(toolchain::vfs::FileSystem &VFS, StringRef Path) {
  auto Status = VFS.status(Path);
  if (!Status)
    return false;
  return (Status->getPermissions() & toolchain::sys::fs::perms::all_exe) != 0;
}
...
static std::string FindVisualStudioExecutable(const ToolChain &TC,
  ...
  SmallString<128> FilePath(
      MSVC.getSubDirectoryPath(toolchain::SubDirectoryType::Bin));
  toolchain::sys::path::append(FilePath, Exe);
...
void visualstudio::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (const Arg *A = Args.getLastArg(options::OPT__SLASH_diasdkdir,
    ...
    toolchain::SmallString<128> DIAPath(A->getValue());
    if (A->getOption().getID() == options::OPT__SLASH_winsysroot)
      toolchain::sys::path::append(DIAPath, "DIA SDK");

    // The DIA SDK always uses the legacy vc arch, even in new MSVC versions.
    toolchain::sys::path::append(DIAPath, "lib",
                            toolchain::archToLegacyVCArch(TC.getArch()));
    CmdArgs.push_back(Args.MakeArgString(Twine("-libpath:") + DIAPath));
  }
  if (!toolchain::sys::Process::GetEnv("LIB") ||
      Args.hasArg(options::OPT__SLASH_vctoolsdir,
                  options::OPT__SLASH_vctoolsversion,
                  options::OPT__SLASH_winsysroot)) {
    CmdArgs.push_back(Args.MakeArgString(
        Twine("-libpath:") +
        TC.getSubDirectoryPath(toolchain::SubDirectoryType::Lib)));
    CmdArgs.push_back(Args.MakeArgString(
        Twine("-libpath:") +
        TC.getSubDirectoryPath(toolchain::SubDirectoryType::Lib, "atlmfc")));
  }
  if (!toolchain::sys::Process::GetEnv("LIB") ||
  ...
  if (DLL) {
    ...
    toolchain::sys::path::replace_extension(ImplibName, "lib");
  ...
  if (TC.getSanitizerArgs(Args).needsAsanRt()) {
    ...
    if (Args.hasArg(options::OPT__SLASH_MD, options::OPT__SLASH_MDd) ||
        toolchain::is_contained(defines, "_DLL")) {
  ...
  toolchain::SmallString<128> linkPath;
  if (Linker.equals_insensitive("link")) {
    ...
    if (!TC.FoundMSVCInstall() && !canExecute(TC.getVFS(), linkPath)) {
      toolchain::SmallString<128> ClPath;
      ClPath = TC.GetProgramPath("cl.exe");
      if (canExecute(TC.getVFS(), ClPath)) {
        linkPath = toolchain::sys::path::parent_path(ClPath);
        toolchain::sys::path::append(linkPath, "link.exe");
        if (!canExecute(TC.getVFS(), linkPath))
          C.getDriver().Diag(language::Core::diag::warn_drv_msvc_not_found);
      } else {
        C.getDriver().Diag(language::Core::diag::warn_drv_msvc_not_found);
      }
    }
    ...
    // This doesn't attempt to handle toolchain::ToolsetLayout::DevDivInternal.
    if (TC.getIsVS2017OrNewer() &&
        toolchain::Triple(toolchain::sys::getProcessTriple()).getArch() != TC.getArch()) {
      auto HostArch = toolchain::Triple(toolchain::sys::getProcessTriple()).getArch();
      ...
      std::string EnvBlock;
      if (!toolchain::convertUTF16ToUTF8String(
              toolchain::ArrayRef<char>(reinterpret_cast<char *>(EnvBlockWide.get()),
      ...
      for (const char *Cursor = EnvBlock.data(); *Cursor != '\0';) {
        toolchain::StringRef EnvVar(Cursor);
        if (EnvVar.starts_with_insensitive("path=")) {
          constexpr size_t PrefixLen = 5; // strlen("path=")
          Environment.push_back(Args.MakeArgString(
              EnvVar.substr(0, PrefixLen) +
              TC.getSubDirectoryPath(toolchain::SubDirectoryType::Bin) +
              toolchain::Twine(toolchain::sys::EnvPathSeparator) +
              TC.getSubDirectoryPath(toolchain::SubDirectoryType::Bin, HostArch) +
...
MSVCToolChain::MSVCToolChain(const Driver &D, const toolchain::Triple &Triple,
                             const ArgList &Args)
    : ToolChain(D, Triple, Args), CudaInstallation(D, Triple, Args),
      RocmInstallation(D, Triple, Args), SYCLInstallation(D, Triple, Args) {
  getProgramPaths().push_back(getDriver().Dir);

  std::optional<toolchain::StringRef> VCToolsDir, VCToolsVersion;
  ...
  toolchain::findVCToolChainViaCommandLine(getVFS(), VCToolsDir, VCToolsVersion,
                                      WinSysRoot, VCToolChainPath, VSLayout) ||
      toolchain::findVCToolChainViaEnvironment(getVFS(), VCToolChainPath,
                                          VSLayout) ||
      toolchain::findVCToolChainViaSetupConfig(getVFS(), VCToolsVersion,
                                          VCToolChainPath, VSLayout) ||
      toolchain::findVCToolChainViaRegistry(VCToolChainPath, VSLayout);
}
...
Tool *MSVCToolChain::buildAssembler() const {
  ...
  getDriver().Diag(language::Core::diag::err_no_external_assembler);
  return nullptr;
}
...
bool MSVCToolChain::isPIEDefault(const toolchain::opt::ArgList &Args) const {
  return false;
}
...
std::string
MSVCToolChain::getSubDirectoryPath(toolchain::SubDirectoryType Type,
                                   toolchain::StringRef SubdirParent) const {
  return toolchain::getSubDirectoryPath(Type, VSLayout, VCToolChainPath, getArch(),
                                   SubdirParent);
}
...
MSVCToolChain::getSubDirectoryPath(toolchain::SubDirectoryType Type,
  ...
  return toolchain::getSubDirectoryPath(Type, VSLayout, VCToolChainPath, TargetArch,
                                   "");
}
...
bool MSVCToolChain::getWindowsSDKLibraryPath(const ArgList &Args,
  ...
  path.clear();
  if (!toolchain::getWindowsSDKDir(getVFS(), WinSdkDir, WinSdkVersion, WinSysRoot,
                              sdkPath, sdkMajor, windowsSDKIncludeVersion,
                              windowsSDKLibVersion))
    return false;

  toolchain::SmallString<128> libPath(sdkPath);
  ...
  if (sdkMajor >= 8)
    toolchain::sys::path::append(libPath, windowsSDKLibVersion, "um");
  return toolchain::appendArchToWindowsSDKLibPath(sdkMajor, libPath, getArch(),
                                             path);
}

bool MSVCToolChain::useUniversalCRT() const {
  return toolchain::useUniversalCRT(VSLayout, VCToolChainPath, getArch(), getVFS());
}
...
bool MSVCToolChain::getUniversalCRTLibraryPath(const ArgList &Args,
  ...
  Path.clear();
  if (!toolchain::getUniversalCRTSdkDir(getVFS(), WinSdkDir, WinSdkVersion,
  ...
  StringRef ArchName = toolchain::archToWindowsSDKArch(getArch());
  if (ArchName.empty())
    return false;

  toolchain::SmallString<128> LibPath(UniversalCRTSdkPath);
  toolchain::sys::path::append(LibPath, "Lib", UCRTVersion, "ucrt", ArchName);
...
static VersionTuple getMSVCVersionFromExe(const std::string &BinDir) {
  ...
  toolchain::sys::path::append(ClExe, "cl.exe");

  std::wstring ClExeWide;
  if (!toolchain::ConvertUTF8toWide(ClExe.c_str(), ClExeWide))
    return Version;
...
void MSVCToolChain::AddSystemIncludeWithSubfolder(
  ...
  toolchain::SmallString<128> path(folder);
  toolchain::sys::path::append(path, subfolder1, subfolder2, subfolder3);
...
void MSVCToolChain::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  auto AddSystemIncludesFromEnv = [&](StringRef Var) -> bool {
    if (auto Val = toolchain::sys::Process::GetEnv(Var)) {
  ...
  if (const Arg *A = DriverArgs.getLastArg(options::OPT__SLASH_diasdkdir,
    ...
    // explicit flags and doesn't automatically look in "DIA SDK" relative
    // to the path we found for VCToolChainPath.
    toolchain::SmallString<128> DIASDKPath(A->getValue());
    if (A->getOption().getID() == options::OPT__SLASH_winsysroot)
      toolchain::sys::path::append(DIASDKPath, "DIA SDK");
  ...
  if (!VCToolChainPath.empty()) {
    addSystemInclude(DriverArgs, CC1Args,
                     getSubDirectoryPath(toolchain::SubDirectoryType::Include));
    addSystemInclude(
        DriverArgs, CC1Args,
        getSubDirectoryPath(toolchain::SubDirectoryType::Include, "atlmfc"));
    ...
    if (toolchain::getWindowsSDKDir(getVFS(), WinSdkDir, WinSdkVersion, WinSysRoot,
      ...
      if (major >= 8) {
        ...
        // Anyway, toolchain::sys::path::append is able to manage it.
...
VersionTuple MSVCToolChain::computeMSVCVersion(const Driver *D,
  ...
  if (MSVT.empty() && IsWindowsMSVC)
    MSVT =
        getMSVCVersionFromExe(getSubDirectoryPath(toolchain::SubDirectoryType::Bin));
  if (MSVT.empty() &&
...
MSVCToolChain::ComputeEffectiveClangTriple(const ArgList &Args,
  ...
  // be needed.
  toolchain::Triple Triple(ToolChain::ComputeEffectiveClangTriple(Args, InputType));
  if (Triple.getEnvironment() == toolchain::Triple::MSVC) {
...
static void TranslateOptArg(Arg *A, toolchain::opt::DerivedArgList &DAL,
...
static void TranslateDArg(Arg *A, toolchain::opt::DerivedArgList &DAL,
                          const OptTable &Opts) {
...
static void TranslatePermissive(Arg *A, toolchain::opt::DerivedArgList &DAL,
                                const OptTable &Opts) {
...
static void TranslatePermissiveMinus(Arg *A, toolchain::opt::DerivedArgList &DAL,
                                     const OptTable &Opts) {
...
MSVCToolChain::TranslateArgs(const toolchain::opt::DerivedArgList &Args,
                             StringRef BoundArch,
  ...
  bool SupportsForcingFramePointer = getArch() != toolchain::Triple::x86_64;
