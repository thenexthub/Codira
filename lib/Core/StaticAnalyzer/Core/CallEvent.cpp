...
static void findPtrToConstParams(toolchain::SmallSet<unsigned, 4> &PreserveArgs,
...
ProgramStateRef CallEvent::invalidateRegions(unsigned BlockCount,
  ...
  toolchain::SmallSet<unsigned, 4> PreserveArgs;
...
LLVM_DUMP_METHOD void CallEvent::dump() const { dump(toolchain::errs()); }
...
QualType CallEvent::getDeclaredResultType(const Decl *D) {
  ...
  toolchain_unreachable("unknown callable kind");
}
...
static SVal processArgument(SVal Value, const Expr *ArgumentExpr,
  ...
  if (isTransparentUnion(ParamType) &&
    ...
    toolchain::ImmutableList<SVal> CompoundSVals = BVF.getEmptySValList();
...
RuntimeDefinition AnyFunctionCall::getRuntimeDefinition() const {
  ...
  LLVM_DEBUG({
    if (IsAutosynthesized)
      toolchain::dbgs() << "Using autosynthesized body for " << FD->getName()
                   << "\n";
  });
  ...
  toolchain::Expected<const FunctionDecl *> CTUDeclOrError =
...
using ObjCMessageDataTy = toolchain::PointerIntPair<const PseudoObjectExpr *, 2>;
...
bool ObjCMethodCall::canBeOverridenInSubclass(ObjCInterfaceDecl *IDecl,
  ...
  toolchain_unreachable("The while loop should always terminate.");
}
...
template <> struct DenseMapInfo<PrivateMethodKey> {
  ...
  static unsigned getHashValue(const PrivateMethodKey &Key) {
    return toolchain::hash_combine(
        toolchain::hash_code(InterfaceInfo::getHashValue(Key.Interface)),
        toolchain::hash_code(SelectorInfo::getHashValue(Key.LookupSelector)),
        Key.IsClassMethod);
  }
...
lookupRuntimeDefinition(const ObjCInterfaceDecl *Interface,
  ...
  using PrivateMethodCache =
      toolchain::DenseMap<PrivateMethodKey, std::optional<const ObjCMethodDecl *>>;
...
CallEventManager::getCaller(const StackFrameContext *CalleeCtx,
  ...
  if (CallSite) {
    ...
    if (const auto *CE = dyn_cast<CXXConstructExpr>(CallSite))
      ...
      toolchain_unreachable("This is not an inlineable statement");
    }
  }
