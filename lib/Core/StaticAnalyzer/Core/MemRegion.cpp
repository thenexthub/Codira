...
#include "toolchain/Support/CheckedArithmetic.h"
...
[[maybe_unused]] static bool isAReferenceTypedValueRegion(const MemRegion *R) {
  const auto *TyReg = toolchain::dyn_cast<TypedValueRegion>(R);
...
void MemSpaceRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ID.AddInteger(static_cast<unsigned>(getKind()));
}

void StackSpaceRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ID.AddInteger(static_cast<unsigned>(getKind()));
  ID.AddPointer(getStackFrame());
}

void StaticGlobalSpaceRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ID.AddInteger(static_cast<unsigned>(getKind()));
  ID.AddPointer(getCodeRegion());
}

void StringRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
...
void ObjCStringRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
...
void AllocaRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
...
void AllocaRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  ProfileRegion(ID, Ex, Cnt, superRegion);
}

void CompoundLiteralRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  CompoundLiteralRegion::ProfileRegion(ID, CL, superRegion);
}

void CompoundLiteralRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
...
void CXXThisRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
...
void CXXThisRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  CXXThisRegion::ProfileRegion(ID, ThisPointerTy, superRegion);
}

void FieldRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, getDecl(), superRegion);
}

void ObjCIvarRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
...
void ObjCIvarRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, getDecl(), superRegion);
}

void NonParamVarRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
                                      const VarDecl *VD,
                                      const MemRegion *superRegion) {
...
void NonParamVarRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, getDecl(), superRegion);
}

void ParamVarRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID, const Expr *OE,
...
void ParamVarRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, getOriginExpr(), getIndex(), superRegion);
}

void SymbolicRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID, SymbolRef sym,
...
void SymbolicRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  SymbolicRegion::ProfileRegion(ID, sym, getSuperRegion());
}

void ElementRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
...
void ElementRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  ElementRegion::ProfileRegion(ID, ElementType, Index, superRegion);
}

void FunctionCodeRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
                                       const NamedDecl *FD,
                                       const MemRegion*) {
...
void FunctionCodeRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  FunctionCodeRegion::ProfileRegion(ID, FD, superRegion);
}

void BlockCodeRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
...
void BlockCodeRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  BlockCodeRegion::ProfileRegion(ID, BD, locTy, AC, superRegion);
}

void BlockDataRegion::ProfileRegion(toolchain::FoldingSetNodeID& ID,
...
void BlockDataRegion::Profile(toolchain::FoldingSetNodeID& ID) const {
  BlockDataRegion::ProfileRegion(ID, BC, LC, BlockCount, getSuperRegion());
}

void CXXTempObjectRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
                                        Expr const *Ex,
                                        const MemRegion *sReg) {
  ID.AddPointer(Ex);
  ID.AddPointer(sReg);
...
void CXXTempObjectRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, Ex, getSuperRegion());
}

void CXXLifetimeExtendedObjectRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
                                                    const Expr *E,
                                                    const ValueDecl *D,
                                                    const MemRegion *sReg) {
  ID.AddPointer(E);
...
void CXXLifetimeExtendedObjectRegion::Profile(
    toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, Ex, ExD, getSuperRegion());
}

void CXXBaseObjectRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
...
void CXXBaseObjectRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, getDecl(), isVirtual(), superRegion);
}

void CXXDerivedObjectRegion::ProfileRegion(toolchain::FoldingSetNodeID &ID,
                                           const CXXRecordDecl *RD,
                                           const MemRegion *SReg) {
  ID.AddPointer(RD);
  ID.AddPointer(SReg);
...
void CXXDerivedObjectRegion::Profile(toolchain::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, getDecl(), superRegion);
}
...
void MemRegion::printPrettyAsExpr(raw_ostream &) const {
  toolchain_unreachable("This region cannot be printed pretty.");
}
...
std::string MemRegion::getDescriptiveName(bool UseQuotes) const {
  ...
  while ((ER = R->getAs<ElementRegion>())) {
    ...
    if (auto CI = ER->getIndex().getAs<nonloc::ConcreteInt>()) {
      toolchain::SmallString<2> Idx;
      CI->getValue()->toString(Idx);
      ArrayIndices = (toolchain::Twine("[") + Idx.str() + "]" + ArrayIndices).str();
    }
    // Index is symbolic, but may have a descriptive name.
      ...
      ArrayIndices = (toolchain::Twine("[") + Idx + "]" + ArrayIndices).str();
    }
    R = ER->getSuperRegion();
  }
  ...
  if (R) {
    ...
    if (R->canPrintPrettyAsExpr()) {
      ...
      return QuoteIfNeeded(toolchain::Twine(os.str()) + ArrayIndices);
    }
...
DefinedOrUnknownSVal MemRegionManager::getStaticSize(const MemRegion *MR,
  ...
  case MemRegion::FunctionCodeRegionKind:
    return nonloc::SymbolVal(SymMgr.acquire<SymbolExtent>(SR));
  default:
    toolchain_unreachable("Unhandled region");
  }
}
...
static toolchain::PointerUnion<const StackFrameContext *, const VarRegion *>
...
const VarRegion *MemRegionManager::getVarRegion(const VarDecl *D,
  ...
  // Finally handle static locals.
    ...
    toolchain::PointerUnion<const StackFrameContext *, const VarRegion *> V =
  ...
  while (ER) {
    ...
    if (auto CI = index.getAs<nonloc::ConcreteInt>()) {
      ...
      if (int64_t i = CI->getValue()->getSExtValue(); i != 0) {
        ...
        if (auto NewOffset = toolchain::checkedMulAdd(i, size, offset)) {
          offset = *NewOffset;
        } else {
          LLVM_DEBUG(toolchain::dbgs() << "MemRegion::getAsArrayOffset: "
...
void BlockDataRegion::LazyInitializeReferencedVars() {
  ...
  MemRegionManager &MemMgr = getMemRegionManager();
  toolchain::BumpPtrAllocator &A = MemMgr.getAllocator();
...
toolchain::iterator_range<BlockDataRegion::referenced_vars_iterator>
BlockDataRegion::referenced_vars() const {
  return toolchain::make_range(referenced_vars_begin(), referenced_vars_end());
}
