...
#include "language/Core/StaticAnalyzer/Core/PathSensitive/CoreEngine.h"
...
#include "language/Core/StaticAnalyzer/Core/PathSensitive/LoopUnrolling.h"
...
public:
  ...
  void printJson(toolchain::raw_ostream &Out, PrinterHelper *Helper,
...
typedef toolchain::ImmutableMap<ConstructedObjectKey, SVal>
...
// the index is 3.
typedef toolchain::ImmutableMap<
...
typedef toolchain::ImmutableMap<const LocationContext *, unsigned>
...
ProgramStateRef ExprEngine::createTemporaryRegionIfNeeded(
  ...
  for (const SubobjectAdjustment &Adj : toolchain::reverse(Adjustments)) {
    switch (Adj.Kind) {
...
void ExprEngine::removeDead(ExplodedNode *Pred, ExplodedNodeSet &Out,
                            ...
                            ProgramPoint::Kind K) {
  toolchain::TimeTraceScope TimeScope("ExprEngine::removeDead");
...
void ExprEngine::ProcessImplicitDtor(const CFGImplicitDtor D,
  ...
  case CFGElement::DeleteDtor:
    ...
    break;
  default:
    toolchain_unreachable("Unexpected dtor kind.");
  }
...
void ExprEngine::Visit(const Stmt *S, ExplodedNode *Pred,
  ...
  switch (S->getStmtClass()) {
    ...
    case Stmt::ParenExprClass:
      toolchain_unreachable("ParenExprs already handled.");
    case Stmt::GenericSelectionExprClass:
      toolchain_unreachable("GenericSelectionExprs already handled.");
    ...
    case Expr::MSDependentExistsStmtClass:
      toolchain_unreachable("Stmt should not be in analyzer evaluation loop");
    case Stmt::ImplicitValueInitExprClass:
      ...
      toolchain_unreachable("Should be pruned from CFG");

    case Stmt::ObjCSubscriptRefExprClass:
    case Stmt::ObjCPropertyRefExprClass:
      toolchain_unreachable("These are handled by PseudoObjectExpr");
    ...
    case Stmt::CXXDefaultInitExprClass: {
      ...
      if (const auto *DefE = dyn_cast<CXXDefaultArgExpr>(S))
        ...
        toolchain_unreachable("unknown constant wrapper kind");
    ...
    case Stmt::MatrixSubscriptExprClass:
      toolchain_unreachable("Support for MatrixSubscriptExpr is not implemented.");
      break;
...
void ExprEngine::runCheckersForBlockEntrance(const NodeBuilderContext &BldCtx,
                                             ...
                                             ExplodedNode *Pred,
                                             ExplodedNodeSet &Dst) {
  toolchain::PrettyStackTraceFormat CrashInfo(
...
static const Stmt *ResolveCondition(const Stmt *Condition,
  ...
  for (CFGElement Elem : toolchain::reverse(*B)) {
  ...
  toolchain_unreachable("could not resolve condition");
}
...
REGISTER_TRAIT_WITH_PROGRAMSTATE(InitializedGlobalsSet,
                                 toolchain::ImmutableSet<const VarDecl *>)
...
void ExprEngine::processIndirectGoto(IndirectGotoNodeBuilder &builder) {
  ...
  if (std::optional<loc::GotoLabel> LV = V.getAs<loc::GotoLabel>()) {
    ...
    toolchain_unreachable("No block with label.");
  }
...
void ExprEngine::processSwitch(SwitchNodeBuilder& builder) {
  ...
  for ( ; I != EI; ++I) {
    ...
    toolchain::APSInt V1 = Case->getLHS()->EvaluateKnownConstInt(getContext());
...
void ExprEngine::VisitCommonDeclRefExpr(const Expr *Ex, const NamedDecl *D,
  ...
  if (const auto *BD = dyn_cast<BindingDecl>(D)) {
    ...
    // Handle binding to tuple-like structures
      ...
      toolchain_unreachable("An unknown case of structured binding encountered!");
  ...
  toolchain_unreachable("Support for this Decl not implemented.");
}
...
void ExprEngine::VisitArraySubscriptExpr(const ArraySubscriptExpr *A,
  ...
  for (auto *Node : CheckerPreStmt) {
    ...
    if (IsGLValueLike) {
      ...
      toolchain_unreachable("Array subscript should be an lValue when not \
      ...
      if (M->isGLValue() || M->getType()->isArrayType()) {
        ...
        if (!M->isGLValue()) {
          ...
          if (!PE || PE->getCastKind() != CK_ArrayToPointerDecay) {
            toolchain_unreachable("should always be wrapped in ArrayToPointerDecay");
          }
        }
...
struct DOTGraphTraits<ExplodedGraph*> : public DefaultDOTGraphTraits {
  ...
  static bool traverseHiddenNodes(
      ...
      toolchain::function_ref<void(const ExplodedNode *)> PreCallback,
      toolchain::function_ref<void(const ExplodedNode *)> PostCallback,
      toolchain::function_ref<bool(const ExplodedNode *)> Stop) {
    while (true) {
      PreCallback(N);
      if (Stop(N))
        return true;
  ...
  static std::string getNodeLabel(const ExplodedNode *N, ExplodedGraph *G){
    std::string Buf;
    toolchain::raw_string_ostream Out(Buf);
...
void ExprEngine::ViewGraph(bool trim) {
  ...
  toolchain::DisplayGraph(Filename, false, toolchain::GraphProgram::DOT);
}
...
std::string ExprEngine::DumpGraph(bool trim, StringRef Filename) {
  ...
  return toolchain::WriteGraph(&G, "ExprEngine", /*ShortNames=*/false,
...
std::string ExprEngine::DumpGraph(ArrayRef<const ExplodedNode *> Nodes,
  ...
  if (!TrimmedG) {
    toolchain::errs() << "warning: Trimmed ExplodedGraph is empty.\n";
    return "";
  }

  return toolchain::WriteGraph(TrimmedG.get(), "TrimmedExprEngine",
                          /*ShortNames=*/false,
...
void ExprEngine::ConstructInitList(const Expr *E, ArrayRef<Expr *> Args,
  ...
  if (Args.size() > 1 || (E->isPRValue() && IsCompound && !IsTransparent)) {
    toolchain::ImmutableList<SVal> ArgList = getBasicVals().getEmptySValList();
    for (Expr *E : toolchain::reverse(Args))
