...
SValBuilder::SValBuilder(toolchain::BumpPtrAllocator &alloc, ASTContext &context,
...
SVal SValBuilder::convertToArrayIndex(SVal val) {
  ...
  if (std::optional<nonloc::ConcreteInt> CI =
    ...
    const toolchain::APSInt& I = CI->getValue();
...
SVal SValBuilder::evalUnaryOp(ProgramStateRef state, UnaryOperator::Opcode opc,
  ...
  toolchain_unreachable("Unexpected unary operator");
}
...
SVal SValBuilder::evalIntegralCast(ProgramStateRef state, SVal val,
  ...
  toolchain::APSInt ToTypeMax = ToType.getMaxValue();
...
public:
  ...
  SVal VisitConcreteInt(loc::ConcreteInt V) {
    ...
    if (CastTy->isIntegralOrEnumerationType()) {
      toolchain::APSInt Value = V.getValue();
