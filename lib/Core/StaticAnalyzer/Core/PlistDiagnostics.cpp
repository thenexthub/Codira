...
#include "language/Core/Basic/PlistSupport.h"
...
#include "language/Core/Lex/TokenConcatenation.h"
...
namespace {
  class PlistDiagnostics : public PathDiagnosticConsumer {
    ...
    void printBugPath(toolchain::raw_ostream &o, const FIDMap &FM,
...
class PlistPrinter {
  ...
  const cross_tu::CrossTranslationUnitContext &CTU;
  const MacroExpansionContext &MacroExpansions;
  toolchain::SmallVector<const PathDiagnosticMacroPiece *, 0> MacroPieces;
...
void PlistDiagnostics::printBugPath(toolchain::raw_ostream &o, const FIDMap &FM,
                                    const PathPieces &Path) {
  ...
  PathPieces::const_iterator FirstNonNote =
      toolchain::partition_point(Path, [](const PathDiagnosticPieceRef &E) {
        return E->getKind() == PathDiagnosticPiece::Note;
      });
  ...
  for (const auto &Piece : toolchain::make_range(I, Path.end()))
...
void PlistDiagnostics::FlushDiagnosticsImpl(
  ...
  // Open the file.
  std::error_code EC;
  toolchain::raw_fd_ostream o(OutputFile, EC, toolchain::sys::fs::OF_TextWithCRLF);
  if (EC) {
  ...
  if (toolchain::AreStatisticsEnabled() && DiagOpts.ShouldSerializeStats) {
    o << " <key>statistics</key>\n";
    std::string stats;
    toolchain::raw_string_ostream os(stats);
    toolchain::PrintStatisticsJSON(os);
