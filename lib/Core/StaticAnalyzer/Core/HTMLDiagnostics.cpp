...
class HTMLDiagnostics : public PathDiagnosticConsumer {
  ...
  toolchain::StringSet<> EmittedHashes;
...
private:
  ...
  void dumpCoverageData(const PathDiagnostic &D, const PathPieces &path,
                        toolchain::raw_string_ostream &os);
};
...
unsigned getPathSizeWithoutArrows(const PathPieces &Path) {
  ...
  unsigned TotalArrowPieces = toolchain::count_if(
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, const ArrowMap &Indices) {
  OS << "[ ";
  toolchain::interleave(Indices, OS, ",");
  return OS << " ]";
}
...
void ento::createHTMLSingleFileDiagnosticConsumer(
    ...
    const std::string &OutputDir, const Preprocessor &PP,
    const cross_tu::CrossTranslationUnitContext &CTU,
    const language::Core::MacroExpansionContext &MacroExpansions) {
  createTextMinimalPathDiagnosticConsumer(DiagOpts, C, OutputDir, PP, CTU,
                                          MacroExpansions);
...
void ento::createPlistHTMLDiagnosticConsumer(
  ...
  createHTMLDiagnosticConsumer(
      DiagOpts, C, std::string(toolchain::sys::path::parent_path(prefix)), PP, CTU,
      MacroExpansions);
...
void ento::createSarifHTMLDiagnosticConsumer(
    ...
    const MacroExpansionContext &MacroExpansions) {
  createHTMLDiagnosticConsumer(
      DiagOpts, C, std::string(toolchain::sys::path::parent_path(sarif_file)), PP,
      CTU, MacroExpansions);
...
static toolchain::SmallString<32> getIssueHash(const PathDiagnostic &D,
                                          const Preprocessor &PP) {
...
void HTMLDiagnostics::ReportDiag(const PathDiagnostic& D,
  ...
  if (!createdDir) {
    createdDir = true;
    if (std::error_code ec = toolchain::sys::fs::create_directories(Directory)) {
      toolchain::errs() << "warning: could not create directory '"
  ...
  if (report.empty()) {
    toolchain::errs() << "warning: no diagnostics generated for main file.\n";
    return;
  }
  ...
  toolchain::raw_svector_ostream FileName(FileNameStr);
  FileName << "report-";
  ...
  if (DiagOpts.ShouldWriteVerboseReportFilename) {
    ...
    FileName << toolchain::sys::path::filename(Entry->getName()).str() << "-"
  ...
  toolchain::sys::path::append(ResultPath, Directory, FileName.str());
  if (std::error_code EC = toolchain::sys::fs::make_absolute(ResultPath)) {
    toolchain::errs() << "warning: could not make '" << ResultPath
                 << "' absolute: " << EC.message() << '\n';
    return;
  }

  if (std::error_code EC = toolchain::sys::fs::openFileForReadWrite(
          ResultPath, FD, toolchain::sys::fs::CD_CreateNew,
    ...
    if (EC != toolchain::errc::file_exists) {
      toolchain::errs() << "warning: could not create file in '" << Directory
                   << "': " << EC.message() << '\n';
    }
    return;
  }

  toolchain::raw_fd_ostream os(FD, true);

  if (filesMade)
    filesMade->addDiagnostic(D, getName(),
                             toolchain::sys::path::filename(ResultPath));
...
std::string HTMLDiagnostics::GenerateHTML(const PathDiagnostic& D, Rewriter &R,
  ...
  for (auto I : path) {
    ...
    if (toolchain::is_contained(FileIDs, FID))
      continue;
  ...
  if (SupportsCrossFileDiagnostics && FileIDs.size() > 1) {
    ...
    for (auto I : toolchain::drop_begin(FileIDs)) {
      std::string s;
      toolchain::raw_string_ostream os(s);
  ...
  std::string file;
  toolchain::raw_string_ostream os(file);
  for (auto BI : *Buf)
    os << BI;
...
void HTMLDiagnostics::FinalizeHTML(const PathDiagnostic &D, Rewriter &R,
  ...
  toolchain::SmallString<0> DirName;

  if (toolchain::sys::path::is_relative(Entry.getName())) {
    toolchain::sys::fs::current_path(DirName);
    DirName += '/';
  }
  ...
  // Add the name of the file as an <h1> tag.
  {
    ...
    for (const std::string &Metadata :
         toolchain::make_range(D.meta_begin(), D.meta_end())) {
...
<div id='tooltiphint' hidden="true">
  ...
  // Embed meta-data tags.
  {
    ...
    os << "\n<!-- FILENAME " << toolchain::sys::path::filename(Entry.getName()) << " -->\n";
...
static void HandlePopUpPieceEndTag(Rewriter &R,
  ...
  SmallString<256> Buf;
  toolchain::raw_svector_ostream Out(Buf);
  ...
  if (!toolchain::is_contained(PopUpRanges, Range)) {
...
void HTMLDiagnostics::RewriteFile(Rewriter &R, const PathPieces &path,
  ...
  unsigned TotalNotePieces =
      toolchain::count_if(path, [](const PathDiagnosticPieceRef &p) {
  ...
  std::vector<SourceRange> PopUpRanges;
  for (const PathDiagnosticPieceRef &I : toolchain::reverse(path)) {
  ...
  assert(toolchain::is_sorted(ArrowIndices, std::greater<unsigned>()) &&
...
void HTMLDiagnostics::HandlePiece(Rewriter &R, FileID BugFileID,
  ...
  toolchain::MemoryBufferRef Buf = SM.getBufferOrFake(LPosInfo.first);
  ...
  case PathDiagnosticPiece::PopUp:
    toolchain_unreachable("Calls and extra notes should already be handled");
  }
...
<svg xmlns="http://www.w3.org/2000/svg">
  ...
  for (unsigned Index : toolchain::seq(0u, ArrowIndices.getTotalNumberOfArrows())) {
