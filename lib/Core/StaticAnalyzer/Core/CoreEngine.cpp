...
#include "language/Core/StaticAnalyzer/Core/PathSensitive/FunctionSummary.h"
...
static std::unique_ptr<WorkList> generateWorkList(AnalyzerOptions &Opts) {
  ...
  toolchain_unreachable("Unknown AnalyzerOptions::ExplorationStrategyKind");
}
...
static std::string timeTraceScopeName(const ProgramPoint &Loc) {
  if (toolchain::timeTraceProfilerEnabled()) {
    return toolchain::formatv("dispatchWorkItem {0}",
...
static toolchain::TimeTraceMetadata timeTraceMetadata(const ExplodedNode *Pred,
  ...
  if (!SLoc)
    return toolchain::TimeTraceMetadata{std::move(Detail), ""};
  ...
  return toolchain::TimeTraceMetadata{std::move(Detail), Fname.str(),
                                 static_cast<int>(Line)};
}

void CoreEngine::dispatchWorkItem(ExplodedNode *Pred, ProgramPoint Loc,
                                  const WorkListUnit &WU) {
  toolchain::TimeTraceScope tcs{timeTraceScopeName(Loc), [Loc, Pred]() {
...
void CoreEngine::HandleBlockExit(const CFGBlock * B, ExplodedNode *Pred) {
  if (const Stmt *Term = B->getTerminatorStmt()) {
    switch (Term->getStmtClass()) {
      default:
        toolchain_unreachable("Analysis for this terminator not implemented.");
