...
static bool isPossiblyEscaped(ExplodedNode *N, const DeclRefExpr *DR) {
  ...
  toolchain_unreachable("Reached root without finding the declaration of VD");
}
...
static bool madeNewBranch(ExplodedNode *N, const Stmt *LoopStmt) {
  ...
  toolchain_unreachable("Reached root without encountering the previous step");
}
