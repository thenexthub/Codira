...
using VisitorsDiagnosticsTy =
    toolchain::DenseMap<const ExplodedNode *, std::vector<PathDiagnosticPieceRef>>;

/// A map from PathDiagnosticPiece to the LocationContext of the inlined
/// function call it represents.
using LocationContextMap =
    toolchain::DenseMap<const PathPieces *, const LocationContext *>;
...
private:
  ...
  PathDiagnosticLocation
  ExecutionContinues(toolchain::raw_string_ostream &os,
...
toolchain::TimeTraceMetadata timeTraceMetadata(const BugReportEquivClass &EQ,
                                          const SourceManager &SM) {
...
std::string StackHintGeneratorForSymbol::getMessage(const ExplodedNode *N){
  ...
  for (auto [Idx, ArgExpr] : toolchain::enumerate(CE->arguments())) {
...
std::string StackHintGeneratorForSymbol::getMessageForArg(const Expr *ArgE,
  ...
  return (toolchain::Twine(Msg) + " via " + std::to_string(ArgIndex) +
          toolchain::getOrdinalSuffix(ArgIndex) + " parameter").str();
}
...
PathDiagnosticLocation PathDiagnosticBuilder::ExecutionContinues(
    toolchain::raw_string_ostream &os, const PathDiagnosticConstruct &C) const {
...
constexpr toolchain::StringLiteral StrEnteringLoop = "Entering loop body";
constexpr toolchain::StringLiteral StrLoopBodyZero = "Loop body executed 0 times";
constexpr toolchain::StringLiteral StrLoopRangeEmpty =
    "Loop body skipped when range is empty";
constexpr toolchain::StringLiteral StrLoopCollectionEmpty =
...
using OptimizedCallsSet = toolchain::DenseSet<const PathDiagnosticCallPiece *>;
...
static void addContextEdges(PathPieces &pieces, const LocationContext *LC) {
  ...
  for (PathPieces::iterator I = pieces.begin(), E = Prev; I != E;
    ...
    while (true) {
      ...
      if (toolchain::is_contained(SrcContexts, DstContext))
        break;
...
PathDiagnosticBuilder::generate(const PathDiagnosticConsumer *PDC) const {
  ...
  while (Construct.ascendToPrevNode()) {
    ...
    // into a FoldingSet.
    std::set<toolchain::FoldingSetNodeID> DeduplicationSet;
...
void BasicBugReport::Profile(toolchain::FoldingSetNodeID& hash) const {
...
void PathSensitiveBugReport::Profile(toolchain::FoldingSetNodeID &hash) const {
  hash.AddInteger(static_cast<int>(getKind()));
  hash.AddPointer(&BT);
  hash.AddString(getShortDescription());
...
static void insertToInterestingnessMap(
    toolchain::DenseMap<T, bugreporter::TrackingKind> &InterestingnessMap, T Val,
...
class BugPathGetter {
  ...
  using PriorityMapTy = toolchain::DenseMap<const ExplodedNode *, unsigned>;
...
BugPathGetter::BugPathGetter(const ExplodedGraph *OriginalGraph,
  ...
  // in the new graph.
  toolchain::SmallPtrSet<const ExplodedNode *, 32> RemainingNodes;
  ...
  toolchain::sort(ReportNodes, PriorityCompare<true>(PriorityMap));
}
...
static void CompactMacroExpandedPieces(PathPieces &path,
  ...
  toolchain::append_range(path, Pieces);
}
  ...
  while (BugPathInfo *BugPath = BugGraph.getNextBugPath()) {
    ...
    if (R->isValid()) {
      if (Reporter.getAnalyzerOptions().ShouldCrosscheckWithZ3) {
        toolchain::TimeTraceScope TCS{"Crosscheck with Z3"};
...
BugReport *PathSensitiveBugReporter::findReportInEquivalenceClass(
  ...
  for (const auto &I: EQ.getReports()) {
    ...
    toolchain::DenseMap<const ExplodedNode *, unsigned> Visited;
...
void BugReporter::FlushReport(BugReportEquivClass &EQ) {
  toolchain::TimeTraceScope TCS{timeTraceName(EQ), [&]() {
  ...
  for (auto &P : *Diagnostics) {
    ...
    if (getAnalyzerOptions().ShouldDisplayNotesAsEvents) {
      ...
      for (const auto &I : toolchain::reverse(report->getNotes())) {
...
BugType *BugReporter::getBugTypeForName(CheckerNameRef CheckName,
  ...
  toolchain::raw_svector_ostream(fullDesc)
