...
BindResult StoreManager::enterStackFrame(Store OldStore, const CallEvent &Call,
  ...
  for (const auto &[Location, Val] : InitialBindings) {
    ...
    toolchain::append_range(Result.FailedToBindValues, Curr.FailedToBindValues);
  }
...
std::optional<const MemRegion *> StoreManager::castRegion(const MemRegion *R,
  ...
  switch (R->getKind()) {
    ...
    case MemRegion::GlobalImmutableSpaceRegionKind: {
      toolchain_unreachable("Invalid region cast");
    }
  ...
  toolchain_unreachable("unreachable");
}
...
SVal StoreManager::getLValueFieldOrIvar(const Decl *D, SVal Base) {
  ...
  default:
    toolchain_unreachable("Unhandled Base.");
  }
...
SVal StoreManager::getLValueElement(QualType elementType, NonLoc Offset,
  ...
  const toolchain::APSInt &BaseIdxI =
  ...
  const toolchain::APSInt& OffI = Offset.castAs<nonloc::ConcreteInt>().getValue();
