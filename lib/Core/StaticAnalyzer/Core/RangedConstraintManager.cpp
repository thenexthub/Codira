...
ProgramStateRef RangedConstraintManager::assumeSym(ProgramStateRef State,
  ...
  if (const SymIntExpr *SIE = dyn_cast<SymIntExpr>(Sym)) {
    ...
    const toolchain::APSInt &Zero = getBasicVals().getValue(0, SIE->getType());
  ...
  if (const auto *SSE = dyn_cast<SymSymExpr>(Sym)) {
    ...
    if (BinaryOperator::isComparisonOp(Op)) {
      ...
      if (Loc::isLocType(SSE->getLHS()->getType()) &&
        ...
        const toolchain::APSInt &Zero = getBasicVals().getValue(0, DiffTy);
      ...
      if (BinaryOperator::isEqualityOp(Op)) {
        ...
        const toolchain::APSInt &Zero = getBasicVals().getValue(0, ExprType);
...
ProgramStateRef RangedConstraintManager::assumeSymInclusiveRange(
  ...
  toolchain::APSInt Adjustment = WraparoundType.getZeroValue();
  ...
  toolchain::APSInt ConvertedFrom = ComparisonType.convert(From);
  toolchain::APSInt ConvertedTo = ComparisonType.convert(To);
...
RangedConstraintManager::assumeSymUnsupported(ProgramStateRef State,
  ...
  const toolchain::APSInt &Zero = BVF.getValue(0, T);
  if (Assumption)
...
ProgramStateRef RangedConstraintManager::assumeSymRel(ProgramStateRef State,
  ...
  toolchain::APSInt ConvertedInt = ComparisonType.convert(Int);
  ...
  switch (Op) {
  default:
    toolchain_unreachable("invalid operation not caught by assertion above");
