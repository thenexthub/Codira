...
private:
  ...
  toolchain::PointerIntPair<const MemRegion *, 2> P;
  uint64_t Data;
...
void BindingKey::dump() const { toolchain::errs() << *this; }
#endif
...
typedef toolchain::ImmutableMap<BindingKey, SVal>    ClusterBindings;
typedef toolchain::ImmutableMapRef<BindingKey, SVal> ClusterBindingsRef;
typedef std::pair<BindingKey, SVal> BindingPair;

typedef toolchain::ImmutableMap<const MemRegion *, ClusterBindings>
        RegionBindings;

namespace {
class RegionBindingsRef : public toolchain::ImmutableMapRef<const MemRegion *,
                                 ClusterBindings> {
...
public:
  typedef toolchain::ImmutableMapRef<const MemRegion *, ClusterBindings>
          ParentTy;
  ...
  using toolchain::ImmutableMapRef<const MemRegion *, ClusterBindings>::lookup;
  ...
  Store asStore() const {
    toolchain::PointerIntPair<Store, 1, bool> Ptr = {
  ...
  void printJson(raw_ostream &Out, const char *NL = "\n",
    ...
    toolchain::sort(SortedClusters, MemSpaceBeforeRegionName);

    for (auto [Idx, C] : toolchain::enumerate(SortedClusters)) {
      ...
      toolchain::sort(SortedBindings, DefaultBindingBeforeDirectBindings);

      ++Space;
      for (auto [Idx, B] : toolchain::enumerate(SortedBindings)) {
  ...
  LLVM_DUMP_METHOD void dump() const { printJson(toolchain::errs()); }
...
public:
  ...
  withValuesEscaped(nonloc::CompoundVal::iterator Begin,
    ...
    for (SVal V : toolchain::make_range(Begin, End))
      withValuesEscaped(V);
    return *this;
  }
...
private:
  typedef toolchain::DenseMap<const LazyCompoundValData *,
...
public: // Part of public interface to class.
  BindResult Bind(Store store, Loc LV, SVal V) override {
    toolchain::SmallVector<SVal, 0> EscapedValuesDuringBind;
...
protected:
  typedef toolchain::DenseMap<const MemRegion *, const ClusterBindings *> ClusterMap;
  typedef const MemRegion * WorkListElement;
  typedef SmallVector<WorkListElement, 10> WorkList;

  toolchain::SmallPtrSet<const ClusterBindings *, 16> Visited;
...
collectSubRegionBindings(SmallVectorImpl<BindingPair> &Bindings,
  ...
  if (std::optional<nonloc::ConcreteInt> ExtentCI =
    ...
    const toolchain::APSInt &ExtentInt = ExtentCI->getValue();
...
RegionStoreManager::removeSubRegionBindings(LimitedRegionBindingsConstRef B,
  ...
  for (BindingKey Key : toolchain::make_first_range(Bindings))
...
void InvalidateRegionsWorker::VisitCluster(const MemRegion *baseR,
  ...
  if (C) {
    for (SVal Val : toolchain::make_second_range(*C))
      VisitBinding(Val);
...
bool InvalidateRegionsWorker::isInitiallyIncludedGlobalRegion(
  ...
  toolchain_unreachable("unknown globals filter");
}
...
convertOffsetsFromSvalToUnsigneds(const SmallVector<SVal, 2> &SrcOffsets,
  ...
  for (SVal V : toolchain::reverse(SrcOffsets)) {
    if (auto CI = V.getAs<nonloc::ConcreteInt>()) {
      // When offset is out of array's bounds, result is UB.
      const toolchain::APSInt &Offset = CI->getValue();
...
SVal RegionStoreManager::getBindingForElement(RegionBindingsConstRef B,
  ...
  if (const StringRegion *StrR = dyn_cast<StringRegion>(superR)) {
    ...
    if (const auto CI = R->getIndex().getAs<nonloc::ConcreteInt>()) {
      const toolchain::APSInt &Idx = CI->getValue();
      if (Idx < 0)
        return UndefinedVal();
...
std::optional<SVal> RegionStoreManager::getBindingForDerivedDefaultValue(
  ...
  if (const std::optional<SVal> &D = B.getDefaultBinding(superR)) {
    ...
    toolchain_unreachable("Unknown default value");
  }
...
RegionStoreManager::getInterestingValues(nonloc::LazyCompoundVal LCV) {
  ...
  collectSubRegionBindings(Bindings, svalBuilder, *Cluster, LazyR,
                           /*IncludeAllDefaultBindings=*/true);
  for (SVal V : toolchain::make_second_range(Bindings)) {
    ...
    if (auto InnerLCV = V.getAs<nonloc::LazyCompoundVal>()) {
      ...
      toolchain::append_range(List, InnerList);
    }
...
RegionStoreManager::bind(LimitedRegionBindingsConstRef B, Loc L, SVal V) {
  toolchain::TimeTraceScope TimeScope("RegionStoreManager::bind",
                                 [&L]() { return locDescr(L); });
...
RegionStoreManager::bindArray(LimitedRegionBindingsConstRef B,
  ...
  toolchain::TimeTraceScope TimeScope("RegionStoreManager::bindArray",
...
RegionStoreManager::bindVector(LimitedRegionBindingsConstRef B,
                               const TypedValueRegion *R, SVal V) {
  toolchain::TimeTraceScope TimeScope("RegionStoreManager::bindVector",
                                 [R]() { return R->getDescriptiveName(); });
  if (B.hasExhaustedBindingLimit())
    return B.withValuesEscaped(V);
...
RegionStoreManager::getUniqueDefaultBinding(RegionBindingsConstRef B,
  ...
  if (!Cluster || !toolchain::hasSingleElement(*Cluster))
    return std::nullopt;
...
RegionStoreManager::bindStruct(LimitedRegionBindingsConstRef B,
                               const TypedValueRegion *R, SVal V) {
  toolchain::TimeTraceScope TimeScope("RegionStoreManager::bindStruct",
                                 [R]() { return R->getDescriptiveName(); });
  if (B.hasExhaustedBindingLimit())
    return B.withValuesEscaped(V);
...
RegionStoreManager::bindAggregate(LimitedRegionBindingsConstRef B,
  ...
  toolchain::TimeTraceScope TimeScope("RegionStoreManager::bindAggregate",
                                 [R]() { return R->getDescriptiveName(); });
  if (B.hasExhaustedBindingLimit())
...
StoreRef RegionStoreManager::removeDeadBindings(Store store,
  ...
  for (const MemRegion *Base : toolchain::make_first_range(B)) {
