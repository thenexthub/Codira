...
#include "language/Core/StaticAnalyzer/Core/PathSensitive/RangedConstraintManager.h"
...
RangeSet RangeSet::Factory::add(RangeSet Original, Range Element) {
  ...
  const_iterator Lower = toolchain::lower_bound(Original, Element);
...
RangeSet RangeSet::Factory::add(RangeSet Original, const toolchain::APSInt &Point) {
...
RangeSet RangeSet::Factory::unite(RangeSet Original, toolchain::APSInt Point) {
  return unite(Original, Range(ValueFactory.getValue(Point)));
}

RangeSet RangeSet::Factory::unite(RangeSet Original, toolchain::APSInt From,
                                  toolchain::APSInt To) {
...
RangeSet::ContainerType RangeSet::Factory::unite(const ContainerType &LHS,
  ...
  while (true) {
    ...
    // ----------[ Union ------>
    // UnionStart^
    const toolchain::APSInt &UnionStart = First->From();
  ...
  toolchain_unreachable("Normally, we should not reach here");
}
...
const toolchain::APSInt &RangeSet::getMinValue() const {
  assert(!isEmpty());
  return begin()->From();
}

const toolchain::APSInt &RangeSet::getMaxValue() const {
  assert(!isEmpty());
  return std::prev(end())->To();
}

bool language::Core::ento::RangeSet::isUnsigned() const {
  assert(!isEmpty());
  return begin()->From().isUnsigned();
}

uint32_t language::Core::ento::RangeSet::getBitWidth() const {
  assert(!isEmpty());
  return begin()->From().getBitWidth();
}

APSIntType language::Core::ento::RangeSet::getAPSIntType() const {
  assert(!isEmpty());
  return APSIntType(begin()->From());
}

bool RangeSet::containsImpl(toolchain::APSInt &Point) const {
  if (isEmpty() || !pin(Point))
    return false;

  Range Dummy(Point);
  const_iterator It = toolchain::upper_bound(*this, Dummy);
  if (It == begin())
    return false;
...
bool RangeSet::pin(toolchain::APSInt &Point) const {
...
bool RangeSet::pin(toolchain::APSInt &Lower, toolchain::APSInt &Upper) const {
...
RangeSet RangeSet::Factory::intersect(RangeSet What, toolchain::APSInt Lower,
                                      toolchain::APSInt Upper) {
...
RangeSet RangeSet::Factory::intersect(const RangeSet::ContainerType &LHS,
  ...
  while (First != FirstEnd && Second != SecondEnd) {
    ...
    do {
      ...
      const toolchain::APSInt &IntersectionStart = Second->From();
...
RangeSet RangeSet::Factory::intersect(RangeSet LHS, toolchain::APSInt Point) {
...
RangeSet RangeSet::Factory::negate(RangeSet What) {
  ...
  const toolchain::APSInt SampleValue = What.getMinValue();
  const toolchain::APSInt &MIN = ValueFactory.getMinValue(SampleValue);
  const toolchain::APSInt &MAX = ValueFactory.getMaxValue(SampleValue);
  ...
  const toolchain::APSInt &From = It->From();
  const toolchain::APSInt &To = It->To();
  ...
  for (; It != End; ++It) {
    // Negate int values.
    const toolchain::APSInt &NewFrom = ValueFactory.getValue(-It->To());
    const toolchain::APSInt &NewTo = ValueFactory.getValue(-It->From());
...
RangeSet::ContainerType RangeSet::Factory::promoteTo(RangeSet What,
  ...
  for (const Range &R : What) {
    // Get bounds of the given range.
    toolchain::APSInt FromInt = R.From();
    toolchain::APSInt ToInt = R.To();
    // Cast the bounds.
    Ty.apply(FromInt);
    Ty.apply(ToInt);
...
RangeSet RangeSet::Factory::deletePoint(RangeSet From,
                                        const toolchain::APSInt &Point) {
  if (!From.contains(Point))
    return From;

  toolchain::APSInt Upper = Point;
  toolchain::APSInt Lower = Point;
...
LLVM_DUMP_METHOD void Range::dump() const { dump(toolchain::errs()); }

LLVM_DUMP_METHOD void RangeSet::dump(raw_ostream &OS) const {
  OS << "{ ";
  toolchain::interleaveComma(*this, OS, [&OS](const Range &R) { R.dump(OS); });
  OS << " }";
}
LLVM_DUMP_METHOD void RangeSet::dump() const { dump(toolchain::errs()); }
...
class EquivalenceClass : public toolchain::FoldingSetNode {
public:
  ...
  LLVM_DUMP_METHOD void dump(ProgramStateRef State) const {
    dumpToStream(State, toolchain::errs());
  }
  ...
  static void Profile(toolchain::FoldingSetNodeID &ID, uintptr_t CID) {
    ID.AddInteger(CID);
  }

  void Profile(toolchain::FoldingSetNodeID &ID) const { Profile(ID, this->ID); }
...
private:
  ...
  RangeSet inferAs(const toolchain::APSInt &Val, QualType) {
  ...
  Range getSymmetricalRange(Range Origin, QualType T) {
    ...
    toolchain::APSInt AbsMax = std::max(-Origin.From(), Origin.To());
  ...
  std::optional<RangeSet> getRangeCommutativeSymSym(const SymSymExpr *SSE) {
    ...
    bool IsCommutative = toolchain::is_contained(
        // ==, !=, |, &, +, *, ^
  ...
  std::optional<RangeSet> getRangeForComparisonSymbol(const SymSymExpr *SSE) {
    ...
    for (size_t i = 0; i < CmpOpTable.getCmpOpCount(); ++i) {
      ...
      const toolchain::APSInt *ConcreteValue = QueriedRangeSet->getConcreteValue();
  ...
  RangeSet getFalseRange(QualType T) {
    const toolchain::APSInt &Zero = ValueFactory.getValue(0, T);
...
RangeSet SymbolicRangeInferrer::VisitBinaryOperator<BO_Or>(Range LHS, Range RHS,
                                                           QualType T) {
  APSIntType ResultType = ValueFactory.getAPSIntType(T);
  toolchain::APSInt Zero = ResultType.getZeroValue();
  ...
  if ((IsLHSPositiveOrZero && IsRHSPositiveOrZero) ||
    ...
    const toolchain::APSInt &Min = std::max(LHS.From(), RHS.From());
    ...
    //       the N-th bit set.
    const toolchain::APSInt &Max = IsLHSNegative
...
RangeSet SymbolicRangeInferrer::VisitBinaryOperator<BO_And>(Range LHS,
  ...
  if ((IsLHSPositiveOrZero && IsRHSPositiveOrZero) ||
    ...
    const toolchain::APSInt &Max = std::min(LHS.To(), RHS.To());
    ...
    const toolchain::APSInt &Min = IsLHSNegative
  ...
  if (IsLHSPositiveOrZero || IsRHSPositiveOrZero) {
    ...
    const toolchain::APSInt &Max = IsLHSPositiveOrZero ? LHS.To() : RHS.To();
...
RangeSet SymbolicRangeInferrer::VisitBinaryOperator<BO_Rem>(Range LHS,
                                                            Range RHS,
                                                            QualType T) {
  toolchain::APSInt Zero = ValueFactory.getAPSIntType(T).getZeroValue();

  Range ConservativeRange = getSymmetricalRange(RHS, T);

  toolchain::APSInt Max = ConservativeRange.To();
  toolchain::APSInt Min = ConservativeRange.From();
...
public:
  ...
  const toolchain::APSInt *getSymVal(ProgramStateRef State,
                                SymbolRef Sym) const override;

  const toolchain::APSInt *getSymMinVal(ProgramStateRef State,
                                   SymbolRef Sym) const override;

  const toolchain::APSInt *getSymMaxVal(ProgramStateRef State,
                                   SymbolRef Sym) const override;
  ...
  ProgramStateRef assumeSymNE(ProgramStateRef State, SymbolRef Sym,
                              ...
                              const toolchain::APSInt &Adjustment) override;
  ...
  ProgramStateRef assumeSymWithinInclusiveRange(
      ...
      const toolchain::APSInt &To, const toolchain::APSInt &Adjustment) override;
...
private:
  ...
  RangeSet getSymLTRange(ProgramStateRef St, SymbolRef Sym,
                         ...
                         const toolchain::APSInt &Adjustment) const;
  ...
  RangeSet getSymLERange(ProgramStateRef St, SymbolRef Sym,
                         const toolchain::APSInt &Int,
                         const toolchain::APSInt &Adjustment) const;
  RangeSet getSymLERange(toolchain::function_ref<RangeSet()> RS,
                         const toolchain::APSInt &Int,
                         const toolchain::APSInt &Adjustment) const;
...
public:
  using Const = const toolchain::APSInt &;
...
#define CONSTRAINT_DISPATCH(Id)                                                \
  if (const toolchain::APSInt *Const = Constraint.getConcreteValue()) {             \
...
private:
  ...
  [[nodiscard]] ProgramStateRef assign(EquivalenceClass Class,
    ...
    if (const toolchain::APSInt *Point = NewConstraint.getConcreteValue()) {
...
bool ConstraintAssignor::assignSymExprToConst(const SymExpr *Sym,
                                              const toolchain::APSInt &Constraint) {
  toolchain::SmallSet<EquivalenceClass, 4> SimplifiedClasses;
...
inline bool EquivalenceClass::addToDisequalityInfo(
  ...
  if (const RangeSet *SecondConstraint = Constraints.lookup(Second))
    if (const toolchain::APSInt *Point = SecondConstraint->getConcreteValue()) {
...
EquivalenceClass::simplify(SValBuilder &SVB, RangeSet::Factory &F,
  ...
  for (const SymbolRef &MemberSym : ClassMembers) {
    ...
    if (const auto CI = SimplifiedMemberVal.getAs<nonloc::ConcreteInt>()) {
      const toolchain::APSInt &SV = CI->getValue();
...
ConditionTruthVal RangeConstraintManager::checkNull(ProgramStateRef State,
  ...
  if (const toolchain::APSInt *Value = Ranges->getConcreteValue())
    return *Value == 0;

  BasicValueFactory &BV = getBasicVals();
  APSIntType IntType = BV.getAPSIntType(Sym->getType());
  toolchain::APSInt Zero = IntType.getZeroValue();
...
const toolchain::APSInt *RangeConstraintManager::getSymVal(ProgramStateRef St,
                                                      SymbolRef Sym) const {
  return getRange(St, Sym).getConcreteValue();
}

const toolchain::APSInt *RangeConstraintManager::getSymMinVal(ProgramStateRef St,
                                                         SymbolRef Sym) const {
  RangeSet Range = getRange(St, Sym);
  return Range.isEmpty() ? nullptr : &Range.getMinValue();
}

const toolchain::APSInt *RangeConstraintManager::getSymMaxVal(ProgramStateRef St,
                                                         SymbolRef Sym) const {
  RangeSet Range = getRange(St, Sym);
...
RangeConstraintManager::assumeSymNE(ProgramStateRef St, SymbolRef Sym,
                                    const toolchain::APSInt &Int,
                                    const toolchain::APSInt &Adjustment) {
  // Before we do any real work, see if the value can even show up.
  APSIntType AdjustmentType(Adjustment);
  if (AdjustmentType.testInRange(Int, true) != APSIntType::RTR_Within)
    return St;

  toolchain::APSInt Point = AdjustmentType.convert(Int) - Adjustment;
...
RangeConstraintManager::assumeSymEQ(ProgramStateRef St, SymbolRef Sym,
  ...
  toolchain::APSInt AdjInt = AdjustmentType.convert(Int) - Adjustment;
  RangeSet New = getRange(St, Sym);
...
RangeConstraintManager::getSymLTRange(ProgramStateRef St, SymbolRef Sym,
  ...
  toolchain::APSInt ComparisonVal = AdjustmentType.convert(Int);
  toolchain::APSInt Min = AdjustmentType.getMinValue();
  if (ComparisonVal == Min)
    return F.getEmptySet();

  toolchain::APSInt Lower = Min - Adjustment;
  toolchain::APSInt Upper = ComparisonVal - Adjustment;
  --Upper;
...
RangeConstraintManager::getSymGTRange(ProgramStateRef St, SymbolRef Sym,
  ...
  toolchain::APSInt ComparisonVal = AdjustmentType.convert(Int);
  toolchain::APSInt Max = AdjustmentType.getMaxValue();
  if (ComparisonVal == Max)
    return F.getEmptySet();

  toolchain::APSInt Lower = ComparisonVal - Adjustment;
  toolchain::APSInt Upper = Max - Adjustment;
  ++Lower;
...
RangeSet
RangeConstraintManager::getSymLERange(toolchain::function_ref<RangeSet()> RS,
                                      const toolchain::APSInt &Int,
                                      const toolchain::APSInt &Adjustment) const {
  // Before we do any real work, see if the value can even show up.
  APSIntType AdjustmentType(Adjustment);
...
ProgramStateRef RangeConstraintManager::assumeSymWithinInclusiveRange(
    ProgramStateRef State, SymbolRef Sym, const toolchain::APSInt &From,
    const toolchain::APSInt &To, const toolchain::APSInt &Adjustment) {
...
static std::string toString(const SymbolRef &Sym) {
  std::string S;
  toolchain::raw_string_ostream O(S);
  Sym->dumpToStream(O);
  return S;
}
...
static std::string toString(ProgramStateRef State, EquivalenceClass Class) {
  SymbolSet ClassMembers = Class.getClassMembers(State);
  toolchain::SmallVector<SymbolRef, 8> ClassMembersSorted(ClassMembers.begin(),
                                                     ClassMembers.end());
  toolchain::sort(ClassMembersSorted,
  ...
  std::string Str;
  toolchain::raw_string_ostream Out(Str);
  Out << "[ ";
