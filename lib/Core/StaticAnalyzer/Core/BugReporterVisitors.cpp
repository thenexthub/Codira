...
static std::optional<const toolchain::APSInt *>
...
static toolchain::StringLiteral WillBeUsedForACondition =
...
public:
  ...
  PathDiagnosticPieceRef VisitNode(const ExplodedNode *N,
    ...
    toolchain_unreachable("Invalid visit mode!");
  }
...
void StoreSiteFinder::Profile(toolchain::FoldingSetNodeID &ID) const {
  static int tag = 0;
  ID.AddPointer(&tag);
  ID.AddPointer(R);
  ID.Add(V);
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, const DestTypeValue &Val) {
...
static void showBRDiagnostics(toolchain::raw_svector_ostream &OS, StoreInfo SI) {
  ...
  case StoreInfo::BlockCapture:
    ...
    break;
  default:
    toolchain_unreachable("Unexpected store kind");
  }
...
static void showBRParamDiagnostics(toolchain::raw_svector_ostream &OS,
                                   StoreInfo SI) {
  ...
  if (const auto *Param = dyn_cast<ParmVarDecl>(VR->getDecl())) {
    ...
    OS << " via " << Idx << toolchain::getOrdinalSuffix(Idx) << " parameter";
...
static void showBRDefaultDiagnostics(toolchain::raw_svector_ostream &OS,
                                     StoreInfo SI) {
...
void TrackConstraintBRVisitor::Profile(toolchain::FoldingSetNodeID &ID) const {
  static int tag = 0;
  ID.AddPointer(&tag);
...
class TrackControlDependencyCondBRVisitor final
  ...
  toolchain::SmallSet<const CFGBlock *, 32> VisitedBlocks;
...
TrackControlDependencyCondBRVisitor::VisitNode(const ExplodedNode *N,
  ...
  if (ControlDeps.isControlDependent(OriginB, NB)) {
    ...
    if (toolchain::isa_and_nonnull<CXXForRangeStmt>(NB->getTerminatorStmt()))
      return nullptr;
...
PathDiagnosticPieceRef ConditionBRVisitor::VisitTrueTest(
  ...
  {
    toolchain::raw_svector_ostream OutLHS(LhsString), OutRHS(RhsString);
...
bool ConditionBRVisitor::printValue(const Expr *CondVarExpr, raw_ostream &Out,
  ...
  std::optional<const toolchain::APSInt *> IntValue;
  if (!IsAssuming)
...
constexpr toolchain::StringLiteral ConditionBRVisitor::GenericTrueMessage;
constexpr toolchain::StringLiteral ConditionBRVisitor::GenericFalseMessage;
...
void TagVisitor::Profile(toolchain::FoldingSetNodeID &ID) const {
  static int Tag = 0;
  ID.AddPointer(&Tag);
}
