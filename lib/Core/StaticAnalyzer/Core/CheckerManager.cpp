...
void CheckerManager::reportInvalidCheckerOptionValue(
  ...
  getDiagnostics().Report(diag::err_analyzer_checker_option_invalid_input)
      << (toolchain::Twine(Checker->getName()) + ":" + OptionName).str()
      << ExpectedValueDesc;
}
...
std::string checkerScopeName(StringRef Name, const CheckerBackend *Checker) {
  ...
  struct CheckStmtContext {
    ...
    void runChecker(CheckerManager::CheckStmtFunc checkFn,
      ...
      toolchain::TimeTraceScope TimeScope(checkerScopeName("Stmt", checkFn.Checker));
...
void CheckerManager::runCheckersForObjCMessage(ObjCMessageVisitKind visitKind,
  ...
  toolchain::TimeTraceScope TimeScope("CheckerManager::runCheckersForObjCMessage");
  expandGraphWithCheckers(C, Dst, Src);
}
...
namespace {
  ...
  struct CheckCallContext {
    ...
    void runChecker(CheckerManager::CheckCallFunc checkFn,
                    NodeBuilder &Bldr, ExplodedNode *Pred) {
      toolchain::TimeTraceScope TimeScope(checkerScopeName("Call", checkFn.Checker));
...
namespace {
  ...
  struct CheckLocationContext {
    ...
    void runChecker(CheckerManager::CheckLocationFunc checkFn,
                    NodeBuilder &Bldr, ExplodedNode *Pred) {
      toolchain::TimeTraceScope TimeScope(checkerScopeName("Loc", checkFn.Checker));
...
namespace {
  ...
  struct CheckBindContext {
    ...
    void runChecker(CheckerManager::CheckBindFunc checkFn,
                    NodeBuilder &Bldr, ExplodedNode *Pred) {
      toolchain::TimeTraceScope TimeScope(checkerScopeName("Bind", checkFn.Checker));
  ...
  toolchain::TimeTraceMetadata getTimeTraceBindMetadata(SVal Val) {
    assert(toolchain::timeTraceProfilerEnabled());
    std::string Name;
    toolchain::raw_string_ostream OS(Name);
    Val.dumpToStream(OS);
    return toolchain::TimeTraceMetadata{OS.str(), ""};
  }
...
void CheckerManager::runCheckersForBlockEntrance(ExplodedNodeSet &Dst,
  ...
  toolchain::TimeTraceScope TimeScope{"CheckerManager::runCheckersForBlockEntrance"};
  expandGraphWithCheckers(C, Dst, Src);
}
...
struct CheckBeginFunctionContext {
  ...
  void runChecker(CheckerManager::CheckBeginFunctionFunc checkFn,
                  NodeBuilder &Bldr, ExplodedNode *Pred) {
    toolchain::TimeTraceScope TimeScope(checkerScopeName("Begin", checkFn.Checker));
    const ProgramPoint &L = PP.withTag(checkFn.Checker);
    CheckerContext C(Bldr, Eng, Pred, L);
...
void CheckerManager::runCheckersForBeginFunction(ExplodedNodeSet &Dst,
  ...
  toolchain::TimeTraceScope TimeScope("CheckerManager::runCheckersForBeginFunction");
  expandGraphWithCheckers(C, Dst, Src);
}
...
void CheckerManager::runCheckersForEndFunction(NodeBuilderContext &BC,
  ...
  for (const auto &checkFn : EndFunctionCheckers) {
    ...
    CheckerContext C(Bldr, Eng, Pred, L);
    toolchain::TimeTraceScope TimeScope(checkerScopeName("End", checkFn.Checker));
    checkFn(RS, C);
  }
}
...
void CheckerManager::runCheckersForNewAllocator(const CXXAllocatorCall &Call,
  ...
  toolchain::TimeTraceScope TimeScope("CheckerManager::runCheckersForNewAllocator");
  expandGraphWithCheckers(C, Dst, Src);
}
...
void CheckerManager::runCheckersForDeadSymbols(ExplodedNodeSet &Dst,
  ...
  toolchain::TimeTraceScope TimeScope("CheckerManager::runCheckersForDeadSymbols");
  expandGraphWithCheckers(C, Dst, Src);
}
  ...
  for (auto *const Pred : Src) {
    ...
    for (const auto &EvalCallChecker : EvalCallCheckers) {
      ...
      if (evaluated && evaluatorChecker) {
        ...
        std::string AssertionMessage = toolchain::formatv(
        ...
        toolchain_unreachable(AssertionMessage.c_str());
      }
#endif
      if (evaluated) {
