...
class SimpleSValBuilder : public SValBuilder {
  ...
  // instead.
  static const toolchain::APSInt *getConstValue(ProgramStateRef state, SVal V);

  // Helper function that returns the value stored in a nonloc::ConcreteInt or
  // loc::ConcreteInt.
  static const toolchain::APSInt *getConcreteValue(SVal V);
...
public:
  SimpleSValBuilder(toolchain::BumpPtrAllocator &alloc, ASTContext &context,
  ...
  const toolchain::APSInt *getKnownValue(ProgramStateRef state, SVal V) override;
  ...
  const toolchain::APSInt *getMinValue(ProgramStateRef state, SVal V) override;

  /// Evaluates a given SVal by recursively evaluating and simplifying the
  /// children SVals, then returns its maximal possible (integer) value. If the
  /// constraint manager cannot provide a meaningful answer, this returns NULL.
  const toolchain::APSInt *getMaxValue(ProgramStateRef state, SVal V) override;

  SVal simplifySVal(ProgramStateRef State, SVal V) override;

  SVal MakeSymIntVal(const SymExpr *LHS, BinaryOperator::Opcode op,
                     const toolchain::APSInt &RHS, QualType resultTy);
};
} // end anonymous namespace

SValBuilder *ento::createSimpleSValBuilder(toolchain::BumpPtrAllocator &alloc,
                                           ASTContext &context,
                                           ProgramStateManager &stateMgr) {
...
static bool isNegationValuePreserving(const toolchain::APSInt &Value,
...
static bool isInRelation(BinaryOperator::Opcode Rel, SymbolRef Sym,
                         toolchain::APSInt Bound, ProgramStateRef State) {
...
static bool isWithinConstantOverflowBounds(SymbolRef Sym,
  ...
  toolchain::APSInt Max = AT.getMaxValue() / AT.getValue(4), Min = -Max;
...
static bool isWithinConstantOverflowBounds(toolchain::APSInt I) {
  APSIntType AT(I);
  assert(!AT.isUnsigned() &&
         "This only works with signed integers!");

  toolchain::APSInt Max = AT.getMaxValue() / AT.getValue(4);
  return (I <= Max) && (I >= -Max);
}
...
static NonLoc doRearrangeUnchecked(ProgramStateRef State,
                                   BinaryOperator::Opcode Op,
                                   SymbolRef LSym, toolchain::APSInt LInt,
                                   SymbolRef RSym, toolchain::APSInt RInt) {
  SValBuilder &SVB = State->getStateManager().getSValBuilder();
  BasicValueFactory &BV = SVB.getBasicValueFactory();
  ...
  if (BinaryOperator::isComparisonOp(Op))
    ...
    toolchain_unreachable("Operation not suitable for unchecked rearrangement!");
...
static bool shouldRearrange(ProgramStateRef State, BinaryOperator::Opcode Op,
                            SymbolRef Sym, toolchain::APSInt Int, QualType Ty) {
  ...
  while (true) {
    ...
    case nonloc::LocAsIntegerKind: {
      ...
      case nonloc::ConcreteIntKind: {
        ...
        toolchain::APSInt i = rhs.castAs<nonloc::ConcreteInt>().getValue();
    ...
    case nonloc::ConcreteIntKind: {
      toolchain::APSInt LHSValue = lhs.castAs<nonloc::ConcreteInt>().getValue();

      // If we're dealing with two known constants, just perform the operation.
      if (const toolchain::APSInt *KnownRHSValue = getConstValue(state, rhs)) {
        toolchain::APSInt RHSValue = *KnownRHSValue;
        if (BinaryOperator::isComparisonOp(op)) {
    ...
    case nonloc::SymbolValKind: {
      ...
      if (const SymIntExpr *symIntExpr = dyn_cast<SymIntExpr>(Sym)) {
        ...
        if (op == BO_EQ && rhs.isZeroConstant()) {
          ...
          case BO_LAnd:
          case BO_LOr:
            toolchain_unreachable("Logical operators handled by branching logic.");
          case BO_Assign:
          case BO_MulAssign:
          case BO_DivAssign:
          case BO_RemAssign:
          ...
          case BO_OrAssign:
          case BO_Comma:
            toolchain_unreachable("'=' and ',' operators handled by ExprEngine.");
          case BO_PtrMemD:
          case BO_PtrMemI:
            toolchain_unreachable("Pointer arithmetic not handled here.");
          case BO_LT:
          case BO_GT:
          case BO_LE:
          case BO_GE:
        ...
        if (const toolchain::APSInt *RHSValue = getConstValue(state, rhs)) {
          ...
          if (BinaryOperator::isAdditiveOp(op)) {
            ...
            if (BinaryOperator::isAdditiveOp(lop)) {
              ...
              APSIntType IntType = BasicVals.getAPSIntType(resultTy);
              const toolchain::APSInt &first = IntType.convert(symIntExpr->getRHS());
              const toolchain::APSInt &second = IntType.convert(*RHSValue);
...
static SVal evalBinOpFieldRegionFieldRegion(const FieldRegion *LeftFR,
  ...
  toolchain_unreachable("Fields not found in parent record's definition");
}
...
SVal SimpleSValBuilder::evalBinOpLL(ProgramStateRef state,
  ...
  if (lhs == rhs) {
    switch (op) {
    default:
      toolchain_unreachable("Unimplemented operation for two identical values");
    case BO_Sub:
      return makeZeroVal(resultTy);
    case BO_EQ:
    case BO_LE:
  ...
  switch (lhs.getKind()) {
  default:
    toolchain_unreachable("Ordering not implemented for this Loc.");
...
SVal SimpleSValBuilder::evalBinOpLN(ProgramStateRef state,
  ...
  if (std::optional<nonloc::ConcreteInt> rhsInt =
    ...
    if (std::optional<loc::ConcreteInt> lhsInt =
      ...
      const toolchain::APSInt &leftI = lhsInt->getValue();
      assert(leftI.isUnsigned());
      toolchain::APSInt rightI(rhsInt->getValue(), /* isUnsigned */ true);
      ...
      toolchain::APSInt Multiplicand(rightI.getBitWidth(), /* isUnsigned */ true);
      ...
      switch (op) {
        ...
        case BO_Sub:
          ...
          break;
        default:
          toolchain_unreachable("Invalid pointer arithmetic operation");
      }
...
const toolchain::APSInt *SimpleSValBuilder::getConstValue(ProgramStateRef state,
                                                     SVal V) {
  if (const toolchain::APSInt *Res = getConcreteValue(V))
    return Res;
...
const toolchain::APSInt *SimpleSValBuilder::getConcreteValue(SVal V) {
...
const toolchain::APSInt *SimpleSValBuilder::getKnownValue(ProgramStateRef state,
                                                     SVal V) {
  return getConstValue(state, simplifySVal(state, V));
}

const toolchain::APSInt *SimpleSValBuilder::getMinValue(ProgramStateRef state,
                                                   SVal V) {
...
const toolchain::APSInt *SimpleSValBuilder::getMaxValue(ProgramStateRef state,
                                                   SVal V) {
  V = simplifySVal(state, V);
...
SVal SimpleSValBuilder::simplifySValOnce(ProgramStateRef State, SVal V) {
  ...
  class Simplifier : public FullSValVisitor<Simplifier, SVal> {
    ...
    toolchain::DenseMap<SymbolRef, SVal> Cached;
