...
#include "language/Core/StaticAnalyzer/Frontend/AnalysisConsumer.h"
...
public:
  ...
  std::unique_ptr<toolchain::TimerGroup> AnalyzerTimers;
  std::unique_ptr<toolchain::Timer> SyntaxCheckTimer;
  std::unique_ptr<toolchain::Timer> ExprEngineTimer;
  std::unique_ptr<toolchain::Timer> BugReporterTimer;
  ...
  AnalysisConsumer(CompilerInstance &CI, const std::string &outdir,
    ...
    if (Opts.AnalyzerDisplayProgress || Opts.PrintStats ||
      ...
      AnalyzerTimers = std::make_unique<toolchain::TimerGroup>(
          "analyzer", "Analyzer timers");
      SyntaxCheckTimer = std::make_unique<toolchain::Timer>(
          "syntaxchecks", "Syntax-based analysis time", *AnalyzerTimers);
      ExprEngineTimer = std::make_unique<toolchain::Timer>(
          "exprengine", "Path exploration time", *AnalyzerTimers);
      BugReporterTimer = std::make_unique<toolchain::Timer>(
          "bugreporter", "Path-sensitive report post-processing time",
          *AnalyzerTimers);
    }

    if (Opts.PrintStats || Opts.ShouldSerializeStats) {
      toolchain::EnableStatistics(/* DoPrintOnExit= */ false);
    }
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
    default:
      toolchain_unreachable("Unknown analyzer output type!");
    }
    ...
    default:
      toolchain_unreachable("Unknown constraint manager.");
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
  ...
  void DisplayTime(toolchain::TimeRecord &Time) {
    if (!Opts.AnalyzerDisplayProgress) {
      return;
    }
    toolchain::errs() << " : " << toolchain::format("%1.1f", Time.getWallTime() * 1000)
                 << " ms\n";
  }
  ...
  void DisplayFunction(const Decl *D, AnalysisMode Mode,
    ...
    if (Loc.isValid()) {
      toolchain::errs() << "ANALYZE";

      if (Mode == AM_Syntax)
        toolchain::errs() << " (Syntax)";
      else if (Mode == AM_Path) {
        toolchain::errs() << " (Path, ";
        switch (IMode) {
        case ExprEngine::Inline_Minimal:
          toolchain::errs() << " Inline_Minimal";
          break;
        case ExprEngine::Inline_Regular:
          toolchain::errs() << " Inline_Regular";
          break;
        }
        toolchain::errs() << ")";
      } else
      ...
      toolchain::errs() << ": " << Loc.getFilename() << ' '
  ...
  bool VisitVarDecl(VarDecl *VD) override {
    ...
    toolchain::Expected<const VarDecl *> CTUDeclOrError =
...
toolchain::TimeTraceMetadata timeTraceScopeDeclMetadata(const Decl *D) {
  ...
  if (const auto &Loc = D->getBeginLoc(); Loc.isValid()) {
    ...
    return toolchain::TimeTraceMetadata{
...
void flushReports(toolchain::Timer *BugReporterTimer, BugReporter &BR) {
  toolchain::TimeTraceScope TCS{"Flushing reports"};
  // Display warnings.
  if (BugReporterTimer)
...
void AnalysisConsumer::HandleDeclsCallGraph(const unsigned LocalTUDeclsSize) {
  ...
  toolchain::ReversePostOrderTraversal<language::Core::CallGraph*> RPOT(&CG);
  for (auto &N : RPOT) {
    NumFunctionTopLevel++;
...
static void reportAnalyzerFunctionMisuse(const AnalyzerOptions &Opts,
  ...
  toolchain::errs() << "Every top-level function was skipped.\n";

  if (!Opts.AnalyzerDisplayProgress)
    toolchain::errs() << "Pass the -analyzer-display-progress for tracking which "
  ...
  if (Ctx.getLangOpts().CPlusPlus && !HasBrackets) {
    ...
    toolchain::errs() << "For analyzing C code you shouldn't pass the function "
...
void AnalysisConsumer::HandleTranslationUnit(ASTContext &C) {
  ...
  const auto DiagFlusherScopeExit =
      toolchain::make_scope_exit([this] { Mgr.reset(); });
...
void AnalysisConsumer::HandleCode(Decl *D, AnalysisMode Mode,
                                  ExprEngine::InliningModes IMode,
                                  SetOfConstDecls *VisitedCallees) {
  toolchain::TimeTraceScope TCS(timeTraceScopeDeclName("HandleCode", D),
  ...
  if (Mode & AM_Syntax) {
    toolchain::TimeRecord CheckerStartTime;
    if (SyntaxCheckTimer) {
    ...
    if (SyntaxCheckTimer) {
      SyntaxCheckTimer->stopTimer();
      toolchain::TimeRecord CheckerEndTime = SyntaxCheckTimer->getTotalTime();
...
void AnalysisConsumer::RunPathSensitiveChecks(Decl *D,
  ...
  toolchain::TimeRecord ExprEngineStartTime;
  if (ExprEngineTimer) {
  ...
  if (ExprEngineTimer) {
    ...
    toolchain::TimeRecord ExprEngineEndTime = ExprEngineTimer->getTotalTime();
