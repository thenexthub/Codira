...
using toolchain::sys::DynamicLibrary;
...
#undef GET_PACKAGES
  ...
  for (const std::string &Plugin : Plugins) {
    ...
    if (!isCompatibleAPIVersion(PluginAPIVersion)) {
      Diags.Report(diag::warn_incompatible_analyzer_plugin_api)
          << toolchain::sys::path::filename(Plugin);
  ...
  toolchain::sort(Data.Packages, checker_registry::PackageNameLT{});
  toolchain::sort(Data.Checkers, checker_registry::CheckerNameLT{});
...
static void isOptionContainedIn(const CmdLineOptionList &OptionList,
  ...
  if (toolchain::none_of(OptionList, SameOptName)) {
...
void CheckerRegistry::validateCheckerOptions() const {
  for (const auto &Config : AnOpts.Config) {
    ...
    auto CheckerIt =
        toolchain::find(Data.Checkers, CheckerInfo(SuppliedCheckerOrPackage));
    ...
    const auto *PackageIt =
        toolchain::find(Data.Packages, PackageInfo(SuppliedCheckerOrPackage));
