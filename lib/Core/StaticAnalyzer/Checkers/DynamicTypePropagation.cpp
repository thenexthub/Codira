...
void DynamicTypePropagation::checkDeadSymbols(SymbolReaper &SR,
  ...
  for (SymbolRef Sym : toolchain::make_first_range(TyArgMap)) {
    if (SR.isDead(Sym)) {
...
void DynamicTypePropagation::reportGenericsBug(
  ...
  QualType::print(From, Qualifiers(), OS, C.getLangOpts(), toolchain::Twine());
  OS << "' to incompatible type '";
  QualType::print(To, Qualifiers(), OS, C.getLangOpts(), toolchain::Twine());
  OS << "'";
...
PathDiagnosticPieceRef DynamicTypePropagation::GenericsBugVisitor::VisitNode(
  ...
  toolchain::raw_svector_ostream OS(Buf);
  OS << "Type '";
  QualType::print(*TrackedType, Qualifiers(), OS, LangOpts, toolchain::Twine());
  OS << "' is inferred from ";
