...
static bool hasStdClassWithName(const CXXRecordDecl *RD,
                                ArrayRef<toolchain::StringLiteral> Names) {
  if (!RD || !RD->getDeclContext()->isStdNamespace())
    return false;
  if (RD->getDeclName().isIdentifier())
    return toolchain::is_contained(Names, RD->getName());
  return false;
}

constexpr toolchain::StringLiteral STD_PTR_NAMES[] = {"shared_ptr", "unique_ptr",
                                                 "weak_ptr"};
...
static void checkAndPrettyPrintRegion(toolchain::raw_ostream &OS,
                                      const MemRegion *Region) {
...
constexpr toolchain::StringLiteral BASIC_OSTREAM_NAMES[] = {"basic_ostream"};
...
static bool isStdOstreamOperatorCall(const CallEvent &Call) {
  ...
  if (OOK != language::Core::OO_LessLess)
    return false;
...
bool SmartPtrModeling::handleComparisionOp(const CallEvent &Call,
  ...
  // For reference, there is a discussion at https://reviews.toolchain.org/D104616.
...
void SmartPtrModeling::checkLiveSymbols(ProgramStateRef State,
  ...
  TrackedRegionMapTy TrackedRegions = State->get<TrackedRegionMap>();
  for (SVal Val : toolchain::make_second_range(TrackedRegions)) {
...
void SmartPtrModeling::handleBoolConversion(const CallEvent &Call,
  ...
  if (State->isNull(InnerPointerVal).isConstrainedTrue()) {
    ...
    C.addTransition(
        NotNullState,
        C.getNoteTag(
            [ThisRegion](PathSensitiveBugReport &BR, toolchain::raw_ostream &OS) {
              OS << "Assuming smart pointer";
              checkAndPrettyPrintRegion(OS, ThisRegion);
              OS << " is non-null";
            },
