...
#define CASE(ID) case ID: OS << #ID; break;
  ...
  LLVM_DUMP_METHOD void dump() const { dump(toolchain::errs()); }
};
...
#define CHECK_FN(NAME)                                                         \
  ...
  void reportTaintBug(StringRef Msg, ProgramStateRef State, CheckerContext &C,
                      toolchain::ArrayRef<SymbolRef> TaintedSyms,
...
protected:
  ...
  bool isFreeingCallAsWritten(const CallExpr &Call) const {
    ...
    if (const auto *Func =
            toolchain::dyn_cast_or_null<FunctionDecl>(Call.getCalleeDecl()))
...
private:
  ...
  public:
    ...
    std::string getMessageForArg(const Expr *ArgE, unsigned ArgIndex) override {
      ...
      os << "Reallocation of " << ArgIndex << toolchain::getOrdinalSuffix(ArgIndex)
         << " parameter failed";
...
MallocChecker::performKernelMalloc(const CallEvent &Call, CheckerContext &C,
  ...
  toolchain::Triple::OSType OS = Ctx.getTargetInfo().getTriple().getOS();
...
void MallocChecker::HandleMismatchedDealloc(CheckerContext &C,
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    ...
    toolchain::raw_svector_ostream AllocOs(AllocBuf);
    SmallString<20> DeallocBuf;
    toolchain::raw_svector_ostream DeallocOs(DeallocBuf);
...
void MallocChecker::HandleOffsetFree(CheckerContext &C, SVal ArgVal,
  ...
  toolchain::raw_svector_ostream AllocNameOs(AllocNameBuf);
...
ProgramStateRef MallocChecker::evalAssume(ProgramStateRef state,
  ...
  for (SymbolRef Sym : toolchain::make_first_range(RS)) {
...
bool MallocChecker::isArgZERO_SIZE_PTR(ProgramStateRef State, CheckerContext &C,
  ...
  const toolchain::APSInt *ArgValKnown =
