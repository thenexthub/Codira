...
class StdLibraryFunctionsChecker
  ...
  static RangeKind negateKind(RangeKind K) {
    ...
    toolchain_unreachable("Unknown range kind");
  }
  ...
  static void printArgDesc(ArgNo, toolchain::raw_ostream &Out);
  ...
  static void printArgValueInfo(ArgNo ArgN, ProgramStateRef State,
                                const CallEvent &Call, toolchain::raw_ostream &Out);
  /// Append textual description of a numeric range [RMin,RMax] to
  /// \p Out.
  static void appendInsideRangeDesc(toolchain::APSInt RMin, toolchain::APSInt RMax,
  ...
  /// \p Out.
  static void appendOutOfRangeDesc(toolchain::APSInt RMin, toolchain::APSInt RMax,
                                   QualType ArgT, BasicValueFactory &BVF,
                                   toolchain::raw_ostream &Out);
  ...
  public:
    ...
    virtual bool describeArgumentValue(const CallEvent &Call,
      ...
      if (auto N = getArgSVal(Call, getArgNo()).getAs<NonLoc>()) {
        if (const toolchain::APSInt *Int = N->getAsInteger()) {
          Out << *Int;
          return true;
        }
      }
  ...
  public:
    ...
    void describe(DescriptionKind DK, const CallEvent &Call,
                  ProgramStateRef State, const Summary &Summary,
                  toolchain::raw_ostream &Out) const override;
  ...
  private:
    ...
    using RangeApplyFunction =
        std::function<bool(const toolchain::APSInt &Min, const toolchain::APSInt &Max)>;
  ...
  class BufferSizeConstraint : public ValueConstraint {
    ...
    std::optional<toolchain::APSInt> ConcreteSize;
  ...
  public:
    BufferSizeConstraint(ArgNo Buffer, toolchain::APSInt BufMinSize)
  ...
  using FunctionSummaryMapType = toolchain::DenseMap<const FunctionDecl *, Summary>;
...
private:
  ...
  void reportBug(const CallEvent &Call, ExplodedNode *N,
    ...
    SmallString<256> Msg;
    toolchain::raw_svector_ostream MsgOs(Msg);
...
void StdLibraryFunctionsChecker::printArgDesc(
    StdLibraryFunctionsChecker::ArgNo ArgN, toolchain::raw_ostream &Out) {
  Out << std::to_string(ArgN + 1);
  Out << toolchain::getOrdinalSuffix(ArgN + 1);
  Out << " argument";
}
...
void StdLibraryFunctionsChecker::printArgValueInfo(ArgNo ArgN,
                                                   ...
                                                   toolchain::raw_ostream &Out) {
  if (const toolchain::APSInt *Val =
          State->getStateManager().getSValBuilder().getKnownValue(
              State, getArgSVal(Call, ArgN)))
    Out << " (which is " << *Val << ")";
}

void StdLibraryFunctionsChecker::appendInsideRangeDesc(toolchain::APSInt RMin,
                                                       toolchain::APSInt RMax,
                                                       QualType ArgT,
...
void StdLibraryFunctionsChecker::appendOutOfRangeDesc(toolchain::APSInt RMin,
                                                      toolchain::APSInt RMax,
                                                      QualType ArgT,
                                                      BasicValueFactory &BVF,
                                                      toolchain::raw_ostream &Out) {
...
void StdLibraryFunctionsChecker::RangeConstraint::applyOnWithinRange(
  ...
  for (auto [Start, End] : getRanges()) {
    const toolchain::APSInt &Min = BVF.getValue(Start, ArgT);
    const toolchain::APSInt &Max = BVF.getValue(End, ArgT);
    assert(Min <= Max);
    if (!F(Min, Max))
      return;
  }
...
void StdLibraryFunctionsChecker::RangeConstraint::applyOnOutOfRange(
  ...
  const toolchain::APSInt &MinusInf = BVF.getMinValue(ArgT);
  const toolchain::APSInt &PlusInf = BVF.getMaxValue(ArgT);

  const toolchain::APSInt &RangeLeft = BVF.getValue(R[0].first - 1ULL, ArgT);
  const toolchain::APSInt &RangeRight = BVF.getValue(R[E - 1].second + 1ULL, ArgT);

  // Iterate over the "holes" between intervals.
  for (size_t I = 1; I != E; ++I) {
    const toolchain::APSInt &Min = BVF.getValue(R[I - 1].second + 1ULL, ArgT);
    const toolchain::APSInt &Max = BVF.getValue(R[I].first - 1ULL, ArgT);
    if (Min <= Max) {
      if (!F(Min, Max))
        return;
    }
...
ProgramStateRef StdLibraryFunctionsChecker::RangeConstraint::apply(
  ...
  if (auto N = V.getAs<NonLoc>()) {
    auto ExcludeRangeFromArg = [&](const toolchain::APSInt &Min,
                                   const toolchain::APSInt &Max) {
...
void StdLibraryFunctionsChecker::RangeConstraint::describe(
    ...
    const Summary &Summary, toolchain::raw_ostream &Out) const {
...
bool StdLibraryFunctionsChecker::RangeConstraint::describeArgumentValue(
  ...
  if (auto N = V.getAs<NonLoc>()) {
    ...
    toolchain::raw_svector_ostream MoreInfoOs(MoreInfo);
    auto ApplyF = [&](const toolchain::APSInt &Min, const toolchain::APSInt &Max) {
...
ProgramStateRef StdLibraryFunctionsChecker::BufferSizeConstraint::apply(
  ...
  toolchain_unreachable("Size argument or the dynamic size is Undefined");
}
    ...
    if (NewState != State) {
      ...
      NewNode = C.addTransition(
          ...
          C.getNoteTag([Msg = std::move(Msg), ArgSVal](
                           PathSensitiveBugReport &BR, toolchain::raw_ostream &OS) {
...
void StdLibraryFunctionsChecker::checkPostCall(const CallEvent &Call,
  ...
  for (const SummaryCase &Case : Summary.getCases()) {
    ...
    if (Case.getNote().empty()) {
      if (!ErrnoNote.empty())
        ErrnoNote =
            toolchain::formatv("After calling '{0}' {1}", FunctionName, ErrnoNote);
    } else {
      // Disable formatv() validation as the case note may not always have the
      // {0} placeholder for function name.
      CaseNote =
          toolchain::formatv(false, Case.getNote().str().c_str(), FunctionName);
    }
        ...
        const NoteTag *Tag =
            C.getNoteTag([CaseNote, ErrnoNote,
              ...
              if (ErrnoImportant) {
                ...
                if (CaseNote.empty())
                  return ErrnoNote;
                return toolchain::formatv("{0}; {1}", CaseNote, ErrnoNote);
              } else {
...
bool StdLibraryFunctionsChecker::evalCall(const CallEvent &Call,
  ...
  toolchain_unreachable("Unknown invalidation kind!");
}
...
bool StdLibraryFunctionsChecker::Signature::matches(
  ...
  for (auto [Idx, ArgTy] : toolchain::enumerate(ArgTys)) {
...
void StdLibraryFunctionsChecker::initFunctionSummaries(
        ...
        if (auto *FD = dyn_cast<FunctionDecl>(D)) {
          if (Sum.matchesAndSet(Sign, FD)) {
            ...
            if (DisplayLoadedSummaries) {
              toolchain::errs() << "Loaded summary for: ";
              FD->print(toolchain::errs());
              toolchain::errs() << "\n";
            }
            return true;
          }
  ...
  constexpr toolchain::StringLiteral GenericSuccessMsg(
      "Assuming that '{0}' is successful");
  constexpr toolchain::StringLiteral GenericFailureMsg("Assuming that '{0}' fails");
