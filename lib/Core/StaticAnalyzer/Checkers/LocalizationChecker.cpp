...
public:
  ...
  void Profile(toolchain::FoldingSetNodeID &ID) const { ID.AddInteger(K); }
};
...
class NonLocalizedStringChecker
  ...
  mutable toolchain::DenseMap<const IdentifierInfo *,
                         toolchain::DenseMap<Selector, uint8_t>> UIMethods;
  // Methods that return a localized string
  mutable toolchain::SmallSet<std::pair<const IdentifierInfo *, Selector>, 12> LSM;
  // C Functions that return a localized string
  mutable toolchain::SmallSet<const IdentifierInfo *, 5> LSF;
...
#define NEW_RECEIVER(receiver)                                                 \
  toolchain::DenseMap<Selector, uint8_t> &receiver##M =                             \
...
void NonLocalizedStringChecker::checkPreObjCMessage(const ObjCMethodCall &msg,
  ...
  if (argumentNumber < 0) { // There was no match in UIMethods
    if (const Decl *D = msg.getDecl()) {
      if (const ObjCMethodDecl *OMD = dyn_cast_or_null<ObjCMethodDecl>(D)) {
        for (auto [Idx, FormalParam] : toolchain::enumerate(OMD->parameters())) {
  ...
  if (const ObjCStringRegion *SR =
    ...
    if ((stringValue.trim().size() == 0 && stringValue.size() > 0) ||
        stringValue.empty())
      return;
    if (!IsAggressive && toolchain::sys::unicode::columnWidthUTF8(stringValue) < 2)
      return;
  }
...
void EmptyLocalizationContextChecker::MethodCrawler::VisitObjCMessageExpr(
  ...
  std::optional<toolchain::MemoryBufferRef> BF =
...
class PluralMisuseChecker : public Checker<check::ASTCodeBody> {
  ...
  class MethodCrawler : public DynamicRecursiveASTVisitor {
    ...
    toolchain::SmallVector<const language::Core::Stmt *, 8> MatchingStatements;
...
bool PluralMisuseChecker::MethodCrawler::isCheckingPlurality(
  ...
  if (IntegerLiteral *IL = dyn_cast_or_null<IntegerLiteral>(
    ...
    toolchain::APInt Value = IL->getValue();
    if (Value == 1 || Value == 2) {
      return true;
    }
  }
