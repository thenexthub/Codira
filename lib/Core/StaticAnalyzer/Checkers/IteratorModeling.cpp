...
void IteratorModeling::checkLiveSymbols(ProgramStateRef State,
  ...
  for (const IteratorPosition &Pos : toolchain::make_second_range(RegionMap)) {
  ...
  for (const IteratorPosition &Pos : toolchain::make_second_range(SymbolMap)) {
    for (SymbolRef Sym : Pos.getOffset()->symbols())
      if (isa<SymbolData>(Sym))
        SR.markLive(Sym);
  }
...
void IteratorModeling::handleIncrement(CheckerContext &C, SVal RetVal,
  ...
  auto NewState =
    advancePosition(State, Iter, OO_Plus,
                    nonloc::ConcreteInt(BVF.getValue(toolchain::APSInt::get(1))));
  assert(NewState &&
