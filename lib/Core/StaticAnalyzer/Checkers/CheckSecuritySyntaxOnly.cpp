...
static bool isArc4RandomAvailable(const ASTContext &Ctx) {
  const toolchain::Triple &T = Ctx.getTargetInfo().getTriple();
  return T.getVendor() == toolchain::Triple::Apple ||
         T.isOSFreeBSD() ||
         T.isOSNetBSD() ||
         T.isOSOpenBSD() ||
         T.isOSDragonFly();
...
void WalkAST::VisitCallExpr(CallExpr *CE) {
  ...
  FnCheck evalFunction =
      toolchain::StringSwitch<FnCheck>(Name)
...
void WalkAST::VisitObjCMessageExpr(ObjCMessageExpr *ME) {
  MsgCheck evalFunction =
      toolchain::StringSwitch<MsgCheck>(ME->getSelector().getAsString())
...
void WalkAST::checkCall_mkstemp(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  std::pair<signed, signed> ArgSuffix =
    toolchain::StringSwitch<std::pair<signed, signed> >(Name)
...
void WalkAST::checkDeprecatedOrUnsafeBufferHandling(const CallExpr *CE,
  ...
  int ArgIndex =
      toolchain::StringSwitch<int>(Name)
  ...
  toolchain::raw_svector_ostream Out1(Buf1);
  toolchain::raw_svector_ostream Out2(Buf2);
...
void WalkAST::checkCall_rand(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  toolchain::raw_svector_ostream os1(buf1);
  os1 << '\'' << *FD << "' is a poor random number generator";

  SmallString<256> buf2;
  toolchain::raw_svector_ostream os2(buf2);
  os2 << "Function '" << *FD
...
void WalkAST::checkMsg_decodeValueOfObjCType(const ObjCMessageExpr *ME) {
  ...
  const toolchain::Triple &T = TI.getTriple();
