...
private:
  ...
  const toolchain::StringSet<> StdSmartPtrClasses = {
      "shared_ptr",
      "unique_ptr",
      "weak_ptr",
  };
  ...
  const toolchain::StringSet<> StdSafeClasses = {
      "basic_filebuf",
      "basic_ios",
      "future",
      "optional",
  ...
  // the stream.
  void explainObject(ProgramStateRef State, toolchain::raw_ostream &OS,
                     const MemRegion *MR, const CXXRecordDecl *RD,
                     MisuseKind MK) const;

  bool belongsTo(const CXXRecordDecl *RD, const toolchain::StringSet<> &Set) const;
...
public:
  void setAggressiveness(StringRef Str, CheckerManager &Mgr) {
    Aggressiveness =
        toolchain::StringSwitch<AggressivenessKind>(Str)
...
bool MoveChecker::belongsTo(const CXXRecordDecl *RD,
                            const toolchain::StringSet<> &Set) const {
...
void MoveChecker::explainObject(ProgramStateRef State, toolchain::raw_ostream &OS,
                                const MemRegion *MR, const CXXRecordDecl *RD,
                                MisuseKind MK) const {
  ...
  if (Call) {
    ...
    for (const auto *Region : RequestedRegions) {
      if (ThisRegion != Region &&
          toolchain::is_contained(InvalidatedRegions, Region))
