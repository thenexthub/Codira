...
constexpr toolchain::StringLiteral MsgUncontrolledFormatString =
...
constexpr toolchain::StringLiteral MsgSanitizeSystemArgs =
    "Untrusted data is passed to a system call "
    "(CERT/STR02-C. Sanitize data passed to complex subsystems)";

/// Check if tainted data is used as a custom sink's parameter.
constexpr toolchain::StringLiteral MsgCustomSink =
    "Untrusted data is passed to a user-defined sink";

using ArgIdxTy = int;
using ArgVecTy = toolchain::SmallVector<ArgIdxTy, 2>;
...
const NoteTag *taintOriginTrackerTag(CheckerContext &C,
  ...
  return C.getNoteTag([TaintedSymbols = std::move(TaintedSymbols),
    ...
    LLVM_DEBUG(for (auto Arg
                    : TaintedArgs) {
      toolchain::dbgs() << "Taint Propagated from argument " << Arg + 1 << "\n";
    });
    return "";
  });
}
...
const NoteTag *taintPropagationExplainerTag(
  ...
  return C.getNoteTag([TaintedSymbols = std::move(TaintedSymbols),
    ...
    for (auto [Idx, Sym] : toolchain::enumerate(TaintedSymbols)) {
      if (BR.isInteresting(Sym)) {
        BR.markInteresting(CallLocation);
        if (TaintedArgs[Idx] != ReturnValueIndex) {
          LLVM_DEBUG(toolchain::dbgs() << "Taint Propagated to argument "
                                  << TaintedArgs[Idx] + 1 << "\n");
          if (nofTaintedArgs == 0)
          ...
          Out << TaintedArgs[Idx] + 1
              << toolchain::getOrdinalSuffix(TaintedArgs[Idx] + 1) << " argument";
          nofTaintedArgs++;
        } else {
          LLVM_DEBUG(toolchain::dbgs() << "Taint Propagated to return value.\n");
...
public:
  ...
  bool contains(ArgIdxTy ArgIdx) const {
    if (toolchain::is_contained(DiscreteArgs, ArgIdx))
      return true;
...
private:
  using NamePartsTy = toolchain::SmallVector<StringRef, 2>;
...
void GenericTaintChecker::initTaintRules(CheckerContext &C) const {
  ...
  RulesConstructionTy GlobalCRules{
      ...
      // See the details here: https://github.com/toolchain/toolchain-project/pull/66086
...
void GenericTaintChecker::checkPostCall(const CallEvent &Call,
  ...
  LLVM_DEBUG(for (ArgIdxTy I
                  : *TaintArgs) {
    toolchain::dbgs() << "PostCall<";
    Call.dump(toolchain::dbgs());
    toolchain::dbgs() << "> actually wants to taint arg index: " << I << '\n';
  });
  ...
  ForEachCallArg([this, &C, &IsMatching, &State, &TaintedSymbols,
    ...
    if (TaintedSVal && !isStdin(*TaintedSVal, C.getASTContext())) {
      ...
      if (!TaintedArgSyms.empty()) {
        toolchain::append_range(TaintedSymbols, TaintedArgSyms);
  ...
  ForEachCallArg(
      [&](ArgIdxTy I, const Expr *E, SVal V) {
        if (PropDstArgs.contains(I)) {
          LLVM_DEBUG(toolchain::dbgs() << "PreCall<"; Call.dump(toolchain::dbgs());
                     toolchain::dbgs()
        ...
        if (WouldEscape(V, E->getType()) && getTaintedPointeeOrPointer(State, V)) {
          LLVM_DEBUG(if (!Result.contains(I)) {
            toolchain::dbgs() << "PreCall<";
            Call.dump(toolchain::dbgs());
            toolchain::dbgs() << "> prepares tainting arg index: " << I << '\n';
          });
          Result = F.add(Result, I);
        }
      });
