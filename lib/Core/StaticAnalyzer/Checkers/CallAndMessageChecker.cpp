...
static void describeUninitializedArgumentInCall(const CallEvent &Call,
  ...
  case CE_ObjCMessage: {
    ...
    case OCM_Message:
      Os << (ArgumentNumber + 1) << toolchain::getOrdinalSuffix(ArgumentNumber + 1)
...
static bool supportsNilWithFloatRet(const toolchain::Triple &triple) {
  return (triple.getVendor() == toolchain::Triple::Apple &&
