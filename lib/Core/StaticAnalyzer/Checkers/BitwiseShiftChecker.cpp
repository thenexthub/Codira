...
struct NoteTagTemplate {
  toolchain::StringLiteral SignInfo;
  toolchain::StringLiteral UpperBoundIntro;
};
...
BugReportPtr BitwiseShiftValidator::checkOvershift() {
  ...
  if (auto ConcreteRight = Right.getAs<nonloc::ConcreteInt>())
    ...
    SValBuilder &SVB = Ctx.getSValBuilder();
    if (const toolchain::APSInt *MinRight = SVB.getMinValue(FoldedState, Right);
...
void BitwiseShiftValidator::recordAssumption(OperandSide Side,
  ...
  switch (Comparison)  {
    ...
    case BO_LT:
      ...
      break;
    default:
      toolchain_unreachable("this checker does not use other comparison operators");
  }
}
