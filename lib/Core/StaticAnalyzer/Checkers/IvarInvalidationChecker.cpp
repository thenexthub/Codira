...
class IvarInvalidationCheckerImpl {
  typedef toolchain::SmallSetVector<const ObjCMethodDecl*, 2> MethodSet;
  typedef toolchain::DenseMap<const ObjCMethodDecl*,
                         const ObjCIvarDecl*> MethToIvarMapTy;
  typedef toolchain::DenseMap<const ObjCPropertyDecl*,
                         const ObjCIvarDecl*> PropToIvarMapTy;
  typedef toolchain::DenseMap<const ObjCIvarDecl*,
  ...
  typedef toolchain::DenseMap<const ObjCIvarDecl*, InvalidationInfo> IvarSet;
  ...
  static void printIvar(toolchain::raw_svector_ostream &os,
...
const ObjCIvarDecl *IvarInvalidationCheckerImpl::findPropertyBackingIvar(
  ...
  for (const ObjCIvarDecl *Iv : toolchain::make_first_range(TrackedIvars)) {
    ...
    {
      toolchain::raw_svector_ostream os(PropNameWithUnderscore);
      os << '_' << PropName;
    }
...
void IvarInvalidationCheckerImpl::printIvar(toolchain::raw_svector_ostream &os,
                                      const ObjCIvarDecl *IvarDecl,
                                      const IvarToPropMapTy &IvarToPopertyMap) {
...
visit(const ObjCImplementationDecl *ImplD) const {
  ...
  for (const ObjCPropertyDecl *PD : toolchain::make_second_range(PropMap)) {
  ...
  for (const ObjCMethodDecl *InterfD : Info.InvalidationMethods) {
    ...
    if (D && D->hasBody()) {
      ...
      for (const ObjCIvarDecl *Ivar : toolchain::make_first_range(IvarsI))
  ...
  if (!AtImplementationContainsAtLeastOneInvalidationMethod) {
    if (AtImplementationContainsAtLeastOnePartialInvalidationMethod) {
      ...
      // invalidation methods.
      for (const ObjCIvarDecl *Ivar : toolchain::make_first_range(Ivars))
