...
static std::optional<int> getCreateFlagValue(const ASTContext &Ctx,
  ...
  if (Ctx.getTargetInfo().getTriple().getVendor() == toolchain::Triple::Apple)
    return {0x0200};
  return MacroVal;
}
  ...
  if (Call.getNumArgs() == MaxArgCount) {
    ...
    if (!QT->isIntegerType()) {
      ...
      OS << "The " << CreateModeArgIndex + 1
         << toolchain::getOrdinalSuffix(CreateModeArgIndex + 1)
...
ProgramStateRef UnixAPIMisuseChecker::EnsureGetdelimBufferAndSizeCorrect(
    SVal LinePtrPtrSVal, SVal SizePtrSVal, const Expr *LinePtrPtrExpr,
    const Expr *SizePtrExpr, CheckerContext &C, ProgramStateRef State) const {
  static constexpr toolchain::StringLiteral SizeGreaterThanBufferSize =
      "The buffer from the first argument is smaller than the size "
      "specified by the second parameter";
  static constexpr toolchain::StringLiteral SizeUndef =
