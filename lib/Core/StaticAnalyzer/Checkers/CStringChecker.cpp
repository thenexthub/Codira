...
public:
  ...
  static ProgramStateRef invalidateBufferAux(
      ...
      toolchain::function_ref<bool(RegionAndSymbolInvalidationTraits &,
                              const MemRegion *)>
  ...
  if (stateNull && !stateNonNull) {
    if (NullArg.isEnabled()) {
      ...
      OS << "Null pointer passed as " << (Arg.ArgumentIndex + 1)
         << toolchain::getOrdinalSuffix(Arg.ArgumentIndex + 1) << " argument to "
...
static void printIdxWithOrdinalSuffix(toolchain::raw_ostream &Os, unsigned Idx) {
  Os << Idx << toolchain::getOrdinalSuffix(Idx);
}
...
ProgramStateRef CStringChecker::checkInit(CheckerContext &C,
  ...
  std::optional<NonLoc> Offset =
      SVB.evalBinOpNN(State, language::Core::BO_Div, Size.castAs<NonLoc>(), ElemSize,
                      IdxTy)
          .getAs<NonLoc>();
  ...
  if (UninitializedRead.isEnabled() &&
    ...
    const toolchain::APSInt *IdxInt = LastIdx.getAsInteger();
...
ProgramStateRef CStringChecker::checkAdditionOverflow(CheckerContext &C,
  ...
  const toolchain::APSInt &maxValInt = BVF.getMaxValue(sizeTy);
...
SVal CStringChecker::getCStringLengthForRegion(CheckerContext &C,
  ...
  if (!hypothetical) {
    if (std::optional<NonLoc> strLn = strLength.getAs<NonLoc>()) {
      ...
      BasicValueFactory &BVF = svalBuilder.getBasicValueFactory();
      const toolchain::APSInt &maxValInt = BVF.getMaxValue(sizeTy);
      toolchain::APSInt fourInt = APSIntType(maxValInt).getValue(4);
...
void CStringChecker::evalStrcmpCommon(CheckerContext &C, const CallEvent &Call,
  ...
  if (LeftStrLiteral && RightStrLiteral) {
    ...
    if (IsBounded) {
      ...
      if (const toolchain::APSInt *len = svalBuilder.getKnownValue(state, lenVal)) {
...
void CStringChecker::evalSprintfCommon(CheckerContext &C, const CallEvent &Call,
  ...
  const auto AllArguments =
      toolchain::make_range(CE->getArgs(), CE->getArgs() + CE->getNumArgs());
...
CStringChecker::checkRegionChanges(ProgramStateRef state,
  ...
  toolchain::SmallPtrSet<const MemRegion *, 8> Invalidated;
  toolchain::SmallPtrSet<const MemRegion *, 32> SuperRegions;
  ...
  for (const MemRegion *MR : toolchain::make_first_range(Entries)) {
...
void CStringChecker::checkLiveSymbols(ProgramStateRef state,
  ...
  for (SVal Len : toolchain::make_second_range(Entries)) {
