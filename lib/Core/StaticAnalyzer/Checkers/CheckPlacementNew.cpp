...
SVal PlacementNewChecker::getExtentSizeOfNewTarget(const CXXNewExpr *NE,
  ...
  if (NE->isArray()) {
    ...
    // ArrayIndexType.
    toolchain::APInt I(AstContext.getTypeSizeInChars(SvalBuilder.getArrayIndexType())
                          .getQuantity() *
...
bool PlacementNewChecker::checkPlaceCapacityIsSufficient(
  ...
  if ((SizeOfPlaceCI->getValue() < SizeOfTargetCI->getValue())) {
    if (ExplodedNode *N = C.generateErrorNode(C.getState())) {
      std::string Msg =
          toolchain::formatv("Storage provided to placement new is only {0} bytes, "
...
void PlacementNewChecker::emitBadAlignReport(const Expr *P, CheckerContext &C,
  ...
  if (ExplodedNode *N = C.generateErrorNode(State)) {
    std::string Msg(toolchain::formatv("Storage type is aligned to {0} bytes but "
