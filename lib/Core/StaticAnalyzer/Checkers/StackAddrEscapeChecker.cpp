...
SourceRange StackAddrEscapeChecker::genName(raw_ostream &os, const MemRegion *R,
  ...
  if (const auto *CR = dyn_cast<CompoundLiteralRegion>(R)) {
    ...
    toolchain_unreachable("Invalid region in ReturnStackAddressChecker.");
  }
...
static void EmitReturnedAsPartOfError(toolchain::raw_ostream &OS, SVal ReturnedVal,
...
void StackAddrEscapeChecker::checkAsyncExecutedBlockCaptures(
  ...
  for (const MemRegion *Region : toolchain::make_first_range(Regions)) {
...
void StackAddrEscapeChecker::checkEndFunction(const ReturnStmt *RS,
  ...
  public:
    ...
    toolchain::SmallPtrSet<const MemRegion *, 4> ExcludedRegions;
