...
struct StreamErrorState {
  ...
  LLVM_DUMP_METHOD void dumpToStream(toolchain::raw_ostream &os) const {
...
struct StreamState {
  ...
  StringRef getKindStr() const {
    ...
    toolchain_unreachable("Unknown StreamState!");
  }
...
LLVM_DUMP_METHOD void StreamState::dumpToStream(toolchain::raw_ostream &os) const {
...
static std::optional<int64_t> getKnownValue(ProgramStateRef State, SVal V) {
  ...
  if (const toolchain::APSInt *Int = SVB.getKnownValue(State, V))
...
escapeByStartIndexAndCount(ProgramStateRef State, const CallEvent &Call,
  ...
  for (auto Idx : toolchain::seq(StartIndex, StartIndex + ElementCount)) {
...
void StreamChecker::evalFscanf(const FnDescription *Desc, const CallEvent &Call,
  ...
  if (!E.isStreamEof()) {
    ...
    if (auto const *Callee = Call.getCalleeIdentifier();
      ...
      for (auto EscArg : toolchain::seq(2u, Call.getNumArgs()))
...
public:
  ...
  PathDiagnosticPieceRef VisitNode(const ExplodedNode *N,
    ...
    toolchain::StringLiteral Msg = "Stream is closed here";
...
void StreamChecker::checkDeadSymbols(SymbolReaper &SymReaper,
  ...
  toolchain::SmallVector<SymbolRef, 2> LeakedSyms;
