...
public:
  ...
  bool operator()(const toolchain::APSInt &EnumDeclInitValue) {
...
using EnumValueVector = toolchain::SmallVector<toolchain::APSInt, 6>;
...
EnumValueVector getDeclValuesForEnum(const EnumDecl *ED) {
  ...
  toolchain::transform(ED->enumerators(), DeclValues.begin(),
...
void EnumCastOutOfRangeChecker::checkPreStmt(const CastExpr *CE,
  ...
  // [[language::Core::flag_enum]] annotated enums are by definition should be ignored.
  ...
  bool PossibleValueMatch =
      toolchain::any_of(DeclValues, ConstraintBasedEQEvaluator(C, *ValueToCast));
