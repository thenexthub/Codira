...
#include "language/Core/StaticAnalyzer/Checkers/SValExplainer.h"
...
class ExprInspectionChecker
  ...
  mutable toolchain::DenseMap<const CallExpr *, ReachedStat> ReachedStats;
  ...
  ExplodedNode *reportBug(toolchain::StringRef Msg, CheckerContext &C,
                          std::optional<SVal> ExprVal = std::nullopt) const;
  ExplodedNode *reportBug(toolchain::StringRef Msg, BugReporter &BR, ExplodedNode *N,
                          std::optional<SVal> ExprVal = std::nullopt) const;
...
bool ExprInspectionChecker::evalCall(const CallEvent &Call,
  ...
  FnCheck Handler =
      toolchain::StringSwitch<FnCheck>(C.getCalleeName(CE))
...
static const char *getArgumentValueString(const CallExpr *CE,
  ...
  if (StTrue) {
    ...
    if (StFalse)
      ...
      toolchain_unreachable("Invalid constraint; neither true or false.");
  }
}

ExplodedNode *
ExprInspectionChecker::reportBug(toolchain::StringRef Msg, CheckerContext &C,
...
ExplodedNode *
ExprInspectionChecker::reportBug(toolchain::StringRef Msg, BugReporter &BR,
                                 ExplodedNode *N,
                                 std::optional<SVal> ExprVal) const {
  if (!N)
    return nullptr;
...
static void printHelper(toolchain::raw_svector_ostream &Out, CheckerContext &C,
                        const toolchain::APSInt &I) {
...
void ExprInspectionChecker::analyzerValue(const CallExpr *CE,
  ...
  else if (const toolchain::APSInt *I = V.getAsInteger())
    printAndReport(C, *I);
  else
    reportBug("n/a", C);
}
...
void ExprInspectionChecker::checkEndAnalysis(ExplodedGraph &G, BugReporter &BR,
  ...
  for (auto Item : ReachedStats) {
    ...
    reportBug(toolchain::to_string(NumTimesReached), BR, N);
  }
  ReachedStats.clear();
}
