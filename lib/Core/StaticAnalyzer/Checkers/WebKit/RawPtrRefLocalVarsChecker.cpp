  ...
  void visitVarDecl(const VarDecl *V, const Expr *Value,
    ...
    if (IsUncountedPtr && *IsUncountedPtr) {
      if (tryToFindPtrOrigin(
              ...
              [&](const language::Core::CXXRecordDecl *Record) {
                return isSafePtr(Record);
              },
              [&](const language::Core::QualType Type) { return isSafePtrType(Type); },
              [&](const language::Core::Expr *InitArgOrigin, bool IsSafe) {
                ...
                if (auto *Ref = toolchain::dyn_cast<DeclRefExpr>(InitArgOrigin)) {
