...
public:
  ...
  bool isPtrOriginSafe(const Expr *Arg) const {
    return tryToFindPtrOrigin(
        ...
        [&](const language::Core::CXXRecordDecl *Record) { return isSafePtr(Record); },
        [&](const language::Core::QualType T) { return isSafePtrType(T); },
        [&](const language::Core::Expr *ArgOrigin, bool IsSafe) {
          if (IsSafe)
            return true;
