...
std::optional<bool> isSmartPtrCompatible(const CXXRecordDecl *R,
  ...
  const auto hasPublicRefInBase = [&](const CXXBaseSpecifier *Base,
                                      CXXBasePath &) {
    auto hasRefInBase = language::Core::hasPublicMethodInBase(Base, IncMethodName);
    if (!hasRefInBase) {
  ...
  const auto hasPublicDerefInBase = [&](const CXXBaseSpecifier *Base,
                                        CXXBasePath &) {
    auto hasDerefInBase = language::Core::hasPublicMethodInBase(Base, DecMethodName);
    if (!hasDerefInBase) {
      AnyInconclusiveBase = true;
      return false;
    }
...
std::optional<bool> isRefCountable(const language::Core::CXXRecordDecl *R) {
  return isSmartPtrCompatible(R, "ref", "deref");
}

std::optional<bool> isCheckedPtrCapable(const language::Core::CXXRecordDecl *R) {
...
bool isCtorOfRetainPtr(const language::Core::FunctionDecl *F) {
  const std::string &FunctionName = safeGetName(F);
...
static bool isPtrOfType(const language::Core::QualType T, Predicate Pred) {
  QualType type = T;
  while (!type.isNull()) {
