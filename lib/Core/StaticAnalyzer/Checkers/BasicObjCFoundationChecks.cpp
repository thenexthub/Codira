...
static FoundationClass findKnownClass(const ObjCInterfaceDecl *ID,
  ...
  static toolchain::StringMap<FoundationClass> Classes;
  if (Classes.empty()) {
...
class NilArgChecker : public Checker<check::PreObjCMessage,
  ...
  mutable toolchain::SmallDenseMap<Selector, unsigned, 16> StringSelectors;
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    ...
    if (CanBeSubscript && msg.getMessageKind() == OCM_Subscript) {
      ...
      if (Class == FC_NSArray) {
        ...
        toolchain_unreachable("Missing foundation class for the subscript expr");
...
void ObjCLoopChecker::checkDeadSymbols(SymbolReaper &SymReaper,
  ...
  for (SymbolRef Sym : toolchain::make_first_range(Tracked)) {
