...
const char *getNullabilityString(Nullability Nullab) {
  ...
  toolchain_unreachable("Unexpected enumeration.");
  return "";
}
...
void NullabilityChecker::checkDeadSymbols(SymbolReaper &SR,
  ...
  for (const MemRegion *Reg : toolchain::make_first_range(Nullabilities)) {
  ...
  for (ObjectPropPair PropKey : toolchain::make_first_range(PropertyAccesses)) {
  ...
  for (const ParmVarDecl *Param : Call.parameters()) {
    ...
    if (NullPassedToNonnull.isEnabled() && Nullness == NullConstraint::IsNull &&
      ...
      OS << " passed to a callee that requires a non-null " << ParamIdx
         << toolchain::getOrdinalSuffix(ParamIdx) << " parameter";
    ...
    if (TrackedNullability) {
      ...
      if (NullablePassedToNonnull.isEnabled() &&
        ...
        OS << "Nullable pointer is passed to a callee that requires a non-null "
           << ParamIdx << toolchain::getOrdinalSuffix(ParamIdx) << " parameter";
...
void NullabilityChecker::checkPostCall(const CallEvent &Call,
  ...
  if (toolchain::sys::path::filename(FilePath).starts_with("CG")) {
...
// hardcoded here:
constexpr toolchain::StringLiteral GroupName = "nullability";
constexpr toolchain::StringLiteral GroupOptName = "NoDiagnoseCallsToSystemHeaders";
