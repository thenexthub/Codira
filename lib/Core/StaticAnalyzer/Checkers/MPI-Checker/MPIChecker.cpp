...
void MPIChecker::checkUnmatchedWaits(const CallEvent &PreCallEvent,
  ...
  toolchain::SmallVector<const MemRegion *, 2> ReqRegions;
...
void MPIChecker::allRegionsUsedByWait(
  ...
  if (FuncClassifier->isMPI_Waitall(CE.getCalleeIdentifier())) {
    ...
    const toolchain::APSInt &ArrSize =
...
void language::Core::ento::registerMPIChecker(CheckerManager &MGR) {
  MGR.registerChecker<language::Core::ento::mpi::MPIChecker>();
}

bool language::Core::ento::shouldRegisterMPIChecker(const CheckerManager &mgr) {
  return true;
}
