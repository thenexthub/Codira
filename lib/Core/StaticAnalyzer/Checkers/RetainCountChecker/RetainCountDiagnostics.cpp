...
static bool shouldGenerateNote(toolchain::raw_string_ostream &os,
                               const RefVal *PrevT,
                               const RefVal &CurrV,
                               bool DeallocSent) {
...
using Bindings = toolchain::SmallVector<std::pair<const MemRegion *, SVal>, 4>;
...
void RefLeakReport::createDescription(CheckerContext &Ctx) {
  ...
  Description.clear();
  toolchain::raw_string_ostream os(Description);
...
void RefLeakReport::findBindingToReport(CheckerContext &Ctx,
  ...
  if (!AllVarBindings.empty() &&
      !toolchain::is_contained(toolchain::make_first_range(AllVarBindings),
                          AllocFirstBinding)) {
