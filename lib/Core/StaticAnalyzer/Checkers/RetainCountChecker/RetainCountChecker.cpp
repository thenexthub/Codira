...
void RefVal::print(raw_ostream &Out) const {
  ...
  switch (getKind()) {
    default: toolchain_unreachable("Invalid RefVal kind");
    case Owned: {
      Out << "Owned";
...
ProgramStateRef RetainCountChecker::updateSymbol(ProgramStateRef state,
  ...
  switch (AE.getKind()) {
    ...
    case RetainedOutParameterOnZero:
    case RetainedOutParameterOnNonZero:
      toolchain_unreachable("Applies to pointer-to-pointer parameters, which should "
                       "not have ref state.");

    case Dealloc: // NB. we only need to add a note in a non-error case.
      switch (V.getKind()) {
        default:
          toolchain_unreachable("Invalid RefVal state for an explicit dealloc.");
        case RefVal::Owned:
    ...
    case IncRef:
      switch (V.getKind()) {
        default:
          toolchain_unreachable("Invalid RefVal state for a retain.");
        case RefVal::Owned:
        case RefVal::NotOwned:
          V = V + 1;
          break;
    ...
    case DecRefAndStopTrackingHard:
      switch (V.getKind()) {
        default:
          ...
          toolchain_unreachable("Invalid RefVal state for a release.");
...
RetainCountChecker::errorKindToBugKind(RefVal::Kind ErrorKind,
  ...
  switch (ErrorKind) {
    ...
    default:
      toolchain_unreachable("Unhandled error.");
  }
}
...
ProgramStateRef RetainCountChecker::checkRegionChanges(
  ...
  toolchain::SmallPtrSet<SymbolRef, 8> AllowedSymbols;
