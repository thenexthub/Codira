...
public:
  ...
  void printNoteMsg(toolchain::raw_ostream &Out) const override {
    toolchain_unreachable("This node can never be the final node in the "
                     "fieldchain!");
  }
...
void UninitializedObjectChecker::checkEndFunction(
  ...
  toolchain::raw_svector_ostream WarningOS(WarningBuf);
...
bool FindUninitializedFields::addFieldToUninits(FieldChainInfo Chain,
  ...
  toolchain::raw_svector_ostream OS(NoteMsgBuf);
...
bool FindUninitializedFields::isNonUnionUninit(const TypedValueRegion *R,
  ...
  for (const FieldDecl *I : RD->fields()) {
    ...
    toolchain_unreachable("All cases are handled!");
  }
...
static void printTail(toolchain::raw_ostream &Out,
...
// 'D1::x' or 'D2::x'.
void FieldChainInfo::printNoteMsg(toolchain::raw_ostream &Out) const {
  if (Chain.isEmpty())
    return;
...
std::string language::Core::ento::getVariableName(const FieldDecl *Field) {
  ...
  if (CXXParent && CXXParent->isLambda()) {
    ...
    if (It->capturesVariable())
      return toolchain::Twine("/*captured variable*/" +
    ...
    toolchain_unreachable("No other capture type is expected!");
  }
...
void ento::registerUninitializedObjectChecker(CheckerManager &Mgr) {
  ...
  std::string ErrorMsg;
  if (!toolchain::Regex(ChOpts.IgnoredRecordsWithFieldPattern).isValid(ErrorMsg))
