...
#ifndef LANGUAGE_CORE_STATICANALYZER_UNINITIALIZEDOBJECT_H
#define LANGUAGE_CORE_STATICANALYZER_UNINITIALIZEDOBJECT_H
...
public:
  ...
  void Profile(toolchain::FoldingSetNodeID &ID) const { ID.AddPointer(this); }
  ...
  virtual void printNoteMsg(toolchain::raw_ostream &Out) const = 0;

  /// Print any prefixes before the fieldchain. Could contain casts, etc.
  virtual void printPrefix(toolchain::raw_ostream &Out) const = 0;

  /// Print the node. Should contain the name of the field stored in FR.
  virtual void printNode(toolchain::raw_ostream &Out) const = 0;

  /// Print the separator. For example, fields may be separated with '.' or
  /// "->".
  virtual void printSeparator(toolchain::raw_ostream &Out) const = 0;
...
public:
  using FieldChain = toolchain::ImmutableList<const FieldNode &>;
...
public:
  ...
  void printNoteMsg(toolchain::raw_ostream &Out) const;
};

using UninitFieldMap = std::map<const FieldRegion *, toolchain::SmallString<50>>;
...
#endif // LANGUAGE_CORE_STATICANALYZER_UNINITIALIZEDOBJECT_H
