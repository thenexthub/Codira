...
BuiltinFunctionChecker::checkOverflow(CheckerContext &C, SVal RetVal,
  ...
  auto MinValType = toolchain::APSInt::getMinValue(BitWidth, IsUnsigned);
  auto MaxValType = toolchain::APSInt::getMaxValue(BitWidth, IsUnsigned);
...
bool BuiltinFunctionChecker::isBuiltinLikeFunction(
  ...
  const auto *FD = toolchain::dyn_cast_or_null<FunctionDecl>(Call.getDecl());
