...
class RAIIMutexDescriptor {
  ...
  mutable toolchain::SmallString<32> GuardName{};
...
public:
  ...
  PathDiagnosticPieceRef VisitNode(const ExplodedNode *N,
    ...
    const toolchain::APSInt *FlagVal = N->getSVal(CE->getArg(1)).getAsInteger();
    if (!FlagVal)
      return nullptr;
...
// TODO: Move these to toolchain::ImmutableList when overhauling immutable data
// structures for proper iterator concept support.
template <>
struct std::iterator_traits<
    typename toolchain::ImmutableList<CritSectionMarker>::iterator> {
...
BlockInCriticalSectionChecker::checkDescriptorMatch(const CallEvent &Call,
  ...
  const auto Descriptor =
      toolchain::find_if(MutexDescriptors, [&Call, IsLock](auto &&Descriptor) {
        return std::visit(
...
void BlockInCriticalSectionChecker::handleUnlock(
  ...
  const auto MostRecentLock =
      toolchain::find_if(ActiveSections, [MutexRegion](auto &&Marker) {
  ...
  toolchain::ImmutableList<CritSectionMarker> NewList = Factory.getEmptyList();
...
BlockInCriticalSectionChecker::createCritSectionNote(CritSectionMarker M,
  ...
  return C.getNoteTag([M, BT](PathSensitiveBugReport &BR,
    ...
    toolchain::SmallVector<CritSectionMarker, 4> LocksForMutex;
    toolchain::copy_if(
    ...
    const auto Position =
        toolchain::find_if(std::as_const(LocksForMutex), [M](const auto &Marker) {
    ...
    OS << "Entering critical section for the " << OrdinalOfLock
       << toolchain::getOrdinalSuffix(OrdinalOfLock) << " time here";
  });
}
