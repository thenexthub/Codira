...
public:
  bool inEH;
  toolchain::DenseSet<const VarDecl *> &S;
  ...
  EHCodeVisitor(toolchain::DenseSet<const VarDecl *> &S) :
  inEH(false), S(S) {}
};
...
void ReachableCode::computeReachableBlocks() {
  ...
  while (!worklist.empty()) {
    ...
    toolchain::BitVector::reference isReachable = reachable[block->getBlockID()];
    if (isReachable)
      continue;
    isReachable = true;
...
class DeadStoreObs : public LiveVariables::Observer {
  ...
  ParentMap& Parents;
  toolchain::SmallPtrSet<const VarDecl*, 20> Escaped;
  std::unique_ptr<ReachableCode> reachableCode;
  const CFGBlock *currentBlock;
  std::unique_ptr<toolchain::DenseSet<const VarDecl *>> InEH;
...
public:
  ...
  bool isLive(const LiveVariables::LivenessValues &Live, const VarDecl *D) {
    ...
    // EH code.
    if (!InEH) {
      InEH.reset(new toolchain::DenseSet<const VarDecl *>());
...
private:
  ...
  bool isConstant(const InitListExpr *Candidate) const {
    ...
    return toolchain::all_of(Candidate->inits(), [this](const Expr *Init) {
...
public:
  ...
  void findLambdaReferenceCaptures(const LambdaExpr *LE)  {
    ...
    toolchain::DenseMap<const ValueDecl *, FieldDecl *> CaptureFields;
