...
void taint::dumpTaint(ProgramStateRef State) {
  printTaint(State, toolchain::errs());
}
...
std::vector<SymbolRef> taint::getTaintedSymbolsImpl(ProgramStateRef State,
  ...
  if (const ElementRegion *ER = dyn_cast<ElementRegion>(Reg)) {
    ...
    toolchain::append_range(TaintedSymbols, TaintedIndex);
  ...
  if (const SymbolicRegion *SR = dyn_cast<SymbolicRegion>(Reg)) {
    ...
    toolchain::append_range(TaintedSymbols, TaintedRegions);
    if (returnFirstOnly && !TaintedSymbols.empty())
      return TaintedSymbols; // return early if needed
  }
  ...
  if (const SubRegion *ER = dyn_cast<SubRegion>(Reg)) {
    ...
    toolchain::append_range(TaintedSymbols, TaintedSubRegions);
    if (returnFirstOnly && !TaintedSymbols.empty())
      return TaintedSymbols; // return early if needed
  }
...
std::vector<SymbolRef> taint::getTaintedSymbolsImpl(ProgramStateRef State,
  ...
  // HACK:https://discourse.toolchain.org/t/rfc-make-istainted-and-complex-symbols-friends/79570
  ...
  for (SymbolRef SubSym : Sym->symbols()) {
    ...
    if (const auto *SD = dyn_cast<SymbolDerived>(SubSym)) {
      ...
      toolchain::append_range(TaintedSymbols, TaintedParents);
      if (returnFirstOnly && !TaintedSymbols.empty())
        return TaintedSymbols; // return early if needed
    ...
    if (const auto *SC = dyn_cast<SymbolCast>(SubSym)) {
      ...
      toolchain::append_range(TaintedSymbols, TaintedCasts);
      if (returnFirstOnly && !TaintedSymbols.empty())
        return TaintedSymbols; // return early if needed
    }
  }
