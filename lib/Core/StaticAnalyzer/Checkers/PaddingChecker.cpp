...
public:
  ...
  void visitRecord(const RecordDecl *RD, uint64_t PadMultiplier = 1) const {
    ...
    assert(toolchain::isPowerOf2_64(RL.getAlignment().getQuantity()));
  ...
  bool shouldSkipDecl(const RecordDecl *RD) const {
    ...
    if (toolchain::any_of(RD->fields(), IsTrickyField))
      return true;
    return false;
  }
  ...
  calculateOptimalPad(const RecordDecl *RD, const ASTContext &ASTContext,
    ...
    auto GatherSizesAndAlignments = [](const FieldDecl *FD) {
      ...
      assert(toolchain::isPowerOf2_64(RetVal.Align.getQuantity()));
    ...
    while (!Fields.empty()) {
      unsigned TrailingZeros =
          toolchain::countr_zero((unsigned long long)NewOffset.getQuantity());
      ...
      // aligned field.
      auto Iter = toolchain::upper_bound(Fields, InsertPoint);
