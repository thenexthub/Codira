...
ProgramStateRef VLASizeChecker::checkVLA(CheckerContext &C,
  ...
  toolchain::SmallVector<const Expr *, 2> VLASizes;
  ...
  if (const toolchain::APSInt *KV = SVB.getKnownValue(State, ArrSize))
  ...
  for (const Expr *SizeE : VLASizes) {
    ...
    if (const toolchain::APSInt *IndexLVal = SVB.getKnownValue(State, IndexLength)) {
      ...
      // FIXME: See https://reviews.toolchain.org/D80903 for discussion of
