...
static bool rewriteToNumericBoxedExpression(const ObjCMessageExpr *Msg,
  ...
  if (const ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(Arg)) {
    ...
    case CK_BooleanToSignedIntegral:
      toolchain_unreachable("OpenCL-specific cast in Objective-C?");

    case CK_HLSLVectorTruncation:
    case CK_HLSLElementwiseCast:
    case CK_HLSLAggregateSplatCast:
      toolchain_unreachable("HLSL-specific cast in Objective-C?");
      break;
    ...
    case CK_IntegralToFixedPoint:
      toolchain_unreachable("Fixed point types are disabled for Objective-C");
    }
  }
