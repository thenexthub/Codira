...
#include "language/Core/Analysis/Analyses/UnsafeBufferUsage.h"
...
private:
  toolchain::StringMap<DynTypedNode> Nodes;
};
} // namespace
...
static void findStmtsInUnspecifiedLvalueContext(
    const Stmt *S, const toolchain::function_ref<void(const Expr *)> OnResult) {
...
static void findStmtsInUnspecifiedPointerContext(
    const Stmt *S, toolchain::function_ref<void(const Stmt *)> InnerMatcher) {
  // A UPC can be
...
static bool isPtrBufferSafe(const Expr *Ptr, const Expr *Size,
  ...
  if (auto *MCEPtr = dyn_cast<CXXMemberCallExpr>(Ptr->IgnoreParenImpCasts()))
    if (auto *MCESize =
      ...
      return toolchain::is_contained({SIZED_CONTAINER_OR_VIEW_LIST},
                                ObjII->getName());
    }
  ...
  if (Size->EvaluateAsInt(ER, Ctx)) {
    ...
    if (auto *DRE = dyn_cast<DeclRefExpr>(Ptr->IgnoreParenImpCasts())) {
      if (auto *CAT = Ctx.getAsConstantArrayType(DRE->getType())) {
        toolchain::APSInt SizeInt = ER.Val.getInt();

        return toolchain::APSInt::compareValues(
                   SizeInt, toolchain::APSInt(CAT->getSize(), true)) == 0;
      }
      return false;
    }
...
static bool isSafeSpanTwoParamConstruct(const CXXConstructExpr &Node,
  ...
  auto HaveEqualConstantValues = [&Ctx](const Expr *E0, const Expr *E1) {
    if (auto E0CV = E0->getIntegerConstantExpr(Ctx))
      if (auto E1CV = E1->getIntegerConstantExpr(Ctx)) {
        return toolchain::APSInt::compareValues(*E0CV, *E1CV) == 0;
      }
    return false;
  };
  ...
  std::optional<toolchain::APSInt> Arg1CV = Arg1->getIntegerConstantExpr(Ctx);
...
static bool isSafeArraySubscript(const ArraySubscriptExpr &Node,
  ...
  } else if (const auto *SLiteral = dyn_cast<language::Core::StringLiteral>(
  ...
  if (!IndexExpr->isValueDependent() &&
    ...
    toolchain::APSInt ArrIdx = EVResult.Val.getInt();
...
static bool isNullTermPointer(const Expr *Ptr) {
  if (isa<language::Core::StringLiteral>(Ptr->IgnoreParenImpCasts()))
    return true;
...
static bool hasUnsafeFormatOrSArg(const CallExpr *Call, const Expr *&UnsafeArg,
  ...
  class StringFormatStringHandler
    ...
    getPrecisionAsExpr(const analyze_printf::OptionalAmount &Precision,
      ...
      if (Precision.getHowSpecified() ==
        ...
        toolchain::APSInt PArgVal = toolchain::APSInt(
            toolchain::APInt(Ctx.getTypeSize(SizeTy), Precision.getConstantAmount()),
            true);
  ...
  if (auto *SL = dyn_cast<language::Core::StringLiteral>(Fmt->IgnoreParenImpCasts())) {
    StringRef FmtStr;
...
CHECK_UNSAFE_PTR:
  ...
  return toolchain::any_of(
      toolchain::make_range(Call->arg_begin() + FmtArgIdx, Call->arg_end()),
...
static bool hasUnsafePrintfStringArg(const CallExpr &Node, ASTContext &Ctx,
                                     MatchResult &Result, toolchain::StringRef Tag) {
...
public:
  enum class Kind {
#define GADGET(x) x,
#include "language/Core/Analysis/Analyses/UnsafeBufferUsageGadgets.def"
  };
...
#include "language/Core/Analysis/Analyses/UnsafeBufferUsageGadgets.def"
    ...
    toolchain_unreachable("Unhandled Gadget::Kind enum");
  }
#endif
...
public:
  ...
  static bool matches(const Stmt *S,
                      toolchain::SmallVectorImpl<MatchResult> &Results) {
...
private:
  static bool isDataFunction(const CXXMemberCallExpr *call) {
    ...
    if (method->getNameAsString() == "data" &&
        ...
        toolchain::is_contained({SIZED_CONTAINER_OR_VIEW_LIST},
  ...
  static bool matches(const Stmt *S, ASTContext &Ctx,
    ...
    if (CE->getNumArgs() == 1) {
      isSingleStringLiteralArg =
          isa<language::Core::StringLiteral>(CE->getArg(0)->IgnoreParenImpCasts());
    }
...
class DeclUseTracker {
  using UseSetTy = toolchain::SmallSet<const DeclRefExpr *, 16>;
  using DefMapTy = toolchain::DenseMap<const VarDecl *, const DeclStmt *>;
...
public:
  ...
  bool matches(const DynTypedNode &DynNode, ASTContext &Ctx,
    ...
    toolchain::SmallVector<MatchResult> Results;
...
std::set<const Expr *> language::Core::findUnsafePointers(const FunctionDecl *FD) {
...
struct WarningGadgetSets {
  ...
  toolchain::SmallVector<const WarningGadget *, 16> noVar;
};
...
bool language::Core::internal::anyConflict(const SmallVectorImpl<FixItHint> &FixIts,
...
PtrToPtrAssignmentGadget::getFixits(const FixitStrategy &S) const {
  ...
  case FixitStrategy::Kind::Vector:
    toolchain_unreachable("unsupported strategies for FixableGadgets");
  }
  return std::nullopt;
}
...
static bool hasUnsupportedSpecifiers(const VarDecl *VD,
  ...
  // source range of `VD`:
  bool AttrRangeOverlapping = toolchain::any_of(VD->attrs(), [&](Attr *At) -> bool {
...
DerefSimplePtrArithFixableGadget::getFixits(const FixitStrategy &s) const {
  ...
  if (VD && s.lookup(VD) == FixitStrategy::Kind::Span) {
    ...
    CharSourceRange StarWithTrailWhitespace =
        language::Core::CharSourceRange::getCharRange(DerefOp->getOperatorLoc(),
                                             LHS->getBeginLoc());
    ...
    CharSourceRange PlusWithSurroundingWhitespace =
        language::Core::CharSourceRange::getCharRange(*LHSLocation, RHS->getBeginLoc());
    ...
    CharSourceRange ClosingParenWithPrecWhitespace =
        language::Core::CharSourceRange::getCharRange(*AddOpLocation, *DerefOpLocation);
...
PointerDereferenceGadget::getFixits(const FixitStrategy &S) const {
  ...
  case FixitStrategy::Kind::Span: {
    ...
    // Deletes the *operand
    CharSourceRange derefRange = language::Core::CharSourceRange::getCharRange(
  ...
  case FixitStrategy::Kind::Array:
    return std::nullopt;
  case FixitStrategy::Kind::Vector:
    toolchain_unreachable("FixitStrategy not implemented yet!");
  case FixitStrategy::Kind::Wontfix:
    toolchain_unreachable("Invalid strategy!");
  }
...
//   1. Add the `[[language::Core::unsafe_buffer_usage]]` attribute to each declaration
//   of 'F';
...
// also add overloads:
//
// [[language::Core::unsafe_buffer_usage]] void f(int *p); // original decl
// void f(std::span<int> p);                      // added overload decl
// void f(std::span<int> p) {     // original def where param is changed
//    p[5];
// }
// [[language::Core::unsafe_buffer_usage]] void f(int *p) {  // added def
...
static FixItList fixVarDeclWithArray(const VarDecl *D, const ASTContext &Ctx,
  ...
  if (auto CAT = Ctx.getAsConstantArrayType(D->getType())) {
    ...
    if (isa<language::Core::ArrayType>(ArrayEltT.getCanonicalType()))
      return {};
    ...
    if (!MaybeElemTypeTxt)
      return {};
    const toolchain::StringRef ElemTypeTxt = MaybeElemTypeTxt->trim();
    ...
    if (!MaybeArraySizeTxt)
      return {};
    const toolchain::StringRef ArraySizeTxt = MaybeArraySizeTxt->trim();
    ...
    toolchain::raw_svector_ostream OS(Replacement);
...
fixVariable(const VarDecl *VD, FixitStrategy::Kind K,
  ...
  if (const auto *PVD = dyn_cast<ParmVarDecl>(VD)) {
    auto *FD = dyn_cast<language::Core::FunctionDecl>(PVD->getDeclContext());
    if (!FD || FD != D) {
  ...
  toolchain_unreachable("Unknown strategy!");
}
...
static bool overlapWithMacro(const FixItList &FixIts) {
  ...
  return toolchain::any_of(FixIts, [](const FixItHint &Hint) {
...
getFixIts(FixableGadgetSets &FixablesForAllVars, const FixitStrategy &S,
  ...
  for (auto Iter = FinalFixItsForVariable.begin();
    ...
    if (overlapWithMacro(Iter->second) ||
        language::Core::internal::anyConflict(Iter->second, Ctx.getSourceManager())) {
...
getNaiveStrategy(toolchain::iterator_range<VarDeclIterTy> UnsafeVars) {
  FixitStrategy S;
...
class VariableGroupsManagerImpl : public VariableGroupsManager {
  ...
  const toolchain::SetVector<const VarDecl *> &GrpsUnionForParms;
...
static void applyGadgets(const Decl *D, FixableGadgetList FixableGadgets,
  ...
  for (const auto &it : UnsafeOps.byVar) {
    ...
    for (const language::Core::DeclRefExpr *UnclaimedDRE : UnclaimedDREs) {
  ...
  using DepMapTy =
      toolchain::DenseMap<const VarDecl *, toolchain::SetVector<const VarDecl *>>;
  ...
  toolchain::SetVector<const VarDecl *>
  ...
  FixitStrategy NaiveStrategy = getNaiveStrategy(toolchain::make_filter_range(
...
void language::Core::checkUnsafeBufferUsage(const Decl *D,
                                   UnsafeBufferUsageHandler &Handler,
                                   bool EmitSuggestions) {
#ifndef NDEBUG
