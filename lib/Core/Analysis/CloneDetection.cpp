...
void OnlyLargestCloneConstraint::constrain(
  ...
  for (unsigned I : toolchain::reverse(IndexesToRemove))
...
bool FilenamePatternConstraint::isAutoGenerated(
  ...
  for (const StmtSequence &S : Group) {
    ...
    StringRef Filename = toolchain::sys::path::filename(
...
#include "language/Core/AST/StmtDataCollectors.inc"
...
static size_t createHash(toolchain::MD5 &Hash) {
  size_t HashCode;

  // Create the final hash code for the current Stmt.
  toolchain::MD5::MD5Result HashResult;
...
saveHash(const Stmt *S, const Decl *D,
  ...
  CloneTypeIIStmtDataCollector<toolchain::MD5>(S, Context, Hash);
...
class FoldingSetNodeIDWrapper {
  ...
  toolchain::FoldingSetNodeID &FS;

public:
  FoldingSetNodeIDWrapper(toolchain::FoldingSetNodeID &FS) : FS(FS) {}
...
static bool areSequencesClones(const StmtSequence &LHS,
  ...
  toolchain::FoldingSetNodeID DataLHS, DataRHS;
...
void RecursiveCloneTypeIIHashConstraint::constrain(
  ...
  for (CloneDetector::CloneGroup &Group : Sequences) {
    ...
    toolchain::stable_sort(StmtsByHash, toolchain::less_first());
...
void CloneConstraint::splitCloneGroups(
    std::vector<CloneDetector::CloneGroup> &CloneGroups,
    toolchain::function_ref<bool(const StmtSequence &, const StmtSequence &)>
        Compare) {
  std::vector<CloneDetector::CloneGroup> Result;
  for (auto &HashGroup : CloneGroups) {
    ...
    assert(toolchain::all_of(Indexes, [](char c) { return c == 1; }));
  }
  CloneGroups = Result;
}
