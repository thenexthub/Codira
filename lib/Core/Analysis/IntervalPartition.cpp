...
template <typename Node> struct BuildResult {
  ...
  toolchain::SmallDenseSet<const Node *> Successors;
};
...
static BuildResult<Node> buildInterval(toolchain::BitVector &Partitioned,
                                       const Node *Header) {
  ...
  toolchain::BitVector Workset(Partitioned.size(), false);
  ...
  while (!Worklist.empty()) {
    ...
    // is included as well.
    bool AllInInterval = toolchain::all_of(B->preds(), [&](const Node *P) {
      return toolchain::is_contained(Interval.Nodes, P);
    });
    if (AllInInterval) {
  ...
  for (const Node *B : MaybeSuccessors)
    if (!toolchain::is_contained(Interval.Nodes, B))
...
static void fillIntervalNode(CFGIntervalGraph &Graph,
                             ...
                             toolchain::BitVector &Partitioned, const Node *Header) {
  ...
  if constexpr (std::is_same_v<std::decay_t<Node>, CFGBlock>)
    ...
    Nodes.reserve(Count);
    for (auto &N : Result.Nodes)
      toolchain::append_range(Nodes, N->Nodes);
...
static CFGIntervalGraph partitionIntoIntervalsImpl(unsigned NumBlockIDs,
  ...
  toolchain::BitVector Partitioned(NumBlockIDs, false);
  std::queue<const Node *> Successors;
...
std::vector<const CFGBlock *> buildInterval(const CFGBlock *Header) {
  toolchain::BitVector Partitioned(Header->getParent()->getNumBlockIDs(), false);
