...
//
//===----------------------------------------------------------------------===//
#include "language/Core/Analysis/Analyses/LifetimeSafety.h"
...
#include "language/Core/Analysis/FlowSensitive/DataflowWorklist.h"
...
struct AccessPath {
  const language::Core::ValueDecl *D;

  AccessPath(const language::Core::ValueDecl *D) : D(D) {}
};
...
/// See discussion:
/// https://github.com/toolchain/toolchain-project/pull/142313/commits/0cd187b01e61b200d92ca0b640789c1586075142#r2137644238
struct Origin {
  OriginID ID;
  ...
  toolchain::PointerUnion<const language::Core::ValueDecl *, const language::Core::Expr *> Ptr;

  Origin(OriginID ID, const language::Core::ValueDecl *D) : ID(ID), Ptr(D) {}
  Origin(OriginID ID, const language::Core::Expr *E) : ID(ID), Ptr(E) {}

  const language::Core::ValueDecl *getDecl() const {
    return Ptr.dyn_cast<const language::Core::ValueDecl *>();
  }
  const language::Core::Expr *getExpr() const {
    return Ptr.dyn_cast<const language::Core::Expr *>();
  }
};
...
public:
  ...
  toolchain::ArrayRef<Loan> getLoans() const { return AllLoans; }
...
private:
  ...
  /// optimisation.
  toolchain::SmallVector<Loan> AllLoans;
};
...
public:
  ...
  Origin &addOrigin(OriginID ID, const language::Core::ValueDecl &D) {
    AllOrigins.emplace_back(ID, &D);
    return AllOrigins.back();
  }
  Origin &addOrigin(OriginID ID, const language::Core::Expr &E) {
  ...
  toolchain::ArrayRef<Origin> getOrigins() const { return AllOrigins; }
...
private:
  ...
  /// TODO(opt): Profile and evaluate the usefullness of small buffer
  /// optimisation.
  toolchain::SmallVector<Origin> AllOrigins;
  toolchain::DenseMap<const language::Core::ValueDecl *, OriginID> DeclToOriginID;
  toolchain::DenseMap<const language::Core::Expr *, OriginID> ExprToOriginID;
};
...
public:
  ...
  virtual void dump(toolchain::raw_ostream &OS) const {
...
public:
  ...
  void dump(toolchain::raw_ostream &OS) const override {
...
class FactManager {
public:
  toolchain::ArrayRef<const Fact *> getFacts(const CFGBlock *B) const {
  ...
  void addBlockFacts(const CFGBlock *B, toolchain::ArrayRef<Fact *> NewFacts) {
    if (!NewFacts.empty())
  ...
  void dump(const CFG &Cfg, AnalysisDeclContext &AC) const {
    toolchain::dbgs() << "==========================================\n";
    toolchain::dbgs() << "       Lifetime Analysis Facts:\n";
    toolchain::dbgs() << "==========================================\n";
    if (const Decl *D = AC.getDecl())
      if (const auto *ND = dyn_cast<NamedDecl>(D))
        toolchain::dbgs() << "Function: " << ND->getQualifiedNameAsString() << "\n";
    // Print blocks in the order as they appear in code for a stable ordering.
    for (const CFGBlock *B : *AC.getAnalysis<PostOrderCFGView>()) {
      toolchain::dbgs() << "  Block B" << B->getBlockID() << ":\n";
      auto It = BlockToFactsMap.find(B);
      if (It != BlockToFactsMap.end()) {
        for (const Fact *F : It->second) {
          toolchain::dbgs() << "    ";
          F->dump(toolchain::dbgs());
        }
      }
      toolchain::dbgs() << "  End of Block\n";
    }
  }
...
private:
  ...
  toolchain::DenseMap<const language::Core::CFGBlock *, toolchain::SmallVector<const Fact *>>
      BlockToFactsMap;
  toolchain::BumpPtrAllocator FactAllocator;
};
...
public:
  ...
  void run() {
    toolchain::TimeTraceScope TimeProfile("FactGenerator");
...
private:
  ...
  void handleDestructor(const CFGAutomaticObjDtor &DtorOpt) {
    ...
    /// TODO: Consider using language::Core::CFG::BuildOptions::AddLifetime to reuse the
    /// lifetime ends.
  ...
  bool VisitTestPoint(const CXXFunctionalCastExpr *FCE) {
    ...
    if (const auto *SL = dyn_cast<StringLiteral>(SubExpr)) {
      toolchain::StringRef LiteralValue = SL->getString();
  ...
  toolchain::SmallVector<Fact *> CurrentBlockFacts;
};
...
private:
  ...
  toolchain::DenseMap<const CFGBlock *, Lattice> InStates;
  /// The dataflow state after a basic block is processed.
  toolchain::DenseMap<const CFGBlock *, Lattice> OutStates;
  ...
  toolchain::DenseMap<ProgramPoint, Lattice> PerPointStates;
...
public:
  void run() {
    ...
    toolchain::TimeTraceScope Time(D.getAnalysisName());
    ...
    toolchain::SmallBitVector Visited(Cfg.getNumBlockIDs() + 1);
  ...
  void dump() const {
    ...
    toolchain::dbgs() << "==========================================\n";
    toolchain::dbgs() << D->getAnalysisName() << " results:\n";
    toolchain::dbgs() << "==========================================\n";
    const CFGBlock &B = isForward() ? Cfg.getExit() : Cfg.getEntry();
    getOutState(&B).dump(toolchain::dbgs());
  }
...
private:
  ...
  Lattice transferBlock(const CFGBlock *Block, Lattice State) {
    ...
    if constexpr (isForward()) {
      ...
      for (const Fact *F : toolchain::reverse(Facts)) {
  ...
  Lattice transferFact(Lattice In, const Fact *F) {
    ...
    toolchain_unreachable("Unknown fact kind");
  }
...
template <typename T>
static toolchain::ImmutableSet<T> join(toolchain::ImmutableSet<T> A,
                                  toolchain::ImmutableSet<T> B,
                                  typename toolchain::ImmutableSet<T>::Factory &F) {
...
static toolchain::ImmutableMap<K, V>
join(toolchain::ImmutableMap<K, V> A, toolchain::ImmutableMap<K, V> B,
     typename toolchain::ImmutableMap<K, V>::Factory &F, Joiner joinValues) {
  if (A.getHeight() < B.getHeight())
    std::swap(A, B);
...
using OriginLoanMap = toolchain::ImmutableMap<OriginID, LoanSet>;
...
void LifetimeSafetyAnalysis::run() {
  toolchain::TimeTraceScope TimeProfile("LifetimeSafetyAnalysis");
...
toolchain::StringMap<ProgramPoint> LifetimeSafetyAnalysis::getTestPoints() const {
  assert(FactMgr && "FactManager not initialized");
  toolchain::StringMap<ProgramPoint> AnnotationToPointMap;
