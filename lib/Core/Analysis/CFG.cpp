...
LocalScope::const_iterator::shared_parent(LocalScope::const_iterator L) {
  ...
  toolchain::SmallDenseMap<const LocalScope *, unsigned, 4> ScopesOfL;
  while (true) {
...
class reverse_children {
  toolchain::SmallVector<Stmt *, 12> childrenBuf;
...
reverse_children::reverse_children(Stmt *S, ASTContext &Ctx) {
  ...
  case Stmt::InitListExprClass: {
    ...
    children = toolchain::ArrayRef(reinterpret_cast<Stmt **>(IE->getInits()),
                              IE->getNumInits());
    return;
  }
  ...
  case Stmt::AttributedStmtClass: {
    ...
    toolchain::append_range(childrenBuf, AS->children());
  ...
  toolchain::append_range(childrenBuf, S->children());
...
class CFGBuilder {
  ...
  using LabelMapTy = toolchain::DenseMap<LabelDecl *, JumpTarget>;
  LabelMapTy LabelMap;
  ...
  using LabelSetTy = toolchain::SmallSetVector<LabelDecl *, 8>;
  ...
  toolchain::DenseMap<Expr *, const ConstructionContextLayer *>
  ...
  using CachedBoolEvalsTy = toolchain::DenseMap<Expr *, TryResult>;
...
private:
  ...
  TryResult checkIncorrectRelationalOperator(const BinaryOperator *B) {
    ...
    toolchain::APInt IntValue = IntLiteral->getValue();
  ...
  TryResult checkIncorrectEqualityOperator(const BinaryOperator *B) {
    ...
    std::optional<toolchain::APInt> IntLiteral1 =
    ...
    if (BitOp && (BitOp->getOpcode() == BO_And ||
      ...
      std::optional<toolchain::APInt> IntLiteral2 =
  ...
  getIntegerLiteralSubexpressionValue(const Expr *E) {
    ...
    if (const auto *UnOp = dyn_cast<UnaryOperator>(E->IgnoreParens())) {
      ...
      if (const auto *IntLiteral = dyn_cast<IntegerLiteral>(SubExpr)) {
        ...
        toolchain::APInt Value = IntLiteral->getValue();
        ...
        case UO_Not:
          return ~Value;
        case UO_LNot:
          return toolchain::APInt(Context->getTypeSize(Context->IntTy), !Value);
        default:
  ...
  TryResult checkIncorrectLogicOperator(const BinaryOperator *B) {
    ...
    if (L1Result.Val.getKind() == APValue::Int &&
      ...
      toolchain::APSInt L1 = L1Result.Val.getInt();
      toolchain::APSInt L2 = L2Result.Val.getInt();
      ...
      const toolchain::APSInt Values[] = {
          // Value less than both Value1 and Value2
          toolchain::APSInt::getMinValue(L1.getBitWidth(), L1.isUnsigned()),
          // L1
          L1,
          // Value between Value1 and Value2
          ((L1 < L2) ? L1 : L2) +
              toolchain::APSInt(toolchain::APInt(L1.getBitWidth(), 1), L1.isUnsigned()),
          // L2
          L2,
          // Value greater than both Value1 and Value2
          toolchain::APSInt::getMaxValue(L1.getBitWidth(), L1.isUnsigned()),
      };
    ...
    if (L1Result.Val.getKind() == APValue::Float &&
      ...
      toolchain::APFloat L1 = L1Result.Val.getFloat();
      toolchain::APFloat L2 = L2Result.Val.getFloat();
      ...
      if (Order > 0) {
        // type rank L1 > L2:
        if (toolchain::APFloat::opOK !=
            L2.convert(L1.getSemantics(), toolchain::APFloat::rmNearestTiesToEven,
                       &Ignored))
          return {};
      } else if (Order < 0)
        // type rank L1 < L2:
        if (toolchain::APFloat::opOK !=
            L1.convert(L2.getSemantics(), toolchain::APFloat::rmNearestTiesToEven,
                       &Ignored))
          return {};

      toolchain::APFloat MidValue = L1;
      MidValue.add(L2, toolchain::APFloat::rmNearestTiesToEven);
      MidValue.divide(toolchain::APFloat(MidValue.getSemantics(), "2.0"),
                      toolchain::APFloat::rmNearestTiesToEven);

      const toolchain::APFloat Values[] = {
          toolchain::APFloat::getSmallest(L1.getSemantics(), true), L1, MidValue, L2,
          toolchain::APFloat::getLargest(L2.getSemantics(), false),
      };
        ...
        switch (Bop->getOpcode()) {
          ...
          case BO_And: {
            ...
            if (Bop->getLHS()->EvaluateAsInt(LHSResult, *Context)) {
              toolchain::APSInt IntVal = LHSResult.Val.getInt();
            ...
            if (Bop->getRHS()->EvaluateAsInt(RHSResult, *Context)) {
              toolchain::APSInt IntVal = RHSResult.Val.getInt();
              if (!IntVal.getBoolValue()) {
                return TryResult(false);
              }
            }
...
Expr *
language::Core::extractElementInitializerFromNestedAILE(const ArrayInitLoopExpr *AILE) {
  if (!AILE)
    return nullptr;
...
std::unique_ptr<CFG> CFGBuilder::buildCFG(const Decl *D, Stmt *Statement) {
  ...
  if (const auto *CD = dyn_cast_or_null<CXXConstructorDecl>(D)) {
    ...
    for (auto *I : toolchain::reverse(CD->inits())) {
...
void CFGBuilder::addAutomaticObjHandling(LocalScope::const_iterator B,
  ...
  auto Pairwise =
      toolchain::zip(LocalScopeEndMarkers, toolchain::drop_begin(LocalScopeEndMarkers));
...
void CFGBuilder::addAutomaticObjDestruction(LocalScope::const_iterator B,
  ...
  for (VarDecl* D : toolchain::make_range(B, E))
    if (needsAutomaticDestruction(D))
      DeclsNeedDestruction.push_back(D);

  for (VarDecl *VD : toolchain::reverse(DeclsNeedDestruction)) {
    if (BuildOpts.AddImplicitDtors) {
...
void CFGBuilder::addScopeExitHandling(LocalScope::const_iterator B,
  ...
  autoCreateBlock();
  for (VarDecl *VD : toolchain::reverse(DeclsTrivial))
    appendLifetimeEnds(Block, VD, S);
}
...
LocalScope* CFGBuilder::createOrReuseLocalScope(LocalScope* Scope) {
  if (Scope)
    return Scope;
  toolchain::BumpPtrAllocator &alloc = cfg->getAllocator();
...
CFGBlock *CFGBuilder::VisitCompoundStmt(CompoundStmt *C,
  ...
  for (Stmt *S : toolchain::reverse(C->body())) {
...
CFGBlock *CFGBuilder::VisitDeclSubExpr(DeclStmt *DS) {
  ...
  if (const auto *DD = dyn_cast<DecompositionDecl>(VD)) {
    for (auto *BD : toolchain::reverse(DD->bindings())) {
...
static bool shouldAddCase(bool &switchExclusivelyCovered,
  ...
  if (!switchExclusivelyCovered) {
    if (switchCond->Val.isInt()) {
      ...
      const toolchain::APSInt &lhsInt = CS->getLHS()->EvaluateKnownConstInt(Ctx);
      const toolchain::APSInt &condInt = switchCond->Val.getInt();
      ...
      if (condInt == lhsInt) {
        ...
        if (const Expr *RHS = CS->getRHS()) {
          ...
          const toolchain::APSInt &V2 = RHS->EvaluateKnownConstInt(Ctx);
          if (V2 >= condInt) {
            addCase = true;
            switchExclusivelyCovered = true;
          }
...
CFGBlock *CFGBuilder::VisitOMPExecutableDirective(OMPExecutableDirective *D,
  ...
  for (Stmt *S : toolchain::reverse(Used)) {
...
bool CFG::isLinear() const {
  ...
  toolchain::SmallPtrSet<const CFGBlock *, 4> Visited;
...
CFGImplicitDtor::getDestructorDecl(ASTContext &astContext) const {
  switch (getKind()) {
    ...
    case CFGElement::CleanupFunction:
      toolchain_unreachable("getDestructorDecl should only be used with "
                       "ImplicitDtors");
  ...
  toolchain_unreachable("getKind() returned bogus value");
}
...
class StmtPrinterHelper : public PrinterHelper  {
  using StmtMapTy = toolchain::DenseMap<const Stmt *, std::pair<unsigned, unsigned>>;
  using DeclMapTy = toolchain::DenseMap<const Decl *, std::pair<unsigned, unsigned>>;
  ...
  void VisitBinaryOperator(BinaryOperator* B) {
    ...
    switch (B->getOpcode()) {
      ...
      case BO_LAnd:
        ...
        return;
      default:
        toolchain_unreachable("Invalid logical operator.");
    }
  }
...
void CFGElement::dumpToStream(toolchain::raw_ostream &OS,
...
static void print_block(raw_ostream &OS, const CFG* cfg,
  ...
  if (Stmt *Label = const_cast<Stmt*>(B.getLabel())) {
    ...
    if (LabelStmt *L = dyn_cast<LabelStmt>(Label))
      ...
      toolchain_unreachable("Invalid label statement in CFGBlock.");
...
void CFG::dump(const LangOptions &LO, bool ShowColors) const {
  print(toolchain::errs(), LO, ShowColors);
}
...
size_t CFGBlock::getIndexInCFG() const {
  return toolchain::find(*getParent(), this) - getParent()->begin();
}

/// dump - A simply pretty printer of a CFGBlock that outputs to stderr.
void CFGBlock::dump(const CFG* cfg, const LangOptions &LO,
                    bool ShowColors) const {
  print(toolchain::errs(), cfg, LO, ShowColors);
}
...
static bool isImmediateSinkBlock(const CFGBlock *Blk) {
  ...
  // immediately caught.
  if (toolchain::any_of(*Blk, [](const CFGElement &Elm) {
...
bool CFGBlock::isInevitablySinking() const {
  ...
  toolchain::SmallVector<const CFGBlock *, 32> DFSWorkList;
  toolchain::SmallPtrSet<const CFGBlock *, 32> Visited;
