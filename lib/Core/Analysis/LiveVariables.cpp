...
public:
  ...
  toolchain::ImmutableSet<const Expr *>::Factory ESetFact;
  toolchain::ImmutableSet<const VarDecl *>::Factory DSetFact;
  toolchain::ImmutableSet<const BindingDecl *>::Factory BSetFact;
  toolchain::DenseMap<const CFGBlock *, LiveVariables::LivenessValues> blocksEndToLiveness;
  toolchain::DenseMap<const CFGBlock *, LiveVariables::LivenessValues> blocksBeginToLiveness;
  toolchain::DenseMap<const Stmt *, LiveVariables::LivenessValues> stmtsToLiveness;
  toolchain::DenseMap<const DeclRefExpr *, unsigned> inAssignment;
...
LiveVariablesImpl::merge(LiveVariables::LivenessValues valsA,
  ...
  toolchain::ImmutableSetRef<const Expr *> SSetRefA(
  ...
  toolchain::ImmutableSetRef<const VarDecl *>
    DSetRefA(valsA.liveDecls.getRootWithoutRetain(), DSetFact.getTreeFactory()),
    DSetRefB(valsB.liveDecls.getRootWithoutRetain(), DSetFact.getTreeFactory());

  toolchain::ImmutableSetRef<const BindingDecl *>
...
static void AddLiveExpr(toolchain::ImmutableSet<const Expr *> &Set,
                        toolchain::ImmutableSet<const Expr *>::Factory &F,
                        const Expr *E) {
...
/// "a < b", "c", "d", "((e || f) != (g && h))"
static void AddAllConditionalTerms(toolchain::ImmutableSet<const Expr *> &Set,
                                   toolchain::ImmutableSet<const Expr *>::Factory &F,
                                   const Expr *Cond) {
...
LiveVariables::computeLiveness(AnalysisDeclContext &AC, bool killAtAssign) {
  ...
  toolchain::BitVector everAnalyzedBlock(cfg->getNumBlockIDs());
...
void LiveVariablesImpl::dumpBlockLiveness(const SourceManager &M) {
  ...
  toolchain::sort(vec, [](const CFGBlock *A, const CFGBlock *B) {
  ...
  for (std::vector<const CFGBlock *>::iterator
    ...
    for (toolchain::ImmutableSet<const VarDecl *>::iterator si =
          vals.liveDecls.begin(),
          se = vals.liveDecls.end(); si != se; ++si) {
      declVec.push_back(*si);
    }

    toolchain::sort(declVec, [](const Decl *A, const Decl *B) {
      return A->getBeginLoc() < B->getBeginLoc();
    });

    for (std::vector<const VarDecl*>::iterator di = declVec.begin(),
         de = declVec.end(); di != de; ++di) {
      toolchain::errs() << " " << (*di)->getDeclName().getAsString()
                   << " <";
      (*di)->getLocation().print(toolchain::errs(), M);
      toolchain::errs() << ">\n";
    }
  }
  toolchain::errs() << "\n";
...
void LiveVariablesImpl::dumpExprLiveness(const SourceManager &M) {
  ...
  for (const CFGBlock *B : *analysisContext.getCFG()) {
    ...
    toolchain::append_range(LiveExprs, blocksEndToLiveness[B].liveExprs);
    toolchain::sort(LiveExprs, ByIDs);
