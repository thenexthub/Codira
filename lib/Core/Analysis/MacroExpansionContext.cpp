...
static void dumpTokenInto(const language::Core::Preprocessor &PP, toolchain::raw_ostream &OS,
                          language::Core::Token Tok);
...
public:
  ...
  void MacroExpands(const Token &MacroName, const MacroDefinition &MD,
    ...
    (void)PP;
    LLVM_DEBUG(toolchain::dbgs() << "MacroExpands event: '";
               dumpTokenInto(PP, toolchain::dbgs(), MacroName);
               toolchain::dbgs()
               << "' with length " << MacroName.getLength() << " at ";
               MacroNameBegin.print(toolchain::dbgs(), SM);
               toolchain::dbgs() << ", expansion end at ";
               ExpansionEnd.print(toolchain::dbgs(), SM); toolchain::dbgs() << '\n';);
    ...
    if (Inserted) {
      LLVM_DEBUG(toolchain::dbgs() << "maps ";
                 It->getFirst().print(toolchain::dbgs(), SM); toolchain::dbgs() << " to ";
                 It->getSecond().print(toolchain::dbgs(), SM);
                 toolchain::dbgs() << '\n';);
    } else {
      if (SM.isBeforeInTranslationUnit(It->getSecond(), ExpansionEnd)) {
        It->getSecond() = ExpansionEnd;
        LLVM_DEBUG(
            toolchain::dbgs() << "remaps "; It->getFirst().print(toolchain::dbgs(), SM);
            toolchain::dbgs() << " to "; It->getSecond().print(toolchain::dbgs(), SM);
            toolchain::dbgs() << '\n';);
      }
    }
  }
...
void MacroExpansionContext::dumpExpansionRanges() const {
  dumpExpansionRangesToStream(toolchain::dbgs());
}
void MacroExpansionContext::dumpExpandedTexts() const {
  dumpExpandedTextsToStream(toolchain::dbgs());
}
...
void MacroExpansionContext::dumpExpansionRangesToStream(raw_ostream &OS) const {
  ...
  toolchain::sort(LocalExpansionRanges);
...
void MacroExpansionContext::dumpExpandedTextsToStream(raw_ostream &OS) const {
  ...
  for (const auto &Record : ExpandedTokens)
    LocalExpandedTokens.emplace_back(
        std::make_pair(Record.getFirst(), Record.getSecond()));
  toolchain::sort(LocalExpandedTokens);
...
void MacroExpansionContext::onTokenLexed(const Token &Tok) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "lexed macro expansion token '";
             dumpTokenInto(*PP, toolchain::dbgs(), Tok); toolchain::dbgs() << "' at ";
             SLoc.print(toolchain::dbgs(), *SM); toolchain::dbgs() << '\n';);

  // Remove spelling location.
  SourceLocation CurrExpansionLoc = SM->getExpansionLoc(SLoc);

  MacroExpansionText TokenAsString;
  toolchain::raw_svector_ostream OS(TokenAsString);
