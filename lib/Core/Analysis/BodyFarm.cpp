...
IntegerLiteral *ASTMaker::makeIntegerLiteral(uint64_t Value, QualType Ty) {
  toolchain::APInt APValue = toolchain::APInt(C.getTypeSize(Ty), Value);
...
static CallExpr *create_call_once_funcptr_call(ASTContext &C, ASTMaker M,
  ...
  if (Ty->isRValueReferenceType()) {
    ...
    toolchain_unreachable("Unexpected state");
  }
...
static Stmt *create_std_move_forward(ASTContext &C, const FunctionDecl *D) {
  LLVM_DEBUG(toolchain::dbgs() << "Generating body for std::move / std::forward\n");
...
static Stmt *create_call_once(ASTContext &C, const FunctionDecl *D) {
  LLVM_DEBUG(toolchain::dbgs() << "Generating body for call_once\n");
  ...
  if (!Callback->getType()->isReferenceType()) {
    toolchain::dbgs() << "libcxx03 std::call_once implementation, skipping.\n";
    return nullptr;
  }
  if (!Flag->getType()->isReferenceType()) {
    toolchain::dbgs() << "unknown std::call_once implementation, skipping.\n";
    return nullptr;
  }
  ...
  if (!FlagRecordDecl) {
    LLVM_DEBUG(toolchain::dbgs() << "Flag field is not a record: "
  ...
  if (!FlagFieldDecl) {
    LLVM_DEBUG(toolchain::dbgs() << "No field _M_once or __state_ found on "
  ...
  if (D->getNumParams() != CallbackFunctionType->getNumParams() + 2) {
    LLVM_DEBUG(toolchain::dbgs() << "Types of params of the callback do not match "
...
Stmt *BodyFarm::getBody(const FunctionDecl *D) {
  ...
  if (unsigned BuiltinID = D->getBuiltinID()) {
    ...
    FF = toolchain::StringSwitch<FunctionFarmer>(Name)
