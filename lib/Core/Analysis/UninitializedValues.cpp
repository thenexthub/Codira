...
#include "toolchain/ADT/PackedVector.h"
...
class DeclToIndex {
  toolchain::DenseMap<const VarDecl *, unsigned> map;
...
std::optional<unsigned> DeclToIndex::getValueIndex(const VarDecl *d) const {
  toolchain::DenseMap<const VarDecl *, unsigned>::const_iterator I = map.find(d);
  if (I == map.end())
    return std::nullopt;
  return I->second;
}
...
using ValueVector = toolchain::PackedVector<Value, 2, toolchain::SmallBitVector>;
...
static void printVector(const CFGBlock *block, ValueVector &bv,
                        unsigned num) {
  toolchain::errs() << block->getBlockID() << " :";
  for (const auto &i : bv)
    toolchain::errs() << ' ' << i;
  toolchain::errs() << " : " << num << '\n';
}
...
private:
  ...
  toolchain::DenseMap<const DeclRefExpr *, Class> Classification;
...
public:
  ...
  Class get(const DeclRefExpr *DRE) const {
    toolchain::DenseMap<const DeclRefExpr*, Class>::const_iterator I
...
static bool runOnBlock(const CFGBlock *block, const CFG &cfg,
                       ...
                       const ClassifyRefs &classification,
                       toolchain::BitVector &wasAnalyzed,
                       UninitVariablesHandler &handler) {
...
void language::Core::runUninitializedVariablesAnalysis(
    const DeclContext &dc,
    const CFG &cfg,
    AnalysisDeclContext &ac,
    UninitVariablesHandler &handler,
  ...
  toolchain::BitVector previouslyVisited(cfg.getNumBlockIDs());
  worklist.enqueueSuccessors(&cfg.getEntry());
  toolchain::BitVector wasAnalyzed(cfg.getNumBlockIDs(), false);
