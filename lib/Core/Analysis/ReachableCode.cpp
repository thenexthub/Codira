...
static bool isDeadReturn(const CFGBlock *B, const Stmt *S) {
  ...
  while (true) {
    for (const CFGElement &CE : toolchain::reverse(*Current)) {
  ...
  toolchain_unreachable("Broke out of infinite loop.");
}
...
namespace {
  ...
  public:
    DeadCodeScan(toolchain::BitVector &reachable, Preprocessor &PP, ASTContext &C)
    ...
    language::Core::reachable_code::Callback &CB);
...
bool DeadCodeScan::isDeadCodeRoot(const language::Core::CFGBlock *Block) {
  ...
  for (const CFGElement &Elem : *Block)
    if (std::optional<CFGStmt> CS = Elem.getAs<CFGStmt>()) {
      ...
      if (AfterDeadStmt &&
          ...
          (toolchain::isa<CoreturnStmt>(S) || toolchain::isa<CoroutineSuspendExpr>(S))) {
        CoroStmt = S;
        break;
      }
    }
...
static bool isValidDeadStmt(const Stmt *S, const language::Core::CFGBlock *Block) {
...
const Stmt *DeadCodeScan::findDeadCode(const language::Core::CFGBlock *Block) {
...
unsigned DeadCodeScan::scanBackwards(const language::Core::CFGBlock *Start,
                                     language::Core::reachable_code::Callback &CB) {
  ...
  if (!DeferredLocs.empty()) {
    toolchain::array_pod_sort(DeferredLocs.begin(), DeferredLocs.end(), SrcCmp);
...
namespace language::Core { namespace reachable_code {
...
void FindUnreachableCode(AnalysisDeclContext &AC, Preprocessor &PP,
  ...
  toolchain::BitVector reachable(cfg->getNumBlockIDs());
