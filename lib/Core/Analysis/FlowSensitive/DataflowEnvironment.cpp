...
static toolchain::DenseMap<const ValueDecl *, StorageLocation *> intersectDeclToLoc(
    const toolchain::DenseMap<const ValueDecl *, StorageLocation *> &DeclToLoc1,
    const toolchain::DenseMap<const ValueDecl *, StorageLocation *> &DeclToLoc2) {
  toolchain::DenseMap<const ValueDecl *, StorageLocation *> Result;
...
static bool compareDistinctValues(QualType Type, Value &Val1,
  ...
  toolchain_unreachable("All cases covered in switch");
}
...
static bool compareKeyToValueMaps(const toolchain::MapVector<Key, Value *> &Map1,
                                  const toolchain::MapVector<Key, Value *> &Map2,
...
static toolchain::MapVector<const StorageLocation *, Value *>
joinLocToVal(const toolchain::MapVector<const StorageLocation *, Value *> &LocToVal,
             const toolchain::MapVector<const StorageLocation *, Value *> &LocToVal2,
             const Environment &Env1, const Environment &Env2,
             Environment &JoinedEnv, Environment::ValueModel &Model) {
  toolchain::MapVector<const StorageLocation *, Value *> Result;
...
template <typename Key>
static toolchain::MapVector<Key, Value *>
widenKeyToValueMap(const toolchain::MapVector<Key, Value *> &CurMap,
                   const toolchain::MapVector<Key, Value *> &PrevMap,
                   Environment &CurEnv, const Environment &PrevEnv,
                   Environment::ValueModel &Model, LatticeEffect &Effect) {
  toolchain::MapVector<Key, Value *> WidenedMap;
...
public:
  ...
  explicit ResultObjectVisitor(
      toolchain::DenseMap<const Expr *, RecordStorageLocation *> &ResultObjectMap,
...
bool Environment::canDescend(unsigned MaxDepth,
  ...
  return CallStack.size() < MaxDepth && !toolchain::is_contained(CallStack, Callee);
}
...
Environment Environment::pushCall(const CallExpr *Call) const {
  ...
  Env.pushCallInternal(Call->getDirectCallee(),
                       toolchain::ArrayRef(Call->getArgs(), Call->getNumArgs()));
...
void Environment::initializeFieldsWithValues(RecordStorageLocation &Loc,
  ...
  if (CreatedValuesCount > MaxCompositeValueSize) {
    toolchain::errs() << "Attempting to initialize a huge value of type: " << Type
                 << '\n';
  }
}
...
Value *Environment::createValueUnlessSelfReferential(
    QualType Type, toolchain::DenseSet<QualType> &Visited, int Depth,
...
Environment::createLocAndMaybeValue(QualType Ty,
  ...
  auto EraseVisited = toolchain::make_scope_exit(
...
void Environment::dump(raw_ostream &OS) const {
  toolchain::DenseMap<const StorageLocation *, std::string> LocToName;
...
void Environment::dump() const { dump(toolchain::dbgs()); }
