...
#include "language/Core/Analysis/FlowSensitive/Models/UncheckedOptionalAccessModel.h"
...
auto isOptionalOperatorCallWithName(
    toolchain::StringRef operator_name,
    const std::optional<StatementMatcher> &Ignorable = std::nullopt) {
...
constexpr toolchain::StringLiteral ValueOrCallID = "ValueOrCall";
...
void transferValueOrImpl(
    const language::Core::Expr *ValueOrPredExpr, const MatchFinder::MatchResult &Result,
    LatticeTransferState &State,
  ...
  const auto *MCE =
      Result.Nodes.getNodeAs<language::Core::CXXMemberCallExpr>(ValueOrCallID);
...
void transferValueOrStringEmptyCall(const language::Core::Expr *ComparisonExpr,
...
void transferOptionalAndOptionalCmp(const language::Core::CXXOperatorCallExpr *CmpExpr,
                                    const MatchFinder::MatchResult &,
                                    LatticeTransferState &State) {
  ...
  if (auto *LHasVal = getHasValue(Env, Arg0Loc)) {
    ...
    if (auto *RHasVal = getHasValue(Env, Arg1Loc)) {
      if (CmpExpr->getOperator() == language::Core::OO_ExclaimEqual)
...
void transferOptionalAndValueCmp(const language::Core::CXXOperatorCallExpr *CmpExpr,
                                 const language::Core::Expr *E, Environment &Env) {
  auto &A = Env.arena();
  auto *CmpValue = &forceBoolValue(Env, *CmpExpr);
...
void transferOptionalAndNulloptCmp(const language::Core::CXXOperatorCallExpr *CmpExpr,
                                   const language::Core::Expr *E, Environment &Env) {
  auto &A = Env.arena();
  auto *CmpValue = &forceBoolValue(Env, *CmpExpr);
  auto *Loc = Env.get<RecordStorageLocation>(*E);
...
auto buildTransferMatchSwitch() {
  ...
  return CFGMatchSwitchBuilder<LatticeTransferState>()
      ...
      .CaseOfCFGStmt<CXXOperatorCallExpr>(
          ...
          [](const language::Core::CXXOperatorCallExpr *Cmp,
...
UncheckedOptionalAccessModel::UncheckedOptionalAccessModel(ASTContext &Ctx,
  ...
  Env.getDataflowAnalysisContext().setSyntheticFieldCallback(
      [&Ctx](QualType Ty) -> toolchain::StringMap<QualType> {
        const CXXRecordDecl *Optional =
            getOptionalBaseClass(Ty->getAsCXXRecordDecl());
        if (Optional == nullptr)
          return {};
