...
#include "language/Core/Analysis/FlowSensitive/Transfer.h"
...
template struct CLANG_EXPORT_TEMPLATE Any::TypeId<language::Core::dataflow::NoopLattice>;
} // namespace toolchain
...
static int blockIndexInPredecessor(const CFGBlock &Pred,
  ...
  auto BlockPos = toolchain::find_if(
...
struct AnalysisContext {
  ...
  toolchain::ArrayRef<std::optional<TypeErasedDataflowAnalysisState>> BlockStates;
};

class PrettyStackTraceAnalysis : public toolchain::PrettyStackTraceEntry {
public:
...
class PrettyStackTraceCFGElement : public toolchain::PrettyStackTraceEntry {
public:
...
computeBlockInputState(const CFGBlock &Block, AnalysisContext &AC) {
  ...
  if (Block.getTerminator().isTemporaryDtorsBranch()) {
    ...
    if (Block.succ_begin()->getReachableBlock() != nullptr &&
      ...
      toolchain::erase(Preds, StmtBlock);
    }
  }
...
runTypeErasedDataflowAnalysis(
  ...
  const language::Core::CFG &CFG = ACFG.getCFG();
  ...
  while (const CFGBlock *Block = Worklist.dequeue()) {
    ...
    if (++BlockVisits > MaxBlockVisits) {
      return toolchain::createStringError(std::errc::timed_out,
    ...
    LLVM_DEBUG({
      toolchain::errs() << "New Env:\n";
      NewBlockState.Env.dump();
    });

    if (OldBlockState) {
      LLVM_DEBUG({
        toolchain::errs() << "Old Env:\n";
