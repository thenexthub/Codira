...
toolchain::Expected<std::string> renderSVG(toolchain::StringRef DotGraph);

using StreamFactory = std::function<std::unique_ptr<toolchain::raw_ostream>()>;

// Recursively dumps Values/StorageLocations as JSON
class ModelDumper {
public:
  ModelDumper(toolchain::json::OStream &JOS, const Environment &Env)
      : JOS(JOS), Env(Env) {}

  void dump(Value &V) {
    JOS.attribute("value_id", toolchain::to_string(&V));
    ...
    if (auto *B = toolchain::dyn_cast<BoolValue>(&V)) {
      JOS.attribute("formula", toolchain::to_string(B->formula()));
  ...
  void dump(const StorageLocation &L) {
    JOS.attribute("location", toolchain::to_string(&L));
  ...
  toolchain::DenseSet<const void*> Visited;
  toolchain::json::OStream &JOS;
...
class HTMLLogger : public Logger {
  ...
  std::string JSON;
  toolchain::raw_string_ostream JStringStream{JSON};
  toolchain::json::OStream JOS{JStringStream, /*Indent=*/2};
  ...
  toolchain::DenseMap<const CFGBlock *, toolchain::SmallVector<size_t>> BlockIters;
  // For a given block ID, did the block converge (on the last iteration)?
  toolchain::BitVector BlockConverged;
...
public:
  ...
  void beginAnalysis(const AdornedCFG &ACFG,
    ...
    OS = Streams();
    this->ACFG = &ACFG;
    *OS << toolchain::StringRef(HTMLLogger_html).split("<?INJECT?>").first;
  ...
  void endAnalysis() override {
    ...
    *OS << JSON;
    *OS << ";\n</script>\n";
    *OS << toolchain::StringRef(HTMLLogger_html).split("<?INJECT?>").second;
  }

  void enterBlock(const CFGBlock &B, bool PostVisit) override {
    toolchain::SmallVector<size_t> &BIter = BlockIters[&B];
  ...
  static std::string blockID(unsigned Block) {
    return toolchain::formatv("B{0}", Block);
  }
  static std::string eltID(unsigned Block, unsigned Element) {
    return toolchain::formatv("B{0}.{1}", Block, Element);
  }
  static std::string iterID(unsigned Block, unsigned Iter) {
    return toolchain::formatv("B{0}:{1}", Block, Iter);
  }
  static std::string elementIterID(unsigned Block, unsigned Iter,
                                   unsigned Element) {
    return toolchain::formatv("B{0}:{1}_B{0}.{2}", Block, Iter, Element);
  }
  ...
  void recordState(TypeErasedDataflowAnalysisState &State) override {
    ...
    JOS.attributeObject(elementIterID(Block, Iter, ElementIndex), [&] {
      ...
      if (ElementIndex > 0) {
        ...
        if (const Expr *E = S ? toolchain::dyn_cast<Expr>(S->getStmt()) : nullptr) {
          if (E->isPRValue()) {
      ...
      }
      {
        ...
        toolchain::raw_string_ostream BuiltinLatticeS(BuiltinLattice);
  ...
  void logText(toolchain::StringRef S) override {
...
private:
  ...
  void writeBlock(const CFGBlock &B, toolchain::ArrayRef<size_t> ItersForB) {
    JOS.attributeObject(blockID(B.getBlockID()), [&] {
      ...
      JOS.attributeArray("elements", [&] {
        for (const auto &Elt : B.Elements) {
          std::string Dump;
          toolchain::raw_string_ostream DumpS(Dump);
  ...
  void writeCode() {
    ...
    auto Range = language::Core::Lexer::makeFileCharRange(
        CharSourceRange::getTokenRange(ACFG->getDecl().getSourceRange()),
        AST.getSourceManager(), AST.getLangOpts());
    if (Range.isInvalid())
      return;
    toolchain::StringRef Code = language::Core::Lexer::getSourceText(
    ...
    struct TokenInfo {
      ...
      void write(toolchain::raw_ostream &OS) const {
        OS << "class='c";
        if (BB != Missing)
          OS << " " << blockID(BB);
    ...
    for (const auto *Block : ACFG->getCFG()) {
      ...
      for (const auto& Elt : *Block) {
        ...
        if (const auto S = Elt.getAs<CFGStmt>()) {
          auto EltRange = language::Core::Lexer::makeFileCharRange(
    ...
    *OS << "<code class='filename'>";
    toolchain::printHTMLEscaped(
        toolchain::sys::path::filename(
    ...
    for (unsigned I = 0; I < Code.size(); ++I) {
      ...
      if (Code[I] == '\n')
        *OS << "</code>\n<code class='line' data-line='" << Line++ << "'>";
        ...
        toolchain::printHTMLEscaped(Code.substr(I, 1), *OS);
  ...
  std::string buildCFGDot(const language::Core::CFG &CFG) {
    std::string Graph;
    toolchain::raw_string_ostream GraphS(Graph);
...
toolchain::Expected<std::string> renderSVG(toolchain::StringRef DotGraph) {
  ...
  if (const auto *FromEnv = ::getenv("GRAPHVIZ_DOT"))
    ...
    auto FromPath = toolchain::sys::findProgramByName("dot");
    if (!FromPath)
      return toolchain::createStringError(FromPath.getError(),
  ...
  toolchain::SmallString<256> Input, Output;
  int InputFD;
  if (auto EC = toolchain::sys::fs::createTemporaryFile("analysis", ".dot", InputFD,
                                                   Input))
    return toolchain::createStringError(EC, "failed to create `dot` temp input");
  toolchain::raw_fd_ostream(InputFD, /*shouldClose=*/true) << DotGraph;
  auto DeleteInput =
      toolchain::make_scope_exit([&] { toolchain::sys::fs::remove(Input); });
  if (auto EC = toolchain::sys::fs::createTemporaryFile("analysis", ".svg", Output))
    return toolchain::createStringError(EC, "failed to create `dot` temp output");
  auto DeleteOutput =
      toolchain::make_scope_exit([&] { toolchain::sys::fs::remove(Output); });

  std::vector<std::optional<toolchain::StringRef>> Redirects = {
      Input, Output,
      /*stderr=*/std::nullopt};
  std::string ErrMsg;
  int Code = toolchain::sys::ExecuteAndWait(
  ...
  if (Code != 0)
    return toolchain::createStringError(toolchain::inconvertibleErrorCode(),
                                   "'dot' failed (" + toolchain::Twine(Code) + ")");

  auto Buf = toolchain::MemoryBuffer::getFile(Output);
  if (!Buf)
    return toolchain::createStringError(Buf.getError(), "Can't read `dot` output");

  // Output has <?xml> prefix we don't want. Skip to <svg> tag.
  toolchain::StringRef Result = Buf.get()->getBuffer();
...
Logger::html(std::function<std::unique_ptr<toolchain::raw_ostream>()> Streams) {
