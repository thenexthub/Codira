...
static toolchain::cl::opt<std::string> DataflowLog(
    "dataflow-log", toolchain::cl::Hidden, toolchain::cl::ValueOptional,
    toolchain::cl::desc("Emit log of dataflow analysis. With no arg, writes textual "
...
FieldSet DataflowAnalysisContext::getModeledFields(QualType Type) {
  ...
  return toolchain::set_intersection(getObjectFields(Type), ModeledFields);
}
...
StorageLocation &DataflowAnalysisContext::createStorageLocation(QualType Type) {
  if (!Type.isNull() && Type->isRecordType()) {
    toolchain::DenseMap<const ValueDecl *, StorageLocation *> FieldLocs;
...
template <typename T>
static toolchain::DenseSet<toolchain::StringRef> getKeys(const toolchain::StringMap<T> &Map) {
  return toolchain::DenseSet<toolchain::StringRef>(toolchain::from_range, Map.keys());
}
...
Solver::Result DataflowAnalysisContext::querySolver(
    toolchain::SetVector<const Formula *> Constraints) {
...
void DataflowAnalysisContext::addTransitiveFlowConditionConstraints(
    Atom Token, toolchain::SetVector<const Formula *> &Constraints) {
  toolchain::DenseSet<Atom> AddedTokens;
...
static void printAtomList(const toolchain::SmallVector<Atom> &Atoms,
...
void DataflowAnalysisContext::dumpFlowCondition(Atom Token,
  ...
  toolchain::SetVector<const Formula *> OriginalConstraints = Constraints;
  ...
  if (!Info.EquivalentAtoms.empty()) {
    OS << "Equivalent atoms:\n";
    for (const toolchain::SmallVector<Atom> &Class : Info.EquivalentAtoms)
...
static std::unique_ptr<Logger> makeLoggerFromCommandLine() {
  if (DataflowLog.empty())
    return Logger::textual(toolchain::errs());

  toolchain::StringRef Dir = DataflowLog;
  if (auto EC = toolchain::sys::fs::create_directories(Dir))
    toolchain::errs() << "Failed to create log dir: " << EC.message() << "\n";
  ...
  auto StreamFactory =
      [Dir(Dir.str())]() mutable -> std::unique_ptr<toolchain::raw_ostream> {
    toolchain::SmallString<256> File(Dir);
    toolchain::sys::path::append(File,
                            std::to_string(Counter.fetch_add(1)) + ".html");
    std::error_code EC;
    auto OS = std::make_unique<toolchain::raw_fd_ostream>(File, EC);
    if (EC) {
      toolchain::errs() << "Failed to create log " << File << ": " << EC.message()
                   << "\n";
