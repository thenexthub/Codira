...
struct CNFFormulaBuilder {
  ...
  void addClause(ArrayRef<Literal> Literals) {
    ...
    toolchain::SmallVector<Literal> Simplified;
    for (auto L : Literals) {
      assert(L != NullLit && !toolchain::is_contained(Simplified, L));
      auto X = var(L);
...
private:
  CNFFormula &Formula;
  toolchain::DenseSet<Variable> trueVars;
  toolchain::DenseSet<Variable> falseVars;
};
...
void CNFFormula::addClause(ArrayRef<Literal> lits) {
  assert(!toolchain::is_contained(lits, NullLit));
  ...
  toolchain::append_range(Clauses, lits);
}

CNFFormula buildCNF(const toolchain::ArrayRef<const Formula *> &Formulas,
  ...
  toolchain::DenseMap<const Formula *, Variable> FormulaToVar;
