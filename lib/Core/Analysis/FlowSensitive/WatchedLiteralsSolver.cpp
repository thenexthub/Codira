...
#include "language/Core/Analysis/FlowSensitive/CNFFormula.h"
...
class WatchedLiteralsSolverImpl {
  ...
  /// formula.
  toolchain::DenseMap<Variable, Atom> Atomics;
...
public:
  explicit WatchedLiteralsSolverImpl(
      const toolchain::ArrayRef<const Formula *> &Vals)
...
private:
  ...
  toolchain::DenseMap<Atom, Solver::Result::Assignment> buildSolution() {
    toolchain::DenseMap<Atom, Solver::Result::Assignment> Solution;
  ...
  bool watchedByUnitClause(Literal Lit) const {
    for (ClauseID LitWatcher = WatchedHead[Lit]; LitWatcher != NullClause;
      ...
      toolchain::ArrayRef<Literal> Clause = CNF.clauseLiterals(LitWatcher);
  ...
  bool isUnit(toolchain::ArrayRef<Literal> Clause) const {
    return toolchain::all_of(Clause.drop_front(),
  ...
  toolchain::DenseSet<Literal> watchedLiterals() const {
    toolchain::DenseSet<Literal> WatchedLiterals;
  ...
  bool activeVarsAreUnassigned() const {
    return toolchain::all_of(ActiveVars, [this](Variable Var) {
  ...
  bool activeVarsFormWatchedLiterals() const {
    const toolchain::DenseSet<Literal> WatchedLiterals = watchedLiterals();
    return toolchain::all_of(ActiveVars, [&WatchedLiterals](Variable Var) {
  ...
  bool unassignedVarsFormingWatchedLiteralsAreActive() const {
    const toolchain::DenseSet<Variable> ActiveVarsSet(ActiveVars.begin(),
                                                 ActiveVars.end());
...
Solver::Result
WatchedLiteralsSolver::solve(toolchain::ArrayRef<const Formula *> Vals) {
  if (Vals.empty())
