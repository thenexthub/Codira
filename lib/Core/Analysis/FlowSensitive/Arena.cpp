...
static const Formula &cached(toolchain::DenseMap<Key, const Formula *> &Cache, Key K,
                             ComputeFunc &&Compute) {
...
IntegerValue &Arena::makeIntLiteral(toolchain::APInt Value) {
...
namespace {
const Formula *parse(Arena &A, toolchain::StringRef &In) {
...
class FormulaParseError : public toolchain::ErrorInfo<FormulaParseError> {
  std::string Formula;
  unsigned Offset;

public:
  static char ID;
  FormulaParseError(toolchain::StringRef Formula, unsigned Offset)
...
toolchain::Expected<const Formula &> Arena::parseFormula(toolchain::StringRef In) {
  toolchain::StringRef Rest = In;
  auto *Result = parse(*this, Rest);
  if (!Result) // parse() hit something unparseable
    return toolchain::make_error<FormulaParseError>(In, In.size() - Rest.size());
  Rest = Rest.ltrim();
