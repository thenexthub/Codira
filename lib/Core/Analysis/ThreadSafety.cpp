...
//
// See http://clang.toolchain.org/docs/ThreadSafetyAnalysis.html
...
#include "language/Core/Analysis/Analyses/ThreadSafetyCommon.h"
#include "language/Core/Analysis/Analyses/ThreadSafetyTIL.h"
#include "language/Core/Analysis/Analyses/ThreadSafetyUtil.h"
...
public:
  ...
  void push_back_nodup(const CapabilityExpr &CapE) {
    if (toolchain::none_of(*this, [=](const CapabilityExpr &CapE2) {
...
public:
  FactManager(toolchain::BumpPtrAllocator &Alloc) : Alloc(Alloc) {}
...
public:
  ...
  const FactEntry *findPartialMatch(FactManager &FM,
                                    const CapabilityExpr &CapE) const {
    auto I = toolchain::find_if(*this, [&](FactID ID) -> bool {
...
private:
  ...
  using BeforeMap =
      toolchain::DenseMap<const ValueDecl *, std::unique_ptr<BeforeInfo>>;
  using CycleMap = toolchain::DenseMap<const ValueDecl *, bool>;
...
using LocalVarContext = toolchain::ImmutableMap<const NamedDecl *, unsigned>;
...
public:
  ...
  void dumpVarDefinitionName(unsigned i) {
    ...
    Dec->printName(toolchain::errs());
    toolchain::errs() << "." << i << " " << ((const void*) Dec);
  }

  /// Dumps an ASCII representation of the variable map to toolchain::errs()
  void dump() {
  ...
  /// Dumps an ASCII representation of a Context to toolchain::errs()
  void dumpContext(Context C) {
    for (Context::iterator I = C.begin(), E = C.end(); I != E; ++I) {
      const NamedDecl *D = I.getKey();
      D->printName(toolchain::errs());
      toolchain::errs() << " -> ";
...
public:
  static LockableFactEntry *create(toolchain::BumpPtrAllocator &Alloc,
...
class ScopedLockableFactEntry final
    : public FactEntry,
      private toolchain::TrailingObjects<ScopedLockableFactEntry,
...
public:
  static ScopedLockableFactEntry *create(toolchain::BumpPtrAllocator &Alloc,
...
class ThreadSafetyAnalyzer {
  ...
  toolchain::BumpPtrAllocator Bpa;
  ...
  toolchain::SmallDenseMap<const Expr *, til::LiteralPtr *> ConstructedObjects;
  FactManager FactMan;
  std::vector<CFGBlockInfo> BlockInfo;
...
void BuildLockset::handleCall(const Expr *Exp, const NamedDecl *D,
  ...
  if (Exp) {
    if (const auto *CE = dyn_cast<CallExpr>(Exp))
      ...
      toolchain_unreachable("Unknown call kind");
  }
...
void ThreadSafetyAnalyzer::runAnalysis(AnalysisDeclContext &AC) {
  ...
  if (!SortedGraph->empty()) {
    ...
    if (CurrentFunction)
      ...
      toolchain_unreachable("Unknown function kind");
...
LockKind threadSafety::getLockKindFromAccessKind(AccessKind AK) {
  ...
  toolchain_unreachable("Unknown AccessKind");
}
