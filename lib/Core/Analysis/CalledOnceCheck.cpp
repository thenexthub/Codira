...
#include "language/Core/Analysis/Analyses/CalledOnceCheck.h"
...
template <class T>
using ParamSizedVector = toolchain::SmallVector<T, EXPECTED_MAX_NUMBER_OF_PARAMS>;
static constexpr unsigned EXPECTED_NUMBER_OF_BASIC_BLOCKS = 8;
template <class T>
using CFGSizedVector = toolchain::SmallVector<T, EXPECTED_NUMBER_OF_BASIC_BLOCKS>;
constexpr toolchain::StringLiteral CONVENTIONAL_NAMES[] = {
...
constexpr toolchain::StringLiteral CONVENTIONAL_SUFFIXES[] = {
    "WithCompletionHandler", "WithCompletion", "WithCompletionBlock",
    "WithReplyTo", "WithReply"};
constexpr toolchain::StringLiteral CONVENTIONAL_CONDITIONS[] = {
    "error", "cancel", "shouldCall", "done", "OK", "success"};

struct KnownCalledOnceParameter {
  toolchain::StringLiteral FunctionName;
  unsigned ParamIndex;
};
constexpr KnownCalledOnceParameter KNOWN_CALLED_ONCE_PARAMETERS[] = {
    {toolchain::StringLiteral{"dispatch_async"}, 1},
    {toolchain::StringLiteral{"dispatch_async_and_wait"}, 1},
    {toolchain::StringLiteral{"dispatch_after"}, 2},
    {toolchain::StringLiteral{"dispatch_sync"}, 1},
    {toolchain::StringLiteral{"dispatch_once"}, 1},
    {toolchain::StringLiteral{"dispatch_barrier_async"}, 1},
    {toolchain::StringLiteral{"dispatch_barrier_async_and_wait"}, 1},
    {toolchain::StringLiteral{"dispatch_barrier_sync"}, 1}};
...
private:
  // It would've been a perfect place to use toolchain::PointerIntPair, but
  // unfortunately NumLowBitsAvailable for language::Core::Expr had been reduced to 2.
...
public:
  ...
  bool isVisited() const {
    return toolchain::all_of(ParamData, [](const ParameterStatus &S) {
  ...
  void join(const State &Other) {
    ...
    for (auto Pair : toolchain::zip(ParamData, Other.ParamData)) {
...
public:
  ...
  using NameCollection =
      toolchain::SmallVector<toolchain::StringRef, EXPECTED_NUMBER_OF_NAMES>;
...
bool mentionsAnyOfConventionalNames(const Expr *E) {
  ...
  return toolchain::any_of(MentionedNames, [](toolchain::StringRef ConditionName) {
    return toolchain::any_of(
        CONVENTIONAL_CONDITIONS,
        [ConditionName](const toolchain::StringLiteral &Conventional) {
...
public:
  ...
  std::optional<Clarification> VisitSwitchStmt(const SwitchStmt *Switch) {
    ...
    toolchain_unreachable("Found unexpected switch structure");
  }
  ...
  static unsigned getSuccessorIndex(const CFGBlock *Parent,
    ...
    CFGBlock::const_succ_iterator It = toolchain::find(Parent->succs(), Child);
...
private:
  ...
  void findParamsToTrack(const FunctionLikeDecl *Function) {
    for (unsigned Index : toolchain::seq<unsigned>(0u, Function->param_size())) {
  ...
  void check() {
    ...
    assert(
        toolchain::none_of(States, [](const State &S) { return S.isVisited(); }) &&
  ...
  void check(const CFGBlock *BB) {
    ...
    for (const CFGElement &Element : toolchain::reverse(*BB)) {
  ...
  void checkEntry(const CFGBlock *Entry) {
    ...
    toolchain::BitVector NotCalledOnEveryPath(size(), false);
    toolchain::BitVector NotUsedOnEveryPath(size(), false);

    // Check if there are no calls of the marked parameter at all
    for (const auto &IndexedStatus : toolchain::enumerate(EntryStatus)) {
    ...
    for (const CFGBlock *BB : FunctionCFG) {
      ...
      for (unsigned Index : toolchain::seq(0u, size())) {
  ...
  void checkIndirectCall(const CallLikeExpr *CallOrMessage) {
    // CallExpr::arguments does not interact nicely with toolchain::enumerate.
    toolchain::ArrayRef<const Expr *> Arguments =
        toolchain::ArrayRef(CallOrMessage->getArgs(), CallOrMessage->getNumArgs());

    // Let's check if any of the call arguments is a point of interest.
    for (const auto &Argument : toolchain::enumerate(Arguments)) {
  ...
  static bool isConventional(toolchain::StringRef Name) {
    return toolchain::count(CONVENTIONAL_NAMES, Name) != 0;
  }

  /// Return true if the given name has conventional suffixes.
  static bool hasConventionalSuffix(toolchain::StringRef Name) {
    return toolchain::any_of(CONVENTIONAL_SUFFIXES, [Name](toolchain::StringRef Suffix) {
  ...
  static bool isConventionalSelectorPiece(Selector MethodSelector,
    ...
    toolchain::StringRef PieceName = MethodSelector.getNameForSlot(PieceIndex);
  ...
  static bool shouldBlockArgumentBeCalledOnce(const FunctionDecl *Function,
    ...
    return Function &&
           toolchain::any_of(KNOWN_CALLED_ONCE_PARAMETERS,
                        [Function, ParamIndex](
  ...
  bool isPossiblyEmptyImpl() const {
    ...
    return toolchain::all_of(FunctionCFG, [](const CFGBlock *BB) {
      if (!BB) {
      ...
      return toolchain::all_of(
          toolchain::reverse(*BB), // we should start with return statements, if we
  ...
  bool hasEverEscaped(unsigned Index) const {
    return toolchain::any_of(States, [Index](const State &StateForOneBB) {
  ...
  State joinSuccessors(const CFGBlock *BB) const {
    auto Succs =
        toolchain::make_filter_range(BB->succs(), [this](const CFGBlock *Succ) {
    ...
    for (const CFGBlock *Succ : toolchain::drop_begin(Succs, 1)) {
  ...
  void handleConventionalCheck(const CFGBlock *BB, const Expr *Condition,
    ...
    for (const auto &IndexedStatus : toolchain::enumerate(ToAlter)) {
      const ParmVarDecl *Parameter = getParameter(IndexedStatus.index());
...
public:
  ...
  void VisitBinaryOperator(const BinaryOperator *Op) {
    if (Op->getOpcode() == language::Core::BO_Assign) {
...
private:
  ...
  std::optional<unsigned> getIndex(const ParmVarDecl &Parameter) const {
    ...
    ParamSizedVector<const ParmVarDecl *>::const_iterator It =
        toolchain::find(TrackedParams, &Parameter);
