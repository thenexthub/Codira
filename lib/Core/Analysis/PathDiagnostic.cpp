...
void PathDiagnosticConsumer::HandlePathDiagnostic(
  ...
  if (!supportsCrossFileDiagnostics()) {
    ...
    SmallString<128> buf;
    toolchain::raw_svector_ostream warning(buf);
...
static std::optional<bool> comparePiece(const PathDiagnosticPiece &X,
  ...
  toolchain_unreachable("all cases handled");
}
...
void PathDiagnosticConsumer::FilesMade::addDiagnostic(const PathDiagnostic &PD,
                                                      ...
                                                      StringRef FileName) {
  toolchain::FoldingSetNodeID NodeID;
  NodeID.Add(PD);
  void *InsertPos;
...
getLocationForCaller(const StackFrameContext *SFC,
  ...
  case CFGElement::LoopExit:
    toolchain_unreachable("CFGElement kind should not be on callsite!");
  }

  toolchain_unreachable("Unknown CFGElement kind");
}
...
PathDiagnosticLocation::create(const ProgramPoint& P,
  ...
  if (std::optional<BlockEdge> BE = P.getAs<BlockEdge>()) {
    ...
    if (std::optional<CFGElement> BlockFront = BE->getFirstElement()) {
      ...
      toolchain_unreachable("Unexpected CFG element at front of block");
    }
    ...
    toolchain_unreachable("Unexpected ProgramPoint");
  }
...
void PathDiagnosticLocation::Profile(toolchain::FoldingSetNodeID &ID) const {
  ID.Add(Range.getBegin());
  ID.Add(Range.getEnd());
  ID.Add(static_cast<const SourceLocation &>(Loc));
}

void PathDiagnosticPiece::Profile(toolchain::FoldingSetNodeID &ID) const {
...
void PathDiagnosticCallPiece::Profile(toolchain::FoldingSetNodeID &ID) const {
  PathDiagnosticPiece::Profile(ID);
  for (const auto &I : path)
    ID.Add(*I);
}

void PathDiagnosticSpotPiece::Profile(toolchain::FoldingSetNodeID &ID) const {
  PathDiagnosticPiece::Profile(ID);
  ID.Add(Pos);
}

void PathDiagnosticControlFlowPiece::Profile(toolchain::FoldingSetNodeID &ID) const {
  PathDiagnosticPiece::Profile(ID);
  for (const auto &I : *this)
    ID.Add(I);
}

void PathDiagnosticMacroPiece::Profile(toolchain::FoldingSetNodeID &ID) const {
  PathDiagnosticSpotPiece::Profile(ID);
  for (const auto &I : subPieces)
    ID.Add(*I);
}

void PathDiagnosticNotePiece::Profile(toolchain::FoldingSetNodeID &ID) const {
  PathDiagnosticSpotPiece::Profile(ID);
}

void PathDiagnosticPopUpPiece::Profile(toolchain::FoldingSetNodeID &ID) const {
  PathDiagnosticSpotPiece::Profile(ID);
}

void PathDiagnostic::Profile(toolchain::FoldingSetNodeID &ID) const {
  ID.Add(getLocation());
...
void PathDiagnostic::FullProfile(toolchain::FoldingSetNodeID &ID) const {
  Profile(ID);
  for (const auto &I : path)
    ID.Add(*I);
...
LLVM_DUMP_METHOD void PathDiagnosticEventPiece::dump() const {
  toolchain::errs() << "EVENT\n--------------\n";
  toolchain::errs() << getString() << "\n";
  toolchain::errs() << " ---- at ----\n";
  getLocation().dump();
}

LLVM_DUMP_METHOD void PathDiagnosticControlFlowPiece::dump() const {
  toolchain::errs() << "CONTROL\n--------------\n";
...
LLVM_DUMP_METHOD void PathDiagnosticLocation::dump() const {
  ...
  case DeclK:
    if (const auto *ND = dyn_cast_or_null<NamedDecl>(D))
      ...
      toolchain::errs() << "<block>\n";
    else if (D)
      toolchain::errs() << "<unknown decl>\n";
    else
      toolchain::errs() << "<NULL DECL>\n";
    break;
  }
}
