...
FileManager::FileManager(const FileSystemOptions &FSO,
  ...
  // file system.
  if (!this->FS)
    this->FS = toolchain::vfs::getRealFileSystem();
}
...
static toolchain::Expected<DirectoryEntryRef>
getDirectoryFromFile(FileManager &FileMgr, StringRef Filename,
  ...
  if (toolchain::sys::path::is_separator(Filename[Filename.size() - 1]))
    return toolchain::errorCodeToError(make_error_code(std::errc::is_a_directory));

  StringRef DirName = toolchain::sys::path::parent_path(Filename);
...
DirectoryEntry *&FileManager::getRealDirEntry(const toolchain::vfs::Status &Status) {
...
void FileManager::addAncestorsAsVirtualDirs(StringRef Path) {
  StringRef DirName = toolchain::sys::path::parent_path(Path);
  if (DirName.empty())
    DirName = ".";
...
toolchain::Expected<DirectoryEntryRef>
FileManager::getDirectoryRef(StringRef DirName, bool CacheFailure) {
  // stat doesn't like trailing separators except for root directory.
  // At least, on Win32 MSVCRT, stat() cannot strip trailing '/'.
  // (though it can strip '\\')
  if (DirName.size() > 1 &&
      DirName != toolchain::sys::path::root_path(DirName) &&
      toolchain::sys::path::is_separator(DirName.back()))
  ...
  if (is_style_windows(toolchain::sys::path::Style::native)) {
    ...
    if (DirName.size() > 1 && DirName.back() == ':' &&
        DirName.equals_insensitive(toolchain::sys::path::root_name(DirName))) {
  ...
  if (!SeenDirInsertResult.second) {
    ...
    return toolchain::errorCodeToError(SeenDirInsertResult.first->second.getError());
  }
  ...
  if (statError) {
    ...
    return toolchain::errorCodeToError(statError);
  }
...
toolchain::Expected<FileEntryRef> FileManager::getFileRef(StringRef Filename,
                                                     bool openFile,
                                                     bool CacheFailure,
                                                     bool IsText) {
  ++NumFileLookups;
  ...
  UFE->ModTime = toolchain::sys::toTimeT(Status.getLastModificationTime());
  UFE->Dir = &DirInfo.getDirEntry();
  UFE->UID = NextFileUID++;
  UFE->UniqueID = Status.getUniqueID();
  UFE->IsNamedPipe = Status.getType() == toolchain::sys::fs::file_type::fifo_file;
  UFE->IsDeviceFile =
      Status.getType() == toolchain::sys::fs::file_type::character_file;
  UFE->File = std::move(F);
...
toolchain::Expected<FileEntryRef> FileManager::getSTDIN() {
  // Only read stdin once.
  if (STDIN)
    return *STDIN;

  std::unique_ptr<toolchain::MemoryBuffer> Content;
  if (auto ContentOrError = toolchain::MemoryBuffer::getSTDIN())
    Content = std::move(*ContentOrError);
  else
    return toolchain::errorCodeToError(ContentOrError.getError());
...
void FileManager::trackVFSUsage(bool Active) {
  FS->visit([Active](toolchain::vfs::FileSystem &FileSys) {
    if (auto *RFS = dyn_cast<toolchain::vfs::RedirectingFileSystem>(&FileSys))
...
FileEntryRef FileManager::getVirtualFileRef(StringRef Filename, off_t Size,
  ...
  if (!getStatValue(InterndFileName, Status, true, nullptr)) {
    Status = toolchain::vfs::Status(
      Status.getName(), Status.getUniqueID(),
      toolchain::sys::toTimePoint(ModificationTime),
    ...
    RealFE->IsNamedPipe =
        Status.getType() == toolchain::sys::fs::file_type::fifo_file;
...
OptionalFileEntryRef FileManager::getBypassFile(FileEntryRef VF) {
  ...
  if (!SeenBypassFileEntries)
    SeenBypassFileEntries = std::make_unique<
        toolchain::StringMap<toolchain::ErrorOr<FileEntryRef::MapValue>>>();
  ...
  BFE->ModTime = toolchain::sys::toTimeT(Status.getLastModificationTime());
  BFE->UID = NextFileUID++;
...
bool FileManager::FixupRelativePath(SmallVectorImpl<char> &path) const {
  ...
  if (FileSystemOpts.WorkingDir.empty()
      || toolchain::sys::path::is_absolute(pathRef))
    return false;

  SmallString<128> NewPath(FileSystemOpts.WorkingDir);
  toolchain::sys::path::append(NewPath, pathRef);
  path = NewPath;
  return true;
}
...
bool FileManager::makeAbsolutePath(SmallVectorImpl<char> &Path) const {
  ...
  if (!toolchain::sys::path::is_absolute(StringRef(Path.data(), Path.size()))) {
...
void FileManager::fillRealPathName(FileEntry *UFE, toolchain::StringRef FileName) {
  toolchain::SmallString<128> AbsPath(FileName);
  ...
  toolchain::sys::path::remove_dots(AbsPath, /*remove_dot_dot=*/true);
...
FileManager::getBufferForFile(FileEntryRef FE, bool isVolatile,
  ...
  if (Entry->Content)
    return toolchain::MemoryBuffer::getMemBuffer(Entry->Content->getMemBufferRef());
...
FileManager::getNoncachedStatValue(StringRef Path,
  ...
  toolchain::ErrorOr<toolchain::vfs::Status> S = FS->status(FilePath.c_str());
  if (!S)
    return S.getError();
  Result = *S;
...
StringRef FileManager::getCanonicalName(const void *Entry, StringRef Name) {
  toolchain::DenseMap<const void *, toolchain::StringRef>::iterator Known =
  ...
  if (!FS->getRealPath(Name, RealPathBuf)) {
    if (is_style_windows(toolchain::sys::path::Style::native)) {
      ...
      if (!FS->makeAbsolute(AbsPathBuf)) {
        if (toolchain::sys::path::root_name(RealPathBuf) ==
            toolchain::sys::path::root_name(AbsPathBuf)) {
          ...
          toolchain::sys::path::remove_dots(AbsPathBuf, /*remove_dot_dot=*/true);
...
void FileManager::PrintStats() const {
  toolchain::errs() << "\n*** File Manager Stats:\n";
  toolchain::errs() << UniqueRealFiles.size() << " real files found, "
               << UniqueRealDirs.size() << " real dirs found.\n";
  toolchain::errs() << VirtualFileEntries.size() << " virtual files found, "
               << VirtualDirectoryEntries.size() << " virtual dirs found.\n";
  toolchain::errs() << NumDirLookups << " dir lookups, "
               << NumDirCacheMisses << " dir cache misses.\n";
  toolchain::errs() << NumFileLookups << " file lookups, "
               << NumFileCacheMisses << " file cache misses.\n";

  getVirtualFileSystem().visit([](toolchain::vfs::FileSystem &VFS) {
    if (auto *T = dyn_cast_or_null<toolchain::vfs::TracingFileSystem>(&VFS))
  ...
  //toolchain::errs() << PagesMapped << BytesOfPagesMapped << FSLookups;
}
