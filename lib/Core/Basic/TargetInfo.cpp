...
TargetInfo::TargetInfo(const toolchain::Triple &T) : Triple(T) {
  ...
  HalfFormat = &toolchain::APFloat::IEEEhalf();
  FloatFormat = &toolchain::APFloat::IEEEsingle();
  DoubleFormat = &toolchain::APFloat::IEEEdouble();
  LongDoubleFormat = &toolchain::APFloat::IEEEdouble();
  Float128Format = &toolchain::APFloat::IEEEquad();
  Ibm128Format = &toolchain::APFloat::PPCDoubleDouble();
  MCountName = "mcount";
  UserLabelPrefix = "_";
  RegParmMax = 0;
  SSERegParmMax = 0;
...
CFBranchLabelSchemeKind TargetInfo::getDefaultCFBranchLabelScheme() const {
  ...
  // non-default scheme
  toolchain::report_fatal_error("not implemented");
}
...
const char *TargetInfo::getTypeName(IntType T) {
  switch (T) {
  default: toolchain_unreachable("not an integer!");
...
FloatModeKind TargetInfo::getRealTypeByWidth(unsigned BitWidth,
  ...
  switch (BitWidth) {
  case 96:
    if (&getLongDoubleFormat() == &toolchain::APFloat::x87DoubleExtended())
  ...
  case 128:
    ...
    if (ExplicitType == FloatModeKind::Ibm128)
      return hasIbm128Type() ? FloatModeKind::Ibm128
                             : FloatModeKind::NoFloat;
    if (&getLongDoubleFormat() == &toolchain::APFloat::PPCDoubleDouble() ||
        &getLongDoubleFormat() == &toolchain::APFloat::IEEEquad())
      return FloatModeKind::LongDouble;
    if (hasFloat128Type())
...
void TargetInfo::adjust(DiagnosticsEngine &Diags, LangOptions &Opts,
  ...
  default: toolchain_unreachable("invalid wchar_t width");
  case 0: break;
  ...
  if (Opts.HLSL) {
    ...
    if (!Opts.NativeHalfType) {
      HalfFormat = &toolchain::APFloat::IEEEsingle();
...
bool TargetInfo::isValidGCCRegisterName(StringRef Name) const {
  ...
  if (toolchain::is_contained(Names, Name))
    return true;
...
bool TargetInfo::validatePointerAuthKey(const toolchain::APSInt &value) const {
  return false;
}
