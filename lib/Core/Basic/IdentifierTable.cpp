...
static KeywordStatus getKeywordStatusHelper(const LangOptions &LangOpts,
  ...
  default:
    toolchain_unreachable("Unknown KeywordStatus flag");
  }
}
...
void IdentifierTable::PrintStats() const {
  ...
  for (toolchain::StringMap<IdentifierInfo*, toolchain::BumpPtrAllocator>::const_iterator
...
unsigned toolchain::DenseMapInfo<language::Core::Selector>::getHashValue(language::Core::Selector S) {
...
void Selector::print(toolchain::raw_ostream &OS) const {
  OS << getAsString();
}

LLVM_DUMP_METHOD void Selector::dump() const { print(toolchain::errs()); }
...
struct SelectorTableImpl {
  toolchain::FoldingSet<MultiKeywordSelector> Table;
...
const char *language::Core::getOperatorSpelling(OverloadedOperatorKind Operator) {
  switch (Operator) {
  case OO_None:
...
#include "language/Core/Basic/OperatorKinds.def"
  ...
  toolchain_unreachable("Invalid OverloadedOperatorKind!");
}

StringRef language::Core::getNullabilitySpelling(NullabilityKind kind,
...
IdentifierTable::getFutureCompatDiagKind(const IdentifierInfo &II,
  ...
  unsigned Flags = toolchain::StringSwitch<unsigned>(II.getName())
