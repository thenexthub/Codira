...
#include "toolchain/Support/AutoConvert.h"
...
toolchain::MemoryBuffer::BufferKind ContentCache::getMemoryBufferKind() const {
  if (Buffer == nullptr) {
    assert(0 && "Buffer should never be null");
    return toolchain::MemoryBuffer::MemoryBuffer_Malloc;
  }
...
const char *ContentCache::getInvalidBOM(StringRef BufStr) {
  ...
  const char *InvalidBOM =
      toolchain::StringSwitch<const char *>(BufStr)
          .StartsWith(toolchain::StringLiteral::withInnerNUL("\x00\x00\xFE\xFF"),
                      "UTF-32 (BE)")
          .StartsWith(toolchain::StringLiteral::withInnerNUL("\xFF\xFE\x00\x00"),
                      "UTF-32 (LE)")
...
std::optional<toolchain::MemoryBufferRef>
...
const LineEntry *LineTableInfo::FindNearestLineEntry(FileID FID,
  ...
  std::vector<LineEntry>::const_iterator I = toolchain::upper_bound(Entries, Offset);
  if (I == Entries.begin())
    return nullptr;
  return &*--I;
}
...
toolchain::MemoryBufferRef SourceManager::getFakeBufferForRecovery() const {
  if (!FakeBufferForRecovery)
    FakeBufferForRecovery =
        toolchain::MemoryBuffer::getMemBuffer("<<<INVALID BUFFER>>");
...
FileID SourceManager::createFileID(std::unique_ptr<toolchain::MemoryBuffer> Buffer,
                                   SrcMgr::CharacteristicKind FileCharacter,
                                   int LoadedID,
                                   SourceLocation::UIntTy LoadedOffset,
...
FileID SourceManager::createFileID(const toolchain::MemoryBufferRef &Buffer,
                                   SrcMgr::CharacteristicKind FileCharacter,
                                   int LoadedID,
                                   SourceLocation::UIntTy LoadedOffset,
                                   SourceLocation IncludeLoc) {
  return createFileID(toolchain::MemoryBuffer::getMemBuffer(Buffer), FileCharacter,
...
FileID SourceManager::createFileIDImpl(ContentCache &File, StringRef Filename,
  ...
  toolchain::ErrorOr<bool> NeedConversion =
      toolchain::needConversion(Filename.str().c_str());
  if (NeedConversion && *NeedConversion) {
    // Buffer size may increase due to potential z/OS EBCDIC to UTF-8
    // conversion.
    if (std::optional<toolchain::MemoryBufferRef> Buffer =
...
SourceManager::createExpansionLocImpl(const ExpansionInfo &Info,
  ...
  if (NextLocalOffset + Length + 1 <= NextLocalOffset ||
    ...
    toolchain::report_fatal_error("ran out of source locations");
  }
...
void SourceManager::overrideFileContents(
    FileEntryRef SourceFile, std::unique_ptr<toolchain::MemoryBuffer> Buffer) {
  SrcMgr::ContentCache &IR = getOrCreateContentCache(SourceFile);
...
unsigned SourceManager::getColumnNumber(FileID FID, unsigned FilePos,
                                        bool *Invalid) const {
  std::optional<toolchain::MemoryBufferRef> MemBuf = getBufferOrNone(FID);
  if (Invalid)
    *Invalid = !MemBuf;
...
LineOffsetMapping LineOffsetMapping::get(toolchain::MemoryBufferRef Buffer,
  ...
  if ((unsigned long)(End - Start) > sizeof(Word)) {
    do {
      Word = toolchain::support::endian::read64(Buf, toolchain::endianness::little);
      ...
      unsigned N = toolchain::countr_zero(Mask) - 7; // -7 because 0x80 is the marker
      Word >>= N;
      Buf += N / 8 + 1;
...
FileID SourceManager::getUniqueLoadedASTFileID(SourceLocation Loc) const {
  ...
  const FileID *FirstFID =
      toolchain::lower_bound(LoadedSLocEntryAllocBegin, FID, std::greater<FileID>{});
...
bool SourceManager::isInTheSameTranslationUnitImpl(
  ...
  if (isLoadedFileID(LOffs.first) && isLoadedFileID(ROffs.first)) {
    auto FindSLocEntryAlloc = [this](FileID FID) {
      ...
      return toolchain::lower_bound(LoadedSLocEntryAllocBegin, FID,
                               std::greater<FileID>{});
    };
...
SourceManager::isInTheSameTranslationUnit(FileIDAndOffset &LOffs,
  ...
  toolchain::SmallDenseMap<FileID, Entry, 16> LChain;
  ...
  toolchain_unreachable("Unsortable locations found");
}

void SourceManager::PrintStats() const {
  toolchain::errs() << "\n*** Source Manager Stats:\n";
  toolchain::errs() << FileInfos.size() << " files mapped, " << MemBufferInfos.size()
               << " mem buffers mapped.\n";
  toolchain::errs() << LocalSLocEntryTable.size() << " local SLocEntries allocated ("
               << toolchain::capacity_in_bytes(LocalSLocEntryTable)
               << " bytes of capacity), " << NextLocalOffset
               << "B of SLoc address space used.\n";
  toolchain::errs() << LoadedSLocEntryTable.size()
               << " loaded SLocEntries allocated ("
               << toolchain::capacity_in_bytes(LoadedSLocEntryTable)
               << " bytes of capacity), "
  ...
  toolchain::errs() << NumFileBytesMapped << " bytes of files mapped, "
               << NumLineNumsComputed << " files with line #'s computed, "
               << NumMacroArgsComputed << " files with macro args computed.\n";
  toolchain::errs() << "FileID scans: " << NumLinearScans << " linear, "
...
void SourceManager::noteSLocAddressSpaceUsage(
  ...
  using UsageMap = toolchain::MapVector<const FileEntry*, Info>;
  ...
  for (auto &[Entry, FileInfo] :
       toolchain::make_range(SortedUsage.begin(), SortedEnd)) {
...
size_t SourceManager::getDataStructureSizes() const {
  size_t size = toolchain::capacity_in_bytes(MemBufferInfos) +
                toolchain::capacity_in_bytes(LocalSLocEntryTable) +
                toolchain::capacity_in_bytes(LoadedSLocEntryTable) +
                toolchain::capacity_in_bytes(SLocEntryLoaded) +
                toolchain::capacity_in_bytes(FileInfos);

  if (OverriddenFilesInfo)
    size += toolchain::capacity_in_bytes(OverriddenFilesInfo->OverriddenFiles);
...
SourceManagerForFile::SourceManagerForFile(StringRef FileName,
  ...
  InMemoryFileSystem->addFile(
      FileName, 0,
      toolchain::MemoryBuffer::getMemBuffer(Content, FileName,
  ...
  FileEntryRef FE = toolchain::cantFail(FileMgr->getFileRef(FileName));
  FileID ID =
      SourceMgr->createFileID(FE, SourceLocation(), language::Core::SrcMgr::C_User);
  assert(ID.isValid());
