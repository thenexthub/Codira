...
void RISCVTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (VScale && VScale->first && VScale->first == VScale->second)
    Builder.defineMacro("__riscv_v_fixed_vlen",
                        Twine(VScale->first * toolchain::RISCV::RVVBitsPerBlock));
  ...
  if (Opts.CFProtectionBranch) {
    ...
    case CFBranchLabelSchemeKind::Default:
      toolchain_unreachable("default cf-branch-label scheme should already be "
...
bool RISCVTargetInfo::initFeatureMap(
  ...
  auto ParseResult = toolchain::RISCVISAInfo::parseFeatures(XLen, FeaturesVec);
  if (!ParseResult) {
    std::string Buffer;
  ...
  toolchain::append_range(AllFeatures, (*ParseResult)->toFeatures());
...
bool RISCVTargetInfo::hasFeature(StringRef Feature) const {
  bool Is64Bit = getTriple().isRISCV64();
  auto Result = toolchain::StringSwitch<std::optional<bool>>(Feature)
                    .Case("riscv", true)
...
bool RISCVTargetInfo::handleTargetFeatures(std::vector<std::string> &Features,
  ...
  auto ParseResult = toolchain::RISCVISAInfo::parseFeatures(XLen, Features);
  if (!ParseResult) {
    std::string Buffer;
    toolchain::raw_string_ostream OutputErrMsg(Buffer);
    handleAllErrors(ParseResult.takeError(), [&](toolchain::StringError &ErrMsg) {
  ...
  FastScalarUnalignedAccess =
      toolchain::is_contained(Features, "+unaligned-scalar-mem");

  if (toolchain::is_contained(Features, "+experimental"))
  ...
  auto handleArchExtension = [](StringRef AttrString,
    ...
    for (auto Ext : Exts) {
      ...
      std::string TargetFeature =
          toolchain::RISCVISAInfo::getTargetFeatureForExtension(ExtName);
  ...
  for (auto &Feature : AttrFeatures) {
    ...
    if (Feature.starts_with("arch=")) {
      ...
      if (!FoundArch) {
        ...
        StringRef MarchFromCPU = toolchain::RISCV::getMArchFromMcpu(Ret.CPU);
        if (MarchFromCPU != "") {
          Ret.Features.clear();
...
bool RISCVTargetInfo::validateCpuSupports(StringRef Feature) const {
  ...
  return -1 != toolchain::RISCVISAInfo::getRISCVFeaturesBitsInfo(Feature).second;
}

bool RISCVTargetInfo::isValidFeatureName(StringRef Name) const {
  return toolchain::RISCVISAInfo::isSupportedExtensionFeature(Name);
}
...
bool RISCVTargetInfo::validateCpuIs(StringRef CPUName) const {
  ...
  return toolchain::RISCV::hasValidCPUModel(CPUName);
}
