...
#ifndef LANGUAGE_CORE_LIB_BASIC_TARGETS_PPC_H
#define LANGUAGE_CORE_LIB_BASIC_TARGETS_PPC_H
...
public:
  PPCTargetInfo(const toolchain::Triple &Triple, const TargetOptions &)
      : TargetInfo(Triple) {
    SuitableAlign = 128;
    LongDoubleWidth = LongDoubleAlign = 128;
    LongDoubleFormat = &toolchain::APFloat::PPCDoubleDouble();
    HasStrictFP = true;
    HasIbm128 = true;
  ...
  bool setCPU(const std::string &Name) override {
    ...
    if (CPUKnown) {
      ...
      // CPU identification.
      ArchDefs =
          (ArchDefineTypes)toolchain::StringSwitch<int>(CPU)
  ...
  void addP10SpecificFeatures(toolchain::StringMap<bool> &Features) const;
  void addP11SpecificFeatures(toolchain::StringMap<bool> &Features) const;
  void addFutureSpecificFeatures(toolchain::StringMap<bool> &Features) const;
  ...
  const char *getLongDoubleMangling() const override {
    if (LongDoubleWidth == 64)
      return "e";
    return LongDoubleFormat == &toolchain::APFloat::PPCDoubleDouble()
               ? "g"
               : "u9__ieee128";
  }
...
public:
  PPC32TargetInfo(const toolchain::Triple &Triple, const TargetOptions &Opts)
      : PPCTargetInfo(Triple, Opts) {
    if (Triple.isOSAIX())
      resetDataLayout("E-m:a-p:32:32-Fi32-i64:64-n32");
    else if (Triple.getArch() == toolchain::Triple::ppcle)
...
public:
  PPC64TargetInfo(const toolchain::Triple &Triple, const TargetOptions &Opts)
      : PPCTargetInfo(Triple, Opts) {
    ...
    if (Triple.isOSAIX()) {
      ...
      LongDoubleAlign = DoubleAlign = 32;
      LongDoubleFormat = &toolchain::APFloat::IEEEdouble();
    } else if ((Triple.getArch() == toolchain::Triple::ppc64le)) {
...
#endif // LANGUAGE_CORE_LIB_BASIC_TARGETS_PPC_H
