...
bool CSKYTargetInfo::isValidCPUName(StringRef Name) const {
  return toolchain::CSKY::parseCPUArch(Name) != toolchain::CSKY::ArchKind::INVALID;
}

bool CSKYTargetInfo::setCPU(const std::string &Name) {
  toolchain::CSKY::ArchKind archKind = toolchain::CSKY::parseCPUArch(Name);
  bool isValid = (archKind != toolchain::CSKY::ArchKind::INVALID);
...
void CSKYTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (Arch != toolchain::CSKY::ArchKind::INVALID) {
    ArchName = toolchain::CSKY::getArchName(Arch);
    CPUName = CPU;
  }
