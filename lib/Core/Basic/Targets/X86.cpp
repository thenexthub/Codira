...
bool X86TargetInfo::initFeatureMap(
  ...
  toolchain::append_range(UpdatedFeaturesVec, UpdatedAVX10FeaturesVec);
  ...
  if (I != Features.end() && I->getValue() &&
      !toolchain::is_contained(UpdatedFeaturesVec, "-popcnt"))
  ...
  I = Features.find("sse");
  if (I != Features.end() && I->getValue() &&
      !toolchain::is_contained(UpdatedFeaturesVec, "-mmx"))
    Features["mmx"] = true;

  // Enable xsave if avx is enabled and xsave is not explicitly disabled.
  I = Features.find("avx");
  if (I != Features.end() && I->getValue() &&
      !toolchain::is_contained(UpdatedFeaturesVec, "-xsave"))
    Features["xsave"] = true;

  // Enable CRC32 if SSE4.2 is enabled and CRC32 is not explicitly disabled.
  I = Features.find("sse4.2");
  if (I != Features.end() && I->getValue() &&
      !toolchain::is_contained(UpdatedFeaturesVec, "-crc32"))
    Features["crc32"] = true;

  return true;
}

void X86TargetInfo::setFeatureEnabled(toolchain::StringMap<bool> &Features,
  ...
  toolchain::X86::updateImpliedFeatures(Name, Enabled, Features);
}
...
bool X86TargetInfo::handleTargetFeatures(std::vector<std::string> &Features,
  ...
  for (const auto &Feature : Features) {
    ...
    X86SSEEnum Level = toolchain::StringSwitch<X86SSEEnum>(Feature)
    ...
    XOPEnum XLevel = toolchain::StringSwitch<XOPEnum>(Feature)
                         .Case("+xop", XOP)
                         .Case("+fma4", FMA4)
                         .Case("+sse4a", SSE4A)
                         .Default(NoXOP);
  ...
  if (!HasX87 && LongDoubleFormat == &toolchain::APFloat::x87DoubleExtended())
    HasLongDouble = false;
...
void X86TargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (HasLAHFSAHF || getTriple().getArch() == toolchain::Triple::x86)
  ...
  if (Opts.MicrosoftExt && getTriple().getArch() == toolchain::Triple::x86) {
    switch (SSELevel) {
    case AVX512F:
    case AVX2:
    case AVX:
  ...
  if (HasCX16 && getTriple().getArch() == toolchain::Triple::x86_64)
...
bool X86TargetInfo::isValidFeatureName(StringRef Name) const {
  return toolchain::StringSwitch<bool>(Name)
      .Case("adx", true)
      .Case("aes", true)
...
bool X86TargetInfo::hasFeature(StringRef Feature) const {
  return toolchain::StringSwitch<bool>(Feature)
      ...
      .Case("x86", true)
      .Case("x86_32", getTriple().getArch() == toolchain::Triple::x86)
      .Case("x86_64", getTriple().getArch() == toolchain::Triple::x86_64)
      .Case("x87", HasX87)
...
bool X86TargetInfo::validateCpuSupports(StringRef FeatureStr) const {
  return toolchain::StringSwitch<bool>(FeatureStr)
...
static toolchain::X86::ProcessorFeatures getFeature(StringRef Name) {
  return toolchain::StringSwitch<toolchain::X86::ProcessorFeatures>(Name)
#define X86_FEATURE_COMPAT(ENUM, STR, PRIORITY)                                \
  .Case(STR, toolchain::X86::FEATURE_##ENUM)
...
toolchain::APInt X86TargetInfo::getFMVPriority(ArrayRef<StringRef> Features) const {
  ...
  return toolchain::APInt(32, Priority);
}

bool X86TargetInfo::validateCPUSpecificCPUDispatch(StringRef Name) const {
  return toolchain::X86::validateCPUSpecificCPUDispatch(Name);
}

char X86TargetInfo::CPUSpecificManglingCharacter(StringRef Name) const {
  return toolchain::X86::getCPUDispatchMangling(Name);
}
...
void X86TargetInfo::getCPUSpecificCPUDispatchFeatures(
  ...
  toolchain::X86::getFeaturesForCPU(Name, TargetCPUFeatures, true);
...
std::optional<unsigned> X86TargetInfo::getCPUCacheLineSize() const {
  ...
  toolchain_unreachable("Unknown CPU kind");
}

bool X86TargetInfo::validateOutputSize(const toolchain::StringMap<bool> &FeatureMap,
...
bool X86TargetInfo::validateInputSize(const toolchain::StringMap<bool> &FeatureMap,
                                      StringRef Constraint,
                                      unsigned Size) const {
  return validateOperandSize(FeatureMap, Constraint, Size);
}

bool X86TargetInfo::validateOperandSize(const toolchain::StringMap<bool> &FeatureMap,
                                        StringRef Constraint,
                                        unsigned Size) const {
  switch (Constraint[0]) {
  default:
...
void X86TargetInfo::fillValidCPUList(SmallVectorImpl<StringRef> &Values) const {
  ...
  toolchain::X86::fillValidCPUArchList(Values, Only64Bit);
}

void X86TargetInfo::fillValidTuneCPUList(SmallVectorImpl<StringRef> &Values) const {
  toolchain::X86::fillValidTuneCPUList(Values);
}
