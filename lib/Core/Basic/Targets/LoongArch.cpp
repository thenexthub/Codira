...
static constexpr toolchain::StringTable BuiltinBaseStrings =
...
static constexpr toolchain::StringTable BuiltinLSXStrings =
    CLANG_BUILTIN_STR_TABLE_START
#define TARGET_BUILTIN CLANG_TARGET_BUILTIN_STR_TABLE
...
static constexpr toolchain::StringTable BuiltinLASXStrings =
    CLANG_BUILTIN_STR_TABLE_START
#define TARGET_BUILTIN CLANG_TARGET_BUILTIN_STR_TABLE
...
bool LoongArchTargetInfo::initFeatureMap(
  ...
  if (getTriple().getArch() == toolchain::Triple::loongarch64)
    Features["64bit"] = true;
  if (getTriple().getArch() == toolchain::Triple::loongarch32)
    Features["32bit"] = true;
...
bool LoongArchTargetInfo::hasFeature(StringRef Feature) const {
  bool Is64Bit = getTriple().getArch() == toolchain::Triple::loongarch64;
...
static std::pair<AttrFeatureKind, toolchain::StringRef>
getAttrFeatureTypeAndValue(toolchain::StringRef AttrFeature) {
...
LoongArchTargetInfo::parseTargetAttr(StringRef Features) const {
  ...
  for (auto &Feature : AttrFeatures) {
    ...
    case AttrFeatureKind::Arch: {
      if (toolchain::LoongArch::isValidArchName(Value) || Value == "la64v1.0" ||
          Value == "la64v1.1") {
        std::vector<toolchain::StringRef> ArchFeatures;
        if (toolchain::LoongArch::getArchFeatures(Value, ArchFeatures)) {
...
bool LoongArchTargetInfo::isValidCPUName(StringRef Name) const {
  return toolchain::LoongArch::isValidCPUName(Name);
}

void LoongArchTargetInfo::fillValidCPUList(
    SmallVectorImpl<StringRef> &Values) const {
  toolchain::LoongArch::fillValidCPUList(Values);
}

bool LoongArchTargetInfo::isValidFeatureName(StringRef Name) const {
  return toolchain::LoongArch::isValidFeatureName(Name);
}
