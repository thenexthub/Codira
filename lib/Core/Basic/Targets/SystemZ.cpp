...
static constexpr int NumBuiltins =
    language::Core::SystemZ::LastTSBuiltin - Builtin::FirstTSBuiltin;
...
int SystemZTargetInfo::getISARevision(StringRef Name) const {
  const auto Rev =
      toolchain::find_if(ISARevisions, [Name](const ISANameRevision &CR) {
        return CR.Name == Name;
      });
...
void SystemZTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (!getTriple().getOSVersion().empty()) {
    toolchain::VersionTuple V = getTriple().getOSVersion();
    ...
    Str += toolchain::utohexstr(Librel);
