...
static constexpr int NumBuiltins =
    language::Core::PPC::LastTSBuiltin - Builtin::FirstTSBuiltin;
...
void PPCTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  // Target properties.
  if (getTriple().getArch() == toolchain::Triple::ppc64le ||
      getTriple().getArch() == toolchain::Triple::ppcle) {
  ...
  if (getTriple().getOS() == toolchain::Triple::Linux && PointerWidth == 64)
...
static bool ppcUserFeaturesCheck(DiagnosticsEngine &Diags,
  ...
  auto FindVSXSubfeature = [&](StringRef Feature, StringRef SubOption,
                               StringRef Option) {
    if (toolchain::is_contained(FeaturesVec, Feature)) {
  ...
  // VSX subfeatures.
  bool Found = false;
  if (toolchain::is_contained(FeaturesVec, "-hard-float")) {
  ...
  if (toolchain::is_contained(FeaturesVec, "+vsx") &&
      toolchain::is_contained(FeaturesVec, "-altivec")) {
...
bool PPCTargetInfo::initFeatureMap(
  ...
  const toolchain::Triple &TheTriple = getTriple();

  std::optional<toolchain::StringMap<bool>> FeaturesOpt =
      toolchain::PPC::getPPCDefaultTargetFeatures(TheTriple,
                                             toolchain::PPC::normalizeCPUName(CPU));
  if (FeaturesOpt)
  ...
  if (!(ArchDefs & ArchDefinePwr7) && (ArchDefs & ArchDefinePpcgr) &&
      toolchain::is_contained(FeaturesVec, "+float128")) {
  ...
  if (toolchain::is_contained(FeaturesVec, "+rop-protect")) {
    if (PointerWidth == 32) {
  ...
  if (!(ArchDefs & ArchDefinePwr8) &&
      toolchain::is_contained(FeaturesVec, "+privileged")) {
...
void PPCTargetInfo::setFeatureEnabled(toolchain::StringMap<bool> &Features,
  ...
  if (Enabled) {
    ...
    bool FeatureHasVSX = toolchain::StringSwitch<bool>(Name)
                             .Case("vsx", true)
...
ArrayRef<TargetInfo::AddlRegName> PPCTargetInfo::getGCCAddlRegNames() const {
  return toolchain::ArrayRef(GCCAddlRegNames);
}

bool PPCTargetInfo::isValidCPUName(StringRef Name) const {
  return toolchain::PPC::isValidCPU(Name);
}

void PPCTargetInfo::fillValidCPUList(SmallVectorImpl<StringRef> &Values) const {
  toolchain::PPC::fillValidCPUList(Values);
}
...
void PPCTargetInfo::adjust(DiagnosticsEngine &Diags, LangOptions &Opts,
  ...
  if (LongDoubleFormat != &toolchain::APFloat::IEEEdouble())
    LongDoubleFormat = Opts.PPCIEEELongDouble
                           ? &toolchain::APFloat::IEEEquad()
                           : &toolchain::APFloat::PPCDoubleDouble();
  Opts.IEEE128 = 1;
...
#define PPC_CPU(NAME, Linux_SUPPORT_METHOD, LinuxID, AIX_SUPPORT_METHOD,       \
  ...
  std::pair<unsigned, unsigned> SuppportMethod =
      toolchain::StringSwitch<std::pair<unsigned, unsigned>>(CPUName)
#include "toolchain/TargetParser/PPCTargetParser.def"
