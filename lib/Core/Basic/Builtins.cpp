...
#undef HEADER
  ...
  toolchain_unreachable("Unknown HeaderDesc::HeaderID enum");
}
...
Builtin::Context::getShardAndInfo(unsigned ID) const {
  ...
  toolchain_unreachable("Invalid target builtin shard structure!");
}
...
bool Builtin::Context::isBuiltinFunc(toolchain::StringRef FuncName) {
  bool InStdNamespace = FuncName.consume_front("std-");
  for (const auto &Shard : {InfosShard{&BuiltinStrings, BuiltinInfos}})
    if (toolchain::StringRef FuncNameSuffix = FuncName;
...
static bool builtinIsSupported(const toolchain::StringTable &Strings,
...
void Builtin::Context::initializeBuiltins(IdentifierTable &Table,
  ...
  for (toolchain::StringRef Name : LangOpts.NoBuiltinFuncs) {
...
bool Builtin::evaluateRequiredTargetFeatures(
    StringRef RequiredFeatures, const toolchain::StringMap<bool> &TargetFetureMap) {
