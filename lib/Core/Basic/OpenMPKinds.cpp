...
unsigned language::Core::getOpenMPSimpleClauseType(OpenMPClauseKind Kind, StringRef Str,
                                          const LangOptions &LangOpts) {
  switch (Kind) {
  case OMPC_default:
    return toolchain::StringSwitch<unsigned>(Str)
#define OMP_DEFAULT_KIND(Enum, Name) .Case(Name, unsigned(Enum))
#include "toolchain/Frontend/OpenMP/OMPKinds.def"
        .Default(unsigned(toolchain::omp::OMP_DEFAULT_unknown));
  case OMPC_proc_bind:
    return toolchain::StringSwitch<unsigned>(Str)
#define OMP_PROC_BIND_KIND(Enum, Name, Value) .Case(Name, Value)
#include "toolchain/Frontend/OpenMP/OMPKinds.def"
        .Default(unsigned(toolchain::omp::OMP_PROC_BIND_unknown));
  case OMPC_schedule:
    return toolchain::StringSwitch<unsigned>(Str)
...
#include "language/Core/Basic/OpenMPKinds.def"
        .Default(OMPC_SCHEDULE_unknown);
  case OMPC_depend: {
    unsigned Type = toolchain::StringSwitch<unsigned>(Str)
...
#include "language/Core/Basic/OpenMPKinds.def"
  ...
  case OMPC_doacross:
    return toolchain::StringSwitch<OpenMPDoacrossClauseModifier>(Str)
#define OPENMP_DOACROSS_MODIFIER(Name) .Case(#Name, OMPC_DOACROSS_##Name)
#include "language/Core/Basic/OpenMPKinds.def"
        .Default(OMPC_DOACROSS_unknown);
  case OMPC_linear:
    return toolchain::StringSwitch<OpenMPLinearClauseKind>(Str)
...
#include "language/Core/Basic/OpenMPKinds.def"
  ...
  case OMPC_dist_schedule:
    return toolchain::StringSwitch<OpenMPDistScheduleClauseKind>(Str)
...
#include "language/Core/Basic/OpenMPKinds.def"
  ...
  case OMPC_update:
    return toolchain::StringSwitch<OpenMPDependClauseKind>(Str)
#define OPENMP_DEPEND_KIND(Name) .Case(#Name, OMPC_DEPEND_##Name)
#include "language/Core/Basic/OpenMPKinds.def"
        .Default(OMPC_DEPEND_unknown);
  case OMPC_device:
    return toolchain::StringSwitch<OpenMPDeviceClauseModifier>(Str)
#define OPENMP_DEVICE_MODIFIER(Name) .Case(#Name, OMPC_DEVICE_##Name)
#include "language/Core/Basic/OpenMPKinds.def"
        .Default(OMPC_DEVICE_unknown);
  case OMPC_reduction:
    return toolchain::StringSwitch<OpenMPReductionClauseModifier>(Str)
#define OPENMP_REDUCTION_MODIFIER(Name) .Case(#Name, OMPC_REDUCTION_##Name)
#include "language/Core/Basic/OpenMPKinds.def"
        .Default(OMPC_REDUCTION_unknown);
  case OMPC_adjust_args:
    return toolchain::StringSwitch<OpenMPAdjustArgsOpKind>(Str)
...
#include "language/Core/Basic/OpenMPKinds.def"
  ...
  case OMPC_allocate:
    return toolchain::StringSwitch<OpenMPAllocateClauseModifier>(Str)
...
#include "language/Core/Basic/OpenMPKinds.def"
  ...
  toolchain_unreachable("Invalid OpenMP simple clause kind");
}

const char *language::Core::getOpenMPSimpleClauseTypeName(OpenMPClauseKind Kind,
                                                 unsigned Type) {
  switch (Kind) {
  case OMPC_default:
    switch (toolchain::omp::DefaultKind(Type)) {
#define OMP_DEFAULT_KIND(Enum, Name)                                           \
  case Enum:                                                                   \
    return Name;
#include "toolchain/Frontend/OpenMP/OMPKinds.def"
    }
    toolchain_unreachable("Invalid OpenMP 'default' clause type");
  case OMPC_proc_bind:
    switch (Type) {
...
#include "toolchain/Frontend/OpenMP/OMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'proc_bind' clause type");
  case OMPC_schedule:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'schedule' clause type");
  case OMPC_depend:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'depend' clause type");
  case OMPC_doacross:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'doacross' clause type");
  case OMPC_linear:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'linear' clause type");
  case OMPC_map:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'map' clause type");
  case OMPC_to:
  case OMPC_from:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'to' or 'from' clause type");
  case OMPC_dist_schedule:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'dist_schedule' clause type");
  case OMPC_defaultmap:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'defaultmap' clause type");
  case OMPC_atomic_default_mem_order:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'atomic_default_mem_order' clause type");
  case OMPC_device_type:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'device_type' clause type");
  case OMPC_at:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'at' clause type");
  case OMPC_severity:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'severity' clause type");
  case OMPC_lastprivate:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'lastprivate' clause type");
  case OMPC_order:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'order' clause type");
  case OMPC_update:
    switch (Type) {
    case OMPC_DEPEND_unknown:
      return "unknown";
...
#include "language/Core/Basic/OpenMPKinds.def"
  ...
  case OMPC_fail: {
    ...
    toolchain_unreachable("Invalid OpenMP 'fail' clause modifier");
  }
  case OMPC_device:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'device' clause modifier");
  case OMPC_reduction:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'reduction' clause modifier");
  case OMPC_adjust_args:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'adjust_args' clause kind");
  case OMPC_bind:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'bind' clause type");
  case OMPC_grainsize:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'grainsize' clause modifier");
  case OMPC_num_tasks:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'num_tasks' clause modifier");
  case OMPC_allocate:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'allocate' clause modifier");
  case OMPC_num_threads:
    switch (Type) {
...
#include "language/Core/Basic/OpenMPKinds.def"
    ...
    toolchain_unreachable("Invalid OpenMP 'num_threads' clause modifier");
  case OMPC_unknown:
  case OMPC_threadprivate:
  case OMPC_if:
  case OMPC_final:
...
bool language::Core::isOpenMPLoopDirective(OpenMPDirectiveKind DKind) {
  return getDirectiveAssociation(DKind) == Association::Loop;
}

bool language::Core::isOpenMPWorksharingDirective(OpenMPDirectiveKind DKind) {
...
bool language::Core::isOpenMPTaskLoopDirective(OpenMPDirectiveKind DKind) {
  return DKind == OMPD_taskloop ||
         toolchain::is_contained(getLeafConstructs(DKind), OMPD_taskloop);
}

bool language::Core::isOpenMPParallelDirective(OpenMPDirectiveKind DKind) {
  if (DKind == OMPD_teams_loop)
    return true;
  return DKind == OMPD_parallel ||
         toolchain::is_contained(getLeafConstructs(DKind), OMPD_parallel);
}

bool language::Core::isOpenMPTargetExecutionDirective(OpenMPDirectiveKind DKind) {
  return DKind == OMPD_target ||
         toolchain::is_contained(getLeafConstructs(DKind), OMPD_target);
}

bool language::Core::isOpenMPTargetDataManagementDirective(OpenMPDirectiveKind DKind) {
  return DKind == OMPD_target_data || DKind == OMPD_target_enter_data ||
         DKind == OMPD_target_exit_data || DKind == OMPD_target_update;
}

bool language::Core::isOpenMPNestingTeamsDirective(OpenMPDirectiveKind DKind) {
  if (DKind == OMPD_teams)
    return true;
...
bool language::Core::isOpenMPTeamsDirective(OpenMPDirectiveKind DKind) {
  return DKind == OMPD_teams ||
         toolchain::is_contained(getLeafConstructs(DKind), OMPD_teams);
}

bool language::Core::isOpenMPSimdDirective(OpenMPDirectiveKind DKind) {
  ...
  return DKind == OMPD_simd ||
         toolchain::is_contained(getLeafConstructs(DKind), OMPD_simd);
}

bool language::Core::isOpenMPNestingDistributeDirective(OpenMPDirectiveKind Kind) {
...
bool language::Core::isOpenMPDistributeDirective(OpenMPDirectiveKind Kind) {
  return Kind == OMPD_distribute ||
         toolchain::is_contained(getLeafConstructs(Kind), OMPD_distribute);
}

bool language::Core::isOpenMPGenericLoopDirective(OpenMPDirectiveKind Kind) {
  if (Kind == OMPD_loop)
    return true;
  ArrayRef<Directive> Leaves = getLeafConstructs(Kind);
...
bool language::Core::isOpenMPPrivate(OpenMPClauseKind Kind) {
...
bool language::Core::isOpenMPThreadPrivate(OpenMPClauseKind Kind) {
  return Kind == OMPC_threadprivate || Kind == OMPC_copyin;
}

bool language::Core::isOpenMPTaskingDirective(OpenMPDirectiveKind Kind) {
  return Kind == OMPD_task || isOpenMPTaskLoopDirective(Kind);
}

bool language::Core::isOpenMPLoopBoundSharingDirective(OpenMPDirectiveKind Kind) {
...
bool language::Core::isOpenMPLoopTransformationDirective(OpenMPDirectiveKind DKind) {
  return DKind == OMPD_tile || DKind == OMPD_unroll || DKind == OMPD_reverse ||
         DKind == OMPD_interchange || DKind == OMPD_stripe;
}

bool language::Core::isOpenMPCombinedParallelADirective(OpenMPDirectiveKind DKind) {
...
bool language::Core::needsTaskBasedThreadLimit(OpenMPDirectiveKind DKind) {
...
bool language::Core::isOpenMPExecutableDirective(OpenMPDirectiveKind DKind) {
  if (DKind == OMPD_error)
    return true;
...
bool language::Core::isOpenMPInformationalDirective(OpenMPDirectiveKind DKind) {
  if (DKind == OMPD_error)
    return true;
  Category Cat = getDirectiveCategory(DKind);
...
bool language::Core::isOpenMPCapturingDirective(OpenMPDirectiveKind DKind) {
...
bool language::Core::isOpenMPOrderConcurrentNestableDirective(
...
void language::Core::getOpenMPCaptureRegions(
    SmallVectorImpl<OpenMPDirectiveKind> &CaptureRegions,
    OpenMPDirectiveKind DKind) {
  assert(unsigned(DKind) < toolchain::omp::Directive_enumSize);
  ...
  auto GetRegionsForLeaf = [&](OpenMPDirectiveKind LKind) {
    ...
    case OMPD_loop:
      ...
      if (!CaptureRegions.empty() &&
          !toolchain::is_contained(CaptureRegions, OMPD_parallel))
        CaptureRegions.push_back(OMPD_parallel);
      else
        return true;
      break;
    ...
    case OMPD_master:
      return false;
    default:
      toolchain::errs() << getOpenMPDirectiveName(LKind, toolchain::omp::FallbackVersion)
                   << '\n';
  ...
  assert((CaptureRegions[0] == OMPD_unknown ||
          !toolchain::is_contained(CaptureRegions, OMPD_unknown)) &&
         "Misplaced OMPD_unknown");
}

bool language::Core::checkFailClauseParameter(OpenMPClauseKind FailClauseParameter) {
  return FailClauseParameter == toolchain::omp::OMPC_acquire ||
         FailClauseParameter == toolchain::omp::OMPC_relaxed ||
         FailClauseParameter == toolchain::omp::OMPC_seq_cst;
}
