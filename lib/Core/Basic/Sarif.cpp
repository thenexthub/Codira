...
using language::Core::detail::SarifArtifact;
using language::Core::detail::SarifArtifactLocation;
...
static std::string percentEncodeURICharacter(char C) {
  ...
  // reserved character.
  if (toolchain::isAlnum(C) || StringRef("-._~:@!$&'()*+,;=").contains(C))
    return std::string(&C, 1);
  return "%" + toolchain::toHex(StringRef(&C, 1));
}
...
static std::string fileNameToURI(StringRef Filename) {
  ...
  for (StringRef Component : toolchain::make_range(++Iter, End)) {
...
static StringRef importanceToStr(ThreadFlowImportance I) {
  ...
  toolchain_unreachable("Fully covered switch is not so fully covered");
}
...
static StringRef resultLevelToStr(SarifResultLevel R) {
  ...
  toolchain_unreachable("Potentially un-handled SarifResultLevel. "
...
void SarifDocumentWriter::endRun() {
  ...
  toolchain::sort(Vec, toolchain::less_first());
  ...
  json::Object Tool{
      {"driver",
       json::Object{{"name", ShortToolName},
                    ...
                    {"version", ToolVersion},
                    {"informationUri",
                     "https://clang.toolchain.org/docs/UsersManual.html"}}}};
