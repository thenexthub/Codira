...
const StreamingDiagnostic &language::Core::operator<<(const StreamingDiagnostic &DB,
...
DiagnosticsEngine::DiagStateMap::File::lookup(unsigned Offset) const {
  auto OnePastIt =
      toolchain::partition_point(StateTransitions, [=](const DiagStatePoint &P) {
...
void DiagnosticsEngine::DiagStateMap::dump(SourceManager &SrcMgr,
  ...
  toolchain::errs() << "diagnostic state at ";
  CurDiagStateLoc.print(toolchain::errs(), SrcMgr);
  toolchain::errs() << ": " << CurDiagState << "\n";
    ...
    for (DiagStatePoint &Transition : File.StateTransitions) {
      ...
      auto PrintInnerHeading = [&] {
        ...
        SrcMgr.getLocForStartOfFile(ID)
            ...
            .print(toolchain::errs(), SrcMgr);
        toolchain::errs() << ": state " << Transition.State << ":\n";
      };
      ...
      for (auto &Mapping : *Transition.State) {
        ...
        toolchain::errs() << "    ";
        if (Option.empty())
          toolchain::errs() << "<unknown " << Mapping.first << ">";
        else
          toolchain::errs() << Option;
        toolchain::errs() << ": ";

        switch (Mapping.second.getSeverity()) {
        case diag::Severity::Ignored:
          toolchain::errs() << "ignored";
          break;
...
// here.
class WarningsSpecialCaseList : public toolchain::SpecialCaseList {
public:
  static std::unique_ptr<WarningsSpecialCaseList>
  create(const toolchain::MemoryBuffer &Input, std::string &Err);
...
private:
  ...
  // positive category.
  bool globsMatches(const toolchain::StringMap<Matcher> &CategoriesToMatchers,
                    StringRef FilePath) const;

  toolchain::DenseMap<diag::kind, const Section *> DiagToSection;
};
} // namespace

std::unique_ptr<WarningsSpecialCaseList>
WarningsSpecialCaseList::create(const toolchain::MemoryBuffer &Input,
                                std::string &Err) {
...
void WarningsSpecialCaseList::processSections(DiagnosticsEngine &Diags) {
  ...
  // sec.SectionStr == "*"; });
  toolchain::erase_if(Sections, [](Section &sec) { return sec.SectionStr == "*"; });
  ...
  toolchain::sort(LineAndSectionEntry);
  static constexpr auto WarningFlavor = language::Core::diag::Flavor::WarningOrError;
...
void DiagnosticsEngine::setDiagSuppressionMapping(toolchain::MemoryBuffer &Input) {
  std::string Error;
...
bool WarningsSpecialCaseList::isDiagSuppressed(diag::kind DiagId,
  ...
  if (SrcEntriesIt == EntityTypeToCategories.end())
    return false;
  const toolchain::StringMap<toolchain::SpecialCaseList::Matcher> &CategoriesToMatchers =
  ...
  if (PresumedLoc PLoc = SM.getPresumedLoc(DiagLoc); PLoc.isValid())
    return globsMatches(
        ...
        toolchain::sys::path::remove_leading_dotslash(PLoc.getFilename()));
  return false;
}

bool WarningsSpecialCaseList::globsMatches(
    const toolchain::StringMap<Matcher> &CategoriesToMatchers,
    StringRef FilePath) const {
  ...
  for (const auto &Entry : CategoriesToMatchers) {
    ...
    const toolchain::SpecialCaseList::Matcher &Matcher = Entry.getValue();
...
static void HandleOrdinalModifier(unsigned ValNo,
  ...
  toolchain::raw_svector_ostream Out(OutStr);

  // We could use text forms for the first N ordinals, but the numeric
  // forms are actually nicer in diagnostics because they stand out.
  Out << ValNo << toolchain::getOrdinalSuffix(ValNo);
}
...
static void HandleIntegerHumanModifier(int64_t ValNo,
  ...
  for (const auto &[UnitSize, UnitSign] : Units) {
    if (ValNo >= UnitSize) {
      Out << toolchain::format("%0.2f%c", ValNo / static_cast<double>(UnitSize),
                          UnitSign);
      return;
    }
  }
...
void language::Core::EscapeStringForDiagnostic(StringRef Str,
                                      SmallVectorImpl<char> &OutStr) {
  OutStr.reserve(OutStr.size() + Str.size());
  auto *Begin = reinterpret_cast<const unsigned char *>(Str.data());
  toolchain::raw_svector_ostream OutStream(OutStr);
  ...
  while (Begin != End) {
    ...
    if (toolchain::isLegalUTF8Sequence(Begin, End)) {
      toolchain::UTF32 CodepointValue;
      toolchain::UTF32 *CpPtr = &CodepointValue;
      const unsigned char *CodepointBegin = Begin;
      const unsigned char *CodepointEnd =
          Begin + toolchain::getNumBytesForUTF8(*Begin);
      toolchain::ConversionResult Res = toolchain::ConvertUTF8toUTF32(
          &Begin, CodepointEnd, &CpPtr, CpPtr + 1, toolchain::strictConversion);
      (void)Res;
      assert(
          toolchain::conversionOK == Res &&
          "the sequence is legal UTF-8 but we couldn't convert it to UTF-32");
      assert(Begin == CodepointEnd &&
             "we must be further along in the string now");
      if (toolchain::sys::unicode::isPrintable(CodepointValue) ||
          toolchain::sys::unicode::isFormatting(CodepointValue)) {
        OutStr.append(CodepointBegin, CodepointEnd);
        continue;
      }
      // Unprintable code point.
      OutStream << "<U+" << toolchain::format_hex_no_prefix(CodepointValue, 4, true)
                << ">";
      continue;
    }
    // Invalid code unit.
    OutStream << "<" << toolchain::format_hex_no_prefix(*Begin, 2, true) << ">";
    ++Begin;
  }
}
  ...
  while (DiagStr != DiagEnd) {
    ...
    case DiagnosticsEngine::ak_sint: {
      ...
      if (ModifierIs(Modifier, ModifierLen, "select")) {
        ...
        toolchain::raw_svector_ostream(OutStr) << Val;
      }
      break;
    }
    ...
    case DiagnosticsEngine::ak_identifierinfo: {
      ...
      toolchain::raw_svector_ostream(OutStr) << '\'' << II->getName() << '\'';
      break;
    }
...
toolchain::raw_ostream &language::Core::operator<<(toolchain::raw_ostream &OS,
...
DiagStorageAllocator::~DiagStorageAllocator() {
  ...
  assert((NumFreeListEntries == NumCached ||
          toolchain::CrashRecoveryContext::isRecoveringFromCrash()) &&
