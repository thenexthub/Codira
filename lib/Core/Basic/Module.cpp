...
static bool hasFeature(StringRef Feature, const LangOptions &LangOpts,
  ...
  bool HasFeature = toolchain::StringSwitch<bool>(Feature)
  ...
  if (!HasFeature)
    HasFeature = toolchain::is_contained(LangOpts.ModuleFeatures, Feature);
  return HasFeature;
}
...
bool Module::isUnimportable(const LangOptions &LangOpts,
  ...
  toolchain_unreachable("could not find a reason why module is unimportable");
}
...
bool Module::isAvailable(const LangOptions &LangOpts, const TargetInfo &Target,
  ...
  toolchain_unreachable("could not find a reason why module is unavailable");
}
...
ArrayRef<FileEntryRef> Module::getTopHeaders(FileManager &FileMgr) {
  ...
  return toolchain::ArrayRef(TopHeaders.begin(), TopHeaders.end());
}
...
void Module::print(raw_ostream &OS, unsigned Indent, bool Dump) const {
  ...
  if (Dump) {
    for (Module *M : Imports) {
      OS.indent(Indent + 2);
      toolchain::errs() << "import " << M->getFullModuleName() << "\n";
    }
  }
  ...
  std::function<void(Visiting)> VisitModule = [&](Visiting V) {
    ...
    for (auto &C : V.M->Conflicts) {
      if (isVisible(C.Other)) {
        toolchain::SmallVector<Module*, 8> Path;
