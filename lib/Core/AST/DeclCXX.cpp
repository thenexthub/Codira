...
static bool hasRepeatedBaseClass(const CXXRecordDecl *StartRD) {
  toolchain::SmallPtrSet<const CXXRecordDecl*, 8> SeenBaseTypes;
...
CXXRecordDecl::setBases(CXXBaseSpecifier const * const *Bases,
  ...
  toolchain::SmallPtrSet<CanQualType, 8> SeenVBaseTypes;
...
bool CXXRecordDecl::hasSubobjectAtOffsetZeroOfEmptyBaseType(
  ...
  toolchain::SmallPtrSet<const CXXRecordDecl*, 8> Bases;
  toolchain::SmallPtrSet<const CXXRecordDecl*, 8> M;
...
void CXXRecordDecl::addedMember(Decl *D) {
  ...
  if (const auto *Field = dyn_cast<FieldDecl>(D)) {
    ...
    case AS_none:       toolchain_unreachable("Invalid access specifier");
    };
...
void CXXRecordDecl::addedEligibleSpecialMemberFunction(const CXXMethodDecl *MD,
  ...
  // See https://github.com/toolchain/toolchain-project/issues/59206
...
static bool allLookupResultsAreTheSame(const DeclContext::lookup_result &R) {
  return toolchain::all_of(R, [&](NamedDecl *D) {
...
static NamedDecl* getLambdaCallOperatorHelper(const CXXRecordDecl &RD) {
  ...
  // https://github.com/toolchain/toolchain-project/issues/90154).
  ...
  toolchain_unreachable("Couldn't find our call operator!");
}
...
CXXRecordDecl::getLambdaExplicitTemplateParameters() const {
  ...
  const auto ExplicitEnd = toolchain::partition_point(
...
static void CollectVisibleConversions(
    ...
    const toolchain::SmallPtrSet<CanQualType, 8> &ParentHiddenTypes,
    ASTUnresolvedSet &Output, UnresolvedSetImpl &VOutput,
    toolchain::SmallPtrSet<NamedDecl *, 8> &HiddenVBaseCs) {
  // The set of types which have conversions in this class or its
  // subclasses.  As an optimization, we don't copy the derived set
  // unless it might change.
  const toolchain::SmallPtrSet<CanQualType, 8> *HiddenTypes = &ParentHiddenTypes;
  toolchain::SmallPtrSet<CanQualType, 8> HiddenTypesBuffer;
...
static void CollectVisibleConversions(ASTContext &Context,
  ...
  toolchain::SmallPtrSet<CanQualType, 8> HiddenTypes;
...
toolchain::iterator_range<CXXRecordDecl::conversion_iterator>
CXXRecordDecl::getVisibleConversionFunctions() const {
  ...
  return toolchain::make_range(Set->begin(), Set->end());
}
...
void CXXRecordDecl::removeConversion(const NamedDecl *ConvDecl) {
  ...
  for (unsigned I = 0, E = Convs.size(); I != E; ++I) {
    if (Convs[I].getDecl() == ConvDecl) {
      Convs.erase(I);
      assert(!toolchain::is_contained(Convs, ConvDecl) &&
             "conversion was found multiple times in unresolved set");
      return;
    }
  }

  toolchain_unreachable("conversion not found in set!");
}
...
CXXRecordDecl::setTemplateSpecializationKind(TemplateSpecializationKind TSK) {
  ...
  toolchain_unreachable("Not a class template or member class specialization");
}
...
CXXMethodDecl::getCorrespondingMethodInClass(const CXXRecordDecl *RD,
  ...
  toolchain::SmallVector<CXXMethodDecl*, 4> FinalOverriders;
  auto AddFinalOverrider = [&](CXXMethodDecl *D) {
    ...
    toolchain::erase_if(FinalOverriders, [&](CXXMethodDecl *OtherD) {
...
void BaseUsingDecl::addShadowDecl(UsingShadowDecl *S) {
  assert(!toolchain::is_contained(shadows(), S) && "declaration already in set");
...
void BaseUsingDecl::removeShadowDecl(UsingShadowDecl *S) {
  assert(toolchain::is_contained(shadows(), S) && "declaration not in set");
  assert(S->getIntroducer() == this);
...
VarDecl *ValueDecl::getPotentiallyDecomposedVarDecl() {
  ...
  if (auto *Var = toolchain::dyn_cast<VarDecl>(this))
    return Var;
  if (auto *BD = toolchain::dyn_cast<BindingDecl>(this))
    return toolchain::dyn_cast_if_present<VarDecl>(BD->getDecomposedDecl());
  return nullptr;
}
...
void DecompositionDecl::printName(toolchain::raw_ostream &OS,
...
void MSGuidDecl::printName(toolchain::raw_ostream &OS,
                           const PrintingPolicy &) const {
  OS << toolchain::format("GUID{%08" PRIx32 "-%04" PRIx16 "-%04" PRIx16 "-",
                     PartVal.Part1, PartVal.Part2, PartVal.Part3);
  unsigned I = 0;
  for (uint8_t Byte : PartVal.Part4And5) {
    OS << toolchain::format("%02" PRIx8, Byte);
    if (++I == 2)
      OS << '-';
  }
  OS << '}';
...
static bool isValidStructGUID(ASTContext &Ctx, QualType T) {
  ...
  // GUID APValue.
  using MatcherRef = toolchain::function_ref<bool(QualType)>;
...
void UnnamedGlobalConstantDecl::printName(toolchain::raw_ostream &OS,
                                          const PrintingPolicy &) const {
...
static const char *getAccessName(AccessSpecifier AS) {
  switch (AS) {
    case AS_none:
      toolchain_unreachable("Invalid access specifier!");
    case AS_public:
      return "public";
    case AS_private:
      return "private";
