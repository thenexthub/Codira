...
static bool RetValue(InterpState &S, CodePtr &Pt) {
  toolchain::report_fatal_error("Interpreter cannot return values");
}
...
// https://github.com/toolchain/toolchain-project/issues/102513
...
#undef GET_INTERP
    ...
    toolchain_unreachable("We didn't see an EndSpeculation op?");
  };
...
bool CheckConst(InterpState &S, CodePtr OpPC, const Pointer &Ptr) {
  ...
  if (toolchain::is_contained(S.InitializingBlocks, Ptr.block()))
    return true;
...
void diagnoseEnumValue(InterpState &S, CodePtr OpPC, const EnumDecl *ED,
  ...
  if (ED->getNumNegativeBits() &&
    ...
    S.CCEDiag(Loc, diag::note_constexpr_unscoped_enum_out_of_range)
        << toolchain::toString(Value, 10) << Min.getSExtValue() << Max.getSExtValue()
        << ED;
  } else if (!ED->getNumNegativeBits() && Max.ult(Value.getZExtValue())) {
    const SourceLocation &Loc = S.Current->getLocation(OpPC);
    S.CCEDiag(Loc, diag::note_constexpr_unscoped_enum_out_of_range)
        << toolchain::toString(Value, 10) << Min.getZExtValue() << Max.getZExtValue()
        << ED;
  }
}
...
bool arePotentiallyOverlappingStringLiterals(const Pointer &LHS,
  ...
  for (unsigned NullByte : toolchain::seq(ShorterCharWidth)) {
