...
void ByteCodeEmitter::compileFunc(const FunctionDecl *FuncDecl,
  ...
  if (const auto *MD = dyn_cast<CXXMethodDecl>(FuncDecl);
    ...
    assert(R);
    toolchain::DenseMap<const ValueDecl *, FieldDecl *> LC;
    FieldDecl *LTC;
  ...
  for (auto ParamOffset : toolchain::drop_begin(Func->ParamOffsets, Drop)) {
...
void ByteCodeEmitter::emitLabel(LabelTy Label) {
  ...
  if (auto It = LabelRelocs.find(Label); It != LabelRelocs.end()) {
    for (unsigned Reloc : It->second) {
      ...
      endian::write<int32_t, toolchain::endianness::native>(Location, Offset);
    }
...
template <typename T>
static void emit(Program &P, toolchain::SmallVectorImpl<std::byte> &Code,
...
template <typename T>
static void emitSerialized(toolchain::SmallVectorImpl<std::byte> &Code, const T &Val,
                           bool &Success) {
  size_t ValPos = Code.size();
...
template <>
void emit(Program &P, toolchain::SmallVectorImpl<std::byte> &Code,
