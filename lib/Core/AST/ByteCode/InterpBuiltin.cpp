...
static bool interp__builtin_strlen(InterpState &S, CodePtr OpPC,
  ...
  for (size_t I = StrPtr.getIndex();; ++I, ++Len) {
    ...
    case 4:
      ...
      break;
    default:
      toolchain_unreachable("Unsupported char size");
    }
    if (Val == 0)
      break;
  }
...
static bool interp__builtin_nan(InterpState &S, CodePtr OpPC,
  ...
  const toolchain::fltSemantics &TargetSemantics =
  ...
  if (S.getASTContext().getTargetInfo().isNan2008()) {
    if (Signaling)
      Result.copy(
          toolchain::APFloat::getSNaN(TargetSemantics, /*Negative=*/false, &Fill));
    else
      Result.copy(
          toolchain::APFloat::getQNaN(TargetSemantics, /*Negative=*/false, &Fill));
  } else {
...
static bool interp__builtin_fmin(InterpState &S, CodePtr OpPC,
  ...
  if (IsNumBuiltin)
    Result.copy(toolchain::minimumnum(LHS.getAPFloat(), RHS.getAPFloat()));
  else
...
static bool interp__builtin_fmax(InterpState &S, CodePtr OpPC,
  ...
  if (IsNumBuiltin)
    Result.copy(toolchain::maximumnum(LHS.getAPFloat(), RHS.getAPFloat()));
  else
...
static bool interp_floating_comparison(InterpState &S, CodePtr OpPC,
  ...
  pushInteger(
      ...
      [&] {
        ...
        default:
          toolchain_unreachable("Unexpected builtin ID: Should be a floating point "
                           "comparison function");
        }
      }(),
      Call->getType());
  ...
  if (BaseAlignment.getQuantity() >= Alignment) {
    ...
    CharUnits NewOffset =
        CharUnits::fromQuantity(BuiltinOp == Builtin::BI__builtin_align_down
                                    ? toolchain::alignDown(PtrOffset, Alignment64)
                                    : toolchain::alignTo(PtrOffset, Alignment64));
  ...
  INT_TYPE_SWITCH_NO_BOOL(ElemT, {
    ...
    for (unsigned I = 1; I != NumElems; ++I) {
      ...
      if (ID == Builtin::BI__builtin_reduce_add) {
        ...
        toolchain_unreachable("Unhandled vector reduce builtin");
      }
    }
...
static unsigned computeFullDescSize(const ASTContext &ASTCtx,
  ...
  toolchain_unreachable("Unhandled descriptor type");
  return 0;
}
...
static bool interp__builtin_elementwise_sat(InterpState &S, CodePtr OpPC,
  ...
  if (!Call->getArg(0)->getType()->isVectorType()) {
    ...
    if (BuiltinID == Builtin::BI__builtin_elementwise_add_sat) {
      ...
      toolchain_unreachable("Wrong builtin ID");
    }
...
bool InterpretOffsetOf(InterpState &S, CodePtr OpPC, const OffsetOfExpr *E,
  ...
  for (unsigned I = 0; I != N; ++I) {
    ...
    case OffsetOfNode::Identifier:
      toolchain_unreachable("Dependent OffsetOfExpr?");
    }
  }
