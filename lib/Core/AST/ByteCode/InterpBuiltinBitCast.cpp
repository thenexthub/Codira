...
using DataFunc =
    toolchain::function_ref<bool(const Pointer &P, PrimType Ty, Bits BitOffset,
...
#define BITCAST_TYPE_SWITCH(Expr, B)                                           \
  do {                                                                         \
    ...
    default:                                                                   \
      toolchain_unreachable("Unhandled bitcast type");                              \
    }                                                                          \
  } while (0)
...
static bool enumerateData(const Pointer &P, const Context &Ctx, Bits Offset,
  ...
  toolchain_unreachable("Unhandled data type");
}
...
bool language::Core::interp::readPointerToBuffer(const Context &Ctx,
  ...
  return enumeratePointerFields(
      ...
      [&](const Pointer &P, PrimType T, Bits BitOffset, Bits FullBitWidth,
        ...
        if (T == PT_Float) {
          ...
          Bits NumBits = Bits(
              toolchain::APFloatBase::getSizeInBits(F.getAPFloat().getSemantics()));
          ...
          // the padding bits.
          if (toolchain::sys::IsBigEndianHost)
...
bool language::Core::interp::DoBitCast(InterpState &S, CodePtr OpPC, const Pointer &Ptr,
...
bool language::Core::interp::DoBitCastPtr(InterpState &S, CodePtr OpPC,
  ...
  bool Success = enumeratePointerFields(
      ...
      [&](const Pointer &P, PrimType T, Bits BitOffset, Bits FullBitWidth,
        ...
        if (T == PT_Float) {
          ...
          Bits NumBits = Bits(toolchain::APFloatBase::getSizeInBits(Semantics));
          assert(NumBits.isFullByte());
          assert(NumBits.getQuantity() <= FullBitWidth.getQuantity());
...
//      backwards.
bool language::Core::interp::DoMemcpy(InterpState &S, CodePtr OpPC,
                             const Pointer &SrcPtr, const Pointer &DestPtr,
                             Bits Size) {
  assert(SrcPtr.isBlockPointer());
  assert(DestPtr.isBlockPointer());

  toolchain::SmallVector<PrimTypeVariant> Values;
