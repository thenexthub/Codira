...
void BitcastBuffer::markInitialized(Bits Offset, Bits Length) {
  ...
  if (End <= Offset) {
    ...
    // Insert sorted.
    auto It = toolchain::upper_bound(InitializedBits, Element);
    InitializedBits.insert(It, Element);
  }

#ifndef NDEBUG
  // Ensure ranges are sorted and non-overlapping.
  assert(toolchain::is_sorted(InitializedBits));
  ...
  void BitcastBuffer::dump(bool AsHex = true) const {
    ...
    for (unsigned I = 0; I != (FinalBitSize / 8); ++I) {
      ...
      if (AsHex) {
        ...
        toolchain::errs() << stream.str();
        LineLength += stream.str().size() + 1;
      } else {
        toolchain::errs() << std::bitset<8>((int)B).to_string();
        LineLength += 8 + 1;
        // toolchain::errs() << (int)B;
      }
      toolchain::errs() << ' ';
    }
    toolchain::errs() << '\n';
