...
inline static std::string printArg(Program &P, CodePtr &OpPC) {
  if constexpr (std::is_pointer_v<T>) {
    ...
    std::string Result;
    toolchain::raw_string_ostream SS(Result);
...
template <> inline std::string printArg<Floating>(Program &P, CodePtr &OpPC) {
  ...
  unsigned BitWidth = toolchain::APFloatBase::semanticsSizeInBits(
      toolchain::APFloatBase::EnumToSemantics(Sem));
  auto Memory =
      std::make_unique<uint64_t[]>(toolchain::APInt::getNumWords(BitWidth));
...
LLVM_DUMP_METHOD void Function::dump() const { dump(toolchain::errs()); }

LLVM_DUMP_METHOD void Function::dump(toolchain::raw_ostream &OS) const {
  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::BRIGHT_GREEN, true});
  ...
  struct OpText {
    ...
    std::string Op;
    bool IsJump;
    toolchain::SmallVector<std::string> Args;
  };
  ...
  toolchain::SmallVector<OpText> Code;
...
#undef GET_DISASM
  ...
  toolchain::SmallVector<JmpData> Jumps;
  for (auto &Text : Code) {
    if (Text.IsJump)
  ...
  toolchain::SmallVector<std::string> Text;
  Text.reserve(Code.size());
  size_t LongestLine = 0;
  // Print code to a string, one at a time.
  for (auto C : Code) {
    std::string Line;
    toolchain::raw_string_ostream LS(Line);
    LS << C.Addr;
...
LLVM_DUMP_METHOD void Program::dump() const { dump(toolchain::errs()); }
...
static const char *primTypeToString(PrimType T) {
  ...
  toolchain_unreachable("Unhandled PrimType");
}

LLVM_DUMP_METHOD void Program::dump(toolchain::raw_ostream &OS) const {
  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::BRIGHT_RED, true});
    OS << "\n:: Program\n";
  }

  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::WHITE, true});
  ...
  for (const Global *G : Globals) {
    ...
    OS << GI << ": " << (const void *)G->block() << " ";
    {
      ColorScope SC(OS, true,
                    GP.isInitialized()
                        ? TerminalColor{toolchain::raw_ostream::GREEN, false}
                        : TerminalColor{toolchain::raw_ostream::RED, false});
    ...
    if (GP.isInitialized() && Desc->IsTemporary) {
      if (const auto *MTE =
        ...
        if (const APValue *V =
          ...
          {
            ColorScope SC(OS, true, {toolchain::raw_ostream::BRIGHT_MAGENTA, true});
            std::string VStr;
            toolchain::raw_string_ostream SS(VStr);
    ...
    if (GP.isInitialized() && Desc->isPrimitive() && !G->block()->isDummy()) {
      OS << "   ";
      {
        ColorScope SC(OS, true, {toolchain::raw_ostream::BRIGHT_CYAN, false});
...
LLVM_DUMP_METHOD void Descriptor::dump(toolchain::raw_ostream &OS) const {
  // Source
  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::BLUE, true});
...
LLVM_DUMP_METHOD void InlineDescriptor::dump(toolchain::raw_ostream &OS) const {
  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::BLUE, true});
...
LLVM_DUMP_METHOD void InterpFrame::dump(toolchain::raw_ostream &OS,
                                        unsigned Indent) const {
  unsigned Spaces = Indent * 2;
  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::BLUE, true});
...
LLVM_DUMP_METHOD void Record::dump(toolchain::raw_ostream &OS, unsigned Indentation,
...
LLVM_DUMP_METHOD void Block::dump(toolchain::raw_ostream &OS) const {
  {
    ColorScope SC(OS, true, {toolchain::raw_ostream::BRIGHT_BLUE, true});
