...
#ifndef LANGUAGE_CORE_AST_INTERP_BYTECODEEXPRGEN_H
#define LANGUAGE_CORE_AST_INTERP_BYTECODEEXPRGEN_H
...
protected:
  ...
  using CaseMap = toolchain::DenseMap<const SwitchCase *, LabelTy>;
...
private:
  ...
  bool emitConst(const toolchain::APSInt &Value, PrimType Ty, const Expr *E);
  bool emitConst(const toolchain::APSInt &Value, const Expr *E);
  bool emitConst(const toolchain::APInt &Value, const Expr *E) {
    return emitConst(static_cast<toolchain::APSInt>(Value), E);
  }
  ...
  toolchain::RoundingMode getRoundingMode(const Expr *E) const {
    FPOptions FPO = E->getFPFeaturesInEffect(Ctx.getLangOpts());

    if (FPO.getRoundingMode() == toolchain::RoundingMode::Dynamic)
...
protected:
  ...
  toolchain::DenseMap<const ValueDecl *, Scope::Local> Locals;

  /// OpaqueValueExpr to location mapping.
  toolchain::DenseMap<const OpaqueValueExpr *, unsigned> OpaqueExprs;
  ...
  toolchain::SmallVector<InitLink> InitStack;
...
public:
  ...
  virtual void addLocal(const Scope::Local &Local) {
    toolchain_unreachable("Shouldn't be called");
  }
...
public:
  ...
  bool emitDestructors(const Expr *E = nullptr) override {
    ...
    for (Scope::Local &Local : toolchain::reverse(this->Ctx->Descriptors[*Idx])) {
  ...
  void removeIfStoredOpaqueValue(const Scope::Local &Local) {
    if (const auto *OVE =
            toolchain::dyn_cast_if_present<OpaqueValueExpr>(Local.Desc->asExpr())) {
