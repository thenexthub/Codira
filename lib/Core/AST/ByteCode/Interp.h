...
#ifndef LANGUAGE_CORE_AST_INTERP_INTERP_H
#define LANGUAGE_CORE_AST_INTERP_INTERP_H
...
using FixedPointSemantics = toolchain::FixedPointSemantics;
...
static inline toolchain::RoundingMode getRoundingMode(FPOptions FPO) {
...
bool IncDecFloatHelper(InterpState &S, CodePtr OpPC, const Pointer &Ptr,
  ...
  FPOptions FPO = FPOptions::getFromOpaqueInt(FPOI);
  toolchain::APFloat::opStatus Status;
  if constexpr (Op == IncDecOp::Inc)
...
using CompareFn = toolchain::function_ref<bool(ComparisonCategoryResult)>;
...
inline bool CastFP(InterpState &S, CodePtr OpPC, const toolchain::fltSemantics *Sem,
...
bool CastIntegralFloating(InterpState &S, CodePtr OpPC,
                          const toolchain::fltSemantics *Sem, uint32_t FPOI) {
...
inline bool DoShiftAP(InterpState &S, CodePtr OpPC, const APSInt &LHS,
  ...
  if (S.getLangOpts().OpenCL)
    RHS &=
        APSInt(toolchain::APInt(RHS.getBitWidth(), static_cast<uint64_t>(Bits - 1)),
               RHS.isUnsigned());
...
static inline bool ShiftFixedPoint(InterpState &S, CodePtr OpPC, bool Left) {
  ...
  toolchain::FixedPointSemantics LHSSema = LHS.getSemantics();
...
inline bool OffsetOf(InterpState &S, CodePtr OpPC, const OffsetOfExpr *E) {
  toolchain::SmallVector<int64_t> ArrayIndices;
...
inline bool BitCastPrim(InterpState &S, CodePtr OpPC, bool TargetIsUCharOrByte,
  ...
  if constexpr (std::is_same_v<T, Pointer>) {
    ...
    toolchain::SmallVector<std::byte> Buff(BuffSize);
    ...
    if constexpr (std::is_same_v<T, Floating>) {
      assert(Sem);
      BitWidth = Bits(toolchain::APFloatBase::getSizeInBits(*Sem));
    }
...
template <> inline Floating ReadArg<Floating>(InterpState &S, CodePtr &OpPC) {
  auto &Semantics =
      toolchain::APFloatBase::EnumToSemantics(Floating::deserializeSemantics(*OpPC));
