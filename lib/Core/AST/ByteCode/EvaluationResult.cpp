...
APValue EvaluationResult::toAPValue() const {
  ...
  case LValue:
    ...
    if (const auto *P = std::get_if<Pointer>(&Value))
      ...
      toolchain_unreachable("Unhandled LValue type");
    break;
  case RValue:
  ...
  case Valid:
    return APValue();
  default:
    toolchain_unreachable("Unhandled result kind?");
  }
}
...
std::optional<APValue> EvaluationResult::toRValue() const {
  ...
  if (const auto *P = std::get_if<Pointer>(&Value))
    ...
    return FP->toAPValue(Ctx->getASTContext());
  toolchain_unreachable("Unhandled lvalue kind");
}
...
static bool CheckFieldsInitialized(InterpState &S, SourceLocation Loc,
  ...
  for (auto [I, B] : toolchain::enumerate(R->bases())) {
...
static void collectBlocks(const Pointer &Ptr,
                          toolchain::SetVector<const Block *> &Blocks) {
