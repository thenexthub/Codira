...
#ifndef LANGUAGE_CORE_AST_INTERP_FUNCTION_H
#define LANGUAGE_CORE_AST_INTERP_FUNCTION_H
...
public:
  ...
  using LocalVectorTy = toolchain::SmallVector<Local, 8>;

  Scope(LocalVectorTy &&Descriptors) : Descriptors(std::move(Descriptors)) {}

  toolchain::iterator_range<LocalVectorTy::const_iterator> locals() const {
    return toolchain::make_range(Descriptors.begin(), Descriptors.end());
  }

  toolchain::iterator_range<LocalVectorTy::const_reverse_iterator>
  locals_reverse() const {
    return toolchain::reverse(Descriptors);
  }
...
using FunctionDeclTy =
    toolchain::PointerUnion<const FunctionDecl *, const BlockExpr *>;
...
public:
  ...
  toolchain::iterator_range<toolchain::SmallVector<Scope, 2>::const_iterator>
  scopes() const {
    return toolchain::make_range(Scopes.begin(), Scopes.end());
  }

  /// Range over argument types.
  using arg_reverse_iterator =
      SmallVectorImpl<PrimType>::const_reverse_iterator;
  toolchain::iterator_range<arg_reverse_iterator> args_reverse() const {
    return toolchain::reverse(ParamTypes);
  }
...
private:
  ...
  Function(Program &P, FunctionDeclTy Source, unsigned ArgSize,
           ...
           toolchain::SmallVectorImpl<unsigned> &&ParamOffsets, bool HasThisPointer,
           bool HasRVO, bool IsLambdaStaticInvoker);

  /// Sets the code of a function.
  void setCode(unsigned NewFrameSize, toolchain::SmallVector<std::byte> &&NewCode,
               SourceMap &&NewSrcMap, toolchain::SmallVector<Scope, 2> &&NewScopes,
               bool NewHasBody) {
...
private:
  ...
  toolchain::SmallVector<Scope, 2> Scopes;
  /// List of argument types.
  toolchain::SmallVector<PrimType, 8> ParamTypes;
  /// Map from byte offset to parameter descriptor.
  toolchain::DenseMap<unsigned, ParamDescriptor> Params;
  /// List of parameter offsets.
  toolchain::SmallVector<unsigned, 8> ParamOffsets;
