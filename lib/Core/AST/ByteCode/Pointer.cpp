...
APValue Pointer::toAPValue(const ASTContext &ASTCtx) const {
  toolchain::SmallVector<APValue::LValuePathEntry, 5> Path;
  ...
  if (const auto *VD = Desc->asValueDecl())
    ...
    toolchain_unreachable("Invalid allocation type");
  ...
  while (Ptr.isField() || Ptr.isArrayElement()) {
    ...
    if (Ptr.isArrayRoot()) {
      ...
      toolchain_unreachable("Invalid field type");
    }
  }
...
void Pointer::print(toolchain::raw_ostream &OS) const {
  switch (StorageKind) {
  case Storage::Block: {
...
Pointer::computeSplitPoint(const Pointer &A, const Pointer &B) {
  ...
  toolchain_unreachable("The loop above should've returned.");
}
...
std::optional<APValue> Pointer::toRValue(const Context &Ctx,
  ...
  Composite = [&Composite, &Ctx, &ASTCtx](QualType Ty, const Pointer &Ptr,
    ...
    toolchain_unreachable("invalid value to return");
  };
