...
#ifndef LANGUAGE_CORE_AST_INTERP_INTEGRAL_H
#define LANGUAGE_CORE_AST_INTERP_INTEGRAL_H
...
public:
  ...
  unsigned countLeadingZeros() const {
    if constexpr (!Signed)
      return toolchain::countl_zero<ReprT>(V);
    if (isPositive())
      return toolchain::countl_zero<typename AsUnsigned::ReprT>(
          static_cast<typename AsUnsigned::ReprT>(V));
    toolchain_unreachable("Don't call countLeadingZeros() on negative values.");
  }
...
private:
  template <typename T> static bool CheckAddUB(T A, T B, T &R) {
    if constexpr (std::is_signed_v<T>) {
      return toolchain::AddOverflow<T>(A, B, R);
    } else {
      R = A + B;
      return false;
    }
  ...
  template <typename T> static bool CheckSubUB(T A, T B, T &R) {
    if constexpr (std::is_signed_v<T>) {
      return toolchain::SubOverflow<T>(A, B, R);
    } else {
      R = A - B;
      return false;
    }
  ...
  template <typename T> static bool CheckMulUB(T A, T B, T &R) {
    if constexpr (std::is_signed_v<T>) {
      return toolchain::MulOverflow<T>(A, B, R);
    } else {
      R = A * B;
      return false;
    }
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, Integral<Bits, Signed> I) {
  I.print(OS);
  return OS;
}
