...
bool InitLink::emit(Compiler<Emitter> *Ctx, const Expr *E) const {
  ...
  case K_InitList:
    return true;
  default:
    toolchain_unreachable("Unhandled InitLink kind");
  }
  return true;
}
...
bool Compiler<Emitter>::VisitBinaryOperator(const BinaryOperator *BO) {
  ...
  case BO_LOr:
  case BO_LAnd:
    toolchain_unreachable("Already handled earlier");
  default:
    return false;
  }

  toolchain_unreachable("Unhandled binary op");
}
...
bool Compiler<Emitter>::VisitFixedPointBinOp(const BinaryOperator *E) {
  ...
  toolchain_unreachable("unhandled binop opcode");
}
...
bool Compiler<Emitter>::VisitFixedPointUnaryOperator(const UnaryOperator *E) {
  ...
  toolchain_unreachable("Unhandled unary opcode");
}
...
bool Compiler<Emitter>::VisitCompoundAssignOperator(
  ...
  case BO_OrAssign:
    ...
    break;
  default:
    toolchain_unreachable("Unimplemented compound assign operator");
  }
...
bool Compiler<Emitter>::visitZeroInitializer(PrimType T, QualType QT,
  ...
  toolchain_unreachable("unknown primitive type");
}
...
bool Compiler<Emitter>::emitConst(T Value, PrimType Ty, const Expr *E) {
  ...
  case PT_IntAPS:
  case PT_FixedPoint:
    toolchain_unreachable("Invalid integral type");
    break;
  }
  toolchain_unreachable("unknown primitive type");
}
...
bool Compiler<Emitter>::compileDestructor(const CXXDestructorDecl *Dtor) {
  ...
  if (!R->isUnion()) {
    ...
    for (const Record::Field &Field : toolchain::reverse(R->fields())) {
      const Descriptor *D = Field.Desc;
  ...
  for (const Record::Base &Base : toolchain::reverse(R->bases())) {
...
bool Compiler<Emitter>::VisitVectorUnaryOperator(const UnaryOperator *E) {
  ...
  case UO_Not: // ~x
    ...
    break;
  default:
    toolchain_unreachable("Unsupported unary operators should be handled up front");
  }
  return true;
}
...
bool Compiler<Emitter>::emitPrimCast(PrimType FromT, PrimType ToT,
  ...
  if (FromT == PT_Float) {
    ...
    if (ToT == PT_Float) {
      const toolchain::fltSemantics *ToSem = &Ctx.getFloatSemantics(ToQT);
...
bool Compiler<Emitter>::emitFloat(const APFloat &F, const Expr *E) {
  ...
  return this->emitConstFloat(
      Floating(const_cast<uint64_t *>(I.getRawData()),
               toolchain::APFloatBase::SemanticsToEnum(F.getSemantics())),
      E);
}
...
bool Compiler<Emitter>::emitBuiltinBitCast(const CastExpr *E) {
  ...
  const toolchain::fltSemantics *TargetSemantics = nullptr;
  if (ToT == PT_Float)
