...
static void printIntegral(const TemplateArgument &TemplArg, raw_ostream &Out,
  ...
  const toolchain::APSInt &Val = TemplArg.getAsIntegral();
  ...
  if (Policy.UseEnumerators) {
    if (const EnumType *ET = T->getAs<EnumType>()) {
      for (const EnumConstantDecl *ECD : ET->getOriginalDecl()->enumerators()) {
        ...
        if (toolchain::APSInt::isSameValue(ECD->getInitVal(), Val)) {
...
TemplateArgument::TemplateArgument(const ASTContext &Ctx,
                                   const toolchain::APSInt &Value, QualType Type,
                                   bool IsDefaulted) {
...
TemplateArgumentDependence TemplateArgument::getDependence() const {
  ...
  switch (getKind()) {
  case Null:
    toolchain_unreachable("Should not have a NULL template argument");
  ...
  toolchain_unreachable("unhandled ArgKind");
}
...
void TemplateArgument::Profile(toolchain::FoldingSetNodeID &ID,
...
bool TemplateArgument::structurallyEquals(const TemplateArgument &Other) const {
  ...
  case StructuralValue: {
    ...
    toolchain::FoldingSetNodeID A, B;
...
language::Core::TemplateArgumentLocInfo::TemplateArgumentLocInfo(
