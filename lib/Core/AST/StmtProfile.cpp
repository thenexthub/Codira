...
namespace {
  ...
  public:
    StmtProfiler(toolchain::FoldingSetNodeID &ID, bool Canonical,
                 bool ProfileLambdaExpr)
...
#include "language/Core/AST/StmtNodes.inc"
  ...
  public:
    StmtProfilerWithPointers(toolchain::FoldingSetNodeID &ID,
  ...
  class StmtProfilerWithoutPointers : public StmtProfiler {
    ODRHash &Hash;
  public:
    StmtProfilerWithoutPointers(toolchain::FoldingSetNodeID &ID, ODRHash &Hash)
...
void StmtProfiler::VisitPredefinedExpr(const PredefinedExpr *S) {
  VisitExpr(S);
  ID.AddInteger(toolchain::to_underlying(S->getIdentKind()));
}
...
void StmtProfiler::VisitCharacterLiteral(const CharacterLiteral *S) {
  VisitExpr(S);
  ID.AddInteger(toolchain::to_underlying(S->getKind()));
...
static Stmt::StmtClass DecodeOperatorCall(const CXXOperatorCallExpr *S,
  ...
  case NUM_OVERLOADED_OPERATORS:
    toolchain_unreachable("Invalid operator call kind");
  ...
  toolchain_unreachable("Invalid overloaded operator expression");
}
...
void StmtProfiler::VisitCXXNewExpr(const CXXNewExpr *S) {
  ...
  ID.AddInteger(toolchain::to_underlying(S->getInitializationStyle()));
}
...
void Stmt::Profile(toolchain::FoldingSetNodeID &ID, const ASTContext &Context,
                   bool Canonical, bool ProfileLambdaExpr) const {
  StmtProfilerWithPointers Profiler(ID, Context, Canonical, ProfileLambdaExpr);
  Profiler.Visit(this);
}

void Stmt::ProcessODRHash(toolchain::FoldingSetNodeID &ID,
