...
#include "toolchain/Frontend/OpenMP/OMP.inc"
  ...
  toolchain_unreachable("unknown OMPClause");
}
...
void OMPFirstprivateClause::setInits(ArrayRef<Expr *> VL) {
  ...
  toolchain::copy(VL, getPrivateCopies().end());
}
...
void OMPLastprivateClause::setPrivateCopies(ArrayRef<Expr *> PrivateCopies) {
  assert(PrivateCopies.size() == varlist_size() &&
         "Number of private copies is not the same as the preallocated buffer");
  toolchain::copy(PrivateCopies, varlist_end());
}
...
void OMPLastprivateClause::setSourceExprs(ArrayRef<Expr *> SrcExprs) {
  assert(SrcExprs.size() == varlist_size() && "Number of source expressions is "
                                              "not the same as the "
                                              "preallocated buffer");
  toolchain::copy(SrcExprs, getPrivateCopies().end());
}
...
void OMPLastprivateClause::setDestinationExprs(ArrayRef<Expr *> DstExprs) {
  ...
  toolchain::copy(DstExprs, getSourceExprs().end());
}
...
void OMPLastprivateClause::setAssignmentOps(ArrayRef<Expr *> AssignmentOps) {
  assert(AssignmentOps.size() == varlist_size() &&
         ...
         "buffer");
  toolchain::copy(AssignmentOps, getDestinationExprs().end());
}
...
void OMPLinearClause::setPrivates(ArrayRef<Expr *> PL) {
  ...
  toolchain::copy(PL, varlist_end());
}

void OMPLinearClause::setInits(ArrayRef<Expr *> IL) {
  assert(IL.size() == varlist_size() &&
         "Number of inits is not the same as the preallocated buffer");
  toolchain::copy(IL, getPrivates().end());
}

void OMPLinearClause::setUpdates(ArrayRef<Expr *> UL) {
  assert(UL.size() == varlist_size() &&
         "Number of updates is not the same as the preallocated buffer");
  toolchain::copy(UL, getInits().end());
}

void OMPLinearClause::setFinals(ArrayRef<Expr *> FL) {
  assert(FL.size() == varlist_size() &&
         "Number of final updates is not the same as the preallocated buffer");
  toolchain::copy(FL, getUpdates().end());
}
...
void OMPLinearClause::setUsedExprs(ArrayRef<Expr *> UE) {
  ...
  toolchain::copy(UE, getFinals().end() + 2);
}
...
OMPClause::child_range OMPLinearClause::used_children() {
  ...
  return child_range(
      ...
      reinterpret_cast<Stmt **>(toolchain::find(getUsedExprs(), nullptr)));
}
...
void OMPCopyinClause::setSourceExprs(ArrayRef<Expr *> SrcExprs) {
  assert(SrcExprs.size() == varlist_size() && "Number of source expressions is "
                                              "not the same as the "
                                              "preallocated buffer");
  toolchain::copy(SrcExprs, varlist_end());
}
...
void OMPReductionClause::setPrivates(ArrayRef<Expr *> Privates) {
  assert(Privates.size() == varlist_size() &&
         "Number of private copies is not the same as the preallocated buffer");
  toolchain::copy(Privates, varlist_end());
}
...
void OMPReductionClause::setLHSExprs(ArrayRef<Expr *> LHSExprs) {
  ...
  toolchain::copy(LHSExprs, getPrivates().end());
}
...
void OMPReductionClause::setRHSExprs(ArrayRef<Expr *> RHSExprs) {
  ...
  toolchain::copy(RHSExprs, getLHSExprs().end());
}
...
void OMPReductionClause::setReductionOps(ArrayRef<Expr *> ReductionOps) {
  ...
  toolchain::copy(ReductionOps, getRHSExprs().end());
}
...
void OMPReductionClause::setInscanCopyOps(ArrayRef<Expr *> Ops) {
  ...
  assert(Ops.size() == varlist_size() && "Number of copy "
                                         "expressions is not the same "
                                         "as the preallocated buffer");
  toolchain::copy(Ops, getReductionOps().end());
}
...
void OMPReductionClause::setInscanCopyArrayTemps(
  ...
  assert(CopyArrayTemps.size() == varlist_size() &&
         ...
         "buffer");
  toolchain::copy(CopyArrayTemps, getInscanCopyOps().end());
}
...
void OMPReductionClause::setInscanCopyArrayElems(
  ...
  assert(CopyArrayElems.size() == varlist_size() &&
         "Number of copy temp expressions is not the same as the preallocated "
         "buffer");
  toolchain::copy(CopyArrayElems, getInscanCopyArrayTemps().end());
}
...
void OMPInReductionClause::setTaskgroupDescriptors(
  ...
  assert(TaskgroupDescriptors.size() == varlist_size() &&
         ...
         "preallocated buffer");
  toolchain::copy(TaskgroupDescriptors, getReductionOps().end());
}
...
unsigned OMPClauseMappableExprCommon::getUniqueDeclarationsTotalNumber(
  ...
  toolchain::SmallPtrSet<const ValueDecl *, 8> UniqueDecls;
...
OMPInitClause *OMPInitClause::Create(const ASTContext &C, Expr *InteropVar,
  ...
  toolchain::copy(InteropInfo.PreferTypes, Clause->getTrailingObjects() + 1);
  return Clause;
}
...
void OMPClausePrinter::VisitOMPPermutationClause(OMPPermutationClause *Node) {
  OS << "permutation(";
  toolchain::interleaveComma(Node->getArgsRefs(), OS, [&](const Expr *E) {
...
void OMPClausePrinter::VisitOMPDoacrossClause(OMPDoacrossClause *Node) {
  ...
  case OMPC_DOACROSS_sink_omp_cur_iteration:
    ...
    break;
  default:
    toolchain_unreachable("unknown docaross modifier");
  }
  VisitOMPClauseList(Node, ' ');
  OS << ")";
}
...
void OMPTraitInfo::getAsVariantMatchInfo(ASTContext &ASTCtx,
  ...
  for (const OMPTraitSet &Set : Sets) {
    for (const OMPTraitSelector &Selector : Set.Selectors) {
      ...
      std::optional<toolchain::APSInt> Score;
      toolchain::APInt *ScorePtr = nullptr;
...
void OMPTraitInfo::print(toolchain::raw_ostream &OS,
...
std::string OMPTraitInfo::getMangledName() const {
  ...
  toolchain::raw_string_ostream OS(MangledName);
  for (const OMPTraitSet &Set : Sets) {
...
TargetOMPContext::TargetOMPContext(
    ...
    ArrayRef<toolchain::omp::TraitProperty> ConstructTraits, int DeviceNum)
  ...
  for (toolchain::omp::TraitProperty Property : ConstructTraits)
    addTrait(Property);
}
