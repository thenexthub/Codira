...
unsigned TypeLoc::getFullDataSizeForType(QualType Ty) {
  ...
  while (!TyLoc.isNull()) {
    ...
    Total = toolchain::alignTo(Total, Align);
    Total += TypeSizer().Visit(TyLoc);
    TyLoc = TyLoc.getNextTypeLoc();
  }
  Total = toolchain::alignTo(Total, MaxAlign);
  return Total;
}
...
void TypeLoc::copy(TypeLoc other) {
  ...
  if (reinterpret_cast<uintptr_t>(Data) ==
          toolchain::alignTo(reinterpret_cast<uintptr_t>(Data),
                        TypeLocMaxDataAlign) &&
      reinterpret_cast<uintptr_t>(other.Data) ==
          toolchain::alignTo(reinterpret_cast<uintptr_t>(other.Data),
                        TypeLocMaxDataAlign)) {
...
TypeSpecifierType BuiltinTypeLoc::getWrittenTypeSpec() const {
  ...
  case BuiltinType::BFloat16:
    toolchain_unreachable("Builtin type needs extra local data!");
...
void TemplateSpecializationTypeLoc::initializeArgLocs(
  ...
  for (unsigned i = 0, e = Args.size(); i != e; ++i) {
    ...
    case TemplateArgument::Null:
      toolchain_unreachable("Impossible TemplateArgument");
...
static ConceptReference *createTrivialConceptReference(ASTContext &Context,
  ...
  toolchain::SmallVector<TemplateArgumentLocInfo, 8> TALI(size);
  TemplateSpecializationTypeLoc::initializeArgLocs(
