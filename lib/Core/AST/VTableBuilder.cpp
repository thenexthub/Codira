...
private:
  ...
  typedef toolchain::DenseMap<MethodBaseOffsetPairTy,
  ...
  typedef toolchain::DenseMap<std::pair<const CXXRecordDecl *, unsigned>,
                         CharUnits> SubobjectOffsetMapTy;

  typedef toolchain::DenseMap<const CXXRecordDecl *, unsigned> SubobjectCountMapTy;
...
public:
  ...
  void dump() {
    VisitedVirtualBasesSetTy VisitedVirtualBases;
    dump(toolchain::errs(), BaseSubobject(MostDerivedClass, CharUnits::Zero()),
         VisitedVirtualBases);
  }
...
ComputeReturnAdjustmentBaseOffset(ASTContext &Context,
  ...
  if (isa<ReferenceType>(CanDerivedReturnType)) {
    ...
    toolchain_unreachable("Unexpected return type!");
  }
...
CharUnits VCallOffsetMap::getVCallOffsetOffset(const CXXMethodDecl *MD) {
  ...
  toolchain_unreachable("Should always find a vcall offset offset!");
}

/// VCallAndVBaseOffsetBuilder - Class for building vcall and vbase offsets.
class VCallAndVBaseOffsetBuilder {
public:
  typedef toolchain::DenseMap<const CXXRecordDecl *, CharUnits>
...
private:
  ...
  toolchain::SmallPtrSet<const CXXRecordDecl *, 4> VisitedVirtualBases;
...
public:
  ...
  /// primary bases.
  typedef toolchain::SmallSetVector<const CXXRecordDecl *, 8>
  ...
  typedef toolchain::DenseMap<GlobalDecl, int64_t> MethodVTableIndicesTy;
...
private:
  ...
  toolchain::DenseMap<const CXXRecordDecl *, VCallOffsetMap> VCallOffsetsForVBases;
  ...
  typedef toolchain::DenseMap<const CXXMethodDecl *, MethodInfo> MethodInfoMapTy;
  ...
  typedef toolchain::DenseMap<uint64_t, ThunkInfo> VTableThunksMapTy;
  ...
  typedef toolchain::DenseMap<const CXXMethodDecl *, ThunkInfoVectorTy> ThunksMapTy;
...
void ItaniumVTableBuilder::AddThunk(const CXXMethodDecl *MD,
  ...
  if (toolchain::is_contained(ThunksVector, Thunk))
    return;

  ThunksVector.push_back(Thunk);
}

typedef toolchain::SmallPtrSet<const CXXMethodDecl *, 8> OverriddenMethodsSetTy;
...
bool ItaniumVTableBuilder::IsOverriderUsed(
  ...
  while (true) {
    ...
    if (!PrimaryBases.insert(PrimaryBase))
      toolchain_unreachable("Found a duplicate primary base!");
...
typedef toolchain::SmallSetVector<const CXXRecordDecl *, 8> BasesSetVectorTy;
...
FindNearestOverriddenMethod(const CXXMethodDecl *MD,
  ...
  for (const CXXRecordDecl *PrimaryBase : toolchain::reverse(Bases)) {
...
void ItaniumVTableBuilder::AddMethods(
  ...
  typedef toolchain::SmallVector<const CXXMethodDecl *, 8> NewVirtualFunctionsTy;
  NewVirtualFunctionsTy NewVirtualFunctions;

  toolchain::SmallVector<const CXXMethodDecl*, 4> NewImplicitVirtualFunctions;
...
void ItaniumVTableBuilder::dumpLayout(raw_ostream &Out) {
  ...
  for (unsigned I = 0, E = Components.size(); I != E; ++I) {
    ...
    if (unsigned Count = AddressPointsByIndex.count(NextIndex)) {
      if (Count == 1) {
        ...
        for (const auto &I :
             toolchain::make_range(AddressPointsByIndex.equal_range(NextIndex))) {
  ...
  if (!Thunks.empty()) {
    ...
    for (const auto &I : MethodNamesAndDecls) {
      ...
      toolchain::sort(ThunksVector, [](const ThunkInfo &LHS, const ThunkInfo &RHS) {
  ...
  if (!IndicesMap.empty()) {
    ...
    for (const auto &I : IndicesMap) {
      ...
      Out << toolchain::format("%4" PRIu64 " | ", VTableIndex) << MethodName
          << '\n';
    }
  }
...
VTableLayout::VTableLayout(ArrayRef<size_t> VTableIndices,
  ...
  toolchain::sort(this->VTableThunks, [](const VTableLayout::VTableThunkTy &LHS,
...
public:
  typedef toolchain::DenseMap<GlobalDecl, MethodVFTableLocation>
    MethodVFTableLocationsTy;

  typedef toolchain::iterator_range<MethodVFTableLocationsTy::const_iterator>
...
static void GroupNewVirtualOverloads(
  ...
  typedef toolchain::DenseMap<DeclarationName, unsigned> VisitedGroupIndicesTy;
...
static void PrintBasePath(const VPtrInfo::BasePath &Path, raw_ostream &Out) {
  for (const CXXRecordDecl *Elem : toolchain::reverse(Path)) {
    Out << "'";
...
void VFTableBuilder::dumpLayout(raw_ostream &Out) {
  ...
  if (!Thunks.empty()) {
    ...
    for (const auto &MethodNameAndDecl : MethodNamesAndDecls) {
      ...
      ThunkInfoVectorTy ThunksVector = Thunks[MD];
      toolchain::stable_sort(ThunksVector, [](const ThunkInfo &LHS,
                                         const ThunkInfo &RHS) {
...
static bool setsIntersect(const toolchain::SmallPtrSet<const CXXRecordDecl *, 4> &A,
...
void MicrosoftVTableContext::computeVTablePaths(bool ForVBTables,
  ...
  toolchain::SmallPtrSet<const CXXRecordDecl*, 4> VBasesSeen;
  for (const auto &B : RD->bases()) {
...
static bool rebucketPaths(VPtrInfoVector &Paths) {
  ...
  toolchain::SmallVector<std::reference_wrapper<VPtrInfo>, 2> PathsSorted(
      toolchain::make_pointee_range(Paths));
  toolchain::sort(PathsSorted, [](const VPtrInfo &LHS, const VPtrInfo &RHS) {
...
namespace {
typedef toolchain::SetVector<BaseSubobject, std::vector<BaseSubobject>,
                        toolchain::DenseSet<BaseSubobject>> FullPathTy;
}
  ...
  FullPaths.remove_if([&](const FullPathTy &SpecificPath) {
    for (const FullPathTy &OtherPath : FullPaths) {
      if (&SpecificPath == &OtherPath)
        continue;
      if (toolchain::all_of(SpecificPath, [&](const BaseSubobject &BSO) {
...
static const FullPathTy *selectBestPath(ASTContext &Context,
  ...
  for (const FullPathTy &SpecificPath : FullPaths) {
    ...
    for (const CXXMethodDecl *MD : Info.IntroducingObject->methods()) {
      ...
      if (toolchain::none_of(SpecificPath, [&](const BaseSubobject &BSO) {
    ...
    OverriderSetTy NewOverrides =
        toolchain::set_difference(CurrentOverrides, LastOverrides);
    if (NewOverrides.empty())
      continue;
    OverriderSetTy MissingOverrides =
        toolchain::set_difference(LastOverrides, CurrentOverrides);
...
void MicrosoftVTableContext::computeVTableRelatedInformation(
  ...
  if (Context.getLangOpts().DumpVTableLayouts)
    dumpMethodLocations(RD, NewMethodLocations, toolchain::outs());
}
