...
QualType language::Core::desugarForDiagnostic(ASTContext &Context, QualType QT,
                                     bool &ShouldAKA) {
  ...
  while (true) {
    ...
    if (const auto *AT = dyn_cast<ArrayType>(Ty)) {
      ...
      if (const auto *CAT = dyn_cast<ConstantArrayType>(AT))
        ...
        toolchain_unreachable("Unhandled array type");
      break;
    }
...
ConvertTypeToDiagnosticString(ASTContext &Context, QualType Ty,
  ...
  if (!Repeated) {
    ...
    if (const auto *VTy = Ty->getAs<VectorType>()) {
      ...
      toolchain::raw_string_ostream OS(DecoratedString);
...
void language::Core::FormatASTNodeDiagnosticArgument(
  ...
  toolchain::raw_svector_ostream OS(Output);
  bool NeedQuotes = true;

  switch (Kind) {
    default: toolchain_unreachable("unknown ArgumentKind");
...
class TemplateDiff {
  ...
  public:
    ...
    void SetIntegerDiff(const toolchain::APSInt &FromInt, const toolchain::APSInt &ToInt,
    ...
    void SetFromDeclarationAndToIntegerDiff(
        ...
        Expr *FromExpr, const toolchain::APSInt &ToInt, bool IsValidToInt,
    ...
    void SetFromIntegerAndToDeclarationDiff(
        const toolchain::APSInt &FromInt, bool IsValidFromInt, QualType FromIntType,
    ...
    void GetIntegerDiff(toolchain::APSInt &FromInt, toolchain::APSInt &ToInt,
                        bool &IsValidFromInt, bool &IsValidToInt,
                        QualType &FromIntType, QualType &ToIntType,
    ...
    void GetFromDeclarationAndToIntegerDiff(
        ValueDecl *&FromValueDecl, bool &FromAddressOf, bool &FromNullPtr,
        Expr *&FromExpr, toolchain::APSInt &ToInt, bool &IsValidToInt,
    ...
    void GetFromIntegerAndToDeclarationDiff(
        toolchain::APSInt &FromInt, bool &IsValidFromInt, QualType &FromIntType,
        Expr *&FromExpr, ValueDecl *&ToValueDecl, bool &ToAddressOf,
  ...
  static void InitializeNonTypeDiffVariables(ASTContext &Context,
                                             ...
                                             toolchain::APSInt &Value, bool &HasInt,
    ...
    if (!Iter.isEnd()) {
      ...
      case TemplateArgument::TemplateExpansion:
        toolchain_unreachable("TemplateArgument kind is not expected for NTTP");
      case TemplateArgument::Pack:
        toolchain_unreachable("TemplateArgument kind should be handled elsewhere");
      }
    ...
    toolchain_unreachable("Unexpected TemplateArgument kind");
  }
  ...
  void DiffNonTypes(const TSTiterator &FromIter, const TSTiterator &ToIter,
    ...
    toolchain::APSInt FromInt, ToInt;
  ...
  void DiffTemplate(const TemplateSpecializationType *FromTST,
    ...
    for (TSTiterator FromIter(Context, FromTST), ToIter(Context, ToTST);
      ...
      if (isa<TemplateTypeParmDecl>(FromParamND)) {
        ...
        toolchain_unreachable("Unexpected Decl type.");
      }
  ...
  static bool IsEqualExpr(ASTContext &Context, Expr *FromExpr, Expr *ToExpr) {
    ...
    toolchain::FoldingSetNodeID FromID, ToID;
  ...
  void TreeToString(int Indent = 1) {
    ...
    switch (Tree.GetKind()) {
      case DiffTree::Invalid:
        toolchain_unreachable("Template diffing failed with bad DiffNode");
      case DiffTree::Type: {
  ...
  void PrintAPSInt(const toolchain::APSInt &FromInt, const toolchain::APSInt &ToInt,
  ...
  void PrintAPSInt(const toolchain::APSInt &Val, Expr *E, bool Valid,
  ...
  void PrintValueDeclAndInteger(ValueDecl *VD, bool NeedAddressOf,
                                ...
                                const toolchain::APSInt &Val, QualType IntType,
  ...
  void PrintIntegerAndValueDecl(const toolchain::APSInt &Val, QualType IntType,
...
std::string language::Core::FormatUTFCodeUnitAsCodepoint(unsigned Value, QualType T) {
  auto IsSingleCodeUnitCP = [](unsigned Value, QualType T) {
    ...
    if (T->isChar16Type()) {
      ...
      return toolchain::IsSingleCodeUnitUTF16Codepoint(Value);
    }
    assert(T->isChar32Type());
    return toolchain::IsSingleCodeUnitUTF32Codepoint(Value);
  };
  toolchain::SmallVector<char, 16> Str;
  if (!IsSingleCodeUnitCP(Value, T)) {
    toolchain::raw_svector_ostream OS(Str);
    OS << "<" << toolchain::format_hex(Value, 1, /*Upper=*/true) << ">";
    return std::string(Str.begin(), Str.end());
  }

  char Buffer[UNI_MAX_UTF8_BYTES_PER_CODE_POINT];
  char *Ptr = Buffer;
  [[maybe_unused]] bool Converted = toolchain::ConvertCodePointToUTF8(Value, Ptr);
