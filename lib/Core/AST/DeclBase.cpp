...
void *Decl::operator new(std::size_t Size, const ASTContext &Context,
  ...
  *PrefixPtr = ID.getRawValue();
  ...
  assert(*PrefixPtr < toolchain::maskTrailingOnes<uint64_t>(48));
...
void *Decl::operator new(std::size_t Size, const ASTContext &Ctx,
  ...
  if (Ctx.getLangOpts().trackLocalOwningModule() || !Parent) {
    ...
    size_t ExtraAlign =
        toolchain::offsetToAlignment(sizeof(Module *), toolchain::Align(alignof(Decl)));
...
GlobalDeclID Decl::getGlobalID() const {
  ...
  return GlobalDeclID(ID & toolchain::maskTrailingOnes<uint64_t>(48));
}
...
void Decl::setOwningModuleID(unsigned ID) {
  ...
  *IDAddress &= toolchain::maskTrailingOnes<uint64_t>(48);
...
const char *Decl::getDeclKindName() const {
  switch (DeclKind) {
  default: toolchain_unreachable("Declaration not in DeclNodes.inc!");
...
#include "language/Core/AST/DeclNodes.inc"
  ...
  toolchain_unreachable("Declaration context not in DeclNodes.inc!");
}
...
#define ABSTRACT_DECL(DECL)
#include "language/Core/AST/DeclNodes.inc"
  toolchain::errs() << "  " << totalDecls << " decls total.\n";

  int totalBytes = 0;
#define DECL(DERIVED, BASE)                                             \
  if (n##DERIVED##s > 0) {                                              \
    totalBytes += (int)(n##DERIVED##s * sizeof(DERIVED##Decl));         \
    toolchain::errs() << "    " << n##DERIVED##s << " " #DERIVED " decls, "  \
...
bool Decl::isFlexibleArrayMemberLike(
  ...
  if (CAT) {
    ...
    toolchain::APInt Size = CAT->getSize();
...
#include "language/Core/AST/DeclNodes.inc"
  default:
    toolchain_unreachable("a decl that inherits DeclContext isn't handled");
  }
}
...
void DeclContext::localUncachedLookup(DeclarationName Name,
  ...
  if (!hasExternalVisibleStorage() && !hasExternalLexicalStorage() && Name) {
    ...
    toolchain::append_range(Results, LookupResults);
    if (!Results.empty())
      return;
  }
...
StoredDeclsMap *DeclContext::CreateStoredDeclsMap(ASTContext &C) const {
  ...
  C.LastSDM = toolchain::PointerIntPair<StoredDeclsMap*,1>(M, Dependent);
  LookupPtr = M;
  return M;
}
...
void StoredDeclsMap::DestroyAll(StoredDeclsMap *Map, bool Dependent) {
  while (Map) {
    ...
    toolchain::PointerIntPair<StoredDeclsMap*,1> Next = Map->Previous;
...
unsigned DeclIDBase::getLocalDeclIndex() const {
  return ID & toolchain::maskTrailingOnes<DeclID>(32);
}
