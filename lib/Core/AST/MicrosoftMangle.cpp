...
struct msvc_hashing_ostream : public toolchain::raw_svector_ostream {
  raw_ostream &OS;
  toolchain::SmallString<64> Buffer;

  msvc_hashing_ostream(raw_ostream &OS)
      : toolchain::raw_svector_ostream(Buffer), OS(OS) {}
  ~msvc_hashing_ostream() override {
    ...
    toolchain::MD5::stringifyResult(Hash, HexString);
...
class MicrosoftMangleContextImpl : public MicrosoftMangleContext {
  ...
  toolchain::DenseMap<const CXXRecordDecl *, unsigned> LambdaIds;
  toolchain::DenseMap<GlobalDecl, unsigned> SEHFilterIds;
  toolchain::DenseMap<GlobalDecl, unsigned> SEHFinallyIds;
...
public:
  ...
  unsigned getLambdaId(const CXXRecordDecl *RD) {
    ...
    std::pair<toolchain::DenseMap<const CXXRecordDecl *, unsigned>::iterator, bool>
...
class MicrosoftCXXNameMangler {
  ...
  typedef toolchain::SmallVector<std::string, 10> BackRefVec;
  BackRefVec NameBackReferences;

  typedef toolchain::DenseMap<const void *, unsigned> ArgBackRefMap;
  ArgBackRefMap FunArgBackReferences;
  ArgBackRefMap TemplateArgBackReferences;

  typedef toolchain::DenseMap<const void *, StringRef> TemplateArgStringMap;
  TemplateArgStringMap TemplateArgStrings;
  toolchain::BumpPtrAllocator TemplateArgStringStorageAlloc;
  toolchain::StringSaver TemplateArgStringStorage;
...
public:
  ...
  void mangleNumber(toolchain::APSInt Number);
  void mangleFloat(toolchain::APFloat Number);
  void mangleBits(toolchain::APInt Number);
...
#undef TYPE
  ...
  void mangleIntegerLiteral(const toolchain::APSInt &Number,
...
MicrosoftMangleContextImpl::MicrosoftMangleContextImpl(ASTContext &Context,
  ...
  if (OptionalFileEntryRef FE = SM.getFileEntryRefForID(SM.getMainFileID())) {
    ...
    AnonymousNamespaceHash = toolchain::utohexstr(TruncatedHash);
  } else {
...
void MicrosoftCXXNameMangler::mangle(GlobalDecl GD, StringRef Prefix) {
  ...
  if (const FunctionDecl *FD = dyn_cast<FunctionDecl>(D))
    ...
    toolchain_unreachable("Tried to mangle unexpected NamedDecl!");
}
...
void MicrosoftCXXNameMangler::mangleNumber(int64_t Number) {
  mangleNumber(toolchain::APSInt(toolchain::APInt(64, Number), /*IsUnsigned*/false));
}

void MicrosoftCXXNameMangler::mangleNumber(toolchain::APSInt Number) {
  ...
  toolchain::APInt Value = Number.extend(Width);
...
void MicrosoftCXXNameMangler::mangleFloat(toolchain::APFloat Number) {
  ...
  case APFloat::S_Float4E2M1FN:
    toolchain_unreachable("Tried to mangle unexpected APFloat semantics");
  }

  mangleBits(Number.bitcastToAPInt());
}

void MicrosoftCXXNameMangler::mangleBits(toolchain::APInt Value) {
  if (Value == 0)
    Out << "A@";
    ...
    toolchain::SmallString<32> EncodedNumberBuffer;
  ...
  if (GlobalDecl TD = isTemplate(GD, TemplateArgs)) {
    ...
    if (Found == TemplateArgBackReferences.end()) {
      ...
      if (Found == TemplateArgStrings.end()) {
        ...
        toolchain::SmallString<64> TemplateMangling;
        toolchain::raw_svector_ostream Stream(TemplateMangling);
        ...
        BackRefVec::iterator StringFound =
            toolchain::find(NameBackReferences, TemplateMangling);
  ...
  switch (Name.getNameKind()) {
    case DeclarationName::Identifier: {
      if (const IdentifierInfo *II = Name.getAsIdentifierInfo()) {
        ...
        if (IsDeviceStub)
          mangleSourceName(
              (toolchain::Twine("__device_stub__") + II->getName()).str());
        else if (IsOCLDeviceStub)
          mangleSourceName(
              (toolchain::Twine("__clang_ocl_kern_imp_") + II->getName()).str());
        else
      ...
      if (const DecompositionDecl *DD = dyn_cast<DecompositionDecl>(ND)) {
        ...
        toolchain::SmallString<64> Name("$S");
        // Get a unique id for the anonymous struct.
        Name += toolchain::utostr(Context.getAnonymousStructId(DD) + 1);
      ...
      if (const VarDecl *VD = dyn_cast<VarDecl>(ND)) {
        ...
        toolchain::SmallString<64> Name("$S");
        // Get a unique id for the anonymous struct.
        Name += toolchain::utostr(Context.getAnonymousStructId(RD) + 1);
      ...
      if (const CXXRecordDecl *Record = dyn_cast<CXXRecordDecl>(TD)) {
        if (Record->isLambda()) {
          toolchain::SmallString<10> Name("<lambda_");
      ...
      if (DeclaratorDecl *DD =
        ...
        Name += "<unnamed-type-$S";
        Name += toolchain::utostr(Context.getAnonymousStructId(TD) + 1);
      }
      Name += ">";
      mangleSourceName(Name.str());
      break;
...
void MicrosoftCXXNameMangler::mangleCXXDtorType(CXXDtorType T) {
  ...
  // it.
  case Dtor_Comdat:
    toolchain_unreachable("not expecting a COMDAT");
  }
  toolchain_unreachable("Unsupported dtor type?");
}
...
void MicrosoftCXXNameMangler::mangleSourceName(StringRef Name) {
  ...
  BackRefVec::iterator Found = toolchain::find(NameBackReferences, Name);
...
void MicrosoftCXXNameMangler::mangleIntegerLiteral(
    const toolchain::APSInt &Value, const NonTypeTemplateParmDecl *PD,
    QualType TemplateArgType) {
...
void MicrosoftCXXNameMangler::mangleTemplateArg(const TemplateDecl *TD,
  ...
  case TemplateArgument::Null:
    toolchain_unreachable("Can't mangle null template arguments!");
  case TemplateArgument::TemplateExpansion:
    toolchain_unreachable("Can't mangle template expansion arguments!");
    ...
    if (const MemberPointerType *MPT = T->getAs<MemberPointerType>()) {
      ...
      if (MPT->isMemberDataPointer()) {
        ...
        if (!RD->nullFieldOffsetIsZero()) {
          mangleIntegerLiteral(toolchain::APSInt::get(-1),
                               cast<NonTypeTemplateParmDecl>(Parm), T);
          return;
        }
      }
    }
    mangleIntegerLiteral(toolchain::APSInt::getUnsigned(0),
                         cast<NonTypeTemplateParmDecl>(Parm), T);
    break;
  }
  ...
  case TemplateArgument::Pack: {
    ...
    if (TemplateArgs.empty()) {
      if (isa<TemplateTypeParmDecl>(Parm) ||
        ...
        toolchain_unreachable("unexpected template parameter decl!");
    } else {
  ...
  case TemplateArgument::Template: {
    ...
    if (const auto *TD = dyn_cast<TagDecl>(ND)) {
      ...
      toolchain_unreachable("unexpected template template NamedDecl!");
    }
    break;
  }
  }
...
void MicrosoftCXXNameMangler::manglePassObjectSizeArg(
  ...
  if (Found == FunArgBackReferences.end()) {
    ...
    mangleArtificialTagType(TagTypeKind::Enum, Name + toolchain::utostr(Type),
                            {"__clang"});
...
void MicrosoftCXXNameMangler::mangleAddressSpaceType(QualType T,
  ...
  // __language::Core::struct _AS[language_addr_space]<Type>
  // where:
  ...
  //  __language::Core::struct _AS<TargetAS, Type>
  assert(Quals.hasAddressSpace() && "Not valid without address space");
  toolchain::SmallString<32> ASMangling;
  toolchain::raw_svector_ostream Stream(ASMangling);
  MicrosoftCXXNameMangler Extra(Context, Stream);
  Stream << "?$";
  ...
  if (Context.getASTContext().addressSpaceMapManglingFor(AS)) {
    ...
    Extra.mangleIntegerLiteral(toolchain::APSInt::getUnsigned(TargetAS));
  } else {
    switch (AS) {
    default:
      toolchain_unreachable("Not a language specific address space");
    ...
    case LangAS::ptr64:
      toolchain_unreachable("don't mangle ptr address spaces with _AS");
    }
  }
...
void MicrosoftCXXNameMangler::mangleAutoReturnType(QualType T,
  ...
  case QMM_Mangle:
    ...
    break;
  default:
    toolchain_unreachable("QMM_Escape unexpected");
  }
  ...
  case Type::RValueReference:
    ...
    break;
  default:
    toolchain_unreachable("Invalid type expected");
  }
}
...
#include "language/Core/AST/BuiltinTypes.def"
  case BuiltinType::Dependent:
    toolchain_unreachable("placeholder types shouldn't get to name mangling");
  ...
  if (IsStructor) {
    ...
    if (IsCtorClosure) {
      ...
      if (StructorType == Ctor_DefaultClosure) {
        ...
        toolchain_unreachable("unexpected constructor closure!");
      }
      Out << 'Z';
      return;
    }
...
void MicrosoftCXXNameMangler::mangleFunctionClass(const FunctionDecl *FD) {
  ...
  if (const CXXMethodDecl *MD = dyn_cast<CXXMethodDecl>(FD)) {
    ...
    switch (MD->getAccess()) {
      case AS_none:
        toolchain_unreachable("Unsupported access specifier");
      case AS_private:
        if (!MD->isImplicitObjectMemberFunction())
          Out << 'C';
        else if (IsVirtual)
...
void MicrosoftCXXNameMangler::mangleArtificialTagType(
  ...
  for (StringRef N : toolchain::reverse(NestedNames))
    mangleSourceName(N);
...
void MicrosoftCXXNameMangler::mangleType(const ConstantArrayType *T, Qualifiers,
                                         SourceRange) {
  toolchain_unreachable("Should have been special cased");
}
...
void MicrosoftCXXNameMangler::mangleArrayType(const ArrayType *T) {
  ...
  SmallVector<toolchain::APInt, 3> Dimensions;
  for (;;) {
    if (ElementTy->isConstantArrayType()) {
      ...
      Dimensions.push_back(toolchain::APInt(32, 0));
  ...
  for (const toolchain::APInt &Dimension : Dimensions)
...
void MicrosoftCXXNameMangler::mangleType(const TemplateTypeParmType *T,
  ...
  toolchain::SmallString<64> Name;
  Name += "<TTPT_";
  Name += toolchain::utostr(T->getDepth());
  Name += "_";
  Name += toolchain::utostr(T->getIndex());
  Name += ">";
  mangleSourceName(Name);
}
...
void MicrosoftCXXNameMangler::mangleType(const VectorType *T, Qualifiers Quals,
      ...
      if (Width == 64 && ET->getKind() == BuiltinType::LongLong) {
        ...
        if (ET->getKind() == BuiltinType::Float)
          mangleArtificialTagType(TagTypeKind::Union,
                                  "__m" + toolchain::utostr(Width));
        else if (ET->getKind() == BuiltinType::LongLong)
          mangleArtificialTagType(TagTypeKind::Union,
                                  "__m" + toolchain::utostr(Width) + 'i');
        else if (ET->getKind() == BuiltinType::Double)
          mangleArtificialTagType(TagTypeKind::Struct,
                                  "__m" + toolchain::utostr(Width) + 'd');
      }
    }
  }
  ...
  if (!IsBuiltin) {
    ...
    Extra.mangleType(QualType(ET ? static_cast<const Type *>(ET) : BitIntTy, 0),
                     Range, QMM_Escape);
    Extra.mangleIntegerLiteral(toolchain::APSInt::getUnsigned(T->getNumElements()));
...
void MicrosoftCXXNameMangler::mangleType(const ConstantMatrixType *T,
  ...
  Extra.mangleIntegerLiteral(toolchain::APSInt::getUnsigned(T->getNumRows()));
  Extra.mangleIntegerLiteral(toolchain::APSInt::getUnsigned(T->getNumColumns()));
...
void MicrosoftCXXNameMangler::mangleType(const InjectedClassNameType *,
                                         Qualifiers, SourceRange) {
  toolchain_unreachable("Cannot mangle injected class name type.");
}
...
void MicrosoftCXXNameMangler::mangleType(const PipeType *T, Qualifiers,
  ...
  Extra.mangleType(ElementType, Range, QMM_Escape);
  Extra.mangleIntegerLiteral(toolchain::APSInt::get(T->isReadOnly()));
...
void MicrosoftCXXNameMangler::mangleType(const BitIntType *T, Qualifiers,
  ...
  Extra.mangleIntegerLiteral(toolchain::APSInt::getUnsigned(T->getNumBits()));
...
void MicrosoftCXXNameMangler::mangleType(const HLSLAttributedResourceType *T,
                                         Qualifiers, SourceRange Range) {
  toolchain_unreachable("HLSL uses Itanium name mangling");
}
...
void MicrosoftMangleContextImpl::mangleCXXCatchableType(
  ...
  toolchain::SmallString<64> RTTIMangling;
  {
    toolchain::raw_svector_ostream Stream(RTTIMangling);
  ...
  toolchain::SmallString<64> CopyCtorMangling;
  if (!OmitCopyCtor && CD) {
    toolchain::raw_svector_ostream Stream(CopyCtorMangling);
    msvc_hashing_ostream MHO(Stream);
...
void MicrosoftMangleContextImpl::mangleCXXRTTICompleteObjectLocator(
  ...
  // NOTE: <cvr-qualifiers> here is always 'B' (const). <storage-class>
  // is always '6' for vftables.
  toolchain::SmallString<64> VFTableMangling;
  toolchain::raw_svector_ostream Stream(VFTableMangling);
...
void MicrosoftMangleContextImpl::mangleReferenceTemporary(
  ...
  Mangler.getStream() << "?";
  Mangler.mangleSourceName("$RT" + toolchain::utostr(ManglingNumber));
  Mangler.mangle(VD, "");
}
...
void MicrosoftMangleContextImpl::mangleThreadSafeStaticGuardVariable(
  ...
  Mangler.getStream() << "?";
  Mangler.mangleSourceName("$TSS" + toolchain::utostr(GuardNum));
  ...
  auto MangleByte = [&Mangler](char Byte) {
    ...
    if (isAsciiIdentifierContinue(Byte, /*AllowDollar=*/true)) {
      ...
      const char SpecialChars[] = {',', '/',  '\\', ':',  '.',
                                   ' ', '\n', '\t', '\'', '-'};
      const char *Pos = toolchain::find(SpecialChars, Byte);
