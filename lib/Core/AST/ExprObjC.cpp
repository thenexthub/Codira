...
void ObjCMessageExpr::initArgsAndSelLocs(ArrayRef<Expr *> Args,
  ...
  if (!isImplicit() && SelLocsK == SelLoc_NonStandard)
    toolchain::copy(SelLocs, getStoredSelLocs());
}
...
SourceRange ObjCMessageExpr::getReceiverRange() const {
  ...
  toolchain_unreachable("Invalid ReceiverKind!");
}
...
QualType ObjCMessageExpr::getReceiverType() const {
  ...
  toolchain_unreachable("unexpected receiver kind");
}
...
StringRef ObjCBridgedCastExpr::getBridgeKindName() const {
  ...
  toolchain_unreachable("Invalid BridgeKind!");
}
