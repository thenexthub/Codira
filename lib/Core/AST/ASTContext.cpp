...
template <> struct toolchain::DenseMapInfo<toolchain::FoldingSetNodeID> {
  ...
  if (const auto *CRD = dyn_cast<CXXRecordDecl>(&D)) {
    ...
    if (const auto *CTSD = dyn_cast<ClassTemplateSpecializationDecl>(CRD)) {
      if (CTSD->getSpecializationKind() != TSK_ImplicitInstantiation)
        return D;
      toolchain::PointerUnion<ClassTemplateDecl *,
...
comments::FullComment *ASTContext::getCommentForDecl(
  ...
  toolchain::DenseMap<const Decl *, comments::FullComment *>::iterator Pos =
...
CXXABI *ASTContext::createCXXABI(const TargetInfo &T) {
  ...
  toolchain_unreachable("Invalid CXXABI type!");
}
...
static bool isAddrSpaceMapManglingEnabled(const TargetInfo &TI,
  ...
  toolchain_unreachable("getAddressSpaceMapMangling() doesn't cover anything.");
}
...
void ASTContext::cleanup() {
  ...
  for (toolchain::DenseMap<const ObjCInterfaceDecl *,
  ...
  for (toolchain::DenseMap<const RecordDecl*, const ASTRecordLayout*>::iterator
  ...
  for (toolchain::DenseMap<const Decl*, AttrVec*>::iterator A = DeclAttrs.begin(),
                                                    AEnd = DeclAttrs.end();
       A != AEnd; ++A)
    A->second->~AttrVec();
  DeclAttrs.clear();
...
void ASTContext::PrintStats() const {
  toolchain::errs() << "\n*** AST Context Stats:\n";
  toolchain::errs() << "  " << Types.size() << " types total.\n";
...
#define TYPE(Name, Parent)                                              \
  if (counts[Idx])                                                      \
    toolchain::errs() << "    " << counts[Idx] << " " << #Name               \
...
#include "language/Core/AST/TypeNodes.inc"
  ...
  toolchain::errs() << "Total bytes = " << TotalBytes << "\n";

  // Implicit special member functions.
  toolchain::errs() << NumImplicitDefaultConstructorsDeclared << "/"
               << NumImplicitDefaultConstructors
               << " implicit default constructors created\n";
  toolchain::errs() << NumImplicitCopyConstructorsDeclared << "/"
               << NumImplicitCopyConstructors
               << " implicit copy constructors created\n";
  if (getLangOpts().CPlusPlus)
    toolchain::errs() << NumImplicitMoveConstructorsDeclared << "/"
                 << NumImplicitMoveConstructors
                 << " implicit move constructors created\n";
  toolchain::errs() << NumImplicitCopyAssignmentOperatorsDeclared << "/"
               << NumImplicitCopyAssignmentOperators
               << " implicit copy assignment operators created\n";
  if (getLangOpts().CPlusPlus)
    toolchain::errs() << NumImplicitMoveAssignmentOperatorsDeclared << "/"
                 << NumImplicitMoveAssignmentOperators
                 << " implicit move assignment operators created\n";
  toolchain::errs() << NumImplicitDestructorsDeclared << "/"
...
void ASTContext::deduplicateMergedDefinitionsFor(NamedDecl *ND) {
  ...
  toolchain::DenseSet<Module*> Found;
  for (Module *&M : Merged)
    if (!Found.insert(M).second)
      M = nullptr;
  toolchain::erase(Merged, nullptr);
}
...
void ASTContext::eraseDeclAttrs(const Decl *D) {
  toolchain::DenseMap<const Decl*, AttrVec*>::iterator Pos = DeclAttrs.find(D);
...
ASTContext::getTemplateOrSpecializationInfo(const VarDecl *Var) {
  toolchain::DenseMap<const VarDecl *, TemplateOrSpecializationInfo>::iterator Pos =
...
ASTContext::overridden_methods(const CXXMethodDecl *Method) const {
  toolchain::DenseMap<const CXXMethodDecl *, CXXMethodVector>::const_iterator Pos =
...
const toolchain::fltSemantics &ASTContext::getFloatTypeSemantics(QualType T) const {
...
static getConstantArrayInfoInChars(const ASTContext &Context,
  ...
  if (!Context.getTargetInfo().getCXXABI().isMicrosoft() ||
    ...
    Width = toolchain::alignTo(Width, Align);
...
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("Should not see dependent types");
  ...
  case Type::Vector: {
    ...
    if (Align & (Align-1)) {
      Align = toolchain::bit_ceil(Align);
      Width = toolchain::alignTo(Width, Align);
    }
  ...
  case Type::Builtin:
    ...
    default: toolchain_unreachable("Unknown builtin type!");
...
#include "language/Core/Basic/HLSLIntangibleTypes.def"
  ...
  case Type::Atomic: {
    ...
    if (!Width) {
      ...
      Width = toolchain::bit_ceil(Width);
  ...
  assert(toolchain::isPowerOf2_32(Align) && "Alignment must be power of 2");
...
unsigned ASTContext::getOpenMPDefaultSimdAlign(QualType T) const {
  unsigned SimdAlign = toolchain::OpenMPIRBuilder::getOpenMPDefaultSimdAlign(
...
void ASTContext::DeepCollectObjCIvars(const ObjCInterfaceDecl *OI,
  ...
  if (!leafClass) {
    toolchain::append_range(Ivars, OI->ivars());
  } else {
...
void ASTContext::CollectInheritedProtocols(const Decl *CDecl,
                          toolchain::SmallPtrSet<ObjCProtocolDecl*, 8> &Protocols) {
...
static std::optional<int64_t> structSubobjectsHaveUniqueObjectRepresentations(
    ...
    const ASTContext &Context, const language::Core::ASTRecordLayout &Layout,
    bool CheckIfTriviallyCopyable) {
...
structHasUniqueObjectRepresentations(const ASTContext &Context,
  ...
  if (const auto *ClassDecl = dyn_cast<CXXRecordDecl>(RD)) {
    ...
    toolchain::sort(Bases, [&](const CXXRecordDecl *L, const CXXRecordDecl *R) {
...
ObjCImplementationDecl *ASTContext::getObjCImplementation(ObjCInterfaceDecl *D) {
  toolchain::DenseMap<ObjCContainerDecl*, ObjCImplDecl*>::iterator
    I = ObjCImpls.find(D);
  if (I != ObjCImpls.end())
...
ASTContext::getPointerAuthVTablePointerDiscriminator(const CXXRecordDecl *RD) {
  ...
  SmallString<256> Str;
  toolchain::raw_svector_ostream Out(Str);
  MC->mangleCXXVTable(RD, Out);
  return toolchain::getPointerAuthStableSipHash(Str);
}
...
#include "language/Core/AST/BuiltinTypes.def"
      toolchain_unreachable("placeholder types should not appear here.");
...
#include "language/Core/Basic/HLSLIntangibleTypes.def"
    case BuiltinType::Dependent:
      toolchain_unreachable("should never get here");
...
#define TYPE(Class, Base)
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("unexpected non-canonical or dependent type!");
    return;
  }
}
...
ASTContext::adjustType(QualType Orig,
                       toolchain::function_ref<QualType(QualType)> Adjust) const {
...
QualType ASTContext::getConstantArrayType(QualType EltTy,
                                          const toolchain::APInt &ArySizeIn,
                                          const Expr *SizeExpr,
  ...
  // the target.
  toolchain::APInt ArySize(ArySizeIn);
...
#define NON_CANONICAL_TYPE(Class, Base) case Type::Class:
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("didn't desugar past all non-canonical types?");
  ...
  case Type::HLSLAttributedResource:
  case Type::HLSLInlineSpirv:
    toolchain_unreachable("type should never be variably-modified");
...
#define SVE_INT_ELTTY(BITS, ELTS, SIGNED, NUMVECTORS)                          \
  {getIntTypeForBitwidth(BITS, SIGNED), toolchain::ElementCount::getScalable(ELTS), \
   NUMVECTORS};

#define SVE_ELTTY(ELTTY, ELTS, NUMVECTORS)                                     \
  {ELTTY, toolchain::ElementCount::getScalable(ELTS), NUMVECTORS};

  switch (Ty->getKind()) {
  default:
    toolchain_unreachable("Unsupported builtin vector type");

#define SVE_VECTOR_TYPE_INT(Name, MangledName, Id, SingletonId, NumEls,        \
                            ElBits, NF, IsSigned)                              \
  case BuiltinType::Id:                                                        \
    return {getIntTypeForBitwidth(ElBits, IsSigned),                           \
            toolchain::ElementCount::getScalable(NumEls), NF};
...
#define SVE_VECTOR_TYPE_BFLOAT(Name, MangledName, Id, SingletonId, NumEls,     \
                               ElBits, NF)                                     \
  case BuiltinType::Id:                                                        \
    return {BFloat16Ty, toolchain::ElementCount::getScalable(NumEls), NF};
#define SVE_VECTOR_TYPE_MFLOAT(Name, MangledName, Id, SingletonId, NumEls,     \
                               ElBits, NF)                                     \
  case BuiltinType::Id:                                                        \
    return {MFloat8Ty, toolchain::ElementCount::getScalable(NumEls), NF};
#define SVE_PREDICATE_TYPE_ALL(Name, MangledName, Id, SingletonId, NumEls, NF) \
  case BuiltinType::Id:                                                        \
    return {BoolTy, toolchain::ElementCount::getScalable(NumEls), NF};
#include "language/Core/Basic/AArch64ACLETypes.def"
...
#define RVV_PREDICATE_TYPE(Name, Id, SingletonId, NumEls)                      \
  case BuiltinType::Id:                                                        \
    return {BoolTy, toolchain::ElementCount::getScalable(NumEls), 1};
#include "language/Core/Basic/RISCVVTypes.def"
  }
}
...
QualType ASTContext::getVectorType(QualType vecType, unsigned NumElts,
  ...
  assert(vecType->isBuiltinType() ||
         (vecType->isBitIntType() &&
          ...
          toolchain::isPowerOf2_32(vecType->castAs<BitIntType>()->getNumBits())));
...
QualType ASTContext::getFunctionTypeInternal(
  ...
  if (!isCanonical && Canonical.isNull()) {
    ...
    toolchain::SmallVector<QualType, 8> ExceptionTypeStorage;
    ...
    if (IsCanonicalExceptionSpec) {
      ...
      case EST_DependentNoexcept:
        toolchain_unreachable("dependent noexcept is already canonical");
      }
    } else {
...
ASTContext::getPredefinedSugarType(PredefinedSugarType::Kind KD) const {
  ...
  if (auto *Target = PredefinedSugarTypes[toolchain::to_underlying(KD)];
      Target != nullptr)
  ...
  Types.push_back(New);
  PredefinedSugarTypes[toolchain::to_underlying(KD)] = New;
  return QualType(New, 0);
}
...
TagType *ASTContext::getTagTypeInternal(ElaboratedTypeKeyword Keyword,
  ...
  if (Qualifier) {
    ...
    Size = toolchain::alignTo(Size, alignof(NestedNameSpecifier)) +
  ...
  auto *T = [&, TC = TC]() -> TagType * {
    ...
    default:
      toolchain_unreachable("unexpected type class");
    }
  }();
...
bool ASTContext::isRepresentableIntegerValue(toolchain::APSInt &Value, QualType T) {
...
QualType ASTContext::getAttributedType(NullabilityKind nullability,
  ...
  toolchain_unreachable("Unknown nullability kind");
}
...
getCanonicalElaboratedTypeKeyword(ElaboratedTypeKeyword Keyword) {
  ...
  toolchain_unreachable("unexpected keyword kind");
}
...
#ifndef NDEBUG
  toolchain::FoldingSetNodeID InsertedID;
...
SortAndUniqueProtocols(SmallVectorImpl<ObjCProtocolDecl *> &Protocols) {
  ...
  toolchain::array_pod_sort(Protocols.begin(), Protocols.end(), CmpProtocolNames);
  ...
  // Remove duplicates.
  auto ProtocolsEnd = toolchain::unique(Protocols);
...
QualType ASTContext::getObjCObjectType(
  ...
  // canonicalized.
  QualType canonical;
  bool typeArgsAreCanonical = toolchain::all_of(
...
bool ASTContext::QIdProtocolsAdoptObjCObjectProtocols(QualType QT,
  ...
  if (!IDecl->hasDefinition())
    return false;
  toolchain::SmallPtrSet<ObjCProtocolDecl *, 8> InheritedProtocols;
...
#ifndef NDEBUG
  toolchain::FoldingSetNodeID TempID;
  DTST->Profile(TempID);
...
ASTContext::getNameForTemplate(TemplateName Name,
  ...
  toolchain_unreachable("bad template name kind!");
}
...
getDefaultTemplateArgumentOrNone(const NamedDecl *P) {
  ...
  default:
    toolchain_unreachable("Unexpected template parameter kind");
  }
}
...
TemplateName ASTContext::getCanonicalTemplateName(TemplateName Name,
  ...
  case TemplateName::OverloadedTemplate:
  case TemplateName::AssumedTemplate:
    toolchain_unreachable("cannot canonicalize unresolved template");
  ...
  case TemplateName::QualifiedTemplate:
  case TemplateName::SubstTemplateTemplateParm:
    toolchain_unreachable("always sugar node");
  }

  toolchain_unreachable("bad template name!");
}
...
bool ASTContext::isSameConstraintExpr(const Expr *XCE, const Expr *YCE) const {
  ...
  toolchain::FoldingSetNodeID XCEID, YCEID;
...
bool ASTContext::isSameDefaultTemplateArgument(const NamedDecl *X,
  ...
  if (auto *NTTPX = dyn_cast<NonTypeTemplateParmDecl>(X)) {
    ...
    toolchain::FoldingSetNodeID XID, YID;
...
static bool isSameQualifier(const NestedNameSpecifier X,
  ...
  toolchain_unreachable("unhandled qualifier kind");
}
...
static bool hasSameOverloadableAttrs(const FunctionDecl *A,
  ...
  toolchain::FoldingSetNodeID Cand1ID, Cand2ID;
...
ASTContext::getCanonicalTemplateArgument(const TemplateArgument &Arg) const {
  ...
  // Silence GCC warning
  toolchain_unreachable("Unhandled template argument kind");
}
...
bool ASTContext::isSameTemplateArgument(const TemplateArgument &Arg1,
  ...
  switch (Arg1.getKind()) {
  case TemplateArgument::Null:
    toolchain_unreachable("Comparing NULL template argument");
  ...
  case TemplateArgument::Integral:
    return toolchain::APSInt::isSameValue(Arg1.getAsIntegral(),
                                     Arg2.getAsIntegral());
  ...
  case TemplateArgument::Expression: {
    toolchain::FoldingSetNodeID ID1, ID2;
...
static FloatingRank getFloatingRank(QualType T) {
  ...
  switch (T->castAs<BuiltinType>()->getKind()) {
  default: toolchain_unreachable("getFloatingRank(): not a floating type");
...
unsigned ASTContext::getIntegerRank(const Type *T) const {
  ...
  switch (cast<BuiltinType>(T)->getKind()) {
  default: toolchain_unreachable("getIntegerRank(): not a built-in integer");
  case BuiltinType::Bool:
...
QualType ASTContext::getPromotedIntegerType(QualType Promotable) const {
  ...
  if (const auto *BT = Promotable->getAs<BuiltinType>()) {
    ...
    if (BT->getKind() == BuiltinType::WChar_S ||
      ...
      toolchain_unreachable("char type should fit into long long");
    }
  }
...
bool ASTContext::BlockRequiresCopying(QualType Ty,
  ...
  if (Qualifiers::ObjCLifetime lifetime = qs.getObjCLifetime()) {
    switch (lifetime) {
      case Qualifiers::OCL_None: toolchain_unreachable("impossible");
    ...
    toolchain_unreachable("fell out of lifetime switch!");
  }
...
static std::string charUnitsToString(const CharUnits &CU) {
  return toolchain::itostr(CU.getQuantity());
}
...
#include "language/Core/Basic/AMDGPUTypes.def"
    ...
    case BuiltinType::ObjCClass:
    case BuiltinType::ObjCSel:
      toolchain_unreachable("@encoding ObjC primitive type");
...
#include "language/Core/AST/BuiltinTypes.def"
      toolchain_unreachable("invalid builtin type for @encode");
    }
    toolchain_unreachable("invalid BuiltinType::Kind value");
}
...
static void EncodeBitField(const ASTContext *Ctx, std::string& S,
  ...
  S += toolchain::utostr(FD->getBitWidthValue());
}
...
void ASTContext::getObjCEncodingForTypeImpl(QualType T, std::string &S,
  ...
  case Type::VariableArray: {
    ...
    if (isa<IncompleteArrayType>(AT) && !Options.IsStructField()) {
      ...
      if (const auto *CAT = dyn_cast<ConstantArrayType>(AT))
        S += toolchain::utostr(CAT->getZExtSize());
      else {
...
#define NON_CANONICAL_UNLESS_DEPENDENT_TYPE(KIND, BASE) \
  case Type::KIND:
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("@encode for dependent type!");
  }
...
static TypedefDecl *CreatePowerABIBuiltinVaListDecl(const ASTContext *Context) {
  ...
  toolchain::APInt Size(Context->getTypeSize(Context->getSizeType()), 1);
...
static TypedefDecl *CreateVaListDecl(const ASTContext *Context,
  ...
  toolchain_unreachable("Unhandled __builtin_va_list type kind");
}
...
CanQualType ASTContext::getFromTargetType(unsigned Type) const {
  ...
  toolchain_unreachable("Unhandled TargetInfo::IntType value");
}
...
bool ASTContext::ObjCQualifiedIdTypesAreCompatible(
  ...
  if (lhs->getInterfaceType()) {
    ...
    if (ObjCInterfaceDecl *lhsID = lhs->getInterfaceDecl()) {
      toolchain::SmallPtrSet<ObjCProtocolDecl *, 8> LHSInheritedProtocols;
...
void getIntersectionOfProtocols(ASTContext &Context,
  ...
  toolchain::SmallPtrSet<ObjCProtocolDecl *, 8> LHSProtocolSet;
  ...
  toolchain::SmallPtrSet<ObjCProtocolDecl *, 8> RHSProtocolSet;
  ...
  toolchain::SmallPtrSet<ObjCProtocolDecl *, 8> ImpliedProtocols;
  Context.CollectInheritedProtocols(CommonBase, ImpliedProtocols);

  // Remove any implied protocols from the list of inherited protocols.
  if (!ImpliedProtocols.empty()) {
    toolchain::erase_if(IntersectionSet, [&](ObjCProtocolDecl *proto) -> bool {
      return ImpliedProtocols.contains(proto);
    });
  }

  // Sort the remaining protocols by name.
  toolchain::array_pod_sort(IntersectionSet.begin(), IntersectionSet.end(),
...
QualType ASTContext::areCommonBaseCompatible(
  ...
  toolchain::SmallDenseMap<const ObjCInterfaceDecl *, const ObjCObjectType *, 4>
    LHSAncestors;
  while (true) {
...
bool ASTContext::canAssignObjCInterfaces(const ObjCObjectType *LHS,
  ...
  if (LHS->getNumProtocols() > 0) {
    ...
    toolchain::SmallPtrSet<ObjCProtocolDecl *, 8> SuperClassInheritedProtocols;
...
#include "language/Core/AST/TypeNodes.inc"
  ...
  case Type::RValueReference:
  case Type::MemberPointer:
    toolchain_unreachable("C++ should never be in mergeTypes");
  ...
  case Type::FunctionProto:
  case Type::ExtVector:
    toolchain_unreachable("Types are eliminated above");
  ...
  case Type::ConstantArray:
  {
    ...
    if (LVAT || RVAT) {
      auto SizeFetch = [this](const VariableArrayType* VAT,
          const ConstantArrayType* CAT)
          -> std::pair<bool,toolchain::APInt> {
        if (VAT) {
          std::optional<toolchain::APSInt> TheInt;
          Expr *E = VAT->getSizeExpr();
          if (E && (TheInt = E->getIntegerConstantExpr(*this)))
            return std::make_pair(true, *TheInt);
          return std::make_pair(false, toolchain::APSInt());
        }
        if (CAT)
          return std::make_pair(true, CAT->getSize());
        return std::make_pair(false, toolchain::APInt());
      };

      bool HaveLSize, HaveRSize;
      toolchain::APInt LSize, RSize;
      std::tie(HaveLSize, LSize) = SizeFetch(LVAT, LCAT);
      std::tie(HaveRSize, RSize) = SizeFetch(RVAT, RCAT);
      if (HaveLSize && HaveRSize && !toolchain::APInt::isSameValue(LSize, RSize))
  ...
  toolchain_unreachable("Invalid Type::Class!");
}
...
static QualType DecodeTypeFromStr(const char *&Str, const ASTContext &Context,
  ...
  while (!Done) {
    ...
    case 'W':
      ...
      default:
        toolchain_unreachable("Unexpected integer type");
  ...
  // Read the base type.
  switch (*Str++) {
  default: toolchain_unreachable("Unknown builtin type letter!");
  case 'x':
  ...
  case 'Q': {
    ...
    default:
      toolchain_unreachable("Unexpected target builtin type");
    }
    break;
  }
  case 'V': {
...
void ASTContext::forEachMultiversionedFunctionVersion(
    const FunctionDecl *FD,
    toolchain::function_ref<void(FunctionDecl *)> Pred) const {
  assert(FD->isMultiVersion() && "Only valid for multiversioned functions");
  toolchain::SmallDenseSet<const FunctionDecl*, 4> SeenDecls;
  FD = FD->getMostRecentDecl();
...
MangleContext *ASTContext::createMangleContext(const TargetInfo *T) {
  ...
  toolchain_unreachable("Unsupported ABI");
}
...
size_t ASTContext::getSideTableAllocatedMemory() const {
  return ASTRecordLayouts.getMemorySize() +
         toolchain::capacity_in_bytes(ObjCLayouts) +
         toolchain::capacity_in_bytes(KeyFunctions) +
         toolchain::capacity_in_bytes(ObjCImpls) +
         toolchain::capacity_in_bytes(BlockVarCopyInits) +
         toolchain::capacity_in_bytes(DeclAttrs) +
         toolchain::capacity_in_bytes(TemplateOrInstantiation) +
         toolchain::capacity_in_bytes(InstantiatedFromUsingDecl) +
         toolchain::capacity_in_bytes(InstantiatedFromUsingShadowDecl) +
         toolchain::capacity_in_bytes(InstantiatedFromUnnamedFieldDecl) +
         toolchain::capacity_in_bytes(OverriddenMethods) +
         toolchain::capacity_in_bytes(Types) +
         toolchain::capacity_in_bytes(VariableArrayTypes);
}
...
QualType ASTContext::getRealTypeForBitwidth(unsigned DestWidth,
  ...
  toolchain_unreachable("Unhandled TargetInfo::RealType value");
}
...
QualType ASTContext::getStringLiteralArrayType(QualType EltTy,
  ...
  return getConstantArrayType(EltTy, toolchain::APInt(32, Length + 1), nullptr,
...
bool ASTContext::AtomicUsesUnsupportedLibcall(const AtomicExpr *E) const {
  const toolchain::Triple &T = getTargetInfo().getTriple();
  if (!T.isOSDarwin())
    return false;
...
bool ASTContext::hasSameExpr(const Expr *X, const Expr *Y) const {
  ...
  if (!X || !Y)
    return false;
  toolchain::FoldingSetNodeID IDX, IDY;
...
static Decl *getCommonDecl(Decl *X, Decl *Y) {
  ...
  toolchain_unreachable("Corrupt redecls chain");
}
...
static NestedNameSpecifier getCommonNNS(const ASTContext &Ctx,
  ...
  case NestedNameSpecifier::Kind::Global:
    ...
    toolchain_unreachable("singletons did not compare equal");
  }
...
static void mergeTypeLists(const ASTContext &Ctx,
                           ...
                           ArrayRef<QualType> Y) {
  toolchain::DenseMap<QualType, unsigned> Found;
  for (auto Ts : {X, Y}) {
    for (QualType T : Ts) {
...
ASTContext::mergeExceptionSpecs(FunctionProtoType::ExceptionSpecInfo ESI1,
  ...
  case EST_NoexceptTrue:
  case EST_NoThrow:
    toolchain_unreachable("These ESTs should be handled above");
  ...
  case EST_Unparsed:
    toolchain_unreachable("shouldn't see unresolved exception specifications here");
  }

  toolchain_unreachable("invalid ExceptionSpecificationType");
}
...
#define UNEXPECTED_TYPE(Class, Kind)                                           \
  case Type::Class:                                                            \
    toolchain_unreachable("Unexpected " Kind ": " #Class);
...
#undef UNEXPECTED_TYPE
  ...
  case Type::CountAttributed: {
    ...
    ArrayRef<language::Core::TypeCoupledDeclRefInfo> CDX = DX->getCoupledDecls();
    ...
    toolchain::APSInt VX = *CEX->getIntegerConstantExpr(Ctx);
    toolchain::APSInt VY = *CEY->getIntegerConstantExpr(Ctx);
    if (VX != VY)
      return QualType();
    return Ctx.getCountAttributedType(Ctx.getQualifiedType(Underlying), CEX,
                                      DX->isCountInBytes(), DX->isOrNull(),
  ...
  toolchain_unreachable("Unhandled Type Class");
}
...
QualType ASTContext::getCorrespondingUnsaturatedType(QualType Ty) const {
  ...
  default:
    toolchain_unreachable("Not a saturated fixed point type!");
  case BuiltinType::SatShortAccum:
    return ShortAccumTy;
  case BuiltinType::SatAccum:
    return AccumTy;
...
QualType ASTContext::getCorrespondingSaturatedType(QualType Ty) const {
  ...
  switch (Ty->castAs<BuiltinType>()->getKind()) {
    default:
      toolchain_unreachable("Not a fixed point type!");
    case BuiltinType::ShortAccum:
      return SatShortAccumTy;
    case BuiltinType::Accum:
      return SatAccumTy;
...
template
language::Core::LazyGenerationalUpdatePtr<
    const Decl *, Decl *, &ExternalASTSource::CompleteRedeclChain>::ValueType
language::Core::LazyGenerationalUpdatePtr<
    const Decl *, Decl *, &ExternalASTSource::CompleteRedeclChain>::makeValue(
        const language::Core::ASTContext &Ctx, Decl *Value);
...
ASTContext::getFixedPointSemantics(QualType Ty) const {
  ...
  if (Ty->isIntegerType())
    return toolchain::FixedPointSemantics::GetIntegerSemantics(
        getIntWidth(Ty), Ty->isSignedIntegerType());

  bool isSigned = Ty->isSignedFixedPointType();
  return toolchain::FixedPointSemantics(
      static_cast<unsigned>(getTypeSize(Ty)), getFixedPointScale(Ty), isSigned,
      Ty->isSaturatedFixedPointType(),
      !isSigned && getTargetInfo().doUnsignedFixedPointTypesHavePadding());
}

toolchain::APFixedPoint ASTContext::getFixedPointMax(QualType Ty) const {
  assert(Ty->isFixedPointType());
  return toolchain::APFixedPoint::getMax(getFixedPointSemantics(Ty));
}

toolchain::APFixedPoint ASTContext::getFixedPointMin(QualType Ty) const {
  assert(Ty->isFixedPointType());
  return toolchain::APFixedPoint::getMin(getFixedPointSemantics(Ty));
}
...
QualType ASTContext::getCorrespondingSignedFixedPointType(QualType Ty) const {
  ...
  case BuiltinType::SatULongFract:
    return SatLongFractTy;
  default:
    toolchain_unreachable("Unexpected unsigned fixed point type");
  }
}
...
static std::vector<std::string> getFMVBackendFeaturesFor(
    const toolchain::SmallVectorImpl<StringRef> &FMVFeatStrings) {
  std::vector<std::string> BackendFeats;
  toolchain::AArch64::ExtensionSet FeatureBits;
  for (StringRef F : FMVFeatStrings)
    if (auto FMVExt = toolchain::AArch64::parseFMVExtension(F))
      if (FMVExt->ID)
...
ASTContext::filterFunctionTargetAttrs(const TargetAttr *TD) const {
  ...
  toolchain::erase_if(ParsedAttr.Features, [&](const std::string &Feat) {
    return !Target->isValidFeatureName(StringRef{Feat}.substr(1));
  });
  return ParsedAttr;
}

void ASTContext::getFunctionFeatureMap(toolchain::StringMap<bool> &FeatureMap,
...
void ASTContext::getFunctionFeatureMap(toolchain::StringMap<bool> &FeatureMap,
  ...
  if (const auto *TD = FD->getAttr<TargetAttr>()) {
    ...
    toolchain::SmallVector<StringRef, 32> FeaturesTmp;
...
const StreamingDiagnostic &language::Core::
...
StringRef ASTContext::getCUIDHash() const {
  ...
  if (LangOpts.CUID.empty())
    return StringRef();
  CUIDHash = toolchain::utohexstr(toolchain::MD5Hash(LangOpts.CUID), /*LowerCase=*/true);
  return CUIDHash;
}
...
bool ASTContext::useAbbreviatedThunkName(GlobalDecl VirtualMethodDecl,
  ...
  toolchain::StringMap<toolchain::SmallVector<std::string, 2>> Thunks;
  ...
  if (const auto *ThunkInfos = VtableContext->getThunkInfo(VirtualMethodDecl)) {
    ...
    for (const auto &Thunk : *ThunkInfos) {
      ...
      toolchain::raw_svector_ostream ElidedNameStream(ElidedName);
      if (Destructor)
      ...
      toolchain::raw_svector_ostream mangledNameStream(MangledName);
      if (Destructor)
        Mangler->mangleCXXDtorThunk(Destructor, VirtualMethodDecl.getDtorType(),
  ...
  toolchain::StringSet<> SimplifiedThunkNames;
  for (auto &ThunkList : Thunks) {
    toolchain::sort(ThunkList.second);
