...
Expr *ParentMapContext::traverseIgnored(Expr *E) const {
  ...
  toolchain_unreachable("Invalid Traversal type!");
}
...
class ParentMapContext::ParentMap {
  ...
  private:
    toolchain::SmallVector<DynTypedNode, 1> Items;
    toolchain::SmallPtrSet<const void *, 2> Dedup;
  };
  ...
  using ParentMapPointers =
      toolchain::DenseMap<const void *,
                     toolchain::PointerUnion<const Decl *, const Stmt *,
                                        DynTypedNode *, ParentVector *>>;

  /// Parent map for nodes without pointer identity. We store a full
  /// DynTypedNode for all keys.
  using ParentMapOtherNodes =
      toolchain::DenseMap<DynTypedNode,
                     toolchain::PointerUnion<const Decl *, const Stmt *,
                                        DynTypedNode *, ParentVector *>>;
...
private:
  ...
  void addParent(MapNodeTy MapNode, MapTy *Parents) {
    ...
    if (NodeOrVector.isNull()) {
      ...
      // toolchain::is_contained() because DynTypedNode::operator== can't compare all
      // types.
      bool Found = ParentStack.back().getMemoizationData() &&
                   toolchain::is_contained(*Vector, ParentStack.back());
      if (!Found)
  ...
  ParentMap &Map;
  toolchain::SmallVector<DynTypedNode, 16> ParentStack;
};
