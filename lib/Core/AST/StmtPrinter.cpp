...
void StmtPrinter::PrintFPPragmas(CompoundStmt *S) {
  ...
  if (FPO.hasConstRoundingModeOverride()) {
    ...
    switch (RM) {
    case toolchain::RoundingMode::TowardZero:
      OS << "FE_TOWARDZERO";
      break;
    case toolchain::RoundingMode::NearestTiesToEven:
      OS << "FE_TONEAREST";
      break;
    case toolchain::RoundingMode::TowardPositive:
      OS << "FE_UPWARD";
      break;
    case toolchain::RoundingMode::TowardNegative:
      OS << "FE_DOWNWARD";
      break;
    case toolchain::RoundingMode::NearestTiesToAway:
      OS << "FE_TONEARESTFROMZERO";
      break;
    case toolchain::RoundingMode::Dynamic:
      OS << "FE_DYNAMIC";
      break;
    default:
      toolchain_unreachable("Invalid rounding mode");
    }
    OS << NL;
  }
}
...
void StmtPrinter::PrintOMPExecutableDirective(OMPExecutableDirective *S,
  ...
  unsigned OpenMPVersion =
      Context ? Context->getLangOpts().OpenMP : toolchain::omp::FallbackVersion;
...
void StmtPrinter::VisitOpenACCWaitConstruct(OpenACCWaitConstruct *S) {
  ...
  if (!S->getLParenLoc().isInvalid()) {
    ...
    toolchain::interleaveComma(S->getQueueIdExprs(), OS, [&](const Expr *E) {
...
void StmtPrinter::VisitOpenACCCacheConstruct(OpenACCCacheConstruct *S) {
  ...
  toolchain::interleaveComma(S->getVarList(), OS, [&](const Expr *E) {
    E->printPretty(OS, nullptr, Policy);
  });
...
void StmtPrinter::VisitIntegerLiteral(IntegerLiteral *Node) {
  ...
  default: toolchain_unreachable("Unexpected type for integer literal!");
...
void StmtPrinter::VisitFixedPointLiteral(FixedPointLiteral *Node) {
  ...
  switch (Node->getType()->castAs<BuiltinType>()->getKind()) {
    default: toolchain_unreachable("Unexpected type for fixed point literal!");
...
static void PrintFloatingLiteral(raw_ostream &OS, FloatingLiteral *Node,
  ...
  switch (Node->getType()->castAs<BuiltinType>()->getKind()) {
  default: toolchain_unreachable("Unexpected type for float literal!");
...
void StmtPrinter::VisitCXXOperatorCallExpr(CXXOperatorCallExpr *Node) {
  ...
  if (Kind == OO_PlusPlus || Kind == OO_MinusMinus) {
    ...
    toolchain_unreachable("unknown overloaded operator");
  }
}
...
void StmtPrinter::VisitLambdaExpr(LambdaExpr *Node) {
  ...
  for (LambdaExpr::capture_iterator C = Node->explicit_capture_begin(),
    ...
    case LCK_VLAType:
      toolchain_unreachable("VLA type in explicit captures.");
    }
    ...
    if (Node->isInitCapture(C)) {
      ...
      toolchain::StringRef Pre;
      toolchain::StringRef Post;
...
void StmtPrinter::VisitCXXNewExpr(CXXNewExpr *E) {
  ...
  std::string TypeS;
  if (E->isArray()) {
    toolchain::raw_string_ostream s(TypeS);
    s << '[';
...
void StmtPrinter::VisitCXXParenListInitExpr(CXXParenListInitExpr *Node) {
  toolchain::interleaveComma(Node->getUserSpecifiedInitExprs(), OS,
                        [&](Expr *E) { PrintExpr(E); });
}
...
void Stmt::dumpPretty(const ASTContext &Context) const {
  printPretty(toolchain::errs(), nullptr, PrintingPolicy(Context.getLangOpts()));
}
