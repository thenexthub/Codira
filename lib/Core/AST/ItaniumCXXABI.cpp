...
struct DecompositionDeclName {
  ...
  struct Iterator
      : toolchain::iterator_adaptor_base<Iterator, BindingArray::const_iterator,
...
template <typename T> static bool isDenseMapKeyEmpty(T V) {
  return toolchain::DenseMapInfo<T>::isEqual(
      V, toolchain::DenseMapInfo<T>::getEmptyKey());
}
template <typename T> static bool isDenseMapKeyTombstone(T V) {
  return toolchain::DenseMapInfo<T>::isEqual(
      V, toolchain::DenseMapInfo<T>::getTombstoneKey());
}
...
struct DenseMapInfo<DecompositionDeclName> {
  using ArrayInfo = toolchain::DenseMapInfo<ArrayRef<const BindingDecl*>>;
  ...
  static unsigned getHashValue(DecompositionDeclName Key) {
    ...
    return toolchain::hash_combine_range(Key);
  }
...
class ItaniumNumberingContext : public MangleNumberingContext {
  ...
  toolchain::StringMap<unsigned> LambdaManglingNumbers;
  unsigned BlockManglingNumber = 0;
  toolchain::DenseMap<const IdentifierInfo *, unsigned> VarManglingNumbers;
  toolchain::DenseMap<const IdentifierInfo *, unsigned> TagManglingNumbers;
  toolchain::DenseMap<DecompositionDeclName, unsigned>
...
public:
  ...
  unsigned getManglingNumber(const CXXMethodDecl *CallOperator) override {
    ...
    toolchain::SmallString<128> LambdaSig;
    toolchain::raw_svector_ostream Out(LambdaSig);
...
class ItaniumSYCLNumberingContext : public ItaniumNumberingContext {
  toolchain::DenseMap<const CXXMethodDecl *, unsigned> ManglingNumbers;
...
public:
  ...
  CallingConv getDefaultMethodCallConv(bool isVariadic) const override {
    ...
    if (!isVariadic && T.isWindowsGNUEnvironment() &&
        T.getArch() == toolchain::Triple::x86)
...
CXXABI *language::Core::CreateItaniumCXXABI(ASTContext &Ctx) {
  return new ItaniumCXXABI(Ctx);
}

std::unique_ptr<MangleNumberingContext>
language::Core::createItaniumNumberingContext(MangleContext *Mangler) {
  return std::make_unique<ItaniumNumberingContext>(
