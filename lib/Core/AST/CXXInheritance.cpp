...
void OverridingMethods::add(unsigned OverriddenSubobject,
  ...
  if (!toolchain::is_contained(SubobjectOverrides, Overriding))
...
class FinalOverriderCollector {
  ...
  toolchain::DenseMap<const CXXRecordDecl *, unsigned> SubobjectCount;

  /// Overriders for each virtual base subobject.
  toolchain::DenseMap<const CXXRecordDecl *, CXXFinalOverriderMap *> VirtualOverriders;
...
void FinalOverriderCollector::Collect(const CXXRecordDecl *RD,
  ...
  for (auto *M : RD->methods()) {
    ...
    using OverriddenMethodsRange =
        toolchain::iterator_range<CXXMethodDecl::method_iterator>;
...
FinalOverriderCollector::~FinalOverriderCollector() {
  for (toolchain::DenseMap<const CXXRecordDecl *, CXXFinalOverriderMap *>::iterator
...
CXXRecordDecl::getFinalOverriders(CXXFinalOverriderMap &FinalOverriders) const {
  ...
  for (auto &OM : FinalOverriders) {
    for (auto &SO : OM.second) {
      ...
      toolchain::erase_if(Overriding, IsHidden);
    }
  }
}
