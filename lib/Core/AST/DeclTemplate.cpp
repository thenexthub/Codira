...
bool TemplateParameterList::containsUnexpandedParameterPack() const {
  ...
  for (const NamedDecl *Param : toolchain::reverse(asArray())) {
    if (!Param->isImplicit())
      break;
...
void TemplateParameterList::Profile(toolchain::FoldingSetNodeID &ID,
...
void TemplateParameterList::getAssociatedConstraints(
    toolchain::SmallVectorImpl<AssociatedConstraint> &ACs) const {
  if (HasConstrainedParameters)
...
TemplateParameterList::getInjectedTemplateArgs(const ASTContext &Context) {
  if (!InjectedArgs) {
    ...
    toolchain::transform(*this, InjectedArgs, [&](NamedDecl *ND) {
...
RedeclarableTemplateDecl::findSpecializationLocally(
    toolchain::FoldingSetVector<EntryType> &Specs, void *&InsertPos,
...
void RedeclarableTemplateDecl::addSpecializationImpl(
    toolchain::FoldingSetVector<EntryType> &Specializations, EntryType *Entry,
    void *InsertPos) {
...
void ClassTemplatePartialSpecializationDecl::Profile(
    toolchain::FoldingSetNodeID &ID, ArrayRef<TemplateArgument> TemplateArgs,
...
void ClassTemplateDecl::getPartialSpecializations(
  ...
  toolchain::FoldingSetVector<ClassTemplatePartialSpecializationDecl> &PartialSpecs
...
TemplateArgumentList::TemplateArgumentList(ArrayRef<TemplateArgument> Args)
  ...
  toolchain::uninitialized_copy(Args, getTrailingObjects());
}
...
ClassTemplateSpecializationDecl::getSourceRange() const {
  ...
  toolchain_unreachable("unhandled template specialization kind");
}
...
void ImplicitConceptSpecializationDecl::setTemplateArguments(
  ...
  toolchain::uninitialized_copy(Converted, getTrailingObjects());
}
...
void VarTemplateDecl::getPartialSpecializations(
  ...
  toolchain::FoldingSetVector<VarTemplatePartialSpecializationDecl> &PartialSpecs =
      getPartialSpecializations();
  PS.clear();
  PS.reserve(PartialSpecs.size());
...
#include "language/Core/Basic/BuiltinTemplates.inc"
  ...
  toolchain_unreachable("unhandled BuiltinTemplateKind!");
}
...
void TemplateParamObjectDecl::printName(toolchain::raw_ostream &OS,
...
void TemplateParamObjectDecl::printAsExpr(toolchain::raw_ostream &OS) const {
  printAsExpr(OS, getASTContext().getPrintingPolicy());
}

void TemplateParamObjectDecl::printAsExpr(toolchain::raw_ostream &OS,
                                          const PrintingPolicy &Policy) const {
  getType().getUnqualifiedType().print(OS, Policy);
  printAsInit(OS, Policy);
}

void TemplateParamObjectDecl::printAsInit(toolchain::raw_ostream &OS) const {
  printAsInit(OS, getASTContext().getPrintingPolicy());
}

void TemplateParamObjectDecl::printAsInit(toolchain::raw_ostream &OS,
                                          const PrintingPolicy &Policy) const {
  getValue().printPretty(OS, Policy, getType(), &getASTContext());
}

TemplateParameterList *language::Core::getReplacedTemplateParameterList(const Decl *D) {
  switch (D->getKind()) {
  ...
  default:
    toolchain_unreachable("Unhandled templated declaration kind");
  }
}
