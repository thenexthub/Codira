...
namespace {
#include "language/Core/AST/CommentHTMLTagsProperties.inc"
} // end anonymous namespace

Sema::Sema(toolchain::BumpPtrAllocator &Allocator, const SourceManager &SourceMgr,
...
static ParamCommandPassDirection getParamPassDirection(StringRef Arg) {
  return toolchain::StringSwitch<ParamCommandPassDirection>(Arg)
...
void Sema::actOnParamCommandDirectionArg(ParamCommandComment *Command,
  ...
  if (Direction == static_cast<ParamCommandPassDirection>(-1)) {
    ...
    toolchain::erase_if(ArgLower, language::Core::isWhitespace);
...
InlineCommandRenderKind Sema::getInlineCommandRenderKind(StringRef Name) const {
  ...
  return toolchain::StringSwitch<InlineCommandRenderKind>(Name)
