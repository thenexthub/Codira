...
StringRef language::Core::getParameterABISpelling(ParameterABI ABI) {
  switch (ABI) {
  case ParameterABI::Ordinary:
    toolchain_unreachable("asking for spelling of ordinary parameter ABI");
  ...
  toolchain_unreachable("bad parameter ABI kind");
}
...
void TypePrinter::printUnaryTransformBefore(const UnaryTransformType *T,
  ...
  static toolchain::DenseMap<int, const char *> Transformation = {{
  ...
  if (const IdentifierInfo *II = D->getIdentifier())
    ...
    if (Policy.AnonymousTagLocations) {
      ...
      if (PLoc.isValid()) {
        ...
        toolchain::SmallString<1024> WrittenFile(File);
        if (auto *Callbacks = Policy.Callbacks)
          WrittenFile = Callbacks->remapPath(File);
        // Fix inconsistent path separator created by
        // language::Core::DirectoryLookup::LookupFile when the file path is relative
        // path.
        toolchain::sys::path::Style Style =
            toolchain::sys::path::is_absolute(WrittenFile)
        ...
        toolchain::sys::path::native(WrittenFile, Style);
...
void TypePrinter::printAttributedBefore(const AttributedType *T,
  ...
  if (T->getImmediateNullability()) {
    if (T->getAttrKind() == attr::TypeNonNull)
      ...
      toolchain_unreachable("unhandled nullability");
    spaceBeforePlaceHolder(OS);
  }
}
...
void TypePrinter::printAttributedAfter(const AttributedType *T,
  ...
  if (T->getAttrKind() == attr::LifetimeBound) {
    OS << " [[language::Core::lifetimebound]]";
    return;
  }
  if (T->getAttrKind() == attr::LifetimeCaptureBy) {
    OS << " [[language::Core::lifetime_capture_by(";
    if (auto *attr = dyn_cast_or_null<LifetimeCaptureByAttr>(T->getAttr()))
      toolchain::interleaveComma(attr->getArgIdents(), OS,
  ...
  if (T->getAttrKind() == attr::AnnotateType) {
    ...
    // here. For the meantime, we just print `[[language::Core::annotate_type(...)]]`
    // without the arguments so that we know at least that we had _some_
    // annotation on the type.
    OS << " [[language::Core::annotate_type(...)]]";
    return;
  }
...
#include "language/Core/Basic/AttrList.inc"
    toolchain_unreachable("non-type attribute attached to type");

  case attr::BTFTypeTag:
    toolchain_unreachable("BTFTypeTag attribute handled separately");

  case attr::HLSLResourceClass:
  case attr::HLSLROV:
  case attr::HLSLRawBuffer:
  case attr::HLSLContainedType:
    toolchain_unreachable("HLSL resource type attributes handled separately");
  ...
  case attr::PreserveNone:
    toolchain_unreachable("This attribute should have been handled already");
...
static void printArgument(const TemplateArgument &A, const PrintingPolicy &PP,
                          toolchain::raw_ostream &OS, bool IncludeType) {
  A.print(PP, OS, IncludeType);
}

static void printArgument(const TemplateArgumentLoc &A,
                          const PrintingPolicy &PP, toolchain::raw_ostream &OS,
                          bool IncludeType) {
...
static bool templateArgumentExpressionsEqual(ASTContext const &Ctx,
  ...
  if (Arg.getKind() == TemplateArgument::Integral)
    return toolchain::APSInt::isSameValue(pattern_expr->EvaluateKnownConstInt(Ctx),
                                     Arg.getAsIntegral());
  ...
  if (Arg.getKind() == TemplateArgument::Expression) {
    ...
    return toolchain::APSInt::isSameValue(args_expr->EvaluateKnownConstInt(Ctx),
...
bool language::Core::isSubstitutedDefaultArgument(ASTContext &Ctx, TemplateArgument Arg,
...
printTo(raw_ostream &OS, ArrayRef<TA> Args, const PrintingPolicy &Policy,
  ...
  if (TPL && Policy.SuppressDefaultTemplateArgs && !Policy.PrintAsCanonical &&
    ...
    toolchain::SmallVector<TemplateArgument, 8> OrigArgs;
    for (const TA &A : Args)
  ...
  for (const auto &Arg : Args) {
    ...
    SmallString<128> Buf;
    toolchain::raw_svector_ostream ArgOS(Buf);
...
void language::Core::printTemplateArgumentList(raw_ostream &OS,
...
std::string PointerAuthQualifier::getAsString(const PrintingPolicy &P) const {
  SmallString<64> Buf;
  toolchain::raw_svector_ostream StrOS(Buf);
  print(StrOS, P);
...
void Qualifiers::print(raw_ostream &OS, const PrintingPolicy& Policy,
  ...
  if (Qualifiers::ObjCLifetime lifetime = getObjCLifetime()) {
    ...
    switch (lifetime) {
    case Qualifiers::OCL_None: toolchain_unreachable("none but true");
...
raw_ostream &language::Core::operator<<(raw_ostream &OS, QualType QT) {
