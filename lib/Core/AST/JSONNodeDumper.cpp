#include "language/Core/AST/JSONNodeDumper.h"
...
void JSONNodeDumper::Visit(const Type *T) {
  ...
  JOS.attribute("kind", (toolchain::Twine(T->getTypeClassName()) + "Type").str());
...
void JSONNodeDumper::Visit(TypeLoc TL) {
  if (TL.isNull())
    return;
  JOS.attribute("kind",
                (toolchain::Twine(TL.getTypeLocClass() == TypeLoc::Qualified
                                 ? "Qualified"
...
void JSONNodeDumper::Visit(const Decl *D) {
  ...
  JOS.attribute("kind", (toolchain::Twine(D->getDeclKindName()) + "Decl").str());
...
std::string JSONNodeDumper::createPointerRepresentation(const void *Ptr) {
  ...
  return "0x" + toolchain::utohexstr(reinterpret_cast<uint64_t>(Ptr), true);
}

toolchain::json::Object JSONNodeDumper::createQualType(QualType QT, bool Desugar) {
  SplitQualType SQT = QT.split();
  std::string SQTS = QualType::getAsString(SQT, PrintPolicy);
  toolchain::json::Object Ret{{"qualType", SQTS}};
...
toolchain::json::Object JSONNodeDumper::createBareDeclRef(const Decl *D) {
  toolchain::json::Object Ret{{"id", createPointerRepresentation(D)}};
  if (!D)
    return Ret;

  Ret["kind"] = (toolchain::Twine(D->getDeclKindName()) + "Decl").str();
  if (const auto *ND = dyn_cast<NamedDecl>(D))
...
toolchain::json::Array JSONNodeDumper::createCastPath(const CastExpr *C) {
  toolchain::json::Array Ret;
  if (C->path_empty())
    return Ret;
  ...
  for (auto I = C->path_begin(), E = C->path_end(); I != E; ++I) {
    ...
    toolchain::json::Object Val{{"name", RD->getName()}};
    if (Base->isVirtual())
      Val["isVirtual"] = true;
...
void JSONNodeDumper::VisitUsingDecl(const UsingDecl *UD) {
  ...
  if (NestedNameSpecifier Qualifier = UD->getQualifier()) {
    toolchain::raw_string_ostream SOS(Name);
...
void JSONNodeDumper::VisitObjCCategoryDecl(const ObjCCategoryDecl *D) {
  ...
  toolchain::json::Array Protocols;
...
void JSONNodeDumper::VisitCastExpr(const CastExpr *CE) {
  ...
  toolchain::json::Array Path = createCastPath(CE);
  if (!Path.empty())
...
void JSONNodeDumper::VisitStringLiteral(const StringLiteral *SL) {
  std::string Buffer;
  toolchain::raw_string_ostream SS(Buffer);
  SL->outputString(SS);
  JOS.attribute("value", Buffer);
}
...
void JSONNodeDumper::visitTParamCommandComment(
  ...
  if (C->isPositionValid()) {
    toolchain::json::Array Positions;
...
toolchain::json::Object JSONNodeDumper::createFPOptions(FPOptionsOverride FPO) {
  toolchain::json::Object Ret;
