...
namespace language::Core {
  ...
  using ExpectedTypePtr = toolchain::Expected<const Type *>;
  using ExpectedType = toolchain::Expected<QualType>;
  using ExpectedStmt = toolchain::Expected<Stmt *>;
  using ExpectedExpr = toolchain::Expected<Expr *>;
  using ExpectedDecl = toolchain::Expected<Decl *>;
  using ExpectedSLoc = toolchain::Expected<SourceLocation>;
  using ExpectedName = toolchain::Expected<DeclarationName>;
  ...
  std::string ASTImportError::toString() const {
    ...
    toolchain_unreachable("Invalid error code.");
    ...
    void addDeclToContexts(Decl *FromD, Decl *ToD) {
      ...
      if (!Visible) {
        if (auto *FromNamed = dyn_cast<NamedDecl>(FromD)) {
          ...
          if (toolchain::is_contained(FromLookup, FromNamed))
...
ASTNodeImporter::import(const TemplateArgument &From) {
  ...
  toolchain_unreachable("Invalid template argument kind");
}
...
ASTNodeImporter::import(concepts::Requirement *FromRequire) {
  ...
  toolchain_unreachable("Unhandled requirement kind");
}
...
ExpectedType
ASTNodeImporter::VisitPackIndexingType(language::Core::PackIndexingType const *T) {
...
ExpectedType language::Core::ASTNodeImporter::VisitAdjustedType(const AdjustedType *T) {
  Error Err = Error::success();
...
ExpectedType language::Core::ASTNodeImporter::VisitBitIntType(const BitIntType *T) {
  return Importer.getToContext().getBitIntType(T->isUnsigned(),
                                               T->getNumBits());
}

ExpectedType language::Core::ASTNodeImporter::VisitBTFTagAttributedType(
    const language::Core::BTFTagAttributedType *T) {
  Error Err = Error::success();
...
ExpectedType language::Core::ASTNodeImporter::VisitHLSLAttributedResourceType(
    const language::Core::HLSLAttributedResourceType *T) {
  Error Err = Error::success();
...
ExpectedType language::Core::ASTNodeImporter::VisitHLSLInlineSpirvType(
    const language::Core::HLSLInlineSpirvType *T) {
  Error Err = Error::success();
  ...
  toolchain::SmallVector<SpirvOperand> ToOperands;
...
ExpectedType language::Core::ASTNodeImporter::VisitConstantMatrixType(
    const language::Core::ConstantMatrixType *T) {
  ExpectedType ToElementTypeOrErr = import(T->getElementType());
  if (!ToElementTypeOrErr)
    return ToElementTypeOrErr.takeError();
...
ExpectedType language::Core::ASTNodeImporter::VisitDependentAddressSpaceType(
    const language::Core::DependentAddressSpaceType *T) {
  Error Err = Error::success();
...
ExpectedType language::Core::ASTNodeImporter::VisitDependentBitIntType(
    const language::Core::DependentBitIntType *T) {
...
ExpectedType language::Core::ASTNodeImporter::VisitPredefinedSugarType(
    const language::Core::PredefinedSugarType *T) {
  return Importer.getToContext().getPredefinedSugarType(T->getKind());
}

ExpectedType language::Core::ASTNodeImporter::VisitDependentSizedMatrixType(
    const language::Core::DependentSizedMatrixType *T) {
  Error Err = Error::success();
  QualType ToElementType = importChecked(Err, T->getElementType());
...
ExpectedType language::Core::ASTNodeImporter::VisitDependentVectorType(
    const language::Core::DependentVectorType *T) {
  Error Err = Error::success();
  QualType ToElementType = importChecked(Err, T->getElementType());
  Expr *ToSizeExpr = importChecked(Err, T->getSizeExpr());
  SourceLocation ToAttrLoc = importChecked(Err, T->getAttributeLoc());
...
ExpectedType language::Core::ASTNodeImporter::VisitObjCTypeParamType(
    const language::Core::ObjCTypeParamType *T) {
...
ExpectedType language::Core::ASTNodeImporter::VisitPipeType(const language::Core::PipeType *T) {
  ExpectedType ToElementTypeOrErr = import(T->getElementType());
  if (!ToElementTypeOrErr)
    return ToElementTypeOrErr.takeError();
...
Error ASTNodeImporter::ImportDefinition(
  ...
  // to be calculated (see 'language::Core::computeDependence') at the time they are
  ...
  auto DefinitionCompleterScopeExit =
      toolchain::make_scope_exit(DefinitionCompleter);
...
Error ASTNodeImporter::ImportTemplateInformation(
  ...
  toolchain_unreachable("All cases should be covered!");
}
...
public:
  ...
  std::optional<bool> VisitIncompleteArrayType(const IncompleteArrayType *T) {
    toolchain_unreachable("Incomplete array should not occur in deduced return type "
                     "of a function");
  }

  std::optional<bool> VisitDependentArrayType(const IncompleteArrayType *T) {
    toolchain_unreachable("Dependent array should not occur in deduced return type "
                     "of a function");
  }
...
private:
  ...
  bool checkTemplateArgument(const TemplateArgument &Arg) {
    ...
    toolchain_unreachable("Unknown TemplateArgument::ArgKind enum");
  };
};
} // namespace
...
ExpectedDecl ASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  if (auto *FromConstructor = dyn_cast<CXXConstructorDecl>(D)) {
    if (unsigned NumInitializers = FromConstructor->getNumCtorInitializers()) {
      ...
      toolchain::copy(CtorInitializers, Memory);
...
ASTNodeImporter::VisitFunctionTemplateDecl(FunctionTemplateDecl *D) {
  ...
  toolchain::transform(*Params, std::back_inserter(OldParamDC),
...
ExpectedStmt ASTNodeImporter::VisitShuffleVectorExpr(ShuffleVectorExpr *E) {
  ...
  toolchain::SmallVector<Expr *, 8> ToSubExprs;
...
ExpectedStmt ASTNodeImporter::VisitExplicitCastExpr(ExplicitCastExpr *E) {
  ...
  default:
    toolchain_unreachable("Cast expression of unsupported type!");
    return make_error<ASTImportError>(ASTImportError::UnsupportedConstruct);
  }
}
...
ExpectedStmt ASTNodeImporter::VisitCallExpr(CallExpr *E) {
  ...
  toolchain::SmallVector<Expr *, 2> ToArgs(NumArgs);
  if (Error Err = ImportContainerChecked(E->arguments(), ToArgs))
     return std::move(Err);
...
ExpectedStmt ASTNodeImporter::VisitCXXNamedCastExpr(CXXNamedCastExpr *E) {
  ...
  if (auto CCE = dyn_cast<CXXStaticCastExpr>(E)) {
    ...
    toolchain_unreachable("Unknown cast type");
...
UnsignedOrNone ASTImporter::getFieldIndex(Decl *F) {
  ...
  toolchain_unreachable("Field was not found in its parent context.");
...
toolchain::Expected<ExprWithCleanups::CleanupObject>
...
ExpectedTypePtr ASTImporter::Import(const Type *FromT) {
  ...
  toolchain::DenseMap<const Type *, const Type *>::iterator Pos =
...
template <typename T> struct AttrArgArrayImporter {
  ...
  AttrArgArrayImporter(ASTNodeImporter &I, Error &Err,
                       const toolchain::iterator_range<T *> &From,
                       unsigned ArraySize) {
    if (Err)
      return;
...
public:
  ...
  template <typename T>
  AttrArgArrayImporter<T> importArrayArg(const toolchain::iterator_range<T *> &From,
                                         unsigned ArraySize) {
  ...
  toolchain::Expected<Attr *> getResult() && {
    if (Err)
      return std::move(Err);
...
Expected<Decl *> ASTImporter::Import(Decl *FromD) {
  ...
  auto ImportPathBuilder =
      toolchain::make_scope_exit([this]() { ImportPath.pop(); });
...
toolchain::Expected<InheritedConstructor>
...
Expected<Stmt *> ASTImporter::Import(Stmt *FromS) {
  ...
  toolchain::DenseMap<Stmt *, Stmt *>::iterator Pos = ImportedStmts.find(FromS);
...
Expected<NestedNameSpecifier> ASTImporter::Import(NestedNameSpecifier FromNNS) {
  ...
  toolchain_unreachable("Invalid nested name specifier kind");
}
...
Expected<TemplateName> ASTImporter::Import(TemplateName From) {
  ...
  toolchain_unreachable("Invalid template name kind");
}
...
Expected<FileID> ASTImporter::Import(FileID FromID, bool IsBuiltin) {
  toolchain::DenseMap<FileID, FileID>::iterator Pos = ImportedFileIDs.find(FromID);
  ...
  if (FromSLoc.isExpansion()) {
    ...
    if (ToID.isInvalid() || IsBuiltin) {
      ...
      std::optional<toolchain::MemoryBufferRef> FromBuf =
          Cache->getBufferOrNone(FromContext.getDiagnostics(),
                                 FromSM.getFileManager(), SourceLocation{});
      if (!FromBuf)
        return toolchain::make_error<ASTImportError>(ASTImportError::Unknown);

      std::unique_ptr<toolchain::MemoryBuffer> ToBuf =
          toolchain::MemoryBuffer::getMemBufferCopy(FromBuf->getBuffer(),
...
toolchain::Expected<APValue> ASTImporter::Import(const APValue &FromValue) {
  ASTNodeImporter Importer(*this);
...
Expected<DeclarationName> ASTImporter::Import(DeclarationName FromName) {
  ...
  toolchain_unreachable("Invalid DeclarationName Kind!");
}
...
bool ASTImporter::IsStructurallyEquivalent(QualType From, QualType To,
  ...
  if (Pos != ImportedTypes.end()) {
    if (ExpectedType ToFromOrErr = Import(From)) {
      ...
      toolchain::consumeError(ToFromOrErr.takeError());
    }
  }
