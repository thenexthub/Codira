...
CXXRewrittenBinaryOperator::getDecomposedForm() const {
  ...
  if (auto *BO = dyn_cast<BinaryOperator>(E)) {
    ...
    default: toolchain_unreachable("unexpected binop in rewritten operator expr");
    }
    Result.LHS = BO->getArg(0);
    Result.RHS = BO->getArg(1);
    Result.InnerBinOp = BO;
  } else {
    toolchain_unreachable("unexpected rewritten operator form");
  }
...
CXXNewExpr::CXXNewExpr(bool IsGlobalNew, FunctionDecl *OperatorNew,
  ...
  CXXNewExprBits.StoredInitializationStyle =
      toolchain::to_underlying(InitializationStyle);
...
static bool UnresolvedLookupExprIsVariableOrConceptParameterPack(
  ...
  if (std::distance(Begin, End) != 1)
    return false;
  NamedDecl *ND = *Begin;
  if (const auto *TTP = toolchain::dyn_cast<TemplateTemplateParmDecl>(ND))
...
CXXDynamicCastExpr *CXXDynamicCastExpr::Create(const ASTContext &C, QualType T,
  ...
  if (PathSize)
    toolchain::uninitialized_copy(*BasePath, E->getTrailingObjects());
  return E;
}
...
UserDefinedLiteral::getLiteralOperatorKind() const {
  ...
  toolchain_unreachable("unknown kind of literal operator");
}
...
CXXConstructExpr::CXXConstructExpr(
  ...
  CXXConstructExprBits.ConstructionKind = toolchain::to_underlying(ConstructKind);
  CXXConstructExprBits.IsImmediateEscalating = false;
  CXXConstructExprBits.Loc = Loc;

  Stmt **TrailingArgs = getTrailingArgs();
  toolchain::copy(Args, TrailingArgs);
  assert(!toolchain::is_contained(Args, nullptr));
...
ExprWithCleanups::ExprWithCleanups(Expr *subexpr,
  ...
  toolchain::copy(objects, getTrailingObjects());
}
...
CXXUnresolvedConstructExpr::CXXUnresolvedConstructExpr(
  ...
  toolchain::copy(Args, StoredArgs);
  setDependence(computeDependence(this));
}
