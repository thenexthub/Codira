...
SourceLocation Expr::getExprLoc() const {
  switch (getStmtClass()) {
  case Stmt::NoStmtClass: toolchain_unreachable("statement without class");
...
#include "language/Core/AST/StmtNodes.inc"
  ...
  toolchain_unreachable("unknown expression kind");
}
...
ConstantExpr::ConstantExpr(Expr *SubExpr, ConstantResultStorageKind StorageKind,
  ...
  ConstantExprBits.ResultKind = toolchain::to_underlying(StorageKind);
...
void ConstantExpr::MoveIntoResult(APValue &Value, const ASTContext &Context) {
  ...
  toolchain_unreachable("Invalid ResultKind Bits");
}

toolchain::APSInt ConstantExpr::getResultAsAPSInt() const {
  switch (getResultStorageKind()) {
  case ConstantResultStorageKind::APValue:
    return APValueResult().getInt();
  case ConstantResultStorageKind::Int64:
    return toolchain::APSInt(toolchain::APInt(ConstantExprBits.BitWidth, Int64Result()),
                        ConstantExprBits.IsUnsigned);
  default:
    toolchain_unreachable("invalid Accessor");
  }
}
...
APValue ConstantExpr::getAPValueResult() const {
  ...
  case ConstantResultStorageKind::Int64:
    return APValue(
        toolchain::APSInt(toolchain::APInt(ConstantExprBits.BitWidth, Int64Result()),
                     ConstantExprBits.IsUnsigned));
  case ConstantResultStorageKind::None:
  ...
  toolchain_unreachable("invalid ResultKind");
}
...
PredefinedExpr::PredefinedExpr(SourceLocation L, QualType FNTy,
  ...
  PredefinedExprBits.Kind = toolchain::to_underlying(IK);
...
StringRef PredefinedExpr::getIdentKindName(PredefinedIdentKind IK) {
  ...
  toolchain_unreachable("Unknown ident kind for PredefinedExpr");
}
...
std::string PredefinedExpr::ComputeName(PredefinedIdentKind IK,
  ...
  if (const FunctionDecl *FD = dyn_cast<FunctionDecl>(CurrentDecl)) {
    ...
    std::string Proto;
    toolchain::raw_string_ostream POut(Proto);
    ...
    toolchain::raw_string_ostream TOut(TemplateParams);
    for (const ClassTemplateSpecializationDecl *D : toolchain::reverse(Specs)) {
  ...
  if (const CapturedDecl *CD = dyn_cast<CapturedDecl>(CurrentDecl)) {
    ...
    toolchain_unreachable("CapturedDecl not inside a function or method");
  }
...
IntegerLiteral::IntegerLiteral(const ASTContext &C, const toolchain::APInt &V,
...
IntegerLiteral *
IntegerLiteral::Create(const ASTContext &C, const toolchain::APInt &V,
                       QualType type, SourceLocation l) {
...
FixedPointLiteral::FixedPointLiteral(const ASTContext &C, const toolchain::APInt &V,
                                     QualType type, SourceLocation l,
                                     unsigned Scale)
...
std::string FixedPointLiteral::getValueAsString(unsigned Radix) const {
  ...
  FixedPointValueToString(
      S, toolchain::APSInt::getUnsigned(getValue().getZExtValue()), Scale);
  return std::string(S);
}
...
void CharacterLiteral::print(unsigned Val, CharacterLiteralKind Kind,
  ...
  if (!Escaped.empty()) {
    ...
    if (Val < 256 && isPrintable((unsigned char)Val))
      ...
      OS << "'\\x" << toolchain::format("%02x", Val) << "'";
    else if (Val <= 0xFFFF)
      OS << "'\\u" << toolchain::format("%04x", Val) << "'";
    else
      OS << "'\\U" << toolchain::format("%08x", Val) << "'";
  }
}

FloatingLiteral::FloatingLiteral(const ASTContext &C, const toolchain::APFloat &V,
...
FloatingLiteral::FloatingLiteral(const ASTContext &C, EmptyShell Empty)
  ...
  setRawSemantics(toolchain::APFloatBase::S_IEEEhalf);
  FloatingLiteralBits.IsExact = false;
}

FloatingLiteral *
FloatingLiteral::Create(const ASTContext &C, const toolchain::APFloat &V,
                        bool isexact, QualType Type, SourceLocation L) {
...
double FloatingLiteral::getValueAsApproximateDouble() const {
  toolchain::APFloat V = getValue();
  bool ignored;
  V.convert(toolchain::APFloat::IEEEdouble(), toolchain::APFloat::rmNearestTiesToEven,
            &ignored);
...
StringLiteral::StringLiteral(const ASTContext &Ctx, StringRef Str,
  ...
  StringLiteralBits.Kind = toolchain::to_underlying(Kind);
  ...
  if (Kind != StringLiteralKind::Unevaluated) {
    ...
    // toolchain_unreachable added just here, it is not able to do so.
  ...
  *getTrailingObjects<unsigned>() = Length;
  ...
  toolchain::copy(Locs, getTrailingObjects<SourceLocation>());

  // Initialize the trailing array of char holding the string data.
  toolchain::copy(Str, getTrailingObjects<char>());
...
#include "language/Core/AST/OperationKinds.def"
  ...
  toolchain_unreachable("Unknown unary operator");
}

UnaryOperatorKind
UnaryOperator::getOverloadedOpcode(OverloadedOperatorKind OO, bool Postfix) {
  switch (OO) {
  default: toolchain_unreachable("No unary operator for overloaded function");
...
static unsigned SizeOfCallExprInstance(Expr::StmtClass SC) {
  ...
  default:
    toolchain_unreachable("unexpected class deriving from CallExpr!");
  }
}
#endif
...
#include "language/Core/AST/StmtNodes.inc"
  default:
    toolchain_unreachable("non-cast expressions not possible here");
  }
}
...
FPOptionsOverride *CastExpr::getTrailingFPFeatures() {
  ...
  case CXXStaticCastExprClass:
    return static_cast<CXXStaticCastExpr *>(this)
        ->getTrailingObjects<FPOptionsOverride>();
  default:
    toolchain_unreachable("Cast does not have FPFeatures");
  }
}
...
ImplicitCastExpr *ImplicitCastExpr::Create(const ASTContext &C, QualType T,
  ...
  if (PathSize)
    toolchain::uninitialized_copy(*BasePath,
...
BinaryOperator::getOverloadedOpcode(OverloadedOperatorKind OO) {
  switch (OO) {
  default: toolchain_unreachable("Not an overloadable binary operator");
...
SourceLocExpr::SourceLocExpr(const ASTContext &Ctx, SourceLocIdentKind Kind,
  ...
  SourceLocExprBits.Kind = toolchain::to_underlying(Kind);
...
StringRef SourceLocExpr::getBuiltinStr() const {
  ...
  toolchain_unreachable("unexpected IdentKind!");
}
...
APValue SourceLocExpr::EvaluateInContext(const ASTContext &Ctx,
  ...
  case SourceLocIdentKind::FileName: {
    ...
    language::Core::Preprocessor::processPathToFileName(
        FileName, PLoc, Ctx.getLangOpts(), Ctx.getTargetInfo());
    return MakeStringLiteral(FileName);
  }
  case SourceLocIdentKind::File: {
    SmallString<256> Path(PLoc.getFilename());
    language::Core::Preprocessor::processPathForFileMacro(Path, Ctx.getLangOpts(),
                                                 Ctx.getTargetInfo());
  ...
  case SourceLocIdentKind::SourceLocStruct: {
    ...
    for (const FieldDecl *F : ImplDecl->fields()) {
      ...
      if (Name == "_M_file_name") {
        ...
        toolchain::APSInt IntVal = Ctx.MakeIntValue(PLoc.getLine(), F->getType());
        Value.getStructField(F->getFieldIndex()) = APValue(IntVal);
      } else if (Name == "_M_column") {
        toolchain::APSInt IntVal = Ctx.MakeIntValue(PLoc.getColumn(), F->getType());
        Value.getStructField(F->getFieldIndex()) = APValue(IntVal);
      }
    }
...
EmbedExpr::EmbedExpr(const ASTContext &Ctx, SourceLocation Loc,
  ...
  FakeChildNode = IntegerLiteral::Create(
      Ctx, toolchain::APInt::getZero(Ctx.getTypeSize(getType())), getType(), Loc);
}
...
SourceLocation InitListExpr::getEndLoc() const {
  ...
  if (End.isInvalid()) {
    ...
    for (Stmt *S : toolchain::reverse(InitExprs)) {
      if (S) {
        End = S->getEndLoc();
        break;
      }
...
bool Expr::HasSideEffects(const ASTContext &Ctx,
  ...
  #define EXPR(Type, Base)
  #include "language/Core/AST/StmtNodes.inc"
    toolchain_unreachable("unexpected Expr kind");
...
Expr::isNullPointerConstant(ASTContext &Ctx,
  ...
  if (isValueDependent() &&
    ...
    case NPC_NeverValueDependent:
      toolchain_unreachable("Unexpected value dependent expression!");
    ...
    case ImplicitCastExprClass: {
      ...
      if (Array1 && Array2) {
        ...
        if (Integer1 && Integer2) {
          if (!toolchain::APInt::isSameValue(Integer1->getValue(),
                                        Integer2->getValue()))
            return false;
        } else {
...
/// getEncodedElementAccess - We encode the fields as a toolchain ConstantArray.
...
GenericSelectionExpr::GenericSelectionExpr(
  ...
  getTrailingObjects<Stmt *>()[getIndexOfControllingExpression()] =
      ControllingExpr;
  toolchain::copy(AssocExprs,
             getTrailingObjects<Stmt *>() + getIndexOfStartOfAssociatedExprs());
  toolchain::copy(AssocTypes, getTrailingObjects<TypeSourceInfo *>() +
...
ParenListExpr::ParenListExpr(SourceLocation LParenLoc, ArrayRef<Expr *> Exprs,
  ...
  toolchain::copy(Exprs, getTrailingObjects());
  setDependence(computeDependence(this));
}
...
getOverflowPatternBinOp(const BinaryOperator *E) {
  ...
  if (BO && BO->getOpcode() == language::Core::BO_Add) {
...
PseudoObjectExpr::PseudoObjectExpr(QualType type, ExprValueKind VK,
  ...
  assert(toolchain::all_of(semantics,
                      [](const Expr *E) {
  ...
  toolchain::copy(semantics, Trail.drop_front().begin());
  setDependence(computeDependence(this));
}
...
unsigned AtomicExpr::getNumSubExprs(AtomicOp Op) {
  ...
  toolchain_unreachable("unknown atomic op");
}
...
RecoveryExpr::RecoveryExpr(ASTContext &Ctx, QualType T, SourceLocation BeginLoc,
  ...
  assert(!T.isNull());
  assert(!toolchain::is_contained(SubExprs, nullptr));

  toolchain::copy(SubExprs, getTrailingObjects());
  setDependence(computeDependence(this));
}
...
void OMPArrayShapingExpr::setDimensions(ArrayRef<Expr *> Dims) {
  ...
  toolchain::copy(Dims, getTrailingObjects<Expr *>());
}
...
void OMPArrayShapingExpr::setBracketsRanges(ArrayRef<SourceRange> BR) {
  assert(
      NumDims == BR.size() &&
      "Preallocated number of dimensions is different from the provided one.");
  toolchain::copy(BR, getTrailingObjects<SourceRange>());
}
