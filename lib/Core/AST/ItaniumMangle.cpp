...
#include "language/Core/Basic/Thunk.h"
...
class ItaniumMangleContextImpl : public ItaniumMangleContext {
  ...
  toolchain::DenseMap<DiscriminatorKeyTy, unsigned> Discriminator;
  toolchain::DenseMap<const NamedDecl*, unsigned> Uniquifier;
...
public:
  ...
  std::string getLambdaString(const CXXRecordDecl *Lambda) override {
    ...
    if (Func) {
      ...
      Name += toolchain::utostr(DefaultArgNo);
      Name += "_";
    }
    ...
    Name += toolchain::utostr(LambdaId);
    Name += '>';
    return Name;
  }
...
class CXXNameMangler {
  ...
  public:
    ...
    void write(raw_ostream &Out, const NamedDecl *ND,
      ...
      if (!isa<FunctionDecl>(ND) && !isa<VarDecl>(ND)) {
        ...
        if (const auto *NS = dyn_cast<NamespaceDecl>(ND)) {
          if (const auto *AbiTag = NS->getAttr<AbiTagAttr>())
            toolchain::append_range(UsedAbiTags, AbiTag->tags());
      ...
      if (const auto *AbiTag = ND->getAttr<AbiTagAttr>()) {
        toolchain::append_range(UsedAbiTags, AbiTag->tags());
        toolchain::append_range(TagList, AbiTag->tags());
      }

      if (AdditionalAbiTags) {
        toolchain::append_range(UsedAbiTags, *AdditionalAbiTags);
        toolchain::append_range(TagList, *AdditionalAbiTags);
      }

      toolchain::sort(TagList);
      TagList.erase(toolchain::unique(TagList), TagList.end());
    ...
    const AbiTagList &getSortedUniqueUsedAbiTags() {
      toolchain::sort(UsedAbiTags);
      UsedAbiTags.erase(toolchain::unique(UsedAbiTags), UsedAbiTags.end());
      return UsedAbiTags;
    }
  ...
  toolchain::DenseMap<uintptr_t, unsigned> Substitutions;
  toolchain::DenseMap<StringRef, unsigned> ModuleSubstitutions;
  ...
  bool isSpecializedAs(QualType S, toolchain::StringRef Name, QualType A);
  bool isStdCharSpecialization(const ClassTemplateSpecializationDecl *SD,
                               toolchain::StringRef Name, bool HasAllocator);
...
public:
  ...
  CXXNameMangler(CXXNameMangler &Outer, toolchain::raw_null_ostream &Out_)
  ...
  void mangleNumber(const toolchain::APSInt &I);
  void mangleNumber(int64_t Number);
  void mangleFloat(const toolchain::APFloat &F);
...
#include "language/Core/AST/TypeNodes.inc"
  ...
  void mangleIntegerLiteral(QualType T, const toolchain::APSInt &Value);
  void mangleFloatLiteral(QualType T, const toolchain::APFloat &V);
...
ItaniumMangleContextImpl::getEffectiveDeclContext(const Decl *D) {
  ...
  if (D == getASTContext().getVaListTagDecl()) {
    const toolchain::Triple &T = getASTContext().getTargetInfo().getTriple();
...
void CXXNameMangler::mangle(GlobalDecl GD) {
  ...
  if (isa<FunctionDecl>(GD.getDecl()))
    ...
    toolchain_unreachable("unexpected kind of global decl");
}
...
void CXXNameMangler::mangleFunctionEncoding(GlobalDecl GD) {
  ...
  toolchain::raw_svector_ostream FunctionEncodingStream(FunctionEncodingBuf);
...
void CXXNameMangler::mangleName(GlobalDecl GD) {
  ...
  if (const VarDecl *VD = dyn_cast<VarDecl>(ND)) {
    ...
    toolchain::raw_null_ostream NullOutStream;
    ...
    if (getASTContext().getTargetInfo().getTriple().isOSSolaris()) {
      ...
      if (const RecordDecl *RD = dyn_cast<RecordDecl>(ND)) {
        ...
        if (const IdentifierInfo *II = RD->getIdentifier()) {
          ...
          if (toolchain::is_contained({"div_t", "ldiv_t", "lconv", "tm"}, type)) {
            Out << type.size() << type;
            return;
          }
        }
...
void CXXNameMangler::mangleFloat(const toolchain::APFloat &f) {
  // ABI:
  ...
  toolchain::APInt valueBits = f.bitcastToAPInt();
...
void CXXNameMangler::mangleFloatLiteral(QualType T, const toolchain::APFloat &V) {
  Out << 'L';
  mangleType(T);
  mangleFloat(V);
  Out << 'E';
...
void CXXNameMangler::mangleNumber(const toolchain::APSInt &Value) {
...
void CXXNameMangler::mangleUnresolvedPrefix(NestedNameSpecifier Qualifier,
  ...
  case NestedNameSpecifier::Kind::MicrosoftSuper:
    toolchain_unreachable("Can't mangle __super specifier");
...
void CXXNameMangler::mangleUnresolvedName(
  ...
  switch (name.getNameKind()) {
    ...
    case DeclarationName::CXXConstructorName:
      toolchain_unreachable("Can't mangle a constructor name!");
    case DeclarationName::CXXUsingDirective:
      toolchain_unreachable("Can't mangle a using directive name!");
    case DeclarationName::CXXDeductionGuideName:
      toolchain_unreachable("Can't mangle a deduction guide name!");
    case DeclarationName::ObjCMultiArgSelector:
    case DeclarationName::ObjCOneArgSelector:
    case DeclarationName::ObjCZeroArgSelector:
      toolchain_unreachable("Can't mangle Objective-C selector names here!");
  }
...
void CXXNameMangler::mangleUnqualifiedName(
  ...
  case DeclarationName::Identifier: {
    ...
    if (auto *GD = dyn_cast<MSGuidDecl>(ND)) {
      ...
      toolchain::raw_svector_ostream GUIDOS(GUID);
    ...
    SmallString<8> Str;
    Str += "$_";
    Str += toolchain::utostr(AnonStructId);
...
void CXXNameMangler::mangleTypeConstraint(const TypeConstraint *Constraint) {
  toolchain::SmallVector<TemplateArgument, 8> Args;
...
void CXXNameMangler::manglePrefix(NestedNameSpecifier Qualifier) {
  ...
  toolchain_unreachable("unexpected nested name specifier");
}
...
void CXXNameMangler::mangleType(TemplateName TN) {
  ...
  case TemplateName::AssumedTemplate:
    toolchain_unreachable("can't mangle an overloaded template name as a <type>");
  ...
  case TemplateName::DeducedTemplate:
    toolchain_unreachable("Unexpected DeducedTemplate");
  }
...
bool CXXNameMangler::mangleUnresolvedTypeOrSimpleId(QualType Ty,
  ...
  case Type::CountAttributed:
    toolchain_unreachable("type is illegal as a nested name specifier");
  ...
  case Type::TemplateSpecialization: {
    ...
    case TemplateName::DependentTemplate:
    case TemplateName::DeducedTemplate:
      toolchain_unreachable("invalid base for a template specialization type");
...
void CXXNameMangler::mangleOperatorName(DeclarationName Name, unsigned Arity) {
  ...
  case DeclarationName::ObjCOneArgSelector:
  case DeclarationName::ObjCZeroArgSelector:
    toolchain_unreachable("Not an operator name");
...
CXXNameMangler::mangleOperatorName(OverloadedOperatorKind OO, unsigned Arity) {
  ...
  case NUM_OVERLOADED_OPERATORS:
    toolchain_unreachable("Not an overloaded operator");
  }
}
  ...
  if (Quals.hasAddressSpace()) {
    ...
    if (Context.getASTContext().addressSpaceMapManglingFor(AS)) {
      ...
      if (TargetAS != 0 ||
        ...
        ASString = "AS" + toolchain::utostr(TargetAS);
    } else {
      switch (AS) {
      default: toolchain_unreachable("Not a language specific address space");
...
#define NON_CANONICAL_TYPE(CLASS, PARENT) \
    case Type::CLASS: \
      toolchain_unreachable("can't mangle non-canonical type " #CLASS "Type"); \
      return;
  ...
  if (NormalizeIntegers && T->isInteger()) {
    if (T->isSignedInteger()) {
      ...
      case 128:
        ...
        break;
      default:
        toolchain_unreachable("Unknown integer size for normalization");
      }
    } else {
      switch (getASTContext().getTypeSize(T)) {
      case 8:
...
#include "language/Core/AST/BuiltinTypes.def"
  case BuiltinType::Dependent:
    if (!NullOut)
      toolchain_unreachable("mangling a placeholder type");
    break;
...
static AAPCSBitmaskSME encodeAAPCSZAState(unsigned SMEAttrs) {
  ...
  default:
    toolchain_unreachable("Unrecognised SME attribute");
  }
}
...
void CXXNameMangler::mangleNeonVectorType(const VectorType *T) {
  ...
  if (T->getVectorKind() == VectorKind::NeonPoly) {
    ...
    case BuiltinType::ULongLong:
      EltName = "poly64_t";
      break;
    default: toolchain_unreachable("unexpected Neon polynomial vector element type");
    }
  } else {
    switch (cast<BuiltinType>(EltType)->getKind()) {
    ...
    case BuiltinType::MFloat8:
      ...
      break;
    default:
      toolchain_unreachable("unexpected Neon vector element type");
    }
  }
...
static StringRef mangleAArch64VectorBase(const BuiltinType *EltType) {
  ...
  case BuiltinType::MFloat8:
    return "Mfloat8";
  default:
    toolchain_unreachable("Unexpected vector element base type");
  }
}
...
void CXXNameMangler::mangleAArch64NeonVectorType(const VectorType *T) {
  ...
  if (T->getVectorKind() == VectorKind::NeonPoly) {
    ...
    case BuiltinType::ULongLong:
      ...
      break;
    default:
      toolchain_unreachable("unexpected Neon polynomial vector element type");
    }
  } else
...
void CXXNameMangler::mangleAArch64FixedSveVectorType(const VectorType *T) {
  ...
  case BuiltinType::BFloat16:
    ...
    break;
  default:
    toolchain_unreachable("unexpected element type for fixed-length SVE vector!");
  }
...
void CXXNameMangler::mangleRISCVFixedRVVVectorType(const VectorType *T) {
  ...
  toolchain::raw_svector_ostream TypeNameOS(TypeNameStr);
  TypeNameOS << "__rvv_";
  switch (cast<BuiltinType>(EltType)->getKind()) {
  case BuiltinType::SChar:
    TypeNameOS << "int8";
  ...
  case BuiltinType::Double:
    ...
    break;
  default:
    toolchain_unreachable("unexpected element type for fixed-length RVV vector!");
  }
  ...
  unsigned VLen = VScale->first * toolchain::RISCV::RVVBitsPerBlock;
...
void CXXNameMangler::mangleType(const VectorType *T) {
  if ((T->getVectorKind() == VectorKind::Neon ||
    ...
    toolchain::Triple Target = getASTContext().getTargetInfo().getTriple();
    toolchain::Triple::ArchType Arch =
        getASTContext().getTargetInfo().getTriple().getArch();
    if ((Arch == toolchain::Triple::aarch64 ||
         Arch == toolchain::Triple::aarch64_be) && !Target.isOSDarwin())
...
void CXXNameMangler::mangleType(const ConstantMatrixType *T) {
  ...
  toolchain::APSInt Rows(BitWidth);
  Rows = T->getNumRows();
  mangleIntegerLiteral(ASTCtx.getSizeType(), Rows);
  toolchain::APSInt Columns(BitWidth);
...
void CXXNameMangler::mangleType(const ObjCObjectType *T) {
  ...
  if (!T->qual_empty()) {
    ...
    toolchain::raw_svector_ostream QualOS(QualStr);
    QualOS << "objcproto";
...
void CXXNameMangler::mangleType(const HLSLAttributedResourceType *T) {
  toolchain::SmallString<64> Str("_Res");
  ...
  for (auto &Operand : T->getOperands()) {
    ...
    case SpirvOperandKind::ConstantId:
      ...
      mangleIntegerLiteral(Operand.getResultType(),
                           toolchain::APSInt(Operand.getValue()));
      break;
    ...
    case SpirvOperandKind::TypeId:
      ...
      break;
    default:
      toolchain_unreachable("Invalid SpirvOperand kind");
      break;
    }
...
#include "language/Core/AST/StmtNodes.inc"
  ...
  case Expr::PackIndexingExprClass:
    toolchain_unreachable("unexpected statement kind");
  ...
  case Expr::OpaqueValueExprClass:
    toolchain_unreachable("cannot mangle opaque value; mangling wrong thing?");
  ...
  case Expr::UnaryExprOrTypeTraitExprClass: {
    ...
    if (!SAE->isInstantiationDependent()) {
      ...
      toolchain::APSInt V = SAE->EvaluateKnownConstInt(Context.getASTContext());
  ...
  case Expr::TypeTraitExprClass: {
    ...
    NotPrimaryExpr();
    toolchain::StringRef Spelling = getTraitSpelling(TTE->getTrait());
  ...
  case Expr::IntegerLiteralClass: {
    // <expr-primary>
    toolchain::APSInt Value(cast<IntegerLiteral>(E)->getValue());
  ...
  case Expr::ImaginaryLiteralClass: {
    ...
    if (const FloatingLiteral *Imag =
      ...
      mangleFloat(toolchain::APFloat(Imag->getValue().getSemantics()));
      Out << '_';
      mangleFloat(Imag->getValue());
    } else {
      Out << "0_";
      toolchain::APSInt Value(cast<IntegerLiteral>(IE->getSubExpr())->getValue());
  ...
  case Expr::GNUNullExprClass:
    ...
    mangleIntegerLiteral(E->getType(), toolchain::APSInt(32));
    break;
...
void CXXNameMangler::mangleCXXCtorType(CXXCtorType T,
  ...
  case Ctor_CopyingClosure:
    toolchain_unreachable("closure constructors don't exist for the Itanium ABI!");
  }
  if (InheritedFrom)
...
struct CXXNameMangler::TemplateArgManglingInfo {
  ...
  bool needToMangleTemplateParam(const NamedDecl *Param,
    ...
    toolchain::SmallString<128> ParamTemplateHead, ArgTemplateHead;
...
void CXXNameMangler::mangleTemplateArg(TemplateArgument A, bool NeedExactType) {
  ...
  case TemplateArgument::Null:
    toolchain_unreachable("Cannot mangle NULL template argument");
...
static bool isZeroInitialized(QualType T, const APValue &V) {
  ...
  toolchain_unreachable("Unhandled APValue::ValueKind enum");
}
...
void CXXNameMangler::mangleValueInTemplateArg(QualType T, const APValue &V,
  ...
  case APValue::AddrLabelDiff:
    toolchain_unreachable("unexpected value kind in template argument");
  ...
  case APValue::Struct: {
    ...
    toolchain::SmallVector<const FieldDecl *, 16> Fields(RD->fields());
    while (
        !Fields.empty() &&
  ...
  case APValue::LValue: {
    ...
    if (auto *VD = B.dyn_cast<const ValueDecl*>()) {
      ...
      toolchain_unreachable("unexpected lvalue base kind in template argument");
    }
...
bool CXXNameMangler::mangleSubstitution(uintptr_t Ptr) {
  toolchain::DenseMap<uintptr_t, unsigned>::iterator I = Substitutions.find(Ptr);
...
/// argument of type A.
bool CXXNameMangler::isSpecializedAs(QualType S, toolchain::StringRef Name,
                                     QualType A) {
  if (S.isNull())
    return false;
...
bool CXXNameMangler::isStdCharSpecialization(
    const ClassTemplateSpecializationDecl *SD, toolchain::StringRef Name,
    bool HasAllocator) {
...
void ItaniumMangleContextImpl::mangleStringLiteral(const StringLiteral *, raw_ostream &) {
  toolchain_unreachable("Can't mangle string literals");
}
