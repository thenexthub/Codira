...
ArrayType::ArrayType(TypeClass tc, QualType et, QualType can,
  ...
  ArrayTypeBits.SizeModifier = toolchain::to_underlying(sm);
}

ConstantArrayType *
ConstantArrayType::Create(const ASTContext &Ctx, QualType ET, QualType Can,
                          const toolchain::APInt &Sz, const Expr *SzExpr,
...
ConstantArrayType::getNumAddressingBits(const ASTContext &Context,
  ...
  if (toolchain::isPowerOf2_64(ElementSize)) {
    return NumElements.getActiveBits() + toolchain::Log2_64(ElementSize);
  }
  ...
  if ((ElementSize >> 32) == 0 && NumElements.getBitWidth() <= 64 &&
    ...
    return toolchain::bit_width(TotalSize);
  }

  // Otherwise, use APSInt to handle arbitrary sized values.
  toolchain::APSInt SizeExtended(NumElements, true);
  unsigned SizeTypeBits = Context.getTypeSize(Context.getSizeType());
  SizeExtended = SizeExtended.extend(
      std::max(SizeTypeBits, SizeExtended.getBitWidth()) * 2);

  toolchain::APSInt TotalSize(toolchain::APInt(SizeExtended.getBitWidth(), ElementSize));
...
void ConstantArrayType::Profile(toolchain::FoldingSetNodeID &ID,
...
void DependentSizedArrayType::Profile(toolchain::FoldingSetNodeID &ID,
                                      const ASTContext &Context, QualType ET,
...
DependentVectorType::DependentVectorType(QualType ElementType,
  ...
  VectorTypeBits.VecKind = toolchain::to_underlying(VecKind);
}

void DependentVectorType::Profile(toolchain::FoldingSetNodeID &ID,
...
void DependentSizedExtVectorType::Profile(toolchain::FoldingSetNodeID &ID,
                                          const ASTContext &Context,
                                          QualType ElementType,
                                          Expr *SizeExpr) {
  ID.AddPointer(ElementType.getAsOpaquePtr());
...
void DependentAddressSpaceType::Profile(toolchain::FoldingSetNodeID &ID,
                                        const ASTContext &Context,
...
void DependentSizedMatrixType::Profile(toolchain::FoldingSetNodeID &ID,
                                       const ASTContext &CTX,
...
language::Core::Expr *DependentBitIntType::getNumBitsExpr() const {
  return ExprAndUnsigned.getPointer();
}

void DependentBitIntType::Profile(toolchain::FoldingSetNodeID &ID,
...
bool BoundsAttributedType::referencesFieldDecls() const {
  return toolchain::any_of(dependent_decls(),
                      [](const TypeCoupledDeclRefInfo &Info) {
                        return isa<FieldDecl>(Info.getDecl());
                      });
}

void CountAttributedType::Profile(toolchain::FoldingSetNodeID &ID,
...
#define SUGARED_TYPE_CLASS(Class)                                              \
    ...
    if (info.ExceptionSpec.Type == EST_Dynamic) {
      ...
      if (exceptionChanged) {
        info.ExceptionSpec.Exceptions =
            toolchain::ArrayRef(exceptionTypes).copy(Ctx);
      }
    }
  ...
  QualType VisitObjCObjectType(const ObjCObjectType *T) {
    ...
    return Ctx.getObjCObjectType(
        baseType, typeArgs,
        toolchain::ArrayRef(T->qual_begin(), T->getNumProtocols()),
...
struct SubstObjCTypeArgsVisitor
  ...
  QualType VisitObjCTypeParamType(const ObjCTypeParamType *OTPTy) {
    ...
    toolchain_unreachable("Unexpected ObjCSubstitutionContext!");
  }
...
Type::getObjCSubstitutions(const DeclContext *dc) const {
  ...
  if (!curClassDecl) {
    ...
    return toolchain::ArrayRef<QualType>();
  }
...
Type::ScalarTypeKind Type::getScalarTypeKind() const {
  ...
  if (const auto *BT = dyn_cast<BuiltinType>(T)) {
    ...
    toolchain_unreachable("unknown scalar builtin type");
  ...
  toolchain_unreachable("unknown scalar type");
}
...
TagTypeKind KeywordHelpers::getTagTypeKindForTypeSpec(unsigned TypeSpec) {
  ...
  toolchain_unreachable("Type specifier is not a tag type kind.");
}
...
KeywordHelpers::getKeywordForTagTypeKind(TagTypeKind Kind) {
  ...
  toolchain_unreachable("Unknown tag type kind.");
}
...
KeywordHelpers::getTagTypeKindForKeyword(ElaboratedTypeKeyword Keyword) {
  ...
  case ElaboratedTypeKeyword::Typename:
    toolchain_unreachable("Elaborated type keyword is not a tag type kind.");
  }
  toolchain_unreachable("Unknown elaborated type keyword.");
}
...
void DependentTemplateSpecializationType::Profile(
    toolchain::FoldingSetNodeID &ID, const ASTContext &Context,
    ElaboratedTypeKeyword Keyword, const DependentTemplateStorage &Name,
...
#include "language/Core/Basic/HLSLIntangibleTypes.def"
  ...
  toolchain_unreachable("Invalid builtin type.");
}
...
#undef CC_VLS_CASE
  ...
  toolchain_unreachable("Invalid calling convention.");
}
...
FunctionProtoType::FunctionProtoType(QualType result, ArrayRef<QualType> params,
  ...
  if (!epi.FunctionEffects.empty()) {
    ...
    toolchain::uninitialized_copy(SrcFX, DestFX);
    ...
    if (!SrcConds.empty()) {
      ...
      toolchain::uninitialized_copy(SrcConds, DestConds);
      assert(toolchain::any_of(SrcConds,
...
CanThrowResult FunctionProtoType::canThrow() const {
  ...
  case EST_Unevaluated:
    toolchain_unreachable("should not call this with unresolved exception specs");
  ...
  toolchain_unreachable("unexpected exception specification kind");
}
...
void FunctionProtoType::Profile(toolchain::FoldingSetNodeID &ID, QualType Result,
...
void FunctionProtoType::Profile(toolchain::FoldingSetNodeID &ID,
                                const ASTContext &Ctx) {
...
CountAttributedType::CountAttributedType(
  ...
  toolchain::copy(CoupledDecls, DeclSlot);
  Decls = toolchain::ArrayRef(DeclSlot, CoupledDecls.size());
}
...
void DependentTypeOfExprType::Profile(toolchain::FoldingSetNodeID &ID,
...
void DependentDecltypeType::Profile(toolchain::FoldingSetNodeID &ID,
                                    const ASTContext &Context, Expr *E) {
  E->Profile(ID, Context, true);
}
...
PackIndexingType::PackIndexingType(QualType Canonical, QualType Pattern,
  ...
  toolchain::uninitialized_copy(Expansions, getTrailingObjects());
}
...
void PackIndexingType::Profile(toolchain::FoldingSetNodeID &ID,
                               const ASTContext &Context) {
...
NestedNameSpecifier &TagType::getTrailingQualifier() const {
  ...
  return *reinterpret_cast<NestedNameSpecifier *>(toolchain::alignAddr(
      getTrailingPointer(), toolchain::Align::Of<NestedNameSpecifier *>()));
}
...
void SubstTemplateTypeParmType::Profile(toolchain::FoldingSetNodeID &ID,
...
TemplateArgument SubstTemplateTypeParmPackType::getArgumentPack() const {
  return TemplateArgument(toolchain::ArrayRef(Arguments, getNumArgs()));
}

void SubstTemplateTypeParmPackType::Profile(toolchain::FoldingSetNodeID &ID) {
...
void TemplateSpecializationType::Profile(toolchain::FoldingSetNodeID &ID,
                                         const ASTContext &Ctx) {
...
void ObjCObjectTypeImpl::Profile(toolchain::FoldingSetNodeID &ID, QualType BaseType,
                                 ArrayRef<QualType> typeArgs,
                                 ArrayRef<ObjCProtocolDecl *> protocols,
                                 bool isKindOf) {
...
void ObjCObjectTypeImpl::Profile(toolchain::FoldingSetNodeID &ID) {
  Profile(ID, getBaseType(), getTypeArgsAsWritten(),
          toolchain::ArrayRef(qual_begin(), getNumProtocols()),
          isKindOfTypeAsWritten());
}

void ObjCTypeParamType::Profile(toolchain::FoldingSetNodeID &ID,
...
public:
  ...
  static void ensure(const Type *T) {
    ...
    T->TypeBits.CacheValid = true;
    T->TypeBits.CachedLinkage = toolchain::to_underlying(Result.getLinkage());
...
#include "language/Core/AST/TypeNodes.inc"
    toolchain_unreachable("didn't expect a non-canonical type here");
...
#include "language/Core/Basic/HLSLIntangibleTypes.def"
  ...
  case Type::Record: {
    ...
    return toolchain::any_of(RD->redecls(), [](const TagDecl *RD) {
...
void MemberPointerType::Profile(toolchain::FoldingSetNodeID &ID, QualType Pointee,
...
void language::Core::FixedPointValueToString(SmallVectorImpl<char> &Str,
                                    toolchain::APSInt Val, unsigned Scale) {
  toolchain::FixedPointSemantics FXSema(Val.getBitWidth(), Scale, Val.isSigned(),
                                   /*IsSaturated=*/false,
                                   /*HasUnsignedPadding=*/false);
  toolchain::APFixedPoint(Val, FXSema).toString(Str);
}
...
AutoType::AutoType(QualType DeducedAsType, AutoTypeKeyword Keyword,
  ...
  AutoTypeBits.Keyword = toolchain::to_underlying(Keyword);
...
void AutoType::Profile(toolchain::FoldingSetNodeID &ID, const ASTContext &Context,
...
LLVM_DUMP_METHOD void FunctionEffectsRef::dump(toolchain::raw_ostream &OS) const {
  OS << "Effects{";
  toolchain::interleaveComma(*this, OS);
  OS << "}";
}

LLVM_DUMP_METHOD void FunctionEffectSet::dump(toolchain::raw_ostream &OS) const {
  FunctionEffectsRef(*this).dump(OS);
}

LLVM_DUMP_METHOD void FunctionEffectKindSet::dump(toolchain::raw_ostream &OS) const {
  OS << "Effects{";
  toolchain::interleaveComma(*this, OS);
  OS << "}";
}
...
FunctionEffectsRef::create(ArrayRef<FunctionEffect> FX,
  ...
  assert(toolchain::is_sorted(FX) && "effects should be sorted");
...
HLSLAttributedResourceType::findHandleTypeOnResource(const Type *RT) {
  ...
  const language::Core::Type *Ty = RT->getUnqualifiedDesugaredType();
