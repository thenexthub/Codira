...
void ODRHash::AddNestedNameSpecifier(NestedNameSpecifier NNS) {
  ...
  ID.AddInteger(toolchain::to_underlying(Kind));
  switch (Kind) {
...
void ODRHash::AddTemplateArgument(TemplateArgument TA) {
  ...
  switch (Kind) {
    case TemplateArgument::Null:
      toolchain_unreachable("Expected valid TemplateArgument");
...
public:
  ODRDeclVisitor(toolchain::FoldingSetNodeID &ID, ODRHash &Hash)
      : ID(ID), Hash(Hash) {}
  ...
  void VisitObjCMethodDecl(const ObjCMethodDecl *Method) {
    ...
    ID.AddInteger(toolchain::to_underlying(Method->getImplementationControl()));
    ID.AddInteger(Method->getMethodFamily());
    ImplicitParamDecl *Cmd = Method->getCmdDecl();
    Hash.AddBoolean(Cmd);
    if (Cmd)
      ID.AddInteger(toolchain::to_underlying(Cmd->getParameterKind()));

    ImplicitParamDecl *Self = Method->getSelfDecl();
    Hash.AddBoolean(Self);
    if (Self)
      ID.AddInteger(toolchain::to_underlying(Self->getParameterKind()));
    ...
    if (Method->hasBody()) {
      ...
      if (IsDefinition) {
        ...
        toolchain::SmallVector<const Decl *, 16> Decls;
...
public:
  ODRTypeVisitor(toolchain::FoldingSetNodeID &ID, ODRHash &Hash)
      : ID(ID), Hash(Hash) {}
  ...
  void VisitArrayType(const ArrayType *T) {
    ...
    ID.AddInteger(toolchain::to_underlying(T->getSizeModifier()));
  ...
  void VisitTagType(const TagType *T,
    ...
    ID.AddInteger(toolchain::to_underlying(
  ...
  void VisitTypedefType(const TypedefType *T) {
    ID.AddInteger(toolchain::to_underlying(T->getKeyword()));
    AddNestedNameSpecifier(T->getQualifier());
    AddDecl(T->getDecl());
    VisitType(T);
  }
  ...
  void VisitVectorType(const VectorType *T) {
    ...
    ID.AddInteger(toolchain::to_underlying(T->getVectorKind()));
    VisitType(T);
  }
