...
void TypeInfoLValue::print(toolchain::raw_ostream &Out,
...
static_assert(
    1 << toolchain::PointerLikeTypeTraits<TypeInfoLValue>::NumLowBitsAvailable <=
        alignof(Type),
...
QualType APValue::LValueBase::getType() const {
  ...
  if (const MaterializeTemporaryExpr *MTE =
          toolchain::dyn_cast<MaterializeTemporaryExpr>(Base)) {
...
void APValue::LValueBase::Profile(toolchain::FoldingSetNodeID &ID) const {
...
void APValue::LValuePathEntry::Profile(toolchain::FoldingSetNodeID &ID) const {
  ID.AddInteger(Value);
}
...
language::Core::APValue::LValueBase
toolchain::DenseMapInfo<language::Core::APValue::LValueBase>::getEmptyKey() {
  language::Core::APValue::LValueBase B;
  B.Ptr = DenseMapInfo<const ValueDecl*>::getEmptyKey();
  return B;
}

language::Core::APValue::LValueBase
toolchain::DenseMapInfo<language::Core::APValue::LValueBase>::getTombstoneKey() {
  language::Core::APValue::LValueBase B;
...
namespace language::Core {
toolchain::hash_code hash_value(const APValue::LValueBase &Base) {
  if (Base.is<TypeInfoLValue>() || Base.is<DynamicAllocLValue>())
    return toolchain::hash_value(Base.getOpaqueValue());
  return toolchain::hash_combine(Base.getOpaqueValue(), Base.getCallIndex(),
                            Base.getVersion());
}
}

unsigned toolchain::DenseMapInfo<language::Core::APValue::LValueBase>::getHashValue(
    const language::Core::APValue::LValueBase &Base) {
  return hash_value(Base);
}

bool toolchain::DenseMapInfo<language::Core::APValue::LValueBase>::isEqual(
    const language::Core::APValue::LValueBase &LHS,
...
namespace {
  struct MemberPointerBase {
    toolchain::PointerIntPair<const ValueDecl*, 1, bool> MemberAndIsDerivedMember;
    unsigned PathLength;
  };
}
...
static void profileIntValue(toolchain::FoldingSetNodeID &ID, const toolchain::APInt &V) {
  for (unsigned I = 0, N = V.getBitWidth(); I < N; I += 32)
    ID.AddInteger((uint32_t)V.extractBitsAsZExtValue(std::min(32u, N - I), I));
}

void APValue::Profile(toolchain::FoldingSetNodeID &ID) const {
  ...
  case Array: {
    ...
    //   [5, 'x', 3, 'c', 'a']
    toolchain::FoldingSetNodeID FillerID;
    ...
    while (true) {
      ...
      // element.
      if (N != getArraySize()) {
        toolchain::FoldingSetNodeID ElemID;
...
static double GetApproxValue(const toolchain::APFloat &F) {
  toolchain::APFloat V = F;
  bool ignored;
