...
class EmptySubobjectMap {
  ...
  typedef toolchain::TinyPtrVector<const CXXRecordDecl *> ClassVectorTy;
  typedef toolchain::DenseMap<CharUnits, ClassVectorTy> EmptyClassOffsetsMapTy;
...
EmptySubobjectMap::CanPlaceSubobjectAtOffset(const CXXRecordDecl *RD,
  ...
  if (!toolchain::is_contained(Classes, RD))
    return true;
...
typedef toolchain::SmallPtrSet<const CXXRecordDecl*, 4> ClassSetTy;

class ItaniumRecordLayoutBuilder {
protected:
  // FIXME: Remove this and make the appropriate fields public.
  friend class language::Core::ASTContext;
  ...
  typedef toolchain::DenseMap<const CXXRecordDecl *, CharUnits> BaseOffsetsMapTy;
  ...
  toolchain::SpecificBumpPtrAllocator<BaseSubobjectInfo> BaseSubobjectInfoAllocator;

  typedef toolchain::DenseMap<const CXXRecordDecl *, BaseSubobjectInfo *>
...
void ItaniumRecordLayoutBuilder::Layout(const CXXRecordDecl *RD) {
  ...
  NonVirtualSize = Context.toCharUnitsFromBits(
      toolchain::alignTo(getSizeInBits(), Context.getTargetInfo().getCharAlign()));
...
roundUpSizeToCharAlignment(uint64_t Size,
  ...
  return toolchain::alignTo(Size, CharAlignment);
}
...
void ItaniumRecordLayoutBuilder::LayoutWideBitField(uint64_t FieldSize,
  ...
  if (IsUnion) {
    ...
    FieldOffset = toolchain::alignTo(getDataSizeInBits(), Context.toBits(TypeAlign));

    uint64_t NewSizeInBits = FieldOffset + FieldSize;

    setDataSize(
        toolchain::alignTo(NewSizeInBits, Context.getTargetInfo().getCharAlign()));
...
static bool isAIXLayout(const ASTContext &Context) {
  return Context.getTargetInfo().getTriple().getOS() == toolchain::Triple::AIX;
}
...
void ItaniumRecordLayoutBuilder::LayoutBitField(const FieldDecl *D) {
  ...
  if (IsMsStruct) {
    ...
    if (FieldSize == 0 || FieldSize > UnfilledBitsInLastUnit) {
      FieldOffset = toolchain::alignTo(FieldOffset, FieldAlign);
      UnpackedFieldOffset =
          toolchain::alignTo(UnpackedFieldOffset, UnpackedFieldAlign);
      UnfilledBitsInLastUnit = 0;
    }
    ...
    if (FieldSize == 0 ||
      ...
      FieldOffset = toolchain::alignTo(FieldOffset, ExplicitFieldAlign);
    }
    ...
    if (FieldSize == 0 ||
      ...
      UnpackedFieldOffset =
          toolchain::alignTo(UnpackedFieldOffset, UnpackedFieldAlign);
             ...
             Context.getTargetInfo().useExplicitBitFieldAlignment())
      UnpackedFieldOffset =
          toolchain::alignTo(UnpackedFieldOffset, ExplicitFieldAlign);
  }
  ...
  // storage unit if necessary.
    ...
    uint64_t NewSizeInBits = FieldOffset + FieldSize;
    uint64_t CharAlignment = Context.getTargetInfo().getCharAlign();
    setDataSize(toolchain::alignTo(NewSizeInBits, CharAlignment));
    UnfilledBitsInLastUnit = getDataSizeInBits() - NewSizeInBits;
...
void ItaniumRecordLayoutBuilder::LayoutField(const FieldDecl *D,
  ...
  toolchain::Triple Target = Context.getTargetInfo().getTriple();
  ...
  if (D->getType()->isIncompleteArrayType()) {
    ...
    if (IsMsStruct) {
      ...
      if (const BuiltinType *BTy = T->getAs<BuiltinType>()) {
        ...
        if (!toolchain::isPowerOf2_64(TypeSize.getQuantity())) {
          assert(
        ...
        if (TypeSize > FieldAlign &&
            toolchain::isPowerOf2_64(TypeSize.getQuantity()))
          FieldAlign = TypeSize;
      }
    }
  }
...
void ItaniumRecordLayoutBuilder::FinishLayout(const NamedDecl *D) {
  ...
  uint64_t UnpackedSizeInBits =
      toolchain::alignTo(getSizeInBits(), Context.toBits(UnpackedAlignment));

  uint64_t RoundedSize = toolchain::alignTo(
      getSizeInBits(),
...
void ItaniumRecordLayoutBuilder::UpdateAlignment(
  ...
  if (NewAlignment > Alignment) {
    assert(toolchain::isPowerOf2_64(NewAlignment.getQuantity()) &&
           "Alignment not a power of 2");
    Alignment = NewAlignment;
  }

  if (UnpackedNewAlignment > UnpackedAlignment) {
    assert(toolchain::isPowerOf2_64(UnpackedNewAlignment.getQuantity()) &&
           "Alignment not a power of 2");
    UnpackedAlignment = UnpackedNewAlignment;
  }

  if (PreferredNewAlignment > PreferredAlignment) {
    assert(toolchain::isPowerOf2_64(PreferredNewAlignment.getQuantity()) &&
           "Alignment not a power of 2");
...
static unsigned getPaddingDiagFromTagKind(TagTypeKind Tag) {
  ...
  case TagTypeKind::Class:
    return 2;
  default: toolchain_unreachable("Invalid tag kind for field padding diagnostic!");
  }
}
...
static bool mustSkipTailPadding(TargetCXXABI ABI, const CXXRecordDecl *RD) {
  ...
  toolchain_unreachable("bad tail-padding use kind");
}
...
public:
  ...
  void computeVtorDispSet(
      toolchain::SmallPtrSetImpl<const CXXRecordDecl *> &HasVtorDispSet,
...
void MicrosoftRecordLayoutBuilder::layoutBitField(const FieldDecl *FD) {
  ...
  if (UseExternalLayout) {
    ...
    auto NewSize = Context.toCharUnitsFromBits(
        toolchain::alignDown(FieldBitOffset, Context.toBits(Info.Alignment)) +
...
void MicrosoftRecordLayoutBuilder::layoutVirtualBases(const CXXRecordDecl *RD) {
  ...
  toolchain::SmallPtrSet<const CXXRecordDecl *, 2> HasVtorDispSet;
...
static bool
RequiresVtordisp(const toolchain::SmallPtrSetImpl<const CXXRecordDecl *> &
...
void MicrosoftRecordLayoutBuilder::computeVtorDispSet(
  ...
  // vtordisp.
  toolchain::SmallPtrSet<const CXXMethodDecl *, 8> Work;
  toolchain::SmallPtrSet<const CXXRecordDecl *, 2> BasesWithOverriddenMethods;
...
ASTContext::getASTRecordLayout(const RecordDecl *D) const {
  ...
  if (getLangOpts().DumpRecordLayouts) {
    ...
    DumpRecordLayout(D, toolchain::outs(), getLangOpts().DumpRecordLayoutsSimple);
  }
...
static void PrintOffset(raw_ostream &OS,
  ...
  OS << toolchain::format("%10" PRId64 " | ", (int64_t)Offset.getQuantity());
...
static void PrintBitFieldOffset(raw_ostream &OS, CharUnits Offset,
  ...
  toolchain::SmallString<10> Buffer;
  {
    toolchain::raw_svector_ostream BufferOS(Buffer);
  ...
  OS << toolchain::right_justify(Buffer, 10) << " | ";
  OS.indent(IndentLevel * 2);
}
