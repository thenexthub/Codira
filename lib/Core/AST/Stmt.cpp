...
#define LAST_STMT_RANGE(BASE, FIRST, LAST)                                     \
  static_assert(toolchain::isUInt<NumStmtBits>(Stmt::StmtClass::LAST##Class),             \
...
void Stmt::PrintStats() {
  ...
  toolchain::errs() << "  " << sum << " stmts/exprs total.\n";
  sum = 0;
  for (int i = 0; i != Stmt::lastStmtConstant+1; i++) {
    if (StmtClassInfo[i].Name == nullptr) continue;
    if (StmtClassInfo[i].Counter == 0) continue;
    toolchain::errs() << "    " << StmtClassInfo[i].Counter << " "
  ...
  toolchain::errs() << "Total bytes = " << sum << "\n";
}
...
#include "language/Core/AST/StmtNodes.inc"
  ...
  toolchain_unreachable("unknown statement kind!");
}
...
void CompoundStmt::setStmts(ArrayRef<Stmt *> Stmts) {
  ...
  toolchain::copy(Stmts, body_begin());
}
...
const Expr *ValueStmt::getExprStmt() const {
  ...
  do {
    ...
    if (const auto *LS = dyn_cast<LabelStmt>(S))
      ...
      toolchain_unreachable("unknown kind of ValueStmt");
...
std::string AsmStmt::generateAsmString(const ASTContext &C) const {
  ...
  toolchain_unreachable("unknown asm statement kind!");
}
...
std::string GCCAsmStmt::ExtractStringFromGCCAsmStmtComponent(const Expr *E) {
  if (auto *SL = toolchain::dyn_cast<StringLiteral>(E))
...
std::string GCCAsmStmt::generateAsmString(const ASTContext &C) const {
  ...
  for (const auto &Piece : Pieces) {
    if (Piece.isString())
      ...
      AsmString += '$' + toolchain::utostr(Piece.getOperandNo());
    else
      AsmString += "${" + toolchain::utostr(Piece.getOperandNo()) + ':' +
                   Piece.getModifier() + '}';
  }
...
void MSAsmStmt::initialize(const ASTContext &C, StringRef asmstr,
  ...
  toolchain::copy(exprs, Exprs);

  AsmToks = new (C) Token[asmtoks.size()];
  toolchain::copy(asmtoks, AsmToks);
...
CapturedStmt::Capture *CapturedStmt::getStoredCaptures() const {
  ...
  unsigned FirstCaptureOffset = toolchain::alignTo(Size, alignof(Capture));
...
CapturedStmt::CapturedStmt(Stmt *S, CapturedRegionKind Kind,
  ...
  *Stored = S;
  ...
  toolchain::copy(Captures, Buffer);
}
...
CapturedStmt *CapturedStmt::Create(const ASTContext &Context, Stmt *S,
  ...
  if (!Captures.empty()) {
    ...
    Size = toolchain::alignTo(Size, alignof(Capture));
