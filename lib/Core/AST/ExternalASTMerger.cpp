...
#include "language/Core/AST/ExternalASTMerger.h"
...
LookupSameContext(Source<TranslationUnitDecl *> SourceTU, const DeclContext *DC,
  ...
  if (!SourceNameOrErr) {
    toolchain::consumeError(SourceNameOrErr.takeError());
    return nullptr;
  }
...
private:
  ...
  /// from.
  toolchain::DenseMap<Decl *, Decl *> ToOrigin;
  /// @see ExternalASTMerger::ImporterSource::Merger
  ExternalASTMerger *SourceMerger;
  toolchain::raw_ostream &logs() { return Parent.logs(); }
public:
  ...
  toolchain::Expected<Decl *> ImportImpl(Decl *FromD) override {
    ...
    if (auto *ToDC = dyn_cast<DeclContext>(To)) {
      ...
      if (LoggingEnabled)
        logs() << "(ExternalASTMerger*)" << (void*)&Parent
               ...
               << " from (DeclContext*)" << (void*)toolchain::cast<DeclContext>(From)
...
bool HasDeclOfSameType(ArrayRef<Candidate> Decls, const Candidate &C) {
  if (isa<FunctionDecl>(C.first.get()))
    return false;
  return toolchain::any_of(Decls, [&](const Candidate &D) {
...
ASTImporter &ExternalASTMerger::ImporterForOrigin(ASTContext &OriginContext) {
  for (const std::unique_ptr<ASTImporter> &I : Importers)
    if (&I->getFromContext() == &OriginContext)
      return *I;
  toolchain_unreachable("We should have an importer for this origin!");
}
...
void ExternalASTMerger::CompleteType(TagDecl *Tag) {
  ...
  ForEachMatchingDC(Tag, [&](ASTImporter &Forward, ASTImporter &Reverse,
    ...
    if (toolchain::Error Err = Forward.ImportDefinition(SourceTag))
...
void ExternalASTMerger::CompleteType(ObjCInterfaceDecl *Interface) {
  ...
  ForEachMatchingDC(
      Interface, [&](ASTImporter &Forward, ASTImporter &Reverse,
        ...
        if (toolchain::Error Err = Forward.ImportDefinition(SourceInterface))
          toolchain::consumeError(std::move(Err));
        return true;
      });
}
...
ExternalASTMerger::ExternalASTMerger(const ImporterTarget &Target,
    ...
    : LogStream(&toolchain::nulls()), Target(Target) {
...
static bool importSpecializations(DeclTy *D, ASTImporter *Importer) {
  for (auto *Spec : D->specializations()) {
    ...
    if (!ImportedSpecOrError) {
      toolchain::consumeError(ImportedSpecOrError.takeError());
      return true;
    }
  }
  return false;
...
bool ExternalASTMerger::FindExternalVisibleDeclsByName(
  ...
  toolchain::SmallVector<NamedDecl *, 1> Decls;
  ...
  ForEachMatchingDC(DC,
                    [&](ASTImporter &Forward, ASTImporter &Reverse,
                      ...
                      if (!FromNameOrErr) {
                        toolchain::consumeError(FromNameOrErr.takeError());
                        return false;
                      }
  ...
  for (const Candidate &C : Candidates) {
    ...
    NamedDecl *ND = cast<NamedDecl>(toolchain::cantFail(std::move(NDOrErr)));
    assert(ND);
    ...
    for (const Decl *SourceDecl : SourceDC.get()->decls()) {
      if (IsKindWeWant(SourceDecl->getKind())) {
        ...
        if (ImportedDeclOrErr)
          ...
          toolchain::consumeError(ImportedDeclOrErr.takeError());
      }
    }
    return false;
  });
