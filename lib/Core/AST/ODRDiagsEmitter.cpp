...
bool ODRDiagsEmitter::diagnoseMismatch(
  ...
  if (FirstTemplate && SecondTemplate) {
    ...
    for (auto Pair : toolchain::zip(FirstTemplateParams, SecondTemplateParams)) {
  ...
  case ObjCIvar:
  case ObjCProperty:
    toolchain_unreachable("Invalid diff type");
  ...
  case CXXMethod: {
    ...
    if (FirstTemplateArgs && SecondTemplateArgs) {
      ...
      auto ExpandTemplateArgumentList = [](const TemplateArgumentList *TAL) {
        toolchain::SmallVector<const TemplateArgument *, 8> ExpandedList;
        for (const TemplateArgument &TA : TAL->asArray()) {
          if (TA.getKind() != TemplateArgument::Pack) {
            ExpandedList.push_back(&TA);
            continue;
          }
          toolchain::append_range(ExpandedList,
                             toolchain::make_pointer_range(TA.getPackAsArray()));
        }
        return ExpandedList;
      };
      toolchain::SmallVector<const TemplateArgument *, 8> FirstExpandedList =
          ExpandTemplateArgumentList(FirstTemplateArgs);
      toolchain::SmallVector<const TemplateArgument *, 8> SecondExpandedList =
...
bool ODRDiagsEmitter::diagnoseMismatch(const EnumDecl *FirstEnum,
  ...
  using DeclHashes =
      toolchain::SmallVector<std::pair<const EnumConstantDecl *, unsigned>, 4>;
