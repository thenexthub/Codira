...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
  ...
  toolchain_unreachable("Unhandled kind of DeclarationName");
  return true;
}
...
public:
  ...
  bool IsEquivalent(const Stmt *S1, const Stmt *S2) {
    ...
    case Stmt::NoStmtClass:
      toolchain_unreachable("Can't traverse NoStmtClass");
#define STMT(CLASS, PARENT)                                                    \
...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
  ...
  case Type::ConstantArray: {
    ...
    if (!toolchain::APInt::isSameValue(Array1->getSize(), Array2->getSize()))
      return false;
...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
                                     EnumConstantDecl *D1,
                                     EnumConstantDecl *D2) {
  const toolchain::APSInt &FromVal = D1->getInitVal();
  const toolchain::APSInt &ToVal = D2->getInitVal();
...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
  ...
  toolchain::SmallVector<const EnumConstantDecl *, 8> D1Enums, D2Enums;
  auto CopyEnumerators =
      [](auto &&Range, toolchain::SmallVectorImpl<const EnumConstantDecl *> &Cont) {
  ...
  if (Context.LangOpts.C23) {
    ...
    toolchain::sort(D1Enums, Sorter);
    toolchain::sort(D2Enums, Sorter);
  }
  ...
  for (auto EC1 = D1Enums.begin(), EC1End = D1Enums.end(); EC1 != EC1End;
    ...
    toolchain::APSInt Val1 = (*EC1)->getInitVal();
    toolchain::APSInt Val2 = (*EC2)->getInitVal();
    if (!toolchain::APSInt::isSameValue(Val1, Val2) ||
...
unsigned StructuralEquivalenceContext::getApplicableDiagnostic(
  ...
  toolchain_unreachable("Diagnostic kind not handled in preceding switch");
}
