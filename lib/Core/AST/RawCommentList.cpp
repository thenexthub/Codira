...
std::string RawComment::getFormattedText(const SourceManager &SourceMgr,
  ...
  toolchain::StringRef CommentText = getRawText(SourceMgr);
...
RawComment::getFormattedLines(const SourceManager &SourceMgr,
  ...
  auto LexLine = [&](bool IsFirstLine) -> bool {
    ...
    toolchain::StringRef TokText = L.getSpelling(Tok, SourceMgr);
    ...
    toolchain::StringRef Trimmed = TokText.drop_front(SkipLen);
    Line += Trimmed;
