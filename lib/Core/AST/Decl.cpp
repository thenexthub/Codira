...
#include "language/Core/AST/ODRHash.h"
...
Decl *language::Core::getPrimaryMergedDecl(Decl *D) {
...
static Visibility getVisibilityFromAttr(const T *attr) {
  ...
  toolchain_unreachable("bad visibility kind");
}
...
LinkageComputer::getLVForTemplateArgumentList(ArrayRef<TemplateArgument> Args,
  ...
  for (const TemplateArgument &Arg : Args) {
    ...
    toolchain_unreachable("bad template argument kind");
  }
...
static bool isExportedFromModuleInterfaceUnit(const NamedDecl *D) {
  ...
  toolchain_unreachable("unexpected module ownership kind");
}
...
Linkage NamedDecl::getFormalLinkage() const {
  ...
  return language::Core::getFormalLinkage(InternalLinkage);
}
...
void NamedDecl::printQualifiedName(raw_ostream &OS,
  ...
  if (getDeclName()) {
    ...
    toolchain::raw_svector_ostream NameOS(NameBuffer);
    printName(NameOS, P);
    if (NameBuffer.empty())
      OS << "(anonymous)";
    else
...
void NamedDecl::printNestedNameSpecifier(raw_ostream &OS,
  ...
  for (const DeclContext *DC : toolchain::reverse(Contexts)) {
...
bool NamedDecl::hasLinkage() const {
  ...
  case Linkage::VisibleNone:
    toolchain_unreachable("Non-formal linkage is not allowed here!");
  case Linkage::Module:
...
void QualifierInfo::setTemplateParameterListsInfo(
  ...
  if (!TPLists.empty()) {
    ...
    toolchain::copy(TPLists, TemplParamLists);
  }
}
...
const char *VarDecl::getStorageClassSpecifierString(StorageClass SC) {
  ...
  toolchain_unreachable("Invalid storage class");
}
...
VarDecl::TLSKind VarDecl::getTLSKind() const {
  ...
  toolchain_unreachable("Unknown thread storage class specifier!");
}
...
bool VarDecl::hasDependentAlignment() const {
  ...
  return T->isDependentType() || T->isUndeducedType() ||
         toolchain::any_of(specific_attrs<AlignedAttr>(), [](const AlignedAttr *AA) {
...
FunctionDecl::DefaultedOrDeletedFunctionInfo::Create(
  ...
  toolchain::uninitialized_copy(Lookups, Info->getTrailingObjects<DeclAccessPair>());
  if (DeletedMessage)
...
bool FunctionDecl::isMSVCRTEntryPoint() const {
  ...
  return toolchain::StringSwitch<bool>(getName())
...
bool FunctionDecl::isInlineBuiltinDeclaration() const {
  ...
  toolchain_unreachable("Unknown GVALinkage");
}
...
void FunctionDecl::setParams(ASTContext &C,
  ...
  if (!NewParamInfo.empty()) {
    ...
    toolchain::copy(NewParamInfo, ParamInfo);
  }
}
...
bool FunctionDecl::hasOneParamOrDefaultArgs() const {
  return getNumParams() == 1 ||
         (getNumParams() > 1 &&
          toolchain::all_of(toolchain::drop_begin(parameters()),
...
FunctionDecl::TemplatedKind FunctionDecl::getTemplatedKind() const {
  ...
  toolchain_unreachable("Did we miss a TemplateOrSpecialization type?");
}
...
bool FunctionDecl::isTemplateInstantiation() const {
  ...
  // specialization kind?)
  return language::Core::isTemplateInstantiation(getTemplateSpecializationKind());
}
...
FunctionDecl::getTemplateInstantiationPattern(bool ForDefinition) const {
  ...
  if (MemberSpecializationInfo *Info = FD->getMemberSpecializationInfo()) {
    if (ForDefinition &&
        !language::Core::isTemplateInstantiation(Info->getTemplateSpecializationKind()))
      return nullptr;
    return getDefinitionOrSelf(cast<FunctionDecl>(Info->getInstantiatedFrom()));
  }

  if (ForDefinition &&
      !language::Core::isTemplateInstantiation(getTemplateSpecializationKind()))
    return nullptr;
...
FunctionDecl::setTemplateSpecializationKind(TemplateSpecializationKind TSK,
  ...
  if (FunctionTemplateSpecializationInfo *FTSInfo =
    ...
    toolchain_unreachable("Function cannot have a template specialization kind");
}
...
bool FieldDecl::isZeroSize(const ASTContext &Ctx) const {
  ...
  return !toolchain::any_of(CXXRD->fields(), [](const FieldDecl *Field) {
...
void EnumDecl::getValueRange(toolchain::APInt &Max, toolchain::APInt &Min) const {
  ...
  if (NumNegativeBits) {
    ...
    Max = toolchain::APInt(Bitwidth, 1) << (NumBits - 1);
    Min = -Max;
  } else {
    Max = toolchain::APInt(Bitwidth, 1) << NumPositiveBits;
    Min = toolchain::APInt::getZero(Bitwidth);
  }
}
...
PragmaCommentDecl *PragmaCommentDecl::Create(const ASTContext &C,
  ...
  toolchain::copy(Arg, PCD->getTrailingObjects());
...
PragmaDetectMismatchDecl::Create(const ASTContext &C, TranslationUnitDecl *DC,
  ...
  toolchain::copy(Name, PDMD->getTrailingObjects());
  PDMD->getTrailingObjects()[Name.size()] = '\0';
  toolchain::copy(Value, PDMD->getTrailingObjects() + ValueStart);
...
bool ValueDecl::isInitCapture() const {
  if (auto *Var = toolchain::dyn_cast<VarDecl>(this))
...
EnumConstantDecl::EnumConstantDecl(const ASTContext &C, DeclContext *DC,
                                   ...
                                   QualType T, Expr *E, const toolchain::APSInt &V)
...
EnumConstantDecl *EnumConstantDecl::Create(ASTContext &C, EnumDecl *CD,
                                           ...
                                           Expr *E, const toolchain::APSInt &V) {
...
EnumConstantDecl *EnumConstantDecl::CreateDeserialized(ASTContext &C,
  ...
  return new (C, ID) EnumConstantDecl(C, nullptr, SourceLocation(), nullptr,
                                      QualType(), nullptr, toolchain::APSInt());
}
...
void HLSLBufferDecl::setDefaultBufferDecls(ArrayRef<Decl *> Decls) {
  ...
  toolchain::copy(Decls, DeclsArray);
...
HLSLBufferDecl::buffer_decls_begin() const {
  return buffer_decl_iterator(toolchain::iterator_range(DefaultBufferDecls.begin(),
...
HLSLBufferDecl::buffer_decl_iterator HLSLBufferDecl::buffer_decls_end() const {
  return buffer_decl_iterator(
      toolchain::iterator_range(DefaultBufferDecls.end(), DefaultBufferDecls.end()),
...
HLSLRootSignatureDecl::HLSLRootSignatureDecl(
    ...
    toolchain::dxbc::RootSignatureVersion Version, unsigned NumElems)
...
HLSLRootSignatureDecl *HLSLRootSignatureDecl::Create(
    ...
    ArrayRef<toolchain::hlsl::rootsig::RootElement> RootElements) {
  HLSLRootSignatureDecl *RSDecl =
      new (C, DC,
           additionalSizeToAlloc<toolchain::hlsl::rootsig::RootElement>(
               RootElements.size()))
          HLSLRootSignatureDecl(DC, Loc, ID, Version, RootElements.size());
  auto *StoredElems = RSDecl->getElems();
  toolchain::uninitialized_copy(RootElements, StoredElems);
  return RSDecl;
}
...
HLSLRootSignatureDecl::CreateDeserialized(ASTContext &C, GlobalDeclID ID) {
  HLSLRootSignatureDecl *Result = new (C, ID)
      HLSLRootSignatureDecl(nullptr, SourceLocation(), nullptr,
                            /*Version*/ toolchain::dxbc::RootSignatureVersion::V1_1,
                            /*NumElems=*/0);
  return Result;
}
...
ImportDecl::ImportDecl(DeclContext *DC, SourceLocation StartLoc,
  ...
  toolchain::uninitialized_copy(IdentifierLocs, StoredLocs);
}
...
bool language::Core::IsArmStreamingFunction(const FunctionDecl *FD,
...
bool language::Core::hasArmZAState(const FunctionDecl *FD) {
...
bool language::Core::hasArmZT0State(const FunctionDecl *FD) {
  const auto *T = FD->getType()->getAs<FunctionProtoType>();
