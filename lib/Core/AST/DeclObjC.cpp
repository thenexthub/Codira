...
ObjCPropertyDecl::getDefaultSynthIvarName(ASTContext &Ctx) const {
  ...
  {
    toolchain::raw_svector_ostream os(ivarName);
...
bool ObjCInterfaceDecl::inheritsDesignatedInitializers() const {
  ...
  toolchain_unreachable("unexpected InheritedDesignatedInitializers value");
}
...
void ObjCMethodDecl::setParamsAndSelLocs(ASTContext &C,
  ...
  toolchain::uninitialized_copy(Params, getParams());
  toolchain::uninitialized_copy(SelLocs, getStoredSelLocs());
}
...
ObjCMethodDecl::findPropertyDecl(bool CheckOverrides) const {
  ...
  if (isPropertyAccessor()) {
    ...
    toolchain_unreachable("Marked as a property accessor but no property found!");
  }
...
ObjCTypeParamList::ObjCTypeParamList(SourceLocation lAngleLoc,
  ...
  toolchain::copy(typeParams, begin());
}
...
ObjCInterfaceDecl *ObjCIvarDecl::getContainingInterface() {
  ...
  case ObjCProtocol:
    toolchain_unreachable("invalid ivar container!");
...
void ObjCProtocolDecl::getImpliedProtocols(
    toolchain::DenseSet<const ObjCProtocolDecl *> &IPs) const {
...
raw_ostream &language::Core::operator<<(raw_ostream &OS,
