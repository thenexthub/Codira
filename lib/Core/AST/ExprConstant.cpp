...
namespace {
  ...
  static bool isForManglingOnly(ConstantExprKind Kind) {
    ...
    toolchain_unreachable("unknown ConstantExprKind");
  }
  ...
  struct SubobjectDesignator {
    ...
    SubobjectDesignator(ASTContext &Ctx, const APValue &V)
      ...
      if (!Invalid) {
        ...
        toolchain::append_range(Entries, V.getLValuePath());
        if (V.getLValueBase()) {
          bool IsArray = false;
  ...
  public:
    ...
    void describe(toolchain::raw_ostream &OS) const override;
...
namespace {
  ...
  class Cleanup {
    toolchain::PointerIntPair<APValue*, 2, ScopeKind> Value;
  ...
  struct ObjectUnderConstruction {
    ...
    friend toolchain::hash_code hash_value(const ObjectUnderConstruction &Obj) {
      return toolchain::hash_combine(Obj.Base, Obj.Path);
    }
  };
...
namespace {
  ...
  public:
    ...
    /// evaluation frame.
    toolchain::SmallVector<Cleanup, 16> CleanupStack;
    ...
    toolchain::DenseMap<ObjectUnderConstruction, ConstructionPhase>
    ...
    void performLifetimeExtension() {
      ...
      toolchain::erase_if(CleanupStack, [](Cleanup &C) {
  ...
  public:
    ...
    bool keepEvaluatingAfterSideEffect() const override {
      ...
      toolchain_unreachable("Missed EvalMode case");
    }
...
static bool isModification(AccessKinds AK) {
  ...
  toolchain_unreachable("unknown access kind");
}
...
namespace {
  ...
  struct MemberPtr {
    ...
    void setFrom(const APValue &V) {
      ...
      Path.clear();
      toolchain::append_range(Path, V.getMemberPointerPath());
    }

    /// DeclAndIsDerivedMember - The member declaration, and a flag indicating
    /// whether the member is a member of some class derived from the class type
    /// of the member pointer.
    toolchain::PointerIntPair<const ValueDecl*, 1, bool> DeclAndIsDerivedMember;
...
static bool IsOpaqueConstantCall(const LValue &LVal) {
  const auto *BaseExpr =
      toolchain::dyn_cast_if_present<CallExpr>(LVal.Base.dyn_cast<const Expr *>());
...
static bool ArePotentiallyOverlappingStringLiterals(const EvalInfo &Info,
  ...
  for (int NullByte : toolchain::seq(ShorterCharWidth)) {
...
using CheckedTemporaries =
    toolchain::SmallPtrSet<const MaterializeTemporaryExpr *, 8>;
...
static bool HandleFloatToIntCast(EvalInfo &Info, const Expr *E,
  ...
  bool ignored;
  if (Value.convertToInteger(Result, toolchain::APFloat::rmTowardZero, &ignored)
      & APFloat::opInvalidOp)
...
static toolchain::RoundingMode getActiveRoundingMode(EvalInfo &Info, const Expr *E) {
...
static bool checkFloatingPointResult(EvalInfo &Info, const Expr *E,
  ...
  if ((St & APFloat::opInexact) &&
      FPO.getRoundingMode() == toolchain::RoundingMode::Dynamic) {
...
static bool HandleFloatToFloatCast(EvalInfo &Info, const Expr *E,
  ...
  toolchain::RoundingMode RM = getActiveRoundingMode(Info, E);
  APFloat::opStatus St;
...
static bool handleIntIntBinOp(EvalInfo &Info, const BinaryOperator *E,
  ...
  case BO_Shl: {
    if (Info.getLangOpts().OpenCL)
      ...
      RHS &= APSInt(toolchain::APInt(RHS.getBitWidth(),
  ...
  case BO_Cmp:
    toolchain_unreachable("BO_Cmp should be handled elsewhere");
  }
}
...
static bool CastToBaseClass(EvalInfo &Info, const Expr *E, LValue &Result,
  ...
  if (!DerivedRD->isDerivedFrom(BaseRD, Paths))
    toolchain_unreachable("Class must be derived from the passed in base class!");
...
static unsigned getBaseIndex(const CXXRecordDecl *Derived,
  ...
  toolchain_unreachable("base class missing from derived class's bases list");
}
...
static bool lifetimeStartedInEvaluation(EvalInfo &Info,
  ...
  toolchain_unreachable("unknown evaluating decl kind");
}
...
static CompleteObject findCompleteObject(EvalInfo &Info, const Expr *E,
  ...
  if (Info.getLangOpts().CPlusPlus14 && LVal.Base == Info.EvaluatingDecl &&
    ...
    // reference).
    if (AK != language::Core::AK_Dereference && !Frame) {
    ...
    if (AK != language::Core::AK_Dereference &&
    ...
    if (AK != language::Core::AK_Dereference && !BaseVal) {
      if (!Info.checkingPotentialConstantExpression()) {
  ...
  // within its lifetime.
  else if (AK != language::Core::AK_Dereference) {
  ...
  if (toolchain::isa_and_nonnull<ParmVarDecl>(
...
static const ValueDecl *HandleMemberPointerAccess(EvalInfo &Info,
  ...
  if (IncludeMember) {
    if (const FieldDecl *FD = dyn_cast<FieldDecl>(MemPtr.getDecl())) {
      ...
      toolchain_unreachable("can't construct reference to bound member function");
    }
  }
...
static EvalStmtResult EvaluateLoopBody(StmtResult &Result, EvalInfo &Info,
  ...
  toolchain_unreachable("Invalid EvalStmtResult!");
}
...
static const CXXMethodDecl *HandleVirtualDispatch(
    ...
    toolchain::SmallVectorImpl<QualType> &CovariantAdjustmentPath) {
...
static bool isBaseClassPublic(const CXXRecordDecl *Derived,
  ...
  toolchain_unreachable("Base is not a direct base of Derived");
}
...
struct StartLifetimeOfUnionMemberHandler {
  ...
  bool found(APSInt &Value, QualType SubobjType) {
    toolchain_unreachable("wrong value kind for union object");
  }
  bool found(APFloat &Value, QualType SubobjType) {
    toolchain_unreachable("wrong value kind for union object");
  }
...
static bool MaybeHandleUnionActiveMemberChange(EvalInfo &Info,
  ...
  toolchain::SmallVector<std::pair<unsigned, const FieldDecl*>, 4> UnionPathLengths;
  // C++ [class.union]p5:
  ...
  for (const Expr *E = LHSExpr; E != nullptr;) {
    ...
    if (auto *ME = dyn_cast<MemberExpr>(E)) {
      ...
      for (const CXXBaseSpecifier *Elt : toolchain::reverse(ICE->path())) {
        if (Elt->isVirtual()) {
  ...
  for (std::pair<unsigned, const FieldDecl *> LengthAndField :
           toolchain::reverse(UnionPathLengths)) {
...
static bool EvaluateArgs(ArrayRef<const Expr *> Args, CallRef Call,
  ...
  bool Success = true;
  toolchain::SmallBitVector ForbiddenNullArgs;
...
static bool HandleConstructorCall(const Expr *E, const LValue &This,
  ...
  for (const auto *I : Definition->inits()) {
    ...
    if (I->isBaseInitializer()) {
      ...
      toolchain_unreachable("unknown base initializer kind");
    }
...
static bool HandleDestructionImpl(EvalInfo &Info, SourceRange CallRange,
  ...
  for (const FieldDecl *FD : toolchain::reverse(Fields)) {
  ...
  for (const CXXBaseSpecifier &Base : toolchain::reverse(RD->bases())) {
    --BasesLeft;
...
public:
  ...
  [[nodiscard]] bool readObject(CharUnits Offset, CharUnits Width,
    ...
    if (toolchain::sys::IsLittleEndianHost != TargetIsLittleEndian)
...
class APValueToBufferConverter {
  ...
  bool visit(const APValue &Val, QualType Ty, CharUnits Offset) {
    ...
    case APValue::LValue:
      toolchain_unreachable("LValue subobject in bit_cast?");
    }
    toolchain_unreachable("Unhandled APValue::ValueKind");
  }
  ...
  bool visitVector(const APValue &Val, QualType Ty, CharUnits Offset) {
    ...
    if (VTy->isPackedVectorBoolType(Info.Ctx)) {
      ...
      toolchain::APInt Res = toolchain::APInt::getZero(NElts);
      for (unsigned I = 0; I < NElts; ++I) {
        const toolchain::APSInt &EltAsInt = Val.getVectorElt(I).getInt();
      ...
      toolchain::StoreIntToMemory(Res, &*Bytes.begin(), NElts / 8);
  ...
  bool visitInt(const APSInt &Val, QualType Ty, CharUnits Offset) {
    ...
    toolchain::StoreIntToMemory(AdjustedVal, &*Bytes.begin(), Width / 8);
    Buffer.writeObject(Offset, Bytes);
    return true;
  }
...
class BufferToAPValueConverter {
  ...
  std::optional<APValue> visit(const BuiltinType *T, CharUnits Offset,
    ...
    if (T->isRealFloatingType()) {
      ...
      unsigned NumBits = toolchain::APFloatBase::getSizeInBits(Semantics);
      assert(NumBits % 8 == 0);
    ...
    toolchain::LoadIntFromMemory(Val, &*Bytes.begin(), Bytes.size());
  ...
  std::optional<APValue> visit(const VectorType *VTy, CharUnits Offset) {
    ...
    if (VTy->isPackedVectorBoolType(Info.Ctx)) {
      ...
      toolchain::LoadIntFromMemory(SValInt, &*Bytes.begin(), Bytes.size());
...
#define NON_CANONICAL_TYPE(Class, Base)                                        \
  case Type::Class:                                                            \
    toolchain_unreachable("non-canonical type should be impossible!");
...
#define NON_CANONICAL_UNLESS_DEPENDENT(Class, Base)                            \
  case Type::Class:                                                            \
    toolchain_unreachable("either dependent or not canonical!");
#include "language/Core/AST/TypeNodes.inc"
    }
    toolchain_unreachable("Unhandled Type::TypeClass");
  }
...
public:
  ...
  bool VisitStmt(const Stmt *) {
    toolchain_unreachable("Expression evaluator should not be called on stmts");
  }
  ...
  bool VisitStmtExpr(const StmtExpr *E) {
    ...
    toolchain::SaveAndRestore NotCheckingForUB(Info.CheckingForUndefinedBehavior,
                                          false);
    ...
    toolchain_unreachable("Return from function from the loop above.");
  }
...
static bool getBytesReturnedByAllocSizeCall(const ASTContext &Ctx,
  ...
  bool Overflow;
  toolchain::APInt BytesAvailable = SizeOfElem.umul_ov(NumberOfElems, Overflow);
  if (Overflow)
    return false;
...
public:
  ...
  bool VisitEmbedExpr(const EmbedExpr *E) {
    toolchain::report_fatal_error("Not yet implemented for ExprConstant.cpp");
    return true;
  }
...
static CharUnits GetAlignOfType(const ASTContext &Ctx, QualType T,
  ...
  // alignof and _Alignof are defined to return the ABI alignment.
    ...
    toolchain_unreachable("GetAlignOfType on a non-alignment ExprKind");
}
...
bool PointerExprEvaluator::VisitBuiltinCallExpr(const CallExpr *E,
    ...
    if (BaseAlignment.getQuantity() >= Alignment) {
      ...
      CharUnits NewOffset = CharUnits::fromQuantity(
          BuiltinOp == Builtin::BI__builtin_align_down
              ? toolchain::alignDown(Result.Offset.getQuantity(), Alignment64)
              : toolchain::alignTo(Result.Offset.getQuantity(), Alignment64));
  ...
  case Builtin::BI__builtin_wmemmove: {
    ...
    if (!WChar) {
      uint64_t Remainder;
      toolchain::APInt OrigN = N;
      toolchain::APInt::udivrem(OrigN, TSize, N, Remainder);
      if (Remainder) {
...
bool PointerExprEvaluator::VisitCXXNewExpr(const CXXNewExpr *E) {
  ...
  if (std::optional<const Expr *> ArraySize = E->getArraySize()) {
    ...
    if (!Init) {
      ...
      toolchain::APInt InitBound = CAT->getSize().zext(Bits);
      toolchain::APInt AllocBound = ArrayBound.zext(Bits);
...
bool VectorExprEvaluator::VisitCallExpr(const CallExpr *E) {
  ...
  case Builtin::BI__builtin_elementwise_abs: {
    ...
    for (unsigned EltNum = 0; EltNum < SourceLen; ++EltNum) {
      ...
      APValue Val = DestEltTy->isFloatingType()
                        ? APValue(toolchain::abs(CurrentEle.getFloat()))
                        : APValue(APSInt(
  ...
  case language::Core::X86::BI__builtin_ia32_pmulhuw128:
  case language::Core::X86::BI__builtin_ia32_pmulhuw256:
  case language::Core::X86::BI__builtin_ia32_pmulhuw512:
  case language::Core::X86::BI__builtin_ia32_pmulhw128:
  case language::Core::X86::BI__builtin_ia32_pmulhw256:
  case language::Core::X86::BI__builtin_ia32_pmulhw512: {
    ...
    for (unsigned EltNum = 0; EltNum < SourceLen; ++EltNum) {
      ...
      case language::Core::X86::BI__builtin_ia32_pmulhuw256:
      case language::Core::X86::BI__builtin_ia32_pmulhuw512:
        ResultElements.push_back(APValue(APSInt(toolchain::APIntOps::mulhu(LHS, RHS),
                                                /*isUnsigned=*/true)));
        break;
      case language::Core::X86::BI__builtin_ia32_pmulhw128:
      case language::Core::X86::BI__builtin_ia32_pmulhw256:
      case language::Core::X86::BI__builtin_ia32_pmulhw512:
        ResultElements.push_back(APValue(APSInt(toolchain::APIntOps::mulhs(LHS, RHS),
                                                /*isUnsigned=*/false)));
        break;
      }
    }
...
static bool handleVectorShuffle(EvalInfo &Info, const ShuffleVectorExpr *E,
  ...
  if (index < 0 ||
    ...
    toolchain_unreachable("Out of bounds shuffle index");
...
bool ArrayExprEvaluator::VisitCXXParenListOrInitListExpr(
  ...
  LLVM_DEBUG(toolchain::dbgs() << "The number of elements to initialize: "
                          << NumEltsToInit << ".\n");
...
public:
  ...
  bool Success(const toolchain::APSInt &SI, const Expr *E, APValue &Result) {
  ...
  bool Success(const toolchain::APSInt &SI, const Expr *E) {
    return Success(SI, E, Result);
  }

  bool Success(const toolchain::APInt &I, const Expr *E, APValue &Result) {
    assert(E->getType()->isIntegralOrEnumerationType() &&
           "Invalid evaluation result.");
  ...
  bool Success(const toolchain::APInt &I, const Expr *E) {
...
bool IntExprEvaluator::CheckReferencedDecl(const Expr* E, const Decl* D) {
  ...
  if (const EnumConstantDecl *ECD = dyn_cast<EnumConstantDecl>(D)) {
    ...
    if (SameSign && SameWidth)
      ...
      toolchain::APSInt Val = ECD->getInitVal();
      if (!SameSign)
...
#include "language/Core/Basic/HLSLIntangibleTypes.def"
    ...
    toolchain_unreachable("unexpected placeholder type");
  ...
  case Type::RValueReference:
    toolchain_unreachable("invalid type for expression");
  }
...
static bool convertUnsignedAPIntToCharUnits(const toolchain::APInt &Int,
                                            CharUnits &Result) {
...
bool IntExprEvaluator::VisitBuiltinCallExpr(const CallExpr *E,
  ...
  case Builtin::BI__builtin_object_size: {
    ...
    toolchain_unreachable("unexpected EvalMode");
  }
  ...
  case Builtin::BI__builtin_isunordered: {
    ...
    return Success(
        [&] {
          ...
          default:
            toolchain_unreachable("Unexpected builtin ID: Should be a floating "
  ...
  case Builtin::BI__builtin_isfpclass: {
    ...
    if (!EvaluateInteger(E->getArg(1), MaskVal, Info))
      return false;
    unsigned Test = static_cast<toolchain::FPClassTest>(MaskVal.getZExtValue());
    APFloat Val(0.0);
    return EvaluateFloat(E->getArg(0), Val, Info) &&
  ...
  case Builtin::BI__builtin_subcll: {
    ...
    switch (BuiltinOp) {
    default:
      toolchain_unreachable("Invalid value for BuiltinOp");
    case Builtin::BI__builtin_addcb:
    case Builtin::BI__builtin_addcs:
    case Builtin::BI__builtin_addc:
    case Builtin::BI__builtin_addcl:
  ...
  case language::Core::X86::BI__builtin_ia32_addcarryx_u32:
  case language::Core::X86::BI__builtin_ia32_addcarryx_u64:
  case language::Core::X86::BI__builtin_ia32_subborrow_u32:
  case language::Core::X86::BI__builtin_ia32_subborrow_u64: {
    LValue ResultLValue;
    ...
    bool IsAdd = BuiltinOp == language::Core::X86::BI__builtin_ia32_addcarryx_u32 ||
                 BuiltinOp == language::Core::X86::BI__builtin_ia32_addcarryx_u64;
  ...
  case language::Core::X86::BI__builtin_ia32_bextr_u32:
  case language::Core::X86::BI__builtin_ia32_bextr_u64:
  case language::Core::X86::BI__builtin_ia32_bextri_u32:
  case language::Core::X86::BI__builtin_ia32_bextri_u64: {
    APSInt Val, Idx;
    if (!EvaluateInteger(E->getArg(0), Val, Info) ||
    ...
    Result &= toolchain::maskTrailingOnes<uint64_t>(Length);
    return Success(Result, E);
  }

  case language::Core::X86::BI__builtin_ia32_bzhi_si:
  case language::Core::X86::BI__builtin_ia32_bzhi_di: {
    APSInt Val, Idx;
    if (!EvaluateInteger(E->getArg(0), Val, Info) ||
        !EvaluateInteger(E->getArg(1), Idx, Info))
      return false;
  ...
  case language::Core::X86::BI__builtin_ia32_lzcnt_u16:
  case language::Core::X86::BI__builtin_ia32_lzcnt_u32:
  case language::Core::X86::BI__builtin_ia32_lzcnt_u64: {
    APSInt Val;
    if (!EvaluateInteger(E->getArg(0), Val, Info))
      return false;
  ...
  case language::Core::X86::BI__builtin_ia32_tzcnt_u16:
  case language::Core::X86::BI__builtin_ia32_tzcnt_u32:
  case language::Core::X86::BI__builtin_ia32_tzcnt_u64: {
    APSInt Val;
    if (!EvaluateInteger(E->getArg(0), Val, Info))
      return false;
  ...
  case language::Core::X86::BI__builtin_ia32_pdep_si:
  case language::Core::X86::BI__builtin_ia32_pdep_di: {
    APSInt Val, Msk;
    if (!EvaluateInteger(E->getArg(0), Val, Info) ||
  ...
  case language::Core::X86::BI__builtin_ia32_pext_si:
  case language::Core::X86::BI__builtin_ia32_pext_di: {
    APSInt Val, Msk;
    if (!EvaluateInteger(E->getArg(0), Val, Info) ||
        !EvaluateInteger(E->getArg(1), Msk, Info))
      return false;
...
void DataRecursiveIntBinOpEvaluator::process(EvalResult &Result) {
  ...
  toolchain_unreachable("Invalid Job::Kind!");
}
...
EvaluateComparisonBinaryOperator(EvalInfo &Info, const BinaryOperator *E,
  ...
  if (LHSTy->isRealFloatingType() &&
    ...
    toolchain::APFloatBase::cmpResult APFloatCmpResult = LHS.compare(RHS);
    if (!Info.InConstantContext &&
    ...
    auto GetCmpRes = [&]() {
      ...
      toolchain_unreachable("Unrecognised APFloat::cmpResult enum");
    };
...
bool RecordExprEvaluator::VisitBinCmp(const BinaryOperator *E) {
  ...
  auto OnSuccess = [&](CmpResult CR, const BinaryOperator *E) {
    ...
    case CmpResult::Unequal:
      toolchain_unreachable("should never produce Unequal for three-way comparison");
    case CmpResult::Less:
...
bool IntExprEvaluator::VisitBinaryOperator(const BinaryOperator *E) {
  ...
  if (LHSTy->isPointerType() && RHSTy->isPointerType() &&
    ...
    APSInt LHS(toolchain::APInt(65, (int64_t)LHSOffset.getQuantity(), true), false);
    APSInt RHS(toolchain::APInt(65, (int64_t)RHSOffset.getQuantity(), true), false);
    APSInt ElemSize(toolchain::APInt(65, (int64_t)ElementSize.getQuantity(), true),
                    false);
...
bool IntExprEvaluator::VisitUnaryExprOrTypeTraitExpr(
  ...
  toolchain_unreachable("unknown expr/type trait");
}
...
bool IntExprEvaluator::VisitCastExpr(const CastExpr *E) {
  ...
  case CK_HLSLAggregateSplatCast:
    toolchain_unreachable("invalid cast kind for integral value");
  ...
  case CK_FixedPointToIntegral: {
    ...
    if (!EvaluateFixedPoint(SubExpr, Src, Info))
      return false;
    bool Overflowed;
    toolchain::APSInt Result = Src.convertToInt(
      ...
      if (!ED->isFixed()) {
        ...
        if (ED->getNumNegativeBits() &&
          ...
          Info.CCEDiag(E, diag::note_constexpr_unscoped_enum_out_of_range)
              << toolchain::toString(Result.getInt(), 10) << Min.getSExtValue()
              << Max.getSExtValue() << ED;
        else if (!ED->getNumNegativeBits() &&
                 Max.ult(Result.getInt().getZExtValue()))
          Info.CCEDiag(E, diag::note_constexpr_unscoped_enum_out_of_range)
              << toolchain::toString(Result.getInt(), 10) << Min.getZExtValue()
              << Max.getZExtValue() << ED;
      }
    }
  ...
  case CK_PointerToIntegral: {
    ...
    if (!V.toIntegralConstant(AsInt, SrcType, Info.Ctx))
      toolchain_unreachable("Can't cast this!");
  ...
  toolchain_unreachable("unknown cast resulting in integral value");
}
...
bool FixedPointExprEvaluator::VisitBinaryOperator(const BinaryOperator *E) {
  ...
  case BO_Shr: {
    ...
    toolchain::APSInt RHSVal = RHSFX.getValue();
...
static bool TryEvaluateBuiltinNaN(const ASTContext &Context,
  ...
  const toolchain::fltSemantics &Sem = Context.getFloatTypeSemantics(ResultTy);

  toolchain::APInt fill;

  // Treat empty strings as if they were zero.
  if (S->getString().empty())
    fill = toolchain::APInt(32, 0);
  else if (S->getString().getAsInteger(0, fill))
    return false;

  if (Context.getTargetInfo().isNan2008()) {
    if (SNaN)
      Result = toolchain::APFloat::getSNaN(Sem, false, &fill);
    else
      Result = toolchain::APFloat::getQNaN(Sem, false, &fill);
  } else {
...
bool FloatExprEvaluator::VisitCallExpr(const CallExpr *E) {
  ...
  case Builtin::BI__builtin_inff128: {
    ...
    Result = toolchain::APFloat::getInf(Sem);
    return true;
  }
...
bool FloatExprEvaluator::VisitUnaryImag(const UnaryOperator *E) {
  ...
  VisitIgnoredValue(E->getSubExpr());
  const toolchain::fltSemantics &Sem = Info.Ctx.getFloatTypeSemantics(E->getType());
  Result = toolchain::APFloat::getZero(Sem);
  return true;
}
...
bool ComplexExprEvaluator::VisitCastExpr(const CastExpr *E) {
  ...
  case CK_HLSLElementwiseCast:
  case CK_HLSLAggregateSplatCast:
    toolchain_unreachable("invalid cast kind for complex value");
  ...
  toolchain_unreachable("unknown cast resulting in complex value");
}
...
static bool EvaluateDestruction(const ASTContext &Ctx, APValue::LValueBase Base,
  ...
  if (!Info.discardCleanups())
    toolchain_unreachable("Unhandled cleanup; missing full expression marker?");
...
bool Expr::EvaluateAsInitializer(APValue &Value, const ASTContext &Ctx,
  ...
  toolchain::TimeTraceScope TimeScope("EvaluateAsInitializer", [&] {
    std::string Name;
...
#include "language/Core/AST/StmtNodes.inc"
  ...
  case Expr::UnaryOperatorClass: {
    ...
    toolchain_unreachable("invalid unary operator class");
  }
  ...
  case Expr::BinaryOperatorClass: {
    ...
    case BO_Cmp: {
      ...
      if (Exp->getOpcode() == BO_Div ||
        ...
        if (LHSResult.Kind == IK_ICE && RHSResult.Kind == IK_ICE) {
          toolchain::APSInt REval = Exp->getRHS()->EvaluateKnownConstInt(Ctx);
          if (REval == 0)
            return ICEDiag(IK_ICEIfUnevaluated, E->getBeginLoc());
          if (REval.isSigned() && REval.isAllOnes()) {
            toolchain::APSInt LEval = Exp->getLHS()->EvaluateKnownConstInt(Ctx);
    ...
    toolchain_unreachable("invalid binary operator kind");
  }
    ...
    if (isa<ExplicitCastExpr>(E)) {
      if (const FloatingLiteral *FL
        ...
        if (FL->getValue().convertToInteger(IgnoredVal,
                                            toolchain::APFloat::rmTowardZero,
  ...
  toolchain_unreachable("Invalid StmtClass!");
}

/// Evaluate an expression as a C++11 integral constant expression.
static bool EvaluateCPlusPlus11IntegralConstantExpr(const ASTContext &Ctx,
                                                    const Expr *E,
                                                    toolchain::APSInt *Value) {
...
Expr::getIntegerConstantExpr(const ASTContext &Ctx) const {
  ...
  if (!::EvaluateAsInt(this, ExprResult, Ctx, SE_AllowSideEffects, Info))
    toolchain_unreachable("ICE cannot be evaluated!");
...
bool Expr::EvaluateWithSubstitution(APValue &Value, ASTContext &Ctx,
  ...
  toolchain::TimeTraceScope TimeScope("EvaluateWithSubstitution", [&] {
    std::string Name;
    toolchain::raw_string_ostream OS(Name);
...
bool Expr::isPotentialConstantExpr(const FunctionDecl *FD,
  ...
  toolchain::TimeTraceScope TimeScope("isPotentialConstantExpr", [&] {
    std::string Name;
    toolchain::raw_string_ostream OS(Name);
