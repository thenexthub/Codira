...
void TextNodeDumper::Visit(const CXXCtorInitializer *Init) {
  ...
  if (Init->isAnyMemberInitializer()) {
    ...
    toolchain_unreachable("Unknown initializer type");
  }
}
...
void TextNodeDumper::Visit(const OMPClause *C) {
  ...
  {
    ColorScope Color(OS, ShowColors, AttrColor);
    StringRef ClauseName(toolchain::omp::getOpenMPClauseName(C->getClauseKind()));
...
void TextNodeDumper::dumpCleanupObject(
  ...
  if (auto *BD = dyn_cast<BlockDecl *>(C))
    ...
    toolchain_unreachable("unexpected cleanup type");
}

void language::Core::TextNodeDumper::dumpTemplateSpecializationKind(
...
void language::Core::TextNodeDumper::dumpNestedNameSpecifier(NestedNameSpecifier NNS) {
  if (!NNS)
    return;
...
void TextNodeDumper::dumpTemplateArgument(const TemplateArgument &TA) {
  ...
  {
    toolchain::raw_svector_ostream SS(Str);
  ...
  if (TemplateArgument CanonTA = Context->getCanonicalTemplateArgument(TA);
    ...
    toolchain::SmallString<128> CanonStr;
    {
      toolchain::raw_svector_ostream SS(CanonStr);
...
void TextNodeDumper::dumpBareTemplateName(TemplateName TN) {
  ...
  toolchain_unreachable("Unexpected TemplateName Kind");
}
...
void language::Core::TextNodeDumper::VisitReturnStmt(const ReturnStmt *Node) {
...
void language::Core::TextNodeDumper::VisitCoawaitExpr(const CoawaitExpr *Node) {
  if (Node->isImplicit())
    OS << " implicit";
}

void language::Core::TextNodeDumper::VisitCoreturnStmt(const CoreturnStmt *Node) {
  if (Node->isImplicit())
    OS << " implicit";
}
...
void TextNodeDumper::VisitCXXOperatorCallExpr(const CXXOperatorCallExpr *Node) {
  const char *OperatorSpelling = language::Core::getOperatorSpelling(Node->getOperator());
  if (OperatorSpelling)
    OS << " '" << OperatorSpelling << "'";
...
void language::Core::TextNodeDumper::VisitDependentScopeDeclRefExpr(
  ...
  if (const auto *MD = dyn_cast<CXXMethodDecl>(D)) {
    if (MD->size_overridden_methods() != 0) {
      ...
      AddChild([=] {
        ...
        for (const auto *Override : toolchain::drop_begin(Overrides)) {
          OS << ", ";
    ...
    AddChild([=] {
      ...
      }
      {
        ...
        StringRef ClauseName(
            toolchain::omp::getOpenMPClauseName(C->getClauseKind()));
        OS << "OMP" << ClauseName.substr(/*Start=*/0, /*N=*/1).upper()
           << ClauseName.drop_front() << "Clause";
      }
      dumpPointer(C);
...
void TextNodeDumper::VisitHLSLRootSignatureDecl(
  ...
  OS << " version: ";
  switch (D->getVersion()) {
  case toolchain::dxbc::RootSignatureVersion::V1_0:
    OS << "1.0";
    break;
  case toolchain::dxbc::RootSignatureVersion::V1_1:
    OS << "1.1";
    break;
  }
  OS << ", ";
  toolchain::hlsl::rootsig::dumpRootElements(OS, D->getRootElements());
}
