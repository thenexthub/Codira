...
LLVM_DUMP_METHOD void DeclContext::dumpDeclContext() const {
  ...
  DeclPrinter Printer(toolchain::errs(), Ctx.getPrintingPolicy(), Ctx, 0);
...
void DeclPrinter::Print(AccessSpecifier AS) {
  ...
  if (AccessSpelling.empty())
    toolchain_unreachable("No access specifier!");
  Out << AccessSpelling;
}
...
static void printExplicitSpecifier(ExplicitSpecifier ES, toolchain::raw_ostream &Out,
                                   PrintingPolicy &Policy, unsigned Indentation,
                                   const ASTContext &Context) {
  std::string Proto = "explicit";
  toolchain::raw_string_ostream EOut(Proto);
  if (ES.getExpr()) {
    EOut << "(";
...
void DeclPrinter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  if (Policy.FullyQualifiedName) {
    ...
    toolchain::raw_string_ostream OS(Proto);
