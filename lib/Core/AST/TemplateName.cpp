...
DeducedTemplateStorage::DeducedTemplateStorage(TemplateName Underlying,
  ...
  toolchain::copy(DefArgs.Args, reinterpret_cast<TemplateArgument *>(this + 1));
}

void DeducedTemplateStorage::Profile(toolchain::FoldingSetNodeID &ID,
...
void SubstTemplateTemplateParmStorage::Profile(toolchain::FoldingSetNodeID &ID) {
  Profile(ID, Replacement, getAssociatedDecl(), getIndex(), getPackIndex(),
          getFinal());
}

void SubstTemplateTemplateParmStorage::Profile(
    toolchain::FoldingSetNodeID &ID, TemplateName Replacement, Decl *AssociatedDecl,
...
void SubstTemplateTemplateParmPackStorage::Profile(toolchain::FoldingSetNodeID &ID,
                                                   ASTContext &Context) {
...
void SubstTemplateTemplateParmPackStorage::Profile(
    toolchain::FoldingSetNodeID &ID, ASTContext &Context,
...
void IdentifierOrOverloadedOperator::Profile(toolchain::FoldingSetNodeID &ID) const {
...
TemplateNameDependence TemplateName::getDependence() const {
  ...
  case NameKind::OverloadedTemplate:
    toolchain_unreachable("overloaded templates shouldn't survive to here.");
  }
  toolchain_unreachable("Unknown TemplateName kind");
}
