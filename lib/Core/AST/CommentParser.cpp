...
static inline bool isWhitespace(toolchain::StringRef S) {
...
public:
  TextTokenRetokenizer(toolchain::BumpPtrAllocator &Allocator, Parser &P):
  ...
  bool startsWithParCommand() {
    ...
    toolchain::StringRef LineStart(Pos.BufferPtr - Offset - 3, 4);
...
Parser::Parser(Lexer &L, Sema &S, toolchain::BumpPtrAllocator &Allocator,
...
BlockContentComment *Parser::parseParagraphOrBlockCommand() {
  ...
  while (true) {
    ...
    case tok::html_greater:
    case tok::html_slash_greater:
      toolchain_unreachable("should not see this token");
    }
    break;
  }
...
BlockContentComment *Parser::parseBlockContent() {
  ...
  toolchain_unreachable("bogus token kind");
}
