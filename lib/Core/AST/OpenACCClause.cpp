...
OpenACCSelfClause::OpenACCSelfClause(SourceLocation BeginLoc,
  ...
  assert((!ConditionExpr || ConditionExpr->isInstantiationDependent() ||
          ConditionExpr->getType()->isScalarType()) &&
         "Condition expression type not scalar/dependent");
  toolchain::uninitialized_copy(ArrayRef(ConditionExpr), getTrailingObjects());
}
...
OpenACCGangClause::OpenACCGangClause(SourceLocation BeginLoc,
  ...
  toolchain::uninitialized_copy(GangKinds, getTrailingObjects<OpenACCGangKind>());
}
...
bool language::Core::operator==(const OpenACCBindClause &LHS,
...
void OpenACCClausePrinter::VisitSelfClause(const OpenACCSelfClause &C) {
  ...
  if (C.isConditionExprClause()) {
    ...
    OS << "(";
    toolchain::interleaveComma(C.getVarList(), OS,
...
void OpenACCClausePrinter::VisitNumGangsClause(const OpenACCNumGangsClause &C) {
  OS << "num_gangs(";
  toolchain::interleaveComma(C.getIntExprs(), OS,
                        [&](const Expr *E) { printExpr(E); });
  OS << ")";
}
...
void OpenACCClausePrinter::VisitTileClause(const OpenACCTileClause &C) {
  OS << "tile(";
  toolchain::interleaveComma(C.getSizeExprs(), OS,
                        [&](const Expr *E) { printExpr(E); });
  OS << ")";
}
...
void OpenACCClausePrinter::VisitWaitClause(const OpenACCWaitClause &C) {
  ...
  if (C.hasExprs()) {
    ...
    toolchain::interleaveComma(C.getQueueIdExprs(), OS,
                          [&](const Expr *E) { printExpr(E); });
    OS << ")";
  }
}
...
void OpenACCClausePrinter::VisitDeviceTypeClause(
  ...
  OS << C.getClauseKind();
  OS << "(";
  toolchain::interleaveComma(C.getArchitectures(), OS,
