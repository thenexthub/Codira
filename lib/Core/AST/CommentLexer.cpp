...
void Token::dump(const Lexer &L, const SourceManager &SM) const {
  toolchain::errs() << "comments::Token Kind=" << Kind << " ";
  Loc.print(toolchain::errs(), SM);
  toolchain::errs() << " " << Length << " \"" << L.getSpelling(*this, SM) << "\"\n";
}
...
static inline StringRef convertCodePointToUTF8(
  ...
  if (toolchain::ConvertCodePointToUTF8(CodePoint, ResolvedPtr))
...
#include "language/Core/AST/CommentHTMLTags.inc"
#include "language/Core/AST/CommentHTMLNamedCharacterReferences.inc"

} // end anonymous namespace

StringRef Lexer::resolveHTMLNamedCharacterReference(StringRef Name) const {
  // Fast path, first check a few most widely used named character references.
  return toolchain::StringSwitch<StringRef>(Name)
      .Case("amp", "&")
      .Case("lt", "<")
      .Case("gt", ">")
      .Case("quot", "\"")
...
StringRef Lexer::resolveHTMLHexCharacterReference(StringRef Name) const {
  ...
  for (unsigned i = 0, e = Name.size(); i != e; ++i) {
    ...
    CodePoint += toolchain::hexDigitValue(C);
  }
  return convertCodePointToUTF8(Allocator, CodePoint);
}
...
const char *findCCommentEnd(const char *BufferPtr, const char *BufferEnd) {
  ...
  toolchain_unreachable("buffer end hit before '*/' was seen");
}
...
Lexer::Lexer(toolchain::BumpPtrAllocator &Allocator, DiagnosticsEngine &Diags,
...
again:
  ...
  case LCS_BeforeComment:
    ...
    default:
      toolchain_unreachable("second character of comment should be '/' or '*'");
    }
