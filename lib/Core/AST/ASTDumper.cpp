...
#include "language/Core/AST/ASTDumper.h"
...
using namespace language::Core::comments;
...
LLVM_DUMP_METHOD void QualType::dump() const {
  ASTDumper Dumper(toolchain::errs(), /*ShowColors=*/false);
  Dumper.Visit(*this);
}

LLVM_DUMP_METHOD void QualType::dump(toolchain::raw_ostream &OS,
...
LLVM_DUMP_METHOD void Type::dump(toolchain::raw_ostream &OS,
                                 const ASTContext &Context) const {
...
LLVM_DUMP_METHOD void TypeLoc::dump() const {
  ASTDumper(toolchain::errs(), /*ShowColors=*/false).Visit(*this);
}

LLVM_DUMP_METHOD void TypeLoc::dump(toolchain::raw_ostream &OS,
                                    const ASTContext &Context) const {
...
LLVM_DUMP_METHOD void Decl::dump() const { dump(toolchain::errs()); }
...
LLVM_DUMP_METHOD void Decl::dumpColor() const {
  ...
  ASTDumper P(toolchain::errs(), Ctx, /*ShowColors=*/true);
  P.Visit(this);
}
...
LLVM_DUMP_METHOD void DeclContext::dumpAsDecl(const ASTContext *Ctx) const {
  ...
  if (hasValidDeclKind()) {
    ...
    ASTDumper P = Ctx ? ASTDumper(toolchain::errs(), *Ctx,
                                  Ctx->getDiagnostics().getShowColors())
                      : ASTDumper(toolchain::errs(), /*ShowColors*/ false);
    P.dumpInvalidDeclContext(this);
  }
}

LLVM_DUMP_METHOD void DeclContext::dumpLookups() const {
  dumpLookups(toolchain::errs());
}
...
LLVM_DUMP_METHOD void Stmt::dump() const {
  ASTDumper P(toolchain::errs(), /*ShowColors=*/false);
  P.Visit(this);
}
...
LLVM_DUMP_METHOD void Stmt::dumpColor() const {
  ASTDumper P(toolchain::errs(), /*ShowColors=*/true);
  P.Visit(this);
}
...
LLVM_DUMP_METHOD void Comment::dumpColor() const {
  ...
  if (!FC)
    return;
  ASTDumper Dumper(toolchain::errs(), /*ShowColors=*/true);
  Dumper.Visit(FC, FC);
}
...
LLVM_DUMP_METHOD void TemplateName::dump(toolchain::raw_ostream &OS,
                                         const ASTContext &Context) const {
  ASTDumper Dumper(OS, Context, Context.getDiagnostics().getShowColors());
  Dumper.Visit(*this);
}
...
LLVM_DUMP_METHOD void TemplateArgument::dump(toolchain::raw_ostream &OS,
                                             const ASTContext &Context) const {
  ASTDumper Dumper(OS, Context, Context.getDiagnostics().getShowColors());
  Dumper.Visit(*this);
}
