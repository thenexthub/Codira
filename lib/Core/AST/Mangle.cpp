...
void language::Core::mangleObjCMethodName(raw_ostream &OS, bool includePrefixByte,
...
static CCMangling getCallingConvMangling(const ASTContext &Context,
  ...
  const toolchain::Triple &Triple = TI.getTriple();
...
void MangleContext::mangleName(GlobalDecl GD, raw_ostream &Out) {
  ...
  if (const AsmLabelAttr *ALA = D->getAttr<AsmLabelAttr>()) {
    ...
    if (ALA->getLabel().starts_with("toolchain.")) {
      Out << ALA->getLabel();
      return;
    }
    ...
    // toolchain mangler on ELF is a nop, so we can just avoid adding the \01
    // marker.
    StringRef UserLabelPrefix =
        getASTContext().getTargetInfo().getUserLabelPrefix();
#ifndef NDEBUG
    char GlobalPrefix =
        toolchain::DataLayout(getASTContext().getTargetInfo().getDataLayoutString())
            .getGlobalPrefix();
  ...
  for (const auto &AT : Proto->param_types()) {
    ...
    ArgWords += toolchain::alignTo(ASTContext.getTypeSize(AT), DefaultPtrWidth) /
                DefaultPtrWidth;
  }
...
void MangleContext::mangleMSGuidDecl(const MSGuidDecl *GD,
  ...
  Out << toolchain::format("_GUID_%08" PRIx32 "_%04" PRIx32 "_%04" PRIx32 "_",
                      P.Part1, P.Part2, P.Part3);
  unsigned I = 0;
  for (uint8_t C : P.Part4And5) {
    Out << toolchain::format("%02" PRIx8, C);
    if (++I == 2)
      Out << "_";
  }
}
...
void MangleContext::mangleObjCMethodName(const ObjCMethodDecl *MD,
  ...
  if (const auto *CID = MD->getCategory()) {
    ...
    toolchain_unreachable("Unexpected ObjC method decl context");
  }
  std::string MethodName;
  toolchain::raw_string_ostream MethodNameOS(MethodName);
  MD->getSelector().print(MethodNameOS);
  language::Core::mangleObjCMethodName(OS, includePrefixByte, MD->isInstanceMethod(),
...
public:
  ...
  bool writeName(const Decl *D, raw_ostream &OS) {
    ...
    toolchain::raw_svector_ostream FrontendBufOS(FrontendBuf);
    ...
    toolchain::Mangler::getNameWithPrefix(OS, FrontendBufOS.str(), DL);
    return false;
  }
  ...
  std::vector<std::string> getAllManglings(const ObjCContainerDecl *OCD) {
    ...
    auto Mangle = [&](ObjCKind Kind, StringRef ClassName) -> std::string {
      ...
      toolchain::Mangler::getNameWithPrefix(Mangled, Prefix + ClassName, DL);
...
private:
  ...
  std::string getMangledStructor(const NamedDecl *ND, unsigned StructorType) {
    ...
    toolchain::raw_string_ostream FOS(FrontendBuf);
    ...
    toolchain::raw_string_ostream BOS(BackendBuf);

    toolchain::Mangler::getNameWithPrefix(BOS, FrontendBuf, DL);
