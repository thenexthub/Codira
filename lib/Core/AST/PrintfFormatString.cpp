...
using language::Core::analyze_printf::PrintfSpecifier;

using namespace language::Core;

typedef language::Core::analyze_format_string::SpecifierResult<PrintfSpecifier>
...
static PrintfSpecifierResult ParsePrintfSpecifier(FormatStringHandler &H,
  ...
  using namespace language::Core::analyze_format_string;
  using namespace language::Core::analyze_printf;
  ...
  if (*I == '{') {
    ...
    do {
      ...
      if (R.match(Str, &Matches)) {
        ...
        if (MatchedStr.starts_with("mask")) {
          ...
          PrivacyFlags = language::Core::analyze_os_log::OSLogBufferItem::IsSensitive;
        else if (PrivacyFlags !=
                     language::Core::analyze_os_log::OSLogBufferItem::IsSensitive &&
                 MatchedStr == "private")
          PrivacyFlags = language::Core::analyze_os_log::OSLogBufferItem::IsPrivate;
        else if (PrivacyFlags == 0 && MatchedStr == "public")
          PrivacyFlags = language::Core::analyze_os_log::OSLogBufferItem::IsPublic;
      } else {
    ...
    case 0:
      break;
    case language::Core::analyze_os_log::OSLogBufferItem::IsPrivate:
      FS.setIsPrivate(MatchedStr.data());
      break;
    case language::Core::analyze_os_log::OSLogBufferItem::IsPublic:
      FS.setIsPublic(MatchedStr.data());
      break;
    case language::Core::analyze_os_log::OSLogBufferItem::IsSensitive:
      FS.setIsSensitive(MatchedStr.data());
      break;
    default:
      toolchain_unreachable("Unexpected privacy flag value");
    }
  }
...
bool language::Core::analyze_format_string::ParsePrintfString(FormatStringHandler &H,
                                                     const char *I,
                                                     const char *E,
                                                     const LangOptions &LO,
                                                     const TargetInfo &Target,
...
bool language::Core::analyze_format_string::ParseFormatStringHasSArg(const char *I,
                                                            const char *E,
                                                            const LangOptions &LO,
                                                            const TargetInfo &Target) {
...
bool language::Core::analyze_format_string::parseFormatStringHasFormattingSpecifiers(
  ...
  if (CS.getKind() == ConversionSpecifier::nArg) {
    switch (LM.getKind()) {
      ...
      case LengthModifier::AsWide:
        return ArgType::Invalid();
      case LengthModifier::AsShortLong:
        toolchain_unreachable("only used for OpenCL which doesn not handle nArg");
    }
  }
  ...
  if (hasValidLengthModifier(Ctx.getTargetInfo(), LangOpt)) {
    ...
    case ConversionSpecifier::UArg:
      if (QT->isSignedIntegerType())
        CS.setKind(language::Core::analyze_format_string::ConversionSpecifier::dArg);
      break;
    case ConversionSpecifier::dArg:
    case ConversionSpecifier::DArg:
    case ConversionSpecifier::iArg:
      if (QT->isUnsignedIntegerType() && !HasPlusPrefix)
        CS.setKind(language::Core::analyze_format_string::ConversionSpecifier::uArg);
      break;
    default:
