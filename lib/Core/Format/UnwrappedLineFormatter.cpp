...
static void printLineState(const LineState &State) {
  toolchain::dbgs() << "State: ";
  for (const ParenState &P : State.Stack) {
    toolchain::dbgs() << (P.Tok ? P.Tok->TokenText : "F") << "|" << P.Indent << "|"
                 << P.LastSpace << "|" << P.NestedBlockIndent << " ";
  }
  toolchain::dbgs() << State.NextToken->TokenText << "\n";
}
#endif
...
private:
  ...
  unsigned analyzeSolutionSpace(LineState &InitialState, bool DryRun) {
    ...
    if (Queue.empty()) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Could not find a solution.\n");
      return 0;
    }
  ...
  void reconstructPath(LineState &State, StateNode *Best) {
    toolchain::SmallVector<StateNode *> Path;
    // We do not need a break before the initial token.
    while (Best->Previous) {
      Path.push_back(Best);
      Best = Best->Previous;
    }
    for (const auto &Node : toolchain::reverse(Path)) {
      unsigned Penalty = 0;
      ...
      LLVM_DEBUG({
        ...
        if (Node->NewLine) {
          toolchain::dbgs() << "Penalty for placing "
  ...
  toolchain::SpecificBumpPtrAllocator<StateNode> Allocator;
};
