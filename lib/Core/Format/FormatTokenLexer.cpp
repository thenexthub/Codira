...
const toolchain::StringSet<> FormatTokenLexer::CSharpAttributeTargets = {
...
bool FormatTokenLexer::tryMergeTokensAny(
  ...
  return toolchain::any_of(Kinds, [this, NewType](ArrayRef<tok::TokenKind> Kinds) {
...
void FormatTokenLexer::tryParseJSRegexLiteral() {
  ...
  for (FormatToken *FT : toolchain::drop_begin(toolchain::reverse(Tokens))) {
...
void FormatTokenLexer::handleTableGenNumericLikeIdentifier() {
  ...
  // The following check is based on toolchain::TGLexer::LexToken.
...
FormatToken *FormatTokenLexer::getNextToken() {
  ...
  if (Style.isVerilog()) {
    static const toolchain::Regex NumberBase("^s?[bdho]", toolchain::Regex::IgnoreCase);
