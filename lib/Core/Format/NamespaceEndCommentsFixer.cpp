...
processTokens(const FormatToken *Tok, tok::TokenKind StartTok,
              ...
              toolchain::function_ref<void(const FormatToken *)> Fn) {
...
bool validEndComment(const FormatToken *RBraceTok, StringRef NamespaceName,
  ...
  static const toolchain::Regex NamespaceCommentPattern =
      toolchain::Regex("^/[/*] *(end (of )?)? *(anonymous|unnamed)? *"
                  "namespace( +([a-zA-Z0-9:_ ]+))?\\.? *(\\*/)?$",
                  toolchain::Regex::IgnoreCase);
  static const toolchain::Regex NamespaceMacroCommentPattern =
      toolchain::Regex("^/[/*] *(end (of )?)? *(anonymous|unnamed)? *"
  ...
  static const toolchain::Regex CommentPattern = toolchain::Regex(
      "^/[/*] *( +([a-zA-Z0-9:_]+))?\\.? *(\\*/)?$", toolchain::Regex::IgnoreCase);
...
void addEndComment(const FormatToken *RBraceTok, StringRef EndCommentText,
  ...
  if (Err) {
    toolchain::errs() << "Error while adding namespace end comment: "
...
void updateEndComment(const FormatToken *RBraceTok, StringRef EndCommentText,
  ...
  auto Err = Fixes->add(tooling::Replacement(SourceMgr, Range, EndCommentText));
  if (Err) {
    toolchain::errs() << "Error while updating namespace end comment: "
                 << toolchain::toString(std::move(Err)) << "\n";
  }
}
} // namespace
