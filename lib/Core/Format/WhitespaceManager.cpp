...
const tooling::Replacements &WhitespaceManager::generateReplacements() {
  ...
  toolchain::sort(Changes, Change::IsBeforeInFile(SourceMgr));
...
AlignTokenSequence(const FormatStyle &Style, unsigned Start, unsigned End,
  ...
  for (unsigned i = Start; i != End; ++i) {
    ...
    if (!FoundMatchOnLine && !SkipMatchCheck && Matches(CurrentChange)) {
      ...
      // http://toolchain.org/PR53699. An assertion later below verifies this.
    ...
    if (InsideNestedScope && CurrentChange.NewlinesBefore > 0) {
      ...
      auto ShouldShiftBeAdded = [&] {
        ...
        if (ScopeStart > Start + 1 &&
          ...
          for (unsigned OuterScopeStart : toolchain::reverse(ScopeStack)) {
            // Lambda.
