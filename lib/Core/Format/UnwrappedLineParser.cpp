...
void printLine(toolchain::raw_ostream &OS, const UnwrappedLine &Line,
...
LLVM_ATTRIBUTE_UNUSED static void printDebugInfo(const UnwrappedLine &Line) {
  printLine(toolchain::dbgs(), Line);
}

class ScopedDeclarationState {
public:
  ScopedDeclarationState(UnwrappedLine &Line, toolchain::BitVector &Stack,
...
private:
  UnwrappedLine &Line;
  toolchain::BitVector &Stack;
};

} // end anonymous namespace

std::ostream &operator<<(std::ostream &Stream, const UnwrappedLine &Line) {
  toolchain::raw_os_ostream OS(Stream);
  printLine(OS, Line);
  return Stream;
}
...
void UnwrappedLineParser::parse() {
  ...
  do {
    ...
    if (!ExpandedLines.empty()) {
      LLVM_DEBUG(toolchain::dbgs() << "Expanded lines:\n");
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Unwrapped lines:\n");
...
void UnwrappedLineParser::readTokenWithJavaScriptASI() {
  ...
  if (PreviousMustBeValue || Previous->is(tok::r_paren)) {
    ...
    bool HasAt = toolchain::any_of(Line->Tokens, [](UnwrappedLineNode &LineNode) {
...
static FormatToken *getLastNonComment(const UnwrappedLine &Line) {
  for (const auto &Token : toolchain::reverse(Line.Tokens))
...
void UnwrappedLineParser::parseUnbracedBody(bool CheckEOF) {
  ...
  if (Tok) {
    assert(!Line->InPPDirective);
    Tok = nullptr;
    for (const auto &L : toolchain::reverse(*CurrentLines)) {
...
void UnwrappedLineParser::addUnwrappedLine(LineLevel AdjustLevel) {
  ...
  LLVM_DEBUG({
    if (!parsingPPDirective()) {
      toolchain::dbgs() << "Adding unwrapped line:\n";
      printDebugInfo(*Line);
    }
  });
  ...
  if (!parsingPPDirective() && !InExpansion && containsExpansion(*Line)) {
    ...
    if (Reconstruct->finished()) {
      ...
      LLVM_DEBUG({
        toolchain::dbgs() << "Adding unexpanded line:\n";
...
continuesLineCommentSection(const FormatToken &FormatTok,
                            ...
                            const toolchain::Regex &CommentPragmasRegex) {
...
void UnwrappedLineParser::readToken(int LevelDifference) {
  ...
  do {
    ...
    if (FormatTok->is(tok::identifier) &&
      ...
      LLVM_DEBUG({
        toolchain::dbgs() << "Macro call: " << ID->TokenText << "(";
        if (Args) {
          toolchain::dbgs() << "(";
          for (const auto &Arg : Args.value())
            for (const auto &T : Arg)
              toolchain::dbgs() << T->TokenText << " ";
          toolchain::dbgs() << ")";
        }
        toolchain::dbgs() << "\n";
      });
      ...
      if ((!Args && Macros.objectLike(ID->TokenText)) ||
        ...
        LLVM_DEBUG({
          toolchain::dbgs() << "Did not expand macro \"" << ID->TokenText
                       << "\", because it was used ";
          if (Args)
            toolchain::dbgs() << "with " << Args->size();
          else
            toolchain::dbgs() << "without";
          toolchain::dbgs() << " arguments, which doesn't match any definition.\n";
        });
        Tokens->setPosition(Position);
        FormatTok = ID;
      }
...
std::optional<toolchain::SmallVector<toolchain::SmallVector<FormatToken *, 8>, 1>>
UnwrappedLineParser::parseMacroCall() {
  std::optional<toolchain::SmallVector<toolchain::SmallVector<FormatToken *, 8>, 1>> Args;
