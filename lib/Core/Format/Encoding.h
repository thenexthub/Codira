...
#ifndef LANGUAGE_CORE_LIB_FORMAT_ENCODING_H
#define LANGUAGE_CORE_LIB_FORMAT_ENCODING_H
...
inline Encoding detectEncoding(StringRef Text) {
  const toolchain::UTF8 *Ptr = reinterpret_cast<const toolchain::UTF8 *>(Text.begin());
  const toolchain::UTF8 *BufEnd = reinterpret_cast<const toolchain::UTF8 *>(Text.end());
  if (toolchain::isLegalUTF8String(&Ptr, BufEnd))
...
inline unsigned columnWidth(StringRef Text, Encoding Encoding) {
  if (Encoding == Encoding_UTF8) {
    int ContentWidth = toolchain::sys::unicode::columnWidthUTF8(Text);
...
inline unsigned getCodePointNumBytes(char FirstChar, Encoding Encoding) {
  switch (Encoding) {
  case Encoding_UTF8:
    return toolchain::getNumBytesForUTF8(FirstChar);
  default:
    return 1;
  }
}
...
inline unsigned getEscapeSequenceLength(StringRef Text) {
  ...
  default:
    ...
    return 1 + toolchain::getNumBytesForUTF8(Text[1]);
  }
}
