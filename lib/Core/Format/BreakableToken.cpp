...
static StringRef getLineCommentIndentPrefix(StringRef Comment,
  ...
  assert(
      toolchain::is_sorted(KnownPrefixes, [](StringRef Lhs, StringRef Rhs) noexcept {
...
getCommentSplit(StringRef Text, unsigned ContentStartColumn,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Comment split: \"" << Text
  ...
  static const auto kNumberedListRegexp = toolchain::Regex("^[1-9][0-9]?\\.");
...
unsigned BreakableStringLiteral::getRangeLength(unsigned LineIndex,
  ...
  toolchain_unreachable("Getting the length of a part of the string literal "
...
BreakableToken::Split BreakableStringLiteral::getSplit(
    ...
    unsigned ContentStartColumn, const toolchain::Regex &CommentPragmasRegex) const {
...
BreakableComment::getSplit(unsigned LineIndex, unsigned TailOffset,
                           ...
                           const toolchain::Regex &CommentPragmasRegex) const {
...
BreakableBlockComment::BreakableBlockComment(
  ...
  LLVM_DEBUG({
    toolchain::dbgs() << "IndentAtLineBreak " << IndentAtLineBreak << "\n";
    toolchain::dbgs() << "DelimitersOnNewline " << DelimitersOnNewline << "\n";
    for (size_t i = 0; i < Lines.size(); ++i) {
      toolchain::dbgs() << i << " |" << Content[i] << "| "
                   << "CC=" << ContentColumn[i] << "| "
...
BreakableToken::Split BreakableBlockComment::getReflowSplit(
    unsigned LineIndex, const toolchain::Regex &CommentPragmasRegex) const {
...
BreakableLineCommentSection::BreakableLineCommentSection(
  ...
  for (const FormatToken *CurrentTok = &Tok;
    ...
    for (size_t i = FirstLineIndex, e = Lines.size(); i < e; ++i) {
      ...
      const int SpacesInPrefix = toolchain::count(IndentPrefix, ' ');
