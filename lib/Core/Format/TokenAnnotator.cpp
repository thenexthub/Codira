...
#include "language/Core/Basic/TransformTypeTraits.def"
  ...
  bool isTableGenDAGArgBreakingOperator(const FormatToken &Tok) {
    ...
    if (!Tok.Next || Tok.Next->is(tok::colon))
      return false;
    return toolchain::is_contained(Opes, Tok.TokenText.str());
  }
...
private:
    ...
    auto IsQualifiedPointerOrReference = [](const FormatToken *T,
      ...
      while (T) {
        if (T->is(TT_AttributeRParen)) {
          ...
          // Handle `x = (foo *[[language::Core::foo]])&v;`:
  ...
  toolchain::SmallPtrSet<FormatToken *, 16> NonTemplateLess;
...
bool TokenAnnotator::mustBreakBefore(const AnnotatedLine &Line,
  ...
  if (Style.isCSharp()) {
    ...
    if (Right.is(tok::r_brace) && Left.is(tok::l_brace) && BeforeLeft &&
      ...
      toolchain_unreachable("Unknown FormatStyle::ShortLambdaStyle enum");
    }
...
void TokenAnnotator::printDebugInfo(const AnnotatedLine &Line) const {
  toolchain::errs() << "AnnotatedTokens(L=" << Line.Level << ", P=" << Line.PPLevel
               << ", T=" << Line.Type << ", C=" << Line.IsContinuation
               << "):\n";
  const FormatToken *Tok = Line.First;
  while (Tok) {
    toolchain::errs() << " M=" << Tok->MustBreakBefore
    ...
    for (prec::Level LParen : Tok->FakeLParens)
      toolchain::errs() << LParen << "/";
    toolchain::errs() << " FakeRParens=" << Tok->FakeRParens;
    toolchain::errs() << " II=" << Tok->Tok.getIdentifierInfo();
    toolchain::errs() << " Text='" << Tok->TokenText << "'\n";
    if (!Tok->Next)
