...
#ifndef LANGUAGE_CORE_LIB_FORMAT_UNWRAPPEDLINEPARSER_H
#define LANGUAGE_CORE_LIB_FORMAT_UNWRAPPEDLINEPARSER_H
...
private:
  ...
  // call.
  toolchain::DenseMap<FormatToken *, SmallVector<UnwrappedLine, 8>> ExpandedLines;

  // Map from the macro identifier to a line containing the full unexpanded
  // macro call.
  toolchain::DenseMap<FormatToken *, std::unique_ptr<UnwrappedLine>> Unexpanded;
  ...
  toolchain::BitVector DeclarationScopeStack;
  ...
  toolchain::Regex CommentPragmasRegex;
...
struct UnwrappedLineNode {
  ...
  UnwrappedLineNode(FormatToken *Tok,
                    toolchain::ArrayRef<UnwrappedLine> Children = {})
