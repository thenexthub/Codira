...
#include "language/Core/Tooling/Inclusions/HeaderIncludes.h"
#include "toolchain/ADT/Sequence.h"
#include <limits>

#define DEBUG_TYPE "format-formatter"

using language::Core::format::FormatStyle;
...
inline toolchain::Error make_string_error(const Twine &Message) {
...
std::string ParseErrorCategory::message(int EV) const {
  ...
  toolchain_unreachable("unexpected parse error");
}
...
FormatStyle getLLVMStyle(FormatStyle::LanguageKind Language) {
  ...
  LLVMStyle.IncludeStyle.IncludeCategories = {
      {"^\"(toolchain|toolchain-c|clang|clang-c)/", 2, 0, false},
...
bool getPredefinedStyle(StringRef Name, FormatStyle::LanguageKind Language,
                        FormatStyle *Style) {
  if (Name.equals_insensitive("toolchain"))
...
ParseError validateQualifierOrder(FormatStyle *Style) {
  ...
  if (!toolchain::is_contained(Style->QualifierOrder, "type"))
    return ParseError::MissingQualifierType;

  return ParseError::Success;
}

std::error_code parseConfiguration(toolchain::MemoryBufferRef Config,
                                   FormatStyle *Style, bool AllowUnknownOptions,
                                   toolchain::SourceMgr::DiagHandlerTy DiagHandler,
  ...
  toolchain::yaml::Input Input(Config, /*Ctxt=*/nullptr, DiagHandler,
                          DiagHandlerCtxt);
  ...
  for (const FormatStyle &Style : toolchain::reverse(Styles)) {
...
std::string configurationAsText(const FormatStyle &Style) {
  std::string Text;
  toolchain::raw_string_ostream Stream(Text);
  toolchain::yaml::Output Output(Stream);
    ...
    for (AnnotatedLine *Line : Lines) {
      ...
      for (FormatToken *FormatTok = Line->First; FormatTok;
        ...
        auto Replace = [&](SourceLocation Start, unsigned Length,
          ...
          if (Err) {
            toolchain::errs() << toString(std::move(Err)) << "\n";
            assert(false);
          }
        };
...
private:
  ...
  guessIsObjC(const SourceManager &SourceManager,
    ...
    static constexpr toolchain::StringLiteral FoundationIdentifiers[] = {
        "CGFloat",
        "CGPoint",
        "CGPointMake",
        "CGPointZero",
      ...
      for (const FormatToken *FormatTok = Line->First; FormatTok;
        ...
        if ((FormatTok->Previous && FormatTok->Previous->is(tok::at) &&
            ...
            (FormatTok->Tok.isAnyIdentifier() &&
             toolchain::binary_search(FoundationIdentifiers,
                                 FormatTok->TokenText)) ||
...
static void sortCppIncludes(const FormatStyle &Style,
  ...
  SmallVector<unsigned, 16> Indices =
      toolchain::to_vector<16>(toolchain::seq<unsigned>(0, Includes.size()));
  ...
  if (Style.SortIncludes.Enabled) {
    stable_sort(Indices, [&](unsigned LHSI, unsigned RHSI) {
      ...
      if (Style.SortIncludes.IgnoreExtension) {
        ...
        toolchain::sys::path::replace_extension(LHSStem, "");
        toolchain::sys::path::replace_extension(RHSStem, "");
      }
  ...
  Indices.erase(toolchain::unique(Indices,
...
tooling::Replacements sortCppIncludes(const FormatStyle &Style, StringRef Code,
                                      ...
                                      unsigned *Cursor) {
  unsigned Prev = toolchain::StringSwitch<size_t>(Code)
  ...
  // (http://toolchain.org/docs/CodingStandards.html#include-style).
  //
  ...
  toolchain::Regex RawStringRegex(
      "R\"([][A-Za-z0-9_{}#<>%:;.?*+/^&\\$|~!=,'-]*)\\(");
...
static void sortJavaImports(const FormatStyle &Style,
  ...
  SmallVector<unsigned, 16> Indices =
      toolchain::to_vector<16>(toolchain::seq<unsigned>(0, Imports.size()));
...
tooling::Replacements sortJavaImports(const FormatStyle &Style, StringRef Code,
  ...
  unsigned Prev = 0;
  unsigned SearchFrom = 0;
  toolchain::Regex ImportRegex(JavaImportRegexPattern);
  SmallVector<StringRef, 4> Matches;
...
fixCppIncludeInsertions(StringRef Code, const tooling::Replacements &Replaces,
  ...
  for (const auto &R : Replaces) {
    if (isHeaderInsertion(R)) {
      ...
      toolchain::errs() << "Insertions other than header #include insertion are "
                      "not supported! "
  ...
  for (const auto &Header : HeadersToDelete) {
    ...
    for (const auto &R : Replaces) {
      ...
      if (Err) {
        ...
        toolchain::errs() << "Failed to add header deletion replacement for "
  ...
  if (Style.QualifierAlignment != FormatStyle::QAS_Leave) {
    ...
    for (const tooling::Replacement &Fix : Fixes) {
      ...
      if (OriginalCode != Fix.getReplacementText()) {
        ...
        if (Err) {
          toolchain::errs() << "Error adding replacements : "
...
const char *StyleOptionHelpDescription =
    ...
    "   --style=\"{BasedOnStyle: toolchain, IndentWidth: 8}\"";
...
FormatStyle::LanguageKind guessLanguage(StringRef FileName, StringRef Code) {
  ...
  if (GuessedLanguage == FormatStyle::LK_Cpp) {
    auto Extension = toolchain::sys::path::extension(FileName);
...
loadAndParseConfigFile(StringRef ConfigFile, toolchain::vfs::FileSystem *FS,
                       FormatStyle *Style, bool AllowUnknownOptions,
                       toolchain::SourceMgr::DiagHandlerTy DiagHandler,
                       bool IsDotHFile) {
...
Expected<FormatStyle> getStyle(StringRef StyleName, StringRef FileName,
  ...
  SmallVector<std::unique_ptr<toolchain::MemoryBuffer>, 1> ChildFormatTextToApply;

  if (StyleName.starts_with("{")) {
    // Parse YAML/JSON style from the command line.
    StringRef Source = "<command-line>";
    if (std::error_code ec =
            parseConfiguration(toolchain::MemoryBufferRef(StyleName, Source), &Style,
    ...
    ChildFormatTextToApply.emplace_back(
        toolchain::MemoryBuffer::getMemBuffer(StyleName, Source, false));
  }

  if (!FS)
    FS = toolchain::vfs::getRealFileSystem().get();
  assert(FS);
  ...
  auto dropDiagnosticHandler = [](const toolchain::SMDiagnostic &, void *) {};

  auto applyChildFormatTexts = [&](FormatStyle *Style) {
    for (const auto &MemBuf : toolchain::reverse(ChildFormatTextToApply)) {
      auto EC =
  ...
  for (StringRef Directory = Path; !Directory.empty();
       Directory = toolchain::sys::path::parent_path(Directory)) {
    auto Status = FS->status(Directory);
    if (!Status ||
        Status->getType() != toolchain::sys::fs::file_type::directory_file) {
      continue;
    }
    ...
    for (const auto &F : FilesToLookFor) {
      ...
      toolchain::sys::path::append(ConfigFile, F);
      LLVM_DEBUG(toolchain::dbgs() << "Trying " << ConfigFile << "...\n");

      Status = FS->status(ConfigFile);
      if (!Status ||
          Status->getType() != toolchain::sys::fs::file_type::regular_file) {
        continue;
      }
      ...
      if (!Style.InheritsParentConfig) {
        if (!ChildFormatTextToApply.empty()) {
          LLVM_DEBUG(toolchain::dbgs() << "Applying child configurations\n");
          applyChildFormatTexts(&Style);
        }
        return Style;
      }

      LLVM_DEBUG(toolchain::dbgs() << "Inherits parent configuration\n");
