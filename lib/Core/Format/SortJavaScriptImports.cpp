...
public:
  ...
  analyze(TokenAnnotator &Annotator,
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Replacing imports:\n"
                            << PreviousText << "\nwith:\n"
                            << ReferencesText << "\n");
...
private:
  ...
  sortModuleReferences(const SmallVector<JsModuleReference, 16> &References) {
    ...
    while (Start != References.end()) {
      ...
      toolchain::append_range(ReferencesSorted, SortChunk);
    }
  ...
  parseModuleReferences(const AdditionalKeywords &Keywords,
    ...
    for (auto *Line : AnnotatedLines) {
      ...
      LLVM_DEBUG({
        toolchain::dbgs() << "JsModuleReference: {"
        ...
        for (const JsImportedSymbol &Symbol : Reference.Symbols)
          toolchain::dbgs() << ", " << Symbol.Symbol << " as " << Symbol.Alias;
        toolchain::dbgs() << ", text: " << getSourceText(Reference.Range);
        toolchain::dbgs() << "}\n";
      });
