...
bool ContinuationIndenter::mustBreak(const LineState &State) {
  ...
  if (Style.isJavaScript() && Previous.is(tok::r_paren) &&
    ...
    static const toolchain::StringSet<> BreakBeforeDecoratedTokens = {"get", "set",
                                                                 "function"};
...
unsigned ContinuationIndenter::moveStateToNextToken(LineState &State,
  ...
  if (Current.isNot(tok::comment) && !Current.ClosesRequiresClause &&
    ...
    if (State.Stack[State.Stack.size() - 2].NestedBlockInlined && Newline)
      for (ParenState &PState : toolchain::drop_end(State.Stack))
...
void ContinuationIndenter::moveStatePastFakeLParens(LineState &State,
  ...
  for (const auto &PrecedenceLevel : toolchain::reverse(Current.FakeLParens)) {
    const auto &CurrentState = State.Stack.back();
...
unsigned ContinuationIndenter::reformatRawStringLiteral(
  ...
  if (!DryRun) {
    if (NewDelimiter != OldDelimiter) {
      ...
      if (PrefixErr) {
        toolchain::errs()
            ...
            << toolchain::toString(std::move(PrefixErr)) << "\n";
      }
      ...
      if (SuffixErr) {
        toolchain::errs()
            ...
            << toolchain::toString(std::move(SuffixErr)) << "\n";
      }
    }
    ...
    for (const tooling::Replacement &Fix : Fixes.first) {
      ...
      if (Err) {
        toolchain::errs() << "Failed to reformat raw string: "
...
ContinuationIndenter::breakProtrudingToken(const FormatToken &Current,
  ...
  unsigned Penalty = 0;
  LLVM_DEBUG(toolchain::dbgs() << "Breaking protruding token at column "
                          << StartColumn << ".\n");
  for (unsigned LineIndex = 0, EndIndex = Token->getLineCount();
    ...
    while (ContentStartColumn + RemainingTokenColumns > ColumnLimit) {
      LLVM_DEBUG(toolchain::dbgs() << "    Over limit, need: "
      ...
      if (Split.first == StringRef::npos) {
        ...
        LLVM_DEBUG(toolchain::dbgs() << "    No break opportunity.\n");
        break;
      }
      ...
      if (Token->supportsReflow()) {
        ...
        LLVM_DEBUG(toolchain::dbgs() << "    ToSplit: " << ToSplitColumns << "\n");
        ...
        if (ContinueOnLine) {
          LLVM_DEBUG(toolchain::dbgs() << "    Continuing on line...\n");
      ...
      LLVM_DEBUG(toolchain::dbgs() << "    Breaking...\n");
      ...
      LLVM_DEBUG(toolchain::dbgs() << "    Breaking at: " << TailOffset + Split.first
                              << ", " << Split.second << "\n");
      if (!DryRun) {
    ...
    if (LineIndex + 1 != EndIndex) {
      ...
      if (TryReflow) {
        ...
        if (SplitBeforeNext.first != StringRef::npos) {
          LLVM_DEBUG(toolchain::dbgs() << SplitBeforeNext.first << ", "
          ...
          if (ContentStartColumn + RemainingTokenColumns > ColumnLimit) {
            ...
            BreakableToken::Split Split =
                Token->getSplit(NextLineIndex, TailOffset, ColumnLimit,
                                ContentStartColumn, CommentPragmasRegex);
            if (Split.first == StringRef::npos) {
              LLVM_DEBUG(toolchain::dbgs() << "    Did not find later break\n");
              Reflow = false;
            } else {
              ...
              if (ContentStartColumn + ToSplitColumns > ColumnLimit) {
                LLVM_DEBUG(toolchain::dbgs() << "    Next split protrudes, need: "
          ...
          LLVM_DEBUG(toolchain::dbgs() << "not found.\n");
        }
      }
      if (!Reflow) {
  ...
  if (SplitAfterLastLine.first != StringRef::npos) {
    LLVM_DEBUG(toolchain::dbgs() << "Replacing whitespace after last line.\n");
