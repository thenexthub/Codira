...
CIRGenTypes::arrangeCXXStructorDeclaration(GlobalDecl gd) {
  ...
  toolchain::SmallVector<CanQualType, 16> argTypes;
...
RValue CIRGenFunction::emitCall(const CIRGenFunctionInfo &funcInfo,
  ...
  for (auto [argNo, arg, canQualArgType] :
       toolchain::enumerate(args, funcInfo.argTypes())) {
  ...
  toolchain_unreachable("Invalid evaluation kind");
}

void CIRGenFunction::emitCallArg(CallArgList &args, const language::Core::Expr *e,
...
void CIRGenFunction::emitCallArgs(
  ...
  toolchain::SmallVector<QualType, 16> argTypes;
  ...
  for (const language::Core::Expr *a : toolchain::drop_begin(argRange, argTypes.size()))
