...
struct CIRRecordLowering final {
  ...
  mlir::Type getUIntNType(uint64_t numBits) {
    unsigned alignedBits = toolchain::PowerOf2Ceil(numBits);
  ...
  mlir::Type getBitfieldStorageType(unsigned numBits) {
    unsigned alignedBits = toolchain::alignTo(numBits, astContext.getCharWidth());
  ...
  toolchain::SmallVector<mlir::Type, 16> fieldTypes;
  toolchain::DenseMap<const FieldDecl *, CIRGenBitFieldInfo> bitFields;
  toolchain::DenseMap<const FieldDecl *, unsigned> fieldIdxMap;
...
CIRGenTypes::computeRecordLayout(const RecordDecl *rd, cir::RecordType *ty) {
  ...
  if (toolchain::isa<CXXRecordDecl>(rd) && !rd->isUnion() &&
...
void CIRGenRecordLayout::print(raw_ostream &os) const {
  ...
  toolchain::array_pod_sort(bitInfo.begin(), bitInfo.end());
...
void CIRGenRecordLayout::dump() const { print(toolchain::errs()); }

void CIRGenBitFieldInfo::dump() const { print(toolchain::errs()); }
