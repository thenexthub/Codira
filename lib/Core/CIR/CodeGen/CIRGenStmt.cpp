...
#include "language/Core/AST/StmtNodes.inc"
  ...
  toolchain_unreachable("Unexpected statement class");
}
...
mlir::LogicalResult CIRGenFunction::emitLabelStmt(const language::Core::LabelStmt &s) {
...
mlir::LogicalResult
CIRGenFunction::emitContinueStmt(const language::Core::ContinueStmt &s) {
...
mlir::LogicalResult CIRGenFunction::emitLabel(const language::Core::LabelDecl &d) {
...
mlir::LogicalResult CIRGenFunction::emitBreakStmt(const language::Core::BreakStmt &s) {
...
mlir::LogicalResult CIRGenFunction::emitCaseStmt(const CaseStmt &s,
  ...
  toolchain::APSInt intVal = s.getLHS()->EvaluateKnownConstInt(getContext());
  ...
  if (const Expr *rhs = s.getRHS()) {
    toolchain::APSInt endVal = rhs->EvaluateKnownConstInt(getContext());
...
mlir::LogicalResult CIRGenFunction::emitDefaultStmt(const language::Core::DefaultStmt &s,
                                                    mlir::Type condType,
                                                    bool buildingTopLevelCase) {
...
mlir::LogicalResult CIRGenFunction::emitSwitchCase(const SwitchCase &s,
  ...
  toolchain_unreachable("expect case or default stmt");
}
...
mlir::LogicalResult CIRGenFunction::emitSwitchStmt(const language::Core::SwitchStmt &s) {
  ...
  toolchain::SmallVector<CaseOp> cases;
