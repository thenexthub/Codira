...
bool CIRGenTypes::isFuncParamTypeConvertible(language::Core::QualType type) {
...
std::string CIRGenTypes::getRecordTypeName(const language::Core::RecordDecl *recordDecl,
                                           StringRef suffix) {
  toolchain::SmallString<256> typeName;
  toolchain::raw_svector_ostream outStream(typeName);
...
isSafeToConvert(QualType qt, CIRGenTypes &cgt,
                toolchain::SmallPtrSetImpl<const RecordDecl *> &alreadyChecked);
...
static bool isSafeToConvert(const RecordDecl *rd, CIRGenTypes &cgt) {
  ...
  toolchain::SmallPtrSet<const RecordDecl *, 16> alreadyChecked;
  return isSafeToConvert(rd, cgt, alreadyChecked);
}

/// Lay out a tagged decl type like struct or union.
mlir::Type CIRGenTypes::convertRecordDeclType(const language::Core::RecordDecl *rd) {
...
mlir::Type CIRGenTypes::convertType(QualType type) {
  ...
  case Type::Complex: {
    const auto *ct = cast<language::Core::ComplexType>(ty);
...
mlir::Type CIRGenTypes::convertTypeForMem(language::Core::QualType qualType,
                                          bool forBitField) {
...
bool CIRGenTypes::isZeroInitializable(language::Core::QualType t) {
