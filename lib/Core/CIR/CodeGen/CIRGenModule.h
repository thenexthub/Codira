...
#ifndef LANGUAGE_CORE_LIB_CIR_CODEGEN_CIRGENMODULE_H
#define LANGUAGE_CORE_LIB_CIR_CODEGEN_CIRGENMODULE_H
...
public:
  CIRGenModule(mlir::MLIRContext &mlirContext, language::Core::ASTContext &astContext,
...
private:
  ...
  const language::Core::TargetInfo &target;
  ...
  toolchain::SmallVector<mlir::Attribute> globalScopeAsm;

public:
  mlir::ModuleOp getModule() const { return theModule; }
  CIRGenBuilderTy &getBuilder() { return builder; }
  language::Core::ASTContext &getASTContext() const { return astContext; }
  const language::Core::TargetInfo &getTarget() const { return target; }
  const language::Core::CodeGenOptions &getCodeGenOpts() const { return codeGenOpts; }
  CIRGenTypes &getTypes() { return genTypes; }
  const language::Core::LangOptions &getLangOpts() const { return langOpts; }
  ...
  toolchain::DenseMap<const Decl *, cir::GlobalOp> staticLocalDeclMap;

  mlir::Operation *getGlobalValue(toolchain::StringRef ref);
  ...
  /// mlir::GlobalOp value
  cir::GlobalOp getOrCreateCIRGlobal(toolchain::StringRef mangledName, mlir::Type ty,
  ...
  static cir::GlobalOp createGlobalOp(CIRGenModule &cgm, mlir::Location loc,
                                      toolchain::StringRef name, mlir::Type t,
                                      mlir::Operation *insertPoint = nullptr);

  toolchain::StringMap<unsigned> cgGlobalNames;
  ...
  cir::GlobalOp createOrReplaceCXXRuntimeVariable(
      mlir::Location loc, toolchain::StringRef name, mlir::Type ty,
      cir::GlobalLinkageKind linkage, language::Core::CharUnits alignment);
  ...
  /// MLIR Location.
  mlir::Location getLoc(language::Core::SourceLocation cLoc);
  mlir::Location getLoc(language::Core::SourceRange cRange);

  /// Return the best known alignment for an unknown pointer to a
  /// particular class.
  language::Core::CharUnits getClassPointerAlignment(const language::Core::CXXRecordDecl *rd);

  /// FIXME: this could likely be a common helper and not necessarily related
  /// with codegen.
  language::Core::CharUnits getNaturalTypeAlignment(language::Core::QualType t,
                                           LValueBaseInfo *baseInfo);

  cir::FuncOp
  getAddrOfCXXStructor(language::Core::GlobalDecl gd,
  ...
  std::pair<cir::FuncType, cir::FuncOp> getAddrAndTypeOfCXXStructor(
      language::Core::GlobalDecl gd, const CIRGenFunctionInfo *fnInfo = nullptr,
      cir::FuncType fnType = nullptr, bool dontDefer = false,
      ForDefinition_t isForDefinition = NotForDefinition);
  ...
  /// yet.
  std::map<toolchain::StringRef, language::Core::GlobalDecl> deferredDecls;

  // This is a list of deferred decls which we have seen that *are* actually
  // referenced. These get code generated when the module is done.
  std::vector<language::Core::GlobalDecl> deferredDeclsToEmit;
  void addDeferredDeclToEmit(language::Core::GlobalDecl GD) {
    deferredDeclsToEmit.emplace_back(GD);
  }

  void emitTopLevelDecl(language::Core::Decl *decl);

  /// Determine whether the definition must be emitted; if this returns \c
  /// false, the definition can be emitted lazily if it's used.
  bool mustBeEmitted(const language::Core::ValueDecl *d);
  ...
  bool mayBeEmittedEagerly(const language::Core::ValueDecl *d);
  ...
  cir::FuncOp
  getAddrOfFunction(language::Core::GlobalDecl gd, mlir::Type funcType = nullptr,
                    bool forVTable = false, bool dontDefer = false,
                    ForDefinition_t isForDefinition = NotForDefinition);

  mlir::Operation *
  getAddrOfGlobal(language::Core::GlobalDecl gd,
                  ForDefinition_t isForDefinition = NotForDefinition);
  ...
  void emitGlobal(language::Core::GlobalDecl gd);

  void emitAliasForGlobal(toolchain::StringRef mangledName, mlir::Operation *op,
  ...
  void emitGlobalDefinition(language::Core::GlobalDecl gd,
                            mlir::Operation *op = nullptr);
  void emitGlobalFunctionDefinition(language::Core::GlobalDecl gd, mlir::Operation *op);
  void emitGlobalVarDefinition(const language::Core::VarDecl *vd,
                               bool isTentative = false);

  void emitGlobalOpenACCDecl(const language::Core::OpenACCConstructDecl *cd);
  ...
  toolchain::StringRef getMangledName(language::Core::GlobalDecl gd);
  ...
  cir::FuncOp codegenCXXStructor(language::Core::GlobalDecl gd);

  bool supportsCOMDAT() const;
  void maybeSetTrivialComdat(const language::Core::Decl &d, mlir::Operation *op);
  ...
  cir::FuncOp
  getOrCreateCIRFunction(toolchain::StringRef mangledName, mlir::Type funcType,
                         language::Core::GlobalDecl gd, bool forVTable,
                         bool dontDefer = false, bool isThunk = false,
                         ForDefinition_t isForDefinition = NotForDefinition,
                         mlir::ArrayAttr extraAttrs = {});

  cir::FuncOp createCIRFunction(mlir::Location loc, toolchain::StringRef name,
                                cir::FuncType funcType,
                                const language::Core::FunctionDecl *funcDecl);
  ...
  void emitGlobalDecl(const language::Core::GlobalDecl &d);
  ...
  static cir::VisibilityKind getGlobalVisibilityKindFromClangVisibility(
      language::Core::VisibilityAttr::VisibilityType visibility);
  ...
  void addReplacement(toolchain::StringRef name, mlir::Operation *op);

  /// Helpers to emit "not yet implemented" error diagnostics
  DiagnosticBuilder errorNYI(SourceLocation, toolchain::StringRef);

  template <typename T>
  DiagnosticBuilder errorNYI(SourceLocation loc, toolchain::StringRef feature,
                             const T &name) {
    unsigned diagID =
  ...
  DiagnosticBuilder errorNYI(mlir::Location loc, toolchain::StringRef feature) {
  ...
  DiagnosticBuilder errorNYI(toolchain::StringRef feature) const {
  ...
  DiagnosticBuilder errorNYI(SourceRange, toolchain::StringRef);

  template <typename T>
  DiagnosticBuilder errorNYI(SourceRange loc, toolchain::StringRef feature,
                             const T &name) {
...
private:
  ...
  toolchain::MapVector<language::Core::GlobalDecl, toolchain::StringRef> mangledDeclNames;
  toolchain::StringMap<language::Core::GlobalDecl, toolchain::BumpPtrAllocator> manglings;

  // FIXME: should we use toolchain::TrackingVH<mlir::Operation> here?
  typedef toolchain::StringMap<mlir::Operation *> ReplacementsTy;
...
#endif // LANGUAGE_CORE_LIB_CIR_CODEGEN_CIRGENMODULE_H
