...
bool CIRGenFunction::constantFoldsToSimpleInteger(const Expr *cond,
  ...
  toolchain::APSInt intValue = result.Val.getInt();
...
cir::ReturnOp CIRGenFunction::LexicalScope::emitReturn(mlir::Location loc) {
  ...
  if (!cgf.curFn.getFunctionType().hasVoidReturn()) {
    ...
    return builder.create<cir::ReturnOp>(loc,
                                         toolchain::ArrayRef(value.getResult()));
  }
  return builder.create<cir::ReturnOp>(loc);
}
...
void CIRGenFunction::LexicalScope::emitImplicitReturn() {
  ...
  const auto *fd = cast<language::Core::FunctionDecl>(cgf.curGD.getDecl());
...
void CIRGenFunction::startFunction(GlobalDecl gd, QualType returnType,
  ...
  for (const auto nameValue : toolchain::zip(args, entryBB->getArguments())) {
...
mlir::LogicalResult CIRGenFunction::emitFunctionBody(const language::Core::Stmt *body) {
...
cir::FuncOp CIRGenFunction::generateCode(language::Core::GlobalDecl gd, cir::FuncOp fn,
  ...
  auto validMLIRLoc = [&](language::Core::SourceLocation clangLoc) {
  ...
  SymTableScopeTy varScope(symbolTable);
  {
    ...
    if (isa<CXXDestructorDecl>(funcDecl)) {
      ...
      toolchain_unreachable("no definition for normal function");
    }
...
void CIRGenFunction::emitDestructorBody(FunctionArgList &args) {
  ...
  case Dtor_Deleting:
    toolchain_unreachable("already handled deleting case");
...
language::Core::QualType CIRGenFunction::buildFunctionArgList(language::Core::GlobalDecl gd,
                                                     FunctionArgList &args) {
...
static std::string getVersionedTmpName(toolchain::StringRef name, unsigned cnt) {
...
mlir::Value
CIRGenFunction::emitArrayLength(const language::Core::ArrayType *origArrayType,
                                QualType &baseType, Address &addr) {
  const language::Core::ArrayType *arrayType = origArrayType;
...
#include "language/Core/AST/TypeNodes.inc"
    ...
    case Type::Adjusted:
      type = cast<language::Core::AdjustedType>(ty)->getAdjustedType();
      break;

    case Type::Decayed:
      type = cast<language::Core::DecayedType>(ty)->getPointeeType();
      break;

    case Type::Pointer:
      type = cast<language::Core::PointerType>(ty)->getPointeeType();
      break;

    case Type::BlockPointer:
      type = cast<language::Core::BlockPointerType>(ty)->getPointeeType();
      break;

    case Type::LValueReference:
    case Type::RValueReference:
      type = cast<language::Core::ReferenceType>(ty)->getPointeeType();
      break;

    case Type::MemberPointer:
      type = cast<language::Core::MemberPointerType>(ty)->getPointeeType();
      break;

    case Type::ConstantArray:
    case Type::IncompleteArray:
      // Losing element qualification here is fine.
      type = cast<language::Core::ArrayType>(ty)->getElementType();
      break;
    ...
    case Type::FunctionProto:
    case Type::FunctionNoProto:
      type = cast<language::Core::FunctionType>(ty)->getReturnType();
      break;
    ...
    case Type::TypeOfExpr:
      ...
      emitIgnoredExpr(cast<language::Core::TypeOfExprType>(ty)->getUnderlyingExpr());
      return;

    case Type::Atomic:
      type = cast<language::Core::AtomicType>(ty)->getValueType();
      break;

    case Type::Pipe:
      type = cast<language::Core::PipeType>(ty)->getElementType();
      break;
    }
