...
mlir::Value ComplexExprEmitter::emitCast(CastKind ck, Expr *op,
  ...
  switch (ck) {
  case CK_Dependent:
    toolchain_unreachable("dependent type must be resolved before the CIR codegen");
...
mlir::Value ComplexExprEmitter::emitComplexToComplexCast(mlir::Value val,
  ...
  if (srcElemTy->isFloatingType() && destElemTy->isFloatingType())
    ...
    toolchain_unreachable("unexpected src type or dest type");
...
mlir::Value ComplexExprEmitter::emitScalarToComplexCast(mlir::Value val,
  ...
  if (srcType->isFloatingType())
    ...
    toolchain_unreachable("unexpected src type");
...
ComplexExprEmitter::VisitImaginaryLiteral(const ImaginaryLiteral *il) {
  ...
  if (mlir::isa<cir::IntType>(elementTy)) {
    toolchain::APInt imagValue = cast<IntegerLiteral>(il->getSubExpr())->getValue();
    ...
    toolchain::APFloat imagValue =
        cast<FloatingLiteral>(il->getSubExpr())->getValue();
    realValueAttr = cir::FPAttr::get(
        elementTy, toolchain::APFloat::getZero(imagValue.getSemantics()));
...
static CompoundFunc getComplexOp(BinaryOperatorKind op) {
  ...
  case BO_DivAssign:
    toolchain_unreachable("getComplexOp: BO_DivAssign");
  case BO_SubAssign:
