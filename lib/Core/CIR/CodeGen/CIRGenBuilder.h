...
#ifndef LANGUAGE_CORE_LIB_CIR_CODEGEN_CIRGENBUILDER_H
#define LANGUAGE_CORE_LIB_CIR_CODEGEN_CIRGENBUILDER_H
...
#include "language/Core/CIR/Dialect/Builder/CIRBaseBuilder.h"
#include "language/Core/CIR/MissingFeatures.h"
...
class CIRGenBuilderTy : public cir::CIRBaseBuilderTy {
  ...
  toolchain::StringMap<unsigned> recordNames;
  toolchain::StringMap<unsigned> globalsVersioning;
...
public:
  ...
  mlir::Attribute getString(toolchain::StringRef str, mlir::Type eltTy,
    ...
    // a #cir.const_array.
    if (lastNonZeroPos == toolchain::StringRef::npos) {
  ...
  cir::LongDoubleType getLongDoubleTy(const toolchain::fltSemantics &format) const {
    ...
    if (&format == &toolchain::APFloat::PPCDoubleDouble())
      toolchain_unreachable("NYI: PPC double-double format for long double");
    toolchain_unreachable("Unsupported format for long double");
  }

  /// Get a CIR record kind from a AST declaration tag.
  cir::RecordType::RecordKind getRecordKind(const language::Core::TagTypeKind kind) {
    switch (kind) {
    case language::Core::TagTypeKind::Class:
      return cir::RecordType::Class;
    case language::Core::TagTypeKind::Struct:
      return cir::RecordType::Struct;
    case language::Core::TagTypeKind::Union:
      return cir::RecordType::Union;
    case language::Core::TagTypeKind::Interface:
      toolchain_unreachable("interface records are NYI");
    case language::Core::TagTypeKind::Enum:
      toolchain_unreachable("enums are not records");
    }
    toolchain_unreachable("Unsupported record kind");
  }
  ...
  cir::RecordType getCompleteRecordTy(toolchain::ArrayRef<mlir::Type> members,
                                      toolchain::StringRef name, bool packed,
                                      bool padded) {
  ...
  cir::RecordType getIncompleteRecordTy(toolchain::StringRef name,
                                        const language::Core::RecordDecl *rd) {
  ...
  bool isNullValue(mlir::Attribute attr) const {
    ...
    if (auto fpAttr = mlir::dyn_cast<cir::FPAttr>(attr)) {
      ...
      bool ignored;
      toolchain::APFloat fv(+0.0);
      fv.convert(fpVal.getSemantics(), toolchain::APFloat::rmNearestTiesToEven,
                 &ignored);
  ...
  cir::ConstantOp getConstInt(mlir::Location loc, toolchain::APSInt intVal);

  cir::ConstantOp getConstInt(mlir::Location loc, toolchain::APInt intVal);
  ...
  cir::RecordType getAnonRecordTy(toolchain::ArrayRef<mlir::Type> members,
  ...
  mlir::Value createNeg(mlir::Value value) {
    ...
    toolchain_unreachable("negation for the given type is NYI");
  }
  ...
  void computeGlobalViewIndicesFromFlatOffset(
      ...
      toolchain::SmallVectorImpl<int64_t> &indices);
