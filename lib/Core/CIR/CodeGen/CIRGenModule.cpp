...
void CIRGenModule::emitGlobalDecl(const language::Core::GlobalDecl &d) {
...
void CIRGenModule::emitGlobal(language::Core::GlobalDecl gd) {
  if (const auto *cd = dyn_cast<language::Core::OpenACCConstructDecl>(gd.getDecl())) {
  ...
  toolchain::StringRef mangledName = getMangledName(gd);
...
void CIRGenModule::emitGlobalFunctionDefinition(language::Core::GlobalDecl gd,
                                                mlir::Operation *op) {
...
void CIRGenModule::emitGlobalVarDefinition(const language::Core::VarDecl *vd,
                                           bool isTentative) {
...
void CIRGenModule::emitGlobalDefinition(language::Core::GlobalDecl gd,
                                        mlir::Operation *op) {
  ...
  toolchain_unreachable("Invalid argument to CIRGenModule::emitGlobalDefinition");
}
...
static bool shouldBeInCOMDAT(CIRGenModule &cgm, const Decl &d) {
  ...
  switch (linkage) {
  case language::Core::GVA_Internal:
  case language::Core::GVA_AvailableExternally:
  case language::Core::GVA_StrongExternal:
    return false;
  case language::Core::GVA_DiscardableODR:
  case language::Core::GVA_StrongODR:
    return true;
  }
...
void CIRGenModule::replacePointerTypeArgs(cir::FuncOp oldF, cir::FuncOp newF) {
  ...
  for (const mlir::SymbolTable::SymbolUse &u : *optionalUseRange) {
    ...
    for (const auto [argOp, fnArgType] :
         toolchain::zip(call.getArgs(), newF.getFunctionType().getInputs())) {
...
void CIRGenModule::applyReplacements() {
  for (auto &i : replacements) {
    ...
    if (oldF.replaceAllSymbolUses(newF.getSymNameAttr(), theModule).failed())
      toolchain_unreachable("internal error, cannot RAUW symbol");
    if (newF) {
...
// LLVM IR automatically uniques names when new toolchain::GlobalVariables are
...
cir::GlobalOp CIRGenModule::getGlobalForStringLiteral(const StringLiteral *s,
  ...
  auto typedC = toolchain::cast<mlir::TypedAttr>(c);
  cir::GlobalOp gv =
...
cir::FuncOp CIRGenModule::getAddrOfFunction(language::Core::GlobalDecl gd,
...
static bool shouldAssumeDSOLocal(const CIRGenModule &cgm,
  ...
  if (cgOpts.DirectAccessExternalData) {
    ...
    if (isa<cir::FuncOp>(gv) && !cgOpts.NoPLT && rm == toolchain::Reloc::Static)
      return true;
  }
...
cir::FuncOp CIRGenModule::getOrCreateCIRFunction(
  ...
  if (entry) {
    ...
    // TODO(cir): classic codegen checks here if this is a toolchain::GlobalAlias.
    // How will we support this?
  }

  auto *funcDecl = toolchain::cast_or_null<FunctionDecl>(gd.getDecl());
...
CIRGenModule::getMLIRVisibilityFromCIRLinkage(cir::GlobalLinkageKind glk) {
  ...
  default: {
    toolchain::errs() << "visibility not implemented for '"
                 << stringifyGlobalLinkageKind(glk) << "'\n";
    assert(0 && "not implemented");
  }
  }
  toolchain_unreachable("linkage should be handled above!");
}
...
cir::VisibilityKind CIRGenModule::getGlobalVisibilityKindFromClangVisibility(
  ...
  switch (visibility) {
  case language::Core::VisibilityAttr::VisibilityType::Default:
    return cir::VisibilityKind::Default;
  case language::Core::VisibilityAttr::VisibilityType::Hidden:
    return cir::VisibilityKind::Hidden;
  case language::Core::VisibilityAttr::VisibilityType::Protected:
    return cir::VisibilityKind::Protected;
  }
  toolchain_unreachable("unexpected visibility value");
}

cir::VisibilityAttr
CIRGenModule::getGlobalVisibilityAttrFromDecl(const Decl *decl) {
  const language::Core::VisibilityAttr *va = decl->getAttr<language::Core::VisibilityAttr>();
...
CharUnits CIRGenModule::computeNonVirtualBaseClassOffset(
  ...
  for (const CXXBaseSpecifier *base : path) {
    ...
    const auto *baseDecl =
        cast<CXXRecordDecl>(
            base->getType()->castAs<language::Core::RecordType>()->getOriginalDecl())
            ->getDefinitionOrSelf();
