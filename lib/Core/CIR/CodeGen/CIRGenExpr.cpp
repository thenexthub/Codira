...
Address CIRGenFunction::emitPointerWithAlignment(const Expr *expr,
  ...
  if (auto const *ce = dyn_cast<CastExpr>(expr)) {
    ...
    case CK_ZeroToOCLOpaqueType:
      toolchain_unreachable("unexpected cast for emitPointerWithAlignment");
    }
  }
...
void CIRGenFunction::emitStoreOfScalar(mlir::Value value, Address addr,
  ...
  if (const auto *clangVecTy = ty->getAs<language::Core::VectorType>()) {
...
LValue CIRGenFunction::emitLValueForField(LValue base, const FieldDecl *field) {
  ...
  toolchain::StringRef fieldName = field->getName();
  unsigned fieldIndex;
...
LValue CIRGenFunction::emitLValueForFieldInitialization(
    LValue base, const language::Core::FieldDecl *field, toolchain::StringRef fieldName) {
  QualType fieldType = field->getType();
...
LValue CIRGenFunction::emitUnaryOpLValue(const UnaryOperator *e) {
  ...
  case UO_Imag: {
    ...
    QualType elemTy = exprTy->castAs<language::Core::ComplexType>()->getElementType();
  ...
  case UO_Extension:
    toolchain_unreachable("UnaryOperator extension should be handled above!");
  case UO_Plus:
  case UO_Minus:
  case UO_Not:
  case UO_LNot:
  ...
  case UO_PostDec:
  case UO_Coawait:
    toolchain_unreachable("UnaryOperator of non-lvalue kind!");
  }
  toolchain_unreachable("Unknown unary operator kind!");
}
...
LValue
CIRGenFunction::emitArraySubscriptExpr(const language::Core::ArraySubscriptExpr *e) {
...
LValue CIRGenFunction::emitCastLValue(const CastExpr *e) {
  ...
  case CK_HLSLElementwiseCast:
  case CK_HLSLAggregateSplatCast:
    toolchain_unreachable("unexpected cast lvalue");
  ...
  case CK_DerivedToBase: {
    const auto *derivedClassTy =
        e->getSubExpr()->getType()->castAs<language::Core::RecordType>();
  ...
  toolchain_unreachable("Invalid cast kind");
}
...
static void pushTemporaryCleanup(CIRGenFunction &cgf,
  ...
  if (const language::Core::RecordType *rt = e->getType()
                                        ->getBaseElementTypeUnsafe()
                                        ->getAs<language::Core::RecordType>()) {
...
LValue CIRGenFunction::emitBinaryOperatorLValue(const BinaryOperator *e) {
  ...
  case cir::TEK_Scalar: {
    ...
    if (e->getLHS()->getType().getObjCLifetime() !=
        language::Core::Qualifiers::ObjCLifetime::OCL_None) {
...
RValue CIRGenFunction::emitCall(language::Core::QualType calleeTy,
...
CIRGenCallee CIRGenFunction::emitCallee(const language::Core::Expr *e) {
  e = e->IgnoreParens();
  ...
  if (const auto *ptrType = e->getType()->getAs<language::Core::PointerType>()) {
...
RValue CIRGenFunction::emitCallExpr(const language::Core::CallExpr *e,
                                    ReturnValueSlot returnValue) {
...
RValue CIRGenFunction::convertTempToRValue(Address addr, language::Core::QualType type,
...
cir::IfOp CIRGenFunction::emitIfOnBoolExpr(
    const language::Core::Expr *cond, BuilderCallbackRef thenBuilder,
...
void CIRGenFunction::emitCXXConstructExpr(const CXXConstructExpr *e,
  ...
  if (const ArrayType *arrayType = getContext().getAsArrayType(e->getType())) {
    ...
    language::Core::CXXCtorType type = Ctor_Complete;
...
void CIRGenFunction::emitUnreachable(language::Core::SourceLocation loc,
                                     bool createNewBlock) {
  assert(!cir::MissingFeatures::sanitizers());
