...
cir::ConstantOp CIRGenBuilderTy::getConstInt(mlir::Location loc,
                                             toolchain::APInt intVal) {
  return getConstInt(loc, toolchain::APSInt(intVal));
}
...
cir::ConstantOp
language::Core::CIRGen::CIRGenBuilderTy::getConstFP(mlir::Location loc, mlir::Type t,
...
void CIRGenBuilderTy::computeGlobalViewIndicesFromFlatOffset(
  ...
  mlir::Type subType =
      toolchain::TypeSwitch<mlir::Type, mlir::Type>(ty)
          ...
          .Case<cir::RecordType>([&](auto recordTy) {
            ...
            toolchain_unreachable("offset was not found within the record");
          })
