...
public:
  ...
  mlir::Attribute VisitCastExpr(CastExpr *e, QualType destType) {
    ...
    case CK_Dependent:
      toolchain_unreachable("saw dependent cast!");
...
struct ConstantLValue {
  toolchain::PointerUnion<mlir::Value, mlir::Attribute> value;
...
private:
  ...
  mlir::ArrayAttr getOffset(mlir::Type ty) {
    ...
    toolchain::SmallVector<mlir::Attribute, 3> indices;
  ...
  ConstantLValue applyOffset(ConstantLValue &c) {
    ...
    if (auto attr = mlir::dyn_cast<mlir::Attribute>(c.value)) {
      ...
      toolchain_unreachable("Unsupported attribute type to offset");
    }
...
mlir::Attribute ConstantLValueEmitter::tryEmit() {
  ...
  toolchain::PointerUnion<mlir::Value, mlir::Attribute> &value = result.value;
  if (!value)
    return {};
...
mlir::Attribute ConstantEmitter::tryEmitPrivate(const APValue &value,
  ...
  case APValue::Array: {
    ...
    mlir::TypedAttr typedFiller = toolchain::cast_or_null<mlir::TypedAttr>(filler);
  ...
  case APValue::ComplexFloat: {
    ...
    if (isa<cir::IntType>(complexElemTy)) {
      toolchain::APSInt real = value.getComplexIntReal();
      toolchain::APSInt imag = value.getComplexIntImag();
    ...
    assert(isa<cir::FPTypeInterface>(complexElemTy) &&
           "expected floating-point type");
    toolchain::APFloat real = value.getComplexFloatReal();
    toolchain::APFloat imag = value.getComplexFloatImag();
    return builder.getAttr<cir::ConstComplexAttr>(
