...
static void emitLValueForAnyFieldInitialization(CIRGenFunction &cgf,
  ...
  if (memberInit->isIndirectMemberInitializer()) {
    ...
    for (const auto *nd : indirectField->chain()) {
      auto *fd = cast<language::Core::FieldDecl>(nd);
...
void CIRGenFunction::emitCXXAggrConstructorCall(
    const CXXConstructorDecl *ctor, const language::Core::ArrayType *arrayType,
...
void CIRGenFunction::emitCXXConstructorCall(const language::Core::CXXConstructorDecl *d,
                                            language::Core::CXXCtorType type,
                                            bool forVirtualBase,
                                            bool delegating,
