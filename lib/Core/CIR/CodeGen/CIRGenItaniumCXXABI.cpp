...
protected:
  ...
  toolchain::DenseMap<const CXXRecordDecl *, cir::GlobalOp> vtables;
...
public:
  ...
  bool needsVTTParameter(language::Core::GlobalDecl gd) override;

  void emitInstanceFunctionProlog(SourceLocation loc,
                                  CIRGenFunction &cgf) override;

  void emitCXXConstructors(const language::Core::CXXConstructorDecl *d) override;
  void emitCXXDestructors(const language::Core::CXXDestructorDecl *d) override;
  void emitCXXStructor(language::Core::GlobalDecl gd) override;
  ...
  mlir::Value getVTableAddressPointInStructor(
      CIRGenFunction &cgf, const language::Core::CXXRecordDecl *vtableClass,
      language::Core::BaseSubobject base,
      const language::Core::CXXRecordDecl *nearestVBase) override;
...
CIRGenCXXABI *language::Core::CIRGen::CreateCIRGenItaniumCXXABI(CIRGenModule &cgm) {
  ...
  default:
    toolchain_unreachable("bad or NYI ABI kind");
  }
}
...
mlir::Value CIRGenItaniumCXXABI::getVTableAddressPointInStructor(
    CIRGenFunction &cgf, const language::Core::CXXRecordDecl *vtableClass,
    language::Core::BaseSubobject base, const language::Core::CXXRecordDecl *nearestVBase) {
