...
#include "language/Core/CIR/TypeEvaluationKind.h"
...
public:
  ...
  language::Core::GlobalDecl curGD;
  ...
  language::Core::CharUnits cxxThisAlignment;
  ...
  const language::Core::Decl *curFuncDecl = nullptr;

  /// The function for which code is currently being generated.
  cir::FuncOp curFn;

  using DeclMapTy = toolchain::DenseMap<const language::Core::Decl *, Address>;
  /// This keeps track of the CIR allocas or globals for local C
  /// declarations.
  DeclMapTy localDeclMap;

  /// The type of the condition for the emitting switch statement.
  toolchain::SmallVector<mlir::Type, 2> condTypeStack;

  language::Core::ASTContext &getContext() const { return cgm.getASTContext(); }
  ...
  language::Core::SanitizerSet sanOpts;
  ...
  /// dropped.
  using SymTableTy = toolchain::ScopedHashTable<const language::Core::Decl *, mlir::Value>;
  ...
  mlir::Type convertType(language::Core::QualType t);
  mlir::Type convertType(const TypeDecl *t) {
    return convertType(getContext().getTypeDeclType(t));
  }

  ///  Return the cir::TypeEvaluationKind of QualType \c type.
  static cir::TypeEvaluationKind getEvaluationKind(language::Core::QualType type);

  static bool hasScalarEvaluationKind(language::Core::QualType type) {
    return getEvaluationKind(type) == cir::TEK_Scalar;
  }

  static bool hasAggregateEvaluationKind(language::Core::QualType type) {
...
private:
  ...
  /// wasn't declared yet.
  void declare(mlir::Value addrVal, const language::Core::Decl *var, language::Core::QualType ty,
               mlir::Location loc, language::Core::CharUnits alignment,
               bool isParam = false);

public:
  mlir::Value createDummyValue(mlir::Location loc, language::Core::QualType qt);

  void emitNullInitialization(mlir::Location loc, Address destPtr, QualType ty);

private:
  // Track current variable initialization (if there's one)
  const language::Core::VarDecl *currVarDecl = nullptr;
  class VarDeclContext {
    CIRGenFunction &p;
    const language::Core::VarDecl *oldVal = nullptr;
...
public:
  ...
  using SymTableScopeTy =
      toolchain::ScopedHashTableScope<const language::Core::Decl *, mlir::Value>;
  ...
  mlir::Location getLoc(language::Core::SourceLocation srcLoc);
  mlir::Location getLoc(language::Core::SourceRange srcLoc);
  mlir::Location getLoc(mlir::Location lhs, mlir::Location rhs);

  const language::Core::LangOptions &getLangOpts() const { return cgm.getLangOpts(); }
  ...
  struct PrototypeWrapper {
    toolchain::PointerUnion<const language::Core::FunctionProtoType *,
                       const language::Core::ObjCMethodDecl *>
        p;

    PrototypeWrapper(const language::Core::FunctionProtoType *ft) : p(ft) {}
    PrototypeWrapper(const language::Core::ObjCMethodDecl *md) : p(md) {}
  };
  ...
  class AbstractCallee {
    ...
    [[maybe_unused]] const language::Core::Decl *calleeDecl;

  public:
    AbstractCallee() : calleeDecl(nullptr) {}
    AbstractCallee(const language::Core::FunctionDecl *fd) : calleeDecl(fd) {}

    bool hasFunctionDecl() const {
      return toolchain::isa_and_nonnull<language::Core::FunctionDecl>(calleeDecl);
    }

    unsigned getNumParams() const {
      if (const auto *fd = toolchain::dyn_cast<language::Core::FunctionDecl>(calleeDecl))
        return fd->getNumParams();
      return toolchain::cast<language::Core::ObjCMethodDecl>(calleeDecl)->param_size();
    }

    const language::Core::ParmVarDecl *getParamDecl(unsigned I) const {
      if (const auto *fd = toolchain::dyn_cast<language::Core::FunctionDecl>(calleeDecl))
        return fd->getParamDecl(I);
      return *(toolchain::cast<language::Core::ObjCMethodDecl>(calleeDecl)->param_begin() +
               I);
    }
  };
  ...
  bool constantFoldsToBool(const language::Core::Expr *cond, bool &resultBool,
                           bool allowLabels = false);
  bool constantFoldsToSimpleInteger(const language::Core::Expr *cond,
  ...
  bool containsLabel(const language::Core::Stmt *s, bool ignoreCaseStmts = false);

  class ConstantEmission {
    // Cannot use mlir::TypedAttr directly here because of bit availability.
    toolchain::PointerIntPair<mlir::Attribute, 1, bool> valueAndIsReference;
  ...
  struct AutoVarEmission {
    const language::Core::VarDecl *Variable;
    ...
    AutoVarEmission(const language::Core::VarDecl &variable)
  ...
  mlir::Value evaluateExprAsBool(const language::Core::Expr *e);
  ...
  void setAddrOfLocalVar(const language::Core::VarDecl *vd, Address addr) {
  ...
  RValue convertTempToRValue(Address addr, language::Core::QualType type,
                             language::Core::SourceLocation loc);

  static bool
  isConstructorDelegationValid(const language::Core::CXXConstructorDecl *ctor);

  struct VPtr {
    language::Core::BaseSubobject base;
    const language::Core::CXXRecordDecl *nearestVBase;
    language::Core::CharUnits offsetFromNearestVBase;
    const language::Core::CXXRecordDecl *vtableClass;
  };

  using VPtrsVector = toolchain::SmallVector<VPtr, 4>;
  VPtrsVector getVTablePointers(const language::Core::CXXRecordDecl *vtableClass);
  void getVTablePointers(language::Core::BaseSubobject base,
                         const language::Core::CXXRecordDecl *nearestVBase,
                         language::Core::CharUnits offsetFromNearestVBase,
  ...
  LValue makeNaturalAlignPointeeAddrLValue(mlir::Value v, language::Core::QualType t);
  ...
  Address getAddressOfBaseClass(
      ...
      toolchain::iterator_range<CastExpr::path_const_iterator> path,
  ...
  void initializeVTablePointers(mlir::Location loc,
                                const language::Core::CXXRecordDecl *rd);
  void initializeVTablePointer(mlir::Location loc, const VPtr &vptr);

  /// Return the address of a local variable.
  Address getAddrOfLocalVar(const language::Core::VarDecl *vd) {
  ...
  Address getAddrOfBitFieldStorage(LValue base, const language::Core::FieldDecl *field,
  ...
  /// addressed later.
  RValue getUndefRValue(language::Core::QualType ty);

  cir::FuncOp generateCode(language::Core::GlobalDecl gd, cir::FuncOp fn,
                           cir::FuncType funcType);

  language::Core::QualType buildFunctionArgList(language::Core::GlobalDecl gd,
                                       FunctionArgList &args);

  /// Emit code for the start of a function.
  /// \param loc       The location to be associated with the function.
  /// \param startLoc  The location of the function body.
  void startFunction(language::Core::GlobalDecl gd, language::Core::QualType returnType,
                     cir::FuncOp fn, cir::FuncType funcType,
                     FunctionArgList args, language::Core::SourceLocation loc,
                     language::Core::SourceLocation startLoc);
...
public:
  ...
  Destroyer *getDestroyer(language::Core::QualType::DestructionKind kind);
...
private:
  void emitAndUpdateRetAlloca(language::Core::QualType type, mlir::Location loc,
...
public:
  ...
  mlir::Value emitAlloca(toolchain::StringRef name, mlir::Type ty,
                         mlir::Location loc, language::Core::CharUnits alignment,
  ...
  void emitAggExpr(const language::Core::Expr *e, AggValueSlot slot);
  ...
  /// should be returned.
  RValue emitAnyExpr(const language::Core::Expr *e,
  ...
  RValue emitAnyExprToTemp(const language::Core::Expr *e);

  void emitArrayDestroy(mlir::Value begin, mlir::Value end,
                        QualType elementType, CharUnits elementAlign,
                        Destroyer *destroyer);

  mlir::Value emitArrayLength(const language::Core::ArrayType *arrayType,
                              QualType &baseType, Address &addr);
  LValue emitArraySubscriptExpr(const language::Core::ArraySubscriptExpr *e);
  ...
  AutoVarEmission emitAutoVarAlloca(const language::Core::VarDecl &d,
                                    mlir::OpBuilder::InsertPoint ip = {});

  /// Emit code and set up symbol table for a variable declaration with auto,
  /// register, or no storage class specifier. These turn into simple stack
  /// objects, globals depending on target.
  void emitAutoVarDecl(const language::Core::VarDecl &d);

  void emitAutoVarCleanups(const AutoVarEmission &emission);
  void emitAutoVarInit(const AutoVarEmission &emission);
  void emitAutoVarTypeCleanup(const AutoVarEmission &emission,
                              language::Core::QualType::DestructionKind dtorKind);
  ...
  mlir::LogicalResult emitBreakStmt(const language::Core::BreakStmt &s);

  RValue emitBuiltinExpr(const language::Core::GlobalDecl &gd, unsigned builtinID,
                         const language::Core::CallExpr *e, ReturnValueSlot returnValue);
  ...
  RValue emitCall(language::Core::QualType calleeTy, const CIRGenCallee &callee,
                  const language::Core::CallExpr *e, ReturnValueSlot returnValue);
  void emitCallArg(CallArgList &args, const language::Core::Expr *e,
                   language::Core::QualType argType);
  void emitCallArgs(
      CallArgList &args, PrototypeWrapper prototype,
      toolchain::iterator_range<language::Core::CallExpr::const_arg_iterator> argRange,
      AbstractCallee callee = AbstractCallee(), unsigned paramsToSkip = 0);
  RValue emitCallExpr(const language::Core::CallExpr *e,
                      ReturnValueSlot returnValue = ReturnValueSlot());
  LValue emitCallExprLValue(const language::Core::CallExpr *e);
  CIRGenCallee emitCallee(const language::Core::Expr *e);
  ...
  mlir::LogicalResult emitCaseStmt(const language::Core::CaseStmt &s,
                                   mlir::Type condType,
                                   bool buildingTopLevelCase);
  ...
  LValue emitCompoundAssignmentLValue(const language::Core::CompoundAssignOperator *e);
  ...
  mlir::LogicalResult emitContinueStmt(const language::Core::ContinueStmt &s);

  void emitCXXConstructExpr(const language::Core::CXXConstructExpr *e,
                            AggValueSlot dest);

  void emitCXXAggrConstructorCall(const CXXConstructorDecl *ctor,
                                  const language::Core::ArrayType *arrayType,
  ...
  void emitCXXAggrConstructorCall(const CXXConstructorDecl *ctor,
                                  ...
                                  bool newPointerIsChecked,
                                  bool zeroInitialize);
  void emitCXXConstructorCall(const language::Core::CXXConstructorDecl *d,
                              language::Core::CXXCtorType type, bool forVirtualBase,
                              bool delegating, AggValueSlot thisAVS,
                              const language::Core::CXXConstructExpr *e);

  void emitCXXConstructorCall(const language::Core::CXXConstructorDecl *d,
                              language::Core::CXXCtorType type, bool forVirtualBase,
                              bool delegating, Address thisAddr,
                              CallArgList &args, language::Core::SourceLocation loc);
  ...
  mlir::LogicalResult emitCXXForRangeStmt(const CXXForRangeStmt &s,
                                          toolchain::ArrayRef<const Attr *> attrs);

  RValue emitCXXMemberCallExpr(const language::Core::CXXMemberCallExpr *e,
                               ReturnValueSlot returnValue);

  RValue emitCXXMemberOrOperatorCall(
      const language::Core::CXXMethodDecl *md, const CIRGenCallee &callee,
      ReturnValueSlot returnValue, mlir::Value thisPtr,
      mlir::Value implicitParam, language::Core::QualType implicitParamTy,
      const language::Core::CallExpr *ce, CallArgList *rtlArgs);

  RValue emitCXXMemberOrOperatorMemberCallExpr(
      const language::Core::CallExpr *ce, const language::Core::CXXMethodDecl *md,
      ReturnValueSlot returnValue, bool hasQualifier,
      language::Core::NestedNameSpecifier qualifier, bool isArrow,
      const language::Core::Expr *base);
  ...
  void emitCtorPrologue(const language::Core::CXXConstructorDecl *ctor,
                        language::Core::CXXCtorType ctorType, FunctionArgList &args);
  ...
  mlir::LogicalResult emitDoStmt(const language::Core::DoStmt &s);
  ...
  void emitExprAsInit(const language::Core::Expr *init, const language::Core::ValueDecl *d,
                      LValue lvalue, bool capturedByInit = false);

  mlir::LogicalResult emitFunctionBody(const language::Core::Stmt *body);

  void emitImplicitAssignmentOperatorBody(FunctionArgList &args);

  void emitInitializerForField(language::Core::FieldDecl *field, LValue lhs,
                               language::Core::Expr *init);
  ...
  mlir::Value emitScalarExpr(const language::Core::Expr *e);
  ...
  mlir::LogicalResult emitStmt(const language::Core::Stmt *s, bool useCurrentScope,
                               toolchain::ArrayRef<const Attr *> attrs = {});

  mlir::LogicalResult emitSimpleStmt(const language::Core::Stmt *s,
                                     bool useCurrentScope);

  mlir::LogicalResult emitForStmt(const language::Core::ForStmt &s);
  ...
  void emitCompoundStmt(const language::Core::CompoundStmt &s);

  void emitCompoundStmtWithoutScope(const language::Core::CompoundStmt &s);

  void emitDecl(const language::Core::Decl &d, bool evaluateConditionDecl = false);
  mlir::LogicalResult emitDeclStmt(const language::Core::DeclStmt &s);
  LValue emitDeclRefLValue(const language::Core::DeclRefExpr *e);

  mlir::LogicalResult emitDefaultStmt(const language::Core::DefaultStmt &s,
                                      mlir::Type condType,
                                      bool buildingTopLevelCase);

  void emitDelegateCXXConstructorCall(const language::Core::CXXConstructorDecl *ctor,
                                      language::Core::CXXCtorType ctorType,
                                      const FunctionArgList &args,
                                      language::Core::SourceLocation loc);
  ...
  /// given parameter.
  void emitDelegateCallArg(CallArgList &args, const language::Core::VarDecl *param,
                           language::Core::SourceLocation loc);
  ...
  mlir::LogicalResult emitIfOnBoolExpr(const language::Core::Expr *cond,
                                       const language::Core::Stmt *thenS,
                                       const language::Core::Stmt *elseS);
  cir::IfOp emitIfOnBoolExpr(const language::Core::Expr *cond,
                             BuilderCallbackRef thenBuilder,
                             mlir::Location thenLoc,
                             BuilderCallbackRef elseBuilder,
                             std::optional<mlir::Location> elseLoc = {});

  mlir::Value emitOpOnBoolExpr(mlir::Location loc, const language::Core::Expr *cond);

  mlir::LogicalResult emitLabel(const language::Core::LabelDecl &d);
  mlir::LogicalResult emitLabelStmt(const language::Core::LabelStmt &s);

  mlir::LogicalResult emitIfStmt(const language::Core::IfStmt &s);

  /// Emit code to compute the specified expression,
  /// ignoring the result.
  void emitIgnoredExpr(const language::Core::Expr *e);
  ...
  LValue emitLValue(const language::Core::Expr *e);
  LValue emitLValueForBitField(LValue base, const FieldDecl *field);
  LValue emitLValueForField(LValue base, const language::Core::FieldDecl *field);
  ...
  /// explicit source.
  Address emitPointerWithAlignment(const language::Core::Expr *expr,
                                   LValueBaseInfo *baseInfo = nullptr);

  /// Emits a reference binding to the passed in expression.
  RValue emitReferenceBindingToExpr(const Expr *e);

  mlir::LogicalResult emitReturnStmt(const language::Core::ReturnStmt &s);
  ...
  mlir::Value emitScalarConversion(mlir::Value src, language::Core::QualType srcType,
                                   language::Core::QualType dstType,
                                   language::Core::SourceLocation loc);

  void emitScalarInit(const language::Core::Expr *init, mlir::Location loc,
                      LValue lvalue, bool capturedByInit = false);
  ...
  void emitStoreOfScalar(mlir::Value value, Address addr, bool isVolatile,
                         language::Core::QualType ty, bool isInit = false,
  ...
  mlir::LogicalResult emitSwitchBody(const language::Core::Stmt *s);
  mlir::LogicalResult emitSwitchCase(const language::Core::SwitchCase &s,
                                     bool buildingTopLevelCase);
  mlir::LogicalResult emitSwitchStmt(const language::Core::SwitchStmt &s);

  /// Given a value and its clang type, returns the value casted to its memory
  /// representation.
  /// Note: CIR defers most of the special casting to the final lowering passes
  /// to conserve the high level information.
  mlir::Value emitToMemory(mlir::Value value, language::Core::QualType ty);
  ...
  LValue emitUnaryOpLValue(const language::Core::UnaryOperator *e);
  ...
  void emitUnreachable(language::Core::SourceLocation loc, bool createNewBlock);

  /// This method handles emission of any variable declaration
  /// inside a function, including static vars etc.
  void emitVarDecl(const language::Core::VarDecl &d);

  void emitVariablyModifiedType(QualType ty);

  mlir::LogicalResult emitWhileStmt(const language::Core::WhileStmt &s);
  ...
  mlir::Value emitTernaryOnBoolExpr(const language::Core::Expr *cond, mlir::Location loc,
                                    const language::Core::Stmt *thenS,
                                    const language::Core::Stmt *elseS);
