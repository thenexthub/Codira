...
struct BinOpInfo {
  ...
  bool isFixedPointOp() const {
    ...
    // return an int.
    if (const auto *binOp = toolchain::dyn_cast<BinaryOperator>(e)) {
      QualType lhstype = binOp->getLHS()->getType();
      QualType rhstype = binOp->getRHS()->getType();
      return lhstype->isFixedPointType() || rhstype->isFixedPointType();
    }
    if (const auto *unop = toolchain::dyn_cast<UnaryOperator>(e))
...
public:
  ...
  mlir::Value maybePromoteBoolResult(mlir::Value value,
    ...
    if (mlir::isa<cir::BoolType>(dstTy))
      return value;
    toolchain_unreachable("Can only promote integer or boolean types");
  }
  ...
  mlir::Value VisitStmt(Stmt *s) {
    toolchain_unreachable("Statement passed to ScalarExprEmitter");
  }
  ...
  mlir::Value emitConversionToBool(mlir::Value src, QualType srcType,
    ...
    if (toolchain::isa<MemberPointerType>(srcType)) {
  ...
  mlir::Value emitScalarCast(mlir::Value src, QualType srcType,
    ...
    if (mlir::isa<cir::BoolType>(srcTy)) {
      ...
      if (cgf.getBuilder().isInt(dstTy))
        ...
        toolchain_unreachable("Internal error: Cast to unexpected type");
      ...
      toolchain_unreachable("Internal error: Cast from unexpected type");
    }
  ...
  mlir::Value emitIncDecConsiderOverflowBehavior(const UnaryOperator *e,
    ...
    toolchain_unreachable("Unexpected signed overflow behavior kind");
  }

  mlir::Value VisitUnaryAddrOf(const UnaryOperator *e) {
    if (toolchain::isa<MemberPointerType>(e->getType())) {
  ...
  emitScalarConversion(mlir::Value src, QualType srcType, QualType dstType,
    ...
    if (srcType->isHalfType() &&
      ...
      if (mlir::isa<cir::FPTypeInterface>(mlirDstType)) {
        if (cgf.getContext().getTargetInfo().useFP16ConversionIntrinsics())
          cgf.getCIRGenModule().errorNYI(loc,
                                         "cast via toolchain.convert.from.fp16");
      } else {
    ...
    if (mlirDstType != resTy) {
      if (cgf.getContext().getTargetInfo().useFP16ConversionIntrinsics()) {
        cgf.getCIRGenModule().errorNYI(loc, "cast via toolchain.convert.to.fp16");
      }
...
#undef HANDLEBINOP
  ...
  mlir::Value emitCmp(const BinaryOperator *e) {
    ...
    auto clangCmpToCIRCmp =
        [](language::Core::BinaryOperatorKind clangCmp) -> cir::CmpOpKind {
      switch (clangCmp) {
      case BO_LT:
      ...
      default:
        toolchain_unreachable("unsupported comparison kind for cir.cmp");
      }
    };
...
#undef VISITCOMP
  ...
  mlir::Value VisitBinLAnd(const language::Core::BinaryOperator *e) {
    if (e->getType()->isVectorType()) {
      assert(!cir::MissingFeatures::vectorType());
      return {};
    }
  ...
  mlir::Value VisitBinLOr(const language::Core::BinaryOperator *e) {
    if (e->getType()->isVectorType()) {
      assert(!cir::MissingFeatures::vectorType());
      return {};
    }
...
mlir::Value ScalarExprEmitter::emitComplexToScalarConversion(mlir::Location lov,
  ...
  case CK_IntegralComplexToBoolean:
    ...
    break;
  default:
    toolchain_unreachable("invalid complex-to-scalar cast kind");
  }
...
mlir::Value ScalarExprEmitter::VisitCastExpr(CastExpr *ce) {
  ...
  switch (kind) {
  case language::Core::CK_Dependent:
    toolchain_unreachable("dependent cast kind in CIR gen!");
  case language::Core::CK_BuiltinFnToFnPtr:
  ...
  case CK_BitCast: {
    ...
    // TODO: since the toolchain.experimental.vector.{insert,extract} intrinsics
    ...
    if (cgf.getContext().getAsVariableArrayType(typeToSize)) {
      ...
      return builder.getConstant(
          loc, cir::IntAttr::get(cgf.cgm.UInt64Ty,
                                 toolchain::APSInt(toolchain::APInt(64, 1), true)));
    }
