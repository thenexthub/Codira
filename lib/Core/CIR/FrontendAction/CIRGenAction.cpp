...
#include "language/Core/CIR/CIRToCIRPasses.h"
#include "language/Core/CIR/LowerToLLVM.h"
...
getBackendActionFromOutputType(CIRGenAction::OutputType Action) {
  ...
  toolchain_unreachable("Unsupported output type!");
}

static std::unique_ptr<toolchain::Module>
lowerFromCIRToLLVMIR(mlir::ModuleOp MLIRModule, toolchain::LLVMContext &LLVMCtx) {
  return direct::lowerDirectlyFromCIRToLLVMIR(MLIRModule, LLVMCtx);
}

class CIRGenConsumer : public language::Core::ASTConsumer {
...
public:
  ...
  void HandleCXXStaticMemberVarInstantiation(language::Core::VarDecl *VD) override {
...
getOutputStream(CompilerInstance &CI, StringRef InFile,
  ...
  toolchain_unreachable("Invalid CIRGenAction::OutputType");
}

std::unique_ptr<ASTConsumer>
CIRGenAction::CreateASTConsumer(CompilerInstance &CI, StringRef InFile) {
  std::unique_ptr<toolchain::raw_pwrite_stream> Out = CI.takeOutputStream();
