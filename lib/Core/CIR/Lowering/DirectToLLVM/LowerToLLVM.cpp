...
#include "language/Core/CIR/Passes.h"
...
mlir::LLVM::Linkage convertLinkage(cir::GlobalLinkageKind linkage) {
  ...
  toolchain_unreachable("Unknown CIR linkage type");
}

static mlir::Value getLLVMIntCast(mlir::ConversionPatternRewriter &rewriter,
                                  mlir::Value toolchainSrc, mlir::Type toolchainDstIntTy,
                                  bool isUnsigned, uint64_t cirSrcWidth,
                                  uint64_t cirDstIntWidth) {
  if (cirSrcWidth == cirDstIntWidth)
    return toolchainSrc;

  auto loc = toolchainSrc.getLoc();
  if (cirSrcWidth < cirDstIntWidth) {
    if (isUnsigned)
      return rewriter.create<mlir::LLVM::ZExtOp>(loc, toolchainDstIntTy, toolchainSrc);
    return rewriter.create<mlir::LLVM::SExtOp>(loc, toolchainDstIntTy, toolchainSrc);
  }

  // Otherwise truncate
  return rewriter.create<mlir::LLVM::TruncOp>(loc, toolchainDstIntTy, toolchainSrc);
}
...
public:
  ...
  mlir::Value visit(mlir::Attribute attr) {
    return toolchain::TypeSwitch<mlir::Attribute, mlir::Value>(attr)
...
mlir::Value lowerCirAttrAsValue(mlir::Operation *parentOp,
  ...
  if (!value)
    toolchain_unreachable("unhandled attribute type");
  return value;
}
...
mlir::Value CIRAttrToValue::visitCirAttr(cir::ConstArrayAttr attr) {
  mlir::Type toolchainTy = converter->convertType(attr.getType());
  mlir::Location loc = parentOp->getLoc();
  mlir::Value result;
  ...
  if (attr.hasTrailingZeros()) {
    ...
    result = rewriter.create<mlir::LLVM::UndefOp>(loc, toolchainTy);
  }

  // Iteratively lower each constant element of the array.
  if (auto arrayAttr = mlir::dyn_cast<mlir::ArrayAttr>(attr.getElts())) {
    for (auto [idx, elt] : toolchain::enumerate(arrayAttr)) {
    ...
    for (auto [idx, elt] : toolchain::enumerate(strAttr)) {
    ...
    toolchain_unreachable("unexpected ConstArrayAttr elements");
  }
...
mlir::Value CIRAttrToValue::visitCirAttr(cir::ConstVectorAttr attr) {
  const mlir::Type toolchainTy = converter->convertType(attr.getType());
  ...
  return rewriter.create<mlir::LLVM::ConstantOp>(
      loc, toolchainTy,
      mlir::DenseElementsAttr::get(mlir::cast<mlir::ShapedType>(toolchainTy),
                                   mlirValues));
}
...
mlir::Value CIRAttrToValue::visitCirAttr(cir::GlobalViewAttr globalAttr) {
  ...
  toolchain::StringRef symName;
  mlir::Operation *sourceSymbol =
      mlir::SymbolTable::lookupSymbolIn(moduleOp, globalAttr.getSymbol());
  if (auto toolchainSymbol = dyn_cast<mlir::LLVM::GlobalOp>(sourceSymbol)) {
    sourceType = toolchainSymbol.getType();
    symName = toolchainSymbol.getSymName();
  ...
  } else if (auto toolchainFun = dyn_cast<mlir::LLVM::LLVMFuncOp>(sourceSymbol)) {
    sourceType = toolchainFun.getFunctionType();
    symName = toolchainFun.getSymName();
    ...
    toolchain_unreachable("Unexpected GlobalOp type");
  }
  ...
  if (globalAttr.getIndices()) {
    toolchain::SmallVector<mlir::LLVM::GEPArg> indices;
  ...
  if (auto ptrTy = mlir::dyn_cast<cir::PointerType>(globalAttr.getType())) {
    mlir::Type toolchainEltTy =
        convertTypeForMemory(*converter, dataLayout, ptrTy.getPointee());

    if (toolchainEltTy == sourceType)
      return addrOp;

    mlir::Type toolchainDstTy = converter->convertType(globalAttr.getType());
    return rewriter.create<mlir::LLVM::BitcastOp>(parentOp->getLoc(), toolchainDstTy,
                                                  addrOp);
  }

  toolchain_unreachable("Expecting pointer or integer type for GlobalViewAttr");
}
...
public:
  GlobalInitAttrRewriter(mlir::Type type,
                         mlir::ConversionPatternRewriter &rewriter)
      : toolchainType(type), rewriter(rewriter) {}

  mlir::Attribute visit(mlir::Attribute attr) {
    return toolchain::TypeSwitch<mlir::Attribute, mlir::Attribute>(attr)
  ...
  mlir::Attribute visitCirAttr(cir::IntAttr attr) {
    return rewriter.getIntegerAttr(toolchainType, attr.getValue());
  }

  mlir::Attribute visitCirAttr(cir::FPAttr attr) {
    return rewriter.getFloatAttr(toolchainType, attr.getValue());
  }
...
struct ConvertCIRToLLVMPass
  ...
  StringRef getArgument() const override { return "cir-flat-to-toolchain"; }
};
...
mlir::LogicalResult CIRToLLVMAssumeAlignedOpLowering::matchAndRewrite(
  ...
  // The toolchain.assume operation does not have a result, so we need to replace
...
mlir::LogicalResult CIRToLLVMBitClzOpLowering::matchAndRewrite(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  auto resTy = getTypeConverter()->convertType(op.getType());
  auto toolchainOp = rewriter.create<mlir::LLVM::CountLeadingZerosOp>(
      op.getLoc(), resTy, adaptor.getInput(), op.getPoisonZero());
  rewriter.replaceOp(op, toolchainOp);
  return mlir::LogicalResult::success();
}
...
mlir::LogicalResult CIRToLLVMBitCtzOpLowering::matchAndRewrite(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  auto resTy = getTypeConverter()->convertType(op.getType());
  auto toolchainOp = rewriter.create<mlir::LLVM::CountTrailingZerosOp>(
      op.getLoc(), resTy, adaptor.getInput(), op.getPoisonZero());
  rewriter.replaceOp(op, toolchainOp);
  return mlir::LogicalResult::success();
}
...
mlir::LogicalResult CIRToLLVMBitPopcountOpLowering::matchAndRewrite(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  auto resTy = getTypeConverter()->convertType(op.getType());
  auto toolchainOp = rewriter.create<mlir::LLVM::CtPopOp>(op.getLoc(), resTy,
                                                     adaptor.getInput());
  rewriter.replaceOp(op, toolchainOp);
  return mlir::LogicalResult::success();
}
...
mlir::LogicalResult CIRToLLVMCastOpLowering::matchAndRewrite(
  ...
  case cir::CastKind::array_to_ptrdecay: {
    ...
    mlir::Type elementTy = convertTypeForMemory(*getTypeConverter(), dataLayout,
                                                ptrTy.getPointee());
    toolchain::SmallVector<mlir::LLVM::GEPArg> offset{0};
    rewriter.replaceOpWithNewOp<mlir::LLVM::GEPOp>(
        castOp, targetType, elementTy, sourceValue, offset);
    break;
  }
  case cir::CastKind::int_to_bool: {
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Value zeroInt = rewriter.create<mlir::LLVM::ConstantOp>(
        castOp.getLoc(), toolchainSrcVal.getType(), 0);
    rewriter.replaceOpWithNewOp<mlir::LLVM::ICmpOp>(
        castOp, mlir::LLVM::ICmpPredicate::ne, toolchainSrcVal, zeroInt);
    break;
  }
  case cir::CastKind::integral: {
    ...
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Type toolchainDstType = getTypeConverter()->convertType(dstType);
    ...
    rewriter.replaceOp(castOp, getLLVMIntCast(rewriter, toolchainSrcVal, toolchainDstType,
  ...
  case cir::CastKind::floating: {
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Type toolchainDstTy = getTypeConverter()->convertType(castOp.getType());
    ...
    if (getFloatWidth(srcTy) > getFloatWidth(dstTy))
      rewriter.replaceOpWithNewOp<mlir::LLVM::FPTruncOp>(castOp, toolchainDstTy,
                                                         toolchainSrcVal);
    else
      rewriter.replaceOpWithNewOp<mlir::LLVM::FPExtOp>(castOp, toolchainDstTy,
                                                       toolchainSrcVal);
    return mlir::success();
  }
  case cir::CastKind::int_to_ptr: {
    ...
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Type toolchainDstTy = getTypeConverter()->convertType(dstTy);
    rewriter.replaceOpWithNewOp<mlir::LLVM::IntToPtrOp>(castOp, toolchainDstTy,
                                                        toolchainSrcVal);
    return mlir::success();
  }
  case cir::CastKind::ptr_to_int: {
    ...
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Type toolchainDstTy = getTypeConverter()->convertType(dstTy);
    rewriter.replaceOpWithNewOp<mlir::LLVM::PtrToIntOp>(castOp, toolchainDstTy,
                                                        toolchainSrcVal);
    return mlir::success();
  }
  case cir::CastKind::float_to_bool: {
    ...
    auto zeroFloat = rewriter.create<mlir::LLVM::ConstantOp>(
        castOp.getLoc(), toolchainSrcVal.getType(),
        mlir::FloatAttr::get(toolchainSrcVal.getType(), 0.0));

    // Extend comparison result to either bool (C++) or int (C).
    rewriter.replaceOpWithNewOp<mlir::LLVM::FCmpOp>(castOp, kind, toolchainSrcVal,
                                                    zeroFloat);
  ...
  case cir::CastKind::bool_to_int: {
    auto dstTy = mlir::cast<cir::IntType>(castOp.getType());
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    auto toolchainSrcTy = mlir::cast<mlir::IntegerType>(toolchainSrcVal.getType());
    auto toolchainDstTy =
        mlir::cast<mlir::IntegerType>(getTypeConverter()->convertType(dstTy));
    if (toolchainSrcTy.getWidth() == toolchainDstTy.getWidth())
      rewriter.replaceOpWithNewOp<mlir::LLVM::BitcastOp>(castOp, toolchainDstTy,
                                                         toolchainSrcVal);
    else
      rewriter.replaceOpWithNewOp<mlir::LLVM::ZExtOp>(castOp, toolchainDstTy,
                                                      toolchainSrcVal);
    return mlir::success();
  }
  case cir::CastKind::bool_to_float: {
    ...
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Type toolchainDstTy = getTypeConverter()->convertType(dstTy);
    rewriter.replaceOpWithNewOp<mlir::LLVM::UIToFPOp>(castOp, toolchainDstTy,
                                                      toolchainSrcVal);
    return mlir::success();
  }
  case cir::CastKind::int_to_float: {
    ...
    if (mlir::cast<cir::IntType>(elementTypeIfVector(castOp.getSrc().getType()))
            .isSigned())
      rewriter.replaceOpWithNewOp<mlir::LLVM::SIToFPOp>(castOp, toolchainDstTy,
                                                        toolchainSrcVal);
    else
      rewriter.replaceOpWithNewOp<mlir::LLVM::UIToFPOp>(castOp, toolchainDstTy,
                                                        toolchainSrcVal);
  ...
  case cir::CastKind::float_to_int: {
    ...
    if (mlir::cast<cir::IntType>(elementTypeIfVector(castOp.getType()))
            .isSigned())
      rewriter.replaceOpWithNewOp<mlir::LLVM::FPToSIOp>(castOp, toolchainDstTy,
                                                        toolchainSrcVal);
    else
      rewriter.replaceOpWithNewOp<mlir::LLVM::FPToUIOp>(castOp, toolchainDstTy,
                                                        toolchainSrcVal);
    return mlir::success();
  }
  ...
  case cir::CastKind::ptr_to_bool: {
    ...
    mlir::Value zeroPtr = rewriter.create<mlir::LLVM::ZeroOp>(
        castOp.getLoc(), toolchainSrcVal.getType());
    rewriter.replaceOpWithNewOp<mlir::LLVM::ICmpOp>(
        castOp, mlir::LLVM::ICmpPredicate::ne, toolchainSrcVal, zeroPtr);
    break;
  }
  case cir::CastKind::address_space: {
    ...
    mlir::Value toolchainSrcVal = adaptor.getSrc();
    mlir::Type toolchainDstTy = getTypeConverter()->convertType(dstTy);
    rewriter.replaceOpWithNewOp<mlir::LLVM::AddrSpaceCastOp>(castOp, toolchainDstTy,
                                                             toolchainSrcVal);
    break;
  }
...
mlir::LogicalResult CIRToLLVMPtrStrideOpLowering::matchAndRewrite(
  ...
  if (indexOp && layoutWidth && width != *layoutWidth) {
    ...
    // Handle the cast
    const auto toolchainDstType = mlir::IntegerType::get(ctx, *layoutWidth);
    index = getLLVMIntCast(rewriter, index, toolchainDstType,
...
mlir::LogicalResult CIRToLLVMBaseClassAddrOpLowering::matchAndRewrite(
  ...
  toolchain::SmallVector<mlir::LLVM::GEPArg, 1> offset = {
...
mlir::LogicalResult CIRToLLVMRotateOpLowering::matchAndRewrite(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  // Note that LLVM intrinsic calls to @toolchain.fsh{r,l}.i* have the same type as
  // the operand.
...
rewriteCallOrInvoke(mlir::Operation *op, mlir::ValueRange callOperands,
  ...
  toolchain::SmallVector<mlir::Type, 8> toolchainResults;
  mlir::ValueTypeRange<mlir::ResultRange> cirResults = op->getResultTypes();
  auto call = cast<cir::CIRCallOpInterface>(op);

  if (converter->convertTypes(cirResults, toolchainResults).failed())
    return mlir::failure();
  ...
  mlir::LLVM::LLVMFunctionType toolchainFnTy;
  ...
  if (calleeAttr) { // direct call
    ...
    if (auto fn = mlir::dyn_cast<mlir::FunctionOpInterface>(callee)) {
      toolchainFnTy = converter->convertType<mlir::LLVM::LLVMFunctionType>(
          fn.getFunctionType());
      assert(toolchainFnTy && "Failed to convert function type");
      ...
      toolchain::append_range(adjustedCallOperands, callOperands);
      callOperands = adjustedCallOperands;

      // Clear the callee attribute because we're calling an alias.
      calleeAttr = {};
      toolchainFnTy = mlir::cast<mlir::LLVM::LLVMFunctionType>(alias.getType());
    } else {
      // Was this an ifunc?
    ...
    toolchainFnTy = cast<mlir::LLVM::LLVMFunctionType>(
  ...
  auto newOp = rewriter.replaceOpWithNewOp<mlir::LLVM::CallOp>(
      op, toolchainFnTy, calleeAttr, callOperands);
  if (memoryEffects)
...
mlir::LogicalResult CIRToLLVMLoadOpLowering::matchAndRewrite(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  const mlir::Type toolchainTy =
      convertTypeForMemory(*getTypeConverter(), dataLayout, op.getType());
  assert(!cir::MissingFeatures::opLoadStoreMemOrder());
  std::optional<size_t> opAlign = op.getAlignment();
  unsigned alignment =
      (unsigned)opAlign.value_or(dataLayout.getTypeABIAlignment(toolchainTy));
  ...
  mlir::LLVM::LoadOp newLoad = rewriter.create<mlir::LLVM::LoadOp>(
      op->getLoc(), toolchainTy, adaptor.getAddr(), alignment,
...
mlir::LogicalResult CIRToLLVMConstantOpLowering::matchAndRewrite(
  ...
  if (mlir::isa<mlir::IntegerType>(op.getType())) {
    ...
    // Lower GlobalViewAttr to toolchain.mlir.addressof + toolchain.mlir.ptrtoint
    ...
    // Lower GlobalViewAttr to toolchain.mlir.addressof
...
mlir::LogicalResult CIRToLLVMExpectOpLowering::matchAndRewrite(
  ...
  std::optional<toolchain::APFloat> prob = op.getProb();
  if (prob)
...
/// Convert the `cir.func` attributes to `toolchain.func` attributes.
...
mlir::LogicalResult CIRToLLVMFuncOpLowering::matchAndRewriteAlias(
    cir::FuncOp op, toolchain::StringRef aliasee, mlir::Type ty, OpAdaptor adaptor,
    mlir::ConversionPatternRewriter &rewriter) const {
...
mlir::LogicalResult CIRToLLVMFuncOpLowering::matchAndRewrite(
  ...
  for (const auto &argType : toolchain::enumerate(fnType.getInputs())) {
  ...
  mlir::Type toolchainFnTy = mlir::LLVM::LLVMFunctionType::get(
      resultType ? resultType : mlir::LLVM::LLVMVoidType::get(getContext()),
      signatureConversion.getConvertedTypes(),
      /*isVarArg=*/fnType.isVarArg());

  // If this is an alias, it needs to be lowered to toolchain::AliasOp.
  if (std::optional<toolchain::StringRef> aliasee = op.getAliasee())
    return matchAndRewriteAlias(op, *aliasee, toolchainFnTy, adaptor, rewriter);
  ...
  mlir::LLVM::LLVMFuncOp fn = rewriter.create<mlir::LLVM::LLVMFuncOp>(
      loc, op.getName(), toolchainFnTy, linkage, isDsoLocal, cconv,
...
void CIRToLLVMGlobalOpLowering::setupRegionInitializedLLVMGlobalOp(
  ...
  mlir::LLVM::GlobalOp newGlobalOp =
      rewriter.replaceOpWithNewOp<mlir::LLVM::GlobalOp>(
          op, toolchainType, isConst, linkage, symbol, nullptr, alignment, addrSpace,
...
mlir::LogicalResult CIRToLLVMGlobalOpLowering::matchAndRewrite(
  ...
  if (init.has_value()) {
    if (mlir::isa<cir::FPAttr, cir::IntAttr, cir::BoolAttr>(init.value())) {
      GlobalInitAttrRewriter initRewriter(toolchainType, rewriter);
  ...
  // Rewrite op.
  rewriter.replaceOpWithNewOp<mlir::LLVM::GlobalOp>(
      op, toolchainType, isConst, linkage, symbol, init.value_or(mlir::Attribute()),
...
CIRToLLVMGlobalOpLowering::getComdatAttr(cir::GlobalOp &op,
  ...
  StringRef comdatName("__toolchain_comdat_globals");
  if (!comdatOp) {
...
mlir::LogicalResult CIRToLLVMSwitchFlatOpLowering::matchAndRewrite(
  ...
  toolchain::SmallVector<mlir::APInt, 8> caseValues;
  for (mlir::Attribute val : op.getCaseValues()) {
    auto intAttr = cast<cir::IntAttr>(val);
    caseValues.push_back(intAttr.getValue());
  }

  toolchain::SmallVector<mlir::Block *, 8> caseDestinations;
  toolchain::SmallVector<mlir::ValueRange, 8> caseOperands;
...
mlir::LogicalResult CIRToLLVMUnaryOpLowering::matchAndRewrite(
  ...
  mlir::Type toolchainType = getTypeConverter()->convertType(type);
  mlir::Location loc = op.getLoc();
  ...
  if (mlir::isa<cir::IntType>(elementType)) {
    ...
    case cir::UnaryOpKind::Inc: {
      ...
      auto one = rewriter.create<mlir::LLVM::ConstantOp>(loc, toolchainType, 1);
      rewriter.replaceOpWithNewOp<mlir::LLVM::AddOp>(
          op, toolchainType, adaptor.getInput(), one, maybeNSW);
      return mlir::success();
    }
    ...
    case cir::UnaryOpKind::Minus: {
      mlir::Value zero;
      if (isVector)
        zero = rewriter.create<mlir::LLVM::ZeroOp>(loc, toolchainType);
      else
        zero = rewriter.create<mlir::LLVM::ConstantOp>(loc, toolchainType, 0);
    ...
    case cir::UnaryOpKind::Not: {
      ...
      if (isVector) {
        ...
        minusOne =
            rewriter.create<mlir::LLVM::ConstantOp>(loc, toolchainType, denseVec);
      } else {
        minusOne = rewriter.create<mlir::LLVM::ConstantOp>(loc, toolchainType, -1);
      }
    ...
    toolchain_unreachable("Unexpected unary op for int");
  }
  ...
  if (mlir::isa<cir::FPTypeInterface>(elementType)) {
    ...
    case cir::UnaryOpKind::Inc: {
      ...
      mlir::LLVM::ConstantOp one = rewriter.create<mlir::LLVM::ConstantOp>(
          loc, toolchainType, rewriter.getFloatAttr(toolchainType, 1.0));
      rewriter.replaceOpWithNewOp<mlir::LLVM::FAddOp>(op, toolchainType, one,
                                                      adaptor.getInput());
      return mlir::success();
    }
    case cir::UnaryOpKind::Dec: {
      ...
      mlir::LLVM::ConstantOp minusOne = rewriter.create<mlir::LLVM::ConstantOp>(
          loc, toolchainType, rewriter.getFloatAttr(toolchainType, -1.0));
      rewriter.replaceOpWithNewOp<mlir::LLVM::FAddOp>(op, toolchainType, minusOne,
                                                      adaptor.getInput());
      return mlir::success();
    }
    case cir::UnaryOpKind::Plus:
      ...
      return mlir::success();
    case cir::UnaryOpKind::Minus:
      rewriter.replaceOpWithNewOp<mlir::LLVM::FNegOp>(op, toolchainType,
                                                      adaptor.getInput());
      return mlir::success();
    case cir::UnaryOpKind::Not:
      return op.emitError() << "Unary not is invalid for floating-point types";
    }
    toolchain_unreachable("Unexpected unary op for float");
  }
  ...
  if (mlir::isa<cir::BoolType>(elementType)) {
    ...
    toolchain_unreachable("Unexpected unary op for bool");
  }
...
mlir::LogicalResult CIRToLLVMBinOpLowering::matchAndRewrite(
  ...
  const mlir::Type toolchainTy = getTypeConverter()->convertType(op.getType());
  const mlir::Type toolchainEltTy = elementTypeIfVector(toolchainTy);
  ...
  case cir::BinOpKind::Add:
    if (mlir::isa<mlir::IntegerType>(toolchainEltTy)) {
      if (op.getSaturated()) {
      ...
      rewriter.replaceOpWithNewOp<mlir::LLVM::AddOp>(op, toolchainTy, lhs, rhs,
  ...
  case cir::BinOpKind::Sub:
    if (mlir::isa<mlir::IntegerType>(toolchainEltTy)) {
      ...
      rewriter.replaceOpWithNewOp<mlir::LLVM::SubOp>(op, toolchainTy, lhs, rhs,
                                                     getIntOverflowFlag(op));
    } else {
  ...
  case cir::BinOpKind::Mul:
    if (mlir::isa<mlir::IntegerType>(toolchainEltTy))
      rewriter.replaceOpWithNewOp<mlir::LLVM::MulOp>(op, toolchainTy, lhs, rhs,
                                                     getIntOverflowFlag(op));
    else
  ...
  case cir::BinOpKind::Max:
    if (mlir::isa<mlir::IntegerType>(toolchainEltTy)) {
      auto isUnsigned = isIntTypeUnsigned(type);
      if (isUnsigned)
        rewriter.replaceOpWithNewOp<mlir::LLVM::UMaxOp>(op, toolchainTy, lhs, rhs);
      else
        rewriter.replaceOpWithNewOp<mlir::LLVM::SMaxOp>(op, toolchainTy, lhs, rhs);
    }
    break;
  }
  return mlir::LogicalResult::success();
...
convertCmpKindToICmpPredicate(cir::CmpOpKind kind, bool isSigned) {
  ...
  toolchain_unreachable("Unknown CmpOpKind");
}
...
mlir::LogicalResult CIRToLLVMShiftOpLowering::matchAndRewrite(
  ...
  if (cirAmtTy) {
    ...
    if (cirAmtTy)
      amt = getLLVMIntCast(rewriter, amt, toolchainTy, true, cirAmtTy.getWidth(),
                           cirValTy.getWidth());
  } else {
  ...
  if (op.getIsShiftleft()) {
    rewriter.replaceOpWithNewOp<mlir::LLVM::ShlOp>(op, toolchainTy, val, amt);
    return mlir::success();
  }

  if (isUnsigned)
    rewriter.replaceOpWithNewOp<mlir::LLVM::LShrOp>(op, toolchainTy, val, amt);
  else
    rewriter.replaceOpWithNewOp<mlir::LLVM::AShrOp>(op, toolchainTy, val, amt);
  return mlir::success();
}
...
mlir::LogicalResult CIRToLLVMSelectOpLowering::matchAndRewrite(
  ...
  mlir::Value toolchainCondition = adaptor.getCondition();
  rewriter.replaceOpWithNewOp<mlir::LLVM::SelectOp>(
      op, toolchainCondition, adaptor.getTrueValue(), adaptor.getFalseValue());
...
static void prepareTypeConverter(mlir::LLVMTypeConverter &converter,
  ...
  converter.addConversion([&](cir::FuncType type) -> std::optional<mlir::Type> {
    ...
    toolchain::SmallVector<mlir::Type> arguments;
  ...
  converter.addConversion([&](cir::RecordType type) -> mlir::Type {
    ...
    mlir::LLVM::LLVMStructType toolchainStruct;
    if (type.getName()) {
      toolchainStruct = mlir::LLVM::LLVMStructType::getIdentified(
          type.getContext(), type.getPrefixedName());
      if (toolchainStruct.setBody(toolchainMembers, type.getPacked()).failed())
        toolchain_unreachable("Failed to set body of record");
    } else { // Record has no name: lower as literal record.
      toolchainStruct = mlir::LLVM::LLVMStructType::getLiteral(
          type.getContext(), toolchainMembers, type.getPacked());
    }

    return toolchainStruct;
  });
}
...
// not lowered and the new parent is toolchain.func.
//
...
static void collectUnreachable(mlir::Operation *parent,
                               toolchain::SmallVector<mlir::Operation *> &ops) {

  toolchain::SmallVector<mlir::Block *> unreachableBlocks;
...
void ConvertCIRToLLVMPass::runOnOperation() {
  toolchain::TimeTraceScope scope("Convert CIR to LLVM Pass");
...
mlir::LogicalResult CIRToLLVMGetMemberOpLowering::matchAndRewrite(
  ...
  case cir::RecordType::Struct: {
    ...
    toolchain::SmallVector<mlir::LLVM::GEPArg, 2> offset{0, op.getIndex()};
...
mlir::LogicalResult CIRToLLVMTrapOpLowering::matchAndRewrite(
  ...
  // Note that the call to toolchain.trap is not a terminator in LLVM dialect.
  // So we must emit an additional toolchain.unreachable to terminate the current
  // block.
...
getValueForVTableSymbol(mlir::Operation *op,
  ...
  if (auto toolchainSymbol = mlir::dyn_cast<mlir::LLVM::GlobalOp>(symbol)) {
    eltType = toolchainSymbol.getType();
...
mlir::LogicalResult CIRToLLVMVTableAddrPointOpLowering::matchAndRewrite(
  ...
  toolchain::SmallVector<mlir::LLVM::GEPArg> offsets;
  mlir::Type eltType;
  ...
  offsets = toolchain::SmallVector<mlir::LLVM::GEPArg>{
...
mlir::LogicalResult CIRToLLVMVecCreateOpLowering::matchAndRewrite(
  ...
  const mlir::Type toolchainTy = typeConverter->convertType(vecTy);
  const mlir::Location loc = op.getLoc();
  mlir::Value result = rewriter.create<mlir::LLVM::PoisonOp>(loc, toolchainTy);
...
mlir::LogicalResult CIRToLLVMVecSplatOpLowering::matchAndRewrite(
  ...
  mlir::Type toolchainTy = typeConverter->convertType(vecTy);
  mlir::Location loc = op.getLoc();
  mlir::Value poison = rewriter.create<mlir::LLVM::PoisonOp>(loc, toolchainTy);
  ...
  if (auto constValue = elementValue.getDefiningOp<mlir::LLVM::ConstantOp>()) {
    if (auto intAttr = dyn_cast<mlir::IntegerAttr>(constValue.getValue())) {
      mlir::DenseIntElementsAttr denseVec = mlir::DenseIntElementsAttr::get(
          mlir::cast<mlir::ShapedType>(toolchainTy), intAttr.getValue());
      rewriter.replaceOpWithNewOp<mlir::LLVM::ConstantOp>(
    ...
    if (auto fpAttr = dyn_cast<mlir::FloatAttr>(constValue.getValue())) {
      mlir::DenseFPElementsAttr denseVec = mlir::DenseFPElementsAttr::get(
          mlir::cast<mlir::ShapedType>(toolchainTy), fpAttr.getValue());
      rewriter.replaceOpWithNewOp<mlir::LLVM::ConstantOp>(
          op, denseVec.getType(), denseVec);
      return mlir::success();
    }
...
mlir::LogicalResult CIRToLLVMVecShuffleDynamicOpLowering::matchAndRewrite(
  ...
  mlir::Type toolchainIndexVecType =
      getTypeConverter()->convertType(op.getIndices().getType());
  mlir::Type toolchainIndexType = getTypeConverter()->convertType(
      elementTypeIfVector(op.getIndices().getType()));
  uint64_t numElements =
      mlir::cast<cir::VectorType>(op.getVec().getType()).getSize();

  uint64_t maskBits = toolchain::NextPowerOf2(numElements - 1) - 1;
  mlir::Value maskValue = rewriter.create<mlir::LLVM::ConstantOp>(
      loc, toolchainIndexType, rewriter.getIntegerAttr(toolchainIndexType, maskBits));
  mlir::Value maskVector =
      rewriter.create<mlir::LLVM::UndefOp>(loc, toolchainIndexVecType);
  ...
  mlir::Value maskedIndices = rewriter.create<mlir::LLVM::AndOp>(
      loc, toolchainIndexVecType, adaptor.getIndices(), maskVector);
...
mlir::LogicalResult CIRToLLVMComplexRealOpLowering::matchAndRewrite(
  ...
  rewriter.replaceOpWithNewOp<mlir::LLVM::ExtractValueOp>(
      op, resultLLVMTy, adaptor.getOperand(), toolchain::ArrayRef<std::int64_t>{0});
  return mlir::success();
}
...
mlir::LogicalResult CIRToLLVMComplexImagOpLowering::matchAndRewrite(
  ...
  mlir::Type resultLLVMTy = getTypeConverter()->convertType(op.getType());
  rewriter.replaceOpWithNewOp<mlir::LLVM::ExtractValueOp>(
      op, resultLLVMTy, adaptor.getOperand(), toolchain::ArrayRef<std::int64_t>{1});
  return mlir::success();
}
...
mlir::LogicalResult CIRToLLVMSetBitfieldOpLowering::matchAndRewrite(
  ...
  if (storageSize != size) {
    ...
    srcVal =
        createAnd(rewriter, srcVal, toolchain::APInt::getLowBitsSet(srcWidth, size));
    resultVal = srcVal;
    srcVal = createShL(rewriter, srcVal, offset);

    // Mask out the original value.
    val = createAnd(rewriter, val,
                    ~toolchain::APInt::getBitsSet(srcWidth, offset, offset + size));
...
mlir::LogicalResult CIRToLLVMGetBitfieldOpLowering::matchAndRewrite(
  ...
  if (info.getIsSigned()) {
    ...
    if (static_cast<unsigned>(offset) + size < storageSize)
      val = createAnd(rewriter, val,
                      toolchain::APInt::getLowBitsSet(storageSize, size));
  }
...
mlir::LogicalResult CIRToLLVMInlineAsmOpLowering::matchAndRewrite(
  ...
  StringRef toolchainAttrName = mlir::LLVM::InlineAsmOp::getElementTypeAttrName();
  ...
  SmallVector<mlir::Value> toolchainOperands;
  SmallVector<mlir::Value> cirOperands;
  for (auto const&[toolchainOp, cirOp] :
       zip(adaptor.getAsmOperands(), op.getAsmOperands())) {
    append_range(toolchainOperands, toolchainOp);
    append_range(cirOperands, cirOp);
  }

  // so far we infer the toolchain dialect element type attr from
  // CIR operand type.
  for (auto const&[cirOpAttr, cirOp] : zip(op.getOperandAttrs(), cirOperands)) {    
    ...
    toolchain::SmallVector<mlir::NamedAttribute, 1> attrs;
    cir::PointerType typ =
    ...
    attrs.push_back(rewriter.getNamedAttr(toolchainAttrName, typAttr));
  ...
  rewriter.replaceOpWithNewOp<mlir::LLVM::InlineAsmOp>(
      op, llResTy, toolchainOperands, op.getAsmStringAttr(), op.getConstraintsAttr(),
...
lowerDirectlyFromCIRToLLVMIR(mlir::ModuleOp mlirModule, LLVMContext &toolchainCtx) {
  toolchain::TimeTraceScope scope("lower from CIR to LLVM directly");
  ...
  toolchain::TimeTraceScope translateScope("translateModuleToLLVMIR");

  StringRef moduleName = mlirModule.getName().value_or("CIRToLLVMModule");
  std::unique_ptr<toolchain::Module> toolchainModule =
      mlir::translateModuleToLLVMIR(mlirModule, toolchainCtx, moduleName);

  if (!toolchainModule) {
    // FIXME: Handle any errors where they occurs and return a nullptr here.
    report_fatal_error("Lowering from LLVMIR dialect to toolchain IR failed!");
  }
