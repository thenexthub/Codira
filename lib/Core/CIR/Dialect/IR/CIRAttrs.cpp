...
static void printIntLiteral(mlir::AsmPrinter &p, toolchain::APInt value,
...
static void printFloatLiteral(mlir::AsmPrinter &p, toolchain::APFloat value,
                              mlir::Type ty);
static mlir::ParseResult
parseFloatLiteral(mlir::AsmParser &parser,
                  mlir::FailureOr<toolchain::APFloat> &value,
...
#include "language/Core/CIR/Dialect/IR/CIROpsAttributes.cpp.inc"
...
void CIRDialect::printAttribute(Attribute attr, DialectAsmPrinter &os) const {
  if (failed(generatedAttributePrinter(attr, os)))
    toolchain_unreachable("unexpected CIR type kind");
}
...
mlir::ParseResult parseIntLiteral(mlir::AsmParser &parser, toolchain::APInt &value,
                                  cir::IntTypeInterface ty) {
  if (ty.isSigned())
...
void printIntLiteral(mlir::AsmPrinter &p, toolchain::APInt value,
                     cir::IntTypeInterface ty) {
  if (ty.isSigned())
...
LogicalResult IntAttr::verify(function_ref<InFlightDiagnostic()> emitError,
                              cir::IntTypeInterface type, toolchain::APInt value) {
