...
#include "language/Core/CIR/Dialect/IR/CIROpsDialect.cpp.inc"
#include "language/Core/CIR/Dialect/IR/CIROpsEnums.cpp.inc"
...
struct CIROpAsmDialectInterface : public OpAsmDialectInterface {
  ...
  AliasResult getAlias(Type type, raw_ostream &os) const final {
    ...
    if (auto intType = dyn_cast<cir::IntType>(type)) {
      ...
      if (width < 8 || !toolchain::isPowerOf2_32(width))
...
void cir::CIRDialect::initialize() {
  ...
  addOperations<
#define GET_OP_LIST
#include "language/Core/CIR/Dialect/IR/CIROps.cpp.inc"
      >();
...
static int parseOptionalKeywordAlternative(AsmParser &parser,
                                           ArrayRef<toolchain::StringRef> keywords) {
  for (auto en : toolchain::enumerate(keywords)) {
...
#define REGISTER_ENUM_TYPE(Ty)                                                 \
  template <> struct EnumTraits<cir::Ty> {                                     \
    static toolchain::StringRef stringify(cir::Ty value) {                          \
...
static RetTy parseOptionalCIRKeyword(AsmParser &parser, EnumTy defaultValue) {
  toolchain::SmallVector<toolchain::StringRef, 10> names;
...
void cir::AllocaOp::build(mlir::OpBuilder &odsBuilder,
                          ...
                          mlir::Type allocaType, toolchain::StringRef name,
...
LogicalResult cir::CastOp::verify() {
  ...
  default:
    toolchain_unreachable("Unknown CastOp kind?");
  }
}
...
OpFoldResult cir::CastOp::fold(FoldAdaptor adaptor) {
  ...
  if (getSrc().getType() == getType()) {
    ...
    case cir::CastKind::integral: {
      ...
      toolchain::SmallVector<mlir::OpFoldResult, 1> foldResults;
...
static mlir::ParseResult parseCallCommon(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 4> ops;
  toolchain::SMLoc opsLoc;
  mlir::FlatSymbolRefAttr calleeAttr;
  toolchain::ArrayRef<mlir::Type> allResultTypes;
...
void cir::SwitchOp::collectCases(toolchain::SmallVectorImpl<CaseOp> &cases) {
...
bool cir::SwitchOp::isSimpleForm(toolchain::SmallVectorImpl<CaseOp> &cases) {
  collectCases(cases);
  ...
  if (!toolchain::all_of(getBody().front(),
                    [](Operation &op) { return isa<CaseOp, YieldOp>(op); }))
    return false;

  return toolchain::all_of(cases, [this](CaseOp op) {
...
static ParseResult parseSwitchFlatOpCases(
    ...
    SmallVectorImpl<toolchain::SmallVector<OpAsmParser::UnresolvedOperand>>
        &caseOperands,
    SmallVectorImpl<toolchain::SmallVector<Type>> &caseOperandTypes) {
  if (failed(parser.parseLSquare()))
    return failure();
  if (succeeded(parser.parseOptionalRSquare()))
    return success();
  toolchain::SmallVector<mlir::Attribute> values;
  ...
  auto parseCase = [&]() {
    ...
    Block *destination;
    toolchain::SmallVector<OpAsmParser::UnresolvedOperand> operands;
    toolchain::SmallVector<Type> operandTypes;
...
void cir::GlobalOp::build(OpBuilder &odsBuilder, OperationState &odsState,
                          toolchain::StringRef sym_name, mlir::Type sym_type,
...
cir::GetGlobalOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (auto g = dyn_cast<GlobalOp>(op)) {
    ...
    toolchain_unreachable("Unexpected operation for GetGlobalOp");
  }
...
static toolchain::StringRef getLinkageAttrNameString() { return "linkage"; }
...
ParseResult cir::FuncOp::parse(OpAsmParser &parser, OperationState &state) {
  ...
  ::toolchain::StringRef visAttrStr;
  ...
  if (parser.parseSymbolName(nameAttr, SymbolTable::getSymbolAttrName(),
                             state.attributes))
    return failure();
  toolchain::SmallVector<OpAsmParser::Argument, 8> arguments;
  toolchain::SmallVector<mlir::Type> resultTypes;
  toolchain::SmallVector<DictionaryAttr> resultAttrs;
...
// This function corresponds to `toolchain::GlobalValue::isDeclaration` and should
...
LogicalResult cir::UnaryOp::verify() {
  ...
  toolchain_unreachable("Unknown UnaryOp kind?");
}
...
OpFoldResult cir::VecCreateOp::fold(FoldAdaptor adaptor) {
  if (toolchain::any_of(getElements(), [](mlir::Value value) {
...
OpFoldResult cir::VecExtractOp::fold(FoldAdaptor adaptor) {
  const auto vectorAttr =
      toolchain::dyn_cast_if_present<cir::ConstVectorAttr>(adaptor.getVec());
  if (!vectorAttr)
    return {};

  const auto indexAttr =
      toolchain::dyn_cast_if_present<cir::IntAttr>(adaptor.getIndex());
  if (!indexAttr)
    return {};
...
OpFoldResult cir::VecShuffleDynamicOp::fold(FoldAdaptor adaptor) {
  ...
  if (mlir::isa_and_nonnull<cir::ConstVectorAttr>(vec) &&
    ...
    const uint64_t maskBits = toolchain::NextPowerOf2(numElements - 1) - 1;
    for (const auto &idxAttr : indicesElts.getAsRange<cir::IntAttr>()) {
      uint64_t idxValue = idxAttr.getUInt();
...
OpFoldResult cir::VecTernaryOp::fold(FoldAdaptor adaptor) {
  ...
  for (const auto &[idx, condAttr] :
       toolchain::enumerate(condElts.getAsRange<cir::IntAttr>())) {
    if (condAttr.getSInt()) {
...
foldUnaryBitOp(mlir::Attribute inputAttr,
               toolchain::function_ref<toolchain::APInt(const toolchain::APInt &)> func,
...
OpFoldResult BitClzOp::fold(FoldAdaptor adaptor) {
  return foldUnaryBitOp(
      adaptor.getInput(),
      [](const toolchain::APInt &inputValue) {
...
OpFoldResult BitCtzOp::fold(FoldAdaptor adaptor) {
  return foldUnaryBitOp(
      adaptor.getInput(),
      [](const toolchain::APInt &inputValue) {
        return toolchain::APInt(inputValue.getBitWidth(),
...
OpFoldResult BitFfsOp::fold(FoldAdaptor adaptor) {
  return foldUnaryBitOp(adaptor.getInput(), [](const toolchain::APInt &inputValue) {
    ...
    return toolchain::APInt(inputValue.getBitWidth(), result);
  });
}

OpFoldResult BitParityOp::fold(FoldAdaptor adaptor) {
  return foldUnaryBitOp(adaptor.getInput(), [](const toolchain::APInt &inputValue) {
    return toolchain::APInt(inputValue.getBitWidth(), inputValue.popcount() % 2);
  });
}

OpFoldResult BitPopcountOp::fold(FoldAdaptor adaptor) {
  return foldUnaryBitOp(adaptor.getInput(), [](const toolchain::APInt &inputValue) {
    return toolchain::APInt(inputValue.getBitWidth(), inputValue.popcount());
  });
}
...
void cir::InlineAsmOp::print(OpAsmPrinter &p) {
  ...
  toolchain::SmallVector<std::string, 3> names{"out", "in", "in_out"};
  ...
  for (mlir::OperandRange ops : getAsmOperands()) {
    ...
    p << '[';
    toolchain::interleaveComma(toolchain::make_range(ops.begin(), ops.end()), p,
                          [&](Value value) {
  ...
  std::array elidedAttrs{
      toolchain::StringRef("asm_flavor"),        toolchain::StringRef("asm_string"),
      toolchain::StringRef("constraints"),       toolchain::StringRef("operand_attrs"),
      toolchain::StringRef("operands_segments"), toolchain::StringRef("side_effects")};
...
ParseResult cir::InlineAsmOp::parse(OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::Attribute> operandAttrs;
  toolchain::SmallVector<int32_t> operandsGroupSizes;
  ...
  auto parseValue = [&](Value &v) {
    ...
    toolchain::SmallVector<mlir::Value> tmp;
  ...
  auto parseOperands = [&](toolchain::StringRef name) {
