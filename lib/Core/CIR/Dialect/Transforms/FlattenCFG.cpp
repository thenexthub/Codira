  ...
  matchAndRewrite(cir::SwitchOp op,
    ...
    // block.
    {
      ...
      for (mlir::Block &block :
           toolchain::make_early_inc_range(op.getBody().getBlocks()))
    ...
    toolchain::SmallVector<std::pair<APInt, APInt>> rangeValues;
    toolchain::SmallVector<mlir::Block *> rangeDestinations;
    toolchain::SmallVector<mlir::ValueRange> rangeOperands;
    ...
    for (auto [rangeVal, operand, destination] :
         toolchain::zip(rangeValues, rangeOperands, rangeDestinations)) {
      ...
      if ((upperBound - lowerBound)
              .ult(toolchain::APInt(32, kSmallRangeThreshold))) {
...
public:
  ...
  matchAndRewrite(cir::TernaryOp op,
    ...
    toolchain::SmallVector<mlir::Location, 2> locs;
