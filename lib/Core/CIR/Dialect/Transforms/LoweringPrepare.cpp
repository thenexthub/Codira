...
struct LoweringPreparePass : public LoweringPrepareBase<LoweringPreparePass> {
  ...
  cir::FuncOp buildRuntimeFunction(
      mlir::OpBuilder &builder, toolchain::StringRef name, mlir::Location loc,
      cir::FuncType type,
  ...
  void setASTContext(language::Core::ASTContext *c) { astCtx = c; }
};
...
static mlir::Value buildComplexBinOpLibCall(
    ...
    toolchain::StringRef (*libFuncNameGetter)(toolchain::APFloat::Semantics),
  ...
  toolchain::StringRef libFuncName = libFuncNameGetter(
      toolchain::APFloat::SemanticsToEnum(elementTy.getFloatSemantics()));
  toolchain::SmallVector<mlir::Type, 4> libFuncInputTypes(4, elementTy);
...
getComplexMulLibCallName(toolchain::APFloat::Semantics semantics) {
  switch (semantics) {
  case toolchain::APFloat::S_IEEEhalf:
    return "__mulhc3";
  case toolchain::APFloat::S_IEEEsingle:
    return "__mulsc3";
  case toolchain::APFloat::S_IEEEdouble:
    return "__muldc3";
  case toolchain::APFloat::S_PPCDoubleDouble:
    return "__multc3";
  case toolchain::APFloat::S_x87DoubleExtended:
    return "__mulxc3";
  case toolchain::APFloat::S_IEEEquad:
    return "__multc3";
  default:
...
void LoweringPreparePass::runOnOperation() {
  ...
  toolchain::SmallVector<mlir::Operation *> opsToTransform;
...
std::unique_ptr<Pass>
mlir::createLoweringPreparePass(language::Core::ASTContext *astCtx) {
