...
AST_MATCHER_P(ObjCMessageExpr, hasAnySelectorMatcher, std::vector<std::string>,
              Matches) {
  return toolchain::is_contained(Matches, Node.getSelector().getAsString());
}
...
public:
  explicit DynTraversalMatcherImpl(
      language::Core::TraversalKind TK,
      IntrusiveRefCntPtr<DynMatcherInterface> InnerMatcher)
...
DynTypedMatcher::constructVariadic(DynTypedMatcher::VariadicOperator Op,
  ...
  assert(toolchain::all_of(InnerMatchers,
  ...
  toolchain_unreachable("Invalid Op value.");
}
...
DynTypedMatcher DynTypedMatcher::trueMatcher(ASTNodeKind NodeKind) {
  ...
  static const toolchain::IntrusiveRefCntPtr<TrueMatcherImpl> Instance =
      new TrueMatcherImpl();
...
static bool allOfVariadicOperator(const DynTypedNode &DynNode,
  ...
  return toolchain::all_of(InnerMatchers, [&](const DynTypedMatcher &InnerMatcher) {
...
HasNameMatcher::HasNameMatcher(std::vector<std::string> N)
    : UseUnqualifiedMatch(
          toolchain::all_of(N, [](StringRef Name) { return !Name.contains("::"); })),
      Names(std::move(N)) {
#ifndef NDEBUG
...
static StringRef getNodeName(const NamedDecl &Node,
                             toolchain::SmallString<128> &Scratch) {
  // Simple name.
...
public:
  ...
  bool consumeNameSuffix(StringRef NodeName, bool CanSkip) {
    if (CanSkip) {
      ...
      toolchain::erase_if(Patterns, [&NodeName](auto &Pattern) {
        return !::language::Core::ast_matchers::internal::consumeNameSuffix(Pattern.P,
                                                                   NodeName);
      });
    }
  ...
  bool foundMatch(bool AllowFullyQualified) const {
    return toolchain::any_of(Patterns, [&](const Pattern &Pattern) {
...
private:
  ...
  toolchain::SmallVector<Pattern, 8> Patterns;
};
...
bool HasNameMatcher::matchesNodeUnqualified(const NamedDecl &Node) const {
  ...
  return toolchain::any_of(Names, [&](StringRef Name) {
...
bool HasNameMatcher::matchesNodeFullSlow(const NamedDecl &Node) const {
  ...
  for (bool SkipUnwritten : SkipUnwrittenCases) {
    toolchain::SmallString<128> NodeName = StringRef("::");
    toolchain::raw_svector_ostream OS(NodeName);
...
static std::optional<SourceLocation> getExpansionLocOfMacroRecursive(
    ...
    toolchain::DenseSet<SourceLocation> &CheckedLocations) {
...
std::shared_ptr<toolchain::Regex> createAndVerifyRegex(StringRef Regex,
  ...
  auto SharedRegex = std::make_shared<toolchain::Regex>(Regex, Flags);
  std::string Error;
  if (!SharedRegex->isValid(Error)) {
    toolchain::WithColor::error()
        << "building matcher '" << MatcherID << "': " << Error << "\n";
    toolchain::WithColor::note() << " input was '" << Regex << "'\n";
  }
  return SharedRegex;
}
