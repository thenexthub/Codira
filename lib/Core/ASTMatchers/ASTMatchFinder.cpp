...
public:
  ...
  bool dataTraverseNode(Stmt *S, DataRecursionQueue *Queue) {
    if (auto *RF = dyn_cast<CXXForRangeStmt>(S)) {
      ...
      for (auto I : toolchain::zip(LE->captures(), LE->capture_inits())) {
        auto C = std::get<0>(I);
...
private:
  ...
  classIsDerivedFromImpl(const CXXRecordDecl *Declaration,
                         ...
                         toolchain::SmallPtrSetImpl<const CXXRecordDecl *> &Visited);
...
#define IMPL(Index)                                                            \
  ...
  std::enable_if_t<                                                            \
      toolchain::is_one_of<const NodeType *, CMD_TYPES_##Index>::value>             \
  SetCallbackAndRawNode(const MatchCallback *CB, const NodeType &N) {          \
    ...
    Node##Index = &N;                                                          \
                                                                               ...
                                                                               \
  template <typename T>                                                        \
  std::enable_if_t<toolchain::is_one_of<const T *, CMD_TYPES_##Index>::value,       \
                   const T *>                                                  \
  getNode() const {                                                            \
    assertHoldsState();                                                        \
  ...
  private:
    ...
    toolchain::PointerIntPair<const MatchCallback *, 1> Callback;
    union {
      toolchain::PointerUnion<CMD_TYPES_0> Node0;
      toolchain::PointerUnion<CMD_TYPES_1> Node1;
    };
...
public:
  class TraceReporter : toolchain::PrettyStackTraceEntry {
...
private:
  ...
  public:
    ...
    /// does nothing.
    void setBucket(toolchain::TimeRecord *NewBucket) {
      if (Bucket != NewBucket) {
        auto Now = toolchain::TimeRecord::getCurrentTime(true);
        if (Bucket)
          *Bucket += Now;
        if (NewBucket)
          *NewBucket -= Now;
  ...
  private:
    toolchain::TimeRecord *Bucket = nullptr;
  };
  ...
  bool matchesAnyAncestorOf(DynTypedNode Node, ASTContext &Ctx,
    ...
    if (Parents.empty()) {
      ...
      if (!Node.get<TranslationUnitDecl>() &&
          ...
          toolchain::any_of(ActiveASTContext->getTraversalScope(), [](Decl *D) {
            return D->getKind() == Decl::TranslationUnit;
          })) {
        toolchain::errs() << "Tried to match orphan node:\n";
        Node.dump(toolchain::errs(), *ActiveASTContext);
        toolchain_unreachable("Parent map should be complete!");
      }
#endif
    } else {
  ...
  toolchain::StringMap<toolchain::TimeRecord> TimeByBucket;
  ...
  toolchain::DenseMap<ASTNodeKind, std::vector<unsigned short>> MatcherFiltersMap;

  const MatchFinder::MatchFinderOptions &Options;
  ASTContext *ActiveASTContext;

  // Maps a canonical type to its TypedefDecls.
  toolchain::DenseMap<const Type*, std::set<const TypedefNameDecl*> > TypeAliases;

  // Maps an Objective-C interface to its ObjCCompatibleAliasDecls.
  toolchain::DenseMap<const ObjCInterfaceDecl *,
                 toolchain::SmallPtrSet<const ObjCCompatibleAliasDecl *, 2>>
      CompatibleAliases;
...
void MatchFinder::match(const language::Core::DynTypedNode &Node, ASTContext &Context) {
