...
using ConstructorMap =
    toolchain::StringMap<std::unique_ptr<const MatcherDescriptor>>;
...
#define REGISTER_MATCHER(name)                                                 \
  registerMatcher(#name, internal::makeMatcherAutoMarshall(                    \
                             ::language::Core::ast_matchers::name, #name));
...
#define SPECIFIC_MATCHER_OVERLOAD(name, Id)                                    \
  static_cast<::language::Core::ast_matchers::name##_Type##Id>(                         \
      ::language::Core::ast_matchers::name)
...
static toolchain::ManagedStatic<RegistryMaps> RegistryData;
...
static toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS,
...
Registry::getMatcherCompletions(ArrayRef<ArgKind> AcceptedTypes) {
  ...
  for (const auto &M : RegistryData->constructors()) {
    ...
    if (!RetKinds.empty() && MaxSpecificity > 0) {
      std::string Decl;
      toolchain::raw_string_ostream OS(Decl);
