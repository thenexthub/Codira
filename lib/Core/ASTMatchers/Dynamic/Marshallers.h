...
#ifndef LANGUAGE_CORE_LIB_ASTMATCHERS_DYNAMIC_MARSHALLERS_H
#define LANGUAGE_CORE_LIB_ASTMATCHERS_DYNAMIC_MARSHALLERS_H
...
private:
  static std::optional<attr::Kind> getAttrKind(toolchain::StringRef AttrKind) {
    if (!AttrKind.consume_front("attr::"))
      return std::nullopt;
    return toolchain::StringSwitch<std::optional<attr::Kind>>(AttrKind)
...
private:
  static std::optional<CastKind> getCastKind(toolchain::StringRef AttrKind) {
    if (!AttrKind.consume_front("CK_"))
      return std::nullopt;
    return toolchain::StringSwitch<std::optional<CastKind>>(AttrKind)
...
template <> struct ArgTypeTraits<toolchain::Regex::RegexFlags> {
private:
  static std::optional<toolchain::Regex::RegexFlags> getFlags(toolchain::StringRef Flags);
...
public:
  ...
  static toolchain::Regex::RegexFlags get(const VariantValue &Value) {
...
private:
  ...
  getClauseKind(toolchain::StringRef ClauseKind) {
    return toolchain::StringSwitch<std::optional<OpenMPClauseKind>>(ClauseKind)
#define GEN_CLANG_CLAUSE_CLASS
#define CLAUSE_CLASS(Enum, Str, Class) .Case(#Enum, toolchain::omp::Clause::Enum)
...
private:
  ...
  getUnaryOrTypeTraitKind(toolchain::StringRef ClauseKind) {
    if (!ClauseKind.consume_front("UETT_"))
      return std::nullopt;
    return toolchain::StringSwitch<std::optional<UnaryExprOrTypeTrait>>(ClauseKind)
...
public:
  ...
  VariantMatcher create(SourceRange NameRange, ArrayRef<ParserValue> Args,
    ...
    if (!ArgTypeTraits<toolchain::Regex::RegexFlags>::hasCorrectType(
            Args[1].Value)) {
      Error->addError(Args[1].Range, Error->ET_RegistryWrongArgType)
          << 2 << ArgTypeTraits<toolchain::Regex::RegexFlags>::getKind().asString()
          << Args[1].Value.getTypeAsString();
      return VariantMatcher();
    }
    if (!ArgTypeTraits<toolchain::Regex::RegexFlags>::hasCorrectValue(
            Args[1].Value)) {
      if (std::optional<std::string> BestGuess =
              ArgTypeTraits<toolchain::Regex::RegexFlags>::getBestGuess(
                  Args[1].Value)) {
    ...
    return outvalueToVariantMatcher(
        WithFlags(ArgTypeTraits<StringRef>::get(Args[0].Value),
                  ArgTypeTraits<toolchain::Regex::RegexFlags>::get(Args[1].Value)));
  }

private:
  ReturnType (*const WithFlags)(StringRef, toolchain::Regex::RegexFlags);
...
std::unique_ptr<MatcherDescriptor> makeMatcherRegexMarshall(
    ReturnType (*FuncFlags)(toolchain::StringRef, toolchain::Regex::RegexFlags),
    ReturnType (*Func)(toolchain::StringRef)) {
  std::vector<ASTNodeKind> RetTypes;
  BuildReturnTypeVector<ReturnType>::build(RetTypes);
...
#endif // LANGUAGE_CORE_LIB_ASTMATCHERS_DYNAMIC_MARSHALLERS_H
