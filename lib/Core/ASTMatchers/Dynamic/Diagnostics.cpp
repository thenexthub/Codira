...
static StringRef contextTypeToFormatString(Diagnostics::ContextType Type) {
  ...
  toolchain_unreachable("Unknown ContextType value.");
}
...
static StringRef errorTypeToFormatString(Diagnostics::ErrorType Type) {
  ...
  toolchain_unreachable("Unknown ErrorType value.");
}
...
printMessageToStream(const Diagnostics::ErrorContent::Message &Message,
                     const Twine Prefix, toolchain::raw_ostream &OS) {
...
void Diagnostics::printToStream(toolchain::raw_ostream &OS) const {
...
void Diagnostics::printToStreamFull(toolchain::raw_ostream &OS) const {
  for (size_t i = 0, e = Errors.size(); i != e; ++i) {
    if (i != 0) OS << "\n";
