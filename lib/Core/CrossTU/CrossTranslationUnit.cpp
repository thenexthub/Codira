...
#include "language/Core/CrossTU/CrossTUDiagnostic.h"
...
bool hasEqualKnownFields(const toolchain::Triple &Lhs, const toolchain::Triple &Rhs) {
...
// FIXME: This class is will be removed after the transition to toolchain::Error.
...
public:
  ...
  std::string message(int Condition) const override {
    ...
    toolchain_unreachable("Unrecognized index_error_code.");
  }
};

static toolchain::ManagedStatic<IndexErrorCategory> Category;
...
parseCrossTUIndex(StringRef IndexPath) {
  ...
  if (!ExternalMapFile)
    return toolchain::make_error<IndexError>(index_error_code::missing_index_file,
                                        IndexPath.str());

  toolchain::StringMap<std::string> Result;
  std::string Line;
  unsigned LineNo = 1;
  while (std::getline(ExternalMapFile, Line)) {
    ...
    if (!parseCrossTUIndexItem(Line, LookupName, FilePathInIndex))
      return toolchain::make_error<IndexError>(
          index_error_code::invalid_index_format, IndexPath.str(), LineNo);

    // Store paths with posix-style directory separator.
    SmallString<32> FilePath(FilePathInIndex);
    toolchain::sys::path::native(FilePath, toolchain::sys::path::Style::posix);
...
std::string
createCrossTUIndexString(const toolchain::StringMap<std::string> &Index) {
...
template <typename T>
toolchain::Expected<const T *> CrossTranslationUnitContext::getCrossTUDefinitionImpl(
  ...
  toolchain::Expected<ASTUnit *> ASTUnitOrError =
  ...
  const toolchain::Triple &TripleTo = Context.getTargetInfo().getTriple();
  const toolchain::Triple &TripleFrom =
  ...
  if (!hasEqualKnownFields(TripleTo, TripleFrom)) {
    ...
    // diagnostics.
    ++NumTripleMismatch;
    return toolchain::make_error<IndexError>(index_error_code::triple_mismatch,
  ...
  if (LangTo.CPlusPlus != LangFrom.CPlusPlus) {
    ++NumLangMismatch;
    return toolchain::make_error<IndexError>(index_error_code::lang_mismatch);
  }
  ...
  return toolchain::make_error<IndexError>(index_error_code::failed_import);
}
...
CrossTranslationUnitContext::ASTUnitStorage::getASTUnitForFile(
  ...
  if (ASTCacheEntry == FileASTUnitMap.end()) {
    ...
    if (DisplayCTUProgress)
      toolchain::errs() << "CTU loaded AST file: " << FileName << "\n";
...
CrossTranslationUnitContext::ASTUnitStorage::getASTUnitForFunction(
  ...
  if (ASTCacheEntry == NameASTUnitMap.end()) {
    ...
    if (toolchain::Error IndexLoadError =
    ...
    if (It == NameFileMap.end()) {
      ++NumNotInOtherTU;
      return toolchain::make_error<IndexError>(index_error_code::missing_definition);
    }

    // Search in the index for the filename where the definition of FunctionName
    // resides.
    if (toolchain::Expected<ASTUnit *> FoundForFile =
...
CrossTranslationUnitContext::ASTUnitStorage::getFileForFunction(
    StringRef FunctionName, StringRef CrossTUDir, StringRef IndexName) {
  if (toolchain::Error IndexLoadError = ensureCTUIndexLoaded(CrossTUDir, IndexName))
    return std::move(IndexLoadError);
  return NameFileMap[FunctionName];
}

toolchain::Error CrossTranslationUnitContext::ASTUnitStorage::ensureCTUIndexLoaded(
  ...
  if (toolchain::sys::path::is_absolute(IndexName))
    IndexFile = IndexName;
  else
    toolchain::sys::path::append(IndexFile, IndexName);
...
toolchain::Expected<ASTUnit *> CrossTranslationUnitContext::loadExternalAST(
  ...
  toolchain::Expected<ASTUnit *> Unit = ASTStorage.getASTUnitForFunction(
...
CrossTranslationUnitContext::ASTLoader::load(StringRef Identifier) {
  toolchain::SmallString<256> Path;
  if (toolchain::sys::path::is_absolute(Identifier, PathStyle)) {
    Path = Identifier;
  } else {
    Path = CTUDir;
    toolchain::sys::path::append(Path, PathStyle, Identifier);
  }
  ...
  toolchain::sys::path::native(Path, PathStyle);

  // Normalize by removing relative path components.
  toolchain::sys::path::remove_dots(Path, /*remove_dot_dot*/ true, PathStyle);
...
CrossTranslationUnitContext::ASTLoader::loadFromDump(StringRef ASTDumpPath) {
  ...
  TextDiagnosticPrinter *DiagClient =
      new TextDiagnosticPrinter(toolchain::errs(), *DiagOpts);
...
CrossTranslationUnitContext::ASTLoader::loadFromSource(
  ...
  if (toolchain::Error InitError = lazyInitInvocationList())
  ...
  auto Diags = toolchain::makeIntrusiveRefCnt<DiagnosticsEngine>(DiagID, *DiagOpts,
                                                            DiagClient);
...
toolchain::Expected<InvocationListTy>
parseInvocationList(StringRef FileContent, toolchain::sys::path::Style PathStyle) {
  ...
  toolchain::yaml::Stream InvocationFile(FileContent, SM);

  /// Only the first document is processed.
  toolchain::yaml::document_iterator FirstInvocationFile = InvocationFile.begin();
  ...
  toolchain::yaml::Node *DocumentRoot = FirstInvocationFile->getRoot();
  if (!DocumentRoot)
    return toolchain::make_error<IndexError>(
  ...
  /// parts.
  auto *Mappings = dyn_cast<toolchain::yaml::MappingNode>(DocumentRoot);
  if (!Mappings)
    return toolchain::make_error<IndexError>(
        index_error_code::invocation_list_wrong_format);
  ...
  for (auto &NextMapping : *Mappings) {
    ...
    auto *Key = dyn_cast<toolchain::yaml::ScalarNode>(NextMapping.getKey());
    if (!Key)
      return toolchain::make_error<IndexError>(
          index_error_code::invocation_list_wrong_format);
    ...
    toolchain::sys::path::native(NativeSourcePath, PathStyle);
    ...
    /// the invocation.
    auto *Args = dyn_cast<toolchain::yaml::SequenceNode>(NextMapping.getValue());
    if (!Args)
      return toolchain::make_error<IndexError>(
          index_error_code::invocation_list_wrong_format);

    for (auto &Arg : *Args) {
      auto *CmdString = dyn_cast<toolchain::yaml::ScalarNode>(&Arg);
      if (!CmdString)
        return toolchain::make_error<IndexError>(
            index_error_code::invocation_list_wrong_format);
...
toolchain::Error CrossTranslationUnitContext::ASTLoader::lazyInitInvocationList() {
  /// Lazily initialize the invocation list member used for on-demand parsing.
  if (InvocationList)
    return toolchain::Error::success();
  if (index_error_code::success != PreviousParsingResult)
    return toolchain::make_error<IndexError>(PreviousParsingResult);

  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> FileContent =
      toolchain::MemoryBuffer::getFile(InvocationListFilePath);
  if (!FileContent) {
    PreviousParsingResult = index_error_code::invocation_list_file_not_found;
    return toolchain::make_error<IndexError>(PreviousParsingResult);
  }
  std::unique_ptr<toolchain::MemoryBuffer> ContentBuffer = std::move(*FileContent);
  assert(ContentBuffer && "If no error was produced after loading, the pointer "
                          "should not be nullptr.");

  toolchain::Expected<InvocationListTy> ExpectedInvocationList =
  ...
  if (!ToDeclOrError) {
    handleAllErrors(ToDeclOrError.takeError(), [&](const ASTImportError &IE) {
      ...
      case ASTImportError::Unknown:
        toolchain_unreachable("Unknown import error happened.");
        break;
      }
    });
    return toolchain::make_error<IndexError>(index_error_code::failed_import);
