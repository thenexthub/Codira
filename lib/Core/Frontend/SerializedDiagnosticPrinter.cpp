...
typedef toolchain::DenseMap<unsigned, unsigned> AbbrevLookup;
...
private:
  ...
  struct SharedState {
    ...
    toolchain::DenseSet<unsigned> Categories;

    /// The collection of files used.
    toolchain::DenseMap<const char *, unsigned> Files;

    typedef toolchain::DenseMap<const void *, std::pair<unsigned, StringRef> >
    DiagFlagsTy;
...
static void AddSourceLocationAbbrev(toolchain::BitCodeAbbrev &Abbrev) {
...
static void AddRangeLocationAbbrev(toolchain::BitCodeAbbrev &Abbrev) {
...
#undef CASE
  ...
  toolchain_unreachable("invalid diagnostic level");
}
...
DiagnosticsEngine *SDiagsWriter::getMetaDiags() {
  ...
  if (!State->MetaDiagnostics) {
    auto Client = new TextDiagnosticPrinter(toolchain::errs(), State->DiagOpts);
...
void SDiagsWriter::RemoveOldDiagnostics() {
  if (!toolchain::sys::fs::remove(State->OutputFile))
    return;
...
void SDiagsWriter::finish() {
  ...
  if (MergeChildRecords) {
    ...
    if (toolchain::sys::fs::exists(State->OutputFile))
      if (SDiagsMerger(*this).mergeRecordsFromFile(State->OutputFile.c_str()))
        getMetaDiags()->Report(diag::warn_fe_serialized_diag_merge_failure);
  }

  std::error_code EC;
  auto OS = std::make_unique<toolchain::raw_fd_ostream>(State->OutputFile.c_str(),
                                                    EC, toolchain::sys::fs::OF_None);
  if (EC) {
