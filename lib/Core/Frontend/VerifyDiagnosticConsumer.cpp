...
class VerifyDiagnosticConsumer::MarkerTracker {
  ...
  toolchain::StringMap<Marker> Markers;

  // Directives that couldn't be created yet because they name an unknown
  // marker.
  toolchain::StringMap<toolchain::SmallVector<UnattachedDirective, 2>> DeferredDirectives;
...
static bool ParseDirective(StringRef S, ExpectedData *ED, SourceManager &SM,
  ...
  for (ParseHelper PH(S); !PH.Done();) {
    ...
    if (!toolchain::binary_search(Prefixes, DToken))
      continue;
    ...
    toolchain::SmallString<8> CloseBrace("}}");
...
static unsigned PrintUnexpected(DiagnosticsEngine &Diags, SourceManager *SourceMgr,
  ...
  SmallString<256> Fmt;
  toolchain::raw_svector_ostream OS(Fmt);
...
void VerifyDiagnosticConsumer::CheckDiagnostics() {
  ...
  if (!UnparsedFiles.empty()) {
    ...
    toolchain::SmallPtrSet<const FileEntry *, 8> ParsedFileCache;
    ...
    for (const auto &I : UnparsedFiles) {
      ...
      if (Status.foundDirectives()) {
        toolchain::report_fatal_error("-verify directives found after rather"
...
Directive::create(bool RegexKind, SourceLocation DirectiveLoc,
  ...
  while (!S.empty()) {
    if (S.consume_front("{{")) {
      ...
      RegexStr += toolchain::Regex::escape(S.substr(0, VerbatimMatchLength));
