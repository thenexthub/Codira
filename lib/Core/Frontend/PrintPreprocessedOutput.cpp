...
private:
  ...
  raw_ostream *OrigOS;
  std::unique_ptr<toolchain::raw_null_ostream> NullOS;
...
public:
  PrintPPOutputPPCallbacks(Preprocessor &pp, raw_ostream *os, bool lineMarkers,
    ...
    if (KeepSystemIncludes)
      NullOS = std::make_unique<toolchain::raw_null_ostream>();
...
void PrintPPOutputPPCallbacks::EmbedDirective(
  ...
  *OS << "#embed " << (IsAngled ? '<' : '"') << FileName
  ...
  auto PrintToks = [&](toolchain::ArrayRef<Token> Toks) {
    SmallString<128> SpellingBuffer;
  ...
  if (Params.MaybeOffsetParam) {
    *OS << " language::Core::offset(" << Params.MaybeOffsetParam->Offset << ")";
  }
...
static void DoPrintMacros(Preprocessor &PP, raw_ostream *OS) {
  ...
  toolchain::array_pod_sort(MacrosByID.begin(), MacrosByID.end(), MacroIDCompare);
...
///
void language::Core::DoPrintPreprocessedInput(Preprocessor &PP, raw_ostream *OS,
  ...
  *OS << '\n';
  ...
  // so that it can be reused (for example by a language::Core::Parser instance).
