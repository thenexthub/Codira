...
namespace {
  ...
  protected:
    ...
    toolchain::SmallPtrSet<ObjCInterfaceDecl*, 8> ObjCSynthesizedStructs;
    toolchain::SmallPtrSet<ObjCProtocolDecl*, 8> ObjCSynthesizedProtocols;
    toolchain::SmallPtrSet<ObjCInterfaceDecl*, 8> ObjCWrittenInterfaces;
    toolchain::SmallPtrSet<TagDecl*, 32> GlobalDefinedTags;
    ...
    toolchain::SmallPtrSet<ObjCProtocolDecl *, 32> ProtocolExprDecls;

    toolchain::DenseSet<uint64_t> CopyDestroyCache;
    ...
    toolchain::SmallSetVector<ValueDecl *, 8> BlockByCopyDecls;
    toolchain::SmallSetVector<ValueDecl *, 8> BlockByRefDecls;
    toolchain::DenseMap<ValueDecl *, unsigned> BlockByRefDeclNo;
    toolchain::SmallPtrSet<ValueDecl *, 8> ImportedBlockDecls;
    toolchain::SmallPtrSet<VarDecl *, 8> ImportedLocalExternalDecls;

    toolchain::DenseMap<BlockExpr *, std::string> RewrittenBlockExprs;
    toolchain::DenseMap<ObjCInterfaceDecl *,
                    toolchain::SmallSetVector<ObjCIvarDecl *, 8> > ReferencedIvars;

    // ivar bitfield grouping containers
    toolchain::DenseSet<const ObjCInterfaceDecl *> ObjCInterefaceHasBitfieldGroups;
    toolchain::DenseMap<const ObjCIvarDecl* , unsigned> IvarGroupNumber;
    // This container maps an <class, group number for ivar> tuple to the type
    // of the struct where the bitfield belongs.
    toolchain::DenseMap<std::pair<const ObjCInterfaceDecl*, unsigned>, QualType> GroupRecordType;
    SmallVector<FunctionDecl*, 32> FunctionDefinitionsSeen;

    // This maps an original source AST to it's rewritten form. This allows
    // us to avoid rewriting the same node twice (which is very uncommon).
    // This is needed to support some of the exotic property rewriting.
    toolchain::DenseMap<Stmt *, Stmt *> ReplacedNodes;
  ...
  public:
    toolchain::DenseMap<ObjCMethodDecl*, std::string> MethodInternalNames;
    ...
    void ReplaceStmtWithRange(Stmt *Old, Stmt *New, SourceRange SrcRange) {
      ...
      // Get the new text.
      std::string SStr;
      toolchain::raw_string_ostream S(SStr);
    ...
    void GetInnerBlockDeclRefExprs(Stmt *S,
                ...
                toolchain::SmallPtrSetImpl<const DeclContext *> &InnerContexts);
    ...
    StringLiteral *getStringLiteral(StringRef Str) {
      QualType StrType = Context->getConstantArrayType(
          Context->CharTy, toolchain::APInt(32, Str.size() + 1), nullptr,
...
std::unique_ptr<ASTConsumer> language::Core::CreateModernObjCRewriter(
...
void RewriteModernObjC::InitializeCommon(ASTContext &context) {
  ...
  toolchain::MemoryBufferRef MainBuf = SM->getBufferOrFake(MainFileID);
  ...
  if (!IvarT->getAs<TypedefType>() && IvarT->isRecordType()) {
    ...
    if (RD && !RD->getDeclName().getAsIdentifierInfo()) {
      ...
      Expr *Zero = IntegerLiteral::Create(*Context,
                                          toolchain::APInt(UnsignedIntSize, 0),
...
void RewriteModernObjC::RewriteForwardProtocolDecl(DeclGroupRef D) {
  ...
  if (LocStart.isInvalid())
    toolchain_unreachable("Invalid SourceLocation");
...
Stmt *RewriteModernObjC::RewriteObjCStringLiteral(ObjCStringLiteral *Exp) {
  ...
  toolchain::raw_string_ostream prettyBuf(prettyBufS);
...
Stmt *RewriteModernObjC::RewriteObjCBoolLiteralExpr(ObjCBoolLiteralExpr *Exp) {
  ...
  Expr *FlagExp = IntegerLiteral::Create(*Context,
                                         toolchain::APInt(IntSize, Exp->getValue()),
...
Stmt *RewriteModernObjC::RewriteObjCArrayLiteralExpr(ObjCArrayLiteral *Exp) {
  ...
  Expr *count = IntegerLiteral::Create(*Context,
                                       toolchain::APInt(UnsignedIntSize, NumElements),
                                       Context->UnsignedIntTy, SourceLocation());
  ...
  if (fieldDecl->isBitField()) {
    ...
    do {
      if (const ConstantArrayType *CAT = dyn_cast<ConstantArrayType>(AT)) {
        Result += "[";
        toolchain::APInt Dim = CAT->getSize();
...
void RewriteModernObjC::RewriteObjCInternalStruct(ObjCInterfaceDecl *CDecl,
  ...
  if (!ObjCSynthesizedStructs.insert(CDecl).second)
    toolchain_unreachable("struct already synthesize- RewriteObjCInternalStruct");
}
...
void RewriteModernObjC::RewriteIvarOffsetSymbols(ObjCInterfaceDecl *CDecl,
  ...
  // access expression.
  toolchain::SmallSetVector<ObjCIvarDecl *, 8> Ivars = ReferencedIvars[CDecl];

  if (Ivars.empty())
    return;

  toolchain::DenseSet<std::pair<const ObjCInterfaceDecl*, unsigned> > GroupSymbolOutput;
...
void RewriteModernObjC::SynthesizeBlockLiterals(SourceLocation FunLocStart,
  ...
  if (GlobalConstructionExp) {
    ...
    toolchain::raw_string_ostream constructorExprBuf(SStr);
...
void RewriteModernObjC::RewriteBlockPointerDecl(NamedDecl *ND) {
  ...
  if (VarDecl *VD = dyn_cast<VarDecl>(ND))
    ...
    toolchain_unreachable("RewriteBlockPointerDecl(): Decl type not yet handled");
...
Stmt *RewriteModernObjC::SynthBlockInitExpr(BlockExpr *Exp,
  ...
  if (ImportedBlockDecls.size()) {
    ...
    unsigned IntSize =
      static_cast<unsigned>(Context->getTypeSize(Context->IntTy));
    Expr *FlagExp = IntegerLiteral::Create(*Context, toolchain::APInt(IntSize, flag),
...
Stmt *RewriteModernObjC::RewriteFunctionBodyOrGlobalInitializer(Stmt *S) {
  ...
  if (BlockExpr *BE = dyn_cast<BlockExpr>(S)) {
    ...
    toolchain::SmallPtrSet<const DeclContext *, 8> InnerContexts;
  ...
  if (ObjCMessageExpr *MessExpr = dyn_cast<ObjCMessageExpr>(S)) {
    ...
    // InsertText(language::Core::SourceLocation, char const*, unsigned int).
  ...
  if (ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(S)) {
    ...
    // Get the new text.
    std::string SStr;
    toolchain::raw_string_ostream Buf(SStr);
...
static void Write__class_ro_t_initializer(ASTContext *Context, std::string &Result,
  ...
  Result += " __attribute__ ((used, section (\"__DATA,__objc_const\"))) = {\n";
  Result += "\t";
  Result += toolchain::utostr(flags); Result += ", ";
...
static void Write__ivar_list_t_initializer(RewriteModernObjC &RewriteObj,
  ...
  if (OriginalIvars.size() > 0) {
    ...
    for (unsigned i =0, e = Ivars.size(); i < e; i++) {
      ...
      Result += toolchain::utostr(Align); Result += ", ";
      CharUnits Size = Context->getTypeSizeInChars(IVQT);
      Result += toolchain::utostr(Size.getQuantity());
      if (i  == e-1)
        Result += "}}\n";
      else
        Result += "},\n";
...
void RewriteModernObjC::RewriteObjCProtocolMetaData(ObjCProtocolDecl *PDecl,
  ...
  if (!ObjCSynthesizedProtocols.insert(PDecl->getCanonicalDecl()).second)
    toolchain_unreachable("protocol already synthesized");
}
...
void RewriteModernObjC::RewriteMetaDataIntoBuffer(std::string &Result) {
  ...
  if (ClsDefCount > 0) {
    ...
    Result += toolchain::utostr(ClsDefCount); Result += "]";
    Result +=
  ...
  if (CatDefCount > 0) {
    ...
    Result += toolchain::utostr(CatDefCount); Result += "]";
    Result +=
