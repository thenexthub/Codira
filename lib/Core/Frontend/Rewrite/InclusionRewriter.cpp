...
class InclusionRewriter : public PPCallbacks {
  ...
  toolchain::MemoryBufferRef PredefinesBuffer; ///< The preprocessor predefines.
...
public:
  ...
  void setPredefinesBuffer(const toolchain::MemoryBufferRef &Buf) {
...
InclusionRewriter::getIncludedFileName(const IncludedFile *Inc) const {
  if (Inc) {
    ...
    if (B)
      return toolchain::sys::path::filename(B->getBufferIdentifier());
  }
  return StringRef();
}
...
void language::Core::RewriteIncludesInInput(Preprocessor &PP, raw_ostream *OS,
