...
namespace {
  ...
  protected:
    ...
    toolchain::SmallPtrSet<ObjCInterfaceDecl*, 8> ObjCForwardDecls;
...
language::Core::CreateObjCRewriter(const std::string &InFile,
...
Stmt *RewriteObjC::RewriteObjCSynchronizedStmt(ObjCAtSynchronizedStmt *S) {
  ...
  toolchain::raw_string_ostream syncExprBuf(syncExprBufS);
...
Stmt *RewriteObjC::RewriteObjCTryStmt(ObjCAtTryStmt *S) {
  ...
  for (unsigned I = 0, N = S->getNumCatchStmts(); I != N; ++I) {
    ...
    if (Catch->hasEllipsis()) {
      ...
      toolchain_unreachable("@catch rewrite bug");
    }
  }
...
void RewriteObjC::RewriteObjCInternalStruct(ObjCInterfaceDecl *CDecl,
  ...
  if (!ObjCSynthesizedStructs.insert(CDecl).second)
    toolchain_unreachable("struct already synthesize- SynthesizeObjCInternalStruct");
}
