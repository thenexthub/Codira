...
//===----------------------------------------------------------------------===//
//
// This file implements the language::Core::InitializePreprocessor function.
//
//===----------------------------------------------------------------------===//
...
/// specified FP model.
template <typename T>
static T PickFP(const toolchain::fltSemantics *Sem, T IEEEHalfVal, T IEEESingleVal,
                T IEEEDoubleVal, T X87DoubleExtendedVal, T PPCDoubleDoubleVal,
                T IEEEQuadVal) {
  if (Sem == (const toolchain::fltSemantics*)&toolchain::APFloat::IEEEhalf())
    return IEEEHalfVal;
  if (Sem == (const toolchain::fltSemantics*)&toolchain::APFloat::IEEEsingle())
    return IEEESingleVal;
  if (Sem == (const toolchain::fltSemantics*)&toolchain::APFloat::IEEEdouble())
    return IEEEDoubleVal;
  if (Sem == (const toolchain::fltSemantics*)&toolchain::APFloat::x87DoubleExtended())
    return X87DoubleExtendedVal;
  if (Sem == (const toolchain::fltSemantics*)&toolchain::APFloat::PPCDoubleDouble())
    return PPCDoubleDoubleVal;
  assert(Sem == (const toolchain::fltSemantics*)&toolchain::APFloat::IEEEquad());
  return IEEEQuadVal;
}

static void DefineFloatMacros(MacroBuilder &Builder, StringRef Prefix,
                              const toolchain::fltSemantics *Sem, StringRef Ext) {
...
static void DefineTypeSize(const Twine &MacroName, unsigned TypeWidth,
  ...
  toolchain::APInt MaxVal = isSigned ? toolchain::APInt::getSignedMaxValue(TypeWidth)
                                : toolchain::APInt::getMaxValue(TypeWidth);
...
static void DefineFmt(const LangOptions &LangOpts, const Twine &Prefix,
  ...
  toolchain::for_each(StringRef(IsSigned ? "di" : "ouxX"), Emitter);
  ...
  if (LangOpts.C23 && !IsSigned)
    toolchain::for_each(StringRef("bB"), Emitter);
}
...
static void InitializeStandardPredefinedMacros(const TargetInfo &TI,
  ...
  if (LangOpts.HLSL) {
    ...
    // Add target versions
    if (TI.getTriple().getOS() == toolchain::Triple::ShaderModel) {
  ...
  Builder.defineMacro("__STDC_EMBED_NOT_FOUND__",
                      toolchain::itostr(static_cast<int>(EmbedResult::NotFound)));
  Builder.defineMacro("__STDC_EMBED_FOUND__",
                      toolchain::itostr(static_cast<int>(EmbedResult::Found)));
  Builder.defineMacro("__STDC_EMBED_EMPTY__",
                      toolchain::itostr(static_cast<int>(EmbedResult::Empty)));
  ...
  if (LangOpts.OpenCL) {
    if (LangOpts.CPlusPlus) {
      ...
      case 202100:
        ...
        break;
      default:
        toolchain_unreachable("Unsupported C++ version for OpenCL");
      }
      ...
      case 300:
        ...
        break;
      default:
        toolchain_unreachable("Unsupported OpenCL version");
      }
    }
...
void InitializeOpenCLFeatureTestMacros(const TargetInfo &TI,
                                       ...
                                       MacroBuilder &Builder) {
  const toolchain::StringMap<bool> &OpenCLFeaturesMap = TI.getSupportedOpenCLOpts();
  // FIXME: OpenCL options which affect language semantics/syntax
  // should be moved into LangOptions.
  auto defineOpenCLExtMacro = [&](toolchain::StringRef Name, auto... OptArgs) {
...
toolchain::SmallString<32> ConstructFixedPointLiteral(toolchain::APFixedPoint Val,
                                                 toolchain::StringRef Suffix) {
  if (Val.isSigned() && Val == toolchain::APFixedPoint::getMin(Val.getSemantics())) {
    ...
    toolchain::SmallString<32> Literal;
    Literal.push_back('(');
    toolchain::SmallString<32> HalfStr =
...
void DefineFixedPointMacros(const TargetInfo &TI, MacroBuilder &Builder,
  ...
  if (Signed)
    Builder.defineMacro(
        MacroPrefix + "_MIN__",
        ConstructFixedPointLiteral(toolchain::APFixedPoint::getMin(FXSema), Suffix));
}
...
static void InitializePredefinedMacros(const TargetInfo &TI,
  ...
  Builder.defineMacro("__toolchain__");  // LLVM Backend
...
#undef TOSTR2
  ...
  assert(BitIntMaxWidth <= toolchain::IntegerType::MAX_INT_BITS &&
  ...
  auto addLockFreeMacros = [&](const toolchain::Twine &Prefix) {
...
#include "language/Core/Basic/TargetOSMacros.def"
#undef TARGET_OS
  }
...
void language::Core::InitializePreprocessor(Preprocessor &PP,
  ...
  toolchain::raw_string_ostream Predefines(PredefineBuffer);
