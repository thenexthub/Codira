...
#include "language/Core/Frontend/PrecompiledPreamble.h"
  ...
  public:
    ...
    ~SimpleTimer() {
      if (WantTiming) {
        ...
        Elapsed.print(Elapsed, toolchain::errs());
        toolchain::errs() << '\n';
      }
    }
...
template <class T>
static std::unique_ptr<T> valueOrNull(toolchain::ErrorOr<std::unique_ptr<T>> Val) {
  if (!Val)
    return nullptr;
  return std::move(*Val);
}
...
template <class T>
static bool moveOnNoError(toolchain::ErrorOr<T> Val, T &Output) {
  if (!Val)
    return false;
  Output = std::move(*Val);
  return true;
...
getBufferForFileHandlingRemapping(const CompilerInvocation &Invocation,
  ...
  std::unique_ptr<toolchain::MemoryBuffer> BufferOwner;
  auto FileStatus = VFS->status(FilePath);
  if (FileStatus) {
    toolchain::sys::fs::UniqueID MainFileID = FileStatus->getUniqueID();
    ...
    for (const auto &RF : PreprocessorOpts.RemappedFiles) {
      ...
      if (MPathStatus) {
        toolchain::sys::fs::UniqueID MID = MPathStatus->getUniqueID();
        if (MainFileID == MID) {
    ...
    for (const auto &RB : PreprocessorOpts.RemappedFileBuffers) {
      ...
      if (MPathStatus) {
        ...
        if (MainFileID == MID) {
          ...
          BufferOwner.reset();
          Buffer = const_cast<toolchain::MemoryBuffer *>(RB.second);
        }
      }
    }
  }
  ...
  if (!Buffer)
    return nullptr;
  return toolchain::MemoryBuffer::getMemBufferCopy(Buffer->getBuffer(), FilePath);
}
...
void ASTUnit::CacheCodeCompletionResults() {
  ...
  toolchain::DenseMap<CanQualType, unsigned> CompletionTypes;
...
public:
  ...
  bool ReadHeaderSearchOptions(const HeaderSearchOptions &HSOpts,
    ...
    // toolchain::SaveAndRestore doesn't support bit field.
    auto ForceCheckCXX20ModulesInputFiles =
        this->HSOpts.ForceCheckCXX20ModulesInputFiles;
    toolchain::SaveAndRestore X(this->HSOpts.UserEntries);
    toolchain::SaveAndRestore Y(this->HSOpts.SystemHeaderPrefixes);
    toolchain::SaveAndRestore Z(this->HSOpts.VFSOverlayFiles);
...
static bool isInMainFile(const language::Core::Diagnostic &D) {
...
std::unique_ptr<ASTUnit> ASTUnit::LoadFromASTFile(
    ...
    bool UserFilesAreVolatile, IntrusiveRefCntPtr<toolchain::vfs::FileSystem> VFS) {
  std::unique_ptr<ASTUnit> AST(new ASTUnit(true));

  // Recover resources if we crash before exiting this method.
  toolchain::CrashRecoveryContextCleanupRegistrar<ASTUnit>
    ASTUnitCleanup(AST.get());
  toolchain::CrashRecoveryContextCleanupRegistrar<DiagnosticsEngine,
    toolchain::CrashRecoveryContextReleaseRefCleanup<DiagnosticsEngine>>
  ...
  AST->FileMgr = toolchain::makeIntrusiveRefCnt<FileManager>(FileSystemOpts, VFS);
  AST->UserFilesAreVolatile = UserFilesAreVolatile;
  AST->SourceMgr = toolchain::makeIntrusiveRefCnt<SourceManager>(
  ...
  if (ToLoad >= LoadASTOnly)
    AST->Ctx = toolchain::makeIntrusiveRefCnt<ASTContext>(
  ...
  AST->Reader = toolchain::makeIntrusiveRefCnt<ASTReader>(
...
static void AddDefinedMacroToHash(const Token &MacroNameTok, unsigned &Hash) {
  Hash = toolchain::djbHash(MacroNameTok.getIdentifierInfo()->getName(), Hash);
}
...
static void AddTopLevelDeclarationToHash(Decl *D, unsigned &Hash) {
    ...
    if (const auto *EnumD = dyn_cast<EnumDecl>(D)) {
      ...
      if (!EnumD->isScoped()) {
        for (const auto *EI : EnumD->enumerators()) {
          if (EI->getIdentifier())
            Hash = toolchain::djbHash(EI->getIdentifier()->getName(), Hash);
        }
      }
    }

    if (ND->getIdentifier())
      Hash = toolchain::djbHash(ND->getIdentifier()->getName(), Hash);
    else if (DeclarationName Name = ND->getDeclName()) {
      std::string NameStr = Name.getAsString();
      Hash = toolchain::djbHash(NameStr, Hash);
    }
    return;
  }
  ...
  if (const auto *ImportD = dyn_cast<ImportDecl>(D)) {
    if (const Module *Mod = ImportD->getImportedModule()) {
      ...
      Hash = toolchain::djbHash(ModName, Hash);
    }
    return;
  }
}
...
private:
  ...
  toolchain::SmallVector<ASTUnit::StandaloneDiagnostic, 4> PreambleDiags;
};
...
checkAndRemoveNonDriverDiags(SmallVectorImpl<StoredDiagnostic> &StoredDiags) {
  ...
  toolchain::erase_if(StoredDiags, isNonDriverDiag);
}
...
bool ASTUnit::Parse(std::shared_ptr<PCHContainerOperations> PCHContainerOps,
                    std::unique_ptr<toolchain::MemoryBuffer> OverrideMainBuffer,
  ...
  auto CleanOnError = toolchain::make_scope_exit([&]() {
  ...
  // Recover resources if we crash before exiting this method.
  toolchain::CrashRecoveryContextCleanupRegistrar<CompilerInstance>
    CICleanup(Clang.get());
  ...
  SourceMgr = toolchain::makeIntrusiveRefCnt<SourceManager>(
  ...
  // Recover resources if we crash before exiting this method.
  toolchain::CrashRecoveryContextCleanupRegistrar<TopLevelDeclTrackerAction>
    ActCleanup(Act.get());
...
ASTUnit::getMainBufferWithPrecompiledPreamble(
    ...
    IntrusiveRefCntPtr<toolchain::vfs::FileSystem> VFS, bool AllowRebuild,
    unsigned MaxLines) {
  auto MainFilePath =
      PreambleInvocationIn.getFrontendOpts().Inputs[0].getFile();
  std::unique_ptr<toolchain::MemoryBuffer> MainFileBuffer =
  ...
  ASTUnitPreambleCallbacks Callbacks;
  {
    ...
    toolchain::ErrorOr<PrecompiledPreamble> NewPreamble = PrecompiledPreamble::Build(
    ...
    if (NewPreamble) {
      ...
      toolchain_unreachable("unexpected BuildPreambleError");
    }
  }
...
ASTUnit::create(std::shared_ptr<CompilerInvocation> CI,
  ...
  AST->FileMgr =
      toolchain::makeIntrusiveRefCnt<FileManager>(AST->FileSystemOpts, VFS);
  AST->UserFilesAreVolatile = UserFilesAreVolatile;
  AST->SourceMgr = toolchain::makeIntrusiveRefCnt<SourceManager>(
...
bool ASTUnit::LoadFromCompilerInvocation(
  ...
  // Recover resources if we crash before exiting this method.
  toolchain::CrashRecoveryContextCleanupRegistrar<toolchain::MemoryBuffer>
...
std::unique_ptr<ASTUnit> ASTUnit::LoadFromCommandLine(
  ...
  if (!VFS)
    VFS = toolchain::vfs::createPhysicalFileSystem();
...

  {
    ...
    CI = createInvocation(toolchain::ArrayRef(ArgBegin, ArgEnd), std::move(CIOpts));
    if (!CI)
      return nullptr;
  }
...
static void CalculateHiddenNames(const CodeCompletionContext &Context,
                                 ...
                                 unsigned NumResults,
                                 ASTContext &Ctx,
                          toolchain::StringSet<toolchain::BumpPtrAllocator> &HiddenNames){
    ...
    if (!Context.getPreferredType().isNull()) {
      if (C->Kind == CXCursor_MacroDefinition) {
        ...
        if (ExpectedSTC == C->TypeClass) {
          ...
          toolchain::StringMap<unsigned> &CachedCompletionTypes
            = AST.getCachedCompletionTypes();
          toolchain::StringMap<unsigned>::iterator Pos
...
void ASTUnit::CodeComplete(
    ...
    std::shared_ptr<PCHContainerOperations> PCHContainerOps,
    toolchain::IntrusiveRefCntPtr<DiagnosticsEngine> Diag, LangOptions &LangOpts,
    toolchain::IntrusiveRefCntPtr<SourceManager> SourceMgr,
    toolchain::IntrusiveRefCntPtr<FileManager> FileMgr,
    SmallVectorImpl<StoredDiagnostic> &StoredDiagnostics,
    SmallVectorImpl<const toolchain::MemoryBuffer *> &OwnedBuffers,
  ...
  auto getUniqueID =
      [&FileMgr](StringRef Filename) -> std::optional<toolchain::sys::fs::UniqueID> {
...
bool ASTUnit::Save(StringRef File) {
  ...
  if (toolchain::Error Err = toolchain::writeToOutput(
          File, [this](toolchain::raw_ostream &Out) {
            return serialize(Out) ? toolchain::make_error<toolchain::StringError>(
...
void ASTUnit::addFileLevelDecl(Decl *D) {
  ...
  LocDeclsTy::iterator I =
      toolchain::upper_bound(*Decls, LocDecl, toolchain::less_first());
...
void ASTUnit::findFileRegionDecls(FileID File, unsigned Offset, unsigned Length,
  ...
  LocDeclsTy::iterator BeginIt =
      toolchain::partition_point(LocDecls, [=](std::pair<unsigned, Decl *> LD) {
  ...
  LocDeclsTy::iterator EndIt = toolchain::upper_bound(
      LocDecls, std::make_pair(Offset + Length, (Decl *)nullptr),
      toolchain::less_first());
...
ASTUnit::getLocalPreprocessingEntities() const {
  ...
  if (PreprocessingRecord *PPRec = PP->getPreprocessingRecord())
    return toolchain::make_range(PPRec->local_begin(), PPRec->local_end());
