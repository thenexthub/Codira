...
public:
  ...
  void atTemplateBegin(const Sema &TheSema,
    ...
    displayTemplightEntry<true>(toolchain::outs(), TheSema, Inst);
  }

  void atTemplateEnd(const Sema &TheSema,
                     const CodeSynthesisContext &Inst) override {
    displayTemplightEntry<false>(toolchain::outs(), TheSema, Inst);
  }
...
private:
  ...
  static void displayTemplightEntry(toolchain::raw_ostream &Out, const Sema &TheSema,
                                    const CodeSynthesisContext &Inst) {
    std::string YAML;
    {
      toolchain::raw_string_ostream OS(YAML);
      toolchain::yaml::Output YO(OS);
      TemplightEntry Entry =
          getTemplightEntry<BeginInstantiation>(TheSema, Inst);
      toolchain::yaml::EmptyContext Context;
      toolchain::yaml::yamlize(YO, Entry, true, Context);
    }
    Out << "---" << YAML << "\n";
  }
  ...
  static void printEntryName(const Sema &TheSema, const Decl *Entity,
    ...
    toolchain_unreachable("Failed to retrieve a name for this entry!");
  ...
  static TemplightEntry getTemplightEntry(const Sema &TheSema,
    ...
    toolchain::raw_string_ostream OS(Entry.Name);
...
namespace {
  ...
  public:
    DumpModuleInfoListener(toolchain::raw_ostream &Out) : Out(Out) { }
...
void DumpModuleInfoAction::ExecuteAction() {
  ...
  if (!OutputFileName.empty() && OutputFileName != "-") {
    std::error_code EC;
    OutputStream.reset(new toolchain::raw_fd_ostream(
        OutputFileName.str(), EC, toolchain::sys::fs::OF_TextWithCRLF));
  }
  toolchain::raw_ostream &Out = OutputStream ? *OutputStream : toolchain::outs();
  ...
  if (LO.CPlusPlusModules && !LO.CurrentModule.empty()) {
    ...
    if (auto FilteredMacros = toolchain::make_filter_range(
...
void DumpRawTokensAction::ExecuteAction() {
  ...
  toolchain::MemoryBufferRef FromFile = SM.getBufferOrFake(SM.getMainFileID());
...
void PrintPreprocessedAction::ExecuteAction() {
  ...
  if (toolchain::Triple(LLVM_HOST_TRIPLE).isOSWindows()) {
    BinaryMode = true;
...
void PrintPreambleAction::ExecuteAction() {
  ...
  if (Buffer) {
    ...
    toolchain::outs().write((*Buffer)->getBufferStart(), Preamble);
  }
}
...
#define FEATURE(Name, Predicate)                                               \
  ("\t{\"" #Name "\" : " + toolchain::Twine(Predicate ? "true" : "false") + "},\n") \
      .toVector(Str);
...
void PrintDependencyDirectivesSourceMinimizerAction::ExecuteAction() {
  ...
  toolchain::SmallVector<dependency_directives_scan::Token, 16> Tokens;
  toolchain::SmallVector<dependency_directives_scan::Directive, 32> Directives;
