...
retrieveMacroLocation(SourceLocation Loc, FileID MacroFileID,
  ...
  if (SM->isMacroArgExpansion(Loc)) {
    ...
    if (toolchain::binary_search(CommonArgExpansions, MacroFileID))
      MacroRange =
...
static void computeCommonMacroArgExpansionFileIDs(
  ...
  toolchain::sort(BeginArgExpansions);
  toolchain::sort(EndArgExpansions);
...
mapDiagnosticRanges(FullSourceLoc CaretLoc, ArrayRef<CharSourceRange> Ranges,
  ...
  for (const auto &Range : Ranges) {
    ...
    toolchain::SmallDenseMap<FileID, SourceLocation> BeginLocsMap;
...
void DiagnosticRenderer::emitSingleMacroExpansion(
  ...
  toolchain::raw_svector_ostream Message(MessageStorage);
...
rangesInsideSameMacroArgExpansion(FullSourceLoc Loc,
  ...
  unsigned ValidCount =
      toolchain::count_if(Ranges, [](const auto &R) { return R.isValid(); });
