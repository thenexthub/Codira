...
#include "language/Core/Frontend/LayoutOverrideSource.h"
...
#include "language/Core/Sema/HLSLExternalSemaSource.h"
...
public:
  explicit DeserializedDeclsSourceRangePrinter(
      SourceManager &SM, std::unique_ptr<toolchain::raw_fd_ostream> OS)
  ...
  void HandleTranslationUnit(ASTContext &Context) override {
    ...
    toolchain::DenseMap<const FileEntry *, FileData> FileToRanges;
...
private:
  ...
  toolchain::SmallPtrSet<const DeclContext *, 0> ProcessedDeclContexts;
  ...
  static bool shouldIncludeDeclsIn(const DeclContext *DC) {
    ...
    toolchain_unreachable("DeclContext chain must end with a translation unit");
  }

  toolchain::SmallVector<CharSourceRange, 2> getRangesToMark(const Decl *D) {
  ...
  void printJson(toolchain::ArrayRef<RequiredRanges> Result) {
    *OS << "{\n";
...
public:
  ...
  void DeclRead(GlobalDeclID ID, const Decl *D) override {
    toolchain::outs() << "PCH DECL: " << D->getDeclKindName();
    if (const NamedDecl *ND = dyn_cast<NamedDecl>(D)) {
      toolchain::outs() << " - ";
      ND->printQualifiedName(toolchain::outs());
    }
    toolchain::outs() << "\n";
...
FrontendAction::CreateWrappedASTConsumer(CompilerInstance &CI,
  ...
  toolchain::StringRef DumpDeserializedDeclarationRangesPath =
      CI.getFrontendOpts().DumpMinimizationHintsPath;
  if (!DumpDeserializedDeclarationRangesPath.empty()) {
    std::error_code ErrorCode;
    auto FileStream = std::make_unique<toolchain::raw_fd_ostream>(
    ...
    if (!ErrorCode) {
      ...
      toolchain::errs() << "Failed to create output file for "
  ...
  for (const FrontendPluginRegistry::entry &Plugin :
    ...
    if (ActionType == PluginASTAction::CmdlineAfterMainAction ||
      ...
      if (toolchain::is_contained(CI.getFrontendOpts().AddPluginActions,
                             Plugin.getName())) {
...
static std::error_code collectModuleHeaderIncludes(
    ...
    ModuleMap &ModMap, language::Core::Module *Module, SmallVectorImpl<char> &Includes) {
  ...
  if (std::optional<Module::Header> UmbrellaHeader =
    ...
    toolchain::sys::path::native(UmbrellaDir->Entry.getName(), DirNative);

    toolchain::vfs::FileSystem &FS = FileMgr.getVirtualFileSystem();
    SmallVector<std::pair<std::string, FileEntryRef>, 8> Headers;
    for (toolchain::vfs::recursive_directory_iterator Dir(FS, DirNative, EC), End;
      ...
      for (auto It = Components.rbegin(), End = Components.rend(); It != End;
           ++It)
        toolchain::sys::path::append(RelativeHeader, *It);
    ...
    toolchain::sort(Headers, toolchain::less_first());
    for (auto &H : Headers) {
...
static bool loadModuleMapForModuleBuild(CompilerInstance &CI, bool IsSystem,
  ...
  if (HS.getModuleMap().canInferFrameworkModule(ModuleMap->getDir())) {
    ...
    toolchain::sys::path::append(InferredFrameworkPath,
...
bool FrontendAction::BeginSourceFile(CompilerInstance &CI,
  ...
  auto FailureCleanup = toolchain::make_scope_exit([&]() {
  ...
  if (ReplayASTFile) {
    ...
    auto ASTDiags = toolchain::makeIntrusiveRefCnt<DiagnosticsEngine>(
  ...
  if (Input.getKind().getLanguage() == Language::LLVM_IR) {
    if (!hasIRSupport()) {
      CI.getDiagnostics().Report(diag::err_ast_action_on_toolchain_ir)
          << Input.getFile();
      return false;
    }
  ...
  if (!CI.getFrontendOpts().OverrideRecordLayoutsFile.empty() &&
    ...
    auto Override = toolchain::makeIntrusiveRefCnt<LayoutOverrideSource>(
  ...
  if (CI.getLangOpts().HLSL && CI.hasASTContext()) {
    auto HLSLSema = toolchain::makeIntrusiveRefCnt<HLSLExternalSemaSource>();
    if (auto SemaSource = dyn_cast_if_present<ExternalSemaSource>(
            CI.getASTContext().getExternalSourcePtr())) {
      auto MultiSema = toolchain::makeIntrusiveRefCnt<MultiplexExternalSemaSource>(
...
void FrontendAction::EndSourceFile() {
  ...
  if (DisableFree) {
    ...
    toolchain::BuryPointer(CI.takeASTConsumer().get());
  } else {
    CI.setSema(nullptr);
  ...
  if (isCurrentFileAST()) {
    if (DisableFree) {
      ...
      toolchain::BuryPointer(std::move(CurrentASTUnit));
    } else {
...
void ASTFrontendAction::ExecuteAction() {
  ...
  language::Core::noteBottomOfStack();
...
PreprocessorFrontendAction::CreateASTConsumer(CompilerInstance &CI,
                                              StringRef InFile) {
  toolchain_unreachable("Invalid CreateASTConsumer on preprocessor action!");
}
