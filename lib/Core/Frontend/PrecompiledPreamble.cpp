...
createVFSOverlayForPreamblePCH(StringRef PCHFilename,
                               std::unique_ptr<toolchain::MemoryBuffer> PCHBuffer,
                               IntrusiveRefCntPtr<toolchain::vfs::FileSystem> VFS) {
  // We want only the PCH file from the real filesystem to be available,
  // so we create an in-memory VFS with just that and overlay it on top.
  auto PCHFS = toolchain::makeIntrusiveRefCnt<toolchain::vfs::InMemoryFileSystem>();
  PCHFS->addFile(PCHFilename, 0, std::move(PCHBuffer));
  auto Overlay = toolchain::makeIntrusiveRefCnt<toolchain::vfs::OverlayFileSystem>(VFS);
...
class MissingFileCollector : public PPCallbacks {
  toolchain::StringSet<> &Out;
  const HeaderSearch &Search;
  const SourceManager &SM;

public:
  MissingFileCollector(toolchain::StringSet<> &Out, const HeaderSearch &Search,
                       const SourceManager &SM)
  ...
  void InclusionDirective(SourceLocation HashLoc, const Token &IncludeTok,
    ...
    auto NotFoundRelativeTo = [&](DirectoryEntryRef DE) {
      Buf = DE.getName();
      toolchain::sys::path::append(Buf, FileName);
      toolchain::sys::path::remove_dots(Buf, /*remove_dot_dot=*/true);
      Out.insert(Buf);
    };
    ...
    for (const auto &Dir : toolchain::make_range(
...
TemporaryFiles::~TemporaryFiles() {
  ...
  for (const auto &File : Files)
    toolchain::sys::fs::remove(File.getKey());
}
...
public:
  ...
  static std::unique_ptr<TempPCHFile> create(StringRef StoragePath) {
    ...
    toolchain::SmallString<128> File;
    ...
    if (StoragePath.empty())
      EC = toolchain::sys::fs::createTemporaryFile("preamble", "pch", FD, File);
    else {
      toolchain::SmallString<128> TempPath = StoragePath;
      // Use the same filename model as fs::createTemporaryFile().
      toolchain::sys::path::append(TempPath, "preamble-%%%%%%.pch");
  ...
  toolchain::StringRef getFilePath() const { return FilePath; };
...
private:
  ...
  std::unique_ptr<toolchain::raw_pwrite_stream> FileOS; // null if in-memory
  PreambleCallbacks &Callbacks;
};
...
template <class T> bool moveOnNoError(toolchain::ErrorOr<T> Val, T &Output) {
  if (!Val)
    return false;
  Output = std::move(*Val);
  return true;
...
PreambleBounds language::Core::ComputePreambleBounds(const LangOptions &LangOpts,
                                            const toolchain::MemoryBufferRef &Buffer,
                                            unsigned MaxLines) {
...
public:
  ...
  Kind getKind() const {
    ...
    toolchain_unreachable("Neither Memory nor File?");
  }
  toolchain::StringRef filePath() const {
    assert(getKind() == Kind::TempFile);
    return File->getFilePath();
  }
  toolchain::StringRef memoryContents() const {
...
toolchain::ErrorOr<PrecompiledPreamble> PrecompiledPreamble::Build(
    const CompilerInvocation &Invocation,
    const toolchain::MemoryBuffer *MainFileBuffer, PreambleBounds Bounds,
  ...
  toolchain::CrashRecoveryContextCleanupRegistrar<CompilerInstance> CICleanup(
      Clang.get());
  ...
  Clang->setFileManager(
      toolchain::makeIntrusiveRefCnt<FileManager>(Clang->getFileSystemOpts(), VFS));

  // Create the source manager.
  Clang->setSourceManager(toolchain::makeIntrusiveRefCnt<SourceManager>(
  ...
  auto PreambleInputBuffer = toolchain::MemoryBuffer::getMemBufferCopy(
  ...
  toolchain::StringSet<> MissingFiles;
  ...
  toolchain::StringMap<PrecompiledPreamble::PreambleFileHash> FilesInPreamble;
  ...
  for (auto &Filename : PreambleDepCollector->getDependencies()) {
    ...
    if (time_t ModTime = File.getModificationTime()) {
      ...
      toolchain::MemoryBufferRef Buffer =
...
std::size_t PrecompiledPreamble::getSize() const {
  ...
  case PCHStorage::Kind::TempFile: {
    uint64_t Result;
    if (toolchain::sys::fs::file_size(Storage->filePath(), Result))
      return 0;
  ...
  toolchain_unreachable("Unhandled storage kind");
}

bool PrecompiledPreamble::CanReuse(const CompilerInvocation &Invocation,
                                   const toolchain::MemoryBufferRef &MainFileBuffer,
                                   PreambleBounds Bounds,
                                   toolchain::vfs::FileSystem &VFS) const {
  ...
  std::map<toolchain::sys::fs::UniqueID, PreambleFileHash> OverriddenFiles;
  toolchain::StringSet<> OverriddenAbsPaths; // Either by buffers or files.
  for (const auto &R : PreprocessorOpts.RemappedFiles) {
    toolchain::vfs::Status Status;
    ...
    toolchain::SmallString<128> MappedPath(R.first);
    if (!VFS.makeAbsolute(MappedPath))
      OverriddenAbsPaths.insert(MappedPath);

    OverriddenFiles[Status.getUniqueID()] = PreambleFileHash::createForFile(
        Status.getSize(), toolchain::sys::toTimeT(Status.getLastModificationTime()));
  }

  // OverridenFileBuffers tracks only the files not found in VFS.
  toolchain::StringMap<PreambleFileHash> OverridenFileBuffers;
  for (const auto &RB : PreprocessorOpts.RemappedFileBuffers) {
    ...
    toolchain::SmallString<128> MappedPath(RB.first);
    if (!VFS.makeAbsolute(MappedPath))
      OverriddenAbsPaths.insert(MappedPath);
  }
  ...
  for (const auto &F : FilesInPreamble) {
    ...
    std::map<toolchain::sys::fs::UniqueID, PreambleFileHash>::iterator Overridden =
    ...
    if (Status.getSize() != uint64_t(F.second.Size) ||
        toolchain::sys::toTimeT(Status.getLastModificationTime()) !=
            F.second.ModTime)
      return false;
  }
...
void PrecompiledPreamble::AddImplicitPreamble(
    CompilerInvocation &CI, IntrusiveRefCntPtr<toolchain::vfs::FileSystem> &VFS,
    toolchain::MemoryBuffer *MainFileBuffer) const {
...
PrecompiledPreamble::PrecompiledPreamble(
    ...
    toolchain::StringMap<PreambleFileHash> FilesInPreamble,
    toolchain::StringSet<> MissingFiles)
...
void PrecompiledPreamble::setupPreambleStorage(
  ...
  if (Storage.getKind() == PCHStorage::Kind::TempFile) {
    toolchain::StringRef PCHPath = Storage.filePath();
    PreprocessorOpts.ImplicitPCHInclude = PCHPath.str();

    // Make sure we can access the PCH file even if we're using a VFS
    IntrusiveRefCntPtr<toolchain::vfs::FileSystem> RealFS =
    ...
    auto Buf = toolchain::MemoryBuffer::getMemBuffer(
...
static toolchain::ManagedStatic<BuildPreambleErrorCategory> BuildPreambleErrCategory;

std::error_code language::Core::make_error_code(BuildPreambleError Error) {
