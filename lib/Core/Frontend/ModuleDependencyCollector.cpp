...
struct ModuleDependencyMMCallbacks : public ModuleMapCallbacks {
  ...
  void moduleMapAddHeader(StringRef HeaderPath) override {
    if (toolchain::sys::path::is_absolute(HeaderPath))
...
static bool isCaseSensitivePath(StringRef Path) {
  ...
  if (toolchain::sys::fs::real_path(Path, TmpDest))
  ...
  if (!toolchain::sys::fs::real_path(UpperDest, RealDest) && Path == RealDest)
    return false;
  return true;
}
...
void ModuleDependencyCollector::writeFileMap() {
  ...
  toolchain::sys::path::append(YAMLPath, "vfs.yaml");
  toolchain::raw_fd_ostream OS(YAMLPath, EC, toolchain::sys::fs::OF_TextWithCRLF);
  if (EC) {
    HasErrors = true;
    return;
  }
...
std::error_code ModuleDependencyCollector::copyToRoot(StringRef Src,
  ...
  toolchain::FileCollector::PathCanonicalizer::PathStorage Paths =
