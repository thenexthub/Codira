...
struct DepCollectorPPCallbacks : public PPCallbacks {
  ...
  void LexedFileChanged(FileID FID, LexedFileChangeReason Reason,
    ...
    if (std::optional<StringRef> Filename =
      ...
      DepCollector.maybeAddDependency(
          toolchain::sys::path::remove_leading_dotslash(*Filename),
  ...
  void FileSkipped(const FileEntryRef &SkippedFile, const Token &FilenameTok,
    ...
    StringRef Filename =
        toolchain::sys::path::remove_leading_dotslash(SkippedFile.getName());
  ...
  void EmbedDirective(SourceLocation, StringRef, bool,
    ...
    StringRef FileName =
        toolchain::sys::path::remove_leading_dotslash(File->getName());
...
bool DependencyCollector::addDependency(StringRef Filename) {
  ...
  toolchain::SmallString<256> TmpPath = Filename;
  toolchain::sys::path::native(TmpPath);
...
static void PrintFilename(raw_ostream &OS, StringRef Filename,
  ...
  toolchain::SmallString<256> NativePath;
  toolchain::sys::path::native(Filename.str(), NativePath);
...
void DependencyFileGenerator::outputDependencyFile(DiagnosticsEngine &Diags) {
  if (SeenMissingHeader) {
    toolchain::sys::fs::remove(OutputFile);
    return;
  }
...
void DependencyFileGenerator::outputDependencyFile(toolchain::raw_ostream &OS) {
