...
#include "language/Core/Frontend/ChainedDiagnosticConsumer.h"
...
#include "language/Core/Frontend/FrontendPluginRegistry.h"
#include "language/Core/Frontend/LogDiagnosticPrinter.h"
#include "language/Core/Frontend/SARIFDiagnosticPrinter.h"
#include "language/Core/Frontend/SerializedDiagnosticPrinter.h"
#include "language/Core/Frontend/TextDiagnosticPrinter.h"
#include "language/Core/Frontend/Utils.h"
#include "language/Core/Frontend/VerifyDiagnosticConsumer.h"
...
void CompilerInstance::setDiagnostics(
    toolchain::IntrusiveRefCntPtr<DiagnosticsEngine> Value) {
...
bool CompilerInstance::createTarget() {
  ...
  if (!getAuxTarget() &&
    ...
    TO->Triple = toolchain::Triple::normalize(getFrontendOpts().AuxTriple);
...
toolchain::vfs::FileSystem &CompilerInstance::getVirtualFileSystem() const {
...
void CompilerInstance::setFileManager(
    toolchain::IntrusiveRefCntPtr<FileManager> Value) {
  FileMgr = std::move(Value);
}

void CompilerInstance::setSourceManager(
    toolchain::IntrusiveRefCntPtr<SourceManager> Value) {
...
void CompilerInstance::setASTContext(
    toolchain::IntrusiveRefCntPtr<ASTContext> Value) {
...
static void collectIncludePCH(CompilerInstance &CI,
  ...
  toolchain::sys::path::native(PCHDir->getName(), DirNative);
  toolchain::vfs::FileSystem &FS = FileMgr.getVirtualFileSystem();
  SimpleASTReaderListener Validator(CI.getPreprocessor());
  for (toolchain::vfs::directory_iterator Dir = FS.dir_begin(DirNative, EC), DirEnd;
...
static void collectVFSEntries(CompilerInstance &CI,
  ...
  SmallVector<toolchain::vfs::YAMLVFSEntry, 16> VFSEntries;
  for (const std::string &VFSFile : CI.getHeaderSearchOpts().VFSOverlayFiles) {
    toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> Buffer =
        toolchain::MemoryBuffer::getFile(VFSFile);
    if (!Buffer)
      return;
    toolchain::vfs::collectVFSFromYAML(std::move(Buffer.get()),
...
static void SetUpDiagnosticLog(DiagnosticOptions &DiagOpts,
  ...
  if (DiagOpts.DiagnosticLogFile != "-") {
    ...
    auto FileOS = std::make_unique<toolchain::raw_fd_ostream>(
...
static void SetupSerializedDiagnostics(DiagnosticOptions &DiagOpts,
  ...
  auto SerializedConsumer =
      language::Core::serialized_diags::create(OutputFile, DiagOpts);
...
void CompilerInstance::createDiagnostics(toolchain::vfs::FileSystem &VFS,
...
IntrusiveRefCntPtr<DiagnosticsEngine> CompilerInstance::createDiagnostics(
    toolchain::vfs::FileSystem &VFS, DiagnosticOptions &Opts,
  ...
  if (Client) {
    ...
    Diags->setClient(new SARIFDiagnosticPrinter(toolchain::errs(), Opts));
  } else
...
FileManager *CompilerInstance::createFileManager(
  ...
  if (getFrontendOpts().ShowStats)
    VFS =
        toolchain::makeIntrusiveRefCnt<toolchain::vfs::TracingFileSystem>(std::move(VFS));
  FileMgr = toolchain::makeIntrusiveRefCnt<FileManager>(getFileSystemOpts(),
                                                   std::move(VFS));
  return FileMgr.get();
}
...
void CompilerInstance::createSourceManager(FileManager &FileMgr) {
  SourceMgr =
      toolchain::makeIntrusiveRefCnt<SourceManager>(getDiagnostics(), FileMgr);
}
  ...
  for (const auto &RB : InitOpts.RemappedFileBuffers) {
    ...
    if (InitOpts.RetainRemappedFileBuffers)
      ...
      SourceMgr.overrideFileContents(
          FromFile, std::unique_ptr<toolchain::MemoryBuffer>(RB.second));
  }
...
void CompilerInstance::createPreprocessor(TranslationUnitKind TUKind) {
  ...
  const toolchain::Triple *HeaderSearchTriple = &PP->getTargetInfo().getTriple();
  if (PP->getTargetInfo().getTriple().getOS() == toolchain::Triple::CUDA &&
      PP->getAuxTargetInfo())
...
std::string CompilerInstance::getSpecificModuleCachePath(StringRef ModuleHash) {
  ...
  if (!SpecificModuleCache.empty() && !getHeaderSearchOpts().DisableModuleHash)
    toolchain::sys::path::append(SpecificModuleCache, ModuleHash);
...
void CompilerInstance::createASTContext() {
  ...
  auto Context = toolchain::makeIntrusiveRefCnt<ASTContext>(
...
struct ReadModuleNames : ASTReaderListener {
  Preprocessor &PP;
  toolchain::SmallVector<std::string, 8> LoadedModules;
    ...
    for (const std::string &LoadedModule : LoadedModules) {
      if (Module *M = PP.getHeaderSearchInfo().getModuleMap().findOrLoadModule(
        ...
        while (!Stack.empty()) {
          ...
          toolchain::append_range(Stack, SubmodulesRange);
        }
      }
    }
    LoadedModules.clear();
...
void CompilerInstance::createCodeCompletionConsumer() {
  ...
  if (!CompletionConsumer) {
    setCodeCompletionConsumer(createCodeCompletionConsumer(
        ...
        getFrontendOpts().CodeCompleteOpts, toolchain::outs()));
    return;
...
void CompilerInstance::createFrontendTimer() {
  timerGroup.reset(new toolchain::TimerGroup("clang", "Clang time report"));
  FrontendTimer.reset(new toolchain::Timer("frontend", "Front end", *timerGroup));
}
...
void CompilerInstance::clearOutputFiles(bool EraseFiles) {
  ...
  for (OutputFile &OF : OutputFiles) {
    ...
    toolchain::Error E = OF.File->keep(OF.Filename);
    if (!E)
      continue;
    ...
    toolchain::sys::fs::remove(OF.File->TmpName);
  }
  OutputFiles.clear();
  if (DeleteBuiltModules) {
    for (auto &Module : BuiltModules)
      toolchain::sys::fs::remove(Module.second);
    BuiltModules.clear();
  }
}
...
std::unique_ptr<raw_pwrite_stream> CompilerInstance::createDefaultOutputFile(
  ...
  if (OutputPath.empty()) {
    if (InFile == "-" || Extension.empty()) {
      ...
      toolchain::sys::path::replace_extension(*PathStorage, Extension);
...
std::unique_ptr<raw_pwrite_stream> CompilerInstance::createNullOutputFile() {
  return std::make_unique<toolchain::raw_null_ostream>();
}
...
Expected<std::unique_ptr<toolchain::raw_pwrite_stream>>
CompilerInstance::createOutputFileImpl(StringRef OutputPath, bool Binary,
  ...
  if (OutputPath != "-" && !toolchain::sys::path::is_absolute(OutputPath)) {
  ...
  if (UseTemporary) {
    if (OutputPath == "-")
      ...
      toolchain::sys::fs::status(OutputPath, Status);
      if (toolchain::sys::fs::exists(Status)) {
        // Fail early if we can't write to the final destination.
        if (!toolchain::sys::fs::can_write(OutputPath))
          return toolchain::errorCodeToError(
              make_error_code(toolchain::errc::operation_not_permitted));

        // Don't use a temporary if the output is a special file. This handles
        // things like '-o /dev/null'
        if (!toolchain::sys::fs::is_regular_file(Status))
          UseTemporary = false;
      }
    }
  }

  std::optional<toolchain::sys::fs::TempFile> Temp;
  if (UseTemporary) {
    // Create a temporary file.
    // Insert -%%%%%%%% before the extension (if any), and because some tools
    // (noticeable, clang's own GlobalModuleIndex.cpp) glob for build
    // artifacts, also append .tmp.
    StringRef OutputExtension = toolchain::sys::path::extension(OutputPath);
    SmallString<128> TempPath =
        StringRef(OutputPath).drop_back(OutputExtension.size());
    TempPath += "-%%%%%%%%";
    TempPath += OutputExtension;
    TempPath += ".tmp";
    toolchain::sys::fs::OpenFlags BinaryFlags =
        Binary ? toolchain::sys::fs::OF_None : toolchain::sys::fs::OF_Text;
    Expected<toolchain::sys::fs::TempFile> ExpectedFile =
        toolchain::sys::fs::TempFile::create(
            TempPath, toolchain::sys::fs::all_read | toolchain::sys::fs::all_write,
            BinaryFlags);

    toolchain::Error E = handleErrors(
        ExpectedFile.takeError(), [&](const toolchain::ECError &E) -> toolchain::Error {
          std::error_code EC = E.convertToErrorCode();
          if (CreateMissingDirectories &&
              EC == toolchain::errc::no_such_file_or_directory) {
            StringRef Parent = toolchain::sys::path::parent_path(OutputPath);
            EC = toolchain::sys::fs::create_directories(Parent);
            if (!EC) {
              ExpectedFile = toolchain::sys::fs::TempFile::create(
                  TempPath, toolchain::sys::fs::all_read | toolchain::sys::fs::all_write,
                  BinaryFlags);
              if (!ExpectedFile)
                return toolchain::errorCodeToError(
                    toolchain::errc::no_such_file_or_directory);
            }
          }
    ...
    if (E) {
      ...
      OS.reset(new toolchain::raw_fd_ostream(Temp->FD, /*shouldClose=*/false));
      OSFile = Temp->TmpName;
    }
...
bool CompilerInstance::InitializeSourceManager(const FrontendInputFile &Input,
  ...
  if (!FileOrErr) {
    auto EC = toolchain::errorToErrorCode(FileOrErr.takeError());
    if (InputFile != "-")
...
bool CompilerInstance::ExecuteAction(FrontendAction &Act) {
  ...
  auto FinishDiagnosticClient = toolchain::make_scope_exit([&]() {
  ...
  if (getHeaderSearchOpts().Verbose)
    OS << "clang -cc1 version " CLANG_VERSION_STRING << " based upon LLVM "
       ...
       << toolchain::sys::getDefaultTargetTriple() << "\n";

  if (getFrontendOpts().ShowStats || !getFrontendOpts().StatsFile.empty())
    toolchain::EnableStatistics(false);

  // Sort vectors containing toc data and no toc data variables to facilitate
  // binary search later.
  toolchain::sort(getCodeGenOpts().TocDataVarsUserSpecified);
  toolchain::sort(getCodeGenOpts().NoTocDataVars);
  ...
  if (getFrontendOpts().ShowStats) {
    ...
    toolchain::PrintStatistics(OS);
  }
  StringRef StatsFile = getFrontendOpts().StatsFile;
  if (!StatsFile.empty()) {
    toolchain::sys::fs::OpenFlags FileFlags = toolchain::sys::fs::OF_TextWithCRLF;
    if (getFrontendOpts().AppendStats)
      FileFlags |= toolchain::sys::fs::OF_Append;
    std::error_code EC;
    auto StatS =
        std::make_unique<toolchain::raw_fd_ostream>(StatsFile, EC, FileFlags);
    if (EC) {
      getDiagnostics().Report(diag::warn_fe_unable_to_open_stats_file)
          << StatsFile << EC.message();
    } else {
      toolchain::PrintStatisticsJSON(*StatS);
    }
  }
...
void CompilerInstance::LoadRequestedPlugins() {
  ...
  for (const std::string &Path : getFrontendOpts().Plugins) {
    std::string Error;
    if (toolchain::sys::DynamicLibrary::LoadLibraryPermanently(Path.c_str(), &Error))
  ...
  for (const FrontendPluginRegistry::entry &Plugin :
    ...
    if (P->getActionType() == PluginASTAction::ReplaceAction) {
      getFrontendOpts().ProgramAction = language::Core::frontend::PluginAction;
...
std::unique_ptr<CompilerInstance> CompilerInstance::cloneForModuleCompileImpl(
  ...
  toolchain::erase_if(PPOpts.Macros,
  ...
  if (toolchain::is_contained(DiagOpts.SystemHeaderWarningsModules, ModuleName))
...
bool CompilerInstance::compileModule(SourceLocation ImportLoc,
  ...
  toolchain::TimeTraceScope TimeScope("Module Compile", ModuleName);
  ...
  bool Crashed = !toolchain::CrashRecoveryContext().RunSafelyOnNewStack(
      [&]() {
...
static OptionalFileEntryRef getPublicModuleMap(FileEntryRef File,
                                               FileManager &FileMgr) {
  StringRef Filename = toolchain::sys::path::filename(File.getName());
  SmallString<128> PublicFilename(File.getDir().getName());
  if (Filename == "module_private.map")
    toolchain::sys::path::append(PublicFilename, "module.map");
  else if (Filename == "module.private.modulemap")
    toolchain::sys::path::append(PublicFilename, "module.modulemap");
  else
    return std::nullopt;
...
std::unique_ptr<CompilerInstance> CompilerInstance::cloneForModuleCompile(
  ...
  toolchain::sys::path::append(FakeModuleMapFile, "__inferred_module.map");

  std::string InferredModuleMapContent;
  toolchain::raw_string_ostream OS(InferredModuleMapContent);
  Module->print(OS);
  ...
  std::unique_ptr<toolchain::MemoryBuffer> ModuleMapBuffer =
      toolchain::MemoryBuffer::getMemBufferCopy(InferredModuleMapContent);
...
static void checkConfigMacros(Preprocessor &PP, Module *M,
                              SourceLocation ImportLoc) {
  language::Core::Module *TopModule = M->getTopLevelModule();
...
static void writeTimestampFile(StringRef TimestampFile) {
  std::error_code EC;
  toolchain::raw_fd_ostream Out(TimestampFile.str(), EC, toolchain::sys::fs::OF_None);
}

/// Prune the module cache of modules that haven't been accessed in
/// a long time.
static void pruneModuleCache(const HeaderSearchOptions &HSOpts) {
  toolchain::sys::fs::file_status StatBuf;
  toolchain::SmallString<128> TimestampFile;
  TimestampFile = HSOpts.ModuleCachePath;
  assert(!TimestampFile.empty());
  toolchain::sys::path::append(TimestampFile, "modules.timestamp");

  // Try to stat() the timestamp file.
  if (std::error_code EC = toolchain::sys::fs::status(TimestampFile, StatBuf)) {
  ...
  time_t TimeStampModTime =
      toolchain::sys::toTimeT(StatBuf.getLastModificationTime());
  ...
  // indices.
  std::error_code EC;
  for (toolchain::sys::fs::directory_iterator Dir(HSOpts.ModuleCachePath, EC),
       DirEnd;
       Dir != DirEnd && !EC; Dir.increment(EC)) {
    // If we don't have a directory, there's nothing to look into.
    if (!toolchain::sys::fs::is_directory(Dir->path()))
      continue;

    // Walk all of the files within this directory.
    for (toolchain::sys::fs::directory_iterator File(Dir->path(), EC), FileEnd;
         File != FileEnd && !EC; File.increment(EC)) {
      // We only care about module and global module index files.
      StringRef Extension = toolchain::sys::path::extension(File->path());
      if (Extension != ".pcm" && Extension != ".timestamp" &&
          toolchain::sys::path::filename(File->path()) != "modules.idx")
        continue;

      // Look at this file. If we can't stat it, there's nothing interesting
      // there.
      if (toolchain::sys::fs::status(File->path(), StatBuf))
        continue;

      // If the file has been used recently enough, leave it there.
      time_t FileAccessTime = toolchain::sys::toTimeT(StatBuf.getLastAccessedTime());
      ...
      // Remove the file.
      toolchain::sys::fs::remove(File->path());

      // Remove the timestamp file.
      std::string TimpestampFilename = File->path() + ".timestamp";
      toolchain::sys::fs::remove(TimpestampFilename);
    }

    // If we removed all of the files in the directory, remove the directory
    // itself.
    if (toolchain::sys::fs::directory_iterator(Dir->path(), EC) ==
            toolchain::sys::fs::directory_iterator() && !EC)
      toolchain::sys::fs::remove(Dir->path());
  }
}
...
void CompilerInstance::createASTReader() {
  ...
  if (timerGroup)
    ReadTimer = std::make_unique<toolchain::Timer>("reading_modules",
                                              "Reading modules", *timerGroup);
  TheASTReader = toolchain::makeIntrusiveRefCnt<ASTReader>(
...
bool CompilerInstance::loadModuleFile(
  ...
  toolchain::Timer Timer;
  if (timerGroup)
    Timer.init("preloading." + FileName.str(), "Preloading " + FileName.str(),
               *timerGroup);
  toolchain::TimeRegion TimeLoading(timerGroup ? &Timer : nullptr);
...
ModuleLoadResult CompilerInstance::findOrCompileModuleAndReadAST(
  ...
  if (timerGroup)
    Timer.init("loading." + ModuleFilename, "Loading " + ModuleFilename,
               *timerGroup);
  toolchain::TimeRegion TimeLoading(timerGroup ? &Timer : nullptr);
  toolchain::TimeTraceScope TimeScope("Module Load", ModuleName);
...
CompilerInstance::loadModule(SourceLocation ImportLoc,
  ...
  for (unsigned I = 1, N = Path.size(); I != N; ++I) {
    ...
    language::Core::Module *Sub = Module->findSubmodule(Name);
...
void CompilerInstance::createModuleFromSource(SourceLocation ImportLoc,
  ...
  Other->getSourceManager().overrideFileContents(
      ModuleMapFile, toolchain::MemoryBuffer::getMemBuffer(NullTerminatedSource));
  ...
  if (Success) {
    ...
    toolchain::sys::RemoveFileOnSignal(ModuleFileName);
  }
}
...
GlobalModuleIndex *CompilerInstance::loadGlobalModuleIndex(
  ...
  if (!GlobalIndex && shouldBuildGlobalModuleIndex() && hasFileManager() &&
      hasPreprocessor()) {
    toolchain::sys::fs::create_directories(
      getPreprocessor().getHeaderSearchInfo().getModuleCachePath());
    if (toolchain::Error Err = GlobalModuleIndex::writeIndex(
...
void CompilerInstance::resetAndLeakSema() { toolchain::BuryPointer(takeSema()); }
