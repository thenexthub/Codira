...
printableTextForNextCharacter(StringRef SourceLine, size_t *I,
  ...
  if (*Begin < 0x80 && toolchain::sys::locale::isPrint(*Begin)) {
    ++(*I);
    return std::make_pair(SmallString<16>(Begin, Begin + 1), true);
  }
  unsigned CharSize = toolchain::getNumBytesForUTF8(*Begin);
  const unsigned char *End = Begin + CharSize;

  // Convert it to UTF32 and check if it's printable.
  if (End <= SourceLine.bytes_end() && toolchain::isLegalUTF8Sequence(Begin, End)) {
    toolchain::UTF32 C;
    toolchain::UTF32 *CPtr = &C;
    ...
    toolchain::ConversionResult Res = toolchain::ConvertUTF8toUTF32(
        &Begin, End, &CPtr, CPtr + 1, toolchain::strictConversion);
    (void)Res;
    assert(Res == toolchain::conversionOK);
    ...
    if (toolchain::sys::locale::isPrint(C))
      return std::make_pair(SmallString<16>(OriginalBegin, End), true);

    // Valid but not printable.
    SmallString<16> Str("<U+>");
    while (C) {
      Str.insert(Str.begin() + 3, toolchain::hexdigit(C % 16));
      C /= 16;
    }
    while (Str.size() < 8)
      Str.insert(Str.begin() + 3, toolchain::hexdigit(0));
  ...
  ExpandedByte[1] = toolchain::hexdigit(Byte / 16);
  ExpandedByte[2] = toolchain::hexdigit(Byte % 16);
  ++(*I);
...
static void genColumnByteMapping(StringRef SourceLine, unsigned TabStop,
  ...
  while (I < SourceLine.size()) {
    ...
    Columns += toolchain::sys::locale::columnWidth(Str);
  }
...
static void selectInterestingSourceRegion(std::string &SourceLine,
  ...
  unsigned FixItColumns = toolchain::sys::locale::columnWidth(FixItInsertionLine);
  ...
  assert(toolchain::none_of(CaretLine, [](char c) { return c < ' ' || '~' < c; }));
  ...
  if (!FixItInsertionLine.empty()) {
    ...
    unsigned FixItEndCol
      = toolchain::sys::locale::columnWidth(FixItInsertionLine.substr(0, FixItEnd));
...
static std::string buildFixItInsertionLine(FileID FID, unsigned LineNo,
  ...
  for (const auto &H : Hints) {
    ...
    if (FID == HintLocInfo.first &&
      ...
      PrevHintEndCol = HintCol + toolchain::sys::locale::columnWidth(H.CodeToInsert);
    }
  }
...
/// line.
static std::unique_ptr<toolchain::SmallVector<TextDiagnostic::StyleRange>[]>
highlightLines(StringRef FileData, unsigned StartLineNumber,
  ...
  auto Buff = toolchain::MemoryBuffer::getMemBuffer(FileData);
  ...
  auto appendStyle =
    ...
    if (T.is(tok::raw_identifier)) {
      ...
      // list.
      if (toolchain::StringSwitch<bool>(RawIdent)
              .Case("true", true)
              .Case("false", true)
              .Case("nullptr", true)
              .Case("__func__", true)
...
void TextDiagnostic::emitSnippet(StringRef SourceLine,
  ...
  std::optional<toolchain::raw_ostream::Colors> CurrentColor;
  size_t I = 0;
  while (I < SourceLine.size()) {
    ...
    if (DiagOpts.ShowColors) {
      ...
      const auto *CharStyle = toolchain::find_if(Styles, [I](const StyleRange &R) {
