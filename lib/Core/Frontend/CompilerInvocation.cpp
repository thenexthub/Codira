...
#include "language/Core/Frontend/CommandLineSourceLoc.h"
...
#include "toolchain/Remarks/HotnessThresholdParser.h"
...
static toolchain::StringRef lookupStrInTable(unsigned Offset) {
...
template <typename T> static constexpr bool is_uint64_t_convertible() {
  return !std::is_same_v<T, uint64_t> && toolchain::is_integral_or_enum<T>::value;
}
...
static void denormalizeStringImpl(ArgumentConsumer Consumer,
  ...
  case Option::CommaJoinedClass:
    ...
    break;
  default:
    toolchain_unreachable("Cannot denormalize an option with option class "
...
static void denormalizeSimpleEnumImpl(ArgumentConsumer Consumer,
  ...
  if (auto MaybeEnumVal = findValueTableByValue(Table, Value)) {
    ...
    toolchain_unreachable("The simple enum value was not correctly defined in "
...
static void denormalizeStringVector(ArgumentConsumer Consumer,
  ...
  case Option::CommaJoinedClass: {
    ...
    if (!Values.empty()) {
      ...
      for (const std::string &Value : toolchain::drop_begin(Values, 1)) {
...
static std::optional<std::string> normalizeTriple(OptSpecifier Opt,
  ...
  if (!Arg)
    return std::nullopt;
  return toolchain::Triple::normalize(Arg->getValue());
}
...
static bool FixupInvocation(CompilerInvocation &Invocation,
  ...
  toolchain::Triple T(TargetOpts.Triple);
  toolchain::Triple::ArchType Arch = T.getArch();
  ...
  if (LangOpts.NewAlignOverride &&
      !toolchain::isPowerOf2_32(LangOpts.NewAlignOverride)) {
  ...
  if (Arg *A = Args.getLastArg(OPT_fdefault_calling_conv_EQ)) {
    ...
    bool emitError = (DefaultCC == LangOptions::DCC_FastCall ||
                     ...
                     Arch != toolchain::Triple::x86;
    emitError |= (DefaultCC == LangOptions::DCC_VectorCall ||
                  DefaultCC == LangOptions::DCC_RegCall) &&
                 !T.isX86();
    emitError |= DefaultCC == LangOptions::DCC_RtdCall && Arch != toolchain::Triple::m68k;
    if (emitError)
      Diags.Report(diag::err_drv_argument_not_allowed_with)
...
static void GenerateArg(ArgumentConsumer Consumer,
                        toolchain::opt::OptSpecifier OptSpecifier) {
...
using ParseFn =
    toolchain::function_ref<bool(CompilerInvocation &, ArrayRef<const char *>,
                            DiagnosticsEngine &, const char *)>;

// Generate command line arguments from CompilerInvocation.
using GenerateFn = toolchain::function_ref<void(
...
static bool RoundTrip(ParseFn Parse, GenerateFn Generate,
  ...
  auto SerializeArgs = [](ArrayRef<const char *> Args) {
    ...
    for (const char *Arg : Args) {
      toolchain::sys::printArg(OS, Arg, /*Quote=*/true);
      OS << ' ';
    }
    return Buffer;
  };
  ...
  toolchain::StringSaver StringPool(Alloc);
...
static void getAllNoBuiltinFuncValues(ArgList &Args,
  ...
  auto BuiltinEnd = toolchain::partition(Values, Builtin::Context::isBuiltinFunc);
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
    default:
      toolchain_unreachable("Tried to generate unknown analysis constraint.");
    }
  }
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
    default:
      toolchain_unreachable("Tried to generate unknown analysis diagnostic client.");
    }
  }
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
    default:
      toolchain_unreachable("Tried to generate unknown analysis purge mode.");
    }
  }
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
    default:
      toolchain_unreachable("Tried to generate unknown analysis inlining mode.");
    }
  }
  ...
  toolchain::sort(SortedConfigOpts, toolchain::less_first());
...
#undef ANALYZER_OPTION_WITH_MARSHALLING
  ...
  if (Arg *A = Args.getLastArg(OPT_analyzer_constraints)) {
    ...
    AnalysisConstraints Value = toolchain::StringSwitch<AnalysisConstraints>(Name)
#define ANALYSIS_CONSTRAINTS(NAME, CMDFLAG, DESC, CREATFN) \
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
  ...
  if (Arg *A = Args.getLastArg(OPT_analyzer_output)) {
    StringRef Name = A->getValue();
    AnalysisDiagClients Value = toolchain::StringSwitch<AnalysisDiagClients>(Name)
#define ANALYSIS_DIAGNOSTICS(NAME, CMDFLAG, DESC, CREATFN) \
...
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
  ...
  if (Arg *A = Args.getLastArg(OPT_analyzer_purge)) {
    StringRef Name = A->getValue();
    AnalysisPurgeMode Value = toolchain::StringSwitch<AnalysisPurgeMode>(Name)
#define ANALYSIS_PURGE(NAME, CMDFLAG, DESC) \
      .Case(CMDFLAG, NAME)
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
  ...
  if (Arg *A = Args.getLastArg(OPT_analyzer_inlining_mode)) {
    StringRef Name = A->getValue();
    AnalysisInliningMode Value = toolchain::StringSwitch<AnalysisInliningMode>(Name)
#define ANALYSIS_INLINING_MODE(NAME, CMDFLAG, DESC) \
      .Case(CMDFLAG, NAME)
#include "language/Core/StaticAnalyzer/Core/Analyses.def"
  ...
  toolchain::raw_string_ostream os(Opts.FullCompilerInvocation);
...
#include "language/Core/StaticAnalyzer/Core/AnalyzerOptions.def"
    ...
    for (const StringRef &CheckerOrPackage : CheckersAndPackages) {
      if (Diags) {
        ...
        bool IsValidName = IsChecker
                               ? toolchain::is_contained(Checkers, CheckerOrPackage)
                               : toolchain::is_contained(Packages, CheckerOrPackage);
  ...
  if (!AnOpts.CTUDir.empty() && !toolchain::sys::fs::is_directory(AnOpts.CTUDir))
    Diags->Report(diag::err_analyzer_config_invalid_input) << "ctu-dir"
                                                           << "a filename";

  if (!AnOpts.ModelPath.empty() &&
      !toolchain::sys::fs::is_directory(AnOpts.ModelPath))
...
static bool parseDiagnosticLevelMask(StringRef FlagName,
  ...
  for (const auto &Level : Levels) {
    DiagnosticLevelMask const PM =
      toolchain::StringSwitch<DiagnosticLevelMask>(Level)
...
static void parseXRayInstrumentationBundle(StringRef FlagName, StringRef Bundle,
                                           ...
                                           XRayInstrSet &S) {
  toolchain::SmallVector<StringRef, 2> BundleParts;
  toolchain::SplitString(Bundle, BundleParts, ",");
...
static std::string serializeXRayInstrumentationBundle(const XRayInstrSet &S) {
  ...
  std::string Buffer;
  toolchain::raw_string_ostream OS(Buffer);
  toolchain::interleave(BundleParts, OS, [&OS](StringRef Part) { OS << Part; }, ",");
  return Buffer;
}
...
static void setPGOUseInstrumentor(CodeGenOptions &Opts,
                                  ...
                                  DiagnosticsEngine &Diags) {
  auto ReaderOrErr = toolchain::IndexedInstrProfReader::create(ProfileName, FS);
  if (auto E = ReaderOrErr.takeError()) {
    unsigned DiagID = Diags.getCustomDiagID(DiagnosticsEngine::Error,
                                            "Error in reading profile %0: %1");
    toolchain::handleAllErrors(std::move(E), [&](const toolchain::ErrorInfoBase &EI) {
  ...
  if (PGOReader->isIRLevelProfile() || PGOReader->hasMemoryProfile()) {
    if (PGOReader->hasCSIRLevelProfile())
      Opts.setProfileUse(toolchain::driver::ProfileInstrKind::ProfileCSIRInstr);
    else
      Opts.setProfileUse(toolchain::driver::ProfileInstrKind::ProfileIRInstr);
  } else
    Opts.setProfileUse(toolchain::driver::ProfileInstrKind::ProfileClangInstr);
}

void CompilerInvocation::setDefaultPointerAuthOptions(
    PointerAuthOptions &Opts, const LangOptions &LangOpts,
    const toolchain::Triple &Triple) {
  assert(Triple.getArch() == toolchain::Triple::aarch64);
...
#undef CODEGEN_OPTION_WITH_MARSHALLING
  ...
  case toolchain::codegenoptions::UnusedTypeInfo:
    ...
    break;
  case toolchain::codegenoptions::NoDebugInfo: // default value
    DebugInfoVal = std::nullopt;
    break;
  case toolchain::codegenoptions::LocTrackingOnly: // implied value
    DebugInfoVal = std::nullopt;
    break;
  }
  if (DebugInfoVal)
  ...
  if (Opts.DebugNameTable ==
      static_cast<unsigned>(toolchain::DICompileUnit::DebugNameTableKind::GNU))
  ...
  if (TNK != toolchain::codegenoptions::DebugTemplateNamesKind::Full) {
    if (TNK == toolchain::codegenoptions::DebugTemplateNamesKind::Simple)
      GenerateArg(Consumer, OPT_gsimple_template_names_EQ, "simple");
    else if (TNK == toolchain::codegenoptions::DebugTemplateNamesKind::Mangled)
...
#include "language/Core/Basic/CFProtectionOptions.def"
  ...
  for (const auto &F : Opts.LinkBitcodeFiles) {
    bool Builtint = F.LinkFlags == toolchain::Linker::Flags::LinkOnlyNeeded &&
...
#undef CODEGEN_OPTION_WITH_MARSHALLING
  ...
  if (Arg *A = Args.getLastArg(OPT_debug_info_kind_EQ)) {
    unsigned Val =
        toolchain::StringSwitch<unsigned>(A->getValue())
    ...
    if (Val == ~0U)
      ...
      Opts.setDebugInfo(static_cast<toolchain::codegenoptions::DebugInfoKind>(Val));
  }
  ...
  if (const Arg *A =
    ...
    if (A->getOption().matches(OPT_fuse_ctor_homing) &&
        Opts.getDebugInfo() == toolchain::codegenoptions::LimitedDebugInfo)
      Opts.setDebugInfo(toolchain::codegenoptions::DebugInfoConstructor);
    if (A->getOption().matches(OPT_fno_use_ctor_homing) &&
        Opts.getDebugInfo() == toolchain::codegenoptions::DebugInfoConstructor)
      Opts.setDebugInfo(toolchain::codegenoptions::LimitedDebugInfo);
  }
  ...
  const toolchain::Triple::ArchType DebugEntryValueArchs[] = {
      toolchain::Triple::x86, toolchain::Triple::x86_64, toolchain::Triple::aarch64,
      toolchain::Triple::arm, toolchain::Triple::armeb, toolchain::Triple::mips,
      toolchain::Triple::mipsel, toolchain::Triple::mips64, toolchain::Triple::mips64el};

  if (Opts.OptimizationLevel > 0 && Opts.hasReducedDebugInfo() &&
      toolchain::is_contained(DebugEntryValueArchs, T.getArch()))
  ...
  Opts.DebugNameTable = static_cast<unsigned>(
      Args.hasArg(OPT_ggnu_pubnames)
          ? toolchain::DICompileUnit::DebugNameTableKind::GNU
          : Args.hasArg(OPT_gpubnames)
                ? toolchain::DICompileUnit::DebugNameTableKind::Default
                : toolchain::DICompileUnit::DebugNameTableKind::None);
  ...
  if (Arg *A = Args.getLastArg(OPT_save_temps_EQ))
    Opts.SaveTempsFilePrefix =
        toolchain::StringSwitch<std::string>(A->getValue())
            .Case("obj", OutputFile)
            .Default(toolchain::sys::path::filename(OutputFile).str());

  // The memory profile runtime appends the pid to make this name more unique.
  const char *MemProfileBasename = "memprof.profraw";
  if (Args.hasArg(OPT_fmemory_profile_EQ)) {
    SmallString<128> Path(Args.getLastArgValue(OPT_fmemory_profile_EQ));
    toolchain::sys::path::append(Path, MemProfileBasename);
  ...
  for (const auto &A : Args) {
    ...
    for (const auto &arg : ASL) {
      ...
      toolchain::append_range(Opts.CmdArgs, ArgStr);
  ...
  if (Opts.CFProtectionBranch && T.isRISCV()) {
    if (const Arg *A = Args.getLastArg(OPT_mcf_branch_label_scheme_EQ)) {
      const auto Scheme =
          toolchain::StringSwitch<CFBranchLabelSchemeKind>(A->getValue())
#define CF_BRANCH_LABEL_SCHEME(Kind, FlagVal)                                  \
...
#include "language/Core/Basic/CFProtectionOptions.def"
  ...
  if (const Arg *A = Args.getLastArg(OPT_mfunction_return_EQ)) {
    auto Val = toolchain::StringSwitch<toolchain::FunctionReturnThunksKind>(A->getValue())
                   .Case("keep", toolchain::FunctionReturnThunksKind::Keep)
                   .Case("thunk-extern", toolchain::FunctionReturnThunksKind::Extern)
                   .Default(toolchain::FunctionReturnThunksKind::Invalid);
    // SystemZ might want to add support for "expolines."
    if (!T.isX86())
      Diags.Report(diag::err_drv_argument_not_allowed_with)
          << A->getSpelling() << T.getTriple();
    else if (Val == toolchain::FunctionReturnThunksKind::Invalid)
      Diags.Report(diag::err_drv_invalid_value)
          << A->getAsString(Args) << A->getValue();
    else if (Val == toolchain::FunctionReturnThunksKind::Extern &&
  ...
  for (auto *A :
    ...
    if (A->getOption().matches(OPT_mlink_builtin_bitcode)) {
      F.LinkFlags = toolchain::Linker::Flags::LinkOnlyNeeded;
  ...
  if (Arg *A = Args.getLastArg(OPT_fdenormal_fp_math_EQ)) {
    ...
    Opts.FPDenormalMode = toolchain::parseDenormalFPAttribute(Val);
  ...
  if (Arg *A = Args.getLastArg(OPT_fdenormal_fp_math_f32_EQ)) {
    StringRef Val = A->getValue();
    Opts.FP32DenormalMode = toolchain::parseDenormalFPAttribute(Val);
  ...
  if (auto *arg =
    ...
    auto ResultOrErr =
        toolchain::remarks::parseHotnessThresholdOption(arg->getValue());
  ...
  if (NeedLocTracking &&
      Opts.getDebugInfo() == toolchain::codegenoptions::NoDebugInfo)
...
static bool checkVerifyPrefixes(const std::vector<std::string> &VerifyPrefixes,
  ...
  for (const auto &Prefix : VerifyPrefixes) {
    ...
    auto BadChar = toolchain::find_if(Prefix, [](char C) {
...
#undef DIAG_OPTION_WITH_MARSHALLING
  ...
  if (Opts.VerifyDiagnostics &&
      toolchain::is_contained(Opts.VerifyPrefixes, "expected"))
  ...
  for (const auto &Remark : Opts.Remarks) {
    ...
    if (toolchain::is_contained(IgnoredRemarks, Remark))
      continue;
...
language::Core::CreateAndPopulateDiagOpts(ArrayRef<const char *> Argv) {
  ...
  if (std::optional<std::string> NoColor =
          toolchain::sys::Process::GetEnv("NO_COLOR");
...
bool language::Core::ParseDiagnosticArgs(DiagnosticOptions &Opts, ArgList &Args,
                                DiagnosticsEngine *Diags,
                                bool DefaultDiagColor) {
...
#undef DIAG_OPTION_WITH_MARSHALLING
  ...
  toolchain::sys::Process::UseANSIEscapeCodes(Opts.UseANSIEscapeCodes);
  ...
  if (!checkVerifyPrefixes(Opts.VerifyPrefixes, *Diags))
    ...
    toolchain::sort(Opts.VerifyPrefixes);
...
static const auto &getFrontendActionTable() {
  static const std::pair<frontend::ActionKind, unsigned> Table[] = {
      ...
      {frontend::EmitBC, OPT_emit_toolchain_bc},
      {frontend::EmitCIR, OPT_emit_cir},
      {frontend::EmitHTML, OPT_emit_html},
      {frontend::EmitLLVM, OPT_emit_toolchain},
      {frontend::EmitLLVMOnly, OPT_emit_toolchain_only},
  ...
  if (Opts.ProgramAction == frontend::ASTDump) {
    GenerateProgramAction = [&]() {
      ...
      if (Opts.ASTDumpFormat != ADOF_Default) {
        ...
        case ADOF_Default:
          toolchain_unreachable("Default AST dump format.");
        case ADOF_JSON:
          Format = "json";
          break;
        }
...
#undef FRONTEND_OPTION_WITH_MARSHALLING
  ...
  if (const Arg *A = Args.getLastArg(OPT_Action_Group)) {
    ...
    if (ProgramAction == frontend::ASTDump &&
      ...
      unsigned Val = toolchain::StringSwitch<unsigned>(A->getValue())
    ...
    // -S -emit-toolchain means -emit-toolchain while -emit-toolchain -S means -S). However, to
    // support driver `-c -Xclang ACTION` (-cc1 -emit-toolchain file -main-file-name
...
#undef HEADER_SEARCH_OPTION_WITH_MARSHALLING
  ...
  auto Matches = [](const HeaderSearchOptions::Entry &Entry,
                    toolchain::ArrayRef<frontend::IncludeDirGroup> Groups,
                    std::optional<bool> IsFramework,
                    std::optional<bool> IgnoreSysRoot) {
    return toolchain::is_contained(Groups, Entry.Group) &&
  ...
  for (; It < End && Matches(*It, {frontend::Angled}, std::nullopt, true);
    ...
    OptSpecifier Opt = [It, Matches]() {
      ...
      if (Matches(*It, frontend::Angled, false, true))
        return OPT_I;
      toolchain_unreachable("Unexpected HeaderSearchOptions::Entry.");
    }();
...
#undef HEADER_SEARCH_OPTION_WITH_MARSHALLING
  ...
  if (!(P.empty() || toolchain::sys::path::is_absolute(P))) {
    if (WorkingDir.empty())
      toolchain::sys::fs::make_absolute(P);
    else
      toolchain::sys::fs::make_absolute(WorkingDir, P);
  }
  toolchain::sys::path::remove_dots(P);
  ...
  for (const auto *A : Args.filtered(OPT_fmodules_ignore_macro)) {
    ...
    Opts.ModulesIgnoreMacros.insert(
        toolchain::CachedHashString(MacroDef.split('=').first));
  }
  ...
  auto PrefixHeaderPath = [IsSysrootSpecified,
                           &Opts](const toolchain::opt::Arg *A,
                                  bool IsFramework = false) -> std::string {
    assert(A->getNumValues() && "Unexpected empty search path flag!");
    if (IsSysrootSpecified && !IsFramework && A->getValue()[0] == '=') {
      SmallString<32> Buffer;
      toolchain::sys::path::append(Buffer, Opts.Sysroot,
                              toolchain::StringRef(A->getValue()).substr(1));
...
static bool IsInputCompatibleWithStandard(InputKind IK,
  ...
  case Language::LLVM_IR:
  case Language::CIR:
    toolchain_unreachable("should not parse language flags for this input");
  ...
  toolchain_unreachable("unexpected input language");
}
...
static StringRef GetInputKindName(InputKind IK) {
  ...
  toolchain_unreachable("unknown input language");
}
...
#undef LANG_OPTION_WITH_MARSHALLING
  ...
  if (!Opts.OMPTargetTriples.empty()) {
    std::string Targets;
    toolchain::raw_string_ostream OS(Targets);
    toolchain::interleave(
        Opts.OMPTargetTriples, OS,
        [&OS](const toolchain::Triple &T) { OS << T.str(); }, ",");
...
#define ABI_VER_LATEST(Latest)                                                 \
  case LangOptions::ClangABI::Latest:                                          \
    break;
#include "language/Core/Basic/ABIVersions.def"
  }
...
#include "language/Core/Basic/LangStandards.def"
  ...
  if (const Arg *A = Args.getLastArg(OPT_cl_std_EQ)) {
    LangStandard::Kind OpenCLLangStd
      = toolchain::StringSwitch<LangStandard::Kind>(A->getValue())
...
#include "language/Core/Basic/CFProtectionOptions.def"
  ...
  toolchain::sort(Opts.ModuleFeatures);
  ...
  if (Arg *A = Args.getLastArg(options::OPT_offload_targets_EQ)) {
    ...
    auto getArchPtrSize = [](const toolchain::Triple &T) {
      if (T.isArch16Bit())
        return Arch16Bit;
      if (T.isArch32Bit())
        return Arch32Bit;
    ...
    for (unsigned i = 0; i < A->getNumValues(); ++i) {
      toolchain::Triple TT(A->getValue(i));

      if (TT.getArch() == toolchain::Triple::UnknownArch ||
          !(TT.getArch() == toolchain::Triple::aarch64 || TT.isPPC() ||
            TT.getArch() == toolchain::Triple::spirv64 ||
            TT.getArch() == toolchain::Triple::systemz ||
            TT.getArch() == toolchain::Triple::loongarch64 ||
            TT.getArch() == toolchain::Triple::nvptx ||
            TT.getArch() == toolchain::Triple::nvptx64 || TT.isAMDGCN() ||
            TT.getArch() == toolchain::Triple::x86 ||
            TT.getArch() == toolchain::Triple::x86_64))
  ...
  if (Arg *A = Args.getLastArg(options::OPT_fopenmp_host_ir_file_path)) {
    ...
    if (!toolchain::sys::fs::exists(Opts.OMPHostIRFile))
  ...
  if (auto *A =
    ...
    for (int i = 0, n = A->getNumValues(); i != n; ++i) {
      Opts.OverflowPatternExclusionMask |=
          toolchain::StringSwitch<unsigned>(A->getValue(i))
  ...
  if (Opts.HLSL) {
    ...
    if (T.isDXIL() || T.isSPIRVLogical()) {
      ...
      if (T.isDXIL()) {
        ...
        toolchain_unreachable("expected DXIL or SPIR-V target");
      }
    } else
...
static bool isStrictlyPreprocessorAction(frontend::ActionKind Action) {
  ...
  toolchain_unreachable("invalid frontend action");
}
...
bool CompilerInvocation::CreateFromArgsImpl(
  ...
  toolchain::opt::Visibility VisibilityMask(options::CC1Option);
  unsigned MissingArgIndex, MissingArgCount;
  ...
  if (!Res.getCodeGenOpts().ProfileInstrumentUsePath.empty()) {
    auto FS =
        createVFSFromOverlayFiles(Res.getHeaderSearchOpts().VFSOverlayFiles,
                                  Diags, toolchain::vfs::getRealFileSystem());
...
std::string CompilerInvocation::getModuleHash() const {
  ...
  toolchain::HashBuilder<toolchain::MD5, toolchain::endianness::native> HBuilder;
...
#include "language/Core/Basic/DebugOptions.def"
  ...
  return toString(toolchain::APInt(64, Hash), 36, /*Signed=*/false);
}

void CompilerInvocationBase::generateCC1CommandLine(
    ArgumentConsumer Consumer) const {
  toolchain::Triple T(getTargetOpts().Triple);
...
language::Core::createVFSFromCompilerInvocation(const CompilerInvocation &CI,
                                       DiagnosticsEngine &Diags) {
...
IntrusiveRefCntPtr<toolchain::vfs::FileSystem>
language::Core::createVFSFromCompilerInvocation(
...
IntrusiveRefCntPtr<toolchain::vfs::FileSystem> language::Core::createVFSFromOverlayFiles(
    ArrayRef<std::string> VFSOverlayFiles, DiagnosticsEngine &Diags,
    IntrusiveRefCntPtr<toolchain::vfs::FileSystem> BaseFS) {
  if (VFSOverlayFiles.empty())
    return BaseFS;

  IntrusiveRefCntPtr<toolchain::vfs::FileSystem> Result = BaseFS;
  ...
  for (const auto &File : VFSOverlayFiles) {
    ...
    IntrusiveRefCntPtr<toolchain::vfs::FileSystem> FS = toolchain::vfs::getVFSFromYAML(
