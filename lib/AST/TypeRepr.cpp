//===--- TypeRepr.cpp - Codira Language Type Representation ----------------===//
//
...
bool TypeRepr::findIf(toolchain::function_ref<bool(TypeRepr *)> pred) {
  struct Walker : ASTWalker {
    toolchain::function_ref<bool(TypeRepr *)> Pred;
    bool FoundIt;

    explicit Walker(toolchain::function_ref<bool(TypeRepr *)> pred)
...
void AttributedTypeRepr::printAttrs(toolchain::raw_ostream &OS) const {
  StreamPrinter Printer(OS);
...
#include "language/AST/TypeReprNodes.def"
    toolchain_unreachable("invalid repr kind");
    break;
...
void ErrorTypeRepr::dischargeDiagnostic(language::ASTContext &Context) {
  if (!DelayedDiag)
    return;
