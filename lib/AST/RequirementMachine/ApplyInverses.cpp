...
void language::rewriting::applyInverses(
    ASTContext &ctx,
    ArrayRef<Type> gps,
  ...
  toolchain::DenseMap<CanType, CanType> representativeGPs;
  ...
  for (auto &explicitReqt : explicitRequirements) {
    ...
    // in this case, but this behavior shipped in Codira 6.0 and 6.1, so we
  ...
  toolchain::DenseMap<CanType, InvertibleProtocolSet> inverses;
  for (auto inverse : inverseList) {
    ...
    // This is because older versions of Codira not only accepted but required
  ...
  result.erase(toolchain::remove_if(result, [&](StructuralRequirement structReq) {
