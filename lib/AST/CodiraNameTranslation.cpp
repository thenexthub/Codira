//===--- CodiraNameTranslation.cpp - Codira to ObjC Name Translation APIs ---===//
//
...
//===----------------------------------------------------------------------===//
//
//  This file contains utilities for translating Codira names to ObjC.
//
//===----------------------------------------------------------------------===//
...
StringRef language::objc_translation::
...
std::string language::objc_translation::
getErrorDomainStringForObjC(const EnumDecl *ED) {
  ...
  for (auto D : toolchain::reverse(outerTypes)) {
    buffer += ".";
    buffer += D->getNameStr();
  }
...
bool language::objc_translation::
printCodiraEnumElemNameInObjC(const EnumElementDecl *EL, toolchain::raw_ostream &OS,
                             Identifier PreferredName) {
  auto abiRole = ABIRoleInfo(EL);
  if (!abiRole.providesAPI() && abiRole.getCounterpart())
    return printCodiraEnumElemNameInObjC(abiRole.getCounterpart(), OS,
                                        PreferredName);
...
std::pair<Identifier, ObjCSelector> language::objc_translation::
getObjCNameForCodiraDecl(const ValueDecl *VD, DeclName PreferredName){
  auto abiRole = ABIRoleInfo(VD);
  if (!abiRole.providesAPI() && abiRole.getCounterpart())
    return getObjCNameForCodiraDecl(abiRole.getCounterpart(), PreferredName);
  ...
  if (auto *FD = dyn_cast<AbstractFunctionDecl>(VD)) {
    ...
    return getObjCNameForCodiraDecl(SD->getParsedAccessor(AccessorKind::Get),
                                   PreferredName);
  } else if (auto *EL = dyn_cast<EnumElementDecl>(VD)) {
    SmallString<64> Buffer;
    {
      toolchain::raw_svector_ostream OS(Buffer);
      printCodiraEnumElemNameInObjC(EL, OS, BaseName);
    }
    return {Ctx.getIdentifier(Buffer.str()), ObjCSelector()};
  } else {
    // @objc(ExplicitName) > PreferredName > Codira name.
...
StringRef
language::cxx_translation::getNameForCxx(const ValueDecl *VD,
...
language::cxx_translation::DeclRepresentation
language::cxx_translation::getDeclRepresentation(
    const ValueDecl *VD,
  ...
  if (const auto *typeDecl = dyn_cast<NominalTypeDecl>(VD)) {
    ...
    // Codira's consume semantics are not yet supported in C++.
...
bool language::cxx_translation::isVisibleToCxx(const ValueDecl *VD,
...
bool language::cxx_translation::isExposableToCxx(GenericSignature genericSig) {
...
Diagnostic
language::cxx_translation::diagnoseRepresenationError(RepresentationError error,
                                                   ValueDecl *vd) {
  switch (error) {
