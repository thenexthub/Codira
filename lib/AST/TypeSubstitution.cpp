...
Type TypeSubstituter::transformPackElementType(PackElementType *element,
                                               TypePosition pos) {
  LANGUAGE_DEFER { level -= element->getLevel(); };
...
// FIXME: This exposes a scalability issue; see test/SILGen/opaque_result_type_slow.code.
/*
...
SubstitutionMap TypeBase::getContextSubstitutionMap() {
  ...
  while (baseTy) {
    ...
    if (auto boundGeneric = baseTy->getAs<BoundGenericType>()) {
      ...
      for (auto arg : toolchain::reverse(args)) {
...
static Type substOpaqueTypesWithUnderlyingTypesRec(
    ...
    toolchain::DenseSet<ReplaceOpaqueTypesWithUnderlyingTypes::SeenDecl> &decls) {
...
static bool canSubstituteTypeInto(Type ty, const DeclContext *dc,
  ...
  toolchain_unreachable("invalid substitution kind");
}
...
Type language::substOpaqueTypesWithUnderlyingTypes(Type ty,
...
CanType
language::substOpaqueTypesWithUnderlyingTypes(CanType ty,
                                           TypeExpansionContext context) {
...
ProtocolConformanceRef language::substOpaqueTypesWithUnderlyingTypes(
