//===--- DeclSynthesizer.cpp - Synthesize helper Codira decls --------------===//
//
...
CodiraDeclSynthesizer::createVarWithPattern(DeclContext *dc, Identifier name,
                                           Type ty,
...
Pattern *CodiraDeclSynthesizer::createTypedNamedPattern(VarDecl *decl) {
...
using ConstantGetterBodyContextData =
    toolchain::PointerIntPair<Expr *, 2, ConstantConvertKind>;
}

Type CodiraDeclSynthesizer::getConstantLiteralType(
...
ValueDecl *CodiraDeclSynthesizer::createConstant(Identifier name,
                                                ...
                                                AccessLevel access) {
  auto &context = ImporterImpl.CodiraContext;
...
synthesizeConstantGetterBody(AbstractFunctionDecl *afd, void *voidContext) {
  ...
  auto fn = cast<AccessorDecl>(afd);
  VarDecl *constantVar = cast<VarDecl>(fn->getStorage());
  Type type = fn->mapTypeIntoContext(constantVar->getValueInterfaceType());
...
synthesizeStructDefaultConstructorBody(AbstractFunctionDecl *afd,
  ...
  SubstitutionMap subMap = SubstitutionMap::get(
      zeroInitializerFunc->getGenericSignature(), toolchain::ArrayRef(selfType),
...
ConstructorDecl *
CodiraDeclSynthesizer::createDefaultConstructor(NominalTypeDecl *structDecl) {
  auto &context = ImporterImpl.CodiraContext;
...
ConstructorDecl *CodiraDeclSynthesizer::createValueConstructor(
...
ConstructorDecl *CodiraDeclSynthesizer::createRawValueBridgingConstructor(
...
void CodiraDeclSynthesizer::makeStructRawValuedWithBridge(
...
void CodiraDeclSynthesizer::makeStructRawValued(
...
CodiraDeclSynthesizer::makeUnionFieldAccessors(
...
static VarDecl *findAnonymousInnerFieldDecl(VarDecl *importedFieldDecl,
  ...
  toolchain_unreachable("couldn't find anonymous inner field decl");
}
...
ConstructorDecl *
CodiraDeclSynthesizer::makeEnumRawValueConstructor(EnumDecl *enumDecl) {
  ASTContext &C = ImporterImpl.CodiraContext;
  auto rawTy = enumDecl->getRawType();
...
void CodiraDeclSynthesizer::makeEnumRawValueGetter(EnumDecl *enumDecl,
...
AccessorDecl *CodiraDeclSynthesizer::makeStructRawValueGetter(
...
AccessorDecl *CodiraDeclSynthesizer::buildSubscriptGetterDecl(
...
AccessorDecl *CodiraDeclSynthesizer::buildSubscriptSetterDecl(
...
Expr *CodiraDeclSynthesizer::synthesizeReturnReinterpretCast(ASTContext &ctx,
                                                            Type givenType,
                                                            Type exprType,
                                                            Expr *baseExpr) {
  auto reinterpretCast = cast<FuncDecl>(
...
synthesizeUnwrappingGetterOrAddressGetterBody(AbstractFunctionDecl *afd,
  ...
  if (isAddress &&
    ...
    propertyExpr = CodiraDeclSynthesizer::synthesizeReturnReinterpretCast(
...
SubscriptDecl *CodiraDeclSynthesizer::makeSubscript(FuncDecl *getter,
                                                   FuncDecl *setter) {
...
VarDecl *
CodiraDeclSynthesizer::makeDereferencedPointeeProperty(FuncDecl *getter,
                                                      FuncDecl *setter) {
  assert((getter || setter) &&
...
synthesizeSuccessorFuncBody(AbstractFunctionDecl *afd, void *context) {
  ...
  std::tie(copyDecl, patternDecl) = CodiraDeclSynthesizer::createVarWithPattern(
...
FuncDecl *CodiraDeclSynthesizer::makeSuccessorFunc(FuncDecl *incrementFunc) {
  auto &ctx = ImporterImpl.CodiraContext;
...
synthesizeOperatorMethodBody(AbstractFunctionDecl *afd, void *context) {
  ...
  auto methodDecl =
      static_cast<FuncDecl *>(context); /* Codira version of CXXMethod */
...
VarDecl *
CodiraDeclSynthesizer::makeComputedPropertyFromCXXMethods(FuncDecl *getter,
                                                         FuncDecl *setter) {
  auto &ctx = ImporterImpl.CodiraContext;
...
CodiraDeclSynthesizer::makeDefaultArgument(const clang::ParmVarDecl *param,
  ...
  // Synthesize `fn __defaultArg_XYZ() -> ParamTy { ... }`.
  ...
  auto funcDecl = FuncDecl::createImplicit(
      ...
      {}, ParameterList::createEmpty(ctx), languageParamTy,
...
synthesizeAvailabilityDomainPredicateBody(AbstractFunctionDecl *afd,
  ...
  // Import `fn __XYZ_isAvailable() -> Bool` into Codira.
...
FuncDecl *CodiraDeclSynthesizer::makeAvailabilityDomainPredicate(
  ...
  // Synthesize `fn __language_XYZ_isAvailable() -> Builtin.Int1 { ... }`.
  std::string s;
  toolchain::raw_string_ostream os(s);
  os << "__language_" << featureInfo.first << "_isAvailable";
  DeclName funcName(ctx, DeclBaseName(ctx.getIdentifier(s)),
                    ParameterList::createEmpty(ctx));
