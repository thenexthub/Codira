...
void SILGenFunction::emitIsolatingDestructor(DestructorDecl *dd) {
  ...
  emitDistributedRemoteActorDeinit(selfValue, dd, false, [=] {
    ...
    FuncDecl *languageDeinitOnExecutorDecl =
        useMainActorBackDeploy ? SGM.getDeinitOnExecutorMainActorBackDeploy()
                               : SGM.getDeinitOnExecutor();
    if (!languageDeinitOnExecutorDecl) {
      dd->diagnose(diag::missing_deinit_on_executor_function);
      return;
    }
    SILFunction *languageDeinitOnExecutorSILFunc = SGM.getFunction(
        SILDeclRef(languageDeinitOnExecutorDecl, SILDeclRef::Kind::Func),
        NotForDefinition);
    SILValue languageDeinitOnExecutorFunc =
        B.createFunctionRefFor(loc, languageDeinitOnExecutorSILFunc);
    ...
    B.createApply(loc, languageDeinitOnExecutorFunc, {},
...
static void findRecursiveLinks(ClassDecl *cd,
                               toolchain::SmallSetVector<VarDecl *, 4> &result) {
...
void SILGenFunction::emitClassMemberDestruction(ManagedValue selfValue,
  ...
  toolchain::SmallSetVector<VarDecl *, 4> recursiveLinks;
