...
SILFunction *SILGenModule::getOrCreateForeignAsyncCompletionHandlerImplFunction(
  ...
  // bridged Codira types, we still lower the parameters to their ultimate
  // ObjC types.
  ...
  if (F->empty()) {
    ...
    SILGenFunction SGF(*this, *F, CodiraModule);
    {
      ...
      auto errorIndex = language::transform(convention.completionHandlerErrorParamIndex(),
          [](auto original) { return original + 1; });
      auto flagIndex = language::transform(convention.completionHandlerFlagParamIndex(),
          [](auto original) { return original + 1; });
      ...
      // arguments to the callback.
      {
        ...
        auto prepareArgument = [&](SILValue destBuf, CanType destFormalType,
          ...
          // Convert the ObjC argument to the bridged Codira representation we
          // want.
...
SILFunction *SILGenModule::getOrCreateDerivativeVTableThunk(
  ...
  SILGenFunction SGF(*this, *thunk, CodiraModule);
  SmallVector<ManagedValue, 4> params;
