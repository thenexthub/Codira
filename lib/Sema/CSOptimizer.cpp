...
static bool isSupportedOperator(Constraint *disjunction) {
  ...
  // Operators like &<<, &>>, &+, .== etc.
  if (toolchain::any_of(choices, [](Constraint *choice) {
...
static bool isSupportedGenericOverloadChoice(ValueDecl *decl,
  ...
  if (toolchain::any_of(choiceType->getRequirements(), [](const Requirement &req) {
        switch (req.getKind()) {
  ...
  return toolchain::all_of(paramList->getArray(), [](const ParamDecl *P) {
...
static bool isSupportedDisjunction(Constraint *disjunction) {
  ...
  return toolchain::all_of(choices, [&](Constraint *choice) {
...
static toolchain::PointerIntPair<Type, 1, bool>
inferTypeFromInitializerResultType(ConstraintSystem &cs,
  ...
  auto initRef =
      toolchain::find_if(disjunctions, [&ctorLocator](Constraint *disjunction) {
...
static Type inferTypeOfArithmeticOperatorChain(ConstraintSystem &cs,
  ...
  class OperatorChainAnalyzer : public ASTWalker {
    ...
    toolchain::SmallPtrSet<toolchain::PointerIntPair<Type, 1>, 2> candidates;
    ...
    void addCandidateType(Type type, bool literal) {
      if (literal) {
        if (type->isInt()) {
          ...
          if (toolchain::any_of(candidates, [](const auto &candidate) {
...
void forEachDisjunctionChoice(
    ...
    toolchain::function_ref<void(Constraint *, ValueDecl *decl, FunctionType *)>
        callback) {
...
static unsigned numOverloadChoicesMatchingOnArity(OverloadedDeclRefExpr *ODRE,
  ...
  return toolchain::count_if(ODRE->getDecls(), [&arguments](auto *choice) {
...
static bool isVariadicGenericOverload(ValueDecl *choice) {
  ...
  return toolchain::any_of(GPL->getParams(), [&](const GenericTypeParamDecl *GP) {
...
static void findFavoredChoicesBasedOnArity(
    ...
    toolchain::function_ref<void(Constraint *)> favoredChoice) {
  ...
  forEachDisjunctionChoice(
      ...
      [&](Constraint *choice, ValueDecl *decl, FunctionType *overloadType) {
        ...
        if (overloadType->getNumParams() == argumentList->size() ||
            toolchain::count_if(*decl->getParameterList(), [](auto *param) {
...
static std::optional<DisjunctionInfo> preserveFavoringOfUnlabeledUnaryArgument(
  ...
  if (toolchain::any_of(disjunction->getNestedConstraints(), [](Constraint *choice) {
...
static void determineBestChoicesInContext(
    ...
    toolchain::DenseMap<Constraint *, DisjunctionInfo> &result) {
  ...
  for (auto *disjunction : disjunctions) {
      ...
      if (auto expr = getAsExpr(locator->getAnchor())) {
        ...
        if (parentExpr) {
          ...
          case ExprKind::UnresolvedDot: {
            toolchain::SmallVector<Constraint *, 2> favoredChoices;
    ...
    auto anyNonSpeculativeCandidates =
        [&](ArrayRef<ArgumentCandidate> candidates) {
          ...
          if (toolchain::count_if(candidates, [&](const auto &candidate) {
    ...
    toolchain::TinyPtrVector<Type> resultTypes;
    ...
    std::function<bool(Type, Type)> isSubclassOf = [&](Type candidateType,
      ...
      if (superclassType->isExistentialType() && !superclassType->isAny()) {
        ...
        return toolchain::all_of(layout.getProtocols(), [&](ProtocolDecl *P) {
    ...
    std::function<std::optional<double>(GenericSignature, ValueDecl *, Type,
      ...
      if (genericSig && paramType->isTypeParameter()) {
        ...
        auto checkProtocolRequirementsOnly = [&]() -> double {
          ...
          if (toolchain::all_of(protocolRequirements, [&](ProtocolDecl *protocol) {
        ...
        for (const auto &requirement : genericSig.getRequirements()) {
          ...
          toolchain::SmallPtrSet<GenericTypeParamType *, 2> toExamine;
          ...
          if (toolchain::any_of(toExamine, [&](GenericTypeParamType *GP) {
    ...
    forEachDisjunctionChoice(
        ...
        [&](Constraint *choice, ValueDecl *decl, FunctionType *overloadType) {
          ...
          for (unsigned paramIdx = 0, n = overloadType->getNumParams();
            ...
            toolchain::BitVector mismatches(argumentCandidates[argIdx].size());
          ...
          if (canUseContextualResultTypes &&
            ...
            if (toolchain::any_of(resultTypes, [&](const Type candidateResultTy) {
          ...
          if (score > 0) {
            ...
            if (isArithmeticOperator(decl) &&
              ...
              if (!resultTy->hasTypeParameter() &&
                  toolchain::all_of(overloadType->getParams(),
  ...
  if (cs.isDebugMode() && bestOverallScore > 0) {
    ...
    auto getLogger = [&](unsigned extraIndent = 0) -> toolchain::raw_ostream & {
      return toolchain::errs().indent(cs.solverState->getCurrentIndent() +
                                 extraIndent);
    };
...
ConstraintSystem::selectDisjunction() {
  if (performanceHacksEnabled()) {
    if (auto *disjunction = selectDisjunctionWithHacks())
      return std::make_pair(disjunction, toolchain::TinyPtrVector<Constraint *>());
    return std::nullopt;
  }
  ...
  toolchain::DenseMap<Constraint *, DisjunctionInfo> favorings;
