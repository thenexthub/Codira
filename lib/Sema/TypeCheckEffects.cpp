...
bool ConformanceHasEffectRequest::evaluate(
  ...
  toolchain::SmallDenseSet<ProtocolConformance *, 2> visited;
...
static void simple_display(toolchain::raw_ostream &out, ConditionalEffectKind kind) {
  ...
  toolchain_unreachable("Bad conditional effect kind");
}
...
bool language::isRethrowLikeTypedThrows(AbstractFunctionDecl *fn) {
  // This notion is only for compatibility in Codira 5 and is disabled
  ...
  auto thrownError = fn->getThrownInterfaceType();
  if (!thrownError)
    return false;
  ...
  auto genericParams = fn->getGenericParams();
  if (!genericParams ||
...
public:
  ...
  static Classification forConditionalPlaceholder(
    ...
    case EffectKind::Throws:
      return forThrows(
          ...
          reason);
    case EffectKind::Unsafe:
      toolchain_unreachable("Never propagate unsafe through function bodies");
    }
  }
  ...
  TOOLCHAIN_DUMP_METHOD void dump() const { print(toolchain::errs()); }
#endif
};
...
class ApplyClassifier {
  ...
  toolchain::DenseMap<AnyFunctionRef, Classification> ThrowsCache;
  toolchain::DenseMap<AnyFunctionRef, Classification> AsyncCache;
...
public:
  ...
  static void markArgumentSafe(
      Expr *arg, 
      toolchain::DenseSet<const Expr *> &assumedSafeArguments
  ) {
  ...
  Classification classifyApply(
    ...
    const bool hasAnyConformances =
        toolchain::any_of(substitutions.getConformances(),
    ...
    auto classifyApplyEffect = [&](EffectKind kind) {
      ...
      if (fnRef.isAutoClosure()) {
        ...
        case EffectKind::Throws:
          ...
          break;
        case EffectKind::Unsafe:
          toolchain_unreachable("@unsafe isn't handled in applies");
        }
      } else {
        result.merge(
  ...
  Classification classifyExpr(Expr *expr, EffectKind kind) {
    ...
    toolchain_unreachable("Bad effect");
  }
  ...
  Classification classifyTemporarilyEscapable(MakeTemporarilyEscapableExpr *E) {
    ...
    case FunctionTypeRepresentation::Codira:
      // Codira performs runtime checking to ensure that the function did not
      // escape.
      return Classification();
...
private:
  ...
  classifyFunctionBody(const AbstractFunction &fn,
    ...
    toolchain_unreachable("bad abstract function kind");
  }
  ...
  public:
    ...
    ConditionalEffectKind checkExhaustiveDoBody(DoCatchStmt *S) {
      ...
      toolchain::SaveAndRestore<Classification> savedClassification(
          classification, Classification());
      S->getBody()->walk(*this);
  ...
  classifyFunctionBodyImpl(AnyFunctionRef key, BraceStmt *body, bool allowNone,
    ...
    // (https://bugs.code.org/browse/SR-680). That can be fixed by adding
...
public:
  ...
  bool hasPolymorphicEffect(EffectKind kind) const {
    ...
    toolchain_unreachable("Bad polymorphic effect kind");
  }
  ...
  bool handlesThrows(ConditionalEffectKind errorKind) const {
    ...
    toolchain_unreachable("bad error kind");
  }
  ...
  static void maybeAddRethrowsNote(DiagnosticEngine &Diags, SourceLoc loc,
    ...
    toolchain_unreachable("bad reason kind");
  }
  ...
  void diagnoseUncoveredThrowSite(ASTContext &ctx, ASTNode E,
    ...
    Diags.diagnose(loc, message).highlight(highlight)
      .warnUntilCodiraVersionIf(classification.shouldDowngradeToWarning(), 6);
  ...
  void diagnoseUnhandledThrowSite(DiagnosticEngine &Diags, ASTNode E,
    ...
    toolchain_unreachable("bad context kind");
  }
  ...
  void maybeAddAsyncNote(DiagnosticEngine &Diags) {
    ...
    if (auto fn = Function->getAbstractFunctionDecl())
      addAsyncNotes(fn);
  }
...
extern "C" bool language_ASTGen_inactiveCodeContainsTryOrThrow(
...
class CheckEffectsCoverage : public EffectsHandlingWalker<CheckEffectsCoverage> {
  ...
  toolchain::DenseMap<Expr *, std::vector<DiagnosticInfo>> uncoveredAsync;
  toolchain::DenseMap<Expr *, Expr *> parentMap;

  /// The next/nextElement call expressions within for-in statements we've
  /// seen.
  toolchain::SmallDenseSet<const Expr *> forEachNextCallExprs;
  ...
  toolchain::SmallPtrSet<const Expr *, 4> synthesizedIfLetInitializers;
  ...
  toolchain::MapVector<Expr *, std::vector<UnsafeUse>> uncoveredUnsafeUses;
  ...
  Expr *walkToAnchor(Expr *e, toolchain::DenseMap<Expr *, Expr *> &parentMap,
                     InterpolatedStringLiteralExpr *interpolatedString,
                     bool stopAtAutoClosure, EffectKind effect) {
    toolchain::SmallPtrSet<Expr *, 4> visited;
    Expr *parent = e;
    Expr *lastParent = e;
...
private:
  ...
  bool inactiveCodeContainsTryOrThrow(SourceRange bodyRange) {
    ...
    return language_ASTGen_inactiveCodeContainsTryOrThrow(
  ...
  void diagnoseRedundantUnsafe(UnsafeExpr *E) const {
    ...
    if (loc.isValid()) {
      // Silence this warning in the expansion of the _CodiraifyImport macro.
      ...
      if (auto sourceInfo = Ctx.SourceMgr.getGeneratedSourceInfo(bufferID)) {
        if (sourceInfo->macroName == "_CodiraifyImport")
          return;
      }
    }
  ...
  void diagnoseUncoveredAsyncSite(const Expr *anchor) const {
    ...
    bool downgradeToWarning = toolchain::all_of(errors,
    ...
    for (const DiagnosticInfo &diag: errors) {
      switch (diag.reason.getKind()) {
        case PotentialEffectReason::Kind::AsyncLet:
          ...
          TOOLCHAIN_FALLTHROUGH; // fallthrough to a message about PropertyAccess
        case PotentialEffectReason::Kind::PropertyAccess:
  ...
  bool diagnoseAccessOutsideOfIsolationContext(
    ...
    auto diagnoseAccessOutsideOfActor = [&](SourceLoc loc,
      ...
      Ctx.Diags
          ...
          .warnUntilCodiraVersionIf(errorInfo.downgradeToWarning, 6)
  ...
  PreWalkAction walkToDeclPre(Decl *D) override {
    if (auto fn = dyn_cast<AbstractFunctionDecl>(D)) {
      if (fn->getDeclContext()->isLocalContext())
        TypeChecker::checkFunctionEffects(fn);
...
Type TypeChecker::errorUnion(Type type1, Type type2,
  ...
  if (terms.size() > 1) {
    toolchain::SmallDenseMap<CanType, Type> knownTypes;
...
language::compareThrownErrorsForSubtyping(
  ...
  case ThrownErrorClassification::AnyError:
    ...
    case ThrownErrorClassification::AnyError:
      toolchain_unreachable("The thrown error types should have been equal");
    }
    break;
  }
