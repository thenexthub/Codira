...
#include "language/AST/TypeTransform.h"
...
Type ConstraintSystem::openUnboundGenericType(GenericTypeDecl *decl,
  ...
  toolchain::SmallVector<Type, 2> arguments;
  for (auto gp : decl->getInnermostGenericParamTypes()) {
    auto found = toolchain::find_if(replacements, [&](OpenedType pair) {
  ...
  auto result =
      TypeResolution::forInterface(
          DC, std::nullopt,
          [](auto) -> Type { toolchain_unreachable("should not be used"); },
          [](auto &, auto) -> Type { toolchain_unreachable("should not be used"); },
          [](auto, auto) -> Type { toolchain_unreachable("should not be used"); })
...
Type ConstraintSystem::openOpaqueType(OpaqueTypeArchetypeType *opaque,
  ...
  if (knownReplacements != OpenedTypes.end()) {
    ...
    toolchain_unreachable("Missing opaque type replacement");
  }
  ...
  auto shouldOpen = [&](OpaqueTypeArchetypeType *opaqueType) {
    if (context == CTP_Initialization) {
      ...
      if (auto *fn = dyn_cast<AbstractFunctionDecl>(DC))
        return opaqueType->getDecl()->isOpaqueReturnTypeOf(fn);
...
static FunctionType *substGenericArgs(
    ...
    toolchain::function_ref<Type(Type)> substFn) {
  toolchain::SmallVector<AnyFunctionType::Param, 4> params;
  params.reserve(funcTy->getNumParams());

  toolchain::transform(funcTy->getParams(), std::back_inserter(params),
...
static bool doesStorageProduceLValue(
  ...
  switch (storage->mutabilityInCodira(useDC, base)) {
...
void ConstraintSystem::recordOpenedTypes(
  ...
  if (!fixmeAllowDuplicates || OpenedTypes.count(locatorPtr) == 0)
    recordOpenedType(
      locatorPtr, toolchain::ArrayRef(openedTypes, replacements.size()),
      preparedOverload);
}
...
static unsigned getNumRemovedArgumentLabels(ValueDecl *decl,
  ...
  toolchain_unreachable("Unhandled FunctionRefInfo in switch.");
}
...
FunctionType *ConstraintSystem::adjustFunctionTypeForConcurrency(
  ...
  auto *adjustedTy = language::adjustFunctionTypeForConcurrency(
...
ConstraintSystem::getTypeOfReference(ValueDecl *value,
  ...
  if (value->getDeclContext()->isTypeContext() && isa<FuncDecl>(value)) {
    ...
    auto fn = cast<FuncDecl>(value);
    assert(fn->isOperator() && "Lookup should only find operators");

    SmallVector<OpenedType, 2> replacements;

    AnyFunctionType *funcType = fn->getInterfaceType()
        ->castTo<AnyFunctionType>();
    auto openedType = openFunctionType(
        funcType, locator, replacements, fn->getDeclContext(),
        preparedOverload);
    ...
    if (fn->getResultInterfaceType()->hasDynamicSelfType()) {
    ...
    if (!isRequirementOrWitness(locator)) {
      ...
      openedType = adjustFunctionTypeForConcurrency(
          origOpenedType, /*baseType=*/Type(), fn, useDC, numApplies, false,
...
static DeclReferenceType getTypeOfReferenceWithSpecialTypeCheckingSemantics(
  ...
  case DeclTypeCheckingSemantics::Normal:
    toolchain_unreachable("Decl does not have special type checking semantics!");
  ...
  if (auto *decl = choice.getDeclOrNull()) {
    ...
    if (auto fn = dyn_cast<AbstractFunctionDecl>(decl)) {
      if (!Options.contains(ConstraintSystemFlags::IgnoreAsyncSyncMismatch) &&
          !fn->hasPolymorphicEffect(EffectKind::Async) &&
          fn->isAsyncContext() != isAsynchronousContext(useDC)) {
        increaseScore(fn->isAsyncContext() ? SK_AsyncInSyncMismatch
                                             : SK_SyncInAsync,
                      locator);
      }
    }
    ...
    if (auto *afd = dyn_cast<AbstractFunctionDecl>(decl)) {
      ...
      if (isInvalidPartialApply) {
        ...
        if (level == 0) {
          // Codira 4 and earlier failed to diagnose a reference to a mutating
          // method without any applications at all, which would get
          // miscompiled into a function with undefined behavior. Warn for
          // source compatibility.
          bool isWarning = !getASTContext().isCodiraVersionAtLeast(5);
          (void)recordFix(
