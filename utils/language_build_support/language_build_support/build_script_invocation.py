...
from build_language.build_language.constants import SWIFT_BUILD_ROOT
from build_language.build_language.constants import SWIFT_REPO_NAME
from build_language.build_language.constants import SWIFT_SOURCE_ROOT

from language_build_support.language_build_support import products
from language_build_support.language_build_support import shell
from language_build_support.language_build_support import targets
from language_build_support.language_build_support import workspace
from language_build_support.language_build_support.cmake import CMake
from language_build_support.language_build_support.host_specific_configuration \
import HostSpecificConfiguration
from language_build_support.language_build_support.productpipeline_list_builder \
import ProductPipelineListBuilder
from language_build_support.language_build_support.products.ninja \
import get_ninja_path
from language_build_support.language_build_support.targets \
import StdlibDeploymentTarget
from language_build_support.language_build_support.utils import clear_log_time
from language_build_support.language_build_support.utils \
import exit_rejecting_arguments
from language_build_support.language_build_support.utils import fatal_error
from language_build_support.language_build_support.utils import log_time_in_scope
...
"""
        ...
        impl_args = [
            ...
            "--language-enable-assertions", str(args.language_assertions).lower(),
            "--language-stdlib-enable-assertions", str(
                args.language_stdlib_assertions).lower(),
            "--language-stdlib-enable-strict-availability", str(
                args.language_stdlib_strict_availability).lower(),
            "--language-analyze-code-coverage", str(
                args.language_analyze_code_coverage).lower(),
            ...
            '--language-enable-backtracing', str(args.language_enable_backtracing).lower(),
            ...
            "--language-source-dirname", products.Swift.product_source_name(),
        ]
        ...
        if args.language_disable_dead_stripping:
        ...
        if args.build_early_languagesyntax:
            ...
            args.extra_cmake_options.append(
                '-DSWIFT_PATH_TO_SWIFT_SYNTAX_SOURCE:PATH={}'.format(language_syntax_src))
        ...
        if args.build_early_language_driver:
            configuration = 'release' if str(args.language_build_variant) in [
                'Release',
                'RelWithDebInfo'
            ] else 'debug'
            ...
            language_driver_build = os.path.join(self.workspace.build_root,
                                              ...
                                              'bin')
            args.extra_cmake_options.append(
                '-DSWIFT_EARLY_SWIFT_DRIVER_BUILD={}'.format(language_driver_build))
        ...
        if args.language_runtime_fixed_backtracer_path is not None:
            impl_args += [
                '--language-runtime-fixed-backtracer-path=%s' %
                args.language_runtime_fixed_backtracer_path
            ]
        ...
        if args.lto_type is not None:
            ...
            if args.language_tools_max_parallel_lto_link_jobs is not None:
                impl_args += [
                    ...
                    min(args.language_tools_max_parallel_lto_link_jobs,
                        args.build_jobs)
                ]
        ...
        for host_target in [args.host_target] + args.cross_compile_hosts:
            ...
            options[host_target] = {
                ...
                "SWIFT_STDLIB_TARGETS": " ".join(
                    config.language_stdlib_build_targets),
                "SWIFT_LIBEXEC_TARGETS": " ".join(
                    config.language_libexec_build_targets),
                "SWIFT_BENCHMARK_TARGETS": " ".join(
                    config.language_benchmark_build_targets),
                "SWIFT_RUN_BENCHMARK_TARGETS": " ".join(
                    config.language_benchmark_run_targets),
                "SWIFT_TEST_TARGETS": " ".join(
                    config.language_test_run_targets),
                "SWIFT_FLAGS": config.language_flags,
...
product or not.
        ...
        # `languagec` invocations to. That is, if we find a Swift compiler
        ...
        builder.add_product(products.EarlyLanguageDriver,
    ...
    def _execute_impl(self, pipeline, all_hosts, should_run_epilogue_operations):
        ...
        for host_target in all_hosts:
            ...
            print("Building the standard library for: {}".format(
                      " ".join(config.language_stdlib_build_targets)))
            if config.language_test_run_targets and (
                self.args.test or self.args.long_test):
                print("Running Swift tests for: {}".format(
                          " ".join(config.language_test_run_targets)))
            if config.language_benchmark_run_targets and self.args.benchmark:
                print("Running Swift benchmarks for: {}".format(
                          " ".join(config.language_benchmark_run_targets)))
    ...
    def execute_product_build_steps(self, product_class, host_target):
        ...
        if product_class.is_languagepm_unified_build_product():
            build_dir = self.workspace.languagepm_unified_build_dir(
                host_target)
        else:
            build_dir = self.workspace.build_dir(
