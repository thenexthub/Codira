...
class WasmSwiftSDK(product.Product):
    ...
    def _append_platform_cmake_options(self, cmake_options, language_host_triple, has_pthread, wasi_sysroot, extra_language_flags):
        ...
        cmake_options.define(
            'CMAKE_Swift_COMPILER_TARGET', language_host_triple)
        ...
        cmake_options.define('CMAKE_Swift_FLAGS', ' '.join(language_flags))
    ...
    def _build_foundation(self, language_host_triple, clang_multiarch_triple, has_pthread, wasi_sysroot):
        ...
        foundation.cmake_options.define('_SwiftCollections_SourceDIR', os.path.join(source_root, 'language-collections'))
        foundation.cmake_options.define('_SwiftFoundation_SourceDIR', os.path.join(source_root, 'language-foundation'))
        foundation.cmake_options.define('_SwiftFoundationICU_SourceDIR', os.path.join(source_root, 'language-foundation-icu'))
        foundation.cmake_options.define('SwiftFoundation_MACRO', os.path.join(host_toolchain, 'lib', 'language', 'host', 'plugins'))
    ...
    def _build_language_testing(self, language_host_triple, has_pthread, wasi_sysroot):
        ...
        language_testing.cmake_options.define('SwiftTesting_MACRO', 'NO')
    ...
    def _build_xctest(self, language_host_triple, has_pthread, wasi_sysroot):
        xctest = CMakeProduct(
            args=self.args,
            toolchain=self.toolchain,
            source_dir=os.path.join(
                os.path.dirname(self.source_dir), 'language-corelibs-xctest'),
            build_dir=os.path.join(self.build_dir, 'xctest', language_host_triple))
        self._append_platform_cmake_options(
            xctest.cmake_options, language_host_triple, has_pthread, wasi_sysroot,
            extra_language_flags=[])
    ...
    def _build_target_package(self, language_host_triple, clang_multiarch_triple, has_pthread,
        ...
        self._build_libxml2(language_host_triple, clang_multiarch_triple, has_pthread, wasi_sysroot)
        self._build_foundation(language_host_triple, clang_multiarch_triple, has_pthread, wasi_sysroot)
        # Build language-testing
        self._build_language_testing(language_host_triple, has_pthread, wasi_sysroot)
        self._build_xctest(language_host_triple, has_pthread, wasi_sysroot)
    ...
    def build(self, host_target):
        ...
        languagec_path = os.path.abspath(self.toolchain.languagec)
