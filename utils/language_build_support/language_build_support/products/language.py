...
updated without updating language.py?")
        ...
        if self.args.language_compiler_version is not None:
            language_compiler_version = self.args.language_compiler_version
        ...
        return [
            ...
            ('SWIFT_COMPILER_VERSION', str(language_compiler_version)),
        ]
    ...
    def _version_flags(self):
        r = CMakeOptions()
        if self.args.language_compiler_version is not None:
            language_compiler_version = str(self.args.language_compiler_version)
            r.define('SWIFT_COMPILER_VERSION', language_compiler_version)
            r.define('SWIFT_TOOLCHAIN_VERSION', "languagelang-" + language_compiler_version)
        else:
    ...
    def _enable_backtracing(self):
        return [('SWIFT_ENABLE_BACKTRACING:BOOL',
                 self.args.language_enable_backtracing)]
    ...
    def _freestanding_is_darwin(self):
        return [('SWIFT_FREESTANDING_IS_DARWIN:BOOL',
                 self.args.language_freestanding_is_darwin)]
    ...
    def _language_tools_ld64_lto_codegen_only_for_supporting_targets(self):
        return [('SWIFT_TOOLS_LD64_LTO_CODEGEN_ONLY_FOR_SUPPORTING_TARGETS:BOOL',
                 self.args.language_tools_ld64_lto_codegen_only_for_supporting_targets)]
    ...
    def _handle_language_debuginfo_non_lto_args(self):
        if ('language_debuginfo_non_lto_args' not in self.args
                or self.args.language_debuginfo_non_lto_args is None):
            ...
            self.cmake_options.extend(
                [('SWIFT_DEBUGINFO_NON_LTO_ARGS:STRING',
                 ";".join(self.args.language_debuginfo_non_lto_args))])
