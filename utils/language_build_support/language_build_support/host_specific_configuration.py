...
class HostSpecificConfiguration(object):
    ...
    def __init__(self, host_target, args, stage_dependent_args=None):
        ...
        self.language_stdlib_build_targets = []
        self.language_libexec_build_targets = []
        self.language_test_run_targets = []
        self.language_benchmark_build_targets = []
        self.language_benchmark_run_targets = []
        self.language_flags = ''
        ...
        for deployment_target_name in stdlib_targets_to_configure:
            ...
            if build:
                ...
                if stage_dependent_args.build_language_stdlib_unittest_extra or \
                        ...
                        args.stress_test:
                    self.language_stdlib_build_targets.append(
                        "language-stdlib-" + name)
                else:
                    self.language_stdlib_build_targets.append(
                        "language-test-stdlib-" + name)
            if build_libexec:
                self.language_libexec_build_targets.append(
                    'language-libexec-' + name)
            if build_benchmarks:
                self.language_benchmark_build_targets.append(
                    "language-benchmark-" + name)
                if args.benchmark:
                    self.language_benchmark_run_targets.append(
            ...
            if test:
                ...
                if args.test_optimized and not test_host_only:
                    self.language_test_run_targets.append(
    ...
    def add_flags_for_cross_compilation(self, args, deployment_target):
        self.language_flags = deployment_target.platform.language_flags(args)
