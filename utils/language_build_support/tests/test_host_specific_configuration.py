...
class ToolchainTestCase(unittest.TestCase):
    ...
    def test_should_configure_and_build_when_deployment_is_all(self):
        ...
        self.assertEqual(len(hsc.language_stdlib_build_targets), 2)
        self.assertIn('language-test-stdlib-macosx-x86_64',
                      hsc.language_stdlib_build_targets)
    ...
    def test_should_only_deployment_if_specified(self):
        ...
        self.assertEqual(len(hsc.language_stdlib_build_targets), 1)
        self.assertIn('language-test-stdlib-iphoneos-arm64',
                      hsc.language_stdlib_build_targets)
    ...
    def test_should_only_configure_when_cross_compiling_different_stdlib_targets(self):
        ...
        self.assertEqual(len(hsc.language_stdlib_build_targets), 0)
    ...
    def generate_should_skip_building_platform(
        ...
        def test(self):
            ...
            self.assertNotIn(build_target, before.language_stdlib_build_targets)
            ...
            self.assertIn(build_target, after.language_stdlib_build_targets)
        return test
    ...
    def generate_should_build_full_targets_when_test(test_arg_name):
        def test(self):
            ...
            before = HostSpecificConfiguration(host_target, args)
            self.assertIn('language-test-stdlib-macosx-x86_64',
                          before.language_stdlib_build_targets)
            ...
            self.assertIn('language-stdlib-macosx-x86_64',
                          after.language_stdlib_build_targets)
    ...
    def generate_should_skip_testing_platform(
        ...
        def test(self):
            ...
            before = HostSpecificConfiguration(host_target, args)
            self.assertEqual(len(before.language_test_run_targets), 0)
            ...
            self.assertIn('check-language-{}'.format(host_target),
                          after.language_test_run_targets)
        return test
    ...
    def generate_should_allow_testing_only_host(
        ...
        def test(self):
            ...
            before = HostSpecificConfiguration(host_target, args)
            self.assertIn('check-language-{}'.format(host_target),
                          before.language_test_run_targets)
    ...
    def generate_should_build_benchmarks(host_target, build_arg_name):
        def test(self):
            ...
            self.assertIn('language-benchmark-{}'.format(host_target),
                          with_benchmark.language_benchmark_build_targets)
            self.assertNotIn('check-language-benchmark-{}'.format(host_target),
                             with_benchmark.language_benchmark_run_targets)

            args.benchmark = True
            running_benchmarks = HostSpecificConfiguration(host_target, args)
            self.assertIn('language-benchmark-{}'.format(host_target),
                          running_benchmarks.language_benchmark_build_targets)
            self.assertIn('check-language-benchmark-{}'.format(host_target),
                          running_benchmarks.language_benchmark_run_targets)
            ...
            self.assertIn(
                ...
                with_external_benchmarks.language_benchmark_build_targets)
            self.assertIn(
                'language-benchmark-{}-external'.format(host_target),
                with_external_benchmarks.language_benchmark_build_targets)
            self.assertIn('check-language-benchmark-{}'.format(host_target),
                          with_external_benchmarks.language_benchmark_run_targets)
            self.assertIn(
            ...
            not_running_benchmarks = HostSpecificConfiguration(host_target,
                                                               args)
            self.assertIn('language-benchmark-{}'.format(host_target),
                          not_running_benchmarks.language_benchmark_build_targets)
            self.assertIn('language-benchmark-{}-external'.format(host_target),
                          not_running_benchmarks.language_benchmark_build_targets)
            self.assertNotIn(
                'check-language-benchmark-{}'.format(host_target),
                not_running_benchmarks.language_benchmark_run_targets)
            self.assertNotIn(
                'check-language-benchmark-{}-external'.format(host_target),
                not_running_benchmarks.language_benchmark_run_targets)
        return test
    ...
    def generate_should_test_only_subset(subset_name, subset_arg_name):
        def test(self):
            ...
            before = HostSpecificConfiguration(host_target, args)
            self.assertIn(all, before.language_test_run_targets)
            self.assertNotIn(subset, before.language_test_run_targets)

            setattr(args, subset_arg_name, True)
            after = HostSpecificConfiguration(host_target, args)
            self.assertIn(subset, after.language_test_run_targets)
            self.assertNotIn(all, after.language_test_run_targets)
        return test
    ...
    def generate_should_test_optimizations(
        ...
        def test(self):
            ...
            before = HostSpecificConfiguration(host_target, args)
            self.assertNotIn(target, before.language_test_run_targets)

            setattr(args, optimize_arg_name, True)
            after = HostSpecificConfiguration(host_target, args)
            self.assertIn(target, after.language_test_run_targets)
        return test
