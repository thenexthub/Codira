"""
LLDB Formatters for LLVM data types for use in the language project.

Load into LLDB with 'command script import /path/to/lldbCodiraDataFormatters.py'
"""
...
def __lldb_init_module(debugger, internal_dict):
    category = 'language-compiler-internals'
    debugger.HandleCommand(f'type category define -e {category} -l c++')

    tName = 'lldbCodiraDataFormatters.SmallBitVectorSummaryProvider'
    ...
    debugger.HandleCommand(f'type summary add --expand --skip-references -w {category} '
                           '-F lldbCodiraDataFormatters.DemangleNodeSummaryProvider '
                           '-x "^language::Demangle::Node$"')
    debugger.HandleCommand(f'type synthetic add --skip-references -w {category} '
                           '-l lldbCodiraDataFormatters.DemangleNodeSynthProvider '
                           '-x "^language::Demangle::Node$"')
...
class DemangleNodeSynthProvider:
    """ Provider for language::Demangle::Node """
