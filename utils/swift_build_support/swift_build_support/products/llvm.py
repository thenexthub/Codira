...
class LLVM(cmake_product.CMakeProduct):
    ...
    def __init__(self, args, toolchain, source_dir, build_dir):
        ...
        self.cmake_options.define(
            'TOOLCHAIN_ENABLE_ASSERTIONS:BOOL', args.toolchain_assertions)

        # Add the cmake option for TOOLCHAIN_TARGETS_TO_BUILD.
        self.cmake_options.define(
            'TOOLCHAIN_TARGETS_TO_BUILD', args.toolchain_targets_to_build)
    ...
    def build(self, host_target):
        ...
        if host_target.startswith('linux'):
            ...
            if not self.is_release():
                ...
                toolchain_cmake_options.define('TOOLCHAIN_USE_SPLIT_DWARF:BOOL', 'YES')
        ...
        if not self.args._build_toolchain:
            ...
            # build of Codira depends on these.
        ...
        toolchain_cmake_options.define('TOOLCHAIN_TOOL_LANGUAGE_BUILD:BOOL', 'FALSE')
        toolchain_cmake_options.define('TOOLCHAIN_TOOL_LLD_BUILD:BOOL', 'TRUE')
        toolchain_cmake_options.define('TOOLCHAIN_INCLUDE_DOCS:BOOL', 'TRUE')
        toolchain_cmake_options.define('TOOLCHAIN_ENABLE_LTO:STRING', self.args.lto_type)
        toolchain_cmake_options.define('COMPILER_RT_INTERCEPT_LIBDISPATCH', 'ON')
        # Codira expects the old layout for the runtime directory
        # updating this in tracked in #80180
        toolchain_cmake_options.define('TOOLCHAIN_ENABLE_PER_TARGET_RUNTIME_DIR', 'OFF')
        if host_target.startswith('linux'):
            # This preserves the behaviour we had when using
            # TOOLCHAIN_BUILD_EXTERNAL COMPILER_RT --
        ...
        if system() == "Darwin":
            ...
            toolchain_cmake_options.define(
                ...
                relevant_options['CMAKE_OSX_SYSROOT'])
            toolchain_cmake_options.define(
                'TOOLCHAIN_BUILTIN_TARGETS', builtins_runtimes_target_for_darwin)
            toolchain_cmake_options.define(
                'TOOLCHAIN_RUNTIME_TARGETS', builtins_runtimes_target_for_darwin)
        ...
        if self.args.build_embedded_stdlib and system() == "Darwin":
            # Ask for Mach-O cross-compilation builtins (for Embedded Codira)
            toolchain_cmake_options.define(
                f'BUILTINS_{builtins_runtimes_target_for_darwin}_'
        ...
        toolchain_cmake_options.undefine('TOOLCHAIN_TOOL_COMPILER_RT_BUILD')
        toolchain_cmake_options.undefine('TOOLCHAIN_BUILD_EXTERNAL_COMPILER_RT')
        ...
        # compile the stdlib, but on Darwin too for Embedded Codira use cases.
        #
        ...
        toolchain_cmake_options.define('TOOLCHAIN_ENABLE_PROJECTS',
                                  ';'.join(toolchain_enable_projects))
        toolchain_cmake_options.define('TOOLCHAIN_ENABLE_RUNTIMES',
        ...
        if self.args.build_toolchain_only:
            ...
            toolchain_cmake_options.define('TOOLCHAIN_BUILD_TOOLS', 'NO')
            toolchain_cmake_options.define('TOOLCHAIN_INSTALL_TOOLCHAIN_ONLY', 'YES')
            toolchain_cmake_options.define('TOOLCHAIN_INCLUDE_TESTS', 'NO')
            toolchain_cmake_options.define('CLANG_INCLUDE_TESTS', 'NO')
            toolchain_cmake_options.define('TOOLCHAIN_INCLUDE_UTILS', 'NO')
            toolchain_cmake_options.define('TOOLCHAIN_TOOL_LLI_BUILD', 'NO')
            toolchain_cmake_options.define('TOOLCHAIN_TOOL_TOOLCHAIN_AR_BUILD', 'NO')
        ...
        if ("-DTOOLCHAIN_INCLUDE_TESTS=NO" not in toolchain_cmake_options
            and "-DTOOLCHAIN_INCLUDE_TESTS:BOOL=FALSE" not in toolchain_cmake_options):
        ...
        if self.is_cross_compile_target(host_target):
            ...
            toolchain_cmake_options.define('TOOLCHAIN_TABLEGEN', toolchain_tblgen)
            ...
            toolchain_cmake_options.define('TOOLCHAIN_NATIVE_BUILD', toolchain)
        ...
        # that Codira's stdlib can use compiler-rt builtins when targeting
        # iOS/tvOS/watchOS.
