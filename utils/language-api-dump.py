...
def dump_module_api(cmd, extra_dump_args, output_dir, module, quiet, verbose):
    ...
    output_file = '%s/%s/%s.language' % (output_dir, module, module)
    if not quiet:
    ...
    for submodule in submodules:
        output_file = '%s/%s/%s.language' % (output_dir, module, submodule)
        if not quiet:
            print('Writing %s...' % output_file)
...
def main():
    source_filename = 'language-api-dump.language'
    parser = create_parser()
    args = parser.parse_args()

    cmd_common = [
        args.language_ide_test,
        '-print-module',
        '-source-filename',
        source_filename,
        '-skip-overrides'
    ...
    if args.language_version:
        extra_args = extra_args + ['-language-version', '%s' % args.language_version]

    # Create a .language file we can feed into language-ide-test
    ...
    # Remove the .language file we fed into language-ide-test
