...
asserting if one of the tools does not exist at the specified path"""
    ...
    def __init__(self, language_build_dir):
        self.language_build_dir = language_build_dir

    def _get_tool(self, name):
        path = os.path.join(self.language_build_dir, 'bin', name)
...
runtime error if the tool does not exist"""
    ...
    def sil_func_extractor(self):
        """Return the path to sil-func-extractor in the specified language build
...
class SILConstantInputToolInvoker(SILToolInvoker):
    ...
    def __init__(self, config, tools, initial_input_file, extra_args):
        ...
        # *.languagemodule, *.sil, or *.sib file, we are always using *.sib.
