...
def _is_language_checkout(language_path):
    """Returns true if the given language_path is a valid Swift checkout, false otherwise.
...
def _get_language_repo_name(language_path, env=None):
    ...
    of the given language path or None if it is not a valid Swift checkout.
    """
...
SWIFT_SOURCE_ROOT = _get_language_source_root(PROJECT_PATH, env=os.environ)

# Set SWIFT_BUILD_ROOT to a directory that will contain a subdirectory for each build
# configuration
SWIFT_BUILD_ROOT = _get_language_build_root(SWIFT_SOURCE_ROOT, env=os.environ)

# Set SWIFT_REPO_NAME in your environment to control the name of the language directory
# name that is used.
SWIFT_REPO_NAME = _get_language_repo_name(PROJECT_PATH, env=os.environ)
