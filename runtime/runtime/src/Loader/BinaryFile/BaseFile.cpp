...
#include "BaseFile.h"
#include "CodeFile/CodeFile.h"
namespace MapleRuntime {
BaseFile* BaseFile::CreateCODEFile(FileType type, CString filePath, Uptr cfileMetaAddr)
{
    BaseFile* baseFile = nullptr;
    switch (type) {
        case FileType::C_FILE:
            CHECK(cfileMetaAddr != 0);
            baseFile = new (std::nothrow) CODEFile(filePath, cfileMetaAddr);
        default:
            break;
    }
