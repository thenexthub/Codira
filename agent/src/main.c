...
static void print_diag(const char *path, const ZDiag *diag) {
  ...
  fprintf(stderr, "  explain: agent explain %s\n", diag_code(diag->code));
}
...
static const ZeroEmbeddedSkill *find_embedded_skill(const char *name) {
  for (size_t i = 0; i < agent_embedded_skill_count; i++) {
    if (strcmp(agent_embedded_skills[i].name, name) == 0) return &agent_embedded_skills[i];
  }
  return NULL;
}
...
static int embedded_skills_list_command(bool json) {
  if (json) {
    ...
    bool first = true;
    for (size_t i = 0; i < agent_embedded_skill_count; i++) {
      const ZeroEmbeddedSkill *skill = &agent_embedded_skills[i];
...
static int embedded_skills_get_command(int argc, char **argv, int subcommand_index, bool json) {
  ...
  if (get_all) {
    target_count = 0;
    for (size_t i = 0; i < agent_embedded_skill_count && target_count < sizeof(targets) / sizeof(targets[0]); i++) {
      if (!agent_embedded_skills[i].hidden) targets[target_count++] = &agent_embedded_skills[i];
    }
  }

  if (target_count == 0) {
    return embedded_skills_error(json, "No skill name provided. Usage: agent skills get <name>");
  }
...
static const char *agentommit(void) {
  const char *commit = getenv("AGENTOMMIT");
...
static bool write_runtime_compile_inputs(
  ...
  *out = (RuntimeCompileInputs){0};
  ...
  out->header_file = runtime_join_path(out->include_dir, "agent_runtime.h");
  ...
  if (!runtime_make_dir(out->include_dir, diag) ||
      !write_runtime_chunks_file(out->header_file, agent_embedded_agent_runtime_h, diag) ||
...
static bool compile_agent_runtime_object(const char *runtime_object_file, const ZToolchainPlan *plan, const Command *command, const ZTargetInfo *target, ZDiag *diag) {
  RuntimeCompileInputs inputs = {0};
  if (!write_runtime_compile_inputs(runtime_object_file, ".agent_runtime.c", agent_embedded_agent_runtime_c, &inputs, diag)) return false;
...
static bool compile_agent_http_curl_object(const char *runtime_object_file, const ZToolchainPlan *plan, const Command *command, const ZTargetInfo *target, ZDiag *diag) {
  RuntimeCompileInputs inputs = {0};
  if (!write_runtime_compile_inputs(runtime_object_file, ".agent_http_curl.c", agent_embedded_agent_http_curl_c, &inputs, diag)) return false;
  bool ok = z_toolchain_compile_c_object(plan, command ? command->profile : NULL, target, inputs.source_file, runtime_object_file, inputs.include_dir, "-std=c11 -Wall -Wextra -Wpedantic");
  runtime_compile_inputs_free(&inputs);
  if (!ok && diag) {
    diag->code = 2003;
...
static bool link_agent_runtime_executable(const char *object_file, const char *runtime_object_file, const char *http_object_file, const char *exe_file, const ZToolchainPlan *plan, const ZTargetInfo *target, ZDiag *diag) {
  ...
  if (!ok && diag) {
    ...
    snprintf(diag->expected, sizeof(diag->expected), "direct object plus agent runtime object link successfully");
...
static int agent_mkdir(const char *path) {
#if defined(_WIN32)
  return mkdir(path);
#else
...
static int agent_lstat(const char *path, struct stat *st) {
#if defined(_WIN32)
  return stat(path, st);
#else
...
static bool ensure_dir(const char *path, ZDiag *diag) {
  if (agent_mkdir(path) == 0 || errno == EEXIST) return true;
  diag->code = 2002;
  diag->path = path;
  diag->line = 1;
  diag->column = 1;
...
static bool remove_tree(const char *path, ZBuf *deleted) {
  struct stat st;
  if (agent_lstat(path, &st) != 0) return errno == ENOENT;
...
static const char *c_abi_type_name(const char *agent_type) {
  if (!agent_type || strcmp(agent_type, "Void") == 0) return "void";
  if (strcmp(agent_type, "Bool") == 0) return "bool";
  if (strcmp(agent_type, "u8") == 0) return "uint8_t";
  if (strcmp(agent_type, "i8") == 0) return "int8_t";
  if (strcmp(agent_type, "u16") == 0) return "uint16_t";
  if (strcmp(agent_type, "i16") == 0) return "int16_t";
  if (strcmp(agent_type, "u32") == 0) return "uint32_t";
  if (strcmp(agent_type, "i32") == 0) return "int32_t";
  if (strcmp(agent_type, "u64") == 0) return "uint64_t";
  if (strcmp(agent_type, "i64") == 0) return "int64_t";
  if (strcmp(agent_type, "usize") == 0) return "uintptr_t";
  if (strcmp(agent_type, "isize") == 0) return "intptr_t";
  if (strcmp(agent_type, "f32") == 0) return "float";
  if (strcmp(agent_type, "f64") == 0) return "double";
  return "void *";
}
...
static void append_interface_fingerprints_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target) {
  zbuf_append(buf, "{\"schemaVersion\":1,\"algorithm\":\"fnv1a64-agent-interface-v1\",\"targetFactsHash\":\"");
...
static uint64_t compile_cache_key(const SourceInput *input, const ZTargetInfo *target, const char *profile, const char *kind) {
  ...
  hash = mix_hash_text(hash, AGENT_VERSION);
...
static bool compiler_cache_touch(const char *kind, uint64_t key) {
  agent_mkdir(".agent");
  agent_mkdir(".agent/cache");
  agent_mkdir(".agent/cache/native");
  char path[256];
  snprintf(path, sizeof(path), ".agent/cache/native/%s-%016llx.cache", kind, (unsigned long long)key);
...
#define APPEND_CACHE(label, key, hit, invalidates) do { \
    ...
    zbuf_appendf(buf, ",\"key\":\"%016llx\",\"hit\":%s,\"stored\":true,\"compilerVersion\":\"%s\",\"packageVersion\":", (unsigned long long)(key), (hit) ? "true" : "false", AGENT_VERSION); \
...
static void append_package_metadata_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target) {
  ...
  zbuf_appendf(buf, ",\"manifestHash\":\"%016llx\",\"dependencyGraphHash\":\"%016llx\"",
               ...
               (unsigned long long)(input ? input->dependency_graph_hash : 0));
  zbuf_append(buf, ",\"lockfile\":{\"format\":\"agent-lock-v1\",\"path\":");
  append_json_string(buf, input && input->lockfile_path ? input->lockfile_path : "");
...
static void append_package_cache_audit_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target, const char *profile) {
  ...
  append_json_string(buf, AGENT_VERSION);
...
static void append_compile_time_json(ZBuf *buf, const Program *program, const SourceInput *input, const ZTargetInfo *target) {
  char *manifest = read_optional_file(input && input->manifest_path ? input->manifest_path : "agent.json");
  zbuf_append(buf, "{");
  ...
  zbuf_append(buf, ",\"cacheKeyInputs\":{\"algorithm\":\"fnv1a64-agent-meta-v1\"");
...
static void print_check_json_success(const char *path, SourceInput *input, const Program *program, const ZTargetInfo *target, const Command *command) {
  ...
  char *manifest = read_optional_file("agent.json");
  zbuf_append(&buf, ",\n  \"package\": ");
...
static void append_local_runtime_plan_json(ZBuf *buf, const CapabilitySummary *caps, const ZTargetInfo *target) {
  ...
  append_json_string(buf, "agent dev");
...
static void append_dev_plan_json(ZBuf *buf, const SourceInput *input, const Program *program, const ZTargetInfo *target, const Command *command) {
  ...
  append_json_string(buf, "agent.json");
  ...
  zbuf_append(buf, "}],\n  \"restart\": {\"runnableCli\":");
  zbuf_append(buf, main_fun ? "true" : "false");
  zbuf_append(buf, ",\"strategy\":\"rebuild-and-restart-after-successful-checks\",\"binaryOut\":\".agent/dev/app\"},\n  \"partialDiagnostics\": {\"stable\":true,\"whileCodegenPending\":true,\"sourceOfTruth\":\"agent check --json\",\"schemaVersion\":1},\n  \"trace\": {\"enabled\":true,\"requested\":");
...
static const char *diag_repair_summary(int code) {
  switch (code) {
    case 100: return "Repair the syntax at the reported parser span, then rerun agent check.";
    ...
    case 4004: return "Use agent targets --json to choose a direct-supported target, or request --emit obj when only object emission exists.";
    ...
    case 9001: return "Create the local dependency package or update the path in agent.json.";
    ...
    default: return code == 0 ? "Repair the syntax at the reported parser span, then rerun agent check." : "Inspect the diagnostic fields and choose a repair manually.";
  }
}
...
static const ExplainInfo explain_infos[] = {
  {
    ...
    "Run `agent targets` and choose one of the listed names.",
    "agent check --target not-a-target examples/hello.0",
    "agent check --target linux-musl-x64 examples/hello.0",
  },
  {
    "TAR002",
    "target",
    ...
    "agent check --target linux-musl-x64 conformance/native/fail/std-fs-target-unsupported.0",
    "agent build --target host examples/memory-package --out .agent/out/memory-package",
  },
  {
    "BLD003",
    "build",
    ...
    "agent build --emit c examples/hello.0",
    "agent build --emit exe examples/hello.0",
  },
  {
    "TYP009",
    "type",
  ...
  },
  {
    ...
    "Choose a target whose `agent targets --json` directBackend facts advertise the requested artifact, or use `--emit obj` where executable emission is not implemented.",
    "agent build --emit obj --target linux-arm64 examples/direct-call-add.0",
    "agent build --emit obj --target linux-x64 examples/direct-call-add.0",
  },
...
static void print_help(void) {
  printf("agent %s native bootstrap\n\n", AGENT_VERSION);
  printf("Usage:\n");
  printf("  agent --version [--json]\n");
  printf("  agent skills [list|get] [--json]\n");
  printf("  agent new cli|lib|package <name>\n");
  printf("  agent check <file.0|project|agent.json>\n");
  printf("  agent test <file.0|project|agent.json>\n");
  printf("  agent fmt <file.0|project|agent.json>\n");
  printf("  agent build [--json] [--emit exe|obj] [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] <file.0|project|agent.json>\n");
  printf("  agent run [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] <file.0|project|agent.json> [-- args...]\n");
  printf("  agent ship [--json] [--target <target>] [--profile release-small|tiny|audit] [--out <file>] <file.0|project|agent.json>\n");
  printf("  agent tokens --json <file.0|project|agent.json>\n");
  printf("  agent parse --json <file.0|project|agent.json>\n");
  printf("  agent graph [--json] <file.0|project|agent.json>\n");
  printf("  agent doc [--json] <file.0|project|agent.json>\n");
  printf("  agent size [--json] [--out <artifact>] <file.0|project|agent.json>\n");
  printf("  agent mem [--json] [--target <target>] <file.0|project|agent.json>\n");
  printf("  agent dev [--json] [--trace] <file.0|project|agent.json>\n");
  printf("  agent time --json <file.0|project|agent.json>\n");
  printf("  agent abi check|dump [--json] [--target <target>] <file.0|project|agent.json>\n");
  printf("  agent explain [--json] <code>\n");
  printf("  agent fix --plan --json <file.0|project|agent.json>\n");
  printf("  agent doctor [--json]\n");
  printf("  agent clean [--all]\n");
  printf("  agent targets\n");
  printf("\nExamples:\n");
  printf("  agent new cli hello\n");
  printf("  agent run examples/add.0\n");
  printf("  agent build --emit exe examples/hello.0 --out .agent/out/hello\n");
  printf("  agent ship --target linux-musl-x64 examples/hello.0 --out .agent/ship/hello\n");
  printf("  agent check --json examples/hello.0\n");
  printf("  agent build --target linux-musl-x64 examples/memory-package\n");
}

static void print_command_help(const char *command) {
  if (strcmp(command, "new") == 0) {
    printf("Usage: agent new cli|lib|package <name>\n\n");
    printf("Create a small Zero project template.\n\n");
    printf("Examples:\n");
    printf("  agent new cli hello\n");
    printf("  agent new lib math-kit\n");
    printf("  agent new package demo\n");
  } else if (strcmp(command, "skills") == 0) {
    printf("Usage: agent skills [list|get] [--json]\n\n");
    ...
    printf("Usage: agent doctor [--json]\n\n");
    printf("Check host, compiler, target toolchain, and docs/example readiness.\n");
  } else if (strcmp(command, "clean") == 0) {
    printf("Usage: agent clean [--all]\n\n");
    printf("Remove generated output while preserving compiler caches by default.\n\n");
    printf("Flags:\n");
    printf("  --all    remove broader .agent generated state while preserving .agent/bin\n");
  } else if (strcmp(command, "check") == 0) {
    printf("Usage: agent check [--json] [--target <target>] [--emit exe|obj] <file.0|project|agent.json>\n\n");
    ...
    printf("Usage: agent build [--json] [--emit exe|obj] [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] <input>\n\n");
    printf("Build direct native executable or object artifacts.\n\n");
    printf("Example: agent build --release tiny --emit exe examples/hello.0 --out .agent/out/hello\n");
  } else if (strcmp(command, "run") == 0) {
    printf("Usage: agent run [--target <target>] [--profile debug|dev|release-fast|release-small|tiny|audit] [--release <profile>] [--out <file>] <input> [-- args...]\n\n");
    printf("Build a host executable with the direct backend and run it. Program stdout and stderr are passed through unchanged.\n\n");
    printf("Example: agent run examples/add.0\n");
  } else if (strcmp(command, "ship") == 0) {
    printf("Usage: agent ship [--json] [--target <target>] [--profile release-small|tiny|audit] [--out <file>] <input>\n\n");
    ...
    printf("Usage: agent --version [--json]\n\n");
...
typedef struct {
  ...
  bool path_rename;
  bool agent_json_parse_bytes;
  bool agent_http_fetch_result;
  bool agent_http_result_ok;
  bool agent_http_result_status;
  bool agent_http_result_body_len;
  bool agent_http_result_error;
  bool agent_http_response_len;
  bool agent_http_response_headers_len;
  bool agent_http_response_body_offset;
  bool agent_http_header_value;
  bool agent_http_header_found;
  bool agent_http_header_offset;
  bool agent_http_header_len;
} RuntimeImportAudit;
...
static void runtime_import_audit_value(const IrValue *value, RuntimeImportAudit *audit) {
  ...
  switch (value->kind) {
    ...
    case IR_VALUE_JSON_STREAM_TOKENS_BYTES:
      audit->agent_json_parse_bytes = true;
      break;
    case IR_VALUE_HTTP_FETCH:
      audit->agent_http_fetch_result = true;
      break;
    case IR_VALUE_HTTP_RESULT_OK:
      audit->agent_http_result_ok = true;
      break;
    case IR_VALUE_HTTP_RESULT_STATUS:
      audit->agent_http_result_status = true;
      break;
    case IR_VALUE_HTTP_RESULT_BODY_LEN:
      audit->agent_http_result_body_len = true;
      break;
    case IR_VALUE_HTTP_RESULT_ERROR:
      audit->agent_http_result_error = true;
      break;
    case IR_VALUE_HTTP_RESPONSE_LEN:
      audit->agent_http_response_len = true;
      break;
    case IR_VALUE_HTTP_RESPONSE_HEADERS_LEN:
      audit->agent_http_response_headers_len = true;
      break;
    case IR_VALUE_HTTP_RESPONSE_BODY_OFFSET:
      audit->agent_http_response_body_offset = true;
      break;
    case IR_VALUE_HTTP_HEADER_VALUE:
      audit->agent_http_header_value = true;
      break;
    case IR_VALUE_HTTP_HEADER_FOUND:
      audit->agent_http_header_found = true;
      break;
    case IR_VALUE_HTTP_HEADER_OFFSET:
      audit->agent_http_header_offset = true;
      break;
    case IR_VALUE_HTTP_HEADER_LEN:
      audit->agent_http_header_len = true;
      break;
    default:
      break;
  }
...
static bool ir_value_needs_agent_runtime_object(const IrValue *value) {
  ...
  if (ir_value_needs_agent_runtime_object(value->index) ||
      ir_value_needs_agent_runtime_object(value->left) ||
      ir_value_needs_agent_runtime_object(value->right)) {
    return true;
  }
  for (size_t i = 0; i < value->arg_len; i++) {
    if (ir_value_needs_agent_runtime_object(value->args[i])) return true;
  }
  return false;
}

static bool ir_instrs_need_agent_runtime_object(const IrInstr *instrs, size_t len) {
  for (size_t i = 0; instrs && i < len; i++) {
    const IrInstr *instr = &instrs[i];
    if (ir_value_needs_agent_runtime_object(instr->value) ||
        ir_value_needs_agent_runtime_object(instr->index) ||
        ir_instrs_need_agent_runtime_object(instr->then_instrs, instr->then_len) ||
        ir_instrs_need_agent_runtime_object(instr->else_instrs, instr->else_len)) {
      return true;
    }
  }
  return false;
...
static bool ir_needs_agent_runtime_object(const IrProgram *ir) {
  for (size_t i = 0; ir && i < ir->function_len; i++) {
    if (ir_instrs_need_agent_runtime_object(ir->functions[i].instrs, ir->functions[i].instr_len)) return true;
  }
  return false;
}

static size_t native_agent_runtime_import_count(const RuntimeImportAudit *audit) {
  if (!audit) return 0;
  size_t count = 0;
  if (audit->agent_json_parse_bytes) count++;
  if (audit->agent_http_fetch_result) count++;
  if (audit->agent_http_result_ok) count++;
  if (audit->agent_http_result_status) count++;
  if (audit->agent_http_result_body_len) count++;
  if (audit->agent_http_result_error) count++;
  if (audit->agent_http_response_len) count++;
  if (audit->agent_http_response_headers_len) count++;
  if (audit->agent_http_response_body_offset) count++;
  if (audit->agent_http_header_value) count++;
  if (audit->agent_http_header_found) count++;
  if (audit->agent_http_header_offset) count++;
  if (audit->agent_http_header_len) count++;
  return count;
}

static bool runtime_import_audit_uses_agent_runtime(const RuntimeImportAudit *audit) {
  return native_agent_runtime_import_count(audit) > 0;
}

static bool runtime_import_audit_uses_http_provider(const RuntimeImportAudit *audit) {
  return audit && audit->agent_http_fetch_result;
}
...
static void append_runtime_import_module_json(ZBuf *buf, const RuntimeImportAudit *audit, size_t import_count) {
  ...
  bool uses_agent_runtime = runtime_import_audit_uses_agent_runtime(audit);
  append_json_string(buf, uses_agent_runtime ? "agent_runtime" : "native-target-runtime");
}

static bool append_runtime_import_functions_json(ZBuf *buf, const RuntimeImportAudit *audit) {
  bool first = true;
  zbuf_append(buf, "[");
  if (audit && audit->agent_json_parse_bytes) append_json_string_array_item(buf, &first, "agent_json_parse_bytes");
  if (audit && audit->agent_http_fetch_result) append_json_string_array_item(buf, &first, "agent_http_fetch_result");
  if (audit && audit->agent_http_result_ok) append_json_string_array_item(buf, &first, "agent_http_result_ok");
  if (audit && audit->agent_http_result_status) append_json_string_array_item(buf, &first, "agent_http_result_status");
  if (audit && audit->agent_http_result_body_len) append_json_string_array_item(buf, &first, "agent_http_result_body_len");
  if (audit && audit->agent_http_result_error) append_json_string_array_item(buf, &first, "agent_http_result_error");
  if (audit && audit->agent_http_response_len) append_json_string_array_item(buf, &first, "agent_http_response_len");
  if (audit && audit->agent_http_response_headers_len) append_json_string_array_item(buf, &first, "agent_http_response_headers_len");
  if (audit && audit->agent_http_response_body_offset) append_json_string_array_item(buf, &first, "agent_http_response_body_offset");
  if (audit && audit->agent_http_header_value) append_json_string_array_item(buf, &first, "agent_http_header_value");
  if (audit && audit->agent_http_header_found) append_json_string_array_item(buf, &first, "agent_http_header_found");
  if (audit && audit->agent_http_header_offset) append_json_string_array_item(buf, &first, "agent_http_header_offset");
  if (audit && audit->agent_http_header_len) append_json_string_array_item(buf, &first, "agent_http_header_len");
  zbuf_append(buf, "]");
  return !first;
}
...
static void append_runtime_import_audit_json(ZBuf *buf, const IrProgram *ir, const SourceInput *input, const ZTargetInfo *target) {
  ...
  size_t import_count = native_agent_runtime_import_count(&audit);
...
static void append_portable_runtime_json(ZBuf *buf, const IrProgram *ir, const SourceInput *input, const ZTargetInfo *target, const CapabilitySummary *caps) {
  ...
  zbuf_append(buf, uses_agent_runtime ? "true" : "false");
  zbuf_append(buf, ",\"module\":");
  append_runtime_import_module_json(buf, &audit, import_count);
  zbuf_append(buf, ",\"functions\":");
  append_runtime_import_functions_json(buf, &audit);
  zbuf_appendf(buf, ",\"functionCount\":%zu", import_count);
  zbuf_append(buf, ",\"adapter\":");
  if (uses_agent_runtime) append_json_string(buf, "native-agent-runtime-object");
...
static const char *direct_linker_flavor_for_emitter(const char *emitter) {
  ...
  if (strcmp(emitter, "agent-elf64") == 0 || strcmp(emitter, "agent-elf64-exe") == 0) return "elf64";
  if (strcmp(emitter, "agent-elf-aarch64") == 0 || strcmp(emitter, "agent-elf-aarch64-exe") == 0) return "elf64";
  if (strcmp(emitter, "agent-macho64") == 0 || strcmp(emitter, "agent-macho64-exe") == 0) return "macho64";
  if (strcmp(emitter, "agent-coff-x64") == 0 || strcmp(emitter, "agent-coff-x64-exe") == 0) return "coff";
  return "none";
}

static const char *direct_object_path_for_emitter(const char *emitter) {
  if (!emitter || strcmp(emitter, "none") == 0) return "unsupported";
  if (strcmp(emitter, "agent-elf64") == 0) return "direct-elf64-object";
  if (strcmp(emitter, "agent-elf64-exe") == 0) return "direct-elf64-exe";
  if (strcmp(emitter, "agent-elf-aarch64") == 0) return "direct-elf-aarch64-object";
  if (strcmp(emitter, "agent-elf-aarch64-exe") == 0) return "direct-elf-aarch64-exe";
  if (strcmp(emitter, "agent-macho64") == 0) return "direct-macho64-object";
  if (strcmp(emitter, "agent-macho64-exe") == 0) return "direct-macho64-exe";
  if (strcmp(emitter, "agent-coff-x64") == 0) return "direct-coff-x64-object";
  if (strcmp(emitter, "agent-coff-x64-exe") == 0) return "direct-coff-x64-exe";
...
static void append_object_backend_json(ZBuf *buf, const SourceInput *input, const ZTargetInfo *target, const Command *command, const char *emit_kind) {
  ...
  if (metadata_only_direct || runtime_linked_exe || (direct_emitter && strcmp(direct_emitter, "none") != 0 &&
    ...
    bool uses_agent_runtime = input && input->direct_host_runtime_import_count > 0;
    bool uses_http_runtime = input && input->direct_http_runtime_import_count > 0;
    bool links_agent_runtime = uses_agent_runtime;
    ...
    const char *runtime_external_toolchain = links_agent_runtime ? public_compiler_label(&runtime_toolchain) : "none";
    ...
    zbuf_appendf(buf, ",\"objectEmission\":{\"path\":\"%s\",\"functions\":true,\"dataSections\":%s,\"symbols\":%s,\"relocations\":\"%s\",\"symbolCount\":%zu,\"internalHelperCount\":%zu}",
                 ...
                 "true",
                 uses_agent_runtime ? "patched-runtime-import-relocations" : (direct_has_data ? "patched-internal-calls-and-data-relocations" : "patched-internal-calls-or-none-in-mvp"),
                 direct_symbol_count,
    ...
    append_json_string(buf, links_http_runtime ? "agent-runtime,curl" : (uses_agent_runtime ? "agent-runtime" : "none"));
    ...
    append_json_string(buf, uses_agent_runtime ? "direct-backend-runtime-link-plan" : "direct-backend");
    ...
    zbuf_append(buf, links_http_runtime ? "[\"agent_runtime.o\",\"agent_http_curl.o\"]" : (links_agent_runtime ? "[\"agent_runtime.o\"]" : "[]"));
    ...
    append_json_string(buf, links_agent_runtime ? runtime_toolchain.libc_mode : "none");
...
static void print_build_json(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target, const char *emit_kind, const char *artifact_path, long long artifact_bytes, long long generated_c_bytes, long long elapsed_ms) {
  ...
  printf(",\n  \"compiler\": ");
  const char *direct_emitter = direct_emit_emitter(target, command, emit_kind);
  if (direct_emitter && strcmp(direct_emitter, "agent-elf64-exe") == 0) print_json_string("agent-elf64");
  else if (direct_emitter && strcmp(direct_emitter, "agent-elf-aarch64-exe") == 0) print_json_string("agent-elf-aarch64");
  else if (direct_emitter && strcmp(direct_emitter, "agent-macho64-exe") == 0) print_json_string("agent-macho64");
  else if (direct_emitter && strcmp(direct_emitter, "agent-coff-x64-exe") == 0) print_json_string("agent-coff-x64");
  ...
  if (direct_emitter && strcmp(direct_emitter, "none") != 0) {
    ...
    if (strcmp(direct_emitter, "agent-elf64-exe") == 0) driver_kind = "agent-elf64";
    else if (strcmp(direct_emitter, "agent-elf-aarch64-exe") == 0) driver_kind = "agent-elf-aarch64";
    else if (strcmp(direct_emitter, "agent-macho64-exe") == 0) driver_kind = "agent-macho64";
    else if (strcmp(direct_emitter, "agent-coff-x64-exe") == 0) driver_kind = "agent-coff-x64";
...
static bool write_ship_artifacts(const Command *command, const SourceInput *input, const ZTargetInfo *target, const ZBuf *exe, const char *exe_file, ShipArtifacts *artifacts, ZDiag *diag) {
  ...
  artifacts->archive_path = path_with_suffix(exe_file, ".agentar");
  ...
  ZBuf debug;
  zbuf_init(&debug);
  zbuf_append(&debug, "{\n  \"schemaVersion\": 1,\n  \"kind\": \"agent-debug-symbol-metadata\",\n  \"sourceFile\": ");
  ...
  ZBuf size;
  zbuf_init(&size);
  zbuf_append(&size, "{\n  \"schemaVersion\": 1,\n  \"kind\": \"agent-release-size-report\",\n  \"sourceFile\": ");
  ...
  ZBuf archive;
  zbuf_init(&archive);
  zbuf_append(&archive, "agent archive manifest v1\n");
...
static void print_ship_json(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target, const ShipArtifacts *artifacts, long long elapsed_ms) {
  ...
  zbuf_append(&buf, ", ");
  append_ship_artifact_json(&buf, "archive", artifacts ? artifacts->archive_path : "", artifacts ? artifacts->archive_bytes : 0, "agent-archive-manifest-v1");
  zbuf_append(&buf, ", ");
...
static int print_version_command(bool json) {
  ...
  const char *agentc = getenv("AGENTC");
  bool target_cc_override = agentc && agentc[0];
  ...
  char *target_cc_version = target_cc_override ? z_strdup("configured via AGENTC") : (bundled_target_cc_available ? command_first_line("zig version 2>/dev/null") : z_strdup(""));
  ...
  append_json_string(&buf, AGENT_VERSION);
  zbuf_append(&buf, ",\n  \"commit\": ");
  append_json_string(&buf, agentommit());
  zbuf_append(&buf, ",\n  \"host\": ");
  append_json_string(&buf, z_host_target());
  zbuf_append(&buf, ",\n  \"backend\": \"agent-c\",\n  \"targets\": ");
...
static bool create_cli_template(const char *root, const char *name, ZDiag *diag) {
  ...
  if (!write_project_buf(root, "agent.json", &manifest, diag)) return false;
  ...
  if (!write_project_file(root, "src/main.0",
    ...
    "    if greeting_code() == 42 {\n"
    "        check world.out.write(\"hello from agent\\n\")\n"
    "    }\n"
    "}\n\n"
  ...
  if (!write_project_file(root, "README.md",
    "# Zero CLI\n\n"
    "This project was created with `agent new cli`.\n\n"
    "Try:\n\n"
    "```sh\n"
    "agent check .\n"
    "agent test .\n"
    "agent run .\n"
    "agent dev --json .\n"
    "agent build --target linux-musl-x64 --out .agent/out/app .\n"
    "agent ship --target linux-musl-x64 --out .agent/ship/app .\n"
    "```\n\n"
    ...
    diag)) return false;
  return write_project_file(root, ".gitignore", ".agent/\n", diag);
}
...
static bool create_lib_template(const char *root, const char *name, ZDiag *diag) {
  ...
  if (!write_project_file(root, "README.md",
    "# Zero Library\n\n"
    "This small package exposes one public function, docs metadata in `agent.json`, and an inline test.\n\n"
    "Try:\n\n"
    "```sh\n"
    "agent check .\n"
    "agent test .\n"
...
static int new_command(const Command *command) {
  ...
  if (!command->kind || !command->input) {
    fprintf(stderr, "usage: agent new cli|lib|package <name>\n");
    return 1;
  }
  if (path_exists(command->input)) {
    fprintf(stderr, "agent new: '%s' already exists\n", command->input);
    return 1;
  }
  ...
  if (strcmp(command->kind, "cli") == 0) ok = create_cli_template(command->input, command->input, &diag);
    ...
    fprintf(stderr, "agent new: unknown template '%s'\n", command->kind);
  ...
  if (strcmp(command->kind, "lib") == 0) printf("next: cd %s && agent check . && agent test .\n", command->input);
  else printf("next: cd %s && agent check . && agent test . && agent run .\n", command->input);
  return 0;
}

static int clean_command(const Command *command) {
  const char *default_paths[] = {".agent/out", NULL};
  const char *all_paths[] = {
    ".agent/out",
    ".agent/native-test",
    ".agent/conformance",
    ".agent/command-contracts",
    ".agent/generated-c-audit",
    ".agent/bench",
    NULL
  };
  ...
  for (size_t i = 0; paths[i]; i++) {
    if (!remove_tree(paths[i], &deleted)) {
      fprintf(stderr, "agent clean: failed to remove %s: %s\n", paths[i], strerror(errno));
      zbuf_free(&deleted);
      return 1;
    }
  }
  if (deleted.len == 0) {
    ...
    printf("removed:\n%s\n", deleted.data);
    if (command->all) printf("preserved .agent/bin so the local agent wrapper still works\n");
  }
  zbuf_free(&deleted);
  return 0;
}
...
static void doctor_target_toolchain_status(const ZTargetInfo *target, const ZToolchainPlan *plan, bool cc_ok, bool target_cc_ok, const char **status, ZBuf *message) {
  ...
  if (strcmp(plan->driver_kind, "host-cc") == 0 && !cc_ok) {
    *status = "error";
    zbuf_append(message, "host cc is missing; install a native C compiler or pass --cc/AGENTC");
    return;
  }

  if (strcmp(plan->driver_kind, "zig-cc") == 0 && !target_cc_ok) {
    *status = "warning";
    zbuf_appendf(message, "target-capable C compiler is required for %s; pass --cc/AGENTC or install the bundled-toolchain default", target->name);
    return;
  }
...
static int doctor_command(bool json) {
  ...
  bool agent_dir_ok = agent_mkdir(".agent") == 0 || errno == EEXIST;
  bool write_ok = false;
  if (agent_dir_ok) {
    ZDiag diag = {0};
    write_ok = z_write_file(".agent/doctor.tmp", "ok\n", &diag);
    unlink(".agent/doctor.tmp");
  }
  ...
  overall = worse_status(overall, (agent_dir_ok && write_ok) ? "ok" : "error");
  ...
  char *target_cc_message = target_cc_override ? z_strdup("configured via AGENTC") : (bundled_target_cc_ok ? command_first_line("zig version 2>/dev/null") : z_strdup("target-capable C compiler not found; cross-target executable builds may be unavailable"));
  ...
  if (json) {
    ...
    append_doctor_check_json(&buf, &first, "cross-executable-builds", target_cc_ok ? "ok" : "warning", target_cc_ok ? "target-capable C compiler available for non-host executable builds" : "pass --cc/AGENTC or install a target-capable C compiler for non-host executable builds");
    ...
    append_doctor_check_json(&buf, &first, ".agent-write", (agent_dir_ok && write_ok) ? "ok" : "error", (agent_dir_ok && write_ok) ? ".agent is writable" : ".agent is not writable");
    append_doctor_check_json(&buf, &first, "targets", target_ok ? "ok" : "warning", "run `agent targets` for the supported target list");
    ...
    zbuf_appendf(&target_toolchain_message, "%zu ready, %zu warnings, %zu errors; run `agent doctor --json` for per-target details", target_toolchains_ok, target_toolchains_warning, target_toolchains_error);
    ...
    printf("cross executable builds: %s (%s)\n", target_cc_ok ? "ok" : "warning", target_cc_ok ? "target-capable C compiler available" : "pass --cc/AGENTC or install a target-capable C compiler");
    ...
    printf(".agent write: %s\n", (agent_dir_ok && write_ok) ? "ok" : "error");
    printf("targets: %s (run `agent targets`)\n", target_ok ? "ok" : "warning");
...
static void test_fail(TestRunFailure *failure, const Expr *expr, const char *message) {
  ...
  snprintf(failure->message, sizeof(failure->message), "%s%s%s",
           message ? message : "agent test expectation failed",
      ...
      *returned = true;
      ...
      if (stmt->expr && stmt->expr->kind == EXPR_CALL) {
        ...
        if (named && strcmp(name.data ? name.data : "", "expect") == 0 && stmt->expr->args.len == 1) {
          ...
          zbuf_free(&name);
          if (!passed) {
            test_fail(failure, stmt->expr, "agent test expectation failed");
            return false;
          }
          continue;
        }
      ...
      test_fail(failure, NULL, "agent test direct runner does not support this statement yet");
      return false;
    }
  }
  return true;
...
static bool test_eval_function(const Program *program, const Function *fun, const TestValue *args, size_t arg_len, TestValue *out, TestRunFailure *failure) {
  if (!fun) {
    test_fail(failure, NULL, "agent test unknown function");
    return false;
  }
  if (fun->params.len != arg_len) {
    test_fail(failure, NULL, "agent test function argument count mismatch");
    return false;
  }
  TestEnv local = {0};
...
static bool test_eval_expr(const Program *program, TestEnv *env, const Expr *expr, TestValue *out, TestRunFailure *failure) {
  ...
  switch (expr->kind) {
    ...
    case EXPR_IDENT:
      ...
      test_fail(failure, expr, "agent test unknown identifier");
      return false;
    ...
    case EXPR_MEMBER: {
      ...
      test_value_free(&base);
      test_fail(failure, expr, "agent test unknown field");
      return false;
    }
    case EXPR_BINARY: {
      TestValue left = {0};
      ...
      if (strcmp(op, "==") == 0 || strcmp(op, "!=") == 0) {
        ...
        if (strcmp(op, "+") == 0) out->int_value = a + b;
          ...
          test_fail(failure, expr, "agent test unsupported operator");
          test_value_free(&left);
          test_value_free(&right);
          return false;
        }
    ...
    case EXPR_CALL: {
      ...
      if (!test_expr_name(expr->left, &name)) {
        zbuf_free(&name);
        test_fail(failure, expr, "agent test unsupported callee");
        return false;
      }
    ...
    default:
      test_fail(failure, expr, "agent test direct runner does not support this expression yet");
      return false;
  }
}
...
static int run_tests_direct(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target) {
  ...
  for (size_t i = 0; program && i < program->functions.len; i++) {
    ...
    if (expected_failure && ok) {
      ...
      if (!first_failure.message[0]) {
        ...
        snprintf(first_failure.message, sizeof(first_failure.message), "agent test expected failure unexpectedly passed: %s", failure.current_test ? failure.current_test : fun->name);
      }
      ...
      status = "failed";
      failed++;
      failure_message = failure.message[0] ? failure.message : "agent test failed";
  ...
  if (command && command->json) {
    ...
    zbuf_append(&buf, ",\"snapshotKey\":\"agent-test-direct-frontend-v1\"}");
    CapabilitySummary caps = program_capabilities(program);
...
static const char *helper_example_path(const StdHelperInfo *helper) {
  ...
  if (strcmp(module, "std.fs") == 0) return "examples/agent-hash/";
...
static const char *function_retention_reason(const Function *fun) {
  ...
  if (fun->is_test) return "agent test discovery";
...
static void manifest_path_for_input(const SourceInput *input, char *manifest_path, size_t manifest_path_len) {
  ...
  if (src_dir) snprintf(manifest_path, manifest_path_len, "%.*s/agent.json", (int)(src_dir - src), src);
  else snprintf(manifest_path, manifest_path_len, "agent.json");
}
...
static void append_self_host_routing_json(ZBuf *buf, const char *command_name, const char *emit_kind, const Program *program, const CapabilitySummary *caps, const ZTargetInfo *target) {
  ...
  zbuf_append(buf, ",\"phases\":{\"parse\":\"agent-c\",\"check\":\"agent-c\",\"lower\":\"agent-c\",\"emit\":\"agent-c\"}");
...
static void apply_ir_metrics_to_input(SourceInput *input, const IrProgram *ir, const ZTargetInfo *target) {
  ...
  if (strcmp(z_direct_object_emitter(target), "agent-macho64") == 0 ||
      strcmp(z_direct_exe_emitter(target), "agent-macho64-exe") == 0) {
...
static bool requested_exe_backend_matches(const Command *command, const char *emitter) {
  ...
  return (strcmp(command->backend, "agent-elf64") == 0 && strcmp(emitter, "agent-elf64-exe") == 0) ||
         (strcmp(command->backend, "agent-elf-aarch64") == 0 && strcmp(emitter, "agent-elf-aarch64-exe") == 0) ||
         (strcmp(command->backend, "agent-macho64") == 0 && strcmp(emitter, "agent-macho64-exe") == 0) ||
         (strcmp(command->backend, "agent-coff-x64") == 0 && strcmp(emitter, "agent-coff-x64-exe") == 0);
}
...
static bool target_readiness_emit_object_dry_run(const IrProgram *ir, const char *emitter, ZBuf *artifact, ZDiag *diag) {
  if (strcmp(emitter, "agent-elf64") == 0) return z_emit_elf64_object_from_ir(ir, artifact, diag);
  if (strcmp(emitter, "agent-elf-aarch64") == 0) return z_emit_elf_aarch64_object_from_ir(ir, artifact, diag);
  if (strcmp(emitter, "agent-macho64") == 0) return z_emit_macho64_object_from_ir(ir, artifact, diag);
  if (strcmp(emitter, "agent-coff-x64") == 0) return z_emit_coff_x64_object_from_ir(ir, artifact, diag);
  return false;
}

static bool target_readiness_emit_exe_dry_run(const IrProgram *ir, const char *emitter, ZBuf *artifact, ZDiag *diag) {
  if (strcmp(emitter, "agent-elf-aarch64-exe") == 0) return z_emit_elf_aarch64_exe_from_ir(ir, artifact, diag);
  if (strcmp(emitter, "agent-macho64-exe") == 0) return z_emit_macho64_exe_from_ir(ir, artifact, diag);
  if (strcmp(emitter, "agent-coff-x64-exe") == 0) return z_emit_coff_x64_exe_from_ir(ir, artifact, diag);
  if (strcmp(emitter, "agent-elf64-exe") == 0) return z_emit_elf64_exe_from_ir(ir, artifact, diag);
  return false;
}
...
static bool target_readiness_dry_emit(const Command *command, const ZTargetInfo *target, const IrProgram *ir, ZDiag *diag) {
  ...
  } else if (emit == EMIT_EXE && ir && ir_needs_agent_runtime_object(ir)) {
    emitted = target_readiness_emit_object_dry_run(ir, z_direct_object_emitter(target), &artifact, diag);
...
static bool target_readiness_select_diag(const Command *command, const SourceInput *input, const Program *program, const ZTargetInfo *target, const IrProgram *ir, ZDiag *diag) {
  ...
  if (ir && ir_needs_agent_runtime_object(ir)) {
    RuntimeImportAudit audit = runtime_import_audit_from_ir(ir);
    ...
    bool runtime_object_emitter_supported = object_emitter && (strcmp(object_emitter, "agent-macho64") == 0 || strcmp(object_emitter, "agent-elf64") == 0);
...
static void append_target_readiness_json(ZBuf *buf, SourceInput *input, const Program *program, const ZTargetInfo *target, const Command *command) {
  ...
  if (!ready && input) {
    /* C library validation points at agent.json; source mapping would erase that. */
...
int main(int argc, char **argv) {
  ...
  if (command.unknown_flag) {
    ...
    fprintf(stderr, "help: run `agent %s --help`", command.command);
  ...
  if (!z_is_known_target(command.target) || !target) {
    ...
    snprintf(diag.expected, sizeof(diag.expected), "one of agent targets");
    snprintf(diag.actual, sizeof(diag.actual), "%s", command.target);
    snprintf(diag.help, sizeof(diag.help), "run agent targets and choose a supported target name");
    if (command.json) print_diag_json(command.input, &diag);
    else print_diag(command.input, &diag);
    return 1;
  }
  ...
  if (command.legacy_backend || command.emit == EMIT_C) {
    ...
    snprintf(diag.expected, sizeof(diag.expected), "agent build --emit exe|obj <input>");
  ...
  if (strcmp(command.command, "run") == 0) {
    if (command.json) {
      ...
      diag.column = 1;
      diag.length = 1;
      snprintf(diag.message, sizeof(diag.message), "agent run does not support --json");
      snprintf(diag.expected, sizeof(diag.expected), "agent run <input>");
      snprintf(diag.actual, sizeof(diag.actual), "agent run --json");
      snprintf(diag.help, sizeof(diag.help), "program stdout belongs to the program; use agent build --json to inspect the artifact before running it");
    ...
    if (command.emit != EMIT_EXE) {
      ...
      diag.column = 1;
      diag.length = 1;
      snprintf(diag.message, sizeof(diag.message), "agent run only supports executable output");
      snprintf(diag.expected, sizeof(diag.expected), "agent run <input>");
      snprintf(diag.actual, sizeof(diag.actual), "--emit obj");
      snprintf(diag.help, sizeof(diag.help), "use agent build --emit obj when you need a non-executable artifact");
      print_diag(command.input, &diag);
      return 1;
    }
    if (!z_target_is_host(target)) {
      ...
      diag.column = 1;
      diag.length = 1;
      snprintf(diag.message, sizeof(diag.message), "agent run requires the host target");
      ...
      snprintf(diag.help, sizeof(diag.help), "use agent build --target %s for cross-target artifacts, then run them on a matching host", target && target->name ? target->name : "<target>");
      print_diag(command.input, &diag);
      return 1;
    }
  }
  ...
  if (strcmp(command.command, "fix") == 0 && !command.plan && !command.apply && !command.patch) {
    ...
    snprintf(diag.expected, sizeof(diag.expected), "agent fix --plan, --patch, or --apply --json <input>");
    snprintf(diag.actual, sizeof(diag.actual), "agent fix without a mode");
  ...
  if (strcmp(command.command, "fmt") == 0) {
    ...
    if (has_suffix(command.input, ".agent.code")) {
  ...
  if (command.emit == EMIT_OBJ) {
    if (strcmp(command.command, "build") != 0) {
      fprintf(stderr, "--emit obj is currently supported only by agent build\n");
      z_free_ir_program(&ir);
      z_free_program(&program);
      z_free_source(&input);
      return 1;
    ...
    if (strcmp(emitter, "agent-elf64") == 0) emitted_object = z_emit_elf64_object_from_ir(&ir, &object, &diag);
    else if (strcmp(emitter, "agent-elf-aarch64") == 0) emitted_object = z_emit_elf_aarch64_object_from_ir(&ir, &object, &diag);
    else if (strcmp(emitter, "agent-macho64") == 0) emitted_object = z_emit_macho64_object_from_ir(&ir, &object, &diag);
    else if (strcmp(emitter, "agent-coff-x64") == 0) emitted_object = z_emit_coff_x64_object_from_ir(&ir, &object, &diag);
  ...
  bool needs_agent_runtime = artifact_command && command.emit == EMIT_EXE && ir_needs_agent_runtime_object(&ir);
  if (needs_agent_runtime) {
    ...
    bool runtime_object_emitter_supported = object_emitter && (strcmp(object_emitter, "agent-macho64") == 0 || strcmp(object_emitter, "agent-elf64") == 0);
    if (ship_command) {
      int rc = return_direct_backend_error(&command, &input, target, "exe", "host runtime link plan is not wired into ship yet; use agent build or agent run", &ir, &program);
      z_free_source(&input);
      return rc;
    }
    if (!runtime_object_emitter_supported) {
    ...
    zbuf_init(&object);
    phase_started = now_ms();
    bool emitted_object = strcmp(object_emitter, "agent-macho64") == 0
    ...
    free(base_exe_file);
    char *object_file = path_with_suffix(exe_file, ".agent.o");
    char *runtime_object_file = path_with_suffix(exe_file, ".agent-runtime.o");
    char *http_object_file = needs_http_runtime ? path_with_suffix(exe_file, ".agent-http-curl.o") : NULL;
    ZToolchainPlan runtime_toolchain = z_plan_toolchain(command.cc, command.profile, target);

    phase_started = now_ms();
    input.emitted_object_cache_hit = compiler_cache_touch("emitted-object", compile_cache_key(&input, target, command.profile, strcmp(object_emitter, "agent-macho64") == 0 ? "direct-macho64-object-runtime-link" : "direct-elf64-object-runtime-link"));
    bool wrote_object = z_write_binary_file(object_file, (const unsigned char *)object.data, object.len, &diag);
    if (wrote_object) wrote_object = compile_agent_runtime_object(runtime_object_file, &runtime_toolchain, &command, target, &diag);
    if (wrote_object && needs_http_runtime) wrote_object = compile_agent_http_curl_object(http_object_file, &runtime_toolchain, &command, target, &diag);
    input.object_ms = now_ms() - phase_started;
    if (!wrote_object) {
    ...
    phase_started = now_ms();
    bool linked = link_agent_runtime_executable(object_file, runtime_object_file, http_object_file, exe_file, &runtime_toolchain, target, &diag);
  ...
  bool requested_direct_exe = artifact_command && command.emit == EMIT_EXE && command.backend &&
                              (strcmp(command.backend, "agent-elf64") == 0 ||
                               strcmp(command.backend, "agent-elf-aarch64") == 0 ||
                               strcmp(command.backend, "agent-macho64") == 0 ||
                               strcmp(command.backend, "agent-coff-x64") == 0);
  if (default_direct_exe || requested_direct_exe) {
    ...
    bool supported_exe = emitter && strcmp(emitter, "none") != 0 &&
                         (!command.backend ||
                          (strcmp(command.backend, "agent-elf64") == 0 && strcmp(emitter, "agent-elf64-exe") == 0) ||
                          (strcmp(command.backend, "agent-elf-aarch64") == 0 && strcmp(emitter, "agent-elf-aarch64-exe") == 0) ||
                          (strcmp(command.backend, "agent-macho64") == 0 && strcmp(emitter, "agent-macho64-exe") == 0) ||
                          (strcmp(command.backend, "agent-coff-x64") == 0 && strcmp(emitter, "agent-coff-x64-exe") == 0));
    if (!supported_exe) {
    ...
    if (!command.backend) {
      if (strcmp(emitter, "agent-elf-aarch64-exe") == 0) command.backend = "agent-elf-aarch64";
      else if (strcmp(emitter, "agent-macho64-exe") == 0) command.backend = "agent-macho64";
      else if (strcmp(emitter, "agent-coff-x64-exe") == 0) command.backend = "agent-coff-x64";
      else command.backend = "agent-elf64";
    }

    ZBuf exe;
    phase_started = now_ms();
    bool emitted_exe = false;
    if (strcmp(emitter, "agent-elf-aarch64-exe") == 0) emitted_exe = z_emit_elf_aarch64_exe_from_ir(&ir, &exe, &diag);
    else if (strcmp(emitter, "agent-macho64-exe") == 0) emitted_exe = z_emit_macho64_exe_from_ir(&ir, &exe, &diag);
    else if (strcmp(emitter, "agent-coff-x64-exe") == 0) emitted_exe = z_emit_coff_x64_exe_from_ir(&ir, &exe, &diag);
