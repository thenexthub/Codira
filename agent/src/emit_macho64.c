...
bool z_emit_macho64_object_from_ir(const IrProgram *program, ZBuf *out, ZDiag *diag) {
  ...
  for (size_t i = 0; i < program->function_len; i++) {
    ...
    zbuf_append(&strings, fun->name ? fun->name : "agent_fn");
    append_u8(&strings, 0);
  }
  ...
  if (has_runtime_http_response_headers_len) {
    ...
    zbuf_append(&strings, "_agent_http_response_headers_len");
    append_u8(&strings, 0);
  }
...
bool z_emit_macho64_exe_from_ir(const IrProgram *program, ZBuf *out, ZDiag *diag) {
  ...
  if (ctx.runtime_json_parse_bytes_patch_len > 0 ||
    ...
    zbuf_free(&rodata);
    zbuf_free(&text);
    return macho_diag_at(diag, "direct AArch64 Mach-O executable runtime helpers require object emission and an explicit runtime link step", 1, 1, "use --emit obj and link agent_runtime.c");
  }
  ...
  const uint32_t header_size = 32;
  const uint32_t pageagentmd_size = 72;
  const uint32_t section_count = has_rodata ? 2u : 1u;
  ...
  const uint32_t sizeofcmds = pageagentmd_size + text_segment_cmd_size + linkedit_cmd_size + dyld_info_cmd_size + uuid_cmd_size + dylinker_cmd_size + libsystem_cmd_size + main_cmd_size + build_version_cmd_size + code_signature_cmd_size;
  ...
  const char *code_signature_id = "agent-direct";
  ...
  append_u32le(out, 0x19);             // LC_SEGMENT_64
  append_u32le(out, pageagentmd_size);
  append_fixed(out, "__PAGEAGENT", 16);
  append_u64le(out, 0);
  ...
  macho_pad_to(out, header_size + pageagentmd_size + text_segment_cmd_size + linkedit_cmd_size + dyld_info_cmd_size + uuid_cmd_size + dylinker_cmd_size);
  ...
  macho_pad_to(out, header_size + pageagentmd_size + text_segment_cmd_size + linkedit_cmd_size + dyld_info_cmd_size + uuid_cmd_size + dylinker_cmd_size + libsystem_cmd_size);
