...
static void z_allocation_fatal(const char *operation) {
  fprintf(stderr, "agent: fatal: out of memory while %s\n", operation ? operation : "allocating memory");
  exit(70);
}
...
static char *module_name_from_path(const char *src_root, const char *path) {
  ...
  if (len > 2 && strcmp(relative + len - 2, ".agent.code") == 0) len -= 2;
...
static char *module_path_to_source(const char *src_root, const char *module_name) {
  ...
  zbuf_append(&relative, ".agent.code");
  ...
  free(file_path);
  if (relative.len >= 2 && strcmp(relative.data + relative.len - 2, ".agent.code") == 0) {
...
bool z_parse_manifest_json(const char *manifest, ZManifest *out, ZDiag *diag) {
  ...
  if (*json_skip_ws(manifest) != '{') {
    if (diag) {
      ...
      diag->line = 1;
      diag->column = 1;
      snprintf(diag->message, sizeof(diag->message), "agent.json must be a JSON object");
      snprintf(diag->expected, sizeof(diag->expected), "JSON object with targets.cli.main");
      snprintf(diag->help, sizeof(diag->help), "create agent.json with package and targets metadata");
    }
    return false;
  }
...
static char *dependency_manifest_path(const char *current_manifest_path, const char *dependency_path) {
  ...
  if (ends_with(dep_root, "agent.json")) {
    char *normalized = normalize_path_text(dep_root);
    free(dep_root);
    return normalized;
  }
  char *manifest = join_path(dep_root, "agent.json");
  free(dep_root);
...
static void set_package_diag(ZDiag *diag, int code, const char *path, const char *message, const char *expected, const char *actual, const char *help) {
  ...
  snprintf(diag->help, sizeof(diag->help), "%s", help ? help : "repair agent.json dependency metadata");
}

static unsigned long long source_dependency_graph_hash(const SourceInput *input) {
  unsigned long long hash = fs_fnv1a_text("agent-dependency-graph-v1");
...
static bool write_package_lockfile(SourceInput *input) {
  ...
  char path[256];
  snprintf(path, sizeof(path), ".agent/package-locks/%016llx.lock.json", input->dependency_graph_hash);
...
static bool resolve_manifest_dependencies(const char *manifest_path, const ZManifest *manifest, SourceInput *out, ZDiag *diag, char ***stack, size_t *stack_len, bool direct) {
  for (size_t i = 0; i < manifest->dependency_count; i++) {
    ...
    if (!dep_manifest_path || !file_exists(dep_manifest_path)) {
      set_package_diag(diag, 9001, manifest_path, "package dependency manifest not found", "dependency path containing agent.json", dep->path, "create the dependency package or update the dependency path");
...
bool z_resolve_source(const char *input, SourceInput *out, ZDiag *diag) {
  ...
  if (is_directory(input)) {
    manifest_path = join_path(input, "agent.json");
  } else if (ends_with(input, "agent.json")) {
  ...
  if (manifest_path) {
    ...
    if (!parsed_manifest.main_path) {
      ...
      diag->line = 1;
      diag->column = 1;
      snprintf(diag->message, sizeof(diag->message), "agent.json is missing targets.cli.main");
      z_free_manifest(&parsed_manifest);
      free(manifest);
      // Keep manifest_path alive for the diagnostic; callers print it after return.
      return false;
...
ZToolchainPlan z_plan_toolchain(const char *cc, const char *profile, const ZTargetInfo *target) {
  ...
  const char *env_override = getenv("AGENTC");
...
static bool validate_toolchain_plan(const ZToolchainPlan *plan, const ZTargetInfo *target) {
  ...
  if (strcmp(plan->driver_kind, "zig-cc") == 0 && !command_exists("zig")) {
    fprintf(stderr, "cross target '%s' requires a target-capable C compiler; pass --cc/AGENTC or install the bundled-toolchain default\n", target->name);
    return false;
  }

  if (strcmp(plan->driver_kind, "host-cc") == 0 && !command_exists("cc")) {
    fprintf(stderr, "host target requires cc; install a native C compiler or pass --cc/AGENTC\n");
    return false;
  }
...
static const char *profile_c_flags(const char *profile) {
  ...
  if (strcmp(profile, "debug") == 0 || strcmp(profile, "dev") == 0) return "-O0 -g -DAGENT_DEBUG";
  if (strcmp(profile, "audit") == 0) return "-O0 -g -DAGENT_AUDIT";
  return "-Os -DNDEBUG";
}
...
static void append_toolchain_driver_command(ZBuf *cmd, const ZToolchainPlan *plan) {
  if (strcmp(plan->driver_kind, "override-cc") == 0) {
    ...
    zbuf_append(cmd, "mkdir -p .agent/zig-global-cache .agent/zig-local-cache && ZIG_GLOBAL_CACHE_DIR=.agent/zig-global-cache ZIG_LOCAL_CACHE_DIR=.agent/zig-local-cache zig cc");
