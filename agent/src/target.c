...
static void ensure_targets_loaded(void) {
  if (targets) return;
  ZDiag diag = {0};
  char *text = z_read_file("native/agent-c/targets/targets.manifest", &diag);
...
const char *z_target_sysroot_env_name(const ZTargetInfo *target) {
  ...
  snprintf(name, sizeof(name), "AGENT_SYSROOT_");
...
const char *z_direct_object_emitter(const ZTargetInfo *target) {
  ...
  if (strcmp(format, "elf") == 0 && strcmp(arch, "x86_64") == 0 && strcmp(os, "linux") == 0) return "agent-elf64";
  if (strcmp(format, "elf") == 0 && strcmp(arch, "aarch64") == 0 && strcmp(os, "linux") == 0) return "agent-elf-aarch64";
  if (strcmp(format, "macho") == 0 && strcmp(arch, "aarch64") == 0 && strcmp(os, "macos") == 0) return "agent-macho64";
  if (strcmp(format, "coff") == 0 && strcmp(arch, "x86_64") == 0 && strcmp(os, "windows") == 0) return "agent-coff-x64";
  return "none";
}

const char *z_direct_exe_emitter(const ZTargetInfo *target) {
  if (!target) return "none";
  if (target->name && strcmp(target->name, "linux-x64") == 0) return "agent-elf64-exe";
  if (target->name && strcmp(target->name, "linux-musl-x64") == 0) return "agent-elf64-exe";
  if (target->name && strcmp(target->name, "linux-musl-arm64") == 0) return "agent-elf-aarch64-exe";
  if (target->name && strcmp(target->name, "linux-arm64") == 0) return "agent-elf-aarch64-exe";
  if (target->name && strcmp(target->name, "darwin-arm64") == 0) return "agent-macho64-exe";
  if (target->name && strcmp(target->name, "win32-x64.exe") == 0) return "agent-coff-x64-exe";
  return "none";
}
...
static bool target_http_runtime_supported(const ZTargetInfo *target) {
  ...
  return target &&
         ...
         (strcmp(object_emitter, "agent-macho64") == 0 || strcmp(object_emitter, "agent-elf64") == 0) &&
...
void z_append_http_runtime_json(ZBuf *buf, const ZTargetInfo *target) {
  ...
  else if (target && strcmp(z_direct_object_emitter(target), "agent-macho64") != 0 && strcmp(z_direct_object_emitter(target), "agent-elf64") != 0) reason = "target lacks host runtime relocation support";
  zbuf_appendf(
    buf,
