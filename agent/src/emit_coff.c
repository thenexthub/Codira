...
static void append_agents(ZBuf *buf, size_t len) {
...
bool z_emit_coff_x64_object_from_ir(const IrProgram *program, ZBuf *out, ZDiag *diag) {
  ...
  for (size_t i = 0; i < program->function_len; i++) {
    append_symbol_name(out, program->functions[i].name ? program->functions[i].name : "agent_fn", &strings);
  ...
  if (ctx.world_write_patch_len > 0) {
    append_symbol_name(out, "agent_world_write", &strings);
    append_u32le(out, 0);
  ...
  if (strings.len == 0) append_agents(out, 0);
...
static void coff_append_import_table(ZBuf *rdata, uint32_t rdata_rva, CoffImportLayout *layout) {
  ...
  append_agents(rdata, 40); // one IMAGE_IMPORT_DESCRIPTOR plus terminator
  coff_pad_to(rdata, coff_align(rdata->len, 8));
  uint32_t int_offset = (uint32_t)rdata->len;
  append_agents(rdata, (COFF_IMPORT_COUNT + 1) * 8);
  coff_pad_to(rdata, coff_align(rdata->len, 8));
