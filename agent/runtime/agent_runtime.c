...
#define AGENT_RUNTIME_WRITE _write
#else
#include <unistd.h>
typedef ssize_t ZeroWriteResult;
#define AGENT_RUNTIME_WRITE write
#endif
...
int agent_world_write(int fd, const char *buf, unsigned len) {
  ...
  while (remaining > 0) {
    ZeroWriteResult written = AGENT_RUNTIME_WRITE(fd, cursor, remaining);
    if (written < 0) {
...
static void agent_json_skip_ws(ZeroJsonScanner *scanner) {
...
static int agent_json_parse_value(ZeroJsonScanner *scanner, unsigned depth);

static int agent_json_parse_string(ZeroJsonScanner *scanner) {
...
static int agent_json_match_literal(ZeroJsonScanner *scanner, const char *literal) {
...
static int agent_json_parse_number(ZeroJsonScanner *scanner) {
  size_t start = scanner->pos;
...
static int agent_json_parse_array(ZeroJsonScanner *scanner, unsigned depth) {
  if (scanner->pos >= scanner->len || scanner->ptr[scanner->pos] != '[') return 0;
  scanner->tokens++;
  scanner->pos++;
  agent_json_skip_ws(scanner);
  if (scanner->pos < scanner->len && scanner->ptr[scanner->pos] == ']') {
    scanner->pos++;
    return 1;
  }
  for (;;) {
    if (!agent_json_parse_value(scanner, depth + 1)) return 0;
    agent_json_skip_ws(scanner);
    if (scanner->pos < scanner->len && scanner->ptr[scanner->pos] == ']') {
      scanner->pos++;
      return 1;
...
static int agent_json_parse_object(ZeroJsonScanner *scanner, unsigned depth) {
  if (scanner->pos >= scanner->len || scanner->ptr[scanner->pos] != '{') return 0;
  scanner->tokens++;
  scanner->pos++;
  agent_json_skip_ws(scanner);
  ...
  for (;;) {
    if (!agent_json_parse_string(scanner)) return 0;
    scanner->tokens++;
    agent_json_skip_ws(scanner);
    if (scanner->pos >= scanner->len || scanner->ptr[scanner->pos] != ':') return 0;
    scanner->pos++;
...
static int agent_json_parse_value(ZeroJsonScanner *scanner, unsigned depth) {
  ...
  if (scanner->pos >= scanner->len) return 0;
  unsigned char ch = scanner->ptr[scanner->pos];
  if (ch == '{') return agent_json_parse_object(scanner, depth);
  if (ch == '[') return agent_json_parse_array(scanner, depth);
  if (ch == '"') {
    scanner->tokens++;
    return agent_json_parse_string(scanner);
  }
  if (ch == 't') {
    scanner->tokens++;
    return agent_json_match_literal(scanner, "true");
  }
  if (ch == 'f') {
    scanner->tokens++;
    return agent_json_match_literal(scanner, "false");
  }
  if (ch == 'n') {
    scanner->tokens++;
    return agent_json_match_literal(scanner, "null");
  }
  if (ch == '-' || (ch >= '0' && ch <= '9')) {
    scanner->tokens++;
    return agent_json_parse_number(scanner);
  }
  return 0;
}
...
int64_t agent_json_parse_bytes(ZeroByteView input) {
  ...
  if (!agent_json_parse_value(&scanner, 0)) return -1;
  agent_json_skip_ws(&scanner);
  if (scanner.pos != scanner.len || scanner.tokens > INT64_MAX) return -1;
  return (int64_t)scanner.tokens;
}

static int agent_http_header_token_char(unsigned char ch) {
  return (ch >= 'A' && ch <= 'Z') ||
         (ch >= 'a' && ch <= 'z') ||
         (ch >= '0' && ch <= '9') ||
         ch == '!' || ch == '#' || ch == '$' || ch == '%' ||
...
static unsigned char agent_http_header_lower(unsigned char ch) {
  return ch >= 'A' && ch <= 'Z' ? (unsigned char)(ch + ('a' - 'A')) : ch;
}

static int agent_http_header_name_equal(const unsigned char *ptr, size_t len, ZeroByteView name) {
  if (!ptr || !name.ptr || len != name.len || len == 0) return 0;
  for (size_t i = 0; i < len; i++) {
    unsigned char a = ptr[i];
    unsigned char b = name.ptr[i];
    if (!agent_http_header_token_char(b)) return 0;
    if (agent_http_header_lower(a) != agent_http_header_lower(b)) return 0;
  }
  return 1;
}

static uint64_t agent_http_header_pack(size_t offset, size_t len) {
  if (offset > 0x7fffffffu || len > 0xffffffffu) return 0;
  return (1ull << 63) | ((uint64_t)(uint32_t)offset << 32) | (uint64_t)(uint32_t)len;
}

static uint32_t agent_http_read_u32le(const unsigned char *ptr) {
...
static int agent_http_response_has_meta(ZeroByteView response) {
  if (!response.ptr || response.len < AGENT_HTTP_RESPONSE_META_BYTES) return 0;
...
static size_t agent_http_raw_header_len(ZeroByteView response) {
...
uint32_t agent_http_response_len(ZeroByteView response) {
  if (!agent_http_response_has_meta(response)) {
    return response.len > 0xffffffffu ? 0xffffffffu : (uint32_t)response.len;
  }
  uint32_t headers_len = agent_http_read_u32le(response.ptr + 8);
  uint32_t body_len = agent_http_read_u32le(response.ptr + 12);
...
uint32_t agent_http_response_headers_len(ZeroByteView response) {
  if (agent_http_response_has_meta(response)) return agent_http_read_u32le(response.ptr + 8);
  size_t len = agent_http_raw_header_len(response);
...
uint32_t agent_http_response_body_offset(ZeroByteView response) {
  if (agent_http_response_has_meta(response)) {
    uint32_t headers_len = agent_http_read_u32le(response.ptr + 8);
    return AGENT_HTTP_RESPONSE_META_BYTES + headers_len;
  }
  size_t offset = agent_http_raw_header_len(response);
...
uint64_t agent_http_header_value(ZeroByteView headers, ZeroByteView name) {
  ...
  for (size_t i = 0; i < name.len; i++) {
    if (!agent_http_header_token_char(name.ptr[i])) return 0;
  }

  size_t header_offset = 0;
  size_t header_len = headers.len;
  if (agent_http_response_has_meta(headers)) {
    header_offset = AGENT_HTTP_RESPONSE_META_BYTES;
    header_len = agent_http_read_u32le(headers.ptr + 8);
  ...
  while (pos < limit) {
    ...
    if (!agent_http_header_name_equal(headers.ptr + line_start, colon - line_start, name)) continue;
    ...
    return agent_http_header_pack(value_start, value_end - value_start);
  }
  return 0;
}
...
uint32_t agent_http_header_offset(uint64_t value) {
  return agent_http_header_found(value) ? (uint32_t)((value >> 32) & 0x7fffffffu) : 0u;
}

uint32_t agent_http_header_len(uint64_t value) {
  return agent_http_header_found(value) ? (uint32_t)(value & 0xffffffffu) : 0u;
}

uint32_t agent_http_result_ok(uint64_t result) {
  uint32_t status = agent_http_result_status(result);
  return agent_http_result_error(result) == AGENT_HTTP_OK && status >= 200 && status < 300 ? 1u : 0u;
}
