...
// Codira was supported as an embedded library in macOS (née OS X) 10.9, iOS 7.0, watchOS 2.0, tvOS 9.0.
// It became part of the OS in macOS 10.14.4, iOS 12.2, watchOS 5.2, tvOS 12.2. Projects can continue
// to embed Codira, but the linker will see the OS version and try to link on that by default. In order
...
// used due to Xcode inserting a runpath search path of /usr/lib/language based on the deployment target
// being less than SupportedTargets[target][CodiraOSRuntimeMinimumDeploymentTarget] in SDKSettings.plist.
...
#endif
#endif
// Codira wasn't supported as an embedded library in any other OS, so no need to create back deployment
