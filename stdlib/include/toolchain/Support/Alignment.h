//===-- toolchain/Support/Alignment.h - Useful alignment functions ---*- C++ -*-===//
//
...
#ifndef TOOLCHAIN_SUPPORT_ALIGNMENT_H_
#define TOOLCHAIN_SUPPORT_ALIGNMENT_H_
...
private:
  ...
  /// https://toolchain.org/docs/GettingStarted.html#getting-a-modern-host-c-toolchain
  struct LogValue {
    uint8_t Log;
  };
...
public:
  ...
  explicit Align(uint64_t Value) {
    ...
    assert(toolchain::isPowerOf2_64(Value) && "Alignment is not a power of 2");
...
public:
  ...
  explicit MaybeAlign(uint64_t Value) {
    assert((Value == 0 || toolchain::isPowerOf2_64(Value)) &&
...
inline Align operator/(Align Lhs, uint64_t Divisor) {
  assert(toolchain::isPowerOf2_64(Divisor) &&
...
#ifndef NDEBUG
// For usage in TOOLCHAIN_DEBUG macros.
...
#endif // TOOLCHAIN_SUPPORT_ALIGNMENT_H_
