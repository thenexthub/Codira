...
#ifndef TOOLCHAIN_ADT_HASHING_H
#define TOOLCHAIN_ADT_HASHING_H
...
public:
  ...
  char *combine_data(size_t &length, char *buffer_ptr, char *buffer_end, T data) {
    if (!store_and_advance(buffer_ptr, buffer_end, data)) {
      ...
      if (!store_and_advance(buffer_ptr, buffer_end, data,
                             partial_store_size))
        toolchain_unreachable("buffer smaller than stored type");
    }
    return buffer_ptr;
  }
...
hash_code hash_value(const std::tuple<Ts...> &arg) {
  ...
  return ::toolchain::hashing::detail::hash_value_tuple_helper(
