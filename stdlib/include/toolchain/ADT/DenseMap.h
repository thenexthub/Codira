...
#ifndef TOOLCHAIN_ADT_DENSEMAP_H
#define TOOLCHAIN_ADT_DENSEMAP_H
...
private:
  ...
  BucketT *InsertIntoBucketImpl(const KeyT &Key, const LookupKeyT &Lookup,
    ...
    if (TOOLCHAIN_UNLIKELY(NewNumEntries * 4 >= NumBuckets * 3)) {
      this->grow(NumBuckets * 2);
      LookupBucketFor(Lookup, TheBucket);
      NumBuckets = getNumBuckets();
    } else if (TOOLCHAIN_UNLIKELY(NumBuckets-(NewNumEntries+getNumTombstones()) <=
                             NumBuckets/8)) {
  ...
  bool LookupBucketFor(const LookupKeyT &Val,
    ...
    while (true) {
      ...
      if (TOOLCHAIN_LIKELY(KeyInfoT::isEqual(Val, ThisBucket->getFirst()))) {
      ...
      if (TOOLCHAIN_LIKELY(KeyInfoT::isEqual(ThisBucket->getFirst(), EmptyKey))) {
...
#endif // TOOLCHAIN_ADT_DENSEMAP_H
