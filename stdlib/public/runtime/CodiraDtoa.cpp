...
///
/// For other formats, CodiraDtoa uses a modified form of the Grisu2
...
#if LANGUAGE_DTOA_BINARY32_SUPPORT || LANGUAGE_DTOA_BINARY64_SUPPORT || LANGUAGE_DTOA_FLOAT80_SUPPORT || LANGUAGE_DTOA_BINARY128_SUPPORT
...
#if LANGUAGE_DTOA_BINARY32_SUPPORT
...
#if LANGUAGE_DTOA_BINARY32_SUPPORT || LANGUAGE_DTOA_BINARY64_SUPPORT || LANGUAGE_DTOA_FLOAT80_SUPPORT || LANGUAGE_DTOA_BINARY128_SUPPORT
#if HAVE_UINT128_T
typedef __uint128_t language_uint128_t;
...
static void shiftLeft128(language_uint128_t *, int shift);
#endif
...
#if LANGUAGE_DTOA_BINARY64_SUPPORT
#if HAVE_UINT128_T
...
static int extractIntegerPart128(__uint128_t *fixed128, int integerBits) {
    ...
    const language_uint128_t fixedPointMask = (((__uint128_t)1 << fractionBits) - 1);
...
static int isLessThan128x128(language_uint128_t lhs, language_uint128_t rhs);
static void subtract128x128(language_uint128_t *lhs, language_uint128_t rhs);
static void multiply128xu32(language_uint128_t *lhs, uint32_t rhs);
...
static int extractIntegerPart128(language_uint128_t *fixed128, int integerBits) {
...
static language_uint128_t shiftRightRoundingDown128(language_uint128_t lhs, int shift);
static language_uint128_t shiftRightRoundingUp128(language_uint128_t lhs, int shift);
#endif
static language_uint128_t multiply128x64RoundingDown(language_uint128_t lhs, uint64_t rhs);
static language_uint128_t multiply128x64RoundingUp(language_uint128_t lhs, uint64_t rhs);
static void intervalContainingPowerOf10_Binary64(int p, language_uint128_t *lower, language_uint128_t *upper, int *exponent);
#endif
...
#if LANGUAGE_DTOA_FLOAT80_SUPPORT || LANGUAGE_DTOA_BINARY128_SUPPORT
#if HAVE_UINT128_T
// A 256-bit unsigned integer type stored as 3 64-bit words
typedef struct {uint64_t low, midlow, midhigh, high;} language_uint256_t;
...
static int extractIntegerPart256(language_uint256_t *fixed256, int integerBits) {
...
typedef struct { uint32_t elt[8]; } language_uint256_t; // [0]=low, [7]=high
#define initialize256WithHighMidLow64(dest, high64, midhigh64, midlow64, low64) \
...
#endif
static void multiply256xu32(language_uint256_t *lhs, uint32_t rhs);
// Multiply a 256-bit fraction times a 128-bit fraction, with controlled rounding
static void multiply256x128RoundingDown(language_uint256_t *lhs, language_uint128_t rhs);
static void multiply256x128RoundingUp(language_uint256_t *lhs, language_uint128_t rhs);
static void subtract256x256(language_uint256_t *lhs, language_uint256_t rhs);
static int isLessThan256x256(language_uint256_t lhs, language_uint256_t rhs);
static void shiftRightRoundingDown256(language_uint256_t *lhs, int shift);
static void shiftRightRoundingUp256(language_uint256_t *lhs, int shift);
static void intervalContainingPowerOf10_Binary128(int p, language_uint256_t *lower, language_uint256_t *upper, int *exponent);
static size_t _language_dtoa_256bit_backend(char *, size_t, language_uint128_t, language_uint128_t, int, int, int, int, bool);
#endif


// A table of all two-digit decimal numbers
#if LANGUAGE_DTOA_BINARY16_SUPPORT || LANGUAGE_DTOA_BINARY32_SUPPORT || LANGUAGE_DTOA_BINARY64_SUPPORT || LANGUAGE_DTOA_FLOAT80_SUPPORT || LANGUAGE_DTOA_BINARY128_SUPPORT
...
#if LANGUAGE_DTOA_BINARY16_SUPPORT
#if !LANGUAGE_DTOA_PASS_FLOAT16_AS_FLOAT
// Format a C `_Float16`
size_t language_dtoa_optimal_binary16(_Float16 d, char *dest, size_t length) {
  return language_dtoa_optimal_binary16_p(&d, dest, length);
}
#endif
...
// optimized.
size_t language_dtoa_optimal_binary16_p(const void *f, char *dest, size_t length) {
    ...
    // See comments in language_dtoa_optimal_binary64_p
...
// Format a C `float`
size_t language_dtoa_optimal_float(float d, char *dest, size_t length) {
  return language_dtoa_optimal_binary32_p(&d, dest, length);
}
#endif

// Format an IEEE 754 single-precision binary32 format floating-point number.
size_t language_dtoa_optimal_binary32_p(const void *f, char *dest, size_t length)
{
...
size_t language_dtoa_optimal_long_double(long double d, char *dest, size_t length) {
  return language_dtoa_optimal_binary64_p(&d, dest, length);
}
#endif
#if DOUBLE_IS_BINARY64
size_t language_dtoa_optimal_double(double d, char *dest, size_t length) {
  return language_dtoa_optimal_binary64_p(&d, dest, length);
}
#endif
...
// or library routines.
size_t language_dtoa_optimal_binary64_p(const void *d, char *dest, size_t length)
{
    ...
    language_uint128_t powerOfTenRoundedDown;
    language_uint128_t powerOfTenRoundedUp;
    int powerOfTenExponent = 0;
    ...
    if (isOddSignificand) {
        ...
        language_uint128_t u1 = multiply128x64RoundingDown(powerOfTenRoundedDown,
                                                    upperMidpointExact);
        ...
        language_uint128_t l1 = multiply128x64RoundingUp(powerOfTenRoundedUp,
                                                  lowerMidpointExact);
        ...
        language_uint128_t u1 = multiply128x64RoundingUp(powerOfTenRoundedUp,
                                                  upperMidpointExact);
        u = shiftRightRoundingUp128(u1, integerBits - extraBits);

        language_uint128_t l1 = multiply128x64RoundingDown(powerOfTenRoundedDown,
                                                    lowerMidpointExact);
    ...
    language_uint128_t t = u;
    language_uint128_t delta = u;
    ...
    if (!isLessThan128x128(delta, t)) {
      ...
      language_uint128_t d0 = delta;
      multiply128xu32(&d0, bulkDigitFactor);
      language_uint128_t t0 = t;
...
#if LANGUAGE_DTOA_FLOAT80_SUPPORT
#if LONG_DOUBLE_IS_FLOAT80
size_t language_dtoa_optimal_long_double(long double d, char *dest, size_t length) {
  return language_dtoa_optimal_float80_p(&d, dest, length);
}
#endif
...
size_t language_dtoa_optimal_float80_p(const void *d, char *dest, size_t length)
{
    static const int exponentBitCount = 15;
    static const int exponentMask = (1 << exponentBitCount) - 1;
    // See comments in language_dtoa_optimal_binary64_p to understand
    ...
    language_uint128_t upperMidpointExact, lowerMidpointExact;
    ...
    return _language_dtoa_256bit_backend
      (
       dest,
       length,
       upperMidpointExact,
...
#if LANGUAGE_DTOA_BINARY128_SUPPORT
#if LONG_DOUBLE_IS_BINARY128
size_t language_dtoa_optimal_long_double(long double d, char *dest, size_t length) {
  return language_dtoa_optimal_binary128_p(&d, dest, length);
}
#endif
...
// This does not rely on the C environment for floating-point arithmetic
// or library support of any kind.
size_t language_dtoa_optimal_binary128_p(const void *d, char *dest, size_t length)
{
    static const int exponentBitCount = 15;
    static const int exponentMask = (1 << exponentBitCount) - 1;
    // See comments in language_dtoa_optimal_binary64_p to understand
...
#if LANGUAGE_DTOA_FLOAT80_SUPPORT || LANGUAGE_DTOA_BINARY128_SUPPORT
static size_t _language_dtoa_256bit_backend
(
 char *dest,
 size_t length,
 language_uint128_t upperMidpointExact,
 language_uint128_t lowerMidpointExact,
 int negative,
 int isBoundary,
...
)
{
    ...
    // Step 4: Compute a power-of-10 scale factor
    language_uint256_t powerOfTenRoundedDown;
    language_uint256_t powerOfTenRoundedUp;
    int powerOfTenExponent = 0;
    ...
    language_uint256_t t = u;
    language_uint256_t delta = u;
    ...
    *p++ = '0' + extractIntegerPart256(&t, integerBits);
    ...
    language_uint256_t d0 = delta;
    multiply256xu32(&d0, 10000);
    language_uint256_t t0 = t;
...
static language_uint128_t multiply128x64RoundingDown(language_uint128_t lhs, uint64_t rhs) {
    ...
    language_uint128_t h = (language_uint128_t)lhsh * rhs;
    language_uint128_t l = (language_uint128_t)lhsl * rhs;
    return h + (l >> 64);
#else
    language_uint128_t result;
    static const uint64_t mask32 = UINT32_MAX;
...
#if (LANGUAGE_DTOA_BINARY32_SUPPORT || LANGUAGE_DTOA_BINARY64_SUPPORT) && !HAVE_UINT128_T
static void shiftLeft128(language_uint128_t *lhs, int shift) {
    // Note: Shift is always less than 32
...
static void multiply256x128RoundingUp(language_uint256_t *lhs, language_uint128_t rhs) {
    ...
    language_uint128_t current = (language_uint128_t)lhs->low * (uint64_t)rhs;
    current += UINT64_MAX;

    current = (current >> 64);
    language_uint128_t t = (language_uint128_t)lhs->low * (rhs >> 64);
    current += (uint64_t)t;
    language_uint128_t next = t >> 64;
    t = (language_uint128_t)lhs->midlow * (uint64_t)rhs;
    current += (uint64_t)t;
    next += t >> 64;
    ...
    current = next + (current >> 64);
    t = (language_uint128_t)lhs->midlow * (rhs >> 64);
    current += (uint64_t)t;
    next = t >> 64;
    t = (language_uint128_t)lhs->midhigh * (uint64_t)rhs;
    current += (uint64_t)t;
    next += t >> 64;
    lhs->low = (uint64_t)current;

    current = next + (current >> 64);
    t = (language_uint128_t)lhs->midhigh * (rhs >> 64);
    current += (uint64_t)t;
    next = t >> 64;
    t = (language_uint128_t)lhs->high * (uint64_t)rhs;
    current += (uint64_t)t;
    next += t >> 64;
    lhs->midlow = (uint64_t)current;

    current = next + (current >> 64);
    t = (language_uint128_t)lhs->high * (rhs >> 64);
    current += t;
    lhs->midhigh = (uint64_t)current;
    lhs->high = (uint64_t)(current >> 64);
#else
...
static void subtract256x256(language_uint256_t *lhs, language_uint256_t rhs) {
#if HAVE_UINT128_T
    language_uint128_t t = (language_uint128_t)lhs->low + (~rhs.low) + 1;
    lhs->low = t;
