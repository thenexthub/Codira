...
typedef int (*lhash_cmp_func_helper)(lhash_cmp_func fn, const void *a,
                                     const void *b);
...
typedef uint32_t (*lhash_hash_func_helper)(lhash_hash_func fn, const void *a);
...
// OPENSSL_lh_doall_arg calls |fn| on each element of the hash table and also
...
#define DEFINE_LHASH_OF(type)                                                  \
  ...
  typedef uint32_t (*lhash_##type##_hash_func)(const type *);                  \
                                                                               \
  inline int lh_##type##_call_cmp_func(lhash_cmp_func fn, const void *a,     \
                                       const void *b) {                        \
    return ((lhash_##type##_cmp_func)fn)((const type *)a, (const type *)b);  \
  }                                                                            \
                                                                               \
  inline uint32_t lh_##type##_call_hash_func(lhash_hash_func fn,             \
                                             const void *a) {                  \
    return ((lhash_##type##_hash_func)fn)((const type *)a);                  \
  }                                                                            \
                                                                               \
  ...
  inline void lh_##type##_doall_arg(LHASH_OF(type) *lh,                        \
                                    void (*fn)(type *, void *), void *arg) { \
    LHASH_DOALL_##type cb = {fn, arg};                                       \
