...
// sk_SAMPLE_delete_if calls |fn| with each element of |sk| and removes the
// entries where |fn| returned one. This function does not free or return
// removed pointers so, if |sk| owns its contents, |fn| should release the
// pointers prior to returning one.
void sk_SAMPLE_delete_if(STACK_OF(SAMPLE) *sk, sk_SAMPLE_delete_if_func fn,
                         void *data);
...
OPENSSL_EXPORT void OPENSSL_sk_delete_if(
    ...
    OPENSSL_sk_delete_if_func fn, void *data);
...
#define BORINGSSL_DEFINE_STACK_OF_IMPL(name, ptrtype, constptrtype)            \
  ...
  OPENSSL_INLINE int sk_##name##_call_delete_if_func(                          \
      OPENSSL_sk_delete_if_func fn, void *obj, void *data) {                 \
    return ((sk_##name##_delete_if_func)fn)((ptrtype)obj, data);             \
  }                                                                            \
                                                                               \
  ...
  OPENSSL_INLINE void sk_##name##_delete_if(                                   \
      STACK_OF(name) *sk, sk_##name##_delete_if_func fn, void *data) {       \
    OPENSSL_sk_delete_if((OPENSSL_STACK *)sk, sk_##name##_call_delete_if_func, \
                         (OPENSSL_sk_delete_if_func)fn, data);               \
  }                                                                            \
                                                                               \
