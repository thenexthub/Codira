...
struct AssertionInfoMatcher
{
  ...
  constexpr AssertionInfoMatcher(const char* msg, const char* file = any_file, int line = any_line)
      : is_empty_(false)
      , msg_(msg, __builtin_strlen(msg))
      , file_(file, __builtin_strlen(file))
      , line_(line)
  {}
...
#define CASE(K) \
  ...
  DeathTest(AssertionInfoMatcher const& Matcher)
      : matcher_(Matcher)
  {}
  ...
  int getChildExitCode() const
  {
    return exit_code_;
  }
  std::string const& getChildStdOut() const
  {
    return stdout_from_child_;
  }
  std::string const& getChildStdErr() const
  {
    return stderr_from_child_;
  }
...
#define EXPECT_DEATH(...)                   \
  assert((ExpectDeath(#__VA_ARGS__, [&]() { \
    __VA_ARGS__;                            \
  })))
