...
__host__ __device__ constexpr void test_overflow()
{
  ...
  {
    item.str_signed   = "32768";
    item.str_unsigned = "65536";
...
__host__ __device__ constexpr bool test_base()
{
  ...
  {
    test_from_chars<cuda::std::int16_t>(item, Base);
    test_from_chars<cuda::std::uint16_t>(item, Base);
  }

  // Test overflow cases (base 10 only)
  if constexpr (Base == 10)
  {
    test_overflow<cuda::std::int16_t>();
    test_overflow<cuda::std::uint16_t>();
  }
