...
template <class SV>
__host__ __device__ constexpr void test_find_last_of(const SV& sv, const SV& str, typename SV::size_type x)
{
...
test_find_last_of(const SV& sv, const SV& str, typename SV::size_type pos, typename SV::size_type x)
{
...
#define TEST_FIND_LAST_OF(SV_T, SV1_STR, SV2_STR, ...)                     \
  test_find_last_of(SV_T(TEST_STRLIT(typename SV_T::value_type, SV1_STR)), \
...
__host__ __device__ constexpr void test_find_last_of()
{
  ...
  static_assert(cuda::std::is_same_v<SizeT, decltype(SV{}.find_last_of(SV{}))>);
  static_assert(cuda::std::is_same_v<SizeT, decltype(SV{}.find_last_of(SV{}, SizeT{}))>);

  static_assert(noexcept(SV{}.find_last_of(SV{})));
  static_assert(noexcept(SV{}.find_last_of(SV{}, SizeT{})));
