...
__host__ __device__ void test_edges()
{
  ...
  {
    cuda::std::complex<T> r = exp(testcases[i]);
    ...
    else if (cuda::std::isinf(testcases[i].real()) && testcases[i].real() > T(0) && testcases[i].imag() == T(0))
    {
    ...
    else if (cuda::std::isfinite(testcases[i].imag()) && cuda::std::abs(testcases[i].imag()) <= T(1))
    {
      assert(!cuda::std::signbit(r.real()));
      assert(cuda::std::signbit(r.imag()) == cuda::std::signbit(testcases[i].imag()));
    }
    else if (cuda::std::isinf(r.real()) && testcases[i].imag() == T(0))
    {
      assert(r.imag() == T(0));
      assert(cuda::std::signbit(testcases[i].imag()) == cuda::std::signbit(r.imag()));
    }
