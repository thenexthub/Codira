...
void test_ctor_throws()
{
  ...
  try
  {
    ...
    cuda::std::uninitialized_default_construct(It(p), It(p + N));
    assert(false);
  }
  catch (...)
  {}
...
__host__ __device__ void test_counted()
{
  ...
  cuda::std::uninitialized_default_construct(It(p), It(p + 1));
  assert(Counted_count == 1);
  assert(Counted_constructed == 1);
  cuda::std::uninitialized_default_construct(It(p + 1), It(p + N));
