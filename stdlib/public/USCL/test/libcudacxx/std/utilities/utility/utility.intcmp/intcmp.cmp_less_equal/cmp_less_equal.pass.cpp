...
__host__ __device__ constexpr void test1()
{
  ...
  assert(!cuda::std::cmp_less_equal(-2, tup.min) == cuda::std::is_signed<T>::value);
  assert(cuda::std::cmp_less_equal(tup.min, -2) == cuda::std::is_signed<T>::value);
...
int main(int, char**)
{
  static_assert(noexcept(cuda::std::cmp_less_equal(0, 1)));
  test();
  static_assert(test(), "");
  return 0;
}
