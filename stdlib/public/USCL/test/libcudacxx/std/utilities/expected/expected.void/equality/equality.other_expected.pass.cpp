...
__host__ __device__ constexpr bool test()
{
  ...
  {
    const cuda::std::expected<void, int> e1;
    const cuda::std::expected<void, int> e2;
    assert(e1 == e2);
  }
  ...
  {
    const cuda::std::expected<void, int> e1;
    const cuda::std::expected<void, int> e2(cuda::std::unexpect, 10);
    const cuda::std::expected<void, int> e3(cuda::std::unexpect, 5);
    assert(e1 != e2);
    assert(e1 != e3);
  }
