...
// Test Constraints:
static_assert(cuda::std::is_constructible_v<cuda::std::expected<cuda::std::inplace_vector<int, 3>, int>,
                                            cuda::std::in_place_t,
...
static_assert(
  !cuda::std::is_constructible_v<cuda::std::expected<int, int>, cuda::std::in_place_t, cuda::std::initializer_list<int>>,
  "");
...
static_assert(!ImplicitlyConstructible<cuda::std::expected<cuda::std::inplace_vector<int, 3>, int>,
                                       cuda::std::in_place_t,
                                       cuda::std::initializer_list<int>>,
              "");
...
__host__ __device__ constexpr bool test()
{
  ...
  // multi args
  {
    ...
    int j = 6;
    MoveOnly m(7);
    cuda::std::expected<Data<2, int&, int&&, MoveOnly>, int> e(
      cuda::std::in_place, {1, 2}, i, cuda::std::move(j), cuda::std::move(m));
    assert(e.has_value());
