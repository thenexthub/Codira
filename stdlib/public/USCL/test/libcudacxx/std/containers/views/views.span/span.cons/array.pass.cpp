...
__host__ __device__ void checkCV()
{
  //  Types the same (dynamic sized)
  {
    ...
    cuda::std::span<const volatile int> s4{cvarr}; // a cuda::std::span<const volatile int> pointing at const volatile
                                                   // int.
  ...
  //  Types the same (static sized)
  {
    ...
    cuda::std::span<const volatile int, 3> s4{cvarr}; // a cuda::std::span<const volatile int> pointing at const
                                                      // volatile int.
    assert(s1.size() + s2.size() + s3.size() + s4.size() == 12);
  }
