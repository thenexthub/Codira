...
{
  int data;
  cuda::std::mdspan<int, cuda::std::extents<int>, cuda::std::layout_right, convertible_accessor_but_not_handle<int>>
    m_nc(&data);
...
{
  int data;
  cuda::std::mdspan<int, cuda::std::extents<int>, always_convertible_layout> m(&data);
  // expected-error-re@*:* {{{{(static_assert|static assertion)}} failed {{.*}}mdspan: incompatible extents for mdspan
  // construction}}
  cuda::std::mdspan<int, cuda::std::extents<int, 5>, always_convertible_layout> m2(m);
  unused(m2);
}
