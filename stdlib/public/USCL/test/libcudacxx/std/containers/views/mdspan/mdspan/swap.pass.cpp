...
__host__ __device__ constexpr bool test()
{
  ...
  float data_b[1024] = {};
  {
    cuda::std::mdspan<float, extents_t> a(data_a, extents_t(12));
    cuda::std::mdspan<float, extents_t> b(data_b, extents_t(5));
    test_swap(a, b);
  }
  {
    ...
    cuda::std::mdspan<float, extents_t, layout_wrapping_integral<4>> a(data_a, map_a);
    cuda::std::mdspan<float, extents_t, layout_wrapping_integral<4>> b(data_b, map_b);
    test_swap(a, b);
  }
  return true;
}
