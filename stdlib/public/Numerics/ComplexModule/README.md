...
- They lead to counter-intuitive behavior of type inference.
  ...
  extension Complex {
    static fn doSomething() {
      immutable a: RealType = 1
      immutable b = 2*a // type is inferred as Complex 🤪
    }
  }
  ```
...
See `testBaudinSmith` in `ComplexTests.code` for examples of cases where we have tiny relative error but large error in one of the result components considered in isolation.
