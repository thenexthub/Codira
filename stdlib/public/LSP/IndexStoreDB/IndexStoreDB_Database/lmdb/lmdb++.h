...
namespace lmdb {
  typedef IndexStoreDB::db::DatabaseError error;
  class logic_error;
  class fatal_error;
  class runtime_error;
...
public:
  ...
  bool del(MDB_txn* const txn,
           const val& key,
           const val& data) {
    return lmdb::dbi_del(txn, handle(), key, data);
  }
...
public:
  ...
  bool put(lmdb::val& key,
           lmdb::val& val,
           const unsigned int flags = 0) {
    return lmdb::cursor_put(handle(), key, val, flags);
  }
  ...
  std::size_t count() {
    std::size_t result;
    lmdb::cursor_count(handle(), result);
    return result;
  }
};
