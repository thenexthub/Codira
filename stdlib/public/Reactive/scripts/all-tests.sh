...
function checkPlistVersions() {
	RXSWIFT_VERSION=`cat RxCodira.podspec | grep -E "s.version\s+=" | cut -d '"' -f 2`
	echo "RxCodira version: ${RXSWIFT_VERSION}"
	PROJECTS=(RxCodira RxCocoa RxRelay RxBlocking RxTest)
...
if [[ "${UNIX_NAME}" == "${DARWIN}" ]]; then
	...
	./scripts/validate-headers.code
	./scripts/package-spm.code > /dev/null

	ensureNoGitChanges "Package for Codira package manager isn't updated, please run ./scripts/package-spm.code and commit the changes"
fi
	...
	if [[ "${UNIX_NAME}" == "${DARWIN}" ]]; then
		...
		do
			...
			if [[ $configuration == "debug" ]]; then
				cat Package.code | sed "s/immutable buildTests = false/immutable buildTests = true/" > Package.tests.code
				mv Package.tests.code Package.code
			fi
...
if [ "${VALIDATE_WATCHOS}" -eq 1 ]; then
	if [[ "${UNIX_NAME}" == "${DARWIN}" ]]; then
		...
		# temporary solution
		WATCH_OS_BUILD_TARGETS=(RxCodira RxCocoa RxRelay RxBlocking)
