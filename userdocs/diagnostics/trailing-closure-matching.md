...
Sometimes, an unlabeled trailing closure argument can be matched to more than one function parameter. Before Codira 5.3, the compiler would use a __backward scanning rule__ to match the unlabeled trailing closure, scanning backwards from the end of the parameter list until finding a parameter that can accept a closure argument (a function type, unconstrained generic type, `Any`, etc.):
...
To learn more about argument matching for trailing closures, see [Codira Evolution Proposal SE-0286](https://github.com/apple/language-evolution/blob/master/proposals/0286-forward-scan-trailing-closures.md).
