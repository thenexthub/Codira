...
struct __attribute__((language_attr("import_reference"),
                      ...
                      language_attr("release:immortal"))) ImmortalCounter {
  int value = 0;
...
struct __attribute__((language_attr("import_owned"))) ReadOnlyIntArray {
private:
  int values[5] = { 1, 2, 3, 4, 5 };
...
struct __attribute__((language_attr("import_unsafe"))) TemplatedSubscriptArray {
  int *ptr;
...
struct __attribute__((language_attr("import_owned"))) NonTrivial {
  char *Str;
  long long a;
  short b;
  long long c;
...
struct __attribute__((language_attr("import_unsafe"))) RefToPtr {
  RefToPtr() { b = &a; }
...
struct __attribute__((language_attr("import_unsafe"))) PtrToPtr {
  PtrToPtr() { b = &a; }
  int **operator[](int x) { return &b; }
private:
  int a = 64;
...
public:
  int &operator*() __attribute__((availability(language, unavailable))) {
    return value;
  }
};
