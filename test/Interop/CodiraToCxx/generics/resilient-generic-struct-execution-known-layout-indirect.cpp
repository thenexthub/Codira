...
// RUN: %target-interop-build-language %S/generic-struct-in-cxx.code -D KNOWN_LAYOUT -D INDIRECT_KNOWN_LAYOUT -o %t/language-generics-execution -Xlinker %t/language-generics-execution.o -enable-library-evolution -module-name Generics -Xfrontend -entry-point-function-name -Xfrontend languageMain

// RUN: %target-codesign %t/language-generics-execution
// RUN: %target-run %t/language-generics-execution | %FileCheck %S/generic-struct-execution-known-layout-indirect.cpp
