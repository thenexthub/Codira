...
// RUN: %target-language-frontend %S/generic-function-in-cxx.code -module-name Functions -clang-header-expose-decls=all-public -typecheck -verify -emit-clang-header-path %t/functions.h

// RUN: %target-interop-build-clangxx -std=gnu++20 -c %s -I %t -o %t/language-functions-execution.o
// RUN: %target-interop-build-language %S/generic-function-in-cxx.code -o %t/language-functions-execution -Xlinker %t/language-functions-execution.o -module-name Functions -Xfrontend -entry-point-function-name -Xfrontend languageMain

// RUN: %target-codesign %t/language-functions-execution
// RUN: %target-run %t/language-functions-execution | %FileCheck %s
...
// CHECK-NEXT: deinit TestClass

  {
    ...
    assert(language::_impl::_impl_RefCountedClass::getOpaquePointer(tc) ==
           language::_impl::_impl_RefCountedClass::getOpaquePointer(tc2));
...
// CHECK-NEXT: deinit TestClass

  {
    ...
    const auto p1 = language::_impl::_impl_RefCountedClass::getOpaquePointer(tc);
    const auto p2 = language::_impl::_impl_RefCountedClass::getOpaquePointer(tc2);
    assert(p1 != p2);
    genericSwap(tc, tc2);
    assert(p2 == language::_impl::_impl_RefCountedClass::getOpaquePointer(tc));
    assert(p1 == language::_impl::_impl_RefCountedClass::getOpaquePointer(tc2));
  }
// CHECK-NEXT: deinit TestClass
// CHECK-NEXT: deinit TestClass
