...
// RUN: %target-language-frontend %S/generic-struct-in-cxx.code -module-name Generics -clang-header-expose-decls=all-public -typecheck -verify -emit-clang-header-path %t/generics.h

// RUN: %target-interop-build-clangxx -std=gnu++20 -c %s -I %t -o %t/language-generics-execution.o
// RUN: %target-interop-build-language %S/generic-struct-in-cxx.code -o %t/language-generics-execution -Xlinker %t/language-generics-execution.o -module-name Generics -Xfrontend -entry-point-function-name -Xfrontend languageMain
