...
// RUN: %target-language-frontend %S/small-structs-pass-return-direct-in-cxx.code -module-name Structs -clang-header-expose-decls=all-public -typecheck -verify -emit-clang-header-path %t/structs.h

// RUN: %target-interop-build-clangxx -c %s -I %t -o %t/language-structs-execution.o -Wno-incompatible-pointer-types
// RUN: %target-interop-build-language %S/small-structs-pass-return-direct-in-cxx.code -o %t/language-structs-execution -Xlinker %t/language-structs-execution.o -module-name Structs -Xfrontend -entry-point-function-name -Xfrontend languageMain
