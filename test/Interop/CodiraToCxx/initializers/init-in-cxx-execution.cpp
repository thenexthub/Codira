...
// RUN: %target-language-frontend %S/init-in-cxx.code -module-name Init -clang-header-expose-decls=all-public -typecheck -verify -emit-clang-header-path %t/inits.h

// RUN: %target-interop-build-clangxx -c %s -I %t -o %t/language-init-execution.o
// RUN: %target-interop-build-language %S/init-in-cxx.code -o %t/language-init-execution -Xlinker %t/language-init-execution.o -module-name Init -Xfrontend -entry-point-function-name -Xfrontend languageMain

// RUN: %target-codesign %t/language-init-execution
// RUN: %target-run %t/language-init-execution | %FileCheck %s
