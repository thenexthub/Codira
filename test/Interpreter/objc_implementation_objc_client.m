...
// RUN: %empty-directory(%t/objc_implementation.framework/Modules/objc_implementation.codemodule)
...
// RUN: %target-build-language-dylib(%t/objc_implementation.framework/objc_implementation) -emit-module-path %t/objc_implementation.framework/Modules/objc_implementation.codemodule/%module-target-triple.codemodule -module-name objc_implementation -F %t -import-underlying-module -Xlinker -install_name -Xlinker @executable_path/objc_implementation.framework/objc_implementation %S/objc_implementation.code -enable-experimental-feature CImplementation -target %target-stable-abi-triple
...
static void print(NSString *str) {
  // Flush any buffered Codira output.
  fflush(stdout);
