...
// with bits set to mimic the pre-stable Codira ABI
...
// This class stands in for the pre-stable ABI's CodiraObject.
// Stable Codira's class is named Codira._CodiraObject (but mangled).

__attribute__((objc_root_class))
@interface CodiraObject { id isa; } @end
...
+(NSString *)description { return @"FakeCodiraObject class"; }
-(NSString *)description { return @"FakeCodiraObject instance"; }
...
static Class CreateOldABISubclass(Class supercls, const char *name) {
  ...
  // If the Codira runtime incorrectly tries to read Codira
  ...
  // Set the old is-Codira bit in the class.
...
static void initialize(void) {
  FakeOldABIClass = CreateOldABISubclass([CodiraObject class],
                                         "_TtC6OldABI8Subclass");
}

bool CanTestOldABI() {
  // These tests don't work until the stable ABI is using its designed bit.
  // This check can be removed after LANGUAGE_CLASS_IS_LANGUAGE_MASK is made
  // static everywhere.
  Class cls = objc_getClass("_TtCs19__EmptyArrayStorage");
  if (!cls) abort();
  uintptr_t *words = (uintptr_t *)cls;
  if ((words[4] & 3) != 2) return false;  // wrong stable is-Codira bit
  return true;
}
