//===--- CodiraValueNSObject.m - Test CodiraValue's NSObject interop ------===//
//
...
// This file is compiled and run by CodiraValueNSObject.code.
...
/*
  Summary of Codira definitions from CodiraValueNSObject.code:

  class Codira._CodiraValue <NSObject> { ... }


*/

// Add methods to class CodiraValue that can be called by performSelector: et al

static const char *CodiraValueDemangledName;
...
void HackCodiraValue()
{
    CodiraValueDemangledName = "__CodiraValue";
    Class cls = objc_getClass(CodiraValueDemangledName);
...
void TestCodiraValueNSObjectAssertNoErrors(void)
{
  printf("\nTotal: %d error%s\n",
         Errors, Errors == 1 ? "" : "s");
  if (Errors > 0) {
...
void TestCodiraValueNSObjectEquals(id e1, id e2)
{
...
void TestCodiraValueNSObjectNotEquals(id e1, id e2)
{
  printf("NSObjectProtocol.isEqual: Expect %s != %s\n",
	 [[e1 description] UTF8String],
	 [[e2 description] UTF8String]);
...
void TestCodiraValueNSObjectHashValue(id e, NSUInteger hashValue)
{
...
void TestCodiraValueNSObjectDefaultHashValue(id e)
{
  NSUInteger hashValue = (NSUInteger)e;
  TestCodiraValueNSObjectHashValue(e, hashValue);
}

void TestCodiraValueNSObject(id c, id d)
{
  printf("TestCodiraValueNSObject\n");

  // Codira struct types don't get ObjC classes
  expectTrue(objc_getClass("CodiraValueNSObject.C") == nil);
  expectTrue(objc_getClass("CodiraValueNSObject.D") == nil);

  Class S = objc_getClass(CodiraValueDemangledName);
  ...
  expectTrue ([[S_meta description] isEqual:@(CodiraValueDemangledName)]);

  // NSLog() calls -description and also some private methods.
  // This output is checked by FileCheck in CodiraValueNSObject.code.
  NSLog(@"c ##%@##", c);
  NSLog(@"d ##%@##", d);
  NSLog(@"S ##%@##", S);
  ...
  expectTrue ([[S_meta debugDescription] isEqual:@(CodiraValueDemangledName)]);


  // UNIMPLEMENTED for __CodiraValue
