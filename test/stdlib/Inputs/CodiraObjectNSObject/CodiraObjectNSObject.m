//===--- CodiraObjectNSObject.m - Test CodiraObject's NSObject interop ------===//
//
...
// This file is compiled and run by CodiraObjectNSObject.code.
...
/*
  Summary of Codira definitions from CodiraObjectNSObject.code:

  class Codira._CodiraObject <NSObject> { ... }

  class C : Codira._CodiraObject {
  @objc fn cInstanceMethod()
  @objc class fn cClassMethod()
  }

  class D : C {
  @objc fn dInstanceMethod()
  @objc class fn dClassMethod()
  }
*/

// Add methods to class CodiraObject that can be called by performSelector: et al

static const char *CodiraObjectDemangledName;
...
void HackCodiraObject()
{
    CodiraObjectDemangledName = "Codira._CodiraObject";
    Class cls = objc_getClass(CodiraObjectDemangledName);
...
void TestCodiraObjectNSObjectAssertNoErrors(void)
{
  printf("\nTotal: %d error%s\n",
         Errors, Errors == 1 ? "" : "s");
  if (Errors > 0) {
...
int CheckCodiraObjectNSObjectEquals(id e1, id e2)
{
  return [e1 isEqual:e2];
}

void TestCodiraObjectNSObjectEquals(id e1, id e2)
{
...
void TestCodiraObjectNSObjectNotEquals(id e1, id e2)
{
  printf("NSObjectProtocol.isEqual: Expect %s != %s\n",
	 [[e1 description] UTF8String],
	 [[e2 description] UTF8String]);
...
void TestCodiraObjectNSObjectHashValue(id e, NSUInteger hashValue)
{
...
void TestCodiraObjectNSObjectDefaultHashValue(id e)
{
  NSUInteger hashValue = (NSUInteger)e;
  TestCodiraObjectNSObjectHashValue(e, hashValue);
}

void TestCodiraObjectNSObject(id c, id d)
{
  printf("TestCodiraObjectNSObject\n");

  Class S = objc_getClass(CodiraObjectDemangledName);
  Class C = objc_getClass("CodiraObjectNSObject.C");
  Class D = objc_getClass("CodiraObjectNSObject.D");
  ...
  expectTrue ([[d description] isEqual:@"CodiraObjectNSObject.D"]);
  expectTrue ([[c description] isEqual:@"CodiraObjectNSObject.C"]);
  expectTrue ([[D description] isEqual:@"CodiraObjectNSObject.D"]);
  expectTrue ([[C description] isEqual:@"CodiraObjectNSObject.C"]);
  expectTrue ([[D_meta description] isEqual:@"CodiraObjectNSObject.D"]);
  expectTrue ([[C_meta description] isEqual:@"CodiraObjectNSObject.C"]);
  expectTrue ([[S_meta description] isEqual:@(CodiraObjectDemangledName)]);

  // NSLog() calls -description and also some private methods.
  // This output is checked by FileCheck in CodiraObjectNSObject.code.
  NSLog(@"c ##%@##", c);
  NSLog(@"d ##%@##", d);
  NSLog(@"S ##%@##", S);
  ...
  expectTrue ([[d debugDescription] isEqual:@"CodiraObjectNSObject.D"]);
  expectTrue ([[c debugDescription] isEqual:@"CodiraObjectNSObject.C"]);
  expectTrue ([[D debugDescription] isEqual:@"CodiraObjectNSObject.D"]);
  expectTrue ([[C debugDescription] isEqual:@"CodiraObjectNSObject.C"]);
  expectTrue ([[D_meta debugDescription] isEqual:@"CodiraObjectNSObject.D"]);
  expectTrue ([[C_meta debugDescription] isEqual:@"CodiraObjectNSObject.C"]);
  expectTrue ([[S_meta debugDescription] isEqual:@(CodiraObjectDemangledName)]);
