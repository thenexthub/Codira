...
function(emit_language_interface target)
  ...
  target_compile_options(${target} PRIVATE
    "$<$<COMPILE_LANGUAGE:Codira>:SHELL:-emit-module-path ${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_MODULE_TRIPLE}.codemodule>")
  if(CodiraOverlay_VARIANT_MODULE_TRIPLE)
    target_compile_options(${target} PRIVATE
      "$<$<COMPILE_LANGUAGE:Codira>:SHELL:-emit-variant-module-path ${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_VARIANT_MODULE_TRIPLE}.codemodule>")
  endif()
  add_custom_command(OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_MODULE_TRIPLE}.codemodule"
    DEPENDS ${target})
  target_sources(${target}
    INTERFACE
      $<BUILD_INTERFACE:${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_MODULE_TRIPLE}.codemodule>)

  # Generate textual language interfaces is library-evolution is enabled
  if(CodiraOverlay_ENABLE_LIBRARY_EVOLUTION)
    target_compile_options(${target} PRIVATE
      $<$<COMPILE_LANGUAGE:Codira>:-emit-module-interface-path$<SEMICOLON>${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_MODULE_TRIPLE}.codeinterface>
      $<$<COMPILE_LANGUAGE:Codira>:-emit-private-module-interface-path$<SEMICOLON>${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_MODULE_TRIPLE}.private.codeinterface>)
    if(CodiraOverlay_VARIANT_MODULE_TRIPLE)
      target_compile_options(${target} PRIVATE
        "$<$<COMPILE_LANGUAGE:Codira>:SHELL:-emit-variant-module-interface-path ${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_VARIANT_MODULE_TRIPLE}.codeinterface>"
        "$<$<COMPILE_LANGUAGE:Codira>:SHELL:-emit-variant-private-module-interface-path ${CMAKE_CURRENT_BINARY_DIR}/${module_name}.codemodule/${CodiraOverlay_VARIANT_MODULE_TRIPLE}.private.codeinterface>")
    endif()
    target_compile_options(${target} PRIVATE
