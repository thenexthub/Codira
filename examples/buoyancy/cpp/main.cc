...
//    `/path/to/codira build --manifest-path examples/buoyancy/codira.toml --watch`
// 3. Run the application from the CLI:
//    `main /path/to/buoyancy.codiralib`
int main(int argc, char* argv[]) {
    ...
    options.functions.emplace_back(codira::RuntimeFunction("log_f32", log_f32));

    codira::Error error;
    if (auto runtime = codira::make_runtime(argv[1], options, &error)) {
        auto ctx = codira::invoke_fn<codira::StructRef>(*runtime, "new_sim").wait();
        ...
        while (true) {
            ...
            codira::invoke_fn<void>(*runtime, "sim_update", ctx, elapsed.count()).wait();
            previous = now;

            codira::Error update_error;
    ...
    std::cerr << "Failed to construct Codira runtime due to error: " << error.message().value()
              << std::endl;
    return 2;
}
