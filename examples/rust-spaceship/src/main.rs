...
use codira_runtime::{RootedStruct, Runtime, StructRef};
...
struct SpaceshipGame {
    codira_runtime: codira_runtime::Runtime,
...
impl State for SpaceshipGame {
    ...
    fn update(&mut self, ctx: &mut Context) -> tetra::Result {
        // Collect input to pass it into codira runtime
        let mut player_input = self.player_input.as_ref(&self.codira_runtime).clone();
        ...
        let mut game_struct = self.game_struct.as_ref(&self.codira_runtime).clone();

        if game_struct.get::<bool>("spawn_new_rocket").unwrap() {
            game_struct.set("spawn_new_rocket", false).unwrap();

            if !(self.rockets.len() >= self.codira_runtime.invoke("max_rockets_amount", ()).unwrap()) {
        ...
        if game_struct.get::<bool>("spawn_new_asteroids").unwrap() {
            ...
            self.asteroids = new_asteroids(&self.codira_runtime);
        }

        // Rockets update
        let rockets = self.codira_runtime.construct_typed_array(
            &self
                .codira_runtime
            ...
            self.rockets
                .iter()
                .map(|rocket| rocket.as_ref(&self.codira_runtime)),
        );
        ...
        // Asteroids update
        let asteroids = self.codira_runtime.construct_typed_array(
            &self
                .codira_runtime
            ...
            self.asteroids
                .iter()
                .map(|asteroid| asteroid.as_ref(&self.codira_runtime)),
        );
        ...
        let mut asteroids: Vec<RootedStruct> = self
            ...
            .iter()
            .map(|asteroid| {
                let asteroid = asteroid.as_ref(&self.codira_runtime).clone();
                asteroid.root()
            })
            .collect();
        ...
        for rocket in self.rockets.iter_mut() {
            for asteroid in self.asteroids.iter_mut() {
                let mut rocket = rocket.as_ref(&self.codira_runtime);
                let mut asteroid = asteroid.as_ref(&self.codira_runtime);
        ...
        unsafe { self.codira_runtime.update() };

        let new_player_input: StructRef = self.codira_runtime.invoke("new_player_input", ()).unwrap();
...
fn new_asteroids(runtime: &codira_runtime::Runtime) -> Vec<RootedStruct> {
...
fn main() -> tetra::Result {
    let runtime = Runtime::builder("codira/target/mod.codiralib")
