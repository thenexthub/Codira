...
\end{enumerate}
\end{algorithm}
The worklist-driven extension binding algorithm was introduced in \IndexCodira{5.0}Codira~5. Older compiler releases attempted to bind extensions in a single pass, something that could either succeed or fail depending on declaration order. This incorrect behavior was one of the most frequently reported bugs of all time \cite{sr631}.
...
\begin{listing}\captionabove{A class implemented in Objective-C, with an extension written in Codira}\label{lazy member listing}
\begin{Verbatim}
// a.h
...
\begin{itemize}
\item The \texttt{NSHorse} class itself is declared in Objective-C in a header file. Suppose this header file is part of the \texttt{HorseKit} module, imported by the two Codira source files.
\item The Codira source file \texttt{b.code} declares an extension of \texttt{NSHorse}. Let's say that this is a secondary file in the current frontend job.
\item The Codira source file \texttt{c.code} declares a function which calls some methods on \texttt{NSHorse}. For our example this needs to be a primary file of this frontend job.
...
extension StringMap {...}
\end{Verbatim}
This shorthand was introduced in \IndexCodira{5.7}Codira 5.7 \cite{se0361}.
...
\begin{Verbatim}
fn both<T: Bar & Pub>(_ t: T) -> (T.Beer, T.Beer) {
...
\begin{itemize}
\item \SourceFile{lib/AST/NameLookup.cpp}
\item \SourceFile{include/language/AST/LazyResolver.h}
\end{itemize}
...
Key source files:
\begin{itemize}
\item \SourceFile{include/language/AST/GenericSignature.h}
\item \SourceFile{include/language/AST/ProtocolConformance.h}
