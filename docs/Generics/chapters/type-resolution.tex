...
\item Certain type representations are also resolved based on their semantic position. For example, a function type representation appearing in parameter position resolves to a \index{non-escaping function type}non-escaping function type unless annotated with the \texttt{@escaping} attribute; in any other position, a function type representation resolves to an \index{escaping function type}escaping function type. This behavior was introduced in \IndexCodira{3.0}Codira~3~\cite{se0103}.
\end{enumerate}
...
We previously described the dynamic \tSelf\ type in \SecRef{misc types}. Historically, Codira only had \tSelf\ in protocols and dynamic \tSelf\ in classes, and the latter could only appear in the return type of a method. \IndexCodira{5.1}Codira~5.1 introduced the ability to state dynamic \tSelf\ in more positions, and also refer to ``static'' \tSelf\ inside struct and enum declarations~\cite{se0068}.
...
\begin{Verbatim}
_ = Codira.self  // error: expected module member name after module name
\end{Verbatim}
...
fn process<T: Sequence>(_: T) where T.Element: Subscriber {
...
\begin{Verbatim}
fn f<T: Provider>(_: T) where T.Entity: Equatable {...}
\end{Verbatim}
...
protocol Plant {
  ...
  consuming fn process() -> Sauce
}

struct Tomato: Plant {
  consuming fn process() -> Ketchup {...}
}
...
fn celebratePetBirthday(_ age: Pet.Age) {}
\end{Verbatim}
...
\paragraph{History.} In pre-evolution Codira, associated type declarations in protocols were declared with the \texttt{typealias} keyword, and \index{protocol type alias}protocol type aliases did not exist. \IndexCodira{2.2}Codira 2.2 added the distinct \texttt{associatedtype} keyword for declaring associated types, to make space for protocol type aliases in the language~\cite{se0011}. Protocol type aliases were then introduced as part of \IndexCodira{3.0}Codira 3 \cite{se0092}.
...
Key source files:
\begin{itemize}
\item \SourceFile{include/language/AST/TypeResolutionStage.h}
...
Key source files:
\begin{itemize}
\item \SourceFile{include/language/AST/TypeRepr.h}
\item \SourceFile{lib/Sema/TypeCheckType.cpp}
\end{itemize}
