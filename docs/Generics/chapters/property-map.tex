...
\end{enumerate}
\end{definition}
The second step is to define a mapping from type terms to Codira type parameters, for use by the \texttt{getSuperclassBound()} and \texttt{getConcreteType()} queries when mapping substitutions back to Codira types.
\begin{algorithm} The type lifting map $\Lambda^{-1}:\namesym{Term}\rightarrow\namesym{Type}$ takes as input
a type term $T$ and maps it back to a Codira type parameter. This is the inverse of the type lowering
...
\end{enumerate}
\end{algorithm}
The third and final step before the queries themselves can be presented is the algorithm for mapping a superclass or concrete type symbol back to a Codira type. This algorithm uses the above type lifting map on type parameters appearing in substitutions.
...
Then, \AlgRef{symboltotype} can be applied to convert the symbol to a Codira type.
...
Applying \AlgRef{symboltotype} to the superclass symbol produces the Codira type:
...
Applying \AlgRef{symboltotype} to the adjusted concrete type symbol produces the Codira type:
