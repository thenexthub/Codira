...
The [request-evaluator](https://github.com/languagelang/language/blob/main/include/language/AST/Evaluator.h) is a new architectural approach for the Codira compiler that attempts to provide infrastructure for fine-grained tracking and visualization of dependencies, separately caching the results of computation within the compiler, and eliminating mutable state, particular from the Abstract Syntax Tree (AST). It is intended to address a number of problems that arise in the Codira compiler:
...
* Lazy type checking uses an ad hoc set of callbacks captured in the [lazy resolver](https://github.com/languagelang/language/blob/main/include/language/AST/LazyResolver.h). This ad hoc recursion can lead to cycles in the type-checking problem (e.g., to compute A we need to compute B, which depends on A): with mutable state, it's easy to cause either wrong results or infinite recursion.
...
All existing requests inherit the [`SimpleRequest`](https://github.com/languagelang/language/blob/main/include/language/AST/SimpleRequest.h) class template, which automates most of the boilerplate involved in defining a new request, and mostly abstracts away the handling of the storage/comparison/hashing/printing of the inputs to a request. Some example requests for access control are provided by [`AccessRequests.h`](https://github.com/languagelang/language/blob/main/include/language/AST/AccessRequests.h).
...
The frontend option `-debug-cycles` will provide debugging output for any cycle detected while processing the given source files. For example, running the [`circular_inheritance.code` test](https://github.com/languagelang/language/blob/main/test/decl/class/circular_inheritance.code) from the Codira repository using this flag, e.g.,

```
$ languagec -frontend -typecheck -debug-cycles test/decl/class/circular_inheritance.code
```
...
```
===CYCLE DETECTED===
 `--SuperclassTypeRequest(circular_inheritance.(file).Outer2@test/decl/class/circular_inheritance.code:38:7)
     `--InheritedTypeRequest(circular_inheritance.(file).Outer2@test/decl/class/circular_inheritance.code:38:7, 0)
         `--SuperclassTypeRequest(circular_inheritance.(file).Outer2@test/decl/class/circular_inheritance.code:38:7) (cyclic dependency)
```

Within the compiler, the core [`Evaluator` class](https://github.com/languagelang/language/blob/main/include/language/AST/Evaluator.h) provides dumping routines such as `dumpDependencies()`, so one can see the dependencies for any request as part of a debugging session. It uses the same textual dump format as `-debug-cycles`.

The request-evaluator has the ability to emit "real" Codira diagnostics when it encounters a cycle, but they are currently disabled because they fire too often in real-world code. Once enough of the cyclic dependencies in well-formed code have been removed, such that the only errors come from ill-formed code, these diagnostics will be enabled. For example, they will be used to detect ill-formed programs such as:
...
The request-evaluator is relatively new to the Codira compiler, having been introduced in mid-2018. There are a number of improvements that can be made to the evaluator itself and how it is used in the compiler:
...
* Try to eliminate more boilerplate from subclasses of [`SimpleRequest`](https://github.com/languagelang/language/blob/main/include/language/AST/SimpleRequest.h). We are going to have a *lot* of requests.
