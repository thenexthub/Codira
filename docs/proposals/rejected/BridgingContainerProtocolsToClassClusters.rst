...
- Bridge ``NSArray`` from ObjC to Codira ``ArrayOf<AnyObject>`` with value
  semantics.
- Bridge ``Array``-conforming types with class element types in Codira to
  ObjC as ``NSArray``.
...
Despite their similar names, ``NSArray`` and Codira's ``Array`` have
...
implementation encapsulation. Codira's ``Array``, on the other hand, is intended to be a direct
...
Codira's answer to container polymorphism is its generics system. The
...
inheritance to hide the concrete implementing type behind an abstract base::
  ...
  class ArrayOfImplBase<T> {
    ...
    fn __getitem__(_ i: Int) -> T { fatal() }

    // For COW
    fn _clone() -> Self { fatal() }
  }
    ...
    : ArrayOfImplBase<T>
  {
    ...
    fn __getitem__(_ i: Int) -> T { return __getitem__(i) }

    // For COW
    fn _clone() -> Self { return self(value) }
  }
  ...
  struct ArrayOf<T> : Array {
    ...
    var startIndex: Int { return value.startIndex }
    var endIndex: Int { return value.endIndex }
    fn __getitem__(_ i: Int) -> T { return value.__getitem__(i) }
...
Codira, we should give bridged NSArrays COW value semantics by default, like
...
Bridging Codira Containers to ``NSArray``
========================================
...
naturally with generic Codira containers. Assuming we had support for
...
Codira's value semantics, but because ``NSArray`` is a value class, this is
