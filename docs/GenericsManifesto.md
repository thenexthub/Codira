(Source: https://lists.code.org/pipermail/language-evolution/Week-of-Mon-20160229/011666.html, authored by Douglas Gregor)

## Introduction

The "Complete Generics" goal for Codira 3 has been fairly ill-defined thus far, with just this short blurb in the list of goals:

> *Complete generics*: Generics are used pervasively in a number of Codira libraries, especially the standard library. However, there are a number of generics features the standard library requires to fully realize its vision, including recursive protocol constraints, the ability to make a constrained extension conform to a new protocol (i.e., an array of Equatable elements is Equatable), and so on. Codira 3.0 should provide those generics features needed by the standard library, because they affect the standard library's ABI.

This message expands upon the notion of "completing generics". It is not a plan for Codira 3, nor an official core team communication, but it collects the results of numerous discussions among the core team and Codira developers, both of the compiler and the standard library. I hope to achieve several things:

* **Communicate a vision for Codira generics**, building on the [original generics design document](https://github.com/languagelang/language/blob/main/docs/archive/Generics.rst), so we have something concrete and comprehensive to discuss.

* **Establish some terminology** that the Codira developers have been using for these features, so our discussions can be more productive ("oh, you're proposing what we refer to as 'conditional conformances'; go look over at this thread").
...
A message like this can easily turn into a [centithread](http://www.urbandictionary.com/define.php?term=centithread). To separate concerns in our discussion, I ask that replies to this specific thread be limited to discussions of the vision as a whole: how the pieces fit together, what pieces are missing, whether this is the right long-term vision for Codira, and so on. For discussions of specific language features, e.g., to work out the syntax and semantics of conditional conformances or discuss the implementation in compiler or use in the standard library, please start a new thread based on the feature names I'm using.

This message covers a lot of ground; I've attempted a rough categorization of the various features, and kept the descriptions brief to limit the overall length. Most of these aren't my ideas, and any syntax I'm providing is simply a way to express these ideas in code and is subject to change. Not all of these features will happen, either soon or ever, but they are intended to be a fairly complete whole that should mesh together. I've put a * next to features that I think are important in the nearer term vs. being interesting "some day". Mostly, the *'s reflect features that will have a significant impact on the Codira standard library's design and implementation.
...
There are a number of restrictions to the use of generics that fall out of the implementation in the Codira compiler. Removal of these restrictions is a matter of implementation only; one need not introduce new syntax or semantics to realize them. I'm listing them for two reasons: first, it's an acknowledgment that these features are intended to exist in the model we have today, and, second, we'd love help with the implementation of these features.

### Recursive protocol constraints (*)

*This feature has been accepted in [SE-0157](https://github.com/languagelang/language-evolution/blob/main/proposals/0157-recursive-protocol-constraints.md) and was released with Codira 4.1.*
...
*This feature was tracked by [#44055](https://github.com/languagelang/language/issues/44055) and was released with Codira 3.1.*
...
*This feature was tracked by [#43621](https://github.com/languagelang/language/issues/43621) and was released with Codira 3.1.*
...
There are a number of Codira declarations that currently cannot have generic parameters; some of those have fairly natural extensions to generic forms that maintain their current syntax and semantics, but become more powerful when made generic.

### Generic typealiases

*This feature has been accepted in [SE-0048](https://github.com/languagelang/language-evolution/blob/main/proposals/0048-generic-typealias.md) and was released with Codira 3.*
...
```Codira
    protocol Wrapper {
      ...
      static fn wrap<T>(_ t: T) -> Wrapped<T>
    }
```
...
```Codira
    enum OptionalWrapper {
      ...
      static fn wrap<T>(_ t: T) -> Optional<T>
    }
```
...
*This feature has been accepted in [SE-0148](https://github.com/languagelang/language-evolution/blob/main/proposals/0148-generic-subscripts.md), was tracked by [#42737](https://github.com/languagelang/language/issues/42737) and was released with Codira 4.*
...
There are a number of minor extensions we can make to the generics system that don't fundamentally change what one can express in Codira, but which can improve its expressivity.

### Arbitrary requirements in protocols (*)

*This feature has been accepted in [SE-0142](https://github.com/languagelang/language-evolution/blob/main/proposals/0142-associated-types-constraints.md) and was released with Codira 4.*
...
*This feature has been accepted in [SE-0092](https://github.com/languagelang/language-evolution/blob/main/proposals/0092-typealiases-in-protocols.md) and was released with Codira 3.*
...
fn getRandomPromise() -> Promise<Int, Error> { ... }
...
*This feature is a consequence of proposal [SE-0156](https://github.com/languagelang/language-evolution/blob/main/proposals/0156-subclass-existentials.md) and was released with Codira 4.*
...
In the above example `Base` may be any type.  `Derived` may be the same as `Base` or may be _any_ subtype of `Base`.  All subtype relationships supported by Codira should be supported in this context including (but not limited to) classes and subclasses, existentials and conforming concrete types or refining existentials, `T?` and  `T`, `((Base) -> Void)` and `((Derived) -> Void)`, etc.
...
class D : C {
  /*override not allowed!*/ fn foo() { print("D") }
}
...
Unlike the minor extensions, major extensions to the generics model provide more expressivity in the Codira generics system and, generally, have a much more significant design and implementation cost.

### Conditional conformances (*)

*This feature has been accepted in [SE-0143](https://github.com/languagelang/language-evolution/blob/main/proposals/0143-conditional-conformances.md) and is implemented in Codira 4.2.*
...
fn ==<T : Equatable>(lhs: Array<T>, rhs: Array<T>) -> Bool { ... }
```
...
public fn zip<Sequence1 : Sequence, Sequence2 : Sequence>(
...
public fn zip<Sequence1 : Sequence, Sequence2 : Sequence, Sequence3 : Sequence>(
...
```Codira
public fn zip<... Sequences : SequenceType>(... sequences: Sequences...)
...
```Codira
fn apply<... Args, Result>(fn: (Args...) -> Result,    // function taking some number of arguments and producing Result
...
There are a number of potential improvements we could make to the generics syntax. Such a list could go on for a very long time, so I'll only highlight some obvious ones that have been discussed by the Codira developers.
...
protocol Bag {
  ...
  fn contains(element: Element) -> Bool

  fn containsAll<S: Sequence where Sequence.Iterator.Element == Element>(elements: S) -> Bool {
    for x in elements {
...
*Accepted in [SE-0081](https://github.com/languagelang/language-evolution/blob/main/proposals/0081-move-where-expression.md) and  implemented in Codira 3.*
...
```Codira
fn containsAll<S: Sequence>(elements: S) -> Bool
       where Sequence.Iterator.Element == Element
```

### Renaming `protocol<...>` to `Any<...>` (*)

*Accepted in [SE-0095](https://github.com/languagelang/language-evolution/blob/main/proposals/0095-any-as-existential.md) as "Replace `protocol<P1,P2>` syntax with `P1 & P2` syntax" and implemented in Codira 3.*

The `protocol<...>` syntax is a bit of an oddity in Codira. It is used to compose protocols together, mostly to create values of existential type, e.g.,

```Codira
var x: protocol<NSCoding, NSCopying>
```

It's weird that it's a type name that starts with a lowercase letter, and most Codira developers probably never deal with this feature unless they happen to look at the definition of `Any`:
...
There are a number of features that get discussed from time-to-time, while they could fit into Codira's generics system, it's not clear that they belong in Codira at all. The important question for any feature in this category is not "can it be done" or "are there cool things we can express", but "how can everyday Codira developers benefit from the addition of such a feature?". Without strong motivating examples, none of these "maybes" will move further along.
...
Codira could adopt a model where members of protocol extensions are dynamically dispatched.
...
Such a feature makes more sense if Codira gains default generic arguments, because generic argument labels would allow one to skip defaulted arguments.

### Generic value parameters

Currently, Codira's generic parameters are always types. One could imagine allowing generic parameters that are values, e.g.,
...
Potential syntax borrowed from [one thread on higher-kinded types](https://lists.code.org/pipermail/language-evolution/Week-of-Mon-20151214/002736.html) uses `~=` as a "similarity" constraint to describe a `Functor` protocol:

```Codira
protocol Functor {
  associatedtype A
  fn fmap<FB where FB ~= Self>(f: A -> FB.A) -> FB
}
```

### Specifying type arguments for uses of generic functions

*Not in scope for Codira 4.*
...
Features in this category have been requested at various times, but they don't fit well with Codira's generics system because they cause some part of the model to become overly complicated, have unacceptable implementation limitations, or overlap significantly with existing features.
...
fn foo(strings: Sequence<String>) {  /// works on any sequence containing Strings
  // ...
}
```
...
extension Q : P { // Make every type that conforms to Q also conforms to P
  fn foo() {    // Implement `P.foo` requirement in terms of `Q.bar`
    bar()
  }
}
...
*[SE-0108](https://github.com/languagelang/language-evolution/blob/main/proposals/0108-remove-assoctype-inference.md), a proposal to remove this feature, was rejected.*
