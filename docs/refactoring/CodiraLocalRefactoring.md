# Codira Local Refactoring

Xcode 9 includes a brand new refactoring engine. It can transform code locally
within a single Codira source file, or globally, such as renaming a method or property
...
the [language repository](https://github.com/languagelang/language). Therefore, any Codira enthusiast can
...
cannot be implemented on their own within the Codira codebase. This post focuses on
...
specified by a cursor position in a Codira source file, such as rename refactoring.
...
of these two categories, the Codira repository provides pre-analyzed results called
[ResolvedCursorInfo] and [RangeInfo] to answer several common questions about a cursor
position or a range in a Codira source file.
...
To implement a new refactoring for Codira, we don't
...
Cursor-based refactoring is initiated by a cursor location in a Codira source file.
...
`LocalizeString`, specifies the internal name of this refactoring in the Codira
...
the Codira toolchain uses in communication with the source editor.
...
continuous range of code in a Codira source file. Taking the implementation of the *Extract Expression*
...
2    toolchain::SmallString<64> DeclBuffer;
3    toolchain::raw_svector_ostream OS(DeclBuffer);
4    OS << tok::kw_let << " ";
5    OS << PreferredName;
...
an error message if the given new name is an invalid Codira identifier. To do so,
...
These two parts are both tested using the [language-refactor] command line utility which
...
Similar tests can be found in [test/refactoring/RefactoringKind/](https://github.com/languagelang/language/tree/main/test/refactoring/RefactoringKind).
...
`%refactor` is an alias that gets substituted by the test runner to give the full path to `language-refactor` when the tests get run.
...
change matches our expectations. As a preparation, we need to teach [language-refactor]
a refactoring kind flag to specify the action we are testing with. To achieve this,
the following entry is added in [language-refactor.cpp](https://github.com/languagelang/language/blob/main/tools/language-refactor/language-refactor.cpp):

~~~cpp
  clEnumValN(RefactoringKind::LocalizeString, "localize-string", "Perform String Localization refactoring"),
~~~

With such an entry, [language-refactor] can test the code transformation part of
...
5  // RUN: %refactor -localize-string -source-filename %s -pos=2:14 > %t.result/localized.code
6  // RUN: diff -u %S/Outputs/localized.code.expected %t.result/localized.code
~~~
...
After implementing all of above pieces in the Codira codebase, we
are ready to test/use the newly added refactoring in Xcode by integrating with
a locally-built open source toolchain.

1. Run [build-toolchain](https://github.com/languagelang/language/blob/main/utils/build-toolchain)
...
Codira's issue database contains [several ideas of refactoring transformations](https://github.com/languagelang/language/issues?q=is%3Aissue+is%3Aopen+label%3ARefactoring) awaiting implementations.

For further help with implementing refactoring transformations, please feel free to ask questions on the [Codira forums](https://forums.code.org/c/development/compiler/9).

[sourcekitd]: https://github.com/languagelang/language/tree/main/tools/SourceKit
[ResolvedCursorInfo]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/include/language/IDE/Utils.h#L158
[RangeInfo]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/include/language/IDE/Utils.h#L344
[performChange]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/lib/IDE/Refactoring.cpp#L599
[RefactoringKinds.def]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/include/language/IDE/RefactoringKinds.def
[isApplicable]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/lib/IDE/Refactoring.cpp#L646
[DiagnosticsRefactoring.def]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/include/language/AST/DiagnosticsRefactoring.def
[language-refactor]: https://github.com/languagelang/language/tree/60a91bb7360dde5ce9531889e0ed10a2edbc961a/tools/language-refactor
[Refactoring.cpp]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/lib/IDE/Refactoring.cpp
[EditConsumer]: https://github.com/languagelang/language/blob/60a91bb7360dde5ce9531889e0ed10a2edbc961a/include/language/IDE/Utils.h#L506
