...
#define TEST_MARSHALLING(ty, lhs, rhs, expected)                                             \
    TEST_CASE("function can marshal " #ty, "[marshal]") {                                    \
        codira::Error err;                                                                      \
        if (auto runtime =                                                                   \
                codira::make_runtime(get_codiralib_path("codira-marshal/target/mod.codiralib"), {}, &err)) { \
            REQUIRE(err.is_ok());                                                            \
                                                                                             \
            const ty a = (lhs), b = (rhs);                                                   \
            auto res = codira::invoke_fn<ty>(*runtime, "marshal_" #ty, a, b);                   \
            REQUIRE(res.is_ok());                                                            \
    ...
    TEST_CASE("struct can get, set, and replace " #ty, "[marshal]") {                        \
        ...
        if (auto runtime =                                                                   \
            ...
            REQUIRE(err.is_ok());                                                            \
                                                                                             \
            const ty a = (lhs), b = (rhs);                                                   \
            auto res = codira::invoke_fn<codira::StructRef>(*runtime, "new_" #ty, a, b);           \
            REQUIRE(res.is_ok());                                                            \
                                                                                             \
            auto s = res.wait();                                                             \
            {                                                                                \
...
TEST_CASE("struct can get, set, and replace struct", "[marshal]") {
    ...
    if (auto runtime = codira::make_runtime(get_codiralib_path("codira-marshal/target/mod.codiralib"), {}, &err)) {
        ...
        auto gc_struct_res = codira::invoke_fn<codira::StructRef>(*runtime, "new_gc_struct", a, b);
        REQUIRE(gc_struct_res.is_ok());
        auto value_struct_res = codira::invoke_fn<codira::StructRef>(*runtime, "new_value_struct", a, b);
        ...
        auto gc_wrapper =
            codira::invoke_fn<codira::StructRef>(*runtime, "new_gc_wrapper", gc_struct, value_struct);
        REQUIRE(gc_wrapper.is_ok());

        auto value_wrapper =
            codira::invoke_fn<codira::StructRef>(*runtime, "new_value_wrapper", gc_struct, value_struct);
        REQUIRE(value_wrapper.is_ok());

        // Test `InvokeResult::wait`
        std::array<codira::StructRef, 2> structs = {gc_wrapper.wait(), value_wrapper.wait()};
        for (auto s : structs) {
            // `struct(gc)`
            auto gc = s.get<codira::StructRef>("0");
            ...
            REQUIRE(s.set<codira::StructRef>("0", *gc2));

            // Verify that `set` worked.
            auto gc3 = s.get<codira::StructRef>("0");
            ...
            // `struct(value)`
            auto value = s.get<codira::StructRef>("1");
            ...
            // Verify that `replace` worked
            auto value3 = s.get<codira::StructRef>("1");
            ...
            REQUIRE(s.set<codira::StructRef>("1", *value2));

            // Verify that `set` worked.
            auto value4 = s.get<codira::StructRef>("1");
...
TEST_CASE("can fetch array type", "[marshal]") {
    codira::Error err;
    if (auto runtime = codira::make_runtime(get_codiralib_path("marshal/target/mod.codiralib"), {}, &err)) {
        REQUIRE(err.is_ok());

        auto array_res = codira::invoke_fn<codira::ArrayRef<int32_t>>(*runtime, "new_array_i32", 1, 2, 3);
