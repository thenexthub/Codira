#ifndef CODIRA_STRUCT_REF_H_
#define CODIRA_STRUCT_REF_H_
...
#include "codira/gc.h"
#include "codira/marshal.h"
#include "codira/runtime.h"
#include "codira/static_type_info.h"
#include "codira/struct_type.h"
...
/** Type-agnostic wrapper for interoperability with a Codira struct.
 *
...
class StructRef {
public:
    /** Constructs a `StructRef` that wraps a raw Codira struct.
     *
     ...
     */
    StructRef(const Runtime& runtime, CodiraGcPtr raw) noexcept
     ...
     */
    [[nodiscard]] constexpr CodiraGcPtr raw() const noexcept { return m_handle.handle(); }
...
struct Marshal<StructRef> {
    ...
    static StructRef copy_from(const type* ptr, const Runtime& runtime,
        ...
        CODIRA_ASSERT(codira_type_kind(type_info.type_handle(), &type_kind));

        // Safety: `codira_type_kind` is guaranteed to return a value for `StructRef`s.
        CodiraStructMemoryKind memory_kind;
        CODIRA_ASSERT(codira_struct_type_memory_kind(type_kind.struct_, &memory_kind));

        CodiraGcPtr gc_handle;
        if (memory_kind == CODIRA_STRUCT_MEMORY_KIND_VALUE) {
            ...
            // For a gc struct, `ptr` points to a `CodiraGcPtr`.
            gc_handle = *ptr;
        }
