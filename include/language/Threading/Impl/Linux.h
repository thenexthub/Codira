...
#ifndef LANGUAGE_THREADING_IMPL_LINUX_H
#define LANGUAGE_THREADING_IMPL_LINUX_H
...
#define LANGUAGE_LINUXTHREADS_CHECK(expr)                                         \
  do {                                                                         \
    int res_ = (expr);                                                         \
    if (res_ != 0)                                                             \
...
#define LANGUAGE_LINUXTHREADS_RETURN_TRUE_OR_FALSE(falseerr, expr)                \
  do {                                                                         \
    int res_ = (expr);                                                         \
    switch (res_) {                                                            \
    case 0:                                                                    \
...
inline void mutex_init(mutex_handle &handle, bool checked = false) {
  if (!checked) {
    ...
    LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutexattr_init(&attr));
    LANGUAGE_LINUXTHREADS_CHECK(
        ::pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_ERRORCHECK));
    LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_init(&handle, &attr));
    LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutexattr_destroy(&attr));
  }
}
inline void mutex_destroy(mutex_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_destroy(&handle));
}

inline void mutex_lock(mutex_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_lock(&handle));
}
inline void mutex_unlock(mutex_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_unlock(&handle));
}
inline bool mutex_try_lock(mutex_handle &handle) {
  LANGUAGE_LINUXTHREADS_RETURN_TRUE_OR_FALSE(EBUSY,
...
inline void cond_destroy(cond_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_cond_destroy(&handle.condition));
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_destroy(&handle.mutex));
}
inline void cond_lock(cond_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_lock(&handle.mutex));
}
inline void cond_unlock(cond_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_mutex_unlock(&handle.mutex));
}
inline void cond_signal(cond_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_cond_signal(&handle.condition));
}
inline void cond_broadcast(cond_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_cond_broadcast(&handle.condition));
}
inline void cond_wait(cond_handle &handle) {
  LANGUAGE_LINUXTHREADS_CHECK(::pthread_cond_wait(&handle.condition, &handle.mutex));
}
...
inline bool cond_wait(cond_handle &handle,
  ...
  LANGUAGE_LINUXTHREADS_RETURN_TRUE_OR_FALSE(
    ETIMEDOUT,
