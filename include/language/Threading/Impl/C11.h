...
#ifndef LANGUAGE_THREADING_IMPL_C11_H
#define LANGUAGE_THREADING_IMPL_C11_H
...
#define LANGUAGE_C11THREADS_CHECK(expr)                                           \
  do {                                                                         \
    int res_ = (expr);                                                         \
...
#define LANGUAGE_C11THREADS_RETURN_TRUE_OR_FALSE(falseerr, expr)                  \
  do {                                                                         \
    int res_ = (expr);                                                         \
    switch (res_) {                                                            \
    case thrd_success:                                                         \
...
inline void mutex_init(mutex_handle &handle, bool checked = false) {
  LANGUAGE_C11THREADS_CHECK(::mtx_init(&handle, ::mtx_plain));
}
inline void mutex_destroy(mutex_handle &handle) { ::mtx_destroy(&handle); }

inline void mutex_lock(mutex_handle &handle) {
  LANGUAGE_C11THREADS_CHECK(::mtx_lock(&handle));
}
inline void mutex_unlock(mutex_handle &handle) {
  LANGUAGE_C11THREADS_CHECK(::mtx_unlock(&handle));
}
inline bool mutex_try_lock(mutex_handle &handle) {
  LANGUAGE_C11THREADS_RETURN_TRUE_OR_FALSE(thrd_busy, ::mtx_trylock(&handle));
}
...
#define LANGUAGE_LAZY_MUTEX_INITIALIZER ((threading_impl::lazy_mutex_handle){})
...
inline void lazy_mutex_init(lazy_mutex_handle &handle) {
  ...
  if (std::atomic_compare_exchange_strong_explicit(
    ...
    LANGUAGE_C11THREADS_CHECK(::mtx_init(&handle.mutex, ::mtx_plain));
...
inline void lazy_mutex_lock(lazy_mutex_handle &handle) {
  ...
  LANGUAGE_C11THREADS_CHECK(::mtx_lock(&handle.mutex));
}
inline void lazy_mutex_unlock(lazy_mutex_handle &handle) {
  lazy_mutex_init(handle);
  LANGUAGE_C11THREADS_CHECK(::mtx_unlock(&handle.mutex));
}
inline bool lazy_mutex_try_lock(lazy_mutex_handle &handle) {
  lazy_mutex_init(handle);
  LANGUAGE_C11THREADS_RETURN_TRUE_OR_FALSE(thrd_busy, ::mtx_trylock(&handle.mutex));
}
...
inline void recursive_mutex_init(recursive_mutex_handle &handle,
                                 bool checked = false) {
  LANGUAGE_C11THREADS_CHECK(::mtx_init(&handle, ::mtx_recursive));
}
...
inline void cond_init(cond_handle &handle) {
  LANGUAGE_C11THREADS_CHECK(::cnd_init(&handle.condition));
  LANGUAGE_C11THREADS_CHECK(::mtx_init(&handle.mutex, ::mtx_plain));
}
...
inline void cond_signal(cond_handle &handle) {
  LANGUAGE_C11THREADS_CHECK(::cnd_signal(&handle.condition));
}
inline void cond_broadcast(cond_handle &handle) {
  LANGUAGE_C11THREADS_CHECK(::cnd_broadcast(&handle.condition));
}
inline void cond_wait(cond_handle &handle) {
  LANGUAGE_C11THREADS_CHECK(::cnd_wait(&handle.condition, &handle.mutex));
}
...
inline bool cond_wait(cond_handle &handle,
  ...
  LANGUAGE_C11THREADS_RETURN_TRUE_OR_FALSE(
    thrd_timedout,
...
#endif // LANGUAGE_THREADING_IMPL_C11_H
