...
#ifndef LANGUAGE_SILOPTIMIZER_UTILS_SCOPEOPTUTILS_H
#define LANGUAGE_SILOPTIMIZER_UTILS_SCOPEOPTUTILS_H
...
protected:
  ...
  unsigned pushScope(std::function<Result(Args...)> fn) {
    assert(!didPop && "Can not reuse scope once used?!");
    unsigned index = cleanups.size();
    cleanups.emplace_back(ScopeCleanup{fn});
    return index;
  }
  ...
  void nonDestructivePop(Args... args) const {
    for (auto val : toolchain::reverse(cleanups)) {
      if (val) {
        val->fn(std::forward<Args>(args)...);
      }
    }
