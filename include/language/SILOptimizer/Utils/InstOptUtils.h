...
#ifndef LANGUAGE_SILOPTIMIZER_UTILS_INSTOPTUTILS_H
#define LANGUAGE_SILOPTIMIZER_UTILS_INSTOPTUTILS_H
...
/// Transform a use_iterator range (Operand*) into an toolchain::iterator_range
/// of users (SILInstruction *)
inline iterator_range<toolchain::mapped_iterator<ValueBase::use_iterator, UserTransform>>
makeUserIteratorRange(iterator_range<ValueBase::use_iterator> useRange) {
  auto toUser = [](Operand *operand) { return operand->getUser(); };
  return toolchain::map_range(useRange, UserTransform(toUser));
}

using DeadInstructionSet = toolchain::SmallSetVector<SILInstruction *, 8>;
...
void collectUsesOfValue(SILValue V,
                        toolchain::SmallPtrSetImpl<SILInstruction *> &Insts);
...
void insertDestroyOfCapturedArguments(
    ...
    toolchain::function_ref<SILValue(SILValue)> getValueToDestroy =
        [](SILValue arg) -> SILValue { return arg; },
    SILLocation loc = RegularLocation::getAutoGeneratedLocation());

void insertDeallocOfCapturedArguments(PartialApplyInst *pai,
    DominanceInfo *domInfo,
    toolchain::function_ref<SILValue(SILValue)> getAddressToDealloc =
      [](SILValue arg) -> SILValue { return arg; });
...
struct TOOLCHAIN_LIBRARY_VISIBILITY FindLocalApplySitesResult {
...
#endif // LANGUAGE_SILOPTIMIZER_UTILS_INSTOPTUTILS_H
