...
#ifndef LANGUAGE_CORE_FRONTEND_ASTUNIT_H
#define LANGUAGE_CORE_FRONTEND_ASTUNIT_H
...
private:
  ...
  using FileDeclsTy = toolchain::DenseMap<FileID, std::unique_ptr<LocDeclsTy>>;
  ...
  toolchain::StringMap<SourceLocation> PreambleSrcLocCache;
  ...
  /// preamble.
  std::unique_ptr<toolchain::MemoryBuffer> SavedMainFileBuffer;
...
public:
  ...
  /// identifiers.
  toolchain::StringMap<unsigned> &getCachedCompletionTypes() {
...
private:
  ...
  std::unique_ptr<toolchain::MemoryBuffer> getMainBufferWithPrecompiledPreamble(
      std::shared_ptr<PCHContainerOperations> PCHContainerOps,
      CompilerInvocation &PreambleInvocationIn,
      IntrusiveRefCntPtr<toolchain::vfs::FileSystem> VFS, bool AllowRebuild = true,
      unsigned MaxLines = 0);
...
public:
  ...
  toolchain::IntrusiveRefCntPtr<DiagnosticsEngine> getDiagnosticsPtr() {
    return Diagnostics;
  }

  const SourceManager &getSourceManager() const { return *SourceMgr; }
  SourceManager &getSourceManager() { return *SourceMgr; }
  toolchain::IntrusiveRefCntPtr<SourceManager> getSourceManagerPtr() {
    return SourceMgr;
  }
  ...
  toolchain::IntrusiveRefCntPtr<ASTContext> getASTContextPtr() { return Ctx; }

  void setASTContext(toolchain::IntrusiveRefCntPtr<ASTContext> ctx) {
    Ctx = std::move(ctx);
  }
  ...
  using RemappedFile = std::pair<std::string, toolchain::MemoryBuffer *>;
...
private:
  ...
  ///
  /// \param VFS - A toolchain::vfs::FileSystem to be used for all file accesses.
...
public:
  ...
  void CodeComplete(StringRef File, unsigned Line, unsigned Column,
                    ...
                    std::shared_ptr<PCHContainerOperations> PCHContainerOps,
                    toolchain::IntrusiveRefCntPtr<DiagnosticsEngine> Diag,
...
#endif // LANGUAGE_CORE_FRONTEND_ASTUNIT_H
