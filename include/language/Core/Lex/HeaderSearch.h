...
#ifndef LANGUAGE_CORE_LEX_HEADERSEARCH_H
#define LANGUAGE_CORE_LEX_HEADERSEARCH_H
...
struct SearchDirIteratorImpl
    : toolchain::iterator_facade_base<SearchDirIteratorImpl<IsConst>,
...
using ConstSearchDirRange = toolchain::iterator_range<ConstSearchDirIterator>;
using SearchDirRange = toolchain::iterator_range<SearchDirIterator>;
...
class HeaderSearch {
  ...
  toolchain::DenseMap<unsigned, unsigned> SearchDirToHSEntry;
  ...
  toolchain::StringMap<unsigned, toolchain::BumpPtrAllocator> SearchDirHeaderMapIndex;
  ...
  toolchain::StringMap<LookupFileCacheInfo, toolchain::BumpPtrAllocator> LookupFileCache;

  /// Collection mapping a framework or subframework
  /// name like "Carbon" to the Carbon.framework directory.
  toolchain::StringMap<FrameworkCacheEntry, toolchain::BumpPtrAllocator> FrameworkMap;

  /// Maps include file names (including the quotes or
  /// angle brackets) to other include file names.  This is used to support the
  /// include_alias pragma for Microsoft compatibility.
  using IncludeAliasMap =
      toolchain::StringMap<std::string, toolchain::BumpPtrAllocator>;
  ...
  toolchain::DenseMap<const DirectoryEntry *, ModuleMapDirectoryState>
      DirectoryModuleMap;

  /// Set of module map files we've already loaded, and a flag indicating
  /// whether they were valid or not.
  toolchain::DenseMap<const FileEntry *, bool> LoadedModuleMaps;

  /// Set of module map files we've already parsed, and a flag indicating
  /// whether they were valid or not.
  toolchain::DenseMap<const FileEntry *, bool> ParsedModuleMaps;

  // A map of discovered headers with their associated include file name.
  toolchain::DenseMap<const FileEntry *, toolchain::SmallString<64>> IncludeNames;

  /// Uniqued set of framework names, which is used to track which
  /// headers were included as framework headers.
  toolchain::StringSet<toolchain::BumpPtrAllocator> FrameworkNames;
...
public:
  ...
  std::string suggestPathToFileForDiagnostics(toolchain::StringRef File,
...
#endif // LANGUAGE_CORE_LEX_HEADERSEARCH_H
