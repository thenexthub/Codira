...
#ifndef LANGUAGE_CORE_LEX_MODULEMAP_H
#define LANGUAGE_CORE_LEX_MODULEMAP_H
...
class ModuleMap {
  ...
  toolchain::SmallVector<std::unique_ptr<ModuleMapCallbacks>, 1> Callbacks;
  ...
  toolchain::SpecificBumpPtrAllocator<Module> ModulesAlloc;
  ...
  toolchain::SmallVector<Module *, 8> PendingSubmodules;

  /// The top-level modules that are known.
  toolchain::StringMap<Module *> Modules;
  ...
  toolchain::DenseMap<const IdentifierInfo *, Module *> CachedModuleLoads;

  /// Shadow modules created while building this module map.
  toolchain::SmallVector<Module*, 2> ShadowModules;
  ...
  toolchain::StringMap<toolchain::StringSet<>> PendingLinkAsModule;
...
public:
  ...
  class KnownHeader {
    toolchain::PointerIntPair<Module *, 3, ModuleHeaderRole> Storage;
  ...
  using AdditionalModMapsSet = toolchain::DenseSet<FileEntryRef>;

private:
  friend class ModuleMapLoader;

  using HeadersMap = toolchain::DenseMap<FileEntryRef, SmallVector<KnownHeader, 1>>;
  ...
  mutable toolchain::DenseMap<off_t, toolchain::TinyPtrVector<Module*>> LazyHeadersBySize;

  /// Map from mtimes to modules with lazy header directives with those mtimes.
  mutable toolchain::DenseMap<time_t, toolchain::TinyPtrVector<Module*>>
  ...
  /// header.
  toolchain::DenseMap<const DirectoryEntry *, Module *> UmbrellaDirs;
  ...
  toolchain::DenseMap<Module *, unsigned> ModuleScopeIDs;
  ...
  toolchain::DenseMap<const DirectoryEntry *, InferredDirectory> InferredDirectories;

  /// A mapping from an inferred module to the module map that allowed the
  /// inference.
  toolchain::DenseMap<const Module *, FileID> InferredModuleAllowedBy;

  toolchain::DenseMap<const Module *, AdditionalModMapsSet> AdditionalModMaps;

  /// Describes whether we haved loaded a particular file as a module
  /// map.
  toolchain::DenseMap<const FileEntry *, bool> LoadedModuleMap;
  toolchain::DenseMap<const FileEntry *, const modulemap::ModuleMapFile *>
      ParsedModuleMap;
  ...
  /// redefinitions.
  toolchain::StringMap<SmallVector<std::pair<const modulemap::ModuleMapFile *,
...
public:
  ...
  /// Parse a module map without creating `language::Core::Module` instances.
  ...
  using module_iterator = toolchain::StringMap<Module *>::const_iterator;

  module_iterator module_begin() const { return Modules.begin(); }
  module_iterator module_end()   const { return Modules.end(); }
  toolchain::iterator_range<module_iterator> modules() const {
...
#endif // LANGUAGE_CORE_LEX_MODULEMAP_H
