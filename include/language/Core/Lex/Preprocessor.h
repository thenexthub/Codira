...
//
/// \file
/// Defines the language::Core::Preprocessor interface.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_LEX_PREPROCESSOR_H
#define LANGUAGE_CORE_LEX_PREPROCESSOR_H
...
class Preprocessor {
  ...
  toolchain::unique_function<void(const language::Core::Token &)> OnToken;
  ...
  /// control flow (in language::Core::ParseAST()), make it convenient to put here.
  ///
...
private:
  ...
  toolchain::DenseMap<FileID, SmallVector<const char *>> CheckPoints;
    ...
    void handleSemi() {
      if (!Name.empty() && isModuleCandidate()) {
        if (State == InterfaceCandidate)
          ...
          toolchain_unreachable("Unimaged ModuleDeclState.");
      } else if (!isNamedModule())
        reset();
    }
...
public:
  ...
  using IncludedFilesSet = toolchain::DenseSet<const FileEntry *>;
...
private:
  ...
  struct ModuleMacroInfo {
    ...
    toolchain::TinyPtrVector<ModuleMacro *> ActiveModuleMacros;
    ...
    toolchain::TinyPtrVector<ModuleMacro *> OverriddenMacros;
  ...
  class MacroState {
    mutable toolchain::PointerUnion<MacroDirective *, ModuleMacroInfo *> State;
  ...
  using MacroMap = toolchain::DenseMap<const IdentifierInfo *, MacroState>;
  ...
  /// imported.
  toolchain::SmallSetVector<Module *, 2> AffectingClangModules;

  /// The set of known macros exported from modules.
  toolchain::FoldingSet<ModuleMacro> ModuleMacros;

  /// The names of potential module macros that we've not yet processed.
  toolchain::SmallVector<IdentifierInfo *, 32> PendingModuleMacroNames;

  /// The list of module macros, for each identifier, that are not overridden by
  /// any other module macro.
  toolchain::DenseMap<const IdentifierInfo *, toolchain::TinyPtrVector<ModuleMacro *>>
      LeafModuleMacros;
  ...
  using WarnUnusedMacroLocsTy = toolchain::SmallDenseSet<SourceLocation, 32>;
  ...
  toolchain::DenseMap<const IdentifierInfo *, MacroAnnotations> AnnotationInfos;
  ...
  toolchain::DenseMap<IdentifierInfo *, std::vector<MacroInfo *>>
      PragmaPushMacroInfo;
  ...
  /// skipped.
  toolchain::DenseMap<const char *, unsigned> RecordedSkippedRanges;
...
public:
  ...
  toolchain::BumpPtrAllocator &getPreprocessorAllocator() { return BP; }
  ...
  void setTokenWatcher(toolchain::unique_function<void(const language::Core::Token &)> F) {
    OnToken = std::move(F);
  }
  ...
  toolchain::iterator_range<macro_iterator>
  ...
  /// imported.
  const toolchain::SmallSetVector<Module *, 2> &getAffectingClangModules() const {
  ...
  /// https://github.com/toolchain/toolchain-project/issues/57483).
...
private:
  toolchain::DenseMap<IdentifierInfo*,unsigned> PoisonReasons;
...
public:
  ...
  template <typename... Ts> bool isNextPPTokenOneOf(Ts... Ks) {
    ...
    if (!Val) {
      ...
      if (CurPPLexer)
        return false;
      for (const IncludeStackInfo &Entry : toolchain::reverse(IncludeMacroStack)) {
        if (Entry.TheLexer)
...
private:
  ...
  struct DirectiveEvalResult {
    ...
    std::optional<toolchain::APSInt> Value;
...
private:
  ...
  struct {
    ...
    toolchain::DenseMap<FileID, SafeBufferOptOutRegionsTy> LoadedRegions;
...
using PragmaHandlerRegistry = toolchain::Registry<PragmaHandler>;

} // namespace language::Core

namespace toolchain {
extern template class CLANG_TEMPLATE_ABI Registry<language::Core::PragmaHandler>;
} // namespace toolchain

#endif // LANGUAGE_CORE_LEX_PREPROCESSOR_H
