...
#ifndef LANGUAGE_CORE_LEX_PREPROCESSINGRECORD_H
#define LANGUAGE_CORE_LEX_PREPROCESSINGRECORD_H
...
void *operator new(size_t bytes, language::Core::PreprocessingRecord &PR,
                   unsigned alignment = 8) noexcept;

/// Frees memory allocated in a Clang preprocessing record.
void operator delete(void *ptr, language::Core::PreprocessingRecord &PR,
                     unsigned) noexcept;
...
class Token;
  ...
  class MacroExpansion : public PreprocessedEntity {
    ...
    /// a builtin macro.
    toolchain::PointerUnion<IdentifierInfo *, MacroDefinitionRecord *> NameOrDef;
  ...
  class PreprocessingRecord : public PPCallbacks {
    ...
    toolchain::DenseMap<const MacroInfo *, MacroDefinitionRecord *> MacroDefinitions;
  ...
  public:
    ...
    /// integer arithmetic.
    class iterator : public toolchain::iterator_adaptor_base<
    ...
    toolchain::iterator_range<iterator> getIteratorsForLoadedRange(unsigned start,
                                                              unsigned count) {
    ...
    toolchain::iterator_range<iterator>
...
inline void *operator new(size_t bytes, language::Core::PreprocessingRecord &PR,
                          unsigned alignment) noexcept {
  return PR.Allocate(bytes, alignment);
}

inline void operator delete(void *ptr, language::Core::PreprocessingRecord &PR,
                            unsigned) noexcept {
  PR.Deallocate(ptr);
}

#endif // LANGUAGE_CORE_LEX_PREPROCESSINGRECORD_H
