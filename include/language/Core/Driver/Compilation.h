...
#ifndef LANGUAGE_CORE_DRIVER_COMPILATION_H
#define LANGUAGE_CORE_DRIVER_COMPILATION_H
...
class Compilation {
  ...
  toolchain::opt::DerivedArgList *TranslatedArgs;
  ...
  std::map<TCArgsKey, toolchain::opt::DerivedArgList *> TCArgs;

  /// Temporary files which should be removed on exit.
  toolchain::opt::ArgStringList TempFiles;
...
public:
  Compilation(const Driver &D, const ToolChain &DefaultToolChain,
              toolchain::opt::InputArgList *Args,
              toolchain::opt::DerivedArgList *TranslatedArgs, bool ContainsError);
  ~Compilation();
  ...
  const toolchain::opt::InputArgList &getInputArgs() const { return *Args; }

  const toolchain::opt::DerivedArgList &getArgs() const { return *TranslatedArgs; }

  toolchain::opt::DerivedArgList &getArgs() { return *TranslatedArgs; }
  ...
  toolchain::opt::ArgStringList &getTempFiles() { return TempFiles; }
  const toolchain::opt::ArgStringList &getTempFiles() const { return TempFiles; }
  ...
  const toolchain::opt::DerivedArgList &
  ...
  bool CleanupFileList(const toolchain::opt::ArgStringList &Files,
...
#endif // LANGUAGE_CORE_DRIVER_COMPILATION_H
