...
#ifndef LANGUAGE_CORE_DRIVER_DRIVER_H
#define LANGUAGE_CORE_DRIVER_DRIVER_H
...
public:
  ...
  CUIDOptions() = default;
  CUIDOptions(toolchain::opt::DerivedArgList &Args, const Driver &D);
...
public:
  ...
  using InputTy = std::pair<types::ID, const toolchain::opt::Arg *>;
  ...
  /// process.
  using CC1ToolFunc =
      toolchain::function_ref<int(SmallVectorImpl<const char *> &ArgV)>;
...
private:
  ...
  std::unique_ptr<toolchain::opt::InputArgList> CfgOptionsHead;

  /// Arguments originated from configuration file (tail part).
  std::unique_ptr<toolchain::opt::InputArgList> CfgOptionsTail;

  /// Arguments originated from command line.
  std::unique_ptr<toolchain::opt::InputArgList> CLOptions;

  /// If this is non-null, the driver will prepend this argument before
  /// reinvoking clang. This is useful for the toolchain-driver where clang's
  /// realpath will be to the toolchain binary and not clang, so it must pass
...
public:
  ...
  //       handleArguments.
  phases::ID getFinalPhase(const toolchain::opt::DerivedArgList &DAL,
                           toolchain::opt::Arg **FinalPhaseArg = nullptr) const;

  toolchain::Expected<std::unique_ptr<toolchain::MemoryBuffer>>
  executeProgram(toolchain::ArrayRef<toolchain::StringRef> Args) const;
...
private:
  ...
  mutable toolchain::StringMap<std::unique_ptr<ToolChain>> ToolChains;

private:
  /// TranslateInputArgs - Create a new derived argument list from the input
  /// arguments, after applying the standard argument translations.
  toolchain::opt::DerivedArgList *
  TranslateInputArgs(const toolchain::opt::InputArgList &Args) const;

  // handleArguments - All code related to claiming and printing diagnostics
  // related to arguments to the driver are done here.
  void handleArguments(Compilation &C, toolchain::opt::DerivedArgList &Args,
...
public:
  ...
  toolchain::ArrayRef<std::string> getConfigFiles() const {
    return ConfigFiles;
  }

  const toolchain::opt::OptTable &getOpts() const { return getDriverOptTable(); }

  DiagnosticsEngine &getDiags() const { return Diags; }

  toolchain::vfs::FileSystem &getVFS() const { return *VFS; }
  ...
  OpenMPRuntimeKind getOpenMPRuntime(const toolchain::opt::ArgList &Args) const;
  ...
  /// ArgList.
  toolchain::opt::InputArgList ParseArgStrings(ArrayRef<const char *> Args,
                                          bool UseDriverMode,
  ...
  /// inputs onto.
  void BuildInputs(const ToolChain &TC, toolchain::opt::DerivedArgList &Args,
  ...
  void BuildActions(Compilation &C, toolchain::opt::DerivedArgList &Args,
                    const InputList &Inputs, ActionList &Actions) const;
  ...
  getOffloadArchs(Compilation &C, const toolchain::opt::DerivedArgList &Args,
  ...
  bool DiagnoseInputExistence(const toolchain::opt::DerivedArgList &Args,
  ...
  struct CompilationDiagnosticReport {
    toolchain::SmallVector<std::string, 4> TemporaryFiles;
  };
  ...
  Action *ConstructPhaseAction(
      Compilation &C, const toolchain::opt::ArgList &Args, phases::ID Phase,
      Action *Input,
  ...
  bool ShouldEmitStaticLibrary(const toolchain::opt::ArgList &Args) const;
...
private:
  ...
  ///
  /// \returns true if error occurred.
  bool loadDefaultConfigFiles(toolchain::cl::ExpansionContext &ExpCtx);

  /// Tries to load options from customization file.
  ///
  /// \returns true if error occurred.
  bool loadZOSCustomizationFile(toolchain::cl::ExpansionContext &);
  ...
  bool readConfigFile(StringRef FileName, toolchain::cl::ExpansionContext &ExpCtx);
  ...
  void setLTOMode(const toolchain::opt::ArgList &Args);

  /// Retrieves a ToolChain for a particular \p Target triple.
  ///
  /// Will cache ToolChains for the life of the driver object, and create them
  /// on-demand.
  const ToolChain &getToolChain(const toolchain::opt::ArgList &Args,
  ...
  /// Will cache ToolChains for the life of the driver object, and create them
  /// on-demand.
  const ToolChain &getOffloadToolChain(const toolchain::opt::ArgList &Args,
                                       const Action::OffloadKind Kind,
                                       const toolchain::Triple &Target,
                                       const toolchain::Triple &AuxTarget) const;
...
/// And False otherwise.
bool isOptimizationLevelFast(const toolchain::opt::ArgList &Args);

/// \return True if the argument combination will end up generating remarks.
bool willEmitRemarks(const toolchain::opt::ArgList &Args);
...
/// not be one of these.
toolchain::StringRef getDriverMode(StringRef ProgName, ArrayRef<const char *> Args);
...
toolchain::Error expandResponseFiles(SmallVectorImpl<const char *> &Args,
                                bool ClangCLMode, toolchain::BumpPtrAllocator &Alloc,
                                toolchain::vfs::FileSystem *FS = nullptr);

/// Apply a space separated list of edits to the input argument lists.
/// See applyOneOverrideOption.
void applyOverrideOptions(SmallVectorImpl<const char *> &Args,
                          const char *OverrideOpts,
                          toolchain::StringSet<> &SavedStrings, StringRef EnvVar,
