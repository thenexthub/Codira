...
#ifndef LANGUAGE_CORE_DRIVER_JOB_H
#define LANGUAGE_CORE_DRIVER_JOB_H
...
struct ResponseFileSupport {
  ...
  toolchain::sys::WindowsEncodingMethod ResponseEncoding;
  ...
  static constexpr ResponseFileSupport None() {
    return {RF_None, toolchain::sys::WEM_UTF8, nullptr};
  }
  ...
  static constexpr ResponseFileSupport AtFileUTF8() {
    return {RF_Full, toolchain::sys::WEM_UTF8, "@"};
  }
  ...
  static constexpr ResponseFileSupport AtFileCurCP() {
    return {RF_Full, toolchain::sys::WEM_CurrentCodePage, "@"};
  }
  ...
  static constexpr ResponseFileSupport AtFileUTF16() {
    return {RF_Full, toolchain::sys::WEM_UTF16, "@"};
  }
};
...
class Command {
  ...
  mutable std::optional<toolchain::sys::ProcessStatistics> ProcStat;

  /// When a response file is needed, we try to put most arguments in an
  /// exclusive file, while others remains as regular command line arguments.
  /// This functions fills a vector with the regular command line arguments,
  /// argv, excluding the ones passed in a response file.
  void buildArgvForResponseFile(toolchain::SmallVectorImpl<const char *> &Out) const;
...
public:
  ...
  Command(const Action &Source, const Tool &Creator,
          ...
          const toolchain::opt::ArgStringList &Arguments, ArrayRef<InputInfo> Inputs,
  ...
  virtual void Print(toolchain::raw_ostream &OS, const char *Terminator, bool Quote,
  ...
  /// file support.
  void setInputFileList(toolchain::opt::ArgStringList List) {
  ...
  virtual void setEnvironment(toolchain::ArrayRef<const char *> NewEnvironment);

  void
  setRedirectFiles(const std::vector<std::optional<std::string>> &Redirects);

  void replaceArguments(toolchain::opt::ArgStringList List) {
  ...
  std::optional<toolchain::sys::ProcessStatistics> getProcessStatistics() const {
    return ProcStat;
  }
...
public:
  ...
  void Print(toolchain::raw_ostream &OS, const char *Terminator, bool Quote,
             CrashReportInfo *CrashInfo = nullptr) const override;

  int Execute(ArrayRef<std::optional<StringRef>> Redirects, std::string *ErrMsg,
              bool *ExecutionFailed) const override;

  void setEnvironment(toolchain::ArrayRef<const char *> NewEnvironment) override;
};
...
public:
  ...
  using iterator = toolchain::pointee_iterator<list_type::iterator>;
  using const_iterator = toolchain::pointee_iterator<list_type::const_iterator>;

private:
  list_type Jobs;

public:
  void Print(toolchain::raw_ostream &OS, const char *Terminator,
...
#endif // LANGUAGE_CORE_DRIVER_JOB_H
