...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SYMBOLMANAGER_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SYMBOLMANAGER_H
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID& profile, const TypedValueRegion* R) {
    profile.AddInteger((unsigned)ClassKind);
    profile.AddPointer(R);
  }

  void Profile(toolchain::FoldingSetNodeID& profile) override {
    Profile(profile, R);
  }
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID &profile, ConstCFGElementRef Elem,
                      const LocationContext *LCtx, QualType T, unsigned Count,
                      const void *SymbolTag) {
    profile.AddInteger((unsigned)ClassKind);
    profile.Add(Elem);
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID& profile, SymbolRef parent,
                      const TypedValueRegion *r) {
    profile.AddInteger((unsigned)ClassKind);
    profile.AddPointer(r);
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID& profile, const SubRegion *R) {
    profile.AddInteger((unsigned)ClassKind);
    profile.AddPointer(R);
  }
...
class SymbolMetadata : public SymbolData {
  ...
  public:
    ...
    static void Profile(toolchain::FoldingSetNodeID &profile, const MemRegion *R,
...
public:
  ...
  void Profile(toolchain::FoldingSetNodeID& ID) override {
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID &ID, const SymExpr *In,
...
protected:
  ...
  static unsigned computeOperandComplexity(const toolchain::APSInt &Value) {
    return 1;
  }

  static const toolchain::APSInt *getPointer(APSIntPtr Value) { return Value.get(); }
  static const SymExpr *getPointer(const SymExpr *Value) { return Value; }

  static void dumpToStreamImpl(raw_ostream &os, const SymExpr *Value);
  static void dumpToStreamImpl(raw_ostream &os, const toolchain::APSInt &Value);
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID &ID, LHSTYPE lhs,
...
public:
  explicit SymExprAllocator(toolchain::BumpPtrAllocator &Alloc) : Alloc(Alloc) {}
...
class SymbolManager {
  using DataSetTy = toolchain::FoldingSet<SymExpr>;
  using SymbolDependTy =
      toolchain::DenseMap<SymbolRef, std::unique_ptr<SymbolRefSmallVectorTy>>;
...
class SymbolReaper {
  ...
  using SymbolSetTy = toolchain::DenseSet<SymbolRef>;
  using SymbolMapTy = toolchain::DenseMap<SymbolRef, SymbolStatus>;
  using RegionSetTy = toolchain::DenseSet<const MemRegion *>;
  ...
  toolchain::DenseMap<const MemRegion *, unsigned> includedRegionCache;
...
public:
  ...
  toolchain::iterator_range<RegionSetTy::const_iterator> regions() const {
...
const T *SymbolManager::acquire(Args &&...args) {
  toolchain::FoldingSetNodeID profile;
...
template <>
struct toolchain::ImutContainerInfo<language::Core::ento::SymbolRef>
    : public ImutProfileInfo<language::Core::ento::SymbolRef> {
  using value_type = language::Core::ento::SymbolRef;
  using value_type_ref = language::Core::ento::SymbolRef;
  ...
  static bool isEqual(language::Core::ento::SymbolRef LHS, language::Core::ento::SymbolRef RHS) {
    return LHS->getSymbolID() == RHS->getSymbolID();
  }

  static bool isLess(language::Core::ento::SymbolRef LHS, language::Core::ento::SymbolRef RHS) {
    return LHS->getSymbolID() < RHS->getSymbolID();
  }
...
#endif // LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SYMBOLMANAGER_H
