...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_PROGRAMSTATE_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_PROGRAMSTATE_H
...
class ProgramState : public toolchain::FoldingSetNode {
public:
  typedef toolchain::ImmutableMap<void*, void*>                 GenericDataMap;
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID& ID, const ProgramState *V) {
    V->Env.Profile(ID);
  ...
  /// both) of the returned states may be NULL.
  [[nodiscard]] std::pair<ProgramStateRef, ProgramStateRef>
  assumeInclusiveRange(DefinedOrUnknownSVal Val, const toolchain::APSInt &From,
                       const toolchain::APSInt &To) const;
  ...
  bool scanReachableSymbols(toolchain::iterator_range<region_iterator> Reachable,
                            SymbolVisitor &visitor) const;

  template <typename CB> CB scanReachableSymbols(SVal val) const;
  template <typename CB> CB
  scanReachableSymbols(toolchain::iterator_range<region_iterator> Reachable) const;
...
private:
  ...
  typedef toolchain::DenseMap<void*,std::pair<void*,void (*)(void*)> > GDMContextsTy;
  GDMContextsTy GDMContexts;

  /// StateSet - FoldingSet containing all the states created for analyzing
  ///  a particular function.  This is used to unique states.
  toolchain::FoldingSet<ProgramState> StateSet;
...
public:
  ...
  toolchain::BumpPtrAllocator& getAllocator() { return Alloc; }
...
public:
  ...
  void *FindGDMContext(void *index,
                       void *(*CreateContext)(toolchain::BumpPtrAllocator&),
...
inline ProgramStateRef ProgramState::assumeInclusiveRange(
    DefinedOrUnknownSVal Val, const toolchain::APSInt &From, const toolchain::APSInt &To,
    bool Assumption) const {
  if (Val.isUnknown())
    return this;
...
CB ProgramState::scanReachableSymbols(
    toolchain::iterator_range<region_iterator> Reachable) const {
  CB cb(this);
...
/// returns false.
class ScanReachableSymbols {
  typedef toolchain::DenseSet<const void*> VisitedItems;
