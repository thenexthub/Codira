...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_CALLEVENT_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_CALLEVENT_H
...
private:
  ...
  toolchain::PointerUnion<const Expr *, const Decl *> Origin;
...
private:
  template <typename T> friend struct toolchain::IntrusiveRefCntPtrInfo;
...
public:
  ...
  using param_type_iterator =
      toolchain::mapped_iterator<ArrayRef<ParmVarDecl *>::iterator, GetTypeFn>;
  ...
  param_type_iterator param_type_begin() const {
    return toolchain::map_iterator(parameters().begin(), GetTypeFn());
  }
  /// \sa param_type_begin()
  param_type_iterator param_type_end() const {
    return toolchain::map_iterator(parameters().end(), GetTypeFn());
  }
...
protected:
  using DtorDataTy = toolchain::PointerIntPair<const MemRegion *, 1, bool>;
...
public:
  ...
  std::optional<const language::Core::Expr *> getArraySizeExpr() const {
...
// versions.
// Some pointers:
// http://lists.toolchain.org/pipermail/cfe-dev/2020-April/065080.html
...
public:
  ...
  bool isSetter() const {
    ...
    case OCM_Message:
      toolchain_unreachable("This is not a pseudo-object access!");
    case OCM_PropertyAccess:
      return getNumArgs() > 0;
    case OCM_Subscript:
      return getNumArgs() > 1;
    }
    toolchain_unreachable("Unknown message kind");
  }
...
public:
  CallEventManager(toolchain::BumpPtrAllocator &alloc) : Alloc(alloc) {}
...
template <class T> struct simplify_type<language::Core::ento::CallEventRef<T>> {
  using SimpleType = const T *;

  static SimpleType getSimplifiedValue(language::Core::ento::CallEventRef<T> Val) {
    return Val.get();
  }
};
...
#endif // LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_CALLEVENT_H
