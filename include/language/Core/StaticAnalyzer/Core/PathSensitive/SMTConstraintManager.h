...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SMTCONSTRAINTMANAGER_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SMTCONSTRAINTMANAGER_H
...
typedef toolchain::ImmutableSet<
    std::pair<language::Core::ento::SymbolRef, const toolchain::SMTExpr *>>
    ConstraintSMTType;
...
class SMTConstraintManager : public language::Core::ento::SimpleConstraintManager {
  mutable toolchain::SMTSolverRef Solver = toolchain::CreateZ3Solver();

public:
  SMTConstraintManager(language::Core::ento::ExprEngine *EE,
                       language::Core::ento::SValBuilder &SB)
  ...
  ConditionTruthVal checkNull(ProgramStateRef State, SymbolRef Sym) override {
    ...
    toolchain::SMTExprRef VarExp = SMTConv::getExpr(Solver, Ctx, Sym, &RetTy);
    toolchain::SMTExprRef Exp =
        SMTConv::getZeroExpr(Solver, Ctx, VarExp, RetTy, /*Assumption=*/true);

    // Negate the constraint
    toolchain::SMTExprRef NotExp =
  ...
  const toolchain::APSInt *getSymVal(ProgramStateRef State,
    ...
    if (const SymbolData *SD = dyn_cast<SymbolData>(Sym)) {
      ...
      toolchain::APSInt Value(Ctx.getTypeSize(Ty),
      ...
      toolchain::SMTExprRef Exp = SMTConv::fromData(Solver, Ctx, SD);
      ...
      toolchain::SMTExprRef NotExp = SMTConv::fromBinOp(
          Solver, Exp, BO_NE,
    ...
    if (const BinarySymExpr *BSE = dyn_cast<BinarySymExpr>(Sym)) {
      const toolchain::APSInt *LHS, *RHS;
      if (const SymIntExpr *SIE = dyn_cast<SymIntExpr>(BSE)) {
        ...
        toolchain_unreachable("Unsupported binary expression to get symbol value!");
      }

      if (!LHS || !RHS)
        return nullptr;

      toolchain::APSInt ConvertedLHS, ConvertedRHS;
      QualType LTy, RTy;
      std::tie(ConvertedLHS, LTy) = SMTConv::fixAPSInt(Ctx, *LHS);
      std::tie(ConvertedRHS, RTy) = SMTConv::fixAPSInt(Ctx, *RHS);
      SMTConv::doIntTypeConversion<toolchain::APSInt, &SMTConv::castAPSInt>(
    ...
    toolchain_unreachable("Unsupported expression to get symbol value!");
  }
  ...
  bool canReasonAbout(SVal X) const override {
    ...
    if ((Ty->isSpecificBuiltinType(BuiltinType::LongDouble) &&
         (&TI.getLongDoubleFormat() == &toolchain::APFloat::x87DoubleExtended() ||
          &TI.getLongDoubleFormat() == &toolchain::APFloat::PPCDoubleDouble())))
      return false;
    ...
    toolchain_unreachable("Unsupported expression to reason about!");
  }
...
protected:
  ...
  virtual void addStateConstraints(ProgramStateRef State) const {
    ...
    if (I != IE) {
      toolchain::SMTExprRef Constraint = I++->second;
      while (I != IE) {
  ...
  ConditionTruthVal checkModel(ProgramStateRef State, SymbolRef Sym,
                               const toolchain::SMTExprRef &Exp) const {
  ...
  mutable toolchain::DenseMap<unsigned, ConditionTruthVal> Cached;
