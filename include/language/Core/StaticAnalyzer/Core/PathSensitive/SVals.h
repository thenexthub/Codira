...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SVALS_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SVALS_H
...
public:
  ...
  /// the desired type.
  template <typename T> T castAs() const { return toolchain::cast<T>(*this); }
  ...
  template <typename T> std::optional<T> getAs() const {
    return toolchain::dyn_cast<T>(*this);
  }
  ...
  void Profile(toolchain::FoldingSetNodeID &ID) const {
    ID.AddPointer(Data);
    ID.AddInteger(toolchain::to_underlying(getKind()));
  }
  ...
  const toolchain::APSInt *getAsInteger() const;
  ...
  toolchain::iterator_range<SymExpr::symbol_iterator> symbols() const {
    if (const SymExpr *SE = getAsSymbol(/*IncludeBaseRegions=*/true))
      return SE->symbols();
    SymExpr::symbol_iterator end{};
    return toolchain::make_range(end, end);
  }
...
inline raw_ostream &operator<<(raw_ostream &os, language::Core::ento::SVal V) {
  V.dumpToStream(os);
  return os;
}
...
public:
  ...
  APSIntPtr getValue() const {
    ...
    return APSIntPtr::unsafeConstructor(castDataAs<toolchain::APSInt>());
  }
...
/// Internally, it contains an toolchain::ImmutableList of SVal's stored inside the
/// literal.
...
public:
  using PTMDataType =
      toolchain::PointerUnion<const NamedDecl *, const PointerToMemberData *>;
...
struct CastInfo<
    To, From,
    std::enable_if_t<std::is_base_of<::language::Core::ento::SVal, From>::value>>
    : public CastIsPossible<To, ::language::Core::ento::SVal> {
  using Self = CastInfo<
      To, From,
      std::enable_if_t<std::is_base_of<::language::Core::ento::SVal, From>::value>>;
  static bool isPossible(const From &V) {
    return To::classof(*static_cast<const ::language::Core::ento::SVal *>(&V));
  }
  static std::optional<To> castFailed() { return std::optional<To>{}; }
  static To doCast(const From &f) {
    return *static_cast<const To *>(cast<::language::Core::ento::SVal>(&f));
  }
...
#endif // LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SVALS_H
