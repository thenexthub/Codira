...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_CHECKERCONTEXT_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_CHECKERCONTEXT_H
...
public:
  ...
  *getNoteTag(std::function<std::string(PathSensitiveBugReport &)> &&Cb,
  ...
  /// note.
  ///
  /// @param Cb Callback with 'BugReport &' and 'toolchain::raw_ostream &'.
  /// @param IsPrunable Whether the note is prunable. It allows BugReporter
  ///        to omit the note from the report if it would make the displayed
  ///        bug path significantly shorter.
  const NoteTag *getNoteTag(
      std::function<void(PathSensitiveBugReport &BR, toolchain::raw_ostream &OS)> &&Cb,
      bool IsPrunable = false) {
    return getNoteTag(
  ...
  ///
  /// \sa language::Core::Lexer::getSpelling(), language::Core::Lexer::getImmediateMacroName().
