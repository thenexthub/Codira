...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SMTCONV_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_PATHSENSITIVE_SMTCONV_H
...
public:
  ...
  static inline toolchain::SMTSortRef mkSort(toolchain::SMTSolverRef &Solver,
  ...
  static inline toolchain::SMTExprRef fromUnOp(toolchain::SMTSolverRef &Solver,
                                          const UnaryOperator::Opcode Op,
                                          const toolchain::SMTExprRef &Exp) {
    switch (Op) {
    case UO_Minus:
    ...
    toolchain_unreachable("Unimplemented opcode");
  }

  /// Constructs an SMTSolverRef from a floating-point unary operator.
  static inline toolchain::SMTExprRef fromFloatUnOp(toolchain::SMTSolverRef &Solver,
                                               const UnaryOperator::Opcode Op,
                                               const toolchain::SMTExprRef &Exp) {
    switch (Op) {
    case UO_Minus:
  ...
  static inline toolchain::SMTExprRef
  fromNBinOp(toolchain::SMTSolverRef &Solver, const BinaryOperator::Opcode Op,
             const std::vector<toolchain::SMTExprRef> &ASTs) {
    assert(!ASTs.empty());

    if (Op != BO_LAnd && Op != BO_LOr)
      toolchain_unreachable("Unimplemented opcode");

    toolchain::SMTExprRef res = ASTs.front();
  ...
  static inline toolchain::SMTExprRef fromBinOp(toolchain::SMTSolverRef &Solver,
                                           const toolchain::SMTExprRef &LHS,
                                           const BinaryOperator::Opcode Op,
                                           const toolchain::SMTExprRef &RHS,
                                           bool isSigned) {
  ...
  /// operator.
  static inline toolchain::SMTExprRef
  fromFloatSpecialBinOp(toolchain::SMTSolverRef &Solver, const toolchain::SMTExprRef &LHS,
                        const BinaryOperator::Opcode Op,
                        const toolchain::APFloat::fltCategory &RHS) {
    switch (Op) {
    // Equality operators
    case BO_EQ:
      switch (RHS) {
      case toolchain::APFloat::fcInfinity:
        return Solver->mkFPIsInfinite(LHS);

      case toolchain::APFloat::fcNaN:
        return Solver->mkFPIsNaN(LHS);

      case toolchain::APFloat::fcNormal:
        return Solver->mkFPIsNormal(LHS);

      case toolchain::APFloat::fcZero:
  ...
  static inline toolchain::SMTExprRef fromFloatBinOp(toolchain::SMTSolverRef &Solver,
                                                const toolchain::SMTExprRef &LHS,
                                                const BinaryOperator::Opcode Op,
                                                const toolchain::SMTExprRef &RHS) {
    assert(*Solver->getSort(LHS) == *Solver->getSort(RHS) &&
  ...
  static inline toolchain::SMTExprRef fromCast(toolchain::SMTSolverRef &Solver,
                                          const toolchain::SMTExprRef &Exp,
    ...
    if ((FromTy->isIntegralOrEnumerationType() &&
      ...
      if (FromTy->isBooleanType()) {
        ...
        return Solver->mkIte(
            Exp, Solver->mkBitvector(toolchain::APSInt("1"), ToBitWidth),
            Solver->mkBitvector(toolchain::APSInt("0"), ToBitWidth));
      }
    ...
    if (FromTy->isIntegralOrEnumerationType() && ToTy->isRealFloatingType()) {
      toolchain::SMTSortRef Sort = Solver->getFloatSort(ToBitWidth);
      return FromTy->isSignedIntegerOrEnumerationType()
    ...
    toolchain_unreachable("Unsupported explicit type cast!");
  }

  // Callback function for doCast parameter on APSInt type.
  static inline toolchain::APSInt castAPSInt(toolchain::SMTSolverRef &Solver,
                                        const toolchain::APSInt &V, QualType ToTy,
  ...
  static inline toolchain::SMTExprRef
  fromData(toolchain::SMTSolverRef &Solver, ASTContext &Ctx, const SymbolData *Sym) {
  ...
  static inline toolchain::SMTExprRef getCastExpr(toolchain::SMTSolverRef &Solver,
                                             ASTContext &Ctx,
                                             const toolchain::SMTExprRef &Exp,
  ...
  static inline toolchain::SMTExprRef
  getBinExpr(toolchain::SMTSolverRef &Solver, ASTContext &Ctx,
             const toolchain::SMTExprRef &LHS, QualType LTy,
             BinaryOperator::Opcode Op, const toolchain::SMTExprRef &RHS,
             QualType RTy, QualType *RetTy) {
    toolchain::SMTExprRef NewLHS = LHS;
    toolchain::SMTExprRef NewRHS = RHS;
  ...
  static inline toolchain::SMTExprRef getSymBinExpr(toolchain::SMTSolverRef &Solver,
                                               ASTContext &Ctx,
    ...
    toolchain_unreachable("Unsupported BinarySymExpr type!");
  }

  // Recursive implementation to unpack and generate symbolic expression.
  // Sets the hasComparison and RetTy parameters. See getExpr().
  static inline toolchain::SMTExprRef getSymExpr(toolchain::SMTSolverRef &Solver,
    ...
    if (const UnarySymExpr *USE = dyn_cast<UnarySymExpr>(Sym)) {
      ...
      QualType OperandTy;
      toolchain::SMTExprRef OperandExp =
          getSymExpr(Solver, Ctx, USE->getOperand(), &OperandTy, hasComparison);
      toolchain::SMTExprRef UnaryExp =
    ...
    toolchain_unreachable("Unsupported SymbolRef type!");
  }
  ...
  static inline toolchain::SMTExprRef getExpr(toolchain::SMTSolverRef &Solver,
                                         ASTContext &Ctx, SymbolRef Sym,
  ...
  static inline toolchain::SMTExprRef getZeroExpr(toolchain::SMTSolverRef &Solver,
                                             ASTContext &Ctx,
                                             const toolchain::SMTExprRef &Exp,
                                             QualType Ty, bool Assumption) {
    if (Ty->isRealFloatingType()) {
      toolchain::APFloat Zero =
          toolchain::APFloat::getZero(Ctx.getFloatTypeSemantics(Ty));
    ...
    if (Ty->isIntegralOrEnumerationType() || Ty->isAnyPointerType() ||
      ...
      return fromBinOp(
          ...
          Solver->mkBitvector(toolchain::APSInt("0"), Ctx.getTypeSize(Ty)),
          isSigned);
    }

    toolchain_unreachable("Unsupported type for zero value!");
  }

  // Wrapper to generate SMTSolverRef from a range. If From == To, an
  // equality will be created instead.
  static inline toolchain::SMTExprRef
  getRangeExpr(toolchain::SMTSolverRef &Solver, ASTContext &Ctx, SymbolRef Sym,
               const toolchain::APSInt &From, const toolchain::APSInt &To, bool InRange) {
    // Convert lower bound
    QualType FromTy;
    toolchain::APSInt NewFromInt;
    std::tie(NewFromInt, FromTy) = fixAPSInt(Ctx, From);
    toolchain::SMTExprRef FromExp =
        Solver->mkBitvector(NewFromInt, NewFromInt.getBitWidth());

    // Convert symbol
    QualType SymTy;
    toolchain::SMTExprRef Exp = getExpr(Solver, Ctx, Sym, &SymTy);
    ...
    QualType ToTy;
    toolchain::APSInt NewToInt;
    std::tie(NewToInt, ToTy) = fixAPSInt(Ctx, To);
    toolchain::SMTExprRef ToExp =
    ...
    toolchain::SMTExprRef RHS = getBinExpr(Solver, Ctx, Exp, SymTy,
  ...
  static inline QualType getAPSIntType(ASTContext &Ctx,
    ...
    unsigned Pow2DestWidth =
        std::max(toolchain::bit_ceil(Int.getBitWidth()), CharTypeSize);
    return Ctx.getIntTypeForBitwidth(Pow2DestWidth, Int.isSigned());
  }

  // Get the QualTy for the input APSInt, and fix it if it has a bitwidth of 1.
  static inline std::pair<toolchain::APSInt, QualType>
  fixAPSInt(ASTContext &Ctx, const toolchain::APSInt &Int) {
    toolchain::APSInt NewInt;
    ...
    if (APSIntBitwidth == 1 && Ty.isNull())
      ...
      return {Int.extend(Ctx.getTypeSize(getAPSIntType(Ctx, Int))),
              getAPSIntType(Ctx, NewInt)};
    if (toolchain::isPowerOf2_32(APSIntBitwidth) || Ty.isNull())
      return {Int, Ty};
  ...
  static inline void doTypeConversion(toolchain::SMTSolverRef &Solver,
                                      ASTContext &Ctx, toolchain::SMTExprRef &LHS,
                                      toolchain::SMTExprRef &RHS, QualType &LTy,
                                      QualType &RTy) {
    ...
    if ((LTy->isIntegralOrEnumerationType() &&
      ...
      SMTConv::doIntTypeConversion<toolchain::SMTExprRef, &fromCast>(
          Solver, Ctx, LHS, LTy, RHS, RTy);
      return;
    }

    if (LTy->isRealFloatingType() || RTy->isRealFloatingType()) {
      SMTConv::doFloatTypeConversion<toolchain::SMTExprRef, &fromCast>(
          Solver, Ctx, LHS, LTy, RHS, RTy);
      return;
    }
  ...
  template <typename T, T (*doCast)(toolchain::SMTSolverRef &Solver, const T &,
                                    QualType, uint64_t, QualType, uint64_t)>
  static inline void doIntTypeConversion(toolchain::SMTSolverRef &Solver,
  ...
  template <typename T, T (*doCast)(toolchain::SMTSolverRef &Solver, const T &,
                                    QualType, uint64_t, QualType, uint64_t)>
  static inline void
  doFloatTypeConversion(toolchain::SMTSolverRef &Solver, ASTContext &Ctx, T &LHS,
    ...
    if (order > 0) {
      ...
      toolchain_unreachable("Unsupported floating-point type cast!");
    }
  }
};
} // namespace ento
