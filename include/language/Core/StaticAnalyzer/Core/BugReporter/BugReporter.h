...
#ifndef LANGUAGE_CORE_STATICANALYZER_CORE_BUGREPORTER_BUGREPORTER_H
#define LANGUAGE_CORE_STATICANALYZER_CORE_BUGREPORTER_BUGREPORTER_H
...
using DiagnosticForConsumerMapTy =
    toolchain::DenseMap<PathDiagnosticConsumer *, std::unique_ptr<PathDiagnostic>>;
...
public:
  ...
  toolchain::ArrayRef<FixItHint> getFixits() const { return Fixits; }

  /// Reports are uniqued to ensure that we do not emit multiple diagnostics
  /// for each bug.
  virtual void Profile(toolchain::FoldingSetNodeID& hash) const = 0;
};
...
public:
  ...
  void Profile(toolchain::FoldingSetNodeID& hash) const override;
};
...
public:
  ...
  using visitor_range = toolchain::iterator_range<visitor_iterator>;
...
protected:
  ...
  toolchain::DenseMap<SymbolRef, bugreporter::TrackingKind> InterestingSymbols;
  ...
  /// The stack is largely used by BugReporter when generating PathDiagnostics
  /// for multiple PathDiagnosticConsumers.
  toolchain::DenseMap<const MemRegion *, bugreporter::TrackingKind>
      InterestingRegions;

  /// A set of location contexts that correspoind to call sites which should be
  /// considered "interesting".
  toolchain::SmallSet<const LocationContext *, 2> InterestingLocationContexts;
  ...
  toolchain::FoldingSet<BugReporterVisitor> CallbacksSet;
  ...
  /// \sa markInvalid
  /// \sa removeInvalidation
  toolchain::SmallSet<InvalidationRecord, 4> Invalidations;

  /// Conditions we're already tracking.
  toolchain::SmallSet<const ExplodedNode *, 4> TrackedConditions;
...
class BugReportEquivClass : public toolchain::FoldingSetNode {
  friend class BugReporter;

  /// List of *owned* BugReport objects.
  toolchain::SmallVector<std::unique_ptr<BugReport>, 4> Reports;
...
private:
  ...
  toolchain::FoldingSet<BugReportEquivClass> EQClasses;
...
public:
  ...
  using EQClasses_iterator = toolchain::FoldingSet<BugReportEquivClass>::iterator;
  toolchain::iterator_range<EQClasses_iterator> equivalenceClasses() {
    return EQClasses;
  }
...
private:
  toolchain::StringMap<std::unique_ptr<BugType>> StrBugTypes;
...
#endif // LANGUAGE_CORE_STATICANALYZER_CORE_BUGREPORTER_BUGREPORTER_H
