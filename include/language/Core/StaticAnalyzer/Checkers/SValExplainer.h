...
#ifndef LANGUAGE_CORE_STATICANALYZER_CHECKERS_SVALEXPLAINER_H
#define LANGUAGE_CORE_STATICANALYZER_CHECKERS_SVALEXPLAINER_H
...
public:
  ...
  std::string VisitConcreteInt(loc::ConcreteInt V) {
    const toolchain::APSInt &I = V.getValue();
    std::string Str;
    toolchain::raw_string_ostream OS(Str);
  ...
  std::string VisitNonParamVarRegion(const NonParamVarRegion *R) {
    ...
    if (isa<ParmVarDecl>(VD))
      ...
      toolchain_unreachable("A variable is either local or global");
  }
  ...
  std::string VisitParamVarRegion(const ParamVarRegion *R) {
    ...
    OS << Index << toolchain::getOrdinalSuffix(Index) << " parameter of ";
