...
// reused for other non-C++ languages.  All dependencies on clang/toolchain should
...
#ifndef LANGUAGE_CORE_ANALYSIS_ANALYSES_THREADSAFETYTIL_H
#define LANGUAGE_CORE_ANALYSIS_ANALYSES_THREADSAFETYTIL_H
...
public:
  ...
  StringRef slotName() const {
    ...
    if (!SlotName) {
      SlotName = "";
      toolchain::raw_string_ostream OS(*SlotName);
      Cvdecl->printName(OS);
    }
    return *SlotName;
  }
...
public:
  ...
  /// Return the list of basic blocks that this terminator can branch to.
  ArrayRef<BasicBlock *> successors() const { return toolchain::ArrayRef(Branches); }
...
public:
  ...
  unsigned findPredecessorIndex(const BasicBlock *BB) const {
    auto I = toolchain::find(Predecessors, BB);
...
#endif // LANGUAGE_CORE_ANALYSIS_ANALYSES_THREADSAFETYTIL_H
