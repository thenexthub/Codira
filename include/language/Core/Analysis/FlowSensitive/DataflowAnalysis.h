...
#ifndef LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_DATAFLOWANALYSIS_H
#define LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_DATAFLOWANALYSIS_H
...
#include "language/Core/Analysis/FlowSensitive/WatchedLiteralsSolver.h"
...
public:
  ...
  TypeErasedLattice joinTypeErased(const TypeErasedLattice &E1,
    ...
    Lattice L1 = toolchain::any_cast<const Lattice &>(E1.Value);
    const Lattice &L2 = toolchain::any_cast<const Lattice &>(E2.Value);
    L1.join(L2);
    return {std::move(L1)};
  }
  ...
  LatticeJoinEffect widenTypeErased(TypeErasedLattice &Current,
    ...
    Lattice &C = toolchain::any_cast<Lattice &>(Current.Value);
    const Lattice &P = toolchain::any_cast<const Lattice &>(Previous.Value);
    return widenInternal(Rank0{}, C, P);
  }

  bool isEqualTypeErased(const TypeErasedLattice &E1,
                         const TypeErasedLattice &E2) final {
    const Lattice &L1 = toolchain::any_cast<const Lattice &>(E1.Value);
    const Lattice &L2 = toolchain::any_cast<const Lattice &>(E2.Value);
    return L1 == L2;
  }
  ...
  void transferTypeErased(const CFGElement &Element, TypeErasedLattice &E,
    ...
    Lattice &L = toolchain::any_cast<Lattice &>(E.Value);
...
private:
  ...
  static auto transferBranchInternal(Rank0, Analysis &A, bool Branch,
    ...
    A.transferBranch(Branch, Stmt, toolchain::any_cast<Lattice &>(L.Value), Env);
  }
...
using DiagnosisCallback = toolchain::function_ref<toolchain::SmallVector<Diagnostic>(
...
runDataflowAnalysis(const AdornedCFG &ACFG, AnalysisT &Analysis,
  ...
  if (PostAnalysisCallbacks.Before) {
    TypeErasedCallbacks.Before =
        [&PostAnalysisCallbacks](const CFGElement &Element,
          ...
          auto *Lattice =
              toolchain::any_cast<typename AnalysisT::Lattice>(&State.Lattice.Value);
  ...
  toolchain::transform(
      ...
      [](auto &OptState) {
        return toolchain::transformOptional(
            std::move(OptState), [](TypeErasedDataflowAnalysisState &&State) {
              return DataflowAnalysisState<typename AnalysisT::Lattice>{
                  toolchain::any_cast<typename AnalysisT::Lattice>(
...
/// value.
template <typename AnalysisT, typename Diagnostic>
toolchain::Expected<toolchain::SmallVector<Diagnostic>>
diagnoseFunction(const FunctionDecl &FuncDecl, ASTContext &ASTCtx,
                 ...
                 std::int32_t MaxBlockVisits = kDefaultMaxBlockVisits) {
  toolchain::Expected<AdornedCFG> Context = AdornedCFG::build(FuncDecl);
  if (!Context)
  ...
  toolchain::SmallVector<Diagnostic> Diagnostics;
         ...
         &Diagnostics](const CFGElement &Elt,
          ...
          auto EltDiagnostics = Diagnoser.Before(
              ...
              TransferStateForDiagnostics<typename AnalysisT::Lattice>(
                  toolchain::any_cast<const typename AnalysisT::Lattice &>(
                      State.Lattice.Value),
                  State.Env));
          toolchain::move(EltDiagnostics, std::back_inserter(Diagnostics));
        };
  }
  if (Diagnoser.After) {
    PostAnalysisCallbacks.After =
  ...
  if (Solver->reachedLimit())
    return toolchain::createStringError(toolchain::errc::interrupted,
...
#endif // LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_DATAFLOWANALYSIS_H
