...
#ifndef LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_MATCHSWITCH_H_
#define LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_MATCHSWITCH_H_
  ...
  ASTMatchSwitch<BaseT, State, Result> Build() && {
    return [Matcher = BuildMatcher(), Actions = std::move(Actions)](
      ...
      for (const auto &Element : Results[0].getMap()) {
        toolchain::StringRef ID(Element.first);
        size_t Index = 0;
...
private:
  ast_matchers::internal::DynTypedMatcher BuildMatcher() {
    ...
    for (int I = 0, N = Matchers.size(); I < N; ++I) {
      std::string Tag = ("Tag" + toolchain::Twine(I)).str();
...
#endif // LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_MATCHSWITCH_H_
