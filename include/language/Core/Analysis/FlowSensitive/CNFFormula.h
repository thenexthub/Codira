...
#ifndef LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_CNFFORMULA_H
#define LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_CNFFORMULA_H
...
public:
  ...
  toolchain::ArrayRef<Literal> clauseLiterals(ClauseID C) const {
    size_t S = clauseSize(C);
    if (S == 0)
      return toolchain::ArrayRef<Literal>();
    return toolchain::ArrayRef<Literal>(&Clauses[ClauseStarts[C]], S);
  }
...
#endif // LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_CNFFORMULA_H
