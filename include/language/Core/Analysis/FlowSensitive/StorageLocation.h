...
#ifndef LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_STORAGELOCATION_H
#define LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_STORAGELOCATION_H
...
public:
  using FieldToLoc = toolchain::DenseMap<const ValueDecl *, StorageLocation *>;
  using SyntheticFieldMap = toolchain::StringMap<StorageLocation *>;
  ...
  StorageLocation *getChild(const ValueDecl &D) const {
    ...
    LLVM_DEBUG({
      if (It == Children.end()) {
        toolchain::dbgs() << "Couldn't find child " << D.getNameAsString()
                     << " on StorageLocation " << this << " of type "
                     << getType() << "\n";
        toolchain::dbgs() << "Existing children:\n";
        for ([[maybe_unused]] auto [Field, Loc] : Children) {
          toolchain::dbgs() << Field->getNameAsString() << "\n";
        }
      }
    });
  ...
  StorageLocation &getSyntheticField(toolchain::StringRef Name) const {
    StorageLocation *Loc = SyntheticFields.lookup(Name);
    assert(Loc != nullptr);
    return *Loc;
  }

  toolchain::iterator_range<SyntheticFieldMap::const_iterator>
  ...
  toolchain::iterator_range<FieldToLoc::const_iterator> children() const {
...
#endif // LANGUAGE_CORE_ANALYSIS_FLOWSENSITIVE_STORAGELOCATION_H
