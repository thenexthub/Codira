...
#ifndef LANGUAGE_CORE_ANALYSIS_PROGRAMPOINT_H
#define LANGUAGE_CORE_ANALYSIS_PROGRAMPOINT_H
...
private:
  const void *Data1;
  toolchain::PointerIntPair<const void *, 2, unsigned> Data2;

  // The LocationContext could be NULL to allow ProgramPoint to be used in
  // context insensitive analysis.
  toolchain::PointerIntPair<const LocationContext *, 2, unsigned> L;

  toolchain::PointerIntPair<const ProgramPointTag *, 2, unsigned> Tag;
...
public:
  ...
  void printJson(toolchain::raw_ostream &Out, const char *NL = "\n") const;
...
namespace toolchain { // Traits specialization for DenseMap

template <> struct DenseMapInfo<language::Core::ProgramPoint> {

static inline language::Core::ProgramPoint getEmptyKey() {
  uintptr_t x =
   reinterpret_cast<uintptr_t>(DenseMapInfo<void*>::getEmptyKey()) & ~0x7;
  return language::Core::BlockEntrance(nullptr, reinterpret_cast<language::Core::CFGBlock *>(x),
                              nullptr);
}

static inline language::Core::ProgramPoint getTombstoneKey() {
  uintptr_t x =
...
static unsigned getHashValue(const language::Core::ProgramPoint &Loc) {
  return Loc.getHashValue();
}

static bool isEqual(const language::Core::ProgramPoint &L,
                    const language::Core::ProgramPoint &R) {
  return L == R;
}
