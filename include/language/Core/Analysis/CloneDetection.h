...
#ifndef LANGUAGE_CORE_ANALYSIS_CLONEDETECTION_H
#define LANGUAGE_CORE_ANALYSIS_CLONEDETECTION_H
...
public:
  ...
  typedef toolchain::SmallVector<StmtSequence, 8> CloneGroup;
...
public:
  ...
  static void filterGroups(
      ...
      toolchain::function_ref<bool(const CloneDetector::CloneGroup &)> Filter) {
    toolchain::erase_if(CloneGroups, Filter);
  }
...
struct FilenamePatternConstraint {
  ...
  std::shared_ptr<toolchain::Regex> IgnoredFilesRegex;

  FilenamePatternConstraint(StringRef IgnoredFilesPattern)
      : IgnoredFilesPattern(IgnoredFilesPattern) {
    IgnoredFilesRegex = std::make_shared<toolchain::Regex>("^(" +
...
#endif // LANGUAGE_CORE_ANALYSIS_CLONEDETECTION_H
