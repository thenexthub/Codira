...
#ifndef LANGUAGE_CORE_ASTMATCHERS_ASTMATCHERSINTERNAL_H
#define LANGUAGE_CORE_ASTMATCHERS_ASTMATCHERSINTERNAL_H
...
struct VariadicFunction {
  ...
  ResultT operator()(ArrayRef<ArgT> Args) const {
    return Func(toolchain::to_vector<8>(toolchain::make_pointer_range(Args)));
  }
...
inline language::Core::AccessSpecifier getAccessSpecifier(const Decl &Node) {
  return Node.getAccess();
}

inline language::Core::AccessSpecifier getAccessSpecifier(const CXXBaseSpecifier &Node) {
...
public:
  ...
  bool removeBindings(const ExcludePredicate &Predicate) {
    toolchain::erase_if(Bindings, Predicate);
...
class DynMatcherInterface
    : public toolchain::ThreadSafeRefCountedBase<DynMatcherInterface> {
public:
  ...
  virtual std::optional<language::Core::TraversalKind> TraversalKind() const {
    return std::nullopt;
  }
};
...
public:
  ...
  std::optional<language::Core::TraversalKind> getTraversalKind() const {
...
public:
  ...
  public:
    ...
    std::optional<language::Core::TraversalKind> TraversalKind() const override {
...
static constexpr bool has_getDecl =
    toolchain::is_detected<check_has_getDecl, T>::value;
...
private:
  ...
  bool matchesSpecialized(const CXXOperatorCallExpr &Node) const {
    return toolchain::is_contained(Names, getOperatorSpelling(Node.getOperator()));
  }
...
makeAllOfComposite(ArrayRef<const Matcher<T> *> InnerMatchers) {
  ...
  using PI = toolchain::pointee_iterator<const Matcher<T> *const *>;
...
template <typename T> class TraversalMatcher : public MatcherInterface<T> {
  DynTypedMatcher InnerMatcher;
  language::Core::TraversalKind Traversal;

public:
  explicit TraversalMatcher(language::Core::TraversalKind TK,
...
inline bool ValueEqualsMatcher<FloatingLiteral, double>::matchesNode(
  ...
  if ((&Node.getSemantics()) == &toolchain::APFloat::IEEEsingle())
    return Node.getValue().convertToFloat() == ExpectedValue;
  if ((&Node.getSemantics()) == &toolchain::APFloat::IEEEdouble())
...
template <>
inline bool ValueEqualsMatcher<FloatingLiteral, toolchain::APFloat>::matchesNode(
    const FloatingLiteral &Node) const {
  return ExpectedValue.compare(Node.getValue()) == toolchain::APFloat::cmpEqual;
}
...
public:
  static const Matcher &getInstance() {
    static toolchain::ManagedStatic<Wrapper> Instance;
    return Instance->M;
  }
};
...
public:
  ...
  bool matchesNode(const T &Node) const override {
    ...
    return OptOpName && toolchain::is_contained(Names, *OptOpName);
  }
...
std::shared_ptr<toolchain::Regex> createAndVerifyRegex(StringRef Regex,
                                                  toolchain::Regex::RegexFlags Flags,
                                                  StringRef MatcherID);
...
MatchTemplateArgLocAt(const DeclRefExpr &Node, unsigned int Index,
  ...
  toolchain::ArrayRef<TemplateArgumentLoc> ArgLocs = Node.template_arguments();
...
#endif // LANGUAGE_CORE_ASTMATCHERS_ASTMATCHERSINTERNAL_H
