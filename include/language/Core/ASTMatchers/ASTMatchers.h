...
#ifndef LANGUAGE_CORE_ASTMATCHERS_ASTMATCHERS_H
#define LANGUAGE_CORE_ASTMATCHERS_ASTMATCHERS_H
...
public:
  ...
  /// type of \c language::Core::DynTypedNode
...
AST_MATCHER_P(CXXRecordDecl, hasDirectBase, internal::Matcher<CXXBaseSpecifier>,
  ...
  return Node.hasDefinition() &&
         toolchain::any_of(Node.bases(), [&](const CXXBaseSpecifier &Base) {
...
AST_MATCHER_P(LambdaExpr, hasAnyCapture, internal::Matcher<LambdaCapture>,
  ...
  for (const LambdaCapture &Capture : Node.captures()) {
    language::Core::ast_matchers::internal::BoundNodesTreeBuilder Result(*Builder);
...
AST_POLYMORPHIC_MATCHER_P2(forEachArgumentWithParamType,
  ...
  if (auto *Call = toolchain::dyn_cast<CallExpr>(&Node))
    matchEachArgumentWithParamType(*Call, ProcessParamAndArg);
  else if (auto *Construct = toolchain::dyn_cast<CXXConstructExpr>(&Node))
    matchEachArgumentWithParamType(*Construct, ProcessParamAndArg);
  else
    toolchain_unreachable("expected CallExpr or CXXConstructExpr");
...
AST_MATCHER_P(ParmVarDecl, isAtPosition, unsigned, N) {
  const language::Core::DeclContext *Context = Node.getParentFunctionOrMethod();
...
AST_POLYMORPHIC_MATCHER_P(
  ...
  ArrayRef<TemplateArgument> TemplateArgs =
      language::Core::ast_matchers::internal::getTemplateSpecializationArgs(Node);
  language::Core::ast_matchers::internal::BoundNodesTreeBuilder Result;
  bool Matched = false;
  for (const auto &Arg : TemplateArgs) {
    language::Core::ast_matchers::internal::BoundNodesTreeBuilder ArgBuilder(*Builder);
...
///   int baz = 42;
///   [[language::Core::require_constant_initialization]] int xyz = 42;
/// \endcode
/// varDecl(isConstinit())
...
AST_POLYMORPHIC_MATCHER(isInline, AST_POLYMORPHIC_SUPPORTED_TYPES(NamespaceDecl,
  ...
  if (const auto *VD = dyn_cast<VarDecl>(&Node))
    return VD->isInline();
  toolchain_unreachable("Not a valid polymorphic type");
}
...
/// \endcode
/// decl(hasAttr(language::Core::attr::CUDADevice)) matches the function declaration of
...
AST_MATCHER_P(DecompositionDecl, hasAnyBinding, internal::Matcher<BindingDecl>,
              InnerMatcher) {
  return toolchain::any_of(Node.bindings(), [&](const auto *Binding) {
...
AST_MATCHER_P(Stmt, forFunction, internal::Matcher<FunctionDecl>,
  ...
  toolchain::SmallVector<DynTypedNode, 8> Stack(Parents.begin(), Parents.end());
  while (!Stack.empty()) {
    ...
    if (const auto *FuncDeclNode = CurNode.get<FunctionDecl>()) {
      ...
      toolchain::append_range(Stack, Finder->getASTContext().getParents(CurNode));
    }
  }
  return false;
}
...
AST_MATCHER_P(CXXNewExpr, hasAnyPlacementArg, internal::Matcher<Expr>,
              InnerMatcher) {
  return toolchain::any_of(Node.placement_arguments(), [&](const Expr *Arg) {
...
AST_MATCHER(OMPDefaultClause, isNoneKind) {
  return Node.getDefaultKind() == toolchain::omp::OMP_DEFAULT_none;
}
...
AST_MATCHER(OMPDefaultClause, isSharedKind) {
  return Node.getDefaultKind() == toolchain::omp::OMP_DEFAULT_shared;
}
...
AST_MATCHER(OMPDefaultClause, isPrivateKind) {
  return Node.getDefaultKind() == toolchain::omp::OMP_DEFAULT_private;
}
...
AST_MATCHER(OMPDefaultClause, isFirstPrivateKind) {
  return Node.getDefaultKind() == toolchain::omp::OMP_DEFAULT_firstprivate;
}
...
AST_MATCHER_P(OMPExecutableDirective, isAllowedToContainClauseKind,
  ...
  return toolchain::omp::isAllowedClauseForDirective(
...
#endif // LANGUAGE_CORE_ASTMATCHERS_ASTMATCHERS_H
