...
#ifndef LANGUAGE_CORE_ASTMATCHERS_ASTMATCHFINDER_H
#define LANGUAGE_CORE_ASTMATCHERS_ASTMATCHFINDER_H
...
public:
  ...
  struct MatchResult {
    MatchResult(const BoundNodes &Nodes, language::Core::ASTContext *Context);
    ...
    /// @{
    language::Core::ASTContext * const Context;
    language::Core::SourceManager * const SourceManager;
    /// @}
  };
  ...
  struct MatchFinderOptions {
    struct Profiling {
      Profiling(toolchain::StringMap<toolchain::TimeRecord> &Records)
          : Records(Records) {}

      /// Per bucket timing information.
      toolchain::StringMap<toolchain::TimeRecord> &Records;
    };
  ...
  template <typename T> void match(const T &Node, ASTContext &Context) {
    match(language::Core::DynTypedNode::create(Node), Context);
  }
  void match(const language::Core::DynTypedNode &Node, ASTContext &Context);
  /// @}
  ...
  struct MatchersByType {
    ...
    toolchain::SmallPtrSet<MatchCallback *, 16> AllCallbacks;
  };
