...
#ifndef LANGUAGE_CORE_SEMA_SCOPEINFO_H
#define LANGUAGE_CORE_SEMA_SCOPEINFO_H
...
public:
  ...
  SourceLocation Loc;
  toolchain::TinyPtrVector<const Stmt*> Stmts;
...
public:
  ...
  using SwitchInfo = toolchain::PointerIntPair<SwitchStmt*, 1, bool>;
  ...
  toolchain::SmallMapVector<ParmVarDecl *, Stmt *, 4> CoroutineParameterMoves;
  ...
  toolchain::SmallPtrSet<const BlockDecl *, 1> Blocks;

  /// The set of __block variables that are introduced in this function.
  toolchain::TinyPtrVector<VarDecl *> ByrefBlockVars;
  ...
  toolchain::SmallPtrSet<const ParmVarDecl *, 8> ModifiedNonNullParams;

  /// The set of GNU address of label extension "&&label".
  toolchain::SmallVector<AddrLabelExpr *, 4> AddrLabels;
...
public:
  ...
  class WeakObjectProfileTy {
    ...
    ///
    /// \sa isExactProfile()
    using BaseInfoTy = toolchain::PointerIntPair<const NamedDecl *, 1, bool>;
    BaseInfoTy Base;
  ...
  public:
    ...
    // For use in DenseMap.
    // We can't specialize the usual toolchain::DenseMapInfo at the end of the file
    ...
    public:
      ...
      static unsigned getHashValue(const WeakObjectProfileTy &Val) {
        ...
        return toolchain::DenseMapInfo<Pair>::getHashValue(Pair(Val.Base,
                                                           Val.Property));
      }
  ...
  /// Part of the implementation of -Wrepeated-use-of-weak.
  class WeakUseTy {
    toolchain::PointerIntPair<const Expr *, 1, bool> Rep;
  ...
  using WeakObjectUseMap =
      toolchain::SmallDenseMap<WeakObjectProfileTy, WeakUseVector, 8,
...
public:
  ...
  void setFirstCoroutineStmt(SourceLocation Loc, StringRef Keyword) {
    ...
    FirstCoroutineStmtKind =
        toolchain::StringSwitch<unsigned char>(Keyword)
            .Case("co_return",
                  toolchain::to_underlying(FirstCoroutineStmtKind::CoReturn))
            .Case("co_await",
                  toolchain::to_underlying(FirstCoroutineStmtKind::CoAwait))
            .Case("co_yield",
                  toolchain::to_underlying(FirstCoroutineStmtKind::CoYield));
  }
  ...
  StringRef getFirstCoroutineStmtKeyword() const {
    ...
    toolchain_unreachable("FirstCoroutineStmtKind has an invalid value");
  }
...
public:
  ...
  toolchain::DenseMap<ValueDecl *, unsigned> CaptureMap;
  ...
  const Capture &getCapture(ValueDecl *Var) const {
    toolchain::DenseMap<ValueDecl *, unsigned>::const_iterator Known =
        CaptureMap.find(Var);
...
public:
  ...
  StringRef getRegionName() const {
    ...
    toolchain_unreachable("Invalid captured region kind!");
  }
...
public:
  ...
  toolchain::SmallVector<Expr*, 4> PotentiallyCapturingExprs;

  /// Contains all variable-referring-expressions that refer
  ///  to local variables that are usable as constant expressions and
  ///  do not involve an odr-use (they may still need to be captured
  ///  if the enclosing full-expression is instantiation dependent).
  toolchain::SmallSet<Expr *, 8> NonODRUsedCapturingExprs;

  /// A map of explicit capture indices to their introducer source ranges.
  toolchain::DenseMap<unsigned, SourceRange> ExplicitCaptureRanges;
  ...
  toolchain::SmallVector<ShadowedOuterDecl, 4> ShadowingDecls;

  SourceLocation PotentialThisCaptureLocation;

  /// Variables that are potentially ODR-used in CUDA/HIP.
  toolchain::SmallPtrSet<VarDecl *, 4> CUDAPotentialODRUsedVars;
  ...
  void removePotentialCapture(Expr *E) {
    toolchain::erase(PotentiallyCapturingExprs, E);
  }
  ...
  void visitPotentialCaptures(
      toolchain::function_ref<void(ValueDecl *, Expr *)> Callback) const;
...
#endif // LANGUAGE_CORE_SEMA_SCOPEINFO_H
