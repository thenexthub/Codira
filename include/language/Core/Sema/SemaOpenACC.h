...
#ifndef LANGUAGE_CORE_SEMA_SEMAOPENACC_H
#define LANGUAGE_CORE_SEMA_SEMAOPENACC_H
...
private:
  ...
  struct CollapseCheckingInfo {
    ...
    std::optional<toolchain::APSInt> CurCollapseCount;
  ...
  toolchain::SmallVector<OpenACCReductionClause *> ActiveReductionClauses;
  ...
  toolchain::DenseMap<const language::Core::DeclaratorDecl *, SourceLocation>
  ...
  // check them later.
  toolchain::SmallDenseMap<const language::Core::FunctionDecl *, SourceLocation>
      MagicStaticLocs;
...
public:
  ...
  public:
    ...
    void setIntExprDetails(toolchain::SmallVector<Expr *> &&IntExprs) {
      assert((ClauseKind == OpenACCClauseKind::NumGangs ||
              ClauseKind == OpenACCClauseKind::NumWorkers ||
              ClauseKind == OpenACCClauseKind::Async ||
              ClauseKind == OpenACCClauseKind::DeviceNum ||
    ...
    void setGangDetails(toolchain::SmallVector<OpenACCGangKind> &&GKs,
    ...
    void setVarListDetails(toolchain::SmallVector<Expr *> &&VarList,
                           OpenACCModifierKind ModKind) {
      assert((ClauseKind == OpenACCClauseKind::Private ||
              ClauseKind == OpenACCClauseKind::NoCreate ||
              ClauseKind == OpenACCClauseKind::Present ||
    ...
    void setDeviceTypeDetails(toolchain::SmallVector<DeviceTypeArgument> &&Archs) {
      assert((ClauseKind == OpenACCClauseKind::DeviceType ||
              ClauseKind == OpenACCClauseKind::DType) &&
             "Only 'device_type'/'dtype' has a device-type-arg list");
  ...
  toolchain::SmallVector<Expr *> CheckLinkClauseVarList(ArrayRef<Expr *> VarExpr);
...
#endif // LANGUAGE_CORE_SEMA_SEMAOPENACC_H
