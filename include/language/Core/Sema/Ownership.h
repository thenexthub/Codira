...
#ifndef LANGUAGE_CORE_SEMA_OWNERSHIP_H
#define LANGUAGE_CORE_SEMA_OWNERSHIP_H
...
class TemplateParameterList;
  ...
  class OpaquePtr {
    ...
    using Traits = toolchain::PointerLikeTypeTraits<PtrTy>;
...
namespace toolchain {
  ...
  template <class T>
  struct PointerLikeTypeTraits<language::Core::OpaquePtr<T>> {
    static constexpr int NumLowBitsAvailable = 0;

    static inline void *getAsVoidPointer(language::Core::OpaquePtr<T> P) {
      // FIXME: Doesn't work? return P.getAs< void >();
      return P.getAsOpaquePtr();
    }

    static inline language::Core::OpaquePtr<T> getFromVoidPointer(void *P) {
      return language::Core::OpaquePtr<T>::getFromOpaquePtr(P);
    }
  };
...
template <typename PtrTy> class ActionResult<PtrTy, true> {
  ...
  using PtrTraits = toolchain::PointerLikeTypeTraits<PtrTy>;
...
#endif // LANGUAGE_CORE_SEMA_OWNERSHIP_H
