...
#ifndef LANGUAGE_CORE_SEMA_PARSEDATTR_H
#define LANGUAGE_CORE_SEMA_PARSEDATTR_H
...
using ArgsUnion = toolchain::PointerUnion<Expr *, IdentifierLoc *>;
using ArgsVector = toolchain::SmallVector<ArgsUnion, 12U>;
...
class AttributePool {
  ...
  toolchain::SmallVector<ParsedAttr *, 2> Attrs;
  ...
  void remove(ParsedAttr *attr) {
    assert(toolchain::is_contained(Attrs, attr) &&
           "Can't take attribute from a pool that doesn't own it!");
    Attrs.erase(toolchain::find(Attrs, attr));
  }
...
class ParsedAttributesView {
  friend class AttributePool;
  using VecTy = toolchain::SmallVector<ParsedAttr *, 2>;
...
public:
  ...
  void remove(ParsedAttr *ToBeRemoved) {
    ...
    AttrList.erase(toolchain::find(AttrList, ToBeRemoved));
  }

  void clearListOnly() { AttrList.clear(); }

  struct iterator : toolchain::iterator_adaptor_base<iterator, VecTy::iterator,
  ...
  struct const_iterator
      : toolchain::iterator_adaptor_base<const_iterator, VecTy::const_iterator,
                                    std::random_access_iterator_tag,
                                    ParsedAttr> {
  ...
  bool hasAttribute(ParsedAttr::Kind K) const {
    return toolchain::any_of(AttrList, [K](const ParsedAttr *AL) {
      return AL->getParsedKind() == K;
    });
  }

  const ParsedAttr *getMSPropertyAttr() const {
    auto It = toolchain::find_if(AttrList, [](const ParsedAttr *AL) {
...
#endif // LANGUAGE_CORE_SEMA_PARSEDATTR_H
