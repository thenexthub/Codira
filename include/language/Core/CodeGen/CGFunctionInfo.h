...
#ifndef LANGUAGE_CORE_CODEGEN_CGFUNCTIONINFO_H
#define LANGUAGE_CORE_CODEGEN_CGFUNCTIONINFO_H
...
private:
  toolchain::Type *TypeData; // canHaveCoerceToType()
  union {
    toolchain::Type *PaddingType; // canHavePaddingType()
    toolchain::Type *UnpaddedCoerceAndExpandType; // isCoerceAndExpand()
  };
  ...
  void setPaddingType(toolchain::Type *T) {
    assert(canHavePaddingType());
    PaddingType = T;
  }

  void setUnpaddedCoerceToType(toolchain::Type *T) {
...
public:
  ...
  static ABIArgInfo getDirect(toolchain::Type *T = nullptr, unsigned Offset = 0,
                              toolchain::Type *Padding = nullptr,
  ...
  static ABIArgInfo getDirectInReg(toolchain::Type *T = nullptr) {
    auto AI = getDirect(T);
    AI.setInReg(true);
    return AI;
  }

  static ABIArgInfo getSignExtend(QualType Ty, toolchain::Type *T = nullptr) {
  ...
  static ABIArgInfo getZeroExtend(QualType Ty, toolchain::Type *T = nullptr) {
    assert(Ty->isIntegralOrEnumerationType() && "Unexpected QualType");
    auto AI = ABIArgInfo(Extend);
    AI.setCoerceToType(T);
    AI.setPaddingType(nullptr);
  ...
  static ABIArgInfo getExtend(QualType Ty, toolchain::Type *T = nullptr) {
    assert(Ty->isIntegralOrEnumerationType() && "Unexpected QualType");
  ...
  static ABIArgInfo getNoExtend(toolchain::IntegerType *T) {
    auto AI = ABIArgInfo(Extend);
    AI.setCoerceToType(T);
    AI.setPaddingType(nullptr);
    AI.setDirectOffset(0);
  ...
  static ABIArgInfo getExtendInReg(QualType Ty, toolchain::Type *T = nullptr) {
  ...
  static ABIArgInfo getCoerceAndExpand(toolchain::StructType *coerceToType,
                                       toolchain::Type *unpaddedCoerceToType) {
#ifndef NDEBUG
    // Check that unpaddedCoerceToType has roughly the right shape.

    // Assert that we only have a struct type if there are multiple elements.
    auto unpaddedStruct = dyn_cast<toolchain::StructType>(unpaddedCoerceToType);
  ...
  static bool isPaddingForCoerceAndExpand(toolchain::Type *eltType) {
  ...
  toolchain::Type *getPaddingType() const {
  ...
  toolchain::Type *getCoerceToType() const {
    assert(canHaveCoerceToType() && "Invalid kind!");
    return TypeData;
  }

  void setCoerceToType(toolchain::Type *T) {
    assert(canHaveCoerceToType() && "Invalid kind!");
    TypeData = T;
  }

  toolchain::StructType *getCoerceAndExpandType() const {
    assert(isCoerceAndExpand());
    return cast<toolchain::StructType>(TypeData);
  }

  toolchain::Type *getUnpaddedCoerceAndExpandType() const {
    assert(isCoerceAndExpand());
    return UnpaddedCoerceAndExpandType;
  }

  ArrayRef<toolchain::Type *>getCoerceAndExpandTypeSequence() const {
    assert(isCoerceAndExpand());
    if (auto structTy =
          dyn_cast<toolchain::StructType>(UnpaddedCoerceAndExpandType)) {
      return structTy->elements();
    } else {
      return toolchain::ArrayRef(&UnpaddedCoerceAndExpandType, 1);
    }
  }
...
class CGFunctionInfo final
    : public toolchain::FoldingSetNode,
      private toolchain::TrailingObjects<CGFunctionInfo, CGFunctionInfoArgInfo,
  ...
  /// The language::Core::CallingConv that this was originally created with.
  ...
  toolchain::StructType *ArgStruct;
...
public:
  ...
  create(unsigned toolchainCC, bool instanceMethod, bool chainCall,
  ...
  ArrayRef<ExtParameterInfo> getExtParameterInfos() const {
    ...
    return toolchain::ArrayRef(getExtParameterInfosBuffer(), NumArgs);
  }
  ...
  toolchain::StructType *getArgStruct() const { return ArgStruct; }
  CharUnits getArgStructAlignment() const {
    return CharUnits::fromQuantity(ArgStructAlign);
  }
  void setArgStruct(toolchain::StructType *Ty, CharUnits Align) {
    ArgStruct = Ty;
  ...
  void setMaxVectorWidth(unsigned Width) {
    assert(toolchain::isPowerOf2_32(Width) && "Expected power of 2 vector");
    MaxVectorWidth = toolchain::countr_zero(Width) + 1;
  }
  ...
  static void Profile(toolchain::FoldingSetNodeID &ID, bool InstanceMethod,
