...
#ifndef LANGUAGE_CORE_CODEGEN_CONSTANTINITBUILDER_H
#define LANGUAGE_CORE_CODEGEN_CONSTANTINITBUILDER_H
...
class ConstantInitBuilderBase {
  struct SelfReference {
    toolchain::GlobalVariable *Dummy;
    toolchain::SmallVector<toolchain::Constant*, 4> Indices;

    SelfReference(toolchain::GlobalVariable *dummy) : Dummy(dummy) {}
  };
  CodeGenModule &CGM;
  toolchain::SmallVector<toolchain::Constant*, 16> Buffer;
...
private:
  toolchain::GlobalVariable *createGlobal(toolchain::Constant *initializer,
  ...
  ConstantInitFuture createFuture(toolchain::Constant *initializer);

  void setGlobalInitializer(toolchain::GlobalVariable *GV,
                            toolchain::Constant *initializer);

  void resolveSelfReferences(toolchain::GlobalVariable *GV);
...
protected:
  ...
  toolchain::SmallVectorImpl<toolchain::Constant*> &getBuffer() {
    return Builder.Buffer;
  }

  const toolchain::SmallVectorImpl<toolchain::Constant*> &getBuffer() const {
    return Builder.Buffer;
  }
...
public:
  ...
  void add(toolchain::Constant *value) {
  ...
  void addInt(toolchain::IntegerType *intTy, uint64_t value,
              bool isSigned = false) {
    add(toolchain::ConstantInt::get(intTy, value, isSigned));
  }
  ...
  void addNullPointer(toolchain::PointerType *ptrTy) {
    add(toolchain::ConstantPointerNull::get(ptrTy));
  }

  /// Add a bunch of new values to this initializer.
  void addAll(toolchain::ArrayRef<toolchain::Constant *> values) {
    assert(!Finished && "cannot add more values after finishing builder");
    assert(!Frozen && "cannot add values while subbuilder is active");
  ...
  void addRelativeOffset(toolchain::IntegerType *type, toolchain::Constant *target) {
  ...
  void addRelativeOffsetToPosition(toolchain::IntegerType *type,
                                   toolchain::Constant *target, size_t position) {
  ...
  void addTaggedRelativeOffset(toolchain::IntegerType *type,
                               toolchain::Constant *address,
                               unsigned tag) {
    toolchain::Constant *offset = getRelativeOffset(type, address);
    if (tag) {
      offset = toolchain::ConstantExpr::getAdd(offset,
                                          toolchain::ConstantInt::get(type, tag));
    }
    add(offset);
  }
  ...
  PlaceholderPosition addPlaceholderWithSize(toolchain::Type *expectedType);

  /// Fill a previously-added placeholder.
  void fillPlaceholderWithInt(PlaceholderPosition position,
                              toolchain::IntegerType *type, uint64_t value,
                              bool isSigned = false) {
    fillPlaceholder(position, toolchain::ConstantInt::get(type, value, isSigned));
  }

  /// Fill a previously-added placeholder.
  void fillPlaceholder(PlaceholderPosition position, toolchain::Constant *value) {
    assert(!Finished && "cannot change values after finishing builder");
    assert(!Frozen && "cannot add values while subbuilder is active");
    toolchain::Constant *&slot = Builder.Buffer[position.Index];
  ...
  toolchain::Constant *getAddrOfCurrentPosition(toolchain::Type *type);
  ...
  /// The returned pointer will have type T*, where T is the given type. This
  /// type can differ from the type of the actual element.
  toolchain::Constant *getAddrOfPosition(toolchain::Type *type, size_t position);

  toolchain::ArrayRef<toolchain::Constant*> getGEPIndicesToCurrentPosition(
...
protected:
  toolchain::Constant *finishArray(toolchain::Type *eltTy);
  toolchain::Constant *finishStruct(toolchain::StructType *structTy);

private:
  void getGEPIndicesTo(toolchain::SmallVectorImpl<toolchain::Constant*> &indices,
                       size_t position) const;

  toolchain::Constant *getRelativeOffset(toolchain::IntegerType *offsetType,
                                    toolchain::Constant *target);

  toolchain::Constant *getRelativeOffsetToPosition(toolchain::IntegerType *offsetType,
                                              toolchain::Constant *target,
                                              size_t position);
...
public:
  ArrayBuilder beginArray(toolchain::Type *eltTy = nullptr) {
    return ArrayBuilder(static_cast<InitBuilder&>(this->Builder), this, eltTy);
  }

  StructBuilder beginStruct(toolchain::StructType *ty = nullptr) {
  ...
  template <class... As>
  toolchain::GlobalVariable *finishAndCreateGlobal(As &&...args) {
  ...
  void finishAndSetAsInitializer(toolchain::GlobalVariable *global) {
    assert(!this->Parent && "finishing non-root builder");
...
public:
  ...
  void suggestType(toolchain::StructType *structTy) {
...
public:
  ...
  StructBuilder beginStruct(toolchain::StructType *structTy = nullptr) {
