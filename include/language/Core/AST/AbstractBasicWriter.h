...
#ifndef LANGUAGE_CORE_AST_ABSTRACTBASICWRITER_H
#define LANGUAGE_CORE_AST_ABSTRACTBASICWRITER_H
...
//   BasicWriter find(toolchain::StringRef propertyName);
...
#include "language/Core/AST/AbstractBasicWriter.inc"
...
public:
  ...
  void writeAPSInt(const toolchain::APSInt &value) {
    asImpl().writeBool(value.isUnsigned());
    asImpl().writeAPInt(value);
  }

  void writeAPInt(const toolchain::APInt &value) {
  ...
  void writeFixedPointSemantics(const toolchain::FixedPointSemantics &sema) {
