...
#ifndef LANGUAGE_CORE_AST_ASTIMPORTER_H
#define LANGUAGE_CORE_AST_ASTIMPORTER_H
...
class TypeSourceInfo;
  ...
  toolchain::SmallVector<Decl*, 2> getCanonicalForwardRedeclChain(Decl* D);
  ...
  public:
    using NonEquivalentDeclSet =
        toolchain::DenseSet<std::tuple<Decl *, Decl *, int>>;
    using ImportedCXXBaseSpecifierMap =
        toolchain::DenseMap<const CXXBaseSpecifier *, CXXBaseSpecifier *>;
    ...
    class ImportPathTy {
    public:
      using VecTy = toolchain::SmallVector<Decl *, 32>;
      ...
      using Cycle = toolchain::iterator_range<VecTy::const_reverse_iterator>;
    ...
    private:
      ...
      toolchain::SmallDenseMap<Decl *, int, 32> Aux;
    };
  ...
  private:
    ...
    using SavedImportPathsForOneDecl =
        toolchain::SmallVector<ImportPathTy::VecTy, 32>;
    using SavedImportPathsTy =
        toolchain::SmallDenseMap<Decl *, SavedImportPathsForOneDecl, 32>;
    ...
    toolchain::DenseMap<const Type *, const Type *> ImportedTypes;

    /// Mapping from the already-imported declarations in the "from"
    /// context to the corresponding declarations in the "to" context.
    toolchain::DenseMap<Decl *, Decl *> ImportedDecls;
  ...
  public:
    ...
    [[nodiscard]] toolchain::Error importInto(ImportT &To, const ImportT &From) {
    ...
    toolchain::Expected<const Type *> Import(const Type *FromT);

    /// Import the given qualified type from the "from" context into the "to"
    /// context. A null type is imported as a null type (no error).
    ///
    /// \returns The equivalent type in the "to" context, or the import error.
    toolchain::Expected<QualType> Import(QualType FromT);
    ...
    toolchain::Expected<TypeSourceInfo *> Import(TypeSourceInfo *FromTSI);
    ...
    /// error.
    toolchain::Expected<Attr *> Import(const Attr *FromAttr);
    ...
    /// error.
    toolchain::Expected<Decl *> Import(Decl *FromD);
    toolchain::Expected<const Decl *> Import(const Decl *FromD) {
    ...
    toolchain::Expected<DeclContext *> ImportContext(DeclContext *FromDC);
    ...
    /// error.
    toolchain::Expected<Expr *> Import(Expr *FromE);
    ...
    /// error.
    toolchain::Expected<Stmt *> Import(Stmt *FromS);
    ...
    /// context, or the import error.
    toolchain::Expected<NestedNameSpecifier> Import(NestedNameSpecifier FromNNS);
    ...
    /// import error.
    toolchain::Expected<SourceLocation> Import(SourceLocation FromLoc);
    ...
    /// error.
    toolchain::Expected<SourceRange> Import(SourceRange FromRange);
    ...
    /// import error.
    toolchain::Expected<DeclarationName> Import(DeclarationName FromName);
    ...
    /// error.
    toolchain::Expected<Selector> Import(Selector FromSel);
    ...
    /// context, or the import error.
    toolchain::Expected<FileID> Import(FileID, bool IsBuiltin = false);
    ...
    /// error.
    toolchain::Expected<CXXCtorInitializer *> Import(CXXCtorInitializer *FromInit);
    ...
    /// "to" context, or the import error.
    toolchain::Expected<CXXBaseSpecifier *> Import(const CXXBaseSpecifier *FromSpec);
    ...
    /// error.
    toolchain::Expected<APValue> Import(const APValue &FromValue);

    /// Import the definition of the given declaration, including all of
    /// the declarations it contains.
    [[nodiscard]] toolchain::Error ImportDefinition(Decl *From);
...
#endif // LANGUAGE_CORE_AST_ASTIMPORTER_H
