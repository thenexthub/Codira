...
//
/// \file
/// Defines the language::Core::Expr interface and subclasses for C++ expressions.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_AST_EXPRCXX_H
#define LANGUAGE_CORE_AST_EXPRCXX_H
...
class CXXStaticCastExpr final
    : public CXXNamedCastExpr,
      private toolchain::TrailingObjects<CXXStaticCastExpr, CXXBaseSpecifier *,
                                    FPOptionsOverride> {
...
class CXXDynamicCastExpr final
    : public CXXNamedCastExpr,
      private toolchain::TrailingObjects<CXXDynamicCastExpr, CXXBaseSpecifier *> {
...
class CXXReinterpretCastExpr final
    : public CXXNamedCastExpr,
      private toolchain::TrailingObjects<CXXReinterpretCastExpr,
                                    CXXBaseSpecifier *> {
...
class CXXConstCastExpr final
    : public CXXNamedCastExpr,
      private toolchain::TrailingObjects<CXXConstCastExpr, CXXBaseSpecifier *> {
...
class CXXAddrspaceCastExpr final
    : public CXXNamedCastExpr,
      private toolchain::TrailingObjects<CXXAddrspaceCastExpr, CXXBaseSpecifier *> {
...
private:
  toolchain::PointerUnion<Stmt *, TypeSourceInfo *> Operand;
  SourceRange Range;
...
class CXXDefaultArgExpr final
    : public Expr,
      private toolchain::TrailingObjects<CXXDefaultArgExpr, Expr *> {
  friend class ASTStmtReader;
  friend class ASTReader;
  friend TrailingObjects;
...
class CXXDefaultInitExpr final
    : public Expr,
      private toolchain::TrailingObjects<CXXDefaultInitExpr, Expr *> {
...
class CXXConstructExpr : public Expr {
  ...
  // CXXTemporaryObjectExpr which makes the use of toolchain::TrailingObjects
  // impossible.
  //
...
public:
  ...
  void setConstructionKind(CXXConstructionKind CK) {
    CXXConstructExprBits.ConstructionKind = toolchain::to_underlying(CK);
  }

  using arg_iterator = ExprIterator;
  using const_arg_iterator = ConstExprIterator;
  using arg_range = toolchain::iterator_range<arg_iterator>;
  using const_arg_range = toolchain::iterator_range<const_arg_iterator>;
...
class CXXFunctionalCastExpr final
    : public ExplicitCastExpr,
      private toolchain::TrailingObjects<CXXFunctionalCastExpr, CXXBaseSpecifier *,
                                    FPOptionsOverride> {
...
class LambdaExpr final : public Expr,
                         private toolchain::TrailingObjects<LambdaExpr, Stmt *> {
...
public:
  ...
  using capture_range = toolchain::iterator_range<capture_iterator>;
  ...
  toolchain::iterator_range<capture_init_iterator> capture_inits() {
    return toolchain::make_range(capture_init_begin(), capture_init_end());
  }

  /// Retrieve the initialization expressions for this lambda's captures.
  toolchain::iterator_range<const_capture_init_iterator> capture_inits() const {
    return toolchain::make_range(capture_init_begin(), capture_init_end());
  }
...
class CXXNewExpr final
    : public Expr,
      private toolchain::TrailingObjects<CXXNewExpr, Stmt *, SourceRange> {
  friend class ASTStmtReader;
  friend class ASTStmtWriter;
  friend TrailingObjects;
...
public:
  ...
  toolchain::iterator_range<arg_iterator> placement_arguments() {
    return toolchain::make_range(placement_arg_begin(), placement_arg_end());
  }

  toolchain::iterator_range<const_arg_iterator> placement_arguments() const {
    return toolchain::make_range(placement_arg_begin(), placement_arg_end());
  }
...
class PseudoDestructorTypeStorage {
  ...
  toolchain::PointerUnion<TypeSourceInfo *, const IdentifierInfo *> Type;
...
class TypeTraitExpr final
    : public Expr,
      private toolchain::TrailingObjects<TypeTraitExpr, APValue, TypeSourceInfo *> {
...
public:
  ...
  toolchain::iterator_range<decls_iterator> decls() const {
    return toolchain::make_range(decls_begin(), decls_end());
  }
...
class UnresolvedLookupExpr final
    : public OverloadExpr,
      private toolchain::TrailingObjects<UnresolvedLookupExpr, DeclAccessPair,
...
class DependentScopeDeclRefExpr final
    : public Expr,
      private toolchain::TrailingObjects<DependentScopeDeclRefExpr,
                                    ASTTemplateKWAndArgsInfo,
                                    TemplateArgumentLoc> {
  friend class ASTStmtReader;
  friend class ASTStmtWriter;
...
class ExprWithCleanups final
    : public FullExpr,
      private toolchain::TrailingObjects<
          ExprWithCleanups,
          toolchain::PointerUnion<BlockDecl *, CompoundLiteralExpr *>> {
public:
  ...
  /// currently no need.
  using CleanupObject = toolchain::PointerUnion<BlockDecl *, CompoundLiteralExpr *>;
...
class CXXUnresolvedConstructExpr final
    : public Expr,
      private toolchain::TrailingObjects<CXXUnresolvedConstructExpr, Expr *> {
  friend class ASTStmtReader;
  friend TrailingObjects;

  /// The type being constructed, and whether the construct expression models
  /// list initialization or not.
  toolchain::PointerIntPair<TypeSourceInfo *, 1> TypeAndInitForm;
...
class CXXDependentScopeMemberExpr final
    : public Expr,
      private toolchain::TrailingObjects<CXXDependentScopeMemberExpr,
                                    ASTTemplateKWAndArgsInfo,
...
class UnresolvedMemberExpr final
    : public OverloadExpr,
      private toolchain::TrailingObjects<UnresolvedMemberExpr, DeclAccessPair,
                                    ASTTemplateKWAndArgsInfo,
                                    TemplateArgumentLoc> {
  friend class ASTStmtReader;
  friend class OverloadExpr;
...
class SizeOfPackExpr final
    : public Expr,
      private toolchain::TrailingObjects<SizeOfPackExpr, TemplateArgument> {
  friend class ASTStmtReader;
  friend class ASTStmtWriter;
  friend TrailingObjects;
  ...
  SizeOfPackExpr(QualType SizeType, SourceLocation OperatorLoc, NamedDecl *Pack,
    ...
    toolchain::uninitialized_copy(PartialArgs, Args);
...
class PackIndexingExpr final
    : public Expr,
      private toolchain::TrailingObjects<PackIndexingExpr, Expr *> {
  friend class ASTStmtReader;
  friend class ASTStmtWriter;
  friend TrailingObjects;
  ...
  PackIndexingExpr(QualType Type, SourceLocation EllipsisLoc,
    ...
    toolchain::uninitialized_copy(SubstitutedExprs, getTrailingObjects());
...
class SubstNonTypeTemplateParmExpr : public Expr {
  ...
  /// category alone.
  toolchain::PointerIntPair<Decl *, 1, bool> AssociatedDeclAndRef;
...
class FunctionParmPackExpr final
    : public Expr,
      private toolchain::TrailingObjects<FunctionParmPackExpr, ValueDecl *> {
  friend class ASTReader;
  friend class ASTStmtReader;
  friend TrailingObjects;
...
private:
  ...
  toolchain::PointerUnion<Stmt *, LifetimeExtendedTemporaryDecl *> State;
...
class CXXParenListInitExpr final
    : public Expr,
      private toolchain::TrailingObjects<CXXParenListInitExpr, Expr *> {
  ...
  CXXParenListInitExpr(ArrayRef<Expr *> Args, QualType T,
    ...
    toolchain::copy(Args, getTrailingObjects());
...
class BuiltinBitCastExpr final
    : public ExplicitCastExpr,
      private toolchain::TrailingObjects<BuiltinBitCastExpr, CXXBaseSpecifier *> {
  friend class ASTStmtReader;
  friend class CastExpr;
  friend TrailingObjects;
...
#endif // LANGUAGE_CORE_AST_EXPRCXX_H
