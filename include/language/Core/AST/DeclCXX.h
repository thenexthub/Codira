...
#ifndef LANGUAGE_CORE_AST_DECLCXX_H
#define LANGUAGE_CORE_AST_DECLCXX_H
...
private:
  ...
  struct LambdaDefinitionData : public DefinitionData {
    ...
    toolchain::TinyPtrVector<Capture*> Captures;
  ...
  toolchain::PointerUnion<ClassTemplateDecl *, MemberSpecializationInfo *>
...
public:
  ...
  using base_class_range = toolchain::iterator_range<base_class_iterator>;
  using base_class_const_range =
      toolchain::iterator_range<base_class_const_iterator>;
  ...
  using method_range =
      toolchain::iterator_range<specific_decl_iterator<CXXMethodDecl>>;
  ...
  using ctor_range =
      toolchain::iterator_range<specific_decl_iterator<CXXConstructorDecl>>;
  ...
  using friend_range = toolchain::iterator_range<friend_iterator>;
  ...
  void
  getCaptureFields(toolchain::DenseMap<const ValueDecl *, FieldDecl *> &Captures,
                   FieldDecl *&ThisCapture) const;

  using capture_const_iterator = const LambdaCapture *;
  using capture_const_range = toolchain::iterator_range<capture_const_iterator>;
  ...
  toolchain::iterator_range<conversion_iterator>
  ...
  using ForallBasesCallback =
      toolchain::function_ref<bool(const CXXRecordDecl *BaseDefinition)>;
  ...
  using BaseMatchesCallback =
      toolchain::function_ref<bool(const CXXBaseSpecifier *Specifier,
                              CXXBasePath &Path)>;
...
class ExplicitSpecifier {
  toolchain::PointerIntPair<Expr *, 2, ExplicitSpecKind> ExplicitSpec{
...
private:
  ...
  toolchain::PointerIntPair<const CXXDeductionGuideDecl *, 2,
...
public:
  ...
  using overridden_method_range = toolchain::iterator_range<
      toolchain::TinyPtrVector<const CXXMethodDecl *>::const_iterator>;
...
class CXXCtorInitializer final {
  ...
  toolchain::PointerUnion<TypeSourceInfo *, FieldDecl *, IndirectFieldDecl *>
      Initializee;
...
class CXXConstructorDecl final
    : public CXXMethodDecl,
      private toolchain::TrailingObjects<CXXConstructorDecl, InheritedConstructor,
                                    ExplicitSpecifier> {
...
public:
  ...
  using init_range = toolchain::iterator_range<init_iterator>;
  using init_const_range = toolchain::iterator_range<init_const_iterator>;
...
public:
  ...
  void setLanguage(LinkageSpecLanguageIDs L) {
    LinkageSpecDeclBits.Language = toolchain::to_underlying(L);
  }
...
class BaseUsingDecl : public NamedDecl {
  ...
  toolchain::PointerIntPair<UsingShadowDecl *, 1, bool> FirstUsingShadow;
...
public:
  ...
  using shadow_range = toolchain::iterator_range<shadow_iterator>;
...
class UsingPackDecl final
    : public NamedDecl, public Mergeable<UsingPackDecl>,
      private toolchain::TrailingObjects<UsingPackDecl, NamedDecl *> {
  ...
  UsingPackDecl(DeclContext *DC, NamedDecl *InstantiatedFrom,
    ...
    toolchain::uninitialized_copy(UsingDecls, getTrailingObjects());
  }
...
class StaticAssertDecl : public Decl {
  toolchain::PointerIntPair<Expr *, 1, bool> AssertExprAndFailed;
  Expr *Message;
  SourceLocation RParenLoc;
...
class DecompositionDecl final
    : public VarDecl,
      private toolchain::TrailingObjects<DecompositionDecl, BindingDecl *> {
  ...
  DecompositionDecl(ASTContext &C, DeclContext *DC, SourceLocation StartLoc,
    ...
    toolchain::uninitialized_copy(Bindings, getTrailingObjects());
    for (auto *B : Bindings) {
...
public:
  ...
  auto flat_bindings() const {
    ...
    return toolchain::concat<BindingDecl *const>(std::move(BeforePackBindings),
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID &ID, Parts P) {
    ID.AddInteger(P.Part1);
    ID.AddInteger(P.Part2);
    ID.AddInteger(P.Part3);
    ID.AddInteger(P.getPart4And5AsUint64());
  }
  void Profile(toolchain::FoldingSetNodeID &ID) { Profile(ID, PartVal); }
...
public:
  ...
  static void Profile(toolchain::FoldingSetNodeID &ID, QualType Ty,
                      const APValue &APVal) {
    Ty.Profile(ID);
    APVal.Profile(ID);
  }
...
#endif // LANGUAGE_CORE_AST_DECLCXX_H
