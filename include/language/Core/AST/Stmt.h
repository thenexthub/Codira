...
#ifndef LANGUAGE_CORE_AST_STMT_H
#define LANGUAGE_CORE_AST_STMT_H
...
protected:
  ...
  void *operator new(size_t bytes) noexcept {
    toolchain_unreachable("Stmts cannot be allocated with regular 'new'.");
  }

  void operator delete(void *data) noexcept {
    toolchain_unreachable("Stmts cannot be released with regular 'delete'.");
  }
  ...
  class ExprBitfields {
    ...
    unsigned Dependent : toolchain::BitWidth<ExprDependence>;
  };
  enum { NumExprBits = NumStmtBits + 5 + toolchain::BitWidth<ExprDependence> };
  ...
  class FloatingLiteralBitfields {
    ...
    static_assert(
        toolchain::APFloat::S_MaxSemantics < 32,
        "Too many Semantics enum values to fit in bitfield of size 5");
    LLVM_PREFERRED_TYPE(toolchain::APFloat::Semantics)
...
protected:
  ...
  struct CastIterator
      : toolchain::iterator_adaptor_base<CastIterator<T, TPtr, StmtPtr>, StmtPtr *,
...
public:
  ...
  void ProcessODRHash(toolchain::FoldingSetNodeID &ID, ODRHash& Hash) const;
};
...
public:
  ...
  using decl_const_range = toolchain::iterator_range<const_decl_iterator>;
...
class CompoundStmt final
    : public Stmt,
      private toolchain::TrailingObjects<CompoundStmt, Stmt *, FPOptionsOverride> {
  friend class ASTStmtReader;
...
public:
  ...
  using body_range = toolchain::iterator_range<body_iterator>;
  ...
  using body_const_range = toolchain::iterator_range<const_body_iterator>;
  ...
  Stmt *getStmtExprResult() {
    for (auto *B : toolchain::reverse(body())) {
      if (!isa<NullStmt>(B))
        return B;
    }
    return body_back();
...
class CaseStmt final
    : public SwitchCase,
      private toolchain::TrailingObjects<CaseStmt, Stmt *, SourceLocation> {
  friend TrailingObjects;
...
SourceLocation SwitchCase::getEndLoc() const {
  if (const auto *CS = dyn_cast<CaseStmt>(this))
    ...
    return DS->getEndLoc();
  toolchain_unreachable("SwitchCase is neither a CaseStmt nor a DefaultStmt!");
}
...
class AttributedStmt final
    : public ValueStmt,
      private toolchain::TrailingObjects<AttributedStmt, const Attr *> {
  friend class ASTStmtReader;
  friend TrailingObjects;
  ...
  AttributedStmt(SourceLocation Loc, ArrayRef<const Attr *> Attrs,
    ...
    toolchain::copy(Attrs, getAttrArrayPtr());
  }
...
class IfStmt final
    : public Stmt,
      private toolchain::TrailingObjects<IfStmt, Stmt *, SourceLocation> {
  friend TrailingObjects;
...
class SwitchStmt final : public Stmt,
                         private toolchain::TrailingObjects<SwitchStmt, Stmt *> {
  friend TrailingObjects;
...
class WhileStmt final : public Stmt,
                        private toolchain::TrailingObjects<WhileStmt, Stmt *> {
  friend TrailingObjects;
...
class ReturnStmt final
    : public Stmt,
      private toolchain::TrailingObjects<ReturnStmt, const VarDecl *> {
  friend TrailingObjects;
...
public:
  ...
  using inputs_range = toolchain::iterator_range<inputs_iterator>;
  using inputs_const_range = toolchain::iterator_range<const_inputs_iterator>;
  ...
  using outputs_range = toolchain::iterator_range<outputs_iterator>;
  using outputs_const_range = toolchain::iterator_range<const_outputs_iterator>;
...
public:
  ...
  using labels_range = toolchain::iterator_range<labels_iterator>;
  using labels_const_range = toolchain::iterator_range<const_labels_iterator>;
...
public:
  ...
  class Capture {
    toolchain::PointerIntPair<VarDecl *, 2, VariableCaptureKind> VarAndKind;
    SourceLocation Loc;
...
private:
  ...
  toolchain::PointerIntPair<CapturedDecl *, 2, CapturedRegionKind> CapDeclAndKind;
...
public:
  ...
  using capture_const_range = toolchain::iterator_range<const_capture_iterator>;
  ...
  using capture_init_range = toolchain::iterator_range<capture_init_iterator>;

  /// Const iterator that walks over the capture initialization
  /// arguments.
  using const_capture_init_iterator = Expr *const *;
  using const_capture_init_range =
      toolchain::iterator_range<const_capture_init_iterator>;
...
#endif // LANGUAGE_CORE_AST_STMT_H
