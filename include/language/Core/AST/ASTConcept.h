...
#ifndef LANGUAGE_CORE_AST_ASTCONCEPT_H
#define LANGUAGE_CORE_AST_ASTCONCEPT_H
...
class ConstraintSatisfaction : public toolchain::FoldingSetNode {
  // The template-like entity that 'owns' the constraint checked here (can be a
  // constrained entity or a concept).
  const NamedDecl *ConstraintOwner = nullptr;
  toolchain::SmallVector<TemplateArgument, 4> TemplateArgs;
...
public:
  ...
  using Detail = toolchain::PointerUnion<Expr *, SubstitutionDiagnostic *>;
  ...
  /// invalid expression.
  toolchain::SmallVector<Detail, 4> Details;

  void Profile(toolchain::FoldingSetNodeID &ID, const ASTContext &C) {
    Profile(ID, C, ConstraintOwner, TemplateArgs);
  }

  static void Profile(toolchain::FoldingSetNodeID &ID, const ASTContext &C,
...
using UnsatisfiedConstraintRecord =
    toolchain::PointerUnion<Expr *, std::pair<SourceLocation, StringRef> *>;
...
struct ASTConstraintSatisfaction final :
    toolchain::TrailingObjects<ASTConstraintSatisfaction,
...
#endif // LANGUAGE_CORE_AST_ASTCONCEPT_H
