...
#ifndef LANGUAGE_CORE_AST_DECLCONTEXTINTERNALS_H
#define LANGUAGE_CORE_AST_DECLCONTEXTINTERNALS_H
...
class StoredDeclsList {
  ...
  using DeclsAndHasExternalTy = toolchain::PointerIntPair<Decls, 1, bool>;
  ...
  template <typename Fn> DeclListNode::Decls *erase_if(Fn ShouldErase) {
    ...
    assert(toolchain::none_of(getLookupResult(), ShouldErase) && "Still exists!");
...
public:
  ...
  void replaceExternalDecls(ArrayRef<NamedDecl*> Decls) {
    ...
    DeclListNode::Decls *Tail = erase_if([Decls](NamedDecl *ND) {
      ...
      return toolchain::any_of(Decls, [ND](NamedDecl *D) {
  ...
  void addOrReplaceDecl(NamedDecl *D) {
    ...
    assert(!toolchain::is_contained(getLookupResult(), D) && "Already exists!");
  ...
  LLVM_DUMP_METHOD void dump() const {
    ...
    while (true) {
      if (auto *Node = D.dyn_cast<DeclListNode*>()) {
        toolchain::errs() << '[' << Node->D << "] -> ";
        D = Node->Rest;
      } else {
        toolchain::errs() << '[' << cast<NamedDecl *>(D) << "]\n";
        return;
      }
    }
  }
...
class StoredDeclsMap
    : public toolchain::SmallDenseMap<DeclarationName, StoredDeclsList, 4> {
  friend class ASTContext; // walks the chain deleting these
  friend class DeclContext;

  toolchain::PointerIntPair<StoredDeclsMap*, 1> Previous;
public:
...
#endif // LANGUAGE_CORE_AST_DECLCONTEXTINTERNALS_H
