...
//
//===----------------------------------------------------------------------===//
#ifndef LANGUAGE_CORE_AST_RECURSIVEASTVISITOR_H
#define LANGUAGE_CORE_AST_RECURSIVEASTVISITOR_H
...
public:
  ...
  typedef SmallVectorImpl<toolchain::PointerIntPair<Stmt *, 1, bool>>
    DataRecursionQueue;
...
#include "language/Core/AST/AttrVisitor.inc"
...
#define TRAVERSE_STMT_BASE(NAME, CLASS, VAR, QUEUE)                            \
  (::language::Core::detail::has_same_member_pointer_type<                              \
...
bool RecursiveASTVisitor<Derived>::TraverseConceptRequirement(
  ...
  toolchain_unreachable("unexpected case");
}
...
#define STMT(CLASS, PARENT)                                                    \
  case Stmt::CLASS##Class:                                                     \
    if (::language::Core::detail::isSameMethod(&RecursiveASTVisitor::Traverse##CLASS,   \
...
RecursiveASTVisitor<Derived>::TraverseStmt(Stmt *S, DataRecursionQueue *Queue) {
  ...
  SmallVector<toolchain::PointerIntPair<Stmt *, 1, bool>, 8> LocalQueue;
...
bool RecursiveASTVisitor<Derived>::TraverseDecl(Decl *D) {
  ...
  if (!getDerived().shouldVisitImplicitCode()) {
    ...
    if (auto *FTD = dyn_cast<FunctionTemplateDecl>(D))
      if (toolchain::isa_and_present<TypeAliasTemplateDecl>(
...
DEF_TRAVERSE_DECL(ObjCCategoryDecl, {
  ...
  for (auto It : toolchain::zip(D->protocols(), D->protocol_locs())) {
...
//   http://clang.toolchain.org/doxygen/Stmt_8cpp_source.html
...
// Candidates:
//
//    http://clang.toolchain.org/doxygen/classclang_1_1CXXTypeidExpr.html
//    http://clang.toolchain.org/doxygen/classclang_1_1UnaryExprOrTypeTraitExpr.html
//    http://clang.toolchain.org/doxygen/classclang_1_1TypesCompatibleExpr.html
...
#endif // LANGUAGE_CORE_AST_RECURSIVEASTVISITOR_H
