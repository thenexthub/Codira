...
#ifndef LANGUAGE_CORE_AST_APVALUE_H
#define LANGUAGE_CORE_AST_APVALUE_H
...
#include "toolchain/Support/AlignOf.h"
...
public:
  ...
  void print(toolchain::raw_ostream &Out, const PrintingPolicy &Policy) const;
};
...
namespace toolchain {
template<> struct PointerLikeTypeTraits<language::Core::TypeInfoLValue> {
  static void *getAsVoidPointer(language::Core::TypeInfoLValue V) {
    return V.getOpaqueValue();
  }
  static language::Core::TypeInfoLValue getFromVoidPointer(void *P) {
    return language::Core::TypeInfoLValue::getFromOpaqueValue(P);
  }
...
template<> struct PointerLikeTypeTraits<language::Core::DynamicAllocLValue> {
  static void *getAsVoidPointer(language::Core::DynamicAllocLValue V) {
    return V.getOpaqueValue();
  }
  static language::Core::DynamicAllocLValue getFromVoidPointer(void *P) {
    return language::Core::DynamicAllocLValue::getFromOpaqueValue(P);
  }
  static constexpr int NumLowBitsAvailable =
      language::Core::DynamicAllocLValue::NumLowBitsAvailable;
};
}
...
class APValue {
  typedef toolchain::APFixedPoint APFixedPoint;
  typedef toolchain::APSInt APSInt;
  typedef toolchain::APFloat APFloat;
public:
  enum ValueKind {
  ...
  class alignas(uint64_t) LValueBase {
    typedef toolchain::PointerUnion<const ValueDecl *, const Expr *, TypeInfoLValue,
                               DynamicAllocLValue>
        PtrTy;
  ...
  public:
    ...
    friend toolchain::hash_code hash_value(const LValueBase &Base);
    friend struct toolchain::DenseMapInfo<LValueBase>;
  ...
  typedef toolchain::PointerIntPair<const Decl *, 1, bool> BaseOrMemberType;
  ...
  public:
    ...
    friend toolchain::hash_code hash_value(LValuePathEntry A) {
      return toolchain::hash_value(A.Value);
    }
  };
  ...
  template <typename Impl> friend class language::Core::serialization::BasicReaderBase;
...
private:
  ...
  typedef toolchain::AlignedCharArrayUnion<void *, APSInt, APFloat, ComplexAPSInt,
...
namespace toolchain {
template<> struct DenseMapInfo<language::Core::APValue::LValueBase> {
  static language::Core::APValue::LValueBase getEmptyKey();
  static language::Core::APValue::LValueBase getTombstoneKey();
  static unsigned getHashValue(const language::Core::APValue::LValueBase &Base);
  static bool isEqual(const language::Core::APValue::LValueBase &LHS,
                      const language::Core::APValue::LValueBase &RHS);
};
}
