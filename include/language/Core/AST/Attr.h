...
#ifndef LANGUAGE_CORE_AST_ATTR_H
#define LANGUAGE_CORE_AST_ATTR_H
...
protected:
  ...
  void *operator new(size_t bytes) noexcept {
    toolchain_unreachable("Attrs cannot be allocated with regular 'new'.");
  }
  void operator delete(void *data) noexcept {
    toolchain_unreachable("Attrs cannot be released with regular 'delete'.");
  }
...
#include "language/Core/AST/Attrs.inc" // IWYU pragma: export
...
inline ParameterABI ParameterABIAttr::getABI() const {
  ...
  default:
    toolchain_unreachable("bad parameter ABI attribute kind");
  }
}
