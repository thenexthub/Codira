...
#ifndef LANGUAGE_CORE_AST_NESTEDNAMESPECIFIERBASE_H
#define LANGUAGE_CORE_AST_NESTEDNAMESPECIFIERBASE_H
...
public:
  ...
  void dump(toolchain::raw_ostream *OS = nullptr,
            const LangOptions *LO = nullptr) const;
  void dump(const LangOptions &LO) const;
  void dump(toolchain::raw_ostream &OS) const;
  void dump(toolchain::raw_ostream &OS, const LangOptions &LO) const;
...
struct alignas(8) NamespaceAndPrefixStorage : NamespaceAndPrefix,
                                              toolchain::FoldingSetNode {
  NamespaceAndPrefixStorage(const NamespaceBaseDecl *Namespace,
                            NestedNameSpecifier Prefix)
      : NamespaceAndPrefix{Namespace, Prefix} {}
  void Profile(toolchain::FoldingSetNodeID &ID) { Profile(ID, Namespace, Prefix); }
...
NamespaceAndPrefix NestedNameSpecifier::getAsNamespaceAndPrefix() const {
  ...
  toolchain_unreachable("unexpected stored kind");
}
...
template <> struct PointerLikeTypeTraits<language::Core::NestedNameSpecifier> {
  static void *getAsVoidPointer(language::Core::NestedNameSpecifier P) {
    return P.getAsVoidPointer();
  }
  static language::Core::NestedNameSpecifier getFromVoidPointer(const void *P) {
    return language::Core::NestedNameSpecifier::getFromVoidPointer(P);
  }
  static constexpr int NumLowBitsAvailable =
      language::Core::NestedNameSpecifier::NumLowBitsAvailable;
};

} // namespace toolchain

#endif // LANGUAGE_CORE_AST_NESTEDNAMESPECIFIERBASE_H
