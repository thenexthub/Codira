...
#ifndef LANGUAGE_CORE_AST_EXPR_H
#define LANGUAGE_CORE_AST_EXPR_H

#include "language/Core/AST/APNumericStorage.h"
...
public:
  ...
  /// want to.
  bool EvaluateAsFloat(toolchain::APFloat &Result, const ASTContext &Ctx,
                       SideEffectsKind AllowSideEffects = SE_NoSideEffects,
                       bool InConstantContext = false) const;
  ...
  /// integer.
  toolchain::APSInt EvaluateKnownConstInt(
      const ASTContext &Ctx,
      SmallVectorImpl<PartialDiagnosticAt> *Diag = nullptr) const;

  toolchain::APSInt EvaluateKnownConstIntCheckOverflow(
      const ASTContext &Ctx,
      SmallVectorImpl<PartialDiagnosticAt> *Diag = nullptr) const;
...
static_assert(toolchain::PointerLikeTypeTraits<Expr *>::NumLowBitsAvailable <=
                  toolchain::detail::ConstantLog2<alignof(Expr)>::value,
...
class ConstantExpr final
    : public FullExpr,
      private toolchain::TrailingObjects<ConstantExpr, APValue, uint64_t> {
  static_assert(std::is_same<uint64_t, toolchain::APInt::WordType>::value,
                "ConstantExpr assumes that toolchain::APInt::WordType is uint64_t "
...
public:
  ...
  toolchain::APSInt getResultAsAPSInt() const;
  // Iterators
...
class DeclRefExpr final
    : public Expr,
      private toolchain::TrailingObjects<DeclRefExpr, NestedNameSpecifierLoc,
...
public:
  ...
  IntegerLiteral(const ASTContext &C, const toolchain::APInt &V, QualType type,
                 SourceLocation l);
  ...
  static IntegerLiteral *Create(const ASTContext &C, const toolchain::APInt &V,
...
class FixedPointLiteral : public Expr, public APIntStorage {
 ...
 public:
  FixedPointLiteral(const ASTContext &C, const toolchain::APInt &V, QualType type,
                    SourceLocation l, unsigned Scale);

  // Store the int as is without any bit shifting.
  static FixedPointLiteral *CreateFromRawInt(const ASTContext &C,
                                             const toolchain::APInt &V,
                                             QualType type, SourceLocation l,
                                             unsigned Scale);
  ...
  toolchain::APInt getValue() const { return APIntStorage::getValue(); }
...
public:
  ...
  CharacterLiteral(unsigned value, CharacterLiteralKind kind, QualType type,
      ...
      : Expr(CharacterLiteralClass, type, VK_PRValue, OK_Ordinary),
        Value(value), Loc(l) {
    CharacterLiteralBits.Kind = toolchain::to_underlying(kind);
...
class FloatingLiteral : public Expr, private APFloatStorage {
  ...
  FloatingLiteral(const ASTContext &C, const toolchain::APFloat &V, bool isexact,
                  QualType Type, SourceLocation L);
...
public:
  static FloatingLiteral *Create(const ASTContext &C, const toolchain::APFloat &V,
  ...
  toolchain::APFloatBase::Semantics getRawSemantics() const {
    return static_cast<toolchain::APFloatBase::Semantics>(
        FloatingLiteralBits.Semantics);
  }

  /// Set the raw enumeration value representing the floating-point semantics of
  /// this literal (32-bit IEEE, x87, ...), suitable for serialization.
  void setRawSemantics(toolchain::APFloatBase::Semantics Sem) {
    FloatingLiteralBits.Semantics = Sem;
  }

  /// Return the APFloat semantics this literal uses.
  const toolchain::fltSemantics &getSemantics() const {
    return toolchain::APFloatBase::EnumToSemantics(
        static_cast<toolchain::APFloatBase::Semantics>(
            FloatingLiteralBits.Semantics));
  }

  /// Set the APFloat semantics this literal uses.
  void setSemantics(const toolchain::fltSemantics &Sem) {
    FloatingLiteralBits.Semantics = toolchain::APFloatBase::SemanticsToEnum(Sem);
  }
...
class StringLiteral final
    : public Expr,
      private toolchain::TrailingObjects<StringLiteral, unsigned, SourceLocation,
                                    char> {
  friend class ASTStmtReader;
  friend TrailingObjects;
...
public:
  ...
  uint32_t getCodeUnit(size_t i) const {
    ...
    toolchain_unreachable("Unsupported character width!");
  }
  ...
  int64_t getCodeUnitS(size_t I, uint64_t BitWidth) const {
    ...
    if (isOrdinary() || isWide()) {
      ...
      toolchain::APInt AInt(Width, (uint64_t)V);
      V = AInt.getSExtValue();
    }
    return V;
  }
...
class PredefinedExpr final
    : public Expr,
      private toolchain::TrailingObjects<PredefinedExpr, Stmt *> {
  friend class ASTStmtReader;
  friend TrailingObjects;
...
class UnaryOperator final
    : public Expr,
      private toolchain::TrailingObjects<UnaryOperator, FPOptionsOverride> {
  Stmt *Val;
...
class OffsetOfExpr final
    : public Expr,
      private toolchain::TrailingObjects<OffsetOfExpr, OffsetOfNode, Expr *> {
...
class CallExpr : public Expr {
  ...
  // toolchain::TrailingObjects. Instead we manually perform the pointer arithmetic
  // and casts.
  //
...
public:
  ...
  void computeDependence() {
    setDependence(language::Core::computeDependence(
        this,
  ...
  typedef toolchain::iterator_range<arg_iterator> arg_range;
  typedef toolchain::iterator_range<const_arg_iterator> const_arg_range;
...
class MemberExpr final
    : public Expr,
      private toolchain::TrailingObjects<MemberExpr, NestedNameSpecifierLoc,
...
class CompoundLiteralExpr : public Expr {
  ...
  toolchain::PointerIntPair<TypeSourceInfo *, 1, bool> TInfoAndScope;
  Stmt *Init;
...
public:
  ...
  /// in that order.
  toolchain::iterator_range<path_iterator> path() {
    return toolchain::make_range(path_begin(), path_end());
  }
  toolchain::iterator_range<path_const_iterator> path() const {
    return toolchain::make_range(path_begin(), path_end());
  }
...
class ImplicitCastExpr final
    : public CastExpr,
      private toolchain::TrailingObjects<ImplicitCastExpr, CXXBaseSpecifier *,
                                    FPOptionsOverride> {
...
class CStyleCastExpr final
    : public ExplicitCastExpr,
      private toolchain::TrailingObjects<CStyleCastExpr, CXXBaseSpecifier *,
                                    FPOptionsOverride> {
...
public:
  ...
  static Opcode negateComparisonOp(Opcode Opc) {
    switch (Opc) {
    default:
      toolchain_unreachable("Not a comparison operator.");
...
public:
  ...
  toolchain::APSInt getShuffleMaskIdx(unsigned N) const {
...
class ConvertVectorExpr final
    : public Expr,
      private toolchain::TrailingObjects<ConvertVectorExpr, FPOptionsOverride> {
private:
  Stmt *SrcExpr;
  TypeSourceInfo *TInfo;
  SourceLocation BuiltinLoc, RParenLoc;
...
class VAArgExpr : public Expr {
  Stmt *Val;
  toolchain::PointerIntPair<TypeSourceInfo *, 1, bool> TInfo;
  SourceLocation BuiltinLoc, RParenLoc;
public:
...
public:
  ...
  bool isIntType() const {
    ...
    toolchain_unreachable("unknown source location expression kind");
  }
...
public:
  ...
  class ChildElementIter
      : public toolchain::iterator_facade_base<
  ...
  public:
    ...
    typename BaseTy::reference operator*() const {
      ...
      N->setValue(*EExpr->Ctx,
                  toolchain::APInt(N->getValue().getBitWidth(),
...
public:
  using fake_child_range = toolchain::iterator_range<ChildElementIter<false>>;
  using const_fake_child_range = toolchain::iterator_range<ChildElementIter<true>>;
...
class InitListExpr : public Expr {
  ...
  toolchain::PointerIntPair<InitListExpr *, 1, bool> AltForm;
  ...
  toolchain::PointerUnion<Expr *, FieldDecl *> ArrayFillerOrUnionFieldInit;
...
class DesignatedInitExpr final
    : public Expr,
      private toolchain::TrailingObjects<DesignatedInitExpr, Stmt *> {
public:
...
public:
  ...
  ///
  /// @todo This class is infuriatingly similar to language::Core::Designator,
...
public:
  ...
  toolchain::APInt getArraySize() const {
...
class ParenListExpr final
    : public Expr,
      private toolchain::TrailingObjects<ParenListExpr, Stmt *> {
  friend class ASTStmtReader;
  friend TrailingObjects;
...
class GenericSelectionExpr final
    : public Expr,
      private toolchain::TrailingObjects<GenericSelectionExpr, Stmt *,
                                    TypeSourceInfo *> {
  friend class ASTStmtReader;
  friend class ASTStmtWriter;
  friend TrailingObjects;
...
public:
  ...
  using association_range = toolchain::iterator_range<AssociationIterator>;
  using const_association_range =
      toolchain::iterator_range<ConstAssociationIterator>;
...
public:
  ...
  /// getEncodedElementAccess - Encode the elements accessed into an toolchain
...
struct BlockVarCopyInit {
  ...
  toolchain::PointerIntPair<Expr *, 1, bool> ExprAndFlag;
};
...
class PseudoObjectExpr final
    : public Expr,
      private toolchain::TrailingObjects<PseudoObjectExpr, Expr *> {
...
#include "language/Core/Basic/Builtins.inc"
    ...
    toolchain_unreachable("not an atomic operator?");
  }
  unsigned getNumSubExprs() const { return NumSubExprs; }
...
class RecoveryExpr final : public Expr,
                           private toolchain::TrailingObjects<RecoveryExpr, Expr *> {
public:
...
#endif // LANGUAGE_CORE_AST_EXPR_H
