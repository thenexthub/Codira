...
#ifndef LANGUAGE_CORE_AST_DECLBASE_H
#define LANGUAGE_CORE_AST_DECLBASE_H
...
protected:
  ...
  toolchain::PointerIntPair<Decl *, 3, ModuleOwnershipKind> NextInContextAndBits;
...
private:
  ...
  toolchain::PointerUnion<DeclContext*, MultipleDC*> DeclCtx;
...
protected:
  Decl(Kind DK, DeclContext *DC, SourceLocation L)
      : NextInContextAndBits(nullptr, getModuleOwnershipKindForChildOf(DC)),
        ...
        CacheValidAndLinkage(toolchain::to_underlying(Linkage::Invalid)) {
  ...
  void setCachedLinkage(Linkage L) const {
    CacheValidAndLinkage = toolchain::to_underlying(L);
  }
...
public:
  ...
  using attr_range = toolchain::iterator_range<attr_iterator>;
  ...
  template <typename... Ts> void dropAttrs() {
    ...
    toolchain::erase_if(Vec, [](Attr *A) { return isa<Ts...>(A); });
  ...
  template <typename T>
  toolchain::iterator_range<specific_attr_iterator<T>> specific_attrs() const {
    return toolchain::make_range(specific_attr_begin<T>(), specific_attr_end<T>());
  }
...
public:
  ...
  using redecl_range = toolchain::iterator_range<redecl_iterator>;
...
namespace toolchain {
  template <> struct PointerLikeTypeTraits<::language::Core::NamedDecl *> {
    static inline void *getAsVoidPointer(::language::Core::NamedDecl *P) { return P; }
    static inline ::language::Core::NamedDecl *getFromVoidPointer(void *P) {
      return static_cast<::language::Core::NamedDecl *>(P);
    }
...
public:
  using Decls = toolchain::PointerUnion<NamedDecl*, DeclListNode*>;
  class iterator {
...
protected:
  ...
  class RecordDeclBitfields {
    ...
    /// [[language::Core::require_explicit_initialization]].
...
public:
  ...
  using lookups_range = toolchain::iterator_range<all_lookups_iterator>;
  ...
  using udir_iterator_base =
      toolchain::iterator_adaptor_base<udir_iterator, lookup_iterator,
  ...
  using udir_range = toolchain::iterator_range<udir_iterator>;
  ...
  using ddiag_range = toolchain::iterator_range<DeclContext::ddiag_iterator>;
  ...
  void dumpLookups(toolchain::raw_ostream &OS, bool DumpDecls = false,
...
struct isa_impl<To, ::language::Core::DeclContext> {
  static bool doit(const ::language::Core::DeclContext &Val) {
...
struct cast_convert_val<ToTy,
                        const ::language::Core::DeclContext,const ::language::Core::DeclContext> {
  static const ToTy &doit(const ::language::Core::DeclContext &Val) {
    return *::language::Core::cast_convert_decl_context<ToTy>::doit(&Val);
  }
};

template<class ToTy>
struct cast_convert_val<ToTy, ::language::Core::DeclContext, ::language::Core::DeclContext> {
  static ToTy &doit(::language::Core::DeclContext &Val) {
    return *::language::Core::cast_convert_decl_context<ToTy>::doit(&Val);
  }
};
...
struct cast_convert_val< ::language::Core::DeclContext, FromTy, FromTy> {
  static ::language::Core::DeclContext &doit(const FromTy &Val) {
...
template<class FromTy>
struct cast_convert_val< const ::language::Core::DeclContext, FromTy, FromTy> {
  static const ::language::Core::DeclContext &doit(const FromTy &Val) {
    return *FromTy::castToDeclContext(&Val);
  }
};
...
#endif // LANGUAGE_CORE_AST_DECLBASE_H
