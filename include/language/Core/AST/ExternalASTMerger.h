...
//
//===----------------------------------------------------------------------===//
#ifndef LANGUAGE_CORE_AST_EXTERNALASTMERGER_H
#define LANGUAGE_CORE_AST_EXTERNALASTMERGER_H
...
private:
  ...
  toolchain::raw_ostream *LogStream;
...
public:
  ...
  void SetLogStream(toolchain::raw_string_ostream &Stream) { LogStream = &Stream; }
private:
...
public:
  ...
  toolchain::raw_ostream &logs() { return *LogStream; }

  /// True if the log stream is not toolchain::nulls();
  bool LoggingEnabled() { return LogStream != &toolchain::nulls(); }
};
