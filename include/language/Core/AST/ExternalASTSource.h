...
#ifndef LANGUAGE_CORE_AST_EXTERNALASTSOURCE_H
#define LANGUAGE_CORE_AST_EXTERNALASTSOURCE_H
...
struct LazyOffsetPtr {
  ...
  unsigned char GetLSB() const {
    return Data[toolchain::sys::IsBigEndianHost ? DataSize - 1 : 0];
  }

  template <typename U> U &As(bool New) const {
    unsigned char *Obj =
        Data + (toolchain::sys::IsBigEndianHost ? DataSize - sizeof(U) : 0);
    if (New)
      return *new (Obj) U;
...
struct LazyGenerationalUpdatePtr {
  ...
  using ValueType = toolchain::PointerUnion<T, LazyData*>;
  ValueType Value;
...
struct PointerLikeTypeTraits<
    language::Core::LazyGenerationalUpdatePtr<Owner, T, Update>> {
  using Ptr = language::Core::LazyGenerationalUpdatePtr<Owner, T, Update>;
...
#endif // LANGUAGE_CORE_AST_EXTERNALASTSOURCE_H
