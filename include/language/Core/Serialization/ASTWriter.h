...
#ifndef LANGUAGE_CORE_SERIALIZATION_ASTWRITER_H
#define LANGUAGE_CORE_SERIALIZATION_ASTWRITER_H
...
private:
  ...
  using TypeIdxMap = toolchain::DenseMap<QualType, serialization::TypeIdx,
  ...
  toolchain::DenseMap<const FileEntry *, uint32_t> InputFileIDs;
  ...
  toolchain::SmallVector<NamespaceDecl *, 16> DelayedNamespace;

  /// The first ID number we can use for our own declarations.
  LocalDeclID FirstDeclID = LocalDeclID(language::Core::NUM_PREDEF_DECL_IDS);
  ...
  toolchain::DenseMap<const Decl *, LocalDeclID> DeclIDs;
  ...
  toolchain::SmallPtrSet<const Decl *, 32> PredefinedDecls;
  ...
  toolchain::DenseMap<LocalDeclID, SmallVector<LocalDeclID, 4>> RelatedDeclsMap;
  ...
  using FileDeclIDsTy =
      toolchain::DenseMap<FileID, std::unique_ptr<DeclIDInFileInfo>>;
  ...
  /// IdentifierInfo.
  toolchain::MapVector<const IdentifierInfo *, serialization::IdentifierID> IdentifierIDs;
  ...
  toolchain::DenseMap<MacroInfo *, serialization::MacroID> MacroIDs;
  ...
  toolchain::DenseMap<const IdentifierInfo *, uint32_t>
  ...
  toolchain::DenseSet<Stmt *> ParentStmts;

  /// Offsets of sub-stmts already serialized. The offset points
  /// just after the stmt record.
  toolchain::DenseMap<Stmt *, uint64_t> SubStmtEntries;
  ...
  toolchain::MapVector<Selector, serialization::SelectorID> SelectorIDs;
  ...
  toolchain::SetVector<Module *> TouchedTopLevelModules;
  toolchain::SetVector<serialization::ModuleFile *> TouchedModuleFiles;
  ...
  using DeclUpdateMap = toolchain::MapVector<const Decl *, UpdateRecord>;
  ...
  using SpecializationUpdateMap =
      toolchain::MapVector<const NamedDecl *, SmallVector<const Decl *>>;
  SpecializationUpdateMap SpecializationsUpdates;
  SpecializationUpdateMap PartialSpecializationsUpdates;

  using FirstLatestDeclMap = toolchain::DenseMap<Decl *, Decl *>;
  ...
  /// it.
  toolchain::SmallSetVector<const DeclContext *, 16> UpdatedDeclContexts;
  ...
  /// should serialize.
  toolchain::SetVector<ObjCInterfaceDecl *> ObjCClassesWithCategories;

  /// The set of declarations that may have redeclaration chains that
  /// need to be serialized.
  toolchain::SmallVector<const Decl *, 16> Redeclarations;

  /// A cache of the first local declaration for "interesting"
  /// redeclaration chains.
  toolchain::DenseMap<const Decl *, const Decl *> FirstLocalDeclCache;

  /// Mapping from SwitchCase statements to IDs.
  toolchain::DenseMap<SwitchCase *, unsigned> SwitchCaseIDs;
  ...
  toolchain::DenseMap<const Module *, unsigned> SubmoduleIDs;
  ...
  toolchain::BitVector IsSLocAffecting;
  /// Mapping from a source location entry to whether it must be included as
  /// input file.
  toolchain::BitVector IsSLocFileEntryAffecting;
  ...
  toolchain::SmallVector<CXXRecordDecl *> PendingEmittingVTables;
  ...
  GenerateNameLookupTable(ASTContext &Context, const DeclContext *DC,
                          ...
                          toolchain::SmallVectorImpl<char> &TULocalLookupTable);
...
public:
  ...
  /// the given bitstream.
  ASTWriter(toolchain::BitstreamWriter &Stream, SmallVectorImpl<char> &Buffer,
  ...
  ASTFileSignature WriteAST(toolchain::PointerUnion<Sema *, Preprocessor *> Subject,
  ...
  bool IsLocalDecl(const Decl *D) {
    ...
    return (I == DeclIDs.end() || I->second >= language::Core::NUM_PREDEF_DECL_IDS);
  };
  ...
  unsigned getDeclCXXMethodAbbrev(FunctionDecl::TemplatedKind Kind) const {
    ...
    toolchain_unreachable("Unknwon Template Kind!");
  }
...
class PCHGenerator : public SemaConsumer {
  ...
  Preprocessor &PP;
  toolchain::PointerUnion<Sema *, Preprocessor *> Subject;
...
#endif // LANGUAGE_CORE_SERIALIZATION_ASTWRITER_H
