...
#ifndef LANGUAGE_CORE_SERIALIZATION_ASTREADER_H
#define LANGUAGE_CORE_SERIALIZATION_ASTREADER_H
...
#include "language/Core/Lex/ExternalPreprocessorSource.h"
...
#include "toolchain/ADT/PagedVector.h"
...
private:
  ...
  toolchain::StringMap<std::shared_ptr<ModuleFileExtension>> ModuleFileExtensions;
  ...
  toolchain::PagedVector<QualType> TypesLoaded;

  /// Declarations that have already been loaded from the chain.
  ///
  /// When the pointer at index I is non-NULL, the declaration with ID
  /// = I + 1 has already been loaded.
  toolchain::PagedVector<Decl *> DeclsLoaded;

  using FileOffset = std::pair<ModuleFile *, uint64_t>;
  using FileOffsetsTy = SmallVector<FileOffset, 2>;
  using DeclUpdateOffsetsMap = toolchain::DenseMap<GlobalDeclID, FileOffsetsTy>;
  ...
  using DelayedNamespaceOffsetMapTy =
      toolchain::DenseMap<GlobalDeclID, LookupBlockOffsets>;
  ...
  toolchain::DenseMap<GlobalDeclID, SmallVector<GlobalDeclID, 4>> RelatedDeclsMap;
  ...
  toolchain::SmallVector<PendingUpdateRecord, 16> PendingUpdateRecords;

  enum class PendingFakeDefinitionKind { NotFake, Fake, FakeLoaded };

  /// The DefinitionData pointers that we faked up for class definitions
  /// that we needed but hadn't loaded yet.
  toolchain::DenseMap<void *, PendingFakeDefinitionKind> PendingFakeDefinitionData;
  ...
  toolchain::SmallMapVector<Decl *, FunctionDecl *, 4> PendingExceptionSpecUpdates;
  ...
  toolchain::SmallMapVector<FunctionDecl *, QualType, 4> PendingDeducedTypeUpdates;
  ...
  toolchain::SmallVector<FunctionDecl *, 4> PendingUndeducedFunctionDecls;

  /// Declarations that have been imported and have typedef names for
  /// linkage purposes.
  toolchain::DenseMap<std::pair<DeclContext *, IdentifierInfo *>, NamedDecl *>
      ImportedTypedefNamesForLinkage;

  /// Mergeable declaration contexts that have anonymous declarations
  /// within them, and those anonymous declarations.
  toolchain::DenseMap<Decl*, toolchain::SmallVector<NamedDecl*, 2>>
    AnonymousDeclarationsForMerging;

  /// Map from numbering information for lambdas to the corresponding lambdas.
  toolchain::DenseMap<std::pair<const Decl *, unsigned>, NamedDecl *>
  ...
  toolchain::DenseMap<LETemporaryKey, LifetimeExtendedTemporaryDecl *>
  ...
  toolchain::DenseMap<FileID, FileDeclsInfo> FileDeclIDs;
  ...
  toolchain::DenseMap<const DeclContext*, std::pair<ModuleFile*, LexicalContents>>
      LexicalDecls;
  ...
  toolchain::DenseMap<const DeclContext *,
  ...
  toolchain::DenseMap<GlobalDeclID, DeclContextVisibleUpdates> PendingVisibleUpdates;
  toolchain::DenseMap<GlobalDeclID, DeclContextVisibleUpdates>
      PendingModuleLocalVisibleUpdates;
  toolchain::DenseMap<GlobalDeclID, DeclContextVisibleUpdates> TULocalUpdates;

  using SpecializationsUpdate = SmallVector<UpdateData, 1>;
  using SpecializationsUpdateMap =
      toolchain::DenseMap<GlobalDeclID, SpecializationsUpdate>;
  ...
  toolchain::SmallPtrSet<Decl *, 4> PendingDefinitions;

  using PendingBodiesMap =
      toolchain::MapVector<Decl *, uint64_t,
                      toolchain::SmallDenseMap<Decl *, unsigned, 4>,
  ...
  toolchain::SetVector<NamedDecl *> PendingMergedDefinitionsToDeduplicate;
  ...
  toolchain::SmallVector<std::pair<Decl *, Decl *>>
  ...
  bool ReadSpecializations(ModuleFile &M, toolchain::BitstreamCursor &Cursor,
  ...
  /// files.
  toolchain::DenseSet<LoadedMacroInfo> LoadedUndefs;
  ...
  using HiddenNamesMapType = toolchain::DenseMap<Module *, HiddenNames>;
  ...
  toolchain::DenseMap<Selector, unsigned> SelectorGeneration;
  ...
  toolchain::DenseMap<Selector, bool> SelectorOutOfDate;
  ...
  using PendingMacroIDsMap =
      toolchain::MapVector<IdentifierInfo *, SmallVector<PendingMacroInfo, 2>>;
  ...
  toolchain::SmallVector<FpPragmaStackEntry, 2> FpPragmaStack;
  toolchain::SmallVector<std::string, 2> FpPragmaStrings;
  ...
  toolchain::SmallVector<PragmaAlignPackStackEntry, 2> PragmaAlignPackStack;
  toolchain::SmallVector<std::string, 2> PragmaAlignPackStrings;

  /// The OpenCL extension settings.
  OpenCLOptions OpenCLExtensions;

  /// Extensions required by an OpenCL type.
  toolchain::DenseMap<const Type *, std::set<std::string>> OpenCLTypeExtMap;

  /// Extensions required by an OpenCL declaration.
  toolchain::DenseMap<const Decl *, std::set<std::string>> OpenCLDeclExtMap;
  ...
  toolchain::SmallSetVector<GlobalDeclID, 4> DeclsToCheckForDeferredDiags;
...
private:
  ...
  using SwitchCaseMapTy = toolchain::DenseMap<unsigned, SwitchCase *>;
  ...
  toolchain::MapVector<IdentifierInfo *, SmallVector<GlobalDeclID, 4>>
      PendingIdentifierInfos;

  /// The set of lookup results that we have faked in order to support
  /// merging of partially deserialized decls but that we have not yet removed.
  toolchain::SmallMapVector<const IdentifierInfo *, SmallVector<NamedDecl *, 2>, 16>
      PendingFakeLookupResults;

  /// The generation number of each identifier, which keeps track of
  /// the last time we loaded information about this identifier.
  toolchain::DenseMap<const IdentifierInfo *, unsigned> IdentifierGeneration;
  ...
  toolchain::SmallMapVector<DuplicateObjCDecls<ObjCCategoryDecl>,
                       toolchain::SmallVector<DuplicateObjCDecls<ObjCIvarDecl>, 4>,
                       2>
  ...
  toolchain::SmallVector<NamedDecl *, 16> PendingOdrMergeChecks;
  ...
  toolchain::SmallDenseMap<CXXRecordDecl *, toolchain::SmallVector<DataPointers, 2>, 2>
      PendingOdrMergeFailures;

  /// C/ObjC record definitions in which we found an ODR violation.
  toolchain::SmallDenseMap<RecordDecl *, toolchain::SmallVector<RecordDecl *, 2>, 2>
      PendingRecordOdrMergeFailures;

  /// Function definitions in which we found an ODR violation.
  toolchain::SmallDenseMap<FunctionDecl *, toolchain::SmallVector<FunctionDecl *, 2>, 2>
      PendingFunctionOdrMergeFailures;

  /// Enum definitions in which we found an ODR violation.
  toolchain::SmallDenseMap<EnumDecl *, toolchain::SmallVector<EnumDecl *, 2>, 2>
      PendingEnumOdrMergeFailures;

  /// ObjCInterfaceDecl in which we found an ODR violation.
  toolchain::SmallDenseMap<ObjCInterfaceDecl *,
                      toolchain::SmallVector<ObjCInterfaceDataPointers, 2>, 2>
      PendingObjCInterfaceOdrMergeFailures;

  /// ObjCProtocolDecl in which we found an ODR violation.
  toolchain::SmallDenseMap<ObjCProtocolDecl *,
                      toolchain::SmallVector<ObjCProtocolDataPointers, 2>, 2>
      PendingObjCProtocolOdrMergeFailures;

  /// DeclContexts in which we have diagnosed an ODR violation.
  toolchain::SmallPtrSet<DeclContext*, 2> DiagnosedOdrMergeFailures;

  /// The set of Objective-C categories that have been deserialized
  /// since the last time the declaration chains were linked.
  toolchain::SmallPtrSet<ObjCCategoryDecl *, 16> CategoriesDeserialized;
  ...
  using KeyDeclsMap = toolchain::DenseMap<Decl *, SmallVector<GlobalDeclID, 2>>;
  ...
  toolchain::DenseMap<const Decl *, bool> DefinitionSource;

  /// Friend functions that were defined but might have had their bodies
  /// removed.
  toolchain::DenseSet<const FunctionDecl *> ThisDeclarationWasADefinitionSet;
  ...
  class TemporarilyOwnedStringRef {
    StringRef String;
    toolchain::SaveAndRestore<SmallString<0>> UnderlyingBuffer;
...
private:
  ...
  ReadOptionsBlock(toolchain::BitstreamCursor &Stream, StringRef Filename,
                   unsigned ClientLoadCapabilities,
  ...
  toolchain::Error ReadASTBlock(ModuleFile &F, unsigned ClientLoadCapabilities);
  toolchain::Error ReadExtensionBlock(ModuleFile &F);
  void ReadModuleOffsetMap(ModuleFile &F) const;
  void ParseLineTable(ModuleFile &F, const RecordData &Record);
  toolchain::Error ReadSourceManagerBlock(ModuleFile &F);
  SourceLocation getImportLocation(ModuleFile *F);
  ASTReadResult ReadModuleMapFileBlock(RecordData &Record, ModuleFile &F,
                                       const ModuleFile *ImportedBy,
                                       unsigned ClientLoadCapabilities);
  toolchain::Error ReadSubmoduleBlock(ModuleFile &F,
                                 unsigned ClientLoadCapabilities);
...
public:
  ...
  toolchain::iterator_range<ModuleDeclIterator>
...
private:
  ...
  void Error(toolchain::Error &&Err) const;
...
public:
  ...
  static toolchain::Error ReadBlockAbbrevs(toolchain::BitstreamCursor &Cursor,
                                      unsigned BlockID,
  ...
  FindExternalLexicalDecls(const DeclContext *DC,
                           toolchain::function_ref<bool(Decl::Kind)> IsKindWeWant,
  ...
  void ReadUndefinedButUsed(
      toolchain::MapVector<NamedDecl *, SourceLocation> &Undefined) override;

  void ReadMismatchingDeleteExpressions(toolchain::MapVector<
  ...
  void ReadUnusedLocalTypedefNameCandidates(
      toolchain::SmallSetVector<const TypedefNameDecl *, 4> &Decls) override;

  void ReadDeclsToCheckForDeferredDiags(
      toolchain::SmallSetVector<Decl *, 4> &Decls) override;
  ...
  /// module file.
  toolchain::Expected<SourceLocation::UIntTy> readSLocOffset(ModuleFile *F,
                                                        unsigned Index);
  ...
  static toolchain::BitVector ReadBitVector(const RecordData &Record,
  ...
  SmallVector<std::pair<toolchain::BitstreamCursor,
  ...
  void visitTopLevelModuleMaps(serialization::ModuleFile &MF,
                               toolchain::function_ref<void(FileEntryRef)> Visitor);
...
#endif // LANGUAGE_CORE_SERIALIZATION_ASTREADER_H
