...
#ifndef LANGUAGE_CORE_SERIALIZATION_MODULEMANAGER_H
#define LANGUAGE_CORE_SERIALIZATION_MODULEMANAGER_H
...
class ModuleManager {
  ...
  toolchain::DenseMap<const FileEntry *, ModuleFile *> Modules;
  ...
  toolchain::DenseMap<const FileEntry *, std::unique_ptr<toolchain::MemoryBuffer>>
      InMemoryBuffers;
...
public:
  using ModuleIterator = toolchain::pointee_iterator<
      SmallVectorImpl<std::unique_ptr<ModuleFile>>::iterator>;
  using ModuleConstIterator = toolchain::pointee_iterator<
      SmallVectorImpl<std::unique_ptr<ModuleFile>>::const_iterator>;
  using ModuleReverseIterator = toolchain::pointee_iterator<
  ...
  toolchain::iterator_range<SmallVectorImpl<ModuleFile *>::const_iterator>
  pch_modules() const {
    return toolchain::make_range(PCHChain.begin(), PCHChain.end());
  }
  ...
  std::unique_ptr<toolchain::MemoryBuffer> lookupBuffer(StringRef Name);
  ...
  void visit(toolchain::function_ref<bool(ModuleFile &M)> Visitor,
             toolchain::SmallPtrSetImpl<ModuleFile *> *ModuleFilesHit = nullptr);
...
#endif // LANGUAGE_CORE_SERIALIZATION_MODULEMANAGER_H
