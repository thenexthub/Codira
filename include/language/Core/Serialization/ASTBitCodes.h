...
#ifndef LANGUAGE_CORE_SERIALIZATION_ASTBITCODES_H
#define LANGUAGE_CORE_SERIALIZATION_ASTBITCODES_H
...
public:
  ...
  static TypeIdx fromTypeID(TypeID ID) {
    ...
    return TypeIdx(ID >> 32, (ID & toolchain::maskTrailingOnes<TypeID>(32)) >>
...
using unaligned_decl_id_t =
    toolchain::support::detail::packed_endian_specific_integral<
        serialization::DeclID, toolchain::endianness::native,
        toolchain::support::unaligned>;
...
enum BlockIDs {
  ...
  /// full AST block.
  AST_BLOCK_ID = toolchain::bitc::FIRST_APPLICATION_BLOCKID,
...
template <> struct DenseMapInfo<language::Core::serialization::DeclarationNameKey> {
  static language::Core::serialization::DeclarationNameKey getEmptyKey() {
    return language::Core::serialization::DeclarationNameKey(-1, 1);
  }

  static language::Core::serialization::DeclarationNameKey getTombstoneKey() {
    return language::Core::serialization::DeclarationNameKey(-1, 2);
  }

  static unsigned
  getHashValue(const language::Core::serialization::DeclarationNameKey &Key) {
    return Key.getHash();
  }

  static bool isEqual(const language::Core::serialization::DeclarationNameKey &L,
                      const language::Core::serialization::DeclarationNameKey &R) {
    return L == R;
  }
};
...
#endif // LANGUAGE_CORE_SERIALIZATION_ASTBITCODES_H
