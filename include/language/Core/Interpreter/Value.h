...
// V.getType(); // <-- Yields a language::Core::QualType.
// V.getInt(); // <-- Yields 42.
//
...
#ifndef LANGUAGE_CORE_INTERPRETER_VALUE_H
#define LANGUAGE_CORE_INTERPRETER_VALUE_H

#include "toolchain/Config/toolchain-config.h" // for LLVM_BUILD_LLVM_DYLIB, LLVM_BUILD_SHARED_LIBS
...
#undef X
  ...
  void printType(toolchain::raw_ostream &Out) const;
  void printData(toolchain::raw_ostream &Out) const;
