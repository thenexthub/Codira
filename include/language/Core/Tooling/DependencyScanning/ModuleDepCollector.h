...
#ifndef LANGUAGE_CORE_TOOLING_DEPENDENCYSCANNING_MODULEDEPCOLLECTOR_H
#define LANGUAGE_CORE_TOOLING_DEPENDENCYSCANNING_MODULEDEPCOLLECTOR_H
...
using PrebuiltModulesAttrsMap = toolchain::StringMap<PrebuiltModuleASTAttrs>;
class PrebuiltModuleASTAttrs {
public:
  ...
  const toolchain::StringSet<> &getVFS() const { return VFSMap; }

  /// Update the VFSMap to the one discovered from serializing the AST file.
  void setVFS(toolchain::StringSet<> &&VFS) { VFSMap = std::move(VFS); }
...
struct ModuleDeps {
  ...
  toolchain::SmallVector<Module::LinkLibrary, 2> LinkLibraries;
  ...
  void forEachFileDep(toolchain::function_ref<void(StringRef)> Cb) const;
...
private:
  ...
  toolchain::MapVector<const Module *, std::unique_ptr<ModuleDeps>> ModularDeps;
  /// Secondary mapping for \c ModularDeps allowing lookup by ModuleID without
  /// a preprocessor. Storage owned by \c ModularDeps.
  toolchain::DenseMap<ModuleID, ModuleDeps *> ModuleDepsByID;
  /// Direct modular dependencies that have already been built.
  toolchain::MapVector<const Module *, PrebuiltModuleDep> DirectPrebuiltModularDeps;
  /// Working set of direct modular dependencies.
  toolchain::SetVector<const Module *> DirectModularDeps;
  /// Working set of direct modular dependencies, as they were imported.
  toolchain::SmallPtrSet<const Module *, 32> DirectImports;
  /// All direct and transitive visible modules.
  toolchain::StringSet<> VisibleModules;
  ...
  toolchain::DenseSet<const FileEntry *>
...
namespace toolchain {
inline hash_code hash_value(const language::Core::tooling::dependencies::ModuleID &ID) {
  return hash_combine(ID.ModuleName, ID.ContextHash);
}

template <> struct DenseMapInfo<language::Core::tooling::dependencies::ModuleID> {
  using ModuleID = language::Core::tooling::dependencies::ModuleID;
...
#endif // LANGUAGE_CORE_TOOLING_DEPENDENCYSCANNING_MODULEDEPCOLLECTOR_H
