...
#ifndef LANGUAGE_CORE_TOOLING_DEPENDENCYSCANNING_DEPENDENCYSCANNINGFILESYSTEM_H
#define LANGUAGE_CORE_TOOLING_DEPENDENCYSCANNING_DEPENDENCYSCANNINGFILESYSTEM_H

#include "language/Core/Basic/LLVM.h"
#include "language/Core/Lex/DependencyDirectivesScanner.h"
...
struct CachedFileContents {
  CachedFileContents(std::unique_ptr<toolchain::MemoryBuffer> Contents)
      : Original(std::move(Contents)), DepDirectives(nullptr) {}

  /// Owning storage for the original contents.
  std::unique_ptr<toolchain::MemoryBuffer> Original;
...
public:
  ...
  CachedFileSystemEntry(toolchain::ErrorOr<toolchain::vfs::Status> Stat)
  ...
  toolchain::vfs::Status getStatus() const {
    assert(!isError() && "error");
  ...
  toolchain::sys::fs::UniqueID getUniqueID() const {
    assert(!isError() && "error");
...
using CachedRealPath = toolchain::ErrorOr<std::string>;
...
public:
  struct CacheShard {
    ...
    toolchain::DenseMap<toolchain::sys::fs::UniqueID, const CachedFileSystemEntry *>
        EntriesByUID;

    /// The backing storage for cached entries.
    toolchain::SpecificBumpPtrAllocator<CachedFileSystemEntry> EntryStorage;

    /// The backing storage for cached contents.
    toolchain::SpecificBumpPtrAllocator<CachedFileContents> ContentsStorage;

    /// The backing storage for cached real paths.
    toolchain::SpecificBumpPtrAllocator<CachedRealPath> RealPathStorage;
    ...
    findEntryByUID(toolchain::sys::fs::UniqueID UID) const;
    ...
    const CachedFileSystemEntry &
    getOrEmplaceEntryForUID(toolchain::sys::fs::UniqueID UID, toolchain::vfs::Status Stat,
    ...
    getOrEmplaceRealPathForFilename(StringRef Filename,
                                    toolchain::ErrorOr<StringRef> RealPath);
  };
  ...
  CacheShard &getShardForUID(toolchain::sys::fs::UniqueID UID) const;
  ...
  getOutOfDateEntries(toolchain::vfs::FileSystem &UnderlyingFS) const;
...
public:
  ...
  toolchain::vfs::Status getStatus() const {
    toolchain::vfs::Status Stat = Entry.getStatus();
    if (!Stat.isDirectory())
      Stat = toolchain::vfs::Status::copyWithNewSize(Stat, getContents().size());
    return toolchain::vfs::Status::copyWithNewName(Stat, Filename);
  }
  ...
  toolchain::ErrorOr<EntryRef> unwrapError() const {
    if (isError())
...
class DependencyScanningWorkerFilesystem
    : public toolchain::RTTIExtends<DependencyScanningWorkerFilesystem,
                               toolchain::vfs::ProxyFileSystem> {
public:
  ...
  toolchain::ErrorOr<toolchain::vfs::Status> status(const Twine &Path) override;
  ...
  toolchain::ErrorOr<EntryRef> getOrCreateFileSystemEntry(StringRef Filename);
  ...
  getDirectiveTokens(const Twine &Path) {
    if (toolchain::ErrorOr<EntryRef> Entry = getOrCreateFileSystemEntry(Path.str()))
...
private:
  ...
  struct TentativeEntry {
    ...
    TentativeEntry(toolchain::vfs::Status Status,
                   std::unique_ptr<toolchain::MemoryBuffer> Contents = nullptr)
  ...
  toolchain::ErrorOr<TentativeEntry> readFile(StringRef Filename);
  ...
  /// if none is found.
  const CachedFileSystemEntry *
  findSharedEntryByUID(toolchain::vfs::Status Stat) const {
...
#endif // LANGUAGE_CORE_TOOLING_DEPENDENCYSCANNING_DEPENDENCYSCANNINGFILESYSTEM_H
