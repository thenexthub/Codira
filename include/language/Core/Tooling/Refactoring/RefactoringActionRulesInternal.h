...
#ifndef LANGUAGE_CORE_TOOLING_REFACTORING_REFACTORINGACTIONRULESINTERNAL_H
#define LANGUAGE_CORE_TOOLING_REFACTORING_REFACTORINGACTIONRULESINTERNAL_H
...
#include "language/Core/Tooling/Refactoring/RefactoringResultConsumer.h"
...
inline toolchain::Error findError() { return toolchain::Error::success(); }
...
void ignoreError(Expected<FirstT> &First, Expected<RestT> &... Rest) {
  if (!First)
    toolchain::consumeError(First.takeError());
  ignoreError(Rest...);
}
...
/// invalid.
template <typename FirstT, typename... RestT>
toolchain::Error findError(Expected<FirstT> &First, Expected<RestT> &... Rest) {
  if (!First) {
    ignoreError(Rest...);
...
#endif // LANGUAGE_CORE_TOOLING_REFACTORING_REFACTORINGACTIONRULESINTERNAL_H
