...
#ifndef LANGUAGE_CORE_TOOLING_CORE_REPLACEMENT_H
#define LANGUAGE_CORE_TOOLING_CORE_REPLACEMENT_H
...
/// Carries extra error information in replacement-related toolchain::Error,
/// e.g. fail applying replacements and replacements conflict.
class ReplacementError : public toolchain::ErrorInfo<ReplacementError> {
public:
...
public:
  ...
  /// it returns an toolchain::Error, i.e. there is a conflict between R and the
  ...
  /// converted to a string message with `toolchain::toString()`. This prevents users
  ...
  toolchain::Error add(const Replacement &R);
...
/// replacements applied; otherwise, an toolchain::Error carrying toolchain::StringError
/// is returned (the Error message can be converted to string using
/// `toolchain::toString()` and 'std::error_code` in the `Error` should be ignored).
toolchain::Expected<std::string> applyAllReplacements(StringRef Code,
...
#endif // LANGUAGE_CORE_TOOLING_CORE_REPLACEMENT_H
