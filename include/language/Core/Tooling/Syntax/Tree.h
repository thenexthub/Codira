...
//===----------------------------------------------------------------------===//
#ifndef LANGUAGE_CORE_TOOLING_SYNTAX_TREE_H
#define LANGUAGE_CORE_TOOLING_SYNTAX_TREE_H
...
class Tree : public Node {
  ...
  class ChildIteratorBase
      : public toolchain::iterator_facade_base<DerivedT, std::forward_iterator_tag,
                                          NodeT> {
  protected:
    NodeT *N = nullptr;
...
public:
  ...
  toolchain::iterator_range<ChildIterator> getChildren() {
    return {ChildIterator(getFirstChild()), ChildIterator()};
  }
  toolchain::iterator_range<ConstChildIterator> getChildren() const {
...
public:
  ...
  language::Core::tok::TokenKind getDelimiterTokenKind() const;
