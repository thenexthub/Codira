...
#ifndef LANGUAGE_CORE_TOOLING_SYNTAX_TOKENS_H
#define LANGUAGE_CORE_TOOLING_SYNTAX_TOKENS_H
...
struct FileRange {
  ...
  toolchain::StringRef text(const SourceManager &SM) const;
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, const FileRange &R);
...
public:
  ...
  /// EXPECTS: language::Core::Token is not an annotation token.
  explicit Token(const language::Core::Token &T);
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, const Token &T);
...
public:
  ...
  toolchain::ArrayRef<syntax::Token> expandedTokens() const {
  ...
  toolchain::ArrayRef<syntax::Token> expandedTokens(SourceRange R) const;
  ...
  spelledForExpanded(toolchain::ArrayRef<syntax::Token> Expanded) const;
  ...
  expandedForSpelled(toolchain::ArrayRef<syntax::Token> Spelled) const;
  ...
  struct Expansion {
    toolchain::ArrayRef<syntax::Token> Spelled;
  ...
  expansionsOverlapping(toolchain::ArrayRef<syntax::Token> Spelled) const;
  ...
  ///    {"#", "define", "DECL", "(", "name", ")", "int", "name", "=", "10",
  ///     "DECL", "(", "a", ")", ";"}
  toolchain::ArrayRef<syntax::Token> spelledTokens(FileID FID) const;
...
private:
  ...
  const MarkedFile &fileForSpelled(toolchain::ArrayRef<syntax::Token> Spelled) const;
  ...
  toolchain::DenseMap<SourceLocation, unsigned> ExpandedTokIndex;
  toolchain::DenseMap<FileID, MarkedFile> Files;
...
toolchain::ArrayRef<syntax::Token>
spelledTokensTouching(SourceLocation Loc, toolchain::ArrayRef<syntax::Token> Tokens);
...
private:
  ...
  /// stored in this map.
  using PPExpansions = toolchain::DenseMap<SourceLocation, SourceLocation>;
  class Builder;
