...
#ifndef LANGUAGE_CORE_TOOLING_TRANSFORMER_REWRITERULE_H
#define LANGUAGE_CORE_TOOLING_TRANSFORMER_REWRITERULE_H
...
struct ASTEdit {
  ...
  AnyGenerator Metadata = [](const ast_matchers::MatchFinder::MatchResult &)
      -> toolchain::Expected<toolchain::Any> {
    return toolchain::Expected<toolchain::Any>(toolchain::Any());
  };
};
...
EditGenerator editList(toolchain::SmallVector<ASTEdit, 1> Edits);
...
// FIXME: If `Metadata` returns an `toolchain::Expected<T>` the `AnyGenerator` will
// construct an `toolchain::Expected<toolchain::Any>` where no error is present but the
// `toolchain::Any` holds the error. This is unlikely but potentially surprising.
// Perhaps the `toolchain::Expected` should be unwrapped, or perhaps this should be a
...
inline ASTEdit withMetadata(ASTEdit Edit, Callable Metadata) {
  Edit.Metadata =
      [Gen = std::move(Metadata)](
          const ast_matchers::MatchFinder::MatchResult &R) -> toolchain::Any {
    return Gen(R);
  };
...
EditGenerator makeEditGenerator(toolchain::SmallVector<ASTEdit, 1> Edits);
...
/// \endcode
void addInclude(RewriteRuleBase &Rule, toolchain::StringRef Header,
                IncludeFormat Format = IncludeFormat::Quoted);
...
applyFirst(const std::vector<RewriteRuleWith<MetadataT>> &Rules) {
  return applyFirst(toolchain::ArrayRef(Rules));
}

template <typename MetadataT>
RewriteRuleWith<MetadataT>
applyFirst(std::initializer_list<RewriteRuleWith<MetadataT>> Rules) {
  return applyFirst(toolchain::ArrayRef(Rules.begin(), Rules.end()));
}
/// @}
...
/// ```
/// @{
toolchain::Expected<SmallVector<Edit, 1>>
...
#endif // LANGUAGE_CORE_TOOLING_TRANSFORMER_REWRITERULE_H
