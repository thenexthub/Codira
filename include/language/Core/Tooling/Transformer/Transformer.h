...
#ifndef LANGUAGE_CORE_TOOLING_TRANSFORMER_TRANSFORMER_H_
#define LANGUAGE_CORE_TOOLING_TRANSFORMER_TRANSFORMER_H_

#include "language/Core/ASTMatchers/ASTMatchFinder.h"
#include "language/Core/Tooling/Refactoring/AtomicChange.h"
#include "language/Core/Tooling/Transformer/RewriteRule.h"
...
protected:
  ...
  static toolchain::Expected<toolchain::SmallVector<AtomicChange, 1>>
  convertToAtomicChanges(const toolchain::SmallVectorImpl<transformer::Edit> &Edits,
...
template <typename T> struct TransformerResult {
  toolchain::MutableArrayRef<AtomicChange> Changes;
  T Metadata;
};
...
public:
  ...
  using ChangeSetConsumer = std::function<void(
      Expected<toolchain::MutableArrayRef<AtomicChange>> Changes)>;
  ...
  explicit Transformer(transformer::RewriteRuleWith<void> Rule,
      ...
      : Transformer(std::move(Rule),
                    [Consumer = std::move(Consumer)](
                        toolchain::Expected<TransformerResult<void>> Result) {
                      if (Result)
  ...
  explicit Transformer(
      ...
      std::function<void(
          toolchain::Expected<TransformerResult<toolchain::type_identity_t<MetadataT>>>)>
          Consumer);
...
template <typename T> class WithMetadataImpl final : public TransformerImpl {
  ...
  std::function<void(toolchain::Expected<TransformerResult<T>>)> Consumer;
...
private:
  void onMatchImpl(const ast_matchers::MatchFinder::MatchResult &Result) final {
    ...
    RewriteResult.Changes = toolchain::MutableArrayRef<AtomicChange>(Changes);
...
#endif // LANGUAGE_CORE_TOOLING_TRANSFORMER_TRANSFORMER_H_
