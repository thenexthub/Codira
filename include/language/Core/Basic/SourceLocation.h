...
//
/// \file
/// Defines the language::Core::SourceLocation class and associated facilities.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_BASIC_SOURCELOCATION_H
#define LANGUAGE_CORE_BASIC_SOURCELOCATION_H
...
class SourceLocation {
  ...
  friend class ASTWriter;
  friend class SourceManager;
  friend struct toolchain::FoldingSetTrait<SourceLocation, void>;
...
namespace toolchain {
  ...
  /// DenseSets.
  template <>
  struct DenseMapInfo<language::Core::FileID, void> {
    static language::Core::FileID getEmptyKey() {
      return {};
    }

    static language::Core::FileID getTombstoneKey() {
      return language::Core::FileID::getSentinel();
    }

    static unsigned getHashValue(language::Core::FileID S) {
      return S.getHashValue();
    }

    static bool isEqual(language::Core::FileID LHS, language::Core::FileID RHS) {
      return LHS == RHS;
    }
  };
  ...
  template <> struct DenseMapInfo<language::Core::SourceLocation, void> {
    static language::Core::SourceLocation getEmptyKey() {
      constexpr language::Core::SourceLocation::UIntTy Zero = 0;
      return language::Core::SourceLocation::getFromRawEncoding(~Zero);
    }

    static language::Core::SourceLocation getTombstoneKey() {
      constexpr language::Core::SourceLocation::UIntTy Zero = 0;
      return language::Core::SourceLocation::getFromRawEncoding(~Zero - 1);
    }

    static unsigned getHashValue(language::Core::SourceLocation Loc) {
      return Loc.getHashValue();
    }

    static bool isEqual(language::Core::SourceLocation LHS, language::Core::SourceLocation RHS) {
      return LHS == RHS;
    }
  };

  // Allow calling FoldingSetNodeID::Add with SourceLocation object as parameter
  template <> struct FoldingSetTrait<language::Core::SourceLocation, void> {
    static void Profile(const language::Core::SourceLocation &X, FoldingSetNodeID &ID);
  };

} // namespace toolchain

#endif // LANGUAGE_CORE_BASIC_SOURCELOCATION_H
