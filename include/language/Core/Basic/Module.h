...
//
/// \file
/// Defines the language::Core::Module class, which describes a module in the
/// source code.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_BASIC_MODULE_H
#define LANGUAGE_CORE_BASIC_MODULE_H
...
///
/// Aligned to 8 bytes to allow for toolchain::PointerIntPair<Module *, 3>.
...
private:
  ...
  mutable toolchain::StringMap<unsigned> SubModuleIndex;
  ...
  toolchain::SmallSetVector<FileEntryRef, 2> TopHeaders;
  ...
  mutable toolchain::DenseSet<const Module*> VisibleModulesCache;
...
public:
  ...
  /// module depends.
  toolchain::SmallSetVector<Module *, 2> Imports;
  ...
  using ExportDecl = toolchain::PointerIntPair<Module *, 1, bool>;
  ...
  toolchain::SmallSetVector<const Module *, 2> UndeclaredUses;
  ...
  toolchain::SmallVector<LinkLibrary, 2> LinkLibraries;
  ...
  toolchain::iterator_range<submodule_iterator> submodules() {
    return toolchain::make_range(SubModules.begin(), SubModules.end());
  }
  toolchain::iterator_range<submodule_const_iterator> submodules() const {
    return toolchain::make_range(SubModules.begin(), SubModules.end());
  }
...
public:
  ...
  using VisibleCallback = toolchain::function_ref<void(Module *M)>;
  ...
  using ConflictCallback =
      toolchain::function_ref<void(ArrayRef<Module *> Path, Module *Conflict,
                         StringRef Message)>;
...
#endif // LANGUAGE_CORE_BASIC_MODULE_H
