...
#ifndef LANGUAGE_CORE_BASIC_SOURCEMANAGER_H
#define LANGUAGE_CORE_BASIC_SOURCEMANAGER_H
...
public:
  ...
  static LineOffsetMapping get(toolchain::MemoryBufferRef Buffer,
...
class alignas(8) ContentCache {
  ...
  /// file.
  mutable std::unique_ptr<toolchain::MemoryBuffer> Buffer;
...
public:
  ...
  toolchain::MemoryBuffer::BufferKind getMemoryBufferKind() const;

  /// Return the buffer, only if it has been loaded.
  std::optional<toolchain::MemoryBufferRef> getBufferIfLoaded() const {
    if (Buffer)
  ...
  /// Set the buffer.
  void setBuffer(std::unique_ptr<toolchain::MemoryBuffer> B) {
    IsBufferInvalid = false;
    Buffer = std::move(B);
  }
  ...
  void setUnownedBuffer(std::optional<toolchain::MemoryBufferRef> B) {
    assert(!Buffer && "Expected to be called right after construction");
    if (B)
      setBuffer(toolchain::MemoryBuffer::getMemBuffer(*B));
  }
...
///
class FileInfo {
  friend class language::Core::SourceManager;
  friend class language::Core::ASTWriter;
  friend class language::Core::ASTReader;
  ...
  toolchain::PointerIntPair<const ContentCache *, 3, CharacteristicKind>
      ContentAndKind;
...
class SourceManager : public RefCountedBase<SourceManager> {
  ...
  mutable toolchain::BumpPtrAllocator ContentCacheAlloc;
  ...
  toolchain::DenseMap<FileEntryRef, SrcMgr::ContentCache*> FileInfos;
  ...
  struct OverriddenFilesInfoTy {
    ...
    /// file.
    toolchain::DenseMap<const FileEntry *, FileEntryRef> OverriddenFiles;

    /// Files that were overridden with a memory buffer.
    toolchain::DenseSet<const FileEntry *> OverriddenFilesWithBuffer;
  };
  ...
  /// use (-ID - 2).
  toolchain::PagedVector<SrcMgr::SLocEntry, 32> LoadedSLocEntryTable;
  ...
  toolchain::BitVector SLocEntryLoaded;
  ...
  toolchain::BitVector SLocEntryOffsetLoaded;
  ...
  /// function.
  mutable toolchain::DenseMap<FileID, FileIDAndOffset> IncludedLocMap;
  ...
  using InBeforeInTUCache =
      toolchain::DenseMap<IsBeforeInTUCacheKey, InBeforeInTUCacheEntry>;
  ...
  mutable std::unique_ptr<toolchain::MemoryBuffer> FakeBufferForRecovery;
  ...
  mutable toolchain::DenseMap<FileID, std::unique_ptr<MacroArgsMap>>
      MacroArgsCacheMap;
...
public:
  ...
  FileID createFileID(std::unique_ptr<toolchain::MemoryBuffer> Buffer,
  ...
  FileID createFileID(const toolchain::MemoryBufferRef &Buffer,
                      SrcMgr::CharacteristicKind FileCharacter = SrcMgr::C_User,
                      int LoadedID = 0, SourceLocation::UIntTy LoadedOffset = 0,
                      SourceLocation IncludeLoc = SourceLocation());
  ...
  toolchain::MemoryBufferRef getMemoryBufferForFileOrFake(FileEntryRef File) {
  ...
  void overrideFileContents(FileEntryRef SourceFile,
    ...
    overrideFileContents(SourceFile, toolchain::MemoryBuffer::getMemBuffer(Buffer));
  }
  ...
  using fileinfo_iterator =
      toolchain::DenseMap<FileEntryRef, SrcMgr::ContentCache *>::const_iterator;
...
private:
  ...
  toolchain::MemoryBufferRef getFakeBufferForRecovery() const;
  ...
  SrcMgr::ContentCache &
  createMemBufferContentCache(std::unique_ptr<toolchain::MemoryBuffer> Buf);
...
#endif // LANGUAGE_CORE_BASIC_SOURCEMANAGER_H
