...
#ifndef LANGUAGE_CORE_BASIC_CODEGENOPTIONS_H
#define LANGUAGE_CORE_BASIC_CODEGENOPTIONS_H
...
public:
  ...
  using CFBranchLabelSchemeKind = language::Core::CFBranchLabelSchemeKind;
  using ProfileInstrKind = toolchain::driver::ProfileInstrKind;
  using AsanDetectStackUseAfterReturnMode =
      toolchain::AsanDetectStackUseAfterReturnMode;
  using AsanDtorKind = toolchain::AsanDtorKind;
  using VectorLibrary = toolchain::driver::VectorLibrary;
  using ZeroCallUsedRegsKind = toolchain::ZeroCallUsedRegs::ZeroCallUsedRegsKind;
  using WinX64EHUnwindV2Mode = toolchain::WinX64EHUnwindV2Mode;

  using DebugCompressionType = toolchain::DebugCompressionType;
  using EmitDwarfUnwindType = toolchain::EmitDwarfUnwindType;
  using DebugTemplateNamesKind = toolchain::codegenoptions::DebugTemplateNamesKind;
  using DebugInfoKind = toolchain::codegenoptions::DebugInfoKind;
  using DebuggerKind = toolchain::DebuggerKind;
...
public:
  ...
  static StringRef getFramePointerKindName(FramePointerKind Kind) {
    ...
    toolchain_unreachable("invalid FramePointerKind");
  }
  ...
  /// The string containing the commandline for the toolchain.commandline metadata,
  /// if non-empty.
  std::string RecordCommandLine;

  toolchain::SmallVector<std::pair<std::string, std::string>, 0> DebugPrefixMap;

  /// Prefix replacement map for source-based code coverage to remap source
  /// file paths in coverage mapping.
  toolchain::SmallVector<std::pair<std::string, std::string>, 0> CoveragePrefixMap;
  ...
  toolchain::DenormalMode FPDenormalMode = toolchain::DenormalMode::getIEEE();

  /// The floating-point denormal mode to use, for float.
  toolchain::DenormalMode FP32DenormalMode = toolchain::DenormalMode::getIEEE();
  ...
  struct BitcodeFileToLink {
    ...
    /// Bitwise combination of toolchain::Linker::Flags, passed to the LLVM linker.
  ...
  /// -fsymbol-partition (see https://lld.toolchain.org/Partitions.html).
  ...
  struct OptRemark {
    ...
    std::string Pattern;
    std::shared_ptr<toolchain::Regex> Regex;
  ...
  std::vector<std::function<void(toolchain::PassBuilder &)>> PassBuilderCallbacks;
...
#include "language/Core/Basic/CodeGenOptions.def"
  ...
  bool hasProfileClangInstr() const {
    return getProfileInstr() ==
           toolchain::driver::ProfileInstrKind::ProfileClangInstr;
  }

  /// Check if IR level profile instrumentation is on.
  bool hasProfileIRInstr() const {
    return getProfileInstr() == toolchain::driver::ProfileInstrKind::ProfileIRInstr;
  }

  /// Check if CS IR level profile instrumentation is on.
  bool hasProfileCSIRInstr() const {
    return getProfileInstr() ==
           toolchain::driver::ProfileInstrKind::ProfileCSIRInstr;
  }

  /// Check if any form of instrumentation is on.
  bool hasProfileInstr() const {
    return getProfileInstr() != toolchain::driver::ProfileInstrKind::ProfileNone;
  }

  /// Check if Clang profile use is on.
  bool hasProfileClangUse() const {
    return getProfileUse() == toolchain::driver::ProfileInstrKind::ProfileClangInstr;
  }

  /// Check if IR level profile use is on.
  bool hasProfileIRUse() const {
    return getProfileUse() == toolchain::driver::ProfileInstrKind::ProfileIRInstr ||
           getProfileUse() == toolchain::driver::ProfileInstrKind::ProfileCSIRInstr;
  }

  /// Check if CSIR profile use is on.
  bool hasProfileCSIRUse() const {
    return getProfileUse() == toolchain::driver::ProfileInstrKind::ProfileCSIRInstr;
  }

  /// Check if type and variable info should be emitted.
  bool hasReducedDebugInfo() const {
    return getDebugInfo() >= toolchain::codegenoptions::DebugInfoConstructor;
  }

  /// Check if maybe unused type info should be emitted.
  bool hasMaybeUnusedDebugInfo() const {
    return getDebugInfo() >= toolchain::codegenoptions::UnusedTypeInfo;
  }
  ...
  bool isLoaderReplaceableFunctionName(StringRef FuncName) const {
    return toolchain::is_contained(LoaderReplaceableFunctionNames, FuncName);
  }
};
