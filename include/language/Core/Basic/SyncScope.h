...
#ifndef LANGUAGE_CORE_BASIC_SYNCSCOPE_H
#define LANGUAGE_CORE_BASIC_SYNCSCOPE_H
...
inline toolchain::StringRef getAsString(SyncScope S) {
  switch (S) {
  ...
  toolchain_unreachable("Invalid sync scope");
}
...
public:
  ...
  SyncScope map(unsigned S) const override {
    ...
    toolchain_unreachable("Invalid language sync scope value");
  }
  ...
  ArrayRef<unsigned> getRuntimeValues() const override {
    ...
    return toolchain::ArrayRef(Scopes);
  }
...
AtomicScopeModel::create(AtomicScopeModelKind K) {
  ...
  toolchain_unreachable("Invalid atomic scope model kind");
}
} // namespace language::Core
