...
#ifndef LANGUAGE_CORE_BASIC_BUILTINS_H
#define LANGUAGE_CORE_BASIC_BUILTINS_H
...
#include "language/Core/Basic/BuiltinHeaders.def"
#undef HEADER
  } ID;
...
struct Info {
  ...
  struct StrOffsets {
    toolchain::StringTable::Offset Name = {};
    toolchain::StringTable::Offset Type = {};
    toolchain::StringTable::Offset Attributes = {};

    // Defaults to the empty string offset.
    toolchain::StringTable::Offset Features = {};
  } Offsets;
...
struct InfosShard {
  const toolchain::StringTable *Strings;
  toolchain::ArrayRef<Info> Infos;

  toolchain::StringLiteral NamePrefix = "";
};
...
class Context {
  toolchain::SmallVector<InfosShard> BuiltinShards;

  toolchain::SmallVector<InfosShard> TargetShards;
  toolchain::SmallVector<InfosShard> AuxTargetShards;
...
public:
  ...
  /// toolchain::AbstractCallSites for details). If so, add the index to the
  /// callback callee argument and the callback payload arguments.
  bool performsCallback(unsigned ID,
                        toolchain::SmallVectorImpl<int> &Encoding) const;
  ...
  /// prefix.
  static bool isBuiltinFunc(toolchain::StringRef Name);
...
bool evaluateRequiredTargetFeatures(
    toolchain::StringRef RequiredFatures,
    const toolchain::StringMap<bool> &TargetFetureMap);
