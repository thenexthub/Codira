...
#ifndef LANGUAGE_CORE_BASIC_OBJCRUNTIME_H
#define LANGUAGE_CORE_BASIC_OBJCRUNTIME_H
...
public:
  ...
  bool isLegacyDispatchDefaultForArch(toolchain::Triple::ArchType Arch) {
    ...
    if (getKind() == GNUstep) {
      ...
      return Arch != toolchain::Triple::x86_64;
  ...
  friend toolchain::hash_code hash_value(const ObjCRuntime &OCR) {
    return toolchain::hash_combine(OCR.getKind(), OCR.getVersion());
  }

  template <typename HasherT, toolchain::endianness Endianness>
  friend void addHash(toolchain::HashBuilder<HasherT, Endianness> &HBuilder,
...
#endif // LANGUAGE_CORE_BASIC_OBJCRUNTIME_H
