...
//
/// \file
/// Defines the language::Core::LangOptions interface.
//
//===----------------------------------------------------------------------===//

#ifndef LANGUAGE_CORE_BASIC_LANGOPTIONS_H
#define LANGUAGE_CORE_BASIC_LANGOPTIONS_H
...
/// Triple Environment enum in toolchain::Triple. The ordering is enforced in
...
public:
  using Visibility = language::Core::Visibility;
  using RoundingMode = toolchain::RoundingMode;
  ...
  using MSVtorDispMode = language::Core::MSVtorDispMode;
...
public:
  ...
  language::Core::ObjCRuntime ObjCRuntime;
  ...
  toolchain::dxbc::RootSignatureVersion HLSLRootSigVer =
      toolchain::dxbc::RootSignatureVersion::V1_1;
  ...
  static void
  setLangDefaults(LangOptions &Opts, Language Lang, const toolchain::Triple &T,
...
public:
  ...
  explicit FPOptions(const LangOptions &LO) {
    ...
    if (getFPContractMode() == LangOptions::FPM_On &&
        getRoundingMode() == toolchain::RoundingMode::Dynamic &&
  ...
  bool isFPConstrained() const {
    return getRoundingMode() != toolchain::RoundingMode::NearestTiesToEven ||
...
struct AtomicOptions {
  ...
  bool getOption(AtomicOptionKind Kind) const {
    ...
    toolchain_unreachable("Invalid AtomicOptionKind");
  }
...
#endif // LANGUAGE_CORE_BASIC_LANGOPTIONS_H
