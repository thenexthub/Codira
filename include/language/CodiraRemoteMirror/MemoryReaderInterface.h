...
#ifndef LANGUAGE_REFLECTION_MEMORYREADERINTERFACE_H
#define LANGUAGE_REFLECTION_MEMORYREADERINTERFACE_H
...
// in the system library, so we use 'language_addr_t'.
typedef uint64_t language_addr_t;

/// The following callbacks provide "reader_context", which is an opaque context
/// provided in language_reflection_createReflectionContext.
...
typedef const void *(*ReadBytesFunction)(void *reader_context, language_addr_t address,
                                         uint64_t size,
                                         void **outFreeContext);
...
typedef uint64_t (*GetStringLengthFunction)(void *reader_context,
                                            language_addr_t address);

/// Get the address of a symbol in the target address space.
///
/// \returns true if the lookup was successful.
typedef language_addr_t (*GetSymbolAddressFunction)(void *reader_context,
                                                 const char *name,
...
typedef enum {
  ...
  /// buffer (the size of a target pointer, not a language_addr_t) which should be
...
#endif //LANGUAGE_REFLECTION_MEMORYREADERINTERFACE_H
