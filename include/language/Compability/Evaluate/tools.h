//===-- language/Compability/Evaluate/tools.h --------------------------*- C++ -*-===//
//
...
#ifndef LANGUAGE_COMPABILITY_EVALUATE_TOOLS_H_
#define LANGUAGE_COMPABILITY_EVALUATE_TOOLS_H_
...
template <typename T>
bool isStructureComponent(const language::Compability::evaluate::Expr<T> &expr) {
  if (auto dataRef{ExtractDataRef(expr, /*intoSubstring=*/false)}) {
    const language::Compability::evaluate::DataRef *ref{&*dataRef};
    return std::holds_alternative<language::Compability::evaluate::Component>(ref->u);
  }
