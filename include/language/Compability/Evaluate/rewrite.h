//===-- language/Compability/Evaluate/rewrite.h ------------------------*- C++ -*-===//
//
...
//
//===----------------------------------------------------------------------===//
#ifndef LANGUAGE_COMPABILITY_EVALUATE_REWRITE_H_
#define LANGUAGE_COMPABILITY_EVALUATE_REWRITE_H_
...
template <typename Rewriter> struct Mutator {
  ...
  template <typename T, typename U = toolchain::remove_cvref_t<T>>
  U operator()(T &&x) {
