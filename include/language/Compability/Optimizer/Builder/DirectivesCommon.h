...
#ifndef LANGUAGE_COMPABILITY_OPTIMIZER_BUILDER_DIRECTIVESCOMMON_H_
#define LANGUAGE_COMPABILITY_OPTIMIZER_BUILDER_DIRECTIVESCOMMON_H_
...
struct AddrAndBoundsInfo {
  ...
  void dump(toolchain::raw_ostream &os) {
...
inline AddrAndBoundsInfo getDataOperandBaseAddr(fir::FirOpBuilder &builder,
  ...
  if (!symAddr)
    toolchain::report_fatal_error("could not retrieve symbol address");
...
genBoundsOpFromBoxChar(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  auto [extent, stride] = [&]() -> ExtentAndStride {
    if (info.isPresent) {
      toolchain::SmallVector<mlir::Type> resTypes = {idxTy, idxTy};
            ...
            .genThen([&]() {
              mlir::Value box =
                  !fir::isBoxAddress(info.addr.getType())
                      ? info.addr
                      : fir::LoadOp::create(builder, loc, info.addr);
              toolchain::SmallVector<mlir::Value> boundValues =
