...
#ifndef LANGUAGE_COMPABILITY_OPTIMIZER_BUILDER_FACTORY_H
#define LANGUAGE_COMPABILITY_OPTIMIZER_BUILDER_FACTORY_H
...
constexpr toolchain::StringRef attrFortranArrayOffsets() {
...
inline toolchain::SmallVector<mlir::Value> getExtents(mlir::Value shapeVal) {
  if (shapeVal)
...
inline toolchain::SmallVector<mlir::Value> getOrigins(mlir::Value shapeVal) {
  if (shapeVal)
    if (auto *shapeOp = shapeVal.getDefiningOp()) {
      if (auto shOp = mlir::dyn_cast<fir::ShapeShiftOp>(shapeOp)) {
...
originateIndices(mlir::Location loc, B &builder, mlir::Type memTy,
  ...
  if (origins.empty()) {
    ...
    for (auto i : toolchain::enumerate(indices)) {
