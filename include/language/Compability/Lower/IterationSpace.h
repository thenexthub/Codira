...
#ifndef LANGUAGE_COMPABILITY_LOWER_ITERATIONSPACE_H
#define LANGUAGE_COMPABILITY_LOWER_ITERATIONSPACE_H
...
public:
  ...
  explicit IterationSpace(const IterationSpace &from,
                          toolchain::ArrayRef<mlir::Value> prefix,
                          toolchain::ArrayRef<mlir::Value> suffix)
      : inArg(from.inArg), outRes(from.outRes), element(from.element) {
  ...
  toolchain::SmallVector<mlir::Value> iterVec() const { return indices; }
  ...
  void setIndexValues(toolchain::ArrayRef<mlir::Value> vals) {
...
protected:
  ...
  toolchain::SmallVector<A> stack;

  // Map each mask expression back to the temporary holding the initial
  // evaluation results.
  toolchain::DenseMap<FrontEndExpr, GenerateElementalArrayFunc> vmap;
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const ImplicitIterSpace &);
...
class ImplicitIterSpace
    : public StackableConstructExpr<toolchain::SmallVector<FrontEndExpr>> {
public:
  using Base = StackableConstructExpr<toolchain::SmallVector<FrontEndExpr>>;
  using FrontEndMaskExpr = FrontEndExpr;

  friend toolchain::raw_ostream &operator<<(toolchain::raw_ostream &,
  ...
  toolchain::SmallVector<FrontEndMaskExpr> getExprs() const {
    toolchain::SmallVector<FrontEndMaskExpr> maskList = getMasks()[0];
  ...
  toolchain::SmallVector<toolchain::SmallVector<FrontEndMaskExpr>> &getMasks() {
    return stack;
  }
  const toolchain::SmallVector<toolchain::SmallVector<FrontEndMaskExpr>> &
  getMasks() const {
    return stack;
  }
...
private:
  toolchain::DenseMap<FrontEndExpr,
                 std::tuple<mlir::Value, mlir::Value, mlir::Value>>
      maskVarMap;
};

class ExplicitIterSpace;
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const ExplicitIterSpace &);
...
namespace toolchain {
template <>
struct DenseMapInfo<language::Compability::lower::ExplicitSpaceArrayBases> {
  static inline language::Compability::lower::ExplicitSpaceArrayBases getEmptyKey() {
    return reinterpret_cast<language::Compability::lower::FrontEndSymbol>(~0);
  }
  static inline language::Compability::lower::ExplicitSpaceArrayBases getTombstoneKey() {
    return reinterpret_cast<language::Compability::lower::FrontEndSymbol>(~0 - 1);
  }
  static unsigned
  getHashValue(const language::Compability::lower::ExplicitSpaceArrayBases &v) {
    return language::Compability::lower::getHashValue(v);
  }
  static bool isEqual(const language::Compability::lower::ExplicitSpaceArrayBases &lhs,
                      const language::Compability::lower::ExplicitSpaceArrayBases &rhs) {
...
public:
  ...
  using ConcurrentSpec =
      std::pair<toolchain::SmallVector<IterSpaceDim>, FrontEndExpr>;
  ...
  toolchain::SmallVector<FrontEndSymbol> collectAllSymbols();
  ...
  void setInnerArgs(toolchain::ArrayRef<mlir::BlockArgument> args) {
    innerArgs.clear();
    for (auto &arg : args)
  ...
  toolchain::SmallVector<mlir::Type> innerArgTypes() const {
    toolchain::SmallVector<mlir::Type> result;
  ...
  mlir::Value findArgumentOfLoad(fir::ArrayLoadOp load) {
    ...
    toolchain_unreachable("array load argument not found");
  }

  size_t argPosition(mlir::Value arg) {
    for (auto i : toolchain::enumerate(innerArgs))
      if (arg == i.value())
        return i.index();
    toolchain_unreachable("inner argument value was not found");
  }
  ...
  void appendLoops(const toolchain::SmallVector<fir::DoLoopOp> &loops) {
    loopStack.push_back(loops);
  }

  void clearLoops() { loopStack.clear(); }

  toolchain::SmallVector<toolchain::SmallVector<fir::DoLoopOp>> getLoopStack() const {
    return loopStack;
  }
...
private:
  ...
  toolchain::SmallVector<toolchain::SmallVector<FrontEndSymbol>> symbolStack;
  toolchain::SmallVector<std::optional<ArrayBases>> lhsBases;
  toolchain::SmallVector<toolchain::SmallVector<ArrayBases>> rhsBases;
  toolchain::DenseMap<ArrayBases, fir::ArrayLoadOp> loadBindings;

  // Stack of lambdas to create the loop nest.
  toolchain::SmallVector<std::function<void()>> ccLoopNest;

  // Assignment statement context (inside the loop nest).
  StatementContext stmtCtx;
  toolchain::SmallVector<mlir::Value> innerArgs;
  toolchain::SmallVector<mlir::Value> initialArgs;
...
template <typename A>
bool symbolSetsIntersect(toolchain::ArrayRef<FrontEndSymbol> ctrlSet,
                         const A &exprSyms) {
  for (const auto &sym : exprSyms)
    if (toolchain::is_contained(ctrlSet, &sym.get()))
      return true;
  return false;
}
...
template <typename A>
bool symbolsIntersectSubscripts(toolchain::ArrayRef<FrontEndSymbol> ctrlSet,
                                const A &subscripts) {
