...
#ifndef LANGUAGE_COMPABILITY_LOWER_SYMBOLMAP_H
#define LANGUAGE_COMPABILITY_LOWER_SYMBOLMAP_H
...
struct SymbolBox;
class SymMap;
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const SymbolBox &symMap);
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const SymMap &symMap);
...
struct SymbolBox : public fir::details::matcher<SymbolBox> {
  ...
  friend toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os,
...
class SymMap {
public:
  using AcDoVar = toolchain::StringRef;
...
private:
  ...
  toolchain::SmallVector<toolchain::DenseMap<const semantics::Symbol *, SymbolBox>>
      symbolMapStack;

  // Implied DO induction variables are not represented as Se::Symbol in
  // Ev::Expr. Keep the variable markers in their own stack.
  toolchain::SmallVector<std::pair<AcDoVar, mlir::Value>> impliedDoStack;
};
