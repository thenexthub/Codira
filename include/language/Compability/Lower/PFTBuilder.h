...
#ifndef LANGUAGE_COMPABILITY_LOWER_PFTBUILDER_H
#define LANGUAGE_COMPABILITY_LOWER_PFTBUILDER_H
...
namespace language::Compability::lower::pft {
...
struct Evaluation : EvaluationVariant {
  ...
  toolchain::SmallVector<const parser::CompilerDirective *, 1> dirs;
...
struct Variable {
  ...
  struct AggregateStore {
    AggregateStore(Interval &&interval,
                   const language::Compability::semantics::Symbol &namingSym,
                   bool isGlobal = false)
  ...
  explicit Variable(const language::Compability::semantics::Symbol &sym, bool global = false,
                    int depth = 0)
  ...
  bool isGlobal() const {
    return language::Compability::common::visit([](const auto &x) { return x.isGlobal(); },
                                  var);
  }
  ...
  bool isModuleOrSubmoduleVariable() const {
    ...
    return scope && scope->kind() == language::Compability::semantics::Scope::Kind::Module;
  }

  const language::Compability::semantics::Scope *getOwningScope() const {
    return language::Compability::common::visit(
        common::visitors{
  ...
  void setAlias(std::size_t offset) {
    if (auto *s = std::get_if<Nominal>(&var)) {
      ...
      toolchain_unreachable("not a nominal var");
    }
  }
...
using ScopeVariableListMap =
    std::map<const language::Compability::semantics::Scope *, VariableList>;

/// Find or create an ordered list of the equivalence sets and variables that
/// appear in \p scope. The result is cached in \p map.
const VariableList &getScopeVariableList(const language::Compability::semantics::Scope &scope,
...
VariableList getScopeVariableList(const language::Compability::semantics::Scope &scope);
...
VariableList getDependentVariableList(const language::Compability::semantics::Symbol &);
...
struct FunctionLikeUnit : public ProgramUnit {
  ...
  const semantics::Symbol *getMainProgramSymbol() const {
    if (!isMainProgram()) {
      toolchain::report_fatal_error("call only on main program.");
    }
  ...
  void setHostAssociatedSymbols(
      const toolchain::SetVector<const semantics::Symbol *> &symbols) {
  ...
  const language::Compability::semantics::Scope &getScope() const { return *scope; }
  ...
  toolchain::SmallVector<std::pair<const semantics::Symbol *, Evaluation *>, 1>
...
struct ModuleLikeUnit : public ProgramUnit {
  ...
  const language::Compability::semantics::Scope &getScope() const;
...
struct BlockDataUnit : public ProgramUnit {
  ...
  const language::Compability::semantics::Scope &symTab; // symbol table
};
...
void dumpPFT(toolchain::raw_ostream &outputStream, const pft::Program &pft);
} // namespace language::Compability::lower
