...
//
//===----------------------------------------------------------------------===//
#ifndef LANGUAGE_COMPABILITY_LOWER_OPENMP_CLAUSES_H
#define LANGUAGE_COMPABILITY_LOWER_OPENMP_CLAUSES_H
...
struct IdTyTemplate {
  ...
  language::Compability::semantics::Symbol *symbol;
...
namespace tomp::type {
template <>
struct ObjectT<language::Compability::lower::omp::IdTyTemplate<language::Compability::lower::omp::ExprTy>,
               language::Compability::lower::omp::ExprTy> {
  using IdTy = language::Compability::lower::omp::IdTyTemplate<language::Compability::lower::omp::ExprTy>;
  using ExprTy = language::Compability::lower::omp::ExprTy;

  IdTy id() const { return identity; }
  language::Compability::semantics::Symbol *sym() const { return identity.symbol; }
...
namespace std {
template <>
struct hash<language::Compability::lower::omp::IdTy> {
  size_t operator()(const language::Compability::lower::omp::IdTy &id) const {
...
template <
    ...
    typename ElemTy = typename toolchain::remove_cvref_t<ContainerTy>::value_type,
    typename ResultTy = std::invoke_result_t<FunctionTy, ElemTy>>
List<ResultTy> makeList(ContainerTy &&container, FunctionTy &&func) {
  List<ResultTy> v;
  toolchain::transform(container, std::back_inserter(v), func);
  return v;
}
...
struct CancellationConstructType {
  ...
  toolchain::omp::CancellationConstructType v;
};
struct Depobj {
...
Clause makeClause(toolchain::omp::Clause id, Specific &&specific,
                  parser::CharBlock source = {}) {
