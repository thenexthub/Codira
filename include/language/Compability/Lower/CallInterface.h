...
#ifndef LANGUAGE_COMPABILITY_LOWER_CALLINTERFACE_H
#define LANGUAGE_COMPABILITY_LOWER_CALLINTERFACE_H

#include "language/Compability/Common/reference.h"
#include "language/Compability/Evaluate/characteristics.h"
...
struct PassedEntityTypes<CallerInterface> {
  using FortranEntity = const language::Compability::evaluate::ActualArgument *;
  using FirValue = int;
};
template <>
...
public:
  ...
  struct PassedEntity {
    ...
    bool testTKR(language::Compability::common::IgnoreTKR flag) const;
    ...
    const language::Compability::evaluate::characteristics::DummyArgument *characteristics =
        nullptr;
  };
  ...
  toolchain::SmallVector<mlir::Type> getResultType() const;

  /// Return a container of Symbol/ActualArgument* and how they must
  /// be plugged with the mlir::func::FuncOp.
  toolchain::ArrayRef<PassedEntity> getPassedArguments() const {
...
protected:
  CallInterface(language::Compability::lower::AbstractConverter &c) : converter{c} {}
  /// CRTP handle.
  ...
  toolchain::SmallVector<FirPlaceHolder> outputs;
  toolchain::SmallVector<FirPlaceHolder> inputs;
  mlir::func::FuncOp func;
  toolchain::SmallVector<PassedEntity> passedArguments;
...
public:
  CallerInterface(const language::Compability::evaluate::ProcedureRef &p,
  ...
  const language::Compability::evaluate::ProcedureRef &getCallDescription() const {
    return procRef;
  }
  ...
  const language::Compability::semantics::SubprogramDetails *getInterfaceDetails() const;
  ...
  const language::Compability::evaluate::ProcedureDesignator *getIfIndirectCall() const;

  /// Get the input vector once it is complete.
  toolchain::ArrayRef<mlir::Value> getInputs() const {
    if (!verifyActualInputs())
      toolchain::report_fatal_error("lowered arguments are incomplete");
    return actualInputs;
  }
  ...
  /// crash.
  const language::Compability::semantics::Symbol &getResultSymbol() const;
  ...
  mlir::Type getHostAssociatedTy() const {
    toolchain_unreachable("getting host associated type in CallerInterface");
  }
...
private:
  ...
  toolchain::SmallVector<mlir::Value> actualInputs;
};
...
public:
  CalleeInterface(language::Compability::lower::pft::FunctionLikeUnit &f,
                  language::Compability::lower::AbstractConverter &c)
  ...
  language::Compability::lower::pft::FunctionLikeUnit &getCallDescription() const {
    return funit;
  }
...
mlir::FunctionType
translateSignature(const language::Compability::evaluate::ProcedureDesignator &,
...
/// already declared.
mlir::func::FuncOp
getOrDeclareFunction(const language::Compability::evaluate::ProcedureDesignator &,
                     language::Compability::lower::AbstractConverter &);
...
/// functions).
mlir::Type getDummyProcedureType(const language::Compability::semantics::Symbol &dummyProc,
                                 language::Compability::lower::AbstractConverter &);
