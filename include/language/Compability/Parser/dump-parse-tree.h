//===-- language/Compability/Parser/dump-parse-tree.h ------------------*- C++ -*-===//
//
...
#ifndef LANGUAGE_COMPABILITY_PARSER_DUMP_PARSE_TREE_H_
#define LANGUAGE_COMPABILITY_PARSER_DUMP_PARSE_TREE_H_
...
public:
  explicit ParseTreeDumper(toolchain::raw_ostream &out,
...
#include "toolchain/Frontend/OpenACC/ACC.inc"
  ...
  static std::string GetNodeName(const toolchain::acc::DefaultValue &x) {
    return toolchain::Twine(
        "toolchain::acc::DefaultValue = ", toolchain::acc::getOpenACCDefaultValueName(x))
        .str();
  }
  ...
  static std::string GetNodeName(const toolchain::acc::Directive &x) {
    return toolchain::Twine(
        "toolchain::acc::Directive = ", toolchain::acc::getOpenACCDirectiveName(x))
        .str();
  }
  ...
  static std::string GetNodeName(const toolchain::omp::Directive &x) {
    return toolchain::Twine("toolchain::omp::Directive = ",
        toolchain::omp::getOpenMPDirectiveName(x, toolchain::omp::FallbackVersion))
        .str();
  }
...
#include "toolchain/Frontend/OpenMP/OMP.inc"
  ...
  static std::string GetNodeName(const toolchain::omp::Clause &x) {
    return toolchain::Twine(
        "toolchain::omp::Clause = ", toolchain::omp::getOpenMPClauseName(x))
        .str();
  }
...
template <typename T>
toolchain::raw_ostream &DumpTree(toolchain::raw_ostream &out, const T &x,
    const AnalyzedObjectsAsFortran *asFortran = nullptr) {
