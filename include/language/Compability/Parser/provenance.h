//===-- language/Compability/Parser/provenance.h -----------------------*- C++ -*-===//
//
...
#ifndef LANGUAGE_COMPABILITY_PARSER_PROVENANCE_H_
#define LANGUAGE_COMPABILITY_PARSER_PROVENANCE_H_
...
public:
  ...
  const SourceFile *OpenPath(std::string path, toolchain::raw_ostream &error);
  const SourceFile *Open(std::string path, toolchain::raw_ostream &error,
      std::optional<std::string> &&prependPath = std::nullopt);
  const SourceFile *ReadStandardInput(toolchain::raw_ostream &error);
  ...
  void EmitMessage(toolchain::raw_ostream &, const std::optional<ProvenanceRange> &,
      const std::string &message, const std::string &prefix,
      toolchain::raw_ostream::Colors color, bool echoSourceLine = false) const;
