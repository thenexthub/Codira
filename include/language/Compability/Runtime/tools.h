//===-- language/Compability-rt/runtime/tools.h ------------------------*- C++ -*-===//
//
...
template <int KIND> struct StoreIntegerAt {
  RT_API_ATTRS void operator()(const language::Compability::runtime::Descriptor &result,
      std::size_t at, std::int64_t value) const {
    *result.ZeroBasedIndexedElement<language::Compability::runtime::CppTypeFor<
        language::Compability::common::TypeCategory::Integer, KIND>>(at) = value;
  }
};
...
template <int KIND> struct StoreFloatingPointAt {
  RT_API_ATTRS void operator()(const language::Compability::runtime::Descriptor &result,
    ...
    *result.ZeroBasedIndexedElement<language::Compability::runtime::CppTypeFor<
        language::Compability::common::TypeCategory::Real, KIND>>(at) = value;
  }
};
...
static inline RT_API_ATTRS language::Compability::common::optional<std::int64_t> GetInt64Safe(
    const char *p, std::size_t bytes, Terminator &terminator) {
  switch (bytes) {
  case 1:
    return *reinterpret_cast<const CppTypeFor<TypeCategory::Integer, 1> *>(p);
...
language::Compability::common::optional<
