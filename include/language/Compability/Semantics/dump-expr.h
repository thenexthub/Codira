...
#ifndef LANGUAGE_COMPABILITY_SEMANTICS_DUMPEXPR_H
#define LANGUAGE_COMPABILITY_SEMANTICS_DUMPEXPR_H
...
public:
  DumpEvaluateExpr() : outs_(toolchain::errs()) {}
  DumpEvaluateExpr(toolchain::raw_ostream &str) : outs_(str) {}

  template <typename A> static void Dump(const A &x) {
    DumpEvaluateExpr{}.Show(x);
  }
  template <typename A>
  static void Dump(toolchain::raw_ostream &stream, const A &x) {
...
#undef DUMP_EXPR_SHOW_TYPE
  ...
  void Print(toolchain::Twine s);
  void Indent(toolchain::StringRef s);
  void Outdent();

  toolchain::raw_ostream &outs_;
  unsigned level_{0};
};
