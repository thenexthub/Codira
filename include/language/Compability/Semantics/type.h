//===-- language/Compability/Semantics/type.h --------------------------*- C++ -*-===//
//
...
#ifndef LANGUAGE_COMPABILITY_SEMANTICS_TYPE_H_
#define LANGUAGE_COMPABILITY_SEMANTICS_TYPE_H_
...
namespace language::Compability::evaluate { // avoid including all of Evaluate/tools.h
template <typename T>
...
private:
  ...
  friend toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const Bound &);
};
...
private:
  ...
  MaybeIntExpr expr_;
  friend toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const ParamValue &);
};
...
private:
  ...
  friend toolchain::raw_ostream &operator<<(
      toolchain::raw_ostream &os, const IntrinsicTypeSpec &x);
};
...
private:
  ParamValue length_;
  friend toolchain::raw_ostream &operator<<(
      toolchain::raw_ostream &os, const CharacterTypeSpec &x);
};
...
private:
  ...
  Bound lb_;
  Bound ub_;
  friend toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const ShapeSpec &);
};
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const ArraySpec &);
...
private:
  ...
  friend toolchain::raw_ostream &operator<<(
      toolchain::raw_ostream &, const DerivedTypeSpec &);
};
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &, const DeclTypeSpec &);
