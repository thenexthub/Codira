...
#ifndef LANGUAGE_COMPABILITY_SEMANTICS_OPENMP_MODIFIERS_H_
#define LANGUAGE_COMPABILITY_SEMANTICS_OPENMP_MODIFIERS_H_
...
using OmpClauses =
    common::EnumSet<toolchain::omp::Clause, toolchain::omp::Clause_enumSize>;
...
struct OmpModifierDescriptor {
  ...
  unsigned since(toolchain::omp::Clause id) const;

  const toolchain::StringRef name;
...
const OmpModifierDescriptor &OmpGetDescriptor(const UnionTy &modifier) {
  return common::visit(
      [](auto &&m) -> decltype(auto) {
        using SpecificTy = toolchain::remove_cvref_t<decltype(m)>;
...
template <typename SpecificTy, typename UnionTy>
toolchain::iterator_range<OmpSpecificModifierIterator<SpecificTy>>
OmpGetRepeatableModifier(const std::optional<std::list<UnionTy>> &modifiers) {
  ...
  return toolchain::iterator_range(
...
template <typename SpecificTy, typename UnionTy>
language::Compability::parser::CharBlock OmpGetModifierSource(
    const std::optional<std::list<UnionTy>> &modifiers,
    const SpecificTy *specific) {
  if (!modifiers || !specific) {
    return language::Compability::parser::CharBlock{};
  }
  for (auto &m : *modifiers) {
  ...
  toolchain_unreachable("`specific` must be a member of `modifiers`");
}
...
bool verifyVersions(const std::optional<std::list<UnionTy>> &modifiers,
    toolchain::omp::Clause id, parser::CharBlock clauseSource,
...
bool verifyIfRequired(const SpecificTy *,
  ...
  present = present && toolchain::any_of(*modifiers, [](auto &&m) {
        ...
        common::visit(
            [&](auto &&m) {
              ...
              if (props.test(OmpProperty::Ultimate)) {
                ...
                toolchain::StringRef where{isPre ? "last" : "first"};
...
template <typename ClauseTy>
bool OmpVerifyModifiers(const ClauseTy &clause, toolchain::omp::Clause id,
    parser::CharBlock clauseSource, SemanticsContext &semaCtx) {
  ...
  return toolchain::all_of(results, [](bool x) { return x; });
}
} // namespace language::Compability::semantics
