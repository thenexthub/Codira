//===--- TypeRefBuilder.h - Codira Type Reference Builder --------*- C++ -*-===//
//
...
#ifndef LANGUAGE_REFLECTION_TYPEREFBUILDER_H
#define LANGUAGE_REFLECTION_TYPEREFBUILDER_H

#include "language/Demangling/ManglingFlavor.h"
#include "language/Remote/ExternalTypeRefCache.h"
#include "language/Remote/MetadataReader.h"
#include "language/RemoteInspection/DescriptorFinder.h"
#include "language/RemoteInspection/MetadataSourceBuilder.h"
...
#include "language/RemoteInspection/TypeRefs.def"
...
public:
  ...
  using BuiltSubstitutionMap = toolchain::DenseMap<DepthAndIndex, const TypeRef *>;
...
public:
  ...
  struct ReflectionTypeDescriptorFinder
      : public language::reflection::DescriptorFinder {
  ...
  private:
    ...
    toolchain::DenseSet<size_t> ProcessedReflectionInfoIndexes;
  ...
  const BoundGenericTypeRef *reconstructParentsOfBoundGenericType(
      ...
      const toolchain::ArrayRef<const TypeRef *> &args) {
  ...
  const BoundGenericTypeRef *
  createBoundGenericType(const BuiltTypeDecl &builtTypeDecl,
                         toolchain::ArrayRef<const TypeRef *> args,
                         const TypeRef *parent) {
    if (!builtTypeDecl)
      return nullptr;
  ...
  resolveOpaqueType(NodePointer opaqueDescriptor,
                    toolchain::ArrayRef<toolchain::ArrayRef<const TypeRef *>> genericArgs,
                    unsigned ordinal) {
  ...
  const TupleTypeRef *createTupleType(toolchain::ArrayRef<const TypeRef *> elements,
                                      toolchain::ArrayRef<StringRef> labels) {
    std::vector<std::string> labelsVec(labels.begin(), labels.end());
    return TupleTypeRef::create(*this, elements, labelsVec);
  }

  const TypeRef *createPackType(toolchain::ArrayRef<const TypeRef *> elements) {
    return PackTypeRef::create(*this, elements);
  }

  const TypeRef *createSILPackType(toolchain::ArrayRef<const TypeRef *> elements,
  ...
  const FunctionTypeRef *createFunctionType(
      toolchain::ArrayRef<remote::FunctionParam<const TypeRef *>> params,
  ...
  const FunctionTypeRef *createImplFunctionType(
      ...
      toolchain::ArrayRef<Demangle::ImplFunctionParam<const TypeRef *>> params,
      toolchain::ArrayRef<Demangle::ImplFunctionYield<const TypeRef *>> yields,
      toolchain::ArrayRef<Demangle::ImplFunctionResult<const TypeRef *>> results,
    ...
    case Demangle::ImplFunctionRepresentation::Closure:
      funcFlags = funcFlags.withConvention(FunctionMetadataConvention::Codira);
      break;
    ...
    auto result = createTupleType({}, toolchain::ArrayRef<toolchain::StringRef>());
  ...
  BuiltType createProtocolTypeFromDecl(BuiltProtocolDecl protocol) {
    if (protocol->second) {
      return toolchain::cast<TypeRef>(createObjCProtocolType(protocol->first));
    } else {
      return toolchain::cast<TypeRef>(
  ...
  const ConstrainedExistentialTypeRef *createConstrainedExistentialType(
      const TypeRef *base, toolchain::ArrayRef<BuiltRequirement> constraints,
      toolchain::ArrayRef<BuiltInverseRequirement> InverseRequirements) {
    // FIXME: Handle inverse requirements.
    auto *baseProto = toolchain::dyn_cast<ProtocolCompositionTypeRef>(base);
    if (!baseProto)
      return nullptr;
  ...
  void pushGenericParams(
      toolchain::ArrayRef<std::pair<unsigned, unsigned>> parameterPacks) {}
...
#include "language/AST/ReferenceStorage.def"
  ...
  const SILBoxTypeWithLayoutTypeRef *createSILBoxTypeWithLayout(
      const toolchain::SmallVectorImpl<BuiltSILBoxField> &Fields,
      const toolchain::SmallVectorImpl<BuiltSubstitution> &Substitutions,
      const toolchain::SmallVectorImpl<BuiltRequirement> &Requirements,
      toolchain::ArrayRef<BuiltInverseRequirement> InverseRequirements) {
    // FIXME: Handle inverse requirements.
  ...
  createGenericSignature(toolchain::ArrayRef<BuiltType> builtParams,
                         toolchain::ArrayRef<BuiltRequirement> requirements) {
    std::vector<BuiltGenericTypeParam> params;
    for (auto &builtParam : builtParams) {
      auto *genericRef =
          toolchain::dyn_cast<GenericTypeParameterTypeRef>(builtParam);
      if (!genericRef)
        return nullptr;
  ...
  createSubstitutionMap(BuiltGenericSignature sig,
                        toolchain::ArrayRef<BuiltType> replacements) {
...
private:
  ...
  /// in.
  toolchain::SmallVector<DescriptorFinder *, 2> getDescriptorFinders() {
    if (EDF)
      return {EDF, &RDF};
    return {&RDF};
  }
  ...
  uint64_t getMultiPayloadEnumPointerMask() {
    ...
    if (!multiPayloadEnumPointerMask.has_value()) {
      ...
      multiPayloadEnumPointerMask = OpaqueIntVariableReader(
          "_language_debug_multiPayloadEnumPointerSpareBitsMask", pointerSize);
    }
    if (!multiPayloadEnumPointerMask.has_value()) {
      if (pointerSize == sizeof(void *)) {
        // Most reflection tools run on the target machine,
        // in which case, they use the same configuration:
        multiPayloadEnumPointerMask = _language_abi_CodiraSpareBitsMask;
      } else if (pointerSize == 4) {
        // All 32-bit platforms are the same, so this is always correct.
        multiPayloadEnumPointerMask = LANGUAGE_ABI_ARM_LANGUAGE_SPARE_BITS_MASK;
      } else {
        // This is not always correct.  But we can't do any better?
        multiPayloadEnumPointerMask = LANGUAGE_ABI_ARM64_LANGUAGE_SPARE_BITS_MASK;
      }
    }
    ...
    std::string constructFullyQualifiedNameFromContextChain(
      ...
      for (size_t i = 0; i < contextNameChain.size(); ++i) {
        ...
        if (nextContextIsAnonymous && !currentContextIsAnonymous) {
          ...
          toolchain_unreachable("Exhausted possibilities.");
        }
      }
...
#endif // LANGUAGE_REFLECTION_TYPEREFBUILDER_H
