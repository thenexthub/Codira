//===-- toolchain/BinaryFormat/MachO.h - The MachO file format -------*- C++ -*-===//
//
...
#ifndef TOOLCHAIN_BINARYFORMAT_MACHO_H
#define TOOLCHAIN_BINARYFORMAT_MACHO_H
...
enum : uint32_t {
  // Constants for the "magic" field in toolchain::MachO::mach_header and
  // toolchain::MachO::mach_header_64
...
enum HeaderFileType {
  // Constants for the "filetype" field in toolchain::MachO::mach_header and
  // toolchain::MachO::mach_header_64
  MH_OBJECT = 0x1u,
  MH_EXECUTE = 0x2u,
  MH_FVMLIB = 0x3u,
...
enum {
  // Constant bits for the "flags" field in toolchain::MachO::mach_header and
  // toolchain::MachO::mach_header_64
...
enum : uint32_t {
  // Flags for the "cmd" field in toolchain::MachO::load_command
...
#include "toolchain/BinaryFormat/MachO.def"
};

#undef HANDLE_LOAD_COMMAND

enum : uint32_t {
  // Constant bits for the "flags" field in toolchain::MachO::segment_command
  SG_HIGHVM = 0x1u,
  SG_FVMLIB = 0x2u,
  SG_NORELOC = 0x4u,
  ...
  // Constant masks for the "flags" field in toolchain::MachO::section and
  // toolchain::MachO::section_64
...
enum SectionType : uint32_t {
  // Constant masks for the "flags[7:0]" field in toolchain::MachO::section and
  // toolchain::MachO::section_64 (mask "flags" with SECTION_TYPE)
...
enum : uint32_t {
  // Constant masks for the "flags[31:24]" field in toolchain::MachO::section and
  // toolchain::MachO::section_64 (mask "flags" with SECTION_ATTRIBUTES_USR)
  ...
  // Constant masks for the "flags[23:8]" field in toolchain::MachO::section and
  // toolchain::MachO::section_64 (mask "flags" with SECTION_ATTRIBUTES_SYS)
...
enum {
  // Constant masks for the "n_type" field in toolchain::MachO::nlist and
  // toolchain::MachO::nlist_64
  N_STAB = 0xe0,
  N_PEXT = 0x10,
  N_TYPE = 0x0e,
  N_EXT = 0x01
...
enum NListType : uint8_t {
  // Constants for the "n_type & N_TYPE" toolchain::MachO::nlist and
  // toolchain::MachO::nlist_64
  N_UNDF = 0x0u,
  N_ABS = 0x2u,
  N_SECT = 0xeu,
...
enum SectionOrdinal {
  // Constants for the "n_sect" field in toolchain::MachO::nlist and
  // toolchain::MachO::nlist_64
  NO_SECT = 0u,
  MAX_SECT = 0xffu
};
...
enum {
  // Constant masks for the "n_desc" field in toolchain::MachO::nlist and
  // toolchain::MachO::nlist_64
...
enum StabType {
  // Constant values for the "n_type" field in toolchain::MachO::nlist and
  // toolchain::MachO::nlist_64 when "(n_type & N_STAB) != 0"
  N_GSYM = 0x20u,
  N_FNAME = 0x22u,
  N_FUN = 0x24u,
  N_STSYM = 0x26u,
...
enum : uint32_t {
  ...
  // toolchain::MachO::relocation_info structure when r_extern is 0.
  R_ABS = 0,

  // Constant bits for the r_address field in an
  // toolchain::MachO::relocation_info structure.
  R_SCATTERED = 0x80000000
};

enum RelocationInfoType {
  // Constant values for the r_type field in an
  // toolchain::MachO::relocation_info or toolchain::MachO::scattered_relocation_info
  // structure.
...
enum { TOOL_CLANG = 1, TOOL_LANGUAGE = 2, TOOL_LD = 3, TOOL_LLD = 4 };
