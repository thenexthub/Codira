...
public:
  ...
  bool isBigObj() const {
    ...
    if (CS32)
      return true;
    toolchain_unreachable("COFFSymbolRef points to nothing!");
  }
...
struct coff_tls_directory {
  ...
  void setAlignment(uint32_t Align) {
    ...
    if (Align) {
      assert(toolchain::isPowerOf2_32(Align) && "alignment is not a power of 2");
      assert(toolchain::Log2_32(Align) <= 13 && "alignment requested is too large");
      AlignBits = (toolchain::Log2_32(Align) + 1) << 20;
    }
    Characteristics =
...
public:
  ...
  uint16_t getMachine() const {
    ...
    toolchain_unreachable("no COFF header!");
  }
...
public:
  ...
  size_t getSymbolTableEntrySize() const {
    ...
    toolchain_unreachable("null symbol table pointer!");
  }
