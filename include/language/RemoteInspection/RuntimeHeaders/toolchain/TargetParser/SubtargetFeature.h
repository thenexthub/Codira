//=== toolchain/TargetParser/SubtargetFeature.h - CPU characteristics-*- C++ -*-===//
//
...
#ifndef TOOLCHAIN_TARGETPARSER_SUBTARGETFEATURE_H
#define TOOLCHAIN_TARGETPARSER_SUBTARGETFEATURE_H
...
public:
  ...
  bool any() const {
    return toolchain::any_of(Bits, [](uint64_t I) { return I != 0; });
  }
  bool none() const { return !any(); }
  size_t count() const {
    size_t Count = 0;
    for (auto B : Bits)
      Count += toolchain::countPopulation(B);
    return Count;
  }
...
#endif // TOOLCHAIN_TARGETPARSER_SUBTARGETFEATURE_H
