//===--- DeclContext.h - Codira Language Context ASTs ------------*- C++ -*-===//
//
...
#ifndef LANGUAGE_DECLCONTEXT_H
#define LANGUAGE_DECLCONTEXT_H
...
using DeclID = toolchain::PointerEmbeddedInt<unsigned, 31>;
}
...
class alignas(1 << DeclContextAlignInBits) DeclContext
  ...
  toolchain::PointerIntPair<DeclContext*, 3, ASTHierarchy> ParentAndKind;
  ...
  friend struct ::toolchain::CastInfo;
  
  // See language/AST/Decl.h
  ...
  static ASTHierarchy getASTHierarchyFromKind(DeclContextKind Kind) {
    ...
    toolchain_unreachable("Unhandled DeclContextKind");
  }
...
public:
  ...
  TOOLCHAIN_READONLY
  bool isPackageContext() const; // see language/AST/Module.h

  /// isModuleContext - Return true if this is a subclass of Module.
  TOOLCHAIN_READONLY
  bool isModuleContext() const; // see language/AST/Module.h

  /// \returns true if this is a context with module-wide scope, e.g. a module
  /// or a source file.
  TOOLCHAIN_READONLY
  bool isModuleScopeContext() const; // see language/AST/Module.h
  ...
  /// `.codeinterface` file.
  TOOLCHAIN_READONLY
  bool isInCodirainterface() const;
  ...
  LANGUAGE_DEBUG_DUMPER(dumpContext());
  unsigned printContext(toolchain::raw_ostream &OS, unsigned indent = 0,
                        bool onlyAPartialLine = false) const;
  
  // Some Decls are DeclContexts, but not all. See language/AST/Decl.h
...
class IterableDeclContext {
  ...
  mutable toolchain::PointerIntPair<Decl *, 1, bool> FirstDeclAndLazyMembers;

  /// The last declaration in this context, used for efficient insertion,
  /// along with the kind of iterable declaration context.
  mutable toolchain::PointerIntPair<Decl *, 1, IterableDeclContextKind>
    LastDeclAndKind;
...
public:
  ...
  /// category, it may be a Codira extension instead.
...
void simple_display(toolchain::raw_ostream &out, const ParamT *dc) {
  if (dc)
...
void simple_display(toolchain::raw_ostream &out, const IterableDeclContext *idc);
...
namespace toolchain {
  ...
  struct CastInfo<::language::DeclContext, FromTy, std::enable_if_t<is_simple_type<FromTy>::value>>
      : public CastIsPossible<::language::DeclContext, FromTy>,
        public DefaultDoCastIfPossible<::language::DeclContext *, FromTy,
                                       CastInfo<::language::DeclContext, FromTy>> {
    static inline ::language::DeclContext *castFailed() { return nullptr; }

    static inline ::language::DeclContext *doCast(const FromTy &val) {
      return ::language::DeclContext::castDeclToDeclContext(val);
    }
  };

  template<class FromTy>
  struct CastInfo<::language::GenericContext, FromTy, std::enable_if_t<is_simple_type<FromTy>::value>>
      : public CastIsPossible<::language::GenericContext, FromTy>,
        public DefaultDoCastIfPossible<::language::GenericContext *, FromTy,
                                       CastInfo<::language::GenericContext, FromTy>> {
    static inline ::language::GenericContext *castFailed() { return nullptr; }

    static inline ::language::GenericContext *doCast(const FromTy &val) {
      auto *genCtxt = val->getAsGenericContext();
      assert(genCtxt);
      return const_cast<::language::GenericContext *>(genCtxt);
    }
  };

  template<class FromTy>
  struct CastInfo<::language::IterableDeclContext, FromTy, std::enable_if_t<is_simple_type<FromTy>::value>>
      : public CastIsPossible<::language::IterableDeclContext, FromTy>,
        public DefaultDoCastIfPossible<::language::IterableDeclContext *, FromTy,
                                       CastInfo<::language::IterableDeclContext, FromTy>> {
    static inline ::language::IterableDeclContext *castFailed() { return nullptr; }

    static inline ::language::IterableDeclContext *doCast(const FromTy &val) {
      return ::language::IterableDeclContext::castDeclToIterableDeclContext(val);
    }
  };
}
