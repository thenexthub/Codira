...
#ifndef LANGUAGE_REMOTE_METADATAREADER_H
#define LANGUAGE_REMOTE_METADATAREADER_H
...
using FunctionParam = language::Demangle::FunctionParam<BuiltType>;

template <typename BuilderType>
using TypeDecoder = language::Demangle::TypeDecoder<BuilderType>;
...
private:
  ...
  struct DenseMapInfoTypeCacheKey {
    ...
    using StoredPointerInfo = toolchain::DenseMapInfo<RemoteAddress>;
    ...
    static unsigned getHashValue(const Pair &PairVal) {
      return toolchain::hash_combine(PairVal.first, PairVal.second);
    }
  ...
  toolchain::DenseMap<std::pair<RemoteAddress, bool>, BuiltType,
  ...
  toolchain::DenseMap<RemoteAddress, OwnedMetadataRef> MetadataCache;
  ...
  toolchain::DenseMap<RemoteAddress, OwnedContextDescriptorRef>
  ...
  toolchain::DenseMap<RemoteAddress, OwnedShapeRef> ShapeCache;
...
public:
  ...
  Demangle::NodePointer demangle(RemoteRef<char> mangledName,
    ...
    language::Demangle::NodePointer result;
    switch (kind) {
  ...
  decodeMangledType(NodePointer Node) {
    return language::Demangle::decodeMangledType(Builder, Node);
  }

  /// Get the remote process's language_isaMask.
  ...
  readInstanceStartFromClassMetadata(RemoteAddress MetadataAddress) {
    ...
    if (Runtime::ObjCInterop) {
      ...
      // All language class instances start with an isa pointer,
  ...
  readMetadataAndValueErrorExistential(RemoteAddress ExistentialAddress) {
    ...
    if (auto ClassMeta = dyn_cast<TargetClassMetadata>(Meta)) {
      if (ClassMeta->isPureObjC()) {
        ...
        if (readObjCClassName(*MetadataAddress, ObjCClassName)) {
          if (ObjCClassName == "__CodiraNativeNSError")
            isObjC = true;
          else
            isBridged = true;
        }
    ...
    // conformance pointer for Codira.Error.
  ...
  typename Resolver::Result readProtocol(
    ...
    if (Runtime::ObjCInterop) {
      ...
      if (ProtocolAddress.isObjC()) {
        ...
        // If this is a Codira-defined protocol, demangle it.
        if (NameStr.starts_with("_TtP")) {
          auto Demangled = dem.demangleSymbol(NameStr);
          if (!Demangled)
            return resolver.failure();

          // FIXME: This appears in _language_buildDemanglingForMetadata().
          ...
          return resolver.codeProtocol(Demangled);
        }
    ...
    // Codira-native protocol.
    auto Demangled = readDemanglingForContextDescriptor(
        stripSignedPointer({ProtocolAddress.getCodiraProtocol()}), dem);
    if (!Demangled)
      return resolver.failure();
  ...
  readTypeFromMetadata(RemoteAddress MetadataAddress,
    ...
    case MetadataKind::Tuple: {
      ...
      std::vector<toolchain::StringRef> labels;
      std::string::size_type end, start = 0;
      while (true) {
        end = labelStr.find(' ', start);
        if (end == std::string::npos)
          break;
        labels.push_back(toolchain::StringRef(labelStr.data() + start, end - start));
        start = end + 1;
      }
    ...
    case MetadataKind::Existential: {
      ...
      struct ProtocolResolver {
        ...
        BuiltProtocolDecl languageProtocol(Demangle::Node *node) {
    ...
    for (auto &req : Sig.getRequirements()) {
      ...
      case GenericRequirementKind::Protocol: {
        ...
        struct ProtocolReferenceResolver {
          ...
          BuiltType languageProtocol(Demangle::Node *node) {
  ...
  readMetadataFromInstance(RemoteAddress objectAddress) {
    ...
    language_unreachable("Unhandled IsaEncodingKind in switch.");
  }
    ...
    if (!subclass->hasResilientSuperclass()) {
      ...
      bounds = forTypeReference<ClassMetadataBounds>(
          ...
          [&](MetadataRef metadata) -> std::optional<ClassMetadataBounds> {
            ...
            return cls->getClassBoundsAsCodiraSuperclass();
          },
...
private:
  ...
  Demangle::NodePointer adoptAnonymousContextName(
    ...
    language::Demangle::NodePointer nameChild = mangledNode->getChild(1);
  ...
  buildContextDescriptorMangling(ContextDescriptorRef descriptor,
    ...
    case ContextDescriptorKind::Extension: {
      ...
      struct ProtocolResolver {
        ...
        Result languageProtocol(Demangle::Node *node) {
          return node;
        }
      ...
      if (!requirements.empty()) {
        ...
        for (const auto &req : requirements) {
          ...
          language::Demangle::NodePointer subject =
          ...
          case GenericRequirementKind::BaseClass: {
            ...
            language::Demangle::NodePointer type =
  ...
  using TargetClassMetadataObjCInterop =
      language::TargetClassMetadata<Runtime,
...
#   define tryFindAndReadSymbol(dest, symbolName) do {       \
    ...
    // doesn't use it.
    if (auto maskAddress = Reader->getSymbolAddress("language_isaMask")) {
...
namespace toolchain {
  template<typename T>
  struct simplify_type<language::remote::RemoteRef<T>> {
    using SimpleType = const T *;
    static SimpleType
    getSimplifiedValue(language::remote::RemoteRef<T> value) {
      return value.getLocalBuffer();
    }
  };
}

#endif // LANGUAGE_REFLECTION_READER_H
