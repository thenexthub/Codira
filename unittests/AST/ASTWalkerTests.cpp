...
struct WalkAction {
  ...
  public:
    ...
    /*implicit*/ Payload(toolchain::StringRef name) : isRepr(false), name(name) {
      assert(!name.empty());
    }

    toolchain::StringRef getName() const { return isRepr ? StringRef() : name; }
...
class WalkVerifier {
  ...
  public:
    ...
    static Component consume(toolchain::raw_svector_ostream &os, TypeRepr *repr,
                             internal::WalkCheckpoint checkpoint) {
    ...
    static void print(toolchain::raw_ostream &os, TypeRepr *repr,
                      internal::WalkCheckpoint checkpoint) {
      switch (checkpoint) {
  ...
  private:
    static void printArgument(toolchain::raw_ostream &os, TypeRepr *repr) {
  ...
  toolchain::SmallString<128> walkStr;
  toolchain::SmallVector<Component, 20> components;

public:
  WalkVerifier(TypeRepr *repr, QualifiedIdentTypeReprWalkingScheme scheme)
      : repr(repr), scheme(scheme) {
    toolchain::raw_svector_ostream os(this->walkStr);
  ...
  toolchain::StringRef getWalkString() const { return walkStr; }
  ...
  void testWalkActionsOn(toolchain::StringRef name) const {
    for (auto &action : {
...
private:
  void testWalkAction(internal::WalkAction action) const {
    toolchain::SmallString<128> actualStr;
    {
      toolchain::raw_svector_ostream os(actualStr);
      internal::Walker walker(
          scheme, action,
          [&](TypeRepr *repr, internal::WalkCheckpoint checkpoint) {
  ...
  toolchain::StringRef getStringForStopAction(internal::WalkCheckpoint checkpoint,
                                         toolchain::StringRef argumentStr) const {
  ...
  std::string getStringForSkipChildrenAction(toolchain::StringRef argumentStr,
...
TEST(ASTWalker, QualifiedIdentTypeReprWalkingScheme) {
  ...
  const auto makeDeclNameRef = [&ctx](toolchain::StringRef str) {
