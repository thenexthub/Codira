...
#ifndef LANGUAGE_STDLIB_SINGLE_THREADED_CONCURRENCY
...
struct SingleThreadedValue {
  ...
  friend toolchain::hash_code hash_value(const SingleThreadedValue &value) {
    return toolchain::hash_value(value.key);
  }
};
...
struct MultiThreadedKey {
  int threadNumber;
  int n;
  friend toolchain::hash_code hash_value(const MultiThreadedKey &value) {
    return toolchain::hash_combine(value.threadNumber, value.n);
  }
};
...
struct MultiThreadedValue {
  ...
  friend toolchain::hash_code hash_value(const MultiThreadedValue &value) {
    return toolchain::hash_combine(value.threadNumber, value.n);
  }
};
...
#endif // !LANGUAGE_STDLIB_SINGLE_THREADED_CONCURRENCY
