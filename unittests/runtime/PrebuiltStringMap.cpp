...
#include "language/Runtime/PrebuiltStringMap.h"
...
TEST(PrebuiltStringMapTest, PrebuiltStringMap) {
  auto testOnce = [&](unsigned testEntryCount) {
    ...
    using Map = language::PrebuiltStringMap<const char *, unsigned, stringIsNull>;
...
TEST(PrebuiltStringMapTest, PrebuiltAuxDataImplicitStringMap) {
  auto testOnce = [&](unsigned testEntryCount) {
    ...
    using Map = language::PrebuiltAuxDataImplicitStringMap<uint64_t, int32_t>;
