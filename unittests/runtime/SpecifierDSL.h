...
class Demangler {
  ...
  NodePointer node(Kind kind, toolchain::ArrayRef<NodePointer> children) {
  ...
  NodePointer nodeWithText(Kind kind, toolchain::StringRef string) {
  ...
  NodePointer demangleContext(const ContextDescriptor *context) {
    ...
    language_unreachable("unknown context");
  }
...
#undef CASE
    language_unreachable("unknown type specifier");
  }
...
static void addGenericRequirements(AnyObjectBuilder &builder,
                                   toolchain::ArrayRef<ReqtSpecifier> reqts) {
  for (auto &reqt : reqts) {
    if (auto sameType = reqt.dyn_cast<SameTypeReqtSpecifier>()) {
      ...
      language_unreachable("unknown requirement specifier");
    }
  }
}
