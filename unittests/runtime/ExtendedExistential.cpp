...
static const Metadata *metadata(unsigned n) {
  auto result = language_getTupleTypeMetadata(MetadataState::Abstract,
                                           TupleTypeFlags().withNumElements(0),
                                           nullptr, nullptr, nullptr).Value;
  while (n--) {
    result = language_getFunctionTypeMetadata(FunctionTypeFlags().withNumParameters(0),
...
static void addExistentialShape(AnyObjectBuilder &builder,
  ...
  for (auto &spec : specifiers) {
    if (auto sig = spec.dyn_cast<GenSigShapeSpecifier>()) {
      ...
      language_unreachable("bad shape specifier");
    }
  }
...
TEST(TestExtendedExistential, shapeUniquing) {
  ...
  auto result0 = language_getExtendedExistentialTypeShape(shape0);
  ...
  auto result2 = language_getExtendedExistentialTypeShape(shape0);
  EXPECT_EQ(result2, result0);

  auto result3 = language_getExtendedExistentialTypeShape(shape1);
  EXPECT_EQ(result3, result0);

  auto result4 = shape1->UniqueCache.get()->load(std::memory_order_acquire);
  EXPECT_EQ(result4, result0);

  auto result5 = language_getExtendedExistentialTypeShape(shape2);
  EXPECT_EQ(result5, &shape2->LocalCopy);
  EXPECT_NE(result5, result0);

  auto result6 = language_getExtendedExistentialTypeShape(shape3);
...
TEST(TestExtendedExistential, nullaryMetadata) {
  ...
  auto metadata1 = language_getExtendedExistentialTypeMetadata_unique(shape1, nullptr);
  EXPECT_EQ(metadata1->Shape, shape1);

  auto metadata2 = language_getExtendedExistentialTypeMetadata_unique(shape1, nullptr);
...
TEST(TestExtendedExistential, unaryMetadata) {
  ...
  auto metadata1 = language_getExtendedExistentialTypeMetadata_unique(shape1, args1);
  EXPECT_EQ(metadata1->Shape, shape1);
  EXPECT_EQ(metadata1->getGeneralizationArguments()[0], args1[0]);

  const void *args2[] = { metadata(0) };
  auto metadata2 = language_getExtendedExistentialTypeMetadata_unique(shape1, args2);
  EXPECT_EQ(metadata2->Shape, shape1);
  EXPECT_EQ(metadata2->getGeneralizationArguments()[0], args2[0]);
  EXPECT_EQ(metadata2, metadata1);

  const void *args3[] = { metadata(1) };
  auto metadata3 = language_getExtendedExistentialTypeMetadata_unique(shape1, args3);
  ...
  auto metadata4 = language_getExtendedExistentialTypeMetadata_unique(shape1, args4);
  ...
  auto metadata5 = language_getExtendedExistentialTypeMetadata_unique(shape2, args5);
...
TEST(TestExtendedExistential, overrideValueWitnesses) {
  ...
  auto metadata0 = language_getExtendedExistentialTypeMetadata_unique(shape0, nullptr);
...
TEST(TestExtendedExistential, defaultOpaqueValueWitnesses) {
  ...
  auto metadata2 = language_getExtendedExistentialTypeMetadata_unique(shape2, nullptr);
