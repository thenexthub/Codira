...
protected:
  ...
  struct Object {
    ...
    Object(size_t requiredAlignment) : requiredAlignment(requiredAlignment) {
      assert(toolchain::isPowerOf2_64(requiredAlignment));
    }
...
public:
  ...
  void addBytes(toolchain::StringRef value);
  void addBytes(const void *data, size_t length);

  /// Add a string of ('\0'-terminated) bytes.
  void addString(toolchain::StringRef value);

  /// Add an indirect reference to a string of '\0'-terminated bytes.
  void addRelativeReferenceToString(toolchain::StringRef value,
...
inline void AnyObjectBuilder::padToAlignment(size_t alignment) {
  ...
  assert(toolchain::isPowerOf2_64(alignment));
...
inline void AnyObjectBuilder::addBytes(toolchain::StringRef value) {
  addBytes(value.begin(), value.size());
}
inline void AnyObjectBuilder::addString(toolchain::StringRef value) {
  addBytes(value);
  add8(0);
}
...
inline void
AnyObjectBuilder::addRelativeReferenceToString(toolchain::StringRef value,
                                               size_t alignment,
                                               size_t addend) {
