...
public:
  ...
  static std::string tokToString(language::tok T) {
    switch (T) {
  #define KEYWORD(X) \
    case language::tok::kw_##X: return "kw_" #X; break;
  #define PUNCTUATOR(X, Y) \
    case language::tok::X: return #X; break;
#define POUND(X, Y) \
    case language::tok::pound_##X: return "pound_" #X; break;
  ...
  std::vector<Token> parseAndGetSplitTokens(unsigned BufID) {
    language::ParserUnit PU(SM, SourceFileKind::Main, BufID, LangOpts, "unknown");
  ...
  std::vector<Token> tokenize(unsigned BufID, const std::vector<Token> &SplitTokens = {}) {
    return language::tokenize(LangOpts, 
                           SM, 
                           BufID, 
                           /* Offset = */ 0,
                           /* EndOffset = */ 0,
