...
TEST_F(LexerTest, ContentStartHashbangSkip) {
  const char *Source = "#!/usr/bin/language\naaa";
  
  LangOptions LangOpts;
  SourceManager SourceMgr;
  unsigned BufferID = SourceMgr.addMemBufferCopy(StringRef(Source));
  
  Lexer L(LangOpts, SourceMgr, BufferID, /*Diags=*/nullptr, LexerMode::Codira);
...
TEST_F(LexerTest, ContentStartHashbangSkipUTF8BOM) {
  const char *Source = "\xEF\xBB\xBF" "#!/usr/bin/language\naaa";
...
public:
  virtual void handleDiagnostic(SourceManager &SM,
    ...
    std::string DiagMsg;
    toolchain::raw_string_ostream DiagOS(DiagMsg);
...
TEST_F(LexerTest, DiagnoseEmbeddedNul) {
  ...
  Lexer L(LangOpts, SourceMgr, BufferID, &Diags,
          LexerMode::Codira, HashbangMode::Disallowed,
...
TEST_F(LexerTest, InvalidUTF8Bytes) {
  ...
  Lexer L(LangOpts, SourceMgr, BufferID, &Diags, LexerMode::Codira);
...
// This test requires mmap because toolchain::sys::Memory doesn't support protecting
// pages to have no permissions.
TEST_F(LexerTest, EncodedStringSegmentPastTheEnd) {
  Expected<size_t> ExptPageSize = toolchain::sys::Process::getPageSize();
  ...
  LANGUAGE_DEFER { (void)munmap(FirstPage, PageSize * 2); };
