...
TEST(PartitionUtilsTest, Join1) {
  ...
  Partition p1 = Partition::separateRegions(fakeLoc, toolchain::ArrayRef(data1),
                                            historyFactory.get());
  ...
  Partition p2 = Partition::separateRegions(fakeLoc, toolchain::ArrayRef(data1),
                                            historyFactory.get());
  {
...
TEST(PartitionUtilsTest, Join2) {
  ...
  Partition p2 = Partition::separateRegions(fakeLoc, toolchain::ArrayRef(data2),
                                            historyFactory.get());
  {
    MockedPartitionOpEvaluator eval(p2, factory, transferringOpToStateMap);
