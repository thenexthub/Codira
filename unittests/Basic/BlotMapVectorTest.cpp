...
#define DEBUG_TYPE "language-blot-map-vector-test"
...
private:
  void dump(StringRef Name) const {
    std::string Addr = "0x";
    Addr += toolchain::utohexstr(uintptr_t(this));
    std::string ValueAddr = "0x";
    ValueAddr += toolchain::utohexstr(uintptr_t(Value.get()));
    TOOLCHAIN_DEBUG(toolchain::errs() << Name << " <Tester Addr:" << Addr
...
void CtorTesterSet::dumpLiveTesters() const {
  for (auto *Tester : Constructed) {
    if (Tester->isIgnorableTester())
      continue;
    toolchain::SmallString<64> Hex;
    std::string Addr = toolchain::utohexstr(uintptr_t(Tester));
    toolchain::errs() << "<Tester Addr:" << Addr << " Value:" << Tester->getValue()
                 << ">\n";
  }
}
...
bool CtorTesterSet::numLiveTesters() const {
  return toolchain::count_if(Constructed, [](CtorTester *T) -> bool {
    assert(T);
    return !T->isIgnorableTester();
  });
}
...
public:
  ...
  ~BlotMapVectorTest() override {
    ...
    TOOLCHAIN_DEBUG(toolchain::errs() << "Destroying Fixture\n");
...
typedef ::testing::Types<
    ...
    BlotMapVector<CtorTester, CtorTester,
                  toolchain::DenseMap<CtorTester, size_t, CtorTesterMapInfo>>,
    SmallBlotMapVector<uint32_t, uint32_t, 4>,
    SmallBlotMapVector<uint32_t *, uint32_t *, 4>,
    SmallBlotMapVector<
        CtorTester, CtorTester, 4,
        toolchain::SmallDenseMap<CtorTester, unsigned, 4, CtorTesterMapInfo>>>
...
TEST(BlotMapVectorCustomTest, FindAsTest) {
  BlotMapVector<unsigned, unsigned,
                toolchain::DenseMap<unsigned, unsigned, TestBlotMapVectorInfo>> map;
  map[0] = 1;
  map[1] = 2;
  map[2] = 3;
...
TEST(BlotMapVectorCustomTest, SmallBlotMapVectorGrowTest) {
  SmallBlotMapVector<unsigned, unsigned, 32,
                     toolchain::SmallDenseMap<unsigned, unsigned, 32,
