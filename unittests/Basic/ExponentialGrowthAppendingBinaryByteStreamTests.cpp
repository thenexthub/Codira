...
#include "toolchain/Support/BinaryStreamReader.h"
#include "toolchain/Support/BinaryStreamWriter.h"
#include "toolchain/Testing/Support/Error.h"
...
TEST_F(ExponentialGrowthAppendingBinaryByteStreamTest, WriteAtInvalidOffset) {
  ...
  auto Test = toolchain::ArrayRef(InputData).take_front(4);
...
TEST_F(ExponentialGrowthAppendingBinaryByteStreamTest, WriteIntoMiddle) {
  ...
  auto InitialDataRef = toolchain::ArrayRef(InitialData);
...
TEST_F(ExponentialGrowthAppendingBinaryByteStreamTest, WriteInteger) {
  ...
  auto AfterFirstInsertRef = toolchain::ArrayRef(AfterFirstInsert);
  ...
  std::vector<uint8_t> AfterSecondInsert = {'H', 'e', 'l', 'l', 'o', ' ',
                                            'w', 'o', 'r', 'l'};
  auto AfterSecondInsertRef = toolchain::ArrayRef(AfterSecondInsert);
