...
TEST_F(EditTest, DiagsAfterEdit) {
  ...
  reset(Consumer);
  replaceText(DocName, findOffset("fn foo", Contents)+strlen("fn foo"), 0, "()", Consumer);
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS, const Token &Tok) {
...
std::string dumpStrings(const Collection &Annotations) {
  std::string tmp;
  toolchain::raw_string_ostream OS(tmp);
...
void checkTokens(toolchain::ArrayRef<Token> Annotations, toolchain::ArrayRef<const char *> Expected) {
  ...
  for (unsigned i = 0, e = Annotations.size(); i != e; ++i) {
    std::string tok;
    {
      toolchain::raw_string_ostream OS(tok);
      OS << Annotations[i];
    }
...
TEST_F(EditTest, AnnotationsAfterOpen) {
  ...
  checkTokens(Consumer.Annotations, {
    "[off=22 len=3 source.lang.code.ref.struct system]",
    "[off=59 len=3 source.lang.code.ref.var.instance]",
  });
...
TEST_F(EditTest, AnnotationsRangeShiftingAfterEditInsertStart) {
  ...
  checkTokens(Consumer.Annotations, {
    "[off=22 len=3 source.lang.code.ref.struct system]",
    "[off=60 len=3 source.lang.code.ref.var.instance]",
    // Removed, touches edit.
    // "[off=70 len=3 source.lang.code.ref.var.instance]",
    // Shifted by 1
    "[off=81 len=3 source.lang.code.ref.var.instance]",
  });
...
TEST_F(EditTest, AnnotationsRangeShiftingAfterEditReplaceStart) {
  ...
  checkTokens(Consumer.Annotations, {
    ...
    // Shifted 3-1 = 2
    "[off=82 len=3 source.lang.code.ref.var.instance]",
  });
...
TEST_F(EditTest, AnnotationsRangeShiftingAfterEditDeleteStart) {
  ...
  checkTokens(Consumer.Annotations, {
    ...
    // Shifted -2
    "[off=78 len=3 source.lang.code.ref.var.instance]",
  });
...
TEST_F(EditTest, AnnotationsRangeShiftingAfterEditReplaceEnd) {
  ...
  checkTokens(Consumer.Annotations, {
    ...
    "[off=80 len=3 source.lang.code.ref.var.instance]",
  });
...
TEST_F(EditTest, AnnotationsRangeShiftingAfterEditDeleteEnd) {
  ...
  checkTokens(Consumer.Annotations, {
    ...
    // Shifted -1
    "[off=79 len=3 source.lang.code.ref.var.instance]",
  });
