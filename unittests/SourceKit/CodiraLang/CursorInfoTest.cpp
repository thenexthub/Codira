...
public:
  ...
  LangSupport &getLang() { return getContext().getCodiraLangSupport(); }
  ...
  getCursor(const char *DocName, unsigned Offset, ArrayRef<const char *> CArgs,
    ...
    if (expired)
      toolchain::report_fatal_error("check took too long");
    return TestInfo;
  }
...
TEST_F(CursorInfoTest, CursorInfoMustWaitDueTokenRace) {
  ...
  // the wrong AST.
  setNeedsSema(true);
  open(DocName, Contents, toolchain::ArrayRef(Args));
  // Change 'foo' to 'fog' by replacing the last character.
  replaceText(DocName, FooOffs + 2, 1, "g");
  replaceText(DocName, FooRefOffs + 2, 1, "g");
...
TEST_F(CursorInfoTest, CursorInfoCancelsPreviousRequest) {
  ...
  const char *SlowDocName = "slow.code";
  const char *SlowContents = "fn foo(x: Invalid1, y: Invalid2) {\n"
                             "    x / y / x / y / x / y / x / y\n"
                             "}\n";
  auto SlowOffset = findOffset("x", SlowContents);
  const char *Args[] = {"-parse-as-library"};
  std::vector<const char *> ArgsForSlow = toolchain::ArrayRef(Args).vec();
  ArgsForSlow.push_back(SlowDocName);

  const char *FastDocName = "fast.code";
  const char *FastContents = "fn bar() {\n"
                             "    let foo = 123\n"
                             "}\n";
  auto FastOffset = findOffset("foo", FastContents);
  std::vector<const char *> ArgsForFast = toolchain::ArrayRef(Args).vec();
  ArgsForFast.push_back(FastDocName);

  open(SlowDocName, SlowContents, toolchain::ArrayRef(Args));
  open(FastDocName, FastContents, toolchain::ArrayRef(Args));
  ...
  if (expired)
    toolchain::report_fatal_error("Did not receive a response for the first request");
}
