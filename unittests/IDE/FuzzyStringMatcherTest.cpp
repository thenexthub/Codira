...
using FuzzyStringMatcher = language::ide::FuzzyStringMatcher;
...
TEST(FuzzyStringMatcher, UnicodeMatching) {
  ...
  EXPECT_TRUE(FuzzyStringMatcher(LANGUAGE_UTF8("\u2602a\U0002000Bz"))
                  .matchesCandidate(LANGUAGE_UTF8("\u2602A\U0002000BZ")));

  // Same-order combining marks.
  EXPECT_TRUE(FuzzyStringMatcher(LANGUAGE_UTF8("a\u0323\u0307"))
                  .matchesCandidate(LANGUAGE_UTF8("A\u0323\u0307")));

  // FIXME: Canonical equivalence. These should be the same.
  EXPECT_FALSE(FuzzyStringMatcher(LANGUAGE_UTF8("a\u0307\u0323"))
                   .matchesCandidate(LANGUAGE_UTF8("A\u0323\u0307")));
  EXPECT_FALSE(FuzzyStringMatcher(LANGUAGE_UTF8("a\u00C5"))
                   .matchesCandidate(LANGUAGE_UTF8("A\u030A")));

  // FIXME: Compatibility equivalence.  It would be good to make these the same
  // too, since we're fuzzy matching.
  EXPECT_FALSE(FuzzyStringMatcher(LANGUAGE_UTF8("fi"))
                   .matchesCandidate(LANGUAGE_UTF8("\uFB01")));
  EXPECT_FALSE(FuzzyStringMatcher(LANGUAGE_UTF8("25"))
                   .matchesCandidate(LANGUAGE_UTF8("2\u2075")));

  // FIXME: Case-insensitivity in non-ASCII characters.
  EXPECT_FALSE(FuzzyStringMatcher(LANGUAGE_UTF8("\u00E0"))
                   .matchesCandidate(LANGUAGE_UTF8("\u00C0")));
  EXPECT_FALSE(FuzzyStringMatcher(LANGUAGE_UTF8("ss"))
                   .matchesCandidate(LANGUAGE_UTF8("\u00DF")));
}
