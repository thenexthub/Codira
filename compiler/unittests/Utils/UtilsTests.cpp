...
using namespace Codira::FloatFormat;
using namespace AST;
...
{
    std::string path = MakeTempPath("code_remove_dir_not_exist");
    EXPECT_FALSE(RemoveDirectoryRecursively(path));
}

TEST(UtilsTest, RemoveDirectoryRemoveFile)
{
    std::string tempFile = MakeTempPath("code_remove_dir_file") + ".txt";
...
{
    std::string base = MakeTempPath("code_remove_dir_success");
...
{
    std::string outside = MakeTempPath("code_remove_symlink_outside") + ".txt";
    std::string base = MakeTempPath("code_remove_dir_symlink_child");
...
{
    std::string base = MakeTempPath("code_remove_dir_real");
    std::string baseFile = FileUtil::JoinPath(base, "file.txt");
    EXPECT_EQ(0, CreateDirs(base + DIR_SEPARATOR));
    std::ofstream(baseFile) << "data";
    std::string linkPath = MakeTempPath("code_remove_dir_symlink_root");
...
{
    std::string path = GetDirPath("lalala.code");
    EXPECT_EQ(path, ".");
    path = GetDirPath("/usr/local/bin/lalala.code");
#ifdef _WIN32
...
{
    std::string ext = GetFileExtension("lalala.code");
    EXPECT_EQ(ext, "code");
    ext = GetFileExtension("/usr/local/bin/lalala.code");
    EXPECT_EQ(ext, "code");
...
{
    std::string name = GetFileNameWithoutExtension("main.code");
    EXPECT_EQ(name, "main");

    name = GetFileNameWithoutExtension("testfile");
    EXPECT_EQ(name, "testfile");

    std::string case3 = "/hahaha/yeyeye/test1.code";
    ...
    name = GetFileNameWithoutExtension("/some/path/.test3.code");
    EXPECT_EQ(name, ".test3");
}

TEST(UtilsTest, GetFileBase)
{
    std::string name = GetFileBase("../output/main.code");
    ...
    name = GetFileBase("../.codeo");
    EXPECT_EQ(name, "../.codeo");

    name = GetFileBase(".codeo");
    EXPECT_EQ(name, ".codeo");

    name = GetFileBase("../.test.codeo");
    EXPECT_EQ(name, "../.test");

    name = GetFileBase("main.code");
    EXPECT_EQ(name, "main");
...
{
    std::string name = GetFileName("main.code");
    EXPECT_EQ(name, "main.code");
...
{
    std::string filePath = GetProjectPath() + "/unittests/Utils/CodiraFiles/testpkg01.code";
...
{
    std::string filePath = GetProjectPath() + "/unittests/Utils/CodiraFiles/pkg01.code";
    std::string failedReason;
    auto content = ReadFileContent(filePath, failedReason);
    EXPECT_TRUE(content.has_value() && failedReason.empty());
    EXPECT_EQ(content->at(0), '/');
...
{
    std::string filePath = GetProjectPath() + "/unittests/Utils/CodiraFiles/emptyfile.code";
    std::string failedReason;
    auto content = ReadFileContent(filePath, failedReason);
