...
protected:
    void SetUp() override
    {
        ...
        srcPath = projectPath + "\\unittests\\Modules\\CodiraFiles\\";
        ...
        // Compile core mock.
        auto coreFile = srcPath + "coremock.code";
        ...
        // create modules dir.
        auto modulesName = FileUtil::JoinPath(instance->codiraHome, "modules");
        ...
        std::string coreCodeo = FileUtil::JoinPath(codiraModules, "std.core.codeo");
        if (FileUtil::FileExist(coreCodeo)) {
            FileUtil::Remove(coreCodeo);
        }
        FileUtil::WriteBufferToASTFile(coreCodeo, astData);
        instance->invocation.globalOptions.implicitPrelude = true;

        for (unsigned int i = 0; i < fileNames.size(); i++) {
            auto srcFile = srcPath + fileNames[i] + ".code";
            ...
            std::string astFile = packagePath + fileNames[i] + ".codeo";
...
// Test for package with only one codira file.
...
TEST_F(PackageTest, SingleFile_LoadCache_Package)
{
    ...
    for (unsigned int i = 0; i < fileNames.size(); i++) {
        instance = std::make_unique<TestCompilerInstance>(invocation, diag);
        instance->importManager.SetPackageCodeoCache(fileNames[i], allAstData[fileNames[i]]);
        instance->code = "import " + fileNames[i] + ".*";
        instance->Compile();
...
// Test for one package with multi codira files.
TEST_F(PackageTest, MultiFile_Package)
{
    ...
    std::vector<uint8_t> astData;
    instance->importManager.ExportAST(false, astData, *instance->GetSourcePackages()[0]);
    std::string astFile = packagePath + "MultiFile.codeo";
    ASSERT_TRUE(FileUtil::WriteBufferToASTFile(astFile, astData));
...
TEST_F(PackageTest, LSPImportFunctionWithNamedParam)
{
    ...
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->importManager.SetPackageCodeoCache("pkg", astData);
    instance->invocation.globalOptions.implicitPrelude = true;
    instance->code = R"(
        import pkg.*
        main() {
...
TEST_F(PackageTest, ConstNotExportInitializerForLSP)
{
    ...
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->importManager.SetPackageCodeoCache("initializer", astData);
    instance->invocation.globalOptions.implicitPrelude = true;
    instance->code = R"(
        import initializer.*
        main() {}
...
{
    Codira::ICE::TriggerPointSetter iceSetter(static_cast<int64_t>(Codira::ICE::UNITTEST_TP));
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->invocation.globalOptions.implicitPrelude = true;
    instance->code = R"(
        main() {}
...
TEST_F(PackageTest, CheckCodeoPathLegality)
{
    diag.ClearError();
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    auto import = MakeOwned<ImportSpec>();
    import->EnableAttr(Attribute::IMPLICIT_ADD);
    bool ret = instance->importManager.CheckCodeoPathLegality(import, "", "not found package name", false, false);
    ASSERT_EQ(ret, false);
...
TEST_F(PackageTest, IncrementalMergedSemanticUsage)
{
    ...
    using namespace Codira::Sema::Desugar::AfterTypeCheck;
...
TEST_F(PackageTest, LoadAnnotationTarget)
{
    ...
    auto loader = std::make_unique<ASTLoader>(std::move(data), pkg->fullPackageName, *instance->typeManager,
        *instance->importManager.codeoManager, instance->invocation.globalOptions);
...
TEST_F(PackageTest, LoadPackageFromCodeo)
{
    CompilerInvocation testInvocation;
#ifdef __x86_64__
    testInvocation.globalOptions.target.arch = Codira::Triple::ArchType::X86_64;
#else
    testInvocation.globalOptions.target.arch = Codira::Triple::ArchType::AARCH64;
#endif
#ifdef _WIN32
    testInvocation.globalOptions.target.os = Codira::Triple::OSType::WINDOWS;
#elif __unix__
    testInvocation.globalOptions.target.os = Codira::Triple::OSType::LINUX;
#endif
    ...
    std::unordered_map<std::string, std::string> fullPkgName2CodeoFileNames = {
        {"classdecl", "classdecl.codeo"},
        {"extenddecl", "extenddecl.codeo"},
        {"funcdecl", "funcdecl.codeo"},
        {"vardecl", "vardecl.codeo"},
        {"recorddecl", "recorddecl.codeo"},
    };
    std::unordered_map<std::string, Ptr<Package>> pkgName2PkgNodeMap;

    auto checkPkg = [this, &testIns, &pkgName2PkgNodeMap](const std::string& pkgName, const std::string& codeoFileName) {
        auto pkg = testIns->importManager.LoadPackageFromCodeo(pkgName, FileUtil::JoinPath(packagePath, codeoFileName));
    ...
    for (size_t i = 0; i < 2; ++i) {
        for (auto& [fullPkgName, codeoFileName] : std::as_const(fullPkgName2CodeoFileNames)) {
            checkPkg(fullPkgName, codeoFileName);
        }
    }
}
...
TEST_F(PackageTest, ImportOptFlag)
{
    ...
    for (auto fileName : fileNames) {
        ...
        auto srcFile =
            FileUtil::JoinPath(FileUtil::JoinPath(srcPath, "ImportOpt" + DIR_SEPARATOR + subDir), fileName + ".code");
        std::string failedReason;
        ...
        std::vector<uint8_t> astData;
        instance->importManager.ExportAST(false, astData, *instance->GetSourcePackages()[0]);
        std::string astFile = packagePath + fileName + ".codeo";
