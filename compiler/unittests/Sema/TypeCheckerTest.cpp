...
{
    instance->invocation.globalOptions.enableMacroInLSP = true;
    srcPath = projectPath + "/unittests/Sema/SemaCodiraFiles/";
    invocation.globalOptions.executablePath = projectPath + "/output/bin/";
    instance->compileOnePackageFromSrcFiles = true;

    instance->srcFilePaths = {srcPath + "UnableToInferGenericArgumentInTest.code"};
...
{
    srcPath = projectPath + "/unittests/Sema/SemaCodiraFiles/";
    std::string command = "codec " + srcPath + "AddClassTyInfoMacro.code --compile-macro -Woff all";
    ...
    instance->srcFilePaths = {srcPath + "NoDiagInLSPMacroCall.code"};
    invocation.globalOptions.outputMode = GlobalOptions::OutputMode::STATIC_LIB;
    invocation.globalOptions.enableCompileTest = true;
    instance->Compile(CompileStage::SEMA);
    EXPECT_EQ(diag.GetErrorCount(), 0);
...
{
    srcPath = projectPath + "/unittests/Sema/SemaCodiraFiles/";
    std::vector<std::string> argStrs = {"codec", "--compile-macro", "-Woff", "all", srcPath + "ModifyClassBuildFunc.code"};
    DiagnosticEngine driverDiag;
    std::unique_ptr<Driver> driver = std::make_unique<Driver>(argStrs, driverDiag, projectPath + "/output/bin/codec");
    ...
    instance->srcFilePaths = {srcPath + "NoDiagInLSPMacroCallNode.code"};
    invocation.globalOptions.outputMode = GlobalOptions::OutputMode::STATIC_LIB;
    invocation.globalOptions.enableCompileTest = true;
    instance->Compile(CompileStage::SEMA);
    EXPECT_EQ(diag.GetErrorCount(), 0);
...
{
#ifdef _WIN32
    srcPath = projectPath + "\\unittests\\Sema\\SemaCodiraFiles\\AssumptionTest";
#elif __unix__
    srcPath = projectPath + "/unittests/Sema/SemaCodiraFiles/AssumptionTest";
#endif
...
TEST_F(TypeCheckerTest, DISABLED_SpawnTest)
{
    ...
    srcPath = projectPath + "/unittests/Sema/SemaCodiraFiles/";
#endif
    auto srcFile = srcPath + "spawn.code";
