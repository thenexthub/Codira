...
{
    auto src = srcPath + "func_not_annotation.code";
    ...
    for (auto& decl : file->decls) {
        ...
        if (auto fd = AST::As<ASTKind::FUNC_DECL>(decl.get()); fd && macrocall) {
            ...
            // Get the original begin position of identifier "test" in func.code.
...
TEST_F(MacroTest, MacroCall_Complementatcion_For_LSP)
{
    ...
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->compileOnePackageFromSrcFiles = true;
    instance->srcFilePaths = {srcPath + "class.code"};
    instance->Compile(CompileStage::PARSE);
...
TEST_F(MacroTest, DISABLED_MacroCall_Check_For_LSP)
{
    ...
    defInstance->srcFilePaths = {definePath + "define.code", definePath + "define2.code"};
    defInstance->Compile();
    ...
    std::string astFile = definePath + "define.codeo";
    ASSERT_TRUE(FileUtil::WriteBufferToASTFile(astFile, astData));

    invocation.globalOptions.enableMacroInLSP = true;
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->compileOnePackageFromSrcFiles = true;
    instance->srcFilePaths = {srcPath + "func_arg.code"};
    instance->Compile(CompileStage::PARSE);
    ...
    std::function<VisitAction(Ptr<Node>)> visitPre2 = [&](Ptr<Node> curNode) -> VisitAction {
        ...
        if (curNode->astKind == ASTKind::MACRO_EXPAND_EXPR) {
            ...
            if (mee->invocation.identifier == "B1") {
#ifdef _WIN32
                EXPECT_EQ(path, "define\\define.code");
#else
                EXPECT_EQ(path, "define/define.code");
#endif
            }
...
#ifdef _WIN32
                EXPECT_EQ(path, "define\\define2.code");
#else
                EXPECT_EQ(path, "define/define2.code");
#endif
            }
...
{
    auto src = srcPath + "test_IfAvailable_LSP.code";
    invocation.globalOptions.enableMacroInLSP = true;
    invocation.globalOptions.enableParallelMacro = true;
    invocation.globalOptions.executablePath = projectPath + "/output/bin/";
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
...
{
    std::string command = "cd " + definePath + " && codec define_childMessage.code --compile-macro";
    int err = system(command.c_str());
    ASSERT_EQ(0, err);
    auto src = srcPath + "test_macroWithContext.code";
    invocation.globalOptions.enableMacroInLSP = true;
    invocation.globalOptions.enableParallelMacro = true;
    invocation.globalOptions.executablePath = projectPath + "/output/bin/";
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
...
{
    std::string command = "cd " + definePath + " && codec define_report.code --compile-macro";
    int err = system(command.c_str());
    ASSERT_EQ(0, err);
    err = system("echo $CODIRA_HOME && echo $LD_LIBRARY_PATH && echo $PATH");
    ASSERT_EQ(0, err);
    ...
    instance->srcFilePaths = {srcPath + "func_report.code"};
...
{
    std::string command = "cd " + definePath + " && codec define.code --compile-macro";
    int err = system(command.c_str());
    ASSERT_EQ(0, err);
 
    auto src = srcPath + "derive_enum.code";
    invocation.globalOptions.enableMacroInLSP = true;
    invocation.globalOptions.executablePath = projectPath + "/output/bin/";
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->compileOnePackageFromSrcFiles = true;
...
{
    std::string command = "cd " + definePath + " && codec define3.code --compile-macro";
    int err = system(command.c_str());
    ASSERT_EQ(0, err);
    auto src = srcPath + "test_highlight.code";
    invocation.globalOptions.executablePath = projectPath + "/output/bin/";
    instance = std::make_unique<TestCompilerInstance>(invocation, diag);
    instance->compileOnePackageFromSrcFiles = true;
    instance->srcFilePaths = {src};
