...
6 | | """
  ...
  # help: this is a note help test number 2
    ...
    auto file = std::string{"test.code"};
    SourceManager sm;
...
TEST(EngineTest, OldDiagKindDefGuard)
{
    ...
    // kind is added to the old diagnostic engine def file (codira/Basic/DiagnosticsAll.def). If you need to
...
#include "Codira/Basic/DiagnosticsAll.def"
#undef WARNING
#undef ERROR
#undef NOTE
    };
    ...
    // current number of each ending kind, need be modified when modify the def in codira/Basic/DiagnosticsAll.def
    ...
    for (size_t i = 0; i < endNumber.size(); ++i) {
        ...
        if (diagEnds[i] > endNumber[i]) {
            std::cerr << "The number of diagnostic kind: " + std::string(diagKindStr[diagEnds[i]]) + "(" +
                    ...
                    "include/codira/Basic/DiagRefactor."
                      << std::endl;
            break;
        } else {
