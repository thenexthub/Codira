...
CODE_ARCH_NAME = platform.machine().replace("AMD64", "x86_64").lower() # normalize AMD64 in Windows to x86_64
...
def generate_version_tail(target):
    """generate version string for codec compiler"""

    res = "{}".format("codenative")
    res = " (" + res + ")"
...
def generate_cmake_defs(args):
    ...
    install_prefix = CMAKE_OUTPUT_DIR + ("-{}".format(args.target) if (args.target and args.product == "codec") else "")
    ...
    result = [
        ...
        "-DCODIRA_BUILD_TESTS=" + bool_to_opt((not args.no_tests) and (args.product == "all" or args.product == "libs")),
        ...
        "-DCODIRA_LINK_JOB_POOL=" + (str(args.link_jobs) if args.link_jobs != 0 else ""),
        "-DCODIRA_DISABLE_STACK_GROW_FEATURE=" + bool_to_opt(args.disable_stack_grow_feature),
        "-DCODIRA_USE_OH_LLVM_REPO=" + bool_to_opt(args.use_oh_llvm_repo),
...
def set_codira_env(args):
    ...
    os.environ[LD_LIBRARY_PATH] = PATH_ENV_SEPERATOR.join([
        ...
        os.path.join(HOME_DIR, "output/runtime/lib/{os}_{arch}_codenative".format(os=CODE_OS_NAME, arch=CODE_ARCH_NAME)),
...
def set_codenative_backend_env(args):
    os.environ[LD_LIBRARY_PATH] = PATH_ENV_SEPERATOR.join([
        ...
        os.path.join(HOME_DIR, CMAKE_OUTPUT_DIR, "lib/{os}_{arch}_codenative".format(os=CODE_OS_NAME, arch=CODE_ARCH_NAME)),
        os.path.join(HOME_DIR, "build/build/lib"),
        os.path.join(HOME_DIR, "build/build/lib/{os}_{arch}_codenative".format(os=CODE_OS_NAME, arch=CODE_ARCH_NAME)),
        os.environ.get(LD_LIBRARY_PATH, "")
    ] + (args.target_lib if hasattr(args, 'target_lib') else []))
    LOG.info("set codenative backend env: %s=%s", LD_LIBRARY_PATH, os.environ[LD_LIBRARY_PATH])

    os.environ[LIBRARY_PATH] = PATH_ENV_SEPERATOR.join([
        os.path.join(HOME_DIR, "build/build/lib"),
        os.environ.get(LIBRARY_PATH, "")
    ] + (args.target_lib if hasattr(args, 'target_lib') else []))
    LOG.info("set codenative backend env: %s=%s", LIBRARY_PATH, os.environ[LIBRARY_PATH])
...
def clean(args):
    ...
    # the file handlers (especially for codira.log) that this script holds must be closed first.
    handlerToBeRemoved = []
    ...
    # Files in `codenative_backend` directory may be used while installing if native build is built
    ...
    # Collecting all cross-compiled codecs' build directories.
    cross_output_suffix = []
    for directory in os.listdir(BUILD_DIR):
        build_prefix = "build-codec-"
        if directory.startswith(build_prefix):
...
def main():
    ...
    parser_build = subparsers.add_parser("build", help=" build codira")
    ...
    # codenative BE supports stack grow feature and codec enables stack grow feature by default. However, the feature is
    ...
    parser_build.add_argument(
        "--disable-stack-grow-feature", action="store_true", help="disable default stack grow feature for codenative BE"
    )
    parser_build.add_argument(
    ...
    parser_build.add_argument(
        "--gcc-toolchain", dest="gcc_toolchain", help="Specify toolchain for codec, stdlib & BE build"
    )
    parser_build.add_argument(
    ...
    product_types=['all', 'codec', 'libs']
    parser_build.add_argument(
        "--product", dest="product", choices=product_types,
        help="specify which part of Codira to build"
    )
    parser_build.add_argument(
        "--build-codedb", action="store_true",
        help="build codec with codedb"
    )
    parser_build.add_argument(
        "--codedb-disable-python", action="store_true",
        help="build codedb without python extension"
    )
    parser_build.add_argument(
        "--install-codedb-script", action="store_true",
        help="install codedb with python script"
    )
    parser_build.add_argument(
        "--use-oh-llvm-repo", action="store_true",
        help="use OpenHarmony llvm repo with Codira llvm patch instead of codira llvm repo for building"
    )
    parser_build.add_argument(
        ...
        help="Enable codira sanitizer support for codira libraries."
    )
    parser_build.add_argument(
        "--enable-sanitize-option", action="store_true",
        help="enable --sanitize option for codec"
    )
    parser_build.add_argument(
        "-v", "--version",
        dest="version",
...
def check_compiler(args):
    ...
    if c_compiler == None or cxx_compiler == None:
        ...
        LOG.error("clang/clang++ or gcc/g++ is required to build codira compiler")

    # If cross-compiling codec, a native compiler is also needed by cross-building LLVM,
