    ...
    CGModule& cgMod, const CHIR::EnumType& chirEnumType)
{
    ...
    CODEC_ASSERT_WITH_MSG(!ctors.empty(), "The enum type has at least one constructor.");
    for (auto ctor : ctors) {
...
CGEnumType::CGEnumTypeKind CGEnumType::CalculateCGEnumTypeKind()
{
    ...
    if (!hasRefTypeAssociatedValue && chirEnumType.GetGenericArgs().empty()) {
        CODEC_ASSERT(sizeOfConstructorUnion.has_value());
...
std::string CGEnumType::GetEnumTypeName() const
{
    switch (cgEnumTypeKind) {
        ...
        default:
            CODEC_ASSERT_WITH_MSG(false, "Should not reach here: UNKNOWN enum kind");
            return "";
    }
}
...
{
    CODEC_NULLPTR_CHECK(optionLikeInfo);
