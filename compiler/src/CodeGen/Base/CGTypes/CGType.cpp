    ...
    const CGType::TypeExtraInfo& extraInfo, const CHIR::FuncBase* chirFunc)
{
    ...
    switch (chirType.GetTypeKind()) {
        ...
        case Codira::CHIR::Type::TypeKind::TYPE_CSTRING: {
        ...
        default:
            CODEC_ASSERT_WITH_MSG(false, "Should not reach here: not a contract type from chir.");
            return nullptr;
    }
...
CGType* CGType::GetOrCreate(CGModule& cgModule, const Codira::CHIR::Type* chirTy, const TypeExtraInfo& extraInfo)
{
    CODEC_NULLPTR_CHECK(chirTy);
    if (chirTy->IsAutoEnvInstBase()) {
        ...
        CODEC_ASSERT(chirTy->IsAutoEnvGenericBase());
    }
    ...
    if (auto it = secondCache.find(chirTy->ToString());
        ...
        CODEC_ASSERT(false && "It must be a bug on CHIR: an AST type corresponds to different memory address");
        return it->second;
    }
...
CGType::CGGenericKind CGType::GetCGGenericKind(const CHIR::Type& chirType)
{
    ...
    if (hasGenericTypeArg) {
        ...
        if (auto ct = dynamic_cast<const CHIR::CustomType*>(&chirType); ct && !ct->GetGenericArgs().empty()) {
            CODEC_ASSERT(ct->GetCustomTypeDef()->TestAttr(CHIR::Attribute::GENERIC_INSTANTIATED));
...
{
    CODEC_ASSERT_WITH_MSG(!chirType.IsThis(), "Should not get typeinfo of ThisType.");
    CODEC_ASSERT_WITH_MSG(!chirType.IsRef(), "Should not get typeinfo of RefType.");
    if (typeInfo) {
        return typeInfo;
    }
    ...
    CODEC_ASSERT_WITH_MSG(typeInfo, "This type does not have a typeinfo GV, please check the caller.");
...
llvm::GlobalVariable* CGType::GetOrCreateTypeTemplate()
{
    ...
    CODEC_ASSERT_WITH_MSG(typeTemplate, "This type does not have a type-template GV, please check the caller.");
...
{
    CODEC_ASSERT(!chirType.IsRef() && "RefType should not reach here.");
...
std::string CGType::GetNameOfTypeTemplateGV(const CHIR::Type& chirType)
{
    ...
    if (chirType.IsTuple()) {
        ...
        if (ct->IsAutoEnvBase()) {
            CODEC_ASSERT(!ct->IsAutoEnvInstBase());
            return "Closure.tt";
        }
    ...
    CODEC_ASSERT(false && "Unsupported");
    return "Unsupported";
}

void CGType::GenTypeInfo()
{
    CODEC_ASSERT(!IsDynamicGI() && "Dynamic type should not reach here.");
    CODEC_ASSERT(typeInfo);
    ...
    if (IsStaticGI()) {
        ...
        auto customType = dynamic_cast<const CHIR::CustomType*>(&chirType);
        CODEC_ASSERT(customType && "The chirType that enters this branch is expected to be of CustomType.");
...
llvm::Constant* CGType::GenKindOfTypeInfo()
{
    ...
    CODEC_ASSERT(!chirType.IsRef() && "Unexpected CHIR type to generate typeinfo.");
...
std::string GetTypeQualifiedNameForReflect(CGModule& cgMod, const CHIR::Type& t, bool forNameFieldOfTi = false)
{
    ...
    switch (k) {
        ...
        default:
            CODEC_ASSERT_WITH_MSG(false, "Should not reach here.");
            return "";
    }
}
} // namespace
