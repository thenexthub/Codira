...
#include "CODENative/CGTypes/EnumCtorTIOrTTGenerator.h"
...
void InitializeCodeStringLiteral(const CGModule& cgMod);
...
const std::string CODIRA_SDK_VERSION = CODE_SDK_VERSION;
#else
const std::string CODIRA_SDK_VERSION = "";
#endif
...
std::vector<llvm::Constant*> TopologicalSortStaticGIs(const CGContext& cgCtx, std::vector<llvm::Constant*> content)
{
    ...
    CODEC_ASSERT(content.size() == res.size());
    return res;
}
...
void GenerateStaticGIs(CGModule& cgMod)
{
    ...
    for (auto staticGI : cgMod.GetAliveStaticGIs()) {
        if (staticGI->getNumUses() > 0 || (!staticGI->hasLocalLinkage() && !staticGI->hasLinkOnceODRLinkage())) {
            CODEC_ASSERT(staticGI->hasInitializer());
...
void EmitCODESDKVersion(const CGModule& cgMod)
{
    auto& llvmCtx = cgMod.GetLLVMContext();
    auto linkageType = llvm::GlobalValue::LinkageTypes::PrivateLinkage;
    auto strConstant = llvm::ConstantDataArray::getString(llvmCtx, CODIRA_SDK_VERSION);
    auto codeSdkVersion = llvm::cast<llvm::GlobalVariable>(
        cgMod.GetLLVMModule()->getOrInsertGlobal("code.sdk.version", strConstant->getType()));
    codeSdkVersion->setInitializer(strConstant);
    codeSdkVersion->setAlignment(llvm::Align(1));
    codeSdkVersion->setLinkage(linkageType);
    cgMod.GetCGContext().AddLLVMUsedVars(codeSdkVersion->getName().str());
}
...
void GenSubCHIRPackage(CGModule& cgMod)
{
    ...
    CODENativeReflectionInfo(cgMod, subCHIRPkg).Gen();
    ...
    InitializeCodeStringLiteral(cgMod);
    DumpIRIfNeeded(cgMod, "InitCODEStringLiteral", subDirNum);
...
{
    CODEC_ASSERT(values.size() == 3); // 3 elements: src, min, max
    CODEC_ASSERT(type->isIntegerTy());
...
llvm::Function* CreateMainFunc(const CGModule& cgMod)
{
    ...
    CODEC_ASSERT(!mainFunc && "The main function is generated repeatedly.");
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    ...
    const GlobalOptions& options, DefaultCompilerInstance& compilerInstance, bool enableIncrement)
{
    ...
    if (enableIncrement) {
        cachedMangleMap = StaticCast<Codira::IncrementalCompilerInstance&>(compilerInstance).cacheMangles;
    }
