...
#include "CODENative/CGTypes/CGExtensionDef.h"
...
#endif
{
    // This flag is used to specify that the generated bc is codira bc in LTO mode.
    module->addModuleFlag(llvm::Module::Warning, "CODEBC", static_cast<uint32_t>(1));
    // This flag is used to specify that the generated bc has not been opt-optimized.
    module->addModuleFlag(llvm::Module::Warning, "Codira_OPT", static_cast<uint32_t>(0));
    ...
    if (cgCtx->GetCompileOptions().target.IsMacOS()) {
        ...
        module->addModuleFlag(
            llvm::Module::Warning, "Codira_PACKAGE_ID", llvm::MDString::get(module->getContext(), identifier));
    }
    ...
    if (options.target.os == Triple::OSType::WINDOWS && options.target.arch == Triple::ArchType::X86_64) {
        ...
        cffi = std::make_unique<LinuxOhosArm32CODENativeCGCFFI>(*this);
    } else {
...
{
    CODEC_ASSERT(cgCtx->GetCGPkgContext().IsIncrementEnabled());
...
void CGModule::GenIncremental()
{
    ...
    if (cgCtx->GetCGPkgContext().IsIncrementEnabled()) {
        CODEC_NULLPTR_CHECK(incrementalGen);
...
CGFunction* CGModule::GetOrInsertCGFunction(const CHIR::Value* func, bool forWrapper)
{
    ...
    if (auto it = valueMapping.find(func); it != valueMapping.end()) {
        ...
        CODEC_NULLPTR_CHECK(cgFunction);
        return cgFunction;
    }
    ...
    if (auto f = DynamicCast<const CHIR::FuncBase*>(func); f && f->IsInExtend()) {
        CODEC_ASSERT(outerTIIdx.has_value());
...
{
    CODEC_ASSERT_WITH_MSG(chirGV, "chirGV is null");
    if (auto it = valueMapping.find(chirGV); it != valueMapping.end()) {
        CODEC_NULLPTR_CHECK(it->second);
        return it->second;
    }
...
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    gv->addAttribute(CODEGLOBAL_VALUE_ATTR);
...
CGValue* CGModule::GetMappedCGValue(const CHIR::Value* chirValue)
{
    ...
    if (chirValue->IsImportedFunc()) {
        ...
        CODEC_ASSERT_WITH_MSG(false, "Should not reach here: value not dominate all uses in CHIR.");
        return nullptr;
    }
}
...
llvm::Value* CGModule::GenerateUnitTypeValue()
{
    ...
    if (unitVal == nullptr) {
        ...
        CODEC_NULLPTR_CHECK(unitVal);
        unitVal->addAttribute(CODEGLOBAL_VALUE_ATTR);
...
{
    auto funcName = "__code_personality_v0$";
    // MachO does not support "too many personalities for compact unwind to decode". __code_personality_v0$"
...
llvm::Constant* CGModule::GenerateTypeNameConstantString(const std::string& typeMangledName, bool isWeakODR, const std::string& qualifiedName) const
{
    ...
    klassName->addAttribute(CODETYPE_NAME_ATTR);
...
llvm::Function* CGModule::GetOrInsertFunction(const std::string& funcMangledName, llvm::FunctionType* funcType) const
{
    ...
    CODEC_ASSERT(llvm::isa<llvm::Function>(callee));
...
const std::vector<std::unique_ptr<CGExtensionDef>>& CGModule::GetAllCGExtensionDefs()
{
    ...
    for (auto customDef : subCHIRPkg.chirCustomDefs) {
        if (!traversedCustomTypeDefs.emplace(customDef).second) {
            ...
            CODEC_ASSERT(false && "Duplicated customDef provided by CHIR.");
            continue;
        }
