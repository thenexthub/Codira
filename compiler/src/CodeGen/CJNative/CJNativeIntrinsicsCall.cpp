    ...
    const CHIRIntrinsicWrapper& intrinsic, const std::vector<llvm::Value*>& args, const std::vector<llvm::Type*>& tys)
{
    ...
    CODEC_ASSERT(false && "Unsupported intrinsic.");
    return nullptr;
}

llvm::Function* IRBuilder2::GetExceptionIntrinsicThrow() const
{
    // declare void @code_throw_exception(i8 addrspace(1)* param)
    return llvm::Intrinsic::getDeclaration(GetLLVMModule(), llvm::Intrinsic::code_throw_exception);
}
...
llvm::Value* IRBuilder2::CallSetLocation()
{
    ...
    // Func: anyregcc void @llvm.code.set.location()
    auto setLocation = llvm::Intrinsic::getDeclaration(GetLLVMModule(), llvm::Intrinsic::code_set_location);
...
{
    // declare i8* code_get_exception_wrapper()
    auto getExceptionValue =
        llvm::Intrinsic::getDeclaration(GetLLVMModule(), llvm::Intrinsic::code_get_exception_wrapper);
    return CreateCall(getExceptionValue);
}

llvm::Value* IRBuilder2::CallPostThrowExceptionIntrinsic(llvm::Value* exceptionValue)
{
    // declare i8 addrspace(1)* @llvm.code.post.throw.exception(i8* param)
    auto beginCatch = llvm::Intrinsic::getDeclaration(GetLLVMModule(), llvm::Intrinsic::code_post_throw_exception);
...
llvm::Value* IRBuilder2::CallStackTraceIntrinsic(const CHIRIntrinsicWrapper& syscall, std::vector<CGValue*>& parameters)
{
    ...
    if (intrinsicKind == CHIR::IntrinsicKind::FILL_IN_STACK_TRACE) {
        CODEC_ASSERT(!parameters.empty() && "the arguments number of parameters is incorrect.");
        ...
        // declare i8 addrspace(1)* @code_get_exception_wrapper(i8* rawArray, i8 addrspace(1)* rawArray)
        auto fillStackTrace = llvm::Intrinsic::getDeclaration(GetLLVMModule(), llvm::Intrinsic::code_fill_in_stack_trace);
        ...
        // The last parameter, uint8ArrayClassInfo, is used to construct CODE Ojbect(RawArray) at runtime.
        // Since runtime know nothing about CODE Object, CodeGen need to pass the ClassInfo to runtime.
        ...
        return CallIntrinsicFunction(retCGType->GetLLVMType(), "CODE_MCC_DecodeStackTrace", parameters, {});
    }
    CODEC_ASSERT(false && "Unsupported syscall.");
    return nullptr;
}
...
llvm::Value* IRBuilder2::CallThreadInfoIntrinsic(const CHIRIntrinsicWrapper& syscall, std::vector<CGValue*>& parameters)
{
    ...
    if (intrinsicKind == CHIR::IntrinsicKind::DUMP_CURRENT_THREAD_INFO) {
        ...
        return CallIntrinsicFunction(retCGType->GetLLVMType(), "CODE_MCC_GetCurrentThreadSnapshot", parameters, {});
        ...
        auto charArrayVal = CGValue(CreateBitCast(CreateTypeInfo(*charArrayType), getInt8PtrTy()),
            CGType::GetOrCreate(cgMod, CGType::GetRefTypeOfCHIRInt8(cgCtx.GetCHIRBuilder())));
        parameters.emplace_back(&charArrayVal);
        return CallIntrinsicFunction(retCGType->GetLLVMType(), "CODE_MCC_GetAllThreadSnapshot", parameters, {});
    }
    CODEC_ASSERT(false && "Unsupported syscall.");
    return nullptr;
}
...
void IRBuilder2::CallIntrinsicIndexCheck(llvm::Value* len, llvm::Value* index)
{
    ...
    // after calling CODE_MCC_ThrowException should be set unreachable
...
llvm::Value* IRBuilder2::AcquireRawData(const CHIRIntrinsicWrapper& intrinsic)
{
    ...
    // i8* @code_acquire_rawdata(i8 addrspace(1)*, i8*)
    auto func = llvm::Intrinsic::getDeclaration(
        cgMod.GetLLVMModule(), static_cast<llvm::Intrinsic::ID>(llvm::Intrinsic::code_acquire_rawdata));
...
llvm::Value* IRBuilder2::ReleaseRawData(const CHIRIntrinsicWrapper& intrinsic)
{
    ...
    // void @code_release_rawdata(i8 addrspace(1)*, i8*, i32)
    auto func = llvm::Intrinsic::getDeclaration(
        GetLLVMModule(), static_cast<llvm::Intrinsic::ID>(llvm::Intrinsic::code_release_rawdata));
...
void IRBuilder2::CreateRefStore(CGValue* cgValue, llvm::Value* basePtr, llvm::Value* place, bool isBaseObjStruct)
{
    ...
    CODEC_ASSERT(currentFunction && "Can't get the current function.");
    ...
    const std::vector<std::string>& attributes)
{
    ...
    for (auto value : parameters) {
        CODEC_ASSERT(value);
        CODEC_ASSERT(value->GetRawValue());
...
// Codira function calls the intrinsic function. LLVMCustomizedCreationAPI.cpp:99
// For CODENATIVE: If Codira call C functions and C functions call Codira functions back, need to add "code2c" attribute.
...
{
    CODEC_ASSERT(retType && "The return type of intrinsic function is a nullptr.");
    ...
    CGValue& obj, CGFunction& cgFunc, std::optional<CGValue*>& threadCtx, bool spawnWithFuture, llvm::Value* futureTI)
{
    ...
    auto rtFuncName =
        spawnWithFuture ? PREFIX_OF_BACKEND_SYMS + "NewCODEThread" : PREFIX_OF_BACKEND_SYMS + "NewCODEThreadNoReturn";
...
{
    auto module = cgMod.GetLLVMModule();
    auto function = llvm::Intrinsic::getDeclaration(module, llvm::Intrinsic::code_malloc_object);
...
llvm::Value* IRBuilder2::CallClassIntrinsicAlloc(const CHIR::Type& type)
{
    ...
    if (type.IsClass()) {
        CODEC_ASSERT(!type.IsAutoEnvBase() && "Should not reach here, please check CHIR.");
...
{
    CODEC_ASSERT(parameters.size() == 2U);
...
// The IR generated manually by this function depends on the ABI of the Codira CPointer. If the processing of the
...
void IRBuilder2::CreateOverflowOrArithmeticException(const std::string& ident, bool isOverflow)
{
    ...
    CODEC_ASSERT(createExceptionFuncRawType->getNumParams() == 1U && "Incorrect number of parameters");
    ...
    IRBuilder2& irBuilder, const std::string& funcMangledName, const std::vector<llvm::Value*>& args)
{
    ...
    CODEC_ASSERT(implicitImportedFunc && "Cant get the implicit Imported function.");
    ...
    IRBuilder2& irBuilder, const std::string& exceptionClassName, const std::string& exceptionInitFuncName)
{
    ...
    CODEC_NULLPTR_CHECK(exceptionClassDef);
...
{
    const std::string execAtExitCallbacksFuncName = "_CNat27CODE_CORE_ExecAtexitCallbacksHv";
    CreateImplicitUsedFuncCall(
...
void IRBuilder2::CreateSetRuntimeCODEThreadHandleCall(const std::vector<llvm::Value*>& args)
{
    const std::string setRuntimeCODEThreadHandleFuncName = "_CNat6Thread24setRuntimeCODEThreadHandleHPu";
    CreateImplicitUsedFuncCall(*this, setRuntimeCODEThreadHandleFuncName, args);
}
...
void IRBuilder2::CreateEPrintlnCall(const std::string& eMsg)
{
    ...
    CODEC_ASSERT(eprintlnFuncRawType->getNumParams() == size2 && "Incorrect number of parameters");
...
llvm::Value* IRBuilder2::CallAtomicIntrinsics(const CHIRIntrinsicWrapper& intrinsic, const std::vector<CGValue*>& args)
{
    ...
    CODEC_ASSERT(atomicIntrinsicGenericTypeInfo.size() == 2U);
    ...
    // Currently, Codira only supports one memory order `SeqCst`.
    ...
    if (auto rstType = intrinsic.GetResult()->GetType(); !rstType->IsGeneric()) {
        if (auto cgType = CGType::GetOrCreate(cgMod, rstType); !cgType->GetSize()) {
            CODEC_ASSERT(intrinsic.GetIntrinsicKind() == CHIR::ATOMIC_LOAD ||
    ...
    const CHIRIntrinsicWrapper& intrinsic, std::vector<llvm::Value*>& parameters)
{
    switch (intrinsic.GetIntrinsicKind()) {
        case CHIR::IntrinsicKind::POWI: {
            constexpr size_t size2 = 2;
            CODEC_ASSERT(parameters.size() == size2);
    ...
    llvm::Value* varrayLen, CGValue& autoEnv, const CHIR::VArrayType& vArrayType)
{
    ...
    CODEC_ASSERT(abstractMethods.size() == 1);
...
{
    CODEC_ASSERT(parameters.size() == 3U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_get_type_info);
    auto fixedParams = {
...
{
    CODEC_ASSERT(parameters.size() == 2U || parameters.size() == 3U);
    CODEC_ASSERT(parameters[1]->getType()->isIntegerTy(64U));
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_get_field_offset);
    ...
    // llvm.code.get.field.offset needs 3 arguments: typeinfo, index, pre-offset
...
{
    CODEC_ASSERT(parameters.size() == 2U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_is_subtype);
...
llvm::Instruction* IRBuilder2::CallIntrinsicIsTupleTypeOf(const std::vector<llvm::Value*>& parameters)
{
    ...
    auto funcName = std::string("llvm.code.is.tupletype.of.") + (isRefType ? "p1i8" : "p0i8");
...
llvm::Instruction* IRBuilder2::CallIntrinsicIsTypeEqualTo(const std::vector<llvm::Value*>& parameters)
{
    CODEC_ASSERT(parameters.size() == 2U);
    llvm::Function* func =
        llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_is_typeinfo_equal);
    auto fixedParams = {CreateBitCast(parameters[0], getInt8PtrTy()), CreateBitCast(parameters[1], getInt8PtrTy())};
    return CreateCall(func, fixedParams);
}
...
llvm::Instruction* IRBuilder2::CallIntrinsicAllocaGeneric(const std::vector<llvm::Value*>& parameters)
{
    ...
    CODEC_ASSERT(parameters.size() == 2U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_alloca_generic);
    auto fixedParams = {CreateBitCast(parameters[0], getInt8PtrTy()), parameters[1]};
...
{
    CODEC_ASSERT(parameters.size() == 4U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcwrite_generic);
...
llvm::Instruction* IRBuilder2::CallGCWriteGenericPayload(const std::vector<llvm::Value*>& parameters)
{
    CODEC_ASSERT(parameters.size() == 3U);
    llvm::Function* func =
        llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcwrite_generic_payload);
...
{
    CODEC_ASSERT(parameters.size() == 4U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcread_generic);
...
{
    CODEC_ASSERT(parameters.size() == 3U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_get_mtable_func);
    auto fixedParams = {
...
{
    CODEC_ASSERT(parameters.size() == 3U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_get_method_outertype);
    auto fixedParams = {
        CreateBitCast(parameters[0], getInt8PtrTy()), CreateBitCast(parameters[1], getInt8PtrTy()), parameters[2]};
    return CreateCall(func, fixedParams);
}
...
{
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_get_vtable_func);
...
{
    CODEC_ASSERT(parameters.size() == 3U);
    llvm::Function* func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_assign_generic);
    ...
    const std::unordered_map<const CHIR::Type*, std::function<llvm::Value*(IRBuilder2&)>>& map, bool canChangeBB)
{
    ...
    if (baseType->IsThis()) {
        CODEC_ASSERT(cgFunction && "Should not reach here.");
        ...
        if (cgFuncType->IsStaticMethodType()) {
            ...
            CODEC_ASSERT(thisTIIdx.has_value());
            return cgFunction->GetRawFunction()->getArg(thisTIIdx.value());
        } else {
            CODEC_ASSERT(cgFuncType->IsMethodType() && "CHIR should not use `ThisType` outside a method");
            auto f = cgFunction->GetRawFunction();
            auto thisArg = f->getArg(f->hasStructRetAttr() ? 1U : 0U);
            CODEC_ASSERT(thisArg->getType()->isPointerTy() && thisArg->getType()->getPointerAddressSpace() == 1U);
    ...
    if (auto foundIt = map.find(baseType); foundIt != map.end()) {
        ...
        if (auto upperBounds = static_cast<const CHIR::GenericType*>(baseType)->GetUpperBounds();
            ...
            CODEC_ASSERT(false && "CHIR uses an unexpected Generic-Type.");
            return nullptr;
        }
        ...
        if (customType->IsAutoEnvBase()) {
            ...
            CODEC_ASSERT(customType->IsAutoEnvGenericBase());
...
llvm::Value* IRBuilder2::CreateTypeInfo(const CHIR::Type* type, bool canChangeBB)
{
    if (type == nullptr) { // Just a temporary assertion
        CODEC_ASSERT(type && "Meets a nullptr from CHIR.");
    }
...
llvm::Value* GetRealUUIDForAutoEnvClass(IRBuilder2& irBuilder, llvm::Value* obj)
{
    ...
    auto isWrapperClass = irBuilder.CallIntrinsicFunction(llvm::Type::getInt1Ty(irBuilder.GetLLVMContext()), "CODE_MCC_IsWrapperClassForAutoEnv", {&cgTi}, {});
...
{
    CODEC_ASSERT(parameters.size() == 2 && "Func refEq should have two parameters");
