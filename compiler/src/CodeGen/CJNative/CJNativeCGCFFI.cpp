...
{
    CODEC_ASSERT(idx < func->arg_size());
...
bool IsMetCType(const CHIR::Type& chirTy)
{
    if (chirTy.IsVArray() || IsVArrayRef(chirTy)) {
        ...
        CODEC_ASSERT(elemType && !elemType->IsInvalid());
...
bool LinuxCODENativeCGCFFI::ProcessInvocation(
    const CHIR::FuncType& chirFuncTy, std::vector<CGValue*>& args, IRBuilder2& builder)
{
    ...
    for (size_t i = 0, argIdx = 0; i < kinds.size(); ++i, ++argIdx) {
        ...
        CODEC_ASSERT(i < paramTys.size() && paramTys[i]);
...
llvm::Type* LinuxCODENativeCGCFFI::GetReturnType(CHIR::Type& chirTy, std::vector<llvm::Type*>& params)
{
...
llvm::FunctionType* LinuxAmd64CODENativeCGCFFI::GetCFuncType(const CHIR::FuncType& chirFuncTy)
{
    auto found = cfuncMap.find(&chirFuncTy);
    if (found != cfuncMap.end()) {
        return found->second.first;
    }
    CODEC_ASSERT(chirFuncTy.IsCFunc());
...
void LinuxAmd64CODENativeCGCFFI::AddFunctionAttr(const CHIR::FuncType& chirFuncTy, llvm::Function& llvmFunc)
{
    CODEC_ASSERT(chirFuncTy.IsCFunc());
    auto found = cfuncMap.find(&chirFuncTy);
    if (found == cfuncMap.end()) {
    ...
    for (auto it = kinds.begin(); it != kinds.end(); ++it, ++argIdx) {
        ...
        if (*it == ProcessKind::INDIRECT) {
            CODEC_ASSERT(IsStructPtrTy(GetArgType(&llvmFunc, argIdx)));
...
void LinuxAmd64CODENativeCGCFFI::ProcessParam(
...
void LinuxAmd64CODENativeCGCFFI::ProcessInvocationArg(
    CHIR::StructType& chirParamTy, ProcessKind kind, size_t& argIdx, std::vector<CGValue*>& args, IRBuilder2& builder)
{
    ...
    auto found = typeMap.find(&chirParamTy);
    CODEC_ASSERT(found != typeMap.end());
    auto& info = found->second;
    if (kind == ProcessKind::DIRECT) {
        CODEC_ASSERT(info.IsDirect());
...
llvm::Value* LinuxAmd64CODENativeCGCFFI::ProcessRetValue(const llvm::Type& retType, llvm::Value& val, IRBuilder2& builder)
{
    CODEC_ASSERT(!retType.isVoidTy());
...
llvm::Type* LinuxAmd64CODENativeCGCFFI::GetStructReturnType(CHIR::StructType& chirTy, std::vector<llvm::Type*>& params)
{
    auto& llvmCtx = ctx.GetLLVMContext();
    ...
    if (info.IsIndirect()) {
        ...
        CODEC_ASSERT(structType->isStructTy());
...
ProcessKind LinuxAmd64CODENativeCGCFFI::GetParamType(
    CHIR::Type& chirTy, int8_t& intNum, int8_t& sseNum, std::vector<llvm::Type*>& params)
{
    CODEC_ASSERT(!IsUnitOrNothing(chirTy));
    if (chirTy.IsStruct()) {
        if (GetTypeSize(cgMod, chirTy) == 0) {
...
llvm::Type* LinuxAmd64CODENativeCGCFFI::GetParamType(
    llvm::Type* ty, const ABIArgInfo& info, int8_t& intNum, int8_t& sseNum) const
{
    CODEC_ASSERT(!info.IsIndirect() && "ArgInfo.size() cannot be 0.");
    if (info.IsDirect()) {
    ...
    CODEC_ASSERT(info.IsExpand() && "ArgInfo.size() must be 2 here.");
...
ABIArgInfo LinuxAmd64CODENativeCGCFFI::GetMappingArgInfo(CHIR::StructType& chirTy, bool isArg)
{
    ...
    if (classes[1] == WordClass::INTEGER) {
        ...
        // CONSIDER: When does this scenario occur?
        CODEC_ASSERT(classes[0] == WordClass::X87);
    } else {
        CODEC_ASSERT(false && "MEMORY, NO_CLASS, X87, COMPLEX_X87 cannot be here.");
    }
...
llvm::Type* LinuxAmd64CODENativeCGCFFI::GetLowPartType(
    llvm::Type* type, std::vector<WordClass>& classes, bool isArg) const
{
    ...
    if (classes[0] == WordClass::NO_CLASS) {
        ...
        CODEC_ASSERT(false && "SSEUP, X87UP cannot be here.");
    }
    return lowPartTy;
}
...
std::vector<WordClass> LinuxAmd64CODENativeCGCFFI::ClassifyType(llvm::Type& type) const
{
...
void LinuxAmd64CODENativeCGCFFI::ClassifyType(
    llvm::Type& type, const std::vector<WordClass>::iterator begin, size_t offset) const
{
    ...
    if (type.isIntOrPtrTy()) {
        ...
        if (bits == 16 || bits == 32 || bits == 64) { // Float16, Float32, Float64 in Codira.
            UnifyWordClass(begin + static_cast<std::ptrdiff_t>(wordOffset), WordClass::SSE);
        } else if (bits == 80) { // 80 bits means long double in C, there is no mapping type in Codira.
            ...
            CODEC_ASSERT(false && "Bits in FloatingPointTy are wrong.");
        }
        ...
        CODEC_ASSERT(false && "Unsupported type.");
    }
}

void LinuxAmd64CODENativeCGCFFI::ClassifyStructType(
...
void LinuxAmd64CODENativeCGCFFI::FixupClassification(const llvm::Type& type, std::vector<WordClass>& wordClasses) const
{
...
llvm::Type* LinuxAmd64CODENativeCGCFFI::GetIntegerTypeWithOffset(
    llvm::Type& curType, size_t offset, llvm::Type& srcType, size_t srcOffset) const
{
    CODEC_ASSERT(srcOffset == 0 || srcOffset == BYTES_PER_WORD);
    if (offset == 0) {
    ...
    CODEC_ASSERT(typeSize != srcOffset);
...
llvm::Type* LinuxAmd64CODENativeCGCFFI::GetSseTypeWithOffset(
    llvm::Type& curType, size_t offset, llvm::Type& srcType, size_t srcOffset) const
{
    CODEC_ASSERT(srcOffset == 0 || srcOffset == BYTES_PER_WORD);
...
bool LinuxAmd64CODENativeCGCFFI::ContainsUselessBits(llvm::Type& type, size_t start, size_t end) const
{
    CODEC_ASSERT(start <= end);
...
LinuxAmd64CODENativeCGCFFI::FieldOffsets LinuxAmd64CODENativeCGCFFI::CalcFieldOffsets(const llvm::StructType& type) const
{
...
unsigned LinuxAmd64CODENativeCGCFFI::GetFieldContainsOffset(FieldOffsets fieldOffsets, size_t offset) const
{
    size_t idx = 0;
...
bool LinuxAmd64CODENativeCGCFFI::ContainsFloatAtOffset(llvm::Type& type, size_t offset) const
{
...
llvm::FunctionType* LinuxAarch64CODENativeCGCFFI::GetCFuncType(const CHIR::FuncType& chirFuncTy)
{
    auto found = cfuncMap.find(&chirFuncTy);
    if (found != cfuncMap.end()) {
        return found->second.first;
...
void LinuxAarch64CODENativeCGCFFI::AddFunctionAttr(const CHIR::FuncType& chirFuncTy, llvm::Function& llvmFunc)
{
    CODEC_ASSERT(chirFuncTy.IsCFunc());
    auto found = cfuncMap.find(&chirFuncTy);
    if (found == cfuncMap.end()) {
...
void LinuxAarch64CODENativeCGCFFI::ProcessParam(
    CHIR::Type& chirParamTy, LLVMFuncArgIt& arg, llvm::Value* place, IRBuilder2& builder)
{
    if (!chirParamTy.IsStruct()) {
        return;
    ...
    CODEC_ASSERT(!argInfo.IsExpand() && "ArgInfo in aarch64 cannot be Expand.");
}

void LinuxAarch64CODENativeCGCFFI::ProcessInvocationArg(
    CHIR::StructType& chirParamTy, ProcessKind kind, size_t& argIdx, std::vector<CGValue*>& args, IRBuilder2& builder)
{
    if (kind == ProcessKind::INDIRECT) {
        return;
    ...
    if (kind != ProcessKind::DIRECT) {
        CODEC_ASSERT(false && "ArgInfo in aarch64 cannot be Expand.");
        return;
    }
    auto found = typeMap.find(&chirParamTy);
    CODEC_ASSERT(found != typeMap.end());
...
llvm::Value* LinuxAarch64CODENativeCGCFFI::ProcessRetValue(
...
llvm::Type* LinuxAarch64CODENativeCGCFFI::GetStructReturnType(CHIR::StructType& chirTy, std::vector<llvm::Type*>& params)
{
    auto& llvmCtx = ctx.GetLLVMContext();
    if (GetTypeSize(cgMod, chirTy) == 0) {
        return llvm::Type::getVoidTy(llvmCtx);
    ...
    CODEC_ASSERT(argInfo.IsDirect() && "ArgInfo in aarch64 cannot be Expand.");
    return argInfo[0];
}

ProcessKind LinuxAarch64CODENativeCGCFFI::GetParamType(CHIR::Type& chirTy, std::vector<llvm::Type*>& params)
{
    if (chirTy.IsStruct()) {
        auto info = GetMappingArgInfo(StaticCast<CHIR::StructType&>(chirTy), true);
        auto structType = GetLLVMType(chirTy);
        ...
        if (info.IsIndirect()) {
            ...
            CODEC_ASSERT(info.IsDirect() && "ArgInfo in aarch64 cannot be Expand.");
            params.emplace_back(info[0]);
            return ProcessKind::DIRECT;
        }
    }
...
ABIArgInfo LinuxAarch64CODENativeCGCFFI::GetMappingArgInfo(CHIR::StructType& chirTy, [[maybe_unused]] bool isArg)
{
    auto found = typeMap.find(&chirTy);
    if (found != typeMap.end()) {
        return found->second;
...
bool LinuxAarch64CODENativeCGCFFI::IsHomogeneousAggregate(llvm::Type& type, llvm::Type*& base, size_t& members)
{
...
#ifdef __APPLE__
void MacAArch64CODENativeCGCFFI::AddFunctionAttr(const CHIR::FuncType& chirFuncTy, llvm::Function& llvmFunc)
{
    LinuxAarch64CODENativeCGCFFI::AddFunctionAttr(chirFuncTy, llvmFunc);
    CODEC_ASSERT(chirFuncTy.GetNumOfParams() == llvmFunc.arg_size());
...
llvm::FunctionType* MacAArch64CODENativeCGCFFI::GetCFuncType(const CHIR::FuncType& chirFuncTy)
{
    auto found = cfuncMap.find(&chirFuncTy);
    if (found != cfuncMap.end()) {
        return found->second.first;
...
void MacAArch64CODENativeCGCFFI::ProcessParam(
    CHIR::Type& chirParamTy, LLVMFuncArgIt& arg, llvm::Value* place, IRBuilder2& builder)
{
    if (!chirParamTy.IsStruct()) {
        return;
...
void MacAArch64CODENativeCGCFFI::ProcessInvocationArg(
    CHIR::StructType& chirParamTy, ProcessKind kind, size_t& argIdx, std::vector<CGValue*>& args, IRBuilder2& builder)
{
    if (kind == ProcessKind::INDIRECT) {
        return;
    ...
    auto found = paramTypeMap.find(&chirParamTy);
    CODEC_ASSERT(found != paramTypeMap.end());
    auto& info = found->second;
    CODEC_ASSERT(info.IsDirect());
    CODEC_ASSERT(argIdx < args.size());
    auto arg = args[argIdx];
...
llvm::Type* MacAArch64CODENativeCGCFFI::GetStructReturnType(CHIR::StructType& chirTy, std::vector<llvm::Type*>& params)
{
    auto type = GetLLVMType(chirTy);
    size_t size = GetTypeSize(cgMod, *type);
    auto& llvmCtx = ctx.GetLLVMContext();
    ...
    return LinuxAarch64CODENativeCGCFFI::GetStructReturnType(chirTy, params);
}

ProcessKind MacAArch64CODENativeCGCFFI::GetParamType(CHIR::Type& chirTy, std::vector<llvm::Type*>& params)
{
    if (chirTy.IsStruct()) {
        auto info = GetMappingArgInfo(StaticCast<CHIR::StructType&>(chirTy), true);
        auto structType = GetLLVMType(chirTy);
...
ABIArgInfo MacAArch64CODENativeCGCFFI::GetMappingArgInfo(CHIR::StructType& chirTy, bool isArg)
{
...
llvm::FunctionType* WindowsAmd64CODENativeCGCFFI::GetCFuncType(const CHIR::FuncType& chirFuncTy)
{
    ...
    for (auto paramTy : chirParamTypes) {
        CODEC_ASSERT(IsMetCType(*paramTy));
        if (IsUnsizedStructTy(*paramTy)) {
            return nullptr;
        }
        CODEC_ASSERT(!IsUnitOrNothing(*paramTy));
...
void WindowsAmd64CODENativeCGCFFI::AddFunctionAttr(const CHIR::FuncType& chirFuncTy, llvm::Function& llvmFunc)
{
    CODEC_ASSERT(chirFuncTy.IsCFunc());
...
void WindowsAmd64CODENativeCGCFFI::ProcessParam(
    CHIR::Type& chirParamTy, LLVMFuncArgIt& arg, llvm::Value* place, IRBuilder2& builder)
{
    if (!chirParamTy.IsStruct()) {
        return;
    ...
    CODEC_ASSERT(!argInfo.IsExpand() && "ArgInfo in Windows x64 cannot be Expand.");
}

llvm::Value* WindowsAmd64CODENativeCGCFFI::ProcessRetValue(
    const llvm::Type& retType, llvm::Value& val, IRBuilder2& builder)
{
    CODEC_ASSERT(!retType.isVoidTy());
    llvm::Value* castRes = &val;
...
void WindowsAmd64CODENativeCGCFFI::ProcessInvocationArg(
    CHIR::StructType& chirParamTy, ProcessKind kind, size_t& argIdx, std::vector<CGValue*>& args, IRBuilder2& builder)
{
    auto found = typeMap.find(&chirParamTy);
    CODEC_ASSERT(found != typeMap.end());
    ...
    if (kind == ProcessKind::DIRECT) {
        ...
        CODEC_ASSERT(false && "ArgInfo in Windows x64 cannot be Expand.");
    }
}

llvm::Type* WindowsAmd64CODENativeCGCFFI::GetStructReturnType(CHIR::StructType& chirTy, std::vector<llvm::Type*>& params)
{
    auto& llvmCtx = ctx.GetLLVMContext();
    auto argInfo = GetMappingArgInfo(chirTy, false);
    if (!argInfo.IsIndirect()) {
        CODEC_ASSERT(argInfo.IsDirect() && "ArgInfo in Windows x64 cannot be Expand.");
        return argInfo[0];
    }
    auto structType = GetLLVMType(chirTy);
    CODEC_ASSERT(structType->isStructTy());
...
ProcessKind WindowsAmd64CODENativeCGCFFI::GetParamType(CHIR::Type& chirTy, std::vector<llvm::Type*>& params)
{
    if (chirTy.IsStruct()) {
        ...
        CODEC_ASSERT(info.IsDirect() && "ArgInfo in Windows x64 cannot be Expand.");
        params.emplace_back(info[0]);
        return ProcessKind::DIRECT;
    }
    llvm::Type* paramType = GetLLVMType(chirTy);
...
ABIArgInfo WindowsAmd64CODENativeCGCFFI::GetMappingArgInfo(CHIR::StructType& chirTy, [[maybe_unused]] bool isArg)
{
...
llvm::FunctionType* LinuxOhosArm32CODENativeCGCFFI::GetCFuncType(const CHIR::FuncType& chirFuncTy)
{
    auto found = cfuncMap.find(&chirFuncTy);
    if (found != cfuncMap.end()) {
        return found->second.first;
...
void LinuxOhosArm32CODENativeCGCFFI::ProcessParam(
    CHIR::Type& chirParamTy, LLVMFuncArgIt& arg, llvm::Value* place, IRBuilder2& builder)
{
    if (!chirParamTy.IsStruct()) {
        return;
    ...
    CODEC_ASSERT(!argInfo.IsExpand() && "ArgInfo in arm32 cannot be Expand.");
}

void LinuxOhosArm32CODENativeCGCFFI::ProcessInvocationArg(
    CHIR::StructType& chirParamTy, ProcessKind kind, size_t& argIdx, std::vector<CGValue*>& args, IRBuilder2& builder)
{
    if (kind == ProcessKind::INDIRECT) {
        return;
    ...
    if (kind != ProcessKind::DIRECT) {
        CODEC_ASSERT(false && "ArgInfo in arm32 cannot be Expand.");
        return;
    }
    auto found = paramTypeMap.find(&chirParamTy);
    CODEC_ASSERT(found != paramTypeMap.end());
...
llvm::Type* LinuxOhosArm32CODENativeCGCFFI::GetStructReturnType(CHIR::StructType& chirTy, std::vector<llvm::Type*>& params)
{
    auto& llvmCtx = ctx.GetLLVMContext();
    if (GetTypeSize(cgMod, chirTy) == 0) {
        return llvm::Type::getVoidTy(llvmCtx);
    ...
    CODEC_ASSERT(argInfo.IsDirect() && "ArgInfo in arm32 cannot be Expand.");
    return argInfo[0];
}

ProcessKind LinuxOhosArm32CODENativeCGCFFI::GetParamType(CHIR::Type& chirTy, std::vector<llvm::Type*>& params)
{
    if (chirTy.IsStruct()) {
        auto info = GetMappingArgInfo(StaticCast<CHIR::StructType&>(chirTy), true);
        auto structType = GetLLVMType(chirTy);
        ...
        if (info.IsIndirect()) {
            ...
            CODEC_ASSERT(info.IsDirect() && "ArgInfo in arm32 cannot be Expand.");
            params.emplace_back(info[0]);
            return ProcessKind::DIRECT;
        }
    }
...
ABIArgInfo LinuxOhosArm32CODENativeCGCFFI::GetMappingArgInfo(CHIR::StructType& chirTy, bool isArg)
{
    if (isArg) {
    ...
    if (isArg) {
        ...
        if (alignment <= 4) {
            ...
            CODEC_ASSERT(true && "Alignment should not be between 4 and 8 bytes.");
        }
