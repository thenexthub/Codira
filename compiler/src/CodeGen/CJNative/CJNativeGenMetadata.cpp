...
using CGEnumKind = Codira::CodeGen::CGEnumType::CGEnumTypeKind;
...
{
    llvm::LLVMContext& llvmCtx = module.GetLLVMContext();
    CODEC_ASSERT(method.GetType()->IsFunc());
    ...
    if (method.IsFuncWithBody()) {
        ...
        CODEC_NULLPTR_CHECK(methodValue);
        ...
        CODEC_NULLPTR_CHECK(importedMethodValue);
        uint8_t hasSRetMode =
...
llvm::MDNode* ClassMetadataInfo::GenerateClassAbsMethodMetadata(const CHIR::AbstractMethodInfo& absMethod)
{
    ...
    CODEC_ASSERT(methodType->IsFunc());
