 ...
 *   %0 = landingpad token
 *          catch i8* null
 *   %1 = call i8* @llvm.code.get.exception.wrapper()
 *   %2 = call i8 addrspace(1)* @llvm.code.begin.catch(i8* %1)
 *   %3 = call i1 @llvm.code.is.instance.of(i8 addrspace(1)* %2, i8* bitcast (%KlassInfo.0*
 ...
 *   %x = call i1 @llvm.code.is.instance.of(i8 addrspace(1)* %8, i8* bitcast (%KlassInfo.1*
 *           @_CNat5ErrorE.objKlass to i8*))
 *   br i1 %x, label %is.Error, label %judge.Exception
 *
 * is.Error:                                    ; preds = %judge.Error
 *   %4 = call i8* @llvm.code.get.virtual.func(i8 addrspace(1)* noalias %2, i64 3)
 ...
 *   %6 = call i1 @llvm.code.is.instance.of(i8 addrspace(1)* %2, i8* bitcast (%KlassInfo.0*
 ...
 *   call void @llvm.code.end.catch()
 *   call void @"_CNat27CODE_CORE_ExecAtexitCallbacksEv"(%Unit.Type* %X)
 *   ret xxx
 *
 ...
 *        catch i8* null
 *   call void @llvm.code.end.catch()
 *   call void @"_CNat27CODE_CORE_ExecAtexitCallbacksEv"(%Unit.Type* noalias sret(%Unit.Type) %0)
 *   ret xxx
 */
...
llvm::Function* GenerateEntryFunction(CGModule& cgMod, llvm::Function& userMain)
{
    ...
    auto codeEntryFuncType = llvm::FunctionType::get(irBuilder.getInt32Ty(), {}, false);
    auto codeEntryFunc = llvm::cast<llvm::Function>(
        cgMod.GetLLVMModule()->getOrInsertFunction(CODE_ENTRY_FUNC_NAME, codeEntryFuncType).getCallee());
    codeEntryFunc->setPersonalityFn(cgMod.GetExceptionIntrinsicPersonality());
    SetGCCodira(codeEntryFunc);

    auto entryBB = llvm::BasicBlock::Create(cgMod.GetLLVMContext(), "entry", codeEntryFunc);
    ...
    auto createTryScopeInst = [&irBuilder, &userMain]() {
        ...
        CODEC_ASSERT(globalInitFunc && "Global init function should exist.");
        ...
        if (!userMain.arg_empty()) {
            ...
            irBuilder.CreateCODEMemSetStructWith0(commandLineArgs);
...
void EmitMain(CGModule& cgMod)
{
    ...
    CODEC_ASSERT(chirUserMain && "Cannt find userMain.");
    ...
    /// @main func call CodeRuntimeInit
    auto fType = llvm::FunctionType::get(voidType, false);
    auto f = llvm::cast<llvm::Function>(
        cgMod.GetLLVMModule()->getOrInsertFunction(PREFIX_OF_RUNTIME_SYMS + "CodeRuntimeInit", fType).getCallee());
    ...
    // Create @code_entry func
    auto codeEntryFunc = GenerateEntryFunction(cgMod, *userMainFunc->GetRawFunction());
    CODEC_ASSERT(codeEntryFunc != nullptr);

    /// @main func call CodeRuntimeStart, and it will execute @user.main.
    fType = llvm::FunctionType::get(voidType, {codeEntryFunc->getType()}, false);
    f = llvm::cast<llvm::Function>(
        cgMod.GetLLVMModule()->getOrInsertFunction(PREFIX_OF_RUNTIME_SYMS + "CodeRuntimeStart", fType).getCallee());
    (void)irBuilder.CreateCallOrInvoke(fType, f, {codeEntryFunc});
...
llvm::Function* GetGVResetCallBack(CGModule& cgMod, llvm::FunctionType* callBackFuncType)
{
    ...
    AddFnAttr(wrapFunc, llvm::Attribute::get(llvmCtx, CODE2C_ATTR));
    return wrapFunc;
}
...
void CollectDependentPackages(llvm::Function& func, std::vector<std::string>& giList)
{
    ...
    for (auto& block : func.getBasicBlockList()) {
        for (auto& inst : block) {
            ...
            } else if (calleeName.startswith(MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_PACKAGE_INIT_PREFIX)) {
...
void RegisterExceptionRaiser(CGModule& cgMod)
{
    if (cgMod.GetCGContext().GetCurrentPkgName() == CORE_PACKAGE_NAME) {
        ...
        CODEC_ASSERT(bi != nullptr);
        auto bb = bi->getSuccessor(1);
        CODEC_ASSERT(bb != nullptr);
        IRBuilder2 irBuilder(cgMod);
        ...
        // declare void @code_register_implicit_exception_raisers(i8*)
        llvm::Function* registerExceptionFunc =
            llvm::Intrinsic::getDeclaration(module, llvm::Intrinsic::code_register_implicit_exception_raisers);
...
void CreatePackageInitResetFunction(CGModule& cgMod)
{
    ...
    if (!llvmMod->getFunction(funcName)) {
        ...
        auto createTryScopeInst = [&irBuilder, func, &pkgInitFuncName]() {
            ...
            CODEC_NULLPTR_CHECK(gvInitFunc);
...
void ReplaceFunction(CGModule& cgMod)
{
    ...
    for (auto& item : callBasesToReplace) {
        ...
        CODEC_NULLPTR_CHECK(applyW.GetThisParam());
        ...
        if (auto thisType = CGType::GetOrCreate(cgMod, thisCHIRType)->GetLLVMType(); IsTypeContainsRef(thisType)) {
            if (isCalleeMutOrCtor) {
                ...
                (void)irBuilder.CreateCODEMemSetStructWith0(load);
...
void InlineFunction(CGModule& cgMod)
{
    ...
    auto findAllocInstOfThisWithoutTI = [](llvm::Function* function) -> llvm::Instruction* {
        ...
        CODEC_ASSERT(allocInstOfThisWithoutTI && "Alloc inst not found.");
    ...
    for (auto item : callBasesToInline) {
        ...
        if (!inlineRes.isSuccess()) {
            ...
            CODEC_ASSERT(false && "Can't be inlined.");
        }
        ...
        CODEC_NULLPTR_CHECK(thisWithTI);
        ...
        CODEC_NULLPTR_CHECK(thisParamMetadata);
...
void InitializeCodeStringLiteral(const CGModule& cgMod)
{
    ...
    auto& codeStrings = cgMod.GetCGContext().GetCODEStrings();
    if (codeStrings.empty()) {
        return;
    }

    // 1. Traverse all code string literals' data, concatenate them as a big string named rawData
    std::set<std::string> codeStringContents{};
    // 1.1 Sort and deduplicate codeStringContents to obtain the minimum rawData
    for (auto iter = codeStrings.begin(); iter != codeStrings.end(); ++iter) {
        auto& codeStringName = iter->first;
        auto& codeStringContent = iter->second;
        if (cgMod.GetLLVMModule()->getNamedValue(codeStringName)) {
            codeStringContents.emplace(codeStringContent);
        } else {
            // codeString may be codeGen built-in data and optimized.
        }
    }
    // 1.1 Smaller rawData can be obtained by reverse order
    std::string rawData = "";
    for (auto iter = codeStringContents.crbegin(); iter != codeStringContents.crend(); ++iter) {
        auto& codeStringContent = *iter;
        if (rawData.find(codeStringContent) == std::string::npos) {
            rawData.append(codeStringContent);
        }
    }

    // 2. Construct RawArray which stores all code string literal data
    ...
    std::string codeStringDataName = GetCodeStringDataLiteralName(rawData);
    auto gvRawData =
        llvm::cast<llvm::GlobalVariable>(cgMod.GetLLVMModule()->getOrInsertGlobal(codeStringDataName, rawArrayType));
    CODEC_NULLPTR_CHECK(gvRawData);
    gvRawData->setInitializer(rawArrayData);
    gvRawData->setConstant(true);
    gvRawData->addAttribute(CODESTRING_DATA_ATTR);
    gvRawData->setLinkage(llvm::GlobalValue::PrivateLinkage);

    // 3. Set Initializer for all code string literal
    ...
    for (const auto& codeStringIter : codeStrings) {
        auto& codeStringName = codeStringIter.first;
        auto& codeStringContent = codeStringIter.second;
        size_t pos = rawData.find(codeStringContent);
        if (!cgMod.GetLLVMModule()->getNamedValue(codeStringName) || pos == std::string::npos) {
            // codeString may have been optimized and deleted.
            continue;
        }

        auto dataStart = llvm::ConstantInt::getSigned(int32Type, static_cast<int32_t>(pos));
        auto strSize = llvm::ConstantInt::getSigned(int32Type, static_cast<int32_t>(codeStringContent.size()));
        // in LLVM IR:
        // code string = type { i8 addrspace(1)*, i32, i32 }
        auto codeStringInit = llvm::ConstantStruct::get(codeStringType, {i8PtrAddr1, dataStart, strSize});
        auto gv = llvm::cast<llvm::GlobalVariable>(cgMod.GetLLVMModule()->getNamedValue(codeStringName));
        CODEC_NULLPTR_CHECK(gv);
        gv->setInitializer(codeStringInit);
    }
}
...
void GenerateBinarySectionInfo(const CGModule& cgMod)
{
    ...
    auto module = cgMod.GetLLVMModule();
    const std::string codeInitAttrStr = "codeinit";
    ...
    CODEC_NULLPTR_CHECK(pkgInitFunc);
    ...
    if (pkgInitFunc->getSection().empty()) {
        AddFnAttr(pkgInitFunc, llvm::Attribute::get(pkgInitFunc->getContext(), codeInitAttrStr));
    }
    // Add `codeinit` attr to `code_entry` and `main`
    std::vector<std::string> mainFuncs{CODE_ENTRY_FUNC_NAME, "main"};
    for (auto& f : mainFuncs) {
        if (auto func = module->getFunction(f); func && func->getSection().empty()) {
            func->addFnAttr(llvm::Attribute::get(func->getContext(), codeInitAttrStr));
        }
    }
    // Add `codeinit` attr to `user.main` and functions that are directly called by `user.main`
    if (auto userMainFunc = module->getFunction(USER_MAIN_MANGLED_NAME);
        userMainFunc && userMainFunc->getSection().empty()) {
        userMainFunc->addFnAttr(llvm::Attribute::get(userMainFunc->getContext(), codeInitAttrStr));
        ...
        for (auto callee = userMainNode->begin(); callee != userMainNode->end(); ++callee) {
            if (auto calleeFunc = callee->second->getFunction(); calleeFunc && calleeFunc->getSection().empty()) {
                calleeFunc->addFnAttr(llvm::Attribute::get(calleeFunc->getContext(), codeInitAttrStr));
            }
        }
    }
