...
llvm::Value* IRBuilder2::FixFuncArg(const CGValue& srcValue, const CGType& destType, bool isThisArgInStructMut)
{
    ...
    if ((!srcRawValType->isPointerTy() || srcRawValType->getPointerAddressSpace() == 0 ||
        ...
        if (isThisArgInStructMut) {
            ...
            CODEC_ASSERT(srcRawValType->getPointerAddressSpace() == 0);
    ...
    CODEC_ASSERT(destType.GetLLVMType()->isPointerTy());
    ...
    CODEC_ASSERT(srcTypeAddrspace == destTypeAddrspace && "incorrect addrspace for argument");
    ...
    std::vector<CGValue*> args, bool isClosureCall, llvm::Value* thisTypeInfo)
{
    ...
    for (auto arg : args) {
        ...
        CODEC_ASSERT(!cgType->IsStructType());
    ...
    if (this->chirExpr && dynamic_cast<const CHIRCallExpr*>(this->chirExpr)) {
        ...
        // ThisTypeInfo
        if (applyWrapper->IsCalleeStatic()) {
            CODEC_NULLPTR_CHECK(thisTypeInfo);
...
void ConvertArgsType(llvm::IRBuilder<>& irBuilder, const llvm::Function* func, std::vector<llvm::Value*>& args)
{
    ...
    CODEC_ASSERT(args.size() == func->arg_size());
...
{
    // Func: i8 addr1* @llvm.code.gcread.ref(i8 addr1* baseObj, i8 addr1*addr1* place)
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcread_ref);
...
{
    // Func: i8 addr1* @llvm.code.gcread.weakref(i8 addr1* baseObj, i8 addr1*addr1* place)
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcread_weakref);
    ConvertArgsType(*this, func, args);
    return CreateCall(func, args);
}
...
{
    // Func: void @llvm.code.gcread.struct(i8 addr1* baseObj, i8 addr1* dst, i8 addr1*/ i8* src, i64 length)
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcread_struct, {GetSizetLLVMType()});
...
{
    // Func: void @llvm.code.gcread.static.ref(i8 addr1** val)
    return CreateCall(
        llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcread_static_ref), args);
}

llvm::Instruction* IRBuilder2::CallGCReadStaticAgg(llvm::StructType* type, std::vector<llvm::Value*> args)
{
    // Func: void @llvm.code.gcread.static.struct(i8* dst, i8* src, i64 len)
    // 3 arguments.
    CODEC_ASSERT(args.size() == 3);
    llvm::LLVMContext& ctx = getContext();
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcread_static_struct, {GetSizetLLVMType()});
...
{
    // Func: void @llvm.code.gcwrite.ref(type val, i8 addr1* baseObj, type addr1* place)
    // E.g. void @llvm.code.gcwrite.ref(i8 addr1* val, i8 addr1* baseObj, i8 addr1*addr1* refPtr)
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcwrite_ref);
    ConvertArgsType(*this, func, args);
    return CreateCall(func, args);
}
...
llvm::Instruction* IRBuilder2::CallGCWriteAgg(std::vector<llvm::Value*> args)
{
    ...
    // Func: void @llvm.code.gcwrite.struct(i8 addr1* baseObj, i8 addr1* dst, i8 addr1*/ i8* src, i64 length)
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcwrite_struct, {type, GetSizetLLVMType()});
    args[3] = CreateZExtOrTrunc(args[3], GetSizetLLVMType());
    ConvertArgsType(*this, func, args);
    return CreateCall(func, args);
}
...
{
    // Func: void @llvm.code.gcwrite.static(i8 addr1* val, i8 addr1** place)
    return CreateCall(
        llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_gcwrite_static_ref), args);
}

llvm::Instruction* IRBuilder2::CallGCWriteStaticAgg(llvm::StructType* type, std::vector<llvm::Value*> args)
{
    // Func: void @llvm.code.gcwrite.static.struct(i8* dst, i8* src, i64 len)
    // 3 arguments.
    CODEC_ASSERT(args.size() == 3);
    ...
    auto func = llvm::Intrinsic::getDeclaration(
        cgMod.GetLLVMModule(), static_cast<llvm::Intrinsic::ID>(llvm::Intrinsic::code_gcwrite_static_struct), {GetSizetLLVMType()});
    args[2] = CreateZExtOrTrunc(args[2], GetSizetLLVMType());
    auto typeName = GetCodeGenTypeName(type);
    auto meta = llvm::MDTuple::get(ctx, {llvm::MDString::get(ctx, typeName)});
    ConvertArgsType(*this, func, args);
...
llvm::Instruction* IRBuilder2::CreateWriteBarrierForGlobalVariable(const CGValue& value, const CGValue& globalVar)
{
    ...
    CODEC_ASSERT(valueType != nullptr);
...
llvm::Instruction* IRBuilder2::CreateStore(
    llvm::Value* val, llvm::Value* ptr, const Codira::CHIR::Type* type, bool isVolatile)
{
...
llvm::Instruction* IRBuilder2::CreateStore(const CGValue& cgVal, const CGValue& cgDestAddr)
{
    ...
    CODEC_ASSERT(destDerefType != nullptr);
    ...
    if (!isMemberWrite) {
        if (destDerefType && !destDerefType->GetOriginal().IsRef() && cgVal.GetCGType() &&
            ...
            if (auto valSize = cgVal.GetCGType()->GetPointerElementType()->GetSize()) {
                ...
                CODEC_ASSERT(!destDerefType->GetSize());
    ...
    // When storing to global variable and cgVal contains a code class type value,
    // we need to use gc write barrier, i.e. llvm.code.gcwrite.static.xxx
    ...
    // When writing to an object field, it need to use GC write barrier, i.e. llvm.code.gcwrite intrinsic
...
{
    // For CODENative backend, the structure type is coerced passed by reference,
    ...
    if (this->chirExpr &&
        ...
        if (auto t = DeRef(*elemCHIRType); t && t->IsGeneric()) {
            ...
            if (auto base = GetCGContext().GetBasePtrOf(addr)) {
                ...
                CODEC_ASSERT(addr->getType() == getInt8PtrTy(1U)->getPointerTo());
    ...
    if (auto base = GetCGContext().GetBasePtrOf(addr); base && elementType == CGType::GetRefType(GetLLVMContext())) {
        CODEC_ASSERT(base->getType()->getPointerAddressSpace() == 1);
...
llvm::CallInst* IRBuilder2::CreateMemsetStructWith0(llvm::AllocaInst* addr)
{
    ...
    CODEC_ASSERT(curBB && "Can't get the current basic block.");
    auto curFunc = curBB->getParent();
    CODEC_ASSERT(curFunc && "Can't get the current function.");
    ...
    CODEC_ASSERT(addr->getAllocatedType()->isStructTy() && "incorrect argument");
...
llvm::CallInst* IRBuilder2::CreateCODEMemSetStructWith0(llvm::AllocaInst* addr)
{
    CODEC_ASSERT(addr->getAllocatedType()->isStructTy() && "incorrect argument");
    auto structType = llvm::cast<llvm::StructType>(addr->getAllocatedType());
    auto layOut = GetLLVMModule()->getDataLayout().getStructLayout(structType);
    ...
    SetCurrentDebugLocation(currentLoc);
    auto codeMemsetFunc = llvm::Intrinsic::getDeclaration(
        GetLLVMModule(), llvm::Intrinsic::code_memset, {*res->getFunctionType()->param_begin()});
    res->setCalledFunction(codeMemsetFunc);
    return res;
}
...
llvm::Value* IRBuilder2::CreateNullValue(const CHIR::Type& ty)
{
    ...
    // ------------|-----------------------|----------------------------------------------------
    //  CODENATIVE     |  llvm::Constant::     |  1) struct type: memset a struct with 0, returns
    ...
    const CGValue& cgVal, const std::vector<uint64_t>& idxList, [[maybe_unused]] const llvm::Twine& name)
{
    ...
    for (auto idx : idxList) {
        ...
        if (chirExpr && !eleType->GetSize()) {
            ...
            CODEC_NULLPTR_CHECK(GetInsertFunction());
            ...
            CODEC_ASSERT(!eleType->GetOriginal().IsRef());
...
void IRBuilder2::CreateCopyTo(ArrayCopyToInfo arrayCopyToInfo)
{
    ...
    if (!elemCGType->GetSize()) {
        auto func = llvm::Intrinsic::getDeclaration(
            GetLLVMModule(), static_cast<llvm::Intrinsic::ID>(llvm::Intrinsic::code_array_copy_generic), {GetSizetLLVMType()});
    ...
    if (IsTypeContainsRef(elemCGType->GetLLVMType())) {
        ...
        auto copyToInstrinsic = isRefType ? llvm::Intrinsic::code_array_copy_ref : llvm::Intrinsic::code_array_copy_struct;
        auto func =
            llvm::Intrinsic::getDeclaration(GetLLVMModule(), static_cast<llvm::Intrinsic::ID>(copyToInstrinsic), {GetSizetLLVMType()});
        // llvm.code.array.copyto(i8 addrspace(1)* destBP, i8 addrspace(1)* <dest>,
    ...
    const std::vector<CGValue*>& args, const CHIR::RawArrayType& arrTy, const std::string& serialized)
{
    ...
    CODEC_ASSERT(isAllElemsConstant);
...
llvm::Function* IRBuilder2::GetArrayNonGenericElemMalloc() const
{
    auto gcArrayMallocFunc =
        llvm::Intrinsic::getDeclaration(GetLLVMModule(), static_cast<unsigned>(llvm::Intrinsic::code_malloc_array));
...
llvm::Function* IRBuilder2::GetArrayGenericElemMalloc() const
{
    auto gcArrayMallocFunc = llvm::Intrinsic::getDeclaration(
        GetLLVMModule(), static_cast<unsigned>(llvm::Intrinsic::code_malloc_array_generic));
    AddRetAttr(gcArrayMallocFunc, llvm::Attribute::NoAlias);
    return gcArrayMallocFunc;
}
...
llvm::Value* IRBuilder2::CallArrayIntrinsicInitWithContent(const CHIR::RawArrayLiteralInit& arrayLiteralInit)
{
    ...
    CODEC_ASSERT(arrTy->GetTypeKind() == CHIR::Type::TYPE_RAWARRAY);
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
        ...
        CODEC_ASSERT(element);
        llvm::Value* elemValue = element->GetRawValue();
        CODEC_ASSERT(elemValue);
#ifdef NDEBUG
...
{
    std::string codeStringName = GetCodeStringLiteralName(str);
    auto cachedCodeString = cgMod.GetLLVMModule()->getNamedValue(codeStringName);
    if (cachedCodeString) {
    ...
    // }
    // in LLVM IR:
    // code string = type { i8 addrspace(1)*, i32, i32 }
    auto codeStringType = cgMod.GetCGContext().GetCodeStringType();
    auto gvCodeString =
        llvm::cast<llvm::GlobalVariable>(cgMod.GetLLVMModule()->getOrInsertGlobal(codeStringName, codeStringType));
    CODEC_NULLPTR_CHECK(gvCodeString);
    gvCodeString->setConstant(true);
    gvCodeString->addAttribute(CODESTRING_LITERAL_ATTR);
    gvCodeString->setLinkage(llvm::GlobalValue::PrivateLinkage);
    GetCGContext().AddCODEString(gvCodeString->getName().str(), str);
    return gvCodeString;
}
...
void CreateArrayMemSet(IRBuilder2& irBuilder, const ByteArrayInfo& arrInfo, llvm::Value* value)
{
    ...
    CODEC_ASSERT(isInputValid && "The type size of the value used to initialize the byte-array should be one byte.");
    ...
    llvm::Value* arrPtr, llvm::Value* arrayLen, llvm::Value* elemValue, const CHIR::RawArrayType& arrTy)
{
    ...
    CODEC_ASSERT(typeSize > 0 && "Zero size type is not expected to be array element type.");
    if (typeSize == 1) {
...
{
    CODEC_ASSERT(field.GetPath().size() == 1);
...
llvm::Value* IRBuilder2::GetEnumTag(const CHIR::Field& field)
{
    ...
    CODEC_NULLPTR_CHECK(cgEnum);
    CODEC_ASSERT(field.GetBase()->GetType()->IsEnum());
    ...
    switch (cgEnumType->GetCGEnumTypeKind()) {
        ...
        default:
            CODEC_ASSERT(false && "Should not reach here: UNKNOWN enum kind");
            return nullptr;
    }
}
...
llvm::Value* GetCommonEnumAssociatedValue(IRBuilder2& irBuilder, const CHIR::Field& field, const CGValue& cgEnum)
{
    ...
    if (isSized) {
        ...
        CODEC_ASSERT(typeCastExpr->GetResult()->Get<CHIR::EnumCaseIndex>().has_value());
    ...
    IRBuilder2& irBuilder, const CHIR::Field& field, const CGValue& cgEnum)
{
    ...
    auto offset = 0U;
    auto args = field.GetBase()->GetType()->GetTypeArgs();
    CODEC_ASSERT(associatedValIdx < args.size());
...
{
    CODEC_ASSERT(field.GetBase()->IsLocalVar());
    CODEC_ASSERT(StaticCast<CHIR::LocalVar*>(field.GetBase())->GetExpr()->GetExprKind() == CHIR::ExprKind::TYPECAST);
...
llvm::Value* IRBuilder2::GetLayoutSize_32(const CHIR::Type& type)
{
    ...
    if (auto gt = DynamicCast<const CHIR::GenericType*>(baseType); gt && gt->orphanFlag) {
        CODEC_ASSERT(gt->GetUpperBounds().size() == 1U);
...
{
    auto func = llvm::Intrinsic::getDeclaration(cgMod.GetLLVMModule(), llvm::Intrinsic::code_is_reference);
