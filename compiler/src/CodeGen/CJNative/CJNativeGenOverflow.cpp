 ...
 * @file
 *
 * This file realizes generate overflow APIs for codenative codegen.
 */
...
{
    CODEC_ASSERT(val->getType()->isIntegerTy(1) && "val should be bool type!");
    ...
    const std::function<llvm::Value*()>& condLeft, const std::function<llvm::Value*()>& condRight, bool isLogicAnd)
{
    ...
    CODEC_ASSERT(condLeftVal->getType() == boolType->GetLLVMType());
    if (isLogicAnd) {
    ...
    CODEC_ASSERT(condRightVal->getType() == boolType->GetLLVMType());
...
llvm::Value* OverflowHandler::GenerateOverflowDivOrMod()
{
    ...
    CODEC_NULLPTR_CHECK(normalBB);
    CODEC_NULLPTR_CHECK(overflowBB);
    CODEC_NULLPTR_CHECK(endBB);
...
{
    CODEC_ASSERT(argGenValues.size() == 2 && "should have two operands"); // Binary operations should have 2 operands.
    auto& cgMod = irBuilder.GetCGModule();
...
llvm::Value* Codira::CodeGen::GenerateOverflow(IRBuilder2& irBuilder, const OverflowStrategy& strategy,
