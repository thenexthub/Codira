...
{
    // For codenative backend, just get the size information from classLayout.
    ...
    CodeGenDIVector16& elements, llvm::DICompositeType* fwdDecl)
{
    ...
    CODEC_ASSERT(nonConstClassType.GetSuperClassTy(&chirBuilder));
    uint32_t i = nonConstClassType.GetSuperClassTy(&chirBuilder)->GetInstantiatedMemberTys(chirBuilder).size();
    CODEC_ASSERT(nonConstClassType.GetSuperClassTy(&chirBuilder)->GetInstantiatedMemberTys(chirBuilder).size() +
    ...
    const CHIR::EnumCtorInfo& constructor, llvm::DICompositeType* subEnumType, std::size_t ctorIndex)
{
    ...
    CODEC_ASSERT(!enumCtorlayoutType->isOpaque() && "The body is not set for enumCtorlayoutType.");
