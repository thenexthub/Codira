...
#ifndef CODIRA_CGMODULE_H
#define CODIRA_CGMODULE_H

#include "llvm/IR/IRBuilder.h"
#include "llvm/IR/Module.h"
#include "llvm/IR/CODEStructTypeGCInfo.h"
...
public:
    ...
    {
        CODEC_ASSERT(cgCtx && "Should not be null.");
        return *cgCtx;
    }
    ...
    {
        CODEC_ASSERT(cffi && "Should not be null.");
        return *cffi;
    }
    ...
    {
        CODEC_ASSERT(module && "Should not be null.");
        return module.get();
    }
    ...
    llvm::BasicBlock* GetMappedBB(const Codira::CHIR::Block* chirBB) const
    {
    ...
    CGValue* GetMappedValue(const Codira::CHIR::Value* chirValue) const
    {
    ...
    CGValue* GetMappedCGValue(const Codira::CHIR::Value* chirValue);

    // It is a sugar for `GetMappedCGValue` which could simplify coding process.
    inline CGValue* operator|(const Codira::CHIR::Value* chirValue)
    {
        return this->GetMappedCGValue(chirValue);
    }

    inline void SetOrUpdateMappedBB(const Codira::CHIR::Block* chirBB, llvm::BasicBlock* bb)
    {
        bbMapping[chirBB] = bb;
    }

    inline void SetOrUpdateMappedCGValue(const Codira::CHIR::Value* chirValue, std::unique_ptr<CGValue> value)
    {
        valueMapping[chirValue] = value.get();
        allocatedCGValues.emplace_back(std::move(value));
    }

    inline void SetValuesToLoad(const Codira::CHIR::Value* chirValue, llvm::Type* type, llvm::Value* value)
    {
        valuesToLoad[chirValue] = std::make_pair(type, value);
    }

    std::optional<std::pair<llvm::Type*, llvm::Value*>> GetValuesToLoad(const Codira::CHIR::Value* chirValue)
    {
        auto it = valuesToLoad.find(chirValue);
        return it != valuesToLoad.end() ? std::optional(it->second) : std::nullopt;
    }

    inline void SetBoxedValuesToLoad(const Codira::CHIR::Value* chirValue, llvm::Type* type, llvm::Value* value)
    {
        boxedValuesToLoad[chirValue] = std::make_pair(type, value);
    }

    std::optional<std::pair<llvm::Type*, llvm::Value*>> GetBoxedValuesToLoad(const Codira::CHIR::Value* chirValue)
    {
...
private:
    ...
    std::unordered_map<const Codira::CHIR::Block*, llvm::BasicBlock*> bbMapping;
    std::unordered_map<const Codira::CHIR::Value*, CGValue*> valueMapping;
    std::unordered_map<const Codira::CHIR::Value*, std::pair<llvm::Type*, llvm::Value*>> valuesToLoad;
    std::unordered_map<const Codira::CHIR::Value*, std::pair<llvm::Type*, llvm::Value*>> boxedValuesToLoad;
