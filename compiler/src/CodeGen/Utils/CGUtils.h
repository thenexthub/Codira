...
#ifndef CODIRA_CGUTILS_H
#define CODIRA_CGUTILS_H
...
{
    CODEC_NULLPTR_CHECK(structType);
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    structType->setBodyForCODE(bodyVec);
#endif
}
...
inline void SetGCCodira(llvm::Function* func)
{
    func->setGC("codira");
}
...
{
    CODEC_ASSERT(srcType->IsPointerType() && "a pointer type is expected");
...
{
    CODEC_ASSERT(function->arg_size() == paramName.size());
...
inline std::string GetCodeStringDataLiteralName(const std::string& codeStr)
{
    return CODESTRING_DATA_PREFIX + Utils::HashString64(codeStr);
}

inline std::string GetCodeStringLiteralName(const std::string& codeStr)
{
    return CODESTRING_LITERAL_PREFIX + Utils::HashString64(codeStr);
}
