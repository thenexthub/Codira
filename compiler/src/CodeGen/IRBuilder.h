...
#ifndef CODIRA_IRBUILDER2_H
#define CODIRA_IRBUILDER2_H
...
public:
    ...
    llvm::Instruction* CreateStore(
        llvm::Value* val, llvm::Value* ptr, const Codira::CHIR::Type* type, bool isVolatile = false);
    ...
    {
        CODEC_ASSERT(!GetInsertBlock()->getTerminator());
...
public:
    ...
    llvm::Value* CreateBitCast(llvm::Value* value, llvm::Type* destTy, const llvm::Twine& name = "")
    {
        ...
        if (auto basePtr = GetCGContext().GetBasePtrOf(value)) {
            CODEC_ASSERT(value->getType()->getPointerAddressSpace() == 1U);
     ...
     * The FE and BE conventions use a customized `llvm.code_memset`, and
     * the BE will ensure that it can be retained.
     */
    llvm::CallInst* CreateCODEMemSetStructWith0(llvm::AllocaInst* addr);
#endif
    ...
    void CreateSetRuntimeCODEThreadHandleCall(const std::vector<llvm::Value*>& args);
    ...
    /**
     * For CODENative, call gc write barrier if store ref to ref object's ref field.
     */
    ...
    // `canChangeBB` is just used in `CODENativeExtensionDef.cpp`, it is a workaround for now :(
