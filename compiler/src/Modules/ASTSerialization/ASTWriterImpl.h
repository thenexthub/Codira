...
#ifndef CODIRA_MODULES_ASTSERIALIZATION_ASTWRITER_IMPL_H
#define CODIRA_MODULES_ASTSERIALIZATION_ASTWRITER_IMPL_H
...
public:
    ASTWriterImpl(DiagnosticEngine& diag, const std::string& packageDepInfo, const ExportConfig& exportCfg,
        ...
        : diag(diag), config(exportCfg), packageDepInfo(packageDepInfo), codeoManager(codeoManager)
    {
    }
    ~ASTWriterImpl()
    {
...
private:
    ...
    inline FormattedIndex GetExprIndex(const AST::Expr& expr)
    {
        ...
        CODEC_ASSERT(found != savedExprMap.end());
        return found->second;
    }
