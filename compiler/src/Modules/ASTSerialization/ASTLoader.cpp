...
{
    pImpl = MakeOwned<ASTLoaderImpl>(std::move(data), fullPackageName, typeManager, codeoManager, opts);
}
...
std::string ASTLoader::ASTLoaderImpl::PreReadAndSetPackageName()
{
    ...
    CODEC_NULLPTR_CHECK(package->fullPkgName());
...
bool ASTLoader::ASTLoaderImpl::VerifyForData(const std::string& id)
{
    ...
    if (!PackageFormat::VerifyPackageBuffer(verifier)) {
        diag.DiagnoseRefactor(
            DiagKindRefactor::module_loaded_ast_failed, DEFAULT_POSITION, id, importedPackageName, CODIRA_VERSION);
        return false;
    }
    return true;
}
...
std::vector<OwnedPtr<ImportSpec>> ASTLoader::ASTLoaderImpl::LoadImportSpecs(const PackageFormat::Imports* imports)
{
    ...
    CODEC_NULLPTR_CHECK(importSpecs);
...
void ASTLoader::ASTLoaderImpl::PreloadCommonPartOfPackage(AST::Package& pkg)
{
    ...
    for (uoffset_t i = 0; i < fileSize; i++) {
        CODEC_NULLPTR_CHECK(package->allFileImports());
        auto&& importInfos = LoadImportSpecs(package->allFileImports()->Get(i));
        CODEC_NULLPTR_CHECK(package->allFileInfo());
        // Load file info for CODEMP
...
{
    CODEC_ASSERT(curPackage && package);
    // Get toplevel decls.
    ...
    for (uoffset_t i = 0; i < nDecls; i++) {
        ...
        if (package->allDecls()->Get(i)->isTopLevel()) {
            ...
            CODEC_NULLPTR_CHECK(tmpDecl->curFile);
            ...
            CODEC_ASSERT(tmpDecl->curFile->curPackage == curPackage);
...
Position ASTLoader::ASTLoaderImpl::LoadPos(const PackageFormat::Position* posObj)
{
    ...
    if (pkgIndex == INVALID_FORMAT_INDEX) {
        ...
        CODEC_ASSERT(pkgIndex - 1 < importedFullPackageNames.size());
        std::string fullPackageName = importedFullPackageNames.at(pkgIndex - 1);
        if (auto pd = codeoManager.GetPackageDecl(fullPackageName)) {
            CODEC_ASSERT(fileIndex < pd->srcPackage->files.size());
...
void ASTLoader::ASTLoaderImpl::AddDeclToImportedPackage(Decl& decl)
{
    ...
    auto exportIdDeclMap = codeoManager.GetExportIdDeclMap(importedPackageName);
    // Current loading package must existed in 'packageMap'.
    CODEC_NULLPTR_CHECK(exportIdDeclMap);
    if (exportId.empty()) {
...
{
    // New field of FullId type, it may be empty when read old version codeo.
    if (!fullId) {
        return nullptr;
    }
    ...
    if (pkgIndex == PKG_REFERENCE_INDEX) {
        ...
        return codeoManager.GetPackageDecl(fullId->decl()->str());
    ...
    auto exportIdDeclMap = codeoManager.GetExportIdDeclMap(fullPackageName);
    ...
    if (codeoManager.GetPackageDecl(fullPackageName) == nullptr && !isLoadCache) {
        diag.DiagnoseRefactor(
            DiagKindRefactor::package_invalid_codeo_dependency, DEFAULT_POSITION, importedPackageName, fullPackageName);
    }
    return nullptr;
}
...
OwnedPtr<Generic> ASTLoader::ASTLoaderImpl::LoadGeneric(Decl& decl, const PackageFormat::Generic* generic)
{
    ...
    CODEC_NULLPTR_CHECK(generic->typeParameters());
...
OwnedPtr<Annotation> ASTLoader::ASTLoaderImpl::LoadAnnotation(const PackageFormat::Anno& rawAnno)
{
    ...
    CODEC_NULLPTR_CHECK(rawAnno.identifier());
    annotation->identifier = rawAnno.identifier()->str();
    CODEC_NULLPTR_CHECK(rawAnno.args());
...
void ASTLoader::ASTLoaderImpl::LoadFuncDeclAdvancedInfo(const PackageFormat::Decl& decl, FuncDecl& funcDecl)
{
    ...
    funcDecl.isInline = info->isInline() && codeoManager.GetCanInline();
