...
#include "Codira/Mangle/ASTMangler.h"
    ...
    std::vector<Ptr<Decl>>& decls, FuncDecl& fd, std::queue<Ptr<Decl>>& queue, bool serializingCommon)
{
    ...
    CODEC_ASSERT(!fd.funcBody->paramLists.empty());
...
void SaveDeclBasicInfo(const DeclInfo& declInfo, PackageFormat::DeclBuilder& dbuilder)
{
    ...
    // Only add for common part of CODEMP
    if (!declInfo.dependencies.IsNull()) {
        dbuilder.add_dependencies(declInfo.dependencies);
    }
}

// Check whether the decl is exported for common part of CODEMP when it is not exported for general package.
inline bool CanSkip4CODEMP(const Decl& decl, bool serializingCommon)
{
...
{
    pImpl = std::make_unique<ASTWriter::ASTWriterImpl>(diag, packageDepInfo, exportCfg, codeoManager);
}
...
template <typename T> TVectorOffset<FormattedIndex> ASTWriter::ASTWriterImpl::GetBody(T& decl)
{
    ...
    // Incr compilation need load ty by cached codeo, so not only cache visible signature
    ...
    auto shouldExportDecl = [&decl, onlyVisibleSig, this](const Decl* d) -> bool {
        ...
        // stored in codeo as long as the type is externally visible.
        ...
        // Incr compilation need load ty by cached codeo, so still cache internal or inst member var decls
...
inline bool ASTWriter::ASTWriterImpl::NeedToExportDecl(Ptr<const Decl> decl)
{
    ...
    bool canSkip = !config.exportForIncr && !ShouldAlwaysExport(*decl, serializingCommon) &&
        ...
        CanSkip4CODEMP(*decl, serializingCommon);
    if (canSkip) {
        return false;
    }
...
void ASTWriter::ASTWriterImpl::ExportAST(const PackageDecl& package)
{
    ...
    CODEC_NULLPTR_CHECK(package.srcPackage);
...
{
    auto codecVersion = builder.CreateString(CODIRA_VERSION);
    ...
    // Only for common part of CODEMP
    ...
    PackageFormat::CodeoVersion codeoVersion(CODEO_MAJOR_VERSION, CODEO_MINOR_VERSION, CODEO_PATCH_VERSION);
    auto root = PackageFormat::CreatePackage(builder, codecVersion, &codeoVersion, packageName, dependencyInfo, vimports,
...
flatbuffers::Offset<PackageFormat::Imports> ASTWriter::ASTWriterImpl::SaveFileImports(const File& file)
{
    ...
    for (auto& importSpec : file.imports) {
        ...
        if (importSpec->IsReExport()) {
            ...
            SavePackageName(codeoManager.GetPackageNameByImport(*importSpec));
        }
    }
...
void ASTWriter::ASTWriterImpl::SaveFileInfo(const File& file)
{
    ...
    if (found == savedFileMap.end()) {
        ...
        // Add additional file info for CODEMP
...
{
    CODEC_ASSERT(!fullPackageName.empty());
    // If already saved, just return its index.
...
{
    CODEC_ASSERT(!type.typeArgs.empty());
...
flatbuffers::Offset<PackageFormat::Generic> ASTWriter::ASTWriterImpl::SaveGeneric(const Decl& decl)
{
    ...
    for (auto& constraint : generic->genericConstraints) {
        CODEC_NULLPTR_CHECK(constraint);
        std::vector<FormattedIndex> uppers;
        for (auto& upper : constraint->upperBounds) {
            CODEC_NULLPTR_CHECK(upper);
...
TDeclOffset ASTWriter::ASTWriterImpl::SaveVarWithPatternDecl(const VarWithPatternDecl& vpd, const DeclInfo& declInfo)
{
    ...
    CODEC_NULLPTR_CHECK(vpd.initializer);
...
TDeclOffset ASTWriter::ASTWriterImpl::SaveFuncDecl(const FuncDecl& funcDecl, const DeclInfo& declInfo)
{
    ...
    CODEC_NULLPTR_CHECK(funcDecl.funcBody);
...
FormattedIndex ASTWriter::ASTWriterImpl::SaveDecl(const Decl& decl, bool isTopLevel)
{
    ...
    if (fileIndex == INVALID_FORMAT_INDEX) {
        CODEC_ASSERT(decl.curFile && decl.curFile->curPackage);
    ...
    // The dependencies default is null for compatibility, only may have value for common part of CODEMP.
