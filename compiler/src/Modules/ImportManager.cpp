...
void AddImplicitImports(Package& pkg, const GlobalOptions& opts)
{
    for (auto& file : pkg.files) {
        ...
        if (opts.implicitPrelude) {
            AddImport(*file, CORE_PACKAGE_NAME);
        }
        if (opts.enableCompileTest) {
            AddImport(*file, "std.unittest");
            AddImport(*file, "std.unittest.testmacro");
        }
        if (opts.compileTestsOnly && ImportManager::IsTestPackage(pkg.fullPackageName)) {
            AddImport(*file, ImportManager::GetMainPartPkgNameForTestPkg(pkg.fullPackageName));
        }
        if (opts.target.env == Triple::Environment::OHOS && HasIfAvailable(pkg)) {
            ...
            AddImport(*file, "ohos.device_info");
            AddImport(*file, "ohos.base");
        }
    }
}
...
{
    CODEC_ASSERT(level != AccessLevel::PRIVATE);
...
std::string Jsonfy(const ImportSpec& import, bool exportCODEO)
{
    CODEC_NULLPTR_CHECK(import.curFile);
    std::ostringstream out;
    out << "{"
        << "\"file\":\"" << Jsonfy(exportCODEO ? import.curFile->fileName : import.curFile->filePath) << "\","
        << "\"begin\":" << Jsonfy(import.begin) << ","
        << "\"end\":" << Jsonfy(import.end) << "}";
    return out.str();
}

std::string Jsonfy(const std::set<Ptr<ImportSpec>, CmpNodeByPos>& imports, bool exportCODEO)
{
    if (imports.empty()) {
        return "[]";
    }
...
std::string Jsonfy(const DependencyInfoItem& info, bool exportCODEO)
{
    std::ostringstream out;
    ...
    out << "\"imports\":" << Jsonfy(info.imports, exportCODEO) << "}";
    return out.str();
}

std::string Jsonfy(const std::map<std::string, DependencyInfoItem>& dependencies, bool exportCODEO)
{
    ...
    out << Jsonfy(iter->second, exportCODEO);
    ++iter;
...
Range GetPackageNameRange(const CodeoManager& codeoManager, const ImportSpec& import)
{
    auto& im = import.content;
    return import.TestAttr(Attribute::IMPLICIT_ADD)
        ? MakeRange(DEFAULT_POSITION, im.identifier)
        : (codeoManager.IsImportPackage(import)
    ...
    const std::function<void(ASTWriter&)> additionalSerializations)
{
    ASTWriter writer(diag, GeneratePkgDepInfo(pkg),
        {
            ...
            .compileCoded = opts.compileCoded,
        },
        *codeoManager);
    ...
    auto packageDecl = codeoManager->GetPackageDecl(pkg.fullPackageName);
    CODEC_NULLPTR_CHECK(packageDecl);
...
void ImportManager::ExportDeclsWithContent(bool saveFileWithAbsPath, Package& package)
{
    ...
    auto writer = new ASTWriter(diag, GeneratePkgDepInfo(package),
        {importSrcCode, false, opts.exportForTest, saveFileWithAbsPath, opts.compileCoded}, *codeoManager);
    if (opts.outputMode == GlobalOptions::OutputMode::CHIR) {
        writer->SetSerializingCommon();
    }
...
{
    auto cachedCodeo = opts.GenerateCachedPathName(pkg.fullPackageName, SERIALIZED_FILE_EXTENSION);
    return codeoManager->LoadCachedPackage(pkg, cachedCodeo, mangledName2DeclMap);
}
    ...
    const Package& package, const std::string& filePath, bool isUsedAsCommon, bool isRecursive)
{
    ...
    if (codeoManager->IsOnlyUsedByMacro(package.fullPackageName) &&
...
void ImportManager::SaveDepPkgCodeoPath(const std::string& fullPackageName, const std::string& codeoPath)
{
    codeoFilePaths.emplace(fullPackageName, codeoPath);
    auto codedPath = codeoPath.substr(0, codeoPath.rfind(SERIALIZED_FILE_EXTENSION)) + CODE_D_FILE_EXTENSION;
    codedFilePaths.emplace(fullPackageName, codedPath);
}
...
bool ImportManager::ResolveImportedPackageForFile(File& file, bool isRecursive)
{
    ...
    for (auto& import : file.imports) {
        ...
        auto [fullPackageName, codeoPath] = codeoManager->GetPackageCodeo(*import);
        SaveDepPkgCodeoPath(fullPackageName, codeoPath);
        // 1. Handle the package which has been parsed before.
        auto package = codeoManager->GetPackage(fullPackageName);
        if (package != nullptr) {
            CODEC_ASSERT(
                !package->TestAttr(Attribute::IMPORTED) || package->TestAttr(Attribute::TOOL_ADD) || !codeoPath.empty());
            if (!package->TestAttr(Attribute::IMPORTED) && curPkg->TestAttr(Attribute::IMPORTED)) {
                ...
                success = HandleParsedPackage(*package, codeoPath, !isMacroRelated || isVisible, isRecursive) && success;
            }
            continue;
        }
        ...
        // 2. Check and try to load codeo data.
        if (!CheckCodeoPathLegality(import, codeoPath, fullPackageName, isRecursive, isMainPartPkgForTestPkg) ||
            !codeoManager->LoadPackageHeader(fullPackageName, codeoPath)) {
            success = false;
            continue;
        }
        package = codeoManager->GetPackage(fullPackageName);
        CODEC_NULLPTR_CHECK(package);
        ...
        if (package->isMacroPackage && isRecursive && !isVisible && isMacroRelated) {
            ...
            codeoManager->RemovePackage(fullPackageName, package);
            continue;
        }
        ...
        if ((!IsMacroRelatedPackageName(file.curPackage->fullPackageName) && !package->isMacroPackage) ||
            ...
            HandleStdPackage(fullPackageName, codeoPath, isRecursive);
            for (auto depStdPkg : package->GetAllDependentStdPkgs()) {
                auto [it, succ] = codeoFilePaths.emplace(depStdPkg, "");
                if (succ) {
                    it->second = FileUtil::FindSerializationFile(depStdPkg, SERIALIZED_FILE_EXTENSION, GetSearchPath());
            ...
            codeoManager->SetOnlyUsedByMacro(fullPackageName, true);
        }
...
{
    for (auto& pkg : pkgs) {
        codeoManager->AddImportedPackageFromASTNode(std::move(pkg));
    }
}

void ImportManager::HandleStdPackage(const std::string& fullPackageName, const std::string& codeoPath, bool isRecursive)
{
    auto type = isRecursive ? DepType::INDIRECT : DepType::DIRECT;
    auto typeWithFullPkgName = stdDepsMap.find(codeoPath);
    ...
    if ((STANDARD_LIBS.find(fullPackageName) != STANDARD_LIBS.end()) && FileUtil::FileExist(codeoPath)) {
        stdDepsMap.emplace(codeoPath, std::make_pair(type, fullPackageName));
    }
}

bool ImportManager::CheckCodeoPathLegality(const OwnedPtr<ImportSpec>& import, const std::string& codeoPath,
    const std::string& fullPackageName, bool isRecursive, bool isMainPartPkgForTestPkg)
{
    // Call guarantees the 'codeoPath' existing when it is not empty.
    if (codeoPath.empty()) {
        Range range =
            isRecursive ? MakeRange(import->begin, import->begin + 1) : GetPackageNameRange(*codeoManager, *import);
        CODEC_ASSERT(!range.HasZero()); // Except for IMPLICIT_ADD, there should be no zero.

        DiagnosticBuilder builder = diag.DiagnoseRefactor(isMainPartPkgForTestPkg
                ? DiagKindRefactor::package_missed_codeo_main_pkg_part_for_test_pkg
                : DiagKindRefactor::package_search_error,
            range, fullPackageName);
        builder.AddHelp(DiagHelp(Modules::NO_CODEO_HELP_INFO));
        return false;
    }
    return true;
}
...
void ImportManager::UpdateMacroPackageUsage(const AST::Package& pkg)
{
    for (auto& pd : GetCurImportedPackages(pkg.fullPackageName)) {
        // Collect dependent codira packages.
...
bool ImportManager::ResolveImportedPackages(const std::vector<Ptr<Package>>& packages)
{
    ...
    for (auto pkg : packages) {
        ...
        codeoManager->LoadFilesOfCommonPart(pkg);
    ...
    codeoManager->LoadPackageDeclsOnDemand(packages);
#endif
    for (auto pkg : packages) {
        codeoManager->AddPackageDeclMap(pkg->fullPackageName);
    }
    return success;
}
...
void ImportManager::CheckImports(const AST::Package& pkg)
{
    for (auto& file : pkg.files) {
        for (auto& import : file->imports) {
            ...
            auto range = GetPackageNameRange(*codeoManager, *import);
            ...
            if ((package->accessible == AccessLevel::PROTECTED && relation == PackageRelation::NONE) ||
                ...
                continue;
            } else if (codeoManager->IsImportPackage(*import)) {
            ...
            auto decls = codeoManager->GetPackageMembersByName(package->fullPackageName, import->content.identifier);
...
void ImportManager::CheckRedefinition(const AST::Package& pkg)
{
    ...
    for (auto& file : pkg.files) {
        for (auto& import : file->imports) {
            ...
            auto& pkgMembers = codeoManager->GetPackageMembersByName(pkg.fullPackageName, name);
...
std::string ImportManager::GeneratePkgDepInfo(const Package& pkg, bool exportCODEO) const
{
    ...
    for (auto& file : pkg.files) {
        for (auto& import : file->imports) {
            ...
            auto names = codeoManager->GetPossibleCodeoNames(*import);
            CODEC_ASSERT(!names.empty());
    ...
    out << "{"
        ...
        << "\"dependencies\":" << Jsonfy(dependencies, exportCODEO) << ","
        << "\"features\":" << Jsonfy(refSet) << ","
        << "\"product\":" << Jsonfy(isProduct) << "}";
    return out.str();
}

std::vector<std::string> ImportManager::GeneratePkgDepInfoByCodeo(const std::string& codeoPath)
{
    std::string packageDepInfo = codeoManager->GetPackageDepInfo(codeoPath);
...
std::set<std::string> ImportManager::CollectDirectDepPkg(const Package& package) const
{
    ...
    for (auto& file : package.files) {
        for (auto& import : file->imports) {
            ...
            for (auto name : codeoManager->GetPossibleCodeoNames(*import)) {
    ...
    std::unordered_map<std::string, bool>& refMap, Ptr<File>& refFile, bool& anno)
{
    ...
    CODEC_NULLPTR_CHECK(refFile.get());
...
bool ImportManager::BuildIndex(
    const std::string& codiraModules, const GlobalOptions& globalOptions, std::vector<Ptr<Package>>& packages)
{
    CODEC_ASSERT(!packages.empty());
    codeoManager->UpdateSearchPath(codiraModules);
    ...
    for (auto pkg : packages) {
        ...
        codeoManager->AddSourcePackage(*pkg);
    }
    ResolveImportedPackages(packages);
    ClearPackageCodeoCache();
...
Ptr<Package> ImportManager::LoadPackageFromCodeo(const std::string& fullPkgName, const std::string& codeoPath) const
{
    if (!codeoManager->LoadPackageHeader(fullPkgName, codeoPath)) {
        return nullptr;
    }
    ...
    std::function<bool(const Package&)> loadDependentPkgHeader = [&loadDependentPkgHeader, this](const Package& pkg) {
        for (auto& file : std::as_const(pkg.files)) {
            for (auto& import : std::as_const(file->imports)) {
                ...
                auto [fullDepPkgName, codeoPath1] = codeoManager->GetPackageCodeo(*import);
                if (auto depPkg = codeoManager->GetPackage(fullDepPkgName); depPkg) {
                    continue;
                }
                if (!codeoManager->LoadPackageHeader(fullDepPkgName, codeoPath1)) {
                    return false;
                }
                auto depPkg = codeoManager->GetPackage(fullDepPkgName);
                CODEC_NULLPTR_CHECK(depPkg);
    ...
    auto package = codeoManager->GetPackage(fullPkgName);
    CODEC_NULLPTR_CHECK(package);
    if (!loadDependentPkgHeader(*package)) {
        return nullptr;
    }

    codeoManager->LoadPackageDeclsOnDemand({package}, true);
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    codeoManager->ClearVisitedPkgs();
#endif
    codeoManager->AddPackageDeclMap(fullPkgName);
    return package;
}
...
{
    CODEC_ASSERT(newFile);
    newFile->curPackage = &package;
    // Replace the current file.
    CODEC_ASSERT(file.fileHash != newFile->fileHash);
    ...
    for (auto& import : newFile->imports) {
        ...
        auto [_, codeoPath] = codeoManager->GetPackageCodeo(*import);
        if (codeoPath.empty()) {
            InternalError("Failed to resolve imports for macro file");
        }
        if (!import->IsImportAll() && !codeoManager->IsImportPackage(*import)) {
...
{
    return codeoManager->GetPackageDecl(fullPackageName);
}

Ptr<Package> ImportManager::GetPackage(const std::string& fullPackageName) const
{
    return codeoManager->GetPackage(fullPackageName);
}
...
void ImportManager::AddImportedDeclsForSourcePackage(const AST::Package& pkg)
{
    for (auto& file : pkg.files) {
        for (auto& import : file->imports) {
            ...
            auto fullPackageName = codeoManager->GetPackageNameByImport(*import);
            auto pkgDecl = codeoManager->GetPackageDecl(fullPackageName);
            ...
            if (import->content.kind == ImportKind::IMPORT_ALL) {
                auto members = codeoManager->GetPackageMembers(fullPackageName);
            ...
            if (codeoManager->IsImportPackage(*import)) {
                ...
                auto members = codeoManager->GetPackageMembersByName(fullPackageName, import->content.identifier);
...
{
    return codeoManager->GetPackageMembersByName(package.fullPackageName, name);
}
    ...
    const std::string& srcFullPackageName, const std::string& targetFullPackageName) const
{
    ...
    auto members = codeoManager->GetPackageMembers(targetFullPackageName);
...
{
    CODEC_NULLPTR_CHECK(node->curFile);
...
{
    return codeoManager->GetAllPackageDecls(includeMacroPkg);
}
...
{
    auto& decls = codeoManager->GetPackageMembersByName(fullPackageName, name);
    if (decls.empty()) {
        auto decl = codeoManager->GetImplicitPackageMembersByName(fullPackageName, name);
        return decl;
    }
    return *decls.begin();
}

void ImportManager::SetPackageCodeoCache(const std::string& fullPackageName, const std::vector<uint8_t>& codeoData) const
{
    codeoManager->SetPackageCodeoCache(fullPackageName, codeoData);
}

void ImportManager::ClearPackageCodeoCache() const
{
    codeoManager->ClearCodeoCache();
}

bool ImportManager::IsMacroRelatedPackageName(const std::string& fullPackageName) const
{
    return codeoManager->IsMacroRelatedPackageName(fullPackageName);
}

std::string ImportManager::GetMainPartPkgNameForTestPkg(const std::string& testPkgName)
{
    CODEC_ASSERT(IsTestPackage(testPkgName));
