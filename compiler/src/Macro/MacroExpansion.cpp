...
void MacroExpansion::CollectMacros(Package& package)
{
    ...
    for (const auto& file : package.files) {
        ...
        CODEC_ASSERT(ci->diag.GetErrorCount() != 0 ||
...
void MacroExpansion::ReplaceEachMacro(MacroCall& macCall)
{
    ...
    Parser parser{pInvocation->newTokens, ci->diag, ci->diag.GetSourceManager(),
        ci->invocation.globalOptions.enableAddCommentToAst, ci->invocation.globalOptions.compileCoded};
...
// In the LSP scenario, there may be concurrent calls to macro expansion when process coded file. If Execute is executed
