...
void FindMacroDefPkg(MacroCall& macCall, CompilerInstance* ci)
{
    ...
    if (foundStdMacroPkg != importedMacroPackages.end()) {
        ...
        auto libName =
            "lib" + FileUtil::ConvertPackageNameToLibCodiraBaseFormat(*foundStdMacroPkg) + LIB_SUFFIX;
        macCall.libPath = FileUtil::JoinPath(
            FileUtil::JoinPath(basePath, ci->invocation.globalOptions.GetCodiraLibHostPathName()), libName);
    } else {
        auto libName = "lib-macro_" + FileUtil::ToCodeoFileName(macroDefFunc->fullPackageName) + LIB_SUFFIX;
        auto names = Utils::SplitQualifiedName(macroDefFunc->fullPackageName);
        auto fileName = FileUtil::ToCodeoFileName(names.front()) + DIR_SEPARATOR + libName;
