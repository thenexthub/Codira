...
namespace {
const std::string CODEO_EXTENSION = "codeo";
const std::string BC_EXTENSION = "bc";
const std::string CODE_EXTENSION = "code";
...
{
    return backendType == Triple::BackendType::CODENATIVE;
}
} // namespace Codira
...
{
    // Why there is a marco here? In case of LSP, CODIRA_CODEGEN_CODENATIVE_BACKEND
    // is not set. `chirLLVM` is expected to be false.
    // Since `backend` is always set to `BackendType::CODENATIVE` by default.
    // We need a macro to prevent `chirLLVM` from setting to true.
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    chirLLVM = backend == BackendType::CODENATIVE;
#endif
    chirCC = chirCC || chirLLVM; // codenative backend should enable chir-cc.
...
bool GlobalOptions::ReprocessInputs()
{
    ...
    if (compilePackage) {
        for (auto& path : packagePaths) {
            std::vector<std::string> files = GetAllFilesUnderCurrentPath(path, CODE_EXTENSION);
            std::for_each(files.begin(), files.end(), [&path, &inputs](std::string& file) {
                file = FileUtil::JoinPath(path, file);
                if (GetFileExtension(file) == CODE_EXTENSION) {
...
bool GlobalOptions::CheckScanDependencyOptions() const
{
    ...
    DiagnosticEngine diag;
    if (compilePackage) {
        // Report error for `codec--scan-dependency -p pkg.codeo ...`. When `-p` is specified .codeo file
        // inputs are not accepted.
        if (!inputCodeoFile.empty()) {
            (void)diag.DiagnoseRefactor(
                DiagKindRefactor::driver_not_accept_codeo_inputs_when, DEFAULT_POSITION, "'-p' or '--package'");
            return false;
        }
        // Report error for `codec --scan-dependency -p`. A path input is required.
        ...
        // Report error for `codec --scan-dependency`. A .codeo file input is required.
        if (inputCodeoFile.empty()) {
            (void)diag.DiagnoseRefactor(DiagKindRefactor::driver_source_codeo_empty, DEFAULT_POSITION);
            return false;
        }
    }
    return true;
...
bool GlobalOptions::CheckSanitizerOptions() const
{
    ...
    auto runtimeDirPath = FileUtil::JoinPath(codiraHome, "runtime");
    auto libraryDirPath = FileUtil::JoinPath(FileUtil::JoinPath(runtimeDirPath, "lib"), GetCodiraLibTargetPathName());
    ...
    if (!FileUtil::FileExist(FileUtil::JoinPath(sanitizerPath, "libcodira-runtime" + ext))) {
...
bool GlobalOptions::HandleCODEOExtension(DiagnosticEngine& diag, const std::string& value)
{
    auto maybePath = ValidateInputFilePath(value, DiagKindRefactor::driver_invalid_binary_file);
    if (!maybePath.has_value()) {
        return false;
    }
    if (GetFileExtension(maybePath.value()) != CODEO_EXTENSION) {
        RaiseArgumentUnusedMessage(diag, DiagKindRefactor::driver_warning_not_codeo_file, value, maybePath.value());
        return true;
    }
    if (!inputCodeoFile.empty()) {
...
bool GlobalOptions::HandleCODEExtension(DiagnosticEngine& diag, const std::string& value)
{
    if (compilePackage) {
        (void)diag.DiagnoseRefactor(
    ...
    if (GetFileExtension(maybePath.value()) != CODE_EXTENSION) {
        RaiseArgumentUnusedMessage(diag, DiagKindRefactor::driver_warning_not_code_file, value, maybePath.value());
        return true;
    }
...
bool GlobalOptions::HandleCODEDExtension(DiagnosticEngine& diag, const std::string& value)
{
    if (compilePackage) {
        (void)diag.DiagnoseRefactor(
            DiagKindRefactor::driver_unsupport_compile_package_with_source_file, DEFAULT_POSITION, value);
...
bool GlobalOptions::ProcessInputs(const std::vector<std::string>& inputs)
{
    DiagnosticEngine diag;
    bool ret = true;
    bool needChir = (commonPartCodeo != std::nullopt);
    std::for_each(inputs.begin(), inputs.end(), [this, &ret, &diag, &needChir](const std::string& value) {
        ...
        } else if (ext == CODE_EXTENSION && !compileCoded) {
            ret = HandleCODEExtension(diag, value);
        } else if (ext == BC_EXTENSION) {
            ret = HandleBCExtension(diag, value);
        } else if (ext == CHIR_EXTENSION) {
            needChir = false;
            ret = HandleCHIRExtension(diag, value);
        } else if (ext == CODEO_EXTENSION) {
            ret = HandleCODEOExtension(diag, value);
        } else if (HasCODEDExtension(value) && compileCoded) {
            ret = HandleCODEDExtension(diag, value);
        } else {
...
bool GlobalOptions::TryParseOption(OptionArgInstance& arg, ArgList& argList)
{
    ...
    CODEC_ASSERT(maybeResult.has_value());
...
{
    const std::string codiraPath = "CODIRA_PATH";
    if (environmentVars.find(codiraPath) != environmentVars.end()) {
        environment.codiraPaths = FileUtil::SplitEnvironmentPaths(environmentVars.at(codiraPath));
    }
    if (environmentVars.find(CODIRA_HOME) != environmentVars.end()) {
        environment.codiraHome = FileUtil::GetAbsPath(environmentVars.at(CODIRA_HOME));
    }
...
std::string GlobalOptions::GetCodiraLibHostPathName() const
{
    return host.OSToString() + "_" + host.ArchToString() + "_" + BackendToString(backend);
}

std::string GlobalOptions::GetCodiraLibTargetPathName() const
{
...
void GlobalOptions::SetCompilationCachedPath()
{
    ...
    CODEC_ASSERT(compilationCachedPath != "");
}
...
{
    CODEC_ASSERT(!compilationCachedPath.empty());
...
std::vector<std::string> GlobalOptions::ToSerialized() const
{
    ...
    result.emplace_back(BoolToSerializedString(enableInteropCODEMapping));
