...
std::string Codira::ProcessQuotaMarks(const std::string& value, bool processSingle)
{
    bool inDollar{false};
    auto lCurl = 0;
    auto num = 0;
...
const std::vector<StringPart>& LexerImpl::GetStrParts(const Token& t)
{
    ...
    CODEC_ASSERT(tkKinds.count(t.kind) != 0);
    ...
    CODEC_ASSERT(stringPartsMap.find(t) != stringPartsMap.end() && "stringPartsMap is empty");
...
Token LexerImpl::ScanBackquotedIdentifier(const char* pStart)
{
    ...
    std::function<bool()> scanIdentifier = [this, &res]() {
        ...
        if (IsCODEXIDStart(codePoint)) { // identifier
            ScanUnicodeIdentifierStart(res, codePoint, pCurrent);
            if (res == "_") {
...
{
    CODEC_ASSERT(currentChar == '\'' || currentChar == '"');
...
{
    CODEC_ASSERT(ctx.back() == LexerContext::QUOTE);
    ctx.pop_back();
}
...
{
    CODEC_ASSERT(ctx.back() == LexerContext::NORMAL);
    ctx.pop_back();
}
...
std::pair<Token, bool> LexerImpl::ProcessIllegalCharValue(const char* pStart, size_t beginOffset)
{
    ...
    CODEC_ASSERT(currentChar == '\'' || IsCurrentCharLineTerminator());
    if (currentChar == '\'') {
        // unexpected `'`
        if (success) {
...
Token LexerImpl::ScanSymbol(const char* pStart)
{
    ...
    if (currentChar == '$') {
        ...
        if (IsCODEXIDStart(uch)) { // unicode identifier begin
...
Token LexerImpl::ScanBase()
{
    ...
    // identifier or keyword
    if (IsCODEXIDStart(static_cast<UTF32>(currentChar))) {
