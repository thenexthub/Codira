...
OwnedPtr<Expr> DesugarOptionalChainWithMatchCase(Expr& expr, OwnedPtr<Expr>&& caseExpr, bool isAssign)
{
    ...
    if (opt == nullptr) {
        ...
        CODEC_NULLPTR_CHECK(opt->baseExpr);
        // Create a `MatchExpr`:
...
Ptr<OptionalChainExpr> GetOptionalChainExprForLeftVale(Expr& leftValue)
{
    ...
    while (expr->astKind == ASTKind::PAREN_EXPR) {
        CODEC_NULLPTR_CHECK(StaticAs<ASTKind::PAREN_EXPR>(expr)->expr.get());
...
struct VisitContext {
    ...
    {
        CODEC_ASSERT((!parentStack.empty()) && (parentStack.back() == expected));
