...
OwnedPtr<AssignExpr> CreateUpdate(const ForInExpr& forInExpr, RangeExpr& rangeExpr, VarDecl& index)
{
    ...
    if (rangeExpr.stepExpr) {
        ...
        CODEC_ASSERT(update->rightExpr->ty);
    } else {
...
{
    CODEC_ASSERT(forInExpr.inExpression->astKind == ASTKind::RANGE_EXPR);
    ...
    CODEC_ASSERT(updateTy && updateTy->kind != TypeKind::TYPE_INVALID);
    ...
    CODEC_ASSERT(blockExprTy && blockExprTy->kind != TypeKind::TYPE_INVALID);
...
void TypeChecker::TypeCheckerImpl::ReArrangeForInStringExpr(ASTContext& ctx, ForInExpr& forInExpr)
{
    ...
    CODEC_ASSERT(end->ty && end->ty->kind != TypeKind::TYPE_INVALID);
    // To : 'let pat  = tmp[iter]'
    if (auto vp = DynamicCast<VarPattern*>(forInExpr.pattern.get())) {
        ...
        CODEC_ASSERT(ceTy && ceTy->kind != TypeKind::TYPE_INVALID);
