...
private:
    ...
    void CollectUseOfUnqualifedMember(const Decl& decl, UseInfo& usage) const
    {
        ...
        CODEC_NULLPTR_CHECK(decl.outerDecl->ty);
    ...
    {
        if (vd.type) {
            CODEC_ASSERT(!vd.rawMangleName.empty());
    ...
    void CollectAPIUsage(const FuncDecl& fd)
    {
        ...
        CODEC_ASSERT(fd.funcBody->paramLists.size() == 1 && !fd.rawMangleName.empty());
        auto& usage = info.usages[&fd].apiUsages;
        for (auto& param : fd.funcBody->paramLists[0]->params) {
            CODEC_NULLPTR_CHECK(param->type);
    ...
    {
        CODEC_ASSERT(!id.rawMangleName.empty());
        ...
        for (auto& ctor : ed->constructors) {
            ...
            CODEC_ASSERT(fd->funcBody->paramLists.size() == 1);
            for (auto& param : fd->funcBody->paramLists[0]->params) {
                CODEC_NULLPTR_CHECK(param->type);
                CollectAPIUsage(*param->type, usage);
            }
    ...
    void CollectRelation(const InheritableDecl& id)
    {
        ...
        CODEC_ASSERT(!id.rawMangleName.empty());
        CODEC_NULLPTR_CHECK(id.ty);
        auto decl = Ty::GetDeclPtrOfTy(id.ty);
        CODEC_ASSERT(!decl || !decl->rawMangleName.empty());
        ...
        for (auto& type : id.inheritedTypes) {
            ...
            CODEC_ASSERT(target && !target->rawMangleName.empty());
