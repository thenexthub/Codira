...
{
    // `case Some(v) => throw v`
    CODEC_ASSERT(Ty::IsTyCorrect(freshExc.ty) && freshExc.ty->IsEnum() && freshExc.ty->typeArgs.size() == 1);
...
{
    CODEC_ASSERT(Ty::IsTyCorrect(freshExc.ty) && freshExc.ty->typeArgs.size() == 1);
    auto vp = CreateVarPattern(V_COMPILER, freshExc.ty->typeArgs.front());
    // `if`
...
void TypeChecker::TypeCheckerImpl::DesugarTryWithResourcesExpr(ASTContext& ctx, TryExpr& te)
{
    ...
    CODEC_ASSERT(Ty::IsTyCorrect(tryBlock->ty) && tryBlock->ty->IsUnit());
    for (auto& block : te.catchBlocks) {
        CODEC_NULLPTR_CHECK(block);
