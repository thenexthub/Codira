...
OwnedPtr<FuncDecl> MockSupportManager::GenerateFuncAccessor(FuncDecl& methodDecl)
{
    ...
    for (std::size_t param_idx = 0; param_idx < accessorParams.size(); ++param_idx) {
        ...
        if (param->desugarDecl) {
            ...
            CODEC_ASSERT(originalParam->desugarDecl);
...
OwnedPtr<FuncDecl> MockSupportManager::CreateForeignFunctionAccessorDecl(FuncDecl& funcDecl) const
{
    ...
    CODEC_ASSERT(funcDecl.TestAttr(Attribute::FOREIGN));
    const auto& funcBody = funcDecl.funcBody;

    CODEC_ASSERT(funcDecl.ty->kind == TypeKind::TYPE_FUNC);
...
{
    CODEC_ASSERT(IsFieldOrVariable(kind));
...
{
    CODEC_ASSERT(Is<AssignExpr>(topLevelExpr) ||
    ...
    CODEC_ASSERT(mutBaseExpr->astKind == ASTKind::MEMBER_ACCESS);
...
void MockSupportManager::PrepareInterfaceDecl(InterfaceDecl& interfaceDecl)
{
    ...
    for (auto funcDecl : funcDeclsWithDefault) {
        ...
        CODEC_ASSERT(accessorDecl->TestAttr(Attribute::DEFAULT));
...
void MockSupportManager::PrepareClassWithDefaults(ClassDecl& classDecl, InterfaceDecl& interfaceDecl)
{
    ...
    CODEC_ASSERT(manglerCtxIt != mockUtils->mangler.manglerCtxTable.end());
    ...
    while (accessorDeclIt != accessorDecls.end()) {
        CODEC_ASSERT(originalDeclIt != originalDecls.end());
        ...
        CODEC_ASSERT(accessorDecl);
        CODEC_ASSERT(accessorDecl->identifier == mockUtils->Mangle(*originalFunc) + MockUtils::defaultAccessorSuffix);
        ...
        if (accessorDecl->TestAttr(Attribute::STATIC)) {
            ...
            CODEC_ASSERT(accessorImplDecl->TestAttr(Attribute::ABSTRACT));
        }
    ...
    AST::Expr& originalExpr, Ptr<Decl> outerClassLike, bool isInMockAnnotatedLambda)
{
    ...
    CODEC_ASSERT(!isInMockAnnotatedLambda);
