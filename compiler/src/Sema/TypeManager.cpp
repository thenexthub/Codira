...
{
    CODEC_ASSERT(!uTv.isPlaceholder);
...
bool TypeManager::CheckGenericDeclInstantiation(Ptr<const Decl> d, const std::vector<Ptr<Ty>>& typeArgs)
{
    ...
    if (!genericDecl) {
        CODEC_ASSERT(d->astKind == ASTKind::EXTEND_DECL); // Extend of instantiated type.
        return true;
    }
...
{
    CODEC_ASSERT(decl.TestAttr(Attribute::GENERIC_INSTANTIATED) && HasJavaAttr(decl));
...
{
    CODEC_ASSERT(tyVarPool.count(TypePointer(genTy)) == 0);
...
{
    CODEC_ASSERT(tyVarScopeDepth.count(&tyVar) > 0);
