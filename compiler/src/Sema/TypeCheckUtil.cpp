    ...
    const Ptr<AST::Ty> expectInstParent)
{
    ...
    MultiTypeSubst mts;
    if (expectInstParent) {
        CODEC_ASSERT(src.outerDecl && Is<InheritableDecl>(src.outerDecl));
...
size_t CountOptionNestedLevel(const Ty& ty)
{
    ...
    while (currentTy->IsCoreOptionType()) {
        CODEC_ASSERT(currentTy->typeArgs.size() == 1);
        CODEC_NULLPTR_CHECK(currentTy->typeArgs.front());
    ...
    for (size_t i = 0; i < ce.args.size(); ++i) {
        ...
        if (argName.empty()) {
            ...
            if (i < paramList->params.size()) {
                CODEC_NULLPTR_CHECK(paramList->params[i]);
                ...
                // Codira's variable-length arguments are handled by `ChkVariadicCallExpr`.
        ...
        if (auto res = GetParamTyAccordingToArgName(fd, argName)) {
            ...
            CODEC_ASSERT(index < paramHasTy.size());
...
void MergeSubstPack(SubstPack& target, const SubstPack& src)
{
    for (auto [tvu, tvi] : src.u2i) {
        CODEC_ASSERT(!(target.u2i.count(tvu) > 0 && target.u2i[tvu] != tvi));
        target.u2i[tvu] = tvi;
    }
    ...
    TypeManager& typeManager)
{
    ...
    CODEC_ASSERT(!curComposite || curComposite->node);
