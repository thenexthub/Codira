...
#include "CODEMP/MPTypeCheckerImpl.h"
...
void RemoveInvisibleMember(MemberMap& members, const std::string& currentPkg)
{
    for (auto it = members.cbegin(); it != members.cend();) {
        CODEC_NULLPTR_CHECK(it->second.decl);
...
{
    CODEC_NULLPTR_CHECK(parent.funcBody->paramLists[0]);
    CODEC_NULLPTR_CHECK(child.funcBody->paramLists[0]);
    auto& parentParams = parent.funcBody->paramLists[0]->params;
    auto& childParams = child.funcBody->paramLists[0]->params;
    CODEC_ASSERT(parentParams.size() == childParams.size());
    for (size_t i = 0; i < parentParams.size(); ++i) {
        auto& parentParam = parentParams[i];
        auto& childParam = childParams[i];
        CODEC_NULLPTR_CHECK(parentParam);
        CODEC_NULLPTR_CHECK(childParam);
...
{
    CODEC_ASSERT(index < generic.typeParameters.size());
    CODEC_NULLPTR_CHECK(generic.typeParameters[index]);
    auto gTy = generic.typeParameters[index]->ty;
    for (auto& constraint : generic.genericConstraints) {
        CODEC_NULLPTR_CHECK(constraint->type);
    ...
    const ExtendDecl& curDecl, ExtendDecl& ed, const Ptr<Decl>& extendedDecl)
{
    ...
    for (auto& curDeclSuper : std::as_const(curDecl.inheritedTypes)) {
        ...
        Codira::MPTypeCheckerImpl::GetInheritedTypesWithPlatformImpl(
            ed.inheritedTypes, ed.platformImplementation != nullptr, opts.commonPartCodeo != std::nullopt);
    ...
    const InheritableDecl& curDecl, const MemberSignature& interface, const MemberMap& implDecls)
{
    ...
    for (auto it = foundMembers.first; it != foundMembers.second; ++it) {
        ...
        CODEC_NULLPTR_CHECK(childOuter);
        CODEC_NULLPTR_CHECK(parentOuter);
...
{
    CODEC_ASSERT(!member.inconsistentTypes.empty() && member.decl->IsFuncOrProp());
...
void StructInheritanceChecker::DiagnoseForUnimplementedInterfaces(const MemberMap& members, const Decl& structDecl)
{
    ...
    for (auto& [identifier, member] : members) {
        ...
        bool ignored = notInheritable || member.extendDecl || opts.compileCoded;
        if (ignored) {
...
{
    CODEC_NULLPTR_CHECK(parent.outerDecl);
    ...
    DiagnosticEngine& diag, TypeManager& typeManager)
{
    ...
    CODEC_ASSERT(parentBounds.size() == childBounds.size());
...
void StructInheritanceChecker::CheckGenericTypeArgInfo(const Decl& parent, const Decl& child)
{
    ...
    Codira::CheckGenericTypeBoundsMapped(parent, child, parentBounds, childBounds, diag, typeManager);
}
    ...
    const MemberSignature& parent, const MemberSignature& child) const
{
    ...
    Codira::CheckGenericTypeBoundsMapped(
    ...
    const MemberSignature& parent, const MemberSignature& child) const
{
    ...
    if (sameSignature) {
        CODEC_ASSERT(parentFunc->funcBody && childFunc->funcBody);
