...
void DiagExpectConstExpr(DiagnosticEngine& diag, const Expr& expr, bool isWeak)
{
    ...
    if (expr.astKind == ASTKind::RETURN_EXPR) {
        ...
        CODEC_ASSERT(!expr.ty->typeArgs.empty() && expr.ty->typeArgs.front());
...
{
    CODEC_NULLPTR_CHECK(expr.desugarExpr);
...
private:
    ...
    bool ChkAnnotations(const Decl& decl)
    {
        ...
        for (auto& anno : decl.annotations) {
            ...
            if (HasAnnotationDeclTarget(*anno)) {
                CODEC_ASSERT(anno->args.size() == 1 && anno->args.front() && anno->args.front()->expr &&
    ...
    bool ChkFuncBody(const FuncBody& fb)
    {
        ...
        bool res = true;
        CODEC_ASSERT(fb.paramLists.size() == 1 && fb.paramLists.front());
    ...
    {
        CODEC_NULLPTR_CHECK(expr.leftExpr);
    ...
    {
        CODEC_NULLPTR_CHECK(le.funcBody);
    ...
    bool ChkMatchCase(const MatchCase& mc, bool isWeak)
    {
        ...
        CODEC_NULLPTR_CHECK(mc.exprOrDecls);
    ...
    bool ChkMatchCaseOther(const MatchCaseOther& mco, bool isWeak)
    {
        ...
        CODEC_NULLPTR_CHECK(mco.matchExpr);
        CODEC_NULLPTR_CHECK(mco.exprOrDecls);
        bool res = true;
    ...
    bool ChkTryExpr(const TryExpr& te, bool isWeak)
    {
        ...
        CODEC_NULLPTR_CHECK(te.tryBlock);
    ...
    {
        CODEC_NULLPTR_CHECK(se.baseExpr);
    ...
    {
        CODEC_NULLPTR_CHECK(ae.leftValue);
        CODEC_NULLPTR_CHECK(ae.rightExpr);
