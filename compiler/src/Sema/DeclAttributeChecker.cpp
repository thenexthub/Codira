...
private:
    ...
    void CheckCODEMPAttributesForPropAndFuncDeclInClass(const ClassDecl& cd, Decl& member) const;
...
{
    CODEC_ASSERT(d.IsNominalDecl());
    SetDeclInternal(d);
    for (auto memberDecl : d.GetMemberDeclPtrs()) {
        CODEC_ASSERT(memberDecl);
...
void DeclAttributeChecker::CheckCODEMPAttributesForPropAndFuncDeclInClass(const ClassDecl& cd, Decl& member) const
{
    bool inCODEMP = cd.TestAttr(Attribute::COMMON) || cd.TestAttr(Attribute::PLATFORM);
    bool inAbstractCODEMP = cd.TestAttr(Attribute::ABSTRACT) && inCODEMP;

    bool isCODEMP = member.TestAttr(Attribute::COMMON) || member.TestAttr(Attribute::PLATFORM);
    ...
    if (hasBody && inCODEMP && isAbstract) {
    ...
    if (inAbstractCODEMP && !isCODEMP && !isAbstract && !compilerAddedOrConstructor && !hasBody && !checkedBefore) {
        auto codempKind = cd.TestAttr(Attribute::COMMON) ? "common" : "platform";
        diag.DiagnoseRefactor(DiagKindRefactor::sema_codemp_abstract_class_member_has_no_explicit_modifier, member,
            MakeRange(member.identifier), codempKind, memberKind, codempKind);
    }
}
...
void DeclAttributeChecker::CheckAttributesForPropAndFuncDeclInClass(const ClassDecl& cd, Decl& member) const
{
    ...
    CheckCODEMPAttributesForPropAndFuncDeclInClass(cd, member);
...
void DeclAttributeChecker::CheckPropDeclAttributes(const PropDecl& pd) const
{
    ...
    if (pd.TestAnyAttr(Attribute::ABSTRACT, Attribute::JAVA_MIRROR) || opts.compileCoded) {
        return;
    }
...
void TypeChecker::TypeCheckerImpl::CheckAllDeclAttributes(const ASTContext& ctx)
{
    ...
    for (auto& sym : syms) {
        if (auto decl = AST::As<ASTKind::DECL>(sym->node)) {
            CODEC_ASSERT(!decl->TestAttr(Attribute::IMPORTED) || decl->TestAttr(Attribute::TOOL_ADD) ||
