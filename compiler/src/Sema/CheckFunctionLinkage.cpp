...
inline void MarkVarDeclAbstractLinkage(VarDeclAbstract& vda)
{
    if (vda.IsExportedDecl() || vda.TestAttr(Attribute::IMPLICIT_USED)) {
        vda.linkage = Codira::Linkage::EXTERNAL;
    } else {
        vda.linkage = Codira::Linkage::INTERNAL;
    }
}
...
inline void MarkTypeAliasDeclLinkage(TypeAliasDecl& tad)
{
    if (tad.IsExportedDecl() || tad.TestAttr(Attribute::IMPLICIT_USED)) {
        tad.linkage = Codira::Linkage::EXTERNAL;
    } else {
        tad.linkage = Codira::Linkage::INTERNAL;
    }
}
...
bool IsIntrinsicFile(const File& file)
{
    ...
    static const std::set<std::string> intrinsicFileNames{
        "future.code",
        "runtime_call_throw_exception.code",
    };
 ...
 * should be stored in codeo as long as the type is externally visible.
 * So we should treat the type of the member variable as externally visible and store the latter in the codeo.
 */
inline bool IsMemberInMemLayout(const Decl& member)
{
    CODEC_ASSERT(member.outerDecl);
 ...
 *   - visibility at the Codira level
 ...
 * The former means whether a function can be imported and used (from the CodiraLang level);
