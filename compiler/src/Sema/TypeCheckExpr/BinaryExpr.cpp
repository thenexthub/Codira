...
{
    CODEC_ASSERT(Ty::IsTyCorrect(expr.ty) && expr.ty->IsTuple());
...
{
    CODEC_NULLPTR_CHECK(be.desugarExpr);
    CODEC_ASSERT(be.desugarExpr->astKind == ASTKind::CALL_EXPR);
    auto callexpr = RawStaticCast<CallExpr*>(be.desugarExpr.get());
    CODEC_ASSERT(callexpr->baseFunc->astKind == ASTKind::MEMBER_ACCESS);
    auto ceBaseFunc = RawStaticCast<MemberAccess*>(callexpr->baseFunc.get());
    CODEC_NULLPTR_CHECK(ceBaseFunc->baseExpr);
    CODEC_ASSERT(Ty::IsTyCorrect(ceBaseFunc->baseExpr->ty));
    CODEC_ASSERT(callexpr->args.size() == 1);
    CODEC_NULLPTR_CHECK(callexpr->args[0]->expr);
    CODEC_ASSERT(Ty::IsTyCorrect(callexpr->args[0]->expr->ty));
    ...
    if (answers > 1) {
        ...
        if (CanSkipDiag(exponent)) {
            ...
            if (exTys.size() == 1) {
                ...
                CODEC_ASSERT(!exTys.empty() && exTys.back());
...
{
    CODEC_ASSERT(be.leftExpr != nullptr && Ty::IsTyCorrect(be.leftExpr->ty) && be.leftExpr->ty->IsTuple());
    CODEC_ASSERT(be.rightExpr != nullptr && Ty::IsTyCorrect(be.rightExpr->ty) && be.rightExpr->ty->IsTuple());
    ...
    for (size_t i = 0; i < leftElements.size(); i++) {
        CODEC_ASSERT(i < rightElements.size());
...
bool TypeChecker::TypeCheckerImpl::ChkFlowExpr(ASTContext& ctx, Ptr<Ty> target, BinaryExpr& be)
{
    ...
    CODEC_ASSERT(be.desugarExpr && be.desugarExpr->astKind == ASTKind::CALL_EXPR);
