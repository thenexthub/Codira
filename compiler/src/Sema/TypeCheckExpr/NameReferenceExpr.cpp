...
#include "Codira/Macro/TestEntryConstructor.h"
...
bool TypeChecker::TypeCheckerImpl::ChkRefExpr(ASTContext& ctx, Ty& target, NameReferenceExpr& refNode)
{
    ...
    // Overloading check.
    uint32_t matched = 0;
    CODEC_ASSERT(refNode.IsReferenceExpr());
    ...
    if (matched > 1) {
        ...
        if (Ty::IsTyCorrect(refNode.ty) && refNode.ty->HasQuestTy()) {
            CODEC_ASSERT(refNode.GetTarget());
...
void TypeChecker::TypeCheckerImpl::InferRefExpr(ASTContext& ctx, RefExpr& re)
{
    ...
    CODEC_ASSERT(re.ref.target); // 're.ref.target' should be set in 'ModifyTargetOfRef'.
...
{
    CODEC_ASSERT(ma.baseExpr);
    CODEC_NULLPTR_CHECK(ma.curFile);
