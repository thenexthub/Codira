...
{
    CODEC_NULLPTR_CHECK(ty.declPtr);
...
bool IsSealedLike(const Ty& ty)
{
    if (ty.IsClass()) {
        ...
        CODEC_NULLPTR_CHECK(classTy.declPtr);
...
private:
    ...
    {
        CODEC_ASSERT(Ty::IsTyCorrect(constPattern.ty));
    ...
    {
        std::vector<DestructedPattern> subPatterns;
        CODEC_ASSERT(goalTys.size() == patterns.size());
        for (size_t i = 0; i < goalTys.size(); i++) {
            CODEC_ASSERT(Ty::IsTyCorrect(goalTys[i]));
            CODEC_NULLPTR_CHECK(patterns[i]);
    ...
    {
        CODEC_ASSERT(Ty::IsTyCorrect(tuplePattern.ty) && tuplePattern.ty->IsTuple());
    ...
    static DestructedPattern FromEnumPattern(TypeManager& typeManager, EnumPattern& enumPattern)
    {
        ...
        CODEC_ASSERT(Ty::IsTyCorrect(instTy));
    ...
    {
        CODEC_ASSERT(Ty::IsTyCorrect(typePattern.ty));
    ...
    static std::vector<DestructedPattern> SpecializeWildcard(AST::Ty& ty, const Constructor& ctor)
    {
        ...
        if (ctor.Kind() == ConstructorKind::TUPLE) {
            ...
            CODEC_ASSERT(AST::Ty::IsTyCorrect(&ty));
            CODEC_ASSERT(ty.IsTuple());
            CODEC_ASSERT(ctor.Arity() == ty.typeArgs.size());
            for (size_t i = 0; i < ctor.Arity(); i++) {
                Ptr<AST::Ty> argTy = ty.typeArgs[i];
                CODEC_ASSERT(Ty::IsTyCorrect(argTy));
        ...
        if (ctor.Kind() == ConstructorKind::ENUM) {
            ...
            std::transform(ctorFuncTy.paramTys.cbegin(), ctorFuncTy.paramTys.cend(), std::back_inserter(subPatterns),
                [](auto paramTy) {
                    CODEC_ASSERT(Ty::IsTyCorrect(paramTy));
...
public:
    ...
    {
        CODEC_ASSERT(!stack.empty());
        CODEC_ASSERT(Head().Ctor().Kind() == ConstructorKind::OR);
...
private:
    ...
    static void SplitWildcardForSealed(std::vector<Constructor>& ctors, ClassLikeTy& sealedTy)
    {
        ...
        for (Ptr<Ty> subTy : directSubtypes) {
            CODEC_ASSERT(Ty::IsTyCorrect(subTy));
        ...
        if (sealedTy.IsClass()) {
            ...
            CODEC_NULLPTR_CHECK(ct.declPtr);
    ...
    {
        DestructedPattern& head = vec.Head();
        CODEC_ASSERT(head.Ctor().Kind() == ConstructorKind::WILDCARD);
    ...
    std::vector<PatternStack> FindWitnessesForExpandedOr(std::vector<PatternStack>&& vecs) const
    {
        ...
        if (!totalWitnesses.empty()) {
            ...
            for (Ptr<Node> unreachable : unreachables) {
                CODEC_NULLPTR_CHECK(unreachable);
