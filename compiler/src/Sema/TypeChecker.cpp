...
void AddReturnTypeForPropMemDecl(PropDecl& pd)
{
    for (auto& getter : pd.getters) {
        CODEC_NULLPTR_CHECK(getter);
        CODEC_NULLPTR_CHECK(getter->funcBody);
    ...
    for (auto& setter : pd.setters) {
        ...
        CODEC_NULLPTR_CHECK(setter);
        CODEC_NULLPTR_CHECK(setter->funcBody);
...
{
    CODEC_ASSERT(fb.funcDecl); // Ctor must have related funcDecl.
    ...
    AST::Ty& ty, bool needSubstituteGeneric, const TypeSubst& typeMapping)
{
    ...
    switch (ty.kind) {
        ...
        case TypeKind::TYPE_VARRAY: {
            ...
            CODEC_ASSERT(!typeArgs.empty() && typeArgs[0] != nullptr);
...
{
    CODEC_NULLPTR_CHECK(target.declPtr->type);
...
Ptr<Ty> TypeChecker::TypeCheckerImpl::Synthesize(ASTContext& ctx, Ptr<Node> node)
{
    ...
    switch (node->astKind) {
        ...
        // Effect handlers are only enabled in the CODENative backend, for now
...
#endif // CODIRA_CODEGEN_CODENATIVE_BACKEND
    ...
    CODEC_ASSERT(!Ty::IsTyCorrect(node->ty) || As<ASTKind::EXPR>(node) == nullptr ||
...
{
    CODEC_ASSERT(!fd.funcBody->body->body.empty()); // Caller guarantees.
    ...
    const ASTContext& ctx, const Ty& interfaceTy, Decl& target, const MultiTypeSubst& typeMapping)
{
    ...
    for (auto& member : members) {
        ...
        if (member->IsFunc() && target.IsFunc()) {
            ...
            CODEC_NULLPTR_CHECK(memberFuncDecl);
            CODEC_NULLPTR_CHECK(targetFuncDecl);
...
std::vector<Ptr<ASTContext>> TypeChecker::TypeCheckerImpl::PreTypeCheck(const std::vector<Ptr<AST::Package>>& pkgs)
{
    ...
    std::for_each(pkgs.begin(), pkgs.end(), [this, &contexts](Ptr<Package> pkg) {
        CODEC_NULLPTR_CHECK(pkg); // There should not be any nullptr Ptr<Package> in pkgs.
...
void TypeChecker::TypeCheckerImpl::PostTypeCheck(std::vector<Ptr<ASTContext>>& contexts)
{
    ...
    for (auto& ctx : contexts) {
        ...
        // Because of the codelint checking policy, desugar of propDecl should be done in sema stage for now.
...
void TypeChecker::TypeCheckerImpl::PrepareTypeCheck(ASTContext& ctx, Package& pkg)
{
    ...
    mpImpl->PrepareTypeCheck4CODEMP(pkg);
...
void TypeChecker::TypeCheckerImpl::CheckWhetherHasProgramEntry()
{
    ...
    if (!ci->invocation.globalOptions.CompileExecutable() || ci->invocation.globalOptions.enableCompileTest ||
        ...
        !mainFunctionMap.empty() || ci->invocation.globalOptions.compileCoded) {
        return;
    }
...
{
    CODEC_ASSERT(fd.funcBody && fd.funcBody->body);
    ...
    std::unordered_set<Ptr<ClassLikeDecl>>& passedClassLikeDecls)
{
    ...
    CODEC_NULLPTR_CHECK(ity->declPtr);
