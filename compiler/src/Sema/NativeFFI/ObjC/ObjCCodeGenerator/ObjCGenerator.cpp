...
constexpr auto CODE_IMPORT = "\"Codira.h\"";
...
constexpr auto DELETE_FUNC_NAME = "deleteCODEObject";
constexpr auto DEALLOC_FUNC_NAME = "dealloc";
constexpr auto INIT_CODE_RUNTIME_NAME = "InitCODERuntime";
...
constexpr auto LOAD_LIB_FUNC_NAME = "LoadCODELibraryWithInit";
...
constexpr auto CODE_DLL_HANLDE = "CODEWorldDLHandle";
constexpr auto CODE_RTIME_PARAMS = "defaultCODERuntimeParams";
constexpr auto CODE_RTIME_PARAMS_LOG_LEVEL = "defaultCODERuntimeParams.logParam.logLevel";
...
constexpr auto FAIL_INIT_CODE_RT_MSG = "@\"ERROR: Failed to initialize Codira runtime\"";
constexpr auto FAIL_INIT_CODE_LIB = "@\"ERROR: Failed to init codeworld library \"";
constexpr auto FAIL_OPEN_CODE_LIB = "@\"ERROR: Failed to open codeworld library \"";
constexpr auto FAIL_FIND_SYM_1 = "@\"ERROR: Failed to find ";
constexpr auto FAIL_FIND_SYM_2 = " symbol in codeworld\"";
...
/*
    Main access point to translation of CODE class to Objective C. Generates two files - .h and .m
    ...
    #import "A.h"
            ...
    static void (*CODEImpl_ObjC_A_goo)(size_t) = NULL;
            ...
    @implementation A
    + (void)initialize {
            ...
...
void ObjCGenerator::Generate()
{
    ...
    AddWithIndent(GenerateImport(CODE_IMPORT), GenerationTarget::SOURCE);
    ...
    AddWithIndent(GenerateStaticReference(CODE_DLL_HANLDE, string(VOID_TYPE) + "*", NULL_KW),
        GenerationTarget::SOURCE);
    AddWithIndent(GenerateStaticReference(CODE_RTIME_PARAMS,
    ...
    AddWithIndent(GenerateAssignment(CODE_RTIME_PARAMS_LOG_LEVEL, RTLOG_ERROR) + ";", GenerationTarget::SOURCE);
    AddWithIndent(GenerateIfStatement(GenerateCCall(INIT_CODE_RUNTIME_NAME,
        std::vector<std::string>{"&" + string(CODE_RTIME_PARAMS)}),
        E_OK, NOT_EQ_OP),
        GenerationTarget::SOURCE, OptionalBlockOp::OPEN);
    AddWithIndent(GenerateCCall(NSLOG_FUNC_NAME,
        std::vector<std::string>{FAIL_INIT_CODE_RT_MSG}) + ";",
        GenerationTarget::SOURCE);
    ...
    auto codeLibName = Native::FFI::GetCodiraLibName(codeLibOutputPath, decl->fullPackageName, false);
    // Crutch that solves issue when codeLibOutputPath is dir or not provided.
    if (codeLibName.find("lib", 0) != 0) {
        codeLibName = "lib" + codeLibName;
    }
    size_t extIdx = codeLibName.find_last_of(".");
    if (extIdx == std::string::npos || codeLibName.substr(extIdx) != ".dylib") {
        codeLibName += ".dylib"; // what if we compile on linux target?
    }

    codeLibName = "\"" + codeLibName + "\"";
    AddWithIndent(GenerateIfStatement(GenerateCCall(LOAD_LIB_FUNC_NAME,
        std::vector<std::string>{codeLibName}),
        E_OK, NOT_EQ_OP),
        GenerationTarget::SOURCE, OptionalBlockOp::OPEN);
    AddWithIndent(GenerateCCall(NSLOG_FUNC_NAME,
        std::vector<std::string>{FAIL_INIT_CODE_LIB}) + ";",
        GenerationTarget::SOURCE);
    AddWithIndent(GenerateCCall(EXIT_FUNC_NAME, std::vector<std::string>{"1"}) + ";",
        GenerationTarget::SOURCE);
    ...
    AddWithIndent(
        GenerateIfStatement(
            ...
            GenerateAssignment(
                ...
                GenerateCCall(
                    DLOPEN_FUNC_NAME,
                    std::vector<std::string>{codeLibName, RTLD_LAZY})) +
            ")",
            NULL_KW,
            EQ_CHECK_OP),
        GenerationTarget::SOURCE,
    ...
    AddWithIndent(GenerateCCall(NSLOG_FUNC_NAME,
        std::vector<std::string>{FAIL_OPEN_CODE_LIB}) + ";",
        GenerationTarget::SOURCE);
    AddWithIndent(GenerateCCall(NSLOG_FUNC_NAME,
    ...
    AddWithIndent(GenerateFunctionDeclaration(ObjCFunctionType::INSTANCE, VOID_TYPE,
        DELETE_FUNC_NAME),
        GenerationTarget::BOTH, OptionalBlockOp::OPEN);
    AddWithIndent(GenerateDefaultFunctionImplementation(ctx.nameGenerator.GenerateDeleteCodeObjectName(*decl),
...
/*
 *  return CODEImpl_ObjC_codeworld_A_goo(arg1, arg2, ... arg3);
 */
    ...
    const std::string& name, const Ty& retTy, const ArgsList args, const ObjCFunctionType type) const
{
    ...
    if (ctx.typeMapper.IsValidObjCMirror(retTy) || ctx.typeMapper.IsObjCImpl(retTy)) {
        result += "(__bridge " + ctx.typeMapper.Code2ObjCForObjC(retTy) + ")";
    }
    result += name + "(";
...
void ObjCGenerator::GenerateStaticFunctionsReferences()
{
    for (auto& declPtr : ctx.genDecls) {
        ...
        if (declPtr->astKind == ASTKind::FUNC_DECL) {
            ...
            CODEC_ASSERT(funcDecl.ty->IsFunc());
            ...
            const auto retType = ctx.typeMapper.IsValidObjCMirror(retTy) || ctx.typeMapper.IsObjCImpl(retTy) ||
                ...
                ? VOID_POINTER_TYPE
                : ctx.typeMapper.Code2ObjCForObjC(retTy);
            std::string argTypes = "";
            for (auto& arg: funcDecl.funcBody->paramLists[0]->params) {
                argTypes += ctx.typeMapper.Code2ObjCForObjC(*arg->ty) + ",";
            }
            // trim unnecessary ","
...
/*
 *  static type (*CODEImpl_ObjC_A_funcName)(arg1Type, ... argNType) = NULL;
 */
...
/*
 *  if ((CODEImpl_ObjC_A_name = dlsym(CODEWorldDLHandle, "CODEImpl_ObjC_A_name")) == NULL) {
 *      NSLog(@"ERROR: Failed to find CODEImpl_ObjC_A_name symbol in codeworld");
 *      exit(1);
 *  }
 */
    ...
    AddWithIndent(GenerateIfStatement(
        ...
        GenerateAssignment(fName,
            GenerateCCall(DLSYM_FUNC_NAME,
                std::vector<std::string>{CODE_DLL_HANLDE, "\"" + fName + "\""})) +
        ")",
        NULL_KW, EQ_CHECK_OP),
        GenerationTarget::SOURCE, OptionalBlockOp::OPEN);
    AddWithIndent(
...
void ObjCGenerator::GenerateInterfaceDecl()
{
    ...
    bool isClassInheritedFromClass = superClassPtr && superClassPtr->identifier.Val() != Codira::OBJECT_NAME;
    ...
    FunctionListFormat format, const ObjCFunctionType type)
{
    ...
    for (size_t i = 0; i < paramLists[0]->params.size(); i++) {
        ...
        switch (format) {
            case FunctionListFormat::DECLARATION:
                ...
                genParams += "(" + MapCODETypeToObjCType(cur) + ")";
            ...
            case FunctionListFormat::STATIC_REF:
                ...
                genParams += ")";
                break;
            case FunctionListFormat::CODIRA_DECL:
    ...
    if (format == FunctionListFormat::STATIC_REF || format == FunctionListFormat::CODIRA_DECL) {
        if (paramLists[0]->params.size() == 0 && type != ObjCFunctionType::STATIC) {
            genParams += VOID_POINTER_TYPE;
        }
        genParams += ")";
...
void ObjCGenerator::AddProperties()
{
    ...
    for (OwnedPtr<Decl>& declPtr : decl->GetMemberDecls()) {
        ...
        const std::string& type = ctx.typeMapper.Code2ObjCForObjC(*varDecl.ty);
...
void ObjCGenerator::AddConstructors()
{
    ...
    for (OwnedPtr<Decl>& declPtr : decl->GetMemberDecls()) {
        ...
        CODEC_ASSERT(selectorComponents.size() > 0);
        // wrapper name MUST use generated ctor
        const auto codeWrapperName = ctx.nameGenerator.GenerateInitCodeObjectName(*ctor);
    ...
    for (OwnedPtr<Decl>& declPtr : decl->GetMemberDecls()) {
        ...
        if (declPtr->astKind == ASTKind::FUNC_DECL &&
            ...
            if (funcDecl.funcBody && funcDecl.funcBody->retType) {
                ...
                const std::string& retType = MapCODETypeToObjCType(funcDecl.funcBody->retType);
                const auto selectorComponents = ctx.nameGenerator.GetObjCDeclSelectorComponents(funcDecl);
    ...
    const std::vector<OwnedPtr<FuncParamList>>& paramLists, bool withRegistryId)
{
    ...
    if (!paramLists.empty() && paramLists[0]) {
        for (size_t i = 0; i < paramLists[0]->params.size(); i++) {
            ...
            result.push_back(std::pair<std::string, std::string>(MapCODETypeToObjCType(cur), name));
        }
    }
    return result;
}
...
std::string ObjCGenerator::MapCODETypeToObjCType(const OwnedPtr<Type>& type)
{
    if (!type) {
        return UNSUPPORTED_TYPE;
    }

    return ctx.typeMapper.Code2ObjCForObjC(*type->ty);
}

std::string ObjCGenerator::MapCODETypeToObjCType(const OwnedPtr<FuncParam>& param)
{
    if (!param) {
        return UNSUPPORTED_TYPE;
    }

    return ctx.typeMapper.Code2ObjCForObjC(*param->type->ty);
}
