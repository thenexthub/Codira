...
void DesugarMirrors::DesugarCtor(InteropContext& ctx, ClassLikeDecl& mirror, FuncDecl& ctor)
{
    ...
    CODEC_NULLPTR_CHECK(ctor.funcBody);
    CODEC_ASSERT(!ctor.funcBody->paramLists.empty());
    ...
    CODEC_ASSERT_WITH_MSG(mirror.astKind == ASTKind::CLASS_DECL,
...
void DesugarMirrors::DesugarTopLevelFunc(InteropContext& ctx, FuncDecl& func)
{
    ...
    std::transform(methodTy->paramTys.begin(), methodTy->paramTys.end(), std::back_inserter(cParamTys),
        [&ctx](auto& paramTy) { return ctx.typeMapper.Code2CType(paramTy); });
    auto cFuncTy = ctx.typeManager.GetFunctionTy(
        cParamTys,
        ctx.typeMapper.Code2CType(methodTy->retTy),
...
{
    CODEC_ASSERT(!prop.getters.empty());
...
{
    CODEC_ASSERT(prop.TestAttr(Attribute::MUT));
    CODEC_ASSERT(!prop.setters.empty());
...
{
    CODEC_ASSERT(field.TestAttr(Attribute::MUT));
    CODEC_ASSERT(!field.setters.empty());
