...
{
    CODEC_ASSERT(typeMapper.IsValidObjCMirror(*entity->ty) || typeMapper.IsObjCImpl(*entity->ty));
...
{
    CODEC_ASSERT(typeMapper.IsValidObjCMirror(ty) || typeMapper.IsObjCImpl(ty));
...
OwnedPtr<Expr> ASTFactory::UnwrapEntity(OwnedPtr<Expr> expr)
{
    ...
    if (typeMapper.IsObjCPointer(*expr->ty)) {
        CODEC_ASSERT(expr->ty->typeArgs.size() == 1);
        ...
        return CreateUnsafePointerCast(
            ...
            typeMapper.Code2CType(elementType)
        );
    }

    CODEC_ASSERT(expr->ty->IsPrimitive() || Ty::IsCStructType(*expr->ty));
    return expr;
}
...
OwnedPtr<Expr> ASTFactory::WrapEntity(OwnedPtr<Expr> expr, Ty& wrapTy)
{
    if (typeMapper.IsValidObjCMirror(wrapTy)) {
        CODEC_ASSERT(expr->ty->IsPointer());
        auto classLikeTy = StaticCast<ClassLikeTy>(&wrapTy);
        CODEC_ASSERT_WITH_MSG(classLikeTy->commonDecl->astKind == ASTKind::CLASS_DECL,
    ...
    if (typeMapper.IsObjCPointer(wrapTy)) {
        CODEC_ASSERT(expr->ty->IsPointer());
        CODEC_ASSERT(wrapTy.typeArgs.size() == 1);
    ...
    CODEC_ASSERT(expr->ty->IsPrimitive() || Ty::IsCStructType(*expr->ty));
    CODEC_ASSERT(wrapTy.IsPrimitive() || Ty::IsCStructType(wrapTy));
    return expr;
}
    ...
    Ptr<Ty> ty)
{
    ...
    CODEC_ASSERT(optTy.IsCoreOptionType());
    auto optArgTy = optTy.typeArgs[0];

    auto vp = CreateVarPattern(Codira::V_COMPILER, optArgTy);
    vp->curFile = curFile;
    ...
    OwnedPtr<Expr> entity, Ptr<Ty> ty)
{
    ...
    return CreateOptionMatch(std::move(entity),
        ...
        }, typeMapper.Code2CType(actualTy));
}
/*
    WRAP [HADNLE -> OPTION]
    match (handle.isNull) {
...
{
    CODEC_NULLPTR_CHECK(ctor.outerDecl);
    CODEC_ASSERT_WITH_MSG(ctor.outerDecl->astKind == ASTKind::CLASS_LIKE_DECL,
...
OwnedPtr<FuncDecl> ASTFactory::CreateInitCodeObject(const Decl& target, FuncDecl& ctor, bool generateForOneWayMapping)
{
    ...
    std::transform(ctorParams.begin(), ctorParams.end(), std::back_inserter(wrapperParams), [this](auto& p) {
        return CreateFuncParam(p->identifier.GetRawText(), nullptr, nullptr, typeMapper.Code2CType(p->ty));
    });
    ...
    if (!generateForOneWayMapping) {
        CODEC_ASSERT_WITH_MSG(!wrapperParams.empty(), "wrapperParams should not be empty");
    ...
    auto wrapperName = nameGenerator.GenerateInitCodeObjectName(ctor);
...
OwnedPtr<FuncDecl> ASTFactory::CreateDeleteCodeObject(Decl& target, bool generateForOneWayMapping)
{
    auto registryIdTy = bridge.GetRegistryIdTy();
    ...
    auto funcName = nameGenerator.GenerateDeleteCodeObjectName(target);
...
OwnedPtr<FuncDecl> ASTFactory::CreateMethodWrapper(FuncDecl& method)
{
    ...
    std::transform(originParams.begin(), originParams.end(), std::back_inserter(wrapperParams), [this](auto& p) {
        auto convertedParamTy = typeMapper.Code2CType(p->ty);
    ...
    auto retWrapperTy = typeManager.GetFunctionTy(wrapperParamTys, typeMapper.Code2CType(retTy), {.isC = true});
...
OwnedPtr<FuncDecl> ASTFactory::CreateGetterWrapper(PropDecl& prop)
{
    ...
    auto wrapperTy = typeManager.GetFunctionTy(wrapperParamTys, typeMapper.Code2CType(prop.ty), {.isC = true});
...
OwnedPtr<FuncDecl> ASTFactory::CreateSetterWrapper(PropDecl& prop)
{
    ...
    auto convertedPropTy = typeMapper.Code2CType(prop.ty);
    ...
    CODEC_ASSERT(wrapperParams.size() <= 2);
    {
...
OwnedPtr<FuncDecl> ASTFactory::CreateGetterWrapper(VarDecl& field)
{
    ...
    auto wrapperTy = typeManager.GetFunctionTy(wrapperParamTys, typeMapper.Code2CType(field.ty), {.isC = true});
...
OwnedPtr<FuncDecl> ASTFactory::CreateSetterWrapper(VarDecl& field)
{
    ...
    auto registryIdParam = CreateFuncParam(REGISTRY_ID_IDENT, CreateType(registryIdTy), nullptr, registryIdTy);
    auto registryIdParamRef = CreateRefExpr(*registryIdParam);
    auto convertedFieldTy = typeMapper.Code2CType(field.ty);
...
OwnedPtr<FuncDecl> ASTFactory::CreateImplCtor(ClassDecl& impl, FuncDecl& from)
{
    ...
    CODEC_NULLPTR_CHECK(impl.GetSuperClassDecl());
...
{
    CODEC_ASSERT(decl.astKind == ASTKind::CLASS_DECL);
    CODEC_ASSERT(TypeMapper::IsValidObjCMirror(*decl.ty));
...
{
    CODEC_ASSERT(origin.TestAttr(Attribute::CONSTRUCTOR));
    CODEC_NULLPTR_CHECK(origin.funcBody);

    const auto& originParamLists = origin.funcBody->paramLists;
    CODEC_ASSERT(!originParamLists.empty());
    ...
    for (auto& member : declArg.GetMemberDeclPtrs()) {
        if (auto fd = As<ASTKind::FUNC_DECL>(member); fd && IsGeneratedCtor(*fd)) {
            ...
            CODEC_ASSERT(!fdParamLists.empty());
...
{
    CODEC_ASSERT(nativeHandle->ty->IsPointer());
    ...
    auto ty = typeArg->ty;
    CODEC_ASSERT(TypeMapper::IsObjCImpl(*ty));
    std::vector<OwnedPtr<FuncArg>> args;
    args.emplace_back(CreateFuncArg(std::move(nativeHandle)));
    ...
    FuncDecl& fd, OwnedPtr<Expr> nativeHandle, std::vector<OwnedPtr<Expr>> rawArgs)
{
    ...
    return CreateObjCRuntimeMsgSendCall(
        std::move(nativeHandle), objcname, typeMapper.Code2CType(StaticCast<FuncTy>(fd.ty)->retTy), std::move(rawArgs));
}
...
OwnedPtr<Expr> ASTFactory::CreatePropGetterCallViaMsgSend(PropDecl& pd, OwnedPtr<Expr> nativeHandle)
{
    ...
    return CreateObjCRuntimeMsgSendCall(std::move(nativeHandle), objcname, typeMapper.Code2CType(pd.ty), {});
}
...
OwnedPtr<Expr> ASTFactory::CreatePropSetterCallViaMsgSend(PropDecl& pd, OwnedPtr<Expr> nativeHandle, OwnedPtr<Expr> arg)
{
    ...
    return CreateObjCRuntimeMsgSendCall(
        std::move(nativeHandle), objcname, typeMapper.Code2CType(pd.ty), Nodes<Expr>(std::move(arg)));
}

OwnedPtr<Expr> ASTFactory::CreateAutoreleasePoolScope(Ptr<Ty> ty, std::vector<OwnedPtr<Node>> actions)
{
    CODEC_ASSERT(typeMapper.IsObjCCompatible(*ty));
    CODEC_ASSERT(!actions.empty());
    ...
    if (ty->IsPrimitive() || typeMapper.IsObjCPointer(*ty)) {
        ...
        auto unwrappedTy = typeMapper.Code2CType(ty);
...
{
    CODEC_ASSERT(expr->ty->IsPointer());
    CODEC_ASSERT(Ty::IsMetCType(*elementType));
