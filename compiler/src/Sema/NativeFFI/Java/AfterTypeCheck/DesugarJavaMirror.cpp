...
void JavaDesugarManager::DesugarJavaMirrorConstructor(FuncDecl& ctor, FuncDecl& generatedCtor)
{
    ...
    CODEC_ASSERT(ctor.TestAttr(Attribute::CONSTRUCTOR) && ctor.TestAttr(Attribute::JAVA_MIRROR));
...
void JavaDesugarManager::AddJavaMirrorMethodBody(const ClassLikeDecl& mirror, FuncDecl& fun, OwnedPtr<Expr> javaRefCall)
{
    ...
    CODEC_ASSERT_WITH_MSG(!fun.funcBody->paramLists.empty(), "paramLists cannot be empty");
...
{
    CODEC_ASSERT(!fun.TestAttr(Attribute::CONSTRUCTOR) &&
...
void JavaDesugarManager::InsertJavaMirrorPropSetter(PropDecl& prop)
{
    ...
    CODEC_ASSERT_WITH_MSG(!prop.setters.empty(), "expected at least one setter");
    CODEC_ASSERT_WITH_MSG(!prop.setters[0]->funcBody->paramLists.empty(), "setter paramLists cannot be empty");
    CODEC_ASSERT_WITH_MSG(!prop.setters[0]->funcBody->paramLists[0]->params.empty(), "setter params cannot be empty");
...
void JavaDesugarManager::ReplaceCallsWithArrayJavaEntityGet(File& file)
{
    Walker(&file, Walker::GetNextWalkerID(), [this, &file](auto node) {
        ...
        CODEC_ASSERT_WITH_MSG(!ma->baseExpr->ty->typeArgs.empty(), "JArray type must be generic");
        ...
        CODEC_ASSERT(arrayJavaEntityGetDecl);
...
void JavaDesugarManager::ReplaceCallsWithArrayJavaEntitySet(File& file)
{
    Walker(&file, Walker::GetNextWalkerID(), [this, &file](auto node) {
        ...
        static auto arrayJavaEntitySetDecl = lib.FindArrayJavaEntitySetDecl(
            *As<ASTKind::CLASS_DECL>(funcDecl->outerDecl));
        CODEC_ASSERT(arrayJavaEntitySetDecl);
        auto newCallExpr = ASTCloner::Clone(callExpr);
