...
constexpr auto INTEROPLIB_CFFI_NEW_JAVA_PROXY_OBJECT_FOR_CODEMAPPING_ID = "Java_CFFI_newJavaProxyObjectForCODEMapping";
...
constexpr auto INTEROPLIB_JNI_DELETE_CODE_OBJECT_DECL_ID = "Java_CFFI_deleteCODEObject";
...
constexpr auto INTEROPLIB_CFFI_JAVA_STRING_TO_CODIRA = "Java_CFFI_JavaStringToCodira";
constexpr auto INTEROPLIB_CFFI_CODIRA_STRING_TO_JAVA = "Java_CFFI_CodiraStringToJava";
...
Ptr<FuncDecl> InteropLibBridge::GetNewJavaProxyObjectForCODEMappingDecl()
{
    return GetInteropLibDecl<ASTKind::FUNC_DECL>(INTEROPLIB_CFFI_NEW_JAVA_PROXY_OBJECT_FOR_CODEMAPPING_ID);
}
...
Ptr<FuncDecl> InteropLibBridge::GetDeleteCODEObjectDecl()
{
    return GetInteropLibDecl<ASTKind::FUNC_DECL>(INTEROPLIB_JNI_DELETE_CODE_OBJECT_DECL_ID);
}
...
Ptr<FuncDecl> InteropLibBridge::GetJavaStringToCodira()
{
    return GetInteropLibDecl<ASTKind::FUNC_DECL>(INTEROPLIB_CFFI_JAVA_STRING_TO_CODIRA);
}

Ptr<FuncDecl> InteropLibBridge::GetCodiraStringToJava()
{
    return GetInteropLibDecl<ASTKind::FUNC_DECL>(INTEROPLIB_CFFI_CODIRA_STRING_TO_JAVA);
}
...
OwnedPtr<CallExpr> InteropLibBridge::CreateJavaEntityCall(Ptr<File> file)
{
    ...
    for (auto& decl : javaEntityDecl->body->decls) {
        if (auto ctor = As<ASTKind::FUNC_DECL>(decl.get())) {
            CODEC_ASSERT_WITH_MSG(!ctor->funcBody->paramLists.empty(), "at least one paramLists expected");
...
OwnedPtr<Expr> InteropLibBridge::CreateJavaEntityFromOptionMirror(OwnedPtr<Expr> option, ClassLikeDecl& mirror)
{
    ...
    CODEC_ASSERT_WITH_MSG(!option->ty->typeArgs.empty(), "Option type must be generic");
...
OwnedPtr<Expr> InteropLibBridge::CreateJavaEntityCall(OwnedPtr<Expr> arg)
{
    ...
    if (auto classLTy = DynamicCast<ClassLikeTy*>(arg->ty)) {
        ...
        CODEC_ASSERT_WITH_MSG(!arg->ty->typeArgs.empty(), "Option type must be generic");
...
OwnedPtr<Expr> InteropLibBridge::WrapJavaEntity(OwnedPtr<Expr> codeExpr)
{
    CODEC_NULLPTR_CHECK(codeExpr->curFile);
    if (codeExpr->ty->kind == TypeKind::TYPE_UNIT) {
        return CreateJavaEntityCall(codeExpr->curFile);
    }

    return CreateJavaEntityCall(std::move(codeExpr));
}
    ...
    const std::string& constructorSignature, std::vector<OwnedPtr<Expr>> args)
{
    ...
    CODEC_ASSERT_WITH_MSG(!jclassInitFuncDecl->funcBody->paramLists.empty(), "paramLists cannot be empty");
    CODEC_ASSERT_WITH_MSG(jclassInitFuncDecl->funcBody->paramLists[0]->params.size() > 1,
        "at least 2 parameters expected for classInit function");
    CODEC_ASSERT(jclassInitFuncDecl->funcBody->paramLists[0] && jclassInitFuncDecl->funcBody->paramLists[0]->params[1]);
    CODEC_ASSERT_WITH_MSG(!callNestFuncDecl->funcBody->paramLists.empty(), "paramLists cannot be empty");
    CODEC_ASSERT_WITH_MSG(!callNestFuncDecl->funcBody->paramLists[0]->params.empty(),
        "at least 1 parameter expected for callNestInit function");
    CODEC_ASSERT(callNestFuncDecl->funcBody->paramLists[0] && callNestFuncDecl->funcBody->paramLists[0]->params[0]);
    ...
    OwnedPtr<Expr> jniEnv, FuncParamList& params, const Ptr<GenericParamDecl> genericParam)
{
    ...
    CODEC_ASSERT_WITH_MSG(!params.params.empty(), "mandatory size param is absent");
...
OwnedPtr<CallExpr> InteropLibBridge::CreateCFFINewJavaCFFINewJavaProxyObjectForCODEMappingCall(
    OwnedPtr<Expr> jniEnv, OwnedPtr<Expr> entity, std::string name, bool withMarkerParam)
{
    auto curFile = entity->curFile;
    static auto funcDecl = GetNewJavaProxyObjectForCODEMappingDecl();
    if (!funcDecl) {
        return nullptr;
    }
...
{
    CODEC_ASSERT(kind == ArrayOperationKind::GET || kind == ArrayOperationKind::SET);
    CODEC_ASSERT_WITH_MSG(params.params.size() > 1, "expected at least two parameters");

    static auto javaEntityKindDecl = GetJavaEntityKindDecl();
    static auto jObject = utils.GetJObjectDecl();
    static auto javaRef = GetJavaRefField(*jObject);
    CODEC_ASSERT(javaRef);
    auto curFile = genericParam->curFile;
    ...
    const MemberJNISignature& signature, FuncParamList& params, File& curFile)
{
    ...
    CODEC_NULLPTR_CHECK(obj->curFile);
    for (auto& param : params.params) {
...
OwnedPtr<CallExpr> InteropLibBridge::CreateDeleteCODEObjectCall(
    OwnedPtr<Expr> env, OwnedPtr<Expr> self, OwnedPtr<Expr> getWeakRef, Ptr<Ty> codeTy)
{
    auto curFile = self->curFile;
    auto funcDecl = GetDeleteCODEObjectDecl();
    if (!funcDecl) {
        return nullptr;
    }
    ...
    fdRef->instTys.push_back(codeTy);
...
OwnedPtr<CallExpr> InteropLibBridge::CreateJavaStringToCodiraCall(OwnedPtr<Expr> env, OwnedPtr<Expr> jstring)
{
    auto curFile = jstring->curFile;
    auto funcDecl = GetJavaStringToCodira();
    if (!funcDecl) {
        return nullptr;
    }
...
{
    auto curFile = self->curFile;
    CODEC_ASSERT(ty->IsCoreOptionType());
    ...
    auto fdRef = WithinFile(CreateRefExpr(*funcDecl), curFile);
    CODEC_ASSERT_WITH_MSG(!ty->typeArgs.empty(), "Option type expected to be generic");
...
[[maybe_unused]] OwnedPtr<Expr> CreateJavaEntityIsNullExpr(OwnedPtr<Expr> entity, Ptr<StructDecl> entityDecl)
{
    ...
    CODEC_ASSERT_WITH_MSG(!isNullProp->getters.empty(), "getters cannot be empty");
    ...
    OwnedPtr<Expr> entity, Ptr<Ty> ty, const ClassLikeDecl& mirror, bool toRaw)
{
    ...
    auto curFile = entity->curFile;
    CODEC_NULLPTR_CHECK(curFile);
    CODEC_ASSERT_WITH_MSG(!ty->typeArgs.empty(), "Option type must be generic");
    auto declTy = ty->typeArgs[0];
    auto decl = Ty::GetDeclOfTy(declTy);
    CODEC_ASSERT(IsMirror(*decl) || declTy->IsString() || (toRaw && IsImpl(*decl)));
...
OwnedPtr<Expr> InteropLibBridge::UnwrapJavaArrayEntity(OwnedPtr<Expr> entity, Ptr<Ty> ty, const ClassLikeDecl& mirror)
{
    ...
    varPattern->varDecl->curFile = curFile;
    auto varPatternRef = WithinFile(CreateRefExpr(*(varPattern->varDecl)), curFile);
    CODEC_ASSERT_WITH_MSG(!mirror.generic->typeParameters.empty(), "JArray type must be generic");
...
OwnedPtr<Expr> InteropLibBridge::UnwrapJavaEntity(OwnedPtr<Expr> entity, Ptr<Ty> ty, const Decl& outerDecl, bool toRaw)
{
    ...
    if (ty->IsString()) {
        return CreateJavaStringToCodiraCall(CreateGetJniEnvCall(curFile), std::move(entity));
    }

    if (ty->IsCoreOptionType()) {
        auto classLikeDecl = DynamicCast<const ClassLikeDecl*>(&outerDecl);
        CODEC_NULLPTR_CHECK(classLikeDecl);
        CODEC_ASSERT_WITH_MSG(!ty->typeArgs.empty(), "Option type must be generic");
        auto actualTy = ty->typeArgs[0];
