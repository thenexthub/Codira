...
struct JavaInteropTypeChecker {
    bool isImpl;
    bool isCODEMappingTypeCheck = false;
    DiagnosticEngine& diag;

    auto GetJavaClassKind() const
    {
        if (isCODEMappingTypeCheck) {
            return "CODEMapping";
        }
        if (isImpl) {
            return "@JavaImpl";
    ...
    inline bool IsSupported(const Ty& ty, Position pos)
    {
        ...
        // Use IsCODEMapping to check a ty is supported or not.
        if (isCODEMappingTypeCheck && IsCODEMapping(ty)) {
            return true;
        }
    ...
    inline void CheckCODEMappingMethodTypes(FuncDecl& fd)
    {
        if (fd.funcBody && fd.funcBody->retType &&
            ...
            diag.DiagnoseRefactor(DiagKindRefactor::sema_codemapping_method_ret_unsupported, *node,
                Ty::ToString(fd.funcBody->retType->ty), GetJavaClassKind());
            fd.EnableAttr(Attribute::IS_BROKEN);
        }

        CheckCODEMappingCompatibleParamTypes(fd);
    }

    inline void CheckCODEMappingCompatibleParamTypes(FuncDecl& fdecl)
    {
        if (!fdecl.funcBody) {
            return;
        }
        for (auto& paramList : fdecl.funcBody->paramLists) {
            for (auto& param : paramList->params) {
                ...
                if (isOptionArg || !IsJavaCompatible(*param->ty)) {
                    diag.DiagnoseRefactor(DiagKindRefactor::sema_codemapping_method_arg_not_supported, *param);
                    fdecl.EnableAttr(Attribute::IS_BROKEN);
                    fdecl.outerDecl->EnableAttr(Attribute::HAS_BROKEN);
                    fdecl.outerDecl->EnableAttr(Attribute::IS_BROKEN);
                }
...
void JavaInteropManager::CheckNonJavaSuperType(ClassLikeDecl& decl) const
{
    ...
    for (const auto superDecl : superDecls) {
        if (!IsMirror(*superDecl) && !IsImpl(*superDecl) && !superDecl->ty->IsObject() && !superDecl->ty->IsAny()) {
            DiagJavaDeclCannotInheritPureCodiraType(diag, decl);
...
void JavaInteropManager::CheckCODEMappingDeclSupportRange(Decl& decl)
{
    switch (decl.astKind) {
        ...
        case ASTKind::ENUM_DECL:
            break;
        default:
            diag.DiagnoseRefactor(DiagKindRefactor::sema_codemapping_decl_not_supported, MakeRange(decl.identifier),
                std::string{decl.identifier});
            decl.EnableAttr(Attribute::IS_BROKEN);
    }
}

void JavaInteropManager::CheckCODEMappingType(Decl& decl)
{
    if (!IsCODEMapping(decl)) {
        return;
    }

    CheckCODEMappingDeclSupportRange(decl);
    JavaInteropTypeChecker checker{
        .isImpl = false,
        .isCODEMappingTypeCheck = true,
        .diag = diag,
    };
    for (auto& member : decl.GetMemberDecls()) {
        switch (member->astKind) {
            case ASTKind::FUNC_DECL:
                ...
                checker.CheckCODEMappingMethodTypes(*StaticAs<ASTKind::FUNC_DECL>(member.get()));
                break;
            default:
                continue;
        }
