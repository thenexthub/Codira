 ...
 */
#ifndef CODIRA_SEMA_NATIVE_FFI_JAVA_DESUGAR_MANAGER
#define CODIRA_SEMA_NATIVE_FFI_JAVA_DESUGAR_MANAGER
...
enum class DesugarCODEImplStage : uint8_t { BEGIN, IMPL_GENERATE, IMPL_DESUGAR, TYPECHECKS, END };
...
public:
    ...
    void ProcessCODEImplStage(DesugarCODEImplStage stage, File& file);

    /**
     * Stage 1-2: generate constructors and native init/deinit/method call functions (callable from java) for CODEMapping
     * data structure
     */
    void GenerateInCODEMapping(File& file);

    /**
     * Stage 3: desugar in CODEMapping data structure
     */
    void DesugarInCODEMapping(File& file);
...
private:
    ...
    /**
     * This constructor could be called from codira
     * before [ctor]:
     *   init(a1: A, a2: B, ..., an: N) {
     *       <super(...)> // optional
     *       ...body...
    ...
    void GenerateFuncParamsForNativeDeleteCodeObject(
    ...
    std::string GetJniInitCodeObjectFuncName(const FuncDecl& ctor, bool isGeneratedCtor);

    std::string GetJniInitCodeObjectFuncNameForVarDecl(const VarDecl& ctor) const;

    std::string GetJniDeleteCodeObjectFuncName(const Decl& decl) const;

    /**
     * @C public func Java_<type_signature>_deleteCODEObject(env: JNIEnv_ptr, obj: jobject, self: jlong): Unit {
     *     Java_CFFI_deleteCODEObject<type>(env, self, { objToDelete: <type> => objToDelete.javaref })
     * }
     */
    OwnedPtr<Decl> GenerateNativeDeleteCodeObjectFunc(ClassLikeDecl& javaImpl, VarDecl& javaWeakRefField);

    /**
     * @C public func Java_Vector_deleteCODEObject(env: JNIEnv_ptr, clazz: jclass, self: jlong): Unit {
     *      withExceptionHandling(env) {
     *          Java_CFFI_removeFromRegistry(self)
     *     }
     *   }
     */
    OwnedPtr<Decl> GenerateCODEMappingNativeDeleteCodeObjectFunc(Decl& decl);

    // A helper function to for the ctor of Enum.
    void GenerateNativeInitCODEObjectEnumCtor(AST::EnumDecl& enumDecl);

    /**
     * @C public func Java_<enum_identifier>_initCODEObject(env: JNIEnv_ptr, obj: jobject): jlong {
           return withExceptionHandling(env, { =>
               return Java_CFFI_put_to_registry_1(Enum.VarDecl)
           })
     * }
     */
    OwnedPtr<Decl> GenerateNativeInitCodeObjectFuncForEnumCtorNoParams(AST::EnumDecl& enumDecl, AST::VarDecl& ctor);

    /**
     * when arg ClassLikeDecl is true(e.g. calss decl)::
     *
     * @C public func Java_<type_signature>_initCODEObject{mangled}(env: JNIEnv_ptr, obj: jobject, args...): jlong {
     *     registry.put(
     ...
     * }
     */
    OwnedPtr<Decl> GenerateNativeInitCodeObjectFunc(FuncDecl& ctor, bool isClassLikeDecl);
    ...
    /**
     * For CType ty, ty is returned. For mirrors, impls and CODEMapping jobject is returned
     */
     ...
     *   super(Java_CFFI_CodiraStringToJava(env, s))
     * }
     */
    ...
    void GenerateForCODEStructMapping(AST::StructDecl* structDecl);
    void GenerateForCODEEnumMapping(AST::EnumDecl& enumDecl);
    void GenerateForCODEInterfaceMapping(AST::InterfaceDecl& interfaceDecl);
...
#endif // CODIRA_SEMA_NATIVE_FFI_JAVA_DESUGAR_MANAGER
