...
inline Ptr<CallExpr> TryGetSuperCall(const FuncDecl& ctor)
{
    ...
    CODEC_ASSERT_WITH_MSG(!ctor.funcBody->body->body.empty(), "body must have at least one node");
    ...
    Ptr<Expr> expr, const std::vector<OwnedPtr<FuncParam>>& params, std::vector<Ptr<FuncParam>>& usingParams)
{
    ...
    if (!ids.empty()) {
        ...
        paramIds = psep + Codira::Utils::JoinStrings(ids, psep);
    }
...
/**
 * Unwrap a refExpr of native func param into Codira type.
 * 1. Primitive Type
...
OwnedPtr<Expr> JavaDesugarManager::UnwrapRefExpr(OwnedPtr<RefExpr> ref, Ptr<Ty> targetTy, const ClassLikeDecl& decl)
{
    ...
    // JavaEntity -> Codira Type
    return lib.UnwrapJavaEntity(std::move(entity), targetTy, decl);
}

/**
 * Wrap the codira expr into native type.
 * 1. Primitive Type
    ...
    std::vector<OwnedPtr<Node>>&& nodes, OwnedPtr<Expr> expr, FuncParam& env, const ClassLikeDecl& decl)
{
    ...
    if (retTy->IsPrimitive()) {
        ...
        // Codira Type -> JavaEntity -> jobject
...
void JavaDesugarManager::DesugarJavaImplConstructor(FuncDecl& ctor, FuncDecl& parentCtor)
{
    ...
    CODEC_ASSERT(ctor.funcBody);
    CODEC_ASSERT(ctor.funcBody->paramLists.size() == 1);
...
OwnedPtr<FuncDecl> JavaDesugarManager::GenerateJavaImplConstructor(FuncDecl& sampleCtor, ClassLikeDecl& parent)
{
    ...
    CODEC_ASSERT(parentCtor);
    CODEC_ASSERT(parentCtor->funcBody->paramLists[0]->params.size() == 1); // Java_CFFI_JavaEntity
...
std::string JavaDesugarManager::GetJniMethodName(const FuncDecl& method)
{
    ...
    CODEC_ASSERT_WITH_MSG(!method.funcBody->paramLists.empty(), "paramLists cannot be empty");
...
std::string JavaDesugarManager::GetJniInitCodeObjectFuncName(const FuncDecl& ctor, bool isGeneratedCtor)
{
    std::string fqname = GetJavaFQName(*(ctor.outerDecl));
    MangleJNIName(fqname);
    CODEC_ASSERT_WITH_MSG(!ctor.funcBody->paramLists.empty(), "paramLists cannot be empty");
    auto mangledFuncName = GetMangledJniInitCodeObjectFuncName(mangler, ctor.funcBody->paramLists[0]->params,
                                                             isGeneratedCtor);
    MangleJNIName(mangledFuncName);
...
std::string JavaDesugarManager::GetJniInitCodeObjectFuncNameForVarDecl(const AST::VarDecl& ctor) const
{
    std::string fqname = GetJavaFQName(*(ctor.outerDecl));
    MangleJNIName(fqname);
    auto mangledFuncName = ctor.identifier.Val();
    MangleJNIName(mangledFuncName);
    return "Java_" + fqname + "_" + mangledFuncName + "initCODEObject";
}

std::string JavaDesugarManager::GetJniDeleteCodeObjectFuncName(const Decl& decl) const
{
    std::string fqname = GetJavaFQName(decl);
    MangleJNIName(fqname);

    return "Java_" + fqname + "_deleteCODEObject";
}

OwnedPtr<Decl> JavaDesugarManager::GenerateNativeDeleteCodeObjectFunc(ClassLikeDecl& javaImpl, VarDecl& javaWeakRefField)
{
    auto curFile = javaImpl.curFile;
    CODEC_NULLPTR_CHECK(javaImpl.curFile);
    std::vector<OwnedPtr<FuncParam>> params;
    FuncParam* jniEnvPtrParam = nullptr;
    OwnedPtr<Expr> selfParamRef;
    GenerateFuncParamsForNativeDeleteCodeObject(javaImpl, params, jniEnvPtrParam, selfParamRef);
    /* lambda:
        { objToDelete: <type> =>
    ...
    */
    ...
    auto deleteCodeObjCall = lib.CreateDeleteCODEObjectCall(
        WithinFile(CreateRefExpr(*jniEnvPtrParam), curFile), std::move(selfParamRef), std::move(lambda), javaImpl.ty);
    auto wrappedNodesLambda = WrapReturningLambdaExpr(typeManager, Nodes(std::move(deleteCodeObjCall)));
    Ptr<Ty> unitTy = typeManager.GetPrimitiveTy(TypeKind::TYPE_UNIT).get();
    auto funcName = GetJniDeleteCodeObjectFuncName(javaImpl);
    std::vector<OwnedPtr<FuncParamList>> paramLists;
    paramLists.push_back(CreateFuncParamList(std::move(params)));
...
void JavaDesugarManager::DesugarJavaImpl(ClassDecl& jimpl)
{
    ...
    CODEC_NULLPTR_CHECK(parentMirror);
    CODEC_ASSERT(IsMirror(*parentMirror));
        ...
        if (auto cdecl = As<ASTKind::CLASS_DECL>(decl.get())) {
            ...
            if (JavaSourceCodeGenerator::IsDeclAppropriateForGeneration(*cdecl)) {
                ...
                auto codegen = JavaSourceCodeGenerator(cdecl, mangler, javaCodeGenPath, fileJ,
                    GetCodiraLibName(outputLibPath, cdecl->GetFullPackageName()));
                codegen.Generate();
            }
        }
    }
...
OwnedPtr<Expr> JavaDesugarManager::CreateIsInstanceCall(Ptr<VarDecl> jObjectVar, Ptr<Ty> classTy, Ptr<File> curFile)
{
    ...
    CODEC_ASSERT_WITH_MSG(!isInstanceOfDecl->funcBody->paramLists.empty(), "paramLists cannot be empty");
    CODEC_ASSERT_WITH_MSG(isInstanceOfDecl->funcBody->paramLists[0]->params.size() > 2,
        "at least 3 parameters expected for isInstanceOf function");
    ...
    Ptr<File> curFile)
{
    ...
    if (castDecl->TestAttr(Attribute::JAVA_MIRROR)) {
        ...
        // getFromRegistry(..)
        CODEC_ASSERT(castDecl->TestAttr(Attribute::JAVA_MIRROR_SUBTYPE));
...
void JavaDesugarManager::DesugarMatchCase(MatchCase& matchCase)
{
    ...
    for (auto pat : typePatterns) {
        ...
        CODEC_NULLPTR_CHECK(originalTy);
...
{
    CODEC_ASSERT(call.baseFunc && call.baseFunc->astKind == ASTKind::MEMBER_ACCESS);
    auto& ma = *StaticAs<ASTKind::MEMBER_ACCESS>(call.baseFunc.get());
    CODEC_ASSERT(ma.baseExpr && ma.baseExpr->astKind == ASTKind::REF_EXPR);
    auto& ref = *StaticAs<ASTKind::REF_EXPR>(ma.baseExpr.get().get());
    CODEC_ASSERT(ref.isSuper);
    CODEC_ASSERT(call.resolvedFunction && call.resolvedFunction->outerDecl);
    auto& outerDecl = *call.resolvedFunction->outerDecl;
    CODEC_ASSERT(IsMirror(outerDecl));
...
{
    CODEC_ASSERT(classDecl && IsImpl(*classDecl));
    ...
    if (!generatedCtors.empty()) {
        ...
        generatedDecls.push_back(GenerateNativeDeleteCodeObjectFunc(jimpl, javaWeakRefField));

        for (auto& generatedCtor : generatedCtors) {
            generatedDecls.push_back(GenerateNativeInitCodeObjectFunc(*generatedCtor, true));
