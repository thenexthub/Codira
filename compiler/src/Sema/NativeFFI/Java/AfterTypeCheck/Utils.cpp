...
Ptr<VarDecl> GetJavaRefField(ClassDecl& mirrorLike)
{
    if (mirrorLike.TestAttr(Attribute::JAVA_MIRROR_SUBTYPE)) {
        ...
        if (!superClass || !superClass->TestAnyAttr(Attribute::JAVA_MIRROR, Attribute::JAVA_MIRROR_SUBTYPE)) {
            CODEC_ABORT(); // neither mirror or mirror subtype are super type of [mirror]
        }

        return GetJavaRefField(*superClass);
    }

    CODEC_ASSERT(mirrorLike.TestAttr(  Attribute::JAVA_MIRROR));
    CODEC_ASSERT(IsJObject(mirrorLike));
    CODEC_ASSERT(mirrorLike.body);
    ...
    CODEC_ABORT(); // Internal error: mirror is not @JavaMirror or weak reference field was not generated!
    return nullptr;
}
...
{
    CODEC_ASSERT(mirror.TestAnyAttr(Attribute::JAVA_MIRROR_SUBTYPE, Attribute::JAVA_MIRROR));
...
{
    CODEC_ASSERT(mirrorLike.TestAnyAttr(
...
{
    CODEC_ASSERT(mirrorLike.astKind == ASTKind::CLASS_DECL);
    CODEC_ASSERT(mirrorLike.TestAnyAttr(
        Attribute::JAVA_MIRROR, Attribute::JAVA_MIRROR_SUBTYPE));
    auto thisRef = CreateThisRef(&mirrorLike, mirrorLike.ty, curFile);
...
OwnedPtr<Expr> CreateJavaRefCall(OwnedPtr<Expr> expr, FuncDecl& javaRefGetter)
{
    ...
    CODEC_ASSERT(expr->ty->IsClassLike());
    CODEC_ASSERT(StaticCast<ClassLikeTy*>(expr->ty)->commonDecl->TestAnyAttr(
        Attribute::JAVA_MIRROR, Attribute::JAVA_MIRROR_SUBTYPE));
...
OwnedPtr<Expr> CreateJavaRefCall(OwnedPtr<Expr> expr)
{
    ...
    CODEC_ASSERT(classLikeTy->commonDecl);
...
{
    CODEC_ASSERT(mirror.TestAttr(Attribute::JAVA_MIRROR));
...
{
    CODEC_ASSERT(mirror.astKind == AST::ASTKind::CLASS_DECL);
    if (mirror.TestAttr(Attribute::JAVA_MIRROR_SUBTYPE) && !mirror.TestAttr(Attribute::JAVA_MIRROR)) {
        ...
        CODEC_ABORT(); // impl class must have mirror parent
    }
    CODEC_ASSERT(mirror.TestAttr(Attribute::JAVA_MIRROR));
    if (auto mirrorClass = DynamicCast<ClassDecl*>(&mirror)) {
        auto curCtor = GetGeneratedConstructorInMirror(*mirrorClass);
        CODEC_ASSERT(curCtor);
        return curCtor;
    }
...
Ptr<std::string> GetJavaMirrorAnnoAttr(const ClassLikeDecl& decl)
{
    for (auto& anno : decl.annotations) {
        ...
        CODEC_ASSERT(anno->args.size() < 2); // It is empty if < 2
        if (anno->args.empty()) {
            break;
        }
...
{
    CODEC_ASSERT(IsJArray(*decl.outerDecl));
...
std::string Utils::GetJavaObjectTypeName(const Ty& ty)
{
    ...
    if (ty.kind == TypeKind::TYPE_CLASS || ty.kind == TypeKind::TYPE_INTERFACE) {
        ...
        if (IsJArray(cldecl)) {
            CODEC_ASSERT_WITH_MSG(!ty.typeArgs.empty(), "JArray type must be generic");
            return GetJavaObjectTypeName(*ty.typeArgs[0]) + "[]";
        }
 ...
 */
std::string Utils::GetJavaClassNormalizeSignature(const Ty& codetype) const
{
    CODEC_ASSERT(!IsJArray(codetype));
    return NormalizeJavaSignature(GetJavaFQName(*StaticCast<ClassLikeTy&>(codetype).commonDecl));
}

/*
 * Should be called only on java compatible type or a function type over java compatible types
 */
std::string Utils::GetJavaTypeSignature(const Ty& codetype)
{
    std::string jsig;
    ...
    switch (codetype.kind) {
        ...
        case TypeKind::TYPE_STRUCT: {
            ...
            if (!codetype.IsStructArray()) { break; }
            [[fallthrough]]; // for Array<T> - fallback
        }
        case TypeKind::TYPE_ARRAY: jsig = "[" + GetJavaTypeSignature(*codetype.typeArgs[0]); break;
        case TypeKind::TYPE_ENUM: {
            if (!codetype.IsCoreOptionType()) {
                break;
            };
            CODEC_ASSERT_WITH_MSG(!codetype.typeArgs.empty(), "Option type must be generic");
            auto& argTy = *codetype.typeArgs[0];
            if (IsJArray(argTy)) {
        ...
        case TypeKind::TYPE_INTERFACE:
            if (IsJArray(*StaticCast<ClassLikeTy&>(codetype).commonDecl)) {
                jsig = "[" + GetJavaTypeSignature(*codetype.typeArgs[0]);
            } else {
                jsig = "L" + NormalizeJavaSignature(
                    GetJavaFQName(*StaticCast<ClassLikeTy&>(codetype).commonDecl)
                ) + ";";
            }
            break;
        case TypeKind::TYPE_FUNC: {
            auto& funcTy = *StaticCast<FuncTy>(&codetype);
            jsig = "(";
        ...
        default: CODEC_ABORT(); break; // method must be called only on java-compatible types
    }
...
{
    std::string name("initCODEObject");
...
{
    std::string name = funcName + "initCODEObject";
...
bool IsCODEMapping(const Ty& ty)
{
    ...
    if (auto structTy = DynamicCast<StructTy*>(&ty)) {
        return structTy->decl && IsCODEMapping(*structTy->decl);
    } else if (auto enumTy = DynamicCast<EnumTy*>(&ty)) {
        return enumTy->decl && IsCODEMapping(*enumTy->decl);
    }
    return false;
}
    ...
    Ptr<Ty> ty)
{
    ...
    CODEC_ASSERT_WITH_MSG(!optTy.typeArgs.empty(), "Option type must be generic");
...
constexpr auto NATIVE_CONSTRUCTOR_MARKER_PACKAGE_NAME = "codira.lang.internal";
