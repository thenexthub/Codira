...
using TokenKind = Codira::TokenKind;
...
{
    if (declArg.TestAttr(Attribute::JAVA_CODE_MAPPING) && declArg.astKind == ASTKind::STRUCT_DECL) {
        return true;
    }
    return false;
}
...
JavaSourceCodeGenerator::JavaSourceCodeGenerator(
    ...
    : AbstractSourceCodeGenerator(outputFilePath), decl(decl), codeLibName(std::move(codeLibName)), mangler(mangler)
{
}

JavaSourceCodeGenerator::JavaSourceCodeGenerator(Decl* decl, const BaseMangler& mangler,
    const std::optional<std::string>& outputFolderPath, const std::string& outputFileName, std::string codeLibName)
    : AbstractSourceCodeGenerator(
          outputFolderPath.value_or(JavaSourceCodeGenerator::DEFAULT_OUTPUT_DIR), outputFileName),
      decl(decl),
      codeLibName(std::move(codeLibName)),
      mangler(mangler)
{
}
...
void JavaSourceCodeGenerator::AddHeader()
{
    ...
    header += Join(imports, "\n", mapper) + "\n";
    header += "import static codira.lang.LibraryLoader.loadLibrary;\n";
...
std::string JavaSourceCodeGenerator::MapCODETypeToJavaType(
    const Ptr<Ty> ty, std::set<std::string>* javaImports, const std::string* curPackageName, bool isNativeMethod)
{
    if (ty->IsCoreOptionType()) {
        return MapCODETypeToJavaType(ty->typeArgs[0], javaImports, curPackageName, isNativeMethod);
    }
    ...
    switch (ty->kind) {
        ...
        case TypeKind::TYPE_CLASS:
            if (IsJArray(*declTy)) {
                return MapCODETypeToJavaType(ty->typeArgs[0], javaImports, curPackageName) + "[]";
            } else if (isNativeMethod && declTy && IsCODEMapping(*declTy)) {
                return "long";
            }
        ...
        case TypeKind::TYPE_ENUM:
            if (isNativeMethod && declTy && IsCODEMapping(*declTy)) {
                return "long";
            }
            javaType = ty->name;
            break;
    ...
    CODEC_ASSERT(!javaType.empty());

    return javaType;
}

std::string JavaSourceCodeGenerator::MapCODETypeToJavaType(const OwnedPtr<Type>& type, std::set<std::string>* javaImports,
    const std::string* curPackageName, bool isNativeMethod)
{
    CODEC_ASSERT(type && type->ty);
    return MapCODETypeToJavaType(type->ty, javaImports, curPackageName, isNativeMethod);
}

std::string JavaSourceCodeGenerator::MapCODETypeToJavaType(const OwnedPtr<FuncParam>& param,
    std::set<std::string>* javaImports, const std::string* curPackageName, bool isNativeMethod)
{
    CODEC_ASSERT(param && param->type && param->type->ty);
    return MapCODETypeToJavaType(param->type->ty, javaImports, curPackageName, isNativeMethod);
}
...
void JavaSourceCodeGenerator::AddClassDeclaration()
{
    ...
    if (auto classDecl = As<ASTKind::CLASS_DECL>(decl)) {
        ...
        if (isClassInheritedFromClass) {
            res += " extends ";
            res += MapCODETypeToJavaType(superClassPtr->ty, &imports, &classDecl->fullPackageName);
        }
        ...
        if (implementedInterfacesCnt > 0) {
            ...
            const std::function<std::string(Ptr<InterfaceTy>)>& transformer = [imp, package](Ptr<InterfaceTy> p) {
                return MapCODETypeToJavaType(p, imp, package);
            };
...
void JavaSourceCodeGenerator::AddLoadLibrary()
{
    ...
    AddWithIndent(TAB2, "loadLibrary(\"" + codeLibName + "\");");
    AddWithIndent(TAB, "}\n");
}
...
void JavaSourceCodeGenerator::AddProperties()
{
    for (OwnedPtr<Decl>& declPtr : decl->GetMemberDecls()) {
        if (declPtr->astKind == ASTKind::PROP_DECL && !declPtr->TestAttr(Attribute::COMPILER_ADD)) {
            ...
            CODEC_ASSERT_WITH_MSG(!propDecl.getters.empty(), "property must have at least one getter");
            const OwnedPtr<FuncDecl>& funcDecl = propDecl.getters[0];
            const std::string type =
                MapCODETypeToJavaType(funcDecl->funcBody->retType, &imports, &decl->fullPackageName, false);

            std::string varDecl = GetJavaMemberName(propDecl);
            std::string varDeclSuffix = varDecl;
            CODEC_ASSERT_WITH_MSG(!varDeclSuffix.empty(), "identifier cannot be an empty string");
    ...
    const std::vector<OwnedPtr<FuncParam>>& params, bool isNativeMethod)
{
    ...
    std::function<std::string(const OwnedPtr<FuncParam>& ptr)> mapper = [imp, curPackage, isNativeMethod](
        ...
        return MapCODETypeToJavaType(cur, imp, curPackage, isNativeMethod) + " " + cur->identifier.Val();
    };
    ...
    const std::vector<OwnedPtr<FuncParamList>>& paramLists, bool isNativeMethod)
{
    ...
    std::function<std::string(const OwnedPtr<FuncParam>& ptr)> mapper = [imp, curPackage, isNativeMethod](
                                                                            const OwnedPtr<FuncParam>& cur) {
        CODEC_ASSERT(cur && cur->type && cur->type->ty);
        std::string res = MapCODETypeToJavaType(cur, imp, curPackage, isNativeMethod) + " " + cur->identifier.Val();
        if (auto ty = Ty::GetDeclOfTy(cur->type->ty)) {
            bool castToId = isNativeMethod && IsCODEMapping(*ty);
            res += castToId ? "Id" : "";
        }
        return res;
    };
...
std::string JavaSourceCodeGenerator::GenerateConstructorDecl(const FuncDecl& func, bool isForCodira)
{
    std::string declaration;
    if (!isForCodira && func.TestAttr(Attribute::PUBLIC)) {
...
{
    CODEC_ASSERT(arg.expr->desugarExpr->astKind == ASTKind::REF_EXPR);
    auto ref = StaticCast<const RefExpr*>(arg.expr->desugarExpr.get());
    CODEC_NULLPTR_CHECK(ref->ref.target);
    CODEC_ASSERT(ref->ref.target->astKind == ASTKind::FUNC_DECL);
    ...
    CODEC_ASSERT(pos != std::string::npos);
    // The native id should be `superC0A0...`
    id = id.substr(pos + 1);
    // Decode using parameter ids
    const std::string sep = "P";
    std::vector<std::string> parts = Codira::Utils::SplitString(id.substr(keyword.length()), sep);
    std::vector<std::string> args;
    std::vector<std::string> nativeParams;
    auto mpTy = [this](const Ptr<Ty> ty) { return MapCODETypeToJavaType(ty, &imports, &decl->fullPackageName); };
    // Skip first part.
    for (size_t i = 1; i < parts.size(); i++) {
        auto pid = Codira::Stoi(parts[i]);
        CODEC_ASSERT(pid.has_value());
    ...
    std::string superCall = id + "(" + Codira::Utils::JoinStrings(args, ", ") + ")";
    std::string nativeFnDecl = "public static native " + mpTy(arg.ty) + " " + id +
        "(" + Codira::Utils::JoinStrings(nativeParams, ", ") + ");";
    ...
    const CallExpr& call, const std::vector<OwnedPtr<FuncParam>>& params, std::vector<std::string>& nativeArgs)
{
    ...
    return "super(" + Codira::Utils::JoinStrings(args, ", ") + ");";
}
...
void JavaSourceCodeGenerator::AddConstructor(const FuncDecl& ctor, const std::string& superCall, bool isForCodira)
{
    AddWithIndent(TAB, GenerateConstructorDecl(ctor, isForCodira) + " {");
    // super call
    if (!superCall.empty()) {
    ...
    if (!isForCodira) {
        ...
        std::string selfInit = "self = " + GetMangledJniInitCodeObjectFuncName(mangler, params, false) + "(";
        if (ctor.funcBody) {
...
void JavaSourceCodeGenerator::AddConstructor(const FuncDecl& ctor)
{
    ...
    // For Codira side constructor with marker.
    AddConstructor(ctor, superCall, true);
    // Add native init
    AddNativeInitCODEObject(params);
...
void JavaSourceCodeGenerator::AddAllCtorsForCODEMappingEnum(const EnumDecl& enumDecl)
{
    ...
    for (auto& constructor : enumDecl.constructors) {
        ...
        if (constructor->astKind == ASTKind::FUNC_DECL) {
            ...
            CODEC_NULLPTR_CHECK(funcDecl);
            auto funcName = funcDecl->identifier;
            CODEC_ASSERT_WITH_MSG(!funcDecl->funcBody->paramLists.empty(), "paramLists cannot be empty");
            auto& params = funcDecl->funcBody->paramLists[0]->params;
            AddWithIndent(TAB, declaration);
            auto nativeFuncName = GetMangledJniInitCodeObjectFuncNameForEnum(mangler, params, funcName);
            ...
            auto nativeFuncName = varName + "initCODEObject";
...
void JavaSourceCodeGenerator::AddConstructors()
{
    for (OwnedPtr<Decl>& declPtr : decl->GetMemberDecls()) {
        ...
        if (IsCODEMapping(*decl) && !fd->TestAttr(Attribute::PUBLIC)) {
            continue;
        }
    ...
    if (IsCODEMapping(*decl)) {
        if (auto enumDecl = As<ASTKind::ENUM_DECL>(decl)) {
            AddPrivateCtorForCODEMappringEnum();
            AddAllCtorsForCODEMappingEnum(*enumDecl);
        } else {
            AddPrivateCtorForCODEMappring();
        }
    }
}
...
void JavaSourceCodeGenerator::AddInstanceMethod(const FuncDecl& funcDecl)
{
    ...
    const std::string retType = MapCODETypeToJavaType(funcDecl.funcBody->retType, &imports, &decl->fullPackageName);
    ...
    const std::function<std::string(const OwnedPtr<FuncParam>&)>& mapper = [](const OwnedPtr<FuncParam>& p) {
        CODEC_ASSERT(p && p->type && p->type->ty);
        std::string res = p->identifier.Val();
        if (auto ty = Ty::GetDeclOfTy(p->type->ty)) {
            res += IsCODEMapping(*ty) ? ".self" : "";
        }
...
void JavaSourceCodeGenerator::AddMethods()
{
    ...
    for (OwnedPtr<Decl>& declPtr : decl->GetMemberDecls()) {
        if (IsCODEMapping(*decl) && !declPtr->TestAttr(Attribute::PUBLIC)) {
            continue;
        }
        ...
        if (!declPtr->TestAttr(Attribute::PRIVATE) && IsFuncDeclAndNotConstructor(declPtr)) {
            ...
            if (funcDecl.funcBody && funcDecl.funcBody->retType) {
                ...
                const std::string retType =
                    MapCODETypeToJavaType(funcDecl.funcBody->retType, &imports, &decl->fullPackageName);
                std::string methodSignature = "public " + retType + " ";
                methodSignature += funcIdentifier + "(";
                std::string argsWithTypes = GenerateFuncParamLists(funcDecl.funcBody->paramLists, false);
                methodSignature += argsWithTypes;
 ...
 * @param params is original constructor parameters
 */
void JavaSourceCodeGenerator::AddNativeInitCODEObject(const std::vector<OwnedPtr<Codira::AST::FuncParam>> &params)
{
    auto name = GetMangledJniInitCodeObjectFuncName(mangler, params, false);
    auto strParams = GenerateFuncParams(params, true);
    std::string signature = "public native long " + name + "(" + strParams + ");\n";
    AddWithIndent(TAB, signature);
}

void JavaSourceCodeGenerator::AddNativeDeleteCODEObject()
{
    AddWithIndent(TAB, "public native void deleteCODEObject(long self);\n");
}
...
void JavaSourceCodeGenerator::AddFinalize()
{
    ...
    AddWithIndent(TAB2, "deleteCODEObject(this.self);");
    AddWithIndent(TAB, "}");
}

void JavaSourceCodeGenerator::AddPrivateCtorForCODEMappring()
{
...
void JavaSourceCodeGenerator::AddPrivateCtorForCODEMappringEnum()
{
