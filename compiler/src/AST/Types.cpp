...
#include "Codira/AST/TypeKind.inc"
#undef TYPE_KIND
};
...
{
    return ::Codira::AST::GetGenericTy<ClassTy*>(declPtr);
}

Ptr<InterfaceTy> InterfaceTy::GetGenericTy() const
{
    return ::Codira::AST::GetGenericTy<InterfaceTy*>(declPtr);
}

Ptr<EnumTy> EnumTy::GetGenericTy() const
{
    return ::Codira::AST::GetGenericTy<EnumTy*>(declPtr);
}

Ptr<StructTy> StructTy::GetGenericTy() const
{
    return ::Codira::AST::GetGenericTy<StructTy*>(declPtr);
}
...
{
    if (Is<VArrayTy>(ty)) {
        CODEC_ASSERT(!ty.typeArgs.empty() && Ty::IsTyCorrect(ty.typeArgs[0]));
