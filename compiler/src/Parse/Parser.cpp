...
{
    CODEC_NULLPTR_CHECK(prop.outerDecl);
    ...
    if (HasModifier(prop.modifiers, TokenKind::ABSTRACT) && !isCommon && !inAbstractCODEMP &&
    ...
    if (inAbstractCODEMPClass && prop.TestAttr(Attribute::ABSTRACT) &&
    ...
    if (HasModifier(prop.modifiers, TokenKind::ABSTRACT) && !inAbstractCODEMPClass) {
