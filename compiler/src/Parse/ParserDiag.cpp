...
{
    CODEC_ASSERT(KIND_TO_STR.count(kind) != 0);
...
{
    CODEC_ASSERT(!chainedAST.empty() && "The chainAST is empty in DiagIllegalModifierInScope");
    auto node = chainedAST.back();
    ...
    if (res.first.IsDefault() && sk != ScopeKind::TOPLEVEL) {
        CODEC_ASSERT(SCOPE_TO_DECL.find(sk) != SCOPE_TO_DECL.end());
...
void ParserImpl::DiagNoneAssociativeOp(const Token& preT, const Token& tok)
{
    ...
    CODEC_ASSERT(iter != NONE_ASSO_OP.end());
...
{
    CODEC_ASSERT(KIND_TO_STR.count(decl->astKind) != 0);
    ...
    CODEC_ASSERT(publicModifier != decl->modifiers.end());
...
void ParserImpl::DiagUnExpectedModifierOnDeclaration(const Decl& vd)
{
    ...
    CODEC_ASSERT(constModifier != vd.modifiers.end());
    auto builder = ParseDiagnoseRefactor(
...
void ParserImpl::DiagGetOrSetCannotBeGeneric(const std::string& str, const Generic& ge)
{
    ...
    CODEC_ASSERT(iter != chainedAST.rend() && "cannot find getter or setter");
...
void ParserImpl::DiagUnexpectedWhere(const Token& token)
{
    ...
    CODEC_ASSERT(iter != chainedAST.rend() && "cannot find declaration");
...
{
    CODEC_ASSERT(!acs.empty() && "accepted attributes cannot be empty");
...
void ParserImpl::DiagRedundantModifiers(const Modifier& modifier)
{
    ...
    CODEC_ASSERT(KIND_TO_STR.count(node->astKind) != 0);
