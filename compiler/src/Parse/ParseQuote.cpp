...
void ParserImpl::ParseQuoteDollarInterpolationWithParen(AST::QuoteExpr& qe)
{
    ...
    // codec/fmt/lsp does not need it
...
{
    CODEC_ASSERT(ctx.back() == ParserContext::QUOTE);
    ctx.pop_back();
    lexer->ExitQuoteMod();
}
...
{
    CODEC_ASSERT(ctx.back() == ParserContext::QUOTE_INTERPOLATE);
    ctx.pop_back();
    lexer->ExitNormalMod();
}
...
{
    CODEC_ASSERT(ctx.back() == ParserContext::NORMAL);
    ctx.pop_back();
    lexer->ExitNormalMod();
}
