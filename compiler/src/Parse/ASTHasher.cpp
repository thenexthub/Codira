...
[[maybe_unused]] void PrintDebug(Codira::AST::ASTHasher::hash_type v, int line = __LINE__)
{
    if (!g_print) {
        return;
    }
...
struct [[maybe_unused]] DebugWhen {
    DebugWhen(const Codira::AST::Decl& decl, std::string_view m) noexcept
    {
...
struct ASTHasherImpl {
    ...
    template <int whatTypeToHash, typename Arg> inline void SuperHash(const Arg& input)
    {
        if constexpr ((whatTypeToHash == ONLY_POSITION && std::is_same<Arg, Position>::value) ||
            ...
            // if there is `-g` in codec's parameter, will ignore position's hash
    ...
    template <int whatTypeToHash> void HashAnnotation(const Codira::AST::Annotation& anno)
    {
    ...
    template <int whatTypeToHash> hash_type Hash(Ptr<const AST::Node> node)
    {
        ...
        CODEC_ASSERT(kind < nodeKind);
        if constexpr (whatTypeToHash == ALL) {
            CODEC_NULLPTR_CHECK(hashFuncMap[kind]);
            hashFuncMap[kind](*this, node);
        } else if constexpr (whatTypeToHash == ONLY_POSITION) {
            CODEC_NULLPTR_CHECK(hashFuncMapOnlyPosition[kind]);
            hashFuncMapOnlyPosition[kind](*this, node);
        } else {
            CODEC_NULLPTR_CHECK(hashFuncMapNonPosition[kind]);
...
ASTHasher::hash_type ASTHasher::VirtualHash(const Decl& decl)
{
    ...
    CODEC_ASSERT(decl.astKind != ASTKind::INTERFACE_DECL);
