...
void ParserImpl::CheckCODEMappingAttr(Decl& decl) const
{
    if (enableInteropCODEMapping && decl.TestAttr(Attribute::PUBLIC)) {
        // currently only support struct decl and enum decl.
        if (decl.astKind == ASTKind::STRUCT_DECL || decl.astKind == ASTKind::ENUM_DECL) {
            decl.EnableAttr(Attribute::JAVA_CODE_MAPPING);
        }
    }
}
    ...
    ScopeKind scopeKind, const std::set<Modifier>& modifiers, PtrVector<Annotation> annos)
{
    ...
    mpImpl->CheckCODEMPDecl(*primaryCtorDecl);
    ...
    ScopeKind scopeKind, const std::set<AST::Modifier>& modifiers, PtrVector<Annotation> annos)
{
    ...
    if (modifiers.size() == 1 && mpImpl->HasCODEMPModifiers(*modifiers.begin())) {
        mpImpl->CheckCODEMPModifiers(modifiers);
...
{
    CODEC_ASSERT(lastToken.kind == TokenKind::LT);
...
void ParserImpl::CheckFuncBody(ScopeKind scopeKind, FuncDecl& decl)
{
    ...
    if (decl.TestAnyAttr(Attribute::FOREIGN)) {
        ...
        if (!fb->retType) {
            ...
            CODEC_ASSERT(foreignPos != decl.modifiers.end());
...
void ParserImpl::CheckClassLikeFuncBodyAbstractness(FuncDecl& decl)
{
    ...
    bool inCODEMP = HasModifier(outerModifiers, TokenKind::PLATFORM) || HasModifier(outerModifiers, TokenKind::COMMON);
    bool inAbstractCODEMP = inAbstract && inCODEMP;
    ...
    if (HasModifier(decl.modifiers, TokenKind::ABSTRACT) && !isCommon && !inAbstractCODEMP &&
        !isJavaMirrorOrJavaMirrorSubtype) {
        ParseDiagnoseRefactor(DiagKindRefactor::parse_explicitly_abstract_only_for_codemp_abstract_class,
            decl, "function");
    }
    ...
    bool inAbstractCODEMPClass = inAbstractCODEMP && inClass;

    if (inAbstractCODEMP && !HasModifier(decl.modifiers, TokenKind::ABSTRACT)) {
        // OPEN func without ABSTRACT or COMMON must have body in CODEMP ABSTRACT class
        if (HasModifier(decl.modifiers, TokenKind::OPEN) && inAbstractCODEMP &&
    ...
    if (hasAbstractModifier && !inAbstractCODEMPClass) {
        ...
        CODEC_NULLPTR_CHECK(abstractMod);
