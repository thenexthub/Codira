...
{
    CODEC_ASSERT(GetExprMajorKind() == ExprMajorKind::TERMINATOR);
    ...
    if (static_cast<uint64_t>(ExprKind::APPLY_WITH_EXCEPTION) <= static_cast<uint64_t>(kind) &&
        ...
        CODEC_ASSERT(operands.size() >= WITH_EXCEPTION_SUCCESSOR_NUM);
...
{
    CODEC_ASSERT(newTerminator.IsTerminator());
    if (result != nullptr) {
        CODEC_NULLPTR_CHECK(newTerminator.GetResult());
...
{
    CODEC_ASSERT(operands.size() == 0 || index + GetFirstSuccessorIndex() < operands.size());
    ...
    : Terminator(ExprKind::MULTIBRANCH, {cond}, {}, parent), caseVals(vals)
{
    ...
    CODEC_NULLPTR_CHECK(defaultBlock);
    CODEC_NULLPTR_CHECK(parent);
    ...
    : FuncCallWithException(ExprKind::APPLY_WITH_EXCEPTION, callContext, parent)
{
    ...
    CODEC_NULLPTR_CHECK(sucBlock);
    CODEC_NULLPTR_CHECK(errBlock);
    CODEC_NULLPTR_CHECK(parent);
...
{
    CODEC_NULLPTR_CHECK(callContext.caller);
    CODEC_NULLPTR_CHECK(callContext.funcCallCtx.thisType);
    CODEC_ASSERT(!callContext.virMethodCtx.srcCodeIdentifier.empty());
    CODEC_NULLPTR_CHECK(callContext.virMethodCtx.originalFuncType);
}
...
Codira::OverflowStrategy IntOpWithException::GetOverflowStrategy() const
{
    if (opKind == ExprKind::DIV) {
        return overflowStrategy;
    }
    return Codira::OverflowStrategy::THROWING;
}
...
Codira::OverflowStrategy TypeCastWithException::GetOverflowStrategy() const
{
    return Codira::OverflowStrategy::THROWING;
}
