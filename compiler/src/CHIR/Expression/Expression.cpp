...
{
    CODEC_ASSERT(idx < operands.size());
    return operands[idx];
}
...
{
    CODEC_ASSERT(!newExpr.IsTerminator());
    // 1. replace result
...
{
    CODEC_ASSERT(!this->IsTerminator());
    ...
    CODEC_NULLPTR_CHECK(expr->parent);
    auto pos = std::find(expr->parent->exprs.begin(), expr->parent->exprs.end(), expr);
    CODEC_ASSERT(pos != expr->parent->exprs.end());
...
void Expression::MoveAfter(Expression* expr)
{
    ...
    CODEC_ASSERT(!expr->IsTerminator());
    if (parent != nullptr) {
        parent->RemoveExprOnly(*this);
        if (IsTerminator()) {
...
{
    CODEC_NULLPTR_CHECK(oldOperand);
    CODEC_NULLPTR_CHECK(newOperand);
...
Codira::OverflowStrategy UnaryExpression::GetOverflowStrategy() const
{
...
{
    CODEC_ASSERT(kind >= ExprKind::BITAND);
}
...
Codira::OverflowStrategy BinaryExpression::GetOverflowStrategy() const
{
    return overflowStrategy;
}
...
{
    CODEC_ASSERT(GetOperand(0)->IsLiteral());
...
{
    auto val = GetValue();
    CODEC_ASSERT(val->IsBoolLiteral());
...
{
    auto val = GetValue();
    CODEC_ASSERT(val->IsRuneLiteral());
...
{
    auto val = GetValue();
    CODEC_ASSERT(val->IsStringLiteral());
...
{
    auto val = GetValue();
    CODEC_ASSERT(val->IsFloatLiteral());
...
{
    CODEC_ASSERT(!virMethodCtx.srcCodeIdentifier.empty());
    CODEC_NULLPTR_CHECK(virMethodCtx.originalFuncType);
    CODEC_NULLPTR_CHECK(callContext.funcCallCtx.thisType);
}
...
ClassType* DynamicDispatch::GetInstSrcParentCustomTypeOfMethod(CHIRBuilder& builder) const
{
    for (auto& r : GetVirtualMethodInfo(builder)) {
        if (r.offset == GetVirtualMethodOffset()) {
            CODEC_NULLPTR_CHECK(r.instSrcParentType);
...
TypeCast::TypeCast(Value* operand, Codira::OverflowStrategy overflow, Block* parent)
...
Codira::OverflowStrategy TypeCast::GetOverflowStrategy() const
{
    return overflowStrategy;
}
...
{
    CODEC_ASSERT(blockGroups.empty());
...
{
    CODEC_ASSERT(body.body == nullptr);
    CODEC_ASSERT(blockGroups.empty());
    // newBody -> lambda
...
{
    CODEC_ASSERT(Is<RefType>(val->GetType()) &&
