...
{
    CODEC_NULLPTR_CHECK(interfaceTy.GetCustomTypeDef());
    CODEC_ASSERT(StaticCast<ClassDef*>(interfaceTy.GetCustomTypeDef())->IsInterface());
...
std::vector<ClassDef*> CustomTypeDef::GetImplementedInterfaceDefs() const
{
    ...
    for (auto ty : implementedInterfaceTys) {
        CODEC_NULLPTR_CHECK(ty->GetCustomTypeDef());
    ...
    std::vector<Type*>& funcInstTypeArgs, CHIRBuilder& builder, bool checkAbstractMethod) const
{
    ...
    CODEC_ASSERT(!funcName.empty());
    auto instParamTys = funcType.GetParamTypes();
    if (!isStatic) {
        CODEC_ASSERT(!instParamTys.empty());
    ...
    for (auto method : methods) {
        ...
        if (!method->TestAttr(Attribute::STATIC)) {
            CODEC_ASSERT(!originalFuncParamTys.empty());
...
{
    CODEC_ASSERT(index < instanceVars.size());
...
MemberVarInfo CustomTypeDef::GetInstanceVar(size_t index) const
{
    ...
    CODEC_ASSERT(allVars.size() > index);
