...
const std::vector<ExtendDef*>& Type::GetExtends(CHIRBuilder* builder) const
{
    ...
    CODEC_ABORT();
#else
    // just for release codec, it's not expected to go here
...
{
    CODEC_ASSERT(kind >= TYPE_INT8 && kind <= TYPE_UINT_NATIVE);
}

FloatType::FloatType(TypeKind kind) : NumericType(kind)
{
    CODEC_ASSERT(kind >= TYPE_FLOAT16 && kind <= TYPE_FLOAT64);
}
    ...
    : Type(kind), def(def)
{
    ...
    CODEC_ASSERT((kind == TYPE_CLASS && def->GetCustomKind() == CustomDefKind::TYPE_CLASS) ||
...
void CustomType::GetInstMap(std::unordered_map<const GenericType*, Type*>& instMap, CHIRBuilder& builder) const
{
    ...
    if (!customTypeDefGenericParams.empty()) {
        CODEC_ASSERT(customTypeDefGenericParams.size() == customTypeGenericArgs.size());
    }
    ...
    for (auto parentInterfaceTy : customTypeDef->GetImplementedInterfaceTys()) {
        ...
        if (!customTypeDefGenericParams.empty()) {
            CODEC_ASSERT(parentInterfaceDefGenericParams.size() == parentInterfaceTypeGenericArgs.size());
        }
    ...
    if (DynamicCast<ClassType*>(this)) {
        ...
        if (parentClassTy) {
            ...
            if (!parentClassDefGenericParams.empty()) {
                CODEC_ASSERT(parentClassDefGenericParams.size() == parentClassTypeGenericArgs.size());
            }
...
std::vector<Type*> CustomType::CalculateCurDefInstantiatedMemberTys(CHIRBuilder& builder)
{
    ...
    if (!genericTypes.empty()) {
        CODEC_ASSERT(argTys.size() == genericTypes.size());
    }
...
std::vector<FuncType*> CustomType::GetInstMethodTypes(CHIRBuilder& builder) const
{
    ...
    std::unordered_map<const GenericType*, Type*> instMap;
    CODEC_ASSERT(typeArgs.size() == paramArgs.size());
...
{
    if (auto genericType = Codira::DynamicCast<const GenericType*>(&mayBeGeneric)) {
    ...
    std::unordered_map<const GenericType*, Type*>& replaceTable)
{
    ...
    for (size_t i = 0; i < genericTypeArgs.size(); ++i) {
        if (auto genericTy = Codira::DynamicCast<GenericType*>(genericTypeArgs[i])) {
            ...
            auto genericArgTy = Codira::DynamicCast<CustomType*>(genericTypeArgs[i]);
            auto instArgTy = Codira::DynamicCast<CustomType*>(instTypeArgs[i]);
    ...
    const FuncCallType& funcCallType, bool isStatic, CHIRBuilder& builder) const
{
    ...
    for (auto ex : GetCustomTypeDef()->GetExtends()) {
        ...
        CODEC_ASSERT(classInstArgs.size() == extendedTyGenericArgs.size());
...
std::vector<EnumCtorInfo> EnumType::GetConstructorInfos(CHIRBuilder& builder) const
{
    ...
    CODEC_ASSERT(genericTypeParams.size() == argTys.size());
    std::unordered_map<const GenericType*, Type*> replaceTable;
...
{
    CODEC_ASSERT(!argTys.empty());
    auto elemTy = argTys[0];
    CODEC_NULLPTR_CHECK(elemTy);
    std::stringstream ss;
    std::string prefix;
    std::string suffix;
...
void GenericType::SetUpperBounds(const std::vector<Type*>& args)
{
    for (auto arg : args) {
        if (!orphanFlag) {
            CODEC_ASSERT(!arg->StripAllRefs()->IsValueType() && "Generic type upper bound should NOT be value type!");
        }
    }
    upperBounds = args;
    ...
    const std::vector<uint64_t>& path, CHIRBuilder& builder) const
{
    ...
    CODEC_NULLPTR_CHECK(concretType);
    ...
    const Type& instType, const Type& genericType, std::unordered_map<const GenericType*, Type*>& replaceTable)
{
    ...
    if (auto instCustomType = Codira::DynamicCast<const CustomType*>(&instType)) {
        if (instCustomType->GetCustomTypeDef() !=
            Codira::StaticCast<const CustomType&>(genericType).GetCustomTypeDef()) {
            return false;
        }
    }
    auto instTypeArgs = instType.GetTypeArgs();
