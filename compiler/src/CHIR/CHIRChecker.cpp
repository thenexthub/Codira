...
#include "Codira/CHIR/CHIRChecker.h"
...
bool IsStructOrCStruct(const Type& type, bool needToBeCStruct)
{
    ...
    return Codira::StaticCast<const StructType&>(type).GetStructDef()->IsCStruct();
}

bool IsCType(const Type& type, bool needToBeCStruct = true)
{
    if (auto varray = Codira::DynamicCast<const VArrayType*>(&type)) {
        return IsCType(*varray->GetElementType(), needToBeCStruct);
    } else if (auto tuple = Codira::DynamicCast<const TupleType*>(&type)) {
        ...
        return true;
    } else if (auto ref = Codira::DynamicCast<const RefType*>(&type)) {
...
{
    if (auto lambda = Codira::DynamicCast<const Lambda*>(&expr)) {
...
bool CheckTypeMustBeRef(const Type& type)
{
    ...
    auto baseType = Codira::StaticCast<const RefType&>(type).GetBaseType();
...
{
    if (auto func = Codira::DynamicCast<const Func*>(&value)) {
...
{
    if (type.IsGeneric()) {
        if (auto& genericTy = Codira::StaticCast<const GenericType&>(type); genericTy.orphanFlag) {
            auto upperBounds = genericTy.GetUpperBounds();
            CODEC_ASSERT(upperBounds.size() == 1);
...
std::vector<Type*> CalculateAllUpperBounds(const GenericType& genericType, CHIRBuilder& builder)
{
    ...
    for (auto upperBound : upperBounds) {
        ...
        auto derefUpperBound = Codira::DynamicCast<ClassType*>(upperBound->StripAllRefs());
...
bool HasOrphanGeneric(const Type& type)
{
    ...
    auto visitor = [&flag](const Type& ty) {
        if (auto genericType = Codira::DynamicCast<const GenericType*>(&ty);
...
bool IsFuncTypeOrClosureBaseRefType(const Type& type)
{
    ...
    auto refType = Codira::DynamicCast<const RefType*>(&type);
    if (refType == nullptr) {
        return false;
    }
...
NullLiteral* ConvertToNullLiteral(const Value& value)
{
    ...
    auto localVar = Codira::DynamicCast<const LocalVar*>(&value);
    if (localVar == nullptr) {
        return nullptr;
    }
    auto constExpr = Codira::DynamicCast<Constant*>(localVar->GetExpr());
    if (constExpr == nullptr) {
        return nullptr;
    }
    return Codira::DynamicCast<NullLiteral*>(constExpr->GetValue());
}
...
{
    if (auto lambdaRes = Codira::DynamicCast<const LocalVar*>(&func)) {
        auto lambda = Codira::StaticCast<Lambda*>(lambdaRes->GetExpr());
        ...
        auto funcBase = Codira::VirtualCast<const FuncBase*>(&func);
...
{
    if (auto funcBase = Codira::DynamicCast<const FuncBase*>(&func)) {
        return funcBase->GetIdentifier();
    } else {
        auto lambda = Codira::StaticCast<const LocalVar&>(func).GetExpr();
        return Codira::StaticCast<Lambda*>(lambda)->GetIdentifier();
    }
}
} // namespace

CHIRChecker::CHIRChecker(const Package& package, const Codira::GlobalOptions& opts, CHIRBuilder& builder)
...
{
    CODEC_ASSERT(!expectedNum.empty());
    auto realNum = expr.GetOperands().size();
    ...
    const std::vector<Type*>& paramTypes, const Lambda* lambda, const FuncBase& topLevelFunc)
{
    ...
    auto funcIsInit = [&topLevelFunc]() {
        ...
        // a hack way, in codemp, member var init func is like xxx$varInit
...
{
    CODEC_ASSERT(thisType.IsBuiltinType() || thisType.IsCustomType());
    ...
    const Value& callee, const Type* thisType, const Expression& expr, const Func& topLevelFunc)
{
    ...
    /**
     *  generic type is a little weird, but the following code is allowed in Codira:
     *  class A {
    ...
    const std::vector<GenericType*>& genericTypeParams, Type* instOuterType)
{
    ...
    CODEC_ASSERT(genericTypeParams.size() == instantiatedTypeArgs.size());
...
bool CHIRChecker::InstTypeCanSetToGenericRelatedType(Type& instType, const Type& genericRelatedType)
{
    ...
    if (instType.IsRef() && genericRelatedType.IsRef()) {
        auto instBaseType = Codira::StaticCast<const RefType&>(instType).GetBaseType();
        auto genericBaseType = Codira::StaticCast<const RefType&>(genericRelatedType).GetBaseType();
        return InstTypeCanSetToGenericRelatedType(*instBaseType, *genericBaseType);
    }
    auto instCustomType = Codira::DynamicCast<const CustomType*>(&instType);
    auto genericCustomType = Codira::DynamicCast<const CustomType*>(&genericRelatedType);
    if (instCustomType != nullptr && genericCustomType != nullptr) {
        ...
        if (instCustomType->GetCustomTypeDef() == genericCustomType->GetCustomTypeDef()) {
             ...
             */
            auto superTypes =
                Codira::StaticCast<CustomType*>(instCustomType->StripAllRefs())->GetSuperTypesRecusively(builder);
    ...
    if (auto genericType = Codira::DynamicCast<const GenericType*>(&genericRelatedType)) {
        if (auto gType = Codira::DynamicCast<GenericType*>(&instType)) {
...
void CHIRChecker::CheckGetInstantiateValue(const GetInstantiateValue& expr, const Func& topLevelFunc)
{
    ...
    CODEC_ASSERT(!funcAndGenericTypes.empty());
