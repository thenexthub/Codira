...
#include "Codira/CHIR/Analysis/CallGraphAnalysis.h"
...
#include "Codira/CHIR/Checker/UnreachableBranchCheck.h"
#include "Codira/CHIR/Checker/VarInitCheck.h"
#include "Codira/CHIR/GenerateVTable/GenerateVTable.h"
#include "Codira/CHIR/IRChecker.h"
#include "Codira/CHIR/Interpreter/ConstEval.h"
#include "Codira/CHIR/Serializer/CHIRDeserializer.h"
#include "Codira/CHIR/Serializer/CHIRSerializer.h"
#include "Codira/CHIR/Transformation/ArrayLambdaOpt.h"
#include "Codira/CHIR/Transformation/ArrayListConstStartOpt.h"
#include "Codira/CHIR/Transformation/BoxRecursionValueType.h"
#include "Codira/CHIR/Transformation/ClosureConversion.h"
#include "Codira/CHIR/Transformation/ConstPropagation.h"
#include "Codira/CHIR/Transformation/DeadCodeElimination.h"
#include "Codira/CHIR/Transformation/Devirtualization.h"
#include "Codira/CHIR/Transformation/FlatForInExpr.h"
...
#include "Codira/CHIR/Transformation/MergeBlocks.h"
#include "Codira/CHIR/Transformation/NoSideEffectMarker.h"
#include "Codira/CHIR/Transformation/RangePropagation.h"
...
#include "Codira/CHIR/Transformation/RedundantLoadElimination.h"
#include "Codira/CHIR/Transformation/SanitizerCoverage.h"
    ...
    while (mapIt != oldMap.end()) {
        ...
        while (effectedIt != mapIt->second.end()) {
            if (IsDesugaredNoneStaticConstructor(**effectedIt)) {
                ...
                CODEC_NULLPTR_CHECK(parentClass);
...
void ToCHIR::DumpCHIRToFile(const std::string& suffix, bool checkFlag)
{
    ...
    CODEC_NULLPTR_CHECK(chirPkg);
    CODEC_ASSERT(!CHIR_READABLE_FILE_EXTENSION.empty());
...
void ToCHIR::DoClosureConversion()
{
    ...
    CODEC_NULLPTR_CHECK(builder.GetObjectTy()->GetClassDef());
...
bool ToCHIR::RunConstantPropagation()
{
    ...
    if (threadNum == 1) {
        ...
        cp.RunOnPackage(chirPkg, opts.chirDebugOptimizer, ci.isCODELint);
        ...
        for (size_t idx = 0; idx < funcNum; ++idx) {
            ...
            taskQueue.AddTask<void>([constPropagation = cp.get(), func, isDebug, isCODELint]() {
                return constPropagation->RunOnFunc(func, isDebug, isCODELint);
            });
...
{
    if (!opts.IsConstEvalEnabled() || opts.enIncrementalCompilation || opts.commonPartCodeo.has_value()) {
        return true;
    }
    ...
    std::vector<Codira::CHIR::Interpreter::Bchir> bchirPackages;
...
bool ToCHIR::RunIRChecker(const Phase& phase)
{
    ...
    switch (phase) {
        ...
        case Phase::PLUGIN:
            ...
            break;
        case Phase::ANALYSIS_FOR_CODELINT:
            suffix = "after analysis for codelint";
            break;
    }
...
void ToCHIR::RecordCodeInfoAtTheBegin()
{
    ...
    std::function<int64_t(void)> getCurPkgInstantiatedAstNode = [this]() -> int64_t {
        ...
        for (auto& decl : pkg.genericInstantiatedDecls) {
            ...
            CODEC_NULLPTR_CHECK(genericDecl->curFile);
...
bool ToCHIR::RunAnalysisForCODELint()
{
    Utils::ProfileRecorder recorder("CHIR", "CHIR Opt");
    NothingTypeExprElimination();
    RunConstantAnalysis();
...
void ToCHIR::CFFIFuncWrapper()
{
    ...
    for (auto foreignFunc : chirPkg->GetImportedVarAndFuncs()) {
        if (foreignFunc->GetType()->IsCFunc()) {
            CODEC_ASSERT(foreignFunc->IsImportedFunc());
...
bool ToCHIR::Run()
{
    CODEC_NULLPTR_CHECK(typeManager);
    CODEC_NULLPTR_CHECK(gim);
    CODEC_NULLPTR_CHECK(&needToOptGenericDecl);
    CODEC_NULLPTR_CHECK(&needToOptString);
    CODEC_NULLPTR_CHECK(&releaseCHIRMemory);
    CODEC_NULLPTR_CHECK(&codiraHome);
    ...
    if (ci.isCODELint) {
        return RunAnalysisForCODELint() && RunIRChecker(Phase::ANALYSIS_FOR_CODELINT);
    }
...
namespace {
Codira::Position ConvertCHIRPos2ASTPos(unsigned int fileId, const Position& pos)
{
    return Codira::Position{fileId, static_cast<int>(pos.line), static_cast<int>(pos.column)};
}
Position ConvertASTPos2CHIRPos(const Codira::Position& pos)
{
    ...
    const CustomType& baseType, const std::string& name, CHIRBuilder& builder)
{
    ...
    CODEC_NULLPTR_CHECK(memberVarInstType);
