...
template <bool ForConstEval> void CHIR2BCHIR::TranslateExtends(const Package& chirPkg)
{
    for (const auto chirClass : chirPkg.GetAllExtendDef()) {
        ...
        if (extendedDef == nullptr) {
            ...
            CODEC_ASSERT(ty->IsPrimitive() || ty->IsCString() || ty->IsCPointer());
...
Bchir::ByteCodeContent CHIR2BCHIR::GetTypeIdx(Codira::CHIR::Type& chirType)
{
    ...
    CODEC_ASSERT(tyIdx < static_cast<size_t>(Bchir::BYTECODE_CONTENT_MAX));
    ...
    switch (chirType.GetTypeKind()) {
        ...
        case CHIR::Type::TYPE_CLASS: {
            // Temporarily disable CODEC_ASSERT(false)
            break;
        }
        default:
            break;
...
template <bool ForConstEval> CHIR2BCHIR::Context CHIR2BCHIR::TranslateGlobalVar(const GlobalVar& gv)
{
    ...
    if (auto init = gv.GetInitializer()) {
        ...
        CODEC_ASSERT(gv.GetInitFunc() != nullptr);
...
void CHIR2BCHIR::TranslateFuncDef(Context& ctx, const Func& func)
{
    ...
    for (auto arg = args.crbegin(); arg != args.crend(); ++arg) {
        ...
        CODEC_ASSERT(ctx.val2lvarId.find(*arg) == ctx.val2lvarId.end());
    ...
    CODEC_ASSERT(bbGroup->GetEntryBlock());
...
Bchir::ByteCodeContent CHIR2BCHIR::GetStringIdx(std::string str)
{
    ...
    if (literalId == constStringsMemoization.end()) {
        ...
        CODEC_ASSERT(constIdx <= static_cast<size_t>(Bchir::BYTECODE_CONTENT_MAX));
...
void CHIR2BCHIR::TranslateAllocate(Context& ctx, const Expression& expr)
{
    ...
    if (expr.GetExprKind() == ExprKind::ALLOCATE) {
        ...
        CODEC_ASSERT(expr.GetExprKind() == ExprKind::ALLOCATE_WITH_EXCEPTION);
...
const std::unordered_map<std::string, IntrinsicKind> CHIR2BCHIR::syscall2IntrinsicKind = {
    {CODE_AST_LEX, IntrinsicKind::FFI_CODE_AST_LEX},
    {CODE_ASTPARSEEXPR, IntrinsicKind::FFI_CODE_AST_PARSEEXPR},
    {CODE_ASTPARSEDECL, IntrinsicKind::FFI_CODE_AST_PARSEDECL},
    {CODE_ASTPARSE_PROPMEMBERDECL, IntrinsicKind::FFI_CODE_AST_PARSE_PROPMEMBERDECL},
    {CODE_ASTPARSE_PRICONSTRUCTOR, IntrinsicKind::FFI_CODE_AST_PARSE_PRICONSTRUCTOR},
    {CODE_ASTPARSEPATTERN, IntrinsicKind::FFI_CODE_AST_PARSE_PATTERN},
    {CODE_ASTPARSETYPE, IntrinsicKind::FFI_CODE_AST_PARSE_TYPE},
    {CODE_ASTPARSETOPLEVEL, IntrinsicKind::FFI_CODE_AST_PARSETOPLEVEL},
    {CODE_PARENT_CONTEXT, IntrinsicKind::FFI_CODE_PARENT_CONTEXT},
    {CODE_MACRO_ITEM_INFO, IntrinsicKind::FFI_CODE_MACRO_ITEM_INFO},
    {CODE_GET_CHILD_MESSAGES, IntrinsicKind::FFI_CODE_GET_CHILD_MESSAGES},
    {CODE_CHECK_ADD_SPACE, IntrinsicKind::FFI_CODE_CHECK_ADD_SPACE},
    {CODE_AST_DIAGREPORT, IntrinsicKind::FFI_CODE_AST_DIAGREPORT},
    {CODE_TLS_DYN_SET_SESSION_CALLBACK_NAME, IntrinsicKind::CODE_TLS_DYN_SET_SESSION_CALLBACK},
    {CODE_TLS_DYN_SSL_INIT_NAME, IntrinsicKind::CODE_TLS_DYN_SSL_INIT},
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    {CODE_CORE_CAN_USE_SIMD_NAME, IntrinsicKind::CODE_CORE_CAN_USE_SIMD},
    {"std.core:CODE_CORE_CanUseSIMD", IntrinsicKind::CODE_CORE_CAN_USE_SIMD},
#endif
};
