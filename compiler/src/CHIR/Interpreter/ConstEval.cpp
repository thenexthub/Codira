...
using Codira::CHIR::Interpreter::ConstEvalPass;
using Codira::CHIR::Interpreter::IVal2CHIR;

Codira::CHIR::Constant* IVal2CHIR::TryConvertToConstant(Type& ty, const IVal& val, Block& parent)
{
...
Codira::CHIR::Value* IVal2CHIR::ConvertToChir(
...
Codira::CHIR::Value* IVal2CHIR::ConvertStringToChir(
    Type& ty, const ITuple& val, const std::function<void(Expression*)>& insertExpr, Block& parent)
{
    ...
    CODEC_ASSERT(start == static_cast<uint32_t>(0) && "const strings always start at zero");

    auto len = IValUtils::Get<IUInt32>(val.content[2]).content;

    stringVal.reserve(len);
    CODEC_ASSERT(arrData.size() > static_cast<size_t>(len));
...
Codira::CHIR::Value* IVal2CHIR::ConvertTupleToChir(
    Type& ty, const ITuple& val, std::function<void(Expression*)>& insertExpr, Block& parent)
{
    ...
    CODEC_ASSERT(elementVals.size() == elementTys.size());
...
Codira::CHIR::Value* IVal2CHIR::ConvertEnumToChir(
    EnumType& ty, const IVal& val, std::function<void(Expression*)>& insertExpr, Block& parent)
{
    ...
    if (ty.GetEnumDef()->IsAllCtorsTrivial()) {
        ...
        CODEC_ASSERT(elementVals.size() == elementTys.size() + 1);
...
Codira::CHIR::Value* IVal2CHIR::ConvertRefToChir(
    RefType& ty, const IVal& val, std::function<void(Expression*)>& insertExpr, Block& parent)
{
    ...
    switch (referencedType->GetTypeKind()) {
        case Type::TYPE_CLASS: {
            ...
            CODEC_ASSERT(members.size() == obj->content.size());
...
Codira::CHIR::ClassType* IVal2CHIR::FindClassType(const std::string& mangledName)
{
...
Codira::CHIR::Value* IVal2CHIR::ConvertArrayToChir(
...
std::optional<Codira::CHIR::BlockGroup*> ConstEvalPass::CreateNewInitializer(
