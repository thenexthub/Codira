        ...
        switch (current) {
            ...
            case OpCode::FIELD: {
                ...
                if (auto tuple = IValUtils::GetIf<ITuple>(&arg)) {
                    ...
                    CODEC_ASSERT(std::holds_alternative<IObject>(arg));
...
void BCHIRInterpreter::InterpretStoreInRef()
{
    ...
    for (uint32_t p = 0; p < pathSize; ++p) {
        ...
        if (auto tuple = IValUtils::GetIf<ITuple>(targetVal)) {
            ...
            CODEC_ASSERT(index < arr->content.size());
...
{
    interpreterError = true;
    Codira::Position errorPosition = DEFAULT_POSITION;
    ...
    if (fileId != -1 && !diag.GetSourceManager().GetSource(static_cast<unsigned int>(fileId)).buffer.empty()) {
        errorPosition = Codira::Position(static_cast<unsigned int>(fileId), static_cast<int>(applyPosition.line),
...
{
    CODEC_ASSERT((std::is_same<SourceTyRaw, double>::value) || (std::is_same<SourceTyRaw, float>::value));
    TargetTyRaw res = 0;
...
template <OpCode op> void BCHIRInterpreter::InterpretTypeCast()
{
    ...
    auto strat = static_cast<Codira::OverflowStrategy>(bchir.Get(pc++));
    ...
    switch (srcKind) {
        case CHIR::Type::TypeKind::TYPE_RUNE: {
            if (targetKind == CHIR::Type::TypeKind::TYPE_UINT32) {
                ...
                CODEC_ASSERT(targetKind == CHIR::Type::TypeKind::TYPE_UINT64);
...
template <OpCode op> void BCHIRInterpreter::InterpretApply()
{
    ...
    CODEC_ASSERT(numberArgs > 0);
    CODEC_ASSERT(interpStack.ArgsSize() >= numberArgs);
...
Bchir::ByteCodeIndex BCHIRInterpreter::FindMethod(Bchir::ByteCodeContent classId, Bchir::ByteCodeContent nameId)
{
    ...
    CODEC_ASSERT(classInfoIt != bchir.GetClassTable().end());
    auto& classInfo = classInfoIt->second;
    auto methodIt = classInfo.vtable.find(nameId);
    CODEC_ASSERT(methodIt != classInfo.vtable.end());
...
bool BCHIRInterpreter::PushIfNotOverflow(bool overflow, S res, Codira::OverflowStrategy strat)
{
...
template <OpCode op, typename T, typename S> bool BCHIRInterpreter::BinOpInt(Codira::OverflowStrategy strat)
{
    if constexpr (op == OpCode::BIN_EXP || op == OpCode::BIN_EXP_EXC) {
        // this is a special case: only Int64 ** UInt64 is allowed by the spec.
        CODEC_ASSERT((std::is_same_v<T, IInt64>));
        CODEC_ASSERT((std::is_same_v<S, int64_t>));
...
template <typename T> static bool IsOverflowAfterExp(T x, T y, Codira::OverflowStrategy strategy, T* res)
{
    CODEC_NULLPTR_CHECK(res);
    *res = 1;
    ...
    if (isOverflow && strategy == Codira::OverflowStrategy::SATURATING) {
        T magicNumber = 2;
...
bool BCHIRInterpreter::BinExpOpInt(Codira::OverflowStrategy strat)
{
...
template <OpCode op, typename T, typename S> bool BCHIRInterpreter::BinRegOpInt(Codira::OverflowStrategy strat)
{
...
template <OpCode op>
bool BCHIRInterpreter::BinOpTyKindAndOverflowStrat(CHIR::Type::TypeKind kind, Codira::OverflowStrategy strat)
{
    switch (kind) {
        case CHIR::Type::TypeKind::TYPE_UINT8: {
...
template <OpCode op> void BCHIRInterpreter::BinOp()
{
    ...
    auto strat = static_cast<Codira::OverflowStrategy>(bchir.Get(stratIdx));
    pc = stratIdx + 1;
    ...
    if constexpr (OpHasExceptionHandler(op)) {
        if (!raisedException) {
            CODEC_ASSERT(interpStack.CtrlTop().opCode == op);
            CODEC_ASSERT(interpStack.CtrlTop().byteCodePtr == initPc);
...
{
    CODEC_ASSERT(std::holds_alternative<IUnit>(interpStack.ArgsTopIVal()));
    interpStack.ArgsPopBack();
    CODEC_ASSERT(std::holds_alternative<IUnit>(interpStack.ArgsTopIVal()));
    interpStack.ArgsPopBack();
...
IResult BCHIRInterpreter::Run(size_t baseIdx, bool expectsReturn)
{
    ...
    if (interpreterError || raiseExnToTopLevel) {
        ...
        CODEC_ASSERT(interpStack.ArgsSize() == 0);
        CODEC_ASSERT(interpStack.CtrlIsEmpty());
        result = INotRun{};
        return result;
    } else {
        CODEC_ASSERT(interpStack.ArgsSize() == 1);
        CODEC_ASSERT(interpStack.CtrlIsEmpty());
...
void BCHIRInterpreter::PrintDebugInfo(Bchir::ByteCodeIndex currentPc)
{
    if (printRuntimeDebugInfo) {
        CODEC_ASSERT(debugFile.is_open());
...
template <bool isLiteral, bool isExc> void BCHIRInterpreter::InterpretAllocateRawArray()
{
    ...
    if constexpr (isLiteral) {
        ...
        interpStack.ArgsPop(size, array.content);
        CODEC_ASSERT(array.content.size() == size);
        (void)initPc;
    } else {
        ...
        CODEC_ASSERT(array.content.size() == static_cast<size_t>(size) + 1);
    }
    auto ptr = IPointer();
    ptr.content = AllocateValue(std::move(array));
    interpStack.ArgsPush(ptr);
...
void BCHIRInterpreter::InterpretRawArrayLiteralInit()
{
    ...
    CODEC_ASSERT(static_cast<Bchir::ByteCodeContent>(IValUtils::Get<IInt64>(array.content[0]).content) == size);
    for (size_t i = 0; i < static_cast<size_t>(size); ++i) {
