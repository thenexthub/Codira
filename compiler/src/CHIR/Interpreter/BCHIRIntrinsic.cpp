...
{
    CODEC_ASSERT(static_cast<OpCode>(bchir.Get(pc)) == OpCode::INTRINSIC0 ||
...
{
    CODEC_ASSERT(static_cast<OpCode>(bchir.Get(pc)) == OpCode::INTRINSIC1 ||
...
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
void BCHIRInterpreter::InterpretCODECodeCanUseSIMD()
{
