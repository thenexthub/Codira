...
bool ShouldCreateAlloc(const AST::VarDecl& decl, const Codira::GlobalOptions& opts, bool rValueIsEmpty)
{
    if (decl.isConst) {
    ...
    // Note: example codira code: let (a, b) : (Int64, Int64) = (1, 2)
...
Ptr<Value> Translator::TranslateLeftValueOfVarDecl(const AST::VarDecl& decl, bool rValueIsEmpty, bool isLocalVar)
{
    ...
    if (isLocalVar) {
        ...
        if (ShouldCreateAlloc(decl, opts, rValueIsEmpty)) {
            ...
            // it will generate bug in codedb if we generate debug expr for implicit added
...
{
    CODEC_ASSERT(!decl.TestAttr(AST::Attribute::GLOBAL));
