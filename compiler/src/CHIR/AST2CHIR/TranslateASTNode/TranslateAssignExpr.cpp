...
Value* Translator::TranslateVArrayAssign(const AssignExpr& assign)
{
    ...
    if (COMPOUND_ASSIGN_EXPR_MAP.find(assign.op) == COMPOUND_ASSIGN_EXPR_MAP.end()) {
        ...
        CODEC_ASSERT(lhsBase->GetType()->IsRef());
    ...
    CODEC_ASSERT(lhsBase && lhsBase->GetType()->IsRef());
...
{
    CODEC_ASSERT(COMPOUND_ASSIGN_EXPR_MAP.find(assign.op) != COMPOUND_ASSIGN_EXPR_MAP.end());
    ...
    CODEC_ASSERT(lhsLeftValueBaseRefTy->IsRef());
    ...
    if (!lhsLeftValuePath.empty()) {
        ...
        CODEC_ASSERT(lhsLeftValueBaseRefTy->IsReferenceTypeWithRefDims(1) ||
        ...
        CODEC_ASSERT(!lhsLeftValueBaseTy->IsRef());
...
Value* Translator::TranslateTrivialAssign(const AST::AssignExpr& assign)
{
    ...
    CODEC_ASSERT(lhs && lhs->GetType()->IsRef());
    if (!path.empty()) {
        auto lhsCustomType = StaticCast<CustomType*>(lhs->GetType()->StripAllRefs());
