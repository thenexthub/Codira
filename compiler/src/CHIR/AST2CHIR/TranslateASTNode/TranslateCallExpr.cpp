...
{
    CODEC_ASSERT(expr.resolvedFunction != nullptr);
    ...
    const std::vector<Value*>& args, Value* thisObj, Value* thisRTTI, DebugLocation loc)
{
    ...
    if (thisObj != nullptr) {
        CODEC_ASSERT(args.size() == instantiatedParamTys.size() - 1);
        ...
        CODEC_ASSERT(args.size() == instantiatedParamTys.size());
        for (size_t i = 0; i < args.size(); ++i) {
...
Ptr<Value> Translator::GetCurrentThisObject(const AST::FuncDecl& resolved)
{
    ...
    if (IsStructOrExtendMethod(*curFunc) && isThisRef && !needThisRef) {
        ...
        CODEC_ASSERT(objType->IsRef() && !StaticCast<RefType*>(objType)->GetBaseType()->IsRef());
        auto objBaseType = StaticCast<RefType*>(objType)->GetBaseType();
        CODEC_ASSERT(objBaseType->IsStruct());
    ...
    const DebugLocation& loc)
{
    ...
    if (memAccess.baseExpr->ty->IsClassLike()) {
        ...
        if (objType->IsRef()) {
            // objType is A&
            auto objBaseType = StaticCast<RefType*>(objType)->GetBaseType();
            CODEC_ASSERT(!objBaseType->IsRef());
            // for example: objBaseType is A
    ...
    const AST::CallExpr& expr, std::vector<Value*>& args, bool needsMutableThis)
{
    ...
    CODEC_ASSERT(resolved && IsInstanceMember(*resolved));
    CODEC_NULLPTR_CHECK(resolved->outerDecl);
    // polish here
    // When current is calling a constructor and is not called with 'this' or 'super',
    // it should not using 'this' existed in context.
    CODEC_ASSERT(!IsCallRegularInit(expr));
    ...
    CODEC_ASSERT(thisObjTy->IsReferenceTypeWithRefDims(CLASS_REF_DIM) || thisObjTy->IsReferenceTypeWithRefDims(1) ||
    ...
    if (!needsMutableThis) {
        ...
        CODEC_ASSERT(!thisObjTy->IsValueOrGenericTypeWithRefDims(0));
...
{
    Ptr<AST::FuncDecl> resolved = expr.resolvedFunction;
    CODEC_ASSERT(resolved->funcBody && !resolved->funcBody->paramLists.empty());
    CODEC_ASSERT(resolved->funcBody->paramLists[0]->params.size() == expr.desugarArgs.value().size() ||
    ...
    for (size_t i = 0; i < argExprs.size(); i++) {
        if (argExprs[i]->TestAttr(AST::Attribute::HAS_INITIAL)) {
            ...
            CODEC_NULLPTR_CHECK(params[i]->desugarDecl.get());
            ...
            if (!expectedArgTys.empty()) {
                ...
                CODEC_ASSERT(expectedArgTys.size() > i);
            ...
            if (!expectedArgTys.empty()) {
                if (i < expectedArgTys.size()) {
                    ...
                    CODEC_ASSERT(resolved->ty->IsCFunc());
                }
            }
    ...
    for (auto& arg : expr.args) {
        ...
        if (needCastToExpectedTy) {
            if (i < expectedArgTys.size()) {
                ...
                CODEC_ASSERT(expr.resolvedFunction && expr.resolvedFunction->ty->IsCFunc());
            }
        }
...
Ptr<Value> Translator::TranslateIntrinsicCall(const AST::CallExpr& expr)
{
    ...
    if (auto it = packageMap.find(packageName); it != packageMap.end()) {
        CODEC_ASSERT(it->second.find(identfifier) != it->second.end());
...
Ptr<Value> Translator::TranslateForeignFuncCall(const AST::CallExpr& expr)
{
    ...
    CODEC_ASSERT(callee != nullptr && "TranslateApply: not supported callee now!");
...
Ptr<Value> Translator::TranslateEnumCtorCall(const AST::CallExpr& expr)
{
    ...
    CODEC_ASSERT(fieldIt != constrs.end());
...
{
    auto resolvedFunction = expr.resolvedFunction;
    CODEC_NULLPTR_CHECK(resolvedFunction);
    CODEC_ASSERT(!resolvedFunction->TestAttr(AST::Attribute::STATIC));
    bool isMutFunc =
    ...
    if ((calledByInheritableTy || mayFuncBeMovedInPlatform) && !isSuperCall && IsVirtualMember(*resolvedFunction)) {
        ...
        if (!originalFuncDecl->TestAttr(AST::Attribute::STATIC)) {
            ...
            CODEC_NULLPTR_CHECK(originalOuterDecl);
        ...
        CODEC_NULLPTR_CHECK(instParentCustomTy);
        Ptr<Type> instParentTy = instParentCustomTy;
        if (instParentCustomTy->IsReferenceType()) {
            instParentTy = builder.GetType<RefType>(instParentCustomTy);
        } else if (instParentCustomTy->IsStruct() && isMutFunc) {
...
Value* Translator::TranslateMemberFuncCall(const AST::CallExpr& expr)
{
    ...
    auto resolvedFunction = expr.resolvedFunction;
    CODEC_NULLPTR_CHECK(resolvedFunction);
    CODEC_ASSERT(!resolvedFunction->TestAttr(AST::Attribute::CONSTRUCTOR));
...
std::pair<std::vector<Type*>, Type*> Translator::GetMemberFuncParamAndRetInstTypes(const AST::CallExpr& expr)
{
    ...
    if (auto genericTy = DynamicCast<AST::GenericsTy*>(expr.baseFunc->ty); genericTy) {
        CODEC_ASSERT(genericTy->upperBounds.size() == 1 && "not support multi-upperBounds for funcType in CHIR");
...
Value* Translator::GenerateLoadIfNeccessary(Value& arg, bool isThis, bool isMut, bool isInOut, const DebugLocation& loc)
{
    ...
    if (((isMut && isThis) || isInOut) && (pureArgTy->IsValueType() || pureArgTy->IsGeneric())) {
        ...
        if (argTy->IsCPointer()) {
            ...
            CODEC_ASSERT(argTy->IsRef());
            CODEC_ASSERT(!StaticCast<RefType*>(argTy)->GetBaseType()->IsRef());
...
void Translator::ProcessMapExpr(AST::Node& originExpr, bool isSubScript)
{
    if (auto base = GetMapExpr(originExpr); base) {
        ...
        /* In the following codira code, we do not need to translate `base`.
        open class A {
...
void Translator::TranslateCompoundAssignmentElementRef(const AST::MemberAccess& ma)
{
    ...
    if (baseResLeftValuePath.empty()) {
        ...
        if (baseResLeftValue->GetType()->IsReferenceTypeWithRefDims(1) ||
            ...
            CODEC_ASSERT(baseResLeftValue->GetType()->IsValueOrGenericTypeWithRefDims(0));
            auto getMember = CreateAndAppendExpression<FieldByName>(
...
Ptr<Value> Translator::Visit(const AST::CallExpr& callExpr)
{
    ...
    CODEC_ASSERT(callExpr.resolvedFunction);
    ...
    if (callExpr.resolvedFunction->isSetter) {
        ...
        CODEC_ASSERT(callExpr.baseFunc->astKind == AST::ASTKind::MEMBER_ACCESS);
        auto ma = StaticCast<AST::MemberAccess*>(callExpr.baseFunc.get());
        CODEC_NULLPTR_CHECK(ma->baseExpr->mapExpr);
        TranslateCompoundAssignmentElementRef(*ma);
    } else {
        CODEC_ABORT();
    }
