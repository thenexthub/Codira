...
#include "Codira/CHIR/AST2CHIR/GlobalVarInitializer.h"
...
inline std::string GetPackageInitFuncName(const std::string& pkgName, const std::string& suffix = "")
{
    ...
    return MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_PACKAGE_INIT_PREFIX + MangleUtils::GetOptPkgName(pkgName) +
    ...
    const std::string& pkgName, const std::string& fileName, const std::string_view suffix)
{
    ...
    return MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_FILE_INIT_PREFIX + MangleUtils::GetOptPkgName(pkgName) +
...
{
    return MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_VARIABLE_INIT_PREFIX + MangleUtils::GetOptPkgName(pkgName) +
...
{
    return MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_VARIABLE_INIT_PREFIX +
        var.mangledName.substr((MANGLE_CODIRA_PREFIX + MANGLE_NESTED_PREFIX).size(),
            var.mangledName.size() - (MANGLE_CODIRA_PREFIX + MANGLE_NESTED_PREFIX + MANGLE_SUFFIX).size()) +
...
template <> struct GVInit<AST::VarWithPatternDecl> {
    explicit GVInit(const AST::VarWithPatternDecl& decl, const Translator& trans)
        : srcCodeIdentifier(GetGVInitNameForPattern(decl)),
          mangledName(MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_VARIABLE_INIT_PREFIX +
              decl.mangledName.substr((MANGLE_CODIRA_PREFIX + MANGLE_NESTED_PREFIX).size(),
                  decl.mangledName.size() - (MANGLE_CODIRA_PREFIX + MANGLE_NESTED_PREFIX + MANGLE_SUFFIX).size()) +
              MANGLE_FUNC_PARAM_TYPE_PREFIX + MANGLE_VOID_TY_SUFFIX),
...
static Ptr<Block> GetBlockWithInitializers(const BlockGroup& blockGroup)
{
    ...
    CODEC_ABORT(); // there is no block related to initalization process
    return nullptr;
}
...
{
    CODEC_ASSERT(packageInit.TestAttr(Attribute::INITIALIZER));
...
Func* GlobalVarInitializer::TranslateInitializerToFunction(const AST::VarDecl& decl)
{
    ...
    CODEC_ASSERT(variable->GetType()->IsRef());
...
ImportedFunc* GlobalVarInitializer::TranslateIncrementalNoChangeVar(const AST::VarDecl& decl)
{
    ...
    func->Set<LinkTypeInfo>(Codira::Linkage::INTERNAL);
    if (decl.isConst) {
...
void GlobalVarInitializer::RemoveInitializerForVarDecl(const AST::VarDecl& varDecl, Func& fileInit) const
{
    ...
    for (auto expr : blockWithInitializers->GetExpressions()) {
        ...
        CODEC_ASSERT(initFunc->TestAttr(Attribute::INITIALIZER));
    ...
    const AST::File& file, const std::vector<Ptr<const AST::Decl>>& decls)
{
    ...
    if (fileInit) {
        CODEC_ASSERT(fileInit->TestAttr(Attribute::INITIALIZER));
    ...
    const AST::File& file, const std::vector<Ptr<const AST::Decl>>& decls)
{
    ...
    for (auto vd : varsToGenInit) {
        ...
        // this is in gv init for literal, we can't set breakpoint with codedb, so we can't set DebugLocationInfo
        // for any expression
...
// [CODEMP]: define inlining policy here and in other places after disabling CHIR transformations of common part
...
{
    bool doNotCallInit = dep.srcPackage->isMacroPackage;
#ifdef CODIRA_CODEGEN_CODEVM_BACKEND
    const std::string& pkgName = dep.srcPackage->fullPackageName;
    doNotCallInit =
