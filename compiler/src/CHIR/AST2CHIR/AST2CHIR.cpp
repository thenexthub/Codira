...
#include "Codira/CHIR/AST2CHIR/CollectLocalConstDecl/CollectLocalConstDecl.h"
...
#include "Codira/Utils/ParallelUtil.h"
...
{
    for (auto& decl : decls) {
        CODEC_ASSERT(decl->astKind == AST::ASTKind::VAR_DECL || decl->astKind == AST::ASTKind::VAR_WITH_PATTERN_DECL);
...
void AST2CHIR::SetInitFuncForStaticVar()
{
    for (auto& skipedStaticVar : varsInitedByStaticInitFunc) {
        ...
        CODEC_ASSERT(skipedStaticVar->astKind == AST::ASTKind::VAR_DECL);
        ...
        if (auto skipedStaticVarInCHIR = DynamicCast<GlobalVar*>(skipedStaticVarVal)) {
            ...
            CODEC_NULLPTR_CHECK(staticInitFuncInCHIR);
...
void AST2CHIR::SetGenericDecls() const
{
    ...
    for (auto& mapIt : genericToInsMap) {
        ...
        if (mapIt.first->IsNominalDecl()) {
            ...
            CODEC_NULLPTR_CHECK(chirGeneric);
...
static Package::AccessLevel BuildPackageAccessLevel(const AST::AccessLevel& level)
{
    ...
    CODEC_ASSERT(it != accessLevelMap.end());
    return it->second;
}
...
bool AST2CHIR::ToCHIRPackage(AST::Package& node)
{
    ...
    CODEC_ASSERT(!(outputCHIR && mergingPlatform));
