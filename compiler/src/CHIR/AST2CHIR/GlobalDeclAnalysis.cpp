...
#include "Codira/CHIR/AST2CHIR/GlobalDeclAnalysis.h"
    ...
    const AST::CallExpr& callExpr, std::vector<Ptr<const AST::Decl>>& dependencies)
{
    ...
    //
    // For example, the CODE code below
    //     func foo(x: Int64 = 10) {}
    //     let res = foo()
    ...
    for (size_t i = 0; i < argExprs.size(); ++i) {
        if (argExprs[i]->TestAttr(AST::Attribute::HAS_INITIAL)) {
            CODEC_NULLPTR_CHECK(callExpr.resolvedFunction);
            CODEC_NULLPTR_CHECK(callExpr.resolvedFunction->funcBody.get());
            CODEC_NULLPTR_CHECK(callExpr.resolvedFunction->funcBody->paramLists[0]);
            CODEC_NULLPTR_CHECK(callExpr.resolvedFunction->funcBody->paramLists[0]->params[i]);
...
void GlobalDeclAnalysis::AdditionalAnalysisDepOfNonStaticCtor(const AST::FuncDecl& func,
    std::vector<Ptr<const AST::Decl>>& dependencies, std::vector<Ptr<const AST::Decl>>& localConstVarDeps)
{
    CODEC_ASSERT(func.funcBody->parentClassLike != nullptr || func.funcBody->parentStruct != nullptr);
...
static bool DoesStaticInitDependOnStaticVar(const Ptr<const AST::FuncDecl>& staticInit,
    Ptr<Codira::AST::VarDecl> &staticVar)
{
    ...
    if (staticVar->platformImplementation) {
        ...
        CODEC_NULLPTR_CHECK(platformStaticVar);
    ...
    const Ptr<const AST::FuncDecl>& staticInit, std::vector<Ptr<const AST::Decl>>& dependencies) const
{
    ...
    CODEC_ASSERT(IsStaticInit(*staticInit));
#endif
    ...
    CODEC_NULLPTR_CHECK(parentDecl);
...
static void SaveCODEMPDependencies(Ptr<const AST::Decl> decl, std::vector<Ptr<const AST::Decl>>& dependencies)
{
...
static void RestoreCODEMPDependencies(Ptr<const AST::Decl> decl, std::vector<Ptr<const AST::Decl>>& dependencies)
{
...
void GlobalDeclAnalysis::AnalysisDependency(const ElementList<Ptr<const AST::Decl>>& nodesWithDeps)
{
    ...
    for (auto node : nodesWithDeps.stableOrderValue) {
        ...
        if (outputChir) {
            SaveCODEMPDependencies(node, dependencies);
        }
        if (mergingPlatform) {
            ReplaceCommonDependenciesWithPlatform(dependencies);
            RestoreCODEMPDependencies(node, dependencies);
        }
    ...
    const Ptr<const AST::Decl>& curDecl, std::unordered_set<Ptr<const AST::Decl>>& visited)
{
    ...
    if (pos == funcsAndVarsDepMap.randomOrderValue.end()) {
        CODEC_ASSERT(curDecl->TestAttr(AST::Attribute::SRC_IMPORTED));
        return;
    }
...
{
    auto pos = funcsAndVarsDepMap.randomOrderValue.find(curDecl);
    CODEC_ASSERT(pos != funcsAndVarsDepMap.randomOrderValue.end());
    const auto& directDepsOfCurDecl = pos->second;
    for (auto& dep : directDepsOfCurDecl) {
        if (visited.find(dep) != visited.end()) {
            continue;
