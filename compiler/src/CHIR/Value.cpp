...
Func* Parameter::GetTopLevelFunc() const
{
    ...
    CODEC_NULLPTR_CHECK(ownerLambda);
...
{
    CODEC_ASSERT(!expression->IsTerminator());
...
{
    CODEC_ASSERT(exceptions.has_value());
...
void BlockGroup::SetOwnerExpression(Expression& expr)
{
    ...
    if (expr.IsLambda()) {
        CODEC_ASSERT(ownerExpression == nullptr || ownerExpression->IsLambda());
    } else {
        CODEC_ASSERT(ownerFunc == nullptr && (ownerExpression == nullptr || !ownerExpression->IsLambda()));
    }
#endif
...
{
    CODEC_ASSERT(parent.GetBlocks().empty());
    CODEC_ASSERT(parent.GetEntryBlock() == nullptr);
...
{
    CODEC_ASSERT(funcKind == FuncKind::LAMBDA);
