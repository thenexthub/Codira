...
static size_t CalculateThreshold(const Value& callee, const Codira::GlobalOptions::OptimizationLevel& optLevel)
{
    size_t realThreshold = INIT_INLINE_THRESHOLD;
    auto func = Codira::DynamicCast<const Func*>(&callee);
    CODEC_NULLPTR_CHECK(func);
    if (OnlyCalledOnce(*func)) {
        // Increase the threshold value by 20%
        realThreshold += realThreshold / INCREASE_THRESHOLD;
    }
    if (optLevel < Codira::GlobalOptions::OptimizationLevel::Os) {
...
static size_t GetExprSize(const Expression& expr)
{
    ...
    Visitor::Visit(*Codira::StaticCast<const Lambda&>(expr).GetBody(), postVisit);
    return exprSize;
}
...
{
    CODEC_ASSERT(src.size() == dst.size());
...
void FunctionInline::DoFunctionInline(const Apply& apply, const std::string& name)
{
    ...
    if (apply.GetCallee()->IsFuncWithBody()) {
        ...
        CODEC_ASSERT(apply.GetCallee()->IsLocalVar());
    ...
    CODEC_NULLPTR_CHECK(oldFuncGroup->GetTopLevelFunc());
    ...
    CODEC_NULLPTR_CHECK(apply.GetTopLevelFunc());
    ...
    CODEC_NULLPTR_CHECK(newFuncGroup->GetTopLevelFunc());
    ...
    CODEC_NULLPTR_CHECK(applyGroup->GetTopLevelFunc());
    ...
    //   Exit()
    // }
    CODEC_NULLPTR_CHECK(block1->GetTerminator());
    ...
    for (auto exitBlock : exitBlocks) {
        if (exitBlock != nullptr) {
            CODEC_NULLPTR_CHECK(exitBlock->GetTerminator());
