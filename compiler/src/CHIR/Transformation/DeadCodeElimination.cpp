...
bool ShouldSkipUselessFuncElimination(const Package& package, const Codira::GlobalOptions& opts)
{
    if (opts.optimizationLevel < Codira::GlobalOptions::OptimizationLevel::O2 ||
        opts.enableCoverage || opts.enIncrementalCompilation ||
        package.GetName() == Codira::REFLECT_PACKAGE_NAME) {
        return true;
    }
    return false;
}
...
std::string DeadCodeElimination::GetLiteralFromExprKind(const ExprKind& kind) const
{
    ...
    CODEC_ASSERT(it != EXPR_KIND_TO_STR.end());
    return it->second;
}
...
bool HandleFuncWithNothingParamter(CHIRBuilder& builder, const BlockGroup& funcBody)
{
    ...
    for (auto param : params) {
        ...
        for (auto expr : entry->GetExpressions()) {
            if (auto debug = Codira::DynamicCast<Debug*>(expr);
...
{
    CODEC_ASSERT(Codira::Utils::In(expr->GetExprKind(),
...
std::optional<Block*> GetBranchTargetSucc(const Branch& branch)
{
    ...
    auto condExpr = Codira::StaticCast<LocalVar*>(branch.GetCondition())->GetExpr();
    if (condExpr->GetExprKind() != ExprKind::CONSTANT) {
        return std::nullopt;
    }
    return Codira::StaticCast<Constant*>(condExpr)->GetBoolLitVal() ? branch.GetTrueBlock() : branch.GetFalseBlock();
}
...
std::optional<Block*> GetMultiBranchTargetSucc(const MultiBranch& branch)
{
    ...
    if (condExpr->GetExprKind() == ExprKind::TYPECAST) {
        ...
        condExpr = Codira::StaticCast<LocalVar*>(condExpr->GetOperand(0))->GetExpr();
    }
    if (condExpr->GetExprKind() != ExprKind::CONSTANT) {
        return std::nullopt;
    }

    auto constant = Codira::StaticCast<Constant*>(condExpr);
    uint64_t condVal = 0;
    if (constant->IsBoolLit()) {
        ...
        Codira::InternalError("Unexpected const val kind");
    }
...
void DeadCodeElimination::BreakBranchConnection(const Block& block) const
{
    for (auto pred : block.GetPredecessors()) {
        ...
        if (predTerminator->GetExprKind() == ExprKind::BRANCH) {
            ...
            CODEC_ASSERT(&block == branch->GetFalseBlock());
            ...
            CODEC_ASSERT(&block == multi->GetDefaultBlock());
...
bool DeadCodeElimination::CheckUselessFunc(const Func& func, const GlobalOptions& opts)
{
    ...
    if (func.GetFuncKind() == Codira::CHIR::MAIN_ENTRY && !opts.interpreter) {
        // if output is lib (static or shared), Main method should delete
        // when use chir-interpreter execute codec-frontend ('codec-frontend --chir-interpreter ...'), we should not
        // delete main method
    ...
    // should be revised
    if (func.GetFuncKind() == Codira::CHIR::CLASS_CONSTRUCTOR) {
        return false;
    }
    // The Finalizer func of a class, can not be removed.
    if (func.GetFuncKind() == Codira::CHIR::FINALIZER) {
        return false;
    }
...
Ptr<Expression> DeadCodeElimination::GetUnreachableExpression(const CHIR::Block& block, bool& isNormal) const
{
    ...
    auto it = std::find_if(expressions.begin(), expressions.end(), [&isNormal, &resExpression, this](auto expression) {
        ...
        if (res) {
            // when report deadcode in BinaryExpr, it has 4 situations in tests/LLT/CHIR/DCE/testUnusedOp03.code
