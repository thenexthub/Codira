...
void ConstPropagation::RunOnPackage(const Ptr<const Package>& package, bool isDebug, bool isCODELint)
{
    for (auto func : package->GetGlobalFuncs()) {
        RunOnFunc(func, isDebug, isCODELint);
    }
}

void ConstPropagation::RunOnFunc(const Ptr<const Func>& func, bool isDebug, bool isCODELint)
{
    ...
    const auto actionAfterVisitExpr = [this, &toBeRewrited, func, isDebug, isCODELint](
        ...
        if (expr->IsBinaryExpr()) {
            if (auto absVal = state.CheckAbstractValue(expr->GetResult()); absVal) {
                return (void)toBeRewrited.emplace_back(expr, index, GenerateConstExpr(exprType, absVal, isCODELint));
        ...
        if ((exprType->IsInteger() || exprType->IsFloat() || exprType->IsRune() ||
            ...
            if (absVal) {
                toBeRewrited.emplace_back(expr, index, GenerateConstExpr(exprType, absVal, isCODELint));
    ...
    const auto actionOnTerminator = [this, &targetSuccMap, isCODELint](const ConstDomain& state, Terminator* terminator,
            ...
            case ExprKind::TYPECAST_WITH_EXCEPTION: {
                ...
                if (auto absVal = state.CheckAbstractValue(res)) {
                    targetSuccMap.emplace(terminator,
                        std::make_pair(GenerateConstExpr(res->GetType(), absVal, isCODELint), targetSucc.value()));
                }
                return;
            }
            default:
...
Ptr<LiteralValue> ConstPropagation::GenerateConstExpr(
    const Ptr<Type>& type, const Ptr<const ConstValue>& constVal, bool isCODELint)
{
