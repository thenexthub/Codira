...
*/
static const std::string SanCovBoolFlagInitName = "__code_sancov_bool_flag_ctor";
/*
...
*/
static const std::string SanCovCharArrayInitName = "__code_sancov_8bit_counters_ctor";
/*
...
*/
static const std::string SanCovUInt32ArrayInitName = "__code_sancov_pc_guard_ctor";
...
return: void.
*/
static const std::string SAN_COV_TRACE_MEM_CMP = "__code_sanitizer_weak_hook_memcmp";
static const std::string SAN_COV_TRACE_STRN_CMP = "__code_sanitizer_weak_hook_strncmp";
static const std::string SAN_COV_TRACE_STR_CMP = "__code_sanitizer_weak_hook_strcmp";
static const std::string SAN_COV_TRACE_STR_CASE_CMP = "__code_sanitizer_weak_hook_strcasecmp";
/*
brief: this func is used when sanitizer-coverage-trace-compares is available.
...
return: void
*/
static const std::string SanCovCreatePCTable = "__code_sancov_pcs_init";
static const std::string SanCovUpdateStackDepth = "__code_sancov_update_stack_depth";
    ...
    const std::string& fuzzName, Value& oper1, Value& oper2, Apply& apply)
{
    /*
        ...
        %5: Unit = Apply(@__code_sanitizer_weak_hook_strcmp, %3, %4, %2)
        %c: Bool = Apply(@_CNatX7CString6equalsHk, %a, %b)
    */
    ...
    if (fuzzName == SAN_COV_TRACE_MEM_CMP) {
        // __code_sanitizer_weak_hook_memcmp function need void* input
...
{
    /*
        generate cpointer data from code array:
    ...
    const std::string& fuzzName, Value& oper1, Value& oper2, Apply& apply)
{
    /*
        ...
        %9: CPointer<UInt8> = Intrinsic/addPointer(%7, %5)  // for code array implement, add offset
        %10: CPointer<UInt8> = Intrinsic/addPointer(%8, %6)
        %11: Int64 = Field(%1, 2)  // get size of string array
        %12: UInt32 = TypeCast(%11, UInt32)
        %13: Unit = Apply(@__code_sanitizer_weak_hook_memcmp, %9, %10, %12)
        %c: Bool = Apply(@_CNat6String2==ERNat6StringE, %a, %b)
    */
    std::vector<Value*> res;
    auto parent = apply.GetParentBlock();
...
{
    CODEC_ASSERT(type.IsInteger() || type.IsFloat());
    ...
    const std::string& fuzzName, Value& oper1, Value& oper2, Apply& apply)
{
    /*
        ...
        %7: CPointer<Int64> = Intrinsic/addPointer(%5, %3)  // for code array implement, add offset
    ...
    */
    ...
    auto parent = apply.GetParentBlock();
    auto& loc = apply.GetDebugLocation();
    CODEC_ASSERT(oper1.GetType()->GetTypeKind() == Type::TypeKind::TYPE_STRUCT);
    auto arrayType = StaticCast<StructType*>(oper1.GetType());
    CODEC_ASSERT(arrayType->GetGenericArgs().size() == 1);
...
std::pair<Value*, Value*> SanitizerCoverage::CastArrayListToArray(Value& oper1, Value& oper2, Apply& apply)
{
    ...
    */
    ...
    CODEC_ASSERT(rawArrayType->GetGenericArgs().size() == 1);
    ...
    CODEC_NULLPTR_CHECK(arrayGeneric);
    CODEC_ASSERT(arrayGeneric->GetGenericArgs().size() == 1);
...
void SanitizerCoverage::InsertCoverageAheadBlock(Block& block, bool isDebug)
{
    if (sanCovOption.pcTable) {
        CODEC_ASSERT(block.GetTopLevelFunc());
    ...
    CODEC_ASSERT(block.GetExpressions().size() > 0);
...
Func* SanitizerCoverage::CreateArrayInitFunc(const std::string& initItemName, Type& initType)
{
    ...
    auto initName = MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_VARIABLE_INIT_PREFIX + MangleUtils::MangleName("default") +
...
{
    auto funcTy = builder.GetType<FuncType>(std::vector<Type*>{}, builder.GetVoidTy());
    auto func = CreateInitFunc(MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_VARIABLE_INIT_PREFIX +
...
void SanitizerCoverage::CreateTopLevelInitFunc(const std::vector<Func*>& initFuncs, const Func& globalInitFunc)
{
    ...
    auto funcTy = builder.GetType<FuncType>(std::vector<Type*>{}, builder.GetVoidTy());
    auto initName = MANGLE_CODIRA_PREFIX + MANGLE_GLOBAL_FILE_INIT_PREFIX + MangleUtils::MangleName("default") +
