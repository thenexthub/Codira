...
void Devirtualization::InstantiateFuncIfPossible(CHIRBuilder& builder, std::vector<RewriteInfo>& rewriteInfoList)
{
    for (auto rewriteInfo = rewriteInfoList.rbegin(); rewriteInfo != rewriteInfoList.rend(); ++rewriteInfo) {
        ...
        if (frozenInstFuncMap.count(newId) != 0) {
            ...
            CODEC_ASSERT(parameterType.size() == callee->GetParams().size());
...
void BuildOrphanTypeReplaceTable(
    const Codira::CHIR::Type* mayBeGeneric, std::unordered_map<const GenericType*, Type*>& replaceTable)
{
    if (auto genericType = Codira::DynamicCast<const GenericType*>(mayBeGeneric);
        genericType && genericType->orphanFlag) {
        CODEC_ASSERT(genericType->GetUpperBounds().size() == 1);
    ...
    std::unordered_map<const GenericType*, Type*>& replaceTable) const
{
    ...
    if (expected->IsGeneric()) {
        auto generic = Codira::StaticCast<const GenericType*>(expected);
    ...
    if (expected->IsNominal()) {
        const CustomType* specificCustomTy = Codira::StaticCast<const CustomType*>(specific);
