...
static const FuncInfo ARRAY_INIT_FUNC_INFO{"arrayInitByFunction", "", {NOT_CARE}, NOT_CARE, Codira::CORE_PACKAGE_NAME};
Ptr<Constant> ArrayLambdaOpt::CheckCanRewriteLambda(const Ptr<Expression>& expr) const
{
    ...
    CODEC_ASSERT(apply->GetArgs().size() == INIT_LAMBDA_INDEX + 1);
...
Ptr<Constant> ArrayLambdaOpt::CheckIfLambdaReturnConst(const Lambda& lambda) const
{
    ...
    CODEC_ASSERT(ret->GetExpr()->GetExprKind() == ExprKind::ALLOCATE);
...
{
    CODEC_ASSERT(init->GetResult()->GetUsers().empty());
