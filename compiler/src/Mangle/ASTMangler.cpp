...
{
    CODEC_ASSERT(type.astKind == ASTKind::REF_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::PAREN_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::OPTION_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::FUNC_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::CONSTANT_TYPE);
    ...
    CODEC_ASSERT(ce->astKind == ASTKind::LIT_CONST_EXPR);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::VARRAY_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::TUPLE_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::QUALIFIED_TYPE);
...
{
    CODEC_ASSERT(type.astKind == ASTKind::PRIMITIVE_TYPE);
    ...
    CODEC_ASSERT(primitiveTy.ty && primitiveTy.ty->kind == TypeKind::TYPE_INITIAL);
...
static std::string MangleType(const AST::Type& typeAnnotation)
{
    ...
    CODEC_ASSERT(false && "Unexpected type annotation.");
    return "";
}
...
private:
    ...
    void MangleOthers(const Decl& decl)
    {
        ...
        if (auto funcDecl = DynamicCast<const FuncDecl*>(&decl)) {
            CODEC_ASSERT(!funcDecl->funcBody->paramLists.empty());
