...
{
    CODEC_ASSERT(!decimal.empty() && "the string of decimal is empty.");
...
inline void RecursionTypes(size_t& idx, char pre, std::string& compressed,
    std::tuple<size_t, std::vector<std::unique_ptr<CODEType>>&, std::unordered_map<std::string, size_t>&, size_t&> tuple)
{
...
inline std::string StripCodiraAt(std::string& identifier, size_t &pid)
{
...
inline std::string StripCodiraPrefix(std::string& identifier, size_t &pid)
{
...
inline size_t ForwardGenericTypes(std::string& mangled, std::vector<std::unique_ptr<CODEType>>& tys, bool& isCompressed,
    size_t idx)
{
 ...
 */
inline size_t ForwardClassType(std::string& mangled, BaseType bt, std::vector<std::unique_ptr<CODEType>>& tys,
 ...
 */
inline size_t ForwardFunctionType(std::string& mangled, std::vector<std::unique_ptr<CODEType>>& tys, size_t idx,
    bool& isCompressed)
{
    ForwardCompressed(mangled, idx, isCompressed);
    if (isCompressed) {
    ...
    std::vector<std::unique_ptr<CODEType>> retTy;
...
inline size_t ForwardTupleType(std::string& mangled, std::vector<std::unique_ptr<CODEType>>& tys, size_t idx,
    bool& isCompressed)
{
    ForwardCompressed(mangled, idx, isCompressed);
    if (isCompressed) {
...
inline size_t ForwardCPointer(std::string& mangled, size_t idx, bool& isCompressed)
{
    ...
    std::vector<std::unique_ptr<CODEType>> ty;
...
size_t ForwardType(std::string& mangled, std::vector<std::unique_ptr<CODEType>>& tys, bool& isCompressed, size_t idx)
{
    if (idx >= mangled.size()) {
        return idx;
    }
    ...
    if (nextIdx != idx) {
        ...
        if (IsGeneralType(bt)) {
            tys.push_back(std::make_unique<CODEType>(CODEType(mangledTy, bt)));
        }
        return nextIdx;
    } else {
        tys.push_back(std::make_unique<CODEType>(CODEType(mangledTy, bt)));
...
std::string CODEMangledCompression(const std::string mangled, bool isType)
{
    bool isCompressed = false;
    size_t preIdx = 0;
    std::string cMangled = mangled;
    cMangled = StripCodiraAt(cMangled, preIdx);
    if (cMangled.empty() || isType || cMangled.find(MANGLE_CODIRA_PREFIX, 0) != 0 || NeedCompressed(cMangled)) {
        return mangled;
    }
    // Skip "_C"
    cMangled = StripCodiraPrefix(cMangled, preIdx);
    ...
    std::vector<std::unique_ptr<Entity>>& entities, size_t idx)
{
    ...
    EntityType entityTy = EntityType::ERROR_ENTITY_TYPE;
    std::string entityName = "";
    std::vector<std::unique_ptr<CODEType>> baseTy;
    ...
    std::string& mangled, std::string compressed, size_t idx, size_t& mid, bool& isCompressed)
{
    ...
    if (idx < n && mangled[idx] == MANGLE_GENERIC_PREFIX[0]) {
        ...
        std::vector<std::unique_ptr<CODEType>> genericsTys;
    ...
    std::string& compressed, bool isReplaced)
{
    ...
    switch (ty->baseTy) {
        ...
        case BaseType::VARRAY_TYPE: {
            ...
            std::vector<std::unique_ptr<CODEType>> vty;
        ...
        case BaseType::CPOINTER_TYPE: {
            ...
            std::vector<std::unique_ptr<CODEType>> pty;
