...
const std::string CODEC_VERSION_STR = "codec_version";
...
ConditionalCompilationImpl::ConditionalCompilationImpl(CompilerInstance* c)
    : ci(c),
      ...
      codecVersion(GetVersionUInt(CODIRA_VERSION)),
...
{
    // in codec cfg has been parsed, in lsp need parse and check cfg here.
...
{ // If condition is not builtin and not user defined, break it.
    ...
    // Check `codec_version`.
    if (conditionStr == CODEC_VERSION_STR) {
        std::regex codecVersionRegex("[0-9]{1,2}[.][0-9]{1,2}[.][0-9]{1,2}");
        if (!std::regex_match(right, codecVersionRegex)) {
            (void)ci->diag.DiagnoseRefactor(
                DiagKindRefactor::conditional_compilation_not_support_codec_version_format, begin);
            return false;
        }
    }
...
std::optional<std::string> ConditionalCompilationImpl::GetRelatedInfo(const std::string& target) const
{
    ...
    const std::map<const std::string, std::function<std::string()>> conditionMap = {
        ...
        {CODEC_VERSION_STR, [this]() -> std::string { return GetCODECVersion(); }},
