...
{
    CODEC_ASSERT(ColorPrintMap.find(color) != ColorPrintMap.end() && "no print color found");
...
void DiagnosticEmitterImpl::CollectInformation(std::vector<CollectedInfo>& vec, IntegratedString& str, bool isMain)
{
    ...
    info.isMain = isMain;
    CODEC_ASSERT(str.range.end >= str.range.begin);
    if (str.range.begin.line == str.range.end.line) {
        ...
        CODEC_ASSERT(str.range.end.column > 1 && "end of range must be bigger than 1");
...
void DiagnosticEmitterImpl::SortAndCheck(std::vector<CollectedInfo>& errorInfo) const
{
    std::sort(errorInfo.begin(), errorInfo.end(), [this](auto& a, auto& b) -> bool {
        if (enableRangeCheckICE) {
            CODEC_ASSERT(a.range.begin != b.range.begin);
        }
...
{
    CODEC_ASSERT(!errorInfo.empty() && "error info cannot be empty");
    if (errorInfo.empty()) {
        return std::string{};
    }
...
size_t DiagnosticEmitterImpl::GetDisplayedWidthFromSource(const std::string& source, const Range& range) const
{
    ...
    CODEC_ASSERT(!source.empty() && "source cannot be empty");
    if (source.empty()) {
    ...
    CODEC_ASSERT(start >= 1 && "wrong start position");
    CODEC_ASSERT(end >= start);
    ...
    const CollectedInfo& info, const std::string& sourceLine) const
{
    for (size_t i = 0; i < insertedStr.size(); i++) {
        CODEC_ASSERT(insertedStr[i].meta.size() >= loc);
        if (i == 0) {
...
static size_t GetMaxLineColumn(std::vector<CollectedInfo>& errorInfos)
{
    ...
    std::for_each(errorInfos.begin(), errorInfos.end(), [&](auto& info) {
        CODEC_ASSERT(info.range.end.line >= 0);
...
{
    CODEC_ASSERT(!infoMap.empty() && "info map is empty");
    if (infoMap.empty()) {
        return;
    }
    ...
    const CollectedInfo& info, const std::string& sourceLine)
{
    ...
    if (isFirstLine) {
        ...
        CODEC_ASSERT(!insertedStr.empty() && "source combined vec cannot be empty");
...
{
    CODEC_ASSERT(info.isMultiLine);
    auto loc = multiLineRecordMap[info];
    CODEC_ASSERT(loc < multiLineHangingPtrVec.size() && "multiLineHangingPtrVec is less then loc");
    ...
    if (!info.isEnd) {
        ...
        if (multiLineHangingVec.empty()) {
            ...
            if (c == multiLineHangingVec.rbegin()) {
                ...
                CODEC_ASSERT(index >= 0);
                size_t uIndex = static_cast<size_t>(index);
                CODEC_ASSERT(uIndex < multiLineHangingVec.size() && "index overflow in vector");
        ...
        CODEC_ASSERT(column < multiLineHangingVec.size() && "column is bigger than size of multiLineHangingVec");
        if (column >= multiLineHangingVec.size()) {
            return;
        }
        CODEC_ASSERT(line < multiLineHangingVec[column].size() && "line is bigger than size of multiLineHangingVec");
...
bool DiagnosticEmitterImpl::CombineErrorPrint(CollectedInfoMap& infoMap, SourceCombinedVec& combinedVec)
{
    ...
    for (auto& infos : std::as_const(infoMap)) {
        ...
        CODEC_ASSERT(i < combinedVec.size());
    ...
    HangingStr& hanging, size_t i, size_t begin, size_t end, const DiagColor& color) const
{
    ...
    while (k < hanging.size()) {
        CODEC_ASSERT(begin < hanging[k].size() && "begin is bigger than hanging size");
        ...
        CODEC_ASSERT(end - 1 < hanging[k].size() && "end is bigger than hanging size");
    ...
    k = begin + 1;
    while (k < end) {
        CODEC_ASSERT(k < hanging[i].size() && "begin is bigger than hanging size");
        if (k >= hanging[i].size()) {
            break;
        }
    ...
    std::vector<CollectedInfo>& infos) const
{
    for (auto& sub : subMap[line]) {
        ...
        CODEC_ASSERT(range.end.line == range.begin.line);
        auto size = range.end.column - range.begin.column;
        CODEC_ASSERT(size >= 0);
        CODEC_ASSERT(range.begin.column > 0);
        CODEC_ASSERT(static_cast<size_t>(range.begin.column - 1) < rawStr.size() &&
...
{
    CODEC_ASSERT(!help.substitutions.empty());
    ...
    for (auto& combinedLine : combinedVec) {
        ...
        std::sort(combinedLine.colors.begin(), combinedLine.colors.end(), [](auto& c1, auto& c2) {
            if (std::get<0>(c1) == std::get<0>(c2)) {
                CODEC_ASSERT(std::get<1>(c1) != std::get<1>(c2));
