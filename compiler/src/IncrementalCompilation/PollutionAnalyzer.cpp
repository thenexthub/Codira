...
#include "Codira/Sema/IncrementalUtils.h"
...
void TypeMap::CollectImportedDeclExtraRelation(const AST::Decl& decl)
{
    ...
    for (auto& parentType: type.inheritedTypes) {
        ...
        CODEC_NULLPTR_CHECK(parentDecl); // all builtin types are struct or enum and cannot have child types
...
{
    CODEC_ASSERT(end != 0 && end <= mangle.size());
    for (auto i{end - 1}; ; --i) {
        if (mangle[i] == '.') {
            return i + 1;
        }
...
void PollutionAnalyzer::PollutionForSigChangedDecl(const Decl& decl)
{
    switch (decl.astKind) {
        ...
        case ASTKind::FUNC_PARAM:
            CODEC_ASSERT(decl.astKind == ASTKind::VAR_DECL ||
...
{
    CODEC_ASSERT(decl.astKind == ASTKind::ENUM_DECL || decl.astKind == ASTKind::STRUCT_DECL ||
...
{
    CODEC_ASSERT(decl.astKind == ASTKind::STRUCT_DECL || decl.astKind == ASTKind::CLASS_DECL ||
