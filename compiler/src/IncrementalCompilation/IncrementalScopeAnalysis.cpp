...
using namespace Codira::IncrementalCompilation;
...
static const std::string OutputDeclInfo(const RawMangled2DeclMap& rawMangled2Decl, const std::string& rawMangledName)
{
    ...
    CODEC_NULLPTR_CHECK(srcIt->second);
    ...
    const OptEffectStrMap& newMap, OptEffectStrMap& lastCachedMap, const RawMangled2DeclMap& rawMangled2Decl)
{
    for (auto mapIt : newMap) {
        CODEC_ASSERT(!mapIt.first.empty());
        CODEC_ASSERT(mapIt.second.find(mapIt.first) == mapIt.second.end());
        if (mapIt.second.empty()) {
            continue;
        }
        auto& effectInfo = lastCachedMap[mapIt.first];
        for (auto effectedIt : mapIt.second) {
            CODEC_ASSERT(!effectedIt.empty());
...
struct ImportPackageWalker {
     ...
     *
     * @param decl Imported AST::Decl from codeo
     *
    ...
    static void ReplaceTypeWithTyInMangleName(AST::Decl& decl)
    {
        ...
        if (Utils::NotIn(decl.astKind, {AST::ASTKind::FUNC_DECL, AST::ASTKind::VAR_DECL}) ||
            ...
            // 1. Ignore VarWithPatternDecl because there is no VarWithPatternDecl in codeo.
        ...
        if (decl.astKind == AST::ASTKind::FUNC_DECL) {
            ...
            CODEC_ASSERT(funcTy && funcTy->retTy);
...
IncreResult Codira::IncrementalScopeAnalysis(IncrementalScopeAnalysisArgs&& args)
{
    ...
    std::string codeoPath = args.op.GenerateCachedPathName(args.srcPackage.fullPackageName, SERIALIZED_FILE_EXTENSION);
    if (badCache || !FileUtil::FileExist(codeoPath)) {
    ...
    CODEC_NULLPTR_CHECK(srcPkgDecl);
