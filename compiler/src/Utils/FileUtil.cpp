...
constexpr std::string_view TEST_FILE_NAME{"_test.code"};
...
bool CheckCommandLineInjection(const std::string& cmd)
{
    ...
    // Backticks are allowed becasue module names of Codira accept backticks. A user may wrap a
...
bool HasCODEDExtension(std::string_view path)
{
    auto fileIt = path.rbegin();
    auto fileEnd = path.rend();
    auto extensionIt = CODE_D_FILE_EXTENSION.rbegin();
    auto extensionEnd = CODE_D_FILE_EXTENSION.rend();
    ...
    // exclude .code.d file with no filename
    ...
    bool shouldSkipRegularFiles)
{
    ...
    // Not compile test, and file is ending with '_test.code'.
...
bool IsIdentifier(const std::string& str, bool useContextualKeyWords)
{
    ...
    while (start < end) {
        ...
        if (isFirst) {
            if (!IsASCIIIdStart(codepoint) && !IsCODEXIDStart(codepoint)) {
                return false;
            }
...
std::string ConvertPackageNameToLibCodiraBaseFormat(const std::string& fullPackageName)
{
    if (auto found = STANDARD_LIBS.find(fullPackageName); found != STANDARD_LIBS.end()) {
        ...
        if (names.size() == 1) {
            return "codira-" + found->second;
        }
        auto moduleName = names.front();
        return "codira-" + moduleName + "-" + found->second;
    }
    return {};
}

#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
std::string ConvertFilenameToLibCodiraBaseFormat(const std::string& objectFile)
{
    auto fullPackageName = GetFileNameWithoutExtension(objectFile);
    return ConvertPackageNameToLibCodiraBaseFormat(fullPackageName);
}

std::string ConvertFilenameToLibCodiraFormat(const std::string& objectFile, const std::string& extension)
{
    auto fullPackageName = GetFileNameWithoutExtension(objectFile);
    auto base = ConvertPackageNameToLibCodiraBaseFormat(fullPackageName);
    return base.empty() ? std::string() : "lib" + base + extension;
}
#endif

std::string ConvertFilenameToLtoLibCodiraFormat(const std::string& objectFile)
{
...
{
    auto names = Utils::SplitQualifiedName(fullPackageName);
    auto packageName = ToCodeoFileName(fullPackageName);
    auto fileName = ToCodeoFileName(names.front()) + DIR_SEPARATOR + packageName;
...
{
    // for the path: aa/bb/cc.code, the absolutePath is "aa/bb", and the last "/" is need to be added.
#ifdef _WIN32
    char split = '\\';
...
std::string ToCodeoFileName(std::string_view fullPackageName)
{
...
std::string ToPackageName(std::string_view codeoName)
{
    auto it = codeoName.find(ORG_NAME_SEPARATOR);
    if (it != std::string::npos) {
        return std::string{codeoName.substr(it + 1)} + TOKENS[static_cast<int>(TokenKind::DOUBLE_COLON)] +
            std::string{codeoName.substr(0, it)};
    }
    return std::string{codeoName};
}
} // namespace Codira::FileUtil
