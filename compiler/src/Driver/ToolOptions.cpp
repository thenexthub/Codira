...
{
    setOptionHandler("--codira-pipeline");
    SetOptionIf(setOptionHandler, driverOptions.EnableAsan(), "-code-asan=true");
    SetOptionIf(setOptionHandler, driverOptions.EnableTsan(), "-code-tsan=true");
    SetOptionIf(setOptionHandler, driverOptions.EnableTsan(), "-tsan-instrument-atomics=false");
    SetOptionIf(setOptionHandler, driverOptions.EnableTsan(), "--code-tsan-support-for-mutexopt");
    SetOptionIf(setOptionHandler, driverOptions.EnableHwAsan(), "-code-hwasan=true");
    ...
    SetOptionIf(setOptionHandler,
        ...
        "--code-ptrauth-backward-cfi=true");
}
...
void SetLTOOptions(SetFuncType setOptionHandler, const DriverOptions& driverOptions)
{
    if (driverOptions.IsFullLTOEnabled()) {
        ...
        setOptionHandler("--codira-full-lto");
    } else {
...
{
    if (driverOptions.enablePgoInstrGen) {
        setOptionHandler("-code-pgo-kind=pgo-instr-gen-pipeline");
    }

    if (driverOptions.enablePgoInstrUse) {
        setOptionHandler("-code-pgo-kind=pgo-instr-use-pipeline");
        setOptionHandler("-code-profile-file=" + driverOptions.pgoProfileFile);
    }
}
} // namespace LLD
