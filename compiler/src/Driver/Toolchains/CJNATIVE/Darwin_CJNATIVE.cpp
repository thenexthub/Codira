 ...
 * @file
 *
 * This file implements the Darwin_CODENATIVE ToolChain base class.
 */
...
void Darwin_CODENATIVE::AddSystemLibraryPaths()
{
...
TempFileInfo Darwin_CODENATIVE::GenerateLinkingTool(const std::vector<TempFileInfo>& objFiles,
...
void Darwin_CODENATIVE::GenerateLinkOptions(Tool& tool)
{
    for (auto& option : DARWIN_CODENATIVE_LINK_OPTIONS) {
        tool.AppendArg(option);
    }
    auto codiraLibPath =
        FileUtil::JoinPath(FileUtil::JoinPath(driver.codiraHome, "lib"), driverOptions.GetCodiraLibTargetPathName());
    tool.AppendArg(FileUtil::JoinPath(codiraLibPath, "libclang_rt.osx.a"));
}
