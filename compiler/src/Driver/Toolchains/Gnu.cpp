...
void Gnu::HandleAsanDependencies(Tool& tool, const std::string& codiraLibPath, const std::string& gccLibPath)
{
    if (driverOptions.outputMode != Codira::GlobalOptions::OutputMode::EXECUTABLE) {
        return;
    }
    ...
    // Static library
    auto asanLib = FileUtil::FindFileByName("libclang_rt-asan.a", {codiraLibPath});
...
void Gnu::HandleHwasanDependencies(Tool& tool, const std::string& codiraLibPath)
{
    if (driverOptions.outputMode != Codira::GlobalOptions::OutputMode::EXECUTABLE) {
        return;
    }
    ...
    // Static library
    auto asanLib = FileUtil::FindFileByName("libclang_rt-hwasan.a", {codiraLibPath});
    if (!asanLib.has_value()) {
...
void Gnu::HandleSanitizer(Tool& tool, const std::string& codiraLibPath, const std::string& gccLibPath)
{
    if (!driverOptions.EnableSanitizer()) {
        return;
    }

    auto sanitizerPath = FileUtil::JoinPath(codiraLibPath, driverOptions.SanitizerTypeToShortString());
...
void Gnu::HandleLLVMLinkOptions(
    const std::vector<TempFileInfo>& objFiles, const std::string& gccLibPath, Tool& tool, const std::string& codeldScript)
{
    ...
    if (driverOptions.discardEhFrame && driverOptions.target.os != Triple::OSType::WINDOWS) {
        tool.AppendArg("-T");
        tool.AppendArg(FileUtil::JoinPath(codiraLibPath, "discard_eh_frame.lds"));
    }
    // 2. The codeld.lds and codestart.o
    if (driverOptions.target.os == Triple::OSType::WINDOWS) {
        tool.AppendArg(FileUtil::JoinPath(codiraLibPath, "section.o"));
    } else {
        tool.AppendArg("-T");
        tool.AppendArg(FileUtil::JoinPath(codiraLibPath, codeldScript));
    }
    ...
    HandleSanitizer(tool, codiraLibPath, gccLibPath);
...
void Gnu::HandleLibrarySearchPaths(Tool& tool, const std::string& codiraLibPath)
{
    ...
    tool.AppendArgIf(driverOptions.EnableSanitizer(),
        "-L" + FileUtil::JoinPath(codiraLibPath, driverOptions.SanitizerTypeToShortString()));
    ...
    tool.AppendArgIf(driverOptions.EnableSanitizer(),
        "-L" + FileUtil::JoinPath(codiraRuntimeLibPath, driverOptions.SanitizerTypeToShortString()));
...
bool Gnu::ProcessGeneration(std::vector<TempFileInfo>& objFiles)
{
    ...
    CODEC_ASSERT(outputDir.has_value());
