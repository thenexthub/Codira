 ...
 * @file
 *
 * This file implements the codenative backend class.
 */
...
bool CODENATIVEBackend::GenerateToolChain()
{
    std::string errFieldStr;
    switch (driverOptions.target.os) {
        case OSType::LINUX:
            if (driverOptions.target.env == Environment::OHOS) {
                TC = std::make_unique<Ohos_CODENATIVE>(driver, driverOptions, backendCmds);
                return true;
            }
            if (driverOptions.target.env == Environment::ANDROID) {
                TC = std::make_unique<Android_CODENATIVE>(driver, driverOptions, backendCmds);
                return true;
            }
            if (driverOptions.target.env == Environment::GNU ||
                driverOptions.target.env == Environment::NOT_AVAILABLE) {
                TC = std::make_unique<Linux_CODENATIVE>(driver, driverOptions, backendCmds);
                return true;
            }
        ...
        case OSType::WINDOWS:
            if (driverOptions.target.env == Triple::Environment::GNU) {
                TC = std::make_unique<MinGW_CODENATIVE>(driver, driverOptions, backendCmds);
                return true;
            }
            errFieldStr = driverOptions.target.EnvironmentToString();
            Errorf("Unsupported Environment Type: %s\n", errFieldStr.c_str());
            break;
        case OSType::DARWIN:
            TC = std::make_unique<Darwin_CODENATIVE>(driver, driverOptions, backendCmds);
            return true;
        case OSType::IOS:
            TC = std::make_unique<IOS_CODENATIVE>(driver, driverOptions, backendCmds);
            return true;
        case OSType::UNKNOWN:
        default:
...
bool CODENATIVEBackend::PrepareDependencyPath()
{
    std::vector<std::string> codenativeBinSearchPaths;
    // search in CODIRA_HOME if it is available
    if (driverOptions.environment.codiraHome.has_value()) {
        codenativeBinSearchPaths.emplace_back(
            FileUtil::JoinPath(driverOptions.environment.codiraHome.value(), "third_party/llvm/bin"));
    }
    // relative path to the current executing codec
    codenativeBinSearchPaths.emplace_back(FileUtil::JoinPath(driver.codiraHome, "third_party/llvm/bin"));
    // They don't have to be in the same location.
    optPath = FileUtil::FindProgramByName(g_toolList.at(ToolID::OPT).name, codenativeBinSearchPaths);
    if (optPath.empty()) {
        Errorln("not found `opt` in the Codira installation, " + CODIRA_HOME);
        return false;
    }
    llcPath = FileUtil::FindProgramByName(g_toolList.at(ToolID::LLC).name, codenativeBinSearchPaths);
    if (llcPath.empty()) {
        Errorln("not found `llc` in the Codira installation, " + CODIRA_HOME);
        return false;
    }
    return TC->PrepareDependencyPath();
}

std::vector<TempFileInfo> CODENATIVEBackend::GetFrontendOutputs()
{
    ...
    // 1) codec main.code pkg1.bc
    // 2) codec main.code pkg1.o
...
void CODENATIVEBackend::GenerateCacheCopyTool(const std::vector<TempFileInfo>& files, const std::string& extension)
{
    ToolBatch batch{};
    for (auto& file : files) {
 ...
 */
bool CODENATIVEBackend::ProcessGeneration()
{
    CODEC_ASSERT(!driverOptions.frontendOutputFiles.empty() && "non-compiled file found!");
...
bool CODENATIVEBackend::ProcessGenerationOfNormalCompile(const std::vector<TempFileInfo>& bitCodeFiles)
{
...
bool CODENATIVEBackend::ProcessGenerationOfIncrementalNoChangeCompile(const std::vector<TempFileInfo>& bitCodeFiles)
{
...
void CODENATIVEBackend::PreprocessOfNewPassManager(Tool& tool)
{
...
std::vector<TempFileInfo> CODENATIVEBackend::GeneratePreprocessTools(const std::vector<TempFileInfo>& bitCodeFiles)
{
    ...
    for (const auto& bitCodeFile : bitCodeFiles) {
        ...
        std::unique_ptr<Tool> tool = GenerateCODENativeBaseTool(optPath);
        // set input
...
std::vector<TempFileInfo> CODENATIVEBackend::GenerateCompileTool(
    const std::vector<TempFileInfo>& bitCodeFiles, bool emitAssembly)
{
    ...
    for (const auto& bitCodeFile : bitCodeFiles) {
        ...
        std::unique_ptr<Tool> tool = GenerateCODENativeBaseTool(llcPath);
...
std::unique_ptr<Tool> CODENATIVEBackend::GenerateCODENativeBaseTool(const std::string& toolPath)
{
