...
#endif
const std::string CODIRA_TMP_DIR_PERFIX = "/codira-tmp-";
...
{
    std::optional<std::string> absPath = Codira::FileUtil::GetAbsPath(path);
    if (!absPath.has_value()) {
        Codira::Errorf("%s (%s) does not exist.\n",
    ...
    if (fromEnvSetting) {
        if (!Codira::FileUtil::IsDir(path)) {
            Codira::Errorf("Temporary directory path set by environment variable(%s) is not a directory.\n",
                TMP_DIR_ENVIRONMENT_KEY.c_str());
            return {};
        }
    }
    std::string temp = absPath.value();
    if (!Codira::FileUtil::CanWrite(temp)) {
        Codira::Errorf("%s (%s) does not have write permission.\n",
            fromEnvSetting ? "Temporary directory path set by environment variable" : "Temporary directory path",
            fromEnvSetting ? TMP_DIR_ENVIRONMENT_KEY.c_str() : DEFAULT_TMP_DIR.c_str());
        return {};
    };
...
{
    std::stringstream ss;
    ss << CODIRA_TMP_DIR_PERFIX;
    ss << SetNowTimeEncodedString();
    ss << "-" << Codira::Utils::GenerateRandomHexString();
    return Codira::FileUtil::JoinPath(tempDir, ss.str());
}
...
std::optional<std::string> MakeTempDir(const std::string& tempDir, bool fromEnvSetting)
{
    ...
    do {
        ...
        loopSize--;
#ifdef _WIN32
        std::optional<std::wstring> tempValue = Codira::StringConvertor::StringToWString(path);
    ...
    Codira::Errorf("Failed to create the directory in temporary directory path %s(%s)%s\n",
...
void RemoveDirRecursively(const std::string& dirPath)
{
    ...
    for (auto entry = readdir(dir); entry != nullptr; entry = readdir(dir)) {
        ...
        std::string newPath = Codira::FileUtil::JoinPath(dirPath, fileName);
...
bool TempFileManager::Init(const GlobalOptions& options, bool isFrontend)
{
    ...
    fileSuffixMap = {
        {TempFileKind::O_CODEO, []() { return SERIALIZED_FILE_EXTENSION; }},
...
{
    if (isCodecFrontend) {
        // If compile with codec-frontend, the temporary bc file is used as the output file.
...
{
    // The output is in the same directory as the .codeo file
    std::string dir = opts.outputDir.value_or(outputDir);
    if (outputName.empty()) {
...
void TempFileManager::DeleteTempFiles(bool isSignalSafe)
{
    ...
    while (i >= 0) {
        ...
        std::optional<std::wstring> tempValue = Codira::StringConvertor::StringToWString(filePath);
        if (!tempValue.has_value()) {
            i--;
            continue;
        }
