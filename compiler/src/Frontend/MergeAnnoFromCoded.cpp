...
{
    CODEC_ASSERT(Ty::IsTyCorrect(rt->ty));
    switch (lt->astKind) {
...
// NOTE: l from .code.d, r from .codeo.
bool IsSameDeclByIdentifier(Ptr<Decl> l, Ptr<Decl> r)
{
...
void Codira::MergeCusAnno(Ptr<Package> target, Ptr<Package> source)
{
    // source to target, source from '.code.d', target from '.codeo'.
