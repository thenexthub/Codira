...
#ifndef CODIRA_LEX_LEXER_H
#define CODIRA_LEX_LEXER_H
...
/// Escape a Codira multiline interpolation string by prefixing each double quote with a backslash. Returns a valid
/// CODIRA source that can be lexed into a multiline interpolation string.
/// \param value string literal value of a codira multinline interpolation string.
std::string ProcessQuotaMarks(const std::string& value, bool processSingle = false);
}; // namespace Codira

#endif // CODIRA_LEX_LEXER_H
