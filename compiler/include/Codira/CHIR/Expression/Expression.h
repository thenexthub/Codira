...
#ifndef CODIRA_CHIR_EXPRESSION_H
#define CODIRA_CHIR_EXPRESSION_H
...
#include "Codira/CHIR/ExprKind.inc"
#undef EXPRKIND
#undef MAJORKIND
};
...
#undef MAJORKIND
    ...
    const char* exprKindNames[static_cast<size_t>(Codira::CHIR::ExprKind::MAX_EXPR_KINDS)] = {
        "INVALID",
...
private:
    explicit UnaryExpression(ExprKind kind, Value* operand, Codira::OverflowStrategy ofs, Block* parent)
    ...
    Codira::OverflowStrategy overflowStrategy;
};
...
private:
    ...
    Codira::OverflowStrategy overflowStrategy{Codira::OverflowStrategy::NA};
};

/**
 * @brief Constant expression in CHIR.
 * There are Bool, Rune, String, Int, Float, Unit literal in Codira
 *
 * Codira Code:
...
private:
    ...
    explicit TypeCast(Value* operand, Codira::OverflowStrategy overflow, Block* parent);
 ...
 * All intrinsic kinds are listd in codira/CHIR/IntrinsicKind.h
 */
...
/**
 * @brief A high-level description for `if` in Codira.
...
/**
 * @brief A high-level description for `while` and `do-while` in Codira.
...
/**
 * @brief A high-level description for `for-in` in Codira.
...
public:
    ...
    struct BGExecutionOrder {
        ...
        {
            CODEC_ASSERT(bgs.size() == BG_NUMBER);
...
/**
 * @brief An expression to describe lambda and local function in Codira.
 *
 * Codira Code:
 *      func foo() {
...
/**
 * @brief Generate debug information for codedb.
 */
...
public:
    ...
    // ===--------------------------------------------------------------------===//
    /**
     * @brief obtains the identifier of the corresponding Codira source code.
     */
    std::string GetSrcCodeIdentifier() const;
...
/**
 * @brief An expression to describe `spawn` in Codira.
 *
 * Codira Code:
 *      let x = spawn(arg) {
 *          return 1
...
#endif // CODIRA_CHIR_EXPRESSION_H
