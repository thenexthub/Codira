...
#ifndef CODIRA_CHIR_DIAG_WRAPPER_H
#define CODIRA_CHIR_DIAG_WRAPPER_H
...
public:
    ...
    template <typename... Args>
    DiagnosticBuilder DiagnoseRefactor(DiagKindRefactor kind, const Codira::Position& pos, Args&&... args)
    {
        auto key = static_cast<uint64_t>(pos.Hash64());
        auto it = posRange2MacroCallMap.lower_bound(key);
        CODEC_ASSERT(it == posRange2MacroCallMap.end() || !it->second);
        // means the range's begin is not from macro expanded node
