...
#ifndef CODIRA_CHIR_TRANSLATOR_H
#define CODIRA_CHIR_TRANSLATOR_H

#include "Codira/AST/NodeX.h"
#include "Codira/CHIR/AST2CHIR/AST2CHIRNodeMap.h"
#include "Codira/CHIR/AST2CHIR/TranslateASTNode/ExceptionTypeMapping.h"
...
class Translator {
public:
    Translator(CHIRBuilder& builder, CHIRType& chirTy, const Codira::GlobalOptions& opts,
        ...
        const Codira::TypeManager& typeManager)
        : builder(builder),
          chirTy(chirTy),
          ...
          gim(gim),
          increKind(kind),
          mergingPlatform(opts.IsCompilingCODEMP()),
...
#undef ASTKIND
    ...
    /**
     * @brief Translates a code location from Codira positions to a debug location.
     *
     ...
     */
    DebugLocation TranslateLocation(const Codira::Position& begin, const Codira::Position& end) const;
    ...
    template <typename T> DebugLocation GetOperatorLoc(const T& expr)
    {
        ...
        auto end = begin + Codira::Position(0, 0, static_cast<int>(opStr.size()));
    ...
    {
        return Codira::CHIR::CreateAndAppendExpression<TExpr>(builder, resultTy, args...);
    }
    ...
    {
        return Codira::CHIR::CreateAndAppendExpression<TExpr>(builder, loc, resultTy, args...);
    }
    ...
    {
        return Codira::CHIR::CreateAndAppendExpression<TExpr>(builder, locForWarning, loc, resultTy, args...);
    }
    ...
    {
        return Codira::CHIR::CreateAndAppendTerminator<TExpr>(builder, std::forward<Args>(args)...);
    }
    ...
    {
        return Codira::CHIR::CreateAndAppendTerminator<TExpr>(builder, loc, std::forward<Args>(args)...);
    }
...
private:
    ...
    // this map is used for set return value for forIn expr, gc-pool issue:Codira-manifest/issues/2442
    ...
    const bool mergingPlatform; // add by codemp
    const std::unordered_map<std::string, Value*>& deserializedVals; // add by codemp
...
private:
    ...
    {
        CODEC_ASSERT(!tryCatchContext.empty());
...
static const std::unordered_map<Codira::TokenKind, ExprKind> op2ExprKind = {
...
const static std::unordered_map<std::string, const std::unordered_map<std::string, IntrinsicKind>> packageMap = {
    ...
    {SYNC_PACKAGE_NAME, codenativeSyncIntrinsicMap},
