...
#ifndef CODIRA_CHIR_ANALYSIS_UTILS_H
#define CODIRA_CHIR_ANALYSIS_UTILS_H
...
{
    CODEC_ASSERT(lhs.size() == rhs.size());
    bool changed = false;
...
/**
 * @brief from codira position to range.
 * @param loc location to change.
 * @return codira range change to.
 */
template <typename T> Codira::Range ToRange(const T& loc)
{
    auto begin = loc.GetBeginPos();
    auto end = loc.GetEndPos();
    Codira::Position beginPos = Codira::Position(
        loc.GetFileID(), static_cast<int>(begin.line), static_cast<int>(begin.column));
    Codira::Position endPos = Codira::Position(
        loc.GetFileID(), static_cast<int>(end.line), static_cast<int>(end.column));
    return MakeRange(beginPos, endPos);
}

/**
 * @brief from codira position to range with non zero.
 * @param loc location to change.
 * @return codira range change to.
 */
template <typename T> std::pair<bool, Codira::Range> ToRangeIfNotZero(const T& loc)
{
    auto begin = loc.GetBeginPos();
    auto end = loc.GetEndPos();
    Codira::Position beginPos(loc.GetFileID(), static_cast<int>(begin.line), static_cast<int>(begin.column));
    Codira::Position endPos(loc.GetFileID(), static_cast<int>(end.line), static_cast<int>(end.column));
...
/**
 * @brief from CHIR location to codira position.
 * @param loc CHIR location input.
 * @return codira position change to.
 */
