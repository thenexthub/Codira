...
#ifndef CODIRA_CHIR_ANALYSIS_VALUE_DOMAIN_H
#define CODIRA_CHIR_ANALYSIS_VALUE_DOMAIN_H
...
public:
    ...
    {
        CODEC_ASSERT(kind != ValueKind::VAL);
    ...
    {
        CODEC_ASSERT(this->kind != ValueKind::REF);
    ...
    bool Join(const ValueDomain<AbstractValue>& rhs) override
    {
        ...
        if (this->kind == ValueKind::VAL && rhs.kind == ValueKind::VAL) {
            CODEC_ASSERT(this->absVal && rhs.absVal);
    ...
    {
        CODEC_ASSERT(kind == ValueKind::REF);
        return ref;
    }
