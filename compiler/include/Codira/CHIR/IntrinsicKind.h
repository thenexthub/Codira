...
#ifndef CODIRA_CHIR_INTRINSICKIND_H
#define CODIRA_CHIR_INTRINSICKIND_H
 ...
 * annotation in CODE so that parser will collect the corresponding Decl into a map.
...
static const std::string ARRAY_GET_REF_UNCHECKED_NAME = "arrayGetRefUnchecked"; // codenative only
...
static const std::string VECTOR_COMPARE_32_NAME = "vectorCompare32";      // codenative only
static const std::string VECTOR_INDEX_BYTE_32_NAME = "vectorIndexByte32"; // codenative only

// Foreign functions translated to intrinsic functions in the interpreter
// These functions are not not marked as intrinsic in Codira but they are translated to intrinsics in BCHIR

const std::string CODE_CORE_CAN_USE_SIMD_NAME = "CODE_CORE_CanUseSIMD"; // codenative only

static const std::string CODE_TLS_DYN_SET_SESSION_CALLBACK_NAME = "CODE_TLS_DYN_SetSessionCallback";
static const std::string CODE_TLS_DYN_SSL_INIT_NAME = "CODE_TLS_DYN_SslInit";
...
static const std::string SET_GC_THRESHOLD_NAME = "setCodeGCThreshold";
static const std::string GET_MAX_HEAP_SIZE_NAME = "getMaxCodeHeapSize";
static const std::string GET_ALLOCATE_HEAP_SIZE_NAME = "getAllocatedCodeHeapSize";
static const std::string DUMP_CODE_HEAP_DATA_NAME = "dumpCodeHeapData";
static const std::string GET_GC_COUNT_NAME = "getCodeGCCount";
static const std::string GET_GC_TIME_US_NAME = "getCodeGCTime";
static const std::string GET_GC_FREED_SIZE_NAME = "getCodeGCFreedSize";
static const std::string START_CODE_CPU_PROFILING_NAME = "startCodeCPUProfiling";
static const std::string STOP_CODE_CPU_PROFILING_NAME = "stopCodeCPUProfiling";
...
static const std::string RAW_ARRAY_REFEQ_NAME = "intrinsicRefEqRawArray"; // codenative only
static const std::string FUNC_REFEQ_NAME = "intrinsicRefEqFunc"; // codenative only
...
static const std::string FUTURE_IS_COMPLETE_NAME = "futureIsComplete"; // codenative only
static const std::string FUTURE_WAIT_NAME = "futureWait";              // codenative only
static const std::string FUTURE_NOTIFYALL_NAME = "futureNotifyAll";    // codenative only

// ============================ codenative only start ==============================
// REFLECTION
#define REFLECTION_KIND_TO_RUNTIME_FUNCTION(REFLECTION_KIND, CODE_FUNCTION, RUNTIME_FUNCTION)                            \
    static const std::string REFLECTION_KIND##_NAME = #CODE_FUNCTION;
...
We consider them intrinsic, similar to FFI_CODE_AST_*, so that we can link them statically
in the interpreter.
*/
...
static const std::string CODE_AST_LEX = "CODE_AST_Lex";
static const std::string CODE_ASTPARSEEXPR = "CODE_AST_ParseExpr";
static const std::string CODE_ASTPARSEDECL = "CODE_AST_ParseDecl";
static const std::string CODE_ASTPARSE_PROPMEMBERDECL = "CODE_AST_ParsePropMemberDecl";
static const std::string CODE_ASTPARSE_PRICONSTRUCTOR = "CODE_AST_ParsePrimaryConstructor";
static const std::string CODE_ASTPARSEPATTERN = "CODE_AST_ParsePattern";
static const std::string CODE_ASTPARSETYPE = "CODE_AST_ParseType";
static const std::string CODE_ASTPARSETOPLEVEL = "CODE_AST_ParseTopLevel";
static const std::string CODE_PARENT_CONTEXT = "CODE_CheckParentContext";
static const std::string CODE_MACRO_ITEM_INFO = "CODE_SetItemInfo";
static const std::string CODE_GET_CHILD_MESSAGES = "CODE_GetChildMessages";
static const std::string CODE_CHECK_ADD_SPACE = "CODE_CheckAddSpace";
static const std::string CODE_AST_DIAGREPORT = "CODE_AST_DiagReport";
...
enum IntrinsicKind : uint16_t {
    ...
    RAW_ARRAY_REFEQ, // codenative only
    FUNC_REFEQ, // codenative only
    ...
    GET_GC_TIME_US,
    GET_GC_FREED_SIZE,
    START_CODE_CPU_PROFILING,
    STOP_CODE_CPU_PROFILING,
    BLACK_BOX,
    GET_MAX_HEAP_SIZE,
    ...
    FUTURE_IS_COMPLETE, // codenative only
    FUTURE_WAIT,        // codenative only
    FUTURE_NOTIFYALL,   // codenative only
    ...
    VECTOR_COMPARE_32,    // codenative only
    VECTOR_INDEX_BYTE_32, // codenative only

    // Foreign functions translated to intrinsic functions in the interpreter,
    // These functions are not not marked as intrinsic in Codira but they are translated to intrinsics in BCHIR

    CODE_CORE_CAN_USE_SIMD, // codenative only

    CODE_TLS_DYN_SET_SESSION_CALLBACK,
    CODE_TLS_DYN_SSL_INIT,

    // ============================ codenative only start =================
    REFLECTION_INTRINSIC_START_FLAG,
#define REFLECTION_KIND_TO_RUNTIME_FUNCTION(REFLECTION_KIND, CODE_FUNCTION, RUNTIME_FUNCTION) REFLECTION_KIND,
#include "Codira/CHIR/LLVMReflectionIntrinsics.def"
#undef REFLECTION_KIND_TO_RUNTIME_FUNCTION
    ...
    // AST lib FFI
    FFI_CODE_AST_LEX,
    FFI_CODE_AST_PARSEEXPR,
    FFI_CODE_AST_PARSEDECL,
    FFI_CODE_AST_PARSE_PROPMEMBERDECL,
    FFI_CODE_AST_PARSE_PRICONSTRUCTOR,
    FFI_CODE_AST_PARSE_PATTERN,
    FFI_CODE_AST_PARSE_TYPE,
    FFI_CODE_AST_PARSETOPLEVEL,
    FFI_CODE_AST_DIAGREPORT,

    // Macro With Context FFI
    FFI_CODE_PARENT_CONTEXT,
    FFI_CODE_MACRO_ITEM_INFO,
    FFI_CODE_GET_CHILD_MESSAGES,
    FFI_CODE_CHECK_ADD_SPACE,
    // CodeGen
    CG_UNSAFE_BEGIN,
    CG_UNSAFE_END,
    ...
    // The interpreter is using these for codenative backend as well
    ATOMIC_INT8_LOAD,
    ATOMIC_INT8_STORE,
    ATOMIC_INT8_SWAP,
    ATOMIC_INT8_CAS,
...
static const std::unordered_map<std::string, IntrinsicKind> codenativeSyncIntrinsicMap = {
    {ATOMIC_LOAD_NAME, ATOMIC_LOAD},
    {ATOMIC_STORE_NAME, ATOMIC_STORE},
    ...
    {MUTEX_LOCK_NAME, CODE_MUTEX_LOCK},
...
static const std::unordered_map<std::string, IntrinsicKind> runtimeIntrinsicMap = {
    ...
    {DUMP_CODE_HEAP_DATA_NAME, DUMP_CODE_HEAP_DATA},
    {GET_GC_COUNT_NAME, GET_GC_COUNT},
    {GET_GC_TIME_US_NAME, GET_GC_TIME_US},
    {GET_GC_FREED_SIZE_NAME, GET_GC_FREED_SIZE},
    {START_CODE_CPU_PROFILING_NAME, START_CODE_CPU_PROFILING},
    {STOP_CODE_CPU_PROFILING_NAME, STOP_CODE_CPU_PROFILING},
...
#endif // Codira::CODIRA_CHIR_INTRINSICKIND_H
