...
#ifndef CODIRA_CHIR_ANNOTATION_H
#define CODIRA_CHIR_ANNOTATION_H
...
public:
    explicit LinkTypeInfo() : linkType(Codira::Linkage::EXTERNAL){};
    explicit LinkTypeInfo(const Codira::Linkage& linkType) : linkType(linkType){};

    static Codira::Linkage Extract(const LinkTypeInfo* li)
    {
        return li->linkType;
    }
    ...
    std::string ToString() override
    {
        std::map<Codira::Linkage, std::string> linkToString = {{Codira::Linkage::EXTERNAL, "external"},
            {Codira::Linkage::WEAK_ODR, "weak_odr"}, {Codira::Linkage::INTERNAL, "internal"},
            {Codira::Linkage::LINKONCE_ODR, "linkonce_odr"}};
        return "linkType: " + linkToString[linkType];
    }

private:
    Codira::Linkage linkType;
};
