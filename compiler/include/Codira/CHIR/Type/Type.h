...
#ifndef CODIRA_CHIR_TYPE_H
#define CODIRA_CHIR_TYPE_H
...
public:
    ...
    {
        CODEC_ASSERT(index + 1 < argTys.size());
        return argTys[index];
    }
     ...
     *
     * the Codira self's (without foreign) `VarLenParam` is just a desugar, which is desugared to
...
private:
    ...
    // Note, this identifier is different from the identifier in CODE source code,
