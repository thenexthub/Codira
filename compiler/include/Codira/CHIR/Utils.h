...
#ifndef CODIRA_CHIR_UTILS_H
#define CODIRA_CHIR_UTILS_H
...
template <typename T> struct DepMap {
    ...
    {
        CODEC_ASSERT(randomOrderValue.find(root) == randomOrderValue.end());
...
{
    if (Codira::FileUtil::IsDir(output)) {
        return Codira::FileUtil::JoinPath(output, fullPkgName) + suffix;
    } else {
        return Codira::FileUtil::GetFileBase(output) + suffix;
    }
}
 ...
 */
template <typename T> std::pair<bool, Codira::Range> GetDebugPos(const T& expr)
{
