...
#ifndef CODIRA_MODULES_IMPORTMANAGER_H
#define CODIRA_MODULES_IMPORTMANAGER_H
...
public:
    ...
    {
        codeoManager = MakeOwned<CodeoManager>(CodeoManager::Config{diag, typeManager, opts, importSrcCode});
        CODEC_NULLPTR_CHECK(codeoManager);
        dependencyGraph = MakeOwned<DependencyGraph>(*codeoManager);
        CODEC_NULLPTR_CHECK(dependencyGraph);
    }
    ...
    std::vector<std::string> GeneratePkgDepInfoByCodeo(const std::string& codeoPath);
     ...
     */
    bool BuildIndex(const std::string& codiraModules, const GlobalOptions& globalOptions,
        std::vector<Ptr<AST::Package>>& packages);

    /**
     * Load package from package name and codeo path.
     * Note: The function is used by lsp.
     */
    Ptr<AST::Package> LoadPackageFromCodeo(const std::string& fullPkgName, const std::string& codeoPath) const;
    ...
    std::unordered_set<std::string> GetUsedSTDLibFiles(DepType type)
    {
        ...
        for (auto& [codeoPath, typeWithFullPkgName] : stdDepsMap) {
            if (typeWithFullPkgName.first == type || typeWithFullPkgName.first == DepType::BOTH) {
                (void)result.emplace(codeoPath);
            }
        }
        return result;
    }
    ...
    /** Find codeo and load cached type for given source package, return codegen mangledName of remove decls. */
    ...
    /**
     * For LSP, clear all cached codeo data.
     */
    void ClearPackageCodeoCache() const;
    ...
    Ptr<CodeoManager> GetCodeoManager() const
    {
...
public:
    ...
    std::unordered_map<std::string, Codira::CHIR::Interpreter::Bchir> importedBchirs;

    const std::vector<std::string>& GetSearchPath() const
    {
        return codeoManager->GetSearchPath();
    }
    void DeleteASTWriters() noexcept;
    void DeleteASTLoaders() noexcept;

    std::unordered_map<std::string, std::string> GetDepPkgCodeoPaths() const
    {
        return codeoFilePaths;
    }

    std::unordered_map<std::string, std::string> GetDepPkgCodedPaths() const
    {
        return codedFilePaths;
    }
...
private:
    ...
    /** Store all standard package dependencies. Key is codeopath, value is pair of DepType and fullPackageName. */
...
private:
    ...
    void SaveDepPkgCodeoPath(const std::string& fullPackageName, const std::string& codeoPath);
    ...
    void HandleStdPackage(const std::string& fullPackageName, const std::string& codeoPath, bool isRecursive = false);
     ...
     */
    std::string GeneratePkgDepInfo(const AST::Package& pkg, bool exportCODEO = true) const;

    bool CheckCodeoPathLegality(const OwnedPtr<AST::ImportSpec>& import, const std::string& codeoPath,
    ...
    public:
        explicit DependencyGraph(CodeoManager& codeoManager) : codeoManager(codeoManager)
        {
        }
    ...
    OwnedPtr<CodeoManager> codeoManager;
    OwnedPtr<DependencyGraph> dependencyGraph;
    Ptr<AST::Package> curPackage;
    TypeManager& typeManager;

    // Key is fullPackageName, and value is .codeo file path.
    // Save the path of the imported external codeo and the path of binary dependencies' standard library codeo.
    std::unordered_map<std::string, std::string> codeoFilePaths;
    // Key is fullPackageName, and value is .code.d file path.
    // Save the path of the imported external code.d and the path of the imported standard library coded.
    std::unordered_map<std::string, std::string> codedFilePaths;
};
} // namespace Codira
#endif // CODIRA_MODULES_IMPORTMANAGER_H
