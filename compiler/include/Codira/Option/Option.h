...
#ifndef CODIRA_OPTION_OPTION_H
#define CODIRA_OPTION_OPTION_H
...
#include "Codira/Basic/DiagRefactor/DiagnosticWarnGroupKind.def"
#undef WARN_GROUP
    UNGROUPED,
    NONE,
};
...
public:
    struct Environment {
        ...
        std::optional<std::string> codiraHome = std::nullopt;
        std::vector<std::string> codiraPaths;
    ...
    std::vector<std::string> importPaths; /**< .codeo search paths */
#ifdef CODIRA_CODEGEN_CODENATIVE_BACKEND
    std::vector<std::string> pluginPaths; /**< meta-transform plugins */
#endif
    std::optional<std::string> commonPartCodeo = std::nullopt; /**< .codeo path for common part of package */
    ...
    std::string executablePath; /**< Absolute path of `codec` executable program. */
    ...
    std::string inputCodeoFile; /**< .codeo files to scan */
    ...
    // Built-in packages are official codira libraries bundled with codec compiler release,
    ...
    Triple::BackendType backend = Triple::BackendType::CODENATIVE;

    std::vector<std::string> macroLib; /* used in LSP and CODELINT */
    ...
    bool compileTestsOnly = false; /** Compile *_test.code files only */
    ...
    bool enableInteropCODEMapping = false; /**< Whether enable code data structure mapping for interop */
    ...
    OptimizationLevel optimizationLevel = OptimizationLevel::O0; /**< Backend optimization level in CODENative. */
    ...
    // The 'linkStaticStd' is 'true' when codec uses '--static-std' link option.
    // The 'linkStaticStd' is 'false' when codec uses '--dy-std' link option.
    // The 'linkStaticStd' is 'std::nullopt' when codec neither uses '--static-std' nor '--dy-std' link option.
    ...
    /// Whether to compile .code.d files. Note that when true, .code files are not processed.
    bool compileCoded{false};

    bool codedbMode = false; /** whether the option used in codedb */
    ...
    Options::Backend GetOptionsBackend()
    {
        switch (backend) {
            case Triple::BackendType::CODENATIVE:
                return Options::Backend::CODENATIVE;
            default:
                break;
        }
    ...
    /**
     * @brief Get Codira library path name of host.
     *
     * @return std::string Codira library path name is a name consists of os, arch and
     * backend, such as linux_aarch64_codenative.
     */
    std::string GetCodiraLibHostPathName() const;

    /**
     * @brief Get Codira library path of target.
     *
     * @return std::string Codira library path name.
     */
    std::string GetCodiraLibTargetPathName() const;
...
private:
    ...
    bool HandleCODEOExtension(DiagnosticEngine& diag, const std::string& value);
    bool HandleCODEExtension(DiagnosticEngine& diag, const std::string& value);
    bool HandleCHIRExtension(DiagnosticEngine& diag, const std::string& value);
    bool HandleCODEDExtension(DiagnosticEngine& diag, const std::string& value);
...
public:
    ...
    static void UpdateFlag(size_t index, bool newVal)
    {
        ...
        CODEC_ASSERT(index < flags.size());
        flags[index] = newVal;
    }

    bool IsSuppressed(size_t index)
    {
        CODEC_ASSERT(index < suppressedFlags.size());
