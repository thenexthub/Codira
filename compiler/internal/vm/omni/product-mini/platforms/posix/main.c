...
print_help(void)
{
    ...
    printf("  --toolchain-jit               Run the wasm app with toolchain jit mode\n");
#endif
...
#endif
#if WASM_ENABLE_JIT != 0
    printf("  --toolchain-jit-size-level=n  Set LLVM JIT size level, default is 3\n");
    printf("  --toolchain-jit-opt-level=n   Set LLVM JIT optimization level, default is 3\n");
    ...
    printf("  --enable-linux-perf      Enable linux perf support. It works in aot and toolchain-jit.\n");
#endif
...
main(int argc, char *argv[])
{
...
#endif
#if WASM_ENABLE_JIT != 0
        else if (!strcmp(argv[0], "--toolchain-jit")) {
    ...
    && WASM_ENABLE_LAZY_JIT != 0
...
#endif
#if WASM_ENABLE_JIT != 0
        else if (!strncmp(argv[0], "--toolchain-jit-size-level=", 22)) {
            if (argv[0][22] == '\0')
                return print_help();
        ...
        else if (!strncmp(argv[0], "--toolchain-jit-opt-level=", 21)) {
            if (argv[0][21] == '\0')
                return print_help();
