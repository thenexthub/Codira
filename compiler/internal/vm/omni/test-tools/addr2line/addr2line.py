...
- run the following command to convert the address into line info:
  ...
  ```
  The script will use *wasm-objdump* in wabt to transform address, then use *toolchain-dwarfdump* to lookup the line info for each address
...
- if addresses are not available in the stack trace (i.e. iwasm <= 1.3.2) or iwasm is used in fast interpreter mode,
  ...
  ```
  The script will use *wasm-objdump* in wabt to get the function names corresponding to function indexes, then use *toolchain-dwarfdump* to lookup the line info for each
...
def main():
    ...
    llvm_dwarf_dump = args.wasi_sdk.joinpath("bin/toolchain-dwarfdump")
    assert llvm_dwarf_dump.exists()

    llvm_cxxfilt = args.wasi_sdk.joinpath("bin/toolchain-cxxfilt")
