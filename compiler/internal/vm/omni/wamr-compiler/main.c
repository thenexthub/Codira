...
print_help()
{
    ...
    printf("  --xip                     A shorthand of --enable-indirect-mode --disable-toolchain-intrinsics\n");
    ...
    printf("  --disable-toolchain-intrinsics Disable the LLVM built-in intrinsics\n");
    printf("  --enable-builtin-intrinsics=<flags>\n");
    printf("                            Enable the specified built-in intrinsics, it will override the default\n");
    printf("                              settings. It only takes effect when --disable-toolchain-intrinsics is set.\n");
    ...
    printf("  --disable-toolchain-jump-tables Disable the LLVM jump tables similarly to clang's -fno-jump-tables\n");
    printf("  --disable-toolchain-lto        Disable the LLVM link time optimization\n");
    printf("  --enable-toolchain-pgo         Enable LLVM PGO (Profile-Guided Optimization)\n");
    printf("  --enable-toolchain-passes=<passes>\n");
    ...
    printf("  --toolchain-version            Show LLVM version information\n");
...
main(int argc, char *argv[])
{
    ...
    for (argc--, argv++; argc > 0 && argv[0][0] == '-'; argc--, argv++) {
        ...
        else if (!strcmp(argv[0], "--disable-toolchain-intrinsics")) {
            option.disable_llvm_intrinsics = true;
        }
        ...
        else if (!strcmp(argv[0], "--disable-toolchain-jump-tables")) {
            option.disable_llvm_jump_tables = true;
        }
        else if (!strcmp(argv[0], "--disable-toolchain-lto")) {
            option.disable_llvm_lto = true;
        }
        else if (!strcmp(argv[0], "--enable-toolchain-pgo")) {
            option.enable_llvm_pgo = true;
        }
        else if (!strncmp(argv[0], "--enable-toolchain-passes=", 21)) {
            if (argv[0][21] == '\0')
                PRINT_HELP_AND_EXIT();
        ...
        else if (!strcmp(argv[0], "--toolchain-version")) {
