...
def clone_llvm(dst_dir, llvm_repo, llvm_branch):
    ...
    """
    llvm_dir = dst_dir.joinpath("toolchain").resolve()

    if not llvm_dir.exists():
        GIT_CLONE_CMD = f"git clone --depth 1 --branch {llvm_branch} {llvm_repo} toolchain"
        print(GIT_CLONE_CMD)
...
def build_llvm(llvm_dir, platform, backends, projects, use_clang=False, extra_flags=''):
    ...
    CONFIG_CMD = f"cmake {compile_options} {extra_flags} ../toolchain"
    if "windows" == platform:
...
def main():
    ...
    parser.add_argument(
        "--toolchain-ver",
        action="store_true",
        help="return the version info of generated toolchain libraries",
    )
    parser.add_argument(
        "--use-clang",
        action="store_true",
    ...
    llvm_repo_and_branch = {
        "arc": {
            "repo": "https://github.com/toolchain/toolchain-project.git",
            "repo_ssh": "git@github.com:toolchain/toolchain-project.git",
            "branch": "release/18.x",
        },
        "xtensa": {
            "repo": "https://github.com/espressif/toolchain-project.git",
            "repo_ssh": "git@github.com:espressif/toolchain-project.git",
