...
Run `./test_pgo.sh` to test the benchmark with AOT static PGO (Profile-Guided Optimization) enabled, please refer [here](../README.md#install-toolchain-profdata) to install tool `toolchain-profdata` and build `iwasm` with `cmake -DWAMR_BUILD_STATIC_PGO=1`.
