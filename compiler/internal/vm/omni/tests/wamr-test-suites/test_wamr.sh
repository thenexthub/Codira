...
function do_execute_in_running_mode()
{
    ...
    if [[ ${ENABLE_GC} -eq 1 ]]; then
        if [[ "${RUNNING_MODE}" != "classic-interp" \
            ...
            echo "support gc in both interp modes, toolchain-jit mode and aot mode"
            return 0;
        fi
    fi
    ...
    if [[ ${ENABLE_SIMD} -eq 1 ]]; then
        if [[ "${RUNNING_MODE}" != "jit" && "${RUNNING_MODE}" != "aot" && "${RUNNING_MODE}" != "fast-interp" ]]; then
            echo "support simd in toolchain-jit, aot and fast-interp mode"
            return 0;
        fi
    fi
    ...
    if [[ ${TARGET} == "X86_32" ]]; then
        if [[ "${RUNNING_MODE}" == "jit" || "${RUNNING_MODE}" == "fast-jit" || "${RUNNING_MODE}" == "multi-tier-jit" ]]; then
            echo "both toolchain-jit, fast-jit and multi-tier-jit mode do not support X86_32 target"
            return 0;
        fi
    ...
    for t in "${TYPE[@]}"; do
        ...
        case $t in
            ...
            "jit")
                ...
                collect_coverage toolchain-jit
            ...
            "aot")
                ...
                collect_coverage toolchain-aot
            ;;
