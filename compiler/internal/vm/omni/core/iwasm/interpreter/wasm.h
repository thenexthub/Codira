...
typedef struct WASMFuncType {
    ...
    /* Code block to call toolchain jit functions of this
...
struct WASMFunction {
    ...
    /* The compiled toolchain jit func ptr of this function */
    void *llvm_jit_func_ptr;
    /* Code block to call fast jit jitted code of this function
       from the toolchain jit jitted code */
...
struct WASMModule {
     ...
     *   and when the toolchain jit func ptr of the same function is compiled, it
     *   will be set to call_to_llvm_jit_from_fast_jit of this function type
     *   (tier-up from fast-jit to toolchain-jit)
     */
     ...
     * for non Multi-Tier JIT mode:
     *   each pointer is set to the looked up toolchain jit func ptr, note that it
     *   is a stub and will trigger the actual compilation when it is called
     * for Multi-Tier JIT mode:
     *   each pointer is inited as call_to_fast_jit code block, when the toolchain
     *   jit func ptr is actually compiled, it is set to the compiled toolchain jit
     *   func ptr
     */
    void **func_ptrs;
    ...
    korp_tid orcodeit_threads[WASM_ORC_JIT_BACKEND_THREAD_NUM];
    /* backend thread arguments */
    OrcJitThreadArg orcodeit_thread_args[WASM_ORC_JIT_BACKEND_THREAD_NUM];
    /* whether to stop the compilation of backend threads */
    bool orcodeit_stop_compiling;
#endif

#if WASM_ENABLE_FAST_JIT != 0 && WASM_ENABLE_JIT != 0 \
    && WASM_ENABLE_LAZY_JIT != 0
    /* wait lock/cond for the synchronization of
       the toolchain jit initialization */
    korp_mutex tierup_wait_lock;
    korp_cond tierup_wait_cond;
    bool tierup_wait_lock_inited;
    korp_tid llvm_jit_init_thread;
    /* whether the toolchain jit is initialized */
    bool llvm_jit_inited;
    /* Whether to enable toolchain jit compilation:
       ...
       since no need to enable toolchain jit compilation for Mode_Interp and
