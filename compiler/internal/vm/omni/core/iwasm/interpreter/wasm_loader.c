                               ...
                               uint32 error_buf_size)
{
...
#if WASM_ENABLE_FAST_JIT != 0 && WASM_ENABLE_LAZY_JIT != 0
    if (module->orcodeit_stop_compiling)
        return false;
#endif

    bh_print_time("Begin to lookup toolchain jit functions");
    ...
    for (i = 0; i < module->function_count; i++) {
        ...
        if (error != LLVMErrorSuccess) {
            ...
            set_error_buf_v(error_buf, error_buf_size,
                            "failed to compile toolchain jit function: %s", err_msg);
    ...
    bh_print_time("End lookup toolchain jit functions");
...
init_llvm_jit_functions_stage2_callback(void *arg)
{
    ...
    if (!init_llvm_jit_functions_stage2(module, error_buf, error_buf_size)) {
        module->orcodeit_stop_compiling = true;
        return NULL;
    }
...
static void *
orcodeit_thread_callback(void *arg)
{
...
#if WASM_ENABLE_FAST_JIT != 0 && WASM_ENABLE_JIT != 0 \
    && WASM_ENABLE_LAZY_JIT != 0
    /* For JIT tier-up, set each toolchain jit func to call_to_fast_jit */
...
#if WASM_ENABLE_JIT != 0
    /* Compile toolchain jit functions of this group */
    for (i = group_idx; i < func_count;
         i += group_stride * WASM_ORC_JIT_COMPILE_THREAD_NUM) {
        LLVMOrcJITTargetAddress func_addr = 0;
        LLVMErrorRef error;
        ...
        snprintf(func_name, sizeof(func_name), "%s%d%s", AOT_FUNC_PREFIX, i,
                 "_wrapper");
        LOG_DEBUG("compile toolchain jit func %s", func_name);
        error =
            LLVMOrcLLLazyJITLookup(comp_ctx->orc_jit, &func_addr, func_name);
        if (error != LLVMErrorSuccess) {
            char *err_msg = LLVMGetErrorMessage(error);
            LOG_ERROR("failed to compile toolchain jit function %u: %s", i, err_msg);
            LLVMDisposeErrorMessage(err_msg);
            break;
        }
        ...
        for (j = 0; j < WASM_ORC_JIT_COMPILE_THREAD_NUM; j++) {
            if (i + j * group_stride < func_count) {
                ...
                if (error != LLVMErrorSuccess) {
                    char *err_msg = LLVMGetErrorMessage(error);
                    LOG_ERROR("failed to compile toolchain jit function %u: %s", i,
                              err_msg);
                    LLVMDisposeErrorMessage(err_msg);
                    /* Ignore current toolchain jit func, as its func ptr is
                ...
                /* Try to switch to call this toolchain jit function instead of
...
static void
orcodeit_stop_compile_threads(WASMModule *module)
{
#if WASM_ENABLE_LAZY_JIT != 0
    uint32 i, thread_num = (uint32)(sizeof(module->orcodeit_thread_args)
                                    / sizeof(OrcJitThreadArg));

    module->orcodeit_stop_compiling = true;
    for (i = 0; i < thread_num; i++) {
        if (module->orcodeit_threads[i])
            os_thread_join(module->orcodeit_threads[i], NULL);
    }
#endif
}
...
{
    uint32 thread_num =
        (uint32)(sizeof(module->orcodeit_thread_args) / sizeof(OrcJitThreadArg));
    uint32 i, j;
...
#if WASM_ENABLE_JIT != 0
        module->orcodeit_thread_args[i].comp_ctx = module->comp_ctx;
#endif
        module->orcodeit_thread_args[i].module = module;
        module->orcodeit_thread_args[i].group_idx = i;

        if (os_thread_create(&module->orcodeit_threads[i], orcodeit_thread_callback,
                             (void *)&module->orcodeit_thread_args[i],
                             APP_THREAD_STACK_SIZE_DEFAULT)
            != 0) {
            set_error_buf(error_buf, error_buf_size,
                          "create orcodeit compile thread failed");
            /* Terminate the threads created */
            module->orcodeit_stop_compiling = true;
            for (j = 0; j < i; j++) {
                os_thread_join(module->orcodeit_threads[j], NULL);
            }
            return false;
        }
    }
...
#if WASM_ENABLE_JIT != 0
    /* Ensure all the toolchain-jit functions are compiled */
    for (i = 0; i < module->function_count; i++) {
        if (!module->func_ptrs_compiled[i]) {
            set_error_buf(error_buf, error_buf_size,
                          "failed to compile toolchain jit function");
            return false;
        }
    }
#endif
                   ...
                   bool no_resolve, char *error_buf, uint32 error_buf_size)
{
    ...
    /* Run aot_compile_wasm in a backend thread, so as not to block the main
       thread fast jit execution, since applying toolchain optimizations in
       aot_compile_wasm may cost a lot of time.
       Create thread with enough native stack to apply toolchain optimizations */
...
wasm_loader_unload(WASMModule *module)
{
    ...
    orcodeit_stop_compile_threads(module);
#endif
