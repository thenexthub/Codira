                   ...
                   LLVMValueRef max_value, char *name, bool sign)
{
    ...
    if (!res) {
        aot_set_last_error("toolchain build fcmp failed.");
        goto fail;
    }
    ...
    if (!(res = LLVMBuildOr(comp_ctx->builder, is_less, is_greater,
                            "is_overflow"))) {
        aot_set_last_error("toolchain build logic and failed.");
        goto fail;
    }
    ...
    if (!res) {
        aot_set_last_error("toolchain build conversion failed.");
        return false;
    }
                                   ...
                                   AOTFuncContext *func_ctx)
{
    ...
    if (!(value =
        ...
        aot_set_last_error("toolchain build fp to si failed.");
        return false;
    }
    PUSH_I64(value);
    return true;
                                   ...
                                   AOTFuncContext *func_ctx)
{
    ...
    if (!(value =
        ...
        aot_set_last_error("toolchain build si to fp failed.");
        return false;
    }
    PUSH_F64(value);
    return true;
