...
aot_gen_commit_sp_ip(AOTCompFrame *frame, bool commit_sp, bool commit_ip)
{
    ...
    if (commit_sp && comp_ctx->call_stack_features.values) {
        ...
        if (!(int8_ptr_ptr_type = LLVMPointerType(INT8_PTR_TYPE, 0))) {
            aot_set_last_error("toolchain build pointer type failed");
            return false;
        }
                ...
                uint32 func_idx)
{
    ...
    /* Set all params dirty since they were set to toolchain value but
    ...
    /* Set all locals dirty since they were set to toolchain value but
       haven't been committed to the AOT/JIT stack frame */
...
aot_compile_wasm(AOTCompContext *comp_ctx)
{
    ...
    /* Run IR optimization before feeding in ORCODEIT and AOT codegen */
    if (comp_ctx->optimize) {
        /* Run passes for AOT/JIT mode.
           ...
           possible core dump. */
        bh_print_time("Begin to run toolchain optimization passes");
        aot_apply_llvm_new_pass_manager(comp_ctx, comp_ctx->module);
        bh_print_time("Finish toolchain optimization passes");
    }
...
aot_emit_llvm_file(AOTCompContext *comp_ctx, const char *file_name)
{
    ...
    if (LLVMPrintModuleToFile(comp_ctx->module, file_name, &err) != 0) {
        ...
        aot_set_last_error("emit toolchain ir to file failed.");
        return false;
    }
    ...
    if (comp_ctx->external_llc_compiler || comp_ctx->external_asm_compiler) {
        ...
        if (comp_ctx->external_llc_compiler) {
            ...
            if (LLVMWriteBitcodeToFile(comp_ctx->module, bc_file_name) != 0) {
                aot_set_last_error("emit toolchain bitcode file failed.");
                return false;
            }
