...
#define LLVM_BUILD_ICMP(op, left, right, res, name)                           \
    do {                                                                      \
        if (!(res =                                                           \
            ...
            aot_set_last_error("toolchain build " name " fail.");                  \
            return false;                                                     \
        }                                                                     \
    } while (0)
...
#define ADD_BASIC_BLOCK(block, name)                                           \
    do {                                                                       \
        if (!(block = LLVMAppendBasicBlockInContext(comp_ctx->context,         \
                                                    func_ctx->func, name))) {  \
            aot_set_last_error("toolchain add basic block failed.");                \
            goto fail;                                                         \
        }                                                                      \
                                                                               \
...
/* Call toolchain constrained floating-point intrinsic */
static LLVMValueRef
...
/* Call toolchain constrained libm-equivalent intrinsic */
static LLVMValueRef
                         ...
                         bool is_min)
{
    ...
    char *intrinsic = is_min ? (is_f32 ? "toolchain.minnum.f32" : "toolchain.minnum.f64")
                             : (is_f32 ? "toolchain.maxnum.f32" : "toolchain.maxnum.f64");
    ...
    if (comp_ctx->disable_llvm_intrinsics
        ...
        if (!(is_nan = LLVMBuildIntCast(comp_ctx->builder, is_nan, INT1_TYPE,
                                        "bit_cast_is_nan"))) {
            aot_set_last_error("toolchain build is_nan bit cast fail.");
            return NULL;
        }

        if (!(is_eq = LLVMBuildIntCast(comp_ctx->builder, is_eq, INT1_TYPE,
                                       "bit_cast_is_eq"))) {
            aot_set_last_error("toolchain build is_eq bit cast fail.");
            return NULL;
        }
    }
             ...
             || !(is_eq = LLVMBuildFCmp(comp_ctx->builder, LLVMRealOEQ, left,
                                        right, "is_eq"))) {
        aot_set_last_error("toolchain build fcmp fail.");
        return NULL;
    }
    ...
    if (!(left_int =
        ...
        || !(right_int = LLVMBuildBitCast(comp_ctx->builder, right, int_type,
                                          "right_int"))) {
        aot_set_last_error("toolchain build bitcast fail.");
        return NULL;
    }
    ...
    if (!(cmp = LLVMBuildSelect(comp_ctx->builder, is_eq, tmp, cmp, "cmp"))) {
        aot_set_last_error("toolchain build select fail.");
        return NULL;
    }
             ...
             bool is_i32)
{
    ...
    if (!LLVMBuildCondBr(comp_ctx->builder, overflow_cond, rems_end_block,
                         no_overflow_block)) {
        aot_set_last_error("toolchain build cond br failed.");
        return false;
    }
    ...
    if (!LLVMBuildBr(comp_ctx->builder, rems_end_block)) {
        aot_set_last_error("toolchain build br failed.");
        return false;
    }
    ...
    if (!(phi = LLVMBuildPhi(comp_ctx->builder, is_i32 ? I32_TYPE : I64_TYPE,
                             "rems_result_phi"))) {
        aot_set_last_error("toolchain build phi failed.");
        return false;
    }
            ...
            case -1:
                if (arith_op == INT_DIV_S) {
                    ...
                    if (!(res = LLVMBuildNeg(comp_ctx->builder, left, "neg"))) {
                        aot_set_last_error("toolchain build neg fail.");
                        return false;
                    }
                    PUSH_INT(res);
                    return true;
                ...
                bool is_rotl, bool is_i32)
{
    ...
    if (!bits_minus_shift_count) {
        aot_set_last_error("toolchain build and failed.");
        return NULL;
    }
...
is_targeting_soft_float(AOTCompContext *comp_ctx, bool is_f32)
{
    ...
    if (!(feature_string =
        ...
        aot_set_last_error("toolchain get target machine feature string fail.");
        return false;
    }
        ...
        case FLOAT_ADD:
            if (is_targeting_soft_float(comp_ctx, is_f32))
                DEF_FP_BINARY_OP(
                    ...
                    "toolchain build fadd fail.");
            else
                DEF_FP_BINARY_OP(
                    call_llvm_float_experimental_constrained_intrinsic(
                        comp_ctx, func_ctx, is_f32,
                        (is_f32 ? "toolchain.experimental.constrained.fadd.f32"
                                : "toolchain.experimental.constrained.fadd.f64"),
        ...
        case FLOAT_SUB:
            if (is_targeting_soft_float(comp_ctx, is_f32))
                DEF_FP_BINARY_OP(
                    ...
                    "toolchain build fsub fail.");
            else
                DEF_FP_BINARY_OP(
                    call_llvm_float_experimental_constrained_intrinsic(
                        comp_ctx, func_ctx, is_f32,
                        (is_f32 ? "toolchain.experimental.constrained.fsub.f32"
                                : "toolchain.experimental.constrained.fsub.f64"),
                        left, right, comp_ctx->fp_rounding_mode,
                        comp_ctx->fp_exception_behavior),
                    NULL);
            return true;
        case FLOAT_MUL:
            if (is_targeting_soft_float(comp_ctx, is_f32))
                DEF_FP_BINARY_OP(
                    ...
                    "toolchain build fmul fail.");
            else
                DEF_FP_BINARY_OP(
                    call_llvm_float_experimental_constrained_intrinsic(
                        comp_ctx, func_ctx, is_f32,
                        (is_f32 ? "toolchain.experimental.constrained.fmul.f32"
                                : "toolchain.experimental.constrained.fmul.f64"),
                        left, right, comp_ctx->fp_rounding_mode,
                        comp_ctx->fp_exception_behavior),
                    NULL);
            return true;
        case FLOAT_DIV:
            if (is_targeting_soft_float(comp_ctx, is_f32))
                DEF_FP_BINARY_OP(
                    ...
                    "toolchain build fdiv fail.");
            else
                DEF_FP_BINARY_OP(
                    call_llvm_float_experimental_constrained_intrinsic(
                        comp_ctx, func_ctx, is_f32,
                        (is_f32 ? "toolchain.experimental.constrained.fdiv.f32"
                                : "toolchain.experimental.constrained.fdiv.f64"),
                        left, right, comp_ctx->fp_rounding_mode,
                        comp_ctx->fp_exception_behavior),
                    NULL);
            return true;
    ...
    switch (math_op) {
        case FLOAT_ABS:
            DEF_FP_UNARY_OP(call_llvm_float_math_intrinsic(
                                comp_ctx, func_ctx, is_f32,
                                is_f32 ? "toolchain.fabs.f32" : "toolchain.fabs.f64",
                                operand),
                            NULL);
            return true;
        case FLOAT_NEG:
            DEF_FP_UNARY_OP(LLVMBuildFNeg(comp_ctx->builder, operand, "fneg"),
                            "toolchain build fneg fail.");
            return true;

        case FLOAT_CEIL:
            DEF_FP_UNARY_OP(call_llvm_float_math_intrinsic(
                                comp_ctx, func_ctx, is_f32,
                                is_f32 ? "toolchain.ceil.f32" : "toolchain.ceil.f64",
                                operand),
                            NULL);
            return true;
        case FLOAT_FLOOR:
            DEF_FP_UNARY_OP(call_llvm_float_math_intrinsic(
                                comp_ctx, func_ctx, is_f32,
                                is_f32 ? "toolchain.floor.f32" : "toolchain.floor.f64",
                                operand),
                            NULL);
            return true;
        case FLOAT_TRUNC:
            DEF_FP_UNARY_OP(call_llvm_float_math_intrinsic(
                                comp_ctx, func_ctx, is_f32,
                                is_f32 ? "toolchain.trunc.f32" : "toolchain.trunc.f64",
                                operand),
                            NULL);
            return true;
        case FLOAT_NEAREST:
            DEF_FP_UNARY_OP(call_llvm_float_math_intrinsic(
                                comp_ctx, func_ctx, is_f32,
                                is_f32 ? "toolchain.rint.f32" : "toolchain.rint.f64",
                                operand),
                            NULL);
            return true;
        case FLOAT_SQRT:
            if (is_targeting_soft_float(comp_ctx, is_f32)
                ...
                DEF_FP_UNARY_OP(call_llvm_float_math_intrinsic(
                                    comp_ctx, func_ctx, is_f32,
                                    is_f32 ? "toolchain.sqrt.f32" : "toolchain.sqrt.f64",
                                    operand),
                                NULL);
            else
                DEF_FP_UNARY_OP(
                    call_llvm_libm_experimental_constrained_intrinsic(
                        comp_ctx, func_ctx, is_f32,
                        (is_f32 ? "toolchain.experimental.constrained.sqrt.f32"
                                : "toolchain.experimental.constrained.sqrt.f64"),
                       ...
                       bool is_f32)
{
    ...
    DEF_FP_BINARY_OP(aot_call_llvm_intrinsic(
                         comp_ctx, func_ctx,
                         is_f32 ? "toolchain.copysign.f32" : "toolchain.copysign.f64",
