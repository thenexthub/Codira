...
aot_target_precheck_can_use_musttail(const AOTCompContext *comp_ctx)
{
    if (!strcmp(comp_ctx->target_arch, "xtensa")) {
         ...
         * https://github.com/espressif/toolchain-project/pull/73
         */
        return false;
    }
                            ...
                            LLVMTypeRef func_type, LLVMValueRef wrapped_func)
{
    ...
    if (comp_ctx->enable_stack_bound_check) {
        ...
        /* todo: @toolchain.expect.i1(i1 %cmp, i1 0) */
                  ...
                  LLVMTypeRef *p_func_type, LLVMValueRef *p_precheck_func)
{
    ...
    if (comp_ctx->is_jit_mode
        ...
        if (!LLVMBuildRetVoid(comp_ctx->builder)) {
            aot_set_last_error("toolchain build ret failed.");
            goto fail;
        }
    }
...
create_aux_stack_info(const AOTCompContext *comp_ctx, AOTFuncContext *func_ctx)
{
    ...
    if (!(func_ctx->aux_stack_bound =
        ...
        aot_set_last_error("toolchain build truncOrBitCast failed.");
        return false;
    }
                       ...
                       const AOTFunc *func)
{
    ...
    for (i = 0; i < aot_func_type->param_count; i++, j++) {
        ...
        if (!func_ctx->locals[i]) {
            aot_set_last_error("toolchain build alloca failed.");
            return false;
        }
                   ...
                   LLVMTypeRef int8_ptr_type, uint32 func_index)
{
    ...
    if (is_shared_memory) {
        ...
        if (!offset) {
            aot_set_last_error("create toolchain const failed.");
            return false;
        }
...
#define BUILD_IS_NOT_NULL(value, res, name)                                \
    do {                                                                   \
        if (!(res = LLVMBuildIsNotNull(comp_ctx->builder, value, name))) { \
            aot_set_last_error("toolchain build is not null failed.");          \
            goto fail;                                                     \
        }                                                                  \
    } while (0)
...
create_shared_heap_info(AOTCompContext *comp_ctx, AOTFuncContext *func_ctx)
{
    ...
    if (!(field_p_or_default = LLVMBuildSelect(comp_ctx->builder, cmp, field_p,
                                               ...
                                               "ptr_or_default"))) {
        aot_set_last_error("toolchain build select failed");
        goto fail;
    }
...
create_target_machine_detect_host(AOTCompContext *comp_ctx)
{
    ...
    if (LLVMGetTargetFromTriple(triple, &target, &err_msg) != 0) {
        aot_set_last_error_v("failed to get toolchain target from triple %s.",
                             err_msg);
    ...
    LOG_VERBOSE("LLVM ORCODEIT detected CPU \"%s\", with features \"%s\"\n", cpu,
                features);
...
orc_jit_create(AOTCompContext *comp_ctx)
{
    ...
    if (err != LLVMErrorSuccess) {
        aot_handle_llvm_errmsg("quited to create toolchain lazy orcodeit instance",
                               err);
        goto fail;
    }
...
aot_create_comp_context(const AOTCompData *comp_data, aot_comp_option_t option)
{
    ...
    if (option->is_jit_mode) {
        ...
        if (!abi) {
            ...
            if (!arch && !cpu && !features) {
                ...
                if (!(cpu = cpu_new = LLVMGetHostCPUName())) {
                    aot_set_last_error("toolchain get host cpu name failed.");
                    goto fail;
                }
            }
        }
        ...
        if (abi) {
            ...
            if (!arch1) {
                ...
                if (!default_triple) {
                    aot_set_last_error(
                        "toolchain get default target triple failed.");
                    goto fail;
                }
            ...
            if (!default_triple) {
                aot_set_last_error("toolchain get default target triple failed.");
                goto fail;
            }
        ...
        if (!triple && !cpu) {
            ...
            if (!(triple_norm = triple_norm_new =
                ...
                snprintf(buf, sizeof(buf),
                         "toolchain normlalize target triple (%s) failed.", triple);
        ...
        if (LLVMGetTargetFromTriple(triple_norm, &target, &err) != 0) {
            ...
            snprintf(buf, sizeof(buf),
                     "toolchain get target from triple (%s) failed", triple_norm);
            aot_set_last_error(buf);
            goto fail;
        }
    ...
    /* Create metadata for toolchain float experimental constrained intrinsics */
    if (!(comp_ctx->fp_rounding_mode = LLVMMDStringInContext(
        ...
        aot_set_last_error("create float toolchain metadata failed.");
        goto fail;
    }
                      ...
                      int param_count, LLVMValueRef *param_values)
{
    ...
    if (comp_ctx->disable_llvm_intrinsics
        ...
        /* Declare toolchain intrinsic function if necessary */
    ...
    if (!(ret = LLVMBuildCall2(comp_ctx->builder, func_type, func, param_values,
        ...
        aot_set_last_error("toolchain build intrinsic call failed.");
        return NULL;
    }
