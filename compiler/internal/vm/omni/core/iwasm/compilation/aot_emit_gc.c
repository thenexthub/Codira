...
#define BUILD_ISNOTNULL(ptr, res, name)                                  \
    do {                                                                 \
        if (!(res = LLVMBuildIsNotNull(comp_ctx->builder, ptr, name))) { \
            aot_set_last_error("toolchain build isnotnull failed.");          \
            goto fail;                                                   \
        }                                                                \
    } while (0)
    ...
    if (extend) {
        if (sign_extend) {
            if (!(field_value = LLVMBuildSExt(comp_ctx->builder, field_value,
                ...
                aot_set_last_error("toolchain build signed ext failed.");
                goto fail;
            }
        }
        else {
            if (!(field_value = LLVMBuildZExt(comp_ctx->builder, field_value,
                ...
                aot_set_last_error("toolchain build unsigned ext failed.");
                goto fail;
            }
        }
    }
                        ...
                        LLVMValueRef *p_elem_data, uint8 array_elem_type)
{
    ...
    if (!(elem_offset =
        ...
        aot_set_last_error("toolchain build shl failed.");
        goto fail;
    }
                     ...
                     LLVMValueRef *p_array_len)
{
    ...
    if (!(array_len = LLVMBuildLShr(comp_ctx->builder, array_len,
        ...
        aot_set_last_error("toolchain build lshr failed.");
        goto fail;
    }
                          ...
                          uint32 type_index)
{
    ...
    if (!(boundary = LLVMBuildAdd(comp_ctx->builder, offset, len, ""))) {
        aot_set_last_error("toolchain build failed.");
        goto fail;
    }
    ...
    if (!(loop_counter_addr = LLVMBuildAlloca(comp_ctx->builder, I32_TYPE,
                                              "fill_loop_counter"))) {
        aot_set_last_error("toolchain build alloc failed.");
        goto fail;
    }
                       ...
                       bool sign)
{
    ...
    if (!(i31_val = LLVMBuildPtrToInt(comp_ctx->builder, i31_obj, I32_TYPE,
                                      "i31_val"))) {
        aot_set_last_error("toolchain build ptr to init failed.");
        goto fail;
    }
    ...
    if (!sign) {
        ...
        if (!(i31_val = LLVMBuildAShr(comp_ctx->builder, i31_val, I32_ONE,
                                      "i31_value"))) {
            aot_set_last_error("toolchain build ashr failed.");
            goto fail;
        }
    }
                        ...
                        int32 heap_type, bool nullable)
{
    ...
    if (!(cmp = LLVMBuildICmp(comp_ctx->builder, LLVMIntEQ, castable, I8_ZERO,
                              "is_uncastable"))) {
        aot_set_last_error("toolchain build not failed");
        return false;
    }
