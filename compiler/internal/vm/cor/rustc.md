Omni Runtime in Rustc
==================

One goal for Omni Runtime is to be usable as a backend suitable for
...
this matches up fairly well with what we expect Omni Runtime's initial
strengths and weaknesses will be. Omni Runtime is being designed to take
...
Omni Runtime to capture the low-hanging fruit, and then use that along with
...
far Omni Runtime will go yet, but we do have some crazy ideas about what
such a thing might look like, including:
 ...
 - Build an optimizer IR without the constraints of fast-debug-build
   compilation. Omni Runtime's base IR is focused on Codegen, so a
