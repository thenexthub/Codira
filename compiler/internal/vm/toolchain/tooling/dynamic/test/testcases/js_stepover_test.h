...
#ifndef ECMASCRIPT_TOOLING_TEST_TESTCASES_JS_STEPOVER_TEST_H
#define ECMASCRIPT_TOOLING_TEST_TESTCASES_JS_STEPOVER_TEST_H

#include "tooling/dynamic/test/client_utils/test_util.h"

namespace panda::ecmascript::tooling::test {
class JsStepoverTest : public TestActions {
public:
    JsStepoverTest()
    {
        testAction = {
            ...
            {SocketAction::RECV, "", ActionRule::CUSTOM_RULE, MatchRule::replySuccess},
            {SocketAction::RECV, "Debugger.paused", ActionRule::CUSTOM_RULE,
                [this](auto recv, auto, auto) -> bool { return RecvStepoverInfo(recv, 22); }},

            {SocketAction::SEND, "sov"},
            {SocketAction::RECV, "Debugger.resumed", ActionRule::STRING_CONTAIN},
            {SocketAction::RECV, "", ActionRule::CUSTOM_RULE, MatchRule::replySuccess},
            {SocketAction::RECV, "Debugger.paused", ActionRule::CUSTOM_RULE,
                [this](auto recv, auto, auto) -> bool { return RecvStepoverInfo(recv, 23); }},
    ...
    bool RecvStepoverInfo(std::string recv, int lineNumber)
    {
        std::unique_ptr<PtJson> json = PtJson::Parse(recv);
        Result ret;
        std::string method = "";
...
std::unique_ptr<TestActions> GetJsStepoverTest()
{
    return std::make_unique<JsStepoverTest>();
}
}  // namespace panda::ecmascript::tooling::test

#endif // ECMASCRIPT_TOOLING_TEST_TESTCASES_JS_STEPOVER_TEST_H
