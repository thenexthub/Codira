...
#ifndef ECMASCRIPT_TOOLING_TEST_TESTCASES_JS_STEPOUT_TEST_H
#define ECMASCRIPT_TOOLING_TEST_TESTCASES_JS_STEPOUT_TEST_H

#include "tooling/dynamic/test/client_utils/test_util.h"

namespace panda::ecmascript::tooling::test {
class JsStepoutTest : public TestActions {
public:
    JsStepoutTest()
    {
        testAction = {
            ...
            {SocketAction::SEND, "b " DEBUGGER_JS_DIR "common_func.js 17"},
            {SocketAction::RECV, "", ActionRule::CUSTOM_RULE, MatchRule::replySuccess},
            ...
            {SocketAction::RECV, "", ActionRule::CUSTOM_RULE, MatchRule::replySuccess},
            {SocketAction::RECV, "Debugger.paused", ActionRule::CUSTOM_RULE,
                [this](auto recv, auto, auto) -> bool { return RecvStepoutInfo(recv); }},
    ...
    bool RecvBreakInfo(std::string recv)
    {
        ...
        if (ret != Result::SUCCESS || breakpoint.find(sourceFile_) == std::string::npos ||
            breakpoint.find("16") == std::string::npos) {
            return false;
        }
    ...
    bool RecvStepoutInfo(std::string recv)
    {
        std::unique_ptr<PtJson> json = PtJson::Parse(recv);
        Result ret;
        std::string method = "";
        ...
        int lineNum = 0;
        int lineNumExpectation = 23;
        ret = location->GetInt("lineNumber", &lineNum);
        if (ret != Result::SUCCESS || lineNum != lineNumExpectation) {
            return false;
        }
...
std::unique_ptr<TestActions> GetJsStepoutTest()
{
    return std::make_unique<JsStepoutTest>();
}
}  // namespace panda::ecmascript::tooling::test

#endif // ECMASCRIPT_TOOLING_TEST_TESTCASES_JS_STEPOUT_TEST_H
