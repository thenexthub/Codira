...
class AnimationImplTest : public testing::Test {
public:
...
HWTEST_F_L0(AnimationImplTest, DispatchTest_001)
{
    ...
    auto animationImpl = std::make_unique<AnimationImpl>();
    auto dispatcherImpl = std::make_unique<AnimationImpl::DispatcherImpl>(protocolChannel, std::move(animationImpl));
    std::string msg = std::string() + R"({"id":0,"method":"Animation.disable","params":{}})";
    DispatchRequest request(msg);
    dispatcherImpl->Dispatch(request);
    EXPECT_STREQ(outStrForCallbackCheck.c_str(), R"({"id":0,"result":{}})");
}

HWTEST_F_L0(AnimationImplTest, DispatchTest_002)
{
    std::string outStrForCallbackCheck = "";
    std::function<void(const void*, const std::string &)> callback =
        [&outStrForCallbackCheck]([[maybe_unused]] const void *ptr, const std::string &inStrOfReply) {
    ...
    auto animationImpl = std::make_unique<AnimationImpl>();
    auto dispatcherImpl = std::make_unique<AnimationImpl::DispatcherImpl>(protocolChannel, std::move(animationImpl));
    std::string msg = "AnimationImplTestCommand";
    DispatchRequest request(msg);
    dispatcherImpl->Dispatch(request);
