...
#include "baseptspausedgetreasonstring_fuzzer.h"
...
namespace OHOS {
    void BasePtsPausedGetReasonStringFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        Paused paused;
        paused.GetReasonString(PauseReason::EXCEPTION);
        cn->DestroyEcvm(vm);
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::BasePtsPausedGetReasonStringFuzzTest(data, size);
    return 0;
}