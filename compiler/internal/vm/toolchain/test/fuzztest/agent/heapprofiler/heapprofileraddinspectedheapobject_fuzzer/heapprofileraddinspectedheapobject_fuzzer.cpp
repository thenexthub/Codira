...
#include "heapprofileraddinspectedheapobject_fuzzer.h"
...
namespace OHOS {
    void HeapprofilerAddInspectedHeapObjectFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        dispatcherImpl->AddInspectedHeapObject(*req);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::HeapprofilerAddInspectedHeapObjectFuzzTest(data, size);
    return 0;
}