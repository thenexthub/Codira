...
#==================================================================
#文 件 名：                 TestArkTSHybridCorotines.py
#文件说明：                 多协程调试
#==================================================================
测试步骤：
    ...
    6.  1000个session并发
    7.  主线程resume
    8. 关闭主线程debugger server和connect server连接
#==================================================================
"""
import sys
...
class TestArkTSHybridCorotines(TestCase):
    def __init__(self, controllers):
        ...
        self.config = {
            'start_mode': '-D',
            'connect_server_port': 15656,
            'debugger_server_port': 15657,
            'bundle_name': 'com.example.Hybrid',
            'hap_name': 'InteropCorotines.hap',
            'hap_path': str(resource_path / 'hap' / 'InteropCorotines.hap'),
            'file_path': {
    ...
    async def test(self, websocket):
        ...
        self.common_utils.assert_equal(response['params']['reason'], 'Break on start')
        ################################################################################################################
        # main thread: 1.1 resume，执行完所有的session
        ################################################################################################################
        await self.debugger_impl.send("Debugger.resume", main_thread, None)
        time.sleep(20)
        ################################################################################################################
