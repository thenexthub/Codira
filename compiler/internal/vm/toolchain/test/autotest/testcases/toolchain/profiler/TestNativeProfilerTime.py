...
#==================================================================
#文 件 名：                 TestNativeProfilerTime.py
#文件说明：                 测试 Time 录制数据中 native 函数名是否正常
#==================================================================
测试步骤：
    1.  连接 connect server 和主线程 debugger server
    2.  主线程使能 Runtime（Runtime.enable）
    3.  主线程设置采样间隔（Profiler.setSamplingInterval）
    4.  主线程启动 CPU 调试（Profiler.start）
    5.  主线程去使能 Debugger（Debugger.disable）
    6.  连续点击屏幕 5 次，每次间隔 1 秒
    7.  关闭 CPU 调优，获取调优数据（Profiler.stop）
...
class TestNativeProfilerTime(TestCase):
    ...
    async def test(self, websocket):
        ...
        ################################################################################################################
        # main thread: Five clicks on the screen
        ################################################################################################################
        for i in range(5):
            time.sleep(1)
