...
#include "vm/core/SandboxIR/Tracker.h"
...
std::string IRSnapshotChecker::dumpIR(const toolchain::Function &F) const {
  std::string Result;
...
EraseFromParent::EraseFromParent(std::unique_ptr<sandboxir::Value> &&ErasedIPtr)
  ...
  for (auto *LLVMI : reverse(LLVMInstrs)) {
    SmallVector<toolchain::Value *> Operands;
  ...
  auto *BotLLVMI = cast<toolchain::Instruction>(I->Val);
...
void EraseFromParent::revert(Tracker &Tracker) {
  ...
  if (auto *NextLLVMI = dyn_cast<toolchain::Instruction *>(NextLLVMIOrBB)) {
    BotLLVMI->insertBefore(NextLLVMI->getIterator());
  } else {
    auto *LLVMBB = cast<toolchain::BasicBlock *>(NextLLVMIOrBB);
...
void CatchSwitchAddHandler::revert(Tracker &Tracker) {
  ...
  auto *LLVMCSI = cast<toolchain::CatchSwitchInst>(CSI->Val);
