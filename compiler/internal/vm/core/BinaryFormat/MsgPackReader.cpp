...
#include "vm/core/BinaryFormat/MsgPackReader.h"
#include "vm/core/BinaryFormat/MsgPack.h"
...
Expected<bool> Reader::read(Object &Obj) {
  ...
  case FirstByte::Float32:
    ...
    Obj.Float =
        toolchain::bit_cast<float>(endian::read<uint32_t, Endianness>(Current));
  ...
  case FirstByte::Float64:
    ...
    if (sizeof(double) > remainingSpace())
      return make_error<StringError>(
          ...
          std::make_error_code(std::errc::invalid_argument));
    Obj.Float =
        toolchain::bit_cast<double>(endian::read<uint64_t, Endianness>(Current));
