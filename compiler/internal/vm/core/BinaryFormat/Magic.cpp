//===- toolchain/BinaryFormat/Magic.cpp - File magic identification --*- C++ -*-===//
//
...
file_magic toolchain::identify_magic(StringRef Magic) {
  if (Magic.size() < 4)
...
std::error_code toolchain::identify_magic(const Twine &Path, file_magic &Result) {
