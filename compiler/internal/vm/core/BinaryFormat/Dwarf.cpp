//===-- toolchain/BinaryFormat/Dwarf.cpp - Dwarf Framework ------------*- C++-*-===//
//
...
StringRef toolchain::dwarf::TagString(unsigned Tag) {
  switch (Tag) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getTag(StringRef TagString) {
...
unsigned toolchain::dwarf::TagVersion(dwarf::Tag Tag) {
  switch (Tag) {
  default:
    return 0;
#define HANDLE_DW_TAG(ID, NAME, VERSION, VENDOR, KIND)                         \
...
unsigned toolchain::dwarf::TagVendor(dwarf::Tag Tag) {
  switch (Tag) {
  default:
    return 0;
#define HANDLE_DW_TAG(ID, NAME, VERSION, VENDOR, KIND)                         \
...
StringRef toolchain::dwarf::ChildrenString(unsigned Children) {
  switch (Children) {
  case DW_CHILDREN_no:
...
StringRef toolchain::dwarf::AttributeString(unsigned Attribute) {
  switch (Attribute) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::AttributeVersion(dwarf::Attribute Attribute) {
  switch (Attribute) {
  default:
    return 0;
#define HANDLE_DW_AT(ID, NAME, VERSION, VENDOR)                                \
...
unsigned toolchain::dwarf::AttributeVendor(dwarf::Attribute Attribute) {
  switch (Attribute) {
  default:
    return 0;
#define HANDLE_DW_AT(ID, NAME, VERSION, VENDOR)                                \
...
StringRef toolchain::dwarf::FormEncodingString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::FormVersion(dwarf::Form Form) {
  switch (Form) {
  default:
    return 0;
#define HANDLE_DW_FORM(ID, NAME, VERSION, VENDOR)                              \
...
unsigned toolchain::dwarf::FormVendor(dwarf::Form Form) {
  switch (Form) {
  default:
    return 0;
#define HANDLE_DW_FORM(ID, NAME, VERSION, VENDOR)                              \
...
StringRef toolchain::dwarf::OperationEncodingString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getOperationEncoding(StringRef OperationEncodingString) {
...
StringRef toolchain::dwarf::SubOperationEncodingString(unsigned OpEncoding,
...
unsigned
toolchain::dwarf::getSubOperationEncoding(unsigned OpEncoding,
                                     StringRef SubOperationEncodingString) {
  assert(OpEncoding == DW_OP_LLVM_user);
  return getLlvmUserOperationEncoding(SubOperationEncodingString);
}

unsigned toolchain::dwarf::OperationVersion(dwarf::LocationAtom Op) {
  switch (Op) {
  default:
    return 0;
#define HANDLE_DW_OP(ID, NAME, OPERANDS, ARITY, VERSION, VENDOR)               \
...
std::optional<unsigned> toolchain::dwarf::OperationOperands(dwarf::LocationAtom Op) {
  switch (Op) {
  default:
    return std::nullopt;
#define HANDLE_DW_OP(ID, NAME, OPERANDS, ARITY, VERSION, VENDOR)               \
...
std::optional<unsigned> toolchain::dwarf::OperationArity(dwarf::LocationAtom Op) {
  switch (Op) {
  default:
    return std::nullopt;
#define HANDLE_DW_OP(ID, NAME, OPERANDS, ARITY, VERSION, VENDOR)               \
...
unsigned toolchain::dwarf::OperationVendor(dwarf::LocationAtom Op) {
  switch (Op) {
  default:
    return 0;
#define HANDLE_DW_OP(ID, NAME, OPERANDS, ARITY, VERSION, VENDOR)               \
...
StringRef toolchain::dwarf::AttributeEncodingString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getAttributeEncoding(StringRef EncodingString) {
...
unsigned toolchain::dwarf::AttributeEncodingVersion(dwarf::TypeKind ATE) {
  switch (ATE) {
  default:
    return 0;
#define HANDLE_DW_ATE(ID, NAME, VERSION, VENDOR)                               \
...
unsigned toolchain::dwarf::AttributeEncodingVendor(dwarf::TypeKind ATE) {
  switch (ATE) {
  default:
    return 0;
#define HANDLE_DW_ATE(ID, NAME, VERSION, VENDOR)                               \
...
StringRef toolchain::dwarf::DecimalSignString(unsigned Sign) {
  switch (Sign) {
  case DW_DS_unsigned:
...
StringRef toolchain::dwarf::EndianityString(unsigned Endian) {
  switch (Endian) {
  case DW_END_default:
...
StringRef toolchain::dwarf::AccessibilityString(unsigned Access) {
  switch (Access) {
  // Accessibility codes
...
StringRef toolchain::dwarf::DefaultedMemberString(unsigned DefaultedEncodings) {
...
StringRef toolchain::dwarf::VisibilityString(unsigned Visibility) {
  switch (Visibility) {
  case DW_VIS_local:
    return "DW_VIS_local";
...
StringRef toolchain::dwarf::VirtualityString(unsigned Virtuality) {
  switch (Virtuality) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getVirtuality(StringRef VirtualityString) {
...
StringRef toolchain::dwarf::EnumKindString(unsigned EnumKind) {
  switch (EnumKind) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getEnumKind(StringRef EnumKindString) {
...
StringRef toolchain::dwarf::LanguageString(unsigned Language) {
  switch (Language) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getLanguage(StringRef LanguageString) {
...
unsigned toolchain::dwarf::LanguageVersion(dwarf::SourceLanguage Lang) {
  switch (Lang) {
  default:
    return 0;
#define HANDLE_DW_LANG(ID, NAME, LOWER_BOUND, VERSION, VENDOR)                 \
...
unsigned toolchain::dwarf::LanguageVendor(dwarf::SourceLanguage Lang) {
  switch (Lang) {
  default:
    return 0;
#define HANDLE_DW_LANG(ID, NAME, LOWER_BOUND, VERSION, VENDOR)                 \
...
toolchain::dwarf::LanguageLowerBound(dwarf::SourceLanguage Lang) {
  switch (Lang) {
  default:
    return std::nullopt;
#define HANDLE_DW_LANG(ID, NAME, LOWER_BOUND, VERSION, VENDOR)                 \
...
StringRef toolchain::dwarf::LanguageDescription(dwarf::SourceLanguageName lname) {
  switch (lname) {
...
StringRef toolchain::dwarf::LanguageDescription(dwarf::SourceLanguageName Name,
                                           uint32_t Version) {
  switch (Name) {
  // YYYY
  case DW_LNAME_Ada: {
...
toolchain::StringRef toolchain::dwarf::SourceLanguageNameString(SourceLanguageName Lang) {
  switch (Lang) {
#define HANDLE_DW_LNAME(ID, NAME, DESC, LOWER_BOUND)                           \
  case DW_LNAME_##NAME:                                                        \
...
unsigned
toolchain::dwarf::getSourceLanguageName(StringRef SourceLanguageNameString) {
...
StringRef toolchain::dwarf::CaseString(unsigned Case) {
  switch (Case) {
...
StringRef toolchain::dwarf::ConventionString(unsigned CC) {
  switch (CC) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getCallingConvention(StringRef CCString) {
...
StringRef toolchain::dwarf::InlineCodeString(unsigned Code) {
  switch (Code) {
...
StringRef toolchain::dwarf::ArrayOrderString(unsigned Order) {
  switch (Order) {
...
StringRef toolchain::dwarf::LNStandardString(unsigned Standard) {
  switch (Standard) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::LNExtendedString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::MacinfoString(unsigned Encoding) {
  switch (Encoding) {
...
unsigned toolchain::dwarf::getMacinfo(StringRef MacinfoString) {
...
StringRef toolchain::dwarf::MacroString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::GnuMacroString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
unsigned toolchain::dwarf::getMacro(StringRef MacroString) {
  return StringSwitch<unsigned>(MacroString)
#define HANDLE_DW_MACRO(ID, NAME) .Case("DW_MACRO_" #NAME, ID)
#include "vm/core/BinaryFormat/Dwarf.def"
      .Default(DW_MACINFO_invalid);
}
StringRef toolchain::dwarf::RangeListEncodingString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::LocListEncodingString(unsigned Encoding) {
  switch (Encoding) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::CallFrameString(unsigned Encoding,
    Triple::ArchType Arch) {
  assert(Arch != toolchain::Triple::ArchType::UnknownArch);
#define SELECT_AARCH64 (Arch == toolchain::Triple::aarch64_be || Arch == toolchain::Triple::aarch64)
#define SELECT_MIPS64 Arch == toolchain::Triple::mips64
#define SELECT_SPARC (Arch == toolchain::Triple::sparc || Arch == toolchain::Triple::sparcv9)
#define SELECT_X86 (Arch == toolchain::Triple::x86 || Arch == toolchain::Triple::x86_64)
...
StringRef toolchain::dwarf::ApplePropertyString(unsigned Prop) {
  switch (Prop) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::UnitTypeString(unsigned UT) {
  switch (UT) {
  default:
    return StringRef();
...
StringRef toolchain::dwarf::AtomTypeString(unsigned AT) {
  switch (AT) {
...
StringRef toolchain::dwarf::GDBIndexEntryKindString(GDBIndexEntryKind Kind) {
  switch (Kind) {
  case GIEK_NONE:
    return "NONE";
  case GIEK_TYPE:
...
StringRef
toolchain::dwarf::GDBIndexEntryLinkageString(GDBIndexEntryLinkage Linkage) {
  switch (Linkage) {
  case GIEL_EXTERNAL:
    return "EXTERNAL";
  case GIEL_STATIC:
...
StringRef toolchain::dwarf::AttributeValueString(uint16_t Attr, unsigned Val) {
  switch (Attr) {
...
StringRef toolchain::dwarf::AtomValueString(uint16_t Atom, unsigned Val) {
  switch (Atom) {
  case DW_ATOM_null:
    return "NULL";
  case DW_ATOM_die_tag:
...
StringRef toolchain::dwarf::IndexString(unsigned Idx) {
  switch (Idx) {
  default:
    return StringRef();
...
std::optional<uint8_t> toolchain::dwarf::getFixedFormByteSize(dwarf::Form Form,
                                                         FormParams Params) {
  switch (Form) {
  case DW_FORM_addr:
    if (Params)
...
bool toolchain::dwarf::isValidFormForVersion(Form F, unsigned Version,
                                        bool ExtensionsOk) {
...
StringRef toolchain::dwarf::FormatString(DwarfFormat Format) {
  switch (Format) {
  case DWARF32:
    return "DWARF32";
  case DWARF64:
...
StringRef toolchain::dwarf::FormatString(bool IsDWARF64) {
  return FormatString(IsDWARF64 ? DWARF64 : DWARF32);
}

StringRef toolchain::dwarf::RLEString(unsigned RLE) {
  switch (RLE) {
  default:
    return StringRef();
#define HANDLE_DW_RLE(ID, NAME)                                                \
...
StringRef toolchain::dwarf::AddressSpaceString(unsigned AS, const toolchain::Triple &TT) {
  switch (AS) {
...
StringRef (*const toolchain::dwarf::EnumTraits<Tag>::StringFn)(unsigned) = TagString;
StringRef (*const toolchain::dwarf::EnumTraits<Attribute>::StringFn)(unsigned) =
    AttributeString;
StringRef (*const toolchain::dwarf::EnumTraits<Form>::StringFn)(unsigned) =
    FormEncodingString;
StringRef (*const toolchain::dwarf::EnumTraits<LocationAtom>::StringFn)(unsigned) =
    OperationEncodingString;
StringRef (*const toolchain::dwarf::EnumTraits<LineNumberOps>::StringFn)(unsigned) =
    LNStandardString;
StringRef (*const toolchain::dwarf::EnumTraits<Index>::StringFn)(unsigned) =
    IndexString;
