...
#include "vm/core/Demangle/MicrosoftDemangle.h"

#include "vm/core/Demangle/Demangle.h"
#include "vm/core/Demangle/DemangleConfig.h"
#include "vm/core/Demangle/MicrosoftDemangleNodes.h"
#include "vm/core/Demangle/StringViewExtras.h"
#include "vm/core/Demangle/Utility.h"
...
static bool consumeFront(std::string_view &S, char C) {
  if (!toolchain::itanium_demangle::starts_with(S, C))
    return false;
  S.remove_prefix(1);
  return true;
}
...
static bool startsWith(std::string_view S, std::string_view PrefixA,
                       std::string_view PrefixB, bool A) {
  const std::string_view &Prefix = A ? PrefixA : PrefixB;
  return toolchain::itanium_demangle::starts_with(S, Prefix);
}
...
static bool isPointerType(std::string_view S) {
  if (toolchain::itanium_demangle::starts_with(S, "$$Q")) // foo &&
    return true;
...
static bool isFunctionType(std::string_view S) {
  return toolchain::itanium_demangle::starts_with(S, "$$A8@@") ||
         toolchain::itanium_demangle::starts_with(S, "$$A6");
}
...
Demangler::demangleFunctionIdentifierCode(std::string_view &MangledName) {
  assert(toolchain::itanium_demangle::starts_with(MangledName, '?'));
  MangledName.remove_prefix(1);
  if (MangledName.empty()) {
    Error = true;
    return nullptr;
...
SymbolNode *Demangler::parse(std::string_view &MangledName) {
  ...
  if (toolchain::itanium_demangle::starts_with(MangledName, '.'))
...
Demangler::demangleAnonymousNamespaceName(std::string_view &MangledName) {
  assert(toolchain::itanium_demangle::starts_with(MangledName, "?A"));
  consumeFront(MangledName, "?A");
...
Demangler::demangleNameScopePiece(std::string_view &MangledName) {
  ...
  if (toolchain::itanium_demangle::starts_with(MangledName, "?A"))
...
TypeNode *Demangler::demangleType(std::string_view &MangledName,
  ...
  if (isTagType(MangledName))
    ...
    if (consumeFront(MangledName, "$$A8@@"))
      ...
      assert(toolchain::itanium_demangle::starts_with(MangledName, "$$A6"));
...
Demangler::demangleFunctionParameterList(std::string_view &MangledName,
  ...
  size_t Count = 0;
  while (!Error && !toolchain::itanium_demangle::starts_with(MangledName, '@') &&
         !toolchain::itanium_demangle::starts_with(MangledName, 'Z')) {
    ++Count;
...
Demangler::demangleTemplateParameterList(std::string_view &MangledName) {
  ...
  while (!toolchain::itanium_demangle::starts_with(MangledName, '@')) {
    ...
    *Current = Arena.alloc<NodeList>();
    ...
    if (consumeFront(MangledName, "$$Y")) {
      ...
      TPRN->Symbol = S;
    } else if (toolchain::itanium_demangle::starts_with(MangledName, "$E?")) {
  ...
  assert(toolchain::itanium_demangle::starts_with(
...
std::optional<size_t>
toolchain::getArm64ECInsertionPointInMangledName(std::string_view MangledName) {
...
char *toolchain::microsoftDemangle(std::string_view MangledName, size_t *NMangled,
