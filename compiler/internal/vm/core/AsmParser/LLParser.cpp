...
#include "vm/core/AsmParser/LLToken.h"
...
bool LLParser::validateEndOfModule(bool UpgradeDebugInfo) {
  ...
  for (const auto &[Name, Info] : make_early_inc_range(ForwardRefVals)) {
    if (StringRef(Name).starts_with("toolchain.")) {
...
bool LLParser::parseAllocKind(AllocFnKind &Kind) {
  ...
  for (StringRef A : toolchain::split(Arg, ",")) {
    if (A == "alloc") {
...
std::optional<MemoryEffects> LLParser::parseMemoryAttr() {
  ...
  Lex.setIgnoreColonInIdentifiers(true);
  toolchain::scope_exit _([&] { Lex.setIgnoreColonInIdentifiers(false); });
...
bool isOldDbgFormatIntrinsic(StringRef Name) {
  ...
  // We can make this the only check when we begin supporting all "toolchain.dbg"
  // intrinsics in the new debug info format.
  if (!Name.starts_with("toolchain.dbg."))
    return false;
...
bool LLParser::parseCall(Instruction *&Inst, PerFunctionState &PFS,
  ...
  if (CalleeID.Kind == ValID::t_GlobalName &&
    ...
    if (SeenNewDbgInfoFormat) {
      CI->deleteValue();
      return error(CallLoc, "toolchain.dbg intrinsic should not appear in a module "
...
bool LLParser::parseOptionalRefs(SmallVectorImpl<ValueInfo> &Refs) {
  ...
  toolchain::sort(VContexts, [](const ValueContext &VC1, const ValueContext &VC2) {
