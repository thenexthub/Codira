...
#include "vm/core/WindowsManifest/WindowsManifestMerger.h"
...
static void checkAndStripPrefixes(xmlNodePtr Node,
  ...
  if (Node->ns && Node->ns->prefix != nullptr) {
    ...
    } else if (!toolchain::is_contained(RequiredPrefixes, Node->ns)) {
  ...
  xmlNs Temp;
  for (xmlNsPtr Def = Node->nsDef; Def; Def = Def->next) {
    if (!Def->prefix || toolchain::is_contained(RequiredPrefixes, Def)) {
      Prev = Def;
      continue;
    }
    if (Def == Node->nsDef) {
