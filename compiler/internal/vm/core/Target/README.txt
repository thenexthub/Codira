...
Get the C front-end to expand hypot(x,y) -> toolchain.sqrt(x*x+y*y) when errno and
...
into toolchain.powi calls, allowing the code generator to produce balanced
...
http://lists.toolchain.org/pipermail/toolchain-commits/Week-of-Mon-20060213/031986.html
...
This should be recognized as CLZ:  https://github.com/toolchain/toolchain-project/issues/64167
...
; RUN: toolchain-as < %s | opt -tailcallelim | toolchain-dis | not grep call
...
; RUN: toolchain-as < %s | opt -argpromotion | toolchain-dis | grep x.val
...
Clang compiles this into:
  ...
  call void @toolchain.memset.p0i8.i64(i8* %tmp, i8 0, i64 64, i32 16, i1 false)
...
$ toolchain-g++ perf.cpp -O3 -fno-exceptions
$ time ./a.out fast
...
"clang -emit-toolchain-bc | opt -O3".
...
optimized with "clang -emit-toolchain-bc | opt -O3".
...
currently not optimized with "clang -emit-toolchain-bc | opt
-O3".
...
-emit-toolchain-bc | opt -O3" (although llc can optimize it).

//===---------------------------------------------------------------------===//

int a(unsigned b) {return ((b << 31) | (b << 30)) >> 31;}
Should be combined to  "((b >> 1) | b) & 1".  Currently not optimized
with "clang -emit-toolchain-bc | opt -O3".
...
Should fold to "a".  Currently not optimized with "clang -emit-toolchain-bc
| opt -O3".
...
not optimized with "clang -emit-toolchain-bc | opt -O3".
...
With: toolchain-gcc t2.c -S -o - -O0 -emit-toolchain | toolchain-as | 
             opt -mem2reg -gvn -instcombine | toolchain-dis
...
252.eon contains this interesting code:
        ...
        call void @toolchain.memcpy.i32(i8* %endptr, 
...
call void @toolchain.memcpy.i32(
...
This can be seen at:
$ clang t.c -S -o - -mkernel -O0 -emit-toolchain | opt -function-attrs -S
...
We should use DSE + toolchain.lifetime.end to delete dead vtable pointer updates.
See GCC PR34949
...
...
  %67 = call i64 @toolchain.objectsize.i64(i8* %64, i1 false) nounwind
  %68 = call i8* @__memset_chk(i8* %64, i32 0, i64 %62, i64 %67) nounwind

toolchain.objectsize.i64 should be taught about malloc/calloc, allowing it to
...
Note that this would change the semantics of @toolchain.objectsize which by its
...
define i8* @test10(i32 %x) {
  ...
  call void @toolchain.memset.p0i8.i32(i8* %alloc, i8 0, i32 %x, i32 1, i1 false)
  ret i8* %alloc
}
...
_ZNSt12_Vector_baseIiSaIiEEC2EmRKS0_.exit.i.i:    ; preds = %cond.true.i.i.i.i
  ...
  call void @toolchain.memset.p0i8.i64(i8* %call3.i.i.i.i.i, i8 0, i64 %mul.i.i.i.i.i, i32 4, i1 false)
  br label %_ZNSt6vectorIiSaIiEEC1EmRKiRKS0_.exit
...
entry:
  %conv = sext i32 %n to i64
  tail call void @toolchain.memset.p0i8.i64(i8* %a, i8 0, i64 %conv, i32 1, i1 false)
...
for.body.lr.ph:                                   ; preds = %entry
  ...
  call void @toolchain.memset.p0i8.i64(i8* %a, i8 0, i64 %tmp12, i32 1, i1 false)
  ret void
...
$ clang t.c -S -o - -O0 -emit-toolchain | opt -sroa -S
