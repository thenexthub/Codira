...
#include "vm/core/TargetParser/PPCTargetParser.h"
...
STATISTIC(NumTOCODEumpTable, "Number of Jump Table TOC Entries.");
...
void PPCAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  auto getTOCRelocAdjustedExprForXCOFF = [this](const MCExpr *Expr,
    ...
    ptrdiff_t Adjustment =
        OriginalOffset - toolchain::SignExtend32<16>(OriginalOffset);
...
void PPCLinuxAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  case TargetOpcode::PATCHABLE_RET: {
    ...
    for (const auto &MO : toolchain::drop_begin(MI->operands())) {
      MCOperand MCOp;
...
static bool isSpecialLLVMGlobalArrayToSkip(const GlobalVariable *GV) {
  return GV->hasAppendingLinkage() &&
         StringSwitch<bool>(GV->getName())
             ...
             // handling toolchain.used array. Skipping them for now until we or the
             // AIX OS team come up with a good solution.
             .Case("toolchain.used", true)
             // It's correct to just skip toolchain.compiler.used array here.
             .Case("toolchain.compiler.used", true)
             .Default(false);
}

static bool isSpecialLLVMGlobalArrayForStaticInit(const GlobalVariable *GV) {
  return StringSwitch<bool>(GV->getName())
      .Cases({"toolchain.global_ctors", "toolchain.global_dtors"}, true)
      .Default(false);
}
...
void PPCAIXAsmPrinter::emitGlobalVariableHelper(const GlobalVariable *GV) {
  assert(!GV->getName().starts_with("toolchain.") &&
      ...
      if (FormatIndicatorAndUniqueModId.empty()) {
        ...
        if (UniqueModuleId != "")
          ...
          FormatIndicatorAndUniqueModId =
              "clangPidTidTime_" + toolchain::itostr(sys::Process::getProcessId()) +
              "_" + toolchain::itostr(toolchain::get_threadid()) + "_" +
              toolchain::itostr(CurTime);
        }
      }
...
static std::string convertToSinitPriority(int Priority) {
  ...
  toolchain::raw_string_ostream os(PrioritySuffix);
  os << toolchain::format_hex_no_prefix(P, 8);
...
void PPCAIXAsmPrinter::emitXXStructorList(const DataLayout &DL,
  ...
  for (Structor &S : Structors) {
    ...
    toolchain::GlobalAlias::create(
        GlobalValue::ExternalLinkage,
        (IsCtor ? toolchain::Twine("__sinit") : toolchain::Twine("__sterm")) +
            toolchain::Twine(convertToSinitPriority(S.Priority)) +
            toolchain::Twine("_", FormatIndicatorAndUniqueModId) +
            toolchain::Twine("_", toolchain::utostr(Index++)),
