...
static unsigned getKnownLeadingZeroCount(const unsigned Reg,
  ...
  if (Opcode == PPC::ANDI_rec) {
    ...
    return 48 + toolchain::countl_zero(Imm);
  }
...
static bool collectUnprimedAccPHIs(MachineRegisterInfo *MRI,
  ...
  while (VisitedIndex < PHIs.size()) {
    ...
    for (unsigned PHIOp = 1, NumOps = VisitedPHI->getNumOperands();
      ...
      if (Opcode != PPC::PHI)
        continue;
      if (toolchain::is_contained(PHIs, Instr))
        return false;
...
void PPCMIPeephole::convertUnprimedAccPHIs(
  ...
  for (MachineInstr *PHI : toolchain::reverse(PHIs)) {
    ...
    for (MachineInstr &MI : MBB) {
      ...
      case PPC::ANDIS8_rec: {
        ...
        if (MI.getOpcode() == PPC::ANDIS_rec ||
          ...
          AndImm <<= 16;
        uint64_t LZeroAndImm = toolchain::countl_zero<uint64_t>(AndImm);
        uint64_t RZeroAndImm = toolchain::countr_zero<uint64_t>(AndImm);
...
FunctionPass*
toolchain::createPPCMIPeepholePass() { return new PPCMIPeephole(); }
