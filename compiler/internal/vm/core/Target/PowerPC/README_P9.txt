...
- Vector Extract Unsigned: vextractub vextractuh vextractuw vextractd
  . Don't use toolchain extractelement because they have different semantics
  . Use instrinstics:
...
- Vector Shift Left/Right: vslv vsrv
  . Use intrinsic, don't map to toolchain shl and lshr, because they have different
    semantics, e.g. vslv:
...
- QP Compare Ordered/Unordered: xscmpoqp xscmpuqp
  ...
  . No SDAG, intrinsic, builtin are required??
    Or toolchain fcmp order/unorder compare??
...
- Vector Insert Word: xxinsertw
  ...
  . Note: toolchain has insertelem in "Vector Operations"
    ; yields <n x <ty>>
...
- Vector Extract Unsigned Word: xxextractuw
  ...
  . Note: toolchain has extractelement in "Vector Operations"
    ; yields <ty>
...
- Maximum/Minimum Type-C/Type-J DP: xsmaxcdp xsmaxjdp xsmincdp xsminjdp
  ...
  . dtype map to toolchain fmaxnum/fminnum
    jtype use intrinsic
