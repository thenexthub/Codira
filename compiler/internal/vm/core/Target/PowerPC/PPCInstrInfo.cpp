...
void PPCInstrInfo::finalizeInsInstrs(
  ...
  assert(isa<toolchain::ConstantFP>(C) && "not a valid constant!");
...
Register PPCInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
  ...
  if (toolchain::is_contained(getLoadOpcodesForSpillArray(), MI.getOpcode())) {
...
Register PPCInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                          int &FrameIndex) const {
  if (toolchain::is_contained(getStoreOpcodesForSpillArray(), MI.getOpcode())) {
    if (MI.getOperand(1).isImm() && !MI.getOperand(1).getImm() &&
        MI.getOperand(2).isFI()) {
      FrameIndex = MI.getOperand(2).getIndex();
      return MI.getOperand(0).getReg();
...
#include "vm/core/TargetParser/PPCTargetParser.def"
