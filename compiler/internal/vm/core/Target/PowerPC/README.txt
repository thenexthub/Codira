...
; RUN: toolchain-as < %s | llc -march=ppc32  | grep or | count 3
; This should produce one 'or' or 'cror' instruction per function.

; RUN: toolchain-as < %s | llc -march=ppc32  | grep mfcr | count 3
; PR2964
...
//===----------------------------------------------------------------------===//
; RUN: toolchain-as < %s | llc -march=ppc32 | not grep fneg
...
See https://reviews.toolchain.org/D24924#555306
