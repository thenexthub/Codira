...
static void emitAssignName(Instruction *I, IRBuilder<> &B) {
  ...
  if (!KeepName) {
    if (isa<AllocaInst>(I)) {
      ...
      if (F && F->getName().starts_with("toolchain.spv.alloca"))
        KeepName = true;
    }
  }
...
SPIRVEmitIntrinsics::buildLogicalAccessChainFromGEP(GetElementPtrInst &GEP) {
  ...
  toolchain::append_range(Args, Indices);
...
bool SPIRVEmitIntrinsics::deduceOperandElementTypeCalledFunction(
  ...
  std::string DemangledName =
      getOclOrSpirvBuiltinDemangledName(CalledF->getName());
  if (DemangledName.length() > 0 &&
      !StringRef(DemangledName).starts_with("toolchain.")) {
...
void SPIRVEmitIntrinsics::deduceOperandElementTypeFunctionPointer(
  ...
  for (auto &&[ParmIdx, Arg] : toolchain::enumerate(CI->args())) {
    ...
    for (const auto &Op : I->operands()) {
      ...
      if (AggrConst) {
        ...
        if (auto *COp = dyn_cast<ConstantDataSequential>(Op))
          ...
          toolchain::append_range(Args, AggrConst->operands());
        if (!BPrepared) {
...
Instruction *SPIRVEmitIntrinsics::visitGetElementPtrInst(GetElementPtrInst &I) {
  ...
  SmallVector<Value *, 4> Args;
  Args.push_back(B.getInt1(I.isInBounds()));
  toolchain::append_range(Args, I.operands());
  auto *NewI = B.CreateIntrinsic(Intrinsic::spv_gep, {Types}, {Args});
  replaceAllUsesWithAndErase(B, &I, NewI);
  return NewI;
}
...
void SPIRVEmitIntrinsics::processGlobalValue(GlobalVariable &GV,
  ...
  static const StringSet<> ArtificialGlobals{"toolchain.global.annotations",
...
bool SPIRVEmitIntrinsics::runOnFunction(Function &Func) {
  ...
  SmallVector<Instruction *> Worklist(
      toolchain::make_pointer_range(instructions(Func)));
  ...
  // where needed.
  SmallPtrSet<Instruction *, 4> IncompleteRets;
  for (auto &I : toolchain::reverse(instructions(Func)))
...
bool SPIRVEmitIntrinsics::postprocessTypes(Module &M) {
  ...
  for (auto &F : M) {
    CurrF = &F;
    SmallPtrSet<Instruction *, 4> IncompleteRets;
    for (auto &I : toolchain::reverse(instructions(F))) {
...
ModulePass *toolchain::createSPIRVEmitIntrinsicsPass(SPIRVTargetMachine *TM) {
