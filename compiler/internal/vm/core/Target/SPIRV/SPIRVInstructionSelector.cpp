...
toolchain::SPIRV::SelectionControl::SelectionControl
...
bool sampledTypeIsSignedInteger(const toolchain::Type *HandleType) {
...
bool SPIRVInstructionSelector::selectStore(MachineInstr &I) const {
  ...
  if (IntPtrDef &&
    ...
    if (HandleType->getOpcode() == SPIRV::OpTypeImage) {
      ...
      const toolchain::Type *LLVMHandleType = GR.getTypeForSPIRVType(HandleType);
...
bool SPIRVInstructionSelector::selectStackSave(Register ResVReg,
  ...
  if (!STI.canUseExtension(SPIRV::Extension::SPV_INTEL_variable_length_array))
    report_fatal_error(
        "toolchain.stacksave intrinsic: this instruction requires the following "
...
bool SPIRVInstructionSelector::selectStackRestore(MachineInstr &I) const {
  if (!STI.canUseExtension(SPIRV::Extension::SPV_INTEL_variable_length_array))
    report_fatal_error(
        "toolchain.stackrestore intrinsic: this instruction requires the following "
        "SPIR-V extension: SPV_INTEL_variable_length_array",
        false);
...
static bool isASCastInGVar(MachineRegisterInfo *MRI, Register ResVReg) {
  bool IsGRef = false;
  bool IsAllowedRefs =
      toolchain::all_of(MRI->use_instructions(ResVReg), [&IsGRef](auto const &It) {
...
bool SPIRVInstructionSelector::selectDiscard(Register ResVReg,
  ...
  if (STI.canUseExtension(
      ...
      STI.isAtLeastSPIRVVer(toolchain::VersionTuple(1, 6))) {
...
bool SPIRVInstructionSelector::selectSUCmp(Register ResVReg,
  ...
  // toolchain/lib/Target/SPIRV/SPIRVUtils.cpp when PR #116609 is merged.
...
bool SPIRVInstructionSelector::selectIntrinsic(Register ResVReg,
  ...
  case Intrinsic::arithmetic_fence:
    ...
    break;
  case Intrinsic::spv_thread_id:
    // The HLSL SV_DispatchThreadID semantic is lowered to toolchain.spv.thread.id
    // intrinsic in LLVM IR for SPIR-V backend.
    //
    // In SPIR-V backend, toolchain.spv.thread.id is now correctly translated to a
  ...
  case Intrinsic::spv_thread_id_in_group:
    ...
    // toolchain.spv.thread.id.in.group intrinsic in LLVM IR for SPIR-V backend.
    //
    // In SPIR-V backend, toolchain.spv.thread.id.in.group is now correctly
  ...
  case Intrinsic::spv_group_id:
    ...
    // toolchain.spv.group.id intrinsic in LLVM IR for SPIR-V backend.
    //
    // In SPIR-V backend, toolchain.spv.group.id is now translated to a `WorkgroupId`
    // builtin variable
  ...
  case Intrinsic::spv_flattened_thread_id_in_group:
    ...
    // toolchain.spv.flattened.thread.id.in.group() intrinsic in LLVM IR for SPIR-V
    // backend.
    //
    // In SPIR-V backend, toolchain.spv.flattened.thread.id.in.group is translated to
...
void SPIRVInstructionSelector::decorateUsesAsNonUniform(
  ...
  toolchain::SmallVector<Register> WorkList = {NonUniformReg};
...
bool SPIRVInstructionSelector::selectGlobalValue(
  ...
  assert(GlobalVar->getName() != "toolchain.global.annotations");
...
bool SPIRVInstructionSelector::selectModf(Register ResVReg,
                                          const SPIRVType *ResType,
                                          MachineInstr &I) const {
  // toolchain.modf has a single arg --the number to be decomposed-- and returns a
  ...
  // from this ptr to place it in the struct. toolchain.modf returns the fractional
