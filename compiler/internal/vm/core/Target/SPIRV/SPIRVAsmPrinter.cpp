...
void SPIRVAsmPrinter::cleanUp(Module &M) {
  ...
  for (StringRef GVName :
       {"toolchain.global_ctors", "toolchain.global_dtors", "toolchain.used"}) {
...
void SPIRVAsmPrinter::outputAnnotations(const Module &M) {
  ...
  // Process toolchain.global.annotations special global variable.
  for (auto F = M.global_begin(), E = M.global_end(); F != E; ++F) {
    if ((*F).getName() != "toolchain.global.annotations")
      continue;
    ...
    for (Value *Op : CA->operands()) {
      ...
      if (!isa<Function>(AnnotatedVar))
        report_fatal_error("Unsupported value in toolchain.global.annotations");
      ...
      if (!Reg.isValid()) {
        ...
        DiagMsg = "Unknown function in toolchain.global.annotations: " + DiagMsg;
