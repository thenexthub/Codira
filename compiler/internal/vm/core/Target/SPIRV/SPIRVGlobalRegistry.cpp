...
// after lowering from LLVM IR to GMIR. It can convert an toolchain::Type into
...
SPIRVType *SPIRVGlobalRegistry::getOpTypeArray(uint32_t NumElems,
  ...
  if (NumElems != 0) {
    ...
    if (!ST.isShader()) {
      toolchain::reportFatalUsageError(
...
SPIRVType *SPIRVGlobalRegistry::restOfCreateSPIRVType(
  ...
  // TODO: We could end up with two SPIR-V types pointing to the same toolchain type.
  // Is that a problem?
