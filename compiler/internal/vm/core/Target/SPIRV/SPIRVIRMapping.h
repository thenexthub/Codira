...
inline size_t to_hash(const MachineInstr *MI) {
  hash_code H = toolchain::hash_combine(MI->getOpcode(), MI->getNumOperands());
  for (unsigned I = MI->getNumDefs(); I < MI->getNumOperands(); ++I) {
    const MachineOperand &MO = MI->getOperand(I);
    if (MO.getType() == MachineOperand::MO_CImmediate)
      H = toolchain::hash_combine(H, MO.getType(), MO.getCImm());
    else if (MO.getType() == MachineOperand::MO_FPImmediate)
      H = toolchain::hash_combine(H, MO.getType(), MO.getFPImm());
    else
      H = toolchain::hash_combine(H, MO.getType());
  }
  return H;
}
