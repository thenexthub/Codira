...
char toolchain::SPIRVModuleAnalysis::ID = 0;
...
getSymbolicOperandRequirements(SPIRV::OperandCategory::OperandCategory Category,
  ...
  // can handle them all).
  if (toolchain::all_of(ReqExts, [&ST](const SPIRV::Extension::Extension &Ext) {
...
// hash values, see toolchain::hash_value(const MachineOperand &MO) for details.
...
static InstrSignature instrToSignature(const MachineInstr &MI,
  ...
  for (unsigned i = 0; i < MI.getNumOperands(); ++i) {
    ...
    if (MO.isReg()) {
      ...
      // mimic toolchain::hash_value(const MachineOperand &MO)
...
void addInstrRequirements(const MachineInstr &MI,
  ...
  case SPIRV::OpImageQueryFormat: {
    ...
    for (MachineInstr &UseInst : MRI.use_instructions(ResultReg)) {
      ...
      } else if (toolchain::is_contained(CompareOps, Opc)) {
  ...
  case SPIRV::OpDemoteToHelperInvocation:
    ...
    if (ST.canUseExtension(
      ...
      if (!ST.isAtLeastSPIRVVer(toolchain::VersionTuple(1, 6)))
        Reqs.addExtension(
            SPIRV::Extension::SPV_EXT_demote_to_helper_invocation);
    }
    break;
