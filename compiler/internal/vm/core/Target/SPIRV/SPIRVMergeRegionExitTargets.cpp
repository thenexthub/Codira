...
public:
  ...
  toolchain::Value *createExitVariable(
      BasicBlock *BB,
  ...
  bool runOnConvergenceRegionNoRecurse(LoopInfo &LI,
    ...
    for (auto Exit : SortedExits) {
      toolchain::Value *Value = createExitVariable(Exit, TargetToValue);
      IRBuilder<> B2(Exit);
    ...
    toolchain::Value *Load = Builder.CreateLoad(Builder.getInt32Ty(), Variable);

    // Creating the switch to jump to the correct exit target.
    toolchain::SwitchInst *Sw = Builder.CreateSwitch(Load, SortedExitTargets[0],
...
FunctionPass *toolchain::createSPIRVMergeRegionExitTargetsPass() {
