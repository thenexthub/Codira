...
toolchain::SPIRV::getConvergenceToken(BasicBlock *BB) {
  return getConvergenceTokenInternal<BasicBlock, IntrinsicInst>(BB);
}

std::optional<const IntrinsicInst *>
toolchain::SPIRV::getConvergenceToken(const BasicBlock *BB) {
...
public:
  ConvergenceRegionInfo analyze() {
    ...
    while (ToProcess.size() != 0) {
      ...
      SmallPtrSet<BasicBlock *, 8> RegionBlocks(toolchain::from_range, L->blocks());
...
ConvergenceRegionInfo toolchain::SPIRV::getConvergenceRegions(Function &F,
                                                         DominatorTree &DT,
                                                         LoopInfo &LI) {
