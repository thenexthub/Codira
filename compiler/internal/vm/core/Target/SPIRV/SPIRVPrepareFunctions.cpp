...
// This pass also substitutes some toolchain intrinsic calls with calls to newly
...
static bool lowerIntrinsicToFunction(IntrinsicInst *Intrinsic) {
  // For @toolchain.memset.* intrinsic cases with constant value and length arguments
  ...
  if (Intrinsic->isVolatile())
    FuncName += ".volatile";
  // Redirect @toolchain.intrinsic.* call to @spirv.llvm_intrinsic_*
...
static SmallVector<Metadata *> parseAnnotation(Value *I,
  ...
  for (std::sregex_iterator
    ...
    for (std::size_t i = 1; i < Match.size(); ++i) {
      ...
      } else if (int32_t Num; toolchain::to_integer(StringRef(Item), Num, 10)) {
        MDsItem.push_back(
...
static void lowerExpectAssume(IntrinsicInst *II) {
  ...
  // instruction.
  // For @toolchain.assume we have OpAssumeTrueKHR.
  // For @toolchain.expect we have OpExpectKHR.
  //
...
SPIRVPrepareFunctions::removeAggregateTypesFromSignature(Function *F) {
  ...
  bool HasAggrArg = toolchain::any_of(F->args(), [](Argument &Arg) {
...
ModulePass *
toolchain::createSPIRVPrepareFunctionsPass(const SPIRVTargetMachine &TM) {
