...
SPIRV::Scope::Scope getMemScope(LLVMContext &Ctx, SyncScope::ID Id) {
  ...
  static const toolchain::SyncScope::ID SubGroup =
      Ctx.getOrInsertSyncScopeID("subgroup");
  static const toolchain::SyncScope::ID WorkGroup =
      Ctx.getOrInsertSyncScopeID("workgroup");
  static const toolchain::SyncScope::ID Device =
      Ctx.getOrInsertSyncScopeID("device");

  if (Id == toolchain::SyncScope::SingleThread)
    return SPIRV::Scope::Invocation;
  else if (Id == toolchain::SyncScope::System)
...
bool sortBlocks(Function &F) {
  ...
  toolchain::append_range(Order, RPOT);
...
CallInst *buildIntrWithMD(Intrinsic::ID IntrID, ArrayRef<Type *> Types,
  ...
  toolchain::append_range(Args, Imms);
...
bool isSpvIntrinsic(const Value *Arg) {
  if (const auto *II = dyn_cast<IntrinsicInst>(Arg))
    if (Function *F = II->getCalledFunction())
      if (F->getName().starts_with("toolchain.spv."))
        return true;
  return false;
}
...
SmallVector<unsigned, 1> getSpirvLoopControlOperandsFromLoopMetadata(Loop *L) {
  ...
  if (getBooleanLoopAttribute(L, "toolchain.loop.unroll.disable")) {
    ...
    if (getBooleanLoopAttribute(L, "toolchain.loop.unroll.enable") ||
        getBooleanLoopAttribute(L, "toolchain.loop.unroll.full")) {
