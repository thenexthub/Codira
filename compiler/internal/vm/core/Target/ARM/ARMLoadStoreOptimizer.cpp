...
void ARMLoadStoreOpt::FormCandidates(const MemOpQueue &MemOps) {
  ...
  do {
    ...
    // On codira vldm/vstm starting with an odd register number as that needs
...
bool ARMLoadStoreOpt::MergeBaseUpdateLSMultiple(MachineInstr *MI) {
  ...
  for (const MachineOperand &MO : toolchain::drop_begin(MI->operands(), 3))
    MIB.add(MO);
...
bool ARMLoadStoreOpt::LoadStoreMultipleOpti(MachineBasicBlock &MBB) {
  ...
  toolchain::sort(Candidates, LessThan);
...
bool ARMPreAllocLoadStoreOpt::RescheduleOps(
  ...
  toolchain::sort(Ops, [](const MachineInstr *LHS, const MachineInstr *RHS) {
  ...
  while (Ops.size() > 1) {
    ...
    unsigned NumMove = 0;
    for (MachineInstr *Op : toolchain::reverse(Ops)) {
...
static void updateRegisterMapForDbgValueListAfterMove(
  ...
  forEachDbgRegOperand(DbgValueListInstr, [&](MachineOperand &Op) {
    ...
    toolchain::replace(InstrVec, InstrToReplace, DbgValueListInstr);
  });
}
      ...
      if (InstrIt != DbgValueSinkCandidates.end()) {
        ...
        if (RegIt != InstrMap.end()) {
          ...
          for (auto &Reg : RegVec) {
            ...
            toolchain::erase_if(InstrVec, IsDbgVar);
          }
...
FunctionPass *toolchain::createARMLoadStoreOptimizationPass(bool PreAlloc) {
  if (PreAlloc)
