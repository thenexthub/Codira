...
#include "vm/core/MC/MCSymbolMachO.h"
...
MCFixupKindInfo ARMAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  ...
  assert(unsigned(Kind - FirstTargetFixupKind) < ARM::NumTargetFixupKinds &&
         "Invalid kind!");
  return (Endian == toolchain::endianness::little
              ? InfosLE
...
unsigned ARMAsmBackend::adjustFixupValue(const MCAssembler &Asm,
  ...
  case ARM::fixup_t2_movw_lo16: {
    ...
    return swapHalfWords(Value, Endian == toolchain::endianness::little);
  }
  ...
  case ARM::fixup_t2_adr_pcrel_12: {
    ...
    return swapHalfWords(out, Endian == toolchain::endianness::little);
  }
  ...
  case ARM::fixup_t2_so_imm: {
    ...
    EncValue |= (Value & 0xff);
    return swapHalfWords(EncValue, Endian == toolchain::endianness::little);
  }
  case ARM::fixup_bf_branch: {
    const char *FixupDiagnostic = reasonForFixupRelaxation(Fixup, Value);
    if (FixupDiagnostic) {
...
void ARMAsmBackend::applyFixup(const MCFragment &F, const MCFixup &Fixup,
  ...
  if (Endian == toolchain::endianness::big) {
  ...
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned Idx =
        Endian == toolchain::endianness::little ? i : (FullSizeBytes - 1 - i);
...
uint64_t ARMAsmBackendDarwin::generateCompactUnwindEncoding(
  ...
  DEBUG_WITH_TYPE("compact-unwind", toolchain::dbgs() << "generateCU()\n");
    ...
    case MCCFIInstruction::OpOffset: // DW_CFA_offset
      ...
      if (ARMMCRegisterClasses[ARM::GPRRegClassID].contains(Reg))
        ...
        DEBUG_WITH_TYPE("compact-unwind",
                        toolchain::dbgs() << ".cfi_offset on unknown register="
                                     << Inst.getRegister() << "\n");
        return CU::UNWIND_ARM_MODE_DWARF;
      }
      break;
  ...
  if (CFARegister != ARM::R7) {
    DEBUG_WITH_TYPE("compact-unwind", toolchain::dbgs() << "frame register is "
                                                   << CFARegister.id()
                                                   << " instead of r7\n");
    return CU::UNWIND_ARM_MODE_DWARF;
  }
  ...
  if (RegOffsets.lookup(ARM::LR) != (-4 - StackAdjust)) {
    DEBUG_WITH_TYPE(
        "compact-unwind",
        toolchain::dbgs() << "LR not saved as standard frame, StackAdjust="
                     << StackAdjust
  ...
  if (RegOffsets.lookup(ARM::R7) != (-8 - StackAdjust)) {
    DEBUG_WITH_TYPE("compact-unwind",
                    toolchain::dbgs() << "r7 not saved as standard frame\n");
    return CU::UNWIND_ARM_MODE_DWARF;
  }
  ...
  case 12:
    ...
    break;
  default:
    DEBUG_WITH_TYPE("compact-unwind", toolchain::dbgs()
  ...
  for (auto CSReg : GPRCSRegs) {
    ...
    if (RegOffset != CurOffset - 4) {
      DEBUG_WITH_TYPE("compact-unwind",
                      toolchain::dbgs() << MRI.getName(CSReg.Reg) << " saved at "
  ...
  if (FloatRegCount > 4) {
    DEBUG_WITH_TYPE("compact-unwind",
                    toolchain::dbgs() << "unsupported number of D registers saved ("
                                 << FloatRegCount << ")\n");
      return CU::UNWIND_ARM_MODE_DWARF;
  }
  ...
  for (int Idx = FloatRegCount - 1; Idx >= 0; --Idx) {
    ...
    if (Offset == RegOffsets.end()) {
      DEBUG_WITH_TYPE("compact-unwind",
                      toolchain::dbgs() << FloatRegCount << " D-regs saved, but "
...
MCAsmBackend *toolchain::createARMLEAsmBackend(const Target &T,
                                          const MCSubtargetInfo &STI,
                                          const MCRegisterInfo &MRI,
                                          const MCTargetOptions &Options) {
  return createARMAsmBackend(T, STI, MRI, Options, toolchain::endianness::little);
}

MCAsmBackend *toolchain::createARMBEAsmBackend(const Target &T,
                                          const MCSubtargetInfo &STI,
                                          const MCRegisterInfo &MRI,
                                          const MCTargetOptions &Options) {
  return createARMAsmBackend(T, STI, MRI, Options, toolchain::endianness::big);
}
