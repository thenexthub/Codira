...
static void printRegImmShift(raw_ostream &O, ARM_AM::ShiftOpc ShOpc,
  ...
  if (ShOpc != ARM_AM::rrx) {
    O << " ";
    printer.markup(O, toolchain::MCInstPrinter::Markup::Immediate)
...
void ARMInstPrinter::printBitfieldInvMaskImmOperand(const MCInst *MI,
  ...
  int32_t lsb = toolchain::countr_zero(v);
  int32_t width = toolchain::bit_width(v) - lsb;
...
void ARMInstPrinter::printThumbITMask(const MCInst *MI, unsigned OpNum,
  ...
  unsigned NumTZ = toolchain::countr_zero(Mask);
...
void ARMInstPrinter::printModImmOperand(const MCInst *MI, unsigned OpNum,
  ...
  int32_t Rotated = toolchain::rotr<uint32_t>(Bits, Rot);
