...
namespace ARM_AM {
  ...
  inline unsigned getSOImmValRotate(unsigned Imm) {
    ...
    unsigned TZ = toolchain::countr_zero(Imm);
    ...
    if ((toolchain::rotr<uint32_t>(Imm, RotAmt) & ~255U) == 0)
      return (32-RotAmt)&31;  // HW rotates right, not left.

    // For values like 0xF000000F, we should ignore the low 6 bits, then
    // retry the hunt.
    if (Imm & 63U) {
      unsigned TZ2 = toolchain::countr_zero(Imm & ~63U);
      unsigned RotAmt2 = TZ2 & ~1;
      if ((toolchain::rotr<uint32_t>(Imm, RotAmt2) & ~255U) == 0)
  ...
  inline int getSOImmVal(unsigned Arg) {
    ...
    if (toolchain::rotr<uint32_t>(~255U, RotAmt) & Arg)
      return -1;

    // Encode this correctly.
    return toolchain::rotl<uint32_t>(Arg, RotAmt) | ((RotAmt >> 1) << 8);
  }
  ...
  inline bool isSOImmTwoPartVal(unsigned V) {
    ...
    V = toolchain::rotr<uint32_t>(~255U, getSOImmValRotate(V)) & V;
    if (V == 0)
      return false;
  ...
  inline unsigned getSOImmTwoPartFirst(unsigned V) {
    return toolchain::rotr<uint32_t>(255U, getSOImmValRotate(V)) & V;
  }
  ...
  inline unsigned getSOImmTwoPartSecond(unsigned V) {
    ...
    // Take what's left.
    assert(V == (toolchain::rotr<uint32_t>(255U, getSOImmValRotate(V)) & V));
    return V;
  }
  ...
  inline bool isSOImmTwoPartValNeg(unsigned V) {
    ...
    First = ~(-First);
    return !(toolchain::rotr<uint32_t>(~255U, getSOImmValRotate(First)) & First);
  }
  ...
  inline unsigned getThumbImmValShift(unsigned Imm) {
    ...
    return toolchain::countr_zero(Imm);
  }
  ...
  inline int getT2SOImmValRotateVal(unsigned V) {
    unsigned RotAmt = toolchain::countl_zero(V);
    if (RotAmt >= 24)
      return -1;

    // If 'Arg' can be handled with a single shifter_op return the value.
    if ((toolchain::rotr<uint32_t>(0xff000000U, RotAmt) & V) == V)
      return (toolchain::rotr<uint32_t>(V, 24 - RotAmt) & 0x7f) |
             ((RotAmt + 8) << 7);
  ...
  inline unsigned getT2SOImmValRotate(unsigned V) {
    ...
    // Use CTZ to compute the rotate amount.
    unsigned RotAmt = toolchain::countr_zero(V);
    return (32 - RotAmt) & 31;
  }
  ...
  inline bool isT2SOImmTwoPartVal(unsigned Imm) {
    ...
    if (getT2SOImmValSplatVal(V) != -1)
      return false;
    V = toolchain::rotr<uint32_t>(~255U, getT2SOImmValRotate(V)) & V;
    if (V == 0)
      return false;
  ...
  inline unsigned getT2SOImmTwoPartFirst(unsigned Imm) {
    ...
    unsigned V = toolchain::rotr<uint32_t>(~255, getT2SOImmValRotate(Imm)) & Imm;
