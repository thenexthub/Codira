...
static bool getMCRDeprecationInfo(MCInst &MI, const MCSubtargetInfo &STI,
                                  std::string &Info) {
  if (STI.hasFeature(toolchain::ARM::HasV7Ops) &&
...
static bool getARMStoreDeprecationInfo(MCInst &MI, const MCSubtargetInfo &STI,
                                       std::string &Info) {
  assert(!STI.hasFeature(toolchain::ARM::ModeThumb) &&
...
static MCRelocationInfo *createARMMCRelocationInfo(const Triple &TT,
  ...
  return toolchain::createMCRelocationInfo(TT, Ctx);
}
...
ARMMCInstrAnalysis::findPltEntries(uint64_t PltSectionVA,
  ...
  toolchain::endianness DataEndianness = STI.getTargetTriple().isLittleEndian()
                                        ? endianness::little
                                        : endianness::big;
  toolchain::endianness InstrEndianness =
