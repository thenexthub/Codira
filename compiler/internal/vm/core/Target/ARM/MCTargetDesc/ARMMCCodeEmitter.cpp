...
getBitfieldInvertedMaskOpValue(const MCInst &MI, unsigned Op,
  ...
  uint32_t lsb = toolchain::countr_zero(v);
  uint32_t msb = toolchain::Log2_32(v);
...
void ARMMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  auto Endian =
      IsLittleEndian ? toolchain::endianness::little : toolchain::endianness::big;
...
getPowerTwoOpValue(const MCInst &MI, unsigned OpIdx,
  ...
  const MCOperand &MO = MI.getOperand(OpIdx);
  assert(MO.isImm() && "Unexpected operand type!");
  return toolchain::countr_zero((uint64_t)MO.getImm());
}
...
MCCodeEmitter *toolchain::createARMLEMCCodeEmitter(const MCInstrInfo &MCII,
                                              MCContext &Ctx) {
  return new ARMMCCodeEmitter(MCII, Ctx, true);
}

MCCodeEmitter *toolchain::createARMBEMCCodeEmitter(const MCInstrInfo &MCII,
                                              MCContext &Ctx) {
