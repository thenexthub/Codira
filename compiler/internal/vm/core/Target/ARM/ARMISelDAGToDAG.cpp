...
void ARMDAGToDAGISel::PreprocessISelDAG() {
  ...
  for (SDNode &N : toolchain::make_early_inc_range(CurDAG->allnodes())) {
    ...
    unsigned TZ = toolchain::countr_zero(And_imm);
    if (TZ != 1 && TZ != 2)
...
bool ARMDAGToDAGISel::SelectAddrMode6(SDNode *Parent, SDValue N, SDValue &Addr,
  ...
  if (isa<LSBaseSDNode>(MemN) ||
    ...
    toolchain::Align MMOAlign = MemN->getAlign();
...
bool ARMDAGToDAGISel::transformFixedFloatingPointConversion(SDNode *N,
  ...
  bool IsExact;
  ToConvert.convertToInteger(Converted, toolchain::RoundingMode::NearestTiesToEven,
                             &IsExact);
  ...
  if (N->getOpcode() == ISD::AND) {
    if (isOpcWithIntImmediate(N, ISD::AND, And_imm)) {
      ...
      if (isOpcWithIntImmediate(N->getOperand(0).getNode(), ISD::SRL,
        ...
        unsigned Width = toolchain::countr_one(And_imm) - 1;
...
void ARMDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::arm_neon_vcvtbfp2bf: {
      ...
      toolchain::EVT DestTy = N->getValueType(0);
      SDValue Pred = getAL(CurDAG, dl);
      SDValue Reg0 = CurDAG->getRegister(0, MVT::i32);
...
///
FunctionPass *toolchain::createARMISelDag(ARMBaseTargetMachine &TM,
