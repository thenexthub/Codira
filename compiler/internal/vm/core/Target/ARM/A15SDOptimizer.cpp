...
unsigned A15SDOptimizer::optimizeSDPattern(MachineInstr *MI) {
  ...
  if (MI->isRegSequence() && usesRegClass(MI->getOperand(1),
    ...
    for (MachineOperand &MO : toolchain::drop_begin(MI->explicit_operands())) {
      if (!MO.isReg())
        continue;
      ++NumTotal;
  ...
  for (unsigned I : Defs) {
    ...
    for (MachineInstr *MI : DefSrcs) {
      ...
      SmallVector<MachineOperand *, 8> Uses(
          toolchain::make_pointer_range(MRI->use_operands(DPRDefReg)));
...
FunctionPass *toolchain::createA15SDOptimizerPass() {
