...
Thumb2SizeReduce::ReduceLoadStore(MachineBasicBlock &MBB, MachineInstr *MI,
  ...
  case ARM::t2STMIA: {
    ...
    Register BaseReg = MI->getOperand(0).getReg();
    for (const MachineOperand &MO : toolchain::drop_begin(MI->operands(), 4))
      if (MO.getReg() == BaseReg)
        return false;
  ...
  for (const MachineOperand &MO : toolchain::drop_begin(MI->operands(), OpNum))
    MIB.add(MO);
...
/// reduction pass.
FunctionPass *toolchain::createThumb2SizeReductionPass(
