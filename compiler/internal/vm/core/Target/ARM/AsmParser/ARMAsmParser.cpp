...
#include "vm/core/MC/MCParser/MCAsmParserUtils.h"
...
#include "vm/core/Support/ARMBuildAttributes.h"
...
class ARMAsmParser : public MCTargetAsmParser {
  ...
  bool lastInITBlock() {
    return ITState.CurPosition == 4 - (unsigned)toolchain::countr_zero(ITState.Mask);
  }
  ...
  void forwardITPosition() {
    ...
    unsigned TZ = toolchain::countr_zero(ITState.Mask);
  ...
  void forwardVPTPosition() {
    ...
    unsigned TZ = toolchain::countr_zero(VPTState.Mask);
...
public:
  ...
  bool isPowerTwoInRange() const {
    ...
    if (!CE) return false;
    int64_t Value = CE->getValue();
    return Value > 0 && toolchain::popcount((uint64_t)Value) == 1 && Value >= Min &&
           Value <= Max;
  }
  ...
  CreateRegList(SmallVectorImpl<std::pair<unsigned, MCRegister>> &Regs,
    ...
    assert(toolchain::is_sorted(Regs) && "Register list must be sorted by encoding");
...
void ARMAsmParser::cvtThumbMultiply(MCInst &Inst,
  ...
  if (CondI != 0) {
    ...
    ARMOperand Op = *ARMOperand::CreateCondCode(
        toolchain::ARMCC::AL, Operands[0]->getEndLoc(), *this);
...
bool ARMAsmParser::parsePrefix(ARM::Specifier &Spec) {
  ...
  const auto &Prefix =
      toolchain::find_if(PrefixEntries, [&IDVal](const PrefixEntry &PE) {
...
bool ARMAsmParser::parseInstruction(ParseInstructionInfo &Info, StringRef Name,
  ...
  if (Mnemonic == "it" || Mnemonic.starts_with("vpt") ||
    ...
    unsigned Mask = 8;
    for (char Pos : toolchain::reverse(ITMask)) {
      if (Pos != 't' && Pos != 'e') {
  ...
  if (CanAcceptPredicationCode && PredicationCode != toolchain::ARMCC::AL) {
    SMLoc Loc = SMLoc::getFromPointer(NameLoc.getPointer() + Mnemonic.size() +
                                      CarrySetting);
  ...
  if (CanAcceptVPTPredicationCode && VPTPredicationCode != toolchain::ARMVCC::None &&
...
bool ARMAsmParser::validateInstruction(MCInst &Inst,
  ...
  case ARM::t2IT: {
    ...
    if (Cond == ARMCC::AL && toolchain::popcount(Mask) != 1)
  ...
  case ARM::ADDri: {
    ...
    if (Inst.getOperand(2).isImm()) {
      ...
      TmpInst.addOperand(MCOperand::createImm(
          toolchain::rotr<uint32_t>(Enc & 0xFF, (Enc & 0xF00) >> 7)));
    } else {
