...
#include "vm/core/CodeGen/SelectionDAGAddressAnalysis.h"
...
#include "vm/core/MC/MCInstrItineraries.h"
...
ARMTargetLowering::ARMTargetLowering(const TargetMachine &TM_,
  ...
  // @toolchain.readcyclecounter requires the Performance Monitors extension.
  ...
  MaxStoresPerMemcpy = 4; // For @toolchain.memcpy -> sequence of stores
  MaxStoresPerMemcpyOptSize = 2;
  MaxStoresPerMemmove = 4; // For @toolchain.memmove -> sequence of stores
...
SDValue ARMTargetLowering::getARMCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
  ...
  if (Subtarget->isThumb1Only() && LHS->getOpcode() == ISD::AND &&
    ...
    if (isMask_32(Mask) && (RHSV & ~Mask) == 0 && Mask != 255 && Mask != 65535) {
      unsigned ShiftBits = toolchain::countl_zero(Mask);
...
static SDValue LowerSaturatingConditional(SDValue Op, SelectionDAG &DAG) {
  ...
  if (Val1 == ~Val2)
    return DAG.getNode(ARMISD::SSAT, dl, VT, V2Tmp,
                       DAG.getConstant(toolchain::countr_one(K), dl, VT));
  if (NegVal == 0)
...
SDValue ARMTargetLowering::LowerSET_ROUNDING(SDValue Op,
  ...
  // The rounding mode is in bits 23:22 of the FPSCR.
  // The toolchain.set.rounding argument value to ARM rounding mode value mapping
  // is 0->3, 1->0, 2->1, 3->2. The formula we use to implement this is
  // ((arg - 1) & 3) << 22).
  //
  // It is expected that the argument of toolchain.set.rounding is within the
  // segment [0, 3], so NearestTiesToAway (4) is not handled here. It is
  // responsibility of the code generated toolchain.set.rounding to ensure this
  // condition.
...
static SDValue LowerBUILD_VECTOR_i1(SDValue Op, SelectionDAG &DAG,
  ...
  if (!isa<ConstantSDNode>(FirstOp) &&
      toolchain::all_of(toolchain::drop_begin(Op->ops()), [&FirstOp](const SDUse &U) {
...
SDValue ARMTargetLowering::ReconstructShuffle(SDValue Op,
  ...
  for (unsigned i = 0; i < NumElts; ++i) {
    ...
    auto Source = toolchain::find(Sources, SourceVec);
  ...
  for (unsigned i = 0; i < VT.getVectorNumElements(); ++i) {
    ...
    auto Src = toolchain::find(Sources, Entry.getOperand(0));
...
static SDValue LowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG,
  ...
  if (EltSize <= 32) {
    ...
    if (ST->hasNEON() && V1->getOpcode() == ISD::CONCAT_VECTORS && V2->isUndef()) {
      ...
      assert(toolchain::all_of(ShuffleMask, [&](int i) {
...
static SDValue LowerTruncate(SDNode *N, SelectionDAG &DAG,
  ...
  // to move order.
  //
  // But truncates and sext/zext are always going to be fairly common from toolchain.
...
static SDValue LowerPredicateLoad(SDValue Op, SelectionDAG &DAG) {
  ...
  // for BE).
  // Speaking of BE, apparently the rest of toolchain will assume a reverse order to
...
// which toolchain will have optimized to min(ashr(mul(sext(x), sext(y)), 15))) as
...
static SDValue PerformMULCombine(SDNode *N,
  ...
  unsigned ShiftAmt = toolchain::countr_zero<uint64_t>(MulAmt);
  ...
  if (MulAmt >= 0) {
    if (toolchain::has_single_bit<uint32_t>(MulAmt - 1)) {
      ...
      Res = DAG.getNode(ISD::ADD, DL, VT,
                        ...
                        DAG.getNode(ISD::SHL, DL, VT,
                                    ...
                                    DAG.getConstant(Log2_32(MulAmt - 1), DL,
                                                    MVT::i32)));
    } else if (toolchain::has_single_bit<uint32_t>(MulAmt + 1)) {
    ...
    if (toolchain::has_single_bit<uint32_t>(MulAmtAbs + 1)) {
      ...
      Res = DAG.getNode(ISD::SUB, DL, VT,
                        ...
                        DAG.getNode(ISD::SHL, DL, VT,
                                    ...
                                    DAG.getConstant(Log2_32(MulAmtAbs + 1), DL,
                                                    MVT::i32)));
    } else if (toolchain::has_single_bit<uint32_t>(MulAmtAbs - 1)) {
      // (mul x, -(2^N + 1)) => - (add (shl x, N), x)
      Res = DAG.getNode(ISD::ADD, DL, VT,
                        V,
                        DAG.getNode(ISD::SHL, DL, VT,
...
static SDValue CombineANDShift(SDNode *N,
  ...
  if (!LeftShift && isMask_32(C1)) {
    uint32_t C3 = toolchain::countl_zero(C1);
    if (C2 < C3) {
  ...
  if (LeftShift && isMask_32(~C1)) {
    uint32_t C3 = toolchain::countr_zero(C1);
    if (C2 < C3) {
  ...
  if (LeftShift && isShiftedMask_32(C1)) {
    uint32_t Trailing = toolchain::countr_zero(C1);
    uint32_t C3 = toolchain::countl_zero(C1);
  ...
  if (!LeftShift && isShiftedMask_32(C1)) {
    uint32_t Leading = toolchain::countl_zero(C1);
    uint32_t C3 = toolchain::countr_zero(C1);
...
static SDValue PerformORCombineToBFI(SDNode *N,
  ...
  if (N1C) {
    ...
    if (ARM::isBitFieldInvertedMask(Mask)) {
      Val >>= toolchain::countr_zero(~Mask);
    ...
    if (ARM::isBitFieldInvertedMask(Mask) &&
      ...
      if (Subtarget->hasDSP() &&
        ...
        return SDValue();
      // 2a
      unsigned amt = toolchain::countr_zero(Mask2);
      ...
      if (Subtarget->hasDSP() &&
        ...
        return SDValue();
      // 2b
      unsigned lsb = toolchain::countr_zero(Mask);
...
static SDValue PerformBFICombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (N1.getOpcode() == ISD::AND) {
    ...
    unsigned LSB = toolchain::countr_zero(~InvMask);
    unsigned Width = toolchain::bit_width<unsigned>(~InvMask) - LSB;
    assert(Width <
...
static SDValue CombineBaseUpdate(SDNode *N,
  ...
  toolchain::stable_sort(BaseUpdates,
...
static SDValue PerformShiftCombine(SDNode *N,
  ...
  if (ST->isThumb1Only() && N->getOpcode() == ISD::SHL && VT == MVT::i32 &&
    ...
    if (isMask_32(AndMask)) {
      uint32_t MaskedBits = toolchain::countl_zero(AndMask);
...
///
///      Into:
///        %vld2 = { <4 x i32>, <4 x i32> } call toolchain.arm.neon.vld2(%ptr, 4)
...
///        call void toolchain.arm.neon.vst3(%ptr, %sub.v0, %sub.v1, %sub.v2, 4)
///
...
bool ARMTargetLowering::isComplexDeinterleavingOperationSupported(
  ...
  if (VTyWidth < 128 || !toolchain::isPowerOf2_32(VTyWidth))
    return false;
...
Value *ARMTargetLowering::createComplexDeinterleavingIR(
  ...
  if (TyWidth > 128) {
    ...
    auto SplitSeq = toolchain::seq<int>(0, Ty->getNumElements());
    auto SplitSeqVec = toolchain::to_vector(SplitSeq);
