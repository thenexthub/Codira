...
/// Convert a vector load intrinsic into a simple toolchain load instruction.
...
InstructionCost ARMTTIImpl::getVectorInstrCost(unsigned Opcode, Type *ValTy,
  ...
  // lower estimated throughput on codira.
...
InstructionCost ARMTTIImpl::getShuffleCost(TTI::ShuffleKind Kind,
  ...
  if (ST->hasMVEIntegerOps()) {
    ...
    if (!Mask.empty()) {
      std::pair<InstructionCost, MVT> LT = getTypeLegalizationCost(SrcTy);
      // Check for LD2/LD4 instructions, which are represented in toolchain IR as
      ...
      // Check for ST2/ST4 instructions, which are represented in toolchain IR as
...
bool ARMTTIImpl::isLoweredToCall(const Function *F) const {
  ...
  if (F->getName().starts_with("toolchain.arm"))
    return false;
...
static bool canTailPredicateLoop(Loop *L, LoopInfo *LI, ScalarEvolution &SE,
  ...
  LiveOuts = toolchain::findDefsUsedOutsideOfLoop(L);
...
ARMTTIImpl::getPreferredTailFoldingStyle(bool IVUpdateMayOverflow) const {
  ...
  // Intrinsic @toolchain.get.active.lane.mask is supported.
