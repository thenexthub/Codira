...
GetARMJTIPICODEumpTableLabel(unsigned uid) const {
  const DataLayout &DL = getDataLayout();
  SmallString<60> Name;
...
void ARMAsmPrinter::emitJumpTableAddrs(const MachineInstr *MI) {
  ...
  MCSymbol *JTISymbol = GetARMJTIPICODEumpTableLabel(JTI);
...
ARMAsmPrinter::getCodeViewJumpTableInfo(int JTI,
  ...
  case ARM::tBR_JTr:
    ...
    BaseLabel = GetARMJTIPICODEumpTableLabel(JTI);
    break;
  case ARM::tTBH_JT:
  case ARM::t2TBH_JT:
...
void ARMAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  case ARM::t2LEApcrelJT: {
    MCSymbol *JTIPICSymbol =
      GetARMJTIPICODEumpTableLabel(MI->getOperand(1).getIndex());
    EmitToStreamer(*OutStreamer, MCInstBuilder(MI->getOpcode() ==
