...
void toolchain::emitT2RegPlusImmediate(MachineBasicBlock &MBB,
  ...
  while (NumBytes) {
    ...
    if (ImmIsT2SO != -1) {
      ...
      unsigned RotAmt = toolchain::countl_zero(ThisVal);
      ThisVal = ThisVal & toolchain::rotr<uint32_t>(0xff000000U, RotAmt);
      NumBytes &= ~ThisVal;
...
bool toolchain::rewriteT2FrameIndex(MachineInstr &MI, unsigned FrameRegIdx,
  ...
  if (IsSP || Opcode == ARM::t2ADDri || Opcode == ARM::t2ADDri12) {
    ...
    // t2ADDri/t2SUBri.
    unsigned RotAmt = toolchain::countl_zero<unsigned>(Offset);
    unsigned ThisImmVal = Offset & toolchain::rotr<uint32_t>(0xff000000U, RotAmt);
...
ARMCC::CondCodes toolchain::getITInstrPredicate(const MachineInstr &MI,
                                           Register &PredReg) {
...
int toolchain::findFirstVPTPredOperandIdx(const MachineInstr &MI) {
...
int toolchain::findVPTInactiveOperandIdx(const MachineInstr &MI) {
  const MCInstrDesc &MCID = MI.getDesc();
...
ARMVCC::VPTCodes toolchain::getVPTInstrPredicate(const MachineInstr &MI,
                                            Register &PredReg) {
...
void toolchain::recomputeVPTBlockMask(MachineInstr &Instr) {
