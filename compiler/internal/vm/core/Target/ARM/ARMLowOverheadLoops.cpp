...
static bool isVectorPredicated(MachineInstr *MI) {
  int PIdx = toolchain::findFirstVPTPredOperandIdx(*MI);
...
namespace {
  ...
  public:
    ...
    bool containsVCTP() const { return toolchain::any_of(Insts, isVCTP); }
  ...
  public:
    ...
    bool isPredicatedOnVCTP(MachineInstr *MI, bool Exclusive = false) {
      ...
      if (getVPTInstrPredicate(*MI) == ARMVCC::Else)
        return false;
      return toolchain::any_of(Predicates, isVCTP);
    }
...
bool LowOverheadLoop::ValidateTailPredicate() {
  ...
  if ((Start->getOpcode() == ARM::t2DoLoopStartTP ||
    ...
    if (auto *Def = RDI.getUniqueReachingMIDef(
      ...
      SmallPtrSet<MachineInstr *, 2> Ignore(toolchain::from_range, VCTPs);
...
bool ARMLowOverheadLoops::RevertLoopDec(MachineInstr *MI) const {
  ...
  toolchain::RevertLoopDec(MI, TII, SetFlags);
  return SetFlags;
}
...
void ARMLowOverheadLoops::RevertLoopEnd(MachineInstr *MI, bool SkipCmp) const {
  ...
  toolchain::RevertLoopEnd(MI, TII, BrOpc, SkipCmp);
}
...
FunctionPass *toolchain::createARMLowOverheadLoopsPass() {
