...
MachineInstr *MVETPAndVPTOptimisations::CheckForLRUseInPredecessors(
  ...
  while (!Worklist.empty()) {
    ...
    Visited.insert(MBB);
    toolchain::append_range(Worklist, MBB->predecessors());
  }
  return LoopStart;
}
  ...
  auto CheckUsers = [&Copies](Register BaseReg,
    ...
    while (!Worklist.empty()) {
      ...
      for (MachineInstr &MI : MRI->use_nodbg_instructions(Reg)) {
        if (toolchain::is_contained(ExpectedUsers, &MI))
          continue;
...
bool MVETPAndVPTOptimisations::ReplaceConstByVPNOTs(MachineBasicBlock &MBB,
  ...
  for (MachineInstr &Instr : MBB.instrs()) {
    ...
    int PIdx = toolchain::findFirstVPTPredOperandIdx(Instr);
    if (PIdx == -1)
      continue;
...
FunctionPass *toolchain::createMVETPAndVPTOptimisationsPass() {
