...
#include "vm/core/CodeGen/MachinePipeliner.h"
...
#include "vm/core/CodeGen/ScoreboardHazardRecognizer.h"
...
void toolchain::addUnpredicatedMveVpredNOp(MachineInstrBuilder &MIB) {
  MIB.addImm(ARMVCC::None);
  MIB.addReg(0);
  MIB.addReg(0); // tp_reg
}

void toolchain::addUnpredicatedMveVpredROp(MachineInstrBuilder &MIB,
                                      Register DestReg) {
  addUnpredicatedMveVpredNOp(MIB);
  MIB.addReg(DestReg, RegState::Undef);
}

void toolchain::addPredicatedMveVpredNOp(MachineInstrBuilder &MIB, unsigned Cond) {
  MIB.addImm(Cond);
  MIB.addReg(ARM::VPR, RegState::Implicit);
  MIB.addReg(0); // tp_reg
}

void toolchain::addPredicatedMveVpredROp(MachineInstrBuilder &MIB,
...
void ARMBaseInstrInfo::expandMEMCPY(MachineBasicBlock::iterator MI) const {
  ...
  for (MachineOperand &MO : toolchain::drop_begin(MI->operands(), 5))
    ScratchRegs.push_back(MO.getReg());
  toolchain::sort(ScratchRegs,
...
ARMCC::CondCodes toolchain::getInstrPredicate(const MachineInstr &MI,
                                         Register &PredReg) {
  int PIdx = MI.findFirstPredOperandIdx();
  if (PIdx == -1) {
    PredReg = 0;
...
unsigned toolchain::getMatchingCondBranchOpcode(unsigned Opc) {
  if (Opc == ARM::B)
    return ARM::Bcc;
  if (Opc == ARM::tB)
    return ARM::tBcc;
...
ARMBaseInstrInfo::canFoldIntoMOVCC(Register Reg, const MachineRegisterInfo &MRI,
  ...
  for (const MachineOperand &MO : toolchain::drop_begin(MI->operands(), 1)) {
...
unsigned toolchain::convertAddSubFlagsOpcode(unsigned OldOpc) {
...
void toolchain::emitARMRegPlusImmediate(MachineBasicBlock &MBB,
  ...
  while (NumBytes) {
    ...
    unsigned ThisVal = NumBytes & toolchain::rotr<uint32_t>(0xFF, RotAmt);
...
bool toolchain::tryFoldSPUpdateIntoPushPop(const ARMSubtarget &Subtarget,
  ...
  for (const MachineOperand &MO : toolchain::reverse(RegList))
    MIB.add(MO);

  return true;
}

bool toolchain::rewriteARMFrameIndex(MachineInstr &MI, unsigned FrameRegIdx,
  ...
  if (Opcode == ARM::ADDri) {
    ...
    unsigned ThisImmVal = Offset & toolchain::rotr<uint32_t>(0xFF, RotAmt);
...
bool ARMBaseInstrInfo::verifyInstruction(const MachineInstr &MI,
  ...
  if (MI.getOpcode() == ARM::tPUSH ||
      MI.getOpcode() == ARM::tPOP ||
      MI.getOpcode() == ARM::tPOP_RET) {
    for (const MachineOperand &MO : toolchain::drop_begin(MI.operands(), 2)) {
...
bool toolchain::registerDefinedBetween(unsigned Reg,
...
MachineInstr *toolchain::findCMPToFoldIntoCBZ(MachineInstr *Br,
                                         const TargetRegisterInfo *TRI) {
...
unsigned toolchain::ConstantMaterializationCost(unsigned Val,
...
bool toolchain::HasLowerConstantMaterializationCost(unsigned Val1, unsigned Val2,
                                               const ARMSubtarget *Subtarget,
                                               bool ForCodesize) {
...
ARMBaseInstrInfo::getOutliningCandidateInfo(
  ...
  if (!(FlagsSetInAll & UnsafeRegsDead)) {
    ...
    toolchain::erase_if(RepeatedSequenceLocs, CantGuaranteeValueAcrossCall);
  ...
  // overhead.
  auto NoBTI =
      toolchain::partition(RepeatedSequenceLocs, [](const outliner::Candidate &C) {
...
void ARMBaseInstrInfo::buildOutlinedFrame(
  ...
  if (toolchain::any_of(MBB.instrs(), IsNonTailCall)) {
...
unsigned toolchain::getBLXOpcode(const MachineFunction &MF) {
  return (MF.getSubtarget<ARMSubtarget>().hardenSlsBlr()) ? ARM::BLX_noip
                                                          : ARM::BLX;
}

unsigned toolchain::gettBLXrOpcode(const MachineFunction &MF) {
  return (MF.getSubtarget<ARMSubtarget>().hardenSlsBlr()) ? ARM::tBLXr_noip
                                                          : ARM::tBLXr;
}

unsigned toolchain::getBLXpredOpcode(const MachineFunction &MF) {
...
bool ARMPipelinerLoopInfo::tooMuchRegisterPressure(SwingSchedulerDAG &SSD,
  ...
  for (int Cycle = SMS.getFinalCycle(); Cycle >= SMS.getFirstCycle(); --Cycle)
    for (int Stage = 0, StageEnd = SMS.getMaxStageCount(); Stage <= StageEnd;
      ...
      toolchain::append_range(ProposedSchedule, Instrs);
    }
