...
static const NEONLdStTableEntry *LookupNEONLdSt(unsigned Opcode) {
  ...
  if (!TableChecked.load(std::memory_order_relaxed)) {
    assert(toolchain::is_sorted(NEONLdStTable) && "NEONLdStTable is not sorted!");
    TableChecked.store(true, std::memory_order_relaxed);
  }
#endif

  auto I = toolchain::lower_bound(NEONLdStTable, Opcode);
...
bool ARMExpandPseudo::ExpandMI(MachineBasicBlock &MBB,
  ...
  switch (Opcode) {
    ...
    case ARM::tBXNS_RET: {
      ...
      assert(toolchain::all_of(MBBI->operands(), [](const MachineOperand &Op) {
...
/// expansion pass.
FunctionPass *toolchain::createARMExpandPseudoPass() {
