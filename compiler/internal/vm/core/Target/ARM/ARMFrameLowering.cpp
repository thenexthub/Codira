...
static MachineBasicBlock::iterator insertSEH(MachineBasicBlock::iterator MBBI,
  ...
  case ARM::t2MOVi16: { // mov(w) r4, #xx
    ...
    if (!Wide) {
      ...
      for (MachineOperand &MO : toolchain::drop_begin(MBBI->operands()))
        NewInstr.add(MO);
  ...
  case ARM::VLDMDIA_UPD: {
    int First = -1, Last = 0;
    for (const MachineOperand &MO : toolchain::drop_begin(MBBI->operands(), 4)) {
      unsigned Reg = RegInfo->getSEHRegNum(MO.getReg());
      if (First == -1)
        First = Reg;
      Last = Reg;
...
struct StackAdjustingInsts {
  ...
  void addExtraBytes(const MachineBasicBlock::iterator I, unsigned ExtraBytes) {
    auto Info =
        toolchain::find_if(Insts, [&](InstInfo &Info) { return Info.I == I; });
...
void ARMFrameLowering::emitPushInst(MachineBasicBlock &MBB,
  ...
  while (i != 0) {
    ...
    for (; i != 0; --i) {
      ...
      // happens with the @toolchain-returnaddress intrinsic and with arguments
    ...
    toolchain::sort(Regs, [&](const RegAndKill &LHS, const RegAndKill &RHS) {
...
void ARMFrameLowering::emitPopInst(MachineBasicBlock &MBB,
  ...
  while (i != 0) {
    ...
    toolchain::sort(Regs, [&](unsigned LHS, unsigned RHS) {
...
void ARMFrameLowering::emitFPStatusSaves(MachineBasicBlock &MBB,
  ...
  auto RegPresent = [&CSI](MCRegister Reg) {
    return toolchain::any_of(CSI, [Reg](const CalleeSavedInfo &C) {
      return C.getReg() == Reg;
    });
  };
...
bool ARMFrameLowering::spillCalleeSavedRegisters(
  ...
  if (toolchain::any_of(CSI, [](const CalleeSavedInfo &C) {
    ...
    for (auto &MI : MBB) {
      ...
      for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
        ...
        case ARMII::AddrModeT2_i12:
          ...
          // i8 opcodes. See toolchain::rewriteT2FrameIndex.
  ...
  if (BigFrameOffsets ||
    ...
    if (HasFP || (isFPReserved(MF) && AFI->getNumAlignedDPRCS2Regs() > 0)) {
      ...
      if ((requiresAAPCSFrameRecord(MF) ||
        ...
        LRSpilled = true;
        NumGPRSpills++;
        auto LRPos = toolchain::find(UnspilledCS1GPRs, ARM::LR);
        if (LRPos != UnspilledCS1GPRs.end())
          UnspilledCS1GPRs.erase(LRPos);
      }
      auto FPPos = toolchain::find(UnspilledCS1GPRs, FramePtr);
    ...
    if (AFI->isThumb1OnlyFunction()) {
      ...
      // toolchain.returnaddress intrinsic is used. This is only worth doing if we
      ...
      for (; RegDeficit > 0 && !AvailableRegs.empty(); --RegDeficit) {
        ...
        if (Reg != ARM::LR && !MRI.isPhysRegUsed(Reg))
          NumExtraCSSpill++;
        UnspilledCS1GPRs.erase(toolchain::find(UnspilledCS1GPRs, Reg));
        if (Reg == ARM::LR)
          LRSpilled = true;
      }
    ...
    if (!LRSpilled && CS1Spilled && !ExpensiveLRRestore) {
      ...
      LRPos = toolchain::find(UnspilledCS1GPRs, (unsigned)ARM::LR);
      if (LRPos != UnspilledCS1GPRs.end())
        UnspilledCS1GPRs.erase(LRPos);
