...
The toolchain-gcc generated code looks like this:
...
entry:
	...
	call void %toolchain.memcpy.i32( sbyte* %0, sbyte* cast ({ double, int }* %C.0.904 to sbyte*), uint 12, uint 4 )
	cast %struct.s* %agg.result to sbyte*		; <sbyte*>:1 [#uses=2]
	call void %toolchain.memcpy.i32( sbyte* %1, sbyte* %0, uint 12, uint 0 )
	cast %struct.s* %memtmp to sbyte*		; <sbyte*>:2 [#uses=1]
	call void %toolchain.memcpy.i32( sbyte* %2, sbyte* %1, uint 12, uint 0 )
	ret void
}

llc ends up issuing two memcpy's (the first memcpy becomes 3 loads from
constantpool). Perhaps we should 1) fix toolchain-gcc so the memcpy is translated
...
It's very common to see toolchain generated assembly code that looks like this:
...
toolchain decides it's a good idea to turn the repeated if...else into a
...
does not put the tables in-line in the text.  This is more like the toolchain
...
http://lists.toolchain.org/pipermail/toolchain-dev/2009-June/022763.html
...
currently toolchain-gcc generates something like this (nice branchless code I'd say):
