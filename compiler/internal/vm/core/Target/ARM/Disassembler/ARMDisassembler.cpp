...
public:
  ...
  void setITState(char Firstcond, char Mask) {
    ...
    unsigned NumTZ = toolchain::countr_zero<uint8_t>(Mask);
...
private:
  ...
  toolchain::endianness InstructionEndianness;
};
...
static DecodeStatus DecodeT2SOImm(MCInst &Inst, unsigned Val, uint64_t Address,
  ...
  if (ctrl == 0) {
    ...
    unsigned imm = toolchain::rotr<uint32_t>(unrot, rot);
    Inst.addOperand(MCOperand::createImm(imm));
  }
...
uint64_t ARMDisassembler::suggestBytesToSkip(ArrayRef<uint8_t> Bytes,
  ...
  uint16_t Insn16 = toolchain::support::endian::read<uint16_t>(
...
DecodeStatus ARMDisassembler::getARMInstruction(MCInst &MI, uint64_t &Size,
  ...
  uint32_t Insn = toolchain::support::endian::read<uint32_t>(Bytes.data(),
...
DecodeStatus ARMDisassembler::getThumbInstruction(MCInst &MI, uint64_t &Size,
  ...
  uint32_t Insn32 =
      (uint32_t(Insn16) << 16) | toolchain::support::endian::read<uint16_t>(
