...
cl::opt<bool> toolchain::NoKernelInfoEndLTO(
...
uint64_t TargetMachine::getMaxCodeSize() const {
  switch (getCodeModel()) {
  case CodeModel::Tiny:
    return toolchain::maxUIntN(10);
  case CodeModel::Small:
  case CodeModel::Kernel:
  case CodeModel::Medium:
    return toolchain::maxUIntN(31);
  case CodeModel::Large:
    return toolchain::maxUIntN(64);
  }
...
bool TargetMachine::shouldAssumeDSOLocal(const GlobalValue *GV) const {
  ...
  // According to the toolchain language reference, we should be able to
