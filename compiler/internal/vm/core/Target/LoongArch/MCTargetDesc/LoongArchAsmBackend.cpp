...
LoongArchAsmBackend::LoongArchAsmBackend(const MCSubtargetInfo &STI,
    ...
    : MCAsmBackend(toolchain::endianness::little), STI(STI), OSABI(OSABI),
      Is64Bit(Is64Bit), TargetOptions(Options) {}

std::optional<MCFixupKind>
LoongArchAsmBackend::getFixupKind(StringRef Name) const {
  if (STI.getTargetTriple().isOSBinFormatELF()) {
    auto Type = toolchain::StringSwitch<unsigned>(Name)
...
static void reportOutOfRangeError(MCContext &Ctx, SMLoc Loc, unsigned N) {
  Ctx.reportError(Loc, "fixup value out of range [" + Twine(toolchain::minIntN(N)) +
                           ", " + Twine(toolchain::maxIntN(N)) + "]");
}
...
bool LoongArchAsmBackend::relaxDwarfLineAddr(MCFragment &F) const {
  ...
  if (Value > 60000) {
    ...
    support::endian::write<uint16_t>(OS, 0, toolchain::endianness::little);
  }
...
bool LoongArchAsmBackend::relaxDwarfCFA(MCFragment &F) const {
  ...
  if (isUIntN(6, Value)) {
    ...
    support::endian::write<uint8_t>(OS, 0, toolchain::endianness::little);
    ...
    support::endian::write<uint32_t>(OS, 0, toolchain::endianness::little);
...
bool LoongArchAsmBackend::addReloc(const MCFragment &F, const MCFixup &Fixup,
  ...
  if (Target.getSubSym()) {
    ...
    switch (Fixup.getKind()) {
    case toolchain::FK_Data_1:
      FK = getRelocPairForSize(8);
      break;
    case toolchain::FK_Data_2:
      FK = getRelocPairForSize(16);
      break;
    case toolchain::FK_Data_4:
      FK = getRelocPairForSize(32);
      break;
    case toolchain::FK_Data_8:
      FK = getRelocPairForSize(64);
      break;
    case toolchain::FK_Data_leb128:
...
MCAsmBackend *toolchain::createLoongArchAsmBackend(const Target &T,
