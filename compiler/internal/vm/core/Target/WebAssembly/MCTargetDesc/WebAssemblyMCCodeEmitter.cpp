...
MCCodeEmitter *toolchain::createWebAssemblyMCCodeEmitter(const MCInstrInfo &MCII,
                                                    MCContext &Ctx) {
...
void WebAssemblyMCCodeEmitter::encodeInstruction(
  ...
  for (unsigned I = 0, E = MI.getNumOperands(); I < E; ++I) {
    ...
    if (MO.isReg()) {
      ...
      support::endian::write<uint32_t>(OS, F, toolchain::endianness::little);
    } else if (MO.isDFPImm()) {
      uint64_t D = MO.getDFPImm();
      support::endian::write<uint64_t>(OS, D, toolchain::endianness::little);
    } else if (MO.isExpr()) {
      toolchain::MCFixupKind FixupKind;
      size_t PaddedSize = 5;
      if (I < Desc.getNumOperands()) {
        const MCOperandInfo &Info = Desc.operands()[I];
        switch (Info.OperandType) {
