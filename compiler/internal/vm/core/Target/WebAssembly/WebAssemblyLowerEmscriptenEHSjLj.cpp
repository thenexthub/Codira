...
///
/// 6) Lower
///      call @toolchain.eh.typeid.for(type) (intrinsic)
///    into
...
class WebAssemblyLowerEmscriptenEHSjLj final : public ModulePass {
  ...
  Function *EHTypeIDF = nullptr;          // toolchain.eh.typeid.for() (intrinsic)
...
ModulePass *toolchain::createWebAssemblyLowerEmscriptenEHSjLj() {
...
static void markAsImported(Function *F) {
  ...
  if (!F->hasFnAttribute("wasm-import-module")) {
    toolchain::AttrBuilder B(F->getParent()->getContext());
...
bool WebAssemblyLowerEmscriptenEHSjLj::runEHOnFunction(Function &F) {
  ...
  // Process toolchain.eh.typeid.for intrinsics
  for (BasicBlock &BB : F) {
    for (Instruction &I : BB) {
  ...
  for (LandingPadInst *LPI : LandingPads) {
    ...
    for (unsigned I = 0, E = LPI->getNumClauses(); I < E; ++I) {
      ...
      // https://github.com/toolchain/toolchain-project/issues/49740
      if (LPI->isCatch(I))
