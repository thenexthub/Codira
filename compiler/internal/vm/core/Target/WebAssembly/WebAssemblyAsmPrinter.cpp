...
void WebAssemblyAsmPrinter::EmitProducerInfo(Module &M) {
  toolchain::SmallVector<std::pair<std::string, std::string>, 4> Languages;
  if (const NamedMDNode *Debug = M.getNamedMetadata("toolchain.dbg.cu")) {
    toolchain::SmallSet<StringRef, 4> SeenLanguages;
  ...
  toolchain::SmallVector<std::pair<std::string, std::string>, 4> Tools;
  if (const NamedMDNode *Ident = M.getNamedMetadata("toolchain.ident")) {
    toolchain::SmallSet<StringRef, 4> SeenTools;
...
void WebAssemblyAsmPrinter::EmitFunctionAttributes(Module &M) {
  auto V = M.getNamedGlobal("toolchain.global.annotations");
  if (!V)
    return;
  ...
  for (const auto &[Name, Symbols] : CustomSections) {
    MCSectionWasm *CustomSection = OutContext.getWasmSection(
        ".custom_section.toolchain.func_attr.annotate." + Name, SectionKind::getMetadata());
    OutStreamer->pushSection();
