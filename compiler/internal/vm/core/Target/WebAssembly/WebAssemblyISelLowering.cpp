...
SDValue WebAssemblyTargetLowering::LowerOperation(SDValue Op,
  ...
  case ISD::CLEAR_CACHE:
    report_fatal_error("toolchain.clear_cache is not supported on wasm");
  case ISD::SMUL_LOHI:
  case ISD::UMUL_LOHI:
...
SDValue WebAssemblyTargetLowering::LowerBUILD_VECTOR(SDValue Op,
  ...
  auto AddCount = [](auto &Counts, const auto &Val) {
    auto CountIt =
        toolchain::find_if(Counts, [&Val](auto E) { return E.first == Val; });
  ...
  auto GetMostCommon = [](auto &Counts) {
    auto CommonIt = toolchain::max_element(Counts, toolchain::less_second());
  ...
  if (ShuffleCounts.size()) {
    ...
    toolchain::erase_if(ShuffleCounts,
