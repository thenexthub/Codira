...
static bool BBHasFallthrough(MachineBasicBlock *MBB) {
  ...
  return toolchain::is_contained(MBB->successors(), NextBB);
}
...
void MipsConstantIslands::updateForInsertedWaterBlock
  ...
  water_iterator IP = toolchain::lower_bound(WaterList, NewBB, CompareMBBNumbers);
...
MipsConstantIslands::splitBlockBeforeInstr(MachineInstr &MI) {
  ...
  water_iterator IP = toolchain::lower_bound(WaterList, OrigBB, CompareMBBNumbers);
...
bool MipsConstantIslands::handleConstantPoolUser(unsigned CPUserIndex) {
  ...
  if (findAvailableWater(U, UserOffset, IP)) {
    ...
    IP = toolchain::find(WaterList, WaterBB);
    if (IP != WaterList.end())
...
FunctionPass *toolchain::createMipsConstantIslandPass() {
