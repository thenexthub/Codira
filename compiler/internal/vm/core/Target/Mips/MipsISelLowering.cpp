...
SDValue MipsTargetLowering::lowerVAARG(SDValue Op, SelectionDAG &DAG) const {
  ...
  const Align Align =
      toolchain::MaybeAlign(Node->getConstantOperandVal(3)).valueOrOne();
...
// An EH_RETURN is the result of lowering toolchain.eh.return which in turn is
...
SDValue MipsTargetLowering::getPICODEumpTableRelocBase(SDValue Table,
                                                     SelectionDAG &DAG) const {
  if (!isPositionIndependent())
    return Table;
