...
static Value *expandAtan2Intrinsic(CallInst *Orig) {
  ...
  Constant *Pi = ConstantFP::get(Ty, toolchain::numbers::pi);
  Constant *HalfPi = ConstantFP::get(Ty, toolchain::numbers::pi / 2);
  Constant *NegHalfPi = ConstantFP::get(Ty, -toolchain::numbers::pi / 2);
...
static Value *expandFunnelShiftIntrinsic(CallInst *Orig) {
  ...
  assert(toolchain::isPowerOf2_32(BitWidth) &&
...
static Value *expandRadiansIntrinsic(CallInst *Orig) {
  ...
  Type *Ty = X->getType();
  IRBuilder<> Builder(Orig);
  Value *PiOver180 = ConstantFP::get(Ty, toolchain::numbers::pi / 180.0);
...
static bool expandBufferStoreIntrinsic(CallInst *Orig, bool IsRaw) {
  ...
  if (IsDouble) {
    auto *SplitTy = toolchain::StructType::get(SplitElementTy, SplitElementTy);
...
static Value *expandDegreesIntrinsic(CallInst *Orig) {
  ...
  Type *Ty = X->getType();
  IRBuilder<> Builder(Orig);
  Value *DegreesRatio = ConstantFP::get(Ty, 180.0 * toolchain::numbers::inv_pi);
...
ModulePass *toolchain::createDXILIntrinsicExpansionLegacyPass() {
