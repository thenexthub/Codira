...
static void createRawStore(IntrinsicInst *II, StoreInst *SI,
  ...
  if (!RTI.isStruct()) {
    ...
    Offset = toolchain::PoisonValue::get(Builder.getInt32Ty());
  }
...
static SmallVector<Instruction *> collectBlockUseDef(Instruction *Start) {
  ...
  toolchain::sort(Out, [&](Instruction *A, Instruction *B) {
...
static void phiNodeReplacement(IntrinsicInst *II,
  ...
  for (Instruction *Dead : toolchain::reverse(CurrBBDeadInsts))
...
static void replaceAccess(IntrinsicInst *II, dxil::ResourceTypeInfo &RTI) {
  ...
  // Traverse the now-dead instructions in RPO and remove them.
  for (Instruction *Dead : toolchain::reverse(DeadInsts))
    Dead->eraseFromParent();
  II->eraseFromParent();
}
...
FunctionPass *toolchain::createDXILResourceAccessLegacyPass() {
