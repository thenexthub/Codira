...
static void reportOverlappingRegisters(Module &M, const toolchain::hlsl::Binding &R1,
                                       const toolchain::hlsl::Binding &R2) {
  SmallString<128> Message;
...
reportRegNotBound(Module &M, ResourceClass Class,
                  const toolchain::dxil::ResourceInfo::ResourceBinding &Unbound) {
  SmallString<128> Message;
  raw_svector_ostream OS(Message);
...
tripleToVisibility(toolchain::Triple::EnvironmentType ET) {
  switch (ET) {
  case Triple::Pixel:
...
static void validateRootSignature(Module &M,
  ...
  Builder.calculateBindingInfo(
      [&M](const toolchain::hlsl::BindingInfoBuilder &Builder,
           const toolchain::hlsl::Binding &ReportedBinding) {
        const toolchain::hlsl::Binding &Overlaping =
  ...
  for (const ResourceInfo &RI : DRM) {
    ...
    const toolchain::hlsl::Binding *Reg =
...
ModulePass *toolchain::createDXILPostOptimizationValidationLegacyPass() {
