...
public:
  ...
  replaceFunction(Function &F,
                  toolchain::function_ref<Error(CallInst *CI)> ReplaceCall) {
...
#include "DXILOperation.inc"
  ...
  void cleanupHandleCasts() {
    ...
    toolchain::sort(CastFns);
    CastFns.erase(toolchain::unique(CastFns), CastFns.end());
  ...
  void replaceHandleFromBindingCall(CallInst *CI, Value *Replacement) {
    ...
    auto *NameGlobal = dyn_cast<toolchain::GlobalVariable>(CI->getArgOperand(4));
  ...
  bool hasNonUniformIndex(Value *IndexOp) {
    if (isa<toolchain::Constant>(IndexOp))
      return false;
    ...
    while (!WorkList.empty()) {
      ...
      if (auto *U = toolchain::dyn_cast<toolchain::User>(V)) {
        for (toolchain::Value *Op : U->operands()) {
          if (isa<toolchain::Constant>(Op))
            continue;
  ...
  [[nodiscard]] bool lowerToCreateHandle(Function &F) {
    ...
    return replaceFunction(F, [&](CallInst *CI) -> Error {
      ...
      std::array<Value *, 4> Args{
          ConstantInt::get(Int8Ty, toolchain::to_underlying(RC)),
  ...
  bool lowerIntrinsics() {
    ...
    for (Function &F : make_early_inc_range(M.functions())) {
      ...
      // NOTE: toolchain.dbg.value is supported as is in DXIL.
...
#include "DXILOperation.inc"
      ...
      case Intrinsic::dx_resource_nonuniformindex:
        assert(!CleanupNURI &&
               "overloaded toolchain.dx.resource.nonuniformindex intrinsics?");
        CleanupNURI = &F;
        break;
...
ModulePass *toolchain::createDXILOpLoweringLegacyPass() {
