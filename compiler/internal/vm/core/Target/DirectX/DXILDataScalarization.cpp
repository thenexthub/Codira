...
public:
  ...
  friend bool findAndReplaceVectors(toolchain::Module &M);
    ...
    if (ConstantVector *ConstVecInit = dyn_cast<ConstantVector>(Init)) {
      for (unsigned I = 0; I < ConstVecInit->getNumOperands(); ++I)
        ArrayElements.push_back(ConstVecInit->getOperand(I));
                   ...
                   toolchain::dyn_cast<toolchain::ConstantDataVector>(Init)) {
...
ModulePass *toolchain::createDXILDataScalarizationLegacyPass() {
