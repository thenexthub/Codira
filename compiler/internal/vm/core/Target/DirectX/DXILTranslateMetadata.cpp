...
static void reportLoopError(Module &M, Twine Message,
                            DiagnosticSeverity Severity = DS_Error) {
  reportError(M, Twine("Invalid \"toolchain.loop\" metadata: ") + Message, Severity);
}
...
static uint32_t getShaderStage(Triple::EnvironmentType Env) {
  return (uint32_t)Env - (uint32_t)toolchain::Triple::Pixel;
}
...
static MDTuple *getEntryPropAsMetadata(Module &M, const EntryProperties &EP,
  ...
  if (EP.Entry != nullptr) {
    ...
    // See https://github.com/toolchain/toolchain-project/issues/57948.
...
static void translateBranchMetadata(Module &M, Instruction *BBTerminatorInst) {
  ...
  toolchain::Metadata *HintsStr = MDHelper.createString("dx.controlflow.hints");
  toolchain::Metadata *HintsValue = MDHelper.createConstant(
      mdconst::extract<ConstantInt>(HlslControlFlowMD->getOperand(1)));

  MDNode *MDNode = toolchain::MDNode::get(M.getContext(), {HintsStr, HintsValue});
...
static bool isLoopMDCompatible(Module &M, Metadata *MD) {
  ...
  std::array<StringLiteral, 3> ValidHintNames = {"toolchain.loop.unroll.count",
                                                 "toolchain.loop.unroll.disable",
                                                 "toolchain.loop.unroll.full"};
  ...
  if (!toolchain::is_contained(ValidHintNames, HintStr->getString()))
    return false;
  ...
  if (HintStr->getString() == "toolchain.loop.unroll.count") {
    if (!ValidCountNode(HintMD)) {
      reportLoopError(M, "\"toolchain.loop.unroll.count\" must have 2 operands and "
                         "the second must be a constant integer");
      return false;
    }
  } else if (HintMD->getNumOperands() != 1) {
    reportLoopError(
        M, "\"toolchain.loop.unroll.disable\" and \"toolchain.loop.unroll.full\" "
...
static void translateLoopMetadata(Module &M, Instruction *I, MDNode *BaseMD) {
  ...
  if (BaseMD->getNumOperands() == 0 || !IsDistinctNode(BaseMD))
    return I->setMetadata("toolchain.loop", nullptr);
  ...
  // !0 = !{!0, !1}
  // !1 = !{!1, !2}
  // !2 = !{!"toolchain.loop.unroll.disable"}
  //
  ...
  toolchain::TempMDTuple TempNode = toolchain::MDNode::getTemporary(M.getContext(), {});
  ...
  I->setMetadata("toolchain.loop", CompatibleLoopMD);
}

using InstructionMDList = std::array<unsigned, 7>;

static InstructionMDList getCompatibleInstructionMDs(toolchain::Module &M) {
  return {
      M.getMDKindID("dx.nonuniform"),    M.getMDKindID("dx.controlflow.hints"),
      M.getMDKindID("dx.precise"),       toolchain::LLVMContext::MD_range,
      toolchain::LLVMContext::MD_alias_scope, toolchain::LLVMContext::MD_noalias,
      M.getMDKindID("toolchain.loop")};
}
...
static void translateInstructionMetadata(Module &M) {
  ...
  unsigned char MDLoopKind = M.getContext().getMDKindID("toolchain.loop");
...
static void cleanModuleFlags(Module &M) {
  ...
  SmallVector<toolchain::Module::ModuleFlagEntry> FlagEntries;
  M.getModuleFlagsMetadata(FlagEntries);
  bool Updated = false;
  for (auto &Flag : FlagEntries) {
    // toolchain 3.7 only supports behavior up to AppendUnique.
...
static GlobalMDList CompatibleNamedModuleMDs = {
    "toolchain.ident",     "toolchain.module.flags", "dx.resources",   "dx.valver",
...
static void translateGlobalMetadata(Module &M, DXILResourceMap &DRM,
  ...
  // See https://github.com/toolchain/toolchain-project/issues/57928
  ...
  for (NamedMDNode &NamedMD : M.named_metadata())
    if (!NamedMD.getName().starts_with("toolchain.dbg.") &&
        !toolchain::is_contained(CompatibleNamedModuleMDs, NamedMD.getName()))
...
ModulePass *toolchain::createDXILTranslateMetadataLegacyPass() {
