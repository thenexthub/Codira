...
namespace {
class DXContainerGlobals : public toolchain::ModulePass {
...
bool DXContainerGlobals::runOnModule(Module &M) {
  toolchain::SmallVector<GlobalValue *> Globals;
...
void DXContainerGlobals::addSignature(Module &M,
  ...
  //  see issue https://github.com/toolchain/toolchain-project/issues/90504.
...
void DXContainerGlobals::addRootSignature(Module &M,
  ...
  if (MMI.ShaderProfile == toolchain::Triple::Library)
    return;

  auto &RSA = getAnalysis<RootSignatureAnalysisWrapper>().getRSInfo();
  const Function *EntryFunction = nullptr;

  if (MMI.ShaderProfile != toolchain::Triple::RootSignature) {
...
void DXContainerGlobals::addResourcesForPSV(Module &M, PSVRuntimeInfo &PSV) {
  ...
  for (const dxil::ResourceInfo &RI : DRM.uavs()) {
    ...
    // with https://github.com/toolchain/toolchain-project/issues/104392
    Flags.Flags = 0u;
...
void DXContainerGlobals::addPipelineStateValidationInfo(
  ...
  //
  // See issue https://github.com/toolchain/toolchain-project/issues/96674.
...
ModulePass *toolchain::createDXContainerGlobalsPass() {
