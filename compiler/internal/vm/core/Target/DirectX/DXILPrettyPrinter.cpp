...
struct FormatResourceDimension
    : public toolchain::FormatAdapter<const dxil::ResourceTypeInfo &> {
  FormatResourceDimension(const dxil::ResourceTypeInfo &RI, bool HasCounter)
      : toolchain::FormatAdapter<const dxil::ResourceTypeInfo &>(RI),
        HasCounter(HasCounter) {}

  bool HasCounter;

  void format(toolchain::raw_ostream &OS, StringRef Style) override {
...
struct FormatBindingID
    : public toolchain::FormatAdapter<const dxil::ResourceInfo &> {
  dxil::ResourceClass RC;

  explicit FormatBindingID(const dxil::ResourceInfo &RI,
                           const dxil::ResourceTypeInfo &RTI)
      : toolchain::FormatAdapter<const dxil::ResourceInfo &>(RI),
...
namespace {
class DXILPrettyPrinterLegacy : public toolchain::ModulePass {
...
ModulePass *toolchain::createDXILPrettyPrinterLegacyPass(raw_ostream &OS) {
