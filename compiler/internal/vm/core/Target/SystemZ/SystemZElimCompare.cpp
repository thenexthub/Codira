...
bool SystemZElimCompare::optimizeCompareZero(
  ...
  auto MIRange = toolchain::make_range(
      std::next(MachineBasicBlock::iterator(&Compare)), MBB.end());
  for (MachineInstr &MI : toolchain::make_early_inc_range(MIRange)) {
...
FunctionPass *toolchain::createSystemZElimComparePass(SystemZTargetMachine &TM) {
