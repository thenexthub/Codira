...
bool SystemZVectorConstantInfo::isVectorConstantLegal(
  ...
  unsigned LowerBits = toolchain::countr_zero(SplatBitsZ);
  unsigned UpperBits = toolchain::countl_zero(SplatBitsZ);
...
SystemZTargetLowering::emitEHSjLjLongJmp(MachineInstr &MI,
  ...
  // We are restoring R13 even though we never stored in setjmp from toolchain,
  // as gcc always stores R13 in builtin_setjmp. We could have mixed code
  // gcc setjmp and toolchain longjmp.
...
static unsigned getTestUnderMaskCond(unsigned BitSize, unsigned CCMask,
  ...
  uint64_t High = toolchain::bit_floor(Mask);
  uint64_t Low = uint64_t(1) << toolchain::countr_zero(Mask);
...
SDValue SystemZTargetLowering::lowerCTPOP(SDValue Op,
  ...
  int64_t BitSize = toolchain::bit_ceil(NumSignificantBits);
...
SDValue SystemZTargetLowering::combineSTORE(
  ...
  if (Subtarget.hasVector() && DCI.Level == BeforeLegalizeTypes &&
    ...
    auto FindReplicatedImm = [&](ConstantSDNode *C, unsigned TotBytes) {
      ...
      if (!toolchain::isUIntN(TotBytes * 8, Val.getZExtValue())) {
...
SystemZTargetLowering::getJumpConditionMergingParams(Instruction::BinaryOps Opc,
  ...
  const auto isFlagOutOpCC = [](const Value *V) {
    ...
    if (const auto *I = dyn_cast<Instruction>(V)) {
      // PatternMatch.h provides concise tree-based pattern match of toolchain IR.
...
SystemZTargetLowering::emitSelect(MachineInstr &MI,
  ...
  unsigned Count = 0;
  for (MachineInstr &NextMI : toolchain::make_range(
           std::next(MachineBasicBlock::iterator(MI)), MBB->end())) {
