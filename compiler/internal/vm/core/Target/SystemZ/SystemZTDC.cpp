...
// 7: icmp ne/eq (call @toolchain.s390.tdc.*(X, mask)) -> tdc X, mask/~mask
...
FunctionPass *toolchain::createSystemZTDCPass() {
...
void SystemZTDCPass::convertICmp(CmpInst &I) {
  ...
  if (auto *Cast = dyn_cast<BitCastInst>(Op0)) {
    ...
    if (Pred == CmpInst::ICMP_NE) {
      // icmp ne (call toolchain.s390.tdc(...)), 0 -> simple TDC
    } else if (Pred == CmpInst::ICMP_EQ) {
      // icmp eq (call toolchain.s390.tdc(...)), 0 -> TDC with inverted mask
      Mask ^= SystemZ::TDCMASK_ALL;
    } else {
