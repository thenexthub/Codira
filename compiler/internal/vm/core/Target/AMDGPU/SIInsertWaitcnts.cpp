...
char &toolchain::SIInsertWaitcntsID = SIInsertWaitcntsLegacy::ID;

FunctionPass *toolchain::createSIInsertWaitcntsPass() {
  ...
  if (TII->isDS(Inst) && TII->usesLGKM_CNT(Inst)) {
    ...
    if (!SIInstrInfo::isLDSDMA(Inst) && FlatASCount > 1)
      ScoreBrackets->setPendingFlat();
             ...
             !toolchain::AMDGPU::getMUBUFIsBufferInv(Inst.getOpcode())) {
    IsVMEMAccess = true;
    ScoreBrackets->updateByEvent(getVmemWaitEventType(Inst), Inst);
