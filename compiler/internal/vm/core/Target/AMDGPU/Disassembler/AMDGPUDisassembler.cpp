...
#include "vm/core/Support/AMDHSAKernelDescriptor.h"
...
using DecodeStatus = toolchain::MCDisassembler::DecodeStatus;
...
template <typename T> static inline T eatBytes(ArrayRef<uint8_t>& Bytes) {
  ...
  const auto Res =
      support::endian::read<T, toolchain::endianness::little>(Bytes.data());
...
void AMDGPUDisassembler::convertMIMGInst(MCInst &MI) const {
  ...
  unsigned DstSize = IsGather4 ? 4 : std::max(toolchain::popcount(DMask), 1);
...
static int64_t getInlineImmVal32(unsigned Imm) {
  switch (Imm) {
  case 240:
    return toolchain::bit_cast<uint32_t>(0.5f);
  case 241:
    return toolchain::bit_cast<uint32_t>(-0.5f);
  case 242:
    return toolchain::bit_cast<uint32_t>(1.0f);
  case 243:
    return toolchain::bit_cast<uint32_t>(-1.0f);
  case 244:
    return toolchain::bit_cast<uint32_t>(2.0f);
  case 245:
    return toolchain::bit_cast<uint32_t>(-2.0f);
  case 246:
    return toolchain::bit_cast<uint32_t>(4.0f);
  case 247:
    return toolchain::bit_cast<uint32_t>(-4.0f);
  case 248: // 1 / (2 * PI)
    return 0x3e22f983;
...
static int64_t getInlineImmVal64(unsigned Imm) {
  switch (Imm) {
  case 240:
    return toolchain::bit_cast<uint64_t>(0.5);
  case 241:
    return toolchain::bit_cast<uint64_t>(-0.5);
  case 242:
    return toolchain::bit_cast<uint64_t>(1.0);
  case 243:
    return toolchain::bit_cast<uint64_t>(-1.0);
  case 244:
    return toolchain::bit_cast<uint64_t>(2.0);
  case 245:
    return toolchain::bit_cast<uint64_t>(-2.0);
  case 246:
    return toolchain::bit_cast<uint64_t>(4.0);
  case 247:
    return toolchain::bit_cast<uint64_t>(-4.0);
  case 248: // 1 / (2 * PI)
...
/// tables that describe these bits in toolchain.org/docs/AMDGPUUsage.html.
static SmallString<32> getBitRangeFromMask(uint32_t Mask, unsigned BaseBytes) {
  SmallString<32> Result;
  raw_svector_ostream S(Result);

  int TrailingZeros = toolchain::countr_zero(Mask);
  int PopCount = toolchain::popcount(Mask);
...
static Error createReservedKDBytesError(unsigned BaseInBytes,
  ...
  // table here: https://toolchain.org/docs/AMDGPUUsage.html#kernel-descriptor .
  return createStringError(
      std::errc::invalid_argument,
...
bool AMDGPUSymbolizer::tryAddingSymbolicOperand(
  ...
  auto Result = toolchain::find_if(*Symbols, [Value](const SymbolInfoTy &Val) {
