...
void SIFrameLowering::emitEntryFunctionPrologue(MachineFunction &MF,
  ...
  if (!mayReserveScratchForCWSR(MF)) {
    ...
    Register FPReg = MFI->getFrameOffsetReg();
    assert(FPReg != AMDGPU::FP_REG);
    unsigned VGPRSize = toolchain::alignTo(
...
void SIFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  // convention and it doesn't contain any calls to toolchain.amdgcn.cs.chain, then
...
static void assignSlotsUsingVGPRBlocks(MachineFunction &MF,
  ...
  for (auto CSIt = CSI.begin(); CSIt != CSEnd; ++CSIt) {
    ...
    if (!RegBlock) {
      ...
      assert(RegDelta > 0 && toolchain::countl_zero(Mask) >= RegDelta &&
             "Bad shift amount");
      Mask <<= RegDelta;
    }
    ...
    int UnusedBits = toolchain::countl_zero(Mask);
