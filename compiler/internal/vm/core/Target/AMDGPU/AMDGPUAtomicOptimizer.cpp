...
///   of Wavefront using toolchain.cttz and performs scan  using readlane & writelane
///   intrinsics
//===----------------------------------------------------------------------===//
...
char &toolchain::AMDGPUAtomicOptimizerID = AMDGPUAtomicOptimizer::ID;
...
std::pair<Value *, Value *> AMDGPUAtomicOptimizerImpl::buildScanIteratively(
  ...
  // Use toolchain.cttz intrinsic to find the lowest remaining active lane.
  auto *FF1 =
  ...
  // Set bit to zero of current active lane so that for next iteration toolchain.cttz
...
static Constant *getIdentityValueForAtomicOp(Type *const Ty,
  ...
  case AtomicRMWInst::FMin:
  case AtomicRMWInst::FMax:
    // FIXME: atomicrmw fmax/fmin behave like toolchain.maxnum/minnum so NaN is the
...
FunctionPass *toolchain::createAMDGPUAtomicOptimizerPass(ScanOptions ScanStrategy) {
