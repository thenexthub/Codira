...
char &toolchain::SIOptimizeExecMaskingPreRAID = SIOptimizeExecMaskingPreRALegacy::ID;

FunctionPass *toolchain::createSIOptimizeExecMaskingPreRAPass() {
...
bool SIOptimizeExecMaskingPreRA::optimizeVcndVcmpPair(MachineBasicBlock &MBB) {
  auto I = toolchain::find_if(MBB.terminators(), [](const MachineInstr &MI) {
  ...
  if (toolchain::any_of(SelLI->vnis(),
                    [](const VNInfo *VNI) {
                      return VNI->isPHIDef();
                    }))
    return false;
...
bool SIOptimizeExecMaskingPreRA::optimizeElseBranch(MachineBasicBlock &MBB) {
  ...
  auto I = toolchain::find_if(MBB.terminators(), [this](const MachineInstr &MI) {
