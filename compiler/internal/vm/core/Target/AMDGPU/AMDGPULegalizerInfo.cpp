        ...
        .fewerElementsIf(
            ...
            [=](const LegalityQuery &Query) -> std::pair<unsigned, LLT> {
              ...
              if (!isPowerOf2_32(DstSize)) {
                ...
                unsigned FloorSize = toolchain::bit_floor(DstSize);
                return std::pair(
                    0, LLT::scalarOrVector(
    ...
    auto notValidElt = [=](const LegalityQuery &Query, unsigned TypeIdx) {
      ...
      if (Ty.isVector()) {
        ...
        if (EltTy.getSizeInBits() < 8 || EltTy.getSizeInBits() > 512)
          return true;
        if (!toolchain::has_single_bit<uint32_t>(EltTy.getSizeInBits()))
          return true;
      }
      return false;
    };
...
bool AMDGPULegalizerInfo::legalizeAddrSpaceCast(
  ...
  // G_INTRINSIC @toolchain.amdgcn.addrspacecast.nonnull
  ...
  if (SrcAS == AMDGPUAS::FLAT_ADDRESS &&
    ...
    // For toolchain.amdgcn.addrspacecast.nonnull we can always assume non-null, for
...
bool AMDGPULegalizerInfo::legalizeFPTOI(MachineInstr &MI,
  ...
  if (SrcLT == S64) {
    K0 = B.buildFConstant(
        S64, toolchain::bit_cast<double>(UINT64_C(/*2^-32*/ 0x3df0000000000000)));
    K1 = B.buildFConstant(
        S64, toolchain::bit_cast<double>(UINT64_C(/*-2^32*/ 0xc1f0000000000000)));
  } else {
    K0 = B.buildFConstant(
        S32, toolchain::bit_cast<float>(UINT32_C(/*2^-32*/ 0x2f800000)));
    K1 = B.buildFConstant(
        S32, toolchain::bit_cast<float>(UINT32_C(/*-2^32*/ 0xcf800000)));
  }
...
bool AMDGPULegalizerInfo::legalizeFFloor(MachineInstr &MI,
  ...
  auto Const =
      B.buildFConstant(F64, toolchain::bit_cast<double>(0x3fefffffffffffff));
...
void AMDGPULegalizerInfo::buildLoadInputValue(Register DstReg,
  ...
  if (Arg->isMasked()) {
    ...
    const unsigned Shift = toolchain::countr_zero<unsigned>(Mask);
...
bool AMDGPULegalizerInfo::legalizeWorkitemIDIntrinsic(
  ...
  if (Arg->isMasked()) {
    ...
    if (!loadInputValue(TmpReg, B, ArgType))
      return false;
    B.buildAssertZExt(DstReg, TmpReg, toolchain::bit_width(MaxID));
  }
...
void AMDGPULegalizerInfo::legalizeUnsignedDIV_REM32Impl(MachineIRBuilder &B,
  ...
  auto Scale = B.buildFConstant(S32, toolchain::bit_cast<float>(0x4f7ffffe));
...
static std::pair<Register, Register> emitReciprocalU64(MachineIRBuilder &B,
  ...
  auto Mad = B.buildFMAD(
      S32, CvtHi, // 2**32
      B.buildFConstant(S32, toolchain::bit_cast<float>(0x4f800000)), CvtLo);

  auto Rcp = B.buildInstr(AMDGPU::G_AMDGPU_RCP_IFLAG, {S32}, {Mad});
  auto Mul1 = B.buildFMul(
      S32, Rcp, B.buildFConstant(S32, toolchain::bit_cast<float>(0x5f7ffffc)));

  // 2**(-32)
  auto Mul2 = B.buildFMul(
      S32, Mul1, B.buildFConstant(S32, toolchain::bit_cast<float>(0x2f800000)));
  auto Trunc = B.buildIntrinsicTrunc(S32, Mul2);

  // -(2**32)
  auto Mad2 = B.buildFMAD(
      S32, Trunc, B.buildFConstant(S32, toolchain::bit_cast<float>(0xcf800000)),
      Mul1);
...
// Expand toolchain.amdgcn.rsq.clamp on targets that don't support the instruction.
...
bool AMDGPULegalizerInfo::legalizeImageIntrinsic(
  ...
  if (!BaseOpcode->Atomic) {
    ...
    if (BaseOpcode->Gather4) {
      ...
      DMaskLanes = toolchain::popcount(DMask);
...
bool AMDGPULegalizerInfo::legalizeTrapHsaQueuePtr(
  ...
  // Reference: https://toolchain.org/docs/AMDGPUUsage.html#trap-handler-abi
  Register LiveIn =
