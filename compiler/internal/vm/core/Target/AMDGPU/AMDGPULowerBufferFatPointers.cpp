...
//   %a = call @toolchain.amdgcn.raw.ptr.buffer.load(ptr addrspace(8) %x.rsrc, i32
//     %x.off, ...)
//   %b = call @toolchain.amdgcn.raw.ptr.buffer.load(ptr addrspace(8)
//     %x.rsrc, i32 %y.off, ...)
// ```
...
bool StoreFatPtrsAsIntsAndExpandMemcpyVisitor::visitMemCpyInst(
  ...
  if (MCI.getSourceAddressSpace() != AMDGPUAS::BUFFER_FAT_POINTER &&
    ...
    return false;
  toolchain::expandMemCpyAsLoop(&MCI,
...
bool StoreFatPtrsAsIntsAndExpandMemcpyVisitor::visitMemSetInst(
  ...
  if (MSI.getDestAddressSpace() != AMDGPUAS::BUFFER_FAT_POINTER)
    return false;
  toolchain::expandMemSetAsLoop(&MSI);
  MSI.eraseFromParent();
  return true;
}
...
bool StoreFatPtrsAsIntsAndExpandMemcpyVisitor::visitMemSetPatternInst(
  ...
  if (MSPI.getDestAddressSpace() != AMDGPUAS::BUFFER_FAT_POINTER)
    return false;
  toolchain::expandMemSetPatternAsLoop(&MSPI);
...
Value *LegalizeBufferContentTypesVisitor::extractSlice(Value *Vec, VecSlice S,
  ...
  SmallVector<int> Mask = toolchain::to_vector(
      toolchain::iota_range<int>(S.Index, S.Index + S.Length, /*Inclusive=*/false));
...
Value *LegalizeBufferContentTypesVisitor::insertSlice(Value *Whole, Value *Part,
  ...
  for (auto [I, E] : toolchain::enumerate(
  ...
  SmallVector<int> Mask =
      toolchain::to_vector(toolchain::iota_range<int>(0, NumElems, /*Inclusive=*/false));
  for (auto [I, E] :
       toolchain::enumerate(MutableArrayRef<int>(Mask).slice(S.Index, S.Length)))
    E = I + NumElems;
...
bool LegalizeBufferContentTypesVisitor::visitLoadImpl(
  ...
  if (auto *ST = dyn_cast<StructType>(PartType)) {
    ...
    for (auto [I, ElemTy, Offset] :
         toolchain::enumerate(ST->elements(), Layout->getMemberOffsets())) {
      AggIdxs.push_back(I);
  ...
  if (auto *AT = dyn_cast<ArrayType>(PartType)) {
    ...
    if (!ElemTy->isSingleValueType() || !DL.typeSizeEqualsStoreSize(ElemTy) ||
      ...
      bool Changed = false;
      for (auto I : toolchain::iota_range<uint32_t>(0, AT->getNumElements(),
                                               /*Inclusive=*/false)) {
        AggIdxs.push_back(I);
        Changed |= visitLoadImpl(OrigLI, ElemTy, AggIdxs,
  ...
  for (Instruction *I : Conditionals) {
    ...
    if (auto *PHI = dyn_cast<PHINode>(I)) {
      ...
      if (MaybeRsrc) {
        ...
        for (auto [V, BB] : toolchain::zip(PHI->incoming_values(), PHI->blocks())) {
...
void SplitPtrStructs::processFunction(Function &F) {
  ...
  SmallVector<Instruction *, 0> Originals(
      toolchain::make_pointer_range(instructions(F)));
...
char &toolchain::AMDGPULowerBufferFatPointersID = AMDGPULowerBufferFatPointers::ID;
...
ModulePass *toolchain::createAMDGPULowerBufferFatPointersPass() {
