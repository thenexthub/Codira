...
#ifndef NDEBUG
raw_ostream &toolchain::operator<<(raw_ostream &OS, const GCNSchedStageID &StageID) {
  switch (StageID) {
  case GCNSchedStageID::OccInitialSchedule:
...
bool GCNSchedStage::initGCNRegion() {
  ...
  LLVM_DEBUG(
      dbgs() << "Pressure before scheduling:\nRegion live-ins:"
             ...
             << print(toolchain::getRegPressure(DAG.MRI, DAG.LiveIns[RegionIdx]))
  ...
  for (unsigned I = 0, E = DAG.Regions.size(); I != E; ++I) {
    ...
    for (auto MI = Region.first; MI != Region.second; ++MI) {
      ...
      if (Rematerializations.contains(UseMI) ||
          toolchain::any_of(DefMI.operands(), [&RematRegs](MachineOperand &MO) {
