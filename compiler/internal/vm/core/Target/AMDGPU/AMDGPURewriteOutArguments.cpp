...
Type *AMDGPURewriteOutArguments::getStoredType(Value &Arg) const {
  ...
  SmallVector<Use *> Worklist(toolchain::make_pointer_range(Arg.uses()));
  ...
  do {
    ...
    for (const auto &Pair : OutArgs) {
      ...
      for (std::pair<ReturnInst *, StoreInst *> Store : ReplaceableStores) {
        ...
        if (toolchain::is_contained(toolchain::make_first_range(ValVec), OutArg)) {
          LLVM_DEBUG(dbgs()
...
FunctionPass *toolchain::createAMDGPURewriteOutArgumentsPass() {
