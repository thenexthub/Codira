...
public:
  ...
  KnownFPClass computeKnownFPClass(const Value *V, FPClassTest Interested,
    ...
    return toolchain::computeKnownFPClass(V, DL, Interested, TLI, AC, CtxI, DT);
  }
  ...
  /// Replace mul instructions with toolchain.amdgcn.mul.u24 or toolchain.amdgcn.mul.s24.
  ...
  if (MaybeZero || MaybePosInf) {
    ...
    if (MaybePosInf && MaybeZero) {
      if (DenormMode.Input != DenormalMode::DenormalModeKind::Dynamic) {
        ...
        // Avoid using toolchain.is.fpclass for dynamic denormal mode, since it
...
Value *AMDGPUCodeGenPrepareImpl::expandDivRem32(IRBuilder<> &Builder,
  ...
  Constant *Scale = ConstantFP::get(F32Ty, toolchain::bit_cast<float>(0x4F7FFFFE));
...
// Expand toolchain.sqrt.f32 calls with !fpmath metadata in a semi-fast way.
bool AMDGPUCodeGenPrepareImpl::visitSqrt(IntrinsicInst &Sqrt) {
  ...
  // of fast toolchain.sqrt will give the raw instruction anyway.
...
FunctionPass *toolchain::createAMDGPUCodeGenPreparePass() {
