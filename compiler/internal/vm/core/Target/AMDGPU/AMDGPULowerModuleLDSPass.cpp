...
template <typename T> std::vector<T> sortByName(std::vector<T> &&V) {
  toolchain::sort(V, [](const auto *L, const auto *R) {
...
class AMDGPULowerModuleLDS {
  ...
  static void markUsedByKernel(Function *Func, GlobalVariable *SGV) {
    // The toolchain.amdgcn.module.lds instance is implicitly used by all kernels
    ...
    // An operand bundle on toolchain.donothing works because the call instruction
    ...
    // toolchain.donothing that takes a pointer to the instance and is lowered to a
...
public:
  ...
  static GlobalVariable *buildLookupTable(
    ...
    return new GlobalVariable(
        ...
        "toolchain.amdgcn.lds.offset.table", nullptr, GlobalValue::NotThreadLocal,
  ...
  static std::vector<Function *> assignLDSKernelIDToEachKernel(
    ...
    if (!KernelsThatAllocateTableLDS.empty() ||
      ...
      for (size_t i = 0; i < OrderedKernels.size(); i++) {
        ...
        OrderedKernels[i]->setMetadata("toolchain.amdgcn.lds.kernel.id",
  ...
  static GlobalVariable *lowerModuleScopeStructVariables(
    ...
    LDSVariableReplacement ModuleScopeReplacement =
        createLDSVariableReplacement(M, "toolchain.amdgcn.module.lds",
  ...
  lowerKernelScopeStructVariables(
    ...
    for (Function &Func : M.functions()) {
      ...
      // backend) difficult to use. This does mean that toolchain test cases need
      // to name the kernels.
      if (!Func.hasName()) {
      ...
      std::string VarName =
          (Twine("toolchain.amdgcn.kernel.") + Func.getName() + ".lds").str();
  ...
  buildRepresentativeDynamicLDSInstance(Module &M, LDSUsesInfoTy &LDSUsesInfo,
    ...
    GlobalVariable *N = new GlobalVariable(
        ...
        Twine("toolchain.amdgcn." + func->getName() + ".dynlds"), nullptr, GlobalValue::NotThreadLocal, AMDGPUAS::LOCAL_ADDRESS,
        false);
  ...
  DenseMap<Function *, GlobalVariable *> lowerDynamicLDSVariables(
    ...
    if (!KernelsThatIndirectlyAllocateDynamicLDS.empty()) {
      ...
      GlobalVariable *table = new GlobalVariable(
          ...
          "toolchain.amdgcn.dynlds.offset.table", nullptr,
...
char &toolchain::AMDGPULowerModuleLDSLegacyPassID = AMDGPULowerModuleLDSLegacy::ID;
...
ModulePass *
toolchain::createAMDGPULowerModuleLDSLegacyPass(const AMDGPUTargetMachine *TM) {
