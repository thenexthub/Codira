...
bool toolchain::isEqual(const GCNRPTracker::LiveRegSet &S1,
...
Printable toolchain::print(const GCNRegPressure &RP, const GCNSubtarget *ST,
                      unsigned DynamicVGPRBlockSize) {
...
collectVirtualRegUses(SmallVectorImpl<VRegMaskOrUnit> &VRegMaskOrUnits,
  ...
  for (const auto &MO : MI.operands()) {
    ...
    auto I = toolchain::find_if(VRegMaskOrUnits, [Reg](const VRegMaskOrUnit &RM) {
...
LaneBitmask toolchain::getLiveLaneMask(unsigned Reg, SlotIndex SI,
                                  const LiveIntervals &LIS,
                                  const MachineRegisterInfo &MRI,
...
LaneBitmask toolchain::getLiveLaneMask(const LiveInterval &LI, SlotIndex SI,
                                  const MachineRegisterInfo &MRI,
                                  LaneBitmask LaneMaskFilter) {
...
GCNRPTracker::LiveRegSet toolchain::getLiveRegs(SlotIndex SI,
                                           const LiveIntervals &LIS,
                                           const MachineRegisterInfo &MRI,
...
Printable toolchain::reportMismatch(const GCNRPTracker::LiveRegSet &LISLR,
...
bool GCNUpwardRPTracker::isValid() const {
  ...
  const auto LISLR = toolchain::getLiveRegs(SI, LIS, *MRI);
...
Printable toolchain::print(const GCNRPTracker::LiveRegSet &LiveRegs,
                      const MachineRegisterInfo &MRI) {
...
char toolchain::GCNRegPressurePrinter::ID = 0;
char &toolchain::GCNRegPressurePrinterID = GCNRegPressurePrinter::ID;
...
#define PFX "  "
  ...
  auto ReportLISMismatchIfAny = [&](const GCNRPTracker::LiveRegSet &TrackedLR,
    ...
    if (LISLR != TrackedLR) {
      OS << PFX "  mis LIS: " << toolchain::print(LISLR, MRI)
  ...
  for (auto &MBB : MF) {
    ...
    OS << PFX "  Live-in: " << toolchain::print(LiveIn, MRI);
    if (!UseDownwardTracker)
    ...
    OS << PFX "  Live-out:" << toolchain::print(LiveOut, MRI);
    if (UseDownwardTracker)
    ...
    OS << PFX "  Live-thr:" << toolchain::print(LiveThrough, MRI);
...
LLVM_DUMP_METHOD void toolchain::dumpMaxRegPressure(MachineFunction &MF,
  ...
  for (auto [Reg, LaneMask] : *LiveSet) {
    ...
    if (LI.getNumValNums() == 1 ||
        (LI.hasSubRanges() &&
         toolchain::all_of(LI.subranges(), [](const LiveInterval::SubRange &SR) {
  ...
  toolchain::sort(SDefRegs, [&](Register A, Register B) {
