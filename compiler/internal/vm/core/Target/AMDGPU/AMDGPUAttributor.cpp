...
private:
  ...
  bool funcRetrievesMultigridSyncArg(Attributor &A, unsigned COV) {
    auto Pos = toolchain::AMDGPU::getMultigridSyncArgImplicitArgPosition(COV);
  ...
  bool funcRetrievesHostcallPtr(Attributor &A, unsigned COV) {
    auto Pos = toolchain::AMDGPU::getHostcallImplicitArgPosition(COV);
    AA::RangeTy Range(Pos, 8);
    return funcRetrievesImplicitKernelArg(A, Range);
  }

  bool funcRetrievesDefaultQueue(Attributor &A, unsigned COV) {
    auto Pos = toolchain::AMDGPU::getDefaultQueueImplicitArgPosition(COV);
    AA::RangeTy Range(Pos, 8);
    return funcRetrievesImplicitKernelArg(A, Range);
  }

  bool funcRetrievesCompletionAction(Attributor &A, unsigned COV) {
    auto Pos = toolchain::AMDGPU::getCompletionActionImplicitArgPosition(COV);
    AA::RangeTy Range(Pos, 8);
    return funcRetrievesImplicitKernelArg(A, Range);
  }
...
PreservedAnalyses toolchain::AMDGPUAttributorPass::run(Module &M,
