...
static bool fnegFoldsIntoOpcode(unsigned Opc) {
  ...
  case AMDGPUISD::FMED3:
    // TODO: handle toolchain.amdgcn.fma.legacy
    return true;
  case ISD::BITCAST:
...
SDValue AMDGPUTargetLowering::LowerGlobalAddress(AMDGPUMachineFunction* MFI,
  ...
  if (G->getAddressSpace() == AMDGPUAS::LOCAL_ADDRESS ||
    ...
    if (!MFI->isModuleEntryFunction() &&
        GV->getName() != "toolchain.amdgcn.module.lds" &&
...
SDValue AMDGPUTargetLowering::LowerFP_TO_INT64(SDValue Op, SelectionDAG &DAG,
  ...
  if (SrcVT == MVT::f64) {
    K0 = DAG.getConstantFP(
        toolchain::bit_cast<double>(UINT64_C(/*2^-32*/ 0x3df0000000000000)), SL,
        SrcVT);
    K1 = DAG.getConstantFP(
        toolchain::bit_cast<double>(UINT64_C(/*-2^32*/ 0xc1f0000000000000)), SL,
        SrcVT);
  } else {
    K0 = DAG.getConstantFP(
        toolchain::bit_cast<float>(UINT32_C(/*2^-32*/ 0x2f800000)), SL, SrcVT);
    K1 = DAG.getConstantFP(
        toolchain::bit_cast<float>(UINT32_C(/*-2^32*/ 0xcf800000)), SL, SrcVT);
  }
  // TODO: Should this propagate fast-math-flags?
...
SDValue AMDGPUTargetLowering::loadInputValue(SelectionDAG &DAG,
  ...
  unsigned Shift = toolchain::countr_zero<unsigned>(Mask);
