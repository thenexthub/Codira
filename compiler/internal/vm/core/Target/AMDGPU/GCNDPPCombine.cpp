...
char &toolchain::GCNDPPCombineLegacyID = GCNDPPCombineLegacy::ID;

FunctionPass *toolchain::createGCNDPPCombinePass() {
...
bool GCNDPPCombine::combineDPPMov(MachineInstr &MovMI) const {
  ...
  SmallVector<MachineOperand *, 16> Uses(
      toolchain::make_pointer_range(MRI->use_nodbg_operands(DPPMovReg)));
