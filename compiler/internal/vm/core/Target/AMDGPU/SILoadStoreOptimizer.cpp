...
static unsigned getOpcodeWidth(const MachineInstr &MI, const SIInstrInfo &TII) {
  ...
  if (TII.isImage(MI)) {
    ...
    return toolchain::popcount(DMaskImm);
  }
  if (TII.isMTBUF(Opc)) {
...
char &toolchain::SILoadStoreOptimizerLegacyID = SILoadStoreOptimizerLegacy::ID;

FunctionPass *toolchain::createSILoadStoreOptimizerLegacyPass() {
...
bool SILoadStoreOptimizer::dmasksCanBeCombined(const CombineInfo &CI,
  ...
  unsigned AllowedBitsForMin = toolchain::countr_zero(MaxMask);
...
static unsigned getBufferFormatWithCompCount(unsigned OldFormat,
  ...
  const toolchain::AMDGPU::GcnBufferFormatInfo *OldFormatInfo =
      toolchain::AMDGPU::getGcnBufferFormatInfo(OldFormat, STI);
  if (!OldFormatInfo)
    return 0;

  const toolchain::AMDGPU::GcnBufferFormatInfo *NewFormatInfo =
      toolchain::AMDGPU::getGcnBufferFormatInfo(OldFormatInfo->BitsPerComp,
                                           ComponentCount,
...
static uint32_t mostAlignedValueInRange(uint32_t Lo, uint32_t Hi) {
  return Hi & maskLeadingOnes<uint32_t>(toolchain::countl_zero((Lo - 1) ^ Hi) + 1);
}
...
bool SILoadStoreOptimizer::offsetsCanBeCombined(CombineInfo &CI,
  ...
  if (CI.InstClass == TBUFFER_LOAD || CI.InstClass == TBUFFER_STORE) {
    ...
    const toolchain::AMDGPU::GcnBufferFormatInfo *Info0 =
        toolchain::AMDGPU::getGcnBufferFormatInfo(CI.Format, STI);
    const toolchain::AMDGPU::GcnBufferFormatInfo *Info1 =
        toolchain::AMDGPU::getGcnBufferFormatInfo(Paired.Format, STI);
...
unsigned SILoadStoreOptimizer::getNewOpcode(const CombineInfo &CI,
  ...
  case MIMG:
    assert(((unsigned)toolchain::popcount(CI.DMask | Paired.DMask) == Width) &&
           "No overlaps");
...
SILoadStoreOptimizer::getSubRegIdxs(const CombineInfo &CI,
                                    const CombineInfo &Paired) {
  assert((CI.InstClass != MIMG ||
          ((unsigned)toolchain::popcount(CI.DMask | Paired.DMask) ==
