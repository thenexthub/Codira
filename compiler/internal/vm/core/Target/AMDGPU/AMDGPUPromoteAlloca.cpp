...
char &toolchain::AMDGPUPromoteAllocaID = AMDGPUPromoteAlloca::ID;
...
FunctionPass *toolchain::createAMDGPUPromoteAlloca() {
  ...
  case Instruction::Load: {
    ...
    if (isa<FixedVectorType>(AccessTy)) {
      ...
      if (!isa<ConstantInt>(Index) &&
          toolchain::isPowerOf2_32(SubVecTy->getNumElements()) &&
        ...
        const unsigned LShrAmt = toolchain::Log2_32(SubVecTy->getNumElements());
...
bool AMDGPUPromoteAllocaImpl::hasSufficientLocalMem(const Function &F) {
  ...
  // value instead
  toolchain::sort(AllocatedSizes, toolchain::less_second());
