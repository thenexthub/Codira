...
void AMDGPUInstPrinter::printSymbolicFormat(const MCInst *MI,
                                            const MCSubtargetInfo &STI,
                                            raw_ostream &O) {
  using namespace vm::core::AMDGPU::MTBUFFormat;
...
bool AMDGPUInstPrinter::printImmediateFloat32(uint32_t Imm,
                                              const MCSubtargetInfo &STI,
                                              raw_ostream &O) {
  if (Imm == toolchain::bit_cast<uint32_t>(0.0f))
    O << "0.0";
  else if (Imm == toolchain::bit_cast<uint32_t>(1.0f))
    O << "1.0";
  else if (Imm == toolchain::bit_cast<uint32_t>(-1.0f))
    O << "-1.0";
  else if (Imm == toolchain::bit_cast<uint32_t>(0.5f))
    O << "0.5";
  else if (Imm == toolchain::bit_cast<uint32_t>(-0.5f))
    O << "-0.5";
  else if (Imm == toolchain::bit_cast<uint32_t>(2.0f))
    O << "2.0";
  else if (Imm == toolchain::bit_cast<uint32_t>(-2.0f))
    O << "-2.0";
  else if (Imm == toolchain::bit_cast<uint32_t>(4.0f))
    O << "4.0";
  else if (Imm == toolchain::bit_cast<uint32_t>(-4.0f))
    O << "-4.0";
...
void AMDGPUInstPrinter::printImmediate64(uint64_t Imm,
  ...
  if (Imm == toolchain::bit_cast<uint64_t>(0.0))
    O << "0.0";
  else if (Imm == toolchain::bit_cast<uint64_t>(1.0))
    O << "1.0";
  else if (Imm == toolchain::bit_cast<uint64_t>(-1.0))
    O << "-1.0";
  else if (Imm == toolchain::bit_cast<uint64_t>(0.5))
    O << "0.5";
  else if (Imm == toolchain::bit_cast<uint64_t>(-0.5))
    O << "-0.5";
  else if (Imm == toolchain::bit_cast<uint64_t>(2.0))
    O << "2.0";
  else if (Imm == toolchain::bit_cast<uint64_t>(-2.0))
    O << "-2.0";
  else if (Imm == toolchain::bit_cast<uint64_t>(4.0))
    O << "4.0";
  else if (Imm == toolchain::bit_cast<uint64_t>(-4.0))
    O << "-4.0";
...
void AMDGPUInstPrinter::printDppFI(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  using namespace vm::core::AMDGPU::DPP;
  unsigned Imm = MI->getOperand(OpNo).getImm();
...
void AMDGPUInstPrinter::printSDWASel(const MCInst *MI, unsigned OpNo,
                                     raw_ostream &O) {
  using namespace vm::core::AMDGPU::SDWA;
...
void AMDGPUInstPrinter::printExpTgt(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
  using namespace vm::core::AMDGPU::Exp;
...
void AMDGPUInstPrinter::printGPRIdxMode(const MCInst *MI, unsigned OpNo,
                                        const MCSubtargetInfo &STI,
                                        raw_ostream &O) {
  using namespace vm::core::AMDGPU::VGPRIndexMode;
  unsigned Val = MI->getOperand(OpNo).getImm();
...
void AMDGPUInstPrinter::printSendMsg(const MCInst *MI, unsigned OpNo,
                                     const MCSubtargetInfo &STI,
                                     raw_ostream &O) {
  using namespace vm::core::AMDGPU::SendMsg;
...
static void printSwizzleBitmask(const uint16_t AndMask,
                                ...
                                raw_ostream &O) {
  using namespace vm::core::AMDGPU::Swizzle;
...
void AMDGPUInstPrinter::printSwizzle(const MCInst *MI, unsigned OpNo,
  ...
  if ((Imm & QUAD_PERM_ENC_MASK) == QUAD_PERM_ENC) {
    ...
    if (AndMask == BITMASK_MAX && OrMask == 0 && toolchain::popcount(XorMask) == 1) {
...
void AMDGPUInstPrinter::printDepCtr(const MCInst *MI, unsigned OpNo,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
  using namespace vm::core::AMDGPU::DepCtr;
...
void AMDGPUInstPrinter::printHwreg(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  using namespace vm::core::AMDGPU::Hwreg;
  unsigned Val = MI->getOperand(OpNo).getImm();
