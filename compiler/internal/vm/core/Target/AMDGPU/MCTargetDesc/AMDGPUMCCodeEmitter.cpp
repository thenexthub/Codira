...
MCCodeEmitter *toolchain::createAMDGPUMCCodeEmitter(const MCInstrInfo &MCII,
                                               MCContext &Ctx) {
...
static uint32_t getLit32Encoding(uint32_t Val, const MCSubtargetInfo &STI) {
  ...
  if (Val == toolchain::bit_cast<uint32_t>(0.5f))
    return 240;

  if (Val == toolchain::bit_cast<uint32_t>(-0.5f))
    return 241;

  if (Val == toolchain::bit_cast<uint32_t>(1.0f))
    return 242;

  if (Val == toolchain::bit_cast<uint32_t>(-1.0f))
    return 243;

  if (Val == toolchain::bit_cast<uint32_t>(2.0f))
    return 244;

  if (Val == toolchain::bit_cast<uint32_t>(-2.0f))
    return 245;

  if (Val == toolchain::bit_cast<uint32_t>(4.0f))
    return 246;
...
static uint32_t getLit64Encoding(const MCInstrDesc &Desc, uint64_t Val,
  ...
  if (Val == toolchain::bit_cast<uint64_t>(0.5))
    return 240;

  if (Val == toolchain::bit_cast<uint64_t>(-0.5))
    return 241;

  if (Val == toolchain::bit_cast<uint64_t>(1.0))
    return 242;

  if (Val == toolchain::bit_cast<uint64_t>(-1.0))
    return 243;

  if (Val == toolchain::bit_cast<uint64_t>(2.0))
    return 244;

  if (Val == toolchain::bit_cast<uint64_t>(-2.0))
    return 245;

  if (Val == toolchain::bit_cast<uint64_t>(4.0))
    return 246;
...
void AMDGPUMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  for (unsigned i = 0, e = Desc.getNumOperands(); i < e; ++i) {
    ...
    if (*Enc == 254) {
      ...
      support::endian::write<uint64_t>(CB, Imm, toolchain::endianness::little);
    } else {
      auto OpType =
          static_cast<AMDGPU::OperandType>(Desc.operands()[i].OperandType);
      Imm = AMDGPU::encode32BitLiteral(Imm, OpType, IsLit);
      support::endian::write<uint32_t>(CB, Imm, toolchain::endianness::little);
    }
