  ...
  for (MachineBasicBlock *MBB : Blocks) {
    ...
    if (Uses.size() == 1) {
      ...
      for (MachineInstr &MI : *MBB) {
        if (toolchain::is_contained(Uses, &MI))
...
char &toolchain::SIOptimizeVGPRLiveRangeLegacyID = SIOptimizeVGPRLiveRangeLegacy::ID;

FunctionPass *toolchain::createSIOptimizeVGPRLiveRangeLegacyPass() {
