...
#include "vm/core/ADT/PackedVector.h"
...
class AMDGPULowerVGPREncoding {
  ...
  static constexpr unsigned VGPRMSBShift =
      toolchain::countr_zero_constexpr<unsigned>(AMDGPU::Hwreg::DST_VGPR_MSB);
...
static int64_t convertModeToSetregFormat(int64_t Mode) {
  ...
  return toolchain::rotl<uint8_t>(static_cast<uint8_t>(Mode), /*R=*/2);
}
...
bool AMDGPULowerVGPREncoding::run(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    ...
    for (auto &MI : toolchain::make_early_inc_range(MBB.instrs())) {
...
char &toolchain::AMDGPULowerVGPREncodingLegacyID = AMDGPULowerVGPREncodingLegacy::ID;
