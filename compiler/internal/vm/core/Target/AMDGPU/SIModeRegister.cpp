...
char &toolchain::SIModeRegisterID = SIModeRegisterLegacy::ID;

FunctionPass *toolchain::createSIModeRegisterPass() {
...
void SIModeRegister::insertSetreg(MachineBasicBlock &MBB, MachineInstr *MI,
  ...
  while (InstrMode.Mask) {
    unsigned Offset = toolchain::countr_zero<unsigned>(InstrMode.Mask);
    unsigned Width = toolchain::countr_one<unsigned>(InstrMode.Mask >> Offset);
...
bool SIModeRegister::run(MachineFunction &MF) {
  ...
  if (F.hasFnAttribute(toolchain::Attribute::StrictFP))
    return Changed;
