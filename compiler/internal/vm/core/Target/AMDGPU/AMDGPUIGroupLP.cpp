...
void PipelineSolver::reset() {
  ...
  for (auto &SyncPipeline : CurrPipeline) {
    for (auto &SG : SyncPipeline) {
      ...
      SG.Collection.clear();
      auto *SchedBarr = toolchain::find_if(TempCollection, [](SUnit *SU) {
...
void PipelineSolver::populateReadyList(
  ...
  for (; I != E; ++I) {
    std::vector<std::pair<SUnit *, SUnit *>> AddedEdges;
    int CandSGID = *I;
    SchedGroup *Match = toolchain::find_if(SyncPipeline, [CandSGID](SchedGroup &SG) {
  ...
  if (UseCostHeur)
    std::sort(ReadyList.begin(), ReadyList.end(), toolchain::less_second());
...
private:
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
      ...
      unsigned SuccSize = toolchain::count_if(SU->Succs, [](const SDep &Succ) {
      ...
      if (HasIntermediary) {
        for (auto Succ : SU->Succs) {
          unsigned SuccSize =
              toolchain::count_if(Succ.getSUnit()->Succs, [](const SDep &SuccSucc) {
...
bool MFMAExpInterleaveOpt::analyzeDAG(const SIInstrInfo *TII) {
  ...
  MFMAEnablement =
      toolchain::count_if(PackPred->getSUnit()->Succs, [&TII](SDep &Succ) {
        return TII->isMFMAorWMMA(*Succ.getSUnit()->getInstr());
      });
  ...
  ExpRequirement =
      toolchain::count_if(ExpPipeCands, [this, &PackPred](SUnit *ExpBase) {
...
private:
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
      ...
      return toolchain::any_of(*Cache, [&SU](SUnit *Elt) {
        return toolchain::any_of(SU->Succs, [&Elt](const SDep &ThisSucc) {
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
      ...
      // VMEM_READ
      return toolchain::any_of(*Cache, [&SU, &DAG](SUnit *Elt) {
  ...
  if (IsInitial) {
    ...
    for (; I != E; I++) {
      ...
      for (auto &Pred : (*I)->Preds) {
        ...
        if (Cand && toolchain::is_contained(Counted, Cand))
          break;
        ...
        for (auto &Succ : Pred.getSUnit()->Succs) {
          ...
          Cand = It->second;
          if (toolchain::is_contained(Counted, Cand)) {
            MissedAny = true;
            break;
          }
        }
...
toolchain::createIGroupLPDAGMutation(AMDGPU::SchedulingPhase Phase) {
