...
// Constant fold toolchain.amdgcn.fmed3 intrinsics for standard inputs.
//
...
/// Return true if it's legal to contract toolchain.amdgcn.rcp(toolchain.sqrt)
...
/// Simplify a lane index operand (e.g. toolchain.amdgcn.readlane src1).
///
...
GCNTTIImpl::instCombineIntrinsic(InstCombiner &IC, IntrinsicInst &II) const {
  ...
  case Intrinsic::amdgcn_rcp: {
    ...
    // toolchain.amdgcn.rcp(toolchain.amdgcn.sqrt(x)) -> toolchain.amdgcn.rsq(x) if contractable
    //
    // toolchain.amdgcn.rcp(toolchain.sqrt(x)) -> toolchain.amdgcn.rsq(x) if contractable and
    // relaxed.
  ...
  case Intrinsic::amdgcn_fmed3: {
    ...
    // minimumnum/maximumnum. With ieee=1, the v_min_f32 acts like toolchain.minnum
    // with signaling nan handling. With ieee=0, like toolchain.minimumnum except a
  ...
  case Intrinsic::amdgcn_fcmp: {
    ...
    // toolchain.amdgcn.icmp(zext (i1 x), 1, eq)
    //   -> toolchain.amdgcn.icmp(zext (i1 x), 0, ne)
    // toolchain.amdgcn.icmp(sext (i1 x), -1, eq)
    //   -> toolchain.amdgcn.icmp(sext (i1 x), 0, ne)
    Value *ExtSrc;
    ...
    // toolchain.amdgcn.icmp([sz]ext ([if]cmp pred a, b), 0, ne)
    //   -> toolchain.amdgcn.[if]cmp(a, b, pred)
    //
    // toolchain.amdgcn.icmp([sz]ext ([if]cmp pred a, b), 0, eq)
    //   -> toolchain.amdgcn.[if]cmp(a, b, inv pred)
...
static Value *simplifyAMDGCNMemoryIntrinsicDemanded(InstCombiner &IC,
  ...
  if (DMaskIdx < 0) {
    ...
    DemandedElts &= (1 << toolchain::popcount(DMaskVal)) - 1;
