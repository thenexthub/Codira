...
void SIMachineFunctionInfo::allocateWWMSpill(MachineFunction &MF, Register VGPR,
  ...
  // toolchain.amdgcn.cs.chain (otherwise there's no one to save them for, since
  // chain functions do not return) and the function did not contain a call to
  // toolchain.amdgcn.init.whole.wave (since in that case there are no inactive lanes
...
void SIMachineFunctionInfo::shiftWwmVGPRsToLowestRange(
  ...
  for (unsigned I = 0, E = WWMVGPRs.size(); I < E; ++I) {
    ...
    auto *RegItr = toolchain::find(SpillPhysVGPRs, Reg);
...
yaml::SIMachineFunctionInfo::SIMachineFunctionInfo(
    const toolchain::SIMachineFunctionInfo &MFI, const TargetRegisterInfo &TRI,
    const toolchain::MachineFunction &MF)
