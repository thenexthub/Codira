...
static bool dependsOnLocalPhi(const Loop *L, const Value *Cond,
  ...
  for (const Value *V : I->operand_values()) {
    ...
    if (const PHINode *PHI = dyn_cast<PHINode>(V)) {
      if (toolchain::none_of(L->getSubLoops(), [PHI](const Loop* SubLoop) {
...
void AMDGPUTTIImpl::getUnrollingPreferences(
  ...
  for (const BasicBlock *BB : L->getBlocks()) {
    ...
    if (toolchain::any_of(L->getSubLoops(), [BB](const Loop* SubLoop) {
    ...
    for (const Instruction &I : *BB) {
      ...
      for (const Value *Op : GEP->operands()) {
        ...
        if (toolchain::any_of(L->getSubLoops(), [Inst](const Loop* SubLoop) {
    ...
    if (!Mask.empty()) {
      ...
      for (unsigned DstIdx = 0; DstIdx < Mask.size(); DstIdx += EltsPerReg) {
        ...
        for (unsigned I = 0; I < EltsPerReg && DstIdx + I < Mask.size(); ++I) {
          ...
          if (!toolchain::is_contained(Regs, Reg))
            Regs.push_back(Reg);
        }
        if (Regs.size() >= 2)
          Cost += Regs.size() - 1;
