...
void AMDGPUMCInstLower::lowerT16D16Helper(const MachineInstr *MI,
  ...
  toolchain::AMDGPU::OpName OpName;
  if (TII->isDS(Opcode)) {
    if (MI->mayLoad())
      OpName = toolchain::AMDGPU::OpName::vdst;
    else if (MI->mayStore())
      OpName = toolchain::AMDGPU::OpName::data0;
    else
      llvm_unreachable("LDS load or store expected");
  } else {
    OpName = AMDGPU::hasNamedOperand(Opcode, toolchain::AMDGPU::OpName::vdata)
                 ? toolchain::AMDGPU::OpName::vdata
                 : toolchain::AMDGPU::OpName::vdst;
  }
...
void AMDGPUMCInstLower::lowerT16FmaMixFP16(const MachineInstr *MI,
  ...
  int VDstIdx = AMDGPU::getNamedOperandIdx(Opcode, toolchain::AMDGPU::OpName::vdst);
...
static void emitVGPRBlockComment(const MachineInstr *MI, const SIInstrInfo *TII,
  ...
  for (unsigned I = 0; I < sizeof(Mask) * 8; ++I) {
    if (Mask & (1 << I)) {
      (toolchain::Twine(" ") + TRI->getRegAsmName(FirstRegInBlock + I))
