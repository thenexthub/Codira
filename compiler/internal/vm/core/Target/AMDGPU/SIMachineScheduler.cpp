...
void SIScheduleBlock::initRegPressure(MachineBasicBlock::iterator BeginBlock,
  ...
  // Internally toolchain will represent some registers as big 128 bits registers
...
void SIScheduleBlock::nodeScheduled(SUnit *SU) {
  ...
  std::vector<SUnit *>::iterator I = toolchain::find(TopReadySUs, SU);
...
hasDataDependencyPred(const SUnit &SU, const SUnit &FromSU) {
  for (const auto &PredDep : SU.Preds) {
    if (PredDep.getSUnit() == &FromSU &&
        PredDep.getKind() == toolchain::SDep::Data)
      return true;
  }
  return false;
}
  ...
  for (unsigned SUNum : DAG->TopDownIndex2SU) {
    ...
    if (DAG->IsHighLatencySU[SU.NodeNum]) {
      ...
      for (unsigned j : FormingGroup) {
        ...
        toolchain::append_range(AdditionalElements, SubGraph);
      }
      if (CompatibleGroup) {
...
void SIScheduleBlockCreator::colorEndsAccordingToDependencies() {
  ...
  if (*toolchain::max_element(CurrentBottomUpReservedDependencyColoring) == 0 &&
      *toolchain::max_element(CurrentTopDownReservedDependencyColoring) == 0)
    return;
