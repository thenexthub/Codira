    ...
    for (auto &MBB : MF) {
      ...
      while (MII != E) {
        ...
        const auto checkVOPD = [&](bool VOPD3) -> bool {
          toolchain::AMDGPU::CanBeVOPD FirstCanBeVOPD =
              AMDGPU::getCanBeVOPD(Opc, EncodingFamily, VOPD3);
          toolchain::AMDGPU::CanBeVOPD SecondCanBeVOPD =
          ...
          return toolchain::checkVOPDRegConstraints(*SII, *FirstMI, *SecondMI,
                                               VOPD3);
        };
...
PreservedAnalyses
toolchain::GCNCreateVOPDPass::run(MachineFunction &MF,
...
char &toolchain::GCNCreateVOPDID = GCNCreateVOPDLegacy::ID;
