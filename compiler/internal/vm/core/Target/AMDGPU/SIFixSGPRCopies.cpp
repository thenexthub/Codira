...
char &toolchain::SIFixSGPRCopiesLegacyID = SIFixSGPRCopiesLegacy::ID;

FunctionPass *toolchain::createSIFixSGPRCopiesLegacyPass() {
    ...
    for (auto I1 = Defs.begin(), E = Defs.end(); I1 != E; ) {
      ...
      for (auto I2 = std::next(I1); I2 != E; ) {
        ...
        auto interferes = [&](MachineBasicBlock::iterator From,
          ...
          return (toolchain::any_of(Clobbers, interferes)) ||
                 (toolchain::any_of(Inits, [&](InitListMap::value_type &C) {
                    return C.first != Init.first &&
                           toolchain::any_of(C.second, interferes);
                  }));
        };
...
bool SIFixSGPRCopies::needToBeConvertedToVALU(V2SCopyInfo *Info) {
  ...
  Info->Siblings = SiblingPenalty[*toolchain::max_element(
