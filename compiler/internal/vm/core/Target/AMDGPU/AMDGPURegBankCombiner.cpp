...
bool AMDGPURegBankCombinerImpl::matchFPMed3ToClamp(MachineInstr &MI,
                                                   Register &Reg) const {
  // In toolchain-ir, clamp is often represented as an intrinsic call to
  // @toolchain.amdgcn.fmed3.f32(%Val, 0.0, 1.0). Check for other operand orders.
...
FunctionPass *toolchain::createAMDGPURegBankCombiner(bool IsOptNone) {
