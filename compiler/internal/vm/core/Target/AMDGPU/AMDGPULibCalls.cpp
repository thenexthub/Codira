...
private:
  ...
  using FuncInfo = toolchain::AMDGPULibFunc;
...
bool AMDGPULibCalls::useNativeFunc(const StringRef F) const {
  return AllNative || toolchain::is_contained(UseNative, F);
}
...
bool AMDGPULibCalls::fold_read_write_pipe(CallInst *CI, IRBuilder<> &B,
  ...
  SmallVector<toolchain::Type *, 6> ArgTys;
...
// Some library calls are just wrappers around toolchain intrinsics, but compiled
