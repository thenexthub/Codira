...
static bool canRemat(const MachineInstr &MI) {
  ...
  if (SIInstrInfo::isSMRD(MI)) {
    return !MI.memoperands_empty() &&
           toolchain::all_of(MI.memoperands(), [](const MachineMemOperand *MMO) {
...
SIInstrInfo::expandMovDPP64(MachineInstr &MI) const {
  ...
  for (auto Sub : { AMDGPU::sub0, AMDGPU::sub1 }) {
    ...
    for (const MachineOperand &MO : toolchain::drop_begin(MI.explicit_operands(), 3))
...
unsigned SIInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  for (MachineInstr &MI : toolchain::make_early_inc_range(MBB.terminators())) {
...
bool SIInstrInfo::verifyInstruction(const MachineInstr &MI,
  ...
  if (isImage(Opcode) && !MI.mayStore()) {
    ...
    if (DMask) {
      ...
      uint32_t RegCount = isGather4(Opcode) ? 4 : toolchain::popcount(DMaskImm);
  ...
  if (isVALU(MI) && Desc.getOpcode() != AMDGPU::V_WRITELANE_B32) {
    ...
    for (int OpIdx : {Src0Idx, Src1Idx, Src2Idx, Src3Idx}) {
      ...
      if (usesConstantBus(MRI, MO, MI.getDesc().operands()[OpIdx])) {
        if (MO.isReg()) {
          SGPRUsed = MO.getReg();
          if (!toolchain::is_contained(SGPRsUsed, SGPRUsed)) {
            ++ConstantBusCount;
    ...
    if (SGPRUsed) {
      ...
      if (toolchain::all_of(SGPRsUsed, [this, SGPRUsed](unsigned SGPR) {
...
toolchain::getRegSequenceSubReg(MachineInstr &MI, unsigned SubReg) {
...
MachineInstr *toolchain::getVRegSubRegDef(const TargetInstrInfo::RegSubRegPair &P,
                                     const MachineRegisterInfo &MRI) {
  assert(MRI.isSSA());
  if (!P.Reg.isVirtual())
    return nullptr;
...
bool toolchain::execMayBeModifiedBeforeUse(const MachineRegisterInfo &MRI,
                                      Register VReg,
...
bool toolchain::execMayBeModifiedBeforeAnyUse(const MachineRegisterInfo &MRI,
                                         Register VReg,
                                         const MachineInstr &DefMI) {
  assert(MRI.isSSA() && "Must be run on SSA");
...
bool toolchain::SIInstrInfo::isWave32() const { return ST.isWave32(); }
...
SIInstrInfo::getGenericInstructionUniformity(const MachineInstr &MI) const {
  ...
  if (Opcode == AMDGPU::G_LOAD || Opcode == AMDGPU::G_ZEXTLOAD ||
    ...
    if (toolchain::any_of(MI.memoperands(), [](const MachineMemOperand *mmo) {
...
bool SIInstrInfo::optimizeCompareInstr(MachineInstr &CmpInstr, Register SrcReg,
  ...
  const auto optimizeCmpAnd = [&CmpInstr, SrcReg, CmpValue, MRI,
    ...
    assert(toolchain::has_single_bit<uint64_t>(Mask) && "Invalid mask.");
    unsigned BitNo = toolchain::countr_zero((uint64_t)Mask);
