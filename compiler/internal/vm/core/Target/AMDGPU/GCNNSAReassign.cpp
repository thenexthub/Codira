...
char &toolchain::GCNNSAReassignID = GCNNSAReassignLegacy::ID;
...
bool GCNNSAReassignImpl::run(MachineFunction &MF) {
  ...
  for (auto &C : Candidates) {
    ...
    for (unsigned I = 0; I < Info->VAddrOperands; ++I) {
      ...
      if (toolchain::is_contained(Intervals, LI)) {
    ...
    LLVM_DEBUG(dbgs() << "Attempting to reassign NSA: " << *MI
               ...
               for (auto *LI
                    : Intervals) dbgs()
               << " " << toolchain::printReg((VRM->getPhys(LI->reg())), TRI);
               dbgs() << '\n');
    ...
    LLVM_DEBUG(
        dbgs() << "\tNew allocation:\t\t ["
               << toolchain::printReg((VRM->getPhys(Intervals.front()->reg())), TRI)
               << " : "
               << toolchain::printReg((VRM->getPhys(Intervals.back()->reg())), TRI)
               << "]\n");
    Changed = true;
  }
