...
unsigned AMDGPUCustomBehaviour::checkCustomHazard(ArrayRef<InstRef> IssuedInst,
  ...
  // toolchain-mca is generally run on fully compiled assembly so we wouldn't see any
  ...
  case AMDGPU::S_WAITCNT_vi:
    ...
    // to model this in toolchain-mca based on how the iterations work
...
void AMDGPUCustomBehaviour::generateWaitCntInfo() {
  ...
  for (const auto &EN : toolchain::enumerate(SrcMgr.getInstructions())) {
