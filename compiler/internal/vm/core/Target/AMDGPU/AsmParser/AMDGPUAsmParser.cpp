...
using namespace vm::core::amdhsa;
...
void AMDGPUAsmParser::createConstantSymbol(StringRef Id, int64_t Val) {
  ...
  // Currently there is none suitable machinery in the core toolchain-mc for this.
...
MCRegister AMDGPUAsmParser::getRegularReg(RegisterKind RegKind, unsigned RegNum,
  ...
  if (RegKind == IS_SGPR || RegKind == IS_TTMP) {
    ...
    AlignSize = std::min(toolchain::bit_ceil(RegWidth / 32), 4u);
  }
...
bool AMDGPUAsmParser::validateMIMGDataSize(const MCInst &Inst, SMLoc IDLoc) {
  ...
  unsigned DataSize =
      (Desc.TSFlags & SIInstrFlags::Gather4) ? 4 : toolchain::popcount(DMask);
  if (hasPackedD16()) {
