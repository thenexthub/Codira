...
char &toolchain::R600VectorRegMergerID = R600VectorRegMerger::ID;
...
MachineInstr *R600VectorRegMerger::RebuildVector(
  ...
  for (const auto &It : RSI->RegToChan) {
    ...
    std::vector<Register>::iterator ChanPos = toolchain::find(UpdatedUndef, Chan);
...
void R600VectorRegMerger::RemoveMI(MachineInstr *MI) {
  for (auto &It : PreviousRegSeqByReg) {
    ...
    MIs.erase(toolchain::find(MIs, MI), MIs.end());
  }
...
bool R600VectorRegMerger::areAllUsesSwizzeable(Register Reg) const {
  return toolchain::all_of(MRI->use_instructions(Reg),
...
toolchain::FunctionPass *toolchain::createR600VectorRegMerger() {
