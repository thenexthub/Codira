...
char &toolchain::SIPreEmitPeepholeID = SIPreEmitPeepholeLegacy::ID;
  ...
  for (MachineBasicBlock::instr_iterator I = std::next(First.getIterator()),
    ...
    default:
      ...
      if (IdxReg && I->modifiesRegister(IdxReg, TRI))
        return false;
      if (toolchain::any_of(I->operands(), [&MRI, this](const MachineOperand &MO) {
...
PreservedAnalyses
toolchain::SIPreEmitPeepholePass::run(MachineFunction &MF,
