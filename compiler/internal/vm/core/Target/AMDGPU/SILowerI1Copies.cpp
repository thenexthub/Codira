    ...
    for (auto &[MBB, Reachable] : ReachableMap) {
      ...
      if (HaveReachablePred) {
        for (MachineBasicBlock *UnreachablePred : Stack) {
          if (!toolchain::is_contained(Predecessors, UnreachablePred))
...
Register
toolchain::createLaneMaskReg(MachineRegisterInfo *MRI,
                        MachineRegisterInfo::VRegAttrs LaneMaskRegAttrs) {
...
bool PhiLoweringHelper::lowerPhis() {
  ...
  for (MachineInstr *MI : Vreg1Phis) {
    ...
    toolchain::sort(Incomings, [this](Incoming LHS, Incoming RHS) {
...
char &toolchain::SILowerI1CopiesLegacyID = SILowerI1CopiesLegacy::ID;

FunctionPass *toolchain::createSILowerI1CopiesLegacyPass() {
