...
bool CFStack::branchStackContains(CFStack::StackItem Item) {
  return toolchain::is_contained(BranchStack, Item);
}
  ...
  void getLiteral(MachineInstr &MI, std::vector<MachineOperand *> &Lits) const {
    ...
    for (const auto &Src:Srcs) {
      ...
      std::vector<MachineOperand *>::iterator It =
          toolchain::find_if(Lits, [&](MachineOperand *val) {
...
char &toolchain::R600ControlFlowFinalizerID = R600ControlFlowFinalizer::ID;

FunctionPass *toolchain::createR600ControlFlowFinalizer() {
