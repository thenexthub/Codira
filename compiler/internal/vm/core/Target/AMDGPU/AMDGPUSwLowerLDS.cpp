...
//    into the "Sw LDS".
//    A new "toolchain.amdgcn.<kernel>.dynlds" is created per kernel accessing
...
//        "toolchain.amdgcn.sw.lds.<kernel-name>".
//    - Metadata Global:
...
//        have name "toolchain.amdgcn.sw.lds.<kernel-name>.md". This global will have
...
template <typename T> SetVector<T> sortByName(std::vector<T> &&V) {
  ...
  return {SetVector<T>(toolchain::from_range, V)};
}
...
SetVector<Function *> AMDGPUSwLowerLDS::getOrderedIndirectLDSAccessingKernels(
  ...
  for (size_t i = 0; i < Kernels.size(); i++) {
    ...
    Func->setMetadata("toolchain.amdgcn.lds.kernel.id",
...
void AMDGPUSwLowerLDS::buildSwLDSGlobal(Function *Func) {
  ...
  LDSParams.SwLDS = new GlobalVariable(
      ...
      PoisonValue::get(IRB.getPtrTy()), "toolchain.amdgcn.sw.lds." + Func->getName(),
...
void AMDGPUSwLowerLDS::buildSwDynLDSGlobal(Function *Func) {
  ...
  LDSParams.SwDynLDS = new GlobalVariable(
      ...
      "toolchain.amdgcn." + Func->getName() + ".dynlds", nullptr,
...
void AMDGPUSwLowerLDS::populateSwMetadataGlobal(Function *Func) {
  ...
  MDItemOS << "toolchain.amdgcn.sw.lds." << Func->getName() << ".md.item";
...
void AMDGPUSwLowerLDS::buildNonKernelLDSBaseTable(
  ...
  NKLDSParams.LDSBaseTable = new GlobalVariable(
      ...
      "toolchain.amdgcn.sw.lds.base.table", nullptr, GlobalValue::NotThreadLocal,
...
void AMDGPUSwLowerLDS::buildNonKernelLDSOffsetTable(
  ...
  NKLDSParams.LDSOffsetTable = new GlobalVariable(
      M, AllKernelsOffsetsType, true, GlobalValue::InternalLinkage, Init,
      "toolchain.amdgcn.sw.lds.offset.table", nullptr, GlobalValue::NotThreadLocal,
      AMDGPUAS::GLOBAL_ADDRESS);
  GlobalValue::SanitizerMetadata MD;
  MD.NoAddress = true;
...
void AMDGPUSwLowerLDS::lowerNonKernelLDSAccesses(
  ...
  for (GlobalVariable *GV : LDSGlobals) {
    const auto *GVIt = toolchain::find(OrdereLDSGlobals, GV);
...
void AMDGPUSwLowerLDS::initAsanInfo() {
  ...
  int Scale;
  bool OrShadowOffset;
  toolchain::getAddressSanitizerParams(AMDGPUTM.getTargetTriple(), LongSize, false,
...
bool AMDGPUSwLowerLDS::run() {
  ...
  if (!FuncLDSAccessInfo.NonKernelToLDSAccessMap.empty() ||
    ...
    for (Function *Func : FuncLDSAccessInfo.NonKernelsWithLDSArgument) {
      ...
      if (K.contains(Func))
        continue;
      SetVector<toolchain::GlobalVariable *> Vec;
  ...
  if (AsanInstrumentLDS) {
    ...
    for (Instruction *Inst : AsanInfo.Instructions) {
      ...
      toolchain::append_range(OperandsToInstrument, InterestingOperands);
    }
...
char &toolchain::AMDGPUSwLowerLDSLegacyPassID = AMDGPUSwLowerLDSLegacy::ID;
...
ModulePass *
toolchain::createAMDGPUSwLowerLDSLegacyPass(const AMDGPUTargetMachine *TM) {
