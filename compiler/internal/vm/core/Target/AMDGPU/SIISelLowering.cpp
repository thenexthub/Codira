...
#include "vm/core/CodeGen/ByteProvider.h"
...
#include "vm/core/CodeGen/SDPatternMatch.h"
...
#include "vm/core/IR/IntrinsicsR600.h"
...
using namespace vm::core::SDPatternMatch;
    ...
    if (ME.onlyReadsMemory()) {
      if (RsrcIntr->IsImage) {
        ...
        if (!BaseOpcode->Gather4) {
          ...
          MaxNumLanes = DMask == 0 ? 1 : toolchain::popcount(DMask);
        }
      ...
      if (RsrcIntr->IsImage) {
        ...
        unsigned DMaskLanes = DMask == 0 ? 1 : toolchain::popcount(DMask);
  ...
  if (CallConv == CallingConv::AMDGPU_PS) {
    ...
    if (Subtarget->isAmdPalOS()) {
      ...
      if ((PsInputBits & 0x7F) == 0 ||
        ...
        Info->markPSInputEnabled(toolchain::countr_zero(Info->getPSInputAddr()));
    }
  } else if (IsKernel) {
...
SDValue SITargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  toolchain::SmallVector<SDValue, 6> ChainCallSpecialArgs;
  ...
  if (IsChainCallConv) {
    ...
    auto RequestedExecIt =
        toolchain::find_if(CLI.Outs, [](const ISD::OutputArg &Arg) {
  ...
  if (IsTailCall) {
    ...
    if (!IsTailCall &&
      ...
      report_fatal_error("failed to perform tail call elimination on a call "
                         "site marked musttail or on toolchain.amdgcn.cs.chain");
    }
  ...
  if (IsChainCallConv)
    toolchain::append_range(Ops, ChainCallSpecialArgs);
...
void SITargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::amdgcn_s_buffer_load: {
      // Lower toolchain.amdgcn.s.buffer.load.(i8, u8) intrinsics. First, we generate
...
SDValue SITargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  ...
  MachineFrameInfo &MFI = MF.getFrameInfo();
  // There is a call to @toolchain.returnaddress in this function
...
SDValue SITargetLowering::lowerImage(SDValue Op,
  ...
  if (BaseOpcode->Atomic) {
    ...
    DMaskLanes = BaseOpcode->Gather4 ? 4 : toolchain::popcount(DMask);
...
SDValue SITargetLowering::lowerSBuffer(EVT VT, SDLoc DL, SDValue Rsrc,
  ...
  if (!Offset->isDivergent()) {
    ...
    // Lower toolchain.amdgcn.s.buffer.load.{i16, u16} intrinsics. Initially, the
...
SDValue SITargetLowering::lowerWorkitemID(SelectionDAG &DAG, SDValue Op,
  ...
  EVT SmallVT = EVT::getIntegerVT(*DAG.getContext(), toolchain::bit_width(MaxID));
...
bool toolchain::isBoolSGPR(SDValue V) {
...
SDValue SITargetLowering::performAndCombine(SDNode *N,
  ...
  if (CRHS && VT == MVT::i32) {
    ...
    unsigned Bits = toolchain::popcount(Mask);
...
static void placeSources(ByteProvider<SDValue> &Src0,
  ...
  for (int BPI = 0; BPI < 2; BPI++) {
    ...
    for (int I = 0; I < 2; I++) {
      ...
      auto *Match = toolchain::find_if(Srcs, MatchesFirst);
      if (Match != Srcs.end()) {
    ...
    if (FirstGroup != -1) {
      ...
      auto *Match = toolchain::find_if(Srcs, MatchesSecond);
      if (Match != Srcs.end()) {
...
SDNode *SITargetLowering::adjustWritemask(MachineSDNode *&Node,
  ...
  unsigned OldBitsSet = toolchain::popcount(OldDmask);
  ...
  for (SDUse &Use : Node->uses()) {
    ...
    if (UsesTFC && Lane == TFCLane) {
      ...
      for (unsigned i = 0, Dmask = OldDmask; (i <= Lane) && (Dmask != 0); i++) {
        Comp = toolchain::countr_zero(Dmask);
        Dmask &= ~(1 << Comp);
      }
  ...
  unsigned BitsSet = toolchain::popcount(NewDmask);
  ...
  toolchain::append_range(Ops, Node->ops().take_front(DmaskIdx));
  Ops.push_back(DAG.getTargetConstant(NewDmask, SDLoc(Node), MVT::i32));
  toolchain::append_range(Ops, Node->ops().drop_front(DmaskIdx + 1));
...
void SITargetLowering::AddMemOpInit(MachineInstr &MI) const {
  ...
  if (TII->isImage(MI)) {
    ...
    unsigned ActiveLanes = TII->isGather4(MI) ? 4 : toolchain::popcount(dmask);
...
static void knownBitsForWorkitemID(const GCNSubtarget &ST,
  ...
  Known.Zero.setHighBits(toolchain::countl_zero(MaxValue));
}
  ...
  case AMDGPU::G_INTRINSIC_CONVERGENT: {
    ...
    case Intrinsic::amdgcn_groupstaticsize: {
      ...
      Known.Zero.setHighBits(
          toolchain::countl_zero(getSubtarget()->getAddressableLocalMemorySize()));
      break;
    }
    }
    break;
...
void SITargetLowering::emitExpandAtomicAddrSpacePredicate(
  ...
  //   [...]
  //   %is.shared = call i1 @toolchain.amdgcn.is.shared(ptr %addr)
  ...
  //   %is.private = call i1 @toolchain.amdgcn.is.private(ptr %int8ptr)
