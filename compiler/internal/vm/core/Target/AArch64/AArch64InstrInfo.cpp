...
#include "vm/core/CodeGen/MachineCombinerPattern.h"
...
unsigned AArch64InstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  ...
  // toolchain/lib/Target/AArch64/AArch64InstrInfo.td (default case).
...
bool toolchain::optimizeTerminators(MachineBasicBlock *MBB,
...
toolchain::examineCFlagsUse(MachineInstr &MI, MachineInstr &CmpInstr,
...
bool AArch64InstrInfo::isLdStPairSuppressed(const MachineInstr &MI) {
  return toolchain::any_of(MI.memoperands(), [](MachineMemOperand *MMO) {
...
bool AArch64InstrInfo::isHForm(const MachineInstr &MI) {
  ...
  return toolchain::any_of(MI.operands(), IsHFPR);
}
...
bool AArch64InstrInfo::isQForm(const MachineInstr &MI) {
  ...
  return toolchain::any_of(MI.operands(), IsQFPR);
}
...
bool AArch64InstrInfo::isFpOrNEON(const MachineInstr &MI) {
  ...
  return toolchain::any_of(MI.operands(), IsFPR);
}
...
void AArch64InstrInfo::copyGPRRegTuple(MachineBasicBlock &MBB,
                                       ...
                                       toolchain::ArrayRef<unsigned> Indices) const {
  const TargetRegisterInfo *TRI = &getRegisterInfo();
  unsigned NumRegs = Indices.size();
...
bool toolchain::isNZCVTouchedInInstructionRange(const MachineInstr &DefMI,
...
//  (+/-) NumBytes (* RegScale)?
static void appendOffsetComment(int NumBytes, toolchain::raw_string_ostream &Comment,
                                StringRef RegScale = {}) {
  if (NumBytes) {
...
static MCCFIInstruction createDefCFAExpression(const TargetRegisterInfo &TRI,
  ...
  toolchain::raw_string_ostream Comment(CommentBuffer);
...
MCCFIInstruction toolchain::createDefCFA(const TargetRegisterInfo &TRI,
...
MCCFIInstruction
toolchain::createCFAOffset(const TargetRegisterInfo &TRI, unsigned Reg,
...
void toolchain::emitFrameOffset(MachineBasicBlock &MBB,
...
int toolchain::isAArch64FrameOffsetLegal(const MachineInstr &MI,
                                    StackOffset &SOffset,
...
bool toolchain::rewriteAArch64FrameIndex(MachineInstr &MI, unsigned FrameRegIdx,
...
static bool getGatherLanePattern(MachineInstr &Root,
  ...
  auto Range = toolchain::seq<unsigned>(1, NumLanes - 1);
...
generateGatherLanePattern(MachineInstr &Root,
  ...
  toolchain::sort(LoadToLaneInstrs,
  ...
  auto LoadToLaneInstrsAscending = toolchain::reverse(LoadToLaneInstrs);
  ...
  auto LanesToLoadToReg0 =
      toolchain::make_range(LoadToLaneInstrsAscending.begin() + 1,
                       LoadToLaneInstrsAscending.begin() + NumLanes / 2);
  Register PrevReg = SubregToReg->getOperand(0).getReg();
  for (auto [Index, LoadInstr] : toolchain::enumerate(LanesToLoadToReg0)) {
  ...
  auto LanesToLoadToReg1 =
      toolchain::make_range(LoadToLaneInstrsAscending.begin() + NumLanes / 2 + 1,
                       LoadToLaneInstrsAscending.end());
  PrevReg = SubRegToRegInstr->getOperand(0).getReg();
  for (auto [Index, LoadInstr] : toolchain::enumerate(LanesToLoadToReg1)) {
    const MachineOperand &OffsetRegOperand = LoadInstr->getOperand(3);
    PrevReg = CreateLD1Instruction(LoadInstr, PrevReg, Index + 1,
                                   OffsetRegOperand.getReg(),
                                   OffsetRegOperand.isKill());
...
AArch64InstrInfo::getOutliningCandidateInfo(
  ...
  if (RASignCondition != SignReturnAddress::None) {
    ...
    toolchain::erase_if(RepeatedSequenceLocs, hasIllegalSPModification);
  ...
  bool AllStackInstrsSafe =
      toolchain::all_of(RepeatedSequenceLocs[0], IsSafeToFixup);
    ...
    if (!AllStackInstrsSafe ||
      ...
      if (FlagsSetInAll & MachineOutlinerMBBFlags::HasCalls) {
        erase_if(RepeatedSequenceLocs, [this, &TRI](outliner::Candidate &C) {
          ...
          return (toolchain::any_of(C, IsCall)) &&
                 (!C.isAvailableAcrossAndOutOfSeq(AArch64::LR, TRI) ||
...
bool AArch64InstrInfo::isMBBSafeToSplitToCold(
  ...
  if (toolchain::any_of(MBB, isAsmGoto) || MBB.isInlineAsmBrIndirectTarget())
    return false;
  ...
  auto containsMBB = [&MBB](const MachineJumpTableEntry &JTE) {
    return toolchain::is_contained(JTE.MBBs, &MBB);
  };
  if (MJTI != nullptr && toolchain::any_of(MJTI->getJumpTables(), containsMBB))
    return false;
...
unsigned toolchain::getBLRCallOpcode(const MachineFunction &MF) {
