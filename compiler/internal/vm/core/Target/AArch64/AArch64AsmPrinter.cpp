...
void AArch64AsmPrinter::LowerHWASAN_CHECK_MEMACCESS(const MachineInstr &MI) {
  ...
  if (!Sym) {
    ...
    if (!TM.getTargetTriple().isOSBinFormatELF())
      report_fatal_error("toolchain.hwasan.check.memaccess only supported on ELF");
...
void AArch64AsmPrinter::PrintDebugValueComment(const MachineInstr *MI,
  ...
  OS << '[';
  for (unsigned I = 0, E = toolchain::size(MI->debug_operands()); I < E; ++I) {
    if (I != 0)
      OS << ", ";
...
void AArch64AsmPrinter::LowerJumpTableDest(toolchain::MCStreamer &OutStreamer,
                                           const toolchain::MachineInstr &MI) {
...
void AArch64AsmPrinter::LowerMOPS(toolchain::MCStreamer &OutStreamer,
                                  const toolchain::MachineInstr &MI) {
...
void AArch64AsmPrinter::LowerFAULTING_OP(const MachineInstr &FaultingMI) {
  ...
  for (const MachineOperand &MO :
       toolchain::drop_begin(FaultingMI.operands(), OperandsBeginIdx)) {
    MCOperand Dest;
...
#ifndef NDEBUG
  InstsEmitted = 0;
  toolchain::scope_exit CheckMISize([&]() {
...
void AArch64AsmPrinter::recordIfImportCall(
    const toolchain::MachineInstr *BranchInst) {
