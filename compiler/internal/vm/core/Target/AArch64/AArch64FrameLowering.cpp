...
static unsigned getPrologueDeath(MachineFunction &MF, unsigned Reg) {
  ...
  // happens with the @toolchain-returnaddress intrinsic and with arguments passed in
...
void computeCalleeSaveRegisterPairs(const AArch64FrameLowering &AFL,
  ...
  bool SpillExtendedVolatile =
      IsWindows && toolchain::any_of(CSI, [](const CalleeSavedInfo &CSI) {
...
bool AArch64FrameLowering::spillCalleeSavedRegisters(
  ...
  for (const RegPairInfo &RPI : toolchain::reverse(RegPairs)) {
    ...
    Register X0Scratch;
    toolchain::scope_exit RestoreX0([&] {
...
bool AArch64FrameLowering::restoreCalleeSavedRegisters(
  ...
  auto PPRBegin = toolchain::find_if(RegPairs, IsPPR);
  ...
  auto ZPRBegin = toolchain::find_if(RegPairs, IsZPR);
...
void AArch64FrameLowering::orderFrameObjects(
  ...
  toolchain::stable_sort(FrameObjects, FrameObjectCompare);
...
void AArch64FrameLowering::emitRemarks(
  ...
  toolchain::sort(StackAccesses);
  toolchain::erase_if(StackAccesses, [](const StackAccess &S) {
  ...
  auto EmitRemark = [&](toolchain::StringRef Str) {
    ORE->emit([&]() {
