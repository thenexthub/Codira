...
using namespace vm::core::AArch64_IMM;
...
static bool tryCopyWithNegation(uint64_t Imm, bool AllowThreeSequence,
  ...
  if (!Imm || toolchain::isShiftedMask_64(Imm))
    return false;
...
static uint64_t GetRunOfOnesStartingAt(uint64_t V, uint64_t StartPosition) {
  uint64_t NumOnes = toolchain::countr_one(V >> StartPosition);
...
static uint64_t MaximallyReplicateSubImmediate(uint64_t V, uint64_t Subset) {
  ...
  for (uint64_t i = 0; i < 6; ++i) {
    ...
    uint64_t Closure = Result | toolchain::rotl<uint64_t>(Result, Rotation);
...
static uint64_t maximalLogicalImmWithin(uint64_t RemainingBits,
  ...
  uint32_t Position = toolchain::countr_zero(RemainingBits);
...
decomposeIntoOrrOfLogicalImmediates(uint64_t UImm) {
  ...
  uint32_t InitialTrailingOnes = toolchain::countr_one(UImm);
  uint64_t RotatedBits = toolchain::rotr<uint64_t>(UImm, InitialTrailingOnes);
...
static inline void expandMOVImmSimple(uint64_t Imm, unsigned BitSize,
  ...
  if (Imm != 0) {
    unsigned LZ = toolchain::countl_zero(Imm);
...
void AArch64_IMM::expandMOVImm(uint64_t Imm, unsigned BitSize,
  ...
  for (unsigned Shift = 0; Shift < BitSize; Shift += 16) {
    ...
    uint64_t RotatedImm = toolchain::rotl(UImm, 32);
