...
private:
  ...
  /// @toolchain.aarch64.neon.ld2.*, @toolchain.aarch64.neon.ld4.*, etc.
  ...
  MachineInstr *
  emitInstr(unsigned Opcode, std::initializer_list<toolchain::DstOp> DstOps,
            std::initializer_list<toolchain::SrcOp> SrcOps,
            MachineIRBuilder &MIRBuilder,
...
bool AArch64InstructionSelector::selectMOPS(MachineInstr &GI,
  ...
  case TargetOpcode::G_MEMMOVE:
    ...
    break;
  case TargetOpcode::G_MEMSET:
    // For tagged memset see toolchain.aarch64.mops.memset.tag
...
MachineInstr *AArch64InstructionSelector::emitInstr(
    unsigned Opcode, std::initializer_list<toolchain::DstOp> DstOps,
    std::initializer_list<toolchain::SrcOp> SrcOps, MachineIRBuilder &MIRBuilder,
...
bool AArch64InstructionSelector::selectIntrinsicWithSideEffects(
  ...
  case Intrinsic::aarch64_mops_memset_tag: {
    // Transform
    //    %dst:gpr(p0) = \
    //      G_INTRINSIC_W_SIDE_EFFECTS intrinsic(@toolchain.aarch64.mops.memset.tag),
...
AArch64InstructionSelector::selectExtendedSHL(
  ...
  if (OffsetOpc == TargetOpcode::G_MUL) {
    if (!toolchain::has_single_bit<uint32_t>(ImmVal))
      return std::nullopt;
...
static void fixupPHIOpBanks(MachineInstr &MI, MachineRegisterInfo &MRI,
  ...
  for (MachineOperand &MO : toolchain::drop_begin(MI.operands())) {
    if (!MO.isReg())
      continue;
