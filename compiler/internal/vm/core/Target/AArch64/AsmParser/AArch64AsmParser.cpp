...
public:
  ...
  bool isMovWSymbol(ArrayRef<AArch64::Specifier> AllowedModifiers) const {
    ...
    return toolchain::is_contained(AllowedModifiers, ELFSpec);
  }
...
static void setRequiredFeatureString(FeatureBitset FBS, std::string &Str) {
  ...
  if (FBS[AArch64::HasV8_1aOps])
    ...
    Str += !ExtMatches.empty() ? toolchain::join(ExtMatches, ", ") : "(unknown)";
  }
}
...
bool AArch64AsmParser::parseOperand(OperandVector &Operands, bool isCondCode,
  ...
  case AsmToken::Equal: {
    ...
    if (isa<MCConstantExpr>(SubExprVal)) {
      ...
      while (Imm > 0xFFFF && toolchain::countr_zero(Imm) >= 16) {
        ShiftAmt += 16;
        Imm >>= 16;
      }
...
static void ExpandCryptoAEK(const AArch64::ArchInfo &ArchInfo,
  ...
  const bool NoCrypto = toolchain::is_contained(RequestedExtensions, "nocrypto");
  const bool Crypto = toolchain::is_contained(RequestedExtensions, "crypto");
...
bool AArch64AsmParser::parseDirectiveArch(SMLoc L) {
  ...
  for (auto Name : RequestedExtensions) {
    ...
    auto It = toolchain::find_if(ExtensionMap, [&Name](const auto &Extension) {
...
bool AArch64AsmParser::parseDirectiveCPU(SMLoc L) {
  ...
  const toolchain::AArch64::ArchInfo *CpuArch = toolchain::AArch64::getArchForCpu(CPU);
  if (!CpuArch) {
...
bool AArch64AsmParser::parseDirectiveAeabiSubSectionHeader(SMLoc L) {
  ...
  // declared before.
  if (Parser.getTok().is(toolchain::AsmToken::EndOfStatement)) {
    if (SubsectionExists) {
  ...
  if (Parser.getTok().isNot(toolchain::AsmToken::EndOfStatement)) {
