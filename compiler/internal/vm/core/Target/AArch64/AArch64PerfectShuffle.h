...
//===----------------------------------------------------------------------===//
//
// This file, which was autogenerated by toolchain-PerfectShuffle, contains data
...
inline unsigned getPerfectShuffleCost(toolchain::ArrayRef<int> M) {
  assert(M.size() == 4 && "Expected a 4 entry perfect shuffle");

  // Special case zero-cost nop copies, from either LHS or RHS.
  if (toolchain::all_of(toolchain::enumerate(M), [](const auto &E) {
