...
static Function *getOrCreateFrameHelper(Module *M, MachineModuleInfo *MMI,
  ...
  case FrameHelperType::PrologFrame: {
    ...
    auto LRIdx = std::distance(Regs.begin(), toolchain::find(Regs, AArch64::LR));
...
static bool shouldUseFrameHelper(MachineBasicBlock &MBB,
  ...
  if (!toolchain::is_contained(Regs, AArch64::LR))
    return false;
...
ModulePass *toolchain::createAArch64LowerHomogeneousPrologEpilogPass() {
