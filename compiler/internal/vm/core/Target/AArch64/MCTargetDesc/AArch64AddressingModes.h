...
static inline bool processLogicalImmediate(uint64_t Imm, unsigned RegSize,
  ...
  if (isShiftedMask_64(Imm)) {
    I = toolchain::countr_zero(Imm);
    assert(I < 64 && "undefined behavior");
    CTO = toolchain::countr_one(Imm >> I);
  } else {
    Imm |= ~Mask;
    if (!isShiftedMask_64(~Imm))
      return false;

    unsigned CLO = toolchain::countl_one(Imm);
    I = 64 - CLO;
    CTO = CLO + toolchain::countr_one(Imm) - (64 - Size);
  }
...
static inline uint64_t decodeLogicalImmediate(uint64_t val, unsigned regSize) {
  ...
  int len = 31 - toolchain::countl_zero((N << 6) | (~imms & 0x3f));
...
static inline bool isSVEMaskOfIdenticalElements(int64_t Imm) {
  ...
  return toolchain::all_equal(Parts);
}
