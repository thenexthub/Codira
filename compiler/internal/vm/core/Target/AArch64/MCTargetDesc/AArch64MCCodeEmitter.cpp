...
AArch64MCCodeEmitter::EncodeRegMul_MinMax(const MCInst &MI, unsigned OpIdx,
                                          SmallVectorImpl<MCFixup> &Fixups,
                                          const MCSubtargetInfo &STI) const {
  assert(toolchain::isPowerOf2_32(Multiple) && "Multiple is not a power of 2");
...
void AArch64MCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  support::endian::write<uint32_t>(CB, Binary, toolchain::endianness::little);
...
MCCodeEmitter *toolchain::createAArch64MCCodeEmitter(const MCInstrInfo &MCII,
                                                MCContext &Ctx) {
