...
void AArch64Arm64ECCallLowering::getThunkArgTypes(
  ...
  for (unsigned E = FT->getNumParams(); I != E; ++I) {
    ...
    // https://reviews.toolchain.org/D132926
...
ThunkArgInfo AArch64Arm64ECCallLowering::canonicalizeThunkType(
  ...
  auto bitcast = [this](Type *Arm64Ty, uint64_t SizeInBytes) {
    return ThunkArgInfo{Arm64Ty,
                        toolchain::Type::getIntNTy(M->getContext(), SizeInBytes * 8),
  ...
  if (T->isHalfTy()) {
    // Prefix with `toolchain` since MSVC doesn't specify `_Float16`
    Out << "__llvm_h__";
    return direct(T);
  }
...
Function *AArch64Arm64ECCallLowering::buildExitThunk(FunctionType *FT,
  ...
  toolchain::raw_svector_ostream ExitThunkStream(ExitThunkName);
  ...
  for (auto [Arg, X64ArgType, ArgTranslation] : toolchain::zip_equal(
...
Function *AArch64Arm64ECCallLowering::buildEntryThunk(Function *F) {
  ...
  toolchain::raw_svector_ostream EntryThunkStream(EntryThunkName);
  FunctionType *Arm64Ty, *X64Ty;
  SmallVector<ThunkArgTranslation> ArgTranslations;
...
std::optional<std::string> getArm64ECMangledFunctionName(GlobalValue &GV) {
  ...
  return toolchain::getArm64ECMangledFunctionName(GV.getName());
}
...
Function *AArch64Arm64ECCallLowering::buildGuestExitThunk(Function *F) {
  toolchain::raw_null_ostream NullThunkName;
  FunctionType *Arm64Ty, *X64Ty;
  SmallVector<ThunkArgTranslation> ArgTranslations;
  getThunkType(F->getFunctionType(), F->getAttributes(),
  ...
  SmallVector<Value *> Args(toolchain::make_pointer_range(GuestExit->args()));
...
bool AArch64Arm64ECCallLowering::runOnModule(Module &Mod) {
  ...
  if (!ThunkMapping.empty()) {
    ...
    Constant *ThunkMappingArray = ConstantArray::get(
        toolchain::ArrayType::get(ThunkMappingArrayElems[0]->getType(),
...
ModulePass *toolchain::createAArch64Arm64ECCallLoweringPass() {
