...
AArch64LoadStoreOpt::mergePairedInsns(MachineBasicBlock::iterator I,
  ...
  if (RenameReg) {
    ...
    for (auto &MI :
         iterator_range<MachineInstrBundleIterator<toolchain::MachineInstr>>(
  ...
  auto CopyImplicitOps = [&](MachineBasicBlock::iterator MI1,
    ...
    for (const MachineOperand &MO :
         toolchain::drop_begin(MI1->operands(), MI1->getDesc().getNumOperands()))
      if (MO.isReg() && MO.isImplicit() && MO.isDef())
        Ops.insert(MO.getReg());
    for (const MachineOperand &MO :
         toolchain::drop_begin(MI2->operands(), MI2->getDesc().getNumOperands()))
      if (MO.isReg() && MO.isImplicit() && MO.isDef())
        Ops.insert(MO.getReg());
    for (auto Op : Ops)
...
// using alignTo from include/toolchain/Support/MathExtras.h.
...
FunctionPass *toolchain::createAArch64LoadStoreOptimizationPass() {
