...
static void transferImpOps(MachineInstr &OldMI, MachineInstrBuilder &UseMI,
  ...
  for (const MachineOperand &MO :
       toolchain::drop_begin(OldMI.operands(), Desc.getNumOperands())) {
...
static MachineInstr *createCallWithOps(MachineBasicBlock &MBB,
  ...
  for (const MachineOperand &MO :
       toolchain::drop_begin(MBBI->operands(), RegMaskStartIdx))
    Call->addOperand(MO);
...
bool AArch64ExpandPseudo::expandMultiVecPseudo(
  ...
  unsigned Opc;
  if (toolchain::is_contained(ContiguousRange, Tuple.asMCReg())) {
    Opc = ContiguousOp;
  } else if (toolchain::is_contained(StridedRange, Tuple.asMCReg())) {
    Opc = StridedOpc;
  } else
...
FunctionPass *toolchain::createAArch64ExpandPseudoPass() {
