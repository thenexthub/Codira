...
#include "vm/core/IR/Function.h" // To access function attributes.
...
// https://github.com/toolchain/toolchain-project/issues/114425
...
void AArch64DAGToDAGISel::SelectMultiVectorMoveZ(SDNode *N, unsigned NumVecs,
  ...
  // The array to vector: toolchain.aarch64.sme.readz.<h/v>.<sz>(slice)
  // The tile to vector: toolchain.aarch64.sme.readz.<h/v>.<sz>(tile, slice)
...
static bool isBitfieldExtractOpFromAnd(SelectionDAG *CurDAG, SDNode *N,
  ...
  LSB = SrlImm;
  MSB = SrlImm +
        (VT == MVT::i32 ? toolchain::countr_one<uint32_t>(AndImm)
                        : toolchain::countr_one<uint64_t>(AndImm)) -
        1;
  if (ClampMSB)
...
static bool isBitfieldPositioningOpFromAnd(SelectionDAG *CurDAG, SDValue Op,
  ...
  DstLSB = toolchain::countr_zero(NonZeroBits);
  Width = toolchain::countr_one(NonZeroBits >> DstLSB);
...
static bool isSeveralBitsPositioningOpFromShl(const uint64_t ShlImm, SDValue Op,
  ...
  if (isMask_64(ShiftedAndImm)) {
    ...
    // to not extract them.
    Width = toolchain::countr_one(ShiftedAndImm);
    DstLSB = ShlImm;
...
static bool tryBitfieldInsertOpFromOrAndImm(SDNode *N, SelectionDAG *CurDAG) {
  ...
  int LSB = toolchain::countr_one(NotKnownZero);
...
static bool isWorthFoldingIntoOrrWithShift(SDValue Dst, SelectionDAG *CurDAG,
  ...
  if (isOpcWithIntImmediate(Dst.getNode(), ISD::AND, AndImm) &&
    ...
    if (isOpcWithIntImmediate(DstOp0.getNode(), ISD::SRL, SrlImm)) {
      uint64_t NumTrailingZeroInShiftedMask = toolchain::countr_zero(AndImm);
      if ((SrlImm + NumTrailingZeroInShiftedMask) < SizeInBits) {
        unsigned MaskWidth =
            toolchain::countr_one(AndImm >> NumTrailingZeroInShiftedMask);
        unsigned UBFMOpc =
...
static bool tryBitfieldInsertOpFromOr(SDNode *N, const APInt &UsefulBits,
  ...
  if (And0.hasOneUse() && And1.hasOneUse() &&
    ...
    unsigned LSB = toolchain::countr_zero(Mask1Imm);
...
bool AArch64DAGToDAGISel::tryShiftAmountMod(SDNode *N) {
  ...
  if (ShiftAmt->getOpcode() == ISD::ADD || ShiftAmt->getOpcode() == ISD::SUB) {
    ...
    if ((unsigned)toolchain::countr_one(MaskImm) < Bits)
      return false;
...
void AArch64DAGToDAGISel::SelectTagP(SDNode *N) {
  assert(isa<ConstantSDNode>(N->getOperand(3)) &&
         "toolchain.aarch64.tagp third argument must be an immediate");
...
FunctionPass *toolchain::createAArch64ISelDag(AArch64TargetMachine &TM,
                                         CodeGenOptLevel OptLevel) {
