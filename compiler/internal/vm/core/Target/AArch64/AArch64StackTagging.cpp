...
public:
  ...
  bool addRange(uint64_t Start, uint64_t End, Instruction *Inst) {
    auto I =
        toolchain::lower_bound(Ranges, Start, [](const Range &LHS, uint64_t RHS) {
          return LHS.End <= RHS;
        });
...
FunctionPass *toolchain::createAArch64StackTaggingPass(bool IsOptNone) {
...
bool AArch64StackTagging::runOnFunction(Function &Fn) {
  ...
  for (auto &I : SInfo.AllocasToInstrument) {
    ...
    assert(Info.AI && SIB.getAllocaInterestingness(*Info.AI) ==
                          toolchain::memtag::AllocaInterestingness::kInteresting);
