...
#include "vm/core/CodeGen/CostTable.h"
...
#include "vm/core/TargetParser/AArch64TargetParser.h"
...
static bool hasPossibleIncompatibleOps(const Function *F,
  ...
  for (const BasicBlock &BB : *F) {
    for (const Instruction &I : BB) {
      ...
      // @toolchain.aarch64.* or @toolchain.gather/scatter intrinsics). We can assume that
...
bool AArch64TTIImpl::areTypesABICompatible(const Function *Caller,
  ...
  // distinguished in IR.
  if (ST->useSVEForFixedLengthVectors() && toolchain::any_of(Types, [](Type *Ty) {
...
struct SVEIntrinsicInfo {
  ...
  // e.g. toolchain.aarch64.sve.add pg, op1, op2
  //        with IID ==> toolchain.aarch64.sve.add_u
  ...
  // e.g. toolchain.aarch64.sve.neg inactive, pg, op
  ...
  // e.g. toolchain.aarch64.sve.fcvtnt inactive, pg, op
  ...
  // e.g. toolchain.aarch64.sve.add_u pg, op1, op2
  ...
  // e.g. toolchain.aarch64.sve.prf pg, ptr        (GPIndex = 0)
  //      toolchain.aarch64.sve.st1 data, pg, ptr  (GPIndex = 1)
  ...
  // e.g. toolchain.aarch64.sve.cmpeq pg, op1, op2
  //      toolchain.aarch64.sve.ld1 pg, ptr
...
instCombineSVEVectorFuseMulAddSub(InstCombiner &IC, IntrinsicInst &II,
  ...
  if (II.getType()->isFPOrFPVectorTy()) {
    toolchain::FastMathFlags FAddFlags = II.getFastMathFlags();
...
getAppleRuntimeUnrollPreferences(Loop *L, ScalarEvolution &SE,
  ...
  if (findStringMetadataForLoop(L, "toolchain.loop.isvectorized"))
    return;
  ...
  if (!Term || !Term->isConditional() || Preds.size() == 1 ||
      !toolchain::is_contained(Preds, Header) ||
...
void AArch64TTIImpl::getUnrollingPreferences(
  ...
  bool IsVectorized = getBooleanLoopAttribute(L, "toolchain.loop.isvectorized");
  InstructionCost Cost = 0;
  for (auto *BB : L->getBlocks()) {
    for (auto &I : *BB) {
...
AArch64TTIImpl::getArithmeticReductionCost(unsigned Opcode, VectorType *ValTy,
  ...
  // OR: toolchain/test/CodeGen/AArch64/reduce-or.ll
  // XOR: toolchain/test/CodeGen/AArch64/reduce-xor.ll
  // AND: toolchain/test/CodeGen/AArch64/reduce-and.ll
...
InstructionCost AArch64TTIImpl::getPartialReductionCost(
  ...
  if (TypeSize::isKnownLT(InputVectorType->getPrimitiveSizeInBits(),
    ...
    //  https://github.com/toolchain/toolchain-project/pull/147302
    Cost *= 2;
  ...
  if (ST->isSVEorStreamingSVEAvailable() && !IsUSDot) {
    ...
    if (AccumLT.second.getScalarType() == MVT::i64 &&
      ...
      // after https://github.com/toolchain/toolchain-project/pull/147302 has landed.
      return Cost;
  }
...
AArch64TTIImpl::getShuffleCost(TTI::ShuffleKind Kind, VectorType *DstTy,
  ...
  if (!Mask.empty() && isa<FixedVectorType>(SrcTy) && LT.second.isVector() &&
    ...
    // Check for LD3/LD4 instructions, which are represented in toolchain IR as
    ...
    // Check for ST3/ST4 instructions, which are represented in toolchain IR as
