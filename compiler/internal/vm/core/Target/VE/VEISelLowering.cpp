...
void VETargetLowering::initSPUActions() {
  ...
  /// Conversion {
  // VE doesn't have instructions for fp<->uint, so expand them by toolchain
...
SDValue VETargetLowering::lowerATOMIC_SWAP(SDValue Op,
  ...
  // Otherwise, let toolchain legalize it.
  return Op;
}
...
static SDValue lowerLoadI1(SDValue Op, SelectionDAG &DAG) {
  ...
  if (MemVT == MVT::v256i1 || MemVT == MVT::v4i64) {
    ...
    // Otherwise, ask toolchain to expand it.
    return SDValue();
  }
}
...
SDValue VETargetLowering::getPICODEumpTableRelocBase(SDValue Table,
                                                   SelectionDAG &DAG) const {
  assert(isPositionIndependent());
  SDLoc DL(Table);
...
VETargetLowering::emitEHSjLjSetJmp(MachineInstr &MI,
  ...
  // For `v = call @toolchain.eh.sjlj.setjmp(buf)`, we generate following
  // instructions.  SP/FP must be saved in jmpbuf before `toolchain.eh.sjlj.setjmp`.
  //
  // ThisMBB:
  ...
  // SP/FP are already stored in jmpbuf before `toolchain.eh.sjlj.setjmp`.
...
VETargetLowering::emitEHSjLjLongJmp(MachineInstr &MI,
  ...
  // For `call @toolchain.eh.sjlj.longjmp(buf)`, we generate following instructions.
  //
  // ThisMBB:
  //   %fp = load buf[0]
  //   %jmp = load buf[1]
  //   %s10 = buf        ; Store an address of buf to SX10 for RestoreMBB
  //   %sp = load buf[2] ; generated by toolchain.eh.sjlj.setjmp.
  //   jmp %jmp
...
SDValue VETargetLowering::combineSelectCC(SDNode *N,
  ...
  case MVT::f128:
    break;
  default:
    // Return SDValue to let toolchain handle other types.
    return SDValue();
  }
