...
bool HexagonBitSimplify::hasTiedUse(unsigned Reg, MachineRegisterInfo &MRI,
  ...
  if (!PreserveTiedOps)
    return false;
  return toolchain::any_of(MRI.use_operands(Reg),
...
bool DeadCodeElimination::runOnNode(MachineDomTreeNode *N) {
  ...
  for (MachineInstr &MI : toolchain::reverse(*B))
    Instrs.push_back(&MI);
...
void HexagonLoopRescheduling::moveGroup(InstrGroup &G, MachineBasicBlock &LB,
  ...
  for (const MachineInstr *SI : toolchain::reverse(G.Ins)) {
...
bool HexagonLoopRescheduling::processLoop(LoopCand &C) {
  ...
  // in this loop.
  for (MachineInstr &MI : toolchain::reverse(*C.LB)) {
    if (MI.isTerminator())
      continue;
    if (MI.isPHI())
      break;
    ...
    for (auto UI = MRI->use_begin(DefR), UE = MRI->use_end(); UI != UE; ++UI) {
      ...
      if (UseI->getParent() == C.LB) {
        if (UseI->isPHI()) {
          ...
          if (!toolchain::is_contained(ShufIns, UseI))
            BadUse = true;
        }
      } else {
  ...
  for (unsigned i = 0, n = ShufIns.size(); i < n; ++i) {
    ...
    if (toolchain::none_of(Phis, LoopInpEq))
      continue;
  ...
  for (InstrGroup &G : Groups) {
    ...
    auto F = toolchain::find_if(Phis, LoopInpEq);
    if (F == Phis.end())
      continue;
    unsigned PrehR = 0;
...
FunctionPass *toolchain::createHexagonLoopRescheduling() {
  return new HexagonLoopRescheduling();
}

FunctionPass *toolchain::createHexagonBitSimplify() {
