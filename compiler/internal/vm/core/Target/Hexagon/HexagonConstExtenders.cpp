...
namespace {
  ...
  private:
    struct Register {
      Register() = default;
      Register(toolchain::Register R, unsigned S) : Reg(R), Sub(S) {}
      ...
      Register &operator=(const MachineOperand &Op) {
        if (Op.isReg()) {
          ...
          Reg = toolchain::Register::index2StackSlot(Op.getIndex());
        }
        return *this;
      }
      bool isVReg() const {
...
void HCE::assignInits(const ExtRoot &ER, unsigned Begin, unsigned End,
  ...
  while (true) {
    ...
    auto BestIt = toolchain::max_element(
...
HCE::Register HCE::insertInitializer(Loc DefL, const ExtenderInit &ExtI) {
  toolchain::Register DefR = MRI->createVirtualRegister(&Hexagon::IntRegsRegClass);
  ...
  if (Ex.Rs.isSlot()) {
    ...
    if (Ex.trivial()) {
      ...
      if (HST->useCompound()) {
        ...
        toolchain::Register TmpR = MRI->createVirtualRegister(&Hexagon::IntRegsRegClass);
...
bool HCE::replaceInstrExpr(const ExtDesc &ED, const ExtenderInit &ExtI,
  ...
  if (ExtOpc == Hexagon::A2_tfrsi) {
    ...
    if (Diff > 32767) {
      ...
      uint32_t A = std::min<uint32_t>(R.Align, 1u << toolchain::countr_zero(UD));
      D &= ~(A-1);
    }
...
bool HCE::runOnMachineFunction(MachineFunction &MF) {
  ...
  collect(MF);
  toolchain::sort(Extenders, [this](const ExtDesc &A, const ExtDesc &B) {
    ExtValue VA(A), VB(B);
    if (VA != VB)
      return VA < VB;
...
FunctionPass *toolchain::createHexagonConstExtenders() {
