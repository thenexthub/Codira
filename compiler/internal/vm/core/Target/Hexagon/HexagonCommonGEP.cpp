...
void HexagonCommonGEP::processGepInst(GetElementPtrInst *GepI,
  ...
  for (Use &U : toolchain::drop_begin(GepI->indices())) {
    Value *Op = U;
...
static void nodes_for_root(GepNode *Root, NodeChildrenMap &NCM,
    ...
    while (!Work.empty()) {
      ...
      if (CF != NCM.end()) {
        toolchain::append_range(Work, CF->second);
...
void HexagonCommonGEP::common() {
  ...
  ProjMap PM;
  for (const NodeSet &S : EqRel) {
    GepNode *Min = *toolchain::min_element(S, NodeOrder);
...
void HexagonCommonGEP::getAllUsersForNode(GepNode *Node, ValueVect &Values,
  ...
  while (!Work.empty()) {
    ...
    if (CF != NCM.end()) {
      NodeVect &Cs = CF->second;
      toolchain::append_range(Work, Cs);
    }
  }
}
...
void HexagonCommonGEP::removeDeadCode() {
  ...
  for (Value *V : toolchain::reverse(BO)) {
    BasicBlock *B = cast<BasicBlock>(V);
    ValueVect Ins;
    for (Instruction &I : toolchain::reverse(*B))
      Ins.push_back(&I);
    for (Value *I : Ins) {
