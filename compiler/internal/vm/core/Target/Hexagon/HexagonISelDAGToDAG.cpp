...
void HexagonDAGToDAGISel::ppAddrRewriteAndSrl(std::vector<SDNode*> &&Nodes) {
  ...
  for (SDNode *N : Nodes) {
    ...
    // Examine the mask.
    uint32_t TZ = toolchain::countr_zero(Mask);
    uint32_t M1 = toolchain::countr_one(Mask >> TZ);
    uint32_t LZ = toolchain::countl_zero(Mask);
...
void HexagonDAGToDAGISel::rebalanceAddressTrees() {
  for (SDNode &Node : toolchain::make_early_inc_range(CurDAG->allnodes())) {
    SDNode *N = &Node;
