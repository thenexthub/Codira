...
HexagonTargetLowering::validateConstPtrAlignment(SDValue Ptr, Align NeedAlign,
  ...
  Align HaveAlign =
      Addr != 0 ? Align(1ull << toolchain::countr_zero(Addr)) : NeedAlign;
  if (HaveAlign >= NeedAlign)
    return true;

  static int DK_MisalignedTrap = toolchain::getNextAvailablePluginDiagnosticKind();
...
HexagonTargetLowering::LowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG)
  ...
  unsigned F = toolchain::find_if(AM, [](int M) { return M >= 0; }) - AM.data();
  if (F == AM.size())
...
HexagonTargetLowering::buildVector32(ArrayRef<SDValue> Elem, const SDLoc &dl,
  ...
  if (AllConst &&
      toolchain::all_of(Consts, [](ConstantInt *CI) { return CI->isZero(); }))
...
SDValue
HexagonTargetLowering::getPICODEumpTableRelocBase(SDValue Table,
                                                SelectionDAG &DAG) const {
