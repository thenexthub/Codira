...
private:
  ...
  struct ByteSpan {
    ...
    // from somewhere within a toolchain::Value, and placing it at a given offset
    // within the span.
    //
    // The sequence of bytes from toolchain:Value is represented by Segment.
...
template <typename Pred, typename T> void erase_if(T &&container, Pred p) {
  toolchain::erase_if(std::forward<T>(container), p);
}
...
auto AlignVectors::createLoadGroups(const AddrList &Group) const -> MoveList {
  ...
  auto tryAddTo = [&](const AddrInfo &Info, MoveGroup &Move) {
    ...
    if (!toolchain::all_of(Deps, isSafeToCopyAtBase))
      return false;

    Move.Main.push_back(Info.Inst);
    toolchain::append_range(Move.Deps, Deps);
    return true;
  };
...
auto AlignVectors::createStoreGroups(const AddrList &Group) const -> MoveList {
  ...
  if (!VADoFullStores) {
    erase_if(StoreGroups, [this](const MoveGroup &G) {
      return G.IsHvx && toolchain::all_of(G.Main, [this](Instruction *S) {
...
auto AlignVectors::realignGroup(const MoveGroup &Move) -> bool {
  ...
  auto getMaxOf = [](auto Range, auto GetValue) {
    return *toolchain::max_element(Range, [&GetValue](auto &A, auto &B) {
...
auto AlignVectors::run() -> bool {
  ...
  for (auto &G : AddrGroups) {
    toolchain::append_range(LoadGroups, createLoadGroups(G.second));
    toolchain::append_range(StoreGroups, createStoreGroups(G.second));
  }
...
inline Value *getReinterpretiveCast_i16_to_i32(const HexagonVectorCombine &HVC,
  ...
  Constant *Mask = toolchain::ConstantDataVector::get(Ctx, shuffleMask);
  Value *CastShuffle =
...
Value *HvxIdioms::processVGather(Instruction &In) const {
  ...
  assert(InpTy && "Cannot handle no vector type for toolchain.gather");
  [[maybe_unused]] auto *ElemTy =
      dyn_cast<PointerType>(InpTy->getElementType());
  assert(ElemTy && "toolchain.gather needs vector of ptr argument");
  auto *F = In.getFunction();
  LLVMContext &Ctx = F->getContext();
  LLVM_DEBUG(dbgs() << "\n[Process gather](" << In << ")\n"
                    << *In.getParent() << "\n");
  ...
  assert(dyn_cast<VectorType>(In.getOperand(2)->getType()) &&
         "toolchain.gather needs vector for mask");
  IRBuilder Builder(In.getParent(), In.getIterator(),
                    InstSimplifyFolder(HVC.DL));
  ...
  assert(DstType && "Cannot handle non vector dst type for toolchain.gather");
  ...
  if (Qual == HvxIdioms::LdSt || Qual == HvxIdioms::Arithmetic) {
    ...
    assert(IndexType && "Cannot handle non vector index type for toolchain.gather");
    ...
    LLVM_DEBUG({
      ...
      assert(DstInpTy && "Cannot handle no vector type for toolchain.scatter");
      ...
      assert(DstElemTy && "toolchain.scatter needs vector of ptr argument");
...
Value *HvxIdioms::processMStore(Instruction &In) const {
  [[maybe_unused]] auto *InpTy =
      dyn_cast<VectorType>(In.getOperand(0)->getType());
  assert(InpTy && "Cannot handle no vector type for toolchain.masked.store");
  ...
  assert(CI && "Expected toolchain.masked.store to be a call");
...
Value *HvxIdioms::processMLoad(Instruction &In) const {
  ...
  assert(InpTy && "Cannot handle non vector type for toolchain.masked.store");
  LLVM_DEBUG(dbgs() << "\n[Process mload](" << In << ")\n"
                    << *In.getParent() << "\n");
  LLVM_DEBUG(dbgs() << "  Input type(" << *InpTy << ") elements("
                    << HVC.length(InpTy) << ") VecLen(" << HVC.getSizeOf(InpTy)
                    ...
                    << InpTy->getScalarSizeInBits() << ")bits\n");
  auto *CI = dyn_cast<CallBase>(&In);
  assert(CI && "Expected to be a call to toolchain.masked.load");
...
auto HexagonVectorCombine::calculatePointerDifference(Value *Ptr0,
  ...
  struct Builder : IRBuilder<> {
    ...
    ~Builder() {
      for (Instruction *I : toolchain::reverse(ToErase))
        I->eraseFromParent();
    }
...
auto HexagonVectorCombine::isSafeToMoveBeforeInBB(const Instruction &In,
  ...
  for (auto It = Range.first; It != Range.second; ++It) {
    ...
    if (toolchain::is_contained(IgnoreInsts, &I))
      continue;
...
FunctionPass *toolchain::createHexagonVectorCombineLegacyPass() {
