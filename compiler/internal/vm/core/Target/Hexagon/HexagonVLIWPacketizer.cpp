...
bool HexagonPacketizer::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MB : MF) {
    for (MachineInstr &MI : toolchain::make_early_inc_range(MB))
      if (MI.isKill())
        MB.erase(&MI);
  }
...
void HexagonPacketizerList::unpacketizeSoloInstrs(MachineFunction &MF) {
  for (auto &B : MF) {
    ...
    for (MachineInstr &MI : toolchain::make_early_inc_range(B.instrs())) {
      if (MI.isBundle())
...
FunctionPass *toolchain::createHexagonPacketizer(bool Minimal) {
