...
MachineInstr *HexagonInstrInfo::findLoopInstr(MachineBasicBlock *BB,
  ...
  for (MachineBasicBlock *PB : BB->predecessors()) {
    ...
    if (PB == BB)
      continue;
    for (MachineInstr &I : toolchain::reverse(PB->instrs())) {
...
bool HexagonInstrInfo::isCompoundBranchInstr(const MachineInstr &MI) const {
  return getType(MI) == HexagonII::TypeCODE && MI.isBranch();
}
...
bool HexagonInstrInfo::isDependent(const MachineInstr &ProdMI,
  ...
  for (auto &RegA : DefsA)
    for (auto &RegB : UsesB) {
      ...
      if (RegA.isPhysical() && toolchain::is_contained(HRI.subregs(RegA), RegB))
        return true;

      if (RegB.isPhysical() && toolchain::is_contained(HRI.subregs(RegB), RegA))
        return true;
    }
