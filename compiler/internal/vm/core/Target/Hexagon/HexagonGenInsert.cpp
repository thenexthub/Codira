...
void OrderedRegisterList::insert(unsigned VR) {
  iterator L = toolchain::lower_bound(Seq, VR, Ord);
  if (L == Seq.end())
    Seq.push_back(VR);
  else
    Seq.insert(L, VR);
...
void HexagonGenInsert::selectCandidates() {
  ...
  for (IFMapType::iterator I = IFMap.begin(); I != End; ++I) {
    ...
    IFListType::iterator MinI = toolchain::min_element(LL, IFO);
    assert(MinI != LL.end());
...
FunctionPass *toolchain::createHexagonGenInsert() {
