    ...
    if (!RelaxNVChecks) {
      ...
      if (ProducerPredInfo.isPredicated() &&
          (!ConsumerPredInfo.isPredicated() ||
           toolchain::HexagonMCInstrInfo::getType(MCII, ConsumerInst) ==
               HexagonII::TypeNCODE)) {
        reportNote(
            ProducerInst->getLoc(),
...
bool HexagonMCChecker::checkValidTmpDst() {
  ...
  unsigned HasTmpCount =
      toolchain::count_if(HexagonMCInstrInfo::bundleInstructions(MCII, MCB), HasTmp);
...
void HexagonMCChecker::reportNote(SMLoc Loc, toolchain::Twine const &Msg) {
  if (ReportErrors) {
