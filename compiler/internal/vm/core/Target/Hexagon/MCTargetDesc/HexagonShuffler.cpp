...
public:
  HexagonBid() = default;
  HexagonBid(unsigned B) { Bid = B ? MAX / toolchain::popcount(B) : 0; }
...
unsigned HexagonResource::setWeight(unsigned s) {
  ...
  unsigned Ctpop = toolchain::popcount(Units);
  unsigned Cttz = toolchain::countr_zero(Units);
...
bool HexagonShuffler::ValidResourceUsage(HexagonPacketSummary const &Summary) {
  ...
  toolchain::stable_sort(*ShuffledPacket, HexagonInstr::lessCVI);
    ...
    if (HexagonMCInstrInfo::getDesc(MCII, ID).mayStore()) {
      if (!Summary.store0) {
        const bool PacketHasNoOnlySlot0 =
            toolchain::none_of(insts(), [&](HexagonInstr const &I) {
...
static std::string SlotMaskToText(unsigned SlotMask) {
    ...
    return toolchain::join(Slots, StringRef(", "));
}
...
void HexagonShuffler::restrictPreferSlot3(HexagonPacketSummary const &Summary,
  ...
  const bool HasOnlySlot3 = toolchain::any_of(insts(), [&](HexagonInstr const &I) {
...
HexagonShuffler::tryAuction(HexagonPacketSummary const &Summary) {
  ...
  toolchain::stable_sort(PacketResult, HexagonInstr::lessCore);

  const bool ValidSlots =
      toolchain::all_of(insts(PacketResult), [&AuctionCore](HexagonInstr const &I) {
  ...
  LLVM_DEBUG(
    ...
    for (HexagonInstr const &ISJ : insts(PacketResult))
      dbgs() << "\t" << HexagonMCInstrInfo::getName(MCII, *ISJ.ID) << ": "
             << toolchain::format_hex(ISJ.Core.getUnits(), 4, true) << "\n";
  );
