...
char &toolchain::HexagonExpandCondsetsID = HexagonExpandCondsets::ID;
...
void HexagonExpandCondsets::updateDeadsInRange(Register Reg, LaneBitmask LM,
  ...
  auto Dominate = [this] (SetVector<MachineBasicBlock*> &Defs,
    ...
    SetVector<MachineBasicBlock *> Work(toolchain::from_range, Dest->predecessors());
...
bool HexagonExpandCondsets::canMoveMemTo(MachineInstr &TheI, MachineInstr &ToI,
  ...
  for (MachineInstr &MI : toolchain::make_range(std::next(StartI), EndI)) {
...
void HexagonExpandCondsets::renameInRange(RegisterRef RO, RegisterRef RN,
  ...
  for (MachineInstr &MI : toolchain::make_range(First, End)) {
...
bool HexagonExpandCondsets::predicate(MachineInstr &TfrI, bool Cond,
  ...
  bool PredValid = true;
  for (MachineInstr &MI : toolchain::make_range(std::next(DefIt), TfrIt)) {
...
//===----------------------------------------------------------------------===//
FunctionPass *toolchain::createHexagonExpandCondsets() {
