...
FunctionPass *toolchain::createHexagonCallFrameInformation() {
...
void HexagonFrameLowering::insertCFIInstructionsAt(MachineBasicBlock &MBB,
  ...
  for (MCPhysReg Reg : RegsToMove) {
    ...
    if (Reg < Hexagon::D0 || Reg > Hexagon::D15) {
      ...
      // cfi_offsets.
      // The only reason, we are split double regs is, toolchain-mc does not
...
void HexagonFrameLowering::orderFrameObjects(
  ...
  toolchain::stable_sort(SortingObjects, HexagonFrameSortingComparator());
