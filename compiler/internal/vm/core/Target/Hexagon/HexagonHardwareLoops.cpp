...
FunctionPass *toolchain::createHexagonHardwareLoops() {
...
CountValue *HexagonHardwareLoops::computeCount(MachineLoop *Loop,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Loop: " << *Loop << "\n");
  LLVM_DEBUG(toolchain::dbgs() << "Initial Value: " << *Start << "\n");
  LLVM_DEBUG(toolchain::dbgs() << "End Value: " << *End << "\n");
  LLVM_DEBUG(toolchain::dbgs() << "Inc/Dec Value: " << IVBump << "\n");
  LLVM_DEBUG(toolchain::dbgs() << "Comparison: " << Cmp << "\n");
...
bool HexagonHardwareLoops::isLoopFeeder(MachineLoop *L, MachineBasicBlock *A,
  ...
  if (LoopFeederPhi.find(MO->getReg()) == LoopFeederPhi.end()) {
    ...
    if (toolchain::is_contained(L->getBlocks(), A))
      return false;
