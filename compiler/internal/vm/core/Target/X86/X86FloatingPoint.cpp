...
#include "vm/core/CodeGen/MachineFunctionAnalysisManager.h"
...
FunctionPass *toolchain::createX86FPStackifierLegacyPass() {
...
static int Lookup(ArrayRef<TableEntry> Table, unsigned Opcode) {
  const TableEntry *I = toolchain::lower_bound(Table, Opcode);
...
void FPS::adjustLiveRegs(unsigned Mask, MachineBasicBlock::iterator I) {
  ...
  while (Kills && Defs) {
    unsigned KReg = toolchain::countr_zero(Kills);
    unsigned DReg = toolchain::countr_zero(Defs);
  ...
  assert(StackTop == (unsigned)toolchain::popcount(Mask) && "Live count mismatch");
}
...
void FPS::handleCall(MachineBasicBlock::iterator &I) {
  ...
  unsigned N = toolchain::countr_one(STReturns);
...
void FPS::handleSpecialFP(MachineBasicBlock::iterator &Inst) {
  ...
  case TargetOpcode::INLINEASM_BR: {
    ...
    unsigned NumSTUses = toolchain::countr_one(STUses);
    ...
    unsigned NumSTDefs = toolchain::countr_one(STDefs);
    ...
    unsigned NumSTPopped = toolchain::countr_one(STPopped);
    ...
    while (FPKills) {
      unsigned FPReg = toolchain::countr_zero(FPKills);
      if (isLive(FPReg))
