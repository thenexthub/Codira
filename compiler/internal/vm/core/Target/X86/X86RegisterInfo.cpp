...
#include "vm/core/CodeGen/TileShapeInfo.h"
...
bool X86RegisterInfo::isArgumentRegister(const MachineFunction &MF,
  ...
  if (CC != CallingConv::Win64 &&
      toolchain::any_of(SmallVector<MCRegister>{X86::RDI, X86::RSI},
                   [&](MCRegister &RegA) { return IsSubReg(RegA, Reg); }))
    return true;

  if (ST.hasSSE1() &&
      toolchain::any_of(SmallVector<MCRegister>{X86::XMM0, X86::XMM1, X86::XMM2,
...
X86RegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  // matches the behavior of toolchain.frameaddress.
  unsigned Opc = MI.getOpcode();
  if (Opc == TargetOpcode::LOCAL_ESCAPE) {
    MachineOperand &FI = MI.getOperand(FIOperandNum);
    FI.ChangeToImmediate(FIOffset);
...
bool X86RegisterInfo::getRegAllocationHints(Register VirtReg,
  ...
  SmallSet<MCPhysReg, 4> CopyHints(toolchain::from_range, Hints);
  Hints.clear();
