...
FunctionPass *toolchain::createX86FixupLEAsLegacyPass() {
...
bool FixupLEAsImpl::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    bool OptIncDecPerBB =
        OptIncDec || toolchain::shouldOptimizeForSize(&MBB, PSI, MBFI);
...
void FixupLEAsImpl::checkRegUsage(MachineBasicBlock::iterator &LeaI,
  ...
  *KilledBase = *KilledIndex = nullptr;
  ...
  for (MachineInstr &CurInst : toolchain::make_range(std::next(LeaI), AluI)) {
