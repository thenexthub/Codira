...
void X86AsmPrinter::LowerASAN_CHECK_MEMACCESS(const MachineInstr &MI) {
  ...
  if (!TM.getTargetTriple().isOSBinFormatELF()) {
    report_fatal_error("toolchain.asan.check.memaccess only supported on ELF");
    return;
  }
...
bool hasJumpTableInfoInBlock(const toolchain::MachineInstr *MI) {
...
void X86AsmPrinter::emitCallInstruction(const toolchain::MCInst &MCI) {
