...
// i.g: 0xF3660F1EFA, 0xF3670F1EFA
  ...
  while (i < 64) {
    ...
    if (Byte == 0xF3)
      return true;
    if (!toolchain::is_contained(OptionalPrefixBytes, Byte))
      return false;
    i += 8;
  }
...
MachineSDNode *X86DAGToDAGISel::matchBEXTRFromAndImm(SDNode *Node) {
  ...
  uint64_t MaskSize = toolchain::popcount(Mask);
...
bool X86DAGToDAGISel::tryShrinkShlLogicImm(SDNode *N) {
  ...
  if (Opcode == ISD::AND) {
    ...
    ZExtWidth = toolchain::bit_ceil(std::max(ZExtWidth, 8U));
  ...
  case X86ISD::CMP: {
    ...
    if (N0.getOpcode() == ISD::AND && N0.getNode()->hasOneUse() &&
      ...
      if (CmpVT == MVT::i64 && !isInt<8>(Mask) && isShiftedMask_64(Mask) &&
        ...
        MVT SubRegVT;
        unsigned TestOpcode;
        unsigned LeadingZeros = toolchain::countl_zero(Mask);
...
FunctionPass *toolchain::createX86ISelDag(X86TargetMachine &TM,
                                     CodeGenOptLevel OptLevel) {
