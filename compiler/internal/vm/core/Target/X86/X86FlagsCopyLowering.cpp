...
FunctionPass *toolchain::createX86FlagsCopyLoweringLegacyPass() {
...
bool X86FlagsCopyLoweringImpl::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineInstr *CopyI : Copies) {
    ...
    auto HasEFLAGSClobber = [&](MachineBasicBlock::iterator Begin,
      ...
      return toolchain::any_of(
          toolchain::reverse(toolchain::make_range(Begin, End)), [&](MachineInstr &MI) {
    ...
    LLVM_DEBUG({
      auto DefIt = toolchain::find_if(
          toolchain::reverse(toolchain::make_range(TestMBB->instr_begin(), TestPos)),
          [&](MachineInstr &MI) {
...
CondRegArray X86FlagsCopyLoweringImpl::collectCondsInRegs(
  ...
  for (MachineInstr &MI :
       toolchain::reverse(toolchain::make_range(MBB.begin(), TestPos))) {
