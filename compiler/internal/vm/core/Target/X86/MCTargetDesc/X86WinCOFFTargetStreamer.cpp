...
bool X86WinCOFFTargetStreamer::emitFPOStackAlign(unsigned Align, SMLoc L) {
  if (checkInFPOPrologue(L))
    return true;
  if (toolchain::none_of(CurFPOData->Instructions, [](const FPOInstruction &Inst) {
...
MCTargetStreamer *toolchain::createX86AsmTargetStreamer(MCStreamer &S,
                                                   formatted_raw_ostream &OS,
                                                   MCInstPrinter *InstPrinter) {
...
MCTargetStreamer *
toolchain::createX86ObjectTargetStreamer(MCStreamer &S, const MCSubtargetInfo &STI) {
