...
#undef FROM_TO_SIZE
  ...
  case X86::VINSERTPSZrri: {
    ...
    if (DstIdx == SrcIdx && (ZMask & (1 << DstIdx)) == 0 &&
        toolchain::popcount(ZMask) == 2) {
      unsigned AltIdx = toolchain::countr_zero((ZMask | (1 << DstIdx)) ^ 15);
...
static unsigned getNewOpcFromTable(ArrayRef<X86TableEntry> Table,
                                   unsigned Opc) {
  const auto I = toolchain::lower_bound(Table, Opc);
...
unsigned X86::getNFVariant(unsigned Opc) {
  ...
  if (!NFTableChecked.load(std::memory_order_relaxed)) {
    assert(toolchain::is_sorted(X86NFTransformTable) &&
...
unsigned X86::getNonNDVariant(unsigned Opc) {
  ...
  if (!NDTableChecked.load(std::memory_order_relaxed)) {
    assert(toolchain::is_sorted(X86ND2NonNDTable) &&
...
bool X86InstrInfo::analyzeBranchPredicate(MachineBasicBlock &MBB,
  ...
  for (MachineInstr &MI : toolchain::drop_begin(toolchain::reverse(MBB))) {
...
bool X86InstrInfo::preservesZeroValueInReg(
  ...
  // null value.
  case X86::MOV32rr:
    return toolchain::all_of(MI->operands(), [&](const MachineOperand &MO) {
...
static void updateOperandRegConstraints(MachineFunction &MF,
  ...
  for (int Idx : toolchain::seq<int>(0, NewMI.getNumOperands())) {
...
static MachineInstr *fuseTwoAddrInst(MachineFunction &MF, unsigned Opcode,
  ...
  for (const MachineOperand &MO : toolchain::drop_begin(MI.operands(), NumOps + 2))
    MIB.add(MO);
...
bool X86InstrInfo::unfoldMemoryOperand(
  ...
  toolchain::append_range(BeforeOps, AfterOps);
  // Change CMP32ri r, 0 back to TEST32rr r, r, etc.
  switch (Opc) {
  default:
    break;
...
char CGBR::ID = 0;
FunctionPass *toolchain::createX86GlobalBaseRegPass() { return new CGBR(); }
...
FunctionPass *toolchain::createCleanupLocalDynamicTLSPass() {
