...
//
//  External interfaces:
//      FunctionPass *toolchain::createX86CmovConverterPass();
...
bool X86CmovConversionImpl::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (ForceMemOperand || ForceAll) {
    ...
    SmallVector<MachineBasicBlock *, 4> Blocks(toolchain::make_pointer_range(MF));
    if (collectCmovCandidates(Blocks, AllCmovGroups, /*IncludeLoads*/ true)) {
      for (auto &Group : AllCmovGroups) {
        ...
        if (ForceMemOperand && !ForceAll &&
            toolchain::none_of(Group, [&](MachineInstr *I) { return I->mayLoad(); }))
          continue;
  ...
  for (int i = 0; i < (int)Loops.size(); ++i)
    toolchain::append_range(Loops, Loops[i]->getSubLoops());
...
void X86CmovConversionImpl::convertCmovInstsToBranches(
  ...
  if (toolchain::any_of(Group, [&](MachineInstr *I) {
...
FunctionPass *toolchain::createX86CmovConversionLegacyPass() {
