...
X86SpeculativeLoadHardeningImpl::tracePredStateThroughCFG(
  ...
  for (const BlockCondInfo &Info : Infos) {
    ...
    toolchain::sort(UncondCodeSeq);
    UncondCodeSeq.erase(toolchain::unique(UncondCodeSeq), UncondCodeSeq.end());
...
X86SpeculativeLoadHardeningImpl::tracePredStateThroughIndirectBranches(
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (toolchain::any_of(MBB.terminators(), [&](MachineInstr &OtherTI) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (MachineBasicBlock *Pred : MBB.predecessors()) {
      ...
      if (!toolchain::all_of(Pred->successors(), [&](MachineBasicBlock *Succ) {
...
static bool isEFLAGSLive(MachineBasicBlock &MBB, MachineBasicBlock::iterator I,
  ...
  for (MachineInstr &MI : toolchain::reverse(toolchain::make_range(MBB.begin(), I))) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (HardenLoads)
      for (MachineInstr &MI : MBB) {
        ...
        // model here.
        if (toolchain::any_of(MI.uses(), [&](MachineOperand &Op) {
...
void X86SpeculativeLoadHardeningImpl::hardenLoadAddr(
  ...
  toolchain::erase_if(HardenOpRegs, [&](MachineOperand *Op) {
...
FunctionPass *toolchain::createX86SpeculativeLoadHardeningLegacyPass() {
