...
FunctionPass *toolchain::createX86AvoidStoreForwardingBlocksLegacyPass() {
...
findPotentialBlockers(MachineInstr *LoadInst) {
  ...
  if (BlockCount < InspectionLimit) {
    ...
    for (MachineBasicBlock *PMBB : MBB->predecessors()) {
      int PredCount = 0;
      for (MachineInstr &PBInst : toolchain::reverse(*PMBB)) {
        if (PBInst.isMetaInstruction())
          continue;
        PredCount++;
  ...
  for (auto &MBB : MF)
    for (auto &MI : MBB) {
      ...
      for (MachineOperand &StoreMO :
           toolchain::make_early_inc_range(MRI->use_nodbg_operands(DefVR))) {
