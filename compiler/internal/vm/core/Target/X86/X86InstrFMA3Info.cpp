...
static void verifyTables() {
  ...
  if (!TableChecked.load(std::memory_order_relaxed)) {
    assert(toolchain::is_sorted(Groups) && toolchain::is_sorted(RoundGroups) &&
           toolchain::is_sorted(BroadcastGroups) && "FMA3 tables not sorted!");
...
const X86InstrFMA3Group *toolchain::getFMA3Group(unsigned Opcode, uint64_t TSFlags) {
