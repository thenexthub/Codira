...
#define CHECK_SORTED_UNIQUE(TABLE)                                             \
  assert(toolchain::is_sorted(TABLE) && #TABLE " is not sorted");                   \
  ...
  const X86FoldTableEntry *Data = toolchain::lower_bound(Table, RegOp);
...
const X86FoldTableEntry *toolchain::lookupTwoAddrFoldTable(unsigned RegOp) {
  return lookupFoldTableImpl(Table2Addr, RegOp);
}

const X86FoldTableEntry *toolchain::lookupFoldTable(unsigned RegOp, unsigned OpNum) {
...
const X86FoldTableEntry *toolchain::lookupBroadcastFoldTable(unsigned RegOp,
                                                        unsigned OpNum) {
  ArrayRef<X86FoldTableEntry> FoldTable;
  if (OpNum == 1)
...
const X86FoldTableEntry *toolchain::lookupUnfoldTable(unsigned MemOp) {
  static X86MemUnfoldTable MemUnfoldTable;
  auto &Table = MemUnfoldTable.Table;
  auto I = toolchain::lower_bound(Table, MemOp);
...
bool toolchain::matchBroadcastSize(const X86FoldTableEntry &Entry,
...
toolchain::lookupBroadcastFoldTableBySize(unsigned MemOp, unsigned BroadcastBits) {
  static X86BroadcastFoldTable BroadcastFoldTable;
  auto &Table = BroadcastFoldTable.Table;
  for (auto I = toolchain::lower_bound(Table, MemOp);
