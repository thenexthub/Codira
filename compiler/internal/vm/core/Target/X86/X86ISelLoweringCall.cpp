//===- toolchain/lib/Target/X86/X86ISelCallLowering.cpp - Call lowering --------===//
//
...
SDValue X86TargetLowering::getPICODEumpTableRelocBase(SDValue Table,
...
/// the same as getPICODEumpTableRelocBase, but as an MCExpr.
const MCExpr *X86TargetLowering::
getPICODEumpTableRelocBaseExpr(const MachineFunction *MF, unsigned JTI,
                             MCContext &Ctx) const {
...
void VarArgsLoweringHelper::createVarArgAreaAndStoreRegisters(
  ...
  // the start of the first vararg value... for expansion of toolchain.va_start. We
  ...
  if (is64Bit()) {
    ...
    if (!LiveXMMRegs.empty()) {
      ...
      toolchain::append_range(SaveXMMOps, LiveXMMRegs);
