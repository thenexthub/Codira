...
static Value *simplifyX86immShift(const IntrinsicInst &II,
  ...
  if (IsImm) {
    ...
    KnownBits KnownAmtBits =
        toolchain::computeKnownBits(Amt, II.getDataLayout());
    ...
    KnownBits KnownLowerBits = toolchain::computeKnownBits(
        Amt, DemandedLower, II.getDataLayout());
    KnownBits KnownUpperBits = toolchain::computeKnownBits(
...
static Value *simplifyX86varShift(const IntrinsicInst &II,
  ...
  if (toolchain::all_of(ShiftAmts, OutOfRange)) {
  ...
  case Intrinsic::x86_avx2_pblendvb: {
    ...
    if (match(Mask, m_SExt(m_Value(BoolVec))) &&
      ...
      if (MaskSrc) {
        ...
        if (!toolchain::scaleShuffleMaskElts(NumMaskElts, ShuffleMask, ScaledMask))
          break;
