...
void X86FrameLowering::inlineStackProbe(MachineFunction &MF,
  ...
  auto Where = toolchain::find_if(PrologMBB, [](MachineInstr &MI) {
...
void X86FrameLowering::emitStackProbeInlineWindowsCoreCLR64(
  ...
  if (InProlog) {
    ...
    for (MachineInstr &MI :
         toolchain::make_range(ContinueMBB->begin(), ContinueMBBI)) {
      MI.setFlag(MachineInstr::FrameSetup);
    }
  }
}
...
/*
  ...
  ; calls @toolchain.eh.unwind.init
  [if needs FP]
...
bool X86FrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (STI.hasPush2Pop2() && getStackAlignment() >= 16) {
    unsigned NumCSGPR = toolchain::count_if(CSI, [](const CalleeSavedInfo &I) {
  ...
  for (CalleeSavedInfo &I : toolchain::reverse(CSI)) {
    MCRegister Reg = I.getReg();
...
bool X86FrameLowering::spillCalleeSavedRegisters(
  ...
  for (const CalleeSavedInfo &I : toolchain::reverse(CSI)) {
    MCRegister Reg = I.getReg();
    if (X86::GR64RegClass.contains(Reg) || X86::GR32RegClass.contains(Reg))
      continue;
...
void X86FrameLowering::orderFrameObjects(
  ...
  // info).
  toolchain::stable_sort(SortingObjects, X86FrameSortingComparator());
...
void X86FrameLowering::spillFPBP(MachineFunction &MF) const {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    while (MI != ME) {
      ...
      // Don't need to save/restore if FP is accessed through toolchain.frameaddress.
      if (FPLive && !SpillBP)
        continue;
