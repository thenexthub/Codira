...
FunctionPass *toolchain::createX86OptimizeLEAsLegacyPass() {
  ...
  for (auto &E : LEAs) {
    ...
    while (I1 != List.end()) {
      ...
      while (I2 != List.end()) {
        ...
        // toolchain::make_early_inc_range and MRI->use_operands because we could
        // replace two or more uses in a debug instruction in one iteration, and
        // that would deeply confuse toolchain::make_early_inc_range.
