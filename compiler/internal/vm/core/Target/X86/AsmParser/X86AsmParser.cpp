...
private:
  ...
  AddDefaultSrcDestOperands(OperandVector &Operands,
                            std::unique_ptr<toolchain::MCParsedAsmOperand> &&Src,
                            std::unique_ptr<toolchain::MCParsedAsmOperand> &&Dst);
...
void X86AsmParser::AddDefaultSrcDestOperands(
    OperandVector& Operands, std::unique_ptr<toolchain::MCParsedAsmOperand> &&Src,
    std::unique_ptr<toolchain::MCParsedAsmOperand> &&Dst) {
  if (isParsingIntelSyntax()) {
...
static bool convertSSEToAVX(MCInst &Inst) {
  ...
  const auto I = toolchain::lower_bound(Table, Opcode);
...
bool X86AsmParser::matchAndEmitATTInstruction(
  ...
  unsigned NumSuccessfulMatches = toolchain::count(Match, Match_Success);
  ...
  if (toolchain::count(Match, Match_MnemonicFail) == 4) {
  ...
  if (toolchain::count(Match, Match_Unsupported) == 1) {
    return Error(IDLoc, "unsupported instruction", EmptyRange,
                 MatchingInlineAsm);
  }
  ...
  // missing feature.
  if (toolchain::count(Match, Match_MissingFeature) == 1) {
  ...
  // operand failure.
  if (toolchain::count(Match, Match_InvalidOperand) == 1) {
    return Error(IDLoc, "invalid operand for instruction", EmptyRange,
                 MatchingInlineAsm);
  }
...
bool X86AsmParser::matchAndEmitIntelInstruction(
  ...
  if (toolchain::count(Match, Match_InvalidImmUnsignedi4) == 1) {
    SMLoc ErrorLoc = ((X86Operand &)*Operands[ErrorInfo]).getStartLoc();
    if (ErrorLoc == SMLoc())
      ErrorLoc = IDLoc;
    return Error(ErrorLoc, "immediate must be an integer in range [0, 15]",
  ...
  if (toolchain::count(Match, Match_InvalidImmUnsignedi6) == 1) {
    SMLoc ErrorLoc = ((X86Operand &)*Operands[ErrorInfo]).getStartLoc();
    if (ErrorLoc == SMLoc())
      ErrorLoc = IDLoc;
    return Error(ErrorLoc, "immediate must be an integer in range [0, 63]",
