...
/// -emit-toolchain t.c" + "llc t.ll") we should make sure the amx data is volatile,
...
/// ----------------------------------------------------------
/// | def %td = ...                                          |
/// | call void @toolchain.x86.tilestored64.internal(mem, %td)    |
/// | ...                                                    |
/// | %td2 = call x86_amx @toolchain.x86.tileloadd64.internal(mem)|
/// | "use %td2"                                             |
/// ----------------------------------------------------------
//
//===----------------------------------------------------------------------===//
...
static Value *getRowFromCol(Instruction *II, Value *V, unsigned Granularity) {
  ...
  if (isa<ConstantInt>(V))
    ...
    //   %117 = call x86_amx @toolchain.x86.cast.vector.to.tile.v256i32(<256 x
    //   i32> %115).
    //   %118 = call x86_amx @toolchain.x86.tdpbf16ps.internal(i16
...
// -->
// %2 = call x86_amx @toolchain.x86.tileloadd64.internal(i16 %row, i16 %col,
...
// %src = call x86_amx @toolchain.x86.tileloadd64.internal(%row, %col, %addr,
//                                                    %stride);
// %13 = bitcast x86_amx %src to <256 x i32>
// store <256 x i32> %13, <256 x i32>* %addr, align 64
// -->
// call void @toolchain.x86.tilestored64.internal(%row, %col, %addr,
//                                           %stride64, %13)
...
bool X86LowerAMXType::transformBitcast(BitCastInst *Bitcast) {
  ...
  if (Bitcast->getType()->isX86_AMXTy()) {
    ...
    // %addr = alloca <256 x i32>, align 64
    // %addr2 = bitcast <256 x i32>* to i8*
    // call void @toolchain.x86.tilestored64.internal(i16 %row, i16 %col,
    //                                           i8* %addr2, i64 %stride)
...
bool X86LowerAMXType::visit() {
  ...
  for (BasicBlock *BB : post_order(&Func)) {
    for (Instruction &Inst : toolchain::make_early_inc_range(toolchain::reverse(*BB))) {
...
// if.then:
//   def %t0 = ...
//   call void @toolchain.x86.tilestored64.internal(mem, %t0)    *
//   ...
//   %t0` = call x86_amx @toolchain.x86.tileloadd64.internal(mem)*
//   use %t0`                                               *
//   ...
//   br label %if.end
//
// if.else:
//   def %t1 = ...
//   call void @toolchain.x86.tilestored64.internal(mem, %t1)    *
//   br label %if.end
//
// if.end:
//   ...
//   %td = call x86_amx @toolchain.x86.tileloadd64.internal(mem) *
//   use %td
// ------------------------------------------------------
...
// For example:
// --------------------------------------------------------------------------
// %t1 = call x86_amx @toolchain.x86.tileloadd64.internal(m, k, ...)          key
// %t2 = call x86_amx @toolchain.x86.tileloadd64.internal(k, n, ...)
// %t3 = call x86_amx @toolchain.x86.tileloadd64.internal(m, n, ...)          amx
// %td = tail call x86_amx @toolchain.x86.tdpbssd.internal(m, n, k, t1, t2, t3)
// call void @toolchain.x86.tilestored64.internal(... td)                     area
// --------------------------------------------------------------------------
...
// %43 = call <256 x i32> @toolchain.x86.cast.tile.to.vector.v256i32(x86_amx %42)
// store <256 x i32> %43, <256 x i32>* %p, align 64
// -->
// call void @toolchain.x86.tilestored64.internal(i16 %row, i16 %col, i8* %p,
//                                           i64 64, x86_amx %42)
...
// %66 = call x86_amx @toolchain.x86.cast.vector.to.tile(<256 x i32> %65)
// -->
// %66 = call x86_amx @toolchain.x86.tileloadd64.internal(i16 %row, i16 %col,
//                                                   i8* %p, i64 64)
...
// %19 = tail call x86_amx @toolchain.x86.cast.vector.to.tile.v256i32(<256 x i32> zeroinitializer)
// -->
// %19 = tail call x86_amx @toolchain.x86.tilezero.internal(i16 %row, i16 %col)
...
bool X86LowerAMXCast::combineLdSt(SmallVectorImpl<Instruction *> &Casts) {
  ...
  for (auto *Cast : Casts) {
    ...
    // %43 = call <256 x i32> @toolchain.x86.cast.tile.to.vector(x86_amx %42)
    // store <256 x i32> %43, <256 x i32>* %p, align 64
    // -->
    // call void @toolchain.x86.tilestored64.internal(i16 %row, i16 %col, i8* %p,
    //                                           i64 64, x86_amx %42)
...
bool X86LowerAMXCast::transformAMXCast(IntrinsicInst *AMXCast) {
  ...
  if (AMXCast->getType()->isX86_AMXTy()) {
    ...
    // call void @toolchain.x86.tilestored64.internal(i16 15, i16 60,
    ...
    // %2 = call x86_amx @toolchain.x86.tileloadd64.internal(i16 15, i16 60,
    //                                                  i8* %addr2,
    //                                                  i64 60)
    // call void @toolchain.x86.tilestored64.internal(i16 15, i16 60,
    //                                           i8* %addr3, i64 60, x86_amx %2)
...
bool lowerAmxType(Function &F, const TargetMachine *TM,
  ...
  if (TM->getOptLevel() == CodeGenOptLevel::None) {
    ...
    // "Clang -O2 -S -emit-toolchain t.c" + "llc t.ll") we should make
...
FunctionPass *toolchain::createX86LowerAMXTypeLegacyPass() {
