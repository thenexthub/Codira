...
namespace {
  ...
  public:
    ...
    inline bool immMskBitp(SDNode *inN) const {
      ...
      int msksize = toolchain::bit_width(value);
...
///
FunctionPass *toolchain::createXCoreISelDag(XCoreTargetMachine &TM,
                                       CodeGenOptLevel OptLevel) {
...
void XCoreDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::Constant: {
    ...
    if (immMskBitp(N)) {
      ...
      SDValue MskSize = getI32Imm(toolchain::bit_width((uint32_t)Val), dl);
      ReplaceNode(
