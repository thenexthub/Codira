...
//   - remove toolchain.bpf.getelementptr.and.load builtins.
//   - remove toolchain.bpf.getelementptr.and.store builtins.
...
ModulePass *toolchain::createBPFCheckAndAdjustIR() {
...
void BPFCheckAndAdjustIR::checkIR(Module &M) {
  ...
  //   B1:
  //      g1 = @toolchain.skb_buff:0:1...
  //      ...
  //      goto B_COMMON
  //   B2:
  //      g2 = @toolchain.skb_buff:0:2...
  //      ...
  //      goto B_COMMON
  //   B_COMMON:
  //      g = PHI(g1, g2)
  ...
  for (Function &F : M)
    for (auto &BB : F)
      for (auto &I : BB) {
        ...
        if (!GV)
          continue;
        if (!GV->getName().starts_with("toolchain.bpf.passthrough"))
          continue;
        Changed = true;
  ...
  for (Function &F : M)
    for (auto &BB : F)
      for (auto &I : BB) {
        ...
        if (!GV)
          continue;
        if (!GV->getName().starts_with("toolchain.bpf.compare"))
          continue;
