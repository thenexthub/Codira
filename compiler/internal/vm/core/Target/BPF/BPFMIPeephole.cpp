...
FunctionPass* toolchain::createBPFMIPeepholePass() { return new BPFMIPeephole(); }
...
// target offset might exceed 16bit range. If this happens, the toolchain
...
static void collectBPFFastCalls(const TargetRegisterInfo *TRI,
  ...
  Calls.clear();
  for (MachineInstr &MI : toolchain::reverse(BB)) {
    if (MI.isCall()) {
...
FunctionPass* toolchain::createBPFMIPreEmitPeepholePass()
{
