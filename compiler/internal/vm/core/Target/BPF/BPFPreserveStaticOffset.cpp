...
//===----------------------------------------------------------------------===//
//
// TLDR: replaces toolchain.preserve.static.offset + GEP + load / store
//           with toolchain.bpf.getelementptr.and.load / store
//
...
// - toolchain.bpf.getelementptr.and.load
// - toolchain.bpf.getelementptr.and.store
//
...
//   toolchain.preserve.static.offset are inserted.
...
static void rewriteUses(Instruction *Insn,
  ...
  for (User *U : Insn->users()) {
    ...
    if (UI && (isPointerOperand(Insn, UI) || isPreserveStaticOffsetCall(UI) ||
      ...
      LLVM_DEBUG({
        toolchain::dbgs() << "unsupported usage in BPFPreserveStaticOffsetPass:\n";
        toolchain::dbgs() << "  Insn: " << *Insn << "\n";
        toolchain::dbgs() << "  User: " << *U << "\n";
      });
  }
}
...
// toolchain.preserve.static.offset calls. Recursion stops at any other
// instruction. If load or store instruction is reached it is replaced
// by a call to `toolchain.bpf.getelementptr.and.load` or
// `toolchain.bpf.getelementptr.and.store` intrinsic.
// If `toolchain.bpf.getelementptr.and.load/store` is reached the accumulated
// GEPs are merged into the intrinsic call.
// If nested calls to `toolchain.preserve.static.offset` are encountered these
...
// Look for sequences:
// - toolchain.preserve.static.offset -> getelementptr... -> load
// - toolchain.preserve.static.offset -> getelementptr... -> store
...
PreservedAnalyses
toolchain::BPFPreserveStaticOffsetPass::run(Function &F,
