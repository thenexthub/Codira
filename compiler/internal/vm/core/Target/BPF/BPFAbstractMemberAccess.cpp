...
//          !toolchain.preserve.access.index <union_ditype>
//   addr = preserve_struct_access_index(base, gep_index, di_index)
//          !toolchain.preserve.access.index <struct_ditype>
//
...
bool BPFAbstractMemberAccess::IsPreserveDIAccessIndexCall(const CallInst *Call,
  ...
  if (!GV)
    return false;
  if (GV->getName().starts_with("toolchain.preserve.array.access.index")) {
    CInfo.Kind = BPFPreserveArrayAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
      report_fatal_error("Missing metadata for toolchain.preserve.array.access.index intrinsic");
  ...
  if (GV->getName().starts_with("toolchain.preserve.union.access.index")) {
    CInfo.Kind = BPFPreserveUnionAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
      report_fatal_error("Missing metadata for toolchain.preserve.union.access.index intrinsic");
    ResetMetadata(CInfo);
    CInfo.AccessIndex = getConstant(Call->getArgOperand(1));
    CInfo.Base = Call->getArgOperand(0);
    return true;
  }
  if (GV->getName().starts_with("toolchain.preserve.struct.access.index")) {
    CInfo.Kind = BPFPreserveStructAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
      report_fatal_error("Missing metadata for toolchain.preserve.struct.access.index intrinsic");
    ResetMetadata(CInfo);
    CInfo.AccessIndex = getConstant(Call->getArgOperand(2));
    CInfo.Base = Call->getArgOperand(0);
    CInfo.RecordAlignment = DL->getABITypeAlign(getBaseElementType(Call));
  ...
  if (GV->getName().starts_with("toolchain.bpf.preserve.field.info")) {
    ...
    if (InfoKind >= BTF::MAX_FIELD_RELOC_KIND)
      report_fatal_error("Incorrect info_kind for toolchain.bpf.preserve.field.info intrinsic");
    CInfo.AccessIndex = InfoKind;
    return true;
  }
  if (GV->getName().starts_with("toolchain.bpf.preserve.type.info")) {
    CInfo.Kind = BPFPreserveFieldInfoAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
      report_fatal_error("Missing metadata for toolchain.preserve.type.info intrinsic");
    uint64_t Flag = getConstant(Call->getArgOperand(1));
    if (Flag >= BPFCoreSharedInfo::MAX_PRESERVE_TYPE_INFO_FLAG)
      report_fatal_error("Incorrect flag for toolchain.bpf.preserve.type.info intrinsic");
  ...
  if (GV->getName().starts_with("toolchain.bpf.preserve.enum.value")) {
    CInfo.Kind = BPFPreserveFieldInfoAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
      report_fatal_error("Missing metadata for toolchain.preserve.enum.value intrinsic");
    uint64_t Flag = getConstant(Call->getArgOperand(2));
    if (Flag >= BPFCoreSharedInfo::MAX_PRESERVE_ENUM_VALUE_FLAG)
      report_fatal_error("Incorrect flag for toolchain.bpf.preserve.enum.value intrinsic");
...
void BPFAbstractMemberAccess::GetStorageBitRange(DIDerivedType *MemberTy,
  ...
  if (RecordAlignment > 8) {
    ...
    if (MemberBitOffset / 64 != (MemberBitOffset + MemberBitSize) / 64)
      report_fatal_error("Unsupported field expression for toolchain.bpf.preserve.field.info, "
...
uint32_t BPFAbstractMemberAccess::GetFieldInfo(uint32_t InfoKind,
  ...
  if (InfoKind == BTF::FIELD_SIGNEDNESS) {
    ...
    if (Tag == dwarf::DW_TAG_array_type) {
      ...
      if (CTy->getElements().size() != 1)
        report_fatal_error("Invalid array expression for toolchain.bpf.preserve.field.info");
    ...
    while (!BTy) {
      ...
      if (!CompTy || CompTy->getTag() != dwarf::DW_TAG_enumeration_type)
        report_fatal_error("Invalid field expression for toolchain.bpf.preserve.field.info");
  ...
  if (InfoKind == BTF::FIELD_LSHIFT_U64) {
    ...
    if (!IsBitField) {
      if (SizeInBits > 64)
        report_fatal_error("too big field size for toolchain.bpf.preserve.field.info");
  ...
  llvm_unreachable("Unknown toolchain.bpf.preserve.field.info info kind");
}
...
Value *BPFAbstractMemberAccess::computeBaseAndAccessKey(CallInst *Call,
    ...
    if (CheckElemType) {
      ...
      if (!CTy) {
        if (HasPreserveFieldInfoCall(CallStack))
          report_fatal_error("Invalid field access for toolchain.preserve.field.info intrinsic");
        return nullptr;
      }
  ...
  // Access key is the
  //   "toolchain." + type name + ":" + reloc type + ":" + patched imm + "$" +
  //   access string,
  // uniquely identifying one relocation.
  // The prefix "toolchain." indicates this is a temporary global, which should
  // not be emitted to ELF file.
  AccessKey = "toolchain." + TypeName + ":" + std::to_string(InfoKind) + ":" +
...
MDNode *BPFAbstractMemberAccess::computeAccessKey(CallInst *Call,
  ...
  AccessKey = "toolchain." + Ty->getName().str() + ":" +
...
bool BPFAbstractMemberAccess::transformGEPChain(CallInst *Call,
  ...
  //   %6 = load toolchain.sk_buff:0:50$0:0:0:2:0
  ...
  //      if (...) goto Block3 else ...
  //    Block3:
  //      %6 = load toolchain.bpf_map:0:40$0:0:0:2:0
  //      %8 = getelementptr i8, i8* %2, %6
  //      ...
  //      goto CommonExit
  //    CommonExit
  ...
  //      if (...) goto Block_Common else ...
  //    Block_Common:
  //      PHI = [toolchain.sk_buff:0:50$0:0:0:2:0, toolchain.bpf_map:0:40$0:0:0:2:0]
  //      %6 = load PHI
  //      %8 = getelementptr i8, i8* %2, %6
  //      ...
  //      goto CommonExit
