...
FunctionPass *toolchain::createNVPTXISelDag(NVPTXTargetMachine &TM,
                                       toolchain::CodeGenOptLevel OptLevel) {
...
static std::optional<NVPTX::AddressSpace> convertAS(unsigned AS) {
  switch (AS) {
  case toolchain::ADDRESS_SPACE_LOCAL:
    return NVPTX::AddressSpace::Local;
  case toolchain::ADDRESS_SPACE_GLOBAL:
    return NVPTX::AddressSpace::Global;
  case toolchain::ADDRESS_SPACE_SHARED:
    return NVPTX::AddressSpace::Shared;
  case toolchain::ADDRESS_SPACE_SHARED_CLUSTER:
    return NVPTX::AddressSpace::SharedCluster;
  case toolchain::ADDRESS_SPACE_GENERIC:
    return NVPTX::AddressSpace::Generic;
  case toolchain::ADDRESS_SPACE_PARAM:
    return NVPTX::AddressSpace::Param;
  case toolchain::ADDRESS_SPACE_CONST:
...
getOperationOrderings(MemSDNode *N, const NVPTXSubtarget *Subtarget) {
  ...
  //
  //      Example (https://github.com/toolchain/toolchain-project/issues/62057):
  //
  //          void example() {
  ...
  if (N->getOpcode() == ISD::AND) {
    ...
    if (LHS->getOpcode() == ISD::AND) {
      ...
      if (isMask_64(MaskVal)) {
        ...
        NumBits = toolchain::countr_one(MaskVal) - ShiftAmt;
      } else if (isShiftedMask_64(MaskVal)) {
        NumZeros = toolchain::countr_zero(MaskVal);
        unsigned NumOnes = toolchain::countr_one(MaskVal >> NumZeros);
        // The number of bits in the result bitfield will be the number of
...
bool NVPTXDAGToDAGISel::tryIntrinsicVoid(SDNode *N) {
  unsigned IID = N->getConstantOperandVal(1);
  using TMARedTy = toolchain::nvvm::TMAReductionOp;
