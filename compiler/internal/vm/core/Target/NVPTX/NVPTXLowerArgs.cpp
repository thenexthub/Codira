...
//      %input1.gen = call ptr @toolchain.nvvm.ptr.param.to.gen.p0.p101(ptr addrspace(101) %input1)
...
static void convertToParamAS(Use *OldUse, Value *Param, bool HasCvtaParam,
  ...
  // the BitCast.
  for (Instruction *I : toolchain::reverse(InstructionsToDelete))
    I->eraseFromParent();
}
...
static void handleByValParam(const NVPTXTargetMachine &TM, Argument *Arg) {
  ...
  if (ArgUseIsReadOnly && AUC.Conditionals.empty()) {
    ...
    SmallVector<Use *, 16> UsesToUpdate(toolchain::make_pointer_range(Arg->uses()));
...
static bool runOnKernelFunction(const NVPTXTargetMachine &TM, Function &F) {
  ...
  auto HandleIntToPtr = [](Value &V) {
    if (toolchain::all_of(V.users(), [](User *U) { return isa<IntToPtrInst>(U); })) {
...
FunctionPass *toolchain::createNVPTXLowerArgsPass() {
