...
bool GenericToNVVM::runOnModule(Module &M) {
  ...
  for (GlobalVariable &GV : toolchain::make_early_inc_range(M.globals())) {
    if (GV.getType()->getAddressSpace() == toolchain::ADDRESS_SPACE_GENERIC &&
        !toolchain::isTexture(GV) && !toolchain::isSurface(GV) && !toolchain::isSampler(GV) &&
        !GV.getName().starts_with("toolchain.")) {
      GlobalVariable *NewGV = new GlobalVariable(
          M, GV.getValueType(), GV.isConstant(), GV.getLinkage(),
          GV.hasInitializer() ? GV.getInitializer() : nullptr, "", &GV,
          GV.getThreadLocalMode(), toolchain::ADDRESS_SPACE_GLOBAL);
  ...
  if (isa<GlobalVariable>(C)) {
    ...
    if (I != GVMap.end()) {
      ...
      NewValue = Builder.CreateAddrSpaceCast(
          GV, PointerType::get(GV->getContext(), toolchain::ADDRESS_SPACE_GENERIC));
    }
...
ModulePass *toolchain::createGenericToNVVMLegacyPass() {
