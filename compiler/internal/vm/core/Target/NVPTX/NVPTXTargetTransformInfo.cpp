...
static Instruction *convertNvvmIntrinsicToLlvm(InstCombiner &IC,
  ...
  const SimplifyAction Action = [II]() -> SimplifyAction {
    ...
    case Intrinsic::nvvm_fshr_clamp:
      ...
      //
      // Ideally we'd encode them as e.g. "fast call @toolchain.cos", where "fast"
  ...
  case SCP_FunnelShiftClamp: {
    // Canonicalize a clamping funnel shift to the generic toolchain funnel shift
    // when possible, as this is easier for toolchain to optimize further.
