...
static inline SDValue getBuildVectorizedValue(unsigned N, const SDLoc &dl,
  ...
  for (const unsigned I : toolchain::seq(N)) {
...
std::string NVPTXTargetLowering::getPrototype(
  ...
  for (const unsigned I : toolchain::seq(NumArgs)) {
    const auto ArgOuts =
...
SDValue NVPTXTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // to the function
  for (const auto E : toolchain::enumerate(Args)) {
  ...
  if (!Ins.empty()) {
    ...
    for (const unsigned NumElts : VI) {
      ...
      for (const unsigned J : toolchain::seq(NumElts))
  ...
  // dangling.
  for (const auto [I, Reg] : toolchain::enumerate(ProxyRegOps)) {
    SDValue Proxy =
...
// We use extract/insert/build vector just as what LegalizeOp() does in toolchain 2.5
SDValue
...
SDValue NVPTXTargetLowering::LowerBUILD_VECTOR(SDValue Op,
  ...
  if (!toolchain::all_of(Op->ops(), [](SDValue Operand) {
...
SDValue NVPTXTargetLowering::LowerVECTOR_SHUFFLE(SDValue Op,
  ...
  uint32_t Selector = 0;
  for (auto I : toolchain::enumerate(SVN->getMask())) {
...
static SDValue LowerVectorArith(SDValue Op, SelectionDAG &DAG) {
  ...
  for (int I = 0, E = Op.getValueType().getVectorNumElements(); I < E; I++) {
    ...
    toolchain::transform(Op->ops(), std::back_inserter(ScalarArgs),
                    [&](const SDUse &O) {
...
SDValue NVPTXTargetLowering::LowerADDRSPACECAST(SDValue Op,
  ...
  if (SrcAS != toolchain::ADDRESS_SPACE_GENERIC &&
      DestAS != toolchain::ADDRESS_SPACE_GENERIC) {
    // Shared and SharedCluster can be converted to each other through generic
    // space
    if ((SrcAS == toolchain::ADDRESS_SPACE_SHARED &&
         DestAS == toolchain::ADDRESS_SPACE_SHARED_CLUSTER) ||
        (SrcAS == toolchain::ADDRESS_SPACE_SHARED_CLUSTER &&
         DestAS == toolchain::ADDRESS_SPACE_SHARED)) {
      SDLoc DL(Op.getNode());
...
replaceLoadVector(SDNode *N, SelectionDAG &DAG, const NVPTXSubtarget &STI) {
  ...
  if (EltVT.isVector()) {
    ...
    for (const unsigned I : toolchain::seq(NumElts)) {
...
// toolchain.ptx.memcpy.const and toolchain.ptx.memmove.const need to be modeled as
// TgtMemIntrinsic
...
/// (fmaxnum3 a, b, c). Also covers other toolchain min/max intrinsics.
...
static APInt computePRMT(APInt A, APInt B, APInt Selector, unsigned Mode) {
  ...
  APInt Result(32, 0);
  for (unsigned I : toolchain::seq(4U)) {
...
static void computeKnownBitsForPRMT(const SDValue Op, KnownBits &Known,
  ...
  for (unsigned I : toolchain::seq(4)) {
    APInt Sel = SelectorVal.extractBits(4, I * 4);
    unsigned Idx = Sel.getLoBits(3).getZExtValue();
    unsigned Sign = Sel.getHiBits(1).getZExtValue();
