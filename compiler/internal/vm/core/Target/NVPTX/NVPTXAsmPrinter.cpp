...
// toolchain.loop.unroll.disable or toolchain.loop.unroll.count=1.
bool NVPTXAsmPrinter::isLoopHeaderOfNoUnroll(
  ...
  // toolchain.loop.unroll.disable is marked on the back edges of a loop. Therefore,
  // we iterate through each back edge of the loop with header MBB, and check
  // whether its metadata contains toolchain.loop.unroll.disable.
  for (const MachineBasicBlock *PMBB : MBB.predecessors()) {
    ...
    if (const BasicBlock *PBB = PMBB->getBasicBlock()) {
      if (MDNode *LoopID =
        ...
        if (GetUnrollMetadata(LoopID, "toolchain.loop.unroll.disable"))
          return true;
        if (MDNode *UnrollCountMD =
                GetUnrollMetadata(LoopID, "toolchain.loop.unroll.count")) {
...
void NVPTXAsmPrinter::emitKernelFunctionDirectives(const Function &F,
  ...
  if (STI->getSmVersion() >= 90) {
    ...
    if (!ClusterDim.empty()) {
      ...
      if (ClusterDim[0] != 0) {
        assert(toolchain::all_of(ClusterDim, not_equal_to(0)) &&
        ...
        assert(toolchain::all_of(ClusterDim, equal_to(0)) &&
               "cluster_dim_x == 0 implies cluster_dim_y and cluster_dim_z "
               "should be 0 as well");
      }
    }
...
static bool usedInGlobalVarDef(const Constant *C) {
  ...
  if (const GlobalVariable *GV = dyn_cast<GlobalVariable>(C))
    return GV->getName() != "toolchain.used";
...
static bool usedInOneFunc(const User *U, Function const *&OneFunc) {
  if (const GlobalVariable *OtherGV = dyn_cast<GlobalVariable>(U))
    if (OtherGV->getName() == "toolchain.used")
      return true;
...
void NVPTXAsmPrinter::printModuleLevelGV(const GlobalVariable *GVar,
  ...
  // Skip meta data
  if (GVar->hasSection())
    if (GVar->getSection() == "toolchain.metadata")
      return;

  // Skip LLVM intrinsic global variables
  if (GVar->getName().starts_with("toolchain.") ||
...
void NVPTXAsmPrinter::AggBuffer::printBytes(raw_ostream &os) {
  ...
  for (unsigned int pos = 0; pos < InitializerCount;) {
    ...
    std::string symText;
    toolchain::raw_string_ostream oss(symText);
    printSymbol(nSym, oss);
    for (unsigned i = 0; i < ptrSize; ++i) {
      if (i)
        os << ", ";
      toolchain::write_hex(os, 0xFFULL << i * 8, HexPrintStyle::PrefixUpper);
      os << "(" << symText << ")";
    }
    pos += ptrSize;
...
void NVPTXAsmPrinter::setAndEmitFunctionVirtualRegisters(
  ...
  for (unsigned I : toolchain::seq(MRI->getNumVirtRegs())) {
...
void NVPTXAsmPrinter::bufferAggregateConstant(const Constant *CPV,
  ...
  auto ExtendBuffer = [](APInt Val, AggBuffer *Buffer) {
    for (unsigned I : toolchain::seq(Val.getBitWidth() / 8))
  ...
  if (const auto *CDS = dyn_cast<ConstantDataSequential>(CPV)) {
    for (unsigned I : toolchain::seq(CDS->getNumElements()))
  ...
  if (isa<ConstantStruct>(CPV)) {
    if (CPV->getNumOperands()) {
      ...
      for (unsigned I : toolchain::seq(CPV->getNumOperands())) {
