...
//   %cmp = icmp ...
//   %rec = call <vscale x 4 x i1> @toolchain.vp.merge(%cmp, i1 true, %phi, %evl)
//   ...
// middle:
//   %res = call i1 @toolchain.vector.reduce.or(<vscale x 4 x i1> %rec)
//
...
// toolchain/test/CodeGen/RISCV/rvv/vpmerge-sdnode.ll.
//
...
//   %cmp = icmp ...
//   %rec = call <vscale x 4 x i8> @toolchain.vp.merge(%cmp, i8 true, %phi, %evl)
...
bool RISCVCodeGenPrepare::widenVPMerge(IntrinsicInst &II) {
  ...
  toolchain::RecursivelyDeleteTriviallyDeadInstructions(&II);
...
// %acc = call float @toolchain.vector.reduce.fadd.nxv2f32(float %phi,
...
// %acc = call float @toolchain.vector.reduce.fadd.nxv2f32(float %x,
//                                                    <vscale x 2 x float> %vec)
...
bool RISCVCodeGenPrepare::visitIntrinsicInst(IntrinsicInst &I) {
  ...
  if (!PHI || !PHI->hasOneUse() ||
      !toolchain::is_contained(PHI->incoming_values(), &I))
    return false;
...
FunctionPass *toolchain::createRISCVCodeGenPrepareLegacyPass() {
