...
MCCodeEmitter *toolchain::createRISCVMCCodeEmitter(const MCInstrInfo &MCII,
                                              MCContext &Ctx) {
...
void RISCVMCCodeEmitter::expandFunctionCall(const MCInst &MI,
  ...
  support::endian::write(CB, Binary, toolchain::endianness::little);
...
void RISCVMCCodeEmitter::expandLongCondBr(const MCInst &MI,
  ...
  if (UseCompressedBr) {
    ...
    support::endian::write<uint16_t>(CB, Binary, toolchain::endianness::little);
    Offset = 2;
  } else {
...
void RISCVMCCodeEmitter::expandQCLongCondBrImm(const MCInst &MI,
  ...
  if (Size == 4) {
    ...
    support::endian::write(CB, BrBinary, toolchain::endianness::little);
  } else {
    MCInst TmpBr =
    ...
    support::endian::write(Encoding, BrBinary, toolchain::endianness::little);
  ...
  support::endian::write(CB, JBinary, toolchain::endianness::little);
  // Drop any fixup added so we can add the correct one.
  Fixups.resize(FixupStartIndex);
  if (SrcSymbol.isExpr()) {
    addFixup(Fixups, Offset, SrcSymbol.getExpr(), RISCV::fixup_riscv_jal);
...
void RISCVMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  case 2: {
    ...
    support::endian::write<uint16_t>(CB, Bits, toolchain::endianness::little);
    break;
  }
  case 4: {
    uint32_t Bits = getBinaryCodeForInstr(MI, Fixups, STI);
    support::endian::write(CB, Bits, toolchain::endianness::little);
    break;
  }
  case 6: {
    uint64_t Bits = getBinaryCodeForInstr(MI, Fixups, STI) & 0xffff'ffff'ffffu;
    SmallVector<char, 8> Encoding;
    support::endian::write(Encoding, Bits, toolchain::endianness::little);
    assert(Encoding[6] == 0 && Encoding[7] == 0 &&
           "Unexpected encoding for 48-bit instruction");
    Encoding.truncate(6);
    CB.append(Encoding);
...
uint64_t RISCVMCCodeEmitter::getImmOpValue(const MCInst &MI, unsigned OpNo,
  ...
  if (Kind == MCExpr::Specifier) {
    ...
    } else if (MIFrm == RISCVII::InstFormatCODE) {
