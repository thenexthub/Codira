...
parseFeatureBits(bool IsRV64, const FeatureBitset &FeatureBits) {
  ...
  for (auto Feature : RISCVFeatureKV) {
    if (FeatureBits[Feature.Value] &&
        toolchain::RISCVISAInfo::isSupportedExtensionFeature(Feature.Key))
      FeatureVector.push_back(std::string("+") + Feature.Key);
  }
  return toolchain::RISCVISAInfo::parseFeatures(XLen, FeatureVector);
}
...
int RISCVLoadFPImm::getLoadFPImm(APFloat FPImm) {
  ...
  auto EMI = toolchain::lower_bound(LoadFP32ImmArr, std::make_pair(Exp, Mantissa));
