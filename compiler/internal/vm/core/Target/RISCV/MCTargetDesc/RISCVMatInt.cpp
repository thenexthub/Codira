...
static void generateInstSeqImpl(int64_t Val, const MCSubtargetInfo &STI,
  ...
  if (!isInt<32>(Val)) {
    ShiftAmount = toolchain::countr_zero((uint64_t)Val);
    Val >>= ShiftAmount;
...
static unsigned extractRotateInfo(int64_t Val) {
  ...
  unsigned LeadingOnes = toolchain::countl_one((uint64_t)Val);
  unsigned TrailingOnes = toolchain::countr_one((uint64_t)Val);
  if (TrailingOnes > 0 && TrailingOnes < 64 &&
      (LeadingOnes + TrailingOnes) > (64 - 12))
    return 64 - TrailingOnes;

  // for case: 0bxxx1..1..1...xxx
  unsigned UpperTrailingOnes = toolchain::countr_one(Hi_32(Val));
  unsigned LowerLeadingOnes = toolchain::countl_one(Lo_32(Val));
...
static void generateInstSeqLeadingZeros(int64_t Val, const MCSubtargetInfo &STI,
  ...
  unsigned LeadingZeros = toolchain::countl_zero((uint64_t)Val);
...
InstSeq generateInstSeq(int64_t Val, const MCSubtargetInfo &STI) {
  ...
  if ((Val & 0xfff) != 0 && (Val & 1) == 0 && Res.size() >= 2) {
    unsigned TrailingZeros = toolchain::countr_zero((uint64_t)Val);
  ...
  if (Res.size() > 2 && STI.hasFeature(RISCV::FeatureStdExtZbs)) {
    ...
    if (TmpSeq.size() + toolchain::popcount(Hi) < Res.size()) {
      do {
        TmpSeq.emplace_back(RISCV::BSETI, toolchain::countr_zero(Hi));
  ...
  if (Res.size() > 2 && STI.hasFeature(RISCV::FeatureStdExtZbs)) {
    ...
    if (TmpSeq.size() + toolchain::popcount(Hi) < Res.size()) {
      do {
        TmpSeq.emplace_back(RISCV::BCLRI, toolchain::countr_zero(Hi));
        Hi &= (Hi - 1); // Clear lowest set bit.
      } while (Hi != 0);
      Res = TmpSeq;
    }
  ...
  if (Res.size() > 2 && (STI.hasFeature(RISCV::FeatureStdExtZbb) ||
    ...
    if (unsigned Rotate = extractRotateInfo(Val)) {
      RISCVMatInt::InstSeq TmpSeq;
      uint64_t NegImm12 = toolchain::rotl<uint64_t>(Val, Rotate);
...
InstSeq generateTwoRegInstSeq(int64_t Val, const MCSubtargetInfo &STI,
  ...
  unsigned TzLo = toolchain::countr_zero((uint64_t)LoVal);
  unsigned TzHi = toolchain::countr_zero(Tmp);
