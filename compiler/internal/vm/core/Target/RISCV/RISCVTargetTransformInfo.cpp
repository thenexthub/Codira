...
static bool canUseShiftCmp(Instruction *Inst, const APInt &Imm) {
  ...
  unsigned ShiftBits = toolchain::countr_zero(Mask);
...
RISCVTTIImpl::getRegisterBitWidth(TargetTransformInfo::RegisterKind K) const {
  unsigned LMUL =
      toolchain::bit_floor(std::clamp<unsigned>(RVVRegisterWidthLMUL, 1, 8));
  switch (K) {
...
RISCVTTIImpl::getArithmeticReductionCost(unsigned Opcode, VectorType *Ty,
  ...
  if (ElementTy->isIntegerTy(1)) {
    ...
    if (ISD == ISD::AND) {
      ...
      // See the discussion: https://github.com/toolchain/toolchain-project/pull/119160
  ...
  for (auto *BB : L->getBlocks()) {
    for (auto &I : *BB) {
      ...
      if (IsVectorized && (I.getType()->isVectorTy() ||
                           toolchain::any_of(I.operand_values(), [](Value *V) {
