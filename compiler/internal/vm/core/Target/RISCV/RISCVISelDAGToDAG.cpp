...
void RISCVDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::SHL: {
    ...
    if (isShiftedMask_64(Mask)) {
      ...
      unsigned LeadingZeros = XLen - toolchain::bit_width(Mask);
      unsigned TrailingZeros = toolchain::countr_zero(Mask);
  ...
  case ISD::SRL: {
    ...
    if (!isMask_64(Mask))
      break;
    unsigned TrailingOnes = toolchain::countr_one(Mask);
  ...
  case ISD::AND: {
    ...
    if (LeftShift || N0.getOpcode() == ISD::SRL) {
      ...
      if (!LeftShift && isMask_64(C1)) {
        unsigned Leading = XLen - toolchain::bit_width(C1);
        if (C2 < Leading) {
          ...
          const unsigned Msb = toolchain::bit_width(C1) + C2 - 1;
      ...
      if (!LeftShift && isShiftedMask_64(C1)) {
        unsigned Leading = XLen - toolchain::bit_width(C1);
        unsigned Trailing = toolchain::countr_zero(C1);
    ...
    if (isMask_64(C1) && !isInt<12>(N1C->getSExtValue()) &&
      ...
      const unsigned Msb = toolchain::bit_width(C1) - 1;
...
bool RISCVDAGToDAGISel::selectSHXADDOp(SDValue N, unsigned ShAmt,
  ...
  if (N.getOpcode() == ISD::AND && isa<ConstantSDNode>(N.getOperand(1))) {
    ...
    if (bool LeftShift = N0.getOpcode() == ISD::SHL;
      ...
      if (isShiftedMask_64(Mask)) {
        unsigned Leading = XLen - toolchain::bit_width(Mask);
        unsigned Trailing = toolchain::countr_zero(Mask);
        if (Trailing != ShAmt)
          return false;
...
bool RISCVDAGToDAGISel::selectSHXADD_UWOp(SDValue N, unsigned ShAmt,
  ...
  if (N.getOpcode() == ISD::AND && isa<ConstantSDNode>(N.getOperand(1)) &&
    ...
    if (N0.getOpcode() == ISD::SHL && isa<ConstantSDNode>(N0.getOperand(1)) &&
      ...
      if (isShiftedMask_64(Mask)) {
        unsigned Leading = toolchain::countl_zero(Mask);
        unsigned Trailing = toolchain::countr_zero(Mask);
...
bool RISCVDAGToDAGISel::hasAllNBitUsers(SDNode *Node, unsigned Bits,
  ...
  for (SDUse &Use : Node->uses()) {
    ...
    case RISCV::SLLI:
      ...
      return false;
    case RISCV::ANDI:
      if (Bits >= (unsigned)toolchain::bit_width(User->getConstantOperandVal(1)))
        break;
      goto RecCheck;
    case RISCV::ORI: {
      uint64_t Imm = cast<ConstantSDNode>(User->getOperand(1))->getSExtValue();
      if (Bits >= (unsigned)toolchain::bit_width<uint64_t>(~Imm))
        break;
      [[fallthrough]];
    }
    case RISCV::AND:
...
FunctionPass *toolchain::createRISCVISelDag(RISCVTargetMachine &TM,
