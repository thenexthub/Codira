...
static bool isMultipleOfN(const Value *V, const DataLayout &DL, unsigned N) {
  ...
  if (isPowerOf2_32(N)) {
    KnownBits KB = toolchain::computeKnownBits(V, DL);
...
///
/// Into:
/// %ld2 = { <4 x i32>, <4 x i32> } call toolchain.riscv.seg2.load.v4i32.p0.i64(
///                                        %ptr, i64 4)
...
/// call void toolchain.riscv.seg3.store.v4i32.p0.i64(%sub.v0, %sub.v1, %sub.v2,
///                                              %ptr, i32 4)
///
