...
// https://toolchain.org/devmtg/2025-04/slides/technical_talk/lau_accidental_dataflow.pdf
//
//===---------------------------------------------------------------------===//
...
FunctionPass *toolchain::createRISCVVLOptimizerPass() {
...
bool RISCVVLOptimizer::checkUsers(const MachineInstr &MI) const {
  ...
  while (!OpWorklist.empty()) {
    ...
    if (UserMI.isFullCopy() && UserMI.getOperand(0).getReg().isVirtual()) {
      ...
      OpWorklist.insert_range(toolchain::make_pointer_range(
