...
namespace vm::core::RISCVVPseudosTable {
...
static bool analyzeCandidate(outliner::Candidate &C) {
  ...
  if (toolchain::any_of(C, [TRI](const MachineInstr &MI) {
...
RISCVInstrInfo::getOutliningCandidateInfo(
  ...
  toolchain::erase_if(RepeatedSequenceLocs, analyzeCandidate);
...
void RISCVInstrInfo::mulImm(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  MachineRegisterInfo &MRI = MF.getRegInfo();
  if (toolchain::has_single_bit<uint32_t>(Amount)) {
    ...
    BuildMI(MBB, II, DL, get(Opc), DestReg)
        ...
        .addReg(DestReg)
        .setMIFlag(Flag);
  } else if (toolchain::has_single_bit<uint32_t>(Amount - 1)) {
