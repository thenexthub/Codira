...
static void emitSCSPrologue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  const toolchain::RISCVRegisterInfo *TRI = STI.getRegisterInfo();
...
static unsigned getNumPushPopRegs(const std::vector<CalleeSavedInfo> &CSI) {
  ...
  for (auto &CS : CSI) {
    auto *FII = toolchain::find_if(FixedCSRFIMap,
...
getPushOrLibCallsSavedInfo(const MachineFunction &MF,
  ...
  for (const auto &CS : CSI) {
    if (RVFI->useQCIInterrupt(MF)) {
      ...
      // unwinding would use.
      if (toolchain::is_contained(toolchain::make_first_range(FixedCSRFIQCIInterruptMap),
                             CS.getReg()))
        continue;
    }

    if (toolchain::is_contained(FixedCSRFIMap, CS.getReg()))
...
static void appendScalableVectorExpression(const TargetRegisterInfo &TRI,
                                           ...
                                           StackOffset Offset,
                                           toolchain::raw_string_ostream &Comment) {
...
void RISCVFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  for (unsigned i = 0; CSRegs[i]; ++i) {
    ...
    if (!SubRegs.empty() && toolchain::all_of(SubRegs, [&](unsigned Reg) {
...
static bool hasRVVFrameObject(const MachineFunction &MF) {
  ...
  // D103622.
  //
  // Refer to https://github.com/toolchain/toolchain-project/issues/53016.
...
bool RISCVFrameLowering::assignCalleeSavedSpillSlots(
  ...
  for (auto &CS : CSI) {
    ...
    if (RVFI->useQCIInterrupt(MF)) {
      const auto *FFI = toolchain::find_if(FixedCSRFIQCIInterruptMap, [&](auto P) {
    ...
    if (RVFI->useSaveRestoreLibCalls(MF) || RVFI->isPushable(MF)) {
      const auto *FII = toolchain::find_if(
