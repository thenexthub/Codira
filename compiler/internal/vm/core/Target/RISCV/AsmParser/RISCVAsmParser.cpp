  ...
  case AsmToken::Identifier: {
    ...
    if (SysReg) {
      ...
      if (!SysReg->haveRequiredFeatures(FeatureBits)) {
        const auto *Feature = toolchain::find_if(RISCVFeatureKV, [&](auto Feature) {
...
bool RISCVAsmParser::resetToArch(StringRef Arch, SMLoc Loc, std::string &Result,
  ...
  for (auto &Feature : RISCVFeatureKV)
    if (toolchain::RISCVISAInfo::isSupportedExtensionFeature(Feature.Key))
...
bool RISCVAsmParser::parseDirectiveOption() {
  ...
  if (Option == "arch") {
    ...
    do {
      ...
      auto Ext = toolchain::lower_bound(RISCVFeatureKV, Feature);
...
bool isValidInsnFormat(StringRef Format, const MCSubtargetInfo &STI) {
  return StringSwitch<bool>(Format)
      ...
      .Cases({"cr", "ci", "ciw", "css", "cl", "cs", "ca", "cb", "code"},
...
std::unique_ptr<RISCVOperand> RISCVAsmParser::defaultMaskRegOp() const {
  return RISCVOperand::createReg(MCRegister(), toolchain::SMLoc(), toolchain::SMLoc());
}
