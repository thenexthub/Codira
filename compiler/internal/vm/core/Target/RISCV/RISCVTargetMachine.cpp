...
RISCVTargetMachine::getSubtargetImpl(const Function &F) const {
  ...
  if (RVVBitsMin != -1U) {
    ...
    RVVBitsMin = toolchain::bit_floor(
        (RVVBitsMin < 64 || RVVBitsMin > 65536) ? 0 : RVVBitsMin);
  }
  RVVBitsMax =
      toolchain::bit_floor((RVVBitsMax < 64 || RVVBitsMax > 65536) ? 0 : RVVBitsMax);
...
static toolchain::once_flag InitializeDefaultRVVRegisterAllocatorFlag;
...
FunctionPass *RISCVPassConfig::createRVVRegAllocPass(bool Optimized) {
  ...
  toolchain::call_once(InitializeDefaultRVVRegisterAllocatorFlag,
