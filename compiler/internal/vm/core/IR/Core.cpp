...
void toolchain::initializeCore(PassRegistry &Registry) {
...
LLVMContextRef toolchain::getGlobalContextForCAPI() {
...
llvm_getMetadata(size_t *NumEntries,
                 toolchain::function_ref<void(MetadataEntries &)> AccessMD) {
...
static Intrinsic::ID llvm_map_to_intrinsic_id(unsigned ID) {
  assert(ID < toolchain::Intrinsic::num_intrinsics && "Intrinsic ID out of range");
  return toolchain::Intrinsic::ID(ID);
}
...
LLVMValueRef LLVMGetIntrinsicDeclaration(LLVMModuleRef Mod,
  ...
  return wrap(toolchain::Intrinsic::getOrInsertDeclaration(unwrap(Mod), IID, Tys));
}

const char *LLVMIntrinsicGetName(unsigned ID, size_t *NameLength) {
  auto IID = llvm_map_to_intrinsic_id(ID);
  auto Str = toolchain::Intrinsic::getName(IID);
  *NameLength = Str.size();
  return Str.data();
}
...
LLVMTypeRef LLVMIntrinsicGetType(LLVMContextRef Ctx, unsigned ID,
  ...
  auto IID = llvm_map_to_intrinsic_id(ID);
  ArrayRef<Type*> Tys(unwrap(ParamTypes), ParamCount);
  return wrap(toolchain::Intrinsic::getType(*unwrap(Ctx), IID, Tys));
}
...
char *LLVMIntrinsicCopyOverloadedName(unsigned ID, LLVMTypeRef *ParamTypes,
  ...
  auto IID = llvm_map_to_intrinsic_id(ID);
  ArrayRef<Type*> Tys(unwrap(ParamTypes), ParamCount);
  auto Str = toolchain::Intrinsic::getNameNoUnnamedTypes(IID, Tys);
...
char *LLVMIntrinsicCopyOverloadedName2(LLVMModuleRef Mod, unsigned ID,
  ...
  auto IID = llvm_map_to_intrinsic_id(ID);
  ArrayRef<Type *> Tys(unwrap(ParamTypes), ParamCount);
  auto Str = toolchain::Intrinsic::getName(IID, Tys, unwrap(Mod));
  *NameLength = Str.length();
  return strdup(Str.c_str());
}
...
LLVMBool LLVMIntrinsicIsOverloaded(unsigned ID) {
  auto IID = llvm_map_to_intrinsic_id(ID);
  return toolchain::Intrinsic::isOverloaded(IID);
}
...
LLVMBasicBlockRef LLVMCreateBasicBlockInContext(LLVMContextRef C,
                                                const char *Name) {
  return wrap(toolchain::BasicBlock::Create(*unwrap(C), Name));
}
