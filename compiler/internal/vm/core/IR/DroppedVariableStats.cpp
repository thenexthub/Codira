...
#include "vm/core/IR/DroppedVariableStats.h"
...
DroppedVariableStats::DroppedVariableStats(bool DroppedVarStatsEnabled)
  ...
  if (DroppedVarStatsEnabled)
    toolchain::outs() << "Pass Level, Pass Name, Num of Dropped Variables, Func or "
                    "Module Name\n";
}
...
void DroppedVariableStats::calculateDroppedStatsAndPrint(
  ...
  if (DroppedCount > 0) {
    toolchain::outs() << PassLevel << ", " << PassID << ", " << DroppedCount << ", "
                 << FuncOrModName << "\n";
...
void DroppedVariableStats::removeVarFromAllSets(VarID Var, const Function *F) {
  ...
  // stack.
  for (auto &DebugVariablesMap : toolchain::drop_end(DebugVariablesStack))
