...
#include "vm/core/IR/TrackingMDRef.h"
...
SmallVector<Metadata *> ReplaceableMetadataImpl::getAllArgListUsers() {
  ...
  toolchain::sort(MDUsersWithID, [](auto UserA, auto UserB) {
...
ReplaceableMetadataImpl::getAllDbgVariableRecordUsers() {
  ...
  toolchain::sort(DVRUsersWithID, [](auto UserA, auto UserB) {
    return UserA->second > UserB->second;
  });
...
void ReplaceableMetadataImpl::replaceAllUsesWith(Metadata *MD) {
  ...
  toolchain::sort(Uses, [](const UseTy &L, const UseTy &R) {
...
void MDNode::Header::resizeSmallToLarge(size_t NumOps) {
  ...
  toolchain::move(operands(), NewOps.begin());
  resizeSmall(0);
...
static bool hasSelfReference(MDNode *N) {
  return toolchain::is_contained(N->operands(), N);
}
...
// documented in
/// https://toolchain.org/docs/BranchWeightMetadata.html#callinst).
...
void MDAttachments::getAll(
  ...
  if (Result.size() > 1)
    toolchain::stable_sort(Result, less_first());
}
...
bool MDAttachments::erase(unsigned ID) {
  ...
  toolchain::erase_if(Attachments,
...
void Instruction::dropUnknownNonDebugMetadata(ArrayRef<unsigned> KnownIDs) {
  ...
  SmallSet<unsigned, 32> KnownSet(toolchain::from_range, KnownIDs);
...
void Instruction::updateDIAssignIDMapping(DIAssignID *ID) {
  ...
  if (const DIAssignID *CurrentID =
    ...
    auto *InstIt = toolchain::find(InstVec, this);
...
void Instruction::setNoSanitizeMetadata() {
  setMetadata(toolchain::LLVMContext::MD_nosanitize,
              toolchain::MDNode::get(getContext(), {}));
}
...
void GlobalObject::copyMetadata(const GlobalObject *Other, unsigned Offset) {
  ...
  for (auto &MD : MDs) {
    ...
    if (Offset != 0 && MD.first == LLVMContext::MD_dbg) {
      ...
      Elements[1] = Offset;
      toolchain::copy(OrigElements, Elements.begin() + 2);
