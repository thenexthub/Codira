...
// See https://toolchain.org/docs/DebuggingLLVM.html for why these flags are useful.
...
predictValueUseListOrder(const Value *V, unsigned ID, const OrderMap &OM) {
  ...
  toolchain::sort(List, [&](const Entry &L, const Entry &R) {
  ...
  if (toolchain::is_sorted(List, toolchain::less_second()))
...
void toolchain::printLLVMNameWithoutPrefix(raw_ostream &OS, StringRef Name) {
...
///
class toolchain::SlotTracker : public AbstractSlotTrackerStorage {
public:
...
void SlotTracker::processFunction() {
  ...
  for (auto &BB : *TheFunction) {
    ...
    for (auto &I : BB) {
      ...
      // We allow direct calls to any toolchain.foo function here, because the
...
void AssemblyWriter::printArgs(ArrayRef<uint64_t> Args) {
  Out << "args: (" << toolchain::interleaved(Args) << ')';
}
...
void AssemblyWriter::printGlobal(const GlobalVariable *GV) {
  ...
  using SanitizerMetadata = toolchain::GlobalValue::SanitizerMetadata;
...
void AssemblyWriter::printInstruction(const Instruction &I) {
  ...
  if (isa<BranchInst>(I) && cast<BranchInst>(I).isConditional()) {
    ...
    Out << ", ";
    Out << toolchain::interleaved(EVI->indices());
  } else if (const auto *IVI = dyn_cast<InsertValueInst>(&I)) {
    Out << ' ';
    writeOperand(I.getOperand(0), true); Out << ", ";
    writeOperand(I.getOperand(1), true);
    Out << ", ";
    Out << toolchain::interleaved(IVI->indices());
...
void AssemblyWriter::writeAllMDNodes() {
  ...
  for (auto &I : toolchain::make_range(Machine.mdn_begin(), Machine.mdn_end()))
...
void AssemblyWriter::writeAllAttributeGroups() {
  ...
  for (auto &I : toolchain::make_range(Machine.as_begin(), Machine.as_end()))
    asVec[I.second] = I;
...
void AssemblyWriter::printUseListOrder(const Value *V,
  ...
  Out << ", { " << toolchain::interleaved(Shuffle) << " }\n";
}
...
void ModuleSlotTracker::collectMDNodes(MachineMDNodeListType &L, unsigned LB,
  ...
  for (auto &I : toolchain::make_range(ST->mdn_begin(), ST->mdn_end()))
