...
#include "vm/core/IR/LegacyPassManagers.h"
...
void PMDataManager::emitInstrCountChangedRemark(
  ...
  if (!CouldOnlyImpactOneFunction)
    toolchain::for_each(M, UpdateFunctionChanges);
  else
  ...
  if (!CouldOnlyImpactOneFunction) {
    ...
    auto It = toolchain::find_if(M, [](const Function &Fn) { return !Fn.empty(); });
  ...
  if (!CouldOnlyImpactOneFunction)
    toolchain::for_each(FunctionToInstrCount.keys(), EmitFunctionSizeChangedRemark);
  else
...
public:
  ...
  using toolchain::Pass::doInitialization;
  using toolchain::Pass::doFinalization;
...
bool FPPassManager::runOnFunction(Function &F) {
  ...
  toolchain::TimeTraceScope FunctionScope("OptFunction", Name);
  ...
  for (unsigned Index = 0; Index < getNumContainedPasses(); ++Index) {
    ...
    toolchain::TimeTraceScope PassScope(
...

    {
      ...
      if (!LocalChanged && (RefHash != FP->structuralHash(F))) {
        toolchain::errs() << "Pass modifies its input and doesn't report it: "
                     << FP->getPassName() << "\n";
...
MPPassManager::runOnModule(Module &M) {
  toolchain::TimeTraceScope TimeScope("OptModule", M.getName());
