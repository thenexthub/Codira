...
#include "vm/core/Support/CheckedArithmetic.h"
...
void GetElementPtrInst::init(Value *Ptr, ArrayRef<Value *> IdxList,
  ...
  Op<0>() = Ptr;
  toolchain::copy(IdxList, op_begin() + 1);
  setName(Name);
}
...
bool ShuffleVectorInst::isReplicationMask(ArrayRef<int> Mask,
  ...
  if (!toolchain::is_contained(Mask, PoisonMaskElem)) {
    ReplicationFactor =
...
bool ShuffleVectorInst::isInterleave(unsigned Factor) {
  ...
  // vectors, see the @toolchain.vector.interleave2 intrinsic
  if (!OpTy)
    return false;
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, CmpInst::Predicate Pred) {
