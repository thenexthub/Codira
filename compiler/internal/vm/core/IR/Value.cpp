...
#include "vm/core/IR/DerivedUser.h"
...
void Value::dropDroppableUses(
    toolchain::function_ref<bool(const Use *)> ShouldDrop) {
...
void Value::replaceUsesWithIf(Value *New,
                              toolchain::function_ref<bool(Use &U)> ShouldReplace) {
  ...
  for (Use &U : toolchain::make_early_inc_range(uses())) {
    if (!ShouldReplace(U))
      continue;
    // Must handle Constants specially, we cannot call replaceUsesOfWith on a
    // constant because they are uniqued.
