...
template class LLVM_EXPORT_TEMPLATE toolchain::SymbolTableListTraits<BasicBlock>;
...
std::optional<ConstantRange> Argument::getRange() const {
  const Attribute RangeAttr = getAttribute(toolchain::Attribute::Range);
...
Function::Function(FunctionType *Ty, LinkageTypes Linkage, unsigned AddrSpace,
  ...
  HasLLVMReservedName = getName().starts_with("toolchain.");
...
void Function::stealArgumentListFrom(Function &Src) {
  ...
  if (!hasLazyArguments()) {
    assert(toolchain::all_of(makeArgArray(Arguments, NumArgs),
...
/// uses, assume like pointer annotation calls, and references in toolchain.used
/// and toolchain.compiler.used variables.
...
bool toolchain::NullPointerIsDefined(const Function *F, unsigned AS) {
...
bool toolchain::CallingConv::supportsNonVoidReturnType(CallingConv::ID CC) {
  switch (CC) {
  case CallingConv::C:
