...
Attribute Attribute::getWithAlignment(LLVMContext &Context, Align A) {
  assert(A <= toolchain::Value::MaximumAlignment && "Alignment too large.");
...
std::string Attribute::getAsString(bool InAttrGrp) const {
  ...
  if (hasAttribute(Attribute::AllocKind)) {
    ...
    return ("allockind(\"" +
            Twine(toolchain::join(parts.begin(), parts.end(), ",")) + "\")")
        .str();
  }
...
AttributeSetNode::AttributeSetNode(ArrayRef<Attribute> Attrs)
  ...
  toolchain::copy(Attrs, getTrailingObjects());
...
AttributeSetNode *AttributeSetNode::getSorted(LLVMContext &C,
  ...
  assert(toolchain::is_sorted(SortedAttrs) && "Expected sorted attributes!");
...
AttributeListImpl::AttributeListImpl(ArrayRef<AttributeSet> Sets)
  ...
  // There's memory after the node where we can store the entries in.
  toolchain::copy(Sets, getTrailingObjects());
...
AttributeList::get(LLVMContext &C,
  ...
  assert(toolchain::is_sorted(Attrs, toolchain::less_first()) &&
         "Misordered Attributes list!");
  assert(toolchain::all_of(Attrs,
...
AttributeList::get(LLVMContext &C,
  ...
  assert(toolchain::is_sorted(Attrs, toolchain::less_first()) &&
         "Misordered Attributes list!");
  assert(toolchain::none_of(Attrs,
...
AttributeList AttributeList::get(LLVMContext &C, AttributeSet FnAttrs,
  ...
  if (NumSets > 2) {
    ...
    toolchain::append_range(AttrSets, ArgAttrs);
  }
...
AttributeList AttributeList::addParamAttribute(LLVMContext &C,
                                               ...
                                               Attribute A) const {
  assert(toolchain::is_sorted(ArgNos));
...
AttributeList::intersectWith(LLVMContext &C, AttributeList Other) const {
  ...
  toolchain::sort(IntersectedAttrs, toolchain::less_first());
...
AttrBuilder &AttrBuilder::addAlignmentAttr(MaybeAlign Align) {
  ...
  assert(*Align <= toolchain::Value::MaximumAlignment && "Alignment too large.");
...
#define ATTRIBUTE_ENUM(ENUM_NAME, DISPLAY_NAME)                                \
  struct ENUM_NAME##Attr : EnumAttr {                                          \
    static enum Attribute::AttrKind getKind() {                                \
      return toolchain::Attribute::ENUM_NAME;                                       \
    }                                                                          \
  };
...
void AttributeFuncs::updateMinLegalVectorWidthAttr(Function &Fn,
  ...
  if (Attr.isValid()) {
    ...
    if (Width > OldWidth)
      Fn.addFnAttr("min-legal-vector-width", toolchain::utostr(Width));
  }
}
