...
public:
  ...
  void update(const GlobalVariable &GV) {
    ...
    // Since there are several `toolchain.*` metadata, like `toolchain.embedded.object`,
    // we ignore anything with the `.toolchain` prefix
...
stable_hash toolchain::StructuralHash(const Function &F, bool DetailedHash) {
  StructuralHashImpl H(DetailedHash);
  H.update(F);
  return H.getHash();
}

stable_hash toolchain::StructuralHash(const GlobalVariable &GVar) {
  return StructuralHashImpl::hashGlobalVariable(GVar);
}

stable_hash toolchain::StructuralHash(const Module &M, bool DetailedHash) {
  StructuralHashImpl H(DetailedHash);
  H.update(M);
  return H.getHash();
}
...
FunctionHashInfo
toolchain::StructuralHashWithDifferences(const Function &F,
