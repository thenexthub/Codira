...
#include "vm/core/IR/ConstantRangeList.h"
...
#include "vm/core/IR/IntrinsicsARM.h"
#include "vm/core/IR/IntrinsicsNVPTX.h"
#include "vm/core/IR/IntrinsicsWebAssembly.h"
...
#include "vm/core/Support/AMDGPUAddrSpace.h"
...
static cl::opt<bool> VerifyNoAliasScopeDomination(
    ...
    cl::desc("Ensure that toolchain.experimental.noalias.scope.decl for identical "
             "scopes are not dominating"));

struct toolchain::VerifierSupport {
  raw_ostream *OS;
  const Module &M;
...
class Verifier : public InstVisitor<Verifier>, VerifierSupport {
  ...
  /// Whether we've seen a call to @toolchain.localescape in this function
  /// already.
  bool SawFrameEscape;
  ...
  /// Stores the count of how many objects were passed to toolchain.localescape for a
  /// given function and the largest index passed to toolchain.localrecover.
  ...
  /// Cache of declarations of the toolchain.experimental.deoptimize.<ty> intrinsic.
...
public:
  ...
  bool verify(const Function &F) {
    toolchain::TimeTraceScope timeScope("Verifier");
  ...
  bool verify() {
    ...
    // Collect all declarations of the toolchain.experimental.deoptimize intrinsic.
...
#include "vm/core/IR/Metadata.def"
  ...
  /// Module-level verification that all @toolchain.experimental.deoptimize
  ...
  /// Verify the toolchain.experimental.noalias.scope.decl declarations
...
static void forEachUser(const Value *User,
                        ...
                        toolchain::function_ref<bool(const Value *)> Callback) {
...
void Verifier::visitGlobalVariable(const GlobalVariable &GV) {
  ...
  if (GV.hasName() && (GV.getName() == "toolchain.global_ctors" ||
                       GV.getName() == "toolchain.global_dtors")) {
  ...
  if (GV.hasName() && (GV.getName() == "toolchain.used" ||
                       GV.getName() == "toolchain.compiler.used")) {
    Check(!GV.hasInitializer() || GV.hasAppendingLinkage(),
          "invalid linkage for intrinsic global variable", &GV);
    Check(GV.materialized_use_empty(),
          "invalid uses of intrinsic global variable", &GV);
...
void Verifier::visitNamedMDNode(const NamedMDNode &NMD) {
  // There used to be various other toolchain.dbg.* nodes, but we don't support
  // upgrading them and we want to reserve the namespace for future uses.
  if (NMD.getName().starts_with("toolchain.dbg."))
    CheckDI(NMD.getName() == "toolchain.dbg.cu",
            "unrecognized named metadata node in the toolchain.dbg namespace", &NMD);
  for (const MDNode *MD : NMD.operands()) {
    if (NMD.getName() == "toolchain.dbg.cu")
...
#include "vm/core/IR/Metadata.def"
  ...
  // Check toolchain.loop.estimated_trip_count.
...
void Verifier::visitDICompositeType(const DICompositeType &N) {
  ...
  CheckDI(toolchain::all_of(N.getElements(), [](const DINode *N) { return N; }),
...
void Verifier::visitDIFile(const DIFile &N) {
  ...
  if (Checksum) {
    ...
    CheckDI(Checksum->Value.find_if_not(toolchain::isHexDigit) == StringRef::npos,
            "invalid checksum", &N);
  }
}
...
void Verifier::visitModuleIdents() {
  const NamedMDNode *Idents = M.getNamedMetadata("toolchain.ident");
  if (!Idents)
    return;

  // toolchain.ident takes a list of metadata entry. Each entry has only one string.
  // Scan each toolchain.ident entry and make sure that this requirement is met.
  for (const MDNode *N : Idents->operands()) {
    Check(N->getNumOperands() == 1,
          "incorrect number of operands in toolchain.ident metadata", N);
    Check(dyn_cast_or_null<MDString>(N->getOperand(0)),
          ("invalid value for toolchain.ident metadata entry operand"
...
void Verifier::visitModuleCommandLines() {
  const NamedMDNode *CommandLines = M.getNamedMetadata("toolchain.commandline");
  if (!CommandLines)
    return;

  // toolchain.commandline takes a list of metadata entry. Each entry has only one
  // string. Scan each toolchain.commandline entry and make sure that this
  // requirement is met.
  for (const MDNode *N : CommandLines->operands()) {
    Check(N->getNumOperands() == 1,
          "incorrect number of operands in toolchain.commandline metadata", N);
    Check(dyn_cast_or_null<MDString>(N->getOperand(0)),
          ("invalid value for toolchain.commandline metadata entry operand"
           "(the operand should be a string)"),
          N->getOperand(0));
  }
}
...
void Verifier::visitModuleErrnoTBAA() {
  const NamedMDNode *ErrnoTBAA = M.getNamedMetadata("toolchain.errno.tbaa");
  if (!ErrnoTBAA)
    return;

  Check(ErrnoTBAA->getNumOperands() >= 1,
        "toolchain.errno.tbaa must have at least one operand", ErrnoTBAA);
...
Verifier::visitModuleFlag(const MDNode *Op,
  ...
  if (ID->getString() == "Linker Options") {
    // If the toolchain.linker.options named metadata exists, we assume that the
    // bitcode reader has upgraded the module flag. Otherwise the flag might
    // have been created by a client directly.
    Check(M.getNamedMetadata("toolchain.linker.options"),
...
#include "vm/core/IR/Attributes.inc"
      continue;
    }
...
void Verifier::verifyFrameRecoverIndices() {
  for (auto &Counts : FrameEscapeInfo) {
    ...
    Check(MaxRecoveredIndex <= EscapedObjectCount,
          "all indices passed to toolchain.localrecover must be less than the "
          "number of arguments passed to toolchain.localescape in the parent "
          "function",
          F);
  }
}
...
void Verifier::verifySiblingFuncletUnwinds() {
  toolchain::TimeTraceScope timeScope("Verifier verify sibling funclet unwinds");
...
void Verifier::visitFunction(const Function &F) {
  ...
  if (F.isMaterializable()) {
    ...
    // Verify that this function (which has a body) is not named "toolchain.*".  It
    // is not legal to define intrinsics.
    Check(!IsIntrinsic, "toolchain intrinsics cannot be defined!", &F);
  ...
  for (auto &BB : F)
    for (auto &I : BB) {
      ...
      // The toolchain.loop annotations also contain two DILocations.
...
void Verifier::visitCallBase(CallBase &Call) {
  ...
  if (Attrs.hasFnAttr(Attribute::Preallocated)) {
    Check(Call.getIntrinsicID() == Intrinsic::call_preallocated_arg,
          ...
          "toolchain.call.preallocated.arg");
  }
  ...
  for (unsigned i = 0, e = Call.getNumOperandBundles(); i < e; ++i) {
    ...
    if (Tag == LLVMContext::OB_deopt) {
      ...
      Check(Input &&
            ...
            "toolchain.call.preallocated.setup",
            Call);
...
void Verifier::verifySwiftErrorCall(CallBase &Call,
  ...
  for (const auto &I : toolchain::enumerate(Call.args())) {
  ...
  if (auto *AsValue = MetadataAsValue::getIfExists(Context, MD)) {
    for (auto *User : AsValue->users()) {
      CheckDI(isa<DbgAssignIntrinsic>(User),
              "!DIAssignID should only be used by toolchain.dbg.assign intrinsics",
              MD, User);
...
void Verifier::visitMemProfMetadata(Instruction &I, MDNode *MD) {
  ...
  for (auto &MIBOp : MD->operands()) {
    ...
    for (unsigned I = 2; I < MIB->getNumOperands(); ++I) {
      ...
      Check(toolchain::all_of(OpNode->operands(),
                         [](const MDOperand &Op) {
...
void Verifier::visitIntrinsicCall(Intrinsic::ID ID, CallBase &Call) {
  ...
  case Intrinsic::coro_id: {
    ...
    Check(GV && GV->isConstant() && GV->hasDefinitiveInitializer(),
          "info argument of toolchain.coro.id must refer to an initialized "
          "constant");
    Constant *Init = GV->getInitializer();
    Check(isa<ConstantStruct>(Init) || isa<ConstantArray>(Init),
          "info argument of toolchain.coro.id must refer to either a struct or "
          "an array");
    break;
  }
  case Intrinsic::is_fpclass: {
    ...
    Check((TestMask->getZExtValue() & ~static_cast<unsigned>(fcAllFlags)) == 0,
          "unsupported bits for toolchain.is.fpclass test mask");
    break;
  }
  case Intrinsic::fptrunc_round: {
    ...
    Check(isa<MDString>(MD),
          ("invalid value for toolchain.fptrunc.round metadata operand"
           " (the operand should be a string)"),
          MD);
...
#include "vm/core/IR/VPIntrinsics.def"
...
#undef INSTRUCTION
    ...
    break;
  case Intrinsic::dbg_declare: // toolchain.dbg.declare
  case Intrinsic::dbg_value:   // toolchain.dbg.value
  case Intrinsic::dbg_assign:  // toolchain.dbg.assign
  case Intrinsic::dbg_label:   // toolchain.dbg.label
  ...
  case Intrinsic::call_preallocated_setup: {
    ...
    for (User *U : Call.users()) {
      ...
      Check(UseCall != nullptr,
            "Uses of toolchain.call.preallocated.setup must be calls");
      ...
      if (IID == Intrinsic::call_preallocated_arg) {
        ...
        Check(AllocArgIndex != nullptr,
              "toolchain.call.preallocated.alloc arg index must be a constant");
        auto AllocArgIndexInt = AllocArgIndex->getValue();
        Check(AllocArgIndexInt.sge(0) &&
                  AllocArgIndexInt.slt(NumArgs->getValue()),
              "toolchain.call.preallocated.alloc arg index must be between 0 and "
              "corresponding "
              "toolchain.call.preallocated.setup's argument count");
        ...
        Check(NumArgs->equalsInt(NumPreallocatedArgs),
              "toolchain.call.preallocated.setup arg size must be equal to number "
        ...
        Check(PreallocatedBundle,
              "Use of toolchain.call.preallocated.setup outside intrinsics "
  ...
  case Intrinsic::call_preallocated_arg: {
    ...
    Check(Token &&
          ...
          "toolchain.call.preallocated.arg token argument must be a "
          "toolchain.call.preallocated.setup");
    Check(Call.hasFnAttr(Attribute::Preallocated),
          "toolchain.call.preallocated.arg must be called with a \"preallocated\" "
          "call site attribute");
    break;
  }
  case Intrinsic::call_preallocated_teardown: {
    ...
    Check(Token &&
              Token->getIntrinsicID() == Intrinsic::call_preallocated_setup,
          "toolchain.call.preallocated.teardown token argument must be a "
          "toolchain.call.preallocated.setup");
    break;
  }
  ...
  case Intrinsic::gcread:
    if (ID == Intrinsic::gcroot) {
      ...
      Check(AI, "toolchain.gcroot parameter #1 must be an alloca.", Call);
      Check(isa<Constant>(Call.getArgOperand(1)),
            "toolchain.gcroot parameter #2 must be a constant.", Call);
      if (!AI->getAllocatedType()->isPointerTy()) {
        Check(!isa<ConstantPointerNull>(Call.getArgOperand(1)),
              "toolchain.gcroot parameter #1 must either be a pointer alloca, "
  ...
  case Intrinsic::init_trampoline:
    Check(isa<Function>(Call.getArgOperand(1)->stripPointerCasts()),
          "toolchain.init_trampoline parameter #2 must resolve to a function.",
          Call);
    break;
  case Intrinsic::prefetch:
    Check(cast<ConstantInt>(Call.getArgOperand(1))->getZExtValue() < 2,
          "rw argument to toolchain.prefetch must be 0-1", Call);
    Check(cast<ConstantInt>(Call.getArgOperand(2))->getZExtValue() < 4,
          "locality argument to toolchain.prefetch must be 0-3", Call);
    Check(cast<ConstantInt>(Call.getArgOperand(3))->getZExtValue() < 2,
          "cache type argument to toolchain.prefetch must be 0-1", Call);
    break;
  case Intrinsic::reloc_none: {
    Check(isa<MDString>(
              cast<MetadataAsValue>(Call.getArgOperand(0))->getMetadata()),
          "toolchain.reloc.none argument must be a metadata string", &Call);
    break;
  }
  case Intrinsic::stackprotector:
    Check(isa<AllocaInst>(Call.getArgOperand(1)->stripPointerCasts()),
          "toolchain.stackprotector parameter #2 must resolve to an alloca.", Call);
    break;
  case Intrinsic::localescape: {
    BasicBlock *BB = Call.getParent();
    Check(BB->isEntryBlock(), "toolchain.localescape used outside of entry block",
          Call);
    Check(!SawFrameEscape, "multiple calls to toolchain.localescape in one function",
          Call);
    for (Value *Arg : Call.args()) {
      ...
      Check(AI && AI->isStaticAlloca(),
            "toolchain.localescape only accepts static allocas", Call);
    }
  ...
  case Intrinsic::experimental_gc_relocate: {
    ...
    auto GC = toolchain::getGCStrategy(Relocate.getFunction()->getGC());
  ...
  case Intrinsic::aarch64_prefetch: {
    Check(cast<ConstantInt>(Call.getArgOperand(1))->getZExtValue() < 2,
          "write argument to toolchain.aarch64.prefetch must be 0 or 1", Call);
    Check(cast<ConstantInt>(Call.getArgOperand(2))->getZExtValue() < 4,
          "target argument to toolchain.aarch64.prefetch must be 0-3", Call);
    Check(cast<ConstantInt>(Call.getArgOperand(3))->getZExtValue() < 2,
          "stream argument to toolchain.aarch64.prefetch must be 0 or 1", Call);
    Check(cast<ConstantInt>(Call.getArgOperand(4))->getZExtValue() < 2,
          "isdata argument to toolchain.aarch64.prefetch must be 0 or 1", Call);
    break;
  }
  case Intrinsic::aarch64_range_prefetch: {
    Check(cast<ConstantInt>(Call.getArgOperand(1))->getZExtValue() < 2,
          "write argument to toolchain.aarch64.range.prefetch must be 0 or 1", Call);
    Check(cast<ConstantInt>(Call.getArgOperand(2))->getZExtValue() < 2,
          "stream argument to toolchain.aarch64.range.prefetch must be 0 or 1",
          Call);
    break;
  }
  case Intrinsic::callbr_landingpad: {
    ...
    Check(toolchain::is_contained(CBR->getIndirectDests(), LandingPadBB),
  ...
  case Intrinsic::amdgcn_cs_chain: {
    ...
    Check(Next && (isa<UnreachableInst>(Next) || IsAMDUnreachable),
          "toolchain.amdgcn.cs.chain must be followed by unreachable", &Call);
    break;
  }
  ...
  case Intrinsic::amdgcn_s_prefetch_data: {
    Check(
        ...
        "toolchain.amdgcn.s.prefetch.data only supports global or constant memory");
    break;
  }
  ...
  case Intrinsic::ptrmask: {
    ...
    Check(Ty0->isPtrOrPtrVectorTy(),
          "toolchain.ptrmask intrinsic first argument must be pointer or vector "
          "of pointers",
          &Call);
    Check(
        Ty0->isVectorTy() == Ty1->isVectorTy(),
        "toolchain.ptrmask intrinsic arguments must be both scalars or both vectors",
        &Call);
    if (Ty0->isVectorTy())
      Check(cast<VectorType>(Ty0)->getElementCount() ==
                cast<VectorType>(Ty1)->getElementCount(),
            "toolchain.ptrmask intrinsic arguments must have the same number of "
            "elements",
            &Call);
    Check(DL.getIndexTypeSizeInBits(Ty0) == Ty1->getScalarSizeInBits(),
          "toolchain.ptrmask intrinsic second argument bitwidth must match "
  ...
  case Intrinsic::thread_pointer: {
    Check(Call.getType()->getPointerAddressSpace() ==
          ...
          "toolchain.thread.pointer intrinsic return type must be for the globals "
          "address space",
          &Call);
    break;
  }
  case Intrinsic::threadlocal_address: {
    ...
    Check(isa<GlobalValue>(Arg0),
          "toolchain.threadlocal.address first argument must be a GlobalValue");
    Check(cast<GlobalValue>(Arg0).isThreadLocal(),
          "toolchain.threadlocal.address operand isThreadLocal() must be true");
    break;
  }
  ...
  case Intrinsic::lifetime_end: {
    ...
    Check(isa<AllocaInst>(Ptr) || isa<PoisonValue>(Ptr),
          "toolchain.lifetime.start/end can only be used on alloca or poison",
          &Call);
    break;
  }
  };
...
void Verifier::visitVPIntrinsic(VPIntrinsic &VPI) {
  if (auto *VPCast = dyn_cast<VPCastIntrinsic>(&VPI)) {
    ...
    case Intrinsic::vp_trunc:
      Check(RetTy->isIntOrIntVectorTy() && ValTy->isIntOrIntVectorTy(),
            "toolchain.vp.trunc intrinsic first argument and result element type "
            "must be integer",
            *VPCast);
      Check(RetTy->getScalarSizeInBits() < ValTy->getScalarSizeInBits(),
            "toolchain.vp.trunc intrinsic the bit size of first argument must be "
    ...
    case Intrinsic::vp_sext:
      Check(RetTy->isIntOrIntVectorTy() && ValTy->isIntOrIntVectorTy(),
            "toolchain.vp.zext or toolchain.vp.sext intrinsic first argument and result "
            "element type must be integer",
            *VPCast);
      Check(RetTy->getScalarSizeInBits() > ValTy->getScalarSizeInBits(),
            "toolchain.vp.zext or toolchain.vp.sext intrinsic the bit size of first "
    ...
    case Intrinsic::vp_llrint:
      Check(
          ...
          "toolchain.vp.fptoui, toolchain.vp.fptosi, toolchain.vp.lrint or toolchain.vp.llrint" "intrinsic first argument element "
    ...
    case Intrinsic::vp_sitofp:
      Check(
          ...
          "toolchain.vp.uitofp or toolchain.vp.sitofp intrinsic first argument element "
    ...
    case Intrinsic::vp_fptrunc:
      Check(RetTy->isFPOrFPVectorTy() && ValTy->isFPOrFPVectorTy(),
            "toolchain.vp.fptrunc intrinsic first argument and result element type "
            "must be floating-point",
            *VPCast);
      Check(RetTy->getScalarSizeInBits() < ValTy->getScalarSizeInBits(),
            "toolchain.vp.fptrunc intrinsic the bit size of first argument must be "
            "larger than the bit size of the return type",
            *VPCast);
      break;
    case Intrinsic::vp_fpext:
      Check(RetTy->isFPOrFPVectorTy() && ValTy->isFPOrFPVectorTy(),
            "toolchain.vp.fpext intrinsic first argument and result element type "
            "must be floating-point",
            *VPCast);
      Check(RetTy->getScalarSizeInBits() > ValTy->getScalarSizeInBits(),
            "toolchain.vp.fpext intrinsic the bit size of first argument must be "
            "smaller than the bit size of the return type",
            *VPCast);
      break;
    case Intrinsic::vp_ptrtoint:
      Check(RetTy->isIntOrIntVectorTy() && ValTy->isPtrOrPtrVectorTy(),
            "toolchain.vp.ptrtoint intrinsic first argument element type must be "
            "pointer and result element type must be integer",
            *VPCast);
      break;
    case Intrinsic::vp_inttoptr:
      Check(RetTy->isPtrOrPtrVectorTy() && ValTy->isIntOrIntVectorTy(),
            "toolchain.vp.inttoptr intrinsic first argument element type must be "
  ...
  case Intrinsic::vp_is_fpclass: {
    ...
    Check((TestMask->getZExtValue() & ~static_cast<unsigned>(fcAllFlags)) == 0,
          "unsupported bits for toolchain.vp.is.fpclass test mask");
    break;
  }
...
void Verifier::verifyNotEntryValue(const DbgVariableRecord &DVR) {
  ...
  if (isa<ValueAsMetadata>(DVR.getRawLocation())) {
    ...
    if (isa<UndefValue>(VarValue) || isa<PoisonValue>(VarValue))
      return;
    // We allow EntryValues for codira async arguments, as they have an
...
void Verifier::verifyCompileUnits() {
  ...
  if (M.getContext().isODRUniquingDebugTypes())
    return;
  auto *CUs = M.getNamedMetadata("toolchain.dbg.cu");
  SmallPtrSet<const Metadata *, 2> Listed;
  if (CUs)
    Listed.insert_range(CUs->operands());
  for (const auto *CU : CUVisited)
    CheckDI(Listed.count(CU), "DICompileUnit not listed in toolchain.dbg.cu", CU);
  CUVisited.clear();
}
...
void Verifier::verifyDeoptimizeCallingConvs() {
  ...
  for (const auto *F : ArrayRef(DeoptimizeDeclarations).slice(1)) {
    Check(First->getCallingConv() == F->getCallingConv(),
          "All toolchain.experimental.deoptimize declarations must have the same "
          "calling convention",
          First, F);
  }
}
...
void Verifier::verifyNoAliasScopeDecl() {
  ...
  toolchain::sort(NoAliasScopeDecls, Compare);
  ...
  while (ItCurrent != NoAliasScopeDecls.end()) {
    ...
    if (ItNext - ItCurrent < 32)
      for (auto *I : toolchain::make_range(ItCurrent, ItNext))
        for (auto *J : toolchain::make_range(ItCurrent, ItNext))
          if (I != J)
            Check(!DT.dominates(I, J),
                  "toolchain.experimental.noalias.scope.decl dominates another one "
                  "with the same scope",
                  I);
    ItCurrent = ItNext;
  }
...
bool toolchain::verifyFunction(const Function &f, raw_ostream *OS) {
...
bool toolchain::verifyModule(const Module &M, raw_ostream *OS,
...
static bool isNewFormatTBAATypeNode(toolchain::MDNode *Type) {
...
FunctionPass *toolchain::createVerifierPass(bool FatalErrors) {
...
VerifierAnalysis::Result VerifierAnalysis::run(Module &M,
  ...
  Result Res;
  Res.IRBroken = toolchain::verifyModule(M, &dbgs(), &Res.DebugInfoBroken);
  return Res;
}

VerifierAnalysis::Result VerifierAnalysis::run(Function &F,
                                               FunctionAnalysisManager &) {
  return { toolchain::verifyFunction(F, &dbgs()), false };
}
