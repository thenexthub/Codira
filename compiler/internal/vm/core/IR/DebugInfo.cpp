...
using namespace vm::core::at;
using namespace vm::core::dwarf;

TinyPtrVector<DbgVariableRecord *> toolchain::findDVRDeclares(Value *V) {
...
TinyPtrVector<DbgVariableRecord *> toolchain::findDVRDeclareValues(Value *V) {
  // This function is hot. Check whether the value has any metadata to avoid a
  // DenseMap lookup. This check is a bitfield datamember lookup.
  if (!V->isUsedByMetadata())
    return {};
...
TinyPtrVector<DbgVariableRecord *> toolchain::findDVRValues(Value *V) {
  // This function is hot. Check whether the value has any metadata to avoid a
  // DenseMap lookup. This check is a bitfield datamember lookup.
  if (!V->isUsedByMetadata())
    return {};
...
void toolchain::findDbgUsers(
    Value *V, SmallVectorImpl<DbgVariableRecord *> &DbgVariableRecords) {
  findDbgIntrinsics</*DbgAssignAndValuesOnly=*/false>(V, DbgVariableRecords);
}

DISubprogram *toolchain::getDISubprogram(const MDNode *Scope) {
  if (auto *LocalScope = dyn_cast_or_null<DILocalScope>(Scope))
    return LocalScope->getSubprogram();
  return nullptr;
}

DebugLoc toolchain::getDebugValueLoc(DbgVariableRecord *DVR) {
...
void DebugInfoFinder::processSubprogram(DISubprogram *SP) {
  ...
  // toolchain.dbg.cu list. Therefore we need to collect DICompileUnit's here as
...
static Metadata *updateLoopMetadataDebugLocationsRecursive(
  ...
  for (Metadata *MD : toolchain::drop_begin(M->operands())) {
    if (!MD) {
...
static MDNode *updateLoopMetadataDebugLocationsImpl(
  ...
  for (Metadata *MD : toolchain::drop_begin(OrigLoopID->operands())) {
    if (!MD)
      MDs.push_back(nullptr);
...
void toolchain::updateLoopMetadataDebugLocations(
...
static MDNode *stripDebugLocFromLoopID(MDNode *N) {
  ...
  if (!toolchain::count_if(toolchain::drop_begin(N->operands()),
  ...
  if (toolchain::all_of(toolchain::drop_begin(N->operands()),
...
bool toolchain::stripDebugInfo(Function &F) {
  bool Changed = false;
...
bool toolchain::StripDebugInfo(Module &M) {
  toolchain::TimeTraceScope timeScope("Strip debug info");
  bool Changed = false;

  for (NamedMDNode &NMD : toolchain::make_early_inc_range(M.named_metadata())) {
    // We're stripping debug info, and without them, coverage information
    // doesn't quite make sense.
    if (NMD.getName().starts_with("toolchain.dbg.") ||
        NMD.getName() == "toolchain.gcov") {
      NMD.eraseFromParent();
      Changed = true;
    }
  }
...
bool toolchain::stripNonLineTableDebugInfo(Module &M) {
  bool Changed = false;
  ...
  for (auto &F : M) {
    ...
    for (auto &BB : F) {
      for (auto &I : BB) {
        ...
        // Remap DILocations in toolchain.loop attachments.
  ...
  // Create a new toolchain.dbg.cu, which is equivalent to the one
  // -gline-tables-only would have created.
...
unsigned toolchain::getDebugMetadataVersionFromModule(const Module &M) {
...
static toolchain::DIFile::ChecksumKind
...
LLVMMetadataRef LLVMDIBuilderCreateFileWithChecksum(
  ...
  toolchain::DIFile::ChecksumInfo<StringRef> CSInfo(CSK, ChkSum);
...
LLVMDbgRecordRef LLVMDIBuilderInsertDeclareRecordBefore(
  ...
  // debug info format.
  // See https://toolchain.org/docs/RemoveDIsDebugInfo.html#c-api-changes,
...
bool AssignmentTrackingPass::runOnFunction(Function &F) {
  ...
  // toolchain/docs/SourceLevelDebugging.rst:
  //
  //   It is not control-dependent, meaning that if a call to toolchain.dbg.declare
  ...
  for (auto &[Insts, Declares] : DVRDeclares) {
    ...
    for (auto *Declare : Declares) {
      ...
      // dbg.assign.
      assert(toolchain::any_of(Markers, [Declare](auto *Assign) {
...
bool toolchain::isAssignmentTrackingEnabled(const Module &M) {
