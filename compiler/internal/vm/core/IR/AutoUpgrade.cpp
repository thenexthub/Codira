...
#include "vm/core/IR/AutoUpgrade.h"
...
static bool shouldUpgradeX86Intrinsic(Function *F, StringRef Name) {
  // All of the intrinsics matches below should be marked with which toolchain
...
static bool upgradeIntrinsicFunction1(Function *F, Function *&NewFn,
  ...
  if (!Name.consume_front("toolchain.") || Name.empty())
    return false;
  ...
  case 'd':
    if (Name.consume_front("dbg.")) {
      ...
      // Update toolchain.dbg.addr intrinsics even in "new debug mode"; they'll get
  ...
  if (ST && (!ST->isLiteral() || ST->isPacked()) &&
    ...
    if (Desc.front().Kind == Intrinsic::IITDescriptor::Struct) {
      ...
      if (auto Result = toolchain::Intrinsic::remangleIntrinsicFunction(NewFn))
        NewFn = *Result;
      return true;
    }
  }
  ...
  auto Result = toolchain::Intrinsic::remangleIntrinsicFunction(F);
...
bool toolchain::UpgradeIntrinsicFunction(Function *F, Function *&NewFn,
                                    bool CanUpgradeDebugIntrinsicsToRecords) {
  NewFn = nullptr;
  bool Upgraded =
...
GlobalVariable *toolchain::UpgradeGlobalVariable(GlobalVariable *GV) {
...
static Value *getX86MaskVec(IRBuilder<> &Builder, Value *Mask,
  ...
  toolchain::VectorType *MaskTy = FixedVectorType::get(
...
static Value *upgradeX86ALIGNIntrinsics(IRBuilder<> &Builder, Value *Op0,
                                        ...
                                        bool IsVALIGN) {
  unsigned ShiftVal = cast<toolchain::ConstantInt>(Shift)->getZExtValue();
  ...
  // lanes, emit zero.
  if (ShiftVal >= 32)
    return toolchain::Constant::getNullValue(Op0->getType());
  ...
  if (ShiftVal > 16) {
    ShiftVal -= 16;
    Op1 = Op0;
    Op0 = toolchain::Constant::getNullValue(Op0->getType());
  }
...
/// marker.
void toolchain::UpgradeInlineAsmString(std::string *AsmStr) {
  size_t Pos;
...
static Value *upgradeNVVMIntrinsicCall(StringRef Name, CallBase *CI,
  ...
  if (Name == "abs.i" || Name == "abs.ll") {
    ...
    Value *Cmp = Builder.CreateICmpSGE(
        Arg, toolchain::Constant::getNullValue(Arg->getType()), "abs.cond");
    ...
    // toolchain.nvvm.clz.ll returns an i32, but toolchain.ctlz.i64 returns an i64.
    Value *Arg = CI->getArgOperand(0);
    ...
    // toolchain.nvvm.popc.ll returns an i32, but toolchain.ctpop.i64 returns an
    // i64.
    Value *Arg = CI->getArgOperand(0);
    ...
    // nvvm.tanh.approx.f32 -> afn toolchain.tanh.f32
    FastMathFlags FMF;
    FMF.setApproxFunc();
...
static Value *upgradeX86IntrinsicCall(StringRef Name, CallBase *CI, Function *F,
  ...
  if (Name.starts_with("sse4a.movnt.")) {
    ...
    Value *Zero = toolchain::Constant::getNullValue(Ty);
    ...
    Value *Op = CI->getArgOperand(0);
    Value *Zero = toolchain::Constant::getNullValue(Op->getType());
    ...
    Value *PassThru = IsMaskZ   ? toolchain::Constant::getNullValue(CI->getType())
...
/// \p Name - Final part of the intrinsic name, e.g. 'value' in toolchain.dbg.value.
...
static Value *upgradeVectorSplice(CallBase *CI, IRBuilder<> &Builder) {
  ...
  if (!Offset)
    reportFatalUsageError("Invalid toolchain.vector.splice offset argument");
...
void toolchain::UpgradeIntrinsicCall(CallBase *CI, Function *NewFn) {
  ...
  if (!NewFn) {
    ...
    assert(Name.starts_with("toolchain.") && "Intrinsic doesn't start with 'toolchain.'");
    Name = Name.substr(5);
  ...
  case Intrinsic::aarch64_sve_ld2_sret: {
    ...
    Value *Ret = toolchain::PoisonValue::get(RetTy);
    for (unsigned I = 0; I < N; I++) {
  ...
  case Intrinsic::vector_extract: {
    StringRef Name = F->getName();
    Name = Name.substr(5); // Strip toolchain
  ...
  case Intrinsic::memset: {
    ...
    //  @toolchain.mem[cpy|move]...(i8*, i8*, i[32|i64], i32, i1)
    //    -> @toolchain.mem[cpy|move]...(i8*, i8*, i[32|i64], i1)
    //  @toolchain.memset...(i8*, i8, i[32|64], i32, i1)
    //    -> @toolchain.memset...(i8*, i8, i[32|64], i1)
...
void toolchain::UpgradeCallsToIntrinsic(Function *F) {
...
MDNode *toolchain::UpgradeTBAANode(MDNode &MD) {
...
Instruction *toolchain::UpgradeBitCastInst(unsigned Opc, Value *V, Type *DestTy,
                                      Instruction *&Temp) {
...
Constant *toolchain::UpgradeBitCastExpr(unsigned Opc, Constant *C, Type *DestTy) {
  if (Opc != Instruction::BitCast)
    return nullptr;
...
bool toolchain::UpgradeDebugInfo(Module &M) {
  if (DisableAutoUpgradeDebugInfo)
    return false;

  toolchain::TimeTraceScope timeScope("Upgrade debug info");
  ...
  if (Version == DEBUG_METADATA_VERSION) {
    ...
    if (verifyModule(M, &toolchain::errs(), &BrokenDebugInfo))
...
static void upgradeNVVMFnVectorAttr(const StringRef Attr, const char DimC,
  ...
  const std::string VStr = toolchain::utostr(VInt);
  Vect3[Dim] = VStr;
  Length = std::max(Length, Dim + 1);

  const std::string NewAttr = toolchain::join(ArrayRef(Vect3, Length), ",");
...
bool static upgradeSingleNVVMAnnotation(GlobalValue *GV, StringRef K,
  ...
  if (K == "maxclusterrank" || K == "cluster_max_blocks") {
    ...
    cast<Function>(GV)->addFnAttr("nvvm.maxclusterrank", toolchain::utostr(CV));
    return true;
  }
  if (K == "minctasm") {
    const auto CV = mdconst::extract<ConstantInt>(V)->getZExtValue();
    cast<Function>(GV)->addFnAttr("nvvm.minctasm", toolchain::utostr(CV));
    return true;
  }
  if (K == "maxnreg") {
    const auto CV = mdconst::extract<ConstantInt>(V)->getZExtValue();
    cast<Function>(GV)->addFnAttr("nvvm.maxnreg", toolchain::utostr(CV));
    return true;
  }
...
void toolchain::UpgradeNVVMAnnotations(Module &M) {
...
void toolchain::UpgradeARCRuntime(Module &M) {
  ...
  auto UpgradeToIntrinsic = [&](const char *OldFunc,
    ...
    Function *NewFn =
        toolchain::Intrinsic::getOrInsertDeclaration(&M, IntrinsicFunc);
  ...
  // "toolchain.objc.clang.arc.use".
  UpgradeToIntrinsic("clang.arc.use", toolchain::Intrinsic::objc_clang_arc_use);
  ...
  std::pair<const char *, toolchain::Intrinsic::ID> RuntimeFuncs[] = {
      {"objc_autorelease", toolchain::Intrinsic::objc_autorelease},
      {"objc_autoreleasePoolPop", toolchain::Intrinsic::objc_autoreleasePoolPop},
      {"objc_autoreleasePoolPush", toolchain::Intrinsic::objc_autoreleasePoolPush},
      {"objc_autoreleaseReturnValue",
       toolchain::Intrinsic::objc_autoreleaseReturnValue},
      {"objc_copyWeak", toolchain::Intrinsic::objc_copyWeak},
      {"objc_destroyWeak", toolchain::Intrinsic::objc_destroyWeak},
      {"objc_initWeak", toolchain::Intrinsic::objc_initWeak},
      {"objc_loadWeak", toolchain::Intrinsic::objc_loadWeak},
      {"objc_loadWeakRetained", toolchain::Intrinsic::objc_loadWeakRetained},
      {"objc_moveWeak", toolchain::Intrinsic::objc_moveWeak},
      {"objc_release", toolchain::Intrinsic::objc_release},
      {"objc_retain", toolchain::Intrinsic::objc_retain},
      {"objc_retainAutorelease", toolchain::Intrinsic::objc_retainAutorelease},
      {"objc_retainAutoreleaseReturnValue",
       toolchain::Intrinsic::objc_retainAutoreleaseReturnValue},
      {"objc_retainAutoreleasedReturnValue",
       toolchain::Intrinsic::objc_retainAutoreleasedReturnValue},
      {"objc_retainBlock", toolchain::Intrinsic::objc_retainBlock},
      {"objc_storeStrong", toolchain::Intrinsic::objc_storeStrong},
      {"objc_storeWeak", toolchain::Intrinsic::objc_storeWeak},
      {"objc_unsafeClaimAutoreleasedReturnValue",
       toolchain::Intrinsic::objc_unsafeClaimAutoreleasedReturnValue},
      {"objc_retainedObject", toolchain::Intrinsic::objc_retainedObject},
      {"objc_unretainedObject", toolchain::Intrinsic::objc_unretainedObject},
      {"objc_unretainedPointer", toolchain::Intrinsic::objc_unretainedPointer},
      {"objc_retain_autorelease", toolchain::Intrinsic::objc_retain_autorelease},
      {"objc_sync_enter", toolchain::Intrinsic::objc_sync_enter},
      {"objc_sync_exit", toolchain::Intrinsic::objc_sync_exit},
      {"objc_arc_annotation_topdown_bbstart",
       toolchain::Intrinsic::objc_arc_annotation_topdown_bbstart},
      {"objc_arc_annotation_topdown_bbend",
       toolchain::Intrinsic::objc_arc_annotation_topdown_bbend},
      {"objc_arc_annotation_bottomup_bbstart",
       toolchain::Intrinsic::objc_arc_annotation_bottomup_bbstart},
      {"objc_arc_annotation_bottomup_bbend",
       toolchain::Intrinsic::objc_arc_annotation_bottomup_bbend}};

  for (auto &I : RuntimeFuncs)
    UpgradeToIntrinsic(I.first, I.second);
}

bool toolchain::UpgradeModuleFlags(Module &M) {
  ...
  for (unsigned I = 0, E = ModFlags->getNumOperands(); I != E; ++I) {
    ...
    // section name so that toolchain-lto will not complain about mismatching
    ...
    // If the higher bits are set, it adds new module flag for codira info.
  ...
  if (HasObjCFlag && !HasClassProperties) {
    M.addModuleFlag(toolchain::Module::Override, "Objective-C Class Properties",
                    (uint32_t)0);
    Changed = true;
  }
...
void toolchain::UpgradeSectionAttributes(Module &M) {
...
void toolchain::UpgradeFunctionAttributes(Function &F) {
...
void toolchain::copyModuleAttrToFunctions(Module &M) {
  ...
  if (BTE)
    M.setModuleFlag(toolchain::Module::Min, "branch-target-enforcement", 2);
  if (BPPLR)
    M.setModuleFlag(toolchain::Module::Min, "branch-protection-pauth-lr", 2);
  if (GCS)
    M.setModuleFlag(toolchain::Module::Min, "guarded-control-stack", 2);
  if (SRA) {
    M.setModuleFlag(toolchain::Module::Min, "sign-return-address", 2);
    if (SRAALLValue == 1)
      M.setModuleFlag(toolchain::Module::Min, "sign-return-address-all", 2);
    if (SRABKeyValue == 1)
      M.setModuleFlag(toolchain::Module::Min, "sign-return-address-with-bkey", 2);
  }
}
...
static bool isOldLoopArgument(Metadata *MD) {
  ...
  if (!S)
    return false;
  return S->getString().starts_with("toolchain.vectorizer.");
}

static MDString *upgradeLoopTag(LLVMContext &C, StringRef OldTag) {
  StringRef OldPrefix = "toolchain.vectorizer.";
  assert(OldTag.starts_with(OldPrefix) && "Expected old prefix");

  if (OldTag == "toolchain.vectorizer.unroll")
    return MDString::get(C, "toolchain.loop.interleave.count");

  return MDString::get(
      C, (Twine("toolchain.loop.vectorize.") + OldTag.drop_front(OldPrefix.size()))
             .str());
}
...
static Metadata *upgradeLoopArgument(Metadata *MD) {
  ...
  if (!OldTag)
    return MD;
  if (!OldTag->getString().starts_with("toolchain.vectorizer."))
    return MD;
...
MDNode *toolchain::upgradeInstructionLoopAttachment(MDNode &N) {
...
std::string toolchain::UpgradeDataLayoutString(StringRef DL, StringRef TT) {
  Triple T(TT);
...
void toolchain::UpgradeAttributes(AttrBuilder &B) {
...
void toolchain::UpgradeOperandBundles(std::vector<OperandBundleDef> &Bundles) {
