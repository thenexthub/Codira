//===- toolchain/IR/DiagnosticInfo.cpp - Diagnostic Definitions ------*- C++ -*-===//
//
...
int toolchain::getNextAvailablePluginDiagnosticKind() {
...
DiagnosticInfoOptimizationBase::Argument::Argument(StringRef Key,
  ...
  if (isa<toolchain::Argument>(V) || isa<GlobalValue>(V))
...
DiagnosticInfoOptimizationBase::Argument::Argument(StringRef Key, float N)
    : Key(std::string(Key)), Val(toolchain::to_string(N)) {}
...
void toolchain::diagnoseDontCall(const CallInst &CI) {
  const auto *F =
