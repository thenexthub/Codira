...
#include "vm/core/IR/SymbolTableListTraits.h"
...
template class LLVM_EXPORT_TEMPLATE toolchain::SymbolTableListTraits<Function>;
template class LLVM_EXPORT_TEMPLATE toolchain::SymbolTableListTraits<GlobalVariable>;
template class LLVM_EXPORT_TEMPLATE toolchain::SymbolTableListTraits<GlobalAlias>;
template class LLVM_EXPORT_TEMPLATE toolchain::SymbolTableListTraits<GlobalIFunc>;
...
NamedMDNode *Module::getOrInsertNamedMetadata(StringRef Name) {
  ...
  if (!NMD) {
    ...
    if (Name == "toolchain.module.flags")
      ModuleFlags = NMD;
  }
  return NMD;
}
...
NamedMDNode *Module::getOrInsertModuleFlagsMetadata() {
  if (ModuleFlags)
    return ModuleFlags;
  return getOrInsertNamedMetadata("toolchain.module.flags");
}
...
Error Module::materializeMetadata() {
  toolchain::TimeTraceScope timeScope("Materialize metadata");
  if (!Materializer)
    return Error::success();
...
GlobalVariable *toolchain::collectUsedGlobalVariables(
    const Module &M, SmallVectorImpl<GlobalValue *> &Vec, bool CompilerUsed) {
  const char *Name = CompilerUsed ? "toolchain.compiler.used" : "toolchain.used";
