...
void toolchain::verifySafepointIR(Function &F) {
...
FunctionPass *toolchain::createSafepointIRVerifierPass() {
...
static bool containsGCPtrType(Type *Ty) {
  ...
  if (StructType *ST = dyn_cast<StructType>(Ty))
    return toolchain::any_of(ST->elements(), containsGCPtrType);
  return false;
}
...
void GCPtrTracker::recalculateBBsStates() {
  ...
  SetVector<const BasicBlock *> Worklist(toolchain::from_range,
                                         toolchain::make_first_range(BlockMap));
