...
#include "vm/core/IR/IntrinsicsLoongArch.h"
#include "vm/core/IR/IntrinsicsMips.h"
...
#include "vm/core/IR/IntrinsicsS390.h"
#include "vm/core/IR/IntrinsicsSPIRV.h"
#include "vm/core/IR/IntrinsicsVE.h"
...
#include "vm/core/IR/IntrinsicImpl.inc"
...
static Type *DecodeFixedType(ArrayRef<Intrinsic::IITDescriptor> &Infos,
  ...
  case IITDescriptor::MMX:
    return toolchain::FixedVectorType::get(toolchain::IntegerType::get(Context, 64), 1);
...
/// and all entries must start with "toolchain.".  If NameTable contains an exact
...
static int lookupLLVMIntrinsicByName(ArrayRef<unsigned> NameOffsetTable,
  ...
  assert(Name.starts_with("toolchain.") && "Unexpected intrinsic prefix");
  assert(Name.drop_front(5).starts_with(Target) && "Unexpected target");

  // Do successive binary searches of the dotted name components. For
  // "toolchain.gc.experimental.statepoint.p1i8.p1i32", we will find the range of
  // intrinsics starting with "toolchain.gc", then "toolchain.gc.experimental", then
  // "toolchain.gc.experimental.statepoint", and then we will stop as the range is
  // size 1. During the search, we can skip the prefix that we already know is
  // identical. By using strncmp we consider names with differing suffixes to
  // be part of the equal range.
  size_t CmpEnd = 4; // Skip the "toolchain" component.
  if (!Target.empty())
...
findTargetSubtable(StringRef Name) {
  assert(Name.starts_with("toolchain."));

  ArrayRef<IntrinsicTargetInfo> Targets(TargetInfos);
  // Drop "toolchain." and take the first dotted component. That will be the target
