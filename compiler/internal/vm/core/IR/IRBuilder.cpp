...
CallInst *IRBuilderBase::CreateThreadLocalAddress(Value *Ptr) {
  ...
  CallInst *CI = CreateIntrinsic(toolchain::Intrinsic::threadlocal_address,
                                 {Ptr->getType()}, {Ptr});
...
getStatepointArgs(IRBuilderBase &B, uint64_t ID, uint32_t NumPatchBytes,
  ...
  toolchain::append_range(Args, CallArgs);
...
CallInst *IRBuilderBase::CreateConstrainedFPIntrinsic(
  ...
  toolchain::SmallVector<Value *, 5> ExtArgs(Args);
...
CallInst *IRBuilderBase::CreateConstrainedFPCall(
    ...
    std::optional<RoundingMode> Rounding,
    std::optional<fp::ExceptionBehavior> Except) {
  toolchain::SmallVector<Value *, 6> UseArgs(Args);
