...
#include "vm/core/WindowsDriver/MSVCPaths.h"
...
#include "vm/core/Support/COM.h"
#ifdef __clang__
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wnon-virtual-dtor"
#endif
#include "vm/core/WindowsDriver/MSVCSetupApi.h"
#ifdef __clang__
...
static std::string
getHighestNumericTupleInDirectory(toolchain::vfs::FileSystem &VFS,
                                  toolchain::StringRef Directory) {
  std::string Highest;
  toolchain::VersionTuple HighestTuple;

  std::error_code EC;
  for (toolchain::vfs::directory_iterator DirIt = VFS.dir_begin(Directory, EC),
                                     DirEnd;
       !EC && DirIt != DirEnd; DirIt.increment(EC)) {
    auto Status = VFS.status(DirIt->path());
    if (!Status || !Status->isDirectory())
      continue;
    toolchain::StringRef CandidateName = toolchain::sys::path::filename(DirIt->path());
...
static bool getWindows10SDKVersionFromPath(toolchain::vfs::FileSystem &VFS,
                                           const std::string &SDKPath,
                                           std::string &SDKVersion) {
  toolchain::SmallString<128> IncludePath(SDKPath);
...
static bool getWindowsSDKDirViaCommandLine(
    toolchain::vfs::FileSystem &VFS, std::optional<toolchain::StringRef> WinSdkDir,
    std::optional<toolchain::StringRef> WinSdkVersion,
    std::optional<toolchain::StringRef> WinSysRoot, std::string &Path, int &Major,
    std::string &Version) {
  if (WinSdkDir || WinSysRoot) {
    ...
    if (WinSysRoot) {
      toolchain::SmallString<128> SDKPath(*WinSysRoot);
      toolchain::sys::path::append(SDKPath, "Windows Kits");
      if (!SDKVersion.empty())
        toolchain::sys::path::append(SDKPath, toolchain::Twine(SDKVersion.getMajor()));
      else
...
static bool readFullStringValue(HKEY hkey, const char *valueName,
  ...
  if (!toolchain::ConvertUTF8toWide(valueName, WideValueName))
    return false;
  ...
  if (result == ERROR_SUCCESS) {
    ...
    value.clear();
    return toolchain::convertWideToUTF8(WideValue, value);
  }
  return false;
}
#endif
  ...
  if (placeHolder) {
    ...
    if (lResult == ERROR_SUCCESS) {
      ...
      for (index = 0; RegEnumKeyExA(hTopKey, index, keyName, &size, NULL, NULL,
        ...
        while (*sp && !toolchain::isDigit(*sp))
          sp++;
        if (!*sp)
          continue;
        const char *ep = sp + 1;
        while (*ep && (toolchain::isDigit(*ep) || (*ep == '.')))
          ep++;
        char numBuf[32];
...
const char *toolchain::archToWindowsSDKArch(Triple::ArchType Arch) {
  switch (Arch) {
...
const char *toolchain::archToLegacyVCArch(Triple::ArchType Arch) {
  switch (Arch) {
  case Triple::ArchType::x86:
...
const char *toolchain::archToDevDivInternalArch(Triple::ArchType Arch) {
  switch (Arch) {
  case Triple::ArchType::x86:
    return "i386";
  case Triple::ArchType::x86_64:
...
bool toolchain::appendArchToWindowsSDKLibPath(int SDKMajor, SmallString<128> LibPath,
...
std::string toolchain::getSubDirectoryPath(SubDirectoryType Type,
...
bool toolchain::useUniversalCRT(ToolsetLayout VSLayout,
                           const std::string &VCToolChainPath,
...
bool toolchain::getWindowsSDKDir(vfs::FileSystem &VFS,
...
bool toolchain::getUniversalCRTSdkDir(vfs::FileSystem &VFS,
                                 std::optional<StringRef> WinSdkDir,
                                 std::optional<StringRef> WinSdkVersion,
                                 std::optional<StringRef> WinSysRoot,
...
bool toolchain::findVCToolChainViaCommandLine(
...
bool toolchain::findVCToolChainViaEnvironment(vfs::FileSystem &VFS,
                                         std::string &Path,
                                         ToolsetLayout &VSLayout) {
...
bool toolchain::findVCToolChainViaSetupConfig(
...
bool toolchain::findVCToolChainViaRegistry(std::string &Path,
                                      ToolsetLayout &VSLayout) {
