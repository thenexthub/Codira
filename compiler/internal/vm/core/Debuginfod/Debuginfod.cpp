//===-- toolchain/Debuginfod/Debuginfod.cpp - Debuginfod client library --------===//
//
...
#include "vm/core/Debuginfod/Debuginfod.h"
...
using toolchain::object::BuildIDRef;
...
toolchain::sys::RWMutex UrlsMutex;
} // namespace

std::string getDebuginfodCacheKey(toolchain::StringRef S) {
...
static std::string buildIDToString(BuildIDRef ID) {
  return toolchain::toHex(ID, /*LowerCase=*/true);
}
...
SmallVector<StringRef> getDefaultDebuginfodUrls() {
  std::shared_lock<toolchain::sys::RWMutex> ReadGuard(UrlsMutex);
  if (!DebuginfodUrls) {
    // Only read from the environment variable if the user hasn't already
    // set the value.
    ReadGuard.unlock();
    std::unique_lock<toolchain::sys::RWMutex> WriteGuard(UrlsMutex);
...
Expected<std::string> getDefaultDebuginfodCacheDirectory() {
  ...
  sys::path::append(CacheDirectory, "toolchain-debuginfod", "client");
...
static bool isHeader(StringRef S) {
  ...
  if (Name.empty() || Value.empty())
    return false;
  return all_of(Name, [](char C) { return toolchain::isPrint(C) && C != ' '; }) &&
         all_of(Value, [](char C) { return toolchain::isPrint(C) || C == '\t'; });
}
...
static SmallVector<std::string, 0> getHeaders() {
  ...
  for (StringRef Line : toolchain::split((*HeadersFile)->getBuffer(), '\n')) {
    LineNumber++;
    Line.consume_back("\r");
    if (!isHeader(Line)) {
      if (!all_of(Line, toolchain::isSpace))
        WithColor::warning()
