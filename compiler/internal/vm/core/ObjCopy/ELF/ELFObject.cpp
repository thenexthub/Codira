...
#include "vm/core/MC/MCELFExtras.h"
...
using namespace vm::core::objcopy::elf;
...
Error SectionWriter::visit(const Section &Sec) {
  if (Sec.Type != SHT_NOBITS)
    toolchain::copy(Sec.Contents, Out.getBufferStart() + Sec.Offset);
...
Error SectionWriter::visit(const OwnedDataSection &Sec) {
  toolchain::copy(Sec.Data, Out.getBufferStart() + Sec.Offset);
  return Error::success();
}
...
Error ELFSectionWriter<ELFT>::visit(const DecompressedSection &Sec) {
  ...
  toolchain::copy(Decompressed, Buf);
...
Error ELFSectionWriter<ELFT>::visit(const CompressedSection &Sec) {
  ...
  case DebugCompressionType::None:
    toolchain::copy(Sec.OriginalData, Buf);
    return Error::success();
  ...
  toolchain::copy(Sec.CompressedData, Buf);
  return Error::success();
}
...
Error ELFSectionWriter<ELFT>::visit(const SectionIndexSection &Sec) {
  uint8_t *Buf = reinterpret_cast<uint8_t *>(Out.getBufferStart()) + Sec.Offset;
  toolchain::copy(Sec.Indexes, reinterpret_cast<Elf_Word *>(Buf));
  return Error::success();
}
...
void SymbolTableSection::updateSymbols(function_ref<void(Symbol &)> Callable) {
  for (SymPtr &Sym : toolchain::drop_begin(Symbols))
    Callable(*Sym);
...
Error GroupSection::removeSectionReferences(
  ...
  toolchain::erase_if(GroupMembers, ToRemove);
  return Error::success();
}
...
Error ELFSectionWriter<ELFT>::visit(const GnuDebugLinkSection &Sec) {
  ...
  *CRC = Sec.CRC32;
  toolchain::copy(Sec.FileName, Buf);
  return Error::success();
}
  ...
  for (const IHexRecord &R : Records) {
    ...
    case IHexRecord::Data:
      ...
      if (!Section || Section->Addr + Section->Size != RecAddr) {
        ...
        // toolchain::stable_sort(), we can just set it to a constant (zero).
...
template <class ELFT> void ELFWriter<ELFT>::writeEhdr() {
  ...
  Ehdr.e_ident[EI_DATA] =
      ELFT::Endianness == toolchain::endianness::big ? ELFDATA2MSB : ELFDATA2LSB;
  ...
  // We have to use the fully-qualified name toolchain::size
  // since some compilers complain on ambiguous resolution.
  Ehdr.e_phnum = toolchain::size(Obj.segments());
...
template <class ELFT> void ELFWriter<ELFT>::writeSegmentData() {
  ...
  for (const auto &it : Obj.getUpdatedSections()) {
    ...
    toolchain::copy(Data, Buf->getBufferStart() + Offset);
  }
...
Error Object::updateSection(StringRef Name, ArrayRef<uint8_t> Data) {
  auto It = toolchain::find_if(Sections,
  ...
  auto Iter = std::stable_partition(
      std::begin(Sections), std::end(Sections), [=](const SecPtr &Sec) {
        ...
        if (Sec->Type == ELF::SHT_GROUP) {
          ...
          return !toolchain::all_of(GroupSec->members(), ToRemove);
        }
        return true;
      });
...
Error Object::replaceSections(
  ...
  assert(toolchain::is_sorted(Sections, SectionIndexLess) &&
  ...
  if (Error E = removeSections(
    ...
    return E;
  toolchain::sort(Sections, SectionIndexLess);
  return Error::success();
}
...
static void orderSegments(std::vector<Segment *> &Segments) {
  toolchain::stable_sort(Segments, compareSegmentsByOffset);
}
...
static uint64_t layoutSegments(std::vector<Segment *> &Segments,
                               uint64_t Offset) {
  assert(toolchain::is_sorted(Segments, compareSegmentsByOffset));
...
static uint64_t layoutSections(Range Sections, uint64_t Offset) {
  ...
  toolchain::stable_sort(OutOfSegmentSections,
...
static uint64_t layoutSectionsForOnlyKeepDebug(Object &Obj, uint64_t Off) {
  ...
  toolchain::stable_sort(Sections,
                    [](const SectionBase *Lhs, const SectionBase *Rhs) {
                      return Lhs->OriginalOffset < Rhs->OriginalOffset;
                    });
...
template <class ELFT> void ELFWriter<ELFT>::assignOffsets() {
  ...
  if (OnlyKeepDebug) {
    ...
    uint64_t HdrEnd =
        sizeof(Elf_Ehdr) + toolchain::size(Obj.segments()) * sizeof(Elf_Phdr);
...
Error BinaryWriter::write() {
  ...
  toolchain::stable_sort(SectionsToWrite,
                    [](const SectionBase *LHS, const SectionBase *RHS) {
...
Error ASCIIHexWriter::finalize() {
  ...
  toolchain::sort(Sections, [](const SectionBase *A, const SectionBase *B) {
