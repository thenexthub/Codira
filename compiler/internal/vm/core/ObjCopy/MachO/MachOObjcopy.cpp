...
#include "vm/core/Object/MachOUniversalWriter.h"
...
using namespace vm::core::objcopy::macho;
...
static void updateLoadCommandPayloadString(LoadCommand &LC, StringRef S) {
  ...
  toolchain::copy(S, LC.Payload.begin());
}
...
static LoadCommand buildRPathLoadCommand(StringRef Path) {
  ...
  toolchain::copy(Path, LC.Payload.begin());
  return LC;
}
...
static Error processLoadCommands(const MachOConfig &MachOConfig, Object &Obj) {
  ...
  for (LoadCommand &LC : Obj.LoadCommands) {
    ...
    // here once toolchain-objcopy supports them.
  ...
  for (LoadCommand &LC : Obj.LoadCommands)
    for (const std::unique_ptr<Section> &Sec : LC.Sections) {
      if (Sec->CanonicalName == SecName) {
        ...
        toolchain::copy(Sec->Content, Buf->getBufferStart());
...
static Expected<Section &> findSection(StringRef SecName, Object &O) {
  ...
  auto FoundSeg =
      toolchain::find_if(O.LoadCommands, [SegName](const LoadCommand &LC) {
  ...
  if (FoundSeg == O.LoadCommands.end())
    return createStringError(errc::invalid_argument,
                             ...
                             SegName.str().c_str());
  auto FoundSec = toolchain::find_if(FoundSeg->Sections,
