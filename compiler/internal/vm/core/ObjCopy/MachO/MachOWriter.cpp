...
#include "vm/core/Support/SHA256.h"
#include <memory>
...
size_t MachOWriter::totalSize() const {
  ...
  if (!Ends.empty())
    return *toolchain::max_element(Ends);
...
void MachOWriter::writeCodeSignatureData() {
  ...
  // contains a hash of all content in the binary before it. Since toolchain-objcopy
...
void MachOWriter::writeTail() {
  ...
  toolchain::sort(Queue, toolchain::less_first());
