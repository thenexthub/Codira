...
static Error readSection(WasmSection &Section, WasmObjectFile::ReadContext &Ctx,
  ...
  if (!Checker.isValidSectionOrder(Section.Type, Section.Name)) {
    return make_error<StringError>("out of order section type: " +
                                       toolchain::to_string(Section.Type),
                                   object_error::parse_failed);
  }
...
Error WasmObjectFile::parseNameSection(ReadContext &Ctx) {
  toolchain::DenseSet<uint64_t> SeenFunctions;
  toolchain::DenseSet<uint64_t> SeenGlobals;
  toolchain::DenseSet<uint64_t> SeenSegments;
...
Error WasmObjectFile::parseProducersSection(ReadContext &Ctx) {
  toolchain::SmallSet<StringRef, 3> FieldsSeen;
  ...
  for (size_t I = 0; I < Fields; ++I) {
    ...
    toolchain::SmallSet<StringRef, 8> ProducersSeen;
...
Error WasmObjectFile::parseTargetFeaturesSection(ReadContext &Ctx) {
  toolchain::SmallSet<std::string, 8> FeaturesSeen;
