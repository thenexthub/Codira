...
Error Builder::addModule(Module *M) {
  ...
  // Symbols in the toolchain.used list will get the FB_Used bit and will not be
  // internalized. We do this for toolchain.compiler.used as well:
  //
  ...
  // toolchain.compiler.used on ELF targets) to communicate to the compiler that it
  // may be used by inline asm. The usage is perfectly fine, so we treat
  // toolchain.compiler.used conservatively as toolchain.used to work around our own
  // limitation.
  SmallVector<GlobalValue *, 4> UsedV;
  collectUsedGlobalVariables(*M, UsedV, /*CompilerUsed=*/false);
  collectUsedGlobalVariables(*M, UsedV, /*CompilerUsed=*/true);
  SmallPtrSet<GlobalValue *, 4> Used(toolchain::from_range, UsedV);
  ...
  if (TT.isOSBinFormatCOFF()) {
    ...
    if (NamedMDNode *LinkerOptions =
            M->getNamedMetadata("toolchain.linker.options")) {
  ...
  if (TT.isOSBinFormatELF()) {
    if (auto E = M->materializeMetadata())
      return E;
    if (NamedMDNode *N = M->getNamedMetadata("toolchain.dependent-libraries")) {
