...
NewArchiveMember::getOldMember(const object::Archive::Child &OldMember,
                               bool Deterministic) {
  Expected<toolchain::MemoryBufferRef> BufOrErr = OldMember.getMemoryBufferRef();
  if (!BufOrErr)
...
static void print(raw_ostream &Out, object::Archive::Kind Kind, T Val) {
  support::endian::write(Out, Val,
                         isBSDLike(Kind) ? toolchain::endianness::little
                                         : toolchain::endianness::big);
}

template <class T> static void printLE(raw_ostream &Out, T Val) {
  support::endian::write(Out, Val, toolchain::endianness::little);
}
...
static bool isECObject(object::SymbolicFile &Obj) {
  if (Obj.isCOFF())
    return cast<toolchain::object::COFFObjectFile>(&Obj)->getMachine() !=
           COFF::IMAGE_FILE_MACHINE_ARM64;

  if (Obj.isCOFFImportFile())
    return cast<toolchain::object::COFFImportFile>(&Obj)->getMachine() !=
           COFF::IMAGE_FILE_MACHINE_ARM64;
...
computeMemberData(raw_ostream &StringTable, raw_ostream &SymNames,
  ...
  // in toolchain/tools/dsymutil: BinaryHolder::GetArchiveMemberBuffers().
...
void warnToStderr(Error Err) {
  toolchain::logAllUnhandledErrors(std::move(Err), toolchain::errs(), "warning: ");
}
