...
static void writeStringTable(std::vector<uint8_t> &B,
  ...
  for (const auto &S : Strings) {
    ...
    toolchain::copy(S, std::next(B.begin(), Pos));
    B[Pos + S.length()] = 0;
    Pos += S.length() + 1;
  }
...
public:
  ObjectFactory(StringRef S, MachineTypes M)
      : NativeMachine(M), ImportName(S), Library(toolchain::sys::path::stem(S)),
...
Error writeImportLibrary(StringRef ImportName, StringRef Path,
  ...
  ObjectFactory OF(toolchain::sys::path::filename(ImportName), NativeMachine);
