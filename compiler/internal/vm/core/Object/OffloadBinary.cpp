...
Error extractFromObject(const ObjectFile &Obj,
  ...
  for (SectionRef Sec : Obj.sections()) {
    ...
    if (Obj.isCOFF()) {
      ...
      if (!NameOrErr->starts_with(".toolchain.offloading"))
        continue;
    }
...
Error extractFromBitcode(MemoryBufferRef Buffer,
  ...
  // `toolchain.embedded.object` metadata with the `.toolchain.offloading` section.
  auto *MD = M->getNamedMetadata("toolchain.embedded.objects");
  if (!MD)
    return Error::success();
  ...
  for (const MDNode *Op : MD->operands()) {
    ...
    if (!SectionID || SectionID->getString() != ".toolchain.offloading")
      continue;
...
Error object::extractOffloadBinaries(MemoryBufferRef Buffer,
  ...
  case file_magic::archive: {
    Expected<std::unique_ptr<toolchain::object::Archive>> LibFile =
...
OffloadKind object::getOffloadKind(StringRef Name) {
  return toolchain::StringSwitch<OffloadKind>(Name)
...
ImageKind object::getImageKind(StringRef Name) {
  return toolchain::StringSwitch<ImageKind>(Name)
      .Case("o", IMG_Object)
      .Case("bc", IMG_Bitcode)
