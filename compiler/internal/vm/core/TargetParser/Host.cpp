...
[[maybe_unused]] static std::unique_ptr<toolchain::MemoryBuffer>
getProcCpuinfoContent() {
  ...
  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> Text =
      toolchain::MemoryBuffer::getFileAsStream(CPUInfoFile);

  if (std::error_code EC = Text.getError()) {
    toolchain::errs() << "Can't read " << CPUInfoFile << ": " << EC.message()
                 << "\n";
    return nullptr;
  }
...
StringRef sys::detail::getHostCPUNameForARM(StringRef ProcCpuinfoContent) {
  ...
  Parts.erase(toolchain::unique(Parts), Parts.end());
...
StringRef sys::detail::getHostCPUNameForS390x(StringRef ProcCpuinfoContent) {
  ...
  bool HaveVectorSupport = toolchain::is_contained(CPUFeatures, "vx");
...
using namespace vm::core::sys::detail::x86;
    ...
    for (unsigned Index = 0; Index < UINT32_MAX; ++Index) {
      ...
      if ((RegEnumKeyExA(CentralProcessorKey, Index, SubKeyName, &SubKeySize,
        ...
        if ((RegQueryValueExA(SubKey, "CP 4000", nullptr, &ActualType,
          ...
          if (!toolchain::is_contained(Values, RegValue)) {
