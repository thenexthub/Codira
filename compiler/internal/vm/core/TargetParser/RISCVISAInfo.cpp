...
static void verifyTables() {
  ...
  if (!TableChecked.load(std::memory_order_relaxed)) {
    assert(toolchain::is_sorted(SupportedExtensions) &&
           "Extensions are not sorted by name");
    assert(toolchain::is_sorted(SupportedExperimentalExtensions) &&
           "Experimental extensions are not sorted by name");
    assert(toolchain::is_sorted(SupportedProfiles) &&
           "Profiles are not sorted by name");
    assert(toolchain::is_sorted(SupportedExperimentalProfiles) &&
...
findDefaultVersion(StringRef ExtName) {
  ...
  for (auto &ExtInfo : {ArrayRef(SupportedExtensions),
    ...
    auto I = toolchain::lower_bound(ExtInfo, ExtName, LessExtName());
...
isExperimentalExtension(StringRef Ext) {
  auto I =
      toolchain::lower_bound(SupportedExperimentalExtensions, Ext, LessExtName());
...
bool RISCVISAInfo::isSupportedExtensionFeature(StringRef Ext) {
  ...
  auto I = toolchain::lower_bound(ExtInfo, Ext, LessExtName());
...
std::vector<std::string> RISCVISAInfo::toFeatures(bool AddAllExtensions,
  ...
  for (const auto &[ExtName, _] : Exts) {
    ...
    if (isExperimentalExtension(ExtName)) {
      Features.push_back((toolchain::Twine("+experimental-") + ExtName).str());
    } else {
      Features.push_back((toolchain::Twine("+") + ExtName).str());
    }
  }
  if (AddAllExtensions) {
    for (const RISCVSupportedExtension &Ext : SupportedExtensions) {
      if (Exts.count(Ext.Name))
        continue;
      Features.push_back((toolchain::Twine("-") + Ext.Name).str());
    }

    for (const RISCVSupportedExtension &Ext : SupportedExperimentalExtensions) {
      if (Exts.count(Ext.Name))
        continue;
      Features.push_back((toolchain::Twine("-experimental-") + Ext.Name).str());
    }
  }
  return Features;
}
...
toolchain::Expected<std::unique_ptr<RISCVISAInfo>>
...
RISCVISAInfo::parseFeatures(unsigned XLen,
  ...
  for (StringRef ExtName : Features) {
    ...
    auto ExtensionInfoIterator =
        toolchain::lower_bound(ExtensionInfos, ExtName, LessExtName());
...
RISCVISAInfo::parseArchString(StringRef Arch, bool EnableExperimentalExtension,
  ...
  if (Arch.consume_front("rv32")) {
    ...
    auto I = toolchain::upper_bound(SupportedProfiles, Arch, ProfileCmp);
    bool FoundProfile = I != std::begin(SupportedProfiles) &&
                        Arch.starts_with(std::prev(I)->Name);
    if (!FoundProfile) {
      I = toolchain::upper_bound(SupportedExperimentalProfiles, Arch, ProfileCmp);
...
void RISCVISAInfo::updateImplication() {
  assert(toolchain::is_sorted(ImpliedExts) && "Table not sorted by Name");
  ...
  while (!WorkList.empty()) {
    ...
    for (const ImpliedExtsEntry &Implied : toolchain::make_range(Range)) {
