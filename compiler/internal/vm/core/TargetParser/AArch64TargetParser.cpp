...
#include "vm/core/TargetParser/ARMTargetParserCommon.h"
...
#include "vm/core/TargetParser/AArch64TargetParserDef.inc"
...
const AArch64::ArchInfo *AArch64::parseArch(StringRef Arch) {
  Arch = toolchain::ARM::getCanonicalArchName(Arch);
  if (checkArchVersion(Arch) < 8)
    return {};

  StringRef Syn = toolchain::ARM::getArchSynonym(Arch);
...
const toolchain::AArch64::ExtensionInfo &
lookupExtensionByID(toolchain::AArch64::ArchExtKind ExtID) {
  for (const auto &E : toolchain::AArch64::Extensions)
    if (E.ID == ExtID)
      return E;
...
void AArch64::ExtensionSet::enable(ArchExtKind E) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Enable " << lookupExtensionByID(E).UserVisibleName << "\n");
...
void AArch64::ExtensionSet::disable(ArchExtKind E) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Disable " << lookupExtensionByID(E).UserVisibleName << "\n");
...
void AArch64::ExtensionSet::addCPUDefaults(const CpuInfo &CPU) {
  LLVM_DEBUG(toolchain::dbgs() << "addCPUDefaults(" << CPU.Name << ")\n");
  BaseArch = &CPU.Arch;
...
void AArch64::ExtensionSet::addArchDefaults(const ArchInfo &Arch) {
  LLVM_DEBUG(toolchain::dbgs() << "addArchDefaults(" << Arch.Name << ")\n");
  BaseArch = &Arch;
...
bool AArch64::ExtensionSet::parseModifier(StringRef Modifier,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "parseModifier(" << Modifier << ")\n");
