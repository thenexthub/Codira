...
#include "vm/core/TargetParser/X86TargetParser.h"
#include "vm/core/ADT/Bitset.h"
#include "vm/core/ADT/StringSwitch.h"
#include <numeric>

using namespace vm::core;
using namespace vm::core::X86;
...
#include "vm/core/TargetParser/X86TargetParser.def"
...
X86::CPUKind toolchain::X86::parseArchX86(StringRef CPU, bool Only64Bit) {
...
X86::CPUKind toolchain::X86::parseTuneCPU(StringRef CPU, bool Only64Bit) {
  if (toolchain::is_contained(NoTuneList, CPU))
    return CK_None;
  return parseArchX86(CPU, Only64Bit);
}

void toolchain::X86::fillValidCPUArchList(SmallVectorImpl<StringRef> &Values,
                                     bool Only64Bit) {
  for (const auto &P : Processors)
...
void toolchain::X86::fillValidTuneCPUList(SmallVectorImpl<StringRef> &Values,
                                     bool Only64Bit) {
  for (const ProcInfo &P : Processors)
    if (!P.OnlyForCPUDispatchSpecific && !P.Name.empty() &&
        (P.Features[FEATURE_64BIT] || !Only64Bit) &&
        !toolchain::is_contained(NoTuneList, P.Name))
      Values.emplace_back(P.Name);
}

ProcessorFeatures toolchain::X86::getKeyFeature(X86::CPUKind Kind) {
...
void toolchain::X86::getFeaturesForCPU(StringRef CPU,
                                  SmallVectorImpl<StringRef> &EnabledFeatures,
                                  bool NeedPlus) {
  auto I = toolchain::find_if(Processors,
...
void toolchain::X86::updateImpliedFeatures(
    StringRef Feature, bool Enabled,
    StringMap<bool> &Features) {
  auto I = toolchain::find_if(FeatureInfos, [&](const FeatureInfo &FI) {
...
char toolchain::X86::getCPUDispatchMangling(StringRef CPU) {
  auto I = toolchain::find_if(Processors,
                         [&](const ProcInfo &P) { return P.Name == CPU; });
  assert(I != std::end(Processors) && "Processor not found!");
...
bool toolchain::X86::validateCPUSpecificCPUDispatch(StringRef Name) {
  auto I = toolchain::find_if(Processors,
...
toolchain::X86::getCpuSupportsMask(ArrayRef<StringRef> FeatureStrs) {
...
unsigned toolchain::X86::getFeaturePriority(ProcessorFeatures Feat) {
#ifndef NDEBUG
