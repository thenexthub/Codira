...
#include "vm/core/TargetParser/ARMTargetParser.def"
...
StringRef ARM::getARMCPUForArch(const toolchain::Triple &Triple, StringRef MArch) {
  if (MArch.empty())
    MArch = Triple.getArchName();
  MArch = toolchain::ARM::getCanonicalArchName(MArch);
  ...
  case toolchain::Triple::Win32:
    // FIXME: this is invalid for WindowsCE
    if (toolchain::ARM::parseArchVersion(MArch) <= 7)
      return "cortex-a9";
    break;
  ...
  StringRef CPU = toolchain::ARM::getDefaultCPU(MArch);
