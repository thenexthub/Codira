...
find(StringRef S, ArrayRef<BasicSubtargetSubTypeKV> A) {
  ...
  auto F = toolchain::lower_bound(A, S);
...
std::optional<toolchain::StringMap<bool>> toolchain::getCPUDefaultTargetFeatures(
  ...
  FeatureBitset Bits;
  toolchain::StringMap<bool> DefaultFeatures;
...
const GPUInfo *getArchEntry(AMDGPU::GPUKind AK, ArrayRef<GPUInfo> Table) {
  ...
  auto I =
      toolchain::lower_bound(Table, Search, [](const GPUInfo &A, const GPUInfo &B) {
...
StringRef toolchain::AMDGPU::getArchFamilyNameAMDGCN(GPUKind AK) {
  switch (AK) {
...
AMDGPU::fillAMDGPUFeatureMap(StringRef GPU, const Triple &T,
  ...
  if (T.isSPIRV() && T.getOS() == Triple::OSType::AMDHSA) {
    ...
    switch (toolchain::AMDGPU::parseArchR600(GPU)) {
    case GK_CAYMAN:
    case GK_CYPRESS:
    case GK_RV770:
    case GK_RV670:
