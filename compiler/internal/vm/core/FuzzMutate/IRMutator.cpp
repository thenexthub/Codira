...
#include "vm/core/FuzzMutate/IRMutator.h"
...
#include "vm/core/FuzzMutate/Random.h"
#include "vm/core/FuzzMutate/RandomIRBuilder.h"
#include "vm/core/IR/BasicBlock.h"
#include "vm/core/IR/FMF.h"
...
size_t toolchain::IRMutator::getModuleSize(const Module &M) {
...
static inline Instruction *getEffectiveTerminator(BasicBlock &BB) {
  if (Instruction *I = BB.getTerminatingMustTailCall()) {
    ...
    // Certain intrinsics, such as @toolchain.amdgcn.cs.chain, must be immediately
...
void InjectorIRStrategy::mutate(BasicBlock &BB, RandomIRBuilder &IB) {
  SmallVector<Instruction *, 32> Insts(
      toolchain::make_pointer_range(getInsertionRange(BB)));
  if (Insts.size() < 1)
    return;
...
static bool isUnsupportedFunction(Function *F) {
  ...
  // For example, `@toolchain.dbg.declare(metadata, metadata, metadata)`
  // https://toolchain.org/docs/SourceLevelDebugging.html#toolchain-dbg-declare
  ...
  auto IsABIAttribute = [](AttributeSet A) {
    ...
    return toolchain::any_of(ABIAttrs, [&](Attribute::AttrKind kind) {
...
void InsertPHIStrategy::mutate(BasicBlock &BB, RandomIRBuilder &IB) {
  ...
  PHINode *PHI = PHINode::Create(Ty, toolchain::pred_size(&BB), "", BB.begin());
...
std::unique_ptr<Module> toolchain::parseModule(const uint8_t *Data, size_t Size,
                                          LLVMContext &Context) {
...
size_t toolchain::writeModule(const Module &M, uint8_t *Dest, size_t MaxSize) {
  std::string Buf;
  {
...
std::unique_ptr<Module> toolchain::parseAndVerify(const uint8_t *Data, size_t Size,
                                             LLVMContext &Context) {
