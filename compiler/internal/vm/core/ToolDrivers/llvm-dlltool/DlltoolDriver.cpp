...
#include "vm/core/ToolDrivers/toolchain-dlltool/DlltoolDriver.h"
...
std::unique_ptr<MemoryBuffer> openFile(const Twine &Path) {
  ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> MB = MemoryBuffer::getFile(Path);
...
std::optional<std::string> getPrefix(StringRef Argv0) {
  StringRef ProgName = toolchain::sys::path::stem(Argv0);
  // x86_64-w64-mingw32-dlltool -> x86_64-w64-mingw32
  // toolchain-dlltool -> None
  // aarch64-w64-mingw32-toolchain-dlltool-10.exe -> aarch64-w64-mingw32
  ProgName = ProgName.rtrim("0123456789.-");
  if (!ProgName.consume_back_insensitive("dlltool"))
    return std::nullopt;
  ProgName.consume_back_insensitive("toolchain-");
...
int printError(toolchain::Error E, Twine File) {
  if (!E)
    return 0;
...
int doIdentify(StringRef File, bool IdentifyStrict) {
  ...
  if (identify_magic(MaybeBuf.get()->getBuffer()) != file_magic::archive) {
    toolchain::errs() << File << " is not a library\n";
    return 1;
  }
  ...
  if (Names.empty()) {
    toolchain::errs() << "No DLL import name found in " << File << "\n";
    return 1;
  }
  if (Names.size() > 1 && IdentifyStrict) {
    toolchain::errs() << File << "contains imports for two or more DLLs\n";
    return 1;
  }
...
int toolchain::dlltoolDriverMain(toolchain::ArrayRef<const char *> ArgsArr) {
  DllOptTable Table;
  unsigned MissingIndex;
  unsigned MissingCount;
  toolchain::opt::InputArgList Args =
      Table.ParseArgs(ArgsArr.slice(1), MissingIndex, MissingCount);
  if (MissingCount) {
    toolchain::errs() << Args.getArgString(MissingIndex) << ": missing argument\n";
    return 1;
  }
  ...
  if (Args.hasArgNoClaim(OPT_INPUT) ||
    ...
    Table.printHelp(outs(), "toolchain-dlltool [options] file...", "toolchain-dlltool",
                    false);
    toolchain::outs()
  ...
  if (!Args.hasArg(OPT_d)) {
    toolchain::errs() << "no definition file specified\n";
    return 1;
  }
  ...
  if (Args.hasArg(OPT_N)) {
    if (!isArm64EC(Machine)) {
      toolchain::errs() << "native .def file is supported only on arm64ec target\n";
      return 1;
    }
  ...
  if (OutputFile.empty()) {
    toolchain::errs() << "no DLL name specified\n";
    return 1;
  }
