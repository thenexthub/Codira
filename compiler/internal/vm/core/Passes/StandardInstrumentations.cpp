...
template <typename IRUnitT> static const IRUnitT *unwrapIR(Any IR) {
  const IRUnitT **IRPtr = toolchain::any_cast<const IRUnitT *>(&IR);
...
static int prepareDumpIRFileDescriptor(const StringRef DumpIRFilename) {
  std::error_code EC;
  auto ParentPath = toolchain::sys::path::parent_path(DumpIRFilename);
  if (!ParentPath.empty()) {
    std::error_code EC = toolchain::sys::fs::create_directories(ParentPath);
    if (EC)
...
void PrintIRInstrumentation::printBeforePass(StringRef PassID, Any IR) {
  ...
  if (!IRDumpDirectory.empty()) {
    ...
    toolchain::raw_fd_ostream DumpIRFileStream{
  ...
  PIC.registerBeforeNonSkippedPassCallback(
      [&PIC, this](StringRef PassID, Any IR) {
        ...
        OS << formatv("; *** Dump of {0}IR Before Last Pass {1}",
                      toolchain::forcePrintModuleIR() ? "Module " : "", PassID);
