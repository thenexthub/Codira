...
#include "vm/core/DWARFLinker/Classic/DWARFLinker.h"
...
#include "vm/core/DWARFLinker/Classic/DWARFStreamer.h"
#include "vm/core/DWARFLinker/Utils.h"
#include "vm/core/DebugInfo/DWARF/DWARFAbbreviationDeclaration.h"
#include "vm/core/DebugInfo/DWARF/DWARFAcceleratorTable.h"
...
#include "vm/core/DebugInfo/DWARF/DWARFDebugMacro.h"
...
#include "vm/core/DebugInfo/DWARF/DWARFSection.h"
...
DWARFLinker::DIECloner::getCanonicalDIEName(DWARFDie Die, const DWARFFile &File,
  ...
  auto GetDieName = [](const DWARFDie &D) -> toolchain::StringRef {
    auto NameForm = D.find(toolchain::dwarf::DW_AT_name);
    if (!NameForm)
      return {};
  ...
  toolchain::StringRef Name = GetDieName(Die);
  if (!Name.empty())
    return Name;

  while (true) {
    if (!(Ref = Die.find(toolchain::dwarf::DW_AT_specification)) &&
        !(Ref = Die.find(toolchain::dwarf::DW_AT_abstract_origin)))
      break;
...
bool DWARFLinker::DIECloner::getDIENames(
  ...
  if (!Info.Name)
    if (toolchain::StringRef Name = getCanonicalDIEName(Die, File, &Unit);
        !Name.empty())
...
static void constructSeqOffsettoOrigRowMapping(
  ...
  toolchain::sort(StmtAttrs, [](const PatchLocation &A, const PatchLocation &B) {
    return A.get() < B.get();
  });

  std::vector<unsigned> SeqStartRows;
  SeqStartRows.push_back(0);
  for (auto [I, Row] : toolchain::enumerate(ArrayRef(LT.Rows).drop_back()))
    if (Row.EndSequence)
...
unsigned DWARFLinker::DIECloner::cloneScalarAttribute(
  ...
  if (AttrSpec.Attr == dwarf::DW_AT_macro_info) {
    if (std::optional<uint64_t> Offset = Val.getAsSectionOffset()) {
      const toolchain::DWARFDebugMacro *Macro = File.Dwarf->getDebugMacinfo();
  ...
  if (AttrSpec.Attr == dwarf::DW_AT_macros) {
    if (std::optional<uint64_t> Offset = Val.getAsSectionOffset()) {
      const toolchain::DWARFDebugMacro *Macro = File.Dwarf->getDebugMacro();
      if (Macro == nullptr || !Macro->hasEntryForOffset(*Offset))
        return 0;
    }
  }
  ...
  if (!AllRngListAttributes.empty() || UnitRngListAttribute) {
    ...
    for (PatchLocation &AttributePatch : AllRngListAttributes) {
      ...
      if (Expected<DWARFAddressRangesVector> OriginalRanges =
        ...
        toolchain::consumeError(OriginalRanges.takeError());
...
void DWARFLinker::DIECloner::generateUnitLocations(
  ...
  for (auto &CurLocAttr : AllLocListAttributes) {
    ...
    if (!OriginalLocations) {
      toolchain::consumeError(OriginalLocations.takeError());
...
static void insertLineSequence(std::vector<TrackedRow> &Seq,
  ...
  if (!Rows.empty() && Rows.back().Row.Address < Seq.front().Row.Address) {
    toolchain::append_range(Rows, Seq);
    Seq.clear();
    return;
  }
...
remapPath(StringRef Path,
  ...
  for (const auto &Entry : ObjectPrefixMap)
    if (toolchain::sys::path::replace_path_prefix(p, Entry.first, Entry.second))
      break;
  return p.str().str();
}
...
std::pair<bool, bool> DWARFLinker::isClangModuleRef(const DWARFDie &CUDie,
  ...
  if (Cached != ClangModules.end()) {
    // FIXME: Until PR27449 (https://toolchain.org/bugs/show_bug.cgi?id=27449) is
...
Error DWARFLinker::link() {
  ...
  if (Options.Statistics) {
    ...
    toolchain::sort(Sorted, [](auto &LHS, auto &RHS) {
    ...
    outs() << "----------------------------------------------------------------"
              "---------------\n";
    toolchain::outs() << formatv(FormatStr, "Total", InputTotal, OutputTotal,
