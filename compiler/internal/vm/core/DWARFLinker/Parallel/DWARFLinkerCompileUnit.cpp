...
CompileUnit::CompileUnit(LinkingGlobalData &GlobalData, unsigned ID,
                         ...
                         dwarf::FormParams Format, toolchain::endianness Endianess)
...
void CompileUnit::maybeResetToLoadedStage() {
  ...
  toolchain::fill(OutDieOffsetArray, 0);
  toolchain::fill(TypeEntries, nullptr);
  eraseSections();
    ...
    DebugInfoSection.ListDebugRangePatch.forEach([&](DebugRangePatch &Patch) {
      if (Patch.IsCompileUnitRanges) {
        ...
        if (Expected<DWARFAddressRangesVector> InputRanges =
          ...
          toolchain::consumeError(InputRanges.takeError());
  ...
  for (auto &Op : InputExpression) {
    ...
    if ((Desc.Op.size() == 1 && Desc.Op[0] == Encoding::BaseTypeRef) ||
      ...
      if (std::optional<object::SectionedAddress> SA =
        ...
        if (getEndianness() != toolchain::endianness::native)
...
Error CompileUnit::cloneAndEmit(
  ...
  if (toolchain::is_contained(GlobalData.getOptions().AccelTables,
...
LLVM_DUMP_METHOD void CompileUnit::DIEInfo::dump() {
  toolchain::errs() << "{";
  toolchain::errs() << "  Placement: ";
  switch (getPlacement()) {
  case NotSet:
    toolchain::errs() << "NotSet";
    break;
  case TypeTable:
    toolchain::errs() << "TypeTable";
    break;
  case PlainDwarf:
    toolchain::errs() << "PlainDwarf";
    break;
  case Both:
    toolchain::errs() << "Both";
    break;
  ...
  toolchain::errs() << "  Keep: " << getKeep();
  toolchain::errs() << "  KeepPlainChildren: " << getKeepPlainChildren();
  toolchain::errs() << "  KeepTypeChildren: " << getKeepTypeChildren();
  toolchain::errs() << "  IsInMouduleScope: " << getIsInMouduleScope();
  toolchain::errs() << "  IsInFunctionScope: " << getIsInFunctionScope();
  toolchain::errs() << "  IsInAnonNamespaceScope: " << getIsInAnonNamespaceScope();
  toolchain::errs() << "  ODRAvailable: " << getODRAvailable();
  toolchain::errs() << "  TrackLiveness: " << getTrackLiveness();
  toolchain::errs() << "}\n";
}
...
CompileUnit::getDirAndFilenameFromLineTable(uint64_t FileIdx) {
  ...
  if (const DWARFDebugLine::LineTable *LineTable =
    ...
    if (LineTable->hasFileAtIndex(FileIdx)) {
      ...
      const toolchain::DWARFDebugLine::FileNameEntry &Entry =
