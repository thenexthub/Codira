...
DWARFLinkerImpl::LinkContext::LinkContext(LinkingGlobalData &GlobalData,
  ...
  if (File.Dwarf) {
    ...
    Endianness = File.Dwarf->isLittleEndian() ? toolchain::endianness::little
...
Error DWARFLinkerImpl::link() {
  ...
  toolchain::endianness GlobalEndianness = toolchain::endianness::native;
  ...
  if (!GlobalData.Options.NoODR && Language.has_value()) {
    toolchain::parallel::TaskGroup TGroup;
    TGroup.spawn([&]() {
  ...
  if (GlobalData.getOptions().Threads == 0)
    toolchain::parallel::strategy = optimal_concurrency(OverallNumberOfCU);
  else
    toolchain::parallel::strategy =
  ...
  if (GlobalData.getOptions().Threads == 1) {
    ...
    DefaultThreadPool Pool(toolchain::parallel::strategy);
    for (std::unique_ptr<LinkContext> &Context : ObjectContexts)
      Pool.async([&]() {
        // Link object file.
        if (Error Err = Context->link(ArtificialTypeUnit.get()))
...
void DWARFLinkerImpl::emitCommonSectionsAndWriteCompileUnitsToTheOutput() {
  toolchain::parallel::TaskGroup TG;
  ...
  if (toolchain::is_contained(GlobalData.Options.AccelTables,
