...
#include "vm/core/ADT/TrieRawHashMap.h"
...
public:
  CacheEntry() = default;
  CacheEntry(ArrayRef<uint8_t> Hash) { toolchain::copy(Hash, Value.data()); }
  CacheEntry(const CacheEntry &Entry) { toolchain::copy(Entry.Value, Value.data()); }
...
Error UnifiedOnDiskActionCache::validate() const {
  auto ValidateRef = [this](FileOffset Offset, ArrayRef<char> Value) -> Error {
    ...
    if (Error E = UniDB->getGraphDB().validateObjectID(ID))
      return formatError(toolchain::toString(std::move(E)));
    return Error::success();
  };
