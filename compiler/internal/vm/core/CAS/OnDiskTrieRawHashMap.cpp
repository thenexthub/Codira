...
#include "vm/core/ADT/TrieHashIndexGenerator.h"
...
TrieRawHashMapHandle::create(MappedFileRegionArena &Alloc, StringRef Name,
  ...
  toolchain::copy(Name, NameStorage);
...
TrieRawHashMapHandle::createRecord(MappedFileRegionArena &Alloc,
  ...
  toolchain::copy(Hash, const_cast<uint8_t *>(Record.Proxy.Hash.begin()));
  return Record;
}
...
static void appendIndexBits(std::string &Prefix, size_t Index,
  ...
  for (char Ch : toolchain::reverse(Bits))
    Prefix += Ch;
}
