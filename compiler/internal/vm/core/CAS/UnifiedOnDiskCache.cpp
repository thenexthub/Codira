...
getAllDBDirs(StringRef Path, bool IncludeCorrupt = false) {
  ...
  toolchain::sort(FoundDBDirs, [](const DBDir &LHS, const DBDir &RHS) -> bool {
...
/// 'v<version>.<x+1>' name.
static void getNextDBDirName(StringRef DBDir, toolchain::raw_ostream &OS) {
...
static Error validateOutOfProcess(StringRef LLVMCasBinary, StringRef RootPath,
  ...
  toolchain::SmallString<128> StdErrPath;
  int StdErrFD = -1;
  if (std::error_code EC = sys::fs::createTemporaryFile(
          "toolchain-cas-validate-stderr", "txt", StdErrFD, StdErrPath,
  ...
  if (Result == -1)
    return createStringError("failed to exec " + join(Args, " ") + ": " +
                             ErrMsg);
  if (Result != 0) {
    toolchain::SmallString<64> Err("cas contents invalid");
    if (!ErrMsg.empty()) {
      Err += ": ";
      Err += ErrMsg;
    }
...
Expected<ValidationResult> UnifiedOnDiskCache::validateIfNeeded(
  ...
  if (std::error_code EC = lockFileThreadSafe(FD, sys::fs::LockKind::Exclusive))
    return createFileError(PathBuf, EC);
  toolchain::scope_exit UnlockFD([&]() { unlockFileThreadSafe(FD); });
  ...
  if (NeedsRecovery) {
    ...
    toolchain::scope_exit CloseLock([&]() { sys::fs::closeFile(LockFile); });
    ...
    toolchain::scope_exit UnlockFD([&]() { unlockFileThreadSafe(LockFD); });
...
Error UnifiedOnDiskCache::close(bool CheckSizeLimit) {
  ...
  toolchain::scope_exit CloseLock([&]() {
    assert(LockFD >= 0);
    sys::fs::file_t LockFile = sys::fs::convertFDToNativeFile(LockFD);
  ...
  toolchain::scope_exit UnlockFile([&]() { unlockFileThreadSafe(LockFD); });
