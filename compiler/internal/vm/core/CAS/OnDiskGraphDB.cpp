...
#include "vm/core/CAS/OnDiskGraphDB.h"
#include "OnDiskCommon.h"
#include "vm/core/ADT/DenseMap.h"
#include "vm/core/ADT/ScopeExit.h"
#include "vm/core/ADT/StringExtras.h"
#include "vm/core/CAS/OnDiskDataAllocator.h"
#include "vm/core/CAS/OnDiskTrieRawHashMap.h"
...
static constexpr StringLiteral IndexTableName = "toolchain.cas.index";
static constexpr StringLiteral DataPoolTableName = "toolchain.cas.data";
...
Error OnDiskGraphDB::validate(bool Deep, HashingFuncT Hasher) const {
  ...
  return Index.validate([&](FileOffset Offset,
    ...
    case TrieRecord::StorageKind::StandaloneLeaf0:
      ...
      } else if (!toolchain::sys::fs::exists(Path))
        return formatError("record file \'" + Path + "\' does not exist");
    }
    ...
    auto dataError = [&](Twine Msg) {
      return createStringError(toolchain::errc::illegal_byte_sequence,
...
Error OnDiskGraphDB::validateObjectID(ObjectID ExternalRef) {
  ...
  if (!I)
    return formatError(toolchain::toString(I.takeError()));
...
OnDiskGraphDB::load(ObjectID ExternalRef) {
  ...
  toolchain::scope_exit CloseFile([&]() { sys::fs::closeFile(*File); });
...
OnDiskGraphDB::open(StringRef AbsPath, StringRef HashName,
  ...
  if (DataPool->getUserHeader().size() != UserHeaderSize)
    return createStringError(toolchain::errc::argument_out_of_domain,
...
Error OnDiskGraphDB::importFullTree(ObjectID PrimaryID,
  ...
  auto enqueueNode = [&](ObjectID PrimaryID, std::optional<ObjectHandle> Node) {
    ...
    CursorStack.push_back(
        {*Node, (size_t)toolchain::size(Refs), Refs.begin(), Refs.end()});
  };
...
Error OnDiskGraphDB::importSingleNode(ObjectID PrimaryID,
  ...
  Refs.reserve(toolchain::size(UpstreamRefs));
