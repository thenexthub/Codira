...
#include "vm/core/MCA/InstrBuilder.h"
...
#define DEBUG_TYPE "toolchain-mca-instrbuilder"
...
InstrBuilder::InstrBuilder(const toolchain::MCSubtargetInfo &sti,
                           const toolchain::MCInstrInfo &mcii,
                           const toolchain::MCRegisterInfo &mri,
                           const toolchain::MCInstrAnalysis *mcia,
...
static void initializeUsedResources(InstrDesc &ID,
  ...
  sort(Worklist, [](const ResourcePlusCycles &A, const ResourcePlusCycles &B) {
    unsigned popcntA = toolchain::popcount(A.first);
    unsigned popcntB = toolchain::popcount(B.first);
    if (popcntA < popcntB)
      return true;
    if (popcntA > popcntB)
      return false;
  ...
  for (unsigned I = 0, E = Worklist.size(); I < E; ++I) {
    ...
    if (!A.second.size()) {
      assert(toolchain::popcount(A.first) > 1 && "Expected a group!");
      UsedResourceGroups |= toolchain::bit_floor(A.first);
      continue;
    }
    ...
    if (toolchain::popcount(A.first) == 1) {
      UsedResourceUnits |= A.first;
    } else {
      // Remove the leading 1 from the resource group mask.
      NormalizedMask ^= toolchain::bit_floor(NormalizedMask);
    ...
    for (unsigned J = I + 1; J < E; ++J) {
      ...
      if ((NormalizedMask & B.first) == NormalizedMask) {
        ...
        if (toolchain::popcount(B.first) > 1)
          B.second.NumUnits++;
      }
    }
  }
  ...
  for (ResourcePlusCycles &RPC : ID.Resources) {
    if (toolchain::popcount(RPC.first) > 1 && !RPC.second.isReserved()) {
      // Remove the leading 1 from the resource group mask.
      uint64_t Mask = RPC.first ^ toolchain::bit_floor(RPC.first);
      uint64_t MaxResourceUnits = toolchain::popcount(Mask);
      if (RPC.second.NumUnits > (unsigned)toolchain::popcount(Mask)) {
...
InstrBuilder::createInstruction(const MCInst &MCI,
  ...
  if (D.Writes.empty()) {
    if (IsInstRecycled)
      return toolchain::make_error<RecycledInstErr>(NewIS);
    else
