...
bool PyGlobals::loadDialectModule(toolchain::StringRef dialectNamespace) {
  {
...
void PyGlobals::registerAttributeBuilder(const std::string &attributeKind,
  ...
  if (found && !replace) {
    throw std::runtime_error((toolchain::Twine("Attribute builder for '") +
                              attributeKind +
...
void PyGlobals::registerDialectImpl(const std::string &dialectNamespace,
  ...
  if (found) {
    throw std::runtime_error((toolchain::Twine("Dialect namespace '") +
...
void PyGlobals::registerOperationImpl(const std::string &operationName,
  ...
  if (found && !replace) {
    throw std::runtime_error((toolchain::Twine("Operation '") + operationName +
...
std::optional<nb::object>
PyGlobals::lookupOperationClass(toolchain::StringRef operationName) {
  // Make sure dialect module is loaded.
  auto split = operationName.split('.');
  toolchain::StringRef dialectNamespace = split.first;
  if (!loadDialectModule(dialectNamespace))
    return std::nullopt;
...
void PyGlobals::TracebackLoc::registerTracebackFileInclusion(
  ...
  nanobind::ft_lock_guard lock(mutex);
  auto reg = "^" + toolchain::Regex::escape(file);
...
bool PyGlobals::TracebackLoc::isUserTracebackFilename(
    const toolchain::StringRef file) {
  nanobind::ft_lock_guard lock(mutex);
  if (rebuildUserTracebackIncludeRegex) {
    userTracebackIncludeRegex.assign(
        toolchain::join(userTracebackIncludeFiles, "|"));
  ...
  if (rebuildUserTracebackExcludeRegex) {
    userTracebackExcludeRegex.assign(
        toolchain::join(userTracebackExcludeFiles, "|"));
