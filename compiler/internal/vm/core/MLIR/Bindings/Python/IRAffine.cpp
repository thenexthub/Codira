...
static void pyListToVector(const nb::list &list,
                           toolchain::SmallVectorImpl<CType> &result,
                           StringRef action) {
  ...
  for (nb::handle item : list) {
    try {
      ...
      std::string msg = (toolchain::Twine("Invalid expression when ") + action +
                         " (" + err.what() + ")")
                            .str();
      throw std::runtime_error(msg.c_str());
    } catch (std::runtime_error &err) {
      std::string msg = (toolchain::Twine("Invalid expression (None?) when ") +
                         action + " (" + err.what() + ")")
                            .str();
      throw std::runtime_error(msg.c_str());
    }
...
static bool isPermutation(const std::vector<PermutationTy> &permutation) {
  toolchain::SmallVector<bool, 8> seen(permutation.size(), false);
