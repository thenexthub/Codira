...
See also: https://mlir.toolchain.org/docs/LangRef/
)";
...
nb::object PyOperation::create(std::string_view name,
                               ...
                               toolchain::ArrayRef<MlirValue> operands,
                               std::optional<nb::dict> attributes,
                               std::optional<std::vector<PyBlock *>> successors,
                               int regions, PyLocation &location,
                               const nb::object &maybeIp, bool inferType) {
  toolchain::SmallVector<MlirType, 4> mlirResults;
  toolchain::SmallVector<MlirBlock, 4> mlirSuccessors;
  toolchain::SmallVector<std::pair<std::string, MlirAttribute>, 4> mlirAttributes;
  ...
  if (!mlirAttributes.empty()) {
    ...
    // on.
    toolchain::SmallVector<MlirNamedAttribute, 4> mlirNamedAttributes;
  ...
  if (!mlirSuccessors.empty())
    mlirOperationStateAddSuccessors(&state, mlirSuccessors.size(),
                                    mlirSuccessors.data());
  if (regions) {
    toolchain::SmallVector<MlirRegion, 4> mlirRegions;
...
static void populateResultTypes(StringRef name, nb::list resultTypeList,
  ...
  if (resultSegmentSpecObj.is_none()) {
    ...
    for (const auto &it : toolchain::enumerate(resultTypeList)) {
      try {
        resultTypes.push_back(nb::cast<PyType *>(it.value()));
        if (!resultTypes.back())
          throw nb::cast_error();
      } catch (nb::cast_error &err) {
        throw nb::value_error((toolchain::Twine("Result ") +
                               toolchain::Twine(it.index()) + " of operation \"" +
    ...
    if (resultSegmentSpec.size() != resultTypeList.size()) {
      throw nb::value_error((toolchain::Twine("Operation \"") + name +
                             "\" requires " +
                             toolchain::Twine(resultSegmentSpec.size()) +
                             " result segments but was provided " +
                             toolchain::Twine(resultTypeList.size()))
                                .str()
                                .c_str());
    }
    resultSegmentLengths.reserve(resultTypeList.size());
    for (const auto &it :
         toolchain::enumerate(toolchain::zip(resultTypeList, resultSegmentSpec))) {
        ...
        try {
          ...
          if (resultType) {
            ...
            throw nb::value_error(
                (toolchain::Twine("Result ") + toolchain::Twine(it.index()) +
                 " of operation \"" + name +
...
nb::object PyOpView::buildGeneric(
  ...
  if (*regions < opMinRegionCount) {
    throw nb::value_error(
        (toolchain::Twine("Operation \"") + name + "\" requires a minimum of " +
         toolchain::Twine(opMinRegionCount) +
         " regions but was built with regions=" + toolchain::Twine(*regions))
            .str()
            .c_str());
  }
  if (opHasNoVariadicRegions && *regions > opMinRegionCount) {
    throw nb::value_error(
        (toolchain::Twine("Operation \"") + name + "\" requires a maximum of " +
         toolchain::Twine(opMinRegionCount) +
         " regions but was built with regions=" + toolchain::Twine(*regions))
            .str()
            .c_str());
  ...
  if (operandSegmentSpecObj.is_none()) {
    ...
    if (operandSegmentSpec.size() != operandList.size()) {
      throw nb::value_error((toolchain::Twine("Operation \"") + name +
                             "\" requires " +
                             toolchain::Twine(operandSegmentSpec.size()) +
                             "operand segments but was provided " +
                             toolchain::Twine(operandList.size()))
                                .str()
                                .c_str());
    }
    operandSegmentLengths.reserve(operandList.size());
    for (const auto &it :
         toolchain::enumerate(toolchain::zip(operandList, operandSegmentSpec))) {
      int segmentSpec = std::get<1>(it.value());
      if (segmentSpec == 1 || segmentSpec == 0) {
        // Unpack unary element.
        ...
        if (!operand.is_none()) {
          ...
          throw nb::value_error(
              (toolchain::Twine("Operand ") + toolchain::Twine(it.index()) +
               " of operation \"" + name +
...
void PyOpAttributeMap::forEachAttr(
    MlirOperation op,
    toolchain::function_ref<void(MlirStringRef, MlirAttribute)> fn) {
...
MlirLocation tracebackToLocation(MlirContext ctx) {
  ...
  for (; pyFrame != nullptr && count < framesLimit;
    ...
    toolchain::StringRef fileName(fileNameStr);
    ...
    toolchain::StringRef funcName(name);
...
void populateIRCore(nb::module_ &m) {
  ...
  nb::class_<PyLocation>(m, "Location")
      ...
      .def_static(
          ...
          [](PyLocation callee, const std::vector<PyLocation> &frames,
            ...
            for (const PyLocation &frame :
                 toolchain::reverse(toolchain::ArrayRef(frames).drop_back()))
      ...
      .def_static(
          ...
          [](const std::vector<PyLocation> &pyLocations,
             ...
             DefaultingPyMlirContext context) {
            toolchain::SmallVector<MlirLocation, 4> locations;
  ...
  nb::class_<PyOperation, PyOperationBase>(m, "Operation")
      .def_static(
          ...
          [](std::string_view name,
            ...
            toolchain::SmallVector<MlirValue, 4> mlirOperands;
            if (operands) {
  ...
  nb::class_<PyBlock>(m, "Block")
      ...
      .def(
          "__hash__",
          [](PyBlock &self) {
            return static_cast<size_t>(toolchain::hash_value(self.get().ptr));
          },
  ...
  nb::class_<PyType>(m, "Type")
      ...
      .def_static(
          ...
          R"(
            ...
            See also: https://mlir.toolchain.org/docs/LangRef/#type-system)")
      .def_prop_ro(
          "context",
      ...
      .def_prop_ro(
          ...
          [](PyType &self) {
            ...
            throw nb::value_error(
                (origRepr + toolchain::Twine(" has no typeid.")).str().c_str());
          },
  ...
  nb::class_<PyValue>(m, "Value", nb::is_generic(),
      ...
      .def(
          ...
          [](PyValue &self, PyValue &with, const nb::list &exceptions) {
            ...
            toolchain::SmallVector<MlirOperation> exceptionOps;
