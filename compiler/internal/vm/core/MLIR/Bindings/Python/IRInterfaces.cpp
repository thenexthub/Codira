...
toolchain::SmallVector<MlirValue> wrapOperands(std::optional<nb::list> operandList) {
  toolchain::SmallVector<MlirValue> mlirOperands;
  ...
  for (const auto &&it : toolchain::enumerate(*operandList)) {
    ...
    try {
      ...
      throw nb::value_error((toolchain::Twine("Operand ") + toolchain::Twine(it.index()) +
...
toolchain::SmallVector<MlirRegion>
wrapRegions(std::optional<std::vector<PyRegion>> regions) {
  toolchain::SmallVector<MlirRegion> mlirRegions;
...
public:
  ...
  inferReturnTypes(std::optional<nb::list> operandList,
    ...
    toolchain::SmallVector<MlirRegion> mlirRegions = wrapRegions(std::move(regions));
