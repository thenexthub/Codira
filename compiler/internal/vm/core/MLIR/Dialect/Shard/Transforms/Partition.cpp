...
static bool arePartialAxesCompatible(const SourceAxes &sourceAxes,
  ...
  return toolchain::all_of(targetAxes, [&sourceAxes](auto &targetAxis) {
...
static Sharding targetShardingInSplitLastAxis(MLIRContext *ctx,
  ...
  SmallVector<GridAxesAttr> targetShardingSplitAxes =
      toolchain::to_vector(sourceSharding.getSplitAxes());
  ...
  auto targetSplitAxes =
      toolchain::to_vector(targetShardingSplitAxes[splitTensorAxis].asArrayRef());
...
static ShapedType allGatherResultShapeInUnsplitLastAxis(
  ...
  SmallVector<int64_t> targetShape = toolchain::to_vector(sourceShape.getShape());
  ...
  for (size_t sourceTensorAxis = 0;
    ...
    for (size_t targetTensorAxis = 0;
      ...
      if (sourceSharding.getSplitAxes()[sourceTensorAxis].empty() ||
        ...
        continue;
      if (!toolchain::equal(
              toolchain::make_range(sourceSharding.getSplitAxes()[sourceTensorAxis]
                                   .asArrayRef()
                                   .begin(),
                               sourceSharding.getSplitAxes()[sourceTensorAxis]
                                       ...
                                       .end() -
                                   1),
              toolchain::make_range(targetSharding.getSplitAxes()[targetTensorAxis]
                                   .asArrayRef()
                                   .begin(),
                               targetSharding.getSplitAxes()[targetTensorAxis]
                                       .asArrayRef()
...
static Sharding targetShardingInMoveLastAxis(MLIRContext *ctx,
  ...
  auto sourceSplitAxes =
      toolchain::to_vector(targetShardingSplitAxes[sourceTensorAxis].asArrayRef());
  ...
  auto targetSplitAxes =
      toolchain::to_vector(targetShardingSplitAxes[targetTensorAxis].asArrayRef());
...
shardedBlockArgumentTypes(Block &block,
  ...
  toolchain::transform(
      ...
      [&symbolTableCollection](BlockArgument arg) {
        ...
        ShardOp shardOp = toolchain::dyn_cast<ShardOp>(useOp);
        assert(shardOp);
...
partitionOperation(Operation &op, ArrayRef<Value> partitionedOperands,
                   ...
                   SymbolTableCollection &symbolTableCollection,
                   OpBuilder &builder) {
  ShardingInterface shardingInterface = toolchain::dyn_cast<ShardingInterface>(op);
  ...
  assert(toolchain::all_of(op.getResults(), [&partitionMap](OpResult result) {
...
static std::vector<Sharding> getOperandShardings(Operation &op) {
  ...
  toolchain::transform(op.getOperands(), std::back_inserter(res), [](Value operand) {
    ...
    assert(definingOp);
    ShardOp shardOp = toolchain::cast<ShardOp>(definingOp);
...
static std::vector<Sharding> getResultShardings(Operation &op) {
  ...
  toolchain::transform(
      op.getResults(), std::back_inserter(res), [&op](OpResult result) {
        ...
        ShardOp shardOp = toolchain::dyn_cast<ShardOp>(userOp);
        if (shardOp) {
          return Sharding(shardOp.getSharding());
        }
...
partitionOperation(Operation &op, IRMapping &partitionMap,
  ...
  ShardOp shardOp = toolchain::dyn_cast<ShardOp>(op);
  if (shardOp) {
  ...
  toolchain::transform(op.getOperands(), std::back_inserter(partitionedOperands),
...
partitionBlock(Block &block, IRMapping &partitionMap,
  ...
  toolchain::transform(block.getArguments(), std::back_inserter(argLocations),
  ...
  for (auto [unshardedBlockArg, partitionedBlockArg] :
       toolchain::zip(block.getArguments(), newBlock->getArguments())) {
...
partitionFuncOp(FunctionOpInterface op, IRMapping &partitionMap,
  ...
  for (Block &b : op.getBlocks()) {
    if (toolchain::any_of(b.getOperations(),
