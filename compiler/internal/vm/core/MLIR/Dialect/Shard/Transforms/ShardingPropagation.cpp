...
template <typename T>
static toolchain::raw_ostream &operator<<(toolchain::raw_ostream &stream,
...
[[maybe_unused]] static toolchain::raw_ostream &operator<<(toolchain::raw_ostream &stream,
...
[[maybe_unused]] static toolchain::raw_ostream &
operator<<(toolchain::raw_ostream &stream, ReshardingRquirementKind v) {
...
ReshardingRquirementKind getReshardingRquirementKind(
  ...
  auto operandShardings =
      toolchain::make_range(operandAndResultShardings.begin(),
                       operandAndResultShardings.begin() + operandsCount);
  auto resultShardings =
      toolchain::make_range(operandAndResultShardings.begin() + operandsCount,
                       operandAndResultShardings.end());

  for (auto [operand, sharding] :
       toolchain::zip_equal(op->getOperands(), operandShardings)) {
  ...
  for (auto [result, sharding] :
       toolchain::zip_equal(op->getResults(), resultShardings)) {
...
static LogicalResult visitOp(Operation *op, OpBuilder &builder) {
  ShardingInterface shardingOp = toolchain::dyn_cast<ShardingInterface>(op);
  if (op->hasTrait<OpTrait::IsTerminator>() ||
      (op->hasTrait<OpTrait::ConstantLike>() && !shardingOp) ||
      toolchain::isa<shard::ShardOp, shard::ShardingOp, shard::GetShardingOp>(op))
    return success();
  ...
  void runOnOperation() override {
    ...
    LLVM_DEBUG(
        ...
        for (Operation &op : block.getOperations()) {
          if (auto shardingOp = toolchain::dyn_cast<ShardingInterface>(&op))
            shardingOp.printLoopTypesAndIndexingMaps(toolchain::dbgs());
        });
    ...
    if (traversal == TraversalOrder::Backward ||
      ...
      traverse(toolchain::reverse(block), builder, "backward");
