...
using AllocDynamicSizesMap =
    toolchain::DenseMap<func::FuncOp, SmallVector<SmallVector<Value>>>;
...
static bool hasFullyDynamicLayoutMap(MemRefType type) {
  ...
  if (failed(type.getStridesAndOffset(strides, offset)))
    return false;
  if (!toolchain::all_of(strides, ShapedType::isDynamic))
    return false;
...
static SmallVector<Value> getDynamicSize(Value memref, func::FuncOp funcOp) {
  ...
  for (Value size : operands) {
    ...
    auto iter = toolchain::find(arguments, sizeSrc);
...
static SmallVector<Value> mapDynamicSizeAtCaller(func::CallOp call,
  ...
  for (Value size : dynamicSizes) {
    for (auto [src, dst] :
         toolchain::zip_first(call.getOperands(), callee.getArguments())) {
      if (size != dst)
        continue;
...
updateFuncOp(func::FuncOp func,
  ...
  for (const auto &resultType : toolchain::enumerate(functionType.getResults())) {
  ...
  auto newArgTypes = toolchain::to_vector<6>(
      toolchain::concat<const Type>(functionType.getInputs(), erasedResultTypes));
...
updateReturnOps(func::FuncOp func, ArrayRef<BlockArgument> appendedEntryArgs,
  ...
  auto res = func.walk([&](func::ReturnOp op) {
    ...
    for (auto [orig, arg] : toolchain::zip(copyIntoOutParams, appendedEntryArgs)) {
...
updateCalls(ModuleOp module, const AllocDynamicSizesMap &map,
  ...
  module.walk([&](func::CallOp op) {
    ...
    auto newOperands = toolchain::to_vector<6>(op.getOperands());
    newOperands.append(outParams.begin(), outParams.end());
    auto newResultTypes = toolchain::to_vector<6>(toolchain::map_range(
    ...
    for (auto t : toolchain::zip(replaceWithNewCallResults, newCall.getResults()))
