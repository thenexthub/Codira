...
struct FuncOpInterface
  ...
  bool hasTensorSemantics(Operation *op) const {
    auto isaTensor = toolchain::IsaPred<TensorLikeType>;
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    for (const auto &it : toolchain::enumerate(funcType.getInputs())) {
    ...
    for (func::ReturnOp returnOp : getReturnOps(funcOp)) {
      ...
      for (auto [returnVal, bufferizedType] :
           toolchain::zip_equal(returnOp->getOperands(), retTypes)) {
