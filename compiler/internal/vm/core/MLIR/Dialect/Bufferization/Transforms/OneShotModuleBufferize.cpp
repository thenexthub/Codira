...
static void annotateEquivalentReturnBbArg(OpOperand &returnVal,
  ...
  if (op->hasAttr(kEquivalentArgsAttr)) {
    ...
    equivBbArgs = toolchain::to_vector<4>(toolchain::map_range(attr, [](Attribute a) {
...
aliasingFuncOpBBArgsAnalysis(FuncOp funcOp, OneShotAnalysisState &state,
  ...
  if (funcOp.getBody().empty()) {
    ...
    for (const auto &inputIt : toolchain::enumerate(type.getInputs())) {
      if (!isa<TensorType>(inputIt.value()))
        continue;
      for (const auto &resultIt : toolchain::enumerate(type.getResults())) {
...
static bool hasTensorSignature(func::FuncOp funcOp) {
  return toolchain::any_of(funcOp.getFunctionType().getInputs(),
                      toolchain::IsaPred<TensorType>) ||
         toolchain::any_of(funcOp.getFunctionType().getResults(),
                      toolchain::IsaPred<TensorType>);
}
...
LogicalResult mlir::bufferization::bufferizeModuleOp(
  ...
  if (failed(getFuncOpsOrderedByCalls(moduleOp, orderedFuncOps,
    ...
    return failure();
  toolchain::append_range(orderedFuncOps, remainingFuncOps);
  ...
  for (func::FuncOp funcOp : orderedFuncOps) {
    ...
    if (toolchain::is_contained(options.noAnalysisFuncFilter, funcOp.getSymName())) {
  ...
  for (mlir::Region &region : moduleOp->getRegions()) {
    for (mlir::Block &block : region.getBlocks()) {
      for (mlir::Operation &op :
           toolchain::make_early_inc_range(block.getOperations())) {
  ...
  if (!options.copyBeforeWrite) {
    if (options.noAnalysisFuncFilter.empty()) {
      ...
      OpFilter::Entry::FilterFn analysisFilterFn = [=](Operation *op) {
        ...
        if (func)
          return toolchain::is_contained(options.noAnalysisFuncFilter,
                                    func.getSymName());
        return false;
      };
      OneShotBufferizationOptions updatedOptions(options);
