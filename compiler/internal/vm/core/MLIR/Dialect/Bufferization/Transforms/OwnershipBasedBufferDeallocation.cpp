...
static bool hasBufferSemantics(Operation *op) {
  if (toolchain::any_of(op->getOperands(), isMemref) ||
      toolchain::any_of(op->getResults(), isMemref))
    return true;
  for (Region &region : op->getRegions())
    if (!region.empty())
      if (toolchain::any_of(region.front().getArguments(), isMemref))
        return true;
  return false;
}
...
public:
  ...
  using BackedgeSetT = toolchain::DenseSet<std::pair<Block *, Block *>>;
...
BufferDeallocation::updateFunctionSignature(FunctionOpInterface op) {
  SmallVector<TypeRange> returnOperandTypes(toolchain::map_range(
  ...
  if (!toolchain::all_equal(returnOperandTypes))
    return op->emitError(
...
Operation *BufferDeallocation::appendOpResults(Operation *op,
  ...
  for (auto [oldRegion, newRegion] :
       toolchain::zip(op->getRegions(), newOp->getRegions()))
...
BufferDeallocation::handleInterface(RegionBranchOpInterface op) {
  ...
  unsigned numMemrefOperands = toolchain::count_if(entryOperands, isMemref);
  ...
  unsigned numMemrefResults = toolchain::count_if(op->getResults(), isMemref);
  ...
  for (auto result : toolchain::make_filter_range(newOp->getResults(), isMemref)) {
...
BufferDeallocation::handleInterface(BranchOpInterface op) {
  ...
  for (auto [retained, ownership] :
       toolchain::zip(deallocOp.getRetained(), deallocOp.getUpdatedConditions())) {
    state.updateOwnership(retained, ownership, block);
  }

  unsigned numAdditionalReturns = toolchain::count_if(forwardedOperands, isMemref);
...
FailureOr<Operation *> BufferDeallocation::handleInterface(CallOpInterface op) {
  ...
  if (options.privateFuncDynamicOwnership && isPrivate) {
    unsigned numMemrefs = toolchain::count_if(op->getResults(), isMemref);
    ...
    for (auto result : toolchain::make_filter_range(op->getResults(), isMemref)) {
...
BufferDeallocation::handleInterface(MemoryEffectOpInterface op) {
  ...
  for (auto operand : toolchain::make_filter_range(op->getOperands(), isMemref)) {
  ...
  for (auto res : toolchain::make_filter_range(op->getResults(), isMemref)) {
