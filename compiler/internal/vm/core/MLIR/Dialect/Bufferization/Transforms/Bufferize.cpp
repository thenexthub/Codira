    ...
    if (!options) {
      ...
      OpFilter::Entry::FilterFn filterFn = [&](Operation *op) {
        ...
        if (this->dialectFilter.hasValue() && !(*this->dialectFilter).empty())
          return toolchain::is_contained(this->dialectFilter,
...
bufferization::bufferizeBlockSignature(Block *block, RewriterBase &rewriter,
  ...
  for (auto [bbArg, type] : toolchain::zip(block->getArguments(), newTypes)) {
  ...
  for (Operation *op : block->getUsers()) {
    ...
    auto it = toolchain::find(op->getSuccessors(), block);
    ...
    for (auto [operand, type] :
         toolchain::zip(operands.getForwardedOperands(), newTypes)) {
