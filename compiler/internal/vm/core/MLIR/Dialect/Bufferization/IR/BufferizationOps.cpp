...
FailureOr<Value> mlir::bufferization::castOrReallocMemRefValue(
  ...
  auto srcType = toolchain::cast<MemRefType>(value.getType());
...
LogicalResult mlir::bufferization::foldToBufferToTensorPair(
  ...
  auto rankedSrcType = toolchain::dyn_cast<MemRefType>(srcType);
  auto rankedDestType = toolchain::dyn_cast<MemRefType>(destType);
  auto unrankedSrcType = toolchain::dyn_cast<UnrankedMemRefType>(srcType);
...
void mlir::bufferization::populateDynamicDimSizes(
  ...
  auto shapedType = toolchain::cast<ShapedType>(shapedValue.getType());
  for (int64_t i = 0; i < shapedType.getRank(); ++i) {
    if (shapedType.isDynamicDim(i)) {
      if (toolchain::isa<MemRefType>(shapedType)) {
        dynamicDims.push_back(memref::DimOp::create(b, loc, shapedValue, i));
      } else {
        assert(toolchain::isa<RankedTensorType>(shapedType) && "expected tensor");
...
LogicalResult AllocTensorOp::bufferize(RewriterBase &rewriter,
  ...
  FailureOr<Value> alloc = options.createAlloc(
      rewriter, loc, toolchain::cast<MemRefType>(*allocType), dynamicDims);
  if (failed(alloc))
    return failure();
...
struct ReplaceStaticShapeDims : OpRewritePattern<AllocTensorOp> {
  ...
  LogicalResult matchAndRewrite(AllocTensorOp op,
    ...
    if (op.getCopy())
      return failure();
    SmallVector<int64_t> newShape = toolchain::to_vector(op.getType().getShape());
...
LogicalResult AllocTensorOp::reifyResultShapes(
  ...
  auto shapes = toolchain::to_vector<4>(
      toolchain::map_range(toolchain::seq<int64_t>(0, getType().getRank()),
...
void AllocTensorOp::print(OpAsmPrinter &p) {
  ...
  if (auto validType = toolchain::dyn_cast<::mlir::TensorType>(type))
...
LogicalResult MaterializeInDestinationOp::verify() {
  ...
  if (srcType.hasRank()) {
    ...
    for (auto [src, dest] :
         toolchain::zip(srcType.getShape(), destType.getShape())) {
...
struct ToBufferOfCast : public OpRewritePattern<ToBufferOp> {
  ...
  LogicalResult matchAndRewrite(ToBufferOp toBuffer,
    ...
    if (!tensorCastOperand)
      return failure();
    auto srcTensorType = toolchain::dyn_cast<RankedTensorType>(
...
struct DeallocRemoveDuplicateDeallocMemrefs
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    for (auto [i, memref, cond] :
         toolchain::enumerate(deallocOp.getMemrefs(), deallocOp.getConditions())) {
...
struct DeallocRemoveDuplicateRetainedMemrefs
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    SmallVector<Value> replacements(
        toolchain::map_range(resultReplacementIdx, [&](unsigned idx) {
...
struct EraseAlwaysFalseDealloc : public OpRewritePattern<DeallocOp> {
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    for (auto [memref, cond] :
         toolchain::zip(deallocOp.getMemrefs(), deallocOp.getConditions())) {
...
struct SkipExtractMetadataOfAlloc : public OpRewritePattern<DeallocOp> {
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    SmallVector<Value> newMemrefs(
        toolchain::map_range(deallocOp.getMemrefs(), [&](Value memref) {
