...
void DeallocationState::dropMemrefToDeallocate(Value memref, Block *block) {
  toolchain::erase(memrefsToDeallocatePerBlock[block], memref);
}

void DeallocationState::getLiveMemrefsIn(Block *block,
                                         SmallVectorImpl<Value> &memrefs) {
  SmallVector<Value> liveMemrefs(
      toolchain::make_filter_range(liveness.getLiveIn(block), isMemref));
  toolchain::sort(liveMemrefs, ValueComparator());
...
void DeallocationState::getMemrefsToRetain(
  ...
  if (toBlock)
    toolchain::set_intersect(liveOut, liveness.getLiveIn(toBlock));
  ...
  toolchain::sort(retainedByLiveness, ValueComparator());
...
LogicalResult DeallocationState::getMemrefsAndConditionsToDeallocate(
  ...
  for (auto [i, memref] :
       toolchain::enumerate(memrefsToDeallocatePerBlock.lookup(block))) {
...
bool ValueComparator::operator()(const Value &lhs, const Value &rhs) const {
  ...
  Region *rhsRegion;
  if (lhsIsBBArg) {
    auto lhsBBArg = toolchain::cast<BlockArgument>(lhs);
    auto rhsBBArg = toolchain::cast<BlockArgument>(rhs);
    ...
    return toolchain::cast<OpResult>(lhs).getResultNumber() <
           toolchain::cast<OpResult>(rhs).getResultNumber();
  } else {
...
FailureOr<Operation *> deallocation_impl::insertDeallocOpForReturnLike(
  ...
  unsigned numMemrefOperands = toolchain::count_if(operands, isMemref);
