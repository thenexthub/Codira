...
Operation *bufferization::getOwnerOfValue(Value value) {
  if (auto opResult = toolchain::dyn_cast<OpResult>(value))
    return opResult.getDefiningOp();
  return toolchain::cast<BlockArgument>(value).getOwner()->getParentOp();
}
...
FailureOr<Value> bufferization::allocateTensorForShapedValue(
    ...
    bool copy) {
  Value tensor;
  if (toolchain::isa<RankedTensorType>(shapedValue.getType())) {
    tensor = shapedValue;
  } else if (toolchain::isa<MemRefType>(shapedValue.getType())) {
    tensor = ToTensorOp::create(
        b, loc, memref::getTensorTypeFromMemRefType(shapedValue.getType()),
        shapedValue);
  } else if (toolchain::isa<UnrankedTensorType>(shapedValue.getType()) ||
             toolchain::isa<UnrankedMemRefType>(shapedValue.getType())) {
  ...
  RankedTensorType tensorType = toolchain::cast<RankedTensorType>(tensor.getType());
  ...
  if (!copy) {
    ...
    if (toolchain::isa<RankedTensorType>(shapedValue.getType()) &&
        toolchain::isa<OpResult>(shapedValue)) {
      ReifiedRankedShapedTypeDims resultDims;
      if (succeeded(
              reifyResultShapes(b, shapedValue.getDefiningOp(), resultDims))) {
        reifiedShapes = true;
        auto &shape =
            resultDims[toolchain::cast<OpResult>(shapedValue).getResultNumber()];
...
LogicalResult BufferizableOpInterface::resolveTensorOpOperandConflicts(
  ...
  for (OpOperand &opOperand : op->getOpOperands()) {
    ...
    if (!toolchain::isa<TensorType>(operandType))
      continue;
    if (analysisState.isInPlace(opOperand))
      continue;
    if (toolchain::isa<UnrankedTensorType>(operandType))
  ...
  for (Value value : outOfPlaceValues) {
    ...
    if (failed(copy))
      return failure();
    SmallVector<OpOperand *> uses = toolchain::to_vector(
        toolchain::map_range(value.getUses(), [](OpOperand &use) { return &use; }));
...
static void setInsertionPointAfter(OpBuilder &b, Value value) {
  if (auto bbArg = toolchain::dyn_cast<BlockArgument>(value)) {
...
bool AnalysisState::bufferizesToMemoryWrite(Value value) const {
  auto opResult = toolchain::dyn_cast<OpResult>(value);
  if (!opResult)
    return true;
...
bool AnalysisState::isValueRead(Value value) const {
  assert(toolchain::isa<TensorType>(value.getType()) && "expected TensorType");
...
toolchain::SetVector<Value> AnalysisState::findValueInReverseUseDefChain(
    OpOperand *opOperand, toolchain::function_ref<bool(Value)> condition,
    TraversalConfig config,
    toolchain::DenseSet<OpOperand *> *visitedOpOperands) const {
  toolchain::DenseSet<Value> visited;
  toolchain::SetVector<Value> result, workingSet;
...
#ifndef NDEBUG
  auto tensorType = toolchain::dyn_cast<TensorLikeType>(value.getType());
...
bufferization::getBufferType(Value value, const BufferizationOptions &options,
                             const BufferizationState &state,
                             SmallVector<Value> &invocationStack) {
  assert(toolchain::isa<TensorLikeType>(value.getType()) &&
         "unexpected non-tensor type");
  invocationStack.push_back(value);
  toolchain::scope_exit popFromStack([&]() { invocationStack.pop_back(); });
...
void bufferization::replaceOpWithBufferizedValues(RewriterBase &rewriter,
  ...
  for (OpResult opResult : op->getOpResults()) {
    ...
    if (toolchain::isa<TensorLikeType>(opResult.getType())) {
      // The OpResult is a tensor. Such values are replaced with memrefs during
      // bufferization.
      assert(toolchain::isa<BufferLikeType>(replacement.getType()) &&
...
BaseMemRefType bufferization::getMemRefType(TensorType tensorType,
  ...
  if (auto unrankedTensorType =
          toolchain::dyn_cast<UnrankedTensorType>(tensorType)) {
  ...
  auto rankedTensorType = toolchain::cast<RankedTensorType>(tensorType);
  if (layout) {
...
bool bufferization::detail::defaultResultBufferizesToMemoryWrite(
  ...
  if (toolchain::any_of(opOperands, [&](AliasingOpOperand alias) {
...
AliasingOpOperandList bufferization::detail::defaultGetAliasingOpOperands(
  ...
  for (OpOperand &opOperand : op->getOpOperands()) {
    if (!toolchain::isa<TensorType>(opOperand.get().getType()))
      continue;
...
FailureOr<BufferLikeType> bufferization::detail::defaultGetBufferType(
    ...
    const BufferizationState &bufferizationState,
    SmallVector<Value> &invocationStack) {
  assert(toolchain::isa<TensorType>(value.getType()) && "expected tensor type");
  ...
  Operation *op = getOwnerOfValue(value);
  auto opResult = toolchain::cast<OpResult>(value);
...
bufferization::detail::unknownGetAliasingValues(OpOperand &opOperand) {
  ...
  for (OpResult result : opOperand.getOwner()->getOpResults())
    if (toolchain::isa<TensorType>(result.getType()))
