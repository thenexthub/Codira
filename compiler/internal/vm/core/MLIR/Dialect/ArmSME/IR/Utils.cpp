...
LogicalResult verifyOperationHasValidTileId(Operation *op) {
  auto tileOp = toolchain::dyn_cast<ArmSMETileOpInterface>(op);
  if (!tileOp)
...
scf::ForOp createLoopOverTileSlices(
  ...
  auto minTileSlices = arith::ConstantIndexOp::create(
      rewriter, loc, toolchain::cast<VectorType>(initTile.getType()).getDimSize(0));
  auto vscale =
...
OpOperand *getTileOpOperand(arm_sme::ArmSMETileOpInterface tileOp) {
  ...
  assert(toolchain::count_if(tileOp->getOpOperands(), isTileOperandType) <= 1 &&
         "expected at most one tile operand");
  OpOperand *tileOperand =
      toolchain::find_if(tileOp->getOpOperands(), isTileOperandType);
