...
public:
  ...
  FailureOr<unsigned> allocateTileId(ArmSMETileType tileType) {
    ...
    for (auto [tileId, tileMask] : toolchain::enumerate(masks)) {
...
void splitCondBranches(IRRewriter &rewriter, FunctionOpInterface function) {
  ...
  function.walk([&](cf::CondBranchOp condBranch) {
    if (toolchain::any_of(condBranch->getOperands(), [&](Value value) {
...
struct LiveRange {
  using RangeSet = toolchain::IntervalMap<uint64_t, uint8_t, 16,
  ...
  bool overlaps(LiveRange const &otherRange) const {
    return toolchain::IntervalMapOverlaps<RangeSet, RangeSet>(*ranges,
                                                         *otherRange.ranges)
        .valid();
  }
...
coalesceTileLiveRanges(DenseMap<Value, LiveRange> &initialLiveRanges) {
  ...
  auto applyRule = [&](auto rule) {
    toolchain::for_each(toolchain::make_first_range(initialLiveRanges), rule);
  };
  ...
  toolchain::sort(coalescedLiveRanges,
...
chooseSpillUsingHeuristics(OverlappingRangesIterator overlappingRanges,
  ...
  if (isTrivialSpill(*newRange))
    return newRange;
  auto trivialSpill = toolchain::find_if(overlappingRanges, isTrivialSpill);
  ...
  LiveRange &latestEndingLiveRange =
      *toolchain::max_element(overlappingRanges, isSmallerTileTypeOrEndsEarlier);
...
void allocateTilesToLiveRanges(
  ...
  for (LiveRange *nextRange : liveRangesSortedByStartPoint) {
    ...
    if (succeeded(tileId)) {
      ...
      auto allOverlappingRanges = toolchain::concat<LiveRange>(
          toolchain::make_pointee_range(activeRanges.getArrayRef()),
          toolchain::make_pointee_range(overlappingInactiveRanges));
...
void dumpLiveRanges(DenseMap<Operation *, unsigned> const &operationToIndexMap,
                    ...
                    FunctionOpInterface function) {
  toolchain::errs() << "SME Tile Liveness: @" << function.getName()
               << "\nKey:\nS - Start\nE - End\n| - Live\n";
  for (auto [blockIdx, block] : toolchain::enumerate(function.getBlocks())) {
    toolchain::errs() << "^bb" << blockIdx << ":\n";
    for (Operation &op : block.getOperations()) {
      ...
      for (LiveRange const *range : liveRanges) {
        ...
        toolchain::errs() << liveness;
      }
      toolchain::errs() << ' ' << op.getName() << '\n';
    }
  }
  toolchain::errs() << "==========\n";
}
...
LogicalResult mlir::arm_sme::allocateSMETiles(FunctionOpInterface function,
  ...
  if (dumpRanges) {
    ...
    auto nonEmpty = toolchain::make_filter_range(
        toolchain::make_second_range(initialLiveRanges),
        [&](LiveRange const &liveRange) { return !liveRange.empty(); });
    auto initialRanges = toolchain::to_vector(toolchain::map_range(
        nonEmpty, [](LiveRange const &liveRange) { return &liveRange; }));
    toolchain::sort(initialRanges,
