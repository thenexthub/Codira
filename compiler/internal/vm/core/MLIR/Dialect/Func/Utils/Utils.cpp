...
/// ```
///
static toolchain::SmallVector<toolchain::SmallVector<int>>
getInverseMapping(ArrayRef<int> oldIdxToNewIdx) {
  int numOfNewIdxs = 0;
  if (!oldIdxToNewIdx.empty())
    numOfNewIdxs = 1 + *toolchain::max_element(oldIdxToNewIdx);
  toolchain::SmallVector<toolchain::SmallVector<int>> newToOldIdxs(numOfNewIdxs);
  for (auto [oldIdx, newIdx] : toolchain::enumerate(oldIdxToNewIdx))
...
static SmallVector<Element> getMappedElements(
    ...
    const toolchain::SmallVector<toolchain::SmallVector<int>> &newIdxToOldIdxs) {
  SmallVector<Element> newElements;
  for (const auto &oldIdxs : newIdxToOldIdxs) {
    assert(toolchain::all_of(oldIdxs,
...
func::replaceFuncWithNewMapping(RewriterBase &rewriter, func::FuncOp funcOp,
  ...
  toolchain::SmallVector<toolchain::SmallVector<int>> newArgIdxToOldArgIdxs =
  ...
  toolchain::SmallVector<toolchain::SmallVector<int>> newResToOldResIdxs =
  ...
  for (auto [oldArgIdx, newArgIdx] : toolchain::enumerate(oldArgIdxToNewArgIdx))
    operandMapper.map(funcOp.getArgument(oldArgIdx),
                      newFuncOp.getArgument(newArgIdx));
  for (auto [newArgIdx, oldArgIdx] : toolchain::enumerate(newArgIdxToOldArgIdxs))
    newFuncOp.setArgAttrs(newArgIdx, argAttrs[oldArgIdx.front()]);

  funcOp.getAllResultAttrs(resultAttrs);
  for (auto [newResIdx, oldResIdx] : toolchain::enumerate(newResToOldResIdxs))
...
func::replaceCallOpWithNewMapping(RewriterBase &rewriter, func::CallOp callOp,
  ...
  SmallVector<toolchain::SmallVector<int>> newArgIdxToOldArgIdxs =
      getInverseMapping(oldArgIdxToNewArgIdx);
  SmallVector<Value> newOperandsValues =
      getMappedElements<Value>(origOperands, newArgIdxToOldArgIdxs);
  SmallVector<toolchain::SmallVector<int>> newResToOldResIdxs =
      getInverseMapping(oldResIdxToNewResIdx);
  SmallVector<Type> origResultTypes = toolchain::to_vector(callOp.getResultTypes());
  ...
  for (auto &&[oldResIdx, newResIdx] : toolchain::enumerate(oldResIdxToNewResIdx))
...
func::deduplicateArgsOfFuncOp(RewriterBase &rewriter, func::FuncOp funcOp,
  ...
  toolchain::DenseMap<Value, int> valueToNewArgIdx;
  for (auto [operandIdx, operand] : toolchain::enumerate(callOp.getOperands())) {
  ...
  for (int resultIdx : toolchain::seq<int>(0, callOp.getNumResults()))
...
func::lookupFnDecl(SymbolOpInterface symTable, StringRef name,
  ...
  if (symbolTables) {
    ...
    func = toolchain::dyn_cast_or_null<FuncOp>(
