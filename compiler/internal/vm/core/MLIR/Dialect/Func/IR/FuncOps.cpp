...
Operation *FuncDialect::materializeConstant(OpBuilder &builder, Attribute value,
  ...
  if (ConstantOp::isBuildableWith(value, type))
    return ConstantOp::create(builder, loc, type,
                              toolchain::cast<FlatSymbolRefAttr>(value));
  return nullptr;
}
...
bool ConstantOp::isBuildableWith(Attribute value, Type type) {
  return toolchain::isa<FlatSymbolRefAttr>(value) && toolchain::isa<FunctionType>(type);
}
...
void FuncOp::cloneInto(FuncOp dest, IRMapping &mapper) {
  ...
  toolchain::MapVector<StringAttr, Attribute> newAttrMap;
  for (const auto &attr : dest->getAttrs())
    newAttrMap.insert({attr.getName(), attr.getValue()});
  for (const auto &attr : (*this)->getAttrs())
    newAttrMap.insert({attr.getName(), attr.getValue()});

  auto newAttrs = toolchain::to_vector(toolchain::map_range(
