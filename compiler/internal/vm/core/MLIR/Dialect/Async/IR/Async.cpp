...
bool ExecuteOp::areTypesCompatible(Type lhs, Type rhs) {
  const auto getValueOrTokenType = [](Type type) {
    if (auto value = toolchain::dyn_cast<ValueType>(type))
...
void ExecuteOp::build(OpBuilder &builder, OperationState &result,
  ...
  for (Value operand : operands) {
    auto valueType = toolchain::dyn_cast<ValueType>(operand.getType());
...
void ExecuteOp::print(OpAsmPrinter &p) {
  ...
  p.printOptionalArrowTypeList(toolchain::drop_begin(getResultTypes()));
...
ParseResult ExecuteOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  auto parseAsyncValueArg = [&]() -> ParseResult {
    ...
    auto valueTy = toolchain::dyn_cast<ValueType>(valueTypes.back());
...
LogicalResult ExecuteOp::verifyRegions() {
  ...
  auto unwrappedTypes = toolchain::map_range(getBodyOperands(), [](Value operand) {
    return toolchain::cast<ValueType>(operand.getType()).getValueType();
  });
...
LogicalResult CreateGroupOp::canonicalize(CreateGroupOp op,
  ...
  toolchain::SmallVector<AwaitAllOp> awaitAllUsers;
  ...
  if (!toolchain::all_of(op->getUsers(), isAwaitAll))
    return failure();
...
void AwaitOp::build(OpBuilder &builder, OperationState &result, Value operand,
  ...
  if (auto valueType = toolchain::dyn_cast<ValueType>(operand.getType()))
...
static ParseResult parseAwaitResultType(OpAsmParser &parser, Type &operandType,
  ...
  // Add unwrapped async.value type to the returned values types.
  if (auto valueType = toolchain::dyn_cast<ValueType>(operandType))
...
LogicalResult FuncOp::verify() {
  ...
  for (unsigned i = 0, e = resultTypes.size(); i != e; ++i) {
    ...
    if (!toolchain::isa<TokenType>(type) && !toolchain::isa<ValueType>(type))
    ...
    if (toolchain::isa<TokenType>(type) && i != 0) {
      return emitOpError()
...
LogicalResult ReturnOp::verify() {
  ...
  auto types = toolchain::map_range(resultTypes, [](const Type &result) {
    return toolchain::cast<ValueType>(result).getValueType();
  });
