...
struct CoroMachinery {
  ...
  toolchain::SmallVector<Value, 4> returnValues; // returned async values
...
using FuncCoroMapPtr =
    std::shared_ptr<toolchain::DenseMap<func::FuncOp, CoroMachinery>>;
...
///
/// See LLVM coroutines documentation: https://toolchain.org/docs/Coroutines.html
///
...
///  - `suspend block after the @toolchain.coro.end() defines what value will be
///    returned to the initial caller of a coroutine. Everything before the
///    @toolchain.coro.end() will be executed at every suspension point.
///
...
static CoroMachinery setupCoroMachinery(func::FuncOp func) {
  ...
  toolchain::SmallVector<Value, 4> retValues;
  ...
  toolchain::append_range(ret, retValues);
...
outlineExecuteOp(SymbolTable &symbolTable, ExecuteOp execute) {
  ...
  SetVector<mlir::Value> functionInputs(toolchain::from_range,
  ...
  auto typesRange = toolchain::map_range(
...
// operations based on toolchain coroutine
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(async::ReturnOp op, OpAdaptor adaptor,
    ...
    for (auto tuple : toolchain::zip(adaptor.getOperands(), coro.returnValues)) {
...
void AsyncToAsyncRuntimePass::runOnOperation() {
  ...
  FuncCoroMapPtr coros =
      std::make_shared<toolchain::DenseMap<func::FuncOp, CoroMachinery>>();
  ...
  LLVM_DEBUG({
    toolchain::dbgs() << "Outlined " << coros->size()
