...
static LogicalResult walkReferenceCountedValues(
    Operation *op, toolchain::function_ref<LogicalResult(Value)> addRefCounting) {
...
LogicalResult AsyncRuntimeRefCountingPass::addDropRefAfterLastUse(Value value) {
  ...
  toolchain::SmallPtrSet<Operation *, 4> lastUsers;
  ...
  // regions.
  toolchain::DenseMap<Block *, Operation *> usersInTheBlocks;
...
AsyncRuntimeRefCountingPass::addDropRefInDivergentLivenessSuccessor(
    Value value) {
  using BlockSet = toolchain::SmallPtrSet<Block *, 4>;
  ...
  toolchain::SmallDenseMap<Block *, BlockSet> divergentLivenessBlocks;
  ...
  for (auto kv : divergentLivenessBlocks) {
    ...
    if (toolchain::any_of(successors, hasArgs))
    ...
    for (Block *successor : successors) {
      ...
      for (const auto &pair : toolchain::enumerate(terminator->getSuccessors()))
...
private:
  ...
  toolchain::SmallVector<std::function<FailureOr<int>(OpOperand &)>> policy;
};
