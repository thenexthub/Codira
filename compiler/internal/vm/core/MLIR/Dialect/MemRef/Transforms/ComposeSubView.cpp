...
struct ComposeSubViewOpPattern : public OpRewritePattern<memref::SubViewOp> {
  ...
  LogicalResult matchAndRewrite(memref::SubViewOp op,
    ...
    for (auto &&[opStride, sourceStride] :
         toolchain::zip(opStrides, sourceStrides)) {
    ...
    for (auto &&[opOffset, sourceOffset, sourceStride, opSize] :
         toolchain::zip(op.getMixedOffsets(), sourceOp.getMixedOffsets(),
                   sourceOp.getMixedStrides(), op.getMixedSizes())) {
      sizes.push_back(opSize);
      Attribute opOffsetAttr = toolchain::dyn_cast_if_present<Attribute>(opOffset),
                sourceOffsetAttr =
                    toolchain::dyn_cast_if_present<Attribute>(sourceOffset),
                sourceStrideAttr =
                    toolchain::dyn_cast_if_present<Attribute>(sourceStride);
      if (opOffsetAttr && sourceOffsetAttr) {
        ...
        if (auto attr = toolchain::dyn_cast_if_present<Attribute>(sourceOffset)) {
          expr =
        ...
        // result.
        if (auto attr = toolchain::dyn_cast_if_present<Attribute>(opOffset)) {
