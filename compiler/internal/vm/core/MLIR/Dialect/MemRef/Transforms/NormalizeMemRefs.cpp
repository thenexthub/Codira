...
void NormalizeMemRefs::runOnOperation() {
  LLVM_DEBUG(toolchain::dbgs() << "Normalizing Memrefs...\n");
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Normalizing " << normalizableFuncs.size()
                          << " functions\n");
...
static bool isMemRefNormalizable(Value::user_range opUsers) {
  return toolchain::all_of(opUsers, [](Operation *op) {
...
bool NormalizeMemRefs::areMemRefsNormalizable(func::FuncOp funcOp) {
  ...
  if (funcOp
          .walk([&](func::CallOp callOp) -> WalkResult {
            for (unsigned resIndex :
                 toolchain::seq<unsigned>(0, callOp.getNumResults())) {
  ...
  for (unsigned argIndex : toolchain::seq<unsigned>(0, funcOp.getNumArguments())) {
...
void NormalizeMemRefs::updateFunctionSignature(func::FuncOp funcOp,
  ...
  resultTypes = toolchain::to_vector<4>(functionType.getResults());

  // External function's signature was already updated in
  // 'normalizeFuncOpMemRefs()'.
  if (!funcOp.isExternal()) {
    SmallVector<Type, 8> argTypes;
    for (const auto &argEn : toolchain::enumerate(funcOp.getArguments()))
    ...
    funcOp.walk([&](func::ReturnOp returnOp) {
      for (const auto &operandEn : toolchain::enumerate(returnOp.getOperands())) {
  ...
  // signature as well.
  toolchain::SmallDenseSet<func::FuncOp, 8> funcOpsToUpdate;
  ...
  for (SymbolTable::SymbolUse symbolUse : *symbolUses) {
    ...
    for (unsigned resIndex : toolchain::seq<unsigned>(0, userOp->getNumResults())) {
...
void NormalizeMemRefs::normalizeFuncOpMemRefs(func::FuncOp funcOp,
  ...
  FunctionType functionType = funcOp.getFunctionType();
  SmallVector<Location> functionArgLocs(toolchain::map_range(
  ...
  for (unsigned argIndex :
       toolchain::seq<unsigned>(0, functionType.getNumInputs())) {
  ...
  funcOp.walk([&](Operation *op) {
    if (op->hasTrait<OpTrait::MemRefsNormalizable>() &&
      ...
      if (op != newOp) {
        bool replacingMemRefUsesFailed = false;
        for (unsigned resIndex : toolchain::seq<unsigned>(0, op->getNumResults())) {
  ...
  if (funcOp.isExternal()) {
    SmallVector<Type, 4> resultTypes;
    for (unsigned resIndex :
         toolchain::seq<unsigned>(0, functionType.getNumResults())) {
...
Operation *NormalizeMemRefs::createOpResultsNormalized(func::FuncOp funcOp,
  ...
  for (unsigned resIndex : toolchain::seq<unsigned>(0, oldOp->getNumResults())) {
