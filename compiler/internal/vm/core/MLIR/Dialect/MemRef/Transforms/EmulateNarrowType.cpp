...
convertCastingOp(ConversionPatternRewriter &rewriter,
  ...
  if (toolchain::any_of(op.getStaticStrides(),
  ...
  if (!sizes.empty())
    size.push_back(toolchain::divideCeilSigned(sizes[0], elementsPerByte));
...
struct ConvertMemRefCopy final : OpConversionPattern<memref::CopyOp> {
  ...
  matchAndRewrite(memref::CopyOp op, OpAdaptor adaptor,
    ...
    if (maybeRankedSource && maybeRankedDest &&
      ...
      return rewriter.notifyMatchFailure(
          op, toolchain::formatv("memref.copy emulation with distinct layouts ({0} "
...
struct ConvertMemRefSubview final : OpConversionPattern<memref::SubViewOp> {
  ...
  matchAndRewrite(memref::SubViewOp subViewOp, OpAdaptor adaptor,
    ...
    if (!newTy) {
      return rewriter.notifyMatchFailure(
          subViewOp->getLoc(),
          toolchain::formatv("failed to convert memref type: {0}",
                        subViewOp.getType()));
    }
    ...
    // Only support stride of 1.
    if (toolchain::any_of(subViewOp.getStaticStrides(),
                     [](int64_t stride) { return stride != 1; })) {
