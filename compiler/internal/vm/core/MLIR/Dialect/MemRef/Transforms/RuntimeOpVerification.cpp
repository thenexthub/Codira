...
struct CastOpInterface
  ...
  generateRuntimeVerification(Operation *op, OpBuilder &builder, Location loc,
    ...
    for (const auto &it : toolchain::enumerate(resultType.getShape())) {
    ...
    // Check strides.
    for (const auto &it : toolchain::enumerate(resultStrides)) {
...
struct LoadStoreOpInterface
  ...
  generateRuntimeVerification(Operation *op, OpBuilder &builder, Location loc,
    ...
    Value assertCond;
    for (auto i : toolchain::seq<int64_t>(0, rank)) {
...
struct SubViewOpInterface
  ...
  generateRuntimeVerification(Operation *op, OpBuilder &builder, Location loc,
    ...
    for (int64_t i : toolchain::seq<int64_t>(0, sourceType.getRank())) {
...
struct ExpandShapeOpInterface
  ...
  generateRuntimeVerification(Operation *op, OpBuilder &builder, Location loc,
    ...
    // size.
    for (const auto &it :
         toolchain::enumerate(expandShapeOp.getReassociationIndices())) {
      Value srcDimSz =
