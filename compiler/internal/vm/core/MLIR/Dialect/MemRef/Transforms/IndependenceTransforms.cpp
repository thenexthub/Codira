...
FailureOr<Value> memref::buildIndependentOp(OpBuilder &b,
  ...
  if (toolchain::equal(allocaOp.getMixedSizes(), newSizes))
...
static void replaceAndPropagateMemRefType(RewriterBase &rewriter,
  ...
  for (const auto &it :
       toolchain::enumerate(toolchain::zip(from->getResults(), to->getResults()))) {
  ...
  for (int i = 0; i < static_cast<int>(unrealizedConversions.size()); ++i) {
    ...
    SmallVector<Operation *> users = toolchain::to_vector(conversion->getUsers());
    for (Operation *user : users) {
      ...
      if (toolchain::any_of(user->getResultTypes(),
                       [](Type t) { return isa<MemRefType>(t); }))
        continue;
      if (toolchain::any_of(user->getRegions(), [](Region &r) {
            return toolchain::any_of(r.getArguments(), [](BlockArgument bbArg) {
...
memref::AllocaOp memref::allocToAlloca(
  ...
  for (Operation &candidate :
       toolchain::make_range(alloc->getIterator(), alloc->getBlock()->end())) {
