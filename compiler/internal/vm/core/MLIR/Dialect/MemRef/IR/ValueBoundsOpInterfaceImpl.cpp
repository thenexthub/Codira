...
struct CastOpInterface
  ...
  void populateBoundsForShapedValueDim(Operation *op, Value value, int64_t dim,
    ...
    if (toolchain::isa<MemRefType>(castOp.getResult().getType()) &&
        toolchain::isa<MemRefType>(castOp.getSource().getType())) {
...
struct RankOpInterface
  ...
  void populateBoundsForIndexValue(Operation *op, Value value,
    ...
    auto memrefType = toolchain::dyn_cast<MemRefType>(rankOp.getMemref().getType());
    if (!memrefType)
      return;
...
struct SubViewOpInterface
  ...
  void populateBoundsForShapedValueDim(Operation *op, Value value, int64_t dim,
    ...
    toolchain::SmallBitVector dropped = subViewOp.getDroppedDims();
    int64_t ctr = -1;
