...
LogicalResult mlir::memref::foldMemRefCast(Operation *op, Value inner) {
  ...
  for (OpOperand &operand : op->getOpOperands()) {
    ...
    if (cast && operand.get() != inner &&
        !toolchain::isa<UnrankedMemRefType>(cast.getOperand().getType())) {
...
Type mlir::memref::getTensorTypeFromMemRefType(Type type) {
  if (auto memref = toolchain::dyn_cast<MemRefType>(type))
    return RankedTensorType::get(memref.getShape(), memref.getElementType());
  if (auto memref = toolchain::dyn_cast<UnrankedMemRefType>(type))
...
OpFoldResult memref::getMixedSize(OpBuilder &builder, Location loc, Value value,
                                  int64_t dim) {
  auto memrefType = toolchain::cast<MemRefType>(value.getType());
...
static void constifyIndexValues(SmallVectorImpl<OpFoldResult> &values,
  ...
  for (auto [i, cstVal] : toolchain::enumerate(constValues)) {
...
bubbleDownCastsPassthroughOpImpl(ConcreteOpTy op, OpBuilder &builder,
  ...
  toolchain::append_range(operands, op->getOperands());
  ...
  auto newOp = ConcreteOpTy::create(
      ...
      toolchain::to_vector_of<NamedAttribute>(op->getDiscardableAttrs()));
...
static LogicalResult verifyAllocLikeOp(AllocLikeOp op) {
  static_assert(toolchain::is_one_of<AllocLikeOp, AllocOp, AllocaOp>::value,
                "applies to only alloc or alloca");
  auto memRefType = toolchain::dyn_cast<MemRefType>(op.getResult().getType());
  if (!memRefType)
...
struct SimplifyAllocConst : public OpRewritePattern<AllocLikeOp> {
  ...
  LogicalResult matchAndRewrite(AllocLikeOp alloc,
    ...
    if (toolchain::none_of(alloc.getDynamicSizes(), [](Value operand) {
          APInt constSizeArg;
...
struct SimplifyDeadAlloc : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T alloc,
                                PatternRewriter &rewriter) const override {
    if (toolchain::any_of(alloc->getUsers(), [&](Operation *op) {
    ...
    for (Operation *user : toolchain::make_early_inc_range(alloc->getUsers()))
...
LogicalResult ReallocOp::verify() {
  auto sourceType = toolchain::cast<MemRefType>(getOperand(0).getType());
...
struct AllocaScopeHoister : public OpRewritePattern<AllocaScopeOp> {
  ...
  LogicalResult matchAndRewrite(AllocaScopeOp op,
    ...
    op->walk([&](Operation *alloc) {
      ...
      if (toolchain::any_of(alloc->getOperands(), [&](Value v) {
...
LogicalResult AssumeAlignmentOp::verify() {
  if (!toolchain::isPowerOf2_32(getAlignment()))
...
LogicalResult DistinctObjectsOp::inferReturnTypes(
  ...
  toolchain::copy(operands.getTypes(), std::back_inserter(inferredReturnTypes));
  return success();
}
...
bool CastOp::canFoldIntoConsumerOp(CastOp castOp) {
  MemRefType sourceType =
      toolchain::dyn_cast<MemRefType>(castOp.getSource().getType());
  MemRefType resultType = toolchain::dyn_cast<MemRefType>(castOp.getType());
  ...
  for (auto it : toolchain::zip(sourceType.getShape(), resultType.getShape())) {
  ...
  for (auto it : toolchain::zip(sourceStrides, resultStrides)) {
    auto ss = std::get<0>(it), st = std::get<1>(it);
    if (ss != st)
      if (ShapedType::isDynamic(ss) && ShapedType::isStatic(st))
        return false;
...
bool CastOp::areCastCompatible(TypeRange inputs, TypeRange outputs) {
  ...
  auto aT = toolchain::dyn_cast<MemRefType>(a);
  auto bT = toolchain::dyn_cast<MemRefType>(b);

  auto uaT = toolchain::dyn_cast<UnrankedMemRefType>(a);
  auto ubT = toolchain::dyn_cast<UnrankedMemRefType>(b);
...
struct FoldEmptyCopy final : public OpRewritePattern<CopyOp> {
  ...
  static bool isEmptyMemRef(BaseMemRefType type) {
    return type.hasRank() && toolchain::is_contained(type.getShape(), 0);
  }
...
static FailureOr<toolchain::SmallBitVector>
computeMemRefRankReductionMask(MemRefType originalType, MemRefType reducedType,
                               ArrayRef<OpFoldResult> sizes) {
  toolchain::SmallBitVector unusedDims(originalType.getRank());
  if (originalType.getRank() == reducedType.getRank())
    return unusedDims;

  for (const auto &dim : toolchain::enumerate(sizes))
    if (auto attr = toolchain::dyn_cast_if_present<Attribute>(dim.value()))
      if (toolchain::cast<IntegerAttr>(attr).getInt() == 1)
...
toolchain::SmallBitVector SubViewOp::getDroppedDims() {
  MemRefType sourceType = getSourceType();
  MemRefType resultType = getType();
  FailureOr<toolchain::SmallBitVector> unusedDims =
...
OpFoldResult DimOp::fold(FoldAdaptor adaptor) {
  ...
  auto memrefType = toolchain::dyn_cast<MemRefType>(getSource().getType());
  if (!memrefType)
    return {};
  ...
  if (auto subview = dyn_cast_or_null<SubViewOp>(definingOp)) {
    toolchain::SmallBitVector unusedDims = subview.getDroppedDims();
    unsigned resultIndex = 0;
    unsigned sourceRank = subview.getSourceType().getRank();
    unsigned sourceIndex = 0;
    for (auto i : toolchain::seq<unsigned>(0, sourceRank)) {
...
LogicalResult DmaStartOp::verify() {
  ...
  // 1. Source memref.
  if (!toolchain::isa<MemRefType>(getSrcMemRef().getType()))
  ...
  if (!getDstIndices().empty() &&
      !toolchain::all_of(getDstIndices().getTypes(),
                    [](Type t) { return t.isIndex(); }))
  ...
  // 4. Tag memref.
  if (!toolchain::isa<MemRefType>(getTagMemRef().getType()))
  ...
  if (!getTagIndices().empty() &&
      !toolchain::all_of(getTagIndices().getTypes(),
                    [](Type t) { return t.isIndex(); }))
...
LogicalResult ExtractStridedMetadataOp::inferReturnTypes(
    ...
    SmallVectorImpl<Type> &inferredReturnTypes) {
  auto sourceType = toolchain::dyn_cast<MemRefType>(adaptor.getSource().getType());
  if (!sourceType)
    return failure();
...
static bool replaceConstantUsesOf(OpBuilder &rewriter, Location loc,
  ...
  for (auto [maybeConstant, result] : toolchain::zip(maybeConstants, values)) {
    ...
    Value constantVal = arith::ConstantIndexOp::create(
        rewriter, loc,
        toolchain::cast<IntegerAttr>(cast<Attribute>(maybeConstant)).getInt());
    for (Operation *op : toolchain::make_early_inc_range(result.getUsers())) {
...
void GenericAtomicRMWOp::build(OpBuilder &builder, OperationState &result,
  ...
  if (auto memrefType = toolchain::dyn_cast<MemRefType>(memref.getType())) {
...
ParseResult GenericAtomicRMWOp::parse(OpAsmParser &parser,
  ...
  if (parser.parseRegion(*body, {}) ||
    ...
    return failure();
  result.types.push_back(toolchain::cast<MemRefType>(memrefType).getElementType());
  return success();
}
...
parseGlobalMemrefOpTypeAndInitialValue(OpAsmParser &parser, TypeAttr &typeAttr,
  ...
  auto memrefType = toolchain::dyn_cast<MemRefType>(type);
  ...
  if (parser.parseAttribute(initialValue, tensorType))
    return failure();
  if (!toolchain::isa<ElementsAttr>(initialValue))
    return parser.emitError(parser.getNameLoc())
...
LogicalResult GlobalOp::verify() {
  auto memrefType = toolchain::dyn_cast<MemRefType>(getType());
  if (!memrefType || !memrefType.hasStaticShape())
  ...
  if (getInitialValue().has_value()) {
    ...
    if (!toolchain::isa<UnitAttr>(initValue) && !toolchain::isa<ElementsAttr>(initValue))
    ...
    // the global variable.
    if (auto elementsAttr = toolchain::dyn_cast<ElementsAttr>(initValue)) {
...
ElementsAttr GlobalOp::getConstantInitValue() {
  ...
  if (getConstant() && initVal.has_value())
    return toolchain::cast<ElementsAttr>(initVal.value());
  return {};
}
...
void ReinterpretCastOp::build(OpBuilder &b, OperationState &result,
  ...
  SmallVector<OpFoldResult> sizeValues =
      toolchain::to_vector<4>(toolchain::map_range(sizes, [&](int64_t v) -> OpFoldResult {
        return b.getI64IntegerAttr(v);
      }));
  SmallVector<OpFoldResult> strideValues = toolchain::to_vector<4>(
      toolchain::map_range(strides, [&](int64_t v) -> OpFoldResult {
        return b.getI64IntegerAttr(v);
      }));
...
LogicalResult ReinterpretCastOp::verify() {
  ...
  auto srcType = toolchain::cast<BaseMemRefType>(getSource().getType());
  auto resultType = toolchain::cast<MemRefType>(getType());
  ...
  for (auto [idx, resultSize, expectedSize] :
       toolchain::enumerate(resultType.getShape(), getStaticSizes())) {
  ...
  for (auto [idx, resultStride, expectedStride] :
       toolchain::enumerate(resultStrides, getStaticStrides())) {
...
OpFoldResult ReinterpretCastOp::fold(FoldAdaptor /*operands*/) {
  ...
  auto getPrevSrc = [&]() -> Value {
    ...
    if (auto prev = src.getDefiningOp<SubViewOp>())
      if (toolchain::all_of(prev.getMixedOffsets(), isZeroInteger))
        return prev.getSource();
...
public:
  ...
  LogicalResult matchAndRewrite(ReinterpretCastOp op,
    ...
    auto isReinterpretCastNoop = [&]() -> bool {
      ...
      if (!toolchain::equal(extractStridedMetadata.getConstifiedMixedStrides(),
                       op.getConstifiedMixedStrides()))
        return false;

      // Second, check the sizes.
      if (!toolchain::equal(extractStridedMetadata.getConstifiedMixedSizes(),
...
public:
  ...
  LogicalResult matchAndRewrite(ReinterpretCastOp op,
                                PatternRewriter &rewriter) const override {
    unsigned srcStaticCount = toolchain::count_if(
        toolchain::concat<OpFoldResult>(op.getMixedOffsets(), op.getMixedSizes(),
                                   op.getMixedStrides()),
    ...
    if (srcStaticCount ==
        toolchain::count_if(toolchain::concat<OpFoldResult>(offsets, sizes, strides),
                       [](OpFoldResult ofr) { return isa<Attribute>(ofr); }))
      return failure();
...
verifyCollapsedShape(Operation *op, ArrayRef<int64_t> collapsedShape,
  ...
  int64_t nextDim = 0;
  for (const auto &it : toolchain::enumerate(reassociation)) {
...
computeExpandedLayoutMap(MemRefType srcType, ArrayRef<int64_t> resultShape,
  ...
  for (auto it : toolchain::reverse(toolchain::zip(reassociation, srcStrides))) {
  ...
  auto resultStrides = toolchain::to_vector<8>(toolchain::reverse(reverseResultStrides));
...
void ExpandShapeOp::build(OpBuilder &builder, OperationState &result,
  ...
  build(builder, result, toolchain::cast<MemRefType>(resultType), src,
...
void ExpandShapeOp::build(OpBuilder &builder, OperationState &result,
  ...
  MemRefType memrefResultTy = toolchain::cast<MemRefType>(resultType);
...
void ExpandShapeOp::build(OpBuilder &builder, OperationState &result,
  ...
  auto srcType = toolchain::cast<MemRefType>(src.getType());
...
LogicalResult ExpandShapeOp::verify() {
  ...
  for (auto [pos, shape] : toolchain::enumerate(resShape)) {
...
public:
  ...
  LogicalResult matchAndRewrite(ExpandShapeOp op,
    ...
    auto castSourceType = toolchain::cast<MemRefType>(castSource.getType());
    SmallVector<ReassociationIndices> reassociationIndices =
        op.getReassociationIndices();
    for (auto [idx, group] : toolchain::enumerate(reassociationIndices)) {
      auto newOutputShapeSizesSlice =
          ArrayRef(newOutputShapeSizes).slice(group.front(), group.size());
      bool newOutputDynamic =
          toolchain::is_contained(newOutputShapeSizesSlice, ShapedType::kDynamic);
...
computeCollapsedLayoutMap(MemRefType srcType,
  ...
  for (const ReassociationIndices &reassoc : reassociation) {
    ArrayRef<int64_t> ref = toolchain::ArrayRef(reassoc);
  ...
  for (const ReassociationIndices &reassoc : toolchain::reverse(reassociation)) {
    ...
    for (int64_t idx : toolchain::reverse(trailingReassocs)) {
...
public:
  ...
  LogicalResult matchAndRewrite(CollapseShapeOp op,
    ...
    Type newResultType = CollapseShapeOp::computeCollapsedType(
        toolchain::cast<MemRefType>(cast.getOperand().getType()),
        op.getReassociationIndices());
...
LogicalResult ReshapeOp::verify() {
  ...
  Type operandElementType =
      toolchain::cast<ShapedType>(operandType).getElementType();
  Type resultElementType = toolchain::cast<ShapedType>(resultType).getElementType();
  if (operandElementType != resultElementType)
    return emitOpError("element types of source and destination memref "
                       "types should be the same");

  if (auto operandMemRefType = toolchain::dyn_cast<MemRefType>(operandType))
    if (!operandMemRefType.getLayout().isIdentity())
      return emitOpError("source memref type should have identity affine map");

  int64_t shapeSize =
      toolchain::cast<MemRefType>(getShape().getType()).getDimSize(0);
  auto resultMemRefType = toolchain::dyn_cast<MemRefType>(resultType);
  if (resultMemRefType) {
...
MemRefType SubViewOp::inferResultType(MemRefType sourceMemRefType,
  ...
  for (auto it : toolchain::zip(staticOffsets, sourceStrides)) {
  ...
  for (auto it : toolchain::zip(sourceStrides, staticStrides)) {
...
MemRefType SubViewOp::inferRankReducedResultType(
  ...
  std::optional<toolchain::SmallDenseSet<unsigned>> dimsToProject =
  ...
  auto inferredLayout = toolchain::cast<StridedLayoutAttr>(inferredType.getLayout());
  SmallVector<int64_t> rankReducedStrides;
  rankReducedStrides.reserve(resultShape.size());
  for (auto [idx, value] : toolchain::enumerate(inferredLayout.getStrides())) {
...
void SubViewOp::build(OpBuilder &b, OperationState &result,
  ...
  dispatchIndexOpFoldResults(sizes, dynamicSizes, staticSizes);
  dispatchIndexOpFoldResults(strides, dynamicStrides, staticStrides);
  auto sourceMemRefType = toolchain::cast<MemRefType>(source.getType());
...
void SubViewOp::build(OpBuilder &b, OperationState &result, Value source,
  ...
  SmallVector<OpFoldResult> offsetValues = toolchain::to_vector<4>(
      toolchain::map_range(offsets, [&](int64_t v) -> OpFoldResult {
        return b.getI64IntegerAttr(v);
      }));
  SmallVector<OpFoldResult> sizeValues =
      toolchain::to_vector<4>(toolchain::map_range(sizes, [&](int64_t v) -> OpFoldResult {
...
static bool haveCompatibleStrides(MemRefType t1, MemRefType t2,
                                  const toolchain::SmallBitVector &droppedDims) {
...
static LogicalResult produceSubViewErrorMsg(SliceVerificationResult result,
  ...
  auto memrefType = toolchain::cast<ShapedType>(expectedType);
  switch (result) {
...
static MemRefType getCanonicalSubViewResultType(
  ...
  FailureOr<toolchain::SmallBitVector> unusedDims = computeMemRefRankReductionMask(
      currentSourceType, currentResultType, mixedSizes);
  if (failed(unusedDims))
    return nullptr;

  auto layout = toolchain::cast<StridedLayoutAttr>(nonRankReducedType.getLayout());
  ...
  for (const auto &[idx, size, stride] :
       toolchain::zip(toolchain::seq<unsigned>(0, nonRankReducedType.getRank()),
...
Value mlir::memref::createCanonicalRankReducingSubViewOp(
  ...
  auto memrefType = toolchain::cast<MemRefType>(memref.getType());
...
FailureOr<Value> SubViewOp::rankReduceIfNeeded(OpBuilder &b, Location loc,
  ...
  auto sourceMemrefType = toolchain::dyn_cast<MemRefType>(value.getType());
...
static bool isTrivialSubViewOp(SubViewOp subViewOp) {
  ...
  if (toolchain::any_of(mixedOffsets, [](OpFoldResult ofr) {
  ...
  if (toolchain::any_of(mixedStrides, [](OpFoldResult ofr) {
        std::optional<int64_t> intValue = getConstantIntValue(ofr);
  ...
  for (const auto &size : toolchain::enumerate(mixedSizes)) {
...
public:
  ...
  LogicalResult matchAndRewrite(SubViewOp subViewOp,
    ...
    // in.
    if (toolchain::any_of(subViewOp.getOperands(), [](Value operand) {
    ...
    auto resultType = getCanonicalSubViewResultType(
        ...
        toolchain::cast<MemRefType>(castOp.getSource().getType()),
...
OpFoldResult SubViewOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto srcSubview = getViewSource().getDefiningOp<SubViewOp>()) {
    ...
    bool allOffsetsZero = toolchain::all_of(offsets, isZeroInteger);
    auto strides = getMixedStrides();
    bool allStridesOne = toolchain::all_of(strides, isOneInteger);
    bool allSizesSame = toolchain::equal(sizes, srcSizes);
...
void TransposeOp::build(OpBuilder &b, OperationState &result, Value in,
  ...
  auto memRefType = toolchain::cast<MemRefType>(in.getType());
  // Compute result type.
...
LogicalResult TransposeOp::verify() {
  ...
  auto srcType = toolchain::cast<MemRefType>(getIn().getType());
  auto resultType = toolchain::cast<MemRefType>(getType());
...
LogicalResult ViewOp::verify() {
  auto baseType = toolchain::cast<MemRefType>(getOperand(0).getType());
...
struct ViewOpShapeFolder : public OpRewritePattern<ViewOp> {
  ...
  LogicalResult matchAndRewrite(ViewOp viewOp,
    ...
    if (toolchain::none_of(viewOp.getOperands(), [](Value operand) {
          return matchPattern(operand, matchConstantIndex());
        }))
      return failure();
...
LogicalResult AtomicRMWOp::verify() {
  ...
  case arith::AtomicRMWKind::mulf:
    if (!toolchain::isa<FloatType>(getValue().getType()))
  ...
  case arith::AtomicRMWKind::andi:
    if (!toolchain::isa<IntegerType>(getValue().getType()))
      return emitOpError() << "with kind '"
                           << arith::stringifyAtomicRMWKind(getKind())
