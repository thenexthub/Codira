...
bool ConstantOp::isBuildableWith(Attribute value, Type type) {
  if (auto arrAttr = toolchain::dyn_cast<ArrayAttr>(value)) {
    auto complexTy = toolchain::dyn_cast<ComplexType>(type);
    if (!complexTy || arrAttr.size() != 2)
      return false;
    auto complexEltTy = complexTy.getElementType();
    if (auto fre = toolchain::dyn_cast<FloatAttr>(arrAttr[0])) {
      auto im = toolchain::dyn_cast<FloatAttr>(arrAttr[1]);
      return im && fre.getType() == complexEltTy &&
             im.getType() == complexEltTy;
    }
    if (auto ire = toolchain::dyn_cast<IntegerAttr>(arrAttr[0])) {
      auto im = toolchain::dyn_cast<IntegerAttr>(arrAttr[1]);
...
LogicalResult ConstantOp::verify() {
  ...
  auto re = toolchain::dyn_cast<TypedAttr>(arrayAttr[0]);
  auto im = toolchain::dyn_cast<TypedAttr>(arrayAttr[1]);
...
OpFoldResult ImOp::fold(FoldAdaptor adaptor) {
  ArrayAttr arrayAttr =
      toolchain::dyn_cast_if_present<ArrayAttr>(adaptor.getComplex());
...
OpFoldResult AddOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto constantOp = getRhs().getDefiningOp<ConstantOp>()) {
    ...
    if (toolchain::cast<FloatAttr>(arrayAttr[0]).getValue().isZero() &&
        toolchain::cast<FloatAttr>(arrayAttr[1]).getValue().isZero()) {
      return getLhs();
    }
  }
