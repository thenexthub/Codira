...
static Type parseUniformType(DialectAsmParser &parser) {
  ...
  if (isSubChannel) {
    SmallVector<APFloat> apFloatScales =
        toolchain::to_vector(toolchain::map_range(scales, [&](double scale) -> APFloat {
    ...
    SmallVector<APInt> apIntZeroPoints = toolchain::to_vector(
        toolchain::map_range(zeroPoints, [&](int64_t zeroPoint) -> APInt {
...
printBlockSizeInfo(ArrayRef<std::pair<int32_t, int64_t>> blockSizeInfo,
  ...
  toolchain::interleaveComma(
      toolchain::seq<size_t>(0, blockSizeInfo.size()), out, [&](size_t index) {
...
static void printUniformQuantizedPerAxisType(UniformQuantizedPerAxisType type,
  ...
  toolchain::interleave(
      toolchain::seq<size_t>(0, scales.size()), out,
      [&](size_t index) {
...
void QuantDialect::printType(Type type, DialectAsmPrinter &os) const {
  if (auto anyType = toolchain::dyn_cast<AnyQuantizedType>(type))
    printAnyQuantizedType(anyType, os);
  else if (auto uniformType = toolchain::dyn_cast<UniformQuantizedType>(type))
    printUniformQuantizedType(uniformType, os);
  else if (auto perAxisType = toolchain::dyn_cast<UniformQuantizedPerAxisType>(type))
    printUniformQuantizedPerAxisType(perAxisType, os);
  else if (auto perAxisType =
               toolchain::dyn_cast<UniformQuantizedSubChannelType>(type))
    printUniformQuantizedSubChannelType(perAxisType, os);
  else if (auto calibratedType = toolchain::dyn_cast<CalibratedQuantizedType>(type))
