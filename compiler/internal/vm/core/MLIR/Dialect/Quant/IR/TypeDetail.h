...
struct AnyQuantizedTypeStorage : public QuantizedTypeStorage {
  struct KeyTy {
    ...
    unsigned getHashValue() const {
      return toolchain::hash_combine(flags, storageType, expressedType,
...
struct UniformQuantizedTypeStorage : public QuantizedTypeStorage {
  struct KeyTy {
    ...
    unsigned getHashValue() const {
      int64_t scaleBits = toolchain::bit_cast<int64_t>(scale);
      return toolchain::hash_combine(flags, storageType, expressedType, scaleBits,
...
struct UniformQuantizedPerAxisTypeStorage : public QuantizedTypeStorage {
  struct KeyTy {
    ...
    unsigned getHashValue() const {
      int64_t *scalesCast = toolchain::bit_cast<int64_t *>(scales.data());
      ArrayRef<int64_t> scalesBits(scalesCast, scales.size());
      return toolchain::hash_combine(flags, storageType, expressedType,
                                toolchain::hash_combine_range(scalesBits),
                                toolchain::hash_combine_range(zeroPoints),
                                storageTypeMin, storageTypeMax);
    }
  };
...
struct UniformQuantizedSubChannelTypeStorage : public QuantizedTypeStorage {
  struct KeyTy {
    ...
    unsigned getHashValue() const {
      ...
      unsigned hash = toolchain::hash_combine(flags, storageType, expressedType,
                                         storageTypeMin, storageTypeMax);

      // Hash the scales.
      for (auto scaleAttr : scales.getValues<APFloat>()) {
        hash = toolchain::hash_combine(
            hash, toolchain::bit_cast<int64_t>(scaleAttr.convertToDouble()));
      }

      // Hash the zero points.  (Assumed to be integers, adjust if needed).
      for (auto zeroPointAttr : zeroPoints.getValues<APInt>()) {
        hash = toolchain::hash_combine(hash, zeroPointAttr.getSExtValue());
      }

      // Hash the quantized dimensions and block sizes.
      hash = toolchain::hash_combine(hash,
                                toolchain::hash_combine_range(quantizedDimensions),
                                toolchain::hash_combine_range(blockSizes));
...
struct CalibratedQuantizedTypeStorage : public QuantizedTypeStorage {
  struct KeyTy {
    ...
    unsigned getHashValue() const {
      int64_t minBits = toolchain::bit_cast<double>(min);
      int64_t maxBits = toolchain::bit_cast<double>(max);
      return toolchain::hash_combine(expressedType, minBits, maxBits);
    }
  };
