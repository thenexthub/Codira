...
bool QuantizedType::classof(Type type) {
  return toolchain::isa<QuantDialect>(type.getDialect());
}
...
QuantizedType::verifyInvariants(function_ref<InFlightDiagnostic()> emitError,
  ...
  auto intStorageType = toolchain::dyn_cast<IntegerType>(storageType);
  if (!intStorageType)
...
bool QuantizedType::isCompatibleExpressedType(Type candidateExpressedType) {
  if (toolchain::isa<ShapedType>(candidateExpressedType)) {
    return toolchain::cast<ShapedType>(candidateExpressedType).getElementType() ==
           getExpressedType();
  }
...
QuantizedType::getQuantizedElementType(Type primitiveOrContainerType) {
  if (toolchain::isa<ShapedType>(primitiveOrContainerType)) {
    Type elementType =
        toolchain::cast<ShapedType>(primitiveOrContainerType).getElementType();
    return toolchain::dyn_cast<QuantizedType>(elementType);
  }
  return toolchain::dyn_cast<QuantizedType>(primitiveOrContainerType);
}
...
Type QuantizedType::castFromStorageType(Type candidateType) {
  ...
  if (toolchain::isa<RankedTensorType>(candidateType)) {
    // i.e. tensor<4xi8> -> tensor<4x!quant<"uniform[i8:f32]{1.0}">>
    return RankedTensorType::get(
        toolchain::cast<RankedTensorType>(candidateType).getShape(),
        getStorageType());
  }
  if (toolchain::isa<UnrankedTensorType>(candidateType)) {
    // i.e. tensor<xi8> -> tensor<x!quant<"uniform[i8:f32]{1.0}">>
    return UnrankedTensorType::get(getStorageType());
  }
  if (toolchain::isa<VectorType>(candidateType)) {
    // i.e. vector<4xi8> -> vector<4x!quant<"uniform[i8:f32]{1.0}">>
    return VectorType::get(toolchain::cast<VectorType>(candidateType).getShape(),
                           getStorageType());
  }
...
Type QuantizedType::castToStorageType(Type quantizedType) {
  if (toolchain::isa<QuantizedType>(quantizedType)) {
    // i.e. quant<"uniform[i8:f32]{1.0}"> -> i8
    return toolchain::cast<QuantizedType>(quantizedType).getStorageType();
  }
  if (toolchain::isa<ShapedType>(quantizedType)) {
    // i.e. tensor<4xi8> -> tensor<4x!quant<"uniform[i8:f32]{1.0}">>
    ShapedType sType = toolchain::cast<ShapedType>(quantizedType);
    if (!toolchain::isa<QuantizedType>(sType.getElementType())) {
      return nullptr;
    }
    Type storageType =
        toolchain::cast<QuantizedType>(sType.getElementType()).getStorageType();
    if (toolchain::isa<RankedTensorType>(quantizedType)) {
      return RankedTensorType::get(sType.getShape(), storageType);
    }
    if (toolchain::isa<UnrankedTensorType>(quantizedType)) {
      return UnrankedTensorType::get(storageType);
    }
    if (toolchain::isa<VectorType>(quantizedType)) {
...
Type QuantizedType::castFromExpressedType(Type candidateType) {
  ...
  if (toolchain::isa<ShapedType>(candidateType)) {
    ShapedType candidateShapedType = toolchain::cast<ShapedType>(candidateType);
...
Type QuantizedType::castToExpressedType(Type quantizedType) {
  if (toolchain::isa<QuantizedType>(quantizedType)) {
    ...
    return toolchain::cast<QuantizedType>(quantizedType).getExpressedType();
  }
  if (toolchain::isa<ShapedType>(quantizedType)) {
    // i.e. tensor<4xi8> -> tensor<4x!quant<"uniform[i8:f32]{1.0}">>
    ShapedType sType = toolchain::cast<ShapedType>(quantizedType);
    ...
    Type expressedType =
        toolchain::cast<QuantizedType>(sType.getElementType()).getExpressedType();
    if (toolchain::isa<RankedTensorType>(quantizedType)) {
...
AnyQuantizedType::verifyInvariants(function_ref<InFlightDiagnostic()> emitError,
  ...
  // extended.
  if (expressedType && !toolchain::isa<FloatType>(expressedType))
...
LogicalResult UniformQuantizedType::verifyInvariants(
  ...
  // If this restriction is ever eliminated, the parser/printer must be
  // extended.
  if (!toolchain::isa<FloatType>(expressedType))
    return emitError() << "expressed type must be floating point";
...
UniformQuantizedSubChannelType::getBlockSizeInfo() const {
  ...
  for (auto [dim, size] :
       toolchain::zip(getQuantizedDimensions(), getBlockSizes())) {
