...
FailureOr<bool> PaddedShape::initialize(linalg::LinalgOp opToPad,
  ...
  toolchain::append_range(shape, opToPad.getShape(opOperand));
...
void PaddedShape::computePadding(OpBuilder &builder, Value operand) {
  ...
  for (auto &&[i, dim] : toolchain::enumerate(shape)) {
...
linalg::rewriteAsPaddedOp(RewriterBase &rewriter, LinalgOp opToPad,
  ...
  if (options.paddingValues.empty()) {
    ...
    toolchain::append_range(types, opToPad->getResultTypes());
    for (Type t : types) {
  ...
  for (const auto &en : toolchain::enumerate(paddedOp->getResults())) {
  ...
  for (auto it :
       toolchain::zip(paddedSubtensorResults, opToPad.getDpsInitsMutable())) {
...
mlir::linalg::padAndHoistLinalgOp(RewriterBase &rewriter, LinalgOp linalgOp,
  ...
  for (const auto &en : enumerate(options.hoistPaddings)) {
    ...
    if (toolchain::any_of(paddedOp.getShape(&opOperand), ShapedType::isDynamic)) {
