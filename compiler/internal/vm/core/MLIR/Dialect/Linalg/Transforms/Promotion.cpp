...
static Value allocBuffer(ImplicitLocOpBuilder &b,
  ...
  toolchain::TypeSize width = layout.getTypeSize(elementType);
...
static std::optional<Value> defaultAllocBufferCallBack(
  ...
  Value allocSize = one;
  for (const auto &size : toolchain::enumerate(boundingSubViewSize))
...
struct LinalgOpInstancePromotionOptions {
  ...
  toolchain::SmallSet<int64_t, 4> operandsNumbersToCopyIn;
...
LinalgOpInstancePromotionOptions::LinalgOpInstancePromotionOptions(
  ...
  auto vUseFullTileBuffers =
      options.useFullTileBuffers.value_or(toolchain::SmallBitVector());
...
FailureOr<PromotionInfo> mlir::linalg::promoteSubviewAsNewBuffer(
  ...
  toolchain::SmallBitVector droppedDims = subView.getDroppedDims();
  int64_t resultDimIdx = 0;
  for (const auto &en : toolchain::enumerate(subView.getOrCreateRanges(b, loc))) {
    if (droppedDims[en.index()])
      continue;
    auto rangeValue = en.value();
    // Try to extract a tight constant. If the size is known statically, no need
    // to look for the bound.
    LLVM_DEBUG(toolchain::dbgs() << "Extract tightest: " << rangeValue.size << "\n");
    Value size;
    if (toolchain::isa_and_present<Attribute>(rangeValue.size) ||
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Extracted tightest: " << size << "\n");
...
promoteSubViews(ImplicitLocOpBuilder &b,
  ...
  for (auto v : options.subViews) {
    ...
    Value fillVal =
        toolchain::TypeSwitch<Type, Value>(subviewEltType)
            .Case([&](FloatType t) {
