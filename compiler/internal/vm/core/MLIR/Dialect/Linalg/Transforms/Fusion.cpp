...
getShapeDefiningLoopRange(LinalgOp op, unsigned loopDepth,
  ...
  for (OpOperand &opOperand : op->getOpOperands()) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "getShapeDefiningLoopRange I/O idx: "
    ...
    for (const auto &en : toolchain::enumerate(map.getResults())) {
      ...
      if (loopDepth == cast<AffineDimExpr>(en.value()).getPosition()) {
        LLVM_DEBUG(toolchain::dbgs() << "getShapeDefiningLoopRange loopDepth: "
                                << loopDepth << "\n");
        LLVM_DEBUG(toolchain::dbgs() << "getShapeDefiningLoopRange shape: "
                                << opOperand.get() << "\n");
...
static LinalgOp fuse(OpBuilder &b, LinalgOp producer,
  ...
  for (unsigned i = 0, e = producer.getNumLoops(); i < e; ++i) {
    ...
    if (it != fusedLoopsAndRanges.end()) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "tiled loop#" << i << " with LoopRange "
                              << loopRanges.back() << "\n");
    } else {
      tileSizes.push_back(b.getIndexAttr(0));
      loopRanges.push_back(Range{b.getIndexAttr(0), dim, b.getIndexAttr(1)});
      LLVM_DEBUG(toolchain::dbgs() << "full loop#" << i << " with LoopRange "
                              << loopRanges.back() << "\n");
    }
  }
  ...
  SmallVector<OpFoldResult> allIvs = toolchain::to_vector(
      toolchain::map_range(loopRanges, [&](Range range) { return range.offset; }));
...
static LinalgOp fuse(OpBuilder &b, LinalgOp producerOp, AffineMap producerMap,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Producer map: " << producerMap << "\n");
  DenseMap<unsigned, Range> fusedLoopsAndRanges;
  Value shapedOperand = consumerOpOperand.get();
  for (const auto &en : toolchain::enumerate(producerMap.getResults())) {
...
mlir::linalg::fuseProducerOfTensor(OpBuilder &b, OpResult producerOpResult,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Fuse into consumer: " << *consumerOp << "\n");
  OpOperand *opOperand =
  ...
  if (cast<ShapedType>(consumerType).getRank() !=
    ...
    toolchain::SmallBitVector droppedDims = sliceOp.getDroppedDims();
    def =
