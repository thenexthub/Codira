...
getTransposeOpPermutation(linalg::LinalgOp linalgOp) {
  ...
  if (!toolchain::hasSingleElement(linalgOp.getBlock()->getOperations()))
    return failure();
  ...
  return toolchain::map_to_vector(outMap.getResults(),
...
public:
  ...
  LogicalResult matchAndRewrite(PackOp packOp,
    ...
    for (auto [pos, tileSize, high] :
         toolchain::zip_equal(packOp.getInnerDimsPos(), packOp.getStaticInnerTiles(),
...
public:
  ...
  LogicalResult matchAndRewrite(PackOp packOp,
    ...
    SmallVector<int64_t> newInnerDimsPosVec;
    SmallVector<int64_t> newOuterDimsPermVec =
        toolchain::to_vector(transposePermutation);
