...
struct PackInfo {
  ...
  toolchain::DenseMap<int64_t, OpFoldResult> domainDimAndTileMapping;
  // The mapping from a dimension of iteration domain to the corresponding inner
  // tiling dimension on iteration domain.
  toolchain::DenseMap<int64_t, int64_t> tileToPointMapping;
...
getPackingInfoFromOperand(OpOperand *opOperand, linalg::GenericOp genericOp,
                          OpTy packOrUnPackOp) {
  static_assert(toolchain::is_one_of<OpTy, linalg::PackOp, linalg::UnPackOp>::value,
                "applies to only pack or unpack operations");
  LLVM_DEBUG(
      { toolchain::dbgs() << "--- Construct PackInfo From an operand ---\n"; });
  ...
  for (auto [index, innerDimPos, tileSize] :
       toolchain::zip_equal(toolchain::seq<unsigned>(0, innerDimsPos.size()),
    ...
    LLVM_DEBUG({
      toolchain::dbgs() << "map innerDimPos=" << innerDimPos
  ...
  auto areAllAffineDimExpr = [&](int dim) {
    for (AffineMap map : indexingMaps) {
      if (toolchain::any_of(map.getResults(), [dim](AffineExpr expr) {
  ...
  for (auto [index, dim] : toolchain::enumerate(packOrUnPackOp.getOuterDimsPerm())) {
  ...
  if (!permutedOuterDims.empty()) {
    ...
    toolchain::DenseSet<int64_t> permutedDomainDims(permutedOuterDims.begin(),
    ...
    for (int i = 0, e = indexingMap.getNumDims(); i < e; i++)
      packInfo.outerDimsOnDomainPerm.push_back(
          permutedDomainDims.contains(i) ? permutedOuterDims[outerDimIndex++]
                                         : i);
    LLVM_DEBUG({
      toolchain::dbgs() << "map outer dimsDimsPerm to ";
...
static SmallVector<int64_t> computeOuterDims(ArrayRef<int64_t> perm,
  ...
  for (auto [pos, expr] : toolchain::enumerate(exprs)) {
...
static bool getPackedOperandDetails(
  ...
  toolchain::DenseMap<int64_t, int64_t> domainDimToOperandDim;
  ...
  for (auto [index, expr] : toolchain::enumerate(exprs)) {
    if (auto dimExpr = dyn_cast<AffineDimExpr>(expr)) {
  ...
  if (!packInfo.outerDimsOnDomainPerm.empty()) {
    ...
    for (auto i : toolchain::seq<unsigned>(0, origIndexingMap.getNumResults())) {
  ...
  auto maybeIntInnerTileSizes =
      toolchain::map_to_vector(innerTileSizes, [](OpFoldResult ofr) -> int64_t {
...
packGenericOp(RewriterBase &rewriter, GenericOp genericOp, Value dest,
  ...
  auto hasEquivalentTiles = [](PackOp packOp, UnPackOp unPackOp) {
    return packOp.getOuterDimsPerm() == unPackOp.getOuterDimsPerm() &&
           ...
           toolchain::equal(packOp.getMixedTiles(), unPackOp.getMixedTiles());
  };
  DenseMap<OpOperand *, PackedOperandDetails> packedOperandMap;
...
static bool isGenericOutsNotUsed(linalg::GenericOp genericOp) {
  return toolchain::all_of(genericOp.getDpsInitsMutable(), [&](OpOperand &operand) {
...
public:
  ...
  LogicalResult matchAndRewrite(linalg::PackOp packOp,
    ...
    toolchain::SmallBitVector paddedDims = padOp.getPaddedDims();
    toolchain::SmallBitVector innerDims(paddedDims.size());
...
projectToInnerMostNonUnitDimsPos(ArrayRef<int64_t> dimsPos,
  ...
  for (auto pos : dimsPos) {
    ...
    for (auto i : toolchain::reverse(reassocIndices[pos])) {
...
static bool isDimsDivisibleByTileSizes(ArrayRef<int64_t> dimsPos,
  ...
  for (auto [pos, tileSize] : toolchain::zip_equal(dimsPos, tileSizes)) {
...
bubbleUpPackOpThroughCollapseShape(tensor::CollapseShapeOp collapseOp,
  ...
  for (auto outerPos : outerDimsPerm)
    toolchain::append_range(newOuterDimsPerm, reassocIndices[outerPos]);
...
projectDimsPosIntoReassocPos(ArrayRef<int64_t> dimsPos,
  ...
  for (auto pos : dimsPos) {
    for (auto [idx, indices] : toolchain::enumerate(reassocIndices)) {
      ...
      // dimension position.
      if (toolchain::is_contained(indices, pos)) {
...
bubbleUpPackOpThroughExpandShape(tensor::ExpandShapeOp expandOp,
  ...
  toolchain::SetVector<int64_t> packDimsPos(toolchain::from_range, packInnerDims);

  for (auto [idx, indices] : toolchain::enumerate(reassoc)) {
    // For each expand_shape reassociation, figure out which dimensions get
    // packed if any.
    toolchain::SetVector<int64_t> expandDimPos(toolchain::from_range, indices);
    toolchain::SetVector<int64_t> packedDims =
        toolchain::set_intersection(packDimsPos, expandDimPos);
...
public:
  ...
  LogicalResult matchAndRewrite(linalg::PackOp packOp,
    ...
    if (toolchain::any_of(packOp.getStaticTiles(), ShapedType::isDynamic))
      return failure();
...
public:
  ...
  LogicalResult matchAndRewrite(linalg::UnPackOp unPackOp,
    ...
    // Currently only support static inner tile sizes.
    if (toolchain::any_of(unPackOp.getStaticTiles(), ShapedType::isDynamic))
      return failure();
...
static FailureOr<toolchain::DenseMap<int64_t, SliceDimInfo>>
getPartialSliceDimInfo(GenericOp genericOp, OpOperand *sliceOperand) {
  ...
  toolchain::DenseMap<int64_t, SliceDimInfo> partialSliceDimMap;
  ...
  for (auto [idx, expr] : toolchain::enumerate(
  ...
  for (OpOperand &operand : genericOp->getOpOperands()) {
    ...
    if (toolchain::any_of(IndexingMap.getResults(), [&](AffineExpr expr) {
          if (isa<AffineDimExpr>(expr)) {
            return false;
          }
...
pushDownExtractSliceOpThroughGenericOp(RewriterBase &rewriter,
  ...
  bool hasPartialReductionDimSlice =
      toolchain::any_of(partialSliceDimMap, [&](const auto &slice) {
  ...
  for (auto [idx, operand] : toolchain::enumerate(genericOp.getDpsInputOperands())) {
    ...
    SmallVector<OpFoldResult> operandHighPads(IndexingMap.getNumResults(),
                                              getAsIndexOpFoldResult(ctx, 0));
    for (auto [idx, expr] : toolchain::enumerate(IndexingMap.getResults())) {
      if (!isa<AffineDimExpr>(expr)) {
        continue;
      }
  ...
  auto outputShapeType =
      toolchain::cast<ShapedType>(genericOp.getDpsInitOperand(0)->get().getType());
  SmallVector<OpFoldResult> OutputShape = toolchain::map_to_vector(
  ...
  for (auto [idx, expr] : toolchain::enumerate(outputIndexingMap.getResults())) {
    if (!isa<AffineDimExpr>(expr)) {
      continue;
    }
    AffineDimExpr dimExpr = cast<AffineDimExpr>(expr);
