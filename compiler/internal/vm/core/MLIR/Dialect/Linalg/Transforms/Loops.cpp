...
getInputAndOutputIndices(OpBuilder &b, Location loc, ArrayRef<Value> allIvs,
  ...
  auto maps = toolchain::to_vector<8>(
      toolchain::map_range(mapsRange, [](AffineMapAttr a) { return a.getValue(); }));
...
static void replaceIndexOpsByInductionVariables(RewriterBase &rewriter,
  ...
  for (Operation *loopOp : loopOps) {
    toolchain::TypeSwitch<Operation *>(loopOp)
  ...
  if (!loopOps.empty()) {
    ...
    for (Region *r : loopOp.getLoopRegions())
      for (IndexOp indexOp : toolchain::make_early_inc_range(r->getOps<IndexOp>()))
