...
Value linalg::bufferizeToAllocation(
  ...
  assert(toolchain::range_size(maskOp.getMaskBlock()->without_terminator()) == 1 &&
...
mlir::linalg::rewriteInDestinationPassingStyle(RewriterBase &rewriter,
  ...
  if (padOp.getNofoldAttr() &&
      toolchain::all_of(padOp.getMixedLowPad(), isZeroInteger) &&
      toolchain::all_of(padOp.getMixedHighPad(), isZeroInteger)) {
...
Value linalg::bufferizeToAllocation(
  ...
  auto addOutOfPlaceOperand = [&](OpOperand *operand) {
    if (!toolchain::is_contained(outOfPlaceOperands, operand))
