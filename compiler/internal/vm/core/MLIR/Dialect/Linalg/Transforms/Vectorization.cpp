...
LogicalResult VectorizationState::initState(RewriterBase &rewriter,
  ...
  LDBG() << "Canonical vector shape: " << toolchain::interleaved(canonicalVecShape);
  LDBG() << "Scalable vector dims: " << toolchain::interleaved(scalableVecDims);
...
Value VectorizationState::getOrCreateMaskFor(
  ...
  LDBG() << "Mask shape: " << toolchain::interleaved(maskShape);
  ...
  if (assumeDynamicDimsMatchVecSizes) {
    ...
    if (toolchain::all_of(toolchain::zip(permutedStaticSizes, maskType.getShape()),
                     [](auto it) {
...
VectorizationState::maskOperation(RewriterBase &rewriter, Operation *opToMask,
  ...
  if (!mask) {
    ...
    if (assumeDynamicDimsMatchVecSizes) {
      toolchain::TypeSwitch<Operation *>(opToMask)
  ...
  for (auto [resIdx, resVal] : toolchain::enumerate(opToMask->getResults()))
...
mlir::linalg::getCombinerOpKind(Operation *combinerOp) {
  ...
  if (!combinerOp)
    return std::nullopt;
  return toolchain::TypeSwitch<Operation *, std::optional<CombiningKind>>(combinerOp)
...
static SmallVector<bool> getDimsToReduce(LinalgOp linalgOp) {
  return toolchain::to_vector(
      toolchain::map_range(linalgOp.getIteratorTypesArray(), isReductionIterator));
}
...
static bool hasReductionIterator(LinalgOp &op) {
  return isa<linalg::ReduceOp>(op) ||
         (isa<linalg::GenericOp>(op) &&
          toolchain::any_of(op.getIteratorTypesArray(), isReductionIterator));
}
...
static Value buildVectorWrite(RewriterBase &rewriter, Value value,
  ...
  AffineMap vectorTypeMap = AffineMap::getFilteredIdentityMap(
      ...
      [&](AffineDimExpr dimExpr) -> bool {
        return toolchain::is_contained(opOperandMap.getResults(), dimExpr);
      });
...
vectorizeLinalgYield(RewriterBase &rewriter, Operation *op,
  ...
  for (const auto &output : toolchain::enumerate(yieldOp.getValues())) {
...
static VectorizationHookResult vectorizeLinalgIndex(RewriterBase &rewriter,
  ...
  auto permPattern =
      toolchain::to_vector(toolchain::seq<unsigned>(0, targetShape.size()));
  ...
  SmallVector<int64_t> transposition =
      toolchain::to_vector<16>(toolchain::seq<int64_t>(0, linalgOp.getNumLoops()));
...
tensorExtractVectorizationPrecondition(Operation *op, bool vectorizeNDExtract) {
  ...
  if (!toolchain::all_of(extractOp->getResultTypes(),
...
static uint64_t getTrailingNonUnitLoopDimIdx(LinalgOp linalgOp) {
  ...
  assert(
      (linalgOp.hasDynamicShape() ||
       toolchain::count_if(loopRanges, [](int64_t dim) { return dim != 1; }) == 1) &&
...
static bool isLoopInvariantIdx(LinalgOp &linalgOp, Value &val,
  ...
  assert(((toolchain::count_if(resType.getShape(),
  ...
  if (isa<BlockArgument>(val))
    return !toolchain::is_contained(block->getArguments(), val);
...
getTensorExtractMemoryAccessPattern(tensor::ExtractOp extractOp,
  ...
  bool isOutput1DVector =
      (toolchain::count_if(resType.getShape(),
                      [](int64_t dimSize) { return dimSize > 1; }) == 1);
  ...
  for (auto [i, indexVal] : toolchain::enumerate(leadIndices)) {
...
static bool isMaskTriviallyFoldable(SmallVector<OpFoldResult> &maskSizes,
  ...
  for (auto [i, dimSize] : toolchain::enumerate(maskSizes)) {
  ...
  for (auto [i, idx] : toolchain::enumerate(writeIdxs)) {
    APSInt intVal;
  ...
  for (auto [i, idx] : toolchain::enumerate(cstMaskSizes)) {
...
createWriteOrMaskedWrite(OpBuilder &builder, Location loc, Value vecToStore,
  ...
  if (toolchain::equal(vecToStoreShape, destShape.take_back(vecToStoreRank)))
    return write;
...
static LogicalResult reductionPreconditions(LinalgOp op) {
  if (toolchain::none_of(op.getIteratorTypesArray(), isReductionIterator)) {
...
getConvOperationKind(Operation *reduceOp) {
  int numBlockArguments =
      toolchain::count_if(reduceOp->getOperands(), toolchain::IsaPred<BlockArgument>);
  ...
  case 1: {
    ...
    auto feedValIt = toolchain::find_if_not(reduceOp->getOperands(),
                                       toolchain::IsaPred<BlockArgument>);
    ...
    if (!((isa<arith::MulIOp, arith::MulFOp>(feedOp) ||
          ...
          toolchain::all_of(feedOp->getOperands(), [](Value v) {
...
static LogicalResult vectorizeLinalgOpPrecondition(
  ...
  if (toolchain::any_of(linalgOp->getOpOperands(), [&](OpOperand &operand) {
        return toolchain::is_contained(linalgOp.getShape(&operand), 0);
      }))
    return failure();
  ...
  for (Operation &innerOp : linalgOp->getRegion(0).front()) {
    ...
    if (!toolchain::all_of(innerOp.getOperandTypes(),
                      VectorType::isValidElementType)) {
      return failure();
    }
    if (!toolchain::all_of(innerOp.getResultTypes(),
                      VectorType::isValidElementType)) {
      return failure();
    }
  }
...
vectorizePackOpPrecondition(linalg::PackOp packOp,
  ...
  if (toolchain::any_of(packOp.getInnerTiles(), [](OpFoldResult v) {
...
vectorizePadOpPrecondition(tensor::PadOp padOp,
  ...
  if (toolchain::any_of(toolchain::enumerate(padOp.getMixedLowPad()),
                   [&](const auto &en) {
...
vectorizeScalableVectorPrecondition(Operation *op,
  ...
  size_t numOfScalableDims =
      toolchain::count_if(inputScalableVecDims, [](bool flag) { return flag; });
...
FailureOr<VectorizationResult> mlir::linalg::vectorize(
  ...
  LDBG() << "Input vector sizes: " << toolchain::interleaved(inputVectorSizes);
  LDBG() << "Input scalable vector dims: "
         << toolchain::interleaved(inputScalableVecDims);
          ...
          .Case<linalg::LinalgOp>([&](auto linalgOp) {
            ...
            if (isa<ConvolutionOpInterface>(linalgOp.getOperation())) {
              FailureOr<Operation *> convOr = vectorizeConvolution(
                  ...
                  flatten1DDepthwiseConv);
              if (succeeded(convOr)) {
                toolchain::append_range(results, (*convOr)->getResults());
                return success();
              }
...
struct VectorizePadOpUserPattern : public OpRewritePattern<tensor::PadOp> {
  ...
  LogicalResult matchAndRewrite(tensor::PadOp padOp,
    ...
    for (auto *user : toolchain::to_vector<4>(padOp->getUsers()))
...
struct PadOpVectorizationWithTransferWritePattern
  ...
  bool hasSameTensorSize(Value beforePadding,
    ...
    for (unsigned i = 0; i < t1.getRank(); ++i) {
      ...
      auto v1 = toolchain::dyn_cast_if_present<Value>(size1);
      auto v2 = toolchain::dyn_cast_if_present<Value>(size2);
      if (!v1 || !v2)
        return false;
...
static Value getStaticPadVal(Operation *op) {
  ...
  if (auto bcast = toolchain::dyn_cast<vector::BroadcastOp>(op)) {
    auto source = bcast.getSource();
    if (toolchain::dyn_cast<VectorType>(source.getType()))
      return {};
  ...
  // tensor.
  if (auto fill = toolchain::dyn_cast<linalg::FillOp>(op)) {
    return fill.getInputs()[0];
  }

  // 3. tensor.generateOp - can't guarantee the value is fixed without
  // analysing, bail out.
  if (auto generate = toolchain::dyn_cast<tensor::GenerateOp>(op)) {
    return {};
  }
  ...
  if (auto xferWrite = toolchain::dyn_cast<vector::TransferWriteOp>(op))
  ...
  // destination.
  if (auto slice = toolchain::dyn_cast<tensor::InsertSliceOp>(op))
...
struct PadOpVectorizationWithInsertSlicePattern
  ...
  LogicalResult rewriteUser(PatternRewriter &rewriter, tensor::PadOp padOp,
    ...
    if (!toolchain::all_of(
            toolchain::zip(insertOp.getMixedSizes(), expectedSizes), [](auto it) {
...
struct Conv1DGenerator
  ...
  FailureOr<Operation *> depthwiseConv(uint64_t channelDimVecSize,
    ...
    if (!toolchain::all_of(resVals, [](Value v) { return v; })) {
