...
FailureOr<SplitReductionResult> mlir::linalg::splitReduction(
  ...
  for (OpOperand *operand : op.getDpsInputOperands()) {
    ...
    unsigned index = 0;
    for (unsigned idx : toolchain::seq<unsigned>(0, map.getNumResults())) {
  ...
  for (unsigned idx : toolchain::seq<unsigned>(0, oldShape.size() + 1)) {
  ...
  for (auto [index, iteratorType] :
       toolchain::enumerate(op.getIteratorTypesArray())) {
  ...
  SmallVector<AffineExpr> exprs;
  for (unsigned i : toolchain::seq<unsigned>(0, intermRank)) {
...
FailureOr<SplitReductionResult> mlir::linalg::splitReductionByScaling(
  ...
  if (!toolchain::all_of(neutralElements, [](Attribute attr) { return attr; }))
  ...
  for (auto it : toolchain::zip(op.getDpsInitsMutable(), neutralElements)) {
  ...
  for (auto it :
       toolchain::zip(genericOp->getResults(), op.getDpsInits(), combinerOps)) {
