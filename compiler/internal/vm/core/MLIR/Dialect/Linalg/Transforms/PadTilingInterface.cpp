...
linalg::computePaddedShape(OpBuilder &builder, TypedValue<RankedTensorType> v,
  ...
  for (const auto &enResults : enumerate(indexingMap.getResults())) {
    ...
    for (size_t paddingDim = 0, e = paddingSizes.size(); paddingDim != e;
      ...
      toolchain::SmallBitVector projectedDims(partialIndexingMap.getNumDims(), true);
...
linalg::computeIndexingMapOpInterfacePaddedShape(
  ...
  auto transferOp =
      toolchain::dyn_cast<IndexingMapOpInterface>(operandToPad.getOwner());
  if (!transferOp)
    return failure();

  // clang-format off
  assert(toolchain::all_of(iterationDomain, [&builder](Range r) {
...
FailureOr<PadTilingInterfaceResult> linalg::rewriteAsPaddedOp(
  ...
  if (options.paddingValues.empty()) {
    ...
    toolchain::append_range(types, toPad->getResultTypes());
    for (Type t : types) {
  ...
  if (toolchain::any_of(toPad->getOperands(),
