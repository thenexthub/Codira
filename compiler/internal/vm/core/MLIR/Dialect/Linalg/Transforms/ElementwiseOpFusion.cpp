...
static bool isOpOperandCanBeDroppedAfterFusedLinalgs(
  ...
  for (auto &op : ops) {
    for (auto &opOperand : op->getOpOperands()) {
      if (toolchain::is_contained(opOperandsToIgnore, &opOperand)) {
        continue;
      }
...
toolchain::SmallDenseSet<int> mlir::linalg::getPreservedProducerResults(
    GenericOp producer, GenericOp consumer, OpOperand *fusedOperand) {
  toolchain::SmallDenseSet<int> preservedProducerResults;
  toolchain::SmallVector<OpOperand *> opOperandsToIgnore;

  // The fusedOperand will be removed during the fusion
  opOperandsToIgnore.emplace_back(fusedOperand);

  for (const auto &producerResult : toolchain::enumerate(producer->getResults())) {
    ...
    if (producer.payloadUsesValueFromOperand(outputOperand) ||
        !isOpOperandCanBeDroppedAfterFusedLinalgs(producer, consumer,
                                                  opOperandsToIgnore) ||
        toolchain::any_of(producerResult.value().getUsers(), [&](Operation *user) {
...
bool mlir::linalg::areElementwiseOpsFusable(OpOperand *fusedOperand) {
  ...
  if ((consumer.getNumReductionLoops())) {
    ...
    for (auto pair :
         toolchain::zip(consumer->getOperands(), consumer.getIndexingMapsArray())) {
...
static void generateFusedElementwiseOpRegion(
    ...
    unsigned nloops, toolchain::SmallDenseSet<int> &preservedProducerResults) {
  ...
  if (producer.hasIndexSemantics()) {
    ...
    toolchain::transform(toolchain::seq<uint64_t>(0, numFusedOpLoops),
                    std::back_inserter(fusedIndices), [&](uint64_t dim) {
                      return IndexOp::create(rewriter, producer.getLoc(), dim);
                    });
    for (IndexOp indexOp :
         toolchain::make_early_inc_range(producerBlock.getOps<IndexOp>())) {
  ...
  for (const auto &bbArg : toolchain::enumerate(
  ...
  for (const auto &producerYieldVal :
       toolchain::enumerate(producerYieldOp.getOperands())) {
...
mlir::linalg::fuseElementwiseOps(RewriterBase &rewriter,
  ...
  auto *it = toolchain::find_if(consumerInputs, [&](OpOperand *operand) {
    return operand == fusedOperand;
  });
  assert(it != consumerInputs.end() && "expected to find the consumer operand");
  for (OpOperand *opOperand : toolchain::make_range(consumerInputs.begin(), it)) {
  ...
  for (OpOperand *opOperand :
       toolchain::make_range(std::next(it), consumerInputs.end())) {
    fusedInputOperands.push_back(opOperand->get());
    fusedIndexMaps.push_back(consumer.getMatchingIndexingMap(opOperand));
  }

  // 6. Collect all of the producer outputs.
  for (const auto &opOperand : toolchain::enumerate(producer.getDpsInitsMutable())) {
  ...
  int resultNum = 0;
  for (auto [index, producerResult] : toolchain::enumerate(producer->getResults()))
...
static bool isFusableWithReshapeByDimExpansion(LinalgOp linalgOp,
  ...
  return linalgOp.hasPureTensorSemantics() &&
         toolchain::all_of(linalgOp.getIndexingMaps().getValue(),
                      [](Attribute attr) {
...
LogicalResult ExpansionInfo::compute(LinalgOp linalgOp,
  ...
  originalLoopExtent = toolchain::map_to_vector(
  ...
  for (const auto &resultExpr : toolchain::enumerate(fusedIndexMap.getResults())) {
  ...
  for (unsigned i : toolchain::seq<unsigned>(0, fusedIndexMap.getNumDims()))
  ...
  for (const auto &numFoldedDim : toolchain::enumerate(numExpandedDims)) {
    auto seq = toolchain::seq<int64_t>(sum, sum + numFoldedDim.value());
...
getIndexingMapInExpandedOp(OpBuilder &builder, AffineMap indexingMap,
  ...
  for (AffineExpr expr : indexingMap.getResults()) {
    ...
    SmallVector<AffineExpr, 4> expandedExprs = toolchain::to_vector<4>(
        toolchain::map_range(expansionInfo.getExpandedDims(pos), [&](int64_t v) {
...
getReassociationForExpansion(AffineMap indexingMap,
  ...
  for (AffineExpr expr : indexingMap.getResults()) {
    ...
    SmallVector<int64_t, 2> indices = toolchain::to_vector<2>(
        toolchain::seq<int64_t>(numReshapeDims, numReshapeDims + numExpandedDims));
...
static void updateExpandedGenericOpRegion(PatternRewriter &rewriter,
  ...
  for (IndexOp indexOp :
       toolchain::make_early_inc_range(fusedRegion.front().getOps<IndexOp>())) {
    ...
    for (auto [expandedShape, expandedIndex] :
         toolchain::zip(expandedDimsShape, expandedIndices)) {
...
static Operation *createExpandedGenericOp(
  ...
  for (auto [i, type] : toolchain::enumerate(linalgOp.getIteratorTypesArray()))
  ...
  return TypeSwitch<Operation *, Operation *>(linalgOp.getOperation())
      ...
      .Case<FillOp, CopyOp>([&](Operation *op) {
        return clone(rewriter, linalgOp, resultTypes,
                     toolchain::to_vector(toolchain::concat<Value>(
                         toolchain::to_vector(expandedOpOperands),
                         toolchain::to_vector(outputs))));
      })
...
fuseWithReshapeByExpansion(LinalgOp linalgOp, Operation *reshapeOp,
  ...
  SmallVector<AffineMap, 4> expandedOpIndexingMaps = toolchain::to_vector<4>(
      toolchain::map_range(linalgOp.getIndexingMapsArray(), [&](AffineMap m) {
...
computeExpandedPadding(tensor::PadOp padOp, ArrayRef<int64_t> expandedShape,
  ...
  for (auto [reInd, l, h] : toolchain::zip_equal(reassociations, low, high)) {
  ...
  for (auto [idx, reInd] : toolchain::enumerate(reassociations)) {
    if (reInd.size() == 1) {
...
getDomainReassociation(AffineMap indexingMap,
  ...
  ReassociationIndices domainReassociation = toolchain::to_vector<4>(
      toolchain::map_range(rangeReassociation, [&](int64_t pos) -> int64_t {
...
bool mlir::linalg::isDimSequencePreserved(AffineMap indexingMap,
  ...
  toolchain::SmallDenseSet<unsigned, 4> sequenceElements;
...
bool mlir::linalg::areDimSequencesPreserved(
  ...
  return toolchain::all_of(maps, [&](AffineMap map) {
    return toolchain::all_of(dimSequences, [&](ReassociationIndicesRef dimSequence) {
...
getCollapsableIterationSpaceDims(GenericOp genericOp, OpOperand *fusableOperand,
  ...
  if (!toolchain::all_of(genericOp.getIndexingMapsArray(), [](AffineMap map) {
  ...
  toolchain::SmallDenseSet<unsigned, 4> processedIterationDims;
  ...
  for (ReassociationIndicesRef foldedRangeDims : reassociation) {
    ...
    // dims.
    if (toolchain::any_of(foldedIterationSpaceDims, [&](int64_t dim) {
    ...
    if (isReductionIterator(startIteratorType)) {
      ...
      for (const auto &startDim : toolchain::enumerate(reductionDims)) {
        ...
        for (const auto &foldedDim :
             toolchain::enumerate(foldedIterationSpaceDims)) {
    ...
    if (toolchain::any_of(genericOp.getIndexingMapsArray(),
...
public:
  LogicalResult initialize(unsigned origNumLoops,
    ...
    toolchain::SmallDenseSet<int64_t, 4> processedDims;
    ...
    for (auto dim : toolchain::seq<int64_t>(0, origNumLoops)) {
      if (processedDims.count(dim))
        continue;
      collapsedOpToOrigOpIterationDim.emplace_back(ReassociationIndices{dim});
    }

    toolchain::sort(collapsedOpToOrigOpIterationDim,
    ...
    for (const auto &foldedDims :
         toolchain::enumerate(collapsedOpToOrigOpIterationDim)) {
  ...
  while (counter < indexingMap.getNumResults()) {
    ...
    unsigned numFoldedDims =
        collapsedOpToOrigOpMapping[origOpToCollapsedOpMapping[dim].first]
            .size();
    if (origOpToCollapsedOpMapping[dim].second == 0) {
      auto range = toolchain::seq<unsigned>(counter, counter + numFoldedDims);
...
static void generateCollapsedIndexingRegion(
  ...
  auto indexOps = toolchain::to_vector(block->getOps<linalg::IndexOp>());
  ...
  //   i1 = (i_{folded} / d2) % d1
  //   i0 = i_{folded} / (d1 * d2)
  toolchain::DenseMap<unsigned, Value> indexReplacementVals;
  for (auto foldedDims :
    ...
    for (auto dim : toolchain::reverse(foldedDimsRef.drop_front())) {
      Value loopDim =
...
static void collapseOperandsAndResults(LinalgOp op,
  ...
  inputOperands =
      toolchain::map_to_vector(op.getDpsInputOperands(), [&](OpOperand *opOperand) {
...
LinalgOp cloneToCollapsedOp<LinalgOp>(RewriterBase &rewriter, LinalgOp origOp,
  ...
  return clone(
      ...
      toolchain::to_vector(toolchain::concat<Value>(inputOperands, outputOperands)));
}
...
GenericOp cloneToCollapsedOp<GenericOp>(RewriterBase &rewriter,
  ...
  collapseOperandsAndResults(origOp, collapsingInfo, rewriter, inputOperands,
                             outputOperands, resultTypes);
  SmallVector<AffineMap> indexingMaps(
      toolchain::map_range(origOp.getIndexingMapsArray(), [&](AffineMap map) {
...
FailureOr<CollapseResult> mlir::linalg::collapseOpIterationDims(
  ...
  if (op.getNumLoops() <= 1 || foldedIterationDims.empty() ||
      toolchain::all_of(foldedIterationDims, [](ReassociationIndicesRef foldedDims) {
  ...
  if (hasPureBufferSemantics &&
      !toolchain::all_of(op->getOpOperands(), [&](OpOperand &opOperand) -> bool {
  ...
  SmallVector<OpFoldResult> loopBound =
      toolchain::map_to_vector(loopRanges, [](Range range) { return range.size; });
  ...
  for (const auto &originalResult : toolchain::enumerate(op->getResults())) {
...
public:
  ...
  LogicalResult matchAndRewrite(tensor::PadOp padOp,
    ...
    for (auto [reInd, l, h] :
         toolchain::zip_equal(reassociations, collapsedPadding.lowPad,
