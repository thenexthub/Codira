...
rewriteInIm2Col(RewriterBase &rewriter, linalg::Conv2DNhwcHwcfOp convOp) {
  ...
  Im2ColToInputDimsExprs inExprs = getIm2ColInputExpressions(
      i2cToOperExprs, toolchain::to_vector(convOp.getStrides().getValues<int64_t>()),
      rewriter);
  auto inMap =
...
rewriteInIm2Col(RewriterBase &rewriter,
  ...
  auto transposeOperand = [&](Value operand, ArrayRef<int64_t> indices) {
    ...
    SmallVector<AffineExpr> exprs = toolchain::to_vector<4>(
        toolchain::map_range(indices, [&](int64_t index) -> AffineExpr {
          return rewriter.getAffineDimExpr(index);
        }));

    SmallVector<int64_t> targetShape = toolchain::to_vector<4>(toolchain::map_range(
