...
static void debugPrintBackwardSlice(SetVector<Operation *> &backwardSlice) {
  LLVM_DEBUG(toolchain::interleaveComma(backwardSlice, DBGS() << "--backwardSlice:",
                                   [](Operation *op) {
                                     dbgs() << "\n";
                                     DBGS() << "----";
...
void HoistPaddingAnalysis::finalizeHoistPaddingAnalysis() {
  ...
  // multiple times.
  for (scf::ForOp forOp : toolchain::reverse(reverseEnclosingLoops))
...
LogicalResult HoistPaddingAnalysis::dropNonIndexDependencies() {
  ...
  auto hasIndexResult = [&](Operation *operation) {
    return toolchain::any_of(operation->getResults(), [&](Value result) {
  ...
  for (Operation *op : toolchain::reverse(backwardSlice)) {
...
static FailureOr<PackingResult> buildPackingLoopNestImpl(
  ...
  if (nPackedLoops > 0) {
    ...
    for (Value iv : toolchain::reverse(clonedLoopIvs)) {
...
static FailureOr<PackingResult> buildPackingLoopNestImpl(
  ...
  // TODO: go grab dims when needed, atm tensor::PadOp yields a static tensor.
  toolchain::append_range(packedShape, transposedTensorType->getShape());
...
padThroughLoopIterArg(RewriterBase &rewriter, Value paddedValueBeforeHoisting,
  ...
  SmallVector<Value> yieldOperands = toolchain::to_vector(forOp.getYieldedValues());
...
static Value replaceByPackingResult(RewriterBase &rewriter,
  ...
  if (nPackedLoops > 0) {
    loopIterationCounts =
        toolchain::to_vector<4>(toolchain::map_range(packingLoops, [&](Operation *loop) {
          return buildLoopIterationCount(rewriter, outerLoop,
                                         cast<scf::ForOp>(loop));
        }));
    // Assert all loop iteration counts can be computed.
    if (toolchain ::any_of(loopIterationCounts, [](Value v) { return !v; }))
      llvm_unreachable("loop independence prerequisite not met");

    // offsets = [maybe_leading_ivs = originalLoopIvs, 0 .. 0].
    toolchain::copy(loopIterationCounts, offsets.begin());
    hoistedPackedTensor =
