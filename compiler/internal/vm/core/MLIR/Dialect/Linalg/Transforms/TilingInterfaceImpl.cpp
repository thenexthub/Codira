...
static LogicalResult inlinePayload(OpBuilder &b, LinalgOp linalgOp,
  ...
  for (const auto &operand : toolchain::enumerate(terminator->getOperands())) {
...
struct LinalgOpTilingInterface
  ...
  SmallVector<Range> getIterationDomain(Operation *op, OpBuilder &b) const {
    ...
    return toolchain::to_vector(
        toolchain::map_range(map.getResults(), [&](AffineExpr loopExpr) {
  ...
  getTiledImplementation(Operation *op, OpBuilder &b,
    ...
    SmallVector<Operation *> generatedSlices = toolchain::map_to_vector(
  ...
  getMappedOffsetAndSize(LinalgOp linalgOp, OpBuilder &b,
    ...
    for (auto [indexingMap, offsets, sizes] :
         toolchain::zip_equal(indexingMaps, allOffsets, allSizes)) {
      for (auto [resultExpr, offset, size] :
           toolchain::zip_equal(indexingMap.getResults(), offsets, sizes)) {
        ...
        if (it != mappedOffsets.end()) {
          ...
          if (seenOffset != offset || seenSize != size) {
            LLVM_DEBUG({
              toolchain::dbgs() << "inconsistent iteration space mapping from "
    ...
    for (auto [index, domain] : toolchain::enumerate(iterationDomain)) {
  ...
  LogicalResult getIterationDomainTileFromOperandTiles(
    ...
    SmallVector<AffineMap> indexingMaps =
        toolchain::map_to_vector(operandNumbers, [&](unsigned operandNumber) {
  ...
  getResultTilePosition(Operation *op, OpBuilder &b, unsigned resultNumber,
    ...
    SmallVector<OpFoldResult> subShapeSizes =
        toolchain::to_vector(toolchain::map_range(sizes, [&](OpFoldResult ofr) {
  ...
  LogicalResult getIterationDomainTileFromResultTile(
    ...
    SmallVector<OpFoldResult> allOffsets = toolchain::to_vector(offsets);
    SmallVector<OpFoldResult> allSizes = toolchain::to_vector(sizes);
    auto status =
...
std::optional<int> getPositionIn(const toolchain::SetVector<unsigned> &reductionDims,
                                 unsigned value) {
...
getPartialResultAffineMaps(LinalgOp linalgOp,
  ...
  auto partialReductionMaps = toolchain::map_to_vector(
...
struct LinalgOpPartialReductionInterface
  ...
  FailureOr<SmallVector<Value>> generateInitialTensorForPartialReduction(
    ...
    for (auto [initIdx, result, partialMap] :
         toolchain::enumerate(linalgOp->getResults(), partialResultMaps)) {
  ...
  tileToPartialReduction(Operation *op, OpBuilder &b, Location loc,
    ...
    if (tilingStrategy ==
      ...
      newInitMaps = toolchain::to_vector(partialReductionMaps);
    } else {
      newInitMaps = toolchain::map_to_vector(
          linalgOp.getDpsInitsMutable(), [&](OpOperand &opOperand) {
            return linalgOp.getMatchingIndexingMap(&opOperand);
          });
    }
    ...
    for (auto [partialReductionMap, valueToTile] :
         toolchain::zip_equal(partialReductionMaps, init)) {
    ...
    for (auto [initOperand, newInitMap] :
         toolchain::zip_equal(linalgOp.getDpsInitsMutable(), newInitMaps)) {
    ...
    if (tilingStrategy ==
      ...
      toolchain::append_range(operands, tiledInits);
      partialReductionOp = mlir::clone(b, op, resultTypes, operands);
    }
    return TilingResult{
        {partialReductionOp},
        toolchain::map_to_vector(partialReductionOp->getResults(),
  ...
  mergeReductions(Operation *op, OpBuilder &b, Location loc,
    ...
    for (auto [idx, init, partialResult, partialMap] : toolchain::enumerate(
      ...
      for (auto [resultNum, dimExpr] :
           toolchain::enumerate(partialMap.getResults())) {
        unsigned dim = cast<AffineDimExpr>(dimExpr).getPosition();
        if (toolchain::is_contained(reductionDims, dim)) {
...
static SmallVector<Range> getPackUnPackIterationDomain(OpTy op,
  ...
  for (auto dim : toolchain::seq<int64_t>(0, rank)) {
...
struct PackOpTiling
  ...
  getTiledImplementation(Operation *op, OpBuilder &b,
    ...
    for (auto dim : toolchain::seq<int64_t>(0, inputRank)) {
    ...
    return TilingResult{
        ...
        toolchain::to_vector(ArrayRef<Operation *>{sourceSlice, outSlice})};
  }
  ...
  getResultTilePosition(Operation *op, OpBuilder &b, unsigned resultNumber,
    ...
    for (auto dataTileDim : toolchain::seq<unsigned>(inputRank, outputRank))
  ...
  generateResultTileValue(Operation *op, OpBuilder &b, unsigned resultNumber,
    ...
    for (auto iter :
         toolchain::zip_equal(packOp.getMixedTiles(), sizes.take_back(numTiles)))
  ...
  LogicalResult getIterationDomainTileFromOperandTiles(
    ...
    if (operandNumbers.size() != 1 || operandNumbers[0] != 0) {
      LLVM_DEBUG(
          { toolchain::dbgs() << "unsupported operands for consumer fusion"; });
      return failure();
    }
    ...
    for (auto dim : toolchain::seq<int64_t>(packOp.getSourceRank())) {
      if (dimAndTileMapping.count(dim)) {
  ...
  FailureOr<TilingResult> getTiledImplementationFromOperandTiles(
    ...
    if (operandNumbers.size() != 1 || operandNumbers[0] != 0) {
      LLVM_DEBUG(
          { toolchain ::dbgs() << "unhandled operands for consumer fusion"; });
      return failure();
    }
...
struct UnPackOpTiling
  ...
  getTiledImplementation(Operation *op, OpBuilder &b,
    ...
    for (auto dim : toolchain::seq<int64_t>(0, destRank)) {
      UnpackTileDimInfo info =
  ...
  getResultTilePosition(Operation *op, OpBuilder &b, unsigned resultNumber,
                        ...
                        SmallVector<OpFoldResult> &resultOffsets,
                        SmallVector<OpFoldResult> &resultSizes) const {
    resultOffsets = toolchain::to_vector(offsets);
    resultSizes = toolchain::to_vector(sizes);
    return success();
  }
  ...
  LogicalResult getIterationDomainTileFromOperandTiles(
    ...
    if (operandNumbers.size() != 1) {
      LLVM_DEBUG({ toolchain::dbgs() << "unable to handle multiple operands"; });
      return failure();
    }
    auto unPackOp = cast<UnPackOp>(op);
    ...
    for (auto dim : toolchain::seq<int64_t>(0, outputRank)) {
      using AV = affine::AffineValueExpr;
      affine::AffineBuilder ab(b, loc);
      AffineExpr dim0, dim1, sym0;
      bindDims(b.getContext(), dim0, dim1);
  ...
  FailureOr<TilingResult> getTiledImplementationFromOperandTiles(
      ...
      ArrayRef<SmallVector<OpFoldResult>> allSizes) const {
    if (operandNumbers.size() != 1 || operandNumbers[0] != 0) {
      LLVM_DEBUG({ toolchain::dbgs() << "unhandled operands for consumer fusion"; });
      return failure();
    }
    auto unPackOp = cast<UnPackOp>(op);
    ArrayRef<OpFoldResult> offsets(allOffsets[0]);
    ...
    int64_t numTiles = unPackOp.getInnerDimsPos().size();
    for (auto iter :
         toolchain::zip_equal(unPackOp.getMixedTiles(), sizes.take_back(numTiles))) {
      if (!isEqualConstantIntOrValue(std::get<0>(iter), std::get<1>(iter)))
        return failure();
    }
    ...
    return TilingResult{{tiledUnPackOp},
                        SmallVector<Value>(tiledUnPackOp->getResults()),
                        toolchain::to_vector(ArrayRef<Operation *>{
