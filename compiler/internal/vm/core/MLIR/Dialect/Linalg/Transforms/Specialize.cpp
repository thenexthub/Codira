...
static FailureOr<LinalgOp> specializeLinalgContractions(RewriterBase &rewriter,
  ...
  if (toolchain::any_of(indexingMaps, [&dims](AffineMap m) {
  ...
  if (numOfBatchDims) {
    ...
    if (toolchain::any_of(indexingMaps, [numOfBatchDims](AffineMap m) {
  ...
  if (toolchain::is_contained({a, b, c}, IndexMatchResult::Mismatch))
    return failure();
