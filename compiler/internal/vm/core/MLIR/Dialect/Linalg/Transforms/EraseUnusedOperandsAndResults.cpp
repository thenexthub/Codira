...
toolchain::SmallDenseMap<unsigned, unsigned> static deduplicateInputOperands(
    GenericOp genericOp, SmallVector<OpOperand *> &droppedOpOperands,
    SmallVector<Value> &newInputOperands,
    SmallVector<AffineMap> &newIndexingMaps) {
  toolchain::SmallDenseMap<unsigned, unsigned> origToNewPos;
  toolchain::SmallDenseMap<std::pair<Value, AffineMap>, unsigned> dedupedInputs;
  for (const auto &en : toolchain::enumerate(genericOp.getDpsInputOperands())) {
...
toolchain::SmallDenseMap<unsigned, unsigned> static deduplicateOutputOperands(
    GenericOp genericOp, SmallVector<OpOperand *> &droppedOpOperands,
    SmallVector<Value> &newOutputOperands,
    SmallVector<AffineMap> &newIndexingMaps, bool removeOutputs) {
  toolchain::SmallDenseMap<unsigned, unsigned> origToNewPos;
  toolchain::SmallDenseMap<std::tuple<Value, AffineMap, Value>, unsigned>
      dedupedOutpts;
  ...
  if (!genericOp.hasPureTensorSemantics() || !removeOutputs) {
    for (const auto &en : toolchain::enumerate(genericOp.getDpsInitsMutable())) {
  ...
  for (const auto &outputOpOperand :
       toolchain::enumerate(genericOp.getDpsInitsMutable())) {
...
static void populateOpPayload(
    ...
    const toolchain::SmallDenseMap<unsigned, unsigned> &origInsToNewInsPos,
    const toolchain::SmallDenseMap<unsigned, unsigned> &origOutsToNewOutsPos,
  ...
  auto updateReplacements =
      [&](SmallVector<OpOperand *> &origOperands,
          ...
          const toolchain::SmallDenseMap<unsigned, unsigned> &map) {
        for (const auto &origOperand : toolchain::enumerate(origOperands)) {
  ...
  SmallVector<OpOperand *> origOutputOperands =
      toolchain::to_vector(toolchain::make_pointer_range(genericOp.getDpsInitsMutable()));
  SmallVector<OpOperand *> newOutputOperands =
      toolchain::to_vector(toolchain::make_pointer_range(newOp.getDpsInitsMutable()));
  ...
  if (newOp.getNumDpsInits() != genericOp.getNumDpsInits()) {
    ...
    for (const auto &yieldOpOperands :
         toolchain::enumerate(origYieldOp.getValues())) {
...
mlir::linalg::deduplicateOperandsAndRemoveDeadResults(
  ...
  toolchain::SmallDenseMap<unsigned, unsigned> origInsToNewInsPos =
      deduplicateInputOperands(genericOp, droppedOpOperands, newInputOperands,
                               newIndexingMaps);

  // Gather information about the dropped outputs.
  toolchain::SmallDenseMap<unsigned, unsigned> origOutsToNewOutsPos =
  ...
  for (NamedAttribute kv : genericOp->getAttrs())
    if (!toolchain::is_contained(odsAttrs, kv.getName().getValue()))
  ...
  for (const auto &result : toolchain::enumerate(genericOp.getResults())) {
...
struct RemoveUnusedCycleInGenericOp : public OpRewritePattern<GenericOp> {
  ...
  LogicalResult matchAndRewrite(GenericOp genericOp,
    ...
    for (const auto &outputOpOperand :
         toolchain::enumerate(genericOp.getDpsInits())) {
