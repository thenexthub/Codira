...
static scf::ForOp replaceWithDifferentYield(RewriterBase &rewriter,
  ...
  auto inits = toolchain::to_vector(loop.getInits());
...
void mlir::linalg::hoistRedundantVectorTransfers(Operation *root,
  ...
  while (changed) {
    ...
    toolchain::DenseSet<LoopLikeOpInterface> definiteNonZeroTripCountLoops;
    if (verifyNonZeroTrip) {
      root->walk([&](LoopLikeOpInterface loopLike) {
        ...
        for (auto [lb, ub] : toolchain::zip_equal(lbs.value(), ubs.value())) {
    ...
    root->walk([&](vector::TransferReadOp transferRead) {
      ...
      for (auto *sliceOp : toolchain::reverse(forwardSlice)) {
      ...
      if (source) {
        ...
        if (auto assume = dyn_cast<memref::AssumeAlignmentOp>(source)) {
          ...
          auto numInLoopUses =
              toolchain::count_if(base.getUses(), [&loop](OpOperand &use) {
      ...
      if (toolchain::any_of(base.getUsers(), toolchain::IsaPred<ViewLikeOpInterface>))
        return WalkResult::advance();
