...
SmallVector<OpFoldResult> permuteValues(ArrayRef<OpFoldResult> values,
  ...
  for (const auto &position :
       toolchain::enumerate(toolchain::map_range(map.getResults(), [](AffineExpr expr) {
...
DecomposeLinalgOp::createPeeledGenericOp(GenericOp genericOp,
  ...
  SmallVector<Type> resultTypes = toolchain::to_vector(genericOp.getResultTypes());
...
DecomposeLinalgOp::createResidualGenericOp(GenericOp genericOp,
  ...
  for (auto resultNum :
       toolchain::seq<unsigned>(origNumResults, peeledGenericOpNumResults))
  ...
  auto indexingMaps = toolchain::to_vector(
      toolchain::map_range(genericOp.getDpsInputOperands(), [&](OpOperand *operand) {
...
DecomposeLinalgOp::matchAndRewrite(GenericOp genericOp,
  ...
  if (toolchain::any_of(genericOp.getDpsInitsMutable(), [&](OpOperand &outOperand) {
  ...
  if (toolchain::any_of(body->getOperations().begin()->getResultTypes(),
  ...
  auto *yieldOp = residualGenericOpBody->getTerminator();
  {
    ...
    yieldedVals.append(toolchain::to_vector(
        toolchain::map_range(peeledScalarOperation->getResults(),
  ...
  for (const auto &inputBlockArg :
       toolchain::enumerate(genericOp.getBody()->getArguments())) {
  ...
  for (const auto &yieldValue : toolchain::enumerate(yieldOp->getOperands())) {
  ...
  /// to the newly added arguments.
  {
    ...
    for (auto num : toolchain::seq<unsigned>(0, peeledScalarOpNumResults))
