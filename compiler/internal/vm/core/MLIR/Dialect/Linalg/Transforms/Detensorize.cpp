...
bool shouldBeDetensored(Operation *op, TypeConverter typeConverter) {
  ...
  return genericOp &&
         toolchain::all_of(genericOp->getOpOperands(), [&](OpOperand &opOperand) {
...
struct LinalgDetensorize
  ...
  public:
    void compute(FunctionOpInterface func,
      ...
      func->walk([&](cf::CondBranchOp condBr) {
        toolchain::append_range(workList, condBr.getOperands());
      });

      func->walk([&](cf::BranchOp br) {
        toolchain::append_range(workList, br.getOperands());
      });
      ...
      auto updateWorkListWithSuccessorArguments =
          [&](Value value, BranchOpInterface terminator) {
            ...
            for (auto operandIdx :
                 toolchain::seq<unsigned>(0, terminator->getOperands().size())) {
      ...
      while (!workList.empty()) {
        ...
        for (auto *user : currentItem.getUsers())
          toolchain::append_range(workList, user->getResults());
        ...
        if (auto genericOp = dyn_cast<GenericOp>(currentItemDefiningOp)) {
          ...
          toolchain::append_range(workList, genericOp.getInputs());
          continue;
        }
        ...
        if (toolchain::all_of(
          ...
          toolchain::append_range(workList, currentItemDefiningOp->getOperands());
      }
  ...
  public:
    void compute(FunctionOpInterface func,
      ...
      for (Block &block : toolchain::drop_begin(func.getFunctionBody(), 1))
  ...
  void runOnOperation() override {
    ...
    target.markUnknownOpDynamicallyLegal([&](Operation *op) {
      ...
      if (auto funcOp = dyn_cast<FunctionOpInterface>(op)) {
        ...
        return toolchain::all_of(toolchain::drop_begin(body, 1), [&](Block &block) {
