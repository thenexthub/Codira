...
public:
  ...
  LogicalResult matchAndRewrite(LinalgOp linalgOp,
    ...
    if (!toolchain::all_of(linalgOp.getDpsInputs(), [](Value input) {
    ...
    if (!toolchain::all_equal(
            toolchain::map_range(linalgOp->getOperands(), getOperandElementType)))
      return failure();
    ...
    // constant again.
    if (!toolchain::all_of(linalgOp.getIndexingMapsArray(),
    ...
    for (const auto &en : toolchain::enumerate(linalgOp.getDpsInputOperands())) {
    ...
    auto inputShapes = toolchain::to_vector<4>(
        toolchain::map_range(linalgOp.getDpsInputs(), [](Value value) {
