...
mlir::linalg::computeStaticContinuousTileSizes(LinalgOp op, unsigned dimension,
  ...
  while (tileSize > 1 && remainderChunk != 0) {
    ...
    uint64_t maxPower = toolchain::bit_floor(tileSize);
  ...
  auto tripCountCheck = [&](SmallVector<int64_t> tileSizes,
    ...
    for (auto [tileSize, tripCount] : toolchain::zip(tileSizes, tripCounts))
...
mlir::linalg::computeContinuousTileSizes(OpBuilder &builder, TilingInterface op,
  ...
  while (tileSizeInt > 1) {
    uint64_t maxPower = toolchain::bit_floor(tileSizeInt);
    ...
    if (Attribute attr = toolchain::dyn_cast_if_present<Attribute>(tripCountSize)) {
...
static void calculateTileOffsetsAndSizes(
  ...
  SmallVector<OpFoldResult> nonZeroNumThreads = toolchain::filter_to_vector(
...
tileLinalgOpImpl(RewriterBase &b, LinalgOp op, ArrayRef<OpFoldResult> tileSizes,
  ...
  if (toolchain::all_of(tileSizes, [](OpFoldResult ofr) {
  ...
  if (options.distribution) {
    ...
    for (const auto &iteratorType : toolchain::enumerate(iteratorTypes)) {
...
FailureOr<linalg::ForallReductionTilingResult> linalg::tileReductionUsingForall(
  ...
  ArrayRef<BlockArgument> destBbArgs = forallOp.getRegionIterArgs();
  {
    ...
    for (Value initOperand : destinationStyleOp.getDpsInits()) {
      auto *it = toolchain::find(dest, initOperand);
    ...
    b.modifyOpInPlace(clonedOp, [&]() {
      for (auto [initOperandPtr, tiledInitValue] : toolchain::zip_equal(
  ...
  for (auto [index, result, bbArg] : toolchain::zip(
           toolchain::seq<unsigned>(0, dest.size()), tilingResults, destBbArgs)) {
