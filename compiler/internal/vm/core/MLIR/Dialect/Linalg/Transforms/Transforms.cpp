...
SmallVector<Value> mlir::linalg::peelLoop(RewriterBase &rewriter,
                                          Operation *op) {
  return toolchain::TypeSwitch<Operation *, SmallVector<Value, 4>>(op)
...
static std::string stringifyReassocIndices(ReassociationIndicesRef ri) {
  return toolchain::interleaved(ri, ", ", /*Prefix=*/"|", /*Suffix=*/"");
}
...
FailureOr<LowerPackResult> linalg::lowerPack(RewriterBase &rewriter,
  ...
  if (toolchain::any_of(packOp.getStaticInnerTiles(), ShapedType::isDynamic)) {
  ...
  for (auto [pos, innerSize] :
       toolchain::zip_equal(packOp.getInnerDimsPos(), packOp.getMixedTiles())) {
    int outerPos =
  ...
  LDBG() << "insertPositions: "
         << toolchain::interleaved(packingMetadata.insertPositions);
  LDBG() << "outerPositions: "
         << toolchain::interleaved(packingMetadata.outerPositions);
  LDBG() << "packedShape: " << toolchain::interleaved(packedTensorType.getShape());
  LDBG() << "packedToStripMinedShapePerm: "
         << toolchain::interleaved(packedToStripMinedShapePerm);
  LDBG() << "reassociations: "
         << toolchain::interleaved(toolchain::map_range(packingMetadata.reassociations,
                                              stringifyReassocIndices));
  LDBG() << "stripMinedShape: " << toolchain::interleaved(stripMinedShape);
  ...
  LDBG() << "transpPerm: " << toolchain::interleaved(transpPerm);
...
FailureOr<PackResult> linalg::pack(RewriterBase &rewriter,
  ...
  LDBG() << "maps: " << toolchain::interleaved(indexingMaps);
  LDBG() << "iterators: " << toolchain::interleaved(iteratorTypes);
  ...
  for (int64_t i = 0, e = packedSizes.size(); i < e; ++i) {
    ...
    LDBG() << "packedDimForEachOperand: "
           << toolchain::interleaved(packedOperandsDims.packedDimForEachOperand);
  ...
  SmallVector<OpOperand *> initOperands =
      toolchain::to_vector(toolchain::make_pointer_range(linalgOp.getDpsInitsMutable()));
  ...
  for (const auto &operandsList : {inputOperands, initOperands}) {
    for (OpOperand *opOperand : operandsList) {
      ...
      LDBG() << "innerPos: " << toolchain::interleaved(innerPos);
      LDBG() << "innerPackSizes: " << toolchain::interleaved(innerPackSizes);
      ...
      bool areConstantTiles =
          toolchain::all_of(innerPackSizes, [](OpFoldResult tile) {
...
static LinalgOp transposeOneLinalgOperandAndReplace(
  ...
  SmallVector<unsigned> tmpTransposition = toolchain::to_vector(
      toolchain::map_range(permutation, [](int64_t i) -> unsigned { return i; }));
...
linalg::packTranspose(RewriterBase &rewriter, linalg::PackOp packOp,
  ...
  if (permutation.empty())
    toolchain::append_range(permutation, toolchain::seq<int64_t>(0, numLeadingDims));
  // Trailing part needs to reindex positions by `numLeadingDims`.
  if (innerPerm.empty()) {
    toolchain::append_range(
        permutation,
        toolchain::seq<int64_t>(numLeadingDims, numLeadingDims + numTrailingDims));
  } else {
    toolchain::append_range(permutation,
                       toolchain::map_range(innerPerm, [&](int64_t pos) {
...
linalg::packMatmulGreedily(RewriterBase &rewriter, LinalgOp linalgOp,
  ...
  LDBG() << "perm: " << toolchain::interleaved(permutation);
  ...
  LDBG() << "paddedSizesNextMultipleOf: "
         << toolchain::interleaved(paddedSizesNextMultipleOf);
  LDBG() << "loopRanges: "
         << toolchain::interleaved(
                toolchain::map_range(loopRanges, [](Range r) { return r.size; }));
  ...
  LDBG() << "adjustedPackedSizes: " << toolchain::interleaved(adjustedPackedSizes);
...
mlir::linalg::LinalgTilingOptions::setTileSizes(ArrayRef<int64_t> ts) {
  ...
  tileSizeComputationFunction = [tileSizes](OpBuilder &b, Operation *op) {
    ...
    return toolchain::to_vector<4>(map_range(tileSizes, [&](int64_t s) {
...
DecomposePadOpPattern::matchAndRewrite(tensor::PadOp padOp,
  ...
  auto getIdxValue = [&](OpFoldResult ofr) {
    if (auto val = toolchain::dyn_cast_if_present<Value>(ofr))
      return val;
...
static Value getPackOpSourceOrPaddedSource(OpBuilder &builder,
  ...
  assert(toolchain::all_of(packOp.getAllOuterDims(),
  ...
  for (int64_t dimIdx = 0; dimIdx < inputRank; ++dimIdx) {
    ...
    dynamicTileSizes.push_back(toolchain::dyn_cast<Value>(tileSizeForDim));
  }
  auto resultType =
...
getPackUnpackNormalizedPerm(int rank, ArrayRef<int64_t> perm) {
  ...
  for (auto [index, value] : toolchain::enumerate(perm))
    vec[value] = index;
  SmallVector<int64_t> normalizedPerm = toolchain::filter_to_vector(
...
getPackUnpackRankReducedPerm(ArrayRef<int64_t> shape,
  ...
  for (auto i : toolchain::seq<unsigned>(0, unpackedRank)) {
    if (toolchain::is_contained(innerDimsPos, i)) {
...
LogicalResult DecomposeOuterUnitDimsPackOpPattern::matchAndRewrite(
  ...
  if (toolchain::any_of(packOp.getTiledOuterDims(),
  ...
  if (!toolchain::all_of(outerDimsPerm, [&innerDimsPos, &packOp](int64_t dim) {
        static int prev = 0;
        // Skip tiled dims - these can be permuted.
        if (toolchain::is_contained(innerDimsPos, dim))
          return true;
  ...
  // attributes.
  toolchain::transform(shapeForEmptyOp, shapeForEmptyOp.begin(),
                  [&](OpFoldResult ofr) {
                    if (auto val = toolchain::dyn_cast<Value>(ofr))
                      return getAsOpFoldResult(val);
                    return ofr;
                  });

  LDBG() << "Pack permutation: " << packOp;
  LDBG() << "perm: " << toolchain::interleaved(srcPermForTranspose);
  LDBG() << "Shape of empty tensor: " << toolchain::interleaved(shapeForEmptyOp);
...
LogicalResult DecomposeOuterUnitDimsUnPackOpPattern::matchAndRewrite(
  ...
  if (toolchain::any_of(unpackOp.getTiledOuterDims(),
                   [](int64_t dim) { return dim != 1; })) {
    return rewriter.notifyMatchFailure(
        unpackOp,
  ...
  for (auto i : toolchain::seq<unsigned>(0, destRank)) {
