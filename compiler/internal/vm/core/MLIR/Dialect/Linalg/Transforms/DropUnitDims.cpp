...
struct MoveInitOperandsToInput : public OpRewritePattern<GenericOp> {
  ...
  LogicalResult matchAndRewrite(GenericOp genericOp,
    ...
    SmallVector<Value> newOutputOperands =
        toolchain::to_vector(genericOp.getDpsInits());
    for (OpOperand *op : candidates) {
...
replaceUnitDimIndexOps(GenericOp genericOp,
                       const toolchain::SmallDenseSet<unsigned> &unitDims,
  ...
  for (IndexOp indexOp :
    ...
    if (unitDims.count(indexOp.getDim()) != 0) {
      ...
      unsigned droppedDims = toolchain::count_if(
...
static UnitExtentReplacementInfo dropUnitExtentFromOperandMetadata(
    ...
    toolchain::SmallDenseMap<unsigned, unsigned> &oldDimsToNewDimsMap,
...
linalg::dropUnitDims(RewriterBase &rewriter, IndexingMapOpInterface op,
  ...
  for (OpOperand &opOperand : op->getOpOperands())
    toolchain::append_range(allShapesSizes, op.getStaticOperandShape(&opOperand));
  ...
  toolchain::SmallDenseSet<unsigned> unitDimsFilter(allowedUnitDims.begin(),
                                               allowedUnitDims.end());
  toolchain::SmallDenseSet<unsigned> unitDims;
  ...
  //    count loops.
  toolchain::SmallDenseMap<unsigned, unsigned> oldDimToNewDimMap;
  SmallVector<AffineExpr> dimReplacements;
  unsigned newDims = 0;
  for (auto index : toolchain::seq<int64_t>(op.getStaticLoopRanges().size())) {
  ...
  for (auto [index, result] : toolchain::enumerate(replacementOp->getResults())) {
...
linalg::dropUnitDims(RewriterBase &rewriter, GenericOp genericOp,
  ...
  DroppedUnitDimsBuilder build =
      [](Location loc, OpBuilder &b, IndexingMapOpInterface op,
         ...
         const toolchain::SmallDenseSet<unsigned> &droppedDims)
    ...
    for (auto [index, attr] :
         toolchain::enumerate(genericOp.getIteratorTypesArray())) {
    ...
    for (unsigned i : toolchain::seq<unsigned>(0, genericOp.getNumResults()))
...
struct DropPadUnitDims : public OpRewritePattern<tensor::PadOp> {
  ...
  LogicalResult matchAndRewrite(tensor::PadOp padOp,
    ...
    for (const auto [dim, size, outSize, low, high] : zip_equal(
             toolchain::seq(static_cast<int64_t>(0), padRank), sourceShape,
    ...
    if (options.rankReductionStrategy ==
      ...
      for (auto dim : toolchain::seq(static_cast<int64_t>(0), padRank)) {
...
static Value collapseSingletonDimAt(PatternRewriter &rewriter, Value val,
  ...
  assert(toolchain::succeeded(collapsed) && "Collapsing the value failed");
...
struct RankReduceContractionOps : OpRewritePattern<FromOpTy> {
  ...
  collapseOperands(PatternRewriter &rewriter, ArrayRef<Value> operands,
    ...
    return toolchain::map_to_vector(
        toolchain::zip(operands, operandCollapseDims), [&](auto pair) {
...
struct RankReduceToUnBatched : RankReduceContractionOps<FromOpTy, ToOpTy> {
  ...
  getOperandUnitDims(LinalgOp op,
    ...
    if (failed(maybeContractionDims)) {
      LLVM_DEBUG(toolchain::dbgs() << "could not infer contraction dims");
      return failure();
    }
    ...
    if (bOperands.size() != 3 || toolchain::any_of(bOperands, [](auto pair) {
          return cast<ShapedType>(std::get<0>(pair).getType())
                     .getShape()[std::get<1>(pair)] != 1;
        })) {
      LLVM_DEBUG(toolchain::dbgs() << "specified unit dims not found");
      return failure();
    }
...
struct RankReduceMatmul : RankReduceContractionOps<FromOpTy, ToOpTy> {
  ...
  getOperandUnitDims(LinalgOp op,
    ...
    if constexpr (reduceLeft) {
      ...
      if (mOperands.size() != 2)
        return failure();
      if (toolchain::all_of(mOperands, [](auto pair) {
            return cast<ShapedType>(std::get<0>(pair).getType())
                       .getShape()[std::get<1>(pair)] == 1;
          })) {
      ...
      if (nOperands.size() != 2)
        return failure();
      if (toolchain::all_of(nOperands, [](auto pair) {
            return cast<ShapedType>(std::get<0>(pair).getType())
                       .getShape()[std::get<1>(pair)] == 1;
          })) {
