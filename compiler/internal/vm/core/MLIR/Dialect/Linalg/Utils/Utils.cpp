...
RegionMatcher::matchAsScalarBinaryOp(GenericOp op) {
  ...
  if (!toolchain::hasSingleElement(block.without_terminator()))
    return std::nullopt;
...
computePackUnPackPerm(int64_t rank, ArrayRef<int64_t> &innerDimsPos,
  ...
  auto lastDims =
      toolchain::to_vector(toolchain::seq<int64_t>(rank - numPackedDims, rank));
...
bool allIndexingsAreProjectedPermutation(LinalgOp op) {
  return toolchain::all_of(op.getIndexingMapsArray(), [](AffineMap m) {
...
bool hasOnlyScalarElementwiseOp(Region &r) {
  ...
  for (Operation &op : r.front()) {
    if (!(isa<arith::ConstantOp, func::ConstantOp, tensor::ExtractOp,
        ...
        toolchain::any_of(op.getResultTypes(),
...
static bool convLayoutMatches(ArrayRef<ArrayRef<AffineExpr>> mapListExpected,
  ...
  return indexingMaps ==
         ArrayAttr::get(
             context, toolchain::to_vector<4>(toolchain::map_range(
...
matchConvolutionOpOfType<linalg::Conv1DNwcWcfOp>(LinalgOp op) {
  ...
  if (auto convOp = dyn_cast<linalg::Conv1DNwcWcfOp>(op.getOperation())) {
    result.dilations =
        toolchain::to_vector(convOp.getDilations().getValues<int64_t>());
    result.strides = toolchain::to_vector(convOp.getStrides().getValues<int64_t>());
    return result;
  }
...
matchConvolutionOpOfType<linalg::PoolingNhwcMaxOp>(LinalgOp op) {
  ...
  if (auto poolOp = dyn_cast<linalg::PoolingNhwcMaxOp>(op.getOperation())) {
    result.dilations =
        toolchain::to_vector(poolOp.getDilations().getValues<int64_t>());
    result.strides = toolchain::to_vector(poolOp.getStrides().getValues<int64_t>());
    return result;
  }
...
Value makeComposedPadHighOp(OpBuilder &b, Location loc, RankedTensorType type,
  ...
  if (toolchain::any_of(padOp.getMixedLowPad(), [](OpFoldResult ofr) {
  ...
  if (toolchain::any_of(
          toolchain::zip(sliceOp.getMixedSizes(), padOpSliceOp.getMixedSizes()),
...
GenericOp makeMemRefCopyOp(OpBuilder &b, Location loc, Value from, Value to) {
  ...
  return linalg::GenericOp::create(
      ...
      /*inputs=*/from,
      /*outputs=*/to,
      /*indexingMaps=*/toolchain::ArrayRef({id, id}),
...
void GenerateLoopNest<scf::ForOp>::doit(
  ...
  if (!linalgOp.hasPureBufferSemantics())
    toolchain::append_range(iterArgInitValues, linalgOp.getDpsInits());
  ...
  for (const auto &loop : toolchain::enumerate(loopNest.loops)) {
...
void GenerateLoopNest<scf::ParallelOp>::doit(
  ...
  for (const auto &it : toolchain::enumerate(procInfo)) {
...
computeSliceParameters(OpBuilder &builder, Location loc, Value valueToTile,
  ...
  for (unsigned r = 0; r < rank; ++r) {
    LLVM_DEBUG(toolchain::dbgs() << "computeSliceParameters: for dim#" << r);
    if (!isTiled(map.getSubMap({r}), tileSizes)) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << ": not tiled: use size: " << dim << "\n");
      continue;
    }
    LLVM_DEBUG(toolchain::dbgs() << ": tiled: figure out subsize...\n");

    // Tiling creates a new slice at the proper index, the slice step is 1
    // (i.e. the op does not subsample, stepping occurs in the loop).
    auto m = map.getSubMap({r});
    LLVM_DEBUG(toolchain::dbgs() << "computeSliceParameters: submap: " << m << "\n");
    ...
    if (omitPartialTileCheck) {
      ...
      // unnecessary.
      LLVM_DEBUG(toolchain::dbgs() << "makeTiledShape: new size: " << size << "\n");
    ...
    if (!hasTileSizeOne && !dividesEvenly) {
      LLVM_DEBUG(toolchain::dbgs() << "makeTiledShape: shapeSize=" << shapeSize
                              << ", size: " << size
      ...
      SmallVector<OpFoldResult> maxIndices =
          toolchain::to_vector(toolchain::map_range(ubs, [&](OpFoldResult ub) {
...
SmallVector<OpFoldResult> computeTileOffsets(OpBuilder &b, Location loc,
  ...
  for (unsigned idx = 0, idxIvs = 0, e = tileSizes.size(); idx < e; ++idx) {
    LLVM_DEBUG(toolchain::dbgs() << "makeTiledShapes: for loop#" << idx << "\n");
...
SmallVector<OpFoldResult> computeTileSizes(OpBuilder &b, Location loc,
  ...
  for (unsigned idx = 0, e = tileSizes.size(); idx < e; ++idx) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "computeTileSizes: " << sizes.back() << "\n");
  }
  return sizes;
}
...
SmallVector<Type> getTensorOutputTypes(LinalgOp op, ValueRange operands) {
  ...
  return toolchain::to_vector(
      toolchain::map_range(op.getDpsInitsMutable(), [&](OpOperand &opOperand) {
...
computeAllSliceParameters(OpBuilder &builder, Location loc, LinalgOp linalgOp,
                          ...
                          ArrayRef<OpFoldResult> sizeBounds,
                          bool omitPartialTileCheck) {
  assert(ivs.size() == static_cast<size_t>(toolchain::count_if(
                           toolchain::make_range(tileSizes.begin(), tileSizes.end()),
  ...
  for (auto [opOperand, val] :
       toolchain::zip(linalgOp->getOpOperands(), valuesToTile)) {
    Value shapedOp = val;
    LLVM_DEBUG(toolchain::dbgs() << "makeTiledShapes: for operand " << shapedOp);
    ...
    LLVM_DEBUG(toolchain::dbgs() << ": tiled: figure out subshape...\n");
...
SmallVector<Value> makeTiledShapes(OpBuilder &builder, Location loc,
  ...
  for (auto item : toolchain::zip(valuesToTile, allSliceParameter)) {
...
getReassociationMapForFoldingUnitDims(ArrayRef<OpFoldResult> mixedSizes) {
  ...
  for (const auto &it : toolchain::enumerate(mixedSizes)) {
    auto dim = it.index();
    auto size = it.value();
    curr.push_back(dim);
    auto attr = toolchain::dyn_cast_if_present<Attribute>(size);
