...
transform::gpu::CopyMappingInfo::CopyMappingInfo(MLIRContext *ctx,
  ...
  LDBG() << "--copy shape: " << toolchain::interleaved(copySizes);
  ...
  LDBG() << "--copy: " << toolchain::interleaved(copySizes) << "\n"
         << "--numThreads: " << toolchain::interleaved(this->numThreads) << "\n"
  ...
  this->smallestBoundingTileSizes = toolchain::to_vector(
      toolchain::map_range(toolchain::zip(copySizes, this->numThreads), [](auto &&pair) {
        int64_t size, numThreads;
        std::tie(size, numThreads) = pair;
        return toolchain::divideCeilSigned(size, numThreads);
      }));
  ...
  this->threadMapping =
      toolchain::to_vector(ArrayRef(allThreadMappings)
  ...
  for (auto factor : factors) {
    ...
    if (localBest > best && localBest <= maxNumThreads) {
      ...
      LDBG() << indent << "nestedThreadsPerDim: "
             << toolchain::interleaved(nestedThreadsPerDim);
      localThreadsPerDim.clear();
      localThreadsPerDim.push_back(factor);
      toolchain::append_range(localThreadsPerDim, nestedThreadsPerDim);
      best = localBest;
    }
  }

  LDBG() << indent << "found globalBest: " << best;
  LDBG() << indent << "numThreads: " << toolchain::interleaved(localThreadsPerDim);
...
transform::gpu::CopyMappingInfo::inferNumThreadsImpl(
  ...
  LDBG() << "inferred numThreads: " << toolchain::interleaved(inferredNumThreads);
...
void transform::gpu::CopyMappingInfo::print(toolchain::raw_ostream &os) const {
  os << "MappingInfo{"
     << "CopyMappingInfo: " << "valid: " << (status != Status::Invalid) << ", "
     << "vectorSize: " << vectorSize << ", numThreads: {"
     << toolchain::interleaved(numThreads) << "}, smallestBoundingTileSizes: {"
     << toolchain::interleaved(smallestBoundingTileSizes) << "}, threadMapping: {"
     << toolchain::interleaved(threadMapping) << "}}";
}
