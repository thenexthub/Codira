...
static FailureOr<LinalgOp> tryApply(Operation *operation, Args &&...args) {
  ...
  using OpTy = typename toolchain::function_traits<
  ...
  for (OpFoldResult ofr : ofrs) {
    ...
    if (!toolchain::hasSingleElement(payloadOps)) {
      ...
      diag.attachNote(transformValue.getLoc())
          << "mapped to " << toolchain::range_size(payloadOps) << " payload ops";
      return diag;
    }
...
static DiagnosedSilenceableFailure reifyMixedParamAndHandleResults(
  ...
  for (OpFoldResult paramOrHandle : mixedResults) {
    ...
    if (!toolchain::hasSingleElement(payload))
...
DiagnosedSilenceableFailure transform::BufferizeToAllocationOp::apply(
  ...
  toolchain::scope_exit resetListener(
...
static bool mayBeRead(Value value) {
  ...
  if (!isa<TensorType, FloatType, IntegerType>(value.getType()))
    return true;
  return toolchain::any_of(value.getUses(),
...
transform::PromoteTensorOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Value tensor : state.getPayloadValues(getTensor())) {
    ...
    toolchain::SmallPtrSet<Operation *, 4> preservedOps;
    for (auto [pos, dim] : toolchain::enumerate(type.getShape())) {
...
void transform::FuseOp::build(OpBuilder &builder, OperationState &result,
  ...
  unsigned numExpectedLoops =
      useForall ? 1 : staticTileSizes.size() - toolchain::count(staticTileSizes, 0);
  ...
  if (loopTypes.size() == 1)
    ...
    toolchain::append_range(resultTypes, loopTypes);
...
static LogicalResult applyTilingToAll(
  ...
  for (Operation *target : payloadOps) {
    ...
    toolchain::append_range(opsToReplace, tiledResults->fusedProducers);
...
transform::FuseOp::apply(transform::TransformRewriter &rewriter,
  ...
  size_t numLoops =
      useForall ? 1 : tileSizes.size() - toolchain::count(tileSizes, 0);
...
LogicalResult transform::FuseOp::verify() {
  ...
  size_t numExpectedLoops =
      getUseForall() ? 1 : sizes.size() - toolchain::count(sizes, 0);
...
static Operation *replaceForAllWithNewSignature(
  ...
  SmallVector<Operation *> yieldingOps = toolchain::to_vector<4>(toolchain::map_range(
  ...
  for (auto result : toolchain::enumerate(forallOp.getResults())) {
...
tileAndFuseFirstExtractUse(RewriterBase &rewriter, Diagnostic &diag,
  ...
  auto it = toolchain::find_if(tileableProducer->getUsers(), [&](Operation *user) {
...
tileAndFuseFirstExtractUseThroughContainingOpBlockArgument(
  ...
  auto itProducerUses =
      toolchain::find_if(tileableProducer->getUses(), [&](OpOperand &use) {
  ...
  // TODO: Generalize to more extract/insert/parallel_insert triples, maybe
  // evolve into an interface.
  auto itBBArgUsers = toolchain::find_if(bbArg.getUsers(), [&](Operation *user) {
    auto sliceOp = dyn_cast<tensor::ExtractSliceOp>(user);
    return sliceOp && containingOp->isProperAncestor(sliceOp);
  });
  ...
  toolchain::scope_exit scopeGuard(
...
transform::FuseIntoContainingOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (!toolchain::hasSingleElement(containingOps)) {
    return emitDefiniteFailure()
           << "requires exactly one containing_op handle (got "
           << toolchain::range_size(containingOps) << ")";
  }
  ...
  SetVector<Operation *> remainingProducers(toolchain::from_range, producerOps);
  auto getNextProducer = [&]() -> FailureOr<Operation *> {
    for (const auto &it : enumerate(remainingProducers)) {
      Operation *producerOp = it.value();
      // The containing op may be a user of producerOp: use isAncestor.
      int64_t numUsesInContainingOp =
          toolchain::count_if(producerOp->getUsers(), [&](Operation *op) {
...
LogicalResult transform::InterchangeOp::verify() {
  ...
  auto sequence = toolchain::to_vector(toolchain::seq<int64_t>(0, permutation.size()));
    ...
    if (getFilterOperandTypes().has_value()) {
      ...
      if (types.size() == 1) {
        ...
        for (auto [attr, operandType] :
             toolchain::zip_equal(getFilterOperandTypes().value(), operandTypes)) {
...
DiagnosedSilenceableFailure transform::MultiTileSizesOp::applyToOne(
  ...
  if (isa<TransformParamTypeInterface>(getLowSize().getType())) {
    ...
    results.assign(toolchain::map_range(
...
transform::PackOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (!toolchain::hasSingleElement(targetOps) || !linalgOp) {
    return emitSilenceableError()
           << "requires target to map to exactly 1 LinalgOp (got "
           << toolchain::range_size(targetOps) << ")";
  }
...
LogicalResult transform::PackGreedilyOp::verify() {
  ...
  if (!getMatmulPaddedSizesNextMultipleOf().empty()) {
    for (auto [s, nmo] :
         toolchain::zip_equal(getMixedMatmulPackedSizes(),
...
static bool isValidPackingPermutation(
  ...
  static_assert(
      toolchain::is_one_of<RelayoutOpTy, linalg::PackOp, linalg::UnPackOp>::value,
...
transform::PackTransposeOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (!toolchain::hasSingleElement(packOrUnpackOps) ||
      !toolchain::hasSingleElement(linalgOps)) {
    return emitSilenceableError()
           << "requires target to map to exactly 1 "
              "packing op and 1 packed op ("
           << "got " << toolchain::range_size(packOrUnpackOps) << " and "
           << toolchain::range_size(linalgOps) << ")";
  }
...
transform::PadOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Operation *target : state.getPayloadOps(getTarget())) {
    ...
    for (auto const &[untypedAttr, elementOrTensorType] :
         toolchain::zip(getPaddingValues(), linalgTarget->getOperandTypes())) {
    ...
    if (getUsePrescribedTensorShapes() &&
      ...
      for (OpOperand &operand : linalgTarget->getOpOperands()) {
        for (auto [i, dim] : toolchain::enumerate(linalgTarget.getShape(&operand))) {
    ...
    if (options.copyBackOp != LinalgPaddingOptions::CopyBackOp::None) {
      for (Value v : replacements) {
        ...
        if (!toolchain::is_contained(copyBackOps, copyBackOp))
...
LogicalResult transform::PadOp::verify() {
  ...
  for (Attribute attr : transposes) {
    ...
    auto sequence = toolchain::to_vector(toolchain::seq<int64_t>(0, transpose.size()));
    if (!std::is_permutation(sequence.begin(), sequence.end(),
...
transform::PadTilingInterfaceOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Operation *target : state.getPayloadOps(getTarget())) {
    ...
    SmallVector<Attribute> paddingValues;
    for (auto const &[untypedAttr, elementOrTensorType] :
         toolchain::zip(getPaddingValues(), targetOp->getOperandTypes())) {
      auto attr = dyn_cast<TypedAttr>(untypedAttr);
      Type elementType = getElementTypeOrSelf(elementOrTensorType);
...
DiagnosedSilenceableFailure transform::HoistPadBuildPackingLoopNestOp::apply(
  ...
  if (!toolchain::hasSingleElement(targetOps) || !toolchain::hasSingleElement(loopOps)) {
    return emitDefiniteFailure()
           << "requires exactly one target and one loop handle (got "
           << toolchain::range_size(targetOps) << " and "
           << toolchain::range_size(loopOps) << ")";
  }
...
transform::PromoteOp::applyToOne(transform::TransformRewriter &rewriter,
  ...
  if (!getUseFullTileBuffers().empty())
    promotionOptions = promotionOptions.setUseFullTileBuffers(
        toolchain::to_vector(getUseFullTileBuffers().getAsValueRange<BoolAttr>()));
...
SplitOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (getDynamicChunkSizes()) {
    ...
    if (isa<TransformHandleTypeInterface>(getDynamicChunkSizes().getType())) {
      chunkSizes = toolchain::to_vector(toolchain::map_range(
      ...
      chunkSizes = toolchain::to_vector(
          toolchain::map_range(state.getParams(getDynamicChunkSizes()),
  ...
  if (isMultiwaySplit) {
    ...
    for (auto &&[idx, chunkSize] : toolchain::enumerate(chunkSizes)) {
    ...
    for (const auto &pair : toolchain::zip(payload, chunkSizes)) {
...
DiagnosedSilenceableFailure transform::TileReductionUsingForOp::applyToOne(
  ...
  if (reductionDims.empty()) {
    for (auto [idx, iteratorType] :
         toolchain::enumerate(partialReductionOp.getLoopIteratorTypes())) {
...
transform::ContinuousTileSizesOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (!toolchain::hasSingleElement(targetOps)) {
    return mlir::emitSilenceableFailure(getLoc())
           << "requires exactly one target (got " << toolchain::range_size(targetOps)
           << ")";
  }
  ...
  if (isa<TransformParamTypeInterface>(getChunkSizes().getType())) {
    ...
    for (auto &&[tileSize, tripCount] :
         toolchain::zip_equal(spec->tileSizes, spec->tripCounts))
      chunkSizes.push_back(tileSize * tripCount);

    auto getI64AttrsFromI64 = [&](ArrayRef<int64_t> values) {
      return toolchain::map_to_vector(values, [&](int64_t value) -> Attribute {
  ...
  auto getDefiningOps = [&](ArrayRef<Value> values) {
    return toolchain::map_to_vector(values, [&](Value value) -> Operation * {
...
void transform::TileUsingForOp::build(
  ...
  auto staticTileSizesAttr = builder.getDenseI64ArrayAttr(staticTileSizes);
  unsigned numExpectedLoops =
      staticTileSizes.size() - toolchain::count(staticTileSizes, 0);
  SmallVector<Type> resultTypes;
  resultTypes.reserve(numExpectedLoops);
  assert((loopTypes.size() == 1 || loopTypes.size() == numExpectedLoops) &&
         "expected one loop type or as many as loops");
...
LogicalResult transform::TileUsingForOp::verify() {
  ...
  unsigned numExpectedLoops = staticSizes.size() - toolchain::count(staticSizes, 0);
...
transform::TileUsingForOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Value transformValue : getDynamicSizes()) {
    if (isa<ParamType>(transformValue.getType())) {
      ...
      paramSizes.push_back(
          toolchain::to_vector(toolchain::map_range(params, [](Attribute attr) {
    ...
    paramSizes.push_back({});
    dynamicSizeProducers.push_back(
        toolchain::to_vector(state.getPayloadOps(transformValue)));
  ...
  for (auto [i, op] : toolchain::enumerate(targets)) {
    ...
    if (tileSizes.empty()) {
      ...
      tilingOptions.setTileSizeComputationFunction([&, index = i](OpBuilder &b,
        ...
        for (auto [ofrIdx, ofr] : toolchain::enumerate(getMixedSizes())) {
    ...
    for (const auto &en2 : toolchain::enumerate(maybeTilingResult->loops))
      loops[en2.index()].push_back(en2.value());
  }

  transformResults.set(cast<OpResult>(getTiledLinalgOp()), tiled);
  for (const auto &en : toolchain::enumerate(loops))
...
normalizeUpperBounds(RewriterBase &rewriter, Location loc,
  ...
  for (auto [lb, ub, step] : toolchain::zip_equal(lbs, ubs, steps)) {
...
static SmallVector<Value> denormalizeIndVar(RewriterBase &rewriter,
  ...
  for (auto [iv, lb, step] : toolchain::zip_equal(ivs, lbs, steps)) {
...
static scf::ForallOp normalizeForallLoopOp(RewriterBase &rewriter,
  ...
  if (toolchain::all_of(lbs, isZeroInteger) && toolchain::all_of(steps, isOneInteger)) {
    return loop;
  }
...
doit(RewriterBase &rewriter, OpTy target,
     transform::ApplyToEachResultList &results,
     transform::TransformState &state) {
  static_assert(toolchain::is_one_of<OpTy, tensor::InsertSliceOp,
