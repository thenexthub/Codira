...
Value linalg::createOrFoldDimOp(OpBuilder &b, Location loc, Value source,
                                int64_t dim) {
  if (toolchain::isa<UnrankedMemRefType, MemRefType>(source.getType()))
    return b.createOrFold<memref::DimOp>(loc, source, dim);
  if (toolchain::isa<UnrankedTensorType, RankedTensorType>(source.getType()))
...
OpFoldResult linalg::createFoldedDimOp(OpBuilder &b, Location loc, Value source,
                                       int64_t dim) {
  auto shapedType = toolchain::cast<ShapedType>(source.getType());
...
using RegionBuilderFn = toolchain::function_ref<void(
...
static void buildStructuredOp(OpBuilder &b, OperationState &state,
  ...
  if (!resultTensorTypes)
    copy_if(outputs.getTypes(), std::back_inserter(derivedResultTypes),
            toolchain::IsaPred<RankedTensorType>);
...
static void buildMatmulOp(OpBuilder &b, OperationState &state,
  ...
  indexingMapsAttrVal =
      toolchain::map_to_vector(indexingMaps, [](AffineMap map) -> Attribute {
...
parseCommonStructuredOpParts(OpAsmParser &parser, OperationState &result,
  ...
  if (addOperandSegmentSizes) {
    ...
    if (result.propertiesAttr) {
      NamedAttrList attrs = toolchain::cast<DictionaryAttr>(result.propertiesAttr);
...
static ParseResult parseNamedStructuredOpRegion(
  ...
  if (numRegionArgs != inputTypes.size() + outputTypes.size()) {
    return parser.emitError(
        ...
        toolchain::formatv("[parseNamedStructuredOpRegion] ods-gen generated "
...
private:
  ...
  bool isComplex(Value value) {
    return toolchain::isa<ComplexType>(value.getType());
  }
  bool isFloatingPoint(Value value) {
    return toolchain::isa<FloatType>(value.getType());
  }
  bool isInteger(Value value) {
    return toolchain::isa<IntegerType>(value.getType());
  }
...
struct FoldInsertPadIntoFill : public OpRewritePattern<tensor::InsertSliceOp> {
  ...
  LogicalResult matchAndRewrite(tensor::InsertSliceOp insertOp,
    ...
    for (const auto &p : toolchain::zip(lowPads, oldOffsets)) {
...
struct FoldConcatsOfFill : public OpRewritePattern<tensor::ConcatOp> {
  ...
  LogicalResult matchAndRewrite(tensor::ConcatOp concatOp,
    ...
    if (!toolchain::all_of(concatOperands.drop_front(),
...
void GenericOp::print(OpAsmPrinter &p) {
  ...
  toolchain::StringSet<> genericAttrNamesSet;
  ...
  for (auto attr : (*this)->getAttrs()) {
    if (attr.getName() == getIteratorTypesAttrName()) {
      ...
      SmallVector<Attribute> iteratorTypeNames =
          toolchain::to_vector(toolchain::map_range(
...
ParseResult GenericOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // verifier.
  toolchain::SMLoc attributeLocation = parser.getCurrentLocation();
...
static void getGenericEffectsImpl(
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
        &effects,
    LinalgOp linalgOp) {
  for (auto [index, operand] : toolchain::enumerate(linalgOp.getDpsInputs())) {
    if (!toolchain::isa<MemRefType>(operand.getType()))
      continue;
  ...
  for (OpOperand &operand : linalgOp.getDpsInitsMutable()) {
    if (!toolchain::isa<MemRefType>(operand.get().getType()))
      continue;
...
struct EraseIdentityLinalgOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy linalgOp,
    ...
    if (!toolchain::all_equal(linalgOp.getIndexingMapsArray()))
      return failure();
    ...
    for (const auto &yieldVal : toolchain::enumerate(yieldOp.getValues())) {
      auto yieldArg = toolchain::dyn_cast<BlockArgument>(yieldVal.value());
      if (!yieldArg || yieldArg.getOwner() != &body)
        return failure();
...
static ParseResult parseDstStyleOp(
  ...
  for (Type outputType : outputTypes) {
    if (toolchain::isa<RankedTensorType>(outputType))
...
void MapOp::build(
  ...
  if (toolchain::isa<RankedTensorType>(initType))
...
static void addBodyWithPayloadOp(OpAsmParser &parser, OperationState &result,
  ...
  for (auto &operand : operands) {
    block.addArgument(
        toolchain::cast<ShapedType>(operand.getType()).getElementType(),
        b.getUnknownLoc());
  }
  ...
  Operation *payloadOp = b.create(
      ...
      payloadOpOperands,
      TypeRange{toolchain::cast<ShapedType>(result.operands.back().getType())
                    .getElementType()},
      payloadOpAttrs);
...
static bool canUseShortForm(Block *body, bool initFirst = false,
  ...
  if (initFirst) {
    ...
    for (const auto &[operand, bbArg] :
         toolchain::zip(payload.getOperands(), body->getArguments().drop_front())) {
      if (bbArg != operand)
        return false;
    }
  } else {
    for (const auto &[operand, bbArg] :
         toolchain::zip(payload.getOperands(),
...
static void printShortForm(OpAsmPrinter &p, Operation *payloadOp) {
  ...
  for (const auto &attr : payloadOp->getAttrs()) {
    auto fastAttr =
        toolchain::dyn_cast<mlir::arith::FastMathFlagsAttr>(attr.getValue());
...
void MapOp::print(OpAsmPrinter &p) {
  ...
  if (!useShortForm) {
    ...
    p.printNewline();
    p << "(";
    toolchain::interleaveComma(mapper->getArguments(), p,
...
LogicalResult MapOp::verify() {
  ...
  for (const auto &[bbArgType, inputArg] :
       toolchain::zip(bodyBlock->getArgumentTypes(), getInputs())) {
    auto inputElemType =
        toolchain::cast<ShapedType>(inputArg.getType()).getElementType();
  ...
  for (Type inputArgType : TypeRange{getInputs()}) {
    auto inputElemShape = toolchain::cast<ShapedType>(inputArgType).getShape();
...
SmallVector<utils::IteratorType> ReduceOp::getIteratorTypesArray() {
  int64_t inputRank =
      toolchain::cast<ShapedType>(getInputs()[0].getType()).getRank();
...
LogicalResult ReduceOp::verify() {
  ...
  for (int64_t i = 1; i < getNumDpsInputs(); ++i) {
    if (toolchain::cast<ShapedType>(getInputs()[i].getType()).getShape() !=
        toolchain::cast<ShapedType>(getInputs()[0].getType()).getShape()) {
  ...
  for (int64_t i = 1; i < getNumDpsInits(); ++i) {
    if (toolchain::cast<ShapedType>(getInits()[i].getType()).getShape() !=
        toolchain::cast<ShapedType>(getInits()[0].getType()).getShape()) {
  ...
  auto inputType = toolchain::cast<ShapedType>(getInputs()[0].getType());
  auto initType = toolchain::cast<ShapedType>(getInits()[0].getType());
  ...
  for (const auto &en : toolchain::enumerate(inputDims)) {
  ...
  for (auto [input, bbArg] : toolchain::zip(getInputs(), block->getArguments())) {
    Type inputElementType =
        toolchain::cast<ShapedType>(input.getType()).getElementType();
  ...
  for (auto [output, bbArg] : toolchain::zip(
           getDpsInits(), block->getArguments().take_back(getNumDpsInits()))) {
    auto outputElementType =
        toolchain::cast<ShapedType>(output.getType()).getElementType();
...
ArrayAttr TransposeOp::getIndexingMaps() {
  ...
  return builder.getAffineMapArrayAttr(
      {inversePermutation(AffineMap::getPermutationMap(
           toolchain::to_vector_of<unsigned>(getPermutation()), getContext())),
...
LogicalResult BroadcastOp::verify() {
  ...
  for (const auto &[idx, dim] : toolchain::enumerate(dimensionsRef)) {
  ...
  for (auto dim : toolchain::seq<int64_t>(0, initRank)) {
    if (!toolchain::is_contained(dimensionsRef, dim))
      dimMap.push_back(dim);
  }

  for (const auto &[inputDimIdx, initDimIdx] : toolchain::enumerate(dimMap)) {
...
struct FoldBroadcasts : OpRewritePattern<linalg::BroadcastOp> {
  ...
  LogicalResult matchAndRewrite(linalg::BroadcastOp broadcastOp,
    ...
    SmallVector<int64_t> dimMap;
    for (auto dim : toolchain::seq<int64_t>(0, initRank)) {
      if (!toolchain::is_contained(dimensions, dim))
        dimMap.push_back(dim);
    }
    for (auto dim : defDimensions)
      foldedDims.push_back(dimMap[dim]);

    toolchain::sort(foldedDims);
    rewriter.replaceOpWithNewOp<BroadcastOp>(
...
SmallVector<AffineExpr, 4> mlir::linalg::concat(ArrayRef<AffineExpr> a,
  ...
  auto rangeA = toolchain::make_range(a.begin(), a.end());
  auto rangeB = toolchain::make_range(b.begin(), b.end());
  auto concatRanges = toolchain::concat<const AffineExpr>(rangeA, rangeB);
  return toolchain::to_vector<4>(concatRanges);
}

static LogicalResult appendMangledType(toolchain::raw_string_ostream &ss, Type t) {
  if (auto memref = toolchain::dyn_cast<MemRefType>(t)) {
    ss << "view";
    ...
    if (auto as = memref.getMemorySpace()) {
      if (auto attr = toolchain::dyn_cast<IntegerAttr>(as))
        ss << "as" << attr.getInt();
      else
        return failure();
    }
  ...
  if (auto vec = toolchain::dyn_cast<VectorType>(t)) {
    ss << "vector";
...
std::string mlir::linalg::generateLibraryCallName(Operation *op) {
  ...
  for (NamedAttribute kv : op->getAttrs()) {
    if (UnaryFnAttr ufa = toolchain::dyn_cast<UnaryFnAttr>(kv.getValue())) {
      fun = stringifyEnum(ufa.getValue()).str() + "_";
    } else if (BinaryFnAttr bfa = toolchain::dyn_cast<BinaryFnAttr>(kv.getValue())) {
      fun = stringifyEnum(bfa.getValue()).str() + "_";
    }
  }
  name.reserve(128);
  toolchain::replace(name, '.', '_');
  toolchain::raw_string_ostream ss(name);
  ss << "_" << fun;
...
struct EraseDeadLinalgOp : public OpInterfaceRewritePattern<LinalgOp> {
  ...
  LogicalResult matchAndRewrite(LinalgOp op,
    ...
    for (OpOperand &opOperand : op->getOpOperands()) {
      ...
      auto mt = toolchain::dyn_cast<MemRefType>(opOperand.get().getType());
      if (!mt)
        continue;
      if (toolchain::is_contained(op.getShape(&opOperand), 0)) {
        rewriter.eraseOp(op);
        return success();
      }
    }
...
struct FoldTensorCastConsumerOp : public OpRewritePattern<tensor::CastOp> {
  ...
  LogicalResult matchAndRewrite(tensor::CastOp castOp,
    ...
    OpResult resultValue = toolchain::cast<OpResult>(castOp.getSource());
...
static void populateMap(LinalgOp linalgOp, MutableArrayRef<OpOperand> operands,
                        toolchain::DenseMap<AffineExpr, int64_t> &affineExprToSize) {
  for (OpOperand &opOperand : operands) {
    if (linalgOp.isScalar(&opOperand))
      continue;
    Value src = opOperand.get();
    auto sourceType = toolchain::cast<RankedTensorType>(src.getType());
    ...
    if (parentOp) {
      if (auto castOp = dyn_cast<tensor::CastOp>(parentOp)) {
        ...
        auto castSourceType =
            toolchain::dyn_cast<RankedTensorType>(castSource.getType());
...
static void createNewOperandWithStaticSizes(
    ...
    toolchain::DenseMap<AffineExpr, int64_t> &affineExprToSize, LinalgOp linalgOp,
...
struct InferStaticShapeOfOperands : public OpInterfaceRewritePattern<LinalgOp> {
  ...
  LogicalResult matchAndRewrite(LinalgOp linalgOp,
    ...
    if (toolchain::any_of(linalgOp.getIndexingMapsArray(), [](AffineMap map) {
    ...
    for (auto it : toolchain::zip(linalgOp->getResults(), newOp->getResults())) {
...
SmallVector<Range> SoftmaxOp::getIterationDomain(OpBuilder &builder) {
  ...
  for (auto dim : toolchain::seq<int64_t>(0, operandRank)) {
...
SoftmaxOp::getTiledImplementation(OpBuilder &builder,
  ...
  return TilingResult{
      ...
      toolchain::to_vector(ArrayRef<Operation *>{inputSlice, outputSlice})};
}
...
SoftmaxOp::reifyResultShapes(OpBuilder &b,
  ...
  auto inputShapedType = toolchain::cast<ShapedType>(getInputOperandType());
  auto outputShapedType = toolchain::cast<ShapedType>(getOutputOperandType());
  for (int64_t dim : toolchain::seq<int64_t>(0, getOutputOperandRank())) {
...
void SoftmaxOp::getEffects(
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
        &effects) {
  for (auto [index, operand] : toolchain::enumerate(getDpsInputs())) {
    if (!toolchain::isa<MemRefType>(operand.getType()))
      continue;
...
FailureOr<TilingResult> WinogradFilterTransformOp::getTiledImplementation(
  ...
  return TilingResult{
      {tiledOp},
      SmallVector<Value>(tiledOp->getResults()),
      toolchain::to_vector(ArrayRef<Operation *>{filterSlice, outputSlice})};
}
...
FailureOr<TilingResult> WinogradOutputTransformOp::getTiledImplementation(
  ...
  return TilingResult{
      {tiledOp},
      SmallVector<Value>(tiledOp->getResults()),
      toolchain::to_vector(ArrayRef<Operation *>{valueSlice, outputSlice})};
}
...
static bool areResultExprsSubsetOf(AffineMap subMap, AffineMap fullMap) {
  ...
  toolchain::set_union(explicitSet, defaultSet);
...
void MatmulOp::regionBuilder(ImplicitLocOpBuilder &b, Block &block,
  ...
  const auto *castIter = toolchain::find_if(attrs, [&](const NamedAttribute &attr) {
    return attr.getName() == "cast";
  });
  if (castIter != attrs.end()) {
    if (auto attr = toolchain::dyn_cast<TypeFnAttr>(castIter->getValue()))
...
ParseResult MatmulOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (*indexingMapsAttr == nullptr) {
    auto indexingMapAttrs = toolchain::map_to_vector(
...
void MatmulOp::print(OpAsmPrinter &p) {
  SmallVector<Attribute, 3> indexingMaps = toolchain::map_to_vector<3>(
      MatmulOp::getDefaultIndexingMaps(getContext()),
      [](AffineMap map) -> Attribute { return AffineMapAttr::get(map); });
  if (!toolchain::equal(getIndexingMaps(), indexingMaps))
    p << " indexing_maps = " << toolchain::interleaved_array(getIndexingMaps());
...
void ContractOp::regionBuilder(ImplicitLocOpBuilder &b, Block &block,
  ...
  auto castIter = toolchain::find_if(attrs, [&](const NamedAttribute &attr) {
    return attr.getName() == "cast";
  });
  if (castIter != attrs.end()) {
    if (auto attr = toolchain::dyn_cast<TypeFnAttr>(castIter->getValue()))
...
LogicalResult ContractOp::verify() {
  ...
  for (auto &&[affineMap, operandType, isInput] :
       toolchain::zip(getIndexingMapsArray(), getOperandTypes(),
...
ParseResult BatchMatmulOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (indexingMapsAttr.empty()) {
    indexingMapsAttr = toolchain::map_to_vector(
...
ParseResult ElementwiseOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (indexingMapsAttr.empty()) {
    ...
    auto shapedType = toolchain::dyn_cast<ShapedType>(resultType);
    if (!shapedType)
      return parser.emitError(parser.getCurrentLocation(),
...
getNewMixedTileSizes(PatternRewriter &rewriter, Type newPackedTy,
  ...
  for (auto it : toolchain::zip(cast<ShapedType>(newPackedTy)
                               .getShape()
    ...
    if (Attribute attr = toolchain::dyn_cast_if_present<Attribute>(tile)) {
      // Already a constant
...
reifyResultShapesImpl(OpTy op, OpBuilder &builder,
                      ReifiedRankedShapedTypeDims &reifiedReturnShapes) {
  static_assert(toolchain::is_one_of<OpTy, PackOp, UnPackOp>::value,
...
static DenseMap<int64_t, OpFoldResult> getDimAndTileMappingImpl(OpTy op) {
  ...
  for (auto i : toolchain::seq<int64_t>(0, dimsToTile.size()))
...
static bool isInvalidPackingPosSpecification(ArrayRef<int64_t> dimsPos,
  ...
  if (dimsPosSize > rank)
    return true;
  DenseSet<int64_t> uniqued(toolchain::from_range, dimsPos);
  if (dimsPosSize != uniqued.size())
    return true;
  return toolchain::any_of(dimsPos, [rank](int64_t dimPos) {
...
static LogicalResult commonVerifierPackAndUnPackOp(OpTy packOrUnPack) {
  ...
  auto hasZeros = [&](ArrayRef<OpFoldResult> tiles) {
    return toolchain::any_of(tiles, isZeroInteger);
  };
  ...
  if (!toolchain::all_of(
          toolchain::zip(packedType.getShape().take_back(mixedTiles.size()),
                    mixedTiles),
          [](std::tuple<int64_t, OpFoldResult> it) {
            int64_t shape = std::get<0>(it);
            if (Attribute attr =
                    toolchain::dyn_cast_if_present<Attribute>(std::get<1>(it))) {
...
commonPermutationOfPackAndUnPackOp(OpTy packOrUnPackOp,
  ...
  metadata.outerDimsPerm =
      packOrUnPackOp.getOuterDimsPerm().empty()
          ? toolchain::to_vector(toolchain::seq<int64_t>(0, numOuterDims))
...
bool PackOp::requirePaddingValue(ArrayRef<int64_t> inputShape,
  ...
  for (auto [pos, tileSize] : toolchain::zip_equal(innerDimsPos, innerTiles)) {
...
asShapeWithAnyValueAsDynamic(ArrayRef<OpFoldResult> ofrs) {
  ...
  for (auto o : ofrs) {
    ...
    if (toolchain::dyn_cast_if_present<Value>(o))
...
static SmallVector<int64_t> getPackOpResultTypeShape(
  ...
  SmallVector<int64_t> resultShape = toolchain::to_vector(sourceShape);
  for (auto tiledDim : toolchain::enumerate(toolchain::to_vector(innerDimsPos))) {
    ...
    resultShape[tiledDim.value()] = toolchain::divideCeilSigned(
...
SmallVector<OpFoldResult> PackOp::getResultShape(
    ...
    ArrayRef<int64_t> outerDimsPerm) {
  SmallVector<OpFoldResult> resultDims = toolchain::to_vector(sourceDims);
...
Value PackOp::createDestinationTensor(OpBuilder &b, Location loc, Value source,
  ...
  for (auto [index, value] : toolchain::enumerate(
           toolchain::cast<RankedTensorType>(source.getType()).getShape())) {
  ...
  for (auto it : toolchain::zip(innerDimsPos, innerTileSizes)) {
  ...
  auto elemType = toolchain::cast<ShapedType>(source.getType()).getElementType();
...
static bool areTilesAndTiledDimsAllConstant(OpTy op) {
  ...
  for (auto [dimDest, tile] : toolchain::zip(
...
static bool paddingIsNotNeeded(PackOp op) {
  ...
  if (toolchain::any_of(op.getInnerDimsPos(),
...
static bool inferStaticShape(PackOp packOp, SmallVectorImpl<int64_t> &srcShape,
  ...
  toolchain::SmallSetVector<int64_t, 4> innerDims;
  ...
  for (auto i : toolchain::seq<int64_t>(0, srcRank)) {
...
static bool isLikePadUnPad(PackOrUnpackOp packOp,
  ...
  auto orderedDims = toolchain::to_vector<4>(toolchain::seq<int64_t>(0, numPackedDims));
  ...
  return toolchain::all_of(
      toolchain::seq<int64_t>(0, packedRank - numPackedDims),
      [&packedShape](int64_t i) { return packedShape[i] == 1; });
}

bool PackOp::isLikePad() {
  auto packedTensorType =
      toolchain::cast<RankedTensorType>((*this)->getResultTypes().front());
...
Value UnPackOp::createDestinationTensor(OpBuilder &b, Location loc,
  ...
  SmallVector<OpFoldResult> mixedSizes;
  auto srcType = toolchain::cast<RankedTensorType>(source.getType());
  for (auto i :
       toolchain::seq<unsigned>(0, srcType.getRank() - innerTileSizes.size())) {
  ...
  for (auto [dimPos, tileSize] : toolchain::zip_equal(innerDimsPos, innerTileSizes))
...
static bool inferStaticShape(UnPackOp op, SmallVectorImpl<int64_t> &srcShape,
  ...
  for (auto i : toolchain::seq<int64_t>(0, destRank)) {
    if (innerDims.contains(i))
      continue;
    int64_t srcPos = i;
    int64_t destPos = i;
...
bool UnPackOp::canFoldSliceOp(tensor::ExtractSliceOp sliceOp) {
  ...
  for (auto [pos, tileSize] :
       toolchain::zip_equal(this->getInnerDimsPos(), this->getStaticInnerTiles())) {
...
void BatchReduceMatmulOp::print(OpAsmPrinter &p) {
  SmallVector<Attribute, 3> indexingMaps = toolchain::map_to_vector(
      BatchReduceMatmulOp::getDefaultIndexingMaps(getContext()),
      [](AffineMap map) -> Attribute { return AffineMapAttr::get(map); });

  if (!toolchain::equal(getIndexingMaps(), indexingMaps)) {
    p << " indexing_maps = [";
    toolchain::interleaveComma(getIndexingMaps(), p,
