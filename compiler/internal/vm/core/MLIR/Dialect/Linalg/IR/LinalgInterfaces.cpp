...
bool linalg::detail::canOpOperandsBeDroppedImpl(
  ...
  for (auto &opOperand : linalgOp->getOpOperands()) {
    if (toolchain::is_contained(droppedOperands, &opOperand))
      continue;
...
linalg::isaBroadcastOpInterface(GenericOp op) {
  ...
  for (unsigned i = 0; i < srcMap.getNumResults(); ++i) {
    auto expr = toolchain::dyn_cast<AffineDimExpr>(srcMap.getResults()[i]);
    if (!expr)
      return std::nullopt;
  ...
  for (auto dim : toolchain::seq<int64_t>(0, numDims)) {
    if (!toolchain::is_contained(position, dim))
...
linalg::isaTransposeOpInterface(GenericOp op) {
  ...
  for (unsigned i = 0; i < mapOfInput.getNumDims(); ++i) {
    auto expr = toolchain::cast<AffineDimExpr>(mapOfInput.getResults()[i]);
...
static bool isaElemwiseSingleUnaryOrBinaryOpInterface(linalg::GenericOp op,
  ...
  if (op.getNumDpsInputs() != arity || op.getNumDpsInits() != 1 ||
      !toolchain::all_of(op.getIndexingMapsArray(),
...
static toolchain::SmallDenseSet<int64_t>
findPermutationsIndexingOperand(AffineMap indexingMap,
  ...
  toolchain::SmallDenseSet<int64_t> res;
  for (AffineExpr e : indexingMap.getResults()) {
    if (auto d = dyn_cast<AffineDimExpr>(e)) {
      if (iterators[d.getPosition()] == iter &&
          toolchain::count_if(indexingMap.getResults(), [d](AffineExpr e) {
...
inferContractionDimsImpl(ArrayRef<AffineMap> indexingMaps,
  ...
  toolchain::SmallDenseSet<int64_t> ac = a;
  toolchain::set_intersect(ac, c);
  toolchain::set_subtract(ac, b);
  // B & C - A are the iterators involved in an outer-product along B (the RHS).
  toolchain::SmallDenseSet<int64_t> bc = b;
  toolchain::set_intersect(bc, c);
  toolchain::set_subtract(bc, a);
  // A & B & C are the "batch" dimensions.
  toolchain::SmallDenseSet<int64_t> batches = a;
  toolchain::set_intersect(batches, b);
  toolchain::set_intersect(batches, c);

  // A & B red are the reduction dimensions.
  toolchain::SmallDenseSet<int64_t> ra =
      findPermutationsIndexingOperand(indexingMaps[0], iterators, red);
  toolchain::SmallDenseSet<int64_t> rb =
      findPermutationsIndexingOperand(indexingMaps[1], iterators, red);
  toolchain::set_intersect(ra, rb);
  ...
  toolchain::sort(dimensions.batch);
  toolchain::sort(dimensions.m);
  toolchain::sort(dimensions.n);
  toolchain::sort(dimensions.k);
  return dimensions;
}
...
struct ConvAccessExprWalker
  ...
  toolchain::SmallDenseSet<int64_t> convolvedDims;
  // Stores the dual mapping between LHS and RHS of convolution exprs.
  toolchain::SmallDenseMap<int64_t, int64_t> convolvedDimMapping;
  // Stores single use dimensions used by an AffineDimExpr.
  toolchain::SmallDenseSet<int64_t> unConvolvedDims;
  // Stores a mapping from convolved dims to their coefficient.
  toolchain::SmallDenseMap<int64_t, AffineExpr> strideAndDilationMapping;
  ...
  void clearMultiUseDims(AffineMap map) {
    for (int dimPos = 0, e = map.getNumDims(); dimPos < e; ++dimPos) {
      if (toolchain::count_if(map.getResults(), [dimPos](AffineExpr e) {
...
static toolchain::SmallDenseSet<int64_t> getPreservedDims(AffineMap map) {
  assert(map.isProjectedPermutation() &&
         "expected map to have projected permutations");
  toolchain::SmallDenseSet<int64_t> preservedDims;
  for (auto expr : map.getResults())
...
inferConvolutionDimsImpl(LinalgOp linalgOp,
  ...
  toolchain::SmallDenseSet<int64_t> filterDims = findPermutationsIndexingOperand(
      filterMap, linalgOp.getIteratorTypesArray(), par);
  toolchain::SmallDenseSet<int64_t> outputDims = findPermutationsIndexingOperand(
      outputMap, linalgOp.getIteratorTypesArray(), par);

  // unConvolvedDims & outputDims - filterDims are the batch iterators.
  toolchain::SmallDenseSet<int64_t> batch = inputExprWalker.unConvolvedDims;
  toolchain::set_intersect(batch, outputDims);
  toolchain::set_subtract(batch, filterDims);

  // convolvedDims & outputDims are the output image iterators.
  toolchain::SmallDenseSet<int64_t> oi = inputExprWalker.convolvedDims;
  toolchain::set_intersect(oi, outputDims);

  // filterDims & outputDims - unConvolvedDims are the output channel iterators.
  toolchain::SmallDenseSet<int64_t> oc = filterDims;
  toolchain::set_intersect(oc, outputDims);
  toolchain::set_subtract(oc, inputExprWalker.unConvolvedDims);

  // filterDims & outputDims & unConvolvedDims are the depth iterators.
  toolchain::SmallDenseSet<int64_t> depth = filterDims;
  toolchain::set_intersect(depth, outputDims);
  toolchain::set_intersect(depth, inputExprWalker.unConvolvedDims);

  toolchain::SmallDenseSet<int64_t> filterReducedDims =
  ...
  toolchain::SmallDenseSet<int64_t> fl = inputExprWalker.convolvedDims;
  toolchain::set_intersect(fl, filterReducedDims);

  // unConvolvedDims & filterReducedDims are the input channel iterators.
  toolchain::SmallDenseSet<int64_t> ic = inputExprWalker.unConvolvedDims;
  toolchain::set_intersect(ic, filterReducedDims);
  ...
  toolchain::sort(dimensions.batch);
  toolchain::sort(dimensions.outputImage);
  toolchain::sort(dimensions.outputChannel);
  toolchain::sort(dimensions.filterLoop);
  toolchain::sort(dimensions.inputChannel);
  toolchain::sort(dimensions.depth);
  ...
  if (!nativeStrides) {
    ...
    dimensions.strides = toolchain::to_vector<2>(nativeStrides.getValues<int64_t>());
  }
  ...
  if (!nativeDilations) {
    ...
    dimensions.dilations =
        toolchain::to_vector<2>(nativeDilations.getValues<int64_t>());
  }
  return dimensions;
}
...
mlir::linalg::detail::isConvolutionInterfaceImpl(
  ...
  // Check the input indexing map has the right form.
  ConvAccessExprWalker inputExprWalker;
  if (toolchain::any_of(indexingMaps[0].getResults(),
  ...
  toolchain::SmallDenseSet<int64_t> outputDims =
      getPreservedDims(indexingMaps.back());
  toolchain::SmallDenseSet<int64_t> filterDims = getPreservedDims(indexingMaps[1]);
  ...
  //   filter.
  toolchain::SmallDenseSet<int64_t> allLoopDims;
...
SmallVector<int64_t, 4> LinalgOp::createFlatListOfOperandStaticDims() {
  ...
  for (OpOperand &opOperand : getOperation()->getOpOperands())
    toolchain::append_range(res, getShape(&opOperand));
  return res;
}
...
struct HasAffineDimExprVisitor
  ...
  HasAffineDimExprVisitor(toolchain::SmallBitVector positions)
...
private:
  toolchain::SmallBitVector positions;
};
...
LinalgOp::reifyResultShapes(OpBuilder &b,
  ...
  // to the outputs.
  toolchain::SmallBitVector outputDims(resultShapesFromInputShapesMap.getNumDims());
  ...
  for (OpOperand &opOperand : getDpsInitsMutable()) {
    ...
    for (int64_t dim : toolchain::seq<int64_t>(0, getRank(&opOperand))) {
      auto shapedType = toolchain::cast<ShapedType>(opOperand.get().getType());
