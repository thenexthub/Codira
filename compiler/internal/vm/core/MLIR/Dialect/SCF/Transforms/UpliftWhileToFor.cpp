...
FailureOr<scf::ForOp> mlir::scf::upliftWhileToForLoop(RewriterBase &rewriter,
  ...
  if (!toolchain::hasSingleElement(beforeBody->without_terminator()))
  ...
  if (ValueRange(beforeBody->getArguments()) != beforeTerm.getArgs()) {
    auto getArgReordering =
      ...
      toolchain::SmallBitVector forwarded(beforeBody->getNumArguments(), false);
  ...
  // arg.
  if (!toolchain::hasNItems(inductionVar.getUses(), 2))
    return rewriter.notifyMatchFailure(loop, [&](Diagnostic &diag) {
  ...
  auto findAfterArgNo = [](ArrayRef<unsigned> indices, unsigned beforeArgNo) {
    return std::distance(indices.begin(),
                         toolchain::find_if(indices, [beforeArgNo](unsigned n) {
  ...
  for (auto &&[i, init] : toolchain::enumerate(loop.getInits())) {
    if (i == argNumber)
      continue;
  ...
  for (auto i : toolchain::seq<size_t>(0, newBodyArgs.size())) {
    if (i < argNumber) {
  ...
  newArgs.clear();
  for (auto &&[i, arg] : toolchain::enumerate(term.getResults())) {
    if (i == argNumber)
      continue;
  ...
  // into proper place.
  newArgs.clear();
  toolchain::append_range(newArgs, newLoop.getResults());
