...
static bool doesNotAliasExternalValue(Value value, Region *region,
  ...
  state.applyOnAliases(value, [&](Value alias) {
    if (toolchain::is_contained(exceptions, alias))
      return;
...
struct ConditionOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    for (const auto &it : toolchain::enumerate(conditionOp.getArgs())) {
...
struct ExecuteRegionOpInterface
  ...
  getAliasingOpOperands(Operation *op, Value value,
    ...
    auto executeRegionOp = cast<scf::ExecuteRegionOp>(op);
    auto it = toolchain::find(op->getOpResults(), value);
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    for (const auto &it : toolchain::enumerate(executeRegionOp->getResultTypes())) {
...
struct IndexSwitchOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    // Move over blocks.
    for (auto [src, dest] :
         toolchain::zip(switchOp.getCaseRegions(), newSwitchOp.getCaseRegions()))
...
getBbArgReplacements(RewriterBase &rewriter, Block::BlockArgListType bbArgs,
  ...
  SmallVector<Value> result;
  for (const auto &it : toolchain::enumerate(bbArgs)) {
    size_t idx = it.index();
    Value val = it.value();
...
static FailureOr<BufferLikeType> computeLoopRegionIterArgBufferType(
  ...
  if (toolchain::count(invocationStack, iterArg) >= 2) {
  ...
  auto initBufferType = toolchain::cast<BaseMemRefType>(*initArgBufferType);
  ...
  if (auto yieldedRankedBufferType = dyn_cast<MemRefType>(yieldedBufferType)) {
    assert(
        toolchain::all_equal({yieldedRankedBufferType.getShape(),
...
struct ForOpInterface
  ...
  resolveConflicts(Operation *op, RewriterBase &rewriter,
    ...
    for (const auto it : toolchain::enumerate(yieldOp.getResults())) {
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    for (const auto &it : toolchain::enumerate(initArgs)) {
...
struct WhileOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    SmallVector<Type> argsTypesAfter = toolchain::to_vector(
        toolchain::map_range(whileOp.getAfterArguments(), [&](BlockArgument bbArg) {
          if (!isa<TensorType>(bbArg.getType()))
            return bbArg.getType();
          // TODO: error handling
          return toolchain::cast<Type>(
...
struct ForallOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    for (const auto &it : toolchain::zip(
  ...
  bool isRepetitiveRegion(Operation *op, unsigned index) const {
    ...
    for (auto [lb, ub, step] :
         toolchain::zip(forallOp.getMixedLowerBound(), forallOp.getMixedUpperBound(),
