...
scf::SCFTilingOptions::setTileSizes(ArrayRef<OpFoldResult> ts) {
  ...
  auto tileSizes = toolchain::to_vector(ts);
...
scf::SCFTilingOptions::setNumThreads(ArrayRef<OpFoldResult> nt) {
  ...
  auto numThreads = toolchain::to_vector(nt);
...
fillInterchangeVector(ArrayRef<int64_t> interchangeVector,
  ...
  SmallVector<int64_t> filledVector = toolchain::to_vector(interchangeVector);
  if (filledVector.size() < iterationDomainSize) {
    auto range = toolchain::seq<int64_t>(filledVector.size(), iterationDomainSize);
...
getUserTileSizesAndNumThreads(RewriterBase &rewriter, TilingInterface op,
  ...
  if (options.numThreadsComputationFunction) {
    ...
    for (auto [index, range, nt] :
         toolchain::enumerate(iterationDomain, numThreads)) {
      if (isZeroInteger(nt))
        continue;
...
static LogicalResult checkTileSizes(TilingInterface op,
  ...
  for (auto [index, iterator, givenTileSize] :
       toolchain::enumerate(iterators, givenTileSizes)) {
...
getTileOffsetAndSizes(RewriterBase &rewriter, Location loc, ValueRange ivs,
  ...
  for (auto [givenTileSize, loopRange] :
       toolchain::zip_equal(givenTileSizes, iterationDomain)) {
...
getLoopBounds(RewriterBase &rewriter, Location loc, ArrayRef<Range> loopRanges,
  ...
  for (auto [loopRange, givenTileSize] :
       toolchain::zip_equal(loopRanges, givenTileSizes)) {
...
static FailureOr<SmallVector<LoopLikeOpInterface>> generateLoopNestUsingForOp(
  ...
  for (auto [lb, ub, step] : toolchain::zip_equal(lbVals, ubVals, stepVals)) {
    auto loop =
  ...
  for (auto [tiledValue, destinationTensor, resultOffset, resultSize] :
       toolchain::zip_equal(tiledResults, innerDestinationTensors, resultOffsets,
                       resultSizes)) {
  ...
  for (auto [outerLoop, innerLoop] :
       toolchain::zip_equal(MutableArrayRef(loops).drop_back(),
...
getTileOffsetAndSizesWithForAllOp(RewriterBase &rewriter, Location loc,
  ...
  for (auto [index, nt, givenTileSize, loopRange] :
       toolchain::enumerate(numThreads, givenTileSizes, iterationDomain)) {
...
static FailureOr<SmallVector<LoopLikeOpInterface>> generateLoopNest(
  ...
  if (toolchain::all_of(givenTileSizes, isZeroInteger)) {
    SmallVector<Value> tiledResults;
    SmallVector<SmallVector<OpFoldResult>> resultOffsets, resultSizes;
    auto tileOffsets =
        toolchain::map_to_vector(loopRanges, [](Range r) { return r.offset; });
    auto tileSizes =
        toolchain::map_to_vector(loopRanges, [](Range r) { return r.size; });
...
static FailureOr<SmallVector<Value>> createInitialTensorsForTiling(
  ...
  for (auto [index, domain, tileSize] :
       toolchain::enumerate(iterationDomain, givenTileSizes)) {
    if (!numThreads.empty()) {
      // Untiled case.
...
getSplitReductionIvs(RewriterBase &rewriter, Location loc,
  ...
  if (reductionStrategy ==
      ReductionTilingStrategy::PartialReductionOuterParallel) {
    for (auto [index, reductionDim] : toolchain::enumerate(reductionDims)) {
      if (!numThreads.empty()) {
...
FailureOr<LoopLikeOpInterface> yieldTiledValuesAndReplaceLoop<scf::ForOp>(
  ...
  auto inits = toolchain::to_vector(loopOp.getInitArgs());
  ...
  SmallVector<Value> newYieldValues = toolchain::to_vector(yieldOp.getOperands());
  for (auto [tiledValue, regionIterArg, resultOffset, resultSize] :
       toolchain::zip_equal(tiledValues, newRegionIterArgs, resultOffsets,
                       resultSizes)) {
    SmallVector<OpFoldResult> resultStride(resultOffset.size(),
                                           rewriter.getIndexAttr(1));
...
FailureOr<LoopLikeOpInterface> yieldTiledValuesAndReplaceLoop<scf::ForallOp>(
  ...
  Location loc = loopOp.getLoc();
  rewriter.setInsertionPoint(loopOp);
  auto inits = toolchain::to_vector(loopOp.getOutputs());
  inits.append(newInitOperands.begin(), newInitOperands.end());
  ...
  for (auto [tiledValue, iterArg, resultOffset, resultSize] : toolchain::zip_equal(
...
static LogicalResult addInitOperandsToLoopNest(
  ...
  for (auto &loop : loops.drop_back()) {
    ...
    SmallVector<Value> newInits = toolchain::to_vector(forLoop.getInitArgs());
  ...
  // by the inner loop.
  for (auto [outerLoop, innerLoop] :
       toolchain::zip_equal(loops.drop_back(), loops.drop_front())) {
    ...
    SmallVector<Value> newYields =
        toolchain::to_vector(outerLoopYield.getOperands());
...
mlir::scf::tileUsingSCF(RewriterBase &rewriter, TilingInterface op,
  ...
  GenerateTiledBodyFn innerYieldTiledValuesFn =
    ...
    SmallVector<OpFoldResult> tileOffsetsVec = toolchain::to_vector(tileOffsets);
    SmallVector<OpFoldResult> tileSizesVec = toolchain::to_vector(tileSizes);
    ...
    for (auto [index, tiledValue] :
         toolchain::enumerate(tilingResult->tiledValues)) {
  ...
  auto loopResults = toolchain::map_to_vector(loops.front()->getResults(),
...
mlir::scf::tileReductionUsingScf(RewriterBase &b,
  ...
  for (auto [index, iteratorType] : toolchain::enumerate(op.getLoopIteratorTypes()))
...
mlir::scf::tileAndFuseProducerOfSlice(
  ...
  SmallVector<Value> candidateSliceOpOperands =
      toolchain::to_vector(candidateSliceOp->getOperands());
...
FailureOr<SmallVector<Operation *>> mlir::scf::yieldReplacementForFusedProducer(
  ...
  SmallVector<unsigned> initNumberList =
      yieldResultNumber.empty() ? toolchain::to_vector(toolchain::seq<unsigned>(
                                      0, originalOwner->getNumResults()))
                                : toolchain::to_vector(yieldResultNumber);
  ...
  YieldTiledValuesFn newYieldValuesFn =
    ...
    if (!toolchain::all_of(sliceOp.getMixedStrides(), isOneInteger))
      return failure();
    ...
    if (tilableOp->getNumResults() > 1 &&
      ...
      // https://github.com/toolchain/toolchain-project/pull/93144#discussion_r1643760814
      return failure();
    }
    ...
    if (auto tiledDestStyleOp =
      ...
      for (const auto &&[index, newRegionArg] :
           toolchain::enumerate(newRegionIterArgs)) {
    ...
    for (const auto &&[index, resultNumber] : toolchain::enumerate(initNumberList)) {
...
public:
  ...
  void updateReplacementValues(ValueRange origValues,
    ...
    for (auto &[key, val] : replacements) {
      for (auto [orig, replace] : toolchain::zip_equal(origValues, replaceValues)) {
        if (val == orig) {
          val = replace;
        }
      }
...
mlir::scf::tileConsumerAndFuseProducersUsingSCF(
  ...
  for (auto [origVal, replacement] :
       toolchain::zip_equal(consumer->getResults(), tilingResult->replacements)) {
  ...
  while (!sliceTracker.worklist.empty()) {
    ...
    if (worklistItem.controlFnResult.yieldProducerReplacement) {
      ...
      for (auto [index, result] :
           toolchain::enumerate(fusableProducerOp->getResults())) {
...
checkAssumptionForFusingConsumer(tensor::InsertSliceOp candidateSliceOp) {
  ...
  if (!toolchain::hasSingleElement(uses)) {
    LLVM_DEBUG(toolchain::dbgs() << "Too many uses of the candidate slice op\n");
    return failure();
  }
  ...
  if (result.getDefiningOp()->getBlock() != userOp->getBlock()) {
    LLVM_DEBUG(toolchain::dbgs() << "Expected tensor.insert_slice and scf.yield to "
...
static FailureOr<toolchain::SetVector<Operation *>>
checkAssumptionForLoop(Operation *loopOp, Operation *consumerOp,
  ...
  toolchain::SetVector<Operation *> slice;
...
static FailureOr<OpOperand *> getConsumerFromLoopUses(RewriterBase &rewriter,
  ...
  for (OpOperand &opOperand : val.getUses()) {
    ...
    FailureOr<toolchain::SetVector<Operation *>> slice =
...
tileAndFuseConsumerOfSlicesImpl(RewriterBase &rewriter, Operation *consumerOp,
  ...
  if (toolchain::any_of(consumerOpOperands, [&](OpOperand *opOperand) {
  ...
  SmallVector<Value> newInits = toolchain::to_vector(dstOp.getDpsInits());
  ...
  SmallVector<unsigned> operandNumbers =
      toolchain::map_to_vector(consumerOpOperands, [](OpOperand *opOperand) {
        return opOperand->getOperandNumber();
      });
  SmallVector<OpOperand *> clonedOpFusedOperandsList =
      toolchain::map_to_vector(operandNumbers, [&](unsigned operandNum) {
  ...
  rewriter.modifyOpInPlace(clonedConsumerOp, [&]() {
    for (auto [operandToReplace, clonedSliceOp] :
         toolchain::zip_equal(clonedOpFusedOperandsList, clonedInsertSlices)) {
  ...
  for (auto [operandNum, clonedSliceOp] :
       toolchain::zip_equal(operandNumbers, clonedInsertSlices)) {
  ...
  YieldTiledValuesFn newYieldValuesFn =
    ...
    for (auto candidateSliceOp : clonedInsertSlices) {
      ...
      if (!toolchain::all_of(strides, isOneInteger)) {
        return rewriter.notifyMatchFailure(
    ...
    SmallVector<SmallVector<OpFoldResult>> resultSizes(
        totalNumResultsOfConsumer);
    for (auto [idx, v] : toolchain::enumerate(tiledConsumerOp->getResults())) {
    ...
    for (const auto &&[index, result] :
         toolchain::enumerate(tiledConsumerOp->getResults())) {
  ...
  for (auto &&[oldResult, newResult] :
       toolchain::zip(consumerOp->getResults(),
  ...
  auto consumerOpOperandsVec = toolchain::to_vector(consumerOpOperands);
...
mlir::scf::tileAndFuseConsumerOfSlices(
  ...
  if (!(toolchain::all_of(candidateSlices, toolchain::IsaPred<tensor::InsertSliceOp>) ||
        toolchain::all_of(candidateSlices,
                     toolchain::IsaPred<tensor::ParallelInsertSliceOp>))) {
    return rewriter.notifyMatchFailure(
        candidateSlices.front(),
