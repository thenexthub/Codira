...
protected:
  ...
  DenseMap<Value, toolchain::SmallVector<Value>> valueMapping;
...
public:
  ...
  /// than its definition.
  toolchain::MapVector<Value, LiverangeInfo> analyzeCrossStageValues();
  scf::ForOp createKernelLoop(
      const toolchain::MapVector<Value, LiverangeInfo> &crossStageValues,
      RewriterBase &rewriter,
      toolchain::DenseMap<std::pair<Value, unsigned>, unsigned> &loopArgMap);
  ...
  LogicalResult createKernel(
      scf::ForOp newForOp,
      const toolchain::MapVector<Value, LiverangeInfo> &crossStageValues,
      const toolchain::DenseMap<std::pair<Value, unsigned>, unsigned> &loopArgMap,
      RewriterBase &rewriter);
  ...
  LogicalResult emitEpilogue(RewriterBase &rewriter,
                             toolchain::SmallVector<Value> &returnValues);
};
...
bool LoopPipelinerInternal::initializeLoopInfo(
  ...
  if (!upperBoundCst || !lowerBoundCst || !stepCst) {
    ...
    int64_t numIteration = toolchain::divideCeilSigned(ubImm - lbImm, stepImm);
  ...
  if (toolchain::any_of(forOp.getBody()->getTerminator()->getOperands(),
                   [this](Value operand) {
...
LogicalResult LoopPipelinerInternal::emitPrologue(RewriterBase &rewriter) {
  ...
  for (auto [arg, operand] :
       toolchain::zip(forOp.getRegionIterArgs(), forOp.getInitsMutable())) {
  ...
  for (int64_t i = 0; i < maxStage; i++) {
    ...
    for (Operation *op : opOrder) {
      ...
      for (unsigned destId : toolchain::seq(unsigned(0), op->getNumResults())) {
...
toolchain::MapVector<Value, LoopPipelinerInternal::LiverangeInfo>
LoopPipelinerInternal::analyzeCrossStageValues() {
  toolchain::MapVector<Value, LoopPipelinerInternal::LiverangeInfo> crossStageValues;
  for (Operation *op : opOrder) {
...
scf::ForOp LoopPipelinerInternal::createKernelLoop(
    const toolchain::MapVector<Value, LoopPipelinerInternal::LiverangeInfo>
        &crossStageValues,
    RewriterBase &rewriter,
    toolchain::DenseMap<std::pair<Value, unsigned>, unsigned> &loopArgMap) {
  ...
  toolchain::SmallVector<Value> newLoopArg;
  // For existing loop argument initialize them with the right version from the
  // prologue.
  for (const auto &retVal :
       toolchain::enumerate(forOp.getBody()->getTerminator()->getOperands())) {
...
LogicalResult LoopPipelinerInternal::createKernel(
  ...
  for (const auto &arg : toolchain::enumerate(forOp.getRegionIterArgs())) {
  ...
  for (Operation *op : opOrder) {
    ...
    if (predicates[useStage]) {
      ...
      for (auto values : toolchain::zip(op->getResults(), newOp->getResults()))
  ...
  toolchain::SmallVector<Value> yieldOperands;
...
LoopPipelinerInternal::emitEpilogue(RewriterBase &rewriter,
  ...
  for (int64_t i = 1; i <= maxStage; i++) {
    ...
    if (dynamicLoop) {
      ...
      // value.
      for (auto pair : toolchain::enumerate(returnValues)) {
...
void LoopPipelinerInternal::setValueMapping(Value key, Value el, int64_t idx) {
  ...
  if (it == valueMapping.end())
    it =
        valueMapping
            .insert(std::make_pair(key, toolchain::SmallVector<Value>(maxStage + 1)))
            .first;
  it->second[idx] = el;
}
