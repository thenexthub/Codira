...
SmallVector<scf::ForOp> mlir::replaceLoopNestWithNewYields(
  ...
  // https://discourse.toolchain.org/t/rfc-update-to-mlir-developer-policy-on-recursion/62235)
  ...
  NewYieldValuesFn fn =
    ...
    return toolchain::to_vector(toolchain::map_range(
...
FailureOr<func::FuncOp> mlir::outlineSingleBlockRegion(RewriterBase &rewriter,
          ...
          .take_front(numOriginalBlockArguments));
  {
    ...
    toolchain::append_range(callValues, newBlock->getArguments());
    toolchain::append_range(callValues, outlinedValues);
  ...
  for (auto it : toolchain::zip(outlinedValues, outlinedFuncBlockArgs.take_back(
                                               outlinedValues.size()))) {
...
FailureOr<UnrolledLoopInfo> mlir::loopUnrollByFactor(
  ...
  if (toolchain::hasSingleElement(forOp.getBody()->getOperations()))
  ...
  if (generateEpilogueLoop) {
    ...
    for (auto e : toolchain::zip(results, epilogueResults)) {
...
delinearizeInductionVariable(RewriterBase &rewriter, Location loc,
  ...
  if (linearizedIv.getType().isIndex()) {
    ...
    auto resultVals = toolchain::map_to_vector(
  ...
  toolchain::BitVector isUbOne(ubs.size());
  for (auto [index, ub] : toolchain::enumerate(ubs)) {
...
LogicalResult mlir::coalesceLoops(RewriterBase &rewriter,
  ...
  SmallVector<Value> upperBounds = toolchain::map_to_vector(
  ...
  for (int i = loops.size() - 1; i > 0; --i) {
    ...
    auto yieldedVals = toolchain::to_vector(innerTerminator->getOperands());
    assert(toolchain::equal(outerLoop.getRegionIterArgs(), innerLoop.getInitArgs()));
    for (Value &yieldedVal : yieldedVals) {
      ...
      auto iter = toolchain::find(innerLoop.getRegionIterArgs(), yieldedVal);
    ...
    toolchain::append_range(innerBlockArgs, outerLoop.getRegionIterArgs());
...
LogicalResult mlir::coalescePerfectlyNestedSCFForLoops(scf::ForOp op) {
  ...
  for (unsigned i = 1, e = loops.size(); i < e; ++i) {
    ...
    if (!toolchain::equal(outerloop.getRegionIterArgs(), innerLoop.getInitArgs())) {
      continue;
    }
    auto outerloopTerminator = outerloop.getBody()->getTerminator();
    if (!toolchain::equal(outerloopTerminator->getOperands(),
  ...
  for (unsigned end = loops.size(); end > 0; --end) {
    ...
    for (; start < end - 1; ++start) {
      ...
      if (iterArgChainStart[end - 1] > start)
        continue;
      auto band = toolchain::MutableArrayRef(loops.data() + start, end - start);
...
void mlir::collapseParallelLoops(
  ...
  auto sortedDimensions = toolchain::to_vector<3>(combinedDimensions);
...
SmallVector<Loops, 8> mlir::tile(ArrayRef<scf::ForOp> forOps,
  ...
  for (auto it : toolchain::zip(forOps, sizes)) {
...
Loops mlir::tile(ArrayRef<scf::ForOp> forOps, ArrayRef<Value> sizes,
  ...
  for (auto loops : tile(forOps, sizes, ArrayRef<scf::ForOp>(target)))
    res.push_back(toolchain::getSingleElement(loops));
  return res;
}
...
scf::ForallOp mlir::fuseIndependentSiblingForallLoops(scf::ForallOp target,
  ...
  toolchain::append_range(fusedOuts, target.getOutputs());
  toolchain::append_range(fusedOuts, source.getOutputs());
...
scf::ForOp mlir::fuseIndependentSiblingForLoops(scf::ForOp target,
  ...
  toolchain::append_range(fusedInitArgs, target.getInitArgs());
  toolchain::append_range(fusedInitArgs, source.getInitArgs());
...
FailureOr<scf::ForallOp> mlir::normalizeForallOp(RewriterBase &rewriter,
  ...
  for (auto [idx, iv] :
       toolchain::enumerate(normalizedForallOp.getInductionVars())) {
...
bool mlir::isPerfectlyNestedForLoops(
  ...
  for (auto [outerLoop, innerLoop] :
    ...
    for (auto [outerBBArg, innerIterArg] :
         toolchain::zip_equal(outerBBArgs, innerIterArgs)) {
      if (!toolchain::hasSingleElement(outerBBArg.getUses()) ||
    ...
    for (auto [outerYield, innerResult] :
         toolchain::zip_equal(outerYields, innerResults)) {
      if (!toolchain::hasSingleElement(innerResult.getUses()) ||
...
mlir::getConstLoopTripCounts(mlir::LoopLikeOpInterface loopOp) {
  ...
  if (!loBnds || !upBnds || !steps)
    return {};
  toolchain::SmallVector<int64_t> tripCounts;
  for (auto [lb, ub, step] : toolchain::zip(*loBnds, *upBnds, *steps)) {
    std::optional<toolchain::APInt> numIter = constantTripCount(
...
FailureOr<scf::ParallelOp> mlir::parallelLoopUnrollByFactors(
  ...
  assert(toolchain::none_of(unrollFactors, [](uint64_t f) { return f == 0; }) &&
  ...
  if (toolchain::all_of(unrollFactors, [](uint64_t f) { return f == 1; }))
    return rewriter.notifyMatchFailure(
        op, "Unrolling not applied if all factors are 1");

  // Return if the loop body is empty.
  if (toolchain::hasSingleElement(op.getBody()->getOperations()))
  ...
  const toolchain::SmallVector<int64_t> tripCounts = getConstLoopTripCounts(op);
  if (tripCounts.empty())
    return rewriter.notifyMatchFailure(
  ...
  toolchain::SmallVector<size_t> steps{};
