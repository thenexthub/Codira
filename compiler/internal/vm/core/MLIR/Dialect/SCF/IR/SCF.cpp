...
struct SCFInlinerInterface : public DialectInlinerInterface {
  ...
  void handleTerminator(Operation *op, ValueRange valuesToRepl) const final {
    ...
    for (auto retValue : toolchain::zip(valuesToRepl, retValOp.getOperands())) {
...
std::optional<toolchain::APSInt> mlir::scf::computeUbMinusLb(Value lb, Value ub,
                                                        bool isSigned) {
  toolchain::APSInt diff;
...
LogicalResult ForOp::verifyRegions() {
  ...
  unsigned i = 0;
  for (auto e : toolchain::zip(initArgs, iterArgs, opResults)) {
...
LogicalResult ForOp::promoteIfSingleIteration(RewriterBase &rewriter) {
  ...
  toolchain::append_range(bbArgReplacements, getInitArgs());
...
static void printInitializationList(OpAsmPrinter &p,
  ...
  p << prefix << '(';
  toolchain::interleaveComma(toolchain::zip(blocksArgs, initializers), p, [&](auto it) {
...
ParseResult ForOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  for (auto [iterArg, type] :
       toolchain::zip_equal(toolchain::drop_begin(regionArgs), result.types))
    iterArg.type = type;
  ...
  if (parser.resolveOperand(lb, type, result.operands) ||
    ...
    return failure();
  if (hasIterArgs) {
    for (auto argOperandType : toolchain::zip_equal(toolchain::drop_begin(regionArgs),
...
ForOp::replaceWithAdditionalYields(RewriterBase &rewriter,
  ...
  auto inits = toolchain::to_vector(getInitArgs());
...
ForOp mlir::scf::getForInductionVarOwner(Value val) {
  auto ivArg = toolchain::dyn_cast<BlockArgument>(val);
  if (!ivArg)
    return ForOp();
...
LogicalResult scf::ForallOp::promoteIfSingleIteration(RewriterBase &rewriter) {
  for (auto [lb, ub, step] :
       toolchain::zip(getMixedLowerBound(), getMixedUpperBound(), getMixedStep())) {
    auto tripCount =
...
void mlir::scf::promote(RewriterBase &rewriter, scf::ForallOp forallOp) {
  ...
  for (auto &yieldingOp : terminator.getYieldingOps()) {
    ...
    if (toolchain::isa<TensorType>(src.getType())) {
...
LoopNest mlir::scf::buildLoopNest(
  ...
  toolchain::append_range(nestResults, loops.front().getResults());
...
struct SimplifyTrivialLoops : public OpRewritePattern<ForOp> {
  ...
  LogicalResult matchAndRewrite(ForOp op,
    ...
    if (tripCount->getSExtValue() == 1) {
      ...
      toolchain::append_range(blockArgs, op.getInitArgs());
    ...
    if (!toolchain::hasSingleElement(block))
      return failure();
    ...
    if (toolchain::any_of(op.getYieldedValues(),
...
struct ForOpTensorCastFolder : public OpRewritePattern<ForOp> {
  ...
  LogicalResult matchAndRewrite(ForOp op,
                                PatternRewriter &rewriter) const override {
    for (auto it : toolchain::zip(op.getInitArgsMutable(), op.getResults())) {
...
ParseResult ForallOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  for (const auto &it : toolchain::enumerate(regionOutArgs)) {
    auto &out = it.value();
...
bool ForallOp::isNormalized() {
  auto allEqual = [](ArrayRef<OpFoldResult> results, int64_t val) {
    return toolchain::all_of(results, [&](OpFoldResult ofr) {
...
ForallOp mlir::scf::getForallOpThreadIndexOwner(Value val) {
  auto tidxArg = toolchain::dyn_cast<BlockArgument>(val);
  if (!tidxArg)
    return ForallOp();
...
struct DimOfForallOp : public OpRewritePattern<tensor::DimOp> {
  ...
  LogicalResult matchAndRewrite(tensor::DimOp dimOp,
    ...
    if (!forallOp)
      return failure();
    Value sharedOut =
        forallOp.getTiedOpOperand(toolchain::cast<OpResult>(dimOp.getSource()))
            ->get();
...
///          to :
///          https://github.com/toolchain/toolchain-project/pull/90189#discussion_r1589011124
...
struct ForallOpIterArgsFolder : public OpRewritePattern<ForallOp> {
  ...
  LogicalResult matchAndRewrite(ForallOp forallOp,
    ...
    for (auto [blockArg, result, out] :
         toolchain::zip_equal(blockArgsToDelete, resultsToDelete, outsToDelete)) {
...
struct ForallOpSingleOrZeroIterationDimsFolder
  ...
  LogicalResult matchAndRewrite(ForallOp op,
    ...
    for (auto [lb, ub, step, iv] :
         toolchain::zip(op.getMixedLowerBound(), op.getMixedUpperBound(),
    ...
    for (const auto &namedAttr : op->getAttrs()) {
      if (toolchain::is_contained(elidedAttrs, namedAttr.getName()))
        continue;
...
struct FoldTensorCastOfOutputIntoForallOp
  ...
  LogicalResult matchAndRewrite(scf::ForallOp forallOp,
                                PatternRewriter &rewriter) const final {
    toolchain::SmallMapVector<unsigned, TypeCast, 2> tensorCastProducers;
    toolchain::SmallVector<Value> newOutputTensors = forallOp.getOutputs();
    for (auto en : toolchain::enumerate(newOutputTensors)) {
    ...
    auto newForallOp = ForallOp::create(
        ...
        [&](OpBuilder nestedBuilder, Location nestedLoc, ValueRange bbArgs) {
          auto castBlockArgs =
              toolchain::to_vector(bbArgs.take_back(forallOp->getNumResults()));
          ...
          SmallVector<Value> ivsBlockArgs =
              toolchain::to_vector(bbArgs.take_front(forallOp.getRank()));
    ...
    for (auto [yieldingOp, outputBlockArg] : toolchain::zip(
...
LogicalResult InParallelOp::verify() {
  ...
  for (Operation &op : getRegion().front().getOperations()) {
    ...
    for (OpOperand &dest : dests) {
      if (!toolchain::is_contained(regionOutArgs, dest.get()))
...
toolchain::iterator_range<Block::iterator> InParallelOp::getYieldingOps() {
...
IfOp::inferReturnTypes(MLIRContext *ctx, std::optional<Location> loc,
  ...
  if (b.empty())
    return failure();
  auto yieldOp = toolchain::dyn_cast<YieldOp>(b.back());
  if (!yieldOp)
    return failure();
  TypeRange types = yieldOp.getOperandTypes();
  toolchain::append_range(inferredReturnTypes, types);
  return success();
}
...
LogicalResult IfOp::fold(FoldAdaptor adaptor,
  ...
  // callbacks See: https://toolchain.org/doxygen/ilist_8h_source.html#l00224
...
void IfOp::getRegionInvocationBounds(
  ...
  if (auto cond = toolchain::dyn_cast_or_null<BoolAttr>(operands[0])) {
...
struct ConvertTrivialIfToSelect : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp op,
    ...
    for (auto [trueVal, falseVal] : toolchain::zip(thenYieldArgs, elseYieldArgs)) {
    ...
    for (const auto &it :
         toolchain::enumerate(toolchain::zip(thenYieldArgs, elseYieldArgs))) {
...
struct ConditionPropagation : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp op,
    ...
    DenseMap<Region *, Parent> cache;
    for (OpOperand &use :
         toolchain::make_early_inc_range(op.getCondition().getUses())) {
...
struct ReplaceIfYieldWithConditionOrValue : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp op,
    ...
    for (auto [trueResult, falseResult, opResult] :
         toolchain::zip(trueYield.getResults(), falseYield.getResults(),
                   op.getResults())) {
...
struct CombineIfs : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp nextIf,
    ...
    for (auto it : toolchain::zip(prevIf.getResults(),
                             prevIf.thenYield().getOperands(), prevElseYielded))
      for (OpOperand &use :
           toolchain::make_early_inc_range(std::get<0>(it).getUses())) {
    ...
    toolchain::append_range(mergedTypes, nextIf.getResultTypes());
    ...
    if (nextThen) {
      ...
      toolchain::append_range(mergedYields, thenYield2.getOperands());
    ...
    if (nextElse) {
      if (combinedIf.getElseRegion().empty()) {
        ...
        toolchain::append_range(mergedElseYields, elseYield2.getOperands());
    ...
    for (const auto &pair : toolchain::enumerate(combinedIf.getResults())) {
...
struct RemoveEmptyElseBranch : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp ifOp,
    ...
    if (!elseBlock || !toolchain::hasSingleElement(*elseBlock))
      return failure();
...
struct CombineNestedIfs : public OpRewritePattern<IfOp> {
  ...
  LogicalResult matchAndRewrite(IfOp op,
    ...
    if (!toolchain::hasSingleElement(nestedOps))
      return failure();

    // If there is an else block, it can only yield
    if (op.elseBlock() && !toolchain::hasSingleElement(*op.elseBlock()))
      return failure();
    ...
    if (nestedIf.elseBlock() && !toolchain::hasSingleElement(*nestedIf.elseBlock()))
      return failure();

    SmallVector<Value> thenYield(op.thenYield().getOperands());
    SmallVector<Value> elseYield;
    if (op.elseBlock())
      toolchain::append_range(elseYield, op.elseYield().getOperands());
    ...
    for (const auto &tup : toolchain::enumerate(thenYield)) {
      if (tup.value().getDefiningOp() == nestedIf) {
        auto nestedIdx = toolchain::cast<OpResult>(tup.value()).getResultNumber();
    ...
    SmallVector<Value> results;
    toolchain::append_range(results, newIf.getResults());
...
struct ParallelOpSingleOrZeroIterationDimsFolder
  ...
  LogicalResult matchAndRewrite(ParallelOp op,
    ...
    IRMapping mapping;
    for (auto [lb, ub, step, iv] :
         toolchain::zip(op.getLowerBound(), op.getUpperBound(), op.getStep(),
                   op.getInductionVars())) {
      auto numIterations =
          constantTripCount(lb, ub, step, /*isSigned=*/true, computeUbMinusLb);
      if (numIterations.has_value()) {
...
struct MergeNestedParallelLoops : public OpRewritePattern<ParallelOp> {
  ...
  LogicalResult matchAndRewrite(ParallelOp op,
    ...
    if (!toolchain::hasSingleElement(outerBody.without_terminator()))
      return failure();
    ...
    for (auto val : outerBody.getArguments())
      if (toolchain::is_contained(innerOp.getLowerBound(), val) ||
          toolchain::is_contained(innerOp.getUpperBound(), val) ||
          toolchain::is_contained(innerOp.getStep(), val))
        return failure();
...
LogicalResult ReduceOp::verifyRegions() {
  ...
  for (int64_t i = 0, e = getReductions().size(); i < e; ++i) {
    ...
    if (block.getNumArguments() != 2 ||
        toolchain::any_of(block.getArguments(), [&](const BlockArgument &arg) {
...
struct WhileMoveIfDown : public OpRewritePattern<scf::WhileOp> {
  ...
  LogicalResult matchAndRewrite(scf::WhileOp op,
    ...
    assert((ifOp->use_empty() || (toolchain::all_equal(ifOp->getUsers()) &&
    ...
    for (auto [idx, arg] : toolchain::enumerate(conditionOp.getArgs())) {
      auto it = toolchain::find(ifOp->getResults(), arg);
    ...
    auto additionalValueTypes = toolchain::map_to_vector(
...
struct WhileConditionTruth : public OpRewritePattern<WhileOp> {
  ...
  LogicalResult matchAndRewrite(WhileOp op,
    ...
    for (auto yieldedAndBlockArgs :
         toolchain::zip(term.getArgs(), op.getAfterArguments())) {
...
struct WhileCmpCond : public OpRewritePattern<scf::WhileOp> {
  ...
  LogicalResult matchAndRewrite(scf::WhileOp op,
    ...
    if (!cmp)
      return failure();
    bool changed = false;
    for (auto tup : toolchain::zip(cond.getArgs(), op.getAfterArguments())) {
      for (size_t opIdx = 0; opIdx < 2; opIdx++) {
        if (std::get<0>(tup) != cmp.getOperand(opIdx))
          continue;
        for (OpOperand &u :
             toolchain::make_early_inc_range(std::get<1>(tup).getUses())) {
...
static std::optional<SmallVector<unsigned>> getArgsMapping(ValueRange args1,
  ...
  for (auto &&[i, arg1] : toolchain::enumerate(args1)) {
    auto it = toolchain::find(args2, arg1);
    if (it == args2.end())
      return std::nullopt;
...
static bool hasDuplicates(ValueRange args) {
  toolchain::SmallDenseSet<Value> set;
  for (Value arg : args) {
...
struct WhileOpAlignBeforeArgs : public OpRewritePattern<WhileOp> {
  ...
  LogicalResult matchAndRewrite(WhileOp loop,
    ...
    for (auto &&[i, j] : toolchain::enumerate(*mapping))
...
static void printSwitchCases(OpAsmPrinter &p, Operation *op,
  ...
  for (auto [value, region] : toolchain::zip(cases.asArrayRef(), caseRegions)) {
    p.printNewline();
    p << "case " << value << ' ';
...
LogicalResult scf::IndexSwitchOp::verify() {
  ...
  auto verifyRegion = [&](Region &region, const Twine &name) -> LogicalResult {
    ...
    for (auto [idx, result, operand] :
         toolchain::enumerate(getResultTypes(), yield.getOperands())) {
      if (!operand)
  ...
  if (failed(verifyRegion(getDefaultRegion(), "default region")))
    return failure();
  for (auto [idx, caseRegion] : toolchain::enumerate(getCaseRegions()))
...
void IndexSwitchOp::getSuccessorRegions(
  ...
  toolchain::append_range(successors, getRegions());
}
...
void IndexSwitchOp::getEntrySuccessorRegions(
  ...
  for (auto [caseValue, caseRegion] : toolchain::zip(getCases(), getCaseRegions())) {
...
void IndexSwitchOp::getRegionInvocationBounds(
  ...
  auto operandValue = toolchain::dyn_cast_or_null<IntegerAttr>(operands.front());
  if (!operandValue) {
  ...
  const auto *it = toolchain::find(getCases(), operandValue.getInt());
  if (it != getCases().end())
