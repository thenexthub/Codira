...
transform::ForallToForOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (auto &&[i, res] : toolchain::enumerate(opResults)) {
...
transform::LoopFuseSiblingOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (!toolchain::hasSingleElement(targetOps) ||
      !toolchain::hasSingleElement(sourceOps)) {
    return emitDefiniteFailure()
           << "requires exactly one target handle (got "
           << toolchain::range_size(targetOps) << ") and exactly one "
           << "source handle (got " << toolchain::range_size(sourceOps) << ")";
  }
