...
static LogicalResult verifyHasBindingUse(Operation *op) {
  ...
  if (!toolchain::isa_and_nonnull<PatternOp>(op->getParentOp()))
    return success();
  if (hasBindingUse(op))
    return success();
...
LogicalResult ApplyNativeConstraintOp::verify() {
  ...
  if (toolchain::any_of(getResults(), [](OpResult result) {
...
static LogicalResult verifyResultTypesAreInferrable(OperationOp op,
  ...
  // types.
  if (toolchain::any_of(op.getOp().getUses(), canInferTypeFromUse))
    return success();
  ...
  for (const auto &it : toolchain::enumerate(resultTypes)) {
    ...
    if (TypeOp typeOp = dyn_cast<TypeOp>(resultTypeOp)) {
      if (typeOp.getConstantType() ||
          toolchain::any_of(typeOp->getUsers(), constrainsInput))
        continue;
...
LogicalResult ResultsOp::verify() {
  if (!getIndex() && toolchain::isa<pdl::ValueType>(getType())) {
