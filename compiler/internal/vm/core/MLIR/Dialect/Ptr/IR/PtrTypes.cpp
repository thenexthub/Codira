...
bool PtrType::areCompatible(DataLayoutEntryListRef oldLayout,
  ...
  for (DataLayoutEntryInterface newEntry : newLayout) {
    ...
    auto newType = toolchain::cast<PtrType>(toolchain::cast<Type>(newEntry.getKey()));
    const auto *it =
        toolchain::find_if(oldLayout, [&](DataLayoutEntryInterface entry) {
          if (auto type = toolchain::dyn_cast_if_present<Type>(entry.getKey())) {
            return toolchain::cast<PtrType>(type).getMemorySpace() ==
    ...
    if (it == oldLayout.end()) {
      ...
      it = toolchain::find_if(oldLayout, [&](DataLayoutEntryInterface entry) {
        if (auto type = toolchain::dyn_cast_if_present<Type>(entry.getKey())) {
          auto ptrTy = toolchain::cast<PtrType>(type);
    ...
    if (it != oldLayout.end()) {
      auto spec = toolchain::cast<SpecAttr>(*it);
      size = spec.getSize();
      abi = spec.getAbi();
    }

    auto newSpec = toolchain::cast<SpecAttr>(newEntry.getValue());
...
uint64_t PtrType::getABIAlignment(const DataLayout &dataLayout,
  ...
  auto defaultMemorySpace = toolchain::cast_if_present<MemorySpaceAttrInterface>(
...
toolchain::TypeSize PtrType::getTypeSizeInBits(const DataLayout &dataLayout,
                                          DataLayoutEntryListRef params) const {
  auto defaultMemorySpace = toolchain::cast_if_present<MemorySpaceAttrInterface>(
      dataLayout.getDefaultMemorySpace());
  if (SpecAttr spec = getPointerSpec(params, *this, defaultMemorySpace))
    return toolchain::TypeSize::getFixed(spec.getSize());
...
LogicalResult PtrType::verifyEntries(DataLayoutEntryListRef entries,
  ...
  for (DataLayoutEntryInterface entry : entries) {
    ...
    if (!toolchain::isa<SpecAttr>(entry.getValue())) {
