...
verifyAlignment(std::optional<int64_t> alignment,
  ...
  if (!toolchain::isPowerOf2_64(alignment.value()))
...
OpFoldResult PtrAddOp::fold(FoldAdaptor adaptor) {
  ...
  if (!attr)
    return nullptr;
  if (toolchain::APInt value; m_ConstantInt(&value).match(attr) && value.isZero())
    return getBase();
  return nullptr;
}
...
toolchain::TypeSize TypeOffsetOp::getTypeSize(std::optional<DataLayout> layout) {
  if (layout)
