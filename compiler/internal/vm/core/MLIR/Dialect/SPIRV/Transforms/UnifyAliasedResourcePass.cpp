...
deduceCanonicalResource(ArrayRef<spirv::SPIRVType> types) {
  ...
  for (const auto &indexedTypes : toolchain::enumerate(types)) {
  ...
  if (!vectorNumBits.empty()) {
    ...
    // to large vectors.
    auto *minVal = toolchain::min_element(vectorNumBits);
    ...
    if (toolchain::any_of(vectorNumBits,
    ...
    if (toolchain::any_of(scalarNumBits,
  ...
  auto *minVal = toolchain::min_element(scalarNumBits);
  if (toolchain::any_of(scalarNumBits,
...
void ResourceAliasAnalysis::recordIfUnifiable(
  ...
  for (const auto &resource : toolchain::enumerate(resources)) {
...
struct ConvertAccessChain : public ConvertAliasResource<spirv::AccessChainOp> {
  ...
  matchAndRewrite(spirv::AccessChainOp acOp, OpAdaptor adaptor,
    ...
    if (srcElemType.isIntOrFloat() && isa<VectorType>(dstElemType)) {
      ...
      auto indices = toolchain::to_vector<4>(acOp.getIndices());
