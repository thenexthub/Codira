...
static Value lowerExtendedMultiplication(Operation *mulOp,
  ...
  for (auto [i, lhsDigit] : toolchain::enumerate(lhsDigits)) {
    for (auto [j, rhsDigit] : toolchain::enumerate(rhsDigits)) {
  ...
  LogicalResult matchAndRewrite(MulExtendedOp op,
    ...
    if (elemTy.getIntOrFloatBitWidth() != 32)
      return rewriter.notifyMatchFailure(
          loc,
          toolchain::formatv("Unexpected integer type for WebGPU: '{0}'", elemTy));
