...
static std::optional<SmallVector<int64_t>> getTargetShape(VectorType vecType) {
  ...
  if (toolchain::all_of(*maybeShapeRatio, [](int64_t v) { return v == 1; })) {
    LLVM_DEBUG(toolchain::dbgs() << "--no unrolling needed -> SKIP\n");
    return std::nullopt;
  }
  LLVM_DEBUG(toolchain::dbgs()
...
static LogicalResult checkExtensionRequirements(
  ...
  for (const auto &ors : candidates) {
    ...
    LLVM_DEBUG({
      ...
      toolchain::dbgs() << label << " illegal: requires at least one extension in ["
...
static LogicalResult checkCapabilityRequirements(
  ...
  for (const auto &ors : candidates) {
    ...
    LLVM_DEBUG({
      ...
      toolchain::dbgs() << label << " illegal: requires at least one capability in ["
...
getTypeNumBytes(const SPIRVConversionOptions &options, Type type) {
  ...
  if (auto memRefType = dyn_cast<MemRefType>(type)) {
    ...
    if (toolchain::is_contained(dims, ShapedType::kDynamic) ||
        ShapedType::isDynamic(offset) ||
        toolchain::is_contained(strides, ShapedType::kDynamic))
      return std::nullopt;
...
convertScalarType(const spirv::TargetEnv &targetEnv,
  ...
  if (auto floatType = dyn_cast<FloatType>(type)) {
    LLVM_DEBUG(toolchain::dbgs() << type << " converted to 32-bit for SPIR-V\n");
...
static Type convertSubByteIntegerType(const SPIRVConversionOptions &options,
                                      IntegerType type) {
  if (type.getWidth() > 8) {
    LLVM_DEBUG(toolchain::dbgs() << "not a subbyte type\n");
    return nullptr;
  }
  if (options.subByteTypeStorage != SPIRVSubByteTypeStorage::Packed) {
    LLVM_DEBUG(toolchain::dbgs() << "unsupported sub-byte storage kind\n");
    return nullptr;
  }

  if (!toolchain::isPowerOf2_32(type.getWidth())) {
    LLVM_DEBUG(toolchain::dbgs()
...
static Type convert8BitFloatType(const SPIRVConversionOptions &options,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "unsupported 8-bit float type: " << type << "\n");
  return nullptr;
}
...
static Type convertBoolMemrefType(const spirv::TargetEnv &targetEnv,
  ...
  int64_t memrefSize = toolchain::divideCeil(type.getNumElements() * numBoolBits, 8);
  int64_t arrayElemCount = toolchain::divideCeil(memrefSize, *arrayElemSize);
  int64_t stride = needsExplicitLayout(storageClass) ? *arrayElemSize : 0;
...
static Type convertSubByteMemrefType(const spirv::TargetEnv &targetEnv,
  ...
  int64_t memrefSize =
      toolchain::divideCeil(type.getNumElements() * elementType.getWidth(), 8);
  int64_t arrayElemCount = toolchain::divideCeil(memrefSize, arrayElemSize);
  int64_t stride = needsExplicitLayout(storageClass) ? arrayElemSize : 0;
  auto arrayType = spirv::ArrayType::get(arrayElemType, arrayElemCount, stride);
  if (targetEnv.allows(spirv::Capability::Kernel))
...
static Type convertMemrefType(const spirv::TargetEnv &targetEnv,
  ...
  if (storageClass == spirv::StorageClass::Image) {
    ...
    if (!isa<spirv::ScalarType>(elementType)) {
      LLVM_DEBUG(toolchain::dbgs() << type << " illegal: cannot lower memref of "
...
struct FuncOpVectorUnroll final : OpRewritePattern<func::FuncOp> {
  ...
  LogicalResult matchAndRewrite(func::FuncOp funcOp,
    ...
    toolchain::SmallDenseMap<Operation *, size_t> tmpOps;
    ...
    for (auto [origInputNo, origType] : enumerate(fnType.getInputs())) {
      ...
      auto originalShape =
          toolchain::to_vector_of<int64_t, 4>(origVecType.getShape());
...
Value spirv::getPushConstantValue(Operation *op, unsigned elementCount,
  ...
  auto acOp = spirv::AccessChainOp::create(builder, loc, addrOp,
                                           toolchain::ArrayRef({zeroOp, offsetOp}));
...
Value mlir::spirv::linearizeIndex(ValueRange indices, ArrayRef<int64_t> strides,
  ...
  for (const auto &index : toolchain::enumerate(indices)) {
...
bool SPIRVConversionTarget::isLegalOp(Operation *op) {
  ...
  if (toolchain::any_of(valueTypes,
