...
void BranchConditionalOp::print(OpAsmPrinter &printer) {
  ...
  if (std::optional<ArrayAttr> weights = getBranchWeights()) {
    printer << ' '
            << toolchain::interleaved_array(weights->getAsValueRange<IntegerAttr>());
  }
...
LogicalResult BranchConditionalOp::verify() {
  if (auto weights = getBranchWeights()) {
    ...
    if (toolchain::all_of(*weights, [](Attribute attr) {
...
Block *SwitchOp::getSuccessorForOperands(ArrayRef<Attribute> operands) {
  ...
  if (auto value = dyn_cast_or_null<IntegerAttr>(operands.front())) {
    for (auto [index, literal] : toolchain::enumerate(literals->getValues<APInt>()))
...
static bool hasOneBranchOpTo(Block &srcBlock, Block &dstBlock) {
  ...
  if (!toolchain::hasSingleElement(srcBlock))
    return false;
...
static bool isMergeBlock(Block &block) {
  return toolchain::hasSingleElement(block) && isa<spirv::MergeOp>(block.front());
}

/// Returns true if a `spirv.mlir.merge` op outside the merge block.
static bool hasOtherMerge(Region &region) {
  return !region.empty() && toolchain::any_of(region.getOps(), [&](Operation &op) {
...
LogicalResult LoopOp::verifyRegions() {
  ...
  if (toolchain::none_of(
          toolchain::seq<unsigned>(0, cont.getNumSuccessors()),
  ...
  for (auto &block : toolchain::make_range(std::next(region.begin(), 2),
                                      std::prev(region.end(), 2))) {
    for (auto i : toolchain::seq<unsigned>(0, block.getNumSuccessors())) {
