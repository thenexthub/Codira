...
static LogicalResult verifyIntegerDotProduct(Operation *op) {
  assert(toolchain::is_contained({2u, 3u}, op->getNumOperands()) &&
  ...
  if (auto intTy = dyn_cast<IntegerType>(factorTy)) {
    ...
    if (intTy.getWidth() != 32)
      return op->emitOpError(
          toolchain::formatv("with specified Packed Vector Format ({0}) requires "
  ...
  if (factorBitWidth > resultBitWidth)
    return op->emitOpError(
        toolchain::formatv("result type has insufficient bit-width ({0} bits) "
