...
spirv::VerCapExtAttr::ext_iterator::ext_iterator(ArrayAttr::iterator it)
    : toolchain::mapped_iterator<ArrayAttr::iterator,
...
LogicalResult spirv::VerCapExtAttr::verifyInvariants(
  ...
  if (!toolchain::all_of(capabilities.getValue(), [](Attribute attr) {
  ...
  if (!toolchain::all_of(extensions.getValue(), [](Attribute attr) {
