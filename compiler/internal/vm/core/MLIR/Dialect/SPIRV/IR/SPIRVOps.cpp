...
spirv::verifyMemorySemantics(Operation *op,
  ...
  auto bitCount =
      toolchain::popcount(static_cast<uint32_t>(memorySemantics & atMostOneInSet));
  if (bitCount > 1) {
    return op->emitError(
...
static void printOneResultOp(Operation *op, OpAsmPrinter &p) {
  ...
  if (toolchain::any_of(op->getOperandTypes(),
...
static LogicalResult verifyArithmeticExtendedBinaryOp(ExtendedBinaryOp op) {
  ...
  if (!toolchain::all_equal({op.getOperand1().getType(), op.getOperand2().getType(),
...
LogicalResult spirv::CompositeConstructOp::verify() {
  ...
  auto coopElementType =
      toolchain::TypeSwitch<Type, Type>(getType())
  ...
  unsigned totalCount = toolchain::sum_of(sizes);
...
void mlir::spirv::ConstantOp::getAsmResultNames(
    toolchain::function_ref<void(mlir::Value, toolchain::StringRef)> setNameFn) {
  Type type = getType();
...
void spirv::EntryPointOp::print(OpAsmPrinter &printer) {
  ...
  if (!interfaceVars.empty())
    printer << ", " << toolchain::interleaved(interfaceVars);
}
...
void spirv::ExecutionModeOp::print(OpAsmPrinter &printer) {
  ...
  if (!values.empty())
    printer << ", " << toolchain::interleaved(values.getAsValueRange<IntegerAttr>());
}
...
LogicalResult spirv::FuncOp::verifyBody() {
  ...
  if (!isExternal()) {
    ...
    for (auto [index, fnArgType, blockArgType] :
         toolchain::enumerate(getArgumentTypes(), entryBlock.getArgumentTypes())) {
...
LogicalResult spirv::MatrixTimesScalarOp::verify() {
  Type elementType =
      toolchain::TypeSwitch<Type, Type>(getMatrix().getType())
...
void spirv::SpecConstantCompositeOp::print(OpAsmPrinter &printer) {
  printer << " ";
  printer.printSymbolName(getSymName());
  printer << " (" << toolchain::interleaved(this->getConstituents().getValue())
          << ") : " << getType();
}
