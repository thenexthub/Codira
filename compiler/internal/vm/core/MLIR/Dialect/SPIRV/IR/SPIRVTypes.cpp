...
private:
  ...
  std::optional<StorageClass> storage;
  toolchain::SmallDenseSet<std::pair<Type, std::optional<StorageClass>>> seen;
};
...
bool CompositeType::isValid(VectorType type) {
  return type.getRank() == 1 &&
         toolchain::is_contained({2, 3, 4, 8, 16}, type.getNumElements()) &&
...
bool ScalarType::isValid(FloatType type) {
  return toolchain::is_contained({16u, 32u, 64u}, type.getWidth());
}

bool ScalarType::isValid(IntegerType type) {
  return toolchain::is_contained({1u, 8u, 16u, 32u, 64u}, type.getWidth());
}
...
SampledImageType::verifyInvariants(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (toolchain::is_contained({Dim::SubpassData, Dim::Buffer}, image.getDim()))
...
struct spirv::detail::StructTypeStorage : public TypeStorage {
  ...
  toolchain::PointerIntPair<Type const *, 1, bool> memberTypesAndIsBodySet;
...
StructType::get(ArrayRef<Type> memberTypes,
  ...
  SmallVector<StructType::MemberDecorationInfo, 4> sortedMemberDecorations(
      memberDecorations);
  toolchain::array_pod_sort(sortedMemberDecorations.begin(),
                       sortedMemberDecorations.end());
  SmallVector<StructType::StructDecorationInfo, 1> sortedStructDecorations(
      structDecorations);
  toolchain::array_pod_sort(sortedStructDecorations.begin(),
...
toolchain::hash_code spirv::hash_value(
    const StructType::MemberDecorationInfo &memberDecorationInfo) {
  return toolchain::hash_combine(memberDecorationInfo.memberIndex,
                            memberDecorationInfo.decoration);
}

toolchain::hash_code spirv::hash_value(
    const StructType::StructDecorationInfo &structDecorationInfo) {
  return toolchain::hash_value(structDecorationInfo.decoration);
}
...
struct spirv::detail::TensorArmTypeStorage final : TypeStorage {
  ...
  static toolchain::hash_code hashKey(const KeyTy &key) {
    auto [shape, elementType] = key;
    return toolchain::hash_combine(shape, elementType);
  }
...
TensorArmType::verifyInvariants(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (toolchain::is_contained(shape, 0))
    return emitError() << "arm.tensor do not support dimensions = 0";
  if (toolchain::any_of(shape, [](int64_t dim) { return dim < 0; }) &&
      toolchain::any_of(shape, [](int64_t dim) { return dim > 0; }))
    return emitError()
