...
ParseResult spirv::GraphARMOp::parse(OpAsmParser &parser,
  ...
  SmallVector<Type> argTypes = toolchain::map_to_vector(
...
LogicalResult spirv::GraphARMOp::verifyBody() {
  for (auto [index, graphArgType] : toolchain::enumerate(getArgumentTypes())) {
  ...
  for (auto [index, graphResType] : toolchain::enumerate(getResultTypes())) {
  ...
  auto walkResult = walk([grType](spirv::GraphOutputsARMOp op) -> WalkResult {
    ...
    ValueTypeRange<OperandRange> graphOutputOperandTypes =
        op.getValue().getType();
    for (auto [index, type] : toolchain::enumerate(graphOutputOperandTypes)) {
...
LogicalResult spirv::GraphOutputsARMOp::verify() {
  ...
  for (auto [index, result] : toolchain::enumerate(results))
