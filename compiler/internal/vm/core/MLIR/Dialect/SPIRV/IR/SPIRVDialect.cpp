...
static inline bool containsReturn(Region &region) {
  return toolchain::any_of(region, [](Block &block) {
...
static Type parseAndVerifySampledImageType(SPIRVDialect const &dialect,
  ...
  if (toolchain::is_contained({Dim::SubpassData, Dim::Buffer}, imageType.getDim())) {
    parser.emitError(
...
static Type parseTensorArmType(SPIRVDialect const &dialect,
  ...
  if (toolchain::is_contained(dims, 0)) {
    parser.emitError(countLoc, "arm.tensors do not support zero dimensions");
    return {};
  }

  if (toolchain::any_of(dims, [](int64_t dim) { return dim < 0; }) &&
      toolchain::any_of(dims, [](int64_t dim) { return dim > 0; })) {
...
static void print(StructType type, DialectAsmPrinter &os) {
  ...
  auto printMember = [&](unsigned i) {
    ...
    if (type.hasOffset() || !decorations.empty()) {
      ...
      toolchain::interleaveComma(decorations, os, eachFn);
      os << "]";
    }
  };
  toolchain::interleaveComma(toolchain::seq<unsigned>(0, type.getNumElements()), os,
                        printMember);
  os << ")";
