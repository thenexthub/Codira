...
static toolchain::hash_code computeHash(SymbolOpInterface symbolOp) {
  auto range =
      toolchain::make_filter_range(symbolOp->getAttrs(), [](NamedAttribute attr) {
        return attr.getName() != SymbolTable::getSymbolAttrName();
      });

  return toolchain::hash_combine(symbolOp->getName(),
                            toolchain::hash_combine_range(range));
}
...
OwningOpRef<spirv::ModuleOp> combine(ArrayRef<spirv::ModuleOp> inputModules,
  ...
  toolchain::StringMap<spirv::ModuleOp> symNameToModuleMap;
  ...
  DenseMap<toolchain::hash_code, SymbolOpInterface> hashToSymbolOp;
