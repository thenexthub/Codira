...
static std::string keyToStr(DataLayoutEntryKey key) {
  ...
  TypeSwitch<DataLayoutEntryKey>(key)
      .Case<StringAttr, Type>( // The only two kinds of key we know of.
          [&](auto key) { toolchain::raw_string_ostream(buf) << key; });
  return buf;
}
...
static void printAngleBracketedEntries(AsmPrinter &os, T &&entries) {
  os << "<";
  toolchain::interleaveComma(std::forward<T>(entries), os, [&](auto entry) {
...
static LogicalResult combineOneSpec(
    ...
    toolchain::MapVector<TypeID, DataLayoutEntryList> &entriesForType,
    toolchain::MapVector<StringAttr, DataLayoutEntryInterface> &entriesForID) {
  // A missing spec should be fine.
  if (!spec)
    return success();

  toolchain::MapVector<TypeID, DataLayoutEntryList> newEntriesForType;
  toolchain::MapVector<StringAttr, DataLayoutEntryInterface> newEntriesForID;
  ...
  for (auto &kvp : newEntriesForType) {
    ...
    // https://github.com/toolchain/toolchain-project/issues/130321 gets resolved.
...
DataLayoutSpecAttr::combineWith(ArrayRef<DataLayoutSpecInterface> specs) const {
  ...
  if (any_of(specs, [](DataLayoutSpecInterface spec) {
        return !toolchain::isa<DataLayoutSpecAttr>(spec);
      }))
    return {};
  ...
  SmallVector<DataLayoutEntryInterface> entries;
  toolchain::append_range(entries, toolchain::make_second_range(entriesForID));
  for (const auto &kvp : entriesForType)
    toolchain::append_range(entries, kvp.second);
...
TargetSystemSpecAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  for (const auto &entry : entries) {
    ...
    if (auto targetDeviceSpec =
            toolchain::dyn_cast<TargetDeviceSpecInterface>(entry.getValue())) {
...
dlti::query(Operation *op, ArrayRef<DataLayoutEntryKey> keys, bool emitError) {
  ...
  for (auto &&[idx, key] : toolchain::enumerate(keys)) {
...
LogicalResult DLTIDialect::verifyOperationAttribute(Operation *op,
  ...
  if (attr.getName() == DLTIDialect::kDataLayoutAttrName) {
    if (!toolchain::isa<DataLayoutSpecAttr>(attr.getValue())) {
  ...
  if (attr.getName() == DLTIDialect::kTargetSystemDescAttrName) {
    if (!toolchain::isa<TargetSystemSpecAttr>(attr.getValue())) {
      return op->emitError()
  ...
  if (attr.getName() == DLTIDialect::kMapAttrName) {
    if (!toolchain::isa<MapAttr>(attr.getValue())) {
