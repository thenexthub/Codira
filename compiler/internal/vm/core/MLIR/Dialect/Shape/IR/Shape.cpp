...
bool shape::isExtentTensorType(Type type) {
  auto ranked = toolchain::dyn_cast<RankedTensorType>(type);
...
LogicalResult shape::getShapeVec(Value input,
  ...
  if (auto inputOp = input.getDefiningOp<ShapeOfOp>()) {
    auto type = toolchain::cast<ShapedType>(inputOp.getArg().getType());
    if (!type.hasRank())
      return failure();
    toolchain::append_range(shapeValues, type.getShape());
    return success();
  }
  DenseIntElementsAttr attr;
  if (matchPattern(input, m_Constant(&attr))) {
    toolchain::append_range(shapeValues, attr.getValues<int64_t>());
    return success();
  }
  return failure();
}
...
static bool isErrorPropagationPossible(TypeRange operandTypes) {
  return toolchain::any_of(operandTypes,
                      toolchain::IsaPred<SizeType, ShapeType, ValueShapeType>);
}
...
static LogicalResult verifySizeOrIndexOp(Operation *op) {
  ...
  if (isErrorPropagationPossible(op->getOperandTypes())) {
    if (!toolchain::isa<SizeType>(resultTy))
...
static LogicalResult verifyShapeOrExtentTensorOp(Operation *op) {
  ...
  Type resultTy = op->getResultTypes().front();
  if (isErrorPropagationPossible(op->getOperandTypes())) {
    if (!toolchain::isa<ShapeType>(resultTy))
      return op->emitOpError()
             << "if at least one of the operands can hold error values then "
...
static bool eachHasOnlyOneOfTypes(TypeRange typeRange) {
  return typeRange.size() == 1 && toolchain::isa<Ty...>(typeRange.front());
}
...
Operation *ShapeDialect::materializeConstant(OpBuilder &builder,
  ...
  if (toolchain::isa<ShapeType>(type) || isExtentTensorType(type))
    return ConstShapeOp::create(builder, loc, type,
                                toolchain::cast<DenseIntElementsAttr>(value));
  if (toolchain::isa<SizeType>(type))
    return ConstSizeOp::create(builder, loc, type,
                               toolchain::cast<IntegerAttr>(value));
  if (toolchain::isa<WitnessType>(type))
    return ConstWitnessOp::create(builder, loc, type,
                                  toolchain::cast<BoolAttr>(value));
...
LogicalResult ShapeDialect::verifyOperationAttribute(Operation *op,
  ...
  if (attribute.getName() == "shape.lib") {
    ...
    if (auto symbolRef = toolchain::dyn_cast<SymbolRefAttr>(attribute.getValue())) {
...
struct AssumingOpRemoveUnusedResults : public OpRewritePattern<AssumingOp> {
  ...
  LogicalResult matchAndRewrite(AssumingOp op,
    ...
    auto yieldOp = toolchain::cast<AssumingYieldOp>(body->getTerminator());

    // Find used values.
    SmallVector<Value, 4> newYieldOperands;
    for (auto [opResult, yieldOperand] :
         toolchain::zip(op.getResults(), yieldOp.getOperands())) {
...
LogicalResult mlir::shape::AddOp::inferReturnTypes(
  ...
  if (toolchain::isa<SizeType>(adaptor.getLhs().getType()) ||
      toolchain::isa<SizeType>(adaptor.getRhs().getType()))
...
struct AssumingAllOfCstrBroadcastable : public OpRewritePattern<AssumingAllOp> {
  ...
  LogicalResult matchAndRewrite(AssumingAllOp op,
    ...
    toolchain::sort(shapes, [](auto a, auto b) {
    ...
    for (unsigned i = 0; i < shapes.size(); ++i) {
      auto isSubset = [&](auto pair) {
        return toolchain::set_is_subset(pair.second, shapes[i].second);
      };
...
struct AssumingAllToCstrEqCanonicalization
  ...
  LogicalResult matchAndRewrite(AssumingAllOp op,
    ...
    for (Value w : op.getInputs()) {
      ...
      if (!cstrEqOp)
        return failure();
      bool disjointShapes = toolchain::none_of(cstrEqOp.getShapes(), [&](Value s) {
        return toolchain::is_contained(shapes, s);
      });
...
OpFoldResult AssumingAllOp::fold(FoldAdaptor adaptor) {
  ...
  for (int idx = adaptor.getInputs().size() - 1; idx >= 0; idx--) {
    ...
    if (!toolchain::cast<BoolAttr>(a).getValue())
      return a;
  }
...
OpFoldResult BroadcastOp::fold(FoldAdaptor adaptor) {
  ...
  SmallVector<int64_t, 6> resultShape(
      toolchain::cast<DenseIntElementsAttr>(adaptor.getShapes().front())
          .getValues<int64_t>());

  for (auto next : adaptor.getShapes().drop_front()) {
    if (!next)
      return nullptr;
    auto nextShape = toolchain::to_vector<6>(
        toolchain::cast<DenseIntElementsAttr>(next).getValues<int64_t>());
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto isPotentiallyNonEmptyShape = [](Value shape) {
      if (auto extentTensorTy =
              toolchain::dyn_cast<RankedTensorType>(shape.getType())) {
    ...
    auto newOperands = toolchain::filter_to_vector<8>(op->getOperands(),
...
struct BroadcastForwardSingleOperandPattern
  ...
  LogicalResult matchAndRewrite(BroadcastOp op,
    ...
    if (replacement.getType() != op.getType()) {
      auto loc = op.getLoc();
      if (toolchain::isa<ShapeType>(op.getType())) {
        replacement = FromExtentTensorOp::create(rewriter, loc, replacement);
      } else {
        assert(!toolchain::isa<ShapeType>(op.getType()) &&
               !toolchain::isa<ShapeType>(replacement.getType()) &&
    ...
    for (Value shape : op.getShapes()) {
      if (auto constShape = shape.getDefiningOp<ConstShapeOp>()) {
        ...
        if (OpTrait::util::getBroadcastedShape(
                foldedConstantShape,
                toolchain::to_vector<8>(constShape.getShape().getValues<int64_t>()),
...
struct CanonicalizeCastExtentTensorOperandsPattern
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto canonicalizeOperand = [&](Value operand) -> Value {
      if (auto castOp = operand.getDefiningOp<tensor::CastOp>()) {
        ...
        bool isInformationLoosingCast =
            toolchain::cast<RankedTensorType>(castOp.getType()).isDynamicDim(0);
    ...
    auto newOperands = toolchain::to_vector<8>(
        toolchain::map_range(op.getOperands(), canonicalizeOperand));
...
struct BroadcastConcretizeResultTypePattern
  ...
  LogicalResult matchAndRewrite(BroadcastOp op,
    ...
    auto resultTy = toolchain::dyn_cast<RankedTensorType>(op.getType());
...
OpFoldResult ConcatOp::fold(FoldAdaptor adaptor) {
  if (!adaptor.getLhs() || !adaptor.getRhs())
    return nullptr;
  auto lhsShape = toolchain::to_vector<6>(
      toolchain::cast<DenseIntElementsAttr>(adaptor.getLhs()).getValues<int64_t>());
  auto rhsShape = toolchain::to_vector<6>(
      toolchain::cast<DenseIntElementsAttr>(adaptor.getRhs()).getValues<int64_t>());
  SmallVector<int64_t, 6> resultShape;
...
ParseResult ConstShapeOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (parser.parseAttribute(extentsRaw, "dummy", dummy))
    return failure();
  auto extentsArray = toolchain::dyn_cast<ArrayAttr>(extentsRaw);
  if (!extentsArray)
    return failure();
  SmallVector<int64_t, 6> ints;
  for (Attribute extent : extentsArray) {
    IntegerAttr attr = toolchain::dyn_cast<IntegerAttr>(extent);
    if (!attr)
      return failure();
...
bool mlir::shape::ConstShapeOp::isCompatibleReturnTypes(TypeRange l,
  ...
  if (toolchain::isa<ShapeType>(lhs) || toolchain::isa<ShapeType>(rhs))
...
static bool hasAtMostSingleNonScalar(ArrayRef<Attribute> attributes) {
  ...
  for (Attribute a : attributes) {
    if (!a || toolchain::cast<DenseIntElementsAttr>(a).getNumElements() != 0) {
      if (nonScalarSeen)
        return false;
      nonScalarSeen = true;
    }
...
OpFoldResult CstrBroadcastableOp::fold(FoldAdaptor adaptor) {
  ...
  if ([&] {
        ...
        for (const auto &operand : adaptor.getShapes()) {
          if (!operand)
            return false;
          extents.push_back(toolchain::to_vector<6>(
              toolchain::cast<DenseIntElementsAttr>(operand).getValues<int64_t>()));
        }
...
OpFoldResult CstrEqOp::fold(FoldAdaptor adaptor) {
  if (toolchain::all_of(adaptor.getShapes(), [&](Attribute a) {
...
void ConstSizeOp::getAsmResultNames(
    toolchain::function_ref<void(Value, StringRef)> setNameFn) {
...
std::optional<int64_t> DimOp::getConstantIndex() {
  ...
  if (auto constantOp = getIndex().getDefiningOp<arith::ConstantOp>())
    return toolchain::cast<IntegerAttr>(constantOp.getValue()).getInt();
  return std::nullopt;
}

OpFoldResult DimOp::fold(FoldAdaptor adaptor) {
  Type valType = getValue().getType();
  auto valShapedType = toolchain::dyn_cast<ShapedType>(valType);
...
OpFoldResult DivOp::fold(FoldAdaptor adaptor) {
  auto lhs = toolchain::dyn_cast_if_present<IntegerAttr>(adaptor.getLhs());
  if (!lhs)
    return nullptr;
  auto rhs = toolchain::dyn_cast_if_present<IntegerAttr>(adaptor.getRhs());
...
OpFoldResult FromExtentsOp::fold(FoldAdaptor adaptor) {
  if (toolchain::any_of(adaptor.getExtents(), [](Attribute a) { return !a; }))
    return nullptr;
  SmallVector<int64_t, 6> extents;
  for (auto attr : adaptor.getExtents())
    extents.push_back(toolchain::cast<IntegerAttr>(attr).getInt());
  Builder builder(getContext());
...
FuncOp FunctionLibraryOp::getShapeFunction(Operation *op) {
  auto attr = toolchain::dyn_cast_or_null<FlatSymbolRefAttr>(
...
FuncOp FuncOp::create(Location location, StringRef name, FunctionType type,
  ...
  return create(location, name, type, toolchain::ArrayRef(attrRef));
}
FuncOp FuncOp::create(Location location, StringRef name, FunctionType type,
                      ArrayRef<NamedAttribute> attrs,
...
OpFoldResult GetExtentOp::fold(FoldAdaptor adaptor) {
  auto elements = toolchain::dyn_cast_if_present<DenseIntElementsAttr>(adaptor.getShape());
  if (!elements)
    return nullptr;
...
void GetExtentOp::build(OpBuilder &builder, OperationState &result, Value shape,
  ...
  if (toolchain::isa<ShapeType>(shape.getType())) {
...
LogicalResult mlir::shape::MeetOp::inferReturnTypes(
  ...
  auto isShapeType = [](Type arg) {
    if (toolchain::isa<ShapeType>(arg))
      return true;
  ...
  for (auto t : drop_begin(types)) {
    Type l = acc, r = t;
    if (!toolchain::isa<ShapeType, SizeType>(l))
      std::swap(l, r);

    // Handle sizes, propagate error type if present.
    if (toolchain::isa<SizeType>(l)) {
      if (toolchain::isa<SizeType, IndexType>(r))
        acc = l;
      else
        return emitOptionalError(location, "requires all sizes or shapes");
    } else if (toolchain::isa<IndexType>(l)) {
      if (toolchain::isa<IndexType>(r))
        acc = r;
      else
        return emitOptionalError(location, "requires all sizes or shapes");
    } else if (toolchain::isa<ShapeType>(l)) {
      ...
      auto rank1 = toolchain::cast<RankedTensorType>(l).getShape()[0];
      auto rank2 = toolchain::cast<RankedTensorType>(r).getShape()[0];
...
bool mlir::shape::MeetOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  ...
  if (!toolchain::isa<ShapeType, SizeType>(lhs))
    std::swap(lhs, rhs);

  if (toolchain::isa<SizeType>(lhs))
    return toolchain::isa<SizeType, IndexType>(rhs);
  if (toolchain::isa<ShapeType>(lhs))
    return toolchain::isa<ShapeType, TensorType>(rhs);
...
OpFoldResult shape::RankOp::fold(FoldAdaptor adaptor) {
  auto shape = toolchain::dyn_cast_if_present<DenseIntElementsAttr>(adaptor.getShape());
  if (!shape)
    return {};
...
struct RankShapeOfCanonicalizationPattern
  ...
  LogicalResult matchAndRewrite(shape::RankOp op,
    ...
    auto rankedTensorType =
        toolchain::dyn_cast<RankedTensorType>(shapeOfOp.getArg().getType());
    if (!rankedTensorType)
      return failure();
    int64_t rank = rankedTensorType.getRank();
    if (toolchain::isa<IndexType>(op.getType())) {
      rewriter.replaceOpWithNewOp<arith::ConstantIndexOp>(op.getOperation(),
                                                          rank);
    } else if (toolchain::isa<shape::SizeType>(op.getType())) {
...
LogicalResult mlir::shape::RankOp::inferReturnTypes(
  ...
  if (toolchain::isa<ShapeType>(adaptor.getShape().getType()))
    inferredReturnTypes.assign({SizeType::get(context)});
  else
    inferredReturnTypes.assign({IndexType::get(context)});
  return success();
...
OpFoldResult NumElementsOp::fold(FoldAdaptor adaptor) {
  ...
  APInt product(64, 1);
  for (auto value : toolchain::cast<DenseIntElementsAttr>(shape))
    product *= value;
  Builder builder(getContext());
...
bool mlir::shape::MaxOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  if (l.size() != 1 || r.size() != 1)
    return false;
  if (toolchain::isa<ShapeType>(l.front()) && toolchain::isa<ShapeType>(r.front()))
    return true;
  if (toolchain::isa<SizeType>(l.front()) && toolchain::isa<SizeType>(r.front()))
    return true;
  return false;
}
...
struct ShapeOfOpToConstShapeOp : public OpRewritePattern<shape::ShapeOfOp> {
  ...
  LogicalResult matchAndRewrite(shape::ShapeOfOp op,
                                PatternRewriter &rewriter) const override {
    auto type = toolchain::dyn_cast<ShapedType>(op.getArg().getType());
...
struct ShapeOfCastExtentTensor : public OpRewritePattern<tensor::CastOp> {
  ...
  LogicalResult matchAndRewrite(tensor::CastOp op,
                                PatternRewriter &rewriter) const override {
    auto ty = toolchain::dyn_cast<RankedTensorType>(op.getType());
    if (!ty || ty.getRank() != 1)
      return failure();
    ...
    auto argTy = toolchain::dyn_cast<RankedTensorType>(shapeOfOp.getArg().getType());
...
LogicalResult mlir::shape::ShapeOfOp::inferReturnTypes(
  ...
  if (toolchain::isa<ValueShapeType>(adaptor.getArg().getType()))
    inferredReturnTypes.assign({ShapeType::get(context)});
  else {
    auto shapedTy = toolchain::cast<ShapedType>(adaptor.getArg().getType());
    int64_t rank =
...
bool mlir::shape::ShapeOfOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  ...
  if (!toolchain::isa<ShapeType, ShapedType>(lhs) ||
      !toolchain::isa<ShapeType, ShapedType>(rhs))
    return false;
...
bool SizeToIndexOp::areCastCompatible(TypeRange inputs, TypeRange outputs) {
  if (inputs.size() != 1 || outputs.size() != 1)
    return false;
  return toolchain::isa<IndexType, SizeType>(inputs[0]) &&
         toolchain::isa<IndexType>(outputs[0]);
}
...
LogicalResult SplitAtOp::fold(FoldAdaptor adaptor,
  ...
  if (!adaptor.getOperand() || !adaptor.getIndex())
    return failure();
  auto shapeVec = toolchain::to_vector<6>(
      toolchain::cast<DenseIntElementsAttr>(adaptor.getOperand()).getValues<int64_t>());
  auto shape = toolchain::ArrayRef(shapeVec);
  auto splitPoint = toolchain::cast<IntegerAttr>(adaptor.getIndex()).getInt();
...
OpFoldResult ToExtentTensorOp::fold(FoldAdaptor adaptor) {
  if (!adaptor.getInput())
    return OpFoldResult();
  Builder builder(getContext());
  auto shape = toolchain::to_vector<6>(
      toolchain::cast<DenseIntElementsAttr>(adaptor.getInput()).getValues<int64_t>());
...
bool ToExtentTensorOp::areCastCompatible(TypeRange inputs, TypeRange outputs) {
  if (inputs.size() != 1 || outputs.size() != 1)
    return false;
  if (auto inputTensor = toolchain::dyn_cast<RankedTensorType>(inputs[0])) {
    if (!toolchain::isa<IndexType>(inputTensor.getElementType()) ||
        inputTensor.getRank() != 1)
      return false;
  } else if (!toolchain::isa<ShapeType>(inputs[0])) {
    return false;
  }

  TensorType outputTensor = toolchain::dyn_cast<TensorType>(outputs[0]);
  return outputTensor && toolchain::isa<IndexType>(outputTensor.getElementType());
}
...
void ReduceOp::build(OpBuilder &builder, OperationState &result, Value shape,
  ...
  Type elementType;
  if (auto tensorType = toolchain::dyn_cast<TensorType>(shape.getType()))
...
LogicalResult ReduceOp::verify() {
  ...
  if (!toolchain::isa<IndexType>(block.getArgument(0).getType()))
    return emitOpError(
...
ParseResult ReduceOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // Resolve operands.
  auto initVals = toolchain::ArrayRef(operands).drop_front();
