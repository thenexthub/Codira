...
SmallVector<Value, 4>
getInputsOfCluster(const toolchain::SmallVector<Operation *, 8> &cluster) {
  SmallVector<Value, 4> inputs;
  toolchain::SmallDenseSet<Value> inputSet;
  toolchain::SmallDenseSet<Operation *> opSet;
...
createFuncFromCluster(OpBuilder &b, const SmallVector<Operation *, 8> &cluster,
  ...
  if (cluster.empty()) {
    ...
    for (auto inputAndArg : toolchain::zip(inputs, fnOp.getArguments()))
      bvm.map(std::get<0>(inputAndArg), std::get<1>(inputAndArg));
  }

  for (Operation *op : cluster)
    b.clone(*op, bvm);
  toolchain::SmallVector<Value, 4> fnReturns;
...
void OutlineShapeComputationPass::runOnOperation() {
  ...
  moduleOp.walk([&](func::FuncOp funcOp) {
    ...
    LLVM_DEBUG({
      toolchain::dbgs() << "onlyUsedByWithShapes table: \n";
    ...
    for (shape::WithOp withOp : allWithOps) {
      Value value = withOp.getOperand();
      for (Operation *user :
           toolchain::make_early_inc_range(withOp.getResult().getUsers())) {
        if (auto valueOf = toolchain::dyn_cast<shape::ValueOfOp>(user)) {
          // For pattern like
...
bool OutlineShapeComputationPass::calOnlyUsedByWithShapesRecursively(
  ...
  if (auto withOp = toolchain::dyn_cast<shape::WithOp>(op))
