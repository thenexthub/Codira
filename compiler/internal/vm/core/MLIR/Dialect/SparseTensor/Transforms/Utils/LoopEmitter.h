...
public:
  ...
  auto getLoopIVsRange() const {
    return toolchain::map_range(loopStack, [](const LoopInfo &li) { return li.iv; });
  }
  ...
  SmallVector<Value> getLoopIVs() const {
    return toolchain::to_vector(getLoopIVsRange());
  }

  /// Gets the current depth of the loop-stack.
  LoopId getCurrentDepth() const { return toolchain::range_size(getLoopIVsRange()); }
  ...
  auto unpackTensorLevelRange(ContainerTy &&c) const {
    ...
    static_assert(std::is_same_v<toolchain::remove_cvref_t<EltTy>, TensorLevel>,
                  "Must be unpacking a TensorLevel range");
    return toolchain::map_range(std::forward<ContainerTy>(c), [this](EltTy tl) {
  ...
  constexpr static toolchain::StringLiteral getLoopEmitterLoopAttrName() {
    return toolchain::StringLiteral("Emitted from");
  }
...
private:
  ...
  struct LoopInfo final {
    ...
    const toolchain::SmallVector<TensorLevel> tidLvls;
  ...
  using LoopBodyBuilder = toolchain::function_ref<void(OpBuilder &, Location, Value,
