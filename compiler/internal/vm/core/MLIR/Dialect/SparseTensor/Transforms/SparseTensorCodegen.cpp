...
static Value genSliceToSize(OpBuilder &builder, Location loc, Value mem,
                            Value sz) {
  auto memTp = toolchain::cast<MemRefType>(mem.getType());
...
public:
  ...
  SmallVector<Value> genImplementation(TypeRange retTypes, ValueRange args,
    ...
    const SparseTensorType stt(toolchain::cast<RankedTensorType>(rtp));
    const Level lvlRank = stt.getLvlRank();
    // Extract fields and coordinates from args.
    SmallVector<Value> fields = toolchain::to_vector(args.drop_back(lvlRank + 1));
    MutSparseTensorDescriptor desc(stt, fields);
    const SmallVector<Value> coords =
        toolchain::to_vector(args.take_back(lvlRank + 1).drop_back());
  ...
  std::string getMangledFuncName() {
    ...
    toolchain::raw_svector_ostream nameOstream(nameBuffer);
    ...
    for (Level l = 0; l < lvlRank; l++) {
      ...
      std::replace_if(
          ...
          [](char c) { return c == '(' || c == ','; }, '_');
      toolchain::erase_if(lvlType, [](char c) { return c == ')' || c == ' '; });
  ...
  matchAndRewrite(func::CallOp op, OneToNOpAdaptor adaptor,
    ...
    for (auto ret : op.getResults()) {
      ...
      if (sparseFlat.size() > 1) {
        ...
        toolchain::append_range(packedResultVals.back(),
...
public:
  ...
  matchAndRewrite(CompressOp op, OneToNOpAdaptor adaptor,
    ...
    auto desc = getMutDescriptorFromTensorTuple(adaptor.getTensor(), fields,
                                                op.getTensor().getType());
    Value values = toolchain::getSingleElement(adaptor.getValues());
    Value filled = toolchain::getSingleElement(adaptor.getFilled());
    Value added = toolchain::getSingleElement(adaptor.getAdded());
    Value count = toolchain::getSingleElement(adaptor.getCount());
    ...
    SmallVector<Type> flatSpTensorTps = toolchain::to_vector(
        toolchain::map_range(desc.getFields(), [](Value v) { return v.getType(); }));
...
public:
  ...
  matchAndRewrite(tensor::InsertOp op, OneToNOpAdaptor adaptor,
    ...
    SmallVector<Value> params = toolchain::to_vector(desc.getFields());
    SmallVector<Value> flatIndices = flattenValues(adaptor.getIndices());
    params.append(flatIndices.begin(), flatIndices.end());
    params.push_back(toolchain::getSingleElement(adaptor.getScalar()));
...
public:
  ...
  matchAndRewrite(tensor::ExtractSliceOp op, OneToNOpAdaptor adaptor,
    ...
    for (auto [idx, offset, size, stride] : toolchain::enumerate(
...
struct SparseDisassembleOpConverter
  ...
  matchAndRewrite(DisassembleOp op, OneToNOpAdaptor adaptor,
    ...
    SmallVector<Value> retValues = toolchain::to_vector(
        toolchain::map_range(retMem, [&rewriter, loc](Value v) -> Value {
