...
static Value genVectorLoad(PatternRewriter &rewriter, Location loc, VL vl,
  ...
  if (toolchain::isa<VectorType>(idxs.back().getType())) {
...
///
/// See https://toolchain.org/docs/GetElementPtr.html for some background on
...
static bool vectorizeSubscripts(PatternRewriter &rewriter, scf::ForOp forOp,
  ...
  for (auto sub : subs) {
    ...
    if (auto arg = toolchain::dyn_cast<BlockArgument>(sub)) {
    ...
    if (auto load = cast.getDefiningOp<memref::LoadOp>()) {
      ...
      if (codegen) {
        ...
        Type etp = toolchain::cast<VectorType>(vload.getType()).getElementType();
        if (!toolchain::isa<IndexType>(etp)) {
    ...
    if (auto load = cast.getDefiningOp<arith::AddIOp>()) {
      ...
      // Inspect.
      if (isInvariantValue(inv, block)) {
        if (auto arg = toolchain::dyn_cast<BlockArgument>(idx)) {
          if (isInvariantArg(arg, block) || !innermost)
            return false;
          if (codegen)
            idxs.push_back(
...
static bool vectorizeExpr(PatternRewriter &rewriter, scf::ForOp forOp, VL vl,
  ...
  if (auto arg = toolchain::dyn_cast<BlockArgument>(exp)) {
