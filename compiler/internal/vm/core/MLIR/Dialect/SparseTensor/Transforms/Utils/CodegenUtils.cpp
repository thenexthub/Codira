...
void mlir::sparse_tensor::genReshapeDstShape(
  ...
  if (reassociation.size() < srcShape.size()) {
    unsigned start = 0;
    for (const auto &map : toolchain::enumerate(reassociation)) {
...
void sparse_tensor::foreachInSparseConstant(
  ...
  toolchain::sort(elems, [order](const ElementAttr &lhs, const ElementAttr &rhs) {
    if (std::addressof(lhs) == std::addressof(rhs))
      return false;

    auto lhsCoords = toolchain::map_to_vector(
        lhs.first, [](IntegerAttr i) { return i.getInt(); });
    auto rhsCoords = toolchain::map_to_vector(
...
#endif // NDEBUG
  for (const auto &v : toolchain::enumerate(vs)) {
    const Value w =
...
sparse_tensor::genToMemref(OpBuilder &builder, Location loc, Value tensor) {
  auto tTp = toolchain::cast<TensorType>(tensor.getType());
...
Value sparse_tensor::genMapBuffers(
  ...
  for (Level l = 0; l < lvlRank; l++) {
    ...
    default:
      toolchain::report_fatal_error("unsupported dim2lvl in sparse tensor type");
    }
  ...
  for (Dimension d = 0; d < dimRank; d++) {
    ...
    default:
      toolchain::report_fatal_error("unsupported lvl2dim in sparse tensor type");
    }
