...
static bool isIntOrFPZero(Attribute attr) {
  if (auto f = toolchain::dyn_cast<FloatAttr>(attr); f && f.getValue().isZero())
    return true;
  if (auto i = toolchain::dyn_cast<IntegerAttr>(attr); i && i.getValue().isZero())
    return true;
  return false;
}
  ...
  for (TensorId tid = 0; tid < numTensors; tid++) {
    ...
    if (dimGetter && !isSynTensor(tid)) {
      for (Level l = 0; l < lvlRank; l++) {
        ...
        toolchain::sort(deps, toolchain::less_first());
...
void LoopEmitter::initializeLoopEmit(
  ...
  for (TensorId t = 0, numTensors = getNumManifestTensors(); t < numTensors;
    ...
    if (!stt.hasEncoding()) {
      ...
      if (toolchain::isa_and_nonnull<tensor::ExtractSliceOp>(tensor.getDefiningOp()))
...
void LoopEmitter::initSubSectIterator(OpBuilder &builder, Location loc) {
  ...
  for (TensorId t = 0, e = tensors.size(); t < e; t++) {
    ...
    toolchain::sort(depRedOrder, toolchain::less_first());
...
void LoopEmitter::categorizeIterators(
  ...
  toolchain::stable_sort(spIters, [](auto lhs, auto rhs) {
...
Region *LoopEmitter::enterCurrentCoIterationCase(OpBuilder &builder,
  ...
  toolchain::copy(iterArgs, reduc.begin());
  ...
  for (auto [i, tl] : toolchain::enumerate(unpackTensorLevelRange(tidLvls))) {
    if (caseBit[i]) {
...
Operation *LoopEmitter::enterCoIterationOverTensorsAtLvls(
  ...
  if (emitStrategy == SparseEmitStrategy::kSparseIterator) {
    if (tidLvls.size() == 1) {
      ...
      toolchain::copy(iterOp.getRegionIterArgs(), reduc.begin());
...
void LoopEmitter::exitForLoop(RewriterBase &rewriter, Location loc,
  ...
  if (emitStrategy == SparseEmitStrategy::kSparseIterator) {
    auto iterateOp = toolchain::cast<IterateOp>(loopInfo.loop);
    ...
    toolchain::copy(iterateOp.getResults(), reduc.begin());
    return;
  }
  if (auto forOp = toolchain::dyn_cast<scf::ForOp>(loopInfo.loop)) {
    if (!reduc.empty()) {
    ...
    // In-place update reduction variables.
    toolchain::copy(forOp.getResults(), reduc.begin());
  } else {
    auto parOp = toolchain::cast<scf::ParallelOp>(loopInfo.loop);
    if (!reduc.empty()) {
...
void LoopEmitter::exitWhileLoop(OpBuilder &builder, Location loc,
                                MutableArrayRef<Value> reduc) {
  const LoopInfo &loopInfo = loopStack.back();
  auto whileOp = toolchain::cast<scf::WhileOp>(loopInfo.loop);
...
void LoopEmitter::exitCurrentLoop(RewriterBase &rewriter, Location loc,
  ...
  if (emitStrategy == SparseEmitStrategy::kSparseIterator) {
    ...
    // In-place update reduction variables.
    toolchain::copy(p->getResults(), reduc.begin());
    loopStack.pop_back();
    return;
  }
  ...
  if (!loopInfo.userCodeBlock->empty() &&
      toolchain::isa<scf::YieldOp>(&loopInfo.userCodeBlock->back())) {
  ...
  if (toolchain::isa<scf::WhileOp>(loopInfo.loop)) {
...
std::pair<Operation *, Value> sparse_tensor::genCoIteration(
  ...
  assert(!toolchain::is_contained(ivs, nullptr));
