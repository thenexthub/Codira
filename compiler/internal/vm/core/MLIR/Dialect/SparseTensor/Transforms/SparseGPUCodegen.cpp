...
static Value genTensorToMemref(PatternRewriter &rewriter, Location loc,
                               Value tensor) {
  auto tensorType = toolchain::cast<ShapedType>(tensor.getType());
  auto memrefType =
...
static LogicalResult rewriteSpMV(PatternRewriter &rewriter,
  ...
  auto dnYType = toolchain::cast<ShapedType>(y.getType()).getElementType();
...
static LogicalResult rewriteSpMM(PatternRewriter &rewriter,
  ...
  auto dmatCType = toolchain::cast<ShapedType>(c.getType()).getElementType();
  ...
  Value buffer = buf.getResult(0);
  token = buf.getAsyncToken();
  auto dnCType = toolchain::cast<ShapedType>(c.getType()).getElementType();
...
static LogicalResult rewrite2To4SpMM(PatternRewriter &rewriter,
  ...
  Value dnC = dmatC.getResult(0);
  token = dmatC.getAsyncToken();
  auto dmatCType = toolchain::cast<ShapedType>(matC.getType()).getElementType();
  ...
  // Perform the SpMM.
  auto dnCType = toolchain::cast<ShapedType>(matC.getType()).getElementType();
