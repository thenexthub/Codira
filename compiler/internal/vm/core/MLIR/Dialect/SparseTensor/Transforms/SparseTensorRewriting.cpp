...
static bool isSparseTensor(Value v) {
  ...
  return enc && !toolchain::all_of(enc.getLvlTypes(),
...
struct FuseExtractSliceWithConcat
  ...
  LogicalResult matchAndRewrite(tensor::ExtractSliceOp extractOp,
    ...
    for (auto [idx, input] :
         toolchain::enumerate(concatOp.getInputs().drop_back())) {
    ...
    auto allEqual = [](ArrayRef<OpFoldResult> lhs, ArrayRef<OpFoldResult> rhs) {
      for (auto [l, r] : toolchain::zip(lhs, rhs)) {
    ...
    for (auto [i, input, offset] :
         toolchain::enumerate(concatOp.getInputs(), dimOffsets)) {
...
private:
  ...
  static void printContentsLevel(PatternRewriter &rewriter, Location loc,
    ...
    if (i < shape.size() - 1) {
      ...
      if (toolchain::isa<ComplexType>(val.getType())) {
...
public:
  ...
  LogicalResult matchAndRewrite(ReshapeOp op,
    ...
    if (dstTp.hasStaticDimShape()) {
      ...
      for (auto [idx, shape] : toolchain::enumerate(dstShape)) {
...
public:
  ...
  LogicalResult matchAndRewrite(ForeachOp op,
    ...
    if (toolchain::isa<scf::YieldOp>(last)) {
