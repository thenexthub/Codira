...
static scf::ValueVector genWhenInBound(
    ...
    toolchain::function_ref<scf::ValueVector(OpBuilder &, Location, Value)>
        builder) {
...
public:
  using TraverseBuilder = toolchain::function_ref<scf::ValueVector(
  ...
  bool isSubSectRoot() const {
    return !parent || !toolchain::isa<NonEmptySubSectIterator>(parent);
  }
...
public:
  ...
  void genInitImpl(OpBuilder &b, Location l, const SparseIterator *) override {
    if (randomAccessible()) {
      if (auto *p = toolchain::dyn_cast<SubSectIterator>(&parent)) {
        assert(p->lvl + 1 == lvl);
        wrap->genInit(b, l, p);
...
Value FilterIterator::genShouldFilter(OpBuilder &b, Location l) {
  ...
  return toolchain::getSingleElement(r);
}
        ...
        scf::ValueVector cont =
            genWhenInBound(b, l, *wrap, C_FALSE,
                           [this, isFirst](OpBuilder &b, Location l,
                             ...
                             ret = ORI(ret, toolchain::getSingleElement(isFirst));
                             return {ret};
                           });
...
ValueRange NonEmptySubSectIterator::inflateSubSectTree(
  ...
  auto *p = toolchain::cast<NonEmptySubSectIterator>(parent);
  assert(p->lvl + 1 == lvl);
...
void TrivialIterator::genInitImpl(OpBuilder &b, Location l,
  ...
  if (parent && !parent->isBatchIterator()) {
    pPos = parent->getCurPosition();
    if (toolchain::isa<PadIterator>(parent) && parent->randomAccessible()) {
...
SparseIterationSpace mlir::sparse_tensor::SparseIterationSpace::fromValues(
  ...
  for (auto [i, lt] : toolchain::enumerate(dstTp.getLvlTypes())) {
...
static const SparseIterator *tryUnwrapFilter(const SparseIterator *it) {
  auto *filter = toolchain::dyn_cast_or_null<FilterIterator>(it);
  if (filter)
...
std::unique_ptr<SparseIterator> sparse_tensor::makeTraverseSubSectIterator(
  ...
  auto &subSect =
      toolchain::cast<NonEmptySubSectIterator>(*tryUnwrapFilter(&subSectIter));
