...
SymVar DimLvlExpr::castSymVar() const {
  return SymVar(toolchain::cast<AffineSymbolExpr>(expr));
}
...
Var DimLvlExpr::castDimLvlVar() const {
  return Var(getAllowedVarKind(), toolchain::cast<AffineDimExpr>(expr));
}
...
DimLvlExpr::unpackBinop() const {
  ...
  const auto binop = toolchain::dyn_cast<AffineBinaryOpExpr>(expr);
