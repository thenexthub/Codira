...
static toolchain::hash_code hash_value(LevelType lt) {
  return toolchain::hash_value(static_cast<uint64_t>(lt));
}
} // namespace mlir::sparse_tensor
...
void StorageLayout::foreachField(
    toolchain::function_ref<bool(FieldIndex, SparseTensorFieldKind, Level,
                            LevelType)>
        callback) const {
...
void sparse_tensor::foreachFieldAndTypeInSparseTensor(
    ...
    toolchain::function_ref<bool(Type, FieldIndex, SparseTensorFieldKind, Level,
                            LevelType)>
        callback) {
...
void SparseTensorDimSliceAttr::print(toolchain::raw_ostream &os) const {
...
bool SparseTensorEncodingAttr::isAllDense() const {
  return !getImpl() || toolchain::all_of(getLvlTypes(), isDenseLT);
}

bool SparseTensorEncodingAttr::isAllOrdered() const {
  return !getImpl() || toolchain::all_of(getLvlTypes(), isOrderedLT);
}
...
SparseTensorEncodingAttr::translateShape(ArrayRef<int64_t> srcShape,
  ...
  for (AffineExpr exp : transMap.getResults()) {
    ...
    // use toolchain namespace here to avoid ambiguity
    if (auto c = toolchain::dyn_cast<AffineConstantExpr>(evalExp)) {
      ret.push_back(c.getValue() + 1);
    } else {
      if (auto mod = toolchain::dyn_cast<AffineBinaryOpExpr>(evalExp);
        ...
        if (auto bound = toolchain::dyn_cast<AffineConstantExpr>(mod.getRHS())) {
...
Attribute SparseTensorEncodingAttr::parse(AsmParser &parser, Type type) {
  ...
  while (succeeded(parser.parseOptionalKeyword(&attrName))) {
    ...
    case 0: { // map
      ...
      if (toolchain::any_of(dimSlices, isDefined)) {
    ...
    case 1: { // posWidth
      ...
      if (failed(parser.parseAttribute(attr)))
        return {};
      auto intAttr = toolchain::dyn_cast<IntegerAttr>(attr);
      if (!intAttr) {
    ...
    case 3: { // explicitVal
      ...
      if (failed(parser.parseAttribute(attr)))
        return {};
      if (auto result = toolchain::dyn_cast<FloatAttr>(attr)) {
        explicitVal = result;
      } else if (auto result = toolchain::dyn_cast<IntegerAttr>(attr)) {
        explicitVal = result;
      } else if (auto result = toolchain::dyn_cast<complex::NumberAttr>(attr)) {
        explicitVal = result;
      } else {
        parser.emitError(parser.getNameLoc(),
...
LogicalResult SparseTensorEncodingAttr::verify(
  ...
  auto *it = toolchain::find_if(lvlTypes, isSingletonLT);
  ...
  auto soaLvls = toolchain::make_filter_range(lvlTypes, [](LevelType lt) {
    return lt.isa<LevelPropNonDefault::SoA>();
  });
  if (toolchain::any_of(soaLvls, [](LevelType lt) {
  ...
  if (auto it = toolchain::find_if(lvlTypes, isNOutOfMLT);
      it != std::end(lvlTypes)) {
...
LogicalResult SparseTensorEncodingAttr::verifyEncoding(
  ...
  if (auto expVal = getExplicitVal()) {
    Type attrType = toolchain::dyn_cast<TypedAttr>(expVal).getType();
  ...
  if (auto impVal = getImplicitVal()) {
    Type attrType = toolchain::dyn_cast<TypedAttr>(impVal).getType();
    if (attrType != elementType) {
    ...
    auto impFVal = toolchain::dyn_cast<FloatAttr>(impVal);
    auto impIntVal = toolchain::dyn_cast<IntegerAttr>(impVal);
    auto impComplexVal = toolchain::dyn_cast<complex::NumberAttr>(impVal);
...
mlir::sparse_tensor::getSparseTensorEncoding(Type type) {
  if (auto ttp = toolchain::dyn_cast<RankedTensorType>(type))
    return toolchain::dyn_cast_or_null<SparseTensorEncodingAttr>(ttp.getEncoding());
  if (auto mdtp = toolchain::dyn_cast<StorageSpecifierType>(type))
...
bool mlir::sparse_tensor::hasAnyNonIdentityOperandsOrResults(Operation *op) {
  ...
  return toolchain::any_of(op->getOperands(), hasNonIdentityMap) ||
         toolchain::any_of(op->getResults(), hasNonIdentityMap);
}
...
static LogicalResult verifyPackUnPack(Operation *op, bool requiresStaticShape,
  ...
  if (cooStartLvl < stt.getLvlRank()) {
    ...
    auto cooTp = toolchain::cast<ShapedType>(lvlTps.back());
  ...
  layout.foreachField([&idx, &misMatch, stt, valTp,
    ...
    Type inpElemTp = toolchain::cast<TensorType>(inputTp).getElementType();
...
LogicalResult DisassembleOp::verify() {
  ...
  for (auto [ot, rt] : toolchain::zip_equal(getOutLevels(), getRetLevels()))
...
LogicalResult ConvertOp::verify() {
  ...
  auto dstEnc =
      toolchain::dyn_cast_or_null<SparseTensorEncodingAttr>(tp2.getEncoding());
...
LogicalResult CrdTranslateOp::fold(FoldAdaptor adaptor,
  ...
  bool sameDef = def && toolchain::all_of(getInCrds(), [def](Value v) {
  ...
  // coordinates.
  bool sameOrder = toolchain::all_of(toolchain::zip_equal(def.getOutCrds(), getInCrds()),
                                [](auto valuePair) {
...
OpFoldResult LvlOp::fold(FoldAdaptor adaptor) {
  auto lvlIndex = toolchain::dyn_cast_if_present<IntegerAttr>(adaptor.getIndex());
  if (!lvlIndex)
    return {};
...
LogicalResult ReinterpretMapOp::verify() {
  ...
  for (auto [srcLvlTp, dstLvlTp] : toolchain::zip(srcLvlTps, dstLvlTps))
  ...
  for (auto [srcLvlSz, dstLvlSz] : toolchain::zip(srcLvlShape, dstLvlShape)) {
...
bool ConcatenateOp::needsExtraSort() {
  ...
  bool allSameOrdered = toolchain::all_of(getInputs(), [dstStt](Value op) {
...
LogicalResult ForeachOp::verify() {
  ...
  for (Dimension d = 0; d < dimRank; d++)
    if (args[d].getType() != iTp)
      return emitError(
          toolchain::formatv("Expecting Index type for argument at index {0}", d));

  const auto elemTp = t.getElementType();
  const auto valueTp = args[dimRank].getType();
  if (elemTp != valueTp)
    return emitError(
        toolchain::formatv("Unmatched element type between input tensor and "
...
LogicalResult SortOp::verify() {
  ...
  if (nx < 1)
    return emitError(toolchain::formatv("Expected rank(perm_map) > 1, got {0}", nx));

  if (!xPerm.isPermutation())
    return emitError(
        toolchain::formatv("Expected a permutation map, got {0}", xPerm));
  ...
  const auto checkDim = [&](Value v, Size minSize,
    ...
    if (ShapedType::isStatic(sh) && sh < minSize)
      return emitError(
          toolchain::formatv("{0} got {1} < {2}", message, sh, minSize));
    return success();
  };
  uint64_t n = cn.value();
  uint64_t ny = 0;
...
parseSparseIterateLoop(OpAsmParser &parser, OperationState &state,
  ...
  for (auto [it, tp] : toolchain::zip_equal(iterators, iterSpaceTps)) {
    IterSpaceType spaceTp = toolchain::dyn_cast<IterSpaceType>(tp);
    if (!spaceTp)
      return parser.emitError(parser.getNameLoc(),
  ...
  if (hasIterArgs) {
    ...
    for (auto [it, init, tp] : toolchain::zip_equal(args, initArgs, state.types)) {
      it.type = tp;
...
struct RemoveUnusedLvlCrds : public OpRewritePattern<IterateOp> {
  ...
  LogicalResult matchAndRewrite(IterateOp iterateOp,
    ...
    toolchain::BitVector toRemove(iterateOp.getBody()->getNumArguments());
...
void IterateOp::build(OpBuilder &builder, OperationState &odsState,
  ...
  unsigned rank = toolchain::cast<IterSpaceType>(iterSpace.getType()).getSpaceDim();
  // All ones.
...
void IterateOp::build(OpBuilder &builder, OperationState &odsState,
  ...
  bodyBlock->addArgument(
      toolchain::cast<IterSpaceType>(iterSpace.getType()).getIteratorType(),
      odsState.location);
}
...
LogicalResult IterateOp::verifyRegions() {
  ...
  if (!toolchain::all_equal({initArgs.size(), iterArgs.size(), yieldVals.size(),
                        opResults.size()})) {
    return emitOpError() << "number mismatch between iter args and results.";
  }

  for (auto [i, init, iter, yield, ret] :
       toolchain::enumerate(initArgs, iterArgs, yieldVals, opResults)) {
...
ParseResult CoIterateOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  while (succeeded(parser.parseOptionalKeyword("case"))) {
    ...
    for (auto [i, definedIdx] : toolchain::enumerate(definedItSet.bits())) {
      // Resolve the iterator type based on the iteration space type.
      auto spaceTp = toolchain::cast<IterSpaceType>(spaces[definedIdx].getType());
...
void CoIterateOp::print(OpAsmPrinter &p) {
  p << " (";
  toolchain::interleaveComma(getIterSpaces(), p, [&](auto s) { p << s; });
  p << ")";
...
LogicalResult CoIterateOp::verifyRegions() {
  ...
  toolchain::SmallSetVector<uint64_t, 8> set(cases.begin(), cases.end());
