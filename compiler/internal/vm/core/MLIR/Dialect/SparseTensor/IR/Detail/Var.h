...
[[nodiscard]] constexpr bool isWF(VarKind vk) {
  const auto vk_ = toolchain::to_underlying(vk);
  return 0 <= vk_ && vk_ <= 2;
}
...
constexpr char toChar(VarKind vk) {
  ...
  const auto vk_ = static_cast<int_fast8_t>(toolchain::to_underlying(vk));
...
template <typename T>
using VarKindArray = toolchain::EnumeratedArray<T, VarKind, VarKind::Level>;
  ...
  public:
    constexpr Impl(VarKind vk, Num n)
        : data((static_cast<Storage>(n) << 2) |
               static_cast<Storage>(toolchain::to_underlying(vk))) {
...
class Ranks final {
  ...
  static constexpr unsigned to_index(VarKind vk) {
    assert(isWF(vk) && "unknown VarKind");
    return static_cast<unsigned>(toolchain::to_underlying(vk));
  }
...
class VarSet final {
  VarKindArray<toolchain::SmallBitVector> impl;
...
private:
  ...
  toolchain::SMLoc loc;             // The location of the first occurence.
...
public:
  constexpr VarInfo(ID id, StringRef name, toolchain::SMLoc loc, VarKind vk,
  ...
  constexpr toolchain::SMLoc getLoc() const { return loc; }
...
class VarEnv final {
  ...
  toolchain::StringMap<VarInfo::ID> ids;
...
public:
  ...
  VarInfo const &access(VarInfo::ID id) const {
    ...
    return vars[toolchain::to_underlying(id)];
  }
...
public:
  ...
  create(StringRef name, toolchain::SMLoc loc, VarKind vk, bool verifyUsage = false);
  ...
  std::optional<std::pair<VarInfo::ID, bool>>
  lookupOrCreate(Policy creationPolicy, StringRef name, toolchain::SMLoc loc,
                 VarKind vk);
