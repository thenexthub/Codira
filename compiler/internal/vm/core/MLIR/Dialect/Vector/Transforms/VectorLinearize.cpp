...
static bool stridesAllOne(TOp op) {
  ...
  return toolchain::all_of(strides, isOneInteger);
}
...
struct LinearizeVectorInsertStridedSlice final
  ...
  matchAndRewrite(vector::InsertStridedSliceOp insertStridedSliceOp,
    ...
    for (auto [index, sliceIndex] : toolchain::enumerate(sliceIndices)) {
...
struct LinearizeVectorShuffle final
  ...
  matchAndRewrite(vector::ShuffleOp shuffleOp, OpAdaptor adaptor,
    ...
    // outermost dim.
    if (rank > 1) {
      toolchain::ArrayRef<int64_t> shape = shuffleOp.getV1().getType().getShape();
    ...
    toolchain::SmallVector<int64_t, 2> indices(totalSizeOfShuffledElmnts);
    for (auto [i, value] : toolchain::enumerate(mask)) {
...
struct LinearizeVectorExtract final
  ...
  matchAndRewrite(vector::ExtractOp extractOp, OpAdaptor adaptor,
    ...
    toolchain::ArrayRef<int64_t> shape = extractOp.getSource().getType().getShape();
    int64_t size = extractOp.getSource().getType().getNumElements();

    // Compute linearized offset.
    int64_t linearizedOffset = 0;
    toolchain::ArrayRef<int64_t> offsets = extractOp.getStaticPosition();
    for (auto [i, off] : toolchain::enumerate(offsets)) {
      size /= shape[i];
    ...
    toolchain::SmallVector<int64_t, 2> indices(size);
...
struct LinearizeVectorInsert final
  ...
  matchAndRewrite(vector::InsertOp insertOp, OpAdaptor adaptor,
    ...
    for (auto [dim, offset] : toolchain::enumerate(offsetsNd)) {
    ...
    // Vector case: generate a shuffle.
    toolchain::SmallVector<int64_t, 2> indices(dstSize);
...
struct LinearizeVectorLoad final : public OpConversionPattern<vector::LoadOp> {
  ...
  matchAndRewrite(vector::LoadOp loadOp, OpAdaptor adaptor,
    ...
    // any).
    if (!toolchain::all_of(shape.drop_back(1), [](auto d) { return d == 1; }))
      return rewriter.notifyMatchFailure(loadOp,
                                         "only vector<1x1x...xN> supported");

    if (toolchain::any_of(scalableDims.drop_back(1), [](bool s) { return s; }))
      return rewriter.notifyMatchFailure(loadOp,
