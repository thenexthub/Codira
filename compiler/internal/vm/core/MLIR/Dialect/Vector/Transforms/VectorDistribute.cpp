...
struct WarpOpToScfIfPattern : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    for (const auto &it : toolchain::enumerate(warpOp.getArgs())) {
...
std::tuple<toolchain::SmallSetVector<Value, 32>, SmallVector<Type>,
           SmallVector<Type>>
getInnerRegionEscapingValues(WarpExecuteOnLane0Op warpOp, Region &innerRegion,
                             DistributionMapFn distributionMapFn) {
  toolchain::SmallSetVector<Value, 32> escapingValues;
...
struct WarpOpTransferWrite : public WarpDistributionPattern {
  ...
  LogicalResult tryDistributeOp(RewriterBase &rewriter,
    ...
    for (auto [seqSize, distSize] :
         toolchain::zip_equal(writtenVectorType.getShape(), targetType.getShape())) {
    ...
    for (auto it : toolchain::zip(indexMap.getResults(), map.getResults())) {
      AffineExpr d0, d1;
  ...
  LogicalResult tryExtractOp(RewriterBase &rewriter,
    ...
    if (toolchain::all_of(warpOp.getOps(),
                     toolchain::IsaPred<vector::TransferWriteOp, gpu::YieldOp>))
      return failure();
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    if (!toolchain::all_of(writeOp->getOperands(), [&](Value value) {
...
struct WarpOpElementwise : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    for (unsigned i : toolchain::seq(unsigned(0), elementWise->getNumOperands())) {
...
struct WarpOpConstant : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    OpOperand *yieldOperand =
        getWarpResult(warpOp, toolchain::IsaPred<arith::ConstantOp>);
    if (!yieldOperand)
      return failure();
...
struct WarpOpStep final : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *yieldOperand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::StepOp>);
    if (!yieldOperand)
      return failure();
    ...
    if (resTy.getNumElements() != static_cast<int64_t>(warpOp.getWarpSize()))
      return rewriter.notifyMatchFailure(
          warpOp,
          toolchain::formatv("Expected result size ({0}) to be of warp size ({1})",
...
struct WarpOpTransferRead : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    rewriter.setInsertionPointAfter(newWarpOp);
    for (auto it : toolchain::zip_equal(indexMap.getResults(), map.getResults())) {
      AffineExpr d0, d1;
...
struct WarpOpBroadcast : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::BroadcastOp>);
    if (!operand)
      return failure();
...
struct WarpOpShapeCast : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::ShapeCastOp>);
    if (!operand)
      return failure();
    ...
    if (castDistributedRank < castOriginalRank) {
      ...
      toolchain::append_range(shape, castDistributedType.getShape());
      castDistributedType =
...
template <typename OpType,
          typename = std::enable_if_t<toolchain::is_one_of<
              OpType, vector::CreateMaskOp, vector::ConstantMaskOp>::value>>
struct WarpOpCreateMask : public WarpDistributionPattern {
  using Base::Base;
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *yieldOperand = getWarpResult(warpOp, (toolchain::IsaPred<OpType>));
    if (!yieldOperand)
      return failure();
    ...
    if (mask->getOperands().size() &&
        !toolchain::all_of(mask->getOperands(), [&](Value value) {
...
struct WarpOpInsertStridedSlice : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::InsertStridedSliceOp>);
    if (!operand)
      return failure();
    unsigned int operandNumber = operand->getOperandNumber();
    auto insertOp =
...
struct WarpOpExtractStridedSlice : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::ExtractStridedSliceOp>);
    if (!operand)
      return failure();
    unsigned int operandNumber = operand->getOperandNumber();
    auto extractOp =
    ...
    if (distributedDim < numOfExtractedDims) {
      int64_t distributedDimOffset =
          toolchain::cast<IntegerAttr>(extractOp.getOffsets()[distributedDim])
              .getInt();
      int64_t distributedDimSize =
          toolchain::cast<IntegerAttr>(extractOp.getSizes()[distributedDim])
              .getInt();
    ...
    WarpExecuteOnLane0Op newWarpOp = moveRegionToNewWarpOpAndAppendReturns(
        ...
        newRetIndices);
    rewriter.setInsertionPointAfter(newWarpOp);
    SmallVector<Attribute> distributedSizes = toolchain::map_to_vector(
...
struct WarpOpExtract : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::ExtractOp>);
    if (!operand)
      return failure();
    unsigned int operandNumber = operand->getOperandNumber();
...
struct WarpOpInsertScalar : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand = getWarpResult(warpOp, toolchain::IsaPred<vector::InsertOp>);
    if (!operand)
      return failure();
    unsigned int operandNumber = operand->getOperandNumber();
...
struct WarpOpScfIfOp : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    toolchain::SmallDenseMap<unsigned, unsigned> ifResultMapping;
    toolchain::SmallDenseMap<unsigned, VectorType> ifResultDistTypes;
    ...
    auto [escapingValuesElse, escapingValueInputTypesElse,
        ...
        getInnerRegionEscapingValues(warpOp, ifOp.getElseRegion(),
                                     distributionMapFn);
    if (toolchain::is_contained(escapingValueDistTypesThen, Type{}) ||
        toolchain::is_contained(escapingValueDistTypesElse, Type{}))
      return failure();
    ...
    for (auto [idx, val] :
         toolchain::zip_equal(nonIfYieldIndices, nonIfYieldValues)) {
    ...
    for (auto [i, res] : toolchain::enumerate(ifOp.getResults())) {
    ...
    auto encloseRegionInWarpOp =
        [&](Block *oldIfBranch, Block *newIfBranch,
          ...
          toolchain::SmallDenseMap<Value, int64_t> escapeValToBlockArgIndex;
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    auto [escapingValues, escapingValueInputTypes, escapingValueDistTypes] =
        getInnerRegionEscapingValues(warpOp, forOp.getBodyRegion(),
                                     distributionMapFn);
    if (toolchain::is_contained(escapingValueDistTypes, Type{}))
      return failure();
    ...
    toolchain::SmallDenseMap<unsigned, unsigned> forResultMapping;
    toolchain::SmallDenseMap<unsigned, VectorType> forResultDistTypes;
    for (OpOperand &yieldOperand : warpOpYield->getOpOperands()) {
    ...
    for (auto [i, initArg] : toolchain::enumerate(forOp.getInitArgs())) {
    ...
    // types.
    for (auto [i, v] :
         toolchain::zip_equal(nonForResultIndices, nonForYieldedValues)) {
    ...
    toolchain::SmallDenseMap<Value, int64_t> argIndexMapping;
...
struct WarpOpReduction : public WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *yieldOperand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::ReductionOp>);
    if (!yieldOperand)
      return failure();
...
static bool canBeHoisted(Operation *op,
  ...
  return toolchain::all_of(op->getOperands(), definedOutside) &&
...
void mlir::vector::moveScalarUniformCode(WarpExecuteOnLane0Op warpOp) {
  ...
  toolchain::SmallSetVector<Operation *, 8> opsToMove;
  ...
  for (auto &op : body->without_terminator()) {
    bool hasVectorResult = toolchain::any_of(op.getResults(), [](Value result) {
