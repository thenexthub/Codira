...
struct CastAwayInsertLeadingOneDim : public OpRewritePattern<vector::InsertOp> {
  ...
  LogicalResult matchAndRewrite(vector::InsertOp insertOp,
    ...
    SmallVector<OpFoldResult> newPosition =
        toolchain::to_vector(ArrayRef(oldPosition).take_back(newPosRank));
...
mlir::vector::castAwayContractionLeadingOneDim(vector::ContractionOp contractOp,
  ...
  for (const auto &it : toolchain::enumerate(oldIteratorTypes)) {
  ...
  for (const auto &it : toolchain::enumerate(oldIndexingMaps)) {
    ...
    if (orginalZeroDim != dimToDrop) {
      ...
      for (auto [index, dim] :
           toolchain::enumerate(ArrayRef<int64_t>(perm).drop_back(1))) {
...
struct CastAwayConstantMaskLeadingOneDim
  ...
  LogicalResult matchAndRewrite(vector::ConstantMaskOp mask,
    ...
    int64_t flatLeadingSize =
        toolchain::product_of(dimSizes.take_front(dropDim + 1));
