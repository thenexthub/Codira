...
static void pruneNonTransposedDims(ArrayRef<int64_t> transpose,
  ...
  for (size_t transpDim : toolchain::reverse(transpose)) {
...
static Value create4x128BitSuffle(ImplicitLocOpBuilder &b, Value v1, Value v2,
  ...
  auto appendToMask = [&](int64_t base, uint8_t control) {
    switch (control) {
    case 0:
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{base + 0, base + 1,
                                                        base + 2, base + 3});
      break;
    case 1:
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{base + 4, base + 5,
                                                        base + 6, base + 7});
      break;
    case 2:
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{base + 8, base + 9,
                                                        base + 10, base + 11});
      break;
    case 3:
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{base + 12, base + 13,
                                                        base + 14, base + 15});
      break;
    default:
