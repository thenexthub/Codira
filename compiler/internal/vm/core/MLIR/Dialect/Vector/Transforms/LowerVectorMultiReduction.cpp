...
/// Exceptions. See https://toolchain.org/LICENSE.txt for license information.
...
public:
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp multiReductionOp,
    ...
    toolchain::SmallDenseSet<int64_t> reductionDimsSet(reductionDims.begin(),
                                                  reductionDims.end());
    ...
    if (useInnerDimsForReduction &&
        (parallelDims ==
         toolchain::to_vector<4>(toolchain::seq<int64_t>(0, parallelDims.size()))))
      return failure();

    if (!useInnerDimsForReduction &&
        (parallelDims == toolchain::to_vector<4>(toolchain::seq<int64_t>(
                             reductionDims.size(),
...
public:
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp multiReductionOp,
    ...
    if (toolchain::count(srcScalableDims, true) > 1)
      return failure();
    ...
    for (const auto &it : toolchain::enumerate(reductionMask)) {
      int64_t i = it.index();
    ...
    int64_t counter = 0;
    if (useInnerDimsForReduction &&
        toolchain::any_of(parallelDims, [&](int64_t i) { return i != counter++; }))
      return failure();
    ...
    bool isParallelDimScalable = toolchain::is_contained(parallelScalableDims, true);
    ...
    if (maskableOp.isMasked()) {
      ...
      auto maskCastedType = VectorType::get(
          vectorShape,
          toolchain::cast<VectorType>(vectorMask.getType()).getElementType());
...
struct OneDimMultiReductionToTwoDim
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp multiReductionOp,
    ...
    assert(!toolchain::isa<VectorType>(multiReductionOp.getDestType()) &&
    ...
    Value castMask;
    if (maskableOp.isMasked()) {
      auto maskType = toolchain::cast<VectorType>(mask.getType());
