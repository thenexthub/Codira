...
void VectorShuffleTreeBuilder::computeShuffleTreeIntervals() {
  ...
  for (const auto &[idx, toElemsOp] : toolchain::enumerate(toElemsDefs))
  ...
  for (const auto &[idx, element] :
       toolchain::enumerate(fromElemsOp.getElements())) {
  ...
  for (unsigned level = 1; level < numLevels; ++level) {
    ...
    SmallVector<Interval> currentLevelIntervals(
        toolchain::divideCeil(prevLevelIntervals.size(), 2),
        {kMaxUnsigned, kMaxUnsigned});
...
void VectorShuffleTreeBuilder::dump() {
  LLVM_DEBUG({
    ...
    toolchain::dbgs() << "VectorShuffleTreeBuilder Configuration:\n";
    ++indLv;
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale) << "* Inputs:\n";
    ++indLv;
    for (const auto &toElemsOp : toElemsDefs)
      toolchain::dbgs() << toolchain::indent(indLv, kIndScale) << toElemsOp << "\n";
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale) << fromElemsOp << "\n\n";
    --indLv;

    toolchain::dbgs() << toolchain::indent(indLv, kIndScale)
                 << "* Total levels: " << numLevels << "\n";
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale)
                 << "* Vector sizes per level: ";
    toolchain::interleaveComma(vectorSizePerLevel, toolchain::dbgs());
    toolchain::dbgs() << "\n";
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale)
                 << "* Input intervals per level:\n";
    ++indLv;
    for (const auto &[level, intervals] : toolchain::enumerate(intervalsPerLevel)) {
      toolchain::dbgs() << toolchain::indent(indLv, kIndScale) << "* Level " << level
                   << ": ";
      toolchain::interleaveComma(intervals, toolchain::dbgs(),
                            [](const Interval &interval) {
                              toolchain::dbgs() << "[" << interval.first << ","
                                           << interval.second << "]";
                            });
      toolchain::dbgs() << "\n";
    }
...
LogicalResult VectorShuffleTreeBuilder::computeShuffleTree() {
  ...
  numLevels = 1u + std::max(1u, toolchain::Log2_64_Ceil(toElemsDefs.size()));
...
static SmallVector<int64_t> computePermutationShuffleMask(
  ...
  LLVM_DEBUG({
    unsigned indLv = 1;
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale) << "* Permutation mask: [";
    toolchain::interleaveComma(mask, toolchain::dbgs());
    toolchain::dbgs() << "]\n";
    ++indLv;
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale)
...
static SmallVector<int64_t> computePropagationShuffleMask(
  ...
  LLVM_DEBUG({
    ...
    toolchain::dbgs() << toolchain::indent(indLv, kIndScale) << "* Result mask: [";
    toolchain::interleaveComma(mask, toolchain::dbgs());
    toolchain::dbgs() << "]\n";
  });
...
Value VectorShuffleTreeBuilder::generateShuffleTree(PatternRewriter &rewriter) {
  LLVM_DEBUG(toolchain::dbgs() << "VectorShuffleTreeBuilder Code Generation:\n");

  // Initialize work list with the `vector.to_elements` sources.
  SmallVector<Value> levelInputs;
  toolchain::transform(toElemsDefs, std::back_inserter(levelInputs),
  ...
  for (const auto &[nextLevelVectorSize, intervals] :
       toolchain::zip_equal(ArrayRef(vectorSizePerLevel).drop_front(),
                       ArrayRef(intervalsPerLevel).drop_back())) {

    duplicateLastIfOdd(levelInputs);

    LLVM_DEBUG(toolchain::dbgs() << toolchain::indent(1, kIndScale)
...
struct ToFromElementsToShuffleTreeRewrite final
  ...
  LogicalResult matchAndRewrite(vector::FromElementsOp fromElemsOp,
    ...
    if (toolchain::any_of(toElemsDefs, [](ToElementsOp toElemsOp) {
    ...
    if (toElemsDefs.size() == 1) {
      ...
      if (toolchain::equal(fromElemsOp.getElements(), toElemsOp0.getResults())) {
        return rewriter.notifyMatchFailure(
