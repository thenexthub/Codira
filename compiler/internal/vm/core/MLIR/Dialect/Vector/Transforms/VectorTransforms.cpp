...
struct MultiReduceToContract
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp reduceOp,
    ...
    for (const auto &isReduceDim : toolchain::enumerate(reductionMask)) {
    ...
    rewriter.replaceOpWithNewOp<mlir::vector::ContractionOp>(
        ...
        rewriter.getArrayAttr(toolchain::to_vector(toolchain::map_range(
...
struct CombineContractABTranspose final
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp contractOp,
    ...
    SmallVector<AffineMap> maps =
        toolchain::to_vector<4>(contractOp.getIndexingMapsArray());
...
struct CombineContractResultTranspose final
  ...
  LogicalResult matchAndRewrite(vector::TransposeOp resTOp,
    ...
    auto maps = toolchain::to_vector<3>(contractOp.getIndexingMapsArray());
...
FailureOr<Value> combineContractAndBroadcast(vector::ContractionOp contractOp,
  ...
  for (Value *operand : {&lhs, &rhs}) {
    ...
    for (const auto &dim : toolchain::enumerate(srcType.getShape())) {
      if (dim.value() !=
  ...
  toolchain::SmallBitVector unusedDimsBitVector = getUnusedDimsBitVector(maps);
...
struct ReorderElementwiseOpsOnTranspose final
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // same map.
    if (!toolchain::all_equal(transposeMaps))
...
static SmallVector<int64_t> getIntValueVector(ArrayAttr arrayAttr) {
  return toolchain::to_vector<4>(
      toolchain::map_range(arrayAttr.getAsRange<IntegerAttr>(),
...
struct BubbleDownBitCastForStridedSliceExtract
  ...
  LogicalResult matchAndRewrite(vector::ExtractStridedSliceOp extractOp,
    ...
    if (toolchain::any_of(extractOp.getStrides().getAsValueRange<IntegerAttr>(),
    ...
    SmallVector<int64_t> dims =
        toolchain::to_vector<4>(cast<VectorType>(extractOp.getType()).getShape());
...
struct BubbleUpBitCastForStridedSliceInsert
  ...
  LogicalResult matchAndRewrite(vector::BitCastOp bitcastOp,
    ...
    // Only accept all one strides for now.
    if (toolchain::any_of(insertOp.getStrides().getAsValueRange<IntegerAttr>(),
                     [](const APInt &val) { return !val.isOne(); }))
      return failure();
    ...
    SmallVector<int64_t> srcDims =
        toolchain::to_vector<4>(insertOp.getSourceVectorType().getShape());
    ...
    SmallVector<int64_t> dstDims =
        toolchain::to_vector<4>(insertOp.getDestVectorType().getShape());
...
struct ReorderElementwiseOpsOnBroadcast final
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (!toolchain::all_of(op->getOperands(), [broadcastSource](Value val) {
...
public:
  ...
  LogicalResult matchAndRewrite(vector::ExtractOp op,
    ...
    if (!toolchain::all_equal(eltwise->getOperandTypes()))
...
public:
  ...
  LogicalResult matchAndRewrite(vector::ExtractOp op,
    ...
    for (auto i : toolchain::seq<int64_t>(rankOffset, indices.size() - finalRank)) {
...
static Value buildVectorComparison(PatternRewriter &rewriter, Operation *op,
  ...
  if (dim == 0 && force32BitVectorIndices) {
    ...
    indicesAttr = rewriter.getI32VectorAttr(
        toolchain::to_vector<4>(toolchain::seq<int32_t>(0, dim)));
  } else {
    indicesAttr = rewriter.getI64VectorAttr(
        toolchain::to_vector<4>(toolchain::seq<int64_t>(0, dim)));
  }
...
public:
  ...
  LogicalResult matchAndRewrite(ConcreteOp xferOp,
    ...
    //       dimensions here.
    unsigned lastIndex = toolchain::size(xferOp.getIndices()) - 1;
...
class DropInnerMostUnitDimsTransferRead
  ...
  LogicalResult matchAndRewrite(vector::TransferReadOp readOp,
    ...
    if (toolchain::is_contained(droppedInBounds, false))
      return failure();
...
static VectorType dropNonScalableUnitDimFromType(VectorType inVecTy) {
  ...
  for (auto [dim, isScalable] :
       toolchain::zip_equal(inVecShape, inVecTy.getScalableDims())) {
    if (dim == 1 && !isScalable)
      continue;
...
struct DropUnitDimsFromTransposeOp final
  ...
  LogicalResult matchAndRewrite(vector::TransposeOp op,
    ...
    auto sourceDims = toolchain::to_vector(vector::getDims(sourceType));
    SmallVector<int64_t> droppedDimsBefore(sourceType.getRank());
    int64_t droppedDims = 0;
    for (auto [i, dim] : toolchain::enumerate(sourceDims)) {
...
struct BreakDownVectorReduction final : OpRewritePattern<vector::ReductionOp> {
  ...
  LogicalResult matchAndRewrite(vector::ReductionOp op,
    ...
    if (numElems > maxNumElementsToExtract) {
      return rewriter.notifyMatchFailure(
          op, toolchain::formatv("has too many vector elements ({0}) to break down "
                            "(max allowed: {1})",
    ...
    for (auto [idx, extractedElem] : toolchain::enumerate(extracted))
      extractedElem = vector::ExtractOp::create(rewriter, loc, op.getVector(),
                                                static_cast<int64_t>(idx));

    Value res = extracted.front();
    for (auto extractedElem : toolchain::drop_begin(extracted))
...
struct FoldArithToVectorOuterProduct : public OpRewritePattern<MulOpType> {
  ...
  LogicalResult matchAndRewrite(MulOpType mulOp,
                                PatternRewriter &rewriter) const override {
    auto resType = toolchain::dyn_cast<VectorType>(mulOp.getResult().getType());
    if (!resType)
      return failure();
