...
static MaskFormat getMaskFormat(Value mask) {
  if (auto c = mask.getDefiningOp<arith::ConstantOp>()) {
    ...
    if (auto denseElts = toolchain::dyn_cast<DenseIntElementsAttr>(c.getValue())) {
      int64_t val = 0;
    ...
    bool allTrue = true;
    bool allFalse = true;
    for (auto [maskIdx, dimSize] : toolchain::zip_equal(masks, shape)) {
      if (maskIdx < dimSize)
        allTrue = false;
      if (maskIdx > 0)
        allFalse = false;
    ...
    for (Value operand : maskOperands) {
      if (auto constantOp = operand.getDefiningOp<arith::ConstantOp>()) {
        int64_t dimSize =
            toolchain::cast<IntegerAttr>(constantOp.getValue()).getInt();
        if (dimSize <= 0)
          return MaskFormat::AllFalse;
      }
    }
...
static bool isSupportedCombiningKind(CombiningKind combiningKind,
  ...
  case CombiningKind::MAXIMUMF:
    return toolchain::isa<FloatType>(elementType);
  }
  return false;
}
...
static unsigned getEffectiveVectorRankForXferOp(ShapedType shapedType,
  ...
  VectorType elementVectorType =
      toolchain::dyn_cast<VectorType>(shapedType.getElementType());
...
static LogicalResult incSlicePosition(MutableArrayRef<int64_t> position,
  ...
  for (auto [posInDim, dimSize, offsetInDim] :
       toolchain::reverse(toolchain::zip_equal(position, shape, offsets))) {
    ++posInDim;
...
SmallVector<int64_t> vector::getAsIntegers(ArrayRef<Value> values) {
  ...
  toolchain::transform(values, std::back_inserter(ints), [](Value value) {
...
SmallVector<Value> vector::getAsValues(OpBuilder &builder, Location loc,
  ...
  toolchain::transform(foldResults, std::back_inserter(values),
...
void vector::MultiDimReductionOp::build(OpBuilder &builder,
  ...
  for (const auto &en : toolchain::enumerate(reductionMask))
    if (en.value())
...
MultiDimReductionOp::getShapeForUnroll() {
  return toolchain::to_vector<4>(getSourceVectorType().getShape());
}
...
LogicalResult MultiDimReductionOp::verify() {
  ...
  for (auto [dimIdx, dimSize] :
       toolchain::enumerate(getSourceVectorType().getShape()))
    if (!toolchain::any_of(getReductionDims(),
...
void vector::ReductionOp::build(OpBuilder &builder, OperationState &result,
  ...
  build(builder, result,
        toolchain::cast<VectorType>(vector.getType()).getElementType(), kind, vector,
        acc, fastMathFlags);
}
...
void vector::ContractionOp::build(OpBuilder &builder, OperationState &result,
  ...
  result.addAttribute(
      ...
      builder.getArrayAttr(toolchain::to_vector(toolchain::map_range(
...
ParseResult ContractionOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  auto lhsType = toolchain::cast<VectorType>(types[0]);
  auto rhsType = toolchain::cast<VectorType>(types[1]);
...
void ContractionOp::print(OpAsmPrinter &p) {
  ...
  for (auto attr : (*this)->getAttrs()) {
    if (attr.getName() == getIteratorTypesAttrName()) {
      ...
      // TODO: Remove this conversion once tests are fixed.
      SmallVector<Attribute> iteratorTypeNames = toolchain::to_vector(
          toolchain::map_range(iteratorTypes, [&](IteratorType t) -> Attribute {
...
static LogicalResult verifyOutputShape(
  ...
  DenseSet<int64_t> rhsBatchDimSet(toolchain::from_range,
                                   toolchain::make_second_range(batchDimMap));
  ...
  if (expectedResultDims.empty()) {
    ...
    if (toolchain::isa<VectorType>(resType) || toolchain::isa<VectorType>(accType))
    ...
    auto resVectorType = toolchain::dyn_cast<VectorType>(resType);
    auto accVectorType = toolchain::dyn_cast<VectorType>(accType);
    ...
    if (!toolchain::all_of(extents, [](AffineExpr e) { return e; }))
    ...
    assert(toolchain::all_of(expectedMap.getResults(),
                        toolchain::IsaPred<AffineConstantExpr>) &&
           "expected constant extent along all dimensions.");
    // Extract the expected shape and build the type.
    auto expectedShape = toolchain::to_vector<4>(
        toolchain::map_range(expectedMap.getResults(), [](AffineExpr e) {
...
LogicalResult ContractionOp::verify() {
  ...
  if (toolchain::isa<IntegerType>(lhsType.getElementType())) {
  ...
  for (const auto &it : toolchain::enumerate(getIndexingMapsArray())) {
    auto index = it.index();
    auto map = it.value();
    if (map.getNumSymbols() != 0)
      return emitOpError("expected indexing map ")
             << index << " to have no symbols";
    auto vectorType = toolchain::dyn_cast<VectorType>(getOperand(index).getType());
  ...
  auto vectorType = toolchain::dyn_cast<VectorType>(resType);
...
Type ContractionOp::getExpectedMaskType() {
  ...
  for (auto [dimIdx, dimSize] : toolchain::enumerate(lhsType.getShape())) {
  ...
  for (auto [dimIdx, dimSize] : toolchain::enumerate(rhsType.getShape())) {
...
getDimMap(ArrayRef<AffineMap> indexingMaps, ArrayAttr iteratorTypes,
  ...
  for (const auto &it : toolchain::enumerate(iteratorTypes)) {
    auto iteratorType = toolchain::cast<IteratorTypeAttr>(it.value()).getValue();
...
void ContractionOp::getIterationBounds(
  ...
  auto resVectorType = toolchain::dyn_cast<VectorType>(getResultType());
  SmallVector<AffineMap, 4> indexingMaps(getIndexingMapsArray());
  for (const auto &it : toolchain::enumerate(getIteratorTypes())) {
    // Search lhs/rhs map results for 'targetExpr'.
...
ExtractOp::inferReturnTypes(MLIRContext *, std::optional<Location>,
  ...
  auto vectorType = toolchain::cast<VectorType>(adaptor.getSource().getType());
...
bool ExtractOp::isCompatibleReturnTypes(TypeRange l, TypeRange r) {
  ...
  auto isCompatible = [](TypeRange l, TypeRange r) {
    auto vectorType = toolchain::dyn_cast<VectorType>(l.front());
...
LogicalResult vector::ExtractOp::verify() {
  ...
  auto dynamicMarkersCount =
      toolchain::count_if(getStaticPosition(), ShapedType::isDynamic);
  ...
  for (auto [idx, pos] : toolchain::enumerate(position)) {
...
private:
  ...
  bool intersectsWhereNonNegative(const ContainerA &a, const ContainerB &b) {
    for (auto [elemA, elemB] : toolchain::zip(a, b)) {
      if (elemA < 0 || elemB < 0)
        continue;
      if (elemA != elemB)
        return false;
...
ExtractFromInsertTransposeChainState::ExtractFromInsertTransposeChainState(
  ...
  toolchain::append_range(extractPosition, sentinels);
}
...
ExtractFromInsertTransposeChainState::handleInsertOpWithMatchingPos(
  ...
  if (insertedPos != toolchain::ArrayRef(extractPosition).take_front(extractedRank))
    return failure();
...
static bool hasZeroDimVectors(Operation *op) {
  ...
  return toolchain::any_of(op->getOperandTypes(), hasZeroDimVectorType) ||
         toolchain::any_of(op->getResultTypes(), hasZeroDimVectorType);
}
...
static Value foldExtractFromBroadcast(ExtractOp extractOp) {
  ...
  auto inputType = toolchain::dyn_cast<VectorType>(input.getType());
  auto extractType = toolchain::dyn_cast<VectorType>(extractOp.getType());
  ...
  for (auto [i, size] : toolchain::enumerate(inputShape.take_front(deltaOverall))) {
  ...
  extractOp->setOperands(
      toolchain::to_vector(toolchain::concat<Value>(ValueRange(input), dynPos)));
...
static Value foldExtractFromShapeCast(ExtractOp extractOp) {
  ...
  int64_t destinationRank =
      toolchain::isa<VectorType>(extractOp.getType())
          ? toolchain::cast<VectorType>(extractOp.getType()).getRank()
          : 0;
  if (destinationRank > shapeCastOp.getSourceVectorType().getRank())
    return Value();
  if (destinationRank > 0) {
    auto destinationType =
        toolchain::cast<VectorType>(extractOp.getResult().getType());
...
static Value foldExtractFromExtractStrided(ExtractOp extractOp) {
  ...
  if (auto vecType = toolchain::dyn_cast<VectorType>(extractOp.getType()))
...
static Value foldExtractStridedOpFromInsertChain(ExtractOp extractOp) {
  ...
  while (insertOp) {
    ...
    if (toolchain::any_of(insertOp.getStrides(), [](Attribute attr) {
          return toolchain::cast<IntegerAttr>(attr).getInt() != 1;
        }))
      return Value();
    bool disjoint = false;
...
static Value foldScalarExtractFromFromElements(ExtractOp extractOp) {
  ...
  auto vecType = toolchain::cast<VectorType>(fromElementsOp.getType());
...
public:
  ...
  LogicalResult matchAndRewrite(ExtractOp extractOp,
    ...
    VectorType extractedMaskType =
        toolchain::dyn_cast<VectorType>(extractOp.getResult().getType());
...
LogicalResult foldExtractFromFromElements(ExtractOp extractOp,
  ...
  SmallVector<int64_t> firstElementPos =
      toolchain::to_vector(extractOp.getStaticPosition());
...
static void populateFromInt64AttrArray(ArrayAttr arrayAttr,
  ...
  for (auto attr : arrayAttr)
    results.push_back(toolchain::cast<IntegerAttr>(attr).getInt());
}
...
std::optional<SmallVector<int64_t, 4>> FMAOp::getShapeForUnroll() {
  return toolchain::to_vector<4>(getVectorType().getShape());
}
...
static bool haveSameDefiningOp(OperandRange operands, Operation *defOp) {
  ...
  return toolchain::all_of(operands, [&](Value operand) {
...
foldToElementsFromElements(ToElementsOp toElementsOp,
  ...
  toolchain::append_range(results, fromElementsOp.getElements());
  return success();
}
...
struct ToElementsOfBroadcast final : OpRewritePattern<ToElementsOp> {
  ...
  LogicalResult matchAndRewrite(ToElementsOp toElementsOp,
    ...
    int64_t dstCount = toolchain::product_of(dstShape);
...
static OpFoldResult foldFromElementsToElements(FromElementsOp fromElementsOp) {
  ...
  if (fromElementsOp.getType() == toElementsInput.getType() &&
      toolchain::equal(fromElemsOperands, toElementsOp.getResults())) {
...
static OpFoldResult foldFromElementsToConstant(FromElementsOp fromElementsOp,
  ...
  if (toolchain::any_of(elements, [](Attribute attr) {
  ...
  auto convertedElements = toolchain::map_to_vector(elements, [&](Attribute attr) {
...
rewriteFromElementsAsBroadcast(FromElementsOp fromElementsOp,
                               PatternRewriter &rewriter) {
  if (!toolchain::all_equal(fromElementsOp.getElements()))
    return failure();
...
class FromElementsToShapeCast : public OpRewritePattern<FromElementsOp> {
  ...
  LogicalResult matchAndRewrite(FromElementsOp fromElements,
    ...
    for (auto [insertIndex, element] :
         toolchain::enumerate(fromElements.getElements())) {
      ...
      if (insertIndex == 0) {
        ...
        expectedPosition = toolchain::to_vector(position);
  ...
  static void increment(MutableArrayRef<int64_t> indices,
                        ArrayRef<int64_t> shape) {
    for (int dim : toolchain::reverse(toolchain::seq<int>(0, indices.size()))) {
      indices[dim] += 1;
...
std::optional<SmallVector<int64_t, 4>> BroadcastOp::getShapeForUnroll() {
  return toolchain::to_vector<4>(getResultVectorType().getShape());
}

/// Return the dimensions of the result vector that were formerly ones in the
/// source tensor and thus correspond to "dim-1" broadcasting.
static toolchain::SetVector<int64_t>
computeBroadcastedUnitDims(ArrayRef<int64_t> srcShape,
  ...
  toolchain::SetVector<int64_t> res;
  for (auto [s1, s2] :
       toolchain::zip_equal(srcShape, dstShape.drop_front(rankDiff))) {
    if (s1 != s2) {
...
toolchain::SetVector<int64_t> BroadcastOp::computeBroadcastedUnitDims() {
  // Scalar broadcast is without any unit dim broadcast.
  auto srcVectorType = toolchain::dyn_cast<VectorType>(getSourceType());
  if (!srcVectorType)
    return {};
...
Value BroadcastOp::createOrFoldBroadcastOp(
    ...
    const toolchain::SetVector<int64_t> &broadcastedDims) {
  ...
  VectorType srcVectorType = toolchain::dyn_cast<VectorType>(value.getType());
  ...
  toolchain::append_range(broadcastShape, srcVectorType.getShape());
...
BroadcastableToResult mlir::vector::isBroadcastableTo(
  ...
  VectorType srcVectorType = toolchain::dyn_cast<VectorType>(srcType);
  if (!srcVectorType)
...
static LogicalResult foldBroadcastOfShapeCast(BroadcastOp broadcastOp) {
  ...
  if (!toolchain::equal(srcShape.take_back(numTrailingDims),
...
OpFoldResult BroadcastOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto attr = toolchain::dyn_cast<IntegerAttr>(adaptor.getSource())) {
    if (vectorType.getElementType() != attr.getType())
      return {};
    return DenseElementsAttr::get(vectorType, attr);
  }
  if (auto attr = toolchain::dyn_cast<FloatAttr>(adaptor.getSource())) {
    if (vectorType.getElementType() != attr.getType())
      return {};
    return DenseElementsAttr::get(vectorType, attr);
  }
  if (auto attr = toolchain::dyn_cast<SplatElementsAttr>(adaptor.getSource()))
    return DenseElementsAttr::get(vectorType, attr.getSplatValue<Attribute>());
  if (toolchain::dyn_cast<ub::PoisonAttr>(adaptor.getSource()))
...
LogicalResult ShuffleOp::verify() {
  ...
  for (auto [idx, maskPos] : toolchain::enumerate(mask)) {
...
ShuffleOp::inferReturnTypes(MLIRContext *, std::optional<Location>,
                            ...
                            SmallVectorImpl<Type> &inferredReturnTypes) {
  auto v1Type = toolchain::cast<VectorType>(adaptor.getV1().getType());
  ...
  if (v1Rank > 0)
    toolchain::append_range(shape, v1Type.getShape().drop_front());
...
static bool isStepIndexArray(ArrayRef<T> idxArr, uint64_t begin, size_t width) {
  T expected = begin;
  return idxArr.size() == width && toolchain::all_of(idxArr, [&expected](T value) {
...
void vector::InsertOp::build(OpBuilder &builder, OperationState &result,
  ...
  toolchain::transform(position, std::back_inserter(posVals),
...
LogicalResult InsertOp::verify() {
  ...
  auto srcVectorType = toolchain::dyn_cast<VectorType>(getValueToStoreType());
  if (srcVectorType &&
...
static int64_t calculateInsertPosition(VectorType destTy,
  ...
  toolchain::SmallVector<int64_t> completePositions(destTy.getRank(), 0);
...
public:
  ...
  LogicalResult matchAndRewrite(InsertOp insertOp,
                                PatternRewriter &rewriter) const override {
    auto srcVecType =
        toolchain::dyn_cast<VectorType>(insertOp.getValueToStoreType());
...
public:
  ...
  LogicalResult matchAndRewrite(InsertOp op,
    ...
    for (auto insertOp : chainInsertOps) {
      ...
      for (auto index : toolchain::seq<int64_t>(insertBeginPosition,
    ...
    for (auto [insertBeginPosition, insertSize, valueToStore] :
         toolchain::reverse(toolchain::zip(pendingInsertPos, pendingInsertSize,
                                 pendingInsertValues))) {
      auto srcVectorType = toolchain::dyn_cast<VectorType>(valueToStore.getType());
...
foldDenseElementsAttrDestInsertOp(InsertOp insertOp, Attribute srcAttr,
  ...
  auto denseDst = toolchain::dyn_cast_if_present<DenseElementsAttr>(dstAttr);
  if (!denseDst)
    return {};
  ...
  /// a mismatch.
  if (auto denseSource = toolchain::dyn_cast<DenseElementsAttr>(srcAttr)) {
  ...
  auto allValues = toolchain::to_vector(denseDst.getValues<Attribute>());
...
isIntegerArrayAttrConfinedToRange(OpType op, ArrayAttr arrayAttr, int64_t min,
                                  ...
                                  bool halfOpen = true) {
  for (auto attr : arrayAttr) {
    auto val = toolchain::cast<IntegerAttr>(attr).getInt();
    auto upper = max;
    if (!halfOpen)
      upper += 1;
...
isIntegerArrayAttrConfinedToShape(OpType op, ArrayAttr arrayAttr,
  ...
  for (auto [index, attrDimPair] :
       toolchain::enumerate(toolchain::zip_first(arrayAttr, shape))) {
    int64_t val = toolchain::cast<IntegerAttr>(std::get<0>(attrDimPair)).getInt();
...
static LogicalResult isSumOfIntegerArrayAttrConfinedToShape(
  ...
  for (auto [index, it] :
       toolchain::enumerate(toolchain::zip(arrayAttr1, arrayAttr2, shape))) {
    auto val1 = toolchain::cast<IntegerAttr>(std::get<0>(it)).getInt();
    auto val2 = toolchain::cast<IntegerAttr>(std::get<1>(it)).getInt();
...
static ArrayAttr makeI64ArrayAttr(ArrayRef<int64_t> values,
                                  MLIRContext *context) {
  auto attrs = toolchain::map_range(values, [context](int64_t v) -> Attribute {
    return IntegerAttr::get(IntegerType::get(context, 64), APInt(64, v));
  });
  return ArrayAttr::get(context, toolchain::to_vector<8>(attrs));
}
...
public:
  ...
  LogicalResult matchAndRewrite(InsertStridedSliceOp op,
    ...
    auto denseDest = toolchain::cast<DenseElementsAttr>(vectorDestCst);
    auto denseSlice = toolchain::cast<DenseElementsAttr>(sourceCst);
    auto sliceValuesIt = denseSlice.value_begin<Attribute>();
    auto newValues = toolchain::to_vector(denseDest.getValues<Attribute>());
...
ParseResult OuterProductOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  VectorType vLHS = toolchain::dyn_cast<VectorType>(tLHS);
  VectorType vRHS = toolchain::dyn_cast<VectorType>(tRHS);
  if (!vLHS)
    return parser.emitError(parser.getNameLoc(),
...
LogicalResult OuterProductOp::verify() {
  ...
  VectorType vLHS = getOperandVectorTypeLHS(),
             vRHS = toolchain::dyn_cast<VectorType>(tRHS),
...
static Type inferStridedSliceOpResultType(VectorType vectorType,
  ...
  for (unsigned e = offsets.size(); idx < e; ++idx)
    shape.push_back(toolchain::cast<IntegerAttr>(sizes[idx]).getInt());
...
void ExtractStridedSliceOp::build(OpBuilder &builder, OperationState &result,
  ...
  auto stridesAttr = getVectorSubscriptAttr(builder, strides);
  result.addTypes(
      inferStridedSliceOpResultType(toolchain::cast<VectorType>(source.getType()),
...
LogicalResult ExtractStridedSliceOp::verify() {
  ...
  for (unsigned idx = 0; idx < sizes.size(); ++idx) {
    if (type.getScalableDims()[idx]) {
      ...
      auto inputSize = toolchain::cast<IntegerAttr>(sizes[idx]).getInt();
...
foldExtractStridedOpFromInsertChain(ExtractStridedSliceOp op) {
  ...
  auto getElement = [](ArrayAttr array, int idx) {
    return toolchain::cast<IntegerAttr>(array[idx]).getInt();
  };
...
foldExtractStridedSliceNonSplatConstant(ExtractStridedSliceOp op,
  ...
  auto dense = toolchain::dyn_cast_if_present<DenseElementsAttr>(foldInput);
  if (!dense)
    return {};
...
OpFoldResult ExtractStridedSliceOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto splat =
          toolchain::dyn_cast_if_present<SplatElementsAttr>(adaptor.getSource()))
...
public:
  LogicalResult matchAndRewrite(ExtractStridedSliceOp extractStridedSliceOp,
    ...
    // sliceOffsets.size() <= maskDimSizes.size(), so we use toolchain::zip and
    // only iterate on the leading dim sizes. The tail accounts for the
    // remaining dim sizes.
    for (auto [maskDimSize, sliceOffset, sliceSize] :
         toolchain::zip(maskDimSizes, sliceOffsets, sliceSizes)) {
    ...
    toolchain::append_range(
        sliceMaskDimSizes,
        toolchain::drop_begin(maskDimSizes, sliceMaskDimSizes.size()));
...
public:
  ...
  LogicalResult matchAndRewrite(ExtractStridedSliceOp extractStridedSliceOp,
    ...
    if (toolchain::is_contained(sliceMaskDimSizes, 0))
...
public:
  ...
  LogicalResult matchAndRewrite(ExtractStridedSliceOp op,
    ...
    if (!broadcast)
      return failure();
    auto srcVecType =
        toolchain::dyn_cast<VectorType>(broadcast.getSource().getType());
    unsigned srcRank = srcVecType ? srcVecType.getRank() : 0;
    auto dstVecType = toolchain::cast<VectorType>(op.getType());
...
void TransferReadOp::build(OpBuilder &builder, OperationState &result,
  ...
  Type elemType = toolchain::cast<ShapedType>(source.getType()).getElementType();
  if (!padding)
...
void TransferReadOp::build(OpBuilder &builder, OperationState &result,
  ...
  AffineMap permutationMap = getTransferMinorIdentityMap(
      toolchain::cast<ShapedType>(source.getType()), vectorType);
  auto permutationMapAttr = AffineMapAttr::get(permutationMap);
  auto inBoundsAttr = (inBounds && !inBounds.value().empty())
                          ? builder.getBoolArrayAttr(inBounds.value())
                          : builder.getBoolArrayAttr(
...
verifyTransferOp(VectorTransferOpInterface op, ShapedType shapedType,
  ...
  if (!toolchain::isa<MemRefType, RankedTensorType>(shapedType))
    return op->emitOpError(
        "requires source to be a memref or ranked tensor type");

  auto elementType = shapedType.getElementType();
  DataLayout dataLayout = DataLayout::closest(op);
  if (auto vectorElementType = toolchain::dyn_cast<VectorType>(elementType)) {
...
static void printTransferAttrs(OpAsmPrinter &p, VectorTransferOpInterface op) {
  ...
  if (toolchain::none_of(op.getInBoundsValues(), [](bool b) { return b; }))
...
ParseResult TransferReadOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  auto shapedType = toolchain::dyn_cast<ShapedType>(types[0]);
  if (!shapedType || !toolchain::isa<MemRefType, RankedTensorType>(shapedType))
    return parser.emitError(typesLoc, "requires memref or ranked tensor type");
  VectorType vectorType = toolchain::dyn_cast<VectorType>(types[1]);
  if (!vectorType)
  ...
  if (!permMapAttr) {
    ...
    permMap = toolchain::cast<AffineMapAttr>(permMapAttr).getValue();
  }
  ...
  if (parser.resolveOperand(sourceInfo, shapedType, result.operands) ||
    ...
    return failure();
  if (hasMask.succeeded()) {
    if (toolchain::dyn_cast<VectorType>(shapedType.getElementType()))
...
LogicalResult TransferReadOp::verify() {
  ...
  if (auto sourceVectorElementType =
          toolchain::dyn_cast<VectorType>(sourceElementType)) {
...
static LogicalResult foldTransferInBoundsAttribute(TransferOp op) {
  ...
  // "in bounds" as well.
  bool allNonBcastDimsInBounds = toolchain::all_of(
...
static Value foldRAW(TransferReadOp readOp) {
  if (!toolchain::isa<RankedTensorType>(readOp.getShapedType()))
    return {};
...
void TransferReadOp::getEffects(
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>
        &effects) {
  if (toolchain::isa<MemRefType>(getShapedType()))
...
static AffineMap inverseWithUnusedDims(AffineMap map) {
  ...
  for (auto [idx, result] : toolchain::enumerate(map.getResults())) {
...
struct TransferReadAfterWriteToBroadcast
  ...
  LogicalResult matchAndRewrite(TransferReadOp readOp,
    ...
    auto invPerm = toolchain::to_vector_of<int64_t>(broadcastedDims);
    invPerm.resize(composedMap.getNumResults());
    for (auto [idx, expr] : toolchain::enumerate(composedMap.getResults())) {
...
void TransferWriteOp::build(OpBuilder &builder, OperationState &result,
                            ...
                            /*optional*/ Value mask,
                            /*optional*/ ArrayAttr inBoundsAttr) {
  Type resultType = toolchain::dyn_cast<RankedTensorType>(dest.getType());
  ...
  auto inBoundsAttr =
      (inBounds && !inBounds.value().empty())
          ...
          : builder.getBoolArrayAttr(SmallVector<bool>(
                toolchain::cast<VectorType>(vector.getType()).getRank(), false));
  build(builder, result, vector, dest, indices, permutationMapAttr,
        /*mask=*/Value(), inBoundsAttr);
}
...
void TransferWriteOp::build(OpBuilder &builder, OperationState &result,
                            Value vector, Value dest, ValueRange indices,
                            std::optional<ArrayRef<bool>> inBounds) {
  auto vectorType = toolchain::cast<VectorType>(vector.getType());
  AffineMap permutationMap = getTransferMinorIdentityMap(
      toolchain::cast<ShapedType>(dest.getType()), vectorType);
...
ParseResult TransferWriteOp::parse(OpAsmParser &parser,
  ...
  if (types.size() != 2)
    return parser.emitError(typesLoc, "requires two types");
  auto indexType = builder.getIndexType();
  VectorType vectorType = toolchain::dyn_cast<VectorType>(types[0]);
  if (!vectorType)
    return parser.emitError(typesLoc, "requires vector type");
  ShapedType shapedType = toolchain::dyn_cast<ShapedType>(types[1]);
  if (!shapedType || !toolchain::isa<MemRefType, RankedTensorType>(shapedType))
    return parser.emitError(typesLoc, "requires memref or ranked tensor type");
  ...
  result.addAttribute(TransferWriteOp::getOperandSegmentSizeAttr(),
                      builder.getDenseI32ArrayAttr(
                          {1, 1, static_cast<int32_t>(indexInfo.size()),
                           static_cast<int32_t>(hasMask.succeeded())}));
  return failure(toolchain::isa<RankedTensorType>(shapedType) &&
...
LogicalResult TransferWriteOp::verify() {
  ...
  if (toolchain::size(getIndices()) != shapedType.getRank())
    return emitOpError("requires ") << shapedType.getRank() << " indices";
...
static LogicalResult foldReadInitWrite(TransferWriteOp write,
  ...
  auto rankedTensorType =
      toolchain::dyn_cast<RankedTensorType>(write.getBase().getType());
  ...
  if (toolchain::any_of(read.getIndices(), isNotConstantZero) ||
      toolchain::any_of(write.getIndices(), isNotConstantZero))
    return failure();
  // Success.
...
static LogicalResult foldWAR(TransferWriteOp write,
                             SmallVectorImpl<OpFoldResult> &results) {
  if (!toolchain::isa<RankedTensorType>(write.getBase().getType()))
    return failure();
  auto read = write.getVector().getDefiningOp<vector::TransferReadOp>();
  if (!read)
    return failure();
...
public:
  ...
  LogicalResult matchAndRewrite(TransferWriteOp writeOp,
                                PatternRewriter &rewriter) const override {
    if (!toolchain::isa<RankedTensorType>(writeOp.getShapedType()))
      return failure();
...
public:
  ...
  LogicalResult matchAndRewrite(tensor::InsertSliceOp insertOp,
    ...
    if (!toolchain::all_of(transferOp.getIndices(), [](Value value) {
    ...
    for (auto [insertSize, extractSize] :
         toolchain::zip_equal(insertOp.getMixedSizes(), extractOp.getMixedSizes())) {
...
LogicalResult vector::LoadOp::verify() {
  ...
  if (auto memVecTy = toolchain::dyn_cast<VectorType>(memElemTy)) {
  ...
  if (toolchain::size(getIndices()) != memRefTy.getRank())
...
LogicalResult MaskedLoadOp::verify() {
  ...
  if (toolchain::size(getIndices()) != memType.getRank())
...
static LogicalResult isZeroBasedContiguousSeq(Value indexVec) {
  ...
  return success(
      toolchain::equal(elements, toolchain::seq<int64_t>(0, vecType.getNumElements())));
}
...
OpFoldResult ShapeCastOp::fold(FoldAdaptor adaptor) {
  ...
  if (toolchain::dyn_cast_if_present<ub::PoisonAttr>(adaptor.getSource()))
    return ub::PoisonAttr::get(getContext());
...
OpFoldResult BitCastOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto floatPack = toolchain::dyn_cast<DenseFPElementsAttr>(sourceConstant)) {
    if (floatPack.isSplat()) {
      ...
      if (srcElemType.isF16() && dstElemType.isF32()) {
        ...
        APFloat floatBits(toolchain::APFloat::IEEEsingle(), intBits);
        return DenseElementsAttr::get(getResultVectorType(), floatBits);
      }
    }
  }

  if (auto intPack = toolchain::dyn_cast<DenseIntElementsAttr>(sourceConstant)) {
    if (intPack.isSplat()) {
      auto splat = intPack.getSplatValue<IntegerAttr>();

      if (toolchain::isa<IntegerType>(dstElemType)) {
...
static SmallVector<int64_t, 8> extractShape(MemRefType memRefType) {
  auto vectorType = toolchain::dyn_cast<VectorType>(memRefType.getElementType());
...
void TypeCastOp::build(OpBuilder &builder, OperationState &result,
                       Value source) {
  result.addOperands(source);
  MemRefType memRefType = toolchain::cast<MemRefType>(source.getType());
  VectorType vectorType =
...
void vector::TransposeOp::build(OpBuilder &builder, OperationState &result,
  ...
  VectorType vt = toolchain::cast<VectorType>(vector.getType());
...
OpFoldResult vector::TransposeOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto splat =
          toolchain::dyn_cast_if_present<SplatElementsAttr>(adaptor.getVector()))
    return splat.reshape(getResultVectorType());

  // Eliminate poison transpose ops.
  if (toolchain::dyn_cast_if_present<ub::PoisonAttr>(adaptor.getVector()))
    return ub::PoisonAttr::get(getContext());
...
LogicalResult vector::TransposeOp::verify() {
  ...
  for (const auto &ta : toolchain::enumerate(perm)) {
...
LogicalResult ConstantMaskOp::verify() {
  auto resultType = toolchain::cast<VectorType>(getResult().getType());
  ...
  for (const auto [index, maskDimSize] : toolchain::enumerate(maskDimSizes)) {
  ...
  bool anyZeros = toolchain::is_contained(maskDimSizes, 0);
  bool allZeros = toolchain::all_of(maskDimSizes, [](int64_t s) { return s == 0; });
...
bool ConstantMaskOp::isAllOnesMask() {
  ...
  for (const auto [resultSize, maskDimSize] :
       toolchain::zip_equal(resultType.getShape(), getMaskDimSizes())) {
...
OpFoldResult ConstantMaskOp::fold(FoldAdaptor adaptor) {
  ...
  if (toolchain::all_of(bounds, [](int64_t x) { return x == 0; }))
...
LogicalResult CreateMaskOp::verify() {
  auto vectorType = toolchain::cast<VectorType>(getResult().getType());
    ...
    if (getNumOperands() != 1)
      return emitOpError(
          "must specify exactly one operand for 0-D create_mask");
             ...
             toolchain::cast<VectorType>(getResult().getType()).getRank()) {
    return emitOpError(
...
public:
  ...
  LogicalResult matchAndRewrite(CreateMaskOp createMaskOp,
    ...
    for (auto [value, maskDimSize] : toolchain::zip(constantDims, maskTypeDimSizes))
      value = std::clamp<int64_t>(value, 0, maskDimSize);

    // If one of dim sizes is zero, set all dims to zero.
    if (toolchain::is_contained(constantDims, 0))
...
LogicalResult MaskOp::verify() {
  ...
  if (!toolchain::equal(maskableOp->getResults(), terminator.getOperands()))
  ...
  if (!toolchain::equal(maskableOp->getResultTypes(), getResultTypes()))
    return emitOpError(
        "expects result type to match maskable operation result type");

  if (toolchain::count_if(maskableOp->getResultTypes(),
                     [](Type t) { return toolchain::isa<VectorType>(t); }) > 1)
...
static LogicalResult foldEmptyMaskOp(MaskOp maskOp, MaskOp::FoldAdaptor adaptor,
  ...
  toolchain::append_range(results, terminator.getOperands());
  return success();
}
...
LogicalResult MaskOp::fold(FoldAdaptor adaptor,
  ...
  toolchain::append_range(results, maskableOp->getResults());
  return success();
}
...
LogicalResult ScanOp::verify() {
  ...
  if (!toolchain::equal(initialValueShapes, expectedShape)) {
...
Value mlir::vector::makeArithReduction(OpBuilder &b, Location loc,
  ...
  case CombiningKind::ADD:
    ...
    else if (toolchain::isa<FloatType>(t1) && toolchain::isa<FloatType>(tAcc))
  ...
  case CombiningKind::AND:
    ...
    break;
  case CombiningKind::MAXNUMF:
    assert(toolchain::isa<FloatType>(t1) && toolchain::isa<FloatType>(tAcc) &&
