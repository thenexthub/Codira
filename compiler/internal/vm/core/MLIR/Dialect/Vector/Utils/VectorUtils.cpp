...
mlir::vector::isTranspose2DSlice(vector::TransposeOp op) {
  ...
  for (auto [index, size] : toolchain::enumerate(srcType.getShape()))
    if (size > 1)
...
bool vector::isContiguousSlice(MemRefType memrefType, VectorType vectorType) {
  ...
  return toolchain::equal(vectorShape.drop_front(), memrefShape.drop_front());
}
...
vector::createUnrollIterator(VectorType vType, int64_t targetRank) {
  ...
  const auto *it = toolchain::find(inputScalableVecDimsToUnroll, true);
  ...
  assert(!toolchain::is_contained(inputScalableVecDimsToUnroll, true) &&
...
Value vector::createReadOrMaskedRead(OpBuilder &builder, Location loc,
                                     ...
                                     std::optional<Value> padValue,
                                     bool useInBoundsInsteadOfMasking) {
  assert(!toolchain::is_contained(vecToReadTy.getScalableDims(),
                             ShapedType::kDynamic) &&
  ...
  if (toolchain::equal(vecToReadTy.getShape(), sourceShape) ||
      useInBoundsInsteadOfMasking)
...
vector::isValidMaskedInputVector(ArrayRef<int64_t> shape,
                                 ArrayRef<int64_t> inputVectorSizes) {
  LDBG() << "Iteration space static sizes:" << toolchain::interleaved(shape);
  ...
  if (!toolchain::all_of(toolchain::zip(shape, inputVectorSizes),
