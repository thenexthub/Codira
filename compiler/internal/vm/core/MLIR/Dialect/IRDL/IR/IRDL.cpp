...
static toolchain::LogicalResult isValidName(toolchain::StringRef in, mlir::Operation *loc,
                                       const Twine &label) {
  if (in.empty())
...
LogicalResult OperationOp::verifyRegions() {
  ...
  // the operation.
  SmallVector<std::tuple<StringRef, toolchain::SmallDenseSet<StringRef>>> valueNames;

  auto insertNames = [&](StringRef kind, ArrayAttr names) {
    toolchain::SmallDenseSet<StringRef> nameSet;
    nameSet.reserve(names.size());
    for (auto name : names)
      nameSet.insert(toolchain::cast<StringAttr>(name).getValue());
  ...
  for (size_t i : toolchain::seq(valueNames.size())) {
    for (size_t j : toolchain::seq(i + 1, valueNames.size())) {
      auto [lhs, lhsSet] = valueNames[i];
      auto &[rhs, rhsSet] = valueNames[j];
      toolchain::set_intersect(lhsSet, rhsSet);
      if (!lhsSet.empty())
...
static LogicalResult verifyNames(Operation *op, StringRef kindName,
  ...
  for (auto [i, name] : toolchain::enumerate(names)) {
    StringRef nameRef = toolchain::cast<StringAttr>(name).getValue();
...
static void printNamedValueListImpl(OpAsmPrinter &p, Operation *op,
  ...
  p << "(";
  interleaveComma(toolchain::seq<int>(0, operands.size()), p, [&](int i) {
    p << toolchain::cast<StringAttr>(valueNamesAttr[i]).getValue() << ": ";
    if (variadicityAttr) {
...
static void printAttributesOp(OpAsmPrinter &p, AttributesOp op,
  ...
  if (attrNames.empty())
    return;
  p << "{";
  interleaveComma(toolchain::seq<int>(0, attrNames.size()), p,
