...
static LogicalResult irdlRegionVerifier(
  ...
  for (auto [constraint, region] :
       toolchain::zip(regionsConstraints, op->getRegions()))
...
toolchain::unique_function<LogicalResult(Operation *) const>
mlir::irdl::createVerifier(
  ...
  if (attributesOp.has_value()) {
    ...
    for (const auto &[name, value] : toolchain::zip(names, values)) {
      for (auto [i, constr] : enumerate(constrToValue)) {
        if (constr == value) {
