...
Value mlir::x86vector::avx2::inline_asm::mm256BlendPsAsm(
  ...
  auto asmStr = toolchain::formatv(asmTp, toolchain::format_hex(mask, /*width=*/2)).str();
...
Value mlir::x86vector::avx2::intrin::mm256Permute2f128Ps(
  ...
  auto appendToMask = [&](uint8_t control) {
    if (control == 0)
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{0, 1, 2, 3});
    else if (control == 1)
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{4, 5, 6, 7});
    else if (control == 2)
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{8, 9, 10, 11});
    else if (control == 3)
      toolchain::append_range(shuffleMask, ArrayRef<int64_t>{12, 13, 14, 15});
    else
...
void mlir::x86vector::avx2::transpose4x8xf32(ImplicitLocOpBuilder &ib,
  ...
  assert(toolchain::all_of(ValueRange{vs}.getTypes(),
                      [&](Type t) { return t == vt; }) &&
