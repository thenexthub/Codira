...
static bool validateVectorContractOperands(Value prodOp) {
  ...
  toolchain::TypeSwitch<Operation *>(defOp).Case<TransferReadOp, LoadOp>(
      [&](auto readOp) {
  ...
  if (!toolchain::isa<MemRefType>(srcType))
    return false;
  ...
  if (!toolchain::cast<mlir::MemRefType>(srcType).areTrailingDimsContiguous(2))
    return false;
...
getSubviewFromVectorInput(Location loc, PatternRewriter &rewriter, Value prodOp,
  ...
  if (!isUnitDim) {
    for (auto it : toolchain::enumerate(nonUnitDimShape)) {
      if (it.value() != 1) {
        mnDimSize = it.value();
        mnDimIdx = it.index();
        break;
  ...
  toolchain::SmallVector<memref::SubViewOp> subviews;
...
struct VectorContractBF16ToFMA
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp contractOp,
    ...
    toolchain::SmallVector<int64_t> nonUnitDimLhs;
    toolchain::copy_if(lhsShape, std::back_inserter(nonUnitDimLhs),
                  [](int64_t dim) { return dim != 1; });

    VectorType rhsTy = contractOp.getRhsType();
    ArrayRef<int64_t> rhsShape = rhsTy.getShape();
    toolchain::SmallVector<int64_t> nonUnitDimRhs;
    toolchain::copy_if(rhsShape, std::back_inserter(nonUnitDimRhs),
                  [](int64_t dim) { return dim != 1; });
    ...
    toolchain::SmallVector<int64_t> nonUnitDimAcc;
    toolchain::copy_if(accShape, std::back_inserter(nonUnitDimAcc),
                  [](int64_t dim) { return dim != 1; });
