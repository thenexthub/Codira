...
bool OpTrait::util::getBroadcastedShape(ArrayRef<int64_t> shape1,
  ...
  if (shape1.size() > shape2.size()) {
    toolchain::append_range(resultShape, shape1);
  } else {
    toolchain::append_range(resultShape, shape2);
  }
...
static std::tuple<bool, bool> hasTensorOrVectorType(iterator_range types) {
  return {toolchain::any_of(types, toolchain::IsaPred<TensorType>),
          toolchain::any_of(types, toolchain::IsaPred<VectorType>)};
}
...
static bool isCompatibleInferredReturnShape(ArrayRef<int64_t> inferred,
  ...
  if (inferred.size() != existing.size())
    return false;
  for (auto [inferredDim, existingDim] : toolchain::zip_equal(inferred, existing))
...
static std::string getShapeString(ArrayRef<int64_t> shape) {
  ...
  // when in type.
  std::string ret;
  toolchain::raw_string_ostream ss(ret);
  ss << '\'';
...
LogicalResult OpTrait::impl::verifyCompatibleOperandBroadcast(Operation *op) {
  ...
  auto rankedOperands =
      make_filter_range(op->getOperandTypes(), toolchain::IsaPred<RankedTensorType>);
  ...
  auto rankedResults =
      make_filter_range(op->getResultTypes(), toolchain::IsaPred<RankedTensorType>);
