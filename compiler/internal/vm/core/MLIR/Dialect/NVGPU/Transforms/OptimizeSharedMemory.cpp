...
static Value permuteVectorOffset(OpBuilder &b, Location loc,
  ...
  // clang-format on
  int64_t n =
      toolchain::Log2_64(kDefaultVectorSizeBits / memrefTy.getElementTypeBitWidth());
  int64_t m = toolchain::Log2_64(memrefTy.getDimSize(tgtDim));

  // Capture bits[0:(M-N)] of src by first creating a (M-N) mask.
  int64_t mask = (1LL << (m - n)) - 1;
  if (permuteEveryN > 1)
    mask = mask << toolchain::Log2_64(permuteEveryN);
  ...
  // vector offset.
  if (permuteEveryN > 1) {
    int64_t shlBits = n - toolchain::Log2_64(permuteEveryN);
    if (shlBits > 0) {
...
getShmReadAndWriteOps(Operation *parentOp, Value shmMemRef,
  ...
  if (toolchain::any_of(readOps, [](Operation *op) {
        return !isa<memref::LoadOp, vector::LoadOp, nvgpu::LdMatrixOp>(op) ||
               getIndices(op).size() < 2;
      }))
    return failure();
  if (toolchain::any_of(writeOps, [](Operation *op) {
...
mlir::nvgpu::optimizeSharedMemoryReadsAndWrites(Operation *parentOp,
  ...
  const int64_t threadGroupSize =
      1LL << (7 - toolchain::Log2_64(kDefaultVectorSizeBits / 8));
