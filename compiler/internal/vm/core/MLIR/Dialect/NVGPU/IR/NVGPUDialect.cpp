...
bool NVGPUDialect::isSharedMemoryAddressSpace(Attribute memorySpace) {
  if (!memorySpace)
    return false;
  if (auto intAttr = toolchain::dyn_cast<IntegerAttr>(memorySpace))
...
LogicalResult DeviceAsyncCopyOp::verify() {
  auto srcMemref = toolchain::cast<MemRefType>(getSrc().getType());
  auto dstMemref = toolchain::cast<MemRefType>(getDst().getType());
...
LogicalResult LdMatrixOp::verify() {
  ...
  auto srcMemref = toolchain::cast<MemRefType>(getSrcMemref().getType());

  // ldmatrix writes data to result/destination in vector registers
  auto resVector = toolchain::cast<VectorType>(getRes().getType());
