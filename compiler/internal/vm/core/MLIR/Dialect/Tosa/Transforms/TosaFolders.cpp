...
DenseElementsAttr transposeType(const RangeType &data, ShapedType inputType,
                                ...
                                toolchain::ArrayRef<int64_t> permValues) {
  ...
  if (inputType.getNumElements() == 0)
    return DenseElementsAttr::get(outputType, toolchain::ArrayRef<ElementType>{});
  ...
  for (const auto &it : toolchain::enumerate(data)) {
  ...
  return DenseElementsAttr::get(outputType,
                                toolchain::ArrayRef<ElementType>(outputValues));
}
...
struct TosaFoldConstantTranspose : public OpRewritePattern<tosa::TransposeOp> {
  ...
  LogicalResult matchAndRewrite(tosa::TransposeOp op,
    ...
    if (!toolchain::hasSingleElement(op.getInput1().getDefiningOp()->getUsers()))
      return failure();

    auto permValues = toolchain::map_to_vector(
...
getPositionFromIndex(int64_t index, toolchain::ArrayRef<int64_t> tensorShape) {
  int64_t remaining = index;
  toolchain::SmallVector<int64_t> position(tensorShape.size(), 0);
...
int64_t getIndexFromPosition(toolchain::ArrayRef<int64_t> position,
                             toolchain::ArrayRef<int64_t> tensorShape) {
  int64_t index = 0;
...
toolchain::APInt calculateReducedValue(const mlir::ElementsAttr &oldTensorAttr,
                                  toolchain::ArrayRef<int64_t> oldShape,
                                  int64_t reductionAxis,
                                  int64_t reductionIndex) {

  toolchain::SmallVector<int64_t> newShape(oldShape);
  newShape[reductionAxis] = 1;
  /// Let's calculate the position of the index
  toolchain::SmallVector<int64_t> position =
      getPositionFromIndex(reductionIndex, newShape);
  auto oldTensor = oldTensorAttr.getValues<toolchain::APInt>();
  ...
  toolchain::APInt reducedValue = oldTensor[indexAtOldTensor];

  for (int64_t reductionAxisVal = 1; reductionAxisVal < oldShape[reductionAxis];
       ++reductionAxisVal) {

    int64_t stride = toolchain::product_of(oldShape.drop_front(reductionAxis + 1));
...
struct ReduceConstantOptimization : public OpRewritePattern<OperationType> {
  ...
  LogicalResult matchAndRewrite(OperationType op,
    ...
    if (!toolchain::isa<IntegerType>(shapedOldElementsValues.getElementType()))
      return rewriter.notifyMatchFailure(
    ...
    int64_t newNumOfElements = toolchain::product_of(newShape);
    toolchain::SmallVector<APInt> newReducedTensor(newNumOfElements);
