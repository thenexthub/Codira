...
LogicalResult checkErrorIfResize(Operation *op) {
  ...
  if (inputType.hasStaticShape() && outputType.hasStaticShape()) {
    ...
    const int64_t *maxDim = toolchain::max_element(sizes);
...
LogicalResult checkErrorIfRescale(Operation *op) {
  ...
  auto inputType = toolchain::dyn_cast<ShapedType>(rescale.getInput().getType());
  auto outputType = toolchain::dyn_cast<ShapedType>(rescale.getOutput().getType());
...
static bool isOpIsolatedWithinRegion(Operation *op, Region *region) {
  return toolchain::all_of(op->getOperands(), [&](auto operand) {
