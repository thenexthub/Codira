...
struct DepthwiseConv2DIsMul : public OpRewritePattern<tosa::DepthwiseConv2DOp> {
  ...
  LogicalResult matchAndRewrite(tosa::DepthwiseConv2DOp op,
    ...
    if (!toolchain::all_of(op.getStride(), [](int64_t v) { return v == 1; }))
      return failure();
    ...
    toolchain::SmallVector<int64_t, 2> revisedInputShape{
    ...
    toolchain::SmallVector<int64_t> pad(10, 0);
    for (const auto &it : toolchain::enumerate(padAttr))
      pad[it.index() + 2] = it.value();

    if (toolchain::any_of(pad, [](int64_t p) { return p != 0; })) {
      Attribute zeroAttr = rewriter.getZeroAttr(inputETy);

      toolchain::SmallVector<int64_t> newShape(inputType.getShape());
    ...
    toolchain::SmallVector<int64_t, 2> mulShape{
