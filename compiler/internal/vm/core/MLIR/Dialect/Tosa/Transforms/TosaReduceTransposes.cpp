...
TosaReduceTransposes::transposeDenseAttribute(DenseElementsAttr input,
                                              ArrayRef<int32_t> perms) {
  RankedTensorType oldType = toolchain::cast<RankedTensorType>(input.getType());
  ...
  auto mapCoordinates = [&](int64_t destLinearIndex) -> int64_t {
    ...
    for (auto j : toolchain::seq<int64_t>(rank)) {
  ...
  for (auto destLinearIndex : toolchain::seq<int64_t>(numElements)) {
...
bool TosaReduceTransposes::collectFanIn(Operation *op,
  ...
  if (!toolchain::isa_and_present<tosa::TosaDialect>(op->getDialect()))
    return false;
  ...
  if (op->getNumResults() != 1 ||
      !toolchain::isa<RankedTensorType>(op->getResult(0).getType()))
    return false;
  ...
  if (!toolchain::isa<tosa::TransposeOp>(op) && !toolchain::isa<tosa::ReshapeOp>(op) &&
      !toolchain::isa<tosa::ConstOp>(op)) {

    if (!toolchain::isa<tosa::MulOp>(op) &&
    ...
    for (Value operand : op->getOperands()) {
      ...
      if (toolchain::isa<tosa::MulOp>(op) && operand == op->getOperand(2)) {
...
std::optional<Value> TosaReduceTransposes::buildMappedToValue(
    ...
    IRRewriter &rewriter, ArrayRef<int32_t> hoistedPerms) {
  if (op->getNumResults() != 1 ||
      (!toolchain::isa<tosa::MulOp>(op) &&
       !op->hasTrait<OpTrait::tosa::TosaElementwiseOperator>()))
    return std::nullopt;

  auto resultType = toolchain::cast<RankedTensorType>(op->getResult(0).getType());
  SmallVector<Value, 3> operands;
  for (Value v : op->getOperands()) {
    if (valuesMap.contains(v)) {
      operands.push_back(valuesMap.at(v));
    } else if (toolchain::isa<tosa::MulOp>(op) && v == op->getOperand(2)) {
...
std::optional<Value> TosaReduceTransposes::buildMappedToValue(
  ...
  auto reshapeInputType =
      toolchain::dyn_cast<RankedTensorType>(reshapeOp.getInput1().getType());
  ...
  auto reshapeOutputType =
      toolchain::cast<RankedTensorType>(reshapeOutput.getType());
  ...
  size_t ones = toolchain::count(shape, 1);
  // N == 1 and N != 1
  ...
  toolchain::SmallVector<int64_t> newShape;
  ...
  auto foldedReshape = ReshapeOp::create(
      ...
      reshapeOp.getInput1(),
      getTosaConstShape(builder, applyTOSAPermutation(toolchain::ArrayRef(newShape),
                                                      hoistedPerms)));
...
std::optional<Value> TosaReduceTransposes::buildMappedToValue(
    ...
    IRRewriter &rewriter, ArrayRef<int32_t> hoistedPerms) {
  auto denseAttr = toolchain::dyn_cast<DenseElementsAttr>(constOp.getValues());
  if (!denseAttr)
    return std::nullopt;
...
bool TosaReduceTransposes::convertDependentOps(
  ...
  for (Operation *op : dependentOps) {
    ...
    std::optional<Value> maybeValue =
        toolchain::TypeSwitch<Operation *, std::optional<Value>>(op)
...
bool TosaReduceTransposes::dependenciesAreValid(
  ...
  for (Operation *op : dependentOps) {
    ...
    for (OpOperand &use : op->getUses()) {
      ...
      if (auto otherTranspose = toolchain::dyn_cast<TransposeOp>(user)) {
        ...
        if (!toolchain::equal(perms, otherTranspose.getPerms()))
          return false;
...
void TosaReduceTransposes::runOnOperation() {
  ...
  getOperation().walk([&](TransposeOp transposeOp) {
    ...
    if (!toolchain::isa<RankedTensorType>(input.getType()) ||
        !toolchain::isa<RankedTensorType>(output.getType()))
      return;

    toolchain::append_range(perms, transposeOp.getPerms());
