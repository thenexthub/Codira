...
public:
  ...
  void commit() {
    ...
    for (auto [value, oldType] : oldTypes) {
      ...
      // these first.
      toolchain::SmallVector<OpOperand *> uses = toolchain::map_to_vector(
          value.getUses(),
...
private:
  ...
  // previous type.
  toolchain::SmallVector<std::pair<Value, Type>> oldTypes;
};
...
void propagateShapesToTosaWhile(Operation &op, TypeModificationState &state) {
  ...
  SmallVector<Type> argTypes = toolchain::to_vector(op.getOperandTypes());
  ...
  while (hasNewTypes) {
    ...
    toolchain::SmallVector<YieldOp> yieldOps;
    ...
    toolchain::SmallVector<ValueKnowledge> yieldTypeInfo;
  ...
  for (auto &block : region) {
    for (Operation &op : block) {
      ...
      if (shapeInterface
              ...
              .succeeded()) {
        for (auto it : toolchain::zip(op.getResults(), returnedShapes)) {
