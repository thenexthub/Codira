...
LogicalResult mlir::tosa::EqualizeRanks(ImplicitLocOpBuilder &builder,
                                        Value &input1, Value &input2) {
  auto input1Ty = toolchain::dyn_cast<RankedTensorType>(input1.getType());
  auto input2Ty = toolchain::dyn_cast<RankedTensorType>(input2.getType());
  ...
  ArrayRef<int64_t> higherRankShape =
      toolchain::cast<RankedTensorType>(higherTensorValue.getType()).getShape();
  ArrayRef<int64_t> lowerRankShape =
      toolchain::cast<RankedTensorType>(lowerTensorValue.getType()).getShape();
  ...
  auto reshapeInputType =
      toolchain::cast<RankedTensorType>(lowerTensorValue.getType());
...
Value mlir::tosa::getTosaConstShape(ImplicitLocOpBuilder &builder,
                                    toolchain::ArrayRef<int64_t> shape) {
...
bool mlir::tosa::getConstShapeValues(Operation *op,
                                     toolchain::SmallVector<int64_t> &resultShape) {
  if (!op) {
    return false;
  }
...
mlir::tosa::convertFromIntAttr(const DenseElementsAttr &attr, const int rank) {
  ...
  if (auto intArrayAttr = toolchain::dyn_cast<DenseIntElementsAttr>(attr)) {
...
bool mlir::tosa::hasUniqueConstantScatterIndices(
  ...
  const toolchain::ArrayRef<int64_t> indicesShape = indicesType.getShape();
