...
static SmallVector<int64_t> convertToMlirShape(ArrayRef<int64_t> shape) {
  return to_vector(toolchain::map_range(shape, [](int64_t dim) {
...
Operation *TosaDialect::materializeConstant(OpBuilder &builder, Attribute value,
  ...
  if (toolchain::isa<shapeType>(type) && toolchain::isa<DenseIntElementsAttr>(value)) {
    return tosa::ConstShapeOp::create(builder, loc, type,
                                      toolchain::cast<DenseIntElementsAttr>(value));
  }
  if (toolchain::isa<ElementsAttr>(value))
    return tosa::ConstOp::create(builder, loc, type,
                                 toolchain::cast<ElementsAttr>(value));
  return nullptr;
}
...
void printWithNanPropagationHandling(OpAsmPrinter &parser, Operation *op) {
  ...
  if (!toPrint.empty()) {
    parser << " {";
    toolchain::interleaveComma(toPrint, parser, [&](const NamedAttribute namedAttr) {
...
void printWithEnumHandling(OpAsmPrinter &parser, Operation *op) {
  ...
  if (!op->getAttrs().empty()) {
    parser << " {";
    toolchain::interleaveComma(op->getAttrs(), parser,
...
static Type getStorageElementTypeOrSelf(Type type) {
  ...
  if (auto quantType = toolchain::dyn_cast<mlir::quant::QuantizedType>(srcType))
...
Value mlir::tosa::createPadConstTensor(OpBuilder &builder, Location loc,
  ...
  const auto padConstAttr{
      toolchain::isa<FloatType>(srcElemType)
...
static LogicalResult verifyConvOp(T op) {
  const auto inputType = toolchain::dyn_cast<TensorType>(op.getInput().getType());
  const auto weightType = toolchain::dyn_cast<TensorType>(op.getWeight().getType());

  auto inputEType = inputType.getElementType();
  auto weightEType = weightType.getElementType();
  auto biasEType =
      toolchain::cast<ShapedType>(op.getBias().getType()).getElementType();
  auto resultEType =
      toolchain::cast<ShapedType>(op.getResult().getType()).getElementType();
  bool biasIsFloat = toolchain::isa<FloatType>(biasEType);
  bool resultIsFloat = toolchain::isa<FloatType>(resultEType);

  if (auto quantType = toolchain::dyn_cast<mlir::quant::QuantizedType>(inputEType))
    inputEType = getStorageElementTypeFromQuantized(quantType);

  if (auto quantType = toolchain::dyn_cast<mlir::quant::QuantizedType>(weightEType))
    weightEType = getStorageElementTypeFromQuantized(quantType);

  if (auto quantType = toolchain::dyn_cast<mlir::quant::QuantizedType>(biasEType))
    biasEType = getStorageElementTypeFromQuantized(quantType);

  if (auto quantType = toolchain::dyn_cast<mlir::quant::QuantizedType>(resultEType))
  ...
  bool inputIsFloat = toolchain::isa<FloatType>(inputEType);
  bool weightIsFloat = toolchain::isa<FloatType>(weightEType);
...
LogicalResult tosa::ConstOp::verify() {
  ...
  auto attrType = toolchain::dyn_cast<TensorType>(getValuesAttr().getType());
  auto outputType = toolchain::dyn_cast<TensorType>(getOutput().getType());
  ...
  if (auto result = toolchain::dyn_cast<mlir::quant::QuantizedType>(
...
static LogicalResult verifyConvOpModes(T op) {
  auto inputEType =
      toolchain::cast<ShapedType>(op.getInput().getType()).getElementType();
...
static LogicalResult verifyConvOpErrorIf(T op) {
  toolchain::ArrayRef<int64_t> padding = op.getPad();
  if (toolchain::any_of(padding, [](int64_t p) { return p < 0; }))
    return op.emitOpError("expect all padding values to be >= 0, got ")
           << padding;

  toolchain::ArrayRef<int64_t> strides = op.getStride();
  if (toolchain::any_of(strides, [](int64_t s) { return s < 1; }))
    return op.emitOpError("expect all stride values to be >= 1, got ")
           << strides;

  toolchain::ArrayRef<int64_t> dilations = op.getDilation();
  if (toolchain::any_of(dilations, [](int64_t d) { return d < 1; }))
    return op.emitOpError("expect all dilation values to be >= 1, got ")
           << dilations;

  const RankedTensorType outputType =
      toolchain::dyn_cast<RankedTensorType>(op.getOutput().getType());
  if (!outputType)
    // Skip following checks if output is not ranked
    return success();

  const RankedTensorType inputType =
      toolchain::dyn_cast<RankedTensorType>(op.getInput().getType());
  const RankedTensorType weightType =
      toolchain::dyn_cast<RankedTensorType>(op.getWeight().getType());
  ...
  if (inputType && weightType) {
    const auto verifyOutputSize =
        [&op](const int64_t inputSize, const int64_t kernelSize,
              ...
              const int64_t dilation, const toolchain::StringRef dimName,
              const toolchain::StringRef dimAxis,
              const toolchain::StringRef padBeforeName,
              const toolchain::StringRef padAfterName) -> LogicalResult {
  ...
  const RankedTensorType biasType =
      toolchain::dyn_cast<RankedTensorType>(op.getBias().getType());
  if (!biasType)
...
static LogicalResult errorIfTypeOrShapeMismatch(Operation *op, ValueRange list1,
  ...
  for (auto [type1, type2] :
       toolchain::zip_equal(list1.getTypes(), list2.getTypes())) {
...
static LogicalResult verifySameElementTypes(T op, Type aType, Type bType,
  ...
  auto aTType = toolchain::dyn_cast<TensorType>(aType);
  auto bTType = toolchain::dyn_cast<TensorType>(bType);
  if (!aTType) {
  ...
  auto aQuantType =
      toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(aElementType);
  auto bQuantType =
      toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(bElementType);
...
LogicalResult tosa::ArgMaxOp::verify() {
  const ShapedType resultType = toolchain::cast<ShapedType>(getType());
  ...
  const auto inputType = toolchain::cast<ShapedType>(getInput().getType());
  ...
  toolchain::SmallVector<int64_t> expectedOutputShape(inputShape);
...
static LogicalResult verifyPoolingOp(T op) {
  const toolchain::ArrayRef<int64_t> kernel = op.getKernel();
  if (toolchain::any_of(kernel, [](int64_t s) { return s < 1; }))
    return op.emitOpError("expect all kernel values to be >= 1, got ")
           << kernel;

  const toolchain::ArrayRef<int64_t> strides = op.getStride();
  if (toolchain::any_of(strides, [](int64_t s) { return s < 1; }))
    return op.emitOpError("expect all stride values to be >= 1, got ")
           << strides;

  const toolchain::ArrayRef<int64_t> padding = op.getPad();
  if (toolchain::any_of(padding, [](int64_t p) { return p < 0; }))
    return op.emitOpError("expect all padding values to be >= 0, got ")
           << padding;
  ...
  const auto outputType =
      toolchain::dyn_cast<RankedTensorType>(op.getResult().getType());
  ...
  const auto verifyOutputSize =
      [&op](const int64_t inputSize, const int64_t outputSize,
            ...
            const toolchain::StringRef dimName, const toolchain::StringRef dimAxis,
            const toolchain::StringRef padBeforeName,
            const toolchain::StringRef padAfterName) -> LogicalResult {
...
LogicalResult tosa::AvgPool2dOp::verify() {
  ...
  if (toolchain::isa<IntegerType>(inputETy) && !accType.isInteger(32))
...
LogicalResult tosa::ClampOp::verify() {
  mlir::Type inputETy =
      toolchain::cast<ShapedType>(getInput().getType()).getElementType();
  if (auto quantType =
          toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(inputETy)) {
    inputETy = getStorageElementTypeFromQuantized(quantType);
  }
  mlir::Type outputETy =
      toolchain::cast<ShapedType>(getOutput().getType()).getElementType();
  if (auto quantType =
          toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(outputETy)) {
...
static void buildMatMulOpWithQuantInfo(OpBuilder &builder,
  ...
  if (auto quantAttr = buildMatMulOpQuantizationAttr(builder, a, b)) {
    ...
    auto outputShapedType = toolchain::dyn_cast<ShapedType>(outputType);
...
LogicalResult tosa::RFFT2dOp::inferReturnTypeComponents(
  ...
  toolchain::SmallVector<int64_t> outputShape;
...
static LogicalResult verifyDimIsPowerOfTwo(Operation *op, const int64_t dimSize,
                                           const toolchain::StringRef dimName) {
...
LogicalResult tosa::RFFT2dOp::verify() {
  ...
  const auto inputType =
      toolchain::dyn_cast<RankedTensorType>(getInputReal().getType());
  if (!inputType)
    return success();
  ...
  const auto outputType = toolchain::dyn_cast<RankedTensorType>(outputTypes[0]);
  if (!outputType)
    return success();
...
LogicalResult tosa::FFT2dOp::verify() {
  ...
  const auto inputImagType =
      toolchain::dyn_cast<RankedTensorType>(getInputImag().getType());
...
LogicalResult tosa::ConcatOp::inferReturnTypeComponents(
  ...
  Type inputType =
      toolchain::cast<TensorType>(adaptor.getInput1().getType()[0]).getElementType();
  if (!hasRankedInput) {
...
LogicalResult tosa::ConcatOp::verify() {
  ...
  if (!toolchain::all_of(inputList, [&](auto input) {
  ...
  if (toolchain::all_of(inputList, allOperandsHasRank)) {
    const int64_t firstInputRank = firstRankedInputShape.getRank();

    for (const auto &[index, input] : toolchain::enumerate(inputList.drop_front())) {
...
LogicalResult tosa::EqualOp::inferReturnTypeComponents(
  ...
  toolchain::SmallVector<int64_t> outShape;
...
LogicalResult MatMulOp::verify() {
  auto aType = toolchain::dyn_cast<ShapedType>(getA().getType());
  auto bType = toolchain::dyn_cast<ShapedType>(getB().getType());
  ...
  auto aQuantizedEType =
      toolchain::dyn_cast<quant::UniformQuantizedType>(aElementType);
  auto bQuantizedEType =
      toolchain::dyn_cast<quant::UniformQuantizedType>(bElementType);
...
LogicalResult MatmulTBlockScaledOp::verify() {
  ...
  if (outputType.hasRank() &&
    ...
    toolchain::interleaveComma(outputType.getShape(), opError, stringifyDim);
    opError << " to be compatible with expected output shape ";
    toolchain::interleaveComma(expectedOutputShape, opError, stringifyDim);
    return opError;
  }
...
LogicalResult tosa::PadOp::verify() {
  ...
  RankedTensorType inputType =
      toolchain::dyn_cast<RankedTensorType>(getInput1().getType());
  RankedTensorType outputType =
      toolchain::dyn_cast<RankedTensorType>(getOutput().getType());
  if (!inputType || !outputType)
    return success();
...
LogicalResult tosa::SliceOp::verify() {
  ...
  if (inputShape.hasRank()) {
    ...
    const auto startShapeRank =
        toolchain::cast<tosa::shapeType>(getStart().getType()).getRank();
    ...
    const auto sizeShapeRank =
        toolchain::cast<tosa::shapeType>(getSize().getType()).getRank();
...
LogicalResult tosa::TableOp::verify() {
  ...
  for (auto it : toolchain::enumerate(toolchain::zip(inputDims, outputDims))) {
...
tosa::TileOp::getConstantMultiples(SmallVector<int64_t> &multiples) {
  ...
  if (!matchPattern(getMultiples(), m_Constant(&multiplesAttr)))
    return failure();
  multiples = toolchain::to_vector(
      toolchain::map_range(multiplesAttr.getValues<APInt>(),
...
LogicalResult tosa::TileOp::verify() {
  ...
  ShapedType inputType = toolchain::cast<ShapedType>(getInput1().getType());
  ShapedType outputType = toolchain::cast<ShapedType>(getType());

  shapeType multiplesType =
      toolchain::cast<tosa::shapeType>(getMultiples().getType());
  ...
  if (getConstantMultiples(multiples).succeeded() &&
      toolchain::any_of(multiples, [](int64_t v) { return v <= 0 && v != -1; }))
    return emitOpError(
...
LogicalResult tosa::ReshapeOp::inferReturnTypeComponents(
  ...
  ShapeAdaptor inputShape(adaptor.getInput1().getType());
  Type inputType = getElementTypeOrSelf(adaptor.getInput1().getType());
  toolchain::SmallVector<int64_t> newShapeValue;
...
toolchain::LogicalResult tosa::ReshapeOp::verify() {
  if (verifySameElementTypes(*this, /* inType = */ getInput1().getType(),
                             /* outType = */ getOutput().getType())
          .failed()) {
    return failure();
  ...
  int missingDims = toolchain::count(shapeValues, -1);
  if (missingDims > 1)
  ...
  if (inputType.hasStaticShape()) {
    ...
    int64_t newShapeElementsNum =
        toolchain::accumulate(shapeValues, int64_t(1), [](int64_t acc, int64_t dim) {
          return (dim > 0) ? acc * dim : acc;
        });
    bool isStaticNewShape =
        toolchain::all_of(shapeValues, [](int64_t s) { return s > 0; });
...
static FailureOr<int64_t> getZeroPoint(Value val, bool signExtend) {
  ...
  if (toolchain::isa<FloatType>(zpElemType)) {
  ...
  if (toolchain::isa<IntegerType>(zpElemType)) {
    if (signExtend)
...
static LogicalResult verifyZeroPoint(T op, Value val, const int64_t &zp,
  ...
  if (!zpElemType.isInteger(8) && zp != 0) {
    ...
    toolchain::transform(lower, lower.begin(), ::tolower);
    return op.emitOpError()
...
LogicalResult tosa::TransposeOp::inferReturnTypeComponents(
  ...
  if (toolchain::any_of(adaptor.getPerms(),
...
LogicalResult tosa::TransposeOp::verify() {
  ...
  const toolchain::ArrayRef<int32_t> constantPerms = getPerms();
  ...
  if (!toolchain::all_of(constantPerms,
                    [&constantPerms](int32_t s) {
                      return s >= 0 &&
                             static_cast<size_t>(s) < constantPerms.size();
                    }) ||
      !isPermutationVector(toolchain::to_vector(toolchain::map_range(
...
LogicalResult TransposeOp::reifyResultShapes(
  ...
  const toolchain::ArrayRef<int32_t> transposePerms = getPerms();
...
LogicalResult tosa::ResizeOp::inferReturnTypeComponents(
    ...
    SmallVectorImpl<ShapedTypeComponents> &inferredReturnShapes) {
  toolchain::SmallVector<int64_t, 4> outputShape;
...
LogicalResult tosa::ResizeOp::verify() {
  ...
  const Value output = getOutput();
  const RankedTensorType inputType =
      toolchain::dyn_cast<RankedTensorType>(input.getType());
  const RankedTensorType outputType =
      toolchain::dyn_cast<RankedTensorType>(output.getType());
  ...
  if (toolchain::any_of(scaleValues, [](int64_t s) { return s <= 0; }))
...
#define REDUCE_SHAPE_INFER(OP)                                                 \
  LogicalResult OP::inferReturnTypeComponents(                                 \
      ...
      SmallVectorImpl<ShapedTypeComponents> &inferredReturnShapes) {           \
    Type inputType =                                                           \
        toolchain::cast<TensorType>(adaptor.getInput().getType()).getElementType(); \
    ShapeAdaptor inputShape(adaptor.getInput().getType());                     \
    const Properties &prop = adaptor.getProperties();                          \
...
LogicalResult Conv2DOp::inferReturnTypeComponents(
    ...
    SmallVectorImpl<ShapedTypeComponents> &inferredReturnShapes) {
  toolchain::SmallVector<int64_t> outputShape(4, ShapedType::kDynamic);
  ...
  toolchain::ArrayRef<int64_t> dilation = adaptor.getDilation();
  toolchain::ArrayRef<int64_t> stride = adaptor.getStride();
  toolchain::ArrayRef<int64_t> padding = adaptor.getPad();
...
LogicalResult Conv3DOp::inferReturnTypeComponents(
    ...
    SmallVectorImpl<ShapedTypeComponents> &inferredReturnShapes) {
  toolchain::SmallVector<int64_t> outputShape(5, ShapedType::kDynamic);
  ...
  toolchain::ArrayRef<int64_t> dilation = adaptor.getDilation();
  toolchain::ArrayRef<int64_t> stride = adaptor.getStride();
  toolchain::ArrayRef<int64_t> pad = adaptor.getPad();
...
LogicalResult TransposeConv2DOp::inferReturnTypeComponents(
  ...
  toolchain::ArrayRef<int64_t> padding = adaptor.getOutPad();
  toolchain::ArrayRef<int64_t> stride = adaptor.getStride();
...
LogicalResult TransposeConv2DOp::verify() {
  ...
  const toolchain::ArrayRef<int64_t> strides = getStride();
  ...
  const auto checkPadAgainstKernelDim =
      [this](int64_t padValue, int64_t kernelDimSize, toolchain::StringRef padName,
             toolchain::StringRef kernelDimName) -> LogicalResult {
  ...
  const toolchain::ArrayRef<int64_t> padding = getOutPad();
  ...
  const auto weightType =
      toolchain::dyn_cast<RankedTensorType>(getWeight().getType());
  ...
  const auto inputType = toolchain::dyn_cast<RankedTensorType>(getInput().getType());
  if (inputType && weightType) {
  ...
  const auto biasType = toolchain::dyn_cast<RankedTensorType>(getBias().getType());
...
LogicalResult RescaleOp::verify() {
  auto inputType = toolchain::dyn_cast<ShapedType>(getInput().getType());
  if (!inputType) {
  ...
  auto outputType = toolchain::dyn_cast<ShapedType>(getOutput().getType());
  if (!outputType) {
  ...
  auto multiplierType = toolchain::dyn_cast<ShapedType>(getMultiplier().getType());
  if (!multiplierType) {
  ...
  auto shiftType = toolchain::dyn_cast<ShapedType>(getShift().getType());
  if (!shiftType) {
...
LogicalResult IfOp::inferReturnTypeComponents(
    ...
    SmallVectorImpl<ShapedTypeComponents> &inferredReturnShapes) {
  toolchain::SmallVector<tosa::YieldOp> yieldOps;
  ...
  toolchain::SmallVector<ValueKnowledge> resultKnowledge;
  ...
  for (auto yieldOp : yieldOps) {
    ...
    for (const auto &it : toolchain::enumerate(yieldOp.getOperands())) {
...
std::optional<SmallVector<int64_t, 4>> ApplyScaleOp::getShapeForUnroll() {
  if (auto vt = toolchain::dyn_cast<VectorType>(getType()))
    return toolchain::to_vector<4>(vt.getShape());
  return std::nullopt;
}
...
static void printInitializationList(OpAsmPrinter &parser,
  ...
  toolchain::interleaveComma(
      toolchain::zip(blocksArgs, initializers), parser,
...
void IfOp::print(OpAsmPrinter &p) {
  ...
  if (!getInputList().empty()) {
    p << " (";
    toolchain::interleaveComma(getInputList().getTypes(), p);
    p << ")";
  }
...
LogicalResult tosa::SelectOp::verify() {
  ...
  auto predicateType = toolchain::dyn_cast<ShapedType>(getPred().getType());
  if (!predicateType) {
...
std::optional<Value> mlir::tosa::createZeroPointTensor(OpBuilder &builder,
  ...
  if (toolchain::isa<FloatType>(srcElemType)) {
  ...
  if (toolchain::isa<IntegerType>(srcElemType)) {
    auto zpAttr =
        DenseElementsAttr::get(zpType, builder.getIntegerAttr(srcElemType, zp));
    return tosa::ConstOp::create(builder, loc, zpType, zpAttr);
  }
  toolchain::errs() << "zero point is not allowed for unsupported data types\n";
  return std::nullopt;
}
...
LogicalResult tosa::ConcatShapeOp::verify() {
  ...
  const int64_t inputsRank =
      toolchain::accumulate(inputList, 0, [](int64_t acc, const Value &input) {
