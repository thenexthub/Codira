...
struct FoldPadToTensorOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy tensorOp,
    ...
    if (toolchain::any_of(foldedPad, [](int64_t padVal) { return padVal > 8192; })) {
      return rewriter.notifyMatchFailure(
...
struct MaxPool2dIsNoOp : public OpRewritePattern<tosa::MaxPool2dOp> {
  ...
  LogicalResult matchAndRewrite(tosa::MaxPool2dOp op,
    ...
    ShapedType inputType = toolchain::cast<ShapedType>(input.getType());
    ShapedType outputType = toolchain::cast<ShapedType>(output.getType());
...
struct ConsolidateTransposeOptimization
  ...
  LogicalResult matchAndRewrite(tosa::TransposeOp transposeOp,
    ...
    const toolchain::ArrayRef<int32_t> transposePerms = transposeOp.getPerms();
    const toolchain::ArrayRef<int32_t> innerTransposePerms =
...
struct TransposeIsReshape : public OpRewritePattern<tosa::TransposeOp> {
  ...
  LogicalResult matchAndRewrite(tosa::TransposeOp op,
    ...
    auto inputTy = toolchain::cast<ShapedType>(input.getType());
    if (!inputTy.hasRank())
    ...
    const toolchain::ArrayRef<int32_t> permValues = op.getPerms();
...
struct ClampIsNoOp : public OpRewritePattern<tosa::ClampOp> {
  ...
  LogicalResult matchAndRewrite(tosa::ClampOp op,
                                PatternRewriter &rewriter) const override {
    Value input = op.getInput();
    auto inputType = toolchain::dyn_cast<RankedTensorType>(op.getInput().getType());
    auto inputElementType = inputType.getElementType();

    if (isa<FloatType>(inputElementType)) {
      // Unlike integer types, floating point types can represent infinity.
      const auto minClamp =
          toolchain::cast<mlir::FloatAttr>(op.getMinValAttr()).getValue();
      const auto maxClamp =
          toolchain::cast<mlir::FloatAttr>(op.getMaxValAttr()).getValue();
    ...
    if (inputElementType.isUnsignedInteger() || isBoolean) {
      const int64_t minClamp = toolchain::cast<mlir::IntegerAttr>(op.getMinValAttr())
                                   .getValue()
                                   .getZExtValue();
      const int64_t maxClamp = toolchain::cast<mlir::IntegerAttr>(op.getMaxValAttr())
                                   .getValue()
                                   .getZExtValue();
    ...
    if (toolchain::isa<IntegerType>(inputElementType)) {
      const int64_t minClamp =
          toolchain::cast<mlir::IntegerAttr>(op.getMinValAttr()).getInt();
      const int64_t maxClamp =
          toolchain::cast<mlir::IntegerAttr>(op.getMaxValAttr()).getInt();
...
struct ClampClampOptimization : public OpRewritePattern<tosa::ClampOp> {
  ...
  LogicalResult matchAndRewrite(tosa::ClampOp op,
    ...
    auto inputEType = toolchain::cast<ShapedType>(input.getType()).getElementType();
    if (auto quantType =
            toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(inputEType)) {
...
struct ConcatSliceOptimization : public OpRewritePattern<tosa::SliceOp> {
  ...
  LogicalResult matchAndRewrite(tosa::SliceOp sliceOp,
    ...
    toolchain::SmallVector<int64_t> sliceStarts =
        toolchain::to_vector(startElems.getValues<int64_t>());
    toolchain::SmallVector<int64_t> sliceSizes =
        toolchain::to_vector(sizeElems.getValues<int64_t>());
...
struct PadSliceOptimization : public OpRewritePattern<tosa::SliceOp> {
  ...
  LogicalResult matchAndRewrite(tosa::SliceOp sliceOp,
    ...
    toolchain::SmallVector<int64_t> padPaddings =
        toolchain::to_vector(paddingElems.getValues<int64_t>());
    ...
    if (toolchain::any_of(toolchain::seq<int64_t>(0, rank), [&](int64_t i) {
    ...
    toolchain::SmallVector<int64_t> newSliceStarts(rank, 0);
    toolchain::SmallVector<int64_t> newPadPaddings(2 * rank, 0);
    toolchain::SmallVector<int64_t> newPadShape(rank, ShapedType::kDynamic);
    bool updated = false;
...
struct SliceDynamicSizeCanonicalization
  ...
  LogicalResult matchAndRewrite(tosa::SliceOp sliceOp,
    ...
    // shape
    for (const auto &[index, size] : toolchain::enumerate(sliceSizes)) {
...
static DenseElementsAttr binaryFolder(DenseElementsAttr lhs,
  ...
  if (rhs && lhs && rhs.isSplat() && lhs.isSplat()) {
    auto lETy = toolchain::cast<ShapedType>(lhs.getType()).getElementType();
    auto rETy = toolchain::cast<ShapedType>(rhs.getType()).getElementType();
    if (lETy != rETy)
      return {};

    if (toolchain::isa<IntegerType>(lETy)) {
    ...
    if (toolchain::isa<FloatType>(lETy)) {
...
OpFoldResult AddOp::fold(FoldAdaptor adaptor) {
  auto lhsTy = toolchain::dyn_cast<RankedTensorType>(getInput1().getType());
  auto rhsTy = toolchain::dyn_cast<RankedTensorType>(getInput2().getType());
  auto resultTy = toolchain::dyn_cast<RankedTensorType>(getType());
  ...
  auto lhsAttr =
      toolchain::dyn_cast_if_present<DenseElementsAttr>(adaptor.getInput1());
  auto rhsAttr =
      toolchain::dyn_cast_if_present<DenseElementsAttr>(adaptor.getInput2());
...
OpFoldResult ArgMaxOp::fold(FoldAdaptor adaptor) {
  auto inputTy = toolchain::dyn_cast<RankedTensorType>(getInput().getType());
  auto outputTy = toolchain::dyn_cast<RankedTensorType>(getType());
...
OpFoldResult IntDivOp::fold(FoldAdaptor adaptor) {
  ...
  if (lhsAttr && lhsAttr.isSplat()) {
    if (toolchain::isa<IntegerType>(resultETy) &&
  ...
  if (rhsAttr && lhsAttr && rhsAttr.isSplat() && lhsAttr.isSplat() &&
      toolchain::isa<IntegerType>(resultETy)) {
...
DenseElementsAttr mulBinaryFolder(DenseElementsAttr lhs, DenseElementsAttr rhs,
  ...
  if (rhs && lhs && rhs.isSplat() && lhs.isSplat()) {
    if (toolchain::isa<IntegerType>(ty.getElementType())) {
      APInt l = lhs.getSplatValue<APInt>();
      APInt r = rhs.getSplatValue<APInt>();
    ...
    if (toolchain::isa<FloatType>(ty.getElementType())) {
      APFloat l = lhs.getSplatValue<APFloat>();
      APFloat r = rhs.getSplatValue<APFloat>();
      APFloat result = l * r;
...
OpFoldResult MulOp::fold(FoldAdaptor adaptor) {
  auto lhs = getInput1();
  auto rhs = getInput2();
  auto lhsTy = toolchain::dyn_cast<RankedTensorType>(lhs.getType());
  auto rhsTy = toolchain::dyn_cast<RankedTensorType>(rhs.getType());
  auto resultTy = toolchain::dyn_cast<RankedTensorType>(getType());
  if (!lhsTy || !rhsTy || !resultTy)
    return {};
...
OpFoldResult CastOp::fold(FoldAdaptor adaptor) {
  ...
  auto operand = toolchain::dyn_cast_if_present<ElementsAttr>(adaptor.getInput());
  if (!operand)
    return {};

  auto inTy = toolchain::cast<ShapedType>(getInput().getType());
  auto outTy = toolchain::cast<ShapedType>(getType());
  auto inETy = inTy.getElementType();
  auto outETy = outTy.getElementType();

  if (operand.isSplat()) {
    if (toolchain::isa<FloatType>(inETy) && toolchain::isa<FloatType>(outETy)) {
      bool overflow;
      auto splatVal = operand.getSplatValue<APFloat>();
      auto &semantics = toolchain::cast<FloatType>(outETy).getFloatSemantics();
      splatVal.convert(semantics, toolchain::RoundingMode::NearestTiesToEven,
                       &overflow);
      return SplatElementsAttr::get(outTy, splatVal);
    }

    if (toolchain::isa<IntegerType>(inETy) && toolchain::isa<FloatType>(outETy)) {
      auto unsign = toolchain::cast<IntegerType>(inETy).isUnsignedInteger();
      APFloat splatVal(toolchain::cast<FloatType>(outETy).getFloatSemantics());
      splatVal.convertFromAPInt(operand.getSplatValue<APInt>(), !unsign,
                                toolchain::RoundingMode::NearestTiesToEven);
      return SplatElementsAttr::get(outTy, splatVal);
    }

    if (toolchain::isa<FloatType>(inETy) && toolchain::isa<IntegerType>(outETy)) {
      auto unsign = toolchain::cast<IntegerType>(outETy).isUnsignedInteger();
      auto intVal = APSInt(
          toolchain::cast<IntegerType>(outETy).getIntOrFloatBitWidth(), unsign);
      auto floatVal = operand.getSplatValue<APFloat>();
      bool exact;
      floatVal.convertToInteger(intVal, toolchain::RoundingMode::NearestTiesToEven,
                                &exact);
      return SplatElementsAttr::get(outTy, intVal);
    }

    if (toolchain::isa<IntegerType>(inETy) && toolchain::isa<IntegerType>(outETy)) {
      const auto inIntType = toolchain::cast<IntegerType>(inETy);
...
#define REDUCE_FOLDER(OP)                                                      \
  OpFoldResult OP::fold(FoldAdaptor adaptor) {                                 \
    ShapedType inputTy = toolchain::cast<ShapedType>(getInput().getType());         \
    if (!inputTy.hasRank())                                                    \
      return {};                                                               \
    if (inputTy != getType())                                                  \
      return {};                                                               \
...
OpFoldResult ReshapeOp::fold(FoldAdaptor adaptor) {
  auto inputTy = toolchain::dyn_cast<RankedTensorType>(getInput1().getType());
  auto outputTy = toolchain::dyn_cast<RankedTensorType>(getType());
  ...
  if (auto reshapeOp = toolchain::dyn_cast_if_present<tosa::ReshapeOp>(
  ...
  if (auto operand =
    ...
    toolchain::SmallVector<int64_t> shapeVec;
    if (!tosa::getConstShapeValues(getShape().getDefiningOp(), shapeVec))
      return {};

    return operand.reshape(
        toolchain::cast<ShapedType>(operand.getType()).clone(shapeVec));
  }
...
OpFoldResult PadOp::fold(FoldAdaptor adaptor) {
  ...
  if (adaptor.getPadding() && getInput1().getType() == getType()) {
    auto densePad = toolchain::dyn_cast<DenseElementsAttr>(adaptor.getPadding());
...
OpFoldResult ResizeOp::fold(FoldAdaptor adaptor) {
  auto scaleAttr =
      toolchain::dyn_cast_if_present<DenseElementsAttr>(adaptor.getScale());
  auto offsetAttr =
      toolchain::dyn_cast_if_present<DenseElementsAttr>(adaptor.getOffset());
  auto borderAttr =
      toolchain::dyn_cast_if_present<DenseElementsAttr>(adaptor.getBorder());
  ...
  auto input = getInput();
  auto inputTy = toolchain::cast<RankedTensorType>(input.getType());
  auto resultTy = toolchain::cast<RankedTensorType>(getType());
  if (inputTy != resultTy)
    return {};
...
OpFoldResult ReverseOp::fold(FoldAdaptor adaptor) {
  ...
  auto operandTy = toolchain::cast<ShapedType>(operand.getType());
  auto axis = getAxis();
  auto operandAttr =
      toolchain::dyn_cast_if_present<SplatElementsAttr>(adaptor.getInput1());
  if (operandAttr)
    return operandAttr;
...
OpFoldResult SliceOp::fold(FoldAdaptor adaptor) {
  ...
  auto operand = toolchain::cast<ElementsAttr>(adaptor.getInput1());
  ...
  if (inputTy.hasStaticShape() && outputTy.hasStaticShape() &&
    ...
    toolchain::SmallVector<uint64_t> indices =
        toolchain::to_vector(startElems.getValues<uint64_t>());
...
mayRequireBroadcast(ValueTypeRange<mlir::OperandRange> operandTypes) {
  const auto isDynamic = [](Type ty) {
    const auto shapedTy = toolchain::dyn_cast<ShapedType>(ty);
    return !shapedTy || !shapedTy.hasStaticShape();
  };

  return toolchain::any_of(operandTypes, isDynamic) ||
...
OpFoldResult tosa::SelectOp::fold(FoldAdaptor adaptor) {
  ...
  auto predicate =
      toolchain::dyn_cast_if_present<DenseIntElementsAttr>(adaptor.getInput1());
  if (!predicate)
    return {};
...
OpFoldResult TileOp::fold(FoldAdaptor adaptor) {
  if (getInput1().getType() == getType()) {
    if (auto multiples = toolchain::dyn_cast_if_present<DenseElementsAttr>(
            adaptor.getMultiples())) {
      if (multiples.isSplat() &&
          multiples.getSplatValue<APInt>().getSExtValue() == 1)
        return getInput1();
      if (auto int_array_attr =
              toolchain::dyn_cast<DenseIntElementsAttr>(multiples)) {
        if (toolchain::all_of(int_array_attr.getValues<APInt>(),
...
OpFoldResult TransposeOp::fold(FoldAdaptor adaptor) {
  auto resultTy = toolchain::cast<ShapedType>(getType());
  ...
  const toolchain::ArrayRef<int32_t> perms = getPerms();

  if (!toolchain::equal(toolchain::seq<int32_t>(0, perms.size()), perms))
    return {};
...
OpFoldResult ConcatOp::fold(FoldAdaptor adaptor) {
  ...
  for (Value operand : getOperands()) {
    ...
    toolchain::append_range(concatOperands, producer->getOperands());
  }
...
OpFoldResult tosa::ReciprocalOp::fold(FoldAdaptor adaptor) {
  ...
  auto inputAttr = toolchain::dyn_cast_if_present<DenseElementsAttr>(input);
  // Fold splat inputs only.
  if (!inputAttr || !inputAttr.isSplat())
    return {};

  auto shapeType = toolchain::cast<ShapedType>(getType());
  if (auto floatType = toolchain::dyn_cast<FloatType>(inputAttr.getElementType())) {
