...
struct CastOpInterface
  ...
  getBufferType(Operation *op, Value value, const BufferizationOptions &options,
    ...
    return cast<BufferLikeType>(MemRefType::get(
        ...
        toolchain::cast<MemRefType>(*maybeSrcBufferType).getLayout(), memorySpace));
  }
...
struct CollapseShapeOpInterface
  ...
  getBufferType(Operation *op, Value value, const BufferizationOptions &options,
    ...
    if (failed(maybeSrcBufferType))
      return failure();
    auto srcBufferType = toolchain::cast<MemRefType>(*maybeSrcBufferType);
...
struct ExtractSliceOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    Value subView = memref::SubViewOp::create(
        rewriter, loc, toolchain::cast<MemRefType>(*resultMemrefType), *srcMemref,
  ...
  getBufferType(Operation *op, Value value, const BufferizationOptions &options,
    ...
    return cast<BufferLikeType>(memref::SubViewOp::inferRankReducedResultType(
        ...
        toolchain::cast<MemRefType>(*srcMemrefType), mixedOffsets, mixedSizes,
        mixedStrides));
  }
};
...
struct FromElementsOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    auto maxDim = *toolchain::max_element(shape);
...
static bool insertSliceOpRequiresRead(InsertOpTy insertSliceOp,
  ...
  bool allOffsetsZero =
      toolchain::all_of(insertSliceOp.getMixedOffsets(), isZeroInteger);
...
struct ReshapeOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    auto srcType = toolchain::dyn_cast<MemRefType>(srcBuffer->getType());
