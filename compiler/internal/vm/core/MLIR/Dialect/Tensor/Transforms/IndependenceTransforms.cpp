...
FailureOr<Value> tensor::buildIndependentOp(OpBuilder &b, tensor::PadOp padOp,
  ...
  if (toolchain::equal(padOp.getMixedLowPad(), newMixedLow) &&
      toolchain::equal(padOp.getMixedHighPad(), newMixedHigh))
...
FailureOr<Value> tensor::buildIndependentOp(OpBuilder &b,
  ...
  if (toolchain::equal(emptyOp.getMixedSizes(), newSizes))
