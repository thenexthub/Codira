...
struct CastOpInterface
  ...
  void populateBoundsForShapedValueDim(Operation *op, Value value, int64_t dim,
    ...
    if (toolchain::isa<RankedTensorType>(castOp.getResult().getType()) &&
        toolchain::isa<RankedTensorType>(castOp.getSource().getType())) {
...
struct ExtractSliceOpInterface
  ...
  void populateBoundsForShapedValueDim(Operation *op, Value value, int64_t dim,
    ...
    toolchain::SmallBitVector dropped = extractSliceOp.getDroppedDims();
    int64_t ctr = -1;
...
struct RankOpInterface
  ...
  void populateBoundsForIndexValue(Operation *op, Value value,
    ...
    auto tensorType =
        toolchain::dyn_cast<RankedTensorType>(rankOp.getTensor().getType());
    if (!tensorType)
      return;
