...
OpFoldResult tensor::getMixedSize(OpBuilder &builder, Location loc, Value value,
                                  int64_t dim) {
  auto tensorType = toolchain::cast<RankedTensorType>(value.getType());
...
FailureOr<Value> tensor::getOrCreateDestination(OpBuilder &b, Location loc,
                                                OpResult opResult) {
  auto tensorType = toolchain::dyn_cast<TensorType>(opResult.getType());
...
LogicalResult tensor::getOrCreateDestinations(OpBuilder &b, Location loc,
  ...
  for (OpResult opResult : op->getResults()) {
    if (toolchain::isa<TensorType>(opResult.getType())) {
...
bool tensor::isSameTypeWithoutEncoding(Type tp1, Type tp2) {
  if (auto rtp1 = toolchain::dyn_cast<RankedTensorType>(tp1)) {
    if (auto rtp2 = toolchain::dyn_cast<RankedTensorType>(tp2))
...
static toolchain::SmallBitVector getDroppedDims(ArrayRef<int64_t> reducedShape,
                                           ArrayRef<OpFoldResult> mixedSizes) {
  toolchain::SmallBitVector droppedDims(mixedSizes.size());
  int64_t shapePos = reducedShape.size() - 1;

  for (const auto &size : enumerate(toolchain::reverse(mixedSizes))) {
    ...
    bool isStaticUnitSize =
        ...
        toolchain::cast<IntegerAttr>(cast<Attribute>(size.value())).getInt() == 1;
...
bool mlir::tensor::preservesStaticInformation(Type source, Type target) {
  auto sourceType = toolchain::dyn_cast<RankedTensorType>(source);
  auto targetType = toolchain::dyn_cast<RankedTensorType>(target);
  ...
  for (auto t : toolchain::zip(sourceType.getShape(), targetType.getShape())) {
...
bool mlir::tensor::hasFoldableTensorCastOperand(Operation *op) {
  return toolchain::any_of(op->getOpOperands(), [&](OpOperand &opOperand) {
    if (toolchain::isa<BlockArgument>(opOperand.get()))
      return false;
...
SmallVector<Value> mlir::tensor::getUpdatedOperandsAfterCastOpFolding(
  ...
  for (OpOperand &opOperand : op->getOpOperands()) {
    ...
    if (op.isDpsInit(&opOperand) &&
        !toolchain::isa<MemRefType>(newOperands.back().getType()))
...
bool CastOp::areCastCompatible(TypeRange inputs, TypeRange outputs) {
  if (inputs.size() != 1 || outputs.size() != 1)
    return false;
  Type a = inputs.front(), b = outputs.front();
  auto aT = toolchain::dyn_cast<TensorType>(a);
  auto bT = toolchain::dyn_cast<TensorType>(b);
  if (!aT || !bT)
    return false;
...
struct ChainedTensorCast : public OpRewritePattern<CastOp> {
  ...
  LogicalResult matchAndRewrite(CastOp tensorCast,
    ...
    auto sourceType =
        toolchain::cast<TensorType>(tensorCastOperand.getOperand().getType());
    auto intermediateType = toolchain::cast<TensorType>(tensorCastOperand.getType());
    auto resultType = toolchain::cast<TensorType>(tensorCast.getType());
...
struct TensorCastExtractSlice : public OpRewritePattern<CastOp> {
  ...
  LogicalResult matchAndRewrite(CastOp tensorCast,
    ...
    auto rankedResultType =
        toolchain::dyn_cast<RankedTensorType>(tensorCast.getType());
    if (!rankedResultType)
      return failure();

    if (!extractOperand || !canFoldIntoProducerOp(tensorCast) ||
        rankedResultType.getShape() ==
            toolchain::cast<RankedTensorType>(tensorCast.getSource().getType())
                .getShape())
      return failure();
...
RankedTensorType ConcatOp::inferResultType(int64_t dim, TypeRange inputTypes) {
  ...
  auto tensorTypes =
      toolchain::map_to_vector<4>(inputTypes, toolchain::CastTo<RankedTensorType>);
...
LogicalResult ConcatOp::verify() {
  ...
  if (toolchain::any_of(inputTypes, [resultRank](RankedTensorType type) {
        return type.getRank() != resultRank;
      }))
    return emitOpError("rank of concatenated inputs must match result rank");

  Type resultElementType = resultType.getElementType();
  if (toolchain::any_of(inputTypes, [&](RankedTensorType type) {
  ...
  for (auto [inferredSize, actualSize] :
       toolchain::zip_equal(inferredResultType.getShape(), resultType.getShape())) {
...
FailureOr<SmallVector<Value>> ConcatOp::decomposeOperation(OpBuilder &builder) {
  ...
  Location loc = getLoc();
  for (auto [index, input] : toolchain::enumerate(getInputs())) {
...
ConcatOp::reifyResultShapes(OpBuilder &builder,
  ...
  if (getType().isDynamicDim(dim)) {
    ...
    for (auto [idx, input] : toolchain::enumerate(inputs.drop_front())) {
...
struct InferConcatOperandTypes : public OpRewritePattern<ConcatOp> {
  ...
  LogicalResult matchAndRewrite(ConcatOp concatOp,
    ...
    for (auto [operandIdx, operandType] :
         toolchain::enumerate(concatOp->getOperandTypes())) {
...
OpFoldResult DimOp::fold(FoldAdaptor adaptor) {
  ...
  auto index = toolchain::dyn_cast_if_present<IntegerAttr>(adaptor.getIndex());
  if (!index)
    return {};

  // Folding for unranked types (UnrankedTensorType) is not supported.
  auto tensorType = toolchain::dyn_cast<RankedTensorType>(getSource().getType());
  if (!tensorType)
    return {};
  ...
  if (auto fromElements = dyn_cast_or_null<tensor::GenerateOp>(definingOp)) {
    auto resultType =
        toolchain::cast<RankedTensorType>(fromElements.getResult().getType());
...
struct FoldEmptyTensorWithCastOp : public OpRewritePattern<CastOp> {
  ...
  LogicalResult matchAndRewrite(CastOp castOp,
    ...
    auto resultType =
        toolchain::cast<RankedTensorType>(castOp->getResult(0).getType());
    ...
    for (auto it : toolchain::zip(resultShape, currMixedSizes)) {
      ...
      // result dim matches.
      if (auto attr = toolchain::dyn_cast_if_present<Attribute>(currDim)) {
        if (ShapedType::isDynamic(newDim) ||
            newDim != toolchain::cast<IntegerAttr>(attr).getInt()) {
...
struct ExtractFromTensorCast : public OpRewritePattern<tensor::ExtractOp> {
  ...
  LogicalResult matchAndRewrite(tensor::ExtractOp extract,
    ...
    if (!tensorCast)
      return failure();
    if (!toolchain::isa<RankedTensorType>(tensorCast.getSource().getType()))
      return failure();
...
struct ExtractFromCollapseShape : public OpRewritePattern<tensor::ExtractOp> {
  ...
  LogicalResult matchAndRewrite(tensor::ExtractOp extractOp,
    ...
    for (auto [index, group] :
         toolchain::zip(indices, collapseOp.getReassociationIndices())) {
      ...
      SmallVector<int64_t> basis =
          toolchain::map_to_vector(group, [&](int64_t d) { return sourceSizes[d]; });
      auto delinearize = affine::AffineDelinearizeIndexOp::create(
          rewriter, extractOp.getLoc(), index, basis, /*hasOuterBound=*/true);
      toolchain::append_range(sourceIndices, delinearize.getResults());
    }
...
LogicalResult ExtractOp::verify() {
  ...
  auto tensorType = toolchain::cast<RankedTensorType>(getTensor().getType());
...
static Value foldExtractAfterInsert(ExtractOp extractOp) {
  ...
  if (insertOp && insertOp.getScalar().getType() == extractOp.getType() &&
      toolchain::equal(insertOp.getIndices(), extractOp.getIndices(), isSame))
...
OpFoldResult ExtractOp::fold(FoldAdaptor adaptor) {
  if (Attribute tensor = adaptor.getTensor()) {
    ...
    if (auto splatTensor = toolchain::dyn_cast<SplatElementsAttr>(tensor))
  ...
  for (Attribute indice : adaptor.getIndices()) {
    if (!indice || !toolchain::isa<IntegerAttr>(indice))
      return {};
    indices.push_back(toolchain::cast<IntegerAttr>(indice).getInt());
  }

  // Fold extract(from_elements(...)).
  if (auto fromElementsOp = getTensor().getDefiningOp<FromElementsOp>()) {
    auto tensorType = toolchain::cast<RankedTensorType>(fromElementsOp.getType());
  ...
  if (Attribute tensor = adaptor.getTensor()) {
    auto elementsAttr = toolchain::dyn_cast<ElementsAttr>(tensor);
...
OpFoldResult FromElementsOp::fold(FoldAdaptor adaptor) {
  if (!toolchain::is_contained(adaptor.getElements(), nullptr))
...
RankedTensorType GatherOp::inferResultType(RankedTensorType sourceType,
  ...
  for (int64_t idx : toolchain::seq<int64_t>(0, sourceType.getRank())) {
    if (toolchain::binary_search(gatherDims, idx)) {
      if (!rankReduced)
...
OpFoldResult GatherOp::fold(FoldAdaptor adaptor) {
  if (OpFoldResult reshapedSource = reshapeConstantSource(
          toolchain::dyn_cast_if_present<DenseElementsAttr>(adaptor.getSource()),
          getResult().getType()))
...
LogicalResult InsertOp::verify() {
  // Verify the # indices match if we have a ranked type.
  auto destType = toolchain::cast<RankedTensorType>(getDest().getType());
...
OpFoldResult InsertOp::fold(FoldAdaptor adaptor) {
  ...
  if (scalar && dest)
    if (auto splatDest = toolchain::dyn_cast<SplatElementsAttr>(dest))
...
LogicalResult GenerateOp::reifyResultShapes(
  ...
  reifiedReturnShapes.resize(1, SmallVector<OpFoldResult>(getType().getRank()));
  int idx = 0;
  for (auto dim : toolchain::seq<int64_t>(0, getType().getRank())) {
    if (getType().isDynamicDim(dim)) {
...
LogicalResult GenerateOp::verify() {
  ...
  RankedTensorType resultType = toolchain::cast<RankedTensorType>(getType());
...
LogicalResult GenerateOp::verifyRegions() {
  RankedTensorType resultTy = toolchain::cast<RankedTensorType>(getType());
  // Ensure that region arguments span the index space.
  if (!toolchain::all_of(getBody().getArgumentTypes(),
...
void GenerateOp::build(
  ...
  auto rank = toolchain::cast<RankedTensorType>(resultTy).getRank();
...
OpFoldResult RankOp::fold(FoldAdaptor adaptor) {
  ...
  auto shapedType = toolchain::dyn_cast<ShapedType>(type);
...
LogicalResult ReshapeOp::verify() {
  TensorType operandType = toolchain::cast<TensorType>(getSource().getType());
  TensorType resultType = toolchain::cast<TensorType>(getResult().getType());
  ...
  int64_t shapeSize =
      toolchain::cast<RankedTensorType>(getShape().getType()).getDimSize(0);
  auto resultRankedType = toolchain::dyn_cast<RankedTensorType>(resultType);
  auto operandRankedType = toolchain::dyn_cast<RankedTensorType>(operandType);
...
int64_t ExpandShapeOp::getCorrespondingSourceDim(int64_t resultDim) {
  assert(resultDim >= 0 && resultDim < getResultType().getRank() &&
         "invalid resultDim");
  for (const auto &it : toolchain::enumerate(getReassociationIndices()))
    if (toolchain::is_contained(it.value(), resultDim))
      return it.index();
...
CollapseShapeOp::inferCollapsedType(RankedTensorType type,
  ...
  for (AffineMap m : reassociation) {
    ...
    int64_t size = 1;
    if (toolchain::is_contained(band, ShapedType::kDynamic))
...
void CollapseShapeOp::build(OpBuilder &b, OperationState &result, Value src,
  ...
  auto srcType = toolchain::cast<RankedTensorType>(src.getType());
...
LogicalResult ExpandShapeOp::verify() {
  ...
  if ((int64_t)getOutputShape().size() !=
      toolchain::count(getStaticOutputShape(), ShapedType::kDynamic))
...
LogicalResult CollapseShapeOp::verify() {
  ...
  if (toolchain::any_of(op.getReassociationIndices(),
...
struct FoldReshapeWithFromElements : OpRewritePattern<TensorReshapeOp> {
  ...
  LogicalResult matchAndRewrite(TensorReshapeOp reshapeOp,
    ...
    auto shapedTy = toolchain::cast<ShapedType>(reshapeOp.getType());
...
struct FoldCollapseOfCastOp : public OpRewritePattern<CollapseShapeOp> {
  ...
  LogicalResult matchAndRewrite(CollapseShapeOp collapseShapeOp,
    ...
    RankedTensorType srcType =
        toolchain::cast<RankedTensorType>(castOp.getSource().getType());
...
struct ConvertToStaticExpandShape : public OpRewritePattern<ExpandShapeOp> {
  ...
  LogicalResult matchAndRewrite(ExpandShapeOp expandOp,
    ...
    for (const auto &[inputDim, innerReassoc] : toolchain::enumerate(reassoc)) {
    ...
    for (auto inDim : toolchain::seq<int>(0, newInputShape.size())) {
...
void ExtractSliceOp::build(OpBuilder &b, OperationState &result,
  ...
  auto sourceRankedTensorType = toolchain::cast<RankedTensorType>(source.getType());
  // Structuring implementation this way avoids duplication between builders.
  if (!resultType) {
    resultType = toolchain::cast<RankedTensorType>(
...
void ExtractSliceOp::build(OpBuilder &b, OperationState &result,
  ...
  SmallVector<OpFoldResult> offsetValues = toolchain::to_vector<4>(
      toolchain::map_range(offsets, [](Value v) -> OpFoldResult { return v; }));
  SmallVector<OpFoldResult> sizeValues = toolchain::to_vector<4>(
      toolchain::map_range(sizes, [](Value v) -> OpFoldResult { return v; }));
  SmallVector<OpFoldResult> strideValues = toolchain::to_vector<4>(
      toolchain::map_range(strides, [](Value v) -> OpFoldResult { return v; }));
...
toolchain::SmallBitVector ExtractSliceOp::getDroppedDims() {
...
ExtractSliceOp::rankReduceIfNeeded(OpBuilder &b, Location loc, Value value,
  ...
  auto sourceTensorType = toolchain::dyn_cast<RankedTensorType>(value.getType());
...
LogicalResult ExtractSliceOp::reifyResultShapes(
  ...
  toolchain::SmallBitVector droppedDims = getDroppedDims();
...
public:
  ...
  LogicalResult matchAndRewrite(ExtractSliceOp sliceOp,
    ...
    if (toolchain::any_of(sliceOp.getOperands(), [](Value operand) {
...
static void sliceElements(IterTy values, ArrayRef<int64_t> counts,
                          ...
                          toolchain::SmallVectorImpl<ElemTy> *outValues) {
...
public:
  ...
  LogicalResult matchAndRewrite(ExtractSliceOp op,
    ...
    auto sourceType = toolchain::cast<ShapedType>(op.getSource().getType());
    auto resultType = toolchain::cast<ShapedType>(op.getResult().getType());
    ...
    if (toolchain::is_contained(offsets, ShapedType::kDynamic))
      return failure();
    auto sizes = op.getStaticSizes();
    if (toolchain::is_contained(sizes, ShapedType::kDynamic))
      return failure();
    auto strides = op.getStaticStrides();
    if (toolchain::is_contained(strides, ShapedType::kDynamic))
      return failure();
    ...
    if (auto elems = toolchain::dyn_cast<DenseIntElementsAttr>(attr)) {
    ...
    } else if (auto elems = toolchain::dyn_cast<DenseFPElementsAttr>(attr)) {
...
foldIdentityOffsetSizeAndStrideOpInterface(OffsetSizeAndStrideOpInterface op,
  ...
  // toolchain::zip is appropriate.
  auto shape = shapedType.getShape();
  for (auto it : toolchain::zip(op.getMixedSizes(), shape))
...
Value mlir::tensor::createCanonicalRankReducingExtractSliceOp(
  ...
  auto rankedTensorType = toolchain::cast<RankedTensorType>(tensor.getType());
...
OpFoldResult InsertSliceOp::fold(FoldAdaptor) {
  ...
  if (auto result = foldInsertAfterExtractSlice(*this))
    return result;
  if (toolchain::any_of(getMixedSizes(), isZeroInteger))
    return getDest();
  return OpFoldResult();
}
...
struct InsertSliceOpCastFolder final : public OpRewritePattern<InsertOpTy> {
  ...
  LogicalResult matchAndRewrite(InsertOpTy insertSliceOp,
                                PatternRewriter &rewriter) const override {
    if (toolchain::any_of(insertSliceOp.getOperands(), [](Value operand) {
          return matchPattern(operand, matchConstantIndex());
        }))
      return failure();
    ...
    auto srcType = toolchain::dyn_cast<RankedTensorType>(src.getType());
    auto dstType = toolchain::dyn_cast<RankedTensorType>(dst.getType());
    if (!srcType || !dstType)
      return failure();
...
toolchain::SmallBitVector InsertSliceOp::getDroppedDims() {
...
Value mlir::tensor::createCanonicalRankReducingInsertSliceOp(OpBuilder &b,
                                                             ...
                                                             Value tensor,
                                                             Value dest) {
  auto rankedTensorType = toolchain::cast<RankedTensorType>(dest.getType());
  unsigned rank = rankedTensorType.getRank();
  SmallVector<OpFoldResult> offsets(rank, b.getIndexAttr(0));
...
LogicalResult PadOp::verify() {
  auto sourceType = toolchain::cast<RankedTensorType>(getSource().getType());
  auto resultType = toolchain::cast<RankedTensorType>(getResult().getType());
  auto expectedType =
...
LogicalResult PadOp::verifyRegions() {
  ...
  unsigned rank = toolchain::cast<RankedTensorType>(getResult().getType()).getRank();
  Block &block = region.front();
  if (block.getNumArguments() != rank)
    return emitError("expected the block to have ") << rank << " arguments";

  // Note: the number and type of yield values are checked in the YieldOp.
  for (const auto &en : toolchain::enumerate(block.getArgumentTypes())) {
    if (!en.value().isIndex())
  ...
  // Ensure that the region yields an element of the right type.
  auto yieldOp = toolchain::cast<YieldOp>(block.getTerminator());
  if (yieldOp.getValue().getType() !=
      toolchain::cast<ShapedType>(getType()).getElementType())
...
RankedTensorType PadOp::inferResultType(RankedTensorType sourceType,
  ...
  for (auto i : toolchain::seq<unsigned>(0, rank)) {
...
void PadOp::build(OpBuilder &b, OperationState &result, Type resultType,
  ...
  auto sourceType = toolchain::cast<RankedTensorType>(source.getType());
  if (!resultType)
...
void PadOp::build(OpBuilder &b, OperationState &result, Type resultType,
  ...
  assert(toolchain::isa<RankedTensorType>(resultType));
  result.addAttributes(attrs);
...
void PadOp::build(OpBuilder &b, OperationState &result, Type resultType,
  ...
  int sourceRank = toolchain::cast<RankedTensorType>(source.getType()).getRank();
...
toolchain::SmallBitVector PadOp::getPaddedDims() {
  toolchain::SmallBitVector paddedDims(getSourceType().getRank());
...
struct FoldOrthogonalPaddings : public OpRewritePattern<PadOp> {
  ...
  LogicalResult matchAndRewrite(PadOp padOp,
    ...
    toolchain::SmallBitVector innerDims = padOp.getPaddedDims();
    toolchain::SmallBitVector outerDims = outerPadOp.getPaddedDims();
...
struct FoldStaticPadding : public OpRewritePattern<PadOp> {
  ...
  LogicalResult matchAndRewrite(PadOp padTensorOp,
    ...
    if (!toolchain::isa<RankedTensorType>(input.getType()))
      return failure();
    auto inputDims = toolchain::cast<RankedTensorType>(input.getType()).getShape();
  ...
  LogicalResult matchAndRewrite(tensor::PadOp padOp,
    ...
    auto addPaddings = [&](ArrayRef<OpFoldResult> consumerPaddings,
      ...
      for (auto [consumerIndex, producerIndex] :
           toolchain::zip_equal(consumerPaddings, producerPaddings)) {
...
OpResult ParallelInsertSliceOp::getTiedOpResult() {
  ...
  for (const auto &it :
       toolchain::enumerate(parallelCombiningParent.getYieldingOps())) {
...
toolchain::SmallBitVector ParallelInsertSliceOp::getDroppedDims() {
  return ::getDroppedDims(getSourceType().getShape(), getMixedSizes());
}
...
struct FoldTensorCastProducerOp
  ...
  LogicalResult matchAndRewrite(DestinationStyleOpInterface op,
    ...
    for (auto [oldResult, newResult] :
         toolchain::zip(op->getResults(), newOp->getResults())) {
