  ...
  void runOnOperation() override {
    ...
    getOperation()->walk([&](omp::TargetOp targetOp) {
      ...
      for (auto [privVarIdx, privVarSymPair] :
        ...
        DominanceInfo dom;
        toolchain::sort(chainOfOps, [&](Operation *l, Operation *r) {
          if (l == r)
            return false;
        ...
        // Create a toolchain.func for 'region' that is marked always_inline and call
        // it.
        auto createAlwaysInlineFuncAndCallIt =
            [&](Region &region, toolchain::StringRef funcName,
                toolchain::ArrayRef<Value> args, bool returnsValue) -> Value {
        ...
        if (!privatizer.getInitRegion().empty())
          initializedVal = createAlwaysInlineFuncAndCallIt(
              ...
              toolchain::formatv("{0}_{1}", privatizer.getSymName(), "init").str(),
        ...
        if (isFirstPrivate && !privatizer.getCopyRegion().empty())
          initializedVal = createAlwaysInlineFuncAndCallIt(
              ...
              toolchain::formatv("{0}_{1}", privatizer.getSymName(), "copy").str(),
        ...
        if (needsCleanupTask) {
          ...
          (void)createAlwaysInlineFuncAndCallIt(
              ...
              toolchain::formatv("{0}_{1}", privatizer.getSymName(), "dealloc")
                  .str(),
              {initializedVal}, /*returnsValue=*/false);
          toolchain::FailureOr<LLVM::LLVMFuncOp> freeFunc =
              LLVM::lookupOrCreateFreeFn(rewriter, mod);
          assert(toolchain::succeeded(freeFunc) &&
      ...
      if (fakeDependVar) {
        ...
        if (!targetOp.getDependVars().empty()) {
          ...
          toolchain::copy(*dependKinds, std::back_inserter(newDependKinds));
        }
...
private:
  ...
  LLVM::LLVMFuncOp createFuncOpForRegion(Location loc, ModuleOp mod,
    ...
    toolchain::copy(srcRegion.getArgumentTypes(), std::back_inserter(paramTypes));
