...
#include "vm/core/ADT/STLForwardCompat.h"
...
static ArrayAttr makeArrayAttr(MLIRContext *context,
                               toolchain::ArrayRef<Attribute> attrs) {
...
struct MemRefPointerLikeModel
  ...
  Type getElementType(Type pointer) const {
    return toolchain::cast<MemRefType>(pointer).getElementType();
  }
};
...
static std::string generateLoopNestingName(StringRef prefix,
  ...
  while (o) {
    ...
    toolchain::ReversePostOrderTraversal<Block *> traversal(&r->getBlocks().front());
    size_t idx = 0;
    bool found = false;
    ...
    if (parent->getRegions().size() > 1) {
      auto getRegionIndex = [](Operation *o, Region *r) {
        for (auto [idx, region] : toolchain::enumerate(o->getRegions())) {
          if (&region == r)
            return idx;
        }
  ...
  for (Component &c : toolchain::reverse(components)) {
    if (c.skip)
      continue;
  ...
  for (auto &c : toolchain::reverse(components)) {
    if (c.skip)
      continue;
...
static LogicalResult verifyAlignedClause(Operation *op,
  ...
  for (unsigned i = 0; i < (*alignments).size(); ++i) {
    if (auto intAttr = toolchain::dyn_cast<IntegerAttr>((*alignments)[i])) {
...
struct PrivateParseArgs {
  toolchain::SmallVectorImpl<OpAsmParser::UnresolvedOperand> &vars;
...
static ParseResult parseClauseWithRegionArgs(
  ...
  for (auto [prv, type] : toolchain::zip_equal(argsSubrange, types)) {
    prv.type = type;
  }
...
static ParseResult parseBlockArgClause(
    OpAsmParser &parser,
    toolchain::SmallVectorImpl<OpAsmParser::Argument> &entryBlockArgs,
...
static ParseResult parseBlockArgRegion(OpAsmParser &parser, Region &region,
  ...
  toolchain::SmallVector<OpAsmParser::Argument> entryBlockArgs;
...
static ParseResult parseTargetOpRegion(
    ...
    toolchain::SmallVectorImpl<OpAsmParser::UnresolvedOperand> &privateVars,
    toolchain::SmallVectorImpl<Type> &privateTypes, ArrayAttr &privateSyms,
...
static void printClauseWithRegionArgs(
  ...
  if (!symbols) {
    toolchain::SmallVector<Attribute> values(operands.size(), nullptr);
    symbols = ArrayAttr::get(ctx, values);
  }

  if (!mapIndices) {
    toolchain::SmallVector<int64_t> values(operands.size(), -1);
  ...
  toolchain::interleaveComma(toolchain::zip_equal(operands, argsSubrange, symbols,
  ...
  p << " : ";
  toolchain::interleaveComma(types, p);
  p << ") ";
...
static void printBlockArgRegion(OpAsmPrinter &p, Operation *op, Region &region,
  ...
  auto iface = toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(op);
...
verifyReductionVarList(Operation *op, std::optional<ArrayAttr> reductionSyms,
  ...
  for (auto args : toolchain::zip(reductionVars, *reductionSyms)) {
    ...
    auto symbolRef = toolchain::cast<SymbolRefAttr>(std::get<1>(args));
    auto decl =
...
static void printCopyprivate(OpAsmPrinter &p, Operation *op,
  ...
  toolchain::interleaveComma(
      toolchain::zip(copyprivateVars, *copyprivateSyms, copyprivateTypes), p,
      [&](const auto &args) {
...
verifyCopyprivateVarList(Operation *op, OperandRange copyprivateVars,
  ...
  for (auto copyprivateVarAndSym :
       toolchain::zip(copyprivateVars, *copyprivateSyms)) {
    auto symbolRef =
        toolchain::cast<SymbolRefAttr>(std::get<1>(copyprivateVarAndSym));
...
static void printDependVarList(OpAsmPrinter &p, Operation *op,
  ...
  for (unsigned i = 0, e = dependKinds->size(); i < e; ++i) {
    ...
    p << stringifyClauseTaskDepend(
             toolchain::cast<mlir::omp::ClauseTaskDependAttr>((*dependKinds)[i])
                 .getValue())
...
static void printSynchronizationHint(OpAsmPrinter &p, Operation *op,
  ...
  toolchain::interleaveComma(hints, p);
}
...
static void printMapClause(OpAsmPrinter &p, Operation *op,
                           ClauseMapFlagsAttr mapType) {
  toolchain::SmallVector<std::string, 4> mapTypeStrs;
...
static void printMembersIndex(OpAsmPrinter &p, MapInfoOp op,
  ...
  toolchain::interleaveComma(membersIdx, p, [&p](Attribute v) {
    p << "[";
    auto memberIdx = cast<ArrayAttr>(v);
    toolchain::interleaveComma(memberIdx.getValue(), p, [&p](Attribute v2) {
...
static void printCaptureType(OpAsmPrinter &p, Operation *op,
  ...
  toolchain::raw_string_ostream typeCap(typeCapStr);
...
static LogicalResult verifyMapClause(Operation *op, OperandRange mapVars) {
  toolchain::DenseSet<mlir::TypedValue<mlir::omp::PointerLikeType>> updateToVars;
  toolchain::DenseSet<mlir::TypedValue<mlir::omp::PointerLikeType>> updateFromVars;
...
static LogicalResult verifyMapInfoDefinedArgs(Operation *op,
                                              ...
                                              OperandRange vars) {
  for (Value var : vars)
    if (!toolchain::isa_and_present<MapInfoOp>(var.getDefiningOp()))
      return op->emitOpError()
             << "'" << clauseName
  ...
  for (Value hostEvalArg :
    ...
    for (Operation *user : hostEvalArg.getUsers()) {
      if (auto teamsOp = dyn_cast<TeamsOp>(user)) {
        if (toolchain::is_contained({teamsOp.getNumTeamsLower(),
      ...
      if (auto loopNestOp = dyn_cast<LoopNestOp>(user)) {
        if (bitEnumContainsAny(execFlags, TargetRegionFlags::trip_count) &&
            ...
            (toolchain::is_contained(loopNestOp.getLoopLowerBounds(), hostEvalArg) ||
             toolchain::is_contained(loopNestOp.getLoopUpperBounds(), hostEvalArg) ||
             toolchain::is_contained(loopNestOp.getLoopSteps(), hostEvalArg)))
          continue;
...
findCapturedOmpOp(Operation *rootOp, bool checkSingleMandatoryExec,
                  toolchain::function_ref<bool(Operation *)> siblingAllowedFn) {
  ...
  rootOp->walk<WalkOrder::PreOrder>([&](Operation *op) {
    ...
    capturedOp = op;
    return toolchain::isa<LoopNestOp>(op) ? WalkResult::interrupt()
                                     : WalkResult::advance();
  });
...
static LogicalResult verifyPrivateVarList(OpType &op) {
  ...
  for (auto privateVarInfo : toolchain::zip_equal(privateVars, privateSyms)) {
...
LogicalResult LoopOp::verifyRegions() {
  if (toolchain::isa_and_nonnull<LoopWrapperInterface>((*this)->getParentOp()) ||
      getNestedWrapper())
    return emitOpError() << "expected to be a standalone loop wrapper";
...
LogicalResult WsloopOp::verifyRegions() {
  bool isCompositeChildLeaf =
      toolchain::dyn_cast_if_present<LoopWrapperInterface>((*this)->getParentOp());
...
LogicalResult DistributeOp::verifyRegions() {
  if (LoopWrapperInterface nested = getNestedWrapper()) {
    ...
    if (isa<WsloopOp>(nested)) {
      ...
      if (!toolchain::dyn_cast_if_present<ParallelOp>(parentOp) ||
...
LogicalResult DeclareMapperOp::verifyRegions() {
  if (!toolchain::isa_and_present<DeclareMapperInfoOp>(
...
LogicalResult DeclareReductionOp::verifyRegions() {
  ...
  if (!getAtomicReductionRegion().empty()) {
    ...
    auto ptrType = toolchain::dyn_cast<PointerLikeType>(
        atomicReductionEntryBlock.getArgumentTypes()[0]);
    if (!ptrType ||
...
LogicalResult TaskloopOp::verify() {
  ...
  for (auto var : getReductionVars()) {
    if (toolchain::is_contained(getInReductionVars(), var))
...
LogicalResult LoopNestOp::verify() {
  ...
  for (auto [lb, iv] : toolchain::zip_equal(getLoopLowerBounds(), getIVs())) {
  ...
  if (!toolchain::dyn_cast_if_present<LoopWrapperInterface>((*this)->getParentOp()))
...
void LoopNestOp::gatherWrappers(
  ...
  while (auto wrapper =
             toolchain::dyn_cast_if_present<LoopWrapperInterface>(parent)) {
...
static void printLoopTransformClis(OpAsmPrinter &p, TileOp op,
  ...
  if (!generatees.empty())
    p << '(' << toolchain::interleaved(generatees) << ')';

  if (!applyees.empty())
    p << " <- (" << toolchain::interleaved(applyees) << ')';
}
...
LogicalResult TileOp::verify() {
  ...
  for (auto &&applyee : toolchain::drop_begin(getApplyees())) {
...
LogicalResult PrivateClauseOp::verifyRegions() {
  ...
  auto verifyTerminator = [&](Operation *terminator,
    ...
    if (!toolchain::isa<YieldOp>(terminator))
...
LogicalResult AllocateDirOp::verify() {
  ...
  if (align.has_value()) {
    if ((align.value() > 0) && !toolchain::has_single_bit(align.value()))
...
static mlir::ParseResult parseTargetAllocMemOp(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<std::int32_t> segmentSizes{1, typeparamsSize, shapeSize};
...
toolchain::LogicalResult omp::TargetAllocMemOp::verify() {
...
LogicalResult WorkdistributeOp::verify() {
  ...
  Operation *parentOp = (*this)->getParentOp();
  if (!toolchain::dyn_cast<TeamsOp>(parentOp))
