...
static std::optional<T> findProducerOfType(Value val) {
  ...
  if (!currentValue.getDefiningOp()) {
    ...
    int64_t iterArgIdx;
    if (auto iterArg = toolchain::dyn_cast<BlockArgument>(currentValue)) {
...
transform::GetDescOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (!toolchain::hasSingleElement(targetValues)) {
    return emitDefiniteFailure()
           << "requires exactly one target value handle (got "
           << toolchain::range_size(targetValues) << ")";
  }
  ...
  results.set(toolchain::cast<OpResult>(getResult()), {*maybeDescOp});
  return DiagnosedSilenceableFailure::success();
}
...
void transform::SetDescLayoutOp::getEffects(
    ::toolchain::SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
...
transform::InsertPrefetchOp::apply(transform::TransformRewriter &rewriter,
  ...
  auto getPrefetchOffsets =
    ...
    SmallVector<Value> dynamicOffsets =
        toolchain::to_vector(toolchain::map_range(loadOp.getOffsets(), [&](Value v) {
  ...
  results.set(toolchain::cast<OpResult>(getResult()), {newDescOp});
...
transform::ConvertLayoutOp::apply(transform::TransformRewriter &rewriter,
  ...
  results.set(toolchain::cast<OpResult>(getResult()), {convLayoutOp});
  return DiagnosedSilenceableFailure::success();
}
