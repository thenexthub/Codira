...
SmallVector<Value> computeUnrolledOffsets(
  ...
  auto addi = [&](OpFoldResult a, int64_t b) -> Value {
    ...
    if (maybeInt) {
      ...
      auto aV = toolchain::cast<Value>(a);
      auto bV = arith::ConstantIndexOp::create(rewriter, loc, b);
      return rewriter.createOrFold<arith::AddIOp>(loc, aV, bV);
    }
  };

  SmallVector<OpFoldResult> oldOffsets = toolchain::to_vector(
      toolchain::drop_begin(mixedOffsets, mixedOffsets.size() - rank));
  auto validIdxes =
      toolchain::seq<int64_t>(mixedOffsets.size() - rank, mixedOffsets.size());
  ...
  for (SmallVector<int64_t> offsets :
    ...
    for (auto [idx, oldOff, offset] :
         toolchain::zip(validIdxes, oldOffsets, offsets))
...
struct UnrollStoreNdOp : public UnrollPattern<xegpu::StoreNdOp> {
  ...
  LogicalResult matchAndRewrite(xegpu::StoreNdOp op,
    ...
    if (!hasOffsets) {
      for (auto [v, t] : toolchain::zip(convertedValues, convertedTdescs))
...
struct UnrollDpasOp : public UnrollPattern<xegpu::DpasOp> {
  ...
  LogicalResult matchAndRewrite(xegpu::DpasOp op,
    ...
    if (toolchain::any_of(op->getOperandTypes(), [&](Type type) {
    ...
    if (toolchain::any_of(ranges, [](auto &v) { return v.size() == 0; }) ||
        toolchain::all_of(ranges, [](auto &v) { return v.size() == 1; }))
      return failure();
  ...
  LogicalResult matchAndRewrite(xegpu::CreateDescOp op,
    ...
    if (originalChunkSize > 1) {
      ...
      for (auto [indice, indiceType] :
           toolchain::zip(convertedIndiceVec, convertedIndiceTypes)) {
...
struct UnrollLoadGatherOp : public UnrollPattern<xegpu::LoadGatherOp> {
  ...
  LogicalResult matchAndRewrite(xegpu::LoadGatherOp op,
    ...
    Location loc = op.getLoc();
    VectorType valueTy = toolchain::dyn_cast<VectorType>(op.getValue().getType());
    xegpu::TensorDescType tdescTy = op.getTensorDescType();
    ...
    VectorType maskTy = toolchain::dyn_cast<VectorType>(op.getMask().getType());
    ...
    SmallVector<Value> newOps;
    for (auto [t, m] : toolchain::zip(convertedTdescs, convertedMasks)) {
...
struct UnrollLoadGatherOpWithOffset
  ...
  LogicalResult matchAndRewrite(xegpu::LoadGatherOp op,
                                PatternRewriter &rewriter) const override {
    Location loc = op.getLoc();
    VectorType valueTy = toolchain::dyn_cast<VectorType>(op.getType());
    ...
    if (auto chunkSizeAttr = op->getAttr("chunk_size")) {
      if (auto intAttr = toolchain::dyn_cast<IntegerAttr>(chunkSizeAttr))
        chunkSize = intAttr.getInt();
    }

    // Unroll mask and offsets with correct shape
    VectorType maskTy = toolchain::dyn_cast<VectorType>(mask.getType());
    VectorType offsetsTy = toolchain::dyn_cast<VectorType>(offsets.getType());
    ...
    if (chunkSize > 1) {
      ...
      for (auto [baseOffset, offsetType] :
           toolchain::zip(convertedOffsetsBase, convertedOffsetTypes)) {
        for (int64_t i = 0; i < numNewChunks; ++i) {
          Value inc = arith::ConstantIndexOp::create(rewriter, loc,
                                                     i * blockedChunkSize);
          Value incVec =
    ...
    SmallVector<Value> newOps;
    for (auto [o, m] : toolchain::zip(convertedOffsets, convertedMasks)) {
      auto newOp = xegpu::LoadGatherOp::create(
...
struct UnrollStoreScatterOpWithOffsets
  ...
  LogicalResult matchAndRewrite(xegpu::StoreScatterOp op,
    ...
    for (auto [v, o, m] :
         toolchain::zip(convertedValues, convertedOffsets, convertedMasks)) {
...
struct UnrollUpdateOffsetOp : public UnrollPattern<xegpu::UpdateOffsetOp> {
  ...
  LogicalResult matchAndRewrite(xegpu::UpdateOffsetOp op,
    ...
    for (auto [t, o] : toolchain::zip(convertedTdesc, convertedOffsetVec)) {
      auto newOp =
...
struct UnrollStoreMatrixOp : public UnrollPattern<xegpu::StoreMatrixOp> {
  ...
  LogicalResult matchAndRewrite(xegpu::StoreMatrixOp op,
    ...
    Location loc = op.getLoc();
    VectorType valueTy = toolchain::dyn_cast<VectorType>(op.getData().getType());
    assert(valueTy && "the value type must be vector type!");
    ArrayRef<int64_t> shape = valueTy.getShape();
    auto layout = dyn_cast<xegpu::LayoutAttr>(op.getLayoutAttr());
    ...
    for (auto [v, offsets] : toolchain::zip_equal(convertedValues, offsetsList))
