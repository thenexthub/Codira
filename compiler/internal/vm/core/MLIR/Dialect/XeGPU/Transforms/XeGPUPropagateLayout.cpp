...
SmallVector<int> LayoutInfo::getLaneLayout() const {
  if (!isAssigned())
    return {};
  return toolchain::map_to_vector(storage.getEffectiveLaneLayoutAsInt(),
...
SmallVector<int> LayoutInfo::getLaneData() const {
  if (!isAssigned())
    return {};
  return toolchain::map_to_vector(storage.getEffectiveLaneDataAsInt(),
                             [](int64_t val) { return static_cast<int>(val); });
}
...
SmallVector<int> LayoutInfo::getInstData() const {
  if (!isAssigned())
    return {};
  return toolchain::map_to_vector(storage.getEffectiveInstDataAsInt(),
                             [](int64_t val) { return static_cast<int>(val); });
}
...
SmallVector<int> LayoutInfo::getSgLayout() const {
  if (!isAssigned())
    return {};
  return toolchain::map_to_vector(storage.getEffectiveSgLayoutAsInt(),
                             [](int64_t val) { return static_cast<int>(val); });
}
...
SmallVector<int> LayoutInfo::getSgData() const {
  if (!isAssigned())
    return {};
  return toolchain::map_to_vector(storage.getEffectiveSgDataAsInt(),
                             [](int64_t val) { return static_cast<int>(val); });
}
...
SmallVector<int> LayoutInfo::getOrder() const {
  if (!isAssigned() || !storage.getOrder())
    return {};
  return toolchain::map_to_vector(storage.getOrder().asArrayRef(),
                             [](int64_t val) { return static_cast<int>(val); });
}
...
LayoutInfo LayoutInfo::transpose(ArrayRef<int64_t> permutation) const {
  ...
  toolchain::SmallSet<int64_t, 4> seen(permutation.begin(), permutation.end());
  bool hasDuplicates = seen.size() != permutation.size();
  bool withinRange = toolchain::all_of(permutation, [&](int64_t idx) {
      ...
      .Default([&](Operation *op) {
        for (const LayoutInfoLattice *resultInfo : results) {
          ...
          for (auto [operandInfo, operand] :
               toolchain::zip(operands, op->getOpOperands())) {
...
void LayoutInfoPropagation::visitVectorMultiReductionOp(
  ...
  VectorType resultTy = toolchain::dyn_cast<VectorType>(reduction.getDestType());
...
void LayoutInfoPropagation::visitVectorBitcastOp(
  ...
  if (toolchain::any_of(sourceLaneData, [](int64_t d) { return d != 1; })) {
...
public:
  ...
  void printAnalysisResult(toolchain::raw_ostream &os);
...
void RunLayoutInfoPropagation::printAnalysisResult(toolchain::raw_ostream &os) {
  auto printFunctionResult = [&](FunctionOpInterface funcOp) {
    ...
    funcOp.walk([&](Operation *op) {
      ...
      for (auto [i, r] : toolchain::enumerate(op->getResults())) {
