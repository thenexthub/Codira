...
getDistVecTypeBasedOnLaneLayout(xegpu::DistributeLayoutAttr layout,
  ...
  for (auto [i, dim] : toolchain::enumerate(originalType.getShape())) {
...
struct MoveFuncBodyToWarpOp : public OpRewritePattern<gpu::GPUFuncOp> {
  ...
  LogicalResult matchAndRewrite(gpu::GPUFuncOp gpuFuncOp,
    ...
    if (toolchain::all_of(gpuFuncOp.getBody().getOps(), [](Operation &op) {
          return isa<gpu::ReturnOp>(op) && !op.getNumOperands();
        }))
      return failure();
    // If the function already moved inside a warp_execute_on_lane0, skip.
    if (toolchain::any_of(gpuFuncOp.getBody().getOps(), [](Operation &op) {
    ...
    SmallVector<Type> workgroupAttributionsTypes =
        toolchain::map_to_vector(gpuFuncOp.getWorkgroupAttributions(),
                            [](BlockArgument arg) { return arg.getType(); });
    SmallVector<Type> privateAttributionsTypes =
        toolchain::map_to_vector(gpuFuncOp.getPrivateAttributions(),
                            [](BlockArgument arg) { return arg.getType(); });
...
struct CreateNdDescDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<xegpu::CreateNdDescOp>);
    if (!operand)
      return rewriter.notifyMatchFailure(
    ...
    SmallVector<Value> newDescOperands = toolchain::map_to_vector(
...
struct StoreNdDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    SmallVector<Type> offsetTypes = toolchain::to_vector(
        toolchain::map_range(offsetsAsValues, [](Value v) { return v.getType(); }));
...
struct DpasDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand = getWarpResult(warpOp, toolchain::IsaPred<xegpu::DpasOp>);
    if (!operand)
    ...
    toolchain::SmallVector<Value, 3> newYieldValues{dpasOp.getLhs(),
                                               dpasOp.getRhs()};
    toolchain::SmallVector<Type, 3> newYieldTypes{
...
struct StoreDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    SmallVector<Value> newStoreScatterOpOperands = toolchain::map_to_vector(
...
static SmallVector<Value> computeDistributedCoordinatesForMatrixOp(
  ...
  newCoods = toolchain::map_to_vector(ofrVec, toolchain::CastTo<Value>);
  return newCoods;
}
...
struct LoadMatrixDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    SmallVector<Type> operandTypes = toolchain::to_vector(
        toolchain::map_range(operands, [](Value v) { return v.getType(); }));

    SmallVector<size_t> newRetIndices;
    gpu::WarpExecuteOnLane0Op newWarpOp = moveRegionToNewWarpOpAndAppendReturns(
        rewriter, warpOp, operands, operandTypes, newRetIndices);
    SmallVector<Value> newOperands = toolchain::map_to_vector(
        newRetIndices, [&](size_t idx) { return newWarpOp.getResult(idx); });
...
struct LoadDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    SmallVector<Value> newLoadGatherOperands = toolchain::map_to_vector(
        newRetIndices, [&](size_t idx) { return newWarpOp.getResult(idx); });
...
struct VectorMultiReductionDistribution : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    OpOperand *yieldOperand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::MultiDimReductionOp>);
    if (!yieldOperand)
      return failure();
    auto reductionOp =
...
struct VectorExtractStridedSliceDistribution
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    SmallVector<Attribute> updatedSizes = toolchain::map_to_vector(
        extractOp.getSizes(), [](Attribute attr) { return attr; });
    SmallVector<Attribute> updatedOffsets = toolchain::map_to_vector(
        extractOp.getOffsets(), [](Attribute attr) { return attr; });
    SmallVector<Attribute> updatedStrides = toolchain::map_to_vector(
    ...
    if (distributedDims.size() > 0) {
      ...
      if (!toolchain::all_of(sourceLaneData, [](int64_t v) { return v == 1; }))
        return rewriter.notifyMatchFailure(
...
struct VectorInsertStridedSliceDistribution
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    if (destDistributedDims.size() > 0) {
      ...
      auto sourceLaneData = sourceLayout.getEffectiveLaneDataAsInt();
      if (!toolchain::all_of(destLaneData, [](int64_t v) { return v == 1; }) ||
          !toolchain::all_of(sourceLaneData, [](int64_t v) { return v == 1; }))
        return rewriter.notifyMatchFailure(
...
struct MemrefExtractAlignedPointerAsIndexDistribution final
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
    ...
    OpOperand *operand = getWarpResult(
        warpOp, toolchain::IsaPred<memref::ExtractAlignedPointerAsIndexOp>);
    if (!operand)
      return rewriter.notifyMatchFailure(
          warpOp,
...
struct VectorBitcastDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::BitCastOp>);
    if (!operand)
      return rewriter.notifyMatchFailure(
...
struct VectorTransposeDistribution final : public gpu::WarpDistributionPattern {
  ...
  LogicalResult matchAndRewrite(gpu::WarpExecuteOnLane0Op warpOp,
                                PatternRewriter &rewriter) const override {
    OpOperand *operand =
        getWarpResult(warpOp, toolchain::IsaPred<vector::TransposeOp>);
    if (!operand)
      return rewriter.notifyMatchFailure(
...
void XeGPUSubgroupDistributePass::runOnOperation() {
  ...
  auto distributionFn = [](Value val) {
    ...
    for (auto [i, v] : toolchain::enumerate(layout.getEffectiveLaneLayoutAsInt())) {
