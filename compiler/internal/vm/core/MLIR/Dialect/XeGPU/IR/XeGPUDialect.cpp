...
genCoordinates(OpBuilder &builder, Location loc,
  ...
  SmallVector<int64_t> distUnitShape = toolchain::map_to_vector(
      toolchain::zip_equal(srcShape,
  ...
  SmallVector<Value> distUnitLocalOffset = toolchain::map_to_vector(
      toolchain::zip(delinearizedId, subShape), [&](const auto &t) -> Value {
  ...
  for (SmallVector<int64_t> unitOffs :
    ...
    SmallVector<Value> base =
        toolchain::map_to_vector(unitOffs, [&](int64_t d) -> Value {
          return arith::ConstantIndexOp::create(builder, loc, d);
        });
    // Calculate `subShape` offset within `srcShape`.
    SmallVector<Value> adds =
        toolchain::map_to_vector(toolchain::zip_equal(base, distUnitLocalOffset),
                            [&](const auto &t) -> Value {
                              return builder.createOrFold<arith::AddIOp>(
                                  loc, std::get<0>(t), std::get<1>(t));
                            });
    // Do not go beyond `srcShape` bounds.
    SmallVector<Value> mods = toolchain::map_to_vector(
        toolchain::zip_equal(adds, srcShape), [&](const auto &t) -> Value {
...
bool XeGPUDialect::isEvenlyDistributable(toolchain::ArrayRef<int64_t> shape,
  ...
  // share the data.
  auto tryDistribute = [&](toolchain::ArrayRef<int64_t> shape,
                           SmallVector<int64_t> layout,
                           SmallVector<int64_t> data,
                           bool rr = true) -> optional<SmallVector<int64_t>> {
    toolchain::SmallVector<int64_t> newShape(shape);
    if (layout.size()) {
...
//===----------------------------------------------------------------------===//
LogicalResult
LayoutAttr::verify(toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
...
LayoutAttr::delinearizeId(OpBuilder &builder, Location loc, Value linearId) {
  ...
  if (orderAttr && !orderAttr.empty()) {
    order = toolchain::to_vector(
        toolchain::map_range(orderAttr.asArrayRef(),
                        [](int32_t idx) { return static_cast<int64_t>(idx); }));
  } else {
    // Default order: [1, 0] for 2D (row-major), [2, 1, 0] for 3D, etc.
    order = toolchain::to_vector(
        toolchain::reverse(toolchain::seq<int64_t>(0, sgLayoutInt.size())));
  }
...
DistributeLayoutAttr LayoutAttr::setUnitDimData(SetVector<int64_t> unitDims) {
  ...
  if (sgDataOpt) {
    sgData = toolchain::to_vector(sgDataOpt.asArrayRef());
  }
  if (instDataOpt) {
    instData = toolchain::to_vector(instDataOpt.asArrayRef());
  }
  if (laneDataOpt) {
    laneData = toolchain::to_vector(laneDataOpt.asArrayRef());
  }
...
DistributeLayoutAttr LayoutAttr::setUnitDimLayout(SetVector<int64_t> unitDims) {
  ...
  if (sgLayoutOpt) {
    sgLayout = toolchain::to_vector(sgLayoutOpt.asArrayRef());
  }
  if (laneLayoutOpt) {
    laneLayout = toolchain::to_vector(laneLayoutOpt.asArrayRef());
  }
...
//===----------------------------------------------------------------------===//
LogicalResult
SliceAttr::verify(toolchain::function_ref<InFlightDiagnostic()> emitError,
  ...
  toolchain::SmallDenseSet<int64_t> seen;
...
SliceAttr SliceAttr::flatten() const {
  ...
  SmallVector<int64_t> indices =
      toolchain::to_vector(toolchain::seq<int64_t>(0, layoutAttr.getRank()));

  // get remaining dims (flattend) by applying slice ops with all slicedDims
  SmallVector<int64_t> remainingDims(indices);
  for (auto dim : toolchain::reverse(slicedDims))
    remainingDims = XeGPUDialect::slice(toolchain::ArrayRef<int64_t>(remainingDims),
                                        dim.asArrayRef());
  ...
  SmallVector<int64_t> flattendDims = XeGPUDialect::slice(
      toolchain::ArrayRef<int64_t>(indices), toolchain::ArrayRef<int64_t>(remainingDims));
...
bool SliceAttr::isSliceOf(const xegpu::DistributeLayoutAttr &other) {
  ...
  // dims.
  toolchain::SmallDenseSet<int64_t> thisDims(
      flattenedThis.getDims().asArrayRef().begin(),
      flattenedThis.getDims().asArrayRef().end());
  return toolchain::all_of(flattenedOther.getDims().asArrayRef(),
...
adjustUnitDimsWithSliceDims(const SetVector<int64_t> &unitDims,
  ...
  toolchain::SmallDenseSet<int64_t> slicedDimsSet(sliceDims.begin(),
                                             sliceDims.end());
...
LogicalResult
RangeAttr::verify(toolchain::function_ref<mlir::InFlightDiagnostic()> emitError,
...
void TensorDescType::print(AsmPrinter &printer) const {
  ...
  auto blockAttr = toolchain::dyn_cast_if_present<BlockTensorDescAttr>(encoding);
...
TensorDescType TensorDescType::get(toolchain::ArrayRef<int64_t> shape,
...
LogicalResult
TensorDescType::verify(toolchain::function_ref<InFlightDiagnostic()> emitError,
                       toolchain::ArrayRef<int64_t> shape, mlir::Type elementType,
  ...
  auto layoutAttr = toolchain::dyn_cast_if_present<LayoutAttr>(layout);
  if (layoutAttr) {
    ...
    if (!XeGPUDialect::isEvenlyDistributable(shape, layoutAttr)) {
      std::string shapeStr;
      toolchain::raw_string_ostream stream(shapeStr);
      toolchain::interleaveComma(shape, stream);
...
Attribute MemLayoutAttr::parse(AsmParser &parser, Type type) {
  ...
  toolchain::SmallDenseSet<StringRef> seenKeys;
...
SmallVector<OpFoldResult> getBlockedOffsets(OpBuilder &builder, Location loc,
  ...
  for (auto [offset, block] : toolchain::zip(offsets, blockShape)) {
...
SmallVector<int64_t> MemDescType::getStrideShape() {
  ...
  SmallVector<int, 4> perm =
      toolchain::to_vector<4>(toolchain::seq<int>(0, strides.size()));
  toolchain::sort(perm, [&](int a, int b) { return strides[a] < strides[b]; });
...
Value MemDescType::getLinearOffsets(OpBuilder &builder, Location loc,
  ...
  if (toolchain::equal(blockShape, matrixShape)) {
