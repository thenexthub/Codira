...
static bool isSharedMemory(const MemRefType &memrefTy) {
  ...
  if (auto intAttr = toolchain::dyn_cast<IntegerAttr>(attr))
    return intAttr.getInt() == 3;
  if (auto memrefSpace = toolchain::dyn_cast<MemorySpaceAttr>(attr))
    return memrefSpace.getValue() == MemorySpace::SLM;
  if (auto xevmSpace = toolchain::dyn_cast<xevm::AddrSpaceAttr>(attr))
...
static SmallVector<int64_t> getShapeOf(Type type) {
  ...
  if (auto ty = toolchain::dyn_cast<ShapedType>(type))
...
isValidGatherScatterParams(Type maskTy, VectorType valueTy,
  ...
  toolchain::SmallVector<int64_t> expectedMaskShape(tdescShape);
  if (chunkSize > 1)
...
isValidGatherScatterBufferParams(Type offsetsTy, Type maskTy,
  ...
  toolchain::SmallVector<int64_t> expectedMaskShape(valueShape);
  if (maskSize == 1)
    return success();
  if (chunkSize > 1)
    expectedMaskShape.pop_back();
...
IsValidMatrixOpParams(VectorType dataTy, MemDescType mdescTy,
  ...
  if (dataShape.size() == 2) {
    if (toolchain::any_of(toolchain::zip_equal(dataShape, mdescShape),
...
void CreateNdDescOp::build(OpBuilder &builder, OperationState &state,
                           ...
                           toolchain::ArrayRef<OpFoldResult> shape,
                           toolchain::ArrayRef<OpFoldResult> strides) {
  Type srcTy = source.getType();
  assert((isa<IntegerType, MemRefType>(srcTy)) &&
         "Source has to be either int or memref.");

  toolchain::SmallVector<Value> dynamicShape;
  toolchain::SmallVector<Value> dynamicStrides;

  toolchain::SmallVector<int64_t> staticShape;
  toolchain::SmallVector<int64_t> staticStrides;
...
void CreateNdDescOp::build(OpBuilder &builder, OperationState &state,
                           Type tdesc, TypedValue<MemRefType> source,
                           toolchain::ArrayRef<OpFoldResult> offsets) {
  [[maybe_unused]] auto ty = source.getType();
  assert(ty.hasStaticShape() && offsets.size() == (size_t)ty.getRank());

  toolchain::SmallVector<int64_t> staticOffsets;
  toolchain::SmallVector<Value> dynamicOffsets;
...
LogicalResult CreateNdDescOp::verify() {
  ...
  if (toolchain::isa<IntegerType>(getSourceType())) {
...
LogicalResult LoadNdOp::verify() {
  ...
  if (getTranspose()) {
    ...
    if (toolchain::all_of(trans, [&](size_t s) { return s < tdescShape.size(); }))
...
void CreateDescOp::build(OpBuilder &builder, OperationState &state,
                         TensorDescType TensorDesc, Value source,
                         toolchain::ArrayRef<int64_t> offsets) {
...
void UpdateOffsetOp::build(OpBuilder &builder, OperationState &state,
                           Value tensorDesc, toolchain::ArrayRef<int64_t> offsets) {
  auto ofrs = getAsIndexOpFoldResult(builder.getContext(), offsets);
