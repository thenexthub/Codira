...
static NestedPattern &vectorTransferPattern() {
  static auto pattern = affine::matcher::Op(
      toolchain::IsaPred<vector::TransferReadOp, vector::TransferWriteOp>);
  return pattern;
}
...
void VectorizationState::registerOpVectorReplacement(Operation *replaced,
  ...
  for (auto resultTuple :
       toolchain::zip(replaced->getResults(), replacement->getResults()))
...
static bool isIVMappedToMultipleIndices(
  ...
  for (auto &kvp : loopToVectorDim) {
    ...
    toolchain::DenseSet<Value> invariants =
...
static Operation *vectorizeAffineForOp(AffineForOp forOp,
  ...
  if (!isLoopVecDim) {
    ...
    // later.
    for (auto redAndOperand : toolchain::zip(reductions, forOp.getInits())) {
  ...
  for (auto iterTuple :
       toolchain ::zip(forOp.getRegionIterArgs(), vecForOp.getRegionIterArgs()))
...
void Vectorize::runOnOperation() {
  ...
  if (toolchain::any_of(vectorSizes, [](int64_t size) { return size <= 0; })) {
