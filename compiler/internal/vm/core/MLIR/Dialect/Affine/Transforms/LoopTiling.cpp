...
void LoopTiling::getTileSizes(ArrayRef<AffineForOp> band,
  ...
  if (!fp) {
    ...
    toolchain::fill(*tileSizes, LoopTiling::kDefaultTileSize);
  ...
  uint64_t excessFactor = toolchain::divideCeil(*fp, cacheSizeBytes);
  if (excessFactor <= 1) {
...
void LoopTiling::runOnOperation() {
  ...
  for (auto &band : bands) {
    ...
    if (toolchain::DebugFlag) {
      ...
      toolchain::interleaveComma(tileSizes, toolchain::dbgs());
      diag << "]\n";
    }
