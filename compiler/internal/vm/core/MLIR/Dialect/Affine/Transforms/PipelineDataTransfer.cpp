...
static bool doubleBuffer(Value oldMemRef, AffineForOp forOp) {
  ...
  auto doubleShape = [&](MemRefType oldMemRefType) -> MemRefType {
    ...
    newShape[0] = 2;
    toolchain::copy(oldShape, newShape.begin() + 1);
  ...
  for (const auto &dim : toolchain::enumerate(oldMemRefType.getShape())) {
...
void PipelineDataTransfer::runOnAffineForOp(AffineForOp forOp) {
  ...
  for (auto &pair : startWaitPairs) {
    ...
    if (!doubleBuffer(oldTagMemRef, forOp)) {
      LLVM_DEBUG(toolchain::dbgs() << "tag double buffering failed\n";);
      return;
    }
  ...
  if (!isOpwiseShiftValid(forOp, shifts)) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Shifts invalid - unexpected\n";);
    return;
  }

  if (failed(affineForOpBodySkew(forOp, shifts))) {
    LLVM_DEBUG(toolchain::dbgs() << "op body skewing failed - unexpected\n";);
    return;
  }
}
