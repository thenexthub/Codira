...
findInListOrAdd(Value value, toolchain::SmallVectorImpl<Value> &dims,
                function_ref<bool(Value)> isValidElement) {

  Value *loopIV = toolchain::find(dims, value);
  if (loopIV != dims.end()) {
...
static AffineExpr toAffineExpr(Value value,
                               toolchain::SmallVectorImpl<Value> &affineDims,
                               toolchain::SmallVectorImpl<Value> &affineSymbols) {
  ...
  if (toolchain::isa_and_nonnull<arith::AddIOp>(definingOp) ||
      toolchain::isa_and_nonnull<arith::MulIOp>(definingOp)) {
...
computeAffineMapAndArgs(MLIRContext *ctx, ValueRange indices, AffineMap &map,
                        toolchain::SmallVectorImpl<Value> &mapArgs) {
...
struct RaiseMemrefDialect
  ...
  void runOnOperation() override {
    ...
    op->walk([&](Operation *op) {
      ...
      if (auto store = toolchain::dyn_cast_or_null<memref::StoreOp>(op)) {
      ...
      } else if (auto load = toolchain::dyn_cast_or_null<memref::LoadOp>(op)) {
