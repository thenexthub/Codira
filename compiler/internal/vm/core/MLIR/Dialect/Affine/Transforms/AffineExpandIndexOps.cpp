...
static SmallVector<Value> computeStrides(Location loc, RewriterBase &rewriter,
  ...
  for (int64_t elem : toolchain::reverse(staticBasis)) {
...
LogicalResult affine::lowerAffineLinearizeIndexOp(RewriterBase &rewriter,
  ...
  for (auto [stride, idxOp] :
       toolchain::zip_equal(strides, toolchain::drop_end(op.getMultiIndexMutable()))) {
  ...
  // size of the stride.
  toolchain::stable_sort(scaledValues,
                    [&](auto l, auto r) { return l.second > r.second; });

  Value result = scaledValues.front().first;
  for (auto [scaledValue, numHoistableLoops] : toolchain::drop_begin(scaledValues)) {
