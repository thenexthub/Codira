...
void mlir::affine::reorderOperandsByHoistability(RewriterBase &rewriter,
                                                 AffineApplyOp op) {
  SmallVector<int64_t> numInvariant = toolchain::to_vector(
      toolchain::map_range(op->getOpOperands(), [&](OpOperand &operand) {
        return numEnclosingInvariantLoops(operand);
      }));

  int64_t numOperands = op.getNumOperands();
  SmallVector<int64_t> operandPositions =
      toolchain::to_vector(toolchain::seq<int64_t>(0, numOperands));
  toolchain::stable_sort(operandPositions, [&numInvariant](size_t i1, size_t i2) {
...
FailureOr<AffineApplyOp> mlir::affine::decompose(RewriterBase &rewriter,
  ...
  toolchain::stable_sort(subExpressions, [&](AffineExpr e1, AffineExpr e2) {
    return getMaxSymbol(e1) < getMaxSymbol(e2);
  });
  LDBG() << "--sorted subexprs: " << toolchain::interleaved(subExpressions);
