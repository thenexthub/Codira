...
#include "vm/core/ADT/IntEqClasses.h"
...
static bool simplifyAffineMinMaxOp(RewriterBase &rewriter, AffineOp affineOp) {
  ...
  SmallVector<Variable> variables = toolchain::map_to_vector(
      toolchain::iota_range<unsigned>(0u, affineMap.getNumResults(), false),
      [&](unsigned i) {
        return Variable(affineMap.getSliceMap(i, 1), operands);
      });
  LDBG() << "- constructed variables are: "
         << toolchain::interleaved_array(toolchain::map_range(
  ...
  toolchain::IntEqClasses boundedClasses(variables.size());
  DenseMap<unsigned, Variable *> bounds;
  for (auto &&[i, v] : toolchain::enumerate(variables)) {
  ...
  LDBG() << "--- res: " << toolchain::interleaved_array(results);
