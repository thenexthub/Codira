...
static void getProducerCandidates(unsigned dstId,
  ...
  toolchain::sort(srcIdCandidates);
  srcIdCandidates.erase(toolchain::unique(srcIdCandidates), srcIdCandidates.end());
}
...
static bool isEscapingMemref(Value memref, Block *block) {
  ...
  return toolchain::any_of(memref.getUsers(), [&](Operation *user) {
...
public:
  ...
  void performFusionsIntoDest(unsigned dstId, unsigned maxSrcUserCount) {
    ...
    do {
      ...
      for (unsigned srcId : toolchain::reverse(srcIdCandidates)) {
  ...
  void fuseWithSiblingNodes(Node *dstNode) {
    ...
    while (findSiblingNodeToFuse(dstNode, &visitedSibNodeIds, &idAndMemref)) {
      ...
      Operation *sibLoadOpInst = toolchain::getSingleElement(sibLoadOpInsts);
  ...
  bool findSiblingNodeToFuse(Node *dstNode,
    ...
    auto canFuseWithSibNode = [&](Node *sibNode, Value memref) {
      ...
      if (toolchain::any_of(loadAndStoreMemrefSet, [=](Value memref) {
  ...
  getOperation()->walk([&](Operation *op) {
    for (Region &region : op->getRegions()) {
      for (Block &block : region.getBlocks()) {
        ...
        if (!affineFors.empty() && !toolchain::hasSingleElement(affineFors))
          runOnBlock(&block);
      }
    }
  });
