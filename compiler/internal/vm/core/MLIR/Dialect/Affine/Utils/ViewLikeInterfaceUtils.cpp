...
LogicalResult mlir::affine::mergeOffsetsSizesAndStrides(
    ...
    const toolchain::SmallBitVector &droppedProducerDims,
  ...
  for (auto i : toolchain::seq<unsigned>(0, producerOffsets.size())) {
...
void mlir::affine::resolveIndicesIntoOpWithOffsetsAndStrides(
    ...
    const toolchain::SmallBitVector &rankReducedDims,
  ...
  for (auto dim : toolchain::seq<int64_t>(0, mixedSourceOffsets.size())) {
    OpFoldResult ofr =
  ...
  for (auto [offset, index, stride] :
       toolchain::zip_equal(mixedSourceOffsets, indices, mixedSourceStrides)) {
...
void mlir::affine::resolveSizesIntoOpWithSizes(
    ...
    const toolchain::SmallBitVector &rankReducedSourceDims,
