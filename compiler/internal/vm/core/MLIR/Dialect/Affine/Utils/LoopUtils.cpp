...
static void replaceIterArgsAndYieldResults(AffineForOp forOp) {
  ...
  for (auto e : toolchain::zip(iterOperands, iterArgs))
  ...
  for (auto e : toolchain::zip(outerResults, innerResults))
...
static AffineForOp generateShiftedLoop(
  ...
  for (const auto &it : toolchain::drop_begin(opGroupQueue, offset)) {
...
LogicalResult mlir::affine::affineForOpBodySkew(AffineForOp forOp,
  ...
  uint64_t maxShift = *toolchain::max_element(shifts);
...
static LogicalResult generateCleanupLoopForUnroll(AffineForOp forOp,
  ...
  for (auto e : toolchain::zip(results, cleanupResults, cleanupIterOperands)) {
    std::get<0>(e).replaceAllUsesWith(std::get<1>(e));
...
bool mlir::affine::isValidLoopInterchangePermutation(
  ...
  if (toolchain::any_of(loops, [](AffineForOp loop) {
...
unsigned mlir::affine::permuteLoops(ArrayRef<AffineForOp> input,
  ...
  toolchain::sort(checkPermMap);
  if (toolchain::any_of(toolchain::enumerate(checkPermMap),
...
AffineForOp mlir::affine::sinkSequentialLoops(AffineForOp forOp) {
  ...
  unsigned numParallelLoops = toolchain::count(isParallelLoop, true);
...
static void augmentMapAndBounds(OpBuilder &b, Value iv, AffineMap *map,
                                ...
                                int64_t offset = 0) {
  auto bounds = toolchain::to_vector<4>(map->getResults());
...
LogicalResult mlir::affine::coalesceLoops(MutableArrayRef<AffineForOp> loops) {
  ...
  Value prev;
  if (!toolchain::hasSingleElement(origUbMap.getResults()))
  ...
  for (auto [iter, init] :
       toolchain::zip_equal(secondOutermostLoop.getRegionIterArgs(),
...
findHighestBlockForPlacement(const MemRefRegion &region, Block &block,
  ...
  for (auto e = enclosingAffineOps.rend(); it != e; ++it) {
    ...
    if (toolchain::is_contained(symbols, affineFor.getInductionVar()))
      break;
...
generatePointWiseCopy(Location loc, Value memref, Value fastMemRef,
                      ...
                      OpBuilder b) {
  assert(toolchain::all_of(lbMaps, [&](AffineMap lbMap) {
    return lbMap.getNumInputs() == lbOperands.size();
  }));
  assert(toolchain::all_of(ubMaps, [&](AffineMap ubMap) {
...
static LogicalResult generateCopy(
  ...
  *sizeInBytes = 0;
  ...
  if (toolchain::any_of(lbs, [](AffineMap lb) { return lb.getNumResults() > 1; })) {
  ...
  if (!existingBuf) {
    ...
    *sizeInBytes = maySizeInBytes.value_or(0);
    ...
    LLVM_DEBUG(emitRemarkForBlock(*block)
               ...
               << " and size " << toolchain::divideCeil(*sizeInBytes, 1024)
               << " KiB\n");
  } else {
...
mlir::affine::affineDataCopyGenerate(Block::iterator begin, Block::iterator end,
  ...
  AffineForOp forOp;
  if (toolchain::DebugFlag && (forOp = dyn_cast<AffineForOp>(&*begin))) {
    LLVM_DEBUG(forOp.emitRemark()
               << toolchain::divideCeil(totalCopyBuffersSizeInBytes, 1024)
...
static AffineIfOp createSeparationCondition(MutableArrayRef<AffineForOp> loops,
  ...
  toolchain::append_range(ops, loops);
...
createFullTiles(MutableArrayRef<AffineForOp> inputNest,
  ...
  IRMapping operandMap;
  for (const auto &loopEn : toolchain::enumerate(inputNest))
