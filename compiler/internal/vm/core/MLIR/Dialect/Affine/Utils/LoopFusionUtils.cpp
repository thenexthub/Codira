    ...
    opX->walk([&](Operation *op) {
      ...
      for (Value value : op->getResults()) {
        for (Operation *user : value.getUsers()) {
          ...
          if (toolchain::is_contained(loops, cast<AffineForOp>(opB))) {
            lastDepOp = opX;
            return WalkResult::interrupt();
          }
        }
...
static unsigned getMaxLoopDepth(ArrayRef<Operation *> srcOps,
  ...
  if (all_of(targetDstOps, toolchain::IsaPred<AffineReadOpInterface>))
    return loopDepth;
...
static LogicalResult promoteSingleIterReductionLoop(AffineForOp forOp,
  ...
  toolchain::append_range(newOperands,
  ...
  if (siblingFusionUser) {
    ...
    for (Operation *op : toolchain::reverse(forwardSlice))
  ...
  for (auto it : toolchain::zip(forOpIterArgs, newLoop.getRegionIterArgs().take_back(
                                              forOpIterArgs.size()))) {
...
static int64_t getComputeCostHelper(
    ...
    toolchain::SmallDenseMap<Operation *, uint64_t, 8> *tripCountOverrideMap,
...
bool mlir::affine::getFusionComputeCost(AffineForOp srcForOp,
  ...
  if (storeLoadFwdGuaranteed) {
    ...
    toolchain::SmallDenseSet<Value, 4> storeMemrefs;
    ...
    for (Value memref : storeMemrefs) {
      for (Operation *user : memref.getUsers()) {
        ...
        // 'insertPointParent'.
        getAffineForIVs(*user, &loops);
        if (toolchain::is_contained(loops, cast<AffineForOp>(insertPointParent))) {
