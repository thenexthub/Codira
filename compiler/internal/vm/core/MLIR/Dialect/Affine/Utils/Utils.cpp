...
mlir::affine::expandAffineMap(OpBuilder &builder, Location loc,
  ...
  auto expanded = toolchain::to_vector<8>(
      toolchain::map_range(affineMap.getResults(),
                      [numDims, &builder, loc, operands](AffineExpr expr) {
                        return expandAffineExpr(builder, loc, expr,
                                                operands.take_front(numDims),
                                                operands.drop_front(numDims));
                      }));
  if (toolchain::all_of(expanded, [](Value v) { return v; }))
    return expanded;
  return std::nullopt;
}
...
static Operation *getOutermostInvariantForOp(AffineIfOp ifOp) {
  ...
  while (!res->getParentOp()->hasTrait<OpTrait::IsIsolatedFromAbove>()) {
    ...
    if (auto forOp = dyn_cast<AffineForOp>(parentOp)) {
      if (toolchain::is_contained(ifOperands, forOp.getInductionVar()))
        break;
    } else if (auto parallelOp = dyn_cast<AffineParallelOp>(parentOp)) {
      if (toolchain::any_of(parallelOp.getIVs(), [&](Value iv) {
            return toolchain::is_contained(ifOperands, iv);
          }))
        break;
...
mlir::affine::affineParallelize(AffineForOp forOp,
  ...
  auto reducedValues = toolchain::to_vector<4>(toolchain::map_range(
      parallelReductions, [](const LoopReduction &red) { return red.value; }));
  auto reductionKinds = toolchain::to_vector<4>(toolchain::map_range(
  ...
  AffineParallelOp newPloop = AffineParallelOp::create(
      ...
      toolchain::ArrayRef(lowerBoundMap), lowerBoundOperands,
      toolchain::ArrayRef(upperBoundMap), upperBoundOperands,
      toolchain::ArrayRef(forOp.getStepAsInt()));
...
LogicalResult mlir::affine::hoistAffineIfOp(AffineIfOp ifOp, bool *folded) {
  ...
  assert(toolchain::all_of(ifOp.getOperands(),
                      [](Value v) {
...
void mlir::affine::normalizeAffineParallel(AffineParallelOp op) {
  ...
  bool isAlreadyNormalized =
      toolchain::all_of(toolchain::zip(steps, lbMap.getResults()), [](auto tuple) {
...
bool mlir::affine::hasNoInterveningEffect(
    ...
    toolchain::function_ref<bool(Value, Value)> mayAlias) {
...
mlir::affine::hasNoInterveningEffect<mlir::MemoryEffects::Read,
    ...
    toolchain::function_ref<bool(Value, Value)>);
...
static void loadCSE(AffineReadOpInterface loadA,
  ...
  for (AffineReadOpInterface option : loadCandidates) {
    if (toolchain::all_of(loadCandidates, [&](AffineReadOpInterface depStore) {
...
void mlir::affine::affineScalarReplace(func::FuncOp f, DominanceInfo &domInfo,
  ...
  for (auto memref : memrefsToErase) {
    ...
    if (!defOp || !hasSingleEffect<MemoryEffects::Allocate>(defOp, memref))
      ...
      continue;
    if (toolchain::any_of(memref.getUsers(), [&](Operation *ownerOp) {
    ...
    for (auto *user : toolchain::make_early_inc_range(memref.getUsers()))
      user->erase();
    defOp->erase();
  }
...
LogicalResult mlir::affine::replaceAllMemRefUsesWith(
  ...
  for (const auto &opEntry : toolchain::enumerate(op->getOperands())) {
...
LogicalResult mlir::affine::replaceAllMemRefUsesWith(
    ...
    toolchain::function_ref<bool(Operation *)> userFilterFn,
  ...
  for (auto *user : oldMemRef.getUsers()) {
    ...
    if (!isa<AffineMapAccessInterface>(*user)) {
      ...
      if (!user->hasTrait<OpTrait::MemRefsNormalizable>()) {
        LLVM_DEBUG(toolchain::dbgs() << "Memref replacement failed: use without a "
  ...
  return expr
      .walk([&](AffineExpr e) {
        if (isa<AffineDimExpr>(e) &&
            toolchain::any_of(inMemrefTypeDynDims, [&](unsigned dim) {
...
LogicalResult mlir::affine::normalizeMemRef(AllocLikeOp allocOp) {
  ...
  assert(toolchain::all_of(oldMemRef.getUsers(), [&](Operation *op) {
...
mlir::affine::delinearizeIndex(OpBuilder &b, Location loc, Value linearIndex,
  ...
  for (OpFoldResult basisElem : toolchain::reverse(basis)) {
  ...
  for (Value divisor : toolchain::reverse(divisors)) {
...
OpFoldResult mlir::affine::linearizeIndex(OpBuilder &builder, Location loc,
  ...
  toolchain::transform(stridesAffine, std::back_inserter(strides),
