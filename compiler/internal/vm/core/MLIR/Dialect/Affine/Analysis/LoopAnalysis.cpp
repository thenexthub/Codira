...
public:
  ...
  void printEdges() {
    for (auto &en : edges) {
      toolchain::dbgs() << *en.first << " (" << en.first << ")"
                   << " has " << en.second.size() << " edges:\n";
      for (auto *node : en.second) {
        toolchain::dbgs() << '\t' << *node->op << '\n';
      }
    }
  }
...
private:
  ...
  DGNode *getNode(Operation *op) {
    auto *value =
        toolchain::find_if(nodes, [&](const DGNode &node) { return node.op == op; });
  ...
  bool hasNode(Operation *key) const {
    return toolchain::find_if(nodes, [&](const DGNode &node) {
             return node.op == key;
           }) != nodes.end();
  }
...
void mlir::affine::getTripCountMapAndOperands(
  ...
  if (forOp.hasConstantBounds()) {
    ...
    *tripCountMap = AffineMap::getConstantMap(
        toolchain::divideCeilSigned(loopSpan, step), context);
...
bool mlir::affine::isInvariantAccess(LoadOrStoreOp memOp, AffineForOp forOp) {
  ...
  avm.composeSimplifyAndCanonicalize();
  return !toolchain::is_contained(avm.getOperands(), forOp.getInductionVar());
}
...
isVectorizableLoopBodyWithOpCond(AffineForOp loop,
  ...
  auto types = matcher::Op([](Operation &op) -> bool {
    if (toolchain::any_of(op.getOperandTypes(), [](Type type) {
      ...
      return true;
    return !toolchain::all_of(op.getResultTypes(), VectorType::isValidElementType);
  });
...
bool mlir::affine::isOpwiseShiftValid(AffineForOp forOp,
  ...
  for (const auto &it :
       toolchain::enumerate(toolchain::reverse(forBody->getOperations()))) {
    auto &op = it.value();
