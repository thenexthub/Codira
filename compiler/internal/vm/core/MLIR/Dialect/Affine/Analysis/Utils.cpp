...
unsigned Node::getStoreOpCount(Value memref) const {
  ...
  for (auto *storeOp : toolchain::concat<Operation *const>(stores, memrefStores)) {
...
unsigned Node::hasStore(Value memref) const {
  return toolchain::any_of(
      toolchain::concat<Operation *const>(stores, memrefStores),
      [&](Operation *storeOp) {
        if (auto affineStore = dyn_cast<AffineWriteOpInterface>(storeOp)) {
          if (memref == affineStore.getMemRef())
            return true;
...
unsigned Node::hasFree(Value memref) const {
  return toolchain::any_of(memrefFrees, [&](Operation *freeOp) {
...
void Node::getLoadAndStoreMemrefSet(
  ...
  toolchain::SmallDenseSet<Value, 2> loadMemrefs;
  for (Operation *loadOp : loads) {
...
addNodeToMDG(Operation *nodeOp, MemRefDependenceGraph &mdg,
  ...
  for (Operation *op : collector.memrefLoads) {
    ...
    if (toolchain::any_of(((ValueRange)effectedValues).getTypes(),
...
static bool mayDependence(const Node &srcNode, const Node &dstNode,
  ...
  auto hasNonAffineDep = [&](ArrayRef<Operation *> srcMemOps,
    ...
    return toolchain::any_of(srcMemOps, [&](Operation *srcOp) {
      Value srcMemref = getMemRef(srcOp);
      if (srcMemref != memref)
        return false;
      return toolchain::find_if(dstMemOps, [&](Operation *dstOp) {
  ...
  toolchain::append_range(dstOps, toolchain::concat<Operation *const>(
  ...
  dstOps.clear();
  toolchain::append_range(dstOps, dstNode.memrefStores);
  ...
  for (auto *srcMemOp :
       toolchain::concat<Operation *const>(srcNode.stores, srcNode.loads)) {
    MemRefAccess srcAcc(srcMemOp);
    if (srcAcc.memref != memref)
      continue;
    for (auto *destMemOp :
         toolchain::concat<Operation *const>(dstNode.stores, dstNode.loads)) {
  ...
  for (auto &idAndNode : nodes) {
    ...
    for (Value value : opInst->getResults()) {
      for (Operation *user : value.getUsers()) {
        ...
        // block.
        auto *it = toolchain::find_if(loops, [&](AffineForOp loop) {
...
bool MemRefDependenceGraph::hasEdge(unsigned srcId, unsigned dstId,
  ...
  bool hasOutEdge = toolchain::any_of(outEdges.lookup(srcId), [=](const Edge &edge) {
    return edge.id == dstId && (!value || edge.value == value);
  });
  bool hasInEdge = toolchain::any_of(inEdges.lookup(dstId), [=](const Edge &edge) {
...
MemRefDependenceGraph::getFusedLoopNestInsertionPoint(unsigned srcId,
  ...
  if (toolchain::any_of(definingNodes,
  ...
  toolchain::SmallPtrSet<Operation *, 2> srcDepInsts;
  for (auto &outEdge : outEdges.lookup(srcId))
    if (outEdge.id != dstId)
      srcDepInsts.insert(getNode(outEdge.id)->op);

  // Build set of insts in range (srcId, dstId) on which 'dstId' depends.
  toolchain::SmallPtrSet<Operation *, 2> dstDepInsts;
...
void MemRefDependenceGraph::addToNode(unsigned id, ArrayRef<Operation *> loads,
  ...
  toolchain::append_range(node->loads, loads);
  toolchain::append_range(node->stores, stores);
  toolchain::append_range(node->memrefLoads, memrefLoads);
  toolchain::append_range(node->memrefStores, memrefStores);
  toolchain::append_range(node->memrefFrees, memrefFrees);
}
...
void ComputationSliceState::dump() const {
  ...
  toolchain::errs() << "\tLBs:\n";
  for (auto en : toolchain::enumerate(lbs)) {
    toolchain::errs() << "\t\t" << en.value() << "\n";
    toolchain::errs() << "\t\tOperands:\n";
    for (Value lbOp : lbOperands[en.index()])
      toolchain::errs() << "\t\t\t" << lbOp << "\n";
  }

  toolchain::errs() << "\tUBs:\n";
  for (auto en : toolchain::enumerate(ubs)) {
    toolchain::errs() << "\t\t" << en.value() << "\n";
    toolchain::errs() << "\t\tOperands:\n";
...
LogicalResult MemRefRegion::compute(Operation *op, unsigned loopDepth,
  ...
  if (sliceState != nullptr) {
    ...
    for (auto extraOperand : sliceState->lbOperands[0]) {
      if (!toolchain::is_contained(operands, extraOperand)) {
  ...
  for (auto en : toolchain::enumerate(vars)) {
    if ((isAffineInductionVar(en.value())) &&
        !toolchain::is_contained(enclosingIVs, en.value())) {
      if (dropOuterIvs) {
...
mlir::affine::getMemRefIntOrFloatEltSizeInBytes(MemRefType memRefType) {
  ...
  return toolchain::divideCeil(sizeInBits, 8);
}
...
LogicalResult mlir::affine::boundCheckLoadOrStoreOp(LoadOrStoreOp loadOrStoreOp,
                                                    bool emitError) {
  static_assert(toolchain::is_one_of<LoadOrStoreOp, AffineReadOpInterface,
...
bool mlir::affine::buildSliceTripCountMap(
    ...
    toolchain::SmallDenseMap<Operation *, uint64_t, 8> *tripCountMap) {
...
uint64_t mlir::affine::getSliceIterationCount(
    const toolchain::SmallDenseMap<Operation *, uint64_t, 8> &sliceTripCountMap) {
...
void mlir::affine::getComputationSliceState(
  ...
  toolchain::SmallDenseSet<Value, 8> sequentialLoops;
  ...
  toolchain::SmallDenseMap<Operation *, uint64_t, 8> sliceTripCountMap;
  auto srcIsUnitSlice = [&]() {
...
MemRefAccess::MemRefAccess(Operation *memOp) {
  if (auto loadOp = dyn_cast<AffineReadOpInterface>(memOp)) {
    ...
    opInst = memOp;
    toolchain::append_range(indices, loadOp.getMapOperands());
  } else {
    ...
    toolchain::append_range(indices, storeOp.getMapOperands());
  }
}
...
void mlir::affine::getAffineIVs(Operation &op, SmallVectorImpl<Value> &ivs) {
  ...
  while (currOp) {
    if (AffineForOp currAffineForOp = dyn_cast<AffineForOp>(currOp))
      ...
      toolchain::append_range(ivs, parOp.getIVs());
    currOp = currOp->getParentOp();
  }
...
void mlir::affine::getSequentialLoops(
    AffineForOp forOp, toolchain::SmallDenseSet<Value, 8> *sequentialLoops) {
...
static void unpackOptionalValues(ArrayRef<std::optional<Value>> source,
  ...
  target =
      toolchain::to_vector<4>(toolchain::map_range(source, [](std::optional<Value> val) {
