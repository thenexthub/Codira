...
using toolchain::divideCeilSigned;
using toolchain::divideFloorSigned;
using toolchain::mod;
...
remainsLegalAfterInline(Value value, Region *src, Region *dest,
  ...
  if (toolchain::isa<BlockArgument>(value))
...
remainsLegalAfterInline(ValueRange values, Region *src, Region *dest,
                        const IRMapping &mapping,
                        function_ref<bool(Value, Region *)> legalityCheck) {
  return toolchain::all_of(values, [&](Value v) {
...
static bool remainsLegalAfterInline(OpTy op, Region *src, Region *dest,
                                    const IRMapping &mapping) {
  static_assert(toolchain::is_one_of<OpTy, AffineReadOpInterface,
...
//  the inability to see through "toolchain::TypeSwitch".
template <>
...
struct AffineInlinerInterface : public DialectInlinerInterface {
  ...
  bool isLegalToInline(Region *dest, Region *src, bool wouldBeCloned,
    ...
    for (Operation &op : srcBlock) {
      ...
      bool remainsValid =
          toolchain::TypeSwitch<Operation *, bool>(&op)
...
bool mlir::affine::isValidDim(Value value) {
  ...
  if (isAffineInductionVar(value))
    return true;
  auto *parentOp = toolchain::cast<BlockArgument>(value).getOwner()->getParentOp();
  return parentOp && parentOp->hasTrait<OpTrait::AffineScope>();
}
...
bool mlir::affine::isValidDim(Value value, Region *region) {
  ...
  if (isa<AffineDelinearizeIndexOp, AffineLinearizeIndexOp>(op))
    return toolchain::all_of(op->getOperands(),
...
static bool isDimOpValidSymbol(ShapedDimOpInterface dimOp, Region *region) {
  ...
  if (toolchain::isa<BlockArgument>(dimOp.getShapedValue()))
    return false;
...
bool mlir::affine::isValidSymbol(Value value, Region *region) {
  ...
  if (isPure(defOp) && toolchain::all_of(defOp->getOperands(), [&](Value operand) {
...
bool AffineApplyOp::isValidDim() {
  return toolchain::all_of(getOperands(),
...
bool AffineApplyOp::isValidSymbol(Region *region) {
  return toolchain::all_of(getOperands(), [&](Value operand) {
    return affine::isValidSymbol(operand, region);
  });
}
...
static void simplifyMinOrMaxExprWithOperands(AffineMap &map,
  ...
  for (auto exprEn : toolchain::enumerate(map.getResults())) {
    ...
    if (isMax) {
      ...
      if (!toolchain::any_of(toolchain::enumerate(lowerBounds), [&](const auto &en) {
      ...
      if (!toolchain::any_of(toolchain::enumerate(upperBounds), [&](const auto &en) {
...
static LogicalResult replaceAffineMinBoundingBoxExpression(AffineMinOp minOp,
  ...
  for (auto [i, dim] : toolchain::enumerate(minOp.getDimOperands())) {
    auto it = toolchain::find(dims, dim);
    if (it == dims.end()) {
  ...
  for (auto [i, sym] : toolchain::enumerate(minOp.getSymbolOperands())) {
    auto it = toolchain::find(syms, sym);
    if (it == syms.end()) {
  ...
  for (AffineExpr expr : affineMinMap.getResults()) {
    ...
    // sims then continue.
    if (toolchain::any_of(unmappedDims,
                     [&](unsigned i) { return expr.isFunctionOfDim(i); }) ||
        toolchain::any_of(unmappedSyms,
...
static void shortenAddChainsContainingAll(
    AffineExpr e, const toolchain::SmallDenseSet<AffineExpr, 4> &exprsToRemove,
  ...
  toolchain::SmallDenseSet<AffineExpr, 4> ourTracker(exprsToRemove);
  ...
  for (AffineExpr preserved : toolchain::reverse(toPreserve))
...
static LogicalResult replaceAffineDelinearizeIndexInverseExpression(
  ...
  for (auto [pos, dim] : toolchain::enumerate(dims)) {
  ...
  for (auto [pos, sym] : toolchain::enumerate(syms)) {
  ...
  if (toolchain::is_contained(resToExpr, AffineExpr()))
    return failure();

  bool isDimReplacement = toolchain::all_of(resToExpr, toolchain::IsaPred<AffineDimExpr>);
  int64_t stride = 1;
  toolchain::SmallDenseSet<AffineExpr, 4> expectedExprs;
  // This isn't zip_equal since sometimes the delinearize basis is missing a
  // size for the first result.
  for (auto [binding, size] : toolchain::zip(
           toolchain::reverse(resToExpr), toolchain::reverse(delinOp.getStaticBasis()))) {
...
static void composeAffineMapAndOperands(AffineMap *map,
  ...
  for (auto *container : {&dims, &syms}) {
    ...
    for (const auto &en : toolchain::enumerate(*container)) {
      Value v = en.value();
      if (!v) {
...
void mlir::affine::fullyComposeAffineMapAndOperands(
  ...
  while (toolchain::any_of(*operands, [](Value v) {
  ...
  if (composeAffineMin && toolchain::any_of(*operands, [](Value v) {
...
static void composeMultiResultAffineMap(AffineMap &map,
  ...
  for (unsigned i : toolchain::seq<unsigned>(0, map.getNumResults())) {
    ...
    toolchain::append_range(dims,
                       ArrayRef<Value>(submapOperands).take_front(numNewDims));
    toolchain::append_range(symbols,
  ...
  operands = toolchain::to_vector(toolchain::concat<Value>(dims, symbols));
...
OpFoldResult mlir::affine::makeComposedFoldedAffineApply(
  ...
  applyOp->erase();
  return toolchain::getSingleElement(foldResults);
}
...
mlir::affine::makeComposedFoldedMultiResultAffineApply(
    OpBuilder &b, Location loc, AffineMap map, ArrayRef<OpFoldResult> operands,
    bool composeAffineMin) {
  return toolchain::map_to_vector(
      toolchain::seq<unsigned>(0, map.getNumResults()), [&](unsigned i) {
...
static void canonicalizeMapOrSetAndOperands(MapOrSet *mapOrSet,
                                            SmallVectorImpl<Value> *operands) {
  static_assert(toolchain::is_one_of<MapOrSet, AffineMap, IntegerSet>::value,
  ...
  toolchain::SmallBitVector usedDims(mapOrSet->getNumDims());
  toolchain::SmallBitVector usedSyms(mapOrSet->getNumSymbols());
  ...
  toolchain::SmallDenseMap<Value, AffineExpr, 8> seenDims;
  SmallVector<AffineExpr, 8> dimRemapping(mapOrSet->getNumDims());
  unsigned nextDim = 0;
  ...
  toolchain::SmallDenseMap<Value, AffineExpr, 8> seenSymbols;
  SmallVector<AffineExpr, 8> symRemapping(mapOrSet->getNumSymbols());
  unsigned nextSym = 0;
...
struct SimplifyAffineOp : public OpRewritePattern<AffineOpTy> {
  ...
  LogicalResult matchAndRewrite(AffineOpTy affineOp,
    ...
    static_assert(
        toolchain::is_one_of<AffineOpTy, AffineLoadOp, AffinePrefetchOp,
...
LogicalResult AffineDmaStartOp::verifyInvariantsImpl() {
  if (!toolchain::isa<MemRefType>(getOperand(getSrcMemRefOperandIndex()).getType()))
    return emitOpError("expected DMA source to be of memref type");
  if (!toolchain::isa<MemRefType>(getOperand(getDstMemRefOperandIndex()).getType()))
    return emitOpError("expected DMA destination to be of memref type");
  if (!toolchain::isa<MemRefType>(getOperand(getTagMemRefOperandIndex()).getType()))
...
ParseResult AffineDmaWaitOp::parse(OpAsmParser &parser,
  ...
  if (!toolchain::isa<MemRefType>(type))
    return parser.emitError(parser.getNameLoc(),
...
LogicalResult AffineDmaWaitOp::verifyInvariantsImpl() {
  if (!toolchain::isa<MemRefType>(getOperand(0).getType()))
    return emitOpError("expected DMA tag to be of memref type");
  Region *scope = getAffineScope(*this);
  for (auto idx : getTagIndices()) {
    if (!idx.getType().isIndex())
...
ParseResult AffineForOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (succeeded(parser.parseOptionalKeyword("iter_args"))) {
    ...
    for (auto argOperandType :
         toolchain::zip(toolchain::drop_begin(regionArgs), operands, result.types)) {
...
static LogicalResult foldLoopBounds(AffineForOp forOp) {
  auto foldLowerOrUpperBound = [&forOp](bool lower) {
    ...
    auto maxOrMin = toolchain::cast<IntegerAttr>(foldedResults[0]).getValue();
    for (unsigned i = 1, e = foldedResults.size(); i < e; i++) {
      auto foldedResult = toolchain::cast<IntegerAttr>(foldedResults[i]).getValue();
      maxOrMin = lower ? toolchain::APIntOps::smax(maxOrMin, foldedResult)
                       : toolchain::APIntOps::smin(maxOrMin, foldedResult);
    }
...
static SmallVector<OpFoldResult> AffineForEmptyLoopFolder(AffineForOp forOp) {
  if (!toolchain::hasSingleElement(*forOp.getBody()))
    return {};
  ...
  for (unsigned i = 0, e = yieldOp->getNumOperands(); i < e; ++i) {
    ...
    BlockArgument *iterArgIt = toolchain::find(iterArgs, val);
  ...
  if (tripCount.has_value() && tripCount.value() >= 2 && iterArgsNotInOrder)
    return {};
  return toolchain::to_vector_of<OpFoldResult>(replacements);
}
...
FailureOr<LoopLikeOpInterface> AffineForOp::replaceWithAdditionalYields(
  ...
  auto inits = toolchain::to_vector(getInits());
  ...
  if (replaceInitOperandUsesInLoop) {
    ...
    // arguments.
    for (auto it : toolchain::zip(newInitOperands, newIterArgs)) {
...
AffineParallelOp mlir::affine::getAffineParallelInductionVarOwner(Value val) {
  ...
  if (parallelOp && toolchain::is_contained(parallelOp.getIVs(), val))
    return parallelOp;
  return nullptr;
}
...
struct SimplifyDeadElse : public OpRewritePattern<AffineIfOp> {
  ...
  LogicalResult matchAndRewrite(AffineIfOp ifOp,
    ...
    if (ifOp.getElseRegion().empty() ||
        !toolchain::hasSingleElement(*ifOp.getElseBlock()) || ifOp.getNumResults())
      return failure();
...
LogicalResult AffineIfOp::fold(FoldAdaptor, SmallVectorImpl<OpFoldResult> &) {
  ...
  if (getIntegerSet() == set && toolchain::equal(operands, getOperands()))
    return failure();
...
void AffineLoadOp::build(OpBuilder &builder, OperationState &result,
  ...
  auto memrefType = toolchain::cast<MemRefType>(operands[0].getType());
...
OpFoldResult AffineLoadOp::fold(FoldAdaptor adaptor) {
  ...
  if (!getAffineMap().isConstant())
    return {};
  auto indices = toolchain::to_vector<4>(
      toolchain::map_range(getAffineMap().getConstantResults(),
...
static OpFoldResult foldMinMaxOp(T op, ArrayRef<Attribute> operands) {
  static_assert(toolchain::is_one_of<T, AffineMinOp, AffineMaxOp>::value,
  ...
  auto resultIt = std::is_same<T, AffineMinOp>::value
                      ? toolchain::min_element(results)
                      : toolchain::max_element(results);
...
struct DeduplicateAffineMinMaxExpressions : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T affineOp,
    ...
    for (AffineExpr expr : oldMap.getResults()) {
      ...
      if (!toolchain::is_contained(newExprs, expr))
...
struct MergeAffineMinMaxOp : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T affineOp,
    ...
    auto newDimOperands = toolchain::to_vector<8>(dimOperands);
    auto newSymOperands = toolchain::to_vector<8>(symOperands);
    SmallVector<AffineExpr, 4> newExprs;
    ...
    auto newMap = AffineMap::get(numUsedDims, numUsedSyms, newExprs,
                                 rewriter.getContext());
    auto newOperands =
        toolchain::to_vector<8>(toolchain::concat<Value>(newDimOperands, newSymOperands));
...
static LogicalResult canonicalizeMapExprAndTermOrder(AffineMap &map) {
  ...
  if (toolchain::is_sorted(flattenedExprs))
    return failure();

  // Reorder the result expressions according to their flattened form.
  SmallVector<unsigned> resultPermutation =
      toolchain::to_vector(toolchain::seq<unsigned>(0, map.getNumResults()));
  toolchain::sort(resultPermutation, [&](unsigned lhs, unsigned rhs) {
...
void AffineParallelOp::build(OpBuilder &builder, OperationState &result,
  ...
  auto ubs = toolchain::to_vector<4>(toolchain::map_range(ranges, [&](int64_t value) {
...
void AffineParallelOp::build(OpBuilder &builder, OperationState &result,
                             ...
                             ArrayRef<int64_t> steps) {
  assert(toolchain::all_of(lbMaps,
                      [lbMaps](AffineMap m) {
         ...
         "and symbols");
  assert(toolchain::all_of(ubMaps,
                      [ubMaps](AffineMap m) {
  ...
  auto concatMapsSameInput = [&builder](ArrayRef<AffineMap> maps,
    ...
    for (AffineMap m : maps) {
      toolchain::append_range(exprs, m.getResults());
...
LogicalResult AffineParallelOp::verify() {
  ...
  // ops
  for (auto it : toolchain::enumerate((getReductions()))) {
...
static void printMinMaxBound(OpAsmPrinter &p, AffineMapAttr mapAttr,
  ...
  unsigned start = 0;
  for (toolchain::APInt groupSize : group) {
    if (start != 0)
      p << ", ";
...
void AffineParallelOp::print(OpAsmPrinter &p) {
  ...
  bool elideSteps = toolchain::all_of(steps, [](int64_t step) { return step == 1; });
  if (!elideSteps) {
    p << " step (";
    toolchain::interleaveComma(steps, p);
    p << ')';
  }
  if (getNumResults()) {
    p << " reduce (";
    toolchain::interleaveComma(getReductions(), p, [&](auto &attr) {
      arith::AtomicRMWKind sym = *arith::symbolizeAtomicRMWKind(
          toolchain::cast<IntegerAttr>(attr).getInt());
  ...
  for (const auto &list : operands) {
    ...
    for (Value operand : valueOperands) {
      unsigned pos = std::distance(uniqueOperands.begin(),
                                   toolchain::find(uniqueOperands, operand));
...
static ParseResult parseAffineMapWithMinMax(OpAsmParser &parser,
  ...
  // see: https://reviews.toolchain.org/D134227#3821753
  const toolchain::StringLiteral tmpAttrStrName = "__pseudo_bound_map";
  ...
  auto parseOperands = [&]() {
    if (succeeded(parser.parseOptionalKeyword(
      ...
      toolchain::append_range(flatExprs, map.getValue().getResults());
      auto operandsRef = toolchain::ArrayRef(mapOperands);
...
LogicalResult AffineYieldOp::verify() {
  ...
  for (auto it : toolchain::zip(results, operands)) {
...
LogicalResult AffineDelinearizeIndexOp::verify() {
  ...
  auto dynamicMarkersCount = toolchain::count_if(staticBasis, ShapedType::isDynamic);
  ...
  if (!toolchain::all_of(staticBasis, [](int64_t v) {
...
AffineDelinearizeIndexOp::fold(FoldAdaptor adaptor,
  ...
  for (int64_t modulus : toolchain::reverse(staticBasis)) {
    result.push_back(IntegerAttr::get(attrType, toolchain::mod(highPart, modulus)));
    highPart = toolchain::divideFloorSigned(highPart, modulus);
  }
...
struct DropUnitExtentBasis
  ...
  LogicalResult matchAndRewrite(affine::AffineDelinearizeIndexOp delinearizeOp,
    ...
    for (auto [index, basis] :
         toolchain::enumerate(delinearizeOp.getPaddedBasis())) {
...
struct CancelDelinearizeOfLinearizeDisjointExactTail
  ...
  LogicalResult matchAndRewrite(affine::AffineDelinearizeIndexOp delinearizeOp,
    ...
    size_t numMatches = 0;
    for (auto [linSize, delinSize] : toolchain::zip(
             toolchain::reverse(linearizeBasis), toolchain::reverse(delinearizeBasis))) {
...
struct SplitDelinearizeSpanningLastLinearizeArg final
  ...
  LogicalResult matchAndRewrite(affine::AffineDelinearizeIndexOp delinearizeOp,
    ...
    for (int64_t basisElem : toolchain::reverse(basis)) {
    ...
    SmallVector<Value> results = toolchain::to_vector(
        toolchain::concat<Value>(delinearizeWithoutSplitPart.getResults(),
...
LogicalResult AffineLinearizeIndexOp::verify() {
  ...
  auto dynamicMarkersCount =
      toolchain::count_if(getStaticBasis(), ShapedType::isDynamic);
  if (static_cast<size_t>(dynamicMarkersCount) != getDynamicBasis().size())
    return emitOpError(
        "mismatch between dynamic and static basis (kDynamic marker but no "
        "corresponding dynamic basis entry) -- this can only happen due to an "
...
OpFoldResult AffineLinearizeIndexOp::fold(FoldAdaptor adaptor) {
  ...
  if (toolchain::is_contained(adaptor.getMultiIndex(), nullptr))
    return nullptr;
  ...
  for (auto [length, indexAttr] :
       toolchain::zip_first(toolchain::reverse(getStaticBasis()),
                       toolchain::reverse(adaptor.getMultiIndex()))) {
...
struct DropLinearizeUnitComponentsIfDisjointOrZero final
  ...
  LogicalResult matchAndRewrite(affine::AffineLinearizeIndexOp op,
    ...
    for (auto [index, basisElem] : toolchain::zip_equal(multiIndex, basis)) {
...
public:
  LogicalResult matchAndRewrite(affine::AffineLinearizeIndexOp linearizeOp,
    ...
    toolchain::SmallPtrSet<Operation *, 2> alreadyMatchedDelinearize;
    ...
    for (Match m : matches) {
      ...
      toolchain::append_range(newIndex, multiIndex.slice(prevMatchEnd, gap));
      toolchain::append_range(newBasis, linBasisRef.slice(prevMatchEnd, gap));
      ...
      toolchain::append_range(newDelinResults,
                         newDelinearize.getResults().take_front(m.delinStart));
      toolchain::append_range(newDelinResults, residualDelinearize.getResults());
    ...
    toolchain::append_range(newIndex, multiIndex.drop_front(prevMatchEnd));
    toolchain::append_range(newBasis, linBasisRef.drop_front(prevMatchEnd));
    ...
    for (auto [m, newResults] :
         toolchain::zip_equal(matches, delinearizeReplacements)) {
      if (newResults.empty())
        continue;
