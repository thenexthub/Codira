...
struct AffineApplyOpInterface
  ...
  void populateBoundsForIndexValue(Operation *op, Value value,
    ...
    SmallVector<Value> operands = toolchain::to_vector(applyOp.getOperands());
...
struct AffineMinOpInterface
  ...
  void populateBoundsForIndexValue(Operation *op, Value value,
    ...
    for (AffineExpr expr : minOp.getAffineMap().getResults()) {
      SmallVector<AffineExpr> dimReplacements = toolchain::to_vector(toolchain::map_range(
          minOp.getDimOperands(), [&](Value v) { return cstr.getExpr(v); }));
      SmallVector<AffineExpr> symReplacements = toolchain::to_vector(toolchain::map_range(
...
struct AffineDelinearizeIndexOpInterface
  ...
  void populateBoundsForIndexValue(Operation *rawOp, Value value,
    ...
    for (OpFoldResult basisElem : toolchain::drop_begin(basis, resIdx + 1))
...
struct AffineLinearizeIndexOpInterface
  ...
  void populateBoundsForIndexValue(Operation *rawOp, Value value,
    ...
    for (auto [revArgNum, length] : toolchain::enumerate(toolchain::reverse(basis))) {
