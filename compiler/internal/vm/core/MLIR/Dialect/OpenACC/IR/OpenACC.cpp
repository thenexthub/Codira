...
mlir::ArrayAttr addDeviceTypeAffectedOperandHelper(
    ...
    toolchain::ArrayRef<acc::DeviceType> newDeviceTypes) {
  toolchain::SmallVector<mlir::Attribute> deviceTypes;
  if (existingDeviceTypes)
    toolchain::copy(existingDeviceTypes, std::back_inserter(deviceTypes));
...
mlir::ArrayAttr addDeviceTypeAffectedOperandHelper(
    MLIRContext *context, mlir::ArrayAttr existingDeviceTypes,
    toolchain::ArrayRef<acc::DeviceType> newDeviceTypes, mlir::ValueRange arguments,
    mlir::MutableOperandRange argCollection,
    toolchain::SmallVector<int32_t> &segments) {
  toolchain::SmallVector<mlir::Attribute> deviceTypes;
  if (existingDeviceTypes)
    toolchain::copy(existingDeviceTypes, std::back_inserter(deviceTypes));
...
static void printDeviceTypes(mlir::OpAsmPrinter &p,
  ...
  p << "[";
  toolchain::interleaveComma(*deviceTypes, p,
...
getValuesFromSegments(std::optional<mlir::ArrayAttr> arrayAttr,
                      ...
                      std::optional<toolchain::ArrayRef<int32_t>> segments,
                      mlir::acc::DeviceType deviceType) {
  if (!arrayAttr)
...
static LogicalResult checkRecipe(OpT op, toolchain::StringRef operandName) {
...
static LogicalResult checkPrivateOperands(mlir::Operation *accConstructOp,
                                          const mlir::ValueRange &operands,
                                          toolchain::StringRef operandName) {
  toolchain::DenseSet<Value> set;
  for (mlir::Value operand : operands) {
...
static LogicalResult verifyDeviceTypeAndSegmentCountMatch(
    ...
    ArrayAttr deviceTypes, toolchain::StringRef keyword, int32_t maxInSegment = 0) {
...
void acc::ParallelOp::addNumWorkersOperand(
    ...
    toolchain::ArrayRef<DeviceType> effectiveDeviceTypes) {
...
void acc::ParallelOp::addAsyncOnly(
    MLIRContext *context, toolchain::ArrayRef<DeviceType> effectiveDeviceTypes) {
...
void acc::ParallelOp::addNumGangsOperands(
  ...
  if (getNumGangsSegments())
    toolchain::copy(*getNumGangsSegments(), std::back_inserter(segments));
...
void acc::ParallelOp::addWaitOperands(
  ...
  if (getWaitOperandsSegments())
    toolchain::copy(*getWaitOperandsSegments(), std::back_inserter(segments));
  ...
  toolchain::SmallVector<mlir::Attribute> hasDevnums;
  if (getHasWaitDevnumAttr())
    toolchain::copy(getHasWaitDevnumAttr(), std::back_inserter(hasDevnums));
  hasDevnums.insert(
      hasDevnums.end(),
...
static ParseResult parseNumGangs(
    ...
    toolchain::SmallVectorImpl<mlir::OpAsmParser::UnresolvedOperand> &operands,
    toolchain::SmallVectorImpl<Type> &types, mlir::ArrayAttr &deviceTypes,
    mlir::DenseI32ArrayAttr &segments) {
  toolchain::SmallVector<DeviceTypeAttr> attributes;
  toolchain::SmallVector<int32_t> seg;
...
static void printNumGangs(mlir::OpAsmPrinter &p, mlir::Operation *op,
  ...
  unsigned opIdx = 0;
  toolchain::interleaveComma(toolchain::enumerate(*deviceTypes), p, [&](auto it) {
    p << "{";
    toolchain::interleaveComma(
        toolchain::seq<int32_t>(0, (*segments)[it.index()]), p, [&](auto it) {
...
static ParseResult parseWaitClause(
  ...
  toolchain::SmallVector<mlir::Attribute> deviceTypeAttrs, keywordAttrs, devnum;
  toolchain::SmallVector<int32_t> seg;
...
printDeviceTypeOperands(mlir::OpAsmPrinter &p, mlir::Operation *op,
  ...
  if (!hasDeviceTypeValues(deviceTypes))
    return;
  toolchain::interleaveComma(toolchain::zip(*deviceTypes, operands), p, [&](auto it) {
...
static ParseResult parseDeviceTypeOperandsWithKeywordOnly(
  ...
  toolchain::SmallVector<mlir::Attribute> keywordOnlyDeviceTypeAttributes;
  bool needCommaBeforeOperands = false;
...
static ParseResult parseOperandsWithKeywordOnly(
    mlir::OpAsmParser &parser,
    toolchain::SmallVectorImpl<mlir::OpAsmParser::UnresolvedOperand> &operands,
    toolchain::SmallVectorImpl<Type> &types, mlir::UnitAttr &attr) {
  // Keyword only
  if (failed(parser.parseOptionalLParen())) {
    attr = mlir::UnitAttr::get(parser.getContext());
    return success();
...
static void printOperandsWithKeywordOnly(mlir::OpAsmPrinter &p,
  ...
  p << "(";
  toolchain::interleaveComma(operands, p, [&](auto it) { p << it; });
  p << " : ";
  toolchain::interleaveComma(types, p, [&](auto it) { p << it; });
  p << ")";
}
...
LogicalResult acc::HostDataOp::verify() {
  ...
  toolchain::SmallPtrSet<mlir::Value, 4> seenVars;
...
static ParseResult parseGangValue(
    OpAsmParser &parser, toolchain::StringRef keyword,
    toolchain::SmallVectorImpl<mlir::OpAsmParser::UnresolvedOperand> &operands,
    toolchain::SmallVectorImpl<Type> &types,
    toolchain::SmallVector<GangArgTypeAttr> &attributes, GangArgTypeAttr gangArgType,
...
static ParseResult parseGangClause(
    OpAsmParser &parser,
    toolchain::SmallVectorImpl<mlir::OpAsmParser::UnresolvedOperand> &gangOperands,
    toolchain::SmallVectorImpl<Type> &gangOperandsType, mlir::ArrayAttr &gangArgType,
    mlir::ArrayAttr &deviceType, mlir::DenseI32ArrayAttr &segments,
    mlir::ArrayAttr &gangOnlyDeviceType) {
  toolchain::SmallVector<GangArgTypeAttr> gangArgTypeAttributes;
  toolchain::SmallVector<mlir::Attribute> deviceTypeAttributes;
  toolchain::SmallVector<mlir::Attribute> gangOnlyDeviceTypeAttributes;
  toolchain::SmallVector<int32_t> seg;
  ...
  toolchain::SmallVector<mlir::Attribute> arrayAttr(gangArgTypeAttributes.begin(),
  ...
  toolchain::SmallVector<mlir::Attribute> gangOnlyAttr(
...
bool hasDuplicateDeviceTypes(
    ...
    toolchain::SmallSet<mlir::acc::DeviceType, 3> &deviceTypes) {
  if (!segments)
    return false;
  for (auto attr : *segments) {
    auto deviceTypeAttr = mlir::dyn_cast<mlir::acc::DeviceTypeAttr>(attr);
...
checkDeviceTypes(mlir::ArrayAttr deviceTypes) {
  toolchain::SmallSet<mlir::acc::DeviceType, 3> crtDeviceTypes;
  if (!deviceTypes)
    return std::nullopt;
...
LogicalResult acc::LoopOp::verify() {
  ...
  bool hasDefaultSeq =
      getSeqAttr()
          ? toolchain::any_of(getSeqAttr().getAsRange<mlir::acc::DeviceTypeAttr>(),
                         hasDeviceNone)
          : false;
  ...
  bool hasDefaultAuto =
      getAuto_Attr()
          ? toolchain::any_of(getAuto_Attr().getAsRange<mlir::acc::DeviceTypeAttr>(),
                         hasDeviceNone)
          : false;
...
toolchain::SmallVector<mlir::Region *> acc::LoopOp::getLoopRegions() {
  return {&getRegion()};
}
...
void printLoopControl(OpAsmPrinter &p, Operation *op, Region &region,
  ...
  if (!regionArgs.empty()) {
    ...
    toolchain::interleaveComma(regionArgs, p,
...
void acc::LoopOp::setCollapseForDeviceTypes(
  ...
  toolchain::SmallVector<mlir::Attribute> newValues;
  toolchain::SmallVector<mlir::Attribute> newDeviceTypes;
  ...
  if (getCollapseAttr()) {
    for (const auto &existing :
         toolchain::zip_equal(getCollapseAttr(), getCollapseDeviceTypeAttr())) {
...
void acc::LoopOp::setTileForDeviceTypes(
  ...
  if (getTileOperandsSegments())
    toolchain::copy(*getTileOperandsSegments(), std::back_inserter(segments));
...
bool acc::LoopOp::hasParallelismFlag(DeviceType dt) {
  ...
  auto testFromArr = [=](ArrayAttr arr) -> bool {
    return toolchain::any_of(arr.getAsRange<DeviceTypeAttr>(), hasDevice);
  };
...
void acc::LoopOp::addGangOperands(
    MLIRContext *context, toolchain::ArrayRef<DeviceType> effectiveDeviceTypes,
    toolchain::ArrayRef<GangArgType> argTypes, mlir::ValueRange values) {
  toolchain::SmallVector<int32_t> segments;
  if (std::optional<ArrayRef<int32_t>> existingSegments =
          getGangOperandsSegments())
    toolchain::copy(*existingSegments, std::back_inserter(segments));
  ...
  if (numAdded > 0) {
    toolchain::SmallVector<mlir::Attribute> gangTypes;
    if (getGangOperandsArgTypeAttr())
      toolchain::copy(getGangOperandsArgTypeAttr(), std::back_inserter(gangTypes));

    for (auto i : toolchain::index_range(0u, numAdded)) {
      toolchain::transform(argTypes, std::back_inserter(gangTypes),
...
static ParseResult parseBindName(OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::Attribute> bindIdNameAttrs;
  toolchain::SmallVector<mlir::Attribute> bindStrNameAttrs;
  toolchain::SmallVector<mlir::Attribute> deviceIdTypeAttrs;
  toolchain::SmallVector<mlir::Attribute> deviceStrTypeAttrs;
...
static void printBindName(mlir::OpAsmPrinter &p, mlir::Operation *op,
  ...
  toolchain::SmallVector<mlir::Attribute> allBindNames;
  toolchain::SmallVector<mlir::Attribute> allDeviceTypes;
  ...
  if (!allBindNames.empty())
    toolchain::interleaveComma(toolchain::zip(allBindNames, allDeviceTypes), p,
                          [&](const auto &pair) {
                            p << std::get<0>(pair);
...
static ParseResult parseRoutineGangClause(OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::Attribute> gangAttrs, gangDimAttrs,
...
void printRoutineGangClause(OpAsmPrinter &p, Operation *op,
  ...
  if (hasDeviceTypeValues(gangDimDeviceTypes))
    toolchain::interleaveComma(toolchain::zip(*gangDim, *gangDimDeviceTypes), p,
                          [&](const auto &pair) {
...
printDeviceTypeArrayAttr(mlir::OpAsmPrinter &p, mlir::Operation *op,
  ...
  p << "([";
  toolchain::interleaveComma(*deviceTypes, p, [&](mlir::Attribute attr) {
...
void RoutineOp::addGang(MLIRContext *context,
                        toolchain::ArrayRef<DeviceType> effectiveDeviceTypes,
                        uint64_t val) {
  toolchain::SmallVector<mlir::Attribute> dimValues;
  toolchain::SmallVector<mlir::Attribute> deviceTypes;

  if (getGangDimAttr())
    toolchain::copy(getGangDimAttr(), std::back_inserter(dimValues));
  if (getGangDimDeviceTypeAttr())
    toolchain::copy(getGangDimDeviceTypeAttr(), std::back_inserter(deviceTypes));
...
void RoutineOp::addBindStrName(MLIRContext *context,
  ...
  toolchain::SmallVector<mlir::Attribute> vals;
  if (getBindStrNameAttr())
    toolchain::copy(getBindStrNameAttr(), std::back_inserter(vals));
...
void RoutineOp::addBindIDName(MLIRContext *context,
  ...
  if (getBindIdNameAttr())
    toolchain::copy(getBindIdNameAttr(), std::back_inserter(vals));
  for (unsigned i = 0; i < after - before; ++i)
    vals.push_back(val);
...
void acc::InitOp::addDeviceType(MLIRContext *context,
  ...
  if (getDeviceTypesAttr())
    toolchain::copy(getDeviceTypesAttr(), std::back_inserter(deviceTypes));
...
mlir::acc::getVarPtr(mlir::Operation *accDataClauseOp) {
  auto varPtr{toolchain::TypeSwitch<mlir::Operation *,
                               mlir::TypedValue<mlir::acc::PointerLikeType>>(
                  accDataClauseOp)
...
mlir::Value mlir::acc::getVar(mlir::Operation *accDataClauseOp) {
  auto varPtr{
      toolchain::TypeSwitch<mlir::Operation *, mlir::Value>(accDataClauseOp)
...
mlir::Type mlir::acc::getVarType(mlir::Operation *accDataClauseOp) {
  auto varType{toolchain::TypeSwitch<mlir::Operation *, mlir::Type>(accDataClauseOp)
                   .Case<ACC_DATA_ENTRY_OPS>(
...
mlir::acc::getAccPtr(mlir::Operation *accDataClauseOp) {
  auto accPtr{toolchain::TypeSwitch<mlir::Operation *,
                               mlir::TypedValue<mlir::acc::PointerLikeType>>(
                  accDataClauseOp)
...
mlir::Value mlir::acc::getAccVar(mlir::Operation *accDataClauseOp) {
  auto accPtr{toolchain::TypeSwitch<mlir::Operation *, mlir::Value>(accDataClauseOp)
                  .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>(
...
mlir::acc::getBounds(mlir::Operation *accDataClauseOp) {
  mlir::SmallVector<mlir::Value> bounds{
      toolchain::TypeSwitch<mlir::Operation *, mlir::SmallVector<mlir::Value>>(
          accDataClauseOp)
...
mlir::acc::getAsyncOperands(mlir::Operation *accDataClauseOp) {
  return toolchain::TypeSwitch<mlir::Operation *, mlir::SmallVector<mlir::Value>>(
             accDataClauseOp)
      .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>([&](auto dataClause) {
        return mlir::SmallVector<mlir::Value>(
...
mlir::acc::getAsyncOperandsDeviceType(mlir::Operation *accDataClauseOp) {
  return toolchain::TypeSwitch<mlir::Operation *, mlir::ArrayAttr>(accDataClauseOp)
      .Case<ACC_DATA_ENTRY_OPS, ACC_DATA_EXIT_OPS>([&](auto dataClause) {
...
std::optional<toolchain::StringRef> mlir::acc::getVarName(mlir::Operation *accOp) {
  auto name{
      toolchain::TypeSwitch<mlir::Operation *, std::optional<toolchain::StringRef>>(accOp)
          .Case<ACC_DATA_ENTRY_OPS>([&](auto entry) { return entry.getName(); })
          .Default([&](mlir::Operation *) -> std::optional<toolchain::StringRef> {
            return {};
          })};
  return name;
}
...
mlir::acc::getDataClause(mlir::Operation *accDataEntryOp) {
  auto dataClause{
      toolchain::TypeSwitch<mlir::Operation *, std::optional<mlir::acc::DataClause>>(
          accDataEntryOp)
          .Case<ACC_DATA_ENTRY_OPS>(
...
bool mlir::acc::getImplicitFlag(mlir::Operation *accDataEntryOp) {
  auto implicit{toolchain::TypeSwitch<mlir::Operation *, bool>(accDataEntryOp)
                    .Case<ACC_DATA_ENTRY_OPS>(
...
mlir::ValueRange mlir::acc::getDataOperands(mlir::Operation *accOp) {
  auto dataOperands{
      toolchain::TypeSwitch<mlir::Operation *, mlir::ValueRange>(accOp)
...
mlir::acc::getMutableDataOperands(mlir::Operation *accOp) {
  auto dataOperands{
      toolchain::TypeSwitch<mlir::Operation *, mlir::MutableOperandRange>(accOp)
          .Case<ACC_COMPUTE_AND_DATA_CONSTRUCT_OPS>(
...
mlir::SymbolRefAttr mlir::acc::getRecipe(mlir::Operation *accOp) {
  auto recipe{
      toolchain::TypeSwitch<mlir::Operation *, mlir::SymbolRefAttr>(accOp)
          .Case<ACC_DATA_ENTRY_OPS>(
