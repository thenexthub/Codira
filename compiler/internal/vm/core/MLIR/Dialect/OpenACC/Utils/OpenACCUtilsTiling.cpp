...
static void moveOpsAndReplaceIVs(mlir::acc::LoopOp sourceLoop,
                                 ...
                                 toolchain::ArrayRef<mlir::Value> newIVs,
                                 toolchain::ArrayRef<mlir::Value> origIVs,
  ...
  for (auto [i, newIV] : toolchain::enumerate(newIVs))
    mlir::replaceAllUsesInRegionWith(origIVs[i], newIV, targetLoop.getRegion());
}

mlir::acc::LoopOp
mlir::acc::tileACCLoops(toolchain::SmallVector<mlir::acc::LoopOp> &tileLoops,
                        const toolchain::SmallVector<mlir::Value> &tileSizes,
  ...
  toolchain::SmallVector<mlir::Value, 3> origIVs;
  toolchain::SmallVector<mlir::Value, 3> origSteps;
  toolchain::SmallVector<mlir::Value, 3> origUBs;
  toolchain::SmallVector<mlir::Value, 3> newSteps;
  toolchain::SmallVector<mlir::Value, 3> newUBs;
  toolchain::SmallVector<mlir::Value, 3> newIVs;
  size_t nOps = innerLoop.getBody().getOperations().size();

  // Extract original inclusiveUBs
  toolchain::SmallVector<bool> inclusiveUBs;
  for (auto tileLoop : tileLoops) {
    for (auto [j, step] : toolchain::enumerate(tileLoop.getStep())) {
...
toolchain::SmallVector<mlir::acc::LoopOp>
mlir::acc::uncollapseLoops(mlir::acc::LoopOp origLoop, unsigned tileCount,
                           unsigned collapseCount,
                           mlir::RewriterBase &rewriter) {
  toolchain::SmallVector<mlir::acc::LoopOp> newLoops;
  toolchain::SmallVector<mlir::Value, 3> newIVs;
  mlir::Location loc = origLoop.getLoc();
  toolchain::SmallVector<bool> newInclusiveUBs;
  toolchain::SmallVector<mlir::Value, 3> lbs, ubs, steps;
