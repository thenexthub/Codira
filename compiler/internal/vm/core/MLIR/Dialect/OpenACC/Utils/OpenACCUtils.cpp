...
static bool isOnlyUsedByOpClauses(mlir::Value val, mlir::Region &region) {
  ...
  return toolchain::all_of(val.getUsers(), checkIfUsedOnlyByOpInside);
}
...
mlir::acc::getDefaultAttr(Operation *op) {
  ...
  while (!defaultAttr.has_value() && currOp) {
    defaultAttr =
        toolchain::TypeSwitch<mlir::Operation *,
...
std::string mlir::acc::getRecipeName(mlir::acc::RecipeKind kind,
  ...
  if (!toolchain::isa<mlir::acc::PointerLikeType, mlir::acc::MappableType>(type))
    return "";

  std::string recipeName;
  toolchain::raw_string_ostream ss(recipeName);
...
bool mlir::acc::isValidSymbolUse(mlir::Operation *user,
  ...
  if (auto func =
    ...
    // 3. Name starts with "toolchain."
    // 4. The function's name is a valid LLVM intrinsic name
    if (func.getVisibility() == mlir::SymbolTable::Visibility::Private &&
        func.getFunctionBody().empty() && func.getName().starts_with("toolchain.") &&
        toolchain::Intrinsic::lookupIntrinsicID(func.getName()) !=
            toolchain::Intrinsic::not_intrinsic)
      return true;
  }
...
mlir::acc::getDominatingDataClauses(mlir::Operation *computeConstructOp,
  ...
  toolchain::SmallSetVector<mlir::Value, 8> dominatingDataClauses;

  toolchain::TypeSwitch<mlir::Operation *>(computeConstructOp)
  ...
  funcOp->walk([&](mlir::acc::DeclareEnterOp declareEnterOp) {
    if (domInfo.dominates(declareEnterOp.getOperation(), computeConstructOp)) {
      ...
      toolchain::SmallVector<mlir::acc::DeclareExitOp> exits;
      ...
      // compute construct.
      if (!exits.empty() &&
          toolchain::all_of(exits, [&](mlir::acc::DeclareExitOp exitOp) {
...
mlir::acc::emitRemark(mlir::Operation *op, const toolchain::Twine &message,
