...
static bool isRematerializationCandidate(Value val,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "\tChecking candidate: " << *definingOp << "\n");

  // Constants are trivial and useful to rematerialize.
  if (matchPattern(definingOp, m_Constant())) {
    LLVM_DEBUG(toolchain::dbgs() << "\t\t-> constant pattern matched\n");
    return true;
  }
  ...
  if (isa<acc::OutlineRematerializationOpInterface>(definingOp)) {
    LLVM_DEBUG(toolchain::dbgs() << "\t\t-> OutlineRematerializationOpInterface\n");
    return true;
  }
  ...
  if (auto addrOfOp = dyn_cast<acc::AddressOfGlobalOpInterface>(definingOp)) {
    ...
    if (accSupport.isValidSymbolUse(definingOp, symbol, &globalOp)) {
      LLVM_DEBUG(toolchain::dbgs() << "\t\t-> isValidSymbolUse: true\n");
      return true;
    }
    LLVM_DEBUG(toolchain::dbgs() << "\t\t-> isValidSymbolUse: false\n");
    ...
    if (globalOp) {
      if (auto globalVarOp =
        ...
        if (globalVarOp.isConstant()) {
          LLVM_DEBUG(toolchain::dbgs() << "\t\t-> constant global\n");
          return true;
        }
      }
    }
  ...
  LLVM_DEBUG(toolchain::dbgs() << "\t\t-> not a candidate\n");
  return false;
}
...
public:
  ...
  bool canonicalizeLiveInValues(Region &region,
    ...
    SmallVector<Value> sinkCandidates(
        toolchain::make_filter_range(liveInValues, isSinkCandidate));
    SmallVector<Value> rematerializationCandidates(
        toolchain::make_filter_range(liveInValues, isCloneCandidate));

    LLVM_DEBUG(toolchain::dbgs() << "\tSink candidates: " << sinkCandidates.size()
                            << ", clone candidates: "
    ...
    LLVM_DEBUG(toolchain::dbgs() << "\tCanonicalizing values into "
                            << *region.getParentOp() << "\n");
    ...
    for (Value sinkCandidate : sinkCandidates) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "\t\tSunk: " << *sinkOp << "\n");
    }
  ...
  void runOnOperation() override {
    LLVM_DEBUG(toolchain::dbgs() << "Enter OffloadLiveInValueCanonicalization\n");
    ...
    func.walk([&](Operation *op) {
      if (isa<acc::OffloadRegionOpInterface>(op)) {
        ...
        do {
          LLVM_DEBUG(toolchain::dbgs() << "\tIteration " << iteration++ << "\n");
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Exit OffloadLiveInValueCanonicalization\n");
  }
};
