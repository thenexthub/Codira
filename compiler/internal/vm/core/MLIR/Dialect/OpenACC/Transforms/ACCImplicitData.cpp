...
static void legalizeValuesInRegion(Region &accRegion,
  ...
  for (Value dataClause :
       toolchain::concat<Value>(newDataClauseOperands, newPrivateOperands)) {
...
// https://mlir.toolchain.org/docs/Dialects/OpenACCDialect/#operation-categories
// Key ones used here:
...
generateDataExitOperations(OpBuilder &builder, Operation *accOp,
  ...
  for (auto dataEntry : toolchain::reverse(sortedDataClauseOperands)) {
    if (toolchain::find(newDataClauseOperands, dataEntry) ==
...
void ACCImplicitData::generateImplicitDataOps(
  ...
  auto candidateVars(
      toolchain::to_vector(toolchain::make_filter_range(liveInValues, isCandidate)));
  ...
  if (!candidateVars.empty()) {
    LLVM_DEBUG(toolchain::dbgs() << "== Generating clauses for ==\n"
                            << computeConstructOp << "\n");
  }
  ...
  for (auto var : candidateVars) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Generated data clause for " << var << ":\n"
                            << "\t" << *newDataClauseOp << "\n");
