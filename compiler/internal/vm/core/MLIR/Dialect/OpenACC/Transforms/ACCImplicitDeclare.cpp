...
using GlobalOpSetT = toolchain::SmallSetVector<Operation *, 16>;
  ...
  accOp.walk([&](acc::AddressOfGlobalOpInterface addrOfOp) {
    ...
    if (symRef) {
      ...
      if (isGlobalUseCandidateForHoisting(globalOp, addrOfOp, symRef,
        ...
        LLVM_DEBUG(
            toolchain::dbgs() << "Hoisted:\n\t" << addrOfOp << "\n\tfrom:\n\t";
            accOp->print(toolchain::dbgs(),
...
static void collectGlobalsFromDeviceRegion(Region &region,
  ...
  region.walk([&](Operation *op) {
    ...
    if (addrOfOp) {
      ...
      toolchain::SmallVector<SymbolRefAttr> symbols;
...
public:
  ...
  void runOnOperation() override {
    ...
    for (Operation *globalOp : globalsToAccDeclare) {
      LLVM_DEBUG(
          toolchain::dbgs() << "Global is being `acc declare copyin`d: ";
          globalOp->print(toolchain::dbgs(),
                          OpPrintingFlags{}.skipRegions().enableDebugInfo());
          toolchain::dbgs() << "\n");
