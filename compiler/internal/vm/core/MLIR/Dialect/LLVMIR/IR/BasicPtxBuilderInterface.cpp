  ...
  auto getRegisterTypeForScalar = [&](Type type) -> FailureOr<char> {
    ...
    mlir::emitError(
        ...
        << " is not supported. Supported types are:"
           ...
           "pointers.\nPlease use toolchain.bitcast if you have different type. "
...
static SmallVector<Value> extractStructElements(PatternRewriter &rewriter,
  ...
  for (unsigned i : toolchain::seq<unsigned>(0, structTy.getBody().size()))
...
LogicalResult PtxBuilder::insertValue(Value v, PTXRegisterMod itype) {
  ...
  toolchain::raw_string_ostream ss(registerConstraints);
  // Handle Structs
  if (auto stype = dyn_cast<LLVM::LLVMStructType>(v.getType())) {
    if (itype == PTXRegisterMod::Write) {
      addValue(v);
    }
    for (auto [idx, t] : toolchain::enumerate(stype.getBody())) {
      if (itype != PTXRegisterMod::Write) {
        Value extractValue =
...
needsPackUnpack(BasicPtxBuilderInterface interfaceOp,
  ...
  const unsigned readWriteVals =
      toolchain::count_if(registerModifiers, [](PTXRegisterMod m) {
...
packResultTypes(BasicPtxBuilderInterface interfaceOp,
  ...
  if (!needsPackUnpack(interfaceOp, needsManualRegisterMapping,
    ...
    for (auto [m, v] : toolchain::zip(registerModifiers, ptxOperands))
...
static std::string canonicalizeRegisterConstraints(toolchain::StringRef csv) {
  SmallVector<toolchain::StringRef> toks;
...
constexpr toolchain::StringLiteral kReadWritePrefix{"rw"};
constexpr toolchain::StringLiteral kWriteOnlyPrefix{"w"};
constexpr toolchain::StringLiteral kReadOnlyPrefix{"r"};

/// Returns a regex that matches {$rwN}, {$wN}, {$rN}
static toolchain::Regex getPredicateMappingRegex() {
  toolchain::Regex rx(toolchain::formatv(R"(\{\$({0}|{1}|{2})([0-9]+)\})",
...
void mlir::NVVM::countPlaceholderNumbers(
    StringRef ptxCode, toolchain::SmallDenseSet<unsigned int> &seenRW,
    toolchain::SmallDenseSet<unsigned int> &seenW,
    toolchain::SmallDenseSet<unsigned int> &seenR,
    toolchain::SmallVectorImpl<unsigned int> &rwNums,
    toolchain::SmallVectorImpl<unsigned int> &wNums,
    toolchain::SmallVectorImpl<unsigned int> &rNums) {

  toolchain::Regex rx = getPredicateMappingRegex();
...
///       selp.s32 $3,   $4, $5, p;
/// }\n"
static std::string rewriteAsmPlaceholders(toolchain::StringRef ptxCode) {
  toolchain::SmallDenseSet<unsigned> seenRW, seenW, seenR;
  toolchain::SmallVector<unsigned> rwNums, wNums, rNums;
  ...
  toolchain::sort(rwNums);
  toolchain::sort(wNums);
  toolchain::sort(rNums);

  // Step 3. Create mapping from original to new IDs
  toolchain::DenseMap<unsigned, unsigned> rwMap, wMap, rMap;
  unsigned nextId = 0;
...
LLVM::InlineAsmOp PtxBuilder::build() {
  ...
  // Replace all % with $
  toolchain::replace(ptxInstruction, '%', '$');
...
void PtxBuilder::buildAndReplaceOp() {
  ...
  const bool hasRW = toolchain::any_of(registerModifiers, [](PTXRegisterMod m) {
    return m == PTXRegisterMod::ReadWrite;
  });
