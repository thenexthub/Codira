...
/// internal functions to avoid getting a verbose `!toolchain` prefix. Otherwise
/// prints it as usual.
static void dispatchPrint(AsmPrinter &printer, Type type) {
  if (isCompatibleType(type) &&
      !(toolchain::isa<IntegerType, FloatType, VectorType>(type) ||
        (toolchain::isa<PtrLikeTypeInterface>(type) &&
         !toolchain::isa<LLVMPointerType>(type))))
...
static StringRef getTypeKeyword(Type type) {
  return TypeSwitch<Type, StringRef>(type)
      ...
      .DefaultUnreachable("unexpected 'toolchain' type kind");
}
...
void LLVMStructType::print(AsmPrinter &printer) const {
  ...
  if (isIdentified()) {
    ...
    printer << '"';
    toolchain::printEscapedString(getName(), printer.getStream());
    printer << '"';
  ...
  printer << '(';
  toolchain::interleaveComma(getBody(), printer.getStream(),
...
void mlir::LLVM::detail::printType(Type type, AsmPrinter &printer) {
  ...
  toolchain::TypeSwitch<Type>(type)
...
///   toolchain-type ::= `struct<` (string-literal `,`)? `packed`?
///                 `(` toolchain-type-list `)` `>`
...
/// will try to parse any type in full form (including types with the `!toolchain`
/// prefix), and on failure fall back to parsing the short-hand version of the
/// LLVM dialect types without the `!toolchain` prefix.
