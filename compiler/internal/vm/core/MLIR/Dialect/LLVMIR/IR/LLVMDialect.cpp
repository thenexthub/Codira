...
static auto processFMFAttr(ArrayRef<NamedAttribute> attrs) {
  SmallVector<NamedAttribute, 8> filteredAttrs(
      toolchain::make_filter_range(attrs, [&](NamedAttribute attr) {
...
static int parseOptionalKeywordAlternative(OpAsmParser &parser,
  ...
  for (const auto &en : toolchain::enumerate(keywords)) {
...
static ArrayAttr getLLVMAlignParamForCompressExpand(OpBuilder &builder,
  ...
  // From
  // https://toolchain.org/docs/LangRef.html#toolchain-masked-expandload-intrinsics
  // https://toolchain.org/docs/LangRef.html#toolchain-masked-compressstore-intrinsics
  //
...
static void printOneOpBundle(OpAsmPrinter &p, OperandRange operands,
  ...
  if (!operands.empty()) {
    ...
    p << " : ";
    toolchain::interleaveComma(operandTypes, p);
  }
...
static void printOpBundles(OpAsmPrinter &p, Operation *op,
  ...
  toolchain::interleaveComma(
      toolchain::zip(opBundleOperands, opBundleOperandTypes, *opBundleTags), p,
      [&p](auto bundle) {
...
// <operation> ::= `toolchain.icmp` string-literal ssa-use `,` ssa-use
//                 attribute-dict? `:` type
// <operation> ::= `toolchain.fcmp` string-literal ssa-use `,` ssa-use
//                 attribute-dict? `:` type
...
// <operation> ::= `toolchain.alloca` `inalloca`? ssa-use `x` type
...
ParseResult AllocaOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (alignmentAttr.has_value()) {
    auto alignmentInt = toolchain::dyn_cast<IntegerAttr>(alignmentAttr->getValue());
    if (!alignmentInt)
  ...
  auto funcType = toolchain::dyn_cast<FunctionType>(type);
  ...
  if (auto ptrResultType = toolchain::dyn_cast<LLVMPointerType>(resultType))
...
static void printSwitchOpCases(OpAsmPrinter &p, SwitchOp op, Type flagType,
  ...
  toolchain::interleave(
      toolchain::zip(caseValues, caseDestinations),
      [&](auto i) {
        p << "  ";
        p << std::get<0>(i);
        p << ": ";
...
static Type extractVectorElementType(Type type) {
  if (auto vectorType = toolchain::dyn_cast<VectorType>(type))
...
static void destructureIndices(Type currType, ArrayRef<GEPArg> indices,
  ...
  for (const GEPArg &iter : indices) {
    ...
    if (Value val = toolchain::dyn_cast_if_present<Value>(iter)) {
      APInt intC;
...
static void printGEPIndices(OpAsmPrinter &printer, LLVM::GEPOp gepOp,
  ...
  toolchain::interleaveComma(
      ...
      [&](PointerUnion<IntegerAttr, Value> cst) {
        if (Value val = toolchain::dyn_cast_if_present<Value>(cst))
...
LogicalResult LLVM::GEPOp::verify() {
  if (static_cast<size_t>(
          toolchain::count(getRawConstantIndices(), kDynamicIndex)) !=
...
bool LLVM::isTypeCompatibleWithAtomicOp(Type type,
  ...
  toolchain::TypeSize bitWidth = dataLayout.getTypeSizeInBits(type);
...
verifyAtomicMemOp(OpTy memOp, Type valueType,
  ...
  if (memOp.getOrdering() != AtomicOrdering::not_atomic) {
    ...
    if (toolchain::is_contained(unsupportedOrderings, memOp.getOrdering()))
...
static LLVMFunctionType getLLVMFuncType(MLIRContext *context, TypeRange results,
  ...
  return LLVMFunctionType::get(resultType, toolchain::to_vector(args.getTypes()),
                               /*isVarArg=*/false);
}
...
static LogicalResult verifyCallOpVarCalleeType(OpTy callOp) {
  ...
  for (auto [paramType, operand] :
       toolchain::zip(varCalleeType->getParams(), callOp.getArgOperands()))
...
static LogicalResult verifyOperandBundles(OpType &op) {
  ...
  if (opBundleTags && !toolchain::all_of(*opBundleTags, isStringAttr))
...
LogicalResult CallOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!calleeName) {
    ...
    auto ptrType = toolchain::dyn_cast<LLVMPointerType>(getOperand(0).getType());
    if (!ptrType)
  ...
  LLVMFunctionType funcType = toolchain::dyn_cast<LLVMFunctionType>(fnType);
  if (!funcType)
  ...
  if (getNumResults() == 0 &&
      !toolchain::isa<LLVM::LLVMVoidType>(funcType.getReturnType()))
...
static ParseResult parseCallTypeAndResolveOperands(
  ...
  if (resTypes.size() == 1 && toolchain::isa<LLVM::LLVMVoidType>(resTypes[0]))
    return parser.emitError(trailingTypesLoc,
  ...
  // operands.
  toolchain::append_range(types, argTypes);
...
static ParseResult resolveOpBundleOperands(
  ...
  for (const auto &[operands, types] :
       toolchain::zip_equal(opBundleOperands, opBundleOperandTypes)) {
...
// <operation> ::= `toolchain.call` (cconv)? (tailcallkind)? (function-id | ssa-use)
//                             `(` ssa-use-list `)`
...
LogicalResult InvokeOp::verify() {
  ...
  if (!isa<LandingpadOp>(unwindDest->front()))
    return emitError("first operation in unwind destination should be a "
                     "toolchain.landingpad operation");
...
// <operation> ::= `toolchain.invoke` (cconv)? (function-id | ssa-use)
//                  `(` ssa-use-list `)`
...
LogicalResult LandingpadOp::verify() {
  ...
  if (LLVMFuncOp func = (*this)->getParentOfType<LLVMFuncOp>()) {
    if (!func.getPersonality())
      return emitError(
          "toolchain.landingpad needs to be in a function with a personality");
  }

  // Consistency of toolchain.landingpad result types is checked in
  // LLVMFuncOp::verify().
  ...
  for (unsigned idx = 0, ie = getNumOperands(); idx < ie; idx++) {
    ...
    bool isFilter = toolchain::isa<LLVMArrayType>(value.getType());
    if (isFilter) {
...
void LandingpadOp::print(OpAsmPrinter &p) {
  ...
  for (auto value : getOperands()) {
    // Similar to toolchain - if clause is an array type then it is filter
    // clause else catch clause
    bool isArrayTy = toolchain::isa<LLVMArrayType>(value.getType());
...
// <operation> ::= `toolchain.landingpad` `cleanup`?
...
static Type getInsertExtractValueElementType(
  ...
  for (int64_t idx : position) {
    if (auto arrayType = toolchain::dyn_cast<LLVMArrayType>(llvmType)) {
    ...
    } else if (auto structType = toolchain::dyn_cast<LLVMStructType>(llvmType)) {
      if (idx < 0 ||
...
static Type getInsertExtractValueElementType(Type llvmType,
                                             ArrayRef<int64_t> position) {
  for (int64_t idx : position) {
    if (auto structType = toolchain::dyn_cast<LLVMStructType>(llvmType))
      llvmType = structType.getBody()[idx];
    else
      llvmType = toolchain::cast<LLVMArrayType>(llvmType).getElementType();
  }
  return llvmType;
}
...
OpFoldResult LLVM::ExtractValueOp::fold(FoldAdaptor adaptor) {
  ...
  while (auto insertValueOp = dyn_cast_if_present<InsertValueOp>(container)) {
    ...
    // ```
    // %0 = toolchain.insertvalue %arg1, %undef[0] : !toolchain.struct<(i32, i32, i32)>
    // %1 = toolchain.insertvalue %arg2, %0[1] : !toolchain.struct<(i32, i32, i32)>
    // %2 = toolchain.insertvalue %arg3, %1[2] : !toolchain.struct<(i32, i32, i32)>
    // %3 = toolchain.insertvalue %2, %foo[0]
    //     : !toolchain.struct<(struct<(i32, i32, i32)>, i64)>
    // %4 = toolchain.extractvalue %3[0, 0]
    //     : !toolchain.struct<(struct<(i32, i32, i32)>, i64)>
    // ```
    ...
    // following example:
    // ```
    //   %1 = toolchain.insertvalue %f0, %0[0, 0] :
    //     !toolchain.array<4 x !toolchain.array<4 x f32>>
    //   %2 = toolchain.insertvalue %arr, %1[0] :
    //     !toolchain.array<4 x !toolchain.array<4 x f32>>
    //   %3 = toolchain.extractvalue %2[0, 0] : !toolchain.array<4 x !toolchain.array<4 x f32>>
    // ```
...
LogicalResult ReturnOp::verify() {
  ...
  if (toolchain::isa<LLVMVoidType>(expectedType)) {
    if (!getArg())
      return success();
...
AddressOfOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!global && !function && !alias && !ifunc)
    return emitOpError("must reference a global defined by 'toolchain.mlir.global', "
                       "'toolchain.mlir.alias' or 'toolchain.func' or 'toolchain.mlir.ifunc'");
...
DSOLocalEquivalentOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!function && !alias)
    return emitOpError(
        "must reference a global defined by 'toolchain.func' or 'toolchain.mlir.alias'");
...
void GlobalOp::print(OpAsmPrinter &p) {
  ...
  // (as defined in https://mlir.toolchain.org/docs/LangRef/#attributes)
  p.printOptionalAttrDict((*this)->getAttrs(),
  ...
  if (toolchain::dyn_cast_or_null<StringAttr>(getValueOrNull()))
    return;
  p << " : " << getType();
...
static LogicalResult verifyBlockTags(LLVMFuncOp funcOp) {
  toolchain::DenseSet<BlockTagAttr> blockTags;
...
// operation ::= `toolchain.mlir.global` linkage? visibility?
...
ParseResult GlobalOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (types.empty()) {
    if (auto strAttr = toolchain::dyn_cast_or_null<StringAttr>(value)) {
...
static bool isZeroAttribute(Attribute value) {
  if (auto intValue = toolchain::dyn_cast<IntegerAttr>(value))
    return intValue.getValue().isZero();
  if (auto fpValue = toolchain::dyn_cast<FloatAttr>(value))
    return fpValue.getValue().isZero();
  if (auto splatValue = toolchain::dyn_cast<SplatElementsAttr>(value))
    return isZeroAttribute(splatValue.getSplatValue<Attribute>());
  if (auto elementsValue = toolchain::dyn_cast<ElementsAttr>(value))
    return toolchain::all_of(elementsValue.getValues<Attribute>(), isZeroAttribute);
  if (auto arrayValue = toolchain::dyn_cast<ArrayAttr>(value))
    return toolchain::all_of(arrayValue.getValue(), isZeroAttribute);
  return false;
}

LogicalResult GlobalOp::verify() {
  bool validType = isCompatibleOuterType(getType())
                       ? !toolchain::isa<LLVMVoidType, LLVMTokenType,
                                    LLVMMetadataType, LLVMLabelType>(getType())
                       : toolchain::isa<PointerElementTypeInterface>(getType());
  if (!validType)
    return emitOpError(
  ...
  if (auto strAttr = toolchain::dyn_cast_or_null<StringAttr>(getValueOrNull())) {
    auto type = toolchain::dyn_cast<LLVMArrayType>(getType());
    IntegerType elementType =
        type ? toolchain::dyn_cast<IntegerType>(type.getElementType()) : nullptr;
  ...
  if (getLinkage() == Linkage::Appending) {
    if (!toolchain::isa<LLVMArrayType>(getType())) {
  ...
  if (alignAttr.has_value()) {
    ...
    if (!toolchain::isPowerOf2_64(value))
...
static LogicalResult checkGlobalXtorData(Operation *op, ArrayAttr data) {
  ...
  if (toolchain::all_of(data.getAsRange<Attribute>(), [](Attribute v) {
        return isa<FlatSymbolRefAttr, ZeroAttr>(v);
      }))
    return success();
  return op->emitError("data element must be symbol or #toolchain.zero");
}

LogicalResult
GlobalCtorsOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  for (Attribute ctor : getCtors()) {
    if (failed(verifySymbolAttrUse(toolchain::cast<FlatSymbolRefAttr>(ctor), *this,
                                   symbolTable)))
      return failure();
  }
  return success();
...
GlobalDtorsOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  for (Attribute dtor : getDtors()) {
    if (failed(verifySymbolAttrUse(toolchain::cast<FlatSymbolRefAttr>(dtor), *this,
                                   symbolTable)))
      return failure();
  }
  return success();
...
// operation ::= `toolchain.mlir.alias` linkage? visibility?
//               (`unnamed_addr` | `local_unnamed_addr`)?
...
LogicalResult AliasOp::verify() {
  ...
  // This matches LLVM IR verification logic, see toolchain/lib/IR/Verifier.cpp
  switch (getLinkage()) {
...
LogicalResult ShuffleVectorOp::verify() {
  if (LLVM::isScalableVectorType(getV1().getType()) &&
      toolchain::any_of(getMask(), [](int32_t v) { return v != 0; }))
...
OpFoldResult ShuffleVectorOp::fold(FoldAdaptor adaptor) {
  ...
  auto vecType = toolchain::dyn_cast<VectorType>(getV1().getType());
...
void LLVMFuncOp::build(OpBuilder &builder, OperationState &result,
  ...
  assert(toolchain::cast<LLVMFunctionType>(type).getNumParams() == argAttrs.size() &&
...
//
// operation ::= `toolchain.func` linkage? cconv? function-signature
...
void LLVMFuncOp::print(OpAsmPrinter &p) {
  ...
  if (!toolchain::isa<LLVMVoidType>(returnType))
         ...
         return TypeSwitch<Operation *, WalkResult>(op)
             .Case<LandingpadOp>([&](auto landingpad) {
               constexpr StringLiteral errorMessage =
                   "'toolchain.landingpad' should have a consistent result type "
                   "inside a function";
               return checkType(landingpad.getType(), errorMessage);
             })
             .Case<ResumeOp>([&](auto resume) {
               constexpr StringLiteral errorMessage =
                   "'toolchain.resume' should have a consistent input type inside a "
                   "function";
...
static LogicalResult verifyStructArrayConstant(LLVM::ConstantOp op,
  ...
  toolchain::DenseSet<Attribute> elementsVerified;
  ...
  if (auto subArrayType =
    ...
    for (auto [idx, elementAttr] : toolchain::enumerate(arrayAttr))
...
LogicalResult LLVM::ConstantOp::verify() {
  if (StringAttr sAttr = toolchain::dyn_cast<StringAttr>(getValue())) {
    auto arrayType = toolchain::dyn_cast<LLVMArrayType>(getType());
  ...
  if (auto structType = dyn_cast<LLVMStructType>(getType())) {
    ...
    for (auto [i, attr, type] : toolchain::enumerate(arrayAttr, elementTypes)) {
  ...
  auto verifyFloatSemantics =
      [this](const toolchain::fltSemantics &attributeFloatSemantics,
  ...
  if (isa<IntegerAttr>(getValue())) {
    if (!toolchain::isa<IntegerType>(getType()))
...
LogicalResult AtomicRMWOp::verify() {
  ...
  if (getBinOp() == AtomicBinOp::fadd || getBinOp() == AtomicBinOp::fsub ||
    ...
    if (isCompatibleVectorType(valType)) {
      ...
      Type elemType = toolchain::cast<VectorType>(valType).getElementType();
    ...
    auto intType = toolchain::dyn_cast<IntegerType>(valType);
...
LogicalResult AtomicCmpXchgOp::verify() {
  auto ptrType = toolchain::cast<LLVM::LLVMPointerType>(getPtr().getType());
  if (!ptrType)
...
LogicalResult LLVM::BitcastOp::verify() {
  auto resultType = toolchain::dyn_cast<LLVMPointerType>(
      extractVectorElementType(getResult().getType()));
  auto sourceType = toolchain::dyn_cast<LLVMPointerType>(
  ...
  auto isVector = toolchain::IsaPred<VectorType>;
  ...
  // 'toolchain.addrspacecast' must be used for this purpose instead.
  if (resultType.getAddressSpace() != sourceType.getAddressSpace())
    return emitOpError("cannot cast pointers of different address spaces, "
                       "use 'toolchain.addrspacecast' instead");
...
OpFoldResult LLVM::GEPOp::fold(FoldAdaptor adaptor) {
  ...
  if (getBase().getType() == getType() && indices.size() == 1)
    if (auto integer = toolchain::dyn_cast_or_null<IntegerAttr>(indices[0]))
  ...
  for (auto iter : toolchain::enumerate(indices)) {
    auto integer = toolchain::dyn_cast_or_null<IntegerAttr>(iter.value());
    ...
    if (!indices.isDynamicIndex(iter.index()) || !integer ||
      ...
      if (Value val = toolchain::dyn_cast_if_present<Value>(existing))
...
LogicalResult CallIntrinsicOp::verify() {
  if (!getIntrin().starts_with("toolchain."))
    return emitOpError() << "intrinsic name must start with 'toolchain.'";
  if (failed(verifyOperandBundles(*this)))
    return failure();
  return success();
}
...
BlockAddressOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!function)
    return emitOpError("must reference a function defined by 'toolchain.func'");
...
printIndirectBrOpSucessors(OpAsmPrinter &p, IndirectBrOp op, Type flagType,
  ...
  p << "[";
  toolchain::interleave(
      toolchain::zip(succs, succOperands),
      [&](auto i) {
        p.printNewline();
...
void LLVM::AssumeOp::build(OpBuilder &builder, OperationState &state,
                           Value cond, toolchain::StringRef tag, ValueRange args) {
...
LogicalResult LLVMDialect::verifyDataLayoutString(
    StringRef descr, toolchain::function_ref<void(const Twine &)> reportError) {
  toolchain::Expected<toolchain::DataLayout> maybeDataLayout =
      toolchain::DataLayout::parse(descr);
  if (maybeDataLayout)
    return success();

  std::string message;
  toolchain::raw_string_ostream messageStream(message);
  toolchain::logAllUnhandledErrors(maybeDataLayout.takeError(), messageStream);
...
LogicalResult LLVMDialect::verifyOperationAttribute(Operation *op,
  ...
  // toolchain::DataLayout constructor.
  if (attr.getName() != LLVM::LLVMDialect::getDataLayoutAttrName())
    return success();
  if (auto stringAttr = toolchain::dyn_cast<StringAttr>(attr.getValue()))
...
LogicalResult LLVMDialect::verifyRegionResultAttribute(Operation *op,
  ...
  if (toolchain::isa<LLVMVoidType>(resType))
...
Operation *LLVMDialect::materializeConstant(OpBuilder &builder, Attribute value,
                                            Type type, Location loc) {
  // If this was folded from an operation other than toolchain.mlir.constant, it
  // should be materialized as such. Note that an toolchain.mlir.zero may fold into
  // a builtin zero attribute and thus will materialize as a toolchain.mlir.constant.
  ...
  // Otherwise try materializing it as a regular toolchain.mlir.constant op.
