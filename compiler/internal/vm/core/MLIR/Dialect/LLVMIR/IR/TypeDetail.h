...
#include "vm/core/ADT/Bitfields.h"
...
public:
  ...
  public:
    ...
    toolchain::hash_code hashValue() const {
      ...
      if (isIdentified()) {
        ...
        return toolchain::hash_combine(flags, getIdentifier());
      }
      if (isPacked())
        flags |= kPackedHashFlag;
      return toolchain::hash_combine(flags, getTypeList());
    }
  ...
  bool isIdentified() const {
    return toolchain::Bitfield::get<KeyFlagIdentified>(keySizeAndFlags);
  }

  /// Checks whether the struct is packed (both literal and identified structs).
  bool isPacked() const {
    return isIdentified() ? toolchain::Bitfield::get<MutableFlagPacked>(
                                identifiedBodySizeAndFlags)
                          : toolchain::Bitfield::get<KeyFlagPacked>(keySizeAndFlags);
  }
  ...
  bool isOpaque() const {
    return toolchain::Bitfield::get<MutableFlagOpaque>(identifiedBodySizeAndFlags);
  }
  ...
  bool isInitialized() const {
    return toolchain::Bitfield::get<MutableFlagInitialized>(
        identifiedBodySizeAndFlags);
  }
  ...
  LLVMStructTypeStorage(const KeyTy &key) {
    if (!key.isIdentified()) {
      ...
      toolchain::Bitfield::set<KeyFlagPacked>(keySizeAndFlags, key.isPacked());
      return;
    }
    ...
    toolchain::Bitfield::set<KeyFlagIdentified>(keySizeAndFlags, true);

    // If the struct is being constructed directly as opaque, mark it as
    // initialized.
    toolchain::Bitfield::set<MutableFlagInitialized>(identifiedBodySizeAndFlags,
                                                key.isOpaque());
    toolchain::Bitfield::set<MutableFlagOpaque>(identifiedBodySizeAndFlags,
                                           key.isOpaque());
  }

  /// Hook into the type uniquing infrastructure.
  bool operator==(const KeyTy &other) const { return getAsKey() == other; };
  static toolchain::hash_code hashKey(const KeyTy &key) { return key.hashValue(); }
  ...
  LogicalResult mutate(TypeStorageAllocator &allocator, ArrayRef<Type> body,
    ...
    toolchain::Bitfield::set<MutableFlagInitialized>(identifiedBodySizeAndFlags,
                                                true);
    toolchain::Bitfield::set<MutableFlagPacked>(identifiedBodySizeAndFlags, packed);
...
private:
  ...
  using KeyFlagIdentified =
      toolchain::Bitfield::Element<bool, /*Offset=*/0, /*Size=*/1>;
  using KeyFlagPacked = toolchain::Bitfield::Element<bool, /*Offset=*/1, /*Size=*/1>;
  using KeySize =
      toolchain::Bitfield::Element<unsigned, /*Offset=*/2,
  ...
  using MutableFlagPacked =
      toolchain::Bitfield::Element<bool, /*Offset=*/1, /*Size=*/1>;
  using MutableFlagInitialized =
      toolchain::Bitfield::Element<bool, /*Offset=*/2, /*Size=*/1>;
  using MutableSize =
      toolchain::Bitfield::Element<unsigned, /*Offset=*/3,
