...
bool LLVMArrayType::isValidElementType(Type type) {
  return !toolchain::isa<LLVMVoidType, LLVMLabelType, LLVMMetadataType,
...
LLVMArrayType::getTypeSizeInBits(const DataLayout &dataLayout,
  ...
  return toolchain::TypeSize::getFixed(kBitsInByte *
                                  getTypeSize(dataLayout, params));
}

toolchain::TypeSize LLVMArrayType::getTypeSize(const DataLayout &dataLayout,
                                          DataLayoutEntryListRef params) const {
  return toolchain::alignTo(dataLayout.getTypeSize(getElementType()),
...
bool LLVMFunctionType::isValidArgumentType(Type type) {
  return !toolchain::isa<LLVMVoidType, LLVMFunctionType>(type);
}

bool LLVMFunctionType::isValidResultType(Type type) {
  return !toolchain::isa<LLVMFunctionType, LLVMMetadataType, LLVMLabelType>(type);
}
...
LLVMFunctionType LLVMFunctionType::clone(TypeRange inputs,
  ...
  if (results.size() != 1 || !isValidResultType(results[0]))
    return {};
  if (!toolchain::all_of(inputs, isValidArgumentType))
    return {};
  return get(results[0], toolchain::to_vector(inputs), isVarArg());
}
...
LLVMPointerType::getTypeSizeInBits(const DataLayout &dataLayout,
  ...
  if (std::optional<uint64_t> size =
    ...
    return toolchain::TypeSize::getFixed(*size);
...
bool LLVMPointerType::areCompatible(
  ...
  for (DataLayoutEntryInterface newEntry : newLayout) {
    ...
    auto newType =
        toolchain::cast<LLVMPointerType>(toolchain::cast<Type>(newEntry.getKey()));
    const auto *it =
        toolchain::find_if(oldLayout, [&](DataLayoutEntryInterface entry) {
          if (auto type = toolchain::dyn_cast_if_present<Type>(entry.getKey())) {
            return toolchain::cast<LLVMPointerType>(type).getAddressSpace() ==
    ...
    if (it == oldLayout.end()) {
      toolchain::find_if(oldLayout, [&](DataLayoutEntryInterface entry) {
        if (auto type = toolchain::dyn_cast_if_present<Type>(entry.getKey())) {
          return toolchain::cast<LLVMPointerType>(type).getAddressSpace() == 0;
        }
        return false;
      });
    }
    ...
    Attribute newSpec = toolchain::cast<DenseIntElementsAttr>(newEntry.getValue());
...
LogicalResult LLVMPointerType::verifyEntries(DataLayoutEntryListRef entries,
  ...
  for (DataLayoutEntryInterface entry : entries) {
    if (!entry.isTypeEntry())
      continue;
    auto key = toolchain::cast<Type>(entry.getKey());
    auto values = toolchain::dyn_cast<DenseIntElementsAttr>(entry.getValue());
...
LogicalResult LLVMStructType::setBody(ArrayRef<Type> types, bool isPacked) {
  ...
  assert(toolchain::all_of(types, LLVMStructType::isValidElementType) &&
...
LLVMStructType::getTypeSizeInBits(const DataLayout &dataLayout,
                                  DataLayoutEntryListRef params) const {
  auto structSize = toolchain::TypeSize::getFixed(0);
  ...
  for (Type element : getBody()) {
    ...
    structSize = toolchain::alignTo(structSize, elementAlignment);
  ...
  structSize = toolchain::alignTo(structSize, structAlignment);
...
getStructDataLayoutEntry(DataLayoutEntryListRef params, LLVMStructType type,
  ...
  const auto *currentEntry =
      toolchain::find_if(params, [](DataLayoutEntryInterface entry) {
        return entry.isTypeEntry();
      });
  if (currentEntry == params.end())
    return std::nullopt;

  auto attr = toolchain::cast<DenseIntElementsAttr>(currentEntry->getValue());
...
static uint64_t extractStructSpecValue(Attribute attr, StructDLEntryPos pos) {
  return toolchain::cast<DenseIntElementsAttr>(attr)
...
LogicalResult LLVMStructType::verifyEntries(DataLayoutEntryListRef entries,
  ...
  for (DataLayoutEntryInterface entry : entries) {
    ...
    auto key = toolchain::cast<LLVMStructType>(toolchain::cast<Type>(entry.getKey()));
    auto values = toolchain::dyn_cast<DenseIntElementsAttr>(entry.getValue());
    if (!values || (values.size() != 2 && values.size() != 1)) {
      return emitError(loc)
             << "expected layout attribute for "
             << toolchain::cast<Type>(entry.getKey())
...
static constexpr toolchain::StringRef kSpirvPrefix = "spirv.";
static constexpr toolchain::StringRef kArmSVCount = "aarch64.svcount";
static constexpr toolchain::StringRef kAMDGCNNamedBarrier = "amdgcn.named.barrier";

bool LLVM::LLVMTargetExtType::hasProperty(Property prop) const {
  // See toolchain/lib/IR/Type.cpp for reference.
...
const toolchain::fltSemantics &LLVMPPCFP128Type::getFloatSemantics() const {
...
bool mlir::LLVM::isCompatibleOuterType(Type type) {
  ...
  if (auto intType = toolchain::dyn_cast<IntegerType>(type))
    return intType.isSignless();

  // 1D vector types are compatible.
  if (auto vecType = toolchain::dyn_cast<VectorType>(type))
...
static bool isCompatibleImpl(Type type, DenseSet<Type> &compatibleTypes) {
  ...
  bool result =
      toolchain::TypeSwitch<Type, bool>(type)
          .Case<LLVMStructType>([&](auto structType) {
            return toolchain::all_of(structType.getBody(), isCompatible);
          })
          .Case<LLVMFunctionType>([&](auto funcType) {
            return isCompatible(funcType.getReturnType()) &&
                   toolchain::all_of(funcType.getParams(), isCompatible);
          })
          ...
          .Case<LLVMTargetExtType>([&](auto extType) {
            return toolchain::all_of(extType.getTypeParams(), isCompatible);
          })
          // clang-format off
          .Case<
              LLVMArrayType
...
bool mlir::LLVM::isCompatibleFloatingPointType(Type type) {
  return toolchain::isa<BFloat16Type, Float16Type, Float32Type, Float64Type,
...
bool mlir::LLVM::isCompatibleVectorType(Type type) {
  if (auto vecType = toolchain::dyn_cast<VectorType>(type)) {
    ...
    if (auto intType = toolchain::dyn_cast<IntegerType>(elementType))
      return intType.isSignless();
    return toolchain::isa<BFloat16Type, Float16Type, Float32Type, Float64Type,
...
toolchain::ElementCount mlir::LLVM::getVectorNumElements(Type type) {
  auto vecTy = dyn_cast<VectorType>(type);
  assert(vecTy && "incompatible with LLVM vector type");
  if (vecTy.isScalable())
    return toolchain::ElementCount::getScalable(vecTy.getNumElements());
  return toolchain::ElementCount::getFixed(vecTy.getNumElements());
}

bool mlir::LLVM::isScalableVectorType(Type vectorType) {
  assert(toolchain::isa<VectorType>(vectorType) &&
         "expected LLVM-compatible vector type");
  return toolchain::cast<VectorType>(vectorType).isScalable();
}
...
Type mlir::LLVM::getVectorType(Type elementType,
                               const toolchain::ElementCount &numElements) {
...
toolchain::TypeSize mlir::LLVM::getPrimitiveTypeSizeInBits(Type type) {
  assert(isCompatibleType(type) &&
         "expected a type compatible with the LLVM dialect");

  return toolchain::TypeSwitch<Type, toolchain::TypeSize>(type)
      .Case<BFloat16Type, Float16Type>(
          [](Type) { return toolchain::TypeSize::getFixed(16); })
      .Case<Float32Type>([](Type) { return toolchain::TypeSize::getFixed(32); })
      .Case<Float64Type>([](Type) { return toolchain::TypeSize::getFixed(64); })
      .Case<Float80Type>([](Type) { return toolchain::TypeSize::getFixed(80); })
      .Case<Float128Type>([](Type) { return toolchain::TypeSize::getFixed(128); })
      .Case<IntegerType>([](IntegerType intTy) {
        return toolchain::TypeSize::getFixed(intTy.getWidth());
      })
      .Case<LLVMPPCFP128Type>(
          [](Type) { return toolchain::TypeSize::getFixed(128); })
      .Case<VectorType>([](VectorType t) {
        assert(isCompatibleVectorType(t) &&
               "unexpected incompatible with LLVM vector type");
        toolchain::TypeSize elementSize =
            getPrimitiveTypeSizeInBits(t.getElementType());
        return toolchain::TypeSize(elementSize.getFixedValue() * t.getNumElements(),
                              elementSize.isScalable());
      })
      .Default([](Type ty) {
        assert((toolchain::isa<LLVMVoidType, LLVMLabelType, LLVMMetadataType,
        ...
        return toolchain::TypeSize::getFixed(0);
      });
}
