...
toolchain::SmallVector<MemorySlot> LLVM::AllocaOp::getPromotableSlots() {
...
void LLVM::AllocaOp::handleBlockArgument(const MemorySlot &slot,
  ...
  for (Operation *user : getOperation()->getUsers())
    if (auto declareOp = toolchain::dyn_cast<LLVM::DbgDeclareOp>(user))
...
static bool hasAllZeroIndices(LLVM::GEPOp gepOp) {
  return toolchain::all_of(gepOp.getIndices(), [](auto index) {
    auto indexAttr = toolchain::dyn_cast_if_present<IntegerAttr>(index);
...
static std::optional<uint64_t> gepToByteOffset(const DataLayout &dataLayout,
  ...
  for (uint64_t index : toolchain::drop_begin(indices)) {
    bool shouldCancel =
        TypeSwitch<Type, bool>(currentType)
            ...
            .Case([&](LLVM::LLVMStructType structType) {
              ...
              for (uint32_t i : toolchain::seq(index)) {
                if (!structType.isPacked())
                  offset = toolchain::alignTo(
...
getSubslotAccessInfo(const DestructurableMemorySlot &slot,
  ...
  return TypeSwitch<Type, std::optional<SubslotAccessInfo>>(type)
      ...
      .Case([&](LLVM::LLVMStructType structType)
        ...
        // them the offset is.
        for (auto [index, elem] : toolchain::enumerate(structType.getBody())) {
          uint64_t elemSize = dataLayout.getTypeSize(elem);
          if (!structType.isPacked()) {
            distanceToStart = toolchain::alignTo(
...
static bool areAllIndicesI32(const DestructurableMemorySlot &slot) {
  ...
  return toolchain::all_of(toolchain::make_first_range(slot.subelementTypes),
                      [&](Attribute index) {
...
memsetRewire(MemsetIntr op, const DestructurableMemorySlot &slot,
  ...
  for (size_t i = 0; i < types->size(); i++) {
    ...
    if (!packed)
      covered =
          toolchain::alignTo(covered, dataLayout.getTypeABIAlignment(elemType));
...
static bool memcpyCanRewire(MemcpyLike op, const DestructurableMemorySlot &slot,
  ...
  if (op.getSrc() == slot.ptr)
    usedIndices.insert_range(toolchain::make_first_range(slot.subelementTypes));
