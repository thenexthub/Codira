...
AliasScopeAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  (void)domain;
  (void)description;
  if (!toolchain::isa<StringAttr, DistinctAttr>(id))
    return emitError()
...
bool DIScopeAttr::classof(Attribute attr) {
  return toolchain::isa<DICommonBlockAttr, DICompileUnitAttr, DICompositeTypeAttr,
...
bool DILocalScopeAttr::classof(Attribute attr) {
  return toolchain::isa<DILexicalBlockAttr, DILexicalBlockFileAttr,
...
bool DIVariableAttr::classof(Attribute attr) {
  return toolchain::isa<DILocalVariableAttr, DIGlobalVariableAttr>(attr);
}
...
bool DITypeAttr::classof(Attribute attr) {
  return toolchain::isa<DINullTypeAttr, DIBasicTypeAttr, DICompositeTypeAttr,
...
bool TBAANodeAttr::classof(Attribute attr) {
  return toolchain::isa<TBAATypeDescriptorAttr, TBAARootAttr>(attr);
}
...
ParseResult parseExpressionArg(AsmParser &parser, uint64_t opcode,
  ...
  auto operandParser = [&]() -> LogicalResult {
    uint64_t operand = 0;
    if (!args.empty() && opcode == toolchain::dwarf::DW_OP_LLVM_convert) {
      // Attempt to parse a keyword.
      StringRef keyword;
      if (succeeded(parser.parseOptionalKeyword(&keyword))) {
        operand = toolchain::dwarf::getAttributeEncoding(keyword);
        if (operand == 0) {
...
void printExpressionArg(AsmPrinter &printer, uint64_t opcode,
                        ArrayRef<uint64_t> args) {
  size_t i = 0;
  toolchain::interleaveComma(args, printer, [&](uint64_t operand) {
    if (i > 0 && opcode == toolchain::dwarf::DW_OP_LLVM_convert) {
      if (const StringRef keyword =
              toolchain::dwarf::AttributeEncodingString(operand);
          !keyword.empty()) {
        printer << keyword;
        return;
      }
...
LogicalResult
ConstantRangeAttr::verify(toolchain::function_ref<InFlightDiagnostic()> emitError,
...
TargetFeaturesAttr TargetFeaturesAttr::get(MLIRContext *context,
                                           toolchain::ArrayRef<StringRef> features) {
  return Base::get(context,
                   toolchain::map_to_vector(features, [&](StringRef feature) {
...
TargetFeaturesAttr::verify(function_ref<InFlightDiagnostic()> emitError,
                           toolchain::ArrayRef<StringAttr> features) {
...
bool TargetFeaturesAttr::contains(StringAttr feature) const {
  ...
  return toolchain::is_contained(getFeatures(), feature);
}
...
std::string TargetFeaturesAttr::getFeaturesString() const {
  ...
  toolchain::raw_string_ostream ss(featuresString);
...
ModuleFlagAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (key == LLVMDialect::getModuleFlagKeyCGProfileName()) {
    ...
    if ((!arrayAttr) || (!toolchain::all_of(arrayAttr, [](Attribute attr) {
          return isa<ModuleFlagCGProfileEntryAttr>(attr);
        })))
      return emitError()
             << "'CG Profile' key expects an array of '#toolchain.cgprofile_entry'";
    return success();
  }
  ...
  if (key == LLVMDialect::getModuleFlagKeyProfileSummaryName()) {
    if (!isa<ModuleFlagProfileSummaryAttr>(value))
      return emitError() << "'ProfileSummary' key expects a "
                            "'#toolchain.profile_summary' attribute";
    return success();
  }
