...
static constexpr unsigned notIntrinsic = toolchain::Intrinsic::not_intrinsic;
...
static bool isPtrInAddrSpace(mlir::Value ptr, NVVMMemorySpace targetAS) {
  auto ptrTy = toolchain::cast<LLVM::LLVMPointerType>(ptr.getType());
...
LogicalResult CpAsyncBulkTensorGlobalToSharedClusterOp::verify() {
  ...
  if (getPredicate()) { // Inline-asm based lowering
    ...
    unsigned AS = toolchain::cast<LLVM::LLVMPointerType>(getDstMem().getType())
                      .getAddressSpace();
    if (AS != expectedAS)
      return emitError()
             << (isCTAOnly
...
LogicalResult ConvertF32x2ToF6x2Op::verify() {
  ...
  if (!toolchain::isa<mlir::Float6E2M3FNType, mlir::Float6E3M2FNType>(getDstTy())) {
...
LogicalResult ConvertF32x2ToF8x2Op::verify() {
  ...
  return toolchain::TypeSwitch<mlir::Type, LogicalResult>(getDstTy())
...
LogicalResult ConvertF16x2ToF8x2Op::verify() {
  ...
  if (!toolchain::isa<mlir::Float8E4M3FNType, mlir::Float8E5M2Type>(getDstTy())) {
    return emitOpError("Only ")
           << mlir::Float8E4M3FNType::get(ctx) << " and "
...
LogicalResult ConvertBF16x2ToF8x2Op::verify() {
  ...
  if (!toolchain::isa<mlir::Float8E8M0FNUType>(getDstTy()))
...
LogicalResult ConvertF32x2ToF4x2Op::verify() {
  ...
  if (!toolchain::isa<mlir::Float4E2M1FNType>(getDstTy()))
    return emitOpError("Only ")
...
LogicalResult ConvertF8x2ToF16x2Op::verify() {
  ...
  if (!toolchain::isa<Float8E4M3FNType, Float8E5M2Type>(getSrcType()))
    return emitOpError("Only ")
           << mlir::Float8E4M3FNType::get(ctx) << " and "
           << mlir::Float8E5M2Type::get(ctx)
...
LogicalResult ConvertF8x2ToBF16x2Op::verify() {
  mlir::MLIRContext *ctx = getContext();
  if (!toolchain::isa<Float8E8M0FNUType>(getSrcType()))
    return emitOpError("Only ")
           << mlir::Float8E8M0FNUType::get(ctx)
...
LogicalResult ConvertF6x2ToF16x2Op::verify() {
  ...
  if (!toolchain::isa<Float6E2M3FNType, Float6E3M2FNType>(getSrcType()))
    return emitOpError("Only ")
           << mlir::Float6E2M3FNType::get(ctx) << " and "
           << mlir::Float6E3M2FNType::get(ctx)
...
LogicalResult ConvertF4x2ToF16x2Op::verify() {
  ...
  if (!toolchain::isa<Float4E2M1FNType>(getSrcType()))
    return emitOpError("Only ")
           << mlir::Float4E2M1FNType::get(ctx)
...
static LogicalResult verifyConvertF32x2ToFP16x2Op(Twine dstType,
  ...
  if (!toolchain::is_contained(validRndModes, rnd)) {
...
MmaOp::inferOperandMMAType(Type operandElType, bool isAccumulator) {
  ...
  if (toolchain::isa<IntegerType>(operandElType)) {
    if (isAccumulator)
      return NVVM::MMATypes::s32;
    return std::nullopt;
  }

  if (auto structType = toolchain::dyn_cast<LLVM::LLVMStructType>(operandElType)) {
...
void MmaOp::print(OpAsmPrinter &p) {
  ...
  p << " : "
    << "(";
  toolchain::interleaveComma(SmallVector<Type, 3>{frags[0].regs[0].getType(),
...
ParseResult MmaOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  for (const auto &iter : toolchain::enumerate(operandTypes)) {
...
LogicalResult MmaOp::verify() {
  ...
  for (const auto &iter : toolchain::enumerate(
    ...
    SmallVector<Type, 4> operandTySeg(operand_type_begin() + spec.first,
                                      operand_type_begin() + spec.first +
                                          spec.second);
    bool match = toolchain::is_contained(iter.value(), operandTySeg);
    ...
    if (!match) {
      ...
      errorStream << "but got ";
      toolchain::interleaveComma(operandTySeg, errorStream);
      return emitOpError(errorMessage);
    }
  }

  // Check the result type
  if (!toolchain::any_of(expectedResult, [&](Type expectedResultType) {
        return expectedResultType == getResult().getType();
      })) {
    errorStream
        << "Could not match allowed types for the result; expected one of ";
    toolchain::interleaveComma(expectedResult, errorStream);
...
MmaSpOp::getIntrinsicIDAndArgs(Operation &op, LLVM::ModuleTranslation &mt,
  ...
  // Get operands
  toolchain::SmallVector<toolchain::Value *> args;
...
ParseResult MmaBlockScaleOp::parse(OpAsmParser &parser,
  ...
  for (const auto &[idx, frag] : toolchain::enumerate(frags)) {
...
NVVM::IDArgPair MmaBlockScaleOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt, toolchain::IRBuilderBase &builder) {
  auto curOp = cast<NVVM::MmaBlockScaleOp>(op);

  SmallVector<toolchain::Value *> args;
  // Add A, B, C operands
...
LogicalResult ShflOp::verify() {
  auto returnStructType = toolchain::dyn_cast<LLVM::LLVMStructType>(getType());
  ...
  if (returnStructType) {
    ...
    toolchain::ArrayRef<Type> returnStruct = returnStructType.getBody();
...
LogicalResult NVVM::WMMALoadOp::verify() {
  unsigned addressSpace =
      toolchain::cast<LLVM::LLVMPointerType>(getPtr().getType()).getAddressSpace();
...
LogicalResult NVVM::WMMAStoreOp::verify() {
  ...
  if (toolchain::any_of(getArgs(), [&typeInfo](Value operands) {
...
static LogicalResult isAllowedSizeN(int sizeN, NVVM::WGMMATypes typeA) {
  ...
  case WGMMATypes::e5m2:
    if (toolchain::is_contained(allowedN, sizeN))
      return success();
    break;
  case WGMMATypes::u8:
  case WGMMATypes::s8:
  case WGMMATypes::b1:
    if (toolchain::is_contained(allowedNshort, sizeN))
      return success();
    break;
  case WGMMATypes::f32:
  case WGMMATypes::s32:
...
std::string NVVM::WgmmaMmaAsyncOp::getPtx() {
  ...
  std::string ptx;
  toolchain::raw_string_ostream ss(ptx);
...
bool NVVM::WgmmaMmaAsyncOp::getAsmValues(
    ...
    toolchain::SmallVectorImpl<std::pair<mlir::Value, mlir::NVVM::PTXRegisterMod>>
        &asmValues) {
  bool isF16 = getTypeA() == WGMMATypes::f16 || getTypeA() == WGMMATypes::bf16;
  if (getResults())
...
LogicalResult NVVM::MatchSyncOp::verify() {
  if (getKind() == NVVM::MatchSyncKind::all) {
    auto type = toolchain::dyn_cast<LLVM::LLVMStructType>(getType());
...
LogicalResult NVVM::PrefetchOp::verify() {
  ...
  unsigned addressSpace =
      toolchain::cast<LLVM::LLVMPointerType>(getAddr().getType()).getAddressSpace();
...
packValInto64Bits(toolchain::IRBuilderBase &builder,
                  toolchain::Value *result, // the `result` (unset bits are zero)
                  toolchain::Value *field,  // `field` to pack into `result`
...
void Tcgen05MmaSmemDescOp::createSmemDescriptor(Operation &op,
  ...
  toolchain::Value *smemDesc = builder.getInt64(0);
...
std::string NVVM::MBarrierTryWaitParityOp::getPtx() {
  bool isShared = isPtrInSharedCTASpace(getAddr());
  toolchain::StringRef space = isShared ? ".shared" : "";
...
mlir::NVVM::IDArgPair NVVM::BarrierOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Value *barrierId = thisOp.getBarrierId()
                               ? mt.lookupValue(thisOp.getBarrierId())
                               : builder.getInt32(0);
  toolchain::Intrinsic::ID id;
  toolchain::SmallVector<toolchain::Value *> args = {barrierId};
  if (thisOp.getNumberOfThreads()) {
    id = toolchain::Intrinsic::nvvm_barrier_cta_sync_aligned_count;
    ...
    case NVVM::BarrierReduction::AND:
      id = toolchain::Intrinsic::nvvm_barrier_cta_red_and_aligned_all;
      break;
    case NVVM::BarrierReduction::OR:
      id = toolchain::Intrinsic::nvvm_barrier_cta_red_or_aligned_all;
      break;
    case NVVM::BarrierReduction::POPC:
      id = toolchain::Intrinsic::nvvm_barrier_cta_red_popc_aligned_all;
      break;
    }
    args.push_back(builder.CreateICmpNE(
        mt.lookupValue(thisOp.getReductionPredicate()), builder.getInt32(0)));
  } else {
    id = toolchain::Intrinsic::nvvm_barrier_cta_sync_aligned_all;
  }
...
PMEventOp::getIntrinsicIDAndArgs(Operation &op, LLVM::ModuleTranslation &mt,
  ...
  toolchain::Type *i16Ty = toolchain::Type::getInt16Ty(mt.getLLVMContext());

  // With event-id, mask is generated as (1 << event-id)
  toolchain::Value *maskVal;
  if (auto eventAttr = thisOp.getEventIdAttr()) {
    uint16_t mask = static_cast<uint16_t>(1u << eventAttr.getInt());
    maskVal = toolchain::ConstantInt::get(i16Ty, mask);
  } else {
    maskVal =
        toolchain::ConstantInt::get(i16Ty, thisOp.getMaskedEventIdAttr().getValue());
  }

  return {toolchain::Intrinsic::nvvm_pm_event_mask, {maskVal}};
}
...
mlir::NVVM::IDArgPair MBarrierInitOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Intrinsic::ID id = isShared ? toolchain::Intrinsic::nvvm_mbarrier_init_shared
                                    : toolchain::Intrinsic::nvvm_mbarrier_init;
...
mlir::NVVM::IDArgPair MBarrierInvalOp::getIntrinsicIDAndArgs(
  ...
  bool isShared = isPtrInSharedCTASpace(thisOp.getAddr());
  toolchain::Intrinsic::ID id = isShared
                               ? toolchain::Intrinsic::nvvm_mbarrier_inval_shared
                               : toolchain::Intrinsic::nvvm_mbarrier_inval;
...
mlir::NVVM::IDArgPair MBarrierExpectTxOp::getIntrinsicIDAndArgs(
  ...
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_expect_tx_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_expect_tx_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_expect_tx_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_expect_tx_scope_cluster_space_cluster};
...
mlir::NVVM::IDArgPair MBarrierCompleteTxOp::getIntrinsicIDAndArgs(
  ...
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_complete_tx_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_complete_tx_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_complete_tx_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_complete_tx_scope_cluster_space_cluster};
...
mlir::NVVM::IDArgPair MBarrierArriveOp::getIntrinsicIDAndArgs(
  ...
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_scope_cluster_space_cluster};
  static constexpr toolchain::Intrinsic::ID relaxedIDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_relaxed_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::
  ...
  toolchain::Value *mbar = mt.lookupValue(thisOp.getAddr());
  if (needCast)
  ...
  if (!hasCount &&
      (id == toolchain::Intrinsic::nvvm_mbarrier_arrive_scope_cta_space_cta))
    return {toolchain::Intrinsic::nvvm_mbarrier_arrive_shared, {mbar}};

  // When count is not explicitly specified, the default is 1.
  toolchain::LLVMContext &ctx = mt.getLLVMContext();
  toolchain::Value *count =
      hasCount ? mt.lookupValue(thisOp.getCount())
               : toolchain::ConstantInt::get(toolchain::Type::getInt32Ty(ctx), 1);
...
mlir::NVVM::IDArgPair MBarrierArriveDropOp::getIntrinsicIDAndArgs(
  ...
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_scope_cluster_space_cluster};
  static constexpr toolchain::Intrinsic::ID relaxedIDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::
...
mlir::NVVM::IDArgPair MBarrierArriveExpectTxOp::getIntrinsicIDAndArgs(
  ...
  // clang-format off
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_scope_cluster_space_cluster};
  static constexpr toolchain::Intrinsic::ID relaxedIDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_relaxed_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_expect_tx_relaxed_scope_cluster_space_cluster};
  // clang-format on
  auto id = thisOp.getRelaxed() ? relaxedIDs[index] : IDs[index];

  // Tidy-up the Intrinsic Args
  toolchain::Value *txcount = mt.lookupValue(thisOp.getTxcount());
  toolchain::Value *mbar = mt.lookupValue(thisOp.getAddr());
  bool needCast = isPtrInGenericSpace(thisOp.getAddr());
  if (needCast)
    mbar = castPtrToAddrSpace(builder, mbar, NVVMMemorySpace::Shared);
...
mlir::NVVM::IDArgPair MBarrierArriveDropExpectTxOp::getIntrinsicIDAndArgs(
  ...
  // clang-format off
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_scope_cluster_space_cluster};
  static constexpr toolchain::Intrinsic::ID relaxedIDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_relaxed_scope_cta_space_cluster,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_expect_tx_relaxed_scope_cluster_space_cluster};
  // clang-format on
  auto id = thisOp.getRelaxed() ? relaxedIDs[index] : IDs[index];
...
mlir::NVVM::IDArgPair MBarrierArriveNocompleteOp::getIntrinsicIDAndArgs(
  ...
  bool isShared = isPtrInSharedCTASpace(thisOp.getAddr());
  toolchain::Intrinsic::ID id =
      isShared ? toolchain::Intrinsic::nvvm_mbarrier_arrive_noComplete_shared
               : toolchain::Intrinsic::nvvm_mbarrier_arrive_noComplete;
  // Fill the Intrinsic Args
  toolchain::SmallVector<toolchain::Value *> args;
  args.push_back(mt.lookupValue(thisOp.getAddr()));
  args.push_back(mt.lookupValue(thisOp.getCount()));
...
mlir::NVVM::IDArgPair MBarrierArriveDropNocompleteOp::getIntrinsicIDAndArgs(
  ...
  bool isShared = isPtrInSharedCTASpace(thisOp.getAddr());
  toolchain::Intrinsic::ID id =
      isShared ? toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_noComplete_shared
               : toolchain::Intrinsic::nvvm_mbarrier_arrive_drop_noComplete;
  // Fill the Intrinsic Args
  toolchain::SmallVector<toolchain::Value *> args;
  args.push_back(mt.lookupValue(thisOp.getAddr()));
  args.push_back(mt.lookupValue(thisOp.getCount()));
...
mlir::NVVM::IDArgPair MBarrierTestWaitOp::getIntrinsicIDAndArgs(
  ...
  // clang-format off
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_parity_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_parity_scope_cluster_space_cta};
  static constexpr toolchain::Intrinsic::ID relaxedIDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_parity_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_test_wait_parity_relaxed_scope_cluster_space_cta};
  // clang-format on
  auto id = thisOp.getRelaxed() ? relaxedIDs[index] : IDs[index];

  // Tidy-up the Intrinsic Args
  toolchain::Value *mbar = mt.lookupValue(thisOp.getAddr());
  toolchain::Value *input = mt.lookupValue(thisOp.getStateOrPhase());
  bool needCast = isPtrInGenericSpace(thisOp.getAddr());
  if (needCast)
    mbar = castPtrToAddrSpace(builder, mbar, NVVMMemorySpace::Shared);
...
mlir::NVVM::IDArgPair MBarrierTryWaitOp::getIntrinsicIDAndArgs(
  ...
  // clang-format off
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_tl_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_tl_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_tl_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_tl_scope_cluster_space_cta};
  static constexpr toolchain::Intrinsic::ID relaxedIDs[] = {
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_tl_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_tl_relaxed_scope_cta_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_tl_relaxed_scope_cluster_space_cta,
      toolchain::Intrinsic::nvvm_mbarrier_try_wait_parity_tl_relaxed_scope_cluster_space_cta};
  // clang-format on
  auto id = thisOp.getRelaxed() ? relaxedIDs[index] : IDs[index];
...
mlir::NVVM::IDArgPair CpAsyncMBarrierArriveOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Intrinsic::ID id;
  if (thisOp.getNoinc()) {
    id = isShared ? toolchain::Intrinsic::nvvm_cp_async_mbarrier_arrive_noinc_shared
                  : toolchain::Intrinsic::nvvm_cp_async_mbarrier_arrive_noinc;
  } else {
    id = isShared ? toolchain::Intrinsic::nvvm_cp_async_mbarrier_arrive_shared
                  : toolchain::Intrinsic::nvvm_cp_async_mbarrier_arrive;
  }
...
#define CP_ASYNC_ID_IMPL(mod, size, suffix)                                    \
  toolchain::Intrinsic::nvvm_cp_async_##mod##_shared_global_##size##suffix
...
mlir::NVVM::IDArgPair CpAsyncBulkPrefetchOp::getIntrinsicIDAndArgs(
  ...
  toolchain::SmallVector<toolchain::Value *> args;
  toolchain::Intrinsic::ID id = toolchain::Intrinsic::nvvm_cp_async_bulk_prefetch_L2;
  ...
  toolchain::Value *i64Unused =
      toolchain::ConstantInt::get(toolchain::Type::getInt64Ty(mt.getLLVMContext()), 0);
...
mlir::NVVM::IDArgPair CpAsyncBulkGlobalToSharedClusterOp::getIntrinsicIDAndArgs(
  ...
  if (!isSharedCTA) {
    toolchain::Value *i16Unused = toolchain::ConstantInt::get(builder.getInt16Ty(), 0);
  ...
  mlir::Value cacheHint = thisOp.getL2CacheHint();
  const bool hasCacheHint = static_cast<bool>(cacheHint);
  toolchain::Value *i64Unused = toolchain::ConstantInt::get(builder.getInt64Ty(), 0);
  args.push_back(hasCacheHint ? mt.lookupValue(cacheHint) : i64Unused);
  ...
  toolchain::Intrinsic::ID id =
      isSharedCTA
          ? toolchain::Intrinsic::nvvm_cp_async_bulk_global_to_shared_cta
          : toolchain::Intrinsic::nvvm_cp_async_bulk_global_to_shared_cluster;
...
mlir::NVVM::IDArgPair CpAsyncBulkSharedCTAToGlobalOp::getIntrinsicIDAndArgs(
  ...
  toolchain::SmallVector<toolchain::Value *> args;
  toolchain::Intrinsic::ID id =
      toolchain::Intrinsic::nvvm_cp_async_bulk_shared_cta_to_global;
  ...
  if (mlir::Value byteMask = thisOp.getByteMask()) {
    ...
    id = toolchain::Intrinsic::nvvm_cp_async_bulk_shared_cta_to_global_bytemask;
  }
...
CpAsyncBulkTensorGlobalToSharedClusterOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Value *i16Zero =
      toolchain::ConstantInt::get(toolchain::Type::getInt16Ty(mt.getLLVMContext()), 0);

  // CacheHint, if available
  mlir::Value cacheHint = thisOp.getL2CacheHint();
  const bool hasCacheHint = static_cast<bool>(cacheHint);
  toolchain::Value *i64Zero =
      toolchain::ConstantInt::get(toolchain::Type::getInt64Ty(mt.getLLVMContext()), 0);
  ...
  toolchain::Value *cg =
      toolchain::ConstantInt::get(toolchain::Type::getInt32Ty(mt.getLLVMContext()), val);
  ...
  using rowTy = std::array<toolchain::Intrinsic::ID, numDims + 1>;
  using TableTy = std::array<rowTy, numModes>;
  static constexpr TableTy IDTable{
      {{notIntrinsic, toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_tile_1d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_tile_2d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_tile_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_tile_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_tile_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_w_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_w_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_w_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_w_128_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_w_128_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_im2col_w_128_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_tile_gather4_2d}}};

  static constexpr TableTy IDTableCTA{
      {{notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_tile_1d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_tile_2d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_tile_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_tile_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_tile_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_w_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_w_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_w_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_w_128_3d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_w_128_4d,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_im2col_w_128_5d},
       {notIntrinsic, notIntrinsic, notIntrinsic, notIntrinsic, notIntrinsic,
        toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_g2s_cta_tile_gather4_2d}}};
...
mlir::NVVM::IDArgPair CpAsyncBulkTensorPrefetchOp::getIntrinsicIDAndArgs(
  ...
  const unsigned NI = toolchain::Intrinsic::not_intrinsic;
  static constexpr toolchain::Intrinsic::ID IDTable[][6] = {
      {NI, toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_tile_1d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_tile_2d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_tile_3d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_tile_4d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_tile_5d},
      {NI, NI, NI,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_3d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_4d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_5d},
      {NI, NI, NI,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_w_3d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_w_4d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_w_5d},
      {NI, NI, NI,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_w_128_3d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_w_128_4d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_im2col_w_128_5d},
      {NI, NI, NI, NI, NI,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_prefetch_tile_gather4_2d}};

  static_assert(getMaxEnumValForTMALoadMode() == std::size(IDTable) - 1,
                "TMALoadModes must match number of rows in IDTable");
  size_t mode = static_cast<size_t>(thisOp.getMode());
  size_t dim = thisOp.getCoordinates().size();
  toolchain::Intrinsic::ID id = IDTable[mode][dim];
  if (id == toolchain::Intrinsic::not_intrinsic)
...
CpAsyncBulkTensorSharedCTAToGlobalOp::getIntrinsicIDAndArgs(
  ...
  const unsigned NI = toolchain::Intrinsic::not_intrinsic;
  static constexpr toolchain::Intrinsic::ID IDTable[][6] = {
      {NI, toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_tile_1d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_tile_2d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_tile_3d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_tile_4d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_tile_5d},
      {NI, NI, NI, toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_im2col_3d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_im2col_4d,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_im2col_5d},
      {NI, NI, NI, NI, NI,
       toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_s2g_tile_scatter4_2d}};
...
NVVM::IDArgPair CpAsyncBulkTensorReduceOp::getIntrinsicIDAndArgs(
  ...
  mlir::Value cacheHint = thisOp.getL2CacheHint();
  const bool hasCacheHint = static_cast<bool>(cacheHint);
  toolchain::Value *i64ZeroValue =
      toolchain::ConstantInt::get(toolchain::Type::getInt64Ty(ctx), 0);
  args.push_back(hasCacheHint ? mt.lookupValue(cacheHint) : i64ZeroValue);
  args.push_back(builder.getInt1(hasCacheHint));

  const toolchain::Intrinsic::ID notIntrinsic = toolchain::Intrinsic::not_intrinsic;
  ...
  using row = std::array<toolchain::Intrinsic::ID, maxDim + 1>;
  ...
  static constexpr fullTable IDTable{
      {// RedTy::ADD
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_add_im2col_5d}}}},
       // RedTy::MIN
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_min_im2col_5d}}}},
       // RedTy::MAX
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_max_im2col_5d}}}},
       // RedTy::INC
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_inc_im2col_5d}}}},
       // RedTy::DEC
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_dec_im2col_5d}}}},
       // RedTy::AND
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_and_im2col_5d}}}},
       // RedTy::OR
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_im2col_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_or_im2col_5d}}}},
       // RedTy::XOR
       {{{{notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_tile_1d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_tile_2d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_tile_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_tile_4d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_tile_5d}},
         {{notIntrinsic, notIntrinsic, notIntrinsic,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_im2col_3d,
           toolchain::Intrinsic::nvvm_cp_async_bulk_tensor_reduce_xor_im2col_4d,
           toolchain::Intrinsic::
  ...
  toolchain::Intrinsic::ID intrinsicID = IDTable[redKind][mode][dim];
...
#define CVT_F2TF32_ID_IMPL(rnd, relu, sf)                                      \
  hasRelu ? toolchain::Intrinsic::nvvm_f2tf32_##rnd##relu##sf                       \
          : toolchain::Intrinsic::nvvm_f2tf32_##rnd##sf
...
ConvertF32x2ToF4x2Op::getIntrinsicIDAndArgs(NVVM::ConvertF32x2ToF4x2Op op,
  ...
  toolchain::Intrinsic::ID intId =
      hasRelu ? toolchain::Intrinsic::nvvm_ff_to_e2m1x2_rn_relu_satfinite
              : toolchain::Intrinsic::nvvm_ff_to_e2m1x2_rn_satfinite;

  return {intId, std::move(args)};
}

#define GET_F32x2_TO_F6x2_ID(type, has_relu)                                   \
  has_relu ? toolchain::Intrinsic::nvvm_ff_to_##type##_rn_relu_satfinite            \
           : toolchain::Intrinsic::nvvm_ff_to_##type##_rn_satfinite

toolchain::Intrinsic::ID ConvertF32x2ToF6x2Op::getIntrinsicID(mlir::Type dstTy,
                                                         bool hasRelu) {
  return toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(dstTy)
...
#define GET_F32x2_TO_F8X2_US_ID(rnd, has_satf)                                 \
  has_satf ? toolchain::Intrinsic::nvvm_ff_to_ue8m0x2_##rnd##_satfinite             \
           : toolchain::Intrinsic::nvvm_ff_to_ue8m0x2_##rnd

#define GET_F32x2_TO_F8X2_S_ID(type, has_relu)                                 \
  has_relu ? toolchain::Intrinsic::nvvm_ff_to_##type##_rn_relu                      \
           : toolchain::Intrinsic::nvvm_ff_to_##type##_rn
...
#define GET_F16x2_TO_F8X2_ID(type, has_relu)                                   \
  has_relu ? toolchain::Intrinsic::nvvm_f16x2_to_##type##_rn_relu                   \
           : toolchain::Intrinsic::nvvm_f16x2_to_##type##_rn

toolchain::Intrinsic::ID ConvertF16x2ToF8x2Op::getIntrinsicID(mlir::Type dstTy,
                                                         bool hasRelu) {
  return toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(dstTy)
      .Case<mlir::Float8E4M3FNType>([&](mlir::Float8E4M3FNType) {
...
#define GET_BF16X2_TO_F8X2_ID(rnd, has_satf)                                   \
  has_satf ? toolchain::Intrinsic::nvvm_bf16x2_to_ue8m0x2_##rnd##_satfinite         \
           : toolchain::Intrinsic::nvvm_bf16x2_to_ue8m0x2_##rnd
...
NVVM::IDArgPair ConvertF8x2ToF16x2Op::getIntrinsicIDAndArgs(
  ...
  toolchain::Intrinsic::ID intId =
      toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(curOp.getSrcType())
          .Case<Float8E4M3FNType>([&](Float8E4M3FNType type) {
            return hasRelu ? toolchain::Intrinsic::nvvm_e4m3x2_to_f16x2_rn_relu
                           : toolchain::Intrinsic::nvvm_e4m3x2_to_f16x2_rn;
          })
          .Case<Float8E5M2Type>([&](Float8E5M2Type type) {
            return hasRelu ? toolchain::Intrinsic::nvvm_e5m2x2_to_f16x2_rn_relu
                           : toolchain::Intrinsic::nvvm_e5m2x2_to_f16x2_rn;
          })
  ...
  toolchain::Value *packedI16 =
      builder.CreateBitCast(mt.lookupValue(curOp.getSrc()),
                            toolchain::Type::getInt16Ty(builder.getContext()));
...
NVVM::IDArgPair ConvertF8x2ToBF16x2Op::getIntrinsicIDAndArgs(
  ...
  toolchain::Intrinsic::ID intId = toolchain::Intrinsic::nvvm_ue8m0x2_to_bf16x2;
  toolchain::Value *packedI16 =
      builder.CreateBitCast(mt.lookupValue(curOp.getSrc()),
                            toolchain::Type::getInt16Ty(builder.getContext()));
  ...
  toolchain::Intrinsic::ID intId =
      toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(curOp.getSrcType())
          .Case<Float6E2M3FNType>([&](Float6E2M3FNType type) {
            return hasRelu ? toolchain::Intrinsic::nvvm_e2m3x2_to_f16x2_rn_relu
                           : toolchain::Intrinsic::nvvm_e2m3x2_to_f16x2_rn;
          })
          .Case<Float6E3M2FNType>([&](Float6E3M2FNType type) {
            return hasRelu ? toolchain::Intrinsic::nvvm_e3m2x2_to_f16x2_rn_relu
                           : toolchain::Intrinsic::nvvm_e3m2x2_to_f16x2_rn;
          })
          .Default([](mlir::Type type) {
  ...
  toolchain::Intrinsic::ID intId =
      toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(curOp.getSrcType())
          .Case<Float4E2M1FNType>([&](Float4E2M1FNType type) {
            return hasRelu ? toolchain::Intrinsic::nvvm_e2m1x2_to_f16x2_rn_relu
                           : toolchain::Intrinsic::nvvm_e2m1x2_to_f16x2_rn;
          })
          .Default([](mlir::Type type) {
...
Tcgen05AllocOp::getIntrinsicIDAndArgs(Operation &op,
  ...
  unsigned as = toolchain::cast<LLVM::LLVMPointerType>(curOp.getAddr().getType())
                    .getAddressSpace();
  ...
  toolchain::Intrinsic::ID id;
  if (isShared) {
    id = is2CTAMode ? toolchain::Intrinsic::nvvm_tcgen05_alloc_shared_cg2
                    : toolchain::Intrinsic::nvvm_tcgen05_alloc_shared_cg1;
  } else {
    id = is2CTAMode ? toolchain::Intrinsic::nvvm_tcgen05_alloc_cg2
                    : toolchain::Intrinsic::nvvm_tcgen05_alloc_cg1;
  }
...
toolchain::Intrinsic::ID Tcgen05DeallocOp::getIntrinsicIDAndArgs(
    Operation &op, LLVM::ModuleTranslation &mt,
    toolchain::SmallVector<toolchain::Value *> &args) {
  auto curOp = cast<NVVM::Tcgen05DeallocOp>(op);
  auto id = (curOp.getGroup() == CTAGroupKind::CTA_1)
                ? toolchain::Intrinsic::nvvm_tcgen05_dealloc_cg1
                : toolchain::Intrinsic::nvvm_tcgen05_dealloc_cg2;
...
#define TCGEN05_COMMIT_IMPL(cg, is_shared, mc)                                 \
  is_shared ? toolchain::Intrinsic::nvvm_tcgen05_commit##mc##_shared##_##cg         \
            : toolchain::Intrinsic::nvvm_tcgen05_commit##mc##_##cg
...
#define TCGEN05_CP_IMPL(shape_mc, src_fmt, cg)                                 \
  toolchain::Intrinsic::nvvm_tcgen05_cp##shape_mc##src_fmt##cg
...
ConvertF32x2ToF16x2Op::getIntrinsicIDAndArgs(NVVM::ConvertF32x2ToF16x2Op &op,
                                             LLVM::ModuleTranslation &mt,
                                             toolchain::IRBuilderBase &builder) {
  static constexpr toolchain::Intrinsic::ID rndRNIds[] = {
      toolchain::Intrinsic::nvvm_ff2f16x2_rn,
      toolchain::Intrinsic::nvvm_ff2f16x2_rn_relu,
      toolchain::Intrinsic::nvvm_ff2f16x2_rn_satfinite,
      toolchain::Intrinsic::nvvm_ff2f16x2_rn_relu_satfinite,
  };
  static constexpr toolchain::Intrinsic::ID rndRZIds[] = {
      toolchain::Intrinsic::nvvm_ff2f16x2_rz,
      toolchain::Intrinsic::nvvm_ff2f16x2_rz_relu,
      toolchain::Intrinsic::nvvm_ff2f16x2_rz_satfinite,
      toolchain::Intrinsic::nvvm_ff2f16x2_rz_relu_satfinite,
  };
  static constexpr toolchain::Intrinsic::ID rndRSIds[] = {
      toolchain::Intrinsic::nvvm_ff2f16x2_rs,
      toolchain::Intrinsic::nvvm_ff2f16x2_rs_relu,
      toolchain::Intrinsic::nvvm_ff2f16x2_rs_satfinite,
      toolchain::Intrinsic::nvvm_ff2f16x2_rs_relu_satfinite,
  };
...
ConvertF32x2ToBF16x2Op::getIntrinsicIDAndArgs(NVVM::ConvertF32x2ToBF16x2Op &op,
                                              ...
                                              toolchain::IRBuilderBase &builder) {
  static constexpr toolchain::Intrinsic::ID rndRNIds[] = {
      toolchain::Intrinsic::nvvm_ff2bf16x2_rn,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rn_relu,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rn_satfinite,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rn_relu_satfinite,
  };
  static constexpr toolchain::Intrinsic::ID rndRZIds[] = {
      toolchain::Intrinsic::nvvm_ff2bf16x2_rz,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rz_relu,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rz_satfinite,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rz_relu_satfinite,
  };
  static constexpr toolchain::Intrinsic::ID rndRSIds[] = {
      toolchain::Intrinsic::nvvm_ff2bf16x2_rs,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rs_relu,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rs_satfinite,
      toolchain::Intrinsic::nvvm_ff2bf16x2_rs_relu_satfinite,
  };
...
toolchain::Intrinsic::ID ConvertF32x4ToF8x4Op::getIntrinsicID() {
  mlir::Type dstTy = getDstTy();
  bool hasRelu = getRelu();

  return toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(dstTy)
      .Case<mlir::Float8E4M3FNType>([&](mlir::Float8E4M3FNType) {
        return hasRelu ? toolchain::Intrinsic::nvvm_f32x4_to_e4m3x4_rs_relu_satfinite
                       : toolchain::Intrinsic::nvvm_f32x4_to_e4m3x4_rs_satfinite;
      })
      .Case<mlir::Float8E5M2Type>([&](mlir::Float8E5M2Type) {
        return hasRelu ? toolchain::Intrinsic::nvvm_f32x4_to_e5m2x4_rs_relu_satfinite
                       : toolchain::Intrinsic::nvvm_f32x4_to_e5m2x4_rs_satfinite;
      })
      .Default([](mlir::Type) {
...
toolchain::Intrinsic::ID ConvertF32x4ToF6x4Op::getIntrinsicID() {
  mlir::Type dstTy = getDstTy();
  bool hasRelu = getRelu();

  return toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(dstTy)
      .Case<mlir::Float6E2M3FNType>([&](mlir::Float6E2M3FNType) {
        return hasRelu ? toolchain::Intrinsic::nvvm_f32x4_to_e2m3x4_rs_relu_satfinite
                       : toolchain::Intrinsic::nvvm_f32x4_to_e2m3x4_rs_satfinite;
      })
      .Case<mlir::Float6E3M2FNType>([&](mlir::Float6E3M2FNType) {
        return hasRelu ? toolchain::Intrinsic::nvvm_f32x4_to_e3m2x4_rs_relu_satfinite
                       : toolchain::Intrinsic::nvvm_f32x4_to_e3m2x4_rs_satfinite;
      })
      .Default([](mlir::Type) {
...
toolchain::Intrinsic::ID ConvertF32x4ToF4x4Op::getIntrinsicID() {
  mlir::Type dstTy = getDstTy();
  bool hasRelu = getRelu();

  return toolchain::TypeSwitch<mlir::Type, toolchain::Intrinsic::ID>(dstTy)
      .Case<mlir::Float4E2M1FNType>([&](mlir::Float4E2M1FNType) {
        return hasRelu ? toolchain::Intrinsic::nvvm_f32x4_to_e2m1x4_rs_relu_satfinite
                       : toolchain::Intrinsic::nvvm_f32x4_to_e2m1x4_rs_satfinite;
      })
      .Default([](mlir::Type) {
...
toolchain::Intrinsic::ID Tcgen05CpOp::getIntrinsicID(Operation &op) {
...
LogicalResult Tcgen05LdOp::verify() {
  ...
  unsigned resLen = isa<VectorType>(resTy)
                        ? toolchain::cast<VectorType>(resTy).getNumElements()
                        : 1;
  if (!isValidVectorLength(getShape(), resLen))
    result = emitError(toolchain::formatv("invalid result type length {0} for shape "
                                     "{1} in tcgen05.ld Op",
...
LogicalResult Tcgen05StOp::verify() {
  ...
  unsigned valLen = isa<VectorType>(valTy)
                        ? toolchain::cast<VectorType>(valTy).getNumElements()
                        : 1;
  if (!isValidVectorLength(getShape(), valLen))
    result = emitError(toolchain::formatv("invalid input length {0} for shape "
                                     "{1} in tcgen05.st Op",
...
verifyConstantRangeAttr(Operation *op,
  ...
  const toolchain::APInt &lower = rangeAttr->getLower();
  const toolchain::APInt &upper = rangeAttr->getUpper();

  // Check LLVM ConstantRange constructor condition
  if (lower == upper && !lower.isMaxValue() && !lower.isMinValue()) {
    unsigned bitWidth = lower.getBitWidth();
    toolchain::APInt minVal = toolchain::APInt::getMinValue(bitWidth);
    toolchain::APInt maxVal = toolchain::APInt::getMaxValue(bitWidth);
    return op->emitOpError(
               "invalid range attribute: Lower == Upper, but they aren't min (")
           << toolchain::toString(minVal, 10, false) << ") or max ("
           << toolchain::toString(maxVal, 10, false)
...
static toolchain::Value *getAsPackedI32(toolchain::Value *arg,
                                   toolchain::IRBuilderBase &builder) {
...
NVVM::IDArgPair DotAccumulate4WayOp::getIntrinsicIDAndArgs(
  ...
  const toolchain::Intrinsic::ID ids[] = {
      toolchain::Intrinsic::nvvm_idp4a_u_u,
      toolchain::Intrinsic::nvvm_idp4a_u_s,
      toolchain::Intrinsic::nvvm_idp4a_s_u,
      toolchain::Intrinsic::nvvm_idp4a_s_s,
  };
...
NVVM::IDArgPair DotAccumulate2WayOp::getIntrinsicIDAndArgs(
  ...
  unsigned type = (isASigned << 1) | isBSigned;
  const toolchain::Intrinsic::ID ids[] = {
      toolchain::Intrinsic::nvvm_idp2a_u_u,
      toolchain::Intrinsic::nvvm_idp2a_u_s,
      toolchain::Intrinsic::nvvm_idp2a_s_u,
      toolchain::Intrinsic::nvvm_idp2a_s_s,
  };
  return {ids[type], args};
}

static toolchain::Value *getParamCastedAddr(toolchain::Value *addr,
                                       toolchain::IRBuilderBase &builder) {
  return builder.CreateAddrSpaceCast(
      addr,
      toolchain::PointerType::get(builder.getContext(),
                             toolchain::NVPTXAS::AddressSpace::ADDRESS_SPACE_PARAM));
}
...
PrefetchOp::getIntrinsicIDAndArgs(NVVM::PrefetchOp &op,
  ...
  std::optional<NVVM::CacheEvictionPriority> evictPriority =
      op.getEvictPriority();
  unsigned addressSpace =
      toolchain::cast<LLVM::LLVMPointerType>(op.getAddr().getType())
          .getAddressSpace();

  toolchain::SmallVector<toolchain::Value *> args;
  toolchain::Value *addr = mt.lookupValue(op.getAddr());
  args.push_back(op.getInParamSpace() ? getParamCastedAddr(addr, builder)
                                      : addr);

  if (op.getTensormap())
    return {toolchain::Intrinsic::nvvm_prefetch_tensormap, args};

  assert(cacheLevel && "expected cache level for non-tensormap prefetch");

  if (op.getUniform() && *cacheLevel == CacheLevel::L1)
    return {toolchain::Intrinsic::nvvm_prefetchu_L1, args};

  if (evictPriority && *cacheLevel == CacheLevel::L2) {
    switch (*evictPriority) {
    case NVVM::CacheEvictionPriority::EvictLast:
      return {toolchain::Intrinsic::nvvm_prefetch_global_L2_evict_last, args};
    case NVVM::CacheEvictionPriority::EvictNormal:
      return {toolchain::Intrinsic::nvvm_prefetch_global_L2_evict_normal, args};
    default:
  ...
  case MemSpace::Generic:
    return *cacheLevel == CacheLevel::L1
               ? NVVM::IDArgPair({toolchain::Intrinsic::nvvm_prefetch_L1, args})
               : NVVM::IDArgPair({toolchain::Intrinsic::nvvm_prefetch_L2, args});
  case MemSpace::Global:
    return *cacheLevel == CacheLevel::L1
               ? NVVM::IDArgPair(
                     {toolchain::Intrinsic::nvvm_prefetch_global_L1, args})
               : NVVM::IDArgPair(
                     {toolchain::Intrinsic::nvvm_prefetch_global_L2, args});
  case MemSpace::Local:
    return *cacheLevel == CacheLevel::L1
               ? NVVM::IDArgPair(
                     {toolchain::Intrinsic::nvvm_prefetch_local_L1, args})
               : NVVM::IDArgPair(
                     {toolchain::Intrinsic::nvvm_prefetch_local_L2, args});
  default:
...
NVVM::IDArgPair ClusterLaunchControlTryCancelOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Intrinsic::ID intrinsicID =
      curOp.getMulticast()
          ...
          : toolchain::Intrinsic::nvvm_clusterlaunchcontrol_try_cancel_async_shared;
...
NVVM::IDArgPair ClusterLaunchControlQueryCancelOp::getIntrinsicIDAndArgs(
  ...
  case NVVM::ClusterLaunchControlQueryType::IS_CANCELED:
    intrinsicID =
        toolchain::Intrinsic::nvvm_clusterlaunchcontrol_query_cancel_is_canceled;
    break;
  case NVVM::ClusterLaunchControlQueryType::GET_FIRST_CTA_ID_X:
    intrinsicID = toolchain::Intrinsic::
...
PermuteOp::getIntrinsicIDAndArgs(Operation &op, LLVM::ModuleTranslation &mt,
  ...
  static constexpr toolchain::Intrinsic::ID IDs[] = {
      toolchain::Intrinsic::nvvm_prmt,     toolchain::Intrinsic::nvvm_prmt_f4e,
      toolchain::Intrinsic::nvvm_prmt_b4e, toolchain::Intrinsic::nvvm_prmt_rc8,
      toolchain::Intrinsic::nvvm_prmt_ecl, toolchain::Intrinsic::nvvm_prmt_ecr,
      toolchain::Intrinsic::nvvm_prmt_rc16};
...
Tcgen05MMAOp::getIntrinsicIDAndArgs(Operation &op, LLVM::ModuleTranslation &mt,
  ...
  toolchain::Value *A = mt.lookupValue(thisOp.getMatrixA());
  const bool isATensor = isa<toolchain::PointerType>(A->getType());
  args.push_back(A);
  ...
  using EnableAShiftArray = std::array<toolchain::Intrinsic::ID, 2>;
      ...
      {  // without diable output lane
       {{// without scale input D
         {{
             // shared
             {{// cg1
               {toolchain::Intrinsic::nvvm_tcgen05_mma_shared, notIntrinsic},
               // cg2
               {toolchain::Intrinsic::nvvm_tcgen05_mma_shared, notIntrinsic}}},
             {{// tensor
               {
                   // cg1
                   toolchain::Intrinsic::nvvm_tcgen05_mma_tensor,
                   toolchain::Intrinsic::nvvm_tcgen05_mma_tensor_ashift,
               },
               {
                   // cg2
                   toolchain::Intrinsic::nvvm_tcgen05_mma_tensor,
         ...
         {{  // shared
           {{// cg1
             {toolchain::Intrinsic::nvvm_tcgen05_mma_shared_scale_d, notIntrinsic},
             // cg2
             {toolchain::Intrinsic::nvvm_tcgen05_mma_shared_scale_d, notIntrinsic}}},
           {{// tensor
             {
                 // cg1
                 toolchain::Intrinsic::nvvm_tcgen05_mma_tensor_scale_d,
                 toolchain::Intrinsic::nvvm_tcgen05_mma_tensor_scale_d_ashift,
             },
             {
                 // cg2
                 toolchain::Intrinsic::nvvm_tcgen05_mma_tensor_scale_d,
       ...
       {{    // without scale input D
         {{  // shared
           {{// cg1
             {toolchain::Intrinsic::nvvm_tcgen05_mma_shared_disable_output_lane_cg1,
              notIntrinsic},
             // cg2
             {toolchain::Intrinsic::nvvm_tcgen05_mma_shared_disable_output_lane_cg2,
              notIntrinsic}}},
           {{// cg1
             {
                 toolchain::Intrinsic::
  ...
  toolchain::Value *ScaleInputD = mt.lookupValue(thisOp.getScaleInputD());
  bool hasScaleInputD = ScaleInputD != nullptr;

  toolchain::Value *DisableOutputLane =
...
mlir::NVVM::IDArgPair Tcgen05MMASparseOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Value *A = mt.lookupValue(thisOp.getMatrixA());
  bool isATensor = isa<toolchain::PointerType>(A->getType());
  args.push_back(A);
      ...
      {  // without diable output lane
       {{// without scale input D
         {{
             // shared
             {{// cg1
               {toolchain::Intrinsic::nvvm_tcgen05_mma_sp_shared, notIntrinsic},
               // cg2
               {toolchain::Intrinsic::nvvm_tcgen05_mma_sp_shared, notIntrinsic}}},
             {{// tensor
               {
                   // cg1
                   toolchain::Intrinsic::nvvm_tcgen05_mma_sp_tensor,
                   toolchain::Intrinsic::nvvm_tcgen05_mma_sp_tensor_ashift,
               },
               {
                   // cg2
                   toolchain::Intrinsic::nvvm_tcgen05_mma_sp_tensor,
         ...
         {{  // shared
           {{// cg1
             {toolchain::Intrinsic::nvvm_tcgen05_mma_sp_shared_scale_d,
              notIntrinsic},
             // cg2
             {toolchain::Intrinsic::nvvm_tcgen05_mma_sp_shared_scale_d,
              notIntrinsic}}},
           {{// tensor
             {
                 // cg1
                 toolchain::Intrinsic::nvvm_tcgen05_mma_sp_tensor_scale_d,
                 toolchain::Intrinsic::nvvm_tcgen05_mma_sp_tensor_scale_d_ashift,
             },
             {
                 // cg2
                 toolchain::Intrinsic::nvvm_tcgen05_mma_sp_tensor_scale_d,
  ...
  toolchain::Intrinsic::ID ID = [&]() {
    if (kind == NVVM::MMABlockScaleKind::MXF8F6F4) {
      if (blockScale == NVVM::Tcgen05MMABlockScale::DEFAULT) {
        return isATensor ? toolchain::Intrinsic::
      ...
      if (blockScale == NVVM::Tcgen05MMABlockScale::DEFAULT) {
        return isATensor
                   ? toolchain::Intrinsic::nvvm_tcgen05_mma_tensor_mxf4_block_scale
                   : toolchain::Intrinsic::nvvm_tcgen05_mma_shared_mxf4_block_scale;
      } else if (blockScale == NVVM::Tcgen05MMABlockScale::BLOCK32) {
        return isATensor ? toolchain::Intrinsic::
...
static LogicalResult verifyTcgen05MMABlockScaleOp(
  ...
  if (blockScale == NVVM::Tcgen05MMABlockScale::BLOCK16 &&
      kind != MMABlockScaleKind::MXF4NVF4)
    return emitError(loc,
                     toolchain::formatv("{} kind does not support block16 attribute",
                                   stringifyEnum(kind)));
...
mlir::NVVM::IDArgPair Tcgen05MMAWsOp::getIntrinsicIDAndArgs(
  ...
  toolchain::Intrinsic::ID ID = notIntrinsic;
  if (ZeroColMask) {
    args.push_back(mt.lookupValue(ZeroColMask));
    ID = isATensor ? toolchain::Intrinsic::nvvm_tcgen05_mma_ws_tensor_zero_col_mask
                   : toolchain::Intrinsic::nvvm_tcgen05_mma_ws_shared_zero_col_mask;
  } else
    ID = isATensor ? toolchain::Intrinsic::nvvm_tcgen05_mma_ws_tensor
                   : toolchain::Intrinsic::nvvm_tcgen05_mma_ws_shared;
...
mlir::NVVM::IDArgPair Tcgen05MMAWsSparseOp::getIntrinsicIDAndArgs(
  ...
  if (ZeroColMask) {
    args.push_back(mt.lookupValue(ZeroColMask));
    ID = isATensor
             ? toolchain::Intrinsic::nvvm_tcgen05_mma_ws_sp_tensor_zero_col_mask
             : toolchain::Intrinsic::nvvm_tcgen05_mma_ws_sp_shared_zero_col_mask;
  } else
    ID = isATensor ? toolchain::Intrinsic::nvvm_tcgen05_mma_ws_sp_tensor
                   : toolchain::Intrinsic::nvvm_tcgen05_mma_ws_sp_shared;
...
// TODO: This should be the toolchain.nvvm dialect once this is supported.
...
LogicalResult NVVMDialect::verifyOperationAttribute(Operation *op,
  ...
  if (attrName == NVVMDialect::getMaxntidAttrName() ||
    ...
    auto values = toolchain::dyn_cast<DenseI32ArrayAttr>(attr.getValue());
  ...
  if (attrName == NVVMDialect::getMinctasmAttrName() ||
    ...
    if (!toolchain::dyn_cast<IntegerAttr>(attr.getValue())) {
      return op->emitError()
...
NVVMTargetAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (files && !toolchain::all_of(files, [](::mlir::Attribute attr) {
...
LogicalResult NVVMTargetAttr::verifyTarget(Operation *gpuModule) {
  ...
  auto gpuModuleOp = toolchain::dyn_cast<gpu::GPUModuleOp>(gpuModule);
  if (!gpuModuleOp) {
  ...
  if (gpuModuleOp
          ->walk([&](Operation *op) {
            if (auto reqOp = toolchain::dyn_cast<NVVM::RequiresSMInterface>(op)) {
