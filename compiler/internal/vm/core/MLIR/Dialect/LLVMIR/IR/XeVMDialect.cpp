...
LogicalResult verifyMatrixInput(Op op) {
  static_assert(toolchain::is_one_of<Op, BlockLoad2dOp, BlockStore2dOp,
...
template <typename OpType, typename = std::enable_if_t<toolchain::is_one_of<
...
LogicalResult verify1DBlockArg(OpType op) {
  ...
  if (elemTySize == 1) {
    toolchain::SmallSet<int, 4> validSizes{2, 4, 8, 16};
    ...
    toolchain::SmallSet<int, 3> validSizes{2, 4, 8};
    if (validSizes.contains(vTy.getNumElements()))
      return success();
    else
      return op.emitOpError(
...
XeVMTargetAttr::verify(function_ref<InFlightDiagnostic()> emitError, int O,
  ...
  if (linkFiles) {
    for (Attribute fileAttr : linkFiles) {
      if (auto fileStrAttr = toolchain::dyn_cast<StringAttr>(fileAttr)) {
