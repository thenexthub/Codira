...
LogicalResult BVConstantOp::inferReturnTypes(
    ...
    ::toolchain::SmallVectorImpl<::mlir::Type> &inferredReturnTypes) {
...
ParseResult RepeatOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  Type inputType;
  toolchain::SMLoc countLoc = parser.getCurrentLocation();
  ...
  toolchain::SMLoc inputLoc = parser.getCurrentLocation();
...
static LogicalResult verifyQuantifierRegions(QuantifierOp op) {
  ...
  if (!toolchain::all_of(op.getBody().getArgumentTypes(), isAnyNonFuncSMTValueType))
    return op.emitOpError()
  ...
  for (auto regionWithIndex : toolchain::enumerate(op.getPatterns())) {
