...
bool mlir::emitc::isSupportedEmitCType(Type type) {
  if (toolchain::isa<emitc::OpaqueType>(type))
    return true;
  if (auto ptrType = toolchain::dyn_cast<emitc::PointerType>(type))
    return isSupportedEmitCType(ptrType.getPointee());
  if (auto arrayType = toolchain::dyn_cast<emitc::ArrayType>(type)) {
    auto elemType = arrayType.getElementType();
    return !toolchain::isa<emitc::ArrayType>(elemType) &&
  ...
  if (auto tensorType = toolchain::dyn_cast<TensorType>(type)) {
    if (!tensorType.hasStaticShape()) {
      return false;
    }
    auto elemType = tensorType.getElementType();
    if (toolchain::isa<emitc::ArrayType>(elemType)) {
      return false;
    }
    return isSupportedEmitCType(elemType);
  }
  if (auto tupleType = toolchain::dyn_cast<TupleType>(type)) {
    return toolchain::all_of(tupleType.getTypes(), [](Type type) {
      return !toolchain::isa<emitc::ArrayType>(type) && isSupportedEmitCType(type);
    });
  }
  return false;
}
...
bool mlir::emitc::isIntegerIndexOrOpaqueType(Type type) {
  return toolchain::isa<IndexType, emitc::OpaqueType>(type) ||
         isSupportedIntegerType(type) || isPointerWideType(type);
}

bool mlir::emitc::isSupportedFloatType(Type type) {
  if (auto floatType = toolchain::dyn_cast<FloatType>(type)) {
    switch (floatType.getWidth()) {
    case 16:
      return toolchain::isa<Float16Type, BFloat16Type>(type);
    case 32:
    case 64:
      return true;
    default:
...
bool mlir::emitc::isFundamentalType(Type type) {
  return toolchain::isa<IndexType>(type) || isPointerWideType(type) ||
...
static LogicalResult verifyInitializationAttribute(Operation *op,
  ...
  if (toolchain::isa<emitc::OpaqueAttr>(value))
    return success();

  if (toolchain::isa<StringAttr>(value))
...
LogicalResult ApplyOp::verify() {
  ...
  if (applicableOperatorStr == "&") {
    if (!toolchain::isa<emitc::LValueType>(operandType))
      return emitOpError("operand type must be an lvalue when applying `&`");
    if (!toolchain::isa<emitc::PointerType>(resultType))
      return emitOpError("result type must be a pointer when applying `&`");
  } else {
    if (!toolchain::isa<emitc::PointerType>(operandType))
...
LogicalResult emitc::CallOpaqueOp::verify() {
  ...
  if (std::optional<ArrayAttr> argsAttr = getArgs()) {
    for (Attribute arg : *argsAttr) {
      auto intAttr = toolchain::dyn_cast<IntegerAttr>(arg);
      if (intAttr && toolchain::isa<IndexType>(intAttr.getType())) {
      ...
      } else if (toolchain::isa<ArrayAttr>(
                     arg) /*&& toolchain::isa<NoneType>(arg.getType())*/) {
  ...
  if (std::optional<ArrayAttr> templateArgsAttr = getTemplateArgs()) {
    for (Attribute tArg : *templateArgsAttr) {
      if (!toolchain::isa<TypeAttr, IntegerAttr, FloatAttr, emitc::OpaqueAttr>(tArg))
        return emitOpError("template argument has invalid type");
    }
  }

  if (toolchain::any_of(getResultTypes(), toolchain::IsaPred<ArrayType>)) {
...
LogicalResult emitc::ConstantOp::verify() {
  ...
  if (failed(verifyInitializationAttribute(getOperation(), value)))
    return failure();
  if (auto opaqueValue = toolchain::dyn_cast<emitc::OpaqueAttr>(value)) {
...
ParseResult ExpressionOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  auto fnType = toolchain::dyn_cast<FunctionType>(type);
  if (!fnType)
    return parser.emitError(parser.getCurrentLocation(),
                            "expected function type");
  ...
  for (auto [unresolvedOperand, operandType] :
       toolchain::zip(operands, fnType.getInputs())) {
...
LogicalResult FuncOp::verify() {
  if (toolchain::any_of(getArgumentTypes(), toolchain::IsaPred<LValueType>)) {
...
LogicalResult emitc::SubscriptOp::verify() {
  ...
  if (auto arrayType = toolchain::dyn_cast<emitc::ArrayType>(getValue().getType())) {
  ...
  if (auto pointerType =
          toolchain::dyn_cast<emitc::PointerType>(getValue().getType())) {
    // Check number of indices.
...
LogicalResult emitc::VerbatimOp::verify() {
  ...
  size_t numPlaceholders = toolchain::count_if(*fmt, [](ReplacementItem &item) {
...
LogicalResult emitc::ArrayType::verify(
    ::toolchain::function_ref<::mlir::InFlightDiagnostic()> emitError,
    ::toolchain::ArrayRef<int64_t> shape, Type elementType) {
  if (shape.empty())
...
LogicalResult mlir::emitc::LValueType::verify(
  ...
  if (toolchain::isa<emitc::ArrayType>(value))
...
LogicalResult mlir::emitc::PointerType::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError, Type value) {
  if (toolchain::isa<emitc::LValueType>(value))
...
static Type getInitializerTypeForGlobal(Type type) {
  if (auto array = toolchain::dyn_cast<ArrayType>(type))
...
parseEmitCGlobalOpTypeAndInitialValue(OpAsmParser &parser, TypeAttr &typeAttr,
  ...
  if (!toolchain::isa<ElementsAttr, IntegerAttr, FloatAttr, emitc::OpaqueAttr>(
          initialValue))
    return parser.emitError(parser.getNameLoc())
...
LogicalResult GlobalOp::verify() {
  ...
  if (getInitialValue().has_value()) {
    ...
    if (auto elementsAttr = toolchain::dyn_cast<ElementsAttr>(initValue)) {
      auto arrayType = toolchain::dyn_cast<ArrayType>(getType());
      if (!arrayType)
...
GetGlobalOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (toolchain::isa<ArrayType>(globalType)) {
...
void SwitchOp::getRegionInvocationBounds(
  ...
  const auto *iteratorToInt = toolchain::find(getCases(), operandValue.getInt());
