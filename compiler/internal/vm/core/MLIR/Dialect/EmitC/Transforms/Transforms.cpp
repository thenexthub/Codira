...
ExpressionOp createExpression(Operation *op, OpBuilder &builder) {
  ...
  for (auto [operand, arg] :
       toolchain::zip(expressionOp.getOperands(), block.getArguments()))
...
struct FoldExpressionOp : public OpRewritePattern<ExpressionOp> {
  ...
  LogicalResult matchAndRewrite(ExpressionOp expressionOp,
    ...
    // expression.
    for (auto [operand, arg] : toolchain::zip(expressionOp.getOperands(),
      ...
      if (usedExpression || !operandExpression ||
          toolchain::any_of(arg.getUsers(), takesItsOperandsAddress) ||
    ...
    IRMapping mapper;
    for (auto [operand, arg] : toolchain::zip(foldedExpression.getOperands(),
    ...
    auto foldExpression = [&rewriter, &mapper](ExpressionOp expressionToFold,
      ...
      for (auto [operand, arg] :
           toolchain::zip(expressionToFold.getOperands(),
