...
static LogicalResult verifyRawBufferOp(T &op) {
  MemRefType bufferType = toolchain::cast<MemRefType>(op.getMemref().getType());
...
static bool staticallyOutOfBounds(OpType op) {
  ...
  if (strides.size() != op.getIndices().size())
    return false;
  int64_t indexVal = 0;
  for (auto pair : toolchain::zip(strides, op.getIndices())) {
...
LogicalResult ScaledExtPackedMatrixOp::verify() {
  ...
  assert(toolchain::is_contained({16, 32}, blockSize) && "invalid block size");
  ...
  assert(toolchain::is_contained(toolchain::ArrayRef<unsigned>{4, 6, 8}, bitWidth));
  ...
  if (!is_fp8) {
    if (is_block_16) {
      if (!toolchain::is_contained({0, 1}, firstScaleByte)) {
        return emitOpError("blockSize of 16 can only have firstScaleByte be 0 "
                           "or 1 for f4 and f6.");
      }
    } else {
      if (!toolchain::is_contained({0, 2}, firstScaleByte)) {
...
LogicalResult ScaledWMMAOp::verify() {
  ...
  auto isF8 = toolchain::IsaPred<Float8E4M3FNType, Float8E5M2Type>;
  auto isF6 = toolchain::IsaPred<Float6E2M3FNType, Float6E3M2FNType>;
  auto isF4 = toolchain::IsaPred<Float4E2M1FNType>;
  auto isScaleF8 = toolchain::IsaPred<Float8E8M0FNUType, Float8E4M3FNType>;
  auto isE8M0 = toolchain::IsaPred<Float8E8M0FNUType>;
  auto isE4M3 = toolchain::IsaPred<Float8E4M3FNType>;
...
LogicalResult MFMAOp::verify() {
  ...
  if (sourceElem.isFloat(8) || sourceElem.isFloat(6) || sourceElem.isFloat(4)) {
    ...
    if (auto sourceBVector = toolchain::dyn_cast<VectorType>(sourceBType)) {
  ...
  LogicalResult matchAndRewrite(MemoryCounterWaitOp op,
    ...
    rewriter.modifyOpInPlace(op, [&] {
      for (auto [setter, lhs, rhs] :
           toolchain::zip_equal(setters, lhsVals, rhsVals)) {
        if (lhs && rhs) {
...
LogicalResult GatherToLDSOp::verify() {
  ...
  if (!toolchain::is_contained({8, 16, 32, 96, 128}, transferSize))
    return emitOpError(
...
LogicalResult TransposeLoadOp::verify() {
  ...
  const toolchain::SmallDenseMap<size_t, size_t> kValidLoadSizeMap = {
      {4, 16},
      {6, 16},
      {8, 8},
      {16, 4},
...
static LogicalResult verifyBase(BaseOp op) {
  ...
  if (!toolchain::is_contained({8u, 16u, 32u, 64u}, width))
    return op.emitOpError(
...
TDMGatherBaseType::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (!toolchain::is_contained({i16, i32}, indexType))
...
static LogicalResult verifyDescriptorOp(DescriptorOp op) {
  ...
  if (!toolchain::is_contained({8u, 16u, 32u, 64u}, elementTypeWidth))
    return op.emitOpError(
...
struct PackScales final : OpRewritePattern<ScaledMFMAOp> {
  ...
  LogicalResult matchAndRewrite(ScaledMFMAOp op,
    ...
    for (auto opIdx : std::array<int64_t, 2>({3, 4})) {
      ...
      auto extractedPos = toolchain::to_vector_of<int64_t>(
          toolchain::reverse(extractOp.getStaticPosition()));
