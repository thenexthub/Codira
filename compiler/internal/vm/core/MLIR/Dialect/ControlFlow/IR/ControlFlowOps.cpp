...
static LogicalResult collapseBranch(Block *&successor,
  ...
  toolchain::DenseSet<Block *> visited{successor, successorDest};
  while (nextBranch) {
  ...
  for (Value operand : operands) {
    BlockArgument argOperand = toolchain::dyn_cast<BlockArgument>(operand);
...
simplifyBrToBlockWithSinglePred(BranchOp op, PatternRewriter &rewriter) {
  ...
  if (succ == opParent || !toolchain::hasSingleElement(succ->getPredecessors()))
    return failure();
...
struct SimplifyCondBranchIdenticalSuccessors
  ...
  LogicalResult matchAndRewrite(CondBranchOp condbr,
    ...
    for (auto it : toolchain::zip(trueOperands, falseOperands)) {
...
struct CondBranchTruthPropagation : public OpRewritePattern<CondBranchOp> {
  ...
  LogicalResult matchAndRewrite(CondBranchOp condbr,
    ...
    if (condbr.getTrueDest()->getSinglePredecessor()) {
      for (OpOperand &use :
           toolchain::make_early_inc_range(condbr.getCondition().getUses())) {
...
struct DropUnreachableCondBranch : public OpRewritePattern<CondBranchOp> {
  ...
  LogicalResult matchAndRewrite(CondBranchOp condbr,
    ...
    if (toolchain::hasSingleElement(*trueDest) &&
    ...
    // destination.
    if (toolchain::hasSingleElement(*falseDest) &&
...
Block *CondBranchOp::getSuccessorForOperands(ArrayRef<Attribute> operands) {
  if (IntegerAttr condAttr =
          toolchain::dyn_cast_or_null<IntegerAttr>(operands.front()))
...
static void printSwitchOpCases(
  ...
  for (const auto &it : toolchain::enumerate(caseValues.getValues<APInt>())) {
    p << ',';
    p.printNewline();
    p << "  ";
...
Block *SwitchOp::getSuccessorForOperands(ArrayRef<Attribute> operands) {
  ...
  if (auto value = toolchain::dyn_cast_or_null<IntegerAttr>(operands.front())) {
    for (const auto &it : toolchain::enumerate(caseValues->getValues<APInt>()))
...
simplifySwitchFromSwitchOnSameCondition(SwitchOp op,
  ...
  auto it = toolchain::find(predDests, currentBlock);
  if (it != predDests.end()) {
