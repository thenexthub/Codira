...
struct ReassociationIndexRange {
  ...
  getNonOverlappingIndicesWith(ReassociationIndexRange &rhs) const {
    ...
    toolchain::append_range(result, toolchain::seq(leftStart, leftEnd));
    ...
    if (rightStart < rightEnd)
      toolchain::append_range(result, toolchain::seq_inclusive(rightStart, rightEnd));
    return result;
  }
...
mlir::composeReassociationIndices(
  ...
  size_t consumerDims =
      toolchain::accumulate(consumerReassociations, size_t(0),
  ...
  for (ReassociationIndicesRef consumerIndices : consumerReassociations) {
    ...
    for (int64_t consumerIndex : consumerIndices) {
      toolchain::append_range(reassociations, producerReassociations[consumerIndex]);
    }
...
ArrayAttr mlir::getReassociationIndicesAttribute(
  ...
  SmallVector<Attribute, 4> reassociationAttr =
      toolchain::to_vector<4>(toolchain::map_range(
...
LogicalResult mlir::reshapeLikeShapesAreCompatible(
  ...
  for (const auto &map : toolchain::enumerate(reassociationMaps)) {
    bool foundDynamicShape = false;
    int64_t linearizedStaticShape = 1;

    for (const auto &dim : toolchain::enumerate(
...
mlir::getSlicedDimensions(ArrayRef<OpFoldResult> sliceInputShape,
  ...
  toolchain::SmallBitVector mask(sliceInputShape.size());
  unsigned idx = 0;
...
toolchain::SmallBitVector mlir::getLinearizedDimensions(
    ArrayRef<ReassociationIndices> reassociationIndices) {
  toolchain::SmallBitVector result(reassociationIndices.size());
  for (const auto &it : toolchain::enumerate(reassociationIndices))
  ...
  for (const auto &it : toolchain::enumerate(reassociationIndices)) {
    ...
    if (slicedDimensions[it.index()] && linearizedDimensions[it.index()]) {
      toolchain::append_range(
          ...
          toolchain::map_range(multiIndices[loopIdx++], [&](Value v) -> Range {
    ...
    if (linearizedDimensions[it.index()]) {
      toolchain::append_range(offsetsSizesAndStrides,
                         toolchain::map_range(it.value(), [&](int64_t idx) -> Range {
...
canCollapseShapeBeSimplifiedByRankReducingSlice(
  ...
  if (!toolchain::any_of(trivialSegments, [](const std::optional<int64_t> &idx) {
...
mlir::getSimplifyCollapseShapeWithRankReducingSliceInfo(
  ...
  for (const auto &[nonUnitDim, indices] :
       toolchain::zip(*trivialSegments, reassociationIndices)) {
    if (nonUnitDim) {
      sliceShape.push_back(sourceType.getDimSize(*nonUnitDim));
      continue;
    }
    toolchain::append_range(sliceShape, toolchain::map_range(indices, [&](int64_t idx) {
...
PackingMetadata mlir::computePackingMetadata(int64_t packedRank,
  ...
  for (int64_t pos : innerDimPos) {
    int64_t numInsertedBefore = toolchain::count_if(
