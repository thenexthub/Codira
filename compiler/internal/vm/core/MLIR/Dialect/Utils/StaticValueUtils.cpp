...
void dispatchIndexOpFoldResult(OpFoldResult ofr,
  ...
  auto v = toolchain::dyn_cast_if_present<Value>(ofr);
  if (!v) {
...
SmallVector<OpFoldResult> getAsOpFoldResult(ValueRange values) {
  return toolchain::to_vector(
      toolchain::map_range(values, [](Value v) { return getAsOpFoldResult(v); }));
}
...
std::optional<std::pair<APInt, bool>> getConstantAPIntValue(OpFoldResult ofr) {
  ...
  Attribute attr = toolchain::dyn_cast_if_present<Attribute>(ofr);
...
getConstantIntValues(ArrayRef<OpFoldResult> ofrs) {
  bool failed = false;
  SmallVector<int64_t> res = toolchain::map_to_vector(ofrs, [&](OpFoldResult ofr) {
...
bool areConstantIntValues(ArrayRef<OpFoldResult> ofrs,
  ...
  return constOfrs && toolchain::equal(constOfrs.value(), values);
}
...
bool isEqualConstantIntOrValue(OpFoldResult ofr1, OpFoldResult ofr2) {
  ...
  if (cst1 && cst2 && *cst1 == *cst2)
    return true;
  auto v1 = toolchain::dyn_cast_if_present<Value>(ofr1),
       v2 = toolchain::dyn_cast_if_present<Value>(ofr2);
  return v1 && v1 == v2;
}
...
bool isEqualConstantIntOrValueArray(ArrayRef<OpFoldResult> ofrs1,
  ...
  if (ofrs1.size() != ofrs2.size())
    return false;
  for (auto [ofr1, ofr2] : toolchain::zip_equal(ofrs1, ofrs2))
...
SmallVector<OpFoldResult> getMixedValues(ArrayRef<int64_t> staticValues,
                                         ...
                                         MLIRContext *context) {
  assert(dynamicValues.size() == static_cast<size_t>(toolchain::count_if(
...
getValuesSortedByKeyImpl(ArrayRef<K> keys, ArrayRef<V> values,
                         toolchain::function_ref<bool(K, K)> compare) {
  if (keys.empty())
    return SmallVector<V>{values};
  assert(keys.size() == values.size() && "unexpected mismatching sizes");
  auto indices = toolchain::to_vector(toolchain::seq<int64_t>(0, values.size()));
  toolchain::sort(indices,
...
getValuesSortedByKey(ArrayRef<Attribute> keys, ArrayRef<Value> values,
                     toolchain::function_ref<bool(Attribute, Attribute)> compare) {
...
std::optional<APInt> constantTripCount(
    ...
    toolchain::function_ref<std::optional<toolchain::APSInt>(Value, Value, bool)>
        computeUbMinusLb) {
  ...
  if (maybeLbCst) {
    ...
    std::optional<toolchain::APSInt> maybeDiff =
  ...
  auto &stepCst = maybeStepCst->first;
  toolchain::APInt tripCount = isSigned ? diff.sdiv(stepCst) : diff.udiv(stepCst);
  toolchain::APInt remainder = isSigned ? diff.srem(stepCst) : diff.urem(stepCst);
  if (!remainder.isZero())
...
bool hasValidSizesOffsets(SmallVector<int64_t> sizesOrOffsets) {
  return toolchain::none_of(sizesOrOffsets, [](int64_t value) {
    return ShapedType::isStatic(value) && value < 0;
  });
}

bool hasValidStrides(SmallVector<int64_t> strides) {
  return toolchain::none_of(strides, [](int64_t value) {
    return ShapedType::isStatic(value) && value == 0;
  });
}
