...
static SmallVector<ExprType> computeElementwiseMulImpl(ArrayRef<ExprType> v1,
  ...
  for (auto it : toolchain::zip_equal(v1, v2))
...
SmallVector<int64_t> mlir::computeSuffixProduct(ArrayRef<int64_t> sizes) {
  assert((sizes.empty() ||
          toolchain::all_of(sizes.drop_front(), [](int64_t s) { return s >= 0; })) &&
...
int64_t mlir::computeProduct(ArrayRef<int64_t> basis) {
  assert(toolchain::all_of(basis, [](int64_t s) { return s > 0; }) &&
         "basis must be nonnegative");
  return toolchain::product_of(basis);
}
...
SmallVector<int64_t> mlir::delinearize(int64_t linearIndex,
  ...
  assert(toolchain::all_of(strides, [](int64_t s) { return s > 0; }) &&
...
mlir::computeShapeRatio(ArrayRef<int64_t> shape, ArrayRef<int64_t> subShape) {
  if (shape.size() < subShape.size())
    return std::nullopt;
  assert(toolchain::all_of(shape, [](int64_t s) { return s > 0; }) &&
         "shape must be nonnegative");
  assert(toolchain::all_of(subShape, [](int64_t s) { return s > 0; }) &&
  ...
  for (auto [size, subSize] :
       toolchain::zip(toolchain::reverse(shape), toolchain::reverse(subShape))) {
...
AffineExpr mlir::computeSum(MLIRContext *ctx, ArrayRef<AffineExpr> basis) {
  return toolchain::sum_of(basis, getAffineConstantExpr(0, ctx));
}

AffineExpr mlir::computeProduct(MLIRContext *ctx, ArrayRef<AffineExpr> basis) {
  return toolchain::product_of(basis, getAffineConstantExpr(1, ctx));
}
...
mlir::invertPermutationVector(ArrayRef<int64_t> permutation) {
  assert(toolchain::all_of(permutation, [](int64_t s) { return s >= 0; }) &&
         "permutation must be non-negative");
  SmallVector<int64_t> inversion(permutation.size());
  for (const auto &pos : toolchain::enumerate(permutation)) {
...
bool mlir::isIdentityPermutation(ArrayRef<int64_t> permutation) {
  for (auto i : toolchain::seq<int64_t>(0, permutation.size()))
    if (permutation[i] != i)
      return false;
  return true;
}
...
bool mlir::isPermutationVector(ArrayRef<int64_t> interchange) {
  toolchain::SmallDenseSet<int64_t, 4> seenVals;
...
mlir::computePermutationVector(int64_t permSize, ArrayRef<int64_t> positions,
  ...
  for (auto [pos, desiredPos] : toolchain::zip_equal(positions, desiredPositions)) {
    res[desiredPos] = pos;
    seen.insert(pos);
  }
  int64_t nextPos = 0;
...
static SmallVector<int64_t> padTileShapeToSize(ArrayRef<int64_t> tileShape,
  ...
  toolchain::append_range(result, tileShape);
  return result;
}
