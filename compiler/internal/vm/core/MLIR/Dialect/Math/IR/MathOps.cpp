...
static Type getI1SameShape(Type type) {
  ...
  if (auto shapedType = toolchain::dyn_cast<ShapedType>(type))
  ...
  return constFoldBinaryOpConditional<FloatAttr>(
      ...
      [](const APFloat &a, const APFloat &b) -> std::optional<APFloat> {
        if (a.isZero() && b.isZero())
          return toolchain::APFloat::getNaN(a.getSemantics());
...
OpFoldResult math::CeilOp::fold(FoldAdaptor adaptor) {
  return constFoldUnaryOp<FloatAttr>(
      adaptor.getOperands(), [](const APFloat &a) {
        APFloat result(a);
        result.roundToIntegral(toolchain::RoundingMode::TowardPositive);
        return result;
      });
}
...
OpFoldResult math::RoundEvenOp::fold(FoldAdaptor adaptor) {
  return constFoldUnaryOp<FloatAttr>(
      adaptor.getOperands(), [](const APFloat &a) {
        APFloat result(a);
        result.roundToIntegral(toolchain::RoundingMode::NearestTiesToEven);
        return result;
      });
}
...
OpFoldResult math::FloorOp::fold(FoldAdaptor adaptor) {
  return constFoldUnaryOp<FloatAttr>(
      adaptor.getOperands(), [](const APFloat &a) {
        APFloat result(a);
        result.roundToIntegral(toolchain::RoundingMode::TowardNegative);
        return result;
      });
}
