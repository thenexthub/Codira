...
handleMultidimensionalVectors(ImplicitLocOpBuilder &builder,
                              ...
                              toolchain::function_ref<Value(ValueRange)> compute) {
  ...
  if (inputShape == toolchain::ArrayRef(vectorWidth))
  ...
  for (int64_t i = 0; i < maxIndex; ++i) {
    ...
    for (const auto &tuple : toolchain::enumerate(expandedOperands))
...
Value makePolynomialCalculation(ImplicitLocOpBuilder &builder,
                                toolchain::ArrayRef<Value> coeffs, Value x) {
...
LogicalResult insertCasts(Operation *op, PatternRewriter &rewriter) {
  ...
  for (Type t : toolchain::drop_begin(op->getResultTypes()))
    if (origType != t)
...
AtanApproximation::matchAndRewrite(math::AtanOp op,
  ...
  Value mpi4 = bcast(f32Cst(builder, toolchain::numbers::pi / 4));
  Value ans2 = arith::AddFOp::create(builder, mpi4, ans0);
  Value ans = arith::SelectOp::create(builder, cmp2, ans2, ans0);

  Value mpi2 = bcast(f32Cst(builder, toolchain::numbers::pi / 2));
...
LogicalResult SinAndCosApproximation<isSine, OpTy>::matchAndRewrite(
  ...
  static_assert(
      toolchain::is_one_of<OpTy, math::SinOp, math::CosOp>::value,
...
populateMathF32ExpansionPattern(RewritePatternSet &patterns,
                                toolchain::function_ref<bool(StringRef)> predicate,
...
void mlir::populateMathF32ExpansionPatterns(
    RewritePatternSet &patterns, toolchain::function_ref<bool(StringRef)> predicate,
    PatternBenefit benefit) {
