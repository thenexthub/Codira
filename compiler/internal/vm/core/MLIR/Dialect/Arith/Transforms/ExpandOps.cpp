...
struct F8E8M0ExtFOpConverter : public OpRewritePattern<arith::ExtFOp> {
  ...
  LogicalResult matchAndRewrite(arith::ExtFOp op,
    ...
    if (!toolchain::isa<Float8E8M0FNUType>(operandETy)) {
...
struct F8E8M0TruncFOpConverter : public OpRewritePattern<arith::TruncFOp> {
  ...
  LogicalResult matchAndRewrite(arith::TruncFOp op,
    ...
    Type resultTy = op.getType();
    Type resultETy = getElementTypeOrSelf(resultTy);
    if (!toolchain::isa<Float8E8M0FNUType>(resultETy)) {
...
struct ScalingExtFOpConverter : public OpRewritePattern<arith::ScalingExtFOp> {
  ...
  LogicalResult matchAndRewrite(arith::ScalingExtFOp op,
    ...
    if (!toolchain::isa<Float8E8M0FNUType>(scaleETy)) {
      return rewriter.notifyMatchFailure(
  ...
  void runOnOperation() override {
    ...
    target.addDynamicallyLegalOp<arith::ExtFOp>(
      [=](arith::ExtFOp op) {
        ...
        if (includeF8E8M0)
          legalTypes &= !toolchain::isa<Float8E8M0FNUType>(inETy);
        if (includeF4E2M1)
          legalTypes &= !toolchain::isa<Float4E2M1FNType>(inETy);
        return legalTypes;
      });
    ...
    target.addDynamicallyLegalOp<arith::TruncFOp>(
      [=](arith::TruncFOp op)  {
        ...
        if (includeF8E8M0)
          legalTypes &= !(toolchain::isa<Float8E8M0FNUType>(outETy));
        if (includeF4E2M1)
          legalTypes &= !toolchain::isa<Float4E2M1FNType>(outETy);
        return legalTypes;
      });
