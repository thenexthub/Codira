...
struct MaterializeKnownConstantValues : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    bool hasConstantResults = toolchain::any_of(op->getResults(), needsReplacing);
    ...
    for (Region &region : op->getRegions()) {
      for (Block &block : region.getBlocks()) {
        hasConstantRegionArgs |=
            toolchain::any_of(block.getArguments(), needsReplacing);
      }
    }
...
struct NarrowElementwise final : OpTraitRewritePattern<OpTrait::Elementwise> {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (!toolchain::all_equal(op->getResultTypes()))
      return rewriter.notifyMatchFailure(op, "mismatched result types");
    if (op->getNumOperands() == 0 ||
        !toolchain::all_of(op->getOperandTypes(),
    ...
    for (unsigned targetBitwidth : targetBitwidths) {
      ...
      IRMapping mapping;
      for (auto [arg, argRange] : toolchain::zip_first(op->getOperands(), ranges)) {
      ...
      for (auto [newRes, oldRes] :
           toolchain::zip_equal(newOp->getResults(), op->getResults())) {
...
struct FoldIndexCastChain final : OpRewritePattern<CastOp> {
  ...
  LogicalResult matchAndRewrite(CastOp op,
    ...
    if (!intType || !toolchain::is_contained(targetBitwidths, intType.getWidth()))
      return failure();
...
struct NarrowLoopBounds final : OpInterfaceRewritePattern<LoopLikeOpInterface> {
  ...
  LogicalResult matchAndRewrite(LoopLikeOpInterface loopLike,
    ...
    for (auto [idx, indVar, lbOFR, ubOFR, stepOFR] :
         toolchain::enumerate(*inductionVars, *lowerBounds, *upperBounds, *steps)) {
    ...
    for (auto [narrowingIdx, narrowingInfo] : toolchain::enumerate(narrowings)) {
