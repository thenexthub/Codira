...
mlir::inferExpandShapeOutputShape(OpBuilder &b, Location loc,
  ...
  if ((int64_t)outputShapeValues.size() !=
      toolchain::count(outputShapeInts, ShapedType::kDynamic))
    return std::nullopt;
...
toolchain::SmallBitVector mlir::getPositionsOfShapeOne(unsigned rank,
                                                  ArrayRef<int64_t> shape) {
  toolchain::SmallBitVector dimsToProject(shape.size());
...
mlir::getValueOrCreateConstantIndexOp(OpBuilder &b, Location loc,
  ...
  return toolchain::to_vector<4>(
      toolchain::map_range(valueOrAttrVec, [&](OpFoldResult value) -> Value {
...
Value createProduct(OpBuilder &builder, Location loc, ArrayRef<Value> values,
  ...
  return toolchain::accumulate(values, one, [&arithBuilder](Value acc, Value v) {
...
std::optional<FloatType> parseFloatType(MLIRContext *ctx, StringRef name) {
  Builder b(ctx);
  return toolchain::StringSwitch<std::optional<FloatType>>(name)
