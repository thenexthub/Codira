...
applyToIntegerAttrs(PatternRewriter &builder, Value res, Attribute lhs,
  ...
  APInt lhsVal = toolchain::cast<IntegerAttr>(lhs).getValue();
  APInt rhsVal = toolchain::cast<IntegerAttr>(rhs).getValue();
...
static toolchain::RoundingMode
convertArithRoundingModeToLLVMIR(RoundingMode roundingMode) {
  switch (roundingMode) {
  case RoundingMode::downward:
    return toolchain::RoundingMode::TowardNegative;
  case RoundingMode::to_nearest_away:
    return toolchain::RoundingMode::NearestTiesToAway;
  case RoundingMode::to_nearest_even:
    return toolchain::RoundingMode::NearestTiesToEven;
  case RoundingMode::toward_zero:
    return toolchain::RoundingMode::TowardZero;
  case RoundingMode::upward:
    return toolchain::RoundingMode::TowardPositive;
  }
...
static Type getI1SameShape(Type type) {
  ...
  if (toolchain::isa<UnrankedTensorType>(type))
...
LogicalResult arith::ConstantOp::verify() {
  ...
  if (toolchain::isa<IntegerType>(type) &&
      !toolchain::cast<IntegerType>(type).isSignless())
    return emitOpError("integer return type must be signless");
  // Any float or elements attribute are acceptable.
  if (!toolchain::isa<IntegerAttr, FloatAttr, ElementsAttr>(getValue())) {
    return emitOpError(
...
bool arith::ConstantOp::isBuildableWith(Attribute value, Type type) {
  ...
  return toolchain::isa<IntegerAttr, FloatAttr, ElementsAttr>(value);
}
...
bool arith::ConstantFloatOp::classof(Operation *op) {
  if (auto constOp = dyn_cast_or_null<arith::ConstantOp>(op))
    return toolchain::isa<FloatType>(constOp.getType());
  return false;
}
...
arith::AddUIExtendedOp::fold(FoldAdaptor adaptor,
  ...
  if (Attribute sumAttr = constFoldBinaryOp<IntegerAttr>(
    ...
    Attribute overflowAttr = constFoldBinaryOp<IntegerAttr>(
        ...
        getI1SameShape(toolchain::cast<TypedAttr>(sumAttr).getType()),
  ...
  if (Attribute lowAttr = constFoldBinaryOp<IntegerAttr>(
    ...
    Attribute highAttr = constFoldBinaryOp<IntegerAttr>(
        adaptor.getOperands(), [](const APInt &a, const APInt &b) {
          return toolchain::APIntOps::mulhs(a, b);
        });
...
arith::MulUIExtendedOp::fold(FoldAdaptor adaptor,
  ...
  if (Attribute lowAttr = constFoldBinaryOp<IntegerAttr>(
    ...
    Attribute highAttr = constFoldBinaryOp<IntegerAttr>(
        adaptor.getOperands(), [](const APInt &a, const APInt &b) {
          return toolchain::APIntOps::mulhu(a, b);
        });
    assert(highAttr && "Unexpected constant-folding failure");
...
OpFoldResult arith::MaximumFOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<FloatAttr>(
      adaptor.getOperands(),
      [](const APFloat &a, const APFloat &b) { return toolchain::maximum(a, b); });
}
...
OpFoldResult arith::MaxNumFOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<FloatAttr>(adaptor.getOperands(), toolchain::maxnum);
}
...
OpFoldResult MaxSIOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<IntegerAttr>(adaptor.getOperands(),
                                        [](const APInt &a, const APInt &b) {
                                          return toolchain::APIntOps::smax(a, b);
                                        });
}
...
OpFoldResult MaxUIOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<IntegerAttr>(adaptor.getOperands(),
                                        [](const APInt &a, const APInt &b) {
                                          return toolchain::APIntOps::umax(a, b);
                                        });
}
...
OpFoldResult arith::MinimumFOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<FloatAttr>(
      adaptor.getOperands(),
      [](const APFloat &a, const APFloat &b) { return toolchain::minimum(a, b); });
}
...
OpFoldResult arith::MinNumFOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<FloatAttr>(
      adaptor.getOperands(),
      [](const APFloat &a, const APFloat &b) { return toolchain::minnum(a, b); });
}
...
OpFoldResult MinSIOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<IntegerAttr>(adaptor.getOperands(),
                                        [](const APInt &a, const APInt &b) {
                                          return toolchain::APIntOps::smin(a, b);
                                        });
}
...
OpFoldResult MinUIOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldBinaryOp<IntegerAttr>(adaptor.getOperands(),
                                        [](const APInt &a, const APInt &b) {
                                          return toolchain::APIntOps::umin(a, b);
                                        });
}
...
static Type getUnderlyingType(Type type, type_list<ShapedTypes...>,
  ...
  if (toolchain::isa<ShapedType>(type) && !toolchain::isa<ShapedTypes...>(type))
    return {};

  auto underlyingType = getElementTypeOrSelf(type);
  if (!toolchain::isa<ElementTypes...>(underlyingType))
    return {};
...
static LogicalResult verifyExtOp(Op op) {
  ...
  if (toolchain::cast<ValType>(srcType).getWidth() >=
      toolchain::cast<ValType>(dstType).getWidth())
...
static FailureOr<APFloat> convertFloatValue(
    APFloat sourceValue, const toolchain::fltSemantics &targetSemantics,
    toolchain::RoundingMode roundingMode = toolchain::RoundingMode::NearestTiesToEven) {
...
OpFoldResult arith::ExtUIOp::fold(FoldAdaptor adaptor) {
  ...
  unsigned bitWidth = toolchain::cast<IntegerType>(resType).getWidth();
...
OpFoldResult arith::ExtFOp::fold(FoldAdaptor adaptor) {
  ...
  const toolchain::fltSemantics &targetSemantics = resElemType.getFloatSemantics();
...
OpFoldResult arith::TruncIOp::fold(FoldAdaptor adaptor) {
  if (matchPattern(getOperand(), m_Op<arith::ExtUIOp>()) ||
    ...
    if (toolchain::cast<IntegerType>(srcType).getWidth() >
        toolchain::cast<IntegerType>(dstType).getWidth()) {
      setOperand(src);
      return getResult();
    }
...
OpFoldResult arith::TruncFOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto extOp = getOperand().getDefiningOp<arith::ExtFOp>()) {
    ...
    if (toolchain::APFloatBase::isRepresentableBy(
  ...
  return constFoldCastOp<FloatAttr, FloatAttr>(
      ...
      [this, &targetSemantics](const APFloat &a, bool &castStatus) {
        ...
        toolchain::RoundingMode llvmRoundingMode =
...
OpFoldResult arith::UIToFPOp::fold(FoldAdaptor adaptor) {
  ...
  return constFoldCastOp<IntegerAttr, FloatAttr>(
      ...
      [&resEleType](const APInt &a, bool &castStatus) {
        FloatType floatTy = toolchain::cast<FloatType>(resEleType);
...
OpFoldResult arith::BitcastOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto denseAttr = dyn_cast_or_null<DenseElementsAttr>(operand))
    return denseAttr.bitcast(toolchain::cast<ShapedType>(resType).getElementType());
  /// Other shaped types unhandled.
  if (toolchain::isa<ShapedType>(resType))
    return {};

  /// Bitcast poison.
  if (toolchain::isa<ub::PoisonAttr>(operand))
    return ub::PoisonAttr::get(getContext());

  /// Bitcast integer or float to integer or float.
  APInt bits = toolchain::isa<FloatAttr>(operand)
                   ? toolchain::cast<FloatAttr>(operand).getValue().bitcastToAPInt()
                   : toolchain::cast<IntegerAttr>(operand).getValue();
...
static std::optional<int64_t> getIntegerWidth(Type t) {
  ...
  if (auto vectorIntType = dyn_cast<VectorType>(t)) {
    return toolchain::cast<IntegerType>(vectorIntType.getElementType()).getWidth();
  }
  return std::nullopt;
}
...
public:
  ...
  LogicalResult matchAndRewrite(CmpFOp op,
    ...
    FloatType floatTy = toolchain::cast<FloatType>(op.getRhs().getType());
    ...
    auto intTy = toolchain::cast<IntegerType>(intVal.getType());
...
struct SelectToExtUI : public OpRewritePattern<arith::SelectOp> {
  ...
  LogicalResult matchAndRewrite(arith::SelectOp op,
    ...
    if (!toolchain::isa<IntegerType>(op.getType()) || op.getType().isInteger(1))
      return failure();
  ...
  if (auto cond =
    ...
    if (auto lhs =
      ...
      if (auto rhs =
        ...
        auto condVals = toolchain::make_range(cond.value_begin<BoolAttr>(),
                                         cond.value_end<BoolAttr>());
        auto lhsVals = toolchain::make_range(lhs.value_begin<Attribute>(),
                                        lhs.value_end<Attribute>());
        auto rhsVals = toolchain::make_range(rhs.value_begin<Attribute>(),
                                        rhs.value_end<Attribute>());

        for (auto [condVal, lhsVal, rhsVal] :
             toolchain::zip_equal(condVals, lhsVals, rhsVals))
...
LogicalResult arith::SelectOp::verify() {
  ...
  if (!toolchain::isa<TensorType, VectorType>(resultType))
...
TypedAttr mlir::arith::getIdentityValueAttr(AtomicRMWKind kind, Type resultType,
  ...
  case AtomicRMWKind::maximumf: {
    const toolchain::fltSemantics &semantic =
        toolchain::cast<FloatType>(resultType).getFloatSemantics();
  ...
  case AtomicRMWKind::andi:
    return builder.getIntegerAttr(
        resultType,
        APInt::getAllOnes(toolchain::cast<IntegerType>(resultType).getWidth()));
  case AtomicRMWKind::maxs:
    return builder.getIntegerAttr(
        resultType, APInt::getSignedMinValue(
                        toolchain::cast<IntegerType>(resultType).getWidth()));
  ...
  case AtomicRMWKind::minu:
    return builder.getIntegerAttr(
        resultType,
        APInt::getMaxValue(toolchain::cast<IntegerType>(resultType).getWidth()));
...
std::optional<TypedAttr> mlir::arith::getNeutralElement(Operation *op) {
  std::optional<AtomicRMWKind> maybeKind =
      toolchain::TypeSwitch<Operation *, std::optional<AtomicRMWKind>>(op)
