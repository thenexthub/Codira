...
void transform::ApplyGPUToROCDLConversionPatternsOp::populatePatterns(
  ...
  assert(toolchain::succeeded(maybeChipset) && "expected valid chipset");
...
void transform::ApplyGPUPromoteShuffleToAMDGPUPatternsOp::populatePatterns(
  ...
  if (chipsetName) {
    ...
    assert(toolchain::succeeded(parsedChipset) && "expected valid chipset");
...
gpuMmaUnrollOrder(vector::ContractionOp contract) {
  ...
  for (auto [index, iter] : toolchain::enumerate(contract.getIteratorTypes())) {
    if (vector::isReductionIterator(iter)) {
      order.push_back(index);
    }
  }

  toolchain::SmallDenseSet<int64_t> dims;
...
getSubgroupMmaNativeVectorSize(Operation *op, int64_t m, int64_t n, int64_t k) {
  ...
  if (auto readOp = dyn_cast<vector::TransferReadOp>(op)) {
    ...
    return toolchain::to_vector(sliceType.getShape());
  }
...
checkMappingAttributeTypes(std::optional<TransformOpInterface> transformOp,
  ...
  bool hasBlockMapping = toolchain::any_of(forallOp.getMapping().value(),
                                      toolchain::IsaPred<GPUBlockMappingAttr>);
  bool hasWarpgroupMapping = toolchain::any_of(
      forallOp.getMapping().value(), toolchain::IsaPred<GPUWarpgroupMappingAttr>);
  bool hasWarpMapping = toolchain::any_of(forallOp.getMapping().value(),
                                     toolchain::IsaPred<GPUWarpMappingAttr>);
  bool hasThreadMapping = toolchain::any_of(forallOp.getMapping().value(),
                                       toolchain::IsaPred<GPUThreadMappingAttr>);
  bool hasLaneMapping = toolchain::any_of(forallOp.getMapping().value(),
                                     toolchain::IsaPred<GPULaneMappingAttr>);
  ...
  if (toolchain::any_of(forallOp.getDeviceMappingAttrs(), isLinear) &&
      !toolchain::all_of(forallOp.getDeviceMappingAttrs(), isLinear)) {
    return definiteFailureHelper(
        transformOp, forallOp,
...
static DiagnosedSilenceableFailure rewriteOneForallCommonImpl(
  ...
  DeviceMappingAttrInterface maxMapping = cast<DeviceMappingAttrInterface>(
      *toolchain::max_element(forallMappingAttrs, comparator));
  ...
  LDBG() << "----tmpMappingSizes extracted from scf.forall op: "
         << toolchain::interleaved(tmpMappingSizes);
  ...
  LDBG() << "----forallMappingSizes: " << toolchain::interleaved(forallMappingSizes);
  LDBG() << "----forallMappingAttrs: " << toolchain::interleaved(forallMappingAttrs);
  ...
  LDBG() << "------originalBasis: " << toolchain::interleaved(originalBasis);
  ...
  IRMapping bvm;
  for (auto [iv, dim] : toolchain::zip_equal(
  ...
  LDBG() << "----result forallMappingSizes: "
         << toolchain::interleaved(forallMappingSizes);
  LDBG() << "----result mappingIdOps: " << toolchain::interleaved(mappingIdOps);
