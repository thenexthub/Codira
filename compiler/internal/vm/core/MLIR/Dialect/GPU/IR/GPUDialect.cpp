...
bool GPUDialect::isWorkgroupMemoryAddressSpace(Attribute memorySpace) {
  if (!memorySpace)
    return false;
  if (auto gpuAttr = toolchain::dyn_cast<gpu::AddressSpaceAttr>(memorySpace))
...
static void printAsyncDependencies(OpAsmPrinter &printer, Operation *op,
  ...
  if (asyncTokenType)
    printer << ' ';
  printer << toolchain::interleaved_array(asyncDependencies);
}
...
static void printAttributions(OpAsmPrinter &p, StringRef keyword,
  ...
  toolchain::interleaveComma(
      toolchain::enumerate(values), p, [&p, attributes](auto pair) {
        ...
        if (attributes && attributionIndex < attributes.size())
          attrs = toolchain::cast<DictionaryAttr>(attributes[attributionIndex]);
        if (attrs)
...
static LogicalResult verifyAttributions(Operation *op,
  ...
  for (Value v : attributions) {
    auto type = toolchain::dyn_cast<MemRefType>(v.getType());
    if (!type)
    ...
    auto addressSpace =
        toolchain::dyn_cast_or_null<gpu::AddressSpaceAttr>(type.getMemorySpace());
    if (!addressSpace)
      continue;
...
static LogicalResult verifyReduceOpAndType(gpu::AllReduceOperation opName,
  ...
  if (toolchain::is_contained({Kind::MINSI, Kind::MINUI, Kind::MAXSI, Kind::MAXUI,
...
LogicalResult gpu::SubgroupReduceOp::verify() {
  ...
  if (clusterSize) {
    ...
    if (!toolchain::isPowerOf2_32(size)) {
  ...
  if (!toolchain::isPowerOf2_32(stride)) {
...
ParseResult LaunchOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  for (auto ssaValueAndType : toolchain::zip(regionArgs, dataTypes)) {
...
void LaunchFuncOp::build(OpBuilder &builder, OperationState &result,
  ...
  toolchain::fill(prop.operandSegmentSizes, 1);
...
static void printLaunchFuncOperands(OpAsmPrinter &printer, Operation *,
  ...
  if (operands.empty())
    return;
  printer << "args(";
  toolchain::interleaveComma(toolchain::zip_equal(operands, types), printer,
                        [&](const auto &pair) {
...
parseAttributions(OpAsmParser &parser, StringRef keyword,
  ...
  bool hadAttrs = toolchain::any_of(ArrayRef(args).drop_front(existingArgs),
...
static DictionaryAttr getAttributionAttrs(GPUFuncOp op, unsigned index,
                                          StringAttr attrName) {
  auto allAttrs = toolchain::dyn_cast_or_null<ArrayAttr>(op->getAttr(attrName));
  if (!allAttrs || index >= allAttrs.size())
    return DictionaryAttr();
  return toolchain::cast<DictionaryAttr>(allAttrs[index]);
}
...
LogicalResult gpu::ReturnOp::verify() {
  ...
  for (const auto &pair : toolchain::enumerate(
           toolchain::zip(function.getFunctionType().getResults(), getOperands()))) {
...
bool GPUModuleOp::hasTarget(Attribute target) {
  if (ArrayAttr targets = getTargetsAttr())
    return toolchain::count(targets.getValue(), target);
  return false;
}
...
LogicalResult GPUModuleOp::verify() {
  ...
  for (auto target : targets) {
    if (auto verifyTargetAttr =
            toolchain::dyn_cast<TargetAttrVerifyInterface>(target)) {
...
struct EraseTrivialCopyOp : public OpRewritePattern<MemcpyOp> {
  ...
  LogicalResult matchAndRewrite(MemcpyOp op,
    ...
    if (toolchain::any_of(dest.getUsers(), [op, dest](Operation *user) {
          return user != op &&
...
LogicalResult SubgroupMmaLoadMatrixOp::verify() {
  ...
  auto resMatrixType = toolchain::cast<gpu::MMAMatrixType>(resType);
  auto operand = resMatrixType.getOperand();
  auto srcMemrefType = toolchain::cast<MemRefType>(srcType);
...
LogicalResult SubgroupMmaStoreMatrixOp::verify() {
  ...
  auto srcMatrixType = toolchain::cast<gpu::MMAMatrixType>(srcType);
  auto dstMemrefType = toolchain::cast<MemRefType>(dstType);
...
LogicalResult SubgroupMmaComputeOp::verify() {
  ...
  opTypes.push_back(toolchain::cast<MMAMatrixType>(getOpA().getType()));
  opTypes.push_back(toolchain::cast<MMAMatrixType>(getOpB().getType()));
  opTypes.push_back(toolchain::cast<MMAMatrixType>(getOpC().getType()));
...
public:
  ...
  LogicalResult matchAndRewrite(WaitOp op,
    ...
    if (toolchain::none_of(op.getAsyncDependencies(), predicate))
      return failure();
...
public:
  ...
  LogicalResult matchAndRewrite(WaitOp op,
    ...
    if (toolchain::hasSingleElement(op.getAsyncDependencies()) &&
        op.getAsyncToken()) {
      rewriter.replaceOp(op, op.getAsyncDependencies());
      return success();
    }
...
LogicalResult AllocOp::verify() {
  auto memRefType = toolchain::cast<MemRefType>(getMemref().getType());
...
struct SimplifyDimOfAllocOp : public OpRewritePattern<memref::DimOp> {
  ...
  LogicalResult matchAndRewrite(memref::DimOp dimOp,
    ...
    auto memrefType = toolchain::dyn_cast<MemRefType>(dimOp.getSource().getType());
...
void WarpExecuteOnLane0Op::print(OpAsmPrinter &p) {
  ...
  p << "[" << toolchain::cast<IntegerAttr>(warpSizeAttr).getInt() << "]";
...
ParseResult WarpExecuteOnLane0Op::parse(OpAsmParser &parser,
  ...
  toolchain::SMLoc inputsOperandsLoc;
...
void WarpExecuteOnLane0Op::build(OpBuilder &builder, OperationState &result,
  ...
  for (auto [type, arg] : toolchain::zip_equal(blockArgTypes, args))
...
static LogicalResult verifyDistributedType(Type expanded, Type distributed,
  ...
  if (expanded == distributed)
    return success();
  auto expandedVecType = toolchain::dyn_cast<VectorType>(expanded);
  auto distributedVecType = toolchain::dyn_cast<VectorType>(distributed);
  ...
  if (toolchain::product_of(scales) != warpSize)
    return op->emitOpError()
...
KernelMetadataAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (argAttrs) {
    if (toolchain::any_of(argAttrs, [](Attribute attr) {
          return !toolchain::isa<DictionaryAttr>(attr);
        }))
      return emitError()
...
TargetOptions::TargetOptions(
    ...
    function_ref<void(toolchain::Module &)> initialLlvmIRCallback,
...
function_ref<void(toolchain::Module &)>
...
std::pair<toolchain::BumpPtrAllocator, SmallVector<const char *>>
TargetOptions::tokenizeCmdOptions(const std::string &cmdOptions) {
  std::pair<toolchain::BumpPtrAllocator, SmallVector<const char *>> options;
  toolchain::StringSaver stringSaver(options.first);
  ...
  if (!opts.empty() && opts.front() == '\'' && opts.back() == '\'')
    opts.consume_front("'"), opts.consume_back("'");
#ifdef _WIN32
  toolchain::cl::TokenizeWindowsCommandLine(opts, stringSaver, options.second,
                                       /*MarkEOLs=*/false);
#else
  toolchain::cl::TokenizeGNUCommandLine(opts, stringSaver, options.second,
                                   /*MarkEOLs=*/false);
#endif // _WIN32
  return options;
}
...
std::pair<toolchain::BumpPtrAllocator, SmallVector<const char *>>
TargetOptions::tokenizeAndRemoveSuffixCmdOptions(toolchain::StringRef startsWith) {
  size_t startPos = cmdOptions.find(startsWith);
  if (startPos == std::string::npos)
    return {toolchain::BumpPtrAllocator(), SmallVector<const char *>()};
