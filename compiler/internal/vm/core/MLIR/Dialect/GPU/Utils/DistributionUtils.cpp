...
WarpDistributionPattern::moveRegionToNewWarpOpAndAppendReturns(
  ...
  toolchain::SmallDenseMap<Value, unsigned> indexLookup;
  // Record the value -> first index mapping for faster lookup.
  for (auto [i, v] : toolchain::enumerate(yieldValues)) {
    if (!indexLookup.count(v))
      indexLookup[v] = i;
  }

  for (auto [value, type] : toolchain::zip_equal(newYieldedValues, newReturnTypes)) {
...
OpOperand *WarpDistributionPattern::getWarpResult(
    ...
    toolchain::function_ref<bool(Operation *)> fn) const {
  gpu::YieldOp yield = warpOp.getTerminator();
...
bool WarpDistributionPattern::delinearizeLaneId(
  ...
  for (auto [large, small] : toolchain::zip_equal(originalShape, distributedShape)) {
    if (large % small != 0)
      return false;
    sizes.push_back(large / small);
  }
  if (toolchain::product_of(sizes) != warpSize)
    return false;
