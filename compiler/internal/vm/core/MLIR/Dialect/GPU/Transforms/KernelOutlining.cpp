...
static bool extractBeneficiaryOps(
    ...
    toolchain::SmallPtrSetImpl<Value> &availableValues,
    toolchain::function_ref<bool(Operation *)> isSinkingBeneficiary) {
...
LogicalResult mlir::sinkOperationsIntoLaunchOp(
  ...
  toolchain::SmallPtrSet<Value, 4> availableValues;
  ...
  for (Operation *op : toBeSunk) {
    ...
    for (auto pair : toolchain::zip(op->getResults(), clonedOp->getResults()))
...
static gpu::GPUFuncOp outlineKernelFuncImpl(gpu::LaunchOp launchOp,
  ...
  for (const auto &[launchArg, funcArg] :
       toolchain::zip(launchOp.getWorkgroupAttributions(),
                 outlinedFunc.getWorkgroupAttributions()))
    map.map(launchArg, funcArg);
  for (const auto &[launchArg, funcArg] :
       toolchain::zip(launchOp.getPrivateAttributions(),
...
gpu::GPUFuncOp mlir::outlineKernelFunc(gpu::LaunchOp launchOp,
                                       StringRef kernelFnName,
                                       toolchain::SmallVectorImpl<Value> &operands) {
  DenseSet<Value> inputOperandSet;
  inputOperandSet.insert_range(operands);
  SetVector<Value> operandSet(toolchain::from_range, operands);
