...
private:
  ...
  ///
  ///     toolchain.cond_br %condition, ^then, ^continue
  ///   ^then:
  ///     %then_operands = `thenOpsFactory()`
  ///     toolchain.br ^continue(%then_operands)
  ///   ^else:
  ///     %else_operands = `elseOpsFactory()`
  ///     toolchain.br ^continue(%else_operands)
        ...
        [&] {
          ...
          for (int i = 1; i < kSubgroupSize; i <<= 1) {
            ...
            createIf(
                ...
                [&] { return toolchain::ArrayRef(value); });
