  ...
  LogicalResult matchAndRewrite(gpu::SubgroupReduceOp op,
    ...
    if (elemBitwidth >= maxShuffleBitwidth)
      return rewriter.notifyMatchFailure(
          op, toolchain::formatv("element type too large ({0}), cannot break down "
    ...
    unsigned numNewReductions =
        toolchain::divideCeil(vecTy.getNumElements(), elementsPerShuffle);
...
getAndValidateClusterInfo(gpu::SubgroupReduceOp op, unsigned subgroupSize) {
  assert(toolchain::isPowerOf2_32(subgroupSize));

  std::optional<uint32_t> clusterSize = op.getClusterSize();
  assert(!clusterSize ||
         toolchain::isPowerOf2_32(*clusterSize)); // Verifier should've caught this.
  ...
  assert(toolchain::isPowerOf2_32(clusterStride)); // Verifier should've caught this.
...
struct ScalarSubgroupReduceToShuffles final
  ...
  LogicalResult matchAndRewrite(gpu::SubgroupReduceOp op,
    ...
    if (op.getClusterSize().has_value() != matchClustered) {
      return rewriter.notifyMatchFailure(
          op, toolchain::formatv("op is {0}clustered but pattern is configured to "
...
struct VectorSubgroupReduceToShuffles final
  ...
  LogicalResult matchAndRewrite(gpu::SubgroupReduceOp op,
    ...
    if (vecBitwidth > shuffleBitwidth)
      return rewriter.notifyMatchFailure(
          op,
          toolchain::formatv("vector type bitwidth too large ({0}), cannot lower "
