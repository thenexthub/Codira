...
void GpuModuleToBinaryPass::runOnOperation() {
  ...
  auto targetFormat =
      toolchain::StringSwitch<std::optional<CompilationTarget>>(compilationTarget)
          .Cases({"offloading", "toolchain"}, CompilationTarget::Offload)
...
LogicalResult mlir::gpu::transformGpuModulesToBinaries(
  ...
  for (Region &region : op->getRegions())
    for (Block &block : region.getBlocks())
      for (auto module :
           toolchain::make_early_inc_range(block.getOps<GPUModuleOp>()))
