...
collectEffects(Operation *op,
  ...
  if (auto iface = dyn_cast<MemoryEffectOpInterface>(op)) {
    ...
    toolchain::append_range(effects, localEffects);
    return true;
  }
...
static Value propagatesCapture(Operation *op) {
  return toolchain::TypeSwitch<Operation *, Value>(op)
      .Case(
...
static std::optional<bool> getKnownCapturingStatus(Operation *op, Value v) {
  return toolchain::TypeSwitch<Operation *, std::optional<bool>>(op)
...
static bool mayAlias(Value first, Value second) {
  ...
  auto isNoaliasFuncArgument = [](Value value) {
    ...
    return iface.getArgAttr(bbArg.getArgNumber(), "toolchain.noalias") != nullptr;
  };
