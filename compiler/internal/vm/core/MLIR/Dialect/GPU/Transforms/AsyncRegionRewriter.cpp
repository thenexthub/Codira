...
private:
  ...
  LogicalResult visit(Operation *op) {
    ...
    if (auto waitOp = toolchain::dyn_cast<gpu::WaitOp>(op)) {
      if (currentToken)
  ...
  LogicalResult rewriteAsyncOp(gpu::AsyncOpInterface asyncOp) {
    ...
    IRMapping mapping;
    for (auto pair : toolchain::zip_first(op->getRegions(), newOp->getRegions()))
...
struct GpuAsyncRegionPass::DeferWaitCallback {
  ...
  void operator()(async::ExecuteOp executeOp) {
    ...
    for (auto &op : toolchain::reverse(executeOp.getBody()->without_terminator())) {
...
private:
  ...
  static bool areAllUsersExecuteOrAwait(Value token) {
    return !token.use_empty() &&
           toolchain::all_of(token.getUsers(),
                        toolchain::IsaPred<async::ExecuteOp, async::AwaitOp>);
  }
...
struct GpuAsyncRegionPass::SingleTokenUseCallback {
  void operator()(async::ExecuteOp executeOp) {
    ...
    auto multiUseResults = toolchain::make_filter_range(
    ...
    for (auto index : indices) {
      ...
      auto uses = toolchain::drop_begin(executeOp.getBodyResults()[index].getUses());
      ...
      uses = toolchain::drop_begin(executeOp.getBodyResults()[index].getUses());
      auto results = executeOp.getBodyResults().take_back(count);
      for (auto pair : toolchain::zip(uses, results))
