...
void transform::AlternativesOp::getSuccessorRegions(
  ...
  for (Region &alternative : toolchain::drop_begin(
...
transform::AlternativesOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (Value scopeHandle = getScope())
    toolchain::append_range(originals, state.getPayloadOps(scopeHandle));
  else
  ...
  for (Region &reg : getAlternatives()) {
    ...
    auto clones = toolchain::to_vector(
        toolchain::map_range(originals, [](Operation *op) { return op->clone(); }));
    toolchain::scope_exit deleteClones([&] {
    ...
    if (!failed) {
      ...
      for (const auto &kvp : toolchain::zip(originals, clones)) {
...
transform::AnnotateOp::apply(transform::TransformRewriter &rewriter,
  ...
  SmallVector<Operation *> targets =
      toolchain::to_vector(state.getPayloadOps(getTarget()));
  ...
  if (auto paramH = getParam()) {
    ...
    if (params.size() != 1) {
      ...
      for (auto &&[target, attr] : toolchain::zip_equal(targets, params))
...
DiagnosedSilenceableFailure transform::ApplyDeadCodeEliminationOp::applyToOne(
  ...
  auto eraseOp = [&](Operation *op) {
    ...
    op->walk([&](Operation *op) {
      const auto *it = toolchain::find(worklist, op);
      if (it != worklist.end())
        worklist.erase(it);
    });
    rewriter.eraseOp(op);
...
LogicalResult transform::ApplyConversionPatternsOp::verify() {
  ...
  if (getNumRegions() == 2) {
    ...
    if (!toolchain::hasSingleElement(typeConverterRegion.front()))
      return emitOpError()
...
transform::ApplyRegisteredPassOp::apply(transform::TransformRewriter &rewriter,
  ...
  std::string options;
  toolchain::raw_string_ostream optionsStream(options); // For "printing" attrs.
  ...
  std::function<void(Attribute)> appendValueAttr = [&](Attribute valueAttr) {
    if (auto paramOperand = dyn_cast<transform::ParamOperandAttr>(valueAttr)) {
      ...
      toolchain::interleave(attrsAssociatedToParam, optionsStream, appendValueAttr,
                       ",");
    } else if (auto arrayAttr = dyn_cast<ArrayAttr>(valueAttr)) {
      // Recursive so as to append all nested attrs of the array.
      toolchain::interleave(arrayAttr, optionsStream, appendValueAttr, ",");
...
static void printApplyRegisteredPassOptions(OpAsmPrinter &printer,
  ...
  std::function<void(Attribute)> printOptionValue = [&](Attribute valueAttr) {
    if (auto paramOperandAttr =
      ...
      printer << "[";
      toolchain::interleaveComma(arrayAttr, printer, printOptionValue);
      printer << "]";
    } else {
  ...
  printer << "{";
  toolchain::interleaveComma(options, printer, [&](NamedAttribute namedAttribute) {
...
bool transform::CastOp::areCastCompatible(TypeRange inputs, TypeRange outputs) {
  ...
  return toolchain::all_of(
      ...
      toolchain::IsaPred<transform::TransformHandleTypeInterface>);
}
...
transform::CollectMatchingOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Operation *root : state.getPayloadOps(getRoot())) {
    WalkResult walkResult = root->walk([&](Operation *op) {
      ...
      for (auto &&[i, mapping] : toolchain::enumerate(mappings)) {
        if (mapping.size() != 1) {
    ...
    for (auto &&[opResult, rawResult] :
         toolchain::zip_equal(getOperation()->getResults(), rawResults)) {
...
LogicalResult transform::CollectMatchingOp::verifySymbolUses(
  ...
  for (auto &&[i, matcherType, resultType] :
       toolchain::enumerate(resultTypes, getOperation()->getResultTypes())) {
...
transform::ForeachMatchOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (auto &&[matcher, action] :
       toolchain::zip_equal(getMatchers(), getActions())) {
    auto matcherSymbol =
  ...
  results.set(toolchain::cast<OpResult>(getUpdated()),
              state.getPayloadOps(getRoot()));
  for (auto &&[result, mapping] :
       toolchain::zip_equal(getForwardedOutputs(), actionResultMapping)) {
...
static void printForeachMatchSymbols(OpAsmPrinter &printer, Operation *op,
  ...
  printer.increaseIndent();
  for (auto &&[matcher, action, idx] : toolchain::zip_equal(
           matchers, actions, toolchain::seq<unsigned>(0, matchers.size()))) {
...
LogicalResult transform::ForeachMatchOp::verify() {
  ...
  toolchain::SmallPtrSet<Attribute, 8> matcherNames;
...
LogicalResult transform::ForeachMatchOp::verifySymbolUses(
  ...
  for (auto &&[matcher, action] :
    ...
    for (auto &&[i, operand, argument] :
         toolchain::enumerate(operandTypes, matcherArguments)) {
    ...
    for (auto &&[i, matcherType, actionType] :
         toolchain::enumerate(matcherResults, actionArguments)) {
    ...
    for (auto &&[i, resultType, actionType] :
         toolchain::enumerate(resultTypes, actionResults)) {
...
transform::ForeachOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (withZipShortest) {
    numIterations =
        toolchain::min_element(payloads, [&](const SmallVector<MappedValue> &a,
  ...
  for (size_t iterIdx = 0; iterIdx < numIterations; iterIdx++) {
    ...
    for (auto &&[argIdx, blockArg] : toolchain::enumerate(blockArguments)) {
    ...
    for (Operation &transform : getBody().front().without_terminator()) {
      DiagnosedSilenceableFailure result = state.applyTransform(
          toolchain::cast<transform::TransformOpInterface>(transform));
      if (!result.succeeded())
        return result;
    }
    ...
    for (auto &&[result, yieldOperand, resTuple] :
         toolchain::zip_equal(getResults(), yieldOperands, zippedResults))
      // NB: each iteration we add any number of ops/vals/params to a result.
      if (isa<TransformHandleTypeInterface>(result.getType()))
        toolchain::append_range(resTuple, state.getPayloadOps(yieldOperand));
      else if (isa<TransformValueHandleTypeInterface>(result.getType()))
        toolchain::append_range(resTuple, state.getPayloadValues(yieldOperand));
      else if (isa<TransformParamTypeInterface>(result.getType()))
        toolchain::append_range(resTuple, state.getParams(yieldOperand));
      else
  ...
  for (auto &&[result, resPayload] : zip_equal(getResults(), zippedResults))
    results.setMappedValues(toolchain::cast<OpResult>(result), resPayload);
...
void transform::ForeachOp::getEffects(
  ...
  for (auto &&[target, blockArg] :
       toolchain::zip(getTargetsMutable(), getBody().front().getArguments())) {
...
LogicalResult transform::ForeachOp::verify() {
  for (auto [targetOpt, bodyArgOpt] :
       toolchain::zip_longest(getTargets(), getBody().front().getArguments())) {
  ...
  for (auto [resultOpt, yieldOperandOpt] :
       toolchain::zip_longest(getResults(), getYieldOp().getOperands())) {
    ...
    for (int64_t i = 0, e = getNthParent(); i < e; ++i) {
      ...
      if (!parent) {
        if (getAllowEmptyResults()) {
          results.set(toolchain::cast<OpResult>(getResult()), parents);
          return DiagnosedSilenceableFailure::success();
        }
        DiagnosedSilenceableFailure diag =
...
transform::GetConsumersOfResult::apply(transform::TransformRewriter &rewriter,
  ...
  results.set(toolchain::cast<OpResult>(getResult()),
              toolchain::to_vector(target->getResult(resultNumber).getUsers()));
  return DiagnosedSilenceableFailure::success();
}
...
transform::GetDefiningOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Value v : state.getPayloadValues(getTarget())) {
    if (toolchain::isa<BlockArgument>(v)) {
      DiagnosedSilenceableFailure diag =
  ...
  results.set(toolchain::cast<OpResult>(getResult()), definingOps);
  return DiagnosedSilenceableFailure::success();
}
...
transform::GetProducerOfOperand::apply(transform::TransformRewriter &rewriter,
  ...
  results.set(toolchain::cast<OpResult>(getResult()), producers);
  return DiagnosedSilenceableFailure::success();
}
...
transform::GetOperandOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Operation *target : state.getPayloadOps(getTarget())) {
    ...
    toolchain::append_range(operands,
                       toolchain::map_range(operandPositions, [&](int64_t pos) {
...
transform::GetResultOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (Operation *target : state.getPayloadOps(getTarget())) {
    ...
    toolchain::append_range(opResults,
                       toolchain::map_range(resultPositions, [&](int64_t pos) {
...
transform::IncludeOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (auto &&[arg, map] :
       toolchain::zip_equal(callee.getBody().front().getArguments(), mappings)) {
  ...
  for (auto &&[result, mapping] : toolchain::zip_equal(getResults(), mappings))
    results.setMappedValues(result, mapping);
  return result;
}
...
transform::MatchParamCmpIOp::apply(transform::TransformRewriter &rewriter,
  ...
  for (auto &&[i, param, reference] : toolchain::enumerate(params, references)) {
    auto intAttr = toolchain::dyn_cast<IntegerAttr>(param);
    auto refAttr = toolchain::dyn_cast<IntegerAttr>(reference);
    if (!intAttr || !refAttr) {
      return emitDefiniteFailure()
...
transform::MergeHandlesOp::apply(transform::TransformRewriter &rewriter,
  ...
  if (isa<TransformHandleTypeInterface>(handles.front().getType())) {
    ...
    for (Value operand : handles)
      toolchain::append_range(operations, state.getPayloadOps(operand));
    if (!getDeduplicate()) {
      results.set(toolchain::cast<OpResult>(getResult()), operations);
      return DiagnosedSilenceableFailure::success();
    }

    SetVector<Operation *> uniqued(toolchain::from_range, operations);
    results.set(toolchain::cast<OpResult>(getResult()), uniqued.getArrayRef());
    return DiagnosedSilenceableFailure::success();
  }

  if (toolchain::isa<TransformParamTypeInterface>(handles.front().getType())) {
    SmallVector<Attribute> attrs;
    for (Value attribute : handles)
      toolchain::append_range(attrs, state.getParams(attribute));
    if (!getDeduplicate()) {
      results.setParams(cast<OpResult>(getResult()), attrs);
      return DiagnosedSilenceableFailure::success();
    }

    SetVector<Attribute> uniqued(toolchain::from_range, attrs);
    results.setParams(cast<OpResult>(getResult()), uniqued.getArrayRef());
    return DiagnosedSilenceableFailure::success();
  }

  assert(
      toolchain::isa<TransformValueHandleTypeInterface>(handles.front().getType()) &&
      "expected value handle type");
  SmallVector<Value> payloadValues;
  for (Value value : handles)
    toolchain::append_range(payloadValues, state.getPayloadValues(value));
  if (!getDeduplicate()) {
    results.setValues(cast<OpResult>(getResult()), payloadValues);
    return DiagnosedSilenceableFailure::success();
  }

  SetVector<Value> uniqued(toolchain::from_range, payloadValues);
...
verifyYieldingSingleBlockOp(FunctionOpInterface op, bool allowExternal) {
  ...
  for (auto [i, operandType, resultType] : toolchain::zip_equal(
           toolchain::seq<unsigned>(0, terminator->getNumOperands()),
...
verifyNamedSequenceOp(transform::NamedSequenceOp op, bool emitWarnings) {
  ...
  for (auto [i, operandType, resultType] :
       toolchain::zip_equal(toolchain::seq<unsigned>(0, terminator->getNumOperands()),
...
static void buildSequenceBody(OpBuilder &builder, OperationState &state,
  ...
  toolchain::append_range(types, extraBindingTypes);
  ...
  if constexpr (toolchain::function_traits<FnTy>::num_args == 3) {
...
transform::NumAssociationsOp::apply(transform::TransformRewriter &rewriter,
  ...
  size_t numAssociations =
      toolchain::TypeSwitch<Type, size_t>(getHandle().getType())
          .Case([&](TransformHandleTypeInterface opHandle) {
            return toolchain::range_size(state.getPayloadOps(getHandle()));
          })
          .Case([&](TransformValueHandleTypeInterface valueHandle) {
            return toolchain::range_size(state.getPayloadValues(getHandle()));
          })
          .Case([&](TransformParamTypeInterface param) {
            return toolchain::range_size(state.getParams(getHandle()));
          })
...
transform::SplitHandleOp::apply(transform::TransformRewriter &rewriter,
                                ...
                                transform::TransformState &state) {
  int64_t numPayloads =
      toolchain::TypeSwitch<Type, int64_t>(getHandle().getType())
  ...
  auto container = [&]() {
    ...
    if (isa<TransformValueHandleTypeInterface>(getHandle().getType())) {
      return toolchain::map_to_vector(state.getPayloadValues(getHandle()),
    ...
    return toolchain::map_to_vector(state.getParams(getHandle()),
                               [](Attribute a) -> MappedValue { return a; });
  }();

  for (auto &&en : toolchain::enumerate(container)) {
  ...
  for (auto &&it : toolchain::enumerate(resultHandles))
    results.setMappedValues(toolchain::cast<OpResult>(getResult(it.index())),
                            it.value());
...
transform::ReplicateOp::apply(transform::TransformRewriter &rewriter,
                              transform::TransformResults &results,
                              transform::TransformState &state) {
  unsigned numRepetitions = toolchain::range_size(state.getPayloadOps(getPattern()));
  for (const auto &en : toolchain::enumerate(getHandles())) {
    Value handle = en.value();
    if (isa<TransformHandleTypeInterface>(handle.getType())) {
      SmallVector<Operation *> current =
          toolchain::to_vector(state.getPayloadOps(handle));
      SmallVector<Operation *> payload;
      payload.reserve(numRepetitions * current.size());
      for (unsigned i = 0; i < numRepetitions; ++i)
        toolchain::append_range(payload, current);
      results.set(toolchain::cast<OpResult>(getReplicated()[en.index()]), payload);
    } else {
      assert(toolchain::isa<TransformParamTypeInterface>(handle.getType()) &&
             "expected param type");
      ArrayRef<Attribute> current = state.getParams(handle);
      SmallVector<Attribute> params;
      params.reserve(numRepetitions * current.size());
      for (unsigned i = 0; i < numRepetitions; ++i)
        toolchain::append_range(params, current);
      results.setParams(toolchain::cast<OpResult>(getReplicated()[en.index()]),
                        params);
    }
  }
  return DiagnosedSilenceableFailure::success();
...
static void printSequenceOpOperands(OpAsmPrinter &printer, Operation *op,
  ...
  printer << rootType;
  if (hasExtras)
    printer << ", " << toolchain::interleaved(extraBindingTypes) << ')';
}
...
transform::PrintOp::apply(transform::TransformRewriter &rewriter,
                          transform::TransformResults &results,
                          transform::TransformState &state) {
  toolchain::outs() << "[[[ IR printer: ";
  if (getName().has_value())
    toolchain::outs() << *getName() << " ";
  ...
  if (!getTarget()) {
    toolchain::outs() << "top-level ]]]\n";
    state.getTopLevel()->print(toolchain::outs(), printFlags);
    toolchain::outs() << "\n";
  ...
  toolchain::outs() << "]]]\n";
  for (Operation *target : state.getPayloadOps(getTarget())) {
    target->print(toolchain::outs(), printFlags);
    toolchain::outs() << "\n";
  }
