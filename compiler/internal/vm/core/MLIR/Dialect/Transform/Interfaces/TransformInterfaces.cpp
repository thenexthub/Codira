...
LogicalResult transform::TransformState::getHandlesForPayloadOp(
  ...
  bool found = false;
  for (const auto &[region, mapping] : toolchain::reverse(mappings)) {
    auto iterator = mapping->reverse.find(op);
    if (iterator != mapping->reverse.end()) {
      toolchain::append_range(handles, iterator->getSecond());
      found = true;
    }
...
static DiagnosedSilenceableFailure dispatchMappedValues(
  ...
  if (toolchain::isa<transform::TransformHandleTypeInterface>(handle.getType())) {
    SmallVector<Operation *> operations;
    operations.reserve(values.size());
    for (transform::MappedValue value : values) {
      if (auto *op = toolchain::dyn_cast_if_present<Operation *>(value)) {
  ...
  if (toolchain::isa<transform::TransformValueHandleTypeInterface>(
          handle.getType())) {
    SmallVector<Value> payloadValues;
    payloadValues.reserve(values.size());
    for (transform::MappedValue value : values) {
      if (auto v = toolchain::dyn_cast_if_present<Value>(value)) {
  ...
  assert(toolchain::isa<transform::TransformParamTypeInterface>(handle.getType()) &&
         "unsupported kind of block argument");
  SmallVector<transform::Param> parameters;
  parameters.reserve(values.size());
  for (transform::MappedValue value : values) {
    if (auto attr = toolchain::dyn_cast_if_present<Attribute>(value)) {
...
LogicalResult transform::TransformState::mapBlockArguments(
  ...
  for (auto &&[argument, values] : toolchain::zip_equal(arguments, mapping))
...
transform::TransformState::setPayloadOps(Value value,
  ...
  assert(toolchain::isa<TransformHandleTypeInterface>(value.getType()) &&
         "wrong handle type");
  ...
  auto iface = toolchain::cast<TransformHandleTypeInterface>(value.getType());
...
transform::TransformState::setPayloadValues(Value handle,
  ...
  assert(toolchain::isa<TransformValueHandleTypeInterface>(handle.getType()) &&
         "wrong handle type");
  ...
  auto iface = toolchain::cast<TransformValueHandleTypeInterface>(handle.getType());
  SmallVector<Value> payloadValueVector = toolchain::to_vector(payloadValues);
  DiagnosedSilenceableFailure result =
...
LogicalResult transform::TransformState::setParams(Value value,
  ...
  auto valueType = toolchain::dyn_cast<TransformParamTypeInterface>(value.getType());
  assert(value &&
  ...
  Mappings &mappings = getMapping(value);
  bool inserted =
      mappings.params.insert({value, toolchain::to_vector(params)}).second;
...
static void dropMappingEntry(Mapping &mapping, Key key, Mapped mapped) {
  ...
  toolchain::erase(it->getSecond(), mapped);
...
void transform::TransformState::recordOpHandleInvalidationOne(
  ...
  FULL_LDBG() << "--ancestors: "
              << toolchain::interleaved(
                     toolchain::make_pointee_range(potentialAncestors));
...
void transform::TransformState::recordValueHandleInvalidationByOpHandleOne(
  ...
  for (Operation *ancestor : potentialAncestors) {
    ...
    if (auto opResult = toolchain::dyn_cast<OpResult>(payloadValue)) {
      definingOp = opResult.getOwner();
      resultNo = opResult.getResultNumber();
    } else {
      auto arg = toolchain::cast<BlockArgument>(payloadValue);
...
void transform::TransformState::recordValueHandleInvalidation(
  ...
  for (Value payloadValue : getPayloadValuesView(valueHandle.get())) {
    ...
    if (auto opResult = toolchain::dyn_cast<OpResult>(payloadValue)) {
      ...
      auto arg = toolchain::dyn_cast<BlockArgument>(payloadValue);
...
LogicalResult transform::TransformState::checkAndRecordHandleInvalidationImpl(
  ...
  for (OpOperand &target : transform->getOpOperands()) {
    ...
    if (toolchain::any_of(effects, consumesTarget)) {
      FULL_LDBG() << "----found consume effect";
      if (toolchain::isa<transform::TransformHandleTypeInterface>(
              target.get().getType())) {
        FULL_LDBG() << "----recordOpHandleInvalidation";
        SmallVector<Operation *> payloadOps =
            toolchain::to_vector(getPayloadOps(target.get()));
        recordOpHandleInvalidation(target, payloadOps, nullptr,
                                   newlyInvalidated);
      } else if (toolchain::isa<transform::TransformValueHandleTypeInterface>(
                     target.get().getType())) {
...
void transform::TransformState::compactOpHandles() {
  for (Value handle : opHandlesToCompact) {
    Mappings &mappings = getMapping(handle, /*allowOutOfScope=*/true);
#if LLVM_ENABLE_ABI_BREAKING_CHECKS
    if (toolchain::is_contained(mappings.direct[handle], nullptr))
      // Payload IR is removed from the mapping. This invalidates the respective
      // iterators.
      mappings.incrementTimestamp(handle);
#endif // LLVM_ENABLE_ABI_BREAKING_CHECKS
    toolchain::erase(mappings.direct[handle], nullptr);
  }
...
transform::TransformState::applyTransform(TransformOpInterface transform) {
  ...
  toolchain::scope_exit printOnFailureRAII([this] {
    (void)this;
  ...
  if (options.getExpensiveChecksEnabled()) {
    ...
    for (OpOperand &operand : transform->getOpOperands()) {
      ...
      if (toolchain::isa<TransformHandleTypeInterface>(operandType)) {
      ...
      } else if (toolchain::isa<TransformValueHandleTypeInterface>(operandType)) {
  ...
  for (OpOperand *opOperand : consumedOperands) {
    ...
    if (toolchain::isa<TransformHandleTypeInterface>(operand.getType())) {
      for (Operation *payloadOp : getPayloadOps(operand)) {
        toolchain::append_range(origOpFlatResults, payloadOp->getResults());
      }
      continue;
    }
    if (toolchain::isa<TransformValueHandleTypeInterface>(operand.getType())) {
      for (Value payloadValue : getPayloadValuesView(operand)) {
        if (toolchain::isa<OpResult>(payloadValue)) {
          origAssociatedOps.push_back(payloadValue.getDefiningOp());
          continue;
        }
        toolchain::append_range(
            origAssociatedOps,
            toolchain::map_range(*toolchain::cast<BlockArgument>(payloadValue).getOwner(),
  ...
  config.skipHandleFn = [&](Value handle) {
    ...
    auto scopeIt =
        toolchain::find_if(toolchain::reverse(regionStack), [&](RegionScope *scope) {
    ...
    return toolchain::all_of(handle.getUsers(), [&](Operation *user) {
  ...
  for (OpOperand *opOperand : consumedOperands) {
    ...
    } else if (toolchain::isa<TransformValueHandleTypeInterface>(
                   operand.getType())) {
...
LogicalResult transform::TransformState::updateStateFromResults(
  ...
  for (OpResult result : opResults) {
    if (toolchain::isa<TransformParamTypeInterface>(result.getType())) {
    ...
    } else if (toolchain::isa<TransformValueHandleTypeInterface>(result.getType())) {
...
transform::TransformState::RegionScope::~RegionScope() {
  ...
  SmallVector<Operation *> referencedOps =
      toolchain::to_vector(toolchain::make_first_range(state.mappings[region]->reverse));
#endif // LLVM_ENABLE_ABI_BREAKING_CHECKS
...
#ifndef NDEBUG
  if (!diag.succeeded())
    toolchain::dbgs() << diag.getStatusString() << "\n";
...
void transform::TrackingListener::notifyOperationReplaced(
  ...
  for (auto [oldValue, newValue] : toolchain::zip(op->getResults(), newValues))
  ...
  auto handleWasConsumed = [&] {
    return toolchain::any_of(opHandles,
  ...
  if (config.skipHandleFn) {
    auto *it = toolchain::find_if(opHandles,
...
void transform::ErrorCheckingTrackingListener::notifyPayloadReplacementNotFound(
  ...
  for (auto &&[index, value] : toolchain::enumerate(values))
...
transform::detail::checkNestedConsumption(Location loc,
                                          ArrayRef<Operation *> targets) {
  for (auto &&[position, parent] : toolchain::enumerate(targets)) {
...
transform::detail::checkApplyToOne(Operation *transformOp,
  ...
  for (const auto &[ptr, res] :
       toolchain::zip(partialResult, transformOp->getResults())) {
    if (ptr.isNull())
      continue;
    if (toolchain::isa<TransformHandleTypeInterface>(res.getType()) &&
        !isa<Operation *>(ptr)) {
      return emitDiag() << "application of " << transformOpName
                        << " expected to produce an Operation * for result #"
                        << res.getResultNumber();
    }
    if (toolchain::isa<TransformParamTypeInterface>(res.getType()) &&
        !isa<Attribute>(ptr)) {
      return emitDiag() << "application of " << transformOpName
                        << " expected to produce an Attribute for result #"
                        << res.getResultNumber();
    }
    if (toolchain::isa<TransformValueHandleTypeInterface>(res.getType()) &&
        !isa<Value>(ptr)) {
      return emitDiag() << "application of " << transformOpName
...
static SmallVector<T> castVector(ArrayRef<transform::MappedValue> range) {
  return toolchain::map_to_vector(range, toolchain::CastTo<T>);
}
...
void transform::detail::setApplyToOneResults(
  ...
  for (const ApplyToEachResultList &partialResults : results) {
    if (toolchain::any_of(partialResults,
    ...
    for (auto [i, value] : toolchain::enumerate(partialResults))
      transposed[i].push_back(value);
  }

  for (OpResult r : transformOp->getResults()) {
    unsigned position = r.getResultNumber();
    if (toolchain::isa<TransformParamTypeInterface>(r.getType())) {
      transformResults.setParams(r,
                                 castVector<Attribute>(transposed[position]));
    } else if (toolchain::isa<TransformValueHandleTypeInterface>(r.getType())) {
...
LogicalResult transform::detail::appendValueMappings(
  ...
  for (auto &&[operand, mapped] : toolchain::zip_equal(values, mappings)) {
    size_t mappedSize = mapped.size();
    if (toolchain::isa<TransformHandleTypeInterface>(operand.getType())) {
      toolchain::append_range(mapped, state.getPayloadOps(operand));
    } else if (toolchain::isa<TransformValueHandleTypeInterface>(
                   operand.getType())) {
      toolchain::append_range(mapped, state.getPayloadValues(operand));
    } else {
      assert(toolchain::isa<TransformParamTypeInterface>(operand.getType()) &&
             "unsupported kind of transform dialect value");
      toolchain::append_range(mapped, state.getParams(operand));
    }
...
void transform::detail::forwardTerminatorOperands(
  ...
  for (auto &&[terminatorOperand, result] :
       toolchain::zip(block->getTerminator()->getOperands(),
                 block->getParentOp()->getOpResults())) {
    if (toolchain::isa<transform::TransformHandleTypeInterface>(result.getType())) {
      ...
      assert(
          toolchain::isa<transform::TransformParamTypeInterface>(result.getType()) &&
...
remapArgumentEffects(Block &block, MutableArrayRef<OpOperand> operands,
  ...
  for (Operation &op : block) {
    ...
    for (auto &&[source, target] : toolchain::zip(block.getArguments(), operands)) {
    ...
    iface.getEffectsOnResource(transform::PayloadIRResource::get(),
                               nestedEffects);
    toolchain::append_range(effects, nestedEffects);
  }
}
...
LogicalResult transform::detail::mapPossibleTopLevelTransformOpBlockArguments(
  ...
  if (op->getNumOperands() != 0) {
    toolchain::append_range(targets, state.getPayloadOps(op->getOperand(0)));
    ...
    for (unsigned i = 0, e = state.getNumTopLevelMappings(); i < e; ++i)
      extraMappings.push_back(toolchain::to_vector(state.getTopLevelMapping(i)));
  }
...
transform::detail::verifyPossibleTopLevelTransformOpTrait(Operation *op) {
  ...
  if (!toolchain::hasNItems(*bodyRegion, 1))
  ...
  if (!toolchain::isa<TransformHandleTypeInterface>(
  ...
  for (BlockArgument arg : body->getArguments().drop_front()) {
    if (toolchain::isa<TransformHandleTypeInterface, TransformParamTypeInterface,
...
static bool hasEffect(Range &&effects) {
  return toolchain::any_of(effects, [](const MemoryEffects::EffectInstance &effect) {
...
void transform::getConsumedBlockArguments(
    Block &block, toolchain::SmallDenseSet<unsigned int> &consumedArguments) {
  SmallVector<MemoryEffects::EffectInstance> effects;
...
SmallVector<OpOperand *> transform::detail::getConsumedHandleOpOperands(
  ...
  for (OpOperand &target : transformOp->getOpOperands()) {
    ...
    if (toolchain::any_of(effects, [](const MemoryEffects::EffectInstance &effect) {
