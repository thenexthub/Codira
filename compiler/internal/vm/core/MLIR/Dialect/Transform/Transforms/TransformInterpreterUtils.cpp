...
LogicalResult transform::detail::expandPathsToMLIRFiles(
  ...
  for (const std::string &path : paths) {
    ...
    if (toolchain::sys::fs::is_regular_file(path)) {
    ...
    std::error_code ec;
    for (toolchain::sys::fs::directory_iterator it(path, ec), itEnd;
         it != itEnd && !ec; it.increment(ec)) {
      const std::string &fileName = it->path();

      if (it->type() != toolchain::sys::fs::file_type::regular_file &&
          it->type() != toolchain::sys::fs::file_type::symlink_file) {
...
LogicalResult transform::detail::parseTransformModuleFromFile(
    MLIRContext *context, toolchain::StringRef transformFileName,
  ...
  sourceMgr.AddNewSourceBuffer(std::move(memoryBuffer), toolchain::SMLoc());
  transformModule =
