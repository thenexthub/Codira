...
const toolchain::SmallPtrSet<Block *, 4> &
getReachableImpl(Block *block, FnTy getNextNodes,
                 DenseMap<Block *, toolchain::SmallPtrSet<Block *, 4>> &cache) {
  auto [it, inserted] = cache.try_emplace(block);
  if (!inserted)
    return it->getSecond();

  toolchain::SmallPtrSet<Block *, 4> &reachable = it->second;
...
public:
  ...
  public:
    ...
    PotentialDeleters &operator|=(const PotentialDeleters &other) {
      toolchain::append_range(deleters, other.deleters);
      return *this;
    }
  ...
  private:
    ...
    explicit PotentialDeleters(ArrayRef<Operation *> ops) {
      toolchain::append_range(deleters, ops);
    }
  ...
  PotentialDeleters isUseLive(OpOperand &operand) {
    const toolchain::SmallPtrSet<Operation *, 2> &deleters = freedBy[operand.get()];
    if (deleters.empty())
      return live();
...
private:
  ...
  isFreedBetween(Value value, Operation *first, Operation *last,
                 toolchain::function_ref<Operation *(Operation *)> getNext) const {
    auto it = freedBy.find(value);
    if (it == freedBy.end())
      return live();
    const toolchain::SmallPtrSet<Operation *, 2> &deleters = it->getSecond();
  ...
  PotentialDeleters isMaybeFreedOnPaths(Block *from, Block *to, Value value,
    ...
    // is rechable.
    const toolchain::SmallPtrSet<Block *, 4> &sources = getReachableFrom(to);
    if (!sources.contains(from))
      return live();

    toolchain::SmallPtrSet<Block *, 4> reachable(getReachable(from));
    toolchain::set_intersect(reachable, sources);
  ...
  const toolchain::SmallPtrSet<Block *, 4> &getReachable(Block *block) {
  ...
  /// reachable.
  const toolchain::SmallPtrSet<Block *, 4> &getReachableFrom(Block *block) {
    return getReachableImpl(
  ...
  static bool hasEffect(ArrayRef<MemoryEffects::EffectInstance> instances,
                        Value value) {
    return toolchain::any_of(instances,
  ...
  DenseMap<Value, toolchain::SmallPtrSet<Operation *, 2>> freedBy;

  /// Caches for sets of reachable blocks.
  DenseMap<Block *, toolchain::SmallPtrSet<Block *, 4>> reachableCache;
  DenseMap<Block *, toolchain::SmallPtrSet<Block *, 4>> reachableFromCache;
};
