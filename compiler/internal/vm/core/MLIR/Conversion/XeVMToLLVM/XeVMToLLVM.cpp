...
std::string mangle(StringRef baseName, ArrayRef<Type> types,
  ...
  toolchain::SmallDenseMap<Type, unsigned> substitutions;
  os << "_Z" << baseName.size() << baseName;
  for (auto [idx, type] : toolchain::enumerate(types)) {
...
class MMAToOCLPattern : public OpConversionPattern<xevm::MMAOp> {
  ...
  matchAndRewrite(xevm::MMAOp op, xevm::MMAOp::Adaptor adaptor,
    ...
    std::string fnName =
        toolchain::formatv("intel_sub_group_{0}_{1}_matrix_mad_k{2}",
...
class LoadStorePrefetchToOCLPattern : public OpConversionPattern<OpType> {
  ...
  matchAndRewrite(OpType op, typename OpType::Adaptor adaptor,
    ...
    funcName =
        toolchain::formatv("{0}_{1}b_{2}r{3}x{4}c", funcName, op.getElemSizeInBits(),
                      op.getTileHeight(), op.getTileWidth(), op.getVBlocks())
            .str();
    std::string prefetchCode("");
    if (!isPrefetch)
      prefetchCode += "P";
    funcName = toolchain::formatv("_Z{0}{1}PU3AS1viiiDv2_i{2}{3}", funcName.size(),
...
/// Replace `xevm.*` with an `toolchain.call` to the corresponding OpenCL func with
