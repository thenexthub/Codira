...
void AffineLoopToGpuConverter::createLaunch(AffineForOp rootForOp,
  ...
  for (const auto &en : toolchain::enumerate(ivs)) {
    Value id =
...
static LogicalResult processParallelLoop(
  ...
  for (Operation &op : toolchain::reverse(body->without_terminator()))
...
ParallelToGpuLaunchLowering::matchAndRewrite(ParallelOp parallelOp,
  ...
  toolchain::DenseMap<gpu::Processor, Value> launchBounds;
  ...
  toolchain::DenseSet<Value> writtenBuffer;
  while (!worklist.empty()) {
    ...
    if (auto nestedParallel = dyn_cast<ParallelOp>(op)) {
      ...
      toolchain::SetVector<Value> externalValues;
