    ...
    if (tileRank == 2) {
      ...
      if (wScaleFactor > 1) {
        ...
        Value wScaleFactorValLog2 = arith::ConstantIntOp::create(
            rewriter, loc, rewriter.getI32Type(), toolchain::Log2_64(wScaleFactor));
  ...
  matchAndRewrite(OpType op, typename OpType::Adaptor adaptor,
    ...
    if constexpr (std::is_same_v<OpType, xegpu::LoadMatrixOp>) {
      ...
      if (auto vecType = dyn_cast<VectorType>(resType)) {
        assert(toolchain::count_if(vecType.getShape(),
...
struct ConvertXeGPUToXeVMPass
  ...
  void runOnOperation() override {
    ...
    typeConverter.addConversion([&](VectorType type) -> Type {
      ...
      if (toolchain::isa<IndexType>(elemType))
      ...
      int64_t sum = toolchain::product_of(type.getShape());
