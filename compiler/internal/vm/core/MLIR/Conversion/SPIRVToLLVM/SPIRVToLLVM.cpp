...
#define DEBUG_TYPE "spirv-to-toolchain-pattern"
...
/// Creates `toolchain.mlir.constant` with all bits set for the given type.
...
/// Creates `toolchain.mlir.constant` with a floating-point scalar or vector value.
...
public:
  ...
  matchAndRewrite(spirv::AccessChainOp op, OpAdaptor adaptor,
    ...
    auto indices = toolchain::to_vector<4>(adaptor.getIndices());
...
/// Converts `spirv.getCompositeExtract` to `toolchain.extractvalue` if the container
/// type is an aggregate type (struct or array). Otherwise, converts to
/// `toolchain.extractelement` that operates on vectors.
...
/// Converts `spirv.getCompositeInsert` to `toolchain.insertvalue` if the container
/// type is an aggregate type (struct or array). Otherwise, converts to
/// `toolchain.insertelement` that operates on vectors.
...
public:
  ...
  matchAndRewrite(spirv::ExecutionModeOp op, OpAdaptor adaptor,
    ...
    if (module.getName().has_value())
      ...
      moduleName = "";
    std::string executionModeInfoName = toolchain::formatv(
    ...
    // Create `toolchain.mlir.global` with initializer region containing one block.
...
/// Converts `spirv.GlobalVariable` to `toolchain.mlir.global`. Note that SPIR-V
...
/// `toolchain.mlir.addressof`ops that both return a pointer.
...
/// Converts SPIR-V floating-point comparisons to toolchain.fcmp "predicate"
...
/// Converts SPIR-V integer comparisons to toolchain.icmp "predicate"
...
StringRef getTypeMangling(Type type, bool isSigned) {
  return toolchain::TypeSwitch<Type, StringRef>(type)
...
///   |  | ^header:                           |
///   |  |   <header code>                    |
///   |  |   toolchain.cond_br %cond, ^body, ^exit |
///   |  +------------------------------------+
///   |                    |
///   |                    |----------------------+
///   |                    |                      |
      ...
      if (descriptorSet && binding) {
        ...
        auto moduleAndName =
            spvModule.getName().has_value()
                ...
                : op.getSymName().str();
        std::string name =
            toolchain::formatv("{0}_descriptor_set{1}_binding{2}", moduleAndName,
