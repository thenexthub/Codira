...
struct TransferReadToArmSMELowering
  ...
  LogicalResult matchAndRewrite(vector::TransferReadOp transferReadOp,
    ...
    if (!toolchain::isa<MemRefType>(transferReadOp.getBase().getType()))
...
struct TransferWriteToArmSMELowering
  ...
  LogicalResult matchAndRewrite(vector::TransferWriteOp writeOp,
    ...
    if (!toolchain::isa<MemRefType>(writeOp.getBase().getType()))
      return failure();
...
struct ExtractFromCreateMaskToPselLowering
  ...
  LogicalResult matchAndRewrite(vector::ExtractOp extractOp,
    ...
    auto isSVEPredicateSize = [](int64_t size) {
      return size > 0 && size <= 16 && toolchain::isPowerOf2_32(uint32_t(size));
    };
