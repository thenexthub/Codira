...
struct WmmaMmaOpToNVVMLowering
  ...
  matchAndRewrite(gpu::SubgroupMmaComputeOp subgroupMmaComputeOp,
    ...
    // The wmma.mma intrinsic in toolchain requires the operands as individual
    // values. So individual elements from the memrefs need to be extracted and
    // then passed on to the intrinsic call. Emit toolchain ops to extract individual
...
struct WmmaConstantOpToNVVMLowering
  ...
  matchAndRewrite(gpu::SubgroupMmaConstantMatrixOp subgroupMmaConstantOp,
    ...
    for (size_t i : toolchain::seq(size_t(0), structType.getBody().size())) {
      matrixStruct =
