...
struct GPUShuffleOpLowering : public ConvertOpToLLVMPattern<gpu::ShuffleOp> {
  ...
  ///
  ///     %one = toolchain.constant(1 : i32) : i32
  ///     %minus_one = toolchain.constant(-1 : i32) : i32
  ///     %thirty_two = toolchain.constant(32 : i32) : i32
  ///     %num_lanes = toolchain.sub %thirty_two, %width : i32
  ///     %active_mask = toolchain.lshr %minus_one, %num_lanes : i32
  ///     %mask_and_clamp = toolchain.sub %width, %one : i32
  ///     %shfl = nvvm.shfl.sync.bfly %active_mask, %value, %offset,
  ///         %mask_and_clamp : !toolchain<"{ float, i1 }">
  ///     %shfl_value = toolchain.extractvalue %shfl[0] :
  ///         !toolchain<"{ float, i1 }">
  ///     %shfl_pred = toolchain.extractvalue %shfl[1] :
  ///         !toolchain<"{ float, i1 }">
  LogicalResult
...
struct LowerGpuOpsToNVVMOpsPass final
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallDenseSet<StringRef> allowedDialectsSet(allowedDialects.begin(),
                                                      allowedDialects.end());
