...
// A `sincos` is converted into `toolchain.intr.sincos` followed by extractvalue ops.
...
struct IsNaNOpLowering
  ...
  matchAndRewrite(math::IsNaNOp op, OpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<LLVM::IsFPClass>(
        op, resultType, adaptor.getOperand(), toolchain::fcNan);
    return success();
  }
};
...
struct IsFiniteOpLowering
  ...
  matchAndRewrite(math::IsFiniteOp op, OpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<LLVM::IsFPClass>(
        op, resultType, adaptor.getOperand(), toolchain::fcFinite);
    return success();
  }
};
