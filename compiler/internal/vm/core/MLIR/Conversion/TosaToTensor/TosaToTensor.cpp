...
TensorType inferReshapeExpandedType(TensorType inputType,
  ...
  auto resultShape =
      toolchain::map_to_vector(newShape, [&](int64_t size) -> int64_t {
        ...
        int64_t totalSizeNoPlaceholder = -toolchain::product_of(newShape);
...
createReassociationMapForCollapse(OpBuilder &builder, Type srcType,
  ...
  if (ShapedType::isDynamicShape(srcShape) ||
    ...
    for (auto i : toolchain::seq<int64_t>(srcShape.size()))
...
public:
  ...
  matchAndRewrite(tosa::SliceOp sliceOp, OpAdaptor adaptor,
    ...
    if (toolchain::isa<UnrankedTensorType>(resultType))
      return failure();
    ...
    for (const auto &i : toolchain::enumerate(sliceSizes)) {
...
public:
  ...
  matchAndRewrite(tosa::PadOp padOp, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<int64_t> paddingVals;
...
struct ConcatConverter : public OpConversionPattern<tosa::ConcatOp> {
  ...
  matchAndRewrite(tosa::ConcatOp op, OpAdaptor adaptor,
    ...
    for (auto [arg, offset] : toolchain::zip(adaptor.getOperands(), axisOffsets)) {
