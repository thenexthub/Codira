...
struct OpenMPOpConversion : public ConvertOpToLLVMPattern<T> {
  ...
  matchAndRewrite(T op, typename T::Adaptor adaptor,
    ...
    for (auto [originalOperand, convertedOperand] :
      ...
      if constexpr (toolchain::is_one_of<T, omp::AtomicUpdateOp, omp::AtomicWriteOp,
    ...
    for (auto [originalRegion, convertedRegion] :
         toolchain::zip_equal(op->getRegions(), newOp->getRegions())) {
...
#include "mlir/Dialect/OpenMP/OpenMPOps.cpp.inc"
    ...
    return typeConverter.isLegal(op->getOperandTypes()) &&
           ...
           toolchain::all_of(op->getRegions(),
                        [&](Region &region) {
                          return typeConverter.isLegal(&region);
                        }) &&
           toolchain::all_of(op->getAttrs(), [&](NamedAttribute attr) {
