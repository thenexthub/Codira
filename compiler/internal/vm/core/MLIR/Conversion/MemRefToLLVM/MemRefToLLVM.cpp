...
#define DEBUG_TYPE "memref-to-toolchain"
...
public:
  ...
  int64_t alignedAllocationGetAlignment(memref::AllocOp op,
    ...
    return std::max(kMinAlignedAllocAlignment,
                    toolchain::PowerOf2Ceil(eltSizeBytes));
  }
...
struct AssumeAlignmentOpLowering
  ...
  matchAndRewrite(memref::AssumeAlignmentOp op, OpAdaptor adaptor,
    ...
    // Emit toolchain.assume(true) ["align"(memref, alignment)].
...
struct DistinctObjectsOpLowering
  ...
  matchAndRewrite(memref::DistinctObjectsOp op, OpAdaptor adaptor,
    ...
    for (auto [origOperand, newOperand] :
         toolchain::zip_equal(op.getOperands(), operands)) {
    ...
    for (auto i : toolchain::seq<size_t>(ptrs.size() - 1)) {
      for (auto j : toolchain::seq<size_t>(i + 1, ptrs.size())) {
        Value ptr1 = ptrs[i];
        Value ptr2 = ptrs[j];
...
public:
  ...
  matchAndRewrite(memref::DeallocOp op, OpAdaptor adaptor,
    ...
    Value allocatedPtr;
    if (auto unrankedTy =
            toolchain::dyn_cast<UnrankedMemRefType>(op.getMemref().getType())) {
...
/// Wrap a toolchain.cmpxchg operation in a while loop so that the operation can be
...
convertGlobalMemrefTypeToLLVM(MemRefType type,
  ...
  for (int64_t dim : toolchain::reverse(type.getShape()))
...
public:
  ...
  matchAndRewrite(memref::GlobalOp global, OpAdaptor adaptor,
    ...
    if (!isExternal && !isUninitialized) {
      auto elementsAttr = toolchain::cast<ElementsAttr>(*global.getInitialValue());
...
struct LoadOpLowering : public LoadStoreOpLowering<memref::LoadOp> {
  ...
  matchAndRewrite(memref::LoadOp loadOp, OpAdaptor adaptor,
    ...
    // hence inbounds and nuw are used when lowering to toolchain.getelementptr.
...
// except that the toolchain.prefetch operation is used for replacement.
struct PrefetchOpLowering : public LoadStoreOpLowering<memref::PrefetchOp> {
  ...
  matchAndRewrite(memref::PrefetchOp prefetchOp, OpAdaptor adaptor,
    ...
    // Replace with toolchain.prefetch.
...
/// Pattern to lower a `memref.copy` to toolchain.
///
/// For memrefs with identity layouts, the copy is lowered to the toolchain
...
public:
  ...
  lowerToMemCopyFunctionCall(memref::CopyOp op, OpAdaptor adaptor,
    ...
    // Derive size from toolchain.getelementptr which will account for any
    // potential alignment
...
private:
  ...
  convertSourceMemRefToDescriptor(ConversionPatternRewriter &rewriter,
    ...
    if (shapeMemRefType.hasStaticShape()) {
      ...
      for (auto i : toolchain::reverse(toolchain::seq<int64_t>(0, targetRank))) {
...
public:
  ...
  matchAndRewrite(memref::TransposeOp transposeOp, OpAdaptor adaptor,
    ...
    for (const auto &en :
         toolchain::enumerate(transposeOp.getPermutation().getResults())) {
...
///   1. An `toolchain.mlir.undef` operation to create a memref descriptor
...
struct ViewOpLowering : public ConvertOpToLLVMPattern<memref::ViewOp> {
  ...
  Value getSize(ConversionPatternRewriter &rewriter, Location loc,
    ...
    unsigned nDynamic =
        toolchain::count_if(shape.take_front(idx), ShapedType::isDynamic);
...
/// lowering to toolchain.atomicrmw or fallback to toolchain.cmpxchg.
