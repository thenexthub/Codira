...
static Value createReductionNeutralValue(ReductionNeutralAllOnes neutral,
  ...
  return LLVM::ConstantOp::create(
      ...
      rewriter.getIntegerAttr(
          llvmType, toolchain::APInt::getAllOnes(llvmType.getIntOrFloatBitWidth())));
}
...
static Value createReductionNeutralValue(ReductionNeutralSIntMin neutral,
  ...
  return LLVM::ConstantOp::create(
      rewriter, loc, llvmType,
      rewriter.getIntegerAttr(llvmType, toolchain::APInt::getSignedMinValue(
...
static Value createReductionNeutralValue(ReductionNeutralUIntMin neutral,
  ...
  return LLVM::ConstantOp::create(
      rewriter, loc, llvmType,
      rewriter.getIntegerAttr(llvmType, toolchain::APInt::getMinValue(
                                            llvmType.getIntOrFloatBitWidth())));
}
...
static Value createReductionNeutralValue(ReductionNeutralSIntMax neutral,
  ...
  return LLVM::ConstantOp::create(
      rewriter, loc, llvmType,
      rewriter.getIntegerAttr(llvmType, toolchain::APInt::getSignedMaxValue(
                                            llvmType.getIntOrFloatBitWidth())));
}
...
static Value createReductionNeutralValue(ReductionNeutralUIntMax neutral,
  ...
  return LLVM::ConstantOp::create(
      rewriter, loc, llvmType,
      rewriter.getIntegerAttr(llvmType, toolchain::APInt::getMaxValue(
                                            llvmType.getIntOrFloatBitWidth())));
}
...
static Value createReductionNeutralValue(ReductionNeutralFPMin neutral,
  ...
  return LLVM::ConstantOp::create(
      ...
      rewriter.getFloatAttr(
          llvmType, toolchain::APFloat::getQNaN(floatType.getFloatSemantics(),
                                           /*Negative=*/false)));
}
...
static toolchain::APFloat
getMaskNeutralValue(MaskNeutralFMaximum,
                    const toolchain::fltSemantics &floatSemantics) {
  return toolchain::APFloat::getSmallest(floatSemantics, /*Negative=*/true);
}
/// Get the mask neutral floating point minimum value
static toolchain::APFloat
getMaskNeutralValue(MaskNeutralFMinimum,
                    const toolchain::fltSemantics &floatSemantics) {
  return toolchain::APFloat::getLargest(floatSemantics, /*Negative=*/false);
}
...
/// `fmaximum`/`fminimum`.
/// More information: https://github.com/toolchain/toolchain-project/issues/64940
...
/// Overloaded methods to lower a *predicated* reduction to an toolchain intrinsic
...
public:
  ...
  matchAndRewrite(vector::MaskOp maskOp, OpAdaptor adaptor,
    ...
    auto maskedOp = toolchain::dyn_cast_or_null<MaskedOp>(maskOp.getMaskableOp());
    if (!maskedOp)
      return failure();
  ...
  matchAndRewrite(vector::ShuffleOp shuffleOp, OpAdaptor adaptor,
    ...
    if (rank <= 1 && v1Type == v2Type) {
      Value llvmShuffleOp = LLVM::ShuffleVectorOp::create(
          ...
          toolchain::to_vector_of<int32_t>(mask));
...
public:
  ...
  matchAndRewrite(vector::ExtractOp extractOp, OpAdaptor adaptor,
    ...
    if (extractsAggregate) {
      ...
      // toolchain.extractvalue does not support dynamic dimensions.
      if (!toolchain::all_of(position, toolchain::IsaPred<Attribute>)) {
        return failure();
      }
...
/// into an toolchain.intr.fmuladd. This is a trivial 1-1 conversion.
...
/// is converted to:
/// ```
///  toolchain.intr.fmuladd %va, %va, %va:
///    (!toolchain."<8 x f32>">, !toolchain<"<8 x f32>">, !toolchain<"<8 x f32>">)
///    -> !toolchain."<8 x f32>">
/// ```
...
public:
  ...
  matchAndRewrite(vector::InsertOp insertOp, OpAdaptor adaptor,
    ...
    // aggregates (toolchain.array's) of 1D vectors.
    //
    ...
    //     the aggregate. This can be done using toolchain.extractvalue. The
    ...
    if (insertIntoInnermostDim) {
      ...
      if (isNestedAggregate) {
        ...
        if (!toolchain::all_of(positionOf1DVectorWithinAggregate,
                          toolchain::IsaPred<Attribute>)) {
          // toolchain.extractvalue does not support dynamic dimensions.
          return failure();
        }
...
public:
  ...
  matchAndRewrite(vector::TypeCastOp castOp, OpAdaptor adaptor,
    ...
    if (toolchain::any_of(*targetStrides, ShapedType::isDynamic))
      return failure();
    ...
    for (const auto &indexedSize :
         toolchain::enumerate(targetMemRefType.getShape())) {
...
private:
  ...
  LogicalResult emitScalarPrint(ConversionPatternRewriter &rewriter,
    ...
    if (printType.isF32()) {
      ...
      int32_t sem =
          toolchain::APFloatBase::SemanticsToEnum(floatTy.getFloatSemantics());
...
struct VectorInterleaveOpLowering
  ...
  matchAndRewrite(vector::InterleaveOp interleaveOp, OpAdaptor adaptor,
    ...
    // https://toolchain.org/docs/LangRef.html#id876.
...
struct VectorDeinterleaveOpLowering
  ...
  matchAndRewrite(vector::DeinterleaveOp deinterleaveOp, OpAdaptor adaptor,
    ...
    // langref still recommends fixed-vectors use shufflevector, see:
    // https://toolchain.org/docs/LangRef.html#id889.
    int64_t resultVectorSize = resultType.getNumElements();
...
struct VectorFromElementsLowering
  ...
  matchAndRewrite(vector::FromElementsOp fromElementsOp, OpAdaptor adaptor,
    ...
    for (auto [idx, val] : toolchain::enumerate(adaptor.getElements())) {
      auto constIdx =
...
struct VectorToElementsLowering
  ...
  matchAndRewrite(vector::ToElementsOp toElementsOp, OpAdaptor adaptor,
    ...
    for (auto [idx, element] : toolchain::enumerate(toElementsOp.getElements())) {
...
///    %e = add %c, %d
/// ```
/// `vector.matrix_multiply` later lowers to `toolchain.matrix.multiply`.
...
/// semantics directly into `toolchain.intr.matrix.multiply`:
/// BEFORE:
/// ```mlir
...
///   %matmul = toolchain.intr.matrix.multiply %lhs, %rhs
...
/// Lowers vector.transpose directly to toolchain.intr.matrix.transpose
///
/// BEFORE:
/// ```mlir
...
///  %tr = toolchain.intr.matrix.transpose %vec_sc
