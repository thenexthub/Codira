...
protected:
  ...
  LogicalResult visitInterfaces(
      toolchain::function_ref<void(ConvertToEmitCPatternInterface *)> visitor);
  MLIRContext *context;
...
public:
  ...
  void apply(MLIRContext *context,
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Convert to EmitC extension load\n");
    for (Dialect *dialect : dialects) {
      auto *iface = dyn_cast<ConvertToEmitCPatternInterface>(dialect);
      if (!iface)
        continue;
      LLVM_DEBUG(toolchain::dbgs() << "Convert to EmitC found dialect interface for "
