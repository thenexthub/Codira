...
static bool integerExtendSupportsMMAMatrixType(ExtOpTy extOp) {
  ...
  if (!transferReadOp)
    return false;
  return toolchain::all_of(extOp->getUsers(), toolchain::IsaPred<vector::ContractionOp>);
}
...
static SetVector<Operation *> getOpToConvert(mlir::Operation *op,
  ...
  auto hasVectorDest = [](Operation *op) {
    return toolchain::any_of(op->getResultTypes(), toolchain::IsaPred<VectorType>);
  };
  BackwardSliceOptions backwardSliceOptions;
  backwardSliceOptions.filter = hasVectorDest;

  auto hasVectorSrc = [](Operation *op) {
    return toolchain::any_of(op->getOperandTypes(), toolchain::IsaPred<VectorType>);
  };
  ...
  op->walk([&](Operation *nestedOp) {
    ...
    // by all operations.
    if (toolchain::any_of(dependentOps, [useNvGpu](Operation *op) {
...
convertTransferReadOp(RewriterBase &rewriter, vector::TransferReadOp op,
                      toolchain::DenseMap<Value, Value> &valueMapping) {
...
convertConstantOp(RewriterBase &rewriter, arith::ConstantOp op,
  ...
  gpu::MMAMatrixType type = gpu::MMAMatrixType::get(
      vecType.getShape(), vecType.getElementType(), toolchain::StringRef(fragType));
...
static scf::ForOp replaceForOpWithNewSignature(RewriterBase &rewriter,
  ...
  rewriter.setInsertionPoint(loop);
  auto operands = toolchain::to_vector<4>(loop.getInitArgs());
  toolchain::append_range(operands, newInitArgs);
  scf::ForOp newLoop =
  ...
  for (auto it : toolchain::zip(loop.getResults(), newLoop.getResults().take_front(
                                                  loop.getNumResults())))
...
static LogicalResult convertForOp(RewriterBase &rewriter, scf::ForOp op,
  ...
  for (const auto &operand : toolchain::enumerate(op.getInitArgs())) {
...
convertYieldOp(RewriterBase &rewriter, scf::YieldOp op,
  ...
  auto yieldOperands = toolchain::to_vector<4>(op.getOperands());
  for (const auto &operand : toolchain::enumerate(op.getOperands())) {
    auto it = valueMapping.find(operand.value());
    if (it == valueMapping.end())
      continue;
...
LogicalResult mlir::convertVectorToNVVMCompatibleMMASync(RewriterBase &rewriter,
  ...
  toolchain::DenseMap<Value, Value> valueMapping;
  for (Operation *op : ops) {
    if (toolchain::TypeSwitch<Operation *, LogicalResult>(op)
