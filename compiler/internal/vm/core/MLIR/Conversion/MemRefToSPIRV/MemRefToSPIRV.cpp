...
adjustAccessChainForBitwidth(const SPIRVTypeConverter &typeConverter,
  ...
  auto indices = toolchain::to_vector<4>(op.getIndices());
...
calculateMemoryRequirements(Value accessedPtr, LoadOrStoreOp loadOrStoreOp) {
  static_assert(
      toolchain::is_one_of<LoadOrStoreOp, memref::LoadOp, memref::StoreOp>::value,
...
extractLoadCoordsForComposite(memref::LoadOp loadOp, OpAdaptor adaptor,
  ...
  // slowest moving.
  return toolchain::to_vector(toolchain::reverse(coords));
}
