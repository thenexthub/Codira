...
static void inlineIfCase(Region &srcRegion, Region &dstRegion,
  ...
  for (auto it : toolchain::zip(headBlock->getArguments(), operands))
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::ScatterOp scatter,
    ...
    // Loop bounds
    auto lbs = toolchain::SmallVector<Value>(2, zero);
    auto steps = toolchain::SmallVector<Value>(2, one);
    auto ubs = toolchain::SmallVector<Value>{{dimN, dimW}};
    ...
    auto buildBody = [&](OpBuilder &builder, Location loc, ValueRange ivs,
      ...
      auto inputOffset = toolchain::to_vector(ivs);
      inputOffset.push_back(zero);

      toolchain::SmallVector<Value> sizes = {one, one, dimC};
      toolchain::SmallVector<Value> strides = {one, one, one};
      ...
      toolchain::SmallVector<Value> outputOffset = {n, castIndex, zero};
