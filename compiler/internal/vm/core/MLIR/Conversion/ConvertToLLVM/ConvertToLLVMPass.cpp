...
#define DEBUG_TYPE "convert-to-toolchain"
...
namespace {
/// Base class for creating the internal implementation of `convert-to-toolchain`
/// passes.
...
public:
  ...
  /// Get the dependent dialects used by `convert-to-toolchain`.
  static void getDependentDialects(DialectRegistry &registry);

  /// Initialize the internal state of the `convert-to-toolchain` pass
...
protected:
  ...
  LogicalResult visitInterfaces(
      toolchain::function_ref<void(ConvertToLLVMPatternInterface *)> visitor);
  MLIRContext *context;
...
/// Static implementation of the `convert-to-toolchain` pass. This version only looks
...
/// Dynamic implementation of the `convert-to-toolchain` pass. This version inspects
