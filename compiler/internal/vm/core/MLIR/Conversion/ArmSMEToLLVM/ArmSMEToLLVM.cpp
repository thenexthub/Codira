...
static memref::AllocaOp getOrCreateAllocaForTile(
  ...
  for (auto &op : func.getBlocks().front()) {
    auto alloca = toolchain::dyn_cast<memref::AllocaOp>(op);
    if (!alloca)
      continue;
    auto inMemoryTileId = toolchain::dyn_cast_or_null<IntegerAttr>(
...
struct ConvertArmSMESpillsAndFillsToLLVM : public ConvertToLLVMPattern {
  ...
  Value getInMemoryTileSlicePtr(RewriterBase &rewriter, Location loc,
    ...
    return getStridedElementPtr(
        ...
        toolchain::cast<MemRefType>(tileMemory.getType()), descriptor.getResult(0),
        {sliceIndexI64, zero});
  }
...
struct ConvertArmSMEToLLVMPass
  ...
  void runOnOperation() override {
    ...
    function->walk([&](Operation *op) {
      ...
      if (toolchain::any_of(op->getResultTypes(), isSMETileType) ||
          toolchain::any_of(op->getOperandTypes(), isSMETileType)) {
