...
computeMemrefMeta(OpType xferOp, PatternRewriter &rewriter) {
  ...
  if (memrefType.hasStaticShape()) {
    ...
    if (failed(memrefType.getStridesAndOffset(intStrides, offset)))
      return {{}, offsetVal};
    bool hasDynamicStrides = toolchain::any_of(intStrides, [](int64_t strideVal) {
  ...
  if constexpr (toolchain::is_one_of<std::decay_t<OpType>, vector::TransferReadOp,
...
static Value computeOffsets(VectorTransferOpInterface xferOp,
  ...
  toolchain::map_to_vector(vectorShape, [&](int64_t dim) {
