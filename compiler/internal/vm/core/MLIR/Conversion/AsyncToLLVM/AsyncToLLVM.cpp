...
#define DEBUG_TYPE "convert-async-to-toolchain"
...
/// async.getValue type, we use opaque pointers (!toolchain.ptr) instead. After
...
/// A function that takes a coroutine handle and calls a `toolchain.coro.resume`
...
//===----------------------------------------------------------------------===//
// Convert async.coro.id to @toolchain.coro.id intrinsic.
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(CoroIdOp op, OpAdaptor adaptor,
    ...
    // Get coroutine id: @toolchain.coro.id.
...
//===----------------------------------------------------------------------===//
// Convert async.coro.begin to @toolchain.coro.begin intrinsic.
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(CoroBeginOp op, OpAdaptor adaptor,
    ...
    // Get coroutine frame size: @toolchain.coro.size.i64.
    Value coroSize =
        LLVM::CoroSizeOp::create(rewriter, loc, rewriter.getI64Type());
    // Get coroutine frame alignment: @toolchain.coro.align.i64.
    Value coroAlign =
    ...
    // Begin a coroutine: @toolchain.coro.begin.
...
//===----------------------------------------------------------------------===//
// Convert async.coro.free to @toolchain.coro.free intrinsic.
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(CoroFreeOp op, OpAdaptor adaptor,
    ...
    // Get a pointer to the coroutine frame memory: @toolchain.coro.free.
    auto coroMem =
...
//===----------------------------------------------------------------------===//
// Convert async.coro.end to @toolchain.coro.end intrinsic.
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(CoroEndOp op, OpAdaptor adaptor,
    ...
    // Mark the end of a coroutine: @toolchain.coro.end.
...
//===----------------------------------------------------------------------===//
// Convert async.coro.save to @toolchain.coro.save intrinsic.
//===----------------------------------------------------------------------===//
...
public:
  ...
  matchAndRewrite(CoroSaveOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    // Save the coroutine state: @toolchain.coro.save
...
//===----------------------------------------------------------------------===//
// Convert async.coro.suspend to @toolchain.coro.suspend intrinsic.
//===----------------------------------------------------------------------===//

namespace {

/// Convert async.coro.suspend to the @toolchain.coro.suspend intrinsic call, and
...
public:
  ...
  matchAndRewrite(CoroSuspendOp op, OpAdaptor adaptor,
    ...
    // Suspend a coroutine: @toolchain.coro.suspend
    ...
    // or suspend block of the coroutine (see @toolchain.coro.suspend return code
    // documentation).
    toolchain::SmallVector<int32_t, 2> caseValues = {0, 1};
    toolchain::SmallVector<Block *, 2> caseDest = {op.getResumeDest(),
                                              op.getCleanupDest()};
