...
MemRefDescriptor MemRefDescriptor::fromStaticShape(
  ...
  assert(!toolchain::any_of(strides, ShapedType::isDynamic) &&
...
Value MemRefDescriptor::size(OpBuilder &builder, Location loc, Value pos,
  ...
  auto sizes = LLVM::ExtractValueOp::create(
      builder, loc, value,
      toolchain::ArrayRef<int64_t>({kSizePosInMemRefDescriptor}));
...
Value UnrankedMemRefDescriptor::computeSize(
  ...
  Value indexSize = createIndexAttrConstant(
      ...
      toolchain::divideCeil(typeConverter.getIndexTypeBitwidth(), 8));
  ...
  Value pointerSize = createIndexAttrConstant(
      builder, loc, indexType,
      toolchain::divideCeil(typeConverter.getPointerBitwidth(addressSpace), 8));
