...
SmallVector<Type> &LLVMTypeConverter::getCurrentThreadRecursiveStack() {
  {
    ...
    if (getContext().isMultithreadingEnabled())
      lock.lock();
    auto recursiveStack = conversionCallStack.find(toolchain::get_threadid());
  ...
  auto recursiveStackInserted = conversionCallStack.insert(std::make_pair(
      toolchain::get_threadid(), std::make_unique<SmallVector<Type>>()));
...
static Value unrankedMemRefMaterialization(OpBuilder &builder,
  ...
  // (!toolchain.struct) to the original memref type.
  Value packed =
...
static Value rankedMemRefMaterialization(OpBuilder &builder,
  ...
  // so insert a cast from the memref descriptor type (!toolchain.struct) to the
...
LLVMTypeConverter::LLVMTypeConverter(MLIRContext *ctx,
  ...
  addConversion([&](LLVM::LLVMStructType type, SmallVectorImpl<Type> &results)
    ...
    if (type.isIdentified()) {
      ...
      if (toolchain::count(recursiveStack, type)) {
        results.push_back(convertedType);
        return success();
      }
      recursiveStack.push_back(type);
      toolchain::scope_exit popConversionCallStack(
...
/// Returns the `toolchain.byval` or `toolchain.byref` attributes that are present in the
...
filterByValRefArgAttrs(FunctionOpInterface funcOp,
  ...
  for (int argIdx : toolchain::seq(funcOp.getNumArguments())) {
...
// If `byValRefNonPtrAttrs` is provided, converted types of `toolchain.byval` and
// `toolchain.byref` function arguments which are not LLVM pointers are overridden
// with LLVM pointers. `toolchain.byval` and `toolchain.byref` arguments that were already
...
Type LLVMTypeConverter::convertFunctionSignatureImpl(
  ...
  for (auto [idx, type] : toolchain::enumerate(funcTy.getInputs())) {
    SmallVector<Type, 8> converted;
    if (failed(funcArgConverter(*this, type, converted)))
      return {};

    // Rewrite converted type of `toolchain.byval` or `toolchain.byref` function
...
Type LLVMTypeConverter::convertFunctionSignature(
    ...
    LLVMTypeConverter::SignatureConversion &result,
    SmallVectorImpl<std::optional<NamedAttribute>> &byValRefNonPtrAttrs) const {
  // Gather all `toolchain.byval` and `toolchain.byref` function arguments. Only those
...
LLVMTypeConverter::getMemRefDescriptorSize(MemRefType type,
  ...
  return 2 * toolchain::divideCeil(getPointerBitwidth(space), 8) +
...
unsigned LLVMTypeConverter::getUnrankedMemRefDescriptorSize(
  ...
  return layout.getTypeSize(getIndexType()) +
         toolchain::divideCeil(getPointerBitwidth(space), 8);
}
...
///    `!toolchain.array<ax...array<jxvector<kxT>>>`.
...
FailureOr<Type> LLVMTypeConverter::convertVectorType(VectorType type) const {
  ...
  if (toolchain::is_contained(type.getScalableDims().drop_back(), true))
    return failure();
...
Type LLVMTypeConverter::packFunctionResults(
  ...
  for (auto t : types) {
    ...
    if (groupedTypes) {
      ...
      toolchain::append_range(group, ArrayRef(resultTypes).drop_front(sizeBefore));
    }
...
SmallVector<Value, 4> LLVMTypeConverter::promoteOperands(
  ...
  for (auto [operand, llvmOperand] :
       toolchain::zip_equal(opOperands, adaptorOperands)) {
    if (useBarePtrCallConv) {
    ...
    toolchain::append_range(promotedOperands, llvmOperand);
  }
