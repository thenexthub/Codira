...
// Iterates on the toolchain array type until we hit a non-array type (which is
// asserted to be an toolchain vector type).
...
SmallVector<int64_t, 4> LLVM::detail::getCoordinates(ArrayRef<int64_t> basis,
  ...
  for (unsigned basisElement : toolchain::reverse(basis)) {
...
LogicalResult LLVM::detail::vectorOneToOneRewrite(
  ...
  if (!toolchain::all_of(operands.getTypes(), isCompatibleType))
    return failure();
