...
#define DEBUG_TYPE "gpu-to-toolchain-spv"
...
/// Replace `gpu.barrier` with an `toolchain.call` to `barrier` using
...
/// %c3 = toolchain.mlir.constant(3: i32) : i32
/// toolchain.call spir_funccc @_Z7barrierj(%c3) : (i32) -> ()
/// ```
struct GPUBarrierConversion final : ConvertOpToLLVMPattern<gpu::BarrierOp> {
  ...
  matchAndRewrite(gpu::BarrierOp op, OpAdaptor adaptor,
    ...
    // See `toolchain/lib/Target/SPIRV/SPIRVBuiltins.td`.
...
/// Replace `gpu.*` with an `toolchain.call` to the corresponding SPIR-V builtin with
/// a constant argument for the `dimension` attribute. Return type will depend
/// on index width option:
/// ```
/// // %thread_id_y = gpu.thread_id y
/// %c1 = toolchain.mlir.constant(1: i32) : i32
/// %0 = toolchain.call spir_funccc @_Z12get_local_idj(%c1) : (i32) -> i64
/// ```
...
/// Replace `gpu.shuffle` with an `toolchain.call` to the corresponding SPIR-V
...
/// %0 = toolchain.call spir_funccc @_Z17sub_group_shuffledj(%value, %offset)
///     : (f64, i32) -> f64
/// ```
struct GPUShuffleConversion final : ConvertOpToLLVMPattern<gpu::ShuffleOp> {
  ...
  static std::optional<std::string> getFuncName(gpu::ShuffleMode mode,
    ...
    if (!typeMangling)
      return std::nullopt;
    return toolchain::formatv("_Z{}{}{}", baseName.size(), baseName,
                         typeMangling.value());
  }
