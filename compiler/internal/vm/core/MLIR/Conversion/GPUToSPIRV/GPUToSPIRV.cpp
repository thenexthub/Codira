...
lowerAsEntryFunction(gpu::GPUFuncOp funcOp, const TypeConverter &typeConverter,
  ...
  for (auto argIndex : toolchain::seq<unsigned>(0, argABIInfo.size())) {
...
getDefaultABIAttrs(const spirv::TargetEnv &targetEnv, gpu::GPUFuncOp funcOp,
  ...
  for (auto argIndex : toolchain::seq<unsigned>(0, funcOp.getNumArguments())) {
...
createGroupReduceOp(OpBuilder &builder, Location loc, Value arg,
  ...
  // TODO(https://github.com/toolchain/toolchain-project/issues/73459): The SPIR-V spec
...
static std::string makeVarName(spirv::ModuleOp moduleOp, toolchain::Twine prefix) {
  std::string name;
  unsigned number = 0;

  do {
    name.clear();
    name = (prefix + toolchain::Twine(number++)).str();
...
LogicalResult GPUPrintfConversion::matchAndRewrite(
  ...
  std::string globalVarName = makeVarName(moduleOp, toolchain::Twine("printfMsg"));
  ...
  auto createSpecConstant = [&](unsigned value) {
    ...
    std::string specCstName =
        makeVarName(moduleOp, toolchain::Twine(globalVarName) + "_sc");
  ...
  };
  {
    ...
    toolchain::SmallString<20> formatString(adaptor.getFormat());
    ...
    std::string specCstCompositeName =
        (toolchain::Twine(globalVarName) + "_scc").str();
  ...
  auto printfArgs = toolchain::to_vector_of<Value, 4>(adaptor.getArgs());
