...
static unsigned getNumNonRangeValues(ValueRange values) {
  return toolchain::count_if(values.getTypes(),
...
getTreePredicates(std::vector<PositionalPredicate> &predList, Value val,
  ...
  for (auto [attrName, attr] :
       toolchain::zip(op.getAttributeValueNames(), op.getAttributeValues())) {
    getTreePredicates(
  ...
  if (operands.size() == 1 && isa<pdl::RangeType>(operands[0].getType())) {
    ...
    for (const auto &operandIt : toolchain::enumerate(operands)) {
  ...
  bool foundVariableLength = false;
  for (auto [idx, typeValue] : toolchain::enumerate(types)) {
...
static void getConstraintPredicates(pdl::ApplyNativeConstraintOp op,
  ...
  Position *pos = *toolchain::max_element(allPositions, comparePosDepth);
  ...
  for (auto [i, result] : toolchain::enumerate(results)) {
...
static void buildCostGraph(ArrayRef<Value> roots, RootOrderingGraph &graph,
  ...
  toolchain::MapVector<Value, SmallVector<RootDepth, 1>> connectorsRootsDepths;
    ...
    while (!toVisit.empty()) {
      ...
      TypeSwitch<Operation *>(entry.value.getDefiningOp())
          .Case<pdl::OperationOp>([&](auto operationOp) {
            ...
            for (const auto &p :
                 toolchain::enumerate(operationOp.getOperandValues()))
  ...
  assert((toolchain::hasSingleElement(roots) || graph.size() == roots.size()) &&
...
static Value buildPredicateList(pdl::PatternOp pattern,
  ...
  for (const auto &it : toolchain::enumerate(bestEdges)) {
...
struct OrderedPredicateDenseInfo {
  ...
  static unsigned getHashValue(const OrderedPredicate &p) {
    return toolchain::hash_combine(p.position, p.question);
  }
};
...
static void stableTopologicalSort(Iterator begin, Iterator end, Compare cmp) {
  while (begin != end) {
    ...
    // while it runs.
    toolchain::SmallPtrSet<typename Iterator::value_type, 16> sortBeforeOthers;
...
static bool dependsOn(OrderedPredicate *a, OrderedPredicate *b) {
  ...
  if (auto *cqb = dyn_cast<ConstraintQuestion>(b->question)) {
    ...
    return toolchain::any_of(cqb->getArgs(), positionDependsOnA);
  }
...
MatcherNode::generateMatcherTree(ModuleOp module, PredicateBuilder &builder,
  ...
  toolchain::sort(ordered, [](OrderedPredicate *lhs, OrderedPredicate *rhs) {
    return *lhs < *rhs;
  });
