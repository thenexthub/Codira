...
private:
  using ValueMap = toolchain::ScopedHashTable<Position *, Value>;
  using ValueMapScope = toolchain::ScopedHashTableScope<Position *, Value>;
  ...
  case Predicates::TypeQuestion: {
    ...
    if (isa<pdl::RangeType>(val.getType()))
      pdl_interp::CheckTypesOp::create(builder, loc, val,
                                       toolchain::cast<ArrayAttr>(ans->getValue()),
                                       success, failure);
    else
      pdl_interp::CheckTypeOp::create(builder, loc, val,
                                      toolchain::cast<TypeAttr>(ans->getValue()),
                                      success, failure);
    break;
  }
...
static void createSwitchOp(Value val, Block *defaultDest, OpBuilder &builder,
                           toolchain::MapVector<Qualifier *, Block *> &dests) {
...
void PatternLowering::generate(SwitchNode *switchNode, Block *currentBlock,
  ...
  if (kind == Predicates::OperandCountAtLeastQuestion ||
    ...
    SmallVector<unsigned> sortedChildren = toolchain::to_vector<16>(
        toolchain::seq<unsigned>(0, switchNode->getChildren().size()));
    toolchain::sort(sortedChildren, [&](unsigned lhs, unsigned rhs) {
  ...
  // switch.
  toolchain::MapVector<Qualifier *, Block *> children;
...
SymbolRefAttr PatternLowering::generateRewriter(
  ...
  if (StringAttr rewriteName = rewriter.getNameAttr()) {
    ...
    auto mappedArgs =
        toolchain::map_range(rewriter.getExternalArgs(), mapRewriteValue);
    ...
    for (Operation &rewriteOp : *rewriter.getBody()) {
      toolchain::TypeSwitch<Operation *>(&rewriteOp)
  ...
  rewriterFunc.setType(builder.getFunctionType(
      toolchain::to_vector<8>(rewriterFunc.front().getArgumentTypes()),
      /*results=*/{}));
...
void PatternLowering::generateRewriter(
  ...
  for (auto it : toolchain::zip(rewriteOp.getResults(), interpOp.getResults()))
...
void PatternLowering::generateRewriter(
  ...
  for (const auto &it : toolchain::enumerate(resultTys)) {
...
void PatternLowering::generateOperationResultTypeRewriter(
  ...
  auto tryResolveResultTypes = [&] {
    ...
    for (const auto &it : toolchain::enumerate(resultTypeValues)) {
...
void PDLToPDLInterpPass::runOnOperation() {
  ...
  for (pdl::PatternOp pattern :
       toolchain::make_early_inc_range(module.getOps<pdl::PatternOp>())) {
