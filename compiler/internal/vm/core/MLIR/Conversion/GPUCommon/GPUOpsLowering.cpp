...
LLVM::GlobalOp mlir::getOrCreateStringConstant(OpBuilder &b, Location loc,
                                               ...
                                               uint64_t alignment,
                                               unsigned addrSpace) {
  toolchain::SmallString<20> nullTermStr(str);
...
GPUFuncOpLowering::matchAndRewrite(gpu::GPUFuncOp gpuFuncOp, OpAdaptor adaptor,
  ...
  if (encodeWorkgroupAttributionsAsArguments) {
    // Append an `toolchain.ptr` argument to the function signature to encode
    ...
    // New arguments will simply be `toolchain.ptr` with the correct address space
    ...
    // Attributes: noalias, toolchain.mlir.workgroup_attribution(<size>, <type>)
    std::array attrs{
    ...
    for (auto [idx, attribution] :
         toolchain::enumerate(gpuFuncOp.getWorkgroupAttributions())) {
      ...
      std::string name =
          std::string(toolchain::formatv("__wg_{0}_{1}", gpuFuncOp.getName(), idx));
...

  {
    ...
    if (encodeWorkgroupAttributionsAsArguments) {
      ...
      ArrayRef<BlockArgument> attributionArguments =
          gpuFuncOp.getArguments().slice(numProperArguments - numAttributions,
                                         numAttributions);
      for (auto [idx, vals] : toolchain::enumerate(toolchain::zip_equal(
        ...
        // Arguments are of toolchain.ptr type and attributions are of memref type:
      ...
      for (const auto [idx, global] : toolchain::enumerate(workgroupBuffers)) {
    ...
    for (const auto [idx, attribution] :
         toolchain::enumerate(gpuFuncOp.getPrivateAttributions())) {
      auto type = cast<MemRefType>(attribution.getType());
      assert(type && type.hasStaticShape() && "unexpected type in attribution");
  ...
  for (const auto [idx, argTy] :
       toolchain::enumerate(gpuFuncOp.getArgumentTypes())) {
...
LogicalResult GPUPrintfOpToVPrintfLowering::matchAndRewrite(
  ...
  Value tempAlloc =
      LLVM::AllocaOp::create(rewriter, loc, ptrType, structType, one,
                             /*alignment=*/0);
  for (auto [index, arg] : toolchain::enumerate(args)) {
...
static Value scalarizeVectorOpHelper(Operation *op, ValueRange operands,
  ...
  for (int64_t i = 0; i < vectorType.getNumElements(); ++i) {
    ...
    auto scalarOperands = toolchain::map_to_vector(operands, extractElement);
    Operation *scalarOp =
...
LogicalResult impl::scalarizeVectorOp(Operation *op, ValueRange operands,
                                      ...
                                      const LLVMTypeConverter &converter) {
  TypeRange operandTypes(operands);
  if (toolchain::any_of(operandTypes, toolchain::IsaPred<VectorType>)) {
  ...
  if (toolchain::any_of(operandTypes, toolchain::IsaPred<LLVM::LLVMArrayType>)) {
  ...
  return rewriter.notifyMatchFailure(op, "no toolchain.array or vector to unroll");
}
...
static LLVM::GlobalOp getDynamicSharedMemorySymbol(
  ...
  toolchain::StringSet<> existingGlobalNames;
...
LogicalResult GPUReturnOpLowering::matchAndRewrite(
  ...
  if (useBarePtrCallConv) {
    ...
    for (auto it : toolchain::zip(op->getOperands(), adaptor.getOperands())) {
    ...
    updatedOperands = toolchain::to_vector<4>(adaptor.getOperands());
