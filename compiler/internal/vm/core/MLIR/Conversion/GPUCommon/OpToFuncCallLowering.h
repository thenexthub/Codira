...
///   toolchain.call @__nv_expf(%arg_f32) : (f32) -> f32
///
...
///
/// will be transformed into
///   toolchain.call @__nv_fast_expf(%arg_f32) : (f32) -> f32
///
/// Final example with NVVM:
///   %pow_f32 = math.fpowi %arg_f32, %arg_i32
///
/// will be transformed into
///   toolchain.call @__nv_powif(%arg_f32, %arg_i32) : (f32, i32) -> f32
...
public:
  ...
  StringRef getFunctionName(Type type, SourceOp op) const {
    ...
    if constexpr (toolchain::is_detected<has_get_fastmath_t, SourceOp>::value) {
