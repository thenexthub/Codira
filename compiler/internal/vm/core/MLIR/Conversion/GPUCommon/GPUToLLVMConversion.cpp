...
#define DEBUG_TYPE "gpu-to-toolchain"
...
// dialect to toolchain calls, the cusparselt calls are disabled for cuda
// architecture <8.0
static bool is2To4Sparsity(Value spMat) {
  ...
  spMat.getDefiningOp()->print(toolchain::errs());
...
static LogicalResult areAllLLVMTypes(Operation *op, ValueRange operands,
  ...
  if (!toolchain::all_of(operands, [](Value value) {
...
LogicalResult ConvertAsyncYieldToGpuRuntimeCallPattern::matchAndRewrite(
    ...
    ConversionPatternRewriter &rewriter) const {
  if (toolchain::none_of(yieldOp.getOperands(), isGpuAsyncTokenType))
  ...
  toolchain::SmallDenseSet<Value> streams;
...
LogicalResult ConvertWaitAsyncOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  for (auto pair :
       toolchain::zip(waitOp.getAsyncDependencies(), adaptor.getOperands())) {
...
LogicalResult ConvertCreateCooOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  Type iType =
      toolchain::cast<MemRefType>(op.getColIdxs().getType()).getElementType();
  Type dType =
      toolchain::cast<MemRefType>(op.getValues().getType()).getElementType();
...
LogicalResult ConvertCreateCooAoSOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  Value pValues =
      MemRefDescriptor(adaptor.getValues()).allocatedPtr(rewriter, loc);
  Type iType = toolchain::cast<MemRefType>(op.getIdxs().getType()).getElementType();
  Type dType =
      toolchain::cast<MemRefType>(op.getValues().getType()).getElementType();
  auto itp = genConstInt32From(rewriter, loc, getCuSparseIndexTypeFrom(iType));
  auto dtp = genConstInt32From(rewriter, loc, getCuSparseDataTypeFrom(dType));
...
LogicalResult ConvertCreateCsrOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  Value pValues =
      MemRefDescriptor(adaptor.getValues()).allocatedPtr(rewriter, loc);
  Type pType =
      toolchain::cast<MemRefType>(op.getRowPos().getType()).getElementType();
  Type iType =
      toolchain::cast<MemRefType>(op.getColIdxs().getType()).getElementType();
  Type dType =
      toolchain::cast<MemRefType>(op.getValues().getType()).getElementType();
...
LogicalResult ConvertCreate2To4SpMatOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  Value pMat =
      MemRefDescriptor(adaptor.getMemref()).allocatedPtr(rewriter, loc);
  Type dType =
      toolchain::cast<MemRefType>(op.getMemref().getType()).getElementType();
  auto dtp = genConstInt32From(rewriter, loc, getCuSparseDataTypeFrom(dType));
...
LogicalResult ConvertCreateCscOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  Value pValues =
      MemRefDescriptor(adaptor.getValues()).allocatedPtr(rewriter, loc);
  Type pType =
      toolchain::cast<MemRefType>(op.getColPos().getType()).getElementType();
  Type iType =
      toolchain::cast<MemRefType>(op.getRowIdxs().getType()).getElementType();
  Type dType =
      toolchain::cast<MemRefType>(op.getValues().getType()).getElementType();
  auto ptp = genConstInt32From(rewriter, loc, getCuSparseIndexTypeFrom(pType));
  auto itp = genConstInt32From(rewriter, loc, getCuSparseIndexTypeFrom(iType));
...
LogicalResult ConvertCreateBsrOpToGpuRuntimeCallPattern::matchAndRewrite(
  ...
  Value pValues =
      MemRefDescriptor(adaptor.getValues()).allocatedPtr(rewriter, loc);
  Type pType =
      toolchain::cast<MemRefType>(op.getBRowPos().getType()).getElementType();
  Type iType =
      toolchain::cast<MemRefType>(op.getBColIdxs().getType()).getElementType();
  Type dType =
      toolchain::cast<MemRefType>(op.getValues().getType()).getElementType();
  auto ptp = genConstInt32From(rewriter, loc, getCuSparseIndexTypeFrom(pType));
  auto itp = genConstInt32From(rewriter, loc, getCuSparseIndexTypeFrom(iType));
