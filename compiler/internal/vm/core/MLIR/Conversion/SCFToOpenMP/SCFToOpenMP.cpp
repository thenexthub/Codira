...
static bool matchSimpleReduction(Block &block) {
  if (block.empty() || toolchain::hasSingleElement(block) ||
...
matchSelectReduction(Block &block, ArrayRef<Predicate> lessThanPredicates,
  ...
  static_assert(
      toolchain::is_one_of<SelectOpTy, arith::SelectOp, LLVM::SelectOp>::value,
      "only arithmetic and toolchain select ops are supported");
  ...
  bool isLess;
  if (toolchain::is_contained(lessThanPredicates, compare.getPredicate())) {
    isLess = true;
  } else if (toolchain::is_contained(greaterThanPredicates,
...
static const toolchain::fltSemantics &fltSemanticsForType(FloatType type) {
  if (type.isF16())
...
static Attribute minMaxValueForFloat(Type type, bool min) {
  ...
  auto val = toolchain::APFloat::getLargest(fltSemanticsForType(fltType), min);
...
static Attribute minMaxValueForSignedInt(Type type, bool min) {
  ...
  auto val = min ? toolchain::APInt::getSignedMinValue(bitwidth)
                 : toolchain::APInt::getSignedMaxValue(bitwidth);
...
static Attribute minMaxValueForUnsignedInt(Type type, bool min) {
  ...
  unsigned bitwidth = intType.getWidth();
  auto val =
      min ? toolchain::APInt::getZero(bitwidth) : toolchain::APInt::getAllOnes(bitwidth);
...
/// using toolchain.atomicrmw of the given kind.
...
/// Returns true if the type is supported by toolchain.atomicrmw.
...
static omp::DeclareReductionOp declareReduction(PatternRewriter &builder,
  ...
  assert(toolchain::hasSingleElement(reduce.getReductions()[reductionIndex]) &&
  ...
  if (matchSimpleReduction<arith::AndIOp, LLVM::AndOp>(reduction)) {
    APInt allOnes = toolchain::APInt::getAllOnes(elType.getIntOrFloatBitWidth());
    omp::DeclareReductionOp decl = createDecl(
        builder, symbolTable, reduce, reductionIndex,
...
struct ParallelOpLowering : public OpRewritePattern<scf::ParallelOp> {
  ...
  LogicalResult matchAndRewrite(scf::ParallelOp parallelOp,
    ...
    // reduction variables.
    for (auto [x, y, rD] : toolchain::zip_equal(
    ...
    auto ompParallel = omp::ParallelOp::create(
        rewriter, loc,
        /* allocate_vars = */ toolchain::SmallVector<Value>{},
        /* allocator_vars = */ toolchain::SmallVector<Value>{},
        /* if_expr = */ Value{},
        ...
        /* reduction_vars = */ toolchain::SmallVector<Value>{},
        ...
        /* reduction_syms = */ ArrayAttr{});
    {
      ...
      // Replace the loop.
      {
        ...
        if (!reductionVariables.empty()) {
          ...
          toolchain::SmallVector<bool> reductionByRef;
        ...
        // variables.
        toolchain::SmallVector<mlir::Type> reductionTypes;
        reductionTypes.reserve(reductionVariables.size());
        toolchain::transform(reductionVariables, std::back_inserter(reductionTypes),
                        [](mlir::Value v) { return v.getType(); });
        rewriter.createBlock(
            &wsloopOp.getRegion(), {}, reductionTypes,
            toolchain::SmallVector<mlir::Location>(reductionVariables.size(),
                                              parallelOp.getLoc()));
    ...
    for (auto [variable, type] :
         toolchain::zip(reductionVariables, parallelOp.getResultTypes())) {
