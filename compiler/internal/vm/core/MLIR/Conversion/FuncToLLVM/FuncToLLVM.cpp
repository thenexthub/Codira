...
#define PASS_NAME "convert-func-to-toolchain"

static constexpr StringRef varargsAttrName = "func.varargs";
static constexpr StringRef linkageAttrName = "toolchain.linkage";
static constexpr StringRef barePtrAttrName = "toolchain.bareptr";
...
static void wrapForExternalCallers(OpBuilder &rewriter, Location loc,
  ...
  auto wrapperFuncOp = LLVM::LLVMFuncOp::create(
      rewriter, loc, toolchain::formatv("_mlir_ciface_{0}", funcOp.getName()).str(),
  ...
  for (auto [index, argType] : toolchain::enumerate(type.getInputs())) {
...
static void wrapExternalFunction(OpBuilder &builder, Location loc,
  ...
  auto wrapperFunc = LLVM::LLVMFuncOp::create(
      builder, loc, toolchain::formatv("_mlir_ciface_{0}", funcOp.getName()).str(),
...
/// Inserts `toolchain.load` ops in the function body to restore the expected pointee
/// value from `toolchain.byval`/`toolchain.byref` function arguments that were converted
...
static void restoreByValRefArgumentType(
  ...
  for (const auto &[arg, byValRefAttr] :
       toolchain::zip(funcOp.getArguments(), byValRefNonPtrAttrs)) {
    // Skip argument if no `toolchain.byval` or `toolchain.byref` attribute.
    if (!byValRefAttr)
      continue;
    ...
    assert(isa<LLVM::LLVMPointerType>(arg.getType()) &&
           ...
           "`toolchain.byval`/`toolchain.byref` attribute");
...
FailureOr<LLVM::LLVMFuncOp> mlir::convertFuncOpToLLVMFuncOp(
  ...
  // Gather `toolchain.byval` and `toolchain.byref` arguments whose type convertion was
  ...
  // Fix the type mismatch between the materialized `toolchain.ptr` and the expected
  // pointee type in the function body when converting `toolchain.byval`/`toolchain.byref`
  // function arguments.
...
struct CallOpInterfaceLowering : public ConvertOpToLLVMPattern<CallOpType> {
  ...
  LogicalResult matchAndRewriteImpl(CallOpType callOp, Adaptor adaptor,
    ...
    auto resultTypes = toolchain::to_vector<4>(callOp.getResultTypes());
    ...
    // of the new call op. toolchain.call ops support only 0 or 1 result. In case of
...
struct ReturnOpLowering : public ConvertOpToLLVMPattern<func::ReturnOp> {
  ...
  matchAndRewrite(func::ReturnOp op, OneToNOpAdaptor adaptor,
    ...
    for (auto [oldOperand, newOperands] :
         toolchain::zip_equal(op->getOperands(), adaptor.getOperands())) {
      ...
      toolchain::append_range(updatedOperands, newOperands);
    }
    ...
    for (auto [idx, operand] : toolchain::enumerate(updatedOperands)) {
...
struct ConvertFuncToLLVMPass
  ...
  void runOnOperation() override {
    ...
    options.dataLayout = toolchain::DataLayout(dataLayout);
