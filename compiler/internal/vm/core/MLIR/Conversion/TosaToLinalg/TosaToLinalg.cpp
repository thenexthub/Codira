...
static Value createLinalgBodyCalculationForElementwiseOp(
  ...
  // tosa::ErfOp
  if (isa<tosa::ErfOp>(op) && toolchain::isa<FloatType>(elementTy))
  ...
  if (isa<tosa::MaximumOp>(op) && isa<FloatType>(elementTy)) {
    ...
    return materializeBinaryNanCheckIfRequired(toolchain::cast<tosa::MaximumOp>(op),
  ...
  if (isa<tosa::MinimumOp>(op) && isa<FloatType>(elementTy)) {
    ...
    return materializeBinaryNanCheckIfRequired(toolchain::cast<tosa::MinimumOp>(op),
  ...
  if (isa<tosa::ClampOp>(op) && isa<FloatType>(elementTy)) {
    ...
    auto clampOp = toolchain::cast<tosa::ClampOp>(op);
...
static bool operandsAndResultsRanked(Operation *operation) {
  ...
  return toolchain::all_of(operation->getOperands(), isRanked) &&
         toolchain::all_of(operation->getResults(), isRanked);
}
...
computeTargetSize(PatternRewriter &rewriter, Location loc, IndexPool &indexPool,
  ...
  auto operandsWithDynamicDim =
      toolchain::filter_to_vector(operands, [&](Value operand) {
...
static Value broadcastDynamicDimension(PatternRewriter &rewriter, Location loc,
  ...
  for (auto index : toolchain::seq<int64_t>(0, rank)) {
...
broadcastDynamicDimensions(PatternRewriter &rewriter, Location loc,
  ...
  return toolchain::map_to_vector(operands, [&](Value operand) {
...
emitElementwiseComputation(ConversionPatternRewriter &rewriter, Location loc,
  ...
  auto affineMaps = toolchain::map_to_vector(operands, [&](Value operand) {
    auto shape = cast<ShapedType>(operand.getType()).getShape();
    SmallVector<AffineExpr> affineExprs;
    for (auto it : toolchain::enumerate(shape)) {
...
static toolchain::SmallVector<int8_t>
convertToI8(const toolchain::SmallVector<int32_t> &input) {
  toolchain::SmallVector<int8_t> output;
  output.reserve(input.size());

  for (auto v : toolchain::map_range(
...
static void setupLinalgGenericOpInputAndIndexingMap(
    PatternRewriter &rewriter, toolchain::SmallVector<int32_t> &values,
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::RescaleOp op,
    ...
    toolchain::SmallVector<int32_t> shiftValues;
    toolchain::SmallVector<int32_t> multiplierValues;
    bool doubleRound;

    if (isMultiplierConstant && isShiftConstant) {
      // explicit cast is required here
      shiftValues = toolchain::to_vector(toolchain::map_range(
          shiftElems.getValues<IntegerAttr>(), [](IntegerAttr attr) -> int32_t {
            return static_cast<int32_t>(attr.getInt());
          }));
      multiplierValues = toolchain::to_vector(
          toolchain::map_range(multiplierElems.getValues<IntegerAttr>(),
      ...
      doubleRound = op.getRoundingMode() == RoundingMode::DOUBLE_ROUND &&
                    toolchain::any_of(shiftValues, [](int32_t v) { return v > 31; });
    } else
      doubleRound = op.getRoundingMode() == RoundingMode::DOUBLE_ROUND;
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::ResizeOp op,
    ...
    toolchain::SmallVector<Value> outputDynSize;
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::ResizeOp op,
    ...
    // on the output.
    toolchain::SmallVector<int64_t> resizeShape;
    ...
    toolchain::SmallVector<int64_t> collapseShape = {batch};
    if (inputH != 1)
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::ReverseOp op,
    ...
    rewriter.replaceOpWithNewOp<linalg::GenericOp>(
        ...
        getNParallelLoopsAttrs(resultTy.getRank()),
        [&](OpBuilder &nestedBuilder, Location nestedLoc, ValueRange args) {
          toolchain::SmallVector<Value> indices;
...
public:
  ...
  static toolchain::SmallVector<Value> inferDynamicDimsForGather(OpBuilder &builder,
                                                            Location loc,
                                                            Value values,
                                                            Value indices) {
    toolchain::SmallVector<Value> results;

    auto addDynamicDimension = [&](Value source, int64_t dim) {
      auto sz = tensor::getMixedSize(builder, loc, source, dim);
      if (auto dimValue = toolchain::dyn_cast_if_present<Value>(sz))
...
struct RFFT2dConverter final : public OpRewritePattern<RFFT2dOp> {
  ...
  computeOutputShape(OpBuilder &builder, Location loc, Value input,
                     toolchain::SmallVectorImpl<Value> &dynamicSizes) {
    // Get [N, H, W]
    ...
    toolchain::SmallVector<int64_t, 3> staticSizes;
  ...
  static Value createZeroTensor(PatternRewriter &rewriter, Location loc,
                                ...
                                toolchain::ArrayRef<Value> dynamicSizes) {
    auto emptyTensor =
  ...
  static toolchain::SmallVector<AffineExpr, 4> affineDimsExpr(OpBuilder &builder,
                                                         Args... args) {
  ...
  LogicalResult matchAndRewrite(RFFT2dOp rfft2d,
    ...
    if (!toolchain::all_of(rfft2d->getOperandTypes(), isRankedTensor) ||
        !toolchain::all_of(rfft2d->getResultTypes(), isRankedTensor)) {
    ...
    auto indexingMaps = AffineMap::inferFromExprList(
        toolchain::ArrayRef{affineDimsExpr(rewriter, 0, 3, 4),
...
struct FFT2dConverter final : OpRewritePattern<FFT2dOp> {
  ...
  LogicalResult matchAndRewrite(FFT2dOp fft2d,
                                PatternRewriter &rewriter) const override {
    if (!toolchain::all_of(fft2d->getOperandTypes(),
                      RFFT2dConverter::isRankedTensor) ||
        !toolchain::all_of(fft2d->getResultTypes(),
                      RFFT2dConverter::isRankedTensor)) {
