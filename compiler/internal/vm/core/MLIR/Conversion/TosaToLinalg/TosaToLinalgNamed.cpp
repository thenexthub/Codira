...
static mlir::Value applyPad(Location loc, Value input, ArrayRef<int64_t> pad,
  ...
  if (toolchain::all_of(pad, [](int64_t p) { return p == 0; }))
    return input;
  ...
  for (size_t i : toolchain::seq(inputShape.size())) {
    auto lowPad = pad[i * 2];
...
static AffineMap getBroadcastingMap(PatternRewriter &rewriter, Value source,
  ...
  if (sourceRank == 1 && sourceTy.getDimSize(0) == 1) {
    ...
    for (auto dim : toolchain::seq<int64_t>(0, sourceRank)) {
...
public:
  ...
  matchAndRewrite(TosaConvOp op, typename TosaConvOp::Adaptor adaptor,
    ...
    toolchain::SmallVector<int64_t> inputSizeDims;
    toolchain::SmallVector<int64_t> kernelSizeDims;
    ...
    toolchain::SmallVector<int64_t> pad;
    pad.resize(2, 0);
    toolchain::append_range(pad, padAttr.asArrayRef());
      ...
      Value result =
          linalg::GenericOp::create(
              ...
              [&](OpBuilder &nestedBuilder, Location nestedLoc,
                  ValueRange args) {
                Value added;
                if (toolchain::isa<FloatType>(inputETy))
...
public:
  ...
  matchAndRewrite(tosa::MaxPool2dOp op, OpAdaptor adaptor,
    ...
    toolchain::SmallVector<int64_t> pad;
    pad.resize(2, 0);
    toolchain::append_range(pad, op.getPad());
    pad.resize(pad.size() + 2, 0);
...
public:
  ...
  LogicalResult matchAndRewrite(tosa::TransposeOp op,
                                PatternRewriter &rewriter) const final {
    const toolchain::ArrayRef<int32_t> constantPerms = op.getPerms();
