    ...
    for (auto result : ifOp.getResults()) {
      ...
      if (!convertedType)
        return rewriter.notifyMatchFailure(
            loc,
            toolchain::formatv("failed to convert type '{0}'", result.getType()));
...
public:
  ...
  matchAndRewrite(scf::YieldOp terminatorOp, OpAdaptor adaptor,
    ...
    if (parent->getDialect()->getNamespace() ==
      ...
      return rewriter.notifyMatchFailure(
          terminatorOp,
          toolchain::formatv("conversion not supported for parent op: '{0}'",
                        parent->getName()));
...
struct WhileOpConversion final : SCFToSPIRVPattern<scf::WhileOp> {
  ...
  matchAndRewrite(scf::WhileOp whileOp, OpAdaptor adaptor,
    ...
    // that.
    for (const auto &it : toolchain::enumerate(condArgs)) {
      auto res = it.value();
      auto i = it.index();
      auto pointerType =
