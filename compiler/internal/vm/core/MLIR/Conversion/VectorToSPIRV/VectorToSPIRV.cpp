  ...
  matchAndRewrite(vector::BitCastOp bitcastOp, OpAdaptor adaptor,
    ...
    if (getNumBits(dstType) != getNumBits(srcType)) {
      return rewriter.notifyMatchFailure(
          bitcastOp,
          toolchain::formatv("different source ({0}) and target ({1}) bitwidth",
                        srcType, dstType));
    }
...
static Value sanitizeDynamicIndex(ConversionPatternRewriter &rewriter,
  ...
  if (toolchain::isPowerOf2_32(vectorSize)) {
...
struct VectorReductionPattern final : OpConversionPattern<vector::ReductionOp> {
  ...
  matchAndRewrite(vector::ReductionOp reduceOp, OpAdaptor adaptor,
    ...
    for (Value next : toolchain::drop_begin(extractedElements)) {
...
struct VectorShuffleOpConvert final
  ...
  matchAndRewrite(vector::ShuffleOp shuffleOp, OpAdaptor adaptor,
    ...
    auto mask = toolchain::to_vector_of<int32_t>(shuffleOp.getMask());
    ...
    for (auto [shuffleIdx, newOperand] : toolchain::zip_equal(mask, newOperands)) {
...
struct VectorInterleaveOpConvert final
  ...
  matchAndRewrite(vector::InterleaveOp interleaveOp, OpAdaptor adaptor,
    ...
    auto seq = toolchain::seq<int64_t>(2 * n);
    auto indices = toolchain::map_to_vector(
...
struct VectorDeinterleaveOpConvert final
  ...
  matchAndRewrite(vector::DeinterleaveOp deinterleaveOp, OpAdaptor adaptor,
    ...
    auto seqEven = toolchain::seq<int64_t>(n / 2);
    auto indicesEven =
        toolchain::map_to_vector(seqEven, [](int i) { return i * 2; });

    // Indices for `shuffleOdd` (result 1).
    auto seqOdd = toolchain::seq<int64_t>(n / 2);
    auto indicesOdd =
        toolchain::map_to_vector(seqOdd, [](int i) { return i * 2 + 1; });
...
struct VectorToElementOpConvert final
  ...
  matchAndRewrite(vector::ToElementsOp toElementsOp, OpAdaptor adaptor,
    ...
    if (!elementType)
      return rewriter.notifyMatchFailure(
          toElementsOp,
          toolchain::formatv("failed to convert element type '{0}' to SPIR-V",
                        srcElementType));
