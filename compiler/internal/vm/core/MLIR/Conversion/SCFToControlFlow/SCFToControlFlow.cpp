...
static void propagateLoopAttrs(Operation *scfOp, Operation *brOp) {
  ...
  // toolchain.loop_annotation attribute.
  ...
  toolchain::copy_if(scfOp->getAttrs(), std::back_inserter(llvmAttrs),
                [](auto attr) {
...
LogicalResult ForLowering::matchAndRewrite(ForOp forOp,
  ...
  toolchain::append_range(destOperands, forOp.getInitArgs());
...
ParallelLowering::matchAndRewrite(ParallelOp parallelOp,
  ...
  SmallVector<Value, 4> iterArgs = toolchain::to_vector<4>(parallelOp.getInitVals());
  SmallVector<Value, 4> ivs;
  ivs.reserve(parallelOp.getNumLoops());
  bool first = true;
  SmallVector<Value, 4> loopResults(iterArgs);
  for (auto [iv, lower, upper, step] :
       toolchain::zip(parallelOp.getInductionVars(), parallelOp.getLowerBound(),
...
LogicalResult WhileLowering::matchAndRewrite(WhileOp whileOp,
  ...
  SmallVector<Value> args = toolchain::to_vector(condOp.getArgs());
...
DoWhileLowering::matchAndRewrite(WhileOp whileOp,
  ...
  if (!toolchain::hasSingleElement(afterBlock))
...
IndexSwitchLowering::matchAndRewrite(IndexSwitchOp op,
  ...
  for (auto [region, value] : toolchain::zip(op.getCaseRegions(), op.getCases())) {
