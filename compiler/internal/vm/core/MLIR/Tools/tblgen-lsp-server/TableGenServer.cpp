...
using toolchain::RecordVal;
using toolchain::SourceMgr;
...
getLocationFromLoc(SourceMgr &mgr, SMRange loc,
  ...
  return toolchain::lsp::Location(getURIFromLoc(mgr, loc.Start, uri),
                             toolchain::lsp::Range(mgr, loc));
}
static toolchain::lsp::Location
...
getLspDiagnoticFromDiag(const toolchain::SMDiagnostic &diag,
                        const toolchain::lsp::URIForFile &uri) {
  ...
  toolchain::lsp::Location loc = getLocationFromLoc(*sourceMgr, diag.getLoc(), uri);
...
private:
  ...
  using MapT = toolchain::IntervalMap<
      const char *, const TableGenIndexSymbol *,
      toolchain::IntervalMapImpl::NodeSizer<const char *,
...
void TableGenIndex::initialize(const RecordKeeper &records) {
  ...
  auto classes =
      toolchain::make_pointee_range(toolchain::make_second_range(records.getClasses()));
  auto defs =
      toolchain::make_pointee_range(toolchain::make_second_range(records.getDefs()));
  for (const Record &def : toolchain::concat<Record>(classes, defs)) {
...
public:
  TableGenTextFile(const toolchain::lsp::URIForFile &uri, StringRef fileContents,
                   int64_t version,
  ...
  toolchain::lsp::Hover buildHoverForRecord(const Record *record,
                                       const SMRange &hoverRange);
  toolchain::lsp::Hover buildHoverForTemplateArg(const Record *record,
                                            const RecordVal *value,
                                            const SMRange &hoverRange);
  toolchain::lsp::Hover buildHoverForField(const Record *record,
                                      const RecordVal *value,
                                      const SMRange &hoverRange);
...
TableGenTextFile::TableGenTextFile(
    const toolchain::lsp::URIForFile &uri, StringRef fileContents, int64_t version,
    const std::vector<std::string> &extraIncludeDirs,
    std::vector<toolchain::lsp::Diagnostic> &diagnostics)
  ...
  toolchain::append_range(includeDirs, extraIncludeDirs);
...
LogicalResult TableGenTextFile::update(
    const toolchain::lsp::URIForFile &uri, int64_t newVersion,
    ArrayRef<toolchain::lsp::TextDocumentContentChangeEvent> changes,
    std::vector<toolchain::lsp::Diagnostic> &diagnostics) {
...
void TableGenTextFile::initialize(
  ...
  auto memBuffer = toolchain::MemoryBuffer::getMemBuffer(contents, uri.file());
  if (!memBuffer) {
  ...
  sourceMgr.setDiagHandler(
      ...
      &handlerContext);
  bool failedToParse = toolchain::TableGenParseFile(sourceMgr, *recordKeeper);
...
void TableGenTextFile::getLocationsOf(
    const toolchain::lsp::URIForFile &uri, const toolchain::lsp::Position &defPos,
    std::vector<toolchain::lsp::Location> &locations) {
...
std::optional<toolchain::lsp::Hover>
TableGenTextFile::findHover(const toolchain::lsp::URIForFile &uri,
                            const toolchain::lsp::Position &hoverPos) {
  ...
  toolchain::lsp::Hover hover(toolchain::lsp::Range(sourceMgr, hoverRange));
  {
    ...
    auto printAndFormatField = [&](StringRef fieldName) {
      ...
      if (!value || !value->getValue())
        return;
      auto *stringValue = dyn_cast<toolchain::StringInit>(value->getValue());
      if (!stringValue)
        return;
...
toolchain::lsp::Hover TableGenTextFile::buildHoverForTemplateArg(
...
toolchain::lsp::Hover TableGenTextFile::buildHoverForField(
    const Record *record, const RecordVal *value, const SMRange &hoverRange) {
  toolchain::lsp::Hover hover(toolchain::lsp::Range(sourceMgr, hoverRange));
  {
    toolchain::raw_string_ostream hoverOS(hover.contents.value);
...
struct lsp::TableGenServer::Impl {
  ...
  toolchain::StringMap<std::unique_ptr<TableGenTextFile>> files;
};
