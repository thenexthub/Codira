...
lsp::extractSourceDocComment(toolchain::SourceMgr &sourceMgr, SMLoc loc) {
  ...
  if (commentLines.empty())
    return std::nullopt;
  return toolchain::join(toolchain::reverse(commentLines), "\n");
}
...
Hover SourceMgrInclude::buildHover() const {
  Hover hover(range);
  {
    ...
    hoverOS << "`" << toolchain::sys::path::filename(uri.file()) << "`\n***\n"
            << uri.file();
  }
  return hover;
}

void lsp::gatherIncludeFiles(toolchain::SourceMgr &sourceMgr,
  ...
  for (unsigned i = 1, e = sourceMgr.getNumBuffers(); i < e; ++i) {
    ...
    toolchain::SmallString<256> path(buffer->getBufferIdentifier());
    toolchain::sys::path::remove_dots(path, /*remove_dot_dot=*/true);

    toolchain::Expected<URIForFile> includedFileURI = URIForFile::fromFile(path);
    if (!includedFileURI)
      continue;
