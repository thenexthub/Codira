...
struct MlirOptMainConfigCLOptions : public MlirOptMainConfig {
  MlirOptMainConfigCLOptions() {
    ...
    static cl::OptionCategory remarkCategory(
        "Remark Options",
        "Filter remarks by regular expression (toolchain::Regex syntax).");

    static toolchain::cl::opt<RemarkFormat, /*ExternalStorage=*/true> remarkFormat{
        "remark-format",
        toolchain::cl::desc("Specify the format for remark output."),
        cl::location(remarkFormatFlag),
        toolchain::cl::value_desc("format"),
        toolchain::cl::init(RemarkFormat::REMARK_FORMAT_STDOUT),
        toolchain::cl::values(clEnumValN(RemarkFormat::REMARK_FORMAT_STDOUT,
                                    "emitRemark",
        ...
        toolchain::cl::cat(remarkCategory)};

    static toolchain::cl::opt<RemarkPolicy, /*ExternalStorage=*/true> remarkPolicy{
        "remark-policy",
        toolchain::cl::desc("Specify the policy for remark output."),
        cl::location(remarkPolicyFlag),
        toolchain::cl::value_desc("format"),
        toolchain::cl::init(RemarkPolicy::REMARK_POLICY_ALL),
        toolchain::cl::values(clEnumValN(RemarkPolicy::REMARK_POLICY_ALL, "all",
                                    "Print all remarks"),
...
static LogicalResult doVerifyRoundTrip(Operation *op,
  ...
  {
    std::string buffer;
    toolchain::raw_string_ostream ostream(buffer);
    if (useBytecode) {
  ...
  {
    toolchain::raw_string_ostream ostreamref(reference);
    op->print(ostreamref,
              OpPrintingFlags().printGenericOpForm().enableDebugInfo());
    toolchain::raw_string_ostream ostreamrndtrip(roundtrip);
...
performActions(raw_ostream &os,
  ...
  case RemarkFormat::REMARK_FORMAT_YAML: {
    ...
    if (failed(mlir::remark::enableOptimizationRemarksWithLLVMStreamer(
            ctx, file, toolchain::remarks::Format::YAML, createPolicy(), cats)))
      return failure();
    break;
  }
  ...
  case RemarkFormat::REMARK_FORMAT_BITSTREAM: {
    std::string file = config.getRemarksOutputFile().empty()
                           ...
                           : config.getRemarksOutputFile();
    if (failed(mlir::remark::enableOptimizationRemarksWithLLVMStreamer(
            ctx, file, toolchain::remarks::Format::Bitstream, createPolicy(), cats)))
      return failure();
    break;
  }
  }
...
std::string mlir::registerCLIOptions(toolchain::StringRef toolName,
                                     DialectRegistry &registry) {
  ...
  {
    toolchain::raw_string_ostream os(helpHeader);
...
mlir::parseCLIOptions(int argc, char **argv, toolchain::StringRef helpHeader) {
...
static LogicalResult printRegisteredDialects(DialectRegistry &registry) {
  toolchain::outs() << "Available Dialects: ";
  interleave(registry.getDialectNames(), toolchain::outs(), ",");
  toolchain::outs() << "\n";
  return success();
}
...
LogicalResult mlir::MlirOptMain(toolchain::raw_ostream &outputStream,
  ...
  SourceMgr sourceMgr;
  sourceMgr.AddNewSourceBuffer(
      toolchain::MemoryBuffer::getMemBuffer(buffer->getMemBufferRef(),
                                       /*RequiresNullTerminator=*/false),
      SMLoc());
  ...
  auto chunkFn = [&](std::unique_ptr<MemoryBuffer> chunkBuffer,
                     toolchain::MemoryBufferRef sourceBuffer, raw_ostream &os) {
...
LogicalResult mlir::MlirOptMain(int argc, char **argv, toolchain::StringRef toolName,
                                DialectRegistry &registry) {
