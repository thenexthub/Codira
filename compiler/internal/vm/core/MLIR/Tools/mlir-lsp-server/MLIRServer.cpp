...
static std::optional<lsp::Location> getLocationFromLoc(StringRef uriScheme,
                                                       FileLineColLoc loc) {
  toolchain::Expected<lsp::URIForFile> sourceURI =
      lsp::URIForFile::fromFile(loc.getFilename(), uriScheme);
  if (!sourceURI) {
    toolchain::lsp::Logger::error("Failed to create URI for file `{0}`: {1}",
                             loc.getFilename(),
                             toolchain::toString(sourceURI.takeError()));
    return std::nullopt;
  }
...
getLocationFromLoc(toolchain::SourceMgr &sourceMgr, Location loc,
...
static bool isDefOrUse(const AsmParserState::SMDefinition &def, SMLoc loc,
  ...
  // Check the uses.
  const auto *useIt = toolchain::find_if(
...
static std::optional<unsigned> getResultNumberFromLoc(SMLoc loc) {
  ...
  while (toolchain::isDigit(*curPtr))
    ++curPtr;
...
static lsp::Diagnostic getLspDiagnoticFromDiag(toolchain::SourceMgr &sourceMgr,
                                               Diagnostic &diag,
                                               const lsp::URIForFile &uri) {
...
struct MLIRDocument {
  ...
  void getCodeActionForDiagnostic(const lsp::URIForFile &uri,
                                  ...
                                  StringRef message,
                                  std::vector<toolchain::lsp::TextEdit> &edits);

  //===--------------------------------------------------------------------===//
  // Bytecode
  //===--------------------------------------------------------------------===//

  toolchain::Expected<lsp::MLIRConvertBytecodeResult> convertToBytecode();
...
MLIRDocument::findHover(const lsp::URIForFile &uri,
  ...
  for (const AsmParserState::BlockDefinition &block : asmState.getBlockDefs()) {
    ...
    for (const auto &arg : toolchain::enumerate(block.arguments)) {
...
std::optional<lsp::Hover> MLIRDocument::buildHoverForOperation(
  ...
  toolchain::raw_string_ostream os(hover.contents.value);
...
MLIRDocument::buildHoverForBlock(SMRange hoverRange,
  ...
  if (!block.block->hasNoPredecessors()) {
    os << "Predecessors: ";
    toolchain::interleaveComma(block.block->getPredecessors(), os,
                          printBlockToHover);
    os << "\n\n";
  }
  if (!block.block->hasNoSuccessors()) {
    os << "Successors: ";
    toolchain::interleaveComma(block.block->getSuccessors(), os, printBlockToHover);
    os << "\n\n";
  }
  ...
  if (const AsmParserState::OperationDefinition *def = asmState.getOpDef(op)) {
    ...
    if (SymbolOpInterface symbol = dyn_cast<SymbolOpInterface>(op)) {
      ...
      symbols.emplace_back("<" + op->getName().getStringRef() + ">",
                           toolchain::lsp::SymbolKind::Namespace,
                           toolchain::lsp::Range(sourceMgr, def->scopeLoc),
                           toolchain::lsp::Range(sourceMgr, def->loc));
      childSymbols = &symbols.back().children;
    }
  }
...
public:
  ...
  void completeDialectName(StringRef prefix) final {
    for (StringRef dialect : ctx->getAvailableDialects()) {
      toolchain::lsp::CompletionItem item(prefix + dialect,
                                     toolchain::lsp::CompletionItemKind::Module,
                                     /*sortText=*/"3");
      item.detail = "dialect";
  ...
  void completeOperationName(StringRef dialectName) final {
    ...
    for (const auto &op : ctx->getRegisteredOperations()) {
      ...
      toolchain::lsp::CompletionItem item(
          ...
          toolchain::lsp::CompletionItemKind::Field,
          /*sortText=*/"1");
  ...
  void appendSSAValueCompletion(StringRef name, std::string typeData) final {
    ...
    toolchain::lsp::CompletionItem item(name,
                                   toolchain::lsp::CompletionItemKind::Variable);
    if (stripPrefix)
  ...
  void appendBlockCompletion(StringRef name) final {
    ...
    toolchain::lsp::CompletionItem item(name, toolchain::lsp::CompletionItemKind::Field);
    if (stripPrefix)
      item.insertText = name.drop_front(1).str();
    completionList.items.emplace_back(item);
  }
  ...
  void completeExpectedTokens(ArrayRef<StringRef> tokens, bool optional) final {
    for (StringRef token : tokens) {
      toolchain::lsp::CompletionItem item(token,
                                     toolchain::lsp::CompletionItemKind::Keyword,
                                     /*sortText=*/"0");
  ...
  void completeAttribute(const toolchain::StringMap<Attribute> &aliases) override {
  ...
  void completeDialectAttributeOrAlias(
      const toolchain::StringMap<Attribute> &aliases) override {
    completeDialectName();
    completeAliases(aliases);
  }

  /// Signal a completion for a type.
  void completeType(const toolchain::StringMap<Type> &aliases) override {
    ...
    for (StringRef type : {"i", "si", "ui"}) {
      toolchain::lsp::CompletionItem item(type + "<N>",
                                     toolchain::lsp::CompletionItemKind::Field,
                                     /*sortText=*/"1");
  ...
  void
  completeDialectTypeOrAlias(const toolchain::StringMap<Type> &aliases) override {
    completeDialectName();
    completeAliases(aliases);
  }
  ...
  template <typename T>
  void completeAliases(const toolchain::StringMap<T> &aliases,
                       StringRef prefix = "") {
    for (const auto &alias : aliases) {
      toolchain::lsp::CompletionItem item(prefix + alias.getKey(),
                                     toolchain::lsp::CompletionItemKind::Field,
                                     /*sortText=*/"2");
      toolchain::raw_string_ostream(item.detail) << "alias: " << alias.getValue();
      completionList.items.emplace_back(item);
    }
  }
  ...
  void appendSimpleCompletions(ArrayRef<StringRef> completions,
                               toolchain::lsp::CompletionItemKind kind,
                               StringRef sortText = "") {
...
void MLIRDocument::getCodeActionForDiagnostic(
    ...
    StringRef message, std::vector<toolchain::lsp::TextEdit> &edits) {
  ...
  toolchain::raw_string_ostream(edit.newText)
...
toolchain::Expected<lsp::MLIRConvertBytecodeResult>
MLIRDocument::convertToBytecode() {
  ...
  if (!toolchain::hasSingleElement(parsedIR)) {
    if (parsedIR.empty()) {
      return toolchain::make_error<toolchain::lsp::LSPError>(
          "expected a single and valid top-level operation, please ensure "
          "there are no errors",
          toolchain::lsp::ErrorCode::RequestFailed);
    }
    return toolchain::make_error<toolchain::lsp::LSPError>(
...
void MLIRTextFile::getCodeActions(const lsp::URIForFile &uri,
  ...
  for (auto &diag : context.diagnostics) {
    ...
    case toolchain::lsp::DiagnosticSeverity::Error:
      severity = "error";
      break;
    case toolchain::lsp::DiagnosticSeverity::Warning:
      severity = "warning";
      break;
    default:
      continue;
    ...
    for (toolchain::lsp::TextEdit &edit : edits)
      chunk.adjustLocForChunkOffset(edit.range);
...
struct lsp::MLIRServer::Impl {
  ...
  toolchain::StringMap<std::unique_ptr<MLIRTextFile>> files;
};
...
lsp::MLIRServer::convertFromBytecode(const URIForFile &uri) {
  ...
  // conceptually be relaxed.
  if (!toolchain::hasSingleElement(parsedBlock)) {
    return toolchain::make_error<toolchain::lsp::LSPError>(
  ...
  lsp::MLIRConvertBytecodeResult result;
  {
    ...
    toolchain::raw_string_ostream os(result.output);
