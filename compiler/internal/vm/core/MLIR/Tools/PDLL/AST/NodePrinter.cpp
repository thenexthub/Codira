...
private:
  ...
  void printChildren(RangeT &&range) {
    ...
    for (unsigned i = 0, e = toolchain::size(range) - 1; i < e; ++i, ++it)
      print(*it);
  ...
  void printChildren(StringRef label, RangeT &&range) {
    ...
    toolchain::SaveAndRestore lastElement(elementIndentStack.back(), true);
  ...
  void printIndent() {
    ...
    for (bool isLastElt : toolchain::ArrayRef(elementIndentStack).drop_back())
      os << (isLastElt ? "  " : " |");
  ...
  TypeSwitch<Type>(type)
      ...
      .Case([&](TupleType type) {
        ...
        toolchain::interleaveComma(
            toolchain::zip(type.getElementNames(), type.getElementTypes()), os,
            [&](auto it) {
...
void NodePrinter::printImpl(const UserConstraintDecl *decl) {
  ...
  if (std::optional<StringRef> codeBlock = decl->getCodeBlock()) {
    os << " Code<";
    toolchain::printEscapedString(*codeBlock, os);
    os << ">";
  }
  os << "\n";
...
void NodePrinter::printImpl(const VariableDecl *decl) {
  ...
  auto constraints =
      toolchain::map_range(decl->getConstraints(),
