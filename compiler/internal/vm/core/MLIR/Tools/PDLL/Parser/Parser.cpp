...
class Parser {
public:
  Parser(ast::Context &ctx, toolchain::SourceMgr &sourceMgr,
...
private:
  ...
  std::string processAndFormatDoc(const Twine &doc) {
    ...
    std::string docStr;
    {
      toolchain::raw_string_ostream docOS(docStr);
  ...
  void processTdIncludeRecords(const toolchain::RecordKeeper &tdRecords,
                               SmallVectorImpl<ast::Decl *> &decls);
  ...
  toolchain::SpecificBumpPtrAllocator<ast::DeclScope> scopeAllocator;
...
LogicalResult Parser::convertExpressionTo(
  ...
  auto emitConvertError = [&]() -> ast::InFlightDiagnostic {
    ast::InFlightDiagnostic diag = ctx.getDiagEngine().emitError(
        expr->getLoc(), toolchain::formatv("unable to convert expression of type "
    ...
    if (const ods::Operation *odsOp = exprType.getODSOperation()) {
      if (odsOp->getResults().empty()) {
        return emitErrorFn()->attachNote(
            toolchain::formatv("see the definition of `{0}`, which was defined "
                          "with zero results",
                          odsOp->getName()),
            odsOp->getLoc());
      }

      unsigned numSingleResults = toolchain::count_if(
...
LogicalResult Parser::convertTupleExpressionTo(
  ...
  if (auto tupleType = dyn_cast<ast::TupleType>(type)) {
    ...
    for (unsigned i = 0, e = exprType.size(); i < e; ++i) {
      newExprs.push_back(ast::MemberAccessExpr::create(
          ctx, expr->getLoc(), expr, toolchain::to_string(i),
          exprType.getElementTypes()[i]));

      auto diagFn = [&](ast::Diagnostic &diag) {
        diag.attachNote(toolchain::formatv("when converting element #{0} of `{1}`",
                                      i, exprType));
        if (noteAttachFn)
          noteAttachFn(diag);
      };
  ...
  auto convertToRange = [&](ArrayRef<ast::Type> allowedElementTypes,
    ...
    for (ast::Type elementType : exprType.getElementTypes())
      if (!toolchain::is_contained(allowedElementTypes, elementType))
        return emitErrorFn();
...
LogicalResult Parser::parseTdInclude(StringRef filename, toolchain::SMRange fileLoc,
                                     SmallVectorImpl<ast::Decl *> &decls) {
  toolchain::SourceMgr &parserSrcMgr = lexer.getSourceMgr();

  // Use the source manager to open the file, but don't yet add it.
  std::string includedFile;
  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> includeBuffer =
...
void Parser::processTdIncludeRecords(const toolchain::RecordKeeper &tdRecords,
                                     SmallVectorImpl<ast::Decl *> &decls) {
  ...
  auto convertLocToRange = [&](toolchain::SMLoc loc) -> toolchain::SMRange {
    return {loc, toolchain::SMLoc::getFromPointer(loc.getPointer() + 1)};
  };

  // Process the parsed tablegen records to build ODS information.
  /// Operations.
  for (const toolchain::Record *def : tdRecords.getAllDerivedDefinitions("Op")) {
  ...
  auto shouldBeSkipped = [this](const toolchain::Record *def) {
    return def->isAnonymous() || curDeclScope->lookup(def->getName()) ||
           def->isSubClassOf("DeclareInterfaceMethods");
  };

  /// Attr constraints.
  for (const toolchain::Record *def : tdRecords.getAllDerivedDefinitions("Attr")) {
  ...
  /// Type constraints.
  for (const toolchain::Record *def : tdRecords.getAllDerivedDefinitions("Type")) {
    if (shouldBeSkipped(def))
      continue;
  ...
  for (const toolchain::Record *def :
    ...
    std::string cppClassName =
        toolchain::formatv("{0}::{1}", def->getValueAsString("cppNamespace"),
                      def->getValueAsString("cppInterfaceName"))
            .str();
    std::string codeBlock =
        toolchain::formatv("return ::mlir::success(toolchain::isa<{0}>(self));",
                      cppClassName)
            .str();
...
FailureOr<T *> Parser::parseUserConstraintOrRewriteDecl(
  ...
  SMRange loc = curToken.getLoc();
  consumeToken();
  toolchain::SaveAndRestore saveCtx(parserContext, declContext);
  ...
  if (curToken.isNot(Token::identifier)) {
    ...
    // important.
    std::string anonName =
        toolchain::formatv("<anonymous_{0}_{1}>", anonymousNamePrefix,
  ...
  if (bodyIt != bodyE) {
    ...
    if (std::next(bodyIt) != bodyE) {
      return emitError(
          ...
          toolchain::formatv("`return` terminated the `{0}` body, but found "
    ...
    return emitError(
        ...
        toolchain::formatv("missing return in a `{0}` expected to return `{1}`",
                      declType, resultType));
  }
  return success();
}
...
FailureOr<ast::Decl *> Parser::parsePatternDecl() {
  ...
  toolchain::SaveAndRestore saveCtx(parserContext, ParserContext::PatternMatch);
        ...
        if (consumeIf(Token::equal)) {
          ...
          if (!elementNameIt.second) {
            return emitErrorAndNote(
                ...
                toolchain::formatv("duplicate tuple element label `{0}`",
                              elementName),
...
FailureOr<ast::ReplaceStmt *> Parser::parseReplaceStmt() {
  ...
  toolchain::SaveAndRestore saveCtx(parserContext, ParserContext::Rewrite);
...
ast::Type Parser::createUserConstraintRewriteResultType(
  ...
  auto resultTypes = toolchain::map_range(
      results, [&](const auto *result) { return result->getType(); });
  auto resultNames = toolchain::map_range(
      results, [&](const auto *result) { return result->getName().getName(); });
  return ast::TupleType::get(ctx, toolchain::to_vector(resultTypes),
                             toolchain::to_vector(resultNames));
}
...
Parser::createVariableDecl(StringRef name, SMRange loc, ast::Expr *initializer,
  ...
  if (isa<ast::ConstraintType, ast::RewriteType>(type)) {
    return emitError(
        loc, toolchain::formatv("unable to define variable of `{0}` type", type));
  }
...
LogicalResult Parser::validateVariableConstraint(const ast::ConstraintRef &ref,
  ...
  if (!inferredType) {
    ...
    return emitError(ref.referenceLoc,
                     toolchain::formatv("constraint type `{0}` is incompatible "
...
Parser::createCallExpr(SMRange loc, ast::Expr *parentExpr,
  ...
  if (!callableDecl) {
    return emitError(loc,
                     toolchain::formatv("expected a reference to a callable "
  ...
  if (callArgs.size() != arguments.size()) {
    return emitErrorAndNote(
        loc,
        toolchain::formatv("invalid number of arguments for {0} call; expected "
                      "{1}, but got {2}",
                      callableDecl->getCallableType(), callArgs.size(),
                      arguments.size()),
        callableDecl->getLoc(),
        toolchain::formatv("see the definition of {0} here",
                      callableDecl->getName()->getName()));
  }

  /// Handle argument type mismatch.
  auto attachDiagFn = [&](ast::Diagnostic &diag) {
    diag.attachNote(toolchain::formatv("see the definition of `{0}` here",
                                  callableDecl->getName()->getName()),
                    callableDecl->getLoc());
  };
  for (auto it : toolchain::zip(callArgs, arguments)) {
...
FailureOr<ast::Type> Parser::validateMemberAccess(ast::Expr *parentExpr,
  ...
  if (ast::OperationType opType = dyn_cast<ast::OperationType>(parentType)) {
    ...
    if (const ods::Operation *odsOp = opType.getODSOperation()) {
      ...
      unsigned index = 0;
      if (toolchain::isDigit(name[0]) && !name.getAsInteger(/*Radix=*/10, index) &&
          index < results.size()) {
        return results[index].isVariadic() ? valueRangeTy : valueTy;
      }

      // Handle named results.
      const auto *it = toolchain::find_if(results, [&](const auto &result) {
        return result.getName() == name;
      });
      if (it != results.end())
        return it->isVariadic() ? valueRangeTy : valueTy;
    } else if (toolchain::isDigit(name[0])) {
    ...
    const auto *it = toolchain::find(elementNames, name);
    if (it != elementNames.end())
      return tupleType.getElementTypes()[it - elementNames.begin()];
  }
  return emitError(
      loc,
      toolchain::formatv("invalid member access `{0}` on expression of type `{1}`",
                    name, parentType));
}
  ...
  for (ast::NamedAttributeDecl *attr : attributes) {
    ...
    if (!isa<ast::AttributeType>(attrType)) {
      return emitError(
          ...
          toolchain::formatv("expected `Attr` expression, but got `{0}`", attrType));
    }
  }
...
void Parser::checkOperationResultTypeInferrence(SMRange loc, StringRef opName,
  ...
  bool requiresInferrence =
      toolchain::any_of(odsOp->getResults(), [](const ods::OperandOrResult &result) {
  ...
  if (requiresInferrence && !odsOp->hasResultTypeInferrence()) {
    ast::InFlightDiagnostic diag = ctx.getDiagEngine().emitWarning(
        loc,
        toolchain::formatv("operation result types are marked to be inferred, but "
                      ...
                      opName));
    diag->attachNote(toolchain::formatv("see the definition of `{0}` here", opName),
                     odsOp->getLoc());
    return;
  }
}
...
LogicalResult Parser::validateOperationOperandsOrResults(
  ...
  if (odsOpLoc) {
    auto emitSizeMismatchError = [&] {
      return emitErrorAndNote(
          loc,
          toolchain::formatv("invalid number of {0} groups for `{1}`; expected "
                        "{2}, but got {3}",
                        groupName, *name, odsValues.size(), values.size()),
          *odsOpLoc, toolchain::formatv("see the definition of `{0}` here", *name));
    };
    ...
    auto diagFn = [&](ast::Diagnostic &diag) {
      diag.attachNote(toolchain::formatv("see the definition of `{0}` here", *name),
                      *odsOpLoc);
    };
...
Parser::createTupleExpr(SMRange loc, ArrayRef<ast::Expr *> elements,
  ...
  for (const ast::Expr *element : elements) {
    ...
    if (isa<ast::ConstraintType, ast::RewriteType, ast::TupleType>(eleTy)) {
      return emitError(
          element->getLoc(),
          toolchain::formatv("unable to build a tuple with `{0}` element", eleTy));
    }
  }
...
Parser::createReplaceStmt(SMRange loc, ast::Expr *rootOp,
  ...
  if (!isa<ast::OperationType>(rootType)) {
    return emitError(
        rootOp->getLoc(),
        toolchain::formatv("expected `Op` expression, but got `{0}`", rootType));
  }
  ...
  for (ast::Expr *&replExpr : replValues) {
    ...
    if (replType != valueTy && replType != valueRangeTy) {
      return emitError(replExpr->getLoc(),
                       toolchain::formatv("expected `Op`, `Value` or `ValueRange` "
...
mlir::pdll::parsePDLLAST(ast::Context &ctx, toolchain::SourceMgr &sourceMgr,
                         bool enableDocumentation,
                         CodeCompleteContext *codeCompleteContext) {
