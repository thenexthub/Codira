...
std::string Token::getStringValue() const {
  ...
  for (unsigned i = 0, e = bytes.size(); i != e;) {
    ...
    assert(toolchain::isHexDigit(c1) && toolchain::isHexDigit(c2) && "invalid escape");
    result.push_back((toolchain::hexDigitValue(c1) << 4) | toolchain::hexDigitValue(c2));
  }
...
Lexer::Lexer(toolchain::SourceMgr &mgr, ast::DiagnosticEngine &diagEngine,
