...
private:
  ...
  Location genLoc(toolchain::SMLoc loc);
  Location genLoc(toolchain::SMRange loc) { return genLoc(loc.Start); }
  ...
  using VariableMapTy =
      toolchain::ScopedHashTable<const ast::VariableDecl *, SmallVector<Value>>;
...
Location CodeGen::genLoc(toolchain::SMLoc loc) {
...
Value CodeGen::genSingleExpr(const ast::Expr *expr) {
  return TypeSwitch<const ast::Expr *, Value>(expr)
      ...
      .Case<const ast::CallExpr, const ast::DeclRefExpr, const ast::TupleExpr>(
          [&](auto derivedNode) {
            return toolchain::getSingleElement(this->genExprImpl(derivedNode));
          });
}
...
Value CodeGen::genExprImpl(const ast::MemberAccessExpr *expr) {
  ...
  if (ast::OperationType opType = dyn_cast<ast::OperationType>(parentType)) {
    ...
    if (!odsOp) {
      assert(toolchain::isDigit(name[0]) &&
    ...
    if (toolchain::isDigit(name[0])) {
      name.getAsInteger(/*Radix=*/10, resultIndex);
    } else {
      auto findFn = [&](const ods::OperandOrResult &result) {
        return result.getName() == name;
      };
      resultIndex = toolchain::find_if(results, findFn) - results.begin();
    }
  ...
  if (auto tupleType = dyn_cast<ast::TupleType>(parentType)) {
    ...
    if (toolchain::isDigit(name[0]))
      ...
      index = toolchain::find(elementNames, name) - elementNames.begin();
...
Value CodeGen::genExprImpl(const ast::RangeExpr *expr) {
  ...
  for (const ast::Expr *element : expr->getElements())
    toolchain::append_range(elements, genExpr(element));
...
CodeGen::genConstraintCall(const ast::UserConstraintDecl *decl, Location loc,
  ...
  for (auto it : toolchain::zip(decl->getInputs(), inputs))
  ...
  for (auto it : toolchain::zip(decl->getResults(), results))
    applyVarConstraints(std::get<0>(it), std::get<1>(it));
  return results;
}
...
CodeGen::genConstraintOrRewriteCall(const T *decl, Location loc,
  ...
  for (auto it : toolchain::zip(inputs, decl->getInputs()))
...
OwningOpRef<ModuleOp> mlir::pdll::codegenPDLLToMLIR(
    ...
    const toolchain::SourceMgr &sourceMgr, const ast::Module &module) {
