...
struct {0} : ::mlir::PDLPatternModule {{
  ...
  os << toolchain::formatv(patternClassStartStr, patternName);
...
StringRef CodeGen::getNativeTypeName(ast::Type type) {
  return toolchain::TypeSwitch<ast::Type, StringRef>(type)
...
void CodeGen::generateConstraintOrRewrite(const ast::CallableDecl *decl,
  ...
  if (isConstraint) {
    ...
    if (results.empty()) {
      ...
      os << "std::tuple<";
      toolchain::interleaveComma(results, os, [&](ast::VariableDecl *result) {
  ...
  if (!decl->getInputs().empty()) {
    os << ", ";
    toolchain::interleaveComma(decl->getInputs(), os, [&](ast::VariableDecl *input) {
