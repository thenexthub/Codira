...
sortMapByName(const toolchain::StringMap<std::unique_ptr<T>> &map) {
  SmallVector<T *> storage;
  for (auto &entry : map)
    storage.push_back(entry.second.get());
  toolchain::sort(storage, [](const auto &lhs, const auto &rhs) {
...
void Context::print(raw_ostream &os) const {
  ...
  toolchain::ScopedPrinter printer(os);
  toolchain::DictScope odsScope(printer, "ODSContext");
  for (const Dialect *dialect : sortMapByName(dialects)) {
    ...
    for (const Operation *op : sortMapByName(dialect->getOperations())) {
      ...
      if (!attributes.empty()) {
        ...
        toolchain::interleaveComma(attributes, os, [&](const Attribute &attr) {
          os << attr.getName() << " : ";
      ...
      if (!results.empty()) {
        ...
        toolchain::interleaveComma(results, os, [&](const OperandOrResult &result) {
          os << result.getName() << " : ";
