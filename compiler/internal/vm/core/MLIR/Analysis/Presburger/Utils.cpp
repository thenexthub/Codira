...
using toolchain::dynamicAPIntFromInt64;
...
static void normalizeDivisionByGCD(MutableArrayRef<DynamicAPInt> dividend,
  ...
  // `gcd` is positive.
  DynamicAPInt gcd = toolchain::gcd(abs(dividend.front()), divisor);
  ...
  for (size_t i = 1, m = dividend.size() - 1; i < m; i++) {
    gcd = toolchain::gcd(abs(dividend[i]), gcd);
    if (gcd == 1)
      return;
  }

  // Normalize the dividend and the denominator.
  toolchain::transform(dividend, dividend.begin(),
...
toolchain::SmallBitVector presburger::getSubrangeBitVector(unsigned len,
                                                      unsigned setOffset,
                                                      unsigned numSet) {
  toolchain::SmallBitVector vec(len, false);
...
void presburger::mergeLocalVars(
    ...
    toolchain::function_ref<bool(unsigned i, unsigned j)> merge) {
...
presburger::getDivLowerBound(ArrayRef<DynamicAPInt> dividend,
  ...
  toolchain::transform(dividend, ineq.begin(), std::negate<DynamicAPInt>());
  ineq[localVarIdx] = divisor;
  ineq.back() += divisor - 1;
  return ineq;
}
...
DynamicAPInt presburger::gcdRange(ArrayRef<DynamicAPInt> range) {
  ...
  for (const DynamicAPInt &elem : range) {
    gcd = toolchain::gcd(gcd, abs(elem));
    if (gcd == 1)
      return gcd;
  }
  return gcd;
...
void presburger::normalizeDiv(MutableArrayRef<DynamicAPInt> num,
  ...
  DynamicAPInt gcd = toolchain::gcd(gcdRange(num), denom);
  if (gcd == 1)
    return;
...
bool presburger::isRangeZero(ArrayRef<Fraction> arr) {
  return toolchain::all_of(arr, [](const Fraction &f) { return f == 0; });
}
