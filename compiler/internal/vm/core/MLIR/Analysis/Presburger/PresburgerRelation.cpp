...
bool PresburgerRelation::containsPoint(ArrayRef<DynamicAPInt> point) const {
  return toolchain::any_of(disjuncts, [&point](const IntegerRelation &disjunct) {
...
getIneqCoeffsFromIdx(const IntegerRelation &rel, unsigned idx) {
  ...
  if (idx < rel.getNumInequalities())
    return toolchain::to_vector<8>(rel.getInequality(idx));

  idx -= rel.getNumInequalities();
  ArrayRef<DynamicAPInt> eqCoeffs = rel.getEquality(idx / 2);

  if (idx % 2 == 0)
    return toolchain::to_vector<8>(eqCoeffs);
...
static PresburgerRelation getSetDifference(IntegerRelation b,
  ...
  while (level > 0) {
    ...
    if (level > frames.size()) {
      ...
      toolchain::SmallBitVector canIgnoreIneq(sI.getNumInequalities() +
...
bool PresburgerRelation::isIntegerEmpty() const {
  ...
  return toolchain::all_of(disjuncts, std::mem_fn(&IntegerRelation::isIntegerEmpty));
}
...
bool SetCoalescer::isFacetContained(ArrayRef<DynamicAPInt> ineq,
  ...
  return toolchain::all_of(cuttingIneqsB, [&simp](ArrayRef<DynamicAPInt> curr) {
...
LogicalResult SetCoalescer::coalescePairCutCase(unsigned i, unsigned j) {
  ...
  if (toolchain::any_of(cuttingIneqsA, [this, &simp](ArrayRef<DynamicAPInt> curr) {
...
bool PresburgerRelation::hasOnlyDivLocals() const {
  return toolchain::all_of(disjuncts, [](const IntegerRelation &rel) {
...
bool PresburgerRelation::isFullDim() const {
  return toolchain::any_of(getAllDisjuncts(), [](IntegerRelation disjunct) {
...
void PresburgerRelation::dump() const { print(toolchain::errs()); }
