...
static bool rangeIsZero(ArrayRef<DynamicAPInt> range) {
  return toolchain::all_of(range, [](const DynamicAPInt &x) { return x == 0; });
}
...
SymbolicLexOpt IntegerRelation::findSymbolicIntegerLexMin() const {
  ...
  toolchain::SmallBitVector isSymbol(getNumVars(), false);
...
static void eliminateFromConstraint(IntegerRelation *constraints,
  ...
  DynamicAPInt lcm = toolchain::lcm(pivotCoeff, leadCoeff);
...
bool IntegerRelation::isEmptyByGCDTest() const {
  ...
  for (unsigned i = 0, e = getNumEqualities(); i < e; ++i) {
    ...
    for (unsigned j = 1; j < numCols - 1; ++j) {
      gcd = toolchain::gcd(gcd, abs(atEq(i, j)));
    }
...
IntegerRelation::findIntegerSample() const {
  ...
  SmallVector<DynamicAPInt, 8> coneSample(
      toolchain::map_range(shrunkenConeSample, ceil));
...
void IntegerRelation::pruneOrthogonalConstraints(unsigned pos) {
  toolchain::DenseSet<unsigned> relatedCols({pos}), relatedRows;
  ...
  toolchain::SmallVector<unsigned> rowStack, colStack({pos});
  ...
  for (auto ubPos : ubIndices) {
    for (auto lbPos : lbIndices) {
      ...
      for (unsigned l = 0, e = getNumCols(); l < e; l++) {
        ...
        DynamicAPInt lcm = toolchain::lcm(lbCoeff, ubCoeff);
  ...
  for (unsigned d = 0, e = getNumDimVars(); d < e; ++d) {
    ...
    if (uRes == BoundCmpResult::Greater || uRes == BoundCmpResult::Equal) {
      ...
      if (!constUb.has_value() || !constOtherUb.has_value())
        return failure();
      toolchain::fill(maxUb, 0);
      maxUb.back() = std::max(*constUb, *constOtherUb);
    }

    toolchain::fill(newLb, 0);
    toolchain::fill(newUb, 0);
...
void IntegerRelation::removeIndependentConstraints(unsigned pos, unsigned num) {
  ...
  for (auto nbIndex : toolchain::reverse(nbIneqIndices))
    removeInequality(nbIndex);
  for (auto nbIndex : toolchain::reverse(nbEqIndices))
...
bool IntegerRelation::isFullDim() {
  ...
  Simplex simplex(*this);
  return toolchain::none_of(toolchain::seq<int>(getNumInequalities()), [&](int i) {
...
void IntegerRelation::dump() const { print(toolchain::errs()); }
