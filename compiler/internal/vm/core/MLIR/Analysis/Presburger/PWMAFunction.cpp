...
void MultiAffineFunction::dump() const { print(toolchain::errs()); }
...
MultiAffineFunction::valueAt(ArrayRef<DynamicAPInt> point) const {
  ...
  SmallVector<DynamicAPInt, 8> pointHomogenous{toolchain::to_vector(point)};
...
bool MultiAffineFunction::isEqual(const MultiAffineFunction &other,
  ...
  assert(space.isCompatible(other.space) &&
         "Spaces should be compatible for equality check.");
  return toolchain::all_of(domain.getAllDisjuncts(),
...
bool PWMAFunction::isEqual(const PWMAFunction &other) const {
  ...
  return toolchain::all_of(this->pieces, [&other](const Piece &pieceA) {
    return toolchain::all_of(other.pieces, [&pieceA](const Piece &pieceB) {
...
void PWMAFunction::dump() const { print(toolchain::errs()); }

PWMAFunction PWMAFunction::unionFunction(
    const PWMAFunction &func,
    toolchain::function_ref<PresburgerSet(Piece maf1, Piece maf2)> tiebreak) const {
