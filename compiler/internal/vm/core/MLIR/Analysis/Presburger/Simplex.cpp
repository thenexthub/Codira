...
SimplexBase::SimplexBase(unsigned nVar, bool mustUseBigM,
                         const toolchain::SmallBitVector &isSymbol)
...
unsigned SimplexBase::addRow(ArrayRef<DynamicAPInt> coeffs,
  ...
  for (unsigned i = 0; i < var.size(); ++i) {
    ...
    // the lcm of the two.
    DynamicAPInt lcm = toolchain::lcm(tableau(newRow, 0), tableau(pos, 0));
...
MaybeOptimum<SmallVector<DynamicAPInt, 8>> LexSimplex::findIntegerLexMin() {
  ...
  return toolchain::to_vector<8>(
      toolchain::map_range(*sample, std::mem_fn(&Fraction::getAsInteger)));
}
  ...
  if (entry == UndoLogEntry::RemoveLastConstraint) {
    ...
    for (int index : basis) {
      ...
      for (unsigned col = getNumFixedCols(), e = getNumColumns(); col < e;
        ...
        if (toolchain::is_contained(basis, colUnknown[col]))
          continue;
...
Simplex Simplex::makeProduct(const Simplex &a, const Simplex &b) {
  ...
  auto concat = [](ArrayRef<Unknown> v, ArrayRef<Unknown> w) {
    ...
    toolchain::append_range(result, v);
    toolchain::append_range(result, w);
    return result;
  };
...
public:
  ...
  void addEqualityForDirection(ArrayRef<DynamicAPInt> dir) {
    assert(toolchain::any_of(dir, [](const DynamicAPInt &X) { return X != 0; }) &&
  ...
  while (level != -1u) {
    ...
    if (level >= upperBoundStack.size()) {
      ...
      SmallVector<DynamicAPInt, 8> basisCoeffs =
          toolchain::to_vector<8>(basis.getRow(level));
      ...
      if (*minRoundedUp < *maxRoundedDown) {
        ...
        basisCoeffs = toolchain::to_vector<8>(basis.getRow(level));
        basisCoeffs.emplace_back(0);
...
void SimplexBase::dump() const { print(toolchain::errs()); }
