...
ConeV mlir::presburger::detail::getDual(ConeH cone) {
  ...
  for (auto i : toolchain::seq<int>(0, numIneq)) {
...
mlir::presburger::detail::computeUnimodularConeGeneratingFunction(
  ...
  for (auto i : toolchain::seq<int>(0, numVar)) {
  ...
  for (auto i : toolchain::seq<int>(0, numColumns)) {
    for (auto j : toolchain::seq<int>(0, numRows))
      ithCol[j] = vertex(j, i);
...
mlir::presburger::detail::computePolytopeGeneratingFunction(
  ...
  do {
    ...
    if (!vertex)
      continue;
    if (toolchain::is_contained(vertices, vertex))
      continue;
...
mlir::presburger::detail::computeNumTerms(const GeneratingFunction &gf) {
  ...
  for (ArrayRef<Point> den : gf.getDenominators())
    toolchain::append_range(allDenominators, den);
