...
static LogicalResult getBackwardSliceImpl(Operation *op,
  ...
  if (!options.omitUsesFromAbove &&
    ...
    toolchain::for_each(op->getRegions(), [&](Region &region) {
  ...
  toolchain::for_each(op->getOperands(), processValue);
...
static bool dependsOnCarriedVals(Value value,
  ...
  SmallPtrSet<Value, 8> iterCarriedValSet(toolchain::from_range, iterCarriedArgs);
