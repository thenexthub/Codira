  ...
  for (unsigned j = 0; j < ncols; j++) {
    if (j == ncols - 1) {
      ...
      if (row[j] == 0 && !firstNonZero) {
        if (fixedColWidth)
          toolchain::errs().indent(7);
      } else {
        toolchain::errs() << ((row[j] >= 0) ? "+ " : "") << row[j] << ' ';
      }
    } else {
...
void FlatLinearConstraints::dumpPretty() const {
  ...
  toolchain::errs() << "Constraints (" << getNumDimVars() << " dims, "
...
std::pair<AffineMap, AffineMap> FlatLinearConstraints::getLowerAndUpperBound(
  ...
  for (auto idx : lbIndices) {
    ...
    // - 1.
    addCoeffs(ineq, lb);
    toolchain::transform(lb, lb.begin(), std::negate<int64_t>());
    auto expr =
  ...
  for (auto idx : eqIndices) {
    ...
    addCoeffs(eq, b);
    if (eq[pos + offset] > 0)
      toolchain::transform(b, b.begin(), std::negate<int64_t>());
...
void FlatLinearConstraints::getSliceBounds(unsigned offset, unsigned num,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "getSliceBounds for variables at positions ["
                          << offset << ", " << offset + num << ")\n");
  ...
  for (unsigned pos = 0; pos < num; pos++) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Slice bounds:\n");
    LLVM_DEBUG(toolchain::dbgs() << "lb map for pos = " << Twine(pos + offset)
                            << ", expr: " << lbMap << '\n');
    LLVM_DEBUG(toolchain::dbgs() << "ub map for pos = " << Twine(pos + offset)
                            << ", expr: " << ubMap << '\n');
  }
}
...
FlatLinearConstraints::computeLocalVars(SmallVectorImpl<AffineExpr> &memo,
  ...
  return success(
      toolchain::all_of(localExprs, [](AffineExpr expr) { return expr; }));
}
...
IntegerSet FlatLinearConstraints::getAsIntegerSet(MLIRContext *context) const {
  ...
  if (failed(computeLocalVars(memo, context))) {
    ...
    if (!noLocalRepVars.empty()) {
      LLVM_DEBUG({
        toolchain::dbgs() << "local variables at position(s) "
                     << toolchain::interleaved(noLocalRepVars)
...
static void mergeAndAlignVars(unsigned offset, FlatLinearValueConstraints *a,
  ...
  assert(toolchain::all_of(
      toolchain::drop_begin(a->getMaybeValues(), offset),
      [](const std::optional<Value> &var) { return var.has_value(); }));

  assert(toolchain::all_of(
      toolchain::drop_begin(b->getMaybeValues(), offset),
      [](const std::optional<Value> &var) { return var.has_value(); }));
...
AffineMap mlir::alignAffineMapWithValues(AffineMap map, ValueRange operands,
  ...
  for (const auto &operand : toolchain::enumerate(operands)) {
    // Compute replacement dim/sym of operand.
    AffineExpr replacement;
    auto dimIt = toolchain::find(dims, operand.value());
    auto symIt = toolchain::find(syms, operand.value());
    if (dimIt != dims.end()) {
      replacement =
