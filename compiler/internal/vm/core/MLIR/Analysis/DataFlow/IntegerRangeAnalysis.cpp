...
LogicalResult staticallyNonNegative(DataFlowSolver &solver, Operation *op) {
  ...
  return success(toolchain::all_of(op->getOperands(), nonNegativePred) &&
                 toolchain::all_of(op->getResults(), nonNegativePred));
}
} // namespace mlir::dataflow
...
LogicalResult IntegerRangeAnalysis::visitOperation(
  ...
  auto argRanges = toolchain::map_to_vector(
  ...
  auto joinCallback = [&](Value v, const IntegerValueRange &attrs) {
    ...
    // and often can't).
    bool isYieldedResult = toolchain::any_of(v.getUsers(), [](Operation *op) {
...
void IntegerRangeAnalysis::visitNonControlFlowArguments(
  ...
  if (auto inferrable = dyn_cast<InferIntRangeInterface>(op)) {
    ...
    auto argRanges = toolchain::map_to_vector(op->getOperands(), [&](Value value) {
    ...
    auto joinCallback = [&](Value v, const IntegerValueRange &attrs) {
      ...
      if (!arg)
        return;
      if (!toolchain::is_contained(successor.getSuccessor()->getArguments(), arg))
        return;
      ...
      // the dataflow analysis in MLIR doesn't attempt to work out trip counts
      // and often can't).
      bool isYieldedValue = toolchain::any_of(v.getUsers(), [](Operation *op) {
        return op->hasTrait<OpTrait::IsTerminator>();
      });
  ...
  auto getLoopBoundFromFold = [&](OpFoldResult loopBound, Type boundType,
    ...
    } else if (auto value = toolchain::dyn_cast<Value>(loopBound)) {
  ...
  if (auto loop = dyn_cast<LoopLikeOpInterface>(op)) {
    std::optional<toolchain::SmallVector<Value>> maybeIvs =
    ...
    for (auto [iv, lowerBound, upperBound, step] :
         toolchain::zip_equal(*maybeIvs, lowerBounds, upperBounds, steps)) {
