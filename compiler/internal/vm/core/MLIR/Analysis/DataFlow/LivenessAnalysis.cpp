...
#include <toolchain/Support/DebugLog.h>
...
void LivenessAnalysis::visitNonControlFlowArguments(
  ...
  SmallVector<Liveness *> argumentLattices =
      toolchain::map_to_vector(arguments, valuesToLattices);
  SmallVector<Liveness *> parentResultLattices =
      toolchain::map_to_vector(parentOp->getResults(), valuesToLattices);
...
RunLivenessAnalysis::RunLivenessAnalysis(Operation *op) {
  ...
  op->walk([&](Operation *op) {
    for (auto result : toolchain::enumerate(op->getResults())) {
    ...
    for (auto &region : op->getRegions()) {
      for (auto &block : region) {
        for (auto blockArg : toolchain::enumerate(block.getArguments())) {
