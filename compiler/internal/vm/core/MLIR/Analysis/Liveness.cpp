...
struct BlockInfoBuilder {
  ...
  BlockInfoBuilder(Block *block) : block(block) {
    ...
    toolchain::set_subtract(useValues, defValues);
  }
  ...
  bool updateLiveIn() {
    ...
    toolchain::set_union(newIn, outValues);
    toolchain::set_subtract(newIn, defValues);
  ...
  void updateLiveOut(const DenseMap<Block *, BlockInfoBuilder> &builders) {
    for (Block *succ : block->getSuccessors()) {
      ...
      toolchain::set_union(outValues, builder.inValues);
    }
  }
...
void Liveness::dump() const { print(toolchain::errs()); }
...
void Liveness::print(raw_ostream &os) const {
  ...
  auto printValueRefs = [&](const ValueSetT &values) {
    ...
    toolchain::sort(orderedValues, [&](Value left, Value right) {
  ...
  operation->walk<WalkOrder::PreOrder>([&](Block *block) {
    ...
    for (Operation &op : *block) {
      ...
      for (Value result : op.getResults()) {
        ...
        toolchain::sort(liveOperations, [&](Operation *left, Operation *right) {
...
LivenessBlockInfo::currentlyLiveValues(Operation *op) const {
  ...
  for (Operation &walkOp :
       toolchain::make_range(block->begin(), ++op->getIterator()))
