...
bool CallGraphNode::hasChildren() const {
  return toolchain::any_of(edges, [](const Edge &edge) { return edge.isChild(); });
}
...
void CallGraph::eraseNode(CallGraphNode *node) {
  ...
  if (node->hasChildren()) {
    for (const CallGraphNode::Edge &edge : toolchain::make_early_inc_range(*node))
      if (edge.isChild())
...
void CallGraph::dump() const { print(toolchain::errs()); }
void CallGraph::print(raw_ostream &os) const {
  ...
  for (auto &scc : make_range(toolchain::scc_begin(this), toolchain::scc_end(this))) {
    os << "// - SCC : \n";
    for (auto &node : scc) {
      os << "// -- Node :";
      emitNodeName(node);
