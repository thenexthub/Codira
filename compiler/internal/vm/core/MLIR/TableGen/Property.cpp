...
Property::Property(const toolchain::Record *maybeDef, StringRef summary,
...
Pred Property::getPredicate() const {
  if (!def)
    return Pred();
  const toolchain::RecordVal *maybePred = def->getValue("predicate");
...
Property Property::getBaseProperty() const {
  if (const auto *defInit =
          toolchain::dyn_cast<toolchain::DefInit>(def->getValueInit("baseProperty"))) {
