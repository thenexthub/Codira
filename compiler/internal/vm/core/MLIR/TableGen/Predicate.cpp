...
using toolchain::SpecificBumpPtrAllocator;
...
Pred::Pred(const Init *init) {
  if (const auto *defInit = dyn_cast_or_null<toolchain::DefInit>(init))
    def = defInit->getDef();
}
...
static void performSubstitutions(std::string &str,
  ...
  for (const auto &subst : toolchain::reverse(substitutions)) {
...
buildPredicateTree(const Pred &root,
  ...
  auto allSubstitutions = toolchain::to_vector<4>(substitutions);
...
propagateGroundTruth(PredNode *node,
                     const toolchain::SmallPtrSetImpl<Pred *> &knownTruePreds,
                     const toolchain::SmallPtrSetImpl<Pred *> &knownFalsePreds) {
  ...
  toolchain::SmallVector<PredNode *, 4> children;
...
static std::string getCombinedCondition(const PredNode &root) {
  ...
  toolchain::SmallVector<std::string, 4> childExpressions;
  ...
  toolchain::PrintFatalError(root.predicate->getLoc(), "unsupported predicate kind");
}
...
std::string CombinedPred::getConditionImpl() const {
  ...
  predicateTree =
      propagateGroundTruth(predicateTree,
                           /*knownTruePreds=*/toolchain::SmallPtrSet<Pred *, 2>(),
                           /*knownFalsePreds=*/toolchain::SmallPtrSet<Pred *, 2>());
