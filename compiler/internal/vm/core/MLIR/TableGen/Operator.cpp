...
std::string Operator::getOperationName() const {
  ...
  return std::string(toolchain::formatv("{0}.{1}", prefix, opName));
}

std::string Operator::getAdaptorName() const {
  return std::string(toolchain::formatv("{0}Adaptor", getCppClassName()));
}

std::string Operator::getGenericAdaptorName() const {
  return std::string(toolchain::formatv("{0}GenericAdaptor", getCppClassName()));
}
...
std::string Operator::getQualCppClassName() const {
  ...
  return std::string(toolchain::formatv("{0}::{1}", cppNamespace, cppClassName));
}
...
unsigned Operator::getNumVariableLengthResults() const {
  return toolchain::count_if(results, [](const NamedTypeConstraint &c) {
    return c.constraint.isVariableLength();
  });
}

unsigned Operator::getNumVariableLengthOperands() const {
  return toolchain::count_if(operands, [](const NamedTypeConstraint &c) {
    return c.constraint.isVariableLength();
  });
}
...
auto Operator::getRegions() const
    -> toolchain::iterator_range<const_region_iterator> {
...
unsigned Operator::getNumVariadicRegions() const {
  return toolchain::count_if(regions,
...
auto Operator::getSuccessors() const
    -> toolchain::iterator_range<const_successor_iterator> {
...
unsigned Operator::getNumVariadicSuccessors() const {
  return toolchain::count_if(successors,
...
auto Operator::getTraits() const -> toolchain::iterator_range<const_trait_iterator> {
...
auto Operator::getAttributes() const
    -> toolchain::iterator_range<const_attribute_iterator> {
...
auto Operator::getAttributes() -> toolchain::iterator_range<attribute_iterator> {
  return {attribute_begin(), attribute_end()};
}
...
bool Operator::isVariadic() const {
  return any_of(toolchain::concat<const NamedTypeConstraint>(operands, results),
                [](const NamedTypeConstraint &op) { return op.isVariadic(); });
}

void Operator::populateTypeInferenceInfo(
    const toolchain::StringMap<int> &argumentsAndResultsIndex) {
  ...
  if (getTrait("::mlir::OpTrait::SameOperandsAndResultType")) {
    ...
    auto *operandI = toolchain::find_if(arguments, [](const Argument &arg) {
      NamedTypeConstraint *operand =
          toolchain::dyn_cast_if_present<NamedTypeConstraint *>(arg);
  ...
  for (auto [idx, infer] : toolchain::enumerate(inference)) {
  ...
  for (const Trait &trait : traits) {
    ...
    if (def.isSubClassOf(
            toolchain::formatv("{0}::Trait", inferTypeOpInterface).str()))
      return;
      ...
      if (InferredResultType::isResultIndex(elementsIndex) &&
        ...
        if (!infer.inferred) {
          infer.sources.emplace_back(
              shapedIndex,
              "::toolchain::SmallVector<::mlir::Type>(::toolchain::cast<::mlir::"
              "ShapedType>($_self).getNumElements(), "
              "::toolchain::cast<::mlir::ShapedType>($_self).getElementType())");
          infer.inferred = true;
        }
      }
  ...
  do {
    ...
    for (auto cur = worklist.begin(); cur != worklist.end();) {
      ...
      InferredResultType *iter =
          toolchain::find_if(infer.sources, [&](const InferredResultType &source) {
...
void Operator::populateOpStructure() {
  ...
  toolchain::StringMap<int> argumentsAndResultsIndex;
  ...
  for (const auto &val : def.getValues()) {
    if (auto *record = dyn_cast<toolchain::RecordRecTy>(val.getType())) {
  ...
  LLVM_DEBUG(print(toolchain::dbgs()));
}
...
void Operator::print(toolchain::raw_ostream &os) const {
  os << "op '" << getOperationName() << "'\n";
  for (Argument arg : arguments) {
    if (auto *attr = toolchain::dyn_cast_if_present<NamedAttribute *>(arg))
