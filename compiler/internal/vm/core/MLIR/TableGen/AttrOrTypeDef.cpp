...
AttrOrTypeDef::AttrOrTypeDef(const Record *def) : def(def) {
  ...
  if (auto *traitList = def->getValueAsListInit("traits")) {
    ...
    toolchain::unique_function<void(const ListInit *)> processTraitList =
...
std::string AttrOrTypeParameter::getAccessorName() const {
  return "get" +
         toolchain::convertToCamelFromSnakeCase(getName(), /*capitalizeFirst=*/true);
}
