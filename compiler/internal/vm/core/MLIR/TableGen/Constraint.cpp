...
Constraint::Constraint(const toolchain::Record *record)
  ...
  if (def->isSubClassOf("TypeConstraint")) {
    ...
    toolchain::errs() << "Expected a constraint but got: \n" << *def << "\n";
    toolchain::report_fatal_error("Abort");
  }
}
...
Pred Constraint::getPredicate() const {
  ...
  const auto *pred = dyn_cast<toolchain::DefInit>(val->getValue());
  return Pred(pred);
}
...
std::optional<StringRef> Constraint::getBaseDefName() const {
  ...
  auto checkBaseDefFn = [&](StringRef baseName) -> std::optional<StringRef> {
    if (const auto *defValue = def->getValue(baseName)) {
      if (const auto *defInit = dyn_cast<toolchain::DefInit>(defValue->getValue()))
...
unsigned DenseMapInfo<Constraint>::getHashValue(Constraint constraint) {
  ...
  return toolchain::hash_combine(constraint.getPredicate(), constraint.getSummary());
}
