...
bool DagLeaf::isUnspecified() const {
  return isa_and_nonnull<toolchain::UnsetInit>(def);
}
...
bool DagLeaf::isStringAttr() const { return isa<toolchain::StringInit>(def); }
...
std::string SymbolInfoMap::SymbolInfo::getValueAndRangeUse(
  ...
  case Kind::Result: {
    ...
    auto repl = toolchain::join(values, separator);
...
Pattern::getLocation(bool forSourceOutput) const {
  ...
  for (auto loc : def.getLoc()) {
    unsigned buf = toolchain::SrcMgr.FindBufferContainingLoc(loc);
    assert(buf && "invalid source location");

    StringRef bufferName =
        toolchain::SrcMgr.getBufferInfo(buf).Buffer->getBufferIdentifier();
    ...
    if (forSourceOutput && toolchain::sys::path::is_absolute(bufferName))
      bufferName = toolchain::sys::path::filename(bufferName);

    result.emplace_back(bufferName,
                        toolchain::SrcMgr.getLineAndColumn(loc, buf).first);
  }
  return result;
}
