...
void PassExecutionAction::print(raw_ostream &os) const {
  os << toolchain::formatv("`{0}` running `{1}` on Operation `{2}`", tag,
...
Operation *PassExecutionAction::getOp() const {
  ...
  return irUnits.empty() ? nullptr
                         : toolchain::dyn_cast_if_present<Operation *>(irUnits[0]);
}
...
void OpPassManagerImpl::addPass(std::unique_ptr<Pass> pass) {
  ...
  if (pmOpName && passOpName && *pmOpName != *passOpName) {
    ...
    toolchain::report_fatal_error(toolchain::Twine("Can't add pass '") + pass->getName() +
...
LogicalResult OpPassManagerImpl::finalizePassList(MLIRContext *ctx) {
  ...
  toolchain::erase_if(passes, std::logical_not<std::unique_ptr<Pass>>());
...
bool OpPassManagerImpl::canScheduleOn(MLIRContext &context,
  ...
  if (!registeredInfo ||
    ...
    return false;
  return toolchain::all_of(passes, [&](const std::unique_ptr<Pass> &pass) {
...
void printAsTextualPipeline(
    ...
    const toolchain::iterator_range<OpPassManager::pass_iterator> &passes,
    bool pretty = false) {
  os << anchorName << "(";
  if (pretty) {
    os << "\n";
...
void OpPassManager::dump() {
  toolchain::errs() << "Pass Manager with " << impl->passes.size() << " passes:\n";
  printAsTextualPipeline(toolchain::errs(), /*pretty=*/true);
  toolchain::errs() << "\n";
}
...
toolchain::hash_code OpPassManager::hash() {
  toolchain::hash_code hashCode{};
  for (Pass &pass : getPasses()) {
    // If this pass isn't an adaptor, directly hash it.
    auto *adaptor = dyn_cast<OpToOpPassAdaptor>(&pass);
    if (!adaptor) {
      hashCode = toolchain::hash_combine(hashCode, &pass);
      continue;
    }
    // Otherwise, hash recursively each of the adaptors pass managers.
    for (OpPassManager &adaptorPM : adaptor->getPassManagers())
      toolchain::hash_combine(hashCode, adaptorPM.hash());
  }
  return hashCode;
}
...
LogicalResult OpToOpPassAdaptor::run(Pass *pass, Operation *op,
  ...
  PassInstrumentation::PipelineParentInfo parentInfo = {toolchain::get_threadid(),
                                                        pass};
...
LogicalResult OpToOpPassAdaptor::runPipeline(
  ...
  toolchain::scope_exit scopeExit([&] {
...
static OpPassManager *findPassManagerFor(MutableArrayRef<OpPassManager> mgrs,
  ...
  auto *it = toolchain::find_if(mgrs, [&](OpPassManager &mgr) {
...
LogicalResult OpToOpPassAdaptor::tryMergeInto(MLIRContext *ctx,
  ...
  auto hasScheduleConflictWith = [&](OpPassManager &genericPM,
    ...
    return toolchain::any_of(otherPMs, [&](OpPassManager &pm) {
  ...
  auto *lhsGenericPMIt = toolchain::find_if(mgrs, isGenericPM);
  ...
  auto *rhsGenericPMIt = toolchain::find_if(rhs.mgrs, isGenericPM);
  ...
  toolchain::sort(rhs.mgrs, compareFn);
  return success();
}
...
std::string OpToOpPassAdaptor::getAdaptorName() {
  ...
  toolchain::interleaveComma(getPassManagers(), os, [&](OpPassManager &pm) {
...
static bool hasSizeMismatch(ArrayRef<OpPassManager> lhs,
  ...
  return lhs.size() != rhs.size() ||
         toolchain::any_of(toolchain::seq<size_t>(0, lhs.size()),
...
LogicalResult PassManager::run(Operation *op) {
  ...
  toolchain::hash_code newInitKey = context->getRegistryHash();
  toolchain::hash_code pipelineKey = hash();
...
AnalysisManager AnalysisManager::nest(Operation *op) {
  ...
  for (Operation *op : toolchain::reverse(opAncestors))
...
void PassInstrumentor::runBeforePipeline(
    ...
    const PassInstrumentation::PipelineParentInfo &parentInfo) {
  toolchain::sys::SmartScopedLock<true> instrumentationLock(impl->mutex);
...
void PassInstrumentor::runAfterPipeline(
    ...
    const PassInstrumentation::PipelineParentInfo &parentInfo) {
  toolchain::sys::SmartScopedLock<true> instrumentationLock(impl->mutex);
  for (auto &instr : toolchain::reverse(impl->instrumentations))
