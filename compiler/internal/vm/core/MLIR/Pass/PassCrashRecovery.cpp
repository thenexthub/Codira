...
private:
  ...
  static toolchain::ManagedStatic<toolchain::sys::SmartMutex<true>> reproducerMutex;
  static toolchain::ManagedStatic<
      toolchain::SmallSetVector<RecoveryReproducerContext *, 1>>
      reproducerSet;
};
} // namespace detail
} // namespace mlir

toolchain::ManagedStatic<toolchain::sys::SmartMutex<true>>
    RecoveryReproducerContext::reproducerMutex;
toolchain::ManagedStatic<toolchain::SmallSetVector<RecoveryReproducerContext *, 1>>
...
static void appendReproducer(std::string &description, Operation *op,
                             ...
                             bool verifyPasses) {
  toolchain::raw_string_ostream descOS(description);
...
void RecoveryReproducerContext::disable() {
  toolchain::sys::SmartScopedLock<true> lock(*reproducerMutex);
  reproducerSet->remove(this);
  if (reproducerSet->empty())
    toolchain::CrashRecoveryContext::Disable();
}

void RecoveryReproducerContext::enable() {
  toolchain::sys::SmartScopedLock<true> lock(*reproducerMutex);
  if (reproducerSet->empty())
    toolchain::CrashRecoveryContext::Enable();
...
void RecoveryReproducerContext::registerSignalHandler() {
  ...
  static bool registered =
      (toolchain::sys::AddSignalHandler(crashHandler, nullptr), false);
  (void)registered;
}
...
void PassCrashReproducerGenerator::finalize(Operation *rootOp,
  ...
  if (!impl->localReproducer) {
    ...
    toolchain::interleaveComma(impl->runningPasses, note,
...
void PassCrashReproducerGenerator::prepareReproducerFor(Pass *pass,
  ...
  // operation type.
  std::string passStr;
  toolchain::raw_string_ostream passOS(passStr);
  for (OperationName scope : toolchain::reverse(scopes))
    passOS << scope << "(";
...
struct FileReproducerStream : public mlir::ReproducerStream {
  FileReproducerStream(std::unique_ptr<toolchain::ToolOutputFile> outputFile)
...
private:
  ...
  std::unique_ptr<toolchain::ToolOutputFile> outputFile = nullptr;
};
} // namespace
...
LogicalResult PassManager::runWithCrashRecovery(Operation *op,
  ...
  toolchain::CrashRecoveryContext recoveryContext;
...
std::string mlir::makeReproducer(
  ...
  toolchain::raw_string_ostream passOS(pipelineStr);
