...
static toolchain::ManagedStatic<toolchain::StringMap<PassInfo>> passRegistry;

/// A mapping of the above pass registry entries to the corresponding TypeID
/// of the pass that they generate.
static toolchain::ManagedStatic<toolchain::StringMap<TypeID>> passRegistryTypeIDs;

/// Static mapping of all of the registered pass pipelines.
static toolchain::ManagedStatic<toolchain::StringMap<PassPipelineInfo>>
...
buildDefaultRegistryFn(const PassAllocatorFunction &allocator) {
  return [=](OpPassManager &pm, StringRef options,
    ...
    if ((pm.getNesting() == OpPassManager::Nesting::Explicit) && pmOpName &&
      ...
      return errorHandler(toolchain::Twine("Can't add pass '") + pass->getName() +
...
static void printOptionHelp(StringRef arg, StringRef desc, size_t indent,
  ...
  toolchain::outs().indent(indent)
      << "--" << toolchain::left_justify(arg, numSpaces) << "-   " << desc << '\n';
}
...
void mlir::printRegisteredPasses() {
  ...
  auto printOrderedEntries = [&](StringRef header, auto &map) {
    toolchain::SmallVector<PassRegistryEntry *, 32> orderedEntries;
    for (auto &kv : map)
    ...
    toolchain::outs().indent(0) << header << ":\n";
...
void mlir::registerPass(const PassAllocatorFunction &function) {
  ...
  if (arg.empty())
    toolchain::report_fatal_error(toolchain::Twine("Trying to register '") +
                             pass->getName() +
...
LogicalResult detail::pass_options::parseCommaSeparatedList(
    toolchain::cl::Option &opt, StringRef argName, StringRef optionStr,
...
void detail::PassOptions::copyOptionValuesFrom(const PassOptions &other) {
  ...
  if (options.empty())
    return;
  for (auto optionsIt : toolchain::zip(options, other.options))
...
LogicalResult detail::PassOptions::parseFromString(StringRef options,
  ...
  while (!options.empty()) {
    ...
    if (toolchain::cl::ProvidePositionalOption(it->second, value, 0))
      return failure();
  }
...
void detail::PassOptions::print(raw_ostream &os) const {
  ...
  toolchain::array_pod_sort(orderedOps.begin(), orderedOps.end(), compareOptionArgs);
...
void detail::PassOptions::printHelp(size_t indent, size_t descIndent) const {
  ...
  for (OptionBase *option : orderedOps) {
    ...
    // support to toolchain::cl::Option for passing in a base indent to use when
    // printing.
    toolchain::outs().indent(indent);
...
toolchain::cl::OptionValue<OpPassManager>::OptionValue() = default;
toolchain::cl::OptionValue<OpPassManager>::OptionValue(
    const mlir::OpPassManager &value) {
  setValue(value);
}
toolchain::cl::OptionValue<OpPassManager>::OptionValue(
    const toolchain::cl::OptionValue<mlir::OpPassManager> &rhs) {
  if (rhs.hasValue())
    setValue(rhs.getValue());
}
toolchain::cl::OptionValue<OpPassManager> &
toolchain::cl::OptionValue<OpPassManager>::operator=(
    const mlir::OpPassManager &rhs) {
  setValue(rhs);
  return *this;
}

toolchain::cl::OptionValue<OpPassManager>::~OptionValue<OpPassManager>() = default;

void toolchain::cl::OptionValue<OpPassManager>::setValue(
...
void toolchain::cl::OptionValue<OpPassManager>::setValue(StringRef pipelineStr) {
  FailureOr<OpPassManager> pipeline = parsePassPipeline(pipelineStr);
  assert(succeeded(pipeline) && "invalid pass pipeline");
  setValue(*pipeline);
}

bool toolchain::cl::OptionValue<OpPassManager>::compare(
...
void toolchain::cl::OptionValue<OpPassManager>::anchor() {}
...
bool toolchain::cl::parser<OpPassManager>::parse(Option &, StringRef, StringRef arg,
...
void toolchain::cl::parser<OpPassManager>::print(raw_ostream &os,
                                            const OpPassManager &value) {
  value.printAsTextualPipeline(os);
}

void toolchain::cl::parser<OpPassManager>::printOptionDiff(
...
void toolchain::cl::parser<OpPassManager>::anchor() {}

toolchain::cl::parser<OpPassManager>::ParsedPassManager::ParsedPassManager() =
    default;
toolchain::cl::parser<OpPassManager>::ParsedPassManager::ParsedPassManager(
...
LogicalResult TextualPipeline::addToPipeline(
  ...
  toolchain::scope_exit restore([&]() { pm.setNesting(nesting); });
...
struct PassNameParser : public toolchain::cl::parser<PassArgData> {
  PassNameParser(toolchain::cl::Option &opt) : toolchain::cl::parser<PassArgData>(opt) {}

  void initialize();
  void printOptionInfo(const toolchain::cl::Option &opt,
                       size_t globalWidth) const override;
  size_t getOptionWidth(const toolchain::cl::Option &opt) const override;
  bool parse(toolchain::cl::Option &opt, StringRef argName, StringRef arg,
             PassArgData &value);
...
void PassNameParser::initialize() {
  toolchain::cl::parser<PassArgData>::initialize();
...
void PassNameParser::printOptionInfo(const toolchain::cl::Option &opt,
                                     size_t globalWidth) const {
  // If this parser is just parsing pass names, print a simplified option
  // string.
  if (passNamesOnly) {
    toolchain::outs() << "  --" << opt.ArgStr << "=<pass-arg>";
  ...
  auto printOrderedEntries = [&](StringRef header, auto &map) {
    ...
    toolchain::outs().indent(4) << header << ":\n";
    for (PassRegistryEntry *entry : orderedEntries)
...
size_t PassNameParser::getOptionWidth(const toolchain::cl::Option &opt) const {
  size_t maxWidth = toolchain::cl::parser<PassArgData>::getOptionWidth(opt) + 2;
...
bool PassNameParser::parse(toolchain::cl::Option &opt, StringRef argName,
                           StringRef arg, PassArgData &value) {
  if (toolchain::cl::parser<PassArgData>::parse(opt, argName, arg, value))
    return true;
  value.options = arg;
  return false;
}
...
struct PassPipelineCLParserImpl {
  PassPipelineCLParserImpl(StringRef arg, StringRef description,
                           bool passNamesOnly)
      : passList(arg, toolchain::cl::desc(description)) {
    passList.getParser().passNamesOnly = passNamesOnly;
    passList.setValueExpectedFlag(toolchain::cl::ValueExpected::ValueOptional);
  }
  ...
  bool contains(const PassRegistryEntry *entry) const {
    return toolchain::any_of(passList, [&](const PassArgData &data) {
      return data.registryEntry == entry;
    });
  }

  /// The set of passes and pass pipelines to run.
  toolchain::cl::list<PassArgData, bool, PassNameParser> passList;
};
} // namespace detail
} // namespace mlir
...
PassPipelineCLParser::PassPipelineCLParser(StringRef arg, StringRef description)
    : impl(std::make_unique<detail::PassPipelineCLParserImpl>(
      ...
      passPipeline(
          PASS_PIPELINE_ARG,
          toolchain::cl::desc("Textual description of the pass pipeline to run")) {}

PassPipelineCLParser::PassPipelineCLParser(StringRef arg, StringRef description,
                                           StringRef alias)
    : PassPipelineCLParser(arg, description) {
  passPipelineAlias.emplace(alias,
                            toolchain::cl::desc("Alias for --" PASS_PIPELINE_ARG),
                            toolchain::cl::aliasopt(passPipeline));
}
...
PassNameCLParser::PassNameCLParser(StringRef arg, StringRef description)
  ...
  impl->passList.setMiscFlag(toolchain::cl::CommaSeparated);
}
