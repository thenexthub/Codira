...
namespace {
static toolchain::omp::ScheduleKind
convertToScheduleKind(std::optional<omp::ClauseScheduleKind> schedKind) {
  if (!schedKind.has_value())
    return toolchain::omp::OMP_SCHEDULE_Default;
  switch (schedKind.value()) {
  case omp::ClauseScheduleKind::Static:
    return toolchain::omp::OMP_SCHEDULE_Static;
  case omp::ClauseScheduleKind::Dynamic:
    return toolchain::omp::OMP_SCHEDULE_Dynamic;
  case omp::ClauseScheduleKind::Guided:
    return toolchain::omp::OMP_SCHEDULE_Guided;
  case omp::ClauseScheduleKind::Auto:
    return toolchain::omp::OMP_SCHEDULE_Auto;
  case omp::ClauseScheduleKind::Runtime:
    return toolchain::omp::OMP_SCHEDULE_Runtime;
  case omp::ClauseScheduleKind::Distribute:
    return toolchain::omp::OMP_SCHEDULE_Distribute;
  }
...
public:
  ...
  explicit OpenMPAllocaStackFrame(toolchain::OpenMPIRBuilder::InsertPointTy allocaIP)
      : allocaInsertPoint(allocaIP) {}
  toolchain::OpenMPIRBuilder::InsertPointTy allocaInsertPoint;
};

/// Stack frame to hold a \see toolchain::CanonicalLoopInfo representing the
...
public:
  ...
  toolchain::CanonicalLoopInfo *loopInfo = nullptr;
};
...
/// \see LogicalResult instances and \see toolchain::Error instances used to
/// propagate errors through the \see toolchain::OpenMPIRBuilder. Generally, when an
...
/// this error condition as an \see toolchain::Error while avoiding triggering some
/// redundant error reporting later on, we need a custom \see toolchain::ErrorInfo
...
class PreviouslyReportedError
    : public toolchain::ErrorInfo<PreviouslyReportedError> {
public:
...
private:
  SmallVector<toolchain::Value *> linearPreconditionVars;
  SmallVector<toolchain::Value *> linearLoopBodyTemps;
  SmallVector<toolchain::Value *> linearOrigVal;
  SmallVector<toolchain::Value *> linearSteps;
  SmallVector<toolchain::Type *> linearVarTypes;
  toolchain::BasicBlock *linearFinalizationBB;
  toolchain::BasicBlock *linearExitBB;
  toolchain::BasicBlock *linearLastIterExitBB;
...
public:
  ...
  void createLinearVar(toolchain::IRBuilderBase &builder,
                       LLVM::ModuleTranslation &moduleTranslation,
                       toolchain::Value *linearVar, int idx) {
    linearPreconditionVars.push_back(
        builder.CreateAlloca(linearVarTypes[idx], nullptr, ".linear_var"));
    toolchain::Value *linearLoopBodyTemp =
  ...
  void initLinearVar(toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation,
                     toolchain::BasicBlock *loopPreHeader) {
    builder.SetInsertPoint(loopPreHeader->getTerminator());
    for (size_t index = 0; index < linearOrigVal.size(); index++) {
      toolchain::LoadInst *linearVarLoad =
  ...
  void updateLinearVar(toolchain::IRBuilderBase &builder, toolchain::BasicBlock *loopBody,
                       toolchain::Value *loopInductionVar) {
    builder.SetInsertPoint(loopBody->getTerminator());
    for (size_t index = 0; index < linearPreconditionVars.size(); index++) {
      toolchain::Type *linearVarType = linearVarTypes[index];
      toolchain::Value *iv = loopInductionVar;
      toolchain::Value *step = linearSteps[index];
      ...
      if (linearVarType->isIntegerTy()) {
        ...
        toolchain::LoadInst *linearVarStart =
            builder.CreateLoad(linearVarType, linearPreconditionVars[index]);
        toolchain::Value *mulInst = builder.CreateMul(iv, step);
        toolchain::Value *addInst = builder.CreateAdd(linearVarStart, mulInst);
        ...
        toolchain::LoadInst *linearVarStart =
            builder.CreateLoad(linearVarType, linearPreconditionVars[index]);
        toolchain::Value *mulFp = builder.CreateSIToFP(mulInst, linearVarType);
        toolchain::Value *addInst = builder.CreateFAdd(linearVarStart, mulFp);
        builder.CreateStore(addInst, linearLoopBodyTemps[index]);
      } else {
        llvm_unreachable(
  ...
  void splitLinearFiniBB(toolchain::IRBuilderBase &builder,
                         toolchain::BasicBlock *loopExit) {
  ...
  toolchain::OpenMPIRBuilder::InsertPointOrErrorTy
  finalizeLinearVar(toolchain::IRBuilderBase &builder,
                    LLVM::ModuleTranslation &moduleTranslation,
                    toolchain::Value *lastIter) {
    ...
    toolchain::Value *loopLastIterLoad = builder.CreateLoad(
        toolchain::Type::getInt32Ty(builder.getContext()), lastIter);
    toolchain::Value *isLast =
        builder.CreateCmp(toolchain::CmpInst::ICMP_NE, loopLastIterLoad,
                          toolchain::ConstantInt::get(
                              toolchain::Type::getInt32Ty(builder.getContext()), 0));
    // Store the linear variable values to original variables.
    builder.SetInsertPoint(linearLastIterExitBB->getTerminator());
    for (size_t index = 0; index < linearOrigVal.size(); index++) {
      toolchain::LoadInst *linearVarTemp =
    ...
    return moduleTranslation.getOpenMPBuilder()->createBarrier(
        builder.saveIP(), toolchain::omp::OMPD_barrier);
  }

  // Emit stores for linear variables. Useful in case of SIMD
  // construct.
  void emitStoresForLinearVar(toolchain::IRBuilderBase &builder) {
    for (size_t index = 0; index < linearOrigVal.size(); index++) {
      toolchain::LoadInst *linearVarTemp =
          builder.CreateLoad(linearVarTypes[index], linearLoopBodyTemps[index]);
      builder.CreateStore(linearVarTemp, linearOrigVal[index]);
  ...
  void rewriteInPlace(toolchain::IRBuilderBase &builder, const std::string &BBName,
                      size_t varIndex) {
    toolchain::SmallVector<toolchain::User *> users;
    for (toolchain::User *user : linearOrigVal[varIndex]->users())
      users.push_back(user);
    for (auto *user : users) {
      if (auto *userInst = dyn_cast<toolchain::Instruction>(user)) {
...
static LogicalResult checkImplementationStatus(Operation &op) {
  ...
  toolchain::TypeSwitch<Operation &>(op)
...
static LogicalResult handleError(toolchain::Error error, Operation &op) {
  LogicalResult result = success();
  if (error) {
...
template <typename T>
static LogicalResult handleError(toolchain::Expected<T> &result, Operation &op) {
  if (!result)
...
static toolchain::OpenMPIRBuilder::InsertPointTy
findAllocaInsertPoint(toolchain::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation) {
  ...
  if (builder.GetInsertBlock() ==
    ...
    toolchain::BasicBlock *entryBB = toolchain::BasicBlock::Create(
  ...
  toolchain::BasicBlock &funcEntryBlock =
      builder.GetInsertBlock()->getParent()->getEntryBlock();
  return toolchain::OpenMPIRBuilder::InsertPointTy(
...
static toolchain::CanonicalLoopInfo *
...
static toolchain::Expected<toolchain::BasicBlock *> convertOmpOpRegions(
    Region &region, StringRef blockName, toolchain::IRBuilderBase &builder,
    LLVM::ModuleTranslation &moduleTranslation,
    SmallVectorImpl<toolchain::PHINode *> *continuationBlockPHIs = nullptr) {
  bool isLoopWrapper = isa<omp::LoopWrapperInterface>(region.getParentOp());

  toolchain::BasicBlock *continuationBlock =
      splitBB(builder, true, "omp.region.cont");
  toolchain::BasicBlock *sourceBlock = builder.GetInsertBlock();

  toolchain::LLVMContext &llvmContext = builder.getContext();
  for (Block &bb : region) {
  ...
  toolchain::Instruction *sourceTerminator = sourceBlock->getTerminator();
  ...
  SmallVector<toolchain::Type *> continuationBlockPHITypes;
  ...
  if (continuationBlockPHIs) {
    ...
    for (toolchain::Type *ty : continuationBlockPHITypes)
  ...
  for (Block *bb : blocks) {
    ...
    if (failed(
      ...
      return toolchain::make_error<PreviouslyReportedError>();
...
static toolchain::omp::ProcBindKind getProcBindKind(omp::ClauseProcBindKind kind) {
  switch (kind) {
  case omp::ClauseProcBindKind::Close:
    return toolchain::omp::ProcBindKind::OMP_PROC_BIND_close;
  case omp::ClauseProcBindKind::Master:
    return toolchain::omp::ProcBindKind::OMP_PROC_BIND_master;
  case omp::ClauseProcBindKind::Primary:
    return toolchain::omp::ProcBindKind::OMP_PROC_BIND_primary;
  case omp::ClauseProcBindKind::Spread:
    return toolchain::omp::ProcBindKind::OMP_PROC_BIND_spread;
  }
...
static void forwardArgs(LLVM::ModuleTranslation &moduleTranslation,
  ...
  toolchain::SmallVector<std::pair<Value, BlockArgument>> blockArgsPairs;
...
convertOmpMasked(Operation &opInst, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  auto maskedOp = cast<omp::MaskedOp>(opInst);
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  ...
  auto finiCB = [&](InsertPointTy codeGenIP) { return toolchain::Error::success(); };

  toolchain::Value *filterVal = nullptr;
  if (auto filterVar = maskedOp.getFilteredThreadId()) {
    filterVal = moduleTranslation.lookupValue(filterVar);
  } else {
    toolchain::LLVMContext &llvmContext = builder.getContext();
    filterVal =
        toolchain::ConstantInt::get(toolchain::Type::getInt32Ty(llvmContext), /*V=*/0);
  }
  assert(filterVal != nullptr);
  toolchain::OpenMPIRBuilder::LocationDescription ompLoc(builder);
  toolchain::OpenMPIRBuilder::InsertPointOrErrorTy afterIP =
...
static LogicalResult
convertOmpMaster(Operation &opInst, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
...
static LogicalResult
convertOmpCritical(Operation &opInst, toolchain::IRBuilderBase &builder,
                   LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  ...
  toolchain::Constant *hint = nullptr;
  ...
  if (criticalOp.getNameAttr()) {
    ...
    auto criticalDeclareOp =
        SymbolTable::lookupNearestSymbolFrom<omp::CriticalDeclareOp>(criticalOp,
                                                                     symbolRef);
    hint =
        toolchain::ConstantInt::get(toolchain::Type::getInt32Ty(llvmContext),
...
struct PrivateVarsInfo {
  ...
  SmallVector<toolchain::Value *> llvmVars;
...
static LogicalResult inlineConvertOmpRegions(
    Region &region, StringRef blockName, toolchain::IRBuilderBase &builder,
    LLVM::ModuleTranslation &moduleTranslation,
    SmallVectorImpl<toolchain::Value *> *continuationBlockArgs = nullptr) {
  if (region.empty())
    return success();
  ...
  if (region.hasOneBlock()) {
    toolchain::Instruction *potentialTerminator =
    ...
    if (potentialTerminator && potentialTerminator->isTerminator()) {
      toolchain::BasicBlock *block = builder.GetInsertBlock();
      if (block->empty()) {
        // this can happen for really simple reduction init regions e.g.
        // %0 = toolchain.mlir.constant(0 : i32) : i32
        // omp.yield(%0 : i32)
        // because the toolchain.mlir.constant (MLIR op) isn't converted into any
        // toolchain op
  ...
  SmallVector<toolchain::PHINode *> phis;
  toolchain::Expected<toolchain::BasicBlock *> continuationBlock =
  ...
  if (continuationBlockArgs)
    toolchain::append_range(*continuationBlockArgs, phis);
...
using OwningReductionGen =
    std::function<toolchain::OpenMPIRBuilder::InsertPointOrErrorTy(
        toolchain::OpenMPIRBuilder::InsertPointTy, toolchain::Value *, toolchain::Value *,
        toolchain::Value *&)>;
using OwningAtomicReductionGen =
    std::function<toolchain::OpenMPIRBuilder::InsertPointOrErrorTy(
        toolchain::OpenMPIRBuilder::InsertPointTy, toolchain::Type *, toolchain::Value *,
        toolchain::Value *)>;
...
makeReductionGen(omp::DeclareReductionOp decl, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  ...
  OwningReductionGen gen =
    ...
    result = toolchain::getSingleElement(phis);
...
makeAtomicReductionGen(omp::DeclareReductionOp decl,
  ...
  OwningAtomicReductionGen atomicGen =
      [&, decl](toolchain::OpenMPIRBuilder::InsertPointTy insertPoint, toolchain::Type *,
                toolchain::Value *lhs, toolchain::Value *rhs) mutable
      -> toolchain::OpenMPIRBuilder::InsertPointOrErrorTy {
...
makeRefDataPtrGen(omp::DeclareReductionOp decl, toolchain::IRBuilderBase &builder,
  ...
  OwningDataPtrPtrReductionGen refDataPtrGen =
      [&, decl](toolchain::OpenMPIRBuilder::InsertPointTy insertPoint,
                toolchain::Value *byRefVal, toolchain::Value *&result) mutable
      -> toolchain::OpenMPIRBuilder::InsertPointOrErrorTy {
...
static LogicalResult
convertOmpOrdered(Operation &opInst, toolchain::IRBuilderBase &builder,
                  LLVM::ModuleTranslation &moduleTranslation) {
...
/// OpenMPIRBuilder.
static LogicalResult
convertOmpOrderedRegion(Operation &opInst, toolchain::IRBuilderBase &builder,
                        LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
...
struct DeferredStore {
  DeferredStore(toolchain::Value *value, toolchain::Value *address)
...
allocReductionVars(T loop, ArrayRef<BlockArgument> reductionArgs,
                   toolchain::IRBuilderBase &builder,
                   LLVM::ModuleTranslation &moduleTranslation,
                   const toolchain::OpenMPIRBuilder::InsertPointTy &allocaIP,
                   SmallVectorImpl<omp::DeclareReductionOp> &reductionDecls,
                   SmallVectorImpl<toolchain::Value *> &privateReductionVariables,
                   DenseMap<Value, toolchain::Value *> &reductionVariableMap,
                   SmallVectorImpl<DeferredStore> &deferredStores,
                   toolchain::ArrayRef<bool> isByRefs) {
  toolchain::IRBuilderBase::InsertPointGuard guard(builder);
  ...
  for (std::size_t i = 0; i < loop.getNumReductionVars(); ++i) {
    ...
    if (isByRefs[i]) {
      ...
      SmallVector<toolchain::Value *, 1> phis;
      ...
      toolchain::Value *var = builder.CreateAlloca(
          moduleTranslation.convertType(reductionDecls[i].getType()));

      toolchain::Type *ptrTy = builder.getPtrTy();
      toolchain::Value *castVar =
          builder.CreatePointerBitCastOrAddrSpaceCast(var, ptrTy);
      toolchain::Value *castPhi =
...
mapInitializationArgs(T loop, LLVM::ModuleTranslation &moduleTranslation,
  ...
  toolchain::Value *llvmSource = moduleTranslation.lookupValue(mlirSource);
  toolchain::Value *origVal = llvmSource;
...
static void
setInsertPointForPossiblyEmptyBlock(toolchain::IRBuilderBase &builder,
                                    toolchain::BasicBlock *block = nullptr) {
  if (block == nullptr)
...
initReductionVars(OP op, ArrayRef<BlockArgument> reductionArgs,
                  toolchain::IRBuilderBase &builder,
                  LLVM::ModuleTranslation &moduleTranslation,
                  toolchain::BasicBlock *latestAllocaBlock,
                  SmallVectorImpl<omp::DeclareReductionOp> &reductionDecls,
                  SmallVectorImpl<toolchain::Value *> &privateReductionVariables,
                  DenseMap<Value, toolchain::Value *> &reductionVariableMap,
                  toolchain::ArrayRef<bool> isByRef,
                  SmallVectorImpl<DeferredStore> &deferredStores) {
  if (op.getNumReductionVars() == 0)
    return success();

  toolchain::BasicBlock *initBlock = splitBB(builder, true, "omp.reduction.init");
  auto allocaIP = toolchain::IRBuilderBase::InsertPoint(
      latestAllocaBlock, latestAllocaBlock->getTerminator()->getIterator());
  builder.restoreIP(allocaIP);
  SmallVector<toolchain::Value *> byRefVars(op.getNumReductionVars());
...
static void collectReductionInfo(
    T loop, toolchain::IRBuilderBase &builder,
    LLVM::ModuleTranslation &moduleTranslation,
    SmallVectorImpl<omp::DeclareReductionOp> &reductionDecls,
    ...
    const ArrayRef<toolchain::Value *> privateReductionVariables,
    SmallVectorImpl<toolchain::OpenMPIRBuilder::ReductionInfo> &reductionInfos,
  ...
  for (unsigned i = 0; i < numReductions; ++i) {
    toolchain::OpenMPIRBuilder::ReductionGenAtomicCBTy atomicGen = nullptr;
    if (owningAtomicReductionGens[i])
      atomicGen = owningAtomicReductionGens[i];
    toolchain::Value *variable =
    ...
    reductionInfos.push_back(
        {moduleTranslation.convertType(reductionDecls[i].getType()), variable,
         ...
         /*EvaluationKind=*/toolchain::OpenMPIRBuilder::EvalKind::Scalar,
...
static LogicalResult
inlineOmpRegionCleanup(toolchain::SmallVectorImpl<Region *> &cleanupRegions,
                       toolchain::ArrayRef<toolchain::Value *> privateVariables,
                       LLVM::ModuleTranslation &moduleTranslation,
                       toolchain::IRBuilderBase &builder, StringRef regionName,
                       bool shouldLoadCleanupRegionArg = true) {
  for (auto [i, cleanupRegion] : toolchain::enumerate(cleanupRegions)) {
    ...
    toolchain::Value *privateVarValue =
...
static LogicalResult createReductionsAndCleanup(
    OP op, toolchain::IRBuilderBase &builder,
    LLVM::ModuleTranslation &moduleTranslation,
    toolchain::OpenMPIRBuilder::InsertPointTy &allocaIP,
    SmallVectorImpl<omp::DeclareReductionOp> &reductionDecls,
    ArrayRef<toolchain::Value *> privateReductionVariables, ArrayRef<bool> isByRef,
  ...
  SmallVector<toolchain::OpenMPIRBuilder::ReductionInfo, 2> reductionInfos;

  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  // and remove it later.
  toolchain::UnreachableInst *tempTerminator = builder.CreateUnreachable();
  builder.SetInsertPoint(tempTerminator);
  toolchain::OpenMPIRBuilder::InsertPointOrErrorTy contInsertPoint =
  ...
  toolchain::OpenMPIRBuilder::InsertPointOrErrorTy afterIP =
      ompBuilder->createBarrier(*contInsertPoint, toolchain::omp::OMPD_for);
  ...
  toolchain::transform(reductionDecls, std::back_inserter(reductionRegions),
...
static LogicalResult allocAndInitializeReductionVars(
    OP op, ArrayRef<BlockArgument> reductionArgs, toolchain::IRBuilderBase &builder,
    LLVM::ModuleTranslation &moduleTranslation,
    toolchain::OpenMPIRBuilder::InsertPointTy &allocaIP,
    SmallVectorImpl<omp::DeclareReductionOp> &reductionDecls,
    SmallVectorImpl<toolchain::Value *> &privateReductionVariables,
...
/// Return the toolchain::Value * corresponding to the `privateVar` that
...
/// will return a pointer to the toolchain::Value corresponding to the
/// block argument for the mapped descriptor.
static toolchain::Value *
findAssociatedValue(Value privateVar, toolchain::IRBuilderBase &builder,
                    LLVM::ModuleTranslation &moduleTranslation,
                    toolchain::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
  ...
  // boxchar<KIND> and gets lowered to !toolchain.struct<(ptr, i64)>. That is the
...
static toolchain::Expected<toolchain::Value *>
initPrivateVar(toolchain::IRBuilderBase &builder,
               LLVM::ModuleTranslation &moduleTranslation,
               omp::PrivateClauseOp &privDecl, toolchain::Value *nonPrivateVar,
               BlockArgument &blockArg, toolchain::Value *llvmPrivateVar,
               toolchain::BasicBlock *privInitBlock,
               toolchain::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
...
static toolchain::Expected<toolchain::Value *> initPrivateVar(
    ...
    toolchain::Value *llvmPrivateVar, toolchain::BasicBlock *privInitBlock,
    toolchain::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
...
initPrivateVars(toolchain::IRBuilderBase &builder,
                LLVM::ModuleTranslation &moduleTranslation,
  ...
  toolchain::BasicBlock *privInitBlock = splitBB(builder, true, "omp.private.init");
  setInsertPointForPossiblyEmptyBlock(builder, privInitBlock);

  for (auto [idx, zip] : toolchain::enumerate(toolchain::zip_equal(
    ...
    toolchain::Expected<toolchain::Value *> privVarOrErr = initPrivateVar(
...
/// which comes after all of these allocations. toolchain::Value * for each of these
/// private variables are populated in llvmPrivateVars.
static toolchain::Expected<toolchain::BasicBlock *>
allocatePrivateVars(toolchain::IRBuilderBase &builder,
                    LLVM::ModuleTranslation &moduleTranslation,
                    PrivateVarsInfo &privateVarsInfo,
                    const toolchain::OpenMPIRBuilder::InsertPointTy &allocaIP,
                    toolchain::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
  // Allocate private vars
  toolchain::Instruction *allocaTerminator = allocaIP.getBlock()->getTerminator();
  splitBB(toolchain::OpenMPIRBuilder::InsertPointTy(allocaIP.getBlock(),
  ...
  toolchain::DataLayout dataLayout = builder.GetInsertBlock()->getDataLayout();
  toolchain::BasicBlock *afterAllocas = allocaTerminator->getSuccessor(0);
  ...
  for (auto [privDecl, mlirPrivVar, blockArg] :
       toolchain::zip_equal(privateVarsInfo.privatizers, privateVarsInfo.mlirVars,
                       privateVarsInfo.blockArgs)) {
    toolchain::Type *llvmAllocType =
        moduleTranslation.convertType(privDecl.getType());
    builder.SetInsertPoint(allocaIP.getBlock()->getTerminator());
    toolchain::Value *llvmPrivateVar = builder.CreateAlloca(
...
static LogicalResult copyFirstPrivateVars(
    ...
    LLVM::ModuleTranslation &moduleTranslation,
    SmallVectorImpl<toolchain::Value *> &moldVars,
    ArrayRef<toolchain::Value *> llvmPrivateVars,
    SmallVectorImpl<omp::PrivateClauseOp> &privateDecls, bool insertBarrier,
    toolchain::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
  // Apply copy region for firstprivate.
  bool needsFirstprivate =
      toolchain::any_of(privateDecls, [](omp::PrivateClauseOp &privOp) {
  ...
  toolchain::BasicBlock *copyBlock =
      splitBB(builder, /*CreateBranch=*/true, "omp.private.copy");
  setInsertPointForPossiblyEmptyBlock(builder, copyBlock);

  for (auto [decl, moldVar, llvmVar] :
       toolchain::zip_equal(privateDecls, moldVars, llvmPrivateVars)) {
  ...
  if (insertBarrier && !opIsInSingleThread(op)) {
    toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
    toolchain::OpenMPIRBuilder::InsertPointOrErrorTy res =
        ompBuilder->createBarrier(builder.saveIP(), toolchain::omp::OMPD_barrier);
...
static LogicalResult copyFirstPrivateVars(
    ...
    SmallVectorImpl<omp::PrivateClauseOp> &privateDecls, bool insertBarrier,
    toolchain::DenseMap<Value, Value> *mappedPrivateVars = nullptr) {
  toolchain::SmallVector<toolchain::Value *> moldVars(mlirPrivateVars.size());
  toolchain::transform(mlirPrivateVars, moldVars.begin(), [&](mlir::Value mlirVar) {
    // map copyRegion rhs arg
    toolchain::Value *moldVar = findAssociatedValue(
...
static LogicalResult
cleanupPrivateVars(toolchain::IRBuilderBase &builder,
                   LLVM::ModuleTranslation &moduleTranslation, Location loc,
                   SmallVectorImpl<toolchain::Value *> &llvmPrivateVars,
                   SmallVectorImpl<omp::PrivateClauseOp> &privateDecls) {
  // private variable deallocation
  SmallVector<Region *> privateCleanupRegions;
  toolchain::transform(privateDecls, std::back_inserter(privateCleanupRegions),
                  [](omp::PrivateClauseOp privatizer) {
...
static LogicalResult
convertOmpSections(Operation &opInst, toolchain::IRBuilderBase &builder,
                   LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  using StorableBodyGenCallbackTy =
      toolchain::OpenMPIRBuilder::StorableBodyGenCallbackTy;
  ...
  toolchain::ArrayRef<bool> isByRef = getIsByRef(sectionsOp.getReductionByref());
  ...
  SmallVector<toolchain::Value *> privateReductionVariables(
  ...
  for (Operation &op : *sectionsOp.getRegion().begin()) {
    ...
    auto sectionCB = [&sectionsOp, &region, &builder, &moduleTranslation](
      ...
      for (auto [sectionsArg, sectionArg] : toolchain::zip_equal(
               sectionsOp.getRegion().getArguments(), region.getArguments())) {
        toolchain::Value *llvmVal = moduleTranslation.lookupValue(sectionsArg);
        assert(llvmVal);
  ...
  auto privCB = [&](InsertPointTy, InsertPointTy codeGenIP, toolchain::Value &,
                    toolchain::Value &vPtr, toolchain::Value *&replacementValue)
      -> toolchain::OpenMPIRBuilder::InsertPointOrErrorTy {
...
static LogicalResult
convertOmpSingle(omp::SingleOp &singleOp, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  toolchain::OpenMPIRBuilder::LocationDescription ompLoc(builder);
  ...
  toolchain::SmallVector<toolchain::Value *> llvmCPVars;
  toolchain::SmallVector<toolchain::Function *> llvmCPFuncs;
...
static bool teamsReductionContainedInDistribute(omp::TeamsOp teamsOp) {
  auto iface =
      toolchain::cast<mlir::omp::BlockArgOpenMPOpInterface>(teamsOp.getOperation());
  // Check that all uses of the reduction block arg has the same distribute op
  // parent.
  toolchain::SmallVector<mlir::Operation *> debugUses;
...
static LogicalResult
convertOmpTeams(omp::TeamsOp op, toolchain::IRBuilderBase &builder,
                LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  ...
  SmallVector<omp::DeclareReductionOp> reductionDecls;
  SmallVector<toolchain::Value *> privateReductionVariables(numReductionVars);
  toolchain::ArrayRef<bool> isByRef;
  toolchain::OpenMPIRBuilder::InsertPointTy allocaIP =
      findAllocaInsertPoint(builder, moduleTranslation);
  ...
  if (doTeamsReduction) {
    ...
    MutableArrayRef<BlockArgument> reductionArgs =
        toolchain::cast<omp::BlockArgOpenMPOpInterface>(*op).getReductionBlockArgs();
...
buildDependData(std::optional<ArrayAttr> dependKinds, OperandRange dependVars,
                LLVM::ModuleTranslation &moduleTranslation,
                SmallVectorImpl<toolchain::OpenMPIRBuilder::DependData> &dds) {
  if (dependVars.empty())
    return;
  for (auto dep : toolchain::zip(dependVars, dependKinds->getValue())) {
    toolchain::omp::RTLDependenceKindTy type;
    switch (
        cast<mlir::omp::ClauseTaskDependAttr>(std::get<1>(dep)).getValue()) {
    case mlir::omp::ClauseTaskDepend::taskdependin:
      type = toolchain::omp::RTLDependenceKindTy::DepIn;
      break;
    ...
    case mlir::omp::ClauseTaskDepend::taskdependinout:
      type = toolchain::omp::RTLDependenceKindTy::DepInOut;
      break;
    case mlir::omp::ClauseTaskDepend::taskdependmutexinoutset:
      type = toolchain::omp::RTLDependenceKindTy::DepMutexInOutSet;
      break;
    case mlir::omp::ClauseTaskDepend::taskdependinoutset:
      type = toolchain::omp::RTLDependenceKindTy::DepInOutSet;
      break;
    };
    toolchain::Value *depVal = moduleTranslation.lookupValue(std::get<0>(dep));
    toolchain::OpenMPIRBuilder::DependData dd(type, depVal->getType(), depVal);
    dds.emplace_back(dd);
  }
}
...
static void
pushCancelFinalizationCB(SmallVectorImpl<toolchain::BranchInst *> &cancelTerminators,
                         toolchain::IRBuilderBase &llvmBuilder,
                         toolchain::OpenMPIRBuilder &ompBuilder, mlir::Operation *op,
                         toolchain::omp::Directive cancelDirective) {
  auto finiCB = [&](toolchain::OpenMPIRBuilder::InsertPointTy ip) -> toolchain::Error {
    toolchain::IRBuilderBase::InsertPointGuard guard(llvmBuilder);
...
static void
popCancelFinalizationCB(const ArrayRef<toolchain::BranchInst *> cancelTerminators,
                        toolchain::OpenMPIRBuilder &ompBuilder,
                        const toolchain::OpenMPIRBuilder::InsertPointTy &afterIP) {
  ompBuilder.popFinalizationCB();
  toolchain::BasicBlock *constructFini = afterIP.getBlock()->getSinglePredecessor();
  for (toolchain::BranchInst *cancelBranch : cancelTerminators) {
...
public:
  TaskContextStructManager(toolchain::IRBuilderBase &builder,
                           LLVM::ModuleTranslation &moduleTranslation,
  ...
  SmallVector<toolchain::Value *>
  createGEPsToPrivateVars(toolchain::Value *altStructPtr) const;

  /// De-allocate the task context structure.
  void freeStructPtr();

  MutableArrayRef<toolchain::Value *> getLLVMPrivateVarGEPs() {
    return llvmPrivateVarGEPs;
  }

  toolchain::Value *getStructPtr() { return structPtr; }
...
void TaskContextStructManager::generateTaskContextStruct() {
  ...
  structTy = toolchain::StructType::get(moduleTranslation.getLLVMContext(),
                                   privateVarTypes);

  toolchain::DataLayout dataLayout =
      builder.GetInsertBlock()->getModule()->getDataLayout();
  toolchain::Type *intPtrTy = builder.getIntPtrTy(dataLayout);
  toolchain::Constant *allocSize = toolchain::ConstantExpr::getSizeOf(structTy);
...
SmallVector<toolchain::Value *> TaskContextStructManager::createGEPsToPrivateVars(
    toolchain::Value *altStructPtr) const {
  SmallVector<toolchain::Value *> ret;

  // Create GEPs for each struct member
  ret.reserve(privateDecls.size());
  toolchain::Value *zero = builder.getInt32(0);
  unsigned i = 0;
  for (auto privDecl : privateDecls) {
    ...
    toolchain::Value *iVal = builder.getInt32(i);
    toolchain::Value *gep = builder.CreateGEP(structTy, altStructPtr, {zero, iVal});
    ret.push_back(gep);
    i += 1;
  }
  return ret;
...
static LogicalResult
convertOmpTaskOp(omp::TaskOp taskOp, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  ...
  for (auto [privDecl, mlirPrivVar, blockArg, llvmPrivateVarAlloc] :
    ...
    toolchain::Expected<toolchain::Value *> privateVarOrErr =
  ...
  auto bodyCB = [&](InsertPointTy allocaIP,
                    InsertPointTy codegenIP) -> toolchain::Error {
    // Save the alloca insertion point on ModuleTranslation stack for use in
    // nested regions.
    LLVM::ModuleTranslation::SaveStack<OpenMPAllocaStackFrame> frame(
        moduleTranslation, allocaIP);
    ...
    toolchain::BasicBlock *privInitBlock = nullptr;
    privateVarsInfo.llvmVars.resize(privateVarsInfo.blockArgs.size());
    for (auto [i, zip] : toolchain::enumerate(toolchain::zip_equal(
      ...
      toolchain::Expected<toolchain::Value *> privateVarOrError =
          initPrivateVar(builder, moduleTranslation, privDecl, mlirPrivVar,
    ...
    for (auto [i, llvmPrivVar] :
         toolchain::enumerate(taskStructMgr.getLLVMPrivateVarGEPs())) {
      if (!llvmPrivVar) {
    ...
    for (auto [blockArg, llvmPrivateVar, privateDecl] :
         toolchain::zip_equal(privateVarsInfo.blockArgs, privateVarsInfo.llvmVars,
                         privateVarsInfo.privatizers)) {
  ...
  toolchain::OpenMPIRBuilder &ompBuilder = *moduleTranslation.getOpenMPBuilder();
  SmallVector<toolchain::BranchInst *> cancelTerminators;
  ...
  pushCancelFinalizationCB(cancelTerminators, builder, ompBuilder, taskOp,
                           toolchain::omp::Directive::OMPD_taskgroup);

  SmallVector<toolchain::OpenMPIRBuilder::DependData> dds;
...
static LogicalResult
convertOmpTaskloopOp(Operation &opInst, toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  ...
  SmallVector<toolchain::Value *> llvmFirstPrivateVars;
  ...
  assert(builder.GetInsertPoint() == builder.GetInsertBlock()->end());
  toolchain::BasicBlock *taskloopStartBlock = toolchain::BasicBlock::Create(
      builder.getContext(), "omp.taskloop.start",
      /*Parent=*/builder.GetInsertBlock()->getParent());
  toolchain::Instruction *branchToTaskloopStartBlock =
  ...
  auto taskDupCB = [&](InsertPointTy allocaIP, InsertPointTy codegenIP,
                       toolchain::Value *destPtr, toolchain::Value *srcPtr)
      -> toolchain::Expected<toolchain::IRBuilderBase::InsertPoint> {
    toolchain::IRBuilderBase::InsertPointGuard guard(builder);
    builder.restoreIP(codegenIP);

    toolchain::Type *ptrTy =
  ...
  auto loopInfo = [&]() -> toolchain::Expected<toolchain::CanonicalLoopInfo *> {
    toolchain::CanonicalLoopInfo *loopInfo = findCurrentLoopInfo(moduleTranslation);
    return loopInfo;
  };

  toolchain::Value *ifCond = nullptr;
  toolchain::Value *grainsize = nullptr;
  int sched = 0; // default
  ...
  toolchain::OpenMPIRBuilder::TaskDupCallbackTy taskDupOrNull = nullptr;
...
static LogicalResult
convertOmpTaskgroupOp(omp::TaskgroupOp tgOp, toolchain::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
...
static LogicalResult
convertOmpTaskwaitOp(omp::TaskwaitOp twOp, toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation) {
...
static LogicalResult
convertOmpWsloop(Operation &opInst, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  toolchain::ArrayRef<bool> isByRef = getIsByRef(wsloopOp.getReductionByref());
  ...
  toolchain::Value *step = moduleTranslation.lookupValue(loopOp.getLoopSteps()[0]);
  toolchain::Type *ivType = step->getType();
  toolchain::Value *chunk = nullptr;
  if (wsloopOp.getScheduleChunk()) {
    toolchain::Value *chunkVar =
        moduleTranslation.lookupValue(wsloopOp.getScheduleChunk());
    chunk = builder.CreateSExtOrTrunc(chunkVar, ivType);
  }

  omp::DistributeOp distributeOp = nullptr;
  toolchain::Value *distScheduleChunk = nullptr;
  bool hasDistSchedule = false;
  if (toolchain::isa_and_present<omp::DistributeOp>(opInst.getParentOp())) {
    ...
    if (distributeOp.getDistScheduleChunkSize()) {
      toolchain::Value *chunkVar = moduleTranslation.lookupValue(
  ...
  toolchain::Expected<toolchain::BasicBlock *> afterAllocas = allocatePrivateVars(
  ...
  // worksharing loop.
  toolchain::omp::WorksharingLoopType workshareLoopType =
      toolchain::isa_and_present<omp::DistributeOp>(opInst.getParentOp())
          ? toolchain::omp::WorksharingLoopType::DistributeForStaticLoop
          : toolchain::omp::WorksharingLoopType::ForStaticLoop;

  SmallVector<toolchain::BranchInst *> cancelTerminators;
  pushCancelFinalizationCB(cancelTerminators, builder, *ompBuilder, wsloopOp,
                           toolchain::omp::Directive::OMPD_for);
  ...
  if (!wsloopOp.getLinearVars().empty()) {
    ...
    for (auto [idx, linearVar] : toolchain::enumerate(wsloopOp.getLinearVars()))
  ...
  toolchain::Expected<toolchain::BasicBlock *> regionBlock = convertOmpOpRegions(
  ...
  if (!wsloopOp.getLinearVars().empty()) {
    ...
    toolchain::OpenMPIRBuilder::InsertPointOrErrorTy afterBarrierIP =
  ...
  toolchain::OpenMPIRBuilder::InsertPointOrErrorTy wsloopIP =
  ...
  if (!wsloopOp.getLinearVars().empty()) {
    toolchain::OpenMPIRBuilder::InsertPointTy oldIP = builder.saveIP();
...
static LogicalResult
convertOmpParallel(omp::ParallelOp opInst, toolchain::IRBuilderBase &builder,
                   LLVM::ModuleTranslation &moduleTranslation) {
  using InsertPointTy = toolchain::OpenMPIRBuilder::InsertPointTy;
  ...
  auto privCB = [](InsertPointTy allocaIP, InsertPointTy codeGenIP,
                   toolchain::Value &, toolchain::Value &val, toolchain::Value *&replVal) {
  ...
  // TODO: Perform finalization actions for variables. This has to be
  // called for variables which have destructors/finalizers.
  auto finiCB = [&](InsertPointTy codeGenIP) -> toolchain::Error {
    ...
    toolchain::transform(reductionDecls, std::back_inserter(reductionCleanupRegions),
                    [](omp::DeclareReductionOp reductionDecl) {
                      return &reductionDecl.getCleanupRegion();
                    });
    if (failed(inlineOmpRegionCleanup(
    ...
    if (isCancellable) {
      auto IPOrErr = ompBuilder->createBarrier(
          toolchain::OpenMPIRBuilder::LocationDescription(builder),
          toolchain::omp::Directive::OMPD_unknown,
  ...
  if (auto ifVar = opInst.getIfExpr())
    ifCond = moduleTranslation.lookupValue(ifVar);
  toolchain::Value *numThreads = nullptr;
  if (auto numThreadsVar = opInst.getNumThreads())
    numThreads = moduleTranslation.lookupValue(numThreadsVar);
  auto pbKind = toolchain::omp::OMP_PROC_BIND_default;
...
/// Convert Order attribute to toolchain::omp::OrderKind.
static toolchain::omp::OrderKind
convertOrderKind(std::optional<omp::ClauseOrderKind> o) {
  if (!o)
    return toolchain::omp::OrderKind::OMP_ORDER_unknown;
  switch (*o) {
  case omp::ClauseOrderKind::Concurrent:
    return toolchain::omp::OrderKind::OMP_ORDER_concurrent;
  }
...
static LogicalResult
convertOmpSimd(Operation &opInst, toolchain::IRBuilderBase &builder,
               LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  toolchain::ArrayRef<bool> isByRef = getIsByRef(simdOp.getReductionByref());
  ...
  if (!simdOp.getLinearVars().empty()) {
    ...
    for (mlir::Attribute linearVarType : linearVarTypes)
      linearClauseProcessor.registerType(moduleTranslation, linearVarType);
    for (auto [idx, linearVar] : toolchain::enumerate(simdOp.getLinearVars())) {
      bool isImplicit = false;
      for (auto [mlirPrivVar, llvmPrivateVar] : toolchain::zip_equal(
               privateVarsInfo.mlirVars, privateVarsInfo.llvmVars)) {
        // If the linear variable is implicit, reuse the already
        // existing toolchain::Value
  ...
  toolchain::ConstantInt *simdlen = nullptr;
  if (std::optional<uint64_t> simdlenVar = simdOp.getSimdlen())
    simdlen = builder.getInt64(simdlenVar.value());

  toolchain::ConstantInt *safelen = nullptr;
  if (std::optional<uint64_t> safelenVar = simdOp.getSafelen())
    safelen = builder.getInt64(safelenVar.value());

  toolchain::MapVector<toolchain::Value *, toolchain::Value *> alignedVars;
  toolchain::omp::OrderKind order = convertOrderKind(simdOp.getOrder());

  toolchain::BasicBlock *sourceBlock = builder.GetInsertBlock();
  std::optional<ArrayAttr> alignmentValues = simdOp.getAlignments();
  mlir::OperandRange operands = simdOp.getAlignedVars();
  for (size_t i = 0; i < operands.size(); ++i) {
    toolchain::Value *alignment = nullptr;
    toolchain::Value *llvmVal = moduleTranslation.lookupValue(operands[i]);
    toolchain::Type *ty = llvmVal->getType();
  ...
  for (auto [i, tuple] : toolchain::enumerate(
           toolchain::zip(reductionDecls, isByRef, simdOp.getReductionVars(),
    ...
    toolchain::Value *originalVariable = moduleTranslation.lookupValue(reductionVar);
    toolchain::Type *reductionType = moduleTranslation.convertType(decl.getType());

    // We have one less load for by-ref case because that load is now inside of
    // the reduction region.
    toolchain::Value *redValue = originalVariable;
    if (!byRef)
      redValue =
          builder.CreateLoad(reductionType, redValue, "red.value." + Twine(i));
    toolchain::Value *privateRedValue = builder.CreateLoad(
        reductionType, privateReductionVar, "red.private.value." + Twine(i));
    toolchain::Value *reduced;
...
static LogicalResult
convertOmpLoopNest(Operation &opInst, toolchain::IRBuilderBase &builder,
                   LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  SmallVector<toolchain::CanonicalLoopInfo *> loopInfos;
  SmallVector<toolchain::OpenMPIRBuilder::InsertPointTy> bodyInsertPoints;
  auto bodyGen = [&](toolchain::OpenMPIRBuilder::InsertPointTy ip,
                     toolchain::Value *iv) -> toolchain::Error {
  ...
  for (unsigned i = 0, e = loopOp.getNumLoops(); i < e; ++i) {
    toolchain::Value *lowerBound =
        moduleTranslation.lookupValue(loopOp.getLoopLowerBounds()[i]);
    toolchain::Value *upperBound =
        moduleTranslation.lookupValue(loopOp.getLoopUpperBounds()[i]);
    toolchain::Value *step = moduleTranslation.lookupValue(loopOp.getLoopSteps()[i]);

    // Make sure loop trip count are emitted in the preheader of the outermost
    // loop at the latest so that they are all available for the new collapsed
    // loop will be created below.
    toolchain::OpenMPIRBuilder::LocationDescription loc = ompLoc;
    toolchain::OpenMPIRBuilder::InsertPointTy computeIP = ompLoc.IP;
    if (i != 0) {
      loc = toolchain::OpenMPIRBuilder::LocationDescription(bodyInsertPoints.back(),
                                                       ompLoc.DL);
      computeIP = loopInfos.front()->getPreheaderIP();
    }

    toolchain::Expected<toolchain::CanonicalLoopInfo *> loopResult =
  ...
  toolchain::OpenMPIRBuilder::InsertPointTy afterIP =
      loopInfos.front()->getAfterIP();

  // Do tiling.
  if (const auto &tiles = loopOp.getTileSizes()) {
    toolchain::Type *ivType = loopInfos.front()->getIndVarType();
    SmallVector<toolchain::Value *> tileSizes;

    for (auto tile : tiles.value()) {
      toolchain::Value *tileVal = toolchain::ConstantInt::get(ivType, tile);
      tileSizes.push_back(tileVal);
    }

    std::vector<toolchain::CanonicalLoopInfo *> newLoops =
        ompBuilder->tileLoops(ompLoc.DL, loopInfos, tileSizes);

    // Update afterIP to get the correct insertion point after
    // tiling.
    toolchain::BasicBlock *afterBB = newLoops.front()->getAfter();
    toolchain::BasicBlock *afterAfterBB = afterBB->getSingleSuccessor();
  ...
  SmallVector<toolchain::CanonicalLoopInfo *> collapseLoopInfos(
...
static LogicalResult
convertOmpCanonicalLoopOp(omp::CanonicalLoopOp op, toolchain::IRBuilderBase &builder,
                          LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();

  toolchain::OpenMPIRBuilder::LocationDescription loopLoc(builder);
  Value loopIV = op.getInductionVar();
  Value loopTC = op.getTripCount();

  toolchain::Value *llvmTC = moduleTranslation.lookupValue(loopTC);

  toolchain::Expected<toolchain::CanonicalLoopInfo *> llvmOrError =
      ompBuilder->createCanonicalLoop(
          loopLoc,
          [&](toolchain::OpenMPIRBuilder::InsertPointTy ip, toolchain::Value *llvmIV) {
            // Register the mapping of MLIR induction variable to LLVM-IR
            // induction variable
            moduleTranslation.mapValue(loopIV, llvmIV);

            builder.restoreIP(ip);
            toolchain::Expected<toolchain::BasicBlock *> bodyGenStatus =
          ...
          llvmTC, "omp.loop");
  if (!llvmOrError)
    return op.emitError(toolchain::toString(llvmOrError.takeError()));

  toolchain::CanonicalLoopInfo *llvmCLI = *llvmOrError;
  toolchain::IRBuilderBase::InsertPoint afterIP = llvmCLI->getAfterIP();
  builder.restoreIP(afterIP);
...
/// OpenMPIRBuilder.
static LogicalResult
applyUnrollHeuristic(omp::UnrollHeuristicOp op, toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  toolchain::CanonicalLoopInfo *consBuilderCLI =
      moduleTranslation.lookupOMPLoop(applyee);
  toolchain::OpenMPIRBuilder::LocationDescription loc(builder);
...
/// OpenMPIRBuilder.
static LogicalResult applyTile(omp::TileOp op, toolchain::IRBuilderBase &builder,
                               LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  toolchain::OpenMPIRBuilder::LocationDescription loc(builder);

  SmallVector<toolchain::CanonicalLoopInfo *> translatedLoops;
  SmallVector<toolchain::Value *> translatedSizes;

  for (Value size : op.getSizes()) {
    toolchain::Value *translatedSize = moduleTranslation.lookupValue(size);
...
/// Convert an Atomic Ordering attribute to toolchain::AtomicOrdering.
static toolchain::AtomicOrdering
convertAtomicOrdering(std::optional<omp::ClauseMemoryOrderKind> ao) {
  if (!ao)
    return toolchain::AtomicOrdering::Monotonic; // Default Memory Ordering
...
static LogicalResult
convertOmpAtomicRead(Operation &opInst, toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation) {
  ...
  toolchain::AtomicOrdering AO = convertAtomicOrdering(readOp.getMemoryOrder());
  toolchain::Value *x = moduleTranslation.lookupValue(readOp.getX());
  toolchain::Value *v = moduleTranslation.lookupValue(readOp.getV());

  toolchain::Type *elementType =
      moduleTranslation.convertType(readOp.getElementType());

  toolchain::OpenMPIRBuilder::AtomicOpValue V = {v, elementType, false, false};
  toolchain::OpenMPIRBuilder::AtomicOpValue X = {x, elementType, false, false};
...
static LogicalResult
convertOmpAtomicWrite(Operation &opInst, toolchain::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation) {
  ...
  toolchain::OpenMPIRBuilder::LocationDescription ompLoc(builder);
  toolchain::AtomicOrdering ao = convertAtomicOrdering(writeOp.getMemoryOrder());
  toolchain::Value *expr = moduleTranslation.lookupValue(writeOp.getExpr());
  toolchain::Value *dest = moduleTranslation.lookupValue(writeOp.getX());
  toolchain::Type *ty = moduleTranslation.convertType(writeOp.getExpr().getType());
  toolchain::OpenMPIRBuilder::AtomicOpValue x = {dest, ty, /*isSigned=*/false,
                                            /*isVolatile=*/false};
  builder.restoreIP(
...
static toolchain::AtomicRMWInst::BinOp convertBinOpToAtomic(Operation &op) {
  return toolchain::TypeSwitch<Operation *, toolchain::AtomicRMWInst::BinOp>(&op)
      .Case([&](LLVM::AddOp) { return toolchain::AtomicRMWInst::BinOp::Add; })
      .Case([&](LLVM::SubOp) { return toolchain::AtomicRMWInst::BinOp::Sub; })
      .Case([&](LLVM::AndOp) { return toolchain::AtomicRMWInst::BinOp::And; })
      .Case([&](LLVM::OrOp) { return toolchain::AtomicRMWInst::BinOp::Or; })
      .Case([&](LLVM::XOrOp) { return toolchain::AtomicRMWInst::BinOp::Xor; })
      .Case([&](LLVM::UMaxOp) { return toolchain::AtomicRMWInst::BinOp::UMax; })
      .Case([&](LLVM::UMinOp) { return toolchain::AtomicRMWInst::BinOp::UMin; })
      .Case([&](LLVM::FAddOp) { return toolchain::AtomicRMWInst::BinOp::FAdd; })
      .Case([&](LLVM::FSubOp) { return toolchain::AtomicRMWInst::BinOp::FSub; })
      .Default(toolchain::AtomicRMWInst::BinOp::BAD_BINOP);
}
...
convertOmpAtomicUpdate(omp::AtomicUpdateOp &opInst,
  ...
  toolchain::AtomicRMWInst::BinOp binop;
  mlir::Value mlirExpr;
  toolchain::Value *llvmExpr = nullptr;
  toolchain::Value *llvmX = nullptr;
  toolchain::Type *llvmXElementType = nullptr;
  if (innerOpList.size() == 2) {
    ...
    if (!toolchain::is_contained(innerOp.getOperands(),
    ...
    binop = toolchain::AtomicRMWInst::BinOp::BAD_BINOP;
  }
  ...
  toolchain::OpenMPIRBuilder::AtomicOpValue llvmAtomicX = {llvmX, llvmXElementType,
                                                      /*isSigned=*/false,
                                                      /*isVolatile=*/false};

  toolchain::AtomicOrdering atomicOrdering =
      convertAtomicOrdering(opInst.getMemoryOrder());

  // Generate update code.
  auto updateFn =
      [&opInst, &moduleTranslation](
          toolchain::Value *atomicx,
          toolchain::IRBuilder<> &builder) -> toolchain::Expected<toolchain::Value *> {
...
convertOmpAtomicCapture(omp::AtomicCaptureOp atomicCaptureOp,
  ...
  toolchain::AtomicRMWInst::BinOp binop = toolchain::AtomicRMWInst::BinOp::BAD_BINOP;
  ...
  toolchain::Value *llvmExpr = moduleTranslation.lookupValue(mlirExpr);
  toolchain::Value *llvmX =
  ...
  toolchain::Type *llvmXElementType = moduleTranslation.convertType(
      atomicCaptureOp.getAtomicReadOp().getElementType());
  toolchain::OpenMPIRBuilder::AtomicOpValue llvmAtomicX = {llvmX, llvmXElementType,
                                                      /*isSigned=*/false,
                                                      /*isVolatile=*/false};
  toolchain::OpenMPIRBuilder::AtomicOpValue llvmAtomicV = {llvmV, llvmXElementType,
                                                      /*isSigned=*/false,
                                                      /*isVolatile=*/false};
...
static toolchain::omp::Directive convertCancellationConstructType(
    omp::ClauseCancellationConstructType directive) {
  switch (directive) {
  case omp::ClauseCancellationConstructType::Loop:
    return toolchain::omp::Directive::OMPD_for;
  case omp::ClauseCancellationConstructType::Parallel:
    return toolchain::omp::Directive::OMPD_parallel;
  case omp::ClauseCancellationConstructType::Sections:
    return toolchain::omp::Directive::OMPD_sections;
  case omp::ClauseCancellationConstructType::Taskgroup:
    return toolchain::omp::Directive::OMPD_taskgroup;
  }
  llvm_unreachable("Unhandled cancellation construct type");
}

static LogicalResult
convertOmpCancel(omp::CancelOp op, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  ...
  toolchain::omp::Directive cancelledDirective =
...
/// OpenMPIRBuilder.
static LogicalResult
convertOmpThreadprivate(Operation &opInst, toolchain::IRBuilderBase &builder,
                        LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder::LocationDescription ompLoc(builder);
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  toolchain::GlobalValue *globalValue = moduleTranslation.lookupGlobal(global);

  if (!ompBuilder->Config.isTargetDevice()) {
    toolchain::Type *type = globalValue->getValueType();
    toolchain::TypeSize typeSize =
        builder.GetInsertBlock()->getModule()->getDataLayout().getTypeStoreSize(
            type);
    toolchain::ConstantInt *size = builder.getInt64(typeSize.getFixedValue());
    toolchain::Value *callInst = ompBuilder->createCachedThreadPrivate(
...
static toolchain::OffloadEntriesInfoManager::OMPTargetDeviceClauseKind
convertToDeviceClauseKind(mlir::omp::DeclareTargetDeviceType deviceClause) {
  switch (deviceClause) {
  case mlir::omp::DeclareTargetDeviceType::host:
    return toolchain::OffloadEntriesInfoManager::OMPTargetDeviceClauseHost;
    break;
  case mlir::omp::DeclareTargetDeviceType::nohost:
    return toolchain::OffloadEntriesInfoManager::OMPTargetDeviceClauseNoHost;
    break;
  case mlir::omp::DeclareTargetDeviceType::any:
    return toolchain::OffloadEntriesInfoManager::OMPTargetDeviceClauseAny;
    break;
  }
  llvm_unreachable("unhandled device clause");
}

static toolchain::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryKind
convertToCaptureClauseKind(
    mlir::omp::DeclareTargetCaptureClause captureClause) {
  switch (captureClause) {
  case mlir::omp::DeclareTargetCaptureClause::to:
    return toolchain::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryTo;
  case mlir::omp::DeclareTargetCaptureClause::link:
    return toolchain::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryLink;
  case mlir::omp::DeclareTargetCaptureClause::enter:
    return toolchain::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryEnter;
  case mlir::omp::DeclareTargetCaptureClause::none:
    return toolchain::OffloadEntriesInfoManager::OMPTargetGlobalVarEntryNone;
  }
...
static toolchain::SmallString<64>
getDeclareTargetRefPtrSuffix(LLVM::GlobalOp globalOp,
                             toolchain::OpenMPIRBuilder &ompBuilder) {
  toolchain::SmallString<64> suffix;
  toolchain::raw_svector_ostream os(suffix);
  if (globalOp.getVisibility() == mlir::SymbolTable::Visibility::Private) {
    ...
    auto fileInfoCallBack = [&loc]() {
      return std::pair<std::string, uint64_t>(
          toolchain::StringRef(loc.getFilename()), loc.getLine());
    };

    auto vfs = toolchain::vfs::getRealFileSystem();
    os << toolchain::format(
        "_%x",
...
struct MapInfosTy : toolchain::OpenMPIRBuilder::MapInfosTy {
  SmallVector<Operation *, 4> Mappers;

  /// Append arrays in \a CurInfo.
  void append(MapInfosTy &curInfo) {
    Mappers.append(curInfo.Mappers.begin(), curInfo.Mappers.end());
    toolchain::OpenMPIRBuilder::MapInfosTy::append(curInfo);
  }
};
// A small helper structure to contain data gathered
// for map lowering and coalese it into one area and
// avoiding extra computations such as searches in the
// toolchain module for lowered mapped variables or checking
// if something is declare target (and retrieving the
// value) more than neccessary.
struct MapInfoData : MapInfosTy {
  toolchain::SmallVector<bool, 4> IsDeclareTarget;
  toolchain::SmallVector<bool, 4> IsAMember;
  // Identify if mapping was added by mapClause or use_device clauses.
  toolchain::SmallVector<bool, 4> IsAMapping;
  toolchain::SmallVector<mlir::Operation *, 4> MapClause;
  toolchain::SmallVector<toolchain::Value *, 4> OriginalValue;
  // Stripped off array/pointer to get the underlying
  // element type
  toolchain::SmallVector<toolchain::Type *, 4> BaseType;
...
static TargetDirectiveEnumTy getTargetDirectiveEnumTyFromOp(Operation *op) {
  return toolchain::TypeSwitch<Operation *, TargetDirectiveEnumTy>(op)
...
static uint64_t getArrayElementSizeInBits(LLVM::LLVMArrayType arrTy,
                                          DataLayout &dl) {
  if (auto nestedArrTy = toolchain::dyn_cast_if_present<LLVM::LLVMArrayType>(
...
// CGOpenMPRuntime.cpp.
static toolchain::Value *getSizeInBytes(DataLayout &dl, const mlir::Type &type,
                                   Operation *clauseOp,
                                   toolchain::Value *basePointer,
                                   toolchain::Type *baseType,
                                   toolchain::IRBuilderBase &builder,
                                   LLVM::ModuleTranslation &moduleTranslation) {
  if (auto memberClause =
    ...
    if (!memberClause.getBounds().empty()) {
      toolchain::Value *elementCount = builder.getInt64(1);
      ...
      if (auto arrTy = toolchain::dyn_cast_if_present<LLVM::LLVMArrayType>(type))
...
static toolchain::omp::OpenMPOffloadMappingFlags
convertClauseMapFlags(omp::ClauseMapFlags mlirFlags) {
  ...
  toolchain::omp::OpenMPOffloadMappingFlags mapType =
      toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_NONE;

  if (mapTypeToBool(omp::ClauseMapFlags::to))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TO;

  if (mapTypeToBool(omp::ClauseMapFlags::from))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_FROM;

  if (mapTypeToBool(omp::ClauseMapFlags::always))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_ALWAYS;

  if (mapTypeToBool(omp::ClauseMapFlags::del))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_DELETE;

  if (mapTypeToBool(omp::ClauseMapFlags::return_param))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_RETURN_PARAM;

  if (mapTypeToBool(omp::ClauseMapFlags::priv))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_PRIVATE;

  if (mapTypeToBool(omp::ClauseMapFlags::literal))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_LITERAL;

  if (mapTypeToBool(omp::ClauseMapFlags::implicit))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_IMPLICIT;

  if (mapTypeToBool(omp::ClauseMapFlags::close))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_CLOSE;

  if (mapTypeToBool(omp::ClauseMapFlags::present))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_PRESENT;

  if (mapTypeToBool(omp::ClauseMapFlags::ompx_hold))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_OMPX_HOLD;

  if (mapTypeToBool(omp::ClauseMapFlags::attach))
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_ATTACH;

  if (mapTypeToBool(omp::ClauseMapFlags::is_device_ptr)) {
    mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TARGET_PARAM;
    if (!hasExplicitMap)
      mapType |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_LITERAL;
  }
...
static void collectMapDataFromMapOperands(
    ...
    toolchain::IRBuilderBase &builder, ArrayRef<Value> useDevPtrOperands = {},
  ...
  for (Value mapValue : mapVars) {
    ...
    mapData.DevicePointers.push_back(toolchain::OpenMPIRBuilder::DeviceInfoTy::None);
  ...
  auto findMapInfo = [&mapData](toolchain::Value *val,
                                toolchain::OpenMPIRBuilder::DeviceInfoTy devInfoTy) {
    unsigned index = 0;
    bool found = false;
    for (toolchain::Value *basePtr : mapData.OriginalValue) {
      if (basePtr == val && mapData.IsAMapping[index]) {
        found = true;
        mapData.Types[index] |=
            toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_RETURN_PARAM;
  ...
  auto addDevInfos = [&](const toolchain::ArrayRef<Value> &useDevOperands,
                         toolchain::OpenMPIRBuilder::DeviceInfoTy devInfoTy) {
    for (Value mapValue : useDevOperands) {
      auto mapOp = cast<omp::MapInfoOp>(mapValue.getDefiningOp());
      Value offloadPtr =
          mapOp.getVarPtrPtr() ? mapOp.getVarPtrPtr() : mapOp.getVarPtr();
      toolchain::Value *origValue = moduleTranslation.lookupValue(offloadPtr);
  ...
  addDevInfos(useDevAddrOperands, toolchain::OpenMPIRBuilder::DeviceInfoTy::Address);
  addDevInfos(useDevPtrOperands, toolchain::OpenMPIRBuilder::DeviceInfoTy::Pointer);
  ...
  for (Value mapValue : hasDevAddrOperands) {
    ...
    auto mapTypeAlways = toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_ALWAYS;
    bool isDevicePtr =
    ...
    mapData.MapClause.push_back(mapOp.getOperation());
    if (toolchain::to_underlying(mapType & mapTypeAlways)) {
      ...
      mapData.Types.push_back(
          isDevicePtr ? mapType
                      : toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_LITERAL);
      mapData.Mappers.push_back(nullptr);
    }
    mapData.Names.push_back(LLVM::createMappingInformation(
        mapOp.getLoc(), *moduleTranslation.getOpenMPBuilder()));
    mapData.DevicePointers.push_back(
        isDevicePtr ? toolchain::OpenMPIRBuilder::DeviceInfoTy::Pointer
                    : toolchain::OpenMPIRBuilder::DeviceInfoTy::Address);
    mapData.IsAMapping.push_back(false);
...
static int getMapDataMemberIdx(MapInfoData &mapData, omp::MapInfoOp memberOp) {
  auto *res = toolchain::find(mapData.MapClause, memberOp);
  assert(res != mapData.MapClause.end() &&
         "MapInfoOp for member not found in MapData, cannot return index");
  return std::distance(mapData.MapClause.begin(), res);
}

static void sortMapIndices(toolchain::SmallVectorImpl<size_t> &indices,
                           omp::MapInfoOp mapInfo) {
  ArrayAttr indexAttr = mapInfo.getMembersIndexAttr();
  toolchain::SmallVector<size_t> occludedChildren;
  toolchain::sort(
      indices.begin(), indices.end(), [&](const size_t a, const size_t b) {
        ...
        for (auto it : toolchain::zip(memberIndicesA, memberIndicesB)) {
...
static omp::MapInfoOp getFirstOrLastMappedMemberPtr(omp::MapInfoOp mapInfo,
  ...
  toolchain::SmallVector<size_t> indices(indexAttr.size());
  std::iota(indices.begin(), indices.end(), 0);
  sortMapIndices(indices, mapInfo);
  return toolchain::cast<omp::MapInfoOp>(
...
static std::vector<toolchain::Value *>
calculateBoundsOffset(LLVM::ModuleTranslation &moduleTranslation,
                      toolchain::IRBuilderBase &builder, bool isArrayTy,
                      OperandRange bounds) {
  std::vector<toolchain::Value *> idx;
  ...
  if (isArrayTy) {
    ...
    // different formats.
    std::vector<toolchain::Value *> dimensionIndexSizeOffset;
    for (int i = bounds.size() - 1; i >= 0; --i) {
      if (auto boundOp = dyn_cast_if_present<omp::MapBoundsOp>(
              bounds[i].getDefiningOp())) {
...
static void getAsIntegers(ArrayAttr values, toolchain::SmallVector<int64_t> &ints) {
  toolchain::transform(values, std::back_inserter(ints), [](Attribute value) {
...
static void
getOverlappedMembers(toolchain::SmallVectorImpl<size_t> &overlapMapDataIdxs,
  ...
  toolchain::SmallVector<std::pair<int, ArrayAttr>> memberByIndex;
  ArrayAttr indexAttr = parentOp.getMembersIndexAttr();
  for (auto [memIndex, indicesAttr] : toolchain::enumerate(indexAttr))
  ...
  toolchain::sort(memberByIndex.begin(), memberByIndex.end(),
  ...
  // [0,2].. etc.
  toolchain::SmallVector<std::pair<int, ArrayAttr>> skipList;
  for (auto v : memberByIndex) {
    toolchain::SmallVector<int64_t> vArr(v.second.size());
    getAsIntegers(v.second, vArr);
    skipList.push_back(
        *std::find_if(memberByIndex.begin(), memberByIndex.end(), [&](auto x) {
          if (v == x)
            return false;
          toolchain::SmallVector<int64_t> xArr(x.second.size());
...
static toolchain::omp::OpenMPOffloadMappingFlags mapParentWithMembers(
    LLVM::ModuleTranslation &moduleTranslation, toolchain::IRBuilderBase &builder,
    toolchain::OpenMPIRBuilder &ompBuilder, DataLayout &dl, MapInfosTy &combinedInfo,
  ...
  auto parentClause =
      toolchain::cast<omp::MapInfoOp>(mapData.MapClause[mapDataIndex]);
  toolchain::Value *lowAddr, *highAddr;
  ...
  toolchain::Value *size = builder.CreateIntCast(
      builder.CreatePtrDiff(builder.getInt8Ty(), highAddr, lowAddr),
      builder.getInt64Ty(),
      /*isSigned=*/false);
  combinedInfo.Sizes.push_back(size);

  toolchain::omp::OpenMPOffloadMappingFlags memberOfFlag =
  ...
  if (!parentClause.getPartialMap()) {
    ...
    toolchain::omp::OpenMPOffloadMappingFlags mapFlag = mapData.Types[mapDataIndex];
    bool hasMapClose = (toolchain::omp::OpenMPOffloadMappingFlags(mapFlag) &
                        toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_CLOSE) ==
                       toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_CLOSE;
    ...
    if (targetDirective == TargetDirectiveEnumTy::TargetUpdate || hasMapClose) {
      ...
      toolchain::SmallVector<size_t> overlapIdxs;
      ...
      for (auto v : overlapIdxs) {
        ...
        lowAddr = builder.CreateConstGEP1_32(
            checkIfPointerMap(toolchain::cast<omp::MapInfoOp>(
...
static void processMapMembersWithParent(
  ...
  for (auto mappedMembers : parentClause.getMembers()) {
    auto memberClause =
        toolchain::cast<omp::MapInfoOp>(mappedMembers.getDefiningOp());
    ...
    if (checkIfPointerMap(memberClause)) {
      ...
      mapFlag &= ~toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_TARGET_PARAM;
      mapFlag |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_MEMBER_OF;
      ompBuilder.setCorrectMemberOfFlag(mapFlag, memberOfFlag);
      combinedInfo.Types.emplace_back(mapFlag);
      combinedInfo.DevicePointers.emplace_back(
          toolchain::OpenMPIRBuilder::DeviceInfoTy::None);
    ...
    if (checkIfPointerMap(memberClause) &&
      ...
      mapFlag |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_PTR_AND_OBJ;
    }
    ...
    toolchain::Value *size = mapData.Sizes[memberDataIdx];
    if (checkIfPointerMap(memberClause)) {
...
static void processIndividualMap(MapInfoData &mapData, size_t mapDataIdx,
  ...
  auto mapInfoOp = toolchain::cast<omp::MapInfoOp>(mapData.MapClause[mapDataIdx]);
  ...
  if (mapInfoOp.getMapCaptureType() == omp::VariableCaptureKind::ByCopy &&
      !isPtrTy)
    mapFlag |= toolchain::omp::OpenMPOffloadMappingFlags::OMP_MAP_LITERAL;
...
static void processMapWithMembersOf(LLVM::ModuleTranslation &moduleTranslation,
  ...
  if (parentClause.getMembers().size() == 1 && parentClause.getPartialMap()) {
    auto memberClause = toolchain::cast<omp::MapInfoOp>(
  ...
  toolchain::omp::OpenMPOffloadMappingFlags memberOfParentFlag =
  ...
  for (size_t i = 0; i < mapData.MapClause.size(); ++i) {
    ...
    if (!mapData.IsDeclareTarget[i]) {
      ...
      case omp::VariableCaptureKind::ByRef: {
        toolchain::Value *newV = mapData.Pointers[i];
        std::vector<toolchain::Value *> offsetIdx = calculateBoundsOffset(
      ...
      case omp::VariableCaptureKind::ByCopy: {
        toolchain::Type *type = mapData.BaseType[i];
        ...
        if (!isPtrTy) {
          auto curInsert = builder.saveIP();
          toolchain::DebugLoc DbgLoc = builder.getCurrentDebugLocation();
...
static void genMapInfos(toolchain::IRBuilderBase &builder,
                        LLVM::ModuleTranslation &moduleTranslation,
                        DataLayout &dl, MapInfosTy &combinedInfo,
                        MapInfoData &mapData,
                        TargetDirectiveEnumTy targetDirective) {
...
static toolchain::Expected<toolchain::Function *>
emitUserDefinedMapper(Operation *declMapperOp, toolchain::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation,
                      toolchain::StringRef mapperFuncName,
                      TargetDirectiveEnumTy targetDirective);

static toolchain::Expected<toolchain::Function *>
getOrCreateUserDefinedMapperFunc(Operation *op, toolchain::IRBuilderBase &builder,
                                 LLVM::ModuleTranslation &moduleTranslation,
                                 TargetDirectiveEnumTy targetDirective) {
  assert(!moduleTranslation.getOpenMPBuilder()->Config.isTargetDevice() &&
         "function only supported for host device codegen");
...
static toolchain::Expected<toolchain::Function *>
emitUserDefinedMapper(Operation *op, toolchain::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation,
                      toolchain::StringRef mapperFuncName,
                      TargetDirectiveEnumTy targetDirective) {
  assert(!moduleTranslation.getOpenMPBuilder()->Config.isTargetDevice() &&
  ...
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  toolchain::Type *varType = moduleTranslation.convertType(declMapperOp.getType());
  ...
  MapInfosTy combinedInfo;
  auto genMapInfoCB =
      [&](InsertPointTy codeGenIP, toolchain::Value *ptrPHI,
          toolchain::Value *unused2) -> toolchain::OpenMPIRBuilder::MapInfosOrErrorTy {
    builder.restoreIP(codeGenIP);
  ...
  auto customMapperCB = [&](unsigned i) -> toolchain::Expected<toolchain::Function *> {
  ...
  toolchain::Expected<toolchain::Function *> newFn = ompBuilder->emitUserDefinedMapper(
...
static LogicalResult
convertOmpTargetData(Operation *op, toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::Value *ifCond = nullptr;
  toolchain::Value *deviceID = builder.getInt64(toolchain::omp::OMP_DEVICEID_UNDEF);
  SmallVector<Value> mapVars;
  SmallVector<Value> useDevicePtrVars;
  SmallVector<Value> useDeviceAddrVars;
  toolchain::omp::RuntimeFunction RTLFn;
  DataLayout DL = DataLayout(op->getParentOfType<ModuleOp>());
  TargetDirectiveEnumTy targetDirective = getTargetDirectiveEnumTyFromOp(op);

  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  toolchain::OpenMPIRBuilder::TargetDataInfo info(
  ...
  auto getDeviceID = [&](mlir::Value dev) -> toolchain::Value * {
    toolchain::Value *v = moduleTranslation.lookupValue(dev);
    return builder.CreateIntCast(v, builder.getInt64Ty(), /*isSigned=*/true);
  };

  LogicalResult result =
      toolchain::TypeSwitch<Operation *, LogicalResult>(op)
          ...
          .Case([&](omp::TargetEnterDataOp enterDataOp) -> LogicalResult {
            ...
            RTLFn =
                enterDataOp.getNowait()
                    ? toolchain::omp::OMPRTL___tgt_target_data_begin_nowait_mapper
          ...
          .Case([&](omp::TargetExitDataOp exitDataOp) -> LogicalResult {
            ...
            RTLFn = exitDataOp.getNowait()
                        ? toolchain::omp::OMPRTL___tgt_target_data_end_nowait_mapper
          ...
          .Case([&](omp::TargetUpdateOp updateDataOp) -> LogicalResult {
            ...
            RTLFn =
                updateDataOp.getNowait()
                    ? toolchain::omp::OMPRTL___tgt_target_data_update_nowait_mapper
  ...
  auto mapUseDevice =
      [&moduleTranslation](
          toolchain::OpenMPIRBuilder::DeviceInfoTy type,
          toolchain::ArrayRef<BlockArgument> blockArgs,
          toolchain::SmallVectorImpl<Value> &useDeviceVars, MapInfoData &mapInfoData,
          toolchain::function_ref<toolchain::Value *(toolchain::Value *)> mapper = nullptr) {
        for (auto [arg, useDevVar] :
             toolchain::zip_equal(blockArgs, useDeviceVars)) {
          ...
          for (auto [mapClause, devicePointer, basePointer] : toolchain::zip_equal(
            ...
            if (toolchain::Value *devPtrInfoMap =
  ...
  using BodyGenTy = toolchain::OpenMPIRBuilder::BodyGenTy;
  auto bodyGenCB = [&](InsertPointTy codeGenIP, BodyGenTy bodyGenType)
    ...
    case BodyGenTy::Priv:
      ...
      if (!info.DevicePtrInfoMap.empty()) {
        mapUseDevice(toolchain::OpenMPIRBuilder::DeviceInfoTy::Address,
                     blockArgIface.getUseDeviceAddrBlockArgs(),
                     useDeviceAddrVars, mapData,
                     [&](toolchain::Value *basePointer) -> toolchain::Value * {
        ...
        mapUseDevice(toolchain::OpenMPIRBuilder::DeviceInfoTy::Pointer,
                     blockArgIface.getUseDevicePtrBlockArgs(), useDevicePtrVars,
                     mapData, [&](toolchain::Value *basePointer) {
  ...
  auto customMapperCB =
      [&](unsigned int i) -> toolchain::Expected<toolchain::Function *> {
    if (!combinedInfo.Mappers[i])
      return nullptr;
    info.HasMapper = true;
    return getOrCreateUserDefinedMapperFunc(combinedInfo.Mappers[i], builder,
...
static LogicalResult
convertOmpDistribute(Operation &opInst, toolchain::IRBuilderBase &builder,
                     LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
  ...
  if (doDistributeReduction) {
    ...
    MutableArrayRef<BlockArgument> reductionArgs =
        toolchain::cast<omp::BlockArgOpenMPOpInterface>(*teamsOp)
  ...
  auto bodyGenCB = [&](InsertPointTy allocaIP,
    ...
    toolchain::Expected<toolchain::BasicBlock *> afterAllocas =
    ...
    toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
    toolchain::OpenMPIRBuilder::LocationDescription ompLoc(builder);
    toolchain::Expected<toolchain::BasicBlock *> regionBlock =
    ...
    if (!isa_and_present<omp::WsloopOp>(distributeOp.getNestedWrapper())) {
      ...
      bool isSimd = false;
      toolchain::omp::WorksharingLoopType workshareLoopType =
          toolchain::omp::WorksharingLoopType::DistributeStaticLoop;
      bool loopNeedsBarrier = false;
      toolchain::Value *chunk = moduleTranslation.lookupValue(
          distributeOp.getDistScheduleChunkSize());
      toolchain::CanonicalLoopInfo *loopInfo =
...
convertFlagsAttr(Operation *op, mlir::omp::FlagsAttr attribute,
  ...
  ompBuilder->M.addModuleFlag(toolchain::Module::Max, "openmp-device",
...
static void getTargetEntryUniqueInfo(toolchain::TargetRegionEntryInfo &targetInfo,
                                     omp::TargetOp targetOp,
                                     toolchain::StringRef parentName = "") {
  ...
  if (auto ec = toolchain::sys::fs::getUniqueID(fileName, id)) {
    size_t fileHash = toolchain::hash_value(fileName.str());
    size_t deviceId = 0xdeadf17e;
    targetInfo =
        toolchain::TargetRegionEntryInfo(parentName, deviceId, fileHash, line);
  } else {
    targetInfo = toolchain::TargetRegionEntryInfo(parentName, id.getDevice(),
                                             id.getFile(), line);
  }
}
...
handleDeclareTargetMapVar(MapInfoData &mapData,
                          LLVM::ModuleTranslation &moduleTranslation,
                          toolchain::IRBuilderBase &builder, toolchain::Function *func) {
  assert(moduleTranslation.getOpenMPBuilder()->Config.isTargetDevice() &&
  ...
  for (size_t i = 0; i < mapData.MapClause.size(); ++i) {
    ...
    if (mapData.IsDeclareTarget[i]) {
      ...
      if (auto *constant = dyn_cast<toolchain::Constant>(mapData.OriginalValue[i]))
      ...
      // all).
      toolchain::SmallVector<toolchain::User *> userVec;
      for (toolchain::User *user : mapData.OriginalValue[i]->users())
        userVec.push_back(user);

      for (toolchain::User *user : userVec) {
        if (auto *insn = dyn_cast<toolchain::Instruction>(user)) {
          if (insn->getFunction() == func) {
            auto mapOp = cast<omp::MapInfoOp>(mapData.MapClause[i]);
            toolchain::Value *substitute = mapData.BasePointers[i];
            if (isDeclareTargetLink(mapOp.getVarPtrPtr() ? mapOp.getVarPtrPtr()
              ...
              cast<toolchain::LoadInst>(substitute)->moveBefore(insn->getIterator());
            }
...
// directly.
static toolchain::IRBuilderBase::InsertPoint
createDeviceArgumentAccessor(MapInfoData &mapData, toolchain::Argument &arg,
                             toolchain::Value *input, toolchain::Value *&retVal,
                             toolchain::IRBuilderBase &builder,
                             toolchain::OpenMPIRBuilder &ompBuilder,
                             LLVM::ModuleTranslation &moduleTranslation,
                             toolchain::IRBuilderBase::InsertPoint allocaIP,
                             toolchain::IRBuilderBase::InsertPoint codeGenIP) {
  assert(ompBuilder.Config.isTargetDevice() &&
         "function only supported for target device codegen");
  builder.restoreIP(allocaIP);
  ...
  toolchain::Value *v = builder.CreateAlloca(arg.getType(), allocaAS);
  ...
  case omp::VariableCaptureKind::ByRef: {
    ...
    if (v->getType()->isPointerTy() && alignmentValue) {
      toolchain::MDBuilder MDB(builder.getContext());
      loadInst->setMetadata(
          toolchain::LLVMContext::MD_align,
          toolchain::MDNode::get(builder.getContext(),
                            MDB.createConstant(toolchain::ConstantInt::get(
                                toolchain::Type::getInt64Ty(builder.getContext()),
                                alignmentValue))));
    }
    retVal = loadInst;
...
extractHostEvalClauses(omp::TargetOp targetOp, Value &numThreads,
                       ...
                       Value &threadLimit,
                       toolchain::SmallVectorImpl<Value> *lowerBounds = nullptr,
                       toolchain::SmallVectorImpl<Value> *upperBounds = nullptr,
                       toolchain::SmallVectorImpl<Value> *steps = nullptr) {
  auto blockArgIface = toolchain::cast<omp::BlockArgOpenMPOpInterface>(*targetOp);
  for (auto item : toolchain::zip_equal(targetOp.getHostEvalVars(),
                                   blockArgIface.getHostEvalBlockArgs())) {
    Value hostEvalVar = std::get<0>(item), blockArg = std::get<1>(item);

    for (Operation *user : blockArg.getUsers()) {
      toolchain::TypeSwitch<Operation *>(user)
          ...
          .Case([&](omp::LoopNestOp loopOp) {
            auto processBounds =
                [&](OperandRange opBounds,
                    toolchain::SmallVectorImpl<Value> *outBounds) -> bool {
              bool found = false;
              for (auto [i, lb] : toolchain::enumerate(opBounds)) {
                if (lb == blockArg) {
                  found = true;
                  if (outBounds)
...
/// If the given \p value is defined by an \c toolchain.mlir.constant operation and
...
initTargetDefaultAttrs(omp::TargetOp targetOp, Operation *capturedOp,
                       toolchain::OpenMPIRBuilder::TargetKernelDefaultAttrs &attrs,
  ...
  attrs.ExecFlags =
      omp::bitEnumContainsAny(kernelFlags, omp::TargetRegionFlags::generic)
          ? omp::bitEnumContainsAny(kernelFlags, omp::TargetRegionFlags::spmd)
                ? toolchain::omp::OMP_TGT_EXEC_MODE_GENERIC_SPMD
                : toolchain::omp::OMP_TGT_EXEC_MODE_GENERIC
          : toolchain::omp::OMP_TGT_EXEC_MODE_SPMD;
  if (omp::bitEnumContainsAll(kernelFlags,
                              omp::TargetRegionFlags::spmd |
                                  omp::TargetRegionFlags::no_loop) &&
      !omp::bitEnumContainsAny(kernelFlags, omp::TargetRegionFlags::generic))
    attrs.ExecFlags = toolchain::omp::OMP_TGT_EXEC_MODE_SPMD_NO_LOOP;
...
static void
initTargetRuntimeAttrs(toolchain::IRBuilderBase &builder,
                       LLVM::ModuleTranslation &moduleTranslation,
                       omp::TargetOp targetOp, Operation *capturedOp,
                       toolchain::OpenMPIRBuilder::TargetKernelRuntimeAttrs &attrs) {
  ...
  toolchain::SmallVector<Value> lowerBounds(numLoops), upperBounds(numLoops),
      steps(numLoops);
  extractHostEvalClauses(targetOp, numThreads, numTeamsLower, numTeamsUpper,
  ...
  if (omp::bitEnumContainsAny(targetOp.getKernelExecFlags(capturedOp),
    ...
    for (auto [loopLower, loopUpper, loopStep] :
         toolchain::zip_equal(lowerBounds, upperBounds, steps)) {
      toolchain::Value *lowerBound = moduleTranslation.lookupValue(loopLower);
      toolchain::Value *upperBound = moduleTranslation.lookupValue(loopUpper);
      toolchain::Value *step = moduleTranslation.lookupValue(loopStep);

      toolchain::OpenMPIRBuilder::LocationDescription loc(builder);
      toolchain::Value *tripCount = ompBuilder->calculateCanonicalLoopTripCount(
  ...
  attrs.DeviceID = builder.getInt64(toolchain::omp::OMP_DEVICEID_UNDEF);
...
static LogicalResult
convertOmpTarget(Operation &opInst, toolchain::IRBuilderBase &builder,
                 LLVM::ModuleTranslation &moduleTranslation) {
  ...
  toolchain::DebugLoc outlinedFnLoc = builder.getCurrentDebugLocation();
  if (failed(checkImplementationStatus(opInst)))
    return failure();
  ...
  toolchain::BasicBlock *parentBB = builder.GetInsertBlock();
  assert(parentBB && "No insert block is set for the builder");
  toolchain::Function *parentLLVMFn = parentBB->getParent();
  assert(parentLLVMFn && "Parent Function must be valid");
  if (toolchain::DISubprogram *SP = parentLLVMFn->getSubprogram())
    builder.SetCurrentDebugLocation(toolchain::DILocation::get(
  ...
  toolchain::DenseMap<Value, Value> mappedPrivateVars;
  ...
  toolchain::Function *llvmOutlinedFn = nullptr;
  TargetDirectiveEnumTy targetDirective =
  ...
  if (!targetOp.getPrivateVars().empty() && !targetOp.getMapVars().empty()) {
    ...
    for (auto [privVarIdx, privVarSymPair] :
         toolchain::enumerate(toolchain::zip_equal(privateVars, *privateSyms))) {
  ...
  auto bodyCB = [&](InsertPointTy allocaIP, InsertPointTy codeGenIP)
    ...
    toolchain::Function *llvmParentFn =
    ...
    for (auto [arg, mapOp] : toolchain::zip_equal(mapBlockArgs, mapVars)) {
      auto mapInfoOp = cast<omp::MapInfoOp>(mapOp.getDefiningOp());
      toolchain::Value *mapOpValue =
          moduleTranslation.lookupValue(mapInfoOp.getVarPtr());
      moduleTranslation.mapValue(arg, mapOpValue);
    }
    for (auto [arg, mapOp] : toolchain::zip_equal(hdaBlockArgs, hdaVars)) {
      auto mapInfoOp = cast<omp::MapInfoOp>(mapOp.getDefiningOp());
      toolchain::Value *mapOpValue =
          moduleTranslation.lookupValue(mapInfoOp.getVarPtr());
      moduleTranslation.mapValue(arg, mapOpValue);
    ...
    SmallVector<Region *> privateCleanupRegions;
    toolchain::transform(privateVarsInfo.privatizers,
                    std::back_inserter(privateCleanupRegions),
                    [](omp::PrivateClauseOp privatizer) {
                      return &privatizer.getDeallocRegion();
                    });

    toolchain::Expected<toolchain::BasicBlock *> exitBlock = convertOmpOpRegions(
  ...
  toolchain::TargetRegionEntryInfo entryInfo;
  ...
  auto genMapInfoCB =
      [&](toolchain::OpenMPIRBuilder::InsertPointTy codeGenIP) -> MapInfosTy & {
    builder.restoreIP(codeGenIP);
  ...
  auto argAccessorCB = [&](toolchain::Argument &arg, toolchain::Value *input,
                           toolchain::Value *&retVal, InsertPointTy allocaIP,
    ...
    if (!isTargetDevice) {
      retVal = cast<toolchain::Value>(&arg);
      return codeGenIP;
    }
  ...
  toolchain::OpenMPIRBuilder::TargetKernelRuntimeAttrs runtimeAttrs;
  toolchain::OpenMPIRBuilder::TargetKernelDefaultAttrs defaultAttrs;
  Operation *targetCapturedOp = targetOp.getInnermostCapturedOmpOp();
  ...
  toolchain::SmallVector<toolchain::Value *, 4> kernelInput;
  ...
  for (auto [arg, var] : toolchain::zip_equal(hostEvalBlockArgs, hostEvalVars)) {
    toolchain::Value *value = moduleTranslation.lookupValue(var);
    moduleTranslation.mapValue(arg, value);

    if (!toolchain::isa<toolchain::Constant>(value))
...
convertDeclareTargetAttr(Operation *op, mlir::omp::DeclareTargetAttr attribute,
  ...
  if (LLVM::GlobalOp gOp = dyn_cast<LLVM::GlobalOp>(op)) {
    ...
    if (auto *gVal = llvmModule->getNamedValue(gOp.getSymName())) {
      ...
      toolchain::StringRef mangledName = gOp.getSymName();
      auto captureClause =
      ...
      // keeping
      std::vector<toolchain::GlobalVariable *> generatedRefs;

      std::vector<toolchain::Triple> targetTriple;
      ...
      auto fileInfoCallBack = [&loc]() {
        ...
        return std::pair<std::string, std::uint64_t>(toolchain::StringRef(filename),
                                                     lineNo);
      };
...
static bool isTargetDeviceOp(Operation *op) {
  ...
  if (auto parentFn = op->getParentOfType<LLVM::LLVMFuncOp>())
    if (auto declareTargetIface =
            toolchain::dyn_cast<mlir::omp::DeclareTargetInterface>(
...
static toolchain::Function *getOmpTargetAlloc(toolchain::IRBuilderBase &builder,
                                         toolchain::Module *llvmModule) {
  toolchain::Type *i64Ty = builder.getInt64Ty();
  toolchain::Type *i32Ty = builder.getInt32Ty();
  toolchain::Type *returnType = builder.getPtrTy(0);
  toolchain::FunctionType *fnType =
      toolchain::FunctionType::get(returnType, {i64Ty, i32Ty}, false);
  toolchain::Function *func = cast<toolchain::Function>(
      llvmModule->getOrInsertFunction("omp_target_alloc", fnType).getCallee());
  return func;
}

static LogicalResult
convertTargetAllocMemOp(Operation &opInst, toolchain::IRBuilderBase &builder,
                        LLVM::ModuleTranslation &moduleTranslation) {
  ...
  toolchain::Module *llvmModule = moduleTranslation.getLLVMModule();
  toolchain::Function *ompTargetAllocFunc = getOmpTargetAlloc(builder, llvmModule);
  // Get the corresponding device value in toolchain
  mlir::Value deviceNum = allocMemOp.getDevice();
  toolchain::Value *llvmDeviceNum = moduleTranslation.lookupValue(deviceNum);
  // Get the allocation size.
  toolchain::DataLayout dataLayout = llvmModule->getDataLayout();
  mlir::Type heapTy = allocMemOp.getAllocatedType();
  toolchain::Type *llvmHeapTy = moduleTranslation.convertType(heapTy);
  toolchain::TypeSize typeSize = dataLayout.getTypeStoreSize(llvmHeapTy);
  toolchain::Value *allocSize = builder.getInt64(typeSize.getFixedValue());
  for (auto typeParam : allocMemOp.getTypeparams())
    allocSize =
        builder.CreateMul(allocSize, moduleTranslation.lookupValue(typeParam));
  // Create call to "omp_target_alloc" with the args as translated toolchain values.
  toolchain::CallInst *call =
      builder.CreateCall(ompTargetAllocFunc, {allocSize, llvmDeviceNum});
  toolchain::Value *resultI64 = builder.CreatePtrToInt(call, builder.getInt64Ty());
...
static toolchain::Function *getOmpTargetFree(toolchain::IRBuilderBase &builder,
                                        toolchain::Module *llvmModule) {
  toolchain::Type *ptrTy = builder.getPtrTy(0);
  toolchain::Type *i32Ty = builder.getInt32Ty();
  toolchain::Type *voidTy = builder.getVoidTy();
  toolchain::FunctionType *fnType =
      toolchain::FunctionType::get(voidTy, {ptrTy, i32Ty}, false);
  toolchain::Function *func = dyn_cast<toolchain::Function>(
      llvmModule->getOrInsertFunction("omp_target_free", fnType).getCallee());
  return func;
}

static LogicalResult
convertTargetFreeMemOp(Operation &opInst, toolchain::IRBuilderBase &builder,
                       LLVM::ModuleTranslation &moduleTranslation) {
  ...
  toolchain::Module *llvmModule = moduleTranslation.getLLVMModule();
  toolchain::Function *ompTragetFreeFunc = getOmpTargetFree(builder, llvmModule);
  // Get the corresponding device value in toolchain
  mlir::Value deviceNum = freeMemOp.getDevice();
  toolchain::Value *llvmDeviceNum = moduleTranslation.lookupValue(deviceNum);
  // Get the corresponding heapref value in toolchain
  mlir::Value heapref = freeMemOp.getHeapref();
  toolchain::Value *llvmHeapref = moduleTranslation.lookupValue(heapref);
...
static LogicalResult
convertHostOrTargetOperation(Operation *op, toolchain::IRBuilderBase &builder,
                             LLVM::ModuleTranslation &moduleTranslation) {
  toolchain::OpenMPIRBuilder *ompBuilder = moduleTranslation.getOpenMPBuilder();
...
static LogicalResult
convertTargetDeviceOp(Operation *op, toolchain::IRBuilderBase &builder,
                      LLVM::ModuleTranslation &moduleTranslation) {
  return convertHostOrTargetOperation(op, builder, moduleTranslation);
}

static LogicalResult
convertTargetOpsInNest(Operation *op, toolchain::IRBuilderBase &builder,
                       LLVM::ModuleTranslation &moduleTranslation) {
            ...
            if (auto blockArgsIface =
              ...
              if (isa<mlir::omp::AtomicUpdateOp>(oper))
                for (auto [operand, arg] :
                     toolchain::zip_equal(oper->getOperands(),
...
LogicalResult OpenMPDialectLLVMIRTranslationInterface::amendOperation(
  ...
  return toolchain::StringSwitch<toolchain::function_ref<LogicalResult(Attribute)>>(
             attribute.getName())
      ...
      .Case("omp.version",
            [&](Attribute attr) {
              if (auto versionAttr = dyn_cast<omp::VersionAttr>(attr)) {
                toolchain::OpenMPIRBuilder *ompBuilder =
                    moduleTranslation.getOpenMPBuilder();
                ompBuilder->M.addModuleFlag(toolchain::Module::Max, "openmp",
