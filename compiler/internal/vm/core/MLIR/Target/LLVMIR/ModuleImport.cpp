...
static std::string diag(const toolchain::Value &value) {
  std::string str;
...
static std::string diagMD(const toolchain::Metadata *node,
...
static constexpr StringRef getGlobalCtorsVarName() {
  return "toolchain.global_ctors";
}

/// Prefix used for symbols of nameless toolchain globals.
static constexpr StringRef getNamelessGlobalPrefix() {
  return "mlir.toolchain.nameless_global";
}

/// Returns the name of the global_dtors global variables.
static constexpr StringRef getGlobalDtorsVarName() {
  return "toolchain.global_dtors";
}
...
static StringRef getLLVMSyncScope(toolchain::Instruction *inst) {
  std::optional<toolchain::SyncScope::ID> syncScopeID =
      toolchain::getAtomicSyncScopeID(inst);
  if (!syncScopeID)
    return "";
  ...
  toolchain::LLVMContext &llvmContext = inst->getContext();
  llvmContext.getSyncScopeNames(syncScopeName);
  auto *it = toolchain::find_if(syncScopeName, [&](StringRef name) {
...
static LogicalResult convertInstructionImpl(OpBuilder &odsBuilder,
  ...
  SmallVector<toolchain::Value *> operands(inst->operands());
  ArrayRef<toolchain::Value *> llvmOperands(operands);
...
static SetVector<toolchain::BasicBlock *>
getTopologicallySortedBlocks(ArrayRef<toolchain::BasicBlock *> basicBlocks) {
  SetVector<toolchain::BasicBlock *> blocks;
  for (toolchain::BasicBlock *basicBlock : basicBlocks) {
    if (!blocks.contains(basicBlock)) {
      toolchain::ReversePostOrderTraversal<toolchain::BasicBlock *> traversal(basicBlock);
...
LogicalResult ModuleImport::processTBAAMetadata(const toolchain::MDNode *node) {
  ...
  auto getIdentityIfRootNode =
      [&](const toolchain::MDNode *node) -> FailureOr<std::optional<StringRef>> {
    // Root node, e.g.:
    //   !0 = !{!"Simple C/C++ TBAA"}
    //   !1 = !{}
    ...
    if (node->getNumOperands() == 1)
      if (const auto *op0 = dyn_cast<const toolchain::MDString>(node->getOperand(0)))
  ...
  // true is returned).
  auto isTypeDescriptorNode = [&](const toolchain::MDNode *node,
    ...
    const auto *identityNode =
        dyn_cast<const toolchain::MDString>(node->getOperand(0));
    if (!identityNode)
      return std::nullopt;
    ...
    for (unsigned pairNum = 0, e = numOperands / 2; pairNum < e; ++pairNum) {
      const auto *memberNode =
          dyn_cast<const toolchain::MDNode>(node->getOperand(2 * pairNum + 1));
      if (!memberNode) {
      ...
      if (2 * pairNum + 2 >= numOperands) {
        ...
        auto *offsetCI = toolchain::mdconst::dyn_extract<toolchain::ConstantInt>(
  ...
  // returned).
  auto isTagNode = [&](const toolchain::MDNode *node,
    ...
    // https://toolchain.org/docs/LangRef.html#representation).
    unsigned numOperands = node->getNumOperands();
    if (numOperands != 3 && numOperands != 4)
      return std::nullopt;
    const auto *baseMD = dyn_cast<const toolchain::MDNode>(node->getOperand(0));
    const auto *accessMD = dyn_cast<const toolchain::MDNode>(node->getOperand(1));
    auto *offsetCI =
        toolchain::mdconst::dyn_extract<toolchain::ConstantInt>(node->getOperand(2));
    ...
    if (accessMD->getNumOperands() < 1 ||
        !isa<toolchain::MDString>(accessMD->getOperand(0)))
      return std::nullopt;
    bool isConst = false;
    if (numOperands == 4) {
      auto *isConstantCI =
          toolchain::mdconst::dyn_extract<toolchain::ConstantInt>(node->getOperand(3));
      if (!isConstantCI) {
  ...
  DenseSet<const toolchain::MDNode *> seen;
  SmallVector<const toolchain::MDNode *> workList;
  workList.push_back(node);
  while (!workList.empty()) {
    const toolchain::MDNode *current = workList.back();
    ...
    for (const toolchain::MDOperand &operand : current->operands())
      if (auto *childNode = dyn_cast_or_null<const toolchain::MDNode>(operand.get()))
...
LogicalResult
ModuleImport::processAccessGroupMetadata(const toolchain::MDNode *node) {
  Location loc = mlirModule.getLoc();
...
LogicalResult
ModuleImport::processAliasScopeMetadata(const toolchain::MDNode *node) {
  Location loc = mlirModule.getLoc();
  // Helper that verifies the node has a self reference operand.
  auto verifySelfRef = [](const toolchain::MDNode *node) {
    return node->getNumOperands() != 0 &&
           node == dyn_cast<toolchain::MDNode>(node->getOperand(0));
  };
  auto verifySelfRefOrString = [](const toolchain::MDNode *node) {
    return node->getNumOperands() != 0 &&
           (node == dyn_cast<toolchain::MDNode>(node->getOperand(0)) ||
            isa<toolchain::MDString>(node->getOperand(0)));
  };
  // Helper that verifies the given operand is a string or does not exist.
  auto verifyDescription = [](const toolchain::MDNode *node, unsigned idx) {
    return idx >= node->getNumOperands() ||
           isa<toolchain::MDString>(node->getOperand(idx));
  };

  auto getIdAttr = [&](const toolchain::MDNode *node) -> Attribute {
    if (verifySelfRef(node))
      return DistinctAttr::create(builder.getUnitAttr());

    auto *name = cast<toolchain::MDString>(node->getOperand(0));
    return builder.getStringAttr(name->getString());
  };

  // Helper that creates an alias scope domain attribute.
  auto createAliasScopeDomainOp = [&](const toolchain::MDNode *aliasDomain) {
    StringAttr description = nullptr;
    if (aliasDomain->getNumOperands() >= 2)
      if (auto *operand = dyn_cast<toolchain::MDString>(aliasDomain->getOperand(1)))
  ...
  for (const toolchain::MDOperand &operand : node->operands()) {
    if (const auto *scope = dyn_cast<toolchain::MDNode>(operand)) {
      toolchain::AliasScopeNode aliasScope(scope);
      const toolchain::MDNode *domain = aliasScope.getDomain();
...
ModuleImport::lookupAliasScopeAttrs(const toolchain::MDNode *node) const {
  ...
  if (toolchain::is_contained(aliasScopes, nullptr))
    return failure();
  return aliasScopes;
}

void ModuleImport::addDebugIntrinsic(toolchain::CallInst *intrinsic) {
  debugIntrinsics.insert(intrinsic);
}

void ModuleImport::addDebugRecord(toolchain::DbgVariableRecord *dbgRecord) {
...
static Attribute convertCGProfileModuleFlagValue(ModuleOp mlirModule,
                                                 toolchain::MDTuple *mdTuple) {
  auto getLLVMFunction =
      [&](const toolchain::MDOperand &funcMDO) -> toolchain::Function * {
    auto *f = cast_or_null<toolchain::ValueAsMetadata>(funcMDO);
    // nullptr is a valid value for the function pointer.
    if (!f)
      return nullptr;
    auto *llvmFn = cast<toolchain::Function>(f->getValue()->stripPointerCasts());
    return llvmFn;
  };
  ...
  for (unsigned i = 0; i < mdTuple->getNumOperands(); i++) {
    const toolchain::MDOperand &mdo = mdTuple->getOperand(i);
    auto *cgEntry = cast<toolchain::MDNode>(mdo);
    toolchain::Constant *llvmConstant =
        cast<toolchain::ConstantAsMetadata>(cgEntry->getOperand(2))->getValue();
    uint64_t count = cast<toolchain::ConstantInt>(llvmConstant)->getZExtValue();
...
static toolchain::MDTuple *getTwoElementMDTuple(ModuleOp mlirModule,
                                           const toolchain::Module *llvmModule,
                                           const toolchain::MDOperand &md) {
  auto *tupleEntry = dyn_cast_or_null<toolchain::MDTuple>(md);
...
static toolchain::ConstantAsMetadata *getConstantMDFromKeyValueTuple(
    ModuleOp mlirModule, const toolchain::Module *llvmModule,
    const toolchain::MDOperand &md, StringRef matchKey, bool optional = false) {
  toolchain::MDTuple *tupleEntry = getTwoElementMDTuple(mlirModule, llvmModule, md);
  if (!tupleEntry)
    return nullptr;
  auto *keyMD = dyn_cast<toolchain::MDString>(tupleEntry->getOperand(0));
  ...
  return dyn_cast<toolchain::ConstantAsMetadata>(tupleEntry->getOperand(1));
}
...
convertInt64FromKeyValueTuple(ModuleOp mlirModule,
                              const toolchain::Module *llvmModule,
                              const toolchain::MDOperand &md, StringRef matchKey) {
  toolchain::ConstantAsMetadata *valMD =
      getConstantMDFromKeyValueTuple(mlirModule, llvmModule, md, matchKey);
  if (!valMD)
    return failure();

  if (auto *cstInt = dyn_cast<toolchain::ConstantInt>(valMD->getValue()))
...
convertProfileSummaryFormat(ModuleOp mlirModule, const toolchain::Module *llvmModule,
                            const toolchain::MDOperand &formatMD) {
  auto *tupleEntry = getTwoElementMDTuple(mlirModule, llvmModule, formatMD);
  if (!tupleEntry)
    return std::nullopt;

  toolchain::MDString *keyMD = dyn_cast<toolchain::MDString>(tupleEntry->getOperand(0));
  ...
  toolchain::MDString *valMD = dyn_cast<toolchain::MDString>(tupleEntry->getOperand(1));
...
convertProfileSummaryDetailed(ModuleOp mlirModule,
                              const toolchain::Module *llvmModule,
                              const toolchain::MDOperand &summaryMD) {
  ...
  toolchain::MDTuple *entriesMD = dyn_cast<toolchain::MDTuple>(tupleEntry->getOperand(1));
  if (!entriesMD) {
    emitWarning(mlirModule.getLoc())
  ...
  for (auto &&entry : entriesMD->operands()) {
    toolchain::MDTuple *entryMD = dyn_cast<toolchain::MDTuple>(entry);
    ...
    auto *op0 = dyn_cast<toolchain::ConstantAsMetadata>(entryMD->getOperand(0));
    auto *op1 = dyn_cast<toolchain::ConstantAsMetadata>(entryMD->getOperand(1));
    auto *op2 = dyn_cast<toolchain::ConstantAsMetadata>(entryMD->getOperand(2));
    if (!op0 || !op1 || !op2) {
      emitWarning(mlirModule.getLoc())
    ...
    auto detaildSummaryEntry = ModuleFlagProfileSummaryDetailedAttr::get(
        mlirModule->getContext(),
        cast<toolchain::ConstantInt>(op0->getValue())->getZExtValue(),
        cast<toolchain::ConstantInt>(op1->getValue())->getZExtValue(),
        cast<toolchain::ConstantInt>(op2->getValue())->getZExtValue());
...
convertProfileSummaryModuleFlagValue(ModuleOp mlirModule,
  ...
  auto checkOptionalPosition = [&](const toolchain::MDOperand &md,
  ...
  auto getOptDoubleValue = [&](const toolchain::MDOperand &md,
    ...
    if (!valMD)
      return FloatAttr{};
    if (auto *cstFP = dyn_cast<toolchain::ConstantFP>(valMD->getValue())) {
      if (checkOptionalPosition(md, matchKey).failed())
        return failure();
...
convertModuleFlagValueFromMDTuple(ModuleOp mlirModule,
                                  const toolchain::Module *llvmModule, StringRef key,
                                  toolchain::MDTuple *mdTuple) {
...
LogicalResult ModuleImport::convertModuleFlagsMetadata() {
  SmallVector<toolchain::Module::ModuleFlagEntry> llvmModuleFlags;
  ...
  for (const auto [behavior, key, val] : llvmModuleFlags) {
    ...
    if (auto *constInt = toolchain::mdconst::dyn_extract<toolchain::ConstantInt>(val)) {
      valAttr = builder.getI32IntegerAttr(constInt->getZExtValue());
    } else if (auto *mdString = dyn_cast<toolchain::MDString>(val)) {
      valAttr = builder.getStringAttr(mdString->getString());
    } else if (auto *mdTuple = dyn_cast<toolchain::MDTuple>(val)) {
...
LogicalResult ModuleImport::convertLinkerOptionsMetadata() {
  for (const toolchain::NamedMDNode &named : llvmModule->named_metadata()) {
    if (named.getName() != "toolchain.linker.options")
      continue;
    // toolchain.linker.options operands are lists of strings.
    for (const toolchain::MDNode *node : named.operands()) {
      SmallVector<StringRef> options;
      options.reserve(node->getNumOperands());
      for (const toolchain::MDOperand &option : node->operands())
        options.push_back(cast<toolchain::MDString>(option)->getString());
...
LogicalResult ModuleImport::convertDependentLibrariesMetadata() {
  for (const toolchain::NamedMDNode &named : llvmModule->named_metadata()) {
    if (named.getName() != "toolchain.dependent-libraries")
      continue;
    SmallVector<StringRef> libraries;
    for (const toolchain::MDNode *node : named.operands()) {
      if (node->getNumOperands() == 1)
        if (auto *mdString = dyn_cast<toolchain::MDString>(node->getOperand(0)))
...
LogicalResult ModuleImport::convertIdentMetadata() {
  for (const toolchain::NamedMDNode &named : llvmModule->named_metadata()) {
    // toolchain.ident should have a single operand. That operand is itself an
    // MDNode with a single string operand.
    if (named.getName() != LLVMDialect::getIdentAttrName())
      continue;

    if (named.getNumOperands() == 1)
      if (auto *md = dyn_cast<toolchain::MDNode>(named.getOperand(0)))
        if (md->getNumOperands() == 1)
          if (auto *mdStr = dyn_cast<toolchain::MDString>(md->getOperand(0)))
...
LogicalResult ModuleImport::convertCommandlineMetadata() {
  for (const toolchain::NamedMDNode &nmd : llvmModule->named_metadata()) {
    // toolchain.commandline should have a single operand. That operand is itself an
    // MDNode with a single string operand.
    if (nmd.getName() != LLVMDialect::getCommandlineAttrName())
      continue;

    if (nmd.getNumOperands() == 1)
      if (auto *md = dyn_cast<toolchain::MDNode>(nmd.getOperand(0)))
        if (md->getNumOperands() == 1)
          if (auto *mdStr = dyn_cast<toolchain::MDString>(md->getOperand(0)))
...
LogicalResult ModuleImport::convertMetadata() {
  OpBuilder::InsertionGuard guard(builder);
  builder.setInsertionPointToEnd(mlirModule.getBody());
  for (const toolchain::Function &func : llvmModule->functions()) {
    for (const toolchain::Instruction &inst : toolchain::instructions(func)) {
      // Convert access group metadata nodes.
      if (toolchain::MDNode *node =
              inst.getMetadata(toolchain::LLVMContext::MD_access_group))
        if (failed(processAccessGroupMetadata(node)))
          return failure();

      // Convert alias analysis metadata nodes.
      toolchain::AAMDNodes aliasAnalysisNodes = inst.getAAMetadata();
...
void ModuleImport::processComdat(const toolchain::Comdat *comdat) {
...
LogicalResult ModuleImport::convertComdats() {
  for (toolchain::GlobalVariable &globalVar : llvmModule->globals())
    if (globalVar.hasComdat())
      processComdat(globalVar.getComdat());
  for (toolchain::Function &func : llvmModule->functions())
    if (func.hasComdat())
...
LogicalResult ModuleImport::convertAliases() {
  for (toolchain::GlobalAlias &alias : llvmModule->aliases()) {
...
LogicalResult ModuleImport::convertIFuncs() {
  for (toolchain::GlobalIFunc &ifunc : llvmModule->ifuncs()) {
...
void ModuleImport::convertModuleLevelAsm() {
  toolchain::StringRef asmStr = llvmModule->getModuleInlineAsm();
  toolchain::SmallVector<mlir::Attribute> asmArrayAttr;

  for (toolchain::StringRef line : toolchain::split(asmStr, '\n'))
    if (!line.empty())
...
void ModuleImport::setNonDebugMetadataAttrs(toolchain::Instruction *inst,
                                            Operation *op) {
  SmallVector<std::pair<unsigned, toolchain::MDNode *>> allMetadata;
...
void ModuleImport::setIntegerOverflowFlags(toolchain::Instruction *inst,
                                           Operation *op) const {
...
void ModuleImport::setExactFlag(toolchain::Instruction *inst, Operation *op) const {
  auto iface = cast<ExactFlagInterface>(op);

  iface.setIsExact(inst->isExact());
}

void ModuleImport::setDisjointFlag(toolchain::Instruction *inst,
                                   Operation *op) const {
  auto iface = cast<DisjointFlagInterface>(op);
  auto *instDisjoint = cast<toolchain::PossiblyDisjointInst>(inst);

  iface.setIsDisjoint(instDisjoint->isDisjoint());
}

void ModuleImport::setNonNegFlag(toolchain::Instruction *inst, Operation *op) const {
  auto iface = cast<NonNegFlagInterface>(op);

  iface.setNonNeg(inst->hasNonNeg());
}

void ModuleImport::setFastmathFlagsAttr(toolchain::Instruction *inst,
                                        Operation *op) const {
  ...
  // fastmath flags.
  if (!isa<toolchain::FPMathOperator>(inst))
    return;
  toolchain::FastMathFlags flags = inst->getFastMathFlags();
...
static Type getVectorTypeForAttr(Type type, ArrayRef<int64_t> arrayShape = {}) {
  ...
  toolchain::ElementCount numElements = LLVM::getVectorNumElements(type);
...
static TypedAttr getScalarConstantAsAttr(OpBuilder &builder,
                                         toolchain::Constant *constScalar) {
  MLIRContext *context = builder.getContext();

  // Convert scalar integers.
  if (auto *constInt = dyn_cast<toolchain::ConstantInt>(constScalar)) {
  ...
  if (auto *constFloat = dyn_cast<toolchain::ConstantFP>(constScalar)) {
    toolchain::Type *type = constFloat->getType();
    FloatType floatType =
        type->isBFloatTy()
...
getSequenceConstantAsAttrs(OpBuilder &builder,
                           toolchain::ConstantDataSequential *constSequence) {
  SmallVector<Attribute> elementAttrs;
  elementAttrs.reserve(constSequence->getNumElements());
  for (auto idx : toolchain::seq<int64_t>(0, constSequence->getNumElements())) {
    toolchain::Constant *constElement = constSequence->getElementAsConstant(idx);
    elementAttrs.push_back(getScalarConstantAsAttr(builder, constElement));
  }
  return elementAttrs;
}

Attribute ModuleImport::getConstantAsAttr(toolchain::Constant *constant) {
  ...
  auto getConstantShape = [&](toolchain::Type *type) {
    return toolchain::dyn_cast_if_present<ShapedType>(
  ...
  if (auto *constArray = dyn_cast<toolchain::ConstantDataSequential>(constant)) {
    ...
    auto *constVector = dyn_cast<toolchain::ConstantDataVector>(constant);
  ...
  if (auto *constAggregate = dyn_cast<toolchain::ConstantAggregate>(constant)) {
    auto shape = getConstantShape(constAggregate->getType());
    if (!shape)
      return {};
    // Collect the aggregate elements in depths first order.
    SmallVector<Attribute> elementAttrs;
    SmallVector<toolchain::Constant *> workList = {constAggregate};
    while (!workList.empty()) {
      toolchain::Constant *current = workList.pop_back_val();
      ...
      if (auto *constAggregate = dyn_cast<toolchain::ConstantAggregate>(current)) {
        for (auto idx :
             reverse(toolchain::seq<int64_t>(0, constAggregate->getNumOperands())))
      ...
      if (auto *constArray = dyn_cast<toolchain::ConstantDataSequential>(current)) {
  ...
  if (auto *constZero = dyn_cast<toolchain::ConstantAggregateZero>(constant)) {
    auto shape = toolchain::dyn_cast_if_present<ShapedType>(
...
FlatSymbolRefAttr
ModuleImport::getOrCreateNamelessSymbolName(toolchain::GlobalVariable *globalVar) {
...
LogicalResult ModuleImport::convertAlias(toolchain::GlobalAlias *alias) {
...
LogicalResult ModuleImport::convertIFunc(toolchain::GlobalIFunc *ifunc) {
  OpBuilder::InsertionGuard guard = setGlobalInsertionPoint();

  Type type = convertType(ifunc->getValueType());
  toolchain::Constant *resolver = ifunc->getResolver();
...
convertLLVMAttributesToMLIR(Location loc, MLIRContext *context,
                            toolchain::AttributeSet attributes,
                            ArrayRef<StringLiteral> attributesToSkip = {}) {
  SmallVector<Attribute> mlirAttributes;
  for (toolchain::Attribute attr : attributes) {
    StringRef attrName;
    if (attr.isStringAttribute())
      attrName = attr.getKindAsString();
    else
      attrName = toolchain::Attribute::getNameFromAttrKind(attr.getKindAsEnum());
    if (toolchain::is_contained(attributesToSkip, attrName))
      continue;
...
static void processTargetSpecificAttrs(toolchain::GlobalVariable *globalVar,
                                       GlobalOp globalOp) {
...
LogicalResult ModuleImport::convertGlobal(toolchain::GlobalVariable *globalVar) {
  ...
  toolchain::MaybeAlign maybeAlign = globalVar->getAlign();
  if (maybeAlign.has_value()) {
    toolchain::Align align = *maybeAlign;
  ...
  SmallVector<toolchain::DIGlobalVariableExpression *> globalExpressions;
  globalVar->getDebugInfo(globalExpressions);

  for (toolchain::DIGlobalVariableExpression *expr : globalExpressions) {
...
LogicalResult
ModuleImport::convertGlobalCtorsAndDtors(toolchain::GlobalVariable *globalVar) {
  if (!globalVar->hasInitializer() || !globalVar->hasAppendingLinkage())
    return failure();
  toolchain::Constant *initializer = globalVar->getInitializer();

  bool knownInit = isa<toolchain::ConstantArray>(initializer) ||
                   isa<toolchain::ConstantAggregateZero>(initializer);
  if (!knownInit)
    return failure();
  ...
  if (auto *caz = dyn_cast<toolchain::ConstantAggregateZero>(initializer)) {
  ...
  for (toolchain::Value *operand : initializer->operands()) {
    auto *aggregate = dyn_cast<toolchain::ConstantAggregate>(operand);
    if (!aggregate || aggregate->getNumOperands() != 3)
      return failure();

    auto *priority = dyn_cast<toolchain::ConstantInt>(aggregate->getOperand(0));
    auto *func = dyn_cast<toolchain::Function>(aggregate->getOperand(1));
    auto *data = dyn_cast<toolchain::Constant>(aggregate->getOperand(2));
    if (!priority || !func || !data)
      return failure();

    auto *gv = dyn_cast_or_null<toolchain::GlobalValue>(data);
    Attribute dataAttr;
    if (gv)
...
SetVector<toolchain::Constant *>
ModuleImport::getConstantsToConvert(toolchain::Constant *constant) {
  ...
  SetVector<toolchain::Constant *> orderedSet;
  SetVector<toolchain::Constant *> workList;
  DenseMap<toolchain::Constant *, SmallVector<toolchain::Constant *>> adjacencyLists;
  workList.insert(constant);
  while (!workList.empty()) {
    toolchain::Constant *current = workList.back();
    // References of global objects are just pointers to the object. Avoid
    // walking the elements of these here.
    if (isa<toolchain::GlobalObject>(current) || isa<toolchain::GlobalAlias>(current)) {
    ...
    if (inserted) {
      ...
      for (toolchain::Value *operand : current->operands())
        if (auto *constDependency = dyn_cast<toolchain::Constant>(operand))
      ...
      if (auto *constAgg = dyn_cast<toolchain::ConstantAggregateZero>(current)) {
    ...
    // enqueued before.
    toolchain::Constant *dependency = adjacencyIt->getSecond().pop_back_val();
...
FailureOr<Value> ModuleImport::convertConstant(toolchain::Constant *constant) {
  ...
  if (auto *globalObj = dyn_cast<toolchain::GlobalObject>(constant)) {
    ...
    if (globalName.empty())
      symbolRef =
          getOrCreateNamelessSymbolName(cast<toolchain::GlobalVariable>(globalObj));
    else
  ...
  if (auto *globalAliasObj = dyn_cast<toolchain::GlobalAlias>(constant)) {
  ...
  if (auto *constExpr = dyn_cast<toolchain::ConstantExpr>(constant)) {
    ...
    toolchain::Instruction *inst = constExpr->getAsInstruction();
    ...
    assert(toolchain::all_of(inst->operands(), [&](toolchain::Value *value) {
  ...
  if (auto *constAgg = dyn_cast<toolchain::ConstantAggregate>(constant)) {
    ...
    for (const auto &it : toolchain::enumerate(elementValues)) {
      if (isArrayOrStruct) {
        root =
  ...
  if (auto *constTargetNone = dyn_cast<toolchain::ConstantTargetNone>(constant)) {
    ...
    // Create toolchain.mlir.zero operation to represent zero-initialization of
    // target extension type.
    return LLVM::ZeroOp::create(builder, loc, targetExtType).getRes();
  }

  if (auto *blockAddr = dyn_cast<toolchain::BlockAddress>(constant)) {
    auto fnSym =
  ...
  if (isa<toolchain::ConstantPtrAuth>(constant))
    error = " since ptrauth(...) is unsupported";

  if (isa<toolchain::NoCFIValue>(constant))
    error = " since no_cfi is unsupported";

  if (isa<toolchain::GlobalValue>(constant))
    error = " since global value is unsupported";

  return emitError(loc) << "unhandled constant: " << diag(*constant) << error;
}

FailureOr<Value> ModuleImport::convertConstantExpr(toolchain::Constant *constant) {
  ...
  SetVector<toolchain::Constant *> constantsToConvert =
      getConstantsToConvert(constant);
  for (toolchain::Constant *constantToConvert : constantsToConvert) {
...
FailureOr<Value> ModuleImport::convertValue(toolchain::Value *value) {
  assert(!isa<toolchain::MetadataAsValue>(value) &&
  ...
  if (auto *constant = dyn_cast<toolchain::Constant>(value))
    return convertConstantExpr(constant);

  Location loc = UnknownLoc::get(context);
  if (auto *inst = dyn_cast<toolchain::Instruction>(value))
    loc = translateLoc(inst->getDebugLoc());
  return emitError(loc) << "unhandled value: " << diag(*value);
}

FailureOr<Value> ModuleImport::convertMetadataValue(toolchain::Value *value) {
  // A value may be wrapped as metadata, for example, when passed to a debug
  // intrinsic. Unwrap these values before the conversion.
  auto *nodeAsVal = dyn_cast<toolchain::MetadataAsValue>(value);
  if (!nodeAsVal)
    return failure();
  auto *node = dyn_cast<toolchain::ValueAsMetadata>(nodeAsVal->getMetadata());
  if (!node)
    return failure();
  value = node->getValue();
...
ModuleImport::convertValues(ArrayRef<toolchain::Value *> values) {
  SmallVector<Value> remapped;
  remapped.reserve(values.size());
  for (toolchain::Value *value : values) {
...
LogicalResult ModuleImport::convertIntrinsicArguments(
    ArrayRef<toolchain::Value *> values, ArrayRef<toolchain::OperandBundleUse> opBundles,
  ...
  for (toolchain::Value *value : operands) {
    if (!value)
      continue;
...
IntegerAttr ModuleImport::matchIntegerAttr(toolchain::Value *value) {
...
FloatAttr ModuleImport::matchFloatAttr(toolchain::Value *value) {
  FloatAttr floatAttr;
  FailureOr<Value> converted = convertValue(value);
  bool success =
...
DILocalVariableAttr ModuleImport::matchLocalVariableAttr(
    toolchain::PointerUnion<toolchain::Value *, toolchain::DILocalVariable *> valOrVariable) {
  toolchain::DILocalVariable *node = nullptr;
  if (auto *value = dyn_cast<toolchain::Value *>(valOrVariable)) {
    auto *nodeAsVal = cast<toolchain::MetadataAsValue>(value);
    node = cast<toolchain::DILocalVariable>(nodeAsVal->getMetadata());
  } else {
    node = cast<toolchain::DILocalVariable *>(valOrVariable);
  }
  return debugImporter->translate(node);
}

DILabelAttr ModuleImport::matchLabelAttr(toolchain::Value *value) {
  auto *nodeAsVal = cast<toolchain::MetadataAsValue>(value);
  auto *node = cast<toolchain::DILabel>(nodeAsVal->getMetadata());
  return debugImporter->translate(node);
}

FPExceptionBehaviorAttr
ModuleImport::matchFPExceptionBehaviorAttr(toolchain::Value *value) {
  auto *metadata = cast<toolchain::MetadataAsValue>(value);
  auto *mdstr = cast<toolchain::MDString>(metadata->getMetadata());
  std::optional<toolchain::fp::ExceptionBehavior> optLLVM =
      toolchain::convertStrToExceptionBehavior(mdstr->getString());
...
RoundingModeAttr ModuleImport::matchRoundingModeAttr(toolchain::Value *value) {
  auto *metadata = cast<toolchain::MetadataAsValue>(value);
  auto *mdstr = cast<toolchain::MDString>(metadata->getMetadata());
  std::optional<toolchain::RoundingMode> optLLVM =
      toolchain::convertStrToRoundingMode(mdstr->getString());
...
FailureOr<SmallVector<AliasScopeAttr>>
ModuleImport::matchAliasScopeAttrs(toolchain::Value *value) {
  auto *nodeAsVal = cast<toolchain::MetadataAsValue>(value);
  auto *node = cast<toolchain::MDNode>(nodeAsVal->getMetadata());
  return lookupAliasScopeAttrs(node);
}

Location ModuleImport::translateLoc(toolchain::DILocation *loc) {
  return debugImporter->translateLoc(loc);
}

LogicalResult
ModuleImport::convertBranchArgs(toolchain::Instruction *branch,
                                toolchain::BasicBlock *target,
                                SmallVectorImpl<Value> &blockArguments) {
  for (auto inst = target->begin(); isa<toolchain::PHINode>(inst); ++inst) {
    auto *phiInst = cast<toolchain::PHINode>(&*inst);
    toolchain::Value *value = phiInst->getIncomingValueForBlock(branch->getParent());
    FailureOr<Value> converted = convertValue(value);
    if (failed(converted))
      return failure();
...
FailureOr<SmallVector<Value>>
ModuleImport::convertCallOperands(toolchain::CallBase *callInst,
                                  bool allowInlineAsm) {
  ...
  // in InlineAsmOp.
  toolchain::Value *calleeOperand = callInst->getCalledOperand();
  if (!isa<toolchain::Function, toolchain::GlobalIFunc>(calleeOperand) && !isInlineAsm) {
  ...
  SmallVector<toolchain::Value *> args(callInst->args());
  FailureOr<SmallVector<Value>> arguments = convertValues(args);
  if (failed(arguments))
    return failure();

  toolchain::append_range(operands, *arguments);
  return operands;
}
...
checkFunctionTypeCompatibility(LLVMFunctionType callType,
  ...
  for (auto [operandType, argumentType] :
       toolchain::zip(callType.getParams(), calleeType.getParams()))
...
ModuleImport::convertFunctionType(toolchain::CallBase *callInst,
  ...
  toolchain::Value *calledOperand = callInst->getCalledOperand();
  FailureOr<LLVMFunctionType> callType =
      castOrFailure(convertType(callInst->getFunctionType()));
  if (failed(callType))
    return failure();
  auto *callee = dyn_cast<toolchain::Function>(calledOperand);

  toolchain::FunctionType *origCalleeType = nullptr;
  if (callee) {
    origCalleeType = callee->getFunctionType();
  } else if (auto *ifunc = dyn_cast<toolchain::GlobalIFunc>(calledOperand)) {
    origCalleeType = cast<toolchain::FunctionType>(ifunc->getValueType());
  }
...
FlatSymbolRefAttr ModuleImport::convertCalleeName(toolchain::CallBase *callInst) {
  toolchain::Value *calledOperand = callInst->getCalledOperand();
  if (isa<toolchain::Function, toolchain::GlobalIFunc>(calledOperand))
    return SymbolRefAttr::get(context, calledOperand->getName());
  return {};
}

LogicalResult ModuleImport::convertIntrinsic(toolchain::CallInst *inst) {
...
ArrayAttr
ModuleImport::convertAsmInlineOperandAttrs(const toolchain::CallBase &llvmCall) {
  const auto *ia = cast<toolchain::InlineAsm>(llvmCall.getCalledOperand());
  unsigned argIdx = 0;
  SmallVector<mlir::Attribute> opAttrs;
  bool hasIndirect = false;

  for (const toolchain::InlineAsm::ConstraintInfo &ci : ia->ParseConstraints()) {
    // Only deal with constraints that correspond to call arguments.
    if (ci.Type == toolchain::InlineAsm::isLabel || !ci.hasArg())
      continue;
    ...
    if (ci.isIndirect) {
      if (toolchain::Type *paramEltType = llvmCall.getParamElementType(argIdx)) {
...
LogicalResult ModuleImport::convertInstruction(toolchain::Instruction *inst) {
  // Convert all instructions that do not provide an MLIR builder.
  Location loc = translateLoc(inst->getDebugLoc());
  if (inst->getOpcode() == toolchain::Instruction::Br) {
    auto *brInst = cast<toolchain::BranchInst>(inst);

    SmallVector<Block *> succBlocks;
    SmallVector<SmallVector<Value>> succBlockArgs;
    for (auto i : toolchain::seq<unsigned>(0, brInst->getNumSuccessors())) {
      toolchain::BasicBlock *succ = brInst->getSuccessor(i);
  ...
  if (inst->getOpcode() == toolchain::Instruction::Switch) {
    auto *swInst = cast<toolchain::SwitchInst>(inst);
    ...
    toolchain::BasicBlock *defaultBB = swInst->getDefaultDest();
    ...
    for (const auto &it : toolchain::enumerate(swInst->cases())) {
      const toolchain::SwitchInst::CaseHandle &caseHandle = it.value();
      toolchain::BasicBlock *succBB = caseHandle.getCaseSuccessor();
  ...
  if (inst->getOpcode() == toolchain::Instruction::PHI) {
    Type type = convertType(inst->getType());
    mapValue(inst, builder.getInsertionBlock()->addArgument(
                       type, translateLoc(inst->getDebugLoc())));
    return success();
  }
  if (inst->getOpcode() == toolchain::Instruction::Call) {
    auto *callInst = cast<toolchain::CallInst>(inst);
    toolchain::Value *calledOperand = callInst->getCalledOperand();
    ...
    auto callOp = [&]() -> FailureOr<Operation *> {
      if (auto *asmI = dyn_cast<toolchain::InlineAsm>(calledOperand)) {
  ...
  if (inst->getOpcode() == toolchain::Instruction::LandingPad) {
    auto *lpInst = cast<toolchain::LandingPadInst>(inst);

    SmallVector<Value> operands;
    operands.reserve(lpInst->getNumClauses());
    for (auto i : toolchain::seq<unsigned>(0, lpInst->getNumClauses())) {
  ...
  if (inst->getOpcode() == toolchain::Instruction::Invoke) {
    auto *invokeInst = cast<toolchain::InvokeInst>(inst);
    ...
    // block.
    bool invokeResultUsedInPhi = toolchain::any_of(
        invokeInst->getNormalDest()->phis(), [&](const toolchain::PHINode &phi) {
    ...
    if (invokeResultUsedInPhi) {
      ...
      assert(toolchain::none_of(
                 normalArgs,
                 [&](Value val) { return val.getDefiningOp() == invokeOp; }) &&
             "An toolchain.invoke operation cannot pass its result as a block "
             "argument.");
  ...
  if (inst->getOpcode() == toolchain::Instruction::GetElementPtr) {
    auto *gepInst = cast<toolchain::GetElementPtrInst>(inst);
    ...
    // approach is that many unused `toolchain.mlir.constant` would be emitted
    // at first place.
    SmallVector<GEPArg> indices;
    for (toolchain::Value *operand : toolchain::drop_begin(gepInst->operand_values())) {
  ...
  if (inst->getOpcode() == toolchain::Instruction::IndirectBr) {
    auto *indBrInst = cast<toolchain::IndirectBrInst>(inst);
    ...
    SmallVector<Block *> succBlocks;
    SmallVector<SmallVector<Value>> succBlockArgs;
    for (auto i : toolchain::seq<unsigned>(0, indBrInst->getNumSuccessors())) {
      toolchain::BasicBlock *succ = indBrInst->getSuccessor(i);
      SmallVector<Value> blockArgs;
    ...
    SmallVector<ValueRange> succBlockArgsRange =
        toolchain::to_vector_of<ValueRange>(succBlockArgs);
    Location loc = translateLoc(inst->getDebugLoc());
...
LogicalResult ModuleImport::processInstruction(toolchain::Instruction *inst) {
  ...
  if (auto *intrinsic = dyn_cast<toolchain::IntrinsicInst>(inst))
  ...
  if (inst->DebugMarker) {
    for (toolchain::DbgRecord &dbgRecord : inst->DebugMarker->getDbgRecordRange()) {
      // Store debug variable records for later processing.
      if (auto *dbgVariableRecord =
              dyn_cast<toolchain::DbgVariableRecord>(&dbgRecord)) {
      ...
      auto emitUnsupportedWarning = [&]() -> LogicalResult {
        if (!emitExpensiveWarnings)
          return success();
        std::string options;
        toolchain::raw_string_ostream optionsStream(options);
      ...
      if (auto *dbgLabelRecord = dyn_cast<toolchain::DbgLabelRecord>(&dbgRecord)) {
...
FlatSymbolRefAttr ModuleImport::getPersonalityAsAttr(toolchain::Function *f) {
  if (!f->hasPersonalityFn())
    return nullptr;

  toolchain::Constant *pf = f->getPersonalityFn();
  ...
  if (auto *ce = dyn_cast<toolchain::ConstantExpr>(pf)) {
    if (ce->getOpcode() == toolchain::Instruction::BitCast &&
        ce->getType() == toolchain::PointerType::getUnqual(f->getContext())) {
      if (auto *func = dyn_cast<toolchain::Function>(ce->getOperand(0)))
...
static void processMemoryEffects(toolchain::Function *func, LLVMFuncOp funcOp) {
  toolchain::MemoryEffects memEffects = func->getMemoryEffects();

  auto othermem = convertModRefInfoFromLLVM(
      memEffects.getModRef(toolchain::MemoryEffects::Location::Other));
  auto argMem = convertModRefInfoFromLLVM(
      memEffects.getModRef(toolchain::MemoryEffects::Location::ArgMem));
  auto inaccessibleMem = convertModRefInfoFromLLVM(
      memEffects.getModRef(toolchain::MemoryEffects::Location::InaccessibleMem));
  auto errnoMem = convertModRefInfoFromLLVM(
      memEffects.getModRef(toolchain::MemoryEffects::Location::ErrnoMem));
  auto targetMem0 = convertModRefInfoFromLLVM(
      memEffects.getModRef(toolchain::MemoryEffects::Location::TargetMem0));
  auto targetMem1 = convertModRefInfoFromLLVM(
      memEffects.getModRef(toolchain::MemoryEffects::Location::TargetMem1));
  auto memAttr =
...
static void processPassthroughAttrs(toolchain::Function *func, LLVMFuncOp funcOp) {
  toolchain::AttributeSet funcAttrs = func->getAttributes().getAttributes(
      toolchain::AttributeList::AttrIndex::FunctionIndex);
...
void ModuleImport::processFunctionAttributes(toolchain::Function *func,
                                             LLVMFuncOp funcOp) {
  processMemoryEffects(func, funcOp);
  processPassthroughAttrs(func, funcOp);

  if (func->hasFnAttribute(toolchain::Attribute::NoInline))
    funcOp.setNoInline(true);
  if (func->hasFnAttribute(toolchain::Attribute::AlwaysInline))
    funcOp.setAlwaysInline(true);
  if (func->hasFnAttribute(toolchain::Attribute::InlineHint))
    funcOp.setInlineHint(true);
  if (func->hasFnAttribute(toolchain::Attribute::OptimizeNone))
    funcOp.setOptimizeNone(true);
  if (func->hasFnAttribute(toolchain::Attribute::Convergent))
    funcOp.setConvergent(true);
  if (func->hasFnAttribute(toolchain::Attribute::NoUnwind))
    funcOp.setNoUnwind(true);
  if (func->hasFnAttribute(toolchain::Attribute::WillReturn))
  ...
  toolchain::Attribute attr = func->getFnAttribute(toolchain::Attribute::VScaleRange);
  if (attr.isValid()) {
  ...
  if (toolchain::Attribute attr = func->getFnAttribute("target-cpu");
      attr.isStringAttribute())
    funcOp.setTargetCpuAttr(StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("tune-cpu");
      attr.isStringAttribute())
    funcOp.setTuneCpuAttr(StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("target-features");
      attr.isStringAttribute())
    funcOp.setTargetFeaturesAttr(
        LLVM::TargetFeaturesAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("reciprocal-estimates");
      attr.isStringAttribute())
    funcOp.setReciprocalEstimatesAttr(
        StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("prefer-vector-width");
      attr.isStringAttribute())
    funcOp.setPreferVectorWidth(attr.getValueAsString());

  if (toolchain::Attribute attr = func->getFnAttribute("no-infs-fp-math");
      attr.isStringAttribute())
    funcOp.setNoInfsFpMath(attr.getValueAsBool());

  if (toolchain::Attribute attr = func->getFnAttribute("no-nans-fp-math");
      attr.isStringAttribute())
    funcOp.setNoNansFpMath(attr.getValueAsBool());

  if (toolchain::Attribute attr = func->getFnAttribute("instrument-function-entry");
      attr.isStringAttribute())
    funcOp.setInstrumentFunctionEntry(
        StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("instrument-function-exit");
      attr.isStringAttribute())
    funcOp.setInstrumentFunctionExit(
        StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("no-signed-zeros-fp-math");
      attr.isStringAttribute())
    funcOp.setNoSignedZerosFpMath(attr.getValueAsBool());

  if (toolchain::Attribute attr = func->getFnAttribute("denormal-fp-math");
      attr.isStringAttribute())
    funcOp.setDenormalFpMathAttr(
        StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("denormal-fp-math-f32");
      attr.isStringAttribute())
    funcOp.setDenormalFpMathF32Attr(
        StringAttr::get(context, attr.getValueAsString()));

  if (toolchain::Attribute attr = func->getFnAttribute("fp-contract");
      attr.isStringAttribute())
    funcOp.setFpContractAttr(StringAttr::get(context, attr.getValueAsString()));

  if (func->hasUWTable()) {
    ::toolchain::UWTableKind uwtableKind = func->getUWTableKind();
...
DictionaryAttr
ModuleImport::convertArgOrResultAttrSet(toolchain::AttributeSet llvmAttrSet) {
  ...
  for (auto [llvmKind, mlirName] : getAttrKindToNameMapping()) {
    ...
    if (llvmAttr.hasKindAsEnum() &&
        llvmAttr.getKindAsEnum() == toolchain::Attribute::Captures) {
      if (toolchain::capturesNothing(llvmAttr.getCaptureInfo()))
    ...
    if (llvmAttr.isTypeAttribute())
      ...
      const toolchain::ConstantRange &value = llvmAttr.getValueAsConstantRange();
...
void ModuleImport::convertArgAndResultAttrs(toolchain::Function *func,
                                            LLVMFuncOp funcOp) {
  auto llvmAttrs = func->getAttributes();
  for (size_t i = 0, e = funcOp.getNumArguments(); i < e; ++i) {
    toolchain::AttributeSet llvmArgAttrs = llvmAttrs.getParamAttrs(i);
  ...
  // to the funcOp.
  toolchain::AttributeSet llvmResAttr = llvmAttrs.getRetAttrs();
...
void ModuleImport::convertArgAndResultAttrs(
    toolchain::CallBase *call, ArgAndResultAttrsOpInterface attrsOp,
  ...
  toolchain::AttributeList llvmAttrs = call->getAttributes();
  SmallVector<toolchain::AttributeSet> llvmArgAttrsSet;
...
static LogicalResult convertCallBaseAttributes(toolchain::CallBase *inst, Op op) {
  op.setCConv(convertCConvFromLLVM(inst->getCallingConv()));
  return success();
}

LogicalResult ModuleImport::convertInvokeAttributes(toolchain::InvokeInst *inst,
                                                    InvokeOp op) {
  return convertCallBaseAttributes(inst, op);
}

LogicalResult ModuleImport::convertCallAttributes(toolchain::CallInst *inst,
                                                  CallOp op) {
  ...
  // semantics too early.
  toolchain::AttributeList callAttrs = inst->getAttributes();

  op.setTailCallKind(convertTailCallKindFromLLVM(inst->getTailCallKind()));
  op.setConvergent(callAttrs.getFnAttr(toolchain::Attribute::Convergent).isValid());
  op.setNoUnwind(callAttrs.getFnAttr(toolchain::Attribute::NoUnwind).isValid());
  op.setWillReturn(callAttrs.getFnAttr(toolchain::Attribute::WillReturn).isValid());
  op.setNoInline(callAttrs.getFnAttr(toolchain::Attribute::NoInline).isValid());
  op.setAlwaysInline(
      callAttrs.getFnAttr(toolchain::Attribute::AlwaysInline).isValid());
  op.setInlineHint(callAttrs.getFnAttr(toolchain::Attribute::InlineHint).isValid());

  toolchain::MemoryEffects memEffects = inst->getMemoryEffects();
...
LogicalResult ModuleImport::processFunction(toolchain::Function *func) {
  clearRegionState();
  ...
  if (toolchain::MaybeAlign maybeAlign = func->getAlign())
  ...
  toolchain::df_iterator_default_set<toolchain::BasicBlock *> reachable;
  for (toolchain::BasicBlock *basicBlock : toolchain::depth_first_ext(func, reachable))
    (void)basicBlock;

  // Eagerly create all reachable blocks.
  SmallVector<toolchain::BasicBlock *> reachableBasicBlocks;
  for (toolchain::BasicBlock &basicBlock : *func) {
  ...
  for (const auto &it : toolchain::enumerate(func->args())) {
  ...
  for (toolchain::BasicBlock *basicBlock : blocks)
...
/// value.
static bool isMetadataKillLocation(toolchain::DbgVariableIntrinsic *dbgIntr) {
  if (!dbgIntr->isKillLocation())
    return false;
  toolchain::Value *value = dbgIntr->getArgOperand(0);
  auto *nodeAsVal = dyn_cast<toolchain::MetadataAsValue>(value);
  if (!nodeAsVal)
    return false;
  return !isa<toolchain::ValueAsMetadata>(nodeAsVal->getMetadata());
}
...
ModuleImport::processDebugOpArgumentsAndInsertionPt(
    Location loc,
    toolchain::function_ref<FailureOr<Value>()> convertArgOperandToValue,
    toolchain::Value *address,
    toolchain::PointerUnion<toolchain::Value *, toolchain::DILocalVariable *> variable,
    toolchain::DIExpression *expression, DominanceInfo &domInfo) {
...
LogicalResult
ModuleImport::processDebugIntrinsic(toolchain::DbgVariableIntrinsic *dbgIntr,
  ...
  if (isa<toolchain::DbgDeclareInst>(dbgIntr))
    op = LLVM::DbgDeclareOp::create(builder, loc, locVal, localVariableAttr,
                                    locationExprAttr);
  else if (isa<toolchain::DbgValueInst>(dbgIntr))
...
LogicalResult
ModuleImport::processDebugRecord(toolchain::DbgVariableRecord &dbgRecord,
                                 DominanceInfo &domInfo) {
  OpBuilder::InsertionGuard guard(builder);
  Location loc = translateLoc(dbgRecord.getDebugLoc());
  auto emitUnsupportedWarning = [&]() -> LogicalResult {
  ...
  auto convertArgOperandToValue = [&]() -> FailureOr<Value> {
    toolchain::Value *value = dbgRecord.getAddress();
...
LogicalResult ModuleImport::processDebugIntrinsics() {
  DominanceInfo domInfo;
  for (toolchain::Instruction *inst : debugIntrinsics) {
    auto *intrCall = cast<toolchain::DbgVariableIntrinsic>(inst);
...
LogicalResult ModuleImport::processDebugRecords() {
  DominanceInfo domInfo;
  for (toolchain::DbgVariableRecord *dbgRecord : dbgRecords)
...
LogicalResult ModuleImport::processBasicBlock(toolchain::BasicBlock *bb,
                                              Block *block) {
  builder.setInsertionPointToStart(block);
  for (toolchain::Instruction &inst : *bb) {
    if (failed(processInstruction(&inst)))
      return failure();
    ...
    } else if (inst.getOpcode() != toolchain::Instruction::PHI) {
      if (emitExpensiveWarnings) {
        Location loc = debugImporter->translateLoc(inst.getDebugLoc());
...
ModuleImport::lookupAccessGroupAttrs(const toolchain::MDNode *node) const {
  return loopAnnotationImporter->lookupAccessGroupAttrs(node);
}

LoopAnnotationAttr
ModuleImport::translateLoopAnnotationAttr(const toolchain::MDNode *node,
                                          Location loc) const {
  return loopAnnotationImporter->translateLoopAnnotation(node, loc);
}

FailureOr<DereferenceableAttr>
ModuleImport::translateDereferenceableAttr(const toolchain::MDNode *node,
                                           unsigned kindID) {
  Location loc = mlirModule.getLoc();
  ...
  auto *numBytesMD = dyn_cast<toolchain::ConstantAsMetadata>(node->getOperand(0));
  auto *numBytesCst = dyn_cast<toolchain::ConstantInt>(numBytesMD->getValue());
  ...
  bool mayBeNull = kindID == toolchain::LLVMContext::MD_dereferenceable_or_null;
...
OwningOpRef<ModuleOp> mlir::translateLLVMIRToModule(
    std::unique_ptr<toolchain::Module> llvmModule, MLIRContext *context,
  ...
  // registered before.
  assert(toolchain::is_contained(context->getAvailableDialects(),
