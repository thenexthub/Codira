...
static FailureOr<SmallVector<T>> tryToParseIntListImpl(StringRef token) {
  ...
  for (auto [result, token] : toolchain::zip(results, tokens))
...
LogicalResult DataLayoutImporter::tryToEmplaceManglingModeEntry(
    StringRef token, toolchain::StringLiteral manglingKey) {
...
DataLayoutImporter::tryToEmplaceAddrSpaceEntry(StringRef token,
                                               toolchain::StringLiteral spaceKey) {
...
mlir::translateDataLayout(const toolchain::DataLayout &dataLayout,
                          MLIRContext *context) {
