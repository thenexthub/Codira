...
static void moveFuncDeclarationsToTop(spirv::ModuleOp moduleOp) {
  ...
  for (Operation &op : toolchain::drop_begin(ops))
...
LogicalResult Serializer::serialize() {
  LLVM_DEBUG(toolchain::dbgs() << "+++ starting serialization +++\n");
  ...
  LLVM_DEBUG(toolchain::dbgs() << "+++ completed serialization +++\n");
  return success();
}
...
LogicalResult Serializer::processExtension() {
  toolchain::SmallVector<uint32_t, 16> extName;
  toolchain::SmallSet<Extension, 4> deducedExts(
      toolchain::from_range, module.getVceTriple()->getExtensions());
...
static std::string getDecorationName(StringRef attrName) {
  ...
  return toolchain::convertToCamelFromSnakeCase(attrName, /*capitalizeFirst=*/true);
}
...
LogicalResult Serializer::prepareBasicType(
  ...
  if (auto imageType = dyn_cast<spirv::ImageType>(type)) {
    ...
    toolchain::append_values(operands, sampledTypeID,
  ...
  if (auto structType = dyn_cast<spirv::StructType>(type)) {
    ...
    for (auto elementIndex :
         toolchain::seq<uint32_t>(0, structType.getNumElements())) {
      uint32_t elementTypeID = 0;
  ...
  if (auto matrixType = dyn_cast<spirv::MatrixType>(type)) {
    ...
    toolchain::append_values(operands, elementTypeID, matrixType.getNumColumns());
    return success();
  }
  ...
  if (auto tensorArmType = dyn_cast<TensorArmType>(type)) {
    ...
    if (tensorArmType.hasRank()) {
      ...
      bool shaped = toolchain::all_of(dims, [](const auto &dim) { return dim > 0; });
      if (rank > 0 && shaped) {
...
uint32_t Serializer::prepareConstantInt(Location loc, IntegerAttr intAttr,
  ...
  case 64: {
    ...
    if (isSigned) {
      words = toolchain::bit_cast<DoubleWord>(value.getSExtValue());
    } else {
      words = toolchain::bit_cast<DoubleWord>(value.getZExtValue());
    }
  ...
  default: {
    std::string valueStr;
    toolchain::raw_string_ostream rss(valueStr);
...
uint32_t Serializer::prepareConstantFp(Location loc, FloatAttr floatAttr,
  ...
  const toolchain::fltSemantics *semantics = &value.getSemantics();

  auto opcode =
      isSpec ? spirv::Opcode::OpSpecConstant : spirv::Opcode::OpConstant;

  if (semantics == &APFloat::IEEEsingle()) {
    uint32_t word = toolchain::bit_cast<uint32_t>(value.convertToFloat());
    encodeInstructionInto(typesGlobalValues, opcode, {typeID, resultID, word});
  } else if (semantics == &APFloat::IEEEdouble()) {
    struct DoubleWord {
      uint32_t word1;
      uint32_t word2;
    } words = toolchain::bit_cast<DoubleWord>(value.convertToDouble());
    encodeInstructionInto(typesGlobalValues, opcode,
                          {typeID, resultID, words.word1, words.word2});
...
Serializer::processBlock(Block *block, bool omitLabel,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "processing block " << block << ":\n");
  LLVM_DEBUG(block->print(toolchain::dbgs()));
  LLVM_DEBUG(toolchain::dbgs() << '\n');
  if (!omitLabel) {
    uint32_t blockID = getOrCreateBlockID(block);
    LLVM_DEBUG(printBlock(block, toolchain::dbgs()));
  ...
  // in this block.
  if (emitMerge &&
      toolchain::any_of(block->getOperations(),
                   toolchain::IsaPred<spirv::LoopOp, spirv::SelectionOp>)) {
    if (failed(emitMerge()))
      return failure();
    emitMerge = nullptr;
  ...
  for (Operation &op : toolchain::drop_end(*block)) {
    if (failed(processOperation(&op)))
      return failure();
  }
...
LogicalResult Serializer::emitPhiForBlockArguments(Block *block) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "emitting phi instructions..\n");
  ...
  for (Block *mlirPredecessor : block->getPredecessors()) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "  mlir predecessor ");
    LLVM_DEBUG(printBlock(mlirPredecessor, toolchain::dbgs()));
    LLVM_DEBUG(toolchain::dbgs() << "    terminator: " << *terminator << "\n");
    ...
    LLVM_DEBUG(toolchain::dbgs() << "  spirv predecessor ");
    LLVM_DEBUG(printBlock(spirvPredecessor, toolchain::dbgs()));
    if (auto branchOp = dyn_cast<spirv::BranchOp>(terminator)) {
      ...
      if (block == switchOp.getDefaultTarget()) {
        ...
        auto it = toolchain::find(targets, block);
    ...
    LLVM_DEBUG({
      toolchain::dbgs() << "    block arguments:\n";
  ...
  for (auto argIndex : toolchain::seq<unsigned>(0, block->getNumArguments())) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "[phi] for block argument #" << argIndex << ' '
                            << arg << " (id = " << phiID << ")\n");
    ...
    for (auto predIndex : toolchain::seq<unsigned>(0, predecessors.size())) {
      Value value = predecessors[predIndex].second[argIndex];
      uint32_t predBlockId = getOrCreateBlockID(predecessors[predIndex].first);
      LLVM_DEBUG(toolchain::dbgs() << "[phi] use predecessor (id = " << predBlockId
                              << ") value " << value << ' ');
      ...
      if (valueId == 0) {
        ...
        LLVM_DEBUG(toolchain::dbgs() << "(need to fix)\n");
        deferredPhiValues[value].push_back(functionBody.size() + 1 +
                                           phiArgs.size());
      } else {
        LLVM_DEBUG(toolchain::dbgs() << "(id = " << valueId << ")\n");
      }
...
LogicalResult Serializer::processOperation(Operation *opInst) {
  LLVM_DEBUG(toolchain::dbgs() << "[op] '" << opInst->getName() << "'\n");
...
LogicalResult Serializer::emitDecoration(uint32_t target,
  ...
  toolchain::append_range(decorations, params);
  return success();
}
