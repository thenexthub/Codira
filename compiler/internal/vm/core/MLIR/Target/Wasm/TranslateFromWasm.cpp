...
public:
  ...
  /// Writes output to toolchain::dbgs().
...
private:
  ...
  toolchain::FailureOr<FunctionType> getFuncTypeFor(OpBuilder &builder,
                                               EmptyBlockMarker) {
  ...
  toolchain::FailureOr<FunctionType>
  ...
  getFuncTypeFor(OpBuilder &builder,
                 toolchain::FailureOr<BlockTypeParseResult> parseResult) {
    if (toolchain::failed(parseResult))
      return failure();
    return getFuncTypeFor(builder, *parseResult);
  }

  toolchain::FailureOr<FunctionType> parseBlockFuncType(OpBuilder &builder);
...
public:
  ...
  FailureOr<toolchain::SmallVector<Value>> popOperands(TypeRange operandTypes) {
...
public:
  ...
  toolchain::FailureOr<BlockTypeParseResult> parseBlockType(MLIRContext *ctx) {
...
FailureOr<float> ParserHead::parseLiteral<float>() {
  ...
  if (failed(bytes))
    return failure();
  return toolchain::support::endian::read<float>(bytes->bytes_begin(),
...
FailureOr<double> ParserHead::parseLiteral<double>() {
  ...
  if (failed(bytes))
    return failure();
  return toolchain::support::endian::read<double>(bytes->bytes_begin(),
                                             toolchain::endianness::little);
}
...
FailureOr<uint32_t> ParserHead::parseLiteral<uint32_t>() {
  ...
  uint64_t decoded = toolchain::decodeULEB128(src.bytes_begin(), &encodingSize,
                                         src.bytes_end(), &error);
  if (error)
...
FailureOr<int32_t> ParserHead::parseLiteral<int32_t>() {
  ...
  unsigned encodingSize{0};
  StringRef src = curHead();
  int64_t decoded = toolchain::decodeSLEB128(src.bytes_begin(), &encodingSize,
                                        src.bytes_end(), &error);
  if (error)
    return emitError(getLocation(), error);
...
FailureOr<int64_t> ParserHead::parseLiteral<int64_t>() {
  ...
  unsigned encodingSize{0};
  StringRef src = curHead();
  int64_t res = toolchain::decodeSLEB128(src.bytes_begin(), &encodingSize,
                                    src.bytes_end(), &error);
  if (error)
    return emitError(getLocation(), error);
...
void ValueStack::dump() const {
  toolchain::dbgs() << "================= Wasm ValueStack =======================\n";
  toolchain::dbgs() << "size: " << size() << "\n";
  toolchain::dbgs() << "nbFrames: " << labelLevel.size() << '\n';
  toolchain::dbgs() << "<Top>"
               << "\n";
  ...
  for (size_t idx = 0; idx < stackSize; ++idx) {
    ...
    while (nextFrameIdx && (nextFrameIdx->second > actualIdx)) {
      toolchain::dbgs() << "  --------------- Frame (" << nextFrameIdx->first
                   << ")\n";
...
parsed_inst_t ExpressionParser::parseBlockLikeOp(OpBuilder &builder) {
  ...
  toolchain::SmallVector<Location> locations{};
...
ExpressionParser::parseSpecificInstruction<WasmBinaryEncoding::OpCode::call>(
  ...
  toolchain::ArrayRef<Type> inTypes = callee.functionType.getInputs();
  toolchain::ArrayRef<Type> resTypes = callee.functionType.getResults();
...
public:
  WasmBinaryParser(toolchain::SourceMgr &sourceMgr, MLIRContext *ctx)
      : builder{ctx}, ctx{ctx} {
...
namespace mlir::wasm {
OwningOpRef<ModuleOp> importWebAssemblyToModule(toolchain::SourceMgr &source,
                                                MLIRContext *context) {
