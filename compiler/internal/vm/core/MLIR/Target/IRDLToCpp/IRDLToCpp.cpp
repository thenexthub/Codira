...
static std::string joinNameList(toolchain::ArrayRef<std::string> names) {
  std::string nameArray;
  toolchain::raw_string_ostream nameArrayStream(nameArray);
  nameArrayStream << "{\"" << toolchain::join(names, "\", \"") << "\"}";
...
static std::string typeToCppName(irdl::TypeOp type) {
  return toolchain::formatv("{0}Type",
...
static OpStrings getStrings(irdl::OperationOp op) {
  ...
  if (operandOp) {
    strings.opOperandNames = SmallVector<std::string>(
        toolchain::map_range(operandOp->getNames(), [](Attribute attr) {
          return toolchain::formatv("{0}", cast<StringAttr>(attr));
        }));
  }

  if (resultOp) {
    strings.opResultNames = SmallVector<std::string>(
        toolchain::map_range(resultOp->getNames(), [](Attribute attr) {
          return toolchain::formatv("{0}", cast<StringAttr>(attr));
        }));
  }
  ...
  if (regionsOp) {
    strings.opRegionNames = SmallVector<std::string>(
        toolchain::map_range(regionsOp->getNames(), [](Attribute attr) {
          return toolchain::formatv("{0}", cast<StringAttr>(attr));
        }));
  }
...
static LogicalResult generateTypedefList(irdl::DialectOp &dialect,
  ...
  auto range = toolchain::map_range(typeOps, typeToCppName);
...
static LogicalResult generateOpList(irdl::DialectOp &dialect,
  ...
  auto range = toolchain::map_range(operationOps, opToCppName);
...
static void generateOpGetterDeclarations(irdl::detail::dictionary &dict,
  ...
  for (size_t i = 0, end = opStrings.opOperandNames.size(); i < end; ++i) {
    const auto op =
        toolchain::convertToCamelFromSnakeCase(opStrings.opOperandNames[i], true);
    opGetters += toolchain::formatv("::mlir::Value get{0}() { return "
                               "getStructuredOperands({1}).front(); }\n  ",
                               op, i);
  }
  for (size_t i = 0, end = opStrings.opResultNames.size(); i < end; ++i) {
    const auto op =
        toolchain::convertToCamelFromSnakeCase(opStrings.opResultNames[i], true);
    resGetters += toolchain::formatv(
        R"(::mlir::Value get{0}() { return ::toolchain::cast<::mlir::Value>(getStructuredResults({1}).front()); }
  )",
        op, i);
  }
  ...
  for (size_t i = 0, end = opStrings.opRegionNames.size(); i < end; ++i) {
    const auto op =
        toolchain::convertToCamelFromSnakeCase(opStrings.opRegionNames[i], true);
    regionAdaptorGetters += toolchain::formatv(
        R"(::mlir::Region &get{0}() { return *getRegions()[{1}]; }
  )",
        op, i);
    regionGetters += toolchain::formatv(
...
static void generateOpBuilderDeclarations(irdl::detail::dictionary &dict,
  ...
  toolchain::raw_string_ostream stream{buildDecls};

  auto resultParams =
      toolchain::join(toolchain::map_range(opStrings.opResultNames,
                                 [](StringRef name) -> std::string {
                                   return toolchain::formatv(
                                       "::mlir::Type {0}, ",
                                       toolchain::convertToCamelFromSnakeCase(name));
                                 }),
                 "");

  auto operandParams =
      toolchain::join(toolchain::map_range(opStrings.opOperandNames,
                                 [](StringRef name) -> std::string {
                                   return toolchain::formatv(
                                       "::mlir::Value {0}, ",
                                       toolchain::convertToCamelFromSnakeCase(name));
  ...
  stream << toolchain::formatv(
      R"(static void build(::mlir::OpBuilder &opBuilder, ::mlir::OperationState &opState, {0} {1} ::toolchain::ArrayRef<::mlir::NamedAttribute> attributes = {{});)",
      resultParams, operandParams);
  stream << "\n";
  stream << toolchain::formatv(
      R"(static {0} create(::mlir::OpBuilder &opBuilder, ::mlir::Location location, {1} {2} ::toolchain::ArrayRef<::mlir::NamedAttribute> attributes = {{});)",
      opStrings.opCppName, resultParams, operandParams);
  stream << "\n";
  stream << toolchain::formatv(
      R"(static {0} create(::mlir::ImplicitLocOpBuilder &opBuilder, {1} {2} ::toolchain::ArrayRef<::mlir::NamedAttribute> attributes = {{});)",
      opStrings.opCppName, resultParams, operandParams);
  stream << "\n";
...
static SmallVector<std::string> generateTraits(irdl::OperationOp op,
  ...
  if (!strings.opRegionNames.empty()) {
    cppTraitNames.push_back(
        toolchain::formatv("::mlir::OpTrait::NRegions<{0}>::Impl",
...
#include "Templates/PerOperationDecl.txt"
  ...
  if (traitNames.empty())
    ...
    dict["OP_TEMPLATE_ARGS"] = toolchain::formatv("{0}, {1}", opStrings.opCppName,
                                             toolchain::join(traitNames, ", "));
...
#include "Templates/TypeHeaderDecl.txt"
  ...
  auto classDeclarations =
      toolchain::join(toolchain::map_range(opNames,
                                 [](toolchain::StringRef name) -> std::string {
                                   return toolchain::formatv("class {0};", name);
                                 }),
                 "\n");
  const auto forwardDeclarations = toolchain::formatv(
...
static void generateRegionConstraintVerifiers(
  ...
  for (size_t i = 0; i < strings.opRegionNames.size(); ++i) {
    ...
    std::string helperFnName =
        toolchain::formatv("__mlir_irdl_local_region_constraint_{0}_{1}",
    ...
    if (auto regionDefOp =
      ...
      if (auto blockCount = regionDefOp.getNumberOfBlocks()) {
        conditionParts.push_back(
            toolchain::formatv("region.getBlocks().size() == {0}",
                          blockCount.value())
                .str());
        descriptionParts.push_back(
            toolchain::formatv("exactly {0} block(s)", blockCount.value()).str());
      }
      ...
      if (regionDefOp.getConstrainedArguments()) {
        ...
        conditionParts.push_back(
            toolchain::formatv("region.getNumArguments() == {0}", expectedArgCount)
                .str());
        descriptionParts.push_back(
            toolchain::formatv("{0} entry block argument(s)", expectedArgCount)
                .str());
      }

      // Combine conditions
      if (!conditionParts.empty()) {
        condition = toolchain::join(conditionParts, " && ");
      }

      // Generate descriptive error message
      if (!descriptionParts.empty()) {
        textualConditionName =
            toolchain::formatv("region with {0}",
                          toolchain::join(descriptionParts, " and "))
                .str();
      }
    }

    verifierHelpers.push_back(toolchain::formatv(
        R"(static ::toolchain::LogicalResult {0}(::mlir::Operation *op, ::mlir::Region &region, ::toolchain::StringRef regionName, unsigned regionIndex) {{
    if (!({1})) {{
  ...
  return ::mlir::success();
    ...
    verifierCalls.push_back(toolchain::formatv(R"(
...
static void generateVerifiers(irdl::detail::dictionary &dict,
  ...
  std::string verifierDef =
      toolchain::formatv(R"(
::toolchain::LogicalResult {0}::verifyInvariantsImpl() {{
  ...
  return ::mlir::success();
                    ...
                    strings.opCppName, toolchain::join(verifierCalls, "\n"));

  dict["OP_VERIFIER_HELPERS"] = toolchain::join(verifierHelpers, "\n");
...
#include "Templates/PerOperationDef.txt"
  ...
  auto resultTypes = toolchain::join(
      toolchain::map_range(opStrings.opResultNames,
                      [](StringRef attr) -> std::string {
                        return toolchain::formatv("::mlir::Type {0}, ", attr);
                      }),
      "");
  auto operandTypes = toolchain::join(
      toolchain::map_range(opStrings.opOperandNames,
                      [](StringRef attr) -> std::string {
                        return toolchain::formatv("::mlir::Value {0}, ", attr);
                      }),
      "");
  auto operandAdder =
      toolchain::join(toolchain::map_range(opStrings.opOperandNames,
                 ...
                 "\n");
  auto resultAdder = toolchain::join(
      toolchain::map_range(opStrings.opResultNames,
                      [](StringRef attr) -> std::string {
                        return toolchain::formatv("  opState.addTypes({0});", attr);
                      }),
      "\n");

  const auto buildDefinition = toolchain::formatv(
      R"(
void {0}::build(::mlir::OpBuilder &opBuilder, ::mlir::OperationState &opState, {1} {2} ::toolchain::ArrayRef<::mlir::NamedAttribute> attributes) {{
{3}
{4}
}

{0} {0}::create(::mlir::OpBuilder &opBuilder, ::mlir::Location location, {1} {2} ::toolchain::ArrayRef<::mlir::NamedAttribute> attributes) {{
  ::mlir::OperationState __state__(location, getOperationName());
  build(opBuilder, __state__, {5} {6} attributes);
  auto __res__ = ::toolchain::dyn_cast<{0}>(opBuilder.create(__state__));
  assert(__res__ && "builder didn't return the right type");
  return __res__;
}

{0} {0}::create(::mlir::ImplicitLocOpBuilder &opBuilder, {1} {2} ::toolchain::ArrayRef<::mlir::NamedAttribute> attributes) {{
  return create(opBuilder, opBuilder.getLoc(), {5} {6} attributes);
      ...
      toolchain::join(opStrings.opResultNames, ",") +
          (!opStrings.opResultNames.empty() ? "," : ""),
      toolchain::join(opStrings.opOperandNames, ",") +
...
generateTypeVerifierCase(StringRef name, const DialectStrings &dialectStrings) {
  return toolchain::formatv(
      R"(.Case({1}::{0}::getMnemonic(), [&](toolchain::StringRef, toolchain::SMLoc) {
...
#include "Templates/DialectDef.txt"
  ...
  dict["TYPE_LIST"] = toolchain::join(
      toolchain::map_range(typeNames,
                      [&dialectStrings](toolchain::StringRef name) -> std::string {
                        return toolchain::formatv(
  ...
  auto typeCase =
      toolchain::join(toolchain::map_range(typeNames, typeVerifierGenerator), "\n");

  dict["TYPE_PARSER"] = toolchain::formatv(
      R"(static ::mlir::OptionalParseResult generatedTypeParser(::mlir::AsmParser &parser, ::toolchain::StringRef *mnemonic, ::mlir::Type &value) {
  return ::mlir::AsmParser::KeywordSwitch<::mlir::OptionalParseResult>(parser)
    {0}
    .Default([&](toolchain::StringRef keyword, toolchain::SMLoc) {{
      *mnemonic = keyword;
      return std::nullopt;
    });
})",
  ...
  auto typePrintCase =
      toolchain::join(toolchain::map_range(typeNames,
                                 [&](toolchain::StringRef name) -> std::string {
                                   return toolchain::formatv(
                                       R"(.Case<{1}::{0}>([&](auto t) {
      ...
      return ::mlir::success();
                 ...
                 "\n");
  dict["TYPE_PRINTER"] = toolchain::formatv(
      R"(static ::toolchain::LogicalResult generatedTypePrinter(::mlir::Type def, ::mlir::AsmPrinter &printer) {
  return ::toolchain::TypeSwitch<::mlir::Type, ::toolchain::LogicalResult>(def)
    {0}
  ...
  const auto commaSeparatedOpList = toolchain::join(
      map_range(opNames,
                [&dialectStrings](toolchain::StringRef name) -> std::string {
                  return toolchain::formatv("{0}::{1}", dialectStrings.namespacePath,
                                       name);
                }),
      ",\n");
  ...
  const auto perOpDefinitions =
      toolchain::join(toolchain::map_range(operations, opDefinitionGenerator), "\n");
...
LogicalResult
irdl::translateIRDLDialectToCpp(toolchain::ArrayRef<irdl::DialectOp> dialects,
                                raw_ostream &output) {
  static const auto typeDefTempl = detail::Template(
#include "Templates/TypeDef.txt"
  );

  toolchain::SmallMapVector<DialectOp, DialectStrings, 2> dialectStringTable;
  ...
  for (auto dialect : dialects) {
    ...
    std::string namespaceClose;
    std::string namespacePath;
    toolchain::raw_string_ostream namespaceOpenStream(namespaceOpen);
    toolchain::raw_string_ostream namespaceCloseStream(namespaceClose);
    toolchain::raw_string_ostream namespacePathStream(namespacePath);
    for (auto &pathElement : namespaceAbsolutePath) {
      namespaceOpenStream << "namespace " << pathElement << " {\n";
      namespacePathStream << "::" << pathElement;
    }

    for (auto &pathElement : toolchain::reverse(namespaceAbsolutePath))
      namespaceCloseStream << "} // namespace " << pathElement << "\n";

    std::string cppShortName =
        toolchain::convertToCamelFromSnakeCase(dialectName, true);
    std::string dialectBaseTypeName = toolchain::formatv("{0}Type", cppShortName);
    std::string cppName = toolchain::formatv("{0}Dialect", cppShortName);
  ...
  output << toolchain::formatv("#ifdef {0}\n#undef {0}\n", declarationMacroFlag);
  for (auto dialect : dialects) {
  ...
  output << toolchain::formatv("#endif // #ifdef {}\n", declarationMacroFlag);
