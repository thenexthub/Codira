...
SerializeGPUModuleBase::compileToBinary(StringRef asmStr,
  ...
  std::string basename = toolchain::formatv(
  ...
  auto createTemp = [&](StringRef name,
    ...
    toolchain::SmallString<128> filePath;
    if (auto ec = toolchain::sys::fs::createTemporaryFile(name, suffix, filePath))
      return getGPUModuleOp().emitError()
             << "Couldn't create the temp file: `" << filePath
             << "`, error message: " << ec.message();

    return TmpFile(filePath, toolchain::FileRemover(filePath.c_str()));
  };
  // Create temp file
  ...
  std::error_code ec;
  {
    toolchain::raw_fd_ostream asmStream(asmFile->first, ec);
    if (ec)
  ...
  const std::string cmdOptsStr = "\"" + toolchain::join(cmdOpts.second, " ") + "\"";
...
#define DEBUG_TYPE "serialize-to-binary"
  LLVM_DEBUG({
    toolchain::dbgs() << "Tool invocation for module: "
                 << getGPUModuleOp().getNameAttr() << "\n";
    toolchain::interleave(oclocArgs, toolchain::dbgs(), " ");
    toolchain::dbgs() << "\n";
  });
#undef DEBUG_TYPE
  ...
  // Invoke ocloc.
  if (toolchain::sys::ExecuteAndWait(oclocCompiler.value(), oclocArgs, std::nullopt,
  ...
  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> binaryBuffer =
      toolchain::MemoryBuffer::getFile(binFile->first);
  if (!binaryBuffer)
...
private:
  ...
  translateToSPIRVBinary(toolchain::Module &llvmModule,
...
FailureOr<SmallVector<char, 0>>
SPIRVSerializer::moduleToObject(toolchain::Module &llvmModule) {
#define DEBUG_TYPE "serialize-to-toolchain"
  LLVM_DEBUG({
    toolchain::dbgs() << "LLVM IR for module: " << getGPUModuleOp().getNameAttr()
                 << "\n";
    toolchain::dbgs() << llvmModule << "\n";
...
#define DEBUG_TYPE "serialize-to-isa"
    LLVM_DEBUG({
      toolchain::dbgs() << "SPIR-V for module: " << getGPUModuleOp().getNameAttr()
                   << "\n";
      toolchain::dbgs() << *serializedISA << "\n";
      toolchain::dbgs().flush();
    });
#undef DEBUG_TYPE
...
std::optional<std::string>
SPIRVSerializer::translateToSPIRVBinary(toolchain::Module &llvmModule,
                                        toolchain::TargetMachine &targetMachine) {
  std::string targetISA;
  toolchain::raw_string_ostream stream(targetISA);
