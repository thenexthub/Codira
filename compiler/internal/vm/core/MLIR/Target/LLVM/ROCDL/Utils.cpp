...
mlir::ROCDL::getAMDHSAKernelsELFMetadata(Builder &builder,
  ...
  toolchain::StringMap<toolchain::offloading::amdgpu::AMDGPUKernelMetaData> kernels;
  toolchain::MemoryBufferRef buffer(StringRef(elfData.data(), elfData.size()),
                               "buffer");
  // Get the metadata.
  toolchain::Error error = toolchain::offloading::amdgpu::getAMDGPUMetaDataFromImage(
