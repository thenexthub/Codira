...
#include "vm/core/Config/Targets.h"
...
LogicalResult SerializeGPUModuleBase::appendStandardLibs() {
  ...
  if (!pathRef.empty()) {
    ...
    toolchain::sys::path::append(path, "nvvm", "libdevice", "libdevice.10.bc");
    pathRef = StringRef(path.data(), path.size());
...
private:
  using TmpFile = std::pair<toolchain::SmallString<128>, toolchain::FileRemover>;
...
NVPTXSerializer::createTemp(StringRef name, StringRef suffix) {
  ...
  std::error_code ec =
      toolchain::sys::fs::createTemporaryFile(name, suffix, filename);
  if (ec) {
    getOperation().emitError() << "Couldn't create the temp file: `" << filename
                               << "`, error message: " << ec.message();
    return std::nullopt;
  }
  return TmpFile(filename, toolchain::FileRemover(filename.c_str()));
}
...
std::optional<std::string> NVPTXSerializer::findTool(StringRef tool) {
  ...
  if (!pathRef.empty()) {
    path.insert(path.begin(), pathRef.begin(), pathRef.end());
    toolchain::sys::path::append(path, "bin", tool);
  ...
  if (std::optional<std::string> toolPath =
          toolchain::sys::Process::FindInEnvPath("PATH", tool))
    return *toolPath;
...
NVPTXSerializer::compileToBinary(StringRef ptxCode) {
  ...
  std::string basename =
      toolchain::formatv("mlir-{0}-{1}-{2}", getOperation().getNameAttr().getValue(),
  ...
  if (createFatbin) {
    ...
    cubinFile = TmpFile(cubinFilename, toolchain::FileRemover(cubinFilename));
  } else {
  ...
  {
    toolchain::raw_fd_ostream ptxStream(ptxFile->first, ec);
    if (ec)
  ...
  std::pair<toolchain::BumpPtrAllocator, SmallVector<const char *>> cmdOpts =
  ...
  std::string cubinArg =
      toolchain::formatv("--image3=kind=elf,sm={0},file={1}", chip, cubinFile.first)
          .str();
  // Embed the PTX file so the driver can JIT if needed.
  std::string ptxArg =
      toolchain::formatv("--image3=kind=ptx,sm={0},file={1}", chip, ptxFile->first)
          .str();
...
#define DEBUG_TYPE "serialize-to-binary"
  LDBG() << "Tool invocation for module: " << getOperation().getNameAttr()
         ...
         << "\nptxas args: " << toolchain::interleaved(ptxasArgs, " ");
  if (createFatbin)
    LDBG() << "fatbin args: " << toolchain::interleaved(fatbinArgs, " ");
#undef DEBUG_TYPE
  ...
  auto emitLogError =
    ...
    if (message.empty()) {
      toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> toolStderr =
          toolchain::MemoryBuffer::getFile(logFile->first);
      if (toolStderr)
  ...
  // Invoke PTXAS.
  if (toolchain::sys::ExecuteAndWait(ptxasCompiler.value(), ptxasArgs,
                                /*Env=*/std::nullopt,
                                /*Redirects=*/redirects,
                                /*SecondsToWait=*/0,
                                /*MemoryLimit=*/0,
...
#define DEBUG_TYPE "dump-sass"
  LLVM_DEBUG({
    ...
    if (toolchain::sys::ExecuteAndWait(nvdisasm.value(), nvdisasmArgs,
                                  /*Env=*/std::nullopt,
                                  /*Redirects=*/redirects,
                                  /*SecondsToWait=*/0,
                                  /*MemoryLimit=*/0,
    ...
    toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> logBuffer =
        toolchain::MemoryBuffer::getFile(logFile->first);
    if (logBuffer && !(*logBuffer)->getBuffer().empty()) {
      LDBG() << "Output:\n" << (*logBuffer)->getBuffer();
      toolchain::dbgs().flush();
    }
  });
#undef DEBUG_TYPE
  ...
  // Invoke `fatbin`.
  message.clear();
  if (createFatbin && toolchain::sys::ExecuteAndWait(*fatbinaryTool, fatbinArgs,
                                                /*Env=*/std::nullopt,
                                                /*Redirects=*/redirects,
                                                /*SecondsToWait=*/0,
                                                /*MemoryLimit=*/0,
...
#undef DEBUG_TYPE
  ...
  // Read the fatbin.
  toolchain::ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> binaryBuffer =
      toolchain::MemoryBuffer::getFile(binaryFile->first);
  if (!binaryBuffer)
...
#define RETURN_ON_NVPTXCOMPILER_ERROR(expr)                                    \
  do {                                                                         \
    if (auto status = (expr)) {                                                \
      emitError(loc) << toolchain::Twine(#expr).concat(" failed with error code ")  \
                     << status;                                                \
      return std::nullopt;                                                     \
    }                                                                          \
  } while (false)
...
#define RETURN_ON_NVFATBIN_ERROR(expr)                                         \
  do {                                                                         \
    ...
    if (result != nvFatbinResult::NVFATBIN_SUCCESS) {                          \
      emitError(loc) << toolchain::Twine(#expr).concat(" failed with error: ")      \
  ...
  LLVM_DEBUG({
    ...
    if (logSize != 0) {
      ...
      LDBG() << "NVPTX compiler invocation for module: "
             ...
             << "\nArguments: " << toolchain::interleaved(cmdOpts.second, " ")
             << "\nOutput\n"
             << log.data();
    }
  });
#undef DEBUG_TYPE
  ...
  if (targetOptions.getCompilationTarget() == gpu::CompilationTarget::Fatbin) {
    bool useFatbin32 = toolchain::any_of(cmdOpts.second, [](const char *option) {
      return toolchain::StringRef(option) == "-32";
    });
...
FailureOr<SmallVector<char, 0>>
NVPTXSerializer::moduleToObject(toolchain::Module &llvmModule) {
  toolchain::Timer moduleToObjectTimer(
      "moduleToObjectTimer",
      "Timer for perf toolchain-ir -> isa and isa -> binary.");
  toolchain::scope_exit clear([&]() { moduleToObjectTimer.clear(); });
...
NVVMTargetAttrImpl::createObject(Attribute attribute, Operation *module,
  ...
  for (const auto *perfName : {"LLVMIRToISATimeInMs", "ISAToBinaryTimeInMs"}) {
    if (module->hasAttr(perfName)) {
      IntegerAttr attr = toolchain::dyn_cast<IntegerAttr>(module->getAttr(perfName));
