...
using ValueMap = toolchain::ScopedHashTable<mlir::Value, std::string>;
...
struct ExpressionVisitor
  ...
  LogicalResult dispatchSMTOpVisitor(Operation *op, VisitorInfo &info) {
    ...
    if (res.hasOneUse() && options.inlineSingleUseValues) {
      std::string str;
      toolchain::raw_string_ostream sstream(str);
...
#define HANDLE_OP(OPTYPE, NAME, KIND)                                          \
  ...
  LogicalResult quantifierHelper(OpTy op, StringRef operatorString,
    ...
    toolchain::ScopedHashTableScope<Value, std::string> scope(info.valueMap);
    info.stream << "(" << operatorString << " (";
    StringLiteral delimiter = "";

    SmallVector<StringRef> argNames;

    for (auto [i, arg] : toolchain::enumerate(op.getBody().getArguments())) {
    ...
    if (!patterns.empty()) {
      ...
      for (auto &p : patterns) {
        ...
        for (auto [i, arg] : toolchain::enumerate(p.getArguments()))
...
struct StatementVisitor
  ...
  LogicalResult visitSMTOp(AssertOp op, mlir::raw_indented_ostream &stream,
                           ValueMap &valueMap) {
    toolchain::ScopedHashTableScope<Value, std::string> scope1(valueMap);
    SmallVector<Value> worklist;
...
static LogicalResult emit(SolverOp solver, const SMTEmissionOptions &options,
  ...
  ValueMap valueMap;
  toolchain::ScopedHashTableScope<Value, std::string> scope0(valueMap);
  Namespace names;
...
LogicalResult smt::exportSMTLIB(Operation *module, toolchain::raw_ostream &os,
...
void smt::registerExportSMTLIBTranslation() {
  static toolchain::cl::opt<bool> inlineSingleUseValues(
      "smtlibexport-inline-single-use-values",
      toolchain::cl::desc("Inline expressions that are used only once rather than "
