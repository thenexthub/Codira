...
static FailureOr<int> getOperatorPrecedence(Operation *operation) {
  return toolchain::TypeSwitch<Operation *, FailureOr<int>>(operation)
...
struct CppEmitter {
  ...
  private:
    toolchain::ScopedHashTableScope<Value, std::string> valueMapperScope;
    toolchain::ScopedHashTableScope<Block *, std::string> blockMapperScope;
...
private:
  using ValueMapper = toolchain::ScopedHashTable<Value, std::string>;
  using BlockMapper = toolchain::ScopedHashTable<Block *, std::string>;
  ...
  toolchain::ScopedHashTableScope<Value, std::string> defaultValueMapperScope;
  toolchain::ScopedHashTableScope<Block *, std::string> defaultBlockMapperScope;
...
static LogicalResult printOperation(CppEmitter &emitter,
  ...
  std::string out;
  toolchain::raw_string_ostream ss(out);
...
static LogicalResult printOperation(CppEmitter &emitter,
  ...
  for (auto pair : toolchain::zip(switchOp.getCases(), switchOp.getCaseRegions())) {
    os << "\ncase " << std::get<0>(pair) << ": {\n";
    os.indent();
...
static LogicalResult printOperation(CppEmitter &emitter,
  ...
  for (auto pair :
       toolchain::zip(branchOp.getOperands(), successor.getArguments())) {
...
static LogicalResult printOperation(CppEmitter &emitter,
  ...
  for (auto pair : toolchain::zip(condBranchOp.getTrueOperands(),
  ...
  for (auto pair : toolchain::zip(condBranchOp.getFalseOperands(),
...
static LogicalResult printFunctionBody(CppEmitter &emitter,
  ...
  for (Block &block : toolchain::drop_begin(blocks)) {
...
static LogicalResult printOperation(CppEmitter &emitter,
  ...
  if (toolchain::any_of(functionOp.getArgumentTypes(), toolchain::IsaPred<LValueType>)) {
    return functionOp.emitOpError()
           << "cannot emit lvalue type as argument type";
  }

  if (toolchain::any_of(functionOp.getResultTypes(), toolchain::IsaPred<ArrayType>)) {
...
LogicalResult CppEmitter::emitAttribute(Location loc, Attribute attr) {
  ...
  auto printFloat = [&](const APFloat &val) {
    if (val.isFinite()) {
      ...
      os << strValue;
      switch (toolchain::APFloatBase::SemanticsToEnum(val.getSemantics())) {
      case toolchain::APFloatBase::S_IEEEhalf:
        os << "f16";
        break;
      case toolchain::APFloatBase::S_BFloat:
        os << "bf16";
        break;
      case toolchain::APFloatBase::S_IEEEsingle:
        os << "f";
        break;
      case toolchain::APFloatBase::S_IEEEdouble:
        break;
      default:
...
CppEmitter::emitOperandsAndAttributes(Operation &op,
  ...
  if (op.getNumOperands() > 0) {
    for (NamedAttribute attr : op.getAttrs()) {
      if (!toolchain::is_contained(exclude, attr.getName().strref())) {
        os << ", ";
        break;
      }
    }
...
LogicalResult CppEmitter::emitType(Location loc, Type type) {
  ...
  if (auto fType = dyn_cast<FloatType>(type)) {
    ...
    case 16: {
      if (toolchain::isa<Float16Type>(type))
        return (os << "_Float16"), success();
      if (toolchain::isa<BFloat16Type>(type))
...
LogicalResult CppEmitter::emitTupleType(Location loc, ArrayRef<Type> types) {
  if (toolchain::any_of(types, toolchain::IsaPred<ArrayType>)) {
