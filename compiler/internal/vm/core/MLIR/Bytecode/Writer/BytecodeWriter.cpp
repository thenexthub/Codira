...
#include "vm/core/ADT/CachedHashString.h"
...
struct BytecodeWriterConfig::Impl {
  ...
  toolchain::StringMap<std::unique_ptr<DialectVersion>> dialectVersionMap;

  /// The producer of the bytecode.
  StringRef producer;

  /// Printer callbacks used to emit custom type and attribute encodings.
  toolchain::SmallVector<std::unique_ptr<AttrTypeBytecodeWriter<Attribute>>>
      attributeWriterCallbacks;
  toolchain::SmallVector<std::unique_ptr<AttrTypeBytecodeWriter<Type>>>
      typeWriterCallbacks;
...
public:
  ...
  void alignTo(unsigned alignment) {
    if (alignment < 2)
      return;
    assert(toolchain::isPowerOf2_32(alignment) && "expected valid alignment");

    // Check to see if we need to emit any padding bytes to meet the desired
    // alignment.
    size_t curOffset = size();
    size_t paddingSize = toolchain::alignTo(curOffset, alignment) - curOffset;
    while (paddingSize--)
  ...
  void emitBytes(ArrayRef<uint8_t> bytes, StringLiteral desc) {
    ...
    toolchain::append_range(currentResult, bytes);
  }
  ...
  void emitSection(bytecode::Section::ID code, EncodingEmitter &&emitter) {
    ...
    toolchain::append_range(prevResultList, emitter.prevResultList);
...
public:
  ...
  size_t insert(StringRef str) {
    auto it = strings.insert({toolchain::CachedHashStringRef(str), strings.size()});
  ...
  void write(EncodingEmitter &emitter) {
    ...
    for (const auto &it : toolchain::reverse(strings))
...
private:
  ...
  /// unused.
  toolchain::MapVector<toolchain::CachedHashStringRef, size_t> strings;
};
} // namespace

class DialectWriter : public DialectBytecodeWriter {
  using DialectVersionMapT = toolchain::StringMap<std::unique_ptr<DialectVersion>>;
...
private:
  ...
  ssize_t emit(ArrayRef<char> rawProperties) {
    ...
    SmallVector<char> sizeScratch;
    {
      ...
      toolchain::raw_svector_ostream os(sizeScratch);
      sizeEmitter.writeTo(os);
    }
    ...
    toolchain::append_range(newStorage, sizeScratch);
    toolchain::append_range(newStorage, rawProperties);
...
void EncodingEmitter::emitMultiByteVarInt(uint64_t value, StringLiteral desc) {
  ...
  for (size_t numBytes = 2; numBytes < 9; ++numBytes) {
    if (LLVM_LIKELY(it >>= 7) == 0) {
      ...
      toolchain::support::ulittle64_t encodedValueLE(encodedValue);
  ...
  emitByte(0, desc);
  toolchain::support::ulittle64_t valueLE(value);
...
private:
  ...
  LogicalResult writeRegions(EncodingEmitter &emitter,
    ...
    return success(toolchain::all_of(regions, [&](Region &region) {
...
static void writeDialectGrouping(EncodingEmitter &emitter, EntriesT &&entries,
  ...
  for (auto it = entries.begin(), e = entries.end(); it != e;) {
    ...
    for (auto &entry : toolchain::make_range(groupStart, it))
      callback(entry);
  }
}
...
void BytecodeWriter::writeDialectSection(EncodingEmitter &emitter) {
  ...
  dialectEmitter.emitVarInt(toolchain::size(dialects), "dialects count");
...
void BytecodeWriter::writeAttrTypeSection(EncodingEmitter &emitter) {
  ...
  offsetEmitter.emitVarInt(toolchain::size(numberingState.getAttributes()),
                           "attributes count");
  offsetEmitter.emitVarInt(toolchain::size(numberingState.getTypes()),
                           "types count");
...
void BytecodeWriter::writeUseListOrders(EncodingEmitter &emitter,
  ...
  DenseMap<unsigned, toolchain::SmallVector<unsigned>> map;
  for (auto item : toolchain::enumerate(range)) {
    ...
    toolchain::SmallVector<std::pair<unsigned, uint64_t>> useListPairs(
        {{0, prevID}});

    for (auto use : toolchain::drop_begin(toolchain::enumerate(value.getUses()))) {
    ...
    map.try_emplace(item.index(), toolchain::map_range(useListPairs, [](auto elem) {
                      return elem.first;
                    }));
  }
  ...
  for (const auto &item : map) {
    ...
    size_t shuffledElements =
        toolchain::count_if(toolchain::enumerate(useListOrder),
    ...
    if (indexPairEncoding) {
      ...
      for (auto pair : toolchain::enumerate(useListOrder)) {
