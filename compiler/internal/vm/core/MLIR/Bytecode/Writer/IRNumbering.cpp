...
static void groupByDialectPerByte(T range) {
  ...
  for (unsigned i = 1; i < 9; ++i) {
    ...
    toolchain::stable_sort(byteSubRange, [&](const auto &lhs, const auto &rhs) {
  ...
  for (auto [idx, value] : toolchain::enumerate(range))
    value->number = idx;
}
...
IRNumberingState::IRNumberingState(Operation *op,
  ...
  for (auto [idx, dialect] : toolchain::enumerate(dialects))
  ...
  toolchain::stable_sort(orderedAttrs, sortByRefCountFn);
  toolchain::stable_sort(orderedOpNames, sortByRefCountFn);
  toolchain::stable_sort(orderedTypes, sortByRefCountFn);
  ...
  groupByDialectPerByte(toolchain::MutableArrayRef(orderedAttrs));
  groupByDialectPerByte(toolchain::MutableArrayRef(orderedOpNames));
  groupByDialectPerByte(toolchain::MutableArrayRef(orderedTypes));
  ...
  rootOp->walk([&](Operation *op, const WalkStage &stage) {
    ...
    if (!opStack.empty() && opStack.back().hasUnresolvedIsolation) {
      ...
      for (Value operand : op->getOperands()) {
        ...
        for (auto &state : toolchain::make_range(opStack.rbegin(), it)) {
...
void IRNumberingState::number(Attribute attr) {
  ...
  toolchain::raw_null_ostream dummyOS;
...
void IRNumberingState::number(Region &region) {
  ...
  size_t blockCount = 0;
  for (auto it : toolchain::enumerate(region)) {
