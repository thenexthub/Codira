...
public:
  ...
  LogicalResult alignTo(unsigned alignment) {
    if (!toolchain::isPowerOf2_32(alignment))
    ...
    if (LLVM_UNLIKELY(isUnaligned(dataIt))) {
      return emitError("expected data iterator aligned to ", alignment,
                       ", but got pointer: '0x" +
                           toolchain::utohexstr((uintptr_t)dataIt) + "'");
    }
  ...
  LogicalResult parseVarInt(uint64_t &result) {
    ...
    if (LLVM_UNLIKELY(result == 0)) {
      toolchain::support::ulittle64_t resultLE;
...
private:
  ...
  LLVM_ATTRIBUTE_NOINLINE LogicalResult parseMultiByteVarInt(uint64_t &result) {
    ...
    // implementation).
    uint32_t numBytes = toolchain::countr_zero<uint32_t>(result);
    ...
    toolchain::support::ulittle64_t resultLE(result);
    if (failed(
...
static LogicalResult resolveEntry(EncodingReader &reader, RangeT &entries,
  ...
  if constexpr (std::is_convertible_v<toolchain::detail::ValueOfRange<RangeT>, T>)
    entry = entries[index];
  else
    entry = &entries[index];
  return success();
...
LogicalResult StringSectionReader::initialize(Location fileLoc,
  ...
  for (StringRef &string : toolchain::reverse(strings)) {
    uint64_t stringSize;
...
public:
  ...
  initialize(Location fileLoc, const ParserConfig &config,
             ...
             const std::shared_ptr<toolchain::SourceMgr> &bufferOwnerRef);
...
private:
  ...
  toolchain::StringMap<std::string> dialectResourceHandleRenamingMap;
};
...
public:
  ...
  parseAsBlob(BlobAllocatorFn allocator) const final {
    ...
    assert(toolchain::isAddrAligned(toolchain::Align(alignment), blob.getData().data()) &&
           blob.isMutable() &&
...
public:
  AttrTypeReader(const StringSectionReader &stringReader,
                 ...
                 const toolchain::StringMap<BytecodeDialect *> &dialectsMap,
  ...
  LogicalResult parseAttribute(EncodingReader &reader, T &result) {
    ...
    return reader.emitError("expected attribute of type: ",
                            toolchain::getTypeName<T>(), ", but got: ", baseResult);
  }
...
public:
  ...
  FailureOr<APFloat>
  readAPFloatWithKnownSemantics(const toolchain::fltSemantics &semantics) override {
  ...
  LogicalResult readBlob(ArrayRef<char> &result) override {
    ...
    if (failed(reader.parseVarInt(dataSize)) ||
      ...
      return failure();
    result = toolchain::ArrayRef(reinterpret_cast<const char *>(data.data()),
                            data.size());
    return success();
  }
...
public:
  ...
  LogicalResult initialize(Location fileLoc, ArrayRef<uint8_t> sectionData) {
    ...
    for (auto idx : toolchain::seq<int64_t>(0, count)) {
      (void)idx;
...
T AttrTypeReader::resolveEntry(SmallVectorImpl<Entry<T>> &entries,
  ...
  resolving = true;
  toolchain::scope_exit restoreResolving([&]() { resolving = oldResolving; });
  ...
  toolchain::DenseSet<std::pair<uint64_t, EntryKind>> inWorklist;
  ...
  for (auto entry : toolchain::reverse(deferredWorklist))
...
public:
  ...
  LogicalResult read(Block *block,
                     toolchain::function_ref<bool(Operation *)> lazyOps);
  ...
  materialize(Operation *op,
              toolchain::function_ref<bool(Operation *)> lazyOpsCallback) {
    this->lazyOpsCallback = lazyOpsCallback;
    toolchain::scope_exit resetlazyOpsCallback(
...
private:
  ...
  toolchain::StringMap<BytecodeDialect *> dialectsMap;
...
LogicalResult BytecodeReader::Impl::read(
    Block *block, toolchain::function_ref<bool(Operation *)> lazyOpsCallback) {
...
LogicalResult BytecodeReader::Impl::sortUseListOrder(Value value) {
  ...
  toolchain::SmallVector<std::pair<unsigned, uint64_t>> currentOrder = {{0, prevID}};
  for (auto item : toolchain::drop_begin(toolchain::enumerate(value.getUses()))) {
  ...
  if (!hasIncomingOrder) {
    ...
    SmallVector<unsigned> shuffle(toolchain::make_first_range(currentOrder));
  ...
  shuffle = SmallVector<unsigned, 4>(toolchain::map_range(
...
BytecodeReader::BytecodeReader(
    toolchain::MemoryBufferRef buffer, const ParserConfig &config, bool lazyLoading,
    const std::shared_ptr<toolchain::SourceMgr> &bufferOwnerRef) {
...
LogicalResult BytecodeReader::materialize(
    Operation *op, toolchain::function_ref<bool(Operation *)> lazyOpsCallback) {
...
bool mlir::isBytecode(toolchain::MemoryBufferRef buffer) {
...
static LogicalResult
readBytecodeFileImpl(toolchain::MemoryBufferRef buffer, Block *block,
                     const ParserConfig &config,
                     const std::shared_ptr<toolchain::SourceMgr> &bufferOwnerRef) {
  Location sourceFileLoc =
      FileLineColLoc::get(config.getContext(), buffer.getBufferIdentifier(),
...
LogicalResult mlir::readBytecodeFile(toolchain::MemoryBufferRef buffer, Block *block,
                                     const ParserConfig &config) {
  return readBytecodeFileImpl(buffer, block, config, /*bufferOwnerRef=*/{});
}
LogicalResult
mlir::readBytecodeFile(const std::shared_ptr<toolchain::SourceMgr> &sourceMgr,
