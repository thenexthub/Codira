...
MlirType mlirUniformQuantizedPerAxisTypeGet(
  ...
  return wrap(quant::UniformQuantizedPerAxisType::get(
      ...
      toolchain::ArrayRef(scales, nDims), toolchain::ArrayRef(zeroPoints, nDims),
...
MlirType mlirUniformQuantizedSubChannelTypeGet(
  ...
  return wrap(quant::UniformQuantizedSubChannelType::get(
      ...
      toolchain::ArrayRef<int32_t>(quantizedDimensions, nDims),
      toolchain::ArrayRef<int64_t>(blockSizes, nDims), storageTypeMin,
      storageTypeMax));
}
