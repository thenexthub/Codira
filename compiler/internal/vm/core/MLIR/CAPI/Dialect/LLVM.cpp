...
MLIR_DEFINE_CAPI_DIALECT_REGISTRATION(LLVM, toolchain, LLVMDialect)
...
intptr_t mlirLLVMFunctionTypeGetNumInputs(MlirType type) {
  return toolchain::cast<LLVM::LLVMFunctionType>(unwrap(type)).getNumParams();
}

MlirType mlirLLVMFunctionTypeGetInput(MlirType type, intptr_t pos) {
  assert(pos >= 0 && "pos in array must be positive");
  return wrap(toolchain::cast<LLVM::LLVMFunctionType>(unwrap(type))
                  .getParamType(static_cast<unsigned>(pos)));
}

MlirType mlirLLVMFunctionTypeGetReturnType(MlirType type) {
  return wrap(toolchain::cast<LLVM::LLVMFunctionType>(unwrap(type)).getReturnType());
}
...
MlirAttribute mlirLLVMDIExpressionAttrGet(MlirContext ctx, intptr_t nOperations,
  ...
  return wrap(DIExpressionAttr::get(
      unwrap(ctx),
      toolchain::map_to_vector(unwrapList(nOperations, operations, attrStorage),
                          toolchain::CastTo<DIExpressionElemAttr>)));
}
...
MlirAttribute mlirLLVMDICompositeTypeAttrGet(
  ...
  return wrap(DICompositeTypeAttr::get(
      ...
      toolchain::map_to_vector(unwrapList(nElements, elements, elementsStorage),
                          toolchain::CastTo<DINodeAttr>)));
}
...
MlirAttribute mlirLLVMDISubroutineTypeAttrGet(MlirContext ctx,
  ...
  return wrap(DISubroutineTypeAttr::get(
      ...
      toolchain::map_to_vector(unwrapList(nTypes, types, attrStorage),
                          toolchain::CastTo<DITypeAttr>)));
}
