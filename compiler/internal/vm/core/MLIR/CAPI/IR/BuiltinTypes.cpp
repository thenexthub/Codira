...
bool mlirTypeIsAInteger(MlirType type) {
  return toolchain::isa<IntegerType>(unwrap(type));
}
...
unsigned mlirIntegerTypeGetWidth(MlirType type) {
  return toolchain::cast<IntegerType>(unwrap(type)).getWidth();
}

bool mlirIntegerTypeIsSignless(MlirType type) {
  return toolchain::cast<IntegerType>(unwrap(type)).isSignless();
}

bool mlirIntegerTypeIsSigned(MlirType type) {
  return toolchain::cast<IntegerType>(unwrap(type)).isSigned();
}

bool mlirIntegerTypeIsUnsigned(MlirType type) {
  return toolchain::cast<IntegerType>(unwrap(type)).isUnsigned();
}
...
bool mlirTypeIsAIndex(MlirType type) {
  return toolchain::isa<IndexType>(unwrap(type));
}
...
bool mlirTypeIsAFloat(MlirType type) {
  return toolchain::isa<FloatType>(unwrap(type));
}

unsigned mlirFloatTypeGetWidth(MlirType type) {
  return toolchain::cast<FloatType>(unwrap(type)).getWidth();
}
...
bool mlirTypeIsAFloat4E2M1FN(MlirType type) {
  return toolchain::isa<Float4E2M1FNType>(unwrap(type));
}
...
bool mlirTypeIsAFloat6E2M3FN(MlirType type) {
  return toolchain::isa<Float6E2M3FNType>(unwrap(type));
}
...
bool mlirTypeIsAFloat6E3M2FN(MlirType type) {
  return toolchain::isa<Float6E3M2FNType>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E5M2(MlirType type) {
  return toolchain::isa<Float8E5M2Type>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E4M3(MlirType type) {
  return toolchain::isa<Float8E4M3Type>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E4M3FN(MlirType type) {
  return toolchain::isa<Float8E4M3FNType>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E5M2FNUZ(MlirType type) {
  return toolchain::isa<Float8E5M2FNUZType>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E4M3FNUZ(MlirType type) {
  return toolchain::isa<Float8E4M3FNUZType>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E4M3B11FNUZ(MlirType type) {
  return toolchain::isa<Float8E4M3B11FNUZType>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E3M4(MlirType type) {
  return toolchain::isa<Float8E3M4Type>(unwrap(type));
}
...
bool mlirTypeIsAFloat8E8M0FNU(MlirType type) {
  return toolchain::isa<Float8E8M0FNUType>(unwrap(type));
}
...
bool mlirTypeIsABF16(MlirType type) {
  return toolchain::isa<BFloat16Type>(unwrap(type));
}
...
bool mlirTypeIsAF16(MlirType type) {
  return toolchain::isa<Float16Type>(unwrap(type));
}
...
bool mlirTypeIsATF32(MlirType type) {
  return toolchain::isa<FloatTF32Type>(unwrap(type));
}
...
bool mlirTypeIsAF32(MlirType type) {
  return toolchain::isa<Float32Type>(unwrap(type));
}
...
bool mlirTypeIsAF64(MlirType type) {
  return toolchain::isa<Float64Type>(unwrap(type));
}
...
bool mlirTypeIsANone(MlirType type) {
  return toolchain::isa<NoneType>(unwrap(type));
}
...
bool mlirTypeIsAComplex(MlirType type) {
  return toolchain::isa<ComplexType>(unwrap(type));
}
...
MlirType mlirComplexTypeGetElementType(MlirType type) {
  return wrap(toolchain::cast<ComplexType>(unwrap(type)).getElementType());
}
...
bool mlirTypeIsAShaped(MlirType type) {
  return toolchain::isa<ShapedType>(unwrap(type));
}

MlirType mlirShapedTypeGetElementType(MlirType type) {
  return wrap(toolchain::cast<ShapedType>(unwrap(type)).getElementType());
}

bool mlirShapedTypeHasRank(MlirType type) {
  return toolchain::cast<ShapedType>(unwrap(type)).hasRank();
}

int64_t mlirShapedTypeGetRank(MlirType type) {
  return toolchain::cast<ShapedType>(unwrap(type)).getRank();
}

bool mlirShapedTypeHasStaticShape(MlirType type) {
  return toolchain::cast<ShapedType>(unwrap(type)).hasStaticShape();
}

bool mlirShapedTypeIsDynamicDim(MlirType type, intptr_t dim) {
  return toolchain::cast<ShapedType>(unwrap(type))
...
bool mlirTypeIsAVector(MlirType type) {
  return toolchain::isa<VectorType>(unwrap(type));
}

MlirType mlirVectorTypeGet(intptr_t rank, const int64_t *shape,
                           MlirType elementType) {
  return wrap(VectorType::get(toolchain::ArrayRef(shape, static_cast<size_t>(rank)),
                              unwrap(elementType)));
}
...
MlirType mlirVectorTypeGetChecked(MlirLocation loc, intptr_t rank,
  ...
  return wrap(VectorType::getChecked(
      unwrap(loc), toolchain::ArrayRef(shape, static_cast<size_t>(rank)),
      unwrap(elementType)));
}
...
MlirType mlirVectorTypeGetScalable(intptr_t rank, const int64_t *shape,
  ...
  return wrap(VectorType::get(
      toolchain::ArrayRef(shape, static_cast<size_t>(rank)), unwrap(elementType),
      toolchain::ArrayRef(scalable, static_cast<size_t>(rank))));
}
...
bool mlirTypeIsATensor(MlirType type) {
  return toolchain::isa<TensorType>(unwrap(type));
}
...
bool mlirTypeIsARankedTensor(MlirType type) {
  return toolchain::isa<RankedTensorType>(unwrap(type));
}
...
bool mlirTypeIsAUnrankedTensor(MlirType type) {
  return toolchain::isa<UnrankedTensorType>(unwrap(type));
}

MlirType mlirRankedTensorTypeGet(intptr_t rank, const int64_t *shape,
                                 MlirType elementType, MlirAttribute encoding) {
  return wrap(
      RankedTensorType::get(toolchain::ArrayRef(shape, static_cast<size_t>(rank)),
...
MlirAttribute mlirRankedTensorTypeGetEncoding(MlirType type) {
  return wrap(toolchain::cast<RankedTensorType>(unwrap(type)).getEncoding());
}
...
bool mlirTypeIsAMemRef(MlirType type) {
  return toolchain::isa<MemRefType>(unwrap(type));
}
...
MlirType mlirMemRefTypeGet(MlirType elementType, intptr_t rank,
  ...
  return wrap(MemRefType::get(
      ...
      mlirAttributeIsNull(layout)
          ...
          : toolchain::cast<MemRefLayoutAttrInterface>(unwrap(layout)),
      unwrap(memorySpace)));
}
...
MlirType mlirMemRefTypeContiguousGet(MlirType elementType, intptr_t rank,
                                     const int64_t *shape,
                                     MlirAttribute memorySpace) {
  return wrap(MemRefType::get(toolchain::ArrayRef(shape, static_cast<size_t>(rank)),
...
MlirAttribute mlirMemRefTypeGetLayout(MlirType type) {
  return wrap(toolchain::cast<MemRefType>(unwrap(type)).getLayout());
}

MlirAffineMap mlirMemRefTypeGetAffineMap(MlirType type) {
  return wrap(toolchain::cast<MemRefType>(unwrap(type)).getLayout().getAffineMap());
}

MlirAttribute mlirMemRefTypeGetMemorySpace(MlirType type) {
  return wrap(toolchain::cast<MemRefType>(unwrap(type)).getMemorySpace());
}

MlirLogicalResult mlirMemRefTypeGetStridesAndOffset(MlirType type,
                                                    int64_t *strides,
                                                    int64_t *offset) {
  MemRefType memrefType = toolchain::cast<MemRefType>(unwrap(type));
  SmallVector<int64_t> strides_;
  if (failed(memrefType.getStridesAndOffset(strides_, *offset)))
    return mlirLogicalResultFailure();

  (void)toolchain::copy(strides_, strides);
...
bool mlirTypeIsAUnrankedMemRef(MlirType type) {
  return toolchain::isa<UnrankedMemRefType>(unwrap(type));
}
...
MlirAttribute mlirUnrankedMemrefGetMemorySpace(MlirType type) {
  return wrap(toolchain::cast<UnrankedMemRefType>(unwrap(type)).getMemorySpace());
}
...
bool mlirTypeIsATuple(MlirType type) {
  return toolchain::isa<TupleType>(unwrap(type));
}
...
intptr_t mlirTupleTypeGetNumTypes(MlirType type) {
  return toolchain::cast<TupleType>(unwrap(type)).size();
}

MlirType mlirTupleTypeGetType(MlirType type, intptr_t pos) {
  return wrap(
      toolchain::cast<TupleType>(unwrap(type)).getType(static_cast<size_t>(pos)));
}
...
bool mlirTypeIsAFunction(MlirType type) {
  return toolchain::isa<FunctionType>(unwrap(type));
}
...
intptr_t mlirFunctionTypeGetNumInputs(MlirType type) {
  return toolchain::cast<FunctionType>(unwrap(type)).getNumInputs();
}

intptr_t mlirFunctionTypeGetNumResults(MlirType type) {
  return toolchain::cast<FunctionType>(unwrap(type)).getNumResults();
}

MlirType mlirFunctionTypeGetInput(MlirType type, intptr_t pos) {
  assert(pos >= 0 && "pos in array must be positive");
  return wrap(toolchain::cast<FunctionType>(unwrap(type))
...
bool mlirTypeIsAOpaque(MlirType type) {
  return toolchain::isa<OpaqueType>(unwrap(type));
}
...
MlirStringRef mlirOpaqueTypeGetDialectNamespace(MlirType type) {
  return wrap(
      toolchain::cast<OpaqueType>(unwrap(type)).getDialectNamespace().strref());
}

MlirStringRef mlirOpaqueTypeGetData(MlirType type) {
  return wrap(toolchain::cast<OpaqueType>(unwrap(type)).getTypeData());
}
