...
MlirAsmState mlirAsmStateCreateForValue(MlirValue value,
  ...
  if (auto result = toolchain::dyn_cast<OpResult>(val)) {
    op = result.getOwner();
  } else {
    op = toolchain::cast<BlockArgument>(val).getOwner()->getParentOp();
    if (!op) {
...
MlirLocation mlirLocationFromAttribute(MlirAttribute attribute) {
  return wrap(Location(toolchain::dyn_cast<LocationAttr>(unwrap(attribute))));
}
...
MlirIdentifier mlirLocationFileLineColRangeGetFilename(MlirLocation location) {
  return wrap(toolchain::dyn_cast<FileLineColRange>(unwrap(location)).getFilename());
}

int mlirLocationFileLineColRangeGetStartLine(MlirLocation location) {
  if (auto loc = toolchain::dyn_cast<FileLineColRange>(unwrap(location)))
...
MlirLocation mlirLocationCallSiteGetCallee(MlirLocation location) {
  return wrap(
      Location(toolchain::dyn_cast<CallSiteLoc>(unwrap(location)).getCallee()));
}

MlirLocation mlirLocationCallSiteGetCaller(MlirLocation location) {
  return wrap(
      Location(toolchain::dyn_cast<CallSiteLoc>(unwrap(location)).getCaller()));
}
...
unsigned mlirLocationFusedGetNumLocations(MlirLocation location) {
  if (auto locationsArrRef = toolchain::dyn_cast<FusedLoc>(unwrap(location)))
...
void mlirLocationFusedGetLocations(MlirLocation location,
  ...
  if (auto locationsArrRef = toolchain::dyn_cast<FusedLoc>(unwrap(location))) {
    for (auto [i, location] : toolchain::enumerate(locationsArrRef.getLocations()))
      locationsCPtr[i] = wrap(location);
  }
}

MlirAttribute mlirLocationFusedGetMetadata(MlirLocation location) {
  return wrap(toolchain::dyn_cast<FusedLoc>(unwrap(location)).getMetadata());
}
...
MlirIdentifier mlirLocationNameGetName(MlirLocation location) {
  return wrap((toolchain::dyn_cast<NameLoc>(unwrap(location)).getName()));
}

MlirLocation mlirLocationNameGetChildLoc(MlirLocation location) {
  return wrap(
      Location(toolchain::dyn_cast<NameLoc>(unwrap(location)).getChildLoc()));
}
...
intptr_t mlirOperationGetNumDiscardableAttributes(MlirOperation op) {
  return static_cast<intptr_t>(
      toolchain::range_size(unwrap(op)->getDiscardableAttrs()));
}
...
bool mlirValueIsABlockArgument(MlirValue value) {
  return toolchain::isa<BlockArgument>(unwrap(value));
}

bool mlirValueIsAOpResult(MlirValue value) {
  return toolchain::isa<OpResult>(unwrap(value));
}

MlirBlock mlirBlockArgumentGetOwner(MlirValue value) {
  return wrap(toolchain::dyn_cast<BlockArgument>(unwrap(value)).getOwner());
}

intptr_t mlirBlockArgumentGetArgNumber(MlirValue value) {
  return static_cast<intptr_t>(
      toolchain::dyn_cast<BlockArgument>(unwrap(value)).getArgNumber());
}

void mlirBlockArgumentSetType(MlirValue value, MlirType type) {
  if (auto blockArg = toolchain::dyn_cast<BlockArgument>(unwrap(value)))
...
MlirOperation mlirOpResultGetOwner(MlirValue value) {
  return wrap(toolchain::dyn_cast<OpResult>(unwrap(value)).getOwner());
}

intptr_t mlirOpResultGetResultNumber(MlirValue value) {
  return static_cast<intptr_t>(
      toolchain::dyn_cast<OpResult>(unwrap(value)).getResultNumber());
}
...
void mlirValueReplaceAllUsesExcept(MlirValue oldValue, MlirValue newValue,
  ...
  toolchain::SmallPtrSet<mlir::Operation *, 4> exceptionSet;
