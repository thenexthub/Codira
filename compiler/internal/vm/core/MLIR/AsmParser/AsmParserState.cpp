...
struct AsmParserState::Impl {
  ...
  toolchain::StringMap<unsigned> attrAliasToIdx;
  toolchain::StringMap<unsigned> typeAliasToIdx;
  ...
  for (auto &opAndUseMapIt : symbolTableOperations) {
    for (auto &it : *opAndUseMapIt.second) {
      ...
      for (ArrayRef<SMRange> useRange : it.second) {
        for (const auto &symIt : toolchain::zip(symbolOps, useRange)) {
...
auto AsmParserState::getBlockDefs() const -> iterator_range<BlockDefIterator> {
  return toolchain::make_pointee_range(toolchain::ArrayRef(impl->blocks));
}
...
auto AsmParserState::getOpDefs() const -> iterator_range<OperationDefIterator> {
  return toolchain::make_pointee_range(toolchain::ArrayRef(impl->operations));
}
...
auto AsmParserState::getAttributeAliasDefs() const
  ...
  return toolchain::make_pointee_range(ArrayRef(impl->attrAliases));
}
...
auto AsmParserState::getTypeAliasDefs() const
  ...
  return toolchain::make_pointee_range(ArrayRef(impl->typeAliases));
}
  ...
  while (char c = *curPtr++) {
    ...
    if (c == '\\') {
      ...
      if (*curPtr == '"' || *curPtr == '\\' || *curPtr == 'n' || *curPtr == 't')
        ++curPtr;
      else if (toolchain::isHexDigit(*curPtr) && toolchain::isHexDigit(curPtr[1]))
        curPtr += 2;
      else
        return curPtr;
    }
...
void AsmParserState::addUses(Value value, ArrayRef<SMLoc> locations) {
  ...
  if (OpResult result = dyn_cast<OpResult>(value)) {
    ...
    for (auto &resultGroup : toolchain::reverse(def.resultGroups)) {
