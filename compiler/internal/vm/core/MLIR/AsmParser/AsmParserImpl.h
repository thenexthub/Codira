...
public:
  ...
  ParseResult parseBase64Bytes(std::vector<char> *bytes) override {
    ...
    if (bytes) {
      ...
      if (auto err = toolchain::decodeBase64(b64String, *bytes))
  ...
  /// lost.
  ParseResult parseFloat(const toolchain::fltSemantics &semantics,
  ...
  ParseResult parseFloat(double &result) override {
    toolchain::APFloat apResult(0.0);
  ...
  parseOptionalKeyword(StringRef *keyword,
    ...
    if (toolchain::is_contained(allowedKeywords, currentKeyword)) {
