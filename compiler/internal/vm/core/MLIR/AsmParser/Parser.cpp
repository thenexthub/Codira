...
OptionalParseResult Parser::parseOptionalDecimalInteger(APInt &result) {
  ...
  if (spelling[0] == '0' && spelling.size() > 1 &&
      toolchain::toLower(spelling[1]) == 'x') {
    result = 0;
...
Attribute
Parser::codeCompleteDialectSymbol(const toolchain::StringMap<Attribute> &aliases) {
  state.codeCompleteContext->completeDialectAttributeOrAlias(aliases);
  return {};
}
Type Parser::codeCompleteDialectSymbol(const toolchain::StringMap<Type> &aliases) {
...
public:
  ...
  /// have results.
  using OpOrArgument = toolchain::PointerUnion<Operation *, BlockArgument>;
...
private:
  ...
  struct IsolatedSSANameScope {
    ...
    toolchain::StringMap<SmallVector<ValueDefinition, 1>> values;

    /// This keeps track of all of the values defined by a specific name scope.
    SmallVector<toolchain::StringSet<>, 2> definitionsPerScope;
  };
...
ParseResult OperationParser::finalize() {
  ...
  if (!forwardRefPlaceholders.empty()) {
    ...
    toolchain::array_pod_sort(errors.begin(), errors.end());
...
ParseResult OperationParser::parseOperation() {
  ...
  if (!resultIDs.empty()) {
    ...
    for (ResultRecord &resIt : resultIDs) {
      for (unsigned subRes : toolchain::seq<unsigned>(0, std::get<1>(resIt))) {
...
OperationParser::parseCustomOperation(ArrayRef<ResultRecord> resultIDs) {
  ...
  if (auto opInfo = opNameInfo->getRegisteredInfo()) {
    ...
    if (!dialect) {
      ...
      toolchain::interleaveComma(mergedDialects, note, [&](auto &pair) {
        note << pair.first;
        if (pair.second) {
      ...
      note << "; for more info on dialect registration see "
              "https://mlir.toolchain.org/getting_started/Faq/"
  ...
  // debugging.
  toolchain::PrettyStackTraceFormat fmt("MLIR Parser: custom op parser '%s'",
...
OperationParser::parseTrailingLocationSpecifier(OpOrArgument opOrArgument) {
  ...
  if (auto *op = toolchain::dyn_cast_if_present<Operation *>(opOrArgument))
    op->setLoc(directLoc);
  else
...
ParseResult OperationParser::parseRegionBody(Region &region, SMLoc startLoc,
  ...
  toolchain::scope_exit failureCleanup([&] {
    if (owningBlock) {
...
ParseResult OperationParser::parseBlock(Block *&block) {
  ...
  toolchain::scope_exit cleanupOnFailure([&] {
    if (inflightBlock)
...
ParseResult OperationParser::codeCompleteSSAUse() {
  for (IsolatedSSANameScope &scope : isolatedNameScopes) {
    for (auto &it : scope.values) {
      ...
      toolchain::raw_string_ostream detailOS(detailData);
...
public:
  ...
  parseAsBlob(BlobAllocatorFn allocator) const final {
    ...
    toolchain::support::ulittle32_t align;
    memcpy(&align, blobData->data(), sizeof(uint32_t));
    if (align && !toolchain::isPowerOf2_32(align)) {
      return p.emitError(value.getLoc(),
                         "expected hex string blob for key '" + key +
    ...
    assert(toolchain::isAddrAligned(toolchain::Align(align), blob.getData().data()) &&
           blob.isMutable() &&
...
LogicalResult
mlir::parseAsmSourceFile(const toolchain::SourceMgr &sourceMgr, Block *block,
