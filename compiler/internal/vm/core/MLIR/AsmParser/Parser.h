...
public:
  ...
  const toolchain::SourceMgr &getSourceMgr() { return state.lex.getSourceMgr(); }
  ...
  Attribute
  codeCompleteDialectSymbol(const toolchain::StringMap<Attribute> &aliases);
  Type codeCompleteDialectSymbol(const toolchain::StringMap<Type> &aliases);
