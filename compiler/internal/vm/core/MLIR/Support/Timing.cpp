...
constexpr toolchain::StringLiteral kTimingDescription =
...
public:
  ...
  toolchain::BumpPtrAllocator identifierAllocator;
  toolchain::StringSet<toolchain::BumpPtrAllocator &> identifiers;
  toolchain::sys::SmartRWMutex<true> identifierMutex;

  /// A thread local cache of identifiers to reduce lock contention.
  ThreadLocalCache<
      toolchain::StringMap<toolchain::StringMapEntry<toolchain::EmptyStringSetTag> *>>
...
TimingIdentifier TimingIdentifier::get(StringRef str, TimingManager &tm) {
  ...
  {
    toolchain::sys::SmartScopedReader<true> contextLock(impl.identifierMutex);
  ...
  toolchain::sys::SmartScopedWriter<true> contextLock(impl.identifierMutex);
...
public:
  ...
  void printTime(const TimeRecord &time, const TimeRecord &total) override {
    if (total.user != total.wall) {
      os << toolchain::format("  %8.4f (%5.1f%%)", time.user,
                         100.0 * time.user / total.user);
    }
    os << toolchain::format("  %8.4f (%5.1f%%)  ", time.wall,
...
public:
  ...
  void printTime(const TimeRecord &time, const TimeRecord &total) override {
    if (total.user != total.wall) {
      os << "\"user\": {";
      os << "\"duration\": " << toolchain::format("%8.4f", time.user) << ", ";
      os << "\"percentage\": "
         << toolchain::format("%5.1f", 100.0 * time.user / total.user);
      os << "}, ";
    }
    os << "\"wall\": {";
    os << "\"duration\": " << toolchain::format("%8.4f", time.wall) << ", ";
    os << "\"percentage\": "
       << toolchain::format("%5.1f", 100.0 * time.wall / total.wall);
    os << "}";
  }
...
class TimerImpl {
public:
  using ChildrenMap = toolchain::MapVector<const void *, std::unique_ptr<TimerImpl>>;
  using AsyncChildrenMap = toolchain::DenseMap<uint64_t, ChildrenMap>;

  TimerImpl(std::string &&name, std::unique_ptr<OutputStrategy> &output)
      : threadId(toolchain::get_threadid()), name(name), output(output) {}
  ...
  void dump(raw_ostream &os, unsigned indent = 0, unsigned markThreadId = 0) {
    ...
    os << std::string(indent * 2, ' ') << name << " [" << threadId << "]"
       << toolchain::format("  %7.4f / %7.4f", time.user, time.wall);
  ...
  void printAsList(TimeRecord total) {
    ...
    toolchain::StringMap<TimeRecord> mergedTimers;
    ...
    toolchain::array_pod_sort(timerNameAndTime.begin(), timerNameAndTime.end(),
                         [](const std::pair<StringRef, TimeRecord> *lhs,
                            const std::pair<StringRef, TimeRecord> *rhs) {
                           return toolchain::array_pod_sort_comparator<double>(
...
DefaultTimingManager::DefaultTimingManager()
    : impl(std::make_unique<DefaultTimingManagerImpl>()),
      out(std::make_unique<OutputTextStrategy>(toolchain::errs())) {
...
struct DefaultTimingManagerOptions {
  toolchain::cl::opt<bool> timing{"mlir-timing",
                             toolchain::cl::desc("Display execution times"),
                             toolchain::cl::init(false)};
  toolchain::cl::opt<DisplayMode> displayMode{
      "mlir-timing-display", toolchain::cl::desc("Display method for timing data"),
      toolchain::cl::init(DisplayMode::Tree),
      toolchain::cl::values(
  ...
  toolchain::cl::opt<OutputFormat> outputFormat{
      "mlir-output-format", toolchain::cl::desc("Output format for timing data"),
      toolchain::cl::init(OutputFormat::Text),
      toolchain::cl::values(clEnumValN(OutputFormat::Text, "text",
...
static toolchain::ManagedStatic<DefaultTimingManagerOptions> options;
...
void mlir::applyDefaultTimingManagerCLOptions(DefaultTimingManager &tm) {
  ...
  tm.setOutput(createOutputStrategy(options->outputFormat, toolchain::errs()));
}
