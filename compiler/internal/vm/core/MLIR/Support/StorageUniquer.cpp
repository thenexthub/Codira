...
public:
  ...
  ParametricStorageUniquer(function_ref<void(BaseStorage *)> destructorFn,
    ...
    assert(toolchain::isPowerOf2_64(numShards) &&
  ...
  BaseStorage *getOrCreate(bool threadingIsEnabled, unsigned hashValue,
    ...
    {
      toolchain::sys::SmartScopedReader<true> typeLock(shard.mutex);
    ...
    // instance.
    toolchain::sys::SmartScopedWriter<true> typeLock(shard.mutex);
  ...
  LogicalResult mutate(bool threadingIsEnabled, BaseStorage *storage,
    ...
    // deterministic.
    Shard &shard = getShard(toolchain::hash_value(storage));
    toolchain::sys::SmartScopedWriter<true> lock(shard.mutex);
    return mutationFn();
  }
...
struct StorageUniquerImpl {
  ...
  StorageAllocator &getThreadSafeAllocator() {
    ...
    if (!threadAllocator) {
      ...
      // the thread dies.
      toolchain::sys::SmartScopedLock<true> lock(threadAllocatorMutex);
  ...
  toolchain::sys::SmartMutex<true> threadAllocatorMutex;
#endif
