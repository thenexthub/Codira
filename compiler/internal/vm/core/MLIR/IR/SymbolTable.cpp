...
walkSymbolTable(MutableArrayRef<Region> regions,
  ...
  SmallVector<Region *, 1> worklist(toolchain::make_pointer_range(regions));
...
SymbolTable::renameToUnique(StringAttr oldName,
  ...
  StringAttr newName;
  {
    ...
    while (true) {
      ...
      if (!lookupNewName(this) && toolchain::none_of(others, lookupNewName)) {
        break;
      }
    }
  }
...
LogicalResult detail::verifySymbol(Operation *op) {
  ...
  if (Attribute vis = op->getAttr(mlir::SymbolTable::getVisibilityAttrName())) {
    StringAttr visStrAttr = toolchain::dyn_cast<StringAttr>(vis);
    if (!visStrAttr)
      return op->emitOpError() << "requires visibility attribute '"
                               << mlir::SymbolTable::getVisibilityAttrName()
                               << "' to be a string attribute, but got " << vis;

    if (!toolchain::is_contained(ArrayRef<StringRef>{"public", "private", "nested"},
                            visStrAttr.getValue()))
      return op->emitOpError()
...
struct SymbolScope {
  ...
  template <typename CallbackT,
            std::enable_if_t<!std::is_same<
                typename toolchain::function_traits<CallbackT>::result_t,
                void>::value> * = nullptr>
  std::optional<WalkResult> walk(CallbackT cback) {
    if (Region *region = toolchain::dyn_cast_if_present<Region *>(limit))
  ...
  toolchain::PointerUnion<Operation *, Region *> limit;
};
} // namespace
...
static bool isReferencePrefixOf(SymbolRefAttr subRef, SymbolRefAttr ref) {
  ...
  // prefix of `ref`.
  if (toolchain::isa<FlatSymbolRefAttr>(ref) ||
...
static SymbolRefAttr generateNewRefAttr(SymbolRefAttr oldAttr,
  ...
  if (toolchain::isa<FlatSymbolRefAttr>(oldAttr))
    return newLeafAttr;
  auto nestedRefs = toolchain::to_vector<2>(oldAttr.getNestedReferences());
    ...
    replacer.addReplacement(
        [&](SymbolRefAttr attr) -> std::pair<Attribute, WalkResult> {
          ...
          if (isReferencePrefixOf(oldAttr, attr)) {
            ...
            auto newNestedRefs = toolchain::to_vector<4>(nestedRefs);
...
LockedSymbolTableCollection::getSymbolTable(Operation *symbolTableOp) {
  ...
  {
    toolchain::sys::SmartScopedReader<true> lock(mutex);
  ...
  toolchain::sys::SmartScopedWriter<true> lock(mutex);
