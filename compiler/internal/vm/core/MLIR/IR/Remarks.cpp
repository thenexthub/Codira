...
Remark::Arg::Arg(toolchain::StringRef k, Value v) : key(k) {
  toolchain::raw_string_ostream os(val);
  os << v;
}

Remark::Arg::Arg(toolchain::StringRef k, Type t) : key(k) {
  toolchain::raw_string_ostream os(val);
  os << t;
}

Remark::Arg::Arg(toolchain::StringRef k, Attribute a) : key(k), attr(a) {
  toolchain::raw_string_ostream os(val);
  os << a;
}

void Remark::insert(toolchain::StringRef s) { args.emplace_back(s); }
void Remark::insert(Arg a) { args.push_back(std::move(a)); }

// Simple helper to print key=val list (sorted).
static void printArgs(toolchain::raw_ostream &os, toolchain::ArrayRef<Remark::Arg> args) {
  if (args.empty())
    return;

  toolchain::SmallVector<Remark::Arg, 8> sorted(args.begin(), args.end());
  toolchain::sort(sorted, [](const Remark::Arg &a, const Remark::Arg &b) {
    return a.key < b.key;
  });
...
// clang-format on
void Remark::print(toolchain::raw_ostream &os, bool printLocation) const {
...
toolchain::StringRef Remark::getRemarkTypeString() const {
  switch (remarkKind) {
...
toolchain::remarks::Type Remark::getRemarkType() const {
  switch (remarkKind) {
  case RemarkKind::RemarkUnknown:
    return toolchain::remarks::Type::Unknown;
  case RemarkKind::RemarkPassed:
    return toolchain::remarks::Type::Passed;
  case RemarkKind::RemarkMissed:
    return toolchain::remarks::Type::Missed;
  case RemarkKind::RemarkFailure:
    return toolchain::remarks::Type::Failure;
  case RemarkKind::RemarkAnalysis:
    return toolchain::remarks::Type::Analysis;
  }
  llvm_unreachable("Unknown remark kind");
}

toolchain::remarks::Remark Remark::generateRemark() const {
  auto locLambda = [&]() -> toolchain::remarks::RemarkLocation {
    if (auto flc = dyn_cast<FileLineColLoc>(getLocation()))
      return {flc.getFilename(), flc.getLine(), flc.getColumn()};
    return {"<unknown file>", 0, 0};
  };

  toolchain::remarks::Remark r; // The result.
...
toolchain::LogicalResult RemarkEngine::initialize(
  ...
  auto reportFunc = toolchain::bind_front<&RemarkEngine::reportImpl>(this);
...
static bool isAnchored(toolchain::StringRef s) {
  s = s.trim();
  return s.starts_with("^") && s.ends_with("$"); // note: startswith/endswith
}

/// Anchor the entire pattern so it matches the whole string.
static std::string anchorWhole(toolchain::StringRef filter) {
  if (isAnchored(filter))
    return filter.str();
  return (toolchain::Twine("^(") + filter + ")$").str();
}
...
buildFilter(const mlir::remark::RemarkCategories &cats,
  ...
  std::string joined = toolchain::join(parts, "|");
  std::string anchored = anchorWhole(joined);

  toolchain::Regex rx(anchored);
  std::string err;
  if (!rx.isValid(err))
    return std::nullopt;

  return std::make_optional<toolchain::Regex>(std::move(rx));
}
...
toolchain::LogicalResult mlir::remark::enableOptimizationRemarks(
  ...
  if (failed(engine->initialize(std::move(streamer),
    ...
    toolchain::report_fatal_error(
        toolchain::Twine("Failed to initialize remark engine. Error: ") + errMsg);
  }
