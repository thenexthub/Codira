...
inline size_t getDenseElementBitWidth(Type eltType) {
  ...
  if (ComplexType comp = toolchain::dyn_cast<ComplexType>(eltType))
    return toolchain::alignTo<8>(getDenseElementBitWidth(comp.getElementType())) * 2;
  if (eltType.isIndex())
...
struct DenseIntOrFPElementsAttrStorage : public DenseElementsAttributeStorage {
  ...
  struct KeyTy {
    KeyTy(ShapedType type, ArrayRef<char> data, toolchain::hash_code hashCode,
          bool isSplat = false)
  ...
  static KeyTy getKey(ShapedType ty, ArrayRef<char> data, bool isKnownSplat) {
    ...
    if (isKnownSplat) {
      ...
      return KeyTy(ty, data, toolchain::hash_value(data), isKnownSplat);
    }
    ...
    size_t storageSize = toolchain::divideCeil(elementWidth, CHAR_BIT);
    ...
    auto hashVal = toolchain::hash_value(firstElt);
    ...
    for (size_t i = storageSize, e = data.size(); i != e; i += storageSize)
      if (memcmp(data.data(), &data[i], storageSize))
        return KeyTy(ty, data, toolchain::hash_combine(hashVal, data.drop_front(i)));
  ...
  static KeyTy getKeyForBoolData(ShapedType ty, ArrayRef<char> data,
    ...
    if (splatValue && numOddElements != 0) {
      ...
      if (lastElt != toolchain::maskTrailingOnes<unsigned char>(numOddElements))
        return KeyTy(ty, data, toolchain::hash_value(data));
    ...
    return toolchain::all_of(splatData, [mask](char c) { return c == mask; })
               ? getKeyForSplatBoolData(ty, splatValue)
               : KeyTy(ty, data, toolchain::hash_value(data));
  }
  ...
  static KeyTy getKeyForSplatBoolData(ShapedType type, bool splatValue) {
    ...
    return KeyTy(type, splatData, toolchain::hash_value(splatData),
                 /*isSplat=*/true);
  }

  /// Hash the key for the storage.
  static toolchain::hash_code hashKey(const KeyTy &key) {
    return toolchain::hash_combine(key.type, key.hashCode);
  }
...
struct DenseStringElementsAttrStorage : public DenseElementsAttributeStorage {
  ...
  struct KeyTy {
    KeyTy(ShapedType type, ArrayRef<StringRef> data, toolchain::hash_code hashCode,
          bool isSplat = false)
        : type(type), data(data), hashCode(hashCode), isSplat(isSplat) {}
  ...
  static KeyTy getKey(ShapedType ty, ArrayRef<StringRef> data,
    ...
    // directly the data buffer.
    if (isKnownSplat)
      return KeyTy(ty, data, toolchain::hash_value(data.front()), isKnownSplat);
