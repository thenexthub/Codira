...
void Block::moveBefore(Region *region, toolchain::iplist<Block>::iterator iterator) {
...
bool Block::verifyOpOrder() {
  ...
  if (operations.empty() || toolchain::hasSingleElement(operations))
    return false;
...
auto Block::addArguments(TypeRange types, ArrayRef<Location> locs)
  ...
  for (auto typeAndLoc : toolchain::zip(types, locs))
...
BlockArgument Block::insertArgument(unsigned index, Type type, Location loc) {
  ...
  // one.
  ++index;
  for (BlockArgument arg : toolchain::drop_begin(arguments, index))
...
void Block::eraseArguments(unsigned start, unsigned num) {
  ...
  for (BlockArgument arg : toolchain::drop_begin(arguments, start))
...
void Block::eraseArguments(function_ref<bool(BlockArgument)> shouldEraseFn) {
  auto firstDead = toolchain::find_if(arguments, shouldEraseFn);
...
SuccessorRange::SuccessorRange(Block *block) : SuccessorRange() {
  if (block->empty() || toolchain::hasSingleElement(*block->getParent()))
    return;
...
BlockRange::OwnerT BlockRange::offset_base(OwnerT object, ptrdiff_t index) {
  if (auto *operand = toolchain::dyn_cast_if_present<BlockOperand *>(object))
    return {operand + index};
  return {toolchain::dyn_cast_if_present<Block *const *>(object) + index};
}

/// See `toolchain::detail::indexed_accessor_range_base` for details.
Block *BlockRange::dereference_iterator(OwnerT object, ptrdiff_t index) {
  if (const auto *operand = toolchain::dyn_cast_if_present<BlockOperand *>(object))
