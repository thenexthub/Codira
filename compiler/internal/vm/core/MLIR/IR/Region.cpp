...
void Region::cloneInto(Region *dest, Region::iterator destPos,
  ...
  auto newBlocksRange =
      toolchain::make_range(Region::iterator(mapper.lookup(&front())), destPos);
  ...
  for (auto zippedBlocks : toolchain::zip(*this, newBlocksRange)) {
  ...
  for (auto zippedBlocks : toolchain::zip(*this, newBlocksRange)) {
    for (auto ops :
         toolchain::zip(std::get<0>(zippedBlocks), std::get<1>(zippedBlocks))) {
      ...
      for (auto regions : toolchain::zip(source.getRegions(), clone.getRegions()))
...
Region *toolchain::ilist_traits<::mlir::Block>::getParentRegion() {
  size_t offset(
...
void toolchain::ilist_traits<::mlir::Block>::addNodeToList(Block *block) {
...
void toolchain::ilist_traits<::mlir::Block>::removeNodeFromList(Block *block) {
...
void toolchain::ilist_traits<::mlir::Block>::transferNodesFromList(
...
toolchain::raw_ostream &mlir::operator<<(toolchain::raw_ostream &os, Region &region) {
  ...
  for (auto it : toolchain::enumerate(region.getBlocks())) {
    os << "\n  Block #" << it.index() << ":";
...
RegionRange::OwnerT RegionRange::offset_base(const OwnerT &owner,
                                             ptrdiff_t index) {
  if (auto *region = toolchain::dyn_cast_if_present<const std::unique_ptr<Region> *>(owner))
    return region + index;
  if (auto **region = toolchain::dyn_cast_if_present<Region **>(owner))
    return region + index;
