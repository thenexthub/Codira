...
static bool dictionaryAttrSort(ArrayRef<NamedAttribute> value,
  ...
  default:
    ...
    bool isSorted = toolchain::is_sorted(value);
    // If not, do a general sort.
    if (!isSorted)
      toolchain::array_pod_sort(storage.begin(), storage.end());
    return !isSorted;
  }
  return false;
}
...
void StridedLayoutAttr::print(toolchain::raw_ostream &os) const {
  ...
  os << "strided<[";
  toolchain::interleaveComma(getStrides(), os, printIntOrQuestion);
  os << "]";
...
StridedLayoutAttr::getStridesAndOffset(ArrayRef<int64_t>,
  ...
  toolchain::append_range(strides, getStrides());
  offset = getOffset();
  return success();
}
...
LogicalResult FloatAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (!toolchain::isa<FloatType>(type))
    return emitError() << "expected floating point type";

  // Verify that the type semantics match that of the value.
  if (&toolchain::cast<FloatType>(type).getFloatSemantics() !=
      &value.getSemantics()) {
    return emitError()
...
FlatSymbolRefAttr SymbolRefAttr::get(MLIRContext *ctx, StringRef value) {
  return toolchain::cast<FlatSymbolRefAttr>(get(ctx, value, {}));
}

FlatSymbolRefAttr SymbolRefAttr::get(StringAttr value) {
  return toolchain::cast<FlatSymbolRefAttr>(get(value, {}));
}
...
LogicalResult IntegerAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (IntegerType integerType = toolchain::dyn_cast<IntegerType>(type)) {
  ...
  if (toolchain::isa<IndexType>(type)) {
...
BoolAttr IntegerAttr::getBoolAttrUnchecked(IntegerType type, bool value) {
  ...
  return toolchain::cast<BoolAttr>(attr);
}
...
bool BoolAttr::classof(Attribute attr) {
  IntegerAttr intAttr = toolchain::dyn_cast<IntegerAttr>(attr);
...
static size_t getDenseElementStorageWidth(size_t origWidth) {
  return origWidth == 1 ? origWidth : toolchain::alignTo<8>(origWidth);
}
...
static void copyAPIntToArrayForBEmachine(APInt value, size_t numBytes,
                                         char *result) {
  assert(toolchain::endianness::native == toolchain::endianness::big);
...
static void writeBits(char *rawData, size_t bitPos, APInt value) {
  ...
  if (toolchain::endianness::native == toolchain::endianness::big) {
    ...
    // Copying the first `toolchain::divideCeil(bitWidth, CHAR_BIT)` bytes doesn't
    // work correctly in BE format.
    // ex. `value` (2 words including 10 bytes)
    // ==> BE: |abcdefgh|------ij|,  LE: |hgfedcba|ji------|
    copyAPIntToArrayForBEmachine(value, toolchain::divideCeil(bitWidth, CHAR_BIT),
                                 rawData + (bitPos / CHAR_BIT));
  } else {
    std::copy_n(reinterpret_cast<const char *>(value.getRawData()),
                toolchain::divideCeil(bitWidth, CHAR_BIT),
                rawData + (bitPos / CHAR_BIT));
  }
}
...
static APInt readBits(const char *rawData, size_t bitPos, size_t bitWidth) {
  ...
  if (toolchain::endianness::native == toolchain::endianness::big) {
    ...
    copyArrayToAPIntForBEmachine(rawData + (bitPos / CHAR_BIT),
                                 toolchain::divideCeil(bitWidth, CHAR_BIT), result);
  } else {
...
DenseElementsAttr::AttributeElementIterator::AttributeElementIterator(
    ...
    : toolchain::indexed_accessor_iterator<AttributeElementIterator, const void *,
                                      Attribute, Attribute, Attribute>(
          attr.getAsOpaquePointer(), index) {}

Attribute DenseElementsAttr::AttributeElementIterator::operator*() const {
  auto owner = toolchain::cast<DenseElementsAttr>(getFromOpaquePointer(base));
  Type eltTy = owner.getElementType();
  if (toolchain::dyn_cast<IntegerType>(eltTy))
    return IntegerAttr::get(eltTy, *IntElementIterator(owner, index));
  if (toolchain::isa<IndexType>(eltTy))
    return IntegerAttr::get(eltTy, *IntElementIterator(owner, index));
  if (auto floatEltTy = toolchain::dyn_cast<FloatType>(eltTy)) {
  ...
  if (auto complexTy = toolchain::dyn_cast<ComplexType>(eltTy)) {
    auto complexEltTy = complexTy.getElementType();
    ComplexIntElementIterator complexIntIt(owner, index);
    if (toolchain::isa<IntegerType>(complexEltTy)) {
    ...
    ComplexFloatElementIterator complexFloatIt(
        toolchain::cast<FloatType>(complexEltTy).getFloatSemantics(), complexIntIt);
  ...
  if (toolchain::isa<DenseStringElementsAttr>(owner)) {
...
DenseElementsAttr::ComplexIntElementIterator::ComplexIntElementIterator(
    ...
    : DenseElementIndexedIteratorImpl<ComplexIntElementIterator,
                                      ...
                                      std::complex<APInt>>(
          attr.getRawData().data(), attr.isSplat(), dataIndex) {
  auto complexType = toolchain::cast<ComplexType>(attr.getElementType());
...
DenseArrayAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  int64_t elementSize =
      toolchain::divideCeil(elementType.getIntOrFloatBitWidth(), CHAR_BIT);
...
struct DenseArrayAttrIntUtil {
  static bool checkElementType(Type eltType) {
    auto type = toolchain::dyn_cast<IntegerType>(eltType);
...
void DenseArrayAttrImpl<T>::printWithoutBraces(raw_ostream &os) const {
  toolchain::interleaveComma(asArrayRef(), os, [&](T value) {
...
DenseArrayAttrImpl<T> DenseArrayAttrImpl<T>::get(MLIRContext *context,
  ...
  return toolchain::cast<DenseArrayAttrImpl<T>>(
      Base::get(context, elementType, content.size(), rawArray));
}

template <typename T>
bool DenseArrayAttrImpl<T>::classof(Attribute attr) {
  if (auto denseArray = toolchain::dyn_cast<DenseArrayAttr>(attr))
...
bool DenseElementsAttr::classof(Attribute attr) {
  return toolchain::isa<DenseIntOrFPElementsAttr, DenseStringElementsAttr>(attr);
}
...
DenseElementsAttr DenseElementsAttr::get(ShapedType type,
  ...
  if (auto complexType = toolchain::dyn_cast<ComplexType>(eltType)) {
    if (complexType.getElementType().isIntOrIndex()) {
      ...
      for (Attribute attr : values) {
        assert(toolchain::isa<ArrayAttr>(attr) && "expected ArrayAttr for complex");
        auto arrayAttr = toolchain::cast<ArrayAttr>(attr);
        ...
        complexValues.push_back(
            std::complex<APInt>(toolchain::cast<IntegerAttr>(attr0).getValue(),
                                toolchain::cast<IntegerAttr>(attr1).getValue()));
      }
    ...
    for (Attribute attr : values) {
      ...
      auto attr1 = arrayAttr[1];
      complexValues.push_back(
          std::complex<APFloat>(toolchain::cast<FloatAttr>(attr0).getValue(),
                                toolchain::cast<FloatAttr>(attr1).getValue()));
    }
    return DenseElementsAttr::get(type, complexValues);
  }
  ...
  if (!eltType.isIntOrIndexOrFloat()) {
    ...
    for (Attribute attr : values) {
      assert(toolchain::isa<StringAttr>(attr) &&
             "expected string value for non integer/index/float element");
      stringValues.push_back(toolchain::cast<StringAttr>(attr).getValue());
    }
  ...
  SmallVector<char, 8> data(
      toolchain::divideCeil(storageBitWidth * values.size(), CHAR_BIT));
  APInt intVal;
  for (unsigned i = 0, e = values.size(); i < e; ++i) {
    if (auto floatAttr = toolchain::dyn_cast<FloatAttr>(values[i])) {
      ...
      auto intAttr = toolchain::cast<IntegerAttr>(values[i]);
...
DenseElementsAttr DenseElementsAttr::get(ShapedType type,
  ...
  SmallVector<char> buff(toolchain::divideCeil(values.size(), CHAR_BIT));
...
DenseElementsAttr DenseElementsAttr::get(ShapedType type,
  ...
  ComplexType complex = toolchain::cast<ComplexType>(type.getElementType());
  assert(toolchain::isa<IntegerType>(complex.getElementType()));
  assert(hasSameNumElementsOrSplat(type, values));
...
DenseElementsAttr DenseElementsAttr::get(ShapedType type,
  ...
  assert(toolchain::isa<FloatType>(type.getElementType()));
  assert(hasSameNumElementsOrSplat(type, values));
  size_t storageBitWidth = getDenseElementStorageWidth(type.getElementType());
  return DenseIntOrFPElementsAttr::getRaw(type, storageBitWidth, values);
}
...
DenseElementsAttr::get(ShapedType type,
  ...
  ComplexType complex = toolchain::cast<ComplexType>(type.getElementType());
  assert(toolchain::isa<FloatType>(complex.getElementType()));
  assert(hasSameNumElementsOrSplat(type, values));
...
bool DenseElementsAttr::isValidRawBuffer(ShapedType type,
  ...
  if (storageWidth == 1) {
    ...
    return rawBufferWidth == toolchain::alignTo<8>(numElements);
  }
...
static bool isValidIntOrFloat(Type type, int64_t dataEltSize, bool isInt,
  ...
  if (!isInt) {
    bool valid = toolchain::isa<FloatType>(type);
    if (!valid)
  ...
  auto intType = toolchain::dyn_cast<IntegerType>(type);
  if (!intType) {
...
bool DenseElementsAttr::isValidComplex(int64_t dataEltSize, bool isInt,
  ...
  return ::isValidIntOrFloat(
      toolchain::cast<ComplexType>(getElementType()).getElementType(),
...
static bool isComplexOfIntType(Type type) {
  return toolchain::isa<IntegerType>(toolchain::cast<ComplexType>(type).getElementType());
}
...
auto DenseElementsAttr::tryGetFloatValues() const
  ...
  auto eltTy = toolchain::dyn_cast<FloatType>(getElementType());
  if (!eltTy)
    return failure();
...
auto DenseElementsAttr::tryGetComplexFloatValues() const
  ...
  auto complexTy = toolchain::dyn_cast<ComplexType>(getElementType());
  if (!complexTy)
    return failure();
  auto eltTy = toolchain::dyn_cast<FloatType>(complexTy.getElementType());
  if (!eltTy)
    return failure();
...
DenseElementsAttr::mapValues(Type newElementType,
  ...
  return toolchain::cast<DenseIntElementsAttr>(*this).mapValues(newElementType,
                                                           mapping);
}

DenseElementsAttr DenseElementsAttr::mapValues(
    Type newElementType, function_ref<APInt(const APFloat &)> mapping) const {
  return toolchain::cast<DenseFPElementsAttr>(*this).mapValues(newElementType,
                                                          mapping);
}
...
static void writeAPIntsToBuffer(size_t storageWidth,
                                ...
                                APRangeT &&values) {
  size_t numValues = toolchain::size(values);
  data.resize(toolchain::divideCeil(storageWidth * numValues, CHAR_BIT));
  size_t offset = 0;
...
DenseElementsAttr DenseIntOrFPElementsAttr::getRaw(ShapedType type,
  ...
  writeAPIntsToBuffer(storageWidth, data, toolchain::map_range(values, unwrapFloat));
  return DenseIntOrFPElementsAttr::getRaw(type, data);
}
...
DenseElementsAttr DenseIntOrFPElementsAttr::getRawComplex(ShapedType type,
  ...
  assert(::isValidIntOrFloat(
             toolchain::cast<ComplexType>(type.getElementType()).getElementType(),
             dataEltSize / 2, isInt, isSigned) &&
...
void DenseIntOrFPElementsAttr::convertEndianOfCharForBEmachine(
    ...
    size_t numElements) {
  using toolchain::support::ulittle16_t;
  using toolchain::support::ulittle32_t;
  using toolchain::support::ulittle64_t;
...
static ShapedType mappingHelper(Fn mapping, Attr &attr, ShapedType inType,
                                Type newElementType,
                                toolchain::SmallVectorImpl<char> &data) {
  ...
  data.resize(toolchain::divideCeil(storageBitWidth * numRawElements, CHAR_BIT));
...
DenseElementsAttr DenseFPElementsAttr::mapValues(
    Type newElementType, function_ref<APInt(const APFloat &)> mapping) const {
  toolchain::SmallVector<char, 8> elementData;
  auto newArrayType =
...
bool DenseFPElementsAttr::classof(Attribute attr) {
  if (auto denseAttr = toolchain::dyn_cast<DenseElementsAttr>(attr))
    return toolchain::isa<FloatType>(denseAttr.getType().getElementType());
  return false;
}
...
struct DenseResourceElementsAttrIntUtil {
  static bool checkElementType(Type eltType) {
    IntegerType type = toolchain::dyn_cast<IntegerType>(eltType);
    if (!type || type.getWidth() != width)
      return false;
...
DenseResourceElementsAttrBase<T>::get(ShapedType type, StringRef blobName,
  ...
  return toolchain::cast<DenseResourceElementsAttrBase<T>>(
...
bool DenseResourceElementsAttrBase<T>::classof(Attribute attr) {
  auto resourceAttr = toolchain::dyn_cast<DenseResourceElementsAttr>(attr);
...
APFloat SparseElementsAttr::getZeroAPFloat() const {
  auto eltType = toolchain::cast<FloatType>(getElementType());
...
APInt SparseElementsAttr::getZeroAPInt() const {
  auto eltType = toolchain::cast<IntegerType>(getElementType());
...
Attribute SparseElementsAttr::getZeroAttr() const {
  ...
  if (toolchain::isa<FloatType>(eltType))
    return FloatAttr::get(eltType, 0);

  // Handle complex elements.
  if (auto complexTy = toolchain::dyn_cast<ComplexType>(eltType)) {
  ...
  // Handle string type.
  if (toolchain::isa<DenseStringElementsAttr>(getValues()))
...
AffineMap mlir::makeStridedLinearLayoutMap(ArrayRef<int64_t> strides,
  ...
  for (const auto &en : toolchain::enumerate(strides)) {
    auto dim = en.index();
