...
Pattern::Pattern(const void *rootValue, RootKind rootKind,
  ...
  toolchain::append_range(generatedOps,
                     toolchain::map_range(generatedNames, [context](StringRef name) {
...
void RewriterBase::eraseOp(Operation *op) {
  ...
  auto eraseSingleOp = [&](Operation *op) {
    ...
    assert(
        toolchain::all_of(op->getRegions(), [&](Region &r) { return r.empty(); }) &&
  ...
  std::function<void(Operation *)> eraseTree = [&](Operation *op) {
    // Erase nested ops.
    for (Region &r : toolchain::reverse(op->getRegions())) {
      ...
      while (!r.empty()) {
        ...
        toolchain::SmallPtrSet<Block *, 4> visited{nullptr};
        for (Block *b : toolchain::post_order_ext(&r.front(), visited)) {
          // Visit ops in reverse order.
          for (Operation &op :
               toolchain::make_early_inc_range(ReverseIterator::makeIterable(*b)))
            eraseTree(&op);
...
Operation *RewriterBase::eraseOpResults(Operation *op,
  ...
  for ([[maybe_unused]] auto i : toolchain::seq<unsigned>(0, op->getNumRegions()))
    state.addRegion();
  Operation *newOp = create(state);
  for (const auto &[index, region] : toolchain::enumerate(op->getRegions())) {
  ...
  for (auto i : toolchain::seq<unsigned>(0, op->getNumResults()))
    if (!eraseIndices[i])
...
void RewriterBase::replaceUsesWithIf(Value from, Value to,
  ...
  for (OpOperand &operand : toolchain::make_early_inc_range(from.getUses())) {
...
void RewriterBase::replaceUsesWithIf(ValueRange from, ValueRange to,
  ...
  bool allReplaced = true;
  for (auto it : toolchain::zip_equal(from, to)) {
    bool r = true;
