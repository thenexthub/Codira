...
ArrayAttr Builder::getBoolArrayAttr(ArrayRef<bool> values) {
  auto attrs = toolchain::map_to_vector<8>(
...
ArrayAttr Builder::getIndexArrayAttr(ArrayRef<int64_t> values) {
  auto attrs = toolchain::map_to_vector<8>(values, [this](int64_t v) -> Attribute {
...
TypedAttr Builder::getZeroAttr(Type type) {
  ...
  if (toolchain::dyn_cast<IntegerType>(type))
    return getIntegerAttr(type,
                          APInt(toolchain::cast<IntegerType>(type).getWidth(), 0));
  if (toolchain::isa<RankedTensorType, VectorType>(type)) {
    auto vtType = toolchain::cast<ShapedType>(type);
...
TypedAttr Builder::getOneAttr(Type type) {
  ...
  if (toolchain::dyn_cast<IntegerType>(type))
    return getIntegerAttr(type,
                          APInt(toolchain::cast<IntegerType>(type).getWidth(), 1));
  if (toolchain::isa<RankedTensorType, VectorType>(type)) {
    auto vtType = toolchain::cast<ShapedType>(type);
...
OpBuilder::tryFold(Operation *op, SmallVectorImpl<Value> &results,
  ...
  for (auto [foldResult, expectedType] :
       toolchain::zip_equal(foldResults, opResults.getTypes())) {

    // Normal values get pushed back directly.
    if (auto value = toolchain::dyn_cast_if_present<Value>(foldResult)) {
