...
Type mlir::getElementTypeOrSelf(Type type) {
  if (auto st = toolchain::dyn_cast<ShapedType>(type))
...
bool mlir::isOpaqueTypeWithName(Type type, StringRef dialect,
                                StringRef typeData) {
  if (auto opaque = toolchain::dyn_cast<mlir::OpaqueType>(type))
...
LogicalResult mlir::verifyCompatibleShape(ArrayRef<int64_t> shape1,
  ...
  if (shape1.size() != shape2.size())
    return failure();
  for (auto dims : toolchain::zip(shape1, shape2)) {
...
LogicalResult mlir::verifyCompatibleShape(Type type1, Type type2) {
  auto sType1 = toolchain::dyn_cast<ShapedType>(type1);
  auto sType2 = toolchain::dyn_cast<ShapedType>(type2);
...
LogicalResult mlir::verifyCompatibleShapes(TypeRange types1, TypeRange types2) {
  if (types1.size() != types2.size())
    return failure();
  for (auto it : toolchain::zip_first(types1, types2))
...
LogicalResult mlir::verifyCompatibleDims(ArrayRef<int64_t> dims) {
  if (dims.empty())
    return success();
  auto staticDim =
      toolchain::accumulate(dims, dims.front(), [](auto fold, auto dim) {
        return ShapedType::isDynamic(dim) ? fold : dim;
      });
  return success(toolchain::all_of(dims, [&](auto dim) {
...
LogicalResult mlir::verifyCompatibleShapes(TypeRange types) {
  auto shapedTypes = toolchain::map_to_vector<8>(types, toolchain::DynCastTo<ShapedType>);
  // Return failure if some, but not all are not shaped. Return early if none
  // are shaped also.
  if (toolchain::none_of(shapedTypes, [](auto t) { return t; }))
    return success();
  if (!toolchain::all_of(shapedTypes, [](auto t) { return t; }))
    return failure();
  ...
  for (Type t : types) {
    auto vType = toolchain::dyn_cast<VectorType>(t);
  ...
  auto shapes = toolchain::filter_to_vector<8>(
...
Type OperandElementTypeIterator::mapElement(Value value) const {
  return toolchain::cast<ShapedType>(value.getType()).getElementType();
}
...
TypeRange mlir::insertTypesInto(TypeRange oldTypes, ArrayRef<unsigned> indices,
  ...
  for (auto it : toolchain::zip(indices, newTypes)) {
