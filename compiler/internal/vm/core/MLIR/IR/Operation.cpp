...
Operation *Operation::create(Location location, OperationName name,
                             ...
                             OpaqueProperties properties, BlockRange successors,
                             unsigned numRegions) {
  assert(toolchain::all_of(resultTypes, [](Type t) { return t; }) &&
  ...
  int opPropertiesAllocSize = toolchain::alignTo<8>(name.getOpPropertyByteSize());
  ...
  size_t prefixByteSize = toolchain::alignTo(
...
Operation::~Operation() {
  ...
  if (!use_empty()) {
    ...
    toolchain::report_fatal_error("operation destroyed but still has uses");
  }
#endif
...
void Operation::destroy() {
  ...
  char *rawMem = reinterpret_cast<char *>(this) -
                 toolchain::alignTo(prefixAllocSize(), alignof(Operation));
  this->~Operation();
  free(rawMem);
}
...
toolchain::hash_code Operation::hashProperties() {
...
void Operation::updateOrderIfNecessary() {
  ...
  if (hasValidOrder() || toolchain::hasSingleElement(*block))
    return;
...
auto toolchain::ilist_detail::SpecificNodeAccess<
    toolchain::ilist_detail::compute_node_options<::mlir::Operation>::type>::
...
void toolchain::ilist_traits<::mlir::Operation>::deleteNode(Operation *op) {
  op->destroy();
}

Block *toolchain::ilist_traits<::mlir::Operation>::getContainingBlock() {
...
void toolchain::ilist_traits<::mlir::Operation>::addNodeToList(Operation *op) {
...
void toolchain::ilist_traits<::mlir::Operation>::removeNodeFromList(Operation *op) {
...
void toolchain::ilist_traits<::mlir::Operation>::transferNodesFromList(
...
void Operation::moveBefore(Block *block,
                           toolchain::iplist<Operation>::iterator iterator) {
  assert(getBlock() &&
...
static void checkFoldResultTypes(Operation *op,
  ...
  for (auto [ofr, opResult] : toolchain::zip_equal(results, op->getResults())) {
...
void OpState::genericPrintProperties(OpAsmPrinter &p, Attribute properties,
  ...
  if (dictAttr && !elidedProps.empty()) {
    ...
    toolchain::SmallDenseSet<StringRef> elidedAttrsSet(elidedProps.begin(),
                                                  elidedProps.end());
    auto filteredAttrs =
...
OpTrait::impl::foldCommutative(Operation *op, ArrayRef<Attribute> operands,
  ...
  auto *firstConstantIt = toolchain::find_if_not(op->getOpOperands(), isNonConstant);
...
static Type getTensorOrVectorElementType(Type type) {
  if (auto vec = toolchain::dyn_cast<VectorType>(type))
    return vec.getElementType();

  // Look through tensor<vector<...>> to find the underlying element type.
  if (auto tensor = toolchain::dyn_cast<TensorType>(type))
...
LogicalResult OpTrait::impl::verifySameTypeOperands(Operation *op) {
  ...
  for (auto opType : toolchain::drop_begin(op->getOperandTypes(), 1))
    if (opType != type)
...
LogicalResult OpTrait::impl::verifySameOperandsAndResultShape(Operation *op) {
  ...
  types.append(toolchain::to_vector<4>(op->getResultTypes()));
...
LogicalResult OpTrait::impl::verifySameOperandsElementType(Operation *op) {
  ...
  for (auto operand : toolchain::drop_begin(op->getOperands(), 1)) {
...
OpTrait::impl::verifySameOperandsAndResultElementType(Operation *op) {
  ...
  for (auto result : toolchain::drop_begin(op->getResults(), 1)) {
...
LogicalResult OpTrait::impl::verifySameOperandsAndResultType(Operation *op) {
  ...
  for (auto resultType : toolchain::drop_begin(op->getResultTypes())) {
...
LogicalResult OpTrait::impl::verifySameOperandsAndResultRank(Operation *op) {
  ...
  auto rankedOperandTypes =
      toolchain::make_filter_range(op->getOperandTypes(), hasRank);
  auto rankedResultTypes =
      toolchain::make_filter_range(op->getResultTypes(), hasRank);
...
LogicalResult OpTrait::impl::verifyResultsAreFloatLike(Operation *op) {
  for (auto resultType : op->getResultTypes())
    if (!toolchain::isa<FloatType>(getTensorOrVectorElementType(resultType)))
...
LogicalResult OpTrait::impl::verifyValueSizeAttr(Operation *op,
  ...
  if (toolchain::any_of(sizes, [](int32_t element) { return element < 0; }))
    return op->emitOpError("'")
           << attrName << "' attribute cannot have negative elements";

  size_t totalCount = toolchain::sum_of(sizes, size_t(0));
...
LogicalResult OpTrait::impl::verifyElementwise(Operation *op) {
  auto isMappableType = toolchain::IsaPred<VectorType, TensorType>;
  auto resultMappableTypes =
      toolchain::filter_to_vector<1>(op->getResultTypes(), isMappableType);
  auto operandMappableTypes =
      toolchain::filter_to_vector<2>(op->getOperandTypes(), isMappableType);
  ...
  SmallVector<Type, 4> types = toolchain::to_vector<2>(
      toolchain::concat<Type>(operandMappableTypes, resultMappableTypes));
  TypeID expectedBaseTy = types.front().getTypeID();
  if (!toolchain::all_of(types,
