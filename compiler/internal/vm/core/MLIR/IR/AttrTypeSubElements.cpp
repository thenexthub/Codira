...
WalkResult AttrTypeWalker::walkImpl(T element, WalkFns &walkFns,
  ...
  for (auto &walkFn : toolchain::reverse(walkFns)) {
...
static T replaceElementImpl(T element, ReplaceFns &replaceFns,
  ...
  for (auto &replaceFn : toolchain::reverse(replaceFns)) {
...
CyclicAttrTypeReplacer::breakCycleImpl(void *element) {
  ...
  if (auto attr = dyn_cast<Attribute>(attrType)) {
    for (auto &cyclicReplaceFn : toolchain::reverse(attrCycleBreakerFns)) {
    ...
    for (auto &cyclicReplaceFn : toolchain::reverse(typeCycleBreakerFns)) {
