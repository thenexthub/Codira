...
#define FLOAT_TYPE_SEMANTICS(TYPE, SEM)                                        \
  const toolchain::fltSemantics &TYPE::getFloatSemantics() const {                  \
    return APFloat::SEM();                                                     \
  }
...
VectorType VectorType::scaleElementBitwidth(unsigned scale) {
  if (!scale)
    return VectorType();
  if (auto et = toolchain::dyn_cast<IntegerType>(getElementType()))
    if (auto scaledEt = et.scaleElementBitwidth(scale))
      return VectorType::get(getShape(), scaledEt, getScalableDims());
  if (auto et = toolchain::dyn_cast<FloatType>(getElementType()))
    if (auto scaledEt = et.scaleElementBitwidth(scale))
      return VectorType::get(getShape(), scaledEt, getScalableDims());
  return VectorType();
}
...
Type TensorType::getElementType() const {
  return toolchain::TypeSwitch<TensorType, Type>(*this)
      .Case<RankedTensorType, UnrankedTensorType>(
          [](auto type) { return type.getElementType(); });
}

bool TensorType::hasRank() const {
  return !toolchain::isa<UnrankedTensorType>(*this);
}

ArrayRef<int64_t> TensorType::getShape() const {
  return toolchain::cast<RankedTensorType>(*this).getShape();
}

TensorType TensorType::cloneWith(std::optional<ArrayRef<int64_t>> shape,
                                 Type elementType) const {
  if (toolchain::dyn_cast<UnrankedTensorType>(*this)) {
    if (shape)
      return RankedTensorType::get(*shape, elementType);
    return UnrankedTensorType::get(elementType);
  }

  auto rankedTy = toolchain::cast<RankedTensorType>(*this);
  if (!shape)
...
RankedTensorType TensorType::clone(::toolchain::ArrayRef<int64_t> shape,
                                   Type elementType) const {
  return ::toolchain::cast<RankedTensorType>(cloneWith(shape, elementType));
}

RankedTensorType TensorType::clone(::toolchain::ArrayRef<int64_t> shape) const {
  return ::toolchain::cast<RankedTensorType>(cloneWith(shape, getElementType()));
}
...
bool TensorType::isValidElementType(Type type) {
  ...
  return toolchain::isa<ComplexType, FloatType, IntegerType, OpaqueType, VectorType,
                   IndexType>(type) ||
         !toolchain::isa<BuiltinDialect>(type.getDialect());
}
...
RankedTensorType::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (auto v = toolchain::dyn_cast_or_null<VerifiableTensorEncoding>(encoding))
...
BaseMemRefType BaseMemRefType::cloneWith(std::optional<ArrayRef<int64_t>> shape,
  ...
  MemRefType::Builder builder(toolchain::cast<MemRefType>(*this));
  if (shape)
...
MemRefType BaseMemRefType::clone(::toolchain::ArrayRef<int64_t> shape,
                                 Type elementType) const {
  return ::toolchain::cast<MemRefType>(cloneWith(shape, elementType));
}

MemRefType BaseMemRefType::clone(::toolchain::ArrayRef<int64_t> shape) const {
  return ::toolchain::cast<MemRefType>(cloneWith(shape, getElementType()));
}

Attribute BaseMemRefType::getMemorySpace() const {
  if (auto rankedMemRefTy = toolchain::dyn_cast<MemRefType>(*this))
    return rankedMemRefTy.getMemorySpace();
  return toolchain::cast<UnrankedMemRefType>(*this).getMemorySpace();
}

unsigned BaseMemRefType::getMemorySpaceAsInt() const {
  if (auto rankedMemRefTy = toolchain::dyn_cast<MemRefType>(*this))
    return rankedMemRefTy.getMemorySpaceAsInt();
  return toolchain::cast<UnrankedMemRefType>(*this).getMemorySpaceAsInt();
}
...
std::optional<toolchain::SmallDenseSet<unsigned>>
mlir::computeRankReductionMask(ArrayRef<int64_t> originalShape,
                               ArrayRef<int64_t> reducedShape,
                               bool matchDynamic) {
  size_t originalRank = originalShape.size(), reducedRank = reducedShape.size();
  toolchain::SmallDenseSet<unsigned> unusedDims;
...
mlir::isRankReducedType(ShapedType originalType,
  ...
  ShapedType originalShapedType = toolchain::cast<ShapedType>(originalType);
  ShapedType candidateReducedShapedType =
      toolchain::cast<ShapedType>(candidateReducedType);
...
bool mlir::detail::isSupportedMemorySpace(Attribute memorySpace) {
  ...
  if (toolchain::isa<IntegerAttr, StringAttr, DictionaryAttr>(memorySpace))
    return true;
...
Attribute mlir::detail::skipDefaultMemorySpace(Attribute memorySpace) {
  IntegerAttr intMemorySpace = toolchain::dyn_cast_or_null<IntegerAttr>(memorySpace);
...
unsigned mlir::detail::getMemorySpaceAsInt(Attribute memorySpace) {
  ...
  assert(toolchain::isa<IntegerAttr>(memorySpace) &&
         "Using `getMemorySpaceInteger` with non-Integer attribute");

  return static_cast<unsigned>(toolchain::cast<IntegerAttr>(memorySpace).getInt());
}
...
MemRefType MemRefType::canonicalizeStridedLayout() {
  ...
  if (m.getNumDims() == 0 && m.getNumSymbols() == 0) {
    if (auto cst = toolchain::dyn_cast<AffineConstantExpr>(m.getResult(0)))
      if (cst.getValue() == 0)
...
void TupleType::getFlattenedTypes(SmallVectorImpl<Type> &types) {
  for (Type type : getTypes()) {
    if (auto nestedTuple = toolchain::dyn_cast<TupleType>(type))
...
AffineExpr mlir::makeCanonicalStridedLayoutExpr(ArrayRef<int64_t> sizes,
  ...
  for (auto en : toolchain::zip(toolchain::reverse(exprs), toolchain::reverse(sizes))) {
...
AffineExpr mlir::makeCanonicalStridedLayoutExpr(ArrayRef<int64_t> sizes,
  ...
  for (auto dim : toolchain::seq<unsigned>(0, sizes.size()))
