...
#include "vm/core/Support/PrettyStackTrace.h"
...
struct DiagnosticEngineImpl {
  ...
  toolchain::sys::SmartMutex<true> mutex;

  /// These are the handlers used to report diagnostics.
  toolchain::SmallMapVector<DiagnosticEngine::HandlerID, DiagnosticEngine::HandlerTy,
                       2>
      handlers;
...
void DiagnosticEngineImpl::emit(Diagnostic &&diag) {
  toolchain::sys::SmartScopedLock<true> lock(mutex);
  ...
  // processed first.
  for (auto &handlerIt : toolchain::reverse(handlers))
  ...
  if (!toolchain::isa<UnknownLoc>(diag.getLocation()))
    os << diag.getLocation() << ": ";
  os << "error: ";
...
auto DiagnosticEngine::registerHandler(HandlerTy handler) -> HandlerID {
  toolchain::sys::SmartScopedLock<true> lock(impl->mutex);
...
emitDiag(Location location, DiagnosticSeverity severity, const Twine &message) {
  ...
  if (ctx->shouldPrintStackTraceOnDiagnostic()) {
    std::string bt;
    {
      toolchain::raw_string_ostream stream(bt);
      toolchain::sys::PrintStackTrace(stream);
    }
    if (!bt.empty())
...
struct SourceMgrDiagnosticHandlerImpl {
  ...
  /// can be found.
  unsigned getSourceMgrBufferIDForFile(toolchain::SourceMgr &mgr,
                                       StringRef filename) {
  ...
  toolchain::StringMap<unsigned> filenameToBufId;
};
} // namespace detail
} // namespace mlir
...
static toolchain::SourceMgr::DiagKind getDiagKind(DiagnosticSeverity kind) {
  switch (kind) {
  case DiagnosticSeverity::Note:
    return toolchain::SourceMgr::DK_Note;
  case DiagnosticSeverity::Warning:
    return toolchain::SourceMgr::DK_Warning;
  case DiagnosticSeverity::Error:
    return toolchain::SourceMgr::DK_Error;
  case DiagnosticSeverity::Remark:
    return toolchain::SourceMgr::DK_Remark;
  }
  llvm_unreachable("Unknown DiagnosticSeverity");
}

SourceMgrDiagnosticHandler::SourceMgrDiagnosticHandler(
    toolchain::SourceMgr &mgr, MLIRContext *ctx, raw_ostream &os,
...
SourceMgrDiagnosticHandler::SourceMgrDiagnosticHandler(
    toolchain::SourceMgr &mgr, MLIRContext *ctx, ShouldShowLocFn &&shouldShowLocFn)
    : SourceMgrDiagnosticHandler(mgr, ctx, toolchain::errs(),
...
void SourceMgrDiagnosticHandler::emitDiagnostic(Location loc, Twine message,
  ...
  if (!fileLoc) {
    std::string str;
    toolchain::raw_string_ostream strOS(str);
  ...
  locOS << fileLoc.getFilename().getValue() << ":" << fileLoc.getLine() << ":"
        << fileLoc.getColumn();
  toolchain::SMDiagnostic diag(locStr, getDiagKind(kind), message.str());
...
void SourceMgrDiagnosticHandler::emitDiagnostic(Diagnostic &diag) {
  ...
  if (locationStack.empty()) {
    ...
    for (auto &it : toolchain::drop_begin(locationStack))
...
struct ExpectedDiag {
  ...
  LogicalResult emitError(raw_ostream &os, toolchain::SourceMgr &mgr,
                          const Twine &msg) {
    SMRange range(fileLoc, SMLoc::getFromPointer(fileLoc.getPointer() +
                                                 substring.size()));
    mgr.PrintMessage(os, fileLoc, toolchain::SourceMgr::DK_Error, msg, range);
    return failure();
  }
  ...
  LogicalResult computeRegex(raw_ostream &os, toolchain::SourceMgr &mgr) {
    std::string regexStr;
    toolchain::raw_string_ostream regexOS(regexStr);
    ...
    while (!strToProcess.empty()) {
      ...
      if (regexIt == StringRef::npos) {
        regexOS << toolchain::Regex::escape(strToProcess);
        break;
      }
      regexOS << toolchain::Regex::escape(strToProcess.take_front(regexIt));
  ...
  /// regex string.
  std::optional<toolchain::Regex> substringRegex;
};
...
struct SourceMgrDiagnosticVerifierHandlerImpl {
  ...
  computeExpectedDiags(raw_ostream &os, toolchain::SourceMgr &mgr,
                       const toolchain::MemoryBuffer *buf);
  ...
  toolchain::StringMap<SmallVector<ExpectedDiag, 2>> expectedDiagsPerFile;
  ...
  toolchain::Regex expected =
      toolchain::Regex("expected-(error|note|remark|warning)(-re)? "
...
SourceMgrDiagnosticVerifierHandlerImpl::computeExpectedDiags(
    raw_ostream &os, toolchain::SourceMgr &mgr, const toolchain::MemoryBuffer *buf) {
...
SourceMgrDiagnosticVerifierHandler::SourceMgrDiagnosticVerifierHandler(
    toolchain::SourceMgr &srcMgr, MLIRContext *ctx, raw_ostream &out, Level level)
...
SourceMgrDiagnosticVerifierHandler::SourceMgrDiagnosticVerifierHandler(
    toolchain::SourceMgr &srcMgr, MLIRContext *ctx, Level level)
    : SourceMgrDiagnosticVerifierHandler(srcMgr, ctx, toolchain::errs(), level) {}
...
void SourceMgrDiagnosticVerifierHandler::process(LocationAttr loc,
  ...
  if (nearMiss)
    mgr.PrintMessage(os, nearMiss->fileLoc, toolchain::SourceMgr::DK_Error,
                     "'" + getDiagKindStr(kind) +
...
namespace mlir {
namespace detail {
struct ParallelDiagnosticHandlerImpl : public toolchain::PrettyStackTraceEntry {
  ...
  ParallelDiagnosticHandlerImpl(MLIRContext *ctx) : context(ctx) {
    handlerID = ctx->getDiagEngine().registerHandler([this](Diagnostic &diag) {
      uint64_t tid = toolchain::get_threadid();
      toolchain::sys::SmartScopedLock<true> lock(mutex);
  ...
  void emitDiagnostics(toolchain::function_ref<void(Diagnostic &)> emitFn) const {
    ...
    // were emitted for.
    toolchain::stable_sort(diagnostics);
