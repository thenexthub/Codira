...
toolchain::unique_function<void(Operation *, OpAsmPrinter &printer)>
...
bool Dialect::isValidNamespace(StringRef str) {
  toolchain::Regex dialectNameRegex("^[a-zA-Z_][a-zA-Z_0-9\\$]*$");
...
void applyExtensionsFn(
    Fn &&applyExtension,
    const toolchain::MapVector<TypeID, std::unique_ptr<DialectExtensionBase>>
        &extensions) {
  ...
  while (startIt != endIt) {
    ...
    const auto subset =
        toolchain::map_to_vector(toolchain::make_range(startIt, endIt), extractExtension);
...
void DialectRegistry::applyExtensions(Dialect *dialect) const {
  ...
  auto applyExtension = [&](const DialectExtensionBase &extension) {
    ...
    const StringRef *nameIt = toolchain::find(dialectNames, dialectName);
...
bool DialectRegistry::isSubsetOf(const DialectRegistry &rhs) const {
  ...
  if (!toolchain::all_of(make_first_range(extensions), hasExtension))
    return false;
