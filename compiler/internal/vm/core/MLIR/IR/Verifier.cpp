...
private:
  using WorkItem = toolchain::PointerUnion<Operation *, Block *>;
  using WorkItemEntry = toolchain::PointerIntPair<WorkItem, 1, bool>;
...
static bool mayBeValidWithoutTerminator(Block *block) {
  if (!block->getParent())
    return true;
  if (!toolchain::hasSingleElement(*block->getParent()))
    return false;
...
LogicalResult OperationVerifier::verifyOperation(Operation &op) {
  ...
  while (!worklist.empty()) {
    ...
    if (Block *currentBlock = dyn_cast<Block *>(item)) {
      ...
      for (Operation &o : toolchain::reverse(*currentBlock)) {
        if (o.getNumRegions() == 0 ||
            !o.hasTrait<OpTrait::IsIsolatedFromAbove>())
    ...
    if (verifyRecursively)
      for (Region &region : toolchain::reverse(currentOp.getRegions()))
        for (Block &block : toolchain::reverse(region))
...
static void diagnoseInvalidOperandDominance(Operation &op, unsigned operandNo) {
  ...
  // Block argument case.
  Block *block1 = op.getBlock();
  Block *block2 = toolchain::cast<BlockArgument>(operand).getOwner();
  Region *region1 = block1->getParent();
  Region *region2 = block2->getParent();
  ...
  if (block1 == block2)
    toolchain::report_fatal_error("Internal error in dominance verification");
...
OperationVerifier::verifyDominanceOfContainedRegions(Operation &op,
                                                     DominanceInfo &domInfo) {
  toolchain::SmallVector<Operation *, 8> worklist{&op};
  while (!worklist.empty()) {
