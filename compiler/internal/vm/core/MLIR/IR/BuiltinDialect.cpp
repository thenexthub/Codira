...
struct BuiltinOpAsmDialectInterface : public OpAsmDialectInterface {
  ...
  AliasResult getAlias(Attribute attr, raw_ostream &os) const override {
    if (toolchain::isa<LocationAttr>(attr)) {
      os << "loc";
      return AliasResult::OverridableAlias;
    }
    if (auto distinct = toolchain::dyn_cast<DistinctAttr>(attr))
      if (!toolchain::isa<UnitAttr>(distinct.getReferencedAttr())) {
        os << "distinct";
        return AliasResult::OverridableAlias;
      }
...
DataLayoutSpecInterface ModuleOp::getDataLayoutSpec() {
  ...
  for (NamedAttribute attr : getOperation()->getAttrs())
    if (auto spec = toolchain::dyn_cast<DataLayoutSpecInterface>(attr.getValue()))
      return spec;
  return {};
}
...
TargetSystemSpecInterface ModuleOp::getTargetSystemSpec() {
  ...
  // layout object construction that is used for repeated queries.
  for (NamedAttribute attr : getOperation()->getAttrs())
    if (auto spec = toolchain::dyn_cast<TargetSystemSpecInterface>(attr.getValue()))
      return spec;
  return {};
}
...
LogicalResult ModuleOp::verify() {
  ...
  for (const NamedAttribute &na : (*this)->getAttrs()) {
    if (auto spec = toolchain::dyn_cast<DataLayoutSpecInterface>(na.getValue())) {
      if (layoutSpec) {
