...
DictionaryAttr NamedAttrList::getDictionary(MLIRContext *context) const {
  ...
  return toolchain::cast<DictionaryAttr>(dictionarySorted.getPointer());
}
...
MutableArrayRef<OpOperand> detail::OperandStorage::resize(Operation *owner,
  ...
  unsigned newCapacity =
      std::max(unsigned(toolchain::NextPowerOf2(capacity + 2)), newSize);
...
OperandRangeRange::OperandRangeRange(OperandRange operands,
    ...
    : OperandRangeRange(OwnerT(operands.getBase(), operandSegments), 0,
                        toolchain::cast<DenseI32ArrayAttr>(operandSegments).size()) {
}

OperandRange OperandRangeRange::join() const {
  const OwnerT &owner = getBase();
  ArrayRef<int32_t> sizeData = toolchain::cast<DenseI32ArrayAttr>(owner.second);
  return OperandRange(owner.first, toolchain::sum_of(sizeData));
}

OperandRange OperandRangeRange::dereference(const OwnerT &object,
                                            ptrdiff_t index) {
  ArrayRef<int32_t> sizeData = toolchain::cast<DenseI32ArrayAttr>(object.second);
  uint32_t startIndex = toolchain::sum_of(sizeData.take_front(index));
...
void MutableOperandRange::updateLength(unsigned newLength) {
  ...
  for (OperandSegment &segment : operandSegments) {
    auto attr = toolchain::cast<DenseI32ArrayAttr>(segment.second.getValue());
...
MutableOperandRangeRange::MutableOperandRangeRange(
    ...
    : MutableOperandRangeRange(
          ...
          toolchain::cast<DenseI32ArrayAttr>(operandSegmentAttr.getValue()).size()) {
}
...
MutableOperandRange MutableOperandRangeRange::dereference(const OwnerT &object,
  ...
  ArrayRef<int32_t> sizeData =
      toolchain::cast<DenseI32ArrayAttr>(object.second.getValue());
  uint32_t startIndex = toolchain::sum_of(sizeData.take_front(index));
...
ValueRange::OwnerT ValueRange::offset_base(const OwnerT &owner,
                                           ptrdiff_t index) {
  if (const auto *value = toolchain::dyn_cast_if_present<const Value *>(owner))
    return {value + index};
  if (auto *operand = toolchain::dyn_cast_if_present<OpOperand *>(owner))
...
toolchain::hash_code OperationEquivalence::computeHash(
    Operation *op, function_ref<toolchain::hash_code(Value)> hashOperands,
    function_ref<toolchain::hash_code(Value)> hashResults, Flags flags) {
  ...
  toolchain::hash_code hash =
      toolchain::hash_combine(op->getName(), dictAttrs, op->getResultTypes());
  if (!(flags & Flags::IgnoreProperties))
    hash = toolchain::hash_combine(hash, op->hashProperties());

  //   - Location if required
  if (!(flags & Flags::IgnoreLocations))
    hash = toolchain::hash_combine(hash, op->getLoc());
  ...
  if (op->hasTrait<mlir::OpTrait::IsCommutative>() &&
    ...
    hash = toolchain::hash_combine(hash, operandHash);
  } else {
    for (Value operand : op->getOperands())
      hash = toolchain::hash_combine(hash, hashOperands(operand));
  }

  //   - Results
  for (Value result : op->getResults())
    hash = toolchain::hash_combine(hash, hashResults(result));
  return hash;
}
...
/*static*/ bool OperationEquivalence::isRegionEquivalentTo(
  ...
  auto blocksEquivalent = [&](Block &lBlock, Block &rBlock) {
    ...
    for (auto argPair :
         toolchain::zip(lBlock.getArguments(), rBlock.getArguments())) {
    ...
    auto opsEquivalent = [&](Operation &lOp, Operation &rOp) {
      ...
      for (auto successorsPair :
           toolchain::zip(lOp.getSuccessors(), rOp.getSuccessors())) {
    ...
    return toolchain::all_of_zip(lBlock, rBlock, opsEquivalent);
  };
  return toolchain::all_of_zip(*lhs, *rhs, blocksEquivalent);
}
...
struct ValueEquivalenceCache {
  ...
  LogicalResult checkCommutativeEquivalent(ValueRange lhsRange,
    ...
    auto sortValues = [](ValueRange values) {
      SmallVector<Value> sortedValues = toolchain::to_vector(values);
      toolchain::sort(sortedValues, [](Value a, Value b) {
...
/*static*/ bool OperationEquivalence::isEquivalentTo(
  ...
  if (checkCommutativeEquivalent &&
    ...
    for (auto operandPair : toolchain::zip(lhs->getOperands(), rhs->getOperands())) {
  ...
  for (auto resultPair : toolchain::zip(lhs->getResults(), rhs->getResults())) {
  ...
  // 4. Compare regions.
  for (auto regionPair : toolchain::zip(lhs->getRegions(), rhs->getRegions()))
...
template <typename T>
static void addDataToHash(toolchain::SHA1 &hasher, const T &data) {
  hasher.update(
