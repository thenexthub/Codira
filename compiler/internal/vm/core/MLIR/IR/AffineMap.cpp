...
private:
  std::optional<int64_t> constantFoldImpl(AffineExpr expr) {
    ...
    case AffineExprKind::DimId:
      if (auto attr = toolchain::dyn_cast_or_null<IntegerAttr>(
  ...
  std::optional<int64_t> constantFoldBinExpr(
      AffineExpr expr,
      toolchain::function_ref<std::optional<int64_t>(int64_t, int64_t)> op) {
...
AffineMap AffineMap::getFilteredIdentityMap(
    ...
    toolchain::function_ref<bool(AffineDimExpr)> keepDimFilter) {
  auto identityMap = getMultiDimIdentityMap(numDims, ctx);

  // Apply filter to results.
  toolchain::SmallBitVector dropDimResults(numDims);
  for (auto [idx, resultExpr] : toolchain::enumerate(identityMap.getResults()))
...
SmallVector<unsigned> AffineMap::getBroadcastDims() const {
  ...
  for (const auto &[resIdx, expr] : toolchain::enumerate(getResults())) {
...
bool AffineMap::isMinorIdentityWithBroadcasting(
  ...
  for (const auto &idxAndExpr : toolchain::enumerate(getResults())) {
...
bool AffineMap::isPermutationOfMinorIdentityWithBroadcasting(
  ...
  toolchain::SmallBitVector dimFound(std::max(getNumInputs(), getNumResults()),
                                false);
  for (const auto &idxAndExpr : toolchain::enumerate(getResults())) {
    unsigned resIdx = idxAndExpr.index();
    AffineExpr expr = idxAndExpr.value();
...
AffineMap AffineMap::getPermutationMap(ArrayRef<unsigned> permutation,
  ...
  const auto *m = toolchain::max_element(permutation);
...
AffineMap AffineMap::getPermutationMap(ArrayRef<int64_t> permutation,
                                       MLIRContext *context) {
  SmallVector<unsigned> perm = toolchain::map_to_vector(
...
bool AffineMap::isConstant() const {
  return toolchain::all_of(getResults(), toolchain::IsaPred<AffineConstantExpr>);
}
...
LogicalResult AffineMap::constantFold(ArrayRef<Attribute> operandConstants,
  ...
  auto range = toolchain::map_range(integers, [this](int64_t i) {
...
void AffineMap::walkExprs(toolchain::function_ref<void(AffineExpr)> callback) const {
  for (auto expr : getResults())
    expr.walk(callback);
}
...
AffineMap AffineMap::dropResults(const toolchain::SmallBitVector &positions) const {
  auto exprs = toolchain::to_vector<4>(getResults());
...
static SmallVector<AffineMap> compressUnusedListImpl(
    ...
    toolchain::function_ref<AffineMap(AffineMap)> compressionFun) {
  if (maps.empty())
  ...
  for (auto m : maps) {
    ...
    toolchain::append_range(allExprs, m.getResults());
  }
...
AffineMap mlir::compressDims(AffineMap map,
                             const toolchain::SmallBitVector &unusedDims) {
...
AffineMap mlir::compressSymbols(AffineMap map,
                                const toolchain::SmallBitVector &unusedSymbols) {
...
AffineMap mlir::removeDuplicateExprs(AffineMap map) {
  ...
  uniqueExprs.erase(toolchain::unique(uniqueExprs), uniqueExprs.end());
...
AffineMap mlir::inversePermutation(AffineMap map) {
  ...
  for (const auto &en : toolchain::enumerate(map.getResults())) {
    auto expr = en.value();
...
static AffineMap projectCommonImpl(AffineMap map,
                                   const toolchain::SmallBitVector &toProject,
                                   bool compress) {
  static_assert(toolchain::is_one_of<AffineDimOrSymExpr, AffineDimExpr,
...
AffineMap mlir::projectDims(AffineMap map,
                            const toolchain::SmallBitVector &projectedDimensions,
...
AffineMap mlir::projectSymbols(AffineMap map,
                               const toolchain::SmallBitVector &projectedSymbols,
...
toolchain::SmallBitVector mlir::getUnusedDimsBitVector(ArrayRef<AffineMap> maps) {
  unsigned numDims = maps[0].getNumDims();
  toolchain::SmallBitVector numDimsBitVector(numDims, true);
...
toolchain::SmallBitVector mlir::getUnusedSymbolsBitVector(ArrayRef<AffineMap> maps) {
  unsigned numSymbols = maps[0].getNumSymbols();
  toolchain::SmallBitVector numSymbolsBitVector(numSymbols, true);
  for (AffineMap m : maps) {
...
void MutableAffineMap::reset(AffineMap map) {
  ...
  toolchain::append_range(results, map.getResults());
}
