...
struct MLIRContextOptions {
  toolchain::cl::opt<bool> disableThreading{
      "mlir-disable-threading",
      toolchain::cl::desc("Disable multi-threading within MLIR, overrides any "
                     "further call to MLIRContext::enableMultiThreading()")};

  toolchain::cl::opt<bool> printOpOnDiagnostic{
      "mlir-print-op-on-diagnostic",
      toolchain::cl::desc("When a diagnostic is emitted on an operation, also print "
                     "the operation as an attached note"),
      toolchain::cl::init(true)};

  toolchain::cl::opt<bool> printStackTraceOnDiagnostic{
      "mlir-print-stacktrace-on-diagnostic",
      toolchain::cl::desc("When a diagnostic is emitted, also print the stack trace "
                     "as an attached note")};
};
} // namespace

static toolchain::ManagedStatic<MLIRContextOptions> clOptions;
...
struct ScopedWriterLock {
  ScopedWriterLock(toolchain::sys::SmartRWMutex<true> &mutexParam, bool shouldLock)
...
public:
  ...
  toolchain::ThreadPoolInterface *threadPool = nullptr;

  /// In case where the thread pool is owned by the context, this ensures
  /// destruction with the context.
  std::unique_ptr<toolchain::ThreadPoolInterface> ownedThreadPool;

  /// An allocator used for AbstractAttribute and AbstractType objects.
  toolchain::BumpPtrAllocator abstractDialectSymbolAllocator;

  /// This is a mapping from operation name to the operation info describing it.
  toolchain::StringMap<std::unique_ptr<OperationName::Impl>> operations;

  /// A vector of operation info specifically for registered operations.
  toolchain::DenseMap<TypeID, RegisteredOperationName> registeredOperations;
  toolchain::StringMap<RegisteredOperationName> registeredOperationsByName;
  ...
  toolchain::sys::SmartRWMutex<true> operationInfoMutex;
  ...
  toolchain::DenseMap<StringRef, AbstractType *> nameToType;
  ...
  /// cannot use `StringAttr` as the key. The context does not take ownership
  /// of the key, so the `StringRef` must outlive the context.
  toolchain::DenseMap<StringRef, AbstractAttribute *> nameToAttribute;
  ...
  toolchain::sys::SmartMutex<true> dialectRefStrAttrMutex;
...
public:
  MLIRContextImpl(bool threadingIsEnabled)
    ...
    if (threadingIsEnabled) {
      ownedThreadPool = std::make_unique<toolchain::DefaultThreadPool>();
...
template <typename T>
static ArrayRef<T> copyArrayRefInto(toolchain::BumpPtrAllocator &allocator,
                                    ArrayRef<T> elements) {
  auto result = allocator.Allocate<T>(elements.size());
  toolchain::uninitialized_copy(elements, result);
...
std::vector<Dialect *> MLIRContext::getLoadedDialects() {
  ...
  toolchain::array_pod_sort(result.begin(), result.end(),
...
MLIRContext::getOrLoadDialect(StringRef dialectNamespace, TypeID dialectID,
  ...
  if (dialect->getTypeID() != dialectID)
    toolchain::report_fatal_error("a dialect with namespace '" + dialectNamespace +
...
toolchain::hash_code MLIRContext::getRegistryHash() {
  toolchain::hash_code hash(0);
  // Factor in number of loaded dialects, attributes, operations, types.
  hash = toolchain::hash_combine(hash, impl->loadedDialects.size());
  hash = toolchain::hash_combine(hash, impl->registeredAttributes.size());
  hash = toolchain::hash_combine(hash, impl->registeredOperations.size());
  hash = toolchain::hash_combine(hash, impl->registeredTypes.size());
  return hash;
}
...
bool MLIRContext::isMultithreadingEnabled() {
  return impl->threadingIsEnabled && toolchain::llvm_is_multithreaded();
}
...
void MLIRContext::disableMultithreading(bool disable) {
  ...
  if (disable) {
    ...
    impl->ownedThreadPool = std::make_unique<toolchain::DefaultThreadPool>();
    impl->threadPool = impl->ownedThreadPool.get();
  }
}

void MLIRContext::setThreadPool(toolchain::ThreadPoolInterface &pool) {
...
toolchain::ThreadPoolInterface &MLIRContext::getThreadPool() {
  assert(isMultithreadingEnabled() &&
...
MLIRContext::getRegisteredOperationsByDialect(StringRef dialectName) {
  auto *lowerBound = toolchain::lower_bound(
...
void Dialect::addType(TypeID typeID, AbstractType &&typeInfo) {
  ...
  if (!impl.registeredTypes.insert({typeID, newInfo}).second)
    toolchain::report_fatal_error("Dialect Type already registered.");
  if (!impl.nameToType.insert({newInfo->getName(), newInfo}).second)
    toolchain::report_fatal_error("Dialect Type with name " + newInfo->getName() +
...
void Dialect::addAttribute(TypeID typeID, AbstractAttribute &&attrInfo) {
  ...
  if (!impl.registeredAttributes.insert({typeID, newInfo}).second)
    toolchain::report_fatal_error("Dialect Attribute already registered.");
  if (!impl.nameToAttribute.insert({newInfo->getName(), newInfo}).second)
    toolchain::report_fatal_error("Dialect Attribute with name " +
...
const AbstractAttribute &AbstractAttribute::lookup(TypeID typeID,
  ...
  if (!abstract)
    toolchain::report_fatal_error("Trying to create an Attribute that was not "
...
OperationName::OperationName(StringRef name, MLIRContext *context) {
  ...
  if (isMultithreadingEnabled) {
    ...
    toolchain::sys::SmartScopedReader<true> contextLock(ctxImpl.operationInfoMutex);
...
OperationName::UnregisteredOpModel::getParseAssemblyFn() {
  toolchain::report_fatal_error("getParseAssemblyFn hook called on unregistered op");
}
...
OperationName::UnregisteredOpModel::hashProperties(OpaqueProperties prop) {
  return toolchain::hash_combine(*prop.as<Attribute *>());
}
...
void RegisteredOperationName::insert(
  ...
  if (!attrNames.empty()) {
    cachedAttrNames = MutableArrayRef<StringAttr>(
        ctxImpl.abstractDialectSymbolAllocator.Allocate<StringAttr>(
            attrNames.size()),
        attrNames.size());
    for (unsigned i : toolchain::seq<unsigned>(0, attrNames.size()))
  ...
  ctxImpl.sortedRegisteredOperations.insert(
      toolchain::upper_bound(ctxImpl.sortedRegisteredOperations, value,
                        [](auto &lhs, auto &rhs) {
...
void StringAttrStorage::initialize(MLIRContext *context) {
  ...
  MLIRContextImpl &impl = context->getImpl();
  toolchain::sys::SmartScopedLock<true> lock(impl.dialectRefStrAttrMutex);
...
toolchain::unique_function<InFlightDiagnostic()>
