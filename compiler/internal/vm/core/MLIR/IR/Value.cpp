...
Location Value::getLoc() const {
  ...
  return toolchain::cast<BlockArgument>(*this).getLoc();
}
...
void Value::setLoc(Location loc) {
  ...
  return toolchain::cast<BlockArgument>(*this).setLoc(loc);
}
...
Region *Value::getParentRegion() {
  ...
  return toolchain::cast<BlockArgument>(*this).getOwner()->getParent();
}
...
Block *Value::getParentBlock() {
  if (Operation *op = getDefiningOp())
    return op->getBlock();
  return toolchain::cast<BlockArgument>(*this).getOwner();
}
...
void Value::replaceAllUsesExcept(
  ...
  for (OpOperand &use : toolchain::make_early_inc_range(getUses())) {
...
bool Value::isUsedOutsideOfBlock(Block *block) const {
  return toolchain::any_of(getUsers(), [block](Operation *user) {
