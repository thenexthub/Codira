...
#include "vm/core/ADT/ScopedHashTable.h"
...
#include "vm/core/Support/SaveAndRestore.h"
...
void OperationName::dump() const { print(toolchain::errs()); }
...
/// https://github.com/toolchain/toolchain-project/issues/62918
...
void OpAsmPrinter::printFunctionalType(Operation *op) {
  auto &os = getStream();
  os << '(';
  toolchain::interleaveComma(op->getOperands(), os, [&](Value operand) {
  ...
  toolchain::interleaveComma(op->getResults(), os, [&](const OpResult &result) {
    // Print the types of null values as <<NULL TYPE>>.
...
struct AsmPrinterOptions {
  toolchain::cl::opt<int64_t> printElementsAttrWithHexIfLarger{
  ...
  toolchain::cl::opt<unsigned> elideElementsAttrIfLarger{
      "mlir-elide-elementsattrs-if-larger",
      toolchain::cl::desc("Elide ElementsAttrs with \"...\" that have "
                     "more elements than the given upper limit")};

  toolchain::cl::opt<unsigned> elideResourceStringsIfLarger{
      "mlir-elide-resource-strings-if-larger",
      toolchain::cl::desc(
          "Elide printing value of resources if string is too long in chars.")};

  toolchain::cl::opt<bool> printDebugInfoOpt{
      "mlir-print-debuginfo", toolchain::cl::init(false),
      toolchain::cl::desc("Print debug info in MLIR output")};

  toolchain::cl::opt<bool> printPrettyDebugInfoOpt{
      "mlir-pretty-debuginfo", toolchain::cl::init(false),
      toolchain::cl::desc("Print pretty debug info in MLIR output")};

  // Use the generic op output form in the operation printer even if the custom
  // form is defined.
  toolchain::cl::opt<bool> printGenericOpFormOpt{
      "mlir-print-op-generic", toolchain::cl::init(false),
      toolchain::cl::desc("Print the generic op form"), toolchain::cl::Hidden};

  toolchain::cl::opt<bool> assumeVerifiedOpt{
      "mlir-print-assume-verified", toolchain::cl::init(false),
      toolchain::cl::desc("Skip op verification when using custom printers"),
      toolchain::cl::Hidden};

  toolchain::cl::opt<bool> printLocalScopeOpt{
      "mlir-print-local-scope", toolchain::cl::init(false),
      toolchain::cl::desc("Print with local scope and inline information (eliding "
                     "aliases for attributes, types, and locations)")};

  toolchain::cl::opt<bool> skipRegionsOpt{
      "mlir-print-skip-regions", toolchain::cl::init(false),
      toolchain::cl::desc("Skip regions when printing ops.")};

  toolchain::cl::opt<bool> printValueUsers{
      "mlir-print-value-users", toolchain::cl::init(false),
      toolchain::cl::desc(
          "Print users of operation results and block arguments as a comment")};

  toolchain::cl::opt<bool> printUniqueSSAIDs{
      "mlir-print-unique-ssa-ids", toolchain::cl::init(false),
      toolchain::cl::desc("Print unique SSA ID numbers for values, block arguments "
                     "and naming conflicts across all regions")};

  toolchain::cl::opt<bool> useNameLocAsPrefix{
      "mlir-use-nameloc-as-prefix", toolchain::cl::init(false),
      toolchain::cl::desc("Print SSA IDs using NameLocs as prefixes")};
};
} // namespace

static toolchain::ManagedStatic<AsmPrinterOptions> clOptions;
...
bool OpPrintingFlags::shouldElideElementsAttr(ElementsAttr attr) const {
  return elementsAttrElementLimit &&
         *elementsAttrElementLimit < int64_t(attr.getNumElements()) &&
         !toolchain::isa<SplatElementsAttr>(attr);
}
...
public:
  ...
  inline void interleaveComma(const Container &c, UnaryFunctor eachFn) const {
    toolchain::interleaveComma(c, os, eachFn);
  }
...
private:
  ...
  visitImpl(T value,
            toolchain::MapVector<const void *, InProgressAliasInfo> &aliases,
  ...
  static unsigned
  uniqueAliasNameIndex(StringRef alias, toolchain::StringMap<unsigned> &nameCounts,
                       toolchain::StringSet<toolchain::BumpPtrAllocator &> &usedAliases);
  ...
  static void initializeAliases(
      toolchain::MapVector<const void *, InProgressAliasInfo> &visitedSymbols,
      toolchain::MapVector<const void *, SymbolAlias> &symbolToAlias);
  ...
  toolchain::raw_svector_ostream aliasOS;
};
...
private:
  ...
  void print(Block *block, bool printBlockArgs = true,
    ...
    auto range = toolchain::make_range(
        block->begin(),
        std::prev(block->end(),
  ...
  void printRegion(Region &region, bool printEntryBlockArgs,
    ...
    for (Block &b : toolchain::drop_begin(region, 1))
      print(&b);
  }
  ...
  void printOptionalAttrDict(ArrayRef<NamedAttribute> attrs,
    ...
    toolchain::SmallDenseSet<StringRef> elidedAttrsSet(elidedAttrs.begin(),
                                                  elidedAttrs.end());
    for (const NamedAttribute &attr : attrs)
  ...
  mutable toolchain::raw_null_ostream os;
};
...
private:
  ...
  void printAndVisitNestedAliasesImpl(Attribute attr, bool elideType) {
    ...
    } else if (toolchain::isa<AffineMapAttr, DenseArrayAttr, FloatAttr, IntegerAttr,
    ...
    if (!elideType) {
      if (auto typedAttr = toolchain::dyn_cast<TypedAttr>(attr)) {
        Type attrType = typedAttr.getType();
        if (!toolchain::isa<NoneType>(attrType))
          printType(attrType);
      }
    }
  }
  void printAndVisitNestedAliasesImpl(Type type) {
    ...
    if (auto memrefTy = toolchain::dyn_cast<MemRefType>(type)) {
      printType(memrefTy.getElementType());
      MemRefLayoutAttrInterface layout = memrefTy.getLayout();
      if (!toolchain::isa<AffineMapAttr>(layout) || !layout.isIdentity())
...
static StringRef sanitizeIdentifier(StringRef name, SmallString<16> &buffer,
  ...
  auto validChar = [&](char ch) {
    return toolchain::isAlnum(ch) || allowedPunctChars.contains(ch);
  };
  ...
  auto copyNameToBuffer = [&] {
    for (char ch : name) {
      if (validChar(ch))
        ...
        buffer.append(toolchain::utohexstr((unsigned char)ch));
    }
  };
...
unsigned AliasInitializer::uniqueAliasNameIndex(
    StringRef alias, toolchain::StringMap<unsigned> &nameCounts,
    toolchain::StringSet<toolchain::BumpPtrAllocator &> &usedAliases) {
  ...
  while (true) {
    ...
    probeAlias += toolchain::utostr(nameIndex);
...
void AliasInitializer::initializeAliases(
  ...
  toolchain::stable_sort(unprocessedAliases, toolchain::less_second());

  // This keeps track of all of the non-numeric names that are in flight,
  // allowing us to check for duplicates.
  toolchain::BumpPtrAllocator usedAliasAllocator;
  toolchain::StringSet<toolchain::BumpPtrAllocator &> usedAliases(usedAliasAllocator);

  toolchain::StringMap<unsigned> nameCounts;
...
std::pair<size_t, size_t> AliasInitializer::visitImpl(
    T value, toolchain::MapVector<const void *, InProgressAliasInfo> &aliases,
    bool canBeDeferred, PrintArgs &&...printArgs) {
...
void AliasState::printAliases(AsmPrinter::Impl &p, NewLineCounter &newLine,
  ...
  for (auto &[opaqueSymbol, alias] :
       toolchain::make_filter_range(attrTypeToAlias, filterFn)) {
...
private:
  ...
  toolchain::ScopedHashTable<StringRef, char> usedNames;
  toolchain::BumpPtrAllocator usedNameAllocator;
...
SSANameState::SSANameState(Operation *op, const OpPrintingFlags &printerFlags)
  ...
  toolchain::SaveAndRestore valueIDSaver(nextValueID);
  toolchain::SaveAndRestore argumentIDSaver(nextArgumentID);
  toolchain::SaveAndRestore conflictIDSaver(nextConflictID);
  ...
  using UsedNamesScopeTy = toolchain::ScopedHashTable<StringRef, char>::ScopeTy;
  using NamingContext =
...
void SSANameState::shadowRegionArgs(Region &region, ValueRange namesToUse) {
  ...
  for (unsigned i = 0, e = namesToUse.size(); i != e; ++i) {
    ...
    nameStr.clear();
    toolchain::raw_svector_ostream nameStream(nameStr);
...
void SSANameState::numberValuesInRegion(Region &region) {
  ...
  auto setBlockArgNameFn = [&](Value arg, StringRef name) {
    ...
    assert(toolchain::cast<BlockArgument>(arg).getOwner()->getParent() == &region &&
  ...
  for (auto &block : region) {
    ...
    if (blockInfoIt.second) {
      ...
      std::string name;
      toolchain::raw_string_ostream(name) << "^bb" << nextBlockID;
...
void SSANameState::numberValuesInBlock(Block &block) {
  ...
  toolchain::raw_svector_ostream specialName(specialNameBuffer);
...
void SSANameState::numberValuesInOp(Operation &op) {
  ...
  auto setResultNameFn = [&](Value result, StringRef name) {
    ...
    if (int resultNo = toolchain::cast<OpResult>(result).getResultNumber())
  ...
  if (!printerFlags.shouldPrintGenericOpForm()) {
    ...
    if (!opAsmOpInterfaceUsed) {
      ...
      bool allHaveOpAsmTypeInterface =
          toolchain::all_of(op.getResultTypes(), [&](Type type) {
  ...
  if (resultGroups.size() != 1) {
    toolchain::array_pod_sort(resultGroups.begin(), resultGroups.end());
...
void SSANameState::getResultIDAndNumber(
  ...
  const auto *it = toolchain::upper_bound(resultGroups, resultNo);
...
StringRef SSANameState::uniqueValueName(StringRef name) {
  ...
  if (!usedNames.count(name)) {
    ...
    while (true) {
      probeName += toolchain::utostr(nextConflictID++);
...
public:
  ...
  auto getResourcePrinters() {
    return toolchain::make_pointee_range(externalResourcePrinters);
  }
...
static OpPrintingFlags verifyOpAndAdjustFlags(Operation *op,
  ...
  auto parentThreadId = toolchain::get_threadid();
  ScopedDiagnosticHandler diagHandler(op->getContext(), [&](Diagnostic &diag) {
    if (parentThreadId == toolchain::get_threadid()) {
      LLVM_DEBUG({
        diag.print(toolchain::dbgs());
        toolchain::dbgs() << "\n";
      });
      return success();
    }
...
void AsmPrinter::Impl::printLocationInternal(LocationAttr loc, bool pretty,
  ...
  TypeSwitch<LocationAttr>(loc)
      ...
      .Case<NameLoc>([&](NameLoc loc) {
        ...
        if (!toolchain::isa<UnknownLoc>(childLoc)) {
          os << '(';
      ...
      .Case<CallSiteLoc>([&](CallSiteLoc loc) {
        ...
        if (pretty) {
          if (toolchain::isa<NameLoc>(callee)) {
            if (toolchain::isa<FileLineColLoc>(caller)) {
              os << " at ";
            } else {
              os << newLine << " at ";
            }
...
static bool isDialectSymbolSimpleEnoughForPrettyForm(StringRef symName) {
  ...
  symName = symName.drop_while(
      [](char c) { return toolchain::isAlnum(c) || c == '.' || c == '_'; });
  if (symName.empty())
    return true;
...
static bool isBareIdentifier(StringRef name) {
  ...
  if (name.empty() || (!isalpha(name[0]) && name[0] != '_'))
    return false;
  return toolchain::all_of(name.drop_front(), [](unsigned char c) {
...
void AsmPrinter::Impl::printAttributeImpl(Attribute attr,
  ...
  if (!isa<BuiltinDialect>(attr.getDialect())) {
  ...
  } else if (auto opaqueAttr = toolchain::dyn_cast<OpaqueAttr>(attr)) {
    printDialectSymbol(os, "#", opaqueAttr.getDialectNamespace(),
                       opaqueAttr.getAttrData());
  } else if (toolchain::isa<UnitAttr>(attr)) {
    os << "unit";
    return;
  } else if (auto distinctAttr = toolchain::dyn_cast<DistinctAttr>(attr)) {
    os << "distinct[" << state.getDistinctState().getId(distinctAttr) << "]<";
    if (!toolchain::isa<UnitAttr>(distinctAttr.getReferencedAttr())) {
      printAttribute(distinctAttr.getReferencedAttr());
    }
    os << '>';
    return;
  } else if (auto dictAttr = toolchain::dyn_cast<DictionaryAttr>(attr)) {
    os << '{';
    interleaveComma(dictAttr.getValue(),
                    [&](NamedAttribute attr) { printNamedAttribute(attr); });
    os << '}';

  } else if (auto intAttr = toolchain::dyn_cast<IntegerAttr>(attr)) {
  ...
  } else if (auto floatAttr = toolchain::dyn_cast<FloatAttr>(attr)) {
  ...
  } else if (auto strAttr = toolchain::dyn_cast<StringAttr>(attr)) {
    printEscapedString(strAttr.getValue());

  } else if (auto arrayAttr = toolchain::dyn_cast<ArrayAttr>(attr)) {
    os << '[';
  ...
  } else if (auto affineMapAttr = toolchain::dyn_cast<AffineMapAttr>(attr)) {
    os << "affine_map<";
  ...
  } else if (auto integerSetAttr = toolchain::dyn_cast<IntegerSetAttr>(attr)) {
    os << "affine_set<";
  ...
  } else if (auto typeAttr = toolchain::dyn_cast<TypeAttr>(attr)) {
    printType(typeAttr.getValue());

  } else if (auto refAttr = toolchain::dyn_cast<SymbolRefAttr>(attr)) {
    ...
    for (FlatSymbolRefAttr nestedRef : refAttr.getNestedReferences()) {
      ...
      printSymbolReference(nestedRef.getValue(), os);
                 ...
                 toolchain::dyn_cast<DenseIntOrFPElementsAttr>(attr)) {
  ...
  } else if (auto strEltAttr = toolchain::dyn_cast<DenseStringElementsAttr>(attr)) {
  ...
  } else if (auto sparseEltAttr = toolchain::dyn_cast<SparseElementsAttr>(attr)) {
  ...
  } else if (auto stridedLayoutAttr = toolchain::dyn_cast<StridedLayoutAttr>(attr)) {
    stridedLayoutAttr.print(os);
  } else if (auto denseArrayAttr = toolchain::dyn_cast<DenseArrayAttr>(attr)) {
    os << "array<";
    ...
    return;
                 ...
                 toolchain::dyn_cast<DenseResourceElementsAttr>(attr)) {
    os << "dense_resource<";
    printResourceHandle(resourceAttr.getRawHandle());
    os << ">";
  } else if (auto locAttr = toolchain::dyn_cast<LocationAttr>(attr)) {
    printLocation(locAttr);
  } else {
    toolchain::report_fatal_error("Unknown builtin attribute");
  }
  // Don't print the type if we must elide it, or if it is a None type.
...
void AsmPrinter::Impl::printDenseElementsAttr(DenseElementsAttr attr,
                                              bool allowHex) {
  if (auto stringAttr = toolchain::dyn_cast<DenseStringElementsAttr>(attr))
    return printDenseStringElementsAttr(stringAttr);

  printDenseIntOrFPElementsAttr(toolchain::cast<DenseIntOrFPElementsAttr>(attr),
                                allowHex);
}
...
void AsmPrinter::Impl::printDenseIntOrFPElementsAttr(
  ...
  if (allowHex && printerFlags.shouldPrintElementsAttrWithHex(attr)) {
    ...
    if (toolchain::endianness::native == toolchain::endianness::big) {
  ...
  if (ComplexType complexTy = toolchain::dyn_cast<ComplexType>(elementType)) {
    ...
    if (toolchain::isa<IntegerType>(complexElementType)) {
    ...
    assert(toolchain::isa<FloatType>(elementType) && "unexpected element type");
...
void AsmPrinter::Impl::printDenseArrayAttr(DenseArrayAttr attr) {
  ...
  auto printElementAt = [&](unsigned i) {
    ...
    if (type.isIntOrIndex()) {
      ...
      APFloat fltVal(toolchain::cast<FloatType>(type).getFloatSemantics(), value);
      printFloatValue(fltVal, getStream());
    }
  };
  toolchain::interleaveComma(toolchain::seq<unsigned>(0, attr.size()), getStream(),
                        printElementAt);
}
...
void AsmPrinter::Impl::printTypeImpl(Type type) {
  TypeSwitch<Type>(type)
      ...
      .Case<FunctionType>([&](FunctionType funcTy) {
        ...
        if (results.size() == 1 && !toolchain::isa<FunctionType>(results[0])) {
          printType(results[0]);
        } else {
          os << '(';
...
void AsmPrinter::Impl::printOptionalAttrDict(ArrayRef<NamedAttribute> attrs,
  ...
  toolchain::SmallDenseSet<StringRef> elidedAttrsSet(elidedAttrs.begin(),
                                                elidedAttrs.end());
  auto filteredAttrs = toolchain::make_filter_range(attrs, [&](NamedAttribute attr) {
...
void AsmPrinter::Impl::printNamedAttribute(NamedAttribute attr) {
  ...
  if (toolchain::isa<UnitAttr>(attr.getValue()))
    return;
...
void AsmPrinter::Impl::printDialectAttribute(Attribute attr) {
  ...
  std::string attrName;
  {
    toolchain::raw_string_ostream attrNameStr(attrName);
...
void AsmPrinter::Impl::printDialectType(Type type) {
  ...
  std::string typeName;
  {
    toolchain::raw_string_ostream typeNameStr(typeName);
...
void AsmPrinter::Impl::printEscapedString(StringRef str) {
  os << "\"";
  toolchain::printEscapedString(str, os);
  os << "\"";
}

void AsmPrinter::Impl::printHexString(StringRef str) {
  os << "\"0x" << toolchain::toHex(str) << "\"";
}
  ...
  public:
    ...
    void buildString(StringRef key, StringRef data) final {
      printFn(key, [&](raw_ostream &os) {
        os << "\"";
        toolchain::printEscapedString(data, os);
        os << "\"";
      });
    }
    ...
    void buildBlob(StringRef key, ArrayRef<char> data,
      ...
      printFn(key, [&](raw_ostream &os) {
        ...
        toolchain::support::ulittle32_t dataAlignmentLE(dataAlignment);
        os << "\"0x"
           << toolchain::toHex(StringRef(reinterpret_cast<char *>(&dataAlignmentLE),
                                    sizeof(dataAlignment)))
           << toolchain::toHex(StringRef(data.data(), data.size())) << "\"";
      });
    }
  ...
  auto processProvider = [&](StringRef dictName, StringRef name, auto &provider,
    ...
    auto printFn = [&](StringRef key, ResourceBuilder::ValueFn valueFn) {
      ...
      if (charLimit.has_value()) {
        ...
        toolchain::raw_string_ostream ss(resourceStr);
        valueFn(ss);
...
void OperationPrinter::printFullOp(Operation *op) {
  if (size_t numResults = op->getNumResults()) {
    ...
    if (!resultGroups.empty()) {
      ...
      // separately.
      interleaveComma(toolchain::seq<int>(0, resultGroups.size() - 1), [&](int i) {
...
void OperationPrinter::printGenericOp(Operation *op, bool printOpName) {
  ...
  printOptionalAttrDict(op->getPropertiesStorage()
                            ? toolchain::to_vector(op->getDiscardableAttrs())
                            : op->getAttrs());
...
void OperationPrinter::print(Block *block, bool printBlockArgs,
  ...
  if (printBlockArgs) {
    ...
    if (!block->getParent()) {
      ...
      toolchain::sort(predIDs, [](BlockInfo lhs, BlockInfo rhs) {
...
void OperationPrinter::printRegion(Region &region, bool printEntryBlockArgs,
  ...
  os << "{" << newLine;
  if (!region.empty()) {
    toolchain::scope_exit restoreDefaultDialect(
    ...
    for (auto &b : toolchain::drop_begin(region.getBlocks(), 1))
      print(&b);
  }
  os.indent(currentIndent) << "}";
}
...
void Value::print(raw_ostream &os, const OpPrintingFlags &flags) const {
  ...
  BlockArgument arg = toolchain::cast<BlockArgument>(*this);
...
void Value::printAsOperand(raw_ostream &os,
  ...
  Operation *op;
  if (auto result = toolchain::dyn_cast<OpResult>(*this)) {
    op = result.getOwner();
  } else {
    op = toolchain::cast<BlockArgument>(*this).getOwner()->getParentOp();
    if (!op) {
...
void Operation::dump() {
  print(toolchain::errs(), OpPrintingFlags().useLocalScope());
  toolchain::errs() << "\n";
}

void Operation::dumpPretty() {
  print(toolchain::errs(), OpPrintingFlags().useLocalScope().assumeVerified());
  toolchain::errs() << "\n";
}
...
void Block::dump() { print(toolchain::errs()); }
