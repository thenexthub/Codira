...
using toolchain::divideSignedWouldOverflow;
using toolchain::mod;
...
AffineExpr::replaceDimsAndSymbols(ArrayRef<AffineExpr> dimReplacements,
  ...
  case AffineExprKind::DimId: {
    unsigned dimId = toolchain::cast<AffineDimExpr>(*this).getPosition();
    if (dimId >= dimReplacements.size())
      return *this;
    return dimReplacements[dimId];
  }
  case AffineExprKind::SymbolId: {
    unsigned symId = toolchain::cast<AffineSymbolExpr>(*this).getPosition();
  ...
  case AffineExprKind::Mod:
    auto binOp = toolchain::cast<AffineBinaryOpExpr>(*this);
...
bool AffineExpr::isSymbolicOrConstant() const {
  ...
  case AffineExprKind::CeilDiv:
  case AffineExprKind::Mod: {
    auto expr = toolchain::cast<AffineBinaryOpExpr>(*this);
...
bool AffineExpr::isPureAffine() const {
  ...
  case AffineExprKind::Constant:
    return true;
  case AffineExprKind::Add: {
    auto op = toolchain::cast<AffineBinaryOpExpr>(*this);
  ...
  case AffineExprKind::Mul: {
    ...
    auto op = toolchain::cast<AffineBinaryOpExpr>(*this);
    return op.getLHS().isPureAffine() && op.getRHS().isPureAffine() &&
           (toolchain::isa<AffineConstantExpr>(op.getLHS()) ||
            toolchain::isa<AffineConstantExpr>(op.getRHS()));
  }
  case AffineExprKind::FloorDiv:
  case AffineExprKind::CeilDiv:
  case AffineExprKind::Mod: {
...
int64_t AffineExpr::getLargestKnownDivisor() const {
  ...
  case AffineExprKind::FloorDiv: {
    ...
    binExpr = toolchain::cast<AffineBinaryOpExpr>(*this);
    auto rhs = toolchain::dyn_cast<AffineConstantExpr>(binExpr.getRHS());
  ...
  case AffineExprKind::Constant:
    return std::abs(toolchain::cast<AffineConstantExpr>(*this).getValue());
  case AffineExprKind::Mul: {
    binExpr = toolchain::cast<AffineBinaryOpExpr>(*this);
...
bool AffineExpr::isMultipleOf(int64_t factor) const {
  ...
  case AffineExprKind::Constant:
    return toolchain::cast<AffineConstantExpr>(*this).getValue() % factor == 0;
  case AffineExprKind::Mul: {
    binExpr = toolchain::cast<AffineBinaryOpExpr>(*this);
...
bool AffineExpr::isFunctionOfDim(unsigned position) const {
  ...
  if (auto expr = toolchain::dyn_cast<AffineBinaryOpExpr>(*this)) {
...
mlir::getAffineConstantExprs(ArrayRef<int64_t> constants,
                             MLIRContext *context) {
  return toolchain::to_vector(toolchain::map_range(constants, [&](int64_t constant) {
...
static AffineExpr simplifyAdd(AffineExpr lhs, AffineExpr rhs) {
  ...
  if (lhsConst && rhsConst) {
    int64_t sum;
    if (toolchain::AddOverflow(lhsConst.getValue(), rhsConst.getValue(), sum)) {
      return nullptr;
    }
...
static AffineExpr simplifyMul(AffineExpr lhs, AffineExpr rhs) {
  ...
  if (lhsConst && rhsConst) {
    int64_t product;
    if (toolchain::MulOverflow(lhsConst.getValue(), rhsConst.getValue(), product)) {
      return nullptr;
    }
...
static AffineExpr getSemiAffineExprFromFlatForm(ArrayRef<int64_t> flatExprs,
  ...
  auto addEntry = [&](std::pair<unsigned, signed> index, int64_t coefficient,
                      AffineExpr expr) {
    assert(!toolchain::is_contained(indices, index) &&
  ...
  for (const auto &it : toolchain::enumerate(localExprs)) {
...
int SimpleAffineExprFlattener::findLocalId(AffineExpr localExpr) {
  ...
  if ((it = toolchain::find(localExprs, localExpr)) == localExprs.end())
    return -1;
