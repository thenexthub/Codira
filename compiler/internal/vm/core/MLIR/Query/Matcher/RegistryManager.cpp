...
void Registry::registerMatcherDescriptor(
    toolchain::StringRef matcherName,
...
RegistryManager::lookupMatcherCtor(toolchain::StringRef matcherName,
...
RegistryManager::getMatcherCompletions(toolchain::ArrayRef<ArgKind> acceptedTypes,
                                       const Registry &matcherRegistry) {
  ...
  for (const auto &m : matcherRegistry.constructors()) {
    ...
    toolchain::StringRef name = m.getKey();
...
VariantMatcher RegistryManager::constructMatcher(
    ...
    toolchain::StringRef functionName, toolchain::ArrayRef<ParserValue> args,
