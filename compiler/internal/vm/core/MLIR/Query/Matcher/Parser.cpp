...
struct Parser::TokenInfo {
  ...
  void set(TokenKind newKind, toolchain::StringRef newText) {
    kind = newKind;
    text = newText;
  }
...
public:
  ...
  explicit CodeTokenizer(toolchain::StringRef matcherCode, Diagnostics *error)
      : code(matcherCode), startOfLine(matcherCode), error(error) {
    nextToken = getNextToken();
  }

  // Constructor with matcherCode, error, and codeCompletionOffset
  CodeTokenizer(toolchain::StringRef matcherCode, Diagnostics *error,
...
private:
  ...
  toolchain::StringRef firstCharacterAndDrop(toolchain::StringRef &str) {
    assert(!str.empty());
    toolchain::StringRef firstChar = str.substr(0, 1);
    str = str.drop_front();
    return firstChar;
  }
  ...
  TokenInfo getNextToken() {
    ...
    if (codeCompletionLocation && codeCompletionLocation <= code.data()) {
      result.set(TokenKind::CodeCompletion,
                 toolchain::StringRef(codeCompletionLocation, 0));
  ...
  void consumeStringLiteral(TokenInfo *result) {
    ...
    toolchain::StringRef errorText = code;
  ...
  void parseIdentifierOrInvalid(TokenInfo *result) {
    if (isalnum(code[0])) {
      ...
      toolchain::StringRef token = code.substr(0, tokenLength);
      code = code.drop_front(tokenLength);
  ...
  toolchain::StringRef code;
  toolchain::StringRef startOfLine;
  unsigned line = 1;
  Diagnostics *error;
  TokenInfo nextToken;
...
Parser::Sema::getMatcherCompletions(toolchain::ArrayRef<ArgKind> acceptedTypes) {
  return {};
}
...
void Parser::addCompletion(const TokenInfo &compToken,
  ...
  if (toolchain::StringRef(completion.typedText).starts_with(compToken.text)) {
...
std::vector<MatcherCompletion>
Parser::getNamedValueCompletions(toolchain::ArrayRef<ArgKind> acceptedTypes) {
  if (!namedValues)
    return {};
...
Parser::RegistrySema::lookupMatcherCtor(toolchain::StringRef matcherName) {
...
VariantMatcher Parser::RegistrySema::actOnMatcherExpression(
    ...
    toolchain::ArrayRef<ParserValue> args, Diagnostics *error) {
...
std::vector<MatcherCompletion> Parser::RegistrySema::getMatcherCompletions(
    toolchain::ArrayRef<ArgKind> acceptedTypes) {
  return RegistryManager::getMatcherCompletions(acceptedTypes, matcherRegistry);
}

bool Parser::parseExpression(toolchain::StringRef &code,
...
std::vector<MatcherCompletion>
Parser::completeExpression(toolchain::StringRef &code, unsigned completionOffset,
                           const Registry &matcherRegistry,
                           const NamedValueMap *namedValues) {
  Diagnostics error;
...
std::optional<DynMatcher> Parser::parseMatcherExpression(
    toolchain::StringRef &code, const Registry &matcherRegistry,
    const NamedValueMap *namedValues, Diagnostics *error) {
  VariantValue value;
