...
PDLByteCodePattern PDLByteCodePattern::create(pdl_interp::RecordMatchOp matchOp,
  ...
  if (ArrayAttr generatedOpsAttr = matchOp.getGeneratedOpsAttr())
    generatedOps =
        toolchain::to_vector<8>(generatedOpsAttr.getAsValueRange<StringAttr>());
...
public:
  Generator(MLIRContext *ctx, std::vector<const void *> &uniquedData,
            ...
            toolchain::StringMap<PDLRewriteFunction> &rewriteFns,
    ...
    for (const auto &it : toolchain::enumerate(constraintFns))
      constraintToMemIndex.try_emplace(it.value().first(), it.index());
    for (const auto &it : toolchain::enumerate(rewriteFns))
...
struct ByteCodeWriter {
  ...
  /// the bytecode.
  template <typename T>
  std::enable_if_t<toolchain::is_detected<has_pointer_traits, T>::value ||
  ...
  template <typename T, typename IteratorT = toolchain::detail::IterOfRange<T>>
  std::enable_if_t<!toolchain::is_detected<has_pointer_traits, T>::value>
  append(T range) {
    bytecode.push_back(toolchain::size(range));
    for (auto it : range)
      append(it);
  }
...
struct ByteCodeLiveRange {
  using Set = toolchain::IntervalMap<uint64_t, char, 16>;
  ...
  bool overlaps(const ByteCodeLiveRange &rhs) const {
    return toolchain::IntervalMapOverlaps<Set, Set>(*liveness, *rhs.liveness)
        .valid();
  }
  ...
  /// https://reviews.toolchain.org/D113240 lands.
  std::unique_ptr<toolchain::IntervalMap<uint64_t, char, 16>> liveness;
...
void Generator::allocateMemoryIndices(pdl_interp::FuncOp matcherFunc,
  ...
  unsigned index = 0;
  toolchain::unique_function<void(Operation *)> walk = [&](Operation *op) {
  ...
  for (auto &defIt : valueDefRanges) {
    ...
    for (const auto &existingIndexIt : toolchain::enumerate(allocatedIndices)) {
...
void Generator::generate(Region *region, ByteCodeWriter &writer) {
  toolchain::ReversePostOrderTraversal<Region *> rpot(region);
...
void Generator::generate(pdl_interp::CreateOperationOp op,
  ...
  for (auto it : toolchain::zip(op.getInputAttributeNames(), attributes))
...
void Generator::generate(pdl_interp::SwitchOperationNameOp op,
                         ByteCodeWriter &writer) {
  auto cases = toolchain::map_range(op.getCaseValuesAttr(), [&](Attribute attr) {
...
private:
  ...
  template <typename T>
  std::enable_if_t<toolchain::is_detected<has_pointer_traits, T>::value, T>
  readInline() {
    const void *pointer;
  ...
  void handleSwitch(const T &value, RangeT &&cases, Comparator cmp = {}) {
    LDBG() << "Switch operation:\n  * Value: " << value
           << "\n  * Cases: " << toolchain::interleaved(cases);
  ...
  const void *readFromMemory() {
    ...
    if (toolchain::is_one_of<T, Operation *, TypeRange *, ValueRange *,
                        Value>::value ||
        index < memory.size())
      return memory[index];
  ...
  template <typename RangeT, typename T = toolchain::detail::ValueOfRange<RangeT>>
...
void ByteCodeExecutor::executeApplyConstraint(PatternRewriter &rewriter) {
  ...
  LDBG() << "  * Arguments: " << toolchain::interleaved(args);
  ...
  if (succeeded(rewriteResult)) {
    LDBG() << "  * Constraint succeeded, results: "
           << toolchain::interleaved(constraintResults);
  } else {
...
void ByteCodeExecutor::executeCreateOperation(PatternRewriter &rewriter,
  ...
  LDBG() << "  * Attributes: "
         << state.attributes.getDictionary(state.getContext())
         << "\n  * Operands: " << toolchain::interleaved(state.operands)
         << "\n  * Result Types: " << toolchain::interleaved(state.types)
         << "\n  * Result: " << *resultOp;
}
...
void ByteCodeExecutor::executeDynamicCreateRange(StringRef type) {
  ...
  LDBG() << "  * " << type << "s: " << toolchain::interleaved(values);
...
executeGetOperandsResults(RangeT values, Operation *op, unsigned index,
  ...
  if (index == std::numeric_limits<uint32_t>::max()) {
    ...
    unsigned startIndex = toolchain::sum_of(segments.take_front(index));
...
void ByteCodeExecutor::executeGetValueRangeTypes() {
  ...
  LDBG() << "  * Values (" << values->size()
         << "): " << toolchain::interleaved(*values)
         << "\n  * Result: " << toolchain::interleaved(values->getType());
...
void ByteCodeExecutor::executeReplaceOp(PatternRewriter &rewriter) {
  ...
  LDBG() << "  * Operation: " << *op
         << "\n  * Values: " << toolchain::interleaved(args);
  ...
  LLVM_DEBUG({
    ...
    LDBG() << "  * Value: " << value << "\n  * Cases: "
           << toolchain::interleaved(
                  toolchain::map_range(toolchain::seq<size_t>(0, caseCount), [&](size_t) {
...
LogicalResult PDLByteCode::rewrite(PatternRewriter &rewriter,
  ...
  // memory buffer.
  toolchain::copy(match.values, state.memory.begin());
