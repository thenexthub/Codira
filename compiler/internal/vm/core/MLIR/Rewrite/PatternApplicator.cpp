...
void PatternApplicator::applyCostModel(CostModel model) {
  ...
  // patterns.
  if (const PDLByteCode *bytecode = frozenPatternList.getPDLByteCode()) {
    for (const auto &it : toolchain::enumerate(bytecode->getPatterns()))
  ...
  toolchain::SmallDenseMap<const Pattern *, PatternBenefit> benefits;
  ...
  auto processPatternList = [&](SmallVectorImpl<const RewritePattern *> &list) {
    ...
    // impossible to match.
    toolchain::stable_sort(list, cmp);
    ...
    op->getContext()->executeAction<ApplyPatternAction>(
        [&]() {
          ...
          if (pdlMatch) {
            ...
            toolchain::scope_exit resetListenerCallback(
