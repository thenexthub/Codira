...
using BlockingUsesMap =
    toolchain::MapVector<Operation *, SmallPtrSet<OpOperand *, 4>>;
...
LogicalResult MemorySlotPromotionAnalyzer::computeBlockingUses(
  ...
  for (Operation *user : forwardSlice) {
    ...
    for (OpOperand *blockingUse : newBlockingUses) {
      assert(toolchain::is_contained(user->getResults(), blockingUse->get()));
...
using IDFCalculator = toolchain::IDFCalculatorBase<Block, false>;
...
void MemorySlotPromoter::computeReachingDefInRegion(Region *region,
  ...
  struct DfsJob {
    toolchain::DomTreeNodeBase<Block> *block;
    Value reachingDef;
  };
...
getOrCreateBlockIndices(BlockIndexCache &blockIndexCache, Region *region) {
  ...
  for (auto [index, block] : toolchain::enumerate(topologicalOrder))
...
static void dominanceSort(SmallVector<Operation *> &ops, Region &region,
  ...
  // order.
  toolchain::sort(ops, [&](Operation *lhs, Operation *rhs) {
...
void MemorySlotPromoter::removeBlockingUses() {
  toolchain::SmallVector<Operation *> usersToRemoveUses(
      toolchain::make_first_range(info.userToBlockingUses));

  // Sort according to dominance.
  dominanceSort(usersToRemoveUses, *slot.ptr.getParentBlock()->getParent(),
                blockIndexCache);

  toolchain::SmallVector<Operation *> toErase;
  // List of all replaced values in the slot.
  toolchain::SmallVector<std::pair<Operation *, Value>> replacedValuesList;
  // Ops to visit with the `visitReplacedValues` method.
  toolchain::SmallVector<PromotableOpInterface> toVisit;
  for (Operation *toPromote : toolchain::reverse(usersToRemoveUses)) {
