...
class CallGraphSCC {
public:
  CallGraphSCC(toolchain::scc_iterator<const CallGraph *> &parentIterator)
  ...
  void remove(CallGraphNode *node) {
    auto it = toolchain::find(nodes, node);
    if (it != nodes.end()) {
      nodes.erase(it);
...
private:
  ...
  toolchain::scc_iterator<const CallGraph *> &parentIterator;
};
} // namespace
...
static LogicalResult runTransformOnCGSCCs(
  ...
  toolchain::scc_iterator<const CallGraph *> cgi = toolchain::scc_begin(&cg);
...
static std::string getNodeName(CallOpInterface op) {
  if (toolchain::dyn_cast_if_present<SymbolRefAttr>(op.getCallableForCallee()))
...
private:
  ...
  LogicalResult optimizeCallable(CallGraphNode *node,
                                 toolchain::StringMap<OpPassManager> &pipelines);
...
private:
  Inliner &inliner;
  toolchain::SmallVector<toolchain::StringMap<OpPassManager>> pipelines;
};
...
Inliner::Impl::inlineCallsInSCC(InlinerInterfaceImpl &inlinerIface,
  ...
  toolchain::SmallSetVector<CallGraphNode *, 1> deadNodes;
...
bool Inliner::Impl::shouldInline(ResolvedCall &resolvedCall) {
  ...
  if (toolchain::count_if(*resolvedCall.targetNode,
  ...
  if (!inliner.config.getCanHandleMultipleBlocks()) {
    bool calleeHasMultipleBlocks =
        toolchain::hasNItemsOrMore(*callableRegion, /*N=*/2);
