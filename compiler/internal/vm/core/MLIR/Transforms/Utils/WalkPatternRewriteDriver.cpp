...
void walkAndApplyPatterns(Operation *op,
...
#if MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
  if (failed(verify(op)))
    toolchain::report_fatal_error("walk pattern rewriter input IR failed to verify");
#endif // MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
  ...
  struct RegionReachableOpIterator {
    RegionReachableOpIterator(Region *region) : region(region) {
      ...
      if (!toolchain::hasSingleElement(*region))
        ...
        for (Region &region : op->getRegions()) {
          ...
          while (!worklist.empty()) {
            ...
            if (!it.hasVisitedRegions) {
              ...
              for (Region &nestedRegion : toolchain::reverse(op->getRegions())) {
