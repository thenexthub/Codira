...
SmallVector<Value> mlir::makeRegionIsolatedFromAbove(
    ...
    toolchain::function_ref<bool(Operation *)> cloneOperationIntoRegion) {

  // Get initial list of values used within region but defined above.
  toolchain::SetVector<Value> initialCapturedValues;
  ...
  toolchain::DenseSet<Value> visited;
  toolchain::DenseSet<Operation *> visitedOps;

  toolchain::SetVector<Value> finalCapturedValues;
  ...
  SmallVector<Type> newArgTypes =
      toolchain::to_vector(entryBlock->getArgumentTypes());
  SmallVector<Location> newArgLocs = toolchain::to_vector(toolchain::map_range(
  ...
  for (auto [arg, capturedVal] :
       toolchain::zip(newEntryBlockArgs.take_back(finalCapturedValues.size()),
                 finalCapturedValues)) {
  ...
  return toolchain::to_vector(finalCapturedValues);
}
...
LogicalResult mlir::eraseUnreachableBlocks(RewriterBase &rewriter,
  ...
  toolchain::df_iterator_default_set<Block *, 16> reachable;
...
static void processValue(Value value, LiveMap &liveMap) {
  bool provedLive = toolchain::any_of(value.getUses(), [&](OpOperand &use) {
...
static void propagateLiveness(Region &region, LiveMap &liveMap) {
  ...
  for (Block *block : toolchain::post_order(&region.front())) {
    ...
    for (Operation &op : toolchain::reverse(block->getOperations()))
  ...
  for (Region &region : regions) {
    ...
    for (Block *block : toolchain::post_order(&region.front())) {
      ...
      for (Operation &childOp :
           toolchain::make_early_inc_range(toolchain::reverse(block->getOperations()))) {
    ...
    // skip it.
    for (Block &block : toolchain::drop_begin(region.getBlocks(), 1)) {
...
BlockEquivalenceData::BlockEquivalenceData(Block *block)
  ...
  for (Operation &op : *block) {
    ...
    hash = toolchain::hash_combine(hash, opHash);
  }
}
...
private:
  ...
  toolchain::SmallSetVector<Block *, 1> blocksToMerge;
...
LogicalResult BlockMergeCluster::addToCluster(BlockEquivalenceData &blockData) {
  ...
  for (int opI = 0; lhsIt != lhsE && rhsIt != rhsE; ++lhsIt, ++rhsIt, ++opI) {
    ...
    for (int operand : toolchain::seq<int>(0, lhsIt->getNumOperands())) {
      ...
      if (!lhsIsInBlock) {
        ...
        auto isValidSuccessorArg = [](Block *block, Value operand) {
          if (operand.getDefiningOp() !=
            ...
            return true;
          return !toolchain::is_contained(block->getPredecessors(),
...
static SmallVector<SmallVector<Value, 8>, 2> pruneRedundantArguments(
  ...
  toolchain::DenseMap<unsigned, unsigned> idxToReplacement;
  ...
  for (auto [idx, arg] : toolchain::enumerate(block->getArguments())) {
  ...
  for (unsigned idxToErase : toolchain::reverse(toErase))
...
LogicalResult BlockMergeCluster::merge(RewriterBase &rewriter) {
  ...
  if (!operandsToMerge.empty()) {
    ...
    if (!ableToUpdatePredOperands(leaderBlock) ||
        !toolchain::all_of(blocksToMerge, ableToUpdatePredOperands))
      return failure();
    ...
    for (const auto &it : toolchain::enumerate(operandsToMerge)) {
...
static LogicalResult mergeIdenticalBlocks(RewriterBase &rewriter,
  ...
  for (Block &block : toolchain::drop_begin(region, 1))
    matchingSuccessors[block.getSuccessors()].push_back(&block);

  bool mergedAnyBlocks = false;
  for (ArrayRef<Block *> blocks : toolchain::make_second_range(matchingSuccessors)) {
    if (blocks.size() == 1)
      continue;
    ...
    for (Block *block : blocks) {
      ...
      bool hasNonEmptyRegion = toolchain::any_of(*block, [](Operation &op) {
        return toolchain::any_of(op.getRegions(),
      ...
      // original block.
      bool argHasExternalUsers = toolchain::any_of(
...
static LogicalResult mergeIdenticalBlocks(RewriterBase &rewriter,
                                          MutableArrayRef<Region> regions) {
  toolchain::SmallSetVector<Region *, 1> worklist;
...
static LogicalResult dropRedundantArguments(RewriterBase &rewriter,
  ...
  for (auto [argIdx, blockOperand] : toolchain::enumerate(block.getArguments())) {
    bool sameArg = true;
    Value commonValue;
  ...
  for (size_t argIdx : toolchain::reverse(argsToErase)) {
...
///
/// %cond = toolchain.call @rand() : () -> i1
/// %val0 = toolchain.mlir.constant(1 : i64) : i64
/// %val1 = toolchain.mlir.constant(2 : i64) : i64
/// %val2 = toolchain.mlir.constant(3 : i64) : i64
/// toolchain.cond_br %cond, ^bb1(%val0 : i64, %val1 : i64), ^bb2(%val0 : i64, %val2
/// : i64)
///
/// ^bb1(%arg0 : i64, %arg1 : i64):
///    toolchain.call @foo(%arg0, %arg1)
///
...
/// %val1 = toolchain.mlir.constant(2 : i64) : i64
/// %val2 = toolchain.mlir.constant(3 : i64) : i64
/// toolchain.cond_br %cond, ^bb1(%val1 : i64), ^bb2(%val2 : i64)
///
/// ^bb1(%arg0 : i64):
///    toolchain.call @foo(%val0, %arg0)
///
static LogicalResult dropRedundantArguments(RewriterBase &rewriter,
                                            MutableArrayRef<Region> regions) {
  toolchain::SmallSetVector<Region *, 1> worklist;
