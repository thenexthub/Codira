...
struct ExpensiveChecks : public RewriterBase::ForwardingListener {
  ...
  void notifyRewriteSuccess() {
    ...
    if (failed(verify(topLevel)))
      toolchain::report_fatal_error("IR failed to verify after pattern application");
    ...
    for (const auto &it : fingerprints) {
      ...
      if (it.second !=
        ...
        // Note: Run "mlir-opt -debug" to see which pattern is broken.
        toolchain::report_fatal_error("operation finger print changed");
      }
    }
  }
  ...
  void notifyRewriteFailure() {
    ...
    if (*topLevelFingerPrint != afterFingerPrint) {
      // Note: Run "mlir-opt -debug" to see which pattern is broken.
      toolchain::report_fatal_error("pattern returned failure but IR did change");
    }
  }
  ...
  void notifyFoldingSuccess() {
    ...
    // Make sure that the IR still verifies.
    if (failed(verify(topLevel)))
      toolchain::report_fatal_error("IR failed to verify after folding");
  }
...
protected:
  ...
  toolchain::SmallDenseSet<Operation *, 4> strictModeFilteredOps;
...
bool GreedyPatternRewriteDriver::processWorklist() {
  ...
  auto logResult = [&](StringRef result, const toolchain::Twine &msg = {}) {
    logger.unindent();
  ...
  auto logResultWithLine = [&](StringRef result, const toolchain::Twine &msg = {}) {
  ...
  while (!worklist.empty() &&
    ...
    if (config.isFoldingEnabled() && !op->hasTrait<OpTrait::ConstantLike>()) {
      ...
      if (succeeded(op->fold(foldResults))) {
        ...
        for (auto [ofr, resultType] :
             toolchain::zip_equal(foldResults, op->getResultTypes())) {
          ...
          if (!constOp) {
            ...
            toolchain::SmallDenseSet<Operation *> replacementOps;
    ...
    auto canApplyCallback = [&](const Pattern &pattern) {
      LLVM_DEBUG({
        ...
        logger.startLine() << "* Pattern " << pattern.getDebugName() << " : '"
                           << op->getName() << " -> (";
        toolchain::interleaveComma(pattern.getGeneratedOps(), logger.getOStream());
        logger.getOStream() << ")' {\n";
        logger.indent();
      });
    ...
    toolchain::scope_exit clearFingerprints([&]() { expensiveChecks.clear(); });
#endif // MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
...
public:
  explicit MultiOpPatternRewriteDriver(
      ...
      toolchain::SmallDenseSet<Operation *, 4> *survivingOps = nullptr);
...
private:
  ...
  /// of ops.
  toolchain::SmallDenseSet<Operation *, 4> *const survivingOps = nullptr;
};
} // namespace
...
MultiOpPatternRewriteDriver::MultiOpPatternRewriteDriver(
    MLIRContext *ctx, const FrozenRewritePatternSet &patterns,
    const GreedyRewriteConfig &config, ArrayRef<Operation *> ops,
    toolchain::SmallDenseSet<Operation *, 4> *survivingOps)
...
static Region *findCommonAncestor(ArrayRef<Operation *> ops) {
  ...
  ops = ops.drop_front();
  int sz = ops.size();
  toolchain::BitVector remainingOps(sz, true);
  while (region) {
    int pos = -1;
...
LogicalResult mlir::applyOpPatternsGreedily(
...
#ifndef NDEBUG
    bool allOpsInScope = toolchain::all_of(ops, [&](Operation *op) {
  ...
  // Start the pattern driver.
  toolchain::SmallDenseSet<Operation *, 4> surviving;
