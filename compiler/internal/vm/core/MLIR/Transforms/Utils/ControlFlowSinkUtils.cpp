...
bool Sinker::allUsersDominatedBy(Operation *op, Region *region) {
  ...
  return toolchain::all_of(op->getUsers(), [&](Operation *user) {
...
void mlir::getSinglyExecutedRegionsToSink(RegionBranchOpInterface branch,
  ...
  for (auto [idx, operand] : toolchain::enumerate(branch->getOperands()))
  ...
  // most once.
  for (auto it : toolchain::zip(branch->getRegions(), bounds)) {
