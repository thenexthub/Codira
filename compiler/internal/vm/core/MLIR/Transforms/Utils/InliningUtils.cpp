  ...
  for (auto &block : *src) {
    for (auto &op : block) {
      ...
      if (interface.shouldAnalyzeRecursively(&op) &&
          toolchain::any_of(op.getRegions(), [&](Region &region) {
...
static void handleArgumentImpl(InlinerInterface &interface, OpBuilder &builder,
  ...
  if (ArrayAttr arrayAttr = callable.getArgAttrsAttr()) {
    ...
    for (auto [idx, attr] : toolchain::enumerate(arrayAttr))
  ...
  for (auto [blockArg, argAttr] :
       toolchain::zip(callable.getCallableRegion()->getArguments(), argAttrs)) {
...
static void handleResultImpl(InlinerInterface &interface, OpBuilder &builder,
  ...
  for (auto [result, resAttr] : toolchain::zip(results, resAttrs)) {
    // Store the original result users before running the handler.
    DenseSet<Operation *> resultUsers(toolchain::from_range, result.getUsers());
...
static LogicalResult inlineRegionImpl(
  ...
  if (toolchain::any_of(srcEntryBlock->getArguments(),
  ...
  auto newBlocks = toolchain::make_range(std::next(inlineBlock->getIterator()),
  ...
  // was provided.
  if (inlineLoc && !toolchain::isa<UnknownLoc>(*inlineLoc))
  ...
  if (singleBlockFastPath && toolchain::hasSingleElement(newBlocks)) {
    ...
    for (const auto &resultToRepl : toolchain::enumerate(resultsToReplace)) {
