...
static void logSuccess(toolchain::ScopedPrinter &os, StringRef fmt, Args &&...args) {
  LLVM_DEBUG({
    os.unindent();
    os.startLine() << "} -> SUCCESS";
    if (!fmt.empty())
      os.getOStream() << " : "
                      << toolchain::formatv(fmt.data(), std::forward<Args>(args)...);
    os.getOStream() << "\n";
  });
}
...
template <typename... Args>
static void logFailure(toolchain::ScopedPrinter &os, StringRef fmt, Args &&...args) {
  LLVM_DEBUG({
    os.unindent();
...
struct ValueVectorMapInfo {
  ...
  static ::toolchain::hash_code getHashValue(const ValueVector &val) {
    return ::toolchain::hash_combine_range(val);
  }
...
static Operation *getCommonDefiningOp(const ValueVector &values) {
  ...
  for (Value v : toolchain::drop_begin(values)) {
...
public:
  ...
  void cleanup(RewriterBase &rewriter) override {
    ...
    for (auto &op : toolchain::make_early_inc_range(toolchain::reverse(*block)))
      rewriter.eraseOp(&op);
...
public:
  ...
  void rollback() override {
    ...
    for (const auto &it : toolchain::enumerate(successors))
...
private:
  ...
  toolchain::PointerIntPair<const TypeConverter *, 2, MaterializationKind>
      converterAndKind;
...
struct ConversionPatternRewriterImpl : public RewriterBase::Listener {
  ...
  toolchain::impl::raw_ldbg_ostream os{(Twine("[") + DEBUG_TYPE + ":1] ").str(),
                                  toolchain::dbgs()};

  /// A logger used to emit diagnostics during the conversion process.
  toolchain::ScopedPrinter logger{os};
...
void ReplaceOperationRewrite::commit(RewriterBase &rewriter) {
  ...
  SmallVector<Value> replacements =
      toolchain::map_to_vector(op->getResults(), [&](OpResult result) {
  ...
  for (auto [result, newValue] :
       toolchain::zip_equal(op->getResults(), replacements))
    if (newValue)
...
void ConversionPatternRewriterImpl::undoRewrites(unsigned numRewritesToKeep,
                                                 StringRef patternName) {
  for (auto &rewrite :
       toolchain::reverse(toolchain::drop_begin(rewrites, numRewritesToKeep)))
    rewrite->rollback();
...
LogicalResult ConversionPatternRewriterImpl::remapValues(
    ...
    SmallVector<ValueVector> &remapped) {
  remapped.reserve(toolchain::size(values));

  for (const auto &it : toolchain::enumerate(values)) {
...
FailureOr<Block *> ConversionPatternRewriterImpl::convertRegionTypes(
  ...
  for (Block &block :
       toolchain::make_early_inc_range(toolchain::drop_begin(*region, 1))) {
...
Block *ConversionPatternRewriterImpl::applySignatureConversion(
  ...
  if (hasRewrite<BlockTypeConversionRewrite>(rewrites, block))
    toolchain::report_fatal_error("block was already converted");
#endif // MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
  ...
  if (toolchain::equal(block->getArgumentTypes(), convertedTypes))
    return block;
...
Value ConversionPatternRewriterImpl::findOrBuildReplacementValue(
  ...
  if (toolchain::all_of(value.getUsers(),
...
getReplacementValues(ConversionPatternRewriterImpl &impl, ValueRange fromRange,
  ...
  for (auto [from, to] : toolchain::zip_equal(fromRange, toRange)) {
    if (from.use_empty()) {
...
void ConversionPatternRewriterImpl::replaceOp(
    ...
    if (currentTypeConverter) {
      ...
      for (auto [result, repls] :
        ...
        if (TypeRange(convertedTypes) != TypeRange(ValueRange(repls))) {
          ...
          toolchain::interleaveComma(convertedTypes, logger.getOStream(),
  ...
  for (auto [repl, result] : toolchain::zip_equal(newValues, op->getResults()))
...
void ConversionPatternRewriterImpl::replaceValueUses(
  ...
  if (!config.allowPatternRollback) {
    SmallVector<Value> toConv = toolchain::to_vector(to);
    SmallVector<Value> repls =
...
void ConversionPatternRewriter::replaceOp(Operation *op, ValueRange newValues) {
  ...
  SmallVector<SmallVector<Value>> newVals =
      toolchain::map_to_vector(newValues, [](Value v) -> SmallVector<Value> {
...
void ConversionPatternRewriter::inlineBlockBefore(Block *source, Block *dest,
  ...
  assert(toolchain::all_of(source->getUsers(), opIgnored) &&
  ...
  for (auto it : toolchain::zip(source->getArguments(), argValues))
...
void ConversionPatternRewriter::cancelOpModification(Operation *op) {
  ...
  auto it = toolchain::find_if(
      toolchain::reverse(impl->rewrites), [&](std::unique_ptr<IRRewrite> &rewrite) {
...
ConversionPattern::matchAndRewrite(Operation *op,
  ...
  toolchain::SaveAndRestore currentConverterGuard(rewriterImpl.currentTypeConverter,
                                             getTypeConverter());
  ...
  SmallVector<ValueRange> remappedAsRange =
      toolchain::to_vector_of<ValueRange>(remapped);
...
reportNewIrLegalizationFatalError(const Pattern &pattern,
                                  const SetVector<Operation *> &newOps,
                                  const SetVector<Operation *> &modifiedOps) {
  auto newOpNames = toolchain::map_range(
      newOps, [](Operation *op) { return op->getName().getStringRef(); });
  auto modifiedOpNames = toolchain::map_range(
      modifiedOps, [](Operation *op) { return op->getName().getStringRef(); });
  toolchain::report_fatal_error("pattern '" + pattern.getDebugName() +
                           "' produced IR that could not be legalized. " +
                           "new ops: {" + toolchain::join(newOpNames, ", ") + "}, " +
                           "modified ops: {" +
                           toolchain::join(modifiedOpNames, ", ") + "}");
}
...
bool OperationLegalizer::canApplyPattern(Operation *op,
  ...
  LLVM_DEBUG({
    ...
    toolchain::interleaveComma(pattern.getGeneratedOps(), os.getOStream());
    os.getOStream() << ")' {\n";
    os.indent();
  });
...
LogicalResult OperationLegalizer::legalizePatternResult(
  ...
  if (impl.config.allowPatternRollback) {
    ...
    auto newRewrites = toolchain::drop_begin(impl.rewrites, curState.numRewrites);
    auto replacedRoot = [&] {
    ...
    if (!replacedRoot() && !updatedRootInPlace())
      toolchain::report_fatal_error("expected pattern to replace the root operation "
...
void OperationLegalizer::buildLegalizationGraph(
  ...
  while (!patternWorklist.empty()) {
    ...
    if (toolchain::any_of(pattern->getGeneratedOps(), [&](OperationName op) {
...
void OperationLegalizer::computeLegalizationGraphBenefit(
  ...
  applicator.applyCostModel([&](const Pattern &pattern) {
    ...
    auto *it = toolchain::find(orderedPatternList, &pattern);
...
unsigned OperationLegalizer::applyCostModelToPatterns(
  ...
  toolchain::stable_sort(patternsByDepth,
...
static void reconcileUnrealizedCastsImpl(
  ...
  SetVector<UnrealizedConversionCastOp> worklist(toolchain::from_range, castOps);
  ...
  while (!worklist.empty()) {
    ...
    while (nextCast) {
      if (nextCast.getInputs().getTypes() == castOp.getResultTypes()) {
        if (toolchain::any_of(nextCast.getInputs(), [&](Value v) {
  ...
  for (UnrealizedConversionCastOp op : castOps) {
    ...
    // input ops) as live.
    if (toolchain::any_of(op->getUsers(), [&](Operation *user) {
...
void mlir::reconcileUnrealizedCasts(
  ...
  reconcileUnrealizedCastsImpl(
      toolchain::make_range(castOps.begin(), castOps.end()),
...
TypeConverter::convertTypeImpl(PointerUnion<Type, Value> typeOrValue,
  ...
  for (auto indexedConverter : toolchain::enumerate(toolchain::reverse(conversions))) {
    ...
    if (newTypes.size() == 1)
      ...
      cachedMultiConversions.try_emplace(t, toolchain::to_vector<2>(newTypes));
    return success();
  }
  return failure();
}
...
Value TypeConverter::materializeSourceConversion(OpBuilder &builder,
  ...
  for (const SourceMaterializationCallbackFn &fn :
       toolchain::reverse(sourceMaterializations))
...
SmallVector<Value> TypeConverter::materializeTargetConversion(
  ...
  for (const TargetMaterializationCallbackFn &fn :
       toolchain::reverse(targetMaterializations)) {
...
TypeConverter::convertTypeAttribute(Type type, Attribute attr) const {
  for (const TypeAttributeConversionCallbackFn &fn :
       toolchain::reverse(typeAttributeConversions)) {
...
mlir::applyPartialConversion(Operation *op, const ConversionTarget &target,
                             const FrozenRewritePatternSet &patterns,
                             ConversionConfig config) {
  return applyPartialConversion(toolchain::ArrayRef(op), target, patterns, config);
}
...
LogicalResult mlir::applyFullConversion(Operation *op,
                                        ...
                                        const FrozenRewritePatternSet &patterns,
                                        ConversionConfig config) {
  return applyFullConversion(toolchain::ArrayRef(op), target, patterns, config);
}
...
LogicalResult mlir::applyAnalysisConversion(
  ...
  SmallVector<Operation *> opsToConvert = toolchain::map_to_vector(
...
mlir::applyAnalysisConversion(Operation *op, ConversionTarget &target,
                              const FrozenRewritePatternSet &patterns,
                              ConversionConfig config) {
  return applyAnalysisConversion(toolchain::ArrayRef(op), target, patterns, config);
}
