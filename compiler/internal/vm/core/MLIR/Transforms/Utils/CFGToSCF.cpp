...
/// https://toolchain.org/docs/CycleTerminology.html.
struct CycleEdges {
...
public:
  ...
  static EdgeMultiplexer create(Location loc, ArrayRef<Block *> entryBlocks,
    ...
    toolchain::SmallMapVector<Block *, unsigned, 4> blockArgMapping;
  ...
  void createSwitch(
    ...
    for (auto &&[index, pair] : toolchain::enumerate(blockArgMapping)) {
...
private:
  ...
  EdgeMultiplexer(Block *multiplexerBlock,
                  function_ref<Value(unsigned)> getSwitchValue,
                  function_ref<Value(Type)> getUndefValue,
                  toolchain::SmallMapVector<Block *, unsigned, 4> &&entries,
                  Value dispatchFlag)
...
/// to be equivalent.
struct ReturnLikeOpEquivalence : public toolchain::DenseMapInfo<Operation *> {
...
private:
  ...
  toolchain::SmallDenseMap<Operation *, Block *, 4, ReturnLikeOpEquivalence>
...
static auto successorEdges(Block *block) {
  return toolchain::map_range(toolchain::seq(block->getNumSuccessors()),
                         [=](unsigned index) { return Edge(block, index); });
}

/// Calculates entry, exit and back edges of the given cycle.
static CycleEdges
calculateCycleEdges(const toolchain::SmallSetVector<Block *, 4> &cycles) {
  CycleEdges result;
  ...
  for (Block *block : cycles) {
    ...
    for (auto &&[succIndex, succ] : toolchain::enumerate(block->getSuccessors())) {
...
createSingleEntryBlock(Location loc, ArrayRef<Edge> entryEdges,
  ...
  auto result = EdgeMultiplexer::create(
      loc, toolchain::map_to_vector(entryEdges, std::mem_fn(&Edge::getSuccessor)),
...
static FailureOr<StructuredLoopProperties> createSingleExitingLatch(
  ...
  assert(toolchain::all_equal(
             toolchain::map_range(backEdges, std::mem_fn(&Edge::getSuccessor))) &&
  ...
  toolchain::append_range(
      successors, toolchain::map_range(backEdges, std::mem_fn(&Edge::getSuccessor)));
  toolchain::append_range(
      successors, toolchain::map_range(exitEdges, std::mem_fn(&Edge::getSuccessor)));
  auto multiplexer =
...
transformToReduceLoop(Block *loopHeader, Block *exitBlock,
                      const toolchain::SmallSetVector<Block *, 4> &loopBlocks,
                      function_ref<Value(Type)> getUndefValue,
  ...
  SmallVector<Value> loopHeaderSuccessorOperands =
      toolchain::to_vector(getSuccessorOperands(latch, loopHeaderIndex));
  ...
  SmallVector<BlockArgument> latchBlockArgumentsPrior =
      toolchain::to_vector(latch->getArguments());
  SmallVector<BlockArgument> loopHeaderArgumentsPrior =
      toolchain::to_vector(loopHeader->getArguments());
  ...
  for (Block *loopBlock : loopBlocks) {
    ...
    toolchain::SmallDenseMap<Block *, bool> dominanceCache;
    ...
    auto checkValue = [&](Value value) {
      Value blockArgument;
      for (OpOperand &use : toolchain::make_early_inc_range(value.getUses())) {
        ...
        if (!blockArgument) {
          ...
          if (value.getParentBlock() != latch &&
              toolchain::any_of(latch->getPredecessors(), [&](Block *pred) {
    ...
    if (loopBlock == latch)
      toolchain::for_each(latchBlockArgumentsPrior, checkValue);
    else if (loopBlock == loopHeader)
      toolchain::for_each(loopHeaderArgumentsPrior, checkValue);
    else
      toolchain::for_each(loopBlock->getArguments(), checkValue);

    for (Operation &op : *loopBlock)
      toolchain::for_each(op.getResults(), checkValue);
  }
  ...
  for (auto iter = loopHeader->pred_begin(); iter != loopHeader->pred_end();
    ...
    MutableOperandRange succOps =
        getMutableSuccessorOperands(*iter, iter.getSuccessorIndex());
    succOps.append(toolchain::map_to_vector(
...
static FailureOr<SmallVector<Block *>> transformCyclesToSCFLoops(
  ...
  auto scc = toolchain::scc_begin(regionEntry);
  while (!scc.isAtEnd()) {
    if (!scc.hasCycle()) {
      ++scc;
      continue;
    ...
    toolchain::SmallSetVector<Block *, 4> cycleBlockSet(scc->begin(), scc->end());
    ++scc;
    ...
    if (edges.entryEdges.size() > 1) {
      ...
      toolchain::append_range(edgesToEntryBlocks, edges.entryEdges);
      toolchain::append_range(edgesToEntryBlocks, edges.backEdges);
    ...
    for (auto &&[oldValue, newValue] : toolchain::zip(
...
static FailureOr<SmallVector<Block *>> transformToStructuredCFBranches(
  ...
  for (auto &&[blockList, succ] :
       toolchain::zip(successorBranchRegions, regionEntry->getSuccessors())) {
    ...
    for (DominanceInfoNode *curr : toolchain::depth_first(node)) {
  ...
  for (auto &&[entryEdge, branchRegion] :
       toolchain::zip(successorEdges(regionEntry), successorBranchRegions)) {
  ...
  if (noSuccessorHasContinuationEdge)
    return toolchain::to_vector(regionEntry->getSuccessors());

  Block *continuation = toolchain::find_singleton<Block>(
  ...
  for (auto &&[branchRegion, entryEdge, conditionalRegion] :
       toolchain::zip(successorBranchRegions, successorEdges(regionEntry),
                 conditionalRegions)) {
    if (branchRegion.empty()) {
      ...
      createdEmptyBlocks.emplace_back(
          new Block, toolchain::to_vector(entryEdge.getSuccessorOperands()));
    ...
    for (auto &&[oldValue, newValue] :
         toolchain::zip(subRegionEntryBlock->getArguments(),
  ...
  for (Operation *user : toolchain::make_early_inc_range(continuation->getUsers())) {
  ...
  for (auto &&[oldValue, newValue] :
       toolchain::zip(continuation->getArguments(), structuredCondOp->getResults()))
    oldValue.replaceAllUsesWith(newValue);
...
static LogicalResult checkTransformationPreconditions(Region &region) {
  ...
  WalkResult result = region.walk([](Operation *operation) {
    ...
    for (unsigned index : toolchain::seq(operation->getNumSuccessors())) {
...
FailureOr<bool> mlir::transformCFGToSCF(Region &region,
  ...
  while (!workList.empty()) {
    ...
    // bodies of the loops.
    toolchain::append_range(workList, *newRegions);
