...
private:
  ...
  void initColorMapping(T &irEntity) {
    ...
    for (auto indexedOps : toolchain::enumerate(ops)) {
  ...
  Node emitNodeStmt(const std::string &label, StringRef shape = kShapeNode,
    ...
    os << toolchain::format("v%i ", nodeId);
    emitAttrList(os, attrs);
    os << ";\n";
    return Node(nodeId);
  }
...
static void llvmViewGraph(Region &region, const Twine &name) {
  int fd;
  std::string filename = toolchain::createGraphFilename(name.str(), fd);
  {
    toolchain::raw_fd_ostream os(fd, /*shouldClose=*/true);
    if (fd == -1) {
      toolchain::errs() << "error opening file '" << filename << "' for writing\n";
      return;
    }
    PrintOpPass pass(os);
    pass.emitRegionCFG(region);
  }
  toolchain::DisplayGraph(filename, /*wait=*/false, toolchain::GraphProgram::DOT);
}
