...
#include "vm/core/Support/RecyclingAllocator.h"
#include <deque>
...
namespace {
struct SimpleOperationInfo : public toolchain::DenseMapInfo<Operation *> {
...
private:
  ...
  using AllocatorTy = toolchain::RecyclingAllocator<
      toolchain::BumpPtrAllocator,
      toolchain::ScopedHashTableVal<Operation *, Operation *>>;
  using ScopedMapTy = toolchain::ScopedHashTable<Operation *, Operation *,
...
LogicalResult CSEDriver::simplifyOperation(ScopedMapTy &knownValues,
  ...
  if (!toolchain::all_of(op->getRegions(),
...
void CSEDriver::simplifyRegion(ScopedMapTy &knownValues, Region &region) {
  ...
  // the toolchain mailing list discussion on this:
  // http://lists.toolchain.org/pipermail/toolchain-commits/Week-of-Mon-20120116/135228.html
