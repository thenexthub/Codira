...
static void collectNonLiveValues(DenseSet<Value> &nonLiveSet, ValueRange range,
  ...
  for (auto [index, result] : toolchain::enumerate(range)) {
    if (!nonLive[index])
      continue;
...
static void processFuncOp(FunctionOpInterface funcOp, Operation *module,
  ...
  // Do (1).
  for (auto [index, arg] : toolchain::enumerate(arguments))
...
static SmallVector<Value> createPoisonedValues(OpBuilder &b,
                                               ValueRange values) {
  return toolchain::map_to_vector(values, [&](Value value) {
    if (value.use_empty())
      return Value();
...
static void cleanUpDeadVals(MLIRContext *ctx, RDVFinalCleanupList &list) {
  ...
  for (auto &b : list.blocks) {
    ...
    LDBG_OS([&](raw_ostream &os) {
      ...
      toolchain::interleaveComma(b.nonLiveArgs.set_bits(), os);
  ...
  for (auto &op : list.successorOperands) {
    ...
    LDBG_OS([&](raw_ostream &os) {
      ...
      toolchain::interleaveComma(op.nonLiveOperands.set_bits(), os);
  ...
  for (auto &f : list.functions) {
    ...
    LDBG_OS([&](raw_ostream &os) {
      os << "  Erasing non-live arguments [";
      toolchain::interleaveComma(f.nonLiveArgs.set_bits(), os);
      os << "]\n";
      os << "  Erasing non-live return values [";
      toolchain::interleaveComma(f.nonLiveRets.set_bits(), os);
      os << "]";
    });
  ...
  for (OperandsToCleanup &o : list.operands) {
    ...
    if (o.callee && isa<CallOpInterface>(o.op)) {
      ...
      if (it != erasedFuncArgs.end()) {
        ...
        for (unsigned argIdx : toolchain::reverse(deadArgIdxs.set_bits()))
          args.erase(argIdx);
    ...
    if (!handledAsCall && o.nonLive.any()) {
      LDBG_OS([&](raw_ostream &os) {
        ...
        toolchain::interleaveComma(o.nonLive.set_bits(), os);
        os << "] from operation: "
           << OpWithFlags(o.op,
                          OpPrintingFlags().skipRegions().printGenericOpForm());
      });
  ...
  for (auto &r : list.results) {
    LDBG_OS([&](raw_ostream &os) {
      ...
      toolchain::interleaveComma(r.nonLive.set_bits(), os);
      os << "] from operation: "
         << OpWithFlags(r.op,
                        OpPrintingFlags().skipRegions().printGenericOpForm());
    });
