...
public:
  ...
  toolchain::ThreadPoolInterface &getThreadPool() { return threadPool; }
...
private:
  ...
  toolchain::DefaultThreadPool threadPool;
};
...
__mlir_execution_engine_init(toolchain::StringMap<void *> &exportSymbols);

// NOLINTNEXTLINE(*-identifier-naming): externally called.
void __mlir_execution_engine_init(toolchain::StringMap<void *> &exportSymbols) {
  auto exportSymbol = [&](toolchain::StringRef name, auto ptr) {
