...
mgpuMemHostRegisterMemRef(int64_t rank, StridedMemRefType<char, 1> *descriptor,
  ...
  toolchain::SmallVector<int64_t, 4> denseStrides(rank);
  toolchain::ArrayRef<int64_t> sizes(descriptor->sizes, rank);
  toolchain::ArrayRef<int64_t> strides(sizes.end(), rank);
  ...
  std::rotate(denseStrides.begin(), denseStrides.begin() + 1,
              denseStrides.end());
  denseStrides.back() = 1;
  assert(strides == toolchain::ArrayRef(denseStrides));
