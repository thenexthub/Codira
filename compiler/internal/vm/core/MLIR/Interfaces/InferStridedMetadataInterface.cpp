...
void StridedMetadataRange::print(raw_ostream &os) const {
  ...
  toolchain::interleaveComma(*offsets, os, [&](const ConstantIntRanges &range) {
    os << "{" << range << "}";
  });
  os << "], sizes = [";
  toolchain::interleaveComma(sizes, os, [&](const ConstantIntRanges &range) {
    os << "{" << range << "}";
  });
  os << "], strides = [";
  toolchain::interleaveComma(strides, os, [&](const ConstantIntRanges &range) {
    os << "{" << range << "}";
  });
  os << "]>";
}
