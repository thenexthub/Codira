...
static void printFunctionResultList(OpAsmPrinter &p, TypeRange types,
  ...
  auto &os = p.getStream();
  bool needsParens = types.size() > 1 || toolchain::isa<FunctionType>(types[0]) ||
                     (attrs && !toolchain::cast<DictionaryAttr>(attrs[0]).empty());
  if (needsParens)
    os << '(';
  toolchain::interleaveComma(toolchain::seq<size_t>(0, types.size()), os, [&](size_t i) {
    p.printType(types[i]);
    if (attrs)
      p.printOptionalAttrDict(toolchain::cast<DictionaryAttr>(attrs[i]).getValue());
  });
  if (needsParens)
    os << ')';
}
...
void call_interface_impl::printFunctionSignature(
  ...
  for (unsigned i = 0, e = argTypes.size(); i < e; ++i) {
    ...
    if (!isExternal) {
      ...
      if (argAttrs)
        attrs = toolchain::cast<DictionaryAttr>(argAttrs[i]).getValue();
      p.printRegionArgument(body->getArgument(i), attrs);
    } else {
      p.printType(argTypes[i]);
      if (argAttrs)
        p.printOptionalAttrDict(
            toolchain::cast<DictionaryAttr>(argAttrs[i]).getValue());
    }
  }
