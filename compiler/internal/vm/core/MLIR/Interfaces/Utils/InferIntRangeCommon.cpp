...
mlir::intrange::inferIndexOp(const InferRangeFn &inferFn,
  ...
  toolchain::transform(argRanges, std::back_inserter(truncated),
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Index handling: 64-bit result = " << sixtyFour
                          << " 32-bit = " << thirtyTwo << "\n");
...
mlir::intrange::inferRemU(ArrayRef<ConstantIntRanges> argRanges) {
  ...
  APInt umax = toolchain::APIntOps::umin((rhsMax - 1), lhs.umax());
