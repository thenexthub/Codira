...
ValueBoundsConstraintSet::Variable::Variable(AffineMap map,
  ...
  for (auto [index, var] : toolchain::enumerate(mapOperands)) {
    ...
    for (auto valueDim : var.mapOperands) {
      auto *it = toolchain::find(this->mapOperands, valueDim);
...
ValueBoundsConstraintSet::Variable::Variable(AffineMap map,
    ...
    : Variable(map, toolchain::map_to_vector(mapOperands,
...
AffineExpr ValueBoundsConstraintSet::getExpr(OpFoldResult ofr) {
  if (Value value = toolchain::dyn_cast_if_present<Value>(ofr))
...
int64_t ValueBoundsConstraintSet::insert(AffineMap map,
  ...
  SmallVector<AffineExpr> dimReplacements = toolchain::to_vector(
      toolchain::map_range(ArrayRef(operands).take_front(map.getNumDims()), mapper));
  SmallVector<AffineExpr> symReplacements = toolchain::to_vector(
      toolchain::map_range(ArrayRef(operands).drop_front(map.getNumDims()), mapper));
  addBound(
...
LogicalResult ValueBoundsConstraintSet::computeDependentBound(
  ...
  return computeBound(
      ...
      [&](Value v, std::optional<int64_t> d, ValueBoundsConstraintSet &cstr) {
        return toolchain::is_contained(dependencies, std::make_pair(v, d));
      },
      closedUB);
}
  ...
  auto isIndependent = [&](Value v) {
    ...
    while (!worklist.empty()) {
      ...
      if (!visited.insert(next).second)
        continue;
      if (toolchain::is_contained(independencies, next))
        return false;
...
FailureOr<bool> ValueBoundsConstraintSet::areEquivalentSlices(
  ...
  for (auto [offset1, offset2] :
       toolchain::zip_equal(slice1.getMixedOffsets(), slice2.getMixedOffsets())) {
  ...
  for (auto [size1, size2] :
       toolchain::zip_equal(slice1.getMixedSizes(), slice2.getMixedSizes())) {
  ...
  for (auto [stride1, stride2] :
       toolchain::zip_equal(slice1.getMixedStrides(), slice2.getMixedStrides())) {
...
void ValueBoundsConstraintSet::dump() const {
  toolchain::errs() << "==========\nColumns:\n";
  toolchain::errs() << "(column\tdim\tvalue)\n";
  for (auto [index, valueDim] : toolchain::enumerate(positionToValueDim)) {
    toolchain::errs() << " " << index << "\t";
    if (valueDim) {
      if (valueDim->second == kIndexValue) {
        toolchain::errs() << "n/a\t";
      } else {
        toolchain::errs() << valueDim->second << "\t";
      }
      toolchain::errs() << getOwnerOfValue(valueDim->first)->getName() << " ";
      if (OpResult result = dyn_cast<OpResult>(valueDim->first)) {
        toolchain::errs() << "(result " << result.getResultNumber() << ")";
      } else {
        toolchain::errs() << "(bbarg "
                     << cast<BlockArgument>(valueDim->first).getArgNumber()
                     << ")";
      }
      toolchain::errs() << "\n";
    } else {
      toolchain::errs() << "n/a\tn/a\n";
    }
  }
  toolchain::errs() << "\nConstraint set:\n";
  cstr.dump();
