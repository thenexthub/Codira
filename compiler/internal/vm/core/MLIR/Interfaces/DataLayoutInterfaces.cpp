...
[[noreturn]] static void reportMissingDataLayout(Type type) {
  ...
  os << "neither the scoping op nor the type class provide data layout "
        "information for "
     << type;
  toolchain::report_fatal_error(Twine(message));
}
...
mlir::detail::getDefaultTypeSize(Type type, const DataLayout &dataLayout,
  ...
  toolchain::TypeSize bits = getDefaultTypeSizeInBits(type, dataLayout, params);
...
mlir::detail::getDefaultTypeSizeInBits(Type type, const DataLayout &dataLayout,
  ...
  if (type.isIntOrFloat())
    return toolchain::TypeSize::getFixed(type.getIntOrFloatBitWidth());
  ...
  if (auto ctype = dyn_cast<ComplexType>(type)) {
    ...
    toolchain::TypeSize innerSize = getDefaultTypeSizeInBits(et, dataLayout, params);

    // Include padding required to align the imaginary value in the complex
    // type.
    return toolchain::alignTo(innerSize, innerAlignment) + innerSize;
  }
  ...
  if (auto vecType = dyn_cast<VectorType>(type)) {
    uint64_t baseSize = vecType.getNumElements() / vecType.getShape().back() *
                        toolchain::PowerOf2Ceil(vecType.getShape().back()) *
                        dataLayout.getTypeSize(vecType.getElementType()) * 8;
    return toolchain::TypeSize::get(baseSize, vecType.isScalable());
  }
...
getIntegerTypeABIAlignment(IntegerType intType,
  ...
  if (params.empty()) {
    return intType.getWidth() < kSmallIntSize
               ? toolchain::PowerOf2Ceil(
                     toolchain::divideCeil(intType.getWidth(), kDefaultBitsInByte))
...
getFloatTypeABIAlignment(FloatType fltType, const DataLayout &dataLayout,
  ...
  if (params.empty())
    return toolchain::PowerOf2Ceil(dataLayout.getTypeSize(fltType).getFixedValue());
...
uint64_t mlir::detail::getDefaultABIAlignment(
  ...
  if (isa<VectorType>(type))
    return toolchain::PowerOf2Ceil(dataLayout.getTypeSize(type).getKnownMinValue());
...
getIntegerTypePreferredAlignment(IntegerType intType,
                                 ...
                                 ArrayRef<DataLayoutEntryInterface> params) {
  if (params.empty())
    return toolchain::PowerOf2Ceil(dataLayout.getTypeSize(intType).getFixedValue());
...
mlir::detail::filterEntriesForType(DataLayoutEntryListRef entries,
                                   TypeID typeID) {
  return toolchain::filter_to_vector<4>(
...
mlir::detail::filterEntryForIdentifier(DataLayoutEntryListRef entries,
                                       StringAttr id) {
  const auto *it = toolchain::find_if(entries, [id](DataLayoutEntryInterface entry) {
...
static DataLayoutSpecInterface getSpec(Operation *operation) {
  return toolchain::TypeSwitch<Operation *, DataLayoutSpecInterface>(operation)
...
collectParentLayouts(Operation *leaf,
  ...
  for (Operation *parent = leaf->getParentOp(); parent != nullptr;
    ...
    toolchain::TypeSwitch<Operation *>(parent)
...
static DataLayoutSpecInterface getCombinedDataLayout(Operation *leaf) {
  ...
  auto nonNullSpecs = toolchain::filter_to_vector<2>(
      toolchain::reverse(specs),
  ...
  return nonNullSpecs.back().combineWith(
      toolchain::ArrayRef(nonNullSpecs).drop_back());
}
...
toolchain::TypeSize mlir::detail::divideCeil(toolchain::TypeSize numerator,
                                        uint64_t denominator) {
  uint64_t divided =
      toolchain::divideCeil(numerator.getKnownMinValue(), denominator);
  return toolchain::TypeSize::get(divided, numerator.isScalable());
}
...
void mlir::DataLayout::checkValid() const {
  ...
  for (auto pair : toolchain::zip(specs, layoutStack)) {
...
toolchain::TypeSize mlir::DataLayout::getTypeSize(Type t) const {
  checkValid();
  return cachedLookup<toolchain::TypeSize>(t, sizes, [&](Type ty) {
...
toolchain::TypeSize mlir::DataLayout::getTypeSizeInBits(Type t) const {
  checkValid();
  return cachedLookup<toolchain::TypeSize>(t, bitsizes, [&](Type ty) {
    DataLayoutEntryList list;
    if (originalLayout)
      list = originalLayout.getSpecForType(ty.getTypeID());
    if (auto iface = dyn_cast_or_null<DataLayoutOpInterface>(scope))
...
void DataLayoutSpecInterface::bucketEntriesByType(
    toolchain::MapVector<TypeID, DataLayoutEntryList> &types,
    toolchain::MapVector<StringAttr, DataLayoutEntryInterface> &ids) {
  for (DataLayoutEntryInterface entry : getEntries()) {
    if (auto type = toolchain::dyn_cast_if_present<Type>(entry.getKey()))
      ...
      ids[toolchain::cast<StringAttr>(entry.getKey())] = entry;
  }
}
  ...
  for (const auto &kvp : types) {
    ...
    if (sampleType.isIntOrFloat()) {
      for (DataLayoutEntryInterface entry : kvp.second) {
        ...
        auto elements = toolchain::to_vector<2>(value.getValues<uint64_t>());
...
mlir::detail::verifyTargetSystemSpec(TargetSystemSpecInterface spec,
  ...
  for (const auto &entry : spec.getEntries()) {
    ...
    auto deviceID =
        toolchain::dyn_cast<TargetSystemSpecInterface::DeviceID>(entry.getKey());
    if (!deviceID)
      return failure();
