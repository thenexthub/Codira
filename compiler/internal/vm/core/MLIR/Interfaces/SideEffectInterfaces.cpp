...
static bool wouldOpBeTriviallyDeadImpl(Operation *rootOp) {
  ...
  while (!effectingOps.empty()) {
    ...
    if (auto effectInterface = dyn_cast<MemoryEffectOpInterface>(op)) {
      ...
      if (!toolchain::all_of(effects, [&allocResults](
...
bool mlir::hasEffect(Operation *op) {
  ...
  SmallVector<SideEffects::EffectInstance<MemoryEffects::Effect>, 4> effects;
  memOp.getEffects(effects);
  return toolchain::any_of(effects, [&](MemoryEffects::EffectInstance &effect) {
...
std::optional<toolchain::SmallVector<MemoryEffects::EffectInstance>>
