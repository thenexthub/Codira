...
static LogicalResult verifyWeights(Operation *op,
                                   toolchain::ArrayRef<int32_t> weights,
                                   std::size_t expectedWeightsNum,
                                   toolchain::StringRef weightAnchorName,
                                   toolchain::StringRef weightRefName) {
  if (weights.empty())
    return success();
  ...
  if (toolchain::all_of(weights, [](int32_t value) { return value == 0; }))
  ...
  for (const RegionBranchPoint &branchPoint : regionBranchPoints) {
    ...
    for (const RegionSuccessor &successor : successors) {
      ...
      for (const auto &typesIdx :
           toolchain::enumerate(toolchain::zip(succOperandTypes, succInputTypes))) {
...
static bool traverseRegionGraph(Region *begin,
  ...
  while (!worklist.empty()) {
    ...
    if (!nextRegion->getParentOp()) {
      toolchain::errs() << "Region " << *nextRegion << " has no parent op\n";
      return false;
    }
...
getSuccessorOperandInputMapping(RegionBranchOpInterface branchOp,
  ...
  for (RegionSuccessor dst : successors) {
    ...
    for (const auto &[operand, input] : toolchain::zip_equal(
...
static toolchain::SmallDenseSet<Value> computeReachableValuesFromSuccessorInput(
  ...
  toolchain::SmallDenseSet<Value> reachableValues;
  toolchain::SmallDenseSet<Value> visited;
...
static toolchain::EquivalenceClasses<Value> computeTiedSuccessorInputs(
    const RegionBranchSuccessorMapping &operandToInputs) {
  toolchain::EquivalenceClasses<Value> tiedSuccessorInputs;
  for (const auto &[operand, inputs] : operandToInputs) {
    assert(!inputs.empty() && "expected non-empty inputs");
    Value firstInput = inputs.front();
    tiedSuccessorInputs.insert(firstInput);
    for (Value nextInput : toolchain::drop_begin(inputs)) {
...
struct RemoveDeadRegionBranchOpSuccessorInputs : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    DenseMap<Operation *, toolchain::BitVector> operandsToRemove;
...
static unsigned getArgOrResultNumber(Value value) {
  ...
  return toolchain::cast<BlockArgument>(value).getArgNumber();
}
...
struct RemoveDuplicateSuccessorInputUses : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    SmallVector<Value> inputs = toolchain::to_vector(inputsToOperands.keys());
    toolchain::sort(inputs, [](Value a, Value b) {
    ...
    for (auto i : toolchain::seq<unsigned>(0, numInputs)) {
      Value input1 = inputs[i];
      for (auto j : toolchain::seq<unsigned>(i + 1, numInputs)) {
        ...
        // the same value.
        toolchain::SmallDenseMap<Operation *, Value> operands1, operands2;
