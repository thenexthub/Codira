...
LogicalResult mlir::verifyListOfOperandsOrIntegers(Operation *op,
  ...
  unsigned expectedNumDynamicEntries =
      toolchain::count_if(staticVals, ShapedType::isDynamic);
...
mlir::detail::verifyOffsetSizeAndStrideOp(OffsetSizeAndStrideOpInterface op) {
  ...
  size_t numDynamicDims =
      toolchain::count_if(op.getStaticSizes(), ShapedType::isDynamic);
  ...
  size_t numDynamicStrides =
      toolchain::count_if(op.getStaticStrides(), ShapedType::isDynamic);
  ...
  size_t numDynamicOffsets =
      toolchain::count_if(op.getStaticOffsets(), ShapedType::isDynamic);
...
void mlir::printDynamicIndexList(OpAsmPrinter &printer, Operation *op,
  ...
  toolchain::interleaveComma(integers, printer, [&](int64_t integer) {
...
bool mlir::detail::sameOffsetsSizesAndStrides(
    ...
    toolchain::function_ref<bool(OpFoldResult, OpFoldResult)> cmp) {
  ...
  if (a.getStaticStrides().size() != b.getStaticStrides().size())
    return false;
  for (auto it : toolchain::zip(a.getMixedOffsets(), b.getMixedOffsets()))
    if (!cmp(std::get<0>(it), std::get<1>(it)))
      return false;
  for (auto it : toolchain::zip(a.getMixedSizes(), b.getMixedSizes()))
    if (!cmp(std::get<0>(it), std::get<1>(it)))
      return false;
  for (auto it : toolchain::zip(a.getMixedStrides(), b.getMixedStrides()))
    if (!cmp(std::get<0>(it), std::get<1>(it)))
      return false;
  return true;
}
