...
ReductionNode::ReductionNode(
    ...
    toolchain::SpecificBumpPtrAllocator<ReductionNode> &allocator)
...
ArrayRef<ReductionNode *> ReductionNode::generateNewVariants() {
  ...
  if (variants.empty() && getRanges().size() > 1) {
    for (const Range &range : getRanges()) {
      ...
      toolchain::erase(subRanges, range);
  ...
  auto maxElement =
      toolchain::max_element(ranges, [](const Range &lhs, const Range &rhs) {
...
ReductionNode::iterator<SinglePath>::getNeighbors(ReductionNode *node) {
  ...
  if (!toolchain::all_of(variantsFromParent, [](ReductionNode *node) {
