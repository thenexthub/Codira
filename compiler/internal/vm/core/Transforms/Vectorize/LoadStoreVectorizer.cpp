...
using EqClassKey =
    std::tuple<const Value * /* result of getUnderlyingObject() */,
               ...
               >;
[[maybe_unused]] toolchain::raw_ostream &operator<<(toolchain::raw_ostream &OS,
                                               const EqClassKey &K) {
...
Pass *toolchain::createLoadStoreVectorizerPass() {
...
bool Vectorizer::run() {
  ...
  // https://github.com/toolchain/toolchain-project/issues/52950.
...
std::vector<Chain> Vectorizer::splitChainByContiguity(Chain &C) {
  ...
  toolchain::erase_if(Ret, [](const auto &Chain) { return Chain.size() <= 1; });
  return Ret;
}
  ...
  for (unsigned CBegin = 0; CBegin < C.size(); ++CBegin) {
    ...
    for (auto It = CandidateChains.rbegin(), End = CandidateChains.rend();
      ...
      if (CandidateChainsMayContainExtraLoadsStores) {
        ...
        // vector.
        bool CurrCandContainsExtraLoadsStores = toolchain::any_of(
...
bool Vectorizer::vectorizeChain(Chain &C) {
  ...
  bool ChainContainsExtraLoadsStores = toolchain::any_of(
      C, [this](const ChainElem &E) { return ExtraElements.contains(E.Inst); });
  ...
  if (IsLoadChain) {
    ...
    Builder.SetInsertPoint(
        toolchain::min_element(C, [](const auto &A, const auto &B) {
    ...
    for (const ChainElem &E : C) {
      ...
      if (!VecTy->isVectorTy()) {
        ...
        auto Mask = toolchain::to_vector<8>(
            toolchain::seq<int>(VecIdx, VecIdx + VT->getNumElements()));
    ...
    Builder.SetInsertPoint(toolchain::max_element(C, [](auto &A, auto &B) {
                             return A.Inst->comesBefore(B.Inst);
                           })->Inst);
...
Vectorizer::collectEquivalenceClasses(BasicBlock::iterator Begin,
  ...
  auto GetUnderlyingObject = [](const Value *Ptr) -> const Value * {
    const Value *ObjPtr = toolchain::getUnderlyingObject(Ptr);
