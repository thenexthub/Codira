...
const char LLVMLoopVectorizeFollowupAll[] = "toolchain.loop.vectorize.followup_all";
const char LLVMLoopVectorizeFollowupVectorized[] =
    "toolchain.loop.vectorize.followup_vectorized";
const char LLVMLoopVectorizeFollowupEpilogue[] =
    "toolchain.loop.vectorize.followup_epilogue";
/// @}
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const VPRecipeBase &R) {
...
void VPValue::replaceUsesWithIf(
    VPValue *New,
    toolchain::function_ref<bool(VPUser &U, unsigned Idx)> ShouldReplace) {
...
static void addRuntimeUnrollDisableMetaData(Loop *L) {
  ...
  if (LoopID) {
    ...
    for (unsigned I = 1, IE = LoopID->getNumOperands(); I < IE; ++I) {
      ...
      if (MD) {
        ...
        if (!S)
          continue;
        if (S->getString().starts_with("toolchain.loop.unroll.runtime.disable"))
          continue;
        IsUnrollMetadata =
            S->getString().starts_with("toolchain.loop.unroll.disable");
      }
  ...
  if (!IsUnrollMetadata) {
    ...
    DisableOperands.push_back(
        MDString::get(Context, "toolchain.loop.unroll.runtime.disable"));
...
bool toolchain::canConstantBeExtended(const APInt *C, Type *NarrowType,
