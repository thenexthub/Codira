...
#include "vm/core/ADT/ilist_node.h"
...
public:
  ...
  void setMetadata(unsigned Kind, MDNode *Node) {
    auto It =
        toolchain::find_if(Metadata, [Kind](const std::pair<unsigned, MDNode *> &P) {
          return P.first == Kind;
        });
    if (It != Metadata.end())
      It->second = Node;
...
public:
  ...
  enum {
    ...
    // Implemented with @toolchain.experimental.cttz.elts, but returns the expected
...
public:
  ...
  /// incoming blocks.
  detail::zippy<toolchain::detail::zip_first, VPUser::const_operand_range,
...
/// lanes, which is handled by the toolchain.experimental.vector.histogram family
...
public:
  ...
  VPWidenPHIRecipe *clone() override {
    ...
    for (VPValue *Op : toolchain::drop_begin(operands()))
      C->addOperand(Op);
    return C;
  }
...
public:
  ...
  bool usesFirstLaneOnly(const VPValue *Op) const override {
    assert(is_contained(operands(), Op) &&
           "Op must be an operand of the recipe");
    return Op == getAddr() && !toolchain::is_contained(getStoredValues(), Op);
  }
