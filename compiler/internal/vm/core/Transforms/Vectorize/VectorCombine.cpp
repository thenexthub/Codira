...
static ScalarizationResult canScalarizeAccess(VectorType *VecTy, Value *Idx,
  ...
  if (!toolchain::isUIntN(IntWidth, NumElements))
    return ScalarizationResult::unsafe();
...
bool VectorCombine::scalarizeLoadExtract(LoadInst *LI, VectorType *VecTy,
  ...
  toolchain::scope_exit FailureGuard([&]() {
...
bool VectorCombine::foldPermuteOfBinops(Instruction &I) {
  ...
  if (BinOp->isIntDivRem() && toolchain::is_contained(OuterMask, PoisonMaskElem))
    return false;
...
bool VectorCombine::foldShuffleOfBinops(Instruction &I) {
  ...
  if (match(LHS, m_BinOp(m_Value(X), m_Value(Y))) &&
    ...
    // Don't introduce poison into div/rem.
    if (toolchain::is_contained(OldMask, PoisonMaskElem) && BO->isIntDivRem())
      return false;
  ...
  if (X == Z) {
    toolchain::for_each(NewMask0, ConvertToUnary);
  ...
  if (Y == W) {
    toolchain::for_each(NewMask1, ConvertToUnary);
...
bool VectorCombine::foldShufflesOfLengthChangingShuffles(Instruction &I) {
  ...
  for (;;) {
    ...
    if (Match0Leaf) {
      ...
      std::swap(A0, A1);
      std::swap(B0, B1);
      toolchain::append_range(CommutedOuterMask, OuterMask);
    ...
    bool Valid = true;
    for (auto [CombinedM, LeafM] : toolchain::zip(NewYMask, LocalYMask)) {
  ...
  if (toolchain::all_of(Mask, [&](int M) {
...
///
///   %2 = tail call <n x ty1> toolchain.<umin/umax/smin/smax>(<n x ty1> %0, <n x
///   ty1> %1)
///     OR
...
///   ...
///   ...
///   %(i - 1) = tail call <n x ty1> toolchain.<umin/umax/smin/smax>(<n x ty1> %(i -
///   3), <n x ty1> %(i - 2)
///     OR
  ...
  auto collectShuffles = [&](Instruction *I) {
    for (auto *U : I->users()) {
      ...
      if ((SV->getOperand(0) != Op0 && SV->getOperand(0) != Op1) ||
        ...
        return false;
      if (!toolchain::is_contained(Shuffles, SV))
