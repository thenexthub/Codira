...
// http://lists.toolchain.org/pipermail/toolchain-dev/2017-December/119523.html). For this
...
#include "vm/core/Transforms/Vectorize/LoopVectorizationLegality.h"
#include <algorithm>
#include <cassert>
#include <cmath>
#include <cstdint>
...
cl::opt<bool> toolchain::EnableWideActiveLaneMask(
...
cl::opt<unsigned> toolchain::ForceTargetInstructionCost(
...
cl::opt<bool> toolchain::EnableVPlanNativePath(
    "enable-vplan-native-path", cl::Hidden,
    cl::desc("Enable VPlan-native vectorization path with "
             "support for outer loop vectorization."));

cl::opt<bool>
    toolchain::VerifyEachVPlan("vplan-verify-each",
...
cl::opt<bool> toolchain::EnableLoopInterleaving(
    "interleave-loops", cl::init(true), cl::Hidden,
    cl::desc("Enable loop interleaving in Loop vectorization passes"));
cl::opt<bool> toolchain::EnableLoopVectorization(
...
// using the same metadata (toolchain.loop.vectorize, processed by
...
static void legacyCSE(BasicBlock *BB) {
  ...
  for (Instruction &In : toolchain::make_early_inc_range(*BB)) {
...
void LoopVectorizationCostModel::collectLoopScalars(ElementCount VF) {
  ...
  while (Idx != Worklist.size()) {
    ...
    if (toolchain::all_of(Src->users(), [&](User *U) -> bool {
...
void LoopVectorizationCostModel::collectLoopUniforms(ElementCount VF) {
  ...
  while (Idx != Worklist.size()) {
    ...
    for (auto *OV : I->operand_values()) {
      ...
      if (toolchain::all_of(OI->users(), [&](User *U) -> bool {
            auto *J = cast<Instruction>(U);
...
ElementCount LoopVectorizationCostModel::clampVFByMaxTripCount(
  ...
  if (MaxTripCount && MaxTripCount <= EstimatedVF &&
    ...
    auto ClampedUpperTripCount = toolchain::bit_floor(MaxTripCount);
...
ElementCount LoopVectorizationCostModel::getMaximizedVFForTarget(
  ...
  auto MaxVectorElementCount = ElementCount::get(
      toolchain::bit_floor(WidestRegister.getKnownMinValue() / WidestType),
  ...
  if (useMaxBandwidth(RegKind)) {
    auto MaxVectorElementCountMaxBW = ElementCount::get(
        toolchain::bit_floor(WidestRegister.getKnownMinValue() / SmallestType),
        ComputeScalableMaxVF);
...
LoopVectorizationPlanner::selectInterleaveCount(VPlan &Plan, ElementCount VF,
  ...
  for (const auto &Pair : R.MaxLocalUsers) {
    ...
    unsigned TmpIC = toolchain::bit_floor((TargetNumRegisters - LoopInvariantRegs) /
                                     MaxLocalUsers);
    // Don't count the induction variable as interleaved.
    if (EnableIndVarRegisterHeur) {
      TmpIC = toolchain::bit_floor((TargetNumRegisters - LoopInvariantRegs - 1) /
  ...
  if (!ScalarInterleavingRequiresRuntimePointerCheck &&
    ...
    unsigned SmallIC = std::min(IC, (unsigned)toolchain::bit_floor<uint64_t>(
...
LoopVectorizationCostModel::getReductionPatternCost(Instruction *I,
  ...
  // For a call to the toolchain.fmuladd intrinsic we need to add the cost of a
  ...
  if (isScalarAfterVectorization(I, VF)) {
    [[maybe_unused]] auto HasSingleCopyAfterVectorization =
      ...
      return !Scalarized->second.count(I) &&
             toolchain::all_of(I->users(), [&](User *U) {
               auto *UI = cast<Instruction>(U);
...
/// depicted in https://toolchain.org/docs/Vectorizers.html#epilogue-vectorization.
...
VPlanPtr LoopVectorizationPlanner::tryToBuildVPlanWithVPRecipes(
  ...
  using namespace vm::core::VPlanPatternMatch;
...
static void connectEpilogueVectorLoop(
  ...
  SmallVector<PHINode *, 4> PhisInBlock(
      toolchain::make_pointer_range(VecEpilogueIterationCountCheck->phis()));
...
bool LoopVectorizePass::processLoop(Loop *L) {
  ...
  bool OptForSize = toolchain::shouldOptimizeForSize(
      L->getHeader(), PSI,
