...
static void RewriteUsesOfClonedInstructions(BasicBlock *OrigHeader,
  ...
  for (I = OrigHeader->begin(); I != E; ++I) {
    ...
    for (Use &U : toolchain::make_early_inc_range(OrigHeaderVal->uses())) {
    ...
    toolchain::findDbgValues(OrigHeaderVal, DbgVariableRecords);
...
static bool profitableToRotateLoopExitingLatch(Loop *L) {
  ...
  for (auto &Phi : Header->phis()) {
    ...
    if (toolchain::any_of(Phi.users(), [HeaderExit](const User *U) {
...
bool LoopRotate::rotateLoop(Loop *L, bool SimplifiedLatch) {
  ...
  if (!NoAliasDeclInstructions.empty()) {
    ...
    //
    // with D: toolchain.experimental.noalias.scope.decl,
    ...
    // (1) -> : ... D' { D U1 U2 D'' }
    // (2) -> : ... D' U1' { D U2 D'' U1'' }
    // D: original toolchain.experimental.noalias.scope.decl
    ...
    // Clone the toolchain.experimental.noalias.decl again for the NewHeader.
    ...
    for (NoAliasScopeDeclInst *NAD : NoAliasDeclInstructions) {
      LLVM_DEBUG(dbgs() << "  Cloning toolchain.experimental.noalias.scope.decl:"
                        << *NAD << "\n");
...
bool toolchain::LoopRotation(Loop *L, LoopInfo *LI, const TargetTransformInfo *TTI,
