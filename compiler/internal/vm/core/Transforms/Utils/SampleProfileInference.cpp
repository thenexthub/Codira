...
#include "vm/core/Transforms/Utils/SampleProfileInference.h"
...
private:
  ...
  std::vector<FlowJump *> findShortestPath(uint64_t BlockIdx) {
    ...
    toolchain::append_range(Result, ForwardPath);
    toolchain::append_range(Result, BackwardPath);
    return Result;
  }
...
/// profi options
void toolchain::applyFlowInference(const ProfiParams &Params, FlowFunction &Func) {
...
void toolchain::applyFlowInference(FlowFunction &Func) {
  ProfiParams Params;
