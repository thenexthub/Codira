...
static bool isBlockValidForExtraction(const BasicBlock &BB,
  ...
  SmallVector<User const *, 16> ToVisit(toolchain::make_pointer_range(BB));
  ...
  for (BasicBlock::const_iterator I = BB.begin(), E = BB.end(); I != E; ++I) {
    ...
    if (const CallInst *CI = dyn_cast<CallInst>(I)) {
      ...
      if (const Function *F = CI->getCalledFunction()) {
        ...
        // proposals for fixing this in toolchain.org/PR39545.
...
CodeExtractor::findOrCreateBlockForHoisting(BasicBlock *CommonExitBlock) {
  ...
  for (BasicBlock *Pred :
       toolchain::make_early_inc_range(predecessors(CommonExitBlock))) {
    if (Blocks.count(Pred))
      continue;
...
bool CodeExtractor::isEligible() const {
  ...
  if (AllowVarArgs && F->getFunctionType()->isVarArg()) {
    ...
    for (auto &BB : *F) {
      if (Blocks.count(&BB))
        continue;
      if (toolchain::any_of(BB, containsVarArgIntrinsic))
        return false;
    }
  }
      ...
      for (auto &OI : II.operands()) {
        ...
        if (!SinkCands.count(V) &&
            (definedInCaller(Blocks, V) ||
             (CollectGlobalInputs && toolchain::isa<toolchain::GlobalVariable>(V))))
          Inputs.insert(V);
      }
...
CodeExtractor::extractCodeRegion(const CodeExtractorAnalysisCache &CEAC,
  ...
  // Remove @toolchain.assume calls that will be moved to the new function from the
  // old function's assumption cache.
  for (BasicBlock *Block : Blocks) {
    for (Instruction &I : toolchain::make_early_inc_range(*Block)) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "After extractCodeRegion - newFunction:\n");
  LLVM_DEBUG(newFunction->dump());
  LLVM_DEBUG(toolchain::dbgs() << "After extractCodeRegion - oldFunction:\n");
...
bool CodeExtractor::verifyAssumptionCache(const Function &OldFunc,
  ...
  for (auto AssumeVH : AC->assumptions()) {
    ...
    // There shouldn't be any toolchain.assume intrinsics in the new function.
