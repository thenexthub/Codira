...
bool toolchain::applyDebugifyMetadata(
  ...
  if (M.getNamedMetadata("toolchain.dbg.cu")) {
  ...
  NamedMDNode *NMD = M.getOrInsertNamedMetadata("toolchain.debugify");
  ...
  assert(NMD->getNumOperands() == 2 &&
         "toolchain.debugify should have exactly 2 operands!");
...
bool toolchain::stripDebugifyMetadata(Module &M) {
  bool Changed = false;

  // Remove the toolchain.debugify and toolchain.mir.debugify module-level named metadata.
  ...
  if (auto *MIRDebugifyMD = M.getNamedMetadata("toolchain.mir.debugify")) {
  ...
  Function *DbgValF = M.getFunction("toolchain.dbg.value");
  if (DbgValF) {
...
bool toolchain::collectDebugInfoMetadata(Module &M,
  ...
  for (const auto &F : DIFunctionsAfter) {
    ...
    if (SPIt == DIFunctionsBefore.end()) {
      if (ShouldWriteIntoJSON)
        Bugs.push_back(toolchain::json::Object({{"metadata", "DISubprogram"},
...
static bool checkInstructions(const DebugInstMap &DILocsBefore,
  ...
  for (const auto &L : DILocsAfter) {
    ...
    auto CreateJSONBugEntry = [&](const char *Action) {
      Bugs.push_back(toolchain::json::Object({
          {"metadata", "DILocation"},
          {"fn-name", FnName.str()},
          {"bb-name", BBName.str()},
          {"instr", InstName},
...
static bool checkVars(const DebugVarMap &DIVarsBefore,
                      ...
                      bool ShouldWriteIntoJSON, toolchain::json::Array &Bugs) {
  bool Preserved = true;
...
static void writeJSON(StringRef OrigDIVerifyBugsReportFilePath,
  ...
  if (auto L = OS_FILE.lock()) {
    ...
    toolchain::json::Value BugsToPrint{std::move(Bugs)};
...
bool toolchain::checkDebugInfoMetadata(Module &M,
                                  iterator_range<Module::iterator> Functions,
                                  DebugInfoPerPass &DebugInfoBeforePass,
  ...
  StringRef FileNameFromCU =
      (cast<DICompileUnit>(M.getNamedMetadata("toolchain.dbg.cu")->getOperand(0)))
          ->getFilename();
...
bool checkDebugifyMetadata(Module &M,
  ...
  NamedMDNode *NMD = M.getNamedMetadata("toolchain.debugify");
  if (!NMD) {
...
void toolchain::exportDebugifyStats(StringRef Path, const DebugifyStatsMap &Map) {
  std::error_code EC;
...
ModulePass *createDebugifyModulePass(enum DebugifyMode Mode,
                                     toolchain::StringRef NameOfWrappedPass,
                                     DebugInfoPerPass *DebugInfoBeforePass) {
  if (Mode == DebugifyMode::SyntheticDebugInfo)
...
void DebugifyEachInstrumentation::registerCallbacks(
  ...
  PIC.registerBeforeNonSkippedPassCallback([this, &MAM](StringRef P, Any IR) {
    ...
    PreservedAnalyses PA;
    PA.preserveSet<CFGAnalyses>();
    if (const auto **CF = toolchain::any_cast<const Function *>(&IR)) {
      Function &F = *const_cast<Function *>(*CF);
      applyDebugify(F, Mode, DebugInfoBeforePass, P);
      MAM.getResult<FunctionAnalysisManagerModuleProxy>(*F.getParent())
          .getManager()
          .invalidate(F, PA);
    } else if (const auto **CM = toolchain::any_cast<const Module *>(&IR)) {
