...
static void DisableAllLoopOptsOnLoop(Loop &L) {
  ...
  MDNode *DisableUnroll = MDNode::get(
      Context, {MDString::get(Context, "toolchain.loop.unroll.disable")});
  Metadata *FalseVal =
      ConstantAsMetadata::get(ConstantInt::get(Type::getInt1Ty(Context), 0));
  MDNode *DisableVectorize = MDNode::get(
      Context,
      {MDString::get(Context, "toolchain.loop.vectorize.enable"), FalseVal});
  MDNode *DisableLICMVersioning = MDNode::get(
      Context, {MDString::get(Context, "toolchain.loop.licm_versioning.disable")});
  MDNode *DisableDistribution = MDNode::get(
      Context,
      {MDString::get(Context, "toolchain.loop.distribute.enable"), FalseVal});
  MDNode *NewLoopID =
