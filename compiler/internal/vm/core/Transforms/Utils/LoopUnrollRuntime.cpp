...
//    (ModVal) is zero.
/// - At the epilog preheader, add an toolchain.assume call that extra iters is
...
///   later unroll the epilog loop, that toolchain.assume call somehow enables
...
bool toolchain::UnrollRuntimeLoopRemainder(
  ...
  for (auto *BB : OtherExits) {
    ...
    for (BasicBlock *SuccBB : successors(BB)) {
      assert(!(toolchain::is_contained(OtherExits, SuccBB) || SuccBB == LatchExit) &&
