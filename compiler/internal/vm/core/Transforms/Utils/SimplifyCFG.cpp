...
#include "vm/core/Transforms/Utils/LockstepReverseIterator.h"
...
safeToMergeTerminators(Instruction *SI1, Instruction *SI2,
  ...
  SmallPtrSet<BasicBlock *, 16> SI1Succs(toolchain::from_range, successors(SI1BB));
  bool Fail = false;
...
bool SimplifyCFGOpt::performValueComparisonIntoPredecessorFolding(
  ...
  if (DTU) {
    SuccsOfPred = {toolchain::from_range, successors(Pred)};
...
static bool isSafeToHoistInstr(Instruction *I, unsigned Flags) {
  ...
  // Hoisting of toolchain.deoptimize is only legal together with the next return
...
/// \endcode
///
/// where cload/cstore are represented by toolchain.masked.load/store intrinsics,
/// e.g.
///
/// \code
///   %vcond = bitcast i1 %cond to <1 x i1>
///   %v0 = call <1 x i32> @toolchain.masked.load.v1i32.p0
///                         (ptr %b, i32 4, <1 x i1> %vcond, <1 x i32> poison)
///   %maskedloadstore = bitcast <1 x i32> %v0 to i32
///   call void @toolchain.masked.store.v1i32.p0
...
static bool isSafeCheapLoadStore(const Instruction *I,
  ...
  // toolchain.masked.load/store use i32 for alignment while load/store use i64.
...
bool SimplifyCFGOpt::hoistSuccIdenticalTerminatorToSwitchOrIf(
  ...
  // FIXME: Test case toolchain/test/Transforms/SimplifyCFG/2009-06-15-InvokeCrash.ll
...
static bool sinkCommonCodeFromPredecessors(BasicBlock *BB,
  ...
  if (HaveNonUnconditionalPredecessors) {
    if (!followedByDeoptOrUnreachable) {
      ...
      // See https://bugs.toolchain.org/show_bug.cgi?id=30244
      LRI.reset();
      int Idx = 0;
...
bool CompatibleSets::shouldBelongToSameSet(ArrayRef<InvokeInst *> Invokes) {
  ...
  if (any_of(Invokes, HasNormalDest)) {
    ...
    // must be compatible.
    SmallPtrSet<Value *, 16> EquivalenceSet(toolchain::from_range, Invokes);
...
bool SimplifyCFGOpt::speculativelyExecuteBB(BranchInst *BI,
  ...
  if (SpeculatedStoreValue) {
    ...
    for (DbgVariableRecord *DbgAssign :
      ...
      if (toolchain::is_contained(DbgAssign->location_ops(), OrigV))
...
bool toolchain::foldBranchToCommonDest(BranchInst *BI, DomTreeUpdater *DTU,
                                  MemorySSAUpdater *MSSAU,
                                  const TargetTransformInfo *TTI,
  ...
  auto IsWorthwhile = [&](BasicBlock *BB, ArrayRef<StoreInst *> FreeStores) {
    ...
    for (auto &I : BB->instructionsWithoutDebug(false)) {
      ...
      if (auto *S = dyn_cast<StoreInst>(&I))
        if (toolchain::find(FreeStores, S))
          continue;
...
bool SimplifyCFGOpt::simplifyBranchOnICmpChain(BranchInst *BI,
  ...
  Values.erase(toolchain::unique(Values), Values.end());
...
bool SimplifyCFGOpt::simplifyCommonResume(ResumeInst *RI) {
  ...
  for (auto *TrivialBB : TrivialUnwindBlocks) {
    ...
    for (BasicBlock *Pred :
         toolchain::make_early_inc_range(predecessors(TrivialBB))) {
...
bool SimplifyCFGOpt::simplifySingleResume(ResumeInst *RI) {
  ...
  for (BasicBlock *Pred : toolchain::make_early_inc_range(predecessors(BB))) {
    removeUnwindEdge(Pred, DTU);
    ++NumInvokes;
  }
...
static bool removeEmptyCleanup(CleanupReturnInst *RI, DomTreeUpdater *DTU) {
  ...
  for (BasicBlock *PredBB : toolchain::make_early_inc_range(predecessors(BB))) {
...
static bool forwardSwitchConditionToPHI(SwitchInst *SI) {
  ...
  for (auto &ForwardingNode : ForwardingNodes) {
    ...
    if (Indexes.size() < 2 && !toolchain::is_contained(Phi->incoming_values(), SI->getCondition()))
      continue;
...
static bool reduceSwitchRange(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  unsigned Shift = 64;
  for (auto &V : Values)
    Shift = std::min(Shift, (unsigned)toolchain::countr_zero((uint64_t)V));
  assert(Shift < 64);
  if (Shift > 0)
    for (auto &V : Values)
...
static bool simplifySwitchOfPowersOfTwo(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  for (const auto &Case : SI->cases()) {
    ...
    if (toolchain::has_single_bit(CaseValue))
  ...
  toolchain::sort(Values);
  if (!isSwitchDense(Values.size(), toolchain::countr_zero(Values.back()) -
                                        toolchain::countr_zero(Values.front()) + 1))
...
template <> struct toolchain::DenseMapInfo<const SwitchSuccWrapper *> {
...
bool SimplifyCFGOpt::simplifyDuplicateSwitchArms(SwitchInst *SI,
  ...
  for (unsigned I = 0; I < SI->getNumSuccessors(); ++I) {
    ...
    for (BasicBlock *Succ : BI->successors())
      Phis.insert_range(toolchain::make_pointer_range(Succ->phis()));
...
static bool passingValueIsAlwaysUndefined(Value *V, Instruction *I, bool PtrValueMayBeModified) {
  ...
  if (C->isNullValue() || isa<UndefValue>(C)) {
    ...
    // long uselists
    auto FindUse = toolchain::find_if(I->uses(), [](auto &U) {
    ...
    // toolchain.assume(false/undef) always triggers immediate UB.
...
bool toolchain::simplifyCFG(BasicBlock *BB, const TargetTransformInfo &TTI,
