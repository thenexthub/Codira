...
Value *Mapper::mapValue(const Value *V) {
  ...
  if (const auto *E = dyn_cast<DSOLocalEquivalent>(C)) {
    ...
    return getVM()[E] = toolchain::ConstantExpr::getBitCast(
...
void Mapper::remapDbgRecord(DbgRecord &DR) {
  ...
  if (!IgnoreMissingLocals && toolchain::is_contained(NewVals, nullptr)) {
    V.setKillLocation();
  } else {
...
void MDNodeMapper::UniquedGraph::propagateChanges() {
  ...
  do {
    ...
    for (MDNode *N : POT) {
      ...
      if (toolchain::none_of(N->operands(), [&](const Metadata *Op) {
            auto Where = Info.find(Op);
...
void toolchain::RemapSourceAtom(Instruction *I, ValueToValueMapTy &VM) {
