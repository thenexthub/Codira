...
bool toolchain::HasLoopOrEntryConvergenceToken(const BasicBlock *BB) {
...
void toolchain::detachDeadBlocks(ArrayRef<BasicBlock *> BBs,
                            SmallVectorImpl<DominatorTree::UpdateType> *Updates,
                            bool KeepOneInputPHIs) {
...
void toolchain::DeleteDeadBlock(BasicBlock *BB, DomTreeUpdater *DTU,
                           bool KeepOneInputPHIs) {
  DeleteDeadBlocks({BB}, DTU, KeepOneInputPHIs);
}

void toolchain::DeleteDeadBlocks(ArrayRef <BasicBlock *> BBs, DomTreeUpdater *DTU,
                            bool KeepOneInputPHIs) {
#ifndef NDEBUG
  // Make sure that all predecessors of each dead block is also dead.
  SmallPtrSet<BasicBlock *, 4> Dead(toolchain::from_range, BBs);
...
bool toolchain::EliminateUnreachableBlocks(Function &F, DomTreeUpdater *DTU,
                                      bool KeepOneInputPHIs) {
...
bool toolchain::FoldSingleEntryPHINodes(BasicBlock *BB,
...
bool toolchain::DeleteDeadPHIs(BasicBlock *BB, const TargetLibraryInfo *TLI,
                          MemorySSAUpdater *MSSAU) {
  // Recursively deleting a PHI may cause multiple PHIs to be deleted
  // or RAUW'd undef, so use an array of WeakTrackingVH for the PHIs to delete.
  SmallVector<WeakTrackingVH, 8> PHIs(toolchain::make_pointer_range(BB->phis()));
...
bool toolchain::MergeBlockIntoPredecessor(BasicBlock *BB, DomTreeUpdater *DTU,
  ...
  for (PHINode &PN : BB->phis())
    if (toolchain::is_contained(PN.incoming_values(), &PN))
      return false;
  ...
  if (DTU) {
    ...
    SmallPtrSet<BasicBlock *, 2> SuccsOfPredBB(toolchain::from_range,
                                               successors(PredBB));
...
bool toolchain::MergeBlockSuccessorsIntoGivenBlocks(
...
bool toolchain::RemoveRedundantDbgInstrs(BasicBlock *BB) {
...
void toolchain::ReplaceInstWithValue(BasicBlock::iterator &BI, Value *V) {
  Instruction &I = *BI;
...
void toolchain::ReplaceInstWithInst(BasicBlock *BB, BasicBlock::iterator &BI,
                               Instruction *I) {
...
bool toolchain::IsBlockFollowedByDeoptOrUnreachable(const BasicBlock *BB) {
...
void toolchain::ReplaceInstWithInst(Instruction *From, Instruction *To) {
  BasicBlock::iterator BI(From);
  ReplaceInstWithInst(From->getParent(), BI, To);
}

BasicBlock *toolchain::SplitEdge(BasicBlock *BB, BasicBlock *Succ, DominatorTree *DT,
                            LoopInfo *LI, MemorySSAUpdater *MSSAU,
                            const Twine &BBName) {
...
BasicBlock *toolchain::SplitCallBrEdge(BasicBlock *CallBrBlock, BasicBlock *Succ,
...
void toolchain::setUnwindEdgeTo(Instruction *TI, BasicBlock *Succ) {
...
void toolchain::updatePhiNodes(BasicBlock *DestBB, BasicBlock *OldPred,
...
BasicBlock *toolchain::ehAwareSplitEdge(BasicBlock *BB, BasicBlock *Succ,
...
void toolchain::createPHIsForSplitLoopExit(ArrayRef<BasicBlock *> Preds,
...
unsigned
toolchain::SplitAllCriticalEdges(Function &F,
                            const CriticalEdgeSplittingOptions &Options) {
...
BasicBlock *toolchain::SplitBlock(BasicBlock *Old, BasicBlock::iterator SplitPt,
...
BasicBlock *toolchain::splitBlockBefore(BasicBlock *Old, BasicBlock::iterator SplitPt,
                                   DomTreeUpdater *DTU, LoopInfo *LI,
                                   MemorySSAUpdater *MSSAU,
                                   const Twine &BBName) {
...
static void UpdatePHINodes(BasicBlock *OrigBB, BasicBlock *NewBB,
  ...
  SmallPtrSet<BasicBlock *, 16> PredSet(toolchain::from_range, Preds);
...
BasicBlock *toolchain::SplitBlockPredecessors(BasicBlock *BB,
                                         ArrayRef<BasicBlock *> Preds,
...
void toolchain::SplitLandingPadPredecessors(BasicBlock *OrigBB,
                                       ArrayRef<BasicBlock *> Preds,
...
ReturnInst *toolchain::FoldReturnIntoUncondBranch(ReturnInst *RI, BasicBlock *BB,
                                             BasicBlock *Pred,
                                             DomTreeUpdater *DTU) {
...
Instruction *toolchain::SplitBlockAndInsertIfThen(Value *Cond,
...
Instruction *toolchain::SplitBlockAndInsertIfElse(Value *Cond,
                                             BasicBlock::iterator SplitBefore,
                                             bool Unreachable,
                                             MDNode *BranchWeights,
                                             DomTreeUpdater *DTU, LoopInfo *LI,
...
void toolchain::SplitBlockAndInsertIfThenElse(Value *Cond, BasicBlock::iterator SplitBefore,
...
void toolchain::SplitBlockAndInsertIfThenElse(
...
toolchain::SplitBlockAndInsertSimpleForLoop(Value *End,
                                       BasicBlock::iterator SplitBefore) {
...
void toolchain::SplitBlockAndInsertForEachLane(
...
BranchInst *toolchain::GetIfCondition(BasicBlock *BB, BasicBlock *&IfTrue,
                                 BasicBlock *&IfFalse) {
...
void toolchain::InvertBranch(BranchInst *PBI, IRBuilderBase &Builder) {
...
bool toolchain::hasOnlySimpleTerminator(const Function &F) {
  for (auto &BB : F) {
...
Printable toolchain::printBasicBlock(const BasicBlock *BB) {
