...
char &toolchain::BreakCriticalEdgesID = BreakCriticalEdges::ID;

FunctionPass *toolchain::createBreakCriticalEdgesPass() {
...
BasicBlock *toolchain::SplitCriticalEdge(Instruction *TI, unsigned SuccNum,
...
BasicBlock *
toolchain::SplitKnownCriticalEdge(Instruction *TI, unsigned SuccNum,
                             const CriticalEdgeSplittingOptions &Options,
                             const Twine &BBName) {
  ...
  if (DT || PDT) {
    ...
    if (!toolchain::is_contained(successors(TIBB), DestBB))
...
bool toolchain::SplitIndirectBrCriticalEdges(Function &F,
