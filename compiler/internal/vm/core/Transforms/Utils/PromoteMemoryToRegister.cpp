...
#include "vm/core/Analysis/IteratedDominanceFrontier.h"
...
bool toolchain::isAllocaPromotable(const AllocaInst *AI) {
...
struct PromoteMem2Reg {
  ...
  /// A cache of @toolchain.assume intrinsics used by SimplifyInstruction.
  AssumptionCache *AC;
...
static void removeIntrinsicUsers(AllocaInst *AI) {
  ...
  for (Use &U : toolchain::make_early_inc_range(AI->uses())) {
    ...
    if (!I->getType()->isVoidTy()) {
      ...
      for (Use &UU : toolchain::make_early_inc_range(I->uses())) {
...
static bool promoteSingleBlockAlloca(
  ...
  // binary search.
  toolchain::sort(StoresByIndex, less_first());
  ...
  for (User *U : make_early_inc_range(AI->users())) {
    ...
    StoresByIndexTy::iterator I = toolchain::lower_bound(
        StoresByIndex,
...
void PromoteMem2Reg::run() {
  ...
  for (unsigned AllocaNum = 0; AllocaNum != Allocas.size(); ++AllocaNum) {
    ...
    SmallPtrSet<BasicBlock *, 32> DefBlocks(toolchain::from_range,
                                            Info.DefiningBlocks);
    ...
    toolchain::sort(PHIBlocks, [](BasicBlock *A, BasicBlock *B) {
  ...
  for (const auto &PhiNode : NewPhiNodes) {
    ...
    toolchain::sort(Preds, CompareBBNumbers);

    // Now we loop through all BB's which have entries in SomePHI and remove
    // them from the Preds list.
    for (unsigned i = 0, e = SomePHI->getNumIncomingValues(); i != e; ++i) {
      // Do a log(n) search of the Preds list for the entry we want.
      SmallVectorImpl<BasicBlock *>::iterator EntIt = toolchain::lower_bound(
...
void PromoteMem2Reg::RenamePass(BasicBlock *BB, BasicBlock *Pred) {
  ...
  if (PHINode *APN = dyn_cast<PHINode>(BB->begin())) {
    ...
    if (PhiToAllocaMap.count(APN)) {
      ...
      unsigned NumEdges = toolchain::count(successors(Pred), BB);
...
void toolchain::PromoteMemToReg(ArrayRef<AllocaInst *> Allocas, DominatorTree &DT,
                           AssumptionCache *AC) {
