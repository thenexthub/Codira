...
cl::opt<unsigned> toolchain::SCEVCheapExpansionBudget(
...
/// http://toolchain.org/docs/LangRef.html#pointeraliasing
/// for details.
///
...
Value *SCEVExpander::visitAddExpr(const SCEVAddExpr *S) {
  ...
  toolchain::stable_sort(OpsAndLoops, LoopCompare(SE.DT));
...
bool SCEVExpander::isNormalAddRecExprPHI(PHINode *PN, Instruction *IncV,
  ...
  if (L == IVIncInsertLoop) {
    for (Use &Op : toolchain::drop_begin(IncV->operands()))
...
Instruction *SCEVExpander::getIVIncOperand(Instruction *IncV,
  ...
  case Instruction::GetElementPtr:
    for (Use &U : toolchain::drop_begin(IncV->operands())) {
      if (isa<Constant>(U))
        continue;
...
bool SCEVExpander::hoistIVInc(Instruction *IncV, Instruction *InsertPos,
  ...
  for (Instruction *I : toolchain::reverse(IVIncs)) {
    fixupInsertPoints(I);
...
SCEVExpander::replaceCongruentIVs(Loop *L, const DominatorTree *DT,
  ...
  if (TTI)
    ...
    toolchain::stable_sort(Phis, [](Value *LHS, Value *RHS) {
...
Value *SCEVExpander::fixupLCSSAFormFor(Value *V) {
  ...
  toolchain::scope_exit RemoveUserOnExit([User]() { User->eraseFromParent(); });
...
bool SCEVExpander::isSafeToExpandAt(const SCEV *S,
  ...
  if (SE.dominates(S, InsertionPoint->getParent())) {
    ...
    if (const SCEVUnknown *U = dyn_cast<SCEVUnknown>(S))
      if (toolchain::is_contained(InsertionPoint->operand_values(), U->getValue()))
        return true;
  }
  return false;
}
...
#ifndef NDEBUG
  SmallPtrSet<Instruction *, 8> InsertedSet(toolchain::from_range,
