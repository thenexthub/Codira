...
#include "vm/core/Analysis/IndirectCallVisitor.h"
#include "vm/core/Analysis/InstructionSimplify.h"
#include "vm/core/Analysis/MemoryProfileInfo.h"
...
static cl::opt<bool>
    UseNoAliasIntrinsic("use-noalias-intrinsic-during-inlining", cl::Hidden,
                        cl::init(true),
                        cl::desc("Use the toolchain.experimental.noalias.scope.decl "
...
static BasicBlock *HandleCallsInBlockInlinedThroughInvoke(
  ...
  for (Instruction &I : toolchain::make_early_inc_range(*BB)) {
    ...
    // to @toolchain.experimental_deoptimize (resp. @toolchain.experimental.guard) into
    // invokes.  The caller's "segment" of the deoptimization continuation
    // attached to the newly inlined @toolchain.experimental_deoptimize
    // (resp. @toolchain.experimental.guard) call should contain the exception
...
/// When inlining a call site that has !toolchain.mem.parallel_loop_access,
/// !toolchain.access.group, !alias.scope or !noalias metadata, that metadata should
...
static void PropagateOperandBundles(Function::iterator InlinedBB,
  ...
  for (Instruction &II : toolchain::make_early_inc_range(*InlinedBB)) {
...
static void AddAliasScopeMetadata(CallBase &CB, ValueToValueMapTy &VMap,
  ...
  for (unsigned i = 0, e = NoAliasArgs.size(); i != e; ++i) {
    ...
    if (UseNoAliasIntrinsic) {
      // Introduce a toolchain.experimental.noalias.scope.decl for the noalias
      // argument.
      ...
      // toolchain.noalias intrinsic is introduced.
      (void)NoAliasDecl;
    }
  }
...
static bool MayContainThrowingOrExitingCallAfterCB(CallBase *Begin,
  ...
  return !toolchain::isGuaranteedToTransferExecutionToSuccessor(
    ...
    for (Instruction &Ins : BB) {
      ...
      for (unsigned I = 0, E = InnerCB->arg_size(); I < E; ++I) {
        ...
        if (match(NewInnerCB->getArgOperand(I),
                  toolchain::PatternMatch::m_ImmConstant()))
          continue;
...
/// add @toolchain.assume-based alignment assumptions to preserve this information.
...
void toolchain::updateProfileCallee(
...
inlineRetainOrClaimRVCalls(CallBase &CB, objcarc::ARCInstKind RVCallKind,
  ...
  for (auto *RI : Returns) {
    ...
    auto InstRange = toolchain::make_range(++(RI->getIterator().getReverse()),
                                      RI->getParent()->rend());
    for (Instruction &I : toolchain::make_early_inc_range(InstRange)) {
      // Ignore casts.
      if (isa<CastInst>(I))
        continue;
...
toolchain::InlineResult toolchain::InlineFunction(
  ...
  auto Updater = [&](PGOCtxProfContext &Ctx) {
    ...
    assert(
        (Ctx.counters().size() +
             toolchain::count_if(CalleeCounterMap, [](auto V) { return V != -1; }) ==
         NewCountersSize) &&
...
toolchain::InlineResult toolchain::CanInlineCallSite(const CallBase &CB,
                                           InlineFunctionInfo &IFI) {
...
void toolchain::InlineFunctionImpl(CallBase &CB, InlineFunctionInfo &IFI,
    ...
    if (ParentDeopt) {
      ...
      for (auto &VH : InlinedFunctionInfo.OperandBundleCallSites) {
        ...
        for (unsigned COBi = 0, COBe = ICS->getNumOperandBundles(); COBi < COBe;
          ...
          toolchain::append_range(MergedDeoptArgs, ParentDeopt->Inputs);
          toolchain::append_range(MergedDeoptArgs, ChildOB.Inputs);
  ...
  if ((InsertLifetime || Caller->isPresplitCoroutine()) &&
    ...
    for (AllocaInst *AI : IFI.StaticAllocas) {
      ...
      for (ReturnInst *RI : Returns) {
        // Don't insert toolchain.lifetime.end calls between a musttail or deoptimize
  ...
  // code with toolchain.stacksave/toolchain.stackrestore intrinsics.
  if (InlinedFunctionInfo.ContainsDynamicAllocas) {
    // Insert the toolchain.stacksave.
    CallInst *SavedPtr = IRBuilder<>(&*FirstNewBlock, FirstNewBlock->begin())
                             .CreateStackSave("savedstack");

    // Insert a call to toolchain.stackrestore before any return instructions in the
    // inlined function.
    for (ReturnInst *RI : Returns) {
      // Don't insert toolchain.stackrestore calls between a musttail or deoptimize
  ...
  if (InlinedDeoptimizeCalls) {
    ...
    if (Caller->getReturnType() == CB.getType()) {
      toolchain::erase_if(Returns, [](ReturnInst *RI) {
      ...
      for (ReturnInst *RI : Returns) {
        ...
        // @toolchain.experimental.deoptimize declarations have to have the same
...
toolchain::InlineResult toolchain::InlineFunction(CallBase &CB, InlineFunctionInfo &IFI,
                                        bool MergeAttributes,
                                        AAResults *CalleeAAR,
                                        bool InsertLifetime,
                                        Function *ForwardVarArgsTo,
                                        OptimizationRemarkEmitter *ORE) {
  toolchain::InlineResult Result = CanInlineCallSite(CB, IFI);
  if (Result.isSuccess()) {
