...
void toolchain::mapAtomInstance(const DebugLoc &DL, ValueToValueMapTy &VMap) {
...
BasicBlock *toolchain::CloneBasicBlock(const BasicBlock *BB, ValueToValueMapTy &VMap,
...
void toolchain::CloneFunctionAttributesInto(Function *NewFunc,
...
void toolchain::CloneFunctionMetadataInto(Function &NewFunc, const Function &OldFunc,
                                     ValueToValueMapTy &VMap,
...
void toolchain::CloneFunctionBodyInto(Function &NewFunc, const Function &OldFunc,
...
// VMap values.
void toolchain::CloneFunctionInto(Function *NewFunc, const Function *OldFunc,
                             ValueToValueMapTy &VMap,
  ...
  // Only update !toolchain.dbg.cu for DifferentModule (not CloneModule). In the
  // same module, the compile unit will already be listed (or not). When
  // cloning a module, CloneModule() will handle creating the named metadata.
  if (Changes != CloneFunctionChangeType::DifferentModule)
    return;

  // Update !toolchain.dbg.cu with compile units added to the new module if this
  ...
  // !toolchain.dbg.cu (either obsoleting the node, or restricting it to
  ...
  NamedMDNode *NMD = NewModule->getOrInsertNamedMetadata("toolchain.dbg.cu");
  // Avoid multiple insertions of the same DICompileUnit to NMD.
  SmallPtrSet<const void *, 8> Visited(toolchain::from_range, NMD->operands());
...
///
Function *toolchain::CloneFunction(Function *F, ValueToValueMapTy &VMap,
                              ClonedCodeInfo *CodeInfo) {
...
void toolchain::CloneAndPruneIntoFromInst(Function *NewFunc, const Function *OldFunc,
...
void toolchain::CloneAndPruneFunctionInto(
...
void toolchain::remapInstructionsInBlocks(ArrayRef<BasicBlock *> Blocks,
                                     ValueToValueMapTy &VMap) {
...
Loop *toolchain::cloneLoopWithPreheader(BasicBlock *Before, BasicBlock *LoopDomBB,
...
BasicBlock *toolchain::DuplicateInstructionsInSplitBetween(
...
void toolchain::cloneNoAliasScopes(ArrayRef<MDNode *> NoAliasDeclScopes,
...
void toolchain::adaptNoAliasScopes(Instruction *I,
...
void toolchain::cloneAndAdaptNoAliasScopes(ArrayRef<MDNode *> NoAliasDeclScopes,
...
void toolchain::cloneAndAdaptNoAliasScopes(ArrayRef<MDNode *> NoAliasDeclScopes,
  ...
  for (auto &I : toolchain::make_range(ItStart, ItEnd))
    adaptNoAliasScopes(&I, ClonedScopes, Context);
}

void toolchain::identifyNoAliasScopesToClone(
