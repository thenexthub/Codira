...
#include "vm/core/Transforms/Utils/CodeMoverUtils.h"
...
bool toolchain::isSafeToMoveBefore(Instruction &I, Instruction &InsertPoint,
  ...
  if (!isSafeToSpeculativelyExecute(&I))
    if (toolchain::any_of(InstsToCheck, [](Instruction *I) {
          if (I->mayThrow())
            return true;
  ...
  if (toolchain::any_of(InstsToCheck, [&DI, &I](Instruction *CurInst) {
...
bool toolchain::isSafeToMoveBefore(BasicBlock &BB, Instruction &InsertPoint,
                              DominatorTree &DT, const PostDominatorTree *PDT,
                              DependenceInfo *DI) {
  return toolchain::all_of(BB, [&](Instruction &I) {
...
void toolchain::moveInstructionsToTheBeginning(BasicBlock &FromBB, BasicBlock &ToBB,
                                          DominatorTree &DT,
                                          const PostDominatorTree &PDT,
                                          DependenceInfo &DI) {
  for (Instruction &I :
       toolchain::make_early_inc_range(toolchain::drop_begin(toolchain::reverse(FromBB)))) {
...
void toolchain::moveInstructionsToTheEnd(BasicBlock &FromBB, BasicBlock &ToBB,
                                    DominatorTree &DT,
                                    const PostDominatorTree &PDT,
                                    DependenceInfo &DI) {
...
bool toolchain::nonStrictlyPostDominate(const BasicBlock *ThisBlock,
...
bool toolchain::isReachedBefore(const Instruction *I0, const Instruction *I1,
                           const DominatorTree *DT,
                           const PostDominatorTree *PDT) {
