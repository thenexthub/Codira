...
#include "vm/core/Transforms/Utils/AMDGPUEmitPrintf.h"
...
Value *toolchain::emitAMDGPUPrintfCall(IRBuilder<> &Builder, ArrayRef<Value *> Args,
                                  bool IsBuffered) {
  ...
  if (IsBuffered) {
    ...
    NamedMDNode *metaD = M->getOrInsertNamedMetadata("toolchain.printf.fmts");
    if (IsConstFmtStr) {
      MD5 Hasher;
      MD5::MD5Result Hash;
      ...
      // Try sticking to toolchain.printf.fmts format, although we are not going to
      // use the ID and argument size fields while printing,
      std::string MetadataStr =
          "0:0:" + toolchain::utohexstr(Hash.low(), /*LowerCase=*/true) + "," +
          FmtStr.str();
