...
void FixPhis(BasicBlock *SuccBB, BasicBlock *OrigBB, BasicBlock *NewBB,
  ...
  for (auto &I : SuccBB->phis()) {
    ...
    // *successive* index.
    for (unsigned III : toolchain::reverse(Indices))
...
bool LowerSwitch(Function &F, LazyValueInfo *LVI, AssumptionCache *AC) {
  ...
  for (BasicBlock &Cur : toolchain::make_early_inc_range(F)) {
...
char &toolchain::LowerSwitchID = LowerSwitchLegacyPass::ID;
...
FunctionPass *toolchain::createLowerSwitchPass() {
