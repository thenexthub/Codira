...
CallBase &toolchain::versionCallSite(CallBase &CB, Value *Callee,
                                MDNode *BranchWeights) {
...
bool toolchain::isLegalToPromote(const CallBase &CB, Function *Callee,
...
CallBase &toolchain::promoteCall(CallBase &CB, Function *Callee,
                            CastInst **RetBitCast) {
  assert(!CB.getCalledFunction() && "Only indirect call sites can be promoted");
...
CallBase &toolchain::promoteCallWithIfThenElse(CallBase &CB, Function *Callee,
                                          MDNode *BranchWeights) {
...
CallBase &toolchain::promoteCallWithVTableCmp(CallBase &CB, Instruction *VPtr,
                                         Function *Callee,
...
bool toolchain::tryPromoteCall(CallBase &CB) {
