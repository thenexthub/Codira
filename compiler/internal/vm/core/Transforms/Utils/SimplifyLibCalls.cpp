...
static void annotateDereferenceableBytes(CallInst *CI,
  ...
  for (unsigned ArgNo : ArgNos) {
    ...
    if (!toolchain::NullPointerIsDefined(F, AS) ||
...
Value *LibCallSimplifier::optimizeStrStr(CallInst *CI, IRBuilderBase &B) {
  ...
  if (isOnlyUsedInEqualityComparison(CI, CI->getArgOperand(0))) {
    ...
    if (!StrNCmp)
      return nullptr;
    for (User *U : toolchain::make_early_inc_range(CI->users())) {
...
Value *LibCallSimplifier::optimizeMemChr(CallInst *CI, IRBuilderBase &B) {
  ...
  bool OptForSize = toolchain::shouldOptimizeForSize(CI->getParent(), PSI, BFI,
...
Value *LibCallSimplifier::optimizeMemCpy(CallInst *CI, IRBuilderBase &B) {
  ...
  // memcpy(x, y, n) -> toolchain.memcpy(align 1 x, align 1 y, n)
...
Value *LibCallSimplifier::optimizeMemPCpy(CallInst *CI, IRBuilderBase &B) {
  ...
  // mempcpy(x, y, n) -> toolchain.memcpy(align 1 x, align 1 y, n), x + n
  CallInst *NewCI =
...
Value *LibCallSimplifier::optimizeMemMove(CallInst *CI, IRBuilderBase &B) {
  ...
  // memmove(x, y, n) -> toolchain.memmove(align 1 x, align 1 y, n)
...
Value *LibCallSimplifier::optimizeMemSet(CallInst *CI, IRBuilderBase &B) {
  ...
  // memset(p, v, n) -> toolchain.memset(align 1 p, v, n)
...
Value *LibCallSimplifier::optimizeExp2(CallInst *CI, IRBuilderBase &B) {
  ...
  // If we have an toolchain.exp2 intrinsic, emit the toolchain.ldexp intrinsic. If we
...
Value *LibCallSimplifier::optimizeSqrt(CallInst *CI, IRBuilderBase &B) {
  ...
  // libcall) to tell whether our target can lower @toolchain.sqrt, relax the
  // condition below.
...
Value *LibCallSimplifier::optimizeTrigInversionPairs(CallInst *CI,
  ...
  if (F && TLI->getLibFunc(F->getName(), Func) &&
    ...
    LibFunc inverseFunc = toolchain::StringSwitch<LibFunc>(Callee->getName())
...
Value *LibCallSimplifier::optimizeFFS(CallInst *CI, IRBuilderBase &B) {
  ...
  // ffs{,l,ll}(x) -> x != 0 ? (int)toolchain.cttz(x)+1 : 0
...
Value *LibCallSimplifier::optimizeFls(CallInst *CI, IRBuilderBase &B) {
  ...
  // fls{,l,ll}(x) -> (int)(sizeInBits(x) - toolchain.ctlz(x, false))
  Value *Op = CI->getArgOperand(0);
  Type *ArgType = Op->getType();
...
Value *LibCallSimplifier::optimizeSPrintFString(CallInst *CI,
  ...
  if (CI->arg_size() == 2) {
    ...
    // sprintf(str, fmt) -> toolchain.memcpy(align 1 str, align 1 fmt, strlen(fmt)+1)
  ...
  if (FormatStr[1] == 's') {
    // sprintf(dest, "%s", str) -> toolchain.memcpy(align 1 dest, align 1 str,
    // strlen(str)+1)
...
Value *LibCallSimplifier::optimizeSnPrintFString(CallInst *CI,
  ...
  // snprintf(dest, size, "%s", str) to toolchain.memcpy(dest, str, len+1, 1)
  StringRef Str;
...
Value *LibCallSimplifier::optimizeBCopy(CallInst *CI, IRBuilderBase &B) {
  // bcopy(src, dst, n) -> toolchain.memmove(dst, src, n)
