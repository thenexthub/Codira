...
#include "vm/core/Transforms/Utils/SimplifyIndVar.h"
...
/// The function is just a helper function for toolchain::UnrollLoop that returns
...
const Loop* toolchain::addClonedBlockToLoopInfo(BasicBlock *OriginalBB,
...
void toolchain::simplifyLoopAfterUnroll(Loop *L, bool SimplifyIVs, LoopInfo *LI,
...
LoopUnrollResult
toolchain::UnrollLoop(Loop *L, UnrollLoopOptions ULO, LoopInfo *LI,
  ...
  std::optional<unsigned> OriginalTripCount =
      toolchain::getLoopEstimatedTripCount(L);
  BranchProbability OriginalLoopProb = toolchain::getLoopProbability(L);
  ...
  for (BasicBlock *Latch : Latches) {
    ...
    if (Term && Term->isUnconditional()) {
      ...
      if (MergeBlockIntoPredecessor(Dest, /*DTU=*/DTUToUse, LI,
        ...
        toolchain::replace(Latches, Dest, Fold);
        toolchain::erase(UnrolledLoopBlocks, Dest);
      }
    }
  }
...
/// Given an toolchain.loop loop id metadata node, returns the loop hint metadata
/// node with the given name (for example, "toolchain.loop.unroll.count"). If no
/// such metadata node exists, then nullptr is returned.
MDNode *toolchain::GetUnrollMetadata(MDNode *LoopID, StringRef Name) {
...
toolchain::canParallelizeReductionWhenUnrolling(PHINode &Phi, Loop *L,
                                           ScalarEvolution *SE) {
