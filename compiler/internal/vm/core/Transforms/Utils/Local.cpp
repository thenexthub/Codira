...
#include "vm/core/ADT/DenseMapInfo.h"
...
bool toolchain::ConstantFoldTerminator(BasicBlock *BB, bool DeleteDeadConditions,
...
///
bool toolchain::isInstructionTriviallyDead(Instruction *I,
                                      const TargetLibraryInfo *TLI) {
  if (!I->use_empty())
    return false;
...
bool toolchain::wouldInstructionBeTriviallyDeadOnUnusedPaths(
...
bool toolchain::wouldInstructionBeTriviallyDead(const Instruction *I,
                                           const TargetLibraryInfo *TLI) {
  if (I->isTerminator())
    return false;
  ...
  if (const IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
    // Safe to delete toolchain.stacksave and launder.invariant.group if dead.
    if (II->getIntrinsicID() == Intrinsic::stacksave ||
        II->getIntrinsicID() == Intrinsic::launder_invariant_group)
      return true;
    ...
    if (II->isLifetimeStartOrEnd()) {
      ...
      // intrinsics are dead.
      return toolchain::all_of(Arg->uses(), [](Use &Use) {
...
bool toolchain::RecursivelyDeleteTriviallyDeadInstructions(
...
bool toolchain::RecursivelyDeleteTriviallyDeadInstructionsPermissive(
...
void toolchain::RecursivelyDeleteTriviallyDeadInstructions(
    SmallVectorImpl<WeakTrackingVH> &DeadInsts, const TargetLibraryInfo *TLI,
    MemorySSAUpdater *MSSAU,
    std::function<void(Value *)> AboutToDeleteCallback) {
...
bool toolchain::replaceDbgUsesWithUndef(Instruction *I) {
...
bool toolchain::RecursivelyDeleteDeadPHINode(PHINode *PN,
                                        const TargetLibraryInfo *TLI,
                                        toolchain::MemorySSAUpdater *MSSAU) {
...
bool toolchain::SimplifyInstructionsInBlock(BasicBlock *BB,
                                       const TargetLibraryInfo *TLI) {
...
void toolchain::MergeBasicBlockIntoOnlyPred(BasicBlock *DestBB,
                                       DomTreeUpdater *DTU) {
...
bool toolchain::TryToSimplifyUncondBranchFromEmptyBlock(BasicBlock *BB,
                                                   DomTreeUpdater *DTU) {
  ...
  SmallPtrSet<BasicBlock *, 16> BBPreds(toolchain::from_range, predecessors(BB));
  ...
  if (DTU) {
    ...
    SmallPtrSet<BasicBlock *, 16> SuccPreds(toolchain::from_range,
                                            predecessors(Succ));
  ...
  // If the unconditional branch we replaced contains non-debug toolchain.loop
...
bool toolchain::EliminateDuplicatePHINodes(BasicBlock *BB,
                                      SmallPtrSetImpl<PHINode *> &ToRemove) {
  if (
#ifndef NDEBUG
      !PHICSEDebugHash &&
...
Align toolchain::tryEnforceAlignment(Value *V, Align PrefAlign,
                                const DataLayout &DL) {
...
Align toolchain::getOrEnforceKnownAlignment(Value *V, MaybeAlign PrefAlign,
                                       const DataLayout &DL,
...
void toolchain::ConvertDebugDeclareToDebugValue(DbgVariableRecord *DVR,
...
void toolchain::InsertDebugValueAtStoreLoc(DbgVariableRecord *DVR, StoreInst *SI,
                                      DIBuilder &Builder) {
  auto *DIVar = DVR->getVariable();
  assert(DIVar && "Missing variable");
  auto *DIExpr = DVR->getExpression();
...
void toolchain::ConvertDebugDeclareToDebugValue(DbgVariableRecord *DVR, LoadInst *LI,
                                           DIBuilder &Builder) {
  auto *DIVar = DVR->getVariable();
  auto *DIExpr = DVR->getExpression();
  assert(DIVar && "Missing variable");
...
void toolchain::ConvertDebugDeclareToDebugValue(DbgVariableRecord *DVR, PHINode *APN,
                                           DIBuilder &Builder) {
  auto *DIVar = DVR->getVariable();
  auto *DIExpr = DVR->getExpression();
  assert(DIVar && "Missing variable");
...
/// LowerDbgDeclare - Lowers toolchain.dbg.declare intrinsics into appropriate set
/// of toolchain.dbg.value intrinsics.
bool toolchain::LowerDbgDeclare(Function &F) {
  bool Changed = false;
  ...
  auto LowerOne = [&](DbgVariableRecord *DDI) {
    ...
    if (toolchain::any_of(AI->users(), [](User *U) -> bool {
...
void toolchain::insertDebugValuesForPHIs(BasicBlock *BB,
...
bool toolchain::replaceDbgDeclare(Value *Address, Value *NewAddress,
...
void toolchain::replaceDbgValueForAlloca(AllocaInst *AI, Value *NewAllocaAddress,
                                    DIBuilder &Builder, int Offset) {
  SmallVector<DbgVariableRecord *, 1> DPUsers;
...
void toolchain::salvageDebugInfo(Instruction &I) {
  SmallVector<DbgVariableRecord *, 1> DPUsers;
  findDbgUsers(&I, DPUsers);
...
void toolchain::salvageDebugInfoForDbgValues(Instruction &I,
...
Value *toolchain::salvageDebugInfoImpl(Instruction &I, uint64_t CurrentLocOps,
  ...
  if (auto *CI = dyn_cast<CastInst>(&I)) {
    ...
    toolchain::Type *FromType = FromValue->getType();
...
bool toolchain::replaceAllDbgUsesWith(Instruction &From, Value &To,
...
bool toolchain::handleUnreachableTerminator(
...
unsigned toolchain::removeAllNonTerminatorAndEHPadInstructions(BasicBlock *BB) {
...
unsigned toolchain::changeToUnreachable(Instruction *I, bool PreserveLCSSA,
                                   DomTreeUpdater *DTU,
                                   MemorySSAUpdater *MSSAU) {
...
CallInst *toolchain::createCallMatchingInvoke(InvokeInst *II) {
...
CallInst *toolchain::changeToCall(InvokeInst *II, DomTreeUpdater *DTU) {
...
BasicBlock *toolchain::changeToInvokeAndSplitBasicBlock(CallInst *CI,
        ...
        if (Function *F = dyn_cast<Function>(Callee)) {
          ...
          if (IntrinsicID == Intrinsic::assume) {
            if (match(CI->getArgOperand(0), m_CombineOr(m_Zero(), m_Undef()))) {
              // Don't insert a call to toolchain.trap right before the unreachable.
            ...
            // it is dead.
            //
            // Note: unlike in toolchain.assume, it is not "obviously profitable" for
...
Instruction *toolchain::removeUnwindEdge(BasicBlock *BB, DomTreeUpdater *DTU) {
...
/// otherwise.
bool toolchain::removeUnreachableBlocks(Function &F, DomTreeUpdater *DTU,
                                   MemorySSAUpdater *MSSAU) {
...
void toolchain::combineMetadataForCSE(Instruction *K, const Instruction *J,
                                 bool DoesKMove) {
  combineMetadata(K, J, DoesKMove);
}

void toolchain::combineAAMetadata(Instruction *K, const Instruction *J) {
  combineMetadata(K, J, /*DoesKMove=*/true, /*AAOnly=*/true);
}

void toolchain::copyMetadataForLoad(LoadInst &Dest, const LoadInst &Source) {
...
void toolchain::patchReplacementInstruction(Instruction *I, Value *Repl) {
  ...
  // When replacing the result of a toolchain.*.with.overflow intrinsic with a
...
static unsigned replaceDominatedUsesWith(Value *From, Value *To,
  ...
  unsigned Count = 0;
  for (Use &U : toolchain::make_early_inc_range(From->uses())) {
...
unsigned toolchain::replaceNonLocalUsesWith(Instruction *From, Value *To) {
   assert(From->getType() == To->getType());
...
unsigned toolchain::replaceDominatedUsesWith(Value *From, Value *To,
                                        DominatorTree &DT,
...
unsigned toolchain::replaceDominatedUsesWithIf(
...
bool toolchain::callsGCLeafFunction(const CallBase *Call,
                               const TargetLibraryInfo &TLI) {
...
void toolchain::copyNonnullMetadata(const LoadInst &OldLI, MDNode *N,
                               LoadInst &NewLI) {
...
void toolchain::copyRangeMetadata(const DataLayout &DL, const LoadInst &OldLI,
...
void toolchain::dropDebugUsers(Instruction &I) {
  SmallVector<DbgVariableRecord *, 1> DPUsers;
  findDbgUsers(&I, DPUsers);
  for (auto *DVR : DPUsers)
    DVR->eraseFromParent();
...
void toolchain::hoistAllInstructionsInto(BasicBlock *DomBlock, Instruction *InsertPt,
                                    BasicBlock *BB) {
  ...
  //
  // TODO: Extend toolchain.dbg.value to take more than one SSA Value (PR39141) to
...
DIExpression *toolchain::getExpressionForConstant(DIBuilder &DIB, const Constant &C,
                                             Type &Ty) {
...
void toolchain::remapDebugVariable(ValueToValueMapTy &Mapping, Instruction *Inst) {
...
bool toolchain::recognizeBSwapOrBitReverseIdiom(
...
// in CodeGen.
void toolchain::maybeMarkSanitizerLibraryCallNoBuiltin(
...
bool toolchain::canReplaceOperandWithVariable(const Instruction *I, unsigned OpIdx) {
...
Value *toolchain::invertCondition(Value *Condition) {
...
bool toolchain::inferAttributesFromOthers(Function &F) {
