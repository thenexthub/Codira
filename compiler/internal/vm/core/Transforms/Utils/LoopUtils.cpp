...
#include "vm/core/Analysis/InstSimplifyFolder.h"
...
static const char *LLVMLoopDisableNonforced = "toolchain.loop.disable_nonforced";
static const char *LLVMLoopDisableLICM = "toolchain.licm.disable";
namespace vm::core {
extern cl::opt<bool> ProfcheckDisableMetadataFixes;
} // namespace vm::core

bool toolchain::formDedicatedExitBlocks(Loop *L, DominatorTree *DT, LoopInfo *LI,
  ...
  auto RewriteExit = [&](BasicBlock *BB) {
    ...
    toolchain::scope_exit Cleanup([&] { InLoopPredecessors.clear(); });
...
SmallVector<Instruction *, 8> toolchain::findDefsUsedOutsideOfLoop(Loop *L) {
...
void toolchain::getLoopAnalysisUsage(AnalysisUsage &AU) {
...
void toolchain::initializeLoopPassPass(PassRegistry &Registry) {
...
/// different.
void toolchain::addStringMetadataToLoop(Loop *TheLoop, const char *StringMD,
                                   unsigned V) {
...
toolchain::getOptionalElementCountLoopAttribute(const Loop *TheLoop) {
  std::optional<int> Width =
      getOptionalIntLoopAttribute(TheLoop, "toolchain.loop.vectorize.width");

  if (Width) {
    std::optional<int> IsScalable = getOptionalIntLoopAttribute(
        TheLoop, "toolchain.loop.vectorize.scalable.enable");
...
std::optional<MDNode *> toolchain::makeFollowupLoopID(
  ...
  // No attributes is equivalent to having no !toolchain.loop metadata at all.
  if (MDs.size() == 1)
    return nullptr;
...
bool toolchain::hasDisableAllTransformsHint(const Loop *L) {
  return getBooleanLoopAttribute(L, LLVMLoopDisableNonforced);
}

bool toolchain::hasDisableLICMTransformsHint(const Loop *L) {
  return getBooleanLoopAttribute(L, LLVMLoopDisableLICM);
}

TransformationMode toolchain::hasUnrollTransformation(const Loop *L) {
  if (getBooleanLoopAttribute(L, "toolchain.loop.unroll.disable"))
    return TM_SuppressedByUser;

  std::optional<int> Count =
      getOptionalIntLoopAttribute(L, "toolchain.loop.unroll.count");
  if (Count)
    return *Count == 1 ? TM_SuppressedByUser : TM_ForcedByUser;

  if (getBooleanLoopAttribute(L, "toolchain.loop.unroll.enable"))
    return TM_ForcedByUser;

  if (getBooleanLoopAttribute(L, "toolchain.loop.unroll.full"))
    return TM_ForcedByUser;
...
TransformationMode toolchain::hasUnrollAndJamTransformation(const Loop *L) {
  if (getBooleanLoopAttribute(L, "toolchain.loop.unroll_and_jam.disable"))
    return TM_SuppressedByUser;

  std::optional<int> Count =
      getOptionalIntLoopAttribute(L, "toolchain.loop.unroll_and_jam.count");
  if (Count)
    return *Count == 1 ? TM_SuppressedByUser : TM_ForcedByUser;

  if (getBooleanLoopAttribute(L, "toolchain.loop.unroll_and_jam.enable"))
    return TM_ForcedByUser;
...
TransformationMode toolchain::hasVectorizeTransformation(const Loop *L) {
  std::optional<bool> Enable =
      getOptionalBoolLoopAttribute(L, "toolchain.loop.vectorize.enable");
  ...
  if (getBooleanLoopAttribute(L, "toolchain.loop.isvectorized"))
    return TM_Disable;
...
TransformationMode toolchain::hasDistributeTransformation(const Loop *L) {
  if (getBooleanLoopAttribute(L, "toolchain.loop.distribute.enable"))
    return TM_ForcedByUser;
...
TransformationMode toolchain::hasLICMVersioningTransformation(const Loop *L) {
  if (getBooleanLoopAttribute(L, "toolchain.loop.licm_versioning.disable"))
    return TM_SuppressedByUser;
...
SmallVector<BasicBlock *, 16> toolchain::collectChildrenInLoop(DominatorTree *DT,
                                                          DomTreeNode *N,
                                                          const Loop *CurLoop) {
...
bool toolchain::isAlmostDeadIV(PHINode *PN, BasicBlock *LatchBlock, Value *Cond) {
...
void toolchain::deleteDeadLoop(Loop *L, DominatorTree *DT, ScalarEvolution *SE,
  ...
  toolchain::SmallDenseSet<DebugVariable, 4> DeadDebugSet;
  toolchain::SmallVector<DbgVariableRecord *, 4> DeadDbgVariableRecords;
    ...
    for (Instruction &I : *Block) {
      ...
      if (ExitBlock) {
        ...
        for (DbgVariableRecord &DVR :
             toolchain::make_early_inc_range(filterDbgVars(I.getDbgRecordRange()))) {
  ...
  if (ExitBlock) {
    ...
    for (DbgVariableRecord *DVR : toolchain::reverse(DeadDbgVariableRecords))
  ...
  if (LI) {
    ...
    SmallPtrSet<BasicBlock *, 8> blocks(toolchain::from_range, L->blocks());
...
void toolchain::breakLoopBackedge(Loop *L, DominatorTree &DT, ScalarEvolution &SE,
                             LoopInfo &LI, MemorySSA *MSSA) {
...
static std::optional<unsigned> estimateLoopTripCount(Loop *L) {
  ...
  uint64_t ExitCount = toolchain::divideNearest(LoopWeight, ExitWeight);
...
toolchain::getLoopEstimatedTripCount(Loop *L,
  ...
  //
  // Some passes set toolchain.loop.estimated_trip_count to 0.  For example, after
  // peeling 10 or more iterations from a loop with an estimated trip count of
  // 10, toolchain.loop.estimated_trip_count becomes 0 on the remaining loop.  It
  ...
  // historically assume that toolchain::getLoopEstimatedTripCount always returns a
  // positive count or std::nullopt.  Thus, return std::nullopt when
  // toolchain.loop.estimated_trip_count is 0.
...
bool toolchain::setLoopEstimatedTripCount(
...
BranchProbability toolchain::getLoopProbability(Loop *L) {
  BranchInst *LatchBranch = getExpectedExitLoopLatchBranch(L);
  if (!LatchBranch)
...
bool toolchain::setLoopProbability(Loop *L, BranchProbability P) {
  BranchInst *LatchBranch = getExpectedExitLoopLatchBranch(L);
  if (!LatchBranch)
    return false;
  bool FirstTargetIsLoop = LatchBranch->getSuccessor(0) == L->getHeader();
...
BranchProbability toolchain::getBranchProbability(BranchInst *B,
                                             bool ForFirstTarget) {
...
bool toolchain::setBranchProbability(BranchInst *B, BranchProbability P,
                                bool ForFirstTarget) {
  if (B->getNumSuccessors() != 2)
    return false;
...
bool toolchain::hasIterationCountInvariantInParent(Loop *InnerLoop,
                                              ScalarEvolution &SE) {
...
constexpr Intrinsic::ID toolchain::getReductionIntrinsicID(RecurKind RK) {
  switch (RK) {
  default:
...
Intrinsic::ID toolchain::getMinMaxReductionIntrinsicID(Intrinsic::ID IID) {
  switch (IID) {
  default:
...
unsigned toolchain::getArithmeticReductionInstruction(Intrinsic::ID RdxID) {
  switch (RdxID) {
...
Intrinsic::ID toolchain::getReductionForBinop(Instruction::BinaryOps Opc) {
  switch (Opc) {
  default:
    break;
...
Intrinsic::ID toolchain::getMinMaxReductionIntrinsicOp(Intrinsic::ID RdxID) {
  switch (RdxID) {
  default:
...
Intrinsic::ID toolchain::getMinMaxReductionIntrinsicOp(RecurKind RK) {
  switch (RK) {
  default:
    llvm_unreachable("Unknown min/max recurrence kind");
  case RecurKind::UMin:
...
RecurKind toolchain::getMinMaxReductionRecurKind(Intrinsic::ID RdxID) {
  switch (RdxID) {
  case Intrinsic::vector_reduce_smax:
...
CmpInst::Predicate toolchain::getMinMaxReductionPredicate(RecurKind RK) {
  switch (RK) {
  default:
    llvm_unreachable("Unknown min/max recurrence kind");
  case RecurKind::UMin:
  ...
  // semantics of the intrinsics (toolchain.minimum/maximum).
  }
}

Value *toolchain::createMinMaxOp(IRBuilderBase &Builder, RecurKind RK, Value *Left,
                            Value *Right) {
...
Value *toolchain::getOrderedReduction(IRBuilderBase &Builder, Value *Acc, Value *Src,
...
Value *toolchain::getShuffleReduction(IRBuilderBase &Builder, Value *Src,
                                 unsigned Op,
...
Value *toolchain::createAnyOfReduction(IRBuilderBase &Builder, Value *Src,
...
Value *toolchain::getReductionIdentity(Intrinsic::ID RdxID, Type *Ty,
                                  FastMathFlags Flags) {
  bool Negative = false;
  switch (RdxID) {
  default:
...
Value *toolchain::getRecurrenceIdentity(RecurKind K, Type *Tp, FastMathFlags FMF) {
...
Value *toolchain::createSimpleReduction(IRBuilderBase &Builder, Value *Src,
                                   RecurKind RdxKind) {
...
Value *toolchain::createOrderedReduction(IRBuilderBase &B, RecurKind Kind,
...
Value *toolchain::createOrderedReduction(IRBuilderBase &Builder, RecurKind Kind,
...
void toolchain::propagateIRFlags(Value *I, ArrayRef<Value *> VL, Value *OpValue,
...
bool toolchain::isKnownNegativeInLoop(const SCEV *S, const Loop *L,
                                 ScalarEvolution &SE) {
...
bool toolchain::isKnownNonNegativeInLoop(const SCEV *S, const Loop *L,
                                    ScalarEvolution &SE) {
  const SCEV *Zero = SE.getZero(S->getType());
  return SE.isAvailableAtLoopEntry(S, L) &&
...
bool toolchain::isKnownPositiveInLoop(const SCEV *S, const Loop *L,
                                 ScalarEvolution &SE) {
  const SCEV *Zero = SE.getZero(S->getType());
  return SE.isAvailableAtLoopEntry(S, L) &&
...
bool toolchain::isKnownNonPositiveInLoop(const SCEV *S, const Loop *L,
                                    ScalarEvolution &SE) {
  const SCEV *Zero = SE.getZero(S->getType());
  return SE.isAvailableAtLoopEntry(S, L) &&
...
bool toolchain::cannotBeMinInLoop(const SCEV *S, const Loop *L, ScalarEvolution &SE,
                             bool Signed) {
...
bool toolchain::cannotBeMaxInLoop(const SCEV *S, const Loop *L, ScalarEvolution &SE,
                             bool Signed) {
  unsigned BitWidth = cast<IntegerType>(S->getType())->getBitWidth();
...
static bool canLoopBeDeleted(Loop *L, SmallVector<RewritePhi, 8> &RewritePhiSet) {
  ...
  for (auto *BB : L->blocks())
    if (toolchain::any_of(*BB, [](Instruction &I) {
...
int toolchain::rewriteLoopExitValues(Loop *L, LoopInfo *LI, TargetLibraryInfo *TLI,
                                ScalarEvolution *SE,
      ...
      for (unsigned i = 0; i != NumPreds; ++i) {
        ...
        if (ReplaceExitValue == UnusedIndVarInLoop) {
          ...
          if (IndPhi) {
            ...
            if (toolchain::any_of(Inst->users(), [&](User *U) {
...
void toolchain::appendReversedLoopsToWorklist(
...
template <typename RangeT>
void toolchain::appendLoopsToWorklist(RangeT &&Loops,
                                 SmallPriorityWorklist<Loop *, 4> &Worklist) {
  appendReversedLoopsToWorklist(reverse(Loops), Worklist);
}

template LLVM_EXPORT_TEMPLATE void
toolchain::appendLoopsToWorklist<ArrayRef<Loop *> &>(
    ArrayRef<Loop *> &Loops, SmallPriorityWorklist<Loop *, 4> &Worklist);

template LLVM_EXPORT_TEMPLATE void
toolchain::appendLoopsToWorklist<Loop &>(Loop &L,
                                    SmallPriorityWorklist<Loop *, 4> &Worklist);

void toolchain::appendLoopsToWorklist(LoopInfo &LI,
                                 SmallPriorityWorklist<Loop *, 4> &Worklist) {
  appendReversedLoopsToWorklist(LI, Worklist);
}

Loop *toolchain::cloneLoop(Loop *L, Loop *PL, ValueToValueMapTy &VM,
...
Value *toolchain::addRuntimeChecks(
...
Value *toolchain::addDiffRuntimeChecks(
...
toolchain::hasPartialIVCondition(const Loop &L, unsigned MSSAThreshold,
