...
bool canCoerceMustAliasedValueToLoad(Value *StoredVal, Type *LoadTy,
  ...
  // vectors forwarded to fixed-sized vectors @toolchain.vector.extract is used.
  ...
  if (toolchain::alignTo(MinStoreSize, 8) != MinStoreSize)
    return false;
...
Value *coerceAvailableValueToLoadType(Value *StoredVal, Type *LoadedTy,
  ...
  // a @toolchain.vector.extract instead of bitcasts.
...
static Value *getStoreValueForLoadHelper(Value *SrcVal, unsigned Offset,
  ...
  // only equal element types are allowed and a @toolchain.vector.extract will be
