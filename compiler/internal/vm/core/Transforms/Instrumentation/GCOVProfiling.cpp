...
namespace endian = toolchain::support::endian;
...
std::string GCOVProfiler::mangleName(const DICompileUnit *CU,
  ...
  if (NamedMDNode *GCov = M->getNamedMetadata("toolchain.gcov")) {
...
bool GCOVProfiler::runOnModule(
  ...
  NamedMDNode *CUNode = M.getNamedMetadata("toolchain.dbg.cu");
...
bool GCOVProfiler::emitProfileNotes(
  ...
  for (unsigned i = 0, e = CUNode->getNumOperands(); i != e; ++i) {
    ...
    Endian = M->getDataLayout().isLittleEndian() ? toolchain::endianness::little
    ...
    for (auto &F : M->functions()) {
      ...
      for (size_t I : toolchain::seq<size_t>(0, MST.numEdges())) {
      ...
      // as well.
      toolchain::erase_if(MST.allEdges(), [](std::unique_ptr<Edge> &E) {
      ...
      const size_t Measured =
          ...
          MST.allEdges().begin();
      for (size_t I : toolchain::seq<size_t>(0, Measured)) {
...
Function *GCOVProfiler::insertCounterWriteout(
  ...
  NamedMDNode *CUNodes = M->getNamedMetadata("toolchain.dbg.cu");
  if (!CUNodes) {
    Builder.CreateRetVoid();
    return WriteoutF;
  }
  ...
  for (int i : toolchain::seq<int>(0, CUNodes->getNumOperands())) {
    ...
    for (int j : toolchain::seq<int>(0, CountersBySP.size())) {
