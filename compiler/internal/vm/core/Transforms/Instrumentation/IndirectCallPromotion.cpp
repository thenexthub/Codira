...
// but not vice versa. See also https://toolchain.org/docs/TypeMetadata.html
...
static Constant *getVTableAddressPointOffset(GlobalVariable *VTable,
  ...
  return ConstantExpr::getInBoundsGetElementPtr(
      ...
      toolchain::ConstantInt::get(Type::getInt32Ty(Context), AddressPointOffset));
}
...
static int tryToSinkInstructions(BasicBlock *OriginalBB,
  ...
  for (Instruction &I :
       toolchain::make_early_inc_range(toolchain::drop_begin(toolchain::reverse(*OriginalBB))))
...
Instruction *IndirectCallPromoter::computeVTableInfos(
  ...
  //   %0 = tail call i1 @toolchain.type.test(ptr %vptr, metadata !"vtable1")
  //   tail call void @toolchain.assume(i1 %0)
...
CallBase &toolchain::pgo::promoteIndirectCall(CallBase &CB, Function *DirectCallee,
...
void IndirectCallPromoter::updateFuncValueProfiles(
  ...
  toolchain::stable_sort(CallVDs, [](const InstrProfValueData &LHS,
  ...
  ArrayRef<InstrProfValueData> VDs(
      CallVDs.begin(),
      toolchain::upper_bound(CallVDs, 0U,
...
void IndirectCallPromoter::updateVPtrValueProfiles(
  ...
  toolchain::sort(VTableValueProfiles,
...
bool IndirectCallPromoter::tryToPromoteWithVTableCmp(
  ...
  for (const auto &Candidate : Candidates) {
    ...
    // counts, see
    // https://github.com/toolchain/toolchain-project/blob/abedb3b8356d5d56f1c575c4f7682fba2cb19787/toolchain/lib/ProfileData/InstrProf.cpp#L1281-L1288
...
// in the vtable. With type intrinsics (toolchain.type.test), we can find virtual
...
computeVirtualCallSiteTypeInfoMap(Module &M, ModuleAnalysisManager &MAM,
  ...
  // Right now only toolchain.type.test is used to find out virtual call sites.
  // With ThinLTO and whole-program-devirtualization, toolchain.type.test and
  // toolchain.public.type.test are emitted, and toolchain.public.type.test is either
  // refined to toolchain.type.test or dropped before indirect-call-promotion pass.
  //
  // FIXME: For fullLTO with VFE, `toolchain.type.checked.load intrinsic` is emitted.
  // Find out virtual calls by looking at users of toolchain.type.checked.load in
  // that case.
  ...
  for (Use &U : toolchain::make_early_inc_range(TypeTestFunc->uses())) {
    ...
    // Find out all devirtualizable call sites given a toolchain.type.test
    // intrinsic call.
