...
#include "vm/core/ProfileData/CtxInstrContextNode.h"
...
// toolchain.instrprof.increment[.step] captures the total number of counters as one
// of its parameters, and toolchain.instrprof.callsite captures the total number of
    ...
    for (const auto &I : BB) {
      if (const auto *Incr = dyn_cast<InstrProfIncrementInst>(&I)) {
        ...
        assert((!NumCounters || V == NumCounters) &&
               "expected all toolchain.instrprof.increment[.step] intrinsics to "
        ...
        assert((!NumCallsites || V == NumCallsites) &&
               "expected all toolchain.instrprof.callsite intrinsics to have the "
...
#undef _MUTEXDECL
  ...
  CallsiteInfoTLS->setVisibility(toolchain::GlobalValue::HiddenVisibility);
  ExpectedCalleeTLS =
      new GlobalVariable(M, PointerTy, false, GlobalValue::ExternalLinkage,
                         nullptr, CompilerRtAPINames::ExpectedCalleeTLS);
  ExpectedCalleeTLS->setThreadLocal(true);
  ExpectedCalleeTLS->setVisibility(toolchain::GlobalValue::HiddenVisibility);
}
  ...
  for (auto &BB : F) {
    for (auto &I : toolchain::make_early_inc_range(BB)) {
      if (auto *Instr = dyn_cast<InstrProfCntrInstBase>(&I)) {
        ...
        case toolchain::Intrinsic::instrprof_increment:
        case toolchain::Intrinsic::instrprof_increment_step: {
        ...
        case toolchain::Intrinsic::instrprof_callsite:
