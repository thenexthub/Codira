  ...
  PGOCounterPromoter(
    ...
    for (BasicBlock *ExitBlock : LoopExitBlocks) {
      if (BlockSet.insert(ExitBlock).second &&
          toolchain::none_of(predecessors(ExitBlock), [&](const BasicBlock *Pred) {
            return toolchain::isPresplitCoroSuspendExitEdge(*Pred, *ExitBlock);
          })) {
...
private:
  ...
  isPromotionPossible(Loop *LP,
    ...
    if (toolchain::any_of(LoopExitBlocks, [](BasicBlock *Exit) {
...
bool InstrLowerer::lowerIntrinsics(Function *F) {
  ...
  for (BasicBlock &BB : *F) {
    for (Instruction &Instr : toolchain::make_early_inc_range(BB)) {
...
void InstrLowerer::promoteCounterLoadStores(Function *F) {
  ...
  for (auto *Loop : toolchain::reverse(Loops)) {
...
void InstrLowerer::lowerValueProfileInst(InstrProfValueProfileInst *Ind) {
  ...
  bool IsMemOpSize = (Ind->getValueKind()->getZExtValue() ==
                      toolchain::InstrProfValueKind::IPVK_MemOPSize);
...
void InstrLowerer::lowerCover(InstrProfCoverInst *CoverInstruction) {
  ...
  if (ConditionalCounterUpdate) {
    ...
    auto *Int8Ty = toolchain::Type::getInt8Ty(Ctx);
...
void InstrLowerer::getOrCreateVTableProfData(GlobalVariable *GV) {
  ...
  // Skip toolchain internal global variable or __prof variables.
...
#include "vm/core/ProfileData/InstrProfData.inc"
  ...
  // @toolchain.instrprof.value.profile, NS will be 0), and the counter keeps the
...
void InstrLowerer::emitUses() {
  ...
  // retained or discarded as a unit, so toolchain.compiler.used is sufficient.
  ...
  // VNodesVar, so we have to be conservative and place them in toolchain.used
