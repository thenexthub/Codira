...
#include "vm/core/Transforms/Utils/LongestCommonSequence.h"
#include <map>
#include <set>
...
stackFrameIncludesInlinedCallStack(ArrayRef<Frame> ProfileCallStack,
  ...
  return ProfileCallStack.size() >= InlinedCallStack.size() &&
         toolchain::equal(ProfileCallStack.take_front(InlinedCallStack.size()),
...
static void HandleUnsupportedAnnotationKinds(GlobalVariable &GVar,
                                             AnnotationKind Kind) {
  assert(Kind != toolchain::memprof::AnnotationKind::AnnotationOK &&
         "Should not handle AnnotationOK here");
  SmallString<32> Reason;
  switch (Kind) {
  case toolchain::memprof::AnnotationKind::ExplicitSection:
    ++NumOfMemProfExplicitSectionGlobalVars;
    Reason.append("explicit section name");
    break;
  case toolchain::memprof::AnnotationKind::DeclForLinker:
    Reason.append("linker declaration");
    break;
  case toolchain::memprof::AnnotationKind::ReservedName:
    Reason.append("name starts with `toolchain.`");
    break;
  default:
...
readMemprof(Module &M, Function &F, IndexedInstrProfReader *MemProfReader,
  ...
  // toolchain-profdata uses FuncName in dwarf to create GUID which doesn't
...
bool MemProfUsePass::annotateGlobalVariables(
  ...
  for (GlobalVariable &GVar : M.globals()) {
    ...
    auto Kind = toolchain::memprof::getAnnotationKind(GVar);
    if (Kind != toolchain::memprof::AnnotationKind::AnnotationOK) {
    ...
    // For logging, optimization remark emitter requires a toolchain::Function, but
