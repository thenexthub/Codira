...
/// http://clang.toolchain.org/docs/DataFlowSanitizerDesign.html
//
//===----------------------------------------------------------------------===//
...
#include "vm/core/Support/SpecialCaseList.h"
...
transformFunctionAttributes(const TransformedFunction &TransformedFunction,
  ...
  std::vector<toolchain::AttributeSet> ArgumentAttributes(
  ...
  return AttributeList::get(Ctx, CallSiteAttrs.getFnAttrs(),
                            ...
                            toolchain::ArrayRef(ArgumentAttributes));
}
...
public:
  ...
  bool runImpl(Module &M,
               toolchain::function_ref<TargetLibraryInfo &(Function &)> GetTLI);
};
...
DataFlowSanitizer::DataFlowSanitizer(
  ...
  toolchain::append_range(AllABIListFiles, ClABIListFiles);
...
void DFSanFunction::addReachesFunctionCallbacksIfEnabled(IRBuilder<> &IRB,
  ...
  ConstantInt *CILine;
  toolchain::Value *FilePathPtr;

  if (dbgloc.get() == nullptr) {
    CILine = toolchain::ConstantInt::get(I.getContext(), toolchain::APInt(32, 0));
    FilePathPtr = IRB.CreateGlobalString(
        I.getFunction()->getParent()->getSourceFileName());
  } else {
    CILine = toolchain::ConstantInt::get(I.getContext(),
                                    toolchain::APInt(32, dbgloc.getLine()));
    FilePathPtr = IRB.CreateGlobalString(dbgloc->getFilename());
  }

  toolchain::Value *FunctionNamePtr =
...
bool DataFlowSanitizer::runImpl(
    Module &M, toolchain::function_ref<TargetLibraryInfo &(Function &)> GetTLI) {
  initializeModule(M);
  ...
  if (ClIgnorePersonalityRoutine) {
    for (auto *C : PersonalityFns) {
      ...
      if (!isInstrumented(F))
        toolchain::erase(FnsToInstrument, F);
    }
  }
...
DataFlowSanitizer::getShadowOriginAddress(Value *Addr, Align InstAlignment,
  ...
  if (shouldTrackOrigins()) {
    ...
    const Align Alignment = toolchain::assumeAligned(InstAlignment.value());
...
Value *DFSanFunction::combineShadows(Value *V1, Value *V2,
  ...
  if (V1Elems != ShadowElements.end() && V2Elems != ShadowElements.end()) {
    if (toolchain::includes(V1Elems->second, V2Elems->second)) {
      return collapseToPrimitiveShadow(V1, Pos);
    }
    if (toolchain::includes(V2Elems->second, V1Elems->second)) {
      return collapseToPrimitiveShadow(V2, Pos);
    }
...
void DFSanVisitor::visitCallBase(CallBase &CB) {
  ...
  if (DFSF.TLI.getLibFunc(CB, LF)) {
    ...
    case LibFunc_atomic_load:
      if (!isa<CallInst>(CB)) {
        toolchain::errs() << "DFSAN -- cannot instrument invoke of libatomic load. "
                        "Ignoring!\n";
        break;
      }
