...
void ThreadSanitizer::chooseInstructionsToInstrument(
  ...
  for (Instruction *I : reverse(Local)) {
    ...
    if (AI && !PointerMayBeCaptured(AI, /*ReturnCaptures=*/true) &&
      ...
      // (see toolchain/Analysis/CaptureTracking.h for details).
...
// Both toolchain and ThreadSanitizer atomic operations are based on C++11/C1x
...
int ThreadSanitizer::getMemoryAccessFuncIndex(Type *OrigTy, Value *Addr,
  ...
  size_t Idx = toolchain::countr_zero(TypeSize / 8);
