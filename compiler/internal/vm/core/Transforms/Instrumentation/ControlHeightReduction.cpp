...
class CHRScope {
 public:
  ...
  CHRScope *split(Region *Boundary) {
    ...
    auto BoundaryIt = toolchain::find_if(
...
CHRScope * CHR::findScope(Region *R) {
  ...
  for (BasicBlock *BB : R->blocks()) {
    if (BB->hasAddressTaken())
      return nullptr;
    // If we encounter toolchain.coro.id, skip this region because if the basic block
    // is cloned, we end up inserting a token type PHI node to the block with
    // toolchain.coro.begin.
...
void CHR::checkScopeHoistable(CHRScope *Scope) {
  ...
  if (RI.HasBranch || !Selects.empty()) {
    ...
    DenseSet<Instruction *> Unhoistables(toolchain::from_range, Selects);
    ...
    if (RI.HasBranch && InsertPoint != Branch) {
      ...
      if (!IsHoistable) {
        ...
        toolchain::erase_if(Selects, [EntryBB](SelectInst *SI) {
  ...
  for (size_t I = 0; I < Splits.size(); ++I) {
    ...
    for (CHRScope *Sub : Split->Subs) {
      SmallVector<CHRScope *, 8> SubSplits = splitScope(
          ...
          SplitUnhoistables);
      toolchain::append_range(NewSubs, SubSplits);
    }
    Split->Subs = NewSubs;
  }
...
void CHR::sortScopes(SmallVectorImpl<CHRScope *> &Input,
  ...
  toolchain::copy(Input, Output.begin());
  toolchain::stable_sort(Output, CHRScopeSorter);
}
