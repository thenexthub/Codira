...
#include "vm/core/Transforms/Instrumentation/AddressSanitizerOptions.h"
...
const char kAMDGPUAddressSharedName[] = "toolchain.amdgcn.is.shared";
const char kAMDGPUAddressPrivateName[] = "toolchain.amdgcn.is.private";
const char kAMDGPUBallotName[] = "toolchain.amdgcn.ballot.i64";
const char kAMDGPUUnreachableName[] = "toolchain.amdgcn.unreachable";
...
// but due to http://toolchain.org/bugs/show_bug.cgi?id=12652 we temporary
// set it to 10000.
...
void toolchain::getAddressSanitizerParams(const Triple &TargetTriple, int LongSize,
...
void toolchain::removeASanIncompatibleFnAttributes(Function &F, bool ReadsArgMem) {
...
struct FunctionStackPoisoner : public InstVisitor<FunctionStackPoisoner> {
  ...
  void unpoisonDynamicAllocasBeforeInst(Instruction *InstBefore,
    ...
    // When we insert _asan_allocas_unpoison before @toolchain.stackrestore, we
    // need to adjust extracted SP to compute the address of the most recent
    // alloca. We have a special @toolchain.get.dynamic.area.offset intrinsic for
    // this purpose.
  ...
  void visitIntrinsicInst(IntrinsicInst &II) {
    ...
    if (!II.isLifetimeStartOrEnd())
      return;
    // Find alloca instruction that corresponds to toolchain.lifetime argument.
...
static size_t TypeStoreSizeToSizeIndex(uint32_t TypeSize) {
  size_t Res = toolchain::countr_zero(TypeSize / 8);
...
static bool GlobalWasGeneratedByCompiler(GlobalVariable *G) {
  // Do not instrument @toolchain.global_ctors, @toolchain.used, etc.
  if (G->getName().starts_with("toolchain.") ||
...
bool ModuleAddressSanitizer::shouldInstrumentGlobal(GlobalVariable *G) const {
  ...
  if (G->hasSection()) {
    ...
    // Globals from toolchain.metadata aren't emitted, do not instrument them.
    if (Section == "toolchain.metadata") return false;
    ...
    if (TargetTriple.isOSBinFormatELF()) {
      if (toolchain::all_of(Section,
                       [](char c) { return toolchain::isAlnum(c) || c == '_'; }))
        return false;
    }
...
void ModuleAddressSanitizer::InstrumentGlobalsCOFF(
  ...
  // Update toolchain.compiler.used, adding the new metadata globals. This is
...
void ModuleAddressSanitizer::InstrumentGlobalsMachO(
  ...
  // created to be added to toolchain.compiler.used
  ...
  // Update toolchain.compiler.used, adding the new liveness globals. This is
...
// redzones and inserts this function into toolchain.global_ctors.
...
void ModuleAddressSanitizer::instrumentGlobals(IRBuilder<> &IRB,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    GlobalVariable *Name =
        createPrivateGlobalForString(M, toolchain::demangle(NameForGlobal),
  ...
  // Add instrumented globals to toolchain.compiler.used list to avoid LTO from
...
void AddressSanitizer::markEscapedLocalAllocas(Function &F) {
  // Find the one possible call to toolchain.localescape and pre-mark allocas passed
  ...
  // Try to get the declaration of toolchain.localescape. If it's not in the module,
  // we can exit early.
  if (!F.getParent()->getFunction("toolchain.localescape")) return;

  // Look for a call to toolchain.localescape call in the entry block. It can't be in
  // any other block.
...
bool AddressSanitizer::instrumentFunction(Function &F,
  ...
  // We can't instrument allocas used with toolchain.localescape. Only static allocas
...
void FunctionStackPoisoner::processStaticAllocas() {
  ...
  // If we have a call to toolchain.localescape, keep it in the entry block.
  if (LocalEscapeCall)
...
// Handling toolchain.lifetime intrinsics for a given %alloca:
// (1) collect all toolchain.lifetime.xxx(%size, %value) describing the alloca.
// (2) if %size is constant, poison memory for toolchain.lifetime.end (to detect
//     invalid accesses) and unpoison it for toolchain.lifetime.start (the memory
//     could be poisoned by previous toolchain.lifetime.end instruction, as the
