...
#include "vm/core/Transforms/Utils/MemoryTaggingSupport.h"
...
bool HWAddressSanitizer::instrumentMemAccess(InterestingMemoryOperand &O,
  ...
  toolchain::KnownBits Known(DL.getPointerTypeSizeInBits(Addr->getType()));
  toolchain::computeKnownBits(Addr, Known, DL);
  if (Known.isZero())
    return false;
...
void HWAddressSanitizer::tagAlloca(IRBuilder<> &IRB, AllocaInst *AI, Value *Tag,
  ...
  if (InstrumentWithCalls) {
    ...
    // toolchain.memset right here into either a sequence of stores, or a call to
    // hwasan_tag_memory.
    if (ShadowSize)
...
void HWAddressSanitizer::sanitizeFunction(Function &F,
  ...
  if (EntryIRB.GetInsertBlock() != &F.getEntryBlock()) {
    ...
    for (Instruction &I :
         toolchain::make_early_inc_range(*EntryIRB.GetInsertBlock())) {
...
void HWAddressSanitizer::instrumentGlobals() {
  ...
  for (GlobalVariable &GV : M.globals()) {
    ...
    if (GV.isDeclarationForLinker() || GV.getName().starts_with("toolchain.") ||
        GV.isThreadLocal())
      continue;
