...
static bool isFullDominator(const BasicBlock *BB, const DominatorTree &DT) {
  ...
  return toolchain::all_of(successors(BB), [&](const BasicBlock *SUCC) {
...
static bool isFullPostDominator(const BasicBlock *BB,
  ...
  return toolchain::all_of(predecessors(BB), [&](const BasicBlock *PRED) {
...
GlobalVariable *ModuleSanitizerCoverage::CreateFunctionLocalArrayInSection(
  ...
  // will be retained or discarded as a unit, so toolchain.compiler.used is
...
void ModuleSanitizerCoverage::InjectTraceForSwitch(
  ...
  for (auto *I : SwitchTraceTargets) {
    if (SwitchInst *SI = dyn_cast<SwitchInst>(I)) {
      ...
      toolchain::sort(drop_begin(Initializers, 2),
...
void ModuleSanitizerCoverage::InjectCoverageAtBlock(Function &F, BasicBlock &BB,
  ...
  if (IsEntryBB) {
    ...
    // Keep static allocas and toolchain.localescape calls in the entry block.  Even
  ...
  if (Options.StackDepth && IsEntryBB && !IsLeafFunc) {
    ...
    if (Options.StackDepthCallbackMin) {
      ...
      toolchain::Instruction *InsertBefore = nullptr;
