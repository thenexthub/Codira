...
#include "vm/core/Support/DOTGraphTraits.h"
...
#include "vm/core/Transforms/Instrumentation/BlockCoverageInference.h"
#include "vm/core/Transforms/Instrumentation/CFGMST.h"
...
void FunctionInstrumenter::instrument() {
  ...
  if (PGOFunctionEntryCoverage) {
    ...
    // toolchain.instrprof.cover(i8* <name>, i64 <hash>, i32 <num-counters>,
    //                      i32 <index>)
  ...
  if (IsCtxProf) {
    StringSet<> SkipCSInstr(toolchain::from_range, CtxPGOSkipCallsiteInstrument);
    ...
    // is because the toolchain.instrprof.callsite intrinsic has an argument (like
    ...
    auto Visit = [&](toolchain::function_ref<void(CallBase * CB)> Visitor) {
      for (auto &BB : F)
        for (auto &Instr : BB)
  ...
  if (PGOTemporalInstrumentation) {
    ...
    auto &EntryBB = F.getEntryBlock();
    IRBuilder<> Builder(&EntryBB, EntryBB.getFirstNonPHIOrDbgOrAlloca());
    // toolchain.instrprof.timestamp(i8* <name>, i64 <hash>, i32 <num-counters>,
    //                          i32 <index>)
  ...
  for (auto *InstrBB : InstrumentBBs) {
    ...
    // toolchain.instrprof.increment(i8* <name>, i64 <hash>, i32 <num-counters>,
    //                          i32 <index>)
...
void PGOUseFunc::populateCoverage() {
  ...
  for (auto &BB : F) {
    ...
    if (Weights.size() >= 2)
      toolchain::setBranchWeights(*BB.getTerminator(), Weights,
                             /*IsExpected=*/false);
  }
...
static uint32_t getMaxNumAnnotations(InstrProfValueKind ValueProfKind) {
  ...
  if (ValueProfKind == toolchain::IPVK_VTableTarget)
...
static bool skipPGOGen(const Function &F) {
  if (skipPGOUse(F))
    return true;
  if (F.hasFnAttribute(toolchain::Attribute::Naked))
    return true;
  if (F.hasFnAttribute(toolchain::Attribute::NoProfile))
    return true;
  if (F.hasFnAttribute(toolchain::Attribute::SkipProfile))
    return true;
...
void toolchain::setProfMetadata(Instruction *TI, ArrayRef<uint64_t> EdgeCounts,
                           uint64_t MaxCount) {
...
void setIrrLoopHeaderMetadata(Module *M, Instruction *TI, uint64_t Count) {
  ...
  TI->setMetadata(toolchain::LLVMContext::MD_irr_loop,
