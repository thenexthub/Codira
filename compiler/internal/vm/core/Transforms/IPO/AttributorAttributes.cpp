...
template <>
unsigned toolchain::PotentialConstantIntValuesState::MaxPotentialValues = 0;

template <> unsigned toolchain::PotentialLLVMValuesState::MaxPotentialValues = -1;
...
static cl::opt<unsigned, true> MaxPotentialValues(
    ...
    cl::location(toolchain::PotentialConstantIntValuesState::MaxPotentialValues),
    cl::init(7));
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto CalleePred = [&](ArrayRef<const Function *> Callees) {
      for (const Function *Callee : Callees) {
        IRPosition FnPos =
            IRPKind == toolchain::IRPosition::IRP_CALL_SITE_RETURNED
  ...
  for (unsigned u = 0; u < Uses.size(); ++u) {
    ...
    if (const Instruction *UserI = dyn_cast<Instruction>(U->getUser())) {
      ...
      if (Found && AA.followUseInMBEC(A, U, UserI, State))
        Uses.insert_range(toolchain::make_pointer_range(UserI->uses()));
    }
  }
}
...
static raw_ostream &operator<<(raw_ostream &OS,
  ...
  OS << toolchain::interleaved_array(OI);
  return OS;
}
#endif // NDEBUG
...
bool AANonNull::isImpliedByIR(Attributor &A, const IRPosition &IRP,
  ...
  if (toolchain::any_of(Worklist, [&](AA::ValueAndContext VAC) {
...
static int64_t getKnownNonNullAndDerefBytesForUse(
  ...
  bool NullPointerIsDefined =
      F ? toolchain::NullPointerIsDefined(F, PtrTy->getPointerAddressSpace()) : true;
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (!Stripped) {
      ...
      if (auto *PHI = dyn_cast<PHINode>(AssociatedValue))
        if (toolchain::all_of(PHI->incoming_values(), [&](Value *Op) {
              return AA::hasAssumedIRAttr<Attribute::NonNull>(
...
struct AAUndefinedBehaviorImpl : public AAUndefinedBehavior {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto InspectMemAccessInstForUB = [&](Instruction &I) {
      ...
      if (toolchain::NullPointerIsDefined(F, PtrTy->getPointerAddressSpace()))
        AssumedNoUBInsts.insert(&I);
      else
...
struct AAIntraFnReachabilityFunction final
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    if (LivenessAA &&
        toolchain::all_of(DeadEdges,
                     [&](const auto &DeadEdge) {
                       return LivenessAA->isEdgeDead(DeadEdge.first,
                                                     DeadEdge.second);
                     }) &&
        toolchain::all_of(DeadBlocks, [&](const BasicBlock *BB) {
...
struct AAIsDeadFloating : public AAIsDeadValueImpl {
  ...
  bool isDeadStore(Attributor &A, StoreInst &SI,
    ...
    InformationCache &InfoCache = A.getInfoCache();
    return toolchain::all_of(PotentialCopies, [&](Value *V) {
      if (A.isAssumedDead(IRPosition::value(*V), this, nullptr,
                          UsedAssumedInformation))
        return true;
      if (auto *LI = dyn_cast<LoadInst>(V)) {
        if (toolchain::all_of(LI->uses(), [&](const Use &U) {
...
ChangeStatus AAIsDeadFunction::updateImpl(Attributor &A) {
  ...
  if (NewToBeExploredFrom.size() != ToBeExploredFrom.size() ||
      toolchain::any_of(NewToBeExploredFrom, [&](const Instruction *I) {
  ...
  if (ToBeExploredFrom.empty() &&
      ...
      toolchain::all_of(KnownDeadEnds, [](const Instruction *DeadEndI) {
...
static unsigned getKnownAlignForUse(Attributor &A, AAAlign &QueryingAA,
  ...
  if (const Value *Base = GetPointerBaseWithConstantOffset(UseV, Offset, DL)) {
    if (Base == &AssociatedValue) {
      ...
      Alignment = toolchain::bit_floor(gcd);
    }
  }
...
ChangeStatus AAHeapToStackFunction::updateImpl(Attributor &A) {
  ...
  for (auto &It : AllocationInfos) {
    ...
    if (Value *Align = getAllocAlignment(AI.CB, TLI)) {
      ...
      if (APAlign->ugt(toolchain::Value::MaximumAlignment) ||
          !APAlign->isPowerOf2()) {
...
struct AAMemoryBehaviorImpl : public AAMemoryBehavior {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    if (toolchain::all_of(DeducedAttrs, [&](const Attribute &Attr) {
...
ChangeStatus AAMemoryBehaviorFloating::updateImpl(Attributor &A) {
  ...
  auto UsePred = [&](const Use &U, bool &Follow) -> bool {
    ...
    // Droppable users, e.g., toolchain::assume does not actually perform any action.
...
protected:
  ...
  std::array<AccessSet *, toolchain::ConstantLog2<VALID_STATE>()>
      AccessKind2Accesses;
  ...
  void updateStateAndAccessesMap(AAMemoryLocation::StateType &State,
    ...
    auto *&Accesses = AccessKind2Accesses[toolchain::Log2_32(MLK)];
    if (!Accesses)
...
struct AAPotentialValuesFloating : AAPotentialValuesImpl {
  ...
  bool handleLoadInst(Attributor &A, LoadInst &LI, ItemInfo II,
    ...
    // Do not simplify loads that are only used in toolchain.assume if we cannot also
    ...
    if (InfoCache.isOnlyUsedByAssume(LI)) {
      if (!toolchain::all_of(PotentialValueOrigins, [&](Instruction *I) {
    ...
    // single toolchain::Value might represent two runtime values (e.g.,
    // stack locations in different recursive calls).
    const Instruction *CtxI = II.I.getCtxI();
    bool ScopeIsLocal = (II.S & AA::Intraprocedural);
    bool AllLocal = ScopeIsLocal;
    bool DynamicallyUnique = toolchain::all_of(PotentialCopies, [&](Value *PC) {
    ...
    auto HandleReturnedValue = [&](Value &V, Instruction *CtxI,
      ...
      for (AA::ValueScope S : {AA::Interprocedural, AA::Intraprocedural}) {
        ...
        if (S == AA::Interprocedural)
          AllInterAreIntra =
              toolchain::all_of(Values, [&](const AA::ValueAndContext &VAC) {
...
void AttributorCallGraph::print() { toolchain::WriteGraph(outs(), this); }
...
struct AAIndirectCallInfoCallSite : public AAIndirectCallInfo {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    for (Function *NewCallee : AssumedCallees) {
      ...
      LastCmp = new ICmpInst(IP, toolchain::CmpInst::ICMP_EQ, FP, NewCallee);
      Instruction *ThenTI =
