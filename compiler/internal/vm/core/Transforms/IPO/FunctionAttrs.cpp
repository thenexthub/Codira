...
MemoryEffects toolchain::computeFunctionBodyMemoryAccess(Function &F,
                                                    AAResults &AAR) {
...
bool toolchain::thinLTOPropagateFunctionAttrs(
...
void AttributeInferer::run(const SCCNodeSet &SCCNodes,
  ...
  for (Function *F : SCCNodes) {
    ...
    toolchain::erase_if(InferInSCC, [F](const InferenceDescriptor &ID) {
    ...
    for (Instruction &I : instructions(*F)) {
      toolchain::erase_if(InferInThisFunc, [&](const InferenceDescriptor &ID) {
        ...
        toolchain::erase_if(InferInSCC, [&ID](const InferenceDescriptor &D) {
...
static bool deduceFunctionAttributeInRPO(Module &M, LazyCallGraph &CG) {
  ...
  bool Changed = false;
  for (auto *F : toolchain::reverse(Worklist))
