...
public:
  ...
  struct ContextNode {
    ...
    DenseSet<uint32_t> getContextIds() const {
      ...
      auto Edges = toolchain::concat<const std::shared_ptr<ContextEdge>>(
...
public:
  ModuleCallsiteContextGraph(
      Module &M,
      toolchain::function_ref<OptimizationRemarkEmitter &(Function *)> OREGetter);
...
struct IndexCall : public PointerUnion<CallsiteInfo *, AllocInfo *> {
  ...
  void print(raw_ostream &OS) const {
    ...
    if (auto *AI = toolchain::dyn_cast_if_present<AllocInfo *>(Base)) {
      OS << *AI;
    } else {
      auto *CI = toolchain::dyn_cast_if_present<CallsiteInfo *>(Base);
      assert(CI);
      OS << *CI;
    }
  }
...
public:
  IndexCallsiteContextGraph(
      ...
      toolchain::function_ref<bool(GlobalValue::GUID, const GlobalValueSummary *)>
          isPrevailing);
...
template <>
struct toolchain::DenseMapInfo<CallsiteContextGraph<
...
template <>
struct toolchain::DenseMapInfo<IndexCall>
...
static void checkNode(const ContextNode<DerivedCCG, FuncTy, CallTy> *Node,
  ...
  if (Node->CallerEdges.size()) {
    ...
    for (const auto &Edge : toolchain::drop_begin(Node->CallerEdges)) {
      if (CheckEdges)
  ...
  if (Node->CalleeEdges.size()) {
    ...
    for (const auto &Edge : toolchain::drop_begin(Node->CalleeEdges)) {
      if (CheckEdges)
        checkEdge<DerivedCCG, FuncTy, CallTy>(Edge);
...
void CallsiteContextGraph<DerivedCCG, FuncTy, CallTy>::updateStackNodes() {
  ...
  // associated context ids over to the new nodes.
  DenseSet<const ContextNode *> Visited;
  DenseSet<uint32_t> ImportantContextIds(toolchain::from_range,
...
void CallsiteContextGraph<DerivedCCG, FuncTy,
  ...
  for (auto &Entry : NonAllocationCallToContextNodeMap) {
    ...
    toolchain::append_range(AllCalls, Node->MatchingCalls);
  ...
  for (auto &Edge : CalleeEdges) {
    ...
    if (!CalleeNodeToCallInfo.contains(Edge->Callee)) {
      ...
      if (!Info->Node) {
        ...
        toolchain::append_range(Info->Node->MatchingCalls,
                           toolchain::drop_begin(Info->Calls));
...
void CallsiteContextGraph<DerivedCCG, FuncTy, CallTy>::identifyClones(
  ...
  const unsigned AllocTypeCloningPriority[] = {/*None*/ 3, /*NotCold*/ 4,
                                               /*Cold*/ 1,
                                               /*NotColdCold*/ 2};
  toolchain::stable_sort(Node->CallerEdges,
    ...
    if (CallerEdge->IsBackedge && !CallerEdge->Caller->CloneOf &&
      ...
      if (OrigIdCount > CallerEdge->getContextIds().size()) {
        for (auto E : Node->CallerEdges) {
          ...
          if (toolchain::is_contained(CallerEdges, E))
            continue;
...
void ModuleCallsiteContextGraph::updateAllocationCall(
  ...
  auto A = toolchain::Attribute::get(Call.call()->getFunction()->getContext(),
...
void CallsiteContextGraph<DerivedCCG, FuncTy, CallTy>::mergeNodeCalleeClones(
  ...
  for (auto Entry : OrigNodeToCloneEdges) {
    ...
    // merge node.
    toolchain::stable_sort(CalleeEdges, CalleeCallerEdgeLessThan);
...
bool CallsiteContextGraph<DerivedCCG, FuncTy, CallTy>::assignFunctions() {
  ...
  for (auto &[Func, CallsWithMetadata] : FuncToCallsWithMetadata) {
    ...
    for (auto &Call : CallsWithMetadata) {
      ...
      toolchain::append_range(ClonesWorklist, Node->Clones);
...
bool MemProfContextDisambiguation::initializeIndirectCallPromotionInfo(
  ...
  // special handling (they should not get the ".toolchain.*" suffix that the
    ...
    for (auto &BB : F) {
      for (auto &I : BB) {
        ...
        if (MemProfMD) {
          ...
          assert(toolchain::none_of(AllocNode.Versions, [](uint8_t Type) {
          ...
          for (unsigned J = 0; J < AllocNode.Versions.size(); J++) {
            ...
            auto A = toolchain::Attribute::get(F.getContext(), "memprof",
                                          AllocTypeString);
            CallBase *CBClone;
            // Copy 0 is the original function.
            if (!J)
...
unsigned MemProfContextDisambiguation::recordICPInfo(
  ...
  for (const auto &Candidate : CandidateProfileData) {
    ...
    ICPNeeded |= toolchain::any_of(StackNode.Clones,
