...
/// the function.
ModulePass *toolchain::createDeadArgEliminationPass() { return new DAE(); }

ModulePass *toolchain::createDeadArgHackingPass() { return new DAH(); }

/// If this is an function that takes a ... list, and if toolchain.vastart is never
...
bool DeadArgumentEliminationPass::deleteDeadVarargs(Function &F) {
  ...
  // looking for calls marked musttail or calls to toolchain.vastart.
  for (BasicBlock &BB : F) {
  ...
  // If we get here, there are no calls to toolchain.vastart in the function body,
...
bool DeadArgumentEliminationPass::removeDeadStuffFromFunction(Function *F) {
  ...
  if (NFTy != FTy && NF->getSubprogram()) {
    ...
    auto Temp = SP->getType()->cloneWithCC(toolchain::dwarf::DW_CC_nocall);
