...
//===----------------------------------------------------------------------===//
//
// This pass lowers type metadata and calls to the toolchain.type.test intrinsic.
// It also ensures that globals are properly laid out for the
// toolchain.icall.branch.funnel intrinsic.
// See http://toolchain.org/docs/TypeMetadata.html for more information.
//
//===----------------------------------------------------------------------===//
...
#include "vm/core/IR/ModuleSummaryIndexYAML.h"
...
BitSetInfo BitSetBuilder::build() {
  ...
  BSI.AlignLog2 = 0;
  if (Mask != 0)
    BSI.AlignLog2 = toolchain::countr_zero(Mask);
...
void GlobalLayoutBuilder::addFragment(const std::set<uint64_t> &F) {
  ...
  for (auto ObjIndex : F) {
    ...
    if (OldFragmentIndex == 0) {
      ...
      toolchain::append_range(Fragment, OldFragment);
      OldFragment.clear();
    }
  }
...
public:
  static GlobalTypeMember *create(BumpPtrAllocator &Alloc, GlobalObject *GO,
    ...
    toolchain::copy(Types, GTM->getTrailingObjects());
    return GTM;
  }
...
struct ICallBranchFunnel final
  ...
  static ICallBranchFunnel *create(BumpPtrAllocator &Alloc, CallInst *CI,
    ...
    toolchain::copy(Targets, Call->getTrailingObjects());
    return Call;
  }
...
struct ScopedSaveAliaseesAndUsed {
  ...
  // This function only removes functions from toolchain.used and toolchain.compiler.used.
  ...
  void collectAndEraseUsedFunctions(Module &M,
    ...
    // toolchain.used/toolchain.compiler.used, so we remove all of them and add back only
    // the non-functions.
    GV->eraseFromParent();
    auto NonFuncBegin =
  ...
  ScopedSaveAliaseesAndUsed(Module &M) : M(M) {
    ...
    // for aliases and toolchain.used/toolchain.compiler.used with references to a jump
    ...
    // we avoid replacing toolchain.used/toolchain.compiler.used because these global
    // variables describe properties of the global, not the jump table (besides,
    // offseted references to the jump table in toolchain.used are invalid).
    ...
    // toolchain.used/toolchain.compiler.used and aliases, erase the used lists, let RAUW
...
void LowerTypeTestsModule::allocateByteArrays() {
  toolchain::stable_sort(ByteArrayInfos,
...
/// Lower a toolchain.type.test call to its implementation. Returns the value to
...
Value *LowerTypeTestsModule::lowerTypeTestCall(Metadata *TypeId, CallInst *CI,
  ...
  //   br(toolchain.type.test(...), thenbb, elsebb)
...
/// build the bit sets and lower the toolchain.type.test calls.
void LowerTypeTestsModule::buildBitSetsFromGlobalVariables(
  ...
  for (GlobalTypeMember *G : Globals) {
    ...
    // (see https://lists.toolchain.org/pipermail/toolchain-dev/2018-July/124694.html)
...
LowerTypeTestsModule::importTypeId(StringRef TypeId) {
  ...
  if (TIL.TheKind != TypeTestResolution::Unsat) {
    ...
    //
    // [1]https://github.com/toolchain/toolchain-project/pull/137742
...
void LowerTypeTestsModule::importTypeTest(CallInst *CI) {
  ...
  if (!TypeIdMDVal)
    report_fatal_error("Second argument of toolchain.type.test must be metadata");
...
void LowerTypeTestsModule::lowerTypeTestCalls(
  ...
  for (const auto &[TypeId, BSI] : buildBitSets(TypeIds, GlobalLayout)) {
    ...
    // Lower each call to toolchain.type.test for this type identifier.
...
/// and lower the toolchain.type.test calls, architecture dependently.
...
void LowerTypeTestsModule::createJumpTable(
  ...
  for (GlobalTypeMember *GTM : Functions) {
    if (!toolchain::cast<toolchain::Function>(GTM->getGlobal())
             ->hasFnAttribute(toolchain::Attribute::NoUnwind)) {
...
/// for the functions, build the bit sets and lower the toolchain.type.test calls.
...
void LowerTypeTestsModule::buildBitSetsFromDisjointSet(
  ...
  toolchain::stable_sort(TypeMembers, [](const std::set<uint64_t> &O1,
...
LowerTypeTestsModule::LowerTypeTestsModule(
  ...
  GlobalAnnotation = M.getGlobalVariable("toolchain.global.annotations");
...
void LowerTypeTestsModule::replaceCfiUses(Function *Old, Value *New,
  ...
  for (Use &U : toolchain::make_early_inc_range(Old->uses())) {
...
static void dropTypeTests(Module &M, Function &TypeTestFunc,
                          bool ShouldDropAll) {
  for (Use &U : toolchain::make_early_inc_range(TypeTestFunc.uses())) {
    auto *CI = cast<CallInst>(U.getUser());
    // Find and erase toolchain.assume intrinsics for this toolchain.type.test call.
    for (Use &CIU : toolchain::make_early_inc_range(CI->uses()))
...
bool LowerTypeTestsModule::lower() {
  ...
  if (DropTypeTests != DropTestKind::None) {
    ...
    // Normally we'd have already removed all @toolchain.public.type.test calls,
  ...
  if (ImportSummary) {
    ...
    if (ICallBranchFunnelFunc && !ICallBranchFunnelFunc->use_empty())
      report_fatal_error(
          "unexpected call to toolchain.icall.branch.funnel during import phase");
  ...
  if (TypeTestFunc) {
    for (const Use &U : TypeTestFunc->uses()) {
      auto CI = cast<CallInst>(U.getUser());
      // If this type test is only used by toolchain.assume instructions, it
  ...
  if (ICallBranchFunnelFunc) {
    for (const Use &U : ICallBranchFunnelFunc->uses()) {
      if (Arch != Triple::x86_64)
        report_fatal_error(
            "toolchain.icall.branch.funnel not supported on this target");
...

  {
    ...
    for (const auto &C : GlobalClasses) {
      ...
      // IDs.
      toolchain::sort(TypeIds, [&](Metadata *M1, Metadata *M2) {
        return TypeIdInfo[M1].UniqueId < TypeIdInfo[M2].UniqueId;
      });

      // Same for the branch funnels.
      toolchain::sort(ICallBranchFunnels,
    ...
    if (NamedMDNode *SymversMD = M.getNamedMetadata("symvers")) {
      for (auto *Symver : SymversMD->operands()) {
        ...
        M.appendModuleInlineAsm(
            (toolchain::Twine(".symver ") + SymbolName + ", " + Alias).str());
      }
    }
  }
