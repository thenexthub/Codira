...
void PseudoProbeVerifier::runAfterPass(StringRef PassID, Any IR) {
  ...
  dbgs() << Banner;
  if (const auto **M = toolchain::any_cast<const Module *>(&IR))
    runAfterPass(*M);
  else if (const auto **F = toolchain::any_cast<const Function *>(&IR))
    runAfterPass(*F);
  else if (const auto **C = toolchain::any_cast<const LazyCallGraph::SCC *>(&IR))
    runAfterPass(*C);
  else if (const auto **L = toolchain::any_cast<const Loop *>(&IR))
    runAfterPass(*L);
  else
...
void SampleProfileProber::instrumentOneFunc(Function &F, TargetMachine *TM) {
  ...
  for (auto &I : BlockProbeIds) {
    ...
    Function *ProbeFn =
        toolchain::Intrinsic::getOrInsertDeclaration(M, Intrinsic::pseudoprobe);
  ...
  assert(NMD && "toolchain.pseudo_probe_desc should be pre-created");
  NMD->addOperand(MD);
}
