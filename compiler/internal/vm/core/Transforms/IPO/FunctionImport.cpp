...
#include "vm/core/ProfileData/PGOCtxProfReader.h"
...
class WorkloadImportsManager : public ModuleImportsManager {
  ...
  computeImportForModule(const GVSummaryMapTy &DefinedGVSummaries,
    ...
    for (auto &VI : toolchain::make_early_inc_range(ValueInfos)) {
      ...
      auto PotentialCandidates = toolchain::map_range(
      ...
      auto PrevailingCandidates = toolchain::make_filter_range(
      ...
      if (PrevailingCandidates.empty()) {
        ...
        if (!toolchain::hasSingleElement(PotentialCandidates) &&
        ...
        assert(toolchain::hasSingleElement(PrevailingCandidates));
...
void toolchain::ComputeCrossModuleImport(
    const ModuleSummaryIndex &Index,
  ...
  for (auto &ELI : ExportLists) {
    ...
    for (auto &EI : ELI.second) {
      ...
      if (auto *GVS = dyn_cast<GlobalVarSummary>(S)) {
        ...
        NewExports.insert_range(toolchain::make_first_range(FS->calls()));
...
void toolchain::updateIndirectCalls(ModuleSummaryIndex &Index) {
...
void toolchain::computeDeadSymbolsAndUpdateIndirectCalls(
  ...
  auto visit = [&](ValueInfo VI, bool IsAliasee) {
    ...
    if (toolchain::any_of(VI.getSummaryList(),
                     [](const std::unique_ptr<toolchain::GlobalValueSummary> &S) {
                       return S->isLive();
                     }))
      return;
...
void toolchain::computeDeadSymbolsWithConstProp(
    ModuleSummaryIndex &Index,
    const DenseSet<GlobalValue::GUID> &GUIDPreservedSymbols,
    function_ref<PrevailingType(GlobalValue::GUID)> isPrevailing,
    bool ImportEnabled) {
  toolchain::TimeTraceScope timeScope("Drop dead symbols and propagate attributes");
...
/// \p ModulePath.
void toolchain::gatherImportedSummariesForModule(
    StringRef ModulePath,
    const DenseMap<StringRef, GVSummaryMapTy> &ModuleToDefinedGVSummaries,
    const FunctionImporter::ImportMapTy &ImportList,
...
Error toolchain::EmitImportsFiles(
...
/// will import.
void toolchain::processImportsFiles(
    StringRef ModulePath,
    const ModuleToSummariesForIndexTy &ModuleToSummariesForIndex,
...
bool toolchain::convertToDeclaration(GlobalValue &GV) {
...
void toolchain::thinLTOFinalizeInModule(Module &TheModule,
                                   const GVSummaryMapTy &DefinedGlobals,
                                   bool PropagateAttrs) {
  toolchain::TimeTraceScope timeScope("ThinLTO finalize in module");
...
void toolchain::thinLTOInternalizeModule(Module &TheModule,
                                    const GVSummaryMapTy &DefinedGlobals) {
  toolchain::TimeTraceScope timeScope("ThinLTO internalize module");
...
Expected<bool> FunctionImporter::importFunctions(
  ...
  for (const auto &ModName : ImportList.getSourceModules()) {
    toolchain::TimeTraceScope timeScope("Import", ModName);
    ...
    {
      toolchain::TimeTraceScope functionsScope("Functions");
    ...
    {
      toolchain::TimeTraceScope globalsScope("Globals");
    ...
    {
      toolchain::TimeTraceScope aliasesScope("Aliases");
