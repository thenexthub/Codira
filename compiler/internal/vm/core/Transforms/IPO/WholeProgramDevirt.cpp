...
// toolchain.assume(toolchain.type.test) or toolchain.type.checked.load intrinsics).
//
...
#include "vm/core/Transforms/Utils/CallPromotionUtils.h"
#include "vm/core/Transforms/Utils/Evaluator.h"
#include <algorithm>
#include <cmath>
#include <cstddef>
#include <map>
...
/// `toolchain.trap` intrinsic followed by an unreachable IR instruction. In the
...
wholeprogramdevirt::findLowestOffset(ArrayRef<VirtualCallTarget> Targets,
  ...
  // toolchain/Transforms/IPO/WholeProgramDevirt.h) starting at MinByte. Effectively,
    ...
    for (unsigned I = 0;; ++I) {
      ...
      for (auto &&B : Used)
        if (I < B.size())
          BitsUsed |= B[I];
      if (BitsUsed != 0xff)
        return (MinByte + I) * 8 + toolchain::countr_zero(uint8_t(~BitsUsed));
    }
  } else {
...
template <> struct toolchain::DenseMapInfo<VTableSlot> {
...
template <> struct toolchain::DenseMapInfo<VTableSlotSummary> {
...
struct CallSiteInfo {
  ...
  /// need to add a use of toolchain.type.test to each of the function summaries in
  /// the vector.
  ...
  /// the toolchain.type.checked.load intrinsic and therefore will require
  /// resolutions for toolchain.type.test in order to implement CFI checks if
...
struct DevirtModule {
  ...
  // pointer. The key is the associated toolchain.type.test intrinsic call generated
  ...
  // eliminate the type check by RAUWing the associated toolchain.type.test call with
  // true.
...
bool toolchain::hasWholeProgramVisibility(bool WholeProgramVisibilityEnabledInLTO) {
...
void toolchain::updateVCallVisibilityInModule(
...
void toolchain::updatePublicTypeTestCalls(Module &M,
                                     bool WholeProgramVisibilityEnabledInLTO) {
  toolchain::TimeTraceScope timeScope("Update public type test calls");
...
void toolchain::getVisibleToRegularObjVtableGUIDs(
...
void toolchain::updateVCallVisibilityInIndex(
...
void toolchain::runWholeProgramDevirtOnIndex(
    ModuleSummaryIndex &Summary, std::set<GlobalValue::GUID> &ExportedGUIDs,
    std::map<ValueInfo, std::vector<VTableSlotSummary>> &LocalWPDTargetsMap) {
  DevirtIndex(Summary, ExportedGUIDs, LocalWPDTargetsMap).run();
}

void toolchain::updateIndexWPDForExports(
...
bool DevirtModule::trySingleImplDevirt(
  ...
  if (TheFn->hasLocalLinkage()) {
    std::string NewName = (TheFn->getName() + ".toolchain.merged").str();
...
bool DevirtIndex::trySingleImplDevirt(MutableArrayRef<ValueInfo> TargetsForSlot,
  ...
  // index (e.g. toolchain-lto). However, WPD is not supported/invoked for the
...
void DevirtModule::tryICallBranchFunnel(
  ...
  Function *Intr = Intrinsic::getOrInsertDeclaration(
      &M, toolchain::Intrinsic::icall_branch_funnel, {});
  ...
  auto Apply = [&](CallSiteInfo &CSInfo) {
    ...
    for (auto &&VCallSite : CSInfo.CallSites) {
      ...
      if (CallBases.find(&CB) != CallBases.end()) {
        ...
        // vtable passed to multiple toolchain.type.test or toolchain.type.checked.load
      ...
      toolchain::append_range(Args, CB.args());
    ...
    // toolchain.type.test and therefore require an toolchain.type.test resolution for the
    // type identifier.
...
void DevirtModule::scanTypeTestUsers(
  ...
  // of the form toolchain.assume(toolchain.type.test(%p, %md)) or
  // toolchain.assume(toolchain.public.type.test(%p, %md)).
...
void DevirtModule::scanTypeCheckedLoadUsers(Function *TypeCheckedLoadFunc) {
  ...
  for (Use &U : toolchain::make_early_inc_range(TypeCheckedLoadFunc->uses())) {
    auto *CI = dyn_cast<CallInst>(U.getUser());
    if (!CI)
      continue;
...
bool DevirtModule::run() {
  ...
  for (auto &S : CallSlots) {
    ...
    // CFI-specific: if we are exporting and any toolchain.type.checked.load
    // intrinsics were *not* devirtualized, we need to add the resulting
    // toolchain.type.test intrinsics to the function summaries so that the
