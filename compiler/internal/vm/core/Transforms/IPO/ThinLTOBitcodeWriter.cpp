...
void simplifyExternals(Module &M) {
  ...
  for (Function &F : toolchain::make_early_inc_range(M)) {
    ...
    if (!F.isDeclaration() || F.getFunctionType() == EmptyFT ||
        ...
        F.getName().starts_with("toolchain."))
      continue;
  ...
  for (GlobalIFunc &I : toolchain::make_early_inc_range(M.ifuncs())) {
    if (I.use_empty())
      I.eraseFromParent();
    else
...
// Clone any @toolchain[.compiler].used over to the new module and append
...
static void cloneUsedGlobalVariables(const Module &SrcM, Module &DestM,
  ...
  // First collect those in the toolchain[.compiler].used set.
  ...
  // Finally, add them to a toolchain[.compiler].used variable in DestM.
  if (CompilerUsed)
...
void splitAndWriteThinLTOBitcode(
  ...
  // Clone any toolchain.*used globals to ensure the included values are
  // not deleted.
  ...
  for (auto *V : CfiFunctions) {
    ...
    Elts.push_back(ConstantAsMetadata::get(
        toolchain::ConstantInt::get(Type::getInt8Ty(Ctx), Linkage)));
...
PreservedAnalyses
toolchain::ThinLTOBitcodeWriterPass::run(Module &M, ModuleAnalysisManager &AM) {
