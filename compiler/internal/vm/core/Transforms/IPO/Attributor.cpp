...
unsigned toolchain::MaxInitializationChainLength;
...
///
///{
ChangeStatus toolchain::operator|(ChangeStatus L, ChangeStatus R) {
...
SubsumingPositionIterator::SubsumingPositionIterator(const IRPosition &IRP) {
  ...
  // potentially problematic. We handle only toolchain.assume for now.
...
bool Attributor::checkForAllReturnedValues(function_ref<bool(Value &)> Pred,
  ...
  return toolchain::all_of(Values, [&](const AA::ValueAndContext &VAC) {
...
ChangeStatus Attributor::cleanupIR() {
  ...
  toolchain::erase_if(DeadInsts, [&](WeakTrackingVH I) { return !I; });
...
bool Attributor::shouldSeedAttribute(AbstractAttribute &AA) {
  ...
  if (SeedAllowList.size() != 0)
    Result = toolchain::is_contained(SeedAllowList, AA.getName());
  Function *Fn = AA.getAnchorScope();
  if (FunctionSeedAllowList.size() != 0 && Fn)
    Result &= toolchain::is_contained(FunctionSeedAllowList, Fn->getName());
#endif
  return Result;
}
...
ChangeStatus Attributor::rewriteFunctionSignatures(
  ...
  for (auto &It : ArgumentReplacementMap) {
    ...
    for (auto *I : NewArgumentTypes)
      if (auto *VT = dyn_cast<toolchain::VectorType>(I))
        LargestVectorWidth =
...
void InformationCache::initializeInformationCache(const Function &CF,
  ...
  for (Instruction &I : instructions(&F)) {
    ...
    case Instruction::Call:
      ...
      // For `toolchain.assume` calls we also fill the KnowledgeMap as we find them.
...
///
///{
raw_ostream &toolchain::operator<<(raw_ostream &OS, ChangeStatus S) {
  return OS << (S == ChangeStatus::CHANGED ? "changed" : "unchanged");
}

raw_ostream &toolchain::operator<<(raw_ostream &OS, IRPosition::Kind AP) {
  switch (AP) {
  case IRPosition::IRP_INVALID:
    return OS << "inv";
  case IRPosition::IRP_FLOAT:
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const IRPosition &Pos) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const IntegerRangeState &S) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const AbstractState &S) {
  return OS << (!S.isValidState() ? "top" : (S.isAtFixpoint() ? "fix" : ""));
}

raw_ostream &toolchain::operator<<(raw_ostream &OS, const AbstractAttribute &AA) {
  AA.print(OS);
  return OS;
}
...
static bool runAttributorOnFunctions(InformationCache &InfoCache,
  ...
  for (Function *F : Functions) {
    ...
    if (F->hasLocalLinkage()) {
      if (toolchain::all_of(F->uses(), [&Functions](const Use &U) {
...
void AADepGraph::viewGraph() { toolchain::ViewGraph(this, "Dependency Graph"); }
...
void AADepGraph::dumpGraph() {
  ...
  if (!EC)
    toolchain::WriteGraph(File, this);
