...
#include "vm/core/Transforms/Utils/FunctionComparator.h"
...
private:
  ...
  /// Set of values marked as used in toolchain.used and toolchain.compiler.used.
...
template <typename FuncContainer> bool MergeFunctions::run(FuncContainer &M) {
  ...
  toolchain::stable_sort(HashedFuncs, less_first());
...
void MergeFunctions::mergeTwoFunctions(Function *F, Function *G) {
  ...
  if (F->isInterposable() || (isODR(F) && isODR(G))) {
    ...
    if (!G->isInterposable() && !MergeFunctionsPDI) {
      // Functions referred to by toolchain.used/toolchain.compiler.used are special:
