...
static GlobalVariable *SRAGlobal(GlobalVariable *GV, const DataLayout &DL) {
  ...
  sort(TypesVector, toolchain::less_first());
...
static bool OptimizeAwayTrappingUsesOfLoads(
  ...
  for (User *GlobalUser : toolchain::make_early_inc_range(GV->users())) {
...
isValidCandidateForColdCC(Function &F,
  ...
  for (User *U : F.users()) {
    ...
    if (!isColdCallSite(*CB, CallerBFI))
      return false;
    if (!toolchain::is_contained(AllCallsCold, CallerFunc))
      return false;
  }
  return true;
}
...
static void RemovePreallocated(Function *F) {
  ...
  for (User *U : PreallocatedCalls) {
    ...
    // Replace @toolchain.call.preallocated.arg() with alloca.
    // Cannot modify users() while iterating over it, so make a copy.
    // @toolchain.call.preallocated.arg() can be called with the same index multiple
    // times. So for each @toolchain.call.preallocated.arg(), we see if we have
    // already created a Value* for the index, and if not, create an alloca and
    // bitcast right after the @toolchain.call.preallocated.setup() so that it
    // dominates all uses.
    ...
    for (auto *User : PreallocatedArgs) {
      ...
      assert(UseCall->getCalledFunction()->getIntrinsicID() ==
             ...
             "preallocated token use was not a toolchain.call.preallocated.arg");
    ...
    // Remove @toolchain.call.preallocated.setup().
...
static void setUsedInitializer(GlobalVariable &V,
  ...
  NV->takeName(&V);
  NV->setSection("toolchain.metadata");
  delete &V;
}

namespace {

/// An easy to access representation of toolchain.used and toolchain.compiler.used.
class LLVMUsed {
...
public:
  LLVMUsed(Module &M) {
    ...
    Used = {toolchain::from_range, Vec};
    Vec.clear();
    CompilerUsedV = collectUsedGlobalVariables(M, Vec, true);
    CompilerUsed = {toolchain::from_range, Vec};
  }
...
static bool hasUseOtherThanLLVMUsed(GlobalAlias &GA, const LLVMUsed &U) {
  ...
  assert((!U.usedCount(&GA) || !U.compilerUsedCount(&GA)) &&
         ...
         "element from toolchain.compiler.used");
  if (!GA.hasOneUse())
    // Strictly more than one use. So at least one is not in toolchain.used and
    // toolchain.compiler.used.
    return true;

  // Exactly one use. Check if it is in toolchain.used or toolchain.compiler.used.
...
static bool hasUsesToReplace(GlobalAlias &GA, const LLVMUsed &U,
  ...
  // @toolchain.used, @toolchain.compiler.used), give it the name and linkage of the
...
OptimizeGlobalAliases(Module &M,
  ...
  for (GlobalAlias &J : toolchain::make_early_inc_range(M.aliases())) {
...
static bool OptimizeEmptyGlobalAtExitDtors(Function *CXAAtExitFn, bool isCXX) {
  ...
  for (User *U : toolchain::make_early_inc_range(CXAAtExitFn->users())) {
    // We're only interested in calls. Theoretically, we could handle invoke
    // instructions as well, but neither toolchain-gcc nor clang generate invokes
    // to __cxa_atexit.
    CallInst *CI = dyn_cast<CallInst>(U);
    if (!CI)
      continue;
