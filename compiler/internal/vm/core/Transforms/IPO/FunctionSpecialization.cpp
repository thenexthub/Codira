...
#include "vm/core/Transforms/IPO/FunctionSpecialization.h"
...
Constant *FunctionSpecializer::getPromotableAlloca(AllocaInst *Alloca,
  ...
  for (auto *User : Alloca->users()) {
    // We can't use toolchain::isAllocaPromotable() as that would fail because of
...
template <> struct toolchain::DenseMapInfo<SpecSig> {
  ...
  for (unsigned I = 0; I < NSpecs; ++I) {
    ...
    for (CallBase *Call : S.CallSites) {
      ...
      if (Count && !ProfcheckDisableMetadataFixes) {
        std::optional<toolchain::Function::ProfileCount> MaybeCloneCount =
            Clone->getEntryCount();
        if (MaybeCloneCount) {
          uint64_t CallCount = *Count + MaybeCloneCount->getCount();
          Clone->setEntryCount(CallCount);
          if (std::optional<toolchain::Function::ProfileCount> MaybeOriginalCount =
                  S.F->getEntryCount()) {
