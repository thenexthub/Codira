...
Function *IROutliner::createFunction(Module &M, OutlinableGroup &Group,
  ...
  if (DISubprogram *SP = getSubprogramOrNull(Group)) {
    ...
    std::string Dummy;
    toolchain::raw_string_ostream MangledNameStream(Dummy);
...
static void moveFunctionData(Function &Old, Function &New,
  ...
  for (BasicBlock &CurrBB : toolchain::make_early_inc_range(Old)) {
...
static void getCodeExtractorArguments(
  ...
  // allocas or removing toolchain.assumes.
...
static bool outputHasNonPHI(Value *V, unsigned PHILoc, PHINode &PN,
  ...
  if (any_of(toolchain::seq<unsigned>(0, PN.getNumIncomingValues()),
...
static hash_code encodePHINodeData(PHINodeData &PND) {
  return toolchain::hash_combine(toolchain::hash_value(PND.first.first),
                            toolchain::hash_value(PND.first.second),
                            toolchain::hash_combine_range(PND.second));
}
...
unsigned IROutliner::doOutline(Module &M) {
  ...
  if (SimilarityCandidates.size() > 1)
    toolchain::stable_sort(SimilarityCandidates,
