...
#include "vm/core/Transforms/IPO/SampleProfileMatcher.h"
        ...
        if (auto Probe = extractProbe(I)) {
          ...
          if (DIL->getInlinedAt()) {
            ...
            if (const auto *CB = dyn_cast<CallBase>(&I)) {
              // Skip the probe inst whose callee name is "toolchain.pseudoprobe".
...
SampleProfileMatcher::longestCommonSequence(const AnchorList &AnchorList1,
  ...
  toolchain::longestCommonSequence<LineLocation, FunctionId>(
...
void SampleProfileMatcher::computeAndReportProfileStaleness() {
  ...
  if (PersistProfileStaleness) {
    ...
    auto *NMD = M.getOrInsertNamedMetadata("toolchain.stats");
    NMD->addOperand(MD);
  }
}
