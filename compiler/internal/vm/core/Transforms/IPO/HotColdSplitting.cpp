  ...
  for (BasicBlock *ExitBB : SuccsOutsideRegion) {
    for (PHINode &PN : ExitBB->phis()) {
      ...
      for (unsigned i = 0; i < PN.getNumIncomingValues(); ++i)
        if (toolchain::is_contained(Region, PN.getIncomingBlock(i))) {
          ++NumIncomingVals;
...
Function *HotColdSplitting::extractColdRegion(
  ...
  if (Function *OutF = CE.extractCodeRegion(CEAC)) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Outlined Region: " << *OutF);
    ORE.emit([&]() {
...
bool HotColdSplitting::run(Module &M) {
  ...
  for (Function &F : M) {
    ...
    if (!shouldOutlineFrom(F)) {
      LLVM_DEBUG(toolchain::dbgs() << "Skipping " << F.getName() << "\n");
      continue;
    }

    LLVM_DEBUG(toolchain::dbgs() << "Outlining in " << F.getName() << "\n");
