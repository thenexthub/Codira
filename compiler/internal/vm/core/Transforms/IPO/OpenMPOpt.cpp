...
#include "vm/core/ADT/EnumeratedArray.h"
...
#include "vm/core/Frontend/OpenMP/OMPDeviceConstants.h"
...
//   toolchain::omp::OMPTgtExecModeFlags ExecMode;
//   int32_t MinThreads;
//   int32_t MaxThreads;
//   int32_t MinTeams;
//   int32_t MaxTeams;
  ...
  bool mergeParallelRegions() {
    ...
    auto Merge = [&](const SmallVectorImpl<CallInst *> &MergableCIs,
      ...
      auto Remark = [&](OptimizationRemark OR) {
        ...
        for (auto *CI : toolchain::drop_begin(MergableCIs)) {
...
Kernel OpenMPOpt::getUniqueKernelFor(Function &F) {
  ...
  // Use a scope to keep the lifetime of the CachedKernel short.
  {
    ...
    if (!F.hasLocalLinkage()) {
      ...
      // See https://openmp.toolchain.org/remarks/OptimizationRemarks.html
      auto Remark = [&](OptimizationRemarkAnalysis ORA) {
...
struct AAExecutionDomainFunction : public AAExecutionDomain {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto HandleAlignedBarrier = [&](CallBase *CB) {
      ...
      // toolchain.assumes as well to avoid creating UB.
  ...
  bool isExecutedInAlignedRegion(Attributor &A,
    ...
    if (!CurI) {
      ...
      if (!toolchain::all_of(predecessors(BB), [&](const BasicBlock *PredBB) {
  ...
  static bool isInitialThreadOnlyEdge(Attributor &A, BranchInst *Edge,
    ...
    if (C->isZero()) {
      // Match: 0 == toolchain.nvvm.read.ptx.sreg.tid.x()
      if (auto *II = dyn_cast<IntrinsicInst>(Cmp->getOperand(0)))
        if (II->getIntrinsicID() == Intrinsic::nvvm_read_ptx_sreg_tid_x)
          return true;

      // Match: 0 == toolchain.amdgcn.workitem.id.x()
      if (auto *II = dyn_cast<IntrinsicInst>(Cmp->getOperand(0)))
...
#undef KERNEL_ENVIRONMENT_CONFIGURATION_SETTER
  ...
  bool buildCustomStateMachine(Attributor &A, ChangeStatus &Changed) {
    ...
    Instruction *IsWorker =
        ICmpInst::Create(ICmpInst::ICmp, toolchain::CmpInst::ICMP_NE, KernelInitCB,
    ...
    Instruction *IsMainOrWorker = ICmpInst::Create(
        ICmpInst::ICmp, toolchain::CmpInst::ICMP_SLT, KernelInitCB, BlockSize,
    ...
    Instruction *IsDone =
        ICmpInst::Create(ICmpInst::ICmp, toolchain::CmpInst::ICMP_EQ, WorkFn,
    ...
    for (int I = 0, E = ReachedKnownParallelRegions.size(); I < E; ++I) {
      ...
      if (I + 1 < E || !ReachedUnknownParallelRegions.empty()) {
        Instruction *CmpI = ICmpInst::Create(
            ICmpInst::ICmp, toolchain::CmpInst::ICMP_EQ, WorkFn, ParallelRegion,
...
private:
  ...
  ChangeStatus foldKernelFnAttribute(Attributor &A, toolchain::StringRef Attr) {
...
void OpenMPOpt::registerAAs(bool IsModulePass) {
  ...
  for (auto *F : SCC) {
    ...
    // to do it eagerly.
    if (F->hasLocalLinkage()) {
      if (toolchain::all_of(F->uses(), [this](const Use &U) {
...
PreservedAnalyses OpenMPOptCGSCCPass::run(LazyCallGraph::SCC &C,
  ...
  bool PostLink = LTOPhase == ThinOrFullLTOPhase::FullLTOPostLink ||
                  LTOPhase == ThinOrFullLTOPhase::ThinLTOPostLink ||
                  LTOPhase == ThinOrFullLTOPhase::ThinLTOPreLink;
  SetVector<Function *> Functions(toolchain::from_range, SCC);
...
bool toolchain::omp::isOpenMPKernel(Function &Fn) {
  return Fn.hasFnAttribute("kernel");
}

KernelSet toolchain::omp::getDeviceKernels(Module &M) {
  KernelSet Kernels;
...
bool toolchain::omp::containsOpenMP(Module &M) {
...
bool toolchain::omp::isOpenMPDevice(Module &M) {
