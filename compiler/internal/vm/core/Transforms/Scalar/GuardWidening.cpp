...
// @toolchain.experimental.guard intrinsic lets LLVM transform it so that it fails
...
//
//    %cmp0 = 7 u< Length
//    call @toolchain.experimental.guard(i1 %cmp0) [ "deopt"(...) ]
//    call @unknown_side_effects()
//    %cmp1 = 9 u< Length
//    call @toolchain.experimental.guard(i1 %cmp1) [ "deopt"(...) ]
//    ...
//
// =>
//
...
//    ...
//
// If %cmp0 is false, @toolchain.experimental.guard will "deoptimize" back to a
...
/// the one described at https://github.com/toolchain/toolchain-project/issues/60234. The
...
bool GuardWideningImpl::combineRangeChecks(
  ...
  while (!Checks.empty()) {
    ...
    if (CurrentChecks.size() < 3) {
      toolchain::append_range(RangeChecksOut, CurrentChecks);
      continue;
    }
    ...
    toolchain::sort(CurrentChecks, [&](const GuardWideningImpl::RangeCheck &LHS,
