...
STATISTIC(NumAbs,       "Number of toolchain.abs intrinsics removed");
STATISTIC(NumOverflows, "Number of overflow checks removed");
STATISTIC(NumSaturating,
    "Number of saturating arithmetics converted to normal arithmetics");
STATISTIC(NumNonNull, "Number of function pointer arguments marked non-null");
STATISTIC(NumCmpIntr, "Number of toolchain.[us]cmp intrinsics removed");
STATISTIC(NumMinMax, "Number of toolchain.[us]{min,max} intrinsics removed");
STATISTIC(NumSMinMax,
          "Number of toolchain.s{min,max} intrinsics simplified to unsigned");
...
// See if @toolchain.abs argument is alays positive/negative, and simplify.
...
static bool runImpl(Function &F, LazyValueInfo *LVI, DominatorTree *DT,
  ...
  for (BasicBlock *BB : depth_first(&F.getEntryBlock())) {
    ...
    for (Instruction &II : toolchain::make_early_inc_range(*BB)) {
      switch (II.getOpcode()) {
