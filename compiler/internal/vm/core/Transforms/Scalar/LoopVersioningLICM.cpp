...
static const char *LICMVersioningMetaData = "toolchain.loop.licm_versioning.disable";
...
bool LoopVersioningLICM::run(DominatorTree *DT) {
  ...
  if (isLegalForVersioning()) {
    ...
    // Set "toolchain.mem.parallel_loop_access" metaData to versioned loop.
    // FIXME: "toolchain.mem.parallel_loop_access" annotates memory access
    // instructions, not loops.
    addStringMetadataToLoop(LVer.getVersionedLoop(),
                            "toolchain.mem.parallel_loop_access");
