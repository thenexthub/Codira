...
#include "vm/core/Analysis/ScalarEvolutionNormalization.h"
#include "vm/core/Analysis/ScalarEvolutionPatternMatch.h"
...
class LSRInstance {
  ...
  SmallVector<toolchain::WeakVH, 2> ScalarEvolutionIVs;
...
LSRInstance::OptimizeLoopTermCond() {
  ...
  if (!toolchain::is_contained(ExitingBlocks, LatchBlock)) {
    ...
    if (const SCEVNAryExpr *N = dyn_cast<SCEVNAryExpr>(S))
      ...
      for (const Use &U : V->uses()) {
        ...
        if (isa<PHINode>(UserInst)) {
          ...
          toolchain::Value *ExpectedValue = U;
            ...
            if (const SCEVConstant *C = dyn_cast<SCEVConstant>(NewReg)) {
              ...
              if ((C->getAPInt() + NewF.BaseOffset.getFixedValue())
                  ...
                  (C->getAPInt() + NewF.BaseOffset.getFixedValue())
                          .countr_zero() >=
                      (unsigned)toolchain::countr_zero<uint64_t>(
...
static iterator_range<toolchain::DIExpression::expr_op_iterator>
ToDwarfOpIter(SmallVectorImpl<uint64_t> &Expr) {
  toolchain::DIExpression::expr_op_iterator Begin =
      toolchain::DIExpression::expr_op_iterator(Expr.begin());
  toolchain::DIExpression::expr_op_iterator End =
      toolchain::DIExpression::expr_op_iterator(Expr.end());
  return {Begin, End};
}
...
struct SCEVDbgValueBuilder {
  ...
  void pushLocation(toolchain::Value *V) {
    Expr.push_back(toolchain::dwarf::DW_OP_LLVM_arg);
    auto *It = toolchain::find(LocationOps, V);
    unsigned ArgIndex = 0;
  ...
  void pushValue(const SCEVUnknown *U) {
    toolchain::Value *V = cast<SCEVUnknown>(U)->getValue();
    pushLocation(V);
  }
  ...
  bool pushConst(const SCEVConstant *C) {
    if (C->getAPInt().getSignificantBits() > 64)
      return false;
    Expr.push_back(toolchain::dwarf::DW_OP_consts);
  ...
  // DWARF_OP_LLVM_args.
  iterator_range<toolchain::DIExpression::expr_op_iterator> expr_ops() {
  ...
  bool pushArithmeticExpr(const toolchain::SCEVCommutativeExpr *CommExpr,
                          uint64_t DwarfOp) {
    assert((isa<toolchain::SCEVAddExpr>(CommExpr) || isa<SCEVMulExpr>(CommExpr)) &&
  ...
  bool pushCast(const toolchain::SCEVCastExpr *C, bool IsSigned) {
    const toolchain::SCEV *Inner = C->getOperand(0);
    const toolchain::Type *Type = C->getType();
    uint64_t ToWidth = Type->getIntegerBitWidth();
    bool Success = pushSCEV(Inner);
    uint64_t CastOps[] = {dwarf::DW_OP_LLVM_convert, ToWidth,
                          IsSigned ? toolchain::dwarf::DW_ATE_signed
  ...
  bool pushSCEV(const toolchain::SCEV *S) {
    bool Success = true;
    if (const SCEVConstant *StartInt = dyn_cast<SCEVConstant>(S)) {
      ...
      Success &= pushArithmeticExpr(AddExpr, toolchain::dwarf::DW_OP_plus);
  ...
  bool isIdentityFunction(uint64_t Op, const SCEV *S) {
    if (const SCEVConstant *C = dyn_cast<SCEVConstant>(S)) {
      ...
      switch (Op) {
      case toolchain::dwarf::DW_OP_plus:
      case toolchain::dwarf::DW_OP_minus:
        return I == 0;
      case toolchain::dwarf::DW_OP_mul:
      case toolchain::dwarf::DW_OP_div:
        return I == 1;
      }
    }
    return false;
  ...
  bool SCEVToValueExpr(const toolchain::SCEVAddRecExpr &SAR, ScalarEvolution &SE) {
    ...
    if (!isIdentityFunction(toolchain::dwarf::DW_OP_mul, Stride)) {
      if (!pushSCEV(Stride))
        return false;
      pushOperator(toolchain::dwarf::DW_OP_mul);
    }
    if (!isIdentityFunction(toolchain::dwarf::DW_OP_plus, Start)) {
      if (!pushSCEV(Start))
        return false;
      pushOperator(toolchain::dwarf::DW_OP_plus);
    }
    return true;
  }
  ...
  /// the stride and added to the start.
  /// Components of the expression are omitted if they are an identity function.
  bool SCEVToIterCountExpr(const toolchain::SCEVAddRecExpr &SAR,
                           ScalarEvolution &SE) {
    assert(SAR.isAffine() && "Expected affine SCEV");
    const SCEV *Start = SAR.getStart();
    const SCEV *Stride = SAR.getStepRecurrence(SE);
    ...
    // Skip pushing arithmetic noops.
    if (!isIdentityFunction(toolchain::dwarf::DW_OP_minus, Start)) {
      if (!pushSCEV(Start))
        return false;
      pushOperator(toolchain::dwarf::DW_OP_minus);
    }
    if (!isIdentityFunction(toolchain::dwarf::DW_OP_div, Stride)) {
      if (!pushSCEV(Stride))
        return false;
      pushOperator(toolchain::dwarf::DW_OP_div);
    }
...
struct DVIRecoveryRec {
  ...
  SmallVector<const toolchain::SCEV *, 2> SCEVs;
...
static void updateDVIWithLocations(T &DbgVal,
  ...
  auto ValArrayRef = toolchain::ArrayRef<toolchain::ValueAsMetadata *>(MetadataLocs);
  DbgVal.setRawLocation(toolchain::DIArgList::get(DbgVal.getContext(), ValArrayRef));
  DbgVal.setExpression(DIExpression::get(DbgVal.getContext(), Ops));
}
...
static void UpdateDbgValue(DVIRecoveryRec &DVIRec,
  ...
  if (NumLLVMArgs == 0) {
    ...
    toolchain::SmallVector<uint64_t, 6> ShortenedOps(toolchain::drop_begin(NewExpr, 2));
...
static Value *getValueOrPoison(WeakVH &VH, LLVMContext &C) {
  return (VH) ? VH : PoisonValue::get(toolchain::Type::getInt8Ty(C));
}
...
static void restorePreTransformState(DVIRecoveryRec &DVIRec) {
  ...
  if (!DVIRec.HadLocationArgList) {
    ...
    DbgVal->setRawLocation(
        toolchain::DIArgList::get(DbgVal->getContext(), ValArrayRef));
  }
  LLVM_DEBUG(dbgs() << "scev-salvage: pre-LSR: " << *DbgVal << '\n');
}

static bool SalvageDVI(toolchain::Loop *L, ScalarEvolution &SE,
                       toolchain::PHINode *LSRInductionVar, DVIRecoveryRec &DVIRec,
...
static void DbgRewriteSalvageableDVIs(
    toolchain::Loop *L, ScalarEvolution &SE, toolchain::PHINode *LSRInductionVar,
    SmallVector<std::unique_ptr<DVIRecoveryRec>, 2> &DVIToUpdate) {
  if (DVIToUpdate.empty())
    return;

  const toolchain::SCEV *SCEVInductionVar = SE.getSCEV(LSRInductionVar);
...
static toolchain::PHINode *GetInductionVariable(const Loop &L, ScalarEvolution &SE,
...
static bool ReduceLoopStrength(Loop *L, IVUsers &IU, ScalarEvolution &SE,
  ...
  for (const auto &L : LI) {
    if (toolchain::PHINode *IV = GetInductionVariable(*L, SE, Reducer))
...
Pass *toolchain::createLoopStrengthReducePass() { return new LoopStrengthReduce(); }
