...
maybeFloatingPointRecurrence(Loop *L, PHINode *PN) {
  ...
  auto It = toolchain::find_if(Incr->users(),
...
bool IndVarSimplify::rewriteNonIntegerIVs(Loop *L) {
  ...
  SmallVector<WeakTrackingVH, 8> PHIs(toolchain::make_pointer_range(Header->phis()));
...
bool IndVarSimplify::simplifyAndExtend(Loop *L,
  ...
  SmallVector<PHINode *, 8> LoopPhis(
      toolchain::make_pointer_range(L->getHeader()->phis()));
...
bool IndVarSimplify::sinkUnusedInvariants(Loop *L) {
  ...
  bool MadeAnyChanges = false;
  for (Instruction &I : toolchain::make_early_inc_range(toolchain::reverse(*Preheader))) {
...
bool IndVarSimplify::optimizeLoopExits(Loop *L, SCEVExpander &Rewriter) {
  ...
  // iteration.
  toolchain::erase_if(ExitingBlocks, [&](BasicBlock *ExitingBB) {
  ...
  // between them.
  toolchain::sort(ExitingBlocks, [&](BasicBlock *A, BasicBlock *B) {
...
static bool crashingBBWithoutEffect(const BasicBlock &BB) {
  return toolchain::all_of(BB, [](const Instruction &I) {
...
bool IndVarSimplify::predicateLoopExits(Loop *L, SCEVExpander &Rewriter) {
  ...
  toolchain::sort(ExitingBlocks, [&](BasicBlock *A, BasicBlock *B) {
    // toolchain::sort sorts in ascending order, so we want the inverse of
    // the normal dominance relation.
    if (A == B)
      return false;
    if (DT->properlyDominates(A, B))
