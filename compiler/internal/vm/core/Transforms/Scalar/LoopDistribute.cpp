...
static const char *DistributedMetaData = "toolchain.loop.isdistributed";
...
public:
  ...
  void cloneLoops() {
    ...
    for (auto &Part : toolchain::drop_begin(toolchain::reverse(PartitionContainer))) {
  ...
  bool processLoop() {
    ...
    if (!Pred.isAlwaysTrue() || !Checks.empty()) {
      ...
      MDNode *UnversionedLoopID = *makeFollowupLoopID(
          ...
          "toolchain.loop.distribute.", true);
...
private:
  ...
  void setForced() {
    std::optional<const MDOperand *> Value =
        findStringMetadataForLoop(L, "toolchain.loop.distribute.enable");
    if (!Value)
      return;
