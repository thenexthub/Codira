...
bool MemsetRange::isProfitableToUseMemset(const DataLayout &DL) const {
  ...
  // pessimize the toolchain optimizer.
  //
...
bool MemCpyOptPass::moveUp(StoreInst *SI, Instruction *P, const LoadInst *LI) {
  ...
  for (auto I = --SI->getIterator(), E = P->getIterator(); I != E; --I) {
    ...
    if (Args.erase(C))
      ...
      NeedLift = toolchain::any_of(MemLocs, [C, this](const MemoryLocation &ML) {
        return isModOrRefSet(AA->getModRefInfo(C, ML));
      });

      if (!NeedLift)
        NeedLift = toolchain::any_of(Calls, [C, this](const CallBase *Call) {
  ...
  for (auto *I : toolchain::reverse(ToLift)) {
...
bool MemCpyOptPass::processMemCpyMemCpyDependence(MemCpyInst *M,
  ...
  toolchain::scope_exit CleanupOnRet([&] {
  ...
  if (isModSet(BAA.getModRefInfo(M, MemoryLocation::getForSource(MDep)))) {
    // Don't convert toolchain.memcpy.inline into memmove because memmove can be
    // lowered as a call, and that is not allowed for toolchain.memcpy.inline (and
    // there is no inline version of toolchain.memmove)
    if (M->isForceInlined())
      return false;
    UseMemMove = true;
  }
  ...
  if (UseMemMove)
    ...
    // toolchain.memcpy may be promoted to toolchain.memcpy.inline, but the converse is
...
bool MemCpyOptPass::processMemSetMemCpyDependence(MemCpyInst *MemCpy,
  ...
  // https://toolchain.org/docs/HowToUpdateDebugInfo.html about "when to preserve an
      ...
      for (const Use &U : I->uses()) {
        ...
        if (UI->mayReadOrWriteMemory()) {
          if (UI->isLifetimeStartOrEnd()) {
            ...
            // since both toolchain.lifetime.start and toolchain.lifetime.end intrinsics
