...
public:
  ...
  ModelledPHI(const PHINode *PN,
    ...
    toolchain::sort(Ops, ComesBefore);
  ...
  verifyModelledPHI(const DenseMap<const BasicBlock *, unsigned> &BlockOrder) {
    ...
    assert(toolchain::is_sorted(Blocks, ComesBefore));
    int C = 0;
  ...
  ModelledPHI(SmallVectorImpl<Instruction *> &V,
    ...
    toolchain::append_range(Values, V);
    toolchain::append_range(Blocks, B);
  ...
  bool areAllIncomingValuesSame() const {
    return toolchain::all_equal(Values);
  }
  ...
  bool areAnyIncomingValuesConstant() const {
    return toolchain::any_of(Values, [&](Value *V) { return isa<Constant>(V); });
  }
...
template <> struct toolchain::DenseMapInfo<ModelledPHI> {
...
public:
  ...
  bool run(Function &F) {
    ...
    VN.setReachableBBs(BasicBlocksSet(toolchain::from_range, RPOT));
...
private:
  ...
  void foldPointlessPHINodes(BasicBlock *BB) {
    ...
    while (PHINode *PN = dyn_cast<PHINode>(I++)) {
      if (!toolchain::all_of(PN->incoming_values(), [&](const Value *V) {
...
GVNSink::analyzeInstructionForSinking(LockstepReverseIterator<false> &LRI,
  ...
  unsigned VNumToSink = toolchain::max_element(VNums, toolchain::less_second())->first;
...
unsigned GVNSink::sinkBB(BasicBlock *BBEnd) {
  ...
  // Sort in a deterministic order.
  toolchain::sort(Preds, ComesBefore);

  unsigned NumOrigPreds = Preds.size();
  // We can only sink instructions through unconditional branches.
  toolchain::erase_if(Preds, [](BasicBlock *BB) {
  ...
  toolchain::stable_sort(Candidates, std::greater<SinkingInstructionCandidate>());
...
void GVNSink::sinkLastInstruction(ArrayRef<BasicBlock *> Blocks,
  ...
  for (unsigned O = 0, E = I0->getNumOperands(); O != E; ++O) {
    bool NeedPHI = toolchain::any_of(Insts, [&I0, O](const Instruction *I) {
