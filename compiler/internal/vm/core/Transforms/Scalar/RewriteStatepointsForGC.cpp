...
/// constant physical memory: toolchain.invariant.start.
...
static bool containsGCPtrType(Type *Ty, GCStrategy *GC) {
  ...
  if (StructType *ST = dyn_cast<StructType>(Ty))
    return toolchain::any_of(ST->elements(),
...
static Value *findBasePointer(Value *I, DefiningValueMapTy &Cache,
    ...
    if (PHINode *BasePHI = dyn_cast<PHINode>(State.getBaseValue())) {
      ...
      for (unsigned i = 0; i < NumPHIValues; i++) {
        ...
        if (Inserted)
          ...
          // toolchain::Value of the correct type (and still remain pure).
  ...
  [[maybe_unused]] auto &DL =
      cast<toolchain::Instruction>(Def)->getDataLayout();
...
static AttributeList legalizeCallAttributes(CallBase *Call, bool IsMemIntrinsic,
  ...
  for (unsigned I : toolchain::seq(Call->arg_size()))
...
static void CreateGCRelocates(ArrayRef<Value *> LiveVariables,
  ...
  auto FindIndex = [](ArrayRef<Value *> LiveVec, Value *Val) {
    auto ValIt = toolchain::find(LiveVec, Val);
...
public:
  ...
  void doReplacement() {
    ...
    if (IsDeoptimize) {
      // Note: we've inserted instructions, so the call to toolchain.deoptimize may
...
makeStatepointExplicitImpl(CallBase *Call, /* to replace */
  ...
  // Instead of lowering calls to @toolchain.experimental.deoptimize as normal calls
  ...
  if (Function *F = dyn_cast<Function>(CallTarget.getCallee())) {
    ...
    if (IID == Intrinsic::experimental_deoptimize) {
      // Calls to toolchain.experimental.deoptimize are lowered to calls to the
      ...
      // calls to @toolchain.experimental.deoptimize with different argument types in
  ...
  if (IsDeoptimize) {
    // If we're wrapping an @toolchain.experimental.deoptimize in a statepoint, we
    ...
    if (!Call->getType()->isVoidTy() && !Call->use_empty()) {
      ...
      // toolchain::Instruction.  Instead, we defer the replacement and deletion to
...
static void relocationViaAlloca(
  ...
  for (auto Pair : AllocaMap) {
    ...
    toolchain::sort(Uses);
    auto Last = toolchain::unique(Uses);
...
static void rematerializeLiveValuesAtUses(
  ...
  for (auto &It : RematerizationCandidates) {
    ...
    if (toolchain::any_of(Cand->users(),
    ...
    unsigned NumLiveStatepoints = toolchain::count_if(
...
static void stripNonValidData(Module &M) {
#ifndef NDEBUG
  assert(toolchain::any_of(M, shouldRewriteStatepointsIn) && "precondition!");
#endif
