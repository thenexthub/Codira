...
private:
  ...
  /// (http://toolchain.org/docs/LangRef.html#id181).
  ///
...
FunctionPass *toolchain::createSeparateConstOffsetFromGEPPass(bool LowerGEP) {
...
Value *ConstantOffsetExtractor::applyCasts(Value *V) {
  ...
  for (CastInst *I : toolchain::reverse(CastInsts)) {
...
bool SeparateConstOffsetFromGEP::splitGEP(GetElementPtrInst *GEP) {
  ...
  // off-bound. http://toolchain.org/docs/LangRef.html#id181 says "if the
...
bool SeparateConstOffsetFromGEP::run(Function &F) {
  ...
  for (BasicBlock *B : RPOT) {
    ...
    for (Instruction &I : toolchain::make_early_inc_range(*B))
