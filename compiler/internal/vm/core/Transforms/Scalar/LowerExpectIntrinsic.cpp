...
// programmers may be using __builtin_expect() / toolchain.expect to annotate that a
...
// should emit @toolchain.expect intrinsics instead of using these weights directly.
...
static void handlePhiDef(CallInst *Expect) {
  ...
  auto ApplyOperations = [&](const APInt &Value) {
    APInt Result = Value;
    for (auto *Op : toolchain::reverse(Operations)) {
      switch (Op->getOpcode()) {
...
template <class BrSelInst> static bool handleBrSelExpect(BrSelInst &BSI) {
  ...
  //   %expval = call i64 @toolchain.expect.i64(i64 %conv1, i64 1)
  //   %tobool = icmp ne i64 %expval, 0
  //   br i1 %tobool, label %if.then, label %if.end
  //
  // Or the following simpler case:
  //   %expval = call i1 @toolchain.expect.i1(i1 %cmp, i1 1)
...
static bool lowerExpectIntrinsic(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    ...
    // Remove toolchain.expect intrinsics. Iterate backwards in order
    ...
    for (Instruction &Inst : toolchain::make_early_inc_range(toolchain::reverse(BB))) {
      ...
      if (Fn && (Fn->getIntrinsicID() == Intrinsic::expect ||
                 Fn->getIntrinsicID() == Intrinsic::expect_with_probability)) {
        // Before erasing the toolchain.expect, walk backward to find
        // phi that define toolchain.expect's first arg, and
