...
public:
  explicit MemmoveVerifier(const Value &LoadBasePtr, const Value &StoreBasePtr,
                           const DataLayout &DL)
      : DL(DL), BP1(toolchain::GetPointerBaseWithConstantOffset(
                    LoadBasePtr.stripPointerCasts(), LoadOff, DL)),
        BP2(toolchain::GetPointerBaseWithConstantOffset(
...
bool LoopIdiomRecognize::processLoopStoreOfLoopLoad(
  ...
  // FIXME: until toolchain.memcpy.inline supports dynamic sizes, we need to
  // conservatively bail here, since otherwise we may have to transform
  // toolchain.memcpy.inline into toolchain.memcpy which is illegal.
...
static bool detectShiftUntilLessThanIdiom(Loop *CurLoop, const DataLayout &DL,
  ...
  for (Instruction &Inst :
       toolchain::make_range(LoopEntry->getFirstNonPHIIt(), LoopEntry->end())) {
...
bool LoopIdiomRecognize::isProfitableToInsertFFS(Intrinsic::ID IntrinID,
  ...
  // @toolchain.dbg doesn't count as they have no semantic effect.
...
static bool detectShiftUntilBitTestIdiom(Loop *CurLoop, Value *&BaseX,
  ...
  auto MatchDecomposableConstantBitMask = [&]() {
    auto Res = toolchain::decomposeBitTestICmp(
...
///     %x.masked.numleadingzeros = call i32 @toolchain.ctlz.i32(i32 %x.masked,
///                                                         i1 true)
...
///     %extraoffset = <...>
///     <...>
///     %val.numleadingzeros = call i8 @toolchain.ct{l,t}z.i8(i8 %val, i1 0)
...
///     %iv.final = call i8 @toolchain.smax.i8(i8 %tmp, i8 %start)
