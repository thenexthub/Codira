...
#include "vm/core/Analysis/LoopUnrollAnalyzer.h"
...
cl::opt<bool> toolchain::ForgetSCEVInLoopUnroll(
...
TargetTransformInfo::UnrollingPreferences toolchain::gatherUnrollingPreferences(
  ...
  bool OptForSize = L->getHeader()->getParent()->hasOptSize() ||
                    ...
                    (hasUnrollTransformation(L) != TM_ForcedByUser &&
                     toolchain::shouldOptimizeForSize(L->getHeader(), PSI, BFI,
...
// "toolchain.loop.unroll.count").  If no such metadata node exists, then nullptr is
// returned.
...
static bool hasUnrollFullPragma(const Loop *L) {
  return getUnrollMetadataForLoop(L, "toolchain.loop.unroll.full");
}

// Returns true if the loop has an unroll(enable) pragma. This metadata is used
// for both "#pragma unroll" and "#pragma clang loop unroll(enable)" directives.
static bool hasUnrollEnablePragma(const Loop *L) {
  return getUnrollMetadataForLoop(L, "toolchain.loop.unroll.enable");
}

// Returns true if the loop has an runtime unroll(disable) pragma.
static bool hasRuntimeUnrollDisablePragma(const Loop *L) {
  return getUnrollMetadataForLoop(L, "toolchain.loop.unroll.runtime.disable");
}

// If loop has an unroll_count pragma return the (necessarily
// positive) value from the pragma.  Otherwise return 0.
static unsigned unrollCountPragmaValue(const Loop *L) {
  MDNode *MD = getUnrollMetadataForLoop(L, "toolchain.loop.unroll.count");
  if (MD) {
...
bool toolchain::computeUnrollCount(
...
tryToUnrollLoop(Loop *L, DominatorTree &DT, LoopInfo *LI, ScalarEvolution &SE,
  ...
  if (hasUnrollAndJamTransformation(L) == TM_ForcedByUser &&
    ...
    LLVM_DEBUG(
        dbgs()
        << "  Not unrolling loop since it has toolchain.loop.unroll_and_jam.\n");
    return LoopUnrollResult::Unmodified;
  }
...
Pass *toolchain::createLoopUnrollPass(int OptLevel, bool OnlyWhenForced,
