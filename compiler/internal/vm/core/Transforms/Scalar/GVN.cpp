...
#include "vm/core/Analysis/InstructionPrecedenceTracking.h"
...
#include "vm/core/Analysis/PHITransAddr.h"
...
struct toolchain::GVNPass::Expression {
  uint32_t Opcode;
...
template <> struct toolchain::DenseMapInfo<GVNPass::Expression> {
...
struct toolchain::gvn::AvailableValue {
  enum class ValType {
...
struct toolchain::gvn::AvailableValueInBlock {
...
bool GVNPass::PerformLoadPRE(LoadInst *Load, AvailValInBlkVect &ValuesPerBlock,
  ...
  SmallPtrSet<BasicBlock *, 4> Blockers(toolchain::from_range, UnavailableBlocks);
...
bool GVNPass::processMaskedLoad(IntrinsicInst *I) {
  ...
  Value *OpToForward = toolchain::SelectInst::Create(Mask, StoreVal, Passthrough, "",
                                                I->getIterator());
...
class toolchain::gvn::GVNLegacyPass : public FunctionPass {
public:
...
FunctionPass *toolchain::createGVNPass() { return new GVNLegacyPass(); }
