...
static bool isShapePreserving(Value *V) {
  ...
  if (auto *Cast = dyn_cast<CastInst>(V)) {
    ...
    case toolchain::Instruction::CastOpsEnd:
...
public:
  ...
  bool Visit() {
    ...
    for (auto *Inst : reverse(ToRemove)) {
      for (Use &U : toolchain::make_early_inc_range(Inst->uses())) {
  ...
  /// Lowers toolchain.matrix.column.major.load.
  ///
  ...
  /// Lowers toolchain.matrix.column.major.store.
  ///
  ...
  void lowerDotProduct(CallInst *MatMul,
    ...
    // Insert mul/fmul and toolchain.vector.reduce.fadd
    Value *Mul =
  ...
  void createTiledLoops(CallInst *MatMul, Value *LPtr, ShapeInfo LShape,
    ...
    addStringMetadataToLoop(LI->getLoopFor(TI.KLoop.Header),
                            "toolchain.loop.unroll.count", InnerLoopUnrollCount);
  }
  ...
  LowerMatrixMultiplyFused(CallInst *MatMul,
    ...
    if (LoadOp0 && LoadOp1 && Store) {
      ...
      // lifetime has been termined by @toolchain.lifetime.end), either sink them
      ...
      for (unsigned Idx = 0; Idx != LifetimeEnds.size();) {
        ...
        toolchain::scope_exit Inc([&Idx]() { Idx++; });
  ...
  /// Lowers toolchain.matrix.multiply.
  ...
  /// Lowers toolchain.matrix.transpose.
  ...
  MatrixTy VisitPHI(PHINode *Inst, const ShapeInfo &SI, IRBuilder<> &Builder) {
    ...
    for (auto [IncomingV, IncomingB] :
         toolchain::zip_equal(Inst->incoming_values(), Inst->blocks())) {
  ...
  MatrixTy VisitBinaryOperator(BinaryOperator *Inst, const ShapeInfo &SI,
    ...
    for (auto [AV, BV] : toolchain::zip_equal(A.vectors(), B.vectors()))
  ...
  MatrixTy VisitSelectInst(SelectInst *Inst, const ShapeInfo &Shape,
    ...
    if (isa<FixedVectorType>(Cond->getType())) {
      ...
      toolchain::copy(C.vectors(), std::back_inserter(CondV));
    } else {
      CondV.resize(A.getNumVectors());
      toolchain::fill(CondV, Cond);
    }

    for (auto [CV, AV, BV] : toolchain::zip_equal(CondV, A.vectors(), B.vectors()))
  ...
  struct ExprLinearizer {
    ...
    /// Write the called function name. Handles calls to toolchain.matrix.*
    ...
    void writeFnName(CallInst *CI) {
      if (!CI->getCalledFunction())
        ...
        if (!Name.starts_with("toolchain.matrix")) {
          write(Name);
          return;
        }
        auto *II = cast<IntrinsicInst>(CI);
        write(Intrinsic::getBaseName(II->getIntrinsicID())
                  .drop_front(StringRef("toolchain.matrix.").size()));
        write(".");
        std::string Tmp;
