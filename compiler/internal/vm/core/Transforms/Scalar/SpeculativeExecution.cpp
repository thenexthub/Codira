...
// if.then:
//   %x = mul i32 %y, %z
//   call void @toolchain.dbg.value(%x, !"x", !DIExpression())
//   call void foo()
//
...
// decide to hoist the 'toolchain.dbg.value' call.
...
FunctionPass *toolchain::createSpeculativeExecutionPass() {
  return new SpeculativeExecutionLegacyPass();
}

FunctionPass *toolchain::createSpeculativeExecutionIfHasBranchDivergencePass() {
