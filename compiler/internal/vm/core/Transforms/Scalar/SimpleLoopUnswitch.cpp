...
static cl::opt<bool> UnswitchGuards(
    ...
    cl::desc("If enabled, simple loop unswitching will also consider "
             "toolchain.experimental.guard intrinsics as unswitch candidates."));
...
static void replaceLoopInvariantUses(const Loop &L, Value *Invariant,
  ...
  for (Use &U : toolchain::make_early_inc_range(Invariant->uses())) {
...
static void hoistLoopToNewParent(Loop &L, BasicBlock &Preheader,
  ...
  for (Loop *OldContainingL = OldParentL; OldContainingL != NewParentL;
    ...
    toolchain::erase_if(OldContainingL->getBlocksVector(),
                   [&](const BasicBlock *BB) {
...
static BasicBlock *buildClonedLoopBlocks(
  ...
  for (auto *ExitBB : ExitBlocks) {
    ...
    for (auto ZippedInsts : toolchain::zip_first(
             toolchain::make_range(ExitBB->begin(), std::prev(ExitBB->end())),
             toolchain::make_range(ClonedExitBB->begin(),
...
static Loop *cloneLoopNest(Loop &OrigRootL, Loop *RootParentL,
  ...
  // back.
  for (Loop *ChildL : toolchain::reverse(OrigRootL))
  ...
  do {
    ...
    for (Loop *ChildL : toolchain::reverse(*L))
...
static void buildClonedLoops(Loop &OrigL, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  toolchain::sort(OrderedClonedExitsInLoops, [&](BasicBlock *LHS, BasicBlock *RHS) {
  ...
  for (auto *BB : toolchain::concat<BasicBlock *const>(
...
deleteDeadClonedBlocks(Loop &L, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  for (BasicBlock *BB : toolchain::concat<BasicBlock *const>(L.blocks(), ExitBlocks))
...
static void deleteDeadBlocksFromLoop(Loop &L,
  ...
  for (Loop *ParentL = &L; ParentL; ParentL = ParentL->getParentLoop()) {
    ...
    toolchain::erase_if(ParentL->getBlocksVector(),
                   [&](BasicBlock *BB) { return DeadBlockSet.count(BB); });
  }

  // Now delete the dead child loops. This raw delete will clear them
  // recursively.
  toolchain::erase_if(L.getSubLoopsVector(), [&](Loop *ChildL) {
    if (!DeadBlockSet.count(ChildL->getHeader()))
      return false;

    assert(toolchain::all_of(ChildL->blocks(),
                        [&](BasicBlock *ChildBB) {
...
static bool rebuildLoopAfterUnswitch(Loop &L, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  if (!LoopBlockSet.empty() && L.getParentLoop() != ParentL) {
    ...
    for (Loop *IL = L.getParentLoop(); IL != ParentL;
      ...
      toolchain::erase_if(IL->getBlocksVector(), [&](BasicBlock *BB) {
  ...
  toolchain::stable_sort(ExitsInLoops, [&](BasicBlock *LHS, BasicBlock *RHS) {
  ...
  auto RemoveUnloopedBlocksFromLoop =
      [](Loop &L, SmallPtrSetImpl<BasicBlock *> &UnloopedBlocks) {
        for (auto *BB : UnloopedBlocks)
          L.getBlocksSet().erase(BB);
        toolchain::erase_if(L.getBlocksVector(), [&](BasicBlock *BB) {
  ...
  if (Blocks.empty()) {
    ...
    if (Loop *ParentL = L.getParentLoop())
      ParentL->removeChildLoop(toolchain::find(*ParentL, &L));
    else
      LI.removeLoop(toolchain::find(LI, &L));
  ...
  if (CurrentLoopValid) {
    if (PartiallyInvariant) {
      ...
      MDNode *DisableUnswitchMD = MDNode::get(
          Context,
          MDString::get(Context, "toolchain.loop.unswitch.partial.disable"));
      MDNode *NewLoopID = makePostTransformationMetadata(
          Context, L.getLoopID(), {"toolchain.loop.unswitch.partial"},
          {DisableUnswitchMD});
      ...
      MDNode *DisableUnswitchMD = MDNode::get(
          Context,
          MDString::get(Context, "toolchain.loop.unswitch.injection.disable"));
      MDNode *NewLoopID = makePostTransformationMetadata(
          Context, L.getLoopID(), {"toolchain.loop.unswitch.injection"},
          {DisableUnswitchMD});
      L.setLoopID(NewLoopID);
    } else
...
static void unswitchNontrivialInvariants(
  ...
  for (auto *SuccBB : toolchain::concat<BasicBlock *const>(ArrayRef(RetainedSuccBB),
                                                      UnswitchedSuccBBs))
    if (SuccBB->getUniquePredecessor() ||
        toolchain::all_of(predecessors(SuccBB), [&](BasicBlock *PredBB) {
  ...
  for (Loop *UpdatedL :
       toolchain::concat<Loop *>(NonChildClonedLoops, HoistedLoops)) {
    UpdateLoop(*UpdatedL);
  ...
  for (Loop *UpdatedL : toolchain::concat<Loop *>(NonChildClonedLoops, HoistedLoops))
...
/// Turns a toolchain.experimental.guard intrinsic into implicit control flow branch,
/// making the following replacement:
///
///   --code before guard--
///   call void (i1, ...) @toolchain.experimental.guard(i1 %cond) [ "deopt"() ]
///   --code after guard--
///
/// into
///
...
///
/// deopt:
///   call void (i1, ...) @toolchain.experimental.guard(i1 false) [ "deopt"() ]
///   unreachable
///
/// It also makes all relevant DT and LI updates, so that all structures are in
/// valid state after this transform.
static BranchInst *turnGuardIntoBranch(IntrinsicInst *GI, Loop &L,
  ...
  DomTreeUpdater DTU(DT, DomTreeUpdater::UpdateStrategy::Eager);
  // toolchain.experimental.guard doesn't have branch weights. We can assume,
...
static int CalculateUnswitchCostMultiplier(
  ...
  if (DT.dominates(CondBlock, Latch) &&
      (isGuard(&TI) ||
       (TI.isTerminator() &&
        toolchain::count_if(successors(&TI), [&L](const BasicBlock *SuccBB) {
  ...
  int SiblingsCount =
      (ParentL ? ParentL->getSubLoopsVector().size() : toolchain::size(LI));
  ...
  for (const auto &Candidate : UnswitchCandidates) {
    ...
    int NonExitingSuccessors =
        toolchain::count_if(successors(CondBlock),
...
static bool collectUnswitchCandidates(
  ...
  if (MSSAU && !findOptionMDForLoop(&L, "toolchain.loop.unswitch.partial.disable") &&
    ...
    if (auto Info = hasPartialIVCondition(L, MSSAThreshold, *MSSA, AA)) {
      ...
      toolchain::append_range(ValsToDuplicate, Info->InstToDuplicate);
...
static bool unswitchBestCondition(Loop &L, DominatorTree &DT, LoopInfo &LI,
  ...
  if (!findOptionMDForLoop(&L, "toolchain.loop.unswitch.injection.disable"))
...
static bool unswitchLoop(Loop &L, DominatorTree &DT, LoopInfo &LI,
  ...
  // https://bugs.toolchain.org/show_bug.cgi?id=48819
