...
static bool sinkInstruction(
  ...
  if (BBsToSinkInto.size() > 1 &&
      !toolchain::set_is_subset(BBsToSinkInto, LoopBlockNumber))
    return false;
  ...
  toolchain::append_range(SortedBBsToSinkInto, BBsToSinkInto);
  if (SortedBBsToSinkInto.size() > 1) {
    toolchain::sort(SortedBBsToSinkInto, [&](BasicBlock *A, BasicBlock *B) {
...
static bool sinkLoopInvariantInstructions(Loop &L, AAResults &AA, LoopInfo &LI,
  ...
  toolchain::stable_sort(ColdLoopBBs, [&](BasicBlock *A, BasicBlock *B) {
