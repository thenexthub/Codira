...
#include "vm/core/Analysis/ConstraintSystem.h"
...
void State::addInfoFor(BasicBlock &BB) {
  ...
  for (Instruction &I : BB) {
    ...
    case Intrinsic::smin:
    case Intrinsic::smax:
      // TODO: handle toolchain.abs as well
      WorkList.push_back(
          FactOrCheck::getCheck(DT.getNode(&BB), cast<CallInst>(&I)));
      [[fallthrough]];
...
/// The reproducer function contains a series of @toolchain.assume calls, one for
...
static std::optional<bool> checkCondition(CmpInst::Predicate Pred, Value *A,
  ...
  toolchain::scope_exit InfoRestorer([&]() {
...
static bool eliminateConstraints(Function &F, DominatorTree &DT, LoopInfo &LI,
  ...
  bool Changed = false;
  DT.updateDFSNumbers();
  SmallVector<Value *> FunctionArgs(toolchain::make_pointer_range(F.args()));
  ...
  for (FactOrCheck &CB : S.WorkList) {
    ...
    if (!CB.isConditionFact()) {
      ...
      if (match(CB.Inst, m_Intrinsic<Intrinsic::abs>(m_Value(X)))) {
        // If is_int_min_poison is true then we may assume toolchain.abs >= 0.
