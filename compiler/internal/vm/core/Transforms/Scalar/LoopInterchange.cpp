...
struct LoopInterchange {
  ...
  bool processLoop(SmallVectorImpl<Loop *> &LoopList, unsigned InnerLoopId,
    ...
    toolchain::formLCSSARecursively(*OuterLoop, *DT, LI, SE);
...
bool LoopInterchangeLegality::isLoopStructureUnderstood() {
  ...
  if (CmpInst *InnerLoopCmp =
    ...
    IsPathToInnerIndVar = [this, &IsPathToInnerIndVar](const Value *V) -> bool {
      if (toolchain::is_contained(InnerLoopInductions, V))
        return true;
      if (isa<Constant>(V))
        return true;
  ...
  for (PHINode &PHI : L->getHeader()->phis()) {
    ...
    if (InductionDescriptor::isInductionPHI(&PHI, L, SE, ID))
      ...
      if (!InnerLoop) {
        ...
        if (!InnerRedPhi ||
            !toolchain::is_contained(InnerRedPhi->incoming_values(), &PHI)) {
          LLVM_DEBUG(
              dbgs()
    ...
    auto MoveInstructions = [&i, &WorkList, this, &InductionPHIs, NewLatch]() {
      for (; i < WorkList.size(); i++) {
        ...
        assert(!NewI->mayHaveSideEffects() &&
               ...
               "the loop nest!");
        for (Use &U : toolchain::make_early_inc_range(WorkList[i]->uses())) {
          Instruction *UserI = cast<Instruction>(U.getUser());
          if (!InnerLoop->contains(UserI->getParent()) ||
              UserI->getParent() == NewLatch ||
              toolchain::is_contained(InductionPHIs, UserI))
            U.set(NewI);
        }
        ...
        for (Value *Op : WorkList[i]->operands()) {
          ...
          if (!OpI ||
              ...
              toolchain::is_contained(InductionPHIs, OpI))
            continue;
          WorkList.insert(OpI);
        }
      }
...
static void updateSuccessor(BranchInst *BI, BasicBlock *OldBB,
  ...
  assert((!MustUpdateOnce || toolchain::count(successors(BI), OldBB) == 1) &&
...
static void moveLCSSAPhis(BasicBlock *InnerExit, BasicBlock *InnerHeader,
  ...
  SmallVector<PHINode *, 8> LcssaInnerExit(
      toolchain::make_pointer_range(InnerExit->phis()));

  SmallVector<PHINode *, 8> LcssaInnerLatch(
      toolchain::make_pointer_range(InnerLatch->phis()));
...
static void simplifyLCSSAPhis(Loop *OuterLoop, Loop *InnerLoop) {
  ...
  SmallVector<PHINode *, 8> Phis(
      toolchain::make_pointer_range(OuterLoopLatch->phis()));
...
bool LoopInterchangeTransform::adjustLoopBranches() {
  ...
  if (toolchain::is_contained(OuterLoopHeaderBI->successors(), OuterLoopLatch)) {
