...
// Prefix of "toolchain.loop.unroll." returns true if we have any unroll metadata.
...
static bool hasUnrollAndJamEnablePragma(const Loop *L) {
  return getUnrollMetadataForLoop(L, "toolchain.loop.unroll_and_jam.enable");
}

// If loop has an unroll_and_jam_count pragma return the (necessarily
// positive) value from the pragma.  Otherwise return 0.
static unsigned unrollAndJamCountPragmaValue(const Loop *L) {
  MDNode *MD = getUnrollMetadataForLoop(L, "toolchain.loop.unroll_and_jam.count");
  if (MD) {
...
tryToUnrollAndJamLoop(Loop *L, DominatorTree &DT, LoopInfo *LI,
  ...
  // as unrolling
  if (hasAnyUnrollPragma(L, "toolchain.loop.unroll.") &&
      !hasAnyUnrollPragma(L, "toolchain.loop.unroll_and_jam.")) {
