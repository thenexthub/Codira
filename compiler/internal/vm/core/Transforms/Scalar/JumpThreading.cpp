...
void JumpThreadingPass::findLoopHeaders(Function &F) {
  ...
  LoopHeaders.insert_range(toolchain::make_second_range(Edges));
}
...
bool JumpThreadingPass::simplifyPartiallyRedundantLoad(LoadInst *LoadI) {
  ...
  if (PredsScanned.size() == AvailablePreds.size()+1 &&
    ...
    SmallPtrSet<BasicBlock *, 8> AvailablePredSet(
        toolchain::from_range, toolchain::make_first_range(AvailablePreds));
  ...
  for (BasicBlock *P : predecessors(LoadBB)) {
    AvailablePredsTy::iterator I =
        toolchain::lower_bound(AvailablePreds, std::make_pair(P, (Value *)nullptr));
...
findMostPopularDest(BasicBlock *BB,
  ...
  // same order in toolchain::max_element below.  We map nullptr to 0 so that we can
  ...
  auto MostPopular = toolchain::max_element(DestPopularity, toolchain::less_second());
...
Constant *JumpThreadingPass::evaluateOnPredecessorEdge(
  ...
  if (!Visited.insert(V).second)
    return nullptr;
  toolchain::scope_exit _([&Visited, V]() { Visited.erase(V); });
...
void JumpThreadingPass::updateSSA(BasicBlock *BB, BasicBlock *NewBB,
  ...
  for (Instruction &I : *BB) {
    ...
    toolchain::erase_if(DbgVariableRecords, [&](const DbgVariableRecord *DbgVarRec) {
...
bool JumpThreadingPass::maybethreadThroughTwoBasicBlocks(BasicBlock *BB,
  ...
  if (toolchain::is_contained(successors(PredBB), PredBB))
    return false;
...
void JumpThreadingPass::updateBlockFreqAndEdgeWeight(BasicBlock *PredBB,
  ...
  uint64_t MaxBBSuccFreq = *toolchain::max_element(BBSuccFreq);
...
bool JumpThreadingPass::tryToUnfoldSelectInCurrBB(BasicBlock *BB) {
  ...
  for (BasicBlock::iterator BI = BB->begin();
    ...
    if (toolchain::all_of(PN->incoming_values(),
...
/// Merge:
///   %condGuard = ...
///   call void(i1, ...) @toolchain.experimental.guard( i1 %condGuard )[ "deopt"() ]
///
