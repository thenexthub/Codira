...
bool BCECmpBlock::canSinkBCECmpInst(const Instruction *Inst,
  ...
  return toolchain::none_of(Inst->operands(), [&](const Value *Op) {
...
void BCECmpBlock::split(BasicBlock *NewParent, AliasAnalysis &AA) const {
  toolchain::SmallVector<Instruction *, 4> OtherInsts;
...
mergeBlocks(std::vector<BCECmpBlock> &&Blocks) {
  ...
  toolchain::sort(MergedBlocks, [](const BCECmpChain::ContiguousBlocks &LhsBlocks,
...
static BasicBlock *mergeComparisons(ArrayRef<BCECmpBlock> Comparisons,
  ...
  const auto *ToSplit = toolchain::find_if(
...
static bool runImpl(Function &F, const TargetLibraryInfo &TLI,
  ...
  for (BasicBlock &BB : toolchain::drop_begin(F)) {
...
Pass *toolchain::createMergeICmpsLegacyPass() { return new MergeICmpsLegacyPass(); }
