...
#include "vm/core/Analysis/RegionPass.h"
...
#include "vm/core/Transforms/Utils/SSAUpdaterBulk.h"
#include <cassert>
#include <utility>
...
void StructurizeCFG::findUndefBlocks(
  ...
  while (!Stack.empty()) {
    ...
    if (FlowSet.contains(Current))
      toolchain::append_range(Stack, predecessors(Current));
...
void StructurizeCFG::setPhiValues() {
  ...
  for (const auto &[To, From] : AddedPhis) {
    ...
    if (!BlkPhis.empty()) {
      Incomings.insert_range(toolchain::make_first_range(BlkPhis.front().second));
...
void StructurizeCFG::changeExit(RegionNode *Node, BasicBlock *NewExit,
  ...
  if (Node->isSubRegion()) {
    ...
    for (BasicBlock *BB : toolchain::make_early_inc_range(predecessors(OldExit))) {
...
bool StructurizeCFG::dominatesPredicates(BasicBlock *BB, RegionNode *Node) {
  ...
  return toolchain::all_of(Preds, [&](std::pair<BasicBlock *, PredInfo> Pred) {
...
bool StructurizeCFG::run(Region *R, DominatorTree *DT,
  ...
  // See `toolchain/test/Transforms/StructurizeCFG/callbr.ll` for details.
...
Pass *toolchain::createStructurizeCFGPass(bool SkipUniformRegions) {
