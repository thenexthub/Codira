...
#include "vm/core/IR/PredIteratorCache.h"
...
cl::opt<unsigned> toolchain::SetLicmMssaOptCap(
...
// compile time.
cl::opt<unsigned> toolchain::SetLicmMssaNoAccForPromotionCap(
...
Pass *toolchain::createLICMPass() { return new LegacyLICMPass(); }

toolchain::SinkAndHoistLICMFlags::SinkAndHoistLICMFlags(bool IsSink, Loop &L,
                                                   MemorySSA &MSSA)
    : SinkAndHoistLICMFlags(SetLicmMssaOptCap, SetLicmMssaNoAccForPromotionCap,
                            IsSink, L, MSSA) {}

toolchain::SinkAndHoistLICMFlags::SinkAndHoistLICMFlags(
...
bool LoopInvariantCodeMotion::runOnLoop(Loop *L, AAResults *AA, LoopInfo *LI,
  ...
  // across that edge.
  bool HasCoroSuspendInst = toolchain::any_of(L->getBlocks(), [](BasicBlock *BB) {
  ...
  if (!DisablePromotion && Preheader && L->hasDedicatedExits() &&
    ...
    bool HasCatchSwitch = toolchain::any_of(ExitBlocks, [](BasicBlock *Exit) {
...
///
bool toolchain::sinkRegion(DomTreeNode *N, AAResults *AA, LoopInfo *LI,
...
bool toolchain::sinkRegionForLoopNest(DomTreeNode *N, AAResults *AA, LoopInfo *LI,
                                 DominatorTree *DT, TargetLibraryInfo *TLI,
                                 TargetTransformInfo *TTI, Loop *CurLoop,
                                 MemorySSAUpdater &MSSAU,
                                 ICFLoopSafetyInfo *SafetyInfo,
...
public:
  ...
  void registerPossiblyHoistableBranch(BranchInst *BI) {
    ...
    SmallPtrSet<BasicBlock *, 4> TrueDestSucc(toolchain::from_range,
                                              successors(TrueDest));
    SmallPtrSet<BasicBlock *, 4> FalseDestSucc(toolchain::from_range,
    ...
    if (TrueDestSucc.count(FalseDest)) {
      ...
      // unpredicatable which element gets returned).
        ...
        auto It = toolchain::find_if(*F, IsSucc);
  ...
  bool canHoistPHI(PHINode *PN) {
    ...
    SmallPtrSet<BasicBlock *, 8> PredecessorBlocks(toolchain::from_range,
                                                   predecessors(BB));
  ...
  BasicBlock *getOrCreateHoistedBlock(BasicBlock *BB) {
    ...
    auto It = toolchain::find_if(HoistableBranches, HasBBAsSuccessor);
...
///
bool toolchain::hoistRegion(DomTreeNode *N, AAResults *AA, LoopInfo *LI,
                       DominatorTree *DT, AssumptionCache *AC,
  ...
  for (BasicBlock *BB : Worklist) {
    ...
    for (Instruction &I : toolchain::make_early_inc_range(*BB)) {
      ...
      if (I.getOpcode() == Instruction::FDiv && I.hasAllowReciprocal() &&
        ...
        auto One = toolchain::ConstantFP::get(Divisor->getType(), 1.0);
...
bool toolchain::canSinkOrHoistInst(Instruction &I, AAResults *AA, DominatorTree *DT,
...
static void splitPredecessorsOfLoopExit(PHINode *PN, DominatorTree *DT,
  ...
  SmallPtrSet<BasicBlock *, 32> ExitBlockSet(toolchain::from_range, ExitBlocks);
#endif
...
/// loop invariant.
///
bool toolchain::promoteLoopAccessesToScalars(
  ...
  for (Value *ASIV : PointerMustAliases) {
    for (Use &U : ASIV->uses()) {
      ...
      if (LoadInst *Load = dyn_cast<LoadInst>(UI)) {
        ...
        if (StoreSafety == StoreSafetyUnknown &&
            toolchain::all_of(ExitBlocks, [&](BasicBlock *Exit) {
...
collectPromotionCandidates(MemorySSA *MSSA, AliasAnalysis *AA, Loop *L) {
  ...
  foreachMemoryAccess(MSSA, L, [&](Instruction *I) {
    ...
    toolchain::erase_if(Sets, [&](PointerIntPair<const AliasSet *, 1, bool> &Pair) {
...
static bool hoistAdd(ICmpInst::Predicate Pred, Value *VariantLHS,
  ...
  if (IsSigned && computeOverflowForSignedSub(InvariantRHS, InvariantOp, SQ) !=
                      toolchain::OverflowResult::NeverOverflows)
    return false;
  if (!IsSigned &&
