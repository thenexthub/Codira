...
#include "vm/core/ADT/SparseBitVector.h"
...
#include "vm/core/Analysis/CFGPrinter.h"
...
#include "vm/core/Support/ArrayRecycler.h"
#include "vm/core/Support/Casting.h"
#include "vm/core/Support/CommandLine.h"
#include "vm/core/Support/Debug.h"
#include "vm/core/Support/DebugCounter.h"
#include "vm/core/Support/ErrorHandling.h"
#include "vm/core/Support/PointerLikeTypeTraits.h"
#include "vm/core/Support/raw_ostream.h"
#include "vm/core/Transforms/Scalar/GVNExpression.h"
...
#include "vm/core/Transforms/Utils/VNCoercion.h"
#include <algorithm>
#include <cassert>
#include <cstdint>
#include <iterator>
...
using namespace vm::core::GVNExpression;
using namespace vm::core::VNCoercion;
...
public:
  ...
  bool isEquivalentTo(const CongruenceClass *Other) const {
    ...
    return toolchain::set_is_subset(Members, Other->Members);
  }
...
template <> struct toolchain::DenseMapInfo<const Expression *> {
...
void NewGVN::sortPHIOps(MutableArrayRef<ValPair> Ops) const {
  toolchain::sort(Ops, [&](const ValPair &P1, const ValPair &P2) {
...
PHIExpression *NewGVN::createPHIExpression(ArrayRef<ValPair> PHIOperands,
  ...
  toolchain::transform(Filtered, op_inserter(E), [&](const ValPair &P) -> Value * {
...
NewGVN::createAggregateValueExpression(Instruction *I) const {
  if (auto *II = dyn_cast<InsertValueInst>(I)) {
    ...
    toolchain::copy(II->indices(), int_op_inserter(E));
    return E;
    ...
    E->allocateIntOperands(ExpressionAllocator);
    toolchain::copy(EI->indices(), int_op_inserter(E));
    return E;
  }
...
bool NewGVN::someEquivalentDominates(const Instruction *Inst,
  ...
  if (CC->getNextLeader().first &&
    ...
    return true;
  return toolchain::any_of(*CC, [&](const Value *Member) {
...
bool NewGVN::isCycleFree(const Instruction *I) const {
  ...
  if (ICS == ICS_Unknown) {
    ...
    if (SCC.size() == 1)
      ...
      bool AllPhis = toolchain::all_of(SCC, [](const Value *V) {
...
NewGVN::performSymbolicPHIEvaluation(ArrayRef<ValPair> PHIOps,
  ...
  if (toolchain::all_of(Filtered, equal_to(AllSameValue))) {
...
bool NewGVN::runGVN() {
  ...
  for (auto &B : RPOT) {
    ...
    if (Node->getNumChildren() > 1)
      toolchain::sort(*Node, [&](const DomTreeNode *A, const DomTreeNode *B) {
...
bool NewGVN::eliminateInstructions(Function &F) {
  ...
  for (auto *CC : reverse(CongruenceClasses)) {
    ...
    if (CC->getStoreCount() > 0) {
      ...
      toolchain::sort(PossibleDeadStores);
      ValueDFSStack EliminationStack;
