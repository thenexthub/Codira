...
#include "vm/core/Analysis/PtrUseVisitor.h"
...
#include "vm/core/IR/ConstantFolder.h"
...
public:
  ...
  // When this access is via an toolchain.protected.field.ptr intrinsic, contains
...
public:
  ...
  /// Access the users for this alloca that are toolchain.protected.field.ptr
  /// intrinsics.
...
private:
  ...
  /// Users that are toolchain.protected.field.ptr intrinsics. These will be RAUW'd
  ...
  void advance() {
    ...
    if (!P.SplitTails.empty()) {
      if (P.EndOffset >= MaxSplitSliceEndOffset) {
        ...
        toolchain::erase_if(P.SplitTails,
                       [&](Slice *S) { return S->endOffset() <= P.EndOffset; });
        assert(toolchain::any_of(P.SplitTails,
                            [&](Slice *S) {
                              return S->endOffset() == MaxSplitSliceEndOffset;
                            }) &&
               "Could not find the current max split slice offset!");
        assert(toolchain::all_of(P.SplitTails,
                            [&](Slice *S) {
                              return S->endOffset() <= MaxSplitSliceEndOffset;
                            }) &&
...
private:
  ...
  void visitIntrinsicInst(IntrinsicInst &II) {
    ...
    if (II.getIntrinsicID() == Intrinsic::protected_field_ptr) {
      // We only handle loads and stores as users of toolchain.protected.field.ptr.
...
AllocaSlices::AllocaSlices(const DataLayout &DL, AllocaInst &AI)
  ...
  toolchain::erase_if(Slices, [](const Slice &S) { return S.isDead(); });

  // Sort the uses. This arranges for the offsets to be in ascending order,
  // and the sizes to be in descending order.
  toolchain::stable_sort(Slices);
}
...
checkVectorTypesForPromotion(Partition &P, const DataLayout &DL,
  ...
  if (!HaveCommonEltTy && HaveVecPtrTy) {
    ...
    toolchain::sort(CandidateTys, RankVectorTypesComp);
    CandidateTys.erase(toolchain::unique(CandidateTys, RankVectorTypesEq),
                       CandidateTys.end());
  } else {
...
// have the same vector type. Check that and remove all but one.
  ...
  toolchain::erase_if(CandidateTys, [](VectorType *VTy) {
  ...
  auto *VTy = toolchain::find_if(CandidateTys, [&](VectorType *VTy) -> bool {
    uint64_t ElementSize =
...
static Value *extractVector(IRBuilderTy &IRB, Value *V, unsigned BeginIndex,
  ...
  auto Mask = toolchain::to_vector<8>(toolchain::seq<int>(BeginIndex, EndIndex));
...
public:
  ...
  rewriteTreeStructuredMerge(Partition &P) {
    ...
    toolchain::sort(StoreInfos, [](const StoreInfo &A, const StoreInfo &B) {
    ...
    while (VecElements.size() > 1) {
      ...
      for ([[maybe_unused]] const auto _ : toolchain::seq(NumElts / 2)) {
    ...
    if (!IsSplittable) {
      ...
      if (IsDest) {
        ...
        for (DbgVariableRecord *DbgAssign : at::getDVRAssignmentMarkers(&II)) {
          if (toolchain::is_contained(DbgAssign->location_ops(), II.getDest()) ||
...
bool SROA::presplitLoadsAndStores(AllocaInst &AI, AllocaSlices &AS) {
  ...
  // any rewriting.
  toolchain::erase_if(Stores, [&UnsplittableLoads, &SplitOffsetsMap](StoreInst *SI) {
  ...
  toolchain::erase_if(Stores, [&UnsplittableLoads](StoreInst *SI) {
    auto *LI = cast<LoadInst>(SI->getValueOperand());
    return UnsplittableLoads.count(LI);
  });
  // Once we've established all the loads that can't be split for some reason,
  // filter any that made it into our list out.
  toolchain::erase_if(Loads, [&UnsplittableLoads](LoadInst *LI) {
    return UnsplittableLoads.count(LI);
  });
  ...
  toolchain::erase_if(AS, [](const Slice &S) { return S.isDead(); });
...
AllocaInst *SROA::rewritePartition(AllocaInst &AI, AllocaSlices &AS,
  ...
  if (Promotable) {
    ...
    if (PHIUsers.empty() && SelectUsers.empty()) {
      ...
      for (auto &&KV : toolchain::make_range(
...
FunctionPass *toolchain::createSROAPass(bool PreserveCFG) {
