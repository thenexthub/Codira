...
bool InstCombinerImpl::SimplifyDemandedBits(Instruction *I, unsigned OpNo,
  ...
  Value *V = U.get();
  if (isa<Constant>(V)) {
    toolchain::computeKnownBits(V, Known, Q, Depth);
    return false;
  }
...
Value *InstCombinerImpl::SimplifyDemandedUseBits(Instruction *I,
  ...
  switch (I->getOpcode()) {
  default:
    toolchain::computeKnownBits(I, Known, Q, Depth);
    break;
  ...
  case Instruction::Call: {
    ...
    if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
      ...
      case Intrinsic::ptrmask: {
        ...
        // `toolchain.ptrmask(p, 0)`. We can't return `null` here due to pointer
  ...
  if (VerifyKnownBits) {
    KnownBits ReferenceKnown = toolchain::computeKnownBits(I, Q, Depth);
...
Value *InstCombinerImpl::SimplifyMultipleUseDemandedBits(
  ...
  switch (I->getOpcode()) {
  case Instruction::And: {
    toolchain::computeKnownBits(I->getOperand(1), RHSKnown, Q, Depth + 1);
    toolchain::computeKnownBits(I->getOperand(0), LHSKnown, Q, Depth + 1);
    Known = analyzeKnownBitsFromAndXorOr(cast<Operator>(I), LHSKnown, RHSKnown,
                                         Q, Depth);
...
Value *InstCombinerImpl::SimplifyDemandedUseFPClass(Instruction *I,
  ...
  case Instruction::FNeg: {
    if (SimplifyDemandedFPClass(I, 0, toolchain::fneg(DemandedMask), Known,
                                Depth + 1))
      return I;
    Known.fneg();
    break;
  ...
  case Instruction::Call: {
    ...
    switch (IID) {
    case Intrinsic::fabs:
      if (SimplifyDemandedFPClass(I, 0, toolchain::inverse_fabs(DemandedMask), Known,
                                  Depth + 1))
        return I;
    ...
    case Intrinsic::copysign: {
      ...
      const FPClassTest DemandedMaskAnySign = toolchain::unknown_sign(DemandedMask);
