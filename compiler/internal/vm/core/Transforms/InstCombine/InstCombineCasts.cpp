...
private:
  ...
  [[nodiscard]] bool allPendingVisited() const {
    return toolchain::all_of(Pending,
  ...
  canEvaluate(Value *V, Type *Ty,
              toolchain::function_ref<bool(Value *, Type *Type)> Pred) {
    ...
    if (!I->hasOneUse())
      toolchain::append_range(Pending, I->users());
...
bool TypeEvaluationHelper::canEvaluateTruncatedPred(Value *V, Type *Ty,
  ...
  case Instruction::Shl: {
    ...
    KnownBits AmtKnownBits =
        toolchain::computeKnownBits(I->getOperand(1), IC.getDataLayout());
...
Instruction *InstCombinerImpl::narrowFunnelShift(TruncInst &Trunc) {
  ...
  // This prepares ShAmt for toolchain.fshl.i8(trunc(ShVal), trunc(ShVal),
  // zext/trunc(ShAmt)).
...
bool TypeEvaluationHelper::canEvaluateZExtdImpl(Value *V, Type *Ty,
  ...
  case Instruction::Call:
    // toolchain.vscale() can always be executed in larger type, because the
...
optimizeVectorResizeWithIntegerBitCasts(Value *InVal, VectorType *DestTy,
  ...
  // size of the input.
  auto ShuffleMaskStorage = toolchain::to_vector<16>(toolchain::seq<int>(0, SrcElts));
