...
/// \code
///   %0 = tail call i32 @toolchain.cttz.i32(i32 %x, i1 true)
///   %1 = icmp ne i32 %x, 0
///   %2 = select i1 %1, i32 %0, i32 32
/// \code
///
/// into:
///   %0 = tail call i32 @toolchain.cttz.i32(i32 %x, i1 false)
...
/// Folds:
///   %a_sub = call @toolchain.usub.sat(x, IntConst1)
///   %b_sub = call @toolchain.usub.sat(y, IntConst2)
///   %or = or %a_sub, %b_sub
///   %cmp = icmp eq %or, 0
...
///              --> call toolchain.fshl.i32(a, a, b)
/// fshl32(a, b, c) --> (c == 0 ? a : ((b >> (32 - c)) | (a << c)))
///                 --> call toolchain.fshl.i32(a, b, c)
/// fshr32(a, b, c) --> (c == 0 ? b : ((a >> (32 - c)) | (b << c)))
///                 --> call toolchain.fshr.i32(a, b, c)
static Instruction *foldSelectFunnelShift(SelectInst &Sel,
  ...
  if (SV0 != SV1) {
    if (IsFshl && !toolchain::isGuaranteedNotToBePoison(SV1))
      SV1 = Builder.CreateFreeze(SV1);
    else if (!IsFshl && !toolchain::isGuaranteedNotToBePoison(SV0))
...
//
//   %dec = add i32 %x, -1
//   %ctlz = tail call i32 @toolchain.ctlz.i32(i32 %dec, i1 false)
//   %sub = sub i32 32, %ctlz
//   %shl = shl i32 1, %sub
  ...
  if (SelType->isIntegerTy() &&
    ...
    if (!CC.AffectedValues.empty()) {
      if (!isa<Constant>(TrueVal) &&
        ...
        KnownBits Known = toolchain::computeKnownBits(TrueVal, Q);
        if (Known.isConstant())
      ...
      if (!isa<Constant>(FalseVal) &&
        ...
        KnownBits Known = toolchain::computeKnownBits(FalseVal, Q);
        if (Known.isConstant())
