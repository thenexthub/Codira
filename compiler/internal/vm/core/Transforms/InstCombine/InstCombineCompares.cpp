...
///   sum = a + b
///   if (sum+128 >u 255)
/// Then replace it with toolchain.sadd.with.overflow.i8.
///
static Instruction *processUGT_ADDCST_ADD(ICmpInst &I, Value *A, Value *B,
                                          ConstantInt *CI2, ConstantInt *CI1,
                                          InstCombinerImpl &IC) {
  // The transformation we're trying to do here is to transform this into an
  // toolchain.sadd.with.overflow.  To do this, we have to replace the original add
  ...
  // toolchain.sadd.with.overflow, the only uses allowed are the add-with-constant
...
Instruction *InstCombinerImpl::foldICmpWithConstant(ICmpInst &Cmp) {
  ...
  //
  // sum = a + b
  // if (sum+128 >u 255)  ...  -> toolchain.sadd.with.overflow.i8
  CmpInst::Predicate Pred = Cmp.getPredicate();
...
Instruction *InstCombinerImpl::foldICmpAddConstant(ICmpInst &Cmp,
  ...
  if (Pred == CmpInst::ICMP_ULT && C2->isAllOnes()) {
    const SimplifyQuery Q = SQ.getWithInstruction(&Cmp);
    if (toolchain::isKnownNonZero(X, Q))
...
Instruction *InstCombinerImpl::foldICmpBitCast(ICmpInst &Cmp) {
  ...
  if (SrcType->isVectorTy() == DstType->isVectorTy() &&
    ...
    if (match(Op1, m_APInt(C)) && Bitcast->hasOneUse()) {
      ...
      // icmp eq/ne (bitcast X to int), special fp -> toolchain.is.fpclass(X, class)
...
///   ((x * y) ?/ x) != y
/// to
///   @toolchain.?mul.with.overflow(x, y) plus extraction of overflow bit
...
Instruction *InstCombinerImpl::foldICmpWithTrunc(ICmpInst &ICmp) {
  ...
  if (auto *II = dyn_cast<IntrinsicInst>(X)) {
    if (II->getIntrinsicID() == Intrinsic::cttz ||
      ...
      // the intrinsic.
      if (toolchain::Log2_32(MaxRet) + 1 <= Op0->getType()->getScalarSizeInBits())
        if (Instruction *I =
...
/// toolchain.vector.reduce.and which are lowered into instruction chains. This code
...
static Instruction *foldReductionIdiom(ICmpInst &I,
  ...
  // Match lowering of @toolchain.vector.reduce.and. Turn
...
Instruction *InstCombinerImpl::visitICmpInst(ICmpInst &I) {
...

  {
    ...
    // (zext X) * (zext Y)  --> toolchain.umul.with.overflow.
