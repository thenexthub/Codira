...
// toolchain.dbg.declare to avoid inaccurate debug info. Setting this to false
...
Value *InstCombinerImpl::EmitGEPOffset(GEPOperator *GEP, bool RewriteGEP) {
  if (!RewriteGEP)
    return toolchain::emitGEPOffset(&Builder, DL, GEP);
...
Value *InstCombinerImpl::EmitGEPOffsets(ArrayRef<GEPOperator *> GEPs,
  ...
  for (GEPOperator *GEP : reverse(GEPs)) {
    Value *Offset;
    {
      ...
      Offset = toolchain::emitGEPOffset(&Builder, DL, GEP);
...
void InstCombinerImpl::freelyInvertAllUsersOf(Value *I, Value *IgnoredUser) {
  ...
  toolchain::findDbgValues(I, DbgVariableRecords);
...
static bool shouldCanonicalizeGEPToPtrAdd(GetElementPtrInst &GEP) {
  ...
  // Canonicalize scalable GEPs to an explicit offset using the toolchain.vscale
...
Instruction *InstCombinerImpl::visitGetElementPtrInst(GetElementPtrInst &GEP) {
  ...
  if (GEPType->isVectorTy() && toolchain::any_of(GEP.operands(), [](Value *Op) {
...
Instruction *InstCombinerImpl::visitAllocSite(Instruction &MI) {
  ...
  if (Removable) {
    for (WeakTrackingVH &User : Users) {
      // Lowering all @toolchain.objectsize and MTI calls first because they may use
...
static Instruction *tryToMoveFreeBeforeNullTest(CallInst &FI,
  ...
  // before TI.
  for (Instruction &Instr : toolchain::make_early_inc_range(*FreeInstrBB)) {
...
bool InstCombinerImpl::removeInstructionsBeforeUnreachable(Instruction &I) {
  ...
  // This includes instructions like stores and "toolchain.assume" that may not get
...
void InstCombinerImpl::tryToSinkInstructionDbgVariableRecords(
  ...
  toolchain::stable_sort(DbgVariableRecordsToSink, Order);
  ...
  if (DbgVariableRecordsToSink.size() > 1) {
    ...
    for (const Instruction *Inst : DupSet) {
      for (DbgVariableRecord &DVR :
           toolchain::reverse(filterDbgVars(Inst->getDbgRecordRange()))) {
        DebugVariable DbgUserVariable =
            DebugVariable(DVR.getVariable(), DVR.getExpression(),
                          DVR.getDebugLoc()->getInlinedAt());
        auto FilterIt =
...
// @toolchain.experimental.noalias.scope.decl is only useful if that scope is used
...
public:
  ...
  bool isNoAliasScopeDeclDead(Instruction *Inst) {
    ...
    assert(Decl->use_empty() &&
           "toolchain.experimental.noalias.scope.decl in use ?");
    const MDNode *MDSL = Decl->getScopeList();
    assert(MDSL->getNumOperands() == 1 &&
           "toolchain.experimental.noalias.scope should refer to a single scope");
...
bool InstCombinerImpl::prepareWorklist(Function &F) {
  ...
  for (BasicBlock *BB : RPOT) {
    ...
    for (Instruction &Inst : toolchain::make_early_inc_range(*BB)) {
...
void toolchain::initializeInstCombine(PassRegistry &Registry) {
  initializeInstructionCombiningPassPass(Registry);
}

FunctionPass *toolchain::createInstructionCombiningPass() {
