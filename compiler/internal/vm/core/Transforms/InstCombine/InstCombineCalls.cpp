...
#include "vm/core/Transforms/InstCombine/InstCombiner.h"
...
#include "vm/core/Transforms/Utils/InstructionWorklist.h"
...
Instruction *InstCombinerImpl::SimplifyAnyMemSet(AnyMemSetInst *MI) {
  ...
  if (Len <= 8 && isPowerOf2_32((uint32_t)Len)) {
    ...
    for (DbgVariableRecord *DbgAssign : at::getDVRAssignmentMarkers(S)) {
      if (toolchain::is_contained(DbgAssign->location_ops(), FillC))
...
//
//   call @toolchain.foo.start(i1 0)
//   call @toolchain.foo.start(i1 0) ; This one won't be skipped: it will be removed
//   call @toolchain.foo.end(i1 0)
//   call @toolchain.foo.end(i1 0) ; &I
static bool
...
/// toolchain.is.fpclass(x, \p Mask) is equivalent to fcmp o__ x, 0.0 with the
...
///   %umax = call i8 @toolchain.umax.i8(i8 %umax.acc, i8 %b)
...
static Value *foldIdempotentBinaryIntrinsicRecurrence(InstCombinerImpl &IC,
  ...
  // `call @toolchain.binary.intrinsic(Init, OtherOp)`.
...
Instruction *InstCombinerImpl::visitCallInst(CallInst &CI) {
  ...
  case Intrinsic::ptrauth_resign: {
    ...
    if (const auto *CI = dyn_cast<CallBase>(Ptr)) {
      ...
      // ptrauth constants are equivalent to a call to @toolchain.ptrauth.sign for
  ...
  case Intrinsic::stackrestore: {
    ...
    for (++BI; &*BI != TI; ++BI) {
      ...
      case ClassifyResult::CallWithSideEffects:
        ...
        // with side effects (such as toolchain.stacksave and toolchain.read_register),
  ...
  case Intrinsic::assume: {
    ...
    // call void @toolchain.assume(i1 %A)
    // into
    // call void @toolchain.assume(i1 true) [ "nonnull"(i32* %PTR) ]
    ...
    // %D = icmp eq i64 %C, 0
    // call void @toolchain.assume(i1 %D)
    // into
    // call void @toolchain.assume(i1 true) [ "align"(i32* [[A]], i64  Constant + 1)]
    ...
    if (EnableKnowledgeRetention && II->hasOperandBundles()) {
      for (unsigned Idx = 0; Idx < II->getNumOperandBundles(); Idx++) {
        ...
        RetainedKnowledge RK =
          toolchain::getKnowledgeFromBundle(cast<AssumeInst>(*II), BOI);
        ...
        RetainedKnowledge CanonRK =
          toolchain::simplifyRetainedKnowledge(cast<AssumeInst>(II), RK,
                                          &getAssumptionCache(),
                                          &getDominatorTree());
        if (CanonRK == RK)
          continue;
...
static Value *optimizeModularFormat(CallInst *CI, IRBuilderBase &B) {
  ...
  SmallVector<StringRef> Args(
      toolchain::split(CI->getFnAttr("modular-format").getValueAsString(), ','));
  ...
  for (StringRef Aspect : AllAspects) {
    if (Aspect == "float") {
      if (toolchain::any_of(
              toolchain::make_range(std::next(CI->arg_begin(), FirstArgIdx),
                               CI->arg_end()),
...
// Given a call to toolchain.adjust.trampoline, find and return the corresponding
// call to toolchain.init.trampoline if the call to the trampoline can be optimized
...
bool InstCombinerImpl::annotateAnyAllocSite(CallBase &Call,
  ...
  if (AlignOpC && AlignOpC->getValue().ult(toolchain::Value::MaximumAlignment)) {
    uint64_t AlignmentVal = AlignOpC->getZExtValue();
    if (toolchain::isPowerOf2_64(AlignmentVal)) {
...
Instruction *InstCombinerImpl::visitCallBase(CallBase &Call) {
  ...
  if (CalleeF) {
    ...
    if ((CalleeF->getCallingConv() != Call.getCallingConv() &&
         !(CalleeF->getCallingConv() == toolchain::CallingConv::C &&
           TargetLibraryInfoImpl::isCallingConvCCompatible(&Call)) &&
         !(Call.getCallingConv() == toolchain::CallingConv::C &&
