...
raw_ostream &toolchain::operator<<(raw_ostream &OS, BBState &BBInfo) {
...
bool ObjCARCOpt::OptimizeInlinedAutoreleaseRVCall(
  ...
  if (Arg != AutoreleaseRVArg) {
    ...
    if (!toolchain::is_contained(ArgUsers, AutoreleaseRVArg))
      return false;
  }
...
bool ObjCARCOpt::Visit(Function &F,
  ...
  for (BasicBlock *BB : toolchain::reverse(ReverseCFGPostOrder)) {
  ...
  for (BasicBlock *BB : toolchain::reverse(PostOrder)) {
...
void ObjCARCOpt::OptimizeWeakCalls(Function &F) {
  ...
  for (Instruction &Inst : toolchain::make_early_inc_range(instructions(F))) {
    ARCInstKind Class = GetBasicARCInstKind(&Inst);
    ...
    if (AllocaInst *Alloca = dyn_cast<AllocaInst>(Arg)) {
      ...
      Changed = true;
      for (User *U : toolchain::make_early_inc_range(Alloca->users())) {
