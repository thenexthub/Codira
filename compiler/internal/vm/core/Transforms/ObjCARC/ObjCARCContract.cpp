...
void ObjCARCContract::tryToContractReleaseIntoStoreStrong(
  ...
  LLVM_DEBUG(
      toolchain::dbgs() << "    Contracting retain, release into objc_storeStrong.\n"
                   << "        Old:\n"
                   << "            Store:   " << *Store << "\n"
                   << "            Release: " << *Release << "\n"
                   << "            Retain:  " << *Retain << "\n"
  ...
  LLVM_DEBUG(toolchain::dbgs() << "        New Store Strong: " << *StoreStrong
                          << "\n");
...
bool ObjCARCContract::tryToPeepholeInstruction(
  ...
  case ARCInstKind::IntrinsicUser:
    // Remove calls to @toolchain.objc.clang.arc.use(...).
    Changed = true;
  ...
  default:
    if (auto *CI = dyn_cast<CallInst>(Inst))
      if (CI->getIntrinsicID() == Intrinsic::objc_clang_arc_noop_use) {
        // Remove calls to @toolchain.objc.clang.arc.noop.use(...).
        Changed = true;
        CI->eraseFromParent();
      }
    return true;
...
bool ObjCARCContract::run(Function &F, AAResults *A, DominatorTree *D) {
  ...
  LLVM_DEBUG(toolchain::dbgs() << "**** ObjCARC Contract ****\n");
...
Pass *toolchain::createObjCARCContractPass() {
