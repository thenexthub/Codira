...
static bool foldGuardedFunnelShift(Instruction &I, const DominatorTree &DT) {
  ...
  if (ShVal0 != ShVal1) {
    if (IsFshl && !toolchain::isGuaranteedNotToBePoison(ShVal1))
      ShVal1 = Builder.CreateFreeze(ShVal1);
    else if (!IsFshl && !toolchain::isGuaranteedNotToBePoison(ShVal0))
  ...
  // -->
  // toolchain.fshl.i32(i32 %ShVal0, i32 %ShVal1, i32 %ShAmt)
...
/// Fold smin(smax(fptosi(x), C1), C2) to toolchain.fptosi.sat(x), providing C1 and
...
// E.g., an example in C (for more cases please see the toolchain/tests):
// int f(unsigned x) {
...
// %0 = load i8, i8* %arrayidx, align 1, !tbaa !8
//
// All these can be lowered to @toolchain.cttz.i32/64 intrinsics.
...
static bool foldUnusualPatterns(Function &F, DominatorTree &DT,
  ...
  for (BasicBlock &BB : F) {
    ...
    for (Instruction &I : make_early_inc_range(toolchain::reverse(BB))) {
