...
private:
  ...
  bool computeShouldLiveOnFrame() const {
    ...
    if (ShouldUseLifetimeStartInfo && !LifetimeStarts.empty()) {
      ...
      toolchain::SmallVector<BasicBlock *> Worklist(LifetimeStartBBs);
...
void coro::sinkSpillUsesAfterCoroBegin(
  ...
  toolchain::sort(InsertionList, [&Dom](Instruction *A, Instruction *B) -> bool {
