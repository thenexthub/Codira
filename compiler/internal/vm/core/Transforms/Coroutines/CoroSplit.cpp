...
#include "vm/core/Transforms/Coroutines/MaterializationUtils.h"
...
/// Replace an toolchain.coro.end.async.
...
/// Replace a non-unwind call to toolchain.coro.end.
...
/// Replace an unwind call to toolchain.coro.end.
...
/// Replace uses of the active toolchain.coro.suspend.retcon/async call with the
...
void coro::BaseCloner::replaceRetconOrAsyncSuspendUses() {
  ...
  for (Use &U : toolchain::make_early_inc_range(NewS->uses())) {
  ...
  for (auto [Idx, Arg] : toolchain::enumerate(Args))
...
Value *coro::BaseCloner::deriveNewFramePointer() {
  ...
  // by the `toolchain.coro.id.async` intrinsic. We can retrieve the async context of
...
void coro::BaseCloner::create() {
  ...
  NewF->setLinkage(toolchain::GlobalValue::ExternalLinkage);
...
static bool simplifySuspendPoint(CoroSuspendInst *Suspend,
  ...
  // Replace toolchain.coro.suspend with the value that results in resumption over
...
struct SwitchCoroutineSplitter {
  static void split(Function &F, coro::Shape &Shape,
    ...
    // pointed by the last argument of @toolchain.coro.info, so that CoroElide pass
  ...
  static Function *createNoAllocVariant(Function &F, coro::Shape &Shape,
    ...
    NoAllocF->setLinkage(toolchain::GlobalValue::InternalLinkage);
    return NoAllocF;
  }
...
private:
  ...
  static void createResumeEntryBlock(Function &F, coro::Shape &Shape) {
    ...
    for (auto *AnyS : Shape.CoroSuspends) {
      ...
      //  whateverBB:
      //    whatever
      //    %0 = call i8 @toolchain.coro.suspend(token none, i1 false)
      ...
      //     %0 = tail call i8 @toolchain.coro.suspend(token none, i1 false)
      //     br label %resume.0.landing
      //
      //  resume.0.landing:
  ...
  //   define void @f() {
  //     ...
  //     call i8* @toolchain.coro.begin(i8* null, i32 0, i8* null,
...
CallInst *coro::createMustTailCall(DebugLoc Loc, Function *MustTailCallFn,
  ...
  // Coerce the arguments, toolchain optimizations seem to ignore the types in
...
void coro::AsyncABI::splitCoroutine(Function &F, coro::Shape &Shape,
  ...
  // Map all uses of toolchain.coro.begin to the allocated frame pointer.
  {
  ...
  for (auto [Idx, CS] : toolchain::enumerate(Shape.CoroSuspends)) {
...
void coro::AnyRetconABI::splitCoroutine(Function &F, coro::Shape &Shape,
  ...
  for (auto [Idx, CS] : toolchain::enumerate(Shape.CoroSuspends)) {
    ...
    for (auto [Phi, VUse] :
         toolchain::zip_equal(ReturnPHIs, Suspend->value_operands()))
...
/// Remove calls to toolchain.coro.end in the original function.
static void removeCoroEndsFromRampFunction(const coro::Shape &Shape) {
  if (Shape.ABI != coro::ABI::Switch) {
    ...
    for (toolchain::AnyCoroEndInst *End : Shape.CoroEnds)
      End->eraseFromParent();
  }
}
  ...
  for (auto *U : F.users()) {
    if (auto *CB = dyn_cast<CallBase>(U)) {
      ...
      if (Caller && Caller->isPresplitCoroutine() &&
          CB->hasFnAttr(toolchain::Attribute::CoroElideSafe))
        return true;
    }
  }
  return false;
...
static void doSplitCoroutine(Function &F, SmallVectorImpl<Function *> &Clones,
  ...
  bool shouldCreateNoAllocVariant =
      ...
      hasSafeElideCaller(F) && !F.hasFnAttribute(toolchain::Attribute::NoInline);
...
/// Replace a call to toolchain.coro.prepare.retcon.
static void replacePrepare(CallInst *Prepare, LazyCallGraph &CG,
  ...
  //    %1 = call @toolchain.coro.prepare.retcon(i8* %0)
  //    %2 = bitcast %1 to [[TYPE]]
  // ==>
  //    %2 = @some_function
  for (Use &U : toolchain::make_early_inc_range(Prepare->uses())) {
...
static bool replaceAllPrepares(Function *PrepareFn, LazyCallGraph &CG,
                               LazyCallGraph::SCC &C) {
  bool Changed = false;
  for (Use &P : toolchain::make_early_inc_range(PrepareFn->uses())) {
...
PreservedAnalyses CoroSplitPass::run(LazyCallGraph::SCC &C,
  ...
  // Check for uses of toolchain.coro.prepare.retcon/async.
  SmallVector<Function *, 2> PrepareFns;
  addPrepareFunction(M, PrepareFns, "toolchain.coro.prepare.retcon");
  addPrepareFunction(M, PrepareFns, "toolchain.coro.prepare.async");
