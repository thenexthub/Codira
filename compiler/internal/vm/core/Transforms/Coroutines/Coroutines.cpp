...
// Creates a call to toolchain.coro.subfn.addr to obtain a resume function address.
// It generates the following:
//
//    call ptr @toolchain.coro.subfn.addr(ptr %Arg, i8 %index)
...
// Replacing toolchain.coro.alloc with false will suppress dynamic
  ...
  for (Instruction &I : instructions(F)) {
    ...
    if (auto AWS = dyn_cast<CoroAwaitSuspendInst>(&I)) {
      ...
      case Intrinsic::coro_begin_custom_abi: {
        ...
        if (CoroBegin)
          report_fatal_error(
                "coroutine should have exactly one defining @toolchain.coro.begin");
...
[[noreturn]] static void fail(const Instruction *I, const char *Reason,
  ...
  if (V) {
    errs() << "  Value: ";
    V->printAsOperand(toolchain::errs());
    errs() << '\n';
  }
#endif
...
/// toolchain.coro.id.retcon.* intrinsics.
static void checkWFRetconPrototype(const AnyCoroIdRetconInst *I, Value *V) {
  auto F = dyn_cast<Function>(V->stripPointerCasts());
  if (!F)
    fail(I, "toolchain.coro.id.retcon.* prototype not a Function", V);
  ...
  if (isa<CoroIdRetconInst>(I)) {
    ...
    if (!ResultOkay)
      fail(I, "toolchain.coro.id.retcon prototype must return pointer as first "
              "result", F);

    if (FT->getReturnType() !=
          I->getFunction()->getFunctionType()->getReturnType())
      fail(I, "toolchain.coro.id.retcon prototype return type must be same as"
              "current function return type", F);
  } else {
    // No meaningful validation to do here for toolchain.coro.id.unique.once.
  }

  if (FT->getNumParams() == 0 || !FT->getParamType(0)->isPointerTy())
    fail(I, "toolchain.coro.id.retcon.* prototype must take pointer as "
...
static void checkWFAlloc(const Instruction *I, Value *V) {
  auto F = dyn_cast<Function>(V->stripPointerCasts());
  if (!F)
    fail(I, "toolchain.coro.* allocator not a Function", V);

  auto FT = F->getFunctionType();
  if (!FT->getReturnType()->isPointerTy())
    fail(I, "toolchain.coro.* allocator must return a pointer", F);

  if (FT->getNumParams() != 1 ||
      !FT->getParamType(0)->isIntegerTy())
    fail(I, "toolchain.coro.* allocator must take integer as only param", F);
}
...
static void checkWFDealloc(const Instruction *I, Value *V) {
  auto F = dyn_cast<Function>(V->stripPointerCasts());
  if (!F)
    fail(I, "toolchain.coro.* deallocator not a Function", V);

  auto FT = F->getFunctionType();
  if (!FT->getReturnType()->isVoidTy())
    fail(I, "toolchain.coro.* deallocator must return void", F);

  if (FT->getNumParams() != 1 ||
      !FT->getParamType(0)->isPointerTy())
    fail(I, "toolchain.coro.* deallocator must take pointer as only param", F);
}
...
static void checkAsyncFuncPointer(const Instruction *I, Value *V) {
  ...
  if (!AsyncFuncPtrAddr)
    fail(I, "toolchain.coro.id.async async function pointer not a global", V);
}
...
static void checkAsyncContextProjectFunction(const Instruction *I,
  ...
  if (!FunTy->getReturnType()->isPointerTy())
    fail(I,
         "toolchain.coro.suspend.async resume function projection function must "
         "return a ptr type",
         F);
...
void CoroAsyncEndInst::checkWellFormed() const {
  ...
  if (FnTy->getNumParams() != (arg_size() - 3))
    fail(this,
         "toolchain.coro.end.async must tail call function argument type must "
