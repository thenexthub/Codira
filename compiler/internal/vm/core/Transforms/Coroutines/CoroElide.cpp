...
static std::unique_ptr<raw_fd_ostream> getOrCreateLogFile() {
  ...
  if (!EC)
    return Result;
  toolchain::errs() << "Error opening coro-elide-info-output-file '"
...
void FunctionElideInfo::collectPostSplitCoroIds() {
  for (auto &I : instructions(this->ContainingFunction)) {
    ...
    // Consider case like:
    // %0 = call i8 @toolchain.coro.suspend(...)
...
// toolchain.coro.alloc and toolchain.coro.free instructions.
...
bool CoroIdElider::lifetimeEligibleForElide() const {
  ...
  for (const auto *CB : CoroBegins) {
    ...
    auto DominatesTerminator = [&](auto *TI) {
      return toolchain::any_of(CorrespondingDestroyAddrs, [&](auto *Destroy) {
        return DT.dominates(Destroy, TI->getTerminator());
      });
    };

    if (toolchain::all_of(Terminators, DominatesTerminator))
      continue;
