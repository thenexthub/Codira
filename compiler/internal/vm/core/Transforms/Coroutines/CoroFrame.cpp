...
#include "vm/core/Transforms/Coroutines/SpillUtils.h"
#include "vm/core/Transforms/Coroutines/SuspendCrossingInfo.h"
...
public:
  ...
  /// Consider the example at:https://bugs.toolchain.org/show_bug.cgi?id=45566
  ///
...
void FrameTypeBuilder::addFieldForAllocas(const Function &F,
  ...
  toolchain::scope_exit AddFieldForAllocasAtExit([&]() {
...
static void cacheDIVar(FrameDataInfo &FrameData,
  ...
  for (auto *V : FrameData.getAllDefs()) {
    ...
    auto CacheIt = [&DIVarCache, V](const auto &Container) {
      auto *I = toolchain::find_if(Container, [](auto *DDI) {
...
static DIType *solveDIType(DIBuilder &Builder, Type *Ty,
  ...
  if (Ty->isIntegerTy()) {
    ...
    auto *DIStruct = Builder.createStructType(
        ...
        toolchain::DINode::FlagArtificial, nullptr, toolchain::DINodeArray());
    ...
    for (unsigned I = 0; I < StructTy->getNumElements(); I++) {
      ...
      Elements.push_back(Builder.createMemberType(
          ...
          toolchain::DINode::FlagArtificial, DITy));
    }
    ...
    auto *CharSizeType = Builder.createBasicType(
        Name, 8, dwarf::DW_ATE_unsigned_char, toolchain::DINode::FlagArtificial);
...
static void buildFrameDebugInfo(Function &F, coro::Shape &Shape,
  ...
  if (!dwarf::isCPlusPlus(static_cast<toolchain::dwarf::SourceLanguage>(
  ...
  DICompositeType *FrameDITy = DBuilder.createStructType(
      ...
      Shape.FrameAlign.value() * 8, toolchain::DINode::FlagArtificial, nullptr,
...
static void insertSpills(const FrameDataInfo &FrameData, coro::Shape &Shape) {
  ...
  // https://github.com/toolchain/toolchain-project/issues/57861 for an example.
  if (Shape.ABI == coro::ABI::Switch && Shape.SwitchLowering.PromiseAlloca) {
    ...
    bool HasAccessingPromiseBeforeCB = toolchain::any_of(PA->uses(), [&](Use &U) {
      ...
      if (auto *CI = dyn_cast<CallInst>(Inst)) {
        ...
        // This is possible for @toolchain.coro.id intrinsics, which
...
static void eliminateSwiftErrorAlloca(Function &F, AllocaInst *Alloca,
                                      coro::Shape &Shape) {
  for (Use &Use : toolchain::make_early_inc_range(Alloca->uses())) {
...
salvageDebugInfoImpl(SmallDenseMap<Argument *, AllocaInst *, 4> &ArgToAllocaMap,
  ...
  while (auto *Inst = dyn_cast_or_null<Instruction>(Storage)) {
    if (auto *LdInst = dyn_cast<LoadInst>(Inst)) {
      ...
      Value *Op = toolchain::salvageDebugInfoImpl(
