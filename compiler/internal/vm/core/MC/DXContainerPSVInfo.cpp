//===- toolchain/MC/DXContainerPSVInfo.cpp - DXContainer PSVInfo -----*- C++ -*-===//
//
...
#include "vm/core/MC/DXContainerPSVInfo.h"
...
using namespace vm::core::dxbc::PSV;
...
ProcessElementList(StringTableBuilder &StrTabBuilder,
  ...
  for (const auto &El : Elements) {
    ...
    if (Idx == npos) {
      ...
      toolchain::append_range(IndexBuffer, El.Indices);
    } else
...
void PSVRuntimeInfo::write(raw_ostream &OS, uint32_t Version) const {
  ...
  support::endian::write(OS, InfoSize, toolchain::endianness::little);
  ...
  support::endian::write(OS, ResourceCount, toolchain::endianness::little);
  if (ResourceCount > 0)
    support::endian::write(OS, BindingSize, toolchain::endianness::little);
  ...
  for (auto I : IndexBuffer)
    support::endian::write(OS, I, toolchain::endianness::little);
...
void PSVRuntimeInfo::finalize(Triple::EnvironmentType Stage) {
  ...
  for (auto ElAndName : zip(SignatureElements, SemanticNames)) {
    toolchain::dxbc::PSV::v0::SignatureElement &El = std::get<0>(ElAndName);
