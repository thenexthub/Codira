...
#include "vm/core/Support/ConvertEBCDIC.h"
...
public:
  ...
  template <typename value_type> void writebe(value_type Value) {
    Value =
        support::endian::byte_swap<value_type>(Value, toolchain::endianness::big);
...
public:
  ...
  template <typename T> void writebe(T Val) {
    ...
    support::endian::write<T, toolchain::endianness::big>(Ptr, Val);
    Ptr += sizeof(T);
  }
};
} // namespace
...
toolchain::createGOFFObjectWriter(std::unique_ptr<MCGOFFObjectTargetWriter> MOTW,
                             raw_pwrite_stream &OS) {
