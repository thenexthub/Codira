...
MCDwarfLineTableHeader::tryGetFile(StringRef &Directory, StringRef &FileName,
  ...
  if (Directory.empty()) {
    ...
    DirIndex = toolchain::find(MCDwarfDirs, Directory) - MCDwarfDirs.begin();
...
static void EmitGenDwarfInfo(MCStreamer *MCOS,
  ...
  if (!DwarfDebugProducer.empty())
    ...
    MCOS->emitBytes(StringRef("toolchain-mc (based on LLVM " PACKAGE_VERSION ")"));
  MCOS->emitInt8(0); // NULL byte to terminate the string.
...
void MCDwarfFrameEmitter::Emit(MCObjectStreamer &Streamer, MCAsmBackend *MAB,
  ...
  toolchain::stable_sort(FrameArrayX,
...
void MCDwarfFrameEmitter::encodeAdvanceLoc(MCContext &Context,
  ...
  toolchain::endianness E = Context.getAsmInfo()->isLittleEndian()
