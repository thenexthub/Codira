...
XCOFFWriter::XCOFFWriter(std::unique_ptr<MCXCOFFObjectTargetWriter> MOTW,
                         raw_pwrite_stream &OS)
    : W(OS, toolchain::endianness::big), TargetObjectWriter(std::move(MOTW)),
...
void XCOFFWriter::recordRelocation(const MCFragment &F, const MCFixup &Fixup,
  ...
  if (Type == XCOFF::RelocationType::R_POS ||
    ...
    if (SymASec->getCSectType() == XCOFF::XTY_ER) {
      ...
      // toolchain::SignExtend64<16>(TOCEntryOffset);
      ...
      if (Type == XCOFF::RelocationType::R_TOC && !isInt<16>(TOCEntryOffset))
        TOCEntryOffset = toolchain::SignExtend64<16>(TOCEntryOffset);
...
void XCOFFWriter::assignAddressesAndIndices(MCAssembler &Asm) {
  ...
  for (auto *Section : Sections) {
    const bool IsEmpty =
        toolchain::all_of(Section->Groups,
...
void XCOFFWriter::writeSectionForCInfoSymSectionEntry(
  ...
  while (Index + WordSize <= Metadata.size()) {
    uint32_t NextWord =
        toolchain::support::endian::read32be(Metadata.data() + Index);
  ...
  if (CISI->paddingSize()) {
    ...
    W.write<uint32_t>(toolchain::support::endian::read32be(LastWord.data()));
  }
...
toolchain::createXCOFFObjectWriter(std::unique_ptr<MCXCOFFObjectTargetWriter> MOTW,
                              raw_pwrite_stream &OS) {
