...
#include "vm/core/Support/Win64EH.h"
...
static void EmitUnwindInfo(MCStreamer &streamer, WinEH::FrameInfo *info) {
  ...
  if (EnableUnwindV2) {
    ...
    bool IsLast = true;
    for (const auto &Epilog : toolchain::reverse(info->EpilogMap)) {
      if (IsLast) {
        IsLast = false;
...
void toolchain::Win64EH::UnwindEmitter::Emit(MCStreamer &Streamer) const {
...
void toolchain::Win64EH::UnwindEmitter::EmitUnwindInfo(MCStreamer &Streamer,
...
static void simplifyARM64Opcodes(std::vector<WinEH::Instruction> &Instructions,
  ...
  for (WinEH::Instruction &Inst :
       toolchain::reverse_conditionally(Instructions, Reverse)) {
...
static bool tryARM64PackedUnwind(WinEH::FrameInfo *info, uint32_t FuncLength,
  ...
  // See https://github.com/toolchain/toolchain-project/issues/54879 for details.
  //
  ...
  // https://github.com/toolchain/toolchain-project/issues/169588#issuecomment-3584907886.
...
static void ARM64EmitUnwindInfoForSegment(MCStreamer &streamer,
  ...
  for (auto Inst : toolchain::reverse(info->Instructions))
...
void toolchain::Win64EH::ARM64UnwindEmitter::Emit(MCStreamer &Streamer) const {
  // Emit the unwind info structs first.
  for (const auto &CFI : Streamer.getWinFrameInfos()) {
...
void toolchain::Win64EH::ARM64UnwindEmitter::EmitUnwindInfo(MCStreamer &Streamer,
                                                       WinEH::FrameInfo *info,
                                                       bool HandlerData) const {
...
void toolchain::Win64EH::ARMUnwindEmitter::Emit(MCStreamer &Streamer) const {
  // Emit the unwind info structs first.
  for (const auto &CFI : Streamer.getWinFrameInfos()) {
    WinEH::FrameInfo *Info = CFI.get();
    if (Info->empty())
...
void toolchain::Win64EH::ARMUnwindEmitter::EmitUnwindInfo(MCStreamer &Streamer,
                                                     WinEH::FrameInfo *info,
                                                     bool HandlerData) const {
  // Called if there's an .seh_handlerdata directive before the end of the
  // function. This forces writing the xdata record already here - and
