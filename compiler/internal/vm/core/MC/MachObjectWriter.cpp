...
void MachObjectWriter::computeSymbolTable(
  ...
  toolchain::sort(ExternalSymbolData);
  toolchain::sort(UndefinedSymbolData);
  ...
  for (const MCSection &Section : Asm) {
    for (RelAndSymbol &Rel : Relocations[&Section]) {
      ...
      if (W.Endian == toolchain::endianness::little)
...
uint64_t MachObjectWriter::writeObject() {
  ...
  if (!CGProfile.empty()) {
    ...
    toolchain::copy(OS.str(), Sec->curFragList()->Head->getVarContents().data());
  }
  ...
  for (const MCSection &Sec : Asm) {
    ...
    std::vector<RelAndSymbol> &Relocs = Relocations[&Sec];
    for (const RelAndSymbol &Rel : toolchain::reverse(Relocs)) {
