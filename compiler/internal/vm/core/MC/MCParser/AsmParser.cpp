...
bool AsmParser::parseDirectiveAscii(StringRef IDVal, bool ZeroTerminated) {
  auto parseOp = [&]() -> bool {
    ...
    // discusssion at https://reviews.toolchain.org/D91460 for more details.
    do {
...
bool AsmParser::parseDirectiveBase64() {
  auto parseOp = [&]() -> bool {
    ...
    toolchain::Error e = decodeBase64(str, Decoded);
    if (e) {
...
bool AsmParser::parseDirectiveAlign(bool IsPow2, uint8_t ValueSize) {
  ...
  if (IsPow2) {
    ...
    if (Alignment == 0)
      ...
      Alignment = toolchain::bit_floor<uint64_t>(Alignment);
    }
...
bool AsmParser::parseDirectiveCFIStartProc() {
  ...
  // Tools like toolchain-mc on the other hand are not affected by it, and report
...
bool AsmParser::parseDirectivePrint(SMLoc DirectiveLoc) {
  ...
  if (parseEOL())
    return true;
  toolchain::outs() << StrTok.getStringContents() << '\n';
  return false;
}
...
bool toolchain::MCParserUtils::parseAssignmentExpression(StringRef Name,
                                                    bool allow_redef,
                                                    MCAsmParser &Parser,
                                                    MCSymbol *&Sym,
                                                    const MCExpr *&Value) {
...
MCAsmParser *toolchain::createMCAsmParser(SourceMgr &SM, MCContext &C,
