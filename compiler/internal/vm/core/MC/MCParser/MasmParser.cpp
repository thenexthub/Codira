...
#include "vm/core/MC/MCParser/AsmCond.h"
...
#include "vm/core/MC/MCParser/MCParsedAsmOperand.h"
...
FieldInfo &StructInfo::addField(StringRef FieldName, FieldType FT,
  ...
  Field.Offset =
      toolchain::alignTo(NextOffset, std::min(Alignment, FieldAlignmentSize));
  if (!IsUnion) {
...
MasmParser::MasmParser(SourceMgr &SM, MCContext &Ctx, MCStreamer &Out,
  ...
  case MCContext::IsCOFF:
    ...
    break;
  default:
    report_fatal_error("toolchain-ml currently supports only COFF output.");
    break;
  }
...
bool MasmParser::expandMacros() {
  ...
  const toolchain::MCAsmMacro *M = getContext().lookupMacro(IDLower);
...
bool MasmParser::parseFieldInitializer(const FieldInfo &Field,
  ...
  // Default-initialize all remaining values.
  toolchain::append_range(Initializers, toolchain::drop_begin(Contents.Initializers,
                                                    Initializers.size()));
...
bool MasmParser::parseStructInitializer(const StructInfo &Structure,
  ...
  for (const FieldInfo &Field : toolchain::drop_begin(Structure.Fields, FieldIndex))
    FieldInitializers.push_back(Field.Contents);
...
bool MasmParser::parseStructInstList(
  ...
  while (getTok().isNot(EndToken) ||
    ...
    if (NextTok.is(AsmToken::Identifier) &&
      ...
      for (int i = 0; i < Repetitions; ++i)
        toolchain::append_range(Initializers, DuplicatedValues);
    } else {
      Initializers.emplace_back();
...
bool MasmParser::emitFieldInitializer(const FieldInfo &Field,
  ...
  for (const auto &Value :
           toolchain::drop_begin(Contents.Values, Initializer.Values.size())) {
    if (emitIntValue(Value, Field.Type))
      return true;
  }
  return false;
...
bool MasmParser::emitFieldInitializer(const FieldInfo &Field,
  ...
  for (const auto &AsInt :
       toolchain::drop_begin(Contents.AsIntValues, Initializer.AsIntValues.size())) {
    getStreamer().emitIntValue(AsInt.getLimitedValue(),
                               AsInt.getBitWidth() / 8);
  }
  return false;
...
bool MasmParser::emitFieldInitializer(const FieldInfo &Field,
  ...
  // Default-initialize all remaining values.
  for (const auto &Init : toolchain::drop_begin(Contents.Initializers,
...
bool MasmParser::emitStructInitializer(const StructInfo &Structure,
  ...
  // Default-initialize all remaining fields.
  for (const auto &Field : toolchain::drop_begin(
...
bool MasmParser::parseDirectiveEnds(StringRef Name, SMLoc NameLoc) {
  ...
  Structure.Size = toolchain::alignTo(
...
bool MasmParser::parseDirectiveNestedEnds() {
  ...
  Structure.Size = toolchain::alignTo(Structure.Size, Structure.Alignment);
  ...
  if (Structure.Name.empty()) {
    ...
    if (!Structure.Fields.empty() && !ParentStruct.IsUnion) {
      FirstFieldOffset = toolchain::alignTo(
    ...
    if (ParentStruct.IsUnion) {
      ...
      for (auto &Field : toolchain::drop_begin(ParentStruct.Fields, OldFields))
...
bool MasmParser::emitAlignTo(int64_t Alignment) {
  if (StructInProgress.empty()) {
    ...
    StructInfo &Structure = StructInProgress.back();
    Structure.NextOffset = toolchain::alignTo(Structure.NextOffset, Alignment);
  }
...
bool MasmParser::parseMSInlineAsm(
  ...
  while (getLexer().isNot(AsmToken::Eof)) {
    ...
    toolchain::append_range(ClobberRegs, Desc.implicit_defs());
  }
  ...
  ClobberRegs.erase(toolchain::unique(ClobberRegs), ClobberRegs.end());
...
MCAsmParser *toolchain::createMCMasmParser(SourceMgr &SM, MCContext &C,
