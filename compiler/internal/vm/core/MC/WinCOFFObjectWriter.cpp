//===- toolchain/MC/WinCOFFObjectWriter.cpp ------------------------------------===//
//
...
using toolchain::support::endian::write32le;
...
WinCOFFWriter::WinCOFFWriter(WinCOFFObjectWriter &OWriter,
    ...
    : OWriter(OWriter), W(OS, toolchain::endianness::little), Mode(Mode) {
...
void WinCOFFWriter::writeSectionHeaders() {
  ...
  toolchain::sort(Arr, [](const COFFSection *A, const COFFSection *B) {
...
uint64_t WinCOFFWriter::writeObject() {
  ...
  // Create the contents of the .toolchain.call-graph-profile section.
  if (Mode != DwoOnly && !OWriter.getCGProfile().empty()) {
    ...
    auto *Sec = getContext().getCOFFSection(".toolchain.call-graph-profile",
                                            COFF::IMAGE_SCN_LNK_REMOVE);
    Sec->curFragList()->Tail->setVarContents(OS.str());
  }
...
std::unique_ptr<MCObjectWriter> toolchain::createWinCOFFObjectWriter(
    std::unique_ptr<MCWinCOFFObjectTargetWriter> MOTW, raw_pwrite_stream &OS) {
  return std::make_unique<WinCOFFObjectWriter>(std::move(MOTW), OS);
}

std::unique_ptr<MCObjectWriter> toolchain::createWinCOFFDwoObjectWriter(
    std::unique_ptr<MCWinCOFFObjectTargetWriter> MOTW, raw_pwrite_stream &OS,
    raw_pwrite_stream &DwoOS) {
