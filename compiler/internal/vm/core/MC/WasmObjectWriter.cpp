...
#include "vm/core/BinaryFormat/WasmTraits.h"
...
// in memory.  Each MCSection data created by toolchain is modeled as its own
// wasm data segment.
...
// TODO: support alignment at asm and toolchain level?
...
static void addData(SmallVectorImpl<char> &DataBytes,
  ...
  for (const MCFragment &Frag : DataSection) {
    ...
    toolchain::append_range(DataBytes, Frag.getContents());
    if (Frag.getKind() == MCFragment::FT_Align) {
      ...
      toolchain::append_range(DataBytes, Frag.getVarContents());
    } else {
...
uint64_t WasmObjectWriter::writeObject() {
  support::endian::Writer MainWriter(*OS, toolchain::endianness::little);
  W = &MainWriter;
  if (IsSplitDwarf) {
    uint64_t TotalSize = writeOneObject(*Asm, DwoMode::NonDwoOnly);
    assert(DwoOS);
    support::endian::Writer DwoWriter(*DwoOS, toolchain::endianness::little);
    W = &DwoWriter;
...
uint64_t WasmObjectWriter::writeOneObject(MCAssembler &Asm,
  ...
  for (const MCSection &S : Asm) {
    ...
    for (auto *Frag = &*IT; Frag; Frag = Frag->getNext()) {
      ...
      assert(toolchain::all_of(Frag->getContents(), [](char C) { return !C; }));
...
toolchain::createWasmObjectWriter(std::unique_ptr<MCWasmObjectTargetWriter> MOTW,
                             raw_pwrite_stream &OS) {
  return std::make_unique<WasmObjectWriter>(std::move(MOTW), OS);
}

std::unique_ptr<MCObjectWriter>
toolchain::createWasmDwoObjectWriter(std::unique_ptr<MCWasmObjectTargetWriter> MOTW,
                                raw_pwrite_stream &OS,
