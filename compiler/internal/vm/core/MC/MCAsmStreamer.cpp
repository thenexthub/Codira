...
void MCAsmStreamer::emitLinkerOptions(ArrayRef<std::string> Options) {
  ...
  for (const std::string &Opt : toolchain::drop_begin(Options))
    OS << ", " << '"' << Opt << '"';
  EmitEOL();
}
...
void MCAsmStreamer::emitXCOFFCInfoSym(StringRef Name, StringRef Metadata) {
  ...
  auto PrintWord = [&](const uint8_t *WordPtr) {
    ...
    OS << Separator;
    uint32_t Word = toolchain::support::endian::read32be(WordPtr);
...
void MCAsmStreamer::emitValueImpl(const MCExpr *Value, unsigned Size,
  ...
  if (!Directive) {
    ...
    for (unsigned Emitted = 0; Emitted != Size;) {
      ...
      // Size.
      unsigned EmissionSize = toolchain::bit_floor(std::min(Remaining, Size - 1));
...
static void PrintCFIEscape(toolchain::formatted_raw_ostream &OS, StringRef Values) {
  OS << "\t.cfi_escape ";
  if (!Values.empty()) {
...
MCStreamer *toolchain::createAsmStreamer(MCContext &Context,
