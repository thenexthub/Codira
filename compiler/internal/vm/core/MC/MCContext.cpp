...
#include "vm/core/MC/MCSectionSPIRV.h"
...
void MCContext::remapDebugPath(SmallVectorImpl<char> &Path) {
  for (const auto &[From, To] : toolchain::reverse(DebugPrefixMap))
...
void MCContext::setGenDwarfRootFile(StringRef InputFileName, StringRef Buffer) {
  ...
  if (!getMainFileName().empty() && FileNameBuf != getMainFileName()) {
    toolchain::sys::path::remove_filename(FileNameBuf);
    toolchain::sys::path::append(FileNameBuf, getMainFileName());
  }
  StringRef FileName = FileNameBuf;
  if (FileName.consume_front(getCompilationDir()))
    if (toolchain::sys::path::is_separator(FileName.front()))
