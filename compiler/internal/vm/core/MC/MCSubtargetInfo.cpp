...
static FeatureBitset getFeatures(MCSubtargetInfo &STI, StringRef CPU,
  ...
  assert(toolchain::is_sorted(ProcDesc) && "CPU table is not sorted");
  assert(toolchain::is_sorted(ProcFeatures) && "CPU features table is not sorted");
  // Resulting bits
  FeatureBitset Bits;
...
const MCSchedModel &MCSubtargetInfo::getSchedModelForCPU(StringRef CPU) const {
  assert(toolchain::is_sorted(ProcDesc) &&
...
MCSubtargetInfo::getEnabledProcessorFeatures() const {
  ...
  toolchain::copy_if(ProcFeatures, std::back_inserter(EnabledFeatures), IsEnabled);
