...
#include "vm/core/FileCheck/FileCheck.h"
...
Expected<APInt> toolchain::exprAdd(const APInt &LeftOperand,
                              const APInt &RightOperand, bool &Overflow) {
  return LeftOperand.sadd_ov(RightOperand, Overflow);
}

Expected<APInt> toolchain::exprSub(const APInt &LeftOperand,
                              const APInt &RightOperand, bool &Overflow) {
  return LeftOperand.ssub_ov(RightOperand, Overflow);
}

Expected<APInt> toolchain::exprMul(const APInt &LeftOperand,
                              const APInt &RightOperand, bool &Overflow) {
  return LeftOperand.smul_ov(RightOperand, Overflow);
}

Expected<APInt> toolchain::exprDiv(const APInt &LeftOperand,
                              const APInt &RightOperand, bool &Overflow) {
...
Expected<APInt> toolchain::exprMax(const APInt &LeftOperand,
                              const APInt &RightOperand, bool &Overflow) {
  Overflow = false;
  return LeftOperand.slt(RightOperand) ? RightOperand : LeftOperand;
}

Expected<APInt> toolchain::exprMin(const APInt &LeftOperand,
                              const APInt &RightOperand, bool &Overflow) {
  Overflow = false;
...
Expected<std::string> StringSubstitution::getResultForDiagnostics() const {
  ...
  const bool NeedsEscaping =
      toolchain::any_of(*VarVal, [](char C) { return !isPrint(C) || C == '"'; });
  if (NeedsEscaping)
...
void Pattern::printVariableDefs(const SourceMgr &SM,
  ...
  toolchain::sort(VarCaptures, [](const VarCapture &A, const VarCapture &B) {
    if (&A == &B)
      return false;
...
FindCheckType(const FileCheckRequest &Req, StringRef Buffer, StringRef Prefix,
  ...
  // Check for comment.
  if (toolchain::is_contained(Req.CommentPrefixes, Prefix)) {
...
static void addDefaultPrefixes(FileCheckRequest &Req) {
  if (Req.CheckPrefixes.empty()) {
    toolchain::append_range(Req.CheckPrefixes, DefaultCheckPrefixes);
    Req.IsDefaultCheckPrefix = true;
  }
  if (Req.CommentPrefixes.empty())
    toolchain::append_range(Req.CommentPrefixes, DefaultCommentPrefixes);
}
