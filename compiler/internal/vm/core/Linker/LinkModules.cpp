...
bool ModuleLinker::run() {
  ...
  // otherwise.
  for (GlobalAlias &GV : toolchain::make_early_inc_range(DstM.aliases()))
    dropReplacedComdat(GV, ReplacedDstComdats);

  for (GlobalVariable &GV : toolchain::make_early_inc_range(DstM.globals()))
    dropReplacedComdat(GV, ReplacedDstComdats);

  for (Function &GV : toolchain::make_early_inc_range(DstM))
    dropReplacedComdat(GV, ReplacedDstComdats);
