...
IRLinker::linkAppendingVarProto(GlobalVariable *DstGV,
  ...
  // toolchain::UpgradeGlobalVariable() and simplify the logic here and in
  // Mapper::mapAppendingVariable() in ValueMapper.cpp.
  StringRef Name = SrcGV->getName();
  bool IsNewStructor = false;
  bool IsOldStructor = false;
  if (Name == "toolchain.global_ctors" || Name == "toolchain.global_dtors") {
...
void IRLinker::prepareCompileUnitsForImport() {
  NamedMDNode *SrcCompileUnits = SrcM->getNamedMetadata("toolchain.dbg.cu");
  if (!SrcCompileUnits)
    return;
  // When importing for ThinLTO, prevent importing of types listed on
  // the DICompileUnit that we don't need a copy of in the importing
...
void IRLinker::linkNamedMDNodes() {
  ...
  for (const NamedMDNode &NMD : SrcM->named_metadata()) {
    ...
    if (IsPerformingImport && NMD.getName() == "toolchain.stats")
      continue;
...
void IRLinker::updateAttributes(GlobalValue &GV) {
  ...
  if (auto *F = dyn_cast<Function>(&GV)) {
    if (!F->isIntrinsic())
      F->removeFnAttr(toolchain::Attribute::NoCallback);
