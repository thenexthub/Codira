...
#include "vm/core/LTO/legacy/ThinLTOCodeGenerator.h"
...
#include "vm/core/IR/PassTimingInfo.h"
...
#include "vm/core/Remarks/HotnessThresholdParser.h"
#include "vm/core/Support/CachePruning.h"
...
static void saveTempBitcode(const Module &TheModule, StringRef TempDir,
  ...
  std::string SaveTempPath = (TempDir + toolchain::Twine(count) + Suffix).str();
  std::error_code EC;
...
static const GlobalValueSummary *getFirstDefinitionForLinker(
  ...
  auto StrongDefForLinker = toolchain::find_if(
  ...
  // list.
  auto FirstDefForLinker = toolchain::find_if(
      GVSummaryList, [](const std::unique_ptr<GlobalValueSummary> &Summary) {
        auto Linkage = Summary->linkage();
        return !GlobalValue::isAvailableExternallyLinkage(Linkage);
      });
...
public:
  ...
  ModuleCacheEntry(
    ...
    // in include/toolchain/Support/CachePruning.h).
  ...
  void write(const MemoryBuffer &OutputBuffer) {
    ...
    if (auto Err = toolchain::writeToOutput(
            EntryPath, [&OutputBuffer](toolchain::raw_ostream &OS) -> toolchain::Error {
              OS << OutputBuffer.getBuffer();
              return toolchain::Error::success();
            }))
      report_fatal_error(toolchain::formatv("ThinLTO: Can't write file {0}: {1}",
                                       EntryPath,
...
void ThinLTOCodeGenerator::gatherImportedSummariesForModule(
  ...
  toolchain::gatherImportedSummariesForModule(
...
void ThinLTOCodeGenerator::run() {
  ...
  if (toolchain::AreStatisticsEnabled())
