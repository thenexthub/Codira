...
#include "vm/core/CGData/CodeGenData.h"
...
#include "vm/core/LTO/LTOBackend.h"
#include "vm/core/Linker/IRMover.h"
#include "vm/core/MC/TargetRegistry.h"
#include "vm/core/Object/IRObjectFile.h"
#include "vm/core/Support/Caching.h"
...
#include "vm/core/Support/SHA1.h"
...
#include "vm/core/Support/VCSRevision.h"
...
#include "vm/core/Transforms/Utils/FunctionImportUtils.h"
...
std::string toolchain::computeLTOCacheKey(
...
std::string toolchain::recomputeLTOCacheKey(const std::string &Key,
                                       StringRef ExtraID) {
  SHA1 Hasher;
...
// one copy.
void toolchain::thinLTOResolvePrevailingInIndex(
...
static void thinLTOInternalizeAndPromoteGUID(
  ...
  for (auto &S : VI.getSummaryList()) {
    ...
    // are toolchain-lto based tests of the legacy LTO API that do not mark
...
void toolchain::thinLTOInternalizeAndPromoteInIndex(
...
LTO::ThinLTOState::ThinLTOState(ThinBackend BackendParam)
  ...
  if (!Backend.isValid())
    Backend =
        createInProcessThinBackend(toolchain::heavyweight_hardware_concurrency());
}
...
LTO::LTO(Config Conf, ThinBackend Backend,
  ...
  if (Conf.KeepSymbolNameCopies || LTOKeepSymbolCopies) {
    ...
    GlobalResolutionSymbolSaver = std::make_unique<toolchain::StringSaver>(*Alloc);
  }
}
...
void LTO::addModuleToGlobalRes(ArrayRef<InputFile::Symbol> Syms,
  ...
  toolchain::TimeTraceScope timeScope("LTO add module to global resolution");
  auto *ResI = Res.begin();
  auto *ResE = Res.end();
  (void)ResE;
  for (const InputFile::Symbol &Sym : Syms) {
    ...
    // regular object, is referenced from toolchain.compiler.used/toolchain.used, or was
...
Error LTO::add(std::unique_ptr<InputFile> InputPtr,
               ArrayRef<SymbolResolution> Res) {
  toolchain::TimeTraceScope timeScope("LTO add input", InputPtr->getName());
...
LTO::addModule(InputFile &Input, ArrayRef<SymbolResolution> InputRes,
  ...
  toolchain::TimeTraceScope timeScope("LTO add module", Input.getName());
...
LTO::addRegularLTO(InputFile &Input, ArrayRef<SymbolResolution> InputRes,
                   ...
                   ArrayRef<SymbolResolution> Res) {
  toolchain::TimeTraceScope timeScope("LTO add regular LTO");
  ...
  if (!M.getModuleInlineAsm().empty()) {
    ...
    if (!NonPrevailingAsmSymbols.empty()) {
      ...
      NewIA += " " + toolchain::join(NonPrevailingAsmSymbols, ", ");
    }
    NewIA += "\n";
...
Error LTO::linkRegularLTO(RegularLTOState::AddedModule Mod,
  ...
  toolchain::TimeTraceScope timeScope("LTO link regular LTO");
...
LTO::addThinLTO(BitcodeModule BM, ArrayRef<InputFile::Symbol> Syms,
                ArrayRef<SymbolResolution> Res) {
  toolchain::TimeTraceScope timeScope("LTO add thin LTO");
...
Error LTO::runRegularLTO(AddStreamFn AddStream) {
  toolchain::TimeTraceScope timeScope("Run regular LTO");
  ...
  {
    toolchain::TimeTraceScope timeScope("Link regular LTO");
...
Error ThinBackendProc::emitFiles(
    const FunctionImporter::ImportMapTy &ImportList, toolchain::StringRef ModulePath,
...
public:
  ...
  virtual Error runThinLTOBackendThread(
    ...
    toolchain::TimeTraceScope timeScope("Run ThinLTO backend thread (in-process)",
                                   ModuleID);
...
public:
  ...
  Error runThinLTOBackendThread(
      ...
      MapVector<StringRef, BitcodeModule> &ModuleMap) override {
    auto ModuleID = BM.getModuleIdentifier();
    toolchain::TimeTraceScope timeScope("Run ThinLTO backend thread (first round)",
                                   ModuleID);
...
public:
  ...
  Error runThinLTOBackendThread(
      ...
      MapVector<StringRef, BitcodeModule> &ModuleMap) override {
    auto ModuleID = BM.getModuleIdentifier();
    toolchain::TimeTraceScope timeScope("Run ThinLTO backend thread (second round)",
                                   ModuleID);
    auto RunThinBackend = [&](AddStreamFn AddStream) {
      LTOLLVMContext BackendContext(Conf);
...
std::string lto::getThinLTOOutputFile(StringRef Path, StringRef OldPrefix,
  ...
  toolchain::sys::path::replace_path_prefix(NewPath, OldPrefix, NewPrefix);
  StringRef ParentPath = toolchain::sys::path::parent_path(NewPath.str());
  if (!ParentPath.empty()) {
    // Make sure the new directory exists, creating it if necessary.
    if (std::error_code EC = toolchain::sys::fs::create_directories(ParentPath))
      toolchain::errs() << "warning: could not create directory '" << ParentPath
                   << "': " << EC.message() << '\n';
  }
...
Error LTO::runThinLTO(AddStreamFn AddStream, FileCache Cache,
                      const DenseSet<GlobalValue::GUID> &GUIDPreservedSymbols) {
  toolchain::TimeTraceScope timeScope("Run ThinLTO");
  LLVM_DEBUG(dbgs() << "Running ThinLTO\n");
  ThinLTO.CombinedIndex.releaseTemporaryMemory();
  timeTraceProfilerBegin("ThinLink", StringRef(""));
  toolchain::scope_exit TimeTraceScopeExit([]() {
    if (toolchain::timeTraceProfilerEnabled())
      toolchain::timeTraceProfilerEnd();
  });
  if (ThinLTO.ModuleMap.empty())
    return Error::success();

  if (ThinLTO.ModulesToCompile && ThinLTO.ModulesToCompile->empty()) {
    toolchain::errs() << "warning: [ThinLTO] No module compiled\n";
    return Error::success();
  }
...
Expected<LLVMRemarkFileHandle> lto::setupLLVMOptimizationRemarks(
  ...
  if (!Filename.empty() && Count != -1)
    Filename =
        (Twine(Filename) + ".thin." + toolchain::utostr(Count) + "." + RemarksFormat)
            .str();

  auto ResultOrErr = toolchain::setupLLVMOptimizationRemarks(
...
std::vector<int> lto::generateModulesOrdering(ArrayRef<BitcodeModule *> R) {
  auto Seq = toolchain::seq<int>(0, R.size());
  std::vector<int> ModulesOrdering(Seq.begin(), Seq.end());
  toolchain::sort(ModulesOrdering, [&](int LeftIndex, int RightIndex) {
...
public:
  ...
  void setup(unsigned ThinLTONumTasks, unsigned ThinLTOTaskOffset,
             toolchain::Triple Triple) override {
  ...
  Error start(
    ...
    if (!SaveTemps) {
      toolchain::sys::RemoveFileOnSignal(J.NativeObjectPath);
      if (!ShouldEmitIndexFiles)
        toolchain::sys::RemoveFileOnSignal(J.SummaryIndexPath);
    }
      ...
      JOS.attributeArray("jobs", [&]() {
        for (const auto &J : Jobs) {
          ...
          JOS.object([&]() {
            ...
            toolchain::append_range(Inputs, J.ImportsFiles);
  ...
  Error wait() override {
    ...
    toolchain::scope_exit CleanPerJobFiles([&] {
      toolchain::TimeTraceScope TimeScope("Remove DTLTO temporary files");
      if (!SaveTemps)
        for (auto &Job : Jobs) {
...

    {
      ...
      if (CachedJobs.load() < Jobs.size()) {
        ...
        toolchain::append_range(Args, DistributorArgs);
    ...
    {
      toolchain::TimeTraceScope FilesScope("Add DTLTO files to the link");
      for (auto &Job : Jobs) {
