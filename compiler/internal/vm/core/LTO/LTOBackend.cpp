...
bool lto::opt(const Config &Conf, TargetMachine *TM, unsigned Task, Module &Mod,
  ...
  toolchain::TimeTraceScope timeScope("opt");
  if (EmbedBitcode == LTOBitcodeEmbedding::EmbedPostMergePreOptimized) {
    ...
    toolchain::embedBitcodeInModule(Mod, toolchain::MemoryBufferRef(),
...
static void codegen(const Config &Conf, TargetMachine *TM,
  ...
  toolchain::TimeTraceScope timeScope("codegen");
  ...
  if (!Conf.DwoDir.empty()) {
    std::error_code EC;
    if (auto EC = toolchain::sys::fs::create_directories(Conf.DwoDir))
...
Error lto::backend(const Config &C, AddStreamFn AddStream,
  ...
  toolchain::TimeTraceScope timeScope("LTO backend");
...
static void dropDeadSymbols(Module &Mod, const GVSummaryMapTy &DefinedGlobals,
                            const ModuleSummaryIndex &Index) {
  toolchain::TimeTraceScope timeScope("Drop dead symbols");
...
Error lto::thinBackend(const Config &Conf, unsigned Task, AddStreamFn AddStream,
                       ...
                       const std::vector<uint8_t> &CmdArgs) {
  toolchain::TimeTraceScope timeScope("Thin backend", Mod.getModuleIdentifier());
  ...
  auto ModuleLoader = [&](StringRef Identifier) {
    toolchain::TimeTraceScope moduleLoaderScope("Module loader", Identifier);
    ...
    ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>> MBOrErr =
        toolchain::MemoryBuffer::getFile(Identifier);
    if (!MBOrErr)
      return Expected<std::unique_ptr<toolchain::Module>>(make_error<StringError>(
  ...
  {
    toolchain::TimeTraceScope importScope("Import functions");
