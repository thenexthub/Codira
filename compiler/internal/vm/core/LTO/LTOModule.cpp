...
LTOModule::makeLTOModule(MemoryBufferRef Buffer, const TargetOptions &options,
  ...
  toolchain::Triple Triple = M->getTargetTriple();
  if (Triple.empty())
    Triple = toolchain::Triple(sys::getDefaultTargetTriple());
  ...
  if (Triple.isOSDarwin()) {
    if (Triple.getArch() == toolchain::Triple::x86_64)
      CPU = "core2";
    else if (Triple.getArch() == toolchain::Triple::x86)
      CPU = "yonah";
    else if (Triple.isArm64e())
      CPU = "apple-a12";
    else if (Triple.getArch() == toolchain::Triple::aarch64 ||
...
void LTOModule::parseMetadata() {
  ...
  if (NamedMDNode *LinkerOptions =
          getModule().getNamedMetadata("toolchain.linker.options")) {
...
bool LTOModule::hasCtorDtor() const {
  for (auto Sym : SymTab.symbols()) {
    if (auto *GV = dyn_cast_if_present<GlobalValue *>(Sym)) {
      ...
      if (Name.consume_front("toolchain.global_")) {
