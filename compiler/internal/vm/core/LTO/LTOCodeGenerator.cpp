...
#include "vm/core/LTO/legacy/LTOCodeGenerator.h"
...
#include "vm/core/LTO/legacy/LTOModule.h"
...
bool LTOCodeGenerator::runAIXSystemAssembler(SmallString<128> &AssemblyFile) {
  ...
  if (!toolchain::AIXSystemAssemblerPath.empty()) {
    if (toolchain::sys::fs::real_path(toolchain::AIXSystemAssemblerPath, AssemblerPath,
                                 /* expand_tilde */ true)) {
      emitError(
...
bool LTOCodeGenerator::compileOptimizedToFile(const char **Name) {
  ...
  auto AddStream =
    ...
    int FD;
    std::error_code EC =
        sys::fs::createTemporaryFile("lto-toolchain", Extension, FD, Filename);
    if (EC)
      emitError(EC.message());

    return std::make_unique<CachedFileStream>(
        std::make_unique<toolchain::raw_fd_ostream>(FD, true));
  };
...
// "toolchain.compiler.used" global.
void LTOCodeGenerator::preserveDiscardableGVs(
    Module &TheModule,
    toolchain::function_ref<bool(const GlobalValue &)> mustPreserveGV) {
...
void LTOCodeGenerator::applyScopeRestrictions() {
  ...
  // Update the toolchain.compiler_used globals to force preserving libcalls and
...
void LTOCodeGenerator::restoreLinkageForExternals() {
  ...
  toolchain::for_each(MergedModule->functions(), externalize);
  toolchain::for_each(MergedModule->globals(), externalize);
  toolchain::for_each(MergedModule->aliases(), externalize);
}
...
void LTOCodeGenerator::parseCodeGenDebugOptions() {
  if (!CodegenOptions.empty())
    toolchain::parseCommandLineOptions(CodegenOptions);
}

void toolchain::parseCommandLineOptions(std::vector<std::string> &Options) {
  if (!Options.empty()) {
