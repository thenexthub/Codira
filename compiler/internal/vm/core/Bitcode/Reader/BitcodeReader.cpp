...
#include "vm/core/Bitcode/BitcodeCommon.h"
#include "vm/core/Bitcode/LLVMBitCodes.h"
...
static Expected<std::string> readTriple(BitstreamCursor &Stream) {
  ...
  while (true) {
    ...
    case BitstreamEntry::Record:
      if (toolchain::Expected<unsigned> Skipped = Stream.skipRecord(Entry.ID))
        continue;
      else
...
private:
  BitcodeConstant(Type *Ty, const ExtraInfo &Info, ArrayRef<unsigned> OpIDs)
    ...
    toolchain::uninitialized_copy(OpIDs, getTrailingObjects());
  }
...
std::error_code toolchain::errorToErrorCodeAndEmitErrors(LLVMContext &Ctx,
                                                    Error Err) {
  if (Err) {
    std::error_code EC;
...
Error BitcodeReader::parseTypeTableBody() {
  ...
  while (true) {
    ...
    case bitc::TYPE_CODE_X86_MMX:   // X86_MMX
      ...
      ResultTy =
          toolchain::FixedVectorType::get(toolchain::IntegerType::get(Context, 64), 1);
      break;
...
APInt toolchain::readWideAPInt(ArrayRef<uint64_t> Vals, unsigned TypeBits) {
...
Error BitcodeReader::parseConstants() {
  ...
  while (true) {
    ...
    case bitc::CST_CODE_AGGREGATE: {// AGGREGATE: [n x value number]
      ...
      toolchain::append_range(Elts, Record);
...
Error BitcodeReader::materializeMetadata() {
  ...
  // Upgrade "Linker Options" module flag to "toolchain.linker.options" module-level
  // metadata. Only upgrade if the new option doesn't exist to avoid upgrade
  // multiple times.
  if (!TheModule->getNamedMetadata("toolchain.linker.options")) {
    if (Metadata *Val = TheModule->getModuleFlag("Linker Options")) {
      NamedMDNode *LinkerOpts =
          TheModule->getOrInsertNamedMetadata("toolchain.linker.options");
...
Error BitcodeReader::parseModule(uint64_t ResumeBit,
  ...
  auto ResolveDataLayout = [&]() -> Error {
    ...
    TentativeDataLayoutStr = toolchain::UpgradeDataLayoutString(
  ...
  while (true) {
    ...
    // record
    // (https://github.com/toolchain/toolchain-project/blob/b6a93967d9c11e79802b5e75cec1584d6c8aa472/toolchain/lib/Bitcode/Writer/BitcodeWriter.cpp#L4714)
...
Error ModuleSummaryIndexBitcodeReader::parseEntireSummary(unsigned ID) {
  ...
  while (true) {
    ...
    case bitc::FS_TYPE_TESTS:
      ...
      toolchain::append_range(PendingTypeTests, Record);
      break;
...
const std::error_category &toolchain::BitcodeErrorCategory() {
...
toolchain::getBitcodeModuleList(MemoryBufferRef Buffer) {
...
toolchain::getBitcodeFileContents(MemoryBufferRef Buffer) {
  ...
  while (true) {
    ...
    case BitstreamEntry::SubBlock: {
      ...
      if (Entry.ID == bitc::STRTAB_BLOCK_ID) {
        ...
        // with "toolchain-cat -b".
        for (BitcodeModule &I : toolchain::reverse(F.Mods)) {
          if (!I.Strtab.empty())
            break;
          I.Strtab = *Strtab;
        }
...
Expected<std::unique_ptr<Module>>
toolchain::getLazyBitcodeModule(MemoryBufferRef Buffer, LLVMContext &Context,
                           bool ShouldLazyLoadMetadata, bool IsImporting,
                           ParserCallbacks Callbacks) {
...
Expected<std::unique_ptr<Module>> toolchain::getOwningLazyBitcodeModule(
...
Expected<std::unique_ptr<Module>>
toolchain::parseBitcodeFile(MemoryBufferRef Buffer, LLVMContext &Context,
                       ParserCallbacks Callbacks) {
  Expected<BitcodeModule> BM = getSingleModule(Buffer);
  if (!BM)
    return BM.takeError();
...
Expected<std::string> toolchain::getBitcodeTargetTriple(MemoryBufferRef Buffer) {
  Expected<BitstreamCursor> StreamOrErr = initStream(Buffer);
  if (!StreamOrErr)
    return StreamOrErr.takeError();
...
Expected<bool> toolchain::isBitcodeContainingObjCCategory(MemoryBufferRef Buffer) {
  Expected<BitstreamCursor> StreamOrErr = initStream(Buffer);
  if (!StreamOrErr)
    return StreamOrErr.takeError();
...
Expected<std::string> toolchain::getBitcodeProducerString(MemoryBufferRef Buffer) {
  Expected<BitstreamCursor> StreamOrErr = initStream(Buffer);
  if (!StreamOrErr)
    return StreamOrErr.takeError();
...
Error toolchain::readModuleSummaryIndex(MemoryBufferRef Buffer,
...
toolchain::getModuleSummaryIndex(MemoryBufferRef Buffer) {
  Expected<BitcodeModule> BM = getSingleModule(Buffer);
  if (!BM)
    return BM.takeError();
...
Expected<BitcodeLTOInfo> toolchain::getBitcodeLTOInfo(MemoryBufferRef Buffer) {
  Expected<BitcodeModule> BM = getSingleModule(Buffer);
  if (!BM)
    return BM.takeError();
...
Expected<std::unique_ptr<ModuleSummaryIndex>>
toolchain::getModuleSummaryIndexForFile(StringRef Path,
