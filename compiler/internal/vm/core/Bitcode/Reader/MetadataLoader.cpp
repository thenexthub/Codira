...
class MetadataLoader::MetadataLoaderImpl {
  ...
  void upgradeCUVariables() {
    ...
    if (NamedMDNode *CUNodes = TheModule.getNamedMetadata("toolchain.dbg.cu"))
  ...
  DISubprogram *findEnclosingSubprogram(DILocalScope *S) {
    ...
    return ParentSubprogram[InitialScope] =
               toolchain::dyn_cast_or_null<DISubprogram>(S);
  }
...
Error MetadataLoader::MetadataLoaderImpl::parseMetadata(bool ModuleLevel) {
  toolchain::TimeTraceScope timeScope("Parse metadata");
...
#define GET_OR_DISTINCT(CLASS, ARGS)                                           \
  ...
  case bitc::METADATA_STRING_OLD: {
    ...
    // Test for upgrading !toolchain.loop.
