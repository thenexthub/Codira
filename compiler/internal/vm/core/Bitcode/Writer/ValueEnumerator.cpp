...
static UseListOrderStack predictUseListOrder(const Module &M) {
  ...
  for (const Function &F : toolchain::reverse(M)) {
...
void ValueEnumerator::organizeMetadata() {
  ...
  // be unique, the result of toolchain::sort will be deterministic.  There's no need
  // for std::stable_sort.
  toolchain::sort(Order, [this](MDIndex LHS, MDIndex RHS) {
...
void ValueEnumerator::purgeFunction() {
  ...
  for (const auto &V : toolchain::drop_begin(Values, NumModuleValues))
    ValueMap.erase(V.first);
  for (const Metadata *MD : toolchain::drop_begin(MDs, NumModuleMDs))
