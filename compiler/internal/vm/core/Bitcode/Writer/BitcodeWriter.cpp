...
#include "vm/core/Bitstream/BitCodes.h"
#include "vm/core/Bitstream/BitstreamWriter.h"
...
void ModuleBitcodeWriter::writeTypeTable() {
  ...
  for (Type *T : TypeList) {
    ...
    case Type::TargetExtTyID: {
      ...
      toolchain::append_range(TypeVals, TET->int_params());
      break;
    }
...
void ModuleBitcodeWriter::writeFunction(
  ...
  if (!BlockAddressUsers.empty()) {
    ...
    for (auto I : toolchain::enumerate(BlockAddressUsers))
...
void IndexBitcodeWriter::writeModStrings() {
  ...
  forEachModule([&](const StringMapEntry<ModuleHash> &MPSE) {
    ...
    if (toolchain::any_of(Hash, [](uint32_t H) { return H; })) {
...
static void writeFunctionTypeMetadataRecords(BitstreamWriter &Stream,
  ...
  auto WriteConstVCallVec = [&](uint64_t Ty,
    ...
    for (auto &VC : VCs) {
      ...
      toolchain::append_range(Record, VC.Args);
      Stream.EmitRecord(Ty, Record);
    }
  };
...
static void writeWholeProgramDevirtResolutionByArg(
  ...
  toolchain::append_range(NameVals, args);
...
static void collectMemProfCallStacks(
    ...
    MapVector<CallStackId, toolchain::SmallVector<LinearFrameId>> &CallStacks) {
...
static void writeFunctionHeapProfileRecords(
  ...
  for (auto &CI : FS->callsites()) {
    ...
    if (!PerModule)
      toolchain::append_range(Record, CI.Clones);
  ...
  for (auto &AI : FS->allocs()) {
    ...
    if (!PerModule)
      toolchain::append_range(Record, AI.Versions);
...
void ModuleBitcodeWriterBase::writeModuleLevelReferences(
  ...
  toolchain::sort(drop_begin(NameVals, SizeBeforeRefs));
...
void IndexBitcodeWriter::writeCombinedGlobalValueSummary() {
  ...
  auto MaybeEmitOriginalName = [&](GlobalValueSummary &S) {
    ...
    // used in testing the thin link via toolchain-lto.
  ...
  auto EmitCfiFunctions = [&](const CfiFunctionIndex &CfiIndex,
    ...
    for (GlobalValue::GUID GUID : DefOrUseGUIDs) {
      ...
      toolchain::append_range(Functions, Defs);
    }
    if (Functions.empty())
      return;
    toolchain::sort(Functions);
...
/// current toolchain version, and a record for the epoch number.
...
void ModuleBitcodeWriter::writeModuleHash(StringRef View) {
  ...
  if (GenerateHash) {
    ...
    if (ModHash)
      ...
      toolchain::copy(Vals, std::begin(*ModHash));
  }
}
...
void toolchain::WriteBitcodeToFile(const Module &M, raw_ostream &Out,
                              bool ShouldPreserveUseListOrder,
                              const ModuleSummaryIndex *Index,
                              bool GenerateHash, ModuleHash *ModHash) {
...
void toolchain::writeIndexToFile(
...
void toolchain::writeThinLinkBitcodeToFile(const Module &M, raw_ostream &Out,
                                      const ModuleSummaryIndex &Index,
                                      const ModuleHash &ModHash) {
  SmallVector<char, 0> Buffer;
  Buffer.reserve(256 * 1024);
...
void toolchain::embedBitcodeInModule(toolchain::Module &M, toolchain::MemoryBufferRef Buf,
  ...
  if (EmbedBitcode) {
    if (Buf.getBufferSize() == 0 ||
      ...
      toolchain::WriteBitcodeToFile(M, OS, /* ShouldPreserveUseListOrder */ true);
      ModuleData =
  ...
  toolchain::GlobalVariable *GV = new toolchain::GlobalVariable(
      M, ModuleConstant->getType(), true, toolchain::GlobalValue::PrivateLinkage,
      ModuleConstant);
  ...
  if (toolchain::GlobalVariable *Old =
    ...
    assert(Old->hasZeroLiveUses() &&
           "toolchain.embedded.module can only be used once in toolchain.compiler.used");
    GV->takeName(Old);
    Old->eraseFromParent();
  } else {
  ...
  if (EmbedCmdline) {
    ...
    if (toolchain::GlobalVariable *Old = M.getGlobalVariable("toolchain.cmdline", true)) {
      assert(Old->hasZeroLiveUses() &&
             "toolchain.cmdline can only be used once in toolchain.compiler.used");
      GV->takeName(Old);
      Old->eraseFromParent();
    } else {
