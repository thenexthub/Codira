...
saveStringToMap(DataAccessProfData::StringToIndexMap &Map,
                toolchain::UniqueStringSaver &Saver, StringRef Str) {
...
Error DataAccessProfData::deserializeRecords(const unsigned char *&Ptr) {
  SmallVector<StringRef> Strings =
      toolchain::to_vector(toolchain::make_first_range(getStrToIndexMapRef()));
  ...
  for (uint64_t I = 0; I < NumRecords; ++I) {
    ...
    bool IsStringLiteral =
        support::endian::readNext<uint8_t, toolchain::endianness::little>(Ptr);
