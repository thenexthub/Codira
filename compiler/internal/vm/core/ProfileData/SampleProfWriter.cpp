...
#include "vm/core/ProfileData/SampleProfWriter.h"
...
#define DEBUG_TYPE "toolchain-profdata"
...
// Adapter class to toolchain::support::endian::Writer for pwrite().
...
void DefaultFunctionPruningStrategy::Erase(size_t CurrentOutputSize) {
  ...
  for (const NameFunctionSamples &E :
       toolchain::drop_begin(SortedFunctions, SortedFunctions.size() - NumToRemove))
...
std::error_code SampleProfileWriterExtBinaryBase::writeNameTable() {
  ...
  support::endian::Writer Writer(OS, toolchain::endianness::little);
  for (auto N : V)
...
void SampleProfileWriterBinary::addNames(const FunctionSamples &S) {
  ...
  for (const auto &VTableAccessCountMap :
       toolchain::make_second_range(S.getCallsiteTypeCounts())) {
    // Add type name to NameTable.
    for (const auto Type : toolchain::make_first_range(VTableAccessCountMap)) {
      addName(Type);
    }
  }
}
...
void SampleProfileWriterExtBinaryBase::allocSecHdrTable() {
  support::endian::Writer Writer(*OutputStream, toolchain::endianness::little);
