...
LinearCallStackId CallStackRadixTreeBuilder<FrameIdTy>::encodeCallStack(
    const toolchain::SmallVector<FrameIdTy> *CallStack,
    const toolchain::SmallVector<FrameIdTy> *Prev,
    const toolchain::DenseMap<FrameIdTy, LinearFrameId> *MemProfFrameIndexes) {
  ...
  for (FrameIdTy F : toolchain::drop_begin(toolchain::reverse(*CallStack), CommonLen)) {
...
void CallStackRadixTreeBuilder<FrameIdTy>::build(
    toolchain::MapVector<CallStackId, toolchain::SmallVector<FrameIdTy>>
        &&MemProfCallStackData,
    const toolchain::DenseMap<FrameIdTy, LinearFrameId> *MemProfFrameIndexes,
    toolchain::DenseMap<FrameIdTy, FrameStat> &FrameHistogram) {
  ...
  toolchain::SmallVector<CSIdPair, 0> CallStacks = MemProfCallStackData.takeVector();
  ...
  toolchain::sort(CallStacks, [&](const CSIdPair &L, const CSIdPair &R) {
  ...
  const toolchain::SmallVector<FrameIdTy> *Prev = nullptr;
  for (const auto &[CSId, CallStack] : toolchain::reverse(CallStacks)) {
...
template <typename FrameIdTy>
toolchain::DenseMap<FrameIdTy, FrameStat>
computeFrameHistogram(toolchain::MapVector<CallStackId, toolchain::SmallVector<FrameIdTy>>
                          &MemProfCallStackData) {
  toolchain::DenseMap<FrameIdTy, FrameStat> Histogram;
...
template LLVM_ABI toolchain::DenseMap<FrameId, FrameStat>
computeFrameHistogram<FrameId>(
    toolchain::MapVector<CallStackId, toolchain::SmallVector<FrameId>>
        &MemProfCallStackData);
template LLVM_ABI toolchain::DenseMap<LinearFrameId, FrameStat>
computeFrameHistogram<LinearFrameId>(
    toolchain::MapVector<CallStackId, toolchain::SmallVector<LinearFrameId>>
        &MemProfCallStackData);
} // namespace memprof
} // namespace vm::core
