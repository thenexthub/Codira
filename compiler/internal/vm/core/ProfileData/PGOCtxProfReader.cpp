...
void toYaml(yaml::Output &Out,
  ...
  auto AllCS = ::toolchain::make_first_range(Callsites);
  auto MaxIt = ::toolchain::max_element(AllCS);
...
void toolchain::convertCtxProfToYaml(raw_ostream &OS, const PGOCtxProfile &Profile) {
  yaml::Output Out(OS);
  void *SaveInfo = nullptr;
  bool UseDefault = false;
  Out.beginMapping();
