...
#include "vm/core/ProfileData/MIBEntryDef.inc"
#undef MIBEntryDef
  return List;
}
...
static void serializeV2(const IndexedMemProfRecord &Record,
  ...
  endian::Writer LE(OS, toolchain::endianness::little);
...
static void serializeV3(
    ...
    raw_ostream &OS,
    toolchain::DenseMap<CallStackId, LinearCallStackId> &MemProfCallStackIndexes) {
  using namespace support;
...
static IndexedMemProfRecord deserializeV2(const MemProfSchema &Schema,
  ...
  for (uint64_t I = 0; I < NumNodes; I++) {
    ...
    Node.CSId = endian::readNext<CallStackId, toolchain::endianness::little>(Ptr);
  ...
  for (uint64_t J = 0; J < NumCtxs; J++) {
    CallStackId CSId =
        endian::readNext<CallStackId, toolchain::endianness::little>(Ptr);
...
static IndexedMemProfRecord deserializeV3(const MemProfSchema &Schema,
  ...
  for (uint64_t I = 0; I < NumNodes; I++) {
    IndexedAllocationInfo Node;
    Node.CSId =
        endian::readNext<LinearCallStackId, toolchain::endianness::little>(Ptr);
    Node.Info.deserialize(Schema, Ptr);
    Ptr += SerializedSize;
    Record.AllocSites.push_back(Node);
  }
...
static IndexedMemProfRecord deserializeV4(const MemProfSchema &Schema,
  ...
  for (uint64_t J = 0; J < NumCtxs; J++) {
    ...
    for (uint64_t K = 0; K < NumGuids; ++K)
      Guids.push_back(
          endian::readNext<GlobalValue::GUID, toolchain::endianness::little>(Ptr));
...
MemProfRecord IndexedMemProfRecord::toMemProfRecord(
    toolchain::function_ref<std::vector<Frame>(const CallStackId)> Callback) const {
...
GlobalValue::GUID getGUID(const StringRef FunctionName) {
  // Canonicalize the function name to drop suffixes such as ".toolchain.". Note
