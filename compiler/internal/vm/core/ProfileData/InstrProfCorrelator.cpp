...
toolchain::Expected<std::unique_ptr<InstrProfCorrelator::Context>>
...
toolchain::Expected<std::unique_ptr<InstrProfCorrelator>>
InstrProfCorrelator::get(StringRef Filename, ProfCorrelatorKind FileKind,
  ...
  if (BIDFetcher) {
    ...
    if (!Path)
      return make_error<InstrProfError>(
          instrprof_error::unable_to_correlate_profile,
          "Missing build ID: " + toolchain::toHex(BIs.front(),
                                             /*LowerCase=*/true));
    Filename = *Path;
  }
...
toolchain::Expected<std::unique_ptr<InstrProfCorrelatorImpl<IntPtrT>>>
