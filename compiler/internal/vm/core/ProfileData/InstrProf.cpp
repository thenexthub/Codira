...
#include "vm/core/Support/SwapByteOrder.h"
...
class InstrProfErrorCategoryType : public std::error_category {
  const char *name() const noexcept override { return "toolchain.instrprof"; }
...
const std::error_category &toolchain::instrprof_category() {
...
#define INSTR_PROF_SECT_ENTRY(Kind, SectNameCommon, SectNameCoff, Prefix)      \
  SectNameCommon,
#include "vm/core/ProfileData/InstrProfData.inc"
};
...
ProfOStream::ProfOStream(raw_fd_ostream &FD)
    : IsFDOStream(true), OS(FD), LE(FD, toolchain::endianness::little) {}

ProfOStream::ProfOStream(raw_string_ostream &STR)
    : IsFDOStream(false), OS(STR), LE(STR, toolchain::endianness::little) {}
  ...
  if (IsFDOStream) {
    ...
    for (const auto &K : P) {
      for (int I = 0, E = K.D.size(); I != E; I++) {
        uint64_t Bytes =
            endian::byte_swap<uint64_t>(K.D[I], toolchain::endianness::little);
...
static StringRef stripDirPrefix(StringRef PathNameStr, uint32_t NumPrefix) {
  ...
  for (const auto &CI : PathNameStr) {
    ++Pos;
    if (toolchain::sys::path::is_separator(CI)) {
      LastPos = Pos;
      --Count;
    }
    if (Count == 0)
...
StringRef InstrProfSymtab::getCanonicalName(StringRef PGOName) {
  ...
  // suffix ".toolchain." added to the function name. We need to add the
  ...
  // pattern ".xxx" which is kept before matching, other suffixes ".toolchain." and
  ...
  // result.
  const SmallVector<StringRef> SuffixesToRemove{".toolchain.", ".part."};
...
void TemporalProfTraceTy::createBPFunctionNodes(
  ...
  if (RemoveOutlierUNs) {
    ...
    for (auto &[Id, UNs] : IdToUNs)
      toolchain::erase_if(UNs, [&](auto &UN) {
  ...
  toolchain::sort(Nodes, [&](auto &L, auto &R) {
...
void getValueForSiteInstrProf(const void *R, InstrProfValueData *Dst,
  ...
  const auto *IPR = reinterpret_cast<const InstrProfRecord *>(R);
  toolchain::copy(IPR->getValueArrayForSite(K, S), Dst);
}
...
void ValueProfRecord::swapBytes(toolchain::endianness Old, toolchain::endianness New) {
  using namespace support;

  if (Old == New)
    return;

  if (toolchain::endianness::native != Old) {
...
void ValueProfData::swapBytesToHost(toolchain::endianness Endianness) {
  using namespace support;

  if (Endianness == toolchain::endianness::native)
    return;
  ...
  ValueProfRecord *VR = getFirstValueProfRecord(this);
  for (uint32_t K = 0; K < NumValueKinds; K++) {
    VR->swapBytes(Endianness, toolchain::endianness::native);
    VR = getValueProfRecordNext(VR);
  }
}

void ValueProfData::swapBytesFromHost(toolchain::endianness Endianness) {
  using namespace support;
  ...
  for (uint32_t K = 0; K < NumValueKinds; K++) {
    ...
    VR->swapBytes(toolchain::endianness::native, Endianness);
    VR = NVR;
  }
  sys::swapByteOrder<uint32_t>(TotalSize);
  sys::swapByteOrder<uint32_t>(NumValueKinds);
...
Error OverlapStats::accumulateCounts(const std::string &BaseFilename,
  ...
  auto GetProfileSum = [IsCS](const std::string &Filename,
    ...
    // This function is only used from toolchain-profdata that doesn't use any kind
...
Expected<Header> Header::readFromBuffer(const unsigned char *Buffer) {
  ...
  H.Magic = endian::readNext<uint64_t, toolchain::endianness::little>(Buffer);
  // Check the magic number.
  if (H.Magic != IndexedInstrProf::Magic)
    return make_error<InstrProfError>(instrprof_error::bad_magic);

  // Read the version.
  H.Version = endian::readNext<uint64_t, toolchain::endianness::little>(Buffer);
  ...
  H.HashType = endian::readNext<uint64_t, toolchain::endianness::little>(Buffer);
  H.HashOffset = endian::readNext<uint64_t, toolchain::endianness::little>(Buffer);
  if (H.getIndexedProfileVersion() >= 8)
    H.MemProfOffset =
        endian::readNext<uint64_t, toolchain::endianness::little>(Buffer);
