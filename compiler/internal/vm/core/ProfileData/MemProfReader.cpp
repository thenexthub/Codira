...
#include "vm/core/DebugInfo/Symbolize/SymbolizableModule.h"
#include "vm/core/DebugInfo/Symbolize/SymbolizableObjectFile.h"
...
#include "vm/core/ProfileData/MemProfData.inc"
#include "vm/core/ProfileData/MemProfReader.h"
#include "vm/core/ProfileData/MemProfSummaryBuilder.h"
#include "vm/core/ProfileData/MemProfYAML.h"
...
toolchain::SmallVector<SegmentEntry> readSegmentEntries(const char *Ptr) {
  using namespace support;

  const uint64_t NumItemsToRead =
      endian::readNext<uint64_t, toolchain::endianness::little>(Ptr);
  toolchain::SmallVector<SegmentEntry> Items;
...
toolchain::SmallVector<std::pair<uint64_t, MemInfoBlock>>
readMemInfoBlocksV3(const char *Ptr) {
  using namespace support;

  const uint64_t NumItemsToRead =
      endian::readNext<uint64_t, toolchain::endianness::little, unaligned>(Ptr);

  toolchain::SmallVector<std::pair<uint64_t, MemInfoBlock>> Items;
  for (uint64_t I = 0; I < NumItemsToRead; I++) {
    const uint64_t Id =
        endian::readNext<uint64_t, toolchain::endianness::little, unaligned>(Ptr);
...
#define READ_MIB_FIELD(FIELD)                                                  \
  MIB.FIELD = endian::readNext<decltype(MIB.FIELD), toolchain::endianness::little,  \
                               unaligned>(Ptr)
      ...
      for (uint64_t J = 0; J < MIB.AccessHistogramSize; J++) {
        if (!IsHistogramEncoded) {
          ((uint64_t *)MIB.AccessHistogram)[J] =
              endian::readNext<uint64_t, toolchain::endianness::little, unaligned>(
                  Ptr);
        } else {
          // The encoded on-disk format (V5 onwards) uses uint16_t.
          const uint16_t Val =
              endian::readNext<uint16_t, toolchain::endianness::little, unaligned>(
                  Ptr);
...
bool isRuntimePath(const StringRef Path) {
  const StringRef Filename = toolchain::sys::path::filename(Path);
...
RawMemProfReader::create(std::unique_ptr<MemoryBuffer> Buffer,
  ...
  auto BinaryOr = toolchain::object::createBinary(ProfiledBinary);
  if (!BinaryOr) {
...
void RawMemProfReader::printYAML(raw_ostream &OS) {
  ...
  for (const auto &Entry : SegmentInfo) {
    ...
    OS << "    Start: 0x" << toolchain::utohexstr(Entry.Start) << "\n";
    OS << "    End: 0x" << toolchain::utohexstr(Entry.End) << "\n";
    OS << "    Offset: 0x" << toolchain::utohexstr(Entry.Offset) << "\n";
  }
...
Error RawMemProfReader::initialize(std::unique_ptr<MemoryBuffer> DataBuffer) {
  ...
  // symbolization.
  auto *Elf64LEObject = toolchain::cast<toolchain::object::ELF64LEObjectFile>(ElfObject);
  const toolchain::object::ELF64LEFile &ElfFile = Elf64LEObject->getELFFile();
...
Error RawMemProfReader::setupForSymbolization() {
  ...
  int NumMatched = 0;
  for (const auto &Entry : SegmentInfo) {
    toolchain::ArrayRef<uint8_t> SegmentId(Entry.BuildId, Entry.BuildIdSize);
...
Error RawMemProfReader::mapRawProfileToRecords() {
  ...
  using LocationPtr = const toolchain::SmallVector<FrameId> *;
  toolchain::MapVector<GlobalValue::GUID, toolchain::SetVector<LocationPtr>>
  ...
  for (const auto &[StackId, MIB] : CallstackProfileData) {
    ...
    toolchain::SmallVector<FrameId> Callstack;
    Callstack.reserve(It->getSecond().size());

    toolchain::ArrayRef<uint64_t> Addresses = It->getSecond();
...
Error RawMemProfReader::symbolizeAndFilterStackFrames(
    std::unique_ptr<toolchain::symbolize::SymbolizableModule> Symbolizer) {
  ...
  for (auto &Entry : StackMap) {
    ...
    toolchain::erase_if(CallStack, [&AllVAddrsToDiscard](const uint64_t A) {
...
RawMemProfReader::peekBuildIds(MemoryBuffer *DataBuffer) {
  ...
  toolchain::SetVector<std::string, std::vector<std::string>,
                  toolchain::SmallSet<std::string, 10>>
      BuildIds;
  while (Next < DataBuffer->getBufferEnd()) {
    const auto *Header = reinterpret_cast<const memprof::Header *>(Next);

    const toolchain::SmallVector<SegmentEntry> Entries =
