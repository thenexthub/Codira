...
public:
  ...
  toolchain::endianness ValueProfDataEndianness = toolchain::endianness::little;
...
void InstrProfWriter::setValueProfDataEndianness(toolchain::endianness Endianness) {
...
bool InstrProfWriter::addMemProfCallStack(
    ...
    const toolchain::SmallVector<memprof::FrameId> &CallStack,
    function_ref<void(Error)> Warn) {
...
void InstrProfWriter::addBinaryIds(ArrayRef<toolchain::object::BuildID> BIs) {
  toolchain::append_range(BinaryIds, BIs);
}
...
void InstrProfWriter::addTemporalProfileTraces(
  ...
  toolchain::erase_if(SrcTraces, [](auto &T) { return T.FunctionNameRefs.empty(); });
  ...
  toolchain::shuffle(SrcTraceIt, SrcTraces.end(), RNG);
...
bool InstrProfWriter::shouldEncodeData(const ProfilingData &PD) {
  ...
  for (const auto &Func : PD) {
    ...
    if (toolchain::any_of(IPR.Counts, [](uint64_t Count) { return Count > 0; }))
      return true;
    if (toolchain::any_of(IPR.BitmapBytes, [](uint8_t Byte) { return Byte > 0; }))
      return true;
  }
  return false;
}
...
Error InstrProfWriter::writeBinaryIds(ProfOStream &OS) {
  ...
  toolchain::sort(BinaryIds);
  BinaryIds.erase(toolchain::unique(BinaryIds), BinaryIds.end());
...
Error InstrProfWriter::writeImpl(ProfOStream &OS) {
  ...
  toolchain::sort(OrderedData, less_first());
...
Error InstrProfWriter::writeText(raw_fd_ostream &OS) {
  ...
  toolchain::sort(OrderedFuncData, [](const RecordType &A, const RecordType &B) {
