...
#include "vm/core/ProfileData/SampleProfReader.h"
...
class SampleProfErrorCategoryType : public std::error_category {
  const char *name() const noexcept override { return "toolchain.sampleprof"; }
...
const std::error_category &toolchain::sampleprof_category() {
...
raw_ostream &toolchain::sampleprof::operator<<(raw_ostream &OS,
  ...
  if (!CallsiteSamples.empty()) {
    ...
    for (const auto *Element : SortedCallsiteSamples.get()) {
      ...
      for (const FunctionSamples &FuncSample :
           toolchain::make_second_range(FunctionSampleMap)) {
        OS.indent(Indent + 2);
...
void sampleprof::sortFuncProfiles(
  ...
  toolchain::stable_sort(SortedProfiles, [](const NameFunctionSamples &A,
...
void FunctionSamples::findAllNames(DenseSet<FunctionId> &NameSet) const {
  ...
  for (const auto &BS : BodySamples)
    NameSet.insert_range(toolchain::make_first_range(BS.second.getCallTargets()));
