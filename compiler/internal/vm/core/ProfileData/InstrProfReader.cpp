...
readBinaryIdsInternal(const MemoryBuffer &DataBuffer,
                      ...
                      std::vector<toolchain::object::BuildID> &BinaryIds,
                      const toolchain::endianness Endian) {
...
static void printBinaryIdsInternal(raw_ostream &OS,
                                   ArrayRef<toolchain::object::BuildID> BinaryIds) {
  OS << "Binary IDs: \n";
...
bool RawInstrProfReader<IntPtrT>::hasFormat(const MemoryBuffer &DataBuffer) {
  ...
  return RawInstrProf::getMagic<IntPtrT>() == Magic ||
         toolchain::byteswap(RawInstrProf::getMagic<IntPtrT>()) == Magic;
}
...
data_type InstrProfLookupTrait::ReadData(StringRef K, const unsigned char *D,
  ...
  while (D < End) {
    ...
    if (D + sizeof(uint64_t) >= End)
      return data_type();
    uint64_t Hash = endian::readNext<uint64_t, toolchain::endianness::little>(D);
    ...
    if (GET_VERSION(FormatVersion) != IndexedInstrProf::ProfVersion::Version1) {
      if (D + sizeof(uint64_t) > End)
        return data_type();
      CountsSize = endian::readNext<uint64_t, toolchain::endianness::little>(D);
    }
    // Read counter values.
    ...
    for (uint64_t J = 0; J < CountsSize; ++J)
      CounterBuffer.push_back(
          endian::readNext<uint64_t, toolchain::endianness::little>(D));
    ...
    if (GET_VERSION(FormatVersion) > IndexedInstrProf::ProfVersion::Version10) {
      ...
      if (D + sizeof(uint64_t) > End)
        return data_type();
      BitmapBytes = endian::readNext<uint64_t, toolchain::endianness::little>(D);
...
template <typename HashTableImpl>
class toolchain::InstrProfReaderItaniumRemapper
...
public:
  ...
  static void reconstituteName(StringRef OrigName, StringRef ExtractedName,
    ...
    toolchain::append_range(Out, Replacement);
...
IndexedInstrProfReader::readSummary(IndexedInstrProf::ProfVersion Version,
  ...
  if (Version >= IndexedInstrProf::Version4) {
    ...
    uint64_t NFields = endian::byte_swap<uint64_t>(
        SummaryInLE->NumSummaryFields, toolchain::endianness::little);
    uint64_t NEntries = endian::byte_swap<uint64_t>(
        SummaryInLE->NumCutoffEntries, toolchain::endianness::little);
    ...
    for (unsigned I = 0; I < SummarySize / sizeof(uint64_t); I++)
      Dst[I] = endian::byte_swap<uint64_t>(Src[I], toolchain::endianness::little);
    ...
    std::unique_ptr<toolchain::ProfileSummary> &Summary =
...
IndexedMemProfReader::getMemProfCallerCalleePairs() const {
  ...
  for (auto &[CallerGUID, CallList] : Pairs) {
    toolchain::sort(CallList);
    CallList.erase(toolchain::unique(CallList), CallList.end());
  }
  ...
  if (DataAccessProfileData != nullptr) {
    ...
    for (uint64_t Hash : DataAccessProfileData->getKnownColdHashes())
      AllMemProfData.YamlifiedDataAccessProfiles.KnownColdStrHashes.push_back(
          Hash);
    toolchain::stable_sort(AllMemProfData.YamlifiedDataAccessProfiles.Records,
                      [](const toolchain::memprof::DataAccessProfRecord &lhs,
                         const toolchain::memprof::DataAccessProfRecord &rhs) {
