...
#include "vm/core/ProfileData/Coverage/CoverageMappingReader.h"
    ...
    for (size_t I = 1; I < NumFilenames; ++I) {
      ...
      if (sys::path::is_absolute(Filename)) {
        ...
        if (!CompilationDir.empty())
          ...
          P.assign(CWD);
        toolchain::sys::path::append(P, Filename);
...
struct CovMapFuncRecordReader {
  ...
  template <class IntPtrT, toolchain::endianness Endian>
...
template <CovMapVersion Version, class IntPtrT, toolchain::endianness Endian>
  ...
  Expected<const char *> readCoverageHeader(const char *CovBuf,
    ...
    if (Version >= CovMapVersion::Version4) {
      ...
      int64_t FilenamesRef =
          toolchain::IndexedInstrProf::ComputeHash(FilenameRegion);
      auto Insert =
...
template <typename T, toolchain::endianness Endian>
...
BinaryCoverageReader::createCoverageReaderFromBuffer(
    ...
    toolchain::endianness Endian, StringRef CompilationDir) {
  ...
  if (BytesInAddress == 4 && Endian == toolchain::endianness::little) {
    if (Error E = readCoverageMappingData<uint32_t, toolchain::endianness::little>(
            ProfileNames, Coverage, FuncRecordsRef, Reader->MappingRecords,
            CompilationDir, Reader->Filenames))
      return std::move(E);
  } else if (BytesInAddress == 4 && Endian == toolchain::endianness::big) {
    if (Error E = readCoverageMappingData<uint32_t, toolchain::endianness::big>(
            ProfileNames, Coverage, FuncRecordsRef, Reader->MappingRecords,
            CompilationDir, Reader->Filenames))
      return std::move(E);
  } else if (BytesInAddress == 8 && Endian == toolchain::endianness::little) {
    if (Error E = readCoverageMappingData<uint64_t, toolchain::endianness::little>(
            ProfileNames, Coverage, FuncRecordsRef, Reader->MappingRecords,
            CompilationDir, Reader->Filenames))
      return std::move(E);
  } else if (BytesInAddress == 8 && Endian == toolchain::endianness::big) {
    if (Error E = readCoverageMappingData<uint64_t, toolchain::endianness::big>(
            ProfileNames, Coverage, FuncRecordsRef, Reader->MappingRecords,
            CompilationDir, Reader->Filenames))
      return std::move(E);
  } else
...
loadTestingFormat(StringRef Data, StringRef CompilationDir) {
  ...
  toolchain::endianness Endian = toolchain::endianness::little;
  ...
  auto Version =
      CovMapVersion(CovHeader->getVersion<toolchain::endianness::little>());
  ...
  if (TestingVersion == uint64_t(TestingFormatVersion::Version1)) {
    if (Version < CovMapVersion::Version4) {
      ...
      auto FilenamesSize =
          CovHeader->getFilenamesSize<toolchain::endianness::little>();
...
loadBinaryFormat(std::unique_ptr<Binary> Bin, StringRef Arch,
  ...
  toolchain::endianness Endian =
      OF->isLittleEndian() ? toolchain::endianness::little : toolchain::endianness::big;
  ...
  if (auto E = CoverageRecordsSections.takeError()) {
    ...
    for (SectionRef Section : *CoverageRecordsSections) {
      ...
      FuncRecordsBuffer = toolchain::copy(CoverageRecords, FuncRecordsBuffer);
      FuncRecordsBuffer =
