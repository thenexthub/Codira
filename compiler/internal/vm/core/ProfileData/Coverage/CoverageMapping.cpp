...
//===----------------------------------------------------------------------===//
//
// This file contains support for clang's and toolchain's instrumentation based
// code coverage.
//
//===----------------------------------------------------------------------===//

#include "vm/core/ProfileData/Coverage/CoverageMapping.h"
...
Counter CounterExpressionBuilder::simplify(Counter ExpressionTree) {
  ...
  toolchain::sort(Terms, [](const Term &LHS, const Term &RHS) {
...
static unsigned getMaxBitmapSize(const CoverageMappingRecord &Record,
  ...
  if (IsVersion11)
    MaxBitmapIdx = MaxBitmapIdx * CHAR_BIT +
                   toolchain::alignTo(uint64_t(1) << NumConditions, CHAR_BIT);
...
Error CoverageMapping::loadFromFile(
  ...
  if (FoundBinaryIDs && !Readers.empty()) {
    toolchain::append_range(*FoundBinaryIDs,
                       toolchain::map_range(BinaryIDs, [](object::BuildIDRef BID) {
...
Expected<std::unique_ptr<CoverageMapping>> CoverageMapping::load(
  ...
  for (const auto &File : toolchain::enumerate(ObjectFilenames)) {
  ...
  if (BIDFetcher) {
    ...
    if (!ProfileBinaryIDs.empty()) {
      ...
      toolchain::sort(FoundBinaryIDs, Compare);
      std::set_difference(
      ...
      if (PathOpt) {
        ...
        return createFileError(
            ...
            createStringError(errc::no_such_file_or_directory,
                              "Missing binary ID: " +
                                  toolchain::toHex(BinaryID, /*LowerCase=*/true)));
      }
    }
  }
...
class SegmentBuilder {
  ...
  static void sortNestedRegions(MutableArrayRef<CountedRegion> Regions) {
    toolchain::sort(Regions, [](const CountedRegion &LHS, const CountedRegion &RHS) {
...
std::vector<StringRef> CoverageMapping::getUniqueSourceFiles() const {
  ...
  for (const auto &Function : getCoveredFunctions())
    toolchain::append_range(Filenames, Function.Filenames);
  toolchain::sort(Filenames);
  auto Last = toolchain::unique(Filenames);
...
class CoverageMappingErrorCategoryType : public std::error_category {
  const char *name() const noexcept override { return "toolchain.coveragemap"; }
...
const std::error_category &toolchain::coverage::coveragemap_category() {
