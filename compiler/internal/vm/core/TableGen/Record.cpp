...
RecordRecTy::RecordRecTy(RecordKeeper &RK, ArrayRef<const Record *> Classes)
  ...
  toolchain::uninitialized_copy(Classes, getTrailingObjects());
}
...
const RecordRecTy *RecordRecTy::get(RecordKeeper &RK,
  ...
  toolchain::sort(Classes, [](const Record *LHS, const Record *RHS) {
...
bool RecordRecTy::isSubClassOf(const Record *Class) const {
  return toolchain::any_of(getClasses(), [Class](const Record *MySuperClass) {
...
bool RecordRecTy::typeIsConvertibleTo(const RecTy *RHS) const {
  ...
  return toolchain::all_of(RTy->getClasses(), [this](const Record *TargetClass) {
...
static const RecordRecTy *resolveRecordTypes(const RecordRecTy *T1,
  ...
  while (!Stack.empty()) {
    ...
    if (T2->isSubClassOf(R))
      ...
      toolchain::append_range(Stack, make_first_range(R->getDirectSuperClasses()));
  }

  return RecordRecTy::get(T1->getRecordKeeper(), CommonSuperClasses);
}

const RecTy *toolchain::resolveTypes(const RecTy *T1, const RecTy *T2) {
  if (T1 == T2)
    return T1;
...
BitsInit::BitsInit(RecordKeeper &RK, ArrayRef<const Init *> Bits)
    : TypedInit(IK_BitsInit, BitsRecTy::get(RK, Bits.size())),
      NumBits(Bits.size()) {
  toolchain::uninitialized_copy(Bits, getTrailingObjects());
}
...
ListInit::ListInit(ArrayRef<const Init *> Elements, const RecTy *EltTy)
  ...
  toolchain::uninitialized_copy(Elements, getTrailingObjects());
}
    ...
    if (const auto *LHSList = dyn_cast<ListInit>(LHS)) {
      ...
      auto Flatten =
        ...
        for (const Init *InnerInit : List->getElements()) {
          ...
          if (!InnerList)
            return std::nullopt;
          toolchain::append_range(Flattened, InnerList->getElements());
        };
        return Flattened;
      };
...
static const ListInit *ConcatListInits(const ListInit *LHS,
  ...
  toolchain::append_range(Args, *LHS);
  toolchain::append_range(Args, *RHS);
...
const Init *BinOpInit::Fold(const Record *CurRec) const {
  ...
  case CONCAT: {
    ...
    if (LHSs && RHSs) {
      ...
      toolchain::append_range(Args, LHSs->getArgAndNames());
      toolchain::append_range(Args, RHSs->getArgAndNames());
  ...
  case MATCH: {
    ...
    toolchain::Regex Matcher(RegexStr);
    if (!Matcher.isValid())
  ...
  case LISTCONCAT: {
    ...
    if (LHSs && RHSs) {
      SmallVector<const Init *, 8> Args;
      toolchain::append_range(Args, *LHSs);
      toolchain::append_range(Args, *RHSs);
...
CondOpInit::CondOpInit(ArrayRef<const Init *> Conds,
  ...
  toolchain::uninitialized_copy(Conds, TrailingObjects);
  toolchain::uninitialized_copy(Values, TrailingObjects + NumConds);
}
...
DagInit::DagInit(const Init *V, const StringInit *VN,
  ...
  toolchain::uninitialized_copy(Args, getTrailingObjects<const Init *>());
  toolchain::uninitialized_copy(ArgNames, getTrailingObjects<const StringInit *>());
}
...
std::optional<unsigned> DagInit::getArgNo(StringRef Name) const {
  ...
  auto It = toolchain::find_if(ArgNames, [Name](const StringInit *ArgName) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const Record &R) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const RecordKeeper &RK) {
  OS << "------------- Classes -----------------\n";
...
const Init *RecordResolver::resolve(const Init *VarName) {
  ...
  if (toolchain::is_contained(Stack, VarName))
