...
/// machine function.  This is like toolchain-level X->replaceAllUsesWith(Y),
...
void MachineRegisterInfo::replaceRegWith(Register FromReg, Register ToReg) {
  ...
  for (MachineOperand &O : toolchain::make_early_inc_range(reg_operands(FromReg))) {
    if (ToReg.isPhysical()) {
...
void MachineRegisterInfo::markUsesInDebugValueAsUndef(Register Reg) const {
  ...
  for (MachineInstr &UseMI : toolchain::make_early_inc_range(use_instructions(Reg))) {
...
void MachineRegisterInfo::disableCalleeSavedRegister(MCRegister Reg) {
  ...
  for (MCRegAliasIterator AI(Reg, TRI, true); AI.isValid(); ++AI)
    toolchain::erase(UpdatedCSRs, *AI);
}
