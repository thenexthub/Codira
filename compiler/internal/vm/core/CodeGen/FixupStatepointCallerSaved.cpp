...
char &toolchain::FixupStatepointCallerSavedID = FixupStatepointCallerSavedLegacy::ID;
...
public:
  ...
  void reset(const MachineBasicBlock *EHPad) {
    ...
    if (EHPad)
      if (auto It = GlobalIndices.find(EHPad); It != GlobalIndices.end())
        ReservedSlots.insert_range(toolchain::make_second_range(It->second));
  }
  ...
  void sortRegisters(SmallVectorImpl<Register> &Regs) {
    if (!FixupSCSExtendSlotSize)
      return;
    toolchain::sort(Regs, [&](Register &A, Register &B) {
...
public:
  StatepointState(MachineInstr &MI, const uint32_t *Mask,
    ...
    assert(toolchain::count_if(MBB->successors(), IsEHPad) < 2 && "multiple EHPads");

    auto It = toolchain::find_if(MBB->successors(), IsEHPad);
    if (It != MBB->succ_end())
      EHPad = *It;
  }
