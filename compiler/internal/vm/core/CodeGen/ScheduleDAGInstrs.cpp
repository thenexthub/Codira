  ...
  if (TrackLaneMasks) {
    ...
    if (MO.getSubReg() != 0 && MO.isUndef()) {
      ...
      for (const MachineOperand &OtherMO :
           toolchain::drop_begin(MI->operands(), OperIdx + 1))
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const PseudoSourceValue* PSV) {
  PSV->printCustom(OS);
  return OS;
}
...
[[maybe_unused]]
raw_ostream &toolchain::operator<<(raw_ostream &OS, const ILPValue &Val) {
  Val.print(OS);
  return OS;
}
