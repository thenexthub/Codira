...
void IntrinsicLowering::LowerIntrinsicCall(CallInst *CI) {
  ...
  case Intrinsic::stackrestore: {
    if (!Warned)
      errs() << "WARNING: this target does not support the toolchain.stack"
  ...
  case Intrinsic::get_dynamic_area_offset:
    errs() << "WARNING: this target does not support the custom toolchain.get."
              "dynamic.area.offset.  It is being lowered to a constant 0\n";
    // Just lower it to a constant 0 because for most targets
    // @toolchain.get.dynamic.area.offset is lowered to zero.
    CI->replaceAllUsesWith(ConstantInt::get(CI->getType(), 0));
    break;
  case Intrinsic::returnaddress:
  case Intrinsic::frameaddress:
    errs() << "WARNING: this target does not support the toolchain."
  ...
  case Intrinsic::addressofreturnaddress:
    errs() << "WARNING: this target does not support the "
              "toolchain.addressofreturnaddress intrinsic.\n";
    CI->replaceAllUsesWith(
        ConstantPointerNull::get(cast<PointerType>(CI->getType())));
    break;
  ...
  case Intrinsic::readcyclecounter: {
    errs() << "WARNING: this target does not support the toolchain.readcyclecoun"
           << "ter intrinsic.  It is being lowered to a constant 0\n";
    CI->replaceAllUsesWith(ConstantInt::get(Type::getInt64Ty(Context), 0));
    break;
  }
  case Intrinsic::readsteadycounter: {
    errs() << "WARNING: this target does not support the toolchain.readsteadycounter"
