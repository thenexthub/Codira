...
#include "vm/core/ADT/SparseSet.h"
...
#include "vm/core/CodeGen/RegAllocCommon.h"
...
private:
  ...
  bool isClobberedByRegMasks(MCRegister PhysReg) const {
    return toolchain::any_of(RegMasks, [PhysReg](const uint32_t *Mask) {
  ...
  if (LRI != LiveVirtRegs.end()) {
    ...
    if (PrevReg != 0 && isRegUsedInInstr(PrevReg, true)) {
      ...
      BuildMI(*MBB, InsertBefore, MI.getDebugLoc(),
          ...
          .addReg(LRI->PhysReg, toolchain::RegState::Kill);
    }
...
void RegAllocFastImpl::findAndSortDefOperandIndexes(const MachineInstr &MI) {
  ...
  toolchain::sort(DefOperandIndexes, [&](unsigned I0, unsigned I1) {
...
void RegAllocFastImpl::handleDebugValue(MachineInstr &MI) {
  ...
  for (const auto &MO : MI.debug_operands()) {
    ...
    SmallVector<MachineOperand *> DbgOps(
        toolchain::make_pointer_range(MI.getDebugOperandsForReg(Reg)));
...
FunctionPass *toolchain::createFastRegisterAllocator() { return new RegAllocFast(); }

FunctionPass *toolchain::createFastRegisterAllocator(RegAllocFilterFunc Ftor,
                                                bool ClearVirtRegs) {
