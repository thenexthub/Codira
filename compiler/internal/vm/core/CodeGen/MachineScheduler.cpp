...
#include "vm/core/ADT/PriorityQueue.h"
...
#include "vm/core/CodeGen/MachinePassRegistry.h"
...
#include "vm/core/CodeGen/ScheduleDFS.h"
...
cl::opt<MISched::Direction> toolchain::PreRADirection(
...
cl::opt<bool> toolchain::VerifyScheduling(
    "verify-misched", cl::Hidden,
    cl::desc("Verify machine instrs before and after machine scheduling"));

#ifndef NDEBUG
cl::opt<bool> toolchain::ViewMISchedDAGs(
    "view-misched-dags", cl::Hidden,
    cl::desc("Pop up a window to show MISched dags after they are processed"));
cl::opt<bool> toolchain::PrintDAGs("misched-print-dags", cl::Hidden,
...
#else
const bool toolchain::ViewMISchedDAGs = false;
const bool toolchain::PrintDAGs = false;
...
char &toolchain::MachineSchedulerID = MachineSchedulerLegacy::ID;
...
char &toolchain::PostMachineSchedulerID = PostMachineSchedulerLegacy::ID;
...
LLVM_DUMP_METHOD void ScheduleDAGMI::dumpScheduleTraceTopDown() const {
  ...
  dbgs() << toolchain::left_justify("Cycle", HeaderColWidth);
  for (unsigned C = FirstCycle; C <= LastCycle; ++C)
    dbgs() << toolchain::left_justify("| " + std::to_string(C), ColWidth);
  dbgs() << "|\n";
  ...
  for (MachineInstr &MI : *this) {
    ...
    dbgs() << toolchain::left_justify(NodeName, HeaderColWidth);
    unsigned C = FirstCycle;
    for (; C <= LastCycle; ++C) {
      if (C == SU->TopReadyCycle)
        dbgs() << toolchain::left_justify("| i", ColWidth);
      else
        dbgs() << toolchain::left_justify("|", ColWidth);
    }
    dbgs() << "|\n";
    const MCSchedClassDesc *SC = getSchedClass(SU);
  ...
  for (const VRegMaskOrUnit &P : LiveUses) {
    ...
    if (ShouldTrackLaneMasks) {
      ...
      for (const VReg2SUnit &V2SU
        ...
        if (toolchain::any_of(PDiff, [](const PressureChange &Change) {
...
toolchain::createLoadClusterDAGMutation(const TargetInstrInfo *TII,
                                   const TargetRegisterInfo *TRI,
                                   bool ReorderWhileClustering) {
...
std::unique_ptr<ScheduleDAGMutation>
toolchain::createStoreClusterDAGMutation(const TargetInstrInfo *TII,
                                    const TargetRegisterInfo *TRI,
                                    bool ReorderWhileClustering) {
...
std::unique_ptr<ScheduleDAGMutation>
toolchain::createCopyConstrainDAGMutation(const TargetInstrInfo *TII,
                                     const TargetRegisterInfo *TRI) {
...
bool toolchain::tryLess(int TryVal, int CandVal,
...
bool toolchain::tryGreater(int TryVal, int CandVal,
                      GenericSchedulerBase::SchedCandidate &TryCand,
                      GenericSchedulerBase::SchedCandidate &Cand,
                      GenericSchedulerBase::CandReason Reason) {
  if (TryVal > CandVal) {
...
bool toolchain::tryLatency(GenericSchedulerBase::SchedCandidate &TryCand,
                      GenericSchedulerBase::SchedCandidate &Cand,
                      SchedBoundary &Zone) {
  if (Zone.isTop()) {
...
bool toolchain::tryPressure(const PressureChange &TryP, const PressureChange &CandP,
                       GenericSchedulerBase::SchedCandidate &TryCand,
                       GenericSchedulerBase::SchedCandidate &Cand,
                       GenericSchedulerBase::CandReason Reason,
                       const TargetRegisterInfo *TRI,
...
unsigned toolchain::getWeakLeft(const SUnit *SU, bool isTop) {
...
int toolchain::biasPhysReg(const SUnit *SU, bool isTop) {
  const MachineInstr *MI = SU->getInstr();
...
template <>
struct toolchain::GraphTraits<ScheduleDAGMI *> : public GraphTraits<ScheduleDAG *> {
};

template <>
struct toolchain::DOTGraphTraits<ScheduleDAGMI *> : public DefaultDOTGraphTraits {
...
unsigned ResourceSegments::getFirstAvailableAt(
  ...
  assert(toolchain::is_sorted(_Intervals, sortIntervals) &&
