...
NodeList Liveness::getAllReachingDefs(RegisterRef RefRR,
  ...
  for (NodeId T : TmpInst) {
    ...
    toolchain::append_range(RDefs, Ds);
  ...
  toolchain::erase_if(RDefs, DeadP);
...
void Liveness::computePhiInfo() {
  ...
  for (NodeAddr<BlockNode *> BA : Blocks) {
    ...
    toolchain::append_range(Phis, Ps);
  }
...
void Liveness::computeLiveIns() {
  ...
  for (MachineBasicBlock &B : MF) {
    ...
    if (F1 == MDF.end())
      continue;
    SetVector<MachineBasicBlock *> IDFB(toolchain::from_range, F1->second);
  ...
  if (Trace) {
    ...
    for (MachineBasicBlock &B : MF) {
      ...
      toolchain::sort(LV, RegisterRefLess(PRI));
