...
public:
  ...
  bool initializeSaveRestorePoints(
      ...
      toolchain::SaveRestorePoints &SaveRestorePoints);
...
bool MIRParserImpl::parseMDNode(PerFunctionMIParsingState &PFS,
  ...
  if (Source.Value.empty())
    return false;
  SMDiagnostic Error;
  if (toolchain::parseMDNode(PFS, Node, Source.Value, Error))
...
bool MIRParserImpl::parseMBBReference(PerFunctionMIParsingState &PFS,
                                      ...
                                      const yaml::StringValue &Source) {
  SMDiagnostic Error;
  if (toolchain::parseMBBReference(PFS, MBB, Source.Value, Error))
    return error(Error, Source.SourceRange);
  return false;
}
...
bool MIRParserImpl::parseMachineMetadata(PerFunctionMIParsingState &PFS,
                                         const yaml::StringValue &Source) {
  SMDiagnostic Error;
  if (toolchain::parseMachineMetadata(PFS, Source.Value, Source.SourceRange, Error))
    return error(Error, Source.SourceRange);
  return false;
}
...
SMDiagnostic MIRParserImpl::diagFromBlockStringDiag(const SMDiagnostic &Error,
  ...
  // Translate the location of the error from the location in the toolchain IR string
...
std::unique_ptr<MIRParser> toolchain::createMIRParserFromFile(
...
toolchain::createMIRParser(std::unique_ptr<MemoryBuffer> Contents,
                      LLVMContext &Context,
                      std::function<void(Function &)> ProcessIRFunction) {
