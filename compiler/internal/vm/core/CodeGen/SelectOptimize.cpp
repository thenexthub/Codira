...
FunctionPass *toolchain::createSelectOptimizePass() { return new SelectOptimize(); }
...
PreservedAnalyses SelectOptimizeImpl::run(Function &F,
  ...
  if (toolchain::shouldOptimizeForSize(&F, PSI, BFI))
    return PreservedAnalyses::all();
...
void SelectOptimizeImpl::convertProfitableSIGroups(SelectGroups &ProfSIGroups) {
  for (SelectGroup &ASI : ProfSIGroups) {
    ...
    auto TransferDbgRecords = [&](Instruction &I) {
      for (auto &DbgRecord :
           toolchain::make_early_inc_range(I.getDbgRecordRange())) {
    ...
    toolchain::for_each(R, TransferDbgRecords);
