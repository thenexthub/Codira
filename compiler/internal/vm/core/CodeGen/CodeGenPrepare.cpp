...
#include "vm/core/Transforms/Utils/BypassSlowDivision.h"
#include "vm/core/Transforms/Utils/Local.h"
#include "vm/core/Transforms/Utils/SimplifyLibCalls.h"
...
FunctionPass *toolchain::createCodeGenPrepareLegacyPass() {
...
bool CodeGenPrepare::_run(Function &F) {
  ...
  if (!OptSize && !PSI->hasHugeWorkingSetSize() && TLI->isSlowDivBypassed()) {
    ...
    while (BB != nullptr) {
      ...
      if (!toolchain::shouldOptimizeForSize(BB, PSI, BFI.get()))
        EverMadeChange |= bypassSlowDivision(BB, BypassWidths);
      BB = Next;
    }
  }

  // Get rid of @toolchain.assume builtins before attempting to eliminate empty
  // blocks, since there might be blocks that only contain @toolchain.assume calls
  ...
  while (MadeChange) {
    ...
    for (BasicBlock &BB : toolchain::make_early_inc_range(F)) {
...
void CodeGenPrepare::removeAllAssertingVHReferences(Value *V) {
  ...
  toolchain::erase_if(GEPVector, [=](auto &Elt) { return Elt.first == GEP; });
...
bool CodeGenPrepare::eliminateFallThrough(Function &F, DominatorTree *DT) {
  ...
  SmallVector<WeakTrackingVH, 16> Blocks(
      toolchain::make_pointer_range(toolchain::drop_begin(F)));
...
bool CodeGenPrepare::eliminateMostlyEmptyBlocks(Function &F) {
  ...
  while (!LoopList.empty()) {
    ...
    toolchain::append_range(LoopList, *L);
  ...
  SmallVector<WeakTrackingVH, 16> Blocks;
  for (auto &Block : toolchain::drop_begin(F)) {
...
bool CodeGenPrepare::isMergingEmptyBlockProfitable(BasicBlock *BB,
  ...
  for (BasicBlock *Pred : predecessors(BB)) {
    if (isa<CallBrInst>(Pred->getTerminator()) &&
        toolchain::is_contained(successors(Pred), DestBB))
      return false;
  }
  ...
  for (BasicBlock *DestBBPred : predecessors(DestBB)) {
    ...
    if (toolchain::all_of(DestBB->phis(), [&](const PHINode &DestPN) {
...
simplifyRelocatesOffABase(GCRelocateInst *RelocatedBase,
  ...
  for (GCRelocateInst *ToReplace : Targets) {
    ...
    //  @toolchain.experimental.gc.relocate.p1i8(...) br label %merge
    //
    // bb2:
    //  ...
...
/// Try to combine the compare into a call to the toolchain.uadd.with.overflow
...
///     %z = call i64 @toolchain.cttz.i64(i64 %A, i1 false)
///
/// into:
///   entry:
...
///   cond.false:
///     %z = call i64 @toolchain.cttz.i64(i64 %A, i1 true)
///     br label %cond.end
///   cond.end:
...
bool CodeGenPrepare::optimizeCallInst(CallInst *CI, ModifyDT &ModifiedDT) {
  ...
  if (CI->hasFnAttr(Attribute::Cold) &&
      !toolchain::shouldOptimizeForSize(BB, PSI, BFI.get()))
    for (auto &Arg : CI->args()) {
      if (!Arg->getType()->isPointerTy())
        continue;
  ...
  if (II) {
    ...
    case Intrinsic::assume:
      llvm_unreachable("toolchain.assume should have been removed already");
...
bool CodeGenPrepare::dupRetToEnableTailCallOpts(BasicBlock *BB,
  ...
  if (V) {
    ...
    if (EVI) {
      V = EVI->getOperand(0);
      if (!toolchain::all_of(EVI->indices(), equal_to(0)))
        return false;
    }
  ...
  if (PN) {
    for (unsigned I = 0, E = PN->getNumIncomingValues(); I != E; ++I) {
      ...
      if (CI && CI->hasOneUse() && CI->getParent() == PredBB &&
        ...
        //
        // bb0:
        //   tail call void @toolchain.memset.p0.i64(ptr %0, i8 0, i64 %1)
        //   br label %return
        // return:
...
struct ExtAddrMode : public TargetLowering::AddrMode {
  ...
  FieldName compare(const ExtAddrMode &other) {
    ...
    if (toolchain::popcount(Result) > 1)
      return MultipleFields;
    else
  ...
  bool MatchPhiSet(SimplificationTracker &ST, bool AllowNewPhiNodes,
    ...
    while (PhiNodesToMatch.size()) {
      ...
      for (auto &P : PHI->getParent()->phis()) {
        ...
        // later.
        WillNotMatch.insert_range(toolchain::make_first_range(Matched));
        Matched.clear();
      }
      if (IsMatched) {
  ...
  for (Use &U : I->uses()) {
    ...
    if (CallInst *CI = dyn_cast<CallInst>(UserI)) {
      if (CI->hasFnAttr(Attribute::Cold)) {
        ...
        if (!toolchain::shouldOptimizeForSize(CI->getParent(), PSI, BFI))
          continue;
      }
...
bool CodeGenPrepare::splitLargeGEPOffsets() {
  ...
  for (auto &Entry : LargeOffsetGEPMap) {
    ...
    toolchain::sort(LargeOffsetGEPs, compareGEPOffset);
    LargeOffsetGEPs.erase(toolchain::unique(LargeOffsetGEPs), LargeOffsetGEPs.end());
...
bool CodeGenPrepare::optimizeSelectInst(SelectInst *SI) {
  ...
  if (TLI->isSelectSupported(SelectKind) &&
      (!isFormingBranchFromSelectProfitable(TTI, TLI, SI) ||
       toolchain::shouldOptimizeForSize(SI->getParent(), PSI, BFI.get())))
    return false;
  ...
  SmallPtrSet<const Instruction *, 2> INS(toolchain::from_range, ASI);
  // Use reverse iterator because later select may use the value of the
  // earlier select, and we need to propagate value through earlier select
  // to get the PHI operand.
  for (SelectInst *SI : toolchain::reverse(ASI)) {
...
static bool tryUnmergingGEPsAcrossIndirectBr(GetElementPtrInst *GEPI,
  ...
  // IndirectBr edge(s).
  if (toolchain::none_of(GEPI->users(), [&](User *Usr) {
  ...
  assert(toolchain::none_of(GEPIOp->users(),
                       [&](User *Usr) {
...
// A toolchain.dbg.value may be using a value before its definition, due to
...
bool CodeGenPrepare::placeDbgValues(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    for (Instruction &Insn : toolchain::make_early_inc_range(BB)) {
      // Process any DbgVariableRecord records attached to this
      // instruction.
      for (DbgVariableRecord &DVR : toolchain::make_early_inc_range(
