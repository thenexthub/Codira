...
static void replaceRegUsesAfterLoop(Register FromReg, Register ToReg,
  ...
  for (MachineOperand &O :
       toolchain::make_early_inc_range(MRI.use_operands(FromReg)))
...
void ModuloScheduleExpander::removeDeadInstructions(MachineBasicBlock *KernelBB,
  ...
  for (MachineBasicBlock *MBB : toolchain::reverse(EpilogBBs))
  ...
  for (MachineInstr &MI : toolchain::make_early_inc_range(KernelBB->phis())) {
...
void ModuloScheduleExpander::rewriteScheduledInstr(
  ...
  for (MachineOperand &UseOp :
       toolchain::make_early_inc_range(MRI.use_operands(OldReg))) {
    MachineInstr *UseMI = UseOp.getParent();
...
void EliminateDeadPhis(MachineBasicBlock *MBB, MachineRegisterInfo &MRI,
  ...
  while (Changed) {
    Changed = false;
    for (MachineInstr &MI : toolchain::make_early_inc_range(MBB->phis())) {
      assert(MI.isPHI());
...
void PeelingModuloScheduleExpander::moveStageBetweenBlocks(
  ...
  for (MachineInstr &MI : toolchain::make_early_inc_range(
           toolchain::make_range(SourceBB->getFirstNonPHI(), SourceBB->end()))) {
    if (MI.isPHI()) {
...
void PeelingModuloScheduleExpander::peelPrologAndEpilogs() {
  ...
  toolchain::append_range(Blocks, PeeledFront);
  Blocks.push_back(BB);
  toolchain::append_range(Blocks, PeeledBack);
