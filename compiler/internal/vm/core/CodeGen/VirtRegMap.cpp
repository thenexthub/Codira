//===- toolchain/CodeGen/VirtRegMap.cpp - Virtual Register Map -----------------===//
//
...
char &toolchain::VirtRegRewriterID = VirtRegRewriterLegacy::ID;
...
void VirtRegRewriter::expandCopyBundle(MachineInstr &MI) const {
  ...
  if (MI.isBundledWithPred() && !MI.isBundledWithSucc()) {
    ...
    for (MachineInstr *BundledMI : toolchain::reverse(MIs)) {
...
void VirtRegRewriter::rewrite() {
  ...
  for (MachineFunction::iterator MBBI = MF->begin(), MBBE = MF->end();
    ...
    for (MachineInstr &MI : toolchain::make_early_inc_range(MBBI->instrs())) {
...
FunctionPass *toolchain::createVirtRegRewriter(bool ClearVirtRegs) {
