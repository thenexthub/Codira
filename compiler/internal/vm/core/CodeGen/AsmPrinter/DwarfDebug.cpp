//===- toolchain/CodeGen/DwarfDebug.cpp - Dwarf Debug Framework ----------------===//
//
...
#include "vm/core/CodeGen/LexicalScopes.h"
...
bool toolchain::operator<(const FrameIndexExpr &LHS, const FrameIndexExpr &RHS) {
  return getFragmentOffsetInBits(*LHS.Expr) <
         getFragmentOffsetInBits(*RHS.Expr);
}

bool toolchain::operator<(const EntryValueInfo &LHS, const EntryValueInfo &RHS) {
...
void Loc::MMI::addFrameIndexExpr(const DIExpression *Expr, int FI) {
  ...
  assert((FrameIndexExprs.size() == 1 ||
          toolchain::all_of(FrameIndexExprs,
...
void DwarfDebug::addSubprogramNames(
  ...
  toolchain::StringRef LinkageName =
...
static const DIExpression *combineDIExpressions(const DIExpression *Original,
  ...
  if (Original->isImplicit() && Addition->isImplicit())
    toolchain::erase(Elts, dwarf::DW_OP_stack_value);
...
static void collectCallSiteParameters(const MachineInstr *CallMI,
  ...
  if (CallMI->hasDelaySlot()) {
    ...
    auto BundleEnd = toolchain::getBundleEnd(CallMI->getIterator());
    (void)BundleEnd;
...
sortGlobalExprs(SmallVectorImpl<DwarfCompileUnit::GlobalExpr> &GVEs) {
  ...
  GVEs.erase(toolchain::unique(GVEs,
...
void DwarfDebug::endModule() {
  ...
  // conditionalized on the presence of the toolchain.dbg.cu metadata node)
...
DwarfDebug::emitInitialLocDirective(const MachineFunction &MF, unsigned CUID) {
  ...
  if (toolchain::all_of(MF, [](const MachineBasicBlock &MBB) { return MBB.empty(); }))
    return nullptr;
  ...
  for (auto &MBB : *MF) {
    ...
    for (auto &MI : MBB) {
      ...
      if (CandidateRank == Rank)
        toolchain::remove_if(CandidateInsts, [&MI](const MachineInstr *Candidate) {
...
void DwarfDebug::emitDebugPubSection(bool GnuStyle, StringRef Name,
  ...
  toolchain::sort(Vec, [](auto &A, auto &B) {
...
void DebugLocEntry::finalize(const AsmPrinter &AP,
  ...
  if (Value.isFragment()) {
    ...
    assert(toolchain::all_of(Values, [](DbgValueLoc P) {
          return P.isFragment();
        }) && "all values are expected to be fragments");
    assert(toolchain::is_sorted(Values) && "fragments are expected to be sorted");
  ...
  for (const auto &P : SectionRanges) {
    ...
    for (const auto *RS : P.second) {
      ...
      if (Base) {
        ...
        if (DD.useSplitDwarf() && toolchain::isRangeRelaxable(Begin, End)) {
...
static void emitLocList(DwarfDebug &DD, AsmPrinter *Asm, const DebugLocStream::List &List) {
  emitRangeList(
      ...
      dwarf::DW_LLE_end_of_list, toolchain::dwarf::LocListEncodingString,
...
void DwarfDebug::emitDebugARanges() {
  ...
  toolchain::sort(CUs, [](const DwarfCompileUnit *A, const DwarfCompileUnit *B) {
...
static void emitRangeList(DwarfDebug &DD, AsmPrinter *Asm,
  ...
  emitRangeList(DD, Asm, List.Label, List.Ranges, *List.CU,
                ...
                dwarf::DW_RLE_end_of_list, toolchain::dwarf::RangeListEncodingString,
...
void DwarfDebug::emitDebugRangesImpl(const DwarfFile &Holder, MCSection *Section) {
  ...
  assert(!CUMap.empty());
  assert(toolchain::any_of(CUMap, [](const decltype(CUMap)::value_type &Pair) {
...
DwarfDebug::getMD5AsBytes(const DIFile *File) const {
  ...
  MD5::MD5Result CKMem;
  toolchain::copy(ChecksumString, CKMem.data());
  return CKMem;
}
