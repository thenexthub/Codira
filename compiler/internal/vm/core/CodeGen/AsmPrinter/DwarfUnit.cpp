//===-- toolchain/CodeGen/DwarfUnit.cpp - Dwarf Type and Compile Units ---------===//
//
...
toolchain::dwarf::SourceLanguage DwarfUnit::getSourceLanguage() const {
  const auto &Lang = getLanguage();

  if (!Lang.hasVersionedName())
    return static_cast<toolchain::dwarf::SourceLanguage>(Lang.getName());

  return toolchain::dwarf::toDW_LANG(
             static_cast<toolchain::dwarf::SourceLanguageName>(Lang.getName()),
             Lang.getVersion())
      .value_or(toolchain::dwarf::DW_LANG_hi_user);
}
...
std::string DwarfUnit::getParentContextString(const DIScope *Context) const {
  ...
  // innermost.
  for (const DIScope *Ctx : toolchain::reverse(Parents)) {
...
DIE *DwarfUnit::getOrCreateSubprogramDIE(const DISubprogram *SP,
  ...
  if (auto *SPDecl = SP->getDeclaration()) {
    if (!Minimal) {
      ...
      // See https://github.com/toolchain/toolchain-project/pull/154636.
      if (DIE *SPDie = getDIE(SP))
        return SPDie;
    }
  }
