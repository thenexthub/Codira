//===- toolchain/lib/CodeGen/AsmPrinter/CodeViewDebug.cpp ----------------------===//
//
...
#include "vm/core/DebugInfo/CodeView/CodeViewRecordIO.h"
...
StringRef CodeViewDebug::getFullFilepath(const DIFile *File) {
  ...
  if (Dir.starts_with("/") || Filename.starts_with("/")) {
    if (toolchain::sys::path::is_absolute(Filename, toolchain::sys::path::Style::posix))
      return Filename;
...
static void addLocIfNotPresent(SmallVectorImpl<const DILocation *> &Locs,
  ...
  if (!toolchain::is_contained(Locs, Loc))
    Locs.push_back(Loc);
}
...
void CodeViewDebug::beginModule(Module *M) {
  ...
  if (Asm->hasDebugInfo()) {
    ...
    NamedMDNode *CUs = MMI->getModule()->getNamedMetadata("toolchain.dbg.cu");
...
void CodeViewDebug::emitObjName() {
  ...
  toolchain::SmallString<256> PathStore(PathRef);
...
void CodeViewDebug::emitCompilerInformation() {
  ...
  using ArchType = toolchain::Triple::ArchType;
  ...
  for (const MachineFunction::VariableDbgInfo &VI :
    ...
    if (VI.Expr) {
      ...
      if (VI.Expr->getNumElements() == 1 &&
          VI.Expr->getElement(0) == toolchain::dwarf::DW_OP_deref)
        Deref = true;
...
void CodeViewDebug::calculateRanges(
  ...
  for (auto I = Entries.begin(), E = Entries.end(); I != E; ++I) {
    ...
    if (!Location)
    {
      ...
      // transformed it into a constant and we only have an toolchain.dbg.value. We
...
void CodeViewDebug::beginFunctionImpl(const MachineFunction *MF) {
  ...
  bool isThumb = MMI->getModule()->getTargetTriple().getArch() ==
                 toolchain::Triple::ArchType::thumb;
...
void CodeViewDebug::emitLocalVariableList(const FunctionInfo &FI,
  ...
  for (const LocalVariable &L : Locals)
    if (L.DIVar->isParameter())
      Params.push_back(&L);
  toolchain::sort(Params, [](const LocalVariable *L, const LocalVariable *R) {
...
void CodeViewDebug::emitConstantSymbolRecord(const DIType *DTy, APSInt &Value,
  ...
  uint8_t Data[10];
  BinaryStreamWriter Writer(Data, toolchain::endianness::little);
...
void forEachJumpTableBranch(
  ...
  if (JTI && !JTI->isEmpty()) {
#ifndef NDEBUG
    auto UsedJTs = toolchain::SmallBitVector(JTI->getJumpTables().size());
#endif
    for (const auto &MBB : *MF) {
