//===- toolchain/CodeGen/AsmPrinter/DbgEntityHistoryCalculator.cpp -------------===//
//
...
#include "vm/core/CodeGen/DbgEntityHistoryCalculator.h"
...
void DbgValueHistoryMap::trimLocationRanges(
  ...
  for (auto &Record : VarEntries) {
    ...
    for (EntryIndex Idx : toolchain::reverse(ToRemove))
      HistoryMapEntries.erase(HistoryMapEntries.begin() + Idx);
    LLVM_DEBUG(toolchain::dbgs() << "New HistoryMap('" << LocalVar->getName()
...
static void dropRegDescribedVar(RegDescribedVarsMap &RegVars, unsigned RegNo,
  ...
  const auto &VarPos = toolchain::find(VarSet, Var);
...
void toolchain::calculateDbgEntityHistory(const MachineFunction *MF,
