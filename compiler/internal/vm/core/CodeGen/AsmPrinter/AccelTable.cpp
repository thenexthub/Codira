//===- toolchain/CodeGen/AsmPrinter/AccelTable.cpp - Accelerator Tables --------===//
//
...
void AccelTableBase::computeBucketCount() {
  ...
  toolchain::sort(Uniques);
  UniqueHashCount = toolchain::unique(Uniques) - Uniques.begin();
...
void AccelTableBase::finalize(AsmPrinter *Asm, StringRef Prefix) {
  ...
  for (auto &E : Entries) {
    // Unique the entries.
    toolchain::stable_sort(E.second.Values,
                      [](const AccelTableData *A, const AccelTableData *B) {
                        return *A < *B;
                      });
    E.second.Values.erase(toolchain::unique(E.second.Values), E.second.Values.end());
  }
  ...
  for (auto &Bucket : Buckets)
    toolchain::stable_sort(Bucket, [](HashData *LHS, HashData *RHS) {
...
class Dwarf5AccelTableWriter : public AccelTableWriter {
  ...
  toolchain::function_ref<std::optional<DWARF5AccelTable::UnitIndexAndEncoding>(
...
void toolchain::emitAppleAccelTableImpl(AsmPrinter *Asm, AccelTableBase &Contents,
...
void toolchain::emitDWARF5AccelTable(
