...
#include "vm/core/CodeGen/MachineBlockHashInfo.h"
...
#include "vm/core/Object/ELFTypes.h"
...
void AddrLabelMap::UpdateForRAUWBlock(BasicBlock *Old, BasicBlock *New) {
  ...
  toolchain::append_range(NewEntry.Symbols, OldEntry.Symbols);
}
...
bool AsmPrinter::doInitialization(Module &M) {
  ...
  if (MAI->hasSingleParameterDotFile()) {
    ...
    if (MAI->isAIX()) {
      ...
      OutStreamer->emitFileDirective(
          toolchain::sys::path::filename(M.getSourceFileName()));
    }
  }

  // On AIX, emit bytes for toolchain.commandline metadata after .file so that the
  ...
  if (MAI->doesSupportDebugInformation()) {
    ...
    if ((Target.isOSWindows() || (Target.isUEFI() && EmitCodeView)) &&
        M.getNamedMetadata("toolchain.dbg.cu"))
...
static toolchain::object::BBAddrMap::Features
...
static ConstantInt *extractNumericCGTypeId(const Function &F) {
  ...
  for (const auto &Type : Types) {
    if (Type->hasGeneralizedMDString()) {
      ...
      uint64_t TypeIdVal = toolchain::MD5Hash(MDGeneralizedTypeId->getString());
...
/// Emits .toolchain.callgraph section.
...
void AsmPrinter::handleCallsiteForCallgraph(
  ...
  if (CalleeOperand.isGlobal() || CalleeOperand.isSymbol()) {
    ...
    case toolchain::MachineOperand::MO_GlobalAddress:
      CalleeSymbol = getSymbol(CalleeOperand.getGlobal());
      break;
    case toolchain::MachineOperand::MO_ExternalSymbol:
...
static uint64_t globalSize(const toolchain::GlobalVariable &G) {
  const Constant *Initializer = G.getInitializer();
  return G.getParent()->getDataLayout().getTypeAllocSize(
      Initializer->getType());
}

static bool shouldTagGlobal(const toolchain::GlobalVariable &G) {
  ...
  // gadgets.
  if (G.getName().starts_with("toolchain.") || G.isThreadLocal() || G.isConstant())
    return false;
...
static void tagGlobalDefinition(Module &M, GlobalVariable *G) {
  ...
  if (SizeInBytes != NewSize) {
    ...
    toolchain::SmallVector<uint8_t> Init(NewSize - SizeInBytes, 0);
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  for (const auto &Alias : M.aliases()) {
    ...
    for (const GlobalAlias *AncestorAlias : toolchain::reverse(AliasStack))
  ...
  // Emit toolchain.ident metadata in an '.ident' directive.
  emitModuleIdents(M);

  // Emit bytes for toolchain.commandline metadata.
  ...
  Function *InitTrampolineIntrinsic = M.getFunction("toolchain.init.trampoline");
  ...
  if (TM.Options.EmitAddrsig) {
    ...
    for (const GlobalValue &GV : M.global_values()) {
      if (!GV.use_empty() && !GV.isThreadLocal() &&
          ...
          !GV.getName().starts_with("toolchain.") &&
...
void AsmPrinter::emitJumpTableInfo() {
  ...
  if (!TM.Options.EnableStaticDataPartitioning) {
    emitJumpTableImpl(*MJTI, toolchain::to_vector(toolchain::seq<unsigned>(JT.size())));
    return;
  }
...
void AsmPrinter::emitJumpTableImpl(const MachineJumpTableInfo &MJTI,
  ...
  for (const unsigned JumpTableIndex : JumpTableIndices) {
    ...
    if (MJTI.getEntryKind() == MachineJumpTableInfo::EK_LabelDifference32 &&
      ...
      const MCExpr *Base =
          TLI->getPICODEumpTableRelocBaseExpr(MF, JumpTableIndex, OutContext);
...
void AsmPrinter::emitJumpTableEntry(const MachineJumpTableInfo &MJTI,
  ...
  case MachineJumpTableInfo::EK_LabelDifference64: {
    ...
    Value = MCSymbolRefExpr::create(MBB->getSymbol(), OutContext);
    const TargetLowering *TLI = MF->getSubtarget().getTargetLowering();
    const MCExpr *Base = TLI->getPICODEumpTableRelocBaseExpr(MF, UID, OutContext);
...
bool AsmPrinter::emitSpecialLLVMGlobal(const GlobalVariable *GV) {
  if (GV->getName() == "toolchain.used") {
    if (MAI->hasNoDeadStrip())    // No need to emit this at all.
      emitLLVMUsedList(cast<ConstantArray>(GV->getInitializer()));
    return true;
  }

  // Ignore debug and non-emitted data.  This handles toolchain.compiler.used.
  if (GV->getSection() == "toolchain.metadata" ||
      GV->hasAvailableExternallyLinkage())
    return true;

  if (GV->getName() == "toolchain.arm64ec.symbolmap") {
  ...
  if (GV->getName() == "toolchain.global_dtors") {
    emitXXStructorList(GV->getDataLayout(), GV->getInitializer(),
                       /* isCtor */ false);
...
/// global in the specified toolchain.used list.
...
void AsmPrinter::preprocessXXStructorList(const DataLayout &DL,
  ...
  toolchain::stable_sort(Structors, [](const Structor &L, const Structor &R) {
...
void AsmPrinter::emitModuleIdents(Module &M) {
  ...
  if (const NamedMDNode *NMD = M.getNamedMetadata("toolchain.ident")) {
    for (const MDNode *N : NMD->operands()) {
      assert(N->getNumOperands() == 1 &&
             "toolchain.ident metadata entry can have only one operand");
...
void AsmPrinter::emitModuleCommandLines(Module &M) {
  ...
  const NamedMDNode *NMD = M.getNamedMetadata("toolchain.commandline");
  ...
  for (const MDNode *N : NMD->operands()) {
    assert(N->getNumOperands() == 1 &&
           "toolchain.commandline metadata entry can have only one operand");
    const MDString *S = cast<MDString>(N->getOperand(0));
...
static void emitGlobalConstantImpl(const DataLayout &DL, const Constant *CV,
  ...
  if (isa<ConstantAggregateZero>(CV)) {
    ...
    if (AliasList && (structType = toolchain::dyn_cast<StructType>(CV->getType()))) {
...
AsmPrinter::getCodeViewJumpTableInfo(int JTI, const MachineInstr *BranchInstr,
  ...
  const auto BaseExpr =
      TLI->getPICODEumpTableRelocBaseExpr(MF, JTI, MMI->getContext());
