...
bool GlobalMergeImpl::doMerge(SmallVectorImpl<GlobalVariable *> &Globals,
  ...
  // metric.
  toolchain::stable_sort(UsedGlobalSets,
  ...
  for (const UsedGlobalSet &UGS : toolchain::reverse(UsedGlobalSets)) {
    if (UGS.UsageCount == 0)
      continue;
...
void GlobalMergeImpl::collectUsedGlobalVariables(Module &M, StringRef Name) {
  // Extract global variables from toolchain.used array
...
void GlobalMergeImpl::setMustKeepGlobalVariables(Module &M) {
  collectUsedGlobalVariables(M, "toolchain.used");
  collectUsedGlobalVariables(M, "toolchain.compiler.used");
...
bool GlobalMergeImpl::run(Module &M) {
  ...
  for (auto &GV : M.globals()) {
    ...
    if (GV.getName().starts_with("toolchain.") ||
        GV.getName().starts_with(".toolchain.") || Section == "toolchain.metadata")
      continue;
...
Pass *toolchain::createGlobalMergePass(const TargetMachine *TM, unsigned Offset,
