...
char &toolchain::RAGreedyLegacyID = RAGreedyLegacy::ID;
...
FunctionPass* toolchain::createGreedyRegisterAllocator() {
  return new RAGreedyLegacy();
}

FunctionPass *toolchain::createGreedyRegisterAllocator(RegAllocFilterFunc Ftor) {
...
MCRegister RAGreedy::tryLocalSplit(const LiveInterval &VirtReg,
  ...
  if (Matrix->checkRegMaskInterference(VirtReg)) {
    ...
    unsigned RI =
        toolchain::lower_bound(RMS, Uses.front().getRegSlot()) - RMS.begin();
...
MCRegister RAGreedy::tryLastChanceRecoloring(
  ...
  for (MCRegister PhysReg : Order) {
    ...
    if (tryRecoloringCandidates(RecoloringQueue, CurrentNewVRegs,
      ...
      toolchain::append_range(NewVRegs, CurrentNewVRegs);
...
RAGreedy::RAGreedyStats RAGreedy::computeStats(MachineBasicBlock &MBB) {
  ...
  for (MachineInstr &MI : MBB) {
    ...
    if (TII->hasLoadFromStackSlot(MI, Accesses) &&
        toolchain::any_of(Accesses, isSpillSlotAccess)) {
