...
static void VerifyPHIs(MachineFunction &MF, bool CheckExtra) {
  for (MachineBasicBlock &MBB : toolchain::drop_begin(MF)) {
  ...
  if (!SSAUpdateVRs.empty()) {
    for (Register VReg : SSAUpdateVRs) {
      ...
      for (MachineOperand &UseMO :
           toolchain::make_early_inc_range(MRI->use_operands(VReg))) {
...
bool TailDuplicator::tailDuplicateBlocks() {
  ...
  for (MachineBasicBlock &MBB :
       toolchain::make_early_inc_range(toolchain::drop_begin(*MF))) {
...
bool TailDuplicator::shouldTailDuplicate(bool IsSimple,
  ...
  if (toolchain::shouldOptimizeForSize(&TailBB, PSI, MBFI))
    MaxDuplicateCount = 1;
  ...
  // we have to address https://github.com/toolchain/toolchain-project/issues/78578.
...
bool TailDuplicator::duplicateSimpleBB(
    ...
    const DenseSet<Register> &UsedByPhi) {
  SmallPtrSet<MachineBasicBlock *, 8> Succs(toolchain::from_range,
                                            TailBB->successors());
...
bool TailDuplicator::tailDuplicate(bool IsSimple, MachineBasicBlock *TailBB,
  ...
  for (MachineBasicBlock *PredBB : Preds) {
    ...
    for (MachineInstr &MI : toolchain::make_early_inc_range(*TailBB)) {
      if (MI.isPHI()) {
