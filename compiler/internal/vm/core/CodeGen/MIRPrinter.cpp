...
convertSRPoints(ModuleSlotTracker &MST,
                ...
                const toolchain::SaveRestorePoints &SRPoints,
                const TargetRegisterInfo *TRI);
...
static void convertCallSiteObjects(yaml::MachineFunction &YMF,
  ...
  toolchain::sort(YMF.CallSitesInfo.begin(), YMF.CallSitesInfo.end(),
...
static void convertCalledGlobals(yaml::MachineFunction &YMF,
  ...
  toolchain::sort(YMF.CalledGlobals.begin(), YMF.CalledGlobals.end(),
...
void toolchain::guessSuccessors(const MachineBasicBlock &MBB,
...
static void printMI(raw_ostream &OS, MFPrintState &State,
  ...
  // toolchain/utils/update_mir_test_checks.py.
...
void toolchain::printMIR(raw_ostream &OS, const Module &M) {
  yaml::Output Out(OS);
  Out << const_cast<Module &>(M);
}

void toolchain::printMIR(raw_ostream &OS, const MachineModuleInfo &MMI,
                    const MachineFunction &MF) {
  printMF(OS, [&](const Function &F) { return MMI.getMachineFunction(F); }, MF);
}

void toolchain::printMIR(raw_ostream &OS, FunctionAnalysisManager &FAM,
                    const MachineFunction &MF) {
  printMF(
      OS,
      [&](const Function &F) {
