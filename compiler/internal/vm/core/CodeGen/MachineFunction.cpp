...
template <>
struct toolchain::DOTGraphTraits<const MachineFunction *>
...
try_next:;
  ...
  toolchain::append_range(FilterIds, TyIds);
...
bool MachineFunction::shouldUseDebugInstrRef() const {
  ...
  if (toolchain::debuginfoShouldUseDebugInstrRef(getTarget().getTargetTriple()))
    return true;
...
Printable toolchain::printJumpTableEntryReference(unsigned Idx) {
...
ProfileSummaryInfo::getEntryCount<toolchain::MachineFunction>(
    const toolchain::MachineFunction *F) const {
