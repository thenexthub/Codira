...
#include "vm/core/CodeGen/RegAllocPBQP.h"
...
#include "vm/core/CodeGen/PBQP/Graph.h"
#include "vm/core/CodeGen/PBQP/Math.h"
#include "vm/core/CodeGen/PBQP/Solution.h"
...
void RegAllocPBQP::initializeGraph(PBQPRAGraph &G, VirtRegMap &VRM,
  ...
  while (!Worklist.empty()) {
    ...
    if (VRegAllowed.empty()) {
      ...
      toolchain::append_range(Worklist, NewVRegs);
      continue;
    }
...
FunctionPass *toolchain::createPBQPRegisterAllocator(char *customPassID) {
  return new RegAllocPBQP(customPassID);
}

FunctionPass* toolchain::createDefaultPBQPRegisterAllocator() {
