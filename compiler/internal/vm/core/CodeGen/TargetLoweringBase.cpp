...
TargetLoweringBase::TargetLoweringBase(const TargetMachine &tm,
  ...
  MaxLargeFPConvertBitWidthSupported = toolchain::IntegerType::MAX_INT_BITS;
...
void TargetLoweringBase::initActions() {
  ...
  toolchain::fill(RegClassForVT, nullptr);
  toolchain::fill(TargetDAGCombineArray, 0);
...
#include "vm/core/IR/ConstrainedOps.def"
    ...
    // For most targets @toolchain.get.dynamic.area.offset just returns 0.
...
#include "vm/core/IR/VPIntrinsics.def"
  ...
  // Most targets ignore the @toolchain.prefetch intrinsic.
  setOperationAction(ISD::PREFETCH, MVT::Other, Expand);

  // Most targets also ignore the @toolchain.readcyclecounter intrinsic.
  setOperationAction(ISD::READCYCLECOUNTER, MVT::i64, Expand);

  // Most targets also ignore the @toolchain.readsteadycounter intrinsic.
  ...
  // Insert custom handling default for toolchain.canonicalize.*.
...
unsigned TargetLoweringBase::getBitWidthForCttzElements(
  ...
  EltWidth = std::max(toolchain::bit_ceil(EltWidth), (unsigned)8);
...
static unsigned getVectorTypeBreakdownMVT(MVT VT, MVT &IntermediateVT,
  ...
  LaneSizeInBits = toolchain::bit_ceil(LaneSizeInBits);
...
TargetLoweringBase::emitPatchPoint(MachineInstr &InitialMI,
  ...
  if (toolchain::none_of(MI->operands(),
...
unsigned TargetLoweringBase::getVectorTypeBreakdown(LLVMContext &Context,
  ...
  if (EVT(DestVT).bitsLT(NewVT)) {  // Value is expanded, e.g. i64 -> i16.
    ...
    // Convert sizes such as i33 to i64.
    if (!toolchain::has_single_bit<uint32_t>(NewVTSize.getKnownMinValue()))
...
bool TargetLoweringBase::isSuitableForJumpTable(const SwitchInst *SI,
  ...
  const bool OptForSize =
      toolchain::shouldOptimizeForSize(SI->getParent(), PSI, BFI);
...
void toolchain::GetReturnInfo(CallingConv::ID CC, Type *ReturnType,
                         AttributeList attr,
