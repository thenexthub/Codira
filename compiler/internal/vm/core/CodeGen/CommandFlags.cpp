...
toolchain::BasicBlockSection
codegen::getBBSectionsMode(toolchain::TargetOptions &Options) {
...
int codegen::MaybeSaveStatistics(StringRef OutputFilename, StringRef ToolName) {
  ...
  if (SaveStatsValue == codegen::SaveStatsMode::Obj) {
    ...
    toolchain::sys::path::remove_filename(StatsFilename);
  } else {
    assert(SaveStatsValue == codegen::SaveStatsMode::Cwd &&
           "Should have been a valid --save-stats value");
  }

  auto BaseName = toolchain::sys::path::filename(OutputFilename);
  toolchain::sys::path::append(StatsFilename, BaseName);
  toolchain::sys::path::replace_extension(StatsFilename, "stats");

  auto FileFlags = toolchain::sys::fs::OF_TextWithCRLF;
  std::error_code EC;
  auto StatsOS =
      std::make_unique<toolchain::raw_fd_ostream>(StatsFilename, EC, FileFlags);
  if (EC) {
  ...
  toolchain::PrintStatisticsJSON(*StatsOS);
  return 0;
}
