//===- toolchain/CodeGen/TargetLoweringObjectFileImpl.cpp - Object File Info ---===//
//
...
#include "vm/core/MC/MCGOFFAttributes.h"
#include "vm/core/MC/MCSectionCOFF.h"
#include "vm/core/MC/MCSectionELF.h"
#include "vm/core/MC/MCSectionGOFF.h"
...
void TargetLoweringObjectFileELF::emitModuleMetadata(MCStreamer &Streamer,
  ...
  if (NamedMDNode *DependentLibraries = M.getNamedMetadata("toolchain.dependent-libraries")) {
  ...
  if (NamedMDNode *LLVMStats = M.getNamedMetadata("toolchain.stats")) {
    // Emit the metadata for toolchain statistics into .llvm_stats section, which is
...
void TargetLoweringObjectFileELF::emitLinkerDirectives(MCStreamer &Streamer,
                                                       Module &M) const {
  auto &C = getContext();
  if (NamedMDNode *LinkerOptions = M.getNamedMetadata("toolchain.linker.options")) {
    ...
    for (const auto *Operand : LinkerOptions->operands()) {
      if (cast<MDNode>(Operand)->getNumOperands() != 2)
        report_fatal_error("invalid toolchain.linker.options");
...
static unsigned getELFSectionType(StringRef Name, SectionKind K) {
  ...
  if (hasPrefix(Name, ".toolchain.offloading"))
    return ELF::SHT_LLVM_OFFLOADING;
  if (Name == ".toolchain.lto")
...
void TargetLoweringObjectFileMachO::emitLinkerDirectives(MCStreamer &Streamer,
                                                         Module &M) const {
  if (auto *LinkerOptions = M.getNamedMetadata("toolchain.linker.options")) {
...
void TargetLoweringObjectFileCOFF::emitLinkerDirectives(
  ...
  if (const auto *LU = M.getNamedGlobal("toolchain.used")) {
    assert(LU->hasInitializer() && "expected toolchain.used to have an initializer");
    assert(isa<ArrayType>(LU->getValueType()) &&
           "expected toolchain.used to be an array type");
...
static std::string APIntToHexString(const APInt &AI) {
  ...
  toolchain::transform(HexString, HexString.begin(), tolower);
...
MCSection *TargetLoweringObjectFileWasm::getStaticDtorSection(
    unsigned Priority, const MCSymbol *KeySym) const {
  report_fatal_error("@toolchain.global_dtors should have been lowered already");
}
