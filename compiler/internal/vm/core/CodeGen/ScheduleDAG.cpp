...
void SUnit::removePred(const SDep &D) {
  ...
  SmallVectorImpl<SDep>::iterator I = toolchain::find(Preds, D);
  if (I == Preds.end())
    return;
  // Find the corresponding successor in N.
  SDep P = D;
  P.setSUnit(this);
  SUnit *N = D.getSUnit();
  SmallVectorImpl<SDep>::iterator Succ = toolchain::find(N->Succs, P);
...
void ScheduleDAGTopologicalSort::DFS(const SUnit *SU, int UpperBound,
  ...
  do {
    ...
    for (const SDep &SuccDep : toolchain::reverse(SU->Succs)) {
...
std::vector<int> ScheduleDAGTopologicalSort::GetSubGraph(const SUnit &StartSU,
  ...
  do {
    ...
    WorkList.pop_back();
    for (const SDep &SD : toolchain::reverse(SU->Succs)) {
  ...
  do {
    const SUnit *SU = WorkList.back();
    WorkList.pop_back();
    for (const SDep &SD : toolchain::reverse(SU->Preds)) {
