...
#include "vm/core/ADT/AllocatorList.h"
...
template <> struct toolchain::DenseMapInfo<ComplexValue> {
...
std::optional<T> findCommonBetweenCollections(IterT A, IterT B) {
  auto Common = toolchain::find_if(A, [B](T I) { return toolchain::is_contained(B, I); });
  if (Common != A.end())
...
private:
  ...
  ///
  /// middle.block:
  ///   %OutsideUser = toolchain.vector.reduce.fadd(..., %ReductionOp)
  ///
  /// %OutsideUser can be `toolchain.vector.reduce.fadd` or `fadd` preceding
  /// `toolchain.vector.reduce.fadd` when unroll factor isn't one.
...
PreservedAnalyses ComplexDeinterleavingPass::run(Function &F,
  ...
  auto &TLI = AM.getResult<toolchain::TargetLibraryAnalysis>(F);
...
FunctionPass *toolchain::createComplexDeinterleavingPass(const TargetMachine *TM) {
  ...
  for (auto &PMI : Info) {
    ...
    if (IsMultiplicandReal) {
      ...
      if (RealMul.IsPositive && ImagMul.IsPositive)
        Rotation = toolchain::ComplexDeinterleavingRotation::Rotation_0;
      else if (!RealMul.IsPositive && !ImagMul.IsPositive)
        Rotation = toolchain::ComplexDeinterleavingRotation::Rotation_180;
      else
        continue;
      ...
      if (!RealMul.IsPositive && ImagMul.IsPositive)
        Rotation = toolchain::ComplexDeinterleavingRotation::Rotation_90;
      else if (RealMul.IsPositive && !ImagMul.IsPositive)
        Rotation = toolchain::ComplexDeinterleavingRotation::Rotation_270;
      else
        continue;
    }
  ...
  while (!RealAddends.empty()) {
    ...
    for (auto ItI = ImagAddends.begin(); ItI != ImagAddends.end(); ++ItI) {
      ...
      if (AddNode) {
        ...
        if (Rotation == toolchain::ComplexDeinterleavingRotation::Rotation_0) {
          ...
          if (Flags) {
            ...
            TmpNode->Opcode = Instruction::Add;
                   ...
                   toolchain::ComplexDeinterleavingRotation::Rotation_180) {
          TmpNode = prepareCompositeNode(
              ComplexDeinterleavingOperation::Symmetric, nullptr, nullptr);
          if (Flags) {
...
ComplexDeinterleavingGraph::identifyDeinterleave(ComplexValues &Vals) {
  ...
  if (auto *IntrinsicII = dyn_cast_or_null<IntrinsicInst>(II)) {
    ...
    CompositeNode *PlaceholderNode = prepareCompositeNode(
        toolchain::ComplexDeinterleavingOperation::Deinterleave, Vals);
  ...
  CompositeNode *PlaceholderNode =
      prepareCompositeNode(toolchain::ComplexDeinterleavingOperation::Deinterleave,
