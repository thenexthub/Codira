...
char &toolchain::MachineLICMID = MachineLICM::ID;
char &toolchain::EarlyMachineLICMID = EarlyMachineLICM::ID;
...
void MachineLICMImpl::InitializeLoadsHoistableLoops() {
  ...
  while (!Worklist.empty()) {
    ...
    toolchain::append_range(Worklist, L->getSubLoops());
  }
...
template class toolchain::MachineLICMBasePass<EarlyMachineLICMPass, true>;
template class toolchain::MachineLICMBasePass<MachineLICMPass, false>;
