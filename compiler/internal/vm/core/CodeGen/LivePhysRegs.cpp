...
void toolchain::computeLiveIns(LivePhysRegs &LiveRegs,
  ...
  for (const MachineInstr &MI : toolchain::reverse(MBB))
    LiveRegs.stepBackward(MI);
}

void toolchain::addLiveIns(MachineBasicBlock &MBB, const LivePhysRegs &LiveRegs) {
...
void toolchain::recomputeLivenessFlags(MachineBasicBlock &MBB) {
  const MachineFunction &MF = *MBB.getParent();
  const MachineRegisterInfo &MRI = MF.getRegInfo();
  const TargetRegisterInfo &TRI = *MRI.getTargetRegisterInfo();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
void toolchain::computeAndAddLiveIns(LivePhysRegs &LiveRegs,
...
bool toolchain::isPhysRegUsedAfter(Register Reg, MachineBasicBlock::iterator MBI) {
  ...
  for (const MachineInstr &MI : toolchain::make_range(std::next(MBI), MBB->end())) {
