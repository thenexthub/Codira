...
FunctionPass *toolchain::createWinEHPass(bool DemoteCatchSwitchPHIOnly) {
...
void toolchain::calculateCXXStateForAsynchEH(const BasicBlock *BB, int State,
...
void toolchain::calculateSEHStateForAsynchEH(const BasicBlock *BB, int State,
                                        WinEHFuncInfo &EHInfo) {
  SmallVector<struct WorkItem *, 8> WorkList;
  struct WorkItem *WI = new WorkItem(BB, State);
  WorkList.push_back(WI);
...
void toolchain::calculateSEHStateNumbers(const Function *Fn,
                                    WinEHFuncInfo &FuncInfo) {
...
void toolchain::calculateWinCXXEHStateNumbers(const Function *Fn,
                                         WinEHFuncInfo &FuncInfo) {
...
void toolchain::calculateClrEHStateNumbers(const Function *Fn,
                                      WinEHFuncInfo &FuncInfo) {
  // Return if it's already been done.
  if (!FuncInfo.EHPadStateMap.empty())
    return;
  ...
  while (!Worklist.empty()) {
    ...
    if (const auto *Cleanup = dyn_cast<CleanupPadInst>(Pad)) {
      ...
      for (const BasicBlock *CatchBlock : toolchain::reverse(CatchBlocks)) {
  ...
  for (ClrEHUnwindMapEntry &Entry : toolchain::reverse(FuncInfo.ClrEHUnwindMap)) {
    const Instruction *Pad =
...
bool WinEHPrepareImpl::cloneCommonBlocks(Function &F) {
  ...
  for (auto &Funclets : FuncletBlocks) {
    ...
    for (auto &BBMapping : Orig2Clone) {
      ...
      toolchain::erase(BlocksInFunclet, OldBlock);
      ColorVector &OldColors = BlockColors[OldBlock];
      toolchain::erase(OldColors, FuncletPadBB);
            ...
            if (auto *CRI =
              ...
              assert(
                  (IncomingColors.size() == 1 ||
                   !toolchain::is_contained(IncomingColors, FuncletPadBB)) &&
...
AllocaInst *WinEHPrepareImpl::insertPHILoads(PHINode *PN, Function &F) {
  ...
  for (Use &U : toolchain::make_early_inc_range(PN->uses())) {
