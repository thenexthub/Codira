...
char &toolchain::RegisterCoalescerID = RegisterCoalescerLegacy::ID;
...
bool RegisterCoalescer::hasOtherReachingDefs(LiveInterval &IntA,
  ...
  for (LiveRange::Segment &ASeg : IntA.segments) {
    if (ASeg.valno != AValNo)
      continue;
    LiveInterval::iterator BI = toolchain::upper_bound(IntB, ASeg.start);
    if (BI != IntB.begin())
      --BI;
...
RegisterCoalescer::removeCopyByCommutingDef(const CoalescerPair &CP,
  ...
  for (MachineOperand &UseMO :
       toolchain::make_early_inc_range(MRI->use_operands(IntA.reg()))) {
    if (UseMO.isUndef())
      continue;
...
bool RegisterCoalescer::reMaterializeDef(const CoalescerPair &CP,
  ...
  if (MRI->use_nodbg_empty(SrcReg)) {
    for (MachineOperand &UseMO :
         toolchain::make_early_inc_range(MRI->use_operands(SrcReg))) {
      MachineInstr *UseMI = UseMO.getParent();
      if (UseMI->isDebugInstr()) {
        if (DstReg.isPhysical())
...
bool RegisterCoalescer::joinVirtRegs(CoalescerPair &CP) {
  ...
  if (RegIt != RegToPHIIdx.end()) {
    ...
    if (RegIt != RegToPHIIdx.end())
      toolchain::append_range(RegIt->second, InstrNums);
    else
...
bool RegisterCoalescer::run(MachineFunction &fn) {
  ...
  InflateRegs.erase(toolchain::unique(InflateRegs), InflateRegs.end());
