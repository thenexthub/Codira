...
#include "vm/core/Transforms/Utils/CodeLayout.h"
#include <algorithm>
#include <cassert>
#include <cstdint>
#include <iterator>
...
char &toolchain::MachineBlockPlacementID = MachineBlockPlacementLegacy::ID;
...
bool MachineBlockPlacement::isTrellis(
  ...
  SmallPtrSet<const MachineBasicBlock *, 2> Successors(toolchain::from_range,
                                                       BB->successors());
...
MachineBlockPlacement::getBestNonConflictingEdges(
  ...
  toolchain::stable_sort(Edges[0], Cmp);
  toolchain::stable_sort(Edges[1], Cmp);
...
MachineBlockPlacement::getBestTrellisSuccessor(
  ...
  SmallPtrSet<const MachineBasicBlock *, 4> Successors(toolchain::from_range,
                                                       BB->successors());
...
MachineBlockPlacement::selectBestSuccessor(const MachineBasicBlock *BB,
  ...
  toolchain::stable_sort(DupCandidates,
...
MachineBasicBlock *MachineBlockPlacement::selectBestCandidateBlock(
  ...
  toolchain::erase_if(WorkList, [&](MachineBasicBlock *BB) {
...
MachineBlockPlacement::findBestLoopTop(const MachineLoop &L,
  ...
  if (toolchain::shouldOptimizeForSize(L.getHeader(), PSI, MBFI.get()))
    return L.getHeader();
...
void MachineBlockPlacement::rotateLoop(BlockChain &LoopChain,
  ...
  BlockChain::iterator ExitIt = toolchain::find(LoopChain, ExitingBB);
...
void MachineBlockPlacement::optimizeBranches() {
  ...
  for (MachineBasicBlock *ChainBB : FunctionChain) {
    ...
    if (toolchain::shouldOptimizeForSize(ChainBB, PSI, MBFI.get()))
      continue;
  ...
  for (MachineBasicBlock *ChainBB : FunctionChain) {
    ...
    if (LoopID) {
      for (const MDOperand &MDO : toolchain::drop_begin(LoopID->operands())) {
        ...
        if (S == nullptr)
          continue;
        if (S->getString() == "toolchain.loop.align") {
...
bool MachineBlockPlacement::maybeTailDuplicateBlock(
  ...
  auto RemovalCallback = [&](MachineBasicBlock *RemBB) {
    ...
    if (RemBB->isEHPad()) {
      toolchain::erase(EHPadWorkList, RemBB);
    } else {
      toolchain::erase(BlockWorkList, RemBB);
    }

    // Handle the filter set
    if (BlockFilter) {
      auto It = toolchain::find(*BlockFilter, RemBB);
...
void MachineBlockPlacement::findDuplicateCandidates(
  ...
  toolchain::stable_sort(Succs, CmpSucc);
  toolchain::stable_sort(Preds, CmpPred);
...
bool MachineBlockPlacement::run(MachineFunction &MF) {
  ...
  const bool OptForSize =
      toolchain::shouldOptimizeForSize(&MF, PSI, &MBFI->getMBFI());
...
char &toolchain::MachineBlockPlacementStatsID = MachineBlockPlacementStatsLegacy::ID;
