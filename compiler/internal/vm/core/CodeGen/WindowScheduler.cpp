...
//
//===----------------------------------------------------------------------===//
#include "vm/core/CodeGen/WindowScheduler.h"
...
void WindowScheduler::restoreMBB() {
  ...
  toolchain::append_range(*MBB, OriMIs);
  updateLiveIntervals();
}
...
void WindowScheduler::expand() {
  ...
  toolchain::stable_sort(SchedResult,
...
unsigned WindowScheduler::getOriStage(MachineInstr *OriMI, unsigned Offset) {
  assert(toolchain::is_contained(OriMIs, OriMI) && "Cannot find OriMI in OriMIs!");
