...
char &toolchain::LiveIntervalsID = LiveIntervalsWrapperPass::ID;
...
cl::opt<bool> toolchain::UseSegmentSetForPhysRegs(
...
float LiveIntervals::getSpillWeight(bool isDef, bool isUse,
  ...
  if (PSI && toolchain::shouldOptimizeForSize(MF, PSI, MBFI))
    return Weight;
...
bool LiveIntervals::checkRegMaskInterference(const LiveInterval &LI,
  ...
  ArrayRef<SlotIndex>::iterator SlotI = toolchain::lower_bound(Slots, LiveI->start);
...
private:
  ...
  void updateRegMaskSlots() {
    SmallVectorImpl<SlotIndex>::iterator RI =
        toolchain::lower_bound(LIS.RegMaskSlots, OldIdx);
      ...
      if (MO.isReg() && MO.getReg().isVirtual()) {
        ...
        if (MO.getSubReg() && hasInterval(Reg) &&
          ...
          if (!LI.hasSubRanges()) {
            ...
            unsigned SubReg = MO.getSubReg();
            LaneBitmask Mask = TRI->getSubRegIndexLaneMask(SubReg);
            if (toolchain::none_of(LI.subranges(),
        ...
        if (!hasInterval(Reg)) {
          ...
          toolchain::erase(RegsToRepair, Reg);
        }
      }
    }
  }
