...
char &toolchain::IfConverterID = IfConverter::ID;
...
bool IfConverter::ValidForkedDiamond(
  ...
  toolchain::scope_exit UnReverseOnExit([&]() {
    if (FalseReversed)
      reverseBranchCondition(FalseBBI);
  });
...
void IfConverter::AnalyzeBlocks(
  ...
  toolchain::stable_sort(Tokens, IfcvtTokenCmp);
}
...
FunctionPass *
toolchain::createIfConverter(std::function<bool(const MachineFunction &)> Ftor) {
