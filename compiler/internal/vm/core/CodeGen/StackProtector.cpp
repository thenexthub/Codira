...
FunctionPass *toolchain::createStackProtectorPass() { return new StackProtector(); }
...
/// Search for the first call to the toolchain.stackprotector intrinsic and return it
/// if present.
...
///     call void @toolchain.stackprotector(StackGuard, StackGuardSlot)
///
...
bool InsertStackProtectors(const TargetMachine *TM, Function *F,
  ...
  for (BasicBlock &BB : toolchain::make_early_inc_range(*F)) {
    ...
    if (!AI) {
      ...
      assert(SPCall && "Call to toolchain.stackprotector is missing");
