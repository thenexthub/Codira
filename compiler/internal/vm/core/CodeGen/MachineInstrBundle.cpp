...
char &toolchain::UnpackMachineBundlesID = UnpackMachineBundles::ID;
...
FunctionPass *
toolchain::createUnpackMachineBundles(
...
/// instruction.
void toolchain::finalizeBundle(MachineBasicBlock &MBB,
  ...
  for (auto [DefReg, UseReg] : TiedOperands) {
    unsigned DefIdx =
        std::distance(LocalDefs.begin(), toolchain::find(LocalDefs, DefReg));
    unsigned UseIdx =
        std::distance(ExternUses.begin(), toolchain::find(ExternUses, UseReg));
...
toolchain::finalizeBundle(MachineBasicBlock &MBB,
                     MachineBasicBlock::instr_iterator FirstMI) {
...
bool toolchain::finalizeBundles(MachineFunction &MF) {
  bool Changed = false;
  for (MachineBasicBlock &MBB : MF) {
...
VirtRegInfo toolchain::AnalyzeVirtRegInBundle(
...
toolchain::AnalyzeVirtRegLanesInBundle(const MachineInstr &MI, Register Reg,
...
PhysRegInfo toolchain::AnalyzePhysRegInBundle(const MachineInstr &MI, Register Reg,
                                         const TargetRegisterInfo *TRI) {
...
PreservedAnalyses
toolchain::FinalizeBundleTestPass::run(MachineFunction &MF,
