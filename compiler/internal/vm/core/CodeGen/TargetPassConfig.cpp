...
#include "vm/core/CodeGen/CSEConfigBase.h"
...
#include "vm/core/Support/Discriminator.h"
...
CGPassBuilderOption toolchain::getCGPassBuilderOption() {
...
void toolchain::registerCodeGenCallback(PassInstrumentationCallbacks &PIC,
                                   TargetMachine &TM) {
...
void TargetPassConfig::addIRPasses() {
  ...
  // For MachO, lower @toolchain.global_dtors into @toolchain.global_ctors with
...
void TargetPassConfig::addMachinePasses() {
  ...
  if (EnableGCEmptyBlocks)
    addPass(toolchain::createGCEmptyBasicBlocksLegacyPass());
  ...
  // address map (or both).
  if (TM->getBBSectionsType() != toolchain::BasicBlockSection::None ||
      TM->Options.BBAddrMap) {
    if (EmitBBHash || BasicBlockSectionMatchInfer)
      addPass(toolchain::createMachineBlockHashInfoPass());
    if (TM->getBBSectionsType() == toolchain::BasicBlockSection::List) {
      addPass(toolchain::createBasicBlockSectionsProfileReaderWrapperPass(
          TM->getBBSectionsFuncListBuf()));
      if (BasicBlockSectionMatchInfer)
        addPass(toolchain::createBasicBlockMatchingAndInferencePass());
      else {
        addPass(toolchain::createBasicBlockPathCloningPass());
        addPass(toolchain::createInsertCodePrefetchPass());
      }
    }
    addPass(toolchain::createBasicBlockSectionsPass());
  }
...
static toolchain::once_flag InitializeDefaultRegisterAllocatorFlag;
...
FunctionPass *TargetPassConfig::createRegAllocPass(bool Optimized) {
  ...
  toolchain::call_once(InitializeDefaultRegisterAllocatorFlag,
