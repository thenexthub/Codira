...
#include "vm/core/CGData/CodeGenDataWriter.h"
...
bool isEligibleFunction(Function *F) {
  ...
  if (F->hasFnAttribute(toolchain::Attribute::NoMerge) ||
...
void GlobalMergeFunc::analyze(Module &M) {
  ...
  for (Function &Func : M) {
    ...
    if (isEligibleFunction(&Func)) {
      ...
      auto FI = toolchain::StructuralHashWithDifferences(Func, ignoreOp);
...
static void createThunk(FuncMergeInfo &FI, ArrayRef<Constant *> Params,
  ...
  CI->setTailCallKind(isSwiftTailCall ? toolchain::CallInst::TCK_MustTail
...
computeParamInfo(const StableFunctionMap::StableFunctionEntries &SFS) {
  ...
  toolchain::sort(ParamLocsVec, [&](const ParamLocs &L, const ParamLocs &R) {
    return L[0] < R[0];
  });
...
bool GlobalMergeFunc::merge(Module &M, const StableFunctionMap *FunctionMap) {
  ...
  for (auto &F : M) {
    if (!isEligibleFunction(&F))
      continue;
    auto FI = toolchain::StructuralHashWithDifferences(F, ignoreOp);
...
ModulePass *toolchain::createGlobalMergeFuncPass() {
