...
char &toolchain::PrologEpilogCodeInserterID = PEILegacy::ID;
...
MachineFunctionPass *toolchain::createPrologEpilogInserterPass() {
...
static void stashEntryDbgValues(MachineBasicBlock &MBB,
  ...
  for (auto &MI : MBB) {
    ...
    if (toolchain::none_of(FrameIndexValues, Overlaps))
...
void PEIImpl::calculateFrameObjectOffsets(MachineFunction &MF) {
  ...
  if (!TFI.targetHandlesStackFrameRounding()) {
    ...
    if (StackGrowsDown && OffsetBeforeAlignment != Offset && RS &&
      ...
      SmallVector<int, 2> SFIs;
      RS->getScavengingFrameIndices(SFIs);
      LLVM_DEBUG(if (!SFIs.empty()) toolchain::dbgs()
      ...
      for (int SFI : SFIs) {
        ...
        LLVM_DEBUG(toolchain::dbgs() << " to " << MFI.getObjectOffset(SFI) << "\n";);
      }
    }
  }
