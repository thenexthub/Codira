...
//===----------------------------------------------------------------------===//
//
// This pass implements IR lowering for the toolchain.memcpy, toolchain.memmove,
// toolchain.memset, toolchain.load.relative and toolchain.objc.* intrinsics.
//
//===----------------------------------------------------------------------===//
...
#include "vm/core/CodeGen/ExpandVectorPredication.h"
...
static bool lowerLoadRelative(Function &F) {
  ...
  for (Use &U : toolchain::make_early_inc_range(F.uses())) {
...
bool PreISelIntrinsicLowering::expandMemIntrinsicUses(
  ...
  for (User *U : toolchain::make_early_inc_range(F.users())) {
    ...
    case Intrinsic::memcpy_inline: {
      // Only expand toolchain.memcpy.inline with non-constant length in this
    ...
    case Intrinsic::memset_inline: {
      // Only expand toolchain.memset.inline with non-constant length in this
      // codepath, leaving the current SelectionDAG expansion for constant
...
static bool expandProtectedFieldPtr(Function &Intr) {
  ...
  for (User *U : toolchain::make_early_inc_range(Intr.users())) {
    ...
    for (Use &U : toolchain::make_early_inc_range(Call->uses())) {
...
ModulePass *toolchain::createPreISelIntrinsicLoweringPass() {
