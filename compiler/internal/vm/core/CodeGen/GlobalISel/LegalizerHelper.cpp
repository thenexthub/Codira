//===-- toolchain/CodeGen/GlobalISel/LegalizerHelper.cpp -----------------------===//
//
...
LegalizerHelper::LegalizeResult
toolchain::createLibcall(MachineIRBuilder &MIRBuilder, const char *Name,
  ...
  toolchain::append_range(Info.OrigArgs, Args);
...
LegalizerHelper::LegalizeResult
toolchain::createLibcall(MachineIRBuilder &MIRBuilder, RTLIB::Libcall Libcall,
                    const CallLowering::ArgInfo &Result,
                    ArrayRef<CallLowering::ArgInfo> Args,
...
LegalizerHelper::LegalizeResult
toolchain::createMemLibcall(MachineIRBuilder &MIRBuilder, MachineRegisterInfo &MRI,
  ...
  case TargetOpcode::G_ICMP: {
    ...
    if (ICmpInst::isEquality(Pred)) {
      ...
      for (auto LHSAndRHS : zip(LHSLeftoverRegs, RHSLeftoverRegs)) {
        ...
        toolchain::append_range(Xors, WidenedXors);
      }
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerLoad(GAnyLoad &LoadMI) {
  ...
  if (!isPowerOf2_32(MemSizeInBits)) {
    // This load needs splitting into power of 2 sized loads.
    LargeSplitSize = toolchain::bit_floor(MemSizeInBits);
...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerStore(GStore &StoreMI) {
  ...
  if (!isPowerOf2_32(MemSizeInBits)) {
    LargeSplitSize = toolchain::bit_floor<uint64_t>(MemTy.getSizeInBits());
...
Align LegalizerHelper::getStackTemporaryAlignment(LLT Ty,
  ...
  // FIXME: We're missing a way to go back from LLT to toolchain::Type to query the
  ...
  for (unsigned UseIdx = NumDefs, UseNo = 0; UseIdx < MI.getNumOperands();
    ...
    if (is_contained(NonVecOpIndices, UseIdx)) {
      ...
      extractVectorParts(MI.getReg(UseIdx), NumElts, SplitPieces, MIRBuilder,
                         MRI);
      toolchain::append_range(InputOpsPieces[UseNo], SplitPieces);
    }
  }
...
LegalizerHelper::fewerElementsVectorMerge(MachineInstr &MI, unsigned TypeIdx,
  ...
  // longer be possible to generate MIR like this when starting from toolchain-ir
...
LegalizerHelper::equalizeVectorShuffleLengths(MachineInstr &MI) {
  ...
  if (MaskNumElts < SrcNumElts) {
    ...
    toolchain::copy(Mask, NewMask.begin());
...
LegalizerHelper::lowerU64ToF64BitFloatOps(MachineInstr &MI) {
  ...
  auto TwoP52P84 = toolchain::bit_cast<double>(UINT64_C(0x4530000000100000));
...
LegalizerHelper::LegalizeResult
LegalizerHelper::lowerVECTOR_COMPRESS(toolchain::MachineInstr &MI) {
  ...
  if (!PhysReg) {
    ...
    Fn.getContext().diagnose(DiagnosticInfoGenericWithLoc(
        "invalid register \"" + Twine(RegStr->getString().data()) + "\" for " +
            (IsRead ? "toolchain.read_register" : "toolchain.write_register"),
        Fn, MI.getDebugLoc()));
    if (IsRead)
...
static bool findGISelOptimalMemOpLowering(std::vector<LLT> &MemOps,
  ...
  while (Size) {
    ...
    while (TySize > Size) {
      ...
      NewTy = LLT::scalar(toolchain::bit_floor(NewTy.getSizeInBits() - 1));
