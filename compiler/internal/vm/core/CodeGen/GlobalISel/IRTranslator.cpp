//===- toolchain/CodeGen/GlobalISel/IRTranslator.cpp - IRTranslator ---*- C++ -*-==//
//
...
#include "vm/core/CodeGen/SwitchLoweringUtils.h"
...
ArrayRef<Register> IRTranslator::getOrCreateVRegs(const Value &Val) {
  ...
  if (Val.getType()->isAggregateType()) {
    ...
    while (auto Elt = C.getAggregateElement(Idx++)) {
      ...
      toolchain::append_range(*VRegs, EltRegs);
    }
  } else {
...
void IRTranslator::emitBitTestHeader(SwitchCG::BitTestBlock &B,
  ...
  if (MaskTy.getSizeInBits() > PtrTy.getSizeInBits() ||
      !toolchain::has_single_bit<uint32_t>(MaskTy.getSizeInBits()))
...
void IRTranslator::emitBitTestCase(SwitchCG::BitTestBlock &BB,
  ...
  if (PopCount == 1) {
    ...
    auto MaskTrailingZeros =
        MIB.buildConstant(SwitchTy, toolchain::countr_zero(B.Mask));
    Cmp =
    ...
    auto MaskTrailingOnes =
        MIB.buildConstant(SwitchTy, toolchain::countr_one(B.Mask));
...
static uint64_t getOffsetFromIndices(const User &U, const DataLayout &DL) {
  ...
  if (const ExtractValueInst *EVI = dyn_cast<ExtractValueInst>(&U)) {
    ...
    toolchain::append_range(Indices, drop_begin(U.operands()));
  }
...
bool IRTranslator::translateExtractValue(const User &U,
  ...
  unsigned Idx = toolchain::lower_bound(Offsets, Offset) - Offsets.begin();
...
bool IRTranslator::translateTrap(const CallInst &CI,
  ...
  if (TrapFuncName.empty()) {
    if (Opcode == TargetOpcode::G_UBSANTRAP) {
      ...
      MIRBuilder.buildInstr(Opcode, {}, ArrayRef<toolchain::SrcOp>{Code});
    } else {
...
bool IRTranslator::translateSimpleIntrinsic(const CallInst &CI,
  ...
  SmallVector<toolchain::SrcOp, 4> VRegs;
...
bool IRTranslator::translateKnownIntrinsic(const CallInst &CI, Intrinsic::ID ID,
  ...
  case Intrinsic::fake_use: {
    SmallVector<toolchain::SrcOp, 4> VRegs;
    for (const auto &Arg : CI.args())
      toolchain::append_range(VRegs, getOrCreateVRegs(*Arg));
...
bool IRTranslator::translateCallBase(const CallBase &CB,
  ...
  for (const auto &Arg : CB.args()) {
    if (CLI->supportSwiftError() && isSwiftError(Arg)) {
      assert(SwiftInVReg == 0 && "Expected only one codira error argument");
...
bool IRTranslator::emitSPDescriptorParent(StackProtectorDescriptor &SPD,
  ...
  if (TLI->useLoadStackGuardNode(*ParentBB->getBasicBlock()->getModule())) {
    ...
    // TODO: test using android subtarget when we support @toolchain.thread.pointer.
...
static bool checkForMustTailInVarArgFn(bool IsVarArg, const BasicBlock &BB) {
  ...
  // of a block.
  return toolchain::any_of(toolchain::reverse(BB), [](const Instruction &I) {
...
bool IRTranslator::runOnMachineFunction(MachineFunction &CurMF) {
  ...
  toolchain::scope_exit FinalizeOnReturn([this]() { finalizeFunction(); });
