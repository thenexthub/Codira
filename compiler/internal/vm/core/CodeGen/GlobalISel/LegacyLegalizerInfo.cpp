...
#include "vm/core/CodeGen/GlobalISel/LegacyLegalizerInfo.h"
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, LegacyLegalizeAction Action) {
  switch (Action) {
  case Legal:
    OS << "Legal";
    break;
  ...
  for (unsigned OpcodeIdx = 0; OpcodeIdx <= LastOp - FirstOp; ++OpcodeIdx) {
    ...
    for (unsigned TypeIdx = 0; TypeIdx != SpecifiedActions[OpcodeIdx].size();
      ...
      // 1. Handle scalar types
      {
        ...
        toolchain::sort(ScalarSpecifiedActions);
      ...
      for (auto PointerSpecifiedActions : AddressSpace2SpecifiedActions) {
        toolchain::sort(PointerSpecifiedActions.second);
      ...
      for (auto VectorSpecifiedActions : ElemSize2SpecifiedActions) {
        toolchain::sort(VectorSpecifiedActions.second);
      ...
      toolchain::sort(ElementSizesSeen);
