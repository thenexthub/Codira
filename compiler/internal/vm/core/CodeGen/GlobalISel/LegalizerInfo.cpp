...
cl::opt<bool> toolchain::DisableGISelLegalityCheck(
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, LegalizeAction Action) {
  switch (Action) {
  case Legal:
    OS << "Legal";
    break;
...
LegalizeRuleSet &LegalizerInfo::getActionDefinitionsBuilder(
  ...
  for (unsigned Op : toolchain::drop_begin(Opcodes))
...
const MachineInstr *toolchain::machineFunctionIsIllegal(const MachineFunction &MF) {
