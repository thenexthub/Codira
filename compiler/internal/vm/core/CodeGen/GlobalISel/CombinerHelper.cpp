...
bool CombinerHelper::matchCombineExtendingLoads(
  ...
  if (!toolchain::has_single_bit<uint32_t>(LoadValueTy.getSizeInBits()))
    return false;
...
void CombinerHelper::applyCombineExtendingLoads(
  ...
  SmallVector<MachineOperand *, 4> Uses(
      toolchain::make_pointer_range(MRI.use_operands(LoadValue.getReg())));
...
bool CombinerHelper::matchBitfieldExtractFromShrAnd(
  ...
  const int64_t Width = toolchain::countr_one(UMask) - ShrAmt;
