...
char &toolchain::MachineSinkingLegacyID = MachineSinkingLegacy::ID;
  ...
  if (SinkInstsIntoCycle) {
    ...
    for (unsigned Stage = CycleSinkStage::COPY; Stage != CycleSinkStage::END;
      ...
      for (auto *Cycle : Cycles) {
        ...
        for (MachineInstr *I : toolchain::reverse(Candidates)) {
...
bool MachineSinking::hasStoreBetween(MachineBasicBlock *From,
  ...
  if (auto It = StoreInstrCache.find(BlockPair); It != StoreInstrCache.end())
    return toolchain::any_of(It->second, [&](MachineInstr *I) {
...
char &toolchain::PostRAMachineSinkingID = PostRAMachineSinkingLegacy::ID;
...
bool PostRAMachineSinkingImpl::tryToSinkCopy(MachineBasicBlock &CurBB,
  ...
  for (MachineInstr &MI : toolchain::make_early_inc_range(toolchain::reverse(CurBB))) {
    ...
    for (auto &MO : MI.all_defs()) {
      for (MCRegUnit Unit : TRI->regunits(MO.getReg())) {
        for (const auto &MIRegs : SeenDbgInstrs.lookup(Unit)) {
          ...
          toolchain::append_range(Regs, MIRegs.second);
        }
      }
    }
