...
Printable toolchain::printReg(Register Reg, const TargetRegisterInfo *TRI,
...
Printable toolchain::printRegUnit(MCRegUnit Unit, const TargetRegisterInfo *TRI) {
...
Printable toolchain::printVRegOrUnit(VirtRegOrUnit VRegOrUnit,
...
Printable toolchain::printRegClassOrBank(Register Reg,
...
const TargetRegisterClass *firstCommonClass(const uint32_t *A,
  ...
  for (unsigned I = 0, E = TRI->getNumRegClasses(); I < E; I += 32)
    if (unsigned Common = *A++ & *B++)
      return TRI->getRegClass(I + toolchain::countr_zero(Common));
  return nullptr;
}
