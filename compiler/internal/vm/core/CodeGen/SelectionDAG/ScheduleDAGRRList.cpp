...
public:
  RegReductionPQBase(MachineFunction &mf,
    ...
    if (TracksRegPressure) {
      ...
      toolchain::fill(RegLimit, 0);
      toolchain::fill(RegPressure, 0);
  ...
  void remove(SUnit *SU) override {
    ...
    std::vector<SUnit *>::iterator I = toolchain::find(Queue, SU);
...
ScheduleDAGSDNodes *toolchain::createBURRListDAGScheduler(SelectionDAGISel *IS,
                                                     CodeGenOptLevel OptLevel) {
...
ScheduleDAGSDNodes *
toolchain::createSourceListDAGScheduler(SelectionDAGISel *IS,
                                   CodeGenOptLevel OptLevel) {
  const TargetSubtargetInfo &STI = IS->MF->getSubtarget();
  const TargetInstrInfo *TII = STI.getInstrInfo();
  const TargetRegisterInfo *TRI = STI.getRegisterInfo();
...
ScheduleDAGSDNodes *
toolchain::createHybridListDAGScheduler(SelectionDAGISel *IS,
                                   CodeGenOptLevel OptLevel) {
  const TargetSubtargetInfo &STI = IS->MF->getSubtarget();
  const TargetInstrInfo *TII = STI.getInstrInfo();
  const TargetRegisterInfo *TRI = STI.getRegisterInfo();
...
ScheduleDAGSDNodes *toolchain::createILPListDAGScheduler(SelectionDAGISel *IS,
                                                    CodeGenOptLevel OptLevel) {
  const TargetSubtargetInfo &STI = IS->MF->getSubtarget();
  const TargetInstrInfo *TII = STI.getInstrInfo();
  const TargetRegisterInfo *TRI = STI.getRegisterInfo();
