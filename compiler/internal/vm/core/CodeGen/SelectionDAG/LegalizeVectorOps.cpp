...
bool VectorLegalizer::Run() {
  ...
  for (SelectionDAG::allnodes_iterator I = DAG.allnodes_begin(),
    ...
    HasVectors = toolchain::any_of(I->values(), [](EVT T) { return T.isVector(); });
...
SDValue VectorLegalizer::LegalizeOp(SDValue Op) {
  ...
  bool HasVectorValueOrOp =
      toolchain::any_of(Node->values(), [](EVT T) { return T.isVector(); }) ||
      toolchain::any_of(Node->op_values(),
...
SDValue VectorLegalizer::ExpandZERO_EXTEND_VECTOR_INREG(SDNode *Node) {
  ...
  auto ShuffleMask = toolchain::to_vector<16>(toolchain::seq<int>(0, NumSrcElements));
