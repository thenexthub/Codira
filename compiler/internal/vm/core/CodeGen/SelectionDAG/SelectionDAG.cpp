...
bool SelectionDAG::shouldOptForSize() const {
  return toolchain::shouldOptimizeForSize(FLI->MBB->getBasicBlock(), PSI, BFI);
}
...
void SelectionDAG::clear() {
  ...
  MCSymbols.clear();
  SDEI.clear();
  toolchain::fill(CondCodeNodes, nullptr);
  toolchain::fill(ValueTypeNodes, nullptr);
...
SDValue SelectionDAG::getConstant(const ConstantInt &Val, const SDLoc &DL,
  ...
  // only legalize if the DAG tells us we must produce legal types.
    ...
    for (unsigned i = 0, e = VT.getVectorNumElements(); i != e; ++i)
      toolchain::append_range(Ops, EltParts);
...
SDValue SelectionDAG::getVectorShuffle(EVT VT, const SDLoc &dl, SDValue N1,
  ...
  toolchain::copy(MaskVec, MaskAlloc);
...
KnownBits SelectionDAG::computeKnownBits(SDValue Op, const APInt &DemandedElts,
  ...
  case ISD::CTPOP: {
    ...
    Known.Zero.setBitsFrom(toolchain::bit_width(PossibleOnes));
    break;
  }
  case ISD::PARITY: {
...
bool SelectionDAG::isKnownToBeAPowerOfTwo(SDValue Val, unsigned Depth) const {
  ...
  if (Val.getOpcode() == ISD::BUILD_VECTOR)
    if (toolchain::all_of(Val->ops(), [BitWidth](SDValue E) {
...
SDValue toolchain::getBitwiseNotOperand(SDValue V, SDValue Mask, bool AllowUndefs) {
...
// FIXME: unify with toolchain::haveNoCommonBitsSet.
...
static SDValue FoldBUILD_VECTOR(const SDLoc &DL, EVT VT,
  ...
  if (toolchain::all_of(Ops, [](SDValue Op) { return Op.isUndef(); }))
...
bool SelectionDAG::isUndef(unsigned Opcode, ArrayRef<SDValue> Ops) {
  ...
  case ISD::UREM: {
    ...
    return ISD::isBuildVectorOfConstantSDNodes(Divisor.getNode()) &&
           toolchain::any_of(Divisor->op_values(),
...
SDValue SelectionDAG::FoldConstantArithmetic(unsigned Opcode, const SDLoc &DL,
  ...
  if (NumOps == 2) {
    ...
    if (Opcode == ISD::SIGN_EXTEND_INREG) {
      ...
      auto SignExtendInReg = [&](APInt Val, toolchain::EVT ConstantVT) {
      ...
      if (ISD::isBuildVectorOfConstantSDNodes(Ops[0].getNode())) {
        ...
        toolchain::EVT OpVT = Ops[0].getOperand(0).getValueType();
  ...
  // or UNDEF scalars.
  if (!toolchain::all_of(Ops, IsBuildVectorSplatVectorOrUndef) ||
      !toolchain::all_of(Ops, IsScalarOrSameVectorSize))
    return SDValue();
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT,
  ...
  case ISD::AssertNoFPClass: {
    ...
    assert(toolchain::to_underlying(NoFPClass) <=
...
/// SDValue otherwise.
///
/// The function tries to replace 'toolchain.memset' intrinsic with several store
...
void SelectionDAG::setNodeMemRefs(MachineSDNode *N,
  ...
  toolchain::copy(NewMemRefs, MemRefsBuffer);
...
void SelectionDAG::salvageDebugInfo(SDNode &N) {
  ...
  for (SDDbgValue *Dbg : ClonedDVs) {
    assert((!Dbg->getSDNodes().empty() ||
            toolchain::any_of(Dbg->getLocationOps(),
...
void SelectionDAG::updateDivergence(SDNode *N) {
  ...
  do {
    ...
    if (N->SDNodeBits.IsDivergent != IsDivergent) {
      ...
      toolchain::append_range(Worklist, N->users());
    }
...
unsigned SelectionDAG::AssignTopologicalOrder() {
  ...
  for (SDNode &N : toolchain::make_early_inc_range(allnodes())) {
...
bool toolchain::isNullConstant(SDValue V) {
  ConstantSDNode *Const = dyn_cast<ConstantSDNode>(V);
  return Const != nullptr && Const->isZero();
}

bool toolchain::isNullConstantOrUndef(SDValue V) {
  return V.isUndef() || isNullConstant(V);
}

bool toolchain::isNullFPConstant(SDValue V) {
  ConstantFPSDNode *Const = dyn_cast<ConstantFPSDNode>(V);
  return Const != nullptr && Const->isZero() && !Const->isNegative();
}

bool toolchain::isAllOnesConstant(SDValue V) {
  ConstantSDNode *Const = dyn_cast<ConstantSDNode>(V);
  return Const != nullptr && Const->isAllOnes();
}

bool toolchain::isOneConstant(SDValue V) {
  ConstantSDNode *Const = dyn_cast<ConstantSDNode>(V);
  return Const != nullptr && Const->isOne();
}

bool toolchain::isMinSignedConstant(SDValue V) {
  ConstantSDNode *Const = dyn_cast<ConstantSDNode>(V);
  return Const != nullptr && Const->isMinSignedValue();
}

bool toolchain::isNeutralConstant(unsigned Opcode, SDNodeFlags Flags, SDValue V,
                             unsigned OperandNo) {
...
SDValue toolchain::peekThroughBitcasts(SDValue V) {
  while (V.getOpcode() == ISD::BITCAST)
    V = V.getOperand(0);
  return V;
}

SDValue toolchain::peekThroughOneUseBitcasts(SDValue V) {
  while (V.getOpcode() == ISD::BITCAST && V.getOperand(0).hasOneUse())
    V = V.getOperand(0);
  return V;
}

SDValue toolchain::peekThroughExtractSubvectors(SDValue V) {
  while (V.getOpcode() == ISD::EXTRACT_SUBVECTOR)
    V = V.getOperand(0);
  return V;
}

SDValue toolchain::peekThroughInsertVectorElt(SDValue V, const APInt &DemandedElts) {
...
SDValue toolchain::peekThroughTruncates(SDValue V) {
  while (V.getOpcode() == ISD::TRUNCATE)
    V = V.getOperand(0);
  return V;
}

bool toolchain::isBitwiseNot(SDValue V, bool AllowUndefs) {
...
ConstantSDNode *toolchain::isConstOrConstSplat(SDValue N, bool AllowUndefs,
...
ConstantSDNode *toolchain::isConstOrConstSplat(SDValue N, const APInt &DemandedElts,
                                          bool AllowUndefs,
                                          bool AllowTruncation) {
...
ConstantFPSDNode *toolchain::isConstOrConstSplatFP(SDValue N, bool AllowUndefs) {
  EVT VT = N.getValueType();
  APInt DemandedElts = VT.isFixedLengthVector()
                           ? APInt::getAllOnes(VT.getVectorMinNumElements())
                           : APInt(1, 1);
...
ConstantFPSDNode *toolchain::isConstOrConstSplatFP(SDValue N,
                                              const APInt &DemandedElts,
                                              bool AllowUndefs) {
...
bool toolchain::isNullOrNullSplat(SDValue N, bool AllowUndefs) {
...
bool toolchain::isOneOrOneSplat(SDValue N, bool AllowUndefs) {
  ConstantSDNode *C =
      isConstOrConstSplat(N, AllowUndefs, /*AllowTruncation*/ true);
  return C && C->isOne();
}

bool toolchain::isOneOrOneSplatFP(SDValue N, bool AllowUndefs) {
  ConstantFPSDNode *C = isConstOrConstSplatFP(N, AllowUndefs);
  return C && C->isExactlyValue(1.0);
}

bool toolchain::isAllOnesOrAllOnesSplat(SDValue N, bool AllowUndefs) {
...
bool toolchain::isOnesOrOnesSplat(SDValue N, bool AllowUndefs) {
  ConstantSDNode *C = isConstOrConstSplat(N, AllowUndefs);
  return C && APInt::isSameValue(C->getAPIntValue(),
                                 APInt(C->getAPIntValue().getBitWidth(), 1));
}

bool toolchain::isZeroOrZeroSplat(SDValue N, bool AllowUndefs) {
  N = peekThroughBitcasts(N);
  ConstantSDNode *C = isConstOrConstSplat(N, AllowUndefs, true);
  return C && C->isZero();
}

bool toolchain::isZeroOrZeroSplatFP(SDValue N, bool AllowUndefs) {
  ConstantFPSDNode *C = isConstOrConstSplatFP(N, AllowUndefs);
  return C && C->isZero();
}
...
bool SDNode::areOnlyUsersOf(ArrayRef<const SDNode *> Nodes, const SDNode *N) {
  bool Seen = false;
  for (const SDNode *User : N->users()) {
    if (toolchain::is_contained(Nodes, User))
      Seen = true;
    else
      return false;
  }
...
bool SDValue::reachesChainWithoutSideEffects(SDValue Dest,
  ...
  if (getOpcode() == ISD::TokenFactor) {
    ...
    // reaches Dest.
    return toolchain::all_of((*this)->ops(), [=](SDValue Op) {
...
SelectionDAG::matchBinOpReduction(SDNode *Extract, ISD::NodeType &BinOp,
  ...
  if (toolchain::none_of(CandidateBinOps, [Op](ISD::NodeType BinOp) {
...
MaybeAlign SelectionDAG::InferPtrAlign(SDValue Ptr) const {
  ...
  if (TLI->isGAPlusOffset(Ptr.getNode(), GV, GVOffset)) {
    ...
    toolchain::computeKnownBits(GV, Known, getDataLayout());
...
static void checkForCyclesHelper(const SDNode *N,
                                 ...
                                 const toolchain::SelectionDAG *DAG) {
...
void toolchain::checkForCycles(const toolchain::SDNode *N,
                          const toolchain::SelectionDAG *DAG,
                          bool force) {
#ifndef NDEBUG
  bool check = force;
...
void toolchain::checkForCycles(const toolchain::SelectionDAG *DAG, bool force) {
