...
void SelectionDAGLegalize::LegalizeOp(SDNode *Node) {
  ...
  case ISD::POISON: {
    ...
    // (e.g., toolchain/test/CodeGen/PowerPC/vec_shuffle.ll). A broader discussion
...
SDValue SelectionDAGLegalize::ExpandLegalINT_TO_FP(SDNode *Node,
  ...
  if (SrcVT == MVT::i32 && TLI.isTypeLegal(MVT::f64) &&
    ...
    SDValue Bias = DAG.getConstantFP(
        isSigned ? toolchain::bit_cast<double>(0x4330000080000000ULL)
                 : toolchain::bit_cast<double>(0x4330000000000000ULL),
        dl, MVT::f64);
...
bool SelectionDAGLegalize::ExpandNode(SDNode *Node) {
  ...
  case ISD::FCANONICALIZE: {
    // This implements toolchain.canonicalize.f* by multiplication with 1.0, as
    // suggested in
    // https://toolchain.org/docs/LangRef.html#toolchain-canonicalize-intrinsic.
  ...
  case ISD::BR_JT: {
    ...
    if (toolchain::isPowerOf2_32(EntrySize))
      Index = DAG.getNode(
          ISD::SHL, dl, Index.getValueType(), Index,
          DAG.getConstant(toolchain::Log2_32(EntrySize), dl, Index.getValueType()));
    else
    ...
    if (TLI.isJumpTableRelative()) {
      ...
      Addr = DAG.getMemBasePlusOffset(TLI.getPICODEumpTableRelocBase(Table, DAG),
                                      Addr, dl);
    }
  ...
  case ISD::VP_CTTZ_ELTS_ZERO_UNDEF:
    ...
    break;
  case ISD::CLEAR_CACHE:
    // The default expansion of toolchain.clear_cache is simply a no-op for those
