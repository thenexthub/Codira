...
#include "vm/core/CodeGen/FunctionLoweringInfo.h"
#include "vm/core/CodeGen/GCMetadata.h"
#include "vm/core/CodeGen/ISDOpcodes.h"
...
#include "vm/core/CodeGen/SwiftErrorValueTracking.h"
...
#include "vm/core/CodeGen/WinEHFuncInfo.h"
...
// sequence over this should have been converted to toolchain.memcpy by the
...
getCopyFromParts(SelectionDAG &DAG, const SDLoc &DL, const SDValue *Parts,
  ...
  if (NumParts > 1) {
    ...
    if (ValueVT.isInteger()) {
      ...
      unsigned RoundParts = toolchain::bit_floor(NumParts);
...
SDValue SelectionDAGBuilder::getFPOperationRoot(fp::ExceptionBehavior EB) {
  ...
  case fp::ExceptionBehavior::ebStrict:
    ...
    // exception flags, like 'toolchain.get_fpenv' or 'fetestexcept'. It means that
...
void SelectionDAGBuilder::visitRet(const ReturnInst &I) {
  ...
  // Calls to @toolchain.experimental.deoptimize don't generate a return value, so
  // lower
  //
  //   %val = call <ty> @toolchain.experimental.deoptimize()
  //   ret <ty> %val
  //
  // differently.
  ...
  if (TLI.supportSwiftError() &&
    ...
    assert(SwiftError.getFunctionArg() && "Need a codira error argument");
    ISD::ArgFlagsTy Flags = ISD::ArgFlagsTy();
    Flags.setSwiftError();
...
void SelectionDAGBuilder::visitBitTestCase(BitTestBlock &BB,
  ...
  SDValue Cmp;
  unsigned PopCount = toolchain::popcount(B.Mask);
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  if (PopCount == 1) {
    ...
    Cmp = DAG.getSetCC(
        ...
        ShiftOp, DAG.getConstant(toolchain::countr_zero(B.Mask), dl, VT),
        ISD::SETEQ);
  } else if (PopCount == BB.Range) {
    // There is only one zero bit in the range, test for it directly.
    Cmp = DAG.getSetCC(
        dl, TLI.getSetCCResultType(DAG.getDataLayout(), *DAG.getContext(), VT),
        ShiftOp, DAG.getConstant(toolchain::countr_one(B.Mask), dl, VT), ISD::SETNE);
  } else {
    // Make desired shift
...
void SelectionDAGBuilder::visitInvoke(const InvokeInst &I) {
  ...
  if (isa<InlineAsm>(Callee))
    ...
    case Intrinsic::donothing:
      // Ignore invokes to @toolchain.donothing: jump directly to the next BB.
    ...
    // Eventually we will support lowering the @toolchain.experimental.deoptimize
...
static bool hasOnlySelectUsers(const Value *Cond) {
  return toolchain::all_of(Cond->users(), [](const Value *V) {
...
void SelectionDAGBuilder::visitLoad(const LoadInst &I) {
  ...
  for (unsigned i = 0; i != NumValues; ++i, ++ChainI) {
    ...
    // avoid this case by converting large object/array copies to toolchain.memcpy
...
// %res = call <8 x i32> @toolchain.masked.gather.v8i32(<8 x i32*> %gep.ptr, ..
//
...
void SelectionDAGBuilder::visitMaskedScatter(const CallInst &I) {
  ...
  // toolchain.masked.scatter.*(Src0, Ptrs, Mask)
...
void SelectionDAGBuilder::visitMaskedGather(const CallInst &I) {
  ...
  // @toolchain.masked.gather.*(Ptrs, Mask, Src0)
...
/// ExpandPowI - Expand a toolchain.powi intrinsic.
...
bool SelectionDAGBuilder::EmitFuncArgumentDbgValue(
  ...
  if (Kind == FuncArgumentDbgValueKind::Value) {
    ...
    //  entry:
    //    call void @toolchain.dbg.value(metadata i32 %a1, "a", DW_OP_LLVM_fragment
    //    call void @toolchain.dbg.value(metadata i32 %a2, "a", DW_OP_LLVM_fragment
    //    call void @toolchain.dbg.value(metadata i32 %b, "b",
    //    ...
    //    call void @toolchain.dbg.value(metadata i32 %a1, "b"
    //    ...
    //
...
/// Given a @toolchain.call.preallocated.setup, return the corresponding
/// preallocated call.
...
void SelectionDAGBuilder::visitIntrinsicCall(const CallInst &I,
  ...
  case Intrinsic::memcpy_inline: {
    ...
    // @toolchain.memcpy.inline defines 0 and 1 to both mean no alignment.
  ...
  case Intrinsic::memset_inline: {
    ...
    // @toolchain.memset defines 0 and 1 to both mean no alignment.
  ...
  case Intrinsic::memmove: {
    ...
    // @toolchain.memmove defines 0 and 1 to both mean no alignment.
...
#include "vm/core/IR/VPIntrinsics.def"
  ...
  case Intrinsic::is_fpclass: {
    ...
    SDNodeFlags Flags;
    Flags.setNoFPExcept(
        !F.getAttributes().hasFnAttr(toolchain::Attribute::StrictFP));
  ...
  case Intrinsic::objectsize:
    llvm_unreachable("toolchain.objectsize.* should have been lowered already");

  case Intrinsic::is_constant:
    llvm_unreachable("toolchain.is.constant.* should have been lowered already");
  ...
  case Intrinsic::localrecover: {
    // i8* @toolchain.localrecover(i8* %fn, i8* %fp, i32 %idx)
    MachineFunction &MF = DAG.getMachineFunction();
  ...
  case Intrinsic::icall_branch_funnel: {
    ...
    if (!Base)
      report_fatal_error(
          "toolchain.icall.branch.funnel operand must be a GlobalValue");
    ...
    for (unsigned Op = 1, N = I.arg_size(); Op != N; Op += 2) {
      ...
      if (ElemBase != Base)
        report_fatal_error("all toolchain.icall.branch.funnel operands must refer "
...
void SelectionDAGBuilder::visitInlineAsm(const CallBase &Call,
  ...
  for (SDISelAsmOperandInfo &OpInfo : ConstraintOperands) {
    ...
    //
    // Due to current toolchain inline asm architecture designed to not "recognize"
  ...
  for (SDISelAsmOperandInfo &OpInfo : ConstraintOperands) {
    ...
    case InlineAsm::isLabel: {
      ...
      if (OpInfo.ConstraintType == TargetLowering::C_Immediate ||
        ...
        toolchain::append_range(AsmNodeOperands, Ops);
        break;
      }
  ...
  toolchain::Type *CallResultType = Call.getType();
...
/// assumption made by the toolchain.gcroot intrinsic). If the alloca's location were
...
/// Lower toolchain.experimental.stackmap.
void SelectionDAGBuilder::visitStackmap(const CallInst &CI) {
  // void @toolchain.experimental.stackmap(i64 <id>, i32 <numShadowBytes>,
  //                                  [live variables...])
...
/// Lower toolchain.experimental.patchpoint directly to its target opcode.
void SelectionDAGBuilder::visitPatchpoint(const CallBase &CB,
                                          const BasicBlock *EHPadBB) {
  // <ty> @toolchain.experimental.patchpoint.<ty>(i64 <id>,
  //                                         i32 <numBytes>,
  //                                         i8* <target>,
  //                                         i32 <numArgs>,
  //                                         [Args...],
...
void SelectionDAGBuilder::lowerWorkItem(SwitchWorkListItem W, Value *Cond,
  ...
  if (TM.getOptLevel() != CodeGenOptLevel::None) {
    ...
    toolchain::sort(W.FirstCluster, W.LastCluster + 1,
