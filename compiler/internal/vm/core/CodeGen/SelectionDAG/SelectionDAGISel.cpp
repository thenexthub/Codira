...
#include "vm/core/CodeGen/FastISel.h"
...
#include "vm/core/CodeGen/SchedulerRegistry.h"
...
void SelectionDAGISel::reportIPToStateForBlocks(MachineFunction *MF) {
  toolchain::WinEHFuncInfo *EHInfo = MF->getWinEHFuncInfo();
  if (!EHInfo)
    return;
  for (MachineBasicBlock &MBB : *MF) {
...
/// Collect toolchain.dbg.declare information. This is done after argument lowering
...
void SelectionDAGISel::SelectInlineAsmMemoryOperands(std::vector<SDValue> &Ops,
  ...
  while (i != e) {
    ...
    if (!Flags.isMemKind() && !Flags.isFuncKind()) {
      ...
      toolchain::append_range(Handles, SelOps);
      i += 2;
    }
  }
    ...
    for (unsigned i = 0, e = ChainNodesMatched.size(); i != e; ++i) {
      ...
      SelectionDAG::DAGNodeDeletedListener NDL(
          *CurDAG, [&](SDNode *N, SDNode *E) {
            toolchain::replace(ChainNodesMatched, N, static_cast<SDNode *>(nullptr));
          });
      ...
      if (ChainNode != NodeToMatch && ChainNode->use_empty() &&
          !toolchain::is_contained(NowDeadNodes, ChainNode))
  ...
  while (true) {
    ...
    case OPC_MorphNodeTo2GlueOutput: {
      ...
      bool MayRaiseFPException =
          toolchain::any_of(ChainNodesMatched, [this](SDNode *N) {
