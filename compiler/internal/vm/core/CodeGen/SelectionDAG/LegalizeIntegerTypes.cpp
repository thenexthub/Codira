...
SDValue DAGTypeLegalizer::PromoteIntOp_WRITE_REGISTER(SDNode *N,
  ...
  Fn.getContext().diagnose(DiagnosticInfoLegalizationFailure(
      "cannot use toolchain.write_register with illegal type", Fn,
      N->getDebugLoc()));
...
void DAGTypeLegalizer::ExpandIntRes_READ_REGISTER(SDNode *N, SDValue &Lo,
  ...
  const Function &Fn = DAG.getMachineFunction().getFunction();
  Fn.getContext().diagnose(DiagnosticInfoLegalizationFailure(
      "cannot use toolchain.read_register with illegal type", Fn, N->getDebugLoc()));
...
SDValue DAGTypeLegalizer::ExpandIntOp_STACKMAP(SDNode *N, unsigned OpNo) {
  ...
  //  - https://github.com/toolchain/toolchain-project/issues/26431
  //  - https://github.com/toolchain/toolchain-project/issues/55957
  ...
  if (CN->getConstantIntValue()->getValue().getActiveBits() < 64) {
    ...
    // FIXME: https://github.com/toolchain/toolchain-project/issues/55609
    return SDValue();
  }
