//===-- toolchain/CodeGen/SDNodeDbgValue.h - SelectionDAG dbg_value --*- C++ -*-===//
//
...
public:
  SDDbgValue(BumpPtrAllocator &Alloc, DIVariable *Var, DIExpression *Expr,
    ...
    toolchain::copy(L, LocationOps);
    toolchain::copy(Dependencies, AdditionalDependencies);
  }
  ...
  SmallVector<SDNode *> getSDNodes() const {
    ...
    toolchain::append_range(Dependencies, getAdditionalDependencies());
    return Dependencies;
  }
