...
#include "vm/core/Support/DivisionByConstantInfo.h"
...
SDValue TargetLowering::getPICODEumpTableRelocBase(SDValue Table,
                                                 SelectionDAG &DAG) const {
  return Table;
}

/// This returns the relocation base for the given PIC jumptable, the same as
/// getPICODEumpTableRelocBase, but as an MCExpr.
const MCExpr *
TargetLowering::getPICODEumpTableRelocBaseExpr(const MachineFunction *MF,
...
bool TargetLowering::ShrinkDemandedOp(SDValue Op, unsigned BitWidth,
  ...
  for (unsigned SmallVTBits = toolchain::bit_ceil(DemandedSize);
...
static SDValue combineShiftToAVG(SDValue Op,
  ...
  EVT NVT = EVT::getIntegerVT(*DAG.getContext(), toolchain::bit_ceil(MinWidth));
...
bool TargetLowering::SimplifyDemandedBits(
  ...
  if (!isTargetCanonicalConstantNode(Op) &&
    ...
    if (toolchain::any_of(Op->ops(), [](SDValue V) {
...
bool TargetLowering::SimplifyDemandedVectorElts(
  ...
  case ISD::BUILD_VECTOR: {
    ...
    if (!DemandedElts.isAllOnes()) {
      ...
      if (toolchain::any_of(Op->op_values(),
...
SDValue TargetLowering::SimplifySetCC(EVT VT, SDValue N0, SDValue N1,
  ...
  if (auto *N1C = isConstOrConstSplat(N1)) {
    ...
    if (N0.getOpcode() == ISD::SRL && (C1.isZero() || C1.isOne()) &&
        ...
        toolchain::has_single_bit<uint32_t>(N0.getScalarValueSizeInBits())) {
...
TargetLowering::ParseConstraints(const DataLayout &DL,
  ...
  for (InlineAsm::ConstraintInfo &CI : IA->ParseConstraints()) {
    ...
    if (OpInfo.CallOperandVal) {
      toolchain::Type *OpTy = OpInfo.CallOperandVal->getType();
      if (OpInfo.isIndirect) {
...
/// https://github.com/toolchain/toolchain-project/issues/20571
...
TargetLowering::ConstraintGroup TargetLowering::getConstraintPreferences(
  ...
  toolchain::stable_sort(Ret, [](ConstraintPair a, ConstraintPair b) {
...
turnVectorIntoSplatVector(MutableArrayRef<SDValue> Values,
  ...
  auto SplatValue = toolchain::find_if_not(Values, Predicate);
  if (SplatValue != Values.end()) {
    // Does Values consist only of SplatValue's and values matching Predicate?
    if (toolchain::all_of(Values, [Predicate, SplatValue](SDValue Value) {
...
SDValue TargetLowering::getNegatedExpression(SDValue Op, SelectionDAG &DAG,
  ...
  case ISD::BUILD_VECTOR: {
    ...
    if (toolchain::any_of(Op->op_values(), [&](SDValue N) {
    ...
    bool IsOpLegal =
        ...
        toolchain::all_of(Op->op_values(), [&](SDValue N) {
          return N.isUndef() ||
...
bool TargetLowering::expandFP_TO_SINT(SDNode *Node, SDValue &Result,
  ...
  // https://github.com/toolchain/toolchain-project/blob/main/compiler-rt/lib/builtins/fixsfdi.c
...
bool TargetLowering::expandUINT_TO_FP(SDNode *Node, SDValue &Result,
  ...
  SDValue TwoP84PlusTwoP52 = DAG.getConstantFP(
      toolchain::bit_cast<double>(UINT64_C(0x4530000000100000)), dl, DstVT);
...
SDValue TargetLowering::expandIS_FPCLASS(EVT ResultVT, SDValue Op,
  ...
  const toolchain::fltSemantics &Semantics = FloatTy->getFltSemantics();
...
SDValue TargetLowering::expandIntMINMAX(SDNode *Node, SelectionDAG &DAG) const {
  ...
  if (Opcode == ISD::UMAX && toolchain::isOneOrOneSplat(Op1, true) && BoolVT == VT &&
...
SDValue TargetLowering::expandPartialReduceMLA(SDNode *N,
  ...
  if (N->getOpcode() == ISD::PARTIAL_REDUCE_FMLA) {
    if (!toolchain::isOneOrOneSplatFP(MulRHS))
      Input = DAG.getNode(ISD::FMUL, DL, ExtMulOpVT, MulLHS, MulRHS);
  } else if (!toolchain::isOneOrOneSplat(MulRHS)) {
...
bool TargetLowering::expandMultipleResultFPLibCall(
  ...
  for (auto [ResNo, ST] : toolchain::enumerate(ResultStores)) {
  ...
  for (auto [ResNo, ResultPtr] : toolchain::enumerate(ResultPtrs)) {
    if (ResNo == CallRetResNo) {
