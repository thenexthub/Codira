...
static std::optional<int> findPreviousSpillSlot(const Value *Val,
  ...
  // toolchain's ability to remove redundant stores.
...
SDValue SelectionDAGBuilder::LowerAsSTATEPOINT(
  ...
  toolchain::append_range(Ops, LoweredMetaArgs);
...
SelectionDAGBuilder::LowerStatepoint(const GCStatepointInst &I,
  ...
  // toolchain will create export register of the wrong type (always i32 in our
...
void SelectionDAGBuilder::LowerDeoptimizingReturn() {
  // We do not lower the return value from toolchain.deoptimize to any virtual
