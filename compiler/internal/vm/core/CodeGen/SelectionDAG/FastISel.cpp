...
void FastISel::flushLocalValueMap() {
  ...
  if (LastLocalValue != EmitStartPt) {
    ...
    for (MachineInstr &LocalMI :
         toolchain::make_early_inc_range(toolchain::make_range(RI, RE))) {
...
void FastISel::handleDbgInfo(const Instruction *II) {
  ...
  for (DbgRecord &DR : toolchain::reverse(II->getDbgRecordRange())) {
    flushLocalValueMap();
    recomputeInsertPt();
...
bool FastISel::lowerDbgValue(const Value *V, DIExpression *Expr,
  ...
  if (const auto *Arg = dyn_cast<Argument>(V);
    ...
    // As per the Verifier, this case is only valid for codira async Args.
...
bool FastISel::selectIntrinsicCall(const IntrinsicInst *II) {
  ...
  // Neither does the toolchain.experimental.noalias.scope.decl intrinsic
