//===-- toolchain/CodeGen/MachineBasicBlock.cpp ----------------------*- C++ -*-===//
//
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const MachineBasicBlock &MBB) {
  MBB.print(OS);
  return OS;
}

Printable toolchain::printMBBReference(const MachineBasicBlock &MBB) {
...
void MachineBasicBlock::splitSuccessor(MachineBasicBlock *Old,
  ...
  succ_iterator OldI = toolchain::find(successors(), Old);
  assert(OldI != succ_end() && "Old is not a successor of this block!");
  assert(!toolchain::is_contained(successors(), New) &&
...
MachineBasicBlock *MachineBasicBlock::SplitCriticalEdge(
  ...
  if (LV)
    for (MachineInstr &MI :
         toolchain::make_range(getFirstInstrTerminator(), instr_end())) {
...
bool MachineBasicBlock::canPredictBranchProbabilities() const {
  ...
  return toolchain::equal(Normalized, Equal);
}
