...
/// index list.
unsigned toolchain::ComputeLinearIndex(Type *Ty,
  ...
  if (StructType *STy = dyn_cast<StructType>(Ty)) {
    for (auto I : toolchain::enumerate(STy->elements())) {
      Type *ET = I.value();
...
void toolchain::ComputeValueTypes(const DataLayout &DL, Type *Ty,
...
///
void toolchain::ComputeValueVTs(const TargetLowering &TLI, const DataLayout &DL,
...
void toolchain::computeValueLLTs(const DataLayout &DL, Type &Ty,
...
GlobalValue *toolchain::ExtractTypeInfo(Value *V) {
  V = V->stripPointerCasts();
  GlobalValue *GV = dyn_cast<GlobalValue>(V);
  GlobalVariable *Var = dyn_cast<GlobalVariable>(V);

  if (Var && Var->getName() == "toolchain.eh.catch.all.value") {
...
///
ISD::CondCode toolchain::getFCmpCondCode(FCmpInst::Predicate Pred) {
  switch (Pred) {
...
ISD::CondCode toolchain::getFCmpCodeWithoutNaN(ISD::CondCode CC) {
  switch (CC) {
...
ISD::CondCode toolchain::getICmpCondCode(ICmpInst::Predicate Pred) {
  switch (Pred) {
...
ICmpInst::Predicate toolchain::getICmpCondCode(ISD::CondCode Pred) {
  switch (Pred) {
  case ISD::SETEQ:
...
bool toolchain::isInTailCallPosition(const CallBase &Call, const TargetMachine &TM,
...
bool toolchain::attributesPermitTailCall(const Function *F, const Instruction *I,
...
bool toolchain::returnTypeIsEligibleForTailCall(const Function *F,
                                           const Instruction *I,
                                           const ReturnInst *Ret,
                                           const TargetLoweringBase &TLI,
                                           bool ReturnsFirstArg) {
  ...
  do {
    ...
    SmallVector<unsigned, 4> TmpRetPath(toolchain::reverse(RetPath));
    SmallVector<unsigned, 4> TmpCallPath(toolchain::reverse(CallPath));
...
bool toolchain::funcReturnsFirstArgOfCall(const CallInst &CI) {
...
toolchain::getEHScopeMembership(const MachineFunction &MF) {
