...
void SplitAnalysis::analyzeUses() {
  ...
  UseSlots.erase(toolchain::unique(UseSlots, SlotIndex::isSameInstr),
                 UseSlots.end());
...
void SplitEditor::rewriteAssigned(bool ExtendRanges) {
  ...
  for (MachineOperand &MO :
       toolchain::make_early_inc_range(MRI.reg_operands(Edit->getReg()))) {
...
void SplitEditor::finish(SmallVectorImpl<unsigned> *LRMap) {
  ...
  if (LRMap) {
    auto Seq = toolchain::seq<unsigned>(0, Edit->size());
