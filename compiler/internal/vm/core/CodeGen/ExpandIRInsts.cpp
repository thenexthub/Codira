...
#include "vm/core/Analysis/SimplifyQuery.h"
...
#include "vm/core/Transforms/Utils/IntegerDivision.h"
...
static cl::opt<unsigned>
    ExpandFpConvertBits("expand-fp-convert-bits", cl::Hidden,
                        cl::init(toolchain::IntegerType::MAX_INT_BITS),
...
namespace {
bool isConstantPowerOfTwo(toolchain::Value *V, bool SignedOp) {
...
public:
  ...
  static FRemExpander create(IRBuilder<> &B, Type *Ty) {
    ...
    unsigned Precision =
        toolchain::APFloat::semanticsPrecision(Ty->getFltSemantics());
...
///   %0 = tail call i64 @toolchain.ctlz.i64(i64 %sub, i1 true), !range !5
...
static void expandIToFP(Instruction *IToFP) {
  ...
  toolchain::SwitchInst *SI = Builder.CreateSwitch(Sub1, SwDefault);
...
static bool runImpl(Function &F, const TargetLowering &TLI,
  ...
  if (ExpandFpConvertBits != toolchain::IntegerType::MAX_INT_BITS)
    MaxLegalFpConvertBitWidth = ExpandFpConvertBits;

  unsigned MaxLegalDivRemBitWidth = TLI.getMaxDivRemBitWidthSupported();
  if (ExpandDivRemBits != toolchain::IntegerType::MAX_INT_BITS)
    MaxLegalDivRemBitWidth = ExpandDivRemBits;

  bool DisableExpandLargeFp =
      MaxLegalFpConvertBitWidth >= toolchain::IntegerType::MAX_INT_BITS;
  bool DisableExpandLargeDivRem =
      MaxLegalDivRemBitWidth >= toolchain::IntegerType::MAX_INT_BITS;
...
FunctionPass *toolchain::createExpandIRInstsPass(CodeGenOptLevel OptLevel) {
