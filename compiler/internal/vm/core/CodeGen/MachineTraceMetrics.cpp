...
char &toolchain::MachineTraceMetricsID = MachineTraceMetricsWrapperPass::ID;
...
computeHeightResources(const MachineBasicBlock *MBB) {
  ...
  if (!TBI->Succ) {
    ...
    toolchain::copy(PRCycles, ProcResourceHeights.begin() + PROffset);
    return;
  }
...
template <> class toolchain::po_iterator_storage<LoopBounds, true> {
  LoopBounds &LB;
...
addLiveIns(const MachineInstr *DefMI, unsigned DefOp,
  ...
  for (const MachineBasicBlock *MBB : toolchain::reverse(Trace)) {
    if (MBB == DefMBB)
      return;
    TraceBlockInfo &TBI = BlockInfo[MBB->getNumber()];
