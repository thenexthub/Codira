...
#include "vm/core/CodeGen/LiveRangeCalc.h"
...
FunctionPass *toolchain::createMachineVerifierPass(const std::string &Banner) {
  return new MachineVerifierLegacyPass(Banner);
}

void toolchain::verifyMachineFunction(const std::string &Banner,
          ...
          for (const MachineOperand &MOP : MI->uses()) {
            ...
            if (MOP.getReg() != Reg &&
                all_of(TRI->regunits(Reg), [&](const MCRegUnit RegUnit) {
                  return toolchain::is_contained(TRI->regunits(MOP.getReg()),
                                            RegUnit);
                }))
              Bad = false;
          }
...
void MachineVerifier::verifyLiveRangeSegment(const LiveRange &LR,
  ...
  while (true) {
    ...
    for (const MachineBasicBlock *Pred : MFI->predecessors()) {
      ...
      if (MFI->isEHPad()) {
        for (const MachineInstr &MI : toolchain::reverse(*Pred)) {
          if (MI.isCall()) {
