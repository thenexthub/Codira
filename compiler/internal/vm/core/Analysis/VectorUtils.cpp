...
bool toolchain::isTriviallyVectorizable(Intrinsic::ID ID) {
  switch (ID) {
...
bool toolchain::isTriviallyScalarizable(Intrinsic::ID ID,
...
bool toolchain::isVectorIntrinsicWithScalarOpAtArg(Intrinsic::ID ID,
...
bool toolchain::isVectorIntrinsicWithOverloadTypeAtArg(
...
bool toolchain::isVectorIntrinsicWithStructReturnOverloadAtField(
...
Intrinsic::ID toolchain::getVectorIntrinsicIDForCall(const CallInst *CI,
...
unsigned toolchain::getInterleaveIntrinsicFactor(Intrinsic::ID ID) {
  switch (ID) {
...
unsigned toolchain::getDeinterleaveIntrinsicFactor(Intrinsic::ID ID) {
  switch (ID) {
...
VectorType *toolchain::getDeinterleavedVectorType(IntrinsicInst *DI) {
...
/// from the vector.
Value *toolchain::findScalarElement(Value *V, unsigned EltNo) {
...
int toolchain::getSplatIndex(ArrayRef<int> Mask) {
  int SplatIndex = -1;
  for (int M : Mask) {
...
Value *toolchain::getSplatValue(const Value *V) {
...
bool toolchain::isSplatValue(const Value *V, int Index, unsigned Depth) {
...
bool toolchain::getShuffleDemandedElts(int SrcWidth, ArrayRef<int> Mask,
...
bool toolchain::isMaskedSlidePair(ArrayRef<int> Mask, int NumElts,
...
void toolchain::narrowShuffleMaskElts(int Scale, ArrayRef<int> Mask,
...
bool toolchain::widenShuffleMaskElts(int Scale, ArrayRef<int> Mask,
                                SmallVectorImpl<int> &ScaledMask) {
  assert(Scale > 0 && "Unexpected scaling factor");
...
bool toolchain::widenShuffleMaskElts(ArrayRef<int> M,
...
bool toolchain::scaleShuffleMaskElts(unsigned NumDstElts, ArrayRef<int> Mask,
                                SmallVectorImpl<int> &ScaledMask) {
...
void toolchain::getShuffleMaskWithWidestElts(ArrayRef<int> Mask,
                                        SmallVectorImpl<int> &ScaledMask) {
...
void toolchain::processShuffleMasks(
...
void toolchain::getHorizDemandedEltsForFirstOperand(unsigned VectorBitWidth,
...
toolchain::computeMinimumValueSizes(ArrayRef<BasicBlock *> Blocks, DemandedBits &DB,
                               const TargetTransformInfo *TTI) {
  ...
  for (const auto &E : ECs) {
    ...
    uint64_t MinBW = toolchain::bit_width(LeaderDemandedBits);
    // Round up to a power of 2
    MinBW = toolchain::bit_ceil(MinBW);
...
MDNode *toolchain::uniteAccessGroups(MDNode *AccGroups1, MDNode *AccGroups2) {
  if (!AccGroups1)
    return AccGroups2;
  if (!AccGroups2)
    return AccGroups1;
...
MDNode *toolchain::intersectAccessGroups(const Instruction *Inst1,
...
/// vectorization.
void toolchain::getMetadataToPropagate(
    Instruction *Inst,
...
Instruction *toolchain::propagateMetadata(Instruction *Inst, ArrayRef<Value *> VL) {
  if (VL.empty())
    return Inst;
...
Constant *
toolchain::createBitMaskForGaps(IRBuilderBase &Builder, unsigned VF,
...
toolchain::SmallVector<int, 16>
toolchain::createReplicatedMask(unsigned ReplicationFactor, unsigned VF) {
...
toolchain::SmallVector<int, 16> toolchain::createInterleaveMask(unsigned VF,
                                                      unsigned NumVecs) {
...
toolchain::SmallVector<int, 16>
toolchain::createStrideMask(unsigned Start, unsigned Stride, unsigned VF) {
  SmallVector<int, 16> Mask;
  for (unsigned i = 0; i < VF; i++)
...
toolchain::SmallVector<int, 16> toolchain::createSequentialMask(unsigned Start,
                                                      unsigned NumInts,
                                                      unsigned NumUndefs) {
  SmallVector<int, 16> Mask;
...
toolchain::SmallVector<int, 16> toolchain::createUnaryMask(ArrayRef<int> Mask,
                                                 unsigned NumElts) {
...
Value *toolchain::concatenateVectors(IRBuilderBase &Builder,
                                ArrayRef<Value *> Vecs) {
...
bool toolchain::maskIsAllZeroOrUndef(Value *Mask) {
...
bool toolchain::maskIsAllOneOrUndef(Value *Mask) {
  assert(isa<VectorType>(Mask->getType()) &&
         isa<IntegerType>(Mask->getType()->getScalarType()) &&
         cast<IntegerType>(Mask->getType()->getScalarType())->getBitWidth() ==
             1 &&
...
bool toolchain::maskContainsAllOneOrUndef(Value *Mask) {
  assert(isa<VectorType>(Mask->getType()) &&
         isa<IntegerType>(Mask->getType()->getScalarType()) &&
         cast<IntegerType>(Mask->getType()->getScalarType())->getBitWidth() ==
             1 &&
...
APInt toolchain::possiblyDemandedEltsInMask(Value *Mask) {
