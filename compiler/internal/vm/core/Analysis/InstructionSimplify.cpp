...
#include "vm/core/Analysis/CaptureTracking.h"
#include "vm/core/Analysis/CmpInstAnalysis.h"
...
#include "vm/core/Analysis/LoopAnalysisManager.h"
...
Value *toolchain::simplifyAddInst(Value *Op0, Value *Op1, bool IsNSW, bool IsNUW,
...
Value *toolchain::simplifySubInst(Value *Op0, Value *Op1, bool IsNSW, bool IsNUW,
                             const SimplifyQuery &Q) {
...
Value *toolchain::simplifyMulInst(Value *Op0, Value *Op1, bool IsNSW, bool IsNUW,
                             const SimplifyQuery &Q) {
...
Value *toolchain::simplifySDivInst(Value *Op0, Value *Op1, bool IsExact,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifyUDivInst(Value *Op0, Value *Op1, bool IsExact,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifySRemInst(Value *Op0, Value *Op1, const SimplifyQuery &Q) {
...
Value *toolchain::simplifyURemInst(Value *Op0, Value *Op1, const SimplifyQuery &Q) {
...
Value *toolchain::simplifyShlInst(Value *Op0, Value *Op1, bool IsNSW, bool IsNUW,
                             const SimplifyQuery &Q) {
...
Value *toolchain::simplifyLShrInst(Value *Op0, Value *Op1, bool IsExact,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifyAShrInst(Value *Op0, Value *Op1, bool IsExact,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifyAndInst(Value *Op0, Value *Op1, const SimplifyQuery &Q) {
...
Value *toolchain::simplifyOrInst(Value *Op0, Value *Op1, const SimplifyQuery &Q) {
...
Value *toolchain::simplifyXorInst(Value *Op0, Value *Op1, const SimplifyQuery &Q) {
...
static bool haveNonOverlappingStorage(const Value *V1, const Value *V2) {
  ...
  //
  // However, if there's an @toolchain.stackrestore dynamically in between two
  // allocas, they may have the same address. It's tempting to reduce the
  // scope of the problem by only looking at *static* allocas here. That would
  // cover the majority of allocas while significantly reducing the likelihood
  // of having an @toolchain.stackrestore pop up in the middle. However, it's not
  // actually impossible for an @toolchain.stackrestore to pop up in the middle of
  // an entry block. Also, if we have a block that's not attached to a
  // function, we can't tell if it's "static" under the current definition.
  // Theoretically, this problem could be fixed by creating a new kind of
  // instruction kind specifically for static allocas. Such a new instruction
  // could be required to be at the top of the entry block, thus preventing it
  // from being subject to a @toolchain.stackrestore. Instcombine could even
...
static Constant *computePointerICmp(CmpPredicate Pred, Value *LHS, Value *RHS,
  ...
  if (ICmpInst::isEquality(Pred)) {
    ...
    Value *MI = nullptr;
    if (isAllocLikeFn(LHS, TLI) && toolchain::isKnownNonZero(RHS, Q))
      MI = LHS;
    else if (isAllocLikeFn(RHS, TLI) && toolchain::isKnownNonZero(LHS, Q))
      MI = RHS;
    if (MI) {
...
static Value *simplifyICmpInst(CmpPredicate Pred, Value *LHS, Value *RHS,
  ...
  // Matches behavior in toolchain::ConstantFoldCompareInstruction.
...
Value *toolchain::simplifyICmpInst(CmpPredicate Predicate, Value *LHS, Value *RHS,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifyFCmpInst(CmpPredicate Predicate, Value *LHS, Value *RHS,
                              FastMathFlags FMF, const SimplifyQuery &Q) {
...
static Value *simplifyWithOpsReplaced(Value *V,
  ...
  // Don't fold away toolchain.is.constant checks based on assumptions.
...
Value *toolchain::simplifyWithOpReplaced(Value *V, Value *Op, Value *RepOp,
                                    const SimplifyQuery &Q,
                                    bool AllowRefinement,
                                    SmallVectorImpl<Instruction *> *DropFlags) {
...
Value *toolchain::simplifySelectInst(Value *Cond, Value *TrueVal, Value *FalseVal,
                                const SimplifyQuery &Q) {
...
Value *toolchain::simplifyGEPInst(Type *SrcTy, Value *Ptr, ArrayRef<Value *> Indices,
...
Value *toolchain::simplifyInsertValueInst(Value *Agg, Value *Val,
                                     ArrayRef<unsigned> Idxs,
                                     const SimplifyQuery &Q) {
  return ::simplifyInsertValueInst(Agg, Val, Idxs, Q, RecursionLimit);
}

Value *toolchain::simplifyInsertElementInst(Value *Vec, Value *Val, Value *Idx,
                                       const SimplifyQuery &Q) {
...
Value *toolchain::simplifyExtractValueInst(Value *Agg, ArrayRef<unsigned> Idxs,
                                      const SimplifyQuery &Q) {
...
Value *toolchain::simplifyExtractElementInst(Value *Vec, Value *Idx,
                                        const SimplifyQuery &Q) {
...
Value *toolchain::simplifyCastInst(unsigned CastOpc, Value *Op, Type *Ty,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifyShuffleVectorInst(Value *Op0, Value *Op1,
                                       ArrayRef<int> Mask, Type *RetTy,
                                       const SimplifyQuery &Q) {
...
Value *toolchain::simplifyFNegInst(Value *Op, FastMathFlags FMF,
                              const SimplifyQuery &Q) {
...
Value *toolchain::simplifyFAddInst(Value *Op0, Value *Op1, FastMathFlags FMF,
                              const SimplifyQuery &Q,
                              fp::ExceptionBehavior ExBehavior,
                              RoundingMode Rounding) {
...
Value *toolchain::simplifyFSubInst(Value *Op0, Value *Op1, FastMathFlags FMF,
                              const SimplifyQuery &Q,
                              fp::ExceptionBehavior ExBehavior,
                              RoundingMode Rounding) {
...
Value *toolchain::simplifyFMulInst(Value *Op0, Value *Op1, FastMathFlags FMF,
                              const SimplifyQuery &Q,
                              fp::ExceptionBehavior ExBehavior,
                              RoundingMode Rounding) {
...
Value *toolchain::simplifyFMAFMul(Value *Op0, Value *Op1, FastMathFlags FMF,
                             const SimplifyQuery &Q,
                             fp::ExceptionBehavior ExBehavior,
                             RoundingMode Rounding) {
...
Value *toolchain::simplifyFDivInst(Value *Op0, Value *Op1, FastMathFlags FMF,
                              const SimplifyQuery &Q,
                              fp::ExceptionBehavior ExBehavior,
                              RoundingMode Rounding) {
...
Value *toolchain::simplifyFRemInst(Value *Op0, Value *Op1, FastMathFlags FMF,
                              const SimplifyQuery &Q,
                              fp::ExceptionBehavior ExBehavior,
                              RoundingMode Rounding) {
...
Value *toolchain::simplifyUnOp(unsigned Opcode, Value *Op, const SimplifyQuery &Q) {
  return ::simplifyUnOp(Opcode, Op, Q, RecursionLimit);
}

Value *toolchain::simplifyUnOp(unsigned Opcode, Value *Op, FastMathFlags FMF,
                          const SimplifyQuery &Q) {
...
Value *toolchain::simplifyBinOp(unsigned Opcode, Value *LHS, Value *RHS,
                           const SimplifyQuery &Q) {
...
Value *toolchain::simplifyCmpInst(CmpPredicate Predicate, Value *LHS, Value *RHS,
                             const SimplifyQuery &Q) {
...
Value *toolchain::simplifyBinaryIntrinsic(Intrinsic::ID IID, Type *ReturnType,
                                     Value *Op0, Value *Op1,
                                     const SimplifyQuery &Q,
                                     const CallBase *Call) {
  ...
  case Intrinsic::ptrmask: {
    ...
    // toolchain.ptrmask intrinsic that will be lost when we just return the
  ...
  case Intrinsic::minimumnum: {
    ...
    // arguments unmodified instead of inserting an toolchain.canonicalize to
...
Value *toolchain::simplifyCall(CallBase *Call, Value *Callee, ArrayRef<Value *> Args,
                          const SimplifyQuery &Q) {
...
Value *toolchain::simplifyConstrainedFPCall(CallBase *Call, const SimplifyQuery &Q) {
...
static Value *simplifyFreezeInst(Value *Op0, const SimplifyQuery &Q) {
  ...
  if (toolchain::isGuaranteedNotToBeUndefOrPoison(Op0, Q.AC, Q.CxtI, Q.DT))
    return Op0;
  // We have room for improvement.
  return nullptr;
}

Value *toolchain::simplifyFreezeInst(Value *Op0, const SimplifyQuery &Q) {
  return ::simplifyFreezeInst(Op0, Q);
}

Value *toolchain::simplifyLoadInst(LoadInst *LI, Value *PtrOp,
                              const SimplifyQuery &Q) {
  if (LI->isVolatile())
    return nullptr;
...
static Value *simplifyInstructionWithOperands(Instruction *I,
  ...
  case Instruction::Freeze:
    return toolchain::simplifyFreezeInst(NewOps[0], Q);
...
Value *toolchain::simplifyInstructionWithOperands(Instruction *I,
                                             ArrayRef<Value *> NewOps,
                                             const SimplifyQuery &SQ) {
...
Value *toolchain::simplifyInstruction(Instruction *I, const SimplifyQuery &SQ) {
...
bool toolchain::replaceAndRecursivelySimplify(
    Instruction *I, Value *SimpleV, const TargetLibraryInfo *TLI,
    const DominatorTree *DT, AssumptionCache *AC,
