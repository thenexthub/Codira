...
bool LazyCallGraph::invalidate(Module &, const PreservedAnalyses &PA,
  ...
  auto PAC = PA.getChecker<toolchain::LazyCallGraphAnalysis>();
...
#ifdef EXPENSIVE_CHECKS
  verify();
  toolchain::scope_exit VerifyOnExit([&]() { verify(); });
#endif
...
LazyCallGraph::RefSCC::insertIncomingRefEdge(Node &SourceN, Node &TargetN) {
  ...
  SmallPtrSet<RefSCC *, 16> MergeSet(toolchain::from_range, MergeRange);
...
LazyCallGraph::RefSCC::removeInternalRefEdges(
  ...
  verify();
  toolchain::scope_exit VerifyOnExit([&]() {
  ...
  if (toolchain::all_of(Edges, [&](std::pair<Node *, Node *> E) {
  ...
  do {
    ...
    do {
      ...
      if (toolchain::size(RefSCCNodes) == NumRefSCCNodes) {
...
void LazyCallGraph::RefSCC::insertTrivialCallEdge(Node &SourceN,
                                                  Node &TargetN) {
#ifdef EXPENSIVE_CHECKS
  toolchain::scope_exit ExitVerifier([this] { verify(); });
...
#ifdef EXPENSIVE_CHECKS
  OriginalRC->verify();
  toolchain::scope_exit VerifyOnExit([&]() { OriginalRC->verify(); });
#endif
