...
StructType *ResourceTypeInfo::createElementStruct(StringRef CBufferName) {
  ...
  case ResourceKind::FeedbackTexture2DArray: {
    ...
    TypeName = formatv("{0}<{1}>", getResourceKindName(Kind),
                       toolchain::to_underlying(RTy->getFeedbackType()));
    return getOrCreateElementStruct(Type::getInt32Ty(HandleTy->getContext()),
                                    TypeName);
  }
  case ResourceKind::CBuffer: {
  ...
  case ResourceKind::Sampler: {
    ...
    TypeName = formatv("SamplerState<{0}>",
                       toolchain::to_underlying(RTy->getSamplerType()));
    return getOrCreateElementStruct(Type::getInt32Ty(HandleTy->getContext()),
                                    TypeName);
  }
  case ResourceKind::TBuffer:
...
MDTuple *ResourceInfo::getAsMetadata(Module &M,
  ...
  if (RTI.isCBuffer()) {
    ...
    MDVals.push_back(getIntMD(toolchain::to_underlying(RTI.getSamplerType())));
    MDVals.push_back(nullptr);
  } else {
    MDVals.push_back(getIntMD(toolchain::to_underlying(RTI.getResourceKind())));
    ...
    if (RTI.isStruct()) {
      Tags.push_back(
          getIntMD(toolchain::to_underlying(ExtPropTags::StructuredBufferStride)));
      Tags.push_back(getIntMD(RTI.getStruct(DL).Stride));
    } else if (RTI.isTyped()) {
      Tags.push_back(getIntMD(toolchain::to_underlying(ExtPropTags::ElementType)));
      Tags.push_back(
          getIntMD(toolchain::to_underlying(RTI.getTyped().DXILStorageTy)));
    } else if (RTI.isFeedback()) {
      Tags.push_back(
          getIntMD(toolchain::to_underlying(ExtPropTags::SamplerFeedbackKind)));
      Tags.push_back(getIntMD(toolchain::to_underlying(RTI.getFeedbackType())));
    }
...
ResourceInfo::getAnnotateProps(Module &M, dxil::ResourceTypeInfo &RTI) const {
  ...
  uint32_t ResourceKind = toolchain::to_underlying(RTI.getResourceKind());
  ...
  if (RTI.isStruct())
    ...
    Word1 = toolchain::to_underlying(RTI.getFeedbackType());
  else if (RTI.isTyped()) {
    ResourceTypeInfo::TypedInfo Typed = RTI.getTyped();
    uint32_t CompType = toolchain::to_underlying(Typed.ElementTy);
...
StringRef dxil::getResourceNameFromBindingCall(CallInst *CI) {
  ...
  auto *GV = dyn_cast<toolchain::GlobalVariable>(Op);
  if (!GV)
    return "";
...
void DXILResourceMap::populateResourceInfos(Module &M,
  ...
  toolchain::stable_sort(CIToInfos, [](auto &LHS, auto &RHS) {
...
void DXILResourceBindingInfo::populate(Module &M, DXILResourceTypeMap &DRTM) {
  ...
  // collect all of the toolchain.dx.resource.handlefrombinding calls;
  // make a note if there is toolchain.dx.resource.handlefromimplicitbinding
  for (Function &F : M.functions()) {
    if (!F.isDeclaration())
      continue;
...
ModulePass *toolchain::createDXILResourceTypeWrapperPassPass() {
...
ModulePass *toolchain::createDXILResourceWrapperPassPass() {
...
ModulePass *toolchain::createDXILResourceBindingWrapperPassPass() {
  return new DXILResourceWrapperPass();
}
