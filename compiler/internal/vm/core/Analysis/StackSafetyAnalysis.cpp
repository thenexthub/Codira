...
void StackSafetyDataFlowAnalysis<CalleeTy>::runDataFlow() {
  ...
  for (auto &F : Functions) {
    ...
    toolchain::sort(Callees);
    Callees.erase(toolchain::unique(Callees), Callees.end());
...
bool toolchain::needsParamAccessSummary(const Module &M) {
  if (StackSafetyRun)
    return true;
  for (const auto &F : M.functions())
...
void toolchain::generateParamAccessSummary(ModuleSummaryIndex &Index) {
