...
template class toolchain::DGEdge<DDGNode, DDGEdge>;
template class toolchain::DGNode<DDGNode, DDGEdge>;
template class toolchain::DirectedGraph<DDGNode, DDGEdge>;
...
bool DDGNode::collectInstructions(
    toolchain::function_ref<bool(Instruction *)> const &Pred,
  ...
  if (isa<SimpleDDGNode>(this)) {
    ...
    for (const DDGNode *PN : cast<const PiBlockDDGNode>(this)->getNodes()) {
      ...
      toolchain::append_range(IList, TmpIList);
    }
  } else
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const DDGNode::NodeKind K) {
  const char *Out;
  switch (K) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const DDGNode &N) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const DDGEdge::EdgeKind K) {
  const char *Out;
  switch (K) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const DDGEdge &E) {
...
raw_ostream &toolchain::operator<<(raw_ostream &OS, const DataDependenceGraph &G) {
  for (DDGNode *Node : G)
