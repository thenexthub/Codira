...
#include "vm/core/Analysis/IVDescriptors.h"
...
template class LLVM_EXPORT_TEMPLATE toolchain::LoopBase<BasicBlock, Loop>;
template class LLVM_EXPORT_TEMPLATE toolchain::LoopInfoBase<BasicBlock, Loop>;

// Always verify loopinfo if expensive checking is enabled.
#ifdef EXPENSIVE_CHECKS
bool toolchain::VerifyLoopInfo = true;
#else
bool toolchain::VerifyLoopInfo = false;
#endif
...
void Loop::setLoopAlreadyUnrolled() {
  ...
  MDNode *DisableUnrollMD =
      MDNode::get(Context, MDString::get(Context, "toolchain.loop.unroll.disable"));
  MDNode *LoopID = getLoopID();
  MDNode *NewLoopID = makePostTransformationMetadata(
      Context, LoopID, {"toolchain.loop.unroll."}, {DisableUnrollMD});
  setLoopID(NewLoopID);
}
...
void Loop::setLoopMustProgress() {
  ...
  MDNode *MustProgress = findOptionMDForLoop(this, "toolchain.loop.mustprogress");

  if (MustProgress)
    return;

  MDNode *MustProgressMD =
      MDNode::get(Context, MDString::get(Context, "toolchain.loop.mustprogress"));
  MDNode *LoopID = getLoopID();
  MDNode *NewLoopID =
...
bool Loop::isAnnotatedParallel() const {
  ...
  MDNode *ParallelAccesses =
      findOptionMDForLoop(this, "toolchain.loop.parallel_accesses");
  ...
  for (BasicBlock *BB : this->blocks()) {
    for (Instruction &I : *BB) {
      ...
      if (!toolchain::is_contained(LoopIdMD->operands(), DesiredLoopIdMetadata))
        return false;
    }
  }
  return true;
...
Loop::LocRange Loop::getLocRange() const {
  ...
  if (MDNode *LoopID = getLoopID()) {
    ...
    // of the loop.
    for (const MDOperand &MDO : toolchain::drop_begin(LoopID->operands())) {
...
void LoopInfo::erase(Loop *Unloop) {
  ...
  toolchain::scope_exit InvalidateOnExit([&]() { destroy(Unloop); });
...
void toolchain::printLoop(const Loop &L, raw_ostream &OS,
...
MDNode *toolchain::findOptionMDForLoopID(MDNode *LoopID, StringRef Name) {
...
MDNode *toolchain::findOptionMDForLoop(const Loop *TheLoop, StringRef Name) {
  return findOptionMDForLoopID(TheLoop->getLoopID(), Name);
}

/// Find string metadata for loop
///
/// If it has a value (e.g. {"toolchain.distribute", 1} return the value as an
/// operand or null otherwise.  If the string metadata is not found return
/// Optional's not-a-value.
std::optional<const MDOperand *>
toolchain::findStringMetadataForLoop(const Loop *TheLoop, StringRef Name) {
...
std::optional<bool> toolchain::getOptionalBoolLoopAttribute(const Loop *TheLoop,
                                                       StringRef Name) {
  MDNode *MD = findOptionMDForLoop(TheLoop, Name);
  if (!MD)
    return std::nullopt;
...
bool toolchain::getBooleanLoopAttribute(const Loop *TheLoop, StringRef Name) {
  return getOptionalBoolLoopAttribute(TheLoop, Name).value_or(false);
}

std::optional<int> toolchain::getOptionalIntLoopAttribute(const Loop *TheLoop,
                                                     StringRef Name) {
...
int toolchain::getIntLoopAttribute(const Loop *TheLoop, StringRef Name,
                              int Default) {
  return getOptionalIntLoopAttribute(TheLoop, Name).value_or(Default);
}

CallBase *toolchain::getLoopConvergenceHeart(const Loop *TheLoop) {
...
bool toolchain::isFinite(const Loop *L) {
  return L->getHeader()->getParent()->willReturn();
}

static const char *LLVMLoopMustProgress = "toolchain.loop.mustprogress";

bool toolchain::hasMustProgress(const Loop *L) {
  return getBooleanLoopAttribute(L, LLVMLoopMustProgress);
}

bool toolchain::isMustProgress(const Loop *L) {
  return L->getHeader()->getParent()->mustProgress() || hasMustProgress(L);
}

bool toolchain::isValidAsAccessGroup(MDNode *Node) {
  return Node->getNumOperands() == 0 && Node->isDistinct();
}

MDNode *toolchain::makePostTransformationMetadata(LLVMContext &Context,
                                             MDNode *OrigLoopID,
  ...
  // outdated.
  if (OrigLoopID) {
    for (const MDOperand &MDO : toolchain::drop_begin(OrigLoopID->operands())) {
      ...
      if (MDNode *MD = dyn_cast<MDNode>(Op)) {
        ...
        if (S)
          IsVectorMetadata =
              toolchain::any_of(RemovePrefixes, [S](StringRef Prefix) -> bool {
  ...
  // toolchain.loop.unroll.disable and toolchain.loop.isvectorized.
