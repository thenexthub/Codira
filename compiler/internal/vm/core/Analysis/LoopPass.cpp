...
#include "vm/core/IR/OptBisect.h"
...
public:
  ...
  bool runOnLoop(Loop *L, LPPassManager &) override {
    auto BBI = toolchain::find_if(L->blocks(), [](BasicBlock *BB) { return BB; });
    if (BBI != L->blocks().end() &&
...
bool LPPassManager::runOnFunction(Function &F) {
  ...
  while (!LQ.empty()) {
    ...
    for (unsigned Index = 0; Index < getNumContainedPasses(); ++Index) {
      ...
      toolchain::TimeTraceScope LoopPassScope("RunLoopPass", P->getPassName());
