...
bool toolchain::isNewLikeFn(const Value *V, const TargetLibraryInfo *TLI) {
  return getAllocationData(V, OpNewLike, TLI).has_value();
}

/// Tests if a value is a call or invoke to a library function that
/// allocates memory similar to malloc or calloc.
bool toolchain::isMallocOrCallocLikeFn(const Value *V, const TargetLibraryInfo *TLI) {
...
bool toolchain::isAllocLikeFn(const Value *V, const TargetLibraryInfo *TLI) {
...
bool toolchain::isReallocLikeFn(const Function *F) {
  return checkFnAllocKind(F, AllocFnKind::Realloc);
}

Value *toolchain::getReallocatedOperand(const CallBase *CB) {
  if (checkFnAllocKind(CB, AllocFnKind::Realloc))
    return CB->getArgOperandWithAttribute(Attribute::AllocatedPointer);
  return nullptr;
}

bool toolchain::isRemovableAlloc(const CallBase *CB, const TargetLibraryInfo *TLI) {
...
Value *toolchain::getAllocAlignment(const CallBase *V,
                               const TargetLibraryInfo *TLI) {
...
std::optional<APInt>
toolchain::getAllocSize(const CallBase *CB, const TargetLibraryInfo *TLI,
...
Constant *toolchain::getInitialValueOfAllocation(const Value *V,
                                            const TargetLibraryInfo *TLI,
                                            Type *Ty) {
  if (isa<AllocaInst>(V))
...
toolchain::getAllocationFamily(const Value *I, const TargetLibraryInfo *TLI) {
...
bool toolchain::isLibFreeFunction(const Function *F, const LibFunc TLIFn) {
...
Value *toolchain::getFreedOperand(const CallBase *CB, const TargetLibraryInfo *TLI) {
  if (const Function *Callee = getCalledFunction(CB)) {
    LibFunc TLIFn;
    if (TLI && TLI->getLibFunc(*Callee, TLIFn) && TLI->has(TLIFn) &&
...
bool toolchain::getObjectSize(const Value *Ptr, uint64_t &Size, const DataLayout &DL,
...
std::optional<TypeSize> toolchain::getBaseObjectSize(const Value *Ptr,
                                                const DataLayout &DL,
                                                const TargetLibraryInfo *TLI,
                                                ObjectSizeOpts Opts) {
...
Value *toolchain::lowerObjectSizeCall(IntrinsicInst *ObjectSize,
                                 const DataLayout &DL,
                                 const TargetLibraryInfo *TLI,
                                 bool MustSucceed) {
...
Value *toolchain::lowerObjectSizeCall(
    IntrinsicInst *ObjectSize, const DataLayout &DL,
    const TargetLibraryInfo *TLI, AAResults *AA, bool MustSucceed,
    SmallVectorImpl<Instruction *> *InsertedInstructions) {
  assert(ObjectSize->getIntrinsicID() == Intrinsic::objectsize &&
         "ObjectSize must be a call to toolchain.objectsize!");
