...
ModuleToPostOrderCGSCCPassAdaptor::run(Module &M, ModuleAnalysisManager &AM) {
  ...
  for (LazyCallGraph::RefSCC &RC :
       toolchain::make_early_inc_range(CG.postorder_ref_sccs())) {
    ...
    do {
      ...
      for (LazyCallGraph::SCC &C : toolchain::reverse(*RC))
        CWorklist.insert(&C);
...
PreservedAnalyses DevirtSCCRepeatedPass::run(LazyCallGraph::SCC &InitialC,
  ...
  for (int Iteration = 0;; ++Iteration) {
    ...
    bool Devirt = toolchain::any_of(UR.IndirectVHs, [](auto &P) -> bool {
      if (P.second) {
...
PreservedAnalyses CGSCCToFunctionPassAdaptor::run(LazyCallGraph::SCC &C,
  ...
  SmallVector<LazyCallGraph::Node *, 4> Nodes(toolchain::make_pointer_range(C));
...
incorporateNewSCCRange(const SCCRangeT &NewSCCRange, LazyCallGraph &G,
  ...
  for (SCC &NewC : toolchain::reverse(toolchain::drop_begin(NewSCCRange))) {
...
static LazyCallGraph::SCC &updateCGAndAnalysisManagerForPass(
  ...
  toolchain::erase_if(DeadTargets, [&](Node *TargetN) {
  ...
  for (Node *CallTarget : PromotedRefTargets) {
    ...
    if (InitialSCCIndex < NewSCCIndex) {
      ...
      for (SCC &MovedC : toolchain::reverse(make_range(RC->begin() + InitialSCCIndex,
...
LazyCallGraph::SCC &toolchain::updateCGAndAnalysisManagerForFunctionPass(
    LazyCallGraph &G, LazyCallGraph::SCC &InitialC, LazyCallGraph::Node &N,
    CGSCCAnalysisManager &AM, CGSCCUpdateResult &UR,
    FunctionAnalysisManager &FAM) {
...
LazyCallGraph::SCC &toolchain::updateCGAndAnalysisManagerForCGSCCPass(
    LazyCallGraph &G, LazyCallGraph::SCC &InitialC, LazyCallGraph::Node &N,
    CGSCCAnalysisManager &AM, CGSCCUpdateResult &UR,
    FunctionAnalysisManager &FAM) {
  return updateCGAndAnalysisManagerForPass(G, InitialC, N, AM, UR, FAM,
