...
#include "vm/core/Analysis/DomConditionCache.h"
...
#include "vm/core/Support/KnownFPClass.h"
...
static bool getShuffleDemandedElts(const ShuffleVectorInst *Shuf,
  ...
  return toolchain::getShuffleDemandedElts(NumElts, Shuf->getShuffleMask(),
...
void toolchain::computeKnownBits(const Value *V, KnownBits &Known,
...
KnownBits toolchain::computeKnownBits(const Value *V, const DataLayout &DL,
...
KnownBits toolchain::computeKnownBits(const Value *V, const APInt &DemandedElts,
                                 const DataLayout &DL, AssumptionCache *AC,
...
bool toolchain::haveNoCommonBitsSet(const WithCache<const Value *> &LHSCache,
...
bool toolchain::isOnlyUsedInZeroComparison(const Instruction *I) {
  return !I->user_empty() &&
         all_of(I->users(), match_fn(m_ICmp(m_Value(), m_Zero())));
}

bool toolchain::isOnlyUsedInZeroEqualityComparison(const Instruction *I) {
...
bool toolchain::isKnownToBeAPowerOfTwo(const Value *V, const DataLayout &DL,
...
bool toolchain::isKnownNonNegative(const Value *V, const SimplifyQuery &SQ,
                              unsigned Depth) {
  return computeKnownBits(V, SQ, Depth).isNonNegative();
}

bool toolchain::isKnownPositive(const Value *V, const SimplifyQuery &SQ,
                           unsigned Depth) {
...
bool toolchain::isKnownNegative(const Value *V, const SimplifyQuery &SQ,
                           unsigned Depth) {
...
bool toolchain::isKnownNonEqual(const Value *V1, const Value *V2,
                           const SimplifyQuery &Q, unsigned Depth) {
...
bool toolchain::MaskedValueIsZero(const Value *V, const APInt &Mask,
...
unsigned toolchain::ComputeNumSignBits(const Value *V, const DataLayout &DL,
                                  AssumptionCache *AC, const Instruction *CxtI,
                                  const DominatorTree *DT, bool UseInstrInfo,
                                  unsigned Depth) {
...
unsigned toolchain::ComputeMaxSignificantBits(const Value *V, const DataLayout &DL,
                                         AssumptionCache *AC,
                                         const Instruction *CxtI,
...
void toolchain::computeKnownBitsFromRangeMetadata(const MDNode &Ranges,
                                             KnownBits &Known) {
...
bool toolchain::isAssumeLikeIntrinsic(const Instruction *I) {
...
bool toolchain::isValidAssumeForContext(const Instruction *Inv,
                                   const Instruction *CxtI,
                                   const DominatorTree *DT,
...
bool toolchain::willNotFreeBetween(const Instruction *Assume,
...
void toolchain::computeKnownBitsFromContext(const Value *V, KnownBits &Known,
                                       const SimplifyQuery &Q, unsigned Depth) {
...
KnownBits toolchain::analyzeKnownBitsFromAndXorOr(const Operator *I,
...
ConstantRange toolchain::getVScaleRange(const Function *F, unsigned BitWidth) {
  ...
  case Instruction::GetElementPtr: {
    ...
    for (unsigned i = 1, e = I->getNumOperands(); i != e; ++i, ++GTI) {
      ...
      if (Stride.isScalable()) {
        ...
        ScalingFactor.Zero.setLowBits(toolchain::countr_zero(StrideInBytes));
      } else {
        ScalingFactor =
  ...
  case Instruction::Invoke: {
    ...
    if (const IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
      ...
      case Intrinsic::ctlz: {
        ...
        unsigned LowBits = toolchain::bit_width(PossibleLZ);
      ...
      case Intrinsic::cttz: {
        ...
        unsigned LowBits = toolchain::bit_width(PossibleTZ);
        Known.Zero.setBitsFrom(LowBits);
        break;
      }
      case Intrinsic::ctpop: {
        ...
        unsigned LowBits = toolchain::bit_width(BitsPossiblySet);
        Known.Zero.setBitsFrom(LowBits);
...
/// Determine which bits of V are known to be either zero or one and return
/// them.
KnownBits toolchain::computeKnownBits(const Value *V, const SimplifyQuery &Q,
                                 unsigned Depth) {
  KnownBits Known(getBitWidth(V->getType(), Q.DL));
...
bool toolchain::isKnownToBeAPowerOfTwo(const Value *V, bool OrZero,
                                  const SimplifyQuery &Q, unsigned Depth) {
  assert(Depth <= MaxAnalysisRecursionDepth && "Limit Search Depth");
  ...
  case Instruction::PHI: {
    ...
    return toolchain::all_of(PN->operands(), [&](const Use &U) {
...
bool toolchain::isKnownNonZero(const Value *V, const SimplifyQuery &Q,
                          unsigned Depth) {
  auto *FVTy = dyn_cast<FixedVectorType>(V->getType());
  APInt DemandedElts =
      FVTy ? APInt::getAllOnes(FVTy->getNumElements()) : APInt(1, 1);
...
getInvertibleOperands(const Operator *Op1,
  ...
  case Instruction::Mul: {
    ...
    // alive2: https://alive2.toolchain.org/ce/z/Z6D5qK
...
Intrinsic::ID toolchain::getIntrinsicForCallSite(const CallBase &CB,
...
bool toolchain::isSignBitCheck(ICmpInst::Predicate Pred, const APInt &RHS,
                          bool &TrueIfSigned) {
  switch (Pred) {
...
void toolchain::adjustKnownFPClassForSelectArm(KnownFPClass &Known, Value *Cond,
                                          Value *Arm, bool Invert,
                                          const SimplifyQuery &SQ,
                                          unsigned Depth) {
...
void computeKnownFPClass(const Value *V, const APInt &DemandedElts,
  ...
  toolchain::scope_exit ClearClassesFromFlags([=, &Known] {
...
KnownFPClass toolchain::computeKnownFPClass(const Value *V,
                                       const APInt &DemandedElts,
                                       FPClassTest InterestedClasses,
                                       const SimplifyQuery &SQ,
                                       unsigned Depth) {
...
KnownFPClass toolchain::computeKnownFPClass(
...
KnownFPClass
toolchain::computeKnownFPClass(const Value *V, const APInt &DemandedElts,
...
KnownFPClass toolchain::computeKnownFPClass(const Value *V, FastMathFlags FMF,
                                       FPClassTest InterestedClasses,
                                       const SimplifyQuery &SQ,
                                       unsigned Depth) {
  auto *FVTy = dyn_cast<FixedVectorType>(V->getType());
...
bool toolchain::cannotBeNegativeZero(const Value *V, const SimplifyQuery &SQ,
                                unsigned Depth) {
  KnownFPClass Known = computeKnownFPClass(V, fcNegZero, SQ, Depth);
  return Known.isKnownNeverNegZero();
}

bool toolchain::cannotBeOrderedLessThanZero(const Value *V, const SimplifyQuery &SQ,
                                       unsigned Depth) {
  KnownFPClass Known =
...
bool toolchain::isKnownNeverInfinity(const Value *V, const SimplifyQuery &SQ,
                                unsigned Depth) {
...
bool toolchain::isKnownNeverInfOrNaN(const Value *V, const SimplifyQuery &SQ,
                                unsigned Depth) {
...
/// could ever be NaN.
bool toolchain::isKnownNeverNaN(const Value *V, const SimplifyQuery &SQ,
                           unsigned Depth) {
...
std::optional<bool> toolchain::computeKnownFPSignBit(const Value *V,
                                                const SimplifyQuery &SQ,
                                                unsigned Depth) {
...
bool toolchain::canIgnoreSignBitOfZero(const Use &U) {
...
bool toolchain::canIgnoreSignBitOfNaN(const Use &U) {
  auto *User = cast<Instruction>(U.getUser());
  if (auto *FPOp = dyn_cast<FPMathOperator>(User)) {
    if (FPOp->hasNoNaNs())
      return true;
...
Value *toolchain::isBytewiseValue(Value *V, const DataLayout &DL) {
...
Value *
toolchain::FindInsertedValue(Value *V, ArrayRef<unsigned> idx_range,
...
bool toolchain::getConstantDataArrayInfo(const Value *V,
...
bool toolchain::getConstantStringInfo(const Value *V, StringRef &Str,
                                 bool TrimAtNul) {
  ConstantDataArraySlice Slice;
...
/// If we can compute the length of the string pointed to by
/// the specified pointer, return 'len+1'.  If we can't, return 0.
uint64_t toolchain::GetStringLength(const Value *V, unsigned CharSize) {
  if (!V->getType()->isPointerTy())
    return 0;
...
const Value *
toolchain::getArgumentAliasingToReturnedPointer(const CallBase *Call,
...
bool toolchain::isIntrinsicReturningPointerAliasingArgumentWithoutCapturing(
...
const Value *toolchain::getUnderlyingObject(const Value *V, unsigned MaxLookup) {
...
void toolchain::getUnderlyingObjects(const Value *V,
...
const Value *toolchain::getUnderlyingObjectAggressive(const Value *V) {
...
bool toolchain::getUnderlyingObjectsForCodeGen(const Value *V,
...
AllocaInst *toolchain::findAllocaForValue(Value *V, bool OffsetZero) {
...
bool toolchain::onlyUsedByLifetimeMarkers(const Value *V) {
  return onlyUsedByLifetimeMarkersOrDroppableInstsHelper(
      V, /* AllowLifetime */ true, /* AllowDroppable */ false);
}
bool toolchain::onlyUsedByLifetimeMarkersOrDroppableInsts(const Value *V) {
  return onlyUsedByLifetimeMarkersOrDroppableInstsHelper(
      V, /* AllowLifetime */ true, /* AllowDroppable */ true);
}

bool toolchain::isNotCrossLaneOperation(const Instruction *I) {
  if (auto *II = dyn_cast<IntrinsicInst>(I))
...
bool toolchain::isSafeToSpeculativelyExecute(
...
bool toolchain::isSafeToSpeculativelyExecuteWithOpcode(
...
bool toolchain::mayHaveNonDefUseDependency(const Instruction &I) {
...
ConstantRange
toolchain::computeConstantRangeIncludingKnownBits(const WithCache<const Value *> &V,
                                             bool ForSigned,
                                             const SimplifyQuery &SQ) {
  ConstantRange CR1 =
...
OverflowResult toolchain::computeOverflowForUnsignedMul(const Value *LHS,
                                                   const Value *RHS,
                                                   const SimplifyQuery &SQ,
                                                   bool IsNSW) {
...
OverflowResult toolchain::computeOverflowForSignedMul(const Value *LHS,
                                                 const Value *RHS,
                                                 const SimplifyQuery &SQ) {
...
OverflowResult
toolchain::computeOverflowForUnsignedAdd(const WithCache<const Value *> &LHS,
...
OverflowResult toolchain::computeOverflowForUnsignedSub(const Value *LHS,
                                                   const Value *RHS,
                                                   const SimplifyQuery &SQ) {
  // X - (X % ?)
...
OverflowResult toolchain::computeOverflowForSignedSub(const Value *LHS,
                                                 const Value *RHS,
                                                 const SimplifyQuery &SQ) {
  // X - (X % ?)
  // The remainder of a value can't have greater magnitude than itself,
...
bool toolchain::isOverflowIntrinsicNoWrap(const WithOverflowInst *WO,
                                     const DominatorTree &DT) {
  ...
  return toolchain::any_of(GuardingBranches, AllUsesGuardedByBranch);
}
...
static bool shiftAmountKnownInRange(const Value *ShiftAmount) {
  ...
  bool Safe = toolchain::all_of(ShiftAmounts, [](const Constant *C) {
...
bool toolchain::canCreateUndefOrPoison(const Operator *Op,
                                  bool ConsiderFlagsAndMetadata) {
  return ::canCreateUndefOrPoison(Op, UndefPoisonKind::UndefOrPoison,
                                  ConsiderFlagsAndMetadata);
}

bool toolchain::canCreatePoison(const Operator *Op, bool ConsiderFlagsAndMetadata) {
  ...
  if (const auto *I = dyn_cast<Instruction>(V)) {
    ...
    if (match(I, m_ExtractValue(m_WithOverflowInst(II))) &&
        (match(ValAssumedPoison, m_ExtractValue(m_Specific(II))) ||
         toolchain::is_contained(II->args(), ValAssumedPoison)))
      return true;
  }
  return false;
}
...
bool toolchain::impliesPoison(const Value *ValAssumedPoison, const Value *V) {
...
bool toolchain::isGuaranteedNotToBeUndefOrPoison(const Value *V, AssumptionCache *AC,
                                            const Instruction *CtxI,
                                            const DominatorTree *DT,
                                            unsigned Depth) {
...
bool toolchain::isGuaranteedNotToBePoison(const Value *V, AssumptionCache *AC,
                                     const Instruction *CtxI,
...
bool toolchain::isGuaranteedNotToBeUndef(const Value *V, AssumptionCache *AC,
                                    const Instruction *CtxI,
                                    const DominatorTree *DT, unsigned Depth) {
  return ::isGuaranteedNotToBeUndefOrPoison(V, AC, CtxI, DT, Depth,
...
bool toolchain::mustExecuteUBIfPoisonOnPathTo(Instruction *Root,
...
OverflowResult toolchain::computeOverflowForSignedAdd(const AddOperator *Add,
                                                 const SimplifyQuery &SQ) {
...
OverflowResult
toolchain::computeOverflowForSignedAdd(const WithCache<const Value *> &LHS,
                                  const WithCache<const Value *> &RHS,
                                  const SimplifyQuery &SQ) {
  return ::computeOverflowForSignedAdd(LHS, RHS, nullptr, SQ);
}

bool toolchain::isGuaranteedToTransferExecutionToSuccessor(const Instruction *I) {
...
bool toolchain::isGuaranteedToTransferExecutionToSuccessor(const BasicBlock *BB) {
...
bool toolchain::isGuaranteedToTransferExecutionToSuccessor(
...
bool toolchain::isGuaranteedToExecuteForEveryIteration(const Instruction *I,
                                                  const Loop *L) {
...
bool toolchain::intrinsicPropagatesPoison(Intrinsic::ID IID) {
  switch (IID) {
  // TODO: Add more intrinsics.
  case Intrinsic::sadd_with_overflow:
  case Intrinsic::ssub_with_overflow:
...
bool toolchain::propagatesPoison(const Use &PoisonOp) {
...
bool toolchain::mustTriggerUB(const Instruction *I,
...
bool toolchain::programUndefinedIfUndefOrPoison(const Instruction *Inst) {
  return ::programUndefinedIfUndefOrPoison(Inst, false);
}

bool toolchain::programUndefinedIfPoison(const Instruction *Inst) {
...
bool toolchain::isKnownNegation(const Value *X, const Value *Y, bool NeedNSW,
                           bool AllowPoison) {
...
bool toolchain::isKnownInversion(const Value *X, const Value *Y) {
...
SelectPatternResult toolchain::getSelectPattern(CmpInst::Predicate Pred,
...
toolchain::getFlippedStrictnessPredicateAndConstant(CmpPredicate Pred, Constant *C) {
...
SelectPatternResult toolchain::matchSelectPattern(Value *V, Value *&LHS, Value *&RHS,
                                             Instruction::CastOps *CastOp,
                                             unsigned Depth) {
  if (Depth >= MaxAnalysisRecursionDepth)
    return {SPF_UNKNOWN, SPNB_NA, false};
  ...
  return toolchain::matchDecomposedSelectPattern(
      CmpI, TrueVal, FalseVal, LHS, RHS,
      isa<FPMathOperator>(SI) ? SI->getFastMathFlags() : FastMathFlags(),
      CastOp, Depth);
}

SelectPatternResult toolchain::matchDecomposedSelectPattern(
...
CmpInst::Predicate toolchain::getMinMaxPred(SelectPatternFlavor SPF, bool Ordered) {
...
Intrinsic::ID toolchain::getMinMaxIntrinsic(SelectPatternFlavor SPF) {
  switch (SPF) {
...
SelectPatternFlavor toolchain::getInverseMinMaxFlavor(SelectPatternFlavor SPF) {
...
Intrinsic::ID toolchain::getInverseMinMaxIntrinsic(Intrinsic::ID MinMaxID) {
  switch (MinMaxID) {
...
APInt toolchain::getMinMaxLimit(SelectPatternFlavor SPF, unsigned BitWidth) {
  switch (SPF) {
...
toolchain::canConvertToMinOrMaxIntrinsic(ArrayRef<Value *> VL) {
...
bool toolchain::matchSimpleRecurrence(const PHINode *P, BinaryOperator *&BO,
...
bool toolchain::matchSimpleRecurrence(const BinaryOperator *I, PHINode *&P,
                                 Value *&Start, Value *&Step) {
  BinaryOperator *BO = nullptr;
...
bool toolchain::matchSimpleBinaryIntrinsicRecurrence(const IntrinsicInst *I,
...
std::optional<bool>
toolchain::isImpliedCondition(const Value *LHS, CmpPredicate RHSPred,
                         const Value *RHSOp0, const Value *RHSOp1,
                         const DataLayout &DL, bool LHSIsTrue, unsigned Depth) {
...
std::optional<bool> toolchain::isImpliedCondition(const Value *LHS, const Value *RHS,
                                             const DataLayout &DL,
...
std::optional<bool> toolchain::isImpliedByDomCondition(const Value *Cond,
...
std::optional<bool> toolchain::isImpliedByDomCondition(CmpPredicate Pred,
                                                  const Value *LHS,
                                                  const Value *RHS,
                                                  const Instruction *ContextI,
                                                  const DataLayout &DL) {
...
ConstantRange toolchain::computeConstantRange(const Value *V, bool ForSigned,
...
void toolchain::findValuesAffectedByCondition(
...
const Value *toolchain::stripNullTest(const Value *V) {
...
Value *toolchain::stripNullTest(Value *V) {
  return const_cast<Value *>(stripNullTest(const_cast<const Value *>(V)));
}

bool toolchain::collectPossibleValues(const Value *V,
