...
// (https://clang.toolchain.org/docs/AttributeReference.html#section-declspec-allocate)
// or #pragma clang section directives
// (https://clang.toolchain.org/docs/LanguageExtensions.html#specifying-section-names-for-global-objects-pragma-clang-section).
...
AnnotationKind getAnnotationKind(const GlobalVariable &GV) {
  ...
  // Skip 'toolchain.'-prefixed global variables conservatively because they are
...
void StaticDataProfileInfo::addConstantProfileCount(
  ...
  OriginalCount = toolchain::SaturatingAdd(*Count, OriginalCount);
...
StringRef StaticDataProfileInfo::getConstantSectionPrefix(
  ...
  if (EnableDataAccessProf) {
    ...
    if (const GlobalVariable *GV = dyn_cast<GlobalVariable>(C);
        GV && toolchain::memprof::IsAnnotationOK(*GV) &&
