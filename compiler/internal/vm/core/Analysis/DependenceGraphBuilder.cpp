...
#include "vm/core/Analysis/DependenceGraphBuilder.h"
...
template <class G> void AbstractDependenceGraphBuilder<G>::createPiBlocks() {
  ...
  for (NodeListType &NL : ListOfSCCs) {
    ...
    toolchain::sort(NL, [&](NodeType *LHS, NodeType *RHS) {
    ...
    // are inside the SCC.
    SmallPtrSet<NodeType *, 4> NodesInSCC(toolchain::from_range, NL);
...
void AbstractDependenceGraphBuilder<G>::sortNodesTopologically() {
  ...
  for (NodeType *N : post_order(&Graph)) {
    if (N->getKind() == NodeKind::PiBlock) {
      ...
      toolchain::append_range(NodesInPO, PiBlockMembers);
    }
...
template class toolchain::AbstractDependenceGraphBuilder<DataDependenceGraph>;
template class toolchain::DependenceGraphInfo<DDGNode>;
