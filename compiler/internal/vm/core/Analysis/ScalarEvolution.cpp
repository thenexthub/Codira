...
bool toolchain::VerifySCEV = true;
#else
bool toolchain::VerifySCEV = false;
#endif
...
void SCEV::print(raw_ostream &OS) const {
  ...
  case scSequentialUMinExpr: {
    ...
    OS << "("
       << toolchain::interleaved(toolchain::make_pointee_range(NAry->operands()), OpStr)
       << ")";
    switch (NAry->getSCEVType()) {
    case scAddExpr:
    case scMulExpr:
...
static void GroupByComplexity(SmallVectorImpl<const SCEV *> &Ops,
  ...
  toolchain::stable_sort(Ops, [&](const SCEV *LHS, const SCEV *RHS) {
...
ScalarEvolution::getOrCreateAddExpr(ArrayRef<const SCEV *> Ops,
  ...
  if (!S) {
    ...
    toolchain::uninitialized_copy(Ops, O);
    S = new (SCEVAllocator)
...
ScalarEvolution::getAddRecExpr(SmallVectorImpl<const SCEV *> &Operands,
  ...
  for (const SCEV *Op : toolchain::drop_begin(Operands)) {
...
const SCEV *ScalarEvolution::getMinMaxExpr(SCEVTypes Kind,
  ...
  // be adjacent.
  toolchain::CmpInst::Predicate GEPred =
      IsSigned ? ICmpInst::ICMP_SGE : ICmpInst::ICMP_UGE;
  toolchain::CmpInst::Predicate LEPred =
      IsSigned ? ICmpInst::ICMP_SLE : ICmpInst::ICMP_ULE;
  toolchain::CmpInst::Predicate FirstPred = IsMax ? GEPred : LEPred;
  toolchain::CmpInst::Predicate SecondPred = IsMax ? LEPred : GEPred;
...
static bool impliesPoison(const SCEV *AssumedPoison, const SCEV *S) {
  ...
  return toolchain::set_is_subset(PC1.MaybePoison, PC2.MaybePoison);
}
...
bool ScalarEvolution::canReuseInstruction(
  ...
  while (!Worklist.empty()) {
    ...
    toolchain::append_range(Worklist, I->operands());
  }
  return true;
}
  ...
  case scUnknown: {
    ...
    if (U->getType()->isPointerTy() && SignHint == HINT_RANGE_UNSIGNED) {
      ...
      if (DerefBytes > 1 && isUIntN(BitWidth, DerefBytes)) {
        ...
        if (toolchain::isKnownNonZero(V, DL))
          MinVal = Align;
        ConservativeResult = ConservativeResult.intersectWith(
...
ScalarEvolution::getOperandsToCreate(Value *V, SmallVectorImpl<Value *> &Ops) {
  ...
  case Instruction::GetElementPtr:
    assert(cast<GEPOperator>(U)->getSourceElementType()->isSized() &&
           "GEP source element type must be sized");
    toolchain::append_range(Ops, U->operands());
    return nullptr;
...
const SCEV *ScalarEvolution::createSCEV(Value *V) {
  ...
  case Instruction::Invoke:
    ...
    if (auto *II = dyn_cast<IntrinsicInst>(U)) {
      ...
      case Intrinsic::annotation:
      case Intrinsic::ptr_annotation:
        // A start_loop_iterations or toolchain.annotation or toolchain.prt.annotation is
...
void ScalarEvolution::forgetAllLoops() {
  ...
  // - Every toolchain::Value has been updated in place to produce a different
  // result.
...
void ScalarEvolution::forgetLoop(const Loop *L) {
  ...
  while (!LoopWorklist.empty()) {
    ...
    if (LoopUsersItr != LoopUsers.end())
      toolchain::append_range(ToForget, LoopUsersItr->second);
...
bool ScalarEvolution::isKnownViaInduction(CmpPredicate Pred, const SCEV *LHS,
  ...
  const Loop *MDL =
      *toolchain::max_element(LoopsUsed, [&](const Loop *L1, const Loop *L2) {
...
bool ScalarEvolution::isLoopBackedgeGuardedByCond(const Loop *L,
  ...
  // Check conditions due to any @toolchain.assume intrinsics.
...
bool ScalarEvolution::isBasicBlockEntryGuardedByCond(const BasicBlock *BB,
  ...
  // Check conditions due to any @toolchain.experimental.guard intrinsics.
...
bool ScalarEvolution::isImpliedCond(CmpPredicate Pred, const SCEV *LHS,
  ...
  toolchain::scope_exit ClearOnExit(
...
ScalarEvolution::ScalarEvolution(Function &F, TargetLibraryInfo &TLI,
  ...
  // @toolchain.experimental.guard, so do a quick check and remember this beforehand.
  //
...
namespace vm::core {
// Note: these overloaded operators need to be in the toolchain namespace for them
// to be resolved correctly. If we put them outside the toolchain namespace, the
//
...
static raw_ostream &operator<<(raw_ostream &OS,
                               toolchain::ScalarEvolution::BlockDisposition BD) {
  switch (BD) {
...
ScalarEvolution::getLoopDisposition(const SCEV *S, const Loop *L) {
  ...
  for (auto &V : toolchain::reverse(Values2)) {
    if (V.getPointer() == L) {
      V.setInt(D);
      break;
    }
...
void ScalarEvolution::forgetMemoizedResults(ArrayRef<const SCEV *> SCEVs) {
  SmallPtrSet<const SCEV *, 8> ToForget(toolchain::from_range, SCEVs);
...
void ScalarEvolution::forgetMemoizedResultsImpl(const SCEV *S) {
  ...
  if (ScopeIt != ValuesAtScopes.end()) {
    for (const auto &Pair : ScopeIt->second)
      if (!isa_and_nonnull<SCEVConstant>(Pair.second))
        toolchain::erase(ValuesAtScopesUsers[Pair.second],
  ...
  if (ScopeUserIt != ValuesAtScopesUsers.end()) {
    for (const auto &Pair : ScopeUserIt->second)
      toolchain::erase(ValuesAtScopes[Pair.second], std::make_pair(Pair.first, S));
...
void ScalarEvolution::verify() const {
  ...
  while (!LoopStack.empty()) {
    ...
    toolchain::append_range(LoopStack, *L);
...
void ScalarEvolution::LoopGuards::collectFromBlock(
  ...
  // Second, collect information from toolchain.experimental.guards dominating the loop.
  auto *GuardDecl = Intrinsic::getDeclarationIfExists(
