...
void toolchain::FindFunctionBackedges(const Function &F,
...
/// successor.
unsigned toolchain::GetSuccessorNumber(const BasicBlock *BB,
...
bool toolchain::isCriticalEdge(const Instruction *TI, unsigned SuccNum,
                          bool AllowIdenticalEdges) {
  assert(SuccNum < TI->getNumSuccessors() && "Illegal edge specification!");
  return isCriticalEdge(TI, TI->getSuccessor(SuccNum), AllowIdenticalEdges);
}

bool toolchain::isCriticalEdge(const Instruction *TI, const BasicBlock *Dest,
                          bool AllowIdenticalEdges) {
...
static bool isReachableImpl(SmallVectorImpl<BasicBlock *> &Worklist,
  ...
  do {
    ...
    if (ExclusionSet && ExclusionSet->count(BB))
      continue;
    if (DT) {
      if (toolchain::any_of(StopSet, [&](const BasicBlock *StopBB) {
...
bool toolchain::isPotentiallyReachableFromMany(
...
bool toolchain::isManyPotentiallyReachableFromMany(
...
bool toolchain::isPotentiallyReachable(
...
bool toolchain::canReturn(const Function &F) {
...
bool toolchain::isPresplitCoroSuspendExitEdge(const BasicBlock &Src,
