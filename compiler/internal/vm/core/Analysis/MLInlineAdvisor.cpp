...
//
//===----------------------------------------------------------------------===//
#include "vm/core/Analysis/MLInlineAdvisor.h"
...
#include "vm/core/Analysis/InlineModelFeatureMaps.h"
...
using CompiledModelType = toolchain::InlinerSizeModel;
#else
using CompiledModelType = NoopSavedModelImpl;
#endif

std::unique_ptr<InlineAdvisor>
toolchain::getReleaseModeAdvisor(Module &M, ModuleAnalysisManager &MAM,
                            std::function<bool(CallBase &)> GetDefaultAdvice) {
  if (!toolchain::isEmbeddedModelEvaluatorValid<CompiledModelType>() &&
...
const char *const toolchain::DecisionName = "inlining_decision";
const TensorSpec toolchain::InlineDecisionSpec =
    TensorSpec::createSpec<int64_t>(DecisionName, {1});
const char *const toolchain::DefaultDecisionName = "inlining_default";
const TensorSpec toolchain::DefaultDecisionSpec =
    TensorSpec::createSpec<int64_t>(DefaultDecisionName, {1});
const char *const toolchain::RewardName = "delta_size";
...
std::unique_ptr<InlineAdvice> MLInlineAdvisor::getAdviceImpl(CallBase &CB) {
  ...
  if (!Mandatory) {
    auto IsCallSiteInlinable =
        toolchain::getInliningCostEstimate(CB, TIR, GetAssumptionCache);
  ...
  const auto CostFeatures =
      toolchain::getInliningCostFeatures(CB, TIR, GetAssumptionCache);
  if (!CostFeatures) {
    return std::make_unique<InlineAdvice>(this, CB, ORE, false);
  }
  ...
  *ModelRunner->getTensor<int64_t>(FeatureIndex::is_caller_avail_external) =
  ...
  if (UseIR2Vec) {
    ...
    auto setEmbedding = [&](const ir2vec::Embedding &Embedding,
                            FeatureIndex Index) {
      toolchain::transform(Embedding, ModelRunner->getTensor<float>(Index),
