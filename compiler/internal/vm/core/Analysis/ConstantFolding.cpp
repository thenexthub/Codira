...
#include "vm/core/IR/ConstantFold.h"
...
bool toolchain::IsConstantOffsetFromGlobal(Constant *C, GlobalValue *&GV,
...
Constant *toolchain::ConstantFoldLoadThroughBitcast(Constant *C, Type *DestTy,
                                               const DataLayout &DL) {
  do {
...
// return null.
Constant *toolchain::ReadByteArrayFromGlobal(const GlobalVariable *GV,
                                        uint64_t Offset) {
...
Constant *toolchain::ConstantFoldLoadFromConst(Constant *C, Type *Ty,
                                          const APInt &Offset,
                                          const DataLayout &DL) {
...
Constant *toolchain::ConstantFoldLoadFromConstPtr(Constant *C, Type *Ty,
                                             APInt Offset,
                                             const DataLayout &DL) {
...
Constant *toolchain::ConstantFoldLoadFromUniformValue(Constant *C, Type *Ty,
                                                 const DataLayout &DL) {
...
Constant *SymbolicallyEvaluateGEP(const GEPOperator *GEP,
  ...
  while (auto *GEP = dyn_cast<GEPOperator>(Ptr)) {
    ...
    SmallVector<Value *, 4> NestedOps(toolchain::drop_begin(GEP->operands()));
...
Constant *toolchain::ConstantFoldInstruction(const Instruction *I,
                                        const DataLayout &DL,
                                        const TargetLibraryInfo *TLI) {
...
Constant *toolchain::ConstantFoldConstant(const Constant *C, const DataLayout &DL,
                                     const TargetLibraryInfo *TLI) {
  SmallDenseMap<Constant *, Constant *> FoldedOps;
  return ConstantFoldConstantImpl(C, DL, TLI, FoldedOps);
}

Constant *toolchain::ConstantFoldInstOperands(const Instruction *I,
                                         ArrayRef<Constant *> Ops,
                                         const DataLayout &DL,
                                         const TargetLibraryInfo *TLI,
                                         bool AllowNonDeterministic) {
...
Constant *toolchain::ConstantFoldCompareInstOperands(
...
Constant *toolchain::ConstantFoldUnaryOpOperand(unsigned Opcode, Constant *Op,
                                           const DataLayout &DL) {
...
Constant *toolchain::ConstantFoldBinaryOpOperands(unsigned Opcode, Constant *LHS,
                                             Constant *RHS,
                                             const DataLayout &DL) {
...
Constant *toolchain::FlushFPConstant(Constant *Operand, const Instruction *Inst,
                                bool IsOutput) {
...
Constant *toolchain::ConstantFoldFPInstOperands(unsigned Opcode, Constant *LHS,
...
Constant *toolchain::ConstantFoldCastOperand(unsigned Opcode, Constant *C,
...
Constant *toolchain::ConstantFoldIntegerCast(Constant *C, Type *DestTy,
...
bool toolchain::canConstantFoldCallTo(const CallBase *Call, const Function *F) {
  if (Call->isNoBuiltin())
    return false;
...
/// Try to constant fold toolchain.canonicalize for the given caller and value.
...
static Constant *ConstantFoldScalableVectorCall(
  ...
  case Intrinsic::vector_interleave8: {
    ...
    if (!toolchain::all_equal(Operands))
      return nullptr;
...
Constant *toolchain::ConstantFoldBinaryIntrinsic(Intrinsic::ID ID, Constant *LHS,
...
Constant *toolchain::ConstantFoldCall(const CallBase *Call, Function *F,
                                 ArrayRef<Constant *> Operands,
                                 const TargetLibraryInfo *TLI,
                                 bool AllowNonDeterministic) {
  if (Call->isNoBuiltin())
...
bool toolchain::isMathLibCallNoop(const CallBase *Call,
                             const TargetLibraryInfo *TLI) {
...
Constant *toolchain::getLosslessInvCast(Constant *C, Type *InvCastTo,
...
Constant *toolchain::getLosslessUnsignedTrunc(Constant *C, Type *DestTy,
                                         const DataLayout &DL,
                                         PreservedCastFlags *Flags) {
  return getLosslessInvCast(C, DestTy, Instruction::ZExt, DL, Flags);
}

Constant *toolchain::getLosslessSignedTrunc(Constant *C, Type *DestTy,
                                       const DataLayout &DL,
                                       PreservedCastFlags *Flags) {
