...
bool toolchain::isDereferenceableAndAlignedPointer(
    const Value *V, Align Alignment, const APInt &Size, const DataLayout &DL,
    const Instruction *CtxI, AssumptionCache *AC, const DominatorTree *DT,
...
bool toolchain::isDereferenceablePointer(const Value *V, Type *Ty,
                                    const DataLayout &DL,
...
bool toolchain::isDereferenceableAndAlignedInLoop(
...
bool toolchain::mustSuppressSpeculation(const LoadInst &LI) {
...
bool toolchain::isSafeToLoadUnconditionally(Value *V, Align Alignment, const APInt &Size,
                                       const DataLayout &DL,
...
bool toolchain::isSafeToLoadUnconditionally(Value *V, Type *Ty, Align Alignment,
                                       const DataLayout &DL,
                                       Instruction *ScanFrom,
                                       AssumptionCache *AC,
                                       const DominatorTree *DT,
...
cl::opt<unsigned>
toolchain::DefMaxInstsToScan("available-load-scan-limit", cl::init(6), cl::Hidden,
  cl::desc("Use this to specify the default maximum number of instructions "
           "to scan backward from a given instruction, when searching for "
           "available loaded value"));

Value *toolchain::FindAvailableLoadedValue(LoadInst *Load, BasicBlock *ScanBB,
...
Value *toolchain::findAvailablePtrLoadStore(
...
Value *toolchain::FindAvailableLoadedValue(LoadInst *Load, BatchAAResults &AA,
                                      bool *IsLoadCSE,
                                      unsigned MaxInstsToScan) {
...
bool toolchain::canReplacePointersInUseIfEqual(const Use &U, const Value *To,
                                          const DataLayout &DL) {
  Type *Ty = To->getType();
...
bool toolchain::canReplacePointersIfEqual(const Value *From, const Value *To,
                                     const DataLayout &DL) {
...
LinearExpression toolchain::decomposeLinearExpression(const DataLayout &DL,
                                                 Value *Ptr) {
