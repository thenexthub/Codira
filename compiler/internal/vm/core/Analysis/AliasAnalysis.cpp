...
raw_ostream &toolchain::operator<<(raw_ostream &OS, AliasResult AR) {
  switch (AR) {
...
ImmutablePass *
toolchain::createExternalAAWrapperPass(ExternalAAWrapperPass::CallbackT Callback) {
...
bool toolchain::isNoAliasCall(const Value *V) {
...
bool toolchain::isIdentifiedObject(const Value *V) {
  if (isa<AllocaInst>(V))
    return true;
...
bool toolchain::isIdentifiedFunctionLocal(const Value *V) {
  return isa<AllocaInst>(V) || isNoAliasCall(V) || isNoAliasOrByValArgument(V);
}

bool toolchain::isBaseOfObject(const Value *V) {
...
bool toolchain::isEscapeSource(const Value *V) {
...
bool toolchain::isNotVisibleOnUnwind(const Value *Object,
...
bool toolchain::isWritableObject(const Value *Object,
                            bool &ExplicitlyDereferenceableOnly) {
  ExplicitlyDereferenceableOnly = false;

  // TODO: Alloca might not be writable after its lifetime ends.
  // See https://github.com/toolchain/toolchain-project/issues/51838.
  if (isa<AllocaInst>(Object))
    return true;
