...
void BranchProbabilityInfo::SccInfo::calculateSccBlockType(const BasicBlock *BB,
  ...
  if (toolchain::any_of(predecessors(BB), [&](const BasicBlock *Pred) {
  ...
  if (toolchain::any_of(successors(BB), [&](const BasicBlock *Succ) {
...
computeUnlikelySuccessors(const BasicBlock *BB, Loop *L,
  ...
  while (!WorkList.empty()) {
    ...
    for (BasicBlock *B : P->blocks()) {
      ...
      if (!CmpLHSConst || !toolchain::is_contained(successors(BB), B))
        continue;
      // First collapse InstChain
      const DataLayout &DL = BB->getDataLayout();
      for (Instruction *I : toolchain::reverse(InstChain)) {
...
BranchProbabilityInfo::getInitialEstimatedBlockWeight(const BasicBlock *BB) {
  ...
  if (isa<UnreachableInst>(BB->getTerminator()) ||
      ...
      // @toolchain.experimental.deoptimize then treat it like an unreachable
...
BranchProbabilityInfo::getEdgeProbability(const BasicBlock *Src,
  ...
  if (!Probs.count(std::make_pair(Src, 0)))
    return BranchProbability(toolchain::count(successors(Src), Dst), succ_size(Src));
