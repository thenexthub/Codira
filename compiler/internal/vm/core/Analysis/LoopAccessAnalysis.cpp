...
using namespace vm::core::SCEVPatternMatch;
...
const SCEV *toolchain::replaceSymbolicStrideSCEV(PredicatedScalarEvolution &PSE,
...
std::pair<const SCEV *, const SCEV *> toolchain::getStartAndEndForAccess(
...
bool RuntimePointerChecking::tryToCreateDiffCheck(
  ...
  // they may not be hoisted. We should instead let toolchain::addRuntimeChecks
...
void RuntimePointerChecking::groupChecks(
  ...
  for (unsigned I = 0; I < Pointers.size(); ++I) {
    ...
    toolchain::append_range(CheckingGroups, Groups);
  }
}
...
static void visitPointers(Value *StartPtr, const Loop &InnermostLoop,
  ...
  while (!WorkList.empty()) {
    ...
    if (PN && InnermostLoop.contains(PN->getParent()) &&
      ...
      toolchain::append_range(WorkList, PN->incoming_values());
    } else
      AddPointer(Ptr);
  }
}
...
toolchain::getPtrStride(PredicatedScalarEvolution &PSE, Type *AccessTy, Value *Ptr,
...
std::optional<int64_t> toolchain::getPointersDiff(Type *ElemTyA, Value *PtrA,
...
bool toolchain::sortPtrAccesses(ArrayRef<Value *> VL, Type *ElemTy,
  ...
  auto Compare = toolchain::less_first();
...
bool toolchain::isConsecutiveAccess(Value *A, Value *B, const DataLayout &DL,
...
void LoopAccessInfo::emitUnsafeDependenceRemark() {
  ...
  if (!Deps)
    return;
  const auto *Found =
      toolchain::find_if(*Deps, [](const MemoryDepChecker::Dependence &D) {
  ...
  std::optional<const MDOperand *> Value =
      findStringMetadataForLoop(TheLoop, "toolchain.loop.distribute.enable");
  if (Value) {
