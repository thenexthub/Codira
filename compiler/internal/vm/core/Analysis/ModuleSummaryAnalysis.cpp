...
static void addIntrinsicToSummary(
  ...
  case Intrinsic::public_type_test: {
    ...
    // intrinsics that are used other than by an toolchain.assume intrinsic.
    // Intrinsics that are assumed are relevant only to the devirtualization
    // pass, not the type test lowering pass.
    bool HasNonAssumeUses = toolchain::any_of(CI->uses(), [](const Use &CIU) {
  ...
  case Intrinsic::type_checked_load: {
    ...
    // Any non-call uses of the result of toolchain.type.checked.load will
    // prevent us from optimizing away the toolchain.type.test.
    if (HasNonCallUses)
      TypeTests.insert(Guid);
    for (auto &Call : DevirtCalls)
...
static void computeFunctionSummary(
  ...
  if (IsThinLTO) {
    ...
    SmallPtrSet<const toolchain::User *, 8> StoreCache;
...
static void findFuncPointers(const Constant *I, uint64_t StartingOffset,
  ...
  if (auto *C = dyn_cast<ConstantStruct>(I)) {
    ...
    for (auto EI : toolchain::enumerate(STy->elements())) {
...
ModuleSummaryIndex toolchain::buildModuleSummaryIndex(
    const Module &M,
  ...
  // Identify the local values in the toolchain.used and toolchain.compiler.used sets,
  ...
  // First collect those in the toolchain.used set.
  collectUsedGlobalVariables(M, Used, /*CompilerUsed=*/false);
  // Next collect those in the toolchain.compiler.used set.
  ...
  if (!M.getModuleInlineAsm().empty()) {
    ...
    // be listed on the toolchain.used or toolchain.compiler.used global and marked as
  ...
  setLiveRoot(Index, "toolchain.used");
  setLiveRoot(Index, "toolchain.compiler.used");
  setLiveRoot(Index, "toolchain.global_ctors");
  setLiveRoot(Index, "toolchain.global_dtors");
  setLiveRoot(Index, "toolchain.global.annotations");
  ...
  for (auto &GlobalList : Index) {
    ...
    bool AllRefsCanBeExternallyReferenced =
        toolchain::all_of(Summary->refs(), [&](const ValueInfo &VI) {
    ...
    if (auto *FuncSummary = dyn_cast<FunctionSummary>(Summary.get())) {
      bool AllCallsCanBeExternallyReferenced = toolchain::all_of(
...
ModulePass *toolchain::createModuleSummaryIndexWrapperPass() {
...
ImmutablePass *toolchain::createImmutableModuleSummaryIndexWrapperPass(
    const ModuleSummaryIndex *Index) {
...
bool toolchain::mayHaveMemprofSummary(const CallBase *CB) {
  if (!CB)
    return false;
