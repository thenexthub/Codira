...
/// as toolchain::getInlineCost. Note the function_ref constructor parameters - the
...
protected:
  ...
  /// Getter for the cache of @toolchain.assume intrinsics.
...
bool CallAnalyzer::simplifyCmpInstForRecCall(CmpInst &Cmp) {
  ...
  Value *SimplifiedInstruction = toolchain::simplifyInstructionWithOperands(
...
/// Try to simplify a call to toolchain.is.constant.
///
...
/// toolchain.is.constant would evaluate.
...
bool CallAnalyzer::simplifyIntrinsicCallObjectSize(CallBase &CB) {
  // As per the langref, "The fourth argument to toolchain.objectsize determines if
...
void CallAnalyzer::findDeadBlocks(BasicBlock *CurrBB, BasicBlock *NextBB) {
  ...
  auto IsNewlyDead = [&](BasicBlock *BB) {
    ...
    return (!DeadBlocks.count(BB) &&
            toolchain::all_of(predecessors(BB),
...
int toolchain::getCallsiteCost(const TargetTransformInfo &TTI, const CallBase &Call,
                          const DataLayout &DL) {
  int64_t Cost = 0;
...
InlineCost toolchain::getInlineCost(
...
std::optional<int> toolchain::getInliningCostEstimate(
...
std::optional<InlineCostFeatures> toolchain::getInliningCostFeatures(
    CallBase &Call, TargetTransformInfo &CalleeTTI,
    function_ref<AssumptionCache &(Function &)> GetAssumptionCache,
    function_ref<BlockFrequencyInfo &(Function &)> GetBFI,
    function_ref<const TargetLibraryInfo &(Function &)> GetTLI,
...
std::optional<InlineResult> toolchain::getAttributeBasedInliningDecision(
...
InlineCost toolchain::getInlineCost(
  ...
  auto UserDecision =
      toolchain::getAttributeBasedInliningDecision(Call, Callee, CalleeTTI, GetTLI);

  if (UserDecision) {
    if (UserDecision->isSuccess())
      return toolchain::InlineCost::getAlways("always inline attribute");
    return toolchain::InlineCost::getNever(UserDecision->getFailureReason());
  }

  if (InlineAllViableCalls && isInlineViable(*Callee).isSuccess())
    return toolchain::InlineCost::getAlways(
        "Inlining forced by -inline-all-viable-calls");

  LLVM_DEBUG(toolchain::dbgs() << "      Analyzing call of " << Callee->getName()
...
InlineResult toolchain::isInlineViable(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    ...
    for (auto &II : BB) {
      ...
      if (Callee)
        ...
        default:
          break;
        case toolchain::Intrinsic::icall_branch_funnel:
          // Disallow inlining of @toolchain.icall.branch.funnel because current
          // backend can't separate call targets from call arguments.
          return InlineResult::failure(
              "disallowed inlining of @toolchain.icall.branch.funnel");
        case toolchain::Intrinsic::localescape:
          // Disallow inlining functions that call @toolchain.localescape. Doing this
          // correctly would require major changes to the inliner.
          return InlineResult::failure(
              "disallowed inlining of @toolchain.localescape");
        case toolchain::Intrinsic::vastart:
...
InlineParams toolchain::getInlineParams(int Threshold) {
  InlineParams Params;
...
InlineParams toolchain::getInlineParams() {
...
InlineParams toolchain::getInlineParams(unsigned OptLevel, unsigned SizeOptLevel) {
  auto Params =
...
InlineCostAnnotationPrinterPass::run(Function &F,
  ...
  const InlineParams Params = toolchain::getInlineParams();
  for (BasicBlock &BB : F) {
    for (Instruction &I : BB) {
