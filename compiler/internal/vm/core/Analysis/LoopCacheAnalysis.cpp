...
#include "vm/core/ADT/BreadthFirstIterator.h"
...
static Loop *getInnerMostLoop(const LoopVectorTy &Loops) {
  ...
  return (toolchain::is_sorted(Loops,
...
//
raw_ostream &toolchain::operator<<(raw_ostream &OS, const IndexedReference &R) {
  if (!R.IsValid) {
...
bool IndexedReference::delinearize(const LoopInfo &LI) {
  ...
  if (Loop *L = LI.getLoopFor(BB)) {
    ...
    if (!IsFixedSize) {
      LLVM_DEBUG(dbgs().indent(2) << "In Loop '" << L->getName()
                                  << "', AccessFn: " << *AccessFn << "\n");
      toolchain::delinearize(SE, AccessFn, Subscripts, Sizes,
...
//
raw_ostream &toolchain::operator<<(raw_ostream &OS, const CacheCost &CC) {
