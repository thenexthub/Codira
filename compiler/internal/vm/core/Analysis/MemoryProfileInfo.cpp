...
bool toolchain::memprof::metadataIncludesAllContextSizeInfo() {
  return MemProfReportHintedSizes || MinClonedColdBytePercent < 100;
}

bool toolchain::memprof::metadataMayIncludeContextSizeInfo() {
  return metadataIncludesAllContextSizeInfo() || MinPercentMaxColdSize < 100;
}

bool toolchain::memprof::recordContextSizeInfoForAnalysis() {
  return metadataMayIncludeContextSizeInfo() ||
         MinCallsiteColdBytePercent < 100;
}

MDNode *toolchain::memprof::buildCallstackMetadata(ArrayRef<uint64_t> CallStack,
                                              LLVMContext &Ctx) {
...
MDNode *toolchain::memprof::getMIBStackNode(const MDNode *MIB) {
...
AllocationType toolchain::memprof::getMIBAllocType(const MDNode *MIB) {
  assert(MIB->getNumOperands() >= 2);
...
std::string toolchain::memprof::getAllocTypeAttributeString(AllocationType Type) {
  switch (Type) {
...
bool toolchain::memprof::hasSingleAllocType(uint8_t AllocTypes) {
  const unsigned NumAllocTypes = toolchain::popcount(AllocTypes);
  assert(NumAllocTypes != 0);
  return NumAllocTypes == 1;
}

void toolchain::memprof::removeAnyExistingAmbiguousAttribute(CallBase *CB) {
...
void toolchain::memprof::addAmbiguousAttribute(CallBase *CB) {
  ...
  if (CB->hasFnAttr("memprof")) {
    ...
    auto A = toolchain::Attribute::get(CB->getContext(), "memprof", "ambiguous");
    CB->addFnAttr(A);
  }
}
...
void CallStackTrie::addCallStack(
  ...
  toolchain::append_range(Curr->ContextInfo,
                     toolchain::map_range(ContextSizeInfo,
...
void CallStackTrie::collectContextInfo(
  ...
  toolchain::append_range(ContextInfo, Node->ContextInfo);
    ...
    auto NewColdMIBNodes =
        make_filter_range(NewMIBNodes, [&](const Metadata *M) {
          ...
          if (MemProfReportHintedSizes) {
            ...
            toolchain::raw_string_ostream OS(PercentStr);
...
void CallStackTrie::addSingleAllocTypeAttribute(CallBase *CI, AllocationType AT,
  ...
  auto A = toolchain::Attribute::get(CI->getContext(), "memprof", AllocTypeString);
