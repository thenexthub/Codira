...
#include "vm/core/Analysis/Utils/ImportedFunctionsInliningStatistics.h"
...
void DefaultInlineAdvice::recordUnsuccessfulInliningImpl(
  ...
  using namespace ore;
  toolchain::setInlineRemark(*OriginalCB, std::string(Result.getFailureReason()) +
                                         "; " + inlineCostStr(*OIC));
  ORE.emit([&]() {
    return OptimizationRemarkMissed(Advisor->getAnnotatedInlinePassName(),
                                    "NotInlined", DLoc, Block)
...
std::optional<toolchain::InlineCost> static getDefaultInlineAdvice(
...
bool InlineAdvisorAnalysis::Result::tryCreate(
  ...
  case InliningAdvisorMode::Default:
    ...
    if (!ReplaySettings.ReplayFile.empty()) {
      Advisor = toolchain::getReplayInlineAdvisor(M, FAM, M.getContext(),
  ...
  case InliningAdvisorMode::Development:
    ...
    if (!InlineAdvisorAnalysis::initializeIR2VecVocabIfRequested(M, MAM))
      return false;
    Advisor = toolchain::getDevelopmentModeAdvisor(M, MAM, GetDefaultAdvice);
#endif
    break;
  case InliningAdvisorMode::Release:
    ...
    if (!InlineAdvisorAnalysis::initializeIR2VecVocabIfRequested(M, MAM))
      return false;
    Advisor = toolchain::getReleaseModeAdvisor(M, MAM, GetDefaultAdvice);
    break;
  }
...
std::string toolchain::inlineCostStr(const InlineCost &IC) {
  std::string Buffer;
...
void toolchain::setInlineRemark(CallBase &CB, StringRef Message) {
...
toolchain::shouldInline(CallBase &CB, TargetTransformInfo &CalleeTTI,
                   function_ref<InlineCost(CallBase &CB)> GetInlineCost,
...
std::string toolchain::formatCallSiteLocation(DebugLoc DLoc,
  ...
  for (DILocation *DIL = DLoc.get(); DIL; DIL = DIL->getInlinedAt()) {
    ...
    CallSiteLoc << Name.str() << ":" << toolchain::utostr(Offset);
    if (Format.outputColumn())
      CallSiteLoc << ":" << toolchain::utostr(DIL->getColumn());
    if (Format.outputDiscriminator() && Discriminator)
      CallSiteLoc << "." << toolchain::utostr(Discriminator);
  }

  return CallSiteLoc.str();
}

void toolchain::addLocationToRemarks(OptimizationRemark &Remark, DebugLoc DLoc) {
  if (!DLoc)
    return;
...
void toolchain::emitInlinedInto(
...
InlineAdvisor::InlineAdvisor(Module &M, FunctionAnalysisManager &FAM,
    ...
    : M(M), FAM(FAM), IC(IC),
      AnnotatedInlinePassName((IC && AnnotateInlinePhase)
                                  ? toolchain::AnnotateInlinePassName(*IC)
                                  : DEBUG_TYPE) {
...
std::string toolchain::AnnotateInlinePassName(InlineContext IC) {
...
InlineAdvisor::getMandatoryKind(CallBase &CB, FunctionAnalysisManager &FAM,
  ...
  auto TrivialDecision =
      toolchain::getAttributeBasedInliningDecision(CB, &Callee, TIR, GetTLI);
