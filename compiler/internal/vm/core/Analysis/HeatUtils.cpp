...
#include "vm/core/Analysis/HeatUtils.h"
...
uint64_t toolchain::getNumOfCalls(const Function &CallerFunction,
...
uint64_t toolchain::getMaxFreq(const Function &F, const BlockFrequencyInfo *BFI) {
  uint64_t MaxFreq = 0;
...
std::string toolchain::getHeatColor(uint64_t Freq, uint64_t MaxFreq) {
  if (Freq > MaxFreq)
    Freq = MaxFreq;
...
std::string toolchain::getHeatColor(double Percent) {
