...
#include "vm/core/Analysis/IRSimilarityIdentifier.h"
...
#include "vm/core/Support/SuffixTree.h"
...
void IRInstructionData::initializeInstruction() {
  ...
  if (PHINode *PN = dyn_cast<PHINode>(Inst))
    toolchain::append_range(OperVals, PN->blocks());
}
...
bool IRSimilarity::isClose(const IRInstructionData &A,
  ...
  if (!A.Inst->isSameOperationAs(B.Inst)) {
    ...
    if (isa<CmpInst>(A.Inst) && isa<CmpInst>(B.Inst)) {
      ...
      return all_of(
          ZippedTypes, [](std::tuple<toolchain::Value *, toolchain::Value *> R) {
  ...
  if (auto *GEP = dyn_cast<GetElementPtrInst>(A.Inst)) {
    ...
    return all_of(drop_begin(ZippedOperands),
                  [](std::tuple<toolchain::Use &, toolchain::Use &> R) {
...
void IRInstructionMapper::convertToUnsignedVec(
  ...
  toolchain::append_range(InstrList, InstrListForBB);
  toolchain::append_range(IntegerMapping, IntegerMappingForBB);
}
...
void IRSimilarityIdentifier::populateMapper(
  ...
  toolchain::append_range(InstrList, InstrListForModule);
  // Do the same as above, but for IntegerMapping.
  toolchain::append_range(IntegerMapping, IntegerMappingForModule);
}
...
void IRSimilarityIdentifier::findCandidates(
  ...
  toolchain::stable_sort(RSes, [](const SuffixTree::RepeatedSubstring &LHS,
