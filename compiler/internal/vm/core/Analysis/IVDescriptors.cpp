//===- toolchain/Analysis/IVDescriptors.cpp - IndVar Descriptors -----*- C++ -*-===//
//
...
static std::pair<Type *, bool> computeRecurrenceType(Instruction *Exit,
  ...
  MaxBitWidth = toolchain::bit_ceil(MaxBitWidth);
...
bool RecurrenceDescriptor::AddReductionVar(
  ...
  while (!Worklist.empty()) {
    ...
    for (User *U : Cur->users()) {
      ...
      // If the user is a call to toolchain.fmuladd then the instruction can only be
      // the final operand.
  ...
  // toolchain.min/max intrinsic, which is always OK.
...
RecurrenceDescriptor::getReductionOpChain(PHINode *Phi, Loop *L) const {
  ...
  auto isCorrectOpcode = [&](Instruction *Cur) {
    ...
    // Recognize a call to the toolchain.fmuladd intrinsic.
...
InductionDescriptor::InductionDescriptor(Value *Start, InductionKind K,
  ...
  if (Casts)
    toolchain::append_range(RedundantCasts, *Casts);
}
