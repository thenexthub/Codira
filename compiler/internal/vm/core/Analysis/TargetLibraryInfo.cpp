...
#include "vm/core/ADT/StringTable.h"
...
#include "vm/core/Analysis/TargetLibraryInfo.inc"
...
static bool isCallingConvCCompatible(CallingConv::ID CC, const Triple &TT,
  ...
  default:
    return false;
  case toolchain::CallingConv::C:
    return true;
  case toolchain::CallingConv::ARM_APCS:
  case toolchain::CallingConv::ARM_AAPCS:
  case toolchain::CallingConv::ARM_AAPCS_VFP: {
...
static void initializeLibCalls(TargetLibraryInfoImpl &TLI, const Triple &T,
                               const toolchain::StringTable &StandardNames,
  ...
  if (T.isAMDGPU()) {
    ...
    TLI.setAvailable(toolchain::LibFunc___kmpc_alloc_shared);
    TLI.setAvailable(toolchain::LibFunc___kmpc_free_shared);
    return;
  }
  ...
  // but only used functions are provided to toolchain.  Moreover, most of the
  ...
  if (T.isNVPTX()) {
    ...
    TLI.setAvailable(toolchain::LibFunc_malloc);
    TLI.setAvailable(toolchain::LibFunc_free);
    ...
    //
    //    TLI.setAvailable(toolchain::LibFunc_memcpy);
    //    TLI.setAvailable(toolchain::LibFunc_memset);
...
buildIndexMap(const toolchain::StringTable &StandardNames) {
...
void TargetLibraryInfoImpl::addVectorizableFunctions(ArrayRef<VecDesc> Fns) {
  toolchain::append_range(VectorDescs, Fns);
  toolchain::sort(VectorDescs, compareByScalarFnName);

  toolchain::append_range(ScalarDescs, Fns);
  toolchain::sort(ScalarDescs, compareByVectorFnName);
}

static const VecDesc VecFuncs_Accelerate[] = {
#define TLI_DEFINE_ACCELERATE_VECFUNCS
#include "vm/core/Analysis/VecFuncs.def"
...
void TargetLibraryInfoImpl::addVectorizableFunctionsFromVecLib(
    enum VectorLibrary VecLib, const toolchain::Triple &TargetTriple) {
  switch (VecLib) {
...
bool TargetLibraryInfoImpl::isFunctionVectorizable(StringRef funcName) const {
  ...
  std::vector<VecDesc>::const_iterator I =
      toolchain::lower_bound(VectorDescs, funcName, compareWithScalarFnName);
...
TargetLibraryInfoImpl::getVectorMappingInfo(StringRef F, const ElementCount &VF,
  ...
  if (F.empty())
    return nullptr;
  std::vector<VecDesc>::const_iterator I =
      toolchain::lower_bound(VectorDescs, F, compareWithScalarFnName);
...
void TargetLibraryInfoImpl::getWidestVF(StringRef ScalarF,
  ...
  std::vector<VecDesc>::const_iterator I =
      toolchain::lower_bound(VectorDescs, ScalarF, compareWithScalarFnName);
