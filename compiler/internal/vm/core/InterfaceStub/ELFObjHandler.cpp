...
#include "vm/core/InterfaceStub/ELFObjHandler.h"
...
using toolchain::object::ELFObjectFile;
...
static void initELFHeader(typename ELFT::Ehdr &ElfHeader, uint16_t Machine) {
  ...
  bool IsLittleEndian = ELFT::Endianness == toolchain::endianness::little;
...
private:
  toolchain::SmallVector<Elf_Sym, 8> Symbols;
};
...
private:
  toolchain::SmallVector<Elf_Dyn, 8> Entries;
};
  ...
  const Elf_Shdr *findDynSymHdr() {
    for (const Elf_Shdr &Sec : Shdrs)
      if (Sec.sh_type == SHT_DYNSYM) {
        ...
        // This behavior aligns with toolchain::object::ELFFile::getDynSymtabSize()
        return &Sec;
      }
    return nullptr;
  }
