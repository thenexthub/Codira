...
#include "vm/core/Config/config.h" // Detect libffi
...
// https://github.com/toolchain/toolchain-project/issues/58086)
...
static GenericValue lle_X_printf(FunctionType *FT,
  ...
  toolchain::append_range(NewArgs, Args);
...
static GenericValue lle_X_fprintf(FunctionType *FT,
  ...
  toolchain::append_range(NewArgs, toolchain::drop_begin(Args));
  GenericValue GV = lle_X_sprintf(FT, NewArgs);
...
static GenericValue lle_X_memset(FunctionType *FT,
  ...
  // toolchain.memset.* returns void, lle_X_* returns GenericValue,
...
static GenericValue lle_X_memcpy(FunctionType *FT,
  ...
  // toolchain.memcpy* returns void, lle_X_* returns GenericValue,
  // so here we return GenericValue with IntVal set to zero
  GenericValue GV;
  GV.IntVal = 0;
  return GV;
