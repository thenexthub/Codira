...
ExecutionEngine *(*ExecutionEngine::MCODEITCtor)(
...
void *ArgvArray::reset(LLVMContext &C, ExecutionEngine *EE,
  ...
  for (unsigned i = 0; i != InputArgv.size(); ++i) {
    ...
    toolchain::copy(InputArgv[i], Dest.get());
    Dest[Size-1] = 0;
...
void ExecutionEngine::runStaticConstructorsDestructors(Module &module,
                                                       bool isDtors) {
  StringRef Name(isDtors ? "toolchain.global_dtors" : "toolchain.global_ctors");
...
EngineBuilder &EngineBuilder::setMCODEITMemoryManager(
                                   std::unique_ptr<RTDyldMemoryManager> mcodemm) {
  auto SharedMM = std::shared_ptr<RTDyldMemoryManager>(std::move(mcodemm));
  MemMgr = SharedMM;
  Resolver = SharedMM;
  return *this;
}
...
EngineBuilder&
EngineBuilder::setMemoryManager(std::unique_ptr<MCODEITMemoryManager> MM) {
  MemMgr = std::shared_ptr<MCODEITMemoryManager>(std::move(MM));
  return *this;
}
...
ExecutionEngine *EngineBuilder::create(TargetMachine *TM) {
  ...
  if ((WhichEngine & EngineKind::JIT) && TheTM) {
    ...
    if (ExecutionEngine::MCODEITCtor)
      EE = ExecutionEngine::MCODEITCtor(std::move(M), ErrorStr, std::move(MemMgr),
  ...
  if ((WhichEngine & EngineKind::JIT) && !ExecutionEngine::MCODEITCtor) {
    if (ErrorStr)
  ...
  case Type::FixedVectorTyID: {
    ...
    if(ElemTy->isFloatTy()) {
      if (CAZ) {
        ...
        toolchain::fill(Result.AggregateVal, floatZero);
        break;
      }
      if(CV) {
    ...
    if (ElemTy->isDoubleTy()) {
      if (CAZ) {
        ...
        toolchain::fill(Result.AggregateVal, doubleZero);
        break;
      }
      if(CV) {
        for (unsigned i = 0; i < elemNum; ++i)
    ...
    if (ElemTy->isIntegerTy()) {
      if (CAZ) {
        ...
        toolchain::fill(Result.AggregateVal, intZero);
        break;
      }
      if(CV) {
        for (unsigned i = 0; i < elemNum; ++i)
