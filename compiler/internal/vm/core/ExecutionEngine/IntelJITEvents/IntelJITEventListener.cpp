...
public:
  ...
  void reportSection(toolchain::IttEventType EventType, const char *SectionName,
  ...
  int fillSectionInformation(const ObjectFile &Obj,
    ...
    for (auto &Section : Obj.sections()) {
      ...
      if (SectionLoadAddr) {
        ...
        reportSection(toolchain::LoadBinarySection, SectionName.str().c_str(),
                      SectionInfo.size);
...
void IntelJITEventListener::notifyObjectLoaded(
  ...
  if (BackwardCompatibilityMode == 0) {
    if (Obj.isELF()) {
      ...
      Wrapper->iJitIttNotifyInfo(toolchain::LoadBinaryModule,
