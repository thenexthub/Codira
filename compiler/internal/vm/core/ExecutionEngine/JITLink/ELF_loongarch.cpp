...
using namespace vm::core::jitlink::loongarch;
...
private:
  ...
  Error applyFixup(LinkGraph &G, Block &B, const Edge &E) const {
    ...
    case Call30PCRel: {
      ...
      if (Value != toolchain::SignExtend64(Value, 32))
        return makeTargetOutOfRangeError(G, B, E);
    ...
    case Call36PCRel: {
      ...
      if ((Value + 0x20000) != toolchain::SignExtend64(Value + 0x20000, 38))
        return makeTargetOutOfRangeError(G, B, E);
    ...
    case AddUleb128: {
      ...
      if (Count > Maxcount || (Count == Maxcount && Error))
        return make_error<JITLinkError>(
            "0x" + toolchain::utohexstr(orc::ExecutorAddr(FixupAddress).getValue()) +
...
static RelaxAux initRelaxAux(LinkGraph &G) {
  ...
  for (auto &BlockAuxIter : Aux.Blocks) {
    toolchain::sort(BlockAuxIter.second.Anchors, [](auto &A, auto &B) {
...
static bool relaxBlock(LinkGraph &G, Block &Block, BlockRelaxAux &Aux) {
  ...
  for (auto [I, E] : toolchain::enumerate(Aux.RelaxEdges)) {
