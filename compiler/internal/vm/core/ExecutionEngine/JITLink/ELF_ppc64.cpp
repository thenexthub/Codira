...
#include "vm/core/ExecutionEngine/JITLink/TableManager.h"
...
template <toolchain::endianness Endianness>
...
const uint8_t TLSInfoTableManager_ELF_ppc64<
    toolchain::endianness::little>::TLSInfoEntryContent[16] = {
...
template <>
const uint8_t TLSInfoTableManager_ELF_ppc64<
    toolchain::endianness::big>::TLSInfoEntryContent[16] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /*pthread key */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  /*data address*/
};
...
createLinkGraphFromELFObject_ppc64(MemoryBufferRef ObjectBuffer,
                                   std::shared_ptr<orc::SymbolStringPool> SSP) {
  return createLinkGraphFromELFObject_ppc64<toolchain::endianness::big>(
      std::move(ObjectBuffer), std::move(SSP));
}

Expected<std::unique_ptr<LinkGraph>> createLinkGraphFromELFObject_ppc64le(
    MemoryBufferRef ObjectBuffer, std::shared_ptr<orc::SymbolStringPool> SSP) {
  return createLinkGraphFromELFObject_ppc64<toolchain::endianness::little>(
      std::move(ObjectBuffer), std::move(SSP));
}
...
void link_ELF_ppc64(std::unique_ptr<LinkGraph> G,
                    std::unique_ptr<JITLinkContext> Ctx) {
  return link_ELF_ppc64<toolchain::endianness::big>(std::move(G), std::move(Ctx));
}

/// jit-link the given object buffer, which must be a ELF ppc64le object file.
void link_ELF_ppc64le(std::unique_ptr<LinkGraph> G,
                      std::unique_ptr<JITLinkContext> Ctx) {
  return link_ELF_ppc64<toolchain::endianness::little>(std::move(G), std::move(Ctx));
}

} // end namespace vm::core::jitlink
