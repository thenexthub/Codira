...
#include "vm/core/ExecutionEngine/JITLink/ELF_aarch32.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_aarch64.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_loongarch.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_ppc64.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_riscv.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_systemz.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_x86.h"
#include "vm/core/ExecutionEngine/JITLink/ELF_x86_64.h"
...
Expected<uint16_t> readTargetMachineArch(StringRef Buffer) {
  ...
  if (Data[ELF::EI_DATA] == ELF::ELFDATA2LSB) {
    if (Data[ELF::EI_CLASS] == ELF::ELFCLASS64) {
      if (auto File = toolchain::object::ELF64LEFile::create(Buffer)) {
        return File->getHeader().e_machine;
      } else {
        return File.takeError();
      }
    } else if (Data[ELF::EI_CLASS] == ELF::ELFCLASS32) {
      if (auto File = toolchain::object::ELF32LEFile::create(Buffer)) {
        return File->getHeader().e_machine;
      } else {
        return File.takeError();
      }
  ...
  if (Data[ELF::EI_DATA] == ELF::ELFDATA2MSB) {
    if (Data[ELF::EI_CLASS] == ELF::ELFCLASS64) {
      if (auto File = toolchain::object::ELF64BEFile::create(Buffer)) {
        return File->getHeader().e_machine;
      } else {
        return File.takeError();
      }
    } else if (Data[ELF::EI_CLASS] == ELF::ELFCLASS32) {
      if (auto File = toolchain::object::ELF32BEFile::create(Buffer)) {
        return File->getHeader().e_machine;
      } else {
        return File.takeError();
      }
