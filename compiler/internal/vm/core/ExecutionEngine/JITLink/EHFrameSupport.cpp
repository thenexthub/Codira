...
Error EHFrameEdgeFixer::operator()(LinkGraph &G) {
  ...
  toolchain::append_range(EHFrameBlocks, EHFrame->blocks());
  toolchain::sort(EHFrameBlocks, [](const Block *LHS, const Block *RHS) {
...
EHFrameCFIBlockInspector EHFrameCFIBlockInspector::FromEdgeScan(Block &B) {
  ...
  SmallVector<Edge *, 3> Es(toolchain::make_pointer_range(B.edges()));
  assert(Es.size() >= 2 && Es.size() <= 3 && "Unexpected number of edges");
  toolchain::sort(Es, [](const Edge *LHS, const Edge *RHS) {
