...
#include "vm/core/ExecutionEngine/JITLink/ELF.h"
#include "vm/core/ExecutionEngine/JITLink/MachO.h"
#include "vm/core/ExecutionEngine/JITLink/XCOFF.h"
#include "vm/core/ExecutionEngine/JITLink/aarch64.h"
...
#include "vm/core/ExecutionEngine/JITLink/x86_64.h"
...
const char *getLinkageName(Linkage L) {
  ...
  llvm_unreachable("Unrecognized toolchain.jitlink.Linkage enum");
}
...
const char *getScopeName(Scope S) {
  ...
  llvm_unreachable("Unrecognized toolchain.jitlink.Scope enum");
}
...
std::vector<Block *> LinkGraph::splitBlockImpl(std::vector<Block *> Blocks,
      ...
      *Cache = SplitBlockCache::value_type();
      ...
      for (auto *Sym : Blocks[0]->getSection().symbols())
        if (&Sym->getBlock() == Blocks[0])
          (*Cache)->push_back(Sym);
      toolchain::sort(**Cache, [](const Symbol *LHS, const Symbol *RHS) {
  ...
  toolchain::sort(Edges, [](const Edge &LHS, const Edge &RHS) {
...
void LinkGraph::dump(raw_ostream &OS) {
  ...
  for (auto &KV : BlockSymbols)
    toolchain::sort(KV.second, [](const Symbol *LHS, const Symbol *RHS) {
  ...
  toolchain::sort(SortedSections, [](const Section *LHS, const Section *RHS) {
    return LHS->getName() < RHS->getName();
  });
  ...
  for (auto *Sec : SortedSections) {
    ...
    toolchain::append_range(SortedBlocks, Sec->blocks());
    toolchain::sort(SortedBlocks, [](const Block *LHS, const Block *RHS) {
      return LHS->getAddress() < RHS->getAddress();
    });
    ...
    for (auto *B : SortedBlocks) {
      ...
      if (!B->edges_empty()) {
        ...
        toolchain::append_range(SortedEdges, B->edges());
        toolchain::sort(SortedEdges, [](const Edge &LHS, const Edge &RHS) {
          return LHS.getOffset() < RHS.getOffset();
        });
...
Error makeAlignmentError(toolchain::orc::ExecutorAddr Loc, uint64_t Value, int N,
                         const Edge &E) {
  return make_error<JITLinkError>("0x" + toolchain::utohexstr(Loc.getValue()) +
                                  " improper alignment for relocation " +
                                  formatv("{0:d}", E.getKind()) + ": 0x" +
                                  toolchain::utohexstr(Value) +
