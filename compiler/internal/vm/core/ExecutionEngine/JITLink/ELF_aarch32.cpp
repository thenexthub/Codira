...
#include "vm/core/ExecutionEngine/JITLink/aarch32.h"
...
template <toolchain::endianness DataEndianness>
...
public:
  ELFLinkGraphBuilder_aarch32(StringRef FileName,
                              const toolchain::object::ELFFile<ELFT> &Obj,
...
Expected<std::unique_ptr<LinkGraph>> createLinkGraphFromELFObject_aarch32(
  ...
  case Triple::thumb: {
    ...
    return ELFLinkGraphBuilder_aarch32<toolchain::endianness::little>(
  ...
  case Triple::thumbeb: {
    ...
    return ELFLinkGraphBuilder_aarch32<toolchain::endianness::big>(
               (*ELFObj)->getFileName(), ELFFile, std::move(SSP), TT,
               std::move(*Features), ArmCfg)
        .buildGraph();
  }
