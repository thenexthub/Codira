...
using namespace vm::core::jitlink::riscv;
...
static inline bool isInRangeForImm(int64_t Value, int N) {
  return Value == toolchain::SignExtend64(Value, N);
}
...
static RelaxAux initRelaxAux(LinkGraph &G) {
  ...
  Aux.Config.HasRVC = toolchain::is_contained(Features, "+c") ||
                      toolchain::is_contained(Features, "+zca");
