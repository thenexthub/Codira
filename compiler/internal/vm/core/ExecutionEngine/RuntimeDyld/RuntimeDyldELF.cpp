...
template <class T> static void write(bool isBE, void *P, T V) {
  isBE ? write<T, toolchain::endianness::big>(P, V)
       : write<T, toolchain::endianness::little>(P, V);
}
...
toolchain::RuntimeDyldELF::create(Triple::ArchType Arch,
...
void RuntimeDyldELF::resolveAArch64Relocation(const SectionEntry &Section,
  ...
  case ELF::R_AARCH64_ABS16: {
    ...
    assert(Result == static_cast<uint64_t>(toolchain::SignExtend64(Result, 16)) ||
           (Result >> 16) == 0);
  ...
  case ELF::R_AARCH64_ABS32: {
    uint64_t Result = Value + Addend;
    assert(Result == static_cast<uint64_t>(toolchain::SignExtend64(Result, 32)) ||
           (Result >> 32) == 0);
...
bool RuntimeDyldELF::resolveLoongArch64ShortBranch(
  ...
  if (((int64_t)Delta + 0x20000) != toolchain::SignExtend64(Delta + 0x20000, 38))
    return false;
  resolveRelocation(Sections[SectionID], Offset, Address, RelI->getType(),
                    Value.Addend);
  return true;
...
void RuntimeDyldELF::resolveRISCVRelocation(const SectionEntry &Section,
  ...
  default: {
    ...
    toolchain::report_fatal_error(Err.c_str());
  }
