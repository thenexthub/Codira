...
static void __register_frame(void *p) {
  ...
  if (!Searched) {
    Searched = true;
    *(void **)&rf =
        toolchain::sys::DynamicLibrary::SearchForAddressOfSymbol("__register_frame");
  }
  if (rf)
    rf(p);
}
...
static void __deregister_frame(void *p) {
  ...
  if (!Searched) {
    Searched = true;
    *(void **)&df = toolchain::sys::DynamicLibrary::SearchForAddressOfSymbol(
        "__deregister_frame");
  }
  if (df)
    df(p);
...
void RTDyldMemoryManager::registerEHFramesInProcess(uint8_t *Addr,
  ...
  // See http://lists.toolchain.org/pipermail/toolchain-dev/2013-April/061737.html
...
RTDyldMemoryManager::getSymbolAddressInProcess(const std::string &Name) {
  ...
  // 'libc_nonshared.a' on Google, or read http://toolchain.org/PR274.
...
void MCODEITMemoryManager::anchor() {}
} // namespace vm::core
