//===-- MCODEIT.cpp - MC-based Just-in-Time Compiler ------------------------===//
//
...
#include "vm/core/ExecutionEngine/MCODEIT.h"
#include "vm/core/ExecutionEngine/ObjectCache.h"
#include "vm/core/ExecutionEngine/SectionMemoryManager.h"
...
static struct RegisterJIT {
  RegisterJIT() { MCODEIT::Register(); }
} JITRegistrator;

}

extern "C" void LLVMLinkInMCODEIT() {
}

ExecutionEngine *
MCODEIT::createJIT(std::unique_ptr<Module> M, std::string *ErrorStr,
  ...
  return new MCODEIT(std::move(M), std::move(TM), std::move(MemMgr),
                   std::move(Resolver));
}

MCODEIT::MCODEIT(std::unique_ptr<Module> M, std::unique_ptr<TargetMachine> TM,
             std::shared_ptr<MCODEITMemoryManager> MemMgr,
             std::shared_ptr<LegacyJITSymbolResolver> Resolver)
...
void MCODEIT::addModule(std::unique_ptr<Module> M) {
  std::lock_guard<sys::Mutex> locked(lock);
...
bool MCODEIT::removeModule(Module *M) {
  std::lock_guard<sys::Mutex> locked(lock);
  return OwnedModules.removeModule(M);
}

void MCODEIT::addObjectFile(std::unique_ptr<object::ObjectFile> Obj) {
...
void MCODEIT::addObjectFile(object::OwningBinary<object::ObjectFile> Obj) {
...
void MCODEIT::addArchive(object::OwningBinary<object::Archive> A) {
  Archives.push_back(std::move(A));
}

void MCODEIT::setObjectCache(ObjectCache* NewCache) {
  std::lock_guard<sys::Mutex> locked(lock);
  ObjCache = NewCache;
}

std::unique_ptr<MemoryBuffer> MCODEIT::emitObject(Module *M) {
  ...
  // MCODEIT instance, since these conditions are tested by our caller,
...
void MCODEIT::generateCodeForModule(Module *M) {
  // Get a thread lock to make sure we aren't trying to load multiple times
  std::lock_guard<sys::Mutex> locked(lock);

  // This must be a module which has already been added to this MCODEIT instance.
  assert(OwnedModules.ownsModule(M) &&
         "MCODEIT::generateCodeForModule: Unknown module.");
  ...
  // MCODEIT now owns the ObjectImage pointer (via its LoadedObjects list).
...
void MCODEIT::finalizeLoadedModules() {
  std::lock_guard<sys::Mutex> locked(lock);
...
void MCODEIT::finalizeModule(Module *M) {
  std::lock_guard<sys::Mutex> locked(lock);

  // This must be a module which has already been added to this MCODEIT instance.
  assert(OwnedModules.ownsModule(M) && "MCODEIT::finalizeModule: Unknown module.");
...
JITSymbol MCODEIT::findExistingSymbol(const std::string &Name) {
...
Module *MCODEIT::findModuleForSymbol(const std::string &Name,
...
uint64_t MCODEIT::getSymbolAddress(const std::string &Name,
                                 bool CheckFunctionsOnly) {
...
JITSymbol MCODEIT::findSymbol(const std::string &Name,
                            bool CheckFunctionsOnly) {
  std::lock_guard<sys::Mutex> locked(lock);
...
uint64_t MCODEIT::getGlobalValueAddress(const std::string &Name) {
  std::lock_guard<sys::Mutex> locked(lock);
...
uint64_t MCODEIT::getFunctionAddress(const std::string &Name) {
  std::lock_guard<sys::Mutex> locked(lock);
...
void *MCODEIT::getPointerToFunction(Function *F) {
  std::lock_guard<sys::Mutex> locked(lock);
...
void MCODEIT::runStaticConstructorsDestructorsInModulePtrSet(
...
void MCODEIT::runStaticConstructorsDestructors(bool isDtors) {
...
Function *MCODEIT::FindFunctionNamedInModulePtrSet(StringRef FnName,
...
GlobalVariable *MCODEIT::FindGlobalVariableNamedInModulePtrSet(StringRef Name,
                                                             bool AllowInternal,
                                                             ModulePtrSet::iterator I,
                                                             ModulePtrSet::iterator E) {
  for (; I != E; ++I) {
...
Function *MCODEIT::FindFunctionNamed(StringRef FnName) {
...
GlobalVariable *MCODEIT::FindGlobalVariableNamed(StringRef Name, bool AllowInternal) {
...
GenericValue MCODEIT::runFunction(Function *F, ArrayRef<GenericValue> ArgValues) {
  ...
  report_fatal_error("MCODEIT::runFunction does not support full-featured "
                     "argument passing. Please use "
                     "ExecutionEngine::getFunctionAddress and cast the result "
                     "to the desired function pointer type.");
}

void *MCODEIT::getPointerToNamedFunction(StringRef Name, bool AbortOnFailure) {
...
void MCODEIT::RegisterJITEventListener(JITEventListener *L) {
  if (!L)
    return;
  std::lock_guard<sys::Mutex> locked(lock);
...
void MCODEIT::UnregisterJITEventListener(JITEventListener *L) {
  if (!L)
    return;
  std::lock_guard<sys::Mutex> locked(lock);
...
void MCODEIT::notifyObjectLoaded(const object::ObjectFile &Obj,
...
void MCODEIT::notifyFreeingObject(const object::ObjectFile &Obj) {
  uint64_t Key =
      static_cast<uint64_t>(reinterpret_cast<uintptr_t>(Obj.getData().data()));
  std::lock_guard<sys::Mutex> locked(lock);
