...
#include "vm/core/ExecutionEngine/Orc/SimpleRemoteEPC.h"
#include "vm/core/ExecutionEngine/Orc/EPCGenericJITLinkMemoryManager.h"
  ...
  DylibMgr.lookupAsync(Element.Handle, Element.Symbols,
                       [&DylibMgr, Request, Complete = std::move(Complete),
                        Result = std::move(Result)](auto R) mutable {
                         ...
                         toolchain::append_range(Result.back(), *R);
