...
#include "vm/core/ExecutionEngine/Orc/TargetProcess/JITLoaderPerf.h"

#include "vm/core/ExecutionEngine/Orc/Shared/PerfSharedStructs.h"
...
static Error OpenMarker(PerfState &State) {
  ...
  if (State.MarkerAddr == MAP_FAILED)
    return make_error<toolchain::StringError>("could not mmap JIT marker",
                                         inconvertibleErrorCode());
...
static Expected<Header> FillMachine(PerfState &State) {
  ...
  // signature.
  if (!MB)
    return make_error<toolchain::StringError>("could not open /proc/self/exe",
                                         MB.getError());
  ...
  if (Id[0] != 0x7f || Id[1] != 'E' || Id[2] != 'L' || Id[3] != 'F')
    return make_error<toolchain::StringError>("invalid ELF signature",
                                         inconvertibleErrorCode());
...
extern "C" toolchain::orc::shared::CWrapperFunctionBuffer
