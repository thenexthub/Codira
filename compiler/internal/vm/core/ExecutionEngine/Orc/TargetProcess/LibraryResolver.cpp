...
#include "vm/core/ExecutionEngine/Orc/TargetProcess/LibraryResolver.h"
#include "vm/core/ExecutionEngine/Orc/TargetProcess/LibraryScanner.h"
...
static StringRef GetGnuHashSection(toolchain::object::ObjectFile *file) {
  for (auto S : file->sections()) {
    StringRef name = toolchain::cantFail(S.getName());
    if (name == ".gnu.hash") {
      return toolchain::cantFail(S.getContents());
    }
  }
  return "";
}
...
static bool MayExistInElfObjectFile(toolchain::object::ObjectFile *soFile,
                                    StringRef Sym) {
