...
static Error deregisterFrameWrapper(const void *P) {
  ...
  if (!DeregisterFrame)
    *(void **)&DeregisterFrame =
        toolchain::sys::DynamicLibrary::SearchForAddressOfSymbol(
            "__deregister_frame");
...
static Error registerEHFrameWrapper(ExecutorAddrRange EHFrame) {
  return toolchain::orc::registerEHFrameSection(EHFrame.Start.toPtr<const void *>(),
                                           EHFrame.size());
}

static Error deregisterEHFrameWrapper(ExecutorAddrRange EHFrame) {
  return toolchain::orc::deregisterEHFrameSection(
