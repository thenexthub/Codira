...
SymbolsNotFound::SymbolsNotFound(std::shared_ptr<SymbolStringPool> SSP,
    ...
    : SSP(std::move(SSP)) {
  toolchain::append_range(this->Symbols, Symbols);
  assert(!this->Symbols.empty() && "Can not fail to resolve an empty set");
}
...
public:
  ...
  //        https://github.com/toolchain/toolchain-project/issues/55576.
...
void JITDylib::removeGenerator(DefinitionGenerator &G) {
  ...
  ES.runSessionLocked([&] {
    assert(State == Open && "JD is defunct");
    auto I = toolchain::find_if(DefGenerators,
...
void JITDylib::setLinkOrder(JITDylibSearchOrder NewLinkOrder,
  ...
  ES.runSessionLocked([&]() {
    ...
    if (LinkAgainstThisJITDylibFirst) {
      ...
      toolchain::append_range(LinkOrder, NewLinkOrder);
    } else
...
void JITDylib::addToLinkOrder(const JITDylibSearchOrder &NewLinks) {
  ES.runSessionLocked([&]() {
    for (auto &KV : NewLinks) {
      ...
      if (toolchain::is_contained(LinkOrder, KV))
        continue;
...
void JITDylib::removeFromLinkOrder(JITDylib &JD) {
  ES.runSessionLocked([&]() {
    assert(State == Open && "JD is defunct");
    auto I = toolchain::find_if(LinkOrder,
...
void JITDylib::MaterializingInfo::addQuery(
  ...
  auto I = toolchain::lower_bound(
      toolchain::reverse(PendingQueries), Q->getRequiredState(),
...
void ExecutionSession::deregisterResourceManager(ResourceManager &RM) {
  runSessionLocked([&] {
    ...
    if (ResourceManagers.back() == &RM)
      ...
      auto I = toolchain::find(ResourceManagers, &RM);
  ...
  return ES.runSessionLocked([&]() -> Expected<std::vector<JITDylibSP>> {
    ...
    for (auto &JD : JDs) {
      ...
      while (!WorkStack.empty()) {
        ...
        for (auto &KV : toolchain::reverse(Result.back()->LinkOrder)) {
          auto &JD = *KV.first;
  ...
  while (IPLS->CurSearchOrderIndex != IPLS->SearchOrder.size()) {
    ...
    if (IPLS->NewJITDylib) {
      ...
      runSessionLocked([&] {
        ...
        toolchain::append_range(IPLS->CurDefGeneratorStack,
...
ExecutionSession::IL_emit(MaterializationResponsibility &MR,
  ...
  // std::erase_if is not available in C++17, and toolchain::erase_if does not work
  // here.
  ...
  LLVM_DEBUG({
    ...
    for (auto &SN : SR.superNodes()) {
      ...
      auto SortedLibs = [](WaitingOnGraph::ContainerElementsMap &C) {
        ...
        for (auto &[JD, _] : C)
          JDs.push_back(JD);
        toolchain::sort(JDs, [](const JITDylib *LHS, const JITDylib *RHS) {
      ...
      auto SortedNames = [](WaitingOnGraph::ElementSet &Elems) {
        ...
        toolchain::sort(Names, [](const NonOwningSymbolStringPtr &LHS,
