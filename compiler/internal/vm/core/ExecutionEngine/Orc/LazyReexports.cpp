...
#include "vm/core/ExecutionEngine/Orc/OrcABISupport.h"
#include "vm/core/ExecutionEngine/Orc/Shared/SimplePackedSerialization.h"
...
void LazyReexportsManager::handleTransferResources(JITDylib &JD,
  ...
  if (I != KeyToReentryAddrs.end()) {
    ...
    if (J == KeyToReentryAddrs.end()) {
      ...
      toolchain::append_range(DstAddrs, SrcAddrs);
      KeyToReentryAddrs.erase(I);
    }
    if (L)
...
void SimpleLazyReexportsSpeculator::onLazyReexportsTransfered(
  ...
  if (K == MapForJD.end()) {
    ...
    toolchain::append_range(DstNames, SrcNames);
    MapForJD.erase(J);
  }
}
