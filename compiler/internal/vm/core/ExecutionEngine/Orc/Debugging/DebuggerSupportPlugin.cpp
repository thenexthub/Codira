...
public:
  ...
  Error startSynthesis() override {
    ...
    if (!DebugLineSectionData.empty()) {
      assert((G.getEndianness() == toolchain::endianness::big ||
      ...
      DWARFDataExtractor DebugLineData(
          DebugLineSectionData, G.getEndianness() == toolchain::endianness::little,
          G.getPointerSize());
      uint64_t Offset = 0;
...
void GDBJITDebugInfoRegistrationPlugin::modifyPassConfigForMachO(
  ...
  case Triple::aarch64:
    ...
    assert(LG.getEndianness() == toolchain::endianness::little &&
  ...
  default:
    ...
    LLVM_DEBUG({
      dbgs() << "GDBJITDebugInfoRegistrationPlugin skipping unsupported "
             ...
             << (LG.getEndianness() == toolchain::endianness::big ? "big" : "little")
             << ")\n";
    });
    return;
  }
