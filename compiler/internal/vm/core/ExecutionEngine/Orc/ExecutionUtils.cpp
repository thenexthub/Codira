...
#include "vm/core/ExecutionEngine/Orc/ObjectFileInterface.h"
...
CtorDtorIterator::Element CtorDtorIterator::operator*() const {
  ...
  assert(CS && "Unrecognized type in toolchain.global_ctors/toolchain.global_dtors");
...
iterator_range<CtorDtorIterator> getConstructors(const Module &M) {
  const GlobalVariable *CtorsList = M.getNamedGlobal("toolchain.global_ctors");
  return make_range(CtorDtorIterator(CtorsList, false),
                    CtorDtorIterator(CtorsList, true));
}

iterator_range<CtorDtorIterator> getDestructors(const Module &M) {
  const GlobalVariable *DtorsList = M.getNamedGlobal("toolchain.global_dtors");
