...
#include "vm/core/ExecutionEngine/Orc/COFFPlatform.h"
#include "vm/core/ExecutionEngine/Orc/EHFrameRegistrationPlugin.h"
#include "vm/core/ExecutionEngine/Orc/ELFNixPlatform.h"
#include "vm/core/ExecutionEngine/Orc/EPCDynamicLibrarySearchGenerator.h"
#include "vm/core/ExecutionEngine/Orc/ExecutorProcessControl.h"
#include "vm/core/ExecutionEngine/Orc/MachOPlatform.h"
...
#include "vm/core/ExecutionEngine/Orc/SelfExecutorProcessControl.h"
...
Function *addHelperAndWrapper(Module &M, StringRef WrapperName,
  ...
  toolchain::append_range(HelperArgTypes, WrapperFnType->params());
  auto *HelperFnType =
  ...
  toolchain::append_range(HelperArgs, HelperPrefixArgs);
...
/// This transform parses toolchain.global_ctors to produce a single initialization
/// function for the module, records the function, then deletes
/// toolchain.global_ctors.
...
///
/// Scrapes toolchain.global_ctors and toolchain.global_dtors and replaces them with
...
private:
  Expected<std::vector<ExecutorAddr>> getInitializers(JITDylib &JD) {
    ...
    LLVM_DEBUG({
      ...
      for (auto &JD : toolchain::reverse(DFSLinkOrder))
        dbgs() << "\"" << JD->getName() << "\" ";
      dbgs() << "]\n";
...
GlobalCtorDtorScraper::operator()(ThreadSafeModule TSM,
  ...
  auto Err = TSM.withModuleDo([&](Module &M) -> Error {
    ...
    auto *GlobalCtors = M.getNamedGlobal("toolchain.global_ctors");
    auto *GlobalDtors = M.getNamedGlobal("toolchain.global_dtors");

    auto RegisterCOrDtors = [&](GlobalVariable *GlobalCOrDtors,
                                bool isCtor) -> Error {
      // If there's no toolchain.global_c/dtor or it's just a decl then skip.
      ...
      toolchain::stable_sort(InitsOrDeInits, toolchain::less_second());
...
Error ORCPlatformSupport::initialize(orc::JITDylib &JD) {
  using toolchain::orc::shared::SPSExecutorAddr;
  using toolchain::orc::shared::SPSString;
