...
#include "vm/core/ObjectYAML/CodeViewYAMLTypes.h"
...
using namespace vm::core::CodeViewYAML::detail;
...
void ScalarTraits<GUID>::output(const GUID &G, void *, toolchain::raw_ostream &OS) {
  OS << G;
}
...
toolchain::CodeViewYAML::fromDebugT(ArrayRef<uint8_t> DebugTorP,
                               StringRef SectionName) {
  ExitOnError Err("Invalid " + std::string(SectionName) + " section!");
  BinaryStreamReader Reader(DebugTorP, toolchain::endianness::little);
  CVTypeArray Types;
  uint32_t Magic;
...
ArrayRef<uint8_t> toolchain::CodeViewYAML::toDebugT(ArrayRef<LeafRecord> Leafs,
  ...
  BinaryStreamWriter Writer(Output, toolchain::endianness::little);
