...
#include "vm/core/ObjectYAML/DXContainerYAML.h"
...
readDescriptorRanges(DXContainerYAML::RootParameterHeaderYaml &Header,
  ...
  toolchain::Expected<object::DirectX::DescriptorTable<T>> TableOrErr =
      DTV->read<T>();
...
#define DESCRIPTOR_RANGE_FLAG(Num, Enum, Flag)                                 \
  NewR.Enum =                                                                  \
      (R.Flags & toolchain::to_underlying(dxbc::DescriptorRangeFlags::Enum)) != 0;
#include "vm/core/BinaryFormat/DXContainerConstants.def"
    }
...
toolchain::Expected<DXContainerYAML::RootSignatureYamlDesc>
DXContainerYAML::RootSignatureYamlDesc::create(
  ...
  for (const dxbc::RTS0::v1::RootParameterHeader &PH : Data.param_headers()) {
    ...
    toolchain::Expected<object::DirectX::RootParameterView> ParamViewOrErr =
    ...
    if (auto *RCV = dyn_cast<object::DirectX::RootConstantView>(&ParamView)) {
      toolchain::Expected<dxbc::RTS0::v1::RootConstants> ConstantsOrErr =
          RCV->read();
      ...
      toolchain::Expected<dxbc::RTS0::v2::RootDescriptor> DescriptorOrErr =
          RDV->read(Version);
...
#define ROOT_DESCRIPTOR_FLAG(Num, Enum, Flag)                                  \
  YamlDescriptor.Enum =                                                        \
      (Descriptor.Flags &                                                      \
       toolchain::to_underlying(dxbc::RootDescriptorFlags::Enum)) > 0;
#include "vm/core/BinaryFormat/DXContainerConstants.def"
      }
    } else if (auto *DTV =
...
#define STATIC_SAMPLER_FLAG(Num, Enum, Flag)                                   \
  NewS.Enum = (S.Flags & toolchain::to_underlying(dxbc::StaticSamplerFlags::Enum));
#include "vm/core/BinaryFormat/DXContainerConstants.def"
    }
...
#define ROOT_SIGNATURE_FLAG(Num, Val)                                          \
  RootSigDesc.Val = (Flags & toolchain::to_underlying(dxbc::RootFlags::Val)) > 0;
#include "vm/core/BinaryFormat/DXContainerConstants.def"
  return RootSigDesc;
}
...
void MappingTraits<DXContainerYAML::PSVInfo>::mapping(
  ...
  toolchain::scope_exit RestoreContext([&]() { IO.setContext(OldContext); });
  ...
  if (PSV.Info.UsesViewID) {
    MutableArrayRef<SmallVector<toolchain::yaml::Hex32>> MutableOutMasks(
  ...
  MutableArrayRef<SmallVector<toolchain::yaml::Hex32>> MutableIOMap(
      PSV.InputOutputMap);
...
void MappingTraits<toolchain::DXContainerYAML::DescriptorRangeYaml>::mapping(
    IO &IO, toolchain::DXContainerYAML::DescriptorRangeYaml &R) {
...
void MappingTraits<toolchain::DXContainerYAML::DescriptorTableYaml>::mapping(
    IO &IO, toolchain::DXContainerYAML::DescriptorTableYaml &T) {
...
void MappingTraits<toolchain::DXContainerYAML::RootConstantsYaml>::mapping(
    IO &IO, toolchain::DXContainerYAML::RootConstantsYaml &C) {
...
void MappingTraits<toolchain::DXContainerYAML::RootDescriptorYaml>::mapping(
    IO &IO, toolchain::DXContainerYAML::RootDescriptorYaml &D) {
...
void MappingTraits<toolchain::DXContainerYAML::StaticSamplerYamlDesc>::mapping(
    IO &IO, toolchain::DXContainerYAML::StaticSamplerYamlDesc &S) {
