...
struct COFFParser {
  ...
  bool parseSections() {
    for (COFFYAML::Section &Sec : Obj.Sections) {
      ...
      if (Name.size() <= COFF::NameSize) {
        toolchain::copy(Name, Sec.Header.Name);
      } else {
        ...
        Sec.Header.Name[0] = '/';
        toolchain::copy(str, Sec.Header.Name + 1);
      }
  ...
  bool parseSymbols() {
    for (COFFYAML::Symbol &Sym : Obj.Symbols) {
      ...
      if (Name.size() <= COFF::NameSize) {
        toolchain::copy(Name, Sym.Header.Name);
      } else {
        // Add string to the string table and format the index for output.
        unsigned Index = getStringIndex(Name);
...
raw_ostream &operator<<(raw_ostream &OS,
  ...
  support::endian::write<value_type, toolchain::endianness::little>(Buffer,
                                                               BLE.Value);
...
bool yaml2coff(toolchain::COFFYAML::Object &Doc, raw_ostream &Out,
