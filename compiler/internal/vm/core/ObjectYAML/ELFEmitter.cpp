...
#include "vm/core/ObjectYAML/DWARFEmitter.h"
#include "vm/core/ObjectYAML/DWARFYAML.h"
...
public:
  ...
  template <typename T> void write(T Val, toolchain::endianness E) {
...
template <class ELFT> class ELFState {
  ...
  uint64_t alignToOffset(ContiguousBlobAccumulator &CBA, uint64_t Align,
                         std::optional<toolchain::yaml::Hex64> Offset);
...
std::string toolchain::ELFYAML::appendUniqueSuffix(StringRef Name,
                                              const Twine &Msg) {
...
StringRef toolchain::ELFYAML::dropUniqueSuffix(StringRef S) {
...
static uint64_t writeContent(ContiguousBlobAccumulator &CBA,
                             ...
                             const std::optional<toolchain::yaml::Hex64> &Size) {
...
ELFState<ELFT>::getPhdrFragments(const ELFYAML::ProgramHeader &Phdr,
  ...
  for (const ELFYAML::Chunk *C : Phdr.Chunks) {
    if (const ELFYAML::Fill *F = dyn_cast<ELFYAML::Fill>(C)) {
      Ret.push_back({*F->Offset, F->Size, toolchain::ELF::SHT_PROGBITS,
                     /*ShAddrAlign=*/1});
      continue;
    }
...
void ELFState<ELFT>::setProgramHeaderLayout(std::vector<Elf_Phdr> &PHeaders,
  ...
  for (auto &YamlPhdr : Doc.ProgramHeaders) {
    ...
    if (!toolchain::is_sorted(Fragments, [](const Fragment &A, const Fragment &B) {
    ...
    if (YamlPhdr.FileSize) {
      ...
      // segment.
      if (Fragments.back().Type != toolchain::ELF::SHT_NOBITS)
...
bool toolchain::ELFYAML::shouldAllocateFileSpace(
...
static bool isMips64EL(const ELFYAML::Object &Obj) {
  return Obj.getMachine() == toolchain::ELF::EM_MIPS &&
...
void ELFState<ELFT>::writeSectionContent(
    ...
    ContiguousBlobAccumulator &CBA) {
  assert((Section.Type == toolchain::ELF::SHT_REL ||
          Section.Type == toolchain::ELF::SHT_RELA ||
          Section.Type == toolchain::ELF::SHT_CREL) &&
  ...
  const bool IsCrel = Section.Type == toolchain::ELF::SHT_CREL;
  const bool IsRela = Section.Type == toolchain::ELF::SHT_RELA;
  ...
  const int Shift = toolchain::countr_zero(OffsetMask);
  if (IsCrel)
...
void ELFState<ELFT>::writeSectionContent(Elf_Shdr &SHeader,
  ...
  for (toolchain::yaml::Hex64 E : *Section.Entries) {
...
void ELFState<ELFT>::writeSectionContent(Elf_Shdr &SHeader,
                                         ...
                                         ContiguousBlobAccumulator &CBA) {
  assert(Section.Type == toolchain::ELF::SHT_GROUP &&
  ...
  for (const ELFYAML::SectionOrType &Member : *Section.Members) {
    ...
    if (Member.sectionNameOrType == "GRP_COMDAT")
      SectionIndex = toolchain::ELF::GRP_COMDAT;
    else
...
void ELFState<ELFT>::writeSectionContent(
  ...
  for (const auto &[Idx, E] : toolchain::enumerate(*Section.Entries)) {
    // Write version and feature values.
    if (Section.Type == toolchain::ELF::SHT_LLVM_BB_ADDR_MAP) {
      if (E.Version > 5)
    ...
    auto FeatureOrErr = toolchain::object::BBAddrMap::Features::decode(E.Feature);
    ...
    for (const ELFYAML::BBAddrMapEntry::BBRangeEntry &BBR : *E.BBRanges) {
      ...
      for (const ELFYAML::BBAddrMapEntry::BBEntry &BBE : *BBR.BBEntries) {
        ++TotalNumBlocks;
        if (Section.Type == toolchain::ELF::SHT_LLVM_BB_ADDR_MAP && E.Version > 1)
        ...
        if (FeatureOrErr->BBHash || BBE.Hash.has_value()) {
          uint64_t Hash =
              BBE.Hash.has_value() ? BBE.Hash.value() : toolchain::yaml::Hex64(0);
...
void ELFState<ELFT>::writeSectionContent(Elf_Shdr &SHeader,
  ...
  CBA.write<uint32_t>(
      Section.NBucket.value_or(toolchain::yaml::Hex64(Section.Bucket->size())),
      ELFT::Endianness);
  CBA.write<uint32_t>(
      Section.NChain.value_or(toolchain::yaml::Hex64(Section.Chain->size())),
      ELFT::Endianness);
...
void ELFState<ELFT>::writeSectionContent(Elf_Shdr &SHeader,
                                         const ELFYAML::MipsABIFlags &Section,
                                         ContiguousBlobAccumulator &CBA) {
  assert(Section.Type == toolchain::ELF::SHT_MIPS_ABIFLAGS &&
...
void ELFState<ELFT>::writeSectionContent(Elf_Shdr &SHeader,
                                         const ELFYAML::DynamicSection &Section,
                                         ContiguousBlobAccumulator &CBA) {
  assert(Section.Type == toolchain::ELF::SHT_DYNAMIC &&
...
void ELFState<ELFT>::writeSectionContent(Elf_Shdr &SHeader,
  ...
  for (toolchain::yaml::Hex64 Val : *Section.BloomFilter)
    CBA.write<uintX_t>(Val, ELFT::Endianness);

  // Write an array of hash buckets.
  for (toolchain::yaml::Hex32 Val : *Section.HashBuckets)
    CBA.write<uint32_t>(Val, ELFT::Endianness);

  // Write an array of hash values.
  for (toolchain::yaml::Hex32 Val : *Section.HashValues)
    CBA.write<uint32_t>(Val, ELFT::Endianness);
...
bool yaml2elf(toolchain::ELFYAML::Object &Doc, raw_ostream &Out, ErrorHandler EH,
              uint64_t MaxSize) {
