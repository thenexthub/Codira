...
#include "vm/core/ObjectYAML/CodeViewYAMLDebugSections.h"
...
#include "vm/core/ObjectYAML/CodeViewYAMLSymbols.h"
...
YAMLCoffSymbolRVASubsection::fromCodeViewSubsection(
  ...
  toolchain::append_range(Result->RVAs, Section);
  return Result;
}

Expected<std::vector<std::shared_ptr<DebugSubsection>>>
toolchain::CodeViewYAML::toCodeViewSubsectionList(
...
toolchain::CodeViewYAML::fromDebugS(ArrayRef<uint8_t> Data,
                               const StringsAndChecksumsRef &SC) {
  BinaryStreamReader Reader(Data, toolchain::endianness::little);
  uint32_t Magic;
...
void toolchain::CodeViewYAML::initializeStringsAndChecksums(
