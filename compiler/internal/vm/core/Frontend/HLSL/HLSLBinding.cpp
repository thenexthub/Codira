...
#include "vm/core/Frontend/HLSL/HLSLBinding.h"
...
BindingInfo BindingInfoBuilder::calculateBindingInfo(
    toolchain::function_ref<void(const BindingInfoBuilder &Builder,
                            const Binding &Overlapping)>
        ReportOverlap) {
  // sort all the collected bindings
  toolchain::stable_sort(Bindings);

  // remove duplicates
  Binding *NewEnd = toolchain::unique(Bindings);
