...
#include "vm/core/Frontend/HLSL/RootSignatureValidations.h"
...
raw_ostream &operator<<(toolchain::raw_ostream &OS, const FmtRange &Range) {
...
raw_ostream &operator<<(toolchain::raw_ostream &OS, FmtMDNode Fmt) {
...
extractEnumValue(MDNode *Node, unsigned int OpId, StringRef ErrText,
                 toolchain::function_ref<bool(uint32_t)> VerifyFn) {
...
validateDescriptorTableRegisterOverflow(const mcdxbc::DescriptorTable &Table,
  ...
  for (const mcdxbc::DescriptorRange &Range : Table.Ranges) {
    ...
    const uint64_t RangeBound = toolchain::hlsl::rootsig::computeRangeBound(
    ...
    const uint64_t OffsetBound =
        toolchain::hlsl::rootsig::computeRangeBound(Offset, Range.NumDescriptors);
    ...
    IsPrevUnbound =
        Range.NumDescriptors == toolchain::hlsl::rootsig::NumDescriptorsUnbounded;
  }
