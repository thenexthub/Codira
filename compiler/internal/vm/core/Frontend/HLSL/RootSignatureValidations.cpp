...
// https://github.com/toolchain/wg-hlsl/blob/main/proposals/0002-root-signature-in-clang.md#all-the-values-should-be-legal
...
bool verifyRootDescriptorFlag(uint32_t Version,
  ...
  if (popcount(toolchain::to_underlying(Flags & DataFlags)) > 1)
    return false;
...
bool verifyDescriptorRangeFlag(uint32_t Version, dxil::ResourceClass Type,
  ...
  if (popcount(toolchain::to_underlying(Flags & DescriptorFlags)) > 1)
    return false;
