...
#include "vm/core/Frontend/OpenMP/OMPContext.h"
...
template <typename T> static bool isSubset(ArrayRef<T> C0, ArrayRef<T> C1) {
  ...
  assert(toolchain::is_sorted(C0) && toolchain::is_sorted(C1) &&
...
isVariantApplicableInContextHelper(const VariantMatchInfo &VMI,
  ...
  for (unsigned Bit : VMI.RequiredTraits.set_bits()) {
    ...
    if (Property == TraitProperty::device_isa___ANY)
      IsActiveTrait = toolchain::all_of(VMI.ISATraits, [&](StringRef RawString) {
...
bool toolchain::omp::isVariantApplicableInContext(
...
int toolchain::omp::getBestVariantMatchForContext(
...
TraitSet toolchain::omp::getOpenMPContextTraitSetKind(StringRef S) {
...
TraitSet
toolchain::omp::getOpenMPContextTraitSetForSelector(TraitSelector Selector) {
  switch (Selector) {
...
TraitSet
toolchain::omp::getOpenMPContextTraitSetForProperty(TraitProperty Property) {
  switch (Property) {
#define OMP_TRAIT_PROPERTY(Enum, TraitSetEnum, TraitSelectorEnum, Str)         \
...
StringRef toolchain::omp::getOpenMPContextTraitSetName(TraitSet Kind) {
  switch (Kind) {
...
TraitSelector toolchain::omp::getOpenMPContextTraitSelectorKind(StringRef S,
                                                           TraitSet Set) {
...
TraitSelector
toolchain::omp::getOpenMPContextTraitSelectorForProperty(TraitProperty Property) {
  switch (Property) {
#define OMP_TRAIT_PROPERTY(Enum, TraitSetEnum, TraitSelectorEnum, Str)         \
  case TraitProperty::Enum:                                                    \
...
StringRef toolchain::omp::getOpenMPContextTraitSelectorName(TraitSelector Kind) {
  switch (Kind) {
#define OMP_TRAIT_SELECTOR(Enum, TraitSetEnum, Str, ReqProp)                   \
  case TraitSelector::Enum:                                                    \
    return Str;
...
TraitProperty toolchain::omp::getOpenMPContextTraitPropertyKind(
...
TraitProperty
toolchain::omp::getOpenMPContextTraitPropertyForSelector(TraitSelector Selector) {
...
StringRef toolchain::omp::getOpenMPContextTraitPropertyName(TraitProperty Kind,
                                                       StringRef RawString) {
...
StringRef toolchain::omp::getOpenMPContextTraitPropertyFullName(TraitProperty Kind) {
  switch (Kind) {
#define OMP_TRAIT_PROPERTY(Enum, TraitSetEnum, TraitSelectorEnum, Str)         \
  case TraitProperty::Enum:                                                    \
...
bool toolchain::omp::isValidTraitSelectorForTraitSet(TraitSelector Selector,
                                                TraitSet Set,
...
bool toolchain::omp::isValidTraitPropertyForTraitSetAndSelector(
...
std::string toolchain::omp::listOpenMPContextTraitSets() {
  std::string S;
#define OMP_TRAIT_SET(Enum, Str)                                               \
...
std::string toolchain::omp::listOpenMPContextTraitSelectors(TraitSet Set) {
  std::string S;
#define OMP_TRAIT_SELECTOR(Enum, TraitSetEnum, Str, ReqProp)                   \
...
std::string
toolchain::omp::listOpenMPContextTraitProperties(TraitSet Set,
