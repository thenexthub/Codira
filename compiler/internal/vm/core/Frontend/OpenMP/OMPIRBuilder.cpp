...
#include "vm/core/Analysis/CodeMetrics.h"
...
#include "vm/core/Frontend/OpenMP/OMPGridValues.h"
...
#include "vm/core/Transforms/Utils/LoopPeel.h"
#include "vm/core/Transforms/Utils/UnrollLoop.h"
...
static void restoreIPandDebugLoc(toolchain::IRBuilderBase &Builder,
                                 toolchain::IRBuilderBase::InsertPoint IP) {
  Builder.restoreIP(IP);
  toolchain::BasicBlock *BB = Builder.GetInsertBlock();
  toolchain::BasicBlock::iterator I = Builder.GetInsertPoint();
...
getOpenMPBaseScheduleType(toolchain::omp::ScheduleKind ClauseKind, bool HasChunks,
  ...
  case OMP_SCHEDULE_Auto:
    return toolchain::omp::OMPScheduleType::BaseAuto;
...
void toolchain::spliceBB(IRBuilderBase::InsertPoint IP, BasicBlock *New,
...
void toolchain::spliceBB(IRBuilder<> &Builder, BasicBlock *New, bool CreateBranch) {
...
BasicBlock *toolchain::splitBB(IRBuilderBase::InsertPoint IP, bool CreateBranch,
                          DebugLoc DL, toolchain::Twine Name) {
  BasicBlock *Old = IP.getBlock();
...
BasicBlock *toolchain::splitBB(IRBuilderBase &Builder, bool CreateBranch,
...
BasicBlock *toolchain::splitBB(IRBuilder<> &Builder, bool CreateBranch,
                          toolchain::Twine Name) {
  DebugLoc DebugLoc = Builder.getCurrentDebugLocation();
  BasicBlock *New = splitBB(Builder.saveIP(), CreateBranch, DebugLoc, Name);
  if (CreateBranch)
...
BasicBlock *toolchain::splitBBWithSuffix(IRBuilderBase &Builder, bool CreateBranch,
                                    toolchain::Twine Suffix) {
  BasicBlock *Old = Builder.GetInsertBlock();
...
Value *createFakeIntVal(IRBuilderBase &Builder,
                        ...
                        toolchain::SmallVectorImpl<Instruction *> &ToBeDeleted,
...
#include "vm/core/Frontend/OpenMP/OMPKinds.def"
...
Function *OpenMPIRBuilder::getOrCreateRuntimeFunctionPtr(RuntimeFunction FnID) {
  ...
  auto *Fn = dyn_cast<toolchain::Function>(RTLFn.getCallee());
...
static void raiseUserConstantDataAllocasToEntryBlock(IRBuilderBase &Builder,
  ...
  for (auto Block = std::next(Function->begin(), 1); Block != Function->end();
    ...
    for (auto Inst = Block->getReverseIterator()->begin();
      ...
      if (auto *AllocaInst = dyn_cast_if_present<toolchain::AllocaInst>(Inst)) {
        Inst++;
...
static void hoistNonEntryAllocasToEntryBlock(toolchain::BasicBlock &Block) {
  toolchain::SmallVector<toolchain::Instruction *> AllocasToMove;

  auto ShouldHoistAlloca = [](const toolchain::AllocaInst &AllocaInst) {
  ...
  for (toolchain::Instruction &Inst : Block)
    if (auto *AllocaInst = toolchain::dyn_cast<toolchain::AllocaInst>(&Inst))
  ...
  for (toolchain::Instruction *AllocaInst : AllocasToMove)
...
void OpenMPIRBuilder::finalize(Function *Fn) {
  ...
  for (OutlineInfo &OI : OutlineInfos) {
    ...
    if (OI.FixUpNonEntryAllocas) {
      ...
      for (toolchain::BasicBlock &BB : *OutlinedFn)
  ...
  if (Config.EmitLLVMUsedMetaInfo.value_or(false)) {
    ...
    emitUsed("toolchain.compiler.used", LLVMCompilerUsed);
  }
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createParallel(
  ...
  auto PrivHelper = [&](Value &V) -> Error {
    ...
    if (!V.getType()->isPointerTy()) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Forwarding input as pointer: " << V << "\n");
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createTaskloop(
    ...
    toolchain::function_ref<toolchain::Expected<toolchain::CanonicalLoopInfo *>()> LoopInfo,
  ...
  toolchain::Expected<toolchain::CanonicalLoopInfo *> result = LoopInfo();
  if (!result) {
    return result.takeError();
  }

  toolchain::CanonicalLoopInfo *CLI = result.get();
  OutlineInfo OI;
  ...
  OI.PostOutlineCB = [this, Ident, LBVal, UBVal, StepVal, Untied,
    ...
    for (Instruction *I : toolchain::reverse(ToBeDeleted)) {
      I->eraseFromParent();
    }
  };
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createReductions(
  ...
  if (CanGenerateAtomic && toolchain::none_of(IsByRef, [](bool P) { return P; })) {
...
static toolchain::CallInst *emitNoUnwindRuntimeCall(IRBuilder<> &Builder,
  ...
  toolchain::CallInst *Call = Builder.CreateCall(
      Callee, Args, SmallVector<toolchain::OperandBundleDef, 1>(), Name);
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createScan(
    const LocationDescription &Loc, InsertPointTy AllocaIP,
    ArrayRef<toolchain::Value *> ScanVars, ArrayRef<toolchain::Type *> ScanVarsType,
    bool IsInclusive, ScanInfo *ScanRedInfo) {
  if (ScanRedInfo->OMPFirstScanLoop) {
    toolchain::Error Err = emitScanBasedDirectiveDeclsIR(AllocaIP, ScanVars,
  ...
  toolchain::Value *IV = ScanRedInfo->IV;
  ...
  toolchain::Value *CmpI = Builder.getInt1(true);
...
Error OpenMPIRBuilder::emitScanBasedDirectiveDeclsIR(
  ...
  toolchain::Value *FilterVal = Builder.getInt32(0);
  ...
  if (InputBB->getTerminator())
    Builder.SetInsertPoint(Builder.GetInsertBlock()->getTerminator());
  AfterIP = createBarrier(Builder.saveIP(), toolchain::omp::OMPD_barrier);
  if (!AfterIP)
    return AfterIP.takeError();
  Builder.restoreIP(*AfterIP);
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::emitScanReduction(
    ...
    ArrayRef<toolchain::OpenMPIRBuilder::ReductionInfo> ReductionInfos,
  ...
  auto BodyGenCB = [&](InsertPointTy AllocaIP,
    ...
    toolchain::BasicBlock *LoopBB =
        BasicBlock::Create(CurFn->getContext(), "omp.outer.log.scan.body");
    toolchain::BasicBlock *ExitBB =
        splitBB(Builder, false, "omp.outer.log.scan.exit");
    toolchain::Function *F = toolchain::Intrinsic::getOrInsertDeclaration(
        Builder.GetInsertBlock()->getModule(),
        (toolchain::Intrinsic::ID)toolchain::Intrinsic::log2, Builder.getDoubleTy());
    toolchain::BasicBlock *InputBB = Builder.GetInsertBlock();
    toolchain::Value *Arg =
        Builder.CreateUIToFP(ScanRedInfo->Span, Builder.getDoubleTy());
    toolchain::Value *LogVal = emitNoUnwindRuntimeCall(Builder, F, Arg, "");
    F = toolchain::Intrinsic::getOrInsertDeclaration(
        Builder.GetInsertBlock()->getModule(),
        (toolchain::Intrinsic::ID)toolchain::Intrinsic::ceil, Builder.getDoubleTy());
    LogVal = emitNoUnwindRuntimeCall(Builder, F, LogVal, "");
    LogVal = Builder.CreateFPToUI(LogVal, Builder.getInt32Ty());
    toolchain::Value *NMin1 = Builder.CreateNUWSub(
        ScanRedInfo->Span,
        toolchain::ConstantInt::get(ScanRedInfo->Span->getType(), 1));
    ...
    Counter->addIncoming(toolchain::ConstantInt::get(Builder.getInt32Ty(), 0),
                         InputBB);
    Pow2K->addIncoming(toolchain::ConstantInt::get(Builder.getInt32Ty(), 1),
                       InputBB);
    ...
    toolchain::Value *CmpI = Builder.CreateICmpUGE(NMin1, Pow2K);
    ...
    toolchain::Value *NextIVal = Builder.CreateNUWSub(
        IVal, toolchain::ConstantInt::get(Builder.getInt32Ty(), 1));
    IVal->addIncoming(NextIVal, Builder.GetInsertBlock());
    CmpI = Builder.CreateICmpUGE(NextIVal, Pow2K);
    Builder.CreateCondBr(CmpI, InnerLoopBB, InnerExitBB);
    emitBlock(InnerExitBB, CurFn);
    toolchain::Value *Next = Builder.CreateNUWAdd(
        Counter, toolchain::ConstantInt::get(Counter->getType(), 1));
    Counter->addIncoming(Next, Builder.GetInsertBlock());
    // pow2k <<= 1;
    toolchain::Value *NextPow2K = Builder.CreateShl(Pow2K, 1, "", /*HasNUW=*/true);
    Pow2K->addIncoming(NextPow2K, Builder.GetInsertBlock());
    toolchain::Value *Cmp = Builder.CreateICmpNE(Next, LogVal);
...
Error OpenMPIRBuilder::emitScanBasedDirectiveIR(
    toolchain::function_ref<Error()> InputLoopGen,
    toolchain::function_ref<Error(LocationDescription Loc)> ScanLoopGen,
    ScanInfo *ScanRedInfo) {
...
Expected<SmallVector<toolchain::CanonicalLoopInfo *>>
...
static void applyParallelAccessesMetadata(CanonicalLoopInfo *CLI,
  ...
  LoopMDList.push_back(MDNode::get(
      Ctx, {MDString::get(Ctx, "toolchain.loop.parallel_accesses"), AccessGroup}));
}
...
static void removeUnusedBlocksFromParent(ArrayRef<BasicBlock *> BBs) {
  SmallPtrSet<BasicBlock *, 6> BBsToErase(toolchain::from_range, BBs);
...
/// Attach toolchain.access.group metadata to the memref instructions of \p Block
static void addAccessGroupMetadata(BasicBlock *Block, MDNode *AccessGroup,
                                   LoopInfo &LI) {
...
void OpenMPIRBuilder::unrollLoopFull(DebugLoc, CanonicalLoopInfo *Loop) {
  ...
  addLoopMetadata(
      Loop, {MDNode::get(Ctx, MDString::get(Ctx, "toolchain.loop.unroll.enable")),
             MDNode::get(Ctx, MDString::get(Ctx, "toolchain.loop.unroll.full"))});
}
...
void OpenMPIRBuilder::unrollLoopHeuristic(DebugLoc, CanonicalLoopInfo *Loop) {
  ...
  addLoopMetadata(
      Loop, {
                MDNode::get(Ctx, MDString::get(Ctx, "toolchain.loop.unroll.enable")),
            });
}
...
void OpenMPIRBuilder::createIfVersion(CanonicalLoopInfo *CanonicalLoop,
  ...
  toolchain::LLVMContext &C = Cond->getContext();
  toolchain::BasicBlock *ThenBlock = toolchain::BasicBlock::Create(
      C, NamePrefix + ".if.then", Cond->getParent(), Cond->getNextNode());
  toolchain::BasicBlock *ElseBlock = toolchain::BasicBlock::Create(
...
void OpenMPIRBuilder::applySimd(CanonicalLoopInfo *CanonicalLoop,
  ...
  // only for getting toolchain::Loop which corresponds to given CanonicalLoopInfo
  ...
  if (IfCond) {
    // we can still add toolchain.loop.parallel_access
  ...
  LoopMDList.push_back(MDNode::get(
      Ctx, {MDString::get(Ctx, "toolchain.loop.vectorize.enable"), BoolConst}));
  ...
  if (Simdlen || Safelen) {
    ...
    LoopMDList.push_back(
        MDNode::get(Ctx, {MDString::get(Ctx, "toolchain.loop.vectorize.width"),
...
createTargetMachine(Function *F, CodeGenOptLevel OptLevel) {
  ...
  std::string Error;
  const toolchain::Target *TheTarget = TargetRegistry::lookupTarget(Triple, Error);
  if (!TheTarget)
    return {};

  toolchain::TargetOptions Options;
  ...
  if (!UnrolledCLI) {
    ...
    if (Factor >= 1) {
      ...
      LoopMetadata.push_back(MDNode::get(
          Ctx, {MDString::get(Ctx, "toolchain.loop.unroll.count"), FactorConst}));
    }
...
OpenMPIRBuilder::createCopyPrivate(const LocationDescription &Loc,
                                   toolchain::Value *BufSize, toolchain::Value *CpyBuf,
                                   toolchain::Value *CpyFn, toolchain::Value *DidIt) {
  if (!updateToLocation(Loc))
    return Loc.IP;
  ...
  toolchain::Value *DidItLD = Builder.CreateLoad(Builder.getInt32Ty(), DidIt);
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createSingle(
    ...
    FinalizeCallbackTy FiniCB, bool IsNowait, ArrayRef<toolchain::Value *> CPVars,
    ArrayRef<toolchain::Function *> CPFuncs) {
  ...
  toolchain::Value *DidIt = nullptr;
  if (!CPVars.empty()) {
    DidIt = Builder.CreateAlloca(toolchain::Type::getInt32Ty(Builder.getContext()));
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::createCritical(
  ...
  SmallVector<toolchain::Value *, 4> EnterArgs(std::begin(Args), std::end(Args));
...
OpenMPIRBuilder::createOrderedDepend(const LocationDescription &Loc,
                                     ...
                                     ArrayRef<toolchain::Value *> StoreValues,
                                     const Twine &Name, bool IsDependSource) {
  assert(
      toolchain::all_of(StoreValues,
...
OpenMPIRBuilder::InsertPointTy OpenMPIRBuilder::createCopyinClauseBlocks(
    ...
    toolchain::IntegerType *IntPtrTy, bool BranchtoEnd) {
  if (!IP.isSet())
    return IP;
...
CallInst *OpenMPIRBuilder::createCachedThreadPrivate(
    const LocationDescription &Loc, toolchain::Value *Pointer,
    toolchain::ConstantInt *Size, const toolchain::Twine &Name) {
  IRBuilder<>::InsertPointGuard IPG(Builder);
  updateToLocation(Loc);
  ...
  toolchain::Value *Args[] = {Ident, ThreadId, Pointer, Size, ThreadPrivateCache};
...
OpenMPIRBuilder::InsertPointTy OpenMPIRBuilder::createTargetInit(
    const LocationDescription &Loc,
    const toolchain::OpenMPIRBuilder::TargetKernelDefaultAttrs &Attrs) {
...
static void updateNVPTXAttr(Function &Kernel, StringRef Name, int32_t Value,
  ...
  Kernel.addFnAttr(Name, toolchain::utostr(Value));
}
...
OpenMPIRBuilder::readThreadBoundsForKernel(const Triple &T, Function &Kernel) {
  ...
  if (T.isAMDGPU()) {
    ...
    int32_t LB, UB;
    if (!toolchain::to_integer(UBStr, UB, 10))
      return {0, ThreadLimit};
    UB = ThreadLimit ? std::min(ThreadLimit, UB) : UB;
    if (!toolchain::to_integer(LBStr, LB, 10))
      return {0, UB};
    return {LB, UB};
  }
...
void OpenMPIRBuilder::writeThreadBoundsForKernel(const Triple &T,
  ...
  if (T.isAMDGPU()) {
    Kernel.addFnAttr("amdgpu-flat-work-group-size",
                     toolchain::utostr(LB) + "," + toolchain::utostr(UB));
    return;
  }
...
void OpenMPIRBuilder::writeTeamsForKernel(const Triple &T, Function &Kernel,
  ...
  if (T.isNVPTX())
    if (UB > 0)
      Kernel.addFnAttr("nvvm.maxclusterrank", toolchain::utostr(UB));
  if (T.isAMDGPU())
    Kernel.addFnAttr("amdgpu-max-num-workgroups", toolchain::utostr(LB) + ",1,1");
  ...
  auto BeginThenGen = [&](InsertPointTy AllocaIP,
    ...
    if (IsStandAlone) {
      ...
      auto TaskBodyCB = [&](Value *, Value *,
        ...
        if (Info.HasNoWait) {
          OffloadingArgs.append({toolchain::Constant::getNullValue(Int32),
                                 toolchain::Constant::getNullValue(VoidPtr),
                                 toolchain::Constant::getNullValue(Int32),
                                 toolchain::Constant::getNullValue(VoidPtr)});
        }
...
static void FixupDebugInfoForOutlinedFunction(
  ...
  auto GetUpdatedDIVariable = [&](DILocalVariable *OldVar, unsigned arg) {
    ...
    NewVar = toolchain::DILocalVariable::get(
  ...
  for (Instruction &I : instructions(Func)) {
    assert(!isa<toolchain::DbgVariableIntrinsic>(&I) &&
...
static Expected<Function *> createOutlinedFunction(
  ...
  if (OMPBuilder.Config.isTargetDevice()) {
    ...
    OMPBuilder.emitUsed("toolchain.compiler.used", {ExecMode});
  }
  ...
  toolchain::OpenMPIRBuilder::InsertPointOrErrorTy AfterIP = CBFunc(
      Builder.saveIP(),
  ...
  for (auto InArg : zip(Inputs, ArgRange)) {
    ...
    if (toolchain::isa<toolchain::GlobalValue, toolchain::GlobalObject, toolchain::GlobalVariable>(
...
OpenMPIRBuilder::InsertPointOrErrorTy OpenMPIRBuilder::emitTargetTask(
    ...
    const SmallVector<toolchain::OpenMPIRBuilder::DependData> &Dependencies,
  ...
  OI.PostOutlineCB = [this, ToBeDeleted, Dependencies, NeedsTargetTask,
    ...
    SmallVector<toolchain::Value *> TaskAllocArgs = {
...
GlobalVariable *OpenMPIRBuilder::getOrCreateInternalVariable(
  ...
  if (Elem.second) {
    ...
    const toolchain::Align TypeAlign = DL.getABITypeAlign(Ty);
    const toolchain::Align PtrAlign = DL.getPointerABIAlignment(AddressSpaceVal);
...
OpenMPIRBuilder::createOffloadMaptypes(SmallVectorImpl<uint64_t> &Mappings,
                                       std::string VarName) {
  toolchain::Constant *MaptypesArrayInit =
      toolchain::ConstantDataArray::get(M.getContext(), Mappings);
  auto *MaptypesArrayGlobal = new toolchain::GlobalVariable(
      M, MaptypesArrayInit->getType(),
      /*isConstant=*/true, toolchain::GlobalValue::PrivateLinkage, MaptypesArrayInit,
      VarName);
  MaptypesArrayGlobal->setUnnamedAddr(toolchain::GlobalValue::UnnamedAddr::Global);
...
Expected<Function *> OpenMPIRBuilder::emitUserDefinedMapper(
    function_ref<MapInfosOrErrorTy(InsertPointTy CodeGenIP, toolchain::Value *PtrPHI,
                                   toolchain::Value *BeginArg)>
        GenMapInfoCB,
...
bool OpenMPIRBuilder::checkAndEmitFlushAfterAtomic(
    const LocationDescription &Loc, toolchain::AtomicOrdering AO, AtomicKind AK) {
  assert(!(AO == AtomicOrdering::NotAtomic ||
           AO == toolchain::AtomicOrdering::Unordered) &&
         "Unexpected Atomic Ordering.");

  bool Flush = false;
  toolchain::AtomicOrdering FlushAO = AtomicOrdering::Monotonic;
...
Expected<std::pair<Value *, Value *>> OpenMPIRBuilder::emitAtomicUpdate(
  ...
  if (emitRMWOp) {
    AtomicRMWInst *RMWInst =
        Builder.CreateAtomicRMW(RMWOp, X, Expr, toolchain::MaybeAlign(), AO);
    if (T.isAMDGPU()) {
  ...
  } else if (RMWOp == toolchain::AtomicRMWInst::BinOp::BAD_BINOP &&
             XElemTy->isStructTy()) {
    LoadInst *OldVal =
    ...
    toolchain::PHINode *PHI = Builder.CreatePHI(OldVal->getType(), 2);
    ...
    AtomicOrdering Failure =
        toolchain::AtomicCmpXchgInst::getStrongestFailureOrdering(AO);
    ...
    AtomicCmpXchgInst *Result = Builder.CreateAtomicCmpXchg(
        X, PHI, DesiredVal, toolchain::MaybeAlign(), AO, Failure);
...
GlobalVariable *
OpenMPIRBuilder::createOffloadMapnames(SmallVectorImpl<toolchain::Constant *> &Names,
                                       std::string VarName) {
  toolchain::Constant *MapNamesArrayInit = toolchain::ConstantArray::get(
      toolchain::ArrayType::get(toolchain::PointerType::getUnqual(M.getContext()),
                           Names.size()),
      Names);
  auto *MapNamesArrayGlobal = new toolchain::GlobalVariable(
      M, MapNamesArrayInit->getType(),
      /*isConstant=*/true, toolchain::GlobalValue::PrivateLinkage, MapNamesArrayInit,
      VarName);
  return MapNamesArrayGlobal;
}

// Create all simple and struct types exposed by the runtime and remember
// the toolchain::PointerTypes of them for easy access later.
...
void TargetRegionEntryInfo::getTargetRegionEntryFnName(
  ...
  OS << KernelNamePrefix << toolchain::format("%x", DeviceID)
     << toolchain::format("_%x_", FileID) << ParentName << "_l" << Line;
  if (Count)
    OS << "_" << Count;
}
...
void CanonicalLoopInfo::mapIndVar(
    toolchain::function_ref<Value *(Instruction *)> Updater) {
  assert(isValid() && "Requires a valid canonical loop");
