...
using namespace vm::core::omp;

#define GEN_DIRECTIVES_IMPL
#include "vm/core/Frontend/OpenMP/OMP.inc"
...
getFirstCompositeRange(iterator_range<ArrayRef<Directive>::iterator> Leafs) {
  ...
  auto Empty = toolchain::make_range(Leafs.end(), Leafs.end());
  ...
  auto End =
      firstLoopAssociated(toolchain::make_range(std::next(Begin), Leafs.end()));
  if (End == Leafs.end())
    return Empty;
...
static void
collectPrivatizingConstructs(toolchain::SmallSet<Directive, 16> &Constructs,
                             unsigned Version) {
  toolchain::SmallSet<Clause, 16> Privatizing;
  for (auto C :
       toolchain::enum_seq_inclusive<Clause>(Clause::First_, Clause::Last_)) {
    if (isPrivatizingClause(C))
      Privatizing.insert(C);
  }

  for (auto D : toolchain::enum_seq_inclusive<Directive>(Directive::First_,
                                                    Directive::Last_)) {
    bool AllowsPrivatizing = toolchain::any_of(Privatizing, [&](Clause C) {
...
getLeafOrCompositeConstructs(Directive D, SmallVectorImpl<Directive> &Output) {
  ...
  do {
    auto Range = getFirstCompositeRange(toolchain::make_range(Iter, Leafs.end()));
...
Directive getCompoundConstruct(ArrayRef<Directive> Parts) {
  ...
  auto Iter = std::lower_bound(
      ...
      [](const toolchain::omp::Directive *RowA, const toolchain::omp::Directive *RowB) {
...
bool isPrivatizingConstruct(Directive D, unsigned Version) {
  static toolchain::SmallSet<Directive, 16> Privatizing;
  ...
  return toolchain::is_contained(Privatizing, D);
}
