...
#include "vm/core/Frontend/Offloading/OffloadWrapper.h"
...
GlobalVariable *createBinDesc(Module &M, ArrayRef<ArrayRef<char>> Bufs,
  ...
  for (ArrayRef<char> Buf : Bufs) {
    ...
    Image->setSection(Relocatable ? ".toolchain.offloading.relocatable"
                                  : ".toolchain.offloading");
...
GlobalVariable *createFatbinDesc(Module &M, ArrayRef<char> Image, bool IsHIP,
                                 StringRef Suffix) {
  LLVMContext &C = M.getContext();
  toolchain::Type *Int8PtrTy = PointerType::getUnqual(C);
...
Function *createRegisterGlobalsFunction(Module &M, bool IsHIP,
  ...
  auto *NormalizedBit = Builder.CreateAnd(
      Flags, ConstantInt::get(Type::getInt32Ty(C),
                              toolchain::offloading::OffloadGlobalNormalized));
  ...
  Builder.CreateBr(IfEndBB);
  Switch->addCase(Builder.getInt32(toolchain::offloading::OffloadGlobalEntry),
                  SwGlobalBB);
  ...
  Builder.CreateCall(RegManagedVar, {RegGlobalsFn->arg_begin(), AuxAddr, Addr,
                                     Name, Size, Data});
  Builder.CreateBr(IfEndBB);
  Switch->addCase(Builder.getInt32(toolchain::offloading::OffloadGlobalManagedEntry),
                  SwManagedBB);
  ...
  if (EmitSurfacesAndTextures)
    Builder.CreateCall(RegSurface, {RegGlobalsFn->arg_begin(), Addr, Name, Name,
                                    Data, Extern});
  Builder.CreateBr(IfEndBB);
  Switch->addCase(Builder.getInt32(toolchain::offloading::OffloadGlobalSurfaceEntry),
                  SwSurfaceBB);
  ...
  Builder.CreateBr(IfEndBB);
  Switch->addCase(Builder.getInt32(toolchain::offloading::OffloadGlobalTextureEntry),
                  SwTextureBB);
...
void createRegisterFatbinFunction(Module &M, GlobalVariable *FatbinDesc,
  ...
  auto *BinaryHandleGlobal = new toolchain::GlobalVariable(
      M, PtrTy, false, toolchain::GlobalValue::InternalLinkage,
      toolchain::ConstantPointerNull::get(PtrTy),
...
private:
  ...
  Constant *wrapImage(const OffloadBinary &OB, const Twine &ImageID,
    ...
    std::pair<Constant *, Constant *> Binary = addArrayToModule(
        ...
        Twine(OffloadKindTag) + ImageID + ".data", ".toolchain.offloading");
...
Error offloading::wrapOpenMPBinaries(Module &M, ArrayRef<ArrayRef<char>> Images,
                                     EntryArrayTy EntryArray,
                                     toolchain::StringRef Suffix, bool Relocatable) {
  GlobalVariable *Desc =
...
Error offloading::wrapHIPBinary(Module &M, ArrayRef<char> Image,
                                EntryArrayTy EntryArray, toolchain::StringRef Suffix,
                                bool EmitSurfacesAndTextures) {
...
Error toolchain::offloading::wrapSYCLBinaries(toolchain::Module &M, ArrayRef<char> Buffer,
                                         SYCLJITOptions Options) {
