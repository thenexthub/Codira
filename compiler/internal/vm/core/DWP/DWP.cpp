//===-- toolchain-dwp.cpp - Split DWARF merging tool for toolchain ------------------===//
//
...
static uint64_t debugStrOffsetsHeaderSize(DataExtractor StrOffsetsData,
  ...
  if (Length == toolchain::dwarf::DW_LENGTH_DWARF64)
...
static void writeNewOffsetsTo(MCStreamer &Out, DataExtractor &Data,
  ...
  while (Offset < Size) {
    ...
    // Truncate the string offset like the old toolchain-dwp would have if we aren't
  ...
  if (Version > 4) {
    while (Offset < Size) {
      ...
      if (OldOffsetSize == 4 && NewOffsetSize == 8) {
        ...
        Out.emitIntValue(toolchain::dwarf::DW_LENGTH_DWARF64, 4);
...
Error write(MCStreamer &Out, ArrayRef<std::string> Inputs,
  ...
  for (const auto &Input : Inputs) {
    ...
    if (!CurTUIndexSection.empty()) {
      toolchain::DWARFSectionKind TUSectionKind;
