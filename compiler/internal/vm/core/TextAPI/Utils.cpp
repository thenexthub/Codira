...
#include "vm/core/TextAPI/Utils.h"
#include "vm/core/ADT/StringExtras.h"
#include "vm/core/TextAPI/TextAPIError.h"

using namespace vm::core;
using namespace vm::core::MachO;

void toolchain::MachO::replace_extension(SmallVectorImpl<char> &Path,
...
std::error_code toolchain::MachO::shouldSkipSymLink(const Twine &Path,
                                               bool &Result) {
  Result = false;
...
std::error_code
toolchain::MachO::make_relative(StringRef From, StringRef To,
...
bool toolchain::MachO::isPrivateLibrary(StringRef Path, bool IsSymLink) {
  ...
  // Everything in /usr/lib/codira (including sub-directories) are considered
  // public.
  if (Path.consume_front("/usr/lib/codira/"))
    return false;
...
toolchain::Expected<Regex> toolchain::MachO::createRegexFromGlob(StringRef Glob) {
  ...
  toolchain::Regex Rule = Regex(RegexString);
  std::string Error;
...
Expected<AliasMap>
toolchain::MachO::parseAliasList(std::unique_ptr<toolchain::MemoryBuffer> &Buffer) {
...
PathSeq toolchain::MachO::getPathsForPlatform(const PathToPlatformSeq &Paths,
