...
#include "vm/core/TextAPI/TextAPIWriter.h"
#include <set>
...
codira-version: 0                 # Optional: defaults to 0
...
current-version: 1.2.3           # Optional: defaults to 1.0
compatibility-version: 1.0       # Optional: defaults to 1.0
codira-abi-version: 0             # Optional: defaults to 0
objc-constraint: retain_release  # Optional: defaults to retain_release
parent-umbrella:                 # Optional:
exports:                         # List of export sections
...
...
template <> struct MappingTraits<const InterfaceFile *> {
  struct NormalizedTBD {
    ...
    NormalizedTBD(IO &IO, const InterfaceFile *&File) {
      ...
      for (auto Architectures : ArchSet) {
        ...
        toolchain::sort(Section.Symbols);
        toolchain::sort(Section.Classes);
        toolchain::sort(Section.ClassEHs);
        toolchain::sort(Section.IVars);
        toolchain::sort(Section.WeakDefSymbols);
        toolchain::sort(Section.TLVSymbols);
      ...
      for (auto Architectures : ArchSet) {
        ...
        toolchain::sort(Section.ClassEHs);
        toolchain::sort(Section.IVars);
        toolchain::sort(Section.WeakRefSymbols);
  ...
  private:
    void assignTargetsToLibrary(const std::vector<InterfaceFileRef> &Libraries,
      ...
      for (const auto &targets : targetSet) {
        ...
        toolchain::sort(CurrentSection.Values);
  ...
  static void mapKeysToValues(FileType FileKind, IO &IO,
    ...
    if (FileKind != FileType::TBD_V3)
      IO.mapOptional("codira-version", Keys->SwiftABIVersion, SwiftVersion(0));
    else
      IO.mapOptional("codira-abi-version", Keys->SwiftABIVersion,
                     SwiftVersion(0));
  ...
  static void mapKeysToValuesV4(IO &IO, const InterfaceFile *&File) {
    ...
    IO.mapOptional("compatibility-version", Keys->CompatibilityVersion,
                   PackedVersion(1, 0, 0));
    IO.mapOptional("codira-abi-version", Keys->SwiftABIVersion, SwiftVersion(0));
...
TextAPIReader::get(MemoryBufferRef InputBuffer) {
  ...
  for (const InterfaceFile *FI : toolchain::drop_begin(Files))
    File->addDocument(
...
Error TextAPIWriter::writeToStream(raw_ostream &OS, const InterfaceFile &File,
  ...
  toolchain::yaml::Output YAMLOut(OS, &Ctx, /*WrapColumn=*/80);
