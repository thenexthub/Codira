...
private:
  ...
  toolchain::SmallVector<EntryT> Container;
};
...
static toolchain::SmallString<128> getParseErrorMsg(TBDKey Key) {
  return {"invalid ", Keys[Key], " section"};
}

static toolchain::SmallString<128> getSerializeErrorMsg(TBDKey Key) {
  return {"missing ", Keys[Key], " information"};
}

class JSONStubError : public toolchain::ErrorInfo<toolchain::json::ParseError> {
public:
  JSONStubError(Twine ErrMsg) : Message(ErrMsg.str()) {}

  void log(toolchain::raw_ostream &OS) const override { OS << Message << "\n"; }
...
Array serializeSymbols(InterfaceFile::const_filtered_symbol_range Symbols,
  ...
  auto InsertSymbolsToJSON = [](Object &SymSection, TBDKey SegmentKey,
    ...
    if (SymField.empty())
      return;
    toolchain::sort(SymField.Globals);
    toolchain::sort(SymField.TLV);
    toolchain::sort(SymField.Weaks);
    toolchain::sort(SymField.ObjCClasses);
    toolchain::sort(SymField.EHTypes);
    toolchain::sort(SymField.IVars);
    Object Segment;
