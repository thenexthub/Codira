...
Error MSFBuilder::setStreamSize(uint32_t Idx, uint32_t Size) {
  ...
  if (NewBlocks > OldBlocks) {
    ...
    toolchain::append_range(CurrentBlocks, AddedBlockList);
...
Expected<MSFLayout> MSFBuilder::generateLayout() {
  toolchain::TimeTraceScope timeScope("MSF: Generate layout");
  ...
  if (NumDirectoryBlocks > DirectoryBlocks.size()) {
    ...
    if (auto EC = allocateBlocks(NumExtraBlocks, ExtraBlocks))
      return std::move(EC);
    toolchain::append_range(DirectoryBlocks, ExtraBlocks);
  ...
  toolchain::uninitialized_copy(DirectoryBlocks, DirBlocks);
  ...
  if (!StreamData.empty()) {
    ...
    for (uint32_t I = 0; I < StreamData.size(); ++I) {
      ...
      toolchain::uninitialized_copy(StreamData[I].second, BlockList);
      L.StreamMap[I] =
...
Expected<FileBufferByteStream> MSFBuilder::commit(StringRef Path,
                                                  MSFLayout &Layout) {
  toolchain::TimeTraceScope timeScope("Commit MSF");
