...
///
/// See https://toolchain.org/docs/SymbolizerMarkupFormat.html
///
//===----------------------------------------------------------------------===//

#include "vm/core/DebugInfo/Symbolize/MarkupFilter.h"
...
#include "vm/core/DebugInfo/Symbolize/Markup.h"
...
using namespace vm::core::symbolize;
...
void MarkupFilter::endAnyModuleInfoLine() {
  if (!MIL)
    return;
  toolchain::stable_sort(MIL->MMaps, [](const MMap *A, const MMap *B) {
...
bool MarkupFilter::trySymbol(const MarkupNode &Node) {
  ...
  highlight();
  OS << toolchain::demangle(Node.Fields.front().str());
  restoreColor();
  return true;
}
...
object::BuildID MarkupFilter::parseBuildID(StringRef Str) const {
  object::BuildID BID = toolchain::object::parseBuildID(Str);
  if (BID.empty())
