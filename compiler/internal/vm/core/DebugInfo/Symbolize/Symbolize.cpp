...
bool checkFileCRC(StringRef Path, uint32_t CRCHash) {
  ...
  if (!MB)
    return false;
  return CRCHash == toolchain::crc32(arrayRefFromStringRef(MB.get()->getBuffer()));
}
...
bool LLVMSymbolizer::findDebugBinary(const std::string &OrigPath,
  ...
  toolchain::sys::path::remove_filename(OrigDir);
  SmallString<16> DebugPath = OrigDir;
  // Try relative/path/to/original_binary/debuglink_name
  toolchain::sys::path::append(DebugPath, DebuglinkName);
  ...
  DebugPath = OrigDir;
  toolchain::sys::path::append(DebugPath, ".debug", DebuglinkName);
  if (checkFileCRC(DebugPath, CRCHash)) {
    Result = std::string(DebugPath);
    return true;
  }
  ...
  toolchain::sys::fs::make_absolute(OrigDir);
  ...
  toolchain::sys::path::append(DebugPath, toolchain::sys::path::relative_path(OrigDir),
                          DebuglinkName);
  if (checkFileCRC(DebugPath, CRCHash)) {
    Result = std::string(DebugPath);
    return true;
...
std::string LLVMSymbolizer::lookUpGsymFile(const std::string &Path) {
  ...
  auto CheckGsymFile = [](const toolchain::StringRef &GsymPath) {
    sys::fs::file_status Status;
    std::error_code EC = toolchain::sys::fs::status(GsymPath, Status);
    return !EC && !toolchain::sys::fs::is_directory(Status);
  };
  ...
  for (const auto &Directory : Opts.GsymFileDirectory) {
    SmallString<16> GsymPath = toolchain::StringRef{Directory};
    toolchain::sys::path::append(GsymPath,
                            toolchain::sys::path::filename(Path) + ".gsym");
...
Expected<ObjectFile *> LLVMSymbolizer::findOrCacheObject(
    ...
    toolchain::function_ref<Expected<std::unique_ptr<ObjectFile>>()> Loader,
...
LLVMSymbolizer::getOrCreateModuleInfo(StringRef ModuleName) {
  ...
  if (!Context) {
    if (auto CoffObject = dyn_cast<COFFObjectFile>(Objects.first)) {
      ...
      bool HasDwarf = toolchain::any_of(
