...
bool checkIntegrityScopesTree(LVScope *Root) {
  ...
  if (Duplicate.size()) {
    toolchain::stable_sort(Duplicate, [](const auto &l, const auto &r) {
...
Error LVSplitContext::createSplitFolder(StringRef Where) {
  ...
  if (std::error_code EC = toolchain::sys::fs::create_directories(Location))
...
Error LVReader::doLoad() {
  ...
  if (options().getInternalIntegrity() && !checkIntegrityScopesTree(Root))
    return toolchain::make_error<StringError>("Duplicated elements in Scopes Tree",
