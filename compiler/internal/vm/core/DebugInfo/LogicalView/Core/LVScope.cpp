...
#include "vm/core/DebugInfo/LogicalView/Core/LVCompare.h"
#include "vm/core/DebugInfo/LogicalView/Core/LVLine.h"
#include "vm/core/DebugInfo/LogicalView/Core/LVLocation.h"
#include "vm/core/DebugInfo/LogicalView/Core/LVRange.h"
...
LVElements LVScope::getSortedChildren(LVSortFunction SortFunction) const {
  ...
  if (SortFunction)
    toolchain::stable_sort(Elements, SortFunction);
  return Elements;
}
...
bool LVScope::removeElement(LVElement *Element) {
  ...
  auto RemoveElement = [Element, Predicate](auto &Container) -> bool {
    auto Iter = toolchain::remove_if(*Container, Predicate);
  ...
  if (getSymbols())
    for (const LVSymbol *Symbol : *getSymbols())
      if (Symbol->getHasReferenceAbstract())
        toolchain::erase(References, Symbol->getReference());
    ...
    std::function<void(LVScope * Parent, LVSortFunction SortFunction)> Sort =
        [&](LVScope *Parent, LVSortFunction SortFunction) {
          auto Traverse = [&](auto &Set, LVSortFunction SortFunction) {
            if (Set)
              toolchain::stable_sort(*Set, SortFunction);
          };
...
void LVScopeCompileUnit::printMatchedElements(raw_ostream &OS,
  ...
  LVSortFunction SortFunction = getSortFunction();
  if (SortFunction)
    toolchain::stable_sort(MatchedElements, SortFunction);
