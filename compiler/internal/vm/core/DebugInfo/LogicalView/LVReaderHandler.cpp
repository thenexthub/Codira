...
static std::string searchForExe(const StringRef Path,
  ...
  toolchain::sys::path::replace_extension(ExePath, Extension);
...
static std::string searchForObj(const StringRef Path,
  ...
  toolchain::sys::path::replace_extension(ObjPath, Extension);
  if (toolchain::sys::fs::exists(ObjPath)) {
