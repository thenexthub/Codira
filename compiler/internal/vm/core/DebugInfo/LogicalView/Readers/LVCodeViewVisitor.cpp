...
#include "vm/core/DebugInfo/LogicalView/Readers/LVCodeViewVisitor.h"
...
#include "vm/core/DebugInfo/LogicalView/Core/LVType.h"
...
static TypeIndex getTrueType(TypeIndex &TI) {
  ...
  // https://toolchain.org/docs/PDB/TpiStream.html
  //
...
LVScope *LVNamespaceDeduction::get(StringRef ScopedName, bool CheckScope) {
  ...
  if (CheckScope)
    toolchain::erase_if(Components, [&](StringRef Component) {
  ...
  if (LVScope *Scope = LogicalVisitor->CurrentScope) {
    ...
    if (options().getAttributeLanguage())
      Scope->setSourceLanguage(LVSourceLanguage{
          static_cast<toolchain::codeview::SourceLanguage>(Compile2.getLanguage())});
...
Error LVSymbolVisitor::visitKnownRecord(CVSymbol &Record,
  ...
  if (LVScope *Scope = LogicalVisitor->CurrentScope) {
    ...
    if (options().getAttributeLanguage())
      Scope->setSourceLanguage(LVSourceLanguage{
          static_cast<toolchain::codeview::SourceLanguage>(Compile3.getLanguage())});
    getReader().isSystemEntry(Scope, CurrentObjectName);
  ...
  LLVM_DEBUG({
    ...
    if (ObjDelegate) {
      ...
      if (!ExpectedProgram) {
        ...
        return toolchain::make_error<CodeViewError>(
...
Error LVSymbolVisitor::visitKnownRecord(CVSymbol &Record, ProcSym &Proc) {
  if (InFunctionScope)
    return toolchain::make_error<CodeViewError>("Visiting a ProcSym while inside "
                                           "function scope!");
  ...
  if (LVScope *Function = LogicalVisitor->CurrentScope) {
    ...
    if (TIFunctionType.isSimple())
      ...
      if (!GetRecordType()) {
        ...
        if (!CVFunctionType)
          return toolchain::make_error<CodeViewError>("Invalid type index");
      }
...
Error LVSymbolVisitor::visitKnownRecord(CVSymbol &Record, Thunk32Sym &Thunk) {
  if (InFunctionScope)
    return toolchain::make_error<CodeViewError>("Visiting a Thunk32Sym while inside "
                                           "function scope!");
...
Error LVLogicalVisitor::visitFieldListMemberStream(
  ...
  BinaryByteStream Stream(FieldList, toolchain::endianness::little);
