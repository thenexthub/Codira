...
#include "vm/core/DebugInfo/LogicalView/Readers/LVCodeViewReader.h"
...
#include "vm/core/DebugInfo/CodeView/SymbolVisitorCallbackPipeline.h"
#include "vm/core/DebugInfo/LogicalView/Core/LVLine.h"
...
#include "vm/core/DebugInfo/CodeView/CodeViewRegisters.def"
#undef CV_REGISTER
...
void LVCodeViewReader::cacheRelocations() {
  for (const SectionRef &Section : getObj().sections()) {
    ...
    toolchain::append_range(RM, Section.relocations());

    // Sort relocations by address.
    toolchain::sort(RM, [](RelocationRef L, RelocationRef R) {
...
Error LVCodeViewReader::collectInlineeInfo(
    DebugInlineeLinesSubsectionRef &Lines, const toolchain::pdb::SymbolGroup *SG) {
...
Error LVCodeViewReader::traverseInlineeLines(StringRef Subsection) {
  BinaryStreamReader SR(Subsection, toolchain::endianness::little);
...
Error LVCodeViewReader::initializeFileAndStringTables(
  ...
  while (Reader.bytesRemaining() > 0 &&
    ...
    BinaryStreamRef ST(Contents, toolchain::endianness::little);
...
Error LVCodeViewReader::loadPrecompiledObject(PrecompRecord &Precomp,
  ...
  for (const SectionRef &Section : Obj.sections()) {
    ...
    if (*SectionNameOrErr == ".debug$P") {
      ...
      ReaderPrecomp = std::make_unique<BinaryStreamReader>(
          *DataOrErr, toolchain::endianness::little);
      cantFail(
  ...
  ItemStream =
      std::make_unique<BinaryItemStream<CVType>>(toolchain::endianness::little);
...
Error LVCodeViewReader::traverseTypeSection(StringRef SectionName,
  ...
  CVTypeArray CVTypes;
  BinaryStreamReader Reader(*DataOrErr, toolchain::endianness::little);
...
Error LVCodeViewReader::traverseSymbolsSubsection(StringRef Subsection,
  ...
  BinaryStreamReader Reader(BinaryData, toolchain::endianness::little);
...
Error LVCodeViewReader::traverseSymbolSection(StringRef SectionName,
  ...
  BinaryStreamReader FSReader(Data, toolchain::endianness::little);
...
void LVCodeViewReader::mapRangeAddress(const ObjectFile &Obj,
  ...
  for (const SymbolRef &Sym : Object->symbols()) {
    ...
    if (Symbol.getComplexType() != toolchain::COFF::IMAGE_SYM_DTYPE_FUNCTION)
      continue;
