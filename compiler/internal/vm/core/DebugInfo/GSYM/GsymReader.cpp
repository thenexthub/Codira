...
GsymReader::GsymReader(std::unique_ptr<MemoryBuffer> Buffer)
    : MemBuffer(std::move(Buffer)), Endian(toolchain::endianness::native) {}

GsymReader::GsymReader(GsymReader &&RHS) = default;

GsymReader::~GsymReader() = default;

toolchain::Expected<GsymReader> GsymReader::openFile(StringRef Filename) {
...
toolchain::Error
GsymReader::parse() {
  BinaryStreamReader FileData(MemBuffer->getBuffer(), toolchain::endianness::native);
  ...
  const auto HostByteOrder = toolchain::endianness::native;
  switch (Hdr->Magic) {
    case GSYM_MAGIC:
    ...
    case GSYM_CIGAM:
      ...
      Endian = sys::IsBigEndianHost ? toolchain::endianness::little
  ...
  bool DataIsLittleEndian = HostByteOrder != toolchain::endianness::little;
...
toolchain::Expected<DataExtractor>
...
GsymReader::getFunctionInfoDataAtIndex(uint64_t AddrIdx,
  ...
  return DataExtractor(Bytes, Endian == toolchain::endianness::little, 4);
}

toolchain::Expected<FunctionInfo> GsymReader::getFunctionInfo(uint64_t Addr) const {
...
toolchain::Expected<FunctionInfo>
...
toolchain::Expected<LookupResult>
...
toolchain::Expected<std::vector<LookupResult>>
