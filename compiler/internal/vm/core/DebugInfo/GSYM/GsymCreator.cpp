...
#include "vm/core/DebugInfo/GSYM/LineTable.h"
...
uint32_t GsymCreator::insertFile(StringRef Path, toolchain::sys::path::Style Style) {
  toolchain::StringRef directory = toolchain::sys::path::parent_path(Path, Style);
  toolchain::StringRef filename = toolchain::sys::path::filename(Path, Style);
...
toolchain::Error GsymCreator::save(StringRef Path, toolchain::endianness ByteOrder,
...
toolchain::Error GsymCreator::encode(FileWriter &O) const {
  std::lock_guard<std::mutex> Guard(Mutex);
  if (Funcs.empty())
  ...
  toolchain::Error Err = Hdr.encode(O);
  if (Err)
    return Err;
...
toolchain::Error GsymCreator::loadCallSitesFromYAML(StringRef YAMLFile) {
...
toolchain::Error GsymCreator::finalize(OutputAggregator &Out) {
  std::lock_guard<std::mutex> Guard(Mutex);
  if (Finalized)
...
toolchain::Error GsymCreator::saveSegments(StringRef Path,
                                      toolchain::endianness ByteOrder,
  ...
  while (FuncIdx < NumFuncs) {
    toolchain::Expected<std::unique_ptr<GsymCreator>> ExpectedGC =
    ...
    if (ExpectedGC) {
      ...
      toolchain::Error Err = GC->finalize(Out);
      if (Err)
        return Err;
      ...
      if (FirstFuncAddr) {
        SGP << Path << "-" << toolchain::format_hex(*FirstFuncAddr, 1);
...
toolchain::Expected<std::unique_ptr<GsymCreator>>
