...
static toolchain::Error parse(DataExtractor &Data, uint64_t BaseAddr,
...
toolchain::Error LineTable::encode(FileWriter &Out, uint64_t BaseAddr) const {
...
// below.
toolchain::Expected<LineTable> LineTable::decode(DataExtractor &Data,
                                            uint64_t BaseAddr) {
  LineTable LT;
  toolchain::Error Err = parse(Data, BaseAddr, [&](const LineEntry &Row) -> bool {
...
Expected<LineEntry> LineTable::lookup(DataExtractor &Data, uint64_t BaseAddr, uint64_t Addr) {
  LineEntry Result;
  toolchain::Error Err = parse(Data, BaseAddr,
...
raw_ostream &toolchain::gsym::operator<<(raw_ostream &OS, const LineTable &LT) {
