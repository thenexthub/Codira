...
raw_ostream &toolchain::gsym::operator<<(raw_ostream &OS, const FunctionInfo &FI) {
...
toolchain::Expected<FunctionInfo> FunctionInfo::decode(DataExtractor &Data,
                                                  uint64_t BaseAddr) {
  FunctionInfo FI;
  uint64_t Offset = 0;
  ...
  while (!Done) {
    ...
    switch (IT) {
      ...
      case InfoType::CallSiteInfo:
        if (Expected<toolchain::gsym::CallSiteInfoCollection> CI =
                toolchain::gsym::CallSiteInfoCollection::decode(InfoData))
...
uint64_t FunctionInfo::cacheEncoding() {
  ...
  FileWriter FW(OutStrm, toolchain::endianness::native);
  toolchain::Expected<uint64_t> Result = encode(FW);
  if (!Result) {
    EncodingCache.clear();
...
toolchain::Expected<uint64_t> FunctionInfo::encode(FileWriter &Out,
  ...
  if (!EncodingCache.empty() &&
      toolchain::endianness::native == Out.getByteOrder()) {
    // We already encoded this object, just write out the bytes.
    Out.writeData(toolchain::ArrayRef<uint8_t>((const uint8_t *)EncodingCache.data(),
                                          EncodingCache.size()));
  ...
  if (OptLineTable) {
    ...
    toolchain::Error err = OptLineTable->encode(Out, Range.start());
    if (err)
      return std::move(err);
  ...
  if (Inline) {
    ...
    Out.writeU32(0);
    const auto StartOffset = Out.tell();
    toolchain::Error err = Inline->encode(Out, Range.start());
    if (err)
      return std::move(err);
    const auto Length = Out.tell() - StartOffset;
    if (Length > UINT32_MAX)
  ...
  if (MergedFunctions) {
    ...
    Out.writeU32(0);
    const auto StartOffset = Out.tell();
    toolchain::Error err = MergedFunctions->encode(Out);
    if (err)
      return std::move(err);
    const auto Length = Out.tell() - StartOffset;
    if (Length > UINT32_MAX)
...
FunctionInfo::lookup(DataExtractor &Data, const GsymReader &GR,
  ...
  // data.
  toolchain::Error Err = InlineInfo::lookup(GR, *InlineInfoData, FuncAddr, Addr,
                                       LR.Locations);
  if (Err)
    return std::move(Err);
  return LR;
