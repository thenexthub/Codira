...
#include "vm/core/DebugInfo/GSYM/DwarfTransformer.h"
...
static void convertFunctionLineTable(OutputAggregator &Out, CUInfo &CUI,
  ...
  if (auto StmtSeqAttr = Die.find(toolchain::dwarf::DW_AT_LLVM_stmt_sequence)) {
...
void DwarfTransformer::parseCallSiteInfoFromDwarf(CUInfo &CUI, DWARFDie Die,
  ...
  if (!CSIC.CallSites.empty()) {
    ...
    toolchain::append_range(FI.CallSites->CallSites, CSIC.CallSites);
  }
}
...
toolchain::Error DwarfTransformer::verify(StringRef GsymPath,
