...
#include "vm/core/DebugInfo/BTF/BTFParser.h"
...
Error BTFParser::parseTypesInfo(ParseContext &Ctx, uint64_t TypesInfoStart,
  ...
  endianness Endianness = Ctx.Obj.isLittleEndian() ? toolchain::endianness::little
  ...
  while (Pos < RawData.size()) {
    ...
    LLVM_DEBUG({
      toolchain::dbgs() << "Adding BTF type:\n"
                   << "  Id = " << Types.size() << "\n"
                   << "  Kind = " << Type->getKind() << "\n"
...
static const T *findInfo(const DenseMap<uint64_t, SmallVector<T, 0>> &SecMap,
  ...
  typename SmallVector<T, 0>::const_iterator MaybeInfo = toolchain::partition_point(
...
// toolchain/unittests/DebugInfo/BTF/BTFParserTest.cpp.
//
