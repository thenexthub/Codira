...
#include "vm/core/DebugInfo/CodeView/SymbolRecordMapping.h"
...
std::string toolchain::codeview::computeTypeName(TypeCollection &Types,
                                            TypeIndex Index) {
...
StringRef toolchain::codeview::getSymbolName(CVSymbol Sym) {
  if (Sym.kind() == SymbolKind::S_CONSTANT) {
    ...
    BinaryStreamReader Reader(Sym.content(), toolchain::endianness::little);
