...
//
//===----------------------------------------------------------------------===//
#include "vm/core/DebugInfo/DWARF/DWARFVerifier.h"
...
#include "vm/core/DebugInfo/DWARF/DWARFAttribute.h"
...
#include "vm/core/DebugInfo/DWARF/DWARFDebugLoc.h"
#include "vm/core/DebugInfo/DWARF/DWARFDie.h"
#include "vm/core/DebugInfo/DWARF/DWARFFormValue.h"
#include "vm/core/DebugInfo/DWARF/DWARFLocationExpression.h"
...
#include "vm/core/Support/Parallel.h"
...
unsigned DWARFVerifier::verifyDieRanges(const DWARFDie &Die,
  ...
  if (!RangesOrError) {
    ...
    if (!Unit->isDWOUnit())
      ++NumErrors;
    toolchain::consumeError(RangesOrError.takeError());
    return NumErrors;
  }
        ...
        ReportError("DW_AT_ranges offset out of bounds",
                    "DW_AT_ranges offset is beyond " +
                        StringRef(DwarfVersion < 5 ? ".debug_ranges"
                                                   : ".debug_rnglists") +
                        " bounds: " + toolchain::formatv("{0:x8}", *SectionOffset));
      break;
    }
      ...
      if (*SectionOffset >= U->getLineSection().Data.size())
        ReportError("DW_AT_stmt_list offset out of bounds",
                    "DW_AT_stmt_list offset is beyond .debug_line bounds: " +
                        toolchain::formatv("{0:x8}", *SectionOffset));
      break;
    }
          ...
          if (std::optional<uint64_t> LastFileIdx =
            ...
            ReportError("Invalid file index in DW_AT_decl_file",
                        "DIE has " + AttributeString(Attr) +
                            ...
                            toolchain::formatv("{0}", *FileIdx) +
                            " (valid values are [" +
                            (IsZeroIndexed ? "0-" : "1-") +
                            toolchain::formatv("{0}", *LastFileIdx) + "])");
          } else {
            ReportError("Invalid file index in DW_AT_decl_file",
                        "DIE has " + AttributeString(Attr) +
                            " with an invalid file index " +
    ...
    if (*SectionOffset < SequencesStart || *SectionOffset >= LineTableEnd) {
      ReportError("DW_AT_LLVM_stmt_sequence offset out of line table bounds",
                  "DW_AT_LLVM_stmt_sequence offset " +
                      ...
                      toolchain::formatv("{0:x8}", SequencesStart) + ", " +
                      toolchain::formatv("{0:x8}", LineTableEnd) + ")");
      break;
    }

    // Check if the offset matches any of the sequence offset.
    auto It = toolchain::find_if(LineTable->Sequences,
...
unsigned DWARFVerifier::verifyDebugInfoReferences(
    ...
    toolchain::function_ref<DWARFUnit *(uint64_t)> GetUnitForOffset) {
...
void DWARFVerifier::verifyNameIndexEntries(
  ...
  for (; EntryOr; ++NumEntries, EntryID = NextEntryID,
    ...
    if (TUIndex && *TUIndex >= NumLocalTUs) {
      ...
      toolchain::DWARFContext &NonSkeletonDCtx = NonSkeletonUnit->getContext();
  ...
  for (StringRef Name : EntryNames) {
    ...
    if (iter == NamesToDieOffsets.end() || !iter->second.count(DieUnitOffset)) {
      ErrorCategory.Report(
          ...
          toolchain::dwarf::TagString(Die.getTag()), [&]() {
            error() << formatv(
