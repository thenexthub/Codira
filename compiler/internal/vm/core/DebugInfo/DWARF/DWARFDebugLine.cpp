...
const toolchain::DWARFDebugLine::FileNameEntry &
...
// the end of the prologue.
static toolchain::Expected<ContentDescriptors>
...
parseV5DirFileTables(const DWARFDataExtractor &DebugLineData,
  ...
  toolchain::Expected<ContentDescriptors> DirDescriptors =
  ...
  toolchain::Expected<ContentDescriptors> FileDescriptors =
  ...
  for (uint64_t I = 0; I != FileEntryCount; ++I) {
    ...
    for (auto Descriptor : *FileDescriptors) {
      ...
      case DW_LNCT_MD5:
        ...
        toolchain::uninitialized_copy(*Value.getAsBlock(),
...
Error DWARFDebugLine::LineTable::parse(
  ...
  *OffsetPtr = DebugLineOffset + Prologue.getLength();
  ...
  if (!Sequences.empty()) {
    toolchain::stable_sort(Sequences, Sequence::orderByHighPC);
...
DWARFDebugLine::LineTable::lookupAddressImpl(object::SectionedAddress Address,
  ...
  SequenceIter It = toolchain::upper_bound(Sequences, Sequence,
...
bool DWARFDebugLine::Prologue::getFileNameByIndex(
  ...
  if (Kind == FileLineInfoKind::BaseNameOnly) {
    Result = std::string(toolchain::sys::path::filename(FileName));
    return true;
  }
