...
DWARFUnit *DWARFUnitVector::addUnit(std::unique_ptr<DWARFUnit> Unit) {
  auto I = toolchain::upper_bound(*this, Unit,
...
void DWARFUnit::updateAddressDieMap(DWARFDie Die) {
  if (Die.isSubroutineDIE()) {
    ...
    if (DIERangesOrError) {
      ...
      toolchain::consumeError(DIERangesOrError.takeError());
  }
...
const DWARFUnitIndex &toolchain::getDWARFUnitIndex(DWARFContext &Context,
...
parseDWARFStringOffsetsTableHeader(DWARFDataExtractor &DA,
                                   toolchain::dwarf::DwarfFormat Format,
                                   uint64_t Offset) {
...
std::optional<uint64_t> DWARFUnit::getRnglistOffset(uint32_t Index) {
  ...
  DWARFDataExtractor RangesDA(Context.getDWARFObj(), *RangeSection,
                              IsLittleEndian, 0);
  if (std::optional<uint64_t> Off = toolchain::DWARFListTableHeader::getOffsetEntry(
