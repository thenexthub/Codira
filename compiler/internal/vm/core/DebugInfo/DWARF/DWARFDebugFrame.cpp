...
Expected<UnwindTable> toolchain::dwarf::createUnwindTable(const FDE *Fde) {
...
Expected<UnwindTable> toolchain::dwarf::createUnwindTable(const CIE *Cie) {
  // Rows will be empty if there are no CFI instructions.
  if (Cie->cfis().empty())
    return UnwindTable({});
...
Error DWARFDebugFrame::parse(DWARFDataExtractor Data) {
  ...
  while (Data.isValidOffset(Offset)) {
    ...
    // (e.g. toolchain-objdump --dwarf=frames prints "ZERO terminator").
    if (Length == 0) {
