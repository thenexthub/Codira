...
uint32_t toolchain::serializeSectionKind(DWARFSectionKind Kind,
...
DWARFSectionKind toolchain::deserializeSectionKind(uint32_t Value,
                                              unsigned IndexVersion) {
  if (IndexVersion == 5)
...
DWARFUnitIndex::getFromOffset(uint64_t Offset) const {
  if (OffsetLookup.empty()) {
    ...
    toolchain::sort(OffsetLookup, [&](Entry *E1, Entry *E2) {
