...
#include "vm/core/DebugInfo/DWARF/DWARFTypePrinter.h"
...
static void dumpApplePropertyAttribute(raw_ostream &OS, uint64_t Val) {
  OS << " (";
  do {
    uint64_t Shift = toolchain::countr_zero(Val);
...
prettyLanguageVersionString(const DWARFAttribute &AttrValue,
  ...
  return toolchain::dwarf::LanguageDescription(
      static_cast<SourceLanguageName>(*LName), *LVersion);
}

static toolchain::Expected<toolchain::StringRef>
getApplePropertyName(const DWARFDie &PropDIE) {
  if (!PropDIE)
    return toolchain::createStringError("invalid DIE");

  if (PropDIE.getTag() != DW_TAG_APPLE_property)
    return toolchain::createStringError("not referencing a DW_TAG_APPLE_property");
...
static void dumpAttribute(raw_ostream &OS, const DWARFDie &Die,
  ...
  toolchain::StringRef PrettyVersionName =
  ...
  if (Attr == DW_AT_specification || Attr == DW_AT_abstract_origin ||
    ...
    if (auto PropNameOrErr = getApplePropertyName(PropDIE))
      ...
      DumpOpts.RecoverableErrorHandler(createStringError(
          ...
          toolchain::formatv("decoding DW_AT_APPLE_property_name: {}",
        ...
        if (DumpOpts.ShowChildren && DumpOpts.ChildRecurseDepth > 0) {
          ...
          while (Child) {
            if (DumpOpts.FilterChildTag.empty() ||
                toolchain::is_contained(DumpOpts.FilterChildTag, Child.getTag()))
...
LLVM_DUMP_METHOD void DWARFDie::dump() const { dump(toolchain::errs(), 0); }
