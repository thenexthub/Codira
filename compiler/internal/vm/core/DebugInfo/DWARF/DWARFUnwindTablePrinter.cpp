...
#include "vm/core/DebugInfo/DWARF/DWARFUnwindTablePrinter.h"
...
raw_ostream &toolchain::dwarf::operator<<(raw_ostream &OS,
...
raw_ostream &toolchain::dwarf::operator<<(raw_ostream &OS, const UnwindRow &Row) {
  auto DumpOpts = DIDumpOptions();
  printUnwindRow(Row, OS, DumpOpts, 0);
  return OS;
}

void toolchain::dwarf::printUnwindTable(const UnwindTable &Rows, raw_ostream &OS,
                                   DIDumpOptions DumpOpts,
...
raw_ostream &toolchain::dwarf::operator<<(raw_ostream &OS, const UnwindTable &Rows) {
  auto DumpOpts = DIDumpOptions();
