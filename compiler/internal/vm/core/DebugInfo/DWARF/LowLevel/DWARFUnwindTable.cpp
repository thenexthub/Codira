...
toolchain::dwarf::parseRows(const CFIProgram &CFIP, UnwindRow &Row,
  ...
  for (const CFIProgram::Instruction &Inst : CFIP) {
    ...
    case dwarf::DW_CFA_set_loc: {
      ...
      toolchain::Expected<uint64_t> NewAddress = Inst.getOperandAsUnsigned(CFIP, 0);
      if (!NewAddress)
    ...
    case dwarf::DW_CFA_advance_loc4: {
      ...
      Rows.push_back(Row);
      toolchain::Expected<uint64_t> Offset = Inst.getOperandAsUnsigned(CFIP, 0);
      if (!Offset)
    ...
    case dwarf::DW_CFA_restore_extended: {
      ...
      toolchain::Expected<uint64_t> RegNum = Inst.getOperandAsUnsigned(CFIP, 0);
      if (!RegNum)
    ...
    case dwarf::DW_CFA_offset_extended_sf: {
      ...
      if (!RegNum)
        return RegNum.takeError();
      toolchain::Expected<int64_t> Offset = Inst.getOperandAsSigned(CFIP, 1);
      if (!Offset)
        return Offset.takeError();
    ...
    case dwarf::DW_CFA_register: {
      ...
      if (!RegNum)
        return RegNum.takeError();
      toolchain::Expected<uint64_t> NewRegNum = Inst.getOperandAsUnsigned(CFIP, 1);
      if (!NewRegNum)
    ...
    case dwarf::DW_CFA_def_cfa_offset_sf: {
      toolchain::Expected<int64_t> Offset = Inst.getOperandAsSigned(CFIP, 0);
      if (!Offset)
        return Offset.takeError();
      if (Row.getCFAValue().getLocation() != UnwindLocation::RegPlusOffset) {
        return createStringError(
    ...
    case dwarf::DW_CFA_LLVM_def_aspace_cfa_sf: {
      ...
      if (!Offset)
        return Offset.takeError();
      toolchain::Expected<uint32_t> CFAAddrSpace =
