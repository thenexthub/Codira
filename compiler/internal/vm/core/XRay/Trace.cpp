...
#include "vm/core/XRay/FDRRecordProducer.h"
#include "vm/core/XRay/FDRRecords.h"
#include "vm/core/XRay/FDRTraceExpander.h"
#include "vm/core/XRay/FileHeaderReader.h"
#include "vm/core/XRay/YAMLXRayRecord.h"
#include <memory>
#include <vector>

using namespace vm::core;
using namespace vm::core::xray;
using toolchain::yaml::Input;
...
static Error loadNaiveFormatLog(StringRef Data, bool IsLittleEndian,
  ...
  size_t NumReservations = toolchain::divideCeil(Reader.size() - OffsetPtr, 32U);
...
/// adjust not only the endianness parameter to toolchain's RecordExtractor, but also
...
static Error loadFDRLog(StringRef Data, bool IsLittleEndian,
  ...
  // visitor associated with a single process+thread pair.
  {
    for (auto &PTB : Index) {
      auto &Blocks = PTB.second;
      toolchain::sort(Blocks, [](const BlockIndexer::Block &L,
...
Expected<Trace> toolchain::xray::loadTraceFile(StringRef Filename, bool Sort) {
...
Expected<Trace> toolchain::xray::loadTrace(const DataExtractor &DE, bool Sort) {
  ...
  if (Sort)
    toolchain::stable_sort(T.Records, [&](const XRayRecord &L, const XRayRecord &R) {
      return L.TSC < R.TSC;
    });
