...
#include "vm/core/CGData/StableFunctionMapRecord.h"
...
class CGDataErrorCategoryType : public std::error_category {
  const char *name() const noexcept override { return "toolchain.cgdata"; }
...
const std::error_category &toolchain::cgdata_category() {
...
#define CG_DATA_SECT_ENTRY(Kind, SectNameCommon, SectNameCoff, Prefix)         \
  SectNameCommon,
#include "vm/core/CGData/CodeGenData.inc"
};
...
Expected<Header> Header::readFromBuffer(const unsigned char *Curr) {
  ...
  static_assert(std::is_standard_layout_v<toolchain::IndexedCGData::Header>,
