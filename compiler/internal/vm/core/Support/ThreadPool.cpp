//==-- toolchain/Support/ThreadPool.cpp - A ThreadPool implementation -*- C++ -*-==//
//
...
void StdThreadPool::grow(int requested) {
  toolchain::sys::ScopedWriter LockGuard(ThreadsLock);
  ...
  while (static_cast<int>(Threads.size()) < newThreadCount) {
    ...
    Threads.emplace_back([this, ThreadID] {
      set_thread_name(formatv("toolchain-worker-{0}", ThreadID));
...
void StdThreadPool::processTasks(ThreadPoolTaskGroup *WaitingForGroup) {
  while (true) {
    toolchain::unique_function<void()> Task;
...
void StdThreadPool::processTasksWithJobserver() {
  while (true) {
    ...
    // `toolchain::scope_exit` guarantees the job slot is released, even if the
    // task throws or we exit early. This prevents deadlocking the build.
    toolchain::scope_exit SlotReleaser(
...
bool StdThreadPool::workCompletedUnlocked(ThreadPoolTaskGroup *Group) const {
  ...
  return ActiveGroups.count(Group) == 0 &&
         !toolchain::is_contained(toolchain::make_second_range(Tasks), Group);
}
...
void StdThreadPool::wait(ThreadPoolTaskGroup &Group) {
  ...
  assert(CurrentThreadTaskGroups == nullptr ||
         !toolchain::is_contained(*CurrentThreadTaskGroups, &Group));
...
bool StdThreadPool::isWorkerThread() const {
  toolchain::sys::ScopedReader LockGuard(ThreadsLock);
  toolchain::thread::id CurrentThreadId = toolchain::this_thread::get_id();
  for (const toolchain::thread &Thread : Threads)
