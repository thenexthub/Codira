...
#include "vm/core/Support/BalancedPartitioning.h"
...
void BalancedPartitioning::run(std::vector<BPFunctionNode> &Nodes) const {
  ...
  auto NodesRange = toolchain::make_range(Nodes.begin(), Nodes.end());
  ...
  toolchain::stable_sort(NodesRange, [](const auto &L, const auto &R) {
...
void BalancedPartitioning::bisect(const FunctionNodeRange Nodes,
  ...
  unsigned NumNodes = toolchain::size(Nodes);
  if (NumNodes <= 1 || RecDepth >= Config.SplitDepth) {
    ...
    toolchain::sort(Nodes, [](const auto &L, const auto &R) {
  ...
  auto NodesMid =
      toolchain::partition(Nodes, [&](auto &N) { return N.Bucket == LeftBucket; });
  unsigned MidOffset = Offset + std::distance(Nodes.begin(), NodesMid);

  auto LeftNodes = toolchain::make_range(Nodes.begin(), NodesMid);
  auto RightNodes = toolchain::make_range(NodesMid, Nodes.end());
...
void BalancedPartitioning::runIterations(const FunctionNodeRange Nodes,
  ...
  // functions
  for (auto &N : Nodes)
    toolchain::erase_if(N.UtilityNodes, [&](auto &UN) {
...
unsigned BalancedPartitioning::runIteration(const FunctionNodeRange Nodes,
  ...
  auto LeftEnd = toolchain::partition(
      Gains, [&](const auto &GP) { return GP.second->Bucket == LeftBucket; });
  auto LeftRange = toolchain::make_range(Gains.begin(), LeftEnd);
  auto RightRange = toolchain::make_range(LeftEnd, Gains.end());

  // Sort gains in descending order
  auto LargerGain = [](const auto &L, const auto &R) {
    return L.first > R.first;
  };
  toolchain::stable_sort(LeftRange, LargerGain);
  toolchain::stable_sort(RightRange, LargerGain);

  unsigned NumMovedDataVertices = 0;
  for (auto [LeftPair, RightPair] : toolchain::zip(LeftRange, RightRange)) {
...
void BalancedPartitioning::split(const FunctionNodeRange Nodes,
  ...
  for (auto &N : toolchain::make_range(Nodes.begin(), NodesMid))
    N.Bucket = StartBucket;
  for (auto &N : toolchain::make_range(NodesMid, Nodes.end()))
