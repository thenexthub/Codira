...
//
//===----------------------------------------------------------------------===//
#include "vm/core/Support/Mustache.h"
...
using namespace vm::core::mustache;
...
public:
  ...
  void render(const toolchain::json::Value &Data, MustacheOutputStream &OS);

private:
  void renderLambdas(const toolchain::json::Value &Contexts,
                     MustacheOutputStream &OS, Lambda &L);

  void renderSectionLambdas(const toolchain::json::Value &Contexts,
                            MustacheOutputStream &OS, SectionLambda &L);

  void renderPartial(const toolchain::json::Value &Contexts,
                     MustacheOutputStream &OS, ASTNode *Partial);

  void renderChild(const toolchain::json::Value &Context, MustacheOutputStream &OS);

  const toolchain::json::Value *findContext();
  ...
  const toolchain::json::Value *ParentContext;
};
...
public:
  explicit EscapeStringStream(toolchain::raw_ostream &WrappedStream,
                              EscapeMap &Escape)
...
private:
  ...
  toolchain::raw_ostream &WrappedStream;
};
...
protected:
  void write_impl(const char *Ptr, size_t Size) override {
    toolchain::StringRef Data(Ptr, Size);
...
static void toMustacheString(const json::Value &Data, raw_ostream &OS) {
  ...
  case json::Value::Boolean: {
    toolchain::json::OStream JOS(OS, 2);
    JOS.value(Data);
    break;
  }
  }
...
void ASTNode::render(const toolchain::json::Value &Data, MustacheOutputStream &OS) {
...
void ASTNode::renderLambdas(const toolchain::json::Value &Contexts,
                            MustacheOutputStream &OS, Lambda &L) {
...
void ASTNode::renderSectionLambdas(const toolchain::json::Value &Contexts,
                                   MustacheOutputStream &OS, SectionLambda &L) {
...
void Template::render(const toolchain::json::Value &Data, toolchain::raw_ostream &OS) {
