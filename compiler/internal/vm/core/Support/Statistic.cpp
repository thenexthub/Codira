...
void toolchain::initStatisticOptions() {
...
void toolchain::EnableStatistics(bool DoPrintOnExit) {
  Enabled = true;
  PrintOnExit = DoPrintOnExit;
}

bool toolchain::AreStatisticsEnabled() { return Enabled || EnableStats; }
...
std::vector<std::pair<StringRef, uint64_t>> toolchain::GetStatistics() {
  sys::SmartScopedLock<true> Reader(*StatLock);
