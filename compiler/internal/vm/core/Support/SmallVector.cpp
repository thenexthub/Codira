...
static void *replaceAllocation(void *NewElts, size_t TSize, size_t NewCapacity,
                               size_t VSize = 0) {
  void *NewEltsReplace = toolchain::safe_malloc(NewCapacity * TSize);
  if (VSize)
...
void *SmallVectorBase<Size_T>::mallocForGrow(void *FirstEl, size_t MinSize,
  ...
  void *NewElts = toolchain::safe_malloc(NewCapacity * TSize);
  if (NewElts == FirstEl)
...
void SmallVectorBase<Size_T>::grow_pod(void *FirstEl, size_t MinSize,
  ...
  void *NewElts;
  if (BeginX == FirstEl) {
    NewElts = toolchain::safe_malloc(NewCapacity * TSize);
    if (NewElts == FirstEl)
      NewElts = replaceAllocation(NewElts, TSize, NewCapacity);
    ...
    NewElts = toolchain::safe_realloc(this->BeginX, NewCapacity * TSize);
    if (NewElts == FirstEl)
