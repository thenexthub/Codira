...
void toolchain::initGraphWriterOptions() { *ViewBackground; }
#else
void toolchain::initGraphWriterOptions() {}
#endif

std::string toolchain::DOT::EscapeString(const std::string &Label) {
...
StringRef toolchain::DOT::getColorString(unsigned ColorNumber) {
...
static std::string replaceIllegalFilenameChars(std::string Filename,
  ...
  for (char IllegalChar : IllegalChars)
    toolchain::replace(Filename, IllegalChar, ReplacementChar);

  return Filename;
}

std::string toolchain::createGraphFilename(const Twine &Name, int &FD) {
  FD = -1;
...
bool toolchain::DisplayGraph(StringRef FilenameRef, bool wait,
