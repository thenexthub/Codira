...
unsigned SourceMgr::SrcBuffer::getLineNumberSpecialized(const char *Ptr) const {
  ...
  // toolchain::lower_bound gives the number of EOL before PtrOffset. Add 1 to get
  // the line number.
  return toolchain::lower_bound(Offsets, PtrOffset) - Offsets.begin() + 1;
}
...
static void buildFixItLine(std::string &CaretLine, std::string &FixItLine,
  ...
  for (const toolchain::SMFixIt &Fixit : FixIts) {
    ...
    toolchain::copy(Fixit.getText(), FixItLine.begin() + HintCol);
