...
#include "vm/core/ADT/RadixTree.h"
...
unsigned Matcher::match(StringRef Query) const {
  if (RemoveDotSlash)
    Query = toolchain::sys::path::remove_leading_dotslash(Query);
...
SpecialCaseList::create(const std::vector<std::string> &Paths,
                        toolchain::vfs::FileSystem &FS, std::string &Error) {
...
bool SpecialCaseList::parse(unsigned FileIdx, const MemoryBuffer *MB,
  ...
  // In https://reviews.toolchain.org/D154014 we added glob support and planned
  ...
  // https://discourse.toolchain.org/t/use-glob-instead-of-regex-for-specialcaselists/71666
  ...
  for (line_iterator LineIt(*MB, /*SkipBlanks=*/true, /*CommentMarker=*/'#');
    ...
    auto [It, _] = CurrentImpl->Entries[Prefix].try_emplace(
        Category, UseGlobs,
        RemoveDotSlash && toolchain::is_contained(PathPrefixes, Prefix));
