...
static Error createDirectoriesOnDemand(StringRef OutputPath,
                                       OutputConfig Config,
                                       toolchain::function_ref<Error()> CreateFile) {
...
Error OnDiskOutputFile::keep() {
  ...
  toolchain::scope_exit RemoveDiscardOnSignal([&]() {
    if (Config.getDiscardOnSignal())
  ...
  if (Config.getAppend() && OutputPath != "-") {
    ...
    while (1) {
      ...
      toolchain::LockFileManager Lock(OutputPath);
      bool Owned;
      ...
      if (Owned) {
        ...
        std::error_code EC;
        toolchain::raw_fd_ostream Out(OutputPath, EC, toolchain::sys::fs::OF_Append);
        if (EC)
          return convertToOutputError(OutputPath, EC);
