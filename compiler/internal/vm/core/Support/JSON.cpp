...
std::optional<toolchain::StringRef> Object::getString(StringRef K) const {
  if (auto *V = get(K))
...
void Value::print(toolchain::raw_ostream &OS) const { OS << *this; }
...
void Path::report(toolchain::StringLiteral Msg) {
...
Error Path::Root::getError() const {
  ...
  if (ErrorPath.empty()) {
    ...
    for (const Path::Segment &S : toolchain::reverse(ErrorPath)) {
      if (S.isField())
        OS << '.' << S.field();
      else
        OS << '[' << S.index() << ']';
  ...
  return createStringError(toolchain::inconvertibleErrorCode(), S);
}
...
static void abbreviate(const Value &V, OStream &JOS) {
  ...
  case Value::Object:
    ...
    break;
  case Value::String: {
    toolchain::StringRef S = *V.getAsString();
    if (S.size() < 40) {
      JOS.value(V);
    } else {
...
void Path::Root::printErrorContext(const Value &R, raw_ostream &OS) const {
  ...
  auto PrintValue = [&](const Value &V, ArrayRef<Segment> Path, auto &Recurse) {
    ...
    if (S.isField()) {
      ...
      toolchain::StringRef FieldName = S.field();
...
void toolchain::json::OStream::value(const Value &V) {
  switch (V.kind()) {
  case Value::Null:
    valueBegin();
    OS << "null";
...
void toolchain::json::OStream::valueBegin() {
...
void OStream::comment(toolchain::StringRef Comment) {
...
void toolchain::json::OStream::newline() {
  if (IndentSize) {
    OS.write('\n');
    OS.indent(Indent);
  }
...
void toolchain::json::OStream::arrayBegin() {
  valueBegin();
  Stack.emplace_back();
...
void toolchain::json::OStream::arrayEnd() {
...
void toolchain::json::OStream::objectBegin() {
  valueBegin();
  Stack.emplace_back();
...
void toolchain::json::OStream::objectEnd() {
...
void toolchain::json::OStream::attributeBegin(toolchain::StringRef Key) {
  assert(Stack.back().Ctx == Object);
  if (Stack.back().HasValue)
    OS << ',';
  newline();
...
void toolchain::json::OStream::attributeEnd() {
...
raw_ostream &toolchain::json::OStream::rawValueBegin() {
  valueBegin();
  Stack.emplace_back();
...
void toolchain::json::OStream::rawValueEnd() {
