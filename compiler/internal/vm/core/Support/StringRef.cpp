...
#include "vm/core/ADT/edit_distance.h"
...
unsigned StringRef::edit_distance(toolchain::StringRef Other,
  ...
  return toolchain::ComputeEditDistance(ArrayRef(data(), size()),
                                   ArrayRef(Other.data(), Other.size()),
                                   AllowReplacements, MaxEditDistance);
}

unsigned toolchain::StringRef::edit_distance_insensitive(
    StringRef Other, bool AllowReplacements, unsigned MaxEditDistance) const {
  return toolchain::ComputeMappedEditDistance(
      ArrayRef(data(), size()), ArrayRef(Other.data(), Other.size()),
      toolchain::toLower, AllowReplacements, MaxEditDistance);
}
...
unsigned toolchain::getAutoSenseRadix(StringRef &Str) {
  if (Str.empty())
    return 10;
...
hash_code toolchain::hash_value(StringRef S) { return hash_combine_range(S); }
