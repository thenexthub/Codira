...
#include "vm/core/Support/Watchdog.h"
...
void toolchain::setBugReportMsg(const char *Msg) {
  BugReportMsg = Msg;
}

const char *toolchain::getBugReportMsg() {
  return BugReportMsg;
}
...
void toolchain::EnablePrettyStackTraceOnSigInfoForThisThread(bool ShouldEnable) {
#if ENABLE_BACKTRACES
  if (!ShouldEnable) {
