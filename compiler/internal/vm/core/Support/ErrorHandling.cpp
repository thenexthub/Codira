...
#include "vm/core-c/ErrorHandling.h"
...
#include "vm/core/Support/WindowsError.h"
...
void toolchain::reportFatalInternalError(const char *reason) {
  report_fatal_error(reason, /*GenCrashDiag=*/true);
}
void toolchain::reportFatalInternalError(StringRef reason) {
  report_fatal_error(reason, /*GenCrashDiag=*/true);
}
void toolchain::reportFatalInternalError(const Twine &reason) {
  report_fatal_error(reason, /*GenCrashDiag=*/true);
}
void toolchain::reportFatalUsageError(const char *reason) {
  report_fatal_error(reason, /*GenCrashDiag=*/false);
}
void toolchain::reportFatalUsageError(StringRef reason) {
  report_fatal_error(reason, /*GenCrashDiag=*/false);
}
void toolchain::reportFatalUsageError(const Twine &reason) {
  report_fatal_error(reason, /*GenCrashDiag=*/false);
}
...
void toolchain::llvm_unreachable_internal(const char *msg, const char *file,
                                     unsigned line) {
...
// https://github.com/toolchain/toolchain-project/issues/89137
...
std::error_code toolchain::mapLastWindowsError() {
  ...
  if (EV == ERROR_ACCESS_DENIED) {
    toolchain::errc code = RtlGetLastNtStatus() == STATUS_DELETE_PENDING
                          ? errc::delete_pending
