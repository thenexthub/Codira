...
#include "vm/core/Support/ProgramStack.h"
...
uintptr_t toolchain::getStackPointer() {
...
unsigned toolchain::getDefaultStackSize() {
#ifdef LLVM_ON_UNIX
  rlimit RL;
...
#ifdef LLVM_HAS_SPLIT_STACKS
void toolchain::runOnNewStack(unsigned StackSize, function_ref<void()> Fn) {
  if (StackSize == 0)
