...
bool toolchain::cl::ProvidePositionalOption(Option *Handler, StringRef Arg, int i) {
  int Dummy = i;
...
static void ExpandBasePaths(StringRef BasePath, StringSaver &Saver,
  ...
  for (StringRef::size_type TokenPos = ArgString.find(Token);
    ...
    if (ResponseFile.empty())
      ...
      toolchain::sys::path::append(ResponseFile, LHS);
  ...
  if (!ResponseFile.empty()) {
    ...
    if (!Remaining.empty())
      toolchain::sys::path::append(ResponseFile, Remaining);
...
Error ExpansionContext::expandResponseFile(
  ...
  toolchain::ErrorOr<std::unique_ptr<MemoryBuffer>> MemBufOrErr =
      FS->getBufferForFile(FName);
  if (!MemBufOrErr) {
    std::error_code EC = MemBufOrErr.getError();
    return toolchain::createStringError(EC, Twine("cannot not open file '") + FName +
                                           "': " + EC.message());
  }
  ...
  StringRef BasePath = toolchain::sys::path::parent_path(FName);
  for (const char *&Arg : NewArgv) {
    ...
    if (ConfigInclusion && !toolchain::sys::path::has_parent_path(FileName)) {
      ...
      ResponseFile.append(BasePath);
      toolchain::sys::path::append(ResponseFile, FileName);
    }
    Arg = Saver.save(ResponseFile.str()).data();
  }
  return Error::success();
...
Error ExpansionContext::expandResponseFiles(
  ...
  for (unsigned I = 0; I != Argv.size();) {
    ...
    if (toolchain::sys::path::is_relative(FName)) {
      ...
      toolchain::sys::path::append(CurrDir, FName);
      FName = CurrDir.c_str();
    }

    ErrorOr<toolchain::vfs::Status> Res = FS->status(FName);
    if (!Res || !Res->exists()) {
      std::error_code EC = Res.getError();
      if (!InConfigFile) {
        // If the specified file does not exist, leave '@file' unexpanded, as
        // libiberty does.
        if (!EC || EC == toolchain::errc::no_such_file_or_directory) {
          ++I;
          continue;
        }
      }
    ...
    const toolchain::vfs::Status &FileStatus = Res.get();

    auto IsEquivalent =
        [FileStatus, this](const ResponseFileRecord &RFile) -> ErrorOr<bool> {
      ErrorOr<toolchain::vfs::Status> RHS = FS->status(RFile.File);
      if (!RHS)
        return RHS.getError();
...
bool ExpansionContext::findConfigFile(StringRef FileName,
  ...
  // configuration file.
  if (toolchain::sys::path::has_parent_path(FileName)) {
    CfgFilePath = FileName;
    if (toolchain::sys::path::is_relative(FileName) && FS->makeAbsolute(CfgFilePath))
      return false;
  ...
  for (const StringRef &Dir : SearchDirs) {
    ...
    toolchain::sys::path::append(CfgFilePath, FileName);
    toolchain::sys::path::native(CfgFilePath);
    if (FileExists(CfgFilePath)) {
      FilePath.assign(CfgFilePath.begin(), CfgFilePath.end());
      return true;
    }
...
protected:
  void printOptions(StrOptionPairVector &Opts, size_t MaxArgLen) override {
    ...
    // sorting.
    toolchain::append_range(SortedCategories,
    ...
    for (const auto &I : Opts) {
      ...
      for (OptionCategory *Cat : Opt->Categories) {
        assert(toolchain::is_contained(SortedCategories, Cat) &&
...
void cl::printBuildConfig(raw_ostream &OS) {
  ...
  OS << "Build config: ";
  toolchain::interleaveComma(cl::getCompilerBuildConfig(), OS);
  OS << '\n';
#endif
}
