...
TimeTraceProfiler *toolchain::getTimeTraceProfilerInstance() {
...
struct toolchain::TimeTraceProfilerEntry {
...
struct toolchain::TimeTraceProfiler {
  TimeTraceProfiler(unsigned TimeTraceGranularity = 0, StringRef ProcName = "",
      ...
      : BeginningOfTime(system_clock::now()), StartTime(ClockType::now()),
        ...
        Tid(toolchain::get_threadid()), TimeTraceGranularity(TimeTraceGranularity),
        TimeTraceVerbose(TimeTraceVerbose) {
    toolchain::get_thread_name(ThreadName);
  }

  TimeTraceProfilerEntry *
  begin(std::string Name, toolchain::function_ref<std::string()> Detail,
  ...
  TimeTraceProfilerEntry *
  begin(std::string Name, toolchain::function_ref<TimeTraceMetadata()> Metadata,
        TimeTraceEventType EventType = TimeTraceEventType::CompleteEvent) {
    assert(EventType != TimeTraceEventType::InstantEvent &&
           "Instant Events don't have begin and end.");
    Stack.emplace_back(std::make_unique<InProgressEntry>(
  ...
  void insert(std::string Name, toolchain::function_ref<std::string()> Detail) {
    if (Stack.empty())
      return;
  ...
  void end(TimeTraceProfilerEntry &E) {
    ...
    const auto *Iter =
        toolchain::find_if(Stack, [&](const std::unique_ptr<InProgressEntry> &Val) {
          return &Val->Event == &E;
        });
    ...
    // itself.
    if (toolchain::none_of(toolchain::drop_begin(toolchain::reverse(Stack)),
  ...
  void write(raw_pwrite_stream &OS) {
    ...
    assert(toolchain::all_of(Instances.List,
    ...
    toolchain::sort(SortedTotals, [](const NameAndCountAndDurationType &A,
...
bool toolchain::isTimeTraceVerbose() {
  return getTimeTraceProfilerInstance() &&
         getTimeTraceProfilerInstance()->TimeTraceVerbose;
}

void toolchain::timeTraceProfilerInitialize(unsigned TimeTraceGranularity,
                                       StringRef ProcName,
                                       bool TimeTraceVerbose) {
  assert(TimeTraceProfilerInstance == nullptr &&
         "Profiler should not be initialized");
  TimeTraceProfilerInstance = new TimeTraceProfiler(
      TimeTraceGranularity, toolchain::sys::path::filename(ProcName),
      TimeTraceVerbose);
}

// Removes all TimeTraceProfilerInstances.
// Called from main thread.
void toolchain::timeTraceProfilerCleanup() {
...
void toolchain::timeTraceProfilerFinishThread() {
  auto &Instances = getTimeTraceProfilerInstances();
  std::lock_guard<std::mutex> Lock(Instances.Lock);
...
void toolchain::timeTraceProfilerWrite(raw_pwrite_stream &OS) {
  assert(TimeTraceProfilerInstance != nullptr &&
         "Profiler object can't be null");
  TimeTraceProfilerInstance->write(OS);
}

Error toolchain::timeTraceProfilerWrite(StringRef PreferredFileName,
...
TimeTraceProfilerEntry *toolchain::timeTraceProfilerBegin(StringRef Name,
                                                     StringRef Detail) {
...
TimeTraceProfilerEntry *
toolchain::timeTraceProfilerBegin(StringRef Name,
                             toolchain::function_ref<std::string()> Detail) {
  if (TimeTraceProfilerInstance != nullptr)
...
TimeTraceProfilerEntry *
toolchain::timeTraceProfilerBegin(StringRef Name,
                             toolchain::function_ref<TimeTraceMetadata()> Metadata) {
  if (TimeTraceProfilerInstance != nullptr)
...
TimeTraceProfilerEntry *toolchain::timeTraceAsyncProfilerBegin(StringRef Name,
                                                          StringRef Detail) {
  if (TimeTraceProfilerInstance != nullptr)
    return TimeTraceProfilerInstance->begin(
        std::string(Name), [&]() { return std::string(Detail); },
...
void toolchain::timeTraceAddInstantEvent(StringRef Name,
                                    toolchain::function_ref<std::string()> Detail) {
  if (TimeTraceProfilerInstance != nullptr)
    TimeTraceProfilerInstance->insert(std::string(Name), Detail);
}

void toolchain::timeTraceProfilerEnd() {
  if (TimeTraceProfilerInstance != nullptr)
    TimeTraceProfilerInstance->end();
}

void toolchain::timeTraceProfilerEnd(TimeTraceProfilerEntry *E) {
  if (TimeTraceProfilerInstance != nullptr)
