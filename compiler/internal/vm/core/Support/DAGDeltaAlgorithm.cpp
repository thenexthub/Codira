...
#include "vm/core/ADT/DAGDeltaAlgorithm.h"
#include "vm/core/ADT/DeltaAlgorithm.h"
...
DAGDeltaAlgorithmImpl::DAGDeltaAlgorithmImpl(
  ...
  LLVM_DEBUG({
    toolchain::errs() << "-- DAGDeltaAlgorithmImpl --\n";
    toolchain::errs() << "Changes: [";
    ...
    toolchain::errs() << "Predecessor Closure:\n";
    for (change_ty Change : Changes) {
      toolchain::errs() << format("  %-4d: [", Change);
    ...
    toolchain::errs() << "Successor Closure:\n";
    for (change_ty Change : Changes) {
      toolchain::errs() << format("  %-4d: [", Change);
      for (succ_closure_iterator_ty it2 = succ_closure_begin(Change),
                                    ie2 = succ_closure_end(Change);
...
DAGDeltaAlgorithmImpl::Run() {
  ...
  while (!CurrentSet.empty()) {
    LLVM_DEBUG({
      toolchain::errs() << "DAG_DD - " << CurrentSet.size() << " active changes, "
