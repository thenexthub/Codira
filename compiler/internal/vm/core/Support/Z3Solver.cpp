...
#include "vm/core/Support/NativeFormatting.h"
#include "vm/core/Support/SMTAPI.h"
...
void Z3ErrorHandler(Z3_context Context, Z3_error_code Error) {
  toolchain::report_fatal_error("Z3 error: " +
                           toolchain::Twine(Z3_get_error_msg(Context, Error)));
}
...
public:
  ...
  LLVM_DUMP_METHOD void dump() const { print(toolchain::errs()); }
}; // end class Z3Model

/// Get the corresponding IEEE floating-point type for a given bitwidth.
static const toolchain::fltSemantics &getFloatSemantics(unsigned BitWidth) {
  switch (BitWidth) {
  default:
...
static bool areEquivalent(const toolchain::fltSemantics &LHS,
                          const toolchain::fltSemantics &RHS) {
  return (toolchain::APFloat::semanticsPrecision(LHS) ==
          toolchain::APFloat::semanticsPrecision(RHS)) &&
         (toolchain::APFloat::semanticsMinExponent(LHS) ==
          toolchain::APFloat::semanticsMinExponent(RHS)) &&
         (toolchain::APFloat::semanticsMaxExponent(LHS) ==
          toolchain::APFloat::semanticsMaxExponent(RHS)) &&
         (toolchain::APFloat::semanticsSizeInBits(LHS) ==
          toolchain::APFloat::semanticsSizeInBits(RHS));
}
...
public:
  ...
  SMTExprRef mkBitvector(const toolchain::APSInt Int, unsigned BitWidth) override {
  ...
  SMTExprRef mkFloat(const toolchain::APFloat Float) override {
    SMTSortRef Sort =
        getFloatSort(toolchain::APFloat::semanticsSizeInBits(Float.getSemantics()));

    toolchain::APSInt Int = toolchain::APSInt(Float.bitcastToAPInt(), false);
  ...
  toolchain::APSInt getBitvector(const SMTExprRef &Exp, unsigned BitWidth,
                            bool isUnsigned) override {
    return toolchain::APSInt(
        toolchain::APInt(BitWidth,
  ...
  bool toAPFloat(const SMTSortRef &Sort, const SMTExprRef &AST,
                 toolchain::APFloat &Float, bool useSemantics) {
    assert(Sort->isFloatSort() && "Unsupported sort to floating-point!");

    toolchain::APSInt Int(Sort->getFloatSortSize(), true);
    ...
    Float = toolchain::APFloat(Semantics, Int);
    return true;
  }

  bool toAPSInt(const SMTSortRef &Sort, const SMTExprRef &AST,
                toolchain::APSInt &Int, bool useSemantics) {
    ...
    if (Sort->isBooleanSort()) {
      ...
      Int = toolchain::APSInt(toolchain::APInt(Int.getBitWidth(), getBoolean(AST)),
                         Int.isUnsigned());
      return true;
    }
  ...
  bool getInterpretation(const SMTExprRef &Exp, toolchain::APSInt &Int) override {
  ...
  bool getInterpretation(const SMTExprRef &Exp, toolchain::APFloat &Float) override {
    Z3Model Model(Context, Z3_solver_get_model(Context.Context, Solver));
    Z3_func_decl Func = Z3_get_app_decl(
        Context.Context, Z3_to_app(Context.Context, toZ3Expr(*Exp).AST));
    if (Z3_model_has_interp(Context.Context, Model.Model, Func) != Z3_L_TRUE)
...
std::unique_ptr<SMTSolverStatistics> Z3Solver::getStatistics() const {
  ...
  toolchain::scope_exit StatsGuard([&C, &S] { Z3_stats_dec_ref(C, S); });
  Z3Statistics Result;
...
toolchain::SMTSolverRef toolchain::CreateZ3Solver() {
#if LLVM_WITH_Z3
  return std::make_unique<Z3Solver>();
#else
  toolchain::report_fatal_error("LLVM was not compiled with Z3 support, rebuild "
...
LLVM_DUMP_METHOD void SMTSort::dump() const { print(toolchain::errs()); }
LLVM_DUMP_METHOD void SMTExpr::dump() const { print(toolchain::errs()); }
LLVM_DUMP_METHOD void SMTSolver::dump() const { print(toolchain::errs()); }
LLVM_DUMP_METHOD void SMTSolverStatistics::dump() const { print(toolchain::errs()); }
#endif
