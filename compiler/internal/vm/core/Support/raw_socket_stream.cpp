//===-- toolchain/Support/raw_socket_stream.cpp - Socket streams --*- C++ -*-===//
//
...
#include "vm/core/Support/raw_socket_stream.h"
...
WSABalancer::WSABalancer() {
  ...
  if (WSAStartup(MAKEWORD(2, 2), &WsaData) != 0) {
    toolchain::report_fatal_error("WSAStartup failed");
  }
}
...
static Expected<int> getSocketFD(StringRef SocketPath) {
  ...
  if (Socket == -1) {
#endif // _WIN32
    return toolchain::make_error<StringError>(getLastSocketErrorCode(),
  ...
  // toolchain/unittests/Support/raw_socket_stream_test do both on the same thread
...
Expected<ListeningSocket> ListeningSocket::createUnix(StringRef SocketPath,
  ...
  if (toolchain::sys::fs::exists(SocketPath)) {
  ...
  if (::bind(Socket, (struct sockaddr *)&Addr, sizeof(Addr)) == -1) {
    ...
    ::close(Socket);
    return toolchain::make_error<StringError>(EC, "Bind error");
  }
...
ListeningSocket::accept(const std::chrono::milliseconds &Timeout) {
  ...
  if (TimeoutErr)
    return toolchain::make_error<StringError>(TimeoutErr, "Timeout error");
