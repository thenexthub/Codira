...
unsigned APInt::countLeadingOnesSlowCase() const {
  ...
  int i = getNumWords() - 1;
  unsigned Count = toolchain::countl_one(U.pVal[i] << shift);
  if (Count == highWordBits) {
    for (i--; i >= 0; --i) {
      if (U.pVal[i] == WORDTYPE_MAX)
        Count += APINT_BITS_PER_WORD;
      else {
        Count += toolchain::countl_one(U.pVal[i]);
        break;
      }
    }
  }
...
unsigned APInt::countTrailingZerosSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == 0; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += toolchain::countr_zero(U.pVal[i]);
...
unsigned APInt::countTrailingOnesSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == WORDTYPE_MAX; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += toolchain::countr_one(U.pVal[i]);
...
unsigned APInt::countPopulationSlowCase() const {
  ...
  for (unsigned i = 0; i < getNumWords(); ++i)
    Count += toolchain::popcount(U.pVal[i]);
  return Count;
}
...
APInt APInt::byteSwap() const {
  ...
  if (BitWidth == 16)
    return APInt(BitWidth, toolchain::byteswap<uint16_t>(U.VAL));
  if (BitWidth == 32)
    return APInt(BitWidth, toolchain::byteswap<uint32_t>(U.VAL));
  if (BitWidth <= 64) {
    uint64_t Tmp1 = toolchain::byteswap<uint64_t>(U.VAL);
    Tmp1 >>= (64 - BitWidth);
  ...
  for (unsigned I = 0, N = getNumWords(); I != N; ++I)
    Result.U.pVal[I] = toolchain::byteswap<uint64_t>(U.pVal[N - I - 1]);
...
#define DEBUG_KNUTH(X) do {} while(false)
  ...
  // be of length m+n+1.
  unsigned shift = toolchain::countl_zero(v[n - 1]);
  uint32_t v_carry = 0;
  uint32_t u_carry = 0;
  if (shift) {
...
unsigned APInt::tcLSB(const WordType *parts, unsigned n) {
  for (unsigned i = 0; i < n; i++) {
    if (parts[i] != 0) {
      unsigned lsb = toolchain::countr_zero(parts[i]);
...
unsigned APInt::tcMSB(const WordType *parts, unsigned n) {
  do {
    ...
    if (parts[n] != 0) {
      ...
      unsigned msb = toolchain::Log2_64(parts[n]);
...
toolchain::APIntOps::GetMostSignificantDifferentBit(const APInt &A, const APInt &B) {
...
APInt toolchain::APIntOps::ScaleBitMask(const APInt &A, unsigned NewBitWidth,
                                   bool MatchAllBits) {
...
void toolchain::StoreIntToMemory(const APInt &IntVal, uint8_t *Dst,
                            unsigned StoreBytes) {
...
void toolchain::LoadIntFromMemory(APInt &IntVal, const uint8_t *Src,
                             unsigned LoadBytes) {
...
APInt toolchain::APIntOps::fshl(const APInt &Hi, const APInt &Lo,
                           const APInt &Shift) {
...
APInt toolchain::APIntOps::fshr(const APInt &Hi, const APInt &Lo,
                           const APInt &Shift) {
  assert(Hi.getBitWidth() == Lo.getBitWidth());
  unsigned ShiftAmt = rotateModulo(Hi.getBitWidth(), Shift);
  if (ShiftAmt == 0)
...
APInt toolchain::APIntOps::clmul(const APInt &LHS, const APInt &RHS) {
...
APInt toolchain::APIntOps::clmulr(const APInt &LHS, const APInt &RHS) {
  assert(LHS.getBitWidth() == RHS.getBitWidth());
  return clmul(LHS.reverseBits(), RHS.reverseBits()).reverseBits();
}

APInt toolchain::APIntOps::clmulh(const APInt &LHS, const APInt &RHS) {
  assert(LHS.getBitWidth() == RHS.getBitWidth());
