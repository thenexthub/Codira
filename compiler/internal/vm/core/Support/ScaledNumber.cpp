...
using namespace vm::core::ScaledNumbers;
...
std::pair<uint64_t, int16_t> ScaledNumbers::multiply64(uint64_t LHS,
  ...
  unsigned LeadingZeros = toolchain::countl_zero(Upper);
...
std::pair<uint32_t, int16_t> ScaledNumbers::divide32(uint32_t Dividend,
  ...
  int Shift = 0;
  if (int Zeros = toolchain::countl_zero(Dividend64)) {
    Shift -= Zeros;
    Dividend64 <<= Zeros;
  }
...
std::pair<uint64_t, int16_t> ScaledNumbers::divide64(uint64_t Dividend,
  ...
  int Shift = 0;
  if (int Zeros = toolchain::countr_zero(Divisor)) {
    Shift -= Zeros;
    Divisor >>= Zeros;
  }
  ...
  if (int Zeros = toolchain::countl_zero(Dividend)) {
    Shift -= Zeros;
    Dividend <<= Zeros;
  }
