...
const toolchain::fltSemantics &APFloatBase::EnumToSemantics(Semantics S) {
  switch (S) {
  case S_IEEEhalf:
    return IEEEhalf();
  case S_BFloat:
...
APFloatBase::SemanticsToEnum(const toolchain::fltSemantics &Sem) {
  if (&Sem == &toolchain::APFloat::IEEEhalf())
    return S_IEEEhalf;
  else if (&Sem == &toolchain::APFloat::BFloat())
    return S_BFloat;
  else if (&Sem == &toolchain::APFloat::IEEEsingle())
    return S_IEEEsingle;
  else if (&Sem == &toolchain::APFloat::IEEEdouble())
    return S_IEEEdouble;
  else if (&Sem == &toolchain::APFloat::IEEEquad())
    return S_IEEEquad;
  else if (&Sem == &toolchain::APFloat::PPCDoubleDouble())
    return S_PPCDoubleDouble;
  else if (&Sem == &toolchain::APFloat::PPCDoubleDoubleLegacy())
    return S_PPCDoubleDoubleLegacy;
  else if (&Sem == &toolchain::APFloat::Float8E5M2())
    return S_Float8E5M2;
  else if (&Sem == &toolchain::APFloat::Float8E5M2FNUZ())
    return S_Float8E5M2FNUZ;
  else if (&Sem == &toolchain::APFloat::Float8E4M3())
    return S_Float8E4M3;
  else if (&Sem == &toolchain::APFloat::Float8E4M3FN())
    return S_Float8E4M3FN;
  else if (&Sem == &toolchain::APFloat::Float8E4M3FNUZ())
    return S_Float8E4M3FNUZ;
  else if (&Sem == &toolchain::APFloat::Float8E4M3B11FNUZ())
    return S_Float8E4M3B11FNUZ;
  else if (&Sem == &toolchain::APFloat::Float8E3M4())
    return S_Float8E3M4;
  else if (&Sem == &toolchain::APFloat::FloatTF32())
    return S_FloatTF32;
  else if (&Sem == &toolchain::APFloat::Float8E8M0FNU())
    return S_Float8E8M0FNU;
  else if (&Sem == &toolchain::APFloat::Float6E3M2FN())
    return S_Float6E3M2FN;
  else if (&Sem == &toolchain::APFloat::Float6E2M3FN())
    return S_Float6E2M3FN;
  else if (&Sem == &toolchain::APFloat::Float4E2M1FN())
    return S_Float4E2M1FN;
  else if (&Sem == &toolchain::APFloat::x87DoubleExtended())
...
APInt IEEEFloat::convertF80LongDoubleAPFloatToAPInt() const {
  assert(semantics ==
         (const toolchain::fltSemantics *)&APFloatBase::semX87DoubleExtended);
  assert(partCount()==2);
...
APInt IEEEFloat::convertPPCDoubleDoubleLegacyAPFloatToAPInt() const {
  assert(semantics ==
         (const toolchain::fltSemantics *)&APFloatBase::semPPCDoubleDoubleLegacy);
  assert(partCount()==2);
...
APInt IEEEFloat::bitcastToAPInt() const {
  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEhalf)
    return convertHalfAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semBFloat)
    return convertBFloatAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEsingle)
    return convertFloatAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEdouble)
    return convertDoubleAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEquad)
  ...
  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E5M2)
    return convertFloat8E5M2APFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E5M2FNUZ)
    return convertFloat8E5M2FNUZAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E4M3)
    return convertFloat8E4M3APFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E4M3FN)
    return convertFloat8E4M3FNAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E4M3FNUZ)
    return convertFloat8E4M3FNUZAPFloatToAPInt();

  if (semantics ==
      (const toolchain::fltSemantics *)&APFloatBase::semFloat8E4M3B11FNUZ)
    return convertFloat8E4M3B11FNUZAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E3M4)
    return convertFloat8E3M4APFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloatTF32)
    return convertFloatTF32APFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat8E8M0FNU)
    return convertFloat8E8M0FNUAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat6E3M2FN)
    return convertFloat6E3M2FNAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat6E2M3FN)
    return convertFloat6E2M3FNAPFloatToAPInt();

  if (semantics == (const toolchain::fltSemantics *)&APFloatBase::semFloat4E2M1FN)
...
float IEEEFloat::convertToFloat() const {
  assert(semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEsingle &&
...
double IEEEFloat::convertToDouble() const {
  assert(semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEdouble &&
...
float128 IEEEFloat::convertToQuad() const {
  assert(semantics == (const toolchain::fltSemantics *)&APFloatBase::semIEEEquad &&
...
void IEEEFloat::initFromIEEEAPInt(const APInt &api) {
  ...
  bool all_zero_significand = toolchain::all_of(mysignificand, equal_to(0));
...
int IEEEFloat::getExactLog2Abs() const {
  ...
  for (int i = 0; i < PartCount; ++i) {
    PopCount += toolchain::popcount(Parts[i]);
    if (PopCount > 1)
      return INT_MIN;
  }
  ...
  for (int i = 0; i < PartCount;
    ...
    if (Parts[i] != 0) {
      return exponent - semantics->precision + CountrParts +
             toolchain::countr_zero(Parts[i]) + 1;
    }
  }
...
double APFloat::convertToDouble() const {
  if (&getSemantics() ==
      (const toolchain::fltSemantics *)&APFloatBase::semIEEEdouble)
...
float128 APFloat::convertToQuad() const {
  if (&getSemantics() == (const toolchain::fltSemantics *)&APFloatBase::semIEEEquad)
...
float APFloat::convertToFloat() const {
  if (&getSemantics() ==
      (const toolchain::fltSemantics *)&APFloatBase::semIEEEsingle)
...
bool APFloatBase::isValidArbitraryFPFormat(StringRef Format) {
  ...
  return toolchain::is_contained(ValidFormats, Format);
}
