...
void toolchain::initSignalsOptions() {
...
/// Reads a file \p Filename written by toolchain-symbolizer containing function
...
collectAddressSymbols(void **AddressList, unsigned AddressCount,
  ...
  // Lines contains the output from toolchain-symbolizer, which should contain for
  ...
  if (const char *Path = getenv(LLVMSymbolizerPathEnv)) {
    ...
    if (!Parent.empty())
      LLVMSymbolizerPathOrErr =
          sys::findProgramByName("toolchain-symbolizer", Parent);
  }
...
static bool printSymbolizedStackTrace(StringRef Argv0, void **StackTrace,
  ...
  if (Argv0.contains("toolchain-symbolizer"))
    return false;
...
void sys::symbolizeAddresses(AddressSet &Addresses,
  ...
  if (AddressList.empty())
    return;
  toolchain::sort(AddressList);

  // Use toolchain-symbolizer tool to symbolize the stack traces. First look for it
  // alongside our binary, then in $PATH.
  ErrorOr<std::string> LLVMSymbolizerPathOrErr = getLLVMSymbolizerPath();
  if (!LLVMSymbolizerPathOrErr)
    report_fatal_error("Debugify origin stacktraces require toolchain-symbolizer");
  const std::string &LLVMSymbolizerPath = *LLVMSymbolizerPathOrErr;
