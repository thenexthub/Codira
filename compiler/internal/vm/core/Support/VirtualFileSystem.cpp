...
#include "vm/core/ADT/IntrusiveRefCntPtr.h"
...
#include "vm/core/Support/FileSystem/UniqueID.h"
#include "vm/core/Support/IOSandbox.h"
...
using toolchain::sys::fs::file_t;
using toolchain::sys::fs::file_status;
using toolchain::sys::fs::file_type;
using toolchain::sys::fs::kInvalidFile;
using toolchain::sys::fs::perms;
using toolchain::sys::fs::UniqueID;
...
FileSystem::getBufferForFile(const toolchain::Twine &Name, int64_t FileSize,
...
toolchain::ErrorOr<bool> FileSystem::equivalent(const Twine &A, const Twine &B) {
...
static bool pathHasTraversal(StringRef Path) {
  ...
  for (StringRef Comp : toolchain::make_range(path::begin(Path), path::end(Path)))
...
class RealFile : public File {
  ...
  RealFile(file_t RawFD, StringRef NewName, StringRef NewRealPathName)
      : FD(RawFD), S(NewName, {}, {}, {}, {}, {},
                     toolchain::sys::fs::file_type::status_error, {}),
...
/// sending absolute paths to toolchain::sys::fs:: functions.
/// A more principled approach would be to push this down a level, modelling
/// the working dir as an toolchain::sys::fs::WorkingDir or similar.
...
public:
  explicit RealFileSystem(bool LinkCWDToProcess) {
    if (!LinkCWDToProcess) {
      ...
      if (std::error_code EC = toolchain::sys::fs::current_path(PWD))
        WD = EC;
      else if (toolchain::sys::fs::real_path(PWD, RealPWD))
  ...
  toolchain::ErrorOr<std::string> getCurrentWorkingDirectory() const override;
...
private:
  ...
  std::optional<toolchain::ErrorOr<WorkingDirectory>> WD;
};
...
toolchain::ErrorOr<std::string> RealFileSystem::getCurrentWorkingDirectory() const {
  auto BypassSandbox = sys::sandbox::scopedDisable();
  ...
  SmallString<128> Dir;
  if (std::error_code EC = toolchain::sys::fs::current_path(Dir))
    return EC;
...
std::error_code RealFileSystem::setCurrentWorkingDirectory(const Twine &Path) {
  ...
  if (!WD)
    return toolchain::sys::fs::set_current_path(Path);

  SmallString<128> Absolute, Resolved, Storage;
  adjustPath(Path, Storage).toVector(Absolute);
  bool IsDir;
  if (auto Err = toolchain::sys::fs::is_directory(Absolute, IsDir))
    return Err;
  if (!IsDir)
    return std::make_error_code(std::errc::not_a_directory);
  if (auto Err = toolchain::sys::fs::real_path(Absolute, Resolved))
    return Err;
...
std::error_code RealFileSystem::isLocal(const Twine &Path, bool &Result) {
  ...
  SmallString<256> Storage;
  return toolchain::sys::fs::is_local(adjustPath(Path, Storage), Result);
}
...
std::error_code RealFileSystem::getRealPath(const Twine &Path,
  ...
  SmallString<256> Storage;
  return toolchain::sys::fs::real_path(adjustPath(Path, Storage), Output);
}
...
class RealFSDirIter : public toolchain::vfs::detail::DirIterImpl {
  toolchain::sys::fs::directory_iterator Iter;
...
public:
  ...
  std::error_code increment() override {
    ...
    std::error_code EC;
    Iter.increment(EC);
    CurrentEntry = (Iter == toolchain::sys::fs::directory_iterator())
                       ? directory_entry()
...
ErrorOr<Status> OverlayFileSystem::status(const Twine &Path) {
  ...
  for (iterator I = overlays_begin(), E = overlays_end(); I != E; ++I) {
    ...
    if (Status || Status.getError() != toolchain::errc::no_such_file_or_directory)
      return Status;
  }
...
ErrorOr<std::unique_ptr<File>>
OverlayFileSystem::openFileForRead(const toolchain::Twine &Path) {
  // FIXME: handle symlinks that cross file systems
  for (iterator I = overlays_begin(), E = overlays_end(); I != E; ++I) {
    auto Result = (*I)->openFileForRead(Path);
    if (Result || Result.getError() != toolchain::errc::no_such_file_or_directory)
      return Result;
  }
  return make_error_code(toolchain::errc::no_such_file_or_directory);
}

toolchain::ErrorOr<std::string>
...
toolchain::vfs::detail::DirIterImpl::~DirIterImpl() = default;

namespace {

/// Combines and deduplicates directory entries across multiple file systems.
class CombiningDirIterImpl : public toolchain::vfs::detail::DirIterImpl {
  using FileSystemPtr = toolchain::IntrusiveRefCntPtr<toolchain::vfs::FileSystem>;
  ...
  std::error_code incrementImpl(bool IsFirstTime) {
    while (true) {
      ...
      StringRef Name = toolchain::sys::path::filename(CurrentEntry.path());
...
public:
  InMemoryNode(toolchain::StringRef FileName, InMemoryNodeKind Kind)
      : Kind(Kind), FileName(std::string(toolchain::sys::path::filename(FileName))) {
  }
...
public:
  InMemoryFile(Status Stat, std::unique_ptr<toolchain::MemoryBuffer> Buffer)
  ...
  toolchain::MemoryBuffer *getBuffer() const { return Buffer.get(); }
...
public:
  ...
  toolchain::ErrorOr<Status> status() override {
  ...
  getBuffer(const Twine &Name, int64_t FileSize, bool RequiresNullTerminator,
            bool IsVolatile) override {
    toolchain::MemoryBuffer *Buf = Node.getBuffer();
    return toolchain::MemoryBuffer::getMemBuffer(
...
static sys::fs::UniqueID getFileID(sys::fs::UniqueID Parent,
                                   ...
                                   toolchain::StringRef Contents) {
  return getUniqueID(toolchain::hash_combine(Parent.getFile(), Name, Contents));
}
static sys::fs::UniqueID getDirectoryID(sys::fs::UniqueID Parent,
                                        toolchain::StringRef Name) {
  return getUniqueID(toolchain::hash_combine(Parent.getFile(), Name));
}
...
Status detail::NewInMemoryNodeInfo::makeStatus() const {
  ...
  return Status(Path, UID, toolchain::sys::toTimePoint(ModificationTime), User,
                Group, Buffer ? Buffer->getBufferSize() : 0, Type, Perms);
}

InMemoryFileSystem::InMemoryFileSystem(bool UseNormalizedPaths)
    : Root(new detail::InMemoryDirectory(
          Status("", getDirectoryID(toolchain::sys::fs::UniqueID(), ""),
                 toolchain::sys::TimePoint<>(), 0, 0, 0,
                 toolchain::sys::fs::file_type::directory_file,
                 toolchain::sys::fs::perms::all_all))),
...
bool InMemoryFileSystem::addFile(const Twine &P, time_t ModificationTime,
                                 ...
                                 std::optional<toolchain::sys::fs::file_type> Type,
                                 std::optional<toolchain::sys::fs::perms> Perms,
                                 MakeNodeFn MakeNode) {
  ...
  if (useNormalizedPaths())
    toolchain::sys::path::remove_dots(Path, /*remove_dot_dot=*/true);

  if (Path.empty())
    return false;

  detail::InMemoryDirectory *Dir = Root.get();
  auto I = toolchain::sys::path::begin(Path), E = sys::path::end(Path);
  ...
  while (true) {
    ...
    if (!Node) {
      ...
      Status Stat(
          ...
          toolchain::sys::toTimePoint(ModificationTime), ResolvedUser, ResolvedGroup,
...
bool InMemoryFileSystem::addFileNoOwn(
    ...
    const toolchain::MemoryBufferRef &Buffer, std::optional<uint32_t> User,
    std::optional<uint32_t> Group, std::optional<toolchain::sys::fs::file_type> Type,
    std::optional<toolchain::sys::fs::perms> Perms) {
  return addFile(P, ModificationTime, toolchain::MemoryBuffer::getMemBuffer(Buffer),
...
InMemoryFileSystem::lookupNode(const Twine &P, bool FollowFinalSymlink,
  ...
  auto I = toolchain::sys::path::begin(Path), E = toolchain::sys::path::end(Path);
  while (true) {
...
toolchain::ErrorOr<Status> InMemoryFileSystem::status(const Twine &Path) {
...
toolchain::ErrorOr<std::unique_ptr<File>>
InMemoryFileSystem::openFileForRead(const Twine &Path) {
  ...
  return make_error_code(toolchain::errc::invalid_argument);
}

/// Adaptor from InMemoryDir::iterator to directory_iterator.
class InMemoryFileSystem::DirIterator : public toolchain::vfs::detail::DirIterImpl {
  ...
  void setCurrentEntry() {
    if (I != E) {
      ...
      toolchain::sys::path::append(Path, I->second->getFileName());
...
directory_iterator InMemoryFileSystem::dir_begin(const Twine &Dir,
  ...
  EC = make_error_code(toolchain::errc::not_a_directory);
  return directory_iterator(std::make_shared<DirIterator>());
}
...
std::error_code InMemoryFileSystem::getRealPath(const Twine &Path,
  ...
  if (auto EC = makeAbsolute(Output))
    return EC;
  toolchain::sys::path::remove_dots(Output, /*remove_dot_dot=*/true);
  return {};
}
...
static toolchain::sys::path::Style getExistingStyle(toolchain::StringRef Path) {
  // Detect the path style in use by checking the first separator.
  toolchain::sys::path::Style style = toolchain::sys::path::Style::native;
  const size_t n = Path.find_first_of("/\\");
  // Can't distinguish between posix and windows_slash here.
  if (n != static_cast<size_t>(-1))
    style = (Path[n] == '/') ? toolchain::sys::path::Style::posix
                             : toolchain::sys::path::Style::windows_backslash;
  return style;
}

/// Removes leading "./" as well as path components like ".." and ".".
static toolchain::SmallString<256> canonicalize(toolchain::StringRef Path) {
  // First detect the path style in use by checking the first separator.
  toolchain::sys::path::Style style = getExistingStyle(Path);

  // Now remove the dots.  Explicitly specifying the path style prevents the
  // direction of the slashes from changing.
  toolchain::SmallString<256> result =
      toolchain::sys::path::remove_leading_dotslash(Path, style);
  toolchain::sys::path::remove_dots(result, /*remove_dot_dot=*/true, style);
  return result;
}
...
static bool isFileNotFound(std::error_code EC,
  ...
  if (E && !isa<RedirectingFileSystem::DirectoryRemapEntry>(E))
    return false;
  return EC == toolchain::errc::no_such_file_or_directory;
}
...
/// directory entries.
class toolchain::vfs::RedirectingFSDirIterImpl
    : public toolchain::vfs::detail::DirIterImpl {
  std::string Dir;
  ...
  std::error_code incrementImpl(bool IsFirstTime) {
    ...
    if (Current != End) {
      ...
      toolchain::sys::path::append(PathStr, (*Current)->getName());
      sys::fs::file_type Type = sys::fs::file_type::type_unknown;
...
class RedirectingFSDirRemapIterImpl : public toolchain::vfs::detail::DirIterImpl {
  std::string Dir;
  toolchain::sys::path::Style DirStyle;
  toolchain::vfs::directory_iterator ExternalIter;

public:
  RedirectingFSDirRemapIterImpl(std::string DirPath,
                                toolchain::vfs::directory_iterator ExtIter)
      : Dir(std::move(DirPath)), DirStyle(getExistingStyle(Dir)),
        ExternalIter(ExtIter) {
    if (ExternalIter != toolchain::vfs::directory_iterator())
      setCurrentEntry();
  }

  void setCurrentEntry() {
    StringRef ExternalPath = ExternalIter->path();
    toolchain::sys::path::Style ExternalStyle = getExistingStyle(ExternalPath);
    StringRef File = toolchain::sys::path::filename(ExternalPath, ExternalStyle);

    SmallString<128> NewPath(Dir);
    toolchain::sys::path::append(NewPath, DirStyle, File);
  ...
  std::error_code increment() override {
    ...
    if (!EC && ExternalIter != toolchain::vfs::directory_iterator())
      setCurrentEntry();
    else
      CurrentEntry = directory_entry();
    return EC;
...
std::error_code RedirectingFileSystem::makeAbsolute(SmallVectorImpl<char> &Path) const {
  ...
  if (toolchain::sys::path::is_absolute(Path, toolchain::sys::path::Style::posix) ||
      toolchain::sys::path::is_absolute(Path,
                                   toolchain::sys::path::Style::windows_backslash))
    // This covers windows absolute path with forward slash as well, as the
    // forward slashes are treated as path separation in toolchain::path
...
void RedirectingFileSystem::printEntry(raw_ostream &OS,
  ...
  case EK_Directory: {
    ...
    for (std::unique_ptr<Entry> &SubEntry :
         toolchain::make_range(DE->contents_begin(), DE->contents_end()))
...
class toolchain::vfs::RedirectingFileSystemParser {
  yaml::Stream &Stream;
...
std::unique_ptr<RedirectingFileSystem> RedirectingFileSystem::create(
    ...
    bool UseExternalNames, toolchain::IntrusiveRefCntPtr<FileSystem> ExternalFS) {
  std::unique_ptr<RedirectingFileSystem> FS(
      new RedirectingFileSystem(ExternalFS));
  ...
  for (auto &Mapping : toolchain::reverse(RemappedFiles)) {
    ...
    StringRef FromDirectory = toolchain::sys::path::parent_path(From);
    for (auto I = toolchain::sys::path::begin(FromDirectory),
              E = toolchain::sys::path::end(FromDirectory);
         I != E; ++I) {
    ...
    auto NewFile = std::make_unique<RedirectingFileSystem::FileEntry>(
        toolchain::sys::path::filename(From), To,
...
void RedirectingFileSystem::LookupResult::getPath(
    toolchain::SmallVectorImpl<char> &Result) const {
  Result.clear();
  for (Entry *Parent : Parents)
    toolchain::sys::path::append(Result, Parent->getName());
  toolchain::sys::path::append(Result, E->getName());
}
...
std::error_code RedirectingFileSystem::makeCanonicalForLookup(
  ...
  toolchain::SmallString<256> CanonicalPath =
...
RedirectingFileSystem::lookupPath(StringRef Path) const {
  toolchain::SmallString<128> CanonicalPath(Path);
  ...
  toolchain::SmallVector<Entry *, 32> Entries;
  for (const auto &Root : Roots) {
    ...
    if (Result.getError() != toolchain::errc::no_such_file_or_directory)
      return Result;
  }
  return make_error_code(toolchain::errc::no_such_file_or_directory);
}
...
RedirectingFileSystem::lookupPathImpl(
    ...
    toolchain::SmallVectorImpl<Entry *> &Entries) const {
  ...
  if (isa<RedirectingFileSystem::FileEntry>(From))
    return make_error_code(toolchain::errc::not_a_directory);
...
public:
  ...
  ErrorOr<std::unique_ptr<toolchain::MemoryBuffer>>
...
RedirectingFileSystem::getRealPath(const Twine &OriginalPath,
  ...
  return toolchain::errc::invalid_argument;
}
...
static void getVFSEntries(RedirectingFileSystem::Entry *SrcE,
  ...
  if (Kind == RedirectingFileSystem::EK_DirectoryRemap) {
    ...
    for (auto &Comp : Path)
      toolchain::sys::path::append(VPath, Comp);
    Entries.push_back(
...
public:
  JSONWriter(toolchain::raw_ostream &OS) : OS(OS) {}
...
void JSONWriter::startDirectory(StringRef Path) {
  ...
  OS.indent(Indent + 2) << "'name': \"" << toolchain::yaml::escape(Name) << "\",\n";
...
void JSONWriter::writeEntry(StringRef VPath, StringRef RPath) {
  ...
  OS.indent(Indent + 2) << "'name': \"" << toolchain::yaml::escape(VPath) << "\",\n";
  OS.indent(Indent + 2) << "'external-contents': \""
                        << toolchain::yaml::escape(RPath) << "\"\n";
  OS.indent(Indent) << "}";
}
...
void YAMLVFSWriter::write(toolchain::raw_ostream &OS) {
  toolchain::sort(Mappings, [](const YAMLVFSEntry &LHS, const YAMLVFSEntry &RHS) {
