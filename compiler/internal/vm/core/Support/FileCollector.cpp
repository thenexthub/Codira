...
#include "vm/core/Support/FileCollector.h"
...
toolchain::vfs::directory_iterator
FileCollector::addDirectoryImpl(const toolchain::Twine &Dir,
  ...
  if (EC)
    return It;
  addFile(Dir);
  for (; !EC && It != toolchain::vfs::directory_iterator(); It.increment(EC)) {
...
public:
  ...
  toolchain::vfs::directory_iterator dir_begin(const toolchain::Twine &Dir,
  ...
  std::error_code setCurrentWorkingDirectory(const toolchain::Twine &Path) override {
