...
if test -n "${LLVM_OS}" ; then
	wget -O - https://apt.toolchain.org/toolchain-snapshot.gpg.key | sudo apt-key add -
	${SUDO} apt-add-repository "deb http://apt.toolchain.org/${LLVM_OS}/ toolchain-toolchain-${LLVM_OS}-${LLVM_VER} main"
fi
echo ">>>>>"
echo ">>>>> APT: UPDATE.."
echo ">>>>>"
