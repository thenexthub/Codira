...
namespace client { namespace code_gen
{
    ...
    value::operator toolchain::Value*() const
    {
        if (is_lvalue_)
        {
    ...
    struct function::to_value
    {
        ...
        value operator()(toolchain::Value& v) const
        {
            return c->val(&v);
        }
    };
    ...
    function::operator toolchain::Function*() const
    {
        return f;
    }
    ...
    {
        toolchain::verifyFunction(*f);
    }

    value llvm_compiler::val(unsigned int x)
    {
        return value(
            toolchain::ConstantInt::get(context(), toolchain::APInt(int_size, x)),
            false, &llvm_builder);
    }
    ...
    {
        return value(
            toolchain::ConstantInt::get(context(), toolchain::APInt(1, x)),
            false, &llvm_builder);
    }

    value llvm_compiler::val(toolchain::Value* v)
    {
    ...
    namespace
    {
            ...
            toolchain::LLVMContext& context)
        {
            ...
            return builder.CreateAlloca(
                toolchain::Type::getIntNTy(context, int_size), 0, name);
        }
    }

    value llvm_compiler::var(char const* name)
    {
        toolchain::Function* f = llvm_builder.GetInsertBlock()->getParent();
        toolchain::IRBuilder<> builder(
            &f->getEntryBlock(),
            f->getEntryBlock().begin());

        toolchain::AllocaInst* alloca = builder.CreateAlloca(
            toolchain::Type::getIntNTy(context(), int_size), 0, name);
    ...
    {
        typedef toolchain::Value* result_type;
        toolchain::Value* operator()(value const& x) const
        {
            return x;
        }
    };
    ...
    template <typename C>
    toolchain::Value* llvm_compiler::call_impl(
        function callee,
        C const& args_)
    {
        ...
        BOOST_FOREACH(toolchain::Value* arg, args)
        {
            if (arg == 0)
                return 0;
        }
    ...
    {
        toolchain::Value* call = call_impl(
            callee,
    ...
    function llvm_compiler::declare_function(
      ...
      , std::size_t nargs)
    {
        toolchain::Type const* int_type =
            toolchain::Type::getIntNTy(context(), int_size);
        toolchain::Type const* void_type = toolchain::Type::getVoidTy(context());

        std::vector<toolchain::Type const*> ints(nargs, int_type);
        toolchain::Type const* return_type = void_return ? void_type : int_type;

        toolchain::FunctionType* function_type =
            toolchain::FunctionType::get(void_return ? void_type : int_type, ints, false);

        return function(toolchain::Function::Create(
                function_type, toolchain::Function::ExternalLinkage,
                name, vm.module()), this);
    }
    ...
    basic_block llvm_compiler::make_basic_block(
      ...
      , basic_block before)
    {
        return toolchain::BasicBlock::Create(context(), name, parent, before);
    }
    ...
    void llvm_compiler::init_fpm()
    {
        ...
        fpm.add(new toolchain::TargetData(*vm.execution_engine()->getTargetData()));
        // Provide basic AliasAnalysis support for GVN.
        fpm.add(toolchain::createBasicAliasAnalysisPass());
        // Promote allocas to registers.
        fpm.add(toolchain::createPromoteMemoryToRegisterPass());
        // Do simple "peephole" optimizations and bit-twiddling optzns.
        fpm.add(toolchain::createInstructionCombiningPass());
        // Reassociate expressions.
        fpm.add(toolchain::createReassociatePass());
        // Eliminate Common SubExpressions.
        fpm.add(toolchain::createGVNPass());
        // Simplify the control flow graph (deleting unreachable blocks, etc).
        fpm.add(toolchain::createCFGSimplificationPass());
