...
REM
REM Requires: a Rust toolchain (cargo/rustc) and an LLVM 14.x dev package
...
REM   LLVM_SYS_140_PREFIX   Path to the LLVM 14 install (headers/lib/bin).
REM                         Default: C:\llvm14\built
...
if "%LLVM_SYS_140_PREFIX%"=="" set "LLVM_SYS_140_PREFIX=C:\llvm14\built"
if not exist "%LLVM_SYS_140_PREFIX%\bin\llvm-config.exe" (
    echo ERROR: no LLVM 14 install found at "%LLVM_SYS_140_PREFIX%".
    echo Set LLVM_SYS_140_PREFIX to a directory containing an LLVM 14.x dev package.
    exit /b 1
)
